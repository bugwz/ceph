...
{
    if (mds) mds->handle_signal(signum);
}
...
int main(int argc, const char** argv)
{
    ...
    auto cct = global_init(NULL, args, CEPH_ENTITY_TYPE_MDS, CODE_ENVIRONMENT_DAEMON, 0);
    ...
    Messenger* msgr = Messenger::create(
        g_ceph_context, public_msgr_type, entity_name_t::MDS(-1), "mds", Messenger::get_random_nonce());
    if (!msgr) forker.exit(1);
    msgr->set_cluster_protocol(CEPH_MDS_PROTOCOL);

    cout << "starting " << g_conf()->name << " at " << msgr->get_myaddrs() << std::endl;
    ...
    msgr->set_policy(entity_name_t::TYPE_MDS, Messenger::Policy::lossless_peer(CEPH_FEATURE_UID));
    msgr->set_policy(entity_name_t::TYPE_CLIENT, Messenger::Policy::stateful_server(0));

    int r = msgr->bindv(addrs);
    if (r < 0) forker.exit(1);
    ...
    if (mc.build_initial_monmap() < 0) forker.exit(1);
    global_init_chdir(g_ceph_context);
