...
public:
    ...
    constexpr static std::size_t get_num() { return ceph_subsys_get_num(); }

    constexpr static std::size_t get_max_subsys_len() { return ceph_subsys_max_name_length(); }

    int get_log_level(unsigned subsys) const
    {
        if (subsys >= get_num()) subsys = 0;
    ...
    template<unsigned SubV, int LvlV> bool should_gather() const
    {
