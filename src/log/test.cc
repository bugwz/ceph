...
TEST(Log, ManyNoGather)
{
    ...
    for (int i = 0; i < many; i++) {
        int l = 10;
        if (subs.should_gather(1, l)) log.submit_entry(MutableEntry(1, 0));
    }
    log.flush();
    log.stop();
}
...
TEST(Log, ManyGather)
{
    ...
    for (int i = 0; i < many; i++) {
        int l = 10;
        if (subs.should_gather(1, l)) log.submit_entry(MutableEntry(l, 1));
    }
    log.flush();
    log.stop();
}
...
TEST(Log, TimeSwitch)
{
    ...
    for (auto i = 0U; i < 300; ++i) {
        ...
        log.submit_entry(std::move(e));
        if (i % 50) log.set_coarse_timestamps(coarse = !coarse);
    }
    log.flush();
    log.stop();
}
