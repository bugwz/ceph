...
struct taggedrep
{
    ...
    explicit operator uint64_t() { return count; }
};
...
public:
    ...
    time_point now() noexcept { return appropriate_now(); }

    void coarsen() { appropriate_now = coarse_now; }

    void refine() { appropriate_now = fine_now; }
    ...
    static timeval to_timeval(time_point t)
    {
        ...
        return {static_cast<time_t>(std::chrono::duration_cast<std::chrono::seconds>(ts).count()),
                static_cast<suseconds_t>(
                    std::chrono::duration_cast<std::chrono::microseconds>(ts % std::chrono::seconds(1)).count())};
#else
...
private:
    ...
    {
        return time_point(duration(_logclock::taggedrep(coarse_real_clock::now().time_since_epoch().count(), true)));
    }
    static time_point fine_now()
    {
        return time_point(duration(_logclock::taggedrep(real_clock::now().time_since_epoch().count(), false)));
    }
