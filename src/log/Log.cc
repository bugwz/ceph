...
void Log::start_graylog(const std::string& host, const uuid_d& fsid)
{
    std::scoped_lock lock(m_flush_mutex);
    if (!m_graylog.get()) {
        ...
        while ((size_t)(current - start) < len) {
            ...
            while (1) {
                ...
                if (rc == chunk) {
                     ...
                     *
                     * Write requests to a pipe or FIFO shall be handled in the same way as a regular file with the
