...
Log::~Log()
{
    ...
    if (m_fd >= 0) VOID_TEMP_FAILURE_RETRY(::close(m_fd));
}
    ...
    if (m_log_file.length()) {
        ...
        if (m_fd >= 0 && (m_uid || m_gid)) {
            if (::fchown(m_fd, m_uid, m_gid) < 0) {
                int e = errno;
                std::cerr << "failed to chown " << m_log_file << ": " << cpp_strerror(e) << std::endl;
            }
        }
    }
    else {
...
void Log::chown_log_file(uid_t uid, gid_t gid)
{
    ...
    if (m_fd >= 0) {
        ...
        if (r < 0) {
            r = -errno;
            std::cerr << "failed to chown " << m_log_file << ": " << cpp_strerror(r) << std::endl;
        }
    }
}
...
{
    std::scoped_lock lock(m_flush_mutex);
    if (!m_graylog.get()) m_graylog = std::make_shared<Graylog>(m_subs, "dlog");
}
...
void Log::submit_entry(Entry&& e)
{
    ...
    if (unlikely(m_inject_segv)) *(volatile int*)(0) = 0xdead;
...
void Log::_log_safe_write(std::string_view sv)
{
    ...
    if (r != m_fd_last_error) {
        if (r < 0) std::cerr << "problem writing to " << m_log_file << ": " << cpp_strerror(r) << std::endl;
        m_fd_last_error = r;
    }
}
...
template<typename T> static uint64_t tid_to_int(T tid)
{
...
void Log::dump_recent()
{
    ...
    for (const auto& p : m_subs->m_subsys) {
        _log_message(fmt::format("  {:2d}/{:2d} {}", p.log_level, p.gather_level, p.name), true);
    }
    _log_message(fmt::format("  {:2d}/{:2d} (syslog threshold)", m_syslog_log, m_syslog_crash), true);
    _log_message(fmt::format("  {:2d}/{:2d} (stderr threshold)", m_stderr_log, m_stderr_crash), true);
    ...
    for (const auto pthread_id : recent_pthread_ids) {
        ...
        _log_message(fmt::format("  {} / {}", tid_to_int(pthread_id), pthread_name), true);
    }
...
{
    return pthread_self() == m_queue_mutex_holder || pthread_self() == m_flush_mutex_holder;
}
