...
struct pidfh
{
    ...
    bool is_open() const { return !pf_path.empty() && pf_fd != -1; }
    void reset()
    {
        pf_fd = -1;
        pf_path.clear();
...
int pidfh::verify()
{
    ...
    if (pf_fd == -1) return -EINVAL;
    struct stat st;
    if (stat(pf_path.c_str(), &st) == -1) return -errno;
    if (st.st_dev != pf_dev || st.st_ino != pf_ino) return -ESTALE;
    return 0;
}

int pidfh::remove()
{
    if (pf_path.empty()) return 0;
    ...
    if (ret < 0) {
        std::cerr << __func__ << " lseek failed " << cpp_strerror(errno) << std::endl;
        return -errno;
    }
    ...
    ::close(pf_fd);
    if (res < 0) {
        std::cerr << __func__ << " safe_read failed " << cpp_strerror(-res) << std::endl;
        return res;
    }

    int a = atoi(buf);
    if (a != getpid()) {
        std::cerr << __func__ << " the pid found in the file is " << a << " which is different from getpid() "
                  << getpid() << std::endl;
        return -EDOM;
    }
    ret = ::unlink(pf_path.c_str());
    if (ret < 0) {
        std::cerr << __func__ << " unlink " << pf_path.c_str() << " failed " << cpp_strerror(errno) << std::endl;
        return -errno;
    }
    reset();
    return 0;
...
// to this file.
#ifndef _WIN32
    struct flock l = {.l_type = F_WRLCK, .l_whence = SEEK_SET, .l_start = 0, .l_len = 0};
    int r = ::fcntl(pf_fd, F_SETLK, &l);
    if (r < 0) {
        if (errno == EAGAIN || errno == EACCES) {
            derr << __func__ << ": failed to lock pidfile " << pf_path << " because another process locked it"
