...
// file.
static int parse_from_os_release(const char* file, const char* key, char* out)
{
...
void generate_crash_dump(char* base, const BackTrace& bt, std::map<std::string, std::string>* extra)
{
    if (g_ceph_context && g_ceph_context->_conf->crash_dir.size()) {
        ...
        snprintf(base, PATH_MAX, "%s/%s", g_ceph_context->_conf->crash_dir.c_str(), id.c_str());
...
static void handle_oneshot_fatal_signal(int signum)
{
    ...
    if (auto expected{NULL_TID}; !handler_tid.compare_exchange_strong(expected, ceph_gettid())) {
    ...
    // we got the SEGV.
    if (g_ceph_context && g_ceph_context->_log && !g_ceph_context->_log->is_inside_log_lock()) {
...
struct SignalHandler : public Thread
{
    ...
    ~SignalHandler() override { shutdown(); }
