...
static int chown_path(const std::string& pathname, const uid_t owner, const gid_t group, const std::string& uid_str,
                      const std::string& gid_str)
{
    ...
    if (r < 0) {
        r = -errno;
        cerr << "warning: unable to chown() " << pathname << " as " << uid_str << ":" << gid_str << ": "
...
void global_pre_init(const std::map<std::string, std::string>* defaults, std::vector<const char*>& args,
    ...
    if (ret == -EDOM) {
        ...
        if (!(flags & CINIT_FLAG_NO_DEFAULT_CONFIG_FILE)) {
            if (conf_file_list.length()) {
                cct->_log->flush();
                cerr << "global_init: unable to open config file from search list " << conf_file_list << std::endl;
                _exit(1);
            }
            else {
        ...
        cct->_log->flush();
        cerr << "global_init: error reading config file. " << conf.get_parse_error() << std::endl;
        _exit(1);
    }
                                              ...
                                              code_environment_t code_env, int flags, bool run_pre_init)
{
    ...
    if (g_conf()->log_flush_on_exit) g_ceph_context->_log->set_flush_on_exit();
    ...
    if (getuid() != 0) {
        ...
        if (g_conf()->setgroup.length()) {
            cerr << "ignoring --setgroup " << g_conf()->setgroup << " since I am not root" << std::endl;
        }
    }
        ...
        if (g_conf()->setuser.length()) {
            ...
            if (uid) {
                ...
                if (!p) {
                    cerr << "unable to look up user '" << g_conf()->setuser << "'" << std::endl;
                    exit(1);
                }
        ...
        if ((uid || gid) && g_conf()->setuser_match_path.length()) {
            ...
            if (r < 0) {
                cerr << "unable to stat setuser_match_path " << g_conf()->setuser_match_path << ": "
                     << cpp_strerror(errno) << std::endl;
                exit(1);
            }
            if ((uid && uid != st.st_uid) || (gid && gid != st.st_gid)) {
                cerr << "WARNING: will not setuid/gid: " << match_path << " owned by " << st.st_uid << ":" << st.st_gid
                     << " and not requested " << uid << ":" << gid << std::endl;
                uid = 0;
                gid = 0;
                uid_string.erase();
                gid_string.erase();
                ...
                priv_ss << "setuser_match_path " << match_path << " owned by " << st.st_uid << ":" << st.st_gid << ". ";
            }
        }
        ...
        if ((flags & CINIT_FLAG_DEFER_DROP_PRIVILEGES) == 0) {
            if (setgid(gid) != 0) {
                cerr << "unable to setgid " << gid << ": " << cpp_strerror(errno) << std::endl;
                exit(1);
            }
            if (setuid(uid) != 0) {
                cerr << "unable to setuid " << uid << ": " << cpp_strerror(errno) << std::endl;
                exit(1);
            }
            if (setenv("HOME", home_directory.c_str(), 1) != 0) {
                cerr << "warning: unable to set HOME to " << home_directory << ": " << cpp_strerror(errno) << std::endl;
            }
    ...
    if (!g_conf()->no_mon_config) {
        ...
        if (mc_bootstrap.get_monmap_and_config() < 0) {
            ...
            cerr << "failed to fetch mon config (--no-mon-config to skip)" << std::endl;
            _exit(1);
        }
        cp.stop();
    }
    ...
    if (g_conf()->run_dir.length() && code_env == CODE_ENVIRONMENT_DAEMON && !(flags & CINIT_FLAG_NO_DAEMON_ACTIONS)) {

        if (!fs::exists(g_conf()->run_dir.c_str())) {
            std::error_code ec;
            if (!fs::create_directory(g_conf()->run_dir, ec)) {
                cerr << "warning: unable to create " << g_conf()->run_dir << ec.message() << std::endl;
            }
            fs::permissions(g_conf()->run_dir.c_str(),
    ...
    if ((flags & CINIT_FLAG_DEFER_DROP_PRIVILEGES) &&
        ...
        g_ceph_context->_log->chown_log_file(g_ceph_context->get_set_uid(), g_ceph_context->get_set_gid());
    }
    ...
    if (code_env == CODE_ENVIRONMENT_DAEMON && !(flags & CINIT_FLAG_NO_DAEMON_ACTIONS)) output_ceph_version();
...
{
    if (g_code_env != CODE_ENVIRONMENT_DAEMON) return -1;

    const auto& conf = cct->_conf;
    if (!conf->daemonize) {

        if (pidfile_write(conf->pid_file) < 0) exit(1);

        if ((cct->get_init_flags() & CINIT_FLAG_DEFER_DROP_PRIVILEGES) && (cct->get_set_uid() || cct->get_set_gid())) {
            chown_path(conf->pid_file,
                       cct->get_set_uid(),
                       cct->get_set_gid(),
                       cct->get_set_uid_string(),
                       cct->get_set_gid_string());
        }
...
{
    if (global_init_prefork(cct) < 0) return;
...
int reopen_as_null(CephContext* cct, int fd)
{
    ...
    if (newfd < 0) {
        int err = errno;
        lderr(cct) << __func__ << " failed to open /dev/null: " << cpp_strerror(err) << dendl;
        return -1;
    }
    ...
    if (r < 0) {
        int err = errno;
        lderr(cct) << __func__ << " failed to dup2 " << fd << ": " << cpp_strerror(err) << dendl;
        return -1;
    }
...
{
    const auto& conf = cct->_conf;
    if (conf->chdir.empty()) return;
    if (::chdir(conf->chdir.c_str())) {
        int err = errno;
        derr << "global_init_chdir: failed to chdir to directory: '" << conf->chdir << "': " << cpp_strerror(err)
             << dendl;
    }
}
...
int global_init_preload_erasure_code(const CephContext* cct)
{
    ...
    for (auto i = plugins_list.begin(); i != plugins_list.end(); ++i) {
        ...
        if (plugin_name == "jerasure_generic" || plugin_name == "jerasure_sse3" || plugin_name == "jerasure_sse4" ||
            plugin_name == "jerasure_neon") {
            replacement = "jerasure";
        }
        else if (plugin_name == "shec_generic" || plugin_name == "shec_sse3" || plugin_name == "shec_sse4" ||
                 plugin_name == "shec_neon") {
            replacement = "shec";
        }

        if (replacement != "") {
            dout(0) << "WARNING: osd_erasure_code_plugins contains plugin " << plugin_name
