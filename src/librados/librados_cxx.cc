...
int librados::IoCtx::aio_operate(const std::string& oid, AioCompletion* c, librados::ObjectWriteOperation* o)
{
    object_t obj(oid);
    if (unlikely(!o->impl)) return -EINVAL;
    return io_ctx_impl->aio_operate(obj, &o->impl->o, c->pc, io_ctx_impl->snapc, 0);
}
int librados::IoCtx::aio_operate(const std::string& oid, AioCompletion* c, ObjectWriteOperation* o, int flags)
{
    object_t obj(oid);
    if (unlikely(!o->impl)) return -EINVAL;
    return io_ctx_impl->aio_operate(obj, &o->impl->o, c->pc, io_ctx_impl->snapc, translate_flags(flags));
}
                                 ...
                                 snap_t snap_seq, std::vector<snap_t>& snaps)
{
    ...
    return io_ctx_impl->aio_operate(obj, &o->impl->o, c->pc, snapc, 0);
}
                                 ...
                                 snap_t snap_seq, std::vector<snap_t>& snaps, const blkin_trace_info* trace_info)
{
    ...
    for (size_t i = 0; i < snaps.size(); ++i) snv[i] = snaps[i];
    SnapContext snapc(snap_seq, snv);
    return io_ctx_impl->aio_operate(obj, &o->impl->o, c->pc, snapc, 0, trace_info);
}
                                 ...
                                 const blkin_trace_info* trace_info)
{
    ...
    for (size_t i = 0; i < snaps.size(); ++i) snv[i] = snaps[i];
    SnapContext snapc(snap_seq, snv);
    return io_ctx_impl->aio_operate(obj, &o->impl->o, c->pc, snapc, translate_flags(flags), trace_info);
}
