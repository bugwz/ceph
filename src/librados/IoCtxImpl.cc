...
struct CB_notify_Finish
{
    ...
    CB_notify_Finish(CephContext* _cct, Context* _ctx, Objecter* _objecter, Objecter::LingerOp* _linger_op,
                     bufferlist* _preply_bl, char** _preply_buf, size_t* _preply_buf_len)
        : cct(_cct)
        , ctx(_ctx)
        , objecter(_objecter)
        , linger_op(_linger_op)
        , preply_bl(_preply_bl)
    ...
    void operator()(bs::error_code ec, bufferlist&& reply_bl)
    {
        ...
        if (preply_buf_len) *preply_buf_len = reply_bl.length();
        if (preply_bl) *preply_bl = std::move(reply_bl);
...
struct CB_aio_linger_cancel
{
    ...
    void operator()() { objecter->linger_cancel(linger_op); }
};
...
struct C_aio_linger_Complete : public Context
{
    ...
    {
        if (cancel || r < 0)
            boost::asio::defer(c->io->client->finish_strand, CB_aio_linger_cancel(c->io->objecter, linger_op));
...
struct C_aio_notify_Ack : public Context
{
    ...
    C_aio_notify_Ack(CephContext* _cct, C_aio_notify_Complete* _oncomplete)
        : cct(_cct)
...
struct C_aio_selfmanaged_snap_op_Complete : public Context
{
    ...
    C_aio_selfmanaged_snap_op_Complete(librados::RadosClient* client, librados::AioCompletionImpl* c)
        : client(client)
        , c(c)
    {
        c->get();
...
struct C_aio_selfmanaged_snap_create_Complete : public C_aio_selfmanaged_snap_op_Complete
{
    ...
    C_aio_selfmanaged_snap_create_Complete(librados::RadosClient* client, librados::AioCompletionImpl* c,
                                           uint64_t* dest_snapid)
...
librados::IoCtxImpl::IoCtxImpl(RadosClient* c, Objecter* objecter, int64_t poolid, snapid_t s)
    : client(c)
    , poolid(poolid)
    , snap_seq(s)
...
int librados::IoCtxImpl::set_snap_write_context(snapid_t seq, vector<snapid_t>& snaps)
{
    ...
    n.seq = seq;
    n.snaps = snaps;
    if (!n.is_valid()) return -EINVAL;
    snapc = n;
    return 0;
}

int librados::IoCtxImpl::get_object_hash_position(const std::string& oid, uint32_t* hash_position)
{
...
int librados::IoCtxImpl::get_object_pg_hash_position(const std::string& oid, uint32_t* pg_hash_position)
{
...
void librados::IoCtxImpl::queue_aio_write(AioCompletionImpl* c)
{
    ...
    ldout(client->cct, 20) << "queue_aio_write " << this << " completion " << c << " write_seq " << aio_write_seq
                           << dendl;
...
void librados::IoCtxImpl::complete_aio_write(AioCompletionImpl* c)
{
    ...
    while (waiters != aio_write_waiters.end()) {
        if (!aio_write_list.empty() && aio_write_list.front()->aio_write_seq <= waiters->first) {
            ldout(client->cct, 20) << " next outstanding write is " << aio_write_list.front()->aio_write_seq
                                   << " <= waiter " << waiters->first << ", stopping" << dendl;
            break;
        }
        ldout(client->cct, 20) << " waking waiters on seq " << waiters->first << dendl;
        for (std::list<AioCompletionImpl*>::iterator it = waiters->second.begin(); it != waiters->second.end(); ++it) {
...
void librados::IoCtxImpl::flush_aio_writes()
{
    ...
    aio_write_cond.wait(l, [seq = aio_write_seq, this] {
        return (aio_write_list.empty() || aio_write_list.front()->aio_write_seq > seq);
    });
}
...
int librados::IoCtxImpl::selfmanaged_snap_create(uint64_t* psnapid)
{
    ...
    if (reply == 0) *psnapid = snapid;
    return reply;
}

void librados::IoCtxImpl::aio_selfmanaged_snap_create(uint64_t* snapid, AioCompletionImpl* c)
{
    C_aio_selfmanaged_snap_create_Complete* onfinish = new C_aio_selfmanaged_snap_create_Complete(client, c, snapid);
    objecter->allocate_selfmanaged_snap(poolid, &onfinish->snapid, onfinish);
}
...
int librados::IoCtxImpl::selfmanaged_snap_rollback_object(const object_t& oid, ::SnapContext& snapc, uint64_t snapid)
{
    int reply;
    ...
    op.rollback(snapid);
    objecter->mutate(oid, oloc, op, snapc, ceph::real_clock::now(), extra_op_flags, onack, NULL);
...
int librados::IoCtxImpl::selfmanaged_snap_remove(uint64_t snapid)
{
    ...
    ceph::condition_variable cond;
    bool done;
    objecter->delete_selfmanaged_snap(poolid, snapid_t(snapid), new C_SafeCond(mylock, cond, &done, &reply));
...
void librados::IoCtxImpl::aio_selfmanaged_snap_remove(uint64_t snapid, AioCompletionImpl* c)
{
...
uint32_t librados::IoCtxImpl::nlist_seek(Objecter::NListContext* context, uint32_t pos)
{
    context->list.clear();
    return objecter->list_nobjects_seek(context, pos);
}

uint32_t librados::IoCtxImpl::nlist_seek(Objecter::NListContext* context, const rados_object_list_cursor& cursor)
{
    context->list.clear();
...
{
    if (bl.length() > UINT_MAX / 2) return -E2BIG;
    ::ObjectOperation op;
    prepare_assert_ops(&op);
    op.write_full(bl);
    return operate(oid, &op, NULL);
...
int librados::IoCtxImpl::writesame(const object_t& oid, bufferlist& bl, size_t write_len, uint64_t off)
{
    if ((bl.length() > UINT_MAX / 2) || (write_len > UINT_MAX / 2)) return -E2BIG;
    if ((bl.length() == 0) || (write_len % bl.length())) return -EINVAL;
    ::ObjectOperation op;
    prepare_assert_ops(&op);
    bufferlist mybl;
...
{
    ceph::real_time ut = (pmtime ? *pmtime : ceph::real_clock::now());

    /* can't write to a snapshot */
    if (snap_seq != CEPH_NOSNAP) return -EROFS;
    ...
    Objecter::Op* objecter_op =
        objecter->prepare_mutate_op(oid, oloc, *o, snapc, ut, flags | extra_op_flags, oncommit, &ver);
...
int librados::IoCtxImpl::operate_read(const object_t& oid, ::ObjectOperation* o, bufferlist* pbl, int flags)
{
    ...
    ldout(client->cct, 10) << ceph_osd_op_name(op) << " oid=" << oid << " nspace=" << oloc.nspace << dendl;
    Objecter::Op* objecter_op =
        objecter->prepare_read_op(oid, oloc, *o, snap_seq, pbl, flags | extra_op_flags, onack, &ver);
    objecter->op_submit(objecter_op);
...
int librados::IoCtxImpl::aio_operate_read(const object_t& oid, ::ObjectOperation* o, AioCompletionImpl* c, int flags,
                                          bufferlist* pbl, const blkin_trace_info* trace_info)
{
    ...
    Objecter::Op* objecter_op = objecter->prepare_read_op(
        oid, oloc, *o, snap_seq, pbl, flags | extra_op_flags, oncomplete, &c->objver, nullptr, 0, &trace);
...
int librados::IoCtxImpl::aio_operate(const object_t& oid, ::ObjectOperation* o, AioCompletionImpl* c,
                                     const SnapContext& snap_context, int flags, const blkin_trace_info* trace_info)
{
    FUNCTRACE(client->cct);
    ...
    Objecter::Op* op = objecter->prepare_mutate_op(
        oid, oloc, *o, snap_context, ut, flags | extra_op_flags, oncomplete, &c->objver, osd_reqid_t(), &trace);
...
int librados::IoCtxImpl::aio_read(const object_t oid, AioCompletionImpl* c, bufferlist* pbl, size_t len, uint64_t off,
                                  uint64_t snapid, const blkin_trace_info* info)
{
    FUNCTRACE(client->cct);
    if (len > (size_t)INT_MAX) return -EDOM;
    ...
    ZTracer::Trace trace;
    if (info) trace.init("rados read", &objecter->trace_endpoint, info);

    Objecter::Op* o = objecter->prepare_read_op(
        oid, oloc, off, len, snapid, pbl, extra_op_flags, oncomplete, &c->objver, nullptr, 0, &trace);
    objecter->op_submit(o, &c->tid);
    return 0;
}

int librados::IoCtxImpl::aio_read(const object_t oid, AioCompletionImpl* c, char* buf, size_t len, uint64_t off,
                                  uint64_t snapid, const blkin_trace_info* info)
{
    FUNCTRACE(client->cct);
    if (len > (size_t)INT_MAX) return -EDOM;
    ...
    Objecter::Op* o = objecter->prepare_read_op(
        oid, oloc, off, len, snapid, &c->bl, extra_op_flags, oncomplete, &c->objver, nullptr, 0, &trace);
    objecter->op_submit(o, &c->tid);
    return 0;
}
...
public:
    ...
    explicit C_ObjectOperation(Context* c)
        : m_ctx(c)
    {}
    void finish(int r) override { m_ctx->complete(r); }

private:
    Context* m_ctx;
};

int librados::IoCtxImpl::aio_sparse_read(const object_t oid, AioCompletionImpl* c, std::map<uint64_t, uint64_t>* m,
                                         bufferlist* data_bl, size_t len, uint64_t off, uint64_t snapid)
{
    FUNCTRACE(client->cct);
    if (len > (size_t)INT_MAX) return -EDOM;
    ...
    Objecter::Op* o =
        objecter->prepare_read_op(oid, oloc, onack->m_ops, snapid, NULL, extra_op_flags, onack, &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
}

int librados::IoCtxImpl::aio_cmpext(const object_t& oid, AioCompletionImpl* c, uint64_t off, bufferlist& cmp_bl)
{
    if (cmp_bl.length() > UINT_MAX / 2) return -E2BIG;
    ...
    Objecter::Op* o = objecter->prepare_cmpext_op(oid, oloc, off, cmp_bl, snap_seq, extra_op_flags, onack, &c->objver);
    objecter->op_submit(o, &c->tid);
...
int librados::IoCtxImpl::aio_cmpext(const object_t& oid, AioCompletionImpl* c, const char* cmp_buf, size_t cmp_len,
                                    uint64_t off)
{
    if (cmp_len > UINT_MAX / 2) return -E2BIG;
    ...
    Objecter::Op* o =
        objecter->prepare_read_op(oid, oloc, onack->m_ops, snap_seq, NULL, extra_op_flags, onack, &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
}

int librados::IoCtxImpl::aio_write(const object_t& oid, AioCompletionImpl* c, const bufferlist& bl, size_t len,
                                   uint64_t off, const blkin_trace_info* info)
{
    FUNCTRACE(client->cct);
    auto ut = ceph::real_clock::now();
    ldout(client->cct, 20) << "aio_write " << oid << " " << off << "~" << len << " snapc=" << snapc
                           << " snap_seq=" << snap_seq << dendl;
    OID_EVENT_TRACE(oid.name.c_str(), "RADOS_WRITE_OP_BEGIN");
...
#endif
    ZTracer::Trace trace;
    if (info) trace.init("rados write", &objecter->trace_endpoint, info);

    c->io = this;
    queue_aio_write(c);

    Objecter::Op* o = objecter->prepare_write_op(
        oid, oloc, off, len, snapc, bl, ut, extra_op_flags, oncomplete, &c->objver, nullptr, 0, &trace);
    objecter->op_submit(o, &c->tid);

    return 0;
}

int librados::IoCtxImpl::aio_append(const object_t& oid, AioCompletionImpl* c, const bufferlist& bl, size_t len)
{
    FUNCTRACE(client->cct);
    auto ut = ceph::real_clock::now();
    ...
    Objecter::Op* o =
        objecter->prepare_append_op(oid, oloc, len, snapc, bl, ut, extra_op_flags, oncomplete, &c->objver);
    objecter->op_submit(o, &c->tid);

    return 0;
}

int librados::IoCtxImpl::aio_write_full(const object_t& oid, AioCompletionImpl* c, const bufferlist& bl)
{
    FUNCTRACE(client->cct);
    auto ut = ceph::real_clock::now();
    ...
    Objecter::Op* o = objecter->prepare_write_full_op(oid, oloc, snapc, bl, ut, extra_op_flags, oncomplete, &c->objver);
    objecter->op_submit(o, &c->tid);

    return 0;
}

int librados::IoCtxImpl::aio_writesame(const object_t& oid, AioCompletionImpl* c, const bufferlist& bl,
                                       size_t write_len, uint64_t off)
{
    ...
    Objecter::Op* o = objecter->prepare_writesame_op(
        oid, oloc, write_len, off, snapc, bl, ut, extra_op_flags, oncomplete, &c->objver);
    objecter->op_submit(o, &c->tid);
...
int librados::IoCtxImpl::aio_remove(const object_t& oid, AioCompletionImpl* c, int flags)
{
    ...
    Objecter::Op* o = objecter->prepare_remove_op(oid, oloc, snapc, ut, flags | extra_op_flags, oncomplete, &c->objver);
    objecter->op_submit(o, &c->tid);
...
int librados::IoCtxImpl::aio_stat(const object_t& oid, AioCompletionImpl* c, uint64_t* psize, time_t* pmtime)
{
    C_aio_stat_Ack* onack = new C_aio_stat_Ack(c, pmtime);
    c->is_read = true;
    c->io = this;
    Objecter::Op* o =
        objecter->prepare_stat_op(oid, oloc, snap_seq, psize, &onack->mtime, extra_op_flags, onack, &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
}

int librados::IoCtxImpl::aio_stat2(const object_t& oid, AioCompletionImpl* c, uint64_t* psize, struct timespec* pts)
{
...
int librados::IoCtxImpl::aio_getxattr(const object_t& oid, AioCompletionImpl* c, const char* name, bufferlist& bl)
{
    ::ObjectOperation rd;
...
int librados::IoCtxImpl::aio_rmxattr(const object_t& oid, AioCompletionImpl* c, const char* name)
{
    ::ObjectOperation op;
    prepare_assert_ops(&op);
    op.rmxattr(name);
...
int librados::IoCtxImpl::aio_setxattr(const object_t& oid, AioCompletionImpl* c, const char* name, bufferlist& bl)
{
    ::ObjectOperation op;
    prepare_assert_ops(&op);
    op.setxattr(name, bl);
...
{
    AioGetxattrsData(librados::AioCompletionImpl* c, map<string, bufferlist>* attrset, librados::RadosClient* _client)
        : user_completion(c)
...
static void aio_getxattrs_complete(rados_completion_t c, void* arg)
{
    ...
    if (rc >= 0) {
        for (map<string, bufferlist>::iterator p = cdata->result_attrset.begin(); p != cdata->result_attrset.end();
             ++p) {
...
int librados::IoCtxImpl::aio_getxattrs(const object_t& oid, AioCompletionImpl* c, map<std::string, bufferlist>& attrset)
{
...
int librados::IoCtxImpl::hit_set_list(uint32_t hash, AioCompletionImpl* c, std::list<std::pair<time_t, time_t> >* pls)
{
    Context* oncomplete = new C_aio_Complete(c);
    c->is_read = true;
    c->io = this;
    ...
    Objecter::Op* o = objecter->prepare_pg_read_op(hash, oloc, rd, NULL, extra_op_flags, oncomplete, NULL, NULL);
    objecter->op_submit(o, &c->tid);
    return 0;
}

int librados::IoCtxImpl::hit_set_get(uint32_t hash, AioCompletionImpl* c, time_t stamp, bufferlist* pbl)
{
    Context* oncomplete = new C_aio_Complete(c);
    c->is_read = true;
    c->io = this;
...
int librados::IoCtxImpl::get_inconsistent_objects(const pg_t& pg, const librados::object_id_t& start_after,
                                                  uint64_t max_to_get, AioCompletionImpl* c,
                                                  std::vector<inconsistent_obj_t>* objects, uint32_t* interval)
{
    ...
    Objecter::Op* o = objecter->prepare_pg_read_op(
        oloc.hash, oloc, op, nullptr, CEPH_OSD_FLAG_PGOP | extra_op_flags, oncomplete, nullptr, nullptr);
    objecter->op_submit(o, &c->tid);
    return 0;
}

int librados::IoCtxImpl::get_inconsistent_snapsets(const pg_t& pg, const librados::object_id_t& start_after,
                                                   uint64_t max_to_get, AioCompletionImpl* c,
...
int librados::IoCtxImpl::exec(const object_t& oid, const char* cls, const char* method, bufferlist& inbl,
                              bufferlist& outbl)
{
    ::ObjectOperation rd;
    prepare_assert_ops(&rd);
...
int librados::IoCtxImpl::aio_exec(const object_t& oid, AioCompletionImpl* c, const char* cls, const char* method,
                                  bufferlist& inbl, bufferlist* outbl)
{
    ...
    prepare_assert_ops(&rd);
    rd.call(cls, method, inbl);
    Objecter::Op* o = objecter->prepare_read_op(oid, oloc, rd, snap_seq, outbl, extra_op_flags, oncomplete, &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
}
                                  ...
                                  bufferlist& inbl, char* buf, size_t out_len)
{
    ...
    rd.call(cls, method, inbl);
    Objecter::Op* o =
        objecter->prepare_read_op(oid, oloc, rd, snap_seq, &c->bl, extra_op_flags, oncomplete, &c->objver);
    objecter->op_submit(o, &c->tid);
    return 0;
}

int librados::IoCtxImpl::read(const object_t& oid, bufferlist& bl, size_t len, uint64_t off)
{
    if (len > (size_t)INT_MAX) return -EDOM;
    OID_EVENT_TRACE(oid.name.c_str(), "RADOS_READ_OP_BEGIN");
...
int librados::IoCtxImpl::cmpext(const object_t& oid, uint64_t off, bufferlist& cmp_bl)
{
    if (cmp_bl.length() > UINT_MAX / 2) return -E2BIG;
...
int librados::IoCtxImpl::mapext(const object_t& oid, uint64_t off, size_t len, std::map<uint64_t, uint64_t>& m)
{
    bufferlist bl;
    ...
    objecter->mapext(oid, oloc, off, len, snap_seq, &bl, extra_op_flags, onack);
...
int librados::IoCtxImpl::sparse_read(const object_t& oid, std::map<uint64_t, uint64_t>& m, bufferlist& data_bl,
...
int librados::IoCtxImpl::checksum(const object_t& oid, uint8_t type, const bufferlist& init_value, size_t len,
...
int librados::IoCtxImpl::getxattr(const object_t& oid, const char* name, bufferlist& bl)
{
    ::ObjectOperation rd;
    prepare_assert_ops(&rd);
    rd.getxattr(name, &bl, NULL);
...
int librados::IoCtxImpl::setxattr(const object_t& oid, const char* name, bufferlist& bl)
{
    ::ObjectOperation op;
    prepare_assert_ops(&op);
    op.setxattr(name, bl);
...
int librados::IoCtxImpl::getxattrs(const object_t& oid, map<std::string, bufferlist>& attrset)
{
...
{
    ANNOTATE_BENIGN_RACE_SIZED(&last_objver, sizeof(last_objver), "IoCtxImpl last_objver");
    last_objver = ver;
}

namespace librados {
void intrusive_ptr_add_ref(IoCtxImpl* p)
{
    p->get();
}
void intrusive_ptr_release(IoCtxImpl* p)
{
    p->put();
}
}   // namespace librados
...
struct WatchInfo
{
    ...
    WatchInfo(librados::IoCtxImpl* io, object_t o, librados::WatchCtx* c, librados::WatchCtx2* c2)
        : ioctx(io)
        , oid(o)
        , ctx(c)
        , ctx2(c2)
    ...
    void handle_notify(uint64_t notify_id, uint64_t cookie, uint64_t notifier_id, bufferlist& bl)
    {
        ldout(ioctx->client->cct, 10) << __func__ << " " << notify_id << " cookie " << cookie << " notifier_id "
                                      << notifier_id << " len " << bl.length() << dendl;

        if (ctx2) ctx2->handle_notify(notify_id, cookie, notifier_id, bl);
        if (ctx) {
            ctx->notify(0, 0, bl);
    ...
    void handle_error(uint64_t cookie, int err)
    {
        ...
        if (ctx2) ctx2->handle_error(cookie, err);
    }

    void operator()(bs::error_code ec, uint64_t notify_id, uint64_t cookie, uint64_t notifier_id, bufferlist&& bl)
    {
        if (ec) {
...
struct InternalWatchInfo : public WatchInfo
{
    ...
    InternalWatchInfo(librados::IoCtxImpl* io, object_t o, librados::WatchCtx* c, librados::WatchCtx2* c2)
        : WatchInfo(io, o, c, c2)
        , ctx(c)
        , ctx2(c2)
    {}
...
int librados::IoCtxImpl::watch(const object_t& oid, uint64_t* handle, librados::WatchCtx* ctx,
                               librados::WatchCtx2* ctx2, bool internal)
{
    return watch(oid, handle, ctx, ctx2, 0, internal);
}

int librados::IoCtxImpl::watch(const object_t& oid, uint64_t* handle, librados::WatchCtx* ctx,
                               librados::WatchCtx2* ctx2, uint32_t timeout, bool internal)
{
    ::ObjectOperation wr;
    version_t objver;
    C_SaferCond onfinish;

    Objecter::LingerOp* linger_op = objecter->linger_register(oid, oloc, extra_op_flags);
    *handle = linger_op->get_cookie();
    ...
    bufferlist bl;
    objecter->linger_watch(linger_op, wr, snapc, ceph::real_clock::now(), bl, &onfinish, &objver);
...
int librados::IoCtxImpl::aio_watch(const object_t& oid, AioCompletionImpl* c, uint64_t* handle, librados::WatchCtx* ctx,
                                   librados::WatchCtx2* ctx2, bool internal)
{
                                   ...
                                   librados::WatchCtx2* ctx2, uint32_t timeout, bool internal)
{
    ...
    *handle = linger_op->get_cookie();
    ...
    wr.watch(*handle, CEPH_OSD_WATCH_OP_WATCH, timeout);
    bufferlist bl;
    objecter->linger_watch(linger_op, wr, snapc, ceph::real_clock::now(), bl, oncomplete, &c->objver);

    return 0;
}


int librados::IoCtxImpl::notify_ack(const object_t& oid, uint64_t notify_id, uint64_t cookie, bufferlist& bl)
{
    ::ObjectOperation rd;
    prepare_assert_ops(&rd);
...
int librados::IoCtxImpl::watch_check(uint64_t cookie)
{
    ...
    if (r)
        return 1 + std::chrono::duration_cast<std::chrono::milliseconds>(*r).count();
    else
...
int librados::IoCtxImpl::unwatch(uint64_t cookie)
{
    ...
    objecter->mutate(
        linger_op->target.base_oid, oloc, wr, snapc, ceph::real_clock::now(), extra_op_flags, &onfinish, &ver);
    objecter->linger_cancel(linger_op);
...
int librados::IoCtxImpl::aio_unwatch(uint64_t cookie, AioCompletionImpl* c)
{
    ...
    wr.watch(cookie, CEPH_OSD_WATCH_OP_UNWATCH);
    objecter->mutate(
        linger_op->target.base_oid, oloc, wr, snapc, ceph::real_clock::now(), extra_op_flags, oncomplete, &c->objver);
    return 0;
}

int librados::IoCtxImpl::notify(const object_t& oid, bufferlist& bl, uint64_t timeout_ms, bufferlist* preply_bl,
                                char** preply_buf, size_t* preply_buf_len)
{
    ...
    linger_op->on_notify_finish = Objecter::LingerOp::OpComp::create(
        ...
        CB_notify_Finish(client->cct, &notify_finish_cond, objecter, linger_op, preply_bl, preply_buf, preply_buf_len));
    uint32_t timeout = notify_timeout;
    if (timeout_ms) timeout = timeout_ms / 1000;
    ...
    C_SaferCond onack;
    version_t objver;
    objecter->linger_notify(linger_op, rd, snap_seq, inbl, NULL, &onack, &objver);
...
int librados::IoCtxImpl::aio_notify(const object_t& oid, AioCompletionImpl* c, bufferlist& bl, uint64_t timeout_ms,
                                    bufferlist* preply_bl, char** preply_buf, size_t* preply_buf_len)
{
    Objecter::LingerOp* linger_op = objecter->linger_register(oid, oloc, extra_op_flags);
    ...
    linger_op->on_notify_finish = Objecter::LingerOp::OpComp::create(
        objecter->service.get_executor(),
        CB_notify_Finish(client->cct, oncomplete, objecter, linger_op, preply_bl, preply_buf, preply_buf_len));
    ...
    // Issue RADOS op
    objecter->linger_notify(linger_op, rd, snap_seq, inbl, NULL, onack, &c->objver);
    return 0;
}
...
librados::IoCtxImpl::C_aio_stat_Ack::C_aio_stat_Ack(AioCompletionImpl* _c, time_t* pm)
    : c(_c)
    , pmtime(pm)
{
    ceph_assert(!c->io);
...
librados::IoCtxImpl::C_aio_stat2_Ack::C_aio_stat2_Ack(AioCompletionImpl* _c, struct timespec* pt)
    : c(_c)
    , pts(pt)
{
    ceph_assert(!c->io);
...
void librados::IoCtxImpl::C_aio_Complete::finish(int r)
{
    ...
    if (r) c->rval = r;   // This clears the error set in C_ObjectOperation_scrub_ls::finish()
    c->complete = true;
    c->cond.notify_all();
...
void librados::IoCtxImpl::object_list_slice(const hobject_t start, const hobject_t finish, const size_t n,
                                            const size_t m, hobject_t* split_start, hobject_t* split_finish)
{
    if (start.is_max()) {
    ...
    uint64_t finish_hash = finish.is_max() ? 0x100000000 : hobject_t::_reverse_bits(finish.get_hash());
    ...
    if (n == 0) {
        ...
        *split_start =
            hobject_t(object_t(), string(), CEPH_NOSNAP, hobject_t::_reverse_bits(rev_start), poolid, string());
    }
    ...
    if (n == m - 1)
        ...
        *split_finish =
            hobject_t(object_t(), string(), CEPH_NOSNAP, hobject_t::_reverse_bits(rev_finish), poolid, string());
}

int librados::IoCtxImpl::application_enable(const std::string& app_name, bool force)
{
...
void librados::IoCtxImpl::application_enable_async(const std::string& app_name, bool force, PoolAsyncCompletionImpl* c)
{
    ...
    if (!client->get_required_monitor_features().contains_all(ceph::features::mon::FEATURE_LUMINOUS)) {
        boost::asio::defer(client->finish_strand, [cb = CB_PoolAsync_Safe(c)]() mutable { cb(-EOPNOTSUPP); });
        return;
    }
    ...
    bufferlist inbl;
    client->mon_command_async(cmds, inbl, nullptr, nullptr, make_lambda_context(CB_PoolAsync_Safe(c)));
}
...
int librados::IoCtxImpl::application_metadata_get(const std::string& app_name, const std::string& key,
                                                  std::string* value)
{
    int r = 0;
    objecter->with_osdmap([&](const OSDMap& o) {
...
int librados::IoCtxImpl::application_metadata_set(const std::string& app_name, const std::string& key,
...
int librados::IoCtxImpl::application_metadata_remove(const std::string& app_name, const std::string& key)
{
    std::stringstream cmd;
    cmd << "{"
