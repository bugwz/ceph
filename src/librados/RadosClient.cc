...
int64_t librados::RadosClient::lookup_pool(const char* name)
{
    ...
    int64_t ret = objecter->with_osdmap(std::mem_fn(&OSDMap::lookup_pg_pool_name), name);
    if (-ENOENT == ret) {
...
int librados::RadosClient::pool_requires_alignment2(int64_t pool_id, bool* requires)
{
    if (!requires) return -EINVAL;
    ...
    return objecter->with_osdmap([requires, pool_id](const OSDMap& o) {
        ...
        *
            requires
        = o.get_pg_pool(pool_id)->requires_aligned_append();
        return 0;
    });
}
...
int librados::RadosClient::pool_required_alignment2(int64_t pool_id, uint64_t* alignment)
{
    if (!alignment) return -EINVAL;
...
int librados::RadosClient::connect()
{
    ...
    // already connected?
    if (state == CONNECTING) return -EINPROGRESS;
    if (state == CONNECTED) return -EISCONN;
    state = CONNECTING;
    ...
    err = monclient.authenticate(
        std::chrono::duration<double>(conf.get_val<std::chrono::seconds>("client_mount_timeout")).count());
    if (err) {
    ...
    mgrclient.set_mgr_optional(!get_required_monitor_features().contains_all(ceph::features::mon::FEATURE_LUMINOUS));
...
int librados::RadosClient::get_min_compatible_osd(int8_t* require_osd_release)
{
    ...
    objecter->with_osdmap(
        [require_osd_release](const OSDMap& o) { *require_osd_release = to_integer<int8_t>(o.require_osd_release); });
    return 0;
}

int librados::RadosClient::get_min_compatible_client(int8_t* min_compat_client, int8_t* require_min_compat_client)
{
    int r = wait_for_osdmap();
    if (r < 0) {
        return r;
    ...
    objecter->with_osdmap([min_compat_client, require_min_compat_client](const OSDMap& o) {
        *min_compat_client = to_integer<int8_t>(o.get_min_compat_client());
        *require_min_compat_client = to_integer<int8_t>(o.get_require_min_compat_client());
    });
    return 0;
}
...
bool librados::RadosClient::_dispatch(Message* m)
{
    ...
    case CEPH_MSG_MDS_MAP: m->put(); break;

    case MSG_LOG: handle_log(static_cast<MLog*>(m)); break;
...
int librados::RadosClient::get_pool_stats(std::list<string>& pools, map<string, ::pool_stat_t>* result, bool* pper_pool)
{
    bs::error_code ec;
    ...
    if (per_pool) *pper_pool = per_pool;
    if (result) result->insert(res.begin(), res.end());

    return 0;
}

int librados::RadosClient::pool_is_in_selfmanaged_snaps_mode(const std::string& pool)
{
    int r = wait_for_osdmap();
    if (r < 0) {
        return r;
    ...
    return objecter->with_osdmap([&pool](const OSDMap& osdmap) {
        ...
        return static_cast<int>(osdmap.get_pg_pool(poolid)->is_unmanaged_snaps_mode());
    });
}
...
int librados::RadosClient::get_fs_stats(ceph_statfs& stats)
{
    ...
    int ret = 0;
    {
        ...
        objecter->get_fs_stats(stats, boost::optional<int64_t>(), new C_SafeCond(mylock, cond, &done, &ret));
    }
    {
...
int librados::RadosClient::pool_create(string& name, int16_t crush_rule)
{
...
int librados::RadosClient::pool_create_async(string& name, PoolAsyncCompletionImpl* c, int16_t crush_rule)
{
    int r = wait_for_osdmap();
    if (r < 0) return r;
...
int librados::RadosClient::blocklist_add(const string& client_address, uint32_t expire_seconds)
{
    entity_addr_t addr;
...
int librados::RadosClient::mon_command(const vector<string>& cmd, const bufferlist& inbl, bufferlist* outbl,
                                       string* outs)
{
    C_SaferCond ctx;
...
void librados::RadosClient::mon_command_async(const vector<string>& cmd, const bufferlist& inbl, bufferlist* outbl,
                                              string* outs, Context* on_finish)
{
    std::lock_guard l{lock};
    monclient.start_mon_command(cmd,
                                inbl,
                                [outs, outbl, on_finish = std::unique_ptr<Context>(on_finish)](
                                    ...
                                    if (outbl) *outbl = std::move(b);
                                    if (on_finish) on_finish.release()->complete(ceph::from_error_code(e));
                                });
}

int librados::RadosClient::mgr_command(const vector<string>& cmd, const bufferlist& inbl, bufferlist* outbl,
                                       string* outs)
{
    std::lock_guard l(lock);
...
int librados::RadosClient::mgr_command(const string& name, const vector<string>& cmd, const bufferlist& inbl,
...
int librados::RadosClient::mon_command(int rank, const vector<string>& cmd, const bufferlist& inbl, bufferlist* outbl,
                                       string* outs)
{
    bs::error_code ec;
    auto&& [s, bl] = monclient.start_mon_command(rank, cmd, inbl, ca::use_blocked[ec]);
    if (outs) *outs = std::move(s);
...
int librados::RadosClient::mon_command(string name, const vector<string>& cmd, const bufferlist& inbl,
                                       bufferlist* outbl, string* outs)
{
    bs::error_code ec;
    auto&& [s, bl] = monclient.start_mon_command(name, cmd, inbl, ca::use_blocked[ec]);
    if (outs) *outs = std::move(s);
    if (outbl) *outbl = std::move(bl);
...
int librados::RadosClient::osd_command(int osd, vector<string>& cmd, const bufferlist& inbl, bufferlist* poutbl,
                                       string* prs)
{
    ceph_tid_t tid;

    if (osd < 0) return -EINVAL;


    // XXX do anything with tid?
    bs::error_code ec;
    auto [s, bl] = objecter->osd_command(osd, std::move(cmd), cb::list(inbl), &tid, ca::use_blocked[ec]);
    if (poutbl) *poutbl = std::move(bl);
    if (prs) *prs = std::move(s);
    return ceph::from_error_code(ec);
}

int librados::RadosClient::pg_command(pg_t pgid, vector<string>& cmd, const bufferlist& inbl, bufferlist* poutbl,
                                      string* prs)
{
    ceph_tid_t tid;
    bs::error_code ec;
    auto [s, bl] = objecter->pg_command(pgid, std::move(cmd), inbl, &tid, ca::use_blocked[ec]);
    if (poutbl) *poutbl = std::move(bl);
    if (prs) *prs = std::move(s);
    return ceph::from_error_code(ec);
}

int librados::RadosClient::monitor_log(const string& level, rados_log_callback_t cb, rados_log_callback2_t cb2,
                                       void* arg)
{
    std::lock_guard l(lock);
    ...
    if (log_cb || log_cb2) monclient.sub_unwant(log_watch);
...
int librados::RadosClient::service_daemon_register(const std::string& service,   ///< service name (e.g., 'rgw')
                                                   ...
                                                   const std::map<std::string, std::string>& metadata)
{
    ...
    if (service == "osd" || service == "mds" || service == "client" || service == "mon" || service == "mgr") {
...
int librados::RadosClient::service_daemon_update_status(std::map<std::string, std::string>&& status)
{
    if (state != CONNECTED) {
        return -ENOTCONN;
    }
...
{
    return monclient.with_monmap([](const MonMap& monmap) { return monmap.get_required_features(); });
}

int librados::RadosClient::get_inconsistent_pgs(int64_t pool_id, std::vector<std::string>* pgs)
{
    vector<string> cmd = {"{\"prefix\": \"pg ls\","
                          "\"pool\": " +
...
{
    static const char* config_keys[] = {"librados_thread_count", "rados_mon_op_timeout", nullptr};
    return config_keys;
}

void librados::RadosClient::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
