 ...
 */
void calc_snap_set_diff(CephContext* cct, const librados::snap_set_t& snap_set, librados::snap_t start,
                        librados::snap_t end, interval_set<uint64_t>* diff, uint64_t* end_size, bool* end_exists,
                        librados::snap_t* clone_end_snap_id, bool* whole_object)
{
    ldout(cct, 10) << "calc_snap_set_diff start " << start << " end " << end << ", snap_set seq " << snap_set.seq
                   << dendl;
    ...
    *whole_object = false;
    ...
    for (vector<librados::clone_info_t>::const_iterator r = snap_set.clones.begin(); r != snap_set.clones.end();) {
        ...
        ldout(cct, 20) << " clone " << r->cloneid << " snaps " << r->snaps << " -> [" << a << "," << b << "]"
        ...
        if (!saw_start) {
            if (start < a) {
                ...
                if (r->size) diff->insert(0, r->size);
                start_size = 0;
            }
            else {
        ...
        *end_size = r->size;
        ...
        if (max_size) diff_to_next.insert(0, max_size);
        for (vector<pair<uint64_t, uint64_t> >::const_iterator p = overlap->begin(); p != overlap->end(); ++p) {
