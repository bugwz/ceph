...
struct librados::AioCompletionImpl
{
    ...
    int wait_for_safe() { return wait_for_complete(); }
    int is_complete()
    {
        std::scoped_lock l{lock};
        return complete;
    }
    int is_safe() { return is_complete(); }
    ...
    int is_safe_and_cb() { return is_complete_and_cb(); }
...
struct CB_AioComplete
{
    ...
    void operator()()
    {
        ...
        if (cb_complete) cb_complete(c, cb_complete_arg);

        rados_callback_t cb_safe = c->callback_safe;
        void* cb_safe_arg = c->callback_safe_arg;
        if (cb_safe) cb_safe(c, cb_safe_arg);
