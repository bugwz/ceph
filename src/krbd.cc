...
#define DEFINE_UDEV_UPTR(what)          \
    ...
    using udev_##what##_uptr = std::unique_ptr<udev_##what, udev_##what##_deleter>;
...
struct krbd_spec
{
    ...
    krbd_spec(const char* pool_name, const char* nspace_name, const char* image_name, const char* snap_name)
        : pool_name(pool_name)
    ...
    {
        return pool_name == rhs.pool_name && nspace_name == rhs.nspace_name && image_name == rhs.image_name &&
...
{
    os << spec.pool_name << "/";
    if (!spec.nspace_name.empty()) os << spec.nspace_name << "/";
    os << spec.image_name;
    if (spec.snap_name != SNAP_HEAD_NAME) os << "@" << spec.snap_name;
    return os;
}
...
static std::optional<krbd_spec> spec_from_dev(udev_device* dev)
{
    ...
    if (!pool_name || !image_name || !snap_name) return std::nullopt;

    return std::make_optional<krbd_spec>(pool_name, nspace_name ?: "", image_name, snap_name);
}

static udev_device_uptr dev_from_list_entry(udev* udev, udev_list_entry* l)
{
    return udev_device_uptr(udev_device_new_from_syspath(udev, udev_list_entry_get_name(l)));
}
...
static int build_map_buf(CephContext* cct, const krbd_spec& spec, const string& options, string* pbuf)
{
    bool msgr2 = false;
    ...
    KeyRing keyring;
    auto auth_client_required = cct->_conf.get_val<std::string>("auth_client_required");
    if (auth_client_required != "none") {
        ...
        if (r == -ENOENT && keyfile.empty() && key.empty()) r = 0;
        if (r < 0) {
    ...
    if (keyring.get_secret(cct->_conf->name, secret)) {
        ...
        if (r >= 0) {
            if (r == 0) std::cerr << "rbd: warning: secret has length 0" << std::endl;
            oss << ",key=" << key_name;
        }
            ...
            std::cerr << "rbd: failed to add secret '" << key_name << "' to kernel" << std::endl;
            return r;
        }
    }
    ...
    if (!options.empty()) oss << "," << options;
    if (!spec.nspace_name.empty()) oss << ",_pool_ns=" << spec.nspace_name;

    oss << " " << spec.pool_name << " " << spec.image_name << " " << spec.snap_name;
 ...
 */
template<typename F>
static std::pair<int, bool> wait_for_mapping(int sysfs_r_fd, udev_monitor* mon, F udev_device_handler)
{
    struct pollfd fds[2];
...
{
public:
    UdevMapHandler(const krbd_spec* spec, std::string* pdevnode, std::string* majnum, std::string* minnum)
        : m_spec(spec)
        , m_pdevnode(pdevnode)
        , m_majnum(majnum)
        , m_minnum(minnum)
    ...
    bool operator()(udev_device_uptr dev)
    {
        ...
        if (!strcmp(udev_device_get_subsystem(dev.get()), "rbd")) {
            ...
            if (boost::starts_with(udev_device_get_devnode(dev.get()), DEVNODE_PREFIX)) {
            ...
            for (const auto& p : m_block_devs) {
                if (udev_device_get_devnode(p.get()) == m_devnode) {
                    ...
                    *m_minnum = udev_device_get_property_value(p.get(), "MINOR");
                    ceph_assert(*m_majnum == udev_device_get_sysattr_value(m_bus_dev.get(), "major"));
                    ceph_assert(!have_minor_attr() ||
                                *m_minnum == udev_device_get_sysattr_value(m_bus_dev.get(), "minor"));
                    return true;
                }
            }
            m_block_devs.clear();
...
static int do_map(krbd_ctx* ctx, const krbd_spec& spec, const string& buf, string* pname)
{
    ...
    udev_monitor_uptr mon(udev_monitor_new_from_netlink(ctx->udev, get_event_source(ctx)));
    if (!mon) return -ENOMEM;

    r = udev_monitor_filter_add_match_subsystem_devtype(mon.get(), "rbd", nullptr);
    if (r < 0) return r;

    r = udev_monitor_filter_add_match_subsystem_devtype(mon.get(), "block", "disk");
    if (r < 0) return r;

    r = udev_monitor_set_receive_buffer_size(mon.get(), UDEV_BUF_SIZE);
    if (r < 0) {
        std::cerr << "rbd: failed to set udev buffer size: " << cpp_strerror(r) << std::endl;
        /* not fatal */
    }
    ...
    if (pipe2(fds, O_NONBLOCK) < 0) return -errno;
    ...
    std::tie(r, mapped) = wait_for_mapping(fds[0], mon.get(), UdevMapHandler(&spec, pname, &majnum, &minnum));
    if (r < 0) {
        if (!mapped) {
    ...
    if (stat(pname->c_str(), &sb) < 0 || !S_ISBLK(sb.st_mode)) {
        std::cerr << "rbd: mapping succeeded but " << *pname << " is not accessible, is host /dev mounted?"
                  << std::endl;
        return -EINVAL;
    }
    if (stringify(major(sb.st_rdev)) != majnum || stringify(minor(sb.st_rdev)) != minnum) {
        std::cerr << "rbd: mapping succeeded but " << *pname << " (" << major(sb.st_rdev) << ":" << minor(sb.st_rdev)
                  << ") does not match expected " << majnum << ":" << minnum << std::endl;
        return -EINVAL;
    }
...
static int map_image(struct krbd_ctx* ctx, const krbd_spec& spec, const char* options, string* pname)
{
    string buf;
    int r;
    ...
    if (access("/sys/bus/rbd", F_OK) != 0) {
        ...
        if (module_has_param("rbd", "single_major")) module_options = "single_major=Y";

        r = module_load("rbd", module_options);
        if (r) {
            std::cerr << "rbd: failed to load rbd kernel module (" << r << ")" << std::endl;
            /*
...
retry:
    ...
    if (!enm) return -ENOMEM;

    r = udev_enumerate_add_match_subsystem(enm.get(), "rbd");
    if (r < 0) return r;

    r = udev_enumerate_add_match_sysattr(enm.get(), "major", stringify(major(devno)).c_str());
    if (r < 0) return r;

    if (have_minor_attr()) {
        r = udev_enumerate_add_match_sysattr(enm.get(), "minor", stringify(minor(devno)).c_str());
        if (r < 0) return r;
    }
...
static int __enumerate_devices(struct udev* udev, const krbd_spec& spec, bool match_nspace, udev_enumerate_uptr* penm)
{
    udev_enumerate_uptr enm;
    int r;
...
retry:
    ...
    r = udev_enumerate_add_match_sysattr(enm.get(), "pool", escape_glob(spec.pool_name).c_str());
    if (r < 0) return r;

    if (match_nspace) {
        r = udev_enumerate_add_match_sysattr(enm.get(), "pool_ns", escape_glob(spec.nspace_name).c_str());
    }
    else {
        /*
    ...
    r = udev_enumerate_add_match_sysattr(enm.get(), "name", escape_glob(spec.image_name).c_str());
    if (r < 0) return r;

    r = udev_enumerate_add_match_sysattr(enm.get(), "current_snap", escape_glob(spec.snap_name).c_str());
    if (r < 0) return r;
...
static int enumerate_devices(struct udev* udev, const krbd_spec& spec, udev_enumerate_uptr* penm)
{
    udev_enumerate_uptr enm;
    int r;
...
static int spec_to_devno_and_krbd_id(struct udev* udev, const krbd_spec& spec, dev_t* pdevno, string* pid)
{
    udev_enumerate_uptr enm;
    struct udev_list_entry* l;
    ...
    maj = strict_strtoll(udev_device_get_sysattr_value(dev.get(), "major"), 10, &err);
    if (!err.empty()) {
        std::cerr << "rbd: couldn't parse major: " << err << std::endl;
        return -EINVAL;
    }
    if (have_minor_attr()) {
        min = strict_strtoll(udev_device_get_sysattr_value(dev.get(), "minor"), 10, &err);
        if (!err.empty()) {
    ...
    if (udev_list_entry_get_next(l))
        std::cerr << "rbd: " << spec << ": mapped more than once, unmapping " << get_devnode(dev.get()) << " only"
                  << std::endl;
...
static int do_unmap(krbd_ctx* ctx, dev_t devno, const string& buf)
{
    ...
    auto unmapper = make_named_thread("unmapper", [&buf, sysfs_r_fd = fds[1], flags = ctx->flags]() {
        /*
        ...
        for (int tries = 0;; tries++) {
            ...
            if (sysfs_r == -EBUSY && tries < 2) {
                if (!tries) {
                     ...
                     */
                    std::string err = run_cmd("udevadm", "settle", "--timeout", "10", (char*)NULL);
                    if (!err.empty()) std::cerr << "rbd: " << err << std::endl;
                }
            }
            else {
                int r = safe_write(sysfs_r_fd, &sysfs_r, sizeof(sysfs_r));
    ...
    std::tie(r, unmapped) = wait_for_mapping(fds[0], mon.get(), UdevUnmapHandler(devno));
    if (r < 0) {
        if (!unmapped) {
            std::cerr << "rbd: sysfs write failed" << std::endl;
        }
...
static int unmap_image(struct krbd_ctx* ctx, const char* devnode, const char* options)
{
    struct stat sb;
    dev_t wholedevno = 0;
    std::string buf;
    ...
    if (r < 0) {
        std::cerr << "rbd: couldn't compute wholedevno: " << cpp_strerror(r) << std::endl;
        /*
    ...
    for (int tries = 0;; tries++) {
        ...
        if (r == -ENOENT && tries < 2) {
            ...
            if (r < 0) {
                if (r == -ENOENT) {
                    std::cerr << "rbd: '" << devnode << "' is not an rbd device" << std::endl;
                    r = -EINVAL;
                }
                return r;
            }
            if (tries) {
                std::cerr << "rbd: udev enumerate missed a device, tries = " << tries << std::endl;
            }
            break;
        }
    }
...
static int unmap_image(struct krbd_ctx* ctx, const krbd_spec& spec, const char* options)
{
    dev_t devno = 0;
    std::string buf;
    int r;
    ...
    for (int tries = 0;; tries++) {
        ...
        if (r == -ENOENT && tries < 2) {
            ...
            if (r < 0) {
                if (r == -ENOENT) {
                    std::cerr << "rbd: " << spec << ": not a mapped image or snapshot" << std::endl;
                    r = -EINVAL;
                }
                return r;
            }
...
static bool dump_one_image(Formatter* f, TextTable* tbl, struct udev_device* dev)
{
    ...
    if (f) {
        ...
        *tbl << id << spec->pool_name << spec->nspace_name << spec->image_name << spec->snap_name << devnode
             << TextTable::endrow;
    }
...
static int is_mapped_image(struct udev* udev, const krbd_spec& spec, string* pname)
{
    udev_enumerate_uptr enm;
    struct udev_list_entry* l;
    int r;
...
extern "C" int krbd_create_from_context(rados_config_t cct, uint32_t flags, struct krbd_ctx** pctx)
{
...
extern "C" int krbd_map(struct krbd_ctx* ctx, const char* pool_name, const char* nspace_name, const char* image_name,
...
extern "C" int krbd_unmap(struct krbd_ctx* ctx, const char* devnode, const char* options)
{
    return unmap_image(ctx, devnode, options);
}

extern "C" int krbd_unmap_by_spec(struct krbd_ctx* ctx, const char* pool_name, const char* nspace_name,
                                  const char* image_name, const char* snap_name, const char* options)
{
    krbd_spec spec(pool_name, nspace_name, image_name, snap_name);
...
extern "C" int krbd_is_mapped(struct krbd_ctx* ctx, const char* pool_name, const char* nspace_name,
                              const char* image_name, const char* snap_name, char** pdevnode)
{
    krbd_spec spec(pool_name, nspace_name, image_name, snap_name);
    string name;
    char* devnode;
