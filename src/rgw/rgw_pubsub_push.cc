...
template<typename EventType> std::string json_format_pubsub_event(const EventType& event)
{
    std::stringstream ss;
...
private:
    ...
    public:
        PostCR(const std::string& _post_data, RGWDataSyncEnv* _sync_env, const std::string& endpoint,
               ack_level_t _ack_level, bool verify_ssl)
...
private:
    ...
    public:
        NoAckPublishCR(CephContext* cct, const std::string& _topic, amqp::connection_ptr_t& _conn,
    ...
    public:
        AckPublishCR(CephContext* cct, const std::string& _topic, amqp::connection_ptr_t& _conn,
                     const std::string& _message)
            : RGWCoroutine(cct)
            , topic(_topic)
            , conn(_conn)
            ...
            reenter(this)
            {
                yield
                {
                    ...
                    const auto rc = amqp::publish_with_confirm(
                        conn, topic, message, std::bind(&AckPublishCR::request_complete, this, std::placeholders::_1));
                    if (rc < 0) {
        ...
        void* get_io_user_info() override { return nullptr; }
    };

public:
    RGWPubSubAMQPEndpoint(const std::string& _endpoint, const std::string& _topic, const RGWHTTPArgs& args,
                          CephContext* _cct)
        : cct(_cct)
        , endpoint(_endpoint)
        , topic(_topic)
        ...
        , conn(amqp::connect(endpoint, exchange, (ack_level == ack_level_t::Broker), get_verify_ssl(args),
    ...
    int send_to_completion_async(CephContext* cct, const rgw_pubsub_s3_event& event, optional_yield y) override
    {
        ...
        if (ack_level == ack_level_t::None) {
            ...
            // TODO: currently broker and routable are the same - this will require different flags but the same
            // mechanism note: dynamic allocation of Waiter is needed when this is invoked from a beast coroutine
...
private:
    ...
    public:
        NoAckPublishCR(CephContext* cct, const std::string& _topic, kafka::connection_ptr_t& _conn,
                       const std::string& _message)
            : RGWCoroutine(cct)
            , topic(_topic)
            , conn(_conn)
    ...
    public:
        AckPublishCR(CephContext* cct, const std::string& _topic, kafka::connection_ptr_t& _conn,
                     const std::string& _message)
            : RGWCoroutine(cct)
            , topic(_topic)
            , conn(_conn)
...
public:
    RGWPubSubKafkaEndpoint(const std::string& _endpoint, const std::string& _topic, const RGWHTTPArgs& args,
                           CephContext* _cct)
        : cct(_cct)
        , topic(_topic)
...
RGWPubSubEndpoint::Ptr RGWPubSubEndpoint::create(const std::string& endpoint, const std::string& topic,
                                                 const RGWHTTPArgs& args, CephContext* cct)
{
