...
public:
    ...
    virtual uint64_t get_new_req_id() override { return ceph::util::generate_random_number<uint64_t>(); }
...
public:
    StoreUser()
        : info()
    {}
    StoreUser(const rgw_user& _u)
        : info()
    {
        info.user_id = _u;
    }
    StoreUser(const RGWUserInfo& _i)
        : info(_i)
    {}
...
public:
    ...
    StoreBucket(User* _u)
        : owner(_u)
    {}
    StoreBucket(const rgw_bucket& _b)
    {
        ent.bucket = _b;
        info.bucket = _b;
    }
    StoreBucket(const RGWBucketEnt& _e)
        : ent(_e)
    {
    ...
    StoreBucket(const RGWBucketInfo& _i)
        : info(_i)
    {
    ...
    StoreBucket(const RGWBucketEnt& _e, User* _u)
        : ent(_e)
        , owner(_u)
    {
        info.bucket = ent.bucket;
    ...
    StoreBucket(const RGWBucketInfo& _i, User* _u)
        : info(_i)
        , owner(_u)
    {
        ent.bucket = info.bucket;
    ...
    virtual void set_owner(rgw::sal::User* _owner) override { owner = _owner; }
    ...
    virtual bool operator==(const Bucket& b) const override
    {
        ...
        return (info.bucket.tenant == sb.info.bucket.tenant) && (info.bucket.name == sb.info.bucket.name) &&
...
protected:
    virtual void set_ent(RGWBucketEnt& _ent)
    {
        ent = _ent;
        info.bucket = ent.bucket;
        info.placement_rule = ent.placement_rule;
...
public:
    StoreObject() = default;
    StoreObject(const rgw_obj_key& _k) { state.obj.key = _k; }
...
public:
    StoreMultipartUpload(Bucket* _bucket)
        : bucket(_bucket)
    {}
    ...
    {
        out << get_meta();
        if (!get_upload_id().empty()) out << ":" << get_upload_id();
    }
};
...
public:
    StoreMPSerializer()
        : locked(false)
    {}
    StoreMPSerializer(std::string _oid)
        : locked(false)
        , oid(_oid)
    {}
    virtual ~StoreMPSerializer() = default;

    virtual void clear_locked() override { locked = false; }
...
public:
    StoreLCSerializer() {}
    StoreLCSerializer(std::string _oid)
        : oid(_oid)
    {}
...
public:
    struct StoreLCHead : LCHead
    {
        ...
        StoreLCHead() = default;
        StoreLCHead(time_t _start_date, time_t _rollover_date, std::string& _marker)
            : start_date(_start_date)
            , shard_rollover_date(_rollover_date)
            , marker(_marker)
        {}
    ...
    virtual std::unique_ptr<LCEntry> get_entry() override { return std::make_unique<StoreLCEntry>(); }
...
public:
    StoreWriter(const DoutPrefixProvider* _dpp, optional_yield y)
        : dpp(_dpp)
    {}
