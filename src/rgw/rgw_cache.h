...
struct ObjectCacheInfo
{
    ...
    void decode(bufferlist::const_iterator& bl)
    {
        ...
        decode(xattrs, bl);
        decode(meta, bl);
        if (struct_v >= 2) decode(rm_xattrs, bl);
        if (struct_v >= 4) decode(epoch, bl);
        if (struct_v >= 5) decode(version, bl);
        DECODE_FINISH(bl);
    }
    void dump(Formatter* f) const;
...
{
    return m << "[op: " << cni.op << ", obj: " << cni.obj << ", ofs" << cni.ofs << ", ns" << cni.ns << "]";
}
...
public:
    ...
    {}
    ~ObjectCache();
    int get(const DoutPrefixProvider* dpp, const std::string& name, ObjectCacheInfo& bl, uint32_t mask,
    ...
    void put(const DoutPrefixProvider* dpp, const std::string& name, ObjectCacheInfo& bl,
             rgw_cache_entry_info* cache_info);
