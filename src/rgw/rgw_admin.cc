...
{
    switch (r) {
    case ERR_NO_SUCH_BUCKET: r = ENOENT; break;
    default: break;
    }
    return r;
}
...
void usage()
{
    ...
    cout << "  object reindex             reindex the object(s) indicated by --bucket and either --object or "
            "--objects-file\n";
    ...
    cout << "  orphans find               deprecated -- init and run search for leaked rados objects (use job-id, "
            "pool)\n";
    ...
    cout << "                           * the three 'orphans' sub-commands are now deprecated; consider using the "
    ...
    cout << "   --trim-delay-ms           time interval in msec to limit the frequency of sync error log entries "
            "trimming operations,\n";
    cout << "                             the trimming process will sleep the specified msec for every 1000 entries "
            "trimmed\n";
    ...
    cout << "   --max-read-ops            specify max requests per minute for READ ops per RGW (GET and HEAD request "
            "methods), 0 means unlimited\n";
    cout << "   --max-read-bytes          specify max bytes per minute for READ ops per RGW (GET and HEAD request "
            "methods), 0 means unlimited\n";
    cout << "   --max-write-ops           specify max requests per minute for WRITE ops per RGW (Not GET or HEAD "
            "request methods), 0 means unlimited\n";
    cout << "   --max-write-bytes         specify max bytes per minute for WRITE ops per RGW (Not GET or HEAD request "
            "methods), 0 means unlimited\n";
    ...
    cout << "   --orphan-stale-secs       num of seconds to wait before declaring an object to be an orphan (default: "
            "86400)\n";
    ...
    cout << "   --assume-role-policy-doc  the trust relationship policy document that grants an entity permission to "
            "assume the role\n";
    ...
    cout << "   --totp-window             the number of TOTP tokens that are checked before and after the current "
            "token when validating token\n";
    ...
    cout << "   --min-age-hours           minimum age of unlinked objects to consider for bucket check unlinked "
            "(default: 1)\n";
...
public:
    ...
    SimpleCmd(std::optional<Commands> cmds, std::optional<Aliases> aliases)
    {
        if (aliases) {
            add_aliases(*aliases);
        }
    ...
    void add_aliases(Aliases& aliases) { init_alias_map(aliases); }
    ...
    bool find_command(Container& args, std::any* opt_cmd, vector<string>* extra_args, string* error,
...
{
    if (type_str == "plain") return BIIndexType::Plain;
    if (type_str == "instance") return BIIndexType::Instance;
    if (type_str == "olh") return BIIndexType::OLH;
...
{
    if (strcasecmp(type_str.c_str(), "fifo") == 0) return log_type::fifo;
    if (strcasecmp(type_str.c_str(), "omap") == 0) return log_type::omap;
...
static void show_reshard_status(const list<cls_rgw_bucket_instance_entry>& status, Formatter* formatter)
{
...
static int init_bucket(rgw::sal::User* user, const rgw_bucket& b, std::unique_ptr<rgw::sal::Bucket>* bucket)
{
    return driver->get_bucket(dpp(), user, b, bucket, null_yield);
}

static int init_bucket(rgw::sal::User* user, const string& tenant_name, const string& bucket_name,
                       const string& bucket_id, std::unique_ptr<rgw::sal::Bucket>* bucket)
{
...
template<class T> static int read_decode_json(const string& infile, T& t)
{
    bufferlist bl;
...
template<class T, class K> static int read_decode_json(const string& infile, T& t, K* k)
{
    bufferlist bl;
    int ret = read_input(infile, bl);
    if (ret < 0) {
...
template<class T> static bool decode_dump(const char* field_name, bufferlist& bl, Formatter* f)
{
    T t;
...
bool set_ratelimit_info(RGWRateLimitInfo& ratelimit, OPT opt_cmd, int64_t max_read_ops, int64_t max_write_ops,
                        int64_t max_read_bytes, int64_t max_write_bytes, bool have_max_read_ops,
                        bool have_max_write_ops, bool have_max_read_bytes, bool have_max_write_bytes)
{
    bool ratelimit_configured = true;
    switch (opt_cmd) {
    case OPT::RATELIMIT_ENABLE:
    case OPT::GLOBAL_RATELIMIT_ENABLE: ratelimit.enabled = true; break;
    ...
    case OPT::GLOBAL_RATELIMIT_DISABLE: ratelimit.enabled = false; break;
    default: break;
    }
    return ratelimit_configured;
}

void set_quota_info(RGWQuotaInfo& quota, OPT opt_cmd, int64_t max_size, int64_t max_objects, bool have_max_size,
                    bool have_max_objects)
{
    ...
    case OPT::GLOBAL_QUOTA_DISABLE: quota.enabled = false; break;
    default: break;
    }
}

int set_bucket_quota(rgw::sal::Driver* driver, OPT opt_cmd, const string& tenant_name, const string& bucket_name,
                     int64_t max_size, int64_t max_objects, bool have_max_size, bool have_max_objects)
{
...
int set_bucket_ratelimit(rgw::sal::Driver* driver, OPT opt_cmd, const string& tenant_name, const string& bucket_name,
                         int64_t max_read_ops, int64_t max_write_ops, int64_t max_read_bytes, int64_t max_write_bytes,
                         bool have_max_read_ops, bool have_max_write_ops, bool have_max_read_bytes,
                         bool have_max_write_bytes)
{
    ...
    bool ratelimit_configured = set_ratelimit_info(ratelimit_info,
                                                   opt_cmd,
                                                   max_read_ops,
                                                   max_write_ops,
                                                   max_read_bytes,
...
int set_user_ratelimit(OPT opt_cmd, std::unique_ptr<rgw::sal::User>& user, int64_t max_read_ops, int64_t max_write_ops,
                       int64_t max_read_bytes, int64_t max_write_bytes, bool have_max_read_ops, bool have_max_write_ops,
...
int show_bucket_ratelimit(rgw::sal::Driver* driver, const string& tenant_name, const string& bucket_name,
                          Formatter* formatter)
{
    std::unique_ptr<rgw::sal::Bucket> bucket;
    int r = driver->get_bucket(dpp(), nullptr, tenant_name, bucket_name, &bucket, null_yield);
...
int set_user_bucket_quota(OPT opt_cmd, RGWUser& user, RGWUserAdminOpState& op_state, int64_t max_size,
                          int64_t max_objects, bool have_max_size, bool have_max_objects)
{
...
int check_min_obj_stripe_size(rgw::sal::Driver* driver, rgw::sal::Object* obj, uint64_t min_stripe_size,
                              bool* need_rewrite)
{
...
int check_obj_locator_underscore(rgw::sal::Object* obj, bool fix, bool remove_bad, Formatter* f)
{
    ...
    if ((needs_fixing || remove_bad) && fix) {
        ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->fix_head_obj_locator(
            dpp(), obj->get_bucket()->get_info(), needs_fixing, remove_bad, obj->get_key());
        if (ret < 0) {
...
int check_obj_tail_locator_underscore(RGWBucketInfo& bucket_info, rgw_obj_key& key, bool fix, Formatter* f)
{
    ...
    int ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->fix_tail_obj_locator(
        dpp(), bucket_info, key, fix, &needs_fixing, null_yield);
    if (ret < 0) {
...
static boost::optional<RGWRESTConn> get_remote_conn(rgw::sal::RadosStore* driver, const RGWZoneGroup& zonegroup,
...
static boost::optional<RGWRESTConn> get_remote_conn(rgw::sal::RadosStore* driver, const RGWPeriodMap& period_map,
                                                    const std::string& remote)
{
    boost::optional<RGWRESTConn> conn;
...
static int send_to_remote_gateway(RGWRESTConn* conn, req_info& info, bufferlist& in_data, JSONParser& parser)
{
    if (!conn) {
        return -EINVAL;
    }
...
static int send_to_url(const string& url, std::optional<string> opt_region, const string& access, const string& secret,
...
static int send_to_remote_or_url(RGWRESTConn* conn, const string& url, std::optional<string> opt_region,
                                 const string& access, const string& secret, req_info& info, bufferlist& in_data,
                                 JSONParser& parser)
{
    if (url.empty()) {
...
static int commit_period(rgw::sal::ConfigStore* cfgstore, RGWRealm& realm, rgw::sal::RealmWriter& realm_writer,
                         RGWPeriod& period, string remote, const string& url, std::optional<string> opt_region,
                         const string& access, const string& secret, bool force)
{
    ...
    if (driver->get_zone()->get_id() == master_zone) {
        ...
        int ret = cfgstore->read_period(dpp(), null_yield, realm.current_period, std::nullopt, current_period);
        if (ret < 0) {
            cerr << "failed to load current period: " << cpp_strerror(ret) << std::endl;
            return ret;
        }
        // the master zone can commit locally
        ret = rgw::commit_period(
            dpp(), null_yield, cfgstore, driver, realm, realm_writer, current_period, period, cerr, force);
        if (ret < 0) {
    ...
    if (!remote.empty()) {
        ...
        if (!conn) {
            cerr << "failed to find a zone or zonegroup for remote " << remote << std::endl;
            return -ENOENT;
        }
        remote_conn = &*conn;
    }
    ...
    if (ret < 0) {
        cerr << "Error storing committed period " << period.get_id() << ": " << cpp_strerror(ret) << std::endl;
        return ret;
    }
...
static int update_period(rgw::sal::ConfigStore* cfgstore, const string& realm_id, const string& realm_name,
                         const string& period_epoch, bool commit, const string& remote, const string& url,
                         std::optional<string> opt_region, const string& access, const string& secret,
                         Formatter* formatter, bool force)
{
    RGWRealm realm;
    std::unique_ptr<rgw::sal::RealmWriter> realm_writer;
    int ret = rgw::read_realm(dpp(), null_yield, cfgstore, realm_id, realm_name, realm, &realm_writer);
    if (ret < 0) {
    ...
    RGWPeriod period;
    ret = cfgstore->read_period(dpp(), null_yield, realm.current_period, epoch, period);
    if (ret < 0) {
        cerr << "failed to load current period: " << cpp_strerror(-ret) << std::endl;
        return ret;
    }
    ...
    if (commit) {
        ret = commit_period(cfgstore, realm, *realm_writer, period, remote, url, opt_region, access, secret, force);
        if (ret < 0) {
            cerr << "failed to commit period: " << cpp_strerror(-ret) << std::endl;
            return ret;
        }
...
static int init_bucket_for_sync(rgw::sal::User* user, const string& tenant, const string& bucket_name,
                                const string& bucket_id, std::unique_ptr<rgw::sal::Bucket>* bucket)
{
...
static int do_period_pull(rgw::sal::ConfigStore* cfgstore, RGWRESTConn* remote_conn, const string& url,
                          std::optional<string> opt_region, const string& access_key, const string& secret_key,
                          const string& realm_id, const string& realm_name, const string& period_id,
                          const string& period_epoch, RGWPeriod* period)
{
    RGWEnv env;
    req_info info(g_ceph_context, &env);
    info.method = "GET";
    ...
    if (!realm_id.empty()) params["realm_id"] = realm_id;
    if (!realm_name.empty()) params["realm_name"] = realm_name;
    if (!period_id.empty()) params["period_id"] = period_id;
    if (!period_epoch.empty()) params["epoch"] = period_epoch;

    bufferlist bl;
    JSONParser p;
    int ret = send_to_remote_or_url(remote_conn, url, opt_region, access_key, secret_key, info, bl, p);
    if (ret < 0) {
        cerr << "request failed: " << cpp_strerror(-ret) << std::endl;
        return ret;
    }
...
static void get_md_sync_status(list<string>& status)
{
    ...
    case rgw_meta_sync_info::StateInit: status_str = "init"; break;
    case rgw_meta_sync_info::StateBuildingFullSyncMaps: status_str = "preparing for full sync"; break;
    case rgw_meta_sync_info::StateSync: status_str = "syncing"; break;
    ...
    if (sync_status.sync_info.period != master_period) {
        status.push_back(string("master is on a different period: master_period=" + master_period +
                                " local_period=" + sync_status.sync_info.period));
    }
    else {
    ...
    if (total_behind == 0) {
        ...
        push_ss(ss, status) << "behind shards: "
                            << "[" << shards_behind_set << "]";
        if (oldest) {
            push_ss(ss, status) << "oldest incremental change not applied: " << oldest->second << " [" << oldest->first
                                << ']';
        }
    }
...
static void get_data_sync_status(const rgw_zone_id& source_zone, list<string>& status, int tab)
{
    ...
    string status_str;
    switch (sync_status.sync_info.state) {
    case rgw_data_sync_info::StateInit: status_str = "init"; break;
    case rgw_data_sync_info::StateBuildingFullSyncMaps: status_str = "preparing for full sync"; break;
    case rgw_data_sync_info::StateSync: status_str = "syncing"; break;
    default: status_str = "unknown";
    }
    ...
    if (total_behind == 0 && total_recovering == 0) {
        ...
        push_ss(ss, status, tab) << "behind shards: "
                                 << "[" << shards_behind_set << "]";
        if (oldest) {
            push_ss(ss, status, tab) << "oldest incremental change not applied: " << oldest->second << " ["
                                     << oldest->first << ']';
        }
    }
    ...
    if (total_recovering > 0) {
        ...
        push_ss(ss, status, tab) << "recovering shards: "
                                 << "[" << recovering_shards << "]";
    }
...
{
    auto features = rgw::zone_features::set{rgw::zone_features::supported.begin(), rgw::zone_features::supported.end()};
...
static void sync_status(Formatter* formatter)
{
    ...
    cout << std::setw(width) << "realm" << std::setw(1) << " " << zone->get_realm_id() << " (" << zone->get_realm_name()
         << ")" << std::endl;
    cout << std::setw(width) << "zonegroup" << std::setw(1) << " " << zonegroup.get_id() << " (" << zonegroup.get_name()
         << ")" << std::endl;
    cout << std::setw(width) << "zone" << std::setw(1) << " " << zone->get_id() << " (" << zone->get_name() << ")"
         << std::endl;
    cout << std::setw(width) << "current time" << std::setw(1) << " "
         << to_iso_8601(ceph::real_clock::now(), iso_8601_format::YMDhms) << std::endl;

    const auto& rzg = static_cast<const rgw::sal::RadosZoneGroup&>(zonegroup).get_group();
...
struct indented
{
    ...
    indented(int w, std::string_view header = "")
        : w(w)
        , header(header)
    {}
};
...
static int bucket_source_sync_status(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* driver, const RGWZone& zone,
                                     const RGWZone& source, RGWRESTConn* conn, const RGWBucketInfo& bucket_info,
                                     rgw_sync_bucket_pipe pipe, int width, std::ostream& out)
{
    ...
    r = rgw_read_remote_bilog_info(dpp, conn, source_bucket->get_key(), remote_info, remote_markers, null_yield);
    if (r < 0) {
...
static rgw_zone_id resolve_zone_id(const string& s)
{
    ...
    if (ret < 0) ret = driver->get_zone()->get_zonegroup().get_zone_by_name(s, &zone);
    if (ret < 0) return rgw_zone_id(s);
...
static int sync_info(std::optional<rgw_zone_id> opt_target_zone, std::optional<rgw_bucket> opt_bucket,
                     Formatter* formatter)
{
    ...
    if (eff_bucket) {
        ...
        if (ret < 0) {
            cerr << "ERROR: failed to init bucket sync policy handler: " << cpp_strerror(-ret) << " (ret=" << ret << ")"
                 << std::endl;
            return ret;
        }
    ...
    zone_policy_handler->reflect(dpp(),
                                 nullptr,
                                 nullptr,
                                 nullptr,
                                 nullptr,
    ...
    for (auto& hint_entity : hint_entities) {
        if (!hint_entity.zone || !hint_entity.bucket) {
        ...
        if (r < 0) {
            ldpp_dout(dpp(), 20) << "could not get bucket sync policy handler for hint bucket=" << hint_bucket
                                 << " ... skipping" << dendl;
            continue;
        }

        hint_bucket_handler->get_pipes(
            &resolved_dests, &resolved_sources, self_entity); /* flipping resolved dests and sources as these are
...
static int bucket_sync_info(rgw::sal::Driver* driver, const RGWBucketInfo& info, std::ostream& out)
{
    const rgw::sal::ZoneGroup& zonegroup = driver->get_zone()->get_zonegroup();
    rgw::sal::Zone* zone = driver->get_zone();
    ...
    if (!static_cast<rgw::sal::RadosStore*>(driver)->ctl()->bucket->bucket_imports_data(
            info.bucket, null_yield, dpp())) {
    ...
    if (r < 0) {
        ldpp_dout(dpp(), -1) << "ERROR: failed to get policy handler for bucket (" << info.bucket << "): r=" << r
...
static int bucket_sync_status(rgw::sal::Driver* driver, const RGWBucketInfo& info, const rgw_zone_id& source_zone_id,
                              std::optional<rgw_bucket>& opt_source_bucket, std::ostream& out)
{
    const rgw::sal::ZoneGroup& zonegroup = driver->get_zone()->get_zonegroup();
    rgw::sal::Zone* zone = driver->get_zone();
    constexpr int width = 15;
    ...
    if (!source_zone_id.empty()) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp(), -1) << "Source zone not found in zonegroup " << zonegroup.get_name() << dendl;
            return -EINVAL;
        }
    ...
    for (auto& zone_id : zone_ids) {
        ...
        if (z == static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->get_zonegroup().zones.end()) { /* should't
                                                                                                            happen */
            continue;
        }
        ...
        for (auto& entry : sources) {
            ...
            if (opt_source_bucket && pipe.source.bucket != opt_source_bucket) {
                continue;
            }
            if (pipe.source.zone.value_or(rgw_zone_id()) == z->second.id) {
                bucket_source_sync_status(dpp(),
...
static int check_pool_support_omap(const rgw_pool& pool)
{
    ...
    int ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->get_rados_handle()->ioctx_create(
        pool.to_str().c_str(), io_ctx);
    if (ret < 0) {
...
int check_reshard_bucket_params(rgw::sal::Driver* driver, const string& bucket_name, const string& tenant,
                                const string& bucket_id, bool num_shards_specified, int num_shards,
                                int yes_i_really_mean_it, std::unique_ptr<rgw::sal::Bucket>* bucket)
{
...
#define MAX_TOTP_SKEW_HOURS (24 * 7)
    ...
    while (time_ofs_abs < max_skew) {
        int rc = oath_totp_validate2(totp.seed_bin.c_str(),
                                     totp.seed_bin.length(),
                                     start_time,
                                     step_size,
                                     time_ofs,
                                     1,
        ...
        if (rc != OATH_INVALID_OTP) {
            rc = oath_totp_validate2(totp.seed_bin.c_str(),
                                     totp.seed_bin.length(),
                                     start_time,
                                     step_size,
...
{
    auto oid = RGWSyncErrorLogger::get_shard_oid(RGW_SYNC_ERROR_LOG_SHARD_PREFIX, shard_id);
    // call cls_log_trim() until it returns -ENODATA
    for (;;) {
        int ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->timelog.trim(
            dpp(), oid, {}, {}, {}, marker, nullptr, null_yield);
        if (ret == -ENODATA) {
            return 0;
        }
        if (ret < 0) {
...
template<class T> static bool require_opt(std::optional<T> opt, bool extra_check = true)
{
    if (!opt || !extra_check) {
        return false;
    }
...
template<class T> static bool require_non_empty_opt(std::optional<T> opt, bool extra_check = true)
{
...
template<class T> static void show_result(T& obj, Formatter* formatter, ostream& os)
{
...
void init_optional_bucket(std::optional<rgw_bucket>& opt_bucket, std::optional<string>& opt_tenant,
                          std::optional<string>& opt_bucket_name, std::optional<string>& opt_bucket_id)
{
...
public:
    SyncPolicyContext(rgw::sal::ConfigStore* cfgstore, std::optional<rgw_bucket> _bucket)
        : cfgstore(cfgstore)
        , b(std::move(_bucket))
    {}
    ...
    {
        int ret = rgw::read_zonegroup(
            dpp(), null_yield, cfgstore, zonegroup_id, zonegroup_name, zonegroup, &zonegroup_writer);
        if (ret < 0) {
    ...
    rgw_sync_policy_info& get_policy() { return *policy; }

    std::optional<rgw_user>& get_owner() { return owner; }
};
...
public:
    JSONFormatter_PrettyZone(bool pretty_format)
...
int main(int argc, const char** argv)
{
    ...
    auto cct = rgw_global_init(NULL, args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_UTILITY, 0);
    ...
    for (std::vector<const char*>::iterator i = args.begin(); i != args.end();) {
        if (ceph_argparse_double_dash(args, i)) {
            ...
            date = val;
            if (end_date.empty()) end_date = date;
        }
        ...
        else if (ceph_argparse_binary_flag(
                     args, i, &yes_i_really_mean_it, NULL, "--yes-i-really-mean-it", (char*)NULL)) {
            // do nothing
        }
            ...
            // do nothing
                     ...
                     args, i, &check_head_obj_locator, NULL, "--check-head-obj-locator", (char*)NULL)) {
            // do nothing
        }
            ...
            dump_keys = true;
                     ...
                     args, i, &placement_inline_data, NULL, "--placement-inline-data", (char*)NULL)) {
    ...
    if (args.empty()) {
        ...
        if (!extra_args.empty()) {
            ...
            case OPT::METADATA_LIST: metadata_key = extra_args[0]; break;
            default: break;
            }
        }
        ...
        std::set<OPT> raw_storage_ops_list = {OPT::ZONEGROUP_ADD,
        ...
        std::set<OPT> gc_ops_list = {OPT::GC_LIST,
                                     OPT::GC_PROCESS,
                                     OPT::OBJECT_RM,
        ...
        raw_storage_op =
            (raw_storage_ops_list.find(opt_cmd) != raw_storage_ops_list.end() || raw_period_update || raw_period_pull);
        ...
        if (raw_storage_op) {
            driver = DriverManager::get_raw_storage(dpp(), g_ceph_context, cfg);
        }
        else {
        ...
        init_optional_bucket(opt_bucket, opt_tenant, opt_bucket_name, opt_bucket_id);
        init_optional_bucket(opt_source_bucket, opt_source_tenant, opt_source_bucket_name, opt_source_bucket_id);
        init_optional_bucket(opt_dest_bucket, opt_dest_tenant, opt_dest_bucket_name, opt_dest_bucket_id);

        if (tenant.empty()) {
            tenant = user->get_tenant();
        }
        else {
            if (rgw::sal::User::empty(user) && opt_cmd != OPT::ROLE_CREATE && opt_cmd != OPT::ROLE_DELETE &&
                opt_cmd != OPT::ROLE_GET && opt_cmd != OPT::ROLE_TRUST_POLICY_MODIFY && opt_cmd != OPT::ROLE_LIST &&
                opt_cmd != OPT::ROLE_POLICY_PUT && opt_cmd != OPT::ROLE_POLICY_LIST &&
                opt_cmd != OPT::ROLE_POLICY_GET && opt_cmd != OPT::ROLE_POLICY_DELETE && opt_cmd != OPT::ROLE_UPDATE &&
                opt_cmd != OPT::RESHARD_ADD && opt_cmd != OPT::RESHARD_CANCEL && opt_cmd != OPT::RESHARD_STATUS &&
                opt_cmd != OPT::PUBSUB_TOPIC_LIST && opt_cmd != OPT::PUBSUB_NOTIFICATION_LIST &&
                opt_cmd != OPT::PUBSUB_TOPIC_GET && opt_cmd != OPT::PUBSUB_NOTIFICATION_GET &&
                opt_cmd != OPT::PUBSUB_TOPIC_RM && opt_cmd != OPT::PUBSUB_NOTIFICATION_RM) {
    ...
    {
        rgw_curl_setup() { rgw::curl::setup_curl(boost::none); }
        ~rgw_curl_setup() { rgw::curl::cleanup_curl(); }
    } curl_cleanup;
    ...
    if (raw_storage_op) {
        ...
        case OPT::PERIOD_GET:
        {
            ...
            if (staging) {
                RGWRealm realm;
                int ret = rgw::read_realm(dpp(), null_yield, cfgstore.get(), realm_id, realm_name, realm);
                if (ret < 0) {
                    cerr << "failed to load realm: " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
            ...
            RGWPeriod period;
            int ret = cfgstore->read_period(dpp(), null_yield, period_id, epoch, period);
            if (ret < 0) {
        ...
        case OPT::PERIOD_LIST:
        {
            ...
            do {
                int ret = cfgstore->list_period_ids(dpp(), null_yield, listing.next, period_ids, listing);
                if (ret < 0) {
        ...
        {
            int ret = update_period(cfgstore.get(),
                                    realm_id,
                                    realm_name,
                                    period_epoch,
                                    commit,
        ...
        case OPT::PERIOD_PULL:
        {
            ...
            if (url.empty()) {
                ...
                RGWPeriod current_period;
                ret = cfgstore->read_period(dpp(), null_yield, period_id, std::nullopt, current_period);
                if (ret < 0) {
                    cerr << "failed to load current period: " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
            ...
            RGWPeriod period;
            int ret = do_period_pull(cfgstore.get(),
                                     remote_conn,
                                     url,
                                     opt_region,
                                     access_key,
        ...
        case OPT::GLOBAL_RATELIMIT_DISABLE:
        {
            if (realm_id.empty()) {
                if (!realm_name.empty()) {
                    ...
                    int ret = cfgstore->read_realm_id(dpp(), null_yield, realm_name, realm_id);
                    if (ret < 0) {
                        cerr << "ERROR: failed to read realm for " << realm_name << ": " << cpp_strerror(-ret)
                             << std::endl;
                        return -ret;
                    }
                }
                    ...
                    int ret = cfgstore->read_default_realm_id(dpp(), null_yield, realm_id);
                    if (ret < 0 && ret != -ENOENT) {   // on ENOENT, use empty realm_id
                        cerr << "ERROR: failed to read default realm: " << cpp_strerror(-ret) << std::endl;
                        return -ret;
                    }
                }
            }
            ...
            int ret = cfgstore->read_period_config(dpp(), null_yield, realm_id, period_config);
            if (ret < 0 && ret != -ENOENT) {
                cerr << "ERROR: failed to read period config: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
            bool ratelimit_configured = true;
            formatter->open_object_section("period_config");
            if (ratelimit_scope == "bucket") {
                ratelimit_configured = set_ratelimit_info(period_config.bucket_ratelimit,
                                                          opt_cmd,
                                                          max_read_ops,
                                                          max_write_ops,
                                                          max_read_bytes,
                ...
                ratelimit_configured = set_ratelimit_info(period_config.user_ratelimit,
                                                          opt_cmd,
                                                          max_read_ops,
                                                          max_write_ops,
                                                          max_read_bytes,
                ...
                ratelimit_configured = set_ratelimit_info(period_config.anon_ratelimit,
                                                          opt_cmd,
                                                          max_read_ops,
                                                          max_write_ops,
                                                          max_read_bytes,
                ...
                cerr << "ERROR: invalid rate limit scope specification. Please specify "
                        "either --ratelimit-scope=bucket, or --ratelimit-scope=user or --ratelimit-scope=anonymous"
                     << std::endl;
                return EINVAL;
            }
            if (!ratelimit_configured) {
            ...
            if (opt_cmd != OPT::GLOBAL_RATELIMIT_GET) {
                ...
                constexpr bool exclusive = false;
                ret = cfgstore->write_period_config(dpp(), null_yield, exclusive, realm_id, period_config);
                if (ret < 0) {
                    cerr << "ERROR: failed to write period config: " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
                if (!realm_id.empty()) {
        ...
        case OPT::GLOBAL_QUOTA_DISABLE:
        {
            ...
            if (quota_scope == "bucket") {
                set_quota_info(
                    period_config.quota.bucket_quota, opt_cmd, max_size, max_objects, have_max_size, have_max_objects);
                encode_json("bucket quota", period_config.quota.bucket_quota, formatter.get());
            }
            else if (quota_scope == "user") {
                set_quota_info(
                    period_config.quota.user_quota, opt_cmd, max_size, max_objects, have_max_size, have_max_objects);
        ...
        case OPT::REALM_CREATE:
        {
            ...
            int ret = rgw::create_realm(dpp(), null_yield, cfgstore.get(), exclusive, realm);
            if (ret < 0) {
        ...
        case OPT::REALM_DELETE:
        {
            ...
            int ret = rgw::read_realm(dpp(), null_yield, cfgstore.get(), realm_id, realm_name, realm, &writer);
            if (ret < 0) {
                cerr << "failed to load realm: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
        ...
        case OPT::REALM_LIST:
        {
            ...
            do {
                ret = cfgstore->list_realm_names(dpp(), null_yield, listing.next, names, listing);
                if (ret < 0) {
        ...
        case OPT::REALM_LIST_PERIODS:
        {
            ...
            while (!period_id.empty()) {
                RGWPeriod period;
                ret = cfgstore->read_period(dpp(), null_yield, period_id, std::nullopt, period);
                if (ret < 0) {
                    cerr << "failed to load period id " << period_id << ": " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
        ...
        case OPT::REALM_RENAME:
        {
            ...
            cout << "Realm name updated. Note that this change only applies to "
                    ...
                    "on each of the realm's other clusters."
                 << std::endl;
        } break;
        case OPT::REALM_SET:
        {
            ...
            if (new_realm) {
                ...
                constexpr bool exclusive = true;
                ret = rgw::create_realm(dpp(), null_yield, cfgstore.get(), exclusive, realm);
                if (ret < 0) {
        ...
        case OPT::REALM_PULL:
        {
            ...
            map<string, string>& params = info.args.get_params();
            if (!realm_id.empty()) params["id"] = realm_id;
            if (!realm_name.empty()) params["name"] = realm_name;
            ...
            if (!current_period.empty()) {
                ...
                ret = do_period_pull(cfgstore.get(),
                                     nullptr,
                                     url,
                                     opt_region,
                                     access_key,
            ...
            ret = rgw::create_realm(dpp(), null_yield, cfgstore.get(), exclusive, realm);
            if (ret < 0) {
                cerr << "Error storing realm " << realm.get_id() << ": " << cpp_strerror(ret) << std::endl;
                return -ret;
            }
        ...
        case OPT::ZONEGROUP_ADD:
        {
            ...
            std::unique_ptr<rgw::sal::ZoneGroupWriter> zonegroup_writer;
            int ret = rgw::read_zonegroup(
                dpp(), null_yield, cfgstore.get(), zonegroup_id, zonegroup_name, zonegroup, &zonegroup_writer);
            if (ret < 0) {
                cerr << "failed to load zonegroup " << zonegroup_name << " id " << zonegroup_id << ": "
            ...
            ret = rgw::read_zone(dpp(), null_yield, cfgstore.get(), zone_id, zone_name, zone_params, &zone_writer);
            if (ret < 0) {
            ...
            if (ptier_type) {
                ...
                if (!sync_mgr->get_module(*ptier_type, nullptr)) {
                    ldpp_dout(dpp(), -1) << "ERROR: could not find sync module: " << *ptier_type
                                         << ",  valid sync modules: " << sync_mgr->get_registered_module_names()
                                         << dendl;
                    return EINVAL;
                }
            }
            ...
            if (enable_features.empty()) {   // enable all features by default
                enable_features.insert(rgw::zone_features::supported.begin(), rgw::zone_features::supported.end());
            }

            // add/update the public zone information stored in the zonegroup
            ret = rgw::add_zone_to_group(dpp(),
                                         zonegroup,
                                         zone_params,
                                         pis_master,
                                         pread_only,
            ...
            if (ret < 0) {
                cerr << "failed to write updated zonegroup " << zonegroup.get_name() << ": " << cpp_strerror(-ret)
                     << std::endl;
                return -ret;
            }
        ...
        case OPT::ZONEGROUP_CREATE:
        {
            ...
            for (const auto& feature : disable_features) {
                ...
                if (i == zonegroup.enabled_features.end()) {
                    ldout(cct, 1) << "WARNING: zone feature \"" << feature << "\" was not enabled in zonegroup "
                                  << zonegroup_name << dendl;
                    continue;
                }
            ...
            constexpr bool exclusive = true;
            ret = rgw::create_zonegroup(dpp(), null_yield, cfgstore.get(), exclusive, zonegroup);
            if (ret < 0) {
            ...
            if (set_default) {
                ret = rgw::set_default_zonegroup(dpp(), null_yield, cfgstore.get(), zonegroup);
                if (ret < 0) {
                    cerr << "failed to set zonegroup " << zonegroup_name << " as default: " << cpp_strerror(-ret)
                         << std::endl;
                }
            }
        ...
        case OPT::ZONEGROUP_DEFAULT:
        {
            ...
            RGWZoneGroup zonegroup;
            int ret = rgw::read_zonegroup(dpp(), null_yield, cfgstore.get(), zonegroup_id, zonegroup_name, zonegroup);
            if (ret < 0) {
                cerr << "failed to init zonegroup: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
        ...
        case OPT::ZONEGROUP_DELETE:
        {
            ...
            int ret = rgw::read_zonegroup(
                dpp(), null_yield, cfgstore.get(), zonegroup_id, zonegroup_name, zonegroup, &writer);
            if (ret < 0) {
        ...
        case OPT::ZONEGROUP_LIST:
        {
            ...
            int ret = rgw::read_zonegroup(dpp(), null_yield, cfgstore.get(), {}, {}, default_zonegroup);
            if (ret < 0 && ret != -ENOENT) {
            ...
            std::array<std::string, 1000> names;   // list in pages of 1000
            do {
                ret = cfgstore->list_zonegroup_names(dpp(), null_yield, listing.next, names, listing);
                if (ret < 0) {
        ...
        case OPT::ZONEGROUP_MODIFY:
        {
            ...
            if (!realm_id.empty()) {
                ...
                ret = cfgstore->read_realm_id(dpp(), null_yield, realm_name, zonegroup.realm_id);
                if (ret < 0) {
        ...
        case OPT::ZONEGROUP_SET:
        {
            ...
            for (const auto& feature : zonegroup.enabled_features) {
                if (!rgw::zone_features::supports(feature)) {
                    std::cerr << "ERROR: Unrecognized zonegroup feature \"" << feature << "\"" << std::endl;
                    return EINVAL;
                }
            }
            for (const auto& [name, zone] : zonegroup.zones) {
                ...
                for (const auto& feature : zone.supported_features) {
                    if (!rgw::zone_features::supports(feature)) {
                        std::cerr << "ERROR: Unrecognized zone feature \"" << feature << "\" in zone " << zone.name
                                  << std::endl;
                        return EINVAL;
                    }
                }
                ...
                for (const auto& feature : zonegroup.enabled_features) {
                    if (!zone.supports(feature)) {
                        std::cerr << "ERROR: Zone " << name << " does not support feature \"" << feature
        ...
        case OPT::ZONEGROUP_REMOVE:
        {
            ...
            if (zone_id.empty()) {
                ...
                if (zone_id.empty()) {
                    cerr << "zone name " << zone_name << " not found in zonegroup " << zonegroup.get_name()
                         << std::endl;
                    return ENOENT;
                }
            }
        ...
        case OPT::ZONEGROUP_PLACEMENT_DEFAULT:
        {
            ...
            if (!rule.storage_class.empty() && opt_storage_class && rule.storage_class != *opt_storage_class) {
            ...
            if (opt_cmd == OPT::ZONEGROUP_PLACEMENT_ADD || opt_cmd == OPT::ZONEGROUP_PLACEMENT_MODIFY) {
        ...
        case OPT::ZONE_CREATE:
        {
            ...
            constexpr bool exclusive = true;
            int ret = rgw::create_zone(dpp(), null_yield, cfgstore.get(), exclusive, zone_params);
            if (ret < 0) {
            ...
            if (zonegroup_writer) {
                ...
                if (ptier_type) {
                    ...
                    if (!sync_mgr->get_module(*ptier_type, nullptr)) {
                        ldpp_dout(dpp(), -1)
                            << "ERROR: could not find sync module: " << *ptier_type
                            << ",  valid sync modules: " << sync_mgr->get_registered_module_names() << dendl;
                        return EINVAL;
                    }
                }
            ...
            if (set_default) {
                ret = rgw::set_default_zone(dpp(), null_yield, cfgstore.get(), zone_params);
                if (ret < 0) {
        ...
        case OPT::ZONE_DEFAULT:
        {
            ...
            RGWZoneParams zone_params;
            int ret = rgw::read_zone(dpp(), null_yield, cfgstore.get(), zone_id, zone_name, zone_params);
            if (ret < 0) {
                cerr << "unable to load zone: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
        ...
        case OPT::ZONE_DELETE:
        {
            ...
            int ret = rgw::read_zone(dpp(), null_yield, cfgstore.get(), zone_id, zone_name, zone_params, &writer);
            if (ret < 0) {
                cerr << "failed to load zone: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }

            ret = rgw::delete_zone(dpp(), null_yield, cfgstore.get(), zone_params, *writer);
            if (ret < 0) {
                cerr << "failed to delete zone " << zone_params.get_name() << ": " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
        } break;
        case OPT::ZONE_GET:
        ...
        case OPT::ZONE_SET:
        {
            RGWZoneParams zone;
            std::unique_ptr<rgw::sal::ZoneWriter> writer;
            int ret = rgw::read_zone(dpp(), null_yield, cfgstore.get(), zone_id, zone_name, zone, &writer);
            if (ret < 0 && ret != -ENOENT) {
                cerr << "failed to load zone: " << cpp_strerror(ret) << std::endl;
                return -ret;
            }
            ...
            if (zone.realm_id.empty()) {
                RGWRealm realm;
                ret = rgw::read_realm(dpp(), null_yield, cfgstore.get(), realm_id, realm_name, realm);
                if (ret < 0 && ret != -ENOENT) {
                    cerr << "failed to load realm: " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
            ...
            if (!zone_name.empty() && !zone.get_name().empty() && zone.get_name() != zone_name) {
                cerr << "Error: zone name " << zone_name << " is different than the zone name " << zone.get_name()
                     << " in the provided json " << std::endl;
                return EINVAL;
            }
            ...
            constexpr bool exclusive = false;
            ret = rgw::create_zone(dpp(), null_yield, cfgstore.get(), exclusive, zone);
            if (ret < 0) {
        ...
        case OPT::ZONE_LIST:
        {
            ...
            int ret = rgw::read_zone(dpp(), null_yield, cfgstore.get(), {}, {}, default_zone_params);
            if (ret < 0 && ret != -ENOENT) {
            ...
            std::array<std::string, 1000> names;   // list in pages of 1000
            do {
                ret = cfgstore->list_zone_names(dpp(), null_yield, listing.next, names, listing);
                if (ret < 0) {
        ...
        case OPT::ZONE_MODIFY:
        {
            RGWZoneParams zone_params;
            std::unique_ptr<rgw::sal::ZoneWriter> zone_writer;
            int ret = rgw::read_zone(dpp(), null_yield, cfgstore.get(), zone_id, zone_name, zone_params, &zone_writer);
            if (ret < 0) {
                cerr << "failed to load zone: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
            ...
            if (!realm_id.empty()) {
                ...
                // get realm id from name
                ret = cfgstore->read_realm_id(dpp(), null_yield, realm_name, zone_params.realm_id);
                if (ret < 0) {
                    cerr << "failed to find realm by name " << realm_name << std::endl;
                    return -ret;
                }
        ...
        case OPT::ZONE_PLACEMENT_RM:
        {
            ...
            if (opt_cmd == OPT::ZONE_PLACEMENT_ADD || opt_cmd == OPT::ZONE_PLACEMENT_MODIFY) {
                RGWZoneGroup zonegroup;
                ret = rgw::read_zonegroup(dpp(), null_yield, cfgstore.get(), zonegroup_id, zonegroup_name, zonegroup);
                if (ret < 0) {
                    cerr << "failed to init zonegroup: " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
                ...
                if (ptiter == zonegroup.placement_targets.end()) {
                    cerr << "ERROR: placement id '" << placement_id
                         << "' is not configured in zonegroup placement targets" << std::endl;
                    return EINVAL;
                }

                string storage_class =
                    rgw_placement_rule::get_canonical_storage_class(opt_storage_class.value_or(string()));
                if (ptiter->second.storage_classes.find(storage_class) == ptiter->second.storage_classes.end()) {
                    cerr << "ERROR: storage class '" << storage_class << "' is not defined in zonegroup '"
                         << placement_id << "' placement target" << std::endl;
                    return EINVAL;
                }
                if (ptiter->second.tier_targets.find(storage_class) != ptiter->second.tier_targets.end()) {
                    cerr << "ERROR: storage class '" << storage_class << "' is of tier type in zonegroup '"
                         << placement_id << "' placement target" << std::endl;
                    return EINVAL;
                }
        ...
        {
            RGWZoneParams zone;
            int ret = rgw::read_zone(dpp(), null_yield, cfgstore.get(), zone_id, zone_name, zone);
            if (ret < 0) {
    ...
    std::set<OPT> non_master_ops_list = {
        OPT::USER_CREATE,    OPT::USER_RM,         OPT::USER_MODIFY,       OPT::USER_ENABLE, OPT::USER_SUSPEND,
        OPT::SUBUSER_CREATE, OPT::SUBUSER_MODIFY,  OPT::SUBUSER_RM,        OPT::BUCKET_LINK, OPT::BUCKET_UNLINK,
        OPT::BUCKET_RM,      OPT::BUCKET_CHOWN,    OPT::METADATA_PUT,      OPT::METADATA_RM, OPT::MFA_CREATE,
        OPT::MFA_REMOVE,     OPT::MFA_RESYNC,      OPT::CAPS_ADD,          OPT::CAPS_RM,     OPT::ROLE_CREATE,
        OPT::ROLE_DELETE,    OPT::ROLE_POLICY_PUT, OPT::ROLE_POLICY_DELETE};

    bool print_warning_message = (non_master_ops_list.find(opt_cmd) != non_master_ops_list.end() && non_master_cmd);

    if (print_warning_message) {
        cerr << "Please run the command on master zone. Performing this operation on non-master zone leads to "
    ...
    if (!display_name.empty()) user_op.set_display_name(display_name);

    if (!user_email.empty()) user_op.set_user_email(user_email);

    if (!rgw::sal::User::empty(user)) {
        user_op.set_new_user_id(new_user_id);
    }

    if (!access_key.empty()) user_op.set_access_key(access_key);

    if (!secret_key.empty()) user_op.set_secret_key(secret_key);

    if (!subuser.empty()) user_op.set_subuser(subuser);

    if (!caps.empty()) user_op.set_caps(caps);

    user_op.set_purge_data(purge_data);

    if (purge_keys) user_op.set_purge_keys();

    if (gen_access_key) user_op.set_generate_key();

    if (gen_secret_key) user_op.set_gen_secret();   // assume that a key pair should be created

    if (max_buckets_specified) user_op.set_max_buckets(max_buckets);

    if (admin_specified) user_op.set_admin(admin);

    if (system_specified) user_op.set_system(system);

    if (set_perm) user_op.set_perm(perm_mask);
    ...
    if (key_type != KEY_TYPE_UNDEFINED) user_op.set_key_type(key_type);
    ...
    case OPT::USER_CREATE:
        ...
        if (ret < 0) {
            ...
            if (ret == -ERR_INVALID_TENANT_NAME) ret = -EINVAL;
    ...
    case OPT::PERIOD_PUSH:
    {
        ...
        // load the period
        RGWPeriod period;
        int ret = cfgstore->read_period(dpp(), null_yield, period_id, std::nullopt, period);
        if (ret < 0) {
            cerr << "failed to load period: " << cpp_strerror(-ret) << std::endl;
            return -ret;
        }
        ...
        JSONParser p;
        ret = send_to_remote_or_url(nullptr, url, opt_region, access_key, secret_key, info, bl, p);
        if (ret < 0) {
            cerr << "request failed: " << cpp_strerror(-ret) << std::endl;
            return -ret;
        }
    ...
    case OPT::PERIOD_COMMIT:
    {
        ...
        RGWRealm realm;
        std::unique_ptr<rgw::sal::RealmWriter> realm_writer;
        int ret = rgw::read_realm(dpp(), null_yield, cfgstore.get(), realm_id, realm_name, realm, &realm_writer);
        if (ret < 0) {
        ...
        ret = commit_period(cfgstore.get(),
                            realm,
                            *realm_writer,
                            period,
                            remote,
    ...
    case OPT::ROLE_CREATE:
    {
        ...
        try {
            const rgw::IAM::Policy p(g_ceph_context,
                                     tenant,
                                     bl,
                                     g_ceph_context->_conf.get_val<bool>("rgw_policy_reject_invalid_principals"));
        }
    ...
    case OPT::ROLE_POLICY_DELETE:
    {
        ...
        cout << "Policy: " << policy_name << " successfully deleted for role: " << role_name << std::endl;
        return 0;
    }
    ...
    default: output_user_info = false;
    }
    ...
    if (opt_cmd == OPT::BUCKET_LIMIT_CHECK) {
        ...
        if (!rgw::sal::User::empty(user)) {
            ...
            ret = RGWBucketAdminOp::limit_check(
                driver, bucket_op, user_ids, stream_flusher, null_yield, dpp(), warnings_only);
        }
        else {
            ...
            if (ret < 0) {
                cerr << "ERROR: buckets limit check can't get user metadata_key: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
            ...
            do {
                ...
                if (ret < 0 && ret != -ENOENT) {
                    cerr << "ERROR: buckets limit check lists_keys_next(): " << cpp_strerror(-ret) << std::endl;
                    break;
                }
                else {
    ...
    if (opt_cmd == OPT::BUCKETS_LIST) {
        if (bucket_name.empty()) {
            ...
            do {
                ...
                ret = bucket->list(dpp(), params, std::min(remaining, paginate_size), results, null_yield);
                if (ret < 0) {
                    cerr << "ERROR: driver->list_objects(): " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
                ldpp_dout(dpp(), 20) << "INFO: " << __func__
                                     << ": list() returned without error; results.objs.sizie()=" << results.objs.size()
                                     << "results.is_truncated=" << results.is_truncated << ", marker=" << params.marker
                                     << dendl;
    ...
    if (opt_cmd == OPT::BUCKET_RADOS_LIST) {
        RGWRadosList lister(static_cast<rgw::sal::RadosStore*>(driver), max_concurrent_ios, orphan_stale_secs, tenant);
        if (rgw_obj_fs) {
        ...
        if (ret < 0) {
            std::cerr << "ERROR: bucket radoslist failed to finish before "
    ...
    if (opt_cmd == OPT::BUCKET_SHARD_OBJECTS) {
        ...
        if (!num_shards_specified) {
            cerr << "ERROR: num-shards must be specified." << std::endl;
            return EINVAL;
        }

        if (specified_shard_id) {
            if (shard_id >= num_shards) {
                cerr << "ERROR: shard-id must be less than num-shards." << std::endl;
                return EINVAL;
            }
            std::string obj;
            uint64_t ctr = 0;
    ...
    if (opt_cmd == OPT::BUCKET_OBJECT_SHARD) {
        if (!num_shards_specified || object.empty()) {
            cerr << "ERROR: num-shards and object must be specified." << std::endl;
            return EINVAL;
        }
    ...
    if (opt_cmd == OPT::BUCKET_RESYNC_ENCRYPTED_MULTIPART) {
        ...
        ret = rados_driver->getRados()->bucket_resync_encrypted_multipart(
            dpp(), null_yield, rados_driver, bucket->get_info(), marker, stream_flusher);
        if (ret < 0) {
            return -ret;
        }
        formatter->close_section();
    ...
    if (opt_cmd == OPT::LOG_SHOW || opt_cmd == OPT::LOG_RM) {
        ...
        if (opt_cmd == OPT::LOG_SHOW) {
            ...
            if (show_log_entries) formatter->open_array_section("log_entries");
            ...
            do {
                ...
                if (skip_zero_entries && entry.bytes_sent == 0 && entry.bytes_received == 0) goto next;
            ...
            if (show_log_entries) formatter->close_section();
    ...
    if (opt_cmd == OPT::POOL_ADD) {
        ...
        int ret =
            static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->add_bucket_placement(dpp(), pool, null_yield);
        if (ret < 0) cerr << "failed to add bucket placement: " << cpp_strerror(-ret) << std::endl;
    }
    ...
    if (opt_cmd == OPT::POOL_RM) {
        ...
        int ret =
            static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->remove_bucket_placement(dpp(), pool, null_yield);
        if (ret < 0) cerr << "failed to remove bucket placement: " << cpp_strerror(-ret) << std::endl;
    }
    ...
    if (opt_cmd == OPT::USAGE_TRIM) {
        if (rgw::sal::User::empty(user) && bucket_name.empty() && start_date.empty() && end_date.empty() &&
            !yes_i_really_mean_it) {
    ...
    if (opt_cmd == OPT::OLH_READLOG) {
        ...
        ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->bucket_index_read_olh_log(
            dpp(), bucket->get_info(), *state, obj->get_obj(), 0, &log, &is_truncated);
        if (ret < 0) {
            cerr << "ERROR: failed reading olh: " << cpp_strerror(-ret) << std::endl;
            return -ret;
        }
    ...
    if (opt_cmd == OPT::BI_GET) {
        ...
        ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->bi_get(
            dpp(), bucket->get_info(), obj, bi_index_type, &entry);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::BI_LIST) {
        ...
        ldpp_dout(dpp(), 20) << "INFO: " << __func__ << ": max_entries=" << max_entries << ", index=" << index
                             << ", max_shards=" << max_shards << dendl;
        ...
        for (; i < max_shards; i++) {
            ...
            if (ret < 0) {
                cerr << "ERROR: bs.init(bucket=" << bucket << ", shard=" << i << "): " << cpp_strerror(-ret)
                     << std::endl;
                return -ret;
            }
            ...
            do {
                ...
                ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->bi_list(
                    bs, object, marker, max_entries, &entries, &is_truncated);
                if (ret < 0) {
                    cerr << "ERROR: bi_list(): " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
                ldpp_dout(dpp(), 20) << "INFO: " << __func__
                                     << ": bi_list() returned without error; entries.size()=" << entries.size()
                                     << ", is_truncated=" << is_truncated << ", marker=" << marker << dendl;
    ...
    if (opt_cmd == OPT::BI_PURGE) {
        ...
        if (ret == -ENOENT) {
            ...
            cerr << "ERROR: could not init current bucket info for bucket_name=" << bucket_name << ": "
                 << cpp_strerror(-ret) << std::endl;
            return -ret;
        }
        else if (cur_bucket->get_bucket_id() == bucket->get_bucket_id() && !yes_i_really_mean_it) {
    ...
    if (opt_cmd == OPT::OBJECT_REINDEX) {
        ...
        if (!rados_store) {
            cerr << "ERROR: this command can only work when the cluster has a RADOS backing store." << std::endl;
            return EPERM;
        }
        ...
        if (!object.empty()) {
            ...
            if (!file.is_open()) {
                std::cerr << "ERROR: unable to open objects-file \"" << objects_file << "\"." << std::endl;
                return ENOENT;
            }
            ...
            while (std::getline(file, obj_name)) {
                ...
                if (ret < 0) {
                    std::cerr << "ERROR: while processing \"" << obj_name << "\", received " << cpp_strerror(-ret)
                              << "." << std::endl;
                    if (!yes_i_really_mean_it) {
                        std::cerr << "NOTE: with *caution* you can use --yes-i-really-mean-it to push through errors "
    ...
    if (opt_cmd == OPT::BUCKET_REWRITE) {
        ...
        while (is_truncated) {
            ...
            else if (result.size() > NUM_ENTRIES * 7 / 8 && expansion_factor > 1) {
                --expansion_factor;
            }
            ...
            for (auto iter = result.begin(); iter != result.end(); ++iter) {
                ...
                if ((entry.meta.size < min_rewrite_size) || (entry.meta.size > max_rewrite_size) ||
    ...
    if (opt_cmd == OPT::BUCKET_RESHARD) {
        int ret = check_reshard_bucket_params(
            driver, bucket_name, tenant, bucket_id, num_shards_specified, num_shards, yes_i_really_mean_it, &bucket);
        if (ret < 0) {
            return ret;
        }
        ...
        if (!zone_svc->can_reshard()) {
            ...
            std::cerr << "The zonegroup '" << zonegroup.get_name()
                      << "' does not "
                         "have the resharding feature enabled."
                      << std::endl;
            return ENOTSUP;
        }
        if (!RGWBucketReshard::can_reshard(bucket->get_info(), zone_svc) && !yes_i_really_mean_it) {
            std::cerr << "Bucket '" << bucket->get_name()
                      << "' already has too many "
                         "log generations ("
                      << bucket->get_info().layout.logs.size()
                      << ") "
                         ...
                         "you can force a reshard with --yes-i-really-mean-it."
                      << std::endl;
            return EINVAL;
        }
...
#define DEFAULT_RESHARD_MAX_ENTRIES 1000
        ...
        ret = br.execute(num_shards, fault, max_entries, dpp(), verbose, &cout, formatter.get());
        return -ret;
    }
    ...
    if (opt_cmd == OPT::RESHARD_LIST) {
        ...
        int num_logshards = driver->ctx()->_conf.get_val<uint64_t>("rgw_reshard_num_logs");
    ...
    if (opt_cmd == OPT::RESHARD_CANCEL) {
            ...
            if (yes_i_really_mean_it) {
                ...
                cerr << "ERROR: could not init bucket: " << cpp_strerror(-ret)
                     << "; if you want to cancel the reshard request nonetheless, please "
                        "use the --yes-i-really-mean-it option"
                     << std::endl;
                return -ret;
            }
        }
            ...
            if (ret < 0) {
                if (ret == -EBUSY) {
                    cerr << "There is ongoing resharding, please retry after "
                         << driver->ctx()->_conf.get_val<uint64_t>("rgw_reshard_bucket_lock_duration") << " seconds."
                         << std::endl;
                    return -ret;
                }
                else if (ret == -EINVAL) {
                    ...
                    cerr << "Error cancelling bucket \"" << bucket_name << "\" resharding: " << cpp_strerror(-ret)
                         << std::endl;
                    return -ret;
                }
            }
        ...
        if (ret == -ENOENT) {
            ...
            cerr << "Error in updating reshard log with bucket \"" << bucket_name << "\": " << cpp_strerror(-ret)
                 << std::endl;
            return -ret;
        }
    }   // OPT_RESHARD_CANCEL
    ...
    if (opt_cmd == OPT::OBJECT_STAT) {
        ...
        for (iter = obj->get_attrs().begin(); iter != obj->get_attrs().end(); ++iter) {
            ...
            if (!handled) other_attrs[iter->first] = bl;
        }
    ...
    if (opt_cmd == OPT::GC_LIST) {
        ...
        do {
            ...
            int ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->list_gc_objs(
                &index, marker, 1000, !include_all, result, &truncated, processing_queue);
            if (ret < 0) {
...
#define MAX_LC_LIST_ENTRIES 100
        ...
        do {
            int ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->list_lc_progress(
                marker, max_entries, bucket_lc_map, index);
            if (ret < 0) {
                cerr << "ERROR: failed to list objs: " << cpp_strerror(-ret) << std::endl;
                return 1;
            }
            for (const auto& entry : bucket_lc_map) {
                ...
                if (std::strftime(exp_buf, sizeof(exp_buf), "%a, %d %b %Y %T %Z", std::gmtime(&t))) {
    ...
    if (opt_cmd == OPT::LC_PROCESS) {
        ...
        int ret = static_cast<rgw::sal::RadosStore*>(driver)->getRados()->process_lc(bucket);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::USER_STATS) {
        ...
        if (reset_stats) {
            ...
            ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->user->reset_bucket_stats(
                dpp(), user->get_id(), null_yield);
            if (ret < 0) {
                cerr << "ERROR: could not reset user stats: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
        }
        ...
        if (sync_stats) {
            if (!bucket_name.empty()) {
                ...
                if (ret < 0) {
                    cerr << "ERROR: could not sync user stats: " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
            }
        }
    ...
    if (opt_cmd == OPT::METADATA_GET) {
        int ret = static_cast<rgw::sal::RadosStore*>(driver)->ctl()->meta.mgr->get(
            metadata_key, formatter.get(), null_yield, dpp());
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::METADATA_PUT) {
        ...
        ret = static_cast<rgw::sal::RadosStore*>(driver)->ctl()->meta.mgr->put(
            metadata_key, bl, null_yield, dpp(), RGWMDLogSyncType::APPLY_ALWAYS, false);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::MDLOG_LIST) {
        ...
        if (period_id.empty()) {
            ...
            period_id = realm.current_period;
            std::cerr << "No --period given, using current period=" << period_id << std::endl;
        }
    ...
    if (opt_cmd == OPT::MDLOG_AUTOTRIM) {
        ...
        auto mltcr =
            create_admin_meta_log_trim_cr(dpp(), static_cast<rgw::sal::RadosStore*>(driver), &http, num_shards);
        if (!mltcr) {
    ...
    if (opt_cmd == OPT::DATA_SYNC_STATUS) {
        ...
        if (specified_shard_id) {
            ...
            ret = sync.read_shard_status(dpp(),
                                         shard_id,
                                         pending_buckets,
                                         recovering_buckets,
                                         &sync_marker,
    ...
    if (opt_cmd == OPT::DATA_SYNC_RUN) {
        ...
        int ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->sync_modules->get_manager()->create_instance(
            dpp(),
            g_ceph_context,
            static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->get_zone().tier_type,
            static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->get_zone_params().tier_config,
            &sync_module);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::BUCKET_SYNC_INIT) {
        ...
        auto sync = RGWBucketPipeSyncStatusManager::construct(dpp(),
                                                              static_cast<rgw::sal::RadosStore*>(driver),
                                                              source_zone,
                                                              opt_sb,
                                                              bucket->get_key(),
                                                              extra_info ? &std::cout : nullptr);
    ...
    if (opt_cmd == OPT::BUCKET_SYNC_CHECKPOINT) {
        ...
        if (ret < 0) {
            std::cerr << "ERROR: failed to get policy handler for bucket (" << bucket << "): r=" << ret << ": "
                      << cpp_strerror(-ret) << std::endl;
            return -ret;
        }

        auto timeout_at = ceph::coarse_mono_clock::now() + opt_timeout_sec;
        ret = rgw_bucket_sync_checkpoint(dpp(),
                                         static_cast<rgw::sal::RadosStore*>(driver),
                                         *handler,
                                         bucket->get_info(),
                                         opt_source_zone,
    ...
    if (opt_cmd == OPT::BUCKET_SYNC_MARKERS) {
        ...
        if (!sync_status) {
            cerr << "ERROR: sync.read_sync_status() returned error=" << sync_status.error() << std::endl;
    ...
    if (opt_cmd == OPT::BILOG_LIST) {
        ...
        do {
            ...
            ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->bilog_rados->log_list(
                dpp(), bucket->get_info(), log_layout, shard_id, marker, max_entries - count, entries, &truncated);
            if (ret < 0) {
    ...
    if (opt_cmd == OPT::SYNC_ERROR_LIST) {
        ...
        for (; shard_id < ERROR_LOGGER_SHARDS; ++shard_id) {
            ...
            do {
                list<cls_log_entry> entries;
                ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->timelog.list(
                    dpp(), oid, {}, {}, max_entries - count, entries, marker, &marker, &truncated, null_yield);
                if (ret == -ENOENT) {
                    break;
                }
                if (ret < 0) {
    ...
    if (opt_cmd == OPT::SYNC_GROUP_CREATE || opt_cmd == OPT::SYNC_GROUP_MODIFY) {
        CHECK_TRUE(require_non_empty_opt(opt_group_id), "ERROR: --group-id not specified", EINVAL);
        CHECK_TRUE(
            require_opt(opt_status), "ERROR: --status is not specified (options: forbidden, allowed, enabled)", EINVAL);
    ...
    if (opt_cmd == OPT::SYNC_GROUP_FLOW_CREATE) {
        ...
        CHECK_TRUE(
            require_opt(opt_flow_type), "ERROR: --flow-type not specified (options: symmetrical, directional)", EINVAL);
        CHECK_TRUE((symmetrical_flow_opt(*opt_flow_type) || directional_flow_opt(*opt_flow_type)),
                   "ERROR: --flow-type invalid (options: symmetrical, directional)",
                   EINVAL);
        ...
        if (symmetrical_flow_opt(*opt_flow_type)) {
            CHECK_TRUE(require_non_empty_opt(opt_zone_ids),
                       "ERROR: --zones not provided for symmetrical flow, or is empty",
                       EINVAL);
            ...
            CHECK_TRUE(require_non_empty_opt(opt_source_zone_id),
                       "ERROR: --source-zone not provided for directional flow rule, or is empty",
                       EINVAL);
            CHECK_TRUE(require_non_empty_opt(opt_dest_zone_id),
                       "ERROR: --dest-zone not provided for directional flow rule, or is empty",
                       EINVAL);
    ...
    if (opt_cmd == OPT::SYNC_GROUP_PIPE_CREATE || opt_cmd == OPT::SYNC_GROUP_PIPE_MODIFY) {
        CHECK_TRUE(require_non_empty_opt(opt_group_id), "ERROR: --group-id not specified", EINVAL);
        CHECK_TRUE(require_non_empty_opt(opt_pipe_id), "ERROR: --pipe-id not specified", EINVAL);
        if (opt_cmd == OPT::SYNC_GROUP_PIPE_CREATE) {
            CHECK_TRUE(require_non_empty_opt(opt_source_zone_ids),
                       "ERROR: --source-zones not provided or is empty; should be list of zones or '*'",
                       EINVAL);
            CHECK_TRUE(require_non_empty_opt(opt_dest_zone_ids),
                       "ERROR: --dest-zones not provided or is empty; should be list of zones or '*'",
                       EINVAL);
        }
        ...
        pipe->source.set_bucket(opt_source_tenant, opt_source_bucket_name, opt_source_bucket_id);
        if (opt_dest_zone_ids) {
            pipe->dest.add_zones(*opt_dest_zone_ids);
        }
        pipe->dest.set_bucket(opt_dest_tenant, opt_dest_bucket_name, opt_dest_bucket_id);
    ...
    if (opt_cmd == OPT::SYNC_GROUP_PIPE_REMOVE) {
        ...
        pipe->source.remove_bucket(opt_source_tenant, opt_source_bucket_name, opt_source_bucket_id);
        if (opt_dest_zone_ids) {
            pipe->dest.remove_zones(*opt_dest_zone_ids);
        }
        pipe->dest.remove_bucket(opt_dest_tenant, opt_dest_bucket_name, opt_dest_bucket_id);

        if (!(opt_source_zone_ids || opt_source_tenant || opt_source_bucket || opt_source_bucket_id ||
              opt_dest_zone_ids || opt_dest_tenant || opt_dest_bucket || opt_dest_bucket_id)) {
    ...
    if (opt_cmd == OPT::BILOG_STATUS) {
        ...
        ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->bilog_rados->get_log_status(
            dpp(), bucket->get_info(), log_layout, shard_id, &markers, null_yield);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::DATALOG_LIST) {
        ...
        do {
            ...
            if (specified_shard_id) {
                ret = datalog_svc->list_entries(
                    dpp(), shard_id, max_entries - count, entries, marker, &marker, &truncated, null_yield);
            }
            else {
                ret =
                    datalog_svc->list_entries(dpp(), max_entries - count, entries, log_marker, &truncated, null_yield);
            }
            if (ret < 0) {
    ...
    if (opt_cmd == OPT::DATALOG_STATUS) {
        ...
        for (; i < g_ceph_context->_conf->rgw_data_log_num_shards; i++) {
            ...
            static_cast<rgw::sal::RadosStore*>(driver)->svc()->datalog_rados->get_info(dpp(), i, &info, null_yield);
    ...
    if (opt_cmd == OPT::DATALOG_AUTOTRIM) {
        ...
        ret = crs.run(dpp(),
                      create_admin_data_log_trim_cr(
                          dpp(), static_cast<rgw::sal::RadosStore*>(driver), &http, num_shards, markers));
        if (ret < 0) {
    ...
    if (quota_op) {
        ...
        if (!bucket_name.empty()) {
            ...
            set_bucket_quota(
                driver, opt_cmd, tenant, bucket_name, max_size, max_objects, have_max_size, have_max_objects);
        }
        else if (!rgw::sal::User::empty(user)) {
            if (quota_scope == "bucket") {
                return set_user_bucket_quota(
                    opt_cmd, ruser, user_op, max_size, max_objects, have_max_size, have_max_objects);
            }
            else if (quota_scope == "user") {
                return set_user_quota(opt_cmd, ruser, user_op, max_size, max_objects, have_max_size, have_max_objects);
            }
            else {
                cerr << "ERROR: invalid quota scope specification. Please specify either --quota-scope=bucket, or "
                        "--quota-scope=user"
                     << std::endl;
                return EINVAL;
            }
    ...
    bool ratelimit_op_set =
        (opt_cmd == OPT::RATELIMIT_SET || opt_cmd == OPT::RATELIMIT_ENABLE || opt_cmd == OPT::RATELIMIT_DISABLE);
    ...
    if (ratelimit_op_set) {
        ...
        if (!bucket_name.empty()) {
            if (!ratelimit_scope.empty() && ratelimit_scope != "bucket") {
                cerr << "ERROR: invalid ratelimit scope specification. (bucket scope is not bucket but bucket has been "
                        "specified)"
                     << std::endl;
                return EINVAL;
            }
            return set_bucket_ratelimit(driver,
                                        opt_cmd,
                                        tenant,
                                        bucket_name,
                                        max_read_ops,
        ...
        if (ratelimit_scope == "user") {
            return set_user_ratelimit(opt_cmd,
                                      user,
                                      max_read_ops,
                                      max_write_ops,
                                      max_read_bytes,
            ...
            cerr << "ERROR: invalid ratelimit scope specification. Please specify either --ratelimit-scope=bucket, or "
    ...
    if (opt_cmd == OPT::MFA_CREATE) {
        ...
        int ret = static_cast<rgw::sal::RadosStore*>(driver)->ctl()->meta.mgr->mutate(
            RGWSI_MetaBackend_OTP::get_meta_key(user->get_id()),
            mtime,
            &objv_tracker,
            null_yield,
            dpp(),
            MDLOG_STATUS_WRITE,
            [&] {
                return static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->mfa.create_mfa(
                    dpp(), user->get_id(), config, &objv_tracker, mtime, null_yield);
            });
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::MFA_REMOVE) {
        ...
        int ret = static_cast<rgw::sal::RadosStore*>(driver)->ctl()->meta.mgr->mutate(
            ...
            MDLOG_STATUS_WRITE,
            [&] {
                return static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->mfa.remove_mfa(
                    dpp(), user->get_id(), totp_serial, &objv_tracker, mtime, null_yield);
            });
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::MFA_GET) {
        ...
        int ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->mfa.get_mfa(
            dpp(), user->get_id(), totp_serial, &result, null_yield);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::MFA_LIST) {
        ...
        int ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->mfa.list_mfa(
            dpp(), user->get_id(), &result, null_yield);
        if (ret < 0 && ret != -ENOENT) {
    ...
    if (opt_cmd == OPT::MFA_CHECK) {
        ...
        list<rados::cls::otp::otp_info_t> result;
        int ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->mfa.check_mfa(
            dpp(), user->get_id(), totp_serial, totp_pin.front(), null_yield);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::MFA_RESYNC) {
        ...
        rados::cls::otp::otp_info_t config;
        int ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->mfa.get_mfa(
            dpp(), user->get_id(), totp_serial, &config, null_yield);
        if (ret < 0) {
            if (ret == -ENOENT || ret == -ENODATA) {
                cerr << "MFA serial id not found" << std::endl;
            }
        ...
        ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls->mfa.otp_get_current_time(
            dpp(), user->get_id(), &now, null_yield);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::PUBSUB_TOPIC_RM) {
        ...
        ret = rgw::notify::remove_persistent_topic(
            dpp(),
            static_cast<rgw::sal::RadosStore*>(driver)->getRados()->get_notif_pool_ctx(),
            topic_name,
            null_yield);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::SCRIPT_PUT) {
        ...
        if (script_ctx == rgw::lua::context::none) {
            cerr << "ERROR: invalid script context: " << *str_script_ctx << ". must be one of: " << LUA_CONTEXT_LIST
                 << std::endl;
            return EINVAL;
        }
