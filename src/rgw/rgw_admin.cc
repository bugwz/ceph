...
static void show_reshard_status(const list<cls_rgw_bucket_instance_entry>& status, Formatter* formatter)
{
    ...
    for (const auto& entry : status) {
        ...
        formatter->dump_string("new_bucket_instance_id", entry.new_bucket_instance_id);
...
public:
    explicit StoreDestructor(rgw::sal::RGWRadosStore* _s)
        : store(_s)
    {}
    ~StoreDestructor()
    {
                       ...
                       RGWBucketInfo& bucket_info, rgw_bucket& bucket, map<string, bufferlist>* pattrs = nullptr)
{
    if (!bucket_name.empty()) {
        ...
        int r;
        if (bucket_id.empty()) {
            r = store->getRados()->get_bucket_info(
                store->svc(), tenant_name, bucket_name, bucket_info, nullptr, null_yield, dpp(), pattrs);
        }
        else {
            string bucket_instance_id = bucket_name + ":" + bucket_id;
            r = store->getRados()->get_bucket_instance_info(
                obj_ctx, bucket_instance_id, bucket_info, NULL, pattrs, null_yield, dpp());
        }
        if (r < 0) {
...
static int init_bucket(const rgw_bucket& b, RGWBucketInfo& bucket_info, rgw_bucket& bucket,
                       map<string, bufferlist>* pattrs = nullptr)
{
    return init_bucket(b.tenant, b.name, b.bucket_id, bucket_info, bucket, pattrs);
}
...
int set_bucket_quota(rgw::sal::RGWRadosStore* store, OPT opt_cmd, const string& tenant_name, const string& bucket_name,
                     int64_t max_size, int64_t max_objects, bool have_max_size, bool have_max_objects)
{
    ...
    int r = store->getRados()->get_bucket_info(
        store->svc(), tenant_name, bucket_name, bucket_info, NULL, null_yield, dpp(), &attrs);
    if (r < 0) {
...
int check_min_obj_stripe_size(rgw::sal::RGWRadosStore* store, RGWBucketInfo& bucket_info, rgw::sal::RGWObject* obj,
                              uint64_t min_stripe_size, bool* need_rewrite)
{
...
int check_obj_tail_locator_underscore(RGWBucketInfo& bucket_info, rgw_obj& obj, rgw_obj_key& key, bool fix,
                                      Formatter* f)
{
    f->open_object_section("object");
    f->open_object_section("key");
...
static boost::optional<RGWRESTConn> get_remote_conn(rgw::sal::RGWRadosStore* store, const RGWZoneGroup& zonegroup,
...
static boost::optional<RGWRESTConn> get_remote_conn(rgw::sal::RGWRadosStore* store, const RGWPeriodMap& period_map,
                                                    const std::string& remote)
{
    boost::optional<RGWRESTConn> conn;
...
static int send_to_url(const string& url, const string& access, const string& secret, req_info& info,
...
static int send_to_remote_or_url(RGWRESTConn* conn, const string& url, const string& access, const string& secret,
...
static int commit_period(RGWRealm& realm, RGWPeriod& period, string remote, const string& url, const string& access,
                         const string& secret, bool force)
{
    ...
    if (store->svc()->zone->zone_id() == master_zone) {
        ...
        int ret = current_period.init(dpp(), g_ceph_context, store->svc()->sysobj, realm.get_id(), null_yield);
        if (ret < 0) {
            cerr << "Error initializing current period: " << cpp_strerror(-ret) << std::endl;
            return ret;
        }
...
static int update_period(const string& realm_id, const string& realm_name, const string& period_id,
...
static int init_bucket_for_sync(const string& tenant, const string& bucket_name, const string& bucket_id,
                                rgw_bucket& bucket)
{
    RGWBucketInfo bucket_info;
...
static int do_period_pull(RGWRESTConn* remote_conn, const string& url, const string& access_key,
                          const string& secret_key, const string& realm_id, const string& realm_name,
                          const string& period_id, const string& period_epoch, RGWPeriod* period)
{
    RGWEnv env;
    req_info info(g_ceph_context, &env);
    info.method = "GET";
    ...
    bufferlist bl;
    JSONParser p;
    int ret = send_to_remote_or_url(remote_conn, url, access_key, secret_key, info, bl, p);
    if (ret < 0) {
        cerr << "request failed: " << cpp_strerror(-ret) << std::endl;
        return ret;
    }
...
static int read_current_period_id(rgw::sal::RGWRadosStore* store, const std::string& realm_id,
                                  const std::string& realm_name, std::string* period_id)
{
    RGWRealm realm(realm_id, realm_name);
    int ret = realm.init(dpp(), g_ceph_context, store->svc()->sysobj, null_yield);
    if (ret < 0) {
...
static void sync_status(Formatter* formatter)
{
    ...
    cout << std::setw(width) << "realm" << std::setw(1) << " " << realm.get_id() << " (" << realm.get_name() << ")"
         << std::endl;
                     ...
                     Formatter* formatter)
{
    ...
    zone_policy_handler->reflect(nullptr,
                                 nullptr,
                                 nullptr,
                                 nullptr,
                                 &source_zones,
    ...
    for (auto& hint_entity : hint_entities) {
        ...
        int r =
            store->ctl()->bucket->get_sync_policy_handler(zid, hint_bucket, &hint_bucket_handler, null_yield, dpp());
        if (r < 0) {
...
static int bucket_sync_info(rgw::sal::RGWRadosStore* store, const RGWBucketInfo& info, std::ostream& out)
{
    const RGWRealm& realm = store->svc()->zone->get_realm();
    const RGWZoneGroup& zonegroup = store->svc()->zone->get_zonegroup();
    const RGWZone& zone = store->svc()->zone->get_zone();
...
static int bucket_sync_status(rgw::sal::RGWRadosStore* store, const RGWBucketInfo& info,
                              const rgw_zone_id& source_zone_id, std::optional<rgw_bucket>& opt_source_bucket,
                              std::ostream& out)
{
    const RGWRealm& realm = store->svc()->zone->get_realm();
    const RGWZoneGroup& zonegroup = store->svc()->zone->get_zonegroup();
    ...
    for (auto& zone_id : zone_ids) {
        ...
        for (auto& entry : sources) {
            ...
            if (pipe.source.zone.value_or(rgw_zone_id()) == z->second.id) {
                bucket_source_sync_status(dpp(), store, zone, z->second, c->second, info, pipe, width, out);
            }
        }
    }
...
int check_reshard_bucket_params(rgw::sal::RGWRadosStore* store, const string& bucket_name, const string& tenant,
                                const string& bucket_id, bool num_shards_specified, int num_shards,
                                int yes_i_really_mean_it, rgw_bucket& bucket, RGWBucketInfo& bucket_info,
                                map<string, bufferlist>& attrs)
{
    if (bucket_name.empty()) {
    ...
    if (bucket_info.reshard_status != cls_rgw_reshard_status::NOT_RESHARDING) {
        ...
        cerr << "ERROR: the bucket is currently undergoing resharding and "
                "cannot be added to the reshard list at this time"
             << std::endl;
        return -EBUSY;
    }

    int num_source_shards = (bucket_info.layout.current_index.layout.normal.num_shards > 0
...
static int trim_sync_error_log(int shard_id, const string& marker, int delay_ms)
{
    ...
    for (;;) {
        int ret = store->svc()->cls->timelog.trim(dpp(), oid, {}, {}, {}, marker, nullptr, null_yield);
        if (ret == -ENODATA) {
            return 0;
        }
        if (ret < 0) {
...
public:
    SyncPolicyContext(const string& zonegroup_id, const string& zonegroup_name, std::optional<rgw_bucket> _bucket)
        : zonegroup(zonegroup_id, zonegroup_name)
        , bucket(_bucket)
    {}
...
int main(int argc, const char** argv)
{
    ...
    if (args.empty()) {
        ...
        if (tenant.empty()) {
            ...
            if (user_id.empty() && opt_cmd != OPT::ROLE_CREATE && opt_cmd != OPT::ROLE_DELETE &&
                opt_cmd != OPT::ROLE_GET && opt_cmd != OPT::ROLE_MODIFY && opt_cmd != OPT::ROLE_LIST &&
                opt_cmd != OPT::ROLE_POLICY_PUT && opt_cmd != OPT::ROLE_POLICY_LIST &&
                opt_cmd != OPT::ROLE_POLICY_GET && opt_cmd != OPT::ROLE_POLICY_DELETE && opt_cmd != OPT::RESHARD_ADD &&
                opt_cmd != OPT::RESHARD_CANCEL && opt_cmd != OPT::RESHARD_STATUS) {
    ...
    if (raw_storage_op) {
        ...
        store = RGWStoreManager::get_storage(dpp(),
                                             g_ceph_context,
                                             false,
                                             false,
                                             false,
    ...
    if (raw_storage_op) {
        ...
        case OPT::PERIOD_GET:
        {
            ...
            RGWPeriod period(period_id, epoch);
            int ret = period.init(dpp(), g_ceph_context, store->svc()->sysobj, realm_id, null_yield, realm_name);
            if (ret < 0) {
                cerr << "period init failed: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
        ...
        {
            int ret = update_period(realm_id,
                                    realm_name,
                                    period_id,
                                    period_epoch,
                                    commit,
        ...
        case OPT::PERIOD_PULL:
        {
            ...
            RGWPeriod period;
            int ret = do_period_pull(
                remote_conn, url, access_key, secret_key, realm_id, realm_name, period_id, period_epoch, &period);
            if (ret < 0) {
        ...
        case OPT::GLOBAL_QUOTA_DISABLE:
        {
            ...
            if (quota_scope == "bucket") {
                set_quota_info(
                    period_config.bucket_quota, opt_cmd, max_size, max_objects, have_max_size, have_max_objects);
                encode_json("bucket quota", period_config.bucket_quota, formatter.get());
            }
            else if (quota_scope == "user") {
                set_quota_info(
                    period_config.user_quota, opt_cmd, max_size, max_objects, have_max_size, have_max_objects);
        ...
        case OPT::REALM_PULL:
        {
            ...
            if (!current_period.empty()) {
                ...
                ret = do_period_pull(
                    nullptr, url, access_key, secret_key, realm_id, realm_name, current_period, "", &period);
                if (ret < 0) {
        ...
        case OPT::ZONEGROUP_CREATE:
        {
            ...
            RGWZoneGroup zonegroup(
                zonegroup_name, is_master, g_ceph_context, store->svc()->sysobj, realm.get_id(), endpoints);
        ...
        {
            RGWZoneGroup zonegroup;
            int ret = zonegroup.init(dpp(), g_ceph_context, store->svc()->sysobj, null_yield, false);
            if (ret < 0) {
                cerr << "failed to init zonegroup: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
        ...
        case OPT::ZONEGROUP_SET:
        {
            ...
            RGWZoneGroup zonegroup;
            ret = zonegroup.init(dpp(), g_ceph_context, store->svc()->sysobj, null_yield, false);
            if (ret < 0) {
                cerr << "failed to init zonegroup: " << cpp_strerror(-ret) << std::endl;
                return -ret;
            }
        ...
        case OPT::ZONE_CREATE:
        {
            ...
            if (!zonegroup_id.empty() || !zonegroup_name.empty()) {
                ...
                if (ret < 0) {
                    cerr << "unable to initialize zonegroup " << zonegroup_name << ": " << cpp_strerror(-ret)
                         << std::endl;
                    return -ret;
                }
                if (realm_id.empty() && realm_name.empty()) {
        ...
        case OPT::ZONE_SET:
        {
            ...
            int ret = zone.init(dpp(), g_ceph_context, store->svc()->sysobj, null_yield, false);
            if (ret < 0) {
                return -ret;
            }
    ...
    std::set<OPT> non_master_ops_list = {
        ...
        OPT::BUCKET_RESHARD, OPT::BUCKET_RM,      OPT::BUCKET_CHOWN, OPT::METADATA_PUT, OPT::METADATA_RM,
        OPT::RESHARD_CANCEL, OPT::RESHARD_ADD,    OPT::MFA_CREATE,   OPT::MFA_REMOVE,   OPT::MFA_RESYNC,
    ...
    if (!placement_id.empty() || (opt_storage_class && !opt_storage_class->empty())) {
    ...
    case OPT::PERIOD_PUSH:
    {
        ...
        JSONParser p;
        ret = send_to_remote_or_url(nullptr, url, access_key, secret_key, info, bl, p);
        if (ret < 0) {
            cerr << "request failed: " << cpp_strerror(-ret) << std::endl;
            return -ret;
        }
    ...
    case OPT::PERIOD_COMMIT:
    {
        ...
        ret = commit_period(realm, period, remote, url, access_key, secret_key, yes_i_really_mean_it);
        if (ret < 0) {
            cerr << "failed to commit period: " << cpp_strerror(-ret) << std::endl;
            return -ret;
        }
    ...
    case OPT::ROLE_LIST:
    {
        ...
        ret = RGWRole::get_roles_by_path_prefix(
            dpp(), store->getRados(), g_ceph_context, path_prefix, tenant, result, null_yield);
        if (ret < 0) {
            return -ret;
        }
    ...
    if (opt_cmd == OPT::BUCKET_LIMIT_CHECK) {
        ...
        if (!user_id.empty()) {
            ...
            ret = RGWBucketAdminOp::limit_check(store, bucket_op, user_ids, f, null_yield, dpp(), warnings_only);
        }
        else {
            ...
            do {
                ret = store->ctl()->meta.mgr->list_keys_next(handle, max, user_ids, &truncated);
                if (ret < 0 && ret != -ENOENT) {
                    ...
                    ret =
                        RGWBucketAdminOp::limit_check(store, bucket_op, user_ids, f, null_yield, dpp(), warnings_only);
                    if (ret < 0) break;
                }
                user_ids.clear();
            } while (truncated);
    ...
    if (opt_cmd == OPT::BUCKETS_LIST) {
        if (bucket_name.empty()) {
            ...
            do {
                ...
                ret = list_op.list_objects(
                    dpp(), std::min(remaining, paginate_size), &result, &common_prefixes, &truncated, null_yield);
                if (ret < 0) {
                    cerr << "ERROR: store->list_objects(): " << cpp_strerror(-ret) << std::endl;
                    return -ret;
                }
    ...
    if (opt_cmd == OPT::BUCKET_RADOS_LIST) {
        RGWRadosList lister(store, max_concurrent_ios, orphan_stale_secs, tenant);
        if (rgw_obj_fs) {
    ...
    if (opt_cmd == OPT::USAGE_TRIM) {
        if (user_id.empty() && bucket_name.empty() && start_date.empty() && end_date.empty() && !yes_i_really_mean_it) {
    ...
    if (opt_cmd == OPT::OLH_READLOG) {
        ...
        ret = store->getRados()->get_obj_state(
            dpp(), &rctx, bucket_info, obj, &state, false, null_yield); /* don't follow olh */
        if (ret < 0) {
            return -ret;
        }
    ...
    if (opt_cmd == OPT::BI_LIST) {
        ...
        for (; i < max_shards; i++) {
            ...
            int ret =
                bs.init(bucket, shard_id, bucket_info.layout.current_index, nullptr /* no RGWBucketInfo */, dpp());
            marker.clear();

            if (ret < 0) {
                cerr << "ERROR: bs.init(bucket=" << bucket << ", shard=" << shard_id << "): " << cpp_strerror(-ret)
                     << std::endl;
                return -ret;
            }
...
#define DEFAULT_RESHARD_MAX_ENTRIES 1000
        ...
        return br.execute(num_shards, max_entries, dpp(), verbose, &cout, formatter.get());
    }
    ...
    if (opt_cmd == OPT::RESHARD_LIST) {
        ...
        int num_logshards = store->ctx()->_conf.get_val<uint64_t>("rgw_reshard_num_logs");
            ...
            if (ret < 0) {
                if (ret == -EBUSY) {
                    cerr << "There is ongoing resharding, please retry after "
                         << store->ctx()->_conf.get_val<uint64_t>("rgw_reshard_bucket_lock_duration") << " seconds."
                         << std::endl;
                    return -ret;
                }
                else if (ret == -EINVAL) {
    ...
    if (opt_cmd == OPT::GC_LIST) {
        ...
        do {
            ...
            int ret = store->getRados()->list_gc_objs(
...
#define MAX_LC_LIST_ENTRIES 100
        ...
        do {
            int ret = store->getRados()->list_lc_progress(marker, max_entries, bucket_lc_map, index);
            if (ret < 0) {
                cerr << "ERROR: failed to list objs: " << cpp_strerror(-ret) << std::endl;
                return 1;
            }
    ...
    if (opt_cmd == OPT::USER_STATS) {
        ...
        int ret =
            store->ctl()->user->read_stats(dpp(), user_id, &stats, null_yield, &last_stats_sync, &last_stats_update);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::DATA_SYNC_RUN) {
        ...
        int ret = store->svc()->sync_modules->get_manager()->create_instance(
            g_ceph_context,
            store->svc()->zone->get_zone().tier_type,
            store->svc()->zone->get_zone_params().tier_config,
            &sync_module);
        if (ret < 0) {
        ...
        RGWDataSyncStatusManager sync(
            store, store->svc()->rados->get_async_processor(), source_zone, nullptr, sync_module);
    ...
    if (opt_cmd == OPT::BUCKET_SYNC_CHECKPOINT) {
        ...
        if (ret < 0) {
            std::cerr << "ERROR: failed to get policy handler for bucket (" << bucket_info.bucket << "): r=" << ret
                      << ": " << cpp_strerror(-ret) << std::endl;
            return -ret;
        }

        auto timeout_at = ceph::coarse_mono_clock::now() + opt_timeout_sec;
        ret = rgw_bucket_sync_checkpoint(
            dpp(), store, *handler, bucket_info, opt_source_zone, opt_source_bucket, opt_retry_delay_ms, timeout_at);
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::BILOG_LIST) {
        ...
        do {
            ...
            ret = store->svc()->bilog_rados->log_list(
                dpp(), bucket_info, shard_id, marker, max_entries - count, entries, &truncated);
            if (ret < 0) {
    ...
    if (opt_cmd == OPT::SYNC_ERROR_LIST) {
        ...
        for (; shard_id < ERROR_LOGGER_SHARDS; ++shard_id) {
            ...
            do {
                list<cls_log_entry> entries;
                ret = store->svc()->cls->timelog.list(
    ...
    if (opt_cmd == OPT::BILOG_STATUS) {
        ...
        ret = store->svc()->bilog_rados->get_log_status(dpp(), bucket_info, shard_id, &markers, null_yield);
        if (ret < 0) {
        ...
        do {
            ...
            if (specified_shard_id) {
                ret = datalog_svc->list_entries(
                    dpp(), shard_id, max_entries - count, entries, marker, &marker, &truncated);
            }
            else {
                ret = datalog_svc->list_entries(dpp(), max_entries - count, entries, log_marker, &truncated);
            }
            if (ret < 0) {
    ...
    if (quota_op) {
        ...
        if (!bucket_name.empty()) {
            ...
            set_bucket_quota(
                store, opt_cmd, tenant, bucket_name, max_size, max_objects, have_max_size, have_max_objects);
        }
        else if (!user_id.empty()) {
            if (quota_scope == "bucket") {
                return set_user_bucket_quota(
                    opt_cmd, user, user_op, max_size, max_objects, have_max_size, have_max_objects);
            }
            else if (quota_scope == "user") {
    ...
    if (opt_cmd == OPT::MFA_CREATE) {
        ...
        int ret = store->ctl()->meta.mgr->mutate(RGWSI_MetaBackend_OTP::get_meta_key(user_id),
                                                 ...
                                                 MDLOG_STATUS_WRITE,
                                                 [&] {
                                                     return store->svc()->cls->mfa.create_mfa(
                                                         dpp(), user_id, config, &objv_tracker, mtime, null_yield);
                                                 });
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::MFA_REMOVE) {
        ...
        int ret = store->ctl()->meta.mgr->mutate(RGWSI_MetaBackend_OTP::get_meta_key(user_id),
                                                 ...
                                                 MDLOG_STATUS_WRITE,
                                                 [&] {
                                                     return store->svc()->cls->mfa.remove_mfa(
                                                         dpp(), user_id, totp_serial, &objv_tracker, mtime, null_yield);
                                                 });
        if (ret < 0) {
    ...
    if (opt_cmd == OPT::SCRIPT_PUT) {
        ...
        if (script_ctx == rgw::lua::context::none) {
            cerr << "ERROR: invalid script context: " << *str_script_ctx << ". must be one of: preRequest, postRequest"
                 << std::endl;
            return EINVAL;
        }
