...
RGWCivetWebFrontend::RGWCivetWebFrontend(RGWProcessEnv& env, RGWFrontendConfig* conf, dmc::SchedulerCtx& sched_ctx)
    : conf(conf)
    , ctx(nullptr)
    , env(env)
{
    ...
    case dmc::scheduler_t::throttler: break;
...
int RGWCivetWebFrontend::process(struct mg_connection* const conn)
{
    ...
    auto real_client_io = rgw::io::add_reordering(
        rgw::io::add_buffering(dout_context, rgw::io::add_chunking(rgw::io::add_conlen_controlling(&cw_client))));
    ...
    int ret = process_request(env.store,
                              env.rest,
                              &req,
                              env.uri_prefix,
                              *env.auth_registry,
...
int RGWCivetWebFrontend::run()
{
    ...
    set_conf_default(conf_map, "num_threads", std::to_string(g_conf()->rgw_thread_pool_size));
