...
public:
    RGWLibIO() { get_env().set("HTTP_HOST", ""); }
    ...
    void set_account(bool) override { return; }
...
public:
    ...
    int init(const RGWEnv& rgw_env, rgw::sal::Driver* _driver, RGWLibIO* io, req_state* _s)
    {
        ...
        ldpp_dout(_s, 2) << "initializing for trans_id = " << get_state()->trans_id.c_str() << dendl;
...
public:
    RGWLibContinuedReq(CephContext* _cct, const RGWProcessEnv& penv, std::unique_ptr<rgw::sal::User> _user)
        ...
        , rstate(_cct, penv, &io_ctx.get_env(), id)
    {
        ...
        ldpp_dout(get_state(), 2) << "initializing for trans_id = " << get_state()->trans_id.c_str() << dendl;
    }
