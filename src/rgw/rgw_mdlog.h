...
class RGWMetadataLog
{
    ...
    {
        if (period.empty()) return META_LOG_OBJ_PREFIX;
...
public:
    RGWMetadataLog(CephContext* _cct, RGWSI_Zone* _zone_svc, RGWSI_Cls* _cls_svc, const std::string& period)
        : cct(_cct)
    ...
    int store_entries_in_shard(const DoutPrefixProvider* dpp, std::list<cls_log_entry>& entries, int shard_id,
                               librados::AioCompletion* completion);
    ...
    void init_list_entries(int shard_id, const real_time& from_time, const real_time& end_time,
                           const std::string& marker, void** handle);
    void complete_list_entries(void* handle);
    int list_entries(const DoutPrefixProvider* dpp, void* handle, int max_entries, std::list<cls_log_entry>& entries,
                     std::string* out_marker, bool* truncated);

    int trim(const DoutPrefixProvider* dpp, int shard_id, const real_time& from_time, const real_time& end_time,
    ...
    int lock_exclusive(const DoutPrefixProvider* dpp, int shard_id, timespan duration, std::string& zone_id,
                       std::string& owner_id);
...
struct LogStatusDump
{
    ...
    explicit LogStatusDump(RGWMDLogStatus _status)
        : status(_status)
    {}
    void dump(Formatter* f) const;
};
