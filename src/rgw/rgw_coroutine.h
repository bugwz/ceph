...
public:
    ...
    librados::AioCompletion* completion() { return c; }
...
template<typename T> class RGWAioCompletionNotifierWith : public RGWAioCompletionNotifier
{
    T value;

public:
    RGWAioCompletionNotifierWith(RGWCompletionManager* mgr, const rgw_io_id& io_id, void* user_data, T value)
        : RGWAioCompletionNotifier(mgr, io_id, user_data)
...
struct rgw_spawned_stacks
{
    ...
    void add_pending(RGWCoroutinesStack* s) { entries.push_back(s); }
...
class RGWCoroutine : public RefCountedObject, public boost::asio::coroutine
{
    ...
    struct StatusItem
    {
        ...
        StatusItem(utime_t& t, const std::string& s)
            : timestamp(t)
            , status(s)
        {}
...
#define MAX_COROUTINE_HISTORY 10
    ...
    {
        CephContext* cct;
        ceph::shared_mutex lock = ceph::make_shared_mutex("RGWCoroutine::Status::lock");
        int max_history;

        utime_t timestamp;
        std::stringstream status;

        explicit Status(CephContext* _cct)
            : cct(_cct)
...
protected:
    ...
    int set_cr_error(int ret) { return set_state(RGWCoroutine_Error, ret); }
    int set_cr_done() { return set_state(RGWCoroutine_Done, 0); }
    void set_io_blocked(bool flag);

    void reset_description() { description.str(std::string()); }

    std::stringstream& set_description() { return description; }
    std::stringstream& set_status() { return status.set_status(); }
    ...
    virtual int operate_wrapper(const DoutPrefixProvider* dpp) { return operate(dpp); }

public:
    RGWCoroutine(CephContext* _cct)
        : status(_cct)
        , _yield_ret(false)
        , cct(_cct)
        , stack(NULL)
        , retcode(0)
        , state(RGWCoroutine_Run)
    {}
    ...
    std::string error_str() { return error_stream.str(); }

    void set_retcode(int r) { retcode = r; }
    ...
    bool collect(int* ret, RGWCoroutinesStack* skip_stack,
                 uint64_t* stack_id = nullptr); /* returns true if needs to be called again */
    bool collect_next(int* ret,
                      RGWCoroutinesStack** collected_stack = NULL); /* returns true if found a stack to collect */

    int wait(const utime_t& interval);
    bool drain_children(int num_cr_left, RGWCoroutinesStack* skip_stack = nullptr,
                        std::optional<std::function<void(uint64_t stack_id, int ret)> > cb =
                            std::nullopt); /* returns true if needed to be called again,
                                              cb will be called on completion of every
                                              completion. */
    bool drain_children(int num_cr_left, std::optional<std::function<int(uint64_t stack_id, int ret)> >
                                             cb); /* returns true if needed to be called again,
    ...
    size_t num_spawned() { return spawned.entries.size(); }

    void wait_for_child();

    virtual std::string to_str() const;

    RGWCoroutinesStack* get_stack() const { return stack; }
    ...
    int io_block(int ret = 0) { return io_block(ret, -1); }
    int io_block(int ret, int64_t io_id);
    int io_block(int ret, const rgw_io_id& io_id);
    void io_complete() { io_complete(rgw_io_id{}); }
...
#define drain_all_cb(cb) drain_with_cb(0, cb)
...
template<class T> class RGWConsumerCR : public RGWCoroutine
{
    std::list<T> product;

public:
    explicit RGWConsumerCR(CephContext* _cct)
        : RGWCoroutine(_cct)
    {}

    bool has_product() { return !product.empty(); }
...
protected:
    ...
    bool collect(RGWCoroutine* op, int* ret, RGWCoroutinesStack* skip_stack,
                 uint64_t* stack_id); /* returns true if needs to be called again */
    bool collect_next(RGWCoroutine* op, int* ret,
                      RGWCoroutinesStack** collected_stack); /* returns true if found a stack to collect */
public:
    ...
    bool is_error() { return error_flag; }
    bool is_blocked_by_stack() { return !blocked_by_stack.empty(); }
    void set_io_blocked(bool flag) { blocked_flag = flag; }
    void set_io_blocked_id(const rgw_io_id& io_id) { io_blocked_id = io_id; }
    bool is_io_blocked() { return blocked_flag && !done_flag; }
    bool can_io_unblock(const rgw_io_id& io_id) { return ((io_blocked_id.id < 0) || io_blocked_id.intersects(io_id)); }
    bool try_io_unblock(const rgw_io_id& io_id);
    bool consume_io_finish(const rgw_io_id& io_id);
    void set_interval_wait(bool flag) { interval_wait_flag = flag; }
    bool is_interval_waiting() { return interval_wait_flag; }
    ...
    bool is_sleeping() { return sleep_flag; }
    void set_is_scheduled(bool flag) { is_scheduled = flag; }

    bool is_blocked() { return is_blocked_by_stack() || is_sleeping() || is_io_blocked() || waiting_for_child(); }
    ...
    template<typename T> RGWAioCompletionNotifier* create_completion_notifier(T value);
    ...
    void set_wait_for_child(bool flag) { is_waiting_for_child = flag; }

    bool waiting_for_child() { return is_waiting_for_child; }
...
template<class T> void RGWConsumerCR<T>::receive(std::list<T>& l, bool wakeup)
{
...
template<class T> void RGWConsumerCR<T>::receive(const T& p, bool wakeup)
{
    product.push_back(p);
    if (wakeup) {
        set_sleeping(false);
...
class RGWCoroutinesManagerRegistry : public RefCountedObject, public AdminSocketHook
{
    ...
    ceph::shared_mutex lock = ceph::make_shared_mutex("RGWCoroutinesRegistry::lock");

    std::string admin_command;

public:
    explicit RGWCoroutinesManagerRegistry(CephContext* _cct)
        : cct(_cct)
    {}
...
class RGWCoroutinesManager
{
    ...
    mutable ceph::shared_mutex lock = ceph::make_shared_mutex("RGWCoroutinesManager::lock");

    RGWIOIDProvider io_id_provider;

    void handle_unblocked_stack(std::set<RGWCoroutinesStack*>& context_stacks,
                                std::list<RGWCoroutinesStack*>& scheduled_stacks,
...
public:
    RGWCoroutinesManager(CephContext* _cct, RGWCoroutinesManagerRegistry* _cr_registry)
        : cct(_cct)
        , cr_registry(_cr_registry)
    ...
    template<typename T> RGWAioCompletionNotifier* create_completion_notifier(RGWCoroutinesStack* stack, T value);
    ...
    RGWIOIDProvider& get_io_id_provider() { return io_id_provider; }
};
...
RGWAioCompletionNotifier* RGWCoroutinesManager::create_completion_notifier(RGWCoroutinesStack* stack, T value)
{
    ...
    RGWAioCompletionNotifier* cn =
        new RGWAioCompletionNotifierWith<T>(completion_mgr, io_id, (void*)stack, std::move(value));
    completion_mgr->register_completion_notifier(cn);
    return cn;
}

template<typename T> RGWAioCompletionNotifier* RGWCoroutinesStack::create_completion_notifier(T value)
{
...
public:
    RGWSimpleCoroutine(CephContext* _cct)
        : RGWCoroutine(_cct)
