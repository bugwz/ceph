...
void RGWPutUserPolicy::execute(optional_yield y)
{
    ...
    try {
        ...
        op_ret = store->ctl()->user->store_info(
            s, info, s->yield, RGWUserCtl::PutParams().set_objv_tracker(&objv_tracker).set_attrs(&uattrs));
        if (op_ret < 0) {
...
void RGWDeleteUserPolicy::execute(optional_yield y)
{
    ...
    rgw_user user_id(user_name);
    op_ret =
        store->ctl()->user->get_info_by_uid(s, user_id, &info, s->yield, RGWUserCtl::GetParams().set_attrs(&uattrs));
    if (op_ret < 0) {
        op_ret = -ERR_NO_SUCH_ENTITY;
        return;
    }
    ...
    if (auto it = uattrs.find(RGW_ATTR_USER_POLICY); it != uattrs.end()) {
        ...
        if (auto p = policies.find(policy_name); p != policies.end()) {
            ...
            op_ret = store->ctl()->user->store_info(
                ...
                info,
                s->yield,
                RGWUserCtl::PutParams().set_old_info(&info).set_objv_tracker(&objv_tracker).set_attrs(&uattrs));
            if (op_ret < 0) {
                op_ret = -ERR_INTERNAL_ERROR;
            }
            if (op_ret == 0) {
