    ...
    void dump(Formatter* f) const
    {
        ...
        for (auto i : attrs) {
            ...
            if (attr_name == RGW_ATTR_ACL) {
                ...
                try {
                    ...
                    ldout(cct, 0) << "ERROR: failed to decode obj tags for " << bucket_info.bucket << "/" << key
                                  << dendl;
                    continue;
                }
            }
                ...
                try {
                    ...
                    ldout(cct, 0) << "ERROR: failed to decode compression attr for " << bucket_info.bucket << "/" << key
                                  << dendl;
                    continue;
                }
        ...
        for (auto i : custom_meta) {
            ...
            if (config == bucket_info.mdsearch_config.end()) {
                if (!es_conf->explicit_custom_meta) {
                    ...
                    ldout(cct, 20) << "custom meta entry key=" << i.first
                                   << " not found in bucket mdsearch config: " << bucket_info.mdsearch_config << dendl;
                }
                continue;
            }
        ...
        if (!custom_date.empty()) {
            ...
            for (auto i : custom_date) {
                ...
                if (r < 0) {
                    ldout(cct, 20) << __func__ << "(): failed to parse time (" << i.second
...
public:
    RGWElasticDataSyncModule(CephContext* cct, const JSONFormattable& config)
    ...
    {
        ldout(sc->cct, 10) << conf->id << ": sync_object: b=" << sync_pipe.info.source_bs.bucket << " k=" << key
                                ...
                                bool versioned, uint64_t versioned_epoch, rgw_zone_set* zones_trace) override
    {
        ...
        ldout(sc->cct, 10) << conf->id << ": rm_object: b=" << sync_pipe.info.source_bs.bucket << " k=" << key
    ...
    {
        ldout(sc->cct, 10) << conf->id << ": create_delete_marker: b=" << sync_pipe.info.source_bs.bucket
...
int RGWElasticSyncModule::create_instance(CephContext* cct, const JSONFormattable& config,
