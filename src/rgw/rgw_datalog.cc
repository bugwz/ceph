...
public:
    ...
    int list(const DoutPrefixProvider* dpp, int index, int max_entries, std::vector<rgw_data_change_log_entry>& entries,
             std::optional<std::string_view> marker, std::string* out_marker, bool* truncated) override
    {
    ...
    int trim(const DoutPrefixProvider* dpp, int index, std::string_view marker, lr::AioCompletion* c) override
    {
        ...
        if (r == -ENOENT) r = -ENODATA;
        if (r < 0) {
            lderr(cct) << __PRETTY_FUNCTION__ << ": failed to get info from " << oids[index] << cpp_strerror(-r)
                       << dendl;
        }
        return r;
    }
    std::string_view max_marker() const override { return "99999999"sv; }
...
public:
    ...
    int push(const DoutPrefixProvider* dpp, int index, ceph::real_time, const std::string&,
             ceph::buffer::list&& bl) override
    {
             ...
             std::optional<std::string_view> marker, std::string* out_marker, bool* truncated) override
    {
        ...
        bool more = false;
        auto r = fifos[index].list(dpp, max_entries, marker, &log_entries, &more, null_yield);
        if (r < 0) {
    ...
    int is_empty(const DoutPrefixProvider* dpp) override
    {
        ...
        for (auto shard = 0u; shard < fifos.size(); ++shard) {
            auto r = fifos[shard].list(dpp, 1, {}, &log_entries, &more, null_yield);
            if (r < 0) {
                             ...
                             librados::Rados* lr)
{
    ...
    if (!besr) {
        ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": Error initializing backends: " << besr.error().message()
                           << dendl;
...
int RGWDataChangesLog::renew_entries(const DoutPrefixProvider* dpp)
{
    ...
    bc::flat_map<int, std::pair<std::vector<rgw_bucket_shard>, RGWDataChangesBE::entries>> m;
...
void RGWDataChangesLog::update_renewed(const rgw_bucket_shard& bs, real_time expiration)
{
    std::unique_lock l{lock};
                                    ...
                                    bool* ptruncated)
{
    ...
    for (; marker.shard < num_shards && int(entries.size()) < max_entries; marker.shard++, marker.marker.clear()) {
        int ret =
            list_entries(dpp, marker.shard, max_entries - entries.size(), entries, marker.marker, NULL, &truncated);
        if (ret == -ENOENT) {
            continue;
        }
        if (ret < 0) {
