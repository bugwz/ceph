...
int rgw_user_sync_all_stats(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, const rgw_user& user_id,
                            optional_yield y)
{
...
int rgw_user_get_all_buckets_stats(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                   const rgw_user& user_id, map<string, cls_user_bucket_entry>& buckets_usage_map,
                                   optional_yield y)
{
    CephContext* cct = store->ctx();
    size_t max_entries = cct->_conf->rgw_list_buckets_max_chunk;
    ...
    do {
        ...
        ret = rgw_read_user_buckets(dpp, store, user_id, buckets, marker, string(), max_entries, false, y);
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "failed to read user buckets: ret=" << ret << dendl;
            return ret;
        }
 ...
 */
int rgw_store_user_info(const DoutPrefixProvider* dpp, RGWUserCtl* user_ctl, RGWUserInfo& info, RGWUserInfo* old_info,
                        RGWObjVersionTracker* objv_tracker, real_time mtime, bool exclusive, optional_yield y,
                        map<string, bufferlist>* pattrs)
{
    return user_ctl->store_info(dpp,
                                info,
                                y,
 ...
 */
int rgw_get_user_info_by_uid(const DoutPrefixProvider* dpp, RGWUserCtl* user_ctl, const rgw_user& uid,
                             RGWUserInfo& info, optional_yield y, RGWObjVersionTracker* const objv_tracker,
                             real_time* const pmtime, rgw_cache_entry_info* const cache_info,
                             map<string, bufferlist>* const pattrs)
{
    return user_ctl->get_info_by_uid(dpp,
                                     uid,
                                     &info,
                                     y,
 ...
 * returns: 0 on success, -ERR# on failure (including nonexistence)
 */
int rgw_get_user_info_by_email(const DoutPrefixProvider* dpp, RGWUserCtl* user_ctl, string& email, RGWUserInfo& info,
                               optional_yield y, RGWObjVersionTracker* objv_tracker, real_time* pmtime)
{
    return user_ctl->get_info_by_email(
        dpp, email, &info, y, RGWUserCtl::GetParams().set_objv_tracker(objv_tracker).set_mtime(pmtime));
}
 ...
 * returns: 0 on success, -ERR# on failure (including nonexistence)
 */
int rgw_get_user_info_by_swift(const DoutPrefixProvider* dpp, RGWUserCtl* user_ctl, const string& swift_name,
                               RGWUserInfo& info, /* out */
                               optional_yield y, RGWObjVersionTracker* const objv_tracker, real_time* const pmtime)
{
    return user_ctl->get_info_by_swift(
        dpp, swift_name, &info, y, RGWUserCtl::GetParams().set_objv_tracker(objv_tracker).set_mtime(pmtime));
}
 ...
 * returns: 0 on success, -ERR# on failure (including nonexistence)
 */
extern int rgw_get_user_info_by_access_key(const DoutPrefixProvider* dpp, RGWUserCtl* user_ctl,
                                           const std::string& access_key, RGWUserInfo& info, optional_yield y,
                                           RGWObjVersionTracker* objv_tracker, real_time* pmtime)
{
    return user_ctl->get_info_by_access_key(
        dpp, access_key, &info, y, RGWUserCtl::GetParams().set_objv_tracker(objv_tracker).set_mtime(pmtime));
}
...
static struct rgw_flags_desc rgw_perms[] = {{RGW_PERM_FULL_CONTROL, "full-control"},
...
int rgw_validate_tenant_name(const string& t)
{
    struct tench
    {
        static bool is_good(char ch) { return isalnum(ch) || ch == '_'; }
    };
    std::string::const_iterator it = std::find_if_not(t.begin(), t.end(), tench::is_good);
...
static bool validate_access_key(string& key)
{
    ...
    while (*p) {
        if (!char_is_unreserved_url(*p)) return false;
        p++;
    }
    return true;
}
...
static void dump_user_info(Formatter* f, RGWUserInfo& info, RGWStorageStats* stats = NULL)
{
    ...
    case TYPE_RGW: user_source_type = "rgw"; break;
    case TYPE_KEYSTONE: user_source_type = "keystone"; break;
    case TYPE_LDAP: user_source_type = "ldap"; break;
    case TYPE_NONE: user_source_type = "none"; break;
    default: user_source_type = "none"; break;
    }
...
static int user_add_helper(RGWUserAdminOpState& op_state, std::string* err_msg)
{
    ...
    if (op_state.has_existing_user()) {
        if (op_state.found_by_email) {
            set_err_msg(err_msg, "email: " + user_email + " is the email address of an existing user");
            ret = -ERR_EMAIL_EXIST;
        }
...
bool RGWAccessKeyPool::check_existing_key(RGWUserAdminOpState& op_state)
{
    ...
    if (kid.empty() && swift_kid.empty()) return false;
    ...
    case KEY_TYPE_SWIFT:
        ...
        if (existing_key) op_state.set_access_key(swift_kid);
    ...
    default:
        ...
        if (swift_kid.empty()) return false;
...
int RGWAccessKeyPool::check_op(RGWUserAdminOpState& op_state, std::string* err_msg)
{
    RGWUserInfo dup_info;
    ...
    if (key_type == KEY_TYPE_S3 && !op_state.will_gen_access() && op_state.get_access_key().empty()) {
...
int RGWAccessKeyPool::generate_key(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, optional_yield y,
                                   std::string* err_msg)
{
    std::string id;
    std::string key;
    ...
    if (key_type == KEY_TYPE_S3 && gen_access) {
        ...
        do {
            ...
            id = public_id_buf;
            if (!validate_access_key(id)) continue;
...
int RGWAccessKeyPool::modify_key(RGWUserAdminOpState& op_state, std::string* err_msg)
{
    ...
    case KEY_TYPE_SWIFT:
        ...
        break;
    default: set_err_msg(err_msg, "invalid key type"); return -ERR_INVALID_KEY_TYPE;
    }
...
int RGWAccessKeyPool::execute_add(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, std::string* err_msg,
                                  bool defer_user_update, optional_yield y)
{
    ...
    // set the op
    if (op_state.has_existing_key()) key_op = MODIFY_KEY;

    switch (key_op) {
    case GENERATE_KEY: ret = generate_key(dpp, op_state, y, &subprocess_msg); break;
    ...
    if (!defer_user_update) ret = user->update(dpp, op_state, err_msg, y);
...
int RGWAccessKeyPool::add(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, optional_yield y,
                          std::string* err_msg)
{
    return add(dpp, op_state, err_msg, false, y);
}

int RGWAccessKeyPool::add(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, std::string* err_msg,
                          bool defer_user_update, optional_yield y)
{
    int ret;
    std::string subprocess_msg;
...
int RGWAccessKeyPool::execute_remove(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, std::string* err_msg,
                                     bool defer_user_update, optional_yield y)
{
    int ret = 0;
...
int RGWAccessKeyPool::remove(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, std::string* err_msg,
                             bool defer_user_update, optional_yield y)
{
    int ret;
...
int RGWAccessKeyPool::remove_subuser_keys(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state,
                                          std::string* err_msg, bool defer_user_update, optional_yield y)
{
    int ret = 0;
...
{
    if (subuser.empty()) return false;

    if (!subuser_map) return false;

    if (subuser_map->count(subuser)) return true;

    return false;
}

int RGWSubUserPool::check_op(RGWUserAdminOpState& op_state, std::string* err_msg)
{
    bool existing = false;
    ...
    if (!subuser.empty()) existing = exists(subuser);
...
int RGWSubUserPool::execute_add(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, std::string* err_msg,
                                bool defer_user_update, optional_yield y)
{
    int ret = 0;
    std::string subprocess_msg;
    ...
    if (op_state.has_subuser_perm()) subuser.perm_mask = op_state.get_subuser_perm();
...
int RGWSubUserPool::add(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, std::string* err_msg,
                        bool defer_user_update, optional_yield y)
{
    std::string subprocess_msg;
    int ret;
...
int RGWSubUserPool::execute_remove(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, std::string* err_msg,
                                   bool defer_user_update, optional_yield y)
{
    int ret = 0;
    std::string subprocess_msg;
...
int RGWSubUserPool::execute_modify(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, std::string* err_msg,
                                   bool defer_user_update, optional_yield y)
{
    int ret = 0;
    std::string subprocess_msg;
...
int RGWSubUserPool::modify(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, optional_yield y,
                           std::string* err_msg)
{
                        ...
                        bool defer_save, optional_yield y)
{
    ...
    if (!defer_save) ret = user->update(dpp, op_state, err_msg, y);
...
int RGWUser::init(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* storage, RGWUserAdminOpState& op_state,
                  optional_yield y)
{
    init_default();
                    ...
                    optional_yield y)
{
    ...
    ret = rgw_store_user_info(dpp, user_ctl, user_info, pold_info, &op_state.objv, real_time(), false, y);
    if (ret < 0) {
...
int RGWUser::check_op(RGWUserAdminOpState& op_state, std::string* err_msg)
{
    ...
    if (is_populated() && user_id.compare(uid) != 0) {
        set_err_msg(err_msg,
                    "user id mismatch, operation id: " + uid.to_str() + " does not match: " + user_id.to_str());
    ...
    if (ret) {
        set_err_msg(err_msg, "invalid tenant only alphanumeric and _ characters are allowed");
        return ret;
    }
...
static void rename_swift_keys(const rgw_user& user, std::map<std::string, RGWAccessKey>& keys)
{
    std::string user_id;
    user.to_str(user_id);
...
int RGWUser::execute_rename(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, std::string* err_msg,
                            optional_yield y)
{
    int ret;
    ...
    if (old_user.get_tenant() != new_user.get_tenant()) {
        set_err_msg(err_msg,
                    "users have to be under the same tenant namespace " + old_user.get_tenant() +
                        " != " + new_user.get_tenant());
        return -EINVAL;
    }
    ...
    ret = user_ctl->store_info(
        dpp, stub_user_info, y, RGWUserCtl::PutParams().set_objv_tracker(&objv).set_exclusive(exclusive));
    if (ret == -EEXIST) {
                         ...
                         optional_yield y)
{
    ...
    if (!user_email.empty()) user_info.user_email = user_email;
    ...
    if (op_state.op_mask_specified) user_info.op_mask = op_state.get_op_mask();
    ...
    if (op_state.temp_url_key_specified) {
        ...
        for (iter = op_state.temp_url_keys.begin(); iter != op_state.temp_url_keys.end(); ++iter) {
...
int RGWUser::rename(RGWUserAdminOpState& op_state, optional_yield y, const DoutPrefixProvider* dpp,
                    std::string* err_msg)
{
    std::string subprocess_msg;
    int ret;
...
int RGWUser::execute_remove(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, std::string* err_msg,
                            optional_yield y)
{
    int ret;
    ...
    size_t max_buckets = cct->_conf->rgw_list_buckets_max_chunk;
    do {
        ret = rgw_read_user_buckets(dpp, store, uid, buckets, marker, string(), max_buckets, false, y);
        if (ret < 0) {
    ...
    ret = user_ctl->remove_info(dpp, user_info, y, RGWUserCtl::RemoveParams().set_objv_tracker(&op_state.objv));
    if (ret < 0) {
...
int RGWUser::remove(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, optional_yield y,
                    std::string* err_msg)
{
    std::string subprocess_msg;
    int ret;
...
int RGWUser::execute_modify(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, std::string* err_msg,
                            optional_yield y)
{
    bool populated = op_state.is_populated();
    int ret = 0;
    ...
    if (!display_name.empty()) user_info.display_name = display_name;

    if (op_state.max_buckets_specified) user_info.max_buckets = op_state.get_max_buckets();

    if (op_state.admin_specified) user_info.admin = op_state.admin;

    if (op_state.system_specified) user_info.system = op_state.system;
    ...
    if (op_state.has_bucket_quota()) user_info.bucket_quota = op_state.get_bucket_quota();

    if (op_state.has_user_quota()) user_info.user_quota = op_state.get_user_quota();
    ...
    if (op_state.has_suspension_op()) {
        ...
        size_t max_buckets = cct->_conf->rgw_list_buckets_max_chunk;
        do {
            ret = rgw_read_user_buckets(dpp, store, user_id, buckets, marker, string(), max_buckets, false, y);
            if (ret < 0) {
...
int RGWUser::modify(const DoutPrefixProvider* dpp, RGWUserAdminOpState& op_state, optional_yield y,
                    std::string* err_msg)
{
    std::string subprocess_msg;
    int ret;
...
int RGWUserAdminOp_User::list(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
...
int RGWUserAdminOp_User::info(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                              RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    ...
    if (!op_state.has_existing_user()) return -ERR_NO_SUCH_USER;
...
int RGWUserAdminOp_User::create(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
    ...
    if (ret < 0) {
        if (ret == -EEXIST) ret = -ERR_USER_EXIST;
        return ret;
    }
...
int RGWUserAdminOp_User::modify(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
    ...
    if (ret < 0) {
        if (ret == -ENOENT) ret = -ERR_NO_SUCH_USER;
        return ret;
    }
...
int RGWUserAdminOp_User::remove(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Subuser::create(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                   RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Subuser::modify(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                   RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Subuser::remove(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                   RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Key::create(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                               RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Key::remove(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                               RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Caps::add(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                             RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Caps::remove(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
public:
               ...
               const DoutPrefixProvider* dpp) override
    {
        ...
        int ret =
            svc.user->read_user_info(op->ctx(), user, &uci.info, &objv_tracker, &mtime, nullptr, &uci.attrs, y, dpp);
        if (ret < 0) {
            return ret;
        }
                  ...
                  optional_yield y, const DoutPrefixProvider* dpp) override
    {
        ...
        int ret = svc.user->read_user_info(op->ctx(), user, &info, nullptr, nullptr, nullptr, nullptr, y, dpp);
        if (ret < 0) {
            return ret;
        }

        return svc.user->remove_user_info(op->ctx(), info, &objv_tracker, y, dpp);
    }
};
...
public:
    RGWMetadataHandlerPut_User(RGWUserMetadataHandler* _handler, RGWSI_MetaBackend_Handler::Op* op, string& entry,
                               RGWMetadataObject* obj, RGWObjVersionTracker& objv_tracker, optional_yield y,
...
int RGWUserMetadataHandler::do_put(RGWSI_MetaBackend_Handler::Op* op, string& entry, RGWMetadataObject* obj,
                                   RGWObjVersionTracker& objv_tracker, optional_yield y, const DoutPrefixProvider* dpp,
                                   RGWMDLogSyncType type, bool from_remote_zone)
{
...
int RGWMetadataHandlerPut_User::put_checked(const DoutPrefixProvider* dpp)
{
    ...
    int ret = uhandler->svc.user->store_user_info(
        op->ctx(), uci.info, pold_info, &objv_tracker, mtime, false, pattrs, y, dpp);
    if (ret < 0) {
        return ret;
    }
...
RGWUserCtl::RGWUserCtl(RGWSI_Zone* zone_svc, RGWSI_User* user_svc, RGWUserMetadataHandler* _umhandler)
...
template<class T> class optional_default
{
...
public:
    optional_default(const std::optional<T>& _o)
        : opt(_o)
    {
        if (opt) {
            p = &(*opt);
...
int RGWUserCtl::get_info_by_uid(const DoutPrefixProvider* dpp, const rgw_user& uid, RGWUserInfo* info, optional_yield y,
                                const GetParams& params)

{
    return be_handler->call([&](RGWSI_MetaBackend_Handler::Op* op) {
        return svc.user->read_user_info(
            op->ctx(), uid, info, params.objv_tracker, params.mtime, params.cache_info, params.attrs, y, dpp);
    });
}

int RGWUserCtl::get_info_by_email(const DoutPrefixProvider* dpp, const string& email, RGWUserInfo* info,
                                  optional_yield y, const GetParams& params)
{
    return be_handler->call([&](RGWSI_MetaBackend_Handler::Op* op) {
        return svc.user->get_user_info_by_email(op->ctx(), email, info, params.objv_tracker, params.mtime, y, dpp);
    });
}

int RGWUserCtl::get_info_by_swift(const DoutPrefixProvider* dpp, const string& swift_name, RGWUserInfo* info,
                                  optional_yield y, const GetParams& params)
{
    return be_handler->call([&](RGWSI_MetaBackend_Handler::Op* op) {
        return svc.user->get_user_info_by_swift(op->ctx(), swift_name, info, params.objv_tracker, params.mtime, y, dpp);
    });
}

int RGWUserCtl::get_info_by_access_key(const DoutPrefixProvider* dpp, const string& access_key, RGWUserInfo* info,
                                       optional_yield y, const GetParams& params)
{
    return be_handler->call([&](RGWSI_MetaBackend_Handler::Op* op) {
        return svc.user->get_user_info_by_access_key(
            op->ctx(), access_key, info, params.objv_tracker, params.mtime, y, dpp);
    });
}

int RGWUserCtl::get_attrs_by_uid(const DoutPrefixProvider* dpp, const rgw_user& user_id,
                                 map<string, bufferlist>* pattrs, optional_yield y, RGWObjVersionTracker* objv_tracker)
{
    RGWUserInfo user_info;

    return get_info_by_uid(
        dpp, user_id, &user_info, y, RGWUserCtl::GetParams().set_attrs(pattrs).set_objv_tracker(objv_tracker));
}

int RGWUserCtl::store_info(const DoutPrefixProvider* dpp, const RGWUserInfo& info, optional_yield y,
                           const PutParams& params)
{
    string key = RGWSI_User::get_meta_key(info.user_id);

    return be_handler->call([&](RGWSI_MetaBackend_Handler::Op* op) {
        return svc.user->store_user_info(op->ctx(),
                                         info,
                                         params.old_info,
                                         params.objv_tracker,
                                         params.mtime,
...
int RGWUserCtl::remove_info(const DoutPrefixProvider* dpp, const RGWUserInfo& info, optional_yield y,
...

{
    ...
    return be_handler->call([&](RGWSI_MetaBackend_Handler::Op* op) {
        return svc.user->remove_user_info(op->ctx(), info, params.objv_tracker, y, dpp);
    });
}

int RGWUserCtl::add_bucket(const DoutPrefixProvider* dpp, const rgw_user& user, const rgw_bucket& bucket,
                           ceph::real_time creation_time, optional_yield y)
...
int RGWUserCtl::remove_bucket(const DoutPrefixProvider* dpp, const rgw_user& user, const rgw_bucket& bucket,
                              optional_yield y)

{
    return be_handler->call(
        [&](RGWSI_MetaBackend_Handler::Op* op) { return svc.user->remove_bucket(dpp, op->ctx(), user, bucket, y); });
}

int RGWUserCtl::list_buckets(const DoutPrefixProvider* dpp, const rgw_user& user, const string& marker,
                             const string& end_marker, uint64_t max, bool need_stats, RGWUserBuckets* buckets,
                             bool* is_truncated, optional_yield y, uint64_t default_max)
{
    if (!max) {
        max = default_max;
    }
    ...
    return be_handler->call([&](RGWSI_MetaBackend_Handler::Op* op) {
        int ret = svc.user->list_buckets(dpp, op->ctx(), user, marker, end_marker, max, buckets, is_truncated, y);
        if (ret < 0) {
            return ret;
        }
        if (need_stats) {
...
int RGWUserCtl::flush_bucket_stats(const DoutPrefixProvider* dpp, const rgw_user& user, const RGWBucketEnt& ent,
                                   optional_yield y)
{
    return be_handler->call(
        [&](RGWSI_MetaBackend_Handler::Op* op) { return svc.user->flush_bucket_stats(dpp, op->ctx(), user, ent, y); });
}

int RGWUserCtl::complete_flush_stats(const DoutPrefixProvider* dpp, const rgw_user& user, optional_yield y)
{
    return be_handler->call(
        [&](RGWSI_MetaBackend_Handler::Op* op) { return svc.user->complete_flush_stats(dpp, op->ctx(), user, y); });
}

int RGWUserCtl::reset_stats(const DoutPrefixProvider* dpp, const rgw_user& user, optional_yield y)
{
    return be_handler->call(
        [&](RGWSI_MetaBackend_Handler::Op* op) { return svc.user->reset_bucket_stats(dpp, op->ctx(), user, y); });
}

int RGWUserCtl::read_stats(const DoutPrefixProvider* dpp, const rgw_user& user, RGWStorageStats* stats,
                           optional_yield y, ceph::real_time* last_stats_sync, ceph::real_time* last_stats_update)
{
    return be_handler->call([&](RGWSI_MetaBackend_Handler::Op* op) {
        return svc.user->read_stats(dpp, op->ctx(), user, stats, last_stats_sync, last_stats_update, y);
    });
}
...
{
    return be_handler->call(
        [&](RGWSI_MetaBackend_Handler::Op* op) { return svc.user->read_stats_async(dpp, op->ctx(), user, cb); });
}
