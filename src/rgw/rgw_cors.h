...
#define RGW_CORS_ALL    (RGW_CORS_GET | RGW_CORS_PUT | RGW_CORS_HEAD | RGW_CORS_POST | RGW_CORS_DELETE | RGW_CORS_COPY)
...
public:
    RGWCORSRule()
        ...
        , allowed_methods(0)
    {}
    RGWCORSRule(std::set<std::string>& o, std::set<std::string>& h, std::list<std::string>& e, uint8_t f, uint32_t a)
        : max_age(a)
        , allowed_methods(f)
        , allowed_hdrs(h)
        , allowed_origins(o)
...
public:
    ...
    void dump(Formatter* f) const;
    std::list<RGWCORSRule>& get_rules() { return rules; }
    bool is_empty() { return rules.empty(); }
    ...
    void dump();
    void stack_rule(RGWCORSRule& r) { rules.push_front(r); }
};
...
static inline int validate_name_string(std::string_view o)
{
    ...
    if (o.find_first_of("*") != o.find_last_of("*")) return -1;
    return 0;
}
