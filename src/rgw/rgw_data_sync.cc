...
bool RGWReadDataSyncRecoveringShardsCR::spawn_next()
{
    ...
    spawn(new RGWRadosGetOmapKeysCR(env->store,
...
int RGWReadDataSyncStatusCoroutine::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        yield call(new ReadMarkersCR(sc, sync_status->sync_info.num_shards, sync_status->sync_markers));
        if (retcode < 0) {
...
public:
    RGWReadRemoteDataLogShardCR(RGWDataSyncCtx* _sc, int _shard_id, const std::string& marker, string* pnext_marker,
                                list<rgw_data_change_log_entry>* _entries, bool* _truncated)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , shard_id(_shard_id)
...
public:
    RGWInitDataSyncStatusCoroutine(RGWDataSyncCtx* _sc, uint32_t num_shards, uint64_t instance_id,
                                   RGWSyncTraceNodeRef& _tn_parent, rgw_data_sync_status* status)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
...
#define COOKIE_LEN 16
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        ...
        reenter(this)
        {
            ...
            yield call(new LockCR(
                sync_env->async_rados, store, rgw_raw_obj{pool, sync_status_oid}, lock_name, cookie, lock_duration));
            if (retcode < 0) {
                tn->log(0, SSTR("ERROR: failed to take a lock on " << sync_status_oid));
                return set_cr_error(retcode);
            }
            using WriteInfoCR = RGWSimpleRadosWriteCR<rgw_data_sync_info>;
            yield call(new WriteInfoCR(dpp,
                                       sync_env->async_rados,
                                       sync_env->svc->sysobj,
            ...
            yield call(new RGWSimpleRadosUnlockCR(
                sync_env->async_rados, store, rgw_raw_obj{pool, sync_status_oid}, lock_name, cookie));
            return set_cr_done();
        }
        return 0;
    }
...
RGWRemoteDataLog::RGWRemoteDataLog(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                   RGWAsyncRadosProcessor* async_rados)
    : RGWCoroutinesManager(store->ctx(), store->getRados()->get_cr_registry())
    , dpp(dpp)
    , store(store)
    , cct(store->ctx())
    , cr_registry(store->getRados()->get_cr_registry())
...
{
    sync_env.init(
        dpp, cct, store, store->svc(), async_rados, &http_manager, _error_logger, _sync_tracer, _sync_module, counters);
...
public:
    RGWListBucketIndexesCR(RGWDataSyncCtx* _sc, rgw_data_sync_status* _sync_status)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , store(sync_env->store)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            entries_index = new RGWShardedOmapCRManager(sync_env->async_rados,
                                                        store,
                                                        this,
                                                        num_shards,
                        ...
                        for (i = 0; i < num_shards; i++) {
                            ...
                            yield entries_index->append(s,
                                                        sync_env->svc->datalog_rados->get_log_shard_id(
                                                            meta_info.data.get_bucket_info().bucket, i));
                        }
                    }
                    else {
                        yield entries_index->append(key,
                                                    sync_env->svc->datalog_rados->get_log_shard_id(
                                                        meta_info.data.get_bucket_info().bucket, -1));
                    }
                }
...
public:
    RGWRunBucketSyncCoroutine(RGWDataSyncCtx* _sc, boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
                              const rgw_bucket_sync_pair_info& _sync_pair, const RGWSyncTraceNodeRef& _tn_parent,
                              ceph::real_time* progress)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
...
public:
    RGWRunBucketsSyncBySourceCR(RGWDataSyncCtx* _sc, const rgw_bucket_shard& _source,
                                const RGWSyncTraceNodeRef& _tn_parent)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , source(_source)
        , tn(sync_env->sync_tracer->add_node(_tn_parent, "source", SSTR(bucket_shard_str{_source})))
    {}
...
public:
    RGWRunBucketSourcesSyncCR(RGWDataSyncCtx* _sc, boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
                              std::optional<rgw_bucket_shard> _target_bs, std::optional<rgw_bucket_shard> _source_bs,
    ...
    void handle_complete_stack(uint64_t stack_id)
    {
        ...
        if (iter == shard_progress.end()) {
            lderr(cct) << "ERROR: RGWRunBucketSourcesSyncCR::handle_complete_stack(): stack_id=" << stack_id
...
public:
    RGWDataSyncSingleEntryCR(RGWDataSyncCtx* _sc, rgw::bucket_sync::Handle state, rgw_data_sync_obligation obligation,
        ...
        reenter(this)
        {
            if (state->obligation) {
                ...
                while ((state->obligation->timestamp == ceph::real_time() ||
                    ...
                    ldout(cct, 4) << "starting sync on " << bucket_shard_str{state->key} << ' ' << *state->obligation
                                  << dendl;
                ...
                tn->log(10,
                        SSTR("sync finished on " << bucket_shard_str{state->key} << " progress=" << progress << ' '
                                                 << complete << " r=" << retcode));
            }
            sync_status = retcode;
            ...
            if (sync_status < 0) {
                ...
                if (complete->timestamp != ceph::real_time{}) {
                    ...
                    yield call(rgw_error_repo_write_cr(
                        sync_env->store->svc()->rados, error_repo, complete->key, complete->timestamp));
                    if (retcode < 0) {
                ...
                yield call(rgw_error_repo_remove_cr(
                    sync_env->store->svc()->rados, error_repo, complete->key, complete->timestamp));
                if (retcode < 0) {
                    tn->log(0,
...
#define RETRY_BACKOFF_SECS_MAX     600
    ...
    {
        return rgw_bucket_parse_bucket_key(sync_env->cct, key, &bs.bucket, &bs.shard_id);
    }
    RGWCoroutine* sync_single_entry(const rgw_bucket_shard& src, const std::string& key, const std::string& marker,
...
public:
    RGWDataSyncShardCR(RGWDataSyncCtx* _sc, rgw_pool& _pool, uint32_t _shard_id, rgw_data_sync_marker& _marker,
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        ...
        while (true) {
            ...
            case rgw_data_sync_marker::IncrementalSync:
                ...
                return 0;
            default: return set_cr_error(-EIO);
            }
        }
        return 0;
    }
    ...
    void init_lease_cr()
    {
        ...
        lease_cr.reset(new RGWContinuousLeaseCR(
            sync_env->async_rados, store, rgw_raw_obj(pool, status_oid), lock_name, lock_duration, this));
        ...
        reenter(&full_cr)
        {
            ...
            do {
                ...
                yield call(new RGWRadosGetOmapValsCR(
                    sync_env->store, rgw_raw_obj(pool, oid), sync_marker.marker, max_entries, omapvals));
                if (retcode < 0) {
                    lease_cr->go_down();
                    drain_all();
                    return set_cr_error(retcode);
                ...
                for (; iter != entries.end(); ++iter) {
                    ...
                    if (!marker_tracker->start(iter->first, total_entries, entry_timestamp)) {
                        ...
                        yield_spawn_window(
                            sync_single_entry(source_bs, iter->first, iter->first, entry_timestamp, false),
                            spawn_window,
                            std::nullopt);
                    }
            ...
            yield
            {
                ...
                call(new RGWSimpleRadosWriteCR<rgw_data_sync_marker>(sync_env->dpp,
                                                                     sync_env->async_rados,
                                                                     sync_env->svc->sysobj,
        ...
        reenter(&incremental_cr)
        {
            ...
            do {
                ...
                for (modified_iter = current_modified.begin(); modified_iter != current_modified.end();
                    ...
                    spawn(sync_single_entry(source_bs, *modified_iter, string(), ceph::real_time{}, false), false);
                }
                ...
                if (error_retry_time <= ceph::coarse_real_clock::now()) {
                    ...
                    omapvals = std::make_shared<RGWRadosGetOmapValsCR::Result>();
                    yield call(new RGWRadosGetOmapValsCR(
                        sync_env->store, error_repo, error_marker, max_error_entries, omapvals));
                    ...
                    for (; iter != error_entries.end(); ++iter) {
                        ...
                        if (retcode < 0) {
                            ...
                            spawn(rgw_error_repo_remove_cr(
                                      sync_env->store->svc()->rados, error_repo, error_marker, entry_timestamp),
                                  false);
                            continue;
                        }
                        tn->log(20,
                                SSTR("handle error entry key=" << error_marker << " timestamp=" << entry_timestamp));
                        spawn(sync_single_entry(source_bs, error_marker, "", entry_timestamp, true), false);
                    }
                    if (!omapvals->more) {
                ...
                for (log_iter = log_entries.begin(); log_iter != log_entries.end(); ++log_iter) {
                    ...
                    if (!marker_tracker->start(log_iter->log_id, 0, log_iter->log_timestamp)) {
                        ...
                        yield_spawn_window(
                            sync_single_entry(
                                source_bs, log_iter->entry.key, log_iter->log_id, log_iter->log_timestamp, false),
                            spawn_window,
                            std::nullopt);
                    }
                }
...
public:
    RGWDataSyncShardControlCR(RGWDataSyncCtx* _sc, const rgw_pool& _pool, uint32_t _shard_id,
                              rgw_data_sync_marker& _marker, RGWSyncTraceNodeRef& _tn_parent)
                ...
                if ((rgw_data_sync_info::SyncState)sync_status.sync_info.state == rgw_data_sync_info::StateSync) {
                    ...
                    for (map<uint32_t, rgw_data_sync_marker>::iterator iter = sync_status.sync_markers.begin();
                        ...
                        RGWDataSyncShardControlCR* cr = new RGWDataSyncShardControlCR(
                            sc, sync_env->svc->zone->get_zone_params().log_pool, iter->first, iter->second, tn);
                        cr->get();
                        shard_crs_lock.lock();
...
public:
    ...
    RGWCoroutine* sync_object(RGWDataSyncCtx* sc, rgw_bucket_sync_pipe& sync_pipe, rgw_obj_key& key,
                              std::optional<uint64_t> versioned_epoch, rgw_zone_set* zones_trace) override;
    RGWCoroutine* remove_object(RGWDataSyncCtx* sc, rgw_bucket_sync_pipe& sync_pipe, rgw_obj_key& key, real_time& mtime,
                                bool versioned, uint64_t versioned_epoch, rgw_zone_set* zones_trace) override;
    RGWCoroutine* create_delete_marker(RGWDataSyncCtx* sc, rgw_bucket_sync_pipe& sync_pipe, rgw_obj_key& key,
                                       real_time& mtime, rgw_bucket_entry_owner& owner, bool versioned,
                                       uint64_t versioned_epoch, rgw_zone_set* zones_trace) override;
};
...
int RGWDefaultSyncModule::create_instance(CephContext* cct, const JSONFormattable& config,
                ...
                if (param_mode == rgw_sync_pipe_params::MODE_USER) {
                    ...
                    if (!dest_bucket_perms.verify_bucket_permission(RGW_PERM_WRITE)) {
                        ldout(cct, 0) << "ERROR: " << __func__
                                      ...
                                      << sync_pipe.info.dest_bs.bucket.get_key() << ")" << dendl;
                        return -EPERM;
                    }
...
RGWCoroutine* RGWDefaultDataSyncModule::sync_object(RGWDataSyncCtx* sc, rgw_bucket_sync_pipe& sync_pipe,
...
RGWCoroutine* RGWDefaultDataSyncModule::remove_object(RGWDataSyncCtx* sc, rgw_bucket_sync_pipe& sync_pipe,
                                                      rgw_obj_key& key, real_time& mtime, bool versioned,
                                                      uint64_t versioned_epoch, rgw_zone_set* zones_trace)
{
...
RGWCoroutine* RGWDefaultDataSyncModule::create_delete_marker(RGWDataSyncCtx* sc, rgw_bucket_sync_pipe& sync_pipe,
                                                             rgw_obj_key& key, real_time& mtime,
                                                             rgw_bucket_entry_owner& owner, bool versioned,
                                                             uint64_t versioned_epoch, rgw_zone_set* zones_trace)
{
    auto sync_env = sc->env;
    return new RGWRemoveObjCR(sync_env->dpp,
...
int RGWArchiveSyncModule::create_instance(CephContext* cct, const JSONFormattable& config,
                                          RGWSyncModuleInstanceRef* instance)
{
...
RGWCoroutine* RGWArchiveDataSyncModule::sync_object(RGWDataSyncCtx* sc, rgw_bucket_sync_pipe& sync_pipe,
                                                    rgw_obj_key& key, std::optional<uint64_t> versioned_epoch,
                                                    rgw_zone_set* zones_trace)
{
    auto sync_env = sc->env;
    ...
    if (!sync_pipe.dest_bucket_info.versioned() || (sync_pipe.dest_bucket_info.flags & BUCKET_VERSIONS_SUSPENDED)) {
        ...
        int op_ret = sync_env->store->getRados()->put_bucket_instance_info(
            sync_pipe.dest_bucket_info, false, real_time(), NULL, sync_env->dpp);
        if (op_ret < 0) {
...
RGWCoroutine* RGWArchiveDataSyncModule::remove_object(RGWDataSyncCtx* sc, rgw_bucket_sync_pipe& sync_pipe,
                                                      rgw_obj_key& key, real_time& mtime, bool versioned,
                                                      uint64_t versioned_epoch, rgw_zone_set* zones_trace)
{
...
RGWCoroutine* RGWArchiveDataSyncModule::create_delete_marker(RGWDataSyncCtx* sc, rgw_bucket_sync_pipe& sync_pipe,
                                                             rgw_obj_key& key, real_time& mtime,
                                                             rgw_bucket_entry_owner& owner, bool versioned,
                                                             uint64_t versioned_epoch, rgw_zone_set* zones_trace)
{
...
int RGWDataSyncStatusManager::init(const DoutPrefixProvider* dpp)
{
    ...
    int r =
        source_log.init(source_zone, conn, error_logger, store->getRados()->get_sync_tracer(), sync_module, counters);
    if (r < 0) {
...
public:
    RGWReadRemoteBucketIndexLogInfoCR(RGWDataSyncCtx* _sc, const rgw_bucket_shard& bs,
            ...
            yield
            {
                ...
                if (write_status) {
                    ...
                    call(new RGWSimpleRadosWriteAttrsCR(
                        dpp, sync_env->async_rados, sync_env->svc->sysobj, obj, attrs, &objv_tracker));
                }
                else {
...
RGWRemoteBucketManager::RGWRemoteBucketManager(const DoutPrefixProvider* _dpp, RGWDataSyncEnv* _sync_env,
                                               const rgw_zone_id& _source_zone, RGWRESTConn* _conn,
                                               const RGWBucketInfo& source_bucket_info, const rgw_bucket& dest_bucket)
    : dpp(_dpp)
    , sync_env(_sync_env)
{
    conn = _conn;
    ...
    int num_shards = (source_bucket_info.layout.current_index.layout.normal.num_shards <= 0
                          ? 1
                          : source_bucket_info.layout.current_index.layout.normal.num_shards);
    ...
    for (int i = 0; i < num_shards; ++i, ++cur_shard) {
        ...
        sync_pair.source_bs.shard_id =
            (source_bucket_info.layout.current_index.layout.normal.num_shards > 0 ? cur_shard : -1);
...
int RGWReadRecoveringBucketShardsCoroutine::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        do {
            ...
            yield call(
                new RGWRadosGetOmapKeysCR(store,
...
public:
    RGWListBucketShardCR(RGWDataSyncCtx* _sc, const rgw_bucket_shard& bs, rgw_obj_key& _marker_position,
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            yield
            {
                ...
                call(new RGWReadRESTResourceCR<list<rgw_bi_log_entry> >(
...
public:
    RGWBucketFullSyncShardMarkerTrack(RGWDataSyncCtx* _sc, const string& _marker_oid,
                                      const rgw_bucket_shard_full_sync_marker& _marker, RGWSyncTraceNodeRef tn,
                                      RGWObjVersionTracker& objv_tracker)
    ...
    RGWCoroutine* store_marker(const rgw_obj_key& new_marker, uint64_t index_pos, const real_time& timestamp) override
    {
        ...
        tn->log(20, SSTR("updating marker marker_oid=" << marker_oid << " marker=" << new_marker));
        return new RGWSimpleRadosWriteAttrsCR(sync_env->dpp,
                                              sync_env->async_rados,
                                              sync_env->svc->sysobj,
                                              rgw_raw_obj(sync_env->svc->zone->get_zone_params().log_pool, marker_oid),
                                              attrs,
...
public:
    ...
    int operate(const DoutPrefixProvider* dpp)
    {
        reenter(this)
        {
            ...
            yield call(new RGWSimpleRadosWriteAttrsCR(
                sync_env->dpp, sync_env->async_rados, sync_env->svc->sysobj, obj, attrs, &objv_tracker));
            if (retcode < 0) {
                return set_cr_error(retcode);
            }
            if (stable_timestamp) {
                ...
                yield
                {
                    ...
                    if (error_injection &&
                        ...
                        call(data_sync_module->remove_object(
                            sc, sync_pipe, key, timestamp, versioned, versioned_epoch.value_or(0), &zones_trace));
                        ...
                        tn->log(
                            ...
                            SSTR("creating delete marker: obj: " << sc->source_zone << "/" << bs.bucket << "/" << key
                                                                 << "[" << versioned_epoch.value_or(0) << "]"));
                        call(data_sync_module->create_delete_marker(sc,
                                                                    sync_pipe,
                                                                    key,
                                                                    timestamp,
                                                                    owner,
...
public:
    RGWBucketShardFullSyncCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, const std::string& status_oid,
                             boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
                             rgw_bucket_shard_sync_info& sync_info, RGWSyncTraceNodeRef tn_parent,
                             RGWObjVersionTracker& objv_tracker)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
...
int RGWBucketShardFullSyncCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        do {
            ...
            yield call(new RGWListBucketShardCR(sc, bs, list_marker, &list_result));
            if (retcode < 0 && retcode != -ENOENT) {
            ...
            for (; entries_iter != list_result.entries.end(); ++entries_iter) {
                ...
                drain_with_cb(BUCKET_SYNC_SPAWN_WINDOW, [&](uint64_t stack_id, int ret) {
                    if (ret < 0) {
...
public:
    RGWBucketShardIncrementalSyncCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe,
                                    const std::string& status_oid,
                                    boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
                                    rgw_bucket_shard_sync_info& sync_info, RGWSyncTraceNodeRef& _tn_parent,
                                    RGWObjVersionTracker& objv_tracker, ceph::real_time* stable_timestamp)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        ...
        , marker_tracker(sc, status_oid, sync_info.inc_marker, tn, objv_tracker, stable_timestamp)
    {
...
int RGWBucketShardIncrementalSyncCR::operate(const DoutPrefixProvider* dpp)
{
    ...
    reenter(this)
    {
        do {
            ...
            yield call(new RGWListBucketIndexLogCR(sc, bs, sync_info.inc_marker.position, &list_result));
            if (retcode < 0 && retcode != -ENOENT) {
                /* wait for all operations to complete */
                drain_all();
                return set_cr_error(retcode);
...
int RGWRunBucketSourcesSyncCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        ldpp_dout(dpp, 20) << __func__ << "(): requested source_bs=" << source_bs << " target_bs=" << target_bs
                           << dendl;
        ...
        for (siter = pipes.begin(); siter != pipes.end(); ++siter) {
            ...
            for (; num_shards > 0; --num_shards, ++cur_shard) {
                ...
                yield_spawn_window(new RGWRunBucketSyncCoroutine(sc, lease_cr, sync_pair, tn, cur_progress),
...
int RGWSyncGetBucketInfoCR::operate(const DoutPrefixProvider* dpp)
{
    ...
    {
        yield call(
            new RGWGetBucketInstanceInfoCR(sync_env->async_rados, sync_env->store, bucket, pbucket_info, pattrs, dpp));
        if (retcode == -ENOENT) {
            ...
            yield call(new RGWGetBucketInstanceInfoCR(
                sync_env->async_rados, sync_env->store, bucket, pbucket_info, pattrs, dpp));
        }
        if (retcode < 0) {
        ...
        reenter(this)
        {
            for (i = 0; i < 2; ++i) {
                yield call(new RGWBucketGetSyncPolicyHandlerCR(
                    sync_env->async_rados, sync_env->store, get_policy_params, policy, dpp));
                if (retcode < 0 && retcode != -ENOENT) {
                    return set_cr_error(retcode);
                }
...
int RGWRunBucketSyncCoroutine::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        yield call(new RGWSyncGetBucketInfoCR(
            sync_env, sync_pair.source_bs.bucket, &sync_pipe.source_bucket_info, &sync_pipe.source_bucket_attrs, tn));
        if (retcode < 0) {
            tn->log(
        ...
        yield call(new RGWSyncGetBucketInfoCR(
            sync_env, sync_pair.dest_bs.bucket, &sync_pipe.dest_bucket_info, &sync_pipe.dest_bucket_attrs, tn));
        if (retcode < 0) {
            tn->log(
                0, SSTR("ERROR: failed to retrieve bucket info for bucket=" << bucket_str{sync_pair.source_bs.bucket}));
            drain_all();
        ...
        do {
            ...
            if (sync_status.state == rgw_bucket_shard_sync_info::StateFullSync) {
                yield call(
                    new RGWBucketShardFullSyncCR(sc, sync_pipe, status_oid, lease_cr, sync_status, tn, objv_tracker));
                if (retcode < 0) {
            ...
            if (sync_status.state == rgw_bucket_shard_sync_info::StateIncrementalSync) {
                yield call(new RGWBucketShardIncrementalSyncCR(
                    sc, sync_pipe, status_oid, lease_cr, sync_status, tn, objv_tracker, progress));
                if (retcode < 0) {
...
int RGWBucketPipeSyncStatusManager::init(const DoutPrefixProvider* dpp)
{
    ...
    sync_env.init(this,
                  ...
                  &http_manager,
                  error_logger,
                  store->getRados()->get_sync_tracer(),
                  sync_module,
                  nullptr);
    ...
    for (auto& pipe : pipes) {
        ...
        source_mgrs.push_back(new RGWRemoteBucketManager(
            this, &sync_env, szone, conn, pipe.source.get_bucket_info(), pipe.target.get_bucket()));
    }
...
int rgw_read_remote_bilog_info(const DoutPrefixProvider* dpp, RGWRESTConn* conn, const rgw_bucket& bucket,
                               BucketIndexShardsManager& markers, optional_yield y)
{
...
public:
    RGWCollectBucketSyncStatusCR(rgw::sal::RGWRadosStore* store, RGWDataSyncCtx* sc,
                                 const RGWBucketInfo& source_bucket_info, const RGWBucketInfo& dest_bucket_info,
                                 Vector* status)
        : RGWShardCollectCR(sc->cct, max_concurrent_shards)
        , store(store)
        , sc(sc)
        ...
        , end(status->end())
    {
        shard_to_shard_sync = (source_bucket_info.layout.current_index.layout.normal.num_shards ==
                               dest_bucket_info.layout.current_index.layout.normal.num_shards);

        source_bs = rgw_bucket_shard(source_bucket_info.bucket,
                                     source_bucket_info.layout.current_index.layout.normal.num_shards > 0 ? 0 : -1);
        dest_bs = rgw_bucket_shard(dest_bucket_info.bucket,
                                   dest_bucket_info.layout.current_index.layout.normal.num_shards > 0 ? 0 : -1);
...
int rgw_bucket_sync_status(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                           const rgw_sync_bucket_pipe& pipe, const RGWBucketInfo& dest_bucket_info,
                           const RGWBucketInfo* psource_bucket_info, std::vector<rgw_bucket_shard_sync_info>* status)
{
    if (!pipe.source.zone || !pipe.source.bucket || !pipe.dest.zone || !pipe.dest.bucket) {
        return -EINVAL;
    }

    if (*pipe.dest.bucket != dest_bucket_info.bucket) {
        return -EINVAL;
    }
    ...
    if (!psource_bucket_info) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: failed to get bucket instance info: bucket=" << source_bucket << ": "
    ...
    RGWCoroutinesManager crs(store->ctx(), store->getRados()->get_cr_registry());
    return crs.run(dpp, new RGWCollectBucketSyncStatusCR(store, &sc, *psource_bucket_info, dest_bucket_info, status));
}
