...
public:
    ...
    void generate_header(const std::string& dir_path, const std::string& css_path);
...
public:
    explicit RGWFormatterFlusher(Formatter* f)
        : formatter(f)
        , flushed(false)
        , started(false)
    {}
    ...
    {
        if (!started) do_start(client_ret);
        started = true;
    }
...
protected:
    void do_flush() override { formatter->flush(os); }

public:
    RGWStreamFlusher(Formatter* f, std::ostream& _os)
        : RGWFormatterFlusher(f)
        , os(_os)
    {}
};
