...
FlightFrontend::FlightFrontend(RGWProcessEnv& _env, RGWFrontendConfig* _config, int _port)
    : env(_env)
    , config(_config)
    , port(_port)
...
int FlightFrontend::init()
{
    ...
    const std::string url = std::string("grpc+tcp://localhost:") + std::to_string(port);
...
{
    try {
        flight_thread = make_named_thread(server_thread_name, &FlightServer::Serve, env.flight_server);

        INFO << "FlightServer thread started, id=" << flight_thread.get_id()
...
FlightGetObj_Filter::FlightGetObj_Filter(const req_state* request, RGWGetObj_Filter* next)
    ...
    schema_status(arrow::StatusCode::Cancelled, "schema determination incomplete")
...
FlightGetObj_Filter::~FlightGetObj_Filter()
{
    ...
    if (error) {
        ERROR << "FlightGetObj_Filter got error when removing temp file; "
                 "error="
              << error.value() << ", temp_file_name=" << temp_file_name << dendl;
    }
    else {
...
{
    INFO << "flight handling data from offset " << current_offset << " (" << bl_ofs << ") of size " << bl_len << dendl;
    ...
    if (temp_file.is_open()) {
        ...
        if (current_offset >= expected_size) {
            INFO << "data read is completed, current_offset=" << current_offset << ", expected_size=" << expected_size
                 << dendl;
            temp_file.close();
            ...
            if (!schema_status.ok()) {
                ...
                auto key = store->add_flight(FlightData(uri,
                                                        tenant_name,
                                                        bucket_name,
                                                        object_key,
                                                        num_rows,
