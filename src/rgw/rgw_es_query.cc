...
public:
    ESQueryNode(ESQueryCompiler* _compiler)
        : compiler(_compiler)
    {}
    virtual ~ESQueryNode() {}
...
public:
    explicit ESQueryNode_Bool(ESQueryCompiler* compiler)
        : ESQueryNode(compiler)
    {}
    ESQueryNode_Bool(ESQueryCompiler* compiler, const string& _op, ESQueryNode* _first, ESQueryNode* _second)
        : ESQueryNode(compiler)
        , op(_op)
        , first(_first)
        , second(_second)
    ...
    bool init(ESQueryStack* s, ESQueryNode** pnode, string* perr) override
    {
        ...
        valid = alloc_node(compiler, s, &first, perr) && alloc_node(compiler, s, &second, perr);
        if (!valid) {
            return false;
        }
        *pnode = this;
...
public:
    ...
    void encode_json(const string& field, Formatter* f) const override { ::encode_json(field.c_str(), val.c_str(), f); }
};
...
public:
    ...
    void encode_json(const string& field, Formatter* f) const override { ::encode_json(field.c_str(), val, f); }
};
...
protected:
    ...
    {
        switch (entity_type) {
        case ESEntityTypeMap::ES_ENTITY_DATE: val = new ESQueryNodeLeafVal_Date; break;
        case ESEntityTypeMap::ES_ENTITY_INT: val = new ESQueryNodeLeafVal_Int; break;
        default: val = new ESQueryNodeLeafVal_Str;
        }
...
public:
    ESQueryNode_Op(ESQueryCompiler* compiler)
        : ESQueryNode(compiler)
    {}
    ~ESQueryNode_Op() { delete val; }
    virtual bool init(ESQueryStack* s, ESQueryNode** pnode, string* perr) override
    {
        bool valid = s->pop(&op) && s->pop(&str_val) && s->pop(&field);
        if (!valid) {
    ...
    void set_allow_restricted(bool allow) { allow_restricted = allow; }
...
{
public:
    explicit ESQueryNode_Op_Equal(ESQueryCompiler* compiler)
        : ESQueryNode_Op(compiler)
    {}
    ESQueryNode_Op_Equal(ESQueryCompiler* compiler, const string& f, const string& v)
        : ESQueryNode_Op(compiler)
    {
        op = "==";
        field = f;
...
{
public:
    explicit ESQueryNode_Op_NotEqual(ESQueryCompiler* compiler)
        : ESQueryNode_Op(compiler)
    {}
    ESQueryNode_Op_NotEqual(ESQueryCompiler* compiler, const string& f, const string& v)
        : ESQueryNode_Op(compiler)
    {
        op = "!=";
        field = f;
...
public:
    ESQueryNode_Op_Range(ESQueryCompiler* compiler, const string& rs)
        : ESQueryNode_Op(compiler)
        , range_str(rs)
    {}
...
{
public:
    ESQueryNode_Op_Nested_Parent(ESQueryCompiler* compiler)
        : ESQueryNode_Op(compiler)
    {}
...
template<class T> class ESQueryNode_Op_Nested : public ESQueryNode_Op_Nested_Parent
{
    string name;
    ESQueryNode* next;

public:
    ESQueryNode_Op_Nested(ESQueryCompiler* compiler, const string& _name, ESQueryNode* _next)
        : ESQueryNode_Op_Nested_Parent(compiler)
        , name(_name)
        , next(_next)
    {}
    ~ESQueryNode_Op_Nested() { delete next; }
    ...
    string get_custom_leaf_field_name() override { return string("meta.custom-") + type_str() + ".value"; }
};

template<> string ESQueryNode_Op_Nested<string>::type_str() const
{
    return "string";
}

template<> string ESQueryNode_Op_Nested<int64_t>::type_str() const
{
    return "int";
}

template<> string ESQueryNode_Op_Nested<ceph::real_time>::type_str() const
{
    return "date";
}
...
bool ESQueryNode_Op::handle_nested(ESQueryNode** pnode, string* perr)
{
    ...
    if (!boost::algorithm::starts_with(field_name, custom_prefix)) {
        *pnode = this;
        ...
        if (m) {
            bool found =
                m->find(field_name, &entity_type) && (allow_restricted || !compiler->is_restricted(field_name));
            if (!found) {
    ...
    case ESEntityTypeMap::ES_ENTITY_DATE:
        ...
        break;
    default: new_node = new ESQueryNode_Op_Nested<string>(compiler, field_name, this);
    }
...
bool ESInfixQueryParser::parse_condition()
{
    ...
    /* parse key */
    bool valid = get_next_token(is_key_char) && get_next_token(is_op_char) && get_next_token(is_val_char);
