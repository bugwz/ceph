...
class RGWFrontendConfig
{
    ...
    int parse_config(const std::string& config, std::multimap<std::string, std::string>& config_map);
...
public:
    ...
    bool get_val(const std::string& key, const std::string& def_val, std::string* out);
    bool get_val(const std::string& key, int def_val, int* out);

    std::string get_val(const std::string& key, const std::string& def_val)
    {
        std::string out;
    ...
    const std::string& get_config() { return config; }

    std::multimap<std::string, std::string>& get_config_map() { return config_map; }

    std::string get_framework() const { return framework; }
};
...
public:
    ...
    void pause_for_new_config() override { pprocess->pause(); }

    void unpause_with_new_config() override { pprocess->unpause_with_new_config(); }
}; /* RGWProcessFrontend */
...
public:
    ...
    CephContext* get_cct() const { return env.driver->ctx(); }

    unsigned get_subsys() const { return ceph_subsys_rgw; }

    std::ostream& gen_prefix(std::ostream& out) const { return out << "rgw loadgen frontend: "; }
    ...
    int init() override
    {
        ...
        RGWLoadGenProcess* pp = new RGWLoadGenProcess(g_ceph_context, env, num_threads, std::move(uri_prefix), conf);
...
public:
    RGWFrontendPauser(std::vector<RGWFrontend*>& frontends, RGWRealmReloader::Pauser* pauser = nullptr)
        : frontends(frontends)
        , pauser(pauser)
    {}
    ...
    void pause() override
    {
        for (auto frontend : frontends) frontend->pause_for_new_config();
        if (pauser) pauser->pause();
    }
    void resume(rgw::sal::Driver* driver) override
    {
        for (auto frontend : frontends) frontend->unpause_with_new_config();
        if (pauser) pauser->resume(driver);
    }
};
