...
public:
    ...
    bool get_id(rgw_user& _id) const
    {
        ...
        case ACL_TYPE_EMAIL_USER:
            ...
            return true;
        case ACL_TYPE_GROUP:
        case ACL_TYPE_REFERER: return false;
        default: _id = id; return true;
        }
    }
    ...
    {
        switch (type.get_type()) {
        case ACL_TYPE_EMAIL_USER: email_id.from_str(email); return &email_id;
        case ACL_TYPE_GROUP:
        case ACL_TYPE_REFERER: return nullptr;
        default: return &id;
        }
    }
...
struct ACLReferer
{
    ...
    ACLReferer()
        : perm(0)
    {}
    ACLReferer(const std::string& url_spec, const uint32_t perm)
        : url_spec(url_spec)
        , perm(perm)
    {}
...
public:
    explicit RGWAccessControlList(CephContext* _cct)
        : cct(_cct)
    {}
    ...
    uint32_t get_perm(const DoutPrefixProvider* dpp, const rgw::auth::Identity& auth_identity, uint32_t perm_mask);
    uint32_t get_group_perm(const DoutPrefixProvider* dpp, ACLGroupTypeEnum group, uint32_t perm_mask) const;
    uint32_t get_referer_perm(const DoutPrefixProvider* dpp, uint32_t current_perm, std::string http_referer,
                              uint32_t perm_mask);
    void encode(bufferlist& bl) const
    {
        ENCODE_START(4, 3, bl);
...
public:
    explicit RGWAccessControlPolicy(CephContext* _cct)
        : cct(_cct)
        , acl(_cct)
    {}
    ...
    uint32_t get_perm(const DoutPrefixProvider* dpp, const rgw::auth::Identity& auth_identity, uint32_t perm_mask,
                      const char* http_referer, bool ignore_public_acls = false);
    bool verify_permission(const DoutPrefixProvider* dpp, const rgw::auth::Identity& auth_identity,
                           uint32_t user_perm_mask, uint32_t perm, const char* http_referer = nullptr,
    ...
    void decode_owner(bufferlist::const_iterator& bl)
    ...
    RGWAccessControlList& get_acl() { return acl; }
    const RGWAccessControlList& get_acl() const { return acl; }
