...
struct rgw_sync_symmetric_group
{
    ...
    rgw_sync_symmetric_group(const std::string& _id, const std::set<rgw_zone_id> _zones)
        : id(_id)
        , zones(_zones)
    {}
...
struct rgw_sync_bucket_entity
{
    ...
    static bool match_str(const std::string& s1, const std::string& s2)
    ...
    rgw_sync_bucket_entity(const rgw_zone_id& _zone, std::optional<rgw_bucket> _bucket)
        : zone(_zone)
        , bucket(_bucket.value_or(rgw_bucket()))
    {}

    bool specific() const { return zone && bucket; }
    ...
    rgw_bucket get_bucket() const { return bucket.value_or(rgw_bucket()); }
    ...
    bool match_bucket(std::optional<rgw_bucket> b) const
    {
        ...
        return (match_str(bucket->tenant, b->tenant) && match_str(bucket->name, b->name) &&
...
struct rgw_sync_pipe_filter_tag
{
    ...
    rgw_sync_pipe_filter_tag(const std::string& s) { from_str(s); }
    rgw_sync_pipe_filter_tag(const std::string& _key, const std::string& _value)
        : key(_key)
        , value(_value)
    {}
...
struct rgw_sync_pipe_acl_translation
{
    ...
    bool operator==(const rgw_sync_pipe_acl_translation& aclt) const { return (owner == aclt.owner); }
};
...
struct rgw_sync_pipe_dest_params
{
    ...
    {
        return (acl_translation == rhs.acl_translation && storage_class == rhs.storage_class);
    }
};
...
struct rgw_sync_bucket_entities
{
    ...
    void set_bucket(std::optional<std::string> tenant, std::optional<std::string> bucket_name,
                    std::optional<std::string> bucket_id);
    void remove_bucket(std::optional<std::string> tenant, std::optional<std::string> bucket_name,
                       std::optional<std::string> bucket_id);
...
struct rgw_sync_bucket_pipes
{
    ...
    void get_potential_related_buckets(const rgw_bucket& bucket, std::set<rgw_bucket>* sources,
...
struct rgw_sync_data_flow_group
{
    ...
    bool empty() const { return (symmetrical.empty() && directional.empty()); }
    ...
    bool find_or_create_directional(const rgw_zone_id& source_zone, const rgw_zone_id& dest_zone,
