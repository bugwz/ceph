...
RadosWriter::~RadosWriter()
{
    ...
    if (need_to_remove_head) {
        ...
        int r = head_obj->delete_object(
            dpp, &obj_ctx, ACLOwner(), bucket->get_acl_owner(), ceph::real_time(), false, 0, version_id, null_yield);
        if (r < 0 && r != -ENOENT) {
...
int AtomicObjectProcessor::prepare(optional_yield y)
{
    ...
    int r = head_obj->get_max_chunk_size(dpp, bucket->get_placement_rule(), &max_head_chunk_size, &alignment);
    if (r < 0) {
        return r;
    }
    ...
    r = manifest_gen.create_begin(
        store->ctx(), &manifest, bucket->get_placement_rule(), &tail_placement_rule, obj.bucket, obj);
    if (r < 0) {
        return r;
    }
...
int AtomicObjectProcessor::complete(size_t accounted_size, const std::string& etag, ceph::real_time* mtime,
                                    ceph::real_time set_mtime, rgw::sal::RGWAttrs& attrs, ceph::real_time delete_at,
