...
void RGWFormatter_Plain::open_array_section(std::string_view name)
{
    ...
    if (use_kv && min_stack_level > 0 && !stack.empty()) {
        ...
        if (!entry.is_array) dump_format(name, "");
    }
...
void RGWFormatter_Plain::open_object_section(std::string_view name)
{
    ...
    if (use_kv && min_stack_level > 0) dump_format(name, "");

    stack.push_back(new_entry);
}

void RGWFormatter_Plain::open_object_section_in_ns(std::string_view name, const char* ns)
{
    ostringstream oss;
    oss << name << " " << ns;
...
void RGWFormatter_Plain::dump_format_va(std::string_view name, const char* ns, bool quoted, const char* fmt, va_list ap)
{
    ...
    if (!min_stack_level) min_stack_level = stack.size();
...
#define LARGE_ENOUGH_BUF 4096
    ...
    pos = len;
    if (len) pos--;   // squash null termination
    strcpy(buf + pos, p);
    len = pos + strlen(p) + 1;
done_free:
    if (!p_on_stack) free(p);
}
...
void RGWSwiftWebsiteListingFormatter::generate_header(const std::string& dir_path, const std::string& css_path)
{
    ss << R"(<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 )"
       << R"(Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">)";

    ss << "<html><head><title>Listing of " << xml_stream_escaper(dir_path) << "</title>";

    if (!css_path.empty()) {
        ss << boost::format(R"(<link rel="stylesheet" type="text/css" href="%s" />)") % url_encode(css_path);
    }
    else {
...
std::string RGWSwiftWebsiteListingFormatter::format_name(const std::string& item_name) const
{
...
void RGWSwiftWebsiteListingFormatter::dump_object(const rgw_bucket_dir_entry& objent)
{
    ...
    ss << boost::format(R"(<tr class="item %s">)") % "default"
       << boost::format(R"(<td class="colname"><a href="%s">%s</a></td>)") % url_encode(name) % HTMLHelper::escape(name)
       << boost::format(R"(<td class="colsize">%lld</td>)") % objent.meta.size
       << boost::format(R"(<td class="coldate">%s</td>)") % dump_time_to_str(objent.meta.mtime) << R"(</tr>)";
}
...
void RGWSwiftWebsiteListingFormatter::dump_subdir(const std::string& name)
{
    ...
    ss << R"(<tr class="item subdir">)"
       << boost::format(R"(<td class="colname"><a href="%s">%s</a></td>)") % url_encode(fname) %
