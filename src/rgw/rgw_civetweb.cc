    ...
    , txbuf(*this)
{
    ...
    case AF_INET: port = ntohs(((struct sockaddr_in*)lsa)->sin_port); break;
    case AF_INET6: port = ntohs(((struct sockaddr_in6*)lsa)->sin6_port); break;
    default: port = -1;
    }
}
...
int RGWCivetWeb::init_env(CephContext* cct)
{
    ...
    if (port <= 0) lderr(cct) << "init_env: bug: invalid port number" << dendl;
    char port_buf[16];
...
size_t RGWCivetWeb::send_status(int status, const char* status_name)
{
    ...
    const auto statuslen = snprintf(statusbuf, sizeof(statusbuf), "HTTP/1.1 %d %s\r\n", status, status_name);
...
size_t RGWCivetWeb::send_100_continue()
{
    ...
    const size_t sent = txbuf.sputn(HTTTP_100_CONTINUE, sizeof(HTTTP_100_CONTINUE) - 1);
    flush();
    return sent;
}

size_t RGWCivetWeb::send_header(const std::string_view& name, const std::string_view& value)
{
...
size_t RGWCivetWeb::send_content_length(uint64_t len)
{
    ...
    const auto sizelen = snprintf(sizebuf, sizeof(sizebuf), "Content-Length: %" PRIu64 "\r\n", len);
