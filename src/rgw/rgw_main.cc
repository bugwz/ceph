...
int radosgw_Main(int argc, const char** argv)
{
    ...
    if (TEMP_FAILURE_RETRY(dup2(STDOUT_FILENO, STDERR_FILENO)) < 0) {
        int err = errno;
        cout << "failed to redirect stderr to stdout: " << cpp_strerror(err) << std::endl;
        return ENOSYS;
    }

    /* alternative default for module */
    map<string, string> defaults = {{"debug_rgw", "1/5"},
    ...
    auto cct = global_init(&defaults, args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_DAEMON, flags);
    ...
    rgw::sal::RGWRadosStore* store = RGWStoreManager::get_storage(&dp,
                                                                  g_ceph_context,
    ...
    if (!(g_ceph_context->_conf->rgw_keystone_admin_token.empty() ||
        ...
        dout(0) << "WARNING: rgw_keystone_admin_token and rgw_keystone_admin_password should be avoided as they can "
                   "expose secrets.  Prefer the new rgw_keystone_admin_token_path and rgw_keystone_admin_password_path "
                   "options, which read their secrets from files."
                << dendl;
    }
    ...
    if (apis_map.count("swift") > 0) {
        ...
        if (!swift_at_root) {
            rest.register_resource(g_conf()->rgw_swift_url_prefix,
                                   set_logging(rest_filter(store->getRados(), RGW_REST_SWIFT, swift_resource)));
        }
        else {
    ...
    if (cct->_conf.get_val<std::string>("rgw_scheduler_type") == "dmclock" &&
        ...
        derr << "dmclock scheduler type is experimental and needs to be"
             << "set in the option enable experimental data corrupting features" << dendl;
        return EINVAL;
    }
    ...
    auto reloader = std::make_unique<RGWRealmReloader>(store, service_map_meta, &pauser);
