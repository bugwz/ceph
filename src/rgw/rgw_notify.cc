...
public:
    ...
    int remove_persistent_topic(const std::string& topic_name, optional_yield y)
    {
        ...
        if (ret == -ENOENT) {
            ...
            ldpp_dout(this, 20) << "INFO: queue for topic: " << topic_name << " already removed. nothing to do"
                                << dendl;
            return 0;
        }
        if (ret < 0) {
            // failed to remove queue
            ldpp_dout(this, 1) << "ERROR: failed to remove queue for topic: " << topic_name << ". error: " << ret
                               << dendl;
            return ret;
        }
        ...
        ret = rgw_rados_operate(this, rados_ioctx, Q_LIST_OBJECT_NAME, &op, y);
        if (ret < 0) {
            ldpp_dout(this, 1) << "ERROR: failed to remove queue: " << topic_name << " from queue list. error: " << ret
                               << dendl;
            return ret;
        }
...
void populate_event_from_request(const reservation_t& res, rgw::sal::RGWObject* obj, uint64_t size,
                                 const ceph::real_time& mtime, const std::string& etag, EventType event_type,
                                 rgw_pubsub_s3_event& event)
{
    const auto s = res.s;
...
bool notification_match(reservation_t& res, const rgw_pubsub_topic_filter& filter, EventType event,
...
int publish_reserve(const DoutPrefixProvider* dpp, EventType event_type, reservation_t& res, const RGWObjTags* req_tags)
{
    ...
    for (const auto& bucket_topic : bucket_topics.topics) {
        ...
        ldpp_dout(dpp, 20) << "INFO: notification: '" << topic_filter.s3_id << "' on topic: '"
                           << topic_cfg.dest.arn_topic << "' and bucket: '" << res.s->bucket->get_name()
        ...
        if (topic_cfg.dest.persistent) {
            ...
            auto ret = rgw_rados_operate(dpp,
...
int publish_commit(rgw::sal::RGWObject* obj, uint64_t size, const ceph::real_time& mtime, const std::string& etag,
                   EventType event_type, reservation_t& res, const DoutPrefixProvider* dpp)
{
        ...
        if (topic.cfg.dest.persistent) {
            ...
            if (bl.length() > res.size) {
                ...
                auto ret = rgw_rados_operate(
                    dpp, res.store->getRados()->get_notif_pool_ctx(), topic.cfg.dest.arn_topic, &op, res.s->yield);
                if (ret < 0) {
                ...
                if (ret < 0) {
                    ldpp_dout(dpp, 1) << "ERROR: failed to parse reservation id for extra space. error: " << ret
                                      << dendl;
                    return ret;
                }
            }
            ...
            const auto ret =
                rgw_rados_operate(dpp, res.store->getRados()->get_notif_pool_ctx(), queue_name, &op, res.s->yield);
...
int publish_abort(const DoutPrefixProvider* dpp, reservation_t& res)
{
    for (auto& topic : res.topics) {
        ...
        const auto ret =
            rgw_rados_operate(dpp, res.store->getRados()->get_notif_pool_ctx(), queue_name, &op, res.s->yield);
        if (ret < 0) {
            ldpp_dout(dpp, 1) << "ERROR: failed to abort reservation: " << topic.res_id << " from queue: " << queue_name
                              << ". error: " << ret << dendl;
            return ret;
        }
        topic.res_id = cls_2pc_reservation::NO_ID;
...
reservation_t::reservation_t(const DoutPrefixProvider* _dpp, rgw::sal::RGWRadosStore* _store, const req_state* _s,
