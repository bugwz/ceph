...
public:
    canonical_char_sorter(CephContext* cct)
        : cct(cct)
    {
...
bool canonical_char_sorter<M>::make_string_canonical(rapidjson::Value& v, rapidjson::Document::AllocatorType& a)
{
    ...
    if (U_FAILURE(status)) {
        ldout(cct, 5) << "conversion error; code=" << status << " on string " << as << dendl;
        return false;
    }
    std::string ans;
    an.toUTF8String(ans);
...
int make_canonical_context(struct req_state* s, std::string_view& context, std::string& cooked_context)
{
    ...
    if (!b) {
        lderr(s->cct) << "ERROR: can't make canonical json <" << context << ">" << dendl;
...
CryptoAccelRef get_crypto_accel(CephContext* cct)
{
    ...
    if (err) {
        lderr(cct) << __func__ << " factory return error " << err << " with description: " << ss.str() << dendl;
    }
    return ca_impl;
}
...
static inline bool evp_sym_transform(CephContext* const cct, const EVP_CIPHER* const type, unsigned char* const out,
...
public:
    explicit AES_256_CBC(CephContext* cct)
        : cct(cct)
    {}
...
bool AES_256_ECB_encrypt(CephContext* cct, const uint8_t* key, size_t key_size, const uint8_t* data_in,
                         uint8_t* data_out, size_t data_size)
{
    if (key_size == AES_256_KEYSIZE) {
        return evp_sym_transform<AES_256_KEYSIZE, 0 /* no IV in ECB */>(
            cct, EVP_aes_256_ecb(), data_out, data_in, data_size, nullptr /* no IV in ECB */, key, true /* encrypt */);
    }
    else {
...
RGWGetObj_BlockDecrypt::RGWGetObj_BlockDecrypt(CephContext* cct, RGWGetObj_Filter* next,
...
int RGWGetObj_BlockDecrypt::fixup_range(off_t& bl_ofs, off_t& bl_end)
{
    ...
    ldout(cct, 20) << "fixup_range [" << inp_ofs << "," << inp_end << "] => [" << bl_ofs << "," << bl_end << "]"
                   << dendl;
    return 0;
}
...
RGWPutObj_BlockEncrypt::RGWPutObj_BlockEncrypt(CephContext* cct, rgw::putobj::DataProcessor* next,
                                               std::unique_ptr<BlockCrypt> crypt)
    : Pipe(next)
    , cct(cct)
    , crypt(std::move(crypt))
...
static std::string_view get_crypt_attribute(
    const RGWEnv* env, std::map<std::string, RGWPostObj_ObjStore::post_form_part, const ltstr_nocase>* parts,
    crypt_option_e option)
{
    ...
    if (parts != nullptr) {
        auto iter = parts->find(crypt_options[option].post_part_name);
        if (iter == parts->end()) return std::string_view();
...
int rgw_s3_prepare_encrypt(struct req_state* s, std::map<std::string, ceph::bufferlist>& attrs,
                           std::map<std::string, RGWPostObj_ObjStore::post_form_part, const ltstr_nocase>* parts,
                           ...
                           std::map<std::string, std::string>& crypt_http_responses)
{
    ...
    crypt_http_responses.clear();
    {
        ...
        if (!req_sse_ca.empty()) {
            ...
            try {
                key_bin =
                    from_base64(get_crypt_attribute(s->info.env, parts, X_AMZ_SERVER_SIDE_ENCRYPTION_CUSTOMER_KEY));
            }
            catch (...) {
        ...
        std::string_view req_sse = get_crypt_attribute(s->info.env, parts, X_AMZ_SERVER_SIDE_ENCRYPTION);
        if (!req_sse.empty()) {
            ...
            if (!key_id.empty()) {
                ldpp_dout(s, 5) << "ERROR: SSE-KMS encryption request is missing the header "
                                << "x-amz-server-side-encryption" << dendl;
                s->err.message = "Server Side Encryption with KMS managed key requires "
...
int rgw_s3_prepare_decrypt(struct req_state* s, map<string, bufferlist>& attrs,
                           std::unique_ptr<BlockCrypt>* block_crypt,
                           std::map<std::string, std::string>& crypt_http_responses)
{
    int res = 0;
