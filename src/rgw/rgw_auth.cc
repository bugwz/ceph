                                                            ...
                                                            const uint32_t type)
{
    ...
    public:
        DummyIdentityApplier(CephContext* const cct, const rgw_user& auth_id, const int perm_mask, const bool is_admin,
                             const uint32_t type)
            : cct(cct)
            , id(auth_id)
            , perm_mask(perm_mask)
        ...
        bool is_admin_of(const rgw_user& acct_id) const override { return is_admin; }

        bool is_owner_of(const rgw_user& acct_id) const override { return id == acct_id; }
        ...
        bool is_identity(const idset_t& ids) const override
        {
            for (auto& p : ids) {
                ...
                else if (p.is_user() && (p.get_tenant() == id.tenant) && (p.get_id() == id.id)) {
                    return true;
                }
            }
            return false;
        ...
        uint32_t get_identity_type() const override { return type; }
        ...
        {
            out << "RGWDummyIdentityApplier(auth_id=" << id << ", perm_mask=" << perm_mask << ", is_admin=" << is_admin
                << ")";
        }
    };

    return std::unique_ptr<rgw::auth::Identity>(new DummyIdentityApplier(cct, auth_id, perm_mask, is_admin, type));
}
...
static inline std::pair<bool, rgw::auth::Engine::result_t> strategy_handle_rejected(
    rgw::auth::Engine::result_t&& engine_result, const rgw::auth::Strategy::Control policy,
    ...
    rgw::auth::Engine::result_t&& strategy_result)
{
    ...
    case Control::FALLBACK: return std::make_pair(true, std::move(strategy_result));
...
rgw::auth::Engine::result_t rgw::auth::Strategy::authenticate(const DoutPrefixProvider* dpp, const req_state* const s,
    ...
    for (const stack_item_t& kv : auth_stack) {
        ...
        case result_t::Status::REJECTED:
        {
            ...
            std::tie(try_next, strategy_result) =
                strategy_handle_rejected(std::move(engine_result), policy, std::move(strategy_result));
            break;
        }
        case result_t::Status::DENIED:
        {
            ...
            std::tie(try_next, strategy_result) =
                strategy_handle_denied(std::move(engine_result), policy, std::move(strategy_result));
            break;
        }
        case result_t::Status::GRANTED:
        {
            ...
            std::tie(try_next, strategy_result) =
                strategy_handle_granted(std::move(engine_result), policy, std::move(strategy_result));
            break;
        }
        default:
        {
...
int rgw::auth::Strategy::apply(const DoutPrefixProvider* dpp, const rgw::auth::Strategy& auth_strategy,
...
void rgw::auth::Strategy::add_engine(const Control ctrl_flag, const Engine& engine) noexcept
{
...
{
    out << "rgw::auth::WebIdentityApplier(sub =" << sub << ", user_name=" << user_name << ", provider_id =" << iss
        << ")";
}
...
void rgw::auth::WebIdentityApplier::create_account(const DoutPrefixProvider* dpp, const rgw_user& acct_user,
                                                   const string& display_name, RGWUserInfo& user_info) const /* out */
{
    ...
    int ret = ctl->user->store_info(dpp, user_info, null_yield, RGWUserCtl::PutParams().set_exclusive(true));
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to store new user info: user=" << user_info.user_id << " ret=" << ret
                          << dendl;
        throw ret;
    }
}
...
void rgw::auth::WebIdentityApplier::load_acct_info(const DoutPrefixProvider* dpp, RGWUserInfo& user_info) const
{
    ...
    if (ret == -ENOENT) { /* in case of ENOENT, which means user doesnt have buckets */
        ...
        ldpp_dout(dpp, 5) << "NOTICE: incoming user already has buckets associated " << federated_user
...
uint32_t rgw::auth::RemoteApplier::get_perms_from_aclspec(const DoutPrefixProvider* dpp, const aclspec_t& aclspec) const
{
    ...
    perm |= rgw_perms_from_aclspec_default_strategy(info.acct_user, aclspec, dpp);
    ...
    if (info.acct_user.tenant.empty()) {
        ...
        perm |= rgw_perms_from_aclspec_default_strategy(tenanted_acct_user, aclspec, dpp);
    }
    ...
    for (auto& id : ids) {
        if (id.is_wildcard()) {
            ...
            // was enabled. */
                 ...
                 (info.acct_user.tenant.empty() ? info.acct_user.id : info.acct_user.tenant) == id.get_tenant()) {
            return true;
        }
        else if (id.is_user() && info.acct_user.id == id.get_id() &&
                 (info.acct_user.tenant.empty() ? info.acct_user.id : info.acct_user.tenant) == id.get_tenant()) {
            return true;
        }
    }
...
{
    out << "rgw::auth::RemoteApplier(acct_user=" << info.acct_user << ", acct_name=" << info.acct_name
        << ", perm_mask=" << info.perm_mask << ", is_admin=" << info.is_admin << ")";
}
...
void rgw::auth::ImplicitTenants::recompute_value(const ConfigProxy& c)
{
    ...
    int v = 0;
    if (boost::iequals(s, "both") || boost::iequals(s, "true") || boost::iequals(s, "1")) {
        v = IMPLICIT_TENANTS_S3 | IMPLICIT_TENANTS_SWIFT;
    }
    else if (boost::iequals(s, "0") || boost::iequals(s, "none") || boost::iequals(s, "false")) {
        v = 0;
    }
...
{
    static const char* keys[] = {"rgw_keystone_implicit_tenants", nullptr};
    return keys;
}

void rgw::auth::ImplicitTenants::handle_conf_change(const ConfigProxy& c, const std::set<std::string>& changed)
{
...
void rgw::auth::RemoteApplier::create_account(const DoutPrefixProvider* dpp, const rgw_user& acct_user,
                                              bool implicit_tenant, RGWUserInfo& user_info) const /* out */
{
...
{
    out << "rgw::auth::LocalApplier(acct_user=" << user_info.user_id << ", acct_name=" << user_info.display_name
        << ", subuser=" << subuser << ", perm_mask=" << get_perm_mask()
        << ", is_admin=" << static_cast<bool>(user_info.admin) << ")";
}

uint32_t rgw::auth::LocalApplier::get_perm_mask(const std::string& subuser_name, const RGWUserInfo& uinfo) const
{
                                                                     ...
                                                                     const req_state* const s, optional_yield y) const
{
    if (!is_applicable(s)) {
        ...
        auto apl = apl_factory->create_apl_local(cct,
                                                 s,
                                                 user_info,
