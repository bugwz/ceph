        ...
        if (!push_endpoint_name.empty()) {
            ...
            try {
                push_endpoint = RGWPubSubEndpoint::create(
                    push_endpoint_name, arn_topic, string_to_args(push_endpoint_args, dpp), cct);
                ldpp_dout(dpp, 20) << "push endpoint created: " << push_endpoint->to_str() << dendl;
            }
            catch (const RGWPubSubEndpoint::configuration_error& e) {
                ldpp_dout(dpp, 1) << "ERROR: failed to create push endpoint: " << push_endpoint_name
                                  << " due to: " << e.what() << dendl;
            }
        }
    }
...
template<class T> static string json_str(const char* name, const T& obj, bool pretty = false)
{
    stringstream ss;
...
struct PSConfig
{
    ...
    void init_instance(const RGWRealm& realm, uint64_t instance_id) { sync_instance = instance_id; }
};

using PSConfigRef = std::shared_ptr<PSConfig>;
template<typename EventType> using EventRef = std::shared_ptr<EventType>;
...
struct objstore_event
{
    ...
    objstore_event(const rgw_bucket& _bucket, const rgw_obj_key& _key, const ceph::real_time& _mtime,
                   const std::vector<std::pair<std::string, std::string>>* _attrs)
        : bucket(_bucket)
        , key(_key)
        , mtime(_mtime)
        , attrs(_attrs)
...
static void make_event_ref(CephContext* cct, const rgw_bucket& bucket, const rgw_obj_key& key,
                           const ceph::real_time& mtime, const std::vector<std::pair<std::string, std::string>>* attrs,
                           rgw::notify::EventType event_type, EventRef<rgw_pubsub_event>* event)
{
...
static void make_s3_event_ref(CephContext* cct, const rgw_bucket& bucket, const rgw_user& owner, const rgw_obj_key& key,
                              const ceph::real_time& mtime,
                              const std::vector<std::pair<std::string, std::string>>* attrs,
                              rgw::notify::EventType event_type, EventRef<rgw_pubsub_s3_event>* event)
{
    *event = std::make_shared<rgw_pubsub_s3_event>();
    ...
    boost::algorithm::hex(
        (const char*)&ts, (const char*)&ts + sizeof(utime_t), std::back_inserter(e->object_sequencer));
...
struct PSEnv
{
    ...
    PSEnv()
        : conf(make_shared<PSConfig>())
        , data_user_info(make_shared<RGWUserInfo>())
    {}

    void init(CephContext* cct, const JSONFormattable& config) { conf->init(cct, config); }
...
template<typename EventType> class PSEvent
{
    const EventRef<EventType> event;

public:
    PSEvent(const EventRef<EventType>& _event)
        : event(_event)
    {}

    void format(bufferlist* bl) const { bl->append(json_str("", *event)); }

    void encode_event(bufferlist& bl) const { encode(*event, bl); }

    const string& id() const { return event->id; }
};

template<class T> class RGWSingletonCR : public RGWCoroutine
{
    ...
    int operate_wrapper(const DoutPrefixProvider* dpp) override
    {
        reenter(&wrapper_state)
        {
            ...
            ldpp_dout(dpp, 20) << __func__ << "(): RGWSingletonCR: operate_wrapper() done, need to wake up "
                               << waiters.size() << " waiters" << dendl;
...
class PSSubscription
{
    ...
    public:
        InitBucketLifecycleCR(RGWDataSyncCtx* _sc, PSConfigRef& _conf, RGWBucketInfo& _bucket_info,
                              std::map<string, bufferlist>& _bucket_attrs)
            ...
            reenter(this)
            {
...

                {
                    ...
                    for (auto ori : old_rules) {
                        ...
                        if (old_rule.get_prefix().empty() && old_rule.get_expiration().get_days() == retention_days &&
                            old_rule.is_enabled()) {
                            ldpp_dout(dpp, 20)
                                << "no need to set lifecycle rule on bucket, existing rule matches config" << dendl;
                            return set_cr_done();
                        }
                    }
                }
                ...
                yield call(new RGWBucketLifecycleConfigCR(sync_env->async_rados, sync_env->store, lc_config, dpp));
                if (retcode < 0) {
    ...
    public:
        InitCR(RGWDataSyncCtx* _sc, PSSubscriptionRef& _sub)
            : RGWSingletonCR<bool>(_sc->cct)
            , sc(_sc)
            , sync_env(_sc->env)
            , sub(_sub)
            , conf(sub->env->conf)
        ...
        int operate(const DoutPrefixProvider* dpp) override
        {
            reenter(this)
            {
                ...
                for (i = 0; i < 2; ++i) {
                    yield call(new RGWGetBucketInfoCR(
                        sync_env->async_rados, sync_env->store, get_bucket_info, sub->get_bucket_info_result, dpp));
                    if (retcode < 0 && retcode != -ENOENT) {
                        ldpp_dout(dpp, 1) << "ERROR: failed to geting bucket info: "
                                          << "tenant=" << get_bucket_info.tenant
                                          << " name=" << get_bucket_info.bucket_name << ": ret=" << retcode << dendl;
                    }
                    if (retcode == 0) {
                        {
                            ...
                            if (ret < 0) {
                                ldpp_dout(dpp, 1)
                                    << "ERROR: data_access.get_bucket() bucket=" << result->bucket_info.bucket
                                    << " failed, ret=" << ret << dendl;
                                return set_cr_error(ret);
                            }
                        }

                        yield call(new InitBucketLifecycleCR(
                            sc, conf, sub->get_bucket_info_result->bucket_info, sub->get_bucket_info_result->attrs));
                        if (retcode < 0) {
                            ldpp_dout(dpp, 1)
                                << "ERROR: failed to init lifecycle on bucket (bucket=" << sub_conf->data_bucket_name
                                << ") ret=" << retcode << dendl;
                            return set_cr_error(retcode);
                        }
                    ...
                    ldpp_dout(dpp, 20) << "pubsub: bucket create: using user info: "
                                       << json_str("obj", *sub->env->data_user_info, true) << dendl;
                    yield call(new RGWBucketCreateLocalCR(sync_env->async_rados, sync_env->store, create_bucket, dpp));
                    if (retcode < 0) {
                        ldpp_dout(dpp, 1) << "ERROR: failed to create bucket: "
                                          << "tenant=" << get_bucket_info.tenant
                                          << " name=" << get_bucket_info.bucket_name << ": ret=" << retcode << dendl;
                        return set_cr_error(retcode);
                    }
                ...
                ldpp_dout(dpp, 1) << "ERROR: failed to create bucket "
                                  << "tenant=" << get_bucket_info.tenant << " name=" << get_bucket_info.bucket_name
                                  << dendl;
    ...
    template<typename EventType> class StoreEventCR : public RGWCoroutine
    {
    ...
    public:
        StoreEventCR(RGWDataSyncCtx* const _sc, const PSSubscriptionRef& _sub, const EventRef<EventType>& _event)
            : RGWCoroutine(_sc->cct)
            , sc(_sc)
            , sync_env(_sc->env)
            , sub(_sub)
        ...
        int operate(const DoutPrefixProvider* dpp) override
        {
            ...
            reenter(this)
            {
                ...
                yield call(new RGWObjectSimplePutCR(sync_env->async_rados, sync_env->store, put_obj, dpp));
                if (retcode < 0) {
                    ldpp_dout(dpp, 10) << "failed to store event: " << put_obj.bucket << "/" << put_obj.key
                                       << " ret=" << retcode << dendl;
                    return set_cr_error(retcode);
                }
                else {
    ...
    template<typename EventType> class PushEventCR : public RGWCoroutine
    {
        RGWDataSyncCtx* const sc;
        RGWDataSyncEnv* const sync_env;
        const EventRef<EventType> event;
    ...
    public:
        PushEventCR(RGWDataSyncCtx* const _sc, const PSSubscriptionRef& _sub, const EventRef<EventType>& _event)
            : RGWCoroutine(_sc->cct)
            , sc(_sc)
            , sync_env(_sc->env)
            , event(_event)
            ...
            reenter(this)
            {
                ...
                if (retcode < 0) {
                    ldpp_dout(dpp, 10) << "failed to push event: " << event->id
                                       << " to endpoint: " << sub_conf->push_endpoint_name << " ret=" << retcode
                                       << dendl;
                    return set_cr_error(retcode);
                }

                ldpp_dout(dpp, 20) << "event: " << event->id << " pushed to endpoint: " << sub_conf->push_endpoint_name
                                   << dendl;
                return set_cr_done();
            }
            return 0;
...
public:
    PSSubscription(RGWDataSyncCtx* _sc, PSEnvRef _env, PSSubConfigRef& _sub_conf)
        : sc(_sc)
        , sync_env(_sc->env)
        , env(_env)
        , sub_conf(_sub_conf)
    ...
    PSSubscription(RGWDataSyncCtx* _sc, PSEnvRef _env, rgw_pubsub_sub_config& user_sub_conf)
        : sc(_sc)
        , sync_env(_sc->env)
        , env(_env)
    ...
    template<class C> static PSSubscriptionRef get_shared(RGWDataSyncCtx* _sc, PSEnvRef _env, C& _sub_conf)
    {
    ...
    int call_init_cr(RGWCoroutine* caller) { return init_cr->execute(caller); }

    template<typename EventType>
    static RGWCoroutine* store_event_cr(RGWDataSyncCtx* const sc, const PSSubscriptionRef& sub,
                                        const EventRef<EventType>& event)
    {
    ...
    template<typename EventType>
    static RGWCoroutine* push_event_cr(RGWDataSyncCtx* const sc, const PSSubscriptionRef& sub,
                                       const EventRef<EventType>& event)
    {
...
class PSManager
{
    ...
    public:
        GetSubCR(RGWDataSyncCtx* _sc, PSManagerRef& _mgr, const rgw_user& _owner, const string& _sub_name,
                 PSSubscriptionRef* _ref)
            : RGWSingletonCR<PSSubscriptionRef>(_sc->cct)
            , sc(_sc)
            , sync_env(_sc->env)
            , mgr(_mgr)
            , owner(_owner)
        ...
        int operate(const DoutPrefixProvider* dpp) override
        {
            reenter(this)
            {
                ...
                if (retcode < 0) {
                    ldpp_dout(dpp, 1) << "ERROR: failed to init subscription when getting subscription: " << sub_name
                                      << dendl;
                    mgr->remove_get_sub(owner, sub_name);
                    return set_cr_error(retcode);
                }
        ...
        {
            ldout(cct, 20) << __func__ << "(): returning result: retcode=" << retcode << " resultp=" << (void*)result
                           << dendl;
            if (retcode >= 0) {
                *result = *ref;
            }
    ...
    GetSubCR*& get_get_subs(const rgw_user& owner, const string& name) { return get_subs[sub_id(owner, name)]; }

    void remove_get_sub(const rgw_user& owner, const string& name) { get_subs.erase(sub_id(owner, name)); }
    ...
    PSManager(RGWDataSyncCtx* _sc, PSEnvRef _env)
        : sc(_sc)
        , sync_env(_sc->env)
        , env(_env)
    {}
...
public:
    static PSManagerRef get_shared(RGWDataSyncCtx* _sc, PSEnvRef _env)
    {
        return std::shared_ptr<PSManager>(new PSManager(_sc, _env));
    }

    static int call_get_subscription_cr(RGWDataSyncCtx* sc, PSManagerRef& mgr, RGWCoroutine* caller,
                                        const rgw_user& owner, const string& sub_name, PSSubscriptionRef* ref)
    {
...
public:
    RGWPSInitEnvCBCR(RGWDataSyncCtx* _sc, PSEnvRef& _env)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , env(_env)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            yield call(
                new RGWGetUserInfoCR(sync_env->async_rados, sync_env->store, get_user_info, env->data_user_info, dpp));
            if (retcode < 0) {
                ldpp_dout(dpp, 1) << "ERROR: failed to create rgw user: ret=" << retcode << dendl;
                return set_cr_error(retcode);
            }

            ldpp_dout(dpp, 20) << "pubsub: get user info cr returned: " << json_str("obj", *env->data_user_info, true)
                               << dendl;
...
public:
    RGWPSFindBucketTopicsCR(RGWDataSyncCtx* _sc, PSEnvRef& _env, const rgw_user& _owner, const rgw_bucket& _bucket,
                            const rgw_obj_key& _key, rgw::notify::EventType _event_type, TopicsRef* _topics)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , env(_env)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            ldpp_dout(dpp, 20) << "RGWPSFindBucketTopicsCR(): found " << bucket_topics.topics.size()
                               << " topics for bucket " << bucket << dendl;
            ...
            if (!bucket_topics.topics.empty()) {
                ...
                {
                    bool empty_on_enoent = true;
                    call(new ReadUserTopicsInfoCR(dpp,
                                                  sync_env->async_rados,
                                                  sync_env->store->svc()->sysobj,
                                                  user_obj,
                                                  &user_topics,
...
public:
    RGWPSHandleObjEventCR(RGWDataSyncCtx* const _sc, const PSEnvRef _env, const rgw_user& _owner,
                          const EventRef<rgw_pubsub_event>& _event, const EventRef<rgw_pubsub_s3_event>& _s3_event,
                          const TopicsRef& _topics)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , env(_env)
        , owner(_owner)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ldpp_dout(dpp, 20) << ": handle event: obj: z=" << sc->source_zone
                               << " event=" << json_str("event", *event, false) << " owner=" << owner << dendl;
            ...
            for (titer = topics->begin(); titer != topics->end(); ++titer) {
                ldpp_dout(dpp, 20) << ": notification for " << event->source << ": topic=" << (*titer)->name << ", has "
                                   << (*titer)->subs.size() << " subscriptions" << dendl;
                ...
                for (siter = (*titer)->subs.begin(); siter != (*titer)->subs.end(); ++siter) {
                    ...
                    if (sub->sub_conf->s3_id.empty()) {
                        ...
                        ldpp_dout(dpp, 20) << "storing event for subscription=" << *siter << " owner=" << owner
                                           << " ret=" << retcode << dendl;
                        yield call(PSSubscription::store_event_cr(sc, sub, event));
                        if (retcode < 0) {
                            if (perfcounter) perfcounter->inc(l_rgw_pubsub_store_fail);
                            ldpp_dout(dpp, 1) << "ERROR: failed to store event for subscription=" << *siter
                                              << " ret=" << retcode << dendl;
                        }
                        else {
                        ...
                        if (sub->sub_conf->push_endpoint) {
                            ldpp_dout(dpp, 20) << "push event for subscription=" << *siter << " owner=" << owner
                                               << " ret=" << retcode << dendl;
                            yield call(PSSubscription::push_event_cr(sc, sub, event));
                            if (retcode < 0) {
                                if (perfcounter) perfcounter->inc(l_rgw_pubsub_push_failed);
                                ldpp_dout(dpp, 1) << "ERROR: failed to push event for subscription=" << *siter
                                                  << " ret=" << retcode << dendl;
                            }
                            else {
                        ...
                        ldpp_dout(dpp, 20) << "storing s3 event for subscription=" << *siter << " owner=" << owner
                                           << " ret=" << retcode << dendl;
                        ...
                        if (retcode < 0) {
                            if (perfcounter) perfcounter->inc(l_rgw_pubsub_store_fail);
                            ldpp_dout(dpp, 1) << "ERROR: failed to store s3 event for subscription=" << *siter
                                              << " ret=" << retcode << dendl;
                        }
                        else {
                            if (perfcounter) perfcounter->inc(l_rgw_pubsub_store_ok);
                        ...
                        if (sub->sub_conf->push_endpoint) {
                            ldpp_dout(dpp, 20) << "push s3 event for subscription=" << *siter << " owner=" << owner
                                               << " ret=" << retcode << dendl;
                            yield call(PSSubscription::push_event_cr(sc, sub, s3_event));
                            if (retcode < 0) {
                                if (perfcounter) perfcounter->inc(l_rgw_pubsub_push_failed);
                                ldpp_dout(dpp, 1) << "ERROR: failed to push s3 event for subscription=" << *siter
                                                  << " ret=" << retcode << dendl;
                            }
                            else {
                                if (perfcounter) perfcounter->inc(l_rgw_pubsub_push_ok);
...
public:
    RGWPSHandleRemoteObjCBCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, rgw_obj_key& _key, PSEnvRef _env,
                             std::optional<uint64_t> _versioned_epoch, TopicsRef& _topics)
        : RGWStatRemoteObjCBCR(_sc, _sync_pipe.info.source_bs.bucket, _key)
        , sc(_sc)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ldpp_dout(dpp, 20) << ": stat of remote obj: z=" << sc->source_zone
                               << " b=" << sync_pipe.info.source_bs.bucket << " k=" << key << " size=" << size
                               << " mtime=" << mtime << " attrs=" << attrs << dendl;
            {
                ...
                make_event_ref(
                    sc->cct, sync_pipe.info.source_bs.bucket, key, mtime, &attrs, rgw::notify::ObjectCreated, &event);
                make_s3_event_ref(sc->cct,
                                  sync_pipe.info.source_bs.bucket,
                                  sync_pipe.dest_bucket_info.owner,
                                  key,
                                  mtime,
                                  &attrs,
...
public:
    RGWPSHandleRemoteObjCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, rgw_obj_key& _key, PSEnvRef _env,
                           std::optional<uint64_t> _versioned_epoch, TopicsRef& _topics)
        : RGWCallStatRemoteObjCR(_sc, _sync_pipe.info.source_bs.bucket, _key)
        , sync_pipe(_sync_pipe)
        , env(_env)
        , versioned_epoch(_versioned_epoch)
        , topics(_topics)
...
public:
    RGWPSHandleObjCreateCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, rgw_obj_key& _key, PSEnvRef _env,
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            yield call(new RGWPSFindBucketTopicsCR(sc,
                                                   env,
                                                   sync_pipe.dest_bucket_info.owner,
                                                   sync_pipe.info.source_bs.bucket,
                                                   key,
...
public:
    RGWPSGenericObjEventCBCR(RGWDataSyncCtx* _sc, PSEnvRef _env, rgw_bucket_sync_pipe& _sync_pipe, rgw_obj_key& _key,
                             const ceph::real_time& _mtime, rgw::notify::EventType _event_type)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , env(_env)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ldpp_dout(dpp, 20) << ": remove remote obj: z=" << sc->source_zone << " b=" << bucket << " k=" << key
                               << " mtime=" << mtime << dendl;
            ...
            // this is why both are created here, once we have information about the
            // subscription, we will store/push only the relevant ones
            make_event_ref(sc->cct, bucket, key, mtime, nullptr, event_type, &event);
            make_s3_event_ref(sc->cct, bucket, owner, key, mtime, nullptr, event_type, &s3_event);
...
public:
    RGWPSDataSyncModule(CephContext* cct, const JSONFormattable& config)
        : env(std::make_shared<PSEnv>())
        , conf(env->conf)
    {
        env->init(cct, config);
    }
    ...
    {
        ldout(sc->cct, 10) << conf->id << ": sync_object: b=" << sync_pipe << " k=" << key
    ...
    {
        ldout(sc->cct, 10) << conf->id << ": rm_object: b=" << sync_pipe << " k=" << key << " mtime=" << mtime
    ...
    {
        ldout(sc->cct, 10) << conf->id << ": create_delete_marker: b=" << sync_pipe << " k=" << key
                           << " mtime=" << mtime << " versioned=" << versioned << " versioned_epoch=" << versioned_epoch
                           << dendl;
        return new RGWPSGenericObjEventCBCR(
            sc, env, sync_pipe, key, mtime, rgw::notify::ObjectRemovedDeleteMarkerCreated);
    }
...
int RGWPSSyncModule::create_instance(CephContext* cct, const JSONFormattable& config,
