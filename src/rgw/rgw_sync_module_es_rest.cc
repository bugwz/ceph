...
struct es_index_obj_response
{
    ...
    struct
    {
        ...
        template<class T> struct _custom_entry
        {
            string name;
            T value;
...
void RGWMetadataSearchOp::execute(optional_yield y)
{
    ...
    op_ret = conn->get_resource(s, resource, &params, &extra_headers, out, &in, nullptr, y);
    if (op_ret < 0) {
...
{
public:
    explicit RGWMetadataSearch_ObjStore_S3(const RGWSyncModuleInstanceRef& _sync_module)
...
public:
    explicit RGWHandler_REST_MDSearch_S3(const rgw::auth::StrategyRegistry& auth_registry)
...
RGWHandler_REST* RGWRESTMgr_MDSearch_S3::get_handler(rgw::sal::RGWRadosStore* store, struct req_state* const s,
                                                     const rgw::auth::StrategyRegistry& auth_registry,
                                                     const std::string& frontend_prefix)
{
    int ret = RGWHandler_REST_S3::init_from_header(store, s, RGW_FORMAT_XML, true);
    if (ret < 0) {
        return nullptr;
    }
