...
void RGWPSGetSubOp::execute(optional_yield y)
{
    ...
    if (subscription_has_endpoint_secret(result) && !rgw_transport_is_secure(s->cct, *(s->info.env))) {
        ldpp_dout(this, 1) << "subscription '" << sub_name
...
int RGWPSCreateNotifOp::verify_permission(optional_yield y)
{
    ...
    ret = store->getRados()->get_bucket_info(store->svc(), id.tenant, bucket_name, bucket_info, nullptr, y, nullptr);
    if (ret < 0) {
...
int RGWPSDeleteNotifOp::verify_permission(optional_yield y)
{
    ...
    ret = store->getRados()->get_bucket_info(
        store->svc(), s->owner.get_id().tenant, bucket_name, bucket_info, nullptr, y, nullptr);
    if (ret < 0) {
        return ret;
    }
