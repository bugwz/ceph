...
public:
    ...
    int check_caps(const RGWUserCaps& caps) override { return caps.check_cap("usage", RGW_CAP_READ); }
...
void RGWOp_Usage_Get::execute(optional_yield y)
{
    ...
    op_ret = RGWUsage::show(
        this, driver, user.get(), bucket.get(), start, end, show_entries, show_summary, &categories, flusher);
}
...
public:
    ...
    int check_caps(const RGWUserCaps& caps) override { return caps.check_cap("usage", RGW_CAP_WRITE); }
    void execute(optional_yield y) override;
...
void RGWOp_Usage_Delete::execute(optional_yield y)
{
    ...
    if (rgw::sal::User::empty(user.get()) && bucket_name.empty() && !start && end == (uint64_t)-1) {
        bool remove_all;
