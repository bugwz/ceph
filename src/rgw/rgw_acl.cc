...
{
    return lhs.type == rhs.type && lhs.id == rhs.id && lhs.email == rhs.email && lhs.permission == rhs.permission &&
           lhs.name == rhs.name && lhs.group == rhs.group && lhs.url_spec == rhs.url_spec;
}
bool operator!=(const ACLGrant& lhs, const ACLGrant& rhs)
{
    return !(lhs == rhs);
...
bool operator==(const RGWAccessControlList& lhs, const RGWAccessControlList& rhs)
{
    return lhs.acl_user_map == rhs.acl_user_map && lhs.acl_group_map == rhs.acl_group_map &&
           lhs.referer_list == rhs.referer_list && lhs.grant_map == rhs.grant_map;
}
bool operator!=(const RGWAccessControlList& lhs, const RGWAccessControlList& rhs)
{
    return !(lhs == rhs);
...
void RGWAccessControlList::_add_grant(ACLGrant* grant)
{
    ...
    case ACL_TYPE_REFERER:
        ...
        break;
    case ACL_TYPE_GROUP: acl_group_map[grant->get_group()] |= perm.get_permissions(); break;
    default:
    {
        rgw_user id;
        if (!grant->get_id(id)) {
...
uint32_t RGWAccessControlList::get_perm(const DoutPrefixProvider* dpp, const rgw::auth::Identity& auth_identity,
...
uint32_t RGWAccessControlList::get_group_perm(const DoutPrefixProvider* dpp, ACLGroupTypeEnum group,
                                              const uint32_t perm_mask) const
{
    ldpp_dout(dpp, 5) << "Searching permissions for group=" << (int)group << " mask=" << perm_mask << dendl;
...
uint32_t RGWAccessControlList::get_referer_perm(const DoutPrefixProvider* dpp, const uint32_t current_perm,
                                                const std::string http_referer, const uint32_t perm_mask)
{
    ldpp_dout(dpp, 5) << "Searching permissions for referer=" << http_referer << " mask=" << perm_mask << dendl;
...
uint32_t RGWAccessControlPolicy::get_perm(const DoutPrefixProvider* dpp, const rgw::auth::Identity& auth_identity,
                                          const uint32_t perm_mask, const char* const http_referer,
                                          bool ignore_public_acls)
{
    ...
    ldpp_dout(dpp, 5) << "-- Getting permissions done for identity=" << auth_identity << ", owner=" << owner.get_id()
                      << ", perm=" << perm << dendl;

    return perm;
}

bool RGWAccessControlPolicy::verify_permission(const DoutPrefixProvider* dpp, const rgw::auth::Identity& auth_identity,
                                               const uint32_t user_perm_mask, const uint32_t perm,
                                               const char* const http_referer, bool ignore_public_acls)
{
    ...
    ldpp_dout(dpp, 10) << " identity=" << auth_identity << " requested perm (type)=" << perm
                       << ", policy perm=" << policy_perm << ", user_perm_mask=" << user_perm_mask
...
bool RGWAccessControlPolicy::is_public(const DoutPrefixProvider* dpp) const
{
    ...
    static constexpr auto public_groups = {ACL_GROUP_ALL_USERS, ACL_GROUP_AUTHENTICATED_USERS};
    return std::any_of(public_groups.begin(), public_groups.end(), [&, dpp](ACLGroupTypeEnum g) {
