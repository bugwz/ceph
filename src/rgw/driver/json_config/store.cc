...
static void parse_config(const DoutPrefixProvider* dpp, const char* filename)
{
    ...
    if (r < 0) {
        ldpp_dout(dpp, 0) << "failed to read json config file '" << filename << "': " << errmsg << dendl;
...
void sanity_check_config(const DoutPrefixProvider* dpp, DecodedConfig& config)
{
    ...
    if (config.zonegroup.zones.size() == 1) {
        ...
        if (z->first != config.zone.id) {
            ldpp_dout(dpp, 0) << "zonegroup contains unknown zone id=" << z->first << dendl;
            throw std::system_error(make_error_code(std::errc::invalid_argument));
        }
        if (z->second.id != config.zone.id) {
            ldpp_dout(dpp, 0) << "zonegroup contains unknown zone id=" << z->second.id << dendl;
            throw std::system_error(make_error_code(std::errc::invalid_argument));
        }
        if (z->second.name != config.zone.name) {
            ldpp_dout(dpp, 0) << "zonegroup contains unknown zone name=" << z->second.name << dendl;
            throw std::system_error(make_error_code(std::errc::invalid_argument));
        }
        if (config.zonegroup.master_zone != config.zone.id) {
            ldpp_dout(dpp, 0) << "zonegroup contains unknown master_zone=" << config.zonegroup.master_zone << dendl;
            throw std::system_error(make_error_code(std::errc::invalid_argument));
        }
    }
    else {
        ...
        int r = rgw::add_zone_to_group(dpp,
                                       config.zonegroup,
                                       config.zone,
                                       &is_master,
                                       &read_only,
                                       ...
                                       disable_features);
        if (r < 0) {
            ldpp_dout(dpp, 0) << "failed to add zone to zonegroup: " << cpp_strerror(r) << dendl;
            throw std::system_error(-r, std::system_category());
        }
...
auto create_json_config_store(const DoutPrefixProvider* dpp, const std::string& filename)
    -> std::unique_ptr<ConfigStore>
{
    DecodedConfig config;
    parse_config(dpp, filename.c_str());
    sanity_check_config(dpp, config);
    return create_immutable_config_store(dpp, config.zonegroup, config.zone, config.period_config);
}
