...
protected:
    ...
    fs::path getDBFullPath(const std::string& base_dir, const std::string& tenant) const
    {
    ...
    std::string getDBTenant(const std::string& base_dir, const std::string& tenant) const
    {
    ...
    fs::path getDBFullPath(const std::string& tenant) const { return getDBFullPath(getTestDir(), tenant); }

    fs::path getLogFilePath(const std::string& log_file) { return fs::temp_directory_path() / log_file; }

    std::shared_ptr<CephContext> getContext() const { return ctx_; }
