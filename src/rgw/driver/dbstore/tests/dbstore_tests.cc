...
TEST_F(DBStoreTest, ListAllBuckets2)
{
    ...
    do {
        is_truncated = false;
        ret = db->list_buckets(dpp, "all", owner, marker1, "", max, need_stats, &ulist, &is_truncated);
        ASSERT_EQ(ret, 0);
...
TEST_F(DBStoreTest, GetObjectState)
{
    ...
    ret = op_target.get_obj_state(dpp, params.op.bucket.info, params.op.obj.state.obj, false, &s);
    ASSERT_EQ(ret, 0);
    ASSERT_EQ(s->size, 12);
...
TEST_F(DBStoreTest, ReadVersionedObject)
{
    ...
    ret = op_target2.get_obj_state(dpp, params.op.bucket.info, params.op.obj.state.obj, true, &s);
    ASSERT_EQ(ret, 0);
    ...
    ret = op_target3.get_obj_state(dpp, params.op.bucket.info, params.op.obj.state.obj, true, &s);
    ASSERT_EQ(ret, 0);
    decode(data, s->data);
...
TEST_F(DBStoreTest, DeleteVersionedObject)
{
    ...
    for (auto ent : params.op.obj.list_entries) {
        ...
        cout << "ent.name: " << ent.key.name << ". ent.instance: " << ent.key.instance
    ...
    ret = op_target4.get_obj_state(dpp, params.op.bucket.info, params.op.obj.state.obj, true, &s);
    ASSERT_EQ(s->obj.key.instance, instances[2]);
    decode(data, s->data);
    ASSERT_EQ(data, "HELLO WORLD A");
    ASSERT_EQ(s->size, 14);
