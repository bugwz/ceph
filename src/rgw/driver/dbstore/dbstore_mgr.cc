...
DB* DBStoreManager::getDB(string tenant, bool create)
{
    ...
    if (tenant.empty()) return default_db;

    if (DBStoreHandles.empty()) goto not_found;

    iter = DBStoreHandles.find(tenant);

    if (iter != DBStoreHandles.end()) return iter->second;
...
void DBStoreManager::deleteDB(string tenant)
{
    ...
    if (tenant.empty() || DBStoreHandles.empty()) return;

    /* XXX: Check if we need to perform this operation under a lock */
    iter = DBStoreHandles.find(tenant);

    if (iter == DBStoreHandles.end()) return;
...
void DBStoreManager::destroyAllHandles()
{
    ...
    if (DBStoreHandles.empty()) return;
