...
struct Prefix : DoutPrefixPipe
{
    ...
    void add_prefix(std::ostream& out) const override { out << prefix; }
};
...
void read_text_rows(const DoutPrefixProvider* dpp, const sqlite::stmt_execution& stmt, std::span<std::string> entries,
...
using SQLiteConnectionPool = ConnectionPool<sqlite::Connection, sqlite::ConnectionFactory>;
...
public:
    SQLiteRealmWriter(SQLiteImpl* impl, int ver, std::string tag, std::string_view realm_id,
    ...
    int write(const DoutPrefixProvider* dpp, optional_yield y, const RGWRealm& info) override
    {
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql = fmt::format(schema::realm_update5, P1, P2, P3, P4, P5);
    ...
    int rename(const DoutPrefixProvider* dpp, optional_yield y, RGWRealm& info, std::string_view new_name) override
    {
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql = fmt::format(schema::realm_rename4, P1, P2, P3, P4);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding{stmt.get()};
...
int SQLiteConfigStore::write_default_realm_id(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive,
...
int SQLiteConfigStore::read_default_realm_id(const DoutPrefixProvider* dpp, optional_yield y, std::string& realm_id)
{
...
int SQLiteConfigStore::delete_default_realm_id(const DoutPrefixProvider* dpp, optional_yield y)
...
int SQLiteConfigStore::create_realm(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive,
                                    const RGWRealm& info, std::unique_ptr<sal::RealmWriter>* writer)
{
    ...
    try {
        ...
        if (exclusive) {
            ...
            if (!*stmt) {
                const std::string sql = fmt::format(schema::realm_insert4, P1, P2, P3, P4);
                *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
        }
        else {
            ...
            if (!*stmt) {
                const std::string sql = fmt::format(schema::realm_upsert4, P1, P2, P3, P4);
                *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
        }
        auto binding = sqlite::stmt_binding{stmt->get()};
    ...
    if (writer) {
        *writer = std::make_unique<SQLiteRealmWriter>(impl.get(), ver, std::move(tag), info.id, info.name);
    }
    return 0;
}

int SQLiteConfigStore::read_realm_by_id(const DoutPrefixProvider* dpp, optional_yield y, std::string_view realm_id,
                                        RGWRealm& info, std::unique_ptr<sal::RealmWriter>* writer)
{
    ...
    if (writer) {
        *writer = std::make_unique<SQLiteRealmWriter>(impl.get(), row.ver, std::move(row.tag), info.id, info.name);
    }
    return 0;
}

static void realm_select_by_name(const DoutPrefixProvider* dpp, sqlite::Connection& conn, std::string_view realm_name,
                                 RealmRow& row)
{
...
int SQLiteConfigStore::read_realm_by_name(const DoutPrefixProvider* dpp, optional_yield y, std::string_view realm_name,
                                          RGWRealm& info, std::unique_ptr<sal::RealmWriter>* writer)
{
...
int SQLiteConfigStore::read_default_realm(const DoutPrefixProvider* dpp, optional_yield y, RGWRealm& info,
...
int SQLiteConfigStore::read_realm_id(const DoutPrefixProvider* dpp, optional_yield y, std::string_view realm_name,
                                     std::string& realm_id)
{
...
int SQLiteConfigStore::realm_notify_new_period(const DoutPrefixProvider* dpp, optional_yield y, const RGWPeriod& period)
{
    return -ENOTSUP;
}

int SQLiteConfigStore::list_realm_names(const DoutPrefixProvider* dpp, optional_yield y, const std::string& marker,
                                        std::span<std::string> entries, sal::ListResult<std::string>& result)
{
...
int SQLiteConfigStore::create_period(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive,
                                     const RGWPeriod& info)
{
    ...
    try {
        ...
        if (exclusive) {
            ...
            if (!*stmt) {
                const std::string sql = fmt::format(schema::period_insert4, P1, P2, P3, P4);
                *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
        }
        else {
            ...
            if (!*stmt) {
                const std::string sql = fmt::format(schema::period_upsert4, P1, P2, P3, P4);
                *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
        }
        auto binding = sqlite::stmt_binding{stmt->get()};
...
static void period_select_epoch(const DoutPrefixProvider* dpp, sqlite::Connection& conn, std::string_view id,
                                uint32_t epoch, RGWPeriod& row)
{
...
static void period_select_latest(const DoutPrefixProvider* dpp, sqlite::Connection& conn, std::string_view id,
                                 RGWPeriod& row)
{
...
int SQLiteConfigStore::read_period(const DoutPrefixProvider* dpp, optional_yield y, std::string_view period_id,
...
int SQLiteConfigStore::delete_period(const DoutPrefixProvider* dpp, optional_yield y, std::string_view period_id)
{
...
int SQLiteConfigStore::list_period_ids(const DoutPrefixProvider* dpp, optional_yield y, const std::string& marker,
                                       std::span<std::string> entries, sal::ListResult<std::string>& result)
{
...
public:
    SQLiteZoneGroupWriter(SQLiteImpl* impl, int ver, std::string tag, std::string_view zonegroup_id,
    ...
    int write(const DoutPrefixProvider* dpp, optional_yield y, const RGWZoneGroup& info) override
    {
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql = fmt::format(schema::zonegroup_update5, P1, P2, P3, P4, P5);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding{stmt.get()};
            sqlite::bind_text(dpp, binding, P1, info.id);
    ...
    int rename(const DoutPrefixProvider* dpp, optional_yield y, RGWZoneGroup& info, std::string_view new_name) override
    {
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql = fmt::format(schema::zonegroup_rename4, P1, P2, P3, P4);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding{stmt.get()};
            sqlite::bind_text(dpp, binding, P1, info.id);
    ...
    int remove(const DoutPrefixProvider* dpp, optional_yield y) override
    {
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql = fmt::format(schema::zonegroup_delete3, P1, P2, P3);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding{stmt.get()};
...
int SQLiteConfigStore::write_default_zonegroup_id(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive,
                                                  std::string_view realm_id, std::string_view zonegroup_id)
{
    ...
    try {
        ...
        if (exclusive) {
            ...
            if (!*stmt) {
                const std::string sql = fmt::format(schema::default_zonegroup_insert2, P1, P2);
                *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
        }
        else {
            ...
            if (!*stmt) {
                const std::string sql = fmt::format(schema::default_zonegroup_upsert2, P1, P2);
                *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
        }
        auto binding = sqlite::stmt_binding{stmt->get()};
...
int SQLiteConfigStore::read_default_zonegroup_id(const DoutPrefixProvider* dpp, optional_yield y,
                                                 std::string_view realm_id, std::string& zonegroup_id)
{
...
int SQLiteConfigStore::delete_default_zonegroup_id(const DoutPrefixProvider* dpp, optional_yield y,
                                                   std::string_view realm_id)
{
...
int SQLiteConfigStore::create_zonegroup(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive,
                                        const RGWZoneGroup& info, std::unique_ptr<sal::ZoneGroupWriter>* writer)
{
    ...
    try {
        ...
        if (exclusive) {
            ...
            if (!*stmt) {
                const std::string sql = fmt::format(schema::zonegroup_insert6, P1, P2, P3, P4, P5, P6);
                *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
        }
        else {
            ...
            if (!*stmt) {
                const std::string sql = fmt::format(schema::zonegroup_upsert6, P1, P2, P3, P4, P5, P6);
                *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
        }
        auto binding = sqlite::stmt_binding{stmt->get()};
    ...
    if (writer) {
        *writer = std::make_unique<SQLiteZoneGroupWriter>(impl.get(), ver, std::move(tag), info.id, info.name);
    }
    return 0;
}

int SQLiteConfigStore::read_zonegroup_by_id(const DoutPrefixProvider* dpp, optional_yield y,
                                            std::string_view zonegroup_id, RGWZoneGroup& info,
                                            std::unique_ptr<sal::ZoneGroupWriter>* writer)
{
    ...
    info = std::move(row.info);
    if (writer) {
        *writer = std::make_unique<SQLiteZoneGroupWriter>(impl.get(), row.ver, std::move(row.tag), info.id, info.name);
    }
    return 0;
}

int SQLiteConfigStore::read_zonegroup_by_name(const DoutPrefixProvider* dpp, optional_yield y,
                                              std::string_view zonegroup_name, RGWZoneGroup& info,
                                              std::unique_ptr<sal::ZoneGroupWriter>* writer)
{
...
int SQLiteConfigStore::read_default_zonegroup(const DoutPrefixProvider* dpp, optional_yield y,
                                              std::string_view realm_id, RGWZoneGroup& info,
                                              std::unique_ptr<sal::ZoneGroupWriter>* writer)
{
...
int SQLiteConfigStore::list_zonegroup_names(const DoutPrefixProvider* dpp, optional_yield y, const std::string& marker,
                                            std::span<std::string> entries, sal::ListResult<std::string>& result)
{
...
public:
    SQLiteZoneWriter(SQLiteImpl* impl, int ver, std::string tag, std::string_view zone_id, std::string_view zone_name)
        : impl(impl)
        , ver(ver)
        , tag(std::move(tag))
        , zone_id(zone_id)
    ...
    int write(const DoutPrefixProvider* dpp, optional_yield y, const RGWZoneParams& info) override
    {
        ...
        try {
            ...
            if (!stmt) {
                const std::string sql = fmt::format(schema::zone_update5, P1, P2, P3, P4, P5);
                stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
            auto binding = sqlite::stmt_binding{stmt.get()};
            sqlite::bind_text(dpp, binding, P1, info.id);
    ...
    int rename(const DoutPrefixProvider* dpp, optional_yield y, RGWZoneParams& info, std::string_view new_name) override
    {
...
int SQLiteConfigStore::write_default_zone_id(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive,
                                             std::string_view realm_id, std::string_view zone_id)
{
...
int SQLiteConfigStore::read_default_zone_id(const DoutPrefixProvider* dpp, optional_yield y, std::string_view realm_id,
                                            std::string& zone_id)
{
...
int SQLiteConfigStore::delete_default_zone_id(const DoutPrefixProvider* dpp, optional_yield y,
                                              std::string_view realm_id)
{
...
int SQLiteConfigStore::create_zone(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive,
                                   const RGWZoneParams& info, std::unique_ptr<sal::ZoneWriter>* writer)
{
    ...
    try {
        ...
        if (exclusive) {
            ...
            if (!*stmt) {
                const std::string sql = fmt::format(schema::zone_insert6, P1, P2, P3, P4, P5, P6);
                *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
        }
        else {
            ...
            if (!*stmt) {
                const std::string sql = fmt::format(schema::zone_upsert6, P1, P2, P3, P4, P5, P6);
                *stmt = sqlite::prepare_statement(dpp, conn->db.get(), sql);
            }
        }
        auto binding = sqlite::stmt_binding{stmt->get()};
    ...
    if (writer) {
        *writer = std::make_unique<SQLiteZoneWriter>(impl.get(), ver, std::move(tag), info.id, info.name);
    }
    return 0;
}

int SQLiteConfigStore::read_zone_by_id(const DoutPrefixProvider* dpp, optional_yield y, std::string_view zone_id,
                                       RGWZoneParams& info, std::unique_ptr<sal::ZoneWriter>* writer)
{
    ...
    info = std::move(row.info);
    if (writer) {
        *writer = std::make_unique<SQLiteZoneWriter>(impl.get(), row.ver, std::move(row.tag), info.id, info.name);
    }
    return 0;
}

int SQLiteConfigStore::read_zone_by_name(const DoutPrefixProvider* dpp, optional_yield y, std::string_view zone_name,
                                         RGWZoneParams& info, std::unique_ptr<sal::ZoneWriter>* writer)
{
...
int SQLiteConfigStore::list_zone_names(const DoutPrefixProvider* dpp, optional_yield y, const std::string& marker,
                                       std::span<std::string> entries, sal::ListResult<std::string>& result)
{
...
int SQLiteConfigStore::write_period_config(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive,
                                           std::string_view realm_id, const RGWPeriodConfig& info)
{
...
int version_cb(void* user, int count, char** values, char** names)
{
    ...
    auto result = std::from_chars(value.begin(), value.end(), *reinterpret_cast<uint32_t*>(user));
...
void apply_schema_migrations(const DoutPrefixProvider* dpp, sqlite3* db)
{
    ...
    for (; m != schema::migrations.end(); ++m, ++version) {
        try {
            ...
            ldpp_dout(dpp, -1) << "ERROR: schema migration failed on v" << version << ": " << m->description << dendl;
            throw;
        }
    }
    ...
    -> std::unique_ptr<config::SQLiteConfigStore>
{
    ...
    int flags = SQLITE_OPEN_CREATE | SQLITE_OPEN_URI | SQLITE_OPEN_READWRITE | SQLITE_OPEN_NOMUTEX;
