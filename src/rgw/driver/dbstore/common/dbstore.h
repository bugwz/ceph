...
private:
    ...
    static constexpr std::string_view CreateObjectTableTriggerQ = "CREATE TRIGGER IF NOT EXISTS '{}' \
          AFTER INSERT ON '{}' \
       BEGIN \
          UPDATE '{}' \
    ...
    static constexpr std::string_view CreateQuotaTableQ = "CREATE TABLE IF NOT EXISTS '{}' ( \
    ...
    static constexpr std::string_view CreateLCEntryTableQ = "CREATE TABLE IF NOT EXISTS '{}' ( \
    ...
    static constexpr std::string_view CreateLCHeadTableQ = "CREATE TABLE IF NOT EXISTS '{}' ( \
      LCIndex  TEXT NOT NULL , \
      Marker TEXT , \
      StartDate  INTEGER , \
      PRIMARY KEY (LCIndex) \n);";
...
public:
    ...
    static std::string CreateTableSchema(std::string_view type, const DBOpParams* params)
    {
        if (!type.compare("User")) return fmt::format(CreateUserTableQ, params->user_table);
        if (!type.compare("Bucket")) return fmt::format(CreateBucketTableQ, params->bucket_table, params->user_table);
        if (!type.compare("Object")) return fmt::format(CreateObjectTableQ, params->object_table, params->bucket_table);
        ...
        if (!type.compare("ObjectData"))
            return fmt::format(CreateObjectDataTableQ, params->objectdata_table, params->bucket_table);
        if (!type.compare("ObjectView"))
            return fmt::format(CreateObjectTableQ, params->object_view, params->objectdata_table, params->object_table);
        if (!type.compare("Quota")) return fmt::format(CreateQuotaTableQ, params->quota_table);
        if (!type.compare("LCHead")) return fmt::format(CreateLCHeadTableQ, params->lc_head_table);
        if (!type.compare("LCEntry"))
            return fmt::format(CreateLCEntryTableQ, params->lc_entry_table, params->bucket_table);

        ceph_abort_msgf("incorrect table type %.*s", type.size(), type.data());
    }

    static std::string DeleteTableSchema(std::string_view table) { return fmt::format(DropQ, table); }
    static std::string ListTableSchema(std::string_view table) { return fmt::format(ListAllQ, table); }
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        return fmt::format(Query,
                           ...
                           params.op.user.ns,
                           params.op.user.display_name,
                           params.op.user.user_email,
                           params.op.user.access_keys_id,
                           params.op.user.access_keys_secret,
                           params.op.user.access_keys,
                           params.op.user.swift_keys,
                           params.op.user.subusers,
                           params.op.user.suspended,
                           params.op.user.max_buckets,
                           params.op.user.op_mask,
                           params.op.user.user_caps,
                           params.op.user.admin,
                           params.op.user.system,
                           params.op.user.placement_name,
                           params.op.user.placement_storage_class,
                           params.op.user.placement_tags,
                           params.op.user.bucket_quota,
                           params.op.user.temp_url_keys,
                           params.op.user.user_quota,
                           params.op.user.type,
                           params.op.user.mfa_ids,
                           params.op.user.user_attrs,
                           params.op.user.user_ver,
...
{
private:
    static constexpr std::string_view Query = "DELETE from '{}' where UserID = {}";
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        return fmt::format(Query, params.user_table, params.op.user.user_id);
    }
};
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        if (params.op.query_str == "email") {
            return fmt::format(QueryByEmail, params.user_table, params.op.user.user_email);
        }
        else if (params.op.query_str == "access_key") {
            return fmt::format(QueryByAccessKeys, params.user_table, params.op.user.access_keys_id);
        }
        else if (params.op.query_str == "user_id") {
            return fmt::format(QueryByUserID, params.user_table, params.op.user.user_id);
        }
        else {
            return fmt::format(Query, params.user_table, params.op.user.user_id);
        }
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        return fmt::format(Query,
                           ...
                           params.op.bucket.tenant,
                           params.op.bucket.marker,
                           params.op.bucket.bucket_id,
                           params.op.bucket.size,
                           params.op.bucket.size_rounded,
                           params.op.bucket.creation_time,
                           params.op.bucket.count,
                           params.op.bucket.placement_name,
                           params.op.bucket.placement_storage_class,
                           params.op.user.user_id,
                           params.op.bucket.flags,
                           params.op.bucket.zonegroup,
                           params.op.bucket.has_instance_obj,
                           params.op.bucket.quota,
                           params.op.bucket.requester_pays,
                           params.op.bucket.has_website,
                           params.op.bucket.website_conf,
                           params.op.bucket.swift_versioning,
                           params.op.bucket.swift_ver_location,
                           params.op.bucket.mdsearch_config,
                           params.op.bucket.new_bucket_instance_id,
                           params.op.bucket.obj_lock,
                           params.op.bucket.sync_policy_info_groups,
                           params.op.bucket.bucket_attrs,
                           params.op.bucket.bucket_ver,
                           params.op.bucket.bucket_ver_tag,
...
{
private:
    static constexpr std::string_view Query = "DELETE from '{}' where BucketName = {}";
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        return fmt::format(Query, params.bucket_table, params.op.bucket.bucket_name);
    }
};
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        ...
        return fmt::format(Query, params.bucket_table, params.user_table, params.op.bucket.bucket_name);
    }
};
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        if (params.op.query_str == "all") {
            return fmt::format(AllQuery, params.bucket_table, params.op.bucket.min_marker, params.op.list_max_count);
        }
        else {
            return fmt::format(Query,
                               params.bucket_table,
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        return fmt::format(Query,
                           ...
                           params.op.bucket.bucket_name,
                           params.op.obj.acls,
                           params.op.obj.index_ver,
                           params.op.obj.tag,
                           params.op.obj.flags,
                           params.op.obj.versioned_epoch,
                           params.op.obj.obj_category,
                           params.op.obj.etag,
                           params.op.obj.owner,
                           params.op.obj.owner_display_name,
                           params.op.obj.storage_class,
                           params.op.obj.appendable,
                           params.op.obj.content_type,
                           params.op.obj.index_hash_source,
                           params.op.obj.obj_size,
                           params.op.obj.accounted_size,
                           params.op.obj.mtime,
                           params.op.obj.epoch,
                           params.op.obj.obj_tag,
                           params.op.obj.tail_tag,
                           params.op.obj.write_tag,
                           params.op.obj.fake_tag,
                           params.op.obj.shadow_obj,
                           params.op.obj.has_data,
                           params.op.obj.is_versioned,
                           params.op.obj.version_num,
                           params.op.obj.pg_ver,
                           params.op.obj.zone_short_id,
                           params.op.obj.obj_version,
                           params.op.obj.obj_version_tag,
                           params.op.obj.obj_attrs,
                           params.op.obj.head_size,
                           params.op.obj.max_head_size,
                           ...
                           params.op.obj.omap,
                           params.op.obj.is_multipart,
                           params.op.obj.mp_parts,
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        /* XXX: Include obj_id, delim */
        return fmt::format(
            Query, params.object_table, params.op.bucket.bucket_name, params.op.obj.obj_name, params.op.list_max_count);
    }
};
...
private:
    // Updates Omap
    static constexpr std::string_view OmapQuery = "UPDATE '{}' SET Omap = {}, Mtime = {} \
      where BucketName = {} and ObjName = {} and ObjInstance = {}";
    static constexpr std::string_view AttrsQuery = "UPDATE '{}' SET ObjAttrs = {}, Mtime = {}  \
      where BucketName = {} and ObjName = {} and ObjInstance = {}";
    static constexpr std::string_view MPQuery = "UPDATE '{}' SET MPPartsList = {}, Mtime = {}  \
      where BucketName = {} and ObjName = {} and ObjInstance = {}";
    static constexpr std::string_view MetaQuery = "UPDATE '{}' SET \
...
{
private:
    static constexpr std::string_view Query = "UPDATE '{}' \
...
private:
    static constexpr std::string_view Query =
        "DELETE from '{}' WHERE (ObjName, ObjInstance, ObjID) NOT IN (SELECT s.ObjName, s.ObjInstance, s.ObjID from "
        "'{}' as s INNER JOIN '{}' USING (ObjName, BucketName, ObjInstance, ObjID)) and Mtime < {}";
...
public:
    ...
    {
        return fmt::format(
            Query, params.objectdata_table, params.objectdata_table, params.object_table, params.op.obj.mtime);
    }
};
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        return fmt::format(Query,
                           ...
                           params.op.lc_entry.index,
                           params.op.lc_entry.bucket_name,
                           params.op.lc_entry.start_time,
                           params.op.lc_entry.status);
    }
};
...
{
private:
    static constexpr std::string_view Query = "DELETE from '{}' where LCIndex = {} and BucketName = {}";
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        return fmt::format(Query, params.lc_entry_table, params.op.lc_entry.index, params.op.lc_entry.bucket_name);
    }
};
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        if (params.op.query_str == "get_next_entry") {
            return fmt::format(
                NextQuery, params.lc_entry_table, params.op.lc_entry.index, params.op.lc_entry.bucket_name);
        }
        // default
        return fmt::format(Query, params.lc_entry_table, params.op.lc_entry.index, params.op.lc_entry.bucket_name);
    }
...
{
private:
    static constexpr std::string_view Query = "DELETE from '{}' where LCIndex = {}";
...
public:
    ...
    static std::string Schema(DBOpPrepareParams& params)
    {
        return fmt::format(Query, params.lc_head_table, params.op.lc_head.index);
    }
};
...
public:
    DB(std::string db_name, CephContext* _cct)
        : db_name(db_name)
    ...
    const std::string getObjectTable(std::string bucket) { return db_name + "_" + bucket + "_object_table"; }
    const std::string getObjectDataTable(std::string bucket) { return db_name + "_" + bucket + "_objectdata_table"; }
    const std::string getObjectView(std::string bucket) { return db_name + "_" + bucket + "_object_view"; }
    const std::string getObjectTrigger(std::string bucket) { return db_name + "_" + bucket + "_object_trigger"; }

    std::map<std::string, class ObjectOp*> getObjectMap();

    struct DBOps dbops;   // DB operations, make it private?

    void set_driver(rgw::sal::Driver* _driver) { driver = _driver; }
    ...
    virtual int InitPrepareParams(const DoutPrefixProvider* dpp, DBOpPrepareParams& p_params, DBOpParams* params) = 0;
    ...
    int get_user(const DoutPrefixProvider* dpp, const std::string& query_str, const std::string& query_str_val,
                 RGWUserInfo& uinfo, std::map<std::string, bufferlist>* pattrs, RGWObjVersionTracker* pobjv_tracker);
    int store_user(const DoutPrefixProvider* dpp, RGWUserInfo& uinfo, bool exclusive,
                   std::map<std::string, bufferlist>* pattrs, RGWObjVersionTracker* pobjv_tracker,
                   RGWUserInfo* pold_info);
    int remove_user(const DoutPrefixProvider* dpp, RGWUserInfo& uinfo, RGWObjVersionTracker* pobjv_tracker);
    int get_bucket_info(const DoutPrefixProvider* dpp, const std::string& query_str, const std::string& query_str_val,
                        RGWBucketInfo& info, rgw::sal::Attrs* pattrs, ceph::real_time* pmtime,
                        obj_version* pbucket_version);
    int create_bucket(const DoutPrefixProvider* dpp, const RGWUserInfo& owner, rgw_bucket& bucket,
                      ...
                      std::map<std::string, bufferlist>& attrs, RGWBucketInfo& info, obj_version* pobjv,
                      obj_version* pep_objv, real_time creation_time, rgw_bucket* pmaster_bucket,
                      uint32_t* pmaster_num_shards, optional_yield y, bool exclusive);

    int next_bucket_id() { return ++max_bucket_id; };

    int remove_bucket(const DoutPrefixProvider* dpp, const RGWBucketInfo info);
    int list_buckets(const DoutPrefixProvider* dpp, const std::string& query_str, rgw_user& user,
                     const std::string& marker, const std::string& end_marker, uint64_t max, bool need_stats,
                     RGWUserBuckets* buckets, bool* is_truncated);
    int update_bucket(const DoutPrefixProvider* dpp, const std::string& query_str, RGWBucketInfo& info, bool exclusive,
                      const rgw_user* powner_id, std::map<std::string, bufferlist>* pattrs, ceph::real_time* pmtime,
    ...
    std::string to_oid(std::string_view bucket, std::string_view obj_name, std::string_view obj_instance,
                       std::string_view obj_id, std::string_view mp_str, uint64_t partnum)
    {
    ...
    int from_oid(const std::string& oid, std::string& bucket, std::string& obj_name, std::string& obj_id,
                 std::string& obj_instance, std::string& mp_str, uint64_t& partnum)
    {
    ...
    struct raw_obj
    {
        ...
        raw_obj(DB* _db) { db = _db; }

        raw_obj(DB* _db, std::string& _bname, std::string& _obj_name, std::string& _obj_instance, std::string& _obj_ns,
                std::string& _obj_id, std::string _mp_part_str, int _part_num)
        {
            db = _db;
            bucket_name = _bname;
            obj_name = _obj_name;
        ...
        raw_obj(DB* _db, std::string& oid)
        {
            ...
            db = _db;
            r = db->from_oid(oid, bucket_name, obj_name, obj_instance, obj_id, multipart_part_str, part_num);
            if (r < 0) {
    ...
    public:
        Bucket(DB* _store, const RGWBucketInfo& _binfo)
            : store(_store)
            , bucket_info(_binfo)
        {}
        ...
        public:
            ...
            explicit List(DB::Bucket* _target)
                : target(_target)
            {}
    ...
    public:
        Object(DB* _store, const RGWBucketInfo& _bucket_info, const rgw_obj& _obj)
            : store(_store)
        ...
        Object(DB* _store, const RGWBucketInfo& _bucket_info, const rgw_obj& _obj, const std::string& _obj_id)
            : store(_store)
            , bucket_info(_bucket_info)
            , obj(_obj)
            , obj_id(_obj_id)
        ...
        struct Read
        {
            ...
            explicit Read(DB::Object* _source)
                : source(_source)
            {}
        ...
        struct Write
        {
            ...
            explicit Write(DB::Object* _target)
                : target(_target)
            {}

            void set_mp_part_str(std::string _mp_part_str) { mp_part_str = _mp_part_str; }
            int prepare(const DoutPrefixProvider* dpp);
            int write_data(const DoutPrefixProvider* dpp, bufferlist& data, uint64_t ofs);
            int _do_write_meta(const DoutPrefixProvider* dpp, uint64_t size, uint64_t accounted_size,
                               std::map<std::string, bufferlist>& attrs, bool assume_noent, bool modify_tail);
        ...
        struct Delete
        {
            ...
            explicit Delete(DB::Object* _target)
                : target(_target)
            {}
        ...
        int list_versioned_objects(const DoutPrefixProvider* dpp, std::list<rgw_bucket_dir_entry>& list_entries);
        ...
        int set_attrs(const DoutPrefixProvider* dpp, std::map<std::string, bufferlist>& setattrs,
                      std::map<std::string, bufferlist>* rmattrs);
        int transition(const DoutPrefixProvider* dpp, const rgw_placement_rule& rule, const real_time& mtime,
                       uint64_t olh_epoch);
        int obj_omap_set_val_by_key(const DoutPrefixProvider* dpp, const std::string& key, bufferlist& val,
                                    bool must_exist);
        int obj_omap_get_vals_by_keys(const DoutPrefixProvider* dpp, const std::string& oid,
                                      const std::set<std::string>& keys, std::map<std::string, bufferlist>* vals);
        ...
        using iterate_obj_cb = int (*)(const DoutPrefixProvider*, const raw_obj&, off_t, off_t, bool, RGWObjState*,
                                       void*);
        ...
        int iterate_obj(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, const rgw_obj& obj, off_t ofs,
                        off_t end, uint64_t max_chunk_size, iterate_obj_cb cb, void* arg);
    };
    int get_obj_iterate_cb(const DoutPrefixProvider* dpp, const raw_obj& read_obj, off_t obj_ofs, off_t len,
    ...
    int list_entries(const std::string& oid, const std::string& marker, uint32_t max_entries,
                     std::vector<std::unique_ptr<rgw::sal::Lifecycle::LCEntry>>& entries);
    ...
    int delete_stale_objs(const DoutPrefixProvider* dpp, const std::string& bucket, uint32_t min_wait);
