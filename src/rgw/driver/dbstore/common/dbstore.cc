...
int DB::Initialize(string logfile, int loglevel)
{
    ...
    ldpp_dout(dpp, 0) << "DB successfully initialized - name:" << db_name << "" << dendl;
...
int DB::Destroy(const DoutPrefixProvider* dpp)
{
    ...
    ldpp_dout(dpp, 20) << "DB successfully destroyed - name:" << db_name << dendl;

    return 0;
}


std::shared_ptr<class DBOp> DB::getDBOp(const DoutPrefixProvider* dpp, std::string_view Op, const DBOpParams* params)
{
    if (!Op.compare("InsertUser")) return dbops.InsertUser;
    if (!Op.compare("RemoveUser")) return dbops.RemoveUser;
    if (!Op.compare("GetUser")) return dbops.GetUser;
    if (!Op.compare("InsertBucket")) return dbops.InsertBucket;
    if (!Op.compare("UpdateBucket")) return dbops.UpdateBucket;
    if (!Op.compare("RemoveBucket")) return dbops.RemoveBucket;
    if (!Op.compare("GetBucket")) return dbops.GetBucket;
    if (!Op.compare("ListUserBuckets")) return dbops.ListUserBuckets;
    if (!Op.compare("InsertLCEntry")) return dbops.InsertLCEntry;
    if (!Op.compare("RemoveLCEntry")) return dbops.RemoveLCEntry;
    if (!Op.compare("GetLCEntry")) return dbops.GetLCEntry;
    if (!Op.compare("ListLCEntries")) return dbops.ListLCEntries;
    if (!Op.compare("InsertLCHead")) return dbops.InsertLCHead;
    if (!Op.compare("RemoveLCHead")) return dbops.RemoveLCHead;
    if (!Op.compare("GetLCHead")) return dbops.GetLCHead;
    ...
    if (iter == DB::objectmap.end()) {
        ldpp_dout(dpp, 30) << "No objectmap found for bucket: " << params->op.bucket.info.bucket.name << dendl;
        /* not found */
        return nullptr;
    }
    ...
    if (!Op.compare("PutObject")) return Ob->PutObject;
    if (!Op.compare("DeleteObject")) return Ob->DeleteObject;
    if (!Op.compare("GetObject")) return Ob->GetObject;
    if (!Op.compare("UpdateObject")) return Ob->UpdateObject;
    if (!Op.compare("ListBucketObjects")) return Ob->ListBucketObjects;
    if (!Op.compare("ListVersionedObjects")) return Ob->ListVersionedObjects;
    if (!Op.compare("PutObjectData")) return Ob->PutObjectData;
    if (!Op.compare("UpdateObjectData")) return Ob->UpdateObjectData;
    if (!Op.compare("GetObjectData")) return Ob->GetObjectData;
    if (!Op.compare("DeleteObjectData")) return Ob->DeleteObjectData;
    if (!Op.compare("DeleteStaleObjectData")) return Ob->DeleteStaleObjectData;
...
int DB::objectmapInsert(const DoutPrefixProvider* dpp, string bucket, class ObjectOp* ptr)
{
    ...
    if (iter != DB::objectmap.end()) {
        ...
        ldpp_dout(dpp, 30) << "Objectmap entry already exists for bucket(" << bucket << "). Not inserted " << dendl;
        delete ptr;
        return 0;
    }
...
int DB::get_user(const DoutPrefixProvider* dpp, const std::string& query_str, const std::string& query_str_val,
                 RGWUserInfo& uinfo, map<string, bufferlist>* pattrs, RGWObjVersionTracker* pobjv_tracker)
{
    int ret = 0;

    if (query_str.empty() || query_str_val.empty()) {
        ldpp_dout(dpp, 0) << "In GetUser - Invalid query(" << query_str << "), query_str_val(" << query_str_val << ")"
                          << dendl;
        return -1;
    }
    ...
    if (params.op.user.uinfo.access_keys.empty() || params.op.user.uinfo.user_id.id.empty()) {
        ldpp_dout(dpp, 0) << "In GetUser - No user with query(" << query_str.c_str() << "), user_id(" << uinfo.user_id
                          << ") found" << dendl;
        return -ENOENT;
    }
...
int DB::store_user(const DoutPrefixProvider* dpp, RGWUserInfo& uinfo, bool exclusive, map<string, bufferlist>* pattrs,
...
int DB::remove_user(const DoutPrefixProvider* dpp, RGWUserInfo& uinfo, RGWObjVersionTracker* pobjv)
{
    DBOpParams params = {};
    InitializeParams(dpp, &params);
    int ret = 0;
...
int DB::get_bucket_info(const DoutPrefixProvider* dpp, const std::string& query_str, const std::string& query_str_val,
...
int DB::create_bucket(const DoutPrefixProvider* dpp, const RGWUserInfo& owner, rgw_bucket& bucket,
                      const string& zonegroup_id, const rgw_placement_rule& placement_rule,
                      const string& swift_ver_location, const RGWQuotaInfo* pquota_info,
                      map<std::string, bufferlist>& attrs, RGWBucketInfo& info, obj_version* pobjv,
...
int DB::list_buckets(const DoutPrefixProvider* dpp, const std::string& query_str, rgw_user& user, const string& marker,
                     ...
                     bool* is_truncated)
{
     ...
     */
    if (params.op.bucket.list_entries.size() == max) *is_truncated = true;

    for (auto& entry : params.op.bucket.list_entries) {
        if (!end_marker.empty() && end_marker.compare(entry.bucket.marker) <= 0) {
            *is_truncated = false;
            break;
        }
...
int DB::update_bucket(const DoutPrefixProvider* dpp, const std::string& query_str, RGWBucketInfo& info, bool exclusive,
                      const rgw_user* powner_id, map<std::string, bufferlist>* pattrs, ceph::real_time* pmtime,
                      RGWObjVersionTracker* pobjv)
{
    ...
    params.op.bucket.info.bucket.name = info.bucket.name;
    ret = get_bucket_info(dpp, string("name"), "", orig_info, nullptr, nullptr, &bucket_version);
 ...
 * max, then truncated.
 */
int DB::Bucket::List::list_objects(const DoutPrefixProvider* dpp, int64_t max, vector<rgw_bucket_dir_entry>* result,
    ...
    for (auto& entry : db_params.op.obj.list_entries) {
        ...
        if (!params.delim.empty()) {
            ...
            if (delim_pos >= 0) {
                ...
                const std::string& prefix_key = objname.substr(0, delim_pos + params.delim.length());
        ...
        if (!params.end_marker.name.empty() && params.end_marker.name.compare(entry.key.name) <= 0) {
...
int DB::raw_obj::InitializeParamsfromRawObj(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    int ret = 0;
...
int DB::Object::InitializeParamsfromObject(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    int ret = 0;
...
int DB::Object::get_object_impl(const DoutPrefixProvider* dpp, DBOpParams& params)
{
    ...
    if (!ret && !params.op.obj.state.exists) {
        ldpp_dout(dpp, 0) << "Object(bucket:" << bucket_info.bucket.name << ", Object:" << obj.key.name
                          << ") doesn't exist" << dendl;
        ret = -ENOENT;
    }
...
int DB::Object::obj_omap_set_val_by_key(const DoutPrefixProvider* dpp, const std::string& key, bufferlist& val,
                                        bool must_exist)
{
    int ret = 0;
...
int DB::Object::obj_omap_get_vals_by_keys(const DoutPrefixProvider* dpp, const std::string& oid,
...
int DB::Object::add_mp_part(const DoutPrefixProvider* dpp, RGWUploadPartInfo info)
{
    int ret = 0;
...
int DB::Object::get_mp_parts_list(const DoutPrefixProvider* dpp, std::list<RGWUploadPartInfo>& info)
{
    int ret = 0;
    DBOpParams params = {};
    std::map<std::string, bufferlist> omap;
...
int DB::Object::obj_omap_get_all(const DoutPrefixProvider* dpp, std::map<std::string, bufferlist>* m)
{
    int ret = 0;
    DBOpParams params = {};
    std::map<std::string, bufferlist> omap;
...
int DB::Object::obj_omap_get_vals(const DoutPrefixProvider* dpp, const std::string& marker, uint64_t max_count,
...
int DB::Object::set_attrs(const DoutPrefixProvider* dpp, map<string, bufferlist>& setattrs,
...
int DB::Object::transition(const DoutPrefixProvider* dpp, const rgw_placement_rule& rule, const real_time& mtime,
                           uint64_t olh_epoch)
{
    int ret = 0;
...
int DB::raw_obj::read(const DoutPrefixProvider* dpp, int64_t ofs, uint64_t len, bufferlist& bl)
{
    int ret = 0;
    DBOpParams params = {};
...
int DB::raw_obj::write(const DoutPrefixProvider* dpp, int64_t ofs, int64_t write_ofs, uint64_t len, bufferlist& bl)
{
    int ret = 0;
    DBOpParams params = {};
...
int DB::Object::list_versioned_objects(const DoutPrefixProvider* dpp, std::list<rgw_bucket_dir_entry>& list_entries)
{
    int ret = 0;
    store = get_store();
    DBOpParams db_params = {};
...
int DB::Object::get_obj_state(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, const rgw_obj& obj,
...
int DB::Object::Read::read(int64_t ofs, int64_t end, bufferlist& bl, const DoutPrefixProvider* dpp)
{
    ...
    raw_obj read_obj(store,
                     source->get_bucket_info().bucket.name,
                     astate->obj.key.name,
                     astate->obj.key.instance,
                     astate->obj.key.ns,
                     source->obj_id,
                     "0.0",
                     part_num);
    ...
    ldpp_dout(dpp, 20) << "dbstore->read obj-ofs=" << ofs << " read_ofs=" << read_ofs << " read_len=" << read_len
                       << dendl;
...
static int _get_obj_iterate_cb(const DoutPrefixProvider* dpp, const DB::raw_obj& read_obj, off_t obj_ofs, off_t len,
                               bool is_head_obj, RGWObjState* astate, void* arg)
{
    struct db_get_obj_data* d = static_cast<struct db_get_obj_data*>(arg);
    return d->store->get_obj_iterate_cb(dpp, read_obj, obj_ofs, len, is_head_obj, astate, arg);
}

int DB::get_obj_iterate_cb(const DoutPrefixProvider* dpp, const raw_obj& read_obj, off_t obj_ofs, off_t len,
                           bool is_head_obj, RGWObjState* astate, void* arg)
{
    struct db_get_obj_data* d = static_cast<struct db_get_obj_data*>(arg);
    bufferlist bl;
    ...
    while (read_ofs < bl.length()) {
        ...
        read_ofs += chunk_len;
        read_len += chunk_len;
        ldpp_dout(dpp, 20) << "dbstore->get_obj_iterate_cb  obj-ofs=" << obj_ofs << " len=" << len
                           << " chunk_len = " << chunk_len << " read_len = " << read_len << dendl;
    }
...
int DB::Object::Read::iterate(const DoutPrefixProvider* dpp, int64_t ofs, int64_t end, RGWGetDataCB* cb)
{
    ...
    int r = source->iterate_obj(
        dpp, source->get_bucket_info(), state.obj, ofs, end, chunk_size, _get_obj_iterate_cb, &data);
    if (r < 0) {
...
int DB::Object::iterate_obj(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, const rgw_obj& obj,
                            off_t ofs, off_t end, uint64_t max_chunk_size, iterate_obj_cb cb, void* arg)
{
    DB* store = get_store();
    uint64_t len;
    RGWObjState* astate;
...
/* writes tail objects */
int DB::Object::Write::write_data(const DoutPrefixProvider* dpp, bufferlist& data, uint64_t ofs)
{
    DB* store = target->get_store();
    /* tail objects */
    ...
    while (write_ofs < end) {
        ...
        /* XXX: Handle multipart_str */
        raw_obj write_obj(store,
                          target->get_bucket_info().bucket.name,
                          obj_state.obj.key.name,
                          obj_state.obj.key.instance,
                          obj_state.obj.key.ns,
                          target->obj_id,
                          mp_part_str,
                          part_num);
...
int DB::Object::Write::_do_write_meta(const DoutPrefixProvider* dpp, uint64_t size, uint64_t accounted_size,
...
int DB::Object::Delete::delete_obj_impl(const DoutPrefixProvider* dpp, DBOpParams& del_params)
{
    int ret = 0;
    DB* store = target->get_store();
 ...
 */
int DB::Object::Delete::create_dm(const DoutPrefixProvider* dpp, DBOpParams& del_params)
{
...
int DB::list_entries(const std::string& oid, const std::string& marker, uint32_t max_entries,
...
int DB::delete_stale_objs(const DoutPrefixProvider* dpp, const std::string& bucket, uint32_t min_wait)
{
    DBOpParams params = {};
    int ret = -1;
...
void* DB::GC::entry()
{
    do {
        ...
        do {
            ...
            int r = db->list_buckets(dpp, "all", user, marker, string(), max, false, &buckets, &is_truncated);
