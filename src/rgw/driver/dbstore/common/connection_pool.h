...
template<typename Connection> class ConnectionHandle;
...
template<typename Connection> class ConnectionPoolBase
{
public:
...
public:
    ...
    ConnectionHandle(ConnectionPoolBase<Connection>* pool, std::unique_ptr<Connection> conn) noexcept
        : pool(pool)
        , conn(std::move(conn))
    {}
...
public:
    ...
    auto get(const DoutPrefixProvider* dpp) -> ConnectionHandle<Connection>
    {
