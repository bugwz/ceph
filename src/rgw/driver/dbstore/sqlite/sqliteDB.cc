    ...
    do {                                                                                                               \
        ...
        if (!stmt) {                                                                                                   \
            ldpp_dout(dpp, 0) << "failed to prepare statement "                                                        \
                              << "for Op(" << Op << "); Errmsg -" << sqlite3_errmsg(*sdb) << dendl;                    \
            ret = -1;                                                                                                  \
            goto out;                                                                                                  \
        }                                                                                                              \
        ldpp_dout(dpp, 20) << "Successfully Prepared stmt for Op(" << Op << ") schema(" << schema << ") stmt(" << stmt \
                           << ")" << dendl;                                                                            \
        ret = 0;                                                                                                       \
    } while (0);
    ...
    do {                                                                                                         \
        ...
        if (index <= 0) {                                                                                        \
            ldpp_dout(dpp, 0) << "failed to fetch bind parameter"                                                \
                                 " index for str("                                                               \
                              << str << ") in "                                                                  \
                              << "stmt(" << stmt << "); Errmsg -" << sqlite3_errmsg(*sdb) << dendl;              \
            rc = -1;                                                                                             \
            goto out;                                                                                            \
        }                                                                                                        \
        ldpp_dout(dpp, 20) << "Bind parameter index for str(" << str << ") in stmt(" << stmt << ") is " << index \
                           << dendl;                                                                             \
    } while (0);
...
#define SQL_BIND_TEXT(dpp, stmt, index, str, sdb)                                                                  \
    do {                                                                                                           \
        ...
        if (rc != SQLITE_OK) {                                                                                     \
            ldpp_dout(dpp, 0) << "sqlite bind text failed for index(" << index << "), str(" << str << ") in stmt(" \
                              << stmt << "); Errmsg - " << sqlite3_errmsg(*sdb) << dendl;                          \
            rc = -1;                                                                                               \
            goto out;                                                                                              \
        }                                                                                                          \
        ldpp_dout(dpp, 20) << "Bind parameter text for index(" << index << ") in stmt(" << stmt << ") is " << str  \
                           << dendl;                                                                               \
    } while (0);
...
#define SQL_BIND_INT(dpp, stmt, index, num, sdb)                                                                  \
    do {                                                                                                          \
        rc = sqlite3_bind_int(stmt, index, num);                                                                  \
                                                                                                                  \
        if (rc != SQLITE_OK) {                                                                                    \
            ldpp_dout(dpp, 0) << "sqlite bind int failed for index(" << index << "), num(" << num << ") in stmt(" \
                              << stmt << "); Errmsg - " << sqlite3_errmsg(*sdb) << dendl;                         \
            rc = -1;                                                                                              \
            goto out;                                                                                             \
        }                                                                                                         \
        ldpp_dout(dpp, 20) << "Bind parameter int for index(" << index << ") in stmt(" << stmt << ") is " << num  \
                           << dendl;                                                                              \
    } while (0);
...
#define SQL_BIND_BLOB(dpp, stmt, index, blob, size, sdb)                                                             \
    do {                                                                                                             \
        rc = sqlite3_bind_blob(stmt, index, blob, size, SQLITE_TRANSIENT);                                           \
                                                                                                                     \
        if (rc != SQLITE_OK) {                                                                                       \
            ldpp_dout(dpp, 0) << "sqlite bind blob failed for index(" << index << "), blob(" << blob << ") in stmt(" \
                              << stmt << "); Errmsg - " << sqlite3_errmsg(*sdb) << dendl;                            \
            rc = -1;                                                                                                 \
            goto out;                                                                                                \
        }                                                                                                            \
...
#define SQL_READ_BLOB(dpp, stmt, index, void_ptr, len)                                                            \
    do {                                                                                                          \
        ...
        len = sqlite3_column_bytes(stmt, index);                                                                  \
                                                                                                                  \
        if (!void_ptr || len == 0) {                                                                              \
            ldpp_dout(dpp, 20) << "Null value for blob index(" << index << ") in stmt(" << stmt << ") " << dendl; \
        }                                                                                                         \
    } while (0);
...
int SQLiteDB::InitPrepareParams(const DoutPrefixProvider* dpp, DBOpPrepareParams& p_params, DBOpParams* params)
{
    std::string bucket;
...
static int list_object(const DoutPrefixProvider* dpp, DBOpInfo& op, sqlite3_stmt* stmt)
{
    ...
    op.obj.tail_placement.placement_rule.storage_class =
        (const char*)sqlite3_column_text(stmt, TailPlacementStorageClass);
...
void* SQLiteDB::openDB(const DoutPrefixProvider* dpp)
{
    ...
    rc = sqlite3_open_v2(
        dbname.c_str(), (sqlite3**)&db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_FULLMUTEX, NULL);

    if (rc) {
        ldpp_dout(dpp, 0) << "Cant open " << dbname << "; Errmsg - " << sqlite3_errmsg((sqlite3*)db) << dendl;
    }
    else {
...
again:
    ...
    if ((ret != SQLITE_DONE) && (ret != SQLITE_ROW)) {
        ldpp_dout(dpp, 0) << "sqlite step failed for stmt(" << stmt << "); Errmsg - " << sqlite3_errmsg((sqlite3*)db)
                          << dendl;
        return -1;
    }
    ...
    ldpp_dout(dpp, 20) << "sqlite step successfully executed for stmt(" << stmt << ")  ret = " << ret << dendl;

    return 0;
}

int SQLiteDB::exec(const DoutPrefixProvider* dpp, const char* schema, int (*callback)(void*, int, char**, char**))
{
    int ret = -1;
    char* errmsg = NULL;
    ...
    if (ret != SQLITE_OK) {
        ldpp_dout(dpp, 0) << "sqlite exec failed for schema(" << schema << "); Errmsg - " << errmsg << dendl;
        sqlite3_free(errmsg);
        goto out;
    }
    ret = 0;
    ldpp_dout(dpp, 10) << "sqlite exec successfully processed for schema(" << schema << ")" << dendl;
out:
    return ret;
}
...
int SQLiteDB::createTables(const DoutPrefixProvider* dpp)
{
    ...
    if ((cu = createUserTable(dpp, &params))) goto out;

    if ((cb = createBucketTable(dpp, &params))) goto out;

    if ((cq = createQuotaTable(dpp, &params))) goto out;

    ret = 0;
out:
    if (ret) {
        if (cu) DeleteUserTable(dpp, &params);
        if (cb) DeleteBucketTable(dpp, &params);
...
int SQLiteDB::createUserTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    if (ret) ldpp_dout(dpp, 0) << "CreateUserTable failed" << dendl;
...
int SQLiteDB::createBucketTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "CreateBucketTable failed " << dendl;
...
int SQLiteDB::createObjectTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "CreateObjectTable failed " << dendl;
...
int SQLiteDB::createObjectTableTrigger(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "CreateObjectTableTrigger failed " << dendl;
...
int SQLiteDB::createObjectView(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "CreateObjectView failed " << dendl;
...
int SQLiteDB::createQuotaTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "CreateQuotaTable failed " << dendl;
...
int SQLiteDB::createObjectDataTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "CreateObjectDataTable failed " << dendl;
...
int SQLiteDB::DeleteUserTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "DeleteUserTable failed " << dendl;
...
int SQLiteDB::DeleteBucketTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "DeletebucketTable failed " << dendl;
...
int SQLiteDB::DeleteObjectTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "DeleteObjectTable failed " << dendl;
...
int SQLiteDB::DeleteObjectDataTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "DeleteObjectDataTable failed " << dendl;
...
int SQLiteDB::DeleteQuotaTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "DeleteQuotaTable failed " << dendl;
...
int SQLiteDB::DeleteLCEntryTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "DeleteLCEntryTable failed " << dendl;
...
int SQLiteDB::DeleteLCHeadTable(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), NULL);
    if (ret) ldpp_dout(dpp, 0) << "DeleteLCHeadTable failed " << dendl;
...
int SQLiteDB::ListAllUsers(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    if (ret) ldpp_dout(dpp, 0) << "GetUsertable failed " << dendl;
...
int SQLiteDB::ListAllBuckets(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    ret = exec(dpp, schema.c_str(), &list_callback);
    if (ret) ldpp_dout(dpp, 0) << "Listbuckettable failed " << dendl;
...
int SQLiteDB::ListAllObjects(const DoutPrefixProvider* dpp, DBOpParams* params)
{
    ...
    if (objectmap.empty()) ldpp_dout(dpp, 20) << "objectmap empty " << dendl;
    ...
    for (iter = objectmap.begin(); iter != objectmap.end(); ++iter) {
        ...
        ret = exec(dpp, schema.c_str(), &list_callback);
        if (ret) ldpp_dout(dpp, 0) << "ListObjecttable failed " << dendl;
...
int SQLInsertUser::Bind(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
    ...
    if (!params->op.user.uinfo.access_keys.empty()) {
        string access_key;
        string key;
        map<string, RGWAccessKey>::const_iterator it = params->op.user.uinfo.access_keys.begin();
...
int SQLUpdateBucket::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
    ...
    if (params->op.query_str == "attrs") {
        ...
        ldpp_dout(dpp, 0) << "In SQLUpdateBucket invalid query_str:" << params->op.query_str << "" << dendl;
        goto out;
    }
...
int SQLUpdateObject::Prepare(const DoutPrefixProvider* dpp, struct DBOpParams* params)
{
    ...
    if (params->op.query_str == "omap") {
        ...
        ldpp_dout(dpp, 0) << "In SQLUpdateObject invalid query_str:" << params->op.query_str << dendl;
        goto out;
    }
