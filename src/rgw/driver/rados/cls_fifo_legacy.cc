...
struct Updater : public Completion<Updater>
{
    ...
    {
        ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " handling async read_meta: tid=" << tid
                           << dendl;
        if (r < 0 && pcanceled) {
            *pcanceled = false;
        }
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " failed dispatching read_meta: r=" << r
                               << " tid=" << tid << dendl;
        }
        else {
            ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " completing: tid=" << tid << dendl;
                            ...
                            optional_yield y)
{
    ...
    for (auto i = 0; canceled && i < MAX_RACE_RETRIES; ++i) {
        ...
        if (r >= 0 && canceled) {
            ...
            auto found =
                (info.journal.contains({create, new_part_num}) || info.journal.contains({set_head, new_part_num}));
...
private:
    ...
    void finish_je(const DoutPrefixProvider* dpp, Ptr&& p, int r, const fifo::journal_entry& entry)
    {
        ...
        using enum fifo::journal_entry::Op;
        if (entry.op == remove && r == -ENOENT) r = 0;
...
public:
    ...
    void process(const DoutPrefixProvider* dpp, Ptr&& p)
    {
        ...
        while (iter != journal.end()) {
            ...
            switch (entry.op) {
                using enum fifo::journal_entry::Op;
            case create: create_part(dpp, std::move(p), entry.part_num); return;
            case set_head:
                if (entry.part_num > new_head) {
                    new_head = entry.part_num;
                }
                ...
                ++iter;
                continue;
            case remove: remove_part(dpp, std::move(p), entry.part_num); return;
            default:
                ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " unknown journaled op: entry=" << entry
                                   << " tid=" << tid << dendl;
                complete(std::move(p), -EIO);
