...
class RGWReadDataSyncStatusMarkersCR : public RGWShardCollectCR
{
    ...
    int handle_result(int r) override
    {
        ...
        if (r < 0) {
            ldout(cct, 4) << "failed to read data sync status: " << cpp_strerror(r) << dendl;
        }
        return r;
    }

public:
    RGWReadDataSyncStatusMarkersCR(RGWDataSyncCtx* sc, int num_shards, map<uint32_t, rgw_data_sync_marker>& markers,
                                   std::vector<RGWObjVersionTracker>& objvs)
...
class RGWReadDataSyncRecoveringShardsCR : public RGWShardCollectCR
{
    ...
    int handle_result(int r) override
    {
        ...
        if (r < 0) {
            ldout(cct, 4) << "failed to list recovering data sync: " << cpp_strerror(r) << dendl;
        }
        return r;
    }
...
{
    if (shard_id >= num_shards) return false;
    ...
    spawn(new RGWRadosGetOmapKeysCR(env->driver,
...
public:
    RGWReadDataSyncStatusCoroutine(RGWDataSyncCtx* _sc, rgw_data_sync_status* _status,
                                   RGWObjVersionTracker* objv_tracker, std::vector<RGWObjVersionTracker>& objvs)
...
int RGWReadDataSyncStatusCoroutine::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        if (retcode < 0) {
            ldpp_dout(dpp, 4) << "failed to read sync status info with " << cpp_strerror(retcode) << dendl;
            return set_cr_error(retcode);
        }
        // read shard markers
        objvs.resize(sync_status->sync_info.num_shards);
        using ReadMarkersCR = RGWReadDataSyncStatusMarkersCR;
        yield call(new ReadMarkersCR(sc, sync_status->sync_info.num_shards, sync_status->sync_markers, objvs));
        if (retcode < 0) {
            ldpp_dout(dpp, 4) << "failed to read sync status markers with " << cpp_strerror(retcode) << dendl;
            return set_cr_error(retcode);
        }
        return set_cr_done();
    }
...
public:
    RGWReadRemoteDataLogShardInfoCR(RGWDataSyncCtx* _sc, int _shard_id, RGWDataChangesLogInfo* _shard_info)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , http_op(NULL)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            yield
            {
                ...
                rgw_http_param_pair pairs[] = {{"type", "data"}, {"id", buf}, {"info", NULL}, {NULL, NULL}};
                ...
                if (ret < 0) {
                    ...
                    log_error() << "failed to send http operation: " << http_op->to_str() << " ret=" << ret
                                << std::endl;
...
public:
    RGWReadRemoteDataLogShardCR(RGWDataSyncCtx* _sc, int _shard_id, const std::string& marker, string* pnext_marker,
                                vector<rgw_data_change_log_entry>* _entries, bool* _truncated)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , shard_id(_shard_id)
        ...
        reenter(this)
        {
            yield
            {
                ...
                rgw_http_param_pair pairs[] = {
                    {"type", "data"}, {"id", buf}, {"marker", marker.c_str()}, {"extra-info", "true"}, {NULL, NULL}};
...
#define READ_DATALOG_MAX_CONCURRENT 10
    ...
    int handle_result(int r) override
    {
        ...
        if (r < 0) {
            ldout(cct, 4) << "failed to fetch remote datalog info: " << cpp_strerror(r) << dendl;
        }
        return r;
    }

public:
    RGWReadRemoteDataLogInfoCR(RGWDataSyncCtx* _sc, int _num_shards, map<int, RGWDataChangesLogInfo>* _datalog_info)
        : RGWShardCollectCR(_sc->cct, READ_DATALOG_MAX_CONCURRENT)
        , sc(_sc)
        , sync_env(_sc->env)
        , num_shards(_num_shards)
        , datalog_info(_datalog_info)
        , shard_id(0)
    {}
    bool spawn_next() override;
};
...
public:
    RGWListRemoteDataLogShardCR(RGWDataSyncCtx* sc, int _shard_id, const string& _marker, uint32_t _max_entries,
                                rgw_datalog_shard_data* _result)
        : RGWSimpleCoroutine(sc->cct)
        , sc(sc)
        , sync_env(sc->env)
        , http_op(NULL)
        , shard_id(_shard_id)
...
#define READ_DATALOG_MAX_CONCURRENT 10
    ...
    int handle_result(int r) override
    {
        ...
        if (r < 0) {
            ldout(cct, 4) << "failed to list remote datalog: " << cpp_strerror(r) << dendl;
        }
        return r;
    }

public:
    RGWListRemoteDataLogCR(RGWDataSyncCtx* _sc, map<int, string>& _shards, int _max_entries_per_shard,
...
bool RGWListRemoteDataLogCR::spawn_next()
{
    ...
    spawn(
        new RGWListRemoteDataLogShardCR(sc, iter->first, iter->second, max_entries_per_shard, &(*result)[iter->first]),
        false);
    ++iter;
    return true;
}
...
class RGWInitDataSyncStatusCoroutine : public RGWCoroutine
{
    ...
    const string sync_status_oid{RGWDataSyncStatusManager::sync_status_oid(sc->source_zone)};

    map<int, RGWDataChangesLogInfo> shards_info;


public:
    RGWInitDataSyncStatusCoroutine(RGWDataSyncCtx* _sc, uint32_t num_shards, uint64_t instance_id,
    ...
    static auto continuous_lease_cr(RGWDataSyncCtx* const sc, RGWCoroutine* const caller)
    {
        auto lock_duration = sc->cct->_conf->rgw_sync_lease_period;
        return new RGWContinuousLeaseCR(sc->env->async_rados,
                                        sc->env->driver,
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        ...
        reenter(this)
        {
            ...
            yield call(new WriteInfoCR(
                dpp, sync_env->driver, rgw_raw_obj{pool, sync_status_oid}, status->sync_info, &objv_tracker));
            if (retcode < 0) {
            ...
            yield
            {
                ...
                for (uint32_t i = 0; i < num_shards; i++) {
                    ...
                    spawn(new WriteMarkerCR(dpp, sync_env->driver, rgw_raw_obj{pool, oid}, marker, &objv), true);
                }
            }
            while (collect(&ret, NULL)) {
                if (ret < 0) {
...
RGWRemoteDataLog::RGWRemoteDataLog(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* driver,
...
int RGWRemoteDataLog::read_source_log_shards_info(const DoutPrefixProvider* dpp,
                                                  map<int, RGWDataChangesLogInfo>* shards_info)
{
...
int RGWRemoteDataLog::read_source_log_shards_next(const DoutPrefixProvider* dpp, map<int, string> shard_markers,
                                                  map<int, rgw_datalog_shard_data>* result)
{
...
int RGWRemoteDataLog::read_sync_status(const DoutPrefixProvider* dpp, rgw_data_sync_status* sync_status)
{
    ...
    ret = crs.run(dpp, new RGWReadDataSyncStatusCoroutine(&sc_local, sync_status, &objv, shard_objvs));
    http_manager.stop();
    return ret;
}

int RGWRemoteDataLog::read_recovering_shards(const DoutPrefixProvider* dpp, const int num_shards,
...
public:
    DataSyncInitCR(RGWDataSyncCtx* sc, uint32_t num_shards, uint64_t instance_id, const RGWSyncTraceNodeRef& tn,
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            lease_cr.reset(RGWInitDataSyncStatusCoroutine::continuous_lease_cr(sc, this));
            ...
            yield call(new RGWInitDataSyncStatusCoroutine(
                sc, num_shards, instance_id, tn, sync_status, lease_cr, objv_tracker, objvs));
            lease_cr->go_down();
            lease_cr.reset();
            drain_all();
            if (retcode < 0) {
...
int RGWRemoteDataLog::init_sync_status(const DoutPrefixProvider* dpp, int num_shards)
{
    ...
    RGWDataSyncCtx sc_local = sc;
    sc_local.env = &sync_env_local;
    ret = crs.run(dpp, new RGWRDL::DataSyncInitCR(&sc_local, num_shards, instance_id, tn, &sync_status, objvs));
    http_manager.stop();
    return ret;
}
...
static string full_data_sync_index_shard_oid(const rgw_zone_id& source_zone, int shard_id)
{
    ...
    snprintf(
        buf, sizeof(buf), "%s.%s.%d", datalog_sync_full_sync_index_prefix.c_str(), source_zone.id.c_str(), shard_id);
    return string(buf);
}
...
public:
    RGWReadRemoteBucketIndexLogInfoCR(RGWDataSyncCtx* _sc, const rgw_bucket& bucket,
                                      rgw_bucket_index_marker_info* _info)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , instance_key(bucket.get_key())
        , info(_info)
    {}
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            {
                rgw_http_param_pair pairs[] = {
                    {"type", "bucket-index"}, {"bucket-instance", instance_key.c_str()}, {"info", NULL}, {NULL, NULL}};
...
class RGWListBucketIndexesCR : public RGWCoroutine
{
    ...
    string oid_prefix = datalog_sync_full_sync_index_prefix + "." + sc->source_zone.id;
...
public:
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            entries_index = std::make_unique<RGWShardedOmapCRManager>(sync_env->async_rados,
                                                                      driver,
                                                                      this,
            ...
            do {
                yield
                {
                    ...
                    rgw_http_param_pair pairs[] = {
                        {"max-entries", "1000"}, {"marker", result.marker.c_str()}, {NULL, NULL}};

                    call(new RGWReadRESTResourceCR<read_metadata_list>(
                        sync_env->cct, sc->conn, sync_env->http_manager, entrypoint, pairs, &result));
                }
                if (retcode < 0) {
                ...
                for (iter = result.keys.begin(); iter != result.keys.end(); ++iter) {
                    ...
                    yield
                    {
                        ...
                        call(new RGWReadRESTResourceCR<bucket_instance_meta_info>(
                            sync_env->cct, sc->conn, sync_env->http_manager, path, pairs, &meta_info));
                    }
                    if (retcode < 0) {
                        ldpp_dout(dpp, 0) << "ERROR: failed to fetch metadata for key: " << key << dendl;
                        return set_cr_error(retcode);
                    }
                    ...
                    yield entries_index->append(
                        fmt::format("{}:{}", key, 0),
                        sync_env->svc->datalog_rados->get_log_shard_id(meta_info.data.get_bucket_info().bucket, 0));
                }
            ...
            if (!failed) {
                for (auto iter = sync_status->sync_markers.begin(); iter != sync_status->sync_markers.end(); ++iter) {
                ...
                yield call(sync_env->error_logger->log_error_cr(dpp,
                                                                sc->conn->get_remote_id(),
                                                                "data.init",
                                                                "",
                                                                EIO,
...
public:
    RGWDataSyncShardMarkerTrack(RGWDataSyncCtx* _sc, const string& _marker_oid, const rgw_data_sync_marker& _marker,
    ...
    RGWOrderCallCR* allocate_order_control_cr() override { return new RGWLastCallerWinsCR(sync_env->cct); }
};
...
{
    const rgw_bucket& b;
    explicit bucket_str(const rgw_bucket& b)
        : b(b)
    {}
};
...
{
    const rgw_bucket& b;
    explicit bucket_str_noinstance(const rgw_bucket& b)
        : b(b)
    {}
};
...
struct bucket_shard_str
{
    ...
    explicit bucket_shard_str(const rgw_bucket_shard& bs)
        : bs(bs)
    {}
};
...
public:
    ...
    rgw_sync_pipe_info_entity(const rgw_sync_bucket_entity& e, std::optional<all_bucket_info>& binfo)
    {
        if (e.zone) {
            zone = *e.zone;
        }
...
struct rgw_sync_pipe_info_set
{
    ...
    void insert(const RGWBucketSyncFlowManager::pipe_handler& handler,
                std::optional<all_bucket_info>& source_bucket_info, std::optional<all_bucket_info>& target_bucket_info)
    {
...
public:
    RGWRunBucketSourcesSyncCR(RGWDataSyncCtx* _sc, boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
...
public:
    RGWDataSyncSingleEntryCR(RGWDataSyncCtx* _sc, rgw::bucket_sync::Handle state, rgw_data_sync_obligation _obligation,
                             RGWDataSyncShardMarkerTrack* _marker_tracker, const rgw_raw_obj& error_repo,
                             boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
        ...
        reenter(this)
        {
            if (state->obligation) {
                ...
                while ((state->obligation->timestamp == ceph::real_time() ||
                    ...
                    yield call(new RGWRunBucketSourcesSyncCR(
                        sc, lease_cr, state->key.first, tn, state->obligation->gen, &progress));
                    if (retcode < 0) {
                        break;
                    }
                ...
                tn->log(10,
                        SSTR("sync finished on " << bucket_shard_str{state->key.first} << " progress=" << progress
                                                 << ' ' << complete << " r=" << retcode));
            }
            sync_status = retcode;
            ...
            if (sync_status < 0) {
                ...
                if (complete->timestamp != ceph::real_time{}) {
                    ...
                    yield call(rgw::error_repo::write_cr(sync_env->driver->svc()->rados,
                                                         error_repo,
                ...
                yield call(rgw::error_repo::remove_cr(sync_env->driver->svc()->rados,
                                                      error_repo,
                                                      rgw::error_repo::encode_key(complete->bs, complete->gen),
                                                      complete->timestamp));
                if (retcode < 0) {
                    tn->log(0,
                            SSTR("ERROR: failed to remove omap key from error repo (" << error_repo
                                                                                      << " retcode=" << retcode));
                }
            }
...
rgw_raw_obj datalog_oid_for_error_repo(RGWDataSyncCtx* sc, rgw::sal::RadosStore* driver, rgw_pool& pool,
                                       rgw_bucket_shard& bs)
{
        ...
        reenter(this)
        {
            ...
            for (; each != remote_info.generations.cend(); each++) {
                for (sid = 0; sid < each->num_shards; sid++) {
                    ...
                    tn->log(10,
                            SSTR("writing shard_id " << sid << " of gen " << each->gen << " to error repo for retry"));
                    yield_spawn_window(rgw::error_repo::write_cr(sync_env->driver->svc()->rados,
                                                                 error_repo,
            ...
            yield call(rgw::error_repo::remove_cr(sync_env->driver->svc()->rados, error_repo, error_marker, timestamp));
            return set_cr_done();
        }
        return 0;
    }
...
RGWCoroutine* data_sync_single_entry(RGWDataSyncCtx* sc, const rgw_bucket_shard& src, std::optional<uint64_t> gen,
                                     const std::string marker, ceph::real_time timestamp,
                                     boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
                                     ...
                                     RGWSyncTraceNodeRef& tn, bool retry)
{
    ...
    return new RGWDataSyncSingleEntryCR(
        sc, std::move(state), std::move(obligation), &*marker_tracker, error_repo, lease_cr.get(), tn);
}

static ceph::real_time timestamp_for_bucket_shard(rgw::sal::RadosStore* driver, const rgw_data_sync_status& sync_status,
                                                  const rgw_bucket_shard& bs)
{
    int datalog_shard = driver->svc()->datalog_rados->choose_oid(bs);
...
public:
    RGWDataFullSyncSingleEntryCR(RGWDataSyncCtx* _sc, const rgw_pool& _pool, const rgw_bucket_shard& _source_bs,
                                 const std::string& _key, const rgw_data_sync_status& sync_status,
                                 const rgw_raw_obj& _error_repo, ceph::real_time _timestamp,
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            if (error_inject && rand() % 10000 < cct->_conf->rgw_sync_data_full_inject_err_probability * 10000.0) {
            ...
            if (retcode < 0) {
                tn->log(10,
                        SSTR("full sync: failed to read remote bucket info. Writing " << source_bs.shard_id
                ...
                if (retcode < 0) {
                    tn->log(
                        0,
                        SSTR("ERROR: failed to log " << source_bs.shard_id << " in error repo: retcode=" << retcode));
                }
            ...
            for (; each != remote_info.generations.end(); each++) {
                for (sid = 0; sid < each->num_shards; sid++) {
                    ...
                    if (retcode < 0) {
                        ...
                        shard_cr = data_sync_single_entry(sc,
                                                          source_bs,
                                                          each->gen,
                                                          key,
                                                          timestamp,
...
protected:
    ...
    {
        int ret = rgw_bucket_parse_bucket_key(sc->env->cct, key, &bs.bucket, &bs.shard_id);
    ...
    RGWDataBaseSyncShardCR(RGWDataSyncCtx* const _sc, const rgw_pool& pool, const uint32_t shard_id,
                           rgw_data_sync_marker& sync_marker, RGWSyncTraceNodeRef tn, const string& status_oid,
                           const rgw_raw_obj& error_repo, boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
...
public:
    RGWDataFullSyncShardCR(RGWDataSyncCtx* const sc, const rgw_pool& pool, const uint32_t shard_id,
                           rgw_data_sync_marker& sync_marker, RGWSyncTraceNodeRef tn, const string& status_oid,
                           const rgw_raw_obj& error_repo, boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
                           const rgw_data_sync_status& sync_status, RGWObjVersionTracker& objv,
                           const boost::intrusive_ptr<rgw::bucket_sync::Cache>& bucket_shard_cache)
        : RGWDataBaseSyncShardCR(sc, pool, shard_id, sync_marker, tn, status_oid, error_repo, std::move(lease_cr),
        ...
        reenter(this)
        {
            ...
            do {
                ...
                yield call(new RGWRadosGetOmapValsCR(
                    sc->env->driver, rgw_raw_obj(pool, oid), sync_marker.marker, OMAP_GET_MAX_ENTRIES, omapvals));
                if (retcode < 0) {
                    drain_all();
                    return set_cr_error(retcode);
                }
                ...
                for (; iter != entries.end(); ++iter) {
                    ...
                    if (!marker_tracker->start(iter->first, total_entries, entry_timestamp)) {
                        ...
                        yield_spawn_window(new RGWDataFullSyncSingleEntryCR(sc,
                                                                            pool,
                                                                            source_bs,
                                                                            iter->first,
                                                                            sync_status,
            ...
            yield call(new RGWSimpleRadosWriteCR<rgw_data_sync_marker>(
                sc->env->dpp, sc->env->driver, rgw_raw_obj(pool, status_oid), sync_marker, &objv));
            if (retcode < 0) {
...
public:
    RGWDataIncSyncShardCR(RGWDataSyncCtx* const sc, const rgw_pool& pool, const uint32_t shard_id,
                          rgw_data_sync_marker& sync_marker, RGWSyncTraceNodeRef tn, const string& status_oid,
                          const rgw_raw_obj& error_repo, boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
                          const rgw_data_sync_status& sync_status, RGWObjVersionTracker& objv,
                          const boost::intrusive_ptr<rgw::bucket_sync::Cache>& bucket_shard_cache,
                          ceph::mutex& inc_lock, bc::flat_set<rgw_data_notify_entry>& modified_shards)
        : RGWDataBaseSyncShardCR(sc, pool, shard_id, sync_marker, tn, status_oid, error_repo, std::move(lease_cr),
                                 sync_status, objv, bucket_shard_cache)
        , inc_lock(inc_lock)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            do {
                ...
                for (modified_iter = current_modified.begin(); modified_iter != current_modified.end();
                     ++modified_iter) {
                    if (!lease_cr->is_locked()) {
                        drain_all();
                    ...
                    spawn(data_sync_single_entry(sc,
                                                 source_bs,
                                                 modified_iter->gen,
                                                 {},
                                                 ceph::real_time{},
                ...
                if (error_retry_time <= ceph::coarse_real_clock::now()) {
                    ...
                    omapvals = std::make_shared<RGWRadosGetOmapValsCR::Result>();
                    yield call(new RGWRadosGetOmapValsCR(
                        sc->env->driver, error_repo, error_marker, max_error_entries, omapvals));
                    ...
                    for (; iter != error_entries.end(); ++iter) {
                        if (!lease_cr->is_locked()) {
                            ...
                            if (retcode < 0) {
                                tn->log(0,
                                        SSTR("ERROR: data sync marker_tracker.flush() returned retcode=" << retcode));
                                return set_cr_error(retcode);
                            }
                            return set_cr_error(-ECANCELED);
                        }
                        ...
                        if (retcode < 0) {
                            ...
                            spawn(rgw::error_repo::remove_cr(
                                      sc->env->driver->svc()->rados, error_repo, error_marker, entry_timestamp),
                                  false);
                            continue;
                        }
                        ...
                        if (!gen) {
                            ...
                            spawn(new RGWDataIncrementalSyncFullObligationCR(
                                      sc, source_bs, error_repo, error_marker, entry_timestamp, tn),
                                  false);
                        }
                        else {
                            tn->log(20,
                                    SSTR("handle error entry key=" << to_string(source_bs, gen)
                ...
                yield call(new RGWReadRemoteDataLogShardCR(
                    sc, shard_id, sync_marker.marker, &next_marker, &log_entries, &truncated));
                ...
                for (log_iter = log_entries.begin(); log_iter != log_entries.end(); ++log_iter) {
                    ...
                    tn->log(20,
                            SSTR("shard_id=" << shard_id << " log_entry: " << log_iter->log_id << ":"
                                             << log_iter->log_timestamp << ":" << log_iter->entry.key));
                    ...
                    if (!marker_tracker->start(log_iter->log_id, 0, log_iter->log_timestamp)) {
                        ...
                        tn->log(1,
                                SSTR("incremental sync on " << log_iter->entry.key << "shard: " << shard_id << "on gen "
                                                            << log_iter->entry.gen));
                        yield_spawn_window(data_sync_single_entry(sc,
                                                                  source_bs,
                                                                  log_iter->entry.gen,
                                                                  log_iter->log_id,
                ...
                tn->log(20,
                        SSTR("shard_id=" << shard_id << " sync_marker=" << sync_marker.marker
...
class RGWDataSyncShardCR : public RGWCoroutine
{
    ...
    const string status_oid{RGWDataSyncStatusManager::shard_obj_name(sc->source_zone, shard_id)};
...
public:
    RGWDataSyncShardCR(RGWDataSyncCtx* const _sc, const rgw_pool& pool, const uint32_t shard_id,
                       rgw_data_sync_marker& marker, const rgw_data_sync_status& sync_status, RGWSyncTraceNodeRef& tn,
                       RGWObjVersionTracker& objv, bool* reset_backoff)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , pool(pool)
        , shard_id(shard_id)
        ...
        reenter(this)
        {
            ...
            *reset_backoff = true;
            ...
            yield call(new RGWSimpleRadosReadCR<rgw_data_sync_marker>(
                sync_env->dpp, sync_env->driver, rgw_raw_obj(pool, status_oid), &sync_marker, true, &objv));
            if (retcode < 0) {
                lease_cr->go_down();
                drain_all();
                return set_cr_error(retcode);
            ...
            while (true) {
                if (sync_marker.state == rgw_data_sync_marker::FullSync) {
                    yield call(new RGWDataFullSyncShardCR(sc,
                                                          pool,
                                                          shard_id,
                                                          sync_marker,
                                                          tn,
                    ...
                    yield call(new RGWDataIncSyncShardCR(sc,
                                                         pool,
                                                         shard_id,
                                                         sync_marker,
                                                         tn,
    ...
    void init_lease_cr()
    {
        ...
        lease_cr.reset(new RGWContinuousLeaseCR(
            sync_env->async_rados, driver, rgw_raw_obj(pool, status_oid), lock_name, lock_duration, this, &sc->lcc));
...
public:
    RGWDataSyncShardControlCR(RGWDataSyncCtx* _sc, const rgw_pool& _pool, uint32_t _shard_id,
                              rgw_data_sync_marker& _marker, const rgw_data_sync_status& sync_status,
                              RGWObjVersionTracker& objv, RGWSyncTraceNodeRef& _tn_parent)
...
class RGWDataSyncCR : public RGWCoroutine
{
    ...
    ceph::mutex shard_crs_lock = ceph::make_mutex("RGWDataSyncCR::shard_crs_lock");
...
public:
    RGWDataSyncCR(RGWDataSyncCtx* _sc, uint32_t _num_shards, RGWSyncTraceNodeRef& _tn, bool* _reset_backoff)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , num_shards(_num_shards)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            if ((rgw_data_sync_info::SyncState)sync_status.sync_info.state != rgw_data_sync_info::StateSync) {
                init_lease.reset(RGWInitDataSyncStatusCoroutine::continuous_lease_cr(sc, this));
            ...
            if ((rgw_data_sync_info::SyncState)sync_status.sync_info.state == rgw_data_sync_info::StateInit) {
                ...
                yield call(new RGWInitDataSyncStatusCoroutine(
                    sc, num_shards, instance_id, tn, &sync_status, init_lease, obj_version, objvs));
                if (retcode < 0) {
...
public:
    ...
    RGWCoroutine* sync_object(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc, rgw_bucket_sync_pipe& sync_pipe,
                              rgw_obj_key& key, std::optional<uint64_t> versioned_epoch,
                              const rgw_zone_set_entry& source_trace_entry, rgw_zone_set* zones_trace) override;
    RGWCoroutine* remove_object(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc, rgw_bucket_sync_pipe& sync_pipe,
                                rgw_obj_key& key, real_time& mtime, bool versioned, uint64_t versioned_epoch,
                                rgw_zone_set* zones_trace) override;
    RGWCoroutine* create_delete_marker(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc,
...
int RGWDefaultSyncModule::create_instance(const DoutPrefixProvider* dpp, CephContext* cct,
                                          const JSONFormattable& config, RGWSyncModuleInstanceRef* instance)
{
...
public:
    ...
    RGWCoroutine* init_cr()
    {
        ...
        return new RGWGenericAsyncCR(sync_env->cct, sync_env->async_rados, init_action);
    }
    ...
    public:
        ...
        bool verify_object_permission(const map<string, bufferlist>& obj_attrs, int perm);
    };

    static int policy_from_attrs(CephContext* cct, const map<string, bufferlist>& attrs, RGWAccessControlPolicy* acl)
    {
        acl->set_ctx(cct);
    ...
    int init_bucket(const RGWBucketInfo& bucket_info, const map<string, bufferlist>& bucket_attrs, Bucket* bs)
    {
        return bs->init(this, bucket_info, bucket_attrs);
    }
};

int RGWUserPermHandler::Bucket::init(RGWUserPermHandler* handler, const RGWBucketInfo& bucket_info,
                                     const map<string, bufferlist>& bucket_attrs)
{
    sync_env = handler->sync_env;
    info = handler->info;
...
{
    return verify_bucket_permission_no_policy(sync_env->dpp, &(*ps), &info->user_acl, &bucket_acl, perm);
}

bool RGWUserPermHandler::Bucket::verify_object_permission(const map<string, bufferlist>& obj_attrs, int perm)
{
    ...
    return verify_bucket_permission_no_policy(sync_env->dpp, &(*ps), &bucket_acl, &obj_acl, perm);
}
...
public:
    RGWFetchObjFilter_Sync(rgw_bucket_sync_pipe& _sync_pipe, std::shared_ptr<RGWUserPermHandler::Bucket>& _bucket_perms,
    ...
    int filter(CephContext* cct, const rgw_obj_key& source_key, const RGWBucketInfo& dest_bucket_info,
               std::optional<rgw_placement_rule> dest_placement_rule, const map<string, bufferlist>& obj_attrs,
               std::optional<rgw_user>* poverride_owner, const rgw_placement_rule** prule) override;
};

int RGWFetchObjFilter_Sync::filter(CephContext* cct, const rgw_obj_key& source_key,
                                   const RGWBucketInfo& dest_bucket_info,
                                   std::optional<rgw_placement_rule> dest_placement_rule,
                                   const map<string, bufferlist>& obj_attrs, std::optional<rgw_user>* poverride_owner,
                                   const rgw_placement_rule** prule)
{
    ...
    if (!sync_pipe.info.handler.find_obj_params(source_key, obj_tags.get_tags(), &params)) {
        return abort_err;
    }
    ...
    if (verify_dest_params && !(*verify_dest_params == params.dest)) {
        ...
        ldout(cct, 0)
            << "WARNING: " << __func__
            << ": pipe dest params are different than original params, must have raced with object rewrite, retrying"
            << dendl;
        *need_retry = true;
        return -ECANCELED;
    }
    ...
    if (params.dest.acl_translation) {
        ...
        if (!acl_translation_owner.empty()) {
            if (params.mode == rgw_sync_pipe_params::MODE_USER && acl_translation_owner != dest_bucket_info.owner) {
                ldout(cct, 0) << "ERROR: " << __func__ << ": acl translation was requested, but user ("
                              << acl_translation_owner << ") is not dest bucket owner (" << dest_bucket_info.owner
                              << ")" << dendl;
                return -EPERM;
            }
    ...
    if (params.mode == rgw_sync_pipe_params::MODE_USER) {
        if (!bucket_perms->verify_object_permission(obj_attrs, RGW_PERM_READ)) {
            ldout(cct, 0) << "ERROR: " << __func__ << ": permission check failed: user not allowed to fetch object"
                          << dendl;
            return -EPERM;
        }
    }
    ...
    return RGWFetchObjFilter_Default::filter(
        cct, source_key, dest_bucket_info, dest_placement_rule, obj_attrs, poverride_owner, prule);
}
...
public:
    RGWObjFetchCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, rgw_obj_key& _key,
                  std::optional<rgw_obj_key> _dest_key, std::optional<uint64_t> _versioned_epoch,
                  const rgw_zone_set_entry& source_trace_entry, rgw_zone_set* _zones_trace)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , sync_pipe(_sync_pipe)
...
#define MAX_RACE_RETRIES_OBJ_FETCH 10
            for (try_num = 0; try_num < MAX_RACE_RETRIES_OBJ_FETCH; ++try_num) {
                ...
                if (need_more_info) {
                    ldout(cct, 20) << "Could not determine exact policy rule for obj=" << key
                    ...
                    if (iter != src_attrs.end()) {
                        try {
                            ...
                            ldout(cct, 0)
                                << "ERROR: " << __func__ << ": caught buffer::error couldn't decode TagSet " << dendl;
                        }
                    }

                    rgw_sync_pipe_params params;
                    if (!sync_pipe.info.handler.find_obj_params(key, obj_tags.get_tags(), &params)) {
                        return set_cr_error(-ERR_PRECONDITION_FAILED);
                    }
                ...
                if (param_mode == rgw_sync_pipe_params::MODE_USER) {
                    ...
                    if (retcode < 0) {
                        ldout(cct, 20) << "ERROR: " << __func__
                                       << ": failed to init user perms manager for uid=" << *param_user << dendl;
                        return set_cr_error(retcode);
                    }

                    /* verify that user is allowed to write at the target bucket */
                    int r = user_perms->init_bucket(
                        sync_pipe.dest_bucket_info, sync_pipe.dest_bucket_attrs, &dest_bucket_perms);
                    if (r < 0) {
                        ldout(cct, 20) << "ERROR: " << __func__
                                       << ": failed to init bucket perms manager for uid=" << *param_user
                                       << " bucket=" << sync_pipe.source_bucket_info.bucket.get_key() << dendl;
                        return set_cr_error(retcode);
                    }

                    if (!dest_bucket_perms.verify_bucket_permission(RGW_PERM_WRITE)) {
                        ldout(cct, 0) << "ERROR: " << __func__
                                      << ": permission check failed: user not allowed to write into bucket (bucket="
                                      << sync_pipe.info.dest_bucket.get_key() << ")" << dendl;
                        return -EPERM;
                    }

                    /* init source bucket permission structure */
                    source_bucket_perms = make_shared<RGWUserPermHandler::Bucket>();
                    r = user_perms->init_bucket(
                        sync_pipe.source_bucket_info, sync_pipe.source_bucket_attrs, source_bucket_perms.get());
                    if (r < 0) {
                        ldout(cct, 20) << "ERROR: " << __func__
                                       << ": failed to init bucket perms manager for uid=" << *param_user
                                       << " bucket=" << sync_pipe.source_bucket_info.bucket.get_key() << dendl;
                ...
                yield
                {
                    ...
                    auto filter = make_shared<RGWFetchObjFilter_Sync>(
                        sync_pipe, source_bucket_perms, std::move(dest_params), need_retry);

                    call(new RGWFetchRemoteObjCR(sync_env->async_rados,
                                                 sync_env->driver,
                                                 sc->source_zone,
                                                 nullopt,
                                                 sync_pipe.info.source_bs.bucket,
            ...
            ldout(cct, 0) << "ERROR: " << __func__
                          << ": Too many retries trying to fetch object, possibly a bug: bucket="
                          << sync_pipe.source_bucket_info.bucket.get_key() << " key=" << key << dendl;
...
RGWCoroutine* RGWDefaultDataSyncModule::sync_object(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc,
                                                    ...
                                                    rgw_zone_set* zones_trace)
{
    return new RGWObjFetchCR(sc, sync_pipe, key, std::nullopt, versioned_epoch, source_trace_entry, zones_trace);
}

RGWCoroutine* RGWDefaultDataSyncModule::remove_object(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc,
                                                      rgw_bucket_sync_pipe& sync_pipe, rgw_obj_key& key,
                                                      real_time& mtime, bool versioned, uint64_t versioned_epoch,
                                                      rgw_zone_set* zones_trace)
{
    auto sync_env = sc->env;
    return new RGWRemoveObjCR(sync_env->dpp,
...
RGWCoroutine* RGWDefaultDataSyncModule::create_delete_marker(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc,
                                                             rgw_bucket_sync_pipe& sync_pipe, rgw_obj_key& key,
                                                             real_time& mtime, rgw_bucket_entry_owner& owner,
...
public:
    ...
    RGWDataSyncModule* get_data_handler() override { return &data_handler; }
    RGWMetadataHandler* alloc_bucket_meta_handler() override { return RGWArchiveBucketMetaHandlerAllocator::alloc(); }
...
int RGWArchiveSyncModule::create_instance(const DoutPrefixProvider* dpp, CephContext* cct,
                                          const JSONFormattable& config, RGWSyncModuleInstanceRef* instance)
{
...
{
    auto sync_env = sc->env;
    ldout(sc->cct, 5) << "SYNC_ARCHIVE: sync_object: b=" << sync_pipe.info.source_bs.bucket << " k=" << key
                      << " versioned_epoch=" << versioned_epoch.value_or(0) << dendl;
    if (!sync_pipe.dest_bucket_info.versioned() || (sync_pipe.dest_bucket_info.flags & BUCKET_VERSIONS_SUSPENDED)) {
        ldout(sc->cct, 0) << "SYNC_ARCHIVE: sync_object: enabling object versioning for archive bucket" << dendl;
        sync_pipe.dest_bucket_info.flags =
            (sync_pipe.dest_bucket_info.flags & ~BUCKET_VERSIONS_SUSPENDED) | BUCKET_VERSIONED;
        int op_ret = sync_env->driver->getRados()->put_bucket_instance_info(
            sync_pipe.dest_bucket_info, false, real_time(), NULL, sync_env->dpp, null_yield);
        if (op_ret < 0) {
    ...
    return new RGWObjFetchCR(sc, sync_pipe, key, dest_key, versioned_epoch, source_trace_entry, zones_trace);
}

RGWCoroutine* RGWArchiveDataSyncModule::remove_object(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc,
                                                      rgw_bucket_sync_pipe& sync_pipe, rgw_obj_key& key,
                                                      real_time& mtime, bool versioned, uint64_t versioned_epoch,
                                                      rgw_zone_set* zones_trace)
{
    ldout(sc->cct, 0) << "SYNC_ARCHIVE: remove_object: b=" << sync_pipe.info.source_bs.bucket << " k=" << key
                      << " versioned_epoch=" << versioned_epoch << dendl;
    return NULL;
}

RGWCoroutine* RGWArchiveDataSyncModule::create_delete_marker(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc,
                                                             rgw_bucket_sync_pipe& sync_pipe, rgw_obj_key& key,
                                                             real_time& mtime, rgw_bucket_entry_owner& owner,
                                                             bool versioned, uint64_t versioned_epoch,
                                                             rgw_zone_set* zones_trace)
{
    ldout(sc->cct, 0) << "SYNC_ARCHIVE: create_delete_marker: b=" << sync_pipe.info.source_bs.bucket << " k=" << key
                      << " mtime=" << mtime << " versioned=" << versioned << " versioned_epoch=" << versioned_epoch
                      << dendl;
    auto sync_env = sc->env;
    return new RGWRemoveObjCR(sync_env->dpp,
                              sync_env->async_rados,
...
public:
    RGWDataSyncControlCR(RGWDataSyncCtx* _sc, uint32_t _num_shards, RGWSyncTraceNodeRef& _tn_parent)
        : RGWBackoffControlCR(_sc->cct, exit_on_error)
        , sc(_sc)
        , sync_env(_sc->env)
        , num_shards(_num_shards)
    {
    ...
    RGWCoroutine* alloc_cr() override { return new RGWDataSyncCR(sc, num_shards, tn, backoff_ptr()); }
...
int RGWDataSyncStatusManager::init(const DoutPrefixProvider* dpp)
{
    ...
    int r =
        source_log.init(source_zone, conn, error_logger, driver->getRados()->get_sync_tracer(), sync_module, counters);
    if (r < 0) {
...
public:
    RGWInitBucketShardSyncStatusCoroutine(RGWDataSyncCtx* _sc, const rgw_bucket_sync_pair_info& _sync_pair,
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            yield
            {
                ...
                call(new RGWSimpleRadosWriteAttrsCR(dpp, sync_env->driver, obj, attrs, &objv_tracker, exclusive));
            }
...
public:
    RGWReadBucketPipeSyncStatusCoroutine(RGWDataSyncCtx* _sc, const rgw_bucket_sync_pair_info& sync_pair,
...
int RGWReadBucketPipeSyncStatusCoroutine::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        yield call(new RGWSimpleRadosReadAttrsCR(dpp,
                                                 sync_env->driver,
        ...
        if (retcode < 0) {
            ldpp_dout(dpp, 0) << "ERROR: failed to call fetch bucket shard info oid=" << oid << " ret=" << retcode
                              << dendl;
            return set_cr_error(retcode);
        }
...
public:
    CheckBucketShardStatusIsIncremental(RGWDataSyncCtx* sc, const rgw_bucket_sync_pair_info& sync_pair, bool* result)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        ...
        if (state == RGWCoroutine_Done && status.state != rgw_bucket_shard_sync_info::StateIncrementalSync) {
            *result = false;
        }
        return r;
    }
...
public:
    CheckAllBucketShardStatusIsIncremental(RGWDataSyncCtx* sc, const rgw_bucket_sync_pair_info& sync_pair,
...
private:
    ...
    {
        if (r < 0) {
            ldout(cct, 4) << "failed to read bucket shard status: " << cpp_strerror(r) << dendl;
        }
        else if (shard == 0) {
...
public:
    InitBucketShardStatusCR(RGWDataSyncCtx* sc, const rgw_bucket_sync_pair_info& pair, uint64_t gen,
                            const BucketIndexShardsManager& marker_mgr)
        : RGWCoroutine(sc->cct)
        , sc(sc)
        , pair(pair)
...
class InitBucketShardStatusCollectCR : public RGWShardCollectCR
{
    ...
    {
        if (r < 0) {
            ldout(cct, 4) << "failed to init bucket shard status: " << cpp_strerror(r) << dendl;
        }
        return r;
    }

public:
    InitBucketShardStatusCollectCR(RGWDataSyncCtx* sc, const rgw_bucket_sync_pair_info& sync_pair, uint64_t gen,
                                   const BucketIndexShardsManager& marker_mgr, int num_shards)
        : RGWShardCollectCR(sc->cct, max_concurrent_shards)
        , sc(sc)
        , sync_pair(sync_pair)
        , gen(gen)
...
public:
    RemoveBucketShardStatusCR(RGWDataSyncCtx* sc, const rgw_bucket_sync_pair_info& sync_pair, uint64_t gen)
        : RGWCoroutine(sc->cct)
        , sc(sc)
        , sync_env(sc->env)
        , sync_pair(sync_pair)
...
class RemoveBucketShardStatusCollectCR : public RGWShardCollectCR
{
    ...
    {
        if (r < 0) {
            ldout(cct, 4) << "failed to remove bucket shard status object: " << cpp_strerror(r) << dendl;
        }
        return r;
    }

public:
    RemoveBucketShardStatusCollectCR(RGWDataSyncCtx* sc, const rgw_bucket_sync_pair_info& sync_pair, uint64_t gen,
                                     int num_shards)
        : RGWShardCollectCR(sc->cct, max_concurrent_shards)
        , sc(sc)
        , sync_env(sc->env)
...
public:
    InitBucketFullSyncStatusCR(RGWDataSyncCtx* sc, const rgw_bucket_sync_pair_info& sync_pair,
                               const rgw_raw_obj& status_obj, rgw_bucket_sync_status& status,
                               RGWObjVersionTracker& objv, const RGWBucketInfo& source_info, bool check_compat,
                               const rgw_bucket_index_marker_info& info)
        : RGWCoroutine(sc->cct)
        , sc(sc)
        , sync_env(sc->env)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            if (retcode < 0) {
                lderr(cct) << "failed to parse bilog shard markers: " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }
            ...
            if (info.oldest_gen == 0) {
                if (check_compat) {
                    ...
                    if (source_info.layout.logs.empty() || source_info.layout.logs.front().gen > 0) {
                        ...
                        ldpp_dout(dpp, 20)
                            << "unrecognized log layout type when checking compatibility " << log.layout.type << dendl;
                        no_zero = true;
                    }
                    if (!no_zero) {
                        yield
                        {
                            const int num_shards0 =
                                rgw::num_shards(source_info.layout.logs.front().layout.in_index.layout);
            ...
            if (status.state != BucketSyncState::Incremental) {
                ...
                if (retcode < 0) {
                    ldout(cct, 20) << "failed to init bucket shard status: " << cpp_strerror(retcode) << dendl;
                    return set_cr_error(retcode);
                }
            ...
            ldout(cct, 20) << "writing bucket sync status during init. state=" << status.state
                           << ". marker=" << status.full.position << dendl;

            // write bucket sync status
            using CR = RGWSimpleRadosWriteCR<rgw_bucket_sync_status>;
            yield call(new CR(dpp, sync_env->driver, status_obj, status, &objv, false));
            if (retcode < 0) {
                ldout(cct, 20) << "failed to write bucket shard status: " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }
            return set_cr_done();
        }
...
public:
    RGWReadRecoveringBucketShardsCoroutine(RGWDataSyncCtx* _sc, const int _shard_id, set<string>& _recovering_buckets,
        ...
        , shard_id(_shard_id)
        , max_entries(_max_entries)
        , recovering_buckets(_recovering_buckets)
...
int RGWReadRecoveringBucketShardsCoroutine::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        do {
            ...
            yield call(
                new RGWRadosGetOmapKeysCR(driver,
                                          rgw_raw_obj(sync_env->svc->zone->get_zone_params().log_pool, error_oid),
                                          marker,
                                          max_omap_entries,
                                          omapkeys));
            ...
            if (retcode < 0) {
                ldpp_dout(dpp, 0) << "failed to read recovering bucket shards with " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }
...
public:
    RGWReadPendingBucketShardsCoroutine(RGWDataSyncCtx* _sc, const int _shard_id, set<string>& _pending_buckets,
        ...
        , shard_id(_shard_id)
        , max_entries(_max_entries)
        , pending_buckets(_pending_buckets)
...
int RGWReadPendingBucketShardsCoroutine::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        using CR = RGWSimpleRadosReadCR<rgw_data_sync_marker>;
        yield call(new CR(dpp,
                          sync_env->driver,
                          rgw_raw_obj(sync_env->svc->zone->get_zone_params().log_pool, status_oid),
                          sync_marker));
        if (retcode < 0) {
            ldpp_dout(dpp, 0) << "failed to read sync status marker with " << cpp_strerror(retcode) << dendl;
            return set_cr_error(retcode);
        }
        ...
        count = 0;
        do {
            yield call(new RGWReadRemoteDataLogShardCR(sc, shard_id, marker, &next_marker, &log_entries, &truncated));
            ...
            if (retcode < 0) {
                ldpp_dout(dpp, 0) << "failed to read remote data log info with " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }
...
int RGWRemoteDataLog::read_shard_status(const DoutPrefixProvider* dpp, int shard_id, set<string>& pending_buckets,
                                        set<string>& recovering_buckets, rgw_data_sync_marker* sync_marker,
                                        const int max_entries)
{
    ...
    recovering_stack->call(
        new RGWReadRecoveringBucketShardsCoroutine(&sc_local, shard_id, recovering_buckets, max_entries));
    stacks.push_back(recovering_stack);
    RGWCoroutinesStack* pending_stack = new RGWCoroutinesStack(driver->ctx(), &crs);
    pending_stack->call(
        new RGWReadPendingBucketShardsCoroutine(&sc_local, shard_id, pending_buckets, sync_marker, max_entries));
...
public:
    RGWListRemoteBucketCR(RGWDataSyncCtx* _sc, const rgw_bucket_shard& bs, rgw_obj_key& _marker_position,
...
public:
    RGWListBucketIndexLogCR(RGWDataSyncCtx* _sc, const rgw_bucket_shard& bs, string& _marker, uint64_t _generation,
                            bilog_list_result* _result)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , instance_key(bs.get_key())
        , marker(_marker)
        , result(_result)
        ...
        reenter(this)
        {
            ...
            yield
            {
                ...
                call(new RGWReadRESTResourceCR<bilog_list_result>(
                    sync_env->cct, sc->conn, sync_env->http_manager, "/admin/log", pairs, result));
            }
            timer.reset();
            if (retcode < 0) {
                if (sync_env->counters) {
...
public:
    RGWBucketFullSyncMarkerTrack(RGWDataSyncCtx* _sc, const rgw_raw_obj& status_obj,
                                 rgw_bucket_sync_status& sync_status, RGWSyncTraceNodeRef tn,
                                 RGWObjVersionTracker& objv_tracker)
    ...
    RGWCoroutine* store_marker(const rgw_obj_key& new_marker, uint64_t index_pos, const real_time& timestamp) override
    {
        ...
        return new RGWSimpleRadosWriteCR<rgw_bucket_sync_status>(
            sync_env->dpp, sync_env->driver, status_obj, sync_status, &objv_tracker);
    }
...
public:
    ...
    int operate(const DoutPrefixProvider* dpp)
    {
        reenter(this)
        {
            ...
            yield call(new RGWSimpleRadosWriteAttrsCR(sync_env->dpp, sync_env->driver, obj, attrs, &objv_tracker));
            if (retcode < 0) {
                return set_cr_error(retcode);
            }
            if (stable_timestamp) {
...
public:
    RGWBucketIncSyncShardMarkerTrack(RGWDataSyncCtx* _sc, const string& _marker_oid,
    ...
    RGWCoroutine* store_marker(const string& new_marker, uint64_t index_pos, const real_time& timestamp) override
    {
        ...
        tn->log(
            20,
            SSTR("updating marker marker_oid=" << obj.oid << " marker=" << new_marker << " timestamp=" << timestamp));
        return new RGWWriteBucketShardIncSyncStatus(sync_env, obj, sync_marker, stable_timestamp, objv_tracker);
    }
...
template<class T, class K> class RGWBucketSyncSingleEntryCR : public RGWCoroutine
{
    RGWDataSyncCtx* sc;
    RGWDataSyncEnv* sync_env;
...
public:
    RGWBucketSyncSingleEntryCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, const rgw_obj_key& _key,
                               bool _versioned, std::optional<uint64_t> _versioned_epoch, real_time& _timestamp,
                               const rgw_bucket_entry_owner& _owner, RGWModifyOp _op, RGWPendingState _op_state,
        ...
        , sync_status(0)
    {
        ...
        set_description() << "bucket sync single entry (source_zone=" << sc->source_zone << ") b=" << ss.str()
                          << " log_entry=" << entry_marker << " op=" << (int)op << " op_state=" << (int)op_state;
        set_status("init");

        tn = sync_env->sync_tracer->add_node(_tn_parent, "entry", SSTR(key));

        tn->log(20,
                SSTR("bucket sync single entry (source_zone=" << sc->source_zone << ") b=" << ss.str()
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            do {
                yield
                {
                    ...
                    if (error_injection &&
                        ...
                        tn->log(5,
                                SSTR("bucket sync: sync obj: " << sc->source_zone << "/" << bs.bucket << "/" << key
                        ...
                        call(data_sync_module->sync_object(
                            dpp, sc, sync_pipe, key, versioned_epoch, source_trace_entry, &zones_trace));
                    }
                        ...
                        tn->log(10,
                                SSTR("removing obj: " << sc->source_zone << "/" << bs.bucket << "/" << key << "["
                                                      << versioned_epoch.value_or(0) << "]"));
                        call(data_sync_module->remove_object(
                            dpp, sc, sync_pipe, key, timestamp, versioned, versioned_epoch.value_or(0), &zones_trace));
                        ...
                        tn->log(
                            10,
                            SSTR("creating delete marker: obj: " << sc->source_zone << "/" << bs.bucket << "/" << key
                                                                 << "[" << versioned_epoch.value_or(0) << "]"));
                        call(data_sync_module->create_delete_marker(dpp,
                                                                    sc,
                                                                    sync_pipe,
                                                                    key,
                                                                    timestamp,
                ...
                if (retcode == -ERR_PRECONDITION_FAILED) {
                    pretty_print(
                        sc->env, "Skipping object s3://{}/{} in sync from zone {}\n", bs.bucket.name, key, zone_name);
                    set_status("Skipping object sync: precondition failed (object contains newer change or policy "
                               "doesn't allow sync)");
                    tn->log(0,
                            "Skipping object sync: precondition failed (object contains newer change or policy doesn't "
                            "allow sync)");
                    retcode = 0;
                }
            ...
            if (retcode < 0 && retcode != -ENOENT) {
                ...
                tn->log(0, SSTR("ERROR: failed to sync object: " << bucket_shard_str{bs} << "/" << key.name));
            ...
            if (!error_ss.str().empty()) {
                yield call(
                    sync_env->error_logger->log_error_cr(dpp,
                                                         sc->conn->get_remote_id(),
                                                         "data",
                                                         error_ss.str(),
                                                         -retcode,
...
class RGWBucketFullSyncCR : public RGWCoroutine
{
    ...
    struct _prefix_handler
    {
        ...
        void set_rules(RGWBucketSyncFlowManager::pipe_rules_ref& _rules) { rules = _rules; }
...
public:
    RGWBucketFullSyncCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, const rgw_raw_obj& status_obj,
                        boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr, rgw_bucket_sync_status& sync_status,
                        RGWSyncTraceNodeRef tn_parent, RGWObjVersionTracker& objv_tracker)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , sync_pipe(_sync_pipe)
        ...
        , status_obj(status_obj)
        , objv(objv_tracker)
        , tn(sync_env->sync_tracer->add_node(tn_parent, "full_sync", SSTR(bucket_shard_str{bs})))
    ...
    reenter(this)
    {
        ...
        do {
            ...
            for (; entries_iter != list_result.entries.end(); ++entries_iter) {
                ...
                tn->log(20, SSTR("[full sync] syncing object: " << bucket_shard_str{bs} << "/" << entries_iter->key));
                entry = &(*entries_iter);
        ...
        if (retcode < 0 && sync_result == 0) { /* actually tried to set incremental state and failed */
            tn->log(
                0,
                SSTR("ERROR: failed to set sync state on bucket " << bucket_shard_str{bs} << " retcode=" << retcode));
            return set_cr_error(retcode);
        }
        if (sync_result < 0) {
...
public:
    RGWBucketShardIsDoneCR(RGWDataSyncCtx* _sc, const rgw_raw_obj& _bucket_status_obj, int _shard_id,
                           const next_bilog_result& _next_log, const uint64_t _gen)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            do {
                ...
                yield call(new ReadCR(dpp, sync_env->driver, bucket_status_obj, &bucket_status, false, &objv_tracker));
                if (retcode < 0) {
                    ldpp_dout(dpp, 20) << "failed to read bucket shard status: " << cpp_strerror(retcode) << dendl;
                    return set_cr_error(retcode);
                }
                ...
                yield
                {
                    ...
                    if (std::all_of(done.begin(), done.end(), [](const bool done) { return done; })) {
                    ...
                    call(new WriteCR(dpp, sync_env->driver, bucket_status_obj, bucket_status, &objv_tracker, false));
                }
...
public:
    RGWBucketShardIncrementalSyncCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe,
                                    const std::string& shard_status_oid, const rgw_raw_obj& _bucket_status_obj,
                                    boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
        ...
        , tn(sync_env->sync_tracer->add_node(_tn_parent, "inc_sync", SSTR(bucket_shard_str{bs})))
        , marker_tracker(sc, shard_status_oid, sync_info.inc_marker, tn, objv_tracker, stable_timestamp)
    {
        set_description() << "bucket shard incremental sync bucket=" << bucket_shard_str{bs};
        set_status("init");
...
int RGWBucketShardIncrementalSyncCR::operate(const DoutPrefixProvider* dpp)
{
    ...
    reenter(this)
    {
        do {
            ...
            yield call(
                new RGWListBucketIndexLogCR(sc, bs, sync_info.inc_marker.position, generation, &extended_result));
            if (retcode < 0 && retcode != -ENOENT) {
                /* wait for all operations to complete */
                drain_all();
                yield spawn(marker_tracker.flush(), true);
            ...
            for (; entries_iter != entries_end; ++entries_iter) {
                ...
                entry = &(*entries_iter);
                {
                    ssize_t p = entry->id.find(
                        '#'); /* entries might have explicit shard info in them, e.g., 6#00000000004.94.3 */
                    if (p < 0) {
                        cur_id = entry->id;
                    }
                    else {
                ...
                tn->log(20,
                        SSTR("parsed entry: id=" << cur_id << " iter->object=" << entry->object
                                                 << " iter->instance=" << entry->instance << " name=" << key.name
                                                 << " instance=" << key.instance << " ns=" << key.ns));
                ...
                if (entry->op == CLS_RGW_OP_CANCEL) {
                    ...
                    tn->log(20,
                            SSTR("skipping object: " << bucket_shard_str{bs} << "/" << key << ": canceled operation"));
                    marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
                    continue;
                }
                if (entry->state != CLS_RGW_STATE_COMPLETE) {
                    ...
                    tn->log(
                        20,
                        SSTR("skipping object: " << bucket_shard_str{bs} << "/" << key << ": non-complete operation"));
                    marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
                    continue;
                }
                if (entry->zones_trace.exists(zone_id.id, target_location_key)) {
                    ...
                    tn->log(20,
                            SSTR("skipping object: " << bucket_shard_str{bs} << "/" << key << ": redundant operation"));
                    marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
                    continue;
                }
                if (make_pair<>(entry->timestamp, entry->op) != squash_map[make_pair(entry->object, entry->instance)]) {
                    ...
                    tn->log(20,
                            SSTR("skipping object: " << bucket_shard_str{bs} << "/" << key << ": squashed operation"));
                    marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
                    continue;
                }
                tn->set_flag(RGW_SNS_FLAG_ACTIVE);
                tn->log(20, SSTR("syncing object: " << bucket_shard_str{bs} << "/" << key));
                ...
                while (!marker_tracker.can_do_op(key, has_olh_epoch(entry->op))) {
                    ...
                    tn->log(
                        5, SSTR("can't do op on key=" << key << " need to wait for conflicting operation to complete"));
                ...
                if (!marker_tracker.index_key_to_marker(key, cur_id, has_olh_epoch(entry->op))) {
                    ...
                    tn->log(20,
                            SSTR("skipping sync of entry: " << cur_id << ":" << key
        ...
        if (!truncated && extended_result.next_log) {
            yield call(
                new RGWBucketShardIsDoneCR(sc, bucket_status_obj, bs.shard_id, *extended_result.next_log, generation));
            if (retcode < 0) {
                ldout(cct, 20) << "failed to update bucket sync status: " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }
            yield
            {
...
class RGWGetBucketPeersCR : public RGWCoroutine
{
    ...
    static pair<pipe_const_iter, pipe_const_iter> get_pipe_iters(
        const map<rgw_zone_id, RGWBucketSyncFlowManager::pipe_set>& m, std::optional<rgw_zone_id> zone)
    {
        if (!zone) {
    ...
    void filter_sources(std::optional<rgw_zone_id> source_zone, std::optional<rgw_bucket> source_bucket,
    ...
    void filter_targets(std::optional<rgw_zone_id> target_zone, std::optional<rgw_bucket> target_bucket,
                        ...
                        rgw_sync_pipe_info_set* result)
    {
        ...
        for (auto i = iters.first; i != iters.second; ++i) {
            for (auto& handler : i->second) {
                if (target_bucket && handler.dest.bucket && !target_bucket->match(*handler.dest.bucket)) {
                    ldpp_dout(sync_env->dpp, 20) << __func__ << ": pipe_handler=" << handler << ": skipping" << dendl;
                    continue;
                }
                ldpp_dout(sync_env->dpp, 20) << __func__ << ": pipe_handler=" << handler << ": adding" << dendl;
        ...
        int operate() override
        {
            ...
            if (r < 0) {
                ldpp_dout(sync_env->dpp, 0)
                    << "ERROR: " << __func__ << "(): failed to fetch bucket sync hints for bucket=" << source_bucket
                    << dendl;
                return r;
            }
...
public:
    RGWGetBucketPeersCR(RGWDataSyncEnv* _sync_env, std::optional<rgw_bucket> _target_bucket,
                        std::optional<rgw_zone_id> _source_zone, std::optional<rgw_bucket> _source_bucket,
                        rgw_sync_pipe_info_set* _pipes, const RGWSyncTraceNodeRef& _tn_parent)
        ...
        , tn(sync_env->sync_tracer->add_node(_tn_parent, "get_bucket_peers",
                                             SSTR("target=" << target_bucket.value_or(rgw_bucket()) << ":source="
                                                            << target_bucket.value_or(rgw_bucket()) << ":source_zone="
...
static RGWCoroutine* sync_bucket_shard_cr(RGWDataSyncCtx* sc, boost::intrusive_ptr<const RGWContinuousLeaseCR> lease,
...
RGWRunBucketSourcesSyncCR::RGWRunBucketSourcesSyncCR(RGWDataSyncCtx* _sc,
                                                     ...
                                                     const RGWSyncTraceNodeRef& _tn_parent, std::optional<uint64_t> gen,
                                                     ceph::real_time* progress)
    : RGWCoroutine(_sc->env->cct)
    , sc(_sc)
    , sync_env(_sc->env)
    , lease_cr(std::move(lease_cr))
...
int RGWRunBucketSourcesSyncCR::operate(const DoutPrefixProvider* dpp)
{
    ...
    {
        yield call(
            new RGWGetBucketPeersCR(sync_env, std::nullopt, sc->source_zone, sync_pair.source_bs.bucket, &pipes, tn));
        if (retcode < 0 && retcode != -ENOENT) {
        ...
        for (siter = pipes.begin(); siter != pipes.end(); ++siter, ++cur_shard_progress) {
            ...
            yield_spawn_window(sync_bucket_shard_cr(sc, lease_cr, sync_pair, gen, tn, &*cur_shard_progress),
...
public:
    RGWSyncGetBucketInfoCR(RGWDataSyncEnv* _sync_env, const rgw_bucket& _bucket, RGWBucketInfo* _pbucket_info,
                           map<string, bufferlist>* _pattrs, const RGWSyncTraceNodeRef& _tn_parent)
        : RGWCoroutine(_sync_env->cct)
        , sync_env(_sync_env)
        , bucket(_bucket)
        , pbucket_info(_pbucket_info)
        , pattrs(_pattrs)
        , tn(sync_env->sync_tracer->add_node(_tn_parent, "get_bucket_info", SSTR(bucket)))
    {}
...
int RGWSyncGetBucketInfoCR::operate(const DoutPrefixProvider* dpp)
{
    ...
    {
        yield call(
            new RGWGetBucketInstanceInfoCR(sync_env->async_rados, sync_env->driver, bucket, pbucket_info, pattrs, dpp));
        if (retcode == -ENOENT) {
            /* bucket instance info has not been synced in yet, fetch it now */
            yield
            {
                tn->log(10,
                        SSTR("no local info for bucket:"
                             << ": fetching metadata"));
                string raw_key = string("bucket.instance:") + bucket.get_key();

                meta_sync_env.init(dpp,
                                   cct,
                                   sync_env->driver,
                                   sync_env->svc->zone->get_master_conn(),
                                   sync_env->async_rados,
                                   sync_env->http_manager,
                                   sync_env->error_logger,
                                   sync_env->sync_tracer);

                call(new RGWMetaSyncSingleEntryCR(&meta_sync_env,
                                                  raw_key,
                                                  string() /* no marker */,
            ...
            yield call(new RGWGetBucketInstanceInfoCR(
                sync_env->async_rados, sync_env->driver, bucket, pbucket_info, pattrs, dpp));
        }
        if (retcode < 0) {
...
void RGWGetBucketPeersCR::update_from_source_bucket_policy()
{
    ...
    filter_targets(sync_env->svc->zone->get_zone().id, target_bucket, handler->get_targets(), pipes);
...
public:
    RGWSyncGetBucketSyncPolicyHandlerCR(RGWDataSyncEnv* _sync_env, std::optional<rgw_zone_id> zone,
        ...
        reenter(this)
        {
            for (i = 0; i < 2; ++i) {
                yield call(new RGWBucketGetSyncPolicyHandlerCR(
                    sync_env->async_rados, sync_env->driver, get_policy_params, policy, dpp));
                if (retcode < 0 && retcode != -ENOENT) {
                    return set_cr_error(retcode);
                }
                 ...
                 */
                yield call(new RGWSyncGetBucketInfoCR(sync_env, bucket, nullptr, nullptr, tn));
                if (retcode < 0) {
                    return set_cr_error(retcode);
                }
            }
...
int RGWGetBucketPeersCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        if (target_bucket) {
            ...
            yield call(new RGWSyncGetBucketSyncPolicyHandlerCR(sync_env, nullopt, *target_bucket, target_policy, tn));
            if (retcode < 0 && retcode != -ENOENT) {
                return set_cr_error(retcode);
            }
        ...
        if (source_bucket && source_zone) {
            ...
            yield call(
                new RGWSyncGetBucketSyncPolicyHandlerCR(sync_env, source_zone, *source_bucket, source_policy, tn));
            if (retcode < 0 && retcode != -ENOENT) {
                return set_cr_error(retcode);
            }
            ...
            if (!target_bucket) {
                ...
                yield call(new RGWGenericAsyncCR(cct, sync_env->async_rados, get_hint_targets_action));
                if (retcode < 0) {
                    return set_cr_error(retcode);
                }
                 ...
                 * bucket_id
                 */
                for (hiter = get_hint_targets_action->targets.begin(); hiter != get_hint_targets_action->targets.end();
                     ++hiter) {
                    ldpp_dout(dpp, 20) << "Got sync hint for bucket=" << *source_bucket << ": " << hiter->get_key()
                                       << dendl;

                    target_policy = make_shared<rgw_bucket_get_sync_policy_result>();
                    yield call(new RGWSyncGetBucketSyncPolicyHandlerCR(sync_env, nullopt, *hiter, target_policy, tn));
                    if (retcode < 0 && retcode != -ENOENT) {
                        return set_cr_error(retcode);
                    }
                    update_from_target_bucket_policy();
        ...
        for (siiter = buckets_info.begin(); siiter != buckets_info.end(); ++siiter) {
            if (siiter->second.bucket_info.bucket.name.empty()) {
                yield call(new RGWSyncGetBucketInfoCR(
                    sync_env, siiter->first, &siiter->second.bucket_info, &siiter->second.attrs, tn));
            }
        }
...
public:
    RGWSyncBucketShardCR(RGWDataSyncCtx* _sc, boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
                         const rgw_bucket_sync_pair_info& _sync_pair, rgw_bucket_sync_pipe& sync_pipe,
                         bool& bucket_stopped, uint64_t generation, const RGWSyncTraceNodeRef& tn,
                         ceph::real_time* progress)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        ...
        , bucket_status_obj(sc->env->svc->zone->get_zone_params().log_pool,
                            RGWBucketPipeSyncStatusManager::full_status_oid(sc->source_zone, sync_pair.source_bs.bucket,
...
int RGWSyncBucketShardCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        yield call(new RGWBucketShardIncrementalSyncCR(sc,
                                                       sync_pipe,
                                                       shard_status_oid,
                                                       bucket_status_obj,
                                                       lease_cr,
...
public:
    RGWSyncBucketCR(RGWDataSyncCtx* _sc, boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
                    const rgw_bucket_sync_pair_info& _sync_pair, std::optional<uint64_t> gen,
                    const RGWSyncTraceNodeRef& _tn_parent, ceph::real_time* progress)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , env(_sc->env)
        , data_lease_cr(std::move(lease_cr))
        , sync_pair(_sync_pair)
        , gen(gen)
        , progress(progress)
...
static RGWCoroutine* sync_bucket_shard_cr(RGWDataSyncCtx* sc, boost::intrusive_ptr<const RGWContinuousLeaseCR> lease,
                                          ...
                                          const RGWSyncTraceNodeRef& tn, ceph::real_time* progress)
{
    return new RGWSyncBucketCR(sc, std::move(lease), sync_pair, gen, tn, progress);
}
...
int RGWSyncBucketCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        yield call(new RGWSyncGetBucketInfoCR(
            env, sync_pair.source_bs.bucket, &sync_pipe.source_bucket_info, &sync_pipe.source_bucket_attrs, tn));
        if (retcode < 0) {
            tn->log(
                0, SSTR("ERROR: failed to retrieve bucket info for bucket=" << bucket_str{sync_pair.source_bs.bucket}));
            return set_cr_error(retcode);
        }

        yield call(new RGWSyncGetBucketInfoCR(
            env, sync_pair.dest_bucket, &sync_pipe.dest_bucket_info, &sync_pipe.dest_bucket_attrs, tn));
        if (retcode < 0) {
            tn->log(
                0, SSTR("ERROR: failed to retrieve bucket info for bucket=" << bucket_str{sync_pair.source_bs.bucket}));
            return set_cr_error(retcode);
        ...
        objv.clear();
        yield call(new ReadCR(dpp, env->driver, status_obj, &bucket_status, false, &objv));
        if (retcode == -ENOENT) {
        ...
        do {
            tn->log(20,
                    SSTR("sync status for source bucket: " << bucket_status.state << ". lease is: "
                                                           << (bucket_lease_cr ? "taken" : "not taken")
                                                           << ". stop indications is: " << bucket_stopped));

            if (bucket_status.state != BucketSyncState::Incremental || bucket_stopped) {

                if (!bucket_lease_cr) {
                    bucket_lease_cr.reset(new RGWContinuousLeaseCR(
                        env->async_rados, env->driver, status_obj, lock_name, lock_duration, this, &sc->lcc));
                ...
                if (info.syncstopped) {
                    ...
                    if (bucket_status.state != BucketSyncState::Stopped) {
                        ...
                        yield call(new WriteCR(dpp, env->driver, status_obj, bucket_status, &objv, false));
                        if (retcode < 0) {
                ...
                if (bucket_stopped) {
                    tn->log(
                        20,
                        SSTR("ERROR: switched from 'stop' to 'start' sync. while state is: " << bucket_status.state));
            ...
            if (bucket_status.state != BucketSyncState::Incremental) {
                ...
                yield call(new InitBucketFullSyncStatusCR(sc,
                                                          sync_pair,
                                                          status_obj,
                                                          bucket_status,
                                                          objv,
            ...
            assert(bucket_status.state == BucketSyncState::Incremental || bucket_status.state == BucketSyncState::Full);

            if (bucket_status.state == BucketSyncState::Full) {
                assert(bucket_lease_cr);
                yield call(
                    new RGWBucketFullSyncCR(sc, sync_pipe, status_obj, bucket_lease_cr, bucket_status, tn, objv));
                if (retcode < 0) {
            ...
            if (bucket_status.state == BucketSyncState::Incremental) {
                ...
                if (gen) {
                    ...
                    if (*gen > current_gen) {
                        ...
                        if ((static_cast<std::size_t>(source_bs.shard_id) <
                            ...
                            // use the error repo and sync status timestamp from the datalog shard corresponding to
                            // source_bs
                            error_repo = datalog_oid_for_error_repo(sc, sc->env->driver, pool, source_bs);
                            yield call(rgw::error_repo::write_cr(sc->env->driver->svc()->rados,
                                                                 error_repo,
                                                                 rgw::error_repo::encode_key(source_bs, current_gen),
                                                                 ceph::real_clock::zero()));
                            if (retcode < 0) {
                                tn->log(0,
                                        SSTR("ERROR: failed to log prev gen entry (bucket="
                                             << source_bs.bucket << ", shard_id=" << source_bs.shard_id
                                             << ", gen=" << current_gen << " in error repo: retcode=" << retcode));
                            }
                            else {
                                tn->log(20,
                                        SSTR("logged prev gen entry (bucket="
                                             << source_bs.bucket << ", shard_id=" << source_bs.shard_id
                                             << ", gen=" << current_gen << " in error repo: retcode=" << retcode));
                            }
                        }
                        ...
                        tn->log(
                            10,
                            SSTR("ERROR: requested sync of future generation "
                                 << *gen << " > " << current_gen << ", returning " << retcode << " for later retry"));
                        return set_cr_error(retcode);
                    }
                    else if (*gen < current_gen) {
                        tn->log(10,
                                SSTR("WARNING: requested sync of past generation " << *gen << " < " << current_gen
                                                                                   << ", returning success"));
                        return set_cr_done();
                    }
                }
                ...
                if (bucket_status.shards_done_with_gen[sync_pair.source_bs.shard_id]) {
                    tn->log(10,
                            SSTR("bucket shard " << sync_pair.source_bs << " of gen " << gen << " already synced."));
                    return set_cr_done();
                }

                yield call(new RGWSyncBucketShardCR(sc,
                                                    data_lease_cr,
                                                    sync_pair,
                                                    sync_pipe,
                                                    bucket_stopped,
                                                    bucket_status.incremental_gen,
                                                    tn,
                                                    progress));
                if (retcode < 0) {
...
int RGWBucketPipeSyncStatusManager::do_init(const DoutPrefixProvider* dpp, std::ostream* ostr)
{
    int ret = http_manager.start();
    if (ret < 0) {
    ...
    sync_env.init(this,
                  ...
                  &http_manager,
                  error_logger.get(),
                  driver->getRados()->get_sync_tracer(),
                  sync_module,
                  nullptr);
    ...
    ret = cr_mgr.run(dpp,
                     new RGWGetBucketPeersCR(
                         &sync_env, dest_bucket, source_zone, source_bucket, &pipes, sync_env.sync_tracer->root_node));
    if (ret < 0) {
        ldpp_dout(this, 0) << "failed to get bucket source peers info: (ret=" << ret << "): " << cpp_strerror(-ret)
                           << dendl;
        return ret;
    }
    ...
    for (auto& pipe : pipes) {
        ...
        sources.emplace_back(
            &sync_env, szone, conn, pipe.source.get_bucket_info(), pipe.target.get_bucket(), pipe.handler, z->name);
    }

    return 0;
}

int RGWBucketPipeSyncStatusManager::remote_info(const DoutPrefixProvider* dpp, source& s, uint64_t* oldest_gen,
                                                uint64_t* latest_gen, uint64_t* num_shards)
{
    rgw_bucket_index_marker_info remote_info;
    BucketIndexShardsManager remote_markers;
    auto r = rgw_read_remote_bilog_info(dpp, s.sc.conn, s.info.bucket, remote_info, remote_markers, null_yield);

    if (r < 0) {
        ldpp_dout(dpp, 0) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " rgw_read_remote_bilog_info: r=" << r << dendl;
        return r;
    }
    if (oldest_gen) *oldest_gen = remote_info.oldest_gen;

    if (latest_gen) *latest_gen = remote_info.latest_gen;

    if (num_shards) *num_shards = remote_markers.get().size();

    return 0;
}

tl::expected<std::unique_ptr<RGWBucketPipeSyncStatusManager>, int> RGWBucketPipeSyncStatusManager::construct(
    const DoutPrefixProvider* dpp, rgw::sal::RadosStore* driver, std::optional<rgw_zone_id> source_zone,
    std::optional<rgw_bucket> source_bucket, const rgw_bucket& dest_bucket, std::ostream* ostr)
{
    std::unique_ptr<RGWBucketPipeSyncStatusManager> self{
        new RGWBucketPipeSyncStatusManager(driver, source_zone, source_bucket, dest_bucket)};
...
tl::expected<std::map<int, rgw_bucket_shard_sync_info>, int> RGWBucketPipeSyncStatusManager::read_sync_status(
    const DoutPrefixProvider* dpp)
{
    ...
    if (source_zone) {
        sz = std::find_if(
            sources.begin(), sources.end(), [this](const source& s) { return s.sc.source_zone == *source_zone; });
        if (sz == sources.end()) {
            ldpp_dout(this, 0) << "ERROR: failed to find source zone: " << *source_zone << dendl;
            return tl::unexpected(-ENOENT);
        }
    }
    else {
        ldpp_dout(this, 5) << "No source zone specified, using source zone: " << sz->sc.source_zone << dendl;
        return tl::unexpected(-ENOENT);
    }
    ...
    for (auto shard = 0u; shard < num_shards; ++shard) {
        ...
        stack->call(new RGWReadBucketPipeSyncStatusCoroutine(&sz->sc, pair, &sync_status[shard], nullptr, latest_gen));
    }
...
public:
    ShardCR(RGWDataSyncCtx& sc, const rgw_bucket_sync_pair_info& pair, const uint64_t gen)
        : RGWCoroutine(sc.cct)
        , sc(sc)
        , pair(pair)
        , gen(gen)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            for (retries = 0u, retcode = -EDOM; (retries < allowed_retries) && (retcode != 0); ++retries) {
                ldpp_dout(dpp, 5) << "ShardCR: syncing bucket shard on: "
                                  << "zone=" << sc.source_zone << ", bucket=" << pair.source_bs.bucket.name
                                  << ", shard=" << pair.source_bs.shard_id << ", gen=" << gen << dendl;
                yield call(sync_bucket_shard_cr(&sc, nullptr, pair, gen, sc.env->sync_tracer->root_node, &progress));

                if (retcode == -ECANCELED) {
                    ldpp_dout(dpp, -1) << "ERROR: Got -ECANCELED for " << pair.source_bs << dendl;
                    drain_all();
                    return set_cr_error(retcode);
                }
                else if (retcode < 0) {
                    ldpp_dout(dpp, 5) << "WARNING: Got error, retcode=" << retcode << " for " << pair.source_bs
                                      << "on retry " << retries + 1 << " of " << allowed_retries << " allowed" << dendl;
            ...
            if (retcode < 0) {
                ldpp_dout(dpp, -1) << "ERROR: Exhausted retries for " << pair.source_bs << " retcode=" << retcode
                                   << dendl;
                drain_all();
                return set_cr_error(retcode);
            }
...
public:
    GenCR(RGWDataSyncCtx& sc, const rgw_bucket& source, const rgw_bucket& dest, const uint64_t gen,
          const uint64_t shards, const RGWBucketSyncFlowManager::pipe_handler& handler)
        : RGWShardCollectCR(sc.cct, MAX_CONCURRENT_SHARDS)
        , sc(sc)
        , gen(gen)
    {
    ...
    {
        if (r < 0) {
            ldpp_dout(sc.env->dpp, 4) << "ERROR: Error syncing shard: " << cpp_strerror(r) << dendl;
        }
        return r;
    }
};
...
class SourceCR : public RGWCoroutine
{
    ...
    const rgw_raw_obj status_obj{sc.env->svc->zone->get_zone_params().log_pool,
                                 RGWBucketPipeSyncStatusManager::full_status_oid(sc.source_zone, info.bucket, dest)};
...
public:
    SourceCR(RGWDataSyncCtx& sc, const RGWBucketInfo& info, const rgw_bucket& dest,
             const RGWBucketSyncFlowManager::pipe_handler& handler, const std::string& zone_name)
        : RGWCoroutine(sc.cct)
        , sc(sc)
        , info(info)
        , dest(dest)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            yield call(new RGWSimpleRadosReadCR<rgw_bucket_sync_status>(dpp, sc.env->driver, status_obj, &status));
            if (retcode < 0) {
                ldpp_dout(dpp, -1) << "ERROR: Unable to fetch status for zone=" << sc.source_zone
            ...
            if (status.state == BucketSyncState::Stopped) {
                ...
                pretty_print(sc.env,
                             ...
                             dest.name,
                             zone_name);
                ldpp_dout(dpp, 5) << "SourceCR: Bucket is in state Stopped, returning." << dendl;
                drain_all();
                return set_cr_done();
            }
            ...
            do {
                ...
                ldpp_dout(dpp, 5) << "SourceCR: "
                                  << "state=" << state << ", gen=" << gen << ", num_shards=" << num_shards << dendl;
                ...
                if (state != BucketSyncState::Incremental) {
                    ...
                    yield call(new GenCR(sc, info.bucket, dest, gen, num_shards, handler));
                }
                if (retcode < 0) {
                    ldpp_dout(dpp, -1) << "ERROR: Giving up syncing from " << sc.source_zone << " retcode=" << retcode
                                       << dendl;
                    drain_all();
                    return set_cr_error(retcode);
                }
...
int RGWBucketPipeSyncStatusManager::run(const DoutPrefixProvider* dpp)
{
    ...
    for (auto& source : sources) {
        auto stack = new RGWCoroutinesStack(driver->ctx(), &cr_mgr);
        stack->call(
            new rgw::bucket_sync_run::SourceCR(source.sc, source.info, source.dest, source.handler, source.zone_name));
        stacks.push_back(stack);
    }
    auto ret = cr_mgr.run(dpp, stacks);
    if (ret < 0) {
        ldpp_dout(this, 0) << "ERROR: Sync unsuccessful on bucket " << bucket_str{dest_bucket} << dendl;
    }
    return ret;
}
...
std::ostream& RGWBucketPipeSyncStatusManager::gen_prefix(std::ostream& out) const
{
    ...
    return out << "bucket sync zone:" << zone.substr(0, 8) << " bucket:" << dest_bucket << ' ';
}

string RGWBucketPipeSyncStatusManager::full_status_oid(const rgw_zone_id& source_zone, const rgw_bucket& source_bucket,
                                                       const rgw_bucket& dest_bucket)
{
    if (source_bucket == dest_bucket) {
        return bucket_full_status_oid_prefix + "." + source_zone.id + ":" + dest_bucket.get_key();
    }
    else {
        return bucket_full_status_oid_prefix + "." + source_zone.id + ":" + dest_bucket.get_key() + ":" +
                                                      ...
                                                      const rgw_bucket_sync_pair_info& sync_pair, uint64_t gen)
{
    if (sync_pair.source_bs.bucket == sync_pair.dest_bucket) {
        ...
        return bucket_status_oid_prefix + "." + source_zone.id + ":" + sync_pair.dest_bucket.get_key() + ":" +
...
string RGWBucketPipeSyncStatusManager::obj_status_oid(const rgw_bucket_sync_pipe& sync_pipe,
                                                      const rgw_zone_id& source_zone, const rgw_obj& obj)
{
    string prefix = object_status_oid_prefix + "." + source_zone.id + ":" + obj.bucket.get_key();
    if (sync_pipe.source_bucket_info.bucket != sync_pipe.dest_bucket_info.bucket) {
        prefix += string("/") + sync_pipe.dest_bucket_info.bucket.get_key();
    }
    return prefix + ":" + obj.key.name + ":" + obj.key.instance;
}

int rgw_read_remote_bilog_info(const DoutPrefixProvider* dpp, RGWRESTConn* conn, const rgw_bucket& bucket,
                               rgw_bucket_index_marker_info& info, BucketIndexShardsManager& markers, optional_yield y)
{
    const auto instance_key = bucket.get_key();
    const rgw_http_param_pair params[] = {
        {"type", "bucket-index"}, {"bucket-instance", instance_key.c_str()}, {"info", nullptr}, {nullptr, nullptr}};
...
class RGWCollectBucketSyncStatusCR : public RGWShardCollectCR
{
    ...
    int handle_result(int r) override
    {
        ...
        if (r < 0) {
            ldout(cct, 4) << "failed to read bucket shard sync status: " << cpp_strerror(r) << dendl;
        }
        return r;
    }
...
public:
    RGWCollectBucketSyncStatusCR(rgw::sal::RadosStore* driver, RGWDataSyncCtx* sc,
                                 const rgw_bucket_sync_pair_info& sync_pair, uint64_t gen, Vector* status)
        : RGWShardCollectCR(sc->cct, max_concurrent_shards)
        , driver(driver)
        , sc(sc)
        , env(sc->env)
...
int rgw_read_bucket_full_sync_status(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* driver,
                                     const rgw_sync_bucket_pipe& pipe, rgw_bucket_sync_status* status, optional_yield y)
{
...
int rgw_read_bucket_inc_sync_status(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* driver,
                                    const rgw_sync_bucket_pipe& pipe, uint64_t gen,
                                    std::vector<rgw_bucket_shard_sync_info>* status)
{
    if (!pipe.source.zone || !pipe.source.bucket || !pipe.dest.zone || !pipe.dest.bucket) {
        return -EINVAL;
    }
    ...
    env.init(dpp,
             ...
             driver,
             driver->svc(),
             driver->svc()->rados->get_async_processor(),
             nullptr,
             nullptr,
             nullptr,
             module,
    ...
    RGWCoroutinesManager crs(driver->ctx(), driver->getRados()->get_cr_registry());
    return crs.run(dpp, new RGWCollectBucketSyncStatusCR(driver, &sc, sync_pair, gen, status));
}
...
void rgw_bucket_shard_sync_info::dump(Formatter* f) const
{
    ...
    case StateInit: s = "init"; break;
    case StateFullSync: s = "full-sync"; break;
    case StateIncrementalSync: s = "incremental-sync"; break;
    case StateStopped: s = "stopped"; break;
...
{
    switch (state) {
    case BucketSyncState::Init: encode_json(name, "init", f); break;
    case BucketSyncState::Full: encode_json(name, "full-sync", f); break;
    case BucketSyncState::Incremental: encode_json(name, "incremental-sync", f); break;
    case BucketSyncState::Stopped: encode_json(name, "stopped", f); break;
    default: encode_json(name, "unknown", f); break;
    }
}
