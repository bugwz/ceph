...
protected:
    ...
    struct RGWWQ : public DoutPrefixProvider, public ThreadPool::WorkQueue<RGWAsyncRadosRequest>
    {
        ...
        RGWWQ(RGWAsyncRadosProcessor* p, ceph::timespan timeout, ceph::timespan suicide_timeout, ThreadPool* tp)
            : ThreadPool::WorkQueue<RGWAsyncRadosRequest>("RGWWQ", timeout, suicide_timeout, tp)
            , processor(p)
        {}

        bool _enqueue(RGWAsyncRadosRequest* req) override;
        void _dequeue(RGWAsyncRadosRequest* req) override { ceph_abort(); }
        ...
        void _dump_queue();
        void _clear() override { ceph_assert(processor->m_req_queue.empty()); }
...
public:
    ...
    bool is_going_down() { return going_down; }
};

template<class P> class RGWSimpleWriteOnlyAsyncCR : public RGWSimpleCoroutine
{
    ...
    public:
        Request(RGWCoroutine* caller, RGWAioCompletionNotifier* cn, rgw::sal::RadosStore* store, const P& _params,
                const DoutPrefixProvider* dpp)
...
public:
    RGWSimpleWriteOnlyAsyncCR(RGWAsyncRadosProcessor* _async_rados, rgw::sal::RadosStore* _store, const P& _params,
    ...
    ~RGWSimpleWriteOnlyAsyncCR() override { request_cleanup(); }
    ...
    {
        req = new Request(this, stack->create_completion_notifier(), store, params, dpp);
...
template<class P, class R> class RGWSimpleAsyncCR : public RGWSimpleCoroutine
{
    RGWAsyncRadosProcessor* async_rados;
    rgw::sal::RadosStore* store;
    ...
    public:
        Request(const DoutPrefixProvider* dpp, RGWCoroutine* caller, RGWAioCompletionNotifier* cn,
...
public:
    RGWSimpleAsyncCR(RGWAsyncRadosProcessor* _async_rados, rgw::sal::RadosStore* _store, const P& _params,
                     std::shared_ptr<R>& _result, const DoutPrefixProvider* _dpp)
        : RGWSimpleCoroutine(_store->ctx())
        , async_rados(_async_rados)
        , store(_store)
        , params(_params)
    ...
    ~RGWSimpleAsyncCR() override { request_cleanup(); }
    void request_cleanup() override
    {
        if (req) {
            req->finish();
    ...
    int send_request(const DoutPrefixProvider* dpp) override
    {
        req = new Request(dpp, this, stack->create_completion_notifier(), store, params, result, dpp);
...
public:
    RGWGenericAsyncCR(CephContext* _cct, RGWAsyncRadosProcessor* _async_rados, std::shared_ptr<Action>& _action)
        : RGWSimpleCoroutine(_cct)
        , async_rados(_async_rados)
        , action(_action)
    {}
    template<typename T>
    RGWGenericAsyncCR(CephContext* _cct, RGWAsyncRadosProcessor* _async_rados, std::shared_ptr<T>& _action)
        : RGWSimpleCoroutine(_cct)
        , async_rados(_async_rados)
        , action(std::static_pointer_cast<Action>(_action))
    {}

    ~RGWGenericAsyncCR() override { request_cleanup(); }
    void request_cleanup() override
    {
        if (req) {
            req->finish();
    ...
    int send_request(const DoutPrefixProvider* dpp) override
    {
        req = new Request(dpp, this, stack->create_completion_notifier(), action);
...
public:
    RGWAsyncGetSystemObj(const DoutPrefixProvider* dpp, RGWCoroutine* caller, RGWAioCompletionNotifier* cn,
                         RGWSI_SysObj* _svc, RGWObjVersionTracker* _objv_tracker, const rgw_raw_obj& _obj,
...
public:
    RGWAsyncPutSystemObj(const DoutPrefixProvider* dpp, RGWCoroutine* caller, RGWAioCompletionNotifier* cn,
                         RGWSI_SysObj* _svc, RGWObjVersionTracker* _objv_tracker, const rgw_raw_obj& _obj,
...
public:
    RGWAsyncPutSystemObjAttrs(const DoutPrefixProvider* dpp, RGWCoroutine* caller, RGWAioCompletionNotifier* cn,
                              RGWSI_SysObj* _svc, RGWObjVersionTracker* _objv_tracker, const rgw_raw_obj& _obj,
...
public:
    RGWAsyncLockSystemObj(RGWCoroutine* caller, RGWAioCompletionNotifier* cn, rgw::sal::RadosStore* _store,
                          RGWObjVersionTracker* _objv_tracker, const rgw_raw_obj& _obj, const std::string& _name,
...
template<class T> class RGWSimpleRadosReadCR : public RGWSimpleCoroutine
{
    const DoutPrefixProvider* dpp;
    rgw::sal::RadosStore* store;
    rgw_raw_obj obj;
...
public:
    RGWSimpleRadosReadCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, const rgw_raw_obj& obj, T* result,
                         bool empty_on_enoent = true, RGWObjVersionTracker* objv_tracker = nullptr)
    ...
    int send_request(const DoutPrefixProvider* dpp)
    {
        ...
        return ref.pool.ioctx().aio_operate(ref.obj.oid, cn->completion(), &op, nullptr);
    }
    ...
    virtual int handle_data(T& data) { return 0; }
};
...
public:
    RGWSimpleRadosReadAttrsCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, rgw_raw_obj obj,
                              std::map<std::string, bufferlist>* pattrs, bool raw_attrs,
...
template<class T> class RGWSimpleRadosWriteCR : public RGWSimpleCoroutine
{
    const DoutPrefixProvider* dpp;
    rgw::sal::RadosStore* const store;
    rgw_raw_obj obj;
...
public:
    RGWSimpleRadosWriteCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* const store, rgw_raw_obj obj,
                          const T& data, RGWObjVersionTracker* objv_tracker = nullptr, bool exclusive = false)
        : RGWSimpleCoroutine(store->ctx())
        , dpp(dpp)
        , store(store)
        , obj(std::move(obj))
...
public:
    RGWSimpleRadosWriteAttrsCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* const store, rgw_raw_obj obj,
                               std::map<std::string, bufferlist> attrs, RGWObjVersionTracker* objv_tracker = nullptr,
...
public:
    RGWRadosSetOmapKeysCR(rgw::sal::RadosStore* _store, const rgw_raw_obj& _obj,
...
public:
    ...
    RGWRadosGetOmapKeysCR(rgw::sal::RadosStore* _store, const rgw_raw_obj& _obj, const std::string& _marker,
                          int _max_entries, ResultPtr result);
...
public:
    ...
    RGWRadosGetOmapValsCR(rgw::sal::RadosStore* _store, const rgw_raw_obj& _obj, const std::string& _marker,
                          int _max_entries, ResultPtr result);
...
public:
    RGWRadosRemoveOmapKeysCR(rgw::sal::RadosStore* _store, const rgw_raw_obj& _obj, const std::set<std::string>& _keys);
...
public:
    RGWRadosRemoveCR(rgw::sal::RadosStore* store, const rgw_raw_obj& obj, RGWObjVersionTracker* objv_tracker = nullptr);
...
public:
    RGWRadosRemoveOidCR(rgw::sal::RadosStore* store, librados::IoCtx&& ioctx, std::string_view oid,
                        RGWObjVersionTracker* objv_tracker = nullptr);

    RGWRadosRemoveOidCR(rgw::sal::RadosStore* store, RGWSI_RADOS::Obj& obj,
                        RGWObjVersionTracker* objv_tracker = nullptr);
...
public:
    RGWSimpleRadosLockCR(RGWAsyncRadosProcessor* _async_rados, rgw::sal::RadosStore* _store, const rgw_raw_obj& _obj,
                         const std::string& _lock_name, const std::string& _cookie, uint32_t _duration);
    ~RGWSimpleRadosLockCR() override { request_cleanup(); }
    void request_cleanup() override;
...
public:
    RGWSimpleRadosUnlockCR(RGWAsyncRadosProcessor* _async_rados, rgw::sal::RadosStore* _store, const rgw_raw_obj& _obj,
                           const std::string& _lock_name, const std::string& _cookie);
    ~RGWSimpleRadosUnlockCR() override { request_cleanup(); }
    void request_cleanup() override;
...
public:
    RGWOmapAppend(RGWAsyncRadosProcessor* _async_rados, rgw::sal::RadosStore* _store, const rgw_raw_obj& _obj,
    ...
    uint64_t get_total_entries() { return total_entries; }

    const rgw_raw_obj& get_obj() { return obj; }
};
...
public:
    RGWShardedOmapCRManager(RGWAsyncRadosProcessor* _async_rados, rgw::sal::RadosStore* _store, RGWCoroutine* _op,
                            int _num_shards, const rgw_pool& pool, const std::string& oid_prefix)
        : async_rados(_async_rados)
        , store(_store)
        , op(_op)
    ...
    bool append(const std::string& entry, int shard_id) { return shards[shard_id]->append(entry); }
    bool finish()
    {
        bool success = true;
    ...
    uint64_t get_total_entries(int shard_id) { return shards[shard_id]->get_total_entries(); }
};
...
public:
    RGWAsyncGetBucketInstanceInfo(RGWCoroutine* caller, RGWAioCompletionNotifier* cn, rgw::sal::RadosStore* _store,
                                  const rgw_bucket& bucket, const DoutPrefixProvider* dpp)
        : RGWAsyncRadosRequest(caller, cn)
        , store(_store)
        , bucket(bucket)
        , dpp(dpp)
...
public:
    RGWAsyncPutBucketInstanceInfo(RGWCoroutine* caller, RGWAioCompletionNotifier* cn, rgw::sal::RadosStore* store,
                                  RGWBucketInfo& bucket_info, bool exclusive, real_time mtime,
                                  std::map<std::string, ceph::bufferlist>* attrs, const DoutPrefixProvider* dpp)
        : RGWAsyncRadosRequest(caller, cn)
        , store(store)
...
public:
    ...
    RGWGetBucketInstanceInfoCR(RGWAsyncRadosProcessor* _async_rados, rgw::sal::RadosStore* _store,
        ...
        , dpp(dpp)
    {}
    ~RGWGetBucketInstanceInfoCR() override { request_cleanup(); }
    void request_cleanup() override
    {
        if (req) {
            req->finish();
...
public:
    // rgw_bucket constructor
    RGWPutBucketInstanceInfoCR(RGWAsyncRadosProcessor* async_rados, rgw::sal::RadosStore* store,
                               RGWBucketInfo& bucket_info, bool exclusive, real_time mtime,
                               std::map<std::string, ceph::bufferlist>* attrs, const DoutPrefixProvider* dpp)
        : RGWSimpleCoroutine(store->ctx())
        ...
        , dpp(dpp)
    {}
    ~RGWPutBucketInstanceInfoCR() override { request_cleanup(); }
    void request_cleanup() override
    {
        if (req) {
            req->finish();
    ...
    int send_request(const DoutPrefixProvider* dpp) override
    {
        req = new RGWAsyncPutBucketInstanceInfo(
            this, stack->create_completion_notifier(), store, bucket_info, exclusive, mtime, attrs, dpp);
        async_rados->queue(req);
        return 0;
    }
    int request_complete() override { return req->get_ret_status(); }
...
public:
    RGWRadosBILogTrimCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, const RGWBucketInfo& bucket_info,
                        int shard_id, const rgw::bucket_index_layout_generation& generation,
...
public:
    RGWAsyncFetchRemoteObj(RGWCoroutine* caller, RGWAioCompletionNotifier* cn, rgw::sal::RadosStore* _store,
                           const rgw_zone_id& _source_zone, std::optional<rgw_user>& _user_id,
                           const rgw_bucket& _src_bucket, std::optional<rgw_placement_rule> _dest_placement_rule,
                           const RGWBucketInfo& _dest_bucket_info, const rgw_obj_key& _key,
                           const std::optional<rgw_obj_key>& _dest_key, std::optional<uint64_t> _versioned_epoch,
                           bool _if_newer, std::shared_ptr<RGWFetchObjFilter> _filter,
...
public:
    ...
    ~RGWFetchRemoteObjCR() override { request_cleanup(); }
    ...
    int send_request(const DoutPrefixProvider* dpp) override
    {
        req = new RGWAsyncFetchRemoteObj(this,
...
public:
    RGWAsyncStatRemoteObj(RGWCoroutine* caller, RGWAioCompletionNotifier* cn, rgw::sal::RadosStore* _store,
                          const rgw_zone_id& _source_zone, rgw_bucket& _src_bucket, const rgw_obj_key& _key,
                          ceph::real_time* _pmtime, uint64_t* _psize, std::string* _petag,
                          std::map<std::string, bufferlist>* _pattrs, std::map<std::string, std::string>* _pheaders)
        : RGWAsyncRadosRequest(caller, cn)
        , store(_store)
        , source_zone(_source_zone)
        , src_bucket(_src_bucket)
...
public:
    ...
    ~RGWStatRemoteObjCR() override { request_cleanup(); }
    ...
    int send_request(const DoutPrefixProvider* dpp) override
    {
        req = new RGWAsyncStatRemoteObj(this,
                                        stack->create_completion_notifier(),
                                        store,
                                        source_zone,
                                        src_bucket,
...
public:
    RGWAsyncRemoveObj(const DoutPrefixProvider* _dpp, RGWCoroutine* caller, RGWAioCompletionNotifier* cn,
                      ...
                      const rgw_obj_key& _key, const std::string& _owner, const std::string& _owner_display_name,
                      bool _versioned, uint64_t _versioned_epoch, bool _delete_marker, bool _if_older,
                      real_time& _timestamp, rgw_zone_set* _zones_trace)
        : RGWAsyncRadosRequest(caller, cn)
        , dpp(_dpp)
        , store(_store)
        , source_zone(_source_zone)
...
public:
    RGWRemoveObjCR(const DoutPrefixProvider* _dpp, RGWAsyncRadosProcessor* _async_rados, rgw::sal::RadosStore* _store,
                   const rgw_zone_id& _source_zone, RGWBucketInfo& _bucket_info, const rgw_obj_key& _key,
                   bool _versioned, uint64_t _versioned_epoch, std::string* _owner, std::string* _owner_display_name,
                   bool _delete_marker, real_time* _timestamp, rgw_zone_set* _zones_trace)
        : RGWSimpleCoroutine(_store->ctx())
        , dpp(_dpp)
        , cct(_store->ctx())
        , async_rados(_async_rados)
    ...
    ~RGWRemoveObjCR() override { request_cleanup(); }
...
public:
    RGWContinuousLeaseCR(RGWAsyncRadosProcessor* async_rados, rgw::sal::RadosStore* _store, rgw_raw_obj obj,
                         std::string lock_name, int interval, RGWCoroutine* caller, LatencyMonitor* const latency)
        ...
        , interval(interval)
        , interval_tolerance(ceph::make_timespan(9 * interval / 10))
        , ts_interval(ceph::make_timespan(interval))
        , caller(caller)
        , latency(latency)
    {}
    ...
    void set_locked(bool status) { locked = status; }
...
public:
    RGWRadosTimelogTrimCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, const std::string& oid,
                          const real_time& start_time, const real_time& end_time, const std::string& from_marker,
...
public:
    ...
    RGWSyncLogTrimCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, const std::string& oid,
...
public:
    RGWAsyncStatObj(const DoutPrefixProvider* dpp, RGWCoroutine* caller, RGWAioCompletionNotifier* cn,
                    rgw::sal::RadosStore* store, const RGWBucketInfo& _bucket_info, const rgw_obj& obj,
                    uint64_t* psize = nullptr, real_time* pmtime = nullptr, uint64_t* pepoch = nullptr,
                    RGWObjVersionTracker* objv_tracker = nullptr)
        : RGWAsyncRadosRequest(caller, cn)
        , dpp(dpp)
        , store(store)
...
public:
    RGWStatObjCR(const DoutPrefixProvider* dpp, RGWAsyncRadosProcessor* async_rados, rgw::sal::RadosStore* store,
                 ...
                 real_time* pmtime = nullptr, uint64_t* pepoch = nullptr, RGWObjVersionTracker* objv_tracker = nullptr);
    ~RGWStatObjCR() override { request_cleanup(); }
    void request_cleanup() override;
...
public:
    RGWRadosNotifyCR(rgw::sal::RadosStore* store, const rgw_raw_obj& obj, bufferlist& request, uint64_t timeout_ms,
                     bufferlist* response);
...
public:
    RGWDataPostNotifyCR(RGWRados* _store, RGWHTTPManager& _http_manager,
                        bc::flat_map<int, bc::flat_set<rgw_data_notify_entry> >& _shards, const char* _zone,
                        RGWRESTConn* _conn)
        : RGWCoroutine(_store->ctx())
        , store(_store)
