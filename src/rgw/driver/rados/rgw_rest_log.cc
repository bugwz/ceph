...
void RGWOp_MDLog_List::execute(optional_yield y)
{
    ...
    string marker = s->info.args.get("marker"), err;
    void* handle;
    unsigned shard_id, max_entries = LOG_CLASS_LIST_MAX_ENTRIES;

    if (s->info.args.exists("start-time") || s->info.args.exists("end-time")) {
    ...
    RGWMetadataLog meta_log{s->cct,
                            static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone,
                            static_cast<rgw::sal::RadosStore*>(driver)->svc()->cls,
                            period};

    meta_log.init_list_entries(shard_id, {}, {}, marker, &handle);

    op_ret = meta_log.list_entries(this, handle, max_entries, entries, &last_marker, &truncated);
...
{
    string marker = s->info.args.get("marker"), period = s->info.args.get("period"), shard = s->info.args.get("id"),
           err;
    unsigned shard_id;
...
void RGWOp_MDLog_Lock::execute(optional_yield y)
{
    ...
    if (period.empty() || shard_id_str.empty() || (duration_str.empty()) || locker_id.empty() || zone_id.empty()) {
    ...
    op_ret = meta_log.lock_exclusive(s, shard_id, make_timespan(dur), zone_id, locker_id);
    if (op_ret == -EBUSY) op_ret = -ERR_LOCKED;
}
...
void RGWOp_MDLog_Unlock::execute(optional_yield y)
{
    ...
    if (period.empty() || shard_id_str.empty() || locker_id.empty() || zone_id.empty()) {
        ldpp_dout(this, 5) << "Error invalid parameter list" << dendl;
        op_ret = -EINVAL;
        return;
    }
...
void RGWOp_BILog_List::execute(optional_yield y)
{
    ...
    string tenant_name = s->info.args.get("tenant"), bucket_name = s->info.args.get("bucket"),
           marker = s->info.args.get("marker"), max_entries_str = s->info.args.get("max-entries"),
    ...
    max_entries = (unsigned)strict_strtol(max_entries_str.c_str(), 10, &err);
    if (!err.empty()) max_entries = LOG_CLASS_LIST_MAX_ENTRIES;

    send_response();
    do {
        list<rgw_bi_log_entry> entries;
        int ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->bilog_rados->log_list(
            s, bucket->get_info(), log_layout, shard_id, marker, max_entries - count, entries, &truncated);
        if (ret < 0) {
...
void RGWOp_BILog_Delete::execute(optional_yield y)
{
    bool gen_specified = false;
    string tenant_name = s->info.args.get("tenant"), bucket_name = s->info.args.get("bucket"),
           start_marker = s->info.args.get("start-marker"), end_marker = s->info.args.get("end-marker"),
           bucket_instance = s->info.args.get("bucket-instance"),
           gen_str = s->info.args.get("generation", &gen_specified);
    ...
    op_ret = 0;
    if ((bucket_name.empty() && bucket_instance.empty()) || end_marker.empty()) {
    ...
    op_ret = bilog_trim(
        this, static_cast<rgw::sal::RadosStore*>(driver), bucket->get_info(), gen, shard_id, start_marker, end_marker);
    if (op_ret < 0) {
...
void RGWOp_DATALog_List::execute(optional_yield y)
{
    ...
    string max_entries_str = s->info.args.get("max-entries"), marker = s->info.args.get("marker"), err;
    unsigned shard_id, max_entries = LOG_CLASS_LIST_MAX_ENTRIES;
    ...
    // entry listed
    op_ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->datalog_rados->list_entries(
        this, shard_id, max_entries, entries, marker, &last_marker, &truncated, y);
}
...
void RGWOp_DATALog_ShardInfo::execute(optional_yield y)
{
    ...
    op_ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->datalog_rados->get_info(this, shard_id, &info, y);
}
...
#define LARGE_ENOUGH_BUF (128 * 1024)
    ...
    if (driver->ctx()->_conf->subsys.should_gather<ceph_subsys_rgw, 20>()) {
        for (bc::flat_map<int, bc::flat_set<rgw_data_notify_entry>>::iterator iter = updated_shards.begin();
             iter != updated_shards.end();
             ++iter) {
            ...
            for (const auto& [key, gen] : entries) {
                ldpp_dout(this, 20) << __func__ << "(): modified key=" << key << " of gen=" << gen << dendl;
            }
        }
    }
...
#define LARGE_ENOUGH_BUF (128 * 1024)
    ...
    if (driver->ctx()->_conf->subsys.should_gather<ceph_subsys_rgw, 20>()) {
        for (bc::flat_map<int, bc::flat_set<rgw_data_notify_entry>>::iterator iter = updated_shards.begin();
            ...
            bc::flat_set<rgw_data_notify_entry>& entries = iter->second;
            for (const auto& [key, gen] : entries) {
                ldpp_dout(this, 20) << __func__ << "(): modified key=" << key << " of generation=" << gen << dendl;
            }
        }
    }
...
{
    string marker = s->info.args.get("marker"), shard = s->info.args.get("id"), err;
    unsigned shard_id;
    ...
    op_ret = static_cast<rgw::sal::RadosStore*>(driver)->svc()->datalog_rados->trim_entries(this, shard_id, marker, y);
}
...
public:
    int check_caps(const RGWUserCaps& caps) override { return caps.check_cap("mdlog", RGW_CAP_READ); }
    int verify_permission(optional_yield) override { return check_caps(s->user->get_caps()); }
...
public:
    int check_caps(const RGWUserCaps& caps) override { return caps.check_cap("bilog", RGW_CAP_READ); }
    int verify_permission(optional_yield y) override { return check_caps(s->user->get_caps()); }
    void execute(optional_yield y) override;
    void send_response() override;
...
void RGWOp_BILog_Status::execute(optional_yield y)
{
    ...
    if (source_key.empty() || source_key == key) {
    ...
    if (!merge) {
        ...
        ldpp_dout(this, 20) << "RGWOp_BILog_Status::execute(optional_yield y): getting sync status for pipe=" << pipe
                            << dendl;

        op_ret = rgw_read_bucket_full_sync_status(
            this, static_cast<rgw::sal::RadosStore*>(driver), pipe, &status.sync_status, s->yield);
        if (op_ret < 0) {
            ldpp_dout(this, -1) << "ERROR: rgw_read_bucket_full_sync_status() on pipe=" << pipe
                                << " returned ret=" << op_ret << dendl;
            return;
        }
        status.inc_status.resize(status.sync_status.shards_done_with_gen.size());

        op_ret = rgw_read_bucket_inc_sync_status(this,
                                                 static_cast<rgw::sal::RadosStore*>(driver),
                                                 pipe,
                                                 status.sync_status.incremental_gen,
                                                 &status.inc_status);
        if (op_ret < 0) {
            ldpp_dout(this, -1) << "ERROR: rgw_read_bucket_inc_sync_status() on pipe=" << pipe
                                << " returned ret=" << op_ret << dendl;
        }
        return;
    }
    ...
    for (auto& entry : local_dests) {
        ...
        if (!pipe.dest.bucket) {
            ...
            ldpp_dout(this, 20)
                << "ERROR: RGWOp_BILog_Status::execute(optional_yield y): BUG: pipe.dest.bucket was not initialized"
                << pipe << dendl;
            op_ret = -EIO;
            return;
        }
        ...
        int r = rgw_read_bucket_inc_sync_status(this,
                                                static_cast<rgw::sal::RadosStore*>(driver),
                                                pipe,
                                                status.sync_status.incremental_gen,
                                                &current_status);
        if (r < 0) {
            ldpp_dout(this, -1) << "ERROR: rgw_read_bucket_inc_sync_status() on pipe=" << pipe << " returned ret=" << r
                                << dendl;
            op_ret = r;
            return;
        }
        ...
        if (status.inc_status.empty()) {
            ...
            if (current_status.size() != status.inc_status.size()) {
                ...
                ldpp_dout(this, -1) << "ERROR: different number of shards for sync status of buckets "
                                    ...
                                    << status.inc_status.size() << " current_status.size()=" << current_status.size()
                                    << dendl;
                return;
            }
...
public:
    int check_caps(const RGWUserCaps& caps) override { return caps.check_cap("datalog", RGW_CAP_READ); }
    int verify_permission(optional_yield y) override { return check_caps(s->user->get_caps()); }
    void execute(optional_yield y) override;
    void send_response() override;
