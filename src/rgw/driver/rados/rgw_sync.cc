...
RGWSyncErrorLogger::RGWSyncErrorLogger(rgw::sal::RadosStore* _store, const string& oid_prefix, int _num_shards)
    : store(_store)
...
RGWCoroutine* RGWSyncErrorLogger::log_error_cr(const DoutPrefixProvider* dpp, const string& source_zone,
                                               const string& section, const string& name, uint32_t error_code,
...
public:
    RGWReadRemoteMDLogInfoCR(RGWMetaSyncEnv* _sync_env, const std::string& period, int _num_shards,
                             map<int, RGWMetadataLogInfo>* _mdlog_info)
        : RGWShardCollectCR(_sync_env->cct, READ_MDLOG_MAX_CONCURRENT)
        , sync_env(_sync_env)
        , period(period)
        , num_shards(_num_shards)
        , mdlog_info(_mdlog_info)
        , shard_id(0)
    {}
...
public:
    RGWListRemoteMDLogCR(RGWMetaSyncEnv* _sync_env, const std::string& period, map<int, string>& _shards,
                         int _max_entries_per_shard, map<int, rgw_mdlog_shard_data>* _result)
        : RGWShardCollectCR(_sync_env->cct, READ_MDLOG_MAX_CONCURRENT)
        , sync_env(_sync_env)
        , period(period)
...
{
    rgw_http_param_pair pairs[] = {{"type", "metadata"}, {NULL, NULL}};
...
int RGWRemoteMetaLog::read_master_log_shards_info(const DoutPrefixProvider* dpp, const string& master_period,
...
int RGWRemoteMetaLog::read_master_log_shards_next(const DoutPrefixProvider* dpp, const string& period,
...
int RGWMetaSyncStatusManager::init(const DoutPrefixProvider* dpp)
{
    ...
    int r = rgw_init_ioctx(
        dpp, store->getRados()->get_rados_handle(), store->svc()->zone->get_zone_params().log_pool, ioctx, true);
    if (r < 0) {
        ldpp_dout(dpp, -1) << "ERROR: failed to open log pool (" << store->svc()->zone->get_zone_params().log_pool
                           << " ret=" << r << dendl;
        return r;
    }
...
void RGWMetaSyncEnv::init(const DoutPrefixProvider* _dpp, CephContext* _cct, rgw::sal::RadosStore* _store,
                          RGWRESTConn* _conn, RGWAsyncRadosProcessor* _async_rados, RGWHTTPManager* _http_manager,
...
public:
    ...
    RGWAsyncReadMDLogEntries(const DoutPrefixProvider* dpp, RGWCoroutine* caller, RGWAioCompletionNotifier* cn,
                             rgw::sal::RadosStore* _store, RGWMetadataLog* mdlog, int _shard_id, std::string _marker,
                             int _max_entries)
...
public:
    RGWReadMDLogEntriesCR(RGWMetaSyncEnv* _sync_env, RGWMetadataLog* mdlog, int _shard_id, string* _marker,
                          int _max_entries, list<cls_log_entry>* _entries, bool* _truncated)
    ...
    {
        marker = *pmarker;
        req = new RGWAsyncReadMDLogEntries(
            dpp, this, stack->create_completion_notifier(), sync_env->store, mdlog, shard_id, marker, max_entries);
...
public:
    RGWReadRemoteMDLogShardInfoCR(RGWMetaSyncEnv* env, const std::string& period, int _shard_id,
                                  RGWMetadataLogInfo* _shard_info)
        : RGWCoroutine(env->store->ctx())
        , env(env)
        , http_op(NULL)
        , period(period)
        , shard_id(_shard_id)
        ...
        reenter(this)
        {
            yield
            {
                ...
                rgw_http_param_pair pairs[] = {
                    {"type", "metadata"}, {"id", buf}, {"period", period.c_str()}, {"info", NULL}, {NULL, NULL}};

                string p = "/admin/log/";

                http_op = new RGWRESTReadResource(conn, p, pairs, NULL, env->http_manager);
...
RGWCoroutine* create_read_remote_mdlog_shard_info_cr(RGWMetaSyncEnv* env, const std::string& period, int shard_id,
...
public:
    RGWListRemoteMDLogShardCR(RGWMetaSyncEnv* env, const std::string& period, int _shard_id, const string& _marker,
                              uint32_t _max_entries, rgw_mdlog_shard_data* _result)
        : RGWSimpleCoroutine(env->store->ctx())
        , sync_env(env)
        , http_op(NULL)
        , period(period)
        , shard_id(_shard_id)
...
RGWCoroutine* create_list_remote_mdlog_shard_cr(RGWMetaSyncEnv* env, const std::string& period, int shard_id,
                                                const std::string& marker, uint32_t max_entries,
                                                rgw_mdlog_shard_data* result)
{
    return new RGWListRemoteMDLogShardCR(env, period, shard_id, marker, max_entries, result);
}
...
bool RGWListRemoteMDLogCR::spawn_next()
{
    ...
    spawn(new RGWListRemoteMDLogShardCR(
              sync_env, period, iter->first, iter->second, max_entries_per_shard, &(*result)[iter->first]),
          false);
    ++iter;
    return true;
}
...
public:
    RGWInitSyncStatusCoroutine(RGWMetaSyncEnv* _sync_env, const rgw_meta_sync_info& status)
        : RGWCoroutine(_sync_env->store->ctx())
        , sync_env(_sync_env)
        , status(status)
        , shards_info(status.num_shards)
        , lease_cr(nullptr)
        , lease_stack(nullptr)
    {}
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        ...
        reenter(this)
        {
            ...
            yield
            {
                ...
                rgw::sal::RadosStore* store = sync_env->store;
                call(new RGWSimpleRadosWriteCR<rgw_meta_sync_info>(
                    dpp,
                    store,
                    rgw_raw_obj(store->svc()->zone->get_zone_params().log_pool, sync_env->status_oid()),
                    status));
            ...
            yield
            {
                for (int i = 0; i < (int)status.num_shards; i++) {
                    spawn(new RGWReadRemoteMDLogShardInfoCR(sync_env, status.period, i, &shards_info[i]), false);
                }
            }
            ...
            yield
            {
                ...
                for (int i = 0; i < (int)status.num_shards; i++) {
                    ...
                    rgw::sal::RadosStore* store = sync_env->store;
                    spawn(new RGWSimpleRadosWriteCR<rgw_meta_sync_marker>(
                              dpp,
                              store,
...
class RGWReadSyncStatusMarkersCR : public RGWShardCollectCR
{
    ...
    int handle_result(int r) override
    {
        ...
        if (r < 0) {
            ldout(cct, 4) << "failed to read metadata sync markers: " << cpp_strerror(r) << dendl;
        }
        return r;
    }

public:
    RGWReadSyncStatusMarkersCR(RGWMetaSyncEnv* env, int num_shards, map<uint32_t, rgw_meta_sync_marker>& markers)
...
bool RGWReadSyncStatusMarkersCR::spawn_next()
{
    ...
    rgw_raw_obj obj{env->store->svc()->zone->get_zone_params().log_pool, env->shard_obj_name(shard_id)};
...
public:
    RGWReadSyncStatusCoroutine(RGWMetaSyncEnv* _sync_env, rgw_meta_sync_status* _status)
...
int RGWReadSyncStatusCoroutine::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        yield
        {
            ...
            rgw_raw_obj obj{sync_env->store->svc()->zone->get_zone_params().log_pool, sync_env->status_oid()};
            call(new ReadInfoCR(dpp, sync_env->store, obj, &sync_status->sync_info, empty_on_enoent));
        }
        if (retcode < 0) {
        ...
        yield call(new ReadMarkersCR(sync_env, sync_status->sync_info.num_shards, sync_status->sync_markers));
        if (retcode < 0) {
...
public:
    RGWFetchAllMetaCR(RGWMetaSyncEnv* _sync_env, int _num_shards, map<uint32_t, rgw_meta_sync_marker>& _markers,
    ...
    void rearrange_sections()
    {
        ...
        std::move(sections.begin(), sections.end(), std::inserter(all_sections, all_sections.end()));
        sections.clear();
        ...
        std::move(all_sections.begin(), all_sections.end(), std::back_inserter(sections));
    }
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        ...
        reenter(this)
        {
            ...
            entries_index.reset(new RGWShardedOmapCRManager(sync_env->async_rados,
                                                            sync_env->store,
                                                            this,
                                                            num_shards,
            ...
            yield
            {
                call(new RGWReadRESTResourceCR<list<string> >(
                    cct, conn, sync_env->http_manager, "/admin/metadata", NULL, &sections));
            }
            if (get_ret_status() < 0) {
...
#define META_FULL_SYNC_CHUNK_SIZE "1000"
                        ...
                        rgw_http_param_pair pairs[] = {{"max-entries", META_FULL_SYNC_CHUNK_SIZE},
                                                       ...
                                                       {NULL, NULL}};
                        result.keys.clear();
                        call(new RGWReadRESTResourceCR<meta_list_result>(
                            cct, conn, sync_env->http_manager, entrypoint, pairs, &result));
                    }
                    ...
                    for (; iter != result.keys.end(); ++iter) {
                        ...
                        if (ret < 0) {
                            tn->log(0,
                                    SSTR("ERROR: could not determine shard id for " << *sections_iter << ":" << *iter));
                            ret_status = ret;
                            break;
                        }
            ...
            if (!failed) {
                for (map<uint32_t, rgw_meta_sync_marker>::iterator iter = markers.begin(); iter != markers.end();
                     ++iter) {
...
public:
    RGWReadRemoteMetadataCR(RGWMetaSyncEnv* _sync_env, const string& _section, const string& _key, bufferlist* _pbl,
        ...
        , pbl(_pbl)
    {
        tn = sync_env->sync_tracer->add_node(_tn_parent, "read_remote_meta", section + ":" + key);
    }
...
public:
    RGWAsyncMetaStoreEntry(RGWCoroutine* caller, RGWAioCompletionNotifier* cn, rgw::sal::RadosStore* _store,
                           const string& _raw_key, bufferlist& _bl, const DoutPrefixProvider* dpp)
        : RGWAsyncRadosRequest(caller, cn)
        , store(_store)
        , raw_key(_raw_key)
        , bl(_bl)
...
public:
    RGWMetaStoreEntryCR(RGWMetaSyncEnv* _sync_env, const string& _raw_key, bufferlist& _bl)
        : RGWSimpleCoroutine(_sync_env->cct)
        , sync_env(_sync_env)
        , raw_key(_raw_key)
        , bl(_bl)
    ...
    int send_request(const DoutPrefixProvider* dpp) override
    {
        req = new RGWAsyncMetaStoreEntry(this, stack->create_completion_notifier(), sync_env->store, raw_key, bl, dpp);
        sync_env->async_rados->queue(req);
        return 0;
    }

    int request_complete() override { return req->get_ret_status(); }
};
...
public:
    RGWAsyncMetaRemoveEntry(RGWCoroutine* caller, RGWAioCompletionNotifier* cn, rgw::sal::RadosStore* _store,
                            const string& _raw_key, const DoutPrefixProvider* dpp)
        : RGWAsyncRadosRequest(caller, cn)
        , store(_store)
        , raw_key(_raw_key)
        , dpp(dpp)
...
public:
    RGWMetaRemoveEntryCR(RGWMetaSyncEnv* _sync_env, const string& _raw_key)
        : RGWSimpleCoroutine(_sync_env->cct)
        , sync_env(_sync_env)
        , raw_key(_raw_key)
        , req(NULL)
    ...
    int send_request(const DoutPrefixProvider* dpp) override
    {
        req = new RGWAsyncMetaRemoveEntry(this, stack->create_completion_notifier(), sync_env->store, raw_key, dpp);
        sync_env->async_rados->queue(req);
        return 0;
    }
...
public:
    RGWMetaSyncShardMarkerTrack(RGWMetaSyncEnv* _sync_env, const string& _marker_oid,
                                const rgw_meta_sync_marker& _marker, RGWSyncTraceNodeRef& _tn)
    ...
    RGWCoroutine* store_marker(const string& new_marker, uint64_t index_pos, const real_time& timestamp) override
    {
        ...
        ldpp_dout(sync_env->dpp, 20) << __func__ << "(): updating marker marker_oid=" << marker_oid
                                     << " marker=" << new_marker << " realm_epoch=" << sync_marker.realm_epoch << dendl;
        tn->log(20, SSTR("new marker=" << new_marker));
        rgw::sal::RadosStore* store = sync_env->store;
        return new RGWSimpleRadosWriteCR<rgw_meta_sync_marker>(
            sync_env->dpp, store, rgw_raw_obj(store->svc()->zone->get_zone_params().log_pool, marker_oid), sync_marker);
    }

    RGWOrderCallCR* allocate_order_control_cr() override { return new RGWLastCallerWinsCR(sync_env->cct); }
};

RGWMetaSyncSingleEntryCR::RGWMetaSyncSingleEntryCR(RGWMetaSyncEnv* _sync_env, const string& _raw_key,
                                                   const string& _entry_marker, const RGWMDLogStatus& _op_status,
...
#define NUM_TRANSIENT_ERROR_RETRIES 10
        ...
        if (error_injection && rand() % 10000 < cct->_conf->rgw_sync_meta_inject_err_probability * 10000.0) {
        ...
        for (tries = 0; tries < NUM_TRANSIENT_ERROR_RETRIES; tries++) {
            ...
            if (sync_status < 0) {
                if (tries < NUM_TRANSIENT_ERROR_RETRIES - 1) {
                    ldpp_dout(dpp, 20) << *this << ": failed to fetch remote metadata entry: " << section << ":" << key
                                       << ", will retry" << dendl;
                    continue;
                }

                tn->log(10,
                        SSTR("failed to read remote metadata entry: section=" << section << " key=" << key
                                                                              << " status=" << sync_status));
                log_error() << "failed to read remote metadata entry: section=" << section << " key=" << key
                            << " status=" << sync_status << std::endl;
                yield call(sync_env->error_logger->log_error_cr(
                    dpp,
                    sync_env->conn->get_remote_id(),
                    section,
                    key,
                    -sync_status,
        ...
        for (tries = 0; tries < NUM_TRANSIENT_ERROR_RETRIES; tries++) {
            ...
            if ((retcode < 0) && (tries < NUM_TRANSIENT_ERROR_RETRIES - 1)) {
                ldpp_dout(dpp, 20) << *this << ": failed to store metadata entry: " << section << ":" << key
                                   << ", got retcode=" << retcode << ", will retry" << dendl;
                continue;
            }
            break;
        }
...
public:
    RGWCloneMetaLogCoroutine(RGWMetaSyncEnv* _sync_env, RGWMetadataLog* mdlog, const std::string& period, int _id,
...
public:
    RGWMetaSyncShardCR(RGWMetaSyncEnv* _sync_env, const rgw_pool& _pool, const std::string& period, epoch_t realm_epoch,
                       RGWMetadataLog* mdlog, uint32_t _shard_id, rgw_meta_sync_marker& _marker,
                       const std::string& period_marker, bool* _reset_backoff, RGWSyncTraceNodeRef& _tn)
        : RGWCoroutine(_sync_env->cct)
        , sync_env(_sync_env)
        , pool(_pool)
        , period(period)
    ...
    void collect_children()
    {
        ...
        while (collect_next(&child_ret, &child)) {
            ...
            if (child_ret < 0) {
                ldpp_dout(sync_env->dpp, 0) << *this << ": child operation stack=" << child << " entry=" << pos
                                            << " returned " << child_ret << dendl;
...
#define OMAP_GET_MAX_ENTRIES 100
        ...
        reenter(&full_cr)
        {
            ...
            *reset_backoff = true;
            ...
            set_marker_tracker(
                new RGWMetaSyncShardMarkerTrack(sync_env, sync_env->shard_obj_name(shard_id), sync_marker, tn));
            ...
            do {
                ...
                yield call(
                    new RGWRadosGetOmapKeysCR(sync_env->store, rgw_raw_obj(pool, oid), marker, max_entries, omapkeys));
                if (retcode < 0) {
                    ldpp_dout(sync_env->dpp, 0)
                        << "ERROR: " << __func__ << "(): RGWRadosGetOmapKeysCR() returned ret=" << retcode << dendl;
                    ...
                    if (!marker_tracker->start(marker, total_entries, real_time())) {
                        ...
                        yield
                        {
                            RGWCoroutinesStack* stack =
                                spawn(new RGWMetaSyncSingleEntryCR(
                                          sync_env, marker, marker, MDLOG_STATUS_COMPLETE, marker_tracker, tn),
                                      false);
            ...
            if (!lost_lock) {
                ...
                if (can_adjust_marker) {
                    ...
                    ldpp_dout(sync_env->dpp, 4) << *this << ": saving marker pos=" << temp_marker->marker
                                                << " realm_epoch=" << realm_epoch << dendl;

                    using WriteMarkerCR = RGWSimpleRadosWriteCR<rgw_meta_sync_marker>;
                    yield call(new WriteMarkerCR(sync_env->dpp,
                                                 sync_env->store,
                                                 rgw_raw_obj(pool, sync_env->shard_obj_name(shard_id)),
                                                 *temp_marker));
                }
    ...
    int incremental_sync()
    {
        reenter(&incremental_cr)
        {
            ...
            if (sync_marker.realm_epoch < realm_epoch) {
                ldpp_dout(sync_env->dpp, 4)
                    << "clearing marker=" << sync_marker.marker << " from old realm_epoch=" << sync_marker.realm_epoch
...
#define INCREMENTAL_MAX_ENTRIES 100
                ldpp_dout(sync_env->dpp, 20)
                    << __func__ << ":" << __LINE__ << ": shard_id=" << shard_id << " mdlog_marker=" << mdlog_marker
                    << " sync_marker.marker=" << sync_marker.marker << " period_marker=" << period_marker
                    << " truncated=" << truncated << dendl;
                if (!period_marker.empty() && period_marker <= mdlog_marker) {
                    tn->log(10,
                            SSTR("finished syncing current period: mdlog_marker="
                                 << mdlog_marker << " sync_marker=" << sync_marker.marker
                                 << " period_marker=" << period_marker));
                ...
                if (mdlog_marker <= max_marker || !truncated) {
                    ...
                    ldpp_dout(sync_env->dpp, 20) << __func__ << ":" << __LINE__ << ": shard_id=" << shard_id
                                                 << " syncing mdlog for shard_id=" << shard_id << dendl;
                    yield call(
                        new RGWCloneMetaLogCoroutine(sync_env, mdlog, period, shard_id, mdlog_marker, &mdlog_marker));
                }
                if (retcode < 0) {
                ...
                *reset_backoff = true; /* if we got to this point, all systems function */
                if (mdlog_marker > max_marker) {
                    ...
                    marker = max_marker;
                    yield call(new RGWReadMDLogEntriesCR(
                        sync_env, mdlog, shard_id, &max_marker, INCREMENTAL_MAX_ENTRIES, &log_entries, &truncated));
                    if (retcode < 0) {
                    ...
                    for (log_iter = log_entries.begin(); log_iter != log_entries.end() && !done_with_period;
                         ++log_iter) {
                        if (!period_marker.empty() && period_marker <= log_iter->id) {
                            done_with_period = true;
                            if (period_marker < log_iter->id) {
                                tn->log(10,
                                        SSTR("found key=" << log_iter->id << " past period_marker=" << period_marker));
                                break;
                            }
                        ...
                        if (!mdlog_entry.convert_from(*log_iter)) {
                            tn->log(
                                0,
                                SSTR("ERROR: failed to convert mdlog entry, shard_id="
                                     << shard_id << " log_entry: " << log_iter->id << ":" << log_iter->section << ":"
                                     << log_iter->name << ":" << log_iter->timestamp << " ... skipping entry"));
                            continue;
                        }
                        tn->log(20,
                                SSTR("log_entry: " << log_iter->id << ":" << log_iter->section << ":" << log_iter->name
                                                   << ":" << log_iter->timestamp));
                        if (!marker_tracker->start(log_iter->id, 0, log_iter->timestamp.to_real_time())) {
                            ldpp_dout(sync_env->dpp, 0)
                                << "ERROR: cannot start syncing " << log_iter->id << ". Duplicate entry?" << dendl;
                        }
                        else {
                ...
                ldpp_dout(sync_env->dpp, 20)
                    << __func__ << ":" << __LINE__ << ": shard_id=" << shard_id << " mdlog_marker=" << mdlog_marker
                    << " max_marker=" << max_marker << " sync_marker.marker=" << sync_marker.marker
                    << " period_marker=" << period_marker << dendl;
                if (done_with_period) {
...
public:
    RGWMetaSyncShardControlCR(RGWMetaSyncEnv* _sync_env, const rgw_pool& _pool, const std::string& period,
                              epoch_t realm_epoch, RGWMetadataLog* mdlog, uint32_t _shard_id,
                              const rgw_meta_sync_marker& _marker, std::string&& period_marker,
                              RGWSyncTraceNodeRef& _tn_parent)
        : RGWBackoffControlCR(_sync_env->cct, exit_on_error)
        , sync_env(_sync_env)
        , pool(_pool)
        , period(period)
        , realm_epoch(realm_epoch)
    ...
    {
        return new RGWMetaSyncShardCR(
            sync_env, pool, period, realm_epoch, mdlog, shard_id, sync_marker, period_marker, backoff_ptr(), tn);
    }

    RGWCoroutine* alloc_finisher_cr() override
    {
        rgw::sal::RadosStore* store = sync_env->store;
        return new RGWSimpleRadosReadCR<rgw_meta_sync_marker>(
            sync_env->dpp, store, rgw_raw_obj(pool, sync_env->shard_obj_name(shard_id)), &sync_marker);
    }
};
        ...
        reenter(this)
        {
            ...
            for (;;) {
                if (cursor == sync_env->store->svc()->mdlog->get_period_history()->get_current()) {
                    ...
                    if (cursor) {
                        ldpp_dout(dpp, 10)
                            << "RGWMetaSyncCR on current period=" << cursor.get_period().get_id() << dendl;
                    }
                    else {
                    ...
                    next = cursor;
                    next.next();
                    ldpp_dout(dpp, 10) << "RGWMetaSyncCR on period=" << cursor.get_period().get_id()
                                       << ", next=" << next.get_period().get_id() << dendl;
                }
                ...
                yield call(new RGWSimpleRadosWriteCR<rgw_meta_sync_info>(
                    dpp, sync_env->store, rgw_raw_obj(pool, sync_env->status_oid()), sync_status.sync_info));
            }
        }
        return 0;
    }
...
int RGWRemoteMetaLog::store_sync_info(const DoutPrefixProvider* dpp, const rgw_meta_sync_info& sync_info)
{
    ...
    return run(
        dpp,
        new RGWSimpleRadosWriteCR<rgw_meta_sync_info>(
            dpp, store, rgw_raw_obj(store->svc()->zone->get_zone_params().log_pool, sync_env.status_oid()), sync_info));
}

// return a cursor to the period at our sync position
static RGWPeriodHistory::Cursor get_period_at(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store,
                                              const rgw_meta_sync_info& info, optional_yield y)
{
    ...
    if (cursor) {
        ...
        if (existing != info.period) {
            ldpp_dout(dpp, -1) << "ERROR: sync status period=" << info.period << " does not match period=" << existing
    ...
    if (r < 0) {
        ldpp_dout(dpp, -1) << "ERROR: failed to read period id " << info.period << ": " << cpp_strerror(r) << dendl;
    ...
    if (!cursor) {
        r = cursor.get_error();
        ldpp_dout(dpp, -1) << "ERROR: failed to read period history back to " << info.period << ": " << cpp_strerror(r)
                           << dendl;
    }
    return cursor;
}
...
int RGWRemoteMetaLog::run_sync(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    do {
        ...
        if (!mdlog_info.period.empty()) {
            ...
            if (sync_status.sync_info.period.empty() || sync_status.sync_info.realm_epoch < mdlog_info.realm_epoch) {
                ...
                if (sync_status.sync_info.period.empty()) {
                    ...
                    reason = SSTR("sync_info realm epoch is behind: " << sync_status.sync_info.realm_epoch << " < "
                                                                      << mdlog_info.realm_epoch);
                }
    ...
    if (num_shards != mdlog_info.num_shards) {
        ldpp_dout(dpp, -1) << "ERROR: can't sync, mismatch between num shards, master num_shards="
                           << mdlog_info.num_shards << " local num_shards=" << num_shards << dendl;
        return -EINVAL;
    }
    ...
    do {
        ...
        case rgw_meta_sync_info::StateSync:
            ...
            break;
        default: tn->log(0, "ERROR: bad sync state!"); return -EIO;
        }
    } while (!going_down);
...
int RGWCloneMetaLogCoroutine::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        do {
            ...
            yield
            {
                ldpp_dout(dpp, 20) << __func__ << ": shard_id=" << shard_id << ": reading shard status complete"
                                   << dendl;
...
int RGWCloneMetaLogCoroutine::state_read_shard_status()
{
    ...
    completion.reset(new RGWMetadataLogInfoCompletion([this](int ret, const cls_log_header& header) {
                         if (ret < 0) {
                             if (ret != -ENOENT) {
                                 ldpp_dout(sync_env->dpp, 1)
                                     << "ERROR: failed to read mdlog info with " << cpp_strerror(ret) << dendl;
                             }
                         }
                         else {
...
int RGWCloneMetaLogCoroutine::state_read_shard_status_complete()
{
    ...
    ldpp_dout(sync_env->dpp, 20) << "shard_id=" << shard_id << " marker=" << shard_info.marker
                                 << " last_update=" << shard_info.last_update << dendl;
...
int RGWCloneMetaLogCoroutine::state_receive_rest_response()
{
    ...
    if (ret < 0) {
        error_stream << "http operation failed: " << http_op->to_str() << " status=" << http_op->get_http_status()
                     << std::endl;
    ...
    ldpp_dout(sync_env->dpp, 20) << "remote mdlog, shard_id=" << shard_id
                                 << " num of shard entries: " << data.entries.size() << dendl;
...
int RGWCloneMetaLogCoroutine::state_store_mdlog_entries()
{
    ...
    if (ret < 0) {
        cn->put();
        ldpp_dout(sync_env->dpp, 10) << "failed to store md log entries shard_id=" << shard_id << " ret=" << ret
                                     << dendl;
        return set_cr_error(ret);
    }
    return io_block(0);
...
void rgw_meta_sync_info::dump(Formatter* f) const
{
    ...
    case StateBuildingFullSyncMaps: s = "building-full-sync-maps"; break;
    case StateSync: s = "sync"; break;
