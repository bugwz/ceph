...
void RGWOp_Period_Base::send_response()
{
    ...
    if (op_ret < 0) {
        if (!s->err.message.empty()) {
            ldpp_dout(this, 4) << "Request failed with " << op_ret << ": " << s->err.message << dendl;
        }
        end_header(s);
        return;
    }
...
public:
    ...
    int check_caps(const RGWUserCaps& caps) override { return caps.check_cap("zone", RGW_CAP_READ); }
...
void RGWOp_Period_Get::execute(optional_yield y)
{
    ...
    op_ret = period.init(
        this, driver->ctx(), static_cast<rgw::sal::RadosStore*>(driver)->svc()->sysobj, realm_id, y, realm_name);
    if (op_ret < 0) ldpp_dout(this, 5) << "failed to read period" << dendl;
}
...
public:
    void execute(optional_yield y) override;
    int check_caps(const RGWUserCaps& caps) override { return caps.check_cap("zone", RGW_CAP_WRITE); }
    int verify_permission(optional_yield) override { return check_caps(s->user->get_caps()); }
...
void RGWOp_Period_Post::execute(optional_yield y)
{
    ...
    if (period.get_realm() != static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->get_realm().get_id()) {
        error_stream << "period with realm id " << period.get_realm() << " doesn't match current realm "
                     << static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->get_realm().get_id() << std::endl;
        op_ret = -EINVAL;
        return;
    }
    ...
    if (op_ret < 0) {
        ldpp_dout(this, -1) << "failed to read current realm: " << cpp_strerror(-op_ret) << dendl;
        return;
    }

    RGWPeriod current_period;
    op_ret =
        current_period.init(this, cct, static_cast<rgw::sal::RadosStore*>(driver)->svc()->sysobj, realm.get_id(), y);
    if (op_ret < 0) {
        ldpp_dout(this, -1) << "failed to read current period: " << cpp_strerror(-op_ret) << dendl;
        return;
    }
    ...
    if (period.get_master_zone() ==
        static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone->get_zone_params().get_id()) {
        ldpp_dout(this, 10) << "master zone rejecting period id=" << period.get_id() << " epoch=" << period.get_epoch()
                            << dendl;
    ...
    if (op_ret == -EEXIST) {
        ...
        ldpp_dout(this, 4) << "already have epoch >= " << period.get_epoch() << " for period " << period.get_id()
                           << dendl;
        op_ret = 0;
        return;
    }
    ...
    if (period.get_id() != current_period.get_id()) {
        ...
        if (period.get_realm_epoch() < current_epoch) {
            ldpp_dout(this, 10) << "discarding period " << period.get_id() << " with realm epoch "
                                << period.get_realm_epoch() << " older than current epoch " << current_epoch << dendl;
        ...
        if (period.get_realm_epoch() > current_epoch + PERIOD_HISTORY_FETCH_MAX) {
            ldpp_dout(this, -1) << "discarding period " << period.get_id() << " with realm epoch "
                                << period.get_realm_epoch()
                                << " too far in "
                                   "the future from current epoch "
                                << current_epoch << dendl;
        ...
        if (!cursor) {
            ...
            ldpp_dout(this, -1) << "failed to collect the periods between current period " << current_period.get_id()
                                << " (realm epoch " << current_epoch << ") and the new period " << period.get_id()
                                << " (realm epoch " << period.get_realm_epoch() << "): " << cpp_strerror(-op_ret)
                                << dendl;
            return;
        }
        if (cursor.has_next()) {
        ...
        ldpp_dout(this, 4) << "period " << period.get_id() << " is newer than current period "
                           << current_period.get_id() << ", updating realm's current period and notifying zone"
                           << dendl;
    ...
    if (op_ret < 0) {
        ldpp_dout(this, -1) << "failed to update local objects: " << cpp_strerror(-op_ret) << dendl;
        return;
    }
    ldpp_dout(this, 4) << "period epoch " << period.get_epoch() << " is newer than current epoch "
                       << current_period.get_epoch() << ", updating period's latest epoch and notifying zone" << dendl;
    realm.notify_new_period(this, period, y);
...
{
public:
    RGWHandler_REST* get_handler(rgw::sal::Driver* driver, req_state*, const rgw::auth::StrategyRegistry& auth_registry,
...
void RGWOp_Realm_Get::execute(optional_yield y)
{
    ...
    if (op_ret < 0) ldpp_dout(this, -1) << "failed to read realm id=" << id << " name=" << name << dendl;
}
...
void RGWOp_Realm_List::execute(optional_yield y)
{
    ...
    if (op_ret < 0) ldpp_dout(this, -1) << "failed to list realms" << dendl;
}
...
protected:
    ...
    {
        if (s->info.args.sub_resource_exists("list")) return new RGWOp_Realm_List;
...
RGWHandler_REST* RGWRESTMgr_Realm::get_handler(rgw::sal::Driver* driver, req_state*,
                                               const rgw::auth::StrategyRegistry& auth_registry, const std::string&)
{
