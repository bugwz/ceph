...
{
    switch (t) {
    case shard_check::dne: return m << "shard_check::dne";
    case shard_check::omap: return m << "shard_check::omap";
    case shard_check::fifo: return m << "shard_check::fifo";
    case shard_check::corrupt: return m << "shard_check::corrupt";
    }
...
shard_check probe_shard(const DoutPrefixProvider* dpp, librados::IoCtx& ioctx, const std::string& oid,
                        bool& fifo_unsupported, optional_yield y)
{
    ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " probing oid=" << oid << dendl;
    if (!fifo_unsupported) {
        std::unique_ptr<rgw::cls::fifo::FIFO> fifo;
        auto r = rgw::cls::fifo::FIFO::open(dpp, ioctx, oid, &fifo, y, std::nullopt, true);
        switch (r) {
        case 0:
            ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": oid=" << oid << " is FIFO" << dendl;
            return shard_check::fifo;

        case -ENODATA:
            ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": oid=" << oid
                               << " is empty and therefore OMAP" << dendl;
            return shard_check::omap;

        case -ENOENT:
            ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": oid=" << oid << " does not exist"
                               << dendl;
            return shard_check::dne;

        case -EPERM:
            ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": FIFO is unsupported, marking." << dendl;
            fifo_unsupported = true;
            return shard_check::omap;

        default:
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": error probing: r=" << r
                               << ", oid=" << oid << dendl;
...
tl::expected<log_type, bs::error_code> handle_dne(const DoutPrefixProvider* dpp, librados::IoCtx& ioctx, log_type def,
                                                  std::string oid, bool fifo_unsupported, optional_yield y)
{
    if (def == log_type::fifo) {
        ...
        std::unique_ptr<rgw::cls::fifo::FIFO> fifo;
        auto r = rgw::cls::fifo::FIFO::create(dpp, ioctx, oid, &fifo, y, std::nullopt);
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " error creating FIFO: r=" << r
                               << ", oid=" << oid << dendl;
...
tl::expected<log_type, bs::error_code> log_backing_type(const DoutPrefixProvider* dpp, librados::IoCtx& ioctx,
                                                        log_type def, int shards,
                                                        ...
                                                        optional_yield y)
{
    ...
    for (int i = 0; i < shards; ++i) {
        ...
        if (c == shard_check::corrupt) return tl::unexpected(bs::error_code(EIO, bs::system_category()));
        ...
        if (check != c) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " clashing types: check=" << check
                               << ", c=" << c << dendl;
            return tl::unexpected(bs::error_code(EIO, bs::system_category()));
        }
    }
    if (check == shard_check::corrupt) {
        ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " should be unreachable!" << dendl;
        return tl::unexpected(bs::error_code(EIO, bs::system_category()));
    }

    if (check == shard_check::dne) return handle_dne(dpp, ioctx, def, get_oid(0), fifo_unsupported, y);

    return (check == shard_check::fifo ? log_type::fifo : log_type::omap);
}

bs::error_code log_remove(const DoutPrefixProvider* dpp, librados::IoCtx& ioctx, int shards,
                          const fu2::unique_function<std::string(int) const>& get_oid, bool leave_zero,
                          optional_yield y)
{
    bs::error_code ec;
    for (int i = 0; i < shards; ++i) {
        ...
        auto r = rgw::cls::fifo::get_meta(
            dpp, ioctx, oid, std::nullopt, &info, &part_header_size, &part_entry_overhead, 0, y, true);
        ...
        if (r == 0 && info.head_part_num > -1) {
            for (auto j = info.tail_part_num; j <= info.head_part_num; ++j) {
                ...
                if (subr < 0 && subr != -ENOENT) {
                    if (!ec) ec = bs::error_code(-subr, bs::system_category());
                    ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
                                       << ": failed removing FIFO part: part_oid=" << part_oid << ", subr=" << subr
                                       << dendl;
                }
            }
        }
        if (r < 0 && r != -ENODATA) {
            if (!ec) ec = bs::error_code(-r, bs::system_category());
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": failed checking FIFO part: oid=" << oid
                               << ", r=" << r << dendl;
        }
        librados::ObjectWriteOperation op;
        if (i == 0 && leave_zero) {
        ...
        if (r < 0 && r != -ENOENT) {
            if (!ec) ec = bs::error_code(-r, bs::system_category());
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": failed removing shard: oid=" << oid
                               << ", r=" << r << dendl;
        }
    }
    return ec;
...
logback_generations::~logback_generations()
{
    if (watchcookie > 0) {
        ...
        if (r < 0) {
            lderr(cct) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": failed unwatching oid=" << oid << ", r=" << r
                       << dendl;
        }
    }
}

bs::error_code logback_generations::setup(const DoutPrefixProvider* dpp, log_type def, optional_yield y) noexcept
{
    try {
        // First, read.
        auto cct = static_cast<CephContext*>(ioctx.cct());
        ...
        if (res) {
            ...
            // metadata.
            librados::ObjectWriteOperation op;
            auto type = log_backing_type(
                dpp, ioctx, def, shards, [this](int shard) { return this->get_oid(0, shard); }, y);
            ...
            if (r < 0 && r != -EEXIST) {
                ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": failed writing oid=" << oid
                                   << ", r=" << r << dendl;
            ...
            if (r != 0) {
                ...
                if (res->first.empty()) return bs::error_code(EIO, bs::system_category());
                ...
                if (l.gen_id != 0) {
                    auto ec = log_remove(
                        dpp, ioctx, shards, [this](int shard) { return this->get_oid(0, shard); }, true, y);
                    if (ec) return ec;
                }
                std::unique_lock lock(m);
                std::tie(entries_, version) = std::move(*res);
        ...
        entries_t e;
        std::copy(i, entries_.cend(), std::inserter(e, e.end()));
        m.unlock();
        auto ec = watch();
        if (ec) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
                               << ": failed to re-establish watch, unsafe to continue: oid=" << oid
                               << ", ec=" << ec.message() << dendl;
        }
...
bs::error_code logback_generations::update(const DoutPrefixProvider* dpp, optional_yield y) noexcept
{
    try {
        ...
        if (es.empty()) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": INCONSISTENCY! Read empty update."
                               << dendl;
        ...
        if (new_lowest->first < cur_lowest->first) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": INCONSISTENCY! Tail moved wrong way."
                               << dendl;
            return bs::error_code(EFAULT, bs::system_category());
        }
        ...
        if ((es.end() - 1)->first < (entries_.end() - 1)->first) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": INCONSISTENCY! Head moved wrong way."
                               << dendl;
            return bs::error_code(EFAULT, bs::system_category());
        }
    ...
    -> tl::expected<std::pair<entries_t, obj_version>, bs::error_code>
{
    try {
        ...
        if (r < 0) {
            if (r == -ENOENT) {
                ldpp_dout(dpp, 5) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": oid=" << oid << " not found" << dendl;
            }
            else {
                ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": failed reading oid=" << oid
                                   << ", r=" << r << dendl;
            }
            return tl::unexpected(bs::error_code(-r, bs::system_category()));
        }
...
bs::error_code logback_generations::write(const DoutPrefixProvider* dpp, entries_t&& e,
                                          std::unique_lock<std::mutex>&& l_, optional_yield y) noexcept
{
    auto l = std::move(l_);
    ceph_assert(l.mutex() == &m && l.owns_lock());
    try {
        librados::ObjectWriteOperation op;
        cls_version_check(op, version, VER_COND_GE);
        cb::list bl;
...
bs::error_code logback_generations::watch() noexcept
{
    try {
        ...
        if (r < 0) {
            lderr(cct) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": failed to set watch oid=" << oid << ", r=" << r
                       << dendl;
            return {-r, bs::system_category()};
        }
    }
...
bs::error_code logback_generations::new_backing(const DoutPrefixProvider* dpp, log_type type, optional_yield y) noexcept
{
    ...
    try {
        ...
        do {
            ...
            ++tries;
        } while (ec == bs::errc::operation_canceled && tries < max_tries);
        if (tries >= max_tries) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": exhausted retry attempts." << dendl;
            return ec;
        }

        if (ec) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": write failed with ec=" << ec.message()
                               << dendl;
            return ec;
        }
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": notify failed with r=" << r << dendl;
            return {-r, bs::system_category()};
        }
...
bs::error_code logback_generations::empty_to(const DoutPrefixProvider* dpp, uint64_t gen_id, optional_yield y) noexcept
{
    static constexpr auto max_tries = 10;
    try {
        auto ec = update(dpp, y);
...
bs::error_code logback_generations::remove_empty(const DoutPrefixProvider* dpp, optional_yield y) noexcept
{
    ...
    try {
        ...
        l.unlock();
        do {
            std::copy_if(entries_.cbegin(), entries_.cend(), std::inserter(es, es.end()), [now](const auto& e) {
                if (!e.second.pruned) return false;
            ...
            for (const auto& [gen_id, e] : es) {
                ...
                auto ec = log_remove(
                    ...
                    ioctx,
                    shards,
                    [this, gen_id = gen_id](int shard) { return this->get_oid(gen_id, shard); },
                    (gen_id == 0),
                    y);
                if (ec) {
                    ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": Error pruning: gen_id=" << gen_id
                                       << " ec=" << ec.message() << dendl;
                }
...
void logback_generations::handle_notify(uint64_t notify_id, uint64_t cookie, uint64_t notifier_id, bufferlist& bl)
{
    auto cct = static_cast<CephContext*>(ioctx.cct());
    ...
    if (notifier_id != my_id) {
        ...
        if (ec) {
            lderr(cct) << __PRETTY_FUNCTION__ << ":" << __LINE__
                       << ": update failed, no one to report to and no safe way to continue." << dendl;
            abort();
        }
    }
    cb::list rbl;
...
void logback_generations::handle_error(uint64_t cookie, int err)
{
    ...
    auto r = ioctx.unwatch2(watchcookie);
    if (r < 0) {
        lderr(cct) << __PRETTY_FUNCTION__ << ":" << __LINE__ << ": failed to set unwatch oid=" << oid << ", r=" << r
                   << dendl;
    }
    ...
    if (ec) {
        lderr(cct) << __PRETTY_FUNCTION__ << ":" << __LINE__
                   << ": failed to re-establish watch, unsafe to continue: oid=" << oid << ", ec=" << ec.message()
                   << dendl;
    }
}
