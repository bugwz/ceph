...
struct RGWLCCloudTierCtx
{
    ...
    RGWLCCloudTierCtx(CephContext* _cct, const DoutPrefixProvider* _dpp, rgw_bucket_dir_entry& _o,
                      rgw::sal::Driver* _driver, RGWBucketInfo& _binfo, rgw::sal::Object* _obj, RGWRESTConn& _conn,
                      std::string& _bucket, std::string& _storage_class)
        : cct(_cct)
        , dpp(_dpp)
        , o(_o)
        , driver(_driver)
