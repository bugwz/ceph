...
struct rgw_lc_obj_properties
{
    ...
    rgw_lc_obj_properties(ceph::real_time _mtime, std::string _etag, uint64_t _versioned_epoch,
                          std::map<std::string, RGWTierACLMapping>& _t_acl_mappings, std::string _t_storage_class)
        : mtime(_mtime)
        , etag(_etag)
...
static int read_upload_status(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, const rgw_raw_obj* status_obj,
                              rgw_lc_multipart_upload_info* status)
{
    ...
    ret = rgw_get_system_obj(sysobj, pool, oid, bl, nullptr, nullptr, null_yield, dpp);
    ...
    if (bl.length() > 0) {
        try {
            ...
            ldpp_dout(dpp, 10) << "failed to decode status obj: " << e.what() << dendl;
            return -EIO;
        }
    }
    else {
...
static int put_upload_status(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, const rgw_raw_obj* status_obj,
                             rgw_lc_multipart_upload_info* status)
{
    int ret = 0;
    rgw::sal::RadosStore* rados = dynamic_cast<rgw::sal::RadosStore*>(driver);
    ...
    ret = rgw_put_system_obj(dpp, sysobj, pool, oid, bl, true, nullptr, real_time{}, null_yield);

    return ret;
}

static int delete_upload_status(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, const rgw_raw_obj* status_obj)
{
    int ret = 0;
    rgw::sal::RadosStore* rados = dynamic_cast<rgw::sal::RadosStore*>(driver);
...
static std::set<string> keep_headers = {"CONTENT_TYPE", "CONTENT_ENCODING", "CONTENT_DISPOSITION", "CONTENT_LANGUAGE"};
...
static void init_headers(map<string, bufferlist>& attrs, map<string, string>& headers)
{
    for (auto& kv : attrs) {
 ...
 */
static int cloud_tier_get_object(RGWLCCloudTierCtx& tier_ctx, bool head, std::map<std::string, std::string>& headers)
{
    ...
    target_obj_name = tier_ctx.bucket_info.bucket.name + "/" + tier_ctx.obj->get_name();
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(tier_ctx.dpp, 20) << "ERROR: " << __func__ << "(): conn.complete_request() returned ret=" << ret
                                    << dendl;
        return ret;
    }
    return 0;
                              ...
                              ceph::real_time& mtime)
{
    ...
    utime_t ut(mtime);
    snprintf(buf, sizeof(buf), "%lld.%09lld", (long long)ut.sec(), (long long)ut.nsec());

    string s = attrs["X_AMZ_META_RGWX_SOURCE_MTIME"];

    if (s.empty()) s = attrs["x_amz_meta_rgwx_source_mtime"];
...
public:
    RGWLCStreamRead(CephContext* _cct, const DoutPrefixProvider* _dpp, rgw::sal::Object* _obj, const real_time& _mtime)
        : cct(_cct)
        , dpp(_dpp)
        , obj(_obj)
        , mtime(_mtime)
...
public:
    RGWLCCloudStreamPut(const DoutPrefixProvider* _dpp, const rgw_lc_obj_properties& _obj_properties,
                        RGWRESTConn& _conn, const rgw_obj& _dest_obj)
        : dpp(_dpp)
    ...
    static void init_send_attrs(const DoutPrefixProvider* dpp, const rgw_rest_obj& rest_obj,
                                const rgw_lc_obj_properties& obj_properties, std::map<std::string, std::string>& attrs);
...
int RGWLCCloudStreamPut::init()
{
    ...
    if (multipart.is_multipart) {
        ...
        rgw_http_param_pair params[] = {
            {"uploadId", multipart.upload_id.c_str()}, {"partNumber", buf}, {nullptr, nullptr}};
...
void RGWLCCloudStreamPut::init_send_attrs(const DoutPrefixProvider* dpp, const rgw_rest_obj& rest_obj,
                                          const rgw_lc_obj_properties& obj_properties, std::map<string, string>& attrs)
{
    ...
    if (!acl_mappings.empty()) {
        for (auto& grant : acl.get_grant_map()) {
            ...
            case ACL_TYPE_CANON_USER: type = "id"; break;
            case ACL_TYPE_EMAIL_USER: type = "emailAddress"; break;
            case ACL_TYPE_GROUP: type = "uri"; break;
            default: continue;
            }
    ...
    for (const auto& aiter : access_map) {
        ...
        switch (grant_type) {
        case RGW_PERM_READ: header_str.append("read"); break;
        case RGW_PERM_WRITE: header_str.append("write"); break;
        case RGW_PERM_READ_ACP: header_str.append("read-acp"); break;
        case RGW_PERM_WRITE_ACP: header_str.append("write-acp"); break;
        case RGW_PERM_FULL_CONTROL: header_str.append("full-control"); break;
        }
...
static int cloud_tier_transfer_object(const DoutPrefixProvider* dpp, RGWLCStreamRead* readf,
...
static int cloud_tier_plain_transfer(RGWLCCloudTierCtx& tier_ctx)
{
    ...
    readf.reset(new RGWLCStreamRead(tier_ctx.cct, tier_ctx.dpp, tier_ctx.obj, tier_ctx.o.meta.mtime));

    std::shared_ptr<RGWLCCloudStreamPut> writef;
    writef.reset(new RGWLCCloudStreamPut(tier_ctx.dpp, obj_properties, tier_ctx.conn, dest_obj));
...
static int cloud_tier_send_multipart_part(RGWLCCloudTierCtx& tier_ctx, const std::string& upload_id,
                                          const rgw_lc_multipart_part_info& part_info, std::string* petag)
{
    int ret;
...
static int cloud_tier_abort_multipart(const DoutPrefixProvider* dpp, RGWRESTConn& dest_conn, const rgw_obj& dest_obj,
                                      const std::string& upload_id)
{
    ...
    ret = dest_conn.send_resource(dpp, "DELETE", resource, params, nullptr, out_bl, &bl, nullptr, null_yield);


    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to abort multipart upload for dest object=" << dest_obj << " (ret=" << ret
                          << ")" << dendl;
        return ret;
    }
...
static int cloud_tier_init_multipart(const DoutPrefixProvider* dpp, RGWRESTConn& dest_conn, const rgw_obj& dest_obj,
                                     ...
                                     std::string& upload_id)
{
    ...
    ret = dest_conn.send_resource(dpp, "POST", resource, params, &attrs, out_bl, &bl, nullptr, null_yield);
    ...
    if (!parser.init()) {
        ldpp_dout(dpp, 0) << "ERROR: failed to initialize xml parser for parsing multipart init response from server"
                          << dendl;
        return -EIO;
    }
    ...
    ldpp_dout(dpp, 20) << "init multipart result: bucket=" << result.bucket << " key=" << result.key
                       << " upload_id=" << result.upload_id << dendl;
...
static int cloud_tier_complete_multipart(const DoutPrefixProvider* dpp, RGWRESTConn& dest_conn, const rgw_obj& dest_obj,
                                         std::string& upload_id, const std::map<int, rgw_lc_multipart_part_info>& parts)
{
    rgw_http_param_pair params[] = {{"uploadId", upload_id.c_str()}, {nullptr, nullptr}};
    ...
    struct CompleteMultipartReq
    {
        ...
        explicit CompleteMultipartReq(const std::map<int, rgw_lc_multipart_part_info>& _parts)
            : parts(_parts)
        {}
    ...
    ret = dest_conn.send_resource(dpp, "POST", resource, params, nullptr, out_bl, &bl, nullptr, null_yield);
    ...
    ldpp_dout(dpp, 20) << "complete multipart result: location=" << result.location << " bucket=" << result.bucket
                       << " key=" << result.key << " etag=" << result.etag << dendl;

    return ret;
}

static int cloud_tier_abort_multipart_upload(RGWLCCloudTierCtx& tier_ctx, const rgw_obj& dest_obj,
                                             const rgw_raw_obj& status_obj, const std::string& upload_id)
{
    int ret;
    ...
    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to abort multipart upload dest obj=" << dest_obj
    ...
    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to remove sync status obj obj=" << status_obj << " ret=" << ret
                                   << dendl;
        // ignore error, best effort
    }
    return 0;
...
static int cloud_tier_multipart_transfer(RGWLCCloudTierCtx& tier_ctx)
{
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to read sync status of object " << src_obj << " ret=" << ret
                                   << dendl;
        return ret;
    }
    ...
    if (ret >= 0) {
        ...
        if (status.mtime != obj_properties.mtime || status.obj_size != obj_size || status.etag != obj_properties.etag) {
...
#define MULTIPART_MAX_PARTS 10000
    ...
    for (; (uint32_t)cur_part <= num_parts; ++cur_part) {
        ldpp_dout(tier_ctx.dpp, 20) << "cur_part = " << cur_part << ", info.ofs = " << cur_ofs
        ...
        ret = cloud_tier_send_multipart_part(tier_ctx, status.upload_id, cur_part_info, &cur_part_info.etag);

        if (ret < 0) {
            ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to send multipart part of obj=" << tier_ctx.obj
                                       << ", sync via multipart upload, upload_id=" << status.upload_id
                                       << " part number " << cur_part << " (error: " << cpp_strerror(-ret) << ")"
                                       << dendl;
            cloud_tier_abort_multipart_upload(tier_ctx, dest_obj, status_obj, status.upload_id);
            return ret;
        }
    ...
    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to complete multipart upload of obj=" << tier_ctx.obj
    ...
    ret = delete_upload_status(tier_ctx.dpp, tier_ctx.driver, &status_obj);
    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to abort multipart upload obj=" << tier_ctx.obj
                                   << " upload_id=" << status.upload_id << " part number " << cur_part << " ("
...
static int cloud_tier_check_object(RGWLCCloudTierCtx& tier_ctx, bool& already_tiered)
{
    ...
    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to fetch HEAD from cloud for obj=" << tier_ctx.obj
                                   << " , ret = " << ret << dendl;
        return ret;
    }
...
static int cloud_tier_create_bucket(RGWLCCloudTierCtx& tier_ctx)
{
    ...
    struct CreateBucketResult
    {
        ...
        void decode_xml(XMLObj* obj) { RGWXMLDecoder::decode_xml("Code", code, obj); }
    } result;
    ...
    ret =
        tier_ctx.conn.send_resource(tier_ctx.dpp, "PUT", resource, nullptr, nullptr, out_bl, &bl, nullptr, null_yield);

    if (ret < 0) {
        ldpp_dout(tier_ctx.dpp, 0) << "create target bucket : " << tier_ctx.target_bucket_name
                                   << " returned ret:" << ret << dendl;
    }
    if (out_bl.length() > 0) {
        RGWXMLDecoder::XMLParser parser;
        if (!parser.init()) {
            ldpp_dout(tier_ctx.dpp, 0)
                << "ERROR: failed to initialize xml parser for parsing create_bucket response from server" << dendl;
            return -EIO;
        }
...
int rgw_cloud_tier_transfer_object(RGWLCCloudTierCtx& tier_ctx, std::set<std::string>& cloud_targets)
{
    ...
    if (!tier_ctx.target_bucket_created) {
        ...
        if (ret < 0) {
            ldpp_dout(tier_ctx.dpp, 0) << "ERROR: failed to create target bucket on the cloud endpoint ret=" << ret
                                       << dendl;
            return ret;
        }
