...
#define ldout_bitx(_bitx, _dpp, _level) \
    if (_bitx) {                        \
        ldpp_dout(_dpp, 0) << "BITX: "
#define ldout_bitx_c(_bitx, _ctx, _level) \
    if (_bitx) {                          \
        ldout(_ctx, 0) << "BITX: "
#define dendl_bitx \
    dendl;         \
...
public:
    RGWMetaNotifierManager(RGWRados* _driver)
        : RGWCoroutinesManager(_driver->ctx(), _driver->get_cr_registry())
        , store(_driver)
    ...
    {
        rgw_http_param_pair pairs[] = {{"type", "metadata"}, {"notify", NULL}, {NULL, NULL}};

        list<RGWCoroutinesStack*> stacks;
        for (auto iter = conn_map.begin(); iter != conn_map.end(); ++iter) {
            RGWRESTConn* conn = iter->second;
            RGWCoroutinesStack* stack = new RGWCoroutinesStack(store->ctx(), this);
            stack->call(new RGWPostRESTResourceCR<set<int>, int>(
                store->ctx(), conn, &http_manager, "/admin/log", pairs, shards, NULL));
...
public:
    RGWDataNotifierManager(RGWRados* _driver)
        : RGWCoroutinesManager(_driver->ctx(), _driver->get_cr_registry())
        , store(_driver)
        , http_manager(store->ctx(), completion_mgr)
    {
...
void* RGWRadosThread::Worker::entry()
{
    ...
    do {
        ...
        if (cur_msec > 0) {
            if (interval <= end) continue;   // next round
...
class RGWMetaNotifier : public RGWRadosThread
{
    ...
    uint64_t interval_msec() override { return cct->_conf->rgw_md_notify_interval_msec; }
    void stop_process() override { notify_mgr.stop(); }

public:
    RGWMetaNotifier(RGWRados* _driver, RGWMetadataLog* log)
        : RGWRadosThread(_driver, "meta-notifier")
        , notify_mgr(_driver)
        , log(log)
    {}
...
class RGWDataNotifier : public RGWRadosThread
{
    ...
    uint64_t interval_msec() override { return cct->_conf.get_val<int64_t>("rgw_data_notify_interval_msec"); }
    void stop_process() override { notify_mgr.stop(); }

public:
    RGWDataNotifier(RGWRados* _driver)
        : RGWRadosThread(_driver, "data-notifier")
        , notify_mgr(_driver)
    {}
...
int RGWDataNotifier::process(const DoutPrefixProvider* dpp)
{
    ...
    for (const auto& [shard_id, entries] : shards) {
        ...
        for (const auto& entry : entries) {
            ldpp_dout(dpp, 20) << __func__ << "(): notifying datalog change, shard_id=" << shard_id << ":" << entry.gen
                               << ":" << entry.key << dendl;
        }
    }
...
{
public:
    RGWSyncProcessorThread(RGWRados* _driver, const string& thread_name = "radosgw")
        : RGWRadosThread(_driver, thread_name)
    {}
    RGWSyncProcessorThread(RGWRados* _driver)
        : RGWRadosThread(_driver)
    {}
...
class RGWMetaSyncProcessorThread : public RGWSyncProcessorThread
{
    ...
    uint64_t interval_msec() override { return 0; /* no interval associated, it'll run once until stopped */ }
    void stop_process() override { sync.stop(); }

public:
    RGWMetaSyncProcessorThread(rgw::sal::RadosStore* _driver, RGWAsyncRadosProcessor* async_rados)
        : RGWSyncProcessorThread(_driver->getRados(), "meta-sync")
        , sync(_driver, async_rados)
    {}
...
public:
    ...
    {
        for (bc::flat_map<int, bc::flat_set<rgw_data_notify_entry>>::iterator iter = entries.begin();
             iter != entries.end();
             ++iter) {
    ...
    int init(const DoutPrefixProvider* dpp) override { return 0; }
...
public:
    RGWSyncLogTrimThread(rgw::sal::RadosStore* store, rgw::BucketTrimManager* bucket_trim, int interval)
    ...
    int init(const DoutPrefixProvider* dpp) override { return http.start(); }
    int process(const DoutPrefixProvider* dpp) override
    {
        list<RGWCoroutinesStack*> stacks;
        auto metatrimcr = create_meta_log_trim_cr(
            this, static_cast<rgw::sal::RadosStore*>(store), &http, cct->_conf->rgw_md_log_max_shards, trim_interval);
        if (!metatrimcr) {
        ...
        if (store->svc()->zone->sync_module_exports_data()) {
            ...
            data->call(create_data_log_trim_cr(dpp,
    ...
    std::ostream& gen_prefix(std::ostream& out) const override { return out << "sync log trim: "; }
};
...
void RGWRados::wakeup_data_sync_shards(const DoutPrefixProvider* dpp, const rgw_zone_id& source_zone,
                                       bc::flat_map<int, bc::flat_set<rgw_data_notify_entry>>& entries)
{
    ldpp_dout(dpp, 20) << __func__ << ": source_zone=" << source_zone << ", entries=" << entries << dendl;
    for (bc::flat_map<int, bc::flat_set<rgw_data_notify_entry>>::iterator iter = entries.begin(); iter != entries.end();
         ++iter) {
        ...
        for (const auto& [key, gen] : entries) {
            ldpp_dout(dpp, 20) << __func__ << ": source_zone=" << source_zone << ", key=" << key << ", gen=" << gen
                               << dendl;
        }
    }
    ...
    if (iter == data_sync_processor_threads.end()) {
        ldpp_dout(dpp, 10) << __func__ << ": couldn't find sync thread for zone " << source_zone
...
int RGWRados::get_required_alignment(const DoutPrefixProvider* dpp, const rgw_pool& pool, uint64_t* alignment)
{
    ...
    if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: ioctx.pool_requires_alignment2() returned " << r << dendl;
        return r;
    }
    ...
    if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: ioctx.pool_required_alignment2() returned " << r << dendl;
        return r;
    }
    if (align != 0) {
...
int RGWRados::get_max_chunk_size(const rgw_pool& pool, uint64_t* max_chunk_size, const DoutPrefixProvider* dpp,
                                 uint64_t* palignment)
{
    uint64_t alignment;
...
int RGWRados::get_max_chunk_size(const rgw_placement_rule& placement_rule, const rgw_obj& obj, uint64_t* max_chunk_size,
                                 const DoutPrefixProvider* dpp, uint64_t* palignment)
{
    rgw_pool pool;
...
void add_datalog_entry(const DoutPrefixProvider* dpp, RGWDataChangesLog* datalog, const RGWBucketInfo& bucket_info,
...
class RGWIndexCompletionManager
{
    ...
    uint32_t next_shard() { return cur_shard++ % num_shards; }
...
public:
    RGWIndexCompletionManager(RGWRados* _driver)
        ...
        , locks{ceph::make_lock_container<ceph::mutex>(
              num_shards,
              [](const size_t i) { return ceph::make_mutex("RGWIndexCompletionManager::lock::" + std::to_string(i)); })}
        , completions(num_shards)
        , retry_thread(&RGWIndexCompletionManager::process, this)
    {}

    ~RGWIndexCompletionManager() { stop(); }

    void create_completion(const rgw_obj& obj, RGWModifyOp op, string& tag, rgw_bucket_entry_ver& ver,
                           const cls_rgw_obj_key& key, rgw_bucket_dir_entry_meta& dir_meta,
                           list<cls_rgw_obj_key>* remove_objs, bool log_op, uint16_t bilog_op,
                           rgw_zone_set* zones_trace, complete_op_data** result);

    bool handle_completion(completion_t cb, complete_op_data* arg);

    CephContext* ctx() { return store->ctx(); }
};
...
void RGWIndexCompletionManager::process()
{
    ...
    while (!_stop) {
        ...
        for (auto c : comps) {
            ...
            if (r < 0) {
                ldpp_dout(&dpp, 0) << "ERROR: " << __func__ << "(): failed to initialize BucketShard, obj=" << c->obj
                                   << " r=" << r << dendl;
                /* not much to do */
                continue;
            }

            r = store->guard_reshard(&dpp, &bs, c->obj, bucket_info, [&](RGWRados::BucketShard* bs) -> int {
                const bool bitx = ctx()->_conf->rgw_bucket_index_transaction_instrumentation;
                ldout_bitx(bitx, &dpp, 10)
                    << "ENTERING " << __func__ << ": bucket-shard=" << bs << " obj=" << c->obj << " tag=" << c->tag
                    << " op=" << c->op << ", remove_objs=" << c->remove_objs << dendl_bitx;
                ldout_bitx(bitx, &dpp, 25) << "BACKTRACE: " << __func__ << ": " << ClibBackTrace(1) << dendl_bitx;

                librados::ObjectWriteOperation o;
                o.assert_exists();
                cls_rgw_guard_bucket_resharding(o, -ERR_BUSY_RESHARDING);
                cls_rgw_bucket_complete_op(o,
                                           c->op,
                                           c->tag,
                                           c->ver,
                                           c->key,
                ...
                ldout_bitx(bitx, &dpp, 10) << "EXITING " << __func__ << ": ret=" << dendl_bitx;
                return ret;
            });
            if (r < 0) {
                ldpp_dout(&dpp, 0) << "ERROR: " << __func__ << "(): bucket index completion failed, obj=" << c->obj
                                   << " r=" << r << dendl;
            ...
            add_datalog_entry(&dpp, store->svc.datalog_rados, bucket_info, bs.shard_id, null_yield);
        }
    }
}

void RGWIndexCompletionManager::create_completion(const rgw_obj& obj, RGWModifyOp op, string& tag,
                                                  rgw_bucket_entry_ver& ver, const cls_rgw_obj_key& key,
                                                  rgw_bucket_dir_entry_meta& dir_meta,
                                                  list<cls_rgw_obj_key>* remove_objs, bool log_op, uint16_t bilog_op,
                                                  rgw_zone_set* zones_trace, complete_op_data** result)
{
...
bool RGWIndexCompletionManager::handle_completion(completion_t cb, complete_op_data* arg)
{
    ...
    if (r != -ERR_BUSY_RESHARDING) {
        ldout(arg->manager->ctx(), 20) << __func__ << "(): completion "
                                       << (r == 0 ? "ok" : "failed with " + to_string(r)) << " for obj=" << arg->key
                                       << dendl;
        return true;
    }
    add_completion(arg);
...
void RGWRados::finalize()
{
    ...
    if (d3n_data_cache) delete d3n_data_cache;
...
int RGWRados::init_rados()
{
    ...
    auto crs = std::unique_ptr<RGWCoroutinesManagerRegistry>{new RGWCoroutinesManagerRegistry(cct)};
...
int RGWRados::register_to_service_map(const DoutPrefixProvider* dpp, const string& daemon_type,
                                      const map<string, string>& meta)
{
    ...
    metadata["id"] = name;
    int ret = rados.service_daemon_register(daemon_type, stringify(rados.get_instance_id()), metadata);
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: service_daemon_register() returned ret=" << ret << ": " << cpp_strerror(-ret)
                          << dendl;
        return ret;
    }
...
int RGWRados::update_service_map(const DoutPrefixProvider* dpp, std::map<std::string, std::string>&& status)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: service_daemon_update_status() returned ret=" << ret << ": " << cpp_strerror(-ret)
                          << dendl;
        return ret;
    }
...
int RGWRados::init_complete(const DoutPrefixProvider* dpp)
{
    ...
    if (run_sync_thread) {
        for (const auto& pt : zonegroup.placement_targets) {
            if (zone_params.placement_pools.find(pt.second.name) == zone_params.placement_pools.end()) {
                ldpp_dout(dpp, 0) << "WARNING: This zone does not contain the placement target " << pt.second.name
    ...
    if (use_lc_thread) lc->start_processor();

    quota_handler = RGWQuotaHandler::generate_handler(dpp, this->driver, quota_threads);

    bucket_index_max_shards =
        (cct->_conf->rgw_override_bucket_index_max_shards ? cct->_conf->rgw_override_bucket_index_max_shards
    ...
    if (bucket_index_max_shards > get_max_bucket_shards()) {
        ...
        ldpp_dout(dpp, 1) << __func__
                          << " bucket index max shards is too large, reset to value: " << get_max_bucket_shards()
                          << dendl;
    }
...
{
    return rgw_init_ioctx(
        dpp, get_rados_handle(), svc.zone->get_zone_params().reshard_pool, reshard_pool_ctx, true, true);
}
...
int RGWRados::log_list_init(const DoutPrefixProvider* dpp, const string& prefix, RGWAccessHandle* handle)
{
    ...
    try {
        ...
        *handle = (RGWAccessHandle)state;
        ...
        r = -e.code().value();
        ldpp_dout(dpp, 10) << "nobjects_begin threw " << e.what() << ", returning " << r << dendl;
        return r;
    }
    catch (const std::exception& e) {
        ldpp_dout(dpp, 10) << "nobjects_begin threw " << e.what() << ", returning -5" << dendl;
        return -EIO;
    }
}
...
int RGWRados::log_list_next(RGWAccessHandle handle, string* name)
{
    ...
    while (true) {
        ...
        if (state->prefix.length() && state->obit->get_oid().find(state->prefix) != 0) {
            state->obit++;
            continue;
        }
...
int RGWRados::log_show_next(const DoutPrefixProvider* dpp, RGWAccessHandle handle, rgw_log_entry* entry)
{
    ...
    ldpp_dout(dpp, 10) << "log_show_next pos " << state->pos << " bl " << state->bl.length() << " off " << off
                       << " eof " << (int)state->eof << dendl;
    // read some?
    ...
    if ((state->bl.length() - off) < chunk / 2 && !state->eof) {
        ...
        if ((unsigned)r < chunk) state->eof = true;
        ldpp_dout(dpp, 10) << " read " << r << dendl;
    }

    if (state->p.end()) return 0;   // end of file
    try {
...
int RGWRados::log_usage(const DoutPrefixProvider* dpp, map<rgw_user_bucket, RGWUsageBatch>& usage_info)
{
    ...
    for (iter = usage_info.begin(); iter != usage_info.end(); ++iter) {
        ...
        if (ub.user.empty()) {
            ldpp_dout(dpp, 0) << "WARNING: RGWRados::log_usage(): user name empty (bucket=" << ub.bucket
                              << "), skipping" << dendl;
            continue;
        }
...
int RGWRados::read_usage(const DoutPrefixProvider* dpp, const rgw_user& user, const string& bucket_name,
                         uint64_t start_epoch, uint64_t end_epoch, uint32_t max_entries, bool* is_truncated,
                         RGWUsageIter& usage_iter, map<rgw_user_bucket, rgw_usage_log_entry>& usage)
{
    ...
    do {
        ...
        int ret = cls_obj_usage_log_read(dpp,
                                         hash,
                                         user_str,
                                         bucket_name,
                                         start_epoch,
                                         end_epoch,
                                         num,
                                         usage_iter.read_iter,
                                         ret_usage,
                                         is_truncated);
        if (ret == -ENOENT) goto next;
...
int RGWRados::trim_usage(const DoutPrefixProvider* dpp, const rgw_user& user, const string& bucket_name,
...
int RGWRados::decode_policy(const DoutPrefixProvider* dpp, ceph::buffer::list& bl, ACLOwner* owner)
{
    auto i = bl.cbegin();
 ...
 * max, then truncated.
 */
int RGWRados::Bucket::List::list_objects_ordered(const DoutPrefixProvider* dpp, int64_t max_p,
                                                 ...
                                                 optional_yield y)
{
    ...
    rgw_obj_key marker_obj(
        params.marker.name, params.marker.instance, params.ns.empty() ? params.marker.ns : params.ns);
    rgw_obj_index_key cur_marker;
    marker_obj.get_index_key(&cur_marker);

    rgw_obj_key end_marker_obj(
        params.end_marker.name, params.end_marker.instance, params.ns.empty() ? params.end_marker.ns : params.ns);
    ...
    for (uint16_t attempt = 1; /* empty */; ++attempt) {
        ldpp_dout(dpp, 20) << __func__ << ": starting attempt " << attempt << dendl;

        if (attempt > 1 && !(prev_marker < cur_marker)) {
            // we've failed to make forward progress
            ldpp_dout(dpp, 0) << "ERROR: " << __func__ << " marker failed to make forward progress; attempt=" << attempt
                              << ", prev_marker=" << prev_marker << ", cur_marker=" << cur_marker << dendl;
            break;
        }
        ...
        for (auto eiter = ent_map.begin(); eiter != ent_map.end(); ++eiter) {
            ...
            ldpp_dout(dpp, 20) << __func__ << ": considering entry " << entry.key << dendl;
            ...
            if (!valid) {
                ldpp_dout(dpp, 0) << "ERROR: " << __func__ << " could not parse object name: " << obj.name << dendl;
                continue;
            }

            bool matched_ns = (obj.ns == params.ns);
            if (!params.list_versions && !entry.is_visible()) {
                ldpp_dout(dpp, 10) << __func__ << ": skipping not visible entry \"" << entry.key << "\"" << dendl;
                continue;
            }
            ...
            if (params.enforce_ns && !matched_ns) {
                if (!params.ns.empty()) {
                    ...
                    truncated = false;
                    ldpp_dout(dpp, 10) << __func__ << ": finished due to getting past requested namespace \""
                                       << params.ns << "\"" << dendl;
                    goto done;
                }

                /* we're skipping past namespaced objects */
                ldpp_dout(dpp, 20) << __func__ << ": skipping past namespaced objects, including \"" << entry.key
                                   << "\"" << dendl;
                continue;
            }
            ...
            if (cur_end_marker_valid && cur_end_marker <= index_key) {
                truncated = false;
                ldpp_dout(dpp, 10) << __func__ << ": finished due to gitting end marker of \"" << cur_end_marker
            ...
            if (params.access_list_filter && !params.access_list_filter->filter(obj.name, index_key.name)) {
                ldpp_dout(dpp, 20) << __func__ << ": skipping past namespaced objects, including \"" << entry.key
                                   << "\"" << dendl;
                continue;
            }

            if (params.prefix.size() && 0 != obj.name.compare(0, params.prefix.size(), params.prefix)) {
                ldpp_dout(dpp, 20) << __func__ << ": skipping object \"" << entry.key
            ...
            if (!params.delim.empty()) {
                ...
                if (delim_pos >= 0) {
                    ...
                    if (cls_filtered) {
                        ...
                        // prefix
                        if (delim_pos != int(obj.name.length() - params.delim.length())) {
                            ldpp_dout(dpp, 0) << "WARNING: " << __func__
                                              << " found delimiter in place other than the end of "
                                                 "the prefix; obj.name="
                                              << obj.name << ", prefix=" << params.prefix << dendl;
                        }
                        if (common_prefixes) {
                            if (count >= max) {
                                truncated = true;
                                ldpp_dout(dpp, 10)
                                    << __func__ << ": stopping early with common prefix \"" << entry.key
                                    << "\" because requested number (" << max << ") reached (cls filtered)" << dendl;
                                goto done;
                            }
                        ...
                        ldpp_dout(dpp, 20) << __func__ << ": finished entry with common prefix \"" << entry.key
                        ...
                        string prefix_key = obj.name.substr(0, delim_pos + params.delim.length());

                        if (common_prefixes && common_prefixes->find(prefix_key) == common_prefixes->end()) {
                            if (count >= max) {
                                truncated = true;
                                ldpp_dout(dpp, 10) << __func__ << ": stopping early with common prefix \"" << entry.key
            ...
            if (count >= max) {
                truncated = true;
                ldpp_dout(dpp, 10) << __func__ << ": stopping early with entry \"" << entry.key
                                   << "\" because requested number (" << max << ") reached" << dendl;
                goto done;
            }

            ldpp_dout(dpp, 20) << __func__ << ": adding entry " << entry.key << " to result" << dendl;
        ...
        if (!cls_filtered && !params.delim.empty()) {
            ...
            if (marker_delim_pos >= 0) {
                std::string skip_after_delim = cur_marker.name.substr(0, marker_delim_pos);
                skip_after_delim.append(after_delim_s);

                ldpp_dout(dpp, 20) << __func__ << ": skip_after_delim=" << skip_after_delim << dendl;

                if (skip_after_delim > cur_marker.name) {
                    cur_marker = skip_after_delim;
                    ldpp_dout(dpp, 20) << __func__ << ": setting cur_marker=" << cur_marker.name << "["
                                       << cur_marker.instance << "]" << dendl;
                }
            }
        }   // if older osd didn't do delimiter filtering

        ldpp_dout(dpp, 10) << __func__ << ": end of outer loop, truncated=" << truncated << ", count=" << count
        ...
        if (!truncated || count >= (max + 1) / 2) {
            ...
            ldpp_dout(dpp, 10) << __func__ << ": exiting attempt loop because we reached end (" << truncated
                               << ") or we're returning half the requested entries (" << count << " of " << max << ")"
                               << dendl;
            break;
        }
 ...
 *               truncated.
 */
int RGWRados::Bucket::List::list_objects_unordered(const DoutPrefixProvider* dpp, int64_t max_p,
                                                   std::vector<rgw_bucket_dir_entry>* result,
                                                   std::map<std::string, bool>* common_prefixes, bool* is_truncated,
                                                   optional_yield y)
{
    ...
    while (truncated && count <= max) {
        ...
        int r = store->cls_bucket_list_unordered(dpp,
                                                 ...
                                                 y);
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: " << __func__ << " cls_bucket_list_unordered returned " << r << " for "
        ...
        for (auto& entry : ent_list) {
            ...
            if (!params.list_versions && !entry.is_visible()) {
                ldpp_dout(dpp, 20) << __func__ << ": skippping \"" << index_key
                                   << "\" because not listing versions and entry not visibile" << dendl;
                continue;
            }

            if (params.enforce_ns && obj.ns != params.ns) {
                ldpp_dout(dpp, 20) << __func__ << ": skippping \"" << index_key << "\" because namespace does not match"
                                   << dendl;
                continue;
            }
            ...
            if (cur_end_marker_valid && cur_end_marker <= index_key) {
                ...
                // to loop through all
                ldpp_dout(dpp, 20) << __func__ << ": skippping \"" << index_key << "\" because after end_marker"
                                   << dendl;
                continue;
            }

            if (params.access_list_filter && !params.access_list_filter->filter(obj.name, index_key.name)) {
                ldpp_dout(dpp, 20) << __func__ << ": skippping \"" << index_key << "\" because doesn't match filter"
                                   << dendl;
                continue;
            }

            if (params.prefix.size() && (0 != obj.name.compare(0, params.prefix.size(), params.prefix))) {
                ldpp_dout(dpp, 20) << __func__ << ": skippping \"" << index_key << "\" because doesn't match prefix"
                                   << dendl;
                continue;
            }
...
void RGWRados::create_bucket_id(string* bucket_id)
{
    ...
    snprintf(buf, sizeof(buf), "%s.%" PRIu64 ".%" PRIu64, svc.zone->get_zone_params().get_id().c_str(), iid, bid);
    *bucket_id = buf;
}

int RGWRados::create_bucket(const RGWUserInfo& owner, rgw_bucket& bucket, const string& zonegroup_id,
                            const rgw_placement_rule& placement_rule, const string& swift_ver_location,
                            const RGWQuotaInfo* pquota_info, map<std::string, bufferlist>& attrs, RGWBucketInfo& info,
                            obj_version* pobjv, obj_version* pep_objv, real_time creation_time,
                            rgw_bucket* pmaster_bucket, uint32_t* pmaster_num_shards, optional_yield y,
...
#define MAX_CREATE_RETRIES 20 /* need to bound retries */
    ...
    for (int i = 0; i < MAX_CREATE_RETRIES; i++) {
        ...
        ret = svc.zone->select_bucket_placement(
            dpp, owner, zonegroup_id, placement_rule, &selected_placement_rule, &rule_info, y);
        if (ret < 0) return ret;
        ...
        init_default_bucket_layout(cct,
                                   info.layout,
                                   svc.zone->get_zone(),
                                   pmaster_num_shards ? std::optional{*pmaster_num_shards} : std::nullopt,
        ...
        if (ret == -EEXIST) {
            ...
            if (orig_info.bucket.bucket_id != bucket.bucket_id) {
                ...
                if (r < 0) {
                    ldpp_dout(dpp, 0) << "WARNING: " << __func__
                                      << "(): failed to remove bucket instance info: bucket instance="
                                      << info.bucket.get_key() << ": r=" << r << dendl;
                    /* continue anyway */
                }
            }
...
int RGWRados::get_obj_head_ioctx(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, const rgw_obj& obj,
                                 librados::IoCtx* ioctx)
{
    ...
    int r = open_pool_ctx(dpp, pool, *ioctx, false, true);
    if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: unable to open data-pool=" << pool.to_str() << " for obj=" << obj
                          << " with error-code=" << r << dendl;
        return r;
    }
...
int RGWRados::get_obj_head_ref(const DoutPrefixProvider* dpp, const rgw_placement_rule& target_placement_rule,
                               const rgw_obj& obj, rgw_rados_ref* ref)
{
    ...
    int r = ref->pool.open(dpp, RGWSI_RADOS::OpenParams().set_mostly_omap(false));
    if (r < 0) {
...
int RGWRados::get_obj_head_ref(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, const rgw_obj& obj,
                               rgw_rados_ref* ref)
{
 ...
 * up without one
 */
int RGWRados::fix_head_obj_locator(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, bool copy_obj,
                                   bool remove_bad, rgw_obj_key& key)
{
...
#define HEAD_SIZE 512 * 1024
    ...
    if (size != data.length()) {
        ldpp_dout(dpp, -1) << "ERROR: returned object size (" << size << ") != data.length() (" << data.length() << ")"
                           << dendl;
        return -EIO;
    }
...
int RGWRados::move_rados_obj(const DoutPrefixProvider* dpp, librados::IoCtx& src_ioctx, const string& src_oid,
                             const string& src_locator, librados::IoCtx& dst_ioctx, const string& dst_oid,
                             const string& dst_locator)
{
...
#define COPY_BUF_SIZE (4 * 1024 * 1024)
    ...
    if (ofs != size) {
        ldpp_dout(dpp, -1) << "ERROR: " << __func__ << ": copying " << src_oid << " -> " << dst_oid << ": expected "
                           << size << " bytes to copy, ended up with " << ofs << dendl;
        ret = -EIO;
        goto done_err;
    }
 ...
 * up without one
 */
int RGWRados::fix_tail_obj_locator(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info, rgw_obj_key& key,
...
int RGWRados::BucketShard::init(const rgw_bucket& _bucket, const rgw_obj& obj, RGWBucketInfo* bucket_info_out,
                                const DoutPrefixProvider* dpp)
{
    bucket = _bucket;

    RGWBucketInfo bucket_info;
    RGWBucketInfo* bucket_info_p = bucket_info_out ? bucket_info_out : &bucket_info;
    ...
    ret = store->svc.bi_rados->open_bucket_index_shard(
        dpp, *bucket_info_p, obj.get_hash_object(), &bucket_obj, &shard_id);
    if (ret < 0) {
...
int RGWRados::BucketShard::init(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, const rgw_obj& obj)
{
    ...
    int ret =
        store->svc.bi_rados->open_bucket_index_shard(dpp, bucket_info, obj.get_hash_object(), &bucket_obj, &shard_id);
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: open_bucket_index_shard() returned ret=" << ret << dendl;
        return ret;
    }
                                ...
                                const rgw::bucket_index_layout_generation& index, int sid)
{
    ...
    int ret = store->svc.bi_rados->open_bucket_index_shard(dpp, bucket_info, index, shard_id, &bucket_obj);
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: open_bucket_index_shard() returned ret=" << ret << dendl;
        return ret;
    }
...
int RGWRados::on_last_entry_in_listing(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info,
                                       const std::string& obj_prefix, const std::string& obj_delim,
                                       std::function<int(const rgw_bucket_dir_entry&)> handler)
{
    ...
    ldpp_dout(dpp, 20) << "iterating listing for bucket=" << bucket_info.bucket.name << ", obj_prefix=" << obj_prefix
                       << ", obj_delim=" << obj_delim << dendl;
    ...
    do {
        ...
        int ret = list_op.list_objects(dpp, MAX_LIST_OBJS, &entries, nullptr, &is_truncated, null_yield);
        if (ret < 0) {
            return ret;
        }
...
{
    return bucket_info.has_swift_versioning() && bucket_info.swift_ver_location.size();
}

int RGWRados::swift_versioning_copy(RGWObjectCtx& obj_ctx, const rgw_user& user, RGWBucketInfo& bucket_info,
                                    const rgw_obj& obj, const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    r = get_bucket_info(
        &svc, bucket_info.bucket.tenant, bucket_info.swift_ver_location, dest_bucket_info, NULL, null_yield, NULL);
    if (r < 0) {
...
int RGWRados::swift_versioning_restore(RGWObjectCtx& obj_ctx, const rgw_user& user, RGWBucketInfo& bucket_info,
                                       rgw_obj& obj, bool& restored, const DoutPrefixProvider* dpp)
{
    if (!swift_versioning_enabled(bucket_info)) {
        return 0;
    }
    ...
    int ret = get_bucket_info(
        &svc, bucket_info.bucket.tenant, bucket_info.swift_ver_location, archive_binfo, nullptr, null_yield, nullptr);
    if (ret < 0) {
        return ret;
    }
    ...
    const auto handler = [&](const rgw_bucket_dir_entry& entry) -> int {
        ...
        ret = delete_obj(dpp, obj_ctx, archive_binfo, archive_obj, archive_binfo.versioning_status());

        return ret;
    };

    const std::string& obj_name = obj.get_oid();
    const auto prefix = boost::str(boost::format("%03x%s") % obj_name.size() % obj_name);

    return on_last_entry_in_listing(dpp, archive_binfo, prefix, std::string(), handler);
}

int RGWRados::Object::Write::_do_write_meta(const DoutPrefixProvider* dpp, uint64_t size, uint64_t accounted_size,
                                            map<string, bufferlist>& attrs, bool assume_noent, bool modify_tail,
                                            void* _index_op, optional_yield y)
{
    ...
    r = target->prepare_atomic_modification(
        dpp, op, reset_obj, ptag, meta.if_match, meta.if_nomatch, false, modify_tail, y);
    if (r < 0) return r;
    ...
    if (target->get_bucket_info().obj_lock_enabled() && target->get_bucket_info().obj_lock.has_rule() &&
    ...
    if (meta.manifest) {
        ...
        if (iter != attrs.end()) attrs.erase(iter);
    ...
    bool versioned_target = (meta.olh_epoch && *meta.olh_epoch > 0) || !obj.key.instance.empty();
    ...
    if (!real_clock::is_zero(meta.delete_at)) {
        ...
        r = store->obj_expirer->hint_add(
            dpp, meta.delete_at, obj.bucket.tenant, obj.bucket.name, obj.bucket.bucket_id, obj_key);
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: objexp_hint_add() returned r=" << r << ", object will not get removed"
                              << dendl;
...
public:
    RGWRadosPutObj(const DoutPrefixProvider* dpp, CephContext* cct, CompressorRef& plugin,
                   boost::optional<RGWPutObj_Compress>& compressor, rgw::sal::ObjectProcessor* p,
    ...
    int process_attrs(void)
    {
        ...
        if (extra_data_bl.length()) {
            ...
            if (!jp.parse(extra_data_bl.c_str(), extra_data_bl.length())) {
                ldpp_dout(dpp, 0) << "failed to parse response extra data. len=" << extra_data_bl.length()
                                  << " data=" << extra_data_bl.c_str() << dendl;
                return -EIO;
            }
            ...
            if (iter != src_attrs.end()) {
                ...
                if (src_attrs.count(RGW_ATTR_CRYPT_MODE) && !src_attrs.count(RGW_ATTR_CRYPT_PARTS)) {
                    std::vector<size_t> parts_len;
                    int r = RGWGetObj_BlockDecrypt::read_manifest_parts(dpp, manifest_bl, parts_len);
                    if (r < 0) {
        ...
        if (try_etag_verify) {
            ret = rgw::putobj::create_etag_verifier(dpp, cct, filter, manifest_bl, compression_info, etag_verifier);
            if (ret < 0) {
    ...
    int handle_data(bufferlist& bl, bool* pause) override
    {
        ...
        if (extra_data_left) {
            ...
            if (extra_len > extra_data_left) extra_len = extra_data_left;
    ...
    int flush() { return filter->process({}, data_len); }
    ...
    uint64_t get_data_len() { return data_len; }
 ...
 */
static void set_copy_attrs(map<string, bufferlist>& src_attrs, map<string, bufferlist>& attrs,
                           RGWRados::AttrsMod attrs_mod)
{
    switch (attrs_mod) {
    case RGWRados::ATTRSMOD_NONE: attrs = src_attrs; break;
...
int RGWRados::reindex_obj(const RGWBucketInfo& bucket_info, const rgw_obj& obj, const DoutPrefixProvider* dpp,
                          optional_yield y)
{
    if (bucket_info.versioned()) {
        ldpp_dout(dpp, 10) << "WARNING: " << __func__ << ": cannot process versioned bucket \""
                           << bucket_info.bucket.get_key() << "\"" << dendl;
        return -ENOTSUP;
    }
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": update index prepare for \"" << obj
...
public:
    ...
    bufferlist& get_extra_data() { return extra_data; }
};

int RGWRados::stat_remote_obj(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, const rgw_user& user_id,
                              req_info* info, const rgw_zone_id& source_zone, const rgw_obj& src_obj,
                              const RGWBucketInfo* src_bucket_info, real_time* src_mtime, uint64_t* psize,
                              const real_time* mod_ptr, const real_time* unmod_ptr, bool high_precision_time,
                              const char* if_match, const char* if_nomatch, map<string, bufferlist>* pattrs,
                              map<string, string>* pheaders, string* version_id, string* ptag, string* petag)
{
    ...
    int ret = conn->get_obj(dpp,
                            user_id,
                            info,
                            src_obj,
                            pmod,
                            unmod_ptr,
                            dest_mtime_weight.zone_short_id,
                            dest_mtime_weight.pg_ver,
                            prepend_meta,
                            get_op,
                            rgwx_stat,
                            sync_manifest,
    ...
    ret = conn->complete_request(in_stream_req, nullptr, &set_mtime, psize, nullptr, pheaders, null_yield);
    if (ret < 0) {
        return ret;
    }
...
int RGWFetchObjFilter_Default::filter(CephContext* cct, const rgw_obj_key& source_key,
                                      const RGWBucketInfo& dest_bucket_info,
                                      std::optional<rgw_placement_rule> dest_placement_rule,
                                      const map<string, bufferlist>& obj_attrs,
                                      std::optional<rgw_user>* poverride_owner, const rgw_placement_rule** prule)
{
...
int RGWRados::fetch_remote_obj(RGWObjectCtx& obj_ctx, const rgw_user& user_id, req_info* info,
                               const rgw_zone_id& source_zone, const rgw_obj& dest_obj, const rgw_obj& src_obj,
                               RGWBucketInfo& dest_bucket_info, RGWBucketInfo* src_bucket_info,
                               std::optional<rgw_placement_rule> dest_placement_rule, real_time* src_mtime,
                               ...
                               AttrsMod attrs_mod, bool copy_if_newer, rgw::sal::Attrs& attrs, RGWObjCategory category,
                               std::optional<uint64_t> olh_epoch, real_time delete_at, string* ptag, string* petag,
                               void (*progress_cb)(off_t, void*), void* progress_data, const DoutPrefixProvider* dpp,
                               RGWFetchObjFilter* filter, const rgw_zone_set_entry& source_trace_entry,
                               rgw_zone_set* zones_trace, std::optional<uint64_t>* bytes_transferred)
{
    /* source is in a different zonegroup, copy from there */
    ...
    AtomicObjectProcessor processor(
        &aio, this, dest_bucket_info, nullptr, user_id, obj_ctx, dest_obj, olh_epoch, tag, dpp, null_yield);
    RGWRESTConn* conn;
    auto& zone_conn_map = svc.zone->get_zone_conn_map();
    auto& zonegroup_conn_map = svc.zone->get_zonegroup_conn_map();
    if (source_zone.empty()) {
    ...
    RGWRadosPutObj cb(
        dpp, cct, plugin, compressor, &processor, progress_cb, progress_data, [&](map<string, bufferlist>& obj_attrs) {
            const rgw_placement_rule* ptail_rule;

            int ret = filter->filter(
                cct, src_obj.key, dest_bucket_info, dest_placement_rule, obj_attrs, &override_owner, &ptail_rule);
            if (ret < 0) {
            ...
            if (compression_type != "none") {
                ...
                if (!plugin) {
                    ldpp_dout(dpp, 1) << "Cannot load plugin for compression type " << compression_type << dendl;
                }
            }
    ...
    ret = conn->complete_request(in_stream_req, &etag, &set_mtime, &accounted_size, nullptr, nullptr, null_yield);
    if (ret < 0) {
        goto set_err_state;
    }
    ret = cb.flush();
    ...
    if (cb.get_data_len() != accounted_size) {
        ret = -EIO;
        ldpp_dout(dpp, 0) << "ERROR: object truncated during fetching, expected " << accounted_size
    ...
    else if (auto c = cb.get_attrs().find(RGW_ATTR_COMPRESSION); c != cb.get_attrs().end()) {
    ...
    if (override_owner) {
        ...
        if (aiter == obj_attrs.end()) {
            ...
            try {
                ...
                ldpp_dout(dpp, 0) << "ERROR: " << __func__ << "(): could not decode policy, caught buffer::error"
                                  << dendl;
                return -EIO;
            }
        }
    ...
    {   // update replication trace
        ...
        if (auto i = cb.get_attrs().find(RGW_ATTR_OBJ_REPLICATION_TRACE); i != cb.get_attrs().end()) {
            try {
    ...
    if (const auto& verifier_etag = cb.get_verifier_etag(); !verifier_etag.empty()) {
        string trimmed_etag = etag;

        /* Remove the leading and trailing double quotes from etag */
        trimmed_etag.erase(std::remove(trimmed_etag.begin(), trimmed_etag.end(), '\"'), trimmed_etag.end());

        if (verifier_etag != trimmed_etag) {
            ret = -EIO;
            ldpp_dout(dpp, 0) << "ERROR: source and destination objects don't match. Expected etag:" << trimmed_etag
                              << " Computed etag:" << verifier_etag << dendl;
            goto set_err_state;
        }
    }
...
#define MAX_COMPLETE_RETRY 100
    for (i = 0; i < MAX_COMPLETE_RETRY; i++) {
        bool canceled = false;
        ret = processor.complete(accounted_size,
                                 etag,
                                 mtime,
                                 set_mtime,
                                 attrs,
        ...
        if (copy_if_newer && canceled) {
            ...
            if (!dest_state->exists || dest_mtime_weight < set_mtime_weight) {
                ldpp_dout(dpp, 20) << "retrying writing object mtime=" << set_mtime
                                   << " dest_state->mtime=" << dest_state->mtime
                                   << " dest_state->exists=" << dest_state->exists << dendl;
                continue;
            }
            else {
                ldpp_dout(dpp, 20) << "not retrying writing object mtime=" << set_mtime
                                   << " dest_state->mtime=" << dest_state->mtime
                                   << " dest_state->exists=" << dest_state->exists << dendl;
            }
        }
...
int RGWRados::copy_obj_to_remote_dest(const DoutPrefixProvider* dpp, RGWObjState* astate,
                                      map<string, bufferlist>& src_attrs, RGWRados::Object::Read& read_op,
                                      const rgw_user& user_id, const rgw_obj& dest_obj, real_time* mtime)
{
    string etag;
 ...
 * Returns: 0 on success, -ERR# otherwise.
 */
int RGWRados::copy_obj(RGWObjectCtx& obj_ctx, const rgw_user& user_id, req_info* info, const rgw_zone_id& source_zone,
                       const rgw_obj& dest_obj, const rgw_obj& src_obj, RGWBucketInfo& dest_bucket_info,
                       RGWBucketInfo& src_bucket_info, const rgw_placement_rule& dest_placement, real_time* src_mtime,
                       real_time* mtime, const real_time* mod_ptr, const real_time* unmod_ptr, bool high_precision_time,
                       const char* if_match, const char* if_nomatch, AttrsMod attrs_mod, bool copy_if_newer,
                       rgw::sal::Attrs& attrs, RGWObjCategory category, uint64_t olh_epoch, real_time delete_at,
                       string* version_id, string* ptag, string* petag, void (*progress_cb)(off_t, void*),
                       void* progress_data, const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    ldpp_dout(dpp, 5) << "Copy object " << src_obj.bucket << ":" << src_obj.get_oid() << " => " << dest_obj.bucket
                      << ":" << dest_obj.get_oid() << dendl;

    if (remote_src || !source_zone.empty()) {
        rgw_zone_set_entry source_trace_entry{source_zone.id, std::nullopt};
        return fetch_remote_obj(obj_ctx,
                                user_id,
                                info,
                                source_zone,
                                dest_obj,
    ...
    if (rt != attrs.end()) src_attrs[RGW_ATTR_OBJECT_RETENTION] = rt->second;
    map<string, bufferlist>::iterator lh = attrs.find(RGW_ATTR_OBJECT_LEGAL_HOLD);
    if (lh != attrs.end()) src_attrs[RGW_ATTR_OBJECT_LEGAL_HOLD] = lh->second;
    ...
    if (cmp != src_attrs.end()) attrs[RGW_ATTR_COMPRESSION] = cmp->second;
    ...
    bool copy_data = (!amanifest) || (*src_rule != dest_placement) || (src_pool != dest_pool);
    ...
    ldpp_dout(dpp, 20) << "dest_obj=" << dest_obj << " src_obj=" << src_obj << " copy_itself=" << (int)copy_itself
                       << dendl;
...
int RGWRados::copy_obj_data(RGWObjectCtx& obj_ctx, RGWBucketInfo& dest_bucket_info,
                            const rgw_placement_rule& dest_placement, RGWRados::Object::Read& read_op, off_t end,
                            const rgw_obj& dest_obj, real_time* mtime, real_time set_mtime, rgw::sal::Attrs& attrs,
                            uint64_t olh_epoch, real_time delete_at, string* petag, const DoutPrefixProvider* dpp,
                            optional_yield y)
{
    string tag;
    append_rand_alpha(cct, tag, tag, 32);
    ...
    AtomicObjectProcessor processor(&aio,
                                    this,
                                    dest_bucket_info,
                                    &dest_placement,
    ...
    return processor.complete(
        accounted_size, etag, mtime, set_mtime, attrs, delete_at, nullptr, nullptr, nullptr, nullptr, nullptr, y);
}

int RGWRados::transition_obj(RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info, const rgw_obj& obj,
                             const rgw_placement_rule& placement_rule, const real_time& mtime, uint64_t olh_epoch,
                             const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    if (read_mtime != mtime) {
        /* raced */
        ldpp_dout(dpp, 0) << __func__ << " ERROR: failed to transition obj(" << obj.key
                          << ") read_mtime = " << read_mtime << " doesn't match mtime = " << mtime << dendl;
        return -ECANCELED;
    }
 ...
 * Returns 0 on success, -ERR# otherwise.
 */
int RGWRados::delete_bucket(RGWBucketInfo& bucket_info, RGWObjVersionTracker& objv_tracker, optional_yield y,
                            const DoutPrefixProvider* dpp, bool check_empty)
{
    const rgw_bucket& bucket = bucket_info.bucket;
    RGWSI_RADOS::Pool index_pool;
    map<int, string> bucket_objs;
    int r = svc.bi_rados->open_bucket_index(
        dpp, bucket_info, std::nullopt, bucket_info.layout.current_index, &index_pool, &bucket_objs, nullptr);
    if (r < 0) return r;
    ...
    if (objv_tracker.read_version.empty()) {
        ...
        r = ctl.bucket->read_bucket_entrypoint_info(bucket_info.bucket,
                                                    ...
                                                    null_yield,
                                                    dpp,
                                                    RGWBucketCtl::Bucket::GetParams().set_objv_tracker(&objv_tracker));
        if (r < 0 || (!bucket_info.bucket.bucket_id.empty() && ep.bucket.bucket_id != bucket_info.bucket.bucket_id)) {
            if (r != -ENOENT) {
                ldpp_dout(dpp, 0) << "ERROR: read_bucket_entrypoint_info() bucket=" << bucket_info.bucket
                                  << " returned error: r=" << r << dendl;
    ...
    if (remove_ep) {
        r = ctl.bucket->remove_bucket_entrypoint_info(
            bucket_info.bucket, null_yield, dpp, RGWBucketCtl::Bucket::RemoveParams().set_objv_tracker(&objv_tracker));
        if (r < 0) return r;
    }
    ...
    if (!svc.zone->is_syncing_bucket_meta(bucket)) {
        ...
        (void)CLSRGWIssueBucketIndexClean(index_pool.ioctx(), bucket_objs, cct->_conf->rgw_bucket_index_max_aio)();
    }
...
int RGWRados::set_buckets_enabled(vector<rgw_bucket>& buckets, bool enabled, const DoutPrefixProvider* dpp)
{
    ...
    for (iter = buckets.begin(); iter != buckets.end(); ++iter) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "NOTICE: get_bucket_info on bucket=" << bucket.name << " returned err=" << r
        ...
        r = put_bucket_instance_info(info, false, real_time(), &attrs, dpp, null_yield);
        if (r < 0) {
            ldpp_dout(dpp, 0) << "NOTICE: put_bucket_info on bucket=" << bucket.name << " returned err=" << r
                              << ", skipping bucket" << dendl;
            ret = r;
            continue;
        }
...
{
    if ((!manifest) || state->keep_tail) return 0;
...
void RGWRados::update_gc_chain(const DoutPrefixProvider* dpp, rgw_obj head_obj, RGWObjManifest& manifest,
                               cls_rgw_obj_chain* chain)
{
...
void RGWRados::delete_objs_inline(const DoutPrefixProvider* dpp, cls_rgw_obj_chain& chain, const string& tag)
{
    ...
    for (auto liter = chain.objs.begin(); liter != chain.objs.end(); ++liter) {
        ...
        if (obj.pool != last_pool) {
            ...
            if (ret < 0) {
                last_pool = "";
                ldpp_dout(dpp, 0) << "ERROR: failed to create ioctx pool=" << obj.pool << dendl;
                continue;
            }
            last_pool = obj.pool;
        }
        ...
        ldpp_dout(dpp, 5) << "delete_objs_inline: removing " << obj.pool << ":" << obj.key.name << dendl;
        ObjectWriteOperation op;
...
static void accumulate_raw_stats(const rgw_bucket_dir_header& header, map<RGWObjCategory, RGWStorageStats>& stats)
{
                                 ...
                                 map<RGWObjCategory, RGWStorageStats>* calculated_stats)
{
    ...
    int ret = svc.bi_rados->open_bucket_index(
        dpp, bucket_info, std::nullopt, bucket_info.layout.current_index, &index_pool, &oids, nullptr);
    if (ret < 0) {
        return ret;
    }
...
static int resync_encrypted_multipart(const DoutPrefixProvider* dpp, optional_yield y, RGWRados* store,
                                      RGWBucketInfo& bucket_info, RGWObjectCtx& obj_ctx, const RGWObjState& state)
{
    ...
    return store->set_attrs(dpp, &obj_ctx, bucket_info, state.obj, add_attrs, nullptr, y, set_mtime);
}

static void try_resync_encrypted_multipart(const DoutPrefixProvider* dpp, optional_yield y, RGWRados* store,
                                           RGWBucketInfo& bucket_info, RGWObjectCtx& obj_ctx,
                                           const rgw_bucket_dir_entry& dirent, Formatter* f)
{
    ...
    int ret = store->get_obj_state(dpp, &obj_ctx, bucket_info, obj, &astate, &manifest, follow_olh, y);
    if (ret < 0) {
    ...
    if (auto i = astate->attrset.find(RGW_ATTR_CRYPT_MODE); i == astate->attrset.end()) {
    ...
    ret = resync_encrypted_multipart(dpp, y, store, bucket_info, obj_ctx, *astate);
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to update " << obj << ": " << cpp_strerror(ret) << dendl;
        return;
    }
...
int RGWRados::bucket_resync_encrypted_multipart(const DoutPrefixProvider* dpp, optional_yield y,
                                                rgw::sal::RadosStore* driver, RGWBucketInfo& bucket_info,
                                                const std::string& marker, RGWFormatterFlusher& flusher)
{
    RGWRados::Bucket target(this, bucket_info);
    RGWRados::Bucket::List list_op(&target);
    ...
    do {
        int ret = list_op.list_objects(dpp, MAX_LIST_OBJS, &entries, nullptr, &is_truncated, y);
        if (ret < 0) {
            return ret;
        }
        ...
        for (const auto& dirent : entries) {
            ...
            try_resync_encrypted_multipart(dpp, y, this, bucket_info, obj_ctx, dirent, f);
        }
...
int RGWRados::bucket_set_reshard(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                 const cls_rgw_bucket_instance_entry& entry)
{
    ...
    int r = svc.bi_rados->open_bucket_index(
        dpp, bucket_info, std::nullopt, bucket_info.layout.current_index, &index_pool, &bucket_objs, nullptr);
    if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": unable to open bucket index, r=" << r << " ("
    ...
    if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": unable to issue set bucket resharding, r=" << r << " ("
                          << cpp_strerror(-r) << ")" << dendl;
    }
    return r;
}

int RGWRados::defer_gc(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, RGWBucketInfo& bucket_info,
...
void RGWRados::cls_obj_check_mtime(ObjectOperation& op, const real_time& mtime, bool high_precision_time,
...
int RGWRados::Object::Delete::delete_obj(optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    if (params.versioning_status & BUCKET_VERSIONED || explicit_marker_version) {
        if (instance.empty() || explicit_marker_version) {
            ...
            if (result.version_id.empty()) result.version_id = "null";
            ...
            r = store->unlink_obj_instance(
                dpp, target->get_ctx(), target->get_bucket_info(), obj, params.olh_epoch, y, params.zones_trace);
            if (r < 0) {
                return r;
            }
        ...
        add_datalog_entry(dpp, store->svc.datalog_rados, target->get_bucket_info(), bs->shard_id, y);
...
int RGWRados::delete_obj(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, const RGWBucketInfo& bucket_info,
                         const rgw_obj& obj,
                         int versioning_status,   // versioning flags defined in enum RGWBucketFlags
                         uint16_t bilog_flags, const real_time& expiration_time, rgw_zone_set* zones_trace)
{
                               ...
                               optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: " << __func__ << "() get_bucket_instance_info(bucket=" << obj.bucket
...
static void generate_fake_tag(const DoutPrefixProvider* dpp, RGWRados* store, map<string, bufferlist>& attrset,
                              RGWObjManifest& manifest, bufferlist& manifest_bl, bufferlist& tag_bl)
{
    string tag;
...
int RGWRados::get_olh_target_state(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info,
                                   const rgw_obj& obj, RGWObjState* olh_state, RGWObjState** target_state,
                                   RGWObjManifest** target_manifest, optional_yield y)
{
    ...
    r = get_obj_state(dpp, &obj_ctx, bucket_info, target, target_state, target_manifest, false, y);
    if (r < 0) {
        return r;
    }
...
int RGWRados::get_obj_state_impl(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, RGWBucketInfo& bucket_info,
                                 const rgw_obj& obj, RGWObjState** state, RGWObjManifest** manifest, bool follow_olh,
                                 optional_yield y, bool assume_noent)
{
    ...
    *manifest = nullptr;
    ...
    ldpp_dout(dpp, 20) << "get_obj_state: rctx=" << (void*)rctx << " obj=" << obj << " state=" << (void*)s
                       << " s->prefetch_data=" << s->prefetch_data << dendl;
    *state = s;
    if (sm->manifest) {
    ...
    if (!assume_noent) {
        r = RGWRados::raw_obj_stat(
            dpp, raw_obj, &s->size, &s->mtime, &s->epoch, &s->attrset, (s->prefetch_data ? &s->data : NULL), NULL, y);
    }
    ...
    if (r == -ENOENT) {
        ...
        if (obj_tombstone_cache && obj_tombstone_cache->find(obj, entry)) {
            ...
            ldpp_dout(dpp, 20) << __func__ << "(): found obj in tombstone cache: obj=" << obj << " mtime=" << s->mtime
                               << " pgv=" << s->pg_ver << dendl;
        }
        else {
            s->mtime = real_time();
    ...
    if (iter = s->attrset.find(RGW_ATTR_MANIFEST); iter != s->attrset.end()) {
        ...
        try {
            ...
            sm->manifest->set_head(bucket_info.placement_rule, obj, s->size); /* patch manifest to reflect the head we
                                                   just read, some manifests might be broken due to old bugs */
        ...
        *manifest = &(*sm->manifest);
        ...
        if (cct->_conf->subsys.should_gather<ceph_subsys_rgw, 20>() && sm->manifest->has_explicit_objs()) {
            RGWObjManifest::obj_iterator mi;
            for (mi = sm->manifest->obj_begin(dpp); mi != sm->manifest->obj_end(dpp); ++mi) {
                ldpp_dout(dpp, 20) << "manifest: ofs=" << mi.get_ofs() << " loc=" << mi.get_location().get_raw_obj(this)
                                   << dendl;
            }
        }
    ...
    if (iter = s->attrset.find(RGW_ATTR_PG_VER); iter != s->attrset.end()) {
        ...
        if (pg_ver_bl.length()) {
            ...
            try {
                ...
                ldpp_dout(dpp, 0) << "ERROR: couldn't decode pg ver attr for object " << s->obj
    ...
    if (iter = s->attrset.find(RGW_ATTR_SOURCE_ZONE); iter != s->attrset.end()) {
        ...
        if (zone_short_id_bl.length()) {
            ...
            try {
                ...
                ldpp_dout(dpp, 0) << "ERROR: couldn't decode zone short id attr for object " << s->obj
                                  << ", non-critical error, ignoring" << dendl;
            }
        }
    }
    ...
    if (is_olh(s->attrset)) {
        ...
        ldpp_dout(dpp, 20) << __func__ << ": setting s->olh_tag to " << string(s->olh_tag.c_str(), s->olh_tag.length())
                           << dendl;
...
int RGWRados::get_obj_state(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, RGWBucketInfo& bucket_info,
                            const rgw_obj& obj, RGWObjState** state, RGWObjManifest** manifest, bool follow_olh,
                            optional_yield y, bool assume_noent)
{
    int ret;
                                     ...
                                     optional_yield y)
{
    ...
    if (r < 0) return r;
    if (!state->exists) return -ENOENT;
    if (!state->get_attr(name, dest)) return -ENODATA;
...
int RGWRados::Object::Stat::stat_async(const DoutPrefixProvider* dpp)
{
    ...
    if (r < 0) {
        ldpp_dout(dpp, 5) << __func__ << ": ERROR: aio_operate() returned ret=" << r << dendl;
        return r;
    }
...
int RGWRados::append_atomic_test(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, RGWBucketInfo& bucket_info,
                                 const rgw_obj& obj, ObjectOperation& op, RGWObjState** pstate,
...
int RGWRados::append_atomic_test(const DoutPrefixProvider* dpp, const RGWObjState* state, librados::ObjectOperation& op)
{
    if (!state->is_atomic) {
...
int RGWRados::Object::get_state(const DoutPrefixProvider* dpp, RGWObjState** pstate, RGWObjManifest** pmanifest,
...
int RGWRados::Object::prepare_atomic_modification(const DoutPrefixProvider* dpp, ObjectWriteOperation& op,
                                                  bool reset_obj, const string* ptag, const char* if_match,
                                                  const char* if_nomatch, bool removal_op, bool modify_tail,
                                                  optional_yield y)
{
    ...
    bool need_guard =
        ((manifest) || (state->obj_tag.length() != 0) || if_match != NULL || if_nomatch != NULL) && (!state->fake_tag);
    ...
    if (need_guard) {
        ...
        if (if_match) {
            if (strcmp(if_match, "*") == 0) {
                ...
                bufferlist bl;
                if (!state->get_attr(RGW_ATTR_ETAG, bl) || strncmp(if_match, bl.c_str(), bl.length()) != 0) {
                    return -ERR_PRECONDITION_FAILED;
                }
            }
        }
        ...
        if (if_nomatch) {
            if (strcmp(if_nomatch, "*") == 0) {
                ...
                bufferlist bl;
                if (!state->get_attr(RGW_ATTR_ETAG, bl) || strncmp(if_nomatch, bl.c_str(), bl.length()) == 0) {
                    return -ERR_PRECONDITION_FAILED;
                }
            }
        }
 ...
 * Returns: 0 on success, -ERR# otherwise.
 */
int RGWRados::set_attr(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, RGWBucketInfo& bucket_info,
                       const rgw_obj& obj, const char* name, bufferlist& bl)
{
    map<string, bufferlist> attrs;
    attrs[name] = bl;
...
int RGWRados::set_attrs(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, RGWBucketInfo& bucket_info,
                        const rgw_obj& src_obj, map<string, bufferlist>& attrs, map<string, bufferlist>* rmattrs,
                        optional_yield y, ceph::real_time set_mtime /* = zero() */)
{
    rgw_obj obj = src_obj;
...
int RGWRados::Object::Read::prepare(optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    if (conds.if_match || conds.if_nomatch) {
        ...
        if (conds.if_match) {
            ...
            ldpp_dout(dpp, 10) << "ETag: " << string(etag.c_str(), etag.length()) << " "
    ...
    if (params.obj_size) *params.obj_size = astate->size;
    if (params.lastmod) *params.lastmod = astate->mtime;
...
#define NUM_RESHARD_RETRIES 10
    for (int i = 0; i < NUM_RESHARD_RETRIES; ++i) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: failed to get BucketShard object. obj=" << obj_instance.key << ". ret=" << ret
                              << dendl;
            return ret;
        }
        ...
        ldpp_dout(dpp, 10) << "NOTICE: resharding operation on bucket index detected, blocking. obj="
                           << obj_instance.key << dendl;

        r = store->block_while_resharding(bs, obj_instance, target->bucket_info, null_yield, dpp);
        if (r == -ERR_BUSY_RESHARDING) {
            ldpp_dout(dpp, 10) << __func__ << " NOTICE: block_while_resharding() still busy. obj=" << obj_instance.key
                               << dendl;
            continue;
        }
        else if (r < 0) {
            ldpp_dout(dpp, 0) << __func__ << " ERROR: block_while_resharding() failed. obj=" << obj_instance.key
    ...
    if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: bucket shard callback failed. obj=" << obj_instance.key
                          << ". ret=" << cpp_strerror(-r) << dendl;
        return r;
    }
...
int RGWRados::Bucket::UpdateIndex::complete(const DoutPrefixProvider* dpp, int64_t poolid, uint64_t epoch,
                                            uint64_t size, uint64_t accounted_size, ceph::real_time& ut,
                                            const string& etag, const string& content_type, const string& storage_class,
                                            ...
                                            const string* user_data, bool appendable)
{
    ...
    ret = store->cls_obj_complete_add(
        *bs, obj, optag, poolid, epoch, ent, category, remove_objs, bilog_flags, zones_trace);

    add_datalog_entry(dpp, store->svc.datalog_rados, target->bucket_info, bs->shard_id, y);

    return ret;
}

int RGWRados::Bucket::UpdateIndex::complete_del(const DoutPrefixProvider* dpp, int64_t poolid, uint64_t epoch,
                                                real_time& removed_mtime, list<rgw_obj_index_key>* remove_objs,
                                                optional_yield y)
{
    if (blind) {
        return 0;
    ...
    ret = store->cls_obj_complete_del(
        *bs, optag, poolid, epoch, obj, removed_mtime, remove_objs, bilog_flags, zones_trace);
...
int RGWRados::Bucket::UpdateIndex::cancel(const DoutPrefixProvider* dpp, list<rgw_obj_index_key>* remove_objs,
                                          optional_yield y)
{
    if (blind) {
        return 0;
                                 ...
                                 const DoutPrefixProvider* dpp)
{
    ...
    if (len > max_chunk_size) len = max_chunk_size;
    ...
    if (reading_from_head) {
        ...
        r = store->append_atomic_test(
            dpp, &source->get_ctx(), source->get_bucket_info(), state.obj, op, &astate, &manifest, y);
        if (r < 0) return r;
    ...
    ldpp_dout(dpp, 20) << "rados->read obj-ofs=" << ofs << " read_ofs=" << read_ofs << " read_len=" << read_len
                       << dendl;
    ...
    if (state.cur_pool != read_obj.pool) {
        ...
        if (iter == state.io_ctxs.end()) {
            ...
            if (r < 0) {
                ldpp_dout(dpp, 20) << "ERROR: failed to open pool context for pool=" << read_obj.pool << " r=" << r
                                   << dendl;
                return r;
            }
        }
    ...
    while (!completed.empty() && completed.front().id == offset) {
        ...
        if (rgwrados->get_use_datacache()) {
            ...
            if (bl.length() <= g_conf()->rgw_get_obj_max_req_size && !d3n_bypass_cache_write) {
                lsubdout(g_ceph_context, rgw_datacache, 10)
                    << "D3nDataCache: " << __func__
                    << "(): bl.length <= rgw_get_obj_max_req_size (default 4MB) - write to datacache, bl.length="
                    << bl.length() << dendl;
                ...
                lsubdout(g_ceph_context, rgw_datacache, 10)
                    << "D3nDataCache: " << __func__
                    << "(): not writing to datacache - bl.length > rgw_get_obj_max_req_size (default 4MB), bl.length="
                    << bl.length() << " or d3n_bypass_cache_write=" << d3n_bypass_cache_write << dendl;
            }
        }
...
static int _get_obj_iterate_cb(const DoutPrefixProvider* dpp, const rgw_raw_obj& read_obj, off_t obj_ofs,
                               off_t read_ofs, off_t len, bool is_head_obj, RGWObjState* astate, void* arg)
{
    struct get_obj_data* d = static_cast<struct get_obj_data*>(arg);
    return d->rgwrados->get_obj_iterate_cb(dpp, read_obj, obj_ofs, read_ofs, len, is_head_obj, astate, arg);
}
                                 ...
                                 off_t read_ofs, off_t len, bool is_head_obj, RGWObjState* astate, void* arg)
{
    ...
    ldpp_dout(dpp, 20) << "rados->get_obj_iterate_cb oid=" << read_obj.oid << " obj-ofs=" << obj_ofs
                       << " read_ofs=" << read_ofs << " len=" << len << dendl;
                                    ...
                                    optional_yield y)
{
    ...
    int r = store->iterate_obj(dpp,
                               source->get_ctx(),
                               source->get_bucket_info(),
                               state.obj,
                               ofs,
                               end,
                               chunk_size,
...
int RGWRados::iterate_obj(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info,
                          const rgw_obj& obj, off_t ofs, off_t end, uint64_t max_chunk_size, iterate_obj_cb cb,
                          void* arg, optional_yield y)
{
...
int RGWRados::obj_operate(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, const rgw_obj& obj,
                          ObjectWriteOperation* op)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(dpp, bucket_info, obj, &ref);
...
void RGWRados::olh_cancel_modification(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                       RGWObjState& state, const rgw_obj& olh_obj, const std::string& op_tag,
                                       optional_yield y)
{
    ...
    if (r < 0) {
        if (r != -ENOENT && r != -ECANCELED) {
            ldpp_dout(dpp, 0) << __func__ << " target_obj=" << olh_obj << " rmxattr rgw_rados_operate() returned " << r
                              << dendl;
        }
        return;
    }
    ...
    if (r < 0 && (r != -ENOENT && r != -ECANCELED)) {
        ldpp_dout(dpp, 0) << __func__ << " target_obj=" << olh_obj << " olh rm rgw_rados_operate() returned " << r
                          << dendl;
    }
}

int RGWRados::olh_init_modification_impl(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                         RGWObjState& state, const rgw_obj& olh_obj, string* op_tag)
{
    ObjectWriteOperation op;
...
int RGWRados::olh_init_modification(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, RGWObjState& state,
                                    const rgw_obj& obj, string* op_tag)
{
    int ret;
...
int RGWRados::guard_reshard(const DoutPrefixProvider* dpp, BucketShard* bs, const rgw_obj& obj_instance,
...
int RGWRados::block_while_resharding(RGWRados::BucketShard* bs, const rgw_obj& obj_instance, RGWBucketInfo& bucket_info,
                                     optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    auto fetch_new_bucket_info =
        ...
        int ret =
            get_bucket_info(&svc, bs->bucket.tenant, bs->bucket.name, bucket_info, nullptr, y, dpp, &bucket_attrs);
        if (ret < 0) {
            ldpp_dout(dpp, 0) << __func__ << " ERROR: failed to refresh bucket info after reshard at " << log_tag
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << __func__ << " ERROR: failed to refresh bucket shard generation after reshard at "
                              << log_tag << ": " << cpp_strerror(-ret) << dendl;
            return ret;
        }

        const auto gen = bucket_info.layout.logs.empty() ? -1 : bucket_info.layout.logs.back().gen;
        ldpp_dout(dpp, 20) << __func__ << " INFO: refreshed bucket info after reshard at " << log_tag
                           << ". new shard_id=" << bs->shard_id << ". gen=" << gen << dendl;
    ...
    for (int i = 1; i <= num_retries; i++) {   // nb: 1-based for loop
            ...
            if (ret < 0) {
                ldpp_dout(dpp, 0) << "ERROR: " << __func__
                                  << " failed to refresh bucket info after reshard when get bucket "
                                     "resharding failed, error: "
                                  << cpp_strerror(-ret) << dendl;
                return ret;
            }
        }
        else if (ret < 0) {
            ldpp_dout(dpp, 0) << __func__ << " ERROR: failed to get bucket resharding : " << cpp_strerror(-ret)
                              << dendl;
            return ret;
        }
        ...
        // and we're free to clear the flags.
        {
            ...
            if (ret == -ENOENT) {
                ...
                ldpp_dout(dpp, 20) << __func__ << " ERROR: failed to take reshard lock for bucket " << bucket_id
                                   << "; expected if resharding underway" << dendl;
            }
            else {
                ldpp_dout(dpp, 10) << __func__ << " INFO: was able to take reshard lock for bucket " << bucket_id
                                   << dendl;
                ...
                if (ret == -ENOENT) {
                    ...
                    ldpp_dout(dpp, 0) << __func__ << " ERROR: failed to clear resharding flags for bucket " << bucket_id
                                      << ", " << cpp_strerror(-ret) << dendl;
...
int RGWRados::bucket_index_link_olh(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info, RGWObjState& olh_state,
                                    const rgw_obj& obj_instance, bool delete_marker, const string& op_tag,
                                    struct rgw_bucket_dir_entry_meta* meta, uint64_t olh_epoch, real_time unmod_since,
                                    bool high_precision_time, optional_yield y, rgw_zone_set* _zones_trace,
                                    bool log_data_change)
{
    ...
    r = guard_reshard(dpp, &bs, obj_instance, bucket_info, [&](BucketShard* bs) -> int {
        cls_rgw_obj_key key(obj_instance.key.get_index_key_name(), obj_instance.key.instance);
        auto& ref = bs->bucket_obj.get_ref();
        librados::ObjectWriteOperation op;
        op.assert_exists();   // bucket index shard must exist
        cls_rgw_guard_bucket_resharding(op, -ERR_BUSY_RESHARDING);
        cls_rgw_bucket_link_olh(op,
                                key,
                                olh_state.olh_tag,
                                delete_marker,
                                op_tag,
...
void RGWRados::bucket_index_guard_olh_op(const DoutPrefixProvider* dpp, RGWObjState& olh_state, ObjectOperation& op)
{
    ldpp_dout(dpp, 20) << __func__
                       << "(): olh_state.olh_tag=" << string(olh_state.olh_tag.c_str(), olh_state.olh_tag.length())
                       << dendl;
                                           ...
                                           uint64_t olh_epoch, rgw_zone_set* _zones_trace)
{
    ...
    r = guard_reshard(dpp, &bs, obj_instance, bucket_info, [&](BucketShard* bs) -> int {
        ...
        op.assert_exists();   // bucket index shard must exist
        cls_rgw_guard_bucket_resharding(op, -ERR_BUSY_RESHARDING);
        cls_rgw_bucket_unlink_instance(op, key, op_tag, olh_tag, olh_epoch, svc.zone->need_to_log_data(), zones_trace);
        return rgw_rados_operate(dpp, ref.pool.ioctx(), ref.obj.oid, &op, null_yield);
    });
    if (r < 0) {
...
int RGWRados::bucket_index_read_olh_log(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info, RGWObjState& state,
                                        ...
                                        bool* is_truncated)
{
    ...
    BucketShard bs(this);
    int ret = bs.init(obj_instance.bucket, obj_instance, nullptr /* no RGWBucketInfo */, dpp);
    if (ret < 0) {
                         ...
                         const rgw_obj& obj)
{
    ...
    ldpp_dout(dpp, 4) << "repair_olh setting olh_tag=" << olh.tag << " key=" << olh.key
                      << " delete_marker=" << olh.delete_marker << dendl;
    ...
    r = rgw_rados_operate(dpp, ref.pool.ioctx(), ref.obj.oid, &op, null_yield);
    if (r < 0) {
        ldpp_dout(dpp, 0) << "repair_olh failed to write olh attributes with " << cpp_strerror(r) << dendl;
        return r;
    }
    return 0;
}

int RGWRados::bucket_index_trim_olh_log(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info, RGWObjState& state,
                                        const rgw_obj& obj_instance, uint64_t ver)
{
    ...
    ret = guard_reshard(dpp, &bs, obj_instance, bucket_info, [&](BucketShard* pbs) -> int {
        ObjectWriteOperation op;
        op.assert_exists();   // bucket index shard must exist
        cls_rgw_guard_bucket_resharding(op, -ERR_BUSY_RESHARDING);
...
int RGWRados::bucket_index_clear_olh(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info,
                                     const std::string& olh_tag, const rgw_obj& obj_instance)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(dpp, bucket_info, obj_instance, &ref);
    if (r < 0) {
    ...
    int ret = guard_reshard(dpp, &bs, obj_instance, bucket_info, [&](BucketShard* pbs) -> int {
        ObjectWriteOperation op;
        op.assert_exists();   // bucket index shard must exist
...
int RGWRados::apply_olh_log(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, RGWObjState& state,
                            RGWBucketInfo& bucket_info, const rgw_obj& obj, bufferlist& olh_tag,
                            std::map<uint64_t, std::vector<rgw_bucket_olh_log_entry>>& log, uint64_t* plast_ver,
                            rgw_zone_set* zones_trace)
{
    if (log.empty()) {
        return 0;
    ...
    *plast_ver = last_ver;
    ...
    for (iter = log.begin(); iter != log.end(); ++iter) {
        ...
        for (; viter != iter->second.end(); ++viter) {
            ...
            switch (entry.op) {
            case CLS_RGW_OLH_OP_REMOVE_INSTANCE: remove_instances.push_back(entry.key); break;
            case CLS_RGW_OLH_OP_LINK_OLH:
                // only overwrite a link of the same epoch if its key sorts before
                if (link_epoch < iter->first || key.instance.empty() || key.instance > entry.key.instance) {
                    ldpp_dout(dpp, 20) << "apply_olh_log applying key=" << entry.key << " epoch=" << iter->first
                                       << " delete_marker=" << entry.delete_marker << " over current=" << key
                                       << " epoch=" << link_epoch << " delete_marker=" << delete_marker << dendl;
                    need_to_link = true;
                    ...
                    ldpp_dout(dpp, 20) << "apply_olh skipping key=" << entry.key << " epoch=" << iter->first
                                       << " delete_marker=" << entry.delete_marker << " before current=" << key
                                       << " epoch=" << link_epoch << " delete_marker=" << delete_marker << dendl;
                }
                break;
            case CLS_RGW_OLH_OP_UNLINK_OLH:
                ...
                need_to_link = false;
                break;
            default: ldpp_dout(dpp, 0) << "ERROR: apply_olh_log: invalid op: " << (int)entry.op << dendl; return -EIO;
            }
            string attr_name = RGW_ATTR_OLH_PENDING_PREFIX;
    ...
    for (list<cls_rgw_obj_key>::iterator liter = remove_instances.begin(); liter != remove_instances.end(); ++liter) {
        cls_rgw_obj_key& key = *liter;
        rgw_obj obj_instance(bucket, key);
        int ret = delete_obj(
            dpp, obj_ctx, bucket_info, obj_instance, 0, RGW_BILOG_FLAG_VERSIONED_OP, ceph::real_time(), zones_trace);
        if (ret < 0 && ret != -ENOENT) {
...
int RGWRados::clear_olh(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, const rgw_obj& obj,
                        RGWBucketInfo& bucket_info, const std::string& tag, const uint64_t ver, optional_yield y)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(dpp, bucket_info, obj, &ref);
    if (r < 0) {
...
int RGWRados::clear_olh(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, const rgw_obj& obj,
                        RGWBucketInfo& bucket_info, rgw_rados_ref& ref, const std::string& tag, const uint64_t ver,
                        optional_yield y)
{
    ObjectWriteOperation rm_op;
    ...
    cls_obj_check_prefix_exist(
        rm_op, RGW_ATTR_OLH_PENDING_PREFIX, true); /* fail if found one of these, pending modification */
    rm_op.remove();
 ...
 */
int RGWRados::update_olh(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, RGWObjState* state,
                         RGWBucketInfo& bucket_info, const rgw_obj& obj, rgw_zone_set* zones_trace)
{
...
int RGWRados::set_olh(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info,
                      const rgw_obj& target_obj, bool delete_marker, rgw_bucket_dir_entry_meta* meta,
                      uint64_t olh_epoch, real_time unmod_since, bool high_precision_time, optional_yield y,
...
#define MAX_ECANCELED_RETRY 100
    for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 20) << "olh_init_modification() target_obj=" << target_obj
                               << " delete_marker=" << (int)delete_marker << " returned " << ret << dendl;
            if (ret == -ECANCELED) {
                continue;
            }
            return ret;
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 20) << "bucket_index_link_olh() target_obj=" << target_obj
                               << " delete_marker=" << (int)delete_marker << " returned " << ret << dendl;
...
int RGWRados::unlink_obj_instance(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info,
                                  const rgw_obj& target_obj, uint64_t olh_epoch, optional_yield y,
                                  rgw_zone_set* zones_trace)
{
    string op_tag;
    ...
    for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
        ...
        if (ret < 0) {
            olh_cancel_modification(dpp, bucket_info, *state, olh_obj, op_tag, y);
            ldpp_dout(dpp, 20) << "bucket_index_unlink_instance() target_obj=" << target_obj << " returned " << ret
                               << dendl;
            if (ret == -ECANCELED) {
                continue;
            }
...
#define OBJ_INSTANCE_LEN 32
    ...
    gen_rand_alphanumeric_no_underscore(
        cct, buf, OBJ_INSTANCE_LEN); /* don't want it to get url escaped,
...
int RGWRados::remove_olh_pending_entries(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
...
int RGWRados::follow_olh(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info, RGWObjectCtx& obj_ctx,
                         RGWObjState* state, const rgw_obj& olh_obj, rgw_obj* target)
{
    ...
    if (!pending_entries.empty()) {
        ldpp_dout(dpp, 20) << __func__ << "(): found pending entries, need to update_olh() on bucket=" << olh_obj.bucket
                           << dendl;

        int ret = update_olh(dpp, obj_ctx, state, bucket_info, olh_obj);
        if (ret < 0) {
            if (ret == -ECANCELED) {
                // In this context, ECANCELED means that the OLH tag changed in either the bucket index entry or the OLH
                // object. If the OLH tag changed, it indicates that a previous OLH entry was removed since this request
                // started. We return ENOENT to indicate that the OLH object was removed.
                ret = -ENOENT;
            }
            return ret;
        }
...
int RGWRados::raw_obj_stat(const DoutPrefixProvider* dpp, rgw_raw_obj& obj, uint64_t* psize, real_time* pmtime,
                           ...
                           RGWObjVersionTracker* objv_tracker, optional_yield y)
{
    ...
    if (psize) *psize = size;
    if (pmtime) *pmtime = ceph::real_clock::from_timespec(mtime_ts);
    if (attrs) {
...
int RGWRados::get_bucket_stats(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info,
                               const rgw::bucket_index_layout_generation& idx_layout, int shard_id, string* bucket_ver,
                               string* master_ver, map<RGWObjCategory, RGWStorageStats>& stats, string* max_marker,
                               bool* syncstopped)
{
    ...
    for (; iter != headers.end(); ++iter, ++viter) {
        ...
        if (syncstopped != NULL) *syncstopped = iter->syncstopped;
    }
...
int RGWRados::get_bucket_stats_async(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info,
                                     const rgw::bucket_index_layout_generation& idx_layout, int shard_id,
                                     RGWGetBucketStats_CB* ctx)
{
    int num_aio = 0;
    RGWGetBucketStatsContext* get_ctx =
        new RGWGetBucketStatsContext(ctx, bucket_info.layout.current_index.layout.normal.num_shards ?: 1);
    ceph_assert(get_ctx);
...
int RGWRados::get_bucket_instance_info(const string& meta_key, RGWBucketInfo& info, real_time* pmtime,
                                       map<string, bufferlist>* pattrs, optional_yield y, const DoutPrefixProvider* dpp)
{
    rgw_bucket bucket;
...
int RGWRados::get_bucket_instance_info(const rgw_bucket& bucket, RGWBucketInfo& info, real_time* pmtime,
                                       map<string, bufferlist>* pattrs, optional_yield y, const DoutPrefixProvider* dpp)
{
    return ctl.bucket->read_bucket_instance_info(
        bucket, &info, y, dpp, RGWBucketCtl::BucketInstance::GetParams().set_mtime(pmtime).set_attrs(pattrs));
}

int RGWRados::get_bucket_info(RGWServices* svc, const string& tenant, const string& bucket_name, RGWBucketInfo& info,
                              real_time* pmtime, optional_yield y, const DoutPrefixProvider* dpp,
                              map<string, bufferlist>* pattrs)
{
    rgw_bucket bucket;
    bucket.tenant = tenant;
    bucket.name = bucket_name;
    return ctl.bucket->read_bucket_info(
        bucket, &info, y, dpp, RGWBucketCtl::BucketInstance::GetParams().set_mtime(pmtime).set_attrs(pattrs));
}

int RGWRados::try_refresh_bucket_info(RGWBucketInfo& info, ceph::real_time* pmtime, const DoutPrefixProvider* dpp,
                                      map<string, bufferlist>* pattrs)
{
    ...
    return ctl.bucket->read_bucket_info(
        ...
        null_yield,
        dpp,
        RGWBucketCtl::BucketInstance::GetParams().set_mtime(pmtime).set_attrs(pattrs).set_refresh_version(rv));
}

int RGWRados::put_bucket_instance_info(RGWBucketInfo& info, bool exclusive, real_time mtime,
                                       map<string, bufferlist>* pattrs, const DoutPrefixProvider* dpp, optional_yield y)
{
    return ctl.bucket->store_bucket_instance_info(
        info.bucket,
        info,
        y,
        dpp,
        RGWBucketCtl::BucketInstance::PutParams().set_exclusive(exclusive).set_mtime(mtime).set_attrs(pattrs));
}
                                     ...
                                     const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    ret = ctl.bucket->store_bucket_entrypoint_info(
        info.bucket,
        entry_point,
        y,
        dpp,
        RGWBucketCtl::Bucket::PutParams().set_exclusive(exclusive).set_objv_tracker(&ot).set_mtime(mtime));
    if (ret < 0) return ret;
...
int RGWRados::pool_iterate_begin(const DoutPrefixProvider* dpp, const rgw_pool& pool, const string& cursor,
                                 RGWPoolIterCtx& ctx)
{
    librados::IoCtx& io_ctx = ctx.io_ctx;
    librados::NObjectIterator& iter = ctx.iter;
...
static int do_pool_iterate(const DoutPrefixProvider* dpp, CephContext* cct, RGWPoolIterCtx& ctx, uint32_t num,
                           vector<rgw_bucket_dir_entry>& objs, bool* is_truncated, RGWAccessListFilter* filter)
{
    librados::IoCtx& io_ctx = ctx.io_ctx;
    librados::NObjectIterator& iter = ctx.iter;

    if (iter == io_ctx.nobjects_end()) return -ENOENT;
    ...
    for (i = 0; i < num && iter != io_ctx.nobjects_end(); ++i, ++iter) {
        ...
        if (filter && !filter->filter(oid, oid)) continue;

        e.key = oid;
        objs.push_back(e);
    }

    if (is_truncated) *is_truncated = (iter != io_ctx.nobjects_end());

    return objs.size();
}

int RGWRados::pool_iterate(const DoutPrefixProvider* dpp, RGWPoolIterCtx& ctx, uint32_t num,
                           vector<rgw_bucket_dir_entry>& objs, bool* is_truncated, RGWAccessListFilter* filter)
{
    ...
    try {
        ...
        int r = -e.code().value();
        ldpp_dout(dpp, 10) << "NObjectIterator threw exception " << e.what() << ", returning " << r << dendl;
        return r;
    }
    catch (const std::exception& e) {
        ldpp_dout(dpp, 10) << "NObjectIterator threw exception " << e.what() << ", returning -5" << dendl;
        return -EIO;
    }
}

int RGWRados::list_raw_objects_init(const DoutPrefixProvider* dpp, const rgw_pool& pool, const string& marker,
...
int RGWRados::list_raw_objects_next(const DoutPrefixProvider* dpp, const string& prefix_filter, int max,
                                    RGWListRawObjsCtx& ctx, list<string>& oids, bool* is_truncated)
{
    if (!ctx.initialized) {
        return -EINVAL;
    }
    ...
    if (r < 0) {
        if (r != -ENOENT) ldpp_dout(dpp, 10) << "failed to list objects pool_iterate returned r=" << r << dendl;
        return r;
    }
...
int RGWRados::list_raw_objects(const DoutPrefixProvider* dpp, const rgw_pool& pool, const string& prefix_filter,
                               int max, RGWListRawObjsCtx& ctx, list<string>& oids, bool* is_truncated)
{
    if (!ctx.initialized) {
...
int RGWRados::bi_list(const DoutPrefixProvider* dpp, rgw_bucket& bucket, const string& obj_name_filter,
                      const string& marker, uint32_t max, list<rgw_cls_bi_entry>* entries, bool* is_truncated)
{
...
int RGWRados::bi_list(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, int shard_id,
                      const string& obj_name_filter, const string& marker, uint32_t max,
                      list<rgw_cls_bi_entry>* entries, bool* is_truncated)
{
    BucketShard bs(this);
    int ret = bs.init(dpp, bucket_info, bucket_info.layout.current_index, shard_id);
    if (ret < 0) {
        ldpp_dout(dpp, 5) << "bs.init() returned ret=" << ret << dendl;
        return ret;
    }
...
int RGWRados::gc_aio_operate(const string& oid, librados::AioCompletion* c, librados::ObjectWriteOperation* op)
{
...
int RGWRados::list_gc_objs(int* index, string& marker, uint32_t max, bool expired_only,
                           std::list<cls_rgw_gc_obj_info>& result, bool* truncated, bool& processing_queue)
{
...
int RGWRados::list_lc_progress(string& marker, uint32_t max_entries,
                               vector<std::unique_ptr<rgw::sal::Lifecycle::LCEntry>>& progress_map, int& index)
{
                                 ...
                                 rgw_obj& obj, uint16_t bilog_flags, optional_yield y, rgw_zone_set* _zones_trace)
{
    ...
    ldout_bitx(bitx, dpp, 10) << "ENTERING " << __func__ << ": bucket-shard=" << bs << " obj=" << obj << " tag=" << tag
                              << " op=" << op << dendl_bitx;
    ...
    cls_rgw_obj_key key(obj.key.get_index_key_name(), obj.key.instance);
    cls_rgw_guard_bucket_resharding(o, -ERR_BUSY_RESHARDING);
    cls_rgw_bucket_prepare_op(
        o, op, tag, key, obj.key.get_loc(), svc.zone->need_to_log_data(), bilog_flags, zones_trace);
    int ret = bs.bucket_obj.operate(dpp, &o, y);
    ldout_bitx(bitx, dpp, 10) << "EXITING " << __func__ << ": ret=" << ret << dendl_bitx;
    return ret;
}

int RGWRados::cls_obj_complete_op(BucketShard& bs, const rgw_obj& obj, RGWModifyOp op, string& tag, int64_t pool,
                                  uint64_t epoch, rgw_bucket_dir_entry& ent, RGWObjCategory category,
                                  list<rgw_obj_index_key>* remove_objs, uint16_t bilog_flags,
                                  rgw_zone_set* _zones_trace)
{
    const bool bitx = cct->_conf->rgw_bucket_index_transaction_instrumentation;
    ldout_bitx_c(bitx, cct, 10) << "ENTERING " << __func__ << ": bucket-shard=" << bs << " obj=" << obj
                                << " tag=" << tag << " op=" << op
    ...
    cls_rgw_bucket_complete_op(
        o, op, tag, ver, key, dir_meta, remove_objs, svc.zone->need_to_log_data(), bilog_flags, &zones_trace);
...
int RGWRados::cls_obj_complete_add(BucketShard& bs, const rgw_obj& obj, string& tag, int64_t pool, uint64_t epoch,
                                   rgw_bucket_dir_entry& ent, RGWObjCategory category,
                                   list<rgw_obj_index_key>* remove_objs, uint16_t bilog_flags,
                                   rgw_zone_set* zones_trace)
{
    return cls_obj_complete_op(
        bs, obj, CLS_RGW_OP_ADD, tag, pool, epoch, ent, category, remove_objs, bilog_flags, zones_trace);
}

int RGWRados::cls_obj_complete_del(BucketShard& bs, string& tag, int64_t pool, uint64_t epoch, rgw_obj& obj,
                                   real_time& removed_mtime, list<rgw_obj_index_key>* remove_objs, uint16_t bilog_flags,
                                   rgw_zone_set* zones_trace)
{
    rgw_bucket_dir_entry ent;
    ...
    obj.key.get_index_key(&ent.key);
    return cls_obj_complete_op(
        bs, obj, CLS_RGW_OP_DEL, tag, pool, epoch, ent, RGWObjCategory::None, remove_objs, bilog_flags, zones_trace);
}

int RGWRados::cls_obj_complete_cancel(BucketShard& bs, string& tag, rgw_obj& obj, list<rgw_obj_index_key>* remove_objs,
                                      uint16_t bilog_flags, rgw_zone_set* zones_trace)
{
    rgw_bucket_dir_entry ent;
    obj.key.get_index_key(&ent.key);
    return cls_obj_complete_op(bs,
                               obj,
                               CLS_RGW_OP_CANCEL,
                               tag,
                               -1 /* pool id */,
...
int RGWRados::cls_obj_set_bucket_tag_timeout(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info,
                                             uint64_t timeout)
{
    RGWSI_RADOS::Pool index_pool;
    map<int, string> bucket_objs;
...
uint32_t RGWRados::calc_ordered_bucket_list_per_shard(uint32_t num_entries, uint32_t num_shards)
{
    if (num_shards == 0) {
    ...
    uint32_t calc_read =
        1 + static_cast<uint32_t>((num_entries / num_shards) + sqrt((2 * num_entries) * log(num_shards) / num_shards));

    return std::max(min_read, calc_read);
}


int RGWRados::cls_bucket_list_ordered(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info,
                                      const rgw::bucket_index_layout_generation& idx_layout, const int shard_id,
                                      const rgw_obj_index_key& start_after, const std::string& prefix,
                                      const std::string& delimiter, const uint32_t num_entries,
                                      const bool list_versions, const uint16_t expansion_factor, ent_map_t& m,
                                      bool* is_truncated, bool* cls_filtered, rgw_obj_index_key* last_entry,
                                      optional_yield y, RGWBucketListNameFilter force_check_filter)
{
    const bool bitx = cct->_conf->rgw_bucket_index_transaction_instrumentation;
    ...
    ldout_bitx(bitx, dpp, 10) << "ENTERING " << __func__ << ": " << bucket_info.bucket << " start_after=\""
                              << start_after.to_string() << "\", prefix=\"" << prefix << ", delimiter=\"" << delimiter
                              << "\", shard_id=" << shard_id << "\", num_entries=" << num_entries
                              << ", shard_id=" << shard_id << ", list_versions=" << list_versions
                              << ", expansion_factor=" << expansion_factor << ", force_check_filter is "
    ...
    int r = svc.bi_rados->open_bucket_index(dpp, bucket_info, shard_id, idx_layout, &index_pool, &shard_oids, nullptr);
    if (r < 0) {
        ldpp_dout(dpp, 0) << __func__ << ": open_bucket_index for " << bucket_info.bucket << " failed" << dendl;
        return r;
    }
    ...
    if (expansion_factor == 0) {
        num_entries_per_shard = calc_ordered_bucket_list_per_shard(num_entries, shard_count);
    }
    else if (expansion_factor <= 11) {
        // we'll max out the exponential multiplication factor at 1024 (2<<10)
        num_entries_per_shard = std::min(
            num_entries,
            (uint32_t(1 << (expansion_factor - 1)) * calc_ordered_bucket_list_per_shard(num_entries, shard_count)));
    }
    else {
    ...
    ldpp_dout(dpp, 10) << __func__ << ": request from each of " << shard_count << " shard(s) for "
                       << num_entries_per_shard << " entries to get " << num_entries << " total entries" << dendl;
    ...
    if (r < 0) {
        ldpp_dout(dpp, 0) << __func__ << ": CLSRGWIssueBucketList for " << bucket_info.bucket << " failed" << dendl;
        return r;
    }
    ...
    struct ShardTracker
    {
        ...
        // accessors
        ShardTracker(size_t _shard_idx, rgw_cls_list_ret& _result, const std::string& _oid_name)
        ...
        inline const std::string& entry_name() const { return cursor->first; }
        rgw_bucket_dir_entry& dir_entry() const { return cursor->second; }
        inline bool is_truncated() const { return result.is_truncated; }
        ...
        inline bool at_end() const { return cursor == end; }
    };   // ShardTracker

    // add the next unique candidate, or return false if we reach the end
    auto next_candidate =
        [](CephContext* cct, ShardTracker& t, std::multimap<std::string, size_t>& candidates, size_t tracker_idx) {
            if (!t.at_end()) {
    ...
    rgw_bucket_dir_entry* last_entry_visited = nullptr;   // to set last_entry (marker)
    ...
    while (count < num_entries && !candidates.empty()) {
        ...
        ldpp_dout(dpp, 20) << __func__ << ": currently processing " << dirent.key << " from shard " << tracker.shard_idx
                           << dendl;

        const bool force_check = force_check_filter && force_check_filter(dirent.key.name);

        if ((!dirent.exists && !dirent.is_delete_marker() && !dirent.is_common_prefix()) ||
            !dirent.pending_map.empty() || force_check) {
            ...
            r = check_disk_state(dpp, sub_ctx, bucket_info, dirent, dirent, updates[tracker.oid_name], y);
            if (r < 0 && r != -ENOENT) {
                ldpp_dout(dpp, 0) << __func__ << ": check_disk_state for \"" << dirent.key << "\" failed with r=" << r
                                  << dendl;
                return r;
            }
        }
        ...
        if (r >= 0) {   // i.e., if r != -ENOENT
            ldpp_dout(dpp, 10) << __func__ << ": got " << dirent.key << dendl;
            ...
            if (inserted) {
                ...
                ldpp_dout(dpp, 0) << "WARNING: " << __func__ << " reassigned map value at \"" << name
                                  << "\", which should not happen" << dendl;
            }
        }
        else {
            ldpp_dout(dpp, 10) << __func__ << ": skipping " << dirent.key.name << "[" << dirent.key.instance << "]"
                               << dendl;
        ...
        if (need_to_stop) {
            ...
            ldpp_dout(dpp, 10) << __func__ << ": stopped accumulating results at count=" << count << ", dirent=\""
                               << dirent.key << "\", because its shard is truncated and exhausted" << dendl;
            break;
        }
    ...
    for (auto& miter : updates) {
        if (miter.second.length()) {
            ...
            ldout_bitx(bitx, dpp, 10) << "INFO: " << __func__ << ": doing dir_suggest on " << miter.first << dendl_bitx;
    ...
    *is_truncated = false;
    ...
    ldpp_dout(dpp, 20) << __func__ << ": returning, count=" << count << ", is_truncated=" << *is_truncated << dendl;

    if (*is_truncated && count < num_entries) {
        ldpp_dout(dpp, 10) << __func__ << ": requested " << num_entries << " entries but returning " << count
                           << ", which is truncated" << dendl;
    }

    if (last_entry_visited != nullptr && last_entry) {
        *last_entry = last_entry_visited->key;
        ldpp_dout(dpp, 20) << __func__ << ": returning, last_entry=" << *last_entry << dendl;
    }
    else {
        ldpp_dout(dpp, 20) << __func__ << ": returning, last_entry NOT SET" << dendl;
    }
...
int RGWRados::cls_bucket_list_unordered(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info,
                                        const rgw::bucket_index_layout_generation& idx_layout, int shard_id,
                                        const rgw_obj_index_key& start_after, const std::string& prefix,
                                        uint32_t num_entries, bool list_versions,
                                        std::vector<rgw_bucket_dir_entry>& ent_list, bool* is_truncated,
                                        rgw_obj_index_key* last_entry, optional_yield y,
                                        RGWBucketListNameFilter force_check_filter)
{
    const bool bitx = cct->_conf->rgw_bucket_index_transaction_instrumentation;

    ldout_bitx(bitx, dpp, 10) << "ENTERING " << __func__ << ": " << bucket_info.bucket << " start_after=\""
                              << start_after << "\", prefix=\"" << prefix << "\", shard_id=" << shard_id
    ...
    *is_truncated = false;
    ...
    if (shard_id >= 0) {
        ...
        if (!parsed) {
            ldpp_dout(dpp, 0) << "ERROR: " << __func__ << " received an invalid start marker: \"" << start_after << "\""
                              << dendl;
            return -EINVAL;
        }
            ...
            if (obj_key.ns == RGW_OBJ_NS_MULTIPART) {
                ...
                if (r < 0) {
                    ldpp_dout(dpp, 0) << "ERROR: " << __func__ << " parse_index_hash_source unable to parse \""
                                      << obj_key.name << "\", r=" << r << dendl;
                    return r;
                }
    ...
    while (count <= num_entries &&
           ((shard_id >= 0 && current_shard == uint32_t(shard_id)) || current_shard < num_shards)) {
        const std::string& oid = oids[current_shard];
        rgw_cls_list_ret result;

        librados::ObjectReadOperation op;
        const std::string empty_delimiter;
        cls_rgw_bucket_list_op(op, marker, prefix, empty_delimiter, num_entries, list_versions, &result);
        r = rgw_rados_operate(dpp, ioctx, oid, &op, nullptr, null_yield);
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": error in rgw_rados_operate (bucket list op), r=" << r
                              << dendl;
            return r;
        }
        ...
        for (auto& entry : result.dir.m) {
            ...
            if ((!dirent.exists && !dirent.is_delete_marker()) || !dirent.pending_map.empty() || force_check) {
                ...
                librados::IoCtx sub_ctx;
                sub_ctx.dup(ioctx);
                ldout_bitx(bitx, dpp, 20)
                    << "INFO: " << __func__ << ": calling check_disk_state bucket=" << bucket_info.bucket
                    << " entry=" << dirent.key << dendl_bitx;
                r = check_disk_state(dpp, sub_ctx, bucket_info, dirent, dirent, updates[oid], y);
                if (r < 0 && r != -ENOENT) {
                    ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": error in check_disk_state, r=" << r << dendl;
                    return r;
                }
            }
            else {
            ...
            if (r >= 0) {   // i.e., if r != -ENOENT
                ...
                if (count < num_entries) {
                    ...
                    *is_truncated = true;
                    ldpp_dout(dpp, 10) << "INFO: " << __func__ << ": reached max entries (" << num_entries
                                       << ") to return at \"" << dirent.key << "\"" << dendl;
                    goto check_updates;
                }
            }
            else {   // r == -ENOENT
...
int RGWRados::cls_obj_usage_log_add(const DoutPrefixProvider* dpp, const string& oid, rgw_usage_log_info& info)
{
...
int RGWRados::cls_obj_usage_log_read(const DoutPrefixProvider* dpp, const string& oid, const string& user,
                                     const string& bucket, uint64_t start_epoch, uint64_t end_epoch,
                                     ...
                                     map<rgw_user_bucket, rgw_usage_log_entry>& usage, bool* is_truncated)
{
    ...
    *is_truncated = false;
    ...
    r = cls_rgw_usage_log_read(ref.pool.ioctx(),
                               ref.obj.oid,
                               user,
                               bucket,
                               start_epoch,
...
static int cls_rgw_usage_log_trim_repeat(const DoutPrefixProvider* dpp, rgw_rados_ref ref, const string& user,
                                         const string& bucket, uint64_t start_epoch, uint64_t end_epoch)
{
    bool done = false;
    do {
...
int RGWRados::cls_obj_usage_log_trim(const DoutPrefixProvider* dpp, const string& oid, const string& user,
                                     const string& bucket, uint64_t start_epoch, uint64_t end_epoch)
{
    rgw_raw_obj obj(svc.zone->get_zone_params().usage_log_pool, oid);
...
int RGWRados::remove_objs_from_index(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info,
                                     const std::list<rgw_obj_index_key>& entry_key_list)
{
    ...
    int r = svc.bi_rados->open_bucket_index(
        dpp, bucket_info, std::nullopt, bucket_info.layout.current_index, &index_pool, &index_oids, nullptr);
    if (r < 0) {
        ldout_bitx(bitx, dpp, 0) << "ERROR: " << __func__ << " open_bucket_index returned " << r << dendl_bitx;
        return r;
    }
    ...
    for (const auto& entry_key : entry_key_list) {
        ...
        const uint32_t shard = RGWSI_BucketIndex_RADOS::bucket_shard_index(obj_key, num_shards);
        ...
        ldout_bitx(bitx, dpp, 20) << "INFO: " << __func__
                                  ...
                                  << " key=" << combined_key << " designated for shard " << shard << dendl_bitx;
    }
    ...
    for (const auto& removals : sharded_removals) {
        ...
        ldout_bitx(bitx, dpp, 10) << "INFO: " << __func__
                                  << ": removal from bucket index, bucket=" << bucket_info.bucket << ", shard=" << shard
                                  << ", oid=" << oid << ", num_keys=" << removals.second.size() << dendl_bitx;

        r = index_pool.ioctx().omap_rm_keys(oid, removals.second);
        if (r < 0) {
            ldout_bitx(bitx, dpp, 0) << "ERROR: " << __func__ << ": omap_rm_keys returned ret=" << r << dendl_bitx;
            return r;
        }
    }

    ldout_bitx(bitx, dpp, 5) << "EXITING " << __func__ << " and returning " << r << dendl_bitx;

    return r;
}

int RGWRados::check_disk_state(const DoutPrefixProvider* dpp, librados::IoCtx io_ctx, RGWBucketInfo& bucket_info,
                               rgw_bucket_dir_entry& list_state, rgw_bucket_dir_entry& object,
                               bufferlist& suggested_updates, optional_yield y)
{
    const bool bitx = cct->_conf->rgw_bucket_index_transaction_instrumentation;
    ldout_bitx(bitx, dpp, 10) << "ENTERING " << __func__ << ": bucket=" << bucket_info.bucket
                              << " dir_entry=" << list_state.key << dendl_bitx;
    ...
    if (loc != list_state.locator) {
        ldpp_dout(dpp, 0) << "WARNING: generated locator (" << loc << ") is different from listed locator ("
                          << list_state.locator << ")" << dendl;
    }
    ...
    if (!list_state.is_delete_marker() && !astate->exists) {
        ...
        ldout_bitx(bitx, dpp, 10) << "INFO: " << __func__ << ": encoding remove of " << list_state.key
                                  << " on suggested_updates" << dendl_bitx;
    ...
    if (r < 0) {
        ldpp_dout(dpp, 0) << __func__ << " WARNING: unable to find head object data pool for \"" << obj
    ...
    ldout_bitx(bitx, dpp, 10) << "INFO: " << __func__ << ": encoding update of " << list_state.key
                              << " on suggested_updates" << dendl_bitx;
                              ...
                              vector<rgw_bucket_dir_header>& headers, map<int, string>* bucket_instance_ids)
{
    ...
    int r = svc.bi_rados->open_bucket_index(
        dpp, bucket_info, shard_id, idx_layout, &index_pool, &oids, bucket_instance_ids);
    if (r < 0) {
        ldpp_dout(dpp, 20) << "cls_bucket_head: open_bucket_index() returned " << r << dendl;
        return r;
    }

    r = CLSRGWIssueGetDirHeader(index_pool.ioctx(), oids, list_results, cct->_conf->rgw_bucket_index_max_aio)();
    if (r < 0) {
        ldpp_dout(dpp, 20) << "cls_bucket_head: CLSRGWIssueGetDirHeader() returned " << r << dendl;
        return r;
    }
                                    ...
                                    RGWGetDirHeader_CB* ctx, int* num_aio)
{
    ...
    for (; iter != bucket_objs.end(); ++iter) {
        r = cls_rgw_get_dir_header_async(
            index_pool.ioctx(), iter->second, static_cast<RGWGetDirHeader_CB*>(ctx->get()));
        if (r < 0) {
            ctx->put();
            break;
        }
...
int RGWRados::check_bucket_shards(const RGWBucketInfo& bucket_info, const rgw_bucket& bucket, uint64_t num_objs,
                                  const DoutPrefixProvider* dpp)
{
    ...
    const uint32_t max_dynamic_shards = uint32_t(cct->_conf.get_val<uint64_t>("rgw_max_dynamic_shards"));
    ...
    const uint64_t max_objs_per_shard = cct->_conf.get_val<uint64_t>("rgw_max_objs_per_shard");

    // TODO: consider per-bucket sync policy here?
    const bool is_multisite = svc.zone->need_to_log_data();

    quota_handler->check_bucket_shards(
        dpp, max_objs_per_shard, num_source_shards, num_objs, is_multisite, need_resharding, &suggested_num_shards);
    if (!need_resharding) {
        return 0;
    }

    const uint32_t final_num_shards = RGWBucketReshard::get_preferred_shards(suggested_num_shards, max_dynamic_shards);
    ...
    ldpp_dout(dpp, 1) << "RGWRados::" << __func__ << " bucket " << bucket.name
                      << " needs resharding; current num shards "
                      << bucket_info.layout.current_index.layout.normal.num_shards << "; new num shards "
                      << final_num_shards << " (suggested " << suggested_num_shards << ")" << dendl;

    return add_bucket_to_reshard(dpp, bucket_info, final_num_shards);
}

int RGWRados::add_bucket_to_reshard(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                    uint32_t new_num_shards)
{
    ...
    if (new_num_shards <= num_source_shards) {
        ldpp_dout(dpp, 20) << "not resharding bucket name=" << bucket_info.bucket.name
                           << ", orig_num=" << num_source_shards << ", new_num_shards=" << new_num_shards << dendl;
        return 0;
    }
...
int RGWRados::check_quota(const DoutPrefixProvider* dpp, const rgw_user& bucket_owner, rgw_bucket& bucket,
                          RGWQuota& quota, uint64_t obj_size, optional_yield y, bool check_size_only)
{
    // if we only check size, then num_objs will set to 0
    if (check_size_only) return quota_handler->check_quota(dpp, bucket_owner, bucket, quota, 0, obj_size, y);

    return quota_handler->check_quota(dpp, bucket_owner, bucket, quota, 1, obj_size, y);
}

int RGWRados::get_target_shard_id(const rgw::bucket_index_normal_layout& layout, const string& obj_key, int* shard_id)
{
    int r = 0;
...
int RGWRados::delete_raw_obj_aio(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj,
...
int RGWRados::delete_obj_aio(const DoutPrefixProvider* dpp, const rgw_obj& obj, RGWBucketInfo& bucket_info,
                             RGWObjState* astate, list<librados::AioCompletion*>& handles, bool keep_index_consistent,
                             optional_yield y)
{
    rgw_rados_ref ref;
