...
struct ACLMapping
{
    ...
    ACLMapping(ACLGranteeTypeEnum t, const string& s, const string& d)
        : type(t)
        , source_id(s)
        , dest_id(d)
    {}
...
static int conf_to_uint64(const DoutPrefixProvider* dpp, CephContext* cct, const JSONFormattable& config,
                          const string& key, uint64_t* pval)
{
    ...
    if (config.find(key, &sval)) {
        ...
        if (!err.empty()) {
            ldpp_dout(dpp, 0) << "ERROR: could not parse configurable value for cloud sync module: " << key << ": "
                              << sval << dendl;
            return -EINVAL;
        }
        *pval = val;
...
struct AWSSyncConfig
{
    ...
    int init_profile(const DoutPrefixProvider* dpp, CephContext* cct, const JSONFormattable& profile_conf,
                     AWSSyncConfig_Profile& profile, bool connection_must_exist)
    {
        if (!profile.connection_id.empty()) {
            if (profile.conn_conf) {
                ldpp_dout(dpp, 0) << "ERROR: ambiguous profile connection configuration, connection_id="
                                  << profile.connection_id << dendl;
                return -EINVAL;
            }
            if (connections.find(profile.connection_id) == connections.end()) {
                ldpp_dout(dpp, 0) << "ERROR: profile configuration reference non-existent connection_id="
                                  << profile.connection_id << dendl;
                return -EINVAL;
            }
        ...
        if (!profile.acls_id.empty()) {
            if (!acl_profiles.find(profile.acls_id, &acl_mappings)) {
                ldpp_dout(dpp, 0) << "ERROR: profile configuration reference non-existent acls id=" << profile.acls_id
                                  << dendl;
                return -EINVAL;
            }
    ...
    int init_target(const DoutPrefixProvider* dpp, CephContext* cct, const JSONFormattable& profile_conf,
    ...
    bool do_find_profile(const rgw_bucket bucket, std::shared_ptr<AWSSyncConfig_Profile>* result)
    {
        ...
        if (!target->prefix && name.size() != iter->first.size()) {
            return false;
        }
    ...
    string get_path(std::shared_ptr<AWSSyncConfig_Profile>& profile, const RGWBucketInfo& bucket_info,
    ...
    void get_target(std::shared_ptr<AWSSyncConfig_Profile>& profile, const RGWBucketInfo& bucket_info,
                    const rgw_obj_key& obj, string* bucket_name, string* obj_name)
    {
...
struct AWSSyncInstanceEnv
{
    ...
    explicit AWSSyncInstanceEnv(AWSSyncConfig& _conf)
        : conf(_conf)
    {}
...
static int do_decode_rest_obj(const DoutPrefixProvider* dpp, CephContext* cct, map<string, bufferlist>& attrs,
                              map<string, string>& headers, rgw_rest_obj* info)
{
...
public:
    RGWRESTStreamGetCRF(CephContext* _cct, RGWCoroutinesEnv* _env, RGWCoroutine* _caller, RGWDataSyncCtx* _sc,
                        RGWRESTConn* _conn, const rgw_obj& _src_obj,
                        const rgw_sync_aws_src_obj_properties& _src_properties)
        : RGWStreamReadHTTPResourceCRF(_cct, _env, _caller, _sc->env->http_manager, _src_obj.key)
        , sc(_sc)
        , conn(_conn)
        , src_obj(_src_obj)
    ...
    int decode_rest_obj(const DoutPrefixProvider* dpp, map<string, string>& headers, bufferlist& extra_data) override
    {
        ...
        ldpp_dout(dpp, 20) << __func__ << ":"
                           << " headers=" << headers << " extra_data.length()=" << extra_data.length() << dendl;

        if (extra_data.length() > 0) {
            JSONParser jp;
            if (!jp.parse(extra_data.c_str(), extra_data.length())) {
                ldpp_dout(dpp, 0) << "ERROR: failed to parse response extra data. len=" << extra_data.length()
                                  << " data=" << extra_data.c_str() << dendl;
                return -EIO;
            }
    ...
    bool need_extra_data() override { return true; }
};
...
public:
    RGWAWSStreamPutCRF(CephContext* _cct, RGWCoroutinesEnv* _env, RGWCoroutine* _caller, RGWDataSyncCtx* _sc,
                       const rgw_sync_aws_src_obj_properties& _src_properties,
                       std::shared_ptr<AWSSyncConfig_Profile>& _target, const rgw_obj& _dest_obj)
        : RGWStreamWriteHTTPResourceCRF(_cct, _env, _caller, _sc->env->http_manager)
        , sc(_sc)
        , src_properties(_src_properties)
        , target(_target)
        , dest_obj(_dest_obj)
    ...
    {
        return (keep_headers.find(h) != keep_headers.end() || boost::algorithm::starts_with(h, "X_AMZ_"));
    }

    static void init_send_attrs(const DoutPrefixProvider* dpp, CephContext* cct, const rgw_rest_obj& rest_obj,
                                const rgw_sync_aws_src_obj_properties& src_properties,
                                const AWSSyncConfig_Profile* target, map<string, string>* attrs)
    {
...
public:
    RGWAWSStreamObjToCloudPlainCR(RGWDataSyncCtx* _sc, RGWRESTConn* _source_conn, const rgw_obj& _src_obj,
                                  const rgw_sync_aws_src_obj_properties& _src_properties,
                                  std::shared_ptr<AWSSyncConfig_Profile> _target, const rgw_obj& _dest_obj)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        ...
        {
            /* init input */
            in_crf.reset(new RGWRESTStreamGetCRF(cct, get_env(), this, sc, source_conn, src_obj, src_properties));

            /* init output */
            out_crf.reset(new RGWAWSStreamPutCRF(cct, get_env(), this, sc, src_properties, target, dest_obj));
...
public:
    RGWAWSStreamObjToCloudMultipartPartCR(RGWDataSyncCtx* _sc, RGWRESTConn* _source_conn, const rgw_obj& _src_obj,
                                          std::shared_ptr<AWSSyncConfig_Profile>& _target, const rgw_obj& _dest_obj,
                                          const rgw_sync_aws_src_obj_properties& _src_properties,
                                          const string& _upload_id, const rgw_sync_aws_multipart_part_info& _part_info,
                                          string* _petag)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , source_conn(_source_conn)
...
public:
    RGWAWSAbortMultipartCR(RGWDataSyncCtx* _sc, RGWRESTConn* _dest_conn, const rgw_obj& _dest_obj,
        ...
        reenter(this)
        {
            ...
            yield
            {
                ...
                call(new RGWDeleteRESTResourceCR(
                    sc->cct, dest_conn, sc->env->http_manager, obj_to_aws_path(dest_obj), params));
            }

            if (retcode < 0) {
                ldpp_dout(dpp, 0) << "ERROR: failed to abort multipart upload for dest object=" << dest_obj
...
public:
    RGWAWSInitMultipartCR(RGWDataSyncCtx* _sc, RGWRESTConn* _dest_conn, const rgw_obj& _dest_obj, uint64_t _obj_size,
                          const map<string, string>& _attrs, string* _upload_id)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , dest_conn(_dest_conn)
        , dest_obj(_dest_obj)
        ...
        reenter(this)
        {
            ...
            yield
            {
                ...
                call(new RGWPostRawRESTResourceCR<bufferlist>(
                    sc->cct, dest_conn, sc->env->http_manager, obj_to_aws_path(dest_obj), params, &attrs, bl, &out_bl));
            }

            if (retcode < 0) {
                ldpp_dout(dpp, 0) << "ERROR: failed to initialize multipart upload for dest object=" << dest_obj
                                  << dendl;
                return set_cr_error(retcode);
            }
            {
...
class RGWAWSCompleteMultipartCR : public RGWCoroutine
{
    ...
    struct CompleteMultipartReq
    {
        ...
        explicit CompleteMultipartReq(const map<int, rgw_sync_aws_multipart_part_info>& _parts)
            : parts(_parts)
        {}
...
public:
    RGWAWSCompleteMultipartCR(RGWDataSyncCtx* _sc, RGWRESTConn* _dest_conn, const rgw_obj& _dest_obj, string _upload_id,
                              const map<int, rgw_sync_aws_multipart_part_info>& _parts)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , dest_conn(_dest_conn)
        , dest_obj(_dest_obj)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            yield
            {
                ...
                call(new RGWPostRawRESTResourceCR<bufferlist>(sc->cct,
                                                              dest_conn,
                                                              sc->env->http_manager,
            ...
            ldpp_dout(dpp, 20) << "complete multipart result: location=" << result.location
                               << " bucket=" << result.bucket << " key=" << result.key << " etag=" << result.etag
                               << dendl;
...
public:
    RGWAWSStreamAbortMultipartUploadCR(RGWDataSyncCtx* _sc, RGWRESTConn* _dest_conn, const rgw_obj& _dest_obj,
                                       const rgw_raw_obj& _status_obj, const string& _upload_id)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , dest_conn(_dest_conn)
        , dest_obj(_dest_obj)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            if (retcode < 0) {
                ldpp_dout(dpp, 0) << "ERROR: failed to abort multipart upload dest obj=" << dest_obj
                                  << " upload_id=" << upload_id << " retcode=" << retcode << dendl;
                /* ignore error, best effort */
            }
            yield call(new RGWRadosRemoveCR(sc->env->driver, status_obj));
            if (retcode < 0) {
                ldpp_dout(dpp, 0) << "ERROR: failed to remove sync status obj obj=" << status_obj
                                  << " retcode=" << retcode << dendl;
                /* ignore error, best effort */
            }
            return set_cr_done();
...
public:
    RGWAWSStreamObjToCloudMultipartCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, AWSSyncConfig& _conf,
                                      RGWRESTConn* _source_conn, const rgw_obj& _src_obj,
                                      std::shared_ptr<AWSSyncConfig_Profile>& _target, const rgw_obj& _dest_obj,
                                      uint64_t _obj_size, const rgw_sync_aws_src_obj_properties& _src_properties,
                                      const rgw_rest_obj& _rest_obj)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , conf(_conf)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            if (retcode < 0 && retcode != -ENOENT) {
                ldpp_dout(dpp, 0) << "ERROR: failed to read sync status of object " << src_obj << " retcode=" << retcode
                                  << dendl;
                return retcode;
            }
            ...
            if (retcode >= 0) {
                ...
                if (status.src_properties.mtime != src_properties.mtime || status.obj_size != obj_size ||
                    ...
                    yield call(new RGWAWSStreamAbortMultipartUploadCR(
                        sc, target->conn.get(), dest_obj, status_obj, status.upload_id));
                    retcode = -ENOENT;
                }
            }
            ...
            if (retcode == -ENOENT) {
                ...
                yield call(new RGWAWSInitMultipartCR(
                    sc, target->conn.get(), dest_obj, status.obj_size, std::move(new_attrs), &status.upload_id));
                if (retcode < 0) {
                    return set_cr_error(retcode);
                }
...
#define MULTIPART_MAX_PARTS 10000
            ...
            for (; (uint32_t)status.cur_part <= status.num_parts; ++status.cur_part) {
                ...
                if (retcode < 0) {
                    ldpp_dout(dpp, 0) << "ERROR: failed to sync obj=" << src_obj
                                      << ", sync via multipart upload, upload_id=" << status.upload_id
                                      << " part number " << status.cur_part << " (error: " << cpp_strerror(-retcode)
                                      << ")" << dendl;
                    ret_err = retcode;
                    yield call(new RGWAWSStreamAbortMultipartUploadCR(
                        sc, target->conn.get(), dest_obj, status_obj, status.upload_id));
                ...
                yield call(new RGWSimpleRadosWriteCR<rgw_sync_aws_multipart_upload_info>(
                    dpp, sync_env->driver, status_obj, status));
                if (retcode < 0) {
                    ldpp_dout(dpp, 0) << "ERROR: failed to store multipart upload state, retcode=" << retcode << dendl;
                    /* continue with upload anyway */
                }
                ldpp_dout(dpp, 20) << "sync of object=" << src_obj << " via multipart upload, finished sending part #"
                                   << status.cur_part << " etag=" << pcur_part_info->etag << dendl;
            }

            yield call(new RGWAWSCompleteMultipartCR(sc, target->conn.get(), dest_obj, status.upload_id, status.parts));
            if (retcode < 0) {
                ldpp_dout(dpp, 0) << "ERROR: failed to complete multipart upload of obj=" << src_obj
                                  << " (error: " << cpp_strerror(-retcode) << ")" << dendl;
                ret_err = retcode;
                yield call(new RGWAWSStreamAbortMultipartUploadCR(
                    sc, target->conn.get(), dest_obj, status_obj, status.upload_id));
                return set_cr_error(ret_err);
            ...
            if (retcode < 0) {
                ldpp_dout(dpp, 0) << "ERROR: failed to abort multipart upload obj=" << src_obj
                                  << " upload_id=" << status.upload_id << " part number " << status.cur_part << " ("
...
template<class T> int decode_attr(map<string, bufferlist>& attrs, const char* attr_name, T* result, T def_val)
{
...
public:
    RGWAWSHandleRemoteObjCBCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, rgw_obj_key& _key,
                              AWSSyncInstanceEnv& _instance, uint64_t _versioned_epoch)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            ldpp_dout(dpp, 4) << "AWS: download begin: z=" << sc->source_zone << " b=" << src_bucket << " k=" << key
                              << " size=" << size << " mtime=" << mtime << " etag=" << etag
                              << " zone_short_id=" << src_zone_short_id << " pg_ver=" << src_pg_ver << dendl;
            ...
            instance.conf.get_target(
                target, sync_pipe.dest_bucket_info, key, &dest_obj.bucket.name, &dest_obj.key.name);
            ...
            if (bucket_created.find(dest_obj.bucket.name) == bucket_created.end()) {
                yield
                {
                    ...
                    bufferlist bl;
                    call(new RGWPutRawRESTResourceCR<bufferlist>(sc->cct,
                                                                 target->conn.get(),
                                                                 sync_env->http_manager,
                                                                 dest_obj.bucket.name,
                                                                 nullptr,
                                                                 bl,
                                                                 &out_bl));
                }
            ...
            yield
            {
                ...
                if (size < instance.conf.s3.multipart_sync_threshold) {
                    call(new RGWAWSStreamObjToCloudPlainCR(sc, source_conn, src_obj, src_properties, target, dest_obj));
                }
                else {
                    rgw_rest_obj rest_obj;
                    rest_obj.init(key);
                    if (do_decode_rest_obj(dpp, sc->cct, attrs, headers, &rest_obj)) {
                        ldpp_dout(dpp, 0) << "ERROR: failed to decode rest obj out of headers=" << headers
                                          << ", attrs=" << attrs << dendl;
                        return set_cr_error(-EINVAL);
                    }
                    call(new RGWAWSStreamObjToCloudMultipartCR(sc,
                                                               sync_pipe,
                                                               instance.conf,
                                                               source_conn,
                                                               src_obj,
...
public:
    RGWAWSHandleRemoteObjCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, rgw_obj_key& _key,
                            AWSSyncInstanceEnv& _instance, uint64_t _versioned_epoch)
...
public:
    RGWAWSRemoveRemoteObjCBCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, rgw_obj_key& _key,
                              const ceph::real_time& _mtime, AWSSyncInstanceEnv& _instance)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_pipe(_sync_pipe)
        , key(_key)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            yield
            {
                ...
                call(new RGWDeleteRESTResourceCR(
                    sc->cct, target->conn.get(), sc->env->http_manager, path, nullptr /* params */));
            }
            if (retcode < 0) {
                return set_cr_error(retcode);
            }
...
public:
    ...
    void init(RGWDataSyncCtx* sc, uint64_t instance_id) override { instance.init(sc, instance_id); }
    ...
    {
        ldout(sc->cct, 0) << instance.id << ": sync_object: b=" << sync_pipe.info.source_bs.bucket << " k=" << key
    ...
    {
        ldout(sc->cct, 0) << "rm_object: b=" << sync_pipe.info.source_bs.bucket << " k=" << key << " mtime=" << mtime
    ...
    RGWCoroutine* create_delete_marker(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc,
                                       ...
                                       rgw_zone_set* zones_trace) override
    {
        ldout(sc->cct, 0) << "AWS Not implemented: create_delete_marker: b=" << sync_pipe.info.source_bs.bucket
...
public:
    RGWAWSSyncModuleInstance(CephContext* cct, AWSSyncConfig& _conf)
        : data_handler(cct, _conf)
    {}
    RGWDataSyncModule* get_data_handler() override { return &data_handler; }
};

int RGWAWSSyncModule::create_instance(const DoutPrefixProvider* dpp, CephContext* cct, const JSONFormattable& config,
