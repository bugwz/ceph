...
private:
    ...
    public:
        ...
        token make_token() { return token(*this); }
    };
    ...
    bool process_entry(const cls_queue_entry& entry, yield_context yield)
    {
        ...
        try {
            ...
            const auto push_endpoint = RGWPubSubEndpoint::create(event_entry.push_endpoint,
                                                                 event_entry.arn_topic,
            ...
            const auto ret =
                push_endpoint->send_to_completion_async(cct, event_entry.event, optional_yield(io_context, yield));
            if (ret < 0) {
                ldpp_dout(this, 5) << "WARNING: push entry: " << entry.marker
                                   << " to endpoint: " << event_entry.push_endpoint << " failed. error: " << ret
                                   << " (will retry)" << dendl;
                return false;
            }
            else {
                ldpp_dout(this, 20) << "INFO: push entry: " << entry.marker
                                    << " to endpoint: " << event_entry.push_endpoint << " ok" << dendl;
                if (perfcounter) perfcounter->inc(l_rgw_pubsub_push_ok);
                return true;
            }
        }
        catch (const RGWPubSubEndpoint::configuration_error& e) {
            ldpp_dout(this, 5) << "WARNING: failed to create push endpoint: " << event_entry.push_endpoint
                               << " for entry: " << entry.marker << ". error: " << e.what() << " (will retry) "
                               << dendl;
            return false;
        }
    }
    ...
    {
        while (true) {
            ldpp_dout(this, 20) << "INFO: trying to perform stale reservation cleanup for queue: " << queue_name
                                << dendl;
            ...
            rados::cls::lock::assert_locked(
                &op, queue_name + "_lock", ClsLockType::EXCLUSIVE, lock_cookie, "" /*no tag*/);
            ...
            if (ret == -EBUSY) {
                ldpp_dout(this, 5) << "WARNING: queue: " << queue_name
                                   << " ownership moved to another daemon. processing will stop" << dendl;
                return;
            }
            if (ret < 0) {
                ldpp_dout(this, 5) << "WARNING: failed to cleanup stale reservation from queue and/or lock queue: "
    ...
    void process_queue(const std::string& queue_name, yield_context yield)
    {
        ...
        spawn::spawn(
            io_context,
            [this, queue_name](yield_context yield) { cleanup_queue(queue_name, yield); },
        ...
        while (true) {
            ...
            auto total_entries = 0U;
            {
                ...
                auto ret =
                    rgw_rados_operate(this, rados_ioctx, queue_name, &op, nullptr, optional_yield(io_context, yield));
                if (ret == -ENOENT) {
                    // queue was deleted
                    ldpp_dout(this, 5) << "INFO: queue: " << queue_name << ". was removed. processing will stop"
                                       << dendl;
                    return;
                }
                if (ret == -EBUSY) {
                ...
                if (ret < 0) {
                    ldpp_dout(this, 5) << "WARNING: failed to parse list of entries in queue: " << queue_name
            ...
            for (auto& entry : entries) {
                ...
                spawn::spawn(
                    ...
                    [this,
                     ...
                     &has_error,
                     &waiter,
                     entry](yield_context yield) {
                        const auto token = waiter.make_token();
                        if (process_entry(entry, yield)) {
                            ldpp_dout(this, 20) << "INFO: processing of entry: " << entry.marker << " (" << entry_idx
                                                << "/" << total_entries << ") from: " << queue_name << " ok" << dendl;
                            remove_entries = true;
                        }
                        else {
                            if (set_min_marker(end_marker, entry.marker) < 0) {
                                ldpp_dout(this, 1)
                                    << "ERROR: cannot determin minimum between malformed markers: " << end_marker
                                    << ", " << entry.marker << dendl;
                            }
                            else {
                                ldpp_dout(this, 20) << "INFO: new end marker for removal: " << end_marker
                                                    << " from: " << queue_name << dendl;
                            }
                            has_error = true;
                            ldpp_dout(this, 20)
                                << "INFO: processing of entry: " << entry.marker << " (" << entry_idx << "/"
                                << total_entries << ") from: " << queue_name << " failed" << dendl;
                        }
                    },
                    make_stack_allocator());
                ++entry_idx;
            ...
            if (remove_entries) {
                ...
                const auto ret =
                    rgw_rados_operate(this, rados_ioctx, queue_name, &op, optional_yield(io_context, yield));
                if (ret == -ENOENT) {
                    // queue was deleted
                    ldpp_dout(this, 5) << "INFO: queue: " << queue_name << ". was removed. processing will stop"
                                       << dendl;
                ...
                if (ret < 0) {
                    ldpp_dout(this, 1) << "ERROR: failed to remove entries and/or lock queue up to: " << end_marker
    ...
    void process_queues(yield_context yield)
    {
        ...
        while (true) {
            Timer timer(io_context);
            const auto duration = (has_error ? std::chrono::milliseconds(queues_update_retry_ms)
            ...
            for (const auto& queue_name : queues) {
                ...
                if (ret == -EBUSY) {
                    ...
                    ldpp_dout(this, 20) << "INFO: queue: " << queue_name << " owned (locked) by another daemon"
                                        << dendl;
                    // if queue was owned by this RGW, processing should be stopped, queue would be deleted from list
                    // afterwards
                    continue;
                }
                if (ret == -ENOENT) {
                    // queue is deleted - processing will stop the next time we try to read from the queue
                    ldpp_dout(this, 10) << "INFO: queue: " << queue_name << " should not be locked - already deleted"
                                        << dendl;
                    continue;
                }
                if (ret < 0) {
                ...
                if (owned_queues.insert(queue_name).second) {
                    ldpp_dout(this, 10) << "INFO: queue: " << queue_name << " now owned (locked) by this daemon"
                                        << dendl;
                    // start processing this queue
                    spawn::spawn(
                        io_context,
                        [this, &queue_gc, &queue_gc_lock, queue_name](yield_context yield) {
...
public:
    ...
    Manager(CephContext* _cct, uint32_t _max_queue_size, uint32_t _queues_update_period_ms,
            ...
            uint32_t _stale_reservations_period_s, uint32_t _reservations_cleanup_period_s, uint32_t _worker_count,
    ...
    {
        spawn::spawn(
            io_context, [this](yield_context yield) { process_queues(yield); }, make_stack_allocator());
    ...
    int add_persistent_topic(const std::string& topic_name, optional_yield y)
    {
        if (topic_name == Q_LIST_OBJECT_NAME) {
            ldpp_dout(this, 1) << "ERROR: topic name cannot be: " << Q_LIST_OBJECT_NAME
        ...
        if (ret < 0) {
            ...
            ldpp_dout(this, 1) << "ERROR: failed to create queue for topic: " << topic_name << ". error: " << ret
                               << dendl;
            return ret;
        }
        ...
        if (ret < 0) {
            ldpp_dout(this, 1) << "ERROR: failed to add queue: " << topic_name << " to queue list. error: " << ret
                               << dendl;
            return ret;
        }
...
int remove_persistent_topic(const DoutPrefixProvider* dpp, librados::IoCtx& rados_ioctx, const std::string& topic_name,
                            optional_yield y)
{
    librados::ObjectWriteOperation op;
    op.remove();
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 1) << "ERROR: failed to remove queue: " << topic_name << " from queue list. error: " << ret
                          << dendl;
        return ret;
    }
...
rgw::sal::Object* get_object_with_atttributes(const reservation_t& res, rgw::sal::Object* obj)
{
    ...
    if (src_obj->get_attrs().empty()) {
        ...
        if (ret < 0) {
            ldpp_dout(res.dpp, 20) << "failed to get attributes from object: " << src_obj->get_key()
                                   << ". ret = " << ret << dendl;
            return nullptr;
        }
    }
...
{
    std::copy_if(src.cbegin(), src.cend(), std::inserter(dest, dest.end()), [](const auto& m) {
        return (boost::algorithm::starts_with(m.first, RGW_AMZ_META_PREFIX));
    });
}


static inline void metadata_from_attributes(reservation_t& res, rgw::sal::Object* obj)
{
...
static inline void tags_from_attributes(const reservation_t& res, rgw::sal::Object* obj, KeyMultiValueMap& tags)
{
    const auto src_obj = get_object_with_atttributes(res, obj);
    if (!src_obj) {
        return;
...
static inline void populate_event(reservation_t& res, rgw::sal::Object* obj, uint64_t size,
                                  const ceph::real_time& mtime, const std::string& etag, const std::string& version,
                                  EventType event_type, rgw_pubsub_s3_event& event)
{
    ...
    event.bucket_ownerIdentity =
        res.bucket->get_owner() ? res.bucket->get_owner()->get_id().id : res.bucket->get_info().owner.id;
    ...
    boost::algorithm::hex(
        (const char*)&ts, (const char*)&ts + sizeof(utime_t), std::back_inserter(event.object_sequencer));
    ...
    // pass tags
    if (!res.tagset || (*res.tagset).get_tags().empty()) {
...
static inline bool notification_match(reservation_t& res, const rgw_pubsub_topic_filter& filter, EventType event,
                                      const RGWObjTags* req_tags)
{
    ...
    if (!match(filter.s3_filter.key_filter, res.object_name ? *res.object_name : obj->get_name())) {
        return false;
    }
...
int publish_reserve(const DoutPrefixProvider* dpp, EventType event_type, reservation_t& res, const RGWObjTags* req_tags)
{
    ...
    for (const auto& bucket_topic : bucket_topics.topics) {
        ...
        ldpp_dout(res.dpp, 20) << "INFO: notification: '" << topic_filter.s3_id << "' on topic: '"
                               << topic_cfg.dest.arn_topic << "' and bucket: '" << res.bucket->get_name()
                               << "' (unique topic: '" << topic_cfg.name << "') apply to event of type: '"
                               << to_string(event_type) << "'" << dendl;
        ...
        if (topic_cfg.dest.persistent) {
            ...
            auto ret = rgw_rados_operate(res.dpp,
                                         res.store->getRados()->get_notif_pool_ctx(),
                                         queue_name,
                                         &op,
                                         res.yield,
                                         librados::OPERATION_RETURNVEC);
            if (ret < 0) {
                ldpp_dout(res.dpp, 1) << "ERROR: failed to reserve notification on queue: " << queue_name
                                      << ". error: " << ret << dendl;
...
int publish_commit(rgw::sal::Object* obj, uint64_t size, const ceph::real_time& mtime, const std::string& etag,
                   const std::string& version, EventType event_type, reservation_t& res, const DoutPrefixProvider* dpp)
{
        ...
        if (topic.cfg.dest.persistent) {
            ...
            if (bl.length() > res.size) {
                ...
                auto ret = rgw_rados_operate(
                    dpp, res.store->getRados()->get_notif_pool_ctx(), topic.cfg.dest.arn_topic, &op, res.yield);
                if (ret < 0) {
            ...
            const auto ret =
                rgw_rados_operate(dpp, res.store->getRados()->get_notif_pool_ctx(), queue_name, &op, res.yield);
            ...
            try {
                ...
                const auto push_endpoint =
                    RGWPubSubEndpoint::create(topic.cfg.dest.push_endpoint,
                                              topic.cfg.dest.arn_topic,
                                              RGWHTTPArgs(topic.cfg.dest.push_endpoint_args, dpp),
                                              dpp->get_cct());
                ldpp_dout(res.dpp, 20) << "INFO: push endpoint created: " << topic.cfg.dest.push_endpoint << dendl;
                const auto ret = push_endpoint->send_to_completion_async(dpp->get_cct(), event_entry.event, res.yield);
                if (ret < 0) {
                    ldpp_dout(dpp, 1) << "ERROR: push to endpoint " << topic.cfg.dest.push_endpoint
                ...
                ldpp_dout(dpp, 1) << "ERROR: failed to create push endpoint: " << topic.cfg.dest.push_endpoint
                                  << ". error: " << e.what() << dendl;
                if (perfcounter) perfcounter->inc(l_rgw_pubsub_push_failed);
                return -EINVAL;
            }
...
int publish_abort(reservation_t& res)
{
    for (auto& topic : res.topics) {
        ...
        cls_2pc_queue_abort(op, topic.res_id);
        const auto ret =
            rgw_rados_operate(res.dpp, res.store->getRados()->get_notif_pool_ctx(), queue_name, &op, res.yield);
        if (ret < 0) {
            ldpp_dout(res.dpp, 1) << "ERROR: failed to abort reservation: " << topic.res_id
                                  << " from queue: " << queue_name << ". error: " << ret << dendl;
            return ret;
        }
        topic.res_id = cls_2pc_reservation::NO_ID;
...
reservation_t::reservation_t(const DoutPrefixProvider* _dpp, rgw::sal::RadosStore* _store, const req_state* _s,
                             rgw::sal::Object* _object, rgw::sal::Object* _src_object, const std::string* _object_name,
                             optional_yield y)
    : dpp(_s)
    , store(_store)
    , s(_s)
...
reservation_t::reservation_t(const DoutPrefixProvider* _dpp, rgw::sal::RadosStore* _store, rgw::sal::Object* _object,
                             rgw::sal::Object* _src_object, rgw::sal::Bucket* _bucket, const std::string& _user_id,
                             const std::string& _user_tenant, const std::string& _req_id, optional_yield y)
    : dpp(_dpp)
    , store(_store)
    , s(nullptr)
    , size(0) /* XXX */
