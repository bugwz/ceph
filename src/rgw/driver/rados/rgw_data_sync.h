...
struct rgw_bucket_sync_pipe
{
    ...
    RGWBucketSyncFlowManager::pipe_rules_ref& get_rules() { return info.handler.rules; }
};
...
struct rgw_data_sync_marker
{
    ...
    void dump(Formatter* f) const
    {
        ...
        switch ((SyncState)state) {
        case FullSync: s = "full-sync"; break;
        case IncrementalSync: s = "incremental-sync"; break;
        default: s = "unknown"; break;
        }
        encode_json("status", s, f);
...
struct rgw_bucket_entry_owner
{
    ...
    rgw_bucket_entry_owner(const std::string& _id, const std::string& _display_name)
        : id(_id)
...
struct RGWDataSyncEnv
{
    ...
    void init(const DoutPrefixProvider* _dpp, CephContext* _cct, rgw::sal::RadosStore* _driver, RGWServices* _svc,
              RGWAsyncRadosProcessor* _async_rados, RGWHTTPManager* _http_manager, RGWSyncErrorLogger* _error_logger,
              RGWSyncTraceManager* _sync_tracer, RGWSyncModuleInstanceRef& _sync_module, PerfCounters* _counters)
    {
        dpp = _dpp;
        cct = _cct;
        driver = _driver;
...
template<typename... T> void pretty_print(const RGWDataSyncEnv* env, fmt::format_string<T...> fmt, T&&... t)
{
#else
template<typename S, typename... T> void pretty_print(const RGWDataSyncEnv* env, const S& fmt, T&&... t)
{
#endif
    ...
    int64_t adj_concurrency(int64_t concurrency)
    {
        ...
        if (avg_latency() >= 2 * threshold) [[unlikely]] {
            ...
            if (now - last_warning > 5min) {
                ldout(cct, -1) << "WARNING: The OSD cluster is overloaded and struggling to "
...
struct RGWDataSyncCtx
{
    ...
    RGWDataSyncCtx(RGWDataSyncEnv* env, RGWRESTConn* conn, const rgw_zone_id& source_zone)
        : env(env)
        , cct(env->cct)
        , conn(conn)
    ...
    void init(RGWDataSyncEnv* _env, RGWRESTConn* _conn, const rgw_zone_id& _source_zone)
    {
        cct = _env->cct;
        env = _env;
        conn = _conn;
...
public:
    RGWRemoteDataLog(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* _store, RGWAsyncRadosProcessor* async_rados);
    int init(const rgw_zone_id& _source_zone, RGWRESTConn* _conn, RGWSyncErrorLogger* _error_logger,
             RGWSyncTraceManager* _sync_tracer, RGWSyncModuleInstanceRef& module, PerfCounters* _counters);
    void finish();
    ...
    int read_source_log_shards_next(const DoutPrefixProvider* dpp, std::map<int, std::string> shard_markers,
                                    std::map<int, rgw_datalog_shard_data>* result);
    ...
    int read_shard_status(const DoutPrefixProvider* dpp, int shard_id, std::set<std::string>& lagging_buckets,
                          std::set<std::string>& recovering_buckets, rgw_data_sync_marker* sync_marker,
...
public:
    RGWDataSyncStatusManager(rgw::sal::RadosStore* _driver, RGWAsyncRadosProcessor* async_rados,
        ...
        , sync_module(nullptr)
        , counters(counters)
        , source_log(this, driver, async_rados)
        , num_shards(0)
    {}
    RGWDataSyncStatusManager(rgw::sal::RadosStore* _driver, RGWAsyncRadosProcessor* async_rados,
                             const rgw_zone_id& _source_zone, PerfCounters* counters,
        ...
        , num_shards(0)
    {}
    ~RGWDataSyncStatusManager() { finalize(); }
    ...
    int read_shard_status(const DoutPrefixProvider* dpp, int shard_id, std::set<std::string>& lagging_buckets,
                          ...
                          const int max_entries)
    {
        return source_log.read_shard_status(
            dpp, shard_id, lagging_buckets, recovering_buckets, sync_marker, max_entries);
    }
    ...
    void wakeup(int shard_id, bc::flat_set<rgw_data_notify_entry>& entries)
    {
        return source_log.wakeup(shard_id, entries);
    }

    void stop() { source_log.finish(); }
...
class RGWBucketPipeSyncStatusManager : public DoutPrefixProvider
{
    ...
    RGWCoroutinesManager cr_mgr{driver->ctx(), driver->getRados()->get_cr_registry()};
    ...
    std::unique_ptr<RGWSyncErrorLogger> error_logger =
        std::make_unique<RGWSyncErrorLogger>(driver, RGW_SYNC_ERROR_LOG_SHARD_PREFIX, ERROR_LOGGER_SHARDS);
    RGWSyncModuleInstanceRef sync_module;
    ...
    struct source
    {
        ...
        source(RGWDataSyncEnv* env, const rgw_zone_id& zone, RGWRESTConn* conn, const RGWBucketInfo& info,
               const rgw_bucket& dest, const RGWBucketSyncFlowManager::pipe_handler& handler,
    ...
    RGWBucketPipeSyncStatusManager(rgw::sal::RadosStore* driver, std::optional<rgw_zone_id> source_zone,
    ...
    int remote_info(const DoutPrefixProvider* dpp, source& s, uint64_t* oldest_gen, uint64_t* latest_gen,
                    uint64_t* num_shards);

public:
    static tl::expected<std::unique_ptr<RGWBucketPipeSyncStatusManager>, int> construct(
        const DoutPrefixProvider* dpp, rgw::sal::RadosStore* driver, std::optional<rgw_zone_id> source_zone,
        std::optional<rgw_bucket> source_bucket, const rgw_bucket& dest_bucket, std::ostream* ostream);
    ~RGWBucketPipeSyncStatusManager() = default;


    static std::string full_status_oid(const rgw_zone_id& source_zone, const rgw_bucket& source_bucket,
                                       const rgw_bucket& dest_bucket);
    static std::string inc_status_oid(const rgw_zone_id& source_zone, const rgw_bucket_sync_pair_info& bs,
                                      uint64_t gen);
    // specific source obj sync status, can be used by sync modules
    static std::string obj_status_oid(const rgw_bucket_sync_pipe& sync_pipe, const rgw_zone_id& source_zone,
                                      const rgw_obj& obj);
    ...
    tl::expected<std::map<int, rgw_bucket_shard_sync_info>, int> read_sync_status(const DoutPrefixProvider* dpp);
...
int rgw_read_bucket_full_sync_status(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* driver,
                                     const rgw_sync_bucket_pipe& pipe, rgw_bucket_sync_status* status,
                                     optional_yield y);
...
public:
    ...
    int create_instance(const DoutPrefixProvider* dpp, CephContext* cct, const JSONFormattable& config,
