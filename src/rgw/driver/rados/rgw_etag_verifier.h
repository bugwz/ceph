...
public:
    ETagVerifier_MPU(CephContext* cct, std::vector<uint64_t> part_ofs, rgw::sal::DataProcessor* next)
...
constexpr auto max_etag_verifier_size = std::max(sizeof(ETagVerifier_Atomic), sizeof(ETagVerifier_MPU));
using etag_verifier_ptr = ceph::static_ptr<ETagVerifier, max_etag_verifier_size>;

int create_etag_verifier(const DoutPrefixProvider* dpp, CephContext* cct, rgw::sal::DataProcessor* next,
