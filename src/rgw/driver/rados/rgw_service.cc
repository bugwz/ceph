...
int RGWServices_Def::init(CephContext* cct, bool have_cache, bool raw, bool run_sync, optional_yield y,
                          const DoutPrefixProvider* dpp)
{
    ...
    user_rados->init(
        rados.get(), zone.get(), sysobj.get(), sysobj_cache.get(), meta.get(), meta_be_sobj.get(), sync_modules.get());
    ...
    if (!raw) {
        ...
        r = datalog_rados->start(dpp, &zone->get_zone(), zone->get_zone_params(), rados->get_rados_handle());
        if (r < 0) {
...
int RGWCtlDef::init(RGWServices& svc, rgw::sal::Driver* driver, const DoutPrefixProvider* dpp)
{
    ...
    bucket.reset(new RGWBucketCtl(svc.zone, svc.bucket, svc.bucket_sync, svc.bi, svc.user));
    ...
    RGWBucketInstanceMetadataHandlerBase* bi_meta_handler =
        static_cast<RGWBucketInstanceMetadataHandlerBase*>(meta.bucket_instance.get());
