                                                               ...
                                                               const rgw_zone_id& dest_zone)
{
    ...
    for (auto& p : pipes) {
        if (p.source.match_zone(source_zone) && p.dest.match_zone(dest_zone)) {
...
void rgw_sync_group_pipe_map::try_add_to_pipe_map(const rgw_zone_id& source_zone, const rgw_zone_id& dest_zone,
                                                  const std::vector<rgw_sync_bucket_pipes>& pipes,
                                                  zb_pipe_map_t* pipe_map, CB1 filter_cb, CB2 call_filter_cb)
{
...
void rgw_sync_group_pipe_map::try_add_source(const rgw_zone_id& source_zone, const rgw_zone_id& dest_zone,
                                             const std::vector<rgw_sync_bucket_pipes>& pipes, CB filter_cb)
{
    return try_add_to_pipe_map(source_zone,
                               dest_zone,
                               pipes,
                               &sources,
                               filter_cb,
...
template<typename CB>
void rgw_sync_group_pipe_map::try_add_dest(const rgw_zone_id& source_zone, const rgw_zone_id& dest_zone,
                                           const std::vector<rgw_sync_bucket_pipes>& pipes, CB filter_cb)
{
    return try_add_to_pipe_map(source_zone,
                               dest_zone,
...
pair<zb_pipe_map_t::const_iterator, zb_pipe_map_t::const_iterator> rgw_sync_group_pipe_map::find_pipes(
    const zb_pipe_map_t& m, const rgw_zone_id& zone, std::optional<rgw_bucket> b) const
{
    if (!b) {
    ...
    if (range.first == range.second && !is_wildcard_bucket(*b)) {
...
template<typename CB>
void rgw_sync_group_pipe_map::init(const DoutPrefixProvider* dpp, CephContext* cct, const rgw_zone_id& _zone,
                                   std::optional<rgw_bucket> _bucket, const rgw_sync_policy_group& group,
                                   rgw_sync_data_flow_group* _default_flow, std::set<rgw_zone_id>* _pall_zones,
                                   CB filter_cb)
{
    zone = _zone;
    bucket = _bucket;
    ...
    for (auto& pipe : group.pipes) {
        if (pipe.contains_zone_bucket(zone, bucket)) {
            ldpp_dout(dpp, 20) << __func__ << "(): pipe_map (zone=" << zone << " bucket=" << bucket_key
                               << "): adding potential pipe: " << pipe << dendl;
...
void RGWBucketSyncFlowManager::pipe_rules::insert(const rgw_sync_bucket_pipe& pipe)
{
    ...
    for (auto& t : ppipe->params.source.filter.tags) {
        ...
        if (titer != tag_refs.end() && ppipe->params.priority > titer->second->params.priority) {
            titer->second = ppipe;
        }
        else {
            tag_refs[tag] = ppipe;
...
bool RGWBucketSyncFlowManager::pipe_rules::find_basic_info_without_tags(
    const rgw_obj_key& key, std::optional<rgw_user>* user, std::optional<rgw_user>* acl_translation_owner,
    std::optional<string>* storage_class, rgw_sync_pipe_params::Mode* mode, bool* need_more_info) const
{
    ...
    *need_more_info = false;
    ...
    for (auto& iter : iters) {
        ...
        if (first_iter) {
            ...
            const bool conflict = !(_user == rule_params.user && _acl_translation == rule_params.dest.acl_translation &&
                                    _storage_class == rule_params.dest.storage_class && _mode == rule_params.mode);
            if (conflict) {
                *need_more_info = true;
                return false;
            }
...
bool RGWBucketSyncFlowManager::pipe_rules::find_obj_params(const rgw_obj_key& key, const RGWObjTags::tag_map_t& tags,
...
RGWBucketSyncFlowManager::pipe_rules::prefix_map_t::const_iterator RGWBucketSyncFlowManager::pipe_rules::prefix_search(
...
bool RGWBucketSyncFlowManager::allowed_data_flow(const rgw_zone_id& source_zone,
                                                 std::optional<rgw_bucket> source_bucket, const rgw_zone_id& dest_zone,
                                                 std::optional<rgw_bucket> dest_bucket, bool check_activated) const
{
    bool found = false;
    ...
    for (auto m : flow_groups) {
        auto& fm = m.second;
        auto pipes = fm.find_pipes(source_zone, source_bucket, dest_zone, dest_bucket);

        bool is_found = !pipes.empty();

        if (is_found) {
            switch (fm.status) {
            case rgw_sync_policy_group::Status::FORBIDDEN: return false;
            case rgw_sync_policy_group::Status::ENABLED:
                found = true;
                found_activated = true;
                break;
            case rgw_sync_policy_group::Status::ALLOWED: found = true; break;
    ...
    for (auto& item : sync_policy.groups) {
        ...
        flow_group_map.init(
            ...
            [&](const rgw_zone_id& source_zone,
                ...
                return parent->allowed_data_flow(
                    source_zone, source_bucket, dest_zone, dest_bucket, false); /* just check that it's not disabled */
            });
    }
}
 ...
 *
 */
void RGWBucketSyncFlowManager::reflect(const DoutPrefixProvider* dpp, std::optional<rgw_bucket> effective_bucket,
                                       RGWBucketSyncFlowManager::pipe_set* source_pipes,
                                       RGWBucketSyncFlowManager::pipe_set* dest_pipes, bool only_enabled) const

{
    ...
    for (auto& item : flow_groups) {
        ...
        for (auto& entry : flow_group_map.sources) {
            ...
            if (is_forbidden) {
                ldpp_dout(dpp, 20) << __func__ << "(): flow manager (bucket=" << effective_bucket_key
                                   << "): removing source pipe: " << pipe << dendl;
                source_pipes->disable(pipe);
            }
            else {
                ldpp_dout(dpp, 20) << __func__ << "(): flow manager (bucket=" << effective_bucket_key
                                   << "): adding source pipe: " << pipe << dendl;
        ...
        for (auto& entry : flow_group_map.dests) {
            ...
            if (is_forbidden) {
                ldpp_dout(dpp, 20) << __func__ << "(): flow manager (bucket=" << effective_bucket_key
                                   << "): removing dest pipe: " << pipe << dendl;
                dest_pipes->disable(pipe);
            }
            else {
                ldpp_dout(dpp, 20) << __func__ << "(): flow manager (bucket=" << effective_bucket_key
                                   << "): adding dest pipe: " << pipe << dendl;
...
RGWBucketSyncFlowManager::RGWBucketSyncFlowManager(CephContext* _cct, const rgw_zone_id& _zone_id,
...
void RGWSyncPolicyCompat::convert_old_sync_config(RGWSI_Zone* zone_svc, RGWSI_SyncModules* sync_modules_svc,
    ...
    for (const auto& ziter1 : zonegroup.zones) {
        ...
        for (const auto& ziter2 : zonegroup.zones) {
            ...
            if (z1.syncs_from(z2.name)) {
                ...
                group.data_flow.find_or_create_directional(id2, id1, &rule);
            }
        }
    }
...
RGWBucketSyncPolicyHandler::RGWBucketSyncPolicyHandler(RGWSI_Zone* _zone_svc, RGWSI_SyncModules* sync_modules_svc,
                                                       RGWSI_Bucket_Sync* _bucket_sync_svc,
                                                       std::optional<rgw_zone_id> effective_zone)
    : zone_svc(_zone_svc)
    , bucket_sync_svc(_bucket_sync_svc)
{
    zone_id = effective_zone.value_or(zone_svc->zone_id());
    flow_mgr.reset(new RGWBucketSyncFlowManager(zone_svc->ctx(), zone_id, nullopt, nullptr));
    ...
    , bucket_attrs(std::move(_bucket_attrs))
{
    if (_bucket_info.sync_policy) {
        ...
        for (auto& entry : sync_policy.groups) {
            for (auto& pipe : entry.second.pipes) {
                if (pipe.params.mode == rgw_sync_pipe_params::MODE_USER && pipe.params.user.empty()) {
    ...
    flow_mgr.reset(
        new RGWBucketSyncFlowManager(zone_svc->ctx(), parent->zone_id, _bucket_info.bucket, parent->flow_mgr.get()));
}
    ...
    : parent(_parent)
{
    ...
    zone_svc = parent->zone_svc;
    bucket_sync_svc = parent->bucket_sync_svc;
    flow_mgr.reset(new RGWBucketSyncFlowManager(zone_svc->ctx(), parent->zone_id, _bucket, parent->flow_mgr.get()));
}
...
RGWBucketSyncPolicyHandler* RGWBucketSyncPolicyHandler::alloc_child(
    const rgw_bucket& bucket, std::optional<rgw_sync_policy_info> sync_policy) const
{
...
{
    int r = bucket_sync_svc->get_bucket_sync_hints(dpp, bucket.value_or(rgw_bucket()), &source_hints, &target_hints, y);
    if (r < 0) {
        ldpp_dout(dpp, 0)
            << "ERROR: failed to initialize bucket sync policy handler: get_bucket_sync_hints() on bucket=" << bucket
            << " returned r=" << r << dendl;
        return r;
    }
    ...
    reflect(dpp, &source_pipes, &target_pipes, &sources, &targets, &source_zones, &target_zones, true);

    return 0;
}

void RGWBucketSyncPolicyHandler::reflect(const DoutPrefixProvider* dpp,
                                         RGWBucketSyncFlowManager::pipe_set* psource_pipes,
                                         ...
                                         std::set<rgw_zone_id>* psource_zones, std::set<rgw_zone_id>* ptarget_zones,
...
multimap<rgw_zone_id, rgw_sync_bucket_pipe> RGWBucketSyncPolicyHandler::get_all_dests_in_zone(
    const rgw_zone_id& zone_id) const
{
    ...
    for (auto& pipe : resolved_dests) {
        if (!pipe.dest.zone || *pipe.dest.zone != zone_id) {
            continue;
        }
...
void RGWBucketSyncPolicyHandler::get_pipes(std::set<rgw_sync_bucket_pipe>* _sources,
                                           std::set<rgw_sync_bucket_pipe>* _targets,
                                           std::optional<rgw_sync_bucket_entity> filter_peer)
{ /* return raw pipes */
    for (auto& entry : source_pipes.pipe_map) {
        auto& source_pipe = entry.second;
        if (!filter_peer || source_pipe.source.match(*filter_peer)) {
    ...
    for (auto& entry : target_pipes.pipe_map) {
        ...
        if (!filter_peer || target_pipe.dest.match(*filter_peer)) {
...
bool RGWBucketSyncPolicyHandler::bucket_exports_data() const
{
    ...
    return (zone_svc->need_to_log_data() && bucket_info->datasync_flag_enabled());
}
