...
public:
    ...
    virtual int init(RGWSI_Zone* zone, RGWSI_MetaBackend* _meta_be, RGWSI_OTP* _otp) = 0;
};
...
public:
    ...
    int read_all(const rgw_user& uid, RGWOTPInfo* info, optional_yield y, const DoutPrefixProvider* dpp,
                 const GetParams& params = {});
    int store_all(const DoutPrefixProvider* dpp, const RGWOTPInfo& info, optional_yield y,
                  const PutParams& params = {});
    int remove_all(const DoutPrefixProvider* dpp, const rgw_user& user, optional_yield y,
