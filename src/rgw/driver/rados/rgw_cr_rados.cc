...
{
    if (processor->m_req_queue.empty()) return NULL;
...
RGWAsyncRadosProcessor::RGWAsyncRadosProcessor(CephContext* _cct, int num_threads)
    : cct(_cct)
    , m_tp(cct, "RGWAsyncRadosProcessor::m_tp", "rados_async", num_threads)
    , req_throttle(_cct, "rgw_async_rados_ops", num_threads * 2)
    , req_wq(this, ceph::make_timespan(g_conf()->rgw_op_thread_timeout),
             ceph::make_timespan(g_conf()->rgw_op_thread_suicide_timeout), &m_tp)
{}
...
RGWAsyncGetSystemObj::RGWAsyncGetSystemObj(const DoutPrefixProvider* _dpp, RGWCoroutine* caller,
...
int RGWAsyncPutSystemObj::_send_request(const DoutPrefixProvider* dpp)
{
    ...
    return sysobj.wop().set_objv_tracker(&objv_tracker).set_exclusive(exclusive).write_data(dpp, bl, null_yield);
}

RGWAsyncPutSystemObj::RGWAsyncPutSystemObj(const DoutPrefixProvider* _dpp, RGWCoroutine* caller,
                                           RGWAioCompletionNotifier* cn, RGWSI_SysObj* _svc,
                                           RGWObjVersionTracker* _objv_tracker, const rgw_raw_obj& _obj,
...
RGWAsyncPutSystemObjAttrs::RGWAsyncPutSystemObjAttrs(const DoutPrefixProvider* _dpp, RGWCoroutine* caller,
                                                     RGWAioCompletionNotifier* cn, RGWSI_SysObj* _svc,
                                                     RGWObjVersionTracker* _objv_tracker, const rgw_raw_obj& _obj,
...
RGWOmapAppend::RGWOmapAppend(RGWAsyncRadosProcessor* _async_rados, rgw::sal::RadosStore* _store,
                             const rgw_raw_obj& _obj, uint64_t _window_size)
    : RGWConsumerCR<string>(_store->ctx())
...
RGWAsyncLockSystemObj::RGWAsyncLockSystemObj(RGWCoroutine* caller, RGWAioCompletionNotifier* cn,
                                             rgw::sal::RadosStore* _store, RGWObjVersionTracker* _objv_tracker,
                                             const rgw_raw_obj& _obj, const string& _name, const string& _cookie,
...
RGWAsyncUnlockSystemObj::RGWAsyncUnlockSystemObj(RGWCoroutine* caller, RGWAioCompletionNotifier* cn,
                                                 rgw::sal::RadosStore* _store, RGWObjVersionTracker* _objv_tracker,
                                                 const rgw_raw_obj& _obj, const string& _name, const string& _cookie)
    : RGWAsyncRadosRequest(caller, cn)
    , store(_store)
...
RGWRadosSetOmapKeysCR::RGWRadosSetOmapKeysCR(rgw::sal::RadosStore* _store, const rgw_raw_obj& _obj,
...
RGWRadosGetOmapKeysCR::RGWRadosGetOmapKeysCR(rgw::sal::RadosStore* _store, const rgw_raw_obj& _obj,
                                             const string& _marker, int _max_entries, ResultPtr _result)
    : RGWSimpleCoroutine(_store->ctx())
    , store(_store)
    , obj(_obj)
    , marker(_marker)
...
RGWRadosGetOmapValsCR::RGWRadosGetOmapValsCR(rgw::sal::RadosStore* _store, const rgw_raw_obj& _obj,
                                             const string& _marker, int _max_entries, ResultPtr _result)
    : RGWSimpleCoroutine(_store->ctx())
    , store(_store)
    , obj(_obj)
...
RGWRadosRemoveOmapKeysCR::RGWRadosRemoveOmapKeysCR(rgw::sal::RadosStore* _store, const rgw_raw_obj& _obj,
...
RGWRadosRemoveOidCR::RGWRadosRemoveOidCR(rgw::sal::RadosStore* store, librados::IoCtx&& ioctx, std::string_view oid,
                                         RGWObjVersionTracker* objv_tracker)
    : RGWSimpleCoroutine(store->ctx())
...
RGWRadosRemoveOidCR::RGWRadosRemoveOidCR(rgw::sal::RadosStore* store, RGWSI_RADOS::Obj& obj,
                                         RGWObjVersionTracker* objv_tracker)
    : RGWSimpleCoroutine(store->ctx())
...
RGWSimpleRadosLockCR::RGWSimpleRadosLockCR(RGWAsyncRadosProcessor* _async_rados, rgw::sal::RadosStore* _store,
                                           const rgw_raw_obj& _obj, const string& _lock_name, const string& _cookie,
                                           uint32_t _duration)
    : RGWSimpleCoroutine(_store->ctx())
    , async_rados(_async_rados)
    , store(_store)
    ...
    , req(NULL)
{
    set_description() << "rados lock dest=" << obj << " lock=" << lock_name << " cookie=" << cookie
                      << " duration=" << duration;
}
...
{
    set_status() << "sending request";
    req = new RGWAsyncLockSystemObj(
        this, stack->create_completion_notifier(), store, NULL, obj, lock_name, cookie, duration);
...
int RGWSimpleRadosUnlockCR::send_request(const DoutPrefixProvider* dpp)
{
    ...
    req = new RGWAsyncUnlockSystemObj(this, stack->create_completion_notifier(), store, NULL, obj, lock_name, cookie);
    async_rados->queue(req);
    return 0;
}
...
int RGWAsyncGetBucketInstanceInfo::_send_request(const DoutPrefixProvider* dpp)
{
    ...
    if (!bucket.bucket_id.empty()) {
        ...
        r = store->ctl()->bucket->read_bucket_info(
            bucket, &bucket_info, null_yield, dpp, RGWBucketCtl::BucketInstance::GetParams().set_attrs(&attrs));
    }
    if (r < 0) {
...
{
    auto r = store->getRados()->put_bucket_instance_info(bucket_info, exclusive, mtime, attrs, dpp, null_yield);
    if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to put bucket instance info for " << bucket_info.bucket << dendl;
        return r;
    }
...
RGWRadosBILogTrimCR::RGWRadosBILogTrimCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store,
        ...
        if (bytes_transferred) {
            ...
            if (iter != attrs.end()) {
                try {
                    ...
                    ldpp_dout(dpp, 1) << "ERROR: " << __func__ << ": caught buffer::error couldn't decode TagSet "
                                      << dendl;
                }
            }

            // NOTE: we create a mutable copy of bucket.get_tenant as the get_notification function expects a
            // std::string&, not const
            std::string tenant(dest_bucket.get_tenant());

            std::unique_ptr<rgw::sal::Notification> notify = store->get_notification(dpp,
                                                                                     &dest_obj,
                                                                                     nullptr,
            ...
            if (ret < 0) {
                ...
                ret = rgw::notify::publish_commit(&dest_obj,
                                                  *bytes_transferred,
                                                  ceph::real_clock::now(),
                                                  etag,
                                                  dest_obj.get_instance(),
                                                  rgw::notify::ObjectSyncedCreate,
                                                  notify_res,
                                                  dpp);
                if (ret < 0) {
...
int RGWAsyncRemoveObj::_send_request(const DoutPrefixProvider* dpp)
{
    ...
    if (del_if_older && (state->mtime > timestamp)) {
        ldpp_dout(dpp, 20) << __func__ << "(): skipping object removal obj=" << obj << " (obj mtime=" << state->mtime
    ...
    reenter(this)
    {
        ...
        while (!going_down) {
            ...
            if (current_time - last_renew_try_time > interval_tolerance) {
                ...
                ldout(store->ctx(), 1) << *this << ": WARNING: did not renew lock " << obj << ":" << lock_name
                                       << ": within 90\% of interval. " << (current_time - last_renew_try_time) << " > "
                                       << interval_tolerance << dendl;
            }
            ...
            if (retcode < 0) {
                set_locked(false);
                ldout(store->ctx(), 20) << *this << ": couldn't lock " << obj << ":" << lock_name
...
RGWRadosTimelogAddCR::RGWRadosTimelogAddCR(const DoutPrefixProvider* _dpp, rgw::sal::RadosStore* _store,
                                           const string& _oid, const cls_log_entry& entry)
    : RGWSimpleCoroutine(_store->ctx())
    , dpp(_dpp)
    , store(_store)
    , oid(_oid)
...
{
    stringstream& s = set_description();
    s << "timelog add entry oid=" << oid << "entry={id=" << entry.id << ", section=" << entry.section
      << ", name=" << entry.name << "}";
...
RGWRadosTimelogTrimCR::RGWRadosTimelogTrimCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store,
                                             const std::string& oid, const real_time& start_time,
                                             const real_time& end_time, const std::string& from_marker,
    ...
    , to_marker(to_marker)
{
    set_description() << "timelog trim oid=" << oid << " start_time=" << start_time << " end_time=" << end_time
...
int RGWRadosTimelogTrimCR::send_request(const DoutPrefixProvider* dpp)
{
    ...
    cn = stack->create_completion_notifier();
    return store->svc()->cls->timelog.trim(
        dpp, oid, start_time, end_time, from_marker, to_marker, cn->completion(), null_yield);
}
...
RGWSyncLogTrimCR::RGWSyncLogTrimCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, const std::string& oid,
                                   const std::string& to_marker, std::string* last_trim_marker)
    : RGWRadosTimelogTrimCR(dpp, store, oid, real_time{}, real_time{}, std::string{}, to_marker)
    , cct(store->ctx())
...
int RGWAsyncStatObj::_send_request(const DoutPrefixProvider* dpp)
{
    ...
    return store->getRados()->raw_obj_stat(
        dpp, raw_obj, psize, pmtime, pepoch, nullptr, nullptr, objv_tracker, null_yield);
}

RGWStatObjCR::RGWStatObjCR(const DoutPrefixProvider* dpp, RGWAsyncRadosProcessor* async_rados,
                           rgw::sal::RadosStore* store, const RGWBucketInfo& _bucket_info, const rgw_obj& obj,
                           uint64_t* psize, real_time* pmtime, uint64_t* pepoch, RGWObjVersionTracker* objv_tracker)
    : RGWSimpleCoroutine(store->ctx())
    , dpp(dpp)
    , store(store)
...
{
    req = new RGWAsyncStatObj(
        dpp, this, stack->create_completion_notifier(), store, bucket_info, obj, psize, pmtime, pepoch, objv_tracker);
    async_rados->queue(req);
    return 0;
}
...
RGWRadosNotifyCR::RGWRadosNotifyCR(rgw::sal::RadosStore* store, const rgw_raw_obj& obj, bufferlist& request,
                                   uint64_t timeout_ms, bufferlist* response)
    : RGWSimpleCoroutine(store->ctx())
    , store(store)
    , obj(obj)
    , request(request)
...
int RGWRadosNotifyCR::send_request(const DoutPrefixProvider* dpp)
{
    ...
    cn = stack->create_completion_notifier();
    return ref.pool.ioctx().aio_notify(ref.obj.oid, cn->completion(), request, timeout_ms, response);
}
    ...
    reenter(this)
    {
        ...
        yield
        {
            rgw_http_param_pair pairs[] = {
                {"type", "data"}, {"notify2", NULL}, {"source-zone", source_zone}, {NULL, NULL}};
        ...
        if (retcode == -ERR_METHOD_NOT_ALLOWED) {
            ...
            {
                rgw_http_param_pair pairs[] = {
                    {"type", "data"}, {"notify", NULL}, {"source-zone", source_zone}, {NULL, NULL}};
