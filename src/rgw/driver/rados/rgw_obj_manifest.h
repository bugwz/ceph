...
public:
    ...
    void set_placement_rule(const rgw_placement_rule& rule) { placement_rule = rule; }
...
struct RGWObjManifestRule
{
    ...
    void decode(bufferlist::const_iterator& bl)
    {
        ...
        if (struct_v >= 2) decode(override_prefix, bl);
        DECODE_FINISH(bl);
    }
    void dump(Formatter* f) const;
};
...
protected:
    ...
    void convert_to_explicit(const DoutPrefixProvider* dpp, const RGWZoneGroup& zonegroup,
                             const RGWZoneParams& zone_params);
    int append_explicit(const DoutPrefixProvider* dpp, RGWObjManifest& m, const RGWZoneGroup& zonegroup,
                        const RGWZoneParams& zone_params);
    void append_rules(RGWObjManifest& m, std::map<uint64_t, RGWObjManifestRule>::iterator& iter,
                      std::string* override_prefix);
...
public:
    ...
    std::map<uint64_t, RGWObjManifestPart>& get_explicit_objs() { return objs; }
    ...
    void get_implicit_location(uint64_t cur_part_id, uint64_t cur_stripe, uint64_t ofs, std::string* override_prefix,
    ...
    bool empty() const
    {
        if (explicit_objs) return objs.empty();
        return rules.empty();
    }

    bool has_explicit_objs() const { return explicit_objs; }
    ...
    const rgw_obj& get_obj() const { return obj; }
    ...
    const rgw_bucket_placement& get_tail_placement() const { return tail_placement; }

    const rgw_placement_rule& get_head_placement_rule() const { return head_placement_rule; }

    void set_prefix(const std::string& _p) { prefix = _p; }

    const std::string& get_prefix() const { return prefix; }

    void set_tail_instance(const std::string& _ti) { tail_instance = _ti; }

    const std::string& get_tail_instance() const { return tail_instance; }

    void set_head_size(uint64_t _s) { head_size = _s; }

    void set_obj_size(uint64_t s) { obj_size = s; }

    uint64_t get_obj_size() const { return obj_size; }

    uint64_t get_head_size() const { return head_size; }

    uint64_t get_max_head_size() const { return max_head_size; }

    const std::string& get_tier_type() { return tier_type; }
    ...
    public:
        ...
        {}
        obj_iterator(const DoutPrefixProvider* _dpp, const RGWObjManifest* _m, uint64_t _ofs)
            : dpp(_dpp)
            , manifest(_m)
        {
            seek(_ofs);
        ...
        void operator++();
        bool operator==(const obj_iterator& rhs) const { return (ofs == rhs.ofs); }
        bool operator!=(const obj_iterator& rhs) const { return (ofs != rhs.ofs); }
        const rgw_obj_select& get_location() { return location; }

        /* where current part starts */
        uint64_t get_part_ofs() const { return part_ofs; }
        ...
        uint64_t get_ofs() const { return ofs; }

        int get_cur_part_id() const { return cur_part_id; }

        /* stripe number */
        int get_cur_stripe() const { return cur_stripe; }
    ...
    public:
        generator()
            ...
            , cur_stripe_size(0)
        {}
        int create_begin(CephContext* cct, RGWObjManifest* manifest, const rgw_placement_rule& head_placement_rule,
                         const rgw_placement_rule* tail_placement_rule, const rgw_bucket& bucket, const rgw_obj& obj);

        int create_next(uint64_t ofs);

        rgw_raw_obj get_cur_obj(RGWZoneGroup& zonegroup, RGWZoneParams& zone_params)
        {
        ...
        uint64_t cur_stripe_max_size() const { return cur_stripe_size; }
    };
};
