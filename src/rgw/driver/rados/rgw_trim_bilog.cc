...
void TrimCounters::Handler::handle(bufferlist::const_iterator& input, bufferlist& output)
{
    Request request;
...
void TrimComplete::Handler::handle(bufferlist::const_iterator& input, bufferlist& output)
{
    Request request;
    decode(request, input);
...
public:
    BucketTrimWatcher(rgw::sal::RadosStore* store, const rgw_raw_obj& obj, TrimCounters::Server* counters)
        : store(store)
        , obj(obj)
    {
        handlers.emplace(NotifyTrimCounters, std::make_unique<TrimCounters::Handler>(counters));
        handlers.emplace(NotifyTrimComplete, std::make_unique<TrimComplete::Handler>(counters));
    }

    ~BucketTrimWatcher() { stop(); }
    ...
    int start(const DoutPrefixProvider* dpp)
    {
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << "Failed to watch " << ref.obj << " with " << cpp_strerror(-r) << dendl;
    ...
    int restart()
    {
        ...
        if (r < 0) {
            lderr(store->ctx()) << "Failed to unwatch on " << ref.obj << " with " << cpp_strerror(-r) << dendl;
        }
        r = ref.pool.ioctx().watch2(ref.obj.oid, &handle, this);
        if (r < 0) {
            lderr(store->ctx()) << "Failed to restart watch on " << ref.obj << " with " << cpp_strerror(-r) << dendl;
            ref.pool.ioctx().close();
        }
        return r;
    }
...
public:
    BucketTrimShardCollectCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store,
                             const RGWBucketInfo& bucket_info, const rgw::bucket_index_layout_generation& generation,
...
bool BucketTrimShardCollectCR::spawn_next()
{
    while (i < markers.size()) {
        ...
        // skip empty markers
        if (!marker.empty()) {
            ldpp_dout(dpp, 10) << "trimming bilog shard " << shard_id << " of " << bucket_info.bucket << " at marker "
                               << marker << dendl;
            spawn(new RGWRadosBILogTrimCR(dpp, store, bucket_info, shard_id, generation, std::string{}, marker), false);
            return true;
        }
    }
    return false;
...
public:
    BucketCleanIndexCollectCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store,
                              const RGWBucketInfo& bucket_info, rgw::bucket_index_layout_generation index)
        : RGWShardCollectCR(store->ctx(), MAX_CONCURRENT_SHARDS)
        , dpp(dpp)
        , store(store)
        , bucket_info(bucket_info)
    ...
    bool spawn_next() override
    {
        if (shard < num_shards) {
            ...
            spawn(new RGWRadosRemoveOidCR(store, std::move(bs.bucket_obj), nullptr), false);
            ++shard;
            return true;
        }
        else {
...
private:
    ...
    std::optional<std::pair<RGWBucketInfo, rgw::bucket_log_layout_generation>> clean_info;
    ...
    int take_min_generation()
    {
        ...
        if (auto m = std::min_element(
                peer_status.begin(),
                peer_status.end(),
                [](const StatusShards& l, const StatusShards& r) { return l.generation < r.generation; });
            m != peer_status.end()) {
            min_generation = m->generation;
        }

        auto& logs = pbucket_info->layout.logs;
        auto log = std::find_if(logs.begin(), logs.end(), rgw::matches_gen(min_generation));
        if (log == logs.end()) {
            ldpp_dout(dpp, 5) << __PRETTY_FUNCTION__ << ":" << __LINE__
                              << "ERROR: No log layout for min_generation=" << min_generation << dendl;
            return -ENOENT;
        }
    ...
    int maybe_remove_generation()
    {
        ...
        if (pbucket_info->layout.logs.front().gen < totrim.gen) {
            ...
            if (clean_info->first.layout.logs.size() == 1) {
                ldpp_dout(dpp, -1) << "Critical error! Attempt to remove only log generation! "
                                   << "log.gen=" << log->gen << ", totrim.gen=" << totrim.gen << dendl;
                return -EIO;
            }
...
public:
    BucketTrimInstanceCR(rgw::sal::RadosStore* store, RGWHTTPManager* http, BucketTrimObserver* observer,
                         const std::string& bucket_instance, const DoutPrefixProvider* dpp)
...
int take_min_status(CephContext* cct, const uint64_t min_generation,
...
template<>
inline int parse_decode_json<BucketTrimInstanceCR::StatusShards>(BucketTrimInstanceCR::StatusShards& s, bufferlist& bl)
{
    JSONParser p;
...
int BucketTrimInstanceCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        yield call(new RGWBucketGetSyncPolicyHandlerCR(
            store->svc()->rados->get_async_processor(), store, get_policy_params, source_policy, dpp));
        if (retcode < 0) {
            if (retcode != -ENOENT) {
        ...
        if (auto& opt_bucket_info = source_policy->policy_handler->get_bucket_info(); opt_bucket_info) {
        ...
        yield
        {
            ...
            for (auto& zid : zids) {
                ...
                rgw_http_param_pair params[] = {
                    ...
                    {"status", nullptr},
                    {"options", "merge"},
                    {"bucket", bucket_instance.c_str()}, /* equal to source-bucket when `options==merge` and
                                                            source-bucket param is not provided */
                ...
                if (ziter == zone_conn_map.end()) {
                    ldpp_dout(dpp, 0) << "WARNING: no connection to zone " << zid << ", can't trim bucket: " << bucket
                                      << dendl;
                    return set_cr_error(-ECANCELED);
                }

                using StatusCR = RGWReadRESTResourceCR<StatusShards>;
                spawn(new StatusCR(cct, ziter->second, http, "/admin/log/", params, &*p), false);
                ++p;
            }
        }
        ...
        if (retcode < 0) {
            ldpp_dout(dpp, 4) << "error removing old generation from log: " << cpp_strerror(retcode) << dendl;
            return set_cr_error(retcode);
        }

        if (clean_info) {
            if (clean_info->second.layout.type != rgw::BucketLogType::InIndex) {
                ldpp_dout(dpp, 0) << "Unable to convert log of unknown type " << clean_info->second.layout.type
                                  << " to rgw::bucket_index_layout_generation " << dendl;
                return set_cr_error(-EINVAL);
            }

            yield call(
                new BucketCleanIndexCollectCR(dpp, store, clean_info->first, clean_info->second.layout.in_index));
            if (retcode < 0) {
                ldpp_dout(dpp, 0) << "failed to remove previous generation: " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }
            while (clean_info && retries < MAX_RETRIES) {
                yield call(new RGWPutBucketInstanceInfoCR(store->svc()->rados->get_async_processor(),
                                                          store,
                                                          clean_info->first,
                                                          false,
                                                          {},
                ...
                if (retcode == -ECANCELED) {
                    yield call(new RGWGetBucketInstanceInfoCR(store->svc()->rados->get_async_processor(),
                                                              store,
                                                              clean_info->first.bucket,
                                                              &(clean_info->first),
                                                              nullptr,
                                                              dpp));
                    if (retcode < 0) {
                        ldpp_dout(dpp, 0) << "failed to get bucket info: " << cpp_strerror(retcode) << dendl;
                        return set_cr_error(retcode);
                    }
                    if (clean_info->first.layout.logs.front().gen == clean_info->second.gen) {
                        clean_info->first.layout.logs.erase(clean_info->first.layout.logs.begin());
                        ++retries;
                        continue;
                    }
                ...
                if (retcode < 0) {
                    ldpp_dout(dpp, 0) << "failed to put bucket info: " << cpp_strerror(retcode) << dendl;
                    return set_cr_error(retcode);
                }
            ...
            if (totrim.layout.type != rgw::BucketLogType::InIndex) {
                ldpp_dout(dpp, 0) << "Unable to convert log of unknown type " << totrim.layout.type
                                  << " to rgw::bucket_index_layout_generation " << dendl;
                return set_cr_error(-EINVAL);
            }
            ...
            min_markers.assign(std::max(1u, rgw::num_shards(totrim.layout.in_index)), RGWSyncLogTrimCR::max_marker);


            retcode = take_min_status(cct, totrim.gen, peer_status.cbegin(), peer_status.cend(), &min_markers);
            if (retcode < 0) {
            ...
            ldpp_dout(dpp, 10) << "trimming bilogs for bucket=" << pbucket_info->bucket << " markers=" << min_markers
                               << ", shards=" << min_markers.size() << dendl;
            set_status("trimming bilog shards");
            yield call(new BucketTrimShardCollectCR(dpp, store, *pbucket_info, totrim.layout.in_index, min_markers));
            // ENODATA just means there were no keys to trim
            if (retcode == -ENODATA) {
                retcode = 0;
            }
            if (retcode < 0) {
                ldpp_dout(dpp, 4) << "failed to trim bilog shards: " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }
        }
...
public:
    BucketTrimInstanceCollectCR(rgw::sal::RadosStore* store, RGWHTTPManager* http, BucketTrimObserver* observer,
                                const std::vector<std::string>& buckets, int max_concurrent,
                                const DoutPrefixProvider* dpp)
...
public:
    AsyncMetadataList(CephContext* cct, RGWCoroutine* caller, RGWAioCompletionNotifier* cn, RGWMetadataManager* mgr,
                      const std::string& section, const std::string& start_marker, const MetadataListCallback& callback)
...
int AsyncMetadataList::_send_request(const DoutPrefixProvider* dpp)
{
    ...
    if (r == -EINVAL) {
        ...
        ldpp_dout(dpp, 10) << "failed to init metadata listing: " << cpp_strerror(r) << dendl;
        return r;
    }
    else {
        ...
        do {
            ...
            if (r < 0) {
                ldpp_dout(dpp, 10) << "failed to list metadata: " << cpp_strerror(r) << dendl;
                return r;
            }
    ...
    if (r < 0) {
        ldpp_dout(dpp, 10) << "failed to restart metadata listing: " << cpp_strerror(r) << dendl;
        return r;
    }
...
public:
    MetadataListCR(CephContext* cct, RGWAsyncRadosProcessor* async_rados, RGWMetadataManager* mgr,
                   const std::string& section, const std::string& start_marker, const MetadataListCallback& callback)
        : RGWSimpleCoroutine(cct)
        ...
        , callback(callback)
    {}
    ~MetadataListCR() override { request_cleanup(); }

    int send_request(const DoutPrefixProvider* dpp) override
    {
        req =
            new AsyncMetadataList(cct, this, stack->create_completion_notifier(), mgr, section, start_marker, callback);
...
public:
    BucketTrimCR(rgw::sal::RadosStore* store, RGWHTTPManager* http, const BucketTrimConfig& config,
                 BucketTrimObserver* observer, const rgw_raw_obj& obj, const DoutPrefixProvider* dpp)
        : RGWCoroutine(store->ctx())
        , store(store)
        , http(http)
        , config(config)
    ...
    reenter(this)
    {
        ...
        if (config.buckets_per_interval) {
            ...
            yield
            {
                ...
                encode(type, bl);
                encode(request, bl);
                call(new RGWRadosNotifyCR(store, obj, bl, config.notify_timeout_ms, &notify_replies));
            }
            if (retcode < 0) {
            ...
            const int max_count = config.buckets_per_interval - config.min_cold_buckets_per_interval;
            counter.get_highest(max_count, [this](const std::string& bucket, int count) { buckets.push_back(bucket); });
        }
        ...
        if (buckets.size() < config.buckets_per_interval) {
            ...
            if (retcode < 0) {
                ldpp_dout(dpp, 10) << "failed to read bilog trim status: " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }
            if (status.marker == "MAX") {
                status.marker.clear();   // restart at the beginning
            }
            ldpp_dout(dpp, 10) << "listing cold buckets from marker=" << status.marker << dendl;
        ...
        yield call(new BucketTrimInstanceCollectCR(store, http, observer, buckets, config.concurrent_buckets, dpp));
        ...
        yield
        {
            ...
            encode(type, bl);
            encode(request, bl);
            call(new RGWRadosNotifyCR(store, obj, bl, config.notify_timeout_ms, nullptr));
        }
        if (retcode < 0) {
        ...
        ldpp_dout(dpp, 4) << "bucket index log processing completed in " << ceph::mono_clock::now() - start_time
                          << dendl;
        return set_cr_done();
    }
    return 0;
...
public:
    BucketTrimPollCR(rgw::sal::RadosStore* store, RGWHTTPManager* http, const BucketTrimConfig& config,
                     BucketTrimObserver* observer, const rgw_raw_obj& obj, const DoutPrefixProvider* dpp)
        : RGWCoroutine(store->ctx())
        , store(store)
        , http(http)
    ...
    reenter(this)
    {
        for (;;) {
            ...
            yield call(new RGWSimpleRadosLockCR(
                store->svc()->rados->get_async_processor(), store, obj, name, cookie, config.trim_interval_sec));
            if (retcode < 0) {
            ...
            if (retcode < 0) {
                ...
                yield call(
                    new RGWSimpleRadosUnlockCR(store->svc()->rados->get_async_processor(), store, obj, name, cookie));
            }
        }
    }
    return 0;
...
template<typename T, typename Clock = ceph::coarse_mono_clock> class RecentEventList
{
public:
    ...
    template<typename U> bool lookup(const U& key) const
    {
...
void configure_bucket_trim(CephContext* cct, BucketTrimConfig& config)
{
    ...
    config.trim_interval_sec = conf.get_val<int64_t>("rgw_sync_log_trim_interval");
    config.counter_size = 512;
    config.buckets_per_interval = conf.get_val<int64_t>("rgw_sync_log_trim_max_buckets");
    config.min_cold_buckets_per_interval = conf.get_val<int64_t>("rgw_sync_log_trim_min_cold_buckets");
    config.concurrent_buckets = conf.get_val<int64_t>("rgw_sync_log_trim_concurrent_buckets");
    config.notify_timeout_ms = 10000;
    config.recent_size = 128;
    config.recent_duration = std::chrono::hours(2);
}

class BucketTrimManager::Impl : public TrimCounters::Server, public BucketTrimObserver
{
public:
    rgw::sal::RadosStore* const store;
    const BucketTrimConfig config;
    ...
    void get_bucket_counters(int count, TrimCounters::Vector& buckets)
    {
        ...
        counter.get_highest(count, [&buckets](const std::string& key, int count) { buckets.emplace_back(key, count); });
...
BucketTrimManager::BucketTrimManager(rgw::sal::RadosStore* store, const BucketTrimConfig& config)
...
{
    return new BucketTrimPollCR(impl->store, http, impl->config, impl.get(), impl->status_obj, this);
}
...
RGWCoroutine* BucketTrimManager::create_admin_bucket_trim_cr(RGWHTTPManager* http)
{
    ...
    return new BucketTrimCR(impl->store, http, impl->config, impl.get(), impl->status_obj, this);
}
...
int bilog_trim(const DoutPrefixProvider* p, rgw::sal::RadosStore* store, RGWBucketInfo& bucket_info, uint64_t gen,
               int shard_id, std::string_view start_marker, std::string_view end_marker)
{
    ...
    if (log == logs.end()) {
        ldpp_dout(p, 5) << __PRETTY_FUNCTION__ << ":" << __LINE__ << "ERROR: no log layout with gen=" << gen << dendl;
        return -ENOENT;
    }
    ...
    if (r < 0) {
        ldpp_dout(p, 5) << __PRETTY_FUNCTION__ << ":" << __LINE__ << "ERROR: bilog_rados->log_trim returned r=" << r
                        << dendl;
    }
    return r;
}
