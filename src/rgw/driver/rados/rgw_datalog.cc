...
void rgw_data_change::dump(ceph::Formatter* f) const
{
    std::string type;
    switch (entity_type) {
    case ENTITY_TYPE_BUCKET: type = "bucket"; break;
...
public:
    RGWDataChangesOmap(lr::IoCtx& ioctx, RGWDataChangesLog& datalog, uint64_t gen_id, int num_shards)
    ...
    void prepare(ceph::real_time ut, const std::string& key, ceph::buffer::list&& entry, entries& out) override
    {
    ...
    int push(const DoutPrefixProvider* dpp, int index, entries&& items, optional_yield y) override
    {
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": failed to push to " << oids[index] << cpp_strerror(-r)
                               << dendl;
        }
        return r;
    }
    int push(const DoutPrefixProvider* dpp, int index, ceph::real_time now, const std::string& key,
             ceph::buffer::list&& bl, optional_yield y) override
    {
        lr::ObjectWriteOperation op;
    ...
    int list(const DoutPrefixProvider* dpp, int index, int max_entries, std::vector<rgw_data_change_log_entry>& entries,
             std::optional<std::string_view> marker, std::string* out_marker, bool* truncated,
             optional_yield y) override
    {
        std::list<cls_log_entry> log_entries;
        lr::ObjectReadOperation op;
        cls_log_list(op, {}, {}, std::string(marker.value_or("")), max_entries, log_entries, out_marker, truncated);
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": failed to list " << oids[index] << cpp_strerror(-r)
                               << dendl;
            return r;
        }
        for (auto iter = log_entries.begin(); iter != log_entries.end(); ++iter) {
            ...
            try {
                ...
                ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": failed to decode data changes log entry: " << err.what()
                                   << dendl;
                return -EIO;
            }
    ...
    int get_info(const DoutPrefixProvider* dpp, int index, RGWDataChangesLogInfo* info, optional_yield y) override
    {
        ...
        if (r == -ENOENT) r = 0;
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": failed to get info from " << oids[index] << cpp_strerror(-r)
                               << dendl;
        }
        else {
    ...
    int trim(const DoutPrefixProvider* dpp, int index, std::string_view marker, optional_yield y) override
    {
        lr::ObjectWriteOperation op;
    ...
    int trim(const DoutPrefixProvider* dpp, int index, std::string_view marker, lr::AioCompletion* c) override
    {
        lr::ObjectWriteOperation op;
        cls_log_trim(op, {}, {}, {}, std::string(marker));
    ...
    int is_empty(const DoutPrefixProvider* dpp, optional_yield y) override
    {
        for (auto shard = 0u; shard < oids.size(); ++shard) {
            ...
            if (r < 0) {
                ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": failed to list " << oids[shard] << cpp_strerror(-r)
                                   << dendl;
                return r;
            }
...
public:
    RGWDataChangesFIFO(lr::IoCtx& ioctx, RGWDataChangesLog& datalog, uint64_t gen_id, int shards)
        : RGWDataChangesBE(ioctx, datalog, gen_id)
        , fifos(shards, [&ioctx, this](std::size_t i, auto emplacer) { emplacer.emplace(ioctx, get_oid(i)); })
    {}
    ~RGWDataChangesFIFO() override = default;
    void prepare(ceph::real_time, const std::string&, ceph::buffer::list&& entry, entries& out) override
    {
        if (!std::holds_alternative<centries>(out)) {
    ...
    int push(const DoutPrefixProvider* dpp, int index, entries&& items, optional_yield y) override
    {
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": unable to push to FIFO: " << get_oid(index) << ": "
                               << cpp_strerror(-r) << dendl;
        }
        return r;
    }
    int push(const DoutPrefixProvider* dpp, int index, ceph::real_time, const std::string&, ceph::buffer::list&& bl,
             optional_yield y) override
    {
             ...
             optional_yield y) override
    {
        ...
        bool more = false;
        auto r = fifos[index].list(dpp, max_entries, marker, &log_entries, &more, y);
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": unable to list FIFO: " << get_oid(index) << ": "
                               << cpp_strerror(-r) << dendl;
            return r;
        }
    ...
    int get_info(const DoutPrefixProvider* dpp, int index, RGWDataChangesLogInfo* info, optional_yield y) override
    {
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": unable to get FIFO metadata: " << get_oid(index) << ": "
                               << cpp_strerror(-r) << dendl;
            return r;
        }
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": unable to get part info: " << get_oid(index) << "/" << p
                               << ": " << cpp_strerror(-r) << dendl;
            return r;
        }
    ...
    int trim(const DoutPrefixProvider* dpp, int index, std::string_view marker, optional_yield y) override
    {
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": unable to trim FIFO: " << get_oid(index) << ": "
                               << cpp_strerror(-r) << dendl;
        }
        return r;
    }
    int trim(const DoutPrefixProvider* dpp, int index, std::string_view marker, librados::AioCompletion* c) override
    {
        int r = 0;
    ...
    int is_empty(const DoutPrefixProvider* dpp, optional_yield y) override
    {
        ...
        for (auto shard = 0u; shard < fifos.size(); ++shard) {
            ...
            if (r < 0) {
                ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": unable to list FIFO: " << get_oid(shard) << ": "
                                   << cpp_strerror(-r) << dendl;
                return r;
            }
            if (!log_entries.empty()) {
...
bs::error_code DataLogBackends::handle_init(entries_t e) noexcept
{
    ...
    for (const auto& [gen_id, gen] : e) {
        ...
        try {
            switch (gen.type) {
            case log_type::omap: emplace(gen_id, new RGWDataChangesOmap(ioctx, datalog, gen_id, shards)); break;
            case log_type::fifo: emplace(gen_id, new RGWDataChangesFIFO(ioctx, datalog, gen_id, shards)); break;
            default:
                lderr(datalog.cct) << __PRETTY_FUNCTION__ << ":" << __LINE__
                                   << ": IMPOSSIBLE: invalid log type: gen_id=" << gen_id << ", type" << gen.type
                                   << dendl;
            ...
            lderr(datalog.cct) << __PRETTY_FUNCTION__ << ":" << __LINE__
                               << ": error setting up backend: gen_id=" << gen_id << ", err=" << err.what() << dendl;
            return err.code();
        }
    }
    return {};
...
int RGWDataChangesLog::start(const DoutPrefixProvider* dpp, const RGWZone* _zone, const RGWZoneParams& zoneparams,
                             librados::Rados* lr)
{
    zone = _zone;
    ceph_assert(zone);
    auto defbacking = to_log_type(cct->_conf.get_val<std::string>("rgw_default_data_log_backing"));
    ...
    if (r < 0) {
        ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ": Failed to initialized ioctx, r=" << r << ", pool=" << log_pool
                           << dendl;
        return -r;
    }
    ...
    if (!besr) {
        lderr(cct) << __PRETTY_FUNCTION__ << ": Error initializing backends: " << besr.error().message() << dendl;
        return ceph::from_error_code(besr.error());
    }

    bes = std::move(*besr);
    renew_thread = make_named_thread("rgw_dt_lg_renew", &RGWDataChangesLog::renew_run, this);
    return 0;
}
...
auto RGWDataChangesLog::_get_change(const rgw_bucket_shard& bs, uint64_t gen) -> ChangeStatusPtr
{
...
void RGWDataChangesLog::register_renew(const rgw_bucket_shard& bs, const rgw::bucket_log_layout_generation& gen)
{
    std::scoped_lock l{lock};
    cur_cycle.insert({bs, gen.gen});
}

void RGWDataChangesLog::update_renewed(const rgw_bucket_shard& bs, uint64_t gen, real_time expiration)
{
    std::unique_lock l{lock};
    auto status = _get_change(bs, gen);
    l.unlock();

    ldout(cct, 20) << "RGWDataChangesLog::update_renewd() bucket_name=" << bs.bucket.name << " shard_id=" << bs.shard_id
...
bool RGWDataChangesLog::filter_bucket(const DoutPrefixProvider* dpp, const rgw_bucket& bucket, optional_yield y) const
{
    if (!bucket_filter) {
        return true;
    }
...
{
    return (gen_id > 0 ? fmt::format("{}@G{}.{}", prefix, gen_id, i) : fmt::format("{}.{}", prefix, i));
}

int RGWDataChangesLog::add_entry(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                 const rgw::bucket_log_layout_generation& gen, int shard_id, optional_yield y)
{
    ...
    ldpp_dout(dpp, 20) << "RGWDataChangesLog::add_entry() bucket.name=" << bucket.name << " shard_id=" << shard_id
                       << " now=" << now << " cur_expiration=" << status->cur_expiration << dendl;
...
int DataLogBackends::list(const DoutPrefixProvider* dpp, int shard, int max_entries,
                          std::vector<rgw_data_change_log_entry>& entries, std::string_view marker,
                          std::string* out_marker, bool* truncated, optional_yield y)
{
    ...
    while (max_entries > 0) {
        ...
        std::move(gentries.begin(), gentries.end(), std::back_inserter(entries));
        ++gen_id;
    }
    return 0;
}
                                    ...
                                    std::string* out_marker, bool* truncated, optional_yield y)
{
    ...
    return bes->list(dpp, shard, max_entries, entries, marker, out_marker, truncated, y);
}

int RGWDataChangesLog::list_entries(const DoutPrefixProvider* dpp, int max_entries,
                                    std::vector<rgw_data_change_log_entry>& entries, LogMarker& marker,
                                    bool* ptruncated, optional_yield y)
{
    bool truncated;
    entries.clear();
    for (; marker.shard < num_shards && int(entries.size()) < max_entries; marker.shard++, marker.marker.clear()) {
        int ret =
            list_entries(dpp, marker.shard, max_entries - entries.size(), entries, marker.marker, NULL, &truncated, y);
        if (ret == -ENOENT) {
            continue;
        }
        if (ret < 0) {
                                  ...
                                  optional_yield y)
{
    ...
    auto r = 0;
    for (auto be = lower_bound(0)->second; be->gen_id <= target_gen && be->gen_id <= head_gen && r >= 0;
...
public:
    ...
    GenTrim(const DoutPrefixProvider* dpp, DataLogBackends* bes, int shard_id, uint64_t target_gen, std::string cursor,
            uint64_t head_gen, uint64_t tail_gen, boost::intrusive_ptr<RGWDataChangesBE> be, lr::AioCompletion* super)
                                   ...
                                   librados::AioCompletion* c)
{
    ...
    l.unlock();
    auto gt =
        std::make_unique<GenTrim>(dpp, this, shard_id, target_gen, std::string(cursor), head_gen, tail_gen, be, c);

    auto cc = be->gen_id == target_gen ? cursor : be->max_marker();
    be->trim(dpp, shard_id, cc, GenTrim::call(std::move(gt)));
}

int DataLogBackends::trim_generations(const DoutPrefixProvider* dpp, std::optional<uint64_t>& through, optional_yield y)
{
    if (size() != 1) {
...
void RGWDataChangesLog::renew_run() noexcept
{
    ...
    for (;;) {
        ...
        if (run == runs_per_prune) {
            ...
            if (r < 0) {
                derr << "RGWDataChangesLog::ChangesRenewThread: failed pruning r=" << r << dendl;
            }
            else if (through) {
                ...
                ldpp_dout(&dp, 2) << "RGWDataChangesLog::ChangesRenewThread: nothing to prune." << dendl;
            }
            run = 0;
        }
        else {
