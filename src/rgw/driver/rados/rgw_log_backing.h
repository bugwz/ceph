...
{
    switch (t) {
    case log_type::omap: return m << "log_type::omap";
    case log_type::fifo: return m << "log_type::fifo";
    }
...
{
    return m << "[" << g.gen_id << "," << g.type << "," << (g.pruned ? "PRUNED" : "NOT PRUNED") << "]";
}
...
protected:
    ...
    logback_generations(librados::IoCtx& ioctx, std::string oid,
                        fu2::unique_function<std::string(uint64_t, int) const>&& get_oid, int shards) noexcept
        : ioctx(ioctx)
        , oid(oid)
...
private:
    ...
    tl::expected<std::pair<entries_t, obj_version>, bs::error_code> read(const DoutPrefixProvider* dpp,
    ...
    {
        return std::find_if(es.begin(), es.end(), [](const auto& e) { return !e.second.pruned; });
    }

public:
    /// For the use of watch/notify.

    void handle_notify(uint64_t notify_id, uint64_t cookie, uint64_t notifier_id, bufferlist& bl) override final;
    ...
    init(const DoutPrefixProvider* dpp, librados::IoCtx& ioctx_, std::string oid_,
         fu2::unique_function<std::string(uint64_t, int) const>&& get_oid_, int shards_, log_type def, optional_yield y,
         Args&&... args) noexcept
    {
        try {
            T* lgp = new T(ioctx_, std::move(oid_), std::move(get_oid_), shards_, std::forward<Args>(args)...);
    ...
    entries_t entries() const { return entries_; }
...
{
    return (gen_id > 0 ? fmt::format("G{:0>20}@{}", gen_id, cursor) : std::string(cursor));
}

inline std::pair<uint64_t, std::string_view> cursorgen(std::string_view cursor_)
{
    if (cursor_.empty()) {
        return {0, ""};
    }
...
public:
    ...
    int get_part_layout_info(const DoutPrefixProvider* dpp, std::uint32_t& part_header_size,
                             std::uint32_t& part_entry_overhead, optional_yield y)
    {
        auto r = lazy_init(dpp, y);
        if (r < 0) return r;
        std::tie(part_header_size, part_entry_overhead) = fifo->get_part_layout_info();
        return 0;
    }

    int push(const DoutPrefixProvider* dpp, const ceph::buffer::list& bl, optional_yield y)
    {
        auto r = lazy_init(dpp, y);
        if (r < 0) return r;
    ...
    int push(const DoutPrefixProvider* dpp, ceph::buffer::list& bl, librados::AioCompletion* c, optional_yield y)
    {
        auto r = lazy_init(dpp, y);
        if (r < 0) return r;
        fifo->push(dpp, bl, c);
    ...
    int push(const DoutPrefixProvider* dpp, const std::vector<ceph::buffer::list>& data_bufs, optional_yield y)
    {
        auto r = lazy_init(dpp, y);
        if (r < 0) return r;
    ...
    int push(const DoutPrefixProvider* dpp, const std::vector<ceph::buffer::list>& data_bufs,
    ...
    int list(const DoutPrefixProvider* dpp, int max_entries, std::optional<std::string_view> markstr,
             std::vector<rgw::cls::fifo::list_entry>* out, bool* more, optional_yield y)
    {
        auto r = lazy_init(dpp, y);
        if (r < 0) return r;
    ...
    int list(const DoutPrefixProvider* dpp, int max_entries, std::optional<std::string_view> markstr,
             std::vector<rgw::cls::fifo::list_entry>* out, bool* more, librados::AioCompletion* c, optional_yield y)
    {
        auto r = lazy_init(dpp, y);
        if (r < 0) return r;
    ...
    int get_head_info(const DoutPrefixProvider* dpp,
                      fu2::unique_function<void(int r, rados::cls::fifo::part_header&&)>&& f,
                      librados::AioCompletion* c, optional_yield y)
    {
        auto r = lazy_init(dpp, y);
        if (r < 0) return r;
