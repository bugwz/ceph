...
int RGWZoneGroup::create_default(const DoutPrefixProvider* dpp, optional_yield y, bool old_format)
{
    ...
    if (r < 0 && r != -EEXIST) {
        ...
        ldpp_dout(dpp, 20) << "zone_params::create_default() " << zone_params.get_name() << " id "
    ...
    default_zone.supported_features.insert(rgw::zone_features::supported.begin(), rgw::zone_features::supported.end());
    // enable default zonegroup features
    enabled_features.insert(rgw::zone_features::enabled.begin(), rgw::zone_features::enabled.end());
...
{
    if (is_master && other_zonegroup.empty()) return true;

    return (id == other_zonegroup);
}

int RGWZoneGroup::add_zone(const DoutPrefixProvider* dpp, const RGWZoneParams& zone_params, bool* is_master,
                           bool* read_only, const list<string>& endpoints, const string* ptier_type,
                           ...
                           RGWSyncModulesManager* sync_mgr, const rgw::zone_features::set& enable_features,
                           const rgw::zone_features::set& disable_features, optional_yield y)
{
    ...
    if (!zones.count(zone_id)) {
        for (const auto& zone : zones) {
            if (zone.second.name == zone_name) {
                ldpp_dout(dpp, 0) << "ERROR: found existing zone name " << zone_name << " (" << zone.first
    ...
    zone.supported_features.insert(enable_features.begin(), enable_features.end());
    ...
    for (const auto& feature : disable_features) {
        ...
        if (i == zone.supported_features.end()) {
            ldout(cct, 1) << "WARNING: zone feature \"" << feature << "\" was not enabled in zone " << zone.name
                          << dendl;
            continue;
        }
...
int RGWZoneGroup::rename_zone(const DoutPrefixProvider* dpp, const RGWZoneParams& zone_params, optional_yield y)
{
...
void RGWZoneGroup::post_process_params(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    for (auto& item : zones) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "WARNING: could not read zone params for zone id=" << zone.id << " name=" << zone.name
                              << dendl;
            continue;
        }
...
int RGWZoneGroup::remove_zone(const DoutPrefixProvider* dpp, const std::string& zone_id, optional_yield y)
{
    ...
    if (iter == zones.end()) {
        ldpp_dout(dpp, 0) << "zone id " << zone_id << " is not a part of zonegroup " << name << dendl;
        return -ENOENT;
    }
    ...
    for (auto& i : zonegroup.zones) {
        ...
        for (auto& s : short_zone_ids) {
            if (s.second == short_id) {
                ldout(cct, 0) << "New zone '" << zone.name << "' (" << zone.id << ") generates the same short_zone_id "
                              << short_id << " as existing zone id " << s.first << dendl;
                return -EEXIST;
            }
        }
...
bool RGWPeriodMap::find_zone_by_name(const string& zone_name, RGWZoneGroup* zonegroup, RGWZone* zone) const
{
...
int read_realm(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore, std::string_view realm_id,
               std::string_view realm_name, RGWRealm& info, std::unique_ptr<sal::RealmWriter>* writer)
{
    if (!realm_id.empty()) {
...
int create_realm(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore, bool exclusive,
                 RGWRealm& info, std::unique_ptr<sal::RealmWriter>* writer_out)
{
    if (info.name.empty()) {
    ...
    if (!info.current_period.empty()) {
        period.emplace();
        int r = cfgstore->read_period(dpp, y, info.current_period, std::nullopt, *period);
        if (r < 0) {
            ldpp_dout(dpp, -1) << __func__ << " failed to read realm's current_period=" << info.current_period
    ...
    if (!period) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << __func__ << " failed to create the initial period id=" << period->id << " for realm "
    ...
    if (r < 0 && r != -EEXIST) {
        ldpp_dout(dpp, 0) << "WARNING: failed to set realm as default: " << cpp_strerror(r) << dendl;
    }
...
int set_default_realm(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore, const RGWRealm& info,
                      bool exclusive)
{
    return cfgstore->write_default_realm_id(dpp, y, exclusive, info.id);
}

int realm_set_current_period(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore,
                             sal::RealmWriter& writer, RGWRealm& realm, const RGWPeriod& period)
{
    // update realm epoch to match the period's
    if (realm.epoch > period.realm_epoch) {
        ldpp_dout(dpp, -1) << __func__ << " with old realm epoch " << period.realm_epoch
                           << ", current epoch=" << realm.epoch << dendl;
        return -EINVAL;
    }
    if (realm.epoch == period.realm_epoch && realm.current_period != period.id) {
        ldpp_dout(dpp, -1) << __func__ << " with same realm epoch " << period.realm_epoch
                           << ", but different period id " << period.id << " != " << realm.current_period << dendl;
        return -EINVAL;
    }
    ...
    if (r < 0) {
        ldpp_dout(dpp, -1) << __func__ << " failed to overwrite realm " << realm.name << " with " << cpp_strerror(r)
                           << dendl;
        return r;
    }
...
int reflect_period(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore, const RGWPeriod& info)
{
    // overwrite the local period config and zonegroup objects
    constexpr bool exclusive = false;

    int r = cfgstore->write_period_config(dpp, y, exclusive, info.realm_id, info.period_config);
    if (r < 0) {
        ldpp_dout(dpp, -1) << __func__ << " failed to store period config for realm id=" << info.realm_id << " with "
    ...
    for (auto& [zonegroup_id, zonegroup] : info.period_map.zonegroups) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << __func__ << " failed to store zonegroup id=" << zonegroup_id << " with "
                               << cpp_strerror(r) << dendl;
            return r;
        }
        if (zonegroup.is_master) {
            ...
            if (r == 0) {
                ldpp_dout(dpp, 1) << "Set the period's master zonegroup " << zonegroup.name << " as the default"
                                  << dendl;
            }
        }
    }
...
{
    ldpp_dout(dpp, 20) << __func__ << " realm id=" << info.realm_id << " period id=" << info.id << dendl;
...
int update_period(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore, RGWPeriod& info)
{
    ...
    do {
        int ret = cfgstore->list_zonegroup_names(dpp, y, listing.next, zonegroup_names, listing);
        if (ret < 0) {
        ...
        for (const auto& name : listing.entries) {
            ...
            if (ret < 0) {
                ldpp_dout(dpp, 0) << "WARNING: failed to read zonegroup " << name << ": " << cpp_strerror(-ret)
                                  << dendl;
                continue;
            }

            if (zg.realm_id != info.realm_id) {
                ldpp_dout(dpp, 20) << "skipping zonegroup " << zg.get_name() << " with realm id " << zg.realm_id
            ...
            if (zg.zones.find(zg.master_zone) == zg.zones.end()) {
                ldpp_dout(dpp, 0) << "ERROR: zonegroup " << zg.get_name() << " has a non existent master zone "
                                  << dendl;
                return -EINVAL;
            }
    ...
    int ret = cfgstore->read_period_config(dpp, y, info.realm_id, info.period_config);
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(dpp, 0) << "ERROR: failed to read period config: " << cpp_strerror(ret) << dendl;
        return ret;
    }
...
int commit_period(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore, sal::Driver* driver,
                  RGWRealm& realm, sal::RealmWriter& realm_writer, const RGWPeriod& current_period, RGWPeriod& info,
                  std::ostream& error_stream, bool force_if_stale)
{
    auto zone_svc = static_cast<rgw::sal::RadosStore*>(driver)->svc()->zone;   // XXX

    ldpp_dout(dpp, 20) << __func__ << " realm " << realm.id << " period " << current_period.id << dendl;
    // gateway must be in the master zone to commit
    if (info.master_zone != zone_svc->get_zone_params().id) {
        error_stream << "Cannot commit period on zone " << zone_svc->get_zone_params().id
                     << ", it must be sent to "
                        "the period's master zone "
                     << info.master_zone << '.' << std::endl;
        return -EINVAL;
    }
    // period predecessor must match current period
    if (info.predecessor_uuid != current_period.id) {
        error_stream << "Period predecessor " << info.predecessor_uuid << " does not match current period "
                     << current_period.id
                     << ". Use 'period pull' to get the latest period from the master, "
                        "reapply your changes, and try again."
                     << std::endl;
        return -EINVAL;
    }
    ...
    if (info.realm_epoch != current_period.realm_epoch + 1) {
        error_stream << "Period's realm epoch " << info.realm_epoch
                     << " does not come directly after current realm epoch " << current_period.realm_epoch
    ...
    if (info.master_zone != current_period.master_zone) {
        ...
        int r = info.update_sync_status(dpp, driver, current_period, error_stream, force_if_stale);
        if (r < 0) {
        ...
        ldpp_dout(dpp, 4) << "Promoted to master zone and committed new period " << info.id << dendl;
        (void)cfgstore->realm_notify_new_period(dpp, y, info);
        return 0;
    }
    // period must be based on current epoch
    if (info.epoch != current_period.epoch) {
        error_stream << "Period epoch " << info.epoch
                     << " does not match "
                        "predecessor epoch "
                     << current_period.epoch
                     << ". Use "
    ...
    ldpp_dout(dpp, 4) << "Committed new epoch " << info.epoch << " for period " << info.id << dendl;
    (void)cfgstore->realm_notify_new_period(dpp, y, info);
    return 0;
}
...
int read_zonegroup(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore,
                   std::string_view zonegroup_id, std::string_view zonegroup_name, RGWZoneGroup& info,
                   std::unique_ptr<sal::ZoneGroupWriter>* writer)
{
    ...
    if (r == -ENOENT) {
        return cfgstore->read_zonegroup_by_name(dpp, y, default_zonegroup_name, info, writer);
    }
    if (r < 0) {
        return r;
    }
...
int create_zonegroup(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore, bool exclusive,
                     RGWZoneGroup& info)
{
    if (info.name.empty()) {
    ...
    if (r < 0) {
        ldpp_dout(dpp, 0) << "failed to create zonegroup with " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    if (r < 0 && r != -EEXIST) {
        ldpp_dout(dpp, 0) << "WARNING: failed to set zonegroup as default: " << cpp_strerror(r) << dendl;
    }
...
{
    return cfgstore->write_default_zonegroup_id(dpp, y, exclusive, info.realm_id, info.id);
}

int remove_zone_from_group(const DoutPrefixProvider* dpp, RGWZoneGroup& zonegroup, const rgw_zone_id& zone_id)
{
    ...
    if (zonegroup.master_zone == zone_id) {
        ...
        if (m != zonegroup.zones.end()) {
            ...
            ldpp_dout(dpp, 0) << "NOTICE: promoted " << m->second.name << " as new master_zone of zonegroup "
                              << zonegroup.name << dendl;
        }
        else {
            ldpp_dout(dpp, 0) << "NOTICE: removed master_zone of zonegroup " << zonegroup.name << dendl;
        }
    }
...
static int remove_zone_from_groups(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore,
                                   const rgw_zone_id& zone_id)
{
    std::array<std::string, 128> zonegroup_names;
    sal::ListResult<std::string> listing;
    do {
        int r = cfgstore->list_zonegroup_names(dpp, y, listing.next, zonegroup_names, listing);
        if (r < 0) {
            ldpp_dout(dpp, 0) << "failed to list zonegroups with " << cpp_strerror(r) << dendl;
            return r;
        }
        ...
        for (const auto& name : listing.entries) {
            ...
            if (r < 0) {
                ldpp_dout(dpp, 0) << "WARNING: failed to load zonegroup " << name << " with " << cpp_strerror(r)
                                  << dendl;
                continue;
            }
            ...
            if (r < 0) {
                ldpp_dout(dpp, 0) << "WARNING: failed to write zonegroup " << name << " with " << cpp_strerror(r)
                                  << dendl;
                continue;
            }
...
int read_zone(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore, std::string_view zone_id,
              std::string_view zone_name, RGWZoneParams& info, std::unique_ptr<sal::ZoneWriter>* writer)
{
    if (!zone_id.empty()) {
...
extern int get_zones_pool_set(const DoutPrefixProvider* dpp, optional_yield y, rgw::sal::ConfigStore* cfgstore,
                              std::string_view my_zone_id, std::set<rgw_pool>& pools);

int create_zone(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore, bool exclusive,
                RGWZoneParams& info, std::unique_ptr<sal::ZoneWriter>* writer)
{
    ...
    placement.storage_classes.set_storage_class(RGW_STORAGE_CLASS_STANDARD, &pool, nullptr);
    ...
    if (r < 0) {
        ldpp_dout(dpp, 0) << "failed to create zone with " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    if (r < 0 && r != -EEXIST) {
        ldpp_dout(dpp, 0) << "WARNING: failed to set zone as default: " << cpp_strerror(r) << dendl;
    }
...
{
    return cfgstore->write_default_zone_id(dpp, y, exclusive, info.realm_id, info.id);
}

int delete_zone(const DoutPrefixProvider* dpp, optional_yield y, sal::ConfigStore* cfgstore, const RGWZoneParams& info,
