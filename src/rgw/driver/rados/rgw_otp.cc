...
public:
    ...
    void dump(Formatter* f) const override { encode_json("devices", devices, f); }

    otp_devices_list_t& get_devs() { return devices; }
};
...
class RGWOTPMetadataHandler : public RGWOTPMetadataHandlerBase
{
    ...
    int init(RGWSI_Zone* zone, RGWSI_MetaBackend* _meta_be, RGWSI_OTP* _otp)
    {
               ...
               const DoutPrefixProvider* dpp) override
    {
        ...
        int ret = svc.otp->read_all(be_ctx, entry, &mdo->get_devs(), &mdo->get_mtime(), &objv_tracker, y, dpp);
        if (ret < 0) {
            return ret;
        }
               ...
               RGWMDLogSyncType type, bool from_remote_zone) override
    {
        ...
        int ret = svc.otp->store_all(dpp, be_ctx, entry, obj->devices, obj->mtime, &objv_tracker, y);
        if (ret < 0) {
            return ret;
        }
                  ...
                  optional_yield y, const DoutPrefixProvider* dpp) override
    {
        ...
        return svc.otp->remove_all(dpp, be_ctx, entry, &objv_tracker, y);
    }
...
RGWOTPCtl::RGWOTPCtl(RGWSI_Zone* zone_svc, RGWSI_OTP* otp_svc)
{
    svc.zone = zone_svc;
    svc.otp = otp_svc;
}
...
int RGWOTPCtl::read_all(const rgw_user& uid, RGWOTPInfo* info, optional_yield y, const DoutPrefixProvider* dpp,
...
int RGWOTPCtl::store_all(const DoutPrefixProvider* dpp, const RGWOTPInfo& info, optional_yield y,
...
int RGWOTPCtl::remove_all(const DoutPrefixProvider* dpp, const rgw_user& uid, optional_yield y,
                          const RemoveParams& params)
{
    return meta_handler->call(
        [&](RGWSI_OTP_BE_Ctx& ctx) { return svc.otp->remove_all(dpp, ctx, uid, params.objv_tracker, y); });
}
