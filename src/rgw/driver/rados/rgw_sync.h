...
public:
    ...
    RGWCoroutine* log_error_cr(const DoutPrefixProvider* dpp, const std::string& source_zone,
                               const std::string& section, const std::string& name, uint32_t error_code,
...
struct rgw_sync_error_info
{
    ...
    rgw_sync_error_info()
        : error_code(0)
    {}
    rgw_sync_error_info(const std::string& _source_zone, uint32_t _error_code, const std::string& _message)
...
public:
    explicit RGWSyncBackoff(int _max_secs = DEFAULT_BACKOFF_MAX)
        : cur_wait(0)
        , max_secs(_max_secs)
    {}

    void backoff_sleep();
    void reset() { cur_wait = 0; }
...
protected:
    bool* backoff_ptr() { return &reset_backoff; }

    ceph::mutex& cr_lock() { return lock; }

    RGWCoroutine* get_cr() { return cr; }
...
public:
    RGWRemoteMetaLog(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* _store, RGWAsyncRadosProcessor* async_rados,
    ...
    int read_master_log_shards_info(const DoutPrefixProvider* dpp, const std::string& master_period,
                                    std::map<int, RGWMetadataLogInfo>* shards_info);
    int read_master_log_shards_next(const DoutPrefixProvider* dpp, const std::string& period,
                                    std::map<int, std::string> shard_markers,
                                    std::map<int, rgw_mdlog_shard_data>* result);
    ...
    RGWMetaSyncEnv& get_sync_env() { return sync_env; }
};
...
public:
    ...
    void stop() { master_log.finish(); }
};

class RGWOrderCallCR : public RGWCoroutine
{
public:
    RGWOrderCallCR(CephContext* cct)
        : RGWCoroutine(cct)
    {}
...
public:
    explicit RGWLastCallerWinsCR(CephContext* cct)
        : RGWOrderCallCR(cct)
    {}
    ~RGWLastCallerWinsCR()
    {
...
template<class T, class K> class RGWSyncShardMarkerTrack
{
    struct marker_entry
    {
        uint64_t pos;
        ...
        marker_entry()
            : pos(0)
        {}
        marker_entry(uint64_t _p, const real_time& _ts)
            : pos(_p)
            , timestamp(_ts)
        {}
    };
...
public:
    RGWSyncShardMarkerTrack(int _window_size)
     ...
     */
    bool need_retry(const K& key) { return (need_retry_set.find(key) != need_retry_set.end()); }

    void set_need_retry(const K& key) { need_retry_set.insert(key); }

    void reset_need_retry(const K& key) { need_retry_set.erase(key); }
...
public:
    RGWMetaSyncSingleEntryCR(RGWMetaSyncEnv* _sync_env, const std::string& _raw_key, const std::string& _entry_marker,
                             const RGWMDLogStatus& _op_status, RGWMetaSyncShardMarkerTrack* _marker_tracker,
