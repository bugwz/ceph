...
static std::string default_zonegroup_oid(const ceph::common::ConfigProxy& conf, std::string_view realm_id)
{
    const auto prefix = name_or_default(conf->rgw_default_zonegroup_info_oid, default_zonegroup_info_oid);
    return fmt::format("{}.{}", prefix, realm_id);
}


int RadosConfigStore::write_default_zonegroup_id(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive,
...
int RadosConfigStore::read_default_zonegroup_id(const DoutPrefixProvider* dpp, optional_yield y,
...
int RadosConfigStore::delete_default_zonegroup_id(const DoutPrefixProvider* dpp, optional_yield y,
...
public:
    RadosZoneGroupWriter(ConfigImpl* impl, RGWObjVersionTracker objv, std::string_view zonegroup_id,
...
int RadosConfigStore::create_zonegroup(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive,
                                       const RGWZoneGroup& info, std::unique_ptr<sal::ZoneGroupWriter>* writer)
{
    ...
    if (writer) {
        *writer = std::make_unique<RadosZoneGroupWriter>(impl.get(), std::move(objv), info.get_id(), info.get_name());
    }
    return 0;
}

int RadosConfigStore::read_zonegroup_by_id(const DoutPrefixProvider* dpp, optional_yield y,
...
int RadosConfigStore::read_zonegroup_by_name(const DoutPrefixProvider* dpp, optional_yield y,
...
int RadosConfigStore::read_default_zonegroup(const DoutPrefixProvider* dpp, optional_yield y, std::string_view realm_id,
                                             RGWZoneGroup& info, std::unique_ptr<sal::ZoneGroupWriter>* writer)
{
    const auto& pool = impl->zonegroup_pool;
...
int RadosConfigStore::list_zonegroup_names(const DoutPrefixProvider* dpp, optional_yield y, const std::string& marker,
