...
{
    switch (key_type) {
    case KEY_TYPE_SWIFT: return "swift"; break;

    default: return "s3"; break;
    }
}
...
{
    if (user->get_id().empty() || subuser.empty()) return "";
                                     ...
                                     bool defer_user_update, optional_yield y)
{
    ...
    if (!op_state.has_existing_key()) {
        set_err_msg(err_msg, "unable to find access key,  with key type: " + key_type_to_str(key_type));
        return -ERR_INVALID_ACCESS_KEY;
    }
...
int RGWUser::init(const DoutPrefixProvider* dpp, rgw::sal::Driver* _driver, RGWUserAdminOpState& op_state,
                  optional_yield y)
{
    init_default();
                            ...
                            optional_yield y)
{
    ...
    do {
        ...
        for (auto it = m.begin(); it != m.end(); ++it) {
            ...
            ret = rgw_chown_bucket_and_objects(driver, bucket.get(), new_user.get(), std::string(), nullptr, dpp, y);
            if (ret < 0) {
                            ...
                            optional_yield y)
{
    ...
    if (op_state.has_bucket_quota()) user_info.quota.bucket_quota = op_state.get_bucket_quota();

    if (op_state.has_user_quota()) user_info.quota.user_quota = op_state.get_user_quota();
...
int RGWUserAdminOp_User::info(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, RGWUserAdminOpState& op_state,
...
int RGWUserAdminOp_User::create(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, RGWUserAdminOpState& op_state,
                                RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_User::modify(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, RGWUserAdminOpState& op_state,
                                RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_User::remove(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, RGWUserAdminOpState& op_state,
                                RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Subuser::create(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver,
...
int RGWUserAdminOp_Subuser::modify(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver,
                                   RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Subuser::remove(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver,
                                   RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Key::create(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, RGWUserAdminOpState& op_state,
                               RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Key::remove(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, RGWUserAdminOpState& op_state,
                               RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Caps::add(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, RGWUserAdminOpState& op_state,
                             RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
...
int RGWUserAdminOp_Caps::remove(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, RGWUserAdminOpState& op_state,
                                RGWFormatterFlusher& flusher, optional_yield y)
{
    RGWUserInfo info;
    RGWUser user;
                             ...
                             bool* is_truncated, optional_yield y, uint64_t default_max)
{
    ...
    int ret = svc.user->list_buckets(dpp, user, marker, end_marker, max, buckets, is_truncated, y);
    if (ret < 0) {
        return ret;
    }
    if (need_stats) {
