 ...
 * in the head object.
 */
void read_cloudtier_info_from_attrs(rgw::sal::Attrs& attrs, RGWObjCategory& category, RGWObjManifest& manifest)
{
...
int AtomicObjectProcessor::process_first_chunk(bufferlist&& data, DataProcessor** processor)
{
...
int AtomicObjectProcessor::prepare(optional_yield y)
{
    ...
    if (same_pool) {
        ...
        if (!store->svc.zone->get_zone_params().get_placement(bucket_info.placement_rule.name, &placement_info) ||
            placement_info.inline_data) {
    ...
    r = manifest_gen.create_begin(
        store->ctx(), &manifest, bucket_info.placement_rule, &tail_placement_rule, head_obj.bucket, head_obj);
    if (r < 0) {
        return r;
    }
...
int AtomicObjectProcessor::complete(size_t accounted_size, const std::string& etag, ceph::real_time* mtime,
                                    ceph::real_time set_mtime, rgw::sal::Attrs& attrs, ceph::real_time delete_at,
                                    const char* if_match, const char* if_nomatch, const std::string* user_data,
                                    rgw_zone_set* zones_trace, bool* pcanceled, optional_yield y)
{
    int r = writer.drain();
    if (r < 0) {
        return r;
...
int MultipartObjectProcessor::process_first_chunk(bufferlist&& data, DataProcessor** processor)
{
...
int MultipartObjectProcessor::prepare_head()
{
    ...
    if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: unexpected: get_max_chunk_size(): placement_rule=" << tail_placement_rule.to_str()
                          << " obj=" << target_obj << " returned r=" << r << dendl;
        return r;
    }
    store->get_max_aligned_size(default_stripe_size, alignment, &stripe_size);
    ...
    r = manifest_gen.create_begin(
        store->ctx(), &manifest, bucket_info.placement_rule, &tail_placement_rule, target_obj.bucket, target_obj);
    if (r < 0) {
        return r;
    }
...
int MultipartObjectProcessor::complete(size_t accounted_size, const std::string& etag, ceph::real_time* mtime,
                                       ceph::real_time set_mtime, std::map<std::string, bufferlist>& attrs,
                                       ceph::real_time delete_at, const char* if_match, const char* if_nomatch,
                                       const std::string* user_data, rgw_zone_set* zones_trace, bool* pcanceled,
                                       optional_yield y)
{
    int r = writer.drain();
    if (r < 0) {
    ...
    ldpp_dout(dpp, 20) << "Update meta: " << meta_obj_ref.obj.oid << " part " << p << " prefix "
                       << info.manifest.get_prefix() << " return " << r << dendl;
...
{
    RGWObjState* astate;
    int r = store->get_obj_state(dpp, &obj_ctx, bucket_info, head_obj, &astate, &cur_manifest, y);
    if (r < 0) {
        return r;
    }
                                    ...
                                    rgw_zone_set* zones_trace, bool* pcanceled, optional_yield y)
{
    ...
    if (!cur_etag.empty()) {
        ...
        snprintf(&final_etag_str[CEPH_CRYPTO_MD5_DIGESTSIZE * 2],
                 ...
                 "-%lld",
                 (long long)cur_part_num);
        bufferlist etag_bl;
        etag_bl.append(final_etag_str, strlen(final_etag_str) + 1);
        attrs[RGW_ATTR_ETAG] = etag_bl;
    }
    r = obj_op.write_meta(dpp, actual_size + cur_size, accounted_size + *cur_accounted_size, attrs, y);
    if (r < 0) {
        return r;
    }
    if (!obj_op.meta.canceled) {
