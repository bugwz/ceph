...
int D3nCacheAioWriteRequest::d3n_libaio_prepare_write_op(bufferlist& bl, unsigned int len, string oid,
                                                         string cache_location)
{
    std::string location = cache_location + url_encode(oid, true);
    int r = 0;

    lsubdout(g_ceph_context, rgw_datacache, 20)
        << "D3nDataCache: " << __func__ << "(): Write To Cache, location=" << location << dendl;
    cb = new struct aiocb;
    mode_t mode = S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH;
    memset(cb, 0, sizeof(struct aiocb));
    r = fd = ::open(location.c_str(), O_WRONLY | O_CREAT | O_TRUNC, mode);
    if (fd < 0) {
        ldout(cct, 0) << "ERROR: D3nCacheAioWriteRequest::create_io: open file failed, errno=" << errno
                      << ", location='" << location.c_str() << "'" << dendl;
        goto done;
    }
    if (g_conf()->rgw_d3n_l1_fadvise != POSIX_FADV_NORMAL) posix_fadvise(fd, 0, 0, g_conf()->rgw_d3n_l1_fadvise);
    cb->aio_fildes = fd;
...
void D3nDataCache::init(CephContext* _cct)
{
    ...
    try {
        if (efs::exists(cache_location)) {
            ...
            if (g_conf()->rgw_d3n_l1_evict_cache_on_start) {
                lsubdout(g_ceph_context, rgw, 5)
                    << "D3nDataCache: init: evicting the persistent storage directory on start" << dendl;
            ...
            lsubdout(g_ceph_context, rgw, 5)
                << "D3nDataCache: init: creating the persistent storage directory on start" << dendl;
            efs::create_directories(cache_location);
        }
    }
    catch (const efs::filesystem_error& e) {
        lderr(g_ceph_context) << "D3nDataCache: init: ERROR initializing the cache storage directory '"
                              << cache_location << "' : " << e.what() << dendl;
    }
    ...
    if (conf_eviction_policy == "lru") eviction_policy = _eviction_policy::LRU;
    if (conf_eviction_policy == "random") eviction_policy = _eviction_policy::RANDOM;
...
int D3nDataCache::d3n_io_write(bufferlist& bl, unsigned int len, std::string oid)
{
    ...
    if (nbytes != len) {
        ldout(cct, 0) << "ERROR: D3nDataCache::io_write: fwrite has returned error: nbytes!=len, nbytes=" << nbytes
                      << ", len=" << len << dendl;
        return -EIO;
    }
...
{
    lsubdout(g_ceph_context, rgw_datacache, 30)
        << "D3nDataCache: " << __func__ << "(): Write To Cache, oid=" << oid << ", len=" << len << dendl;
...
void D3nDataCache::put(bufferlist& bl, unsigned int len, std::string& oid)
{
    ...
    ldout(cct, 20) << "D3nDataCache: Before eviction _free_data_cache_size:" << _free_data_cache_size
                   << ", _outstanding_write_size:" << _outstanding_write_size << ", freed_size:" << freed_size << dendl;
    while (len > (_free_data_cache_size - _outstanding_write_size + freed_size)) {
        ...
        if (eviction_policy == _eviction_policy::LRU) {
            ...
            ldout(cct, 0) << "D3nDataCache: Warning: unknown cache eviction policy, defaulting to lru eviction"
                          << dendl;
            sr = lru_eviction();
        }
        if (sr == 0) {
            ldout(cct, 2) << "D3nDataCache: Warning: eviction was not able to free disk space, not writing to cache"
                          << dendl;
...
size_t D3nDataCache::random_eviction()
{
    ...
    string del_oid, location;
    {
        ...
        ldout(cct, 20) << "D3nDataCache: random_eviction: index:" << random_index << ", free size: " << del_entry->size
                       << dendl;
