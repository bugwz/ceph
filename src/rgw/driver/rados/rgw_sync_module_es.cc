...
struct ESInfo
{
    ...
    std::string get_version_str() { return std::to_string(version.first) + "." + std::to_string(version.second); }
};
...
struct es_version_decoder
{
    ...
    void decode_json(JSONObj* obj)
    {
        ...
        if (parse_version(s) < 0) throw JSONDecoder::err("Failed to parse ElasticVersion");
    }
};
...
struct ElasticConfig
{
    ...
    void init(CephContext* cct, const JSONFormattable& config)
    {
        ...
        if (string user = config["username"], pw = config["password"]; !user.empty() && !pw.empty()) {
    ...
    string get_index_path() { return index_path; }

    map<string, string>& get_request_headers() { return default_headers; }

    string get_obj_path(const RGWBucketInfo& bucket_info, const rgw_obj_key& key)
    {
        if (es_info.version >= ES_V7) {
            return index_path + "/_doc/" +
                   url_encode(bucket_info.bucket.bucket_id + ":" + key.name + ":" +
                              (key.instance.empty() ? "null" : key.instance));
            ;
        }
        else {
            return index_path + "/object/" +
                   url_encode(bucket_info.bucket.bucket_id + ":" + key.name + ":" +
                              (key.instance.empty() ? "null" : key.instance));
        }
    }
    ...
    {
        return index_buckets.exists(bucket_info.bucket.name) && allow_owners.exists(bucket_info.owner.to_str());
    }
};
...
struct es_type_v2
{
    ...
    void dump(Formatter* f) const
    {
        ...
        if (estype == ESType::String && !is_analyzed) {
            is_analyzed = false;
        }
...
template<class T> struct es_type : public T
{
    es_type(T t)
        : T(t)
    {}
...
template<class T> struct es_index_mappings
{
    ESVersion es_version;
    ESType string_type{ESType::String};

    es_index_mappings(ESVersion esv)
        : es_version(esv)
    {}

    es_type<T> est(ESType t) const { return es_type<T>(t); }
    ...
    void dump(Formatter* f) const
    {
        if (es_version <= ES_V7) f->open_object_section("object");
        f->open_object_section("properties");
        ...
        encode_json("mtime", est(ESType::Date).set_format("strict_date_optional_time||epoch_millis"), f);
        ...
        if (es_version <= ES_V7) f->close_section();   // object
    }
};
...
struct es_index_settings
{
    ...
    es_index_settings(uint32_t _replicas, uint32_t _shards)
...
template<class T> struct es_index_config : public es_index_config_base
{
    es_index_settings settings;
    es_index_mappings<T> mappings;

    es_index_config(es_index_settings& _s, ESVersion esv)
        : settings(_s)
        , mappings(esv)
    {}
...
struct es_obj_metadata
{
    ...
    es_obj_metadata(CephContext* _cct, ElasticConfigRef _es_conf, const RGWBucketInfo& _bucket_info,
                    const rgw_obj_key& _key, ceph::real_time& _mtime, uint64_t _size, map<string, bufferlist>& _attrs,
    ...
    void dump(Formatter* f) const
    {
        ...
        for (auto i : attrs) {
            ...
            if (attr_name == RGW_ATTR_ACL) {
                try {
                    ...
                    ldpp_dout(dpp, 0) << "ERROR: failed to decode acl for " << bucket_info.bucket << "/" << key
                                      << dendl;
                    continue;
                }
                ...
                try {
                    ...
                    ldpp_dout(dpp, 0) << "ERROR: failed to decode obj tags for " << bucket_info.bucket << "/" << key
                                      << dendl;
                    continue;
                }
            }
                ...
                try {
                    ...
                    ldpp_dout(dpp, 0) << "ERROR: failed to decode compression attr for " << bucket_info.bucket << "/"
                                      << key << dendl;
                    continue;
                }
        ...
        for (auto i : custom_meta) {
            ...
            if (config == bucket_info.mdsearch_config.end()) {
                if (!es_conf->explicit_custom_meta) {
                    ...
                    ldpp_dout(dpp, 20) << "custom meta entry key=" << i.first
                                       << " not found in bucket mdsearch config: " << bucket_info.mdsearch_config
                                       << dendl;
                }
                continue;
            }
            switch (config->second) {
            case ESEntityTypeMap::ES_ENTITY_DATE: custom_date[i.first] = i.second; break;
            case ESEntityTypeMap::ES_ENTITY_INT: custom_int[i.first] = i.second; break;
            default: custom_str[i.first] = i.second;
            }
        }
        ...
        if (!custom_date.empty()) {
            ...
            for (auto i : custom_date) {
                ...
                if (r < 0) {
                    ldpp_dout(dpp, 20) << __func__ << "(): failed to parse time (" << i.second
...
{
public:
    RGWElasticGetESInfoCBCR(RGWDataSyncCtx* _sc, ElasticConfigRef _conf)
...
{
public:
    RGWElasticPutIndexCBCR(RGWDataSyncCtx* _sc, ElasticConfigRef _conf)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , conf(_conf)
        ...
        reenter(this)
        {
            ...
            if (retcode < 0) {
                ...
                if (err_response.error.type != "index_already_exists_exception" &&
                    ...
                    ldpp_dout(dpp, 0) << "elasticsearch: failed to initialize index: response.type="
                                      << err_response.error.type << " response.reason=" << err_response.error.reason
                                      << dendl;
                    return set_cr_error(retcode);
                }
...
private:
    ...
    struct _err_response
    {
        ...
        void decode_json(JSONObj* obj) { JSONDecoder::decode_json("error", error, obj); }
    } err_response;
};
...
public:
    RGWElasticInitConfigCBCR(RGWDataSyncCtx* _sc, ElasticConfigRef _conf)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , conf(_conf)
...
public:
    RGWElasticHandleRemoteObjCBCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, rgw_obj_key& _key,
                                  ElasticConfigRef _conf, uint64_t _versioned_epoch)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            yield
            {
                ...
                es_obj_metadata doc(
                    sync_env->cct, conf, sync_pipe.dest_bucket_info, key, mtime, size, attrs, versioned_epoch);

                call(new RGWPutRESTResourceCR<es_obj_metadata, int>(sync_env->cct,
                                                                    conf->conn.get(),
                                                                    sync_env->http_manager,
                                                                    path,
                                                                    nullptr /* params */,
...
public:
    RGWElasticHandleRemoteObjCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, rgw_obj_key& _key,
                                ElasticConfigRef _conf, uint64_t _versioned_epoch)
...
public:
    RGWElasticRemoveRemoteObjCBCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, rgw_obj_key& _key,
                                  const ceph::real_time& _mtime, ElasticConfigRef _conf)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , sync_env(_sc->env)
        , sync_pipe(_sync_pipe)
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ldpp_dout(dpp, 10) << ": remove remote obj: z=" << sc->source_zone
                               << " b=" << sync_pipe.info.source_bs.bucket << " k=" << key << " mtime=" << mtime
                               << dendl;
            yield
            {
                string path = conf->get_obj_path(sync_pipe.dest_bucket_info, key);

                call(new RGWDeleteRESTResourceCR(
                    sync_env->cct, conf->conn.get(), sync_env->http_manager, path, nullptr /* params */));
            }
            if (retcode < 0) {
                return set_cr_error(retcode);
            }
...
public:
    RGWElasticDataSyncModule(const DoutPrefixProvider* dpp, CephContext* cct, const JSONFormattable& config)
        : conf(std::make_shared<ElasticConfig>())
    {
    ...
    {
        ldpp_dout(dpp, 10) << conf->id << ": sync_object: b=" << sync_pipe.info.source_bs.bucket << " k=" << key
                                ...
                                rgw_zone_set* zones_trace) override
    {
        ...
        ldpp_dout(dpp, 10) << conf->id << ": rm_object: b=" << sync_pipe.info.source_bs.bucket << " k=" << key
    ...
    RGWCoroutine* create_delete_marker(const DoutPrefixProvider* dpp, RGWDataSyncCtx* sc,
                                       ...
                                       rgw_zone_set* zones_trace) override
    {
        ldpp_dout(dpp, 10) << conf->id << ": create_delete_marker: b=" << sync_pipe.info.source_bs.bucket
                           << " k=" << key << " mtime=" << mtime << " versioned=" << versioned
                           << " versioned_epoch=" << versioned_epoch << dendl;
        ldpp_dout(dpp, 10) << conf->id << ": skipping operation (not handled)" << dendl;
        return NULL;
    }
    RGWRESTConn* get_rest_conn() { return conf->conn.get(); }

    string get_index_path() { return conf->get_index_path(); }

    map<string, string>& get_request_headers() { return conf->get_request_headers(); }
};

RGWElasticSyncModuleInstance::RGWElasticSyncModuleInstance(const DoutPrefixProvider* dpp, CephContext* cct,
...
int RGWElasticSyncModule::create_instance(const DoutPrefixProvider* dpp, CephContext* cct,
