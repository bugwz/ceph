...
public:
    DatalogTrimImplCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, int shard, const std::string& marker,
                      std::string* last_trim_marker)
    ...
    {
        set_description() << "Datalog trim shard=" << shard << " marker=" << marker;
    }
    ...
    int send_request(const DoutPrefixProvider* dpp) override
    {
        ...
        return store->svc()->datalog_rados->trim_entries(dpp, shard, marker, cn->completion());
    }
    int request_complete() override
    {
        int r = cn->completion()->get_return_value();
        ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << "(): trim of shard=" << shard << " marker=" << marker
                           << " returned r=" << r << dendl;
        ...
        if (*last_trim_marker < marker && marker != store->svc()->datalog_rados->max_marker()) {
...
template<typename IterIn, typename IterOut> void take_min_markers(IterIn first, IterIn last, IterOut dest)
{
    if (first == last) {
        return;
    }
...
public:
    DataLogTrimCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, RGWHTTPManager* http, int num_shards,
        ...
        , min_shard_markers(num_shards, std::string(store->svc()->datalog_rados->max_marker()))
        , last_trim(last_trim)
    {}
    ...
    reenter(this)
    {
        ...
        yield
        {
            ...
            rgw_http_param_pair params[] = {
                {"type", "data"}, {"status", nullptr}, {"source-zone", zone_id.c_str()}, {nullptr, nullptr}};
            ...
            for (auto& c : store->svc()->zone->get_zone_data_notify_to_map()) {
                ...
                spawn(new StatusCR(cct, c.second, http, "/admin/log/", params, &*p), false);
                ++p;
            }
        }
        ...
        yield
        {
            ...
            for (int i = 0; i < num_shards; i++) {
                ...
                ldpp_dout(dpp, 10) << "trimming log shard " << i << " at marker=" << m << " last_trim=" << last_trim[i]
                                   << dendl;
                spawn(new TrimCR(dpp, store, i, m, &last_trim[i]), true);
            }
        }
        return set_cr_done();
    }
...
RGWCoroutine* create_admin_data_log_trim_cr(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store,
                                            RGWHTTPManager* http, int num_shards, std::vector<std::string>& markers)
{
...
public:
    DataLogTrimPollCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, RGWHTTPManager* http, int num_shards,
                      utime_t interval)
        : RGWCoroutine(store->ctx())
        , dpp(dpp)
        , store(store)
...
int DataLogTrimPollCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        for (;;) {
            ...
            yield call(new RGWSimpleRadosLockCR(store->svc()->rados->get_async_processor(),
                                                store,
            ...
            if (retcode < 0) {
                ...
                ldpp_dout(dpp, 4) << "failed to lock " << lock_oid << ", trying again in " << interval.sec() << "s"
                                  << dendl;
                continue;
            }
...
RGWCoroutine* create_data_log_trim_cr(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, RGWHTTPManager* http,
