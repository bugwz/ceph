...
public:
    RGWBucketReshardLock(rgw::sal::RadosStore* _store, const std::string& reshard_lock_oid, bool _ephemeral);
    RGWBucketReshardLock(rgw::sal::RadosStore* _store, const RGWBucketInfo& bucket_info, bool _ephemeral)
    ...
    bool should_renew(const Clock::time_point& now) const { return now >= renew_thresh; }
};   // class RGWBucketReshardLock
...
private:
    ...
    int do_reshard(const rgw::bucket_index_layout_generation& current,
                   const rgw::bucket_index_layout_generation& target, int max_entries, bool verbose, std::ostream* os,
                   Formatter* formatter, const DoutPrefixProvider* dpp);

public:
    // pass nullptr for the final parameter if no outer reshard lock to
    // manage
    RGWBucketReshard(rgw::sal::RadosStore* _store, const RGWBucketInfo& _bucket_info,
                     const std::map<std::string, bufferlist>& _bucket_attrs, RGWBucketReshardLock* _outer_reshard_lock);
    int execute(int num_shards, ReshardFaultInjector& f, int max_op_entries, const DoutPrefixProvider* dpp,
                bool verbose = false, std::ostream* out = nullptr, ceph::Formatter* formatter = nullptr,
    ...
    static int clear_resharding(rgw::sal::RadosStore* store, RGWBucketInfo& bucket_info,
                                std::map<std::string, bufferlist>& bucket_attrs, const DoutPrefixProvider* dpp);

    static uint32_t get_max_prime_shards() { return *std::crbegin(reshard_primes); }
    ...
    {
        auto it = std::upper_bound(reshard_primes.begin(), reshard_primes.end(), requested_shards);
    ...
    static uint32_t get_prime_shards_greater_or_equal(uint32_t requested_shards)
    {
        auto it = std::lower_bound(reshard_primes.begin(), reshard_primes.end(), requested_shards);
    ...
    static uint32_t get_preferred_shards(uint32_t suggested_shards, uint32_t max_dynamic_shards)
    {

        // use a prime if max is within our prime range, otherwise use
        // specified max
        const uint32_t absolute_max = max_dynamic_shards >= get_max_prime_shards()
                                          ? max_dynamic_shards
        ...
        const uint32_t final_num_shards = std::min(prime_ish_num_shards, absolute_max);

        return final_num_shards;
    }

    const std::map<std::string, bufferlist>& get_bucket_attrs() const { return bucket_attrs; }
    ...
    static bool can_reshard(const RGWBucketInfo& bucket, const RGWSI_Zone* zone_svc);
};   // RGWBucketReshard
...
protected:
    ...
    public:
        ReshardWorker(CephContext* const _cct, RGWReshard* const _reshard)
            : cct(_cct)
            , reshard(_reshard)
        {}
...
public:
    RGWReshard(rgw::sal::RadosStore* _store, bool _verbose = false, std::ostream* _out = nullptr,
    ...
    int list(const DoutPrefixProvider* dpp, int logshard_num, std::string& marker, uint32_t max,
             std::list<cls_rgw_reshard_entry>& entries, bool* is_truncated);
    int clear_bucket_resharding(const DoutPrefixProvider* dpp, const std::string& bucket_instance_oid,
                                cls_rgw_reshard_entry& entry);

    /* reshard thread */
    int process_entry(const cls_rgw_reshard_entry& entry, int max_entries, const DoutPrefixProvider* dpp);
...
public:
    RGWReshardWait(ceph::timespan duration = std::chrono::seconds(5))
        : duration(duration)
    {}
    ~RGWReshardWait() { ceph_assert(going_down); }
