...
int create_etag_verifier(const DoutPrefixProvider* dpp, CephContext* cct, rgw::sal::DataProcessor* filter,
                         const bufferlist& manifest_bl, const std::optional<RGWCompressionInfo>& compression,
                         etag_verifier_ptr& verifier)
{
    ...
    for (auto mi = manifest.obj_begin(dpp); mi != manifest.obj_end(dpp); ++mi) {
        if (cur_part_ofs == mi.get_part_ofs()) continue;
    ...
    if (compression) {
        ...
        for (auto& ofs : part_ofs) {
            ...
            constexpr auto less = [](const compression_block& block, uint64_t ofs) { return block.new_ofs < ofs; };
            block = std::lower_bound(block, blocks.end(), ofs, less);
            if (block == blocks.end() || block->new_ofs != ofs) {
                ldpp_dout(dpp, 4) << "no match for compressed offset " << ofs << ", disabling etag verification"
                                  << dendl;
                return -EIO;
            }
            ofs = block->old_ofs;
...
{
    bufferlist out;
    if (in.length() > 0) hash.Update((const unsigned char*)in.c_str(), in.length());
...
void ETagVerifier_Atomic::calculate_etag()
{
    ...
    if (!calculated_etag.empty()) return;

    hash.Final(m);
    buf_to_hex(m, CEPH_CRYPTO_MD5_DIGESTSIZE, calc_md5);
    calculated_etag = calc_md5;
    ldout(cct, 20) << "Single part object: "
                   << " etag:" << calculated_etag << dendl;
}
...
int ETagVerifier_MPU::process(bufferlist&& in, uint64_t logical_offset)
{
    ...
    if (bl_end > part_ofs[next_part_index]) {
        ...
        hash.Update((const unsigned char*)in.c_str() + part_one_len, bl_end - part_ofs[cur_part_index]);
        /*
         ...
         */
        if (size_t(next_part_index) == part_ofs.size()) goto done;
    }
    else {
        hash.Update((const unsigned char*)in.c_str(), in.length());
    }
    ...
    if (logical_offset + in.length() + 1 == part_ofs[next_part_index]) process_end_of_MPU_part();
