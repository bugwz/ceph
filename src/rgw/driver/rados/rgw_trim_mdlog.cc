...
public:
    PurgeLogShardsCR(rgw::sal::RadosStore* store, const RGWMetadataLog* mdlog, const rgw_pool& pool, int num_shards)
...
public:
    PurgePeriodLogsCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, epoch_t realm_epoch,
                      epoch_t* last_trim)
        : RGWCoroutine(store->ctx())
        , dpp(dpp)
        , store(store)
        , metadata(store->ctl()->meta.mgr)
...
int PurgePeriodLogsCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        while (cursor.get_epoch() < realm_epoch) {
            ...
            if (retcode < 0) {
                ldpp_dout(dpp, 1) << "failed to remove log shards: " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }
            ...
            if (retcode == -ENOENT) {
                ...
                ldpp_dout(dpp, 1) << "failed to remove log shards for realm_epoch=" << cursor.get_epoch()
                                  << " period=" << cursor.get_period().get_id() << " with: " << cpp_strerror(retcode)
                                  << dendl;
                return set_cr_error(retcode);
            }
...
connection_map make_peer_connections(rgw::sal::RadosStore* store, const Zonegroups& zonegroups)
{
...
template<typename Iter> int take_min_status(CephContext* cct, Iter first, Iter last, rgw_meta_sync_status* status)
{
    if (first == last) {
        return -EINVAL;
    }
    ...
    for (auto p = first; p != last; ++p) {
        ...
        if (p->sync_markers.size() != num_shards) {
            ldout(cct, 1) << "take_min_status got peer status with " << p->sync_markers.size() << " shards, expected "
                          << num_shards << dendl;
            return -EINVAL;
        }
...
public:
    MetaMasterTrimShardCollectCR(MasterTrimEnv& env, RGWMetadataLog* mdlog, const rgw_meta_sync_status& sync_status)
...
bool MetaMasterTrimShardCollectCR::spawn_next()
{
    while (shard_id < env.num_shards) {
        ...
        if (stable <= last_trim) {
            // already trimmed
            ldpp_dout(env.dpp, 20) << "skipping log shard " << shard_id << " at marker=" << stable
                                   << " last_trim=" << last_trim << " realm_epoch=" << sync_status.sync_info.realm_epoch
                                   << dendl;
            shard_id++;
            continue;
        }
        ...
        ldpp_dout(env.dpp, 10) << "trimming log shard " << shard_id << " at marker=" << stable
                               << " last_trim=" << last_trim << " realm_epoch=" << sync_status.sync_info.realm_epoch
                               << dendl;
...
class MetaMasterStatusCollectCR : public RGWShardCollectCR
{
    ...
    int handle_result(int r) override
    {
        ...
        if (r < 0) {
            ldout(cct, 4) << "failed to fetch metadata sync status: " << cpp_strerror(r) << dendl;
        }
        return r;
    }
...
public:
    explicit MetaMasterStatusCollectCR(MasterTrimEnv& env)
        : RGWShardCollectCR(env.store->ctx(), MAX_CONCURRENT_SHARDS)
        , env(env)
        , c(env.connections.begin())
        , s(env.peer_status.begin())
    {}
    ...
    bool spawn_next() override
    {
        ...
        static rgw_http_param_pair params[] = {{"type", "metadata"}, {"status", nullptr}, {nullptr, nullptr}};
        ...
        spawn(new StatusCR(cct, conn, env.http, "/admin/log/", params, &*s), false);
        ++c;
        ++s;
        return true;
    }
...
int MetaMasterTrimCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        ret = take_min_status(env.store->ctx(), env.peer_status.begin(), env.peer_status.end(), &min_status);
        if (ret < 0) {
        ...
        yield
        {
            ...
            ldpp_dout(dpp, 4) << "realm epoch min=" << epoch << " current=" << env.current.get_epoch() << dendl;
            if (epoch > env.last_trim_epoch + 1) {
                ...
                ldpp_dout(dpp, 10) << "mdlogs already purged up to realm_epoch " << env.last_trim_epoch << dendl;
            }
...
public:
    MetaPeerTrimShardCR(RGWMetaSyncEnv& env, RGWMetadataLog* mdlog, const std::string& period_id, int shard_id,
...
int MetaPeerTrimShardCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        yield call(create_list_remote_mdlog_shard_cr(&env, period_id, shard_id, "", 1, &result));
        if (retcode < 0) {
            ldpp_dout(dpp, 5) << "failed to read first entry from master's mdlog shard " << shard_id << " for period "
                              << period_id << ": " << cpp_strerror(retcode) << dendl;
            return set_cr_error(retcode);
        }
        if (result.entries.empty()) {
            ...
            // that
            ldpp_dout(dpp, 10) << "empty master mdlog shard " << shard_id << ", reading last timestamp from shard info"
                               << dendl;
            ...
            if (retcode < 0) {
                ldpp_dout(dpp, 5) << "failed to read info from master's mdlog shard " << shard_id << " for period "
                                  << period_id << ": " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }
            ...
            ldpp_dout(dpp, 10) << "got mdlog shard info with last update=" << info.last_update << dendl;
            // re-read the master's first mdlog entry to make sure it hasn't changed
            yield call(create_list_remote_mdlog_shard_cr(&env, period_id, shard_id, "", 1, &result));
            if (retcode < 0) {
                ldpp_dout(dpp, 5) << "failed to read first entry from master's mdlog shard " << shard_id
                                  << " for period " << period_id << ": " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }
        ...
        if (stable <= *last_trim) {
            ldpp_dout(dpp, 10) << "skipping log shard " << shard_id << " at timestamp=" << stable
                               << " last_trim=" << *last_trim << dendl;
            return set_cr_done();
        }

        ldpp_dout(dpp, 10) << "trimming log shard " << shard_id << " at timestamp=" << stable
                           << " last_trim=" << *last_trim << dendl;
        yield
        {
            std::string oid;
        ...
        if (retcode < 0 && retcode != -ENODATA) {
            ldpp_dout(dpp, 1) << "failed to trim mdlog shard " << shard_id << ": " << cpp_strerror(retcode) << dendl;
            return set_cr_error(retcode);
        }
        *last_trim = stable;
        return set_cr_done();
...
public:
    MetaPeerTrimShardCollectCR(PeerTrimEnv& env, RGWMetadataLog* mdlog)
        ...
        , env(env)
        , mdlog(mdlog)
        , period_id(env.current.get_period().get_id())
    {
        meta_env.init(env.dpp,
                      cct,
                      env.store,
                      env.store->svc()->zone->get_master_conn(),
                      env.store->svc()->rados->get_async_processor(),
                      env.http,
                      nullptr,
...
bool MetaPeerTrimShardCollectCR::spawn_next()
{
    ...
    spawn(new MetaPeerTrimShardCR(meta_env, mdlog, period_id, shard_id, &last_trim), false);
    shard_id++;
    return true;
}
...
public:
    explicit MetaPeerTrimCR(PeerTrimEnv& env)
        : RGWCoroutine(env.store->ctx())
        , env(env)
    {}
...
int MetaPeerTrimCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        yield
        {
            ...
            rgw_http_param_pair params[] = {{"type", "metadata"}, {nullptr, nullptr}};

            using LogInfoCR = RGWReadRESTResourceCR<rgw_mdlog_info>;
            call(new LogInfoCR(
                cct, env.store->svc()->zone->get_master_conn(), env.http, "/admin/log/", params, &mdlog_info));
        }
        if (retcode < 0) {
        ...
        if (mdlog_info.realm_epoch > env.last_trim_epoch + 1) {
            // delete any prior mdlog periods
            yield call(new PurgePeriodLogsCR(dpp, env.store, mdlog_info.realm_epoch, &env.last_trim_epoch));
        }
        else {
            ldpp_dout(dpp, 10) << "mdlogs already purged through realm_epoch " << env.last_trim_epoch << dendl;
        }
    ...
    reenter(this)
    {
        for (;;) {
            ...
            yield call(new RGWSimpleRadosLockCR(
                store->svc()->rados->get_async_processor(), store, obj, name, cookie, interval.sec()));
            if (retcode < 0) {
...
class MetaMasterTrimPollCR : public MetaTrimPollCR
{
    ...
    RGWCoroutine* alloc_cr() override { return new MetaMasterTrimCR(env); }
...
class MetaPeerTrimPollCR : public MetaTrimPollCR
{
    ...
    RGWCoroutine* alloc_cr() override { return new MetaPeerTrimCR(env); }

public:
    MetaPeerTrimPollCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, RGWHTTPManager* http, int num_shards,
                       utime_t interval)
        : MetaTrimPollCR(store, interval)
        , env(dpp, store, http, num_shards)
    {}
...
bool sanity_check_endpoints(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store)
{
    ...
    for (const auto& [_, zonegroup] : period.get_map().zonegroups) {
        if (zonegroup.endpoints.empty()) {
            ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " WARNING: Cluster is is misconfigured! "
                               << " Zonegroup " << zonegroup.get_name() << " (" << zonegroup.get_id() << ") in Realm "
                               << period.get_realm_name() << " ( " << period.get_realm() << ") "
                               << " has no endpoints!" << dendl;
        }
        for (const auto& [_, zone] : zonegroup.zones) {
            if (zone.endpoints.empty()) {
                ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " ERROR: Cluster is is misconfigured! "
                                   << " Zone " << zone.name << " (" << zone.id << ") in Zonegroup "
                                   << zonegroup.get_name() << " ( " << zonegroup.get_id() << ") in Realm "
                                   << period.get_realm_name() << " ( " << period.get_realm() << ") "
...
{
    MetaMasterAdminTrimCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, RGWHTTPManager* http,
                          int num_shards)
...
{
    MetaPeerAdminTrimCR(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store, RGWHTTPManager* http,
                        int num_shards)
