...
class RGWGC : public DoutPrefixProvider
{
    ...
    public:
        GCWorker(const DoutPrefixProvider* _dpp, CephContext* _cct, RGWGC* _gc)
            : dpp(_dpp)
            , cct(_cct)
            , gc(_gc)
        {}
...
public:
    ...
    std::tuple<int, std::optional<cls_rgw_obj_chain>> send_split_chain(const cls_rgw_obj_chain& chain,
    ...
    int list(int* index, std::string& marker, uint32_t max, bool expired_only, std::list<cls_rgw_gc_obj_info>& result,
             bool* truncated, bool& processing_queue);
    void list_init(int* index) { *index = 0; }
    int process(int index, int process_max_secs, bool expired_only, RGWGCIOManager& io_manager);
