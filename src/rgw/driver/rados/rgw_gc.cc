...
void RGWGC::initialize(CephContext* _cct, RGWRados* _store)
{
    ...
    for (int i = 0; i < max_objs; i++) {
        ...
        const uint64_t queue_size = cct->_conf->rgw_gc_max_queue_size,
                       num_deferred_entries = cct->_conf->rgw_gc_max_deferred;
...
std::tuple<int, std::optional<cls_rgw_obj_chain>> RGWGC::send_split_chain(const cls_rgw_obj_chain& chain,
                                                                          const std::string& tag)
{
    ...
    if (cct->_conf->rgw_max_chunk_size) {
        ...
        ldpp_dout(this, 20) << "RGWGC::send_split_chain - rgw_max_chunk_size is: " << cct->_conf->rgw_max_chunk_size
                            << dendl;
        ...
        for (auto it = chain.objs.begin(); it != chain.objs.end(); it++) {
            ...
            if (total_encoded_size > cct->_conf->rgw_max_chunk_size) {   // dont add to chain, and send to gc
                ...
                --it;
                ldpp_dout(this, 20) << "RGWGC::send_split_chain - more than, dont add to broken chain and send chain"
                                    << dendl;
                auto ret = send_chain(broken_chain, tag);
                if (ret < 0) {
                    broken_chain.objs.insert(
                        broken_chain.objs.end(),
                        it,
...
int RGWGC::async_defer_chain(const string& tag, const cls_rgw_obj_chain& chain)
{
    ...
    state->info.tag = tag;
    state->completion = librados::Rados::aio_create_completion(state.get(), async_defer_callback);
...
int RGWGC::list(int* index, string& marker, uint32_t max, bool expired_only, std::list<cls_rgw_gc_obj_info>& result,
                bool* truncated, bool& processing_queue)
{
    result.clear();
    string next_marker;
    ...
    for (; *index < max_objs && result.size() < max; (*index)++, marker.clear(), check_queue = false) {
        ...
        // processing_queue is set to true from previous iteration if the queue was under process and probably has more
        // elements in it.
        if (!transitioned_objects_cache[*index] && !check_queue && !processing_queue) {
            ret = cls_rgw_gc_list(store->gc_pool_ctx,
                                  obj_names[*index],
                                  marker,
                                  max - result.size(),
                                  expired_only,
        ...
        if (transitioned_objects_cache[*index] || check_queue || processing_queue) {
            ...
            ret = cls_rgw_gc_queue_list_entries(store->gc_pool_ctx,
                                                obj_names[*index],
                                                marker,
                                                (max - result.size()) - entries.size(),
                                                expired_only,
                                                queue_entries,
                                                truncated,
                                                next_marker);
        ...
        if (entries.size() == 0 && queue_entries.size() == 0) continue;
...
public:
    RGWGCIOManager(const DoutPrefixProvider* _dpp, CephContext* _cct, RGWGC* _gc)
        : dpp(_dpp)
        , cct(_cct)
        , gc(_gc)
    {
    ...
    int handle_next_completion()
    {
        ...
        if (io.type == IO::IndexIO && !gc->transitioned_objects_cache[io.index]) {
            if (ret < 0) {
                ldpp_dout(dpp, 0) << "WARNING: gc cleanup of tags on gc shard index=" << io.index
    ...
    void flush_remove_tags(int index, vector<string>& rt)
    {
        ...
        ldpp_dout(dpp, 20) << __func__ << " removing entries from gc log shard index=" << index
                           << ", size=" << rt.size() << ", entries=" << rt << dendl;

        auto rt_guard = make_scope_guard([&] { rt.clear(); });
        ...
        if (ret < 0) {
             ...
             */
            ldpp_dout(dpp, 0) << "WARNING: failed to remove tags on gc shard index=" << index << " ret=" << ret
                              << dendl;
            return;
        }
        if (perfcounter) {
...
{
    ldpp_dout(this, 20) << "RGWGC::process entered with GC index_shard=" << index << ", max_secs=" << max_secs
                        << ", expired_only=" << expired_only << dendl;
    ...
    do {
        ...
        if (!transitioned_objects_cache[index]) {
            ret = cls_rgw_gc_list(
                store->gc_pool_ctx, obj_names[index], marker, max, expired_only, entries, &truncated, next_marker);
            ldpp_dout(this, 20) << "RGWGC::process cls_rgw_gc_list returned with returned:" << ret
            ...
            if ((objv.ver == 1) && entries.size() == 0) {
                ...
                if (non_expired_entries.size() == 0) {
                    transitioned_objects_cache[index] = true;
                    marker.clear();
                    ldpp_dout(this, 20) << "RGWGC::process cls_rgw_gc_list returned NO non expired entries, so setting "
                                           "cache entry to TRUE"
                                        << dendl;
                }
                else {
        ...
        if (transitioned_objects_cache[index]) {
            ret = cls_rgw_gc_queue_list_entries(
                store->gc_pool_ctx, obj_names[index], marker, max, expired_only, entries, &truncated, next_marker);
            ldpp_dout(this, 20) << "RGWGC::process cls_rgw_gc_queue_list_entries returned with return value:" << ret
                                << ", entries.size=" << entries.size() << ", truncated=" << truncated
                                << ", next_marker='" << next_marker << "'" << dendl;
            if (entries.size() == 0) {
                ret = 0;
        ...
        for (iter = entries.begin(); iter != entries.end(); ++iter) {
            ...
            ldpp_dout(this, 20) << "RGWGC::process iterating over entry tag='" << info.tag << "', time=" << info.time
                                << ", chain.objs.size()=" << info.chain.objs.size() << dendl;
