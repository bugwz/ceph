...
void check_bad_user_bucket_mapping(rgw::sal::Driver* driver, rgw::sal::User& user, bool fix, optional_yield y,
                                   const DoutPrefixProvider* dpp)
{
    ...
    do {
        ...
        if (ret < 0) {
            ldout(driver->ctx(), 0) << "failed to read user buckets: " << cpp_strerror(-ret) << dendl;
            return;
        }
...
int rgw_remove_object(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, rgw::sal::Bucket* bucket,
                      rgw_obj_key& key)
{
...
int RGWBucket::init(rgw::sal::Driver* _driver, RGWBucketAdminOpState& op_state, optional_yield y,
                    const DoutPrefixProvider* dpp, std::string* err_msg)
{
    ...
    if (bucket_name.empty() && op_state.get_user_id().empty()) return -EINVAL;
                                         ...
                                         const DoutPrefixProvider* dpp, std::string* err_msg)
{
    ...
    do {
        ...
        if (r < 0) {
            set_err_msg(err_msg, "failed to list objects in bucket=" + bucket->get_name() + " err=" + cpp_strerror(-r));
 ...
 */
static int check_index_olh(rgw::sal::RadosStore* const rados_store, rgw::sal::Bucket* const bucket,
 ...
 */
int RGWBucket::check_index_olh(rgw::sal::RadosStore* const rados_store, const DoutPrefixProvider* dpp,
 ...
 * instance entry.
 */
static int check_index_unlinked(rgw::sal::RadosStore* const rados_store, rgw::sal::Bucket* const bucket,
 ...
 */
int RGWBucket::check_index_unlinked(rgw::sal::RadosStore* const rados_store, const DoutPrefixProvider* dpp,
                                    RGWBucketAdminOpState& op_state, RGWFormatterFlusher& flusher)
{
    const RGWBucketInfo& bucket_info = get_bucket_info();
    if ((bucket_info.versioning_status() & BUCKET_VERSIONED) == 0) {
...
int rgw_object_get_attr(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, rgw::sal::Object* obj,
...
int RGWBucketAdminOp::dump_s3_policy(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state, ostream& os,
                                     const DoutPrefixProvider* dpp)
{
...
int RGWBucketAdminOp::unlink(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state, const DoutPrefixProvider* dpp)
{
    ...
    return static_cast<rgw::sal::RadosStore*>(driver)->ctl()->bucket->unlink_bucket(
        op_state.get_user_id(), op_state.get_bucket()->get_info().bucket, null_yield, dpp, true);
}

int RGWBucketAdminOp::link(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state, const DoutPrefixProvider* dpp,
                           string* err)
{
    ...
    if (!bucket_id.empty() && bucket_id != loc_bucket->get_bucket_id()) {
        set_err_msg(err, "specified bucket id does not match " + loc_bucket->get_bucket_id());
        return -EINVAL;
    }
    ...
    int r = static_cast<rgw::sal::RadosStore*>(driver)->ctl()->bucket->unlink_bucket(
        owner.get_id(), old_bucket->get_info().bucket, null_yield, dpp, false);
    if (r < 0) {
    ...
    r = static_cast<rgw::sal::RadosStore*>(driver)->ctl()->bucket->link_bucket(op_state.get_user_id(),
                                                                               loc_bucket->get_info().bucket,
                                                                               loc_bucket->get_info().creation_time,
                                                                               null_yield,
                                                                               dpp,
                                                                               true,
                                                                               &ep_data);
    ...
    if (*loc_bucket != *old_bucket) {
        ...
        r = static_cast<rgw::sal::RadosStore*>(driver)->ctl()->bucket->remove_bucket_entrypoint_info(
            ...
            null_yield,
            dpp,
            RGWBucketCtl::Bucket::RemoveParams().set_objv_tracker(&ep_data.ep_objv));
        if (r < 0) {
        ...
        r = static_cast<rgw::sal::RadosStore*>(driver)->ctl()->bucket->remove_bucket_instance_info(
            ...
            null_yield,
            dpp,
            RGWBucketCtl::BucketInstance::RemoveParams().set_objv_tracker(&ep_data.ep_objv));
        if (r < 0) {
            set_err_msg(err, "failed to unlink old bucket " + old_bucket->get_tenant() + "/" + old_bucket->get_name());
            return r;
        }
...
int RGWBucketAdminOp::chown(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state, const string& marker,
...
int RGWBucketAdminOp::check_index_unlinked(rgw::sal::RadosStore* store, RGWBucketAdminOpState& op_state,
                                           RGWFormatterFlusher& flusher, const DoutPrefixProvider* dpp)
{
    flusher.start(0);
    RGWBucket bucket;
...
int RGWBucketAdminOp::remove_bucket(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state, optional_yield y,
                                    const DoutPrefixProvider* dpp, bool bypass_gc, bool keep_index_consistent)
{
    ...
    int ret = driver->get_bucket(dpp, user.get(), user->get_tenant(), op_state.get_bucket_name(), &bucket, y);
    if (ret < 0) return ret;
...
int RGWBucketAdminOp::remove_object(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state,
                                    const DoutPrefixProvider* dpp)
{
    RGWBucket bucket;
...
int RGWBucketAdminOp::sync_bucket(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state,
...
static int bucket_stats(rgw::sal::Driver* driver, const std::string& tenant_name, const std::string& bucket_name,
                        Formatter* formatter, const DoutPrefixProvider* dpp)
{
    ...
    if (is_layout_indexless(index)) {
        cerr << "error, indexless buckets do not maintain stats; bucket=" << bucket->get_name() << std::endl;
        return -EINVAL;
    }
    ...
    formatter->dump_int("num_shards", bucket->get_info().layout.current_index.layout.normal.num_shards);
...
int RGWBucketAdminOp::limit_check(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state,
...
{
    int ret = 0;
    const size_t max_entries = driver->ctx()->_conf->rgw_list_buckets_max_chunk;

    const size_t safe_max_objs_per_shard = driver->ctx()->_conf->rgw_safe_max_objects_per_shard;

    uint16_t shard_warn_pct = driver->ctx()->_conf->rgw_shard_warning_threshold;
...
int RGWBucketAdminOp::info(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state, RGWFormatterFlusher& flusher,
                           optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    if (op_state.is_user_op()) {
        ...
        while (ret == 0 && truncated) {
            ...
            ret = driver->meta_list_keys_next(dpp, handle, max_keys, buckets, &truncated);
...
int RGWBucketAdminOp::set_quota(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state,
                                const DoutPrefixProvider* dpp)
{
    RGWBucket bucket;
...
void get_stale_instances(rgw::sal::Driver* driver, const std::string& bucket_name, const vector<std::string>& lst,
...
static int process_stale_instances(
    rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state, RGWFormatterFlusher& flusher,
    const DoutPrefixProvider* dpp,
    std::function<void(const bucket_instance_ls&, Formatter*, rgw::sal::Driver*)> process_f)
{
    std::string marker;
    void* handle;
    Formatter* formatter = flusher.get_formatter();
...
int RGWBucketAdminOp::list_stale_instances(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state,
                                           RGWFormatterFlusher& flusher, const DoutPrefixProvider* dpp)
{
    auto process_f = [](const bucket_instance_ls& lst, Formatter* formatter, rgw::sal::Driver*) {
...
int RGWBucketAdminOp::clear_stale_instances(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state,
                                            RGWFormatterFlusher& flusher, const DoutPrefixProvider* dpp)
{
    auto process_f = [dpp](const bucket_instance_ls& lst, Formatter* formatter, rgw::sal::Driver* driver) {
...
static int fix_single_bucket_lc(rgw::sal::Driver* driver, const std::string& tenant_name,
...
static void process_single_lc_entry(rgw::sal::Driver* driver, Formatter* formatter, const std::string& tenant_name,
                                    const std::string& bucket_name, const DoutPrefixProvider* dpp)
{
...
int RGWBucketAdminOp::fix_lc_shards(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state,
                                    RGWFormatterFlusher& flusher, const DoutPrefixProvider* dpp)
{
    std::string marker;
    void* handle;
...
static bool has_object_expired(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, rgw::sal::Bucket* bucket,
...
static int fix_bucket_obj_expiry(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, rgw::sal::Bucket* bucket,
...
int RGWBucketAdminOp::fix_obj_expiry(rgw::sal::Driver* driver, RGWBucketAdminOpState& op_state,
...
public:
                  ...
                  optional_yield y, const DoutPrefixProvider* dpp) override
    {
        ...
        ret = ctl.bucket->store_bucket_instance_info(new_be.bucket,
                                                     new_bi,
                                                     y,
                                                     dpp,
...
public:
    RGWMetadataHandlerPut_BucketInstance(CephContext* _cct, RGWBucketInstanceMetadataHandler* _handler,
        ...
        : RGWMetadataHandlerPut_SObj(_handler, _op, entry, _obj, objv_tracker, y, type, from_remote_zone)
        , cct(_cct)
        , bihandler(_handler)
    {
    ...
    if (from_remote_zone) {
        ...
        if (!exists) {
            ...
            init_default_bucket_layout(
                cct, bci.info.layout, bihandler->svc.zone->get_zone(), std::nullopt, std::nullopt);
        }
        else {
...
int RGWMetadataHandlerPut_BucketInstance::put_post(const DoutPrefixProvider* dpp)
{
    ...
    /* update lifecyle policy */
    {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << __func__ << " failed to get_bucket(...) for " << bci.info.bucket.name << dendl;
            return ret;
        }
        ...
        if (lc_it != bci.attrs.end()) {
            ...
            if (ret < 0) {
                ldpp_dout(dpp, 0) << __func__ << " failed to set lc config for " << bci.info.bucket.name << dendl;
                return ret;
            }
        }
        else {
            ...
            if (ret < 0) {
                ldpp_dout(dpp, 0) << __func__ << " failed to remove lc config for " << bci.info.bucket.name << dendl;
                return ret;
            }
        }
    } /* update lc */
...
public:
    ...
    // N.B. replication of lifecycle policy relies on logic in RGWBucketInstanceMetadataHandler::do_put(...), override
    // with caution
...
RGWBucketCtl::RGWBucketCtl(RGWSI_Zone* zone_svc, RGWSI_Bucket* bucket_svc, RGWSI_Bucket_Sync* bucket_sync_svc,
                           RGWSI_BucketIndex* bi_svc, RGWSI_User* user_svc)
    : cct(zone_svc->ctx())
{
    svc.zone = zone_svc;
    svc.bucket = bucket_svc;
                                 ...
                                 optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    if (update_entrypoint) {
        ...
        if (pinfo) {
            ...
            if (ret < 0 && ret != -ENOENT) {
                ldpp_dout(dpp, 0) << "ERROR: read_bucket_entrypoint_info() returned: " << cpp_strerror(-ret) << dendl;
            }
            pattrs = &attrs;
        }
    }
