...
RGWSyncTraceNode::RGWSyncTraceNode(CephContext* _cct, uint64_t _handle, const RGWSyncTraceNodeRef& _parent,
                                   const string& _type, const string& _id)
    : cct(_cct)
    , parent(_parent)
    , type(_type)
    , id(_id)
...
void RGWSyncTraceNode::log(int level, const string& s)
{
    ...
    if (cct->_conf->subsys.should_gather(ceph_subsys_rgw_sync, level)) {
        lsubdout(cct, rgw_sync, ceph::dout::need_dynamic(level)) << "RGW-SYNC:" << to_str() << dendl;
    }
    else {
        lsubdout(cct, rgw, ceph::dout::need_dynamic(level)) << "RGW-SYNC:" << to_str() << dendl;
    }
}
...
class RGWSyncTraceServiceMapThread : public RGWRadosThread
{
    ...
    uint64_t interval_msec() override { return cct->_conf->rgw_sync_trace_servicemap_update_interval * 1000; }

public:
    RGWSyncTraceServiceMapThread(RGWRados* _store, RGWSyncTraceManager* _manager)
        : RGWRadosThread(_store, "sync-trace")
        , store(_store)
        , manager(_manager)
    {}
...
RGWSyncTraceNodeRef RGWSyncTraceManager::add_node(const RGWSyncTraceNodeRef& parent, const std::string& type,
                                                  const std::string& id)
{
...
int RGWSyncTraceManager::call(std::string_view command, const cmdmap_t& cmdmap, const bufferlist&, Formatter* f,
