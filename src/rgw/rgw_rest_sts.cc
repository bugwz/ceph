...
boost::optional<RGWOIDCProvider> WebTokenEngine::get_provider(const DoutPrefixProvider* dpp, const string& role_arn,
...
bool WebTokenEngine::is_client_id_valid(vector<string>& client_ids, const string& client_id) const
{
...
bool WebTokenEngine::is_cert_valid(const vector<string>& thumbprints, const string& cert) const
{
    ...
    string pw = "";
    std::unique_ptr<X509, decltype(&X509_free)> x_509cert(
...
template<typename T> void WebTokenEngine::recurse_and_insert(const string& key, const jwt::claim& c, T& t) const
{
    string s_val;
    jwt::claim::type c_type = c.get_type();
    switch (c_type) {
    case jwt::claim::type::null: break;
    ...
    try {
        ...
        if (!client_ids.empty()) {
            ...
            if (!found && !is_client_id_valid(client_ids, client_id) && !is_client_id_valid(client_ids, azp)) {
                ldpp_dout(dpp, 0) << "Client id in token doesn't match with that registered with oidc provider"
                                  << dendl;
                throw -EACCES;
            }
        }
...
std::string WebTokenEngine::get_cert_url(const string& iss, const DoutPrefixProvider* dpp, optional_yield y) const
{
    string cert_url;
...
void WebTokenEngine::validate_signature(const DoutPrefixProvider* dpp, const jwt::decoded_jwt& decoded,
                                        const string& algorithm, const string& iss, const vector<string>& thumbprints,
                                        optional_yield y) const
{
...
WebTokenEngine::result_t WebTokenEngine::authenticate(const DoutPrefixProvider* dpp, const std::string& token,
                                                      const req_state* const s, optional_yield y) const
{
    ...
    try {
        ...
        if (t) {
            ...
            auto apl =
                apl_factory->create_apl_web_identity(cct, s, role_session, role_tenant, *t, role_tags, princ_tags);
...
int RGWSTSGetSessionToken::verify_permission(optional_yield y)
{
    ...
    if (!verify_user_permission(
            this, s, rgw::ARN(partition, service, "", s->user->get_tenant(), ""), rgw::IAM::stsGetSessionToken)) {
...
void RGWSTSAssumeRoleWithWebIdentity::execute(optional_yield y)
{
    ...
    STS::AssumeRoleWithWebIdentityRequest req(
        s->cct, duration, providerId, policy, roleArn, roleSessionName, iss, sub, aud, s->principal_tags);
...
void RGWSTSAssumeRole::execute(optional_yield y)
{
    ...
    STS::AssumeRoleRequest req(s->cct, duration, externalId, policy, roleArn, roleSessionName, serialNumber, tokenCode);
...
int RGW_Auth_STS::authorize(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                            const rgw::auth::StrategyRegistry& auth_registry, struct req_state* s, optional_yield y)
{
...
int RGWHandler_REST_STS::init(rgw::sal::RGWRadosStore* store, struct req_state* s, rgw::io::BasicClient* cio)
{
    s->dialect = "sts";
...
int RGWHandler_REST_STS::init_from_header(struct req_state* s, int default_formatter, bool configurable_format)
{
    ...
    if (int ret = allocate_formatter(s, default_formatter, configurable_format); ret < 0) return ret;
...
RGWHandler_REST* RGWRESTMgr_STS::get_handler(rgw::sal::RGWRadosStore* store, struct req_state* const s,
