...
std::unique_ptr<rgw::sal::RGWOIDCProvider> WebTokenEngine::get_provider(const DoutPrefixProvider* dpp,
                                                                        const string& role_arn, const string& iss) const
{
    ...
    if (!policy.empty()) {
        ...
        try {
            const rgw::IAM::Policy p(
                s->cct, s->user->get_tenant(), bl, s->cct->_conf.get_val<bool>("rgw_policy_reject_invalid_principals"));
        }
        catch (rgw::IAM::PolicyParseException& e) {
...
int RGW_Auth_STS::authorize(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver,
...
int RGWHandler_REST_STS::init(rgw::sal::Driver* driver, req_state* s, rgw::io::BasicClient* cio)
{
    s->dialect = "sts";
...
RGWHandler_REST* RGWRESTMgr_STS::get_handler(rgw::sal::Driver* driver, req_state* const s,
