...
class RateLimiterEntry
{
    /*
      ...
      for example: a user have a limit of single op per minute, the user will consume its single token and then will
      send another request, 1s after it. in that case, without this method, the user will get 0 tokens although it
      should get 0.016 tokens. using this method it will add 16 tokens to the user, and the user will have 16 tokens,
      each time rgw will do comparison rgw will divide by fixed_point_rgw_ratelimit, so the user will be blocked anyway
      until it has enough tokens.
    */
    ...
    int64_t read_ops() const { return read.ops / fixed_point_rgw_ratelimit; }
    int64_t write_ops() const { return write.ops / fixed_point_rgw_ratelimit; }
    int64_t read_bytes() const { return read.bytes / fixed_point_rgw_ratelimit; }
    int64_t write_bytes() const { return write.bytes / fixed_point_rgw_ratelimit; }
    bool should_rate_limit_read(int64_t ops_limit, int64_t bw_limit)
    {
        ...
        if (((read_ops() - 1 < 0) && (ops_limit > 0)) || (read_bytes() < 0 && bw_limit > 0)) {
            return true;
        }
    ...
    {
        // check if tenants did not reach their bw or ops limits and that the limits are not 0 (which is unlimited)
        if (((write_ops() - 1 < 0) && (ops_limit > 0)) || (write_bytes() < 0 && bw_limit > 0)) {
            return true;
        }
    ...
    /* The purpose of this function is to minimum time before overriding the stored timestamp
       This function is necessary to force the increase tokens add at least 1 token when it updates the last stored
       timestamp. That way the user/bucket will not lose tokens because of rounding
    */
    ...
    void increase_tokens(ceph::timespan curr_timestamp, const RGWRateLimitInfo* info)
    {
        ...
        if (first_run) {
            ...
            const int64_t time_in_ms =
                std::chrono::duration_cast<std::chrono::milliseconds>(curr_timestamp - ts).count() / 60.0 /
                std::milli::den * fixed_point;   // / 60 to make it work with 1 min token bucket
            ts = curr_timestamp;
...
class RateLimiter
{
    ...
    auto& find_or_create(const std::string& key)
    {
        ...
        if (ret == ratelimit_entries.end()) {
            ...
            ret =
                ratelimit_entries.emplace(std::piecewise_construct, std::forward_as_tuple(key), std::forward_as_tuple())
                    .first;
        }
        return ret->second;
    }
...
public:
    ...
    bool should_rate_limit(const char* method, const std::string& key, ceph::coarse_real_time curr_timestamp,
    ...
    void clear() { ratelimit_entries.clear(); }
};
...
public:
    ...
    std::shared_ptr<RateLimiter> get_active() { return ratelimit[current_active]; }
    void start()
    {
