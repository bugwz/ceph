...
static inline std::string get_s3_expiration_header(req_state* s, const ceph::real_time& mtime)
{
    return rgw::lc::s3_expiration_header(s, s->object->get_key(), s->tagset, mtime, s->bucket_attrs);
}

static inline bool get_s3_multipart_abort_header(req_state* s, const ceph::real_time& mtime, ceph::real_time& date,
                                                 std::string& rule_id)
{
    return rgw::lc::s3_multipart_abort_header(s, s->object->get_key(), mtime, s->bucket_attrs, date, rule_id);
}
...
int get_encryption_defaults(req_state* s)
{
    ...
    if (aiter != buck_attrs.end()) {
        ldpp_dout(s, 5) << "Found RGW_ATTR_BUCKET_ENCRYPTION_POLICY on " << s->bucket_name << dendl;
    ...
    if (bucket_configuration_found) {
        ldpp_dout(s, 5) << "RGW_ATTR_BUCKET_ENCRYPTION ALGO: " << sse_algorithm << dendl;
    }

    auto iter = s->info.crypt_attribute_map.find(encrypt_attr);
    if (iter != s->info.crypt_attribute_map.end()) {
        ldpp_dout(s, 20) << "get_encryption_defaults: found encrypt_attr " << encrypt_attr << " = " << iter->second
    ...
    if (iter != s->info.crypt_attribute_map.end()) {
        ldpp_dout(s, 20) << "get_encryption_defaults: found kms_attr " << kms_attr << " = " << iter->second
        ...
        ldpp_dout(s, 20) << "get_encryption_defaults: no kms_attr, but kms_master_key_id = " << kms_master_key_id
    ...
    if (iter != s->info.crypt_attribute_map.end()) {
        ldpp_dout(s, 20) << "get_encryption_defaults: found bucket_key_attr " << bucket_key_attr << " = "
                         << iter->second << ", setting kms_attr_seen" << dendl;
        if (!rest_only) {
        ...
        ldpp_dout(s, 20)
            << "get_encryption_defaults: no bucket_key_attr, but bucket_key_enabled,  setting kms_attr_seen" << dendl;
        kms_attr_seen = true;
    ...
    if (iter != s->info.crypt_attribute_map.end()) {
        ldpp_dout(s, 20) << "get_encryption_defaults: found context_attr " << context_attr << " = " << iter->second
                         << ", setting kms_attr_seen" << dendl;
        if (!rest_only) {
    ...
    ldpp_dout(s, 20) << "get_encryption_defaults:  kms_attr_seen is " << kms_attr_seen << " and sse_algorithm is "
                     << sse_algorithm << dendl;
...
int RGWGetObj_ObjStore_S3Website::send_response_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
    ...
    if (iter != attrs.end()) {
        ...
        ldpp_dout(this, 20) << __CEPH_ASSERT_FUNCTION
                            << " redirecting per x-amz-website-redirect-location=" << s->redirect << dendl;
...
int RGWGetObj_ObjStore_S3::send_response_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
    ...
    if (custom_http_ret) {
        ...
        set_req_state_err(s, (partial_content && !op_ret) ? STATUS_PARTIAL_CONTENT : op_ret);
        dump_errno(s);
    }

    if (op_ret) goto done;

    if (range_str) dump_range(s, start, end, s->obj_size);

    if (s->system_request && s->info.args.exists(RGW_SYS_PARAM_PREFIX "prepend-metadata")) {
    ...
    for (auto& it : crypt_http_responses) dump_header(s, it.first, it.second);
    ...
    if (op_ret == -ERR_NOT_MODIFIED) {
        ...
        if (!content_type) content_type = "binary/octet-stream";
...
int RGWGetObj_ObjStore_S3::get_decrypt_filter(std::unique_ptr<RGWGetObj_Filter>* filter, RGWGetObj_Filter* cb,
                                              bufferlist* manifest_bl)
{
    ...
    if (auto i = attrs.find(RGW_ATTR_CRYPT_PARTS); i != attrs.end()) {
        ...
        res = RGWGetObj_BlockDecrypt::read_manifest_parts(this, *manifest_bl, parts_len);
        if (res < 0) {
            return res;
        }
    }

    *filter = std::make_unique<RGWGetObj_BlockDecrypt>(s, s->cct, cb, std::move(block_crypt), std::move(parts_len));
    return 0;
}
...
{
    if (op_ret) set_req_state_err(s, op_ret);
    dump_errno(s);
...
void RGWDeleteBucketTags_ObjStore_S3::send_response()
{
    ...
    if (op_ret == 0) op_ret = STATUS_NO_CONTENT;
    if (op_ret) set_req_state_err(s, op_ret);
    dump_errno(s);
    end_header(s, this, to_mime_type(s->format));
    dump_start(s);
...
struct ReplicationConfiguration
{
    ...
    struct Rule
    {
        struct DeleteMarkerReplication
        {
            ...
            bool is_valid(CephContext* cct) const
            {
                ...
                if (!result) {
                    ldout(cct, 5) << "NOTICE: bad status provided in DeleteMarkerReplication element (status=" << status
                                  << ")" << dendl;
                }
                return result;
            }
        ...
        { /* rgw extension */
            ...
            void decode_xml(XMLObj* obj) { RGWXMLDecoder::decode_xml("Zone", zone_names, obj); }
        ...
        struct Destination
        {
            struct AccessControlTranslation
            {
                ...
                void decode_xml(XMLObj* obj) { RGWXMLDecoder::decode_xml("Owner", owner, obj); }
        ...
        set<rgw_zone_id> get_zone_ids_from_names(rgw::sal::Driver* driver, const vector<string>& zone_names) const
        {
            set<rgw_zone_id> ids;
        ...
        vector<string> get_zone_names_from_ids(rgw::sal::Driver* driver, const set<rgw_zone_id>& zone_ids) const
        {
            vector<string> names;
        ...
        int to_sync_policy_pipe(req_state* s, rgw::sal::Driver* driver, rgw_sync_bucket_pipes* pipe,
                                bool* enabled) const
        {
            if (!is_valid(s->cct)) {
                return -EINVAL;
    ...
    int to_sync_policy_groups(req_state* s, rgw::sal::Driver* driver, vector<rgw_sync_policy_group>* result) const
    {
        result->resize(2);
        ...
        for (auto& rule : rules) {
            ...
            if (r < 0) {
                ldpp_dout(s, 5) << "NOTICE: failed to convert replication configuration into sync policy pipe (rule.id="
    ...
    void from_sync_policy_group(rgw::sal::Driver* driver, const rgw_sync_policy_group& group)
    {
                                       ...
                                       map<string, bool>* categories)
{
    ...
    for (uiter = entry.usage_map.begin(); uiter != entry.usage_map.end(); ++uiter) {
        if (categories && !categories->empty() && !categories->count(uiter->first)) continue;
...
{
    if (op_ret < 0) set_req_state_err(s, op_ret);
    dump_errno(s);
...
void RGWListBucket_ObjStore_S3v2::send_versioned_response()
{
    ...
    if (op_ret >= 0) {
        ...
        vector<rgw_bucket_dir_entry>::iterator iter;
        for (iter = objs.begin(); iter != objs.end(); ++iter) {
            const char* section_name = (iter->is_delete_marker() ? "DeleteContinuationToken" : "Version");
            s->formatter->open_object_section(section_name);
            if (objs_container) {
...
void RGWGetBucketLocation_ObjStore_S3::send_response()
{
    ...
    s->formatter->dump_format_ns("LocationConstraint", XMLNS_AWS_S3, "%s", api_name.c_str());
    rgw_flush_formatter_and_reset(s, s->formatter);
}
...
int RGWSetBucketVersioning_ObjStore_S3::get_params(optional_yield y)
{
    int r = 0;
    bufferlist data;
    std::tie(r, data) = read_all_input(s, s->cct->_conf->rgw_max_put_param_size, false);
    if (r < 0) {
        return r;
    }
    ...
    if (status_conf.mfa_status != ver_config_status::MFA_UNKNOWN) {
        ...
        case ver_config_status::MFA_DISABLED: mfa_status = false; break;
        case ver_config_status::MFA_ENABLED: mfa_status = true; break;
        default:
            ldpp_dout(this, 0) << "ERROR: RGWSetBucketVersioning_ObjStore_S3::get_params(optional_yield y): unexpected "
                                  "switch case mfa_status="
...
int RGWSetBucketWebsite_ObjStore_S3::get_params(optional_yield y)
{
    ...
    else if (!website_conf.is_redirect_all && website_conf.is_set_index_doc && website_conf.index_doc_suffix.empty()) {
...
#define WEBSITE_ROUTING_RULES_MAX_NUM 50
    ...
    if (routing_rules_num > max_num) {
        ldpp_dout(this, 4) << "An website routing config can have up to " << max_num
                           << " rules, request website routing rules num: " << routing_rules_num << dendl;
...
static int create_s3_policy(req_state* s, rgw::sal::Driver* driver, RGWAccessControlPolicy_S3& s3policy,
                            ACLOwner& owner)
{
    if (s->has_acl_header) {
        if (!s->canned_acl.empty()) return -ERR_INVALID_REQUEST;
...
int RGWCreateBucket_ObjStore_S3::get_params(optional_yield y)
{
    ...
    if ((op_ret < 0) && (op_ret != -ERR_LENGTH_REQUIRED)) return op_ret;
...
{
    if (op_ret == -ERR_BUCKET_EXISTS) op_ret = 0;
    if (op_ret) set_req_state_err(s, op_ret);
    dump_errno(s);
    end_header(s);
...
int RGWPutObj_ObjStore_S3::get_params(optional_yield y)
{
    ...
    if (!s->bucket->get_info().obj_lock_enabled() && (obj_retention || obj_legal_hold)) {
        ldpp_dout(this, 0) << "ERROR: object retention or legal hold can't be set if bucket object lock not configured"
                           << dendl;
...
void RGWPutObj_ObjStore_S3::send_response()
{
    if (op_ret) {
        ...
        if (copy_source.empty()) {
            ...
            s->formatter->open_object_section_in_ns("CopyPartResult", "http://s3.amazonaws.com/doc/2006-03-01/");
...
int RGWPutObj_ObjStore_S3::get_decrypt_filter(std::unique_ptr<RGWGetObj_Filter>* filter, RGWGetObj_Filter* cb,
...
int RGWPutObj_ObjStore_S3::get_encrypt_filter(std::unique_ptr<rgw::sal::DataProcessor>* filter,
                                              rgw::sal::DataProcessor* cb)
{
    int res = 0;
    if (!multipart_upload_id.empty()) {
        std::unique_ptr<rgw::sal::MultipartUpload> upload =
            s->bucket->get_multipart_upload(s->object->get_name(), multipart_upload_id);
...
int RGWPostObj_ObjStore_S3::get_params(optional_yield y)
{
    ...
    for (; piter != parts.end(); ++piter) {
        ...
        if (strncasecmp(n.c_str(), RGW_AMZ_META_PREFIX, sizeof(RGW_AMZ_META_PREFIX) - 1) != 0) break;
...
int RGWPostObj_ObjStore_S3::get_policy(optional_yield y)
{
    if (part_bl(parts, "policy", &s->auth.s3_postobj_creds.encoded_policy)) {
        ...
        if (aws4_auth) {
            ...
            if (!part_str(parts, "x-amz-credential", &s->auth.s3_postobj_creds.x_amz_credential)) {
            ...
            if (!part_str(parts, "x-amz-signature", &s->auth.s3_postobj_creds.signature)) {
            ...
            // check that the signature matches the encoded policy
            if (!part_str(parts, "AWSAccessKeyId", &s->auth.s3_postobj_creds.access_key)) {
...
void RGWPostObj_ObjStore_S3::send_response()
{
    if (op_ret == 0 && parts.count("success_action_redirect")) {
        ...
        switch (status_int) {
        case 200: break;
        case 201: op_ret = STATUS_CREATED; break;
        default: op_ret = STATUS_NO_CONTENT; break;
        }
    }
    else if (!op_ret) {
        op_ret = STATUS_NO_CONTENT;
    ...
    if (op_ret == STATUS_CREATED) {
        ...
        if (!s->bucket_tenant.empty()) {
            s->formatter->dump_format("Location",
                                      "%s/%s:%s/%s",
                                      base_uri.c_str(),
    ...
    end_header(s, this);
    if (op_ret != STATUS_CREATED) return;

    rgw_flush_formatter_and_reset(s, s->formatter);
}

int RGWPostObj_ObjStore_S3::get_encrypt_filter(std::unique_ptr<rgw::sal::DataProcessor>* filter,
                                               rgw::sal::DataProcessor* cb)
{
    std::unique_ptr<BlockCrypt> block_crypt;
    int res = rgw_s3_prepare_encrypt(s, attrs, &block_crypt, crypt_http_responses);
    if (res == 0 && block_crypt != nullptr) {
        filter->reset(new RGWPutObj_BlockEncrypt(s, s->cct, cb, std::move(block_crypt)));
    }
    return res;
...
int RGWCopyObj_ObjStore_S3::get_params(optional_yield y)
{
    ...
    if (source_zone.empty() && (s->bucket->get_tenant() == s->src_tenant_name) &&
        (s->bucket->get_name() == s->src_bucket_name) && (s->object->get_name() == s->src_object->get_name()) &&
        s->src_object->get_instance().empty() && (attrs_mod != rgw::sal::ATTRSMOD_REPLACE)) {
...
{
    if (!sent_header) send_partial_response(0);
...
int RGWPutACLs_ObjStore_S3::get_policy_from_state(rgw::sal::Driver* driver, req_state* s, stringstream& ss)
{
    RGWAccessControlPolicy_S3 s3policy(s->cct);

    // bucket-* canned acls do not apply to bucket
    if (rgw::sal::Object::empty(s->object.get())) {
        if (s->canned_acl.find("bucket") != string::npos) s->canned_acl.clear();
    }
...
#define CORS_RULES_MAX_NUM 100
    ...
    if (cors_rules_num > max_num) {
        ldpp_dout(this, 4) << "An cors config can have up to " << max_num
                           << " rules, request cors rules num: " << cors_rules_num << dendl;
        op_ret = -ERR_INVALID_CORS_RULES_ERROR;
        s->err.message =
            "The number of CORS rules should not exceed allowed limit of " + std::to_string(max_num) + " rules.";
        return -ERR_INVALID_REQUEST;
    }
...
{
    int r = op_ret;
    if (!r || r == -ENOENT) r = STATUS_NO_CONTENT;
...
void RGWOptionsCORS_ObjStore_S3::send_response()
{
     ...
     */
    if (op_ret == -ENOENT) op_ret = -EACCES;
    if (op_ret < 0) {
        set_req_state_err(s, op_ret);
        dump_errno(s);
        end_header(s, NULL);
...
void RGWGetBucketEncryption_ObjStore_S3::send_response()
{
    ...
    if (!op_ret) {
        encode_xml("ServerSideEncryptionConfiguration", XMLNS_AWS_S3, bucket_encryption_conf, s->formatter);
        rgw_flush_formatter_and_reset(s, s->formatter);
    }
}
...
void RGWInitMultipart_ObjStore_S3::send_response()
{
    ...
    if (op_ret == 0) {
        ...
        if (!s->bucket_tenant.empty()) s->formatter->dump_string("Tenant", s->bucket_tenant);
        s->formatter->dump_string("Bucket", s->bucket_name);
        s->formatter->dump_string("Key", s->object->get_name());
    ...
    if (op_ret == 0) {
        ...
        if (!s->bucket_tenant.empty()) {
            ...
            s->formatter->dump_format(
                "Location", "%s/%s/%s", base_uri.c_str(), s->bucket_name.c_str(), s->object->get_name().c_str());
        }
        s->formatter->dump_string("Bucket", s->bucket_name);
        s->formatter->dump_string("Key", s->object->get_name());
        s->formatter->dump_string("ETag", etag);
...
void RGWListBucketMultiparts_ObjStore_S3::send_response()
{
    ...
    if (!s->bucket_tenant.empty()) s->formatter->dump_string("Tenant", s->bucket_tenant);
    s->formatter->dump_string("Bucket", s->bucket_name);
    if (!prefix.empty()) s->formatter->dump_string("Prefix", prefix);
    if (!marker_key.empty()) s->formatter->dump_string("KeyMarker", marker_key);
    if (!marker_upload_id.empty()) s->formatter->dump_string("UploadIdMarker", marker_upload_id);
    if (!next_marker_key.empty()) s->formatter->dump_string("NextKeyMarker", next_marker_key);
    if (!next_marker_upload_id.empty()) s->formatter->dump_string("NextUploadIdMarker", next_marker_upload_id);
    s->formatter->dump_int("MaxUploads", max_uploads);
    if (!delimiter.empty()) s->formatter->dump_string("Delimiter", delimiter);
...
void RGWDeleteMultiObj_ObjStore_S3::send_partial_response(const rgw_obj_key& key, bool delete_marker,
...
int RGWConfigBucketMetaSearch_ObjStore_S3::get_params(optional_yield y)
{
    ...
    for (auto& expression : expressions) {
        ...
        if (!boost::algorithm::starts_with(key, RGW_AMZ_META_PREFIX)) {
            s->err.message =
                string("invalid expression, key must start with '" RGW_AMZ_META_PREFIX "' : ") + expression;
            ldpp_dout(this, 5) << s->err.message << dendl;
            return -EINVAL;
        }
...
void RGWGetBucketMetaSearch_ObjStore_S3::send_response()
{
    ...
    for (auto& e : s->bucket->get_info().mdsearch_config) {
        ...
        const char* type;
        switch (e.second) {
        case ESEntityTypeMap::ES_ENTITY_INT: type = "int"; break;
        case ESEntityTypeMap::ES_ENTITY_DATE: type = "date"; break;
        default: type = "str";
        }
...
RGWOp* RGWHandler_REST_Bucket_S3::get_obj_op(bool get_data) const
{
    ...
    if (get_data) {
        ...
        switch (list_type) {
        case 1: return new RGWListBucket_ObjStore_S3;
        case 2: return new RGWListBucket_ObjStore_S3v2;
        default:
...
{
    if (s->info.args.sub_resource_exists("encryption")) return nullptr;

    if (s->info.args.sub_resource_exists("logging")) return new RGWGetBucketLogging_ObjStore_S3;

    if (s->info.args.sub_resource_exists("location")) return new RGWGetBucketLocation_ObjStore_S3;

    if (s->info.args.sub_resource_exists("versioning")) return new RGWGetBucketVersioning_ObjStore_S3;
...
{
    if (s->info.args.sub_resource_exists("logging") || s->info.args.sub_resource_exists("encryption")) return nullptr;
    if (s->info.args.sub_resource_exists("versioning")) return new RGWSetBucketVersioning_ObjStore_S3;
    if (s->info.args.sub_resource_exists("website")) {
        if (!s->cct->_conf->rgw_enable_static_website) {
            return NULL;
        }
    ...
    if (is_tagging_op()) {
        ...
        int ret = driver->get_sync_policy_handler(s, nullopt, nullopt, &sync_policy_handler, null_yield);
        if (ret < 0 || !sync_policy_handler || sync_policy_handler->is_legacy_config()) {
            return nullptr;
        }
...
{
    if (s->info.args.exists("uploadId")) return new RGWCompleteMultipart_ObjStore_S3;

    if (s->info.args.exists("uploads")) return new RGWInitMultipart_ObjStore_S3;

    if (is_select_op()) return rgw::s3select::create_s3select_op();
...
int RGWHandler_REST_S3::init_from_header(rgw::sal::Driver* driver, req_state* s, RGWFormat default_formatter,
...
int RGWHandler_REST_S3::postauth_init(optional_yield y)
{
    ...
    int ret = rgw_parse_url_bucket(t->url_bucket, s->user->get_tenant(), s->bucket_tenant, s->bucket_name);
    if (ret) {
        return ret;
    }
    ...
    if (!t->src_bucket.empty()) {
        ...
        ret = rgw_parse_url_bucket(t->src_bucket, auth_tenant, s->src_tenant_name, s->src_bucket_name);
        if (ret) {
            return ret;
        }
...
int RGWHandler_REST_S3::init(rgw::sal::Driver* driver, req_state* s, rgw::io::BasicClient* cio)
{
    int ret;
    ...
    if (copy_source && (!s->info.env->get("HTTP_X_AMZ_COPY_SOURCE_RANGE")) && (!s->info.args.exists("uploadId"))) {
        ...
        ret = RGWCopyObj::parse_copy_location(copy_source, s->init_state.src_bucket, key, s);
        if (!ret) {
...
static inline std::pair<AwsVersion, AwsRoute> discover_aws_flavour(const req_info& info)
{
    using rgw::auth::s3::AWS4_HMAC_SHA256_STR;
    ...
    if (http_auth && http_auth[0]) {
        ...
        if (!strncmp(http_auth, AWS4_HMAC_SHA256_STR, strlen(AWS4_HMAC_SHA256_STR))) {
            /* AWS v4 */
 ...
 */
int RGW_Auth_S3::authorize(const DoutPrefixProvider* dpp, rgw::sal::Driver* const driver,
                           const rgw::auth::StrategyRegistry& auth_registry, req_state* const s, optional_yield y)
{

    /* neither keystone and rados enabled; warn and exit! */
    if (!driver->ctx()->_conf->rgw_s3_auth_use_rados && !driver->ctx()->_conf->rgw_s3_auth_use_keystone &&
...
int RGWHandler_Auth_S3::init(rgw::sal::Driver* driver, req_state* state, rgw::io::BasicClient* cio)
{
...
public:
    Attribute(const std::string& type, const std::string& key_or_value) { set(type, key_or_value); }
            ...
            for (const auto& t : tokens) {
                ...
                if (pos != string::npos) {
                    ...
                    if (boost::starts_with(key, "Attributes.")) {
                        ...
                        s->info.args.append(t.substr(0, pos), url_decode(t.substr(pos + 1, t.size() - 1)));
                    }
                }
            }
...
RGWHandler_REST* RGWRESTMgr_S3::get_handler(rgw::sal::Driver* driver, req_state* const s,
                                            const rgw::auth::StrategyRegistry& auth_registry,
                                            const std::string& frontend_prefix)
{
    bool is_s3website = enable_s3website && (s->prot_flags & RGW_REST_WEBSITE);
    int ret = RGWHandler_REST_S3::init_from_header(driver, s, is_s3website ? RGWFormat::HTML : RGWFormat::XML, true);
    if (ret < 0) {
        return nullptr;
    }
...
int RGWHandler_REST_S3Website::init(rgw::sal::Driver* driver, req_state* s, rgw::io::BasicClient* cio)
{
...
int RGWHandler_REST_S3Website::retarget(RGWOp* op, RGWOp** new_op, optional_yield y)
{
    *new_op = op;
    ...
    bool should_redirect = s->bucket->get_info().website_conf.should_redirect(new_obj.name, 0, &rrule);

    if (should_redirect) {
        const string& hostname = s->info.env->get("HTTP_HOST", "");
        const string& protocol = (s->info.env->get("SERVER_PORT_SECURE") ? "https" : "http");
        int redirect_code = 0;
        rrule.apply_rule(protocol, hostname, key_name, &s->redirect, &redirect_code);
        // APply a custom HTTP response code
        if (redirect_code > 0) s->err.http_ret = redirect_code;   // Apply a custom HTTP response code
        ldpp_dout(s, 10) << "retarget redirect code=" << redirect_code << " proto+host:" << protocol << "://"
                         << hostname << " -> " << s->redirect << dendl;
...
int RGWHandler_REST_S3Website::serve_errordoc(const DoutPrefixProvider* dpp, int http_ret, const string& errordoc_key,
                                              optional_yield y)
{
    int ret = 0;
...
int RGWHandler_REST_S3Website::error_handler(int err_no, string* error_content, optional_yield y)
{
    int new_err_no = -1;
    ...
    ldpp_dout(s, 10) << "RGWHandler_REST_S3Website::error_handler err_no=" << err_no << " http_ret=" << http_error_code
                     << dendl;
    ...
    if (have_bucket) {
        should_redirect =
            s->bucket->get_info().website_conf.should_redirect(original_object_name, http_error_code, &rrule);
    }
    ...
    if (should_redirect) {
        ...
        const string& protocol = (s->info.env->get("SERVER_PORT_SECURE") ? "https" : "http");
        int redirect_code = 0;
        rrule.apply_rule(protocol, hostname, original_object_name, &s->redirect, &redirect_code);
        // Apply a custom HTTP response code
        if (redirect_code > 0) s->err.http_ret = redirect_code;   // Apply a custom HTTP response code
        ldpp_dout(s, 10) << "error handler redirect code=" << redirect_code << " proto+host:" << protocol << "://"
                         << hostname << " -> " << s->redirect << dendl;
        return -ERR_WEBSITE_REDIRECT;
    }
...
static rgw::auth::Completer::cmplptr_t null_completer_factory(const boost::optional<std::string>& secret_key)
{
    return nullptr;
}


AWSEngine::VersionAbstractor::auth_data_t AWSGeneralAbstractor::get_auth_data(const req_state* const s) const
{
    AwsVersion version;
    AwsRoute route;
...
boost::optional<std::string> AWSGeneralAbstractor::get_v4_canonical_headers(const req_info& info,
                                                                            const std::string_view& signedheaders,
                                                                            const bool using_qs) const
{
    return rgw::auth::s3::get_v4_canonical_headers(info, signedheaders, using_qs, false);
}

AWSSignerV4::prepare_result_t AWSSignerV4::prepare(const DoutPrefixProvider* dpp, const std::string& access_key_id,
                                                   const string& region, const string& service, const req_info& info,
                                                   const bufferlist* opt_content, bool s3_op)
{
    ...
    std::string canonical_headers = gen_v4_canonical_headers(info, extra_headers, &signed_hdrs);
    ...
    auto canonical_req_hash = rgw::auth::s3::get_v4_canon_req_hash(cct,
                                                                   info.method,
    ...
    auto string_to_sign = rgw::auth::s3::get_v4_string_to_sign(
        cct, AWS4_HMAC_SHA256_STR, date, credential_scope, std::move(canonical_req_hash), dpp);
...
AWSSignerV4::signature_headers_t gen_v4_signature(const DoutPrefixProvider* dpp, const std::string_view& secret_key,
                                                  const AWSSignerV4::prepare_result_t& sig_info)
{
    auto signature =
        rgw::auth::s3::get_v4_signature(sig_info.scope, dpp->get_cct(), secret_key, sig_info.string_to_sign, dpp);
...
AWSEngine::VersionAbstractor::auth_data_t AWSGeneralAbstractor::get_auth_data_v4(const req_state* const s,
                                                                                 const bool using_qs) const
{
    ...
    int ret = rgw::auth::s3::parse_v4_credentials(
        s->info, access_key_id, credential_scope, signed_hdrs, client_signature, date, session_token, using_qs, s);
    if (ret < 0) {
        throw ret;
    }

    /* craft canonical headers */
    boost::optional<std::string> canonical_headers = get_v4_canonical_headers(s->info, signed_hdrs, using_qs);
    ...
    /* Craft canonical request. */
    auto canonical_req_hash = rgw::auth::s3::get_v4_canon_req_hash(s->cct,
    ...
    auto string_to_sign = rgw::auth::s3::get_v4_string_to_sign(
        s->cct, AWS4_HMAC_SHA256_STR, date, credential_scope, std::move(canonical_req_hash), s);
    ...
    if (is_v4_payload_unsigned(exp_payload_hash) || is_v4_payload_empty(s) || is_non_s3_op) {
        ...
        if (!is_v4_payload_streamed(exp_payload_hash)) {
            ...
            case RGW_OP_GET_OBJ:   // s3select its post-method(payload contain the query) , the request is get-object
                break;
            default:
                ldpp_dout(s, 10) << "ERROR: AWS4 completion for operation: " << s->op_type << ", NOT IMPLEMENTED"
                                 << dendl;
                throw -ERR_NOT_IMPLEMENTED;
            }

            const auto cmpl_factory = std::bind(AWSv4ComplSingle::create, s, std::placeholders::_1);
            return {
                access_key_id, client_signature, session_token, std::move(string_to_sign), sig_factory, cmpl_factory};
        }
        else {
            ...
            switch (s->op_type) {
            case RGW_OP_PUT_OBJ: break;
            default:
                ldpp_dout(s, 10) << "ERROR: AWS4 completion for this operation NOT IMPLEMENTED (streaming mode)"
                                 << dendl;
                throw -ERR_NOT_IMPLEMENTED;
            }
            ...
            const auto cmpl_factory =
                std::bind(AWSv4ComplMulti::create, s, date, credential_scope, client_signature, std::placeholders::_1);
            return {
                access_key_id, client_signature, session_token, std::move(string_to_sign), sig_factory, cmpl_factory};
        }
    }
...
boost::optional<std::string> AWSGeneralBoto2Abstractor::get_v4_canonical_headers(const req_info& info,
                                                                                 const std::string_view& signedheaders,
                                                                                 const bool using_qs) const
{
    return rgw::auth::s3::get_v4_canonical_headers(info, signedheaders, using_qs, true);
}


AWSEngine::VersionAbstractor::auth_data_t AWSGeneralAbstractor::get_auth_data_v2(const req_state* const s) const
{
    std::string_view access_key_id;
    ...
    utime_t header_time;
    if (!rgw_create_s3_canonical_header(s, s->info, &header_time, string_to_sign, qsr)) {
...
AWSEngine::VersionAbstractor::auth_data_t AWSBrowserUploadAbstractor::get_auth_data_v2(const req_state* const s) const
{
...
AWSEngine::VersionAbstractor::auth_data_t AWSBrowserUploadAbstractor::get_auth_data_v4(const req_state* const s) const
{
...
AWSEngine::VersionAbstractor::auth_data_t AWSBrowserUploadAbstractor::get_auth_data(const req_state* const s) const
{
...
AWSEngine::result_t AWSEngine::authenticate(const DoutPrefixProvider* dpp, const req_state* const s,
...
void rgw::auth::s3::LDAPEngine::init(CephContext* const cct)
{
    ...
    if (!ldh) {
        ...
        if (!ldh) {
            ...
            ldh =
                new rgw::LDAPHelper(ldap_uri, ldap_binddn, ldap_bindpw, ldap_searchdn, ldap_searchfilter, ldap_dnattr);
...
rgw::auth::RemoteApplier::acl_strategy_t rgw::auth::s3::LDAPEngine::get_acl_strategy() const
{
...
rgw::auth::RemoteApplier::AuthInfo rgw::auth::s3::LDAPEngine::get_creds_info(const rgw::RGWToken& token) const noexcept
{
    ...
    return rgw::auth::RemoteApplier::AuthInfo{rgw_user(token.id),
                                              token.id,
...
rgw::auth::Engine::result_t rgw::auth::s3::LDAPEngine::authenticate(
    const DoutPrefixProvider* dpp, const std::string_view& access_key_id, const std::string_view& signature,
    const std::string_view& session_token, const string_to_sign_t& string_to_sign, const signature_factory_t&,
    const completer_factory_t& completer_factory, const req_state* const s, optional_yield y) const
{
    ...
    auto apl = apl_factory->create_apl_remote(cct, s, get_acl_strategy(), get_creds_info(base64_token));
...
rgw::auth::Engine::result_t rgw::auth::s3::LocalEngine::authenticate(
    const DoutPrefixProvider* dpp, const std::string_view& _access_key_id, const std::string_view& signature,
    const std::string_view& session_token, const string_to_sign_t& string_to_sign,
    const signature_factory_t& signature_factory, const completer_factory_t& completer_factory,
    const req_state* const s, optional_yield y) const
{
    ...
    auto apl = apl_factory->create_apl_local(cct, s, user->get_info(), k.subuser, std::nullopt, access_key_id);
...
int rgw::auth::s3::STSEngine::get_session_token(const DoutPrefixProvider* dpp, const std::string_view& session_token,
...
rgw::auth::Engine::result_t rgw::auth::s3::STSEngine::authenticate(
    ...
    const req_state* const s, optional_yield y) const
{
    if (!s->info.args.exists("x-amz-security-token") && !s->info.env->exists("HTTP_X_AMZ_SECURITY_TOKEN") &&
    ...
    if (token.acct_type == TYPE_KEYSTONE || token.acct_type == TYPE_LDAP) {
        auto apl = remote_apl_factory->create_apl_remote(cct, s, get_acl_strategy(), get_creds_info(token));
        return result_t::grant(std::move(apl), completer_factory(token.secret_access_key));
    }
    else if (token.acct_type == TYPE_ROLE) {
        t_attrs.user_id =
            std::move(token.user);   // This is mostly needed to assign the owner of a bucket during its creation
        ...
        auto apl = local_apl_factory->create_apl_local(
            cct, s, user->get_info(), subuser, token.perm_mask, std::string(_access_key_id));
        return result_t::grant(std::move(apl), completer_factory(token.secret_access_key));
    }
}

bool rgw::auth::s3::S3AnonymousEngine::is_applicable(const req_state* s) const noexcept
{
    AwsVersion version;
    AwsRoute route;
    std::tie(version, route) = discover_aws_flavour(s->info);
