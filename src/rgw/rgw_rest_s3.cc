...
static inline std::string get_s3_expiration_header(struct req_state* s, const ceph::real_time& mtime)
{
    return rgw::lc::s3_expiration_header(s, s->object->get_key(), s->tagset, mtime, s->bucket_attrs);
}

static inline bool get_s3_multipart_abort_header(struct req_state* s, const ceph::real_time& mtime,
...
struct ReplicationConfiguration
{
    ...
    struct Rule
    {
        ...
        set<rgw_zone_id> get_zone_ids_from_names(rgw::sal::RGWRadosStore* store, const vector<string>& zone_names) const
        {
            set<rgw_zone_id> ids;
        ...
        vector<string> get_zone_names_from_ids(rgw::sal::RGWRadosStore* store, const set<rgw_zone_id>& zone_ids) const
        {
            vector<string> names;
        ...
        int to_sync_policy_pipe(req_state* s, rgw::sal::RGWRadosStore* store, rgw_sync_bucket_pipes* pipe,
                                bool* enabled) const
        {
            if (!is_valid(s->cct)) {
                return -EINVAL;
        ...
        void from_sync_policy_pipe(rgw::sal::RGWRadosStore* store, const rgw_sync_bucket_pipes& pipe, bool enabled)
        {
            id = pipe.id;
    ...
    int to_sync_policy_groups(req_state* s, rgw::sal::RGWRadosStore* store, vector<rgw_sync_policy_group>* result) const
    {
        result->resize(2);
    ...
    void from_sync_policy_group(rgw::sal::RGWRadosStore* store, const rgw_sync_policy_group& group)
    {
...
int RGWSetBucketVersioning_ObjStore_S3::get_params(optional_yield y)
{
    int r = 0;
    bufferlist data;
    std::tie(r, data) = rgw_rest_read_all_input(s, s->cct->_conf->rgw_max_put_param_size, false);
    if (r < 0) {
        return r;
    }
...
static int create_s3_policy(struct req_state* s, rgw::sal::RGWRadosStore* store, RGWAccessControlPolicy_S3& s3policy,
                            ACLOwner& owner)
{
    if (s->has_acl_header) {
...
{
    if (!s->length) return -ERR_LENGTH_REQUIRED;
...
static inline int get_obj_attrs(rgw::sal::RGWRadosStore* store, struct req_state* s, rgw_obj& obj,
                                              ...
                                              map<string, bufferlist>& attrs, bufferlist* manifest_bl)
{
    ...
    if (res == 0) {
        if (block_crypt != nullptr) {
            auto f =
                std::unique_ptr<RGWGetObj_BlockDecrypt>(new RGWGetObj_BlockDecrypt(s->cct, cb, std::move(block_crypt)));
...
int RGWPutObj_ObjStore_S3::get_encrypt_filter(std::unique_ptr<rgw::putobj::DataProcessor>* filter,
...
int RGWPostObj_ObjStore_S3::get_encrypt_filter(std::unique_ptr<rgw::putobj::DataProcessor>* filter,
                                               rgw::putobj::DataProcessor* cb)
{
    std::unique_ptr<BlockCrypt> block_crypt;
    int res = rgw_s3_prepare_encrypt(s, attrs, &parts, &block_crypt, crypt_http_responses);
    if (res == 0 && block_crypt != nullptr) {
        filter->reset(new RGWPutObj_BlockEncrypt(s->cct, cb, std::move(block_crypt)));
    }
    return res;
...
int RGWCopyObj_ObjStore_S3::get_params(optional_yield y)
{
    ...
    if (source_zone.empty() && (dest_tenant_name.compare(src_tenant_name) == 0) &&
        (dest_bucket_name.compare(src_bucket_name) == 0) && (dest_obj_name.compare(src_object->get_name()) == 0) &&
        src_object->get_instance().empty() && (attrs_mod != rgw::sal::ATTRSMOD_REPLACE)) {
...
int RGWPutACLs_ObjStore_S3::get_policy_from_state(rgw::sal::RGWRadosStore* store, struct req_state* s, stringstream& ss)
{
    RGWAccessControlPolicy_S3 s3policy(s->cct);
...
void RGWListBucketMultiparts_ObjStore_S3::send_response()
{
    ...
    if (!key_marker.empty()) s->formatter->dump_string("KeyMarker", key_marker);
    const string& upload_id_marker = marker.get_upload_id();
    if (!upload_id_marker.empty()) s->formatter->dump_string("UploadIdMarker", upload_id_marker);
    string next_key = next_marker.mp.get_key();
    if (!next_key.empty()) s->formatter->dump_string("NextKeyMarker", next_key);
    string next_upload_id = next_marker.mp.get_upload_id();
    if (!next_upload_id.empty()) s->formatter->dump_string("NextUploadIdMarker", next_upload_id);
...
RGWOp* RGWHandler_REST_Bucket_S3::op_put()
{
    ...
    if (is_tagging_op()) {
        ...
        if (!sync_policy_handler || sync_policy_handler->is_legacy_config()) {
            return nullptr;
        }
...
int RGWHandler_REST_S3::init_from_header(rgw::sal::RGWRadosStore* store, struct req_state* s, int default_formatter,
...
static int verify_mfa(rgw::sal::RGWRadosStore* store, RGWUserInfo* user, const string& mfa_str, bool* verified,
...
int RGWHandler_REST_S3::init(rgw::sal::RGWRadosStore* store, struct req_state* s, rgw::io::BasicClient* cio)
{
    int ret;
 ...
 */
int RGW_Auth_S3::authorize(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* const store,
                           const rgw::auth::StrategyRegistry& auth_registry, struct req_state* const s,
                           optional_yield y)
{

    /* neither keystone and rados enabled; warn and exit! */
    if (!store->ctx()->_conf->rgw_s3_auth_use_rados && !store->ctx()->_conf->rgw_s3_auth_use_keystone &&
...
int RGWHandler_Auth_S3::init(rgw::sal::RGWRadosStore* store, struct req_state* state, rgw::io::BasicClient* cio)
{
...
RGWHandler_REST* RGWRESTMgr_S3::get_handler(rgw::sal::RGWRadosStore* store, struct req_state* const s,
                                            const rgw::auth::StrategyRegistry& auth_registry,
                                            const std::string& frontend_prefix)
{
    bool is_s3website = enable_s3website && (s->prot_flags & RGW_REST_WEBSITE);
    int ret = RGWHandler_REST_S3::init_from_header(store, s, is_s3website ? RGW_FORMAT_HTML : RGW_FORMAT_XML, true);
...
int RGWHandler_REST_S3Website::init(rgw::sal::RGWRadosStore* store, req_state* s, rgw::io::BasicClient* cio)
{
    ...
    const req_state* const s, optional_yield y) const
{
    ...
    if (token.acct_type == TYPE_KEYSTONE || token.acct_type == TYPE_LDAP) {
        ...
        auto apl = local_apl_factory->create_apl_local(
            cct, s, user_info, subuser, token.perm_mask, std::string(_access_key_id));
        return result_t::grant(std::move(apl), completer_factory(token.secret_access_key));
    }
}
...
int RGWSelectObj_ObjStore_S3::create_message(std::string& out_string, u_int32_t result_len, u_int32_t header_len)
{
    ...
    // s3select result is produced into m_result, the m_result is also the response-message, thus the attach headers and
    // CRC are created later to the produced SQL result, and actually wrapping the payload.
    ...
    if (crc32 == 0) {
        ...
        crc32 = std::unique_ptr<boost::crc_32_type>(
            new boost::crc_optimal<32, 0x04C11DB7, 0xFFFFFFFF, 0xFFFFFFFF, true, true>);
    }
...
int RGWSelectObj_ObjStore_S3::run_s3select(const char* query, const char* input, size_t input_length)
{
    ...
    if (m_s3_csv_object == 0) {
        ...
        m_s3_csv_object =
            std::unique_ptr<s3selectEngine::csv_object>(new s3selectEngine::csv_object(s3select_syntax.get(), csv));
    }
    ...
    if (s3select_syntax->get_error_description().empty() == false) {
        ...
        ldpp_dout(this, 10) << "s3-select query: failed to prase query; {" << s3select_syntax->get_error_description()
                            << "}" << dendl;
        status = -1;
    }
    else {
