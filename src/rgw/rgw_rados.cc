...
class RGWIndexCompletionThread : public RGWRadosThread, public DoutPrefixProvider
{
    ...
    ceph::mutex completions_lock = ceph::make_mutex("RGWIndexCompletionThread::completions_lock");

public:
    RGWIndexCompletionThread(RGWRados* _store)
        : RGWRadosThread(_store, "index-complete")
        , store(_store)
    {}
...
int RGWIndexCompletionThread::process(const DoutPrefixProvider* dpp)
{
    ...
    for (auto c : comps) {
        ...
        if (r < 0) {
            ldpp_dout(this, 0) << "ERROR: " << __func__ << "(): failed to initialize BucketShard, obj=" << c->obj
                               << " r=" << r << dendl;
            /* not much to do */
            continue;
        }

        r = store->guard_reshard(this, &bs, c->obj, bucket_info, [&](RGWRados::BucketShard* bs) -> int {
        ...
        if (r < 0) {
            ldpp_dout(this, 0) << "ERROR: " << __func__ << "(): bucket index completion failed, obj=" << c->obj
                               << " r=" << r << dendl;
...
public:
    RGWIndexCompletionManager(RGWRados* _store)
        : store(_store)
        , locks{ceph::make_lock_container<ceph::mutex>(store->ctx()->_conf->rgw_thread_pool_size, [](const size_t i) {
            return ceph::make_mutex("RGWIndexCompletionManager::lock::" + std::to_string(i));
        })}
    {
...
int RGWRados::initialize(const DoutPrefixProvider* dpp)
{
    ...
    inject_notify_timeout_probability = cct->_conf.get_val<double>("rgw_inject_notify_timeout_probability");
...
int RGWRados::bucket_index_clear_olh(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                     RGWObjState& state, const rgw_obj& obj_instance)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(dpp, bucket_info, obj_instance, &ref);
    if (r < 0) {
