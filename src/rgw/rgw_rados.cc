...
static bool rgw_get_obj_data_pool(const RGWZoneGroup& zonegroup, const RGWZoneParams& zone_params,
                                  const rgw_placement_rule& head_placement_rule, const rgw_obj& obj, rgw_pool* pool)
{
...
RGWObjState::RGWObjState(const RGWObjState& rhs)
    : obj(rhs.obj)
{
...
public:
    RGWMetaNotifierManager(RGWRados* _store)
...
public:
    RGWDataNotifierManager(RGWRados* _store)
        : RGWCoroutinesManager(_store->ctx(), _store->get_cr_registry())
        , store(_store)
        , http_manager(store->ctx(), completion_mgr)
    {
                   ...
                   bc::flat_map<int, bc::flat_set<string>>& shards)
    {
        ...
        for (auto iter = conn_map.begin(); iter != conn_map.end(); ++iter) {
            RGWRESTConn* conn = iter->second;
            RGWCoroutinesStack* stack = new RGWCoroutinesStack(store->ctx(), this);
            stack->call(new RGWPostRESTResourceCR<bc::flat_map<int, bc::flat_set<string>>, int>(
                store->ctx(), conn, &http_manager, "/admin/log", pairs, shards, NULL));
...
public:
    RGWMetaNotifier(RGWRados* _store, RGWMetadataLog* log)
        : RGWRadosThread(_store, "meta-notifier")
        , notify_mgr(_store)
        , log(log)
    {}
...
public:
    RGWDataNotifier(RGWRados* _store)
        : RGWRadosThread(_store, "data-notifier")
        , notify_mgr(_store)
    {}
...
int RGWDataNotifier::process(const DoutPrefixProvider* dpp)
{
    ...
    for (const auto& [shard_id, keys] : shards) {
        ldpp_dout(dpp, 20) << __func__ << "(): notifying datalog change, shard_id=" << shard_id << ": " << keys
                           << dendl;
    }
...
{
public:
    RGWSyncProcessorThread(RGWRados* _store, const string& thread_name = "radosgw")
        : RGWRadosThread(_store, thread_name)
    {}
    RGWSyncProcessorThread(RGWRados* _store)
...
public:
    RGWMetaSyncProcessorThread(rgw::sal::RGWRadosStore* _store, RGWAsyncRadosProcessor* async_rados)
        : RGWSyncProcessorThread(_store->getRados(), "meta-sync")
...
public:
    RGWSyncLogTrimThread(rgw::sal::RGWRadosStore* store, rgw::BucketTrimManager* bucket_trim, int interval)
    ...
    int process(const DoutPrefixProvider* dpp) override
    {
        ...
        meta->call(create_meta_log_trim_cr(this, store, &http, cct->_conf->rgw_md_log_max_shards, trim_interval));
        stacks.push_back(meta);

        if (store->svc()->zone->sync_module_exports_data()) {
            auto data = new RGWCoroutinesStack(store->ctx(), &crs);
            data->call(create_data_log_trim_cr(this, store, &http, cct->_conf->rgw_data_log_num_shards, trim_interval));
...
void RGWRados::wakeup_data_sync_shards(const rgw_zone_id& source_zone, map<int, set<string>>& shard_ids)
{
    ...
    if (iter == data_sync_processor_threads.end()) {
        ldout(ctx(), 10) << __func__ << ": couldn't find sync thread for zone " << source_zone
...
int RGWRados::update_service_map(std::map<std::string, std::string>&& status)
{
    ...
    if (ret < 0) {
        ldout(cct, 0) << "ERROR: service_daemon_update_status() returned ret=" << ret << ": " << cpp_strerror(-ret)
                      << dendl;
        return ret;
    }
...
int RGWRados::log_show_next(RGWAccessHandle handle, rgw_log_entry* entry)
{
    ...
    ldout(cct, 10) << "log_show_next pos " << state->pos << " bl " << state->bl.length() << " off " << off << " eof "
                   << (int)state->eof << dendl;
    // read some?
...
int rgw_policy_from_attrset(const DoutPrefixProvider* dpp, CephContext* cct, map<string, bufferlist>& attrset,
                            RGWAccessControlPolicy* policy)
{
    map<string, bufferlist>::iterator aiter = attrset.find(RGW_ATTR_ACL);
    if (aiter == attrset.end()) return -EIO;
                                                 ...
                                                 optional_yield y)
{
    ...
    for (uint16_t attempt = 1; /* empty */; ++attempt) {
        ldpp_dout(dpp, 20) << "RGWRados::Bucket::List::" << __func__ << " starting attempt " << attempt << dendl;
        ...
        for (auto eiter = ent_map.begin(); eiter != ent_map.end(); ++eiter) {
            ...
            ldpp_dout(dpp, 20) << "RGWRados::Bucket::List::" << __func__ << " considering entry " << entry.key << dendl;
            ...
            if (!valid) {
                ldpp_dout(dpp, 0) << "ERROR: could not parse object name: " << obj.name << dendl;
                continue;
            }
                ...
                if (delim_pos >= 0) {
                    ...
                    if (cls_filtered) {
                        ...
                        if (delim_pos != int(obj.name.length() - params.delim.length())) {
                            ldpp_dout(dpp, 0) << "WARNING: found delimiter in place other than the end of "
                                                 "the prefix; obj.name="
            ...
            ldpp_dout(dpp, 20) << "RGWRados::Bucket::List::" << __func__ << " adding entry " << entry.key
                               << " to result" << dendl;
        ...
        if (!cls_filtered && !params.delim.empty()) {
            ...
            if (marker_delim_pos >= 0) {
                ...
                if (skip_after_delim > cur_marker.name) {
                    ...
                    ldpp_dout(dpp, 20) << "setting cur_marker=" << cur_marker.name << "[" << cur_marker.instance << "]"
                                       << dendl;
                }
            }
        ...
        ldpp_dout(dpp, 20) << "RGWRados::Bucket::List::" << __func__
                           << " INFO end of outer loop, truncated=" << truncated << ", count=" << count
                                                   ...
                                                   optional_yield y)
{
    ...
    while (truncated && count <= max) {
        ...
        for (auto& entry : ent_list) {
            ...
            if (params.filter && !params.filter->filter(obj.name, index_key.name)) continue;

            if (params.prefix.size() && (0 != obj.name.compare(0, params.prefix.size(), params.prefix))) continue;
    ...
    if (is_truncated) *is_truncated = truncated;
 ...
 * up without one
 */
int RGWRados::fix_tail_obj_locator(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, rgw_obj_key& key,
...
int RGWRados::BucketShard::init(const rgw_bucket& _bucket, int sid,
                                const rgw::bucket_index_layout_generation& idx_layout, RGWBucketInfo* bucket_info_out,
                                const DoutPrefixProvider* dpp)
{
    bucket = _bucket;
    shard_id = sid;
...
{
    return bucket->get_info().has_swift_versioning() && bucket->get_info().swift_ver_location.size();
}

int RGWRados::swift_versioning_copy(RGWObjectCtx& obj_ctx, const rgw_user& user, rgw::sal::RGWBucket* bucket,
                                    rgw::sal::RGWObject* obj, const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    r = get_bucket_info(
        &svc, bucket->get_tenant(), bucket->get_info().swift_ver_location, dest_bucket_info, NULL, null_yield, NULL);
    if (r < 0) {
...
int RGWRados::swift_versioning_restore(RGWObjectCtx& obj_ctx, const rgw_user& user, rgw::sal::RGWBucket* bucket,
                                       rgw::sal::RGWObject* obj, bool& restored, /* out */
                                       const DoutPrefixProvider* dpp)
{
    if (!swift_versioning_enabled(bucket)) {
        return 0;
    ...
    int ret = get_bucket_info(
        &svc, bucket->get_tenant(), bucket->get_info().swift_ver_location, archive_binfo, nullptr, null_yield, nullptr);
    if (ret < 0) {
        return ret;
    }
    ...
    const auto handler = [&](const rgw_bucket_dir_entry& entry) -> int {
        ...
        ret = delete_obj(dpp, obj_ctx, archive_binfo, archive_obj.get_obj(), archive_binfo.versioning_status());
...
public:
    RGWRadosPutObj(const DoutPrefixProvider* dpp, CephContext* cct, CompressorRef& plugin,
                   boost::optional<RGWPutObj_Compress>& compressor, rgw::putobj::ObjectProcessor* p,
...
int RGWRados::rewrite_obj(RGWBucketInfo& dest_bucket_info, rgw::sal::RGWObject* obj, const DoutPrefixProvider* dpp,
                          optional_yield y)
{
    RGWObjectCtx rctx(this->store);
...
int RGWRados::stat_remote_obj(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, const rgw_user& user_id,
                              req_info* info, const rgw_zone_id& source_zone, rgw::sal::RGWObject* src_obj,
                              ...
                              map<string, string>* pheaders, string* version_id, string* ptag, string* petag)
{
    ...
    if (extra_data_bl.length()) {
        JSONParser jp;
        if (!jp.parse(extra_data_bl.c_str(), extra_data_bl.length())) {
            ldout(cct, 0) << "failed to parse response extra data. len=" << extra_data_bl.length()
                          << " data=" << extra_data_bl.c_str() << dendl;
            return -EIO;
        }
...
int RGWRados::fetch_remote_obj(RGWObjectCtx& obj_ctx, const rgw_user& user_id, req_info* info,
                               const rgw_zone_id& source_zone, rgw::sal::RGWObject* dest_obj,
                               rgw::sal::RGWObject* src_obj, rgw::sal::RGWBucket* dest_bucket,
                               rgw::sal::RGWBucket* src_bucket, std::optional<rgw_placement_rule> dest_placement_rule,
                               real_time* src_mtime, real_time* mtime, const real_time* mod_ptr,
                               const real_time* unmod_ptr, bool high_precision_time, const char* if_match,
                               const char* if_nomatch, AttrsMod attrs_mod, bool copy_if_newer,
                               rgw::sal::RGWAttrs& attrs, RGWObjCategory category, std::optional<uint64_t> olh_epoch,
                               real_time delete_at, string* ptag, string* petag, void (*progress_cb)(off_t, void*),
                               void* progress_data, const DoutPrefixProvider* dpp, RGWFetchObjFilter* filter,
                               rgw_zone_set* zones_trace, std::optional<uint64_t>* bytes_transferred)
{
    ...
    AtomicObjectProcessor processor(
        &aio, this->store, dest_bucket, nullptr, user_id, obj_ctx, dest_obj->clone(), olh_epoch, tag, dpp, null_yield);
    RGWRESTConn* conn;
    auto& zone_conn_map = svc.zone->get_zone_conn_map();
    auto& zonegroup_conn_map = svc.zone->get_zonegroup_conn_map();
    if (source_zone.empty()) {
    ...
    if (copy_if_newer) {
        ...
        ret =
            get_obj_state(dpp, &obj_ctx, dest_bucket->get_info(), dest_obj->get_obj(), &dest_state, false, null_yield);
    ...
    ret = conn->complete_request(in_stream_req, &etag, &set_mtime, &expected_size, nullptr, nullptr, null_yield);
    if (ret < 0) {
        goto set_err_state;
    }
    ret = cb.flush();
    ...
    if (cb.get_data_len() != expected_size) {
        ret = -EIO;
        ldpp_dout(dpp, 0) << "ERROR: object truncated during fetching, expected " << expected_size
...
#define MAX_COMPLETE_RETRY 100
    for (i = 0; i < MAX_COMPLETE_RETRY; i++) {
        bool canceled = false;
        ret = processor.complete(cb.get_data_len(),
                                 etag,
                                 mtime,
                                 set_mtime,
                                 attrs,
        ...
        if (copy_if_newer && canceled) {
            ...
            ret = get_obj_state(
                dpp, &obj_ctx, dest_bucket->get_info(), dest_obj->get_obj(), &dest_state, false, null_yield);
            if (ret < 0) {
...
int RGWRados::copy_obj_to_remote_dest(const DoutPrefixProvider* dpp, RGWObjState* astate,
                                      ...
                                      const rgw_user& user_id, rgw::sal::RGWObject* dest_obj, real_time* mtime)
{
    string etag;
...
int RGWRados::copy_obj(RGWObjectCtx& obj_ctx, const rgw_user& user_id, req_info* info, const rgw_zone_id& source_zone,
                       rgw::sal::RGWObject* dest_obj, rgw::sal::RGWObject* src_obj, rgw::sal::RGWBucket* dest_bucket,
                       rgw::sal::RGWBucket* src_bucket, const rgw_placement_rule& dest_placement, real_time* src_mtime,
                       ...
                       rgw::sal::RGWAttrs& attrs, RGWObjCategory category, uint64_t olh_epoch, real_time delete_at,
                       ...
                       void* progress_data, const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    ldpp_dout(dpp, 5) << "Copy object " << src_obj->get_bucket() << ":" << src_obj->get_oid() << " => "
                      << dest_obj->get_bucket() << ":" << dest_obj->get_oid() << dendl;
    ...
    bool copy_data = (!astate->manifest) || (*src_rule != dest_placement) || (src_pool != dest_pool);
...
int RGWRados::copy_obj_data(RGWObjectCtx& obj_ctx, rgw::sal::RGWBucket* bucket,
                            const rgw_placement_rule& dest_placement, RGWRados::Object::Read& read_op, off_t end,
                            rgw::sal::RGWObject* dest_obj, real_time* mtime, real_time set_mtime,
                            rgw::sal::RGWAttrs& attrs, uint64_t olh_epoch, real_time delete_at, string* petag,
...
int RGWRados::transition_obj(RGWObjectCtx& obj_ctx, rgw::sal::RGWBucket* bucket, rgw::sal::RGWObject& obj,
...
int RGWRados::check_bucket_empty(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info, optional_yield y)
{
    ...
    do {
        ...
        int r = cls_bucket_list_unordered(
            dpp, bucket_info, RGW_NO_SHARD, marker, prefix, NUM_ENTRIES, true, ent_list, &is_truncated, &marker, y);
        if (r < 0) {
            return r;
        }
...
{
    if ((!state->manifest) || state->keep_tail) return 0;
...
int RGWRados::defer_gc(const DoutPrefixProvider* dpp, void* ctx, const RGWBucketInfo& bucket_info, const rgw_obj& obj,
                       optional_yield y)
{
...
static void generate_fake_tag(const DoutPrefixProvider* dpp, rgw::sal::RGWStore* store,
                              map<string, bufferlist>& attrset, RGWObjManifest& manifest, bufferlist& manifest_bl,
                              bufferlist& tag_bl)
{
    string tag;
...
int RGWRados::get_olh_target_state(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx,
                                   const RGWBucketInfo& bucket_info, const rgw_obj& obj, RGWObjState* olh_state,
                                   RGWObjState** target_state, optional_yield y)
{
...
int RGWRados::get_obj_state_impl(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, const RGWBucketInfo& bucket_info,
                                 const rgw_obj& obj, RGWObjState** state, bool follow_olh, optional_yield y,
                                 bool assume_noent)
{
    if (obj.empty()) {
        return -EINVAL;
    ...
    *state = s;
    ...
    if (manifest_bl.length()) {
        ...
        try {
            ...
            s->manifest->set_head(bucket_info.placement_rule, obj, s->size); /* patch manifest to reflect the head we
        ...
        if (cct->_conf->subsys.should_gather<ceph_subsys_rgw, 20>() && s->manifest->has_explicit_objs()) {
            ...
            for (mi = s->manifest->obj_begin(dpp); mi != s->manifest->obj_end(dpp); ++mi) {
                ldpp_dout(dpp, 20) << "manifest: ofs=" << mi.get_ofs()
                                   << " loc=" << mi.get_location().get_raw_obj(store) << dendl;
            }
        }
...
int RGWRados::get_obj_state(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, const RGWBucketInfo& bucket_info,
                            const rgw_obj& obj, RGWObjState** state, bool follow_olh, optional_yield y,
                            bool assume_noent)
{
    int ret;
...
int RGWRados::append_atomic_test(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, const RGWBucketInfo& bucket_info,
                                 const rgw_obj& obj, ObjectOperation& op, RGWObjState** pstate, optional_yield y)
{
    if (!rctx) return 0;
...
int RGWRados::Object::get_state(const DoutPrefixProvider* dpp, RGWObjState** pstate, bool follow_olh, optional_yield y,
                                bool assume_noent)
{
                                                  ...
                                                  optional_yield y)
{
    ...
    bool need_guard = ((state->manifest) || (state->obj_tag.length() != 0) || if_match != NULL || if_nomatch != NULL) &&
                      (!state->fake_tag);
 ...
 * Returns: 0 on success, -ERR# otherwise.
 */
int RGWRados::set_attr(const DoutPrefixProvider* dpp, void* ctx, const RGWBucketInfo& bucket_info, rgw_obj& obj,
...
int RGWRados::set_attrs(const DoutPrefixProvider* dpp, void* ctx, const RGWBucketInfo& bucket_info, rgw_obj& src_obj,
                        map<string, bufferlist>& attrs, map<string, bufferlist>* rmattrs, optional_yield y)
{
    rgw_obj obj = src_obj;
...
int RGWRados::Bucket::UpdateIndex::guard_reshard(const DoutPrefixProvider* dpp, BucketShard** pbs,
...
#define NUM_RESHARD_RETRIES 10
    for (int i = 0; i < NUM_RESHARD_RETRIES; ++i) {
        ...
        if (r == -ENOENT) {   // case where resharding must have completed
            ldpp_dout(dpp, 0) << "NOTICE: resharding operation recently completed, invalidating "
                                 "old BucketInfo"
                              << dendl;

            r = store->fetch_new_bucket_id(target->bucket_info, nullptr, new_bucket_id, dpp);
            if (r == -ENOENT) {
                ...
                ldpp_dout(dpp, 0) << "ERROR: " << __func__
                                  << " unable to refresh stale bucket_id after reshard; r=" << r << dendl;
                return r;
            }
        }
            ...
            r = store->block_while_resharding(bs, &new_bucket_id, target->bucket_info, null_yield, dpp);
            if (r == -ERR_BUSY_RESHARDING) {
                continue;
            }
            if (r < 0) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: update_bucket_id() new_bucket_id=" << new_bucket_id << " returned r=" << r
                              << dendl;
            return r;
        }
...
struct get_obj_data
{
    ...
    get_obj_data(RGWRados* store, RGWGetDataCB* cb, rgw::Aio* aio, uint64_t offset, optional_yield yield)
        : store(store)
        , client_cb(cb)
        , aio(aio)
        , offset(offset)
                               ...
                               off_t read_ofs, off_t len, bool is_head_obj, RGWObjState* astate, void* arg)
{
    ...
    return d->store->get_obj_iterate_cb(dpp, read_obj, obj_ofs, read_ofs, len, is_head_obj, astate, arg);
}
                                    ...
                                    optional_yield y)
{
    ...
    int r = store->iterate_obj(
        dpp, obj_ctx, source->get_bucket_info(), state.obj, ofs, end, chunk_size, _get_obj_iterate_cb, &data, y);
    if (r < 0) {
...
int RGWRados::iterate_obj(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, const RGWBucketInfo& bucket_info,
...
int RGWRados::guard_reshard(const DoutPrefixProvider* dpp, BucketShard* bs, const rgw_obj& obj_instance,
                            const RGWBucketInfo& bucket_info, std::function<int(BucketShard*)> call)
{
    rgw_obj obj;
...
int RGWRados::fetch_new_bucket_id(const RGWBucketInfo& curr_bucket_info,
                                  RGWBucketInfo* save_bucket_info,   // nullptr -> no save
                                  std::string& new_bucket_id, const DoutPrefixProvider* dpp)
{
...
int RGWRados::block_while_resharding(RGWRados::BucketShard* bs, std::string* new_bucket_id,
                                     const RGWBucketInfo& bucket_info, optional_yield y, const DoutPrefixProvider* dpp)
{
    int ret = 0;
...
int RGWRados::bucket_index_link_olh(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                    RGWObjState& olh_state, const rgw_obj& obj_instance, bool delete_marker,
                                    const string& op_tag, struct rgw_bucket_dir_entry_meta* meta, uint64_t olh_epoch,
...
int RGWRados::bucket_index_unlink_instance(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                           ...
                                           uint64_t olh_epoch, rgw_zone_set* _zones_trace)
{
    ...
    r = guard_reshard(dpp, &bs, obj_instance, bucket_info, [&](BucketShard* bs) -> int {
        ...
        op.assert_exists();   // bucket index shard must exist
        cls_rgw_guard_bucket_resharding(op, -ERR_BUSY_RESHARDING);
        cls_rgw_bucket_unlink_instance(op, key, op_tag, olh_tag, olh_epoch, svc.zone->get_zone().log_data, zones_trace);
        return rgw_rados_operate(dpp, ref.pool.ioctx(), ref.obj.oid, &op, null_yield);
    });
    if (r < 0) {
...
int RGWRados::bucket_index_read_olh_log(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                        RGWObjState& state, const rgw_obj& obj_instance, uint64_t ver_marker,
                                        map<uint64_t, vector<rgw_bucket_olh_log_entry>>* log, bool* is_truncated)
{
    ...
    ret = guard_reshard(dpp, &bs, obj_instance, bucket_info, [&](BucketShard* bs) -> int {
        auto& ref = bs->bucket_obj.get_ref();
        ObjectReadOperation op;
        cls_rgw_guard_bucket_resharding(op, -ERR_BUSY_RESHARDING);
...
int RGWRados::bucket_index_trim_olh_log(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                        RGWObjState& state, const rgw_obj& obj_instance, uint64_t ver)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(dpp, bucket_info, obj_instance, &ref);
    if (r < 0) {
...
int RGWRados::bucket_index_clear_olh(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
...
int RGWRados::apply_olh_log(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, RGWObjState& state,
                            const RGWBucketInfo& bucket_info, const rgw_obj& obj, bufferlist& olh_tag,
...
int RGWRados::clear_olh(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, const rgw_obj& obj,
                        const RGWBucketInfo& bucket_info, const std::string& tag, const uint64_t ver, optional_yield y)
{
    rgw_rados_ref ref;
    int r = get_obj_head_ref(dpp, bucket_info, obj, &ref);
    if (r < 0) {
...
int RGWRados::clear_olh(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, const rgw_obj& obj,
                        const RGWBucketInfo& bucket_info, rgw_rados_ref& ref, const std::string& tag,
                        const uint64_t ver, optional_yield y)
{
    ObjectWriteOperation rm_op;
...
int RGWRados::update_olh(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, RGWObjState* state,
                         const RGWBucketInfo& bucket_info, const rgw_obj& obj, rgw_zone_set* zones_trace)
{
...
int RGWRados::set_olh(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx, const RGWBucketInfo& bucket_info,
...
int RGWRados::get_olh(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, const rgw_obj& obj,
                      RGWOLHInfo* olh)
{
...
int RGWRados::follow_olh(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, RGWObjectCtx& obj_ctx,
...
int RGWRados::get_bucket_stats(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info, int shard_id,
                               string* bucket_ver, string* master_ver, map<RGWObjCategory, RGWStorageStats>& stats,
...
int RGWRados::get_bucket_stats_async(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info, int shard_id,
                                       ...
                                       const DoutPrefixProvider* dpp)
{
    ...
    return ctl.bucket->read_bucket_instance_info(
        ...
        y,
        dpp,
        RGWBucketCtl::BucketInstance::GetParams().set_mtime(pmtime).set_attrs(pattrs).set_bectx_params(bectx_params));
}
...
int RGWRados::put_bucket_instance_info(RGWBucketInfo& info, bool exclusive, real_time mtime,
                                       map<string, bufferlist>* pattrs, const DoutPrefixProvider* dpp)
{
...
static int do_pool_iterate(CephContext* cct, RGWPoolIterCtx& ctx, uint32_t num, vector<rgw_bucket_dir_entry>& objs,
...
int RGWRados::pool_iterate(RGWPoolIterCtx& ctx, uint32_t num, vector<rgw_bucket_dir_entry>& objs, bool* is_truncated,
                      ...
                      list<rgw_cls_bi_entry>* entries, bool* is_truncated)
{
    BucketShard bs(this);
    int ret =
        bs.init(bucket_info.bucket, shard_id, bucket_info.layout.current_index, nullptr /* no RGWBucketInfo */, dpp);
    if (ret < 0) {
        ldpp_dout(dpp, 5) << "bs.init() returned ret=" << ret << dendl;
        return ret;
    }
...
int RGWRados::list_lc_progress(string& marker, uint32_t max_entries, vector<rgw::sal::Lifecycle::LCEntry>& progress_map,
                               int& index)
{
                                 ...
                                 rgw_obj& obj, uint16_t bilog_flags, optional_yield y, rgw_zone_set* _zones_trace)
{
    ...
    cls_rgw_bucket_prepare_op(
        o, op, tag, key, obj.key.get_loc(), svc.zone->get_zone().log_data, bilog_flags, zones_trace);
                                  ...
                                  rgw_zone_set* _zones_trace)
{
    ...
    cls_rgw_bucket_complete_op(
        o, op, tag, ver, key, dir_meta, remove_objs, svc.zone->get_zone().log_data, bilog_flags, &zones_trace);
    complete_op_data* arg;
    index_completion_manager->create_completion(
        obj, op, tag, ver, key, dir_meta, remove_objs, svc.zone->get_zone().log_data, bilog_flags, &zones_trace, &arg);
...
int RGWRados::cls_bucket_list_ordered(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info, const int shard_id,
                                      const rgw_obj_index_key& start_after, const string& prefix,
                                      const string& delimiter, const uint32_t num_entries, const bool list_versions,
                                      const uint16_t expansion_factor, ent_map_t& m, bool* is_truncated,
                                      bool* cls_filtered, rgw_obj_index_key* last_entry, optional_yield y,
                                      check_filter_t force_check_filter)
{
    ...
    ldpp_dout(dpp, 10) << "RGWRados::" << __func__ << ": " << bucket_info.bucket << " start_after=\""
                       << start_after.name << "[" << start_after.instance << "]\", prefix=\"" << prefix
    ...
    int r = svc.bi_rados->open_bucket_index(dpp, bucket_info, shard_id, &index_pool, &shard_oids, nullptr);
    if (r < 0) {
        return r;
    }
    ...
    ldpp_dout(dpp, 10) << "RGWRados::" << __func__ << " request from each of " << shard_count << " shard(s) for "
    ...
    while (count < num_entries && !candidates.empty()) {
        ...
        ldpp_dout(dpp, 20) << "RGWRados::" << __func__ << " currently processing " << dirent.key << " from shard "
                           << tracker.shard_idx << dendl;
        ...
        if (r >= 0) {
            ldpp_dout(dpp, 10) << "RGWRados::" << __func__ << ": got " << dirent.key.name << "[" << dirent.key.instance
                               << "]" << dendl;
            ...
            ldpp_dout(dpp, 10) << "RGWRados::" << __func__ << ": skipping " << dirent.key.name << "["
        ...
        if (need_to_stop) {
            ...
            ldpp_dout(dpp, 10) << __PRETTY_FUNCTION__ << ": stopped accumulating results at count=" << count
                               << ", dirent=\"" << dirent.key << "\", because its shard is truncated and exhausted"
                               << dendl;
            break;
        }
    ...
    *is_truncated = false;
    ...
    ldpp_dout(dpp, 20) << "RGWRados::" << __func__ << ": returning, count=" << count
                       << ", is_truncated=" << *is_truncated << dendl;

    if (*is_truncated && count < num_entries) {
        ldpp_dout(dpp, 10) << "RGWRados::" << __func__ << ": INFO requested " << num_entries
                           << " entries but returning " << count << ", which is truncated" << dendl;
    }
...
int RGWRados::cls_bucket_list_unordered(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info, int shard_id,
                                        const rgw_obj_index_key& start_after, const string& prefix,
                                        ...
                                        check_filter_t force_check_filter)
{
    ldpp_dout(dpp, 10) << "cls_bucket_list_unordered " << bucket_info.bucket << " start_after " << start_after.name
                       << "[" << start_after.instance << "] num_entries " << num_entries << dendl;
    ...
    *is_truncated = false;
    ...
    if (shard_id >= 0) {
        ...
        if (!parsed) {
            ldpp_dout(dpp, 0) << "ERROR: RGWRados::cls_bucket_list_unordered received an invalid "
                                 "start marker: '"
                              << start_after << "'" << dendl;
            return -EINVAL;
        }
    ...
    while (count <= num_entries &&
        ...
        for (auto& entry : result.dir.m) {
            ...
            if (r >= 0) {   // i.e., if r != -ENOENT
                ldpp_dout(dpp, 10) << "RGWRados::cls_bucket_list_unordered: got " << dirent.key.name << "["
                                   << dirent.key.instance << "]" << dendl;
                                     ...
                                     list<rgw_obj_index_key>& oid_list)
{
    ...
    for (auto iter = oid_list.begin(); iter != oid_list.end(); ++iter) {
        ...
        entry.key = *iter;
        ldpp_dout(dpp, 2) << "RGWRados::remove_objs_from_index bucket=" << bucket_info.bucket
...
int RGWRados::check_disk_state(const DoutPrefixProvider* dpp, librados::IoCtx io_ctx, const RGWBucketInfo& bucket_info,
...
int RGWRados::cls_bucket_head(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, int shard_id,
...
int RGWRados::cls_bucket_head_async(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, int shard_id,
                                  ...
                                  const DoutPrefixProvider* dpp)
{
    ...
    uint32_t num_source_shards = (bucket_info.layout.current_index.layout.normal.num_shards > 0
    ...
    quota_handler->check_bucket_shards(
        max_objs_per_shard, num_source_shards, num_objs, need_resharding, &suggested_num_shards);
    if (!need_resharding) {
        return 0;
    }
...
int RGWRados::check_quota(const rgw_user& bucket_owner, rgw_bucket& bucket, RGWQuotaInfo& user_quota,
                          RGWQuotaInfo& bucket_quota, uint64_t obj_size, optional_yield y, bool check_size_only)
{
