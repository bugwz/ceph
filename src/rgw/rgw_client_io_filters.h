...
template<typename T> class AccountingFilter : public DecoratedRestfulClient<T>, public Accounter
{
    bool enabled;
    uint64_t total_sent;
...
public:
    ...
    {
        const auto sent = DecoratedRestfulClient<T>::send_status(status, status_name);
        lsubdout(cct, rgw, 30) << "AccountingFilter::send_status: e=" << (enabled ? "1" : "0") << ", sent=" << sent
    ...
    size_t send_100_continue() override
    {
        ...
        lsubdout(cct, rgw, 30) << "AccountingFilter::send_100_continue: e=" << (enabled ? "1" : "0")
                               << ", sent=" << sent << ", total=" << total_sent << dendl;
        if (enabled) {
            total_sent += sent;
        }
        return sent;
    ...
    size_t send_header(const std::string_view& name, const std::string_view& value) override
    {
        const auto sent = DecoratedRestfulClient<T>::send_header(name, value);
        lsubdout(cct, rgw, 30) << "AccountingFilter::send_header: e=" << (enabled ? "1" : "0") << ", sent=" << sent
                               << ", total=" << total_sent << dendl;
        if (enabled) {
            total_sent += sent;
        }
    ...
    size_t send_content_length(const uint64_t len) override
    {
        ...
        lsubdout(cct, rgw, 30) << "AccountingFilter::send_content_length: e=" << (enabled ? "1" : "0")
                               << ", sent=" << sent << ", total=" << total_sent << dendl;
        if (enabled) {
            total_sent += sent;
        }
    ...
    size_t send_chunked_transfer_encoding() override
    {
        ...
        lsubdout(cct, rgw, 30) << "AccountingFilter::send_chunked_transfer_encoding: e=" << (enabled ? "1" : "0")
                               << ", sent=" << sent << ", total=" << total_sent << dendl;
        if (enabled) {
            total_sent += sent;
        }
    ...
    size_t complete_header() override
    {
        ...
        lsubdout(cct, rgw, 30) << "AccountingFilter::complete_header: e=" << (enabled ? "1" : "0") << ", sent=" << sent
                               << ", total=" << total_sent << dendl;
        if (enabled) {
            total_sent += sent;
        }
    ...
    size_t recv_body(char* buf, size_t max) override
    {
        ...
        lsubdout(cct, rgw, 30) << "AccountingFilter::recv_body: e=" << (enabled ? "1" : "0")
    ...
    size_t send_body(const char* const buf, const size_t len) override
    {
        const auto sent = DecoratedRestfulClient<T>::send_body(buf, len);
        lsubdout(cct, rgw, 30) << "AccountingFilter::send_body: e=" << (enabled ? "1" : "0") << ", sent=" << sent
                               << ", total=" << total_sent << dendl;
        if (enabled) {
            total_sent += sent;
        }
    ...
    size_t complete_request() override
    {
        ...
        lsubdout(cct, rgw, 30) << "AccountingFilter::complete_request: e=" << (enabled ? "1" : "0") << ", sent=" << sent
                               << ", total=" << total_sent << dendl;
        if (enabled) {
            total_sent += sent;
        }
    ...
    uint64_t get_bytes_sent() const override { return total_sent; }

    uint64_t get_bytes_received() const override { return total_received; }

    void set_account(bool enabled) override
    {
        this->enabled = enabled;
        lsubdout(cct, rgw, 30) << "AccountingFilter::set_account: e=" << (enabled ? "1" : "0") << dendl;
    }
};
...
template<typename T> class BufferingFilter : public DecoratedRestfulClient<T>
{
...
template<typename T> size_t BufferingFilter<T>::send_body(const char* const buf, const size_t len)
{
    if (buffer_data) {
        data.append(buf, len);

        lsubdout(cct, rgw, 30) << "BufferingFilter<T>::send_body: defer count = " << len << dendl;
        return 0;
    }
...
template<typename T> size_t BufferingFilter<T>::send_content_length(const uint64_t len)
{
    has_content_length = true;
    return DecoratedRestfulClient<T>::send_content_length(len);
}

template<typename T> size_t BufferingFilter<T>::send_chunked_transfer_encoding()
{
    has_content_length = true;
    return DecoratedRestfulClient<T>::send_chunked_transfer_encoding();
}

template<typename T> size_t BufferingFilter<T>::complete_header()
{
...
template<typename T> size_t BufferingFilter<T>::complete_request()
{
    size_t sent = 0;
    ...
    if (!has_content_length) {
        ...
        lsubdout(cct, rgw, 30) << "BufferingFilter::complete_request: !has_content_length: IGNORE: sent=" << sent
                               << dendl;
        sent = 0;
    }
    ...
    if (buffer_data) {
        ...
        data.clear();
        buffer_data = false;
        lsubdout(cct, rgw, 30) << "BufferingFilter::complete_request: buffer_data: sent=" << sent << dendl;
    }

    return sent + DecoratedRestfulClient<T>::complete_request();
}

template<typename T> static inline BufferingFilter<T> add_buffering(CephContext* cct, T&& t)
{
    return BufferingFilter<T>(cct, std::forward<T>(t));
}


template<typename T> class ChunkingFilter : public DecoratedRestfulClient<T>
{
    template<typename Td> friend class DecoratedRestfulClient;
...
public:
    ...
    size_t send_chunked_transfer_encoding() override
    {
        ...
        return DecoratedRestfulClient<T>::send_header("Transfer-Encoding", "chunked");
    }

    size_t send_body(const char* buf, const size_t len) override
    {
        if (!chunking_enabled) {
            return DecoratedRestfulClient<T>::send_body(buf, len);
        }
            ...
            char chunk_size[32];
            const auto chunk_size_len = snprintf(chunk_size, sizeof(chunk_size), "%zx\r\n", len);
            size_t sent = 0;

            sent += DecoratedRestfulClient<T>::send_body(chunk_size, chunk_size_len);
            sent += DecoratedRestfulClient<T>::send_body(buf, len);
            sent += DecoratedRestfulClient<T>::send_body(HEADER_END, sizeof(HEADER_END) - 1);
            return sent;
        }
    }
    ...
    size_t complete_request() override
    {
        ...
        if (chunking_enabled) {
            ...
            sent += DecoratedRestfulClient<T>::send_body(CHUNKED_RESP_END, sizeof(CHUNKED_RESP_END) - 1);
        }
...
template<typename T> static inline ChunkingFilter<T> add_chunking(T&& t)
{
...
template<typename T> class ConLenControllingFilter : public DecoratedRestfulClient<T>
{
protected:
...
public:
    ...
    size_t send_status(const int status, const char* const status_name) override
    {
        if ((204 == status || 304 == status) && !g_conf()->rgw_print_prohibited_content_length) {
    ...
    {
        switch (action) {
        case ContentLengthAction::FORWARD: return DecoratedRestfulClient<T>::send_content_length(len);
        case ContentLengthAction::INHIBIT: return 0;
...
template<typename T> static inline ConLenControllingFilter<T> add_conlen_controlling(T&& t)
{
...
template<typename T> class ReorderingFilter : public DecoratedRestfulClient<T>
{
protected:
    ...
    size_t send_header(const std::string_view& name, const std::string_view& value) override
    {
        ...
        case ReorderState::RGW_STATUS_SEEN:
            headers.emplace_back(
                std::make_pair(std::string(name.data(), name.size()), std::string(value.data(), value.size())));
            return 0;
        case ReorderState::RGW_DATA: return DecoratedRestfulClient<T>::send_header(name, value);
        }
...
template<typename T> static inline ReorderingFilter<T> add_reordering(T&& t)
{
