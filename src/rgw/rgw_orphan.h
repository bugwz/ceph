...
struct RGWOrphanSearchStage
{
    ...
    explicit RGWOrphanSearchStage(RGWOrphanSearchStageId _stage)
        ...
        , shard(0)
    {}
    RGWOrphanSearchStage(RGWOrphanSearchStageId _stage, int _shard, const std::string& _marker)
        : stage(_stage)
        , shard(_shard)
        , marker(_marker)
    {}
...
public:
    ...
    int store_entries(const DoutPrefixProvider* dpp, const std::string& oid,
                      const std::map<std::string, bufferlist>& entries);
    int read_entries(const std::string& oid, const std::string& marker, std::map<std::string, bufferlist>* entries,
                     bool* truncated);
};
...
public:
    RGWOrphanSearch(rgw::sal::RadosStore* _store, int _max_ios, uint64_t _stale_secs)
        : store(_store)
        , orphan_store(store)
    ...
    int init(const DoutPrefixProvider* dpp, const std::string& job_name, RGWOrphanSearchInfo* info,
    ...
    int build_linked_oids_for_bucket(const DoutPrefixProvider* dpp, const std::string& bucket_instance_id,
                                     std::map<int, std::list<std::string> >& oids);
...
class RGWRadosList
{
    ...
    {
        auto p = bucket_process_map.emplace(std::make_pair(bucket_name, process_t()));
        p.first->second.entire_container = true;
    }

    void add_bucket_prefix(const std::string& bucket_name, const std::string& prefix)
    {
        auto p = bucket_process_map.emplace(std::make_pair(bucket_name, process_t()));
        p.first->second.prefixes.insert(prefix);
    }

    void add_bucket_filter(const std::string& bucket_name, const rgw_obj_key& obj_key)
    {
        auto p = bucket_process_map.emplace(std::make_pair(bucket_name, process_t()));
    ...
    int handle_stat_result(const DoutPrefixProvider* dpp, RGWRados::Object::Stat::Result& result,
                           std::string& bucket_name, rgw_obj_key& obj_key, std::set<std::string>& obj_oids);
    int pop_and_handle_stat_op(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx,
                               std::deque<RGWRados::Object::Stat>& ops);

public:
    RGWRadosList(rgw::sal::RadosStore* _store, int _max_ios, uint64_t _stale_secs, const std::string& _tenant_name)
        : store(_store)
        , max_concurrent_ios(_max_ios)
        , stale_secs(_stale_secs)
    ...
    int process_bucket(const DoutPrefixProvider* dpp, const std::string& bucket_instance_id, const std::string& prefix,
                       const std::set<rgw_obj_key>& entries_filter);

    int do_incomplete_multipart(const DoutPrefixProvider* dpp, rgw::sal::Bucket* bucket);

    int build_linked_oids_index();

    int run(const DoutPrefixProvider* dpp, const std::string& bucket_id, const bool silent_indexless = false);
