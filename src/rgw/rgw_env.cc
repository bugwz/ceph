...
const char* rgw_conf_get(const map<string, string, ltstr_nocase>& conf_map, const char* name, const char* def_val)
{
    ...
    if (iter == conf_map.end()) return def_val;

    return iter->second.c_str();
}

boost::optional<const std::string&> rgw_conf_get_optional(const map<string, string, ltstr_nocase>& conf_map,
                                                          const std::string& name)
{
    auto iter = conf_map.find(name);
    if (iter == conf_map.end()) return boost::none;
...
boost::optional<const std::string&> RGWEnv::get_optional(const std::string& name) const
{
...
size_t RGWEnv::get_size(const char* name, size_t def_val) const
{
    ...
    if (iter == env_map.end()) return def_val;
...
{
    if (env_map.empty() || prefix == NULL) return false;

    const auto iter = env_map.lower_bound(prefix);
    if (iter == env_map.end()) return false;
...
void RGWEnv::remove(const char* name)
{
    ...
    if (iter != env_map.end()) env_map.erase(iter);
}
