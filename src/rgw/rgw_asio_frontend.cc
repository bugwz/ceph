...
void handle_connection(boost::asio::io_context& context, RGWProcessEnv& env, Stream& stream, timeout_timer& timeout,
                       parse_buffer& buffer, bool is_ssl, SharedMutex& pause_mutex, rgw::dmclock::Scheduler* scheduler,
                       boost::system::error_code& ec, yield_context yield)
{
    ...
    for (;;) {
...

        {
            ...
            StreamIO real_client{
                cct, stream, timeout, parser, yield, buffer, is_ssl, socket.local_endpoint(), remote_endpoint};
...
int AsioFrontend::init()
{
    ...
    if (timeout != config.end()) {
        ...
        if (timeout_number) {
            ...
            lderr(ctx()) << "WARNING: invalid value for request_timeout_ms: " << timeout->second.data()
    ...
    if (l.use_ssl) {
        spawn::spawn(
            ...
            [this, s = std::move(stream)](yield_context yield) mutable {
                ...
                handle_connection(
                    context, env, stream, timeout, conn->buffer, true, pause_mutex, scheduler.get(), ec, yield);
                if (!ec) {
...
#else
    {
        ...
        spawn::spawn(
            ...
            [this, s = std::move(stream)](yield_context yield) mutable {
                ...
                boost::system::error_code ec;
                handle_connection(
                    context, env, conn->socket, timeout, conn->buffer, false, pause_mutex, scheduler.get(), ec, yield);
