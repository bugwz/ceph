...
using timeout_timer = rgw::basic_timeout_timer<ceph::coarse_mono_clock, executor_type, Connection>;
...
template<typename Stream> class StreamIO : public rgw::asio::ClientIO
{
...
public:
    StreamIO(CephContext* cct, Stream& stream, timeout_timer& timeout, rgw::asio::parser_type& parser,
             yield_context yield, parse_buffer& buffer, bool is_ssl, const tcp::endpoint& local_endpoint,
    ...
    size_t write_data(const char* buf, size_t len) override
    {
        ...
        timeout.start();
        auto bytes = boost::asio::async_write(stream, boost::asio::buffer(buf, len), yield[ec]);
        timeout.cancel();
        if (ec) {
...
struct http_version
{
    ...
    explicit http_version(unsigned version)
        : major_ver(version / 10)
        , minor_ver(version % 10)
    {}
};
...
struct log_header
{
    ...
    log_header(const http::fields& fields, http::field field, std::string_view quote = "")
        : fields(fields)
        , field(field)
        , quote(quote)
    {}
...
struct log_ms_remainder
{
    ...
    log_ms_remainder(ceph::coarse_real_time t)
        : t(t)
    {}
};
...
{
    ceph::coarse_real_time t;
    log_apache_time(ceph::coarse_real_time t)
        : t(t)
    {}
};
std::ostream& operator<<(std::ostream& out, const log_apache_time& a)
{
    ...
    return out << std::put_time(local, "%d/%b/%Y:%T.") << log_ms_remainder{a.t} << std::put_time(local, " %z");
};

using SharedMutex = ceph::async::SharedMutex<boost::asio::io_context::executor_type>;

template<typename Stream>
void handle_connection(boost::asio::io_context& context, RGWProcessEnv& env, Stream& stream, timeout_timer& timeout,
                       size_t header_limit, parse_buffer& buffer, bool is_ssl, SharedMutex& pause_mutex,
                       rgw::dmclock::Scheduler* scheduler, boost::system::error_code& ec, yield_context yield)
{
    ...
    for (;;) {
...

        {
            ...
            StreamIO real_client{cct, stream, timeout, parser, yield, buffer, is_ssl, local_endpoint, remote_endpoint};

            auto real_client_io = rgw::io::add_reordering(
                rgw::io::add_buffering(cct, rgw::io::add_chunking(rgw::io::add_conlen_controlling(&real_client))));
            ...
            if (cct->_conf->subsys.should_gather(dout_subsys, 1)) {
                ...
                ldout(cct, 1) << "beast: " << std::hex << &req << std::dec << ": " << remote_endpoint.address() << " - "
                              << user << " [" << log_apache_time{started} << "] \"" << message.method_string() << ' '
                              << message.target() << ' ' << http_version{message.version()} << "\" " << http_ret << ' '
                              ...
                              << log_header{message, http::field::range} << " latency=" << latency << dendl;
            }
...
struct Connection : boost::intrusive::list_base_hook<>, boost::intrusive_ref_counter<Connection>
{
    tcp_socket socket;
    parse_buffer buffer;
    ...
    void close(boost::system::error_code& ec) { socket.close(ec); }
...
public:
    ...
    public:
        Guard(ConnectionList* list, Connection* conn)
            : list(list)
            , conn(conn)
        {}
...
class AsioFrontend
{
    ...
    int get_config_key_val(string name, const string& type, bufferlist* pbl);
...
public:
    AsioFrontend(const RGWProcessEnv& env, RGWFrontendConfig* conf, dmc::SchedulerCtx& sched_ctx)
        : env(env)
        , conf(conf)
        , pause_mutex(context.get_executor())
    {
        ...
        case dmc::scheduler_t::none:
            ...
            [[fallthrough]];
        case dmc::scheduler_t::throttler: scheduler.reset(new dmc::SimpleThrottler(ctx()));
        }
    }
...
tcp::endpoint parse_endpoint(boost::asio::string_view input, unsigned short default_port, boost::system::error_code& ec)
{
    tcp::endpoint endpoint;
...
static int drop_privileges(CephContext* ctx)
{
    ...
    if (uid && gid) {
        ldout(ctx, 0) << "set uid:gid to " << uid << ":" << gid << " (" << uid_string << ":" << gid_string << ")"
                      << dendl;
    }
    return 0;
}
...
int AsioFrontend::init()
{
    ...
    if (timeout != config.end()) {
        ...
        if (timeout_number) {
            ...
            lderr(ctx()) << "WARNING: invalid value for request_timeout_ms: " << timeout->second
                         << " setting it to the default value: " << REQUEST_TIMEOUT << dendl;
        }
    }
    ...
    if (max_header_size != config.end()) {
        ...
        if (!limit) {
            lderr(ctx()) << "WARNING: invalid value for max_header_size: " << max_header_size->second
                         << ", using the default value: " << header_limit << dendl;
        }
        else if (*limit > parse_buffer_size) {   // can't exceed parse buffer size
            header_limit = parse_buffer_size;
            lderr(ctx()) << "WARNING: max_header_size " << max_header_size->second << " capped at maximum value "
                         << header_limit << dendl;
        }
        else {
            header_limit = *limit;
    ...
    for (auto& l : listeners) {
        ...
        if (ec) {
            if (ec == boost::asio::error::address_family_not_supported) {
                ldout(ctx(), 0) << "WARNING: cannot open socket for endpoint=" << l.endpoint << ", " << ec.message()
                                << dendl;
                continue;
            }
        ...
        if (l.endpoint.protocol() == tcp::v6()) {
            ...
            if (ec) {
                lderr(ctx()) << "failed to set v6_only socket option: " << ec.message() << dendl;
                return -ec.value();
            }
        }
        ...
        if (ec) {
            lderr(ctx()) << "failed to bind address " << l.endpoint << ": " << ec.message() << dendl;
            return -ec.value();
        }
...
string ExpandMetaVar::process_str(const string& in)
{
    ...
    while (pos != std::string::npos) {
        ...
        if (in[pos + 1] == '{') {
            ...
            if (endpos != std::string::npos && in[endpos] == '}') {
...
int AsioFrontend::get_config_key_val(string name, const string& type, bufferlist* pbl)
{
    if (name.empty()) {
...
int AsioFrontend::ssl_set_private_key(const string& name, bool is_ssl_certificate)
{
    ...
    if (!boost::algorithm::starts_with(name, config_val_prefix)) {
        ...
        bufferlist bl;
        int r = get_config_key_val(name.substr(config_val_prefix.size()), "ssl_private_key", &bl);
        if (r < 0) {
            return r;
        }
        ssl_context->use_private_key(boost::asio::buffer(bl.c_str(), bl.length()), ssl::context::pem, ec);
    }

    if (ec) {
        if (!is_ssl_certificate) {
            lderr(ctx()) << "failed to add ssl_private_key=" << name << ": " << ec.message() << dendl;
        }
        else {
            lderr(ctx()) << "failed to use ssl_certificate=" << name << " as a private key: " << ec.message() << dendl;
        }
        return -ec.value();
    }
...
int AsioFrontend::ssl_set_certificate_chain(const string& name)
{
    ...
    if (!boost::algorithm::starts_with(name, config_val_prefix)) {
        ...
        bufferlist bl;
        int r = get_config_key_val(name.substr(config_val_prefix.size()), "ssl_certificate", &bl);
        if (r < 0) {
            return r;
        }
        ssl_context->use_certificate_chain(boost::asio::buffer(bl.c_str(), bl.length()), ec);
    }

    if (ec) {
        lderr(ctx()) << "failed to use ssl_certificate=" << name << ": " << ec.message() << dendl;
        return -ec.value();
    }
...
int AsioFrontend::init_ssl()
{
    ...
    if (ciphers) {
        ...
        int r = SSL_CTX_set_cipher_list(ssl_context->native_handle(), ciphers->c_str());
        if (r == 0) {
            lderr(ctx()) << "no cipher could be selected from ssl_ciphers: " << *ciphers << dendl;
            return -EINVAL;
        }
    }
     ...
     */
    if (ports.first == ports.second && endpoints.first == endpoints.second) {
        return 0;
    }
    ...
    if (l.use_ssl) {
        spawn::spawn(
            ...
            [this, s = std::move(stream)](yield_context yield) mutable {
                ...
                boost::system::error_code ec;
                timeout.start();
                auto bytes = stream.async_handshake(ssl::stream_base::server, conn->buffer.data(), yield[ec]);
                timeout.cancel();
                if (ec) {
...
void AsioFrontend::unpause(rgw::sal::RGWRadosStore* const store, rgw_auth_registry_ptr_t auth_registry)
{
    env.store = store;
...
{
public:
    Impl(const RGWProcessEnv& env, RGWFrontendConfig* conf, rgw::dmclock::SchedulerCtx& sched_ctx)
        : AsioFrontend(env, conf, sched_ctx)
    {}
};

RGWAsioFrontend::RGWAsioFrontend(const RGWProcessEnv& env, RGWFrontendConfig* conf,
...
void RGWAsioFrontend::unpause_with_new_config(rgw::sal::RGWRadosStore* const store,
