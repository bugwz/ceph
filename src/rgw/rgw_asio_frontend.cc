...
void handle_connection(boost::asio::io_context& context, RGWProcessEnv& env, Stream& stream, timeout_timer& timeout,
                       ...
                       rgw::dmclock::Scheduler* scheduler, const std::string& uri_prefix, boost::system::error_code& ec,
                       yield_context yield)
{
    ...
    for (;;) {
...

        {
            ...
            process_request(env, &req, uri_prefix, &client, y, scheduler, &user, &latency, &http_ret);

            if (cct->_conf->subsys.should_gather(ceph_subsys_rgw_access, 1)) {
                // access log line elements begin per Apache Combined Log Format with additions following
                lsubdout(cct, rgw_access, 1)
                    << "beast: " << std::hex << &req << std::dec << ": " << remote_endpoint.address() << " - " << user
                    << " [" << log_apache_time{started} << "] \"" << message.method_string() << ' ' << message.target()
...
public:
    AsioFrontend(RGWProcessEnv& env, RGWFrontendConfig* conf, dmc::SchedulerCtx& sched_ctx)
        : env(env)
        , conf(conf)
...
{
public:
    Impl(RGWProcessEnv& env, RGWFrontendConfig* conf, rgw::dmclock::SchedulerCtx& sched_ctx)
        : AsioFrontend(env, conf, sched_ctx)
    {}
};

RGWAsioFrontend::RGWAsioFrontend(RGWProcessEnv& env, RGWFrontendConfig* conf, rgw::dmclock::SchedulerCtx& sched_ctx)
