...
int aws_response_handler::create_header_records()
{
    ...
    // 2
    push_header(header_name_str[IDX(header_name_En::CONTENT_TYPE)],
                header_value_str[IDX(header_value_En::OCTET_STREAM)]);
    // 3
...
int aws_response_handler::create_message(u_int32_t header_len)
{
    ...
    // s3select result is produced into sql_result, the sql_result is also the response-message, thus the attach headers
    // and CRC are created later to the produced SQL result, and actually wrapping the payload.
...
{
    std::string progress_payload =
        fmt::format("<?xml version=\"1.0\" "
                    "encoding=\"UTF-8\"?><Progress><BytesScanned>{}</BytesScanned><BytesProcessed>{}</"
                    "BytesProcessed><BytesReturned>{}</BytesReturned></Progress>",
                    get_processed_size(),
                    get_processed_size(),
                    get_total_bytes_returned());
...
void aws_response_handler::send_stats_response()
{
    std::string stats_payload =
        fmt::format("<?xml version=\"1.0\" "
                    "encoding=\"UTF-8\"?><Stats><BytesScanned>{}</BytesScanned><BytesProcessed>{}</"
                    "BytesProcessed><BytesReturned>{}</BytesReturned></Stats>",
                    get_processed_size(),
                    get_processed_size(),
                    get_total_bytes_returned());
    ...
    , m_is_trino_request(false)
{
    ...
    fp_debug_mesg = [&](const char* mesg) { ldpp_dout(this, 10) << mesg << dendl; };
...
int RGWSelectObj_ObjStore_S3::run_s3select_on_csv(const char* query, const char* input, size_t input_length)
{
    ...
    if (s3select_syntax.get_error_description().empty() == false) {
        ...
        m_aws_response_handler.send_error_response(
            s3select_syntax_error, s3select_syntax.get_error_description().c_str(), s3select_resource_id);
        ldpp_dout(this, 10) << "s3-select query: failed to prase the following query {" << query << "}" << dendl;
        ldpp_dout(this, 10) << "s3-select query: syntax-error {" << s3select_syntax.get_error_description() << "}"
                            << dendl;
        return -1;
    }
    else {
        ...
        status = m_s3_csv_object.run_s3select_on_stream(
            m_aws_response_handler.get_sql_result(), input, input_length, m_object_size_for_processing);
        ...
        if (status < 0) {
            ...
            m_aws_response_handler.send_error_response(
                s3select_processTime_error, m_s3_csv_object.get_error_description().c_str(), s3select_resource_id);
            ldpp_dout(this, 10) << "s3-select query: failed to process query; {"
                                << m_s3_csv_object.get_error_description() << "}" << dendl;
            return -1;
        }
    }
    if ((length_post_processing - length_before_processing) != 0) {
        ldpp_dout(this, 10) << "s3-select: sql-result-size = " << m_aws_response_handler.get_sql_result().size()
                            << dendl;
    }
    else {
...
int RGWSelectObj_ObjStore_S3::run_s3select_on_parquet(const char* query)
{
    ...
    if (s3select_syntax.get_error_description().empty() == false) {
        ...
        fp_result_header_format(m_aws_response_handler.get_sql_result());
        // at this stage the Parquet-processing "takes control", it keep calling to s3-range-request according to the
        // SQL statement.
        status = m_s3_parquet_object.run_s3select_on_object(
            m_aws_response_handler.get_sql_result(), fp_s3select_result_format, fp_result_header_format);
        if (status < 0) {
            m_aws_response_handler.get_sql_result().append(m_s3_parquet_object.get_error_description());
            fp_s3select_result_format(m_aws_response_handler.get_sql_result());
            ldout(s->cct, 10) << "S3select: failure while execution" << m_s3_parquet_object.get_error_description()
                              << dendl;
        }
    }
#endif
...
int RGWSelectObj_ObjStore_S3::run_s3select_on_json(const char* query, const char* input, size_t input_length)
{
    ...
    if (m_json_datatype.compare("DOCUMENT") != 0) {
        ...
        m_aws_response_handler.send_error_response(s3select_json_error, s3select_json_error_msg, s3select_resource_id);
    ...
    // query is correct(syntax), processing is starting.
    try {
        status = m_s3_json_object.run_s3select_on_stream(
            m_aws_response_handler.get_sql_result(), input, input_length, m_object_size_for_processing);
    }
    catch (base_s3select_exception& e) {
        ldpp_dout(this, 10) << "S3select: failed to process JSON object: " << e.what() << dendl;
        m_aws_response_handler.get_sql_result().append(e.what());
        m_aws_response_handler.send_error_response(s3select_processTime_error, e.what(), s3select_resource_id);
        return -EINVAL;
    }
    ...
    if (status < 0) {
        // error flow(processing-time)
        m_aws_response_handler.send_error_response(
            s3select_processTime_error, m_s3_json_object.get_error_description().c_str(), s3select_resource_id);
        ldpp_dout(this, 10) << "s3-select query: failed to process query; {" << m_s3_json_object.get_error_description()
                            << "}" << dendl;
        return -EINVAL;
    }
    fp_chunked_transfer_encoding();
...
int RGWSelectObj_ObjStore_S3::range_request(int64_t ofs, int64_t len, void* buff, optional_yield y)
{
    ...
    m_request_range = len;
    ldout(s->cct, 10) << "S3select: calling execute(async):"
                      << " request-offset :" << ofs << " request-length :" << len
                      << " buffer size : " << requested_buffer.size() << dendl;
    RGWGetObj::execute(y);
    if (buff) {
...
void RGWSelectObj_ObjStore_S3::execute(optional_yield y)
{
    ...
    if (m_parquet_type) {
        ...
        if (status) {
            ldout(s->cct, 10) << "S3select: failed to process query <" << m_sql_query << "> on object "
                              << s->object->get_name() << dendl;
            op_ret = -ERR_INVALID_REQUEST;
        }
        else {
        ...
        if (m_scan_range_ind) {
            ...
            if (m_is_trino_request) {
                ...
                // assumption: row is smaller than m_scan_offset. (a different approach is to request for additional
                // range)
...
int RGWSelectObj_ObjStore_S3::parquet_processing(bufferlist& bl, off_t ofs, off_t len)
{
    ...
    if (requested_buffer.size() < m_request_range) {
        ldout(s->cct, 10) << "S3select: need another round buffe-size: " << requested_buffer.size()
                          << " request range length:" << m_request_range << dendl;
        return 0;
    }
    else {   // buffer is complete
        ldout(s->cct, 10) << "S3select: buffer is complete " << requested_buffer.size()
                          << " request range length:" << m_request_range << dendl;
        m_request_range = 0;
    }
    return 0;
    ...
    if (m_scan_range_ind) {   // only upon range-scan
        ...
        if ((m_aws_response_handler.get_processed_size() + len) >= m_requested_range) {
            ...
            for (sc = start; sc < len; sc++)   // assumption : row-delimiter must exist or its end ebject
            {
                ...
                if (!strncmp(row_delimiter, p, 1)) {
                    ldout(s->cct, 10) << "S3select: found row-delimiter on " << sc
                                      << " get_processed_size = " << m_aws_response_handler.get_processed_size()
                                      << dendl;
                    len = sc + 1;   //+1 is for delimiter.  TODO what about m_object_size_for_processing (to update
                                    // according to len)
...
int RGWSelectObj_ObjStore_S3::csv_processing(bufferlist& bl, off_t ofs, off_t len)
{
    ...
    if (s->obj_size == 0 || m_object_size_for_processing == 0) {
        ...
        for (auto& it : bl.buffers()) {
            ...
            if ((ofs + len) > it.length()) {
                ldpp_dout(this, 10) << "offset and length may cause invalid read: ofs = " << ofs << " len = " << len
                                    << " it.length() = " << it.length() << dendl;
                ofs = 0;
                len = it.length();
            }
            ...
            ldpp_dout(this, 10) << "s3select: chunk:  ofs = " << ofs << " len = " << len
                                << " it.length() = " << it.length()
    ...
    ldpp_dout(this, 10) << "s3select : m_aws_response_handler.get_processed_size() "
                        << m_aws_response_handler.get_processed_size() << " m_object_size_for_processing "
    ...
    if (m_aws_response_handler.get_processed_size() >= uint64_t(m_object_size_for_processing) ||
        ...
        if (m_s3_csv_object.is_sql_limit_reached()) {
            ...
            ldpp_dout(this, 10) << "s3select : reached the limit :" << m_aws_response_handler.get_processed_size()
                                << dendl;
            status = -ENOENT;
        }
    }
...
int RGWSelectObj_ObjStore_S3::json_processing(bufferlist& bl, off_t ofs, off_t len)
{
    ...
    if (s->obj_size == 0 || m_object_size_for_processing == 0) {
        // in case of empty object the s3select-function returns a correct "empty" result(for aggregation and
        ...
        int i = 0;
        for (auto& it : bl.buffers()) {
            ldpp_dout(this, 10) << "processing segment " << i << " out of " << bl_len << " off " << ofs << " len "
                                << len << " obj-size " << m_object_size_for_processing << dendl;
            ...
            if ((ofs + len) > it.length()) {
                ldpp_dout(this, 10) << "s3select: offset and length may cause invalid read: ofs = " << ofs
                                    << " len = " << len << " it.length() = " << it.length() << dendl;
                ofs = 0;
                len = it.length();
            }
    ...
    if (status >= 0 && (m_aws_response_handler.get_processed_size() == uint64_t(m_object_size_for_processing) ||
                        m_s3_json_object.is_sql_limit_reached())) {
