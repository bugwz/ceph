...
int Credentials::generateCredentials(
    const DoutPrefixProvider* dpp, CephContext* cct, const uint64_t& duration,
    const boost::optional<std::string>& policy, const boost::optional<std::string>& roleId,
    const boost::optional<std::string>& role_session, const boost::optional<std::vector<std::string>>& token_claims,
    const boost::optional<std::vector<std::pair<std::string, std::string>>>& session_princ_tags,
    boost::optional<rgw_user> user, rgw::auth::Identity* identity)
{
...
int AssumedRoleUser::generateAssumedRoleUser(CephContext* cct, rgw::sal::Driver* driver, const string& roleId,
                                             const rgw::ARN& roleArn, const string& roleSessionName)
{
    ...
    rgw::ARN assumed_role_arn(rgw::Partition::aws, rgw::Service::sts, "", roleArn.account, resource);
...
AssumeRoleRequestBase::AssumeRoleRequestBase(CephContext* cct, const string& duration, const string& iamPolicy,
                                             const string& roleArn, const string& roleSessionName)
    : cct(cct)
    , iamPolicy(iamPolicy)
    , roleArn(roleArn)
...
int AssumeRoleRequestBase::validate_input(const DoutPrefixProvider* dpp) const
{
    ...
    if (!iamPolicy.empty() && (iamPolicy.size() < MIN_POLICY_SIZE || iamPolicy.size() > MAX_POLICY_SIZE)) {
        ldpp_dout(dpp, 0) << "ERROR: Incorrect size of iamPolicy: " << iamPolicy.size() << dendl;
        return -ERR_PACKED_POLICY_TOO_LARGE;
    }

    if (!roleArn.empty() && (roleArn.size() < MIN_ROLE_ARN_SIZE || roleArn.size() > MAX_ROLE_ARN_SIZE)) {
    ...
    if (!roleSessionName.empty()) {
        if (roleSessionName.size() < MIN_ROLE_SESSION_SIZE || roleSessionName.size() > MAX_ROLE_SESSION_SIZE) {
            ldpp_dout(dpp, 0) << "ERROR: Either role session name is empty or role session size is incorrect: "
...
{
    if (!providerId.empty()) {
        if (providerId.length() < MIN_PROVIDER_ID_LEN || providerId.length() > MAX_PROVIDER_ID_LEN) {
            ldpp_dout(dpp, 0) << "ERROR: Either provider id is empty or provider id length is incorrect: "
...
{
    if (!externalId.empty()) {
        if (externalId.length() < MIN_EXTERNAL_ID_LEN || externalId.length() > MAX_EXTERNAL_ID_LEN) {
            ldpp_dout(dpp, 0) << "ERROR: Either external id is empty or external id length is incorrect: "
    ...
    if (!serialNumber.empty()) {
        if (serialNumber.size() < MIN_SERIAL_NUMBER_SIZE || serialNumber.size() > MAX_SERIAL_NUMBER_SIZE) {
            ldpp_dout(dpp, 0) << "Either serial number is empty or serial number length is incorrect: "
...
std::tuple<int, rgw::sal::RGWRole*> STSService::getRoleInfo(const DoutPrefixProvider* dpp, const string& arn,
                                                            optional_yield y)
{
    if (auto r_arn = rgw::ARN::parse(arn); r_arn) {
        ...
        if (int ret = role->get(dpp, y); ret < 0) {
            ...
            if (path != r_path) {
                ldpp_dout(dpp, 0) << "Invalid Role ARN: Path in ARN does not match with the role path: " << path << " "
                                  << r_path << dendl;
...
AssumeRoleWithWebIdentityResponse STSService::assumeRoleWithWebIdentity(const DoutPrefixProvider* dpp,
                                                                        AssumeRoleWithWebIdentityRequest& req)
{
    ...
    response.assumeRoleResp.retCode = response.assumeRoleResp.user.generateAssumedRoleUser(
    ...
    response.assumeRoleResp.retCode = response.assumeRoleResp.creds.generateCredentials(dpp,
                                                                                        cct,
                                                                                        req.getDuration(),
                                                                                        req.getPolicy(),
                                                                                        roleId,
...
AssumeRoleResponse STSService::assumeRole(const DoutPrefixProvider* dpp, AssumeRoleRequest& req, optional_yield y)
{
    ...
    // Generate Assumed Role User
    response.retCode =
        response.user.generateAssumedRoleUser(cct, driver, roleId, r_arn.get(), req.getRoleSessionName());
    if (response.retCode < 0) {
        return response;
    }
    ...
    // Generate Credentials
    // Role and Policy provide the authorization info, user id and applier info are not needed
    response.retCode = response.creds.generateCredentials(dpp,
                                                          cct,
                                                          req.getDuration(),
                                                          req.getPolicy(),
                                                          roleId,
...
GetSessionTokenRequest::GetSessionTokenRequest(const string& duration, const string& serialNumber,
...
GetSessionTokenResponse STSService::getSessionToken(const DoutPrefixProvider* dpp, GetSessionTokenRequest& req)
{
    ...
    // Generate Credentials
    if (ret = cred.generateCredentials(dpp,
                                       cct,
                                       req.getDuration(),
                                       boost::none,
                                       boost::none,
