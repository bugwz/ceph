...
int Credentials::generateCredentials(
    CephContext* cct, const uint64_t& duration, const boost::optional<std::string>& policy,
    const boost::optional<std::string>& roleId, const boost::optional<std::string>& role_session,
...
int AssumedRoleUser::generateAssumedRoleUser(CephContext* cct, rgw::sal::RGWRadosStore* store, const string& roleId,
...
int AssumeRoleRequestBase::validate_input() const
{
    ...
    if (!roleSessionName.empty()) {
        if (roleSessionName.size() < MIN_ROLE_SESSION_SIZE || roleSessionName.size() > MAX_ROLE_SESSION_SIZE) {
            ldout(cct, 0) << "ERROR: Either role session name is empty or role session size is incorrect: "
...
int AssumeRoleWithWebIdentityRequest::validate_input() const
{
    if (!providerId.empty()) {
        if (providerId.length() < MIN_PROVIDER_ID_LEN || providerId.length() > MAX_PROVIDER_ID_LEN) {
            ldout(cct, 0) << "ERROR: Either provider id is empty or provider id length is incorrect: "
...
int AssumeRoleRequest::validate_input() const
{
    if (!externalId.empty()) {
        if (externalId.length() < MIN_EXTERNAL_ID_LEN || externalId.length() > MAX_EXTERNAL_ID_LEN) {
            ldout(cct, 0) << "ERROR: Either external id is empty or external id length is incorrect: "
    ...
    if (!serialNumber.empty()) {
        if (serialNumber.size() < MIN_SERIAL_NUMBER_SIZE || serialNumber.size() > MAX_SERIAL_NUMBER_SIZE) {
            ldout(cct, 0) << "Either serial number is empty or serial number length is incorrect: "
...
std::tuple<int, RGWRole> STSService::getRoleInfo(const DoutPrefixProvider* dpp, const string& arn, optional_yield y)
{
...
int STSService::storeARN(const DoutPrefixProvider* dpp, string& arn, optional_yield y)
{
    ...
    if (ret = rgw_store_user_info(dpp, store->ctl()->user, info, &info, &objv_tracker, real_time(), false, y);
        ret < 0) {
...
AssumeRoleWithWebIdentityResponse STSService::assumeRoleWithWebIdentity(AssumeRoleWithWebIdentityRequest& req)
{
    ...
    response.assumeRoleResp.retCode =
        response.assumeRoleResp.user.generateAssumedRoleUser(cct, store, roleId, r_arn.get(), req.getRoleSessionName());
    if (response.assumeRoleResp.retCode < 0) {
        return response;
    }
    ...
    response.assumeRoleResp.retCode = response.assumeRoleResp.creds.generateCredentials(cct,
                                                                                        req.getDuration(),
                                                                                        req.getPolicy(),
                                                                                        roleId,
...
AssumeRoleResponse STSService::assumeRole(const DoutPrefixProvider* dpp, AssumeRoleRequest& req, optional_yield y)
{
    ...
    // Generate Credentials
    // Role and Policy provide the authorization info, user id and applier info are not needed
    response.retCode = response.creds.generateCredentials(cct,
                                                          req.getDuration(),
                                                          req.getPolicy(),
                                                          roleId,
                                                          req.getRoleSessionName(),
...
GetSessionTokenResponse STSService::getSessionToken(GetSessionTokenRequest& req)
{
    ...
    if (ret = cred.generateCredentials(
            cct, req.getDuration(), boost::none, boost::none, boost::none, boost::none, boost::none, user_id, identity);
        ret < 0) {
