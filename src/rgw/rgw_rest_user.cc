...
int fetch_access_keys_from_master(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                  RGWUserAdminOpState& op_state, req_state* s, optional_yield y)
{
    bufferlist data;
    JSONParser jp;
    RGWUserInfo ui;
...
public:
    ...
    int check_caps(const RGWUserCaps& caps) override { return caps.check_cap("users", RGW_CAP_READ); }
...
public:
    ...
    int check_caps(const RGWUserCaps& caps) override { return caps.check_cap("users", RGW_CAP_WRITE); }
...
void RGWOp_User_Create::execute(optional_yield y)
{
    ...
    int32_t max_buckets;
    const int32_t default_max_buckets = s->cct->_conf.get_val<int64_t>("rgw_user_max_buckets");
    ...
    if (s->info.args.exists("suspended")) op_state.set_suspension(suspended);

    if (s->info.args.exists("system")) op_state.set_system(system);

    if (s->info.args.exists("exclusive")) op_state.set_exclusive(exclusive);
...
void RGWOp_User_Modify::execute(optional_yield y)
{
    ...
    if (email_set) op_state.set_user_email(email);
...
void RGWOp_User_Remove::execute(optional_yield y)
{
    ...
    if (!uid.empty()) op_state.set_user_id(uid);
...
void RGWOp_Subuser_Create::execute(optional_yield y)
{
    ...
    if (gen_access) op_state.set_gen_access();

    if (gen_secret) op_state.set_gen_secret();
...
void RGWOp_Subuser_Modify::execute(optional_yield y)
{
    ...
    if (!secret_key.empty()) op_state.set_secret_key(secret_key);
...
void RGWOp_Subuser_Remove::execute(optional_yield y)
{
    ...
    if (purge_keys) op_state.set_purge_keys();
...
void RGWOp_Key_Create::execute(optional_yield y)
{
    ...
    if (gen_key) op_state.set_generate_key();
...
{
    if (s->info.args.sub_resource_exists("quota")) return new RGWOp_Quota_Info;

    if (s->info.args.sub_resource_exists("list")) return new RGWOp_User_List;
...
{
    if (s->info.args.sub_resource_exists("subuser")) return new RGWOp_Subuser_Create;

    if (s->info.args.sub_resource_exists("key")) return new RGWOp_Key_Create;

    if (s->info.args.sub_resource_exists("caps")) return new RGWOp_Caps_Add;

    if (s->info.args.sub_resource_exists("quota")) return new RGWOp_Quota_Set;
...
{
    if (s->info.args.sub_resource_exists("subuser")) return new RGWOp_Subuser_Modify;
...
{
    if (s->info.args.sub_resource_exists("subuser")) return new RGWOp_Subuser_Remove;

    if (s->info.args.sub_resource_exists("key")) return new RGWOp_Key_Remove;

    if (s->info.args.sub_resource_exists("caps")) return new RGWOp_Caps_Remove;
