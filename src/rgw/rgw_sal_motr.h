...
public:
    ...
    // with Hua.
    int distribute_cache(const DoutPrefixProvider* dpp, const std::string& normal_name, ObjectCacheInfo& obj_info,
                         int op);
    int watch_cb(const DoutPrefixProvider* dpp, uint64_t notify_id, uint64_t cookie, uint64_t notifier_id,
                 bufferlist& bl);

    void set_enabled(bool status);

    MotrMetaCache(const DoutPrefixProvider* dpp, CephContext* cct) { cache.set_ctx(cct); }
};
...
public:
    ...
    MotrUser(MotrStore* _st, const rgw_user& _u)
        : StoreUser(_u)
        , store(_st)
    {}
    MotrUser(MotrStore* _st, const RGWUserInfo& _i)
        : StoreUser(_i)
        , store(_st)
    {}
    ...
    virtual std::unique_ptr<User> clone() override { return std::unique_ptr<User>(new MotrUser(*this)); }
    ...
    int load_user_from_idx(const DoutPrefixProvider* dpp, MotrStore* store, RGWUserInfo& info,
                           std::map<std::string, bufferlist>* attrs, RGWObjVersionTracker* objv_tr);
...
public:
    ...
    virtual std::unique_ptr<Bucket> clone() override { return std::make_unique<MotrBucket>(*this); }
...
public:
    MotrPlacementTier(MotrStore* _store, const RGWZoneGroupPlacementTier& _tier)
        : store(_store)
        , tier(_tier)
    {}
...
public:
    MotrZoneGroup(MotrStore* _store)
        : store(_store)
        , group()
    {}
    MotrZoneGroup(MotrStore* _store, const RGWZoneGroup& _group)
        : store(_store)
        , group(_group)
    {}
    ...
    virtual int get_zone_by_id(const std::string& id, std::unique_ptr<Zone>* zone) override { return -1; }
    virtual int get_zone_by_name(const std::string& name, std::unique_ptr<Zone>* zone) override { return -1; }
    ...
    bool supports(std::string_view feature) const override { return group.supports(features); }
    virtual std::unique_ptr<ZoneGroup> clone() override { return std::make_unique<MotrZoneGroup>(store, group); }
};
...
public:
    ...
    MotrZone(MotrStore* _store, MotrZoneGroup _zg)
        : store(_store)
        , zonegroup(_zg)
    {
        realm = new RGWRealm();
    ...
    virtual std::unique_ptr<Zone> clone() override { return std::make_unique<MotrZone>(store); }
...
public:
    MotrOIDCProvider(MotrStore* _store)
        : store(_store)
    {}
...
public:
    ...
    public:
        ...
        virtual int read(int64_t off, int64_t end, bufferlist& bl, optional_yield y,
                         const DoutPrefixProvider* dpp) override;
        virtual int iterate(const DoutPrefixProvider* dpp, int64_t off, int64_t end, RGWGetDataCB* cb,
                            optional_yield y) override;
    ...
    virtual std::unique_ptr<Object> clone() override { return std::unique_ptr<Object>(new MotrObject(*this)); }
...
private:
    // int read_attrs(const DoutPrefixProvider* dpp, Motr::Object::Read &read_op, optional_yield y, rgw_obj* target_obj
    // = nullptr);
...
public:
    ...
    int get_part_objs(const DoutPrefixProvider* dpp, std::map<int, std::unique_ptr<MotrObject>>& part_objs);
    int open_part_objs(const DoutPrefixProvider* dpp, std::map<int, std::unique_ptr<MotrObject>>& part_objs);
    int read_multipart_obj(const DoutPrefixProvider* dpp, int64_t off, int64_t end, RGWGetDataCB* cb,
                           std::map<int, std::unique_ptr<MotrObject>>& part_objs);
...
public:
    MotrAtomicWriter(const DoutPrefixProvider* dpp, optional_yield y, rgw::sal::Object* obj, MotrStore* _store,
                     const rgw_user& _owner, const rgw_placement_rule* _ptail_placement_rule, uint64_t _olh_epoch,
...
public:
    MotrMultipartWriter(const DoutPrefixProvider* dpp, optional_yield y, MultipartUpload* upload, rgw::sal::Object* obj,
                        MotrStore* _store, const rgw_user& owner, const rgw_placement_rule* ptail_placement_rule,
...
public:
    ...
    virtual const std::string get_name() const override { return "motr"; }
    ...
    int do_idx_op(struct m0_idx*, enum m0_idx_opcode opcode, std::vector<uint8_t>& key, std::vector<uint8_t>& val,
                  bool update = false);

    int do_idx_next_op(struct m0_idx* idx, std::vector<std::vector<uint8_t>>& key_vec,
                       std::vector<std::vector<uint8_t>>& val_vec);
    int next_query_by_name(std::string idx_name, std::vector<std::string>& key_str_vec,
                           std::vector<bufferlist>& val_bl_vec, std::string prefix = "", std::string delim = "");
    ...
    int do_idx_op_by_name(std::string idx_name, enum m0_idx_opcode opcode, std::string key_str, bufferlist& bl,
                          bool update = true);
