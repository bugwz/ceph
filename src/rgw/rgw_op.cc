...
int RGWGetObj::parse_range(void)
{
    ...
    if (pos == string::npos) {
        pos = 0;
        while (isspace(rs[pos])) pos++;
        int end = pos;
        while (isalpha(rs[end])) end++;
        if (strncasecmp(rs.c_str(), "bytes", end - pos) != 0) return 0;
        while (isspace(rs[end])) end++;
        if (rs[end] != '=') return 0;
        rs = rs.substr(end + 1);
    }
    else {
...
static int decode_policy(const DoutPrefixProvider* dpp, CephContext* cct, bufferlist& bl,
...
static int get_user_policy_from_attr(const DoutPrefixProvider* dpp, CephContext* const cct,
                                     rgw::sal::RGWRadosStore* const store, map<string, bufferlist>& attrs,
 ...
 */
int rgw_op_get_bucket_policy_from_attr(const DoutPrefixProvider* dpp, CephContext* cct, rgw::sal::RGWStore* store,
                                       RGWBucketInfo& bucket_info, map<string, bufferlist>& bucket_attrs,
                                       RGWAccessControlPolicy* policy, optional_yield y)
{
...
static int get_obj_policy_from_attr(const DoutPrefixProvider* dpp, CephContext* cct, rgw::sal::RGWStore* store,
                                    RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info,
                                    map<string, bufferlist>& bucket_attrs, RGWAccessControlPolicy* policy,
                                    string* storage_class, rgw::sal::RGWObject* obj, optional_yield y)
{
    bufferlist bl;
    int ret = 0;
...
static boost::optional<Policy> get_iam_policy_from_attr(CephContext* cct, map<string, bufferlist>& attrs,
                                                        const string& tenant)
{
...
static boost::optional<PublicAccessBlockConfiguration> get_public_access_conf_from_attr(
    const map<string, bufferlist>& attrs)
{
    if (auto aiter = attrs.find(RGW_ATTR_PUBLIC_ACCESS); aiter != attrs.end()) {
...
vector<Policy> get_iam_user_policy_from_attr(CephContext* cct, rgw::sal::RGWRadosStore* store,
                                             map<string, bufferlist>& attrs, const string& tenant)
{
...
static int get_obj_head(const DoutPrefixProvider* dpp, struct req_state* s, rgw::sal::RGWObject* obj, bufferlist* pbl)
{
...
static int get_multipart_info(const DoutPrefixProvider* dpp, struct req_state* s, rgw::sal::RGWObject* obj,
...
static int get_multipart_info(const DoutPrefixProvider* dpp, struct req_state* s, const string& meta_oid,
                              multipart_upload_info* upload_info)
{
...
static int read_bucket_policy(const DoutPrefixProvider* dpp, rgw::sal::RGWStore* store, struct req_state* s,
                              RGWBucketInfo& bucket_info, map<string, bufferlist>& bucket_attrs,
                              RGWAccessControlPolicy* policy, rgw_bucket& bucket, optional_yield y)
{
...
static int read_obj_policy(const DoutPrefixProvider* dpp, rgw::sal::RGWStore* store, struct req_state* s,
                           RGWBucketInfo& bucket_info, map<string, bufferlist>& bucket_attrs,
                           RGWAccessControlPolicy* acl, string* storage_class, boost::optional<Policy>& policy,
                           rgw::sal::RGWBucket* bucket, rgw::sal::RGWObject* object, optional_yield y,
                           bool copy_src = false)
{
    string upload_id;
    ...
    int ret = get_obj_policy_from_attr(
        dpp, s->cct, store, *obj_ctx, bucket_info, bucket_attrs, acl, storage_class, object, s->yield);
    if (ret == -ENOENT) {
        ...
        if (bucket_owner.compare(s->user->get_id()) != 0 && !s->auth.identity->is_admin_of(bucket_owner)) {
            auto r = eval_identity_or_session_policies(
                s->iam_user_policies, s->env, *s->auth.identity, rgw::IAM::s3ListBucket, ARN(bucket->get_key()));
            if (r == Effect::Allow) return -ENOENT;
            if (r == Effect::Deny) return -EACCES;
            if (policy) {
            ...
            if (!s->session_policies.empty()) {
                r = eval_identity_or_session_policies(
                    s->session_policies, s->env, *s->auth.identity, rgw::IAM::s3ListBucket, ARN(bucket->get_key()));
                if (r == Effect::Allow) return -ENOENT;
                if (r == Effect::Deny) return -EACCES;
            }
 ...
 * Returns: 0 on success, -ERR# otherwise.
 */
int rgw_build_bucket_policies(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, struct req_state* s,
                              optional_yield y)
{
    int ret = 0;
    ...
    if (!bi.empty()) {
        ...
        ret =
            rgw_bucket_parse_bucket_instance(bi, &s->bucket_name, &s->bucket_instance_id, &s->bucket_instance_shard_id);
        if (ret < 0) {
            return ret;
        }
    }
    ...
    if (!s->src_bucket_name.empty()) {
        ...
        if (ret == 0) {
            ret = src_bucket->load_by_name(
                dpp, s->src_tenant_name, s->src_bucket_name, s->bucket_instance_id, &obj_ctx, s->yield);
        }
        if (ret == 0) {
    ...
    if (!s->bucket_name.empty()) {
        ...
        ret = store->get_bucket(dpp,
                                s->user.get(),
                                rgw_bucket(rgw_bucket_key(s->bucket_tenant, s->bucket_name, s->bucket_instance_id)),
                                &s->bucket,
                                y);
        if (ret < 0) {
            if (ret != -ENOENT) {
                ...
                ldpp_dout(dpp, 0) << "NOTICE: couldn't get bucket from bucket_name (name=" << bucket_log << ")"
                                  << dendl;
                return ret;
            }
        ...
        ret = read_bucket_policy(
            dpp, store, s, s->bucket->get_info(), s->bucket->get_attrs(), s->bucket_acl.get(), s->bucket->get_key(), y);
        acct_acl_user = {
        ...
        if (!store->get_zonegroup().equals(s->bucket->get_info().zonegroup)) {
            ldpp_dout(dpp, 0) << "NOTICE: request for data in a different zonegroup ("
                              << s->bucket->get_info().zonegroup << " != " << store->get_zonegroup().get_id() << ")"
                              << dendl;
            ...
            else if (!s->local_source || (s->op != OP_PUT && s->op != OP_COPY) ||
    ...
    if (s->user_acl) {
        ...
        if (-ENOENT == ret) {
            ...
            s->user_acl->create_default(acct_acl_user.uid, acct_acl_user.display_name);
            ret = 0;
        }
        else if (ret < 0) {
...
static int rgw_iam_remove_objtags(const DoutPrefixProvider* dpp, struct req_state* s, rgw::sal::RGWObject* object,
                                  bool has_existing_obj_tag, bool has_resource_tag)
{
...
void rgw_add_to_iam_environment(rgw::IAM::Environment& e, std::string_view key, std::string_view val)
{
    ...
    if (!key.empty()) e.emplace(key, val);
}

static int rgw_iam_add_tags_from_bl(struct req_state* s, bufferlist& bl, bool has_existing_obj_tag = false,
                                    bool has_resource_tag = false)
{
    ...
    for (const auto& tag : tagset.get_tags()) {
        if (has_existing_obj_tag) rgw_add_to_iam_environment(s->env, "s3:ExistingObjectTag/" + tag.first, tag.second);
        if (has_resource_tag) rgw_add_to_iam_environment(s->env, "s3:ResourceTag/" + tag.first, tag.second);
    }
    return 0;
}

static int rgw_iam_add_objtags(const DoutPrefixProvider* dpp, struct req_state* s, rgw::sal::RGWObject* object,
                               bool has_existing_obj_tag, bool has_resource_tag)
{
    object->set_atomic(s->obj_ctx);
    int op_ret = object->get_obj_attrs(s->obj_ctx, s->yield, dpp);
...
static int rgw_iam_add_objtags(const DoutPrefixProvider* dpp, struct req_state* s, bool has_existing_obj_tag,
                                                         ...
                                                         bool check_obj_exist_tag = true)
{
    ...
    if (iam_policy) {
        ...
        iam_policy_s3_resource_tag = iam_policy->has_partial_conditional(S3_RESOURCE_TAG) ||
                                     iam_policy->has_partial_conditional_value(S3_RUNTIME_RESOURCE_VAL);
    }
    ...
    if (identity_policies) {
        for (auto& identity_policy : identity_policies.get()) {
            if (check_obj_exist_tag) {
                if (identity_policy.has_partial_conditional(S3_EXISTING_OBJTAG)) identity_policy_s3_exist_tag = true;
            }
            if (identity_policy.has_partial_conditional(S3_RESOURCE_TAG) ||
                identity_policy.has_partial_conditional_value(S3_RUNTIME_RESOURCE_VAL))
                identity_policy_s3_resource_tag = true;
            if (identity_policy_s3_exist_tag &&
                identity_policy_s3_resource_tag)   // check all policies till both are set to true
                break;
        }
    }
    ...
    if (session_policies) {
        for (auto& session_policy : session_policies.get()) {
            if (check_obj_exist_tag) {
                if (session_policy.has_partial_conditional(S3_EXISTING_OBJTAG)) session_policy_s3_exist_tag = true;
            }
            if (session_policy.has_partial_conditional(S3_RESOURCE_TAG) ||
                session_policy.has_partial_conditional_value(S3_RUNTIME_RESOURCE_VAL))
                session_policy_s3_resource_flag = true;
            if (session_policy_s3_exist_tag && session_policy_s3_resource_flag) break;
        }
    }
...
static std::tuple<bool, bool> rgw_check_policy_condition(const DoutPrefixProvider* dpp, struct req_state* s,
                                                         bool check_obj_exist_tag = true)
{
    return rgw_check_policy_condition(
        dpp, s->iam_policy, s->iam_user_policies, s->session_policies, check_obj_exist_tag);
}
...
void rgw_build_iam_environment(rgw::sal::RGWRadosStore* store, struct req_state* s)
{
    ...
    {
        if (i != m.end()) s->env.emplace("aws:UserAgent", i->second);
    }
...
{
    if (s->expect_cont) dump_continue(s);
...
namespace {
template<typename F> int retry_raced_bucket_write(const DoutPrefixProvider* dpp, rgw::sal::RGWBucket* b, const F& f)
{
    auto r = f();
...
int RGWOp::verify_op_mask()
{
    ...
    ldpp_dout(this, 20) << "required_mask= " << required_mask << " user.op_mask=" << s->user->get_info().op_mask
                        << dendl;
    ...
    if (!s->system_request && (required_mask & RGW_OP_TYPE_MODIFY) && !store->svc()->zone->zone_is_writeable()) {
        ldpp_dout(this, 5) << "NOTICE: modify request to a read-only zone by a "
                              "non-system user, permission denied"
                           << dendl;
        return -EPERM;
    }
...
{
    auto iam_action =
        s->object->get_instance().empty() ? rgw::IAM::s3GetObjectTagging : rgw::IAM::s3GetObjectVersionTagging;

    auto [has_s3_existing_tag, has_s3_resource_tag] = rgw_check_policy_condition(this, s);
    if (has_s3_existing_tag || has_s3_resource_tag)
        rgw_iam_add_objtags(this, s, has_s3_existing_tag, has_s3_resource_tag);
    if (!verify_object_permission(this, s, iam_action)) return -EACCES;
...
{
    auto iam_action =
        s->object->get_instance().empty() ? rgw::IAM::s3PutObjectTagging : rgw::IAM::s3PutObjectVersionTagging;
    ...
    if (has_s3_existing_tag) rgw_iam_add_objtags(this, s, true, false);
    if (has_s3_resource_tag) rgw_iam_add_buckettags(this, s);
    if (!verify_object_permission(this, s, iam_action)) return -EACCES;
    return 0;
}
...
{
    if (!rgw::sal::RGWObject::empty(s->object.get())) {
        auto iam_action = s->object->get_instance().empty() ? rgw::IAM::s3DeleteObjectTagging
...
{
    if (rgw::sal::RGWObject::empty(s->object.get())) return;
...
void RGWPutBucketReplication::execute(optional_yield y)
{
    ...
    op_ret = retry_raced_bucket_write(this, s->bucket.get(), [this] {
        auto sync_policy =
            (s->bucket->get_info().sync_policy ? *s->bucket->get_info().sync_policy : rgw_sync_policy_info());
        ...
        if (ret < 0) {
            ldpp_dout(this, 0) << "ERROR: put_bucket_instance_info (bucket=" << s->bucket << ") returned ret=" << ret
                               << dendl;
            return ret;
        }
...
int RGWOp::init_quota()
{
    ...
    if (rgw::sal::RGWBucket::empty(s->bucket.get()) || rgw::sal::RGWObject::empty(s->object.get())) {
        return 0;
    }
...
static bool validate_cors_rule_method(RGWCORSRule* rule, const char* req_meth)
{
    ...
    if (strcmp(req_meth, "GET") == 0)
        flags = RGW_CORS_GET;
    else if (strcmp(req_meth, "POST") == 0)
        flags = RGW_CORS_POST;
    else if (strcmp(req_meth, "PUT") == 0)
        flags = RGW_CORS_PUT;
    else if (strcmp(req_meth, "DELETE") == 0)
        flags = RGW_CORS_DELETE;
    else if (strcmp(req_meth, "HEAD") == 0)
        flags = RGW_CORS_HEAD;
 ...
 * */
static void get_cors_response_headers(RGWCORSRule* rule, const char* req_hdrs, string& hdrs, string& exp_hdrs,
                                      unsigned* max_age)
{
    if (req_hdrs) {
        list<string> hl;
 ...
 */
bool RGWOp::generate_cors_headers(string& origin, string& method, string& headers, string& exp_headers,
                                  unsigned* max_age)
{
    /* CORS 6.2.1. */
    ...
    if (!authorization && rule->has_wildcard_origin()) origin = "*";
...
int RGWGetObj::read_user_manifest_part(rgw::sal::RGWBucket* bucket, const rgw_bucket_dir_entry& ent,
                                       RGWAccessControlPolicy* const bucket_acl,
                                       const boost::optional<Policy>& bucket_policy, const off_t start_ofs,
                                       const off_t end_ofs, bool swift_slo)
{
    ...
    else if (!verify_object_permission(this,
                                       s,
                                       part->get_obj(),
                                       s->user_acl.get(),
                                       bucket_acl,
    ...
    if (op_ret >= 0) op_ret = filter->flush();
    return op_ret;
}

static int iterate_user_manifest_parts(const DoutPrefixProvider* dpp, CephContext* const cct,
                                       rgw::sal::RGWStore* const store, const off_t ofs, const off_t end,
                                       rgw::sal::RGWBucket* bucket, const string& obj_prefix,
                                       RGWAccessControlPolicy* const bucket_acl,
                                       const boost::optional<Policy>& bucket_policy, uint64_t* const ptotal_len,
                                       uint64_t* const pobj_size, string* const pobj_sum,
                                       int (*cb)(rgw::sal::RGWBucket* bucket, const rgw_bucket_dir_entry& ent,
                                                 RGWAccessControlPolicy* const bucket_acl,
                                                 const boost::optional<Policy>& bucket_policy, off_t start_ofs,
                                                 off_t end_ofs, void* param, bool swift_slo),
                                       void* const cb_param, optional_yield y)
{
    ...
    do {
        ...
        for (rgw_bucket_dir_entry& ent : results.objs) {
            ...
            obj_ofs += obj_size;
            if (pobj_sum) {
                etag_sum.Update((const unsigned char*)ent.meta.etag.c_str(), ent.meta.etag.length());
            }
            ...
            perfcounter->tinc(l_rgw_get_lat, (ceph_clock_now() - start_time));

            if (found_start && !handled_end) {
                len_count += end_ofs - start_ofs;

                if (cb) {
                    r = cb(bucket, ent, bucket_acl, bucket_policy, start_ofs, end_ofs, cb_param, false /* swift_slo */);
                    if (r < 0) {
                        return r;
                    }
                }
...
static int iterate_slo_parts(CephContext* cct, rgw::sal::RGWStore* store, off_t ofs, off_t end,
                             map<uint64_t, rgw_slo_part>& slo_parts,
                             int (*cb)(rgw::sal::RGWBucket* bucket, const rgw_bucket_dir_entry& ent,
                                       RGWAccessControlPolicy* bucket_acl, const boost::optional<Policy>& bucket_policy,
                                       off_t start_ofs, off_t end_ofs, void* param, bool swift_slo),
                             void* cb_param)
{
        ...
        if (found_start) {
            if (cb) {
                dout(20) << "iterate_slo_parts()"
                         << " obj=" << part.obj_name << " start_ofs=" << start_ofs << " end_ofs=" << end_ofs << dendl;

                // SLO is a Swift thing, and Swift has no knowledge of S3 Policies.
                int r = cb(part.bucket,
                           ent,
                           part.bucket_acl,
                           (part.bucket_policy ? boost::optional<Policy>(*part.bucket_policy) : none),
                           start_ofs,
                           end_ofs,
                           cb_param,
                           true /* swift_slo */);
...
static int get_obj_user_manifest_iterate_cb(rgw::sal::RGWBucket* bucket, const rgw_bucket_dir_entry& ent,
                                            RGWAccessControlPolicy* const bucket_acl,
                                            const boost::optional<Policy>& bucket_policy, const off_t start_ofs,
                                            const off_t end_ofs, void* const param, bool swift_slo = false)
{
    RGWGetObj* op = static_cast<RGWGetObj*>(param);
    return op->read_user_manifest_part(bucket, ent, bucket_acl, bucket_policy, start_ofs, end_ofs, swift_slo);
}
...
int RGWGetObj::handle_user_manifest(const char* prefix, optional_yield y)
{
    ...
    r = iterate_user_manifest_parts(this,
                                    s->cct,
                                    store,
                                    ofs,
                                    end,
    ...
    r = iterate_user_manifest_parts(this,
                                    ...
                                    *bucket_policy,
                                    ...
                                    nullptr,
                                    nullptr,
                                    get_obj_user_manifest_iterate_cb,
                                    (void*)this,
                                    y);
    if (r < 0) {
        return r;
...
int RGWGetObj::handle_slo_manifest(bufferlist& bl, optional_yield y)
{
    ...
    for (const auto& entry : slo_info.entries) {
        ...
        if (bucket_name.compare(s->bucket->get_name()) != 0) {
            ...
            if (piter != policies.end()) {
                ...
                auto _bucket_policy =
                    get_iam_policy_from_attr(s->cct, tmp_bucket->get_attrs(), tmp_bucket->get_tenant());
        ...
        ldpp_dout(this, 20) << "slo_part: bucket=" << part.bucket << " obj=" << part.obj_name << " size=" << part.size
                            << " etag=" << part.etag << dendl;

        etag_sum.Update((const unsigned char*)entry.etag.c_str(), entry.etag.length());
    ...
    r = iterate_slo_parts(s->cct, store, ofs, end, slo_parts, get_obj_user_manifest_iterate_cb, (void*)this);
    if (r < 0) {
        return r;
    }
...
static inline void rgw_cond_decode_objtags(struct req_state* s, const std::map<std::string, buffer::list>& attrs)
{
...
void RGWGetObj::execute(optional_yield y)
{
    ...
    op_ret = this->get_decrypt_filter(&decrypt, filter, attr_iter != attrs.end() ? &(attr_iter->second) : nullptr);
    if (decrypt != nullptr) {
        filter = decrypt.get();
    }
    if (op_ret < 0) {
...
int RGWGetObj::init_common()
{
    ...
    if (if_mod) {
        if (parse_time(if_mod, &mod_time) < 0) return -EINVAL;
        mod_ptr = &mod_time;
    }

    if (if_unmod) {
        if (parse_time(if_unmod, &unmod_time) < 0) return -EINVAL;
...
void RGWListBuckets::execute(optional_yield y)
{
    ...
    do {
        ...
        for (const auto& policy : store->get_zonegroup().placement_targets) {
            policies_stats.emplace(policy.second.name, decltype(policies_stats)::mapped_type());
        }
...
void RGWGetUsage::execute(optional_yield y)
{
    ...
    while (s->bucket && is_truncated) {
        op_ret = s->bucket->read_usage(this, start_epoch, end_epoch, max_entries, &is_truncated, usage_iter, usage);
        if (op_ret == -ENOENT) {
            op_ret = 0;
            is_truncated = false;
        }
...
void RGWStatAccount::execute(optional_yield y)
{
    ...
    do {
        ...
        lastmarker = nullptr;
        op_ret = rgw_read_user_buckets(this, store, s->user->get_id(), buckets, marker, string(), max_buckets, true, y);
        if (op_ret < 0) {
            /* hmm.. something wrong here.. the user was authenticated, so it
               should exist */
            ldpp_dout(this, 10) << "WARNING: failed on rgw_read_user_buckets uid=" << s->user->get_id()
                                << " ret=" << op_ret << dendl;
            break;
        }
        else {
        ...
        if (!lastmarker) {
            ldpp_dout(this, -1) << "ERROR: rgw_read_user_buckets, stasis at marker=" << marker
                                << " uid=" << s->user->get_id() << dendl;
            break;
        }
        marker = *lastmarker;
...
void RGWSetBucketVersioning::execute(optional_yield y)
{
    ...
    if (mfa_set_status && !s->mfa_verified) {
...
int RGWListBucket::verify_permission(optional_yield y)
{
    ...
    if (!prefix.empty()) s->env.emplace("s3:prefix", prefix);
    ...
    if (!verify_bucket_permission(this, s, list_versions ? rgw::IAM::s3ListBucketVersions : rgw::IAM::s3ListBucket)) {
        return -EACCES;
    }
...
int RGWListBucket::parse_max_keys()
{
    ...
    return parse_value_and_bound(max_keys, max, 0, g_conf().get_val<uint64_t>("rgw_max_listing_results"), default_max);
}
...
int RGWCreateBucket::verify_permission(optional_yield y)
{
    ...
    if (s->user->get_tenant() != s->bucket_tenant) {
        ...
        if (s->auth.identity->get_identity_type() != TYPE_ROLE) {
            ldpp_dout(this, 10) << "user cannot create a bucket in a different tenant"
                                << " (user_id.tenant=" << s->user->get_tenant() << " requested=" << s->bucket_tenant
                                << ")" << dendl;
            return -EACCES;
        }
    }
    ...
    if (s->user->get_max_buckets()) {
        rgw::sal::RGWBucketList buckets;
        string marker;
        op_ret = rgw_read_user_buckets(
            this, store, s->user->get_id(), buckets, marker, string(), s->user->get_max_buckets(), false, y);
        if (op_ret < 0) {
            return op_ret;
        }
...
static void prepare_add_del_attrs(const map<string, bufferlist>& orig_attrs, map<string, bufferlist>& out_attrs,
...
static void prepare_add_del_attrs(const map<string, bufferlist>& orig_attrs, const set<string>& rmattr_names,
...
static void populate_with_generic_attrs(const req_state* const s, map<string, bufferlist>& out_attrs)
{
...
static int filter_out_quota_info(std::map<std::string, bufferlist>& add_attrs,
                                 const std::set<std::string>& rmattr_names, RGWQuotaInfo& quota,
                                 bool* quota_extracted = nullptr)
{
    ...
    if (std::end(add_attrs) != iter) {
        quota.max_objects = static_cast<int64_t>(strict_strtoll(iter->second.c_str(), 10, &err));
        if (!err.empty()) {
            return -EINVAL;
        }
    ...
    if (iter != add_attrs.end()) {
        quota.max_size = static_cast<int64_t>(strict_strtoll(iter->second.c_str(), 10, &err));
        if (!err.empty()) {
            return -EINVAL;
        }
        add_attrs.erase(iter);
...
static void filter_out_website(std::map<std::string, ceph::bufferlist>& add_attrs,
                               const std::set<std::string>& rmattr_names, RGWBucketWebsiteConf& ws_conf)
{
    std::string lstval;
    ...
    const auto mapping = {std::make_pair(RGW_ATTR_WEB_INDEX, std::ref(ws_conf.index_doc_suffix)),
...
void RGWCreateBucket::execute(optional_yield y)
{
    ...
    if (!placement_rule.name.empty() && !zonegroup.placement_targets.count(placement_rule.name)) {
    ...
    if (op_ret < 0 && op_ret != -ENOENT) return;
    s->bucket_exists = (op_ret != -ENOENT);

    if (s->bucket_exists) {
        if (!s->system_request && store->svc()->zone->get_zonegroup().get_id() != s->bucket->get_info().zonegroup) {
            op_ret = -EEXIST;
            return;
        }
    ...
    op_ret = store->create_bucket(this,
                                  *s->user,
                                  tmp_bucket,
                                  zonegroup_id,
                                  placement_rule,
    ...
    op_ret = store->ctl()->bucket->link_bucket(
        s->user->get_id(), s->bucket->get_key(), s->bucket->get_creation_time(), y, s, false);
    ...
    if (need_metadata_upload() && existed) {
        ...
        do {
            ...
            op_ret = store->ctl()->bucket->set_bucket_instance_attrs(
                s->bucket->get_info(), attrs, &s->bucket->get_info().objv_tracker, y, this);
...
#define VERSION_ID_STR "?versionId="
        ...
        ret = store->get_bucket(this, s->user.get(), copy_source_tenant_name, copy_source_bucket_name, &bucket, y);
        if (ret < 0) {
...
int RGWPutObj::verify_permission(optional_yield y)
{
        ...
        if (!s->auth.identity->is_admin_of(cs_acl.get_owner().get_id())) {
            if (policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
                ...
                auto [has_s3_existing_tag, has_s3_resource_tag] =
                    rgw_check_policy_condition(this, policy, s->iam_user_policies, s->session_policies);
                if (has_s3_existing_tag || has_s3_resource_tag)
                ...
                for (auto& user_policy : s->iam_user_policies) {
                    if (usr_policy_res = user_policy.eval(
                            s->env,
                            *s->auth.identity,
                            cs_object->get_instance().empty() ? rgw::IAM::s3GetObject : rgw::IAM::s3GetObjectVersion,
                            rgw::ARN(cs_object->get_obj()));
                        usr_policy_res == Effect::Deny)
                        return -EACCES;
                ...
                if (e == Effect::Deny) {
                    return -EACCES;
                         ...
                         !cs_acl.verify_permission(this, *s->auth.identity, s->perm_mask, RGW_PERM_READ)) {
                    return -EACCES;
                }
                rgw_iam_remove_objtags(this, s, cs_object.get(), has_s3_existing_tag, has_s3_resource_tag);
            }
            else if (!cs_acl.verify_permission(this, *s->auth.identity, s->perm_mask, RGW_PERM_READ)) {
                return -EACCES;
            }
        }
    }
    ...
    if (s->bucket_access_conf && s->bucket_access_conf->block_public_acls()) {
        if (s->canned_acl.compare("public-read") || s->canned_acl.compare("public-read-write") ||
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        ...
        auto identity_policy_res = eval_identity_or_session_policies(
            s->iam_user_policies, s->env, boost::none, rgw::IAM::s3PutObject, s->object->get_obj());
        if (identity_policy_res == Effect::Deny) return -EACCES;

        rgw::IAM::Effect e = Effect::Pass;
        rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
        if (s->iam_policy) {
            e = s->iam_policy->eval(s->env, *s->auth.identity, rgw::IAM::s3PutObject, s->object->get_obj(), princ_type);
        }
        if (e == Effect::Deny) {
            return -EACCES;
        }
        ...
        if (!s->session_policies.empty()) {
            auto session_policy_res = eval_identity_or_session_policies(
                s->session_policies, s->env, boost::none, rgw::IAM::s3PutObject, s->object->get_obj());
            if (session_policy_res == Effect::Deny) {
                return -EACCES;
            }
            if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
                // Intersection of session policy and identity policy plus intersection of session policy and bucket
                // policy
                ...
                if (session_policy_res == Effect::Allow && identity_policy_res == Effect::Allow) return 0;
            }
            return -EACCES;
        }
...
public:
    explicit RGWPutObj_CB(RGWPutObj* _op)
        : op(_op)
    {}
...
int RGWPutObj::get_data(const off_t fst, const off_t lst, bufferlist& bl)
{
    ...
    op_ret = this->get_decrypt_filter(
        &decrypt, filter, obj->get_attrs(), attr_iter != obj->get_attrs().end() ? &(attr_iter->second) : nullptr);
    if (decrypt != nullptr) {
        filter = decrypt.get();
    }
    if (op_ret < 0) {
...
static CompressorRef get_compressor_plugin(const req_state* s, const std::string& compression_type)
{
...
void RGWPutObj::execute(optional_yield y)
{
    ...
    auto put_lat = make_scope_guard([&] { perfcounter->tinc(l_rgw_put_lat, s->time_elapsed()); });
    ...
    if (supplied_md5_b64) {
        ...
        op_ret = ceph_unarmor(supplied_md5_bin,
                              &supplied_md5_bin[CEPH_CRYPTO_MD5_DIGESTSIZE + 1],
                              supplied_md5_b64,
                              supplied_md5_b64 + strlen(supplied_md5_b64));
    ...
    auto aio = rgw::make_throttle(s->cct->_conf->rgw_put_obj_min_window_size, s->yield);
    using namespace rgw::putobj;
    constexpr auto max_processor_size =
        std::max({sizeof(MultipartObjectProcessor), sizeof(AtomicObjectProcessor), sizeof(AppendObjectProcessor)});
    ...
    if (multipart) {
        ...
        if (op_ret < 0) {
            if (op_ret != -ENOENT) {
                ldpp_dout(this, 0) << "ERROR: get_multipart_info returned " << op_ret << ": " << cpp_strerror(-op_ret)
                                   << dendl;
            }
            else {   // -ENOENT: raced with upload complete/cancel, no need to spam log
                ldpp_dout(this, 20) << "failed to get multipart info (returned " << op_ret << ": "
                                    << cpp_strerror(-op_ret) << "): probably raced with upload complete / cancel"
                                    << dendl;
            }
            return;
        }
        ...
        processor.emplace<MultipartObjectProcessor>(&*aio,
                                                    store,
                                                    s->bucket.get(),
                                                    pdest_placement,
                                                    s->owner.get_id(),
        ...
        processor.emplace<AppendObjectProcessor>(&*aio,
                                                 store,
                                                 s->bucket.get(),
                                                 pdest_placement,
        ...
        pdest_placement = &s->dest_placement;
        processor.emplace<AtomicObjectProcessor>(&*aio,
                                                 store,
                                                 s->bucket.get(),
                                                 pdest_placement,
                                                 s->bucket_owner.get_id(),
    ...
    if (compressor && compressor->is_compressed()) {
        ...
        ldpp_dout(this, 20) << "storing " << RGW_ATTR_COMPRESSION << " with type=" << cs_info.compression_type
                            << ", orig_size=" << cs_info.orig_size << ", blocks=" << cs_info.blocks.size() << dendl;
    }
    ...
    op_ret = processor->complete(s->obj_size,
                                 etag,
                                 &mtime,
                                 real_time(),
                                 attrs,
...
void RGWPostObj::execute(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        ...
        if (identity_policy_res == Effect::Pass && e == Effect::Pass &&
            !verify_bucket_permission_no_policy(this, s, RGW_PERM_WRITE)) {
            op_ret = -EACCES;
            return;
        }
    }
    ...
    do {
        ...
        std::unique_ptr<rgw::sal::RGWObject> obj = s->bucket->get_object(rgw_obj_key(get_current_filename()));
        if (s->bucket->versioning_enabled()) {
        ...
        if (encrypt != nullptr) {
            ...
            const auto& compression_type =
                store->svc()->zone->get_zone_params().get_compression_type(s->dest_placement);
    ...
    // send request to notification manager
    const auto ret =
        rgw::notify::publish_commit(s->object.get(), ofs, ceph::real_clock::now(), etag, event_type, res, this);
    if (ret < 0) {
        ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
        // too late to rollback operation, hence op_ret is not set here
    }
...
void RGWPutMetadataAccount::filter_out_temp_url(map<string, bufferlist>& add_attrs, const set<string>& rmattr_names,
...
int RGWPutMetadataAccount::init_processing(optional_yield y)
{
    ...
    op_ret = store->ctl()->user->get_attrs_by_uid(this, s->user->get_id(), &orig_attrs, s->yield, &acct_op_tracker);
    if (op_ret < 0) {
        return op_ret;
    }
    ...
    op_ret = filter_out_quota_info(attrs, rmattr_names, new_quota, &new_quota_extracted);
    if (op_ret < 0) {
        return op_ret;
    }
...
void RGWPutMetadataAccount::execute(optional_yield y)
{
    ...
    op_ret = store->ctl()->user->get_info_by_uid(
        this, s->user->get_id(), &new_uinfo, s->yield, RGWUserCtl::GetParams().set_objv_tracker(&acct_op_tracker));
    if (op_ret < 0) {
        return;
    }
    ...
    op_ret = store->ctl()->user->store_info(this,
                                            new_uinfo,
                                            s->yield,
...
void RGWPutMetadataBucket::execute(optional_yield y)
{
    ...
    if (!placement_rule.empty() && placement_rule != s->bucket->get_placement_rule()) {
        op_ret = -EEXIST;
        return;
    }
    ...
    op_ret = retry_raced_bucket_write(this, s->bucket.get(), [this] {
        ...
        if (has_policy) {
            if (s->dialect.compare("swift") == 0) {
                auto old_policy = static_cast<RGWAccessControlPolicy_SWIFT*>(s->bucket_acl.get());
...
int RGWDeleteObj::handle_slo_manifest(bufferlist& bl, optional_yield y)
{
    ...
    try {
        deleter = std::unique_ptr<RGWBulkDelete::Deleter>(new RGWBulkDelete::Deleter(this, store, s));
    }
...
int RGWDeleteObj::verify_permission(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        if (s->bucket->get_info().obj_lock_enabled() && bypass_governance_mode) {
            auto r = eval_identity_or_session_policies(s->iam_user_policies,
                                                       s->env,
                                                       boost::none,
            ...
            if (r == Effect::Deny) {
                ...
                r = eval_identity_or_session_policies(s->session_policies,
                                                      s->env,
                                                      boost::none,
                                                      rgw::IAM::s3BypassGovernanceRetention,
                                                      ARN(s->bucket->get_key(), s->object->get_name()));
        ...
        auto identity_policy_res = eval_identity_or_session_policies(
            ...
            s->env,
            boost::none,
            s->object->get_instance().empty() ? rgw::IAM::s3DeleteObject : rgw::IAM::s3DeleteObjectVersion,
            ARN(s->bucket->get_key(), s->object->get_name()));
        if (identity_policy_res == Effect::Deny) {
            return -EACCES;
        }
        ...
        if (r == Effect::Allow || identity_policy_res == Effect::Allow) return 0;
    }
    ...
    if (s->bucket->get_info().mfa_enabled() && !s->object->get_instance().empty() && !s->mfa_verified) {
...
void RGWDeleteObj::execute(optional_yield y)
{
    ...
    if (!rgw::sal::RGWObject::empty(s->object.get())) {
        ...
        RGWObjectCtx* obj_ctx = static_cast<RGWObjectCtx*>(s->obj_ctx);
        {
            ...
            if (check_obj_lock) {
                ceph_assert(astate);
                int object_lock_response =
                    verify_object_lock(this, astate->attrset, bypass_perm, bypass_governance_mode);
        ...
        if (!ver_restored) {
            ...
            op_ret = s->object->delete_object(
                this, obj_ctx, s->owner, s->bucket_owner, unmod_since, s->system_request, epoch, version_id, s->yield);
            if (op_ret >= 0) {
        ...
        // send request to notification manager
        const auto ret = rgw::notify::publish_commit(
            s->object.get(), obj_size, ceph::real_clock::now(), etag, event_type, res, this);
        if (ret < 0) {
            ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
            // too late to rollback operation, hence op_ret is not set here
        }
...
bool RGWCopyObj::parse_copy_location(const std::string_view& url_src, string& bucket_name, rgw_obj_key& key,
                                     req_state* s)
{
...
int RGWCopyObj::verify_permission(optional_yield y)
{
    ...
    op_ret = src_bucket->load_by_name(
        this, src_tenant_name, src_bucket_name, s->bucket_instance_id, s->sysobj_ctx, s->yield);
    if (op_ret < 0) {
        if (op_ret == -ENOENT) {
            op_ret = -ERR_NO_SUCH_BUCKET;
        }
    ...
    if (s->local_source && source_zone.empty()) {
        ...
        if (!s->auth.identity->is_admin_of(src_acl.get_owner().get_id())) {
            if (src_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
                auto [has_s3_existing_tag, has_s3_resource_tag] =
                    rgw_check_policy_condition(this, src_policy, s->iam_user_policies, s->session_policies);
                if (has_s3_existing_tag || has_s3_resource_tag)
                ...
                auto identity_policy_res = eval_identity_or_session_policies(
                    ...
                    s->env,
                    boost::none,
                    src_object->get_instance().empty() ? rgw::IAM::s3GetObject : rgw::IAM::s3GetObjectVersion,
                ...
                if (!s->session_policies.empty()) {
                    ...
                    if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
                        // Intersection of session policy and identity policy plus intersection of session policy and
                        // bucket policy
                        if ((session_policy_res != Effect::Allow || identity_policy_res != Effect::Allow) &&
                            (session_policy_res != Effect::Allow || e != Effect::Allow)) {
                            return -EACCES;
                        ...
                        if ((session_policy_res != Effect::Allow || identity_policy_res != Effect::Allow) &&
                            ...
                            return -EACCES;
                             ...
                             rgw::IAM::PolicyPrincipal::Other) {   // there was no match in the bucket policy
                        if (session_policy_res != Effect::Allow || identity_policy_res != Effect::Allow) {
                            return -EACCES;
                        }
                    }
                ...
                if (identity_policy_res == Effect::Pass && e == Effect::Pass &&
                    !src_acl.verify_permission(this, *s->auth.identity, s->perm_mask, RGW_PERM_READ)) {
                    return -EACCES;
                }
            ...
            else if (!src_acl.verify_permission(this, *s->auth.identity, s->perm_mask, RGW_PERM_READ)) {
                return -EACCES;
            }
        }
    }
    ...
    if (src_bucket_name.compare(dest_bucket_name) == 0) { /* will only happen if s->local_source
        ...
        if (op_ret < 0) {
            if (op_ret == -ENOENT) {
                ldpp_dout(this, 0) << "ERROR: Destination Bucket not found for user: " << s->user->get_id().to_str()
                                   << dendl;
                op_ret = -ERR_NO_SUCH_BUCKET;
            }
            return op_ret;
        ...
        op_ret =
            dest_bucket->load_by_name(this, dest_tenant_name, dest_bucket_name, std::string(), s->sysobj_ctx, s->yield);
        if (op_ret < 0) {
            if (op_ret == -ENOENT) {
                op_ret = -ERR_NO_SUCH_BUCKET;
            }
    ...
    op_ret = read_bucket_policy(this,
                                store,
                                s,
    ...
    if (!s->auth.identity->is_admin_of(dest_policy.get_owner().get_id())) {
        if (dest_iam_policy != boost::none || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
            ...
            auto [has_s3_existing_tag, has_s3_resource_tag] =
                rgw_check_policy_condition(this, dest_iam_policy, s->iam_user_policies, s->session_policies);
            if (has_s3_resource_tag) rgw_iam_add_buckettags(this, s, dest_bucket.get());

            rgw_add_to_iam_environment(s->env, "s3:x-amz-copy-source", copy_source);
            if (md_directive) rgw_add_to_iam_environment(s->env, "s3:x-amz-metadata-directive", *md_directive);

            auto identity_policy_res = eval_identity_or_session_policies(
                s->iam_user_policies, s->env, boost::none, rgw::IAM::s3PutObject, ARN(dest_object->get_obj()));
            if (identity_policy_res == Effect::Deny) {
                return -EACCES;
            }
            auto e = Effect::Pass;
            rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
            if (dest_iam_policy) {
                e = dest_iam_policy->eval(
                    s->env, *s->auth.identity, rgw::IAM::s3PutObject, ARN(dest_object->get_obj()), princ_type);
            }
            if (e == Effect::Deny) {
                return -EACCES;
            }
            if (!s->session_policies.empty()) {
                auto session_policy_res = eval_identity_or_session_policies(
                    s->session_policies, s->env, boost::none, rgw::IAM::s3PutObject, ARN(dest_object->get_obj()));
                if (session_policy_res == Effect::Deny) {
                    return false;
                }
                if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
            ...
            if (identity_policy_res == Effect::Pass && e == Effect::Pass &&
                !dest_bucket_policy.verify_permission(this, *s->auth.identity, s->perm_mask, RGW_PERM_WRITE)) {
                return -EACCES;
            }
        }
        else if (!dest_bucket_policy.verify_permission(this, *s->auth.identity, s->perm_mask, RGW_PERM_WRITE)) {
            return -EACCES;
        }
    }
...
{
    if (!s->cct->_conf->rgw_copy_obj_progress) return;

    if (ofs - last_ofs < s->cct->_conf->rgw_copy_obj_progress_every_bytes) return;
...
void RGWCopyObj::execute(optional_yield y)
{
    ...
    uint64_t obj_size = 0;
    {
        ...
        RGWObjState* astate = nullptr;
        op_ret = src_object->get_obj_state(this, s->obj_ctx, *src_bucket, &astate, s->yield, true);
        if (op_ret < 0) {
            return;
        }
        obj_size = astate->size;
        ...
        if (!s->system_request) {   // no quota enforcement for system requests
            ...
            op_ret = dest_bucket->check_quota(user_quota, bucket_quota, astate->accounted_size, y);
            if (op_ret < 0) {
                return;
            }
        }
...
int RGWGetACLs::verify_permission(optional_yield y)
{
    ...
    if (!rgw::sal::RGWObject::empty(s->object.get())) {
        auto iam_action =
            s->object->get_instance().empty() ? rgw::IAM::s3GetObjectAcl : rgw::IAM::s3GetObjectVersionAcl;
        if (has_s3_existing_tag || has_s3_resource_tag)
            rgw_iam_add_objtags(this, s, has_s3_existing_tag, has_s3_resource_tag);
...
int RGWPutACLs::verify_permission(optional_yield y)
{
    ...
    if (!rgw::sal::RGWObject::empty(s->object.get())) {
        auto iam_action =
            s->object->get_instance().empty() ? rgw::IAM::s3PutObjectAcl : rgw::IAM::s3PutObjectVersionAcl;
    ...
    if (op_ret < 0) {
        if (op_ret == -ERANGE) {
            ...
            s->err.message = "The XML you provided was larger than the maximum " +
                             std::to_string(s->cct->_conf->rgw_max_put_param_size) + " bytes allowed.";
        }
        return;
    }
...
#define ACL_GRANTS_MAX_NUM 100
    ...
    if (grants_num > max_num) {
        ldpp_dout(this, 4) << "An acl can have up to " << max_num << " grants, request acl grants num: " << grants_num
                           << dendl;
        op_ret = -ERR_LIMIT_EXCEEDED;
        s->err.message =
            "The request is rejected, because the acl grants number you requested is larger than the maximum " +
            std::to_string(max_num) + " grants allowed in an acl.";
        return;
    }
    ...
    if (s->bucket_access_conf && s->bucket_access_conf->block_public_acls() && new_policy.is_public(this)) {
        op_ret = -EACCES;
        return;
    }
    ...
    if (!rgw::sal::RGWObject::empty(s->object.get())) {
        ...
        op_ret = store->ctl()->bucket->set_bucket_instance_attrs(
            s->bucket->get_info(), attrs, &s->bucket->get_info().objv_tracker, s->yield, this);
    }
    if (op_ret == -ECANCELED) {
...
void RGWPutLC::execute(optional_yield y)
{
    ...
    if (content_md5_bin) {
        ...
        if (memcmp(data_hash_res, content_md5_bin->c_str(), CEPH_CRYPTO_MD5_DIGESTSIZE) != 0) {
            ...
            ldpp_dout(this, 5) << s->err.message << " Specified content md5: " << content_md5
                               << ", calculated content md5: " << data_hash_res << dendl;
            return;
        }
    }
...
void RGWCompleteMultipart::execute(optional_yield y)
{
    ...
    do {
        op_ret = list_multipart_parts(
            this, store, s, upload_id, meta_oid, max_parts, marker, obj_parts, &marker, &truncated);
        if (op_ret == -ENOENT) {
        ...
        for (obj_iter = obj_parts.begin(); iter != parts->parts.end() && obj_iter != obj_parts.end();
             ++iter, ++obj_iter, ++handled_parts) {
            uint64_t part_size = obj_iter->second.accounted_size;
            if (handled_parts < (int)parts->parts.size() - 1 && part_size < min_part_size) {
                op_ret = -ERR_TOO_SMALL;
                return;
            }
            ...
            if (iter->first != (int)obj_iter->first) {
                ldpp_dout(this, 0) << "NOTICE: parts num mismatch: next requested: " << iter->first
            ...
            if (part_etag.compare(obj_iter->second.etag) != 0) {
                ldpp_dout(this, 0) << "NOTICE: etag mismatch: part: " << iter->first << " etag: " << iter->second
                                   << dendl;
                op_ret = -ERR_INVALID_PART;
                return;
            }
            ...
            if ((handled_parts > 0) &&
                ((part_compressed != compressed) || (cs_info.compression_type != obj_part.cs_info.compression_type))) {
    ...
    int r = meta_obj->delete_object(this,
                                    s->obj_ctx,
                                    ACLOwner(),
                                    ACLOwner(),
                                    ceph::real_time(),
    ...
    // send request to notification manager
    const auto ret = rgw::notify::publish_commit(
        s->object.get(), ofs, ceph::real_clock::now(), final_etag_str, event_type, res, this);
    if (ret < 0) {
        ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
        // too late to rollback operation, hence op_ret is not set here
    }
...
bool RGWCompleteMultipart::check_previously_completed(const DoutPrefixProvider* dpp,
                                                      const RGWMultiCompleteUpload* parts)
{
    ...
    for (const auto& [index, part] : parts->parts) {
        ...
        hash.Update((const unsigned char*)petag, sizeof(petag));
        ldpp_dout(dpp, 20) << __func__ << "() re-calculating multipart etag: part: " << index << ", etag: " << partetag
                           << dendl;
    }
    ...
    if (oetag.compare(final_etag_str) != 0) {
        ldpp_dout(dpp, 1) << __func__ << "() NOTICE: etag mismatch: object etag:" << oetag
                          << ", re-calculated etag:" << final_etag_str << dendl;
        return false;
    }
...
int RGWAbortMultipart::verify_permission(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        auto identity_policy_res = eval_identity_or_session_policies(
            s->iam_user_policies, s->env, boost::none, rgw::IAM::s3AbortMultipartUpload, s->object->get_obj());
        if (identity_policy_res == Effect::Deny) {
            return -EACCES;
        }
        ...
        if (s->iam_policy) {
            e = s->iam_policy->eval(
                s->env, *s->auth.identity, rgw::IAM::s3AbortMultipartUpload, s->object->get_obj(), princ_type);
        }
...
void RGWAbortMultipart::execute(optional_yield y)
{
    ...
    if (upload_id.empty() || rgw::sal::RGWObject::empty(s->object.get())) return;
...
int RGWListMultipart::verify_permission(optional_yield y)
{
    ...
    if (!verify_object_permission(this, s, rgw::IAM::s3ListMultipartUploadParts)) return -EACCES;
...
void RGWListMultipart::execute(optional_yield y)
{
    ...
    op_ret = list_multipart_parts(this, store, s, upload_id, meta_oid, max_parts, marker, parts, NULL, &truncated);
}
...
int RGWListBucketMultiparts::verify_permission(optional_yield y)
{
    ...
    if (!verify_bucket_permission(this, s, rgw::IAM::s3ListBucketMultipartUploads)) return -EACCES;
...
void RGWListBucketMultiparts::execute(optional_yield y)
{
    ...
    op_ret = list_bucket_multiparts(this,
                                    store,
                                    s->bucket->get_info(),
                                    prefix,
                                    marker_meta,
    ...
    if (!objs.empty()) {
        ...
        for (iter = objs.begin(); iter != objs.end(); ++iter) {
            ...
            if (!entry.mp.from_meta(key.name)) continue;
            entry.obj = *iter;
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        if (s->bucket->get_info().obj_lock_enabled() && bypass_governance_mode) {
            ...
            if (r == Effect::Deny) {
                ...
                r = s->iam_policy->eval(
                    s->env, *s->auth.identity, rgw::IAM::s3BypassGovernanceRetention, ARN(s->bucket->get_key()));
                if (r == Effect::Deny) {
                    bypass_perm = false;
                }
            }
        ...
        auto identity_policy_res = eval_identity_or_session_policies(
            ...
            s->env,
            boost::none,
            not_versioned ? rgw::IAM::s3DeleteObject : rgw::IAM::s3DeleteObjectVersion,
            ARN(s->bucket->get_key()));
        if (identity_policy_res == Effect::Deny) {
            return -EACCES;
        }
...
void RGWDeleteMultiObj::write_ops_log_entry(rgw_log_entry& entry) const
{
    int num_err = 0;
    int num_ok = 0;
    for (auto iter = ops_log_entries.begin(); iter != ops_log_entries.end(); ++iter) {
        if (iter->error) {
            num_err++;
        }
        else {
...
void RGWDeleteMultiObj::wait_flush(optional_yield y, boost::asio::deadline_timer* formatter_flush_cond,
                                                 ...
                                                 boost::asio::deadline_timer* formatter_flush_cond)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        auto identity_policy_res = eval_identity_or_session_policies(
            ...
            s->env,
            boost::none,
            o.instance.empty() ? rgw::IAM::s3DeleteObject : rgw::IAM::s3DeleteObjectVersion,
            ARN(obj->get_obj()));
        if (identity_policy_res == Effect::Deny) {
    ...
    op_ret = obj->delete_object(this, obj_ctx, s->owner, s->bucket_owner, ceph::real_time(), false, 0, version_id, y);
    if (op_ret == -ENOENT) {
        op_ret = 0;
    }
    ...
    // send request to notification manager
    const auto ret =
        rgw::notify::publish_commit(obj.get(), obj_size, ceph::real_clock::now(), etag, event_type, res, this);
    if (ret < 0) {
        ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
        // too late to rollback operation, hence op_ret is not set here
    }
...
#define DELETE_MULTI_OBJ_MAX_NUM 1000
    ...
    if (s->bucket->get_info().mfa_enabled()) {
        ...
        if (has_versioned && !s->mfa_verified) {
            ldpp_dout(this, 5) << "NOTICE: multi-object delete request with a versioned object, mfa auth not provided"
                               << dendl;
            op_ret = -ERR_MFA_REQUIRED;
            goto error;
        }
    ...
    for (iter = multi_delete->objects.begin(); iter != multi_delete->objects.end(); ++iter) {
        ...
        if (y) {
            wait_flush(y, &*formatter_flush_cond, [&aio_count, max_aio] { return aio_count < max_aio; });
            aio_count++;
            spawn::spawn(y.get_yield_context(),
                         [this, &y, &aio_count, obj_key, &formatter_flush_cond](yield_context yield) {
                             handle_individual_object(
                                 obj_key, optional_yield{y.get_io_context(), yield}, &*formatter_flush_cond);
                             aio_count--;
                         });
        }
        else {
...
bool RGWBulkDelete::Deleter::verify_permission(RGWBucketInfo& binfo, map<string, bufferlist>& battrs,
                                               ACLOwner& bucket_owner /* out */, optional_yield y)
{
...
bool RGWBulkDelete::Deleter::delete_single(const acct_path_t& path, optional_yield y)
{
    ...
    if (!path.obj_key.empty()) {
        ...
        ret = obj->delete_object(
            dpp, s->obj_ctx, bowner, bucket_owner, ceph::real_time(), false, 0, version_id, s->yield);
        if (ret < 0) {
            goto delop_fail;
        }
    }
...
binfo_fail:
    if (-ENOENT == ret) {
        ...
        fail_desc_t failed_item = {.err = ret, .path = path};
...
boost::optional<std::pair<std::string, rgw_obj_key>> RGWBulkUploadOp::parse_path(const std::string_view& path)
{
    ...
    if (std::string_view::npos != start_pos) {
        ...
        if (std::string_view::npos != sep_pos) {
            ...
            return std::make_pair(std::string(bucket_name), rgw_obj_key(std::string(obj_name)));
        }
        else {
            /* It's guaranteed here that bucket name is at least one character
             * long and is different than slash. */
            return std::make_pair(std::string(path.substr(start_pos)), rgw_obj_key());
        }
    }
...
std::pair<std::string, std::string> RGWBulkUploadOp::handle_upload_path(struct req_state* s)
{
...
int RGWBulkUploadOp::handle_dir_verify_permission(optional_yield y)
{
    if (s->user->get_max_buckets() > 0) {
        ...
        std::string marker;
        op_ret = rgw_read_user_buckets(
            this, store, s->user->get_user(), buckets, marker, std::string(), s->user->get_max_buckets(), false, y);
        if (op_ret < 0) {
            return op_ret;
        }
...
void RGWBulkUploadOp::init(rgw::sal::RGWRadosStore* const store, struct req_state* const s, RGWHandler* const h)
{
...
int RGWBulkUploadOp::handle_dir(const std::string_view path, optional_yield y)
{
    ...
    op_ret =
        store->ctl()->bucket->link_bucket(s->user->get_id(), new_bucket, out_info.creation_time, s->yield, s, false);
    if (op_ret && !existed && op_ret != -EEXIST) {
        /* if it exists (or previously existed), don't remove it! */
...
bool RGWBulkUploadOp::handle_file_verify_permission(RGWBucketInfo& binfo, const rgw_obj& obj,
                                                    ...
                                                    ACLOwner& bucket_owner /* out */, optional_yield y)
{
    ...
    bucket_owner = bacl.get_owner();
    if (policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        auto identity_policy_res =
            eval_identity_or_session_policies(s->iam_user_policies, s->env, boost::none, rgw::IAM::s3PutObject, obj);
        if (identity_policy_res == Effect::Deny) {
            return false;
        }

        rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
        auto e = policy->eval(s->env, *s->auth.identity, rgw::IAM::s3PutObject, obj, princ_type);
        if (e == Effect::Deny) {
            return false;
        }

        if (!s->session_policies.empty()) {
            auto session_policy_res =
                eval_identity_or_session_policies(s->session_policies, s->env, boost::none, rgw::IAM::s3PutObject, obj);
            if (session_policy_res == Effect::Deny) {
                return false;
            }
            if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
    ...
    return verify_bucket_permission_no_policy(this, s, s->user_acl.get(), &bacl, RGW_PERM_WRITE);
}

int RGWBulkUploadOp::handle_file(const std::string_view path, const size_t size, AlignedStreamGetter& body,
                                 optional_yield y)
{
    ...
    op_ret = store->get_bucket(
        this, s->user.get(), rgw_bucket(rgw_bucket_key(s->user->get_tenant(), bucket_name)), &bucket, y);
    if (op_ret == -ENOENT) {
    ...
    if (!handle_file_verify_permission(bucket->get_info(), obj->get_obj(), bucket->get_attrs(), bowner, y)) {
    ...
    const auto& compression_type = store->svc()->zone->get_zone_params().get_compression_type(dest_placement);
    CompressorRef plugin;
    boost::optional<RGWPutObj_Compress> compressor;
    if (compression_type != "none") {
        plugin = Compressor::create(s->cct, compression_type);
    ...
    op_ret = processor.complete(size,
                                etag,
                                nullptr,
                                ceph::real_time(),
                                attrs,
    ...
    do {
        ...
        if (!status.empty() && header) {
            ...
            case rgw::tar::FileType::NORMAL_FILE:
            {
                ...
                auto body = AlignedStreamGetter(0, header->get_filesize(), rgw::tar::BLOCK_SIZE, *stream);
                op_ret = handle_file(filename, header->get_filesize(), body, y);
                if (!op_ret) {
            ...
            if (boost::algorithm::contains(std::initializer_list<int>{op_ret}, terminal_errors)) {
...
ssize_t RGWBulkUploadOp::AlignedStreamGetter::get_at_most(const size_t want, ceph::bufferlist& dst)
{
...
ssize_t RGWBulkUploadOp::AlignedStreamGetter::get_exactly(const size_t want, ceph::bufferlist& dst)
{
...
int RGWGetAttrs::verify_permission(optional_yield y)
{
    ...
    auto iam_action = s->object->get_instance().empty() ? rgw::IAM::s3GetObject : rgw::IAM::s3GetObjectVersion;
...
int RGWHandler::init(rgw::sal::RGWRadosStore* _store, struct req_state* _s, rgw::io::BasicClient* cio)
{
    store = _store;
    s = _s;
...
int RGWHandler::do_init_permissions(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 10) << "init_permissions on " << s->bucket << " failed, ret=" << ret << dendl;
...
int RGWHandler::do_read_permissions(RGWOp* op, bool only_bucket, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(op, 10) << "read_permissions on " << s->bucket << ":" << s->object << " only_bucket=" << only_bucket
                          << " ret=" << ret << dendl;
        if (ret == -ENODATA) ret = -EACCES;
        if (s->auth.identity->is_anonymous() && ret == -EACCES) ret = -EPERM;
    }
...
void RGWPutBucketPolicy::execute(optional_yield y)
{
    ...
    try {
        ...
        rgw::sal::RGWAttrs attrs(s->bucket_attrs);
        if (s->bucket_access_conf && s->bucket_access_conf->block_public_policy() && rgw::IAM::is_public(p)) {
            op_ret = -EACCES;
            return;
        }
...
void RGWPutObjRetention::execute(optional_yield y)
{
    ...
    if (aiter != attrs.end()) {
        ...
        if (ceph::real_clock::to_time_t(obj_retention.get_retain_until_date()) <
            ceph::real_clock::to_time_t(old_obj_retention.get_retain_until_date())) {
            if (old_obj_retention.get_mode().compare("GOVERNANCE") != 0 || !bypass_perm || !bypass_governance_mode) {
                s->err.message = "proposed retain-until date shortens an existing retention period and governance "
                                 "bypass check failed";
                op_ret = -EACCES;
                return;
            }
