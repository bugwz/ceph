...
static void forward_req_info(const DoutPrefixProvider* dpp, CephContext* cct, req_info& info,
...
static int get_user_policy_from_attr(const DoutPrefixProvider* dpp, CephContext* const cct,
                                     map<string, bufferlist>& attrs, RGWAccessControlPolicy& policy /* out */)
{
 ...
 */
int rgw_op_get_bucket_policy_from_attr(const DoutPrefixProvider* dpp, CephContext* cct, rgw::sal::Driver* driver,
...
static int get_obj_policy_from_attr(const DoutPrefixProvider* dpp, CephContext* cct, rgw::sal::Driver* driver,
                                    RGWBucketInfo& bucket_info, map<string, bufferlist>& bucket_attrs,
                                    RGWAccessControlPolicy* policy, string* storage_class, rgw::sal::Object* obj,
                                    optional_yield y)
{
    bufferlist bl;
    int ret = 0;
...
vector<Policy> get_iam_user_policy_from_attr(CephContext* cct, map<string, bufferlist>& attrs, const string& tenant)
{
...
static int read_bucket_policy(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, req_state* s,
                              RGWBucketInfo& bucket_info, map<string, bufferlist>& bucket_attrs,
...
static int read_obj_policy(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, req_state* s,
                           RGWBucketInfo& bucket_info, map<string, bufferlist>& bucket_attrs,
                           RGWAccessControlPolicy* acl, string* storage_class, boost::optional<Policy>& policy,
                           rgw::sal::Bucket* bucket, rgw::sal::Object* object, optional_yield y, bool copy_src = false)
{
    string upload_id;
    ...
    int ret =
        get_obj_policy_from_attr(dpp, s->cct, driver, bucket_info, bucket_attrs, acl, storage_class, object, s->yield);
    if (ret == -ENOENT) {
        ...
        if (bucket_owner.compare(s->user->get_id()) != 0 && !s->auth.identity->is_admin_of(bucket_owner)) {
            auto r = eval_identity_or_session_policies(
                dpp, s->iam_user_policies, s->env, rgw::IAM::s3ListBucket, ARN(bucket->get_key()));
            ...
            if (!s->session_policies.empty()) {
                r = eval_identity_or_session_policies(
                    dpp, s->session_policies, s->env, rgw::IAM::s3ListBucket, ARN(bucket->get_key()));
                if (r == Effect::Allow) return -ENOENT;
                if (r == Effect::Deny) return -EACCES;
            }
...
int rgw_build_bucket_policies(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, req_state* s, optional_yield y)
{
    ...
    if (!s->src_bucket_name.empty()) {
        ...
        ret = driver->get_bucket(dpp, nullptr, rgw_bucket_key(s->src_tenant_name, s->src_bucket_name), &src_bucket, y);
        if (ret == 0) {
    ...
    if (!s->bucket_name.empty()) {
         ...
         * overwritten. */
        ret = driver->get_bucket(
            dpp, s->user.get(), rgw_bucket(s->bucket_tenant, s->bucket_name, s->bucket_instance_id), &s->bucket, y);
        if (ret < 0) {
            if (ret != -ENOENT) {
                string bucket_log;
 ...
 * Returns: 0 on success, -ERR# otherwise.
 */
int rgw_build_object_policies(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, req_state* s, bool prefetch_data,
                              optional_yield y)
{
    int ret = 0;
...
static int rgw_iam_remove_objtags(const DoutPrefixProvider* dpp, req_state* s, rgw::sal::Object* object,
...
static int rgw_iam_add_tags_from_bl(req_state* s, bufferlist& bl, bool has_existing_obj_tag = false,
...
static int rgw_iam_add_objtags(const DoutPrefixProvider* dpp, req_state* s, rgw::sal::Object* object,
                               bool has_existing_obj_tag, bool has_resource_tag)
{
    object->set_atomic();
    int op_ret = object->get_obj_attrs(s->yield, dpp);
...
static int rgw_iam_add_objtags(const DoutPrefixProvider* dpp, req_state* s, bool has_existing_obj_tag,
...
static void rgw_iam_add_crypt_attrs(rgw::IAM::Environment& e, const meta_map_t& attrs)
{
...
static std::tuple<bool, bool> rgw_check_policy_condition(const DoutPrefixProvider* dpp, req_state* s,
                                                         bool check_obj_exist_tag = true)
{
...
void rgw_build_iam_environment(rgw::sal::Driver* driver, req_state* s)
{
...
namespace {
template<typename F> int retry_raced_bucket_write(const DoutPrefixProvider* dpp, rgw::sal::Bucket* b, const F& f)
{
    auto r = f();
...
{
    if (rgw::sal::Object::empty(s->object.get())) return;
...
int RGWOp::init_quota()
{
    ...
    std::unique_ptr<rgw::sal::User> owner_user = driver->get_user(s->bucket->get_info().owner);
    rgw::sal::User* user;
 ...
 * */
static void get_cors_response_headers(const DoutPrefixProvider* dpp, RGWCORSRule* rule, const char* req_hdrs,
                                      string& hdrs, string& exp_hdrs, unsigned* max_age)
{
    if (req_hdrs) {
        list<string> hl;
...
int RGWGetObj::read_user_manifest_part(rgw::sal::Bucket* bucket, const rgw_bucket_dir_entry& ent,
...
static int iterate_user_manifest_parts(const DoutPrefixProvider* dpp, CephContext* const cct,
                                       rgw::sal::Driver* const driver, const off_t ofs, const off_t end,
                                       rgw::sal::Bucket* bucket, const string& obj_prefix,
                                       RGWAccessControlPolicy* const bucket_acl,
                                       const boost::optional<Policy>& bucket_policy, uint64_t* const ptotal_len,
                                       uint64_t* const pobj_size, string* const pobj_sum,
                                       int (*cb)(rgw::sal::Bucket* bucket, const rgw_bucket_dir_entry& ent,
                                                 RGWAccessControlPolicy* const bucket_acl,
...
static int iterate_slo_parts(const DoutPrefixProvider* dpp, CephContext* cct, rgw::sal::Driver* driver, off_t ofs,
                             off_t end, map<uint64_t, rgw_slo_part>& slo_parts,
                             int (*cb)(rgw::sal::Bucket* bucket, const rgw_bucket_dir_entry& ent,
        ...
        if (found_start) {
            if (cb) {
                ldpp_dout(dpp, 20) << "iterate_slo_parts()"
                                   << " obj=" << part.obj_name << " start_ofs=" << start_ofs << " end_ofs=" << end_ofs
                                   << dendl;
...
static int get_obj_user_manifest_iterate_cb(rgw::sal::Bucket* bucket, const rgw_bucket_dir_entry& ent,
                                            RGWAccessControlPolicy* const bucket_acl,
                                            const boost::optional<Policy>& bucket_policy, const off_t start_ofs,
...
int RGWGetObj::handle_slo_manifest(bufferlist& bl, optional_yield y)
{
    ...
    r = iterate_slo_parts(this, s->cct, driver, ofs, end, slo_parts, get_obj_user_manifest_iterate_cb, (void*)this);
    if (r < 0) {
        return r;
    }
...
{
    std::string script;
    const auto rc = rgw::lua::read_script(
        s, s->penv.lua.manager.get(), s->bucket_tenant, s->yield, rgw::lua::context::getData, script);
    if (rc == -ENOENT) {
...
static inline void rgw_cond_decode_objtags(req_state* s, const std::map<std::string, buffer::list>& attrs)
{
...
void RGWGetObj::execute(optional_yield y)
{
        ...
        try {
            ...
            for (const auto& zone : zones) {
                if (zone == dst_zone_trace) {
                    ...
                    ldpp_dout(this, 4) << "Object already has been copied to this destination. Returning " << op_ret
                                       << dendl;
                    goto done_err;
                }
            }
    ...
    if (get_type() == RGW_OP_GET_OBJ && get_data) {
        ...
        if (op_ret < 0) {
            ldpp_dout(this, 4) << "Cannot get cloud tiered object: " << *s->object << ". Failing with " << op_ret
                               << dendl;
...
int RGWListBuckets::verify_permission(optional_yield y)
{
    ...
    if (!verify_user_permission(
            this, s, ARN(partition, service, "", tenant, "*"), rgw::IAM::s3ListAllMyBuckets, false)) {
        return -EACCES;
    }
...
void RGWStatAccount::execute(optional_yield y)
{
    ...
    do {
        ...
        if (op_ret < 0) {
            /* hmm.. something wrong here.. the user was authenticated, so it
               should exist */
            ldpp_dout(this, 10) << "WARNING: failed on list_buckets uid=" << s->user->get_id() << " ret=" << op_ret
                                << dendl;
            break;
        }
        else {
...
int RGWCreateBucket::verify_permission(optional_yield y)
{
    ...
    if (s->user->get_max_buckets()) {
        rgw::sal::BucketList buckets;
        string marker;
        op_ret = s->user->list_buckets(this, marker, string(), s->user->get_max_buckets(), false, buckets, y);
        if (op_ret < 0) {
            return op_ret;
        }
...
void RGWCreateBucket::execute(optional_yield y)
{
    ...
    if (!relaxed_region_enforcement && !driver->get_zone()->get_zonegroup().is_master_zonegroup() &&
        !location_constraint.empty() && driver->get_zone()->get_zonegroup().get_api_name() != location_constraint) {
        ldpp_dout(this, 0) << "location constraint (" << location_constraint << ")"
    ...
    std::set<std::string> names;
    driver->get_zone()->get_zonegroup().get_placement_target_names(names);
    if (!placement_rule.name.empty() && !names.count(placement_rule.name)) {
    ...
    {
        std::unique_ptr<rgw::sal::Bucket> tmp_bucket;
        op_ret = driver->get_bucket(this, s->user.get(), s->bucket_tenant, s->bucket_name, &tmp_bucket, y);
        if (op_ret < 0 && op_ret != -ENOENT) return;
        s->bucket_exists = (op_ret != -ENOENT);

        if (s->bucket_exists) {
            if (!s->system_request &&
                driver->get_zone()->get_zonegroup().get_id() != tmp_bucket->get_info().zonegroup) {
                op_ret = -EEXIST;
                return;
            }
    ...
    op_ret = s->user->create_bucket(this,
                                    tmp_bucket,
                                    zonegroup_id,
                                    placement_rule,
...
#define VERSION_ID_STR "?versionId="
        ...
        ret = driver->get_bucket(this, s->user.get(), copy_source_tenant_name, copy_source_bucket_name, &bucket, y);
        if (ret < 0) {
...
int RGWPutObj::verify_permission(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        ...
        auto identity_policy_res = eval_identity_or_session_policies(
            this, s->iam_user_policies, s->env, rgw::IAM::s3PutObject, s->object->get_obj());
        ...
        if (!s->session_policies.empty()) {
            auto session_policy_res = eval_identity_or_session_policies(
                this, s->session_policies, s->env, rgw::IAM::s3PutObject, s->object->get_obj());
...
int RGWPutObj::get_lua_filter(std::unique_ptr<rgw::sal::DataProcessor>* filter, rgw::sal::DataProcessor* cb)
{
    std::string script;
    const auto rc = rgw::lua::read_script(
        s, s->penv.lua.manager.get(), s->bucket_tenant, s->yield, rgw::lua::context::putData, script);
    if (rc == -ENOENT) {
        // no script, nothing to do
        return 0;
    }
...
void RGWPutObj::execute(optional_yield y)
{
    ...
    std::unique_ptr<rgw::sal::Notification> res =
        driver->get_notification(s->object.get(), s->src_object.get(), s, rgw::notify::ObjectCreatedPut, y);
    if (!multipart) {
    ...
    if (multipart) {
        ...
        processor = upload->get_writer(this,
                                       s->yield,
                                       s->object.get(),
                                       s->user->get_id(),
                                       pdest_placement,
        ...
        processor = driver->get_append_writer(this,
                                              s->yield,
                                              s->object.get(),
        ...
        processor = driver->get_atomic_writer(
            this, s->yield, s->object.get(), s->bucket_owner.get_id(), pdest_placement, olh_epoch, s->req_id);
    }
        ...
        if (astate->get_attr(RGW_ATTR_MANIFEST, bl)) {
            ...
            try {
                ...
                if (m.get_tier_type() == "cloud-s3") {
                    op_ret = -ERR_INVALID_OBJECT_STATE;
                    s->err.message = "This object was transitioned to cloud-s3";
                    ldpp_dout(this, 4) << "Cannot copy cloud tiered object. Failing with " << op_ret << dendl;
                    return;
                }
            }
            catch (const buffer::end_of_buffer&) {
                ...
                ldpp_dout(this, 1) << "WARNING: failed to decode object manifest for " << *s->object << ": " << e.what()
                                   << dendl;
            }
        }
    ...
    if (!append) {   // compression and encryption only apply to full object uploads
        ...
        if (compression_type != "none" && (encrypt == nullptr || compress_encrypted)) {
...
void RGWPostObj::execute(optional_yield y)
{
    ...
    do {
        ...
        std::unique_ptr<rgw::sal::Object> obj = s->bucket->get_object(rgw_obj_key(get_current_filename()));
        if (s->bucket->versioning_enabled()) {
        ...
        std::unique_ptr<rgw::sal::Writer> processor;
        processor = driver->get_atomic_writer(
            this, s->yield, obj.get(), s->bucket_owner.get_id(), &s->dest_placement, 0, s->req_id);
        op_ret = processor->prepare(s->yield);
        if (op_ret < 0) {
            return;
        }
...
int RGWDeleteObj::handle_slo_manifest(bufferlist& bl, optional_yield y)
{
    ...
    try {
        deleter = std::unique_ptr<RGWBulkDelete::Deleter>(new RGWBulkDelete::Deleter(this, driver, s));
    }
...
int RGWDeleteObj::verify_permission(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        if (s->bucket->get_info().obj_lock_enabled() && bypass_governance_mode) {
            auto r = eval_identity_or_session_policies(this,
                                                       s->iam_user_policies,
                                                       s->env,
            ...
            if (r == Effect::Deny) {
                ...
                r = eval_identity_or_session_policies(this,
                                                      s->session_policies,
                                                      s->env,
                                                      rgw::IAM::s3BypassGovernanceRetention,
                                                      ARN(s->bucket->get_key(), s->object->get_name()));
...
void RGWDeleteObj::execute(optional_yield y)
{
    ...
    if (!rgw::sal::Object::empty(s->object.get())) {
        ...
        std::unique_ptr<rgw::sal::Notification> res =
            driver->get_notification(s->object.get(), s->src_object.get(), s, event_type, y);
        op_ret = res->publish_reserve(this);
        if (op_ret < 0) {
            return;
        }
...
int RGWCopyObj::init_processing(optional_yield y)
{
    ...
    op_ret =
        driver->get_bucket(this, s->user.get(), rgw_bucket_key(s->src_tenant_name, s->src_bucket_name), &src_bucket, y);
    if (op_ret < 0) {
        if (op_ret == -ENOENT) {
            op_ret = -ERR_NO_SUCH_BUCKET;
        }
...
int RGWCopyObj::verify_permission(optional_yield y)
{
        ...
        if (!s->auth.identity->is_admin_of(src_acl.get_owner().get_id())) {
            if (src_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
                ...
                auto identity_policy_res = eval_identity_or_session_policies(
                    ...
                    s->iam_user_policies,
                    s->env,
                    s->src_object->get_instance().empty() ? rgw::IAM::s3GetObject : rgw::IAM::s3GetObjectVersion,
                    obj_arn);
                if (identity_policy_res == Effect::Deny) {
                    return -EACCES;
                }
    ...
    op_ret = read_bucket_policy(
        this, driver, s, s->bucket->get_info(), s->bucket->get_attrs(), &dest_bucket_policy, s->bucket->get_key(), y);
    if (op_ret < 0) {
        return op_ret;
    }
    ...
    if (!s->auth.identity->is_admin_of(dest_policy.get_owner().get_id())) {
        if (dest_iam_policy != boost::none || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
            ...
            if (has_s3_resource_tag) rgw_iam_add_buckettags(this, s, s->bucket.get());
            ...
            auto identity_policy_res =
                eval_identity_or_session_policies(this, s->iam_user_policies, s->env, rgw::IAM::s3PutObject, obj_arn);
            if (identity_policy_res == Effect::Deny) {
                return -EACCES;
            }
            auto e = Effect::Pass;
            ...
            if (!s->session_policies.empty()) {
                auto session_policy_res = eval_identity_or_session_policies(
                    this, s->session_policies, s->env, rgw::IAM::s3PutObject, obj_arn);
                if (session_policy_res == Effect::Deny) {
                    return false;
                }
                if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
...
void RGWCopyObj::progress_cb(off_t ofs)
{
    ...
    if (ofs - last_ofs < static_cast<off_t>(s->cct->_conf->rgw_copy_obj_progress_every_bytes)) {
        return;
    }
...
void RGWCopyObj::execute(optional_yield y)
{
    ...
    // make reservation for notification if needed
    std::unique_ptr<rgw::sal::Notification> res =
        driver->get_notification(s->object.get(), s->src_object.get(), s, rgw::notify::ObjectCreatedCopy, y);
    op_ret = res->publish_reserve(this);
    if (op_ret < 0) {
        return;
    }
...
void RGWCompleteMultipart::execute(optional_yield y)
{
    ...
    // make reservation for notification if needed
    std::unique_ptr<rgw::sal::Notification> res = driver->get_notification(
        meta_obj.get(), nullptr, s, rgw::notify::ObjectCreatedCompleteMultipartUpload, y, &s->object->get_name());
    op_ret = res->publish_reserve(this);
    if (op_ret < 0) {
        return;
    }
...
bool RGWCompleteMultipart::check_previously_completed(const RGWMultiCompleteUpload* parts)
{
    ...
    for (const auto& [index, part] : parts->parts) {
        ...
        ldpp_dout(this, 20) << __func__ << "() re-calculating multipart etag: part: " << index << ", etag: " << partetag
                            << dendl;
    }
    ...
    if (oetag.compare(final_etag_str) != 0) {
        ldpp_dout(this, 1) << __func__ << "() NOTICE: etag mismatch: object etag:" << oetag
...
int RGWAbortMultipart::verify_permission(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        auto identity_policy_res = eval_identity_or_session_policies(
            this, s->iam_user_policies, s->env, rgw::IAM::s3AbortMultipartUpload, s->object->get_obj());
        if (identity_policy_res == Effect::Deny) {
            return -EACCES;
        }
...
void RGWAbortMultipart::execute(optional_yield y)
{
    ...
    if (upload_id.empty() || rgw::sal::Object::empty(s->object.get())) return;
...
void RGWListBucketMultiparts::execute(optional_yield y)
{
    ...
    op_ret = s->bucket->list_multiparts(
        this, prefix, marker_meta, delimiter, max_uploads, uploads, &common_prefixes, &is_truncated);
    if (op_ret < 0) {
        return;
    }
...
int RGWDeleteMultiObj::verify_permission(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        if (s->bucket->get_info().obj_lock_enabled() && bypass_governance_mode) {
            ...
            auto r = eval_identity_or_session_policies(
                this, s->iam_user_policies, s->env, rgw::IAM::s3BypassGovernanceRetention, ARN(s->bucket->get_key()));
            if (r == Effect::Deny) {
                bypass_perm = false;
            }
            else if (r == Effect::Pass && s->iam_policy) {
...
std::pair<std::string, std::string> RGWBulkUploadOp::handle_upload_path(req_state* s)
{
...
int RGWBulkUploadOp::handle_dir_verify_permission(optional_yield y)
{
    if (s->user->get_max_buckets() > 0) {
        rgw::sal::BucketList buckets;
        std::string marker;
        op_ret = s->user->list_buckets(this, marker, std::string(), s->user->get_max_buckets(), false, buckets, y);
        if (op_ret < 0) {
            return op_ret;
        }
...
void RGWBulkUploadOp::init(rgw::sal::Driver* const driver, req_state* const s, RGWHandler* const h)
{
                                                    ...
                                                    ACLOwner& bucket_owner /* out */, optional_yield y)
{
    ...
    if (policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        auto identity_policy_res =
            eval_identity_or_session_policies(this, s->iam_user_policies, s->env, rgw::IAM::s3PutObject, obj);
        if (identity_policy_res == Effect::Deny) {
            return false;
        }
        ...
        if (!s->session_policies.empty()) {
            auto session_policy_res =
                eval_identity_or_session_policies(this, s->session_policies, s->env, rgw::IAM::s3PutObject, obj);
            if (session_policy_res == Effect::Deny) {
                return false;
            }
            if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
...
int RGWHandler::init(rgw::sal::Driver* _driver, req_state* _s, rgw::io::BasicClient* cio)
{
    driver = _driver;
    s = _s;
...
void RGWPutBucketPolicy::execute(optional_yield y)
{
    ...
    try {
        const Policy p(
            s->cct, s->bucket_tenant, data, s->cct->_conf.get_val<bool>("rgw_policy_reject_invalid_principals"));
        rgw::sal::Attrs attrs(s->bucket_attrs);
...
void RGWGetBucketEncryption::execute(optional_yield y)
{
    ...
    if (auto aiter = attrs.find(RGW_ATTR_BUCKET_ENCRYPTION_POLICY); aiter == attrs.end()) {
