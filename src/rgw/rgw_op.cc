    ...
    if (ret == -ENOENT) {
        ...
        if (bucket_owner.compare(s->user->get_id()) != 0 && !s->auth.identity->is_admin_of(bucket_owner)) {
            auto r = eval_identity_or_session_policies(
                s->iam_user_policies, s->env, rgw::IAM::s3ListBucket, ARN(bucket->get_key()));
            ...
            if (!s->session_policies.empty()) {
                r = eval_identity_or_session_policies(
                    s->session_policies, s->env, rgw::IAM::s3ListBucket, ARN(bucket->get_key()));
                if (r == Effect::Allow) return -ENOENT;
                if (r == Effect::Deny) return -EACCES;
            }
...
int RGWPutObj::verify_permission(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        ...
        auto identity_policy_res = eval_identity_or_session_policies(
            s->iam_user_policies, s->env, rgw::IAM::s3PutObject, s->object->get_obj());
        ...
        if (!s->session_policies.empty()) {
            auto session_policy_res = eval_identity_or_session_policies(
                s->session_policies, s->env, rgw::IAM::s3PutObject, s->object->get_obj());
    ...
    if (!s->auth.identity->is_admin_of(dest_policy.get_owner().get_id())) {
        if (dest_iam_policy != boost::none || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
            ...
            auto identity_policy_res =
                eval_identity_or_session_policies(s->iam_user_policies, s->env, rgw::IAM::s3PutObject, obj_arn);
            if (identity_policy_res == Effect::Deny) {
                return -EACCES;
            }
            auto e = Effect::Pass;
            ...
            if (!s->session_policies.empty()) {
                auto session_policy_res = eval_identity_or_session_policies(
                    s->session_policies, s->env, rgw::IAM::s3PutObject, ARN(dest_object->get_obj()));
                if (session_policy_res == Effect::Deny) {
                    return false;
                }
                if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
...
int RGWAbortMultipart::verify_permission(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        auto identity_policy_res = eval_identity_or_session_policies(
            s->iam_user_policies, s->env, rgw::IAM::s3AbortMultipartUpload, s->object->get_obj());
        if (identity_policy_res == Effect::Deny) {
            return -EACCES;
        }
...
int RGWDeleteMultiObj::verify_permission(optional_yield y)
{
    ...
    if (s->iam_policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        if (s->bucket->get_info().obj_lock_enabled() && bypass_governance_mode) {
            ...
            auto r = eval_identity_or_session_policies(
                s->iam_user_policies, s->env, rgw::IAM::s3BypassGovernanceRetention, bucket_arn);
            if (r == Effect::Deny) {
                bypass_perm = false;
            }
            else if (r == Effect::Pass && s->iam_policy) {
                r = s->iam_policy->eval(s->env, *s->auth.identity, rgw::IAM::s3BypassGovernanceRetention, bucket_arn);
                if (r == Effect::Deny) {
                    bypass_perm = false;
                }
            }
                ...
                r = eval_identity_or_session_policies(
                    s->session_policies, s->env, rgw::IAM::s3BypassGovernanceRetention, ARN(s->bucket->get_key()));
                if (r == Effect::Deny) {
                    bypass_perm = false;
                }
            }
                                                    ...
                                                    ACLOwner& bucket_owner /* out */, optional_yield y)
{
    ...
    if (policy || !s->iam_user_policies.empty() || !s->session_policies.empty()) {
        auto identity_policy_res =
            eval_identity_or_session_policies(s->iam_user_policies, s->env, rgw::IAM::s3PutObject, obj);
        if (identity_policy_res == Effect::Deny) {
            return false;
        }
        ...
        if (!s->session_policies.empty()) {
            auto session_policy_res =
                eval_identity_or_session_policies(s->session_policies, s->env, rgw::IAM::s3PutObject, obj);
            if (session_policy_res == Effect::Deny) {
                return false;
            }
            if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
