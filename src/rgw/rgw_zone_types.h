...
public:
    ...
    const RGWZoneStorageClass& get_standard() const { return *standard_class; }
    ...
    const std::map<std::string, RGWZoneStorageClass>& get_all() const { return m; }

    std::map<std::string, RGWZoneStorageClass>& get_all() { return m; }
...
struct RGWZonePlacementInfo
{
    ...
    const rgw_pool& get_standard_data_pool() const { return get_data_pool(RGW_STORAGE_CLASS_STANDARD); }
    ...
    bool storage_class_exists(const std::string& sc) const { return storage_classes.exists(sc); }
...
struct RGWZone
{
    ...
    bool supports(std::string_view feature) const { return supported_features.contains(feature); }
};
...
struct RGWTierACLMapping
{
    ...
    RGWTierACLMapping(ACLGranteeTypeEnum t, const std::string& s, const std::string& d)
        : type(t)
        , source_id(s)
        , dest_id(d)
    {}
