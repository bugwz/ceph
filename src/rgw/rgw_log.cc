...
string render_log_object_name(const string& format, struct tm* dt, const string& bucket_id, const string& bucket_name)
{
    string o;
    for (unsigned i = 0; i < format.size(); i++) {
        if (format[i] == '%' && i + 1 < format.size()) {
            ...
            case 'Y': sprintf(buf, "%.4d", dt->tm_year + 1900); break;
            case 'y': sprintf(buf, "%.2d", dt->tm_year % 100); break;
            case 'm': sprintf(buf, "%.2d", dt->tm_mon + 1); break;
            case 'd': sprintf(buf, "%.2d", dt->tm_mday); break;
            case 'H': sprintf(buf, "%.2d", dt->tm_hour); break;
            case 'I': sprintf(buf, "%.2d", (dt->tm_hour % 12) + 1); break;
            case 'k': sprintf(buf, "%d", dt->tm_hour); break;
            case 'l': sprintf(buf, "%d", (dt->tm_hour % 12) + 1); break;
            case 'M': sprintf(buf, "%.2d", dt->tm_min); break;

            case 'i': o += bucket_id; continue;
            case 'n': o += bucket_name; continue;
            default:
                // unknown code
...
class UsageLogger : public DoutPrefixProvider
{
    ...
    public:
        explicit C_UsageLogTimeout(UsageLogger* _l)
            : logger(_l)
        {}
    ...
    void set_timer() { timer.add_event_after(cct->_conf->rgw_usage_log_tick_interval, new C_UsageLogTimeout(this)); }

public:
    UsageLogger(CephContext* _cct, rgw::sal::Driver* _driver)
        : cct(_cct)
        , driver(_driver)
        , num_entries(0)
    ...
    void recalc_round_timestamp(utime_t& ts) { round_timestamp = ts.round_to_hour(); }

    void insert_user(utime_t& timestamp, const rgw_user& user, rgw_usage_log_entry& entry)
    {
        lock.lock();
        if (timestamp.sec() > round_timestamp + 3600) recalc_round_timestamp(timestamp);
...
static void log_usage(req_state* s, const string& op_name)
{
    ...
    if (!bucket_name.empty()) {
        ...
        if (!rgw::sal::Bucket::empty(s->bucket.get()) && s->bucket->get_info().requester_pays) {
            payer = s->user->get_id();
        }
    }
    else {
    ...
    data.ops = 1;
    if (!s->is_err()) data.successful_ops = 1;

    ldpp_dout(s, 30) << "log_usage: bucket_name=" << bucket_name << " tenant=" << s->bucket_tenant
                     << ", bytes_sent=" << bytes_sent << ", bytes_received=" << bytes_received
                     << ", success=" << data.successful_ops << dendl;
...
void rgw_format_ops_log_entry(struct rgw_log_entry& entry, Formatter* formatter)
{
    ...
    if (obj_owner.length()) formatter->dump_string("object_owner", obj_owner);
    ...
    case TYPE_RGW: formatter->dump_string("authentication_type", "Local"); break;
    case TYPE_LDAP: formatter->dump_string("authentication_type", "LDAP"); break;
    case TYPE_KEYSTONE: formatter->dump_string("authentication_type", "Keystone"); break;
    case TYPE_WEB: formatter->dump_string("authentication_type", "OIDC Provider"); break;
    case TYPE_ROLE: formatter->dump_string("authentication_type", "STS"); break;
    default: break;
    }
...
OpsLogSocket::OpsLogSocket(CephContext* cct, uint64_t _backlog)
    : OutputDataSocket(cct, _backlog)
{
    delim.append(",\n");
}
...
OpsLogRados::OpsLogRados(rgw::sal::Driver* const& driver)
    : driver(driver)
{}
...
int OpsLogRados::log(req_state* s, struct rgw_log_entry& entry)
{
    ...
    if (s->cct->_conf->rgw_log_object_name_utc)
        ...
        localtime_r(&t, &bdt);
    string oid = render_log_object_name(s->cct->_conf->rgw_log_object_name, &bdt, entry.bucket_id, entry.bucket);
...
int rgw_log_op(RGWREST* const rest, req_state* s, const RGWOp* op, OpsLogSink* olog)
{
    ...
    if (s->enable_usage_log) log_usage(s, op_name);

    if (!s->enable_ops_log) return 0;
        ...
        if (rest->log_x_headers()) {
            for (const auto& iter : s->info.env->get_map()) {
                if (rest->log_x_header(iter.first)) {
                    entry.x_headers.insert(rgw_log_entry::headers_map::value_type(iter.first, iter.second));
                }
            }
        }
    }
    ...
    if (s->object_acl) entry.object_owner = s->object_acl->get_owner().get_id();
