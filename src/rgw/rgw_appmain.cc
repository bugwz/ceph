...
namespace {
TracepointProvider::Traits rgw_op_tracepoint_traits("librgw_op_tp.so", "rgw_op_tracing");
TracepointProvider::Traits rgw_rados_tracepoint_traits("librgw_rados_tp.so", "rgw_rados_tracing");
}   // namespace
...
void rgw::AppMain::init_frontends1(bool nfs)
{
    ...
    for (auto& f : frontends) {
        if (f.find("beast") != string::npos) {
            ...
            if (f.find("port") != string::npos) {
                ...
                if ((f.find("port=") == string::npos) || (f.find("port= ") != string::npos)) {
                    derr <<
    ...
    before or after '='))"
        ...
        fe_map.insert(pair<string, RGWFrontendConfig*>(config->get_framework(), config));
    } /* for each frontend */
...
{
    auto run_gc = (g_conf()->rgw_enable_gc_threads && ((!nfs) || (nfs && g_conf()->rgw_nfs_run_gc_threads)));

    auto run_lc = (g_conf()->rgw_enable_lc_threads && ((!nfs) || (nfs && g_conf()->rgw_nfs_run_lc_threads)));

    auto run_quota = (g_conf()->rgw_enable_quota_threads && ((!nfs) || (nfs && g_conf()->rgw_nfs_run_quota_threads)));

    auto run_sync = (g_conf()->rgw_run_sync_thread && ((!nfs) || (nfs && g_conf()->rgw_nfs_run_sync_thread)));

    DriverManager::Config cfg = DriverManager::get_config(false, g_ceph_context);
    env.driver = DriverManager::get_storage(dpp,
                                            dpp->get_cct(),
                                            cfg,
                                            run_gc,
                                            run_lc,
...
void rgw::AppMain::cond_init_apis()
{
    ...
    if (have_http_frontend) {
        ...
        if (!(g_ceph_context->_conf->rgw_keystone_admin_token.empty() ||
            ...
            dout(0) << "WARNING: rgw_keystone_admin_token and "
        ...
        if (apis_map.count("swift") > 0) {
            ...
            if (!g_conf()->rgw_cross_domain_policy.empty()) {
                swift_resource->register_resource("crossdomain.xml", set_logging(new RGWRESTMgr_SWIFT_CrossDomain));
            }

            swift_resource->register_resource("healthcheck", set_logging(new RGWRESTMgr_SWIFT_HealthCheck));

            swift_resource->register_resource("info", set_logging(new RGWRESTMgr_SWIFT_Info));

            if (!swift_at_root) {
                rest.register_resource(g_conf()->rgw_swift_url_prefix,
                                       set_logging(rest_filter(env.driver, RGW_REST_SWIFT, swift_resource)));
            }
            else {
...
void rgw::AppMain::init_ldap()
{
    ...
    ldh.reset(
        new rgw::LDAPHelper(ldap_uri, ldap_binddn, ldap_bindpw.c_str(), ldap_searchdn, ldap_searchfilter, ldap_dnattr));
    ldh->init();
    ldh->bind();
} /* init_ldap */
...
void rgw::AppMain::init_opslog()
{
    ...
    if (!g_conf()->rgw_ops_log_file_path.empty()) {
        ops_log_file =
            new OpsLogFile(g_ceph_context, g_conf()->rgw_ops_log_file_path, g_conf()->rgw_ops_log_data_backlog);
        ops_log_file->start();
...
int rgw::AppMain::init_frontends2(RGWLib* rgwlib)
{
    ...
    std::string frontend_defs_str = g_conf().get_val<string>("rgw_frontend_defaults");
    ...
    env.rest = &rest;
    env.olog = olog;
    env.auth_registry = rgw::auth::StrategyRegistry::create(dpp->get_cct(), *implicit_tenant_context, env.driver);
    env.ratelimiting = ratelimiter.get();

    int fe_count = 0;
    for (multimap<string, RGWFrontendConfig*>::iterator fiter = fe_map.begin(); fiter != fe_map.end();
         ++fiter, ++fe_count) {
    ...
    if (env.driver->get_name() == "rados") {
        ...
        reloader =
            std::make_unique<RGWRealmReloader>(env, *implicit_tenant_context, service_map_meta, rgw_pauser.get());
        realm_watcher = std::make_unique<RGWRealmWatcher>(
            dpp, g_ceph_context, static_cast<rgw::sal::RadosStore*>(env.driver)->svc()->zone->get_realm());
...
void rgw::AppMain::init_lua()
{
    ...
    std::string output;
    r = rgw::lua::install_packages(dpp, driver, null_yield, path, failed_packages, output);
    if (r < 0) {
    ...
    if (driver->get_name() == "rados") { /* Supported for only RadosStore */
        lua_background = std::make_unique<rgw::lua::Background>(driver, dpp->get_cct(), path);
