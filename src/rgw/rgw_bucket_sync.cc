...
template<typename CB>
void rgw_sync_group_pipe_map::init(CephContext* cct, const rgw_zone_id& _zone, std::optional<rgw_bucket> _bucket,
                                   const rgw_sync_policy_group& group, rgw_sync_data_flow_group* _default_flow,
                                   std::set<rgw_zone_id>* _pall_zones, CB filter_cb)
{
    zone = _zone;
    bucket = _bucket;
    ...
    for (auto& pipe : group.pipes) {
        if (pipe.contains_zone_bucket(zone, bucket)) {
            ldout(cct, 20) << __func__ << "(): pipe_map (zone=" << zone << " bucket=" << bucket_key
    ...
    std::optional<string>* storage_class, rgw_sync_pipe_params::Mode* mode, bool* need_more_info) const
{
    ...
    *need_more_info = false;
    ...
    for (auto& iter : iters) {
        ...
        conflict = !(_user == rule_params.user && _acl_translation == rule_params.dest.acl_translation &&
...

{
    ...
    for (auto& item : flow_groups) {
        ...
        for (auto& entry : flow_group_map.sources) {
            ...
            ldout(cct, 20) << __func__ << "(): flow manager (bucket=" << effective_bucket_key
...
int RGWBucketSyncPolicyHandler::init(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    reflect(&source_pipes, &target_pipes, &sources, &targets, &source_zones, &target_zones, true);
