...
bool verify_transport_security(CephContext* cct, const RGWEnv& env)
{
    ...
    if (!is_secure && g_conf().get_val<bool>("rgw_allow_notification_secrets_in_cleartext")) {
        ldout(cct, 0) << "WARNING: bypassing endpoint validation, allows sending secrets over insecure transport"
                      << dendl;
        return true;
    }
    return is_secure;
...
void RGWPSGetTopicOp::execute(optional_yield y)
{
    ...
    if (topic_has_endpoint_secret(result) && !verify_transport_security(s->cct, *(s->info.env))) {
        ldpp_dout(this, 1) << "topic '" << topic_name << "' contain secret and cannot be sent over insecure transport"
                           << dendl;
        op_ret = -EPERM;
        return;
    }
...
int remove_notification_by_topic(const DoutPrefixProvider* dpp, const std::string& topic_name,
                                 const RGWPubSub::Bucket& b, optional_yield y, const RGWPubSub& ps)
{
...
int delete_all_notifications(const DoutPrefixProvider* dpp, const rgw_pubsub_bucket_topics& bucket_topics,
                             const RGWPubSub::Bucket& b, optional_yield y, const RGWPubSub& ps)
{
...
void RGWPSCreateNotifOp::execute(optional_yield y)
{
    ...
    if (configurations.list.empty()) {
        ...
        if (op_ret < 0) {
            ldpp_dout(this, 1) << "failed to get list of topics from bucket '" << s->bucket_name << "', ret=" << op_ret
                               << dendl;
            return;
        }
