...
public:
    int get_params() override
    {
        ...
        if (!dest.push_endpoint.empty() && dest.persistent) {
            ...
            if (ret < 0) {
                ldpp_dout(this, 1) << "CreateTopic Action failed to create queue for persistent topics. error:" << ret
                                   << dendl;
                return ret;
            }
        }
        ...
        const rgw::ARN arn(rgw::Partition::aws,
                           rgw::Service::sns,
...
public:
    int get_params() override
    {
        ...
        if (ret < 0) {
            ldpp_dout(this, 1) << "DeleteTopic Action failed to remove queue for persistent topics. error:" << ret
                               << dendl;
            return ret;
        }
...
RGWOp* RGWHandler_REST_PSTopic_AWS::op_post()
{
    ...
    if (s->info.args.exists("Action")) {
        ...
        if (action.compare("CreateTopic") == 0) return new RGWPSCreateTopic_ObjStore_AWS();
        if (action.compare("DeleteTopic") == 0) return new RGWPSDeleteTopic_ObjStore_AWS;
        if (action.compare("ListTopics") == 0) return new RGWPSListTopics_ObjStore_AWS();
        if (action.compare("GetTopic") == 0) return new RGWPSGetTopic_ObjStore_AWS();
        if (action.compare("GetTopicAttributes") == 0) return new RGWPSGetTopicAttributes_ObjStore_AWS();
    }
...
{
    auto it = std::find_if(bucket_topics.topics.begin(), bucket_topics.topics.end(), [&](const auto& val) {
        return notif_name == val.second.s3_id;
    });
...
void RGWPSCreateNotif_ObjStore_S3::execute(optional_yield y)
{
    ...
    for (const auto& c : configurations.list) {
        ...
        if (!arn || arn->resource.empty()) {
            ldpp_dout(this, 1) << "topic ARN has invalid format: '" << c.topic_arn << "' in notification: '"
                               << notif_name << "'" << dendl;
            op_ret = -EINVAL;
            return;
        }
        ...
        if (op_ret < 0) {
            ldpp_dout(this, 1) << "failed to auto-generate unique topic '" << unique_topic_name << "', ret=" << op_ret
                               << dendl;
            return;
        }
        ...
        ldpp_dout(this, 20) << "successfully auto-generated notification for unique topic '" << unique_topic_name << "'"
                            << dendl;
        ...
        if (!push_only) {
            ...
            if (op_ret < 0) {
                ldpp_dout(this, 1) << "failed to auto-generate subscription '" << notif_name << "', ret=" << op_ret
                                   << dendl;
...
private:
    ...
    void remove_notification_by_topic(const std::string& topic_name, const RGWPubSub::BucketRef& b, optional_yield y)
    {
        ...
        if (op_ret < 0) {
            ldpp_dout(this, 1) << "failed to remove notification of topic '" << topic_name << "', ret=" << op_ret
                               << dendl;
        }
        op_ret = ps->remove_topic(this, topic_name, y);
        if (op_ret < 0) {
            ldpp_dout(this, 1) << "failed to remove auto-generated topic '" << topic_name << "', ret=" << op_ret
                               << dendl;
        }
    }
...
void RGWPSDeleteNotif_ObjStore_S3::execute(optional_yield y)
{
    ...
    if (op_ret < 0) {
        ldpp_dout(this, 1) << "failed to get list of topics from bucket '" << bucket_info.bucket.name
                           << "', ret=" << op_ret << dendl;
        return;
    }
    ...
    if (!notif_name.empty()) {
        ...
        if (unique_topic) {
            ...
            if (op_ret < 0 && op_ret != -ENOENT) {
                ldpp_dout(this, 1) << "failed to remove auto-generated subscription '" << notif_name
                                   << "', ret=" << op_ret << dendl;
                return;
            }
    ...
    for (const auto& topic : bucket_topics.topics) {
        ...
        for (const auto& topic_sub_name : topic_subs.subs) {
            ...
            if (op_ret < 0) {
                ldpp_dout(this, 1) << "failed to get subscription '" << topic_sub_name << "' info, ret=" << op_ret
                                   << dendl;
                return;
            }
            if (!sub_conf.s3_id.empty()) {
                ...
                if (op_ret < 0) {
                    ldpp_dout(this, 1) << "failed to remove auto-generated subscription '" << topic_sub_name
                                       << "', ret=" << op_ret << dendl;
                    return;
                }
            }
