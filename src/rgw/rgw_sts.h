...
public:
    AssumeRoleWithWebIdentityRequest(CephContext* cct, const std::string& duration, const std::string& providerId,
                                     const std::string& iamPolicy, const std::string& roleArn,
                                     const std::string& roleSessionName, const std::string& iss, const std::string& sub,
...
public:
    AssumeRoleRequest(CephContext* cct, const std::string& duration, const std::string& externalId,
                      const std::string& iamPolicy, const std::string& roleArn, const std::string& roleSessionName,
                      const std::string& serialNumber, const std::string& tokenCode)
        : AssumeRoleRequestBase(cct, duration, iamPolicy, roleArn, roleSessionName)
...
public:
    int generateAssumedRoleUser(CephContext* cct, rgw::sal::Driver* driver, const std::string& roleId,
                                const rgw::ARN& roleArn, const std::string& roleSessionName);
...
public:
    int generateCredentials(const DoutPrefixProvider* dpp, CephContext* cct, const uint64_t& duration,
...
public:
    STSService() = default;
    STSService(CephContext* cct, rgw::sal::Driver* driver, rgw_user user_id, rgw::auth::Identity* identity)
        : cct(cct)
        , driver(driver)
        , user_id(user_id)
        , identity(identity)
    {}
    std::tuple<int, rgw::sal::RGWRole*> getRoleInfo(const DoutPrefixProvider* dpp, const std::string& arn,
                                                    optional_yield y);
    ...
    AssumeRoleWithWebIdentityResponse assumeRoleWithWebIdentity(const DoutPrefixProvider* dpp,
