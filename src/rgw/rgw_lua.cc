...
{
    switch (ctx) {
    case context::preRequest: return "prerequest";
    case context::postRequest: return "postrequest";
...
int read_script(rgw::sal::RGWRadosStore* store, const std::string& tenant, optional_yield y, context ctx,
                std::string& script)
{
    ...
    const auto rc = rgw_get_system_obj(obj_ctx, obj.pool, obj.oid, bl, &objv_tracker, nullptr, y, nullptr, nullptr);
...
int write_script(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, const std::string& tenant,
                 optional_yield y, context ctx, const std::string& script)
{
    ...
    const auto rc = rgw_put_system_obj(dpp, obj_ctx, obj.pool, obj.oid, bl, false, &objv_tracker, real_time(), y);
...
int delete_script(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, const std::string& tenant,
                  optional_yield y, context ctx)
{
    ...
    const auto rc = rgw_delete_system_obj(dpp, store->svc()->sysobj, obj.pool, obj.oid, &objv_tracker, y);
...
int add_package(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, optional_yield y,
...
int remove_package(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, optional_yield y,
                   const std::string& package_name)
{
    librados::ObjectWriteOperation op;
    op.omap_rm_keys(std::set<std::string>({package_name}));
    const auto ret = rgw_rados_operate(dpp, *(store->getRados()->get_lc_pool_ctx()), PACKAGE_LIST_OBJECT_NAME, &op, y);
...
int install_packages(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, optional_yield y,
                     packages_t& failed_packages, std::string& output)
{
    ...
    for (const auto& package : packages) {
        ...
        std::string line = "CMD: luarocks install --lua-version " + std::string(CEPH_LUA_VERSION) +
                           std::string(" --tree ") + luarocks_path + " --deps-mode one " + package;
