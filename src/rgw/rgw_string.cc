...
bool match_wildcards(std::string_view pattern, std::string_view input, uint32_t flags)
{
    ...
    while (true) {
        if (it1 == pattern.end()) return it2 == input.end();
        if (*it1 == '*') {
            if (it1 + 1 == pattern.end()) return true;
