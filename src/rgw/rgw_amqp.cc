...
struct connection_id_t
{
    ...
    struct hasher
    {
        ...
        {
            return ((std::hash<std::string>()(k.host) ^ (std::hash<int>()(k.port) << 1)) >> 1) ^
...
public:
    ConnectionCleaner(amqp_connection_state_t _conn)
        : conn(_conn)
    {}
    ~ConnectionCleaner()
    {
...
connection_ptr_t create_new_connection(const amqp_connection_info& info, const std::string& exchange,
                                       bool mandatory_delivery, CephContext* cct, bool verify_ssl,
                                       boost::optional<const std::string&> ca_location)
{
...
struct message_wrapper_t
{
    ...
    message_wrapper_t(connection_ptr_t& _conn, const std::string& _topic, const std::string& _message,
    ...
    void publish_internal(message_wrapper_t* message)
    {
        ...
        if (rc == AMQP_STATUS_OK) {
            ...
            if (q_len < max_inflight) {
                ldout(conn->cct, 20) << "AMQP publish (with callback, tag=" << conn->delivery_tag
            ...
            for (; conn_it != end_it;) {
                ...
                if (!conn->is_ok()) {
                    ...
                    if (now >= conn->next_reconnect) {
                        ...
                        if (create_connection(conn, info)->is_ok() == false) {
                            ldout(conn->cct, 10) << "AMQP run: connection (" << to_string(conn_key)
                            ...
                            ldout(conn->cct, 10)
                                << "AMQP run: connection (" << to_string(conn_key) << ") retry successfull" << dendl;
                        }
                    }
                ...
                if (tag_it != callbacks_end) {
                    if (multiple) {
                        ...
                        ldout(conn->cct, 20) << "AMQP run: multiple n/acks received with tag=" << tag
                        ...
                        ldout(conn->cct, 20) << "AMQP run: n/ack received, invoking callback with tag=" << tag
                                             << " and result=" << result << dendl;
                        tag_it->cb(result);
...
public:
                             ...
                             bool verify_ssl, boost::optional<const std::string&> ca_location)
    {
        ...
        if (!conn->is_ok()) {
            ldout(cct, 10) << "AMQP connect: connection (" << to_string(id)
                           << ") creation failed. error:" << status_to_string(conn->status) << "(" << conn->reply_code
                           << ")" << dendl;
        }
    ...
    int publish_with_confirm(connection_ptr_t& conn, const std::string& topic, const std::string& message,
                             reply_callback_t cb)
    {
        if (stopped) {
