...
{
    return lhs.host == rhs.host && lhs.port == rhs.port && lhs.vhost == rhs.vhost && lhs.exchange == rhs.exchange;
}
...
{
    return fmt::format("{}://{}:{}{}?exchange={}", id.ssl ? "amqps" : "amqp", id.host, id.port, id.vhost, id.exchange);
}
...
public:
    ConnectionCleaner(amqp_connection_state_t _state)
        : state(_state)
    {}
    ~ConnectionCleaner()
    {
...
struct reply_callback_with_tag_t
{
    ...
    reply_callback_with_tag_t(uint64_t _tag, reply_callback_t _cb)
        : tag(_tag)
        , cb(_cb)
    {}

    bool operator==(uint64_t rhs) { return tag == rhs; }
};
...
struct connection_t
{
    ...
    connection_t(CephContext* _cct, const amqp_connection_info& info, bool _verify_ssl,
    ...
    bool is_ok() const { return (state != nullptr); }

    // dtor also destroys the internals
    ~connection_t() { destroy(RGW_AMQP_STATUS_CONNECTION_CLOSED); }
};
...
int reply_to_code(const amqp_rpc_reply_t& reply)
{
    ...
    case AMQP_RESPONSE_NORMAL: return RGW_AMQP_NO_REPLY_CODE;
    case AMQP_RESPONSE_LIBRARY_EXCEPTION: return reply.library_error;
...
std::string to_string(const amqp_rpc_reply_t& reply)
{
    ...
    case AMQP_RESPONSE_LIBRARY_EXCEPTION: return amqp_error_string2(reply.library_error);
    case AMQP_RESPONSE_SERVER_EXCEPTION:
    {
        switch (reply.reply.id) {
        case AMQP_CONNECTION_CLOSE_METHOD: ss << "server connection error: "; break;
        case AMQP_CHANNEL_CLOSE_METHOD: ss << "server channel error: "; break;
        default: ss << "server unknown error: "; break;
        }
        if (reply.reply.decoded) {
    ...
    default: ss << "unknown error, method id: " << reply.reply.id; return ss.str();
    }
}
...
std::string to_string(amqp_status_enum s)
{
    ...
    case AMQP_STATUS_SOCKET_CLOSED: return "AMQP_STATUS_SOCKET_CLOSED";
    case AMQP_STATUS_SOCKET_INUSE: return "AMQP_STATUS_SOCKET_INUSE";
    case AMQP_STATUS_BROKER_UNSUPPORTED_SASL_METHOD: return "AMQP_STATUS_BROKER_UNSUPPORTED_SASL_METHOD";
#if AMQP_VERSION >= AMQP_VERSION_CODE(0, 8, 0, 0)
    case AMQP_STATUS_UNSUPPORTED: return "AMQP_STATUS_UNSUPPORTED";
#endif
    case _AMQP_STATUS_NEXT_VALUE: return "AMQP_STATUS_INTERNAL";
    ...
    case AMQP_STATUS_SSL_SET_ENGINE_FAILED: return "AMQP_STATUS_SSL_SET_ENGINE_FAILED";
#endif
    default: return "AMQP_STATUS_UNKNOWN";
    }
}
...
{
    switch (s) {
    case RGW_AMQP_STATUS_BROKER_NACK: return "RGW_AMQP_STATUS_BROKER_NACK";
    case RGW_AMQP_STATUS_CONNECTION_CLOSED: return "RGW_AMQP_STATUS_CONNECTION_CLOSED";
    case RGW_AMQP_STATUS_QUEUE_FULL: return "RGW_AMQP_STATUS_QUEUE_FULL";
    case RGW_AMQP_STATUS_MAX_INFLIGHT: return "RGW_AMQP_STATUS_MAX_INFLIGHT";
    case RGW_AMQP_STATUS_MANAGER_STOPPED: return "RGW_AMQP_STATUS_MANAGER_STOPPED";
    case RGW_AMQP_STATUS_CONN_ALLOC_FAILED: return "RGW_AMQP_STATUS_CONN_ALLOC_FAILED";
    case RGW_AMQP_STATUS_SOCKET_ALLOC_FAILED: return "RGW_AMQP_STATUS_SOCKET_ALLOC_FAILED";
    case RGW_AMQP_STATUS_SOCKET_OPEN_FAILED: return "RGW_AMQP_STATUS_SOCKET_OPEN_FAILED";
    case RGW_AMQP_STATUS_LOGIN_FAILED: return "RGW_AMQP_STATUS_LOGIN_FAILED";
    case RGW_AMQP_STATUS_CHANNEL_OPEN_FAILED: return "RGW_AMQP_STATUS_CHANNEL_OPEN_FAILED";
    case RGW_AMQP_STATUS_VERIFY_EXCHANGE_FAILED: return "RGW_AMQP_STATUS_VERIFY_EXCHANGE_FAILED";
    case RGW_AMQP_STATUS_Q_DECLARE_FAILED: return "RGW_AMQP_STATUS_Q_DECLARE_FAILED";
    case RGW_AMQP_STATUS_CONFIRM_DECLARE_FAILED: return "RGW_AMQP_STATUS_CONFIRM_DECLARE_FAILED";
    case RGW_AMQP_STATUS_CONSUME_DECLARE_FAILED: return "RGW_AMQP_STATUS_CONSUME_DECLARE_FAILED";
    case RGW_AMQP_STATUS_SOCKET_CACERT_FAILED: return "RGW_AMQP_STATUS_SOCKET_CACERT_FAILED";
    }
...
struct message_wrapper_t
{
    ...
    message_wrapper_t(const connection_id_t& _conn_id, const std::string& _topic, const std::string& _message,
...
private:
    ...
    void publish_internal(message_wrapper_t* message)
    {
        ...
        if (rc == AMQP_STATUS_OK) {
            ...
            if (q_len < max_inflight) {
                ldout(cct, 20) << "AMQP publish (with callback, tag=" << conn->delivery_tag
                               << "): OK. Queue has: " << q_len << " callbacks" << dendl;
        ...
        while (!stopped) {
            ...
            for (; conn_it != end_it;) {
                ...
                if (conn->timestamp.sec() + max_idle_time < ceph_clock_now()) {
                    ldout(cct, 20) << "AMQP run: Time for deleting a connection due to idle behaviour: "
                                   << ceph_clock_now() << dendl;
                ...
                if (!conn->is_ok()) {
                    ...
                    if (now >= conn->next_reconnect) {
                        ...
                        if (!new_state(conn.get(), conn_id)) {
                            ldout(cct, 10) << "AMQP run: connection '" << to_string(conn_id)
                                           << "' retry failed. error: " << status_to_string(conn->status) << " ("
                                           << conn->reply_code << ")" << dendl;
                            ...
                            ldout(cct, 10)
                                << "AMQP run: connection '" << to_string(conn_id) << "' retry successfull" << dendl;
                        }
                    }
                ...
                if (tag_it != conn->callbacks.end()) {
                    if (multiple) {
                        ...
                        ldout(cct, 20) << "AMQP run: multiple n/acks received with tag=" << tag
                                       << " and result=" << result << dendl;
                        ...
                        ldout(cct, 20) << "AMQP run: n/ack received, invoking callback with tag=" << tag
                                       << " and result=" << result << dendl;
                        tag_it->cb(result);
    ...
    static void delete_message(const message_wrapper_t* message) { delete message; }

public:
    Manager(size_t _max_connections, size_t _max_inflight, size_t _max_queue, long _usec_timeout,
            unsigned reconnect_time_ms, unsigned idle_time_ms, CephContext* _cct)
    ...
    bool connect(connection_id_t& id, const std::string& url, const std::string& exchange, bool mandatory_delivery,
                 bool verify_ssl, boost::optional<const std::string&> ca_location)
    {
        ...
        ++connection_count;
        auto conn = connections.emplace(tmp_id, std::make_unique<connection_t>(cct, info, verify_ssl, ca_location))
                        .first->second.get();
        ldout(cct, 10) << "AMQP connect: new connection is created. Total connections: " << connection_count << dendl;
        if (!new_state(conn, tmp_id)) {
            ldout(cct, 1) << "AMQP connect: new connection '" << to_string(tmp_id)
    ...
    size_t get_queued() const { return queued; }

    // running counter of the dequeued messages
    size_t get_dequeued() const { return dequeued; }
};
...
bool init(CephContext* cct)
{
    ...
    s_manager = new Manager(MAX_CONNECTIONS_DEFAULT,
