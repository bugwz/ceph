...
int DaosUser::list_buckets(const DoutPrefixProvider* dpp, const string& marker, const string& end_marker, uint64_t max,
                           bool need_stats, BucketList& buckets, optional_yield y)
{
    ldpp_dout(dpp, 20) << "DEBUG: list_user_buckets: marker=" << marker << " end_marker=" << end_marker
                       << " max=" << max << dendl;
    int ret = 0;
    ...
    ret = ds3_bucket_list(&bcount, bucket_infos.data(), daos_marker, &is_truncated, store->ds3, nullptr);
    ldpp_dout(dpp, 20) << "DEBUG: ds3_bucket_list: bcount=" << bcount << " ret=" << ret << dendl;
    if (ret != 0) {
...
int DaosUser::create_bucket(const DoutPrefixProvider* dpp, const rgw_bucket& b, const std::string& zonegroup_id,
                            ...
                            bool* existed, req_info& req_info, std::unique_ptr<Bucket>* bucket_out, optional_yield y)
{
    ...
    if (!*existed) {
        ...
        bufferlist bl;
        std::unique_ptr<struct ds3_bucket_info> bucket_info = daos_bucket->get_encoded_info(bl, ceph::real_time());
        ret = ds3_bucket_create(bucket->get_name().c_str(), bucket_info.get(), nullptr, store->ds3, nullptr);
        if (ret != 0) {
            ldpp_dout(dpp, 0) << "ERROR: ds3_bucket_create failed! ret=" << ret << dendl;
            return ret;
        }
    }
    else {
...
int DaosUser::read_stats(const DoutPrefixProvider* dpp, optional_yield y, RGWStorageStats* stats,
...
int DaosUser::merge_and_store_attrs(const DoutPrefixProvider* dpp, Attrs& new_attrs, optional_yield y)
{
...
int DaosUser::store_user(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive, RGWUserInfo* old_info)
{
    const string name = info.user_id.to_str();
    ...
    if (ret == 0 && obj_ver.ver) {
        ...
        old_user_info.reset(new ds3_user_info{.name = duinfo.info.user_id.to_str().c_str(),
    ...
    bufferlist bl;
    std::unique_ptr<struct ds3_user_info> user_info = get_encoded_info(bl, obj_ver);

    ret = ds3_user_set(name.c_str(), user_info.get(), old_user_info.get(), store->ds3, nullptr);

    if (ret != 0) {
        ldpp_dout(dpp, 0) << "Error: ds3_user_set failed, name=" << name << " ret=" << ret << dendl;
    }

    return ret;
}

int DaosUser::read_user(const DoutPrefixProvider* dpp, std::string name, DaosUserInfo* duinfo)
{
    // Initialize ds3_user_info
    bufferlist bl;
    uint64_t size = DS3_MAX_ENCODED_LEN;
    struct ds3_user_info user_info = {.encoded = bl.append_hole(size).c_str(), .encoded_length = size};

    int ret = ds3_user_get(name.c_str(), &user_info, store->ds3, nullptr);

    if (ret != 0) {
        ldpp_dout(dpp, 0) << "Error: ds3_user_get failed, name=" << name << " ret=" << ret << dendl;
        return ret;
    }
...
std::unique_ptr<struct ds3_user_info> DaosUser::get_encoded_info(bufferlist& bl, obj_version& obj_ver)
{
    // Encode user data
    struct DaosUserInfo duinfo;
    duinfo.info = info;
    ...
    return std::unique_ptr<struct ds3_user_info>(new ds3_user_info{.name = info.user_id.to_str().c_str(),
...
int DaosBucket::open(const DoutPrefixProvider* dpp)
{
    ...
    ldpp_dout(dpp, 20) << "DEBUG: ds3_bucket_open, name=" << get_name() << ", ret=" << ret << dendl;
...
std::unique_ptr<struct ds3_bucket_info> DaosBucket::get_encoded_info(bufferlist& bl, ceph::real_time _mtime)
{
    DaosBucketInfo dbinfo;
    dbinfo.info = info;
...
int DaosBucket::remove_bucket(const DoutPrefixProvider* dpp, bool delete_children, bool forward_to_master,
...
int DaosBucket::remove_bucket_bypass_gc(int concurrent_max, bool keep_index_consistent, optional_yield y,
...
{
    ldpp_dout(dpp, 20) << "DEBUG: put_info(): bucket name=" << get_name() << dendl;
    ...
    bufferlist bl;
    std::unique_ptr<struct ds3_bucket_info> bucket_info = get_encoded_info(bl, ceph::real_time());
...
int DaosBucket::load_bucket(const DoutPrefixProvider* dpp, optional_yield y, bool get_stats)
{
    ldpp_dout(dpp, 20) << "DEBUG: load_bucket(): bucket name=" << get_name() << dendl;
    int ret = open(dpp);
    if (ret != 0) {
        return ret;
    }
    ...
    DaosBucketInfo dbinfo;
    uint64_t size = DS3_MAX_ENCODED_LEN;
    struct ds3_bucket_info bucket_info = {.encoded = bl.append_hole(size).c_str(), .encoded_length = size};
...
/* stats - Not for first pass */
int DaosBucket::read_stats(const DoutPrefixProvider* dpp, const bucket_index_layout_generation& idx_layout,
...
int DaosBucket::read_stats_async(const DoutPrefixProvider* dpp, const bucket_index_layout_generation& idx_layout,
...
int DaosBucket::chown(const DoutPrefixProvider* dpp, User& new_user, optional_yield y)
{
    return DAOS_NOT_IMPLEMENTED_LOG(dpp);
}
...
int DaosBucket::check_quota(const DoutPrefixProvider* dpp, RGWQuota& quota, uint64_t obj_size, optional_yield y,
...
int DaosBucket::check_index(const DoutPrefixProvider* dpp, std::map<RGWObjCategory, RGWStorageStats>& existing_stats,
...
bool compare_rgw_bucket_dir_entry(rgw_bucket_dir_entry& entry1, rgw_bucket_dir_entry& entry2)
{
    return (entry1.key < entry2.key);
}

bool compare_multipart_upload(std::unique_ptr<MultipartUpload>& upload1, std::unique_ptr<MultipartUpload>& upload2)
{
...
{
    ldpp_dout(dpp, 20) << "DEBUG: list bucket=" << get_name() << " max=" << max << " params=" << params << dendl;
    // End
    if (max == 0) {
        return 0;
    }
    ...
    if (ret != 0) {
        ldpp_dout(dpp, 0) << "ERROR: ds3_bucket_list_obj failed, name=" << get_name() << ", ret=" << ret << dendl;
        return ret;
    }
...
int DaosBucket::list_multiparts(const DoutPrefixProvider* dpp, const string& prefix, string& marker,
                                ...
                                bool* is_truncated)
{
    ...
    int ret = ds3_bucket_list_multipart(get_name().c_str(),
                                        &nmp,
                                        multipart_upload_infos.data(),
                                        &ncp,
                                        cps.data(),
                                        prefix.c_str(),
                                        delim.c_str(),
    ...
    for (auto const& mp : multipart_upload_infos) {
        ...
        uploads.push_back(this->get_multipart_upload(name, mp.upload_id, std::move(owner), ent.meta.mtime));
    }
...
void DaosZoneGroup::get_placement_target_names(std::set<std::string>& names) const
{
...
int DaosZoneGroup::get_placement_tier(const rgw_placement_rule& rule, std::unique_ptr<PlacementTier>* tier)
{
...
int DaosZone::get_zonegroup(const std::string& id, std::unique_ptr<ZoneGroup>* group)
{
...
int DaosObject::get_obj_state(const DoutPrefixProvider* dpp, RGWObjState** _state, optional_yield y, bool follow_olh)
{
...
int DaosObject::set_obj_attrs(const DoutPrefixProvider* dpp, Attrs* setattrs, Attrs* delattrs, optional_yield y)
{
...
int DaosObject::get_obj_attrs(optional_yield y, const DoutPrefixProvider* dpp, rgw_obj* target_obj)
{
...
int DaosObject::modify_obj_attrs(const char* attr_name, bufferlist& attr_val, optional_yield y,
                                 const DoutPrefixProvider* dpp)
{
    // Get object's metadata (those stored in rgw_bucket_dir_entry)
...
int DaosObject::delete_obj_attrs(const DoutPrefixProvider* dpp, const char* attr_name, optional_yield y)
{
...
bool DaosObject::is_expired()
{
    ...
    if (iter != attrs.end()) {
        ...
        try {
            ...
            ldout(store->ctx(), 0) << "ERROR: " << __func__ << ": failed to decode " RGW_ATTR_DELETE_AT " attr"
                                   << dendl;
            return false;
        }
...
int DaosObject::omap_get_all(const DoutPrefixProvider* dpp, std::map<std::string, bufferlist>* m, optional_yield y)
{
    return DAOS_NOT_IMPLEMENTED_LOG(dpp);
}
...
std::unique_ptr<MPSerializer> DaosObject::get_serializer(const DoutPrefixProvider* dpp, const std::string& lock_name)
{
    return std::make_unique<MPDaosSerializer>(dpp, store, this, lock_name);
}

int DaosObject::transition(Bucket* bucket, const rgw_placement_rule& placement_rule, const real_time& mtime,
                           uint64_t olh_epoch, const DoutPrefixProvider* dpp, optional_yield y)
{
    return DAOS_NOT_IMPLEMENTED_LOG(dpp);
}

int DaosObject::transition_to_cloud(Bucket* bucket, rgw::sal::PlacementTier* tier, rgw_bucket_dir_entry& o,
...
int DaosObject::dump_obj_layout(const DoutPrefixProvider* dpp, optional_yield y, Formatter* f)
{
    return DAOS_NOT_IMPLEMENTED_LOG(dpp);
}
...
DaosObject::DaosReadOp::DaosReadOp(DaosObject* _source)
    : source(_source)
{}
...
int DaosObject::DaosReadOp::prepare(optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    ldpp_dout(dpp, 20) << __func__ << ": object's size: " << ent.meta.size << dendl;

    return ret;
}

int DaosObject::DaosReadOp::read(int64_t off, int64_t end, bufferlist& bl, optional_yield y,
                                 const DoutPrefixProvider* dpp)
{
...
int DaosObject::DaosReadOp::iterate(const DoutPrefixProvider* dpp, int64_t off, int64_t end, RGWGetDataCB* cb,
                                    optional_yield y)
{
    ldpp_dout(dpp, 20) << __func__ << ": off=" << off << " end=" << end << dendl;
    int ret = source->lookup(dpp);
    ...
    ldpp_dout(dpp, 20) << __func__ << ": call cb to process data, actual=" << size << dendl;
    cb->handle_data(bl, off, size);
    return ret;
}

int DaosObject::DaosReadOp::get_attr(const DoutPrefixProvider* dpp, const char* name, bufferlist& dest,
                                     optional_yield y)
{
    Attrs attrs;
...
DaosObject::DaosDeleteOp::DaosDeleteOp(DaosObject* _source)
    : source(_source)
{}
...
{
    ldpp_dout(dpp, 20) << "DaosDeleteOp::delete_obj " << source->get_key().get_oid() << " from "
    ...
    ldpp_dout(dpp, 20) << "DEBUG: ds3_obj_destroy key=" << key << " ret=" << ret << dendl;
...
int DaosObject::delete_object(const DoutPrefixProvider* dpp, optional_yield y, bool prevent_versioning)
{
...
int DaosObject::delete_obj_aio(const DoutPrefixProvider* dpp, RGWObjState* astate, Completions* aio,
...
int DaosObject::copy_object(User* user, req_info* info, const rgw_zone_id& source_zone, rgw::sal::Object* dest_object,
...
int DaosObject::swift_versioning_restore(bool& restored, const DoutPrefixProvider* dpp)
{
    return DAOS_NOT_IMPLEMENTED_LOG(dpp);
}

int DaosObject::swift_versioning_copy(const DoutPrefixProvider* dpp, optional_yield y)
{
    return DAOS_NOT_IMPLEMENTED_LOG(dpp);
}
...
int DaosObject::lookup(const DoutPrefixProvider* dpp)
{
    ...
    if (ret == -ENOENT) {
        ldpp_dout(dpp, 20) << "DEBUG: daos object (" << get_bucket()->get_name() << ", " << get_key().get_oid()
                           << ") does not exist: ret=" << ret << dendl;
    }
    else if (ret != 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to open daos object (" << get_bucket()->get_name() << ", "
                          << get_key().get_oid() << "): ret=" << ret << dendl;
    }
    return ret;
}
...
int DaosObject::create(const DoutPrefixProvider* dpp)
{
    ...
    if (ret != 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to create daos object (" << get_bucket()->get_name() << ", "
                          << get_key().get_oid() << "): ret=" << ret << dendl;
    }
    return ret;
}
...
int DaosObject::write(const DoutPrefixProvider* dpp, bufferlist&& data, uint64_t offset)
{
    ldpp_dout(dpp, 20) << "DEBUG: write" << dendl;
    uint64_t size = data.length();
    int ret = ds3_obj_write(data.c_str(), offset, &size, get_daos_bucket()->ds3b, ds3o, nullptr);
    if (ret != 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to write into daos object (" << get_bucket()->get_name() << ", "
                          << get_key().get_oid() << "): ret=" << ret << dendl;
    }
    return ret;
}

int DaosObject::read(const DoutPrefixProvider* dpp, bufferlist& data, uint64_t offset, uint64_t& size)
{
    ldpp_dout(dpp, 20) << "DEBUG: read" << dendl;
    int ret = ds3_obj_read(data.append_hole(size).c_str(), offset, &size, get_daos_bucket()->ds3b, ds3o, nullptr);
    if (ret != 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to read from daos object (" << get_bucket()->get_name() << ", "
                          << get_key().get_oid() << "): ret=" << ret << dendl;
    }
    return ret;
}
...
int DaosObject::get_dir_entry_attrs(const DoutPrefixProvider* dpp, rgw_bucket_dir_entry* ent, Attrs* getattrs)
{
    ...
    if (get_key().ns == RGW_OBJ_NS_MULTIPART) {
        struct ds3_multipart_upload_info ui = {.encoded = value.data(), .encoded_length = size};
        ret = ds3_upload_get_info(&ui, bucket->get_name().c_str(), get_key().get_oid().c_str(), store->ds3);
    }
    else {
        ret = lookup(dpp);
        if (ret != 0) {
    ...
    if (ret != 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to get info of daos object (" << get_bucket()->get_name() << ", "
                          << get_key().get_oid() << "): ret=" << ret << dendl;
        return ret;
    }
...
int DaosObject::set_dir_entry_attrs(const DoutPrefixProvider* dpp, rgw_bucket_dir_entry* ent, Attrs* setattrs)
{
    ...
    if (ret != 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to set info of daos object (" << get_bucket()->get_name() << ", "
                          << get_key().get_oid() << "): ret=" << ret << dendl;
    }
    return ret;
}

int DaosObject::mark_as_latest(const DoutPrefixProvider* dpp, ceph::real_time set_mtime)
{
    ...
    std::unique_ptr<DaosObject> latest_object =
        std::make_unique<DaosObject>(store, rgw_obj_key(get_name(), DS3_LATEST_INSTANCE), get_bucket());

    ldpp_dout(dpp, 20) << __func__ << ": key=" << get_key().get_oid()
                       << " latest_object_key= " << latest_object->get_key().get_oid() << dendl;
    ...
    // version.
    ret = ds3_obj_mark_latest(get_key().get_oid().c_str(), get_daos_bucket()->ds3b);
    ldpp_dout(dpp, 20) << "DEBUG: ds3_obj_mark_latest ret=" << ret << dendl;
    return ret;
}

DaosAtomicWriter::DaosAtomicWriter(const DoutPrefixProvider* dpp, optional_yield y, rgw::sal::Object* obj,
...
int DaosAtomicWriter::complete(size_t accounted_size, const std::string& etag, ceph::real_time* mtime,
                               ...
                               optional_yield y)
{
    ...
    ldpp_dout(dpp, 20) << __func__ << ": key=" << obj.get_key().get_oid() << " etag: " << etag << dendl;
...
int DaosMultipartUpload::abort(const DoutPrefixProvider* dpp, CephContext* cct)
{
    ...
    return ds3_upload_remove(bucket->get_name().c_str(), get_upload_id().c_str(), store->ds3);
}

std::unique_ptr<rgw::sal::Object> DaosMultipartUpload::get_meta_obj()
{
    return bucket->get_object(rgw_obj_key(get_upload_id(), string(), RGW_OBJ_NS_MULTIPART));
}

int DaosMultipartUpload::init(const DoutPrefixProvider* dpp, optional_yield y, ACLOwner& _owner,
                              rgw_placement_rule& dest_placement, rgw::sal::Attrs& attrs)
{
    ...
    do {
        gen_rand_alphanumeric(store->ctx(), ui.upload_id + prefix_length, sizeof(ui.upload_id) - 1 - prefix_length);
        mp_obj.init(oid, ui.upload_id);
        ret = ds3_upload_init(&ui, bucket->get_name().c_str(), store->ds3);
    } while (ret == -EEXIST);

    if (ret != 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to create multipart upload dir (" << bucket->get_name() << "/"
                          << get_upload_id() << "): ret=" << ret << dendl;
    }
    return ret;
}

int DaosMultipartUpload::list_parts(const DoutPrefixProvider* dpp, CephContext* cct, int num_parts, int marker,
                                    int* next_marker, bool* truncated, bool assume_unsorted)
{
    ...
    int ret = ds3_upload_list_parts(bucket->get_name().c_str(),
                                    get_upload_id().c_str(),
                                    &npart,
                                    multipart_part_infos.data(),
                                    &daos_marker,
                                    truncated,
                                    store->ds3);
    ...
    for (auto const& pi : multipart_part_infos) {
        ...
        std::unique_ptr<DaosMultipartPart> part = std::make_unique<DaosMultipartPart>();
        auto iter = bl.cbegin();
...
int DaosMultipartUpload::complete(const DoutPrefixProvider* dpp, optional_yield y, CephContext* cct,
                                  ...
                                  std::string& tag, ACLOwner& owner, uint64_t olh_epoch, rgw::sal::Object* target_obj)
{
    ...
    do {
        ...
        for (auto obj_iter = parts.begin(); etags_iter != part_etags.end() && obj_iter != parts.end();
            ...
            DaosMultipartPart* part = dynamic_cast<rgw::sal::DaosMultipartPart*>(obj_iter->second.get());
    ...
    bufferlist bl;
    uint64_t size = DS3_MAX_ENCODED_LEN;
    struct ds3_multipart_upload_info ui = {.encoded = bl.append_hole(size).c_str(), .encoded_length = size};
    ret = ds3_upload_get_info(&ui, bucket->get_name().c_str(), get_upload_id().c_str(), store->ds3);
    ldpp_dout(dpp, 20) << "DEBUG: ds3_upload_get_info entry=" << bucket->get_name() << "/" << get_upload_id() << dendl;
    if (ret != 0) {
        if (ret == -ENOENT) {
            ret = -ERR_NO_SUCH_UPLOAD;
        }
    ...
    for (auto const& [part_num, part] : get_parts()) {
        ds3_part_t* ds3p;
        ret = ds3_part_open(get_bucket_name().c_str(), get_upload_id().c_str(), part_num, false, &ds3p, store->ds3);
        if (ret != 0) {
            return ret;
        }
        ...
        bufferlist bl;
        ret = ds3_part_read(bl.append_hole(size).c_str(), 0, &size, ds3p, store->ds3, nullptr);
        if (ret != 0) {
            ds3_part_close(ds3p);
            return ret;
        }

        ldpp_dout(dpp, 20) << "DaosMultipartUpload::complete(): part " << part_num << " size is " << size << dendl;
...
int DaosMultipartUpload::get_info(const DoutPrefixProvider* dpp, optional_yield y, rgw_placement_rule** rule,
                                  rgw::sal::Attrs* attrs)
{
...
std::unique_ptr<Writer> DaosMultipartUpload::get_writer(const DoutPrefixProvider* dpp, optional_yield y,
                                                        rgw::sal::Object* obj, const rgw_user& owner,
                                                        const rgw_placement_rule* ptail_placement_rule,
                                                        uint64_t part_num, const std::string& part_num_str)
{
    ldpp_dout(dpp, 20) << "DaosMultipartUpload::get_writer(): enter part=" << part_num << " head_obj=" << _head_obj
                       << dendl;
...
{
    ldpp_dout(dpp, 20) << "DaosMultipartWriter::prepare(): enter part=" << part_num_str << dendl;
    int ret = ds3_part_open(get_bucket_name().c_str(), upload_id.c_str(), part_num, true, &ds3p, store->ds3);
    if (ret == -ENOENT) {
        ret = -ERR_NO_SUCH_UPLOAD;
    }
    return ret;
...
{
    ldpp_dout(dpp, 20) << "DaosMultipartWriter::process(): enter part=" << part_num_str << " offset=" << offset
                       << dendl;
    if (data.length() == 0) {
        return 0;
    }
    ...
    uint64_t size = data.length();
    int ret = ds3_part_write(data.c_str(), offset, &size, ds3p, store->ds3, nullptr);
    if (ret == 0) {
        // XXX: Combine multiple streams into one as motr does
        actual_part_size += size;
    }
    else {
        ldpp_dout(dpp, 0) << "ERROR: failed to write into part (" << get_bucket_name() << ", " << upload_id << ", "
                          << part_num << "): ret=" << ret << dendl;
    }
    return ret;
}

int DaosMultipartWriter::complete(size_t accounted_size, const std::string& etag, ceph::real_time* mtime,
                                  ceph::real_time set_mtime, std::map<std::string, bufferlist>& attrs,
                                  ceph::real_time delete_at, const char* if_match, const char* if_nomatch,
                                  const std::string* user_data, rgw_zone_set* zones_trace, bool* canceled,
                                  optional_yield y)
{
    ldpp_dout(dpp, 20) << "DaosMultipartWriter::complete(): enter part=" << part_num_str << dendl;
    ...
    ldpp_dout(dpp, 20) << "DaosMultipartWriter::complete(): compression ret=" << ret << dendl;
    if (ret != 0) {
    ...
    encode(info, bl);
    encode(attrs, bl);
    ldpp_dout(dpp, 20) << "DaosMultipartWriter::complete(): entry size" << bl.length() << dendl;

    struct ds3_multipart_part_info part_info = {
        .part_num = part_num, .encoded = bl.c_str(), .encoded_length = bl.length()};

    ret = ds3_part_set_info(&part_info, ds3p, store->ds3, nullptr);

    if (ret != 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed to set part info (" << get_bucket_name() << ", " << upload_id << ", "
                          << part_num << "): ret=" << ret << dendl;
        if (ret == ENOENT) {
            ret = -ERR_NO_SUCH_UPLOAD;
        }
...
std::unique_ptr<RGWRole> DaosStore::get_role(std::string name, std::string tenant, std::string path,
...
int DaosStore::get_roles(const DoutPrefixProvider* dpp, optional_yield y, const std::string& path_prefix,
...
int DaosStore::get_oidc_providers(const DoutPrefixProvider* dpp, const std::string& tenant,
...
std::unique_ptr<Writer> DaosStore::get_append_writer(const DoutPrefixProvider* dpp, optional_yield y,
                                                     rgw::sal::Object* obj, const rgw_user& owner,
                                                     const rgw_placement_rule* ptail_placement_rule,
...
std::unique_ptr<Writer> DaosStore::get_atomic_writer(const DoutPrefixProvider* dpp, optional_yield y,
                                                     rgw::sal::Object* obj, const rgw_user& owner,
                                                     const rgw_placement_rule* ptail_placement_rule, uint64_t olh_epoch,
                                                     const std::string& unique_tag)
{
    ldpp_dout(dpp, 20) << "get_atomic_writer" << dendl;
    return std::make_unique<DaosAtomicWriter>(dpp, y, obj, this, owner, ptail_placement_rule, olh_epoch, unique_tag);
}

const std::string& DaosStore::get_compression_type(const rgw_placement_rule& rule)
{
...
int DaosStore::get_user_by_access_key(const DoutPrefixProvider* dpp, const std::string& key, optional_yield y,
                                      std::unique_ptr<User>* user)
{
    ...
    if (ret != 0) {
        ldpp_dout(dpp, 0) << "Error: ds3_user_get_by_key failed, key=" << key << " ret=" << ret << dendl;
        return ret;
    }
...
int DaosStore::get_user_by_email(const DoutPrefixProvider* dpp, const std::string& email, optional_yield y,
                                 std::unique_ptr<User>* user)
{
    // Initialize ds3_user_info
    bufferlist bl;
    ...
    if (ret != 0) {
        ldpp_dout(dpp, 0) << "Error: ds3_user_get_by_email failed, email=" << email << " ret=" << ret << dendl;
        return ret;
    }
...
int DaosStore::get_user_by_swift(const DoutPrefixProvider* dpp, const std::string& user_str, optional_yield y,
                                 std::unique_ptr<User>* user)
{
...
int DaosStore::get_bucket(const DoutPrefixProvider* dpp, User* u, const rgw_bucket& b, std::unique_ptr<Bucket>* bucket,
                          optional_yield y)
{
...
int DaosStore::get_bucket(const DoutPrefixProvider* dpp, User* u, const std::string& tenant, const std::string& name,
...
int DaosStore::forward_iam_request_to_master(const DoutPrefixProvider* dpp, const RGWAccessKey& key, obj_version* objv,
...
std::unique_ptr<Notification> DaosStore::get_notification(rgw::sal::Object* obj, rgw::sal::Object* src_obj,
                                                          struct req_state* s, rgw::notify::EventType event_type,
                                                          const std::string* object_name)
{
    return std::make_unique<DaosNotification>(obj, src_obj, event_type);
}

std::unique_ptr<Notification> DaosStore::get_notification(const DoutPrefixProvider* dpp, Object* obj, Object* src_obj,
...
void DaosStore::get_ratelimit(RGWRateLimitInfo& bucket_ratelimit, RGWRateLimitInfo& user_ratelimit,
                              RGWRateLimitInfo& anon_ratelimit)
{
    return;
}

int DaosStore::set_buckets_enabled(const DoutPrefixProvider* dpp, std::vector<rgw_bucket>& buckets, bool enabled)
{
    return DAOS_NOT_IMPLEMENTED_LOG(dpp);
}

int DaosStore::get_sync_policy_handler(const DoutPrefixProvider* dpp, std::optional<rgw_zone_id> zone,
...
int DaosStore::meta_list_keys_init(const DoutPrefixProvider* dpp, const string& section, const string& marker,
                                   void** phandle)
{
    return DAOS_NOT_IMPLEMENTED_LOG(dpp);
}

int DaosStore::meta_list_keys_next(const DoutPrefixProvider* dpp, void* handle, int max, list<string>& keys,
                                   bool* truncated)
{
    return DAOS_NOT_IMPLEMENTED_LOG(dpp);
}
