...
{
protected:
    int init_permissions(RGWOp* op, optional_yield) override { return 0; }
...
public:
    explicit RGWHandler_REST_PSTopic(const rgw::auth::StrategyRegistry& auth_registry)
...
public:
    int get_params() override
    {
        ...
        const int ret = s->info.args.get_int("max-entries", &max_entries, RGWPubSub::Sub::DEFAULT_MAX_EVENTS);
        if (ret < 0) {
...
public:
    explicit RGWHandler_REST_PSSub(const rgw::auth::StrategyRegistry& auth_registry)
        : RGWHandler_REST_S3(auth_registry)
    {}
...
private:
    ...
    int get_params() override { return notif_bucket_path(s->object->get_name(), bucket_name); }
...
public:
    explicit RGWHandler_REST_PSNotifs(const rgw::auth::StrategyRegistry& auth_registry)
        : RGWHandler_REST_S3(auth_registry)
    {}
...
RGWHandler_REST* RGWRESTMgr_PubSub::get_handler(rgw::sal::RGWRadosStore* store, struct req_state* const s,
