...
public:
    explicit ESQueryStack(std::list<std::string>& src) { assign(src); }
...
public:
    explicit ESInfixQueryParser(const std::string& _query)
        : query(_query)
        , size(query.size())
        , str(query.c_str())
    {}
...
struct ESEntityTypeMap
{
    ...
    explicit ESEntityTypeMap(std::map<std::string, EntityType>& _m)
        : m(_m)
    {}
...
public:
    ESQueryCompiler(const std::string& query, std::list<std::pair<std::string, std::string> >* prepend_eq_conds,
    ...
    void set_generic_type_map(ESEntityTypeMap* entity_map) { generic_type_map = entity_map; }

    ESEntityTypeMap* get_generic_type_map() { return generic_type_map; }
    const std::string& get_custom_prefix() { return custom_prefix; }

    void set_custom_type_map(ESEntityTypeMap* entity_map) { custom_type_map = entity_map; }

    ESEntityTypeMap* get_custom_type_map() { return custom_type_map; }

    void set_field_aliases(std::map<std::string, std::string, ltstr_nocase>* fa) { field_aliases = fa; }
    ...
    void set_restricted_fields(std::set<std::string>* rf) { restricted_fields = rf; }
