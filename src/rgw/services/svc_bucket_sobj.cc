...
public:
    RGWSI_Bucket_SObj_Module(RGWSI_Bucket_SObj::Svc& _svc)
        : RGWSI_MBSObj_Handler_Module("bucket")
        , svc(_svc)
    {}
    ...
    bool is_valid_oid(const string& oid) override { return (!oid.empty() && oid[0] != '.'); }
...
public:
    RGWSI_BucketInstance_SObj_Module(RGWSI_Bucket_SObj::Svc& _svc)
        : RGWSI_MBSObj_Handler_Module("bucket.instance")
        , svc(_svc)
        , prefix(RGW_BUCKET_INSTANCE_MD_PREFIX)
    {}
...
void RGWSI_Bucket_SObj::init(RGWSI_Zone* _zone_svc, RGWSI_SysObj* _sysobj_svc, RGWSI_SysObj_Cache* _cache_svc,
                             RGWSI_BucketIndex* _bi, RGWSI_Meta* _meta_svc, RGWSI_MetaBackend* _meta_be_svc,
                             RGWSI_SyncModules* _sync_modules_svc, RGWSI_Bucket_Sync* _bucket_sync_svc)
{
    svc.bucket = this;
    svc.zone = _zone_svc;
...
int RGWSI_Bucket_SObj::read_bucket_entrypoint_info(RGWSI_Bucket_EP_Ctx& ctx, const string& key,
                                                   ...
                                                   const DoutPrefixProvider* dpp, rgw_cache_entry_info* cache_info,
                                                   boost::optional<obj_version> refresh_version)
{
    bufferlist bl;

    auto params =
        RGWSI_MBSObj_GetParams(&bl, pattrs, pmtime).set_cache_info(cache_info).set_refresh_version(refresh_version);
...
int RGWSI_Bucket_SObj::store_bucket_entrypoint_info(RGWSI_Bucket_EP_Ctx& ctx, const string& key,
                                                    RGWBucketEntryPoint& info, bool exclusive, real_time mtime,
...
int RGWSI_Bucket_SObj::remove_bucket_entrypoint_info(RGWSI_Bucket_EP_Ctx& ctx, const string& key,
...
int RGWSI_Bucket_SObj::read_bucket_instance_info(RGWSI_Bucket_BI_Ctx& ctx, const string& key, RGWBucketInfo* info,
                                                 real_time* pmtime, map<string, bufferlist>* pattrs, optional_yield y,
                                                 const DoutPrefixProvider* dpp, rgw_cache_entry_info* cache_info,
                                                 boost::optional<obj_version> refresh_version)
{
    ...
    if (auto e = binfo_cache->find(cache_key)) {
        if (refresh_version && e->info.objv_tracker.read_version.compare(&(*refresh_version))) {
    ...
    int ret = do_read_bucket_instance_info(ctx, key, &e.info, &e.mtime, &e.attrs, &ci, refresh_version, y, dpp);
    *info = e.info;
    ...
    if (ret < 0) {
        if (ret != -ENOENT) {
            ...
            ldpp_dout(dpp, 20) << "do_read_bucket_instance_info, bucket instance not found (key=" << key << ")"
                               << dendl;
        }
        return ret;
    }
    ...
    if (refresh_version && refresh_version->compare(&info->objv_tracker.read_version)) {
...
int RGWSI_Bucket_SObj::do_read_bucket_instance_info(RGWSI_Bucket_BI_Ctx& ctx, const string& key, RGWBucketInfo* info,
...
int RGWSI_Bucket_SObj::read_bucket_info(RGWSI_Bucket_X_Ctx& ctx, const rgw_bucket& bucket, RGWBucketInfo* info,
                                        real_time* pmtime, map<string, bufferlist>* pattrs,
                                        boost::optional<obj_version> refresh_version, optional_yield y,
                                        const DoutPrefixProvider* dpp)
{
    ...
    if (!bucket.bucket_id.empty()) {
        return read_bucket_instance_info(
            ctx.bi, get_bi_meta_key(bucket), info, pmtime, pattrs, y, dpp, &cache_info, refresh_version);
    }
    ...
    if (auto e = binfo_cache->find(cache_key)) {
        bool found_version = (bucket.bucket_id.empty() || bucket.bucket_id == e->info.bucket.bucket_id);

        if (!found_version || (refresh_version && e->info.objv_tracker.read_version.compare(&(*refresh_version)))) {
            ldpp_dout(dpp, -1) << "WARNING: The bucket info cache is inconsistent. This is "
                               << "a failure that should be debugged. I am a nice machine, "
                               << "so I will try to recover." << dendl;
    ...
    int ret = read_bucket_entrypoint_info(
        ctx.ep, bucket_entry, &entry_point, &ot, &ep_mtime, pattrs, y, dpp, &entry_cache_info, refresh_version);
    if (ret < 0) {
    ...
    if (entry_point.has_bucket_info) {
        *info = entry_point.old_bucket_info;
        ...
        ldpp_dout(dpp, 20) << "rgw_get_bucket_info: old bucket info, bucket=" << info->bucket << " owner "
                           << info->owner << dendl;
        return 0;
    }
    ...
    ret = read_bucket_instance_info(
        ctx.bi, get_bi_meta_key(entry_point.bucket), &e.info, &e.mtime, &e.attrs, y, dpp, &cache_info, refresh_version);
    *info = e.info;
    if (ret < 0) {
...
int RGWSI_Bucket_SObj::store_bucket_instance_info(RGWSI_Bucket_BI_Ctx& ctx, const string& key, RGWBucketInfo& info,
                                                  std::optional<RGWBucketInfo*> orig_info, bool exclusive,
                                                  real_time mtime, map<string, bufferlist>* pattrs, optional_yield y,
                                                  const DoutPrefixProvider* dpp)
{
    bufferlist bl;
    encode(info, bl);
    ...
    if (!orig_info && !exclusive) { /* if exclusive, we're going to fail when try
         ...
         */
        int r =
            read_bucket_instance_info(ctx, key, &shared_bucket_info, nullptr, nullptr, y, dpp, nullptr, boost::none);
        if (r < 0) {
            if (r != -ENOENT) {
                ldpp_dout(dpp, 0) << "ERROR: " << __func__ << "(): read_bucket_instance_info() of key=" << key
                                  << " returned r=" << r << dendl;
                return r;
            }
        }
    ...
    if (orig_info && *orig_info && !exclusive) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: " << __func__ << "(): svc.bi->handle_overwrite() of key=" << key
                              << " returned r=" << r << dendl;
            return r;
        }
    }
...
int RGWSI_Bucket_SObj::remove_bucket_instance_info(RGWSI_Bucket_BI_Ctx& ctx, const string& key,
                                                   const RGWBucketInfo& info, RGWObjVersionTracker* objv_tracker,
                                                   optional_yield y, const DoutPrefixProvider* dpp)
{
    RGWSI_MBSObj_RemoveParams params;
...
int RGWSI_Bucket_SObj::read_bucket_stats(const RGWBucketInfo& bucket_info, RGWBucketEnt* ent, optional_yield y,
                                         const DoutPrefixProvider* dpp)
{
    ent->count = 0;
    ent->size = 0;
...
int RGWSI_Bucket_SObj::read_bucket_stats(RGWSI_Bucket_X_Ctx& ctx, const rgw_bucket& bucket, RGWBucketEnt* ent,
                                         optional_yield y, const DoutPrefixProvider* dpp)
{
...
int RGWSI_Bucket_SObj::read_buckets_stats(RGWSI_Bucket_X_Ctx& ctx, map<string, RGWBucketEnt>& m, optional_yield y,
                                          const DoutPrefixProvider* dpp)
{
