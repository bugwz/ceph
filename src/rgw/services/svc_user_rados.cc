...
int RGWSI_User_RADOS::remove_key_index(const DoutPrefixProvider* dpp, const RGWAccessKey& access_key, optional_yield y)
{
...
int RGWSI_User_RADOS::remove_email_index(const DoutPrefixProvider* dpp, const string& email, optional_yield y)
{
    if (email.empty()) {
        return 0;
    }
...
int RGWSI_User_RADOS::remove_swift_name_index(const DoutPrefixProvider* dpp, const string& swift_name, optional_yield y)
{
 ...
 */
int RGWSI_User_RADOS::remove_user_info(RGWSI_MetaBackend::Context* ctx, const RGWUserInfo& info,
...
int RGWSI_User_RADOS::get_user_info_from_index(RGWSI_MetaBackend::Context* ctx, const string& key, const rgw_pool& pool,
                                               RGWUserInfo* info, RGWObjVersionTracker* objv_tracker, real_time* pmtime,
                                               optional_yield y, const DoutPrefixProvider* dpp)
{
...
int RGWSI_User_RADOS::add_bucket(const DoutPrefixProvider* dpp, const rgw_user& user, const rgw_bucket& bucket,
...
int RGWSI_User_RADOS::remove_bucket(const DoutPrefixProvider* dpp, const rgw_user& user, const rgw_bucket& _bucket,
                                    optional_yield y)
{
...
int RGWSI_User_RADOS::list_buckets(const DoutPrefixProvider* dpp, const rgw_user& user, const string& marker,
                                   const string& end_marker, uint64_t max, RGWUserBuckets* buckets, bool* is_truncated,
                                   optional_yield y)
{
    int ret;
...
int RGWSI_User_RADOS::flush_bucket_stats(const DoutPrefixProvider* dpp, const rgw_user& user, const RGWBucketEnt& ent,
                                         optional_yield y)
{
    rgw_raw_obj obj = get_buckets_obj(user);
...
int RGWSI_User_RADOS::reset_bucket_stats(const DoutPrefixProvider* dpp, const rgw_user& user, optional_yield y)
{
...
int RGWSI_User_RADOS::complete_flush_stats(const DoutPrefixProvider* dpp, const rgw_user& user, optional_yield y)
{
    rgw_raw_obj obj = get_buckets_obj(user);
    auto rados_obj = svc.rados->obj(obj);
    int r = rados_obj.open(dpp);
...
int RGWSI_User_RADOS::read_stats_async(const DoutPrefixProvider* dpp, const rgw_user& user, RGWGetUserStats_CB* _cb)
{
    string user_str = user.to_str();
