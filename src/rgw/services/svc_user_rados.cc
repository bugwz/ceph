...
public:
    RGWSI_User_Module(RGWSI_User_RADOS::Svc& _svc)
...
RGWSI_User_RADOS::RGWSI_User_RADOS(CephContext* cct)
    : RGWSI_User(cct)
{}

RGWSI_User_RADOS::~RGWSI_User_RADOS() {}

void RGWSI_User_RADOS::init(RGWSI_RADOS* _rados_svc, RGWSI_Zone* _zone_svc, RGWSI_SysObj* _sysobj_svc,
                            RGWSI_SysObj_Cache* _cache_svc, RGWSI_Meta* _meta_svc, RGWSI_MetaBackend* _meta_be_svc,
...
int RGWSI_User_RADOS::read_user_info(RGWSI_MetaBackend::Context* ctx, const rgw_user& user, RGWUserInfo* info,
                                     RGWObjVersionTracker* const objv_tracker, real_time* const pmtime,
                                     rgw_cache_entry_info* const cache_info, map<string, bufferlist>* const pattrs,
                                     optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    try {
        ...
        if (user_id.user_id != user) {
            ldpp_dout(dpp, -1) << "ERROR: rgw_get_user_info_by_uid(): user id mismatch: " << user_id.user_id
                               << " != " << user << dendl;
            return -EIO;
        }
        if (!iter.end()) {
...
public:
    PutOperation(RGWSI_User_RADOS::Svc& svc, RGWSI_MetaBackend::Context* _ctx, const RGWUserInfo& info,
                 RGWUserInfo* old_info, RGWObjVersionTracker* objv_tracker, const real_time& mtime, bool exclusive,
                 map<string, bufferlist>* pattrs, optional_yield y)
        : svc(svc)
        , info(info)
        , old_info(old_info)
    ...
    int prepare(const DoutPrefixProvider* dpp)
    {
        ...
        for (auto iter = info.swift_keys.begin(); iter != info.swift_keys.end(); ++iter) {
            if (old_info && old_info->swift_keys.count(iter->first) != 0) continue;
            auto& k = iter->second;
            /* check if swift mapping exists */
            RGWUserInfo inf;
            int r = svc.user->get_user_info_by_swift(ctx, k.id, &inf, nullptr, nullptr, y, dpp);
            if (r >= 0 && inf.user_id != info.user_id && (!old_info || inf.user_id != old_info->user_id)) {
        ...
        for (auto iter = info.access_keys.begin(); iter != info.access_keys.end(); ++iter) {
            if (old_info && old_info->access_keys.count(iter->first) != 0) continue;
            auto& k = iter->second;
            RGWUserInfo inf;
            int r = svc.user->get_user_info_by_access_key(ctx, k.id, &inf, nullptr, nullptr, y, dpp);
            if (r >= 0 && inf.user_id != info.user_id && (!old_info || inf.user_id != old_info->user_id)) {
                ldpp_dout(dpp, 0) << "WARNING: can't store user info, access key already mapped to another user"
                                  << dendl;
                return -EEXIST;
            }
        }
    ...
    int complete(const DoutPrefixProvider* dpp)
    {
        ...
        if (!info.user_email.empty()) {
            if (!old_info || old_info->user_email.compare(info.user_email) != 0) { /* only if new index changed */
                ret = rgw_put_system_obj(dpp,
                                         obj_ctx,
                                         svc.zone->get_zone_params().user_email_pool,
                                         info.user_email,
                                         link_bl,
                                         exclusive,
                                         NULL,
        ...
        for (auto iter = info.access_keys.begin(); iter != info.access_keys.end(); ++iter) {
            auto& k = iter->second;
            if (old_info && old_info->access_keys.count(iter->first) != 0 && !renamed) continue;
        ...
        for (auto siter = info.swift_keys.begin(); siter != info.swift_keys.end(); ++siter) {
            auto& k = siter->second;
            if (old_info && old_info->swift_keys.count(siter->first) != 0 && !renamed) continue;
                           ...
                           const DoutPrefixProvider* dpp)
    {
        ...
        if (!old_info.user_id.empty() && old_info.user_id != new_info.user_id) {
            if (old_info.user_id.tenant != new_info.user_id.tenant) {
                ldpp_dout(dpp, 0) << "ERROR: tenant mismatch: " << old_info.user_id.tenant
                                  << " != " << new_info.user_id.tenant << dendl;
                return -EINVAL;
            }
        ...
        if (!old_info.user_email.empty() && old_info.user_email != new_info.user_email) {
    ...
    const string& get_err_msg() { return err_msg; }
};

int RGWSI_User_RADOS::store_user_info(RGWSI_MetaBackend::Context* ctx, const RGWUserInfo& info, RGWUserInfo* old_info,
                                      RGWObjVersionTracker* objv_tracker, const real_time& mtime, bool exclusive,
                                      map<string, bufferlist>* attrs, optional_yield y, const DoutPrefixProvider* dpp)
{
    PutOperation op(svc, ctx, info, old_info, objv_tracker, mtime, exclusive, attrs, y);
...
int RGWSI_User_RADOS::remove_key_index(const DoutPrefixProvider* dpp, RGWSI_MetaBackend::Context* _ctx,
                                       const RGWAccessKey& access_key, optional_yield y)
{
...
int RGWSI_User_RADOS::remove_email_index(const DoutPrefixProvider* dpp, RGWSI_MetaBackend::Context* _ctx,
                                         const string& email, optional_yield y)
{
    if (email.empty()) {
        return 0;
    }
...
int RGWSI_User_RADOS::remove_swift_name_index(const DoutPrefixProvider* dpp, RGWSI_MetaBackend::Context* _ctx,
                                              const string& swift_name, optional_yield y)
{
    RGWSI_MetaBackend_SObj::Context_SObj* ctx = static_cast<RGWSI_MetaBackend_SObj::Context_SObj*>(_ctx);
 ...
 */
int RGWSI_User_RADOS::remove_user_info(RGWSI_MetaBackend::Context* _ctx, const RGWUserInfo& info,
                                       ...
                                       const DoutPrefixProvider* dpp)
{
    ...
    for (; kiter != info.access_keys.end(); ++kiter) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 0) << "ERROR: could not remove " << kiter->first
                              << " (access key object), should be fixed (err=" << ret << ")" << dendl;
            return ret;
        }
    }
    ...
    for (; siter != info.swift_keys.end(); ++siter) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 0) << "ERROR: could not remove " << k.id
                              << " (swift name object), should be fixed (err=" << ret << ")" << dendl;
            return ret;
        }
    }
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(dpp, 0) << "ERROR: could not remove email index object for " << info.user_email
                          << ", should be fixed (err=" << ret << ")" << dendl;
        return ret;
    }
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(dpp, 0) << "ERROR: could not remove " << info.user_id << ":" << uid_bucks
                          << ", should be fixed (err=" << ret << ")" << dendl;
        return ret;
    }
...
int RGWSI_User_RADOS::remove_uid_index(RGWSI_MetaBackend::Context* ctx, const RGWUserInfo& user_info,
                                       RGWObjVersionTracker* objv_tracker, optional_yield y,
                                       const DoutPrefixProvider* dpp)
{
    ...
    if (ret < 0 && ret != -ENOENT && ret != -ECANCELED) {
        ...
        ldpp_dout(dpp, 0) << "ERROR: could not remove " << user_info.user_id << ":" << uid_obj
                          << ", should be fixed (err=" << ret << ")" << dendl;
        return ret;
    }
...
int RGWSI_User_RADOS::get_user_info_from_index(RGWSI_MetaBackend::Context* _ctx, const string& key,
                                               const rgw_pool& pool, RGWUserInfo* info,
                                               RGWObjVersionTracker* const objv_tracker, real_time* const pmtime,
                                               optional_yield y, const DoutPrefixProvider* dpp)
{
    RGWSI_MetaBackend_SObj::Context_SObj* ctx = static_cast<RGWSI_MetaBackend_SObj::Context_SObj*>(_ctx);
    ...
    if (auto e = uinfo_cache->find(cache_key)) {
        *info = e->info;
        if (objv_tracker) *objv_tracker = e->objv_tracker;
        if (pmtime) *pmtime = e->mtime;
        return 0;
    }
    ...
    try {
        ...
        int ret = read_user_info(ctx, uid.user_id, &e.info, &e.objv_tracker, nullptr, &cache_info, nullptr, y, dpp);
        if (ret < 0) {
            return ret;
        }
    }
 ...
 */
int RGWSI_User_RADOS::get_user_info_by_email(RGWSI_MetaBackend::Context* ctx, const string& email, RGWUserInfo* info,
                                             RGWObjVersionTracker* objv_tracker, real_time* pmtime, optional_yield y,
                                             const DoutPrefixProvider* dpp)
{
    return get_user_info_from_index(
        ctx, email, svc.zone->get_zone_params().user_email_pool, info, objv_tracker, pmtime, y, dpp);
}
 ...
 * returns: 0 on success, -ERR# on failure (including nonexistence)
 */
int RGWSI_User_RADOS::get_user_info_by_swift(RGWSI_MetaBackend::Context* ctx, const string& swift_name,
                                             RGWUserInfo* info, /* out */
                                             RGWObjVersionTracker* const objv_tracker, real_time* const pmtime,
                                             optional_yield y, const DoutPrefixProvider* dpp)
{
    return get_user_info_from_index(
        ctx, swift_name, svc.zone->get_zone_params().user_swift_pool, info, objv_tracker, pmtime, y, dpp);
}
 ...
 * returns: 0 on success, -ERR# on failure (including nonexistence)
 */
int RGWSI_User_RADOS::get_user_info_by_access_key(RGWSI_MetaBackend::Context* ctx, const std::string& access_key,
                                                  RGWUserInfo* info, RGWObjVersionTracker* objv_tracker,
                                                  real_time* pmtime, optional_yield y, const DoutPrefixProvider* dpp)
{
    return get_user_info_from_index(
        ctx, access_key, svc.zone->get_zone_params().user_keys_pool, info, objv_tracker, pmtime, y, dpp);
}

int RGWSI_User_RADOS::cls_user_update_buckets(const DoutPrefixProvider* dpp, rgw_raw_obj& obj,
                                              list<cls_user_bucket_entry>& entries, bool add, optional_yield y)
{
...
int RGWSI_User_RADOS::cls_user_add_bucket(const DoutPrefixProvider* dpp, rgw_raw_obj& obj,
                                          const cls_user_bucket_entry& entry, optional_yield y)
{
...
int RGWSI_User_RADOS::cls_user_remove_bucket(const DoutPrefixProvider* dpp, rgw_raw_obj& obj,
                                             const cls_user_bucket& bucket, optional_yield y)
{
    auto rados_obj = svc.rados->obj(obj);
    int r = rados_obj.open(dpp);
    if (r < 0) {
...
int RGWSI_User_RADOS::add_bucket(const DoutPrefixProvider* dpp, RGWSI_MetaBackend::Context* ctx, const rgw_user& user,
                                 const rgw_bucket& bucket, ceph::real_time creation_time, optional_yield y)
{
    int ret;
...
int RGWSI_User_RADOS::remove_bucket(const DoutPrefixProvider* dpp, RGWSI_MetaBackend::Context* ctx,
                                    const rgw_user& user, const rgw_bucket& _bucket, optional_yield y)
{
...
int RGWSI_User_RADOS::cls_user_flush_bucket_stats(const DoutPrefixProvider* dpp, rgw_raw_obj& user_obj,
...
int RGWSI_User_RADOS::cls_user_list_buckets(const DoutPrefixProvider* dpp, rgw_raw_obj& obj, const string& in_marker,
                                            const string& end_marker, const int max_entries,
                                            list<cls_user_bucket_entry>& entries, string* const out_marker,
                                            bool* const truncated, optional_yield y)
{
    auto rados_obj = svc.rados->obj(obj);
    int r = rados_obj.open(dpp);
    if (r < 0) {
...
int RGWSI_User_RADOS::list_buckets(const DoutPrefixProvider* dpp, RGWSI_MetaBackend::Context* ctx, const rgw_user& user,
                                   const string& marker, const string& end_marker, uint64_t max,
                                   RGWUserBuckets* buckets, bool* is_truncated, optional_yield y)
{
    int ret;
...
int RGWSI_User_RADOS::flush_bucket_stats(const DoutPrefixProvider* dpp, RGWSI_MetaBackend::Context* ctx,
                                         const rgw_user& user, const RGWBucketEnt& ent, optional_yield y)
{
    rgw_raw_obj obj = get_buckets_obj(user);
...
int RGWSI_User_RADOS::reset_bucket_stats(const DoutPrefixProvider* dpp, RGWSI_MetaBackend::Context* ctx,
...
int RGWSI_User_RADOS::complete_flush_stats(const DoutPrefixProvider* dpp, RGWSI_MetaBackend::Context* ctx,
                                           const rgw_user& user, optional_yield y)
{
    rgw_raw_obj obj = get_buckets_obj(user);
    auto rados_obj = svc.rados->obj(obj);
...
int RGWSI_User_RADOS::cls_user_get_header(const DoutPrefixProvider* dpp, const rgw_user& user, cls_user_header* header,
                                          optional_yield y)
{
    rgw_raw_obj obj = get_buckets_obj(user);
    auto rados_obj = svc.rados->obj(obj);
...
int RGWSI_User_RADOS::cls_user_get_header_async(const DoutPrefixProvider* dpp, const string& user_str,
                                                RGWGetUserHeader_CB* cb)
{
...
int RGWSI_User_RADOS::read_stats(const DoutPrefixProvider* dpp, RGWSI_MetaBackend::Context* ctx, const rgw_user& user,
                                 RGWStorageStats* stats, ceph::real_time* last_stats_sync,
                                 ceph::real_time* last_stats_update, optional_yield y)
{
