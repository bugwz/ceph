...
void RGWSI_BucketIndex_RADOS::init(RGWSI_Zone* zone_svc, RGWSI_RADOS* rados_svc, RGWSI_BILog_RADOS* bilog_svc,
...
int RGWSI_BucketIndex_RADOS::open_pool(const DoutPrefixProvider* dpp, const rgw_pool& pool,
                                       RGWSI_RADOS::Pool* index_pool, bool mostly_omap)
{
    *index_pool = svc.rados->pool(pool);
    return index_pool->open(dpp, RGWSI_RADOS::OpenParams().set_mostly_omap(mostly_omap));
}

int RGWSI_BucketIndex_RADOS::open_bucket_index_pool(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
...
int RGWSI_BucketIndex_RADOS::open_bucket_index_base(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                                    RGWSI_RADOS::Pool* index_pool, string* bucket_oid_base)
{
...
int RGWSI_BucketIndex_RADOS::open_bucket_index(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                               RGWSI_RADOS::Pool* index_pool, string* bucket_oid)
{
    const rgw_bucket& bucket = bucket_info.bucket;
    int r = open_bucket_index_pool(dpp, bucket_info, index_pool);
    if (r < 0) {
        ldpp_dout(dpp, 20) << __func__ << ": open_bucket_index_pool() returned " << r << dendl;
        return r;
    }
...
static void get_bucket_index_objects(const string& bucket_oid_base, uint32_t num_shards, uint64_t gen_id,
                                     map<int, string>* _bucket_objects, int shard_id = -1)
{
...
static void get_bucket_instance_ids(const RGWBucketInfo& bucket_info, int num_shards, int shard_id,
...
int RGWSI_BucketIndex_RADOS::open_bucket_index(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                               ...
                                               RGWSI_RADOS::Pool* index_pool, map<int, string>* bucket_objs,
                                               map<int, string>* bucket_instance_ids)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 20) << __func__ << ": open_bucket_index_pool() returned " << ret << dendl;
        return ret;
    }

    get_bucket_index_objects(
        bucket_oid_base, idx_layout.layout.normal.num_shards, idx_layout.gen, bucket_objs, shard_id);
    if (bucket_instance_ids) {
        get_bucket_instance_ids(bucket_info, idx_layout.layout.normal.num_shards, shard_id, bucket_instance_ids);
    }
    return 0;
}

void RGWSI_BucketIndex_RADOS::get_bucket_index_object(const std::string& bucket_oid_base,
                                                      const rgw::bucket_index_normal_layout& normal, uint64_t gen_id,
                                                      int shard_id, std::string* bucket_obj)
{
...
int RGWSI_BucketIndex_RADOS::get_bucket_index_object(const std::string& bucket_oid_base,
                                                     const rgw::bucket_index_normal_layout& normal, uint64_t gen_id,
                                                     const std::string& obj_key, std::string* bucket_obj, int* shard_id)
{
    int r = 0;
...
int RGWSI_BucketIndex_RADOS::open_bucket_index_shard(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                                     const string& obj_key, RGWSI_RADOS::Obj* bucket_obj, int* shard_id)
{
    string bucket_oid_base;
    ...
    ret = get_bucket_index_object(
        bucket_oid_base, current_index.layout.normal, current_index.gen, obj_key, &oid, shard_id);
    if (ret < 0) {
                                                     ...
                                                     RGWSI_RADOS::Obj* bucket_obj)
{
    ...
    get_bucket_index_object(bucket_oid_base, index.layout.normal, index.gen, shard_id, &oid);
...
int RGWSI_BucketIndex_RADOS::cls_bucket_head(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                             const rgw::bucket_index_layout_generation& idx_layout, int shard_id,
                                             vector<rgw_bucket_dir_header>* headers,
                                             map<int, string>* bucket_instance_ids, optional_yield y)
{
    RGWSI_RADOS::Pool index_pool;
    map<int, string> oids;
                                        ...
                                        const rgw::bucket_index_layout_generation& idx_layout)
{
    ...
    return CLSRGWIssueBucketIndexInit(index_pool.ioctx(), bucket_objs, cct->_conf->rgw_bucket_index_max_aio)();
}
                                         ...
                                         const rgw::bucket_index_layout_generation& idx_layout)
{
    ...
    return CLSRGWIssueBucketIndexClean(index_pool.ioctx(), bucket_objs, cct->_conf->rgw_bucket_index_max_aio)();
}

int RGWSI_BucketIndex_RADOS::read_stats(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                        RGWBucketEnt* result, optional_yield y)
{
    vector<rgw_bucket_dir_header> headers;
...
int RGWSI_BucketIndex_RADOS::get_reshard_status(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                                list<cls_rgw_bucket_instance_entry>* status)
{
    ...
    for (auto i : bucket_objs) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, -1) << "ERROR: " << __func__ << ": cls_rgw_get_bucket_resharding() returned ret=" << ret
                               << dendl;
            return ret;
        }
...
int RGWSI_BucketIndex_RADOS::handle_overwrite(const DoutPrefixProvider* dpp, const RGWBucketInfo& info,
                                              const RGWBucketInfo& orig_info, optional_yield y)
{
    ...
    for (int i = 0; i < shards_num; ++i) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, -1) << "ERROR: failed writing data log (info.bucket=" << info.bucket << ", shard_id=" << i
                               << ")" << dendl;
