...
static void get_bucket_index_objects(const string& bucket_oid_base, uint32_t num_shards,
...
static void get_bucket_instance_ids(const RGWBucketInfo& bucket_info, int shard_id, map<int, string>* result)
{
    const rgw_bucket& bucket = bucket_info.bucket;
...
int RGWSI_BucketIndex_RADOS::open_bucket_index(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                               std::optional<int> _shard_id, RGWSI_RADOS::Pool* index_pool,
                                               map<int, string>* bucket_objs, map<int, string>* bucket_instance_ids)
{
    ...
    get_bucket_index_objects(
        bucket_oid_base, bucket_info.layout.current_index.layout.normal.num_shards, bucket_objs, shard_id);
...
void RGWSI_BucketIndex_RADOS::get_bucket_index_object(const string& bucket_oid_base, uint32_t num_shards, int shard_id,
                                                      uint64_t gen_id, string* bucket_obj)
{
    if (!num_shards) {
                                                     ...
                                                     const string& obj_key, RGWSI_RADOS::Obj* bucket_obj, int* shard_id)
{
    ...
    ret = get_bucket_index_object(bucket_oid_base,
                                  obj_key,
                                                     ...
                                                     RGWSI_RADOS::Obj* bucket_obj)
{
    ...
    get_bucket_index_object(bucket_oid_base, idx_layout.layout.normal.num_shards, shard_id, idx_layout.gen, &oid);
...
int RGWSI_BucketIndex_RADOS::cls_bucket_head(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                             int shard_id, vector<rgw_bucket_dir_header>* headers,
                                                ...
                                                list<cls_rgw_bucket_instance_entry>* status)
{
    ...
    int r = open_bucket_index(dpp, bucket_info, std::nullopt, &index_pool, &bucket_objs, nullptr);
    if (r < 0) {
        return r;
    }
                                              ...
                                              const RGWBucketInfo& orig_info)
{
    ...
    if (old_sync_enabled != new_sync_enabled) {
        int shards_num =
            info.layout.current_index.layout.normal.num_shards ? info.layout.current_index.layout.normal.num_shards : 1;
        ...
        for (int i = 0; i < shards_num; ++i, ++shard_id) {
            ...
            if (ret < 0) {
                ldpp_dout(dpp, -1) << "ERROR: failed writing data log (info.bucket=" << info.bucket
