...
class RGWWatcher : public DoutPrefixProvider, public librados::WatchCtx2
{
    ...
    public:
        explicit C_ReinitWatch(RGWWatcher* _watcher)
            : watcher(_watcher)
        {}
        void finish(int r) override { watcher->reinit(); }
    };

    CephContext* get_cct() const override { return cct; }
    unsigned get_subsys() const override { return dout_subsys; }
    std::ostream& gen_prefix(std::ostream& out) const override { return out << "rgw watcher librados: "; }
...
public:
    ...
    void handle_notify(uint64_t notify_id, uint64_t cookie, uint64_t notifier_id, bufferlist& bl) override
    {
        ldpp_dout(this, 10) << "RGWWatcher::handle_notify() "
                            << " notify_id " << notify_id << " cookie " << cookie << " notifier " << notifier_id
                            << " bl.length()=" << bl.length() << dendl;

        if (unlikely(svc->inject_notify_timeout_probability == 1) ||
            (svc->inject_notify_timeout_probability > 0 &&
             (svc->inject_notify_timeout_probability > ceph::util::generate_random_number(0.0, 1.0)))) {
            ldpp_dout(this, 0) << "RGWWatcher::handle_notify() dropping notification! "
    ...
    {
        ldpp_dout(this, -1) << "RGWWatcher::handle_error cookie " << cookie << " err " << cpp_strerror(err) << dendl;
...
public:
    RGWSI_Notify_ShutdownCB(RGWSI_Notify* _svc)
        : svc(_svc)
    {}
    void call() override { svc->shutdown(); }
};
...
int RGWSI_Notify::init_watch(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    if (num_watchers <= 0) num_watchers = 1;
...
void RGWSI_Notify::finalize_watch()
{
    for (int i = 0; i < num_watchers; i++) {
        ...
        if (watchers_set.find(i) != watchers_set.end()) watcher->unregister_watch();
        delete watcher;
    }
...
int RGWSI_Notify::watch_cb(const DoutPrefixProvider* dpp, uint64_t notify_id, uint64_t cookie, uint64_t notifier_id,
                           bufferlist& bl)
{
...
int RGWSI_Notify::distribute(const DoutPrefixProvider* dpp, const string& key, const RGWCacheNotifyInfo& cni,
                             optional_yield y)
{
    ...
    */
    if (num_watchers > 0) {
        ...
        ldpp_dout(dpp, 10) << "distributing notification oid=" << notify_obj.get_ref().obj << " cni=" << cni << dendl;
                                ...
                                const RGWCacheNotifyInfo& cni, optional_yield y)
{
    ...
    if (r < 0) {
        ...
        try {
            ...
            ldpp_dout(dpp, 0) << "robust_notify failed to decode notify response: " << e.what() << dendl;
        }

        ldpp_dout(dpp, 1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " Watchers " << timeouts << " did not respond."
                          << " Notify failed on object " << cni.obj << ": " << cpp_strerror(-r) << dendl;
    }
    ...
    if (r == -ETIMEDOUT) {
        ...
        for (auto tries = 0u; r == -ETIMEDOUT && tries < max_notify_retries; ++tries) {
            ldpp_dout(dpp, 1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " Invalidating obj=" << info.obj
                              << " tries=" << tries << dendl;
            ...
            if (r < 0) {
                ...
                ldpp_dout(dpp, 1) << __PRETTY_FUNCTION__ << ":" << __LINE__ << " Watchers " << timeouts
                                  << " did not respond."
                                  << " Invalidation attempt " << tries << " failed: " << cpp_strerror(-r) << dendl;
            }
        }
    }
    return r;
