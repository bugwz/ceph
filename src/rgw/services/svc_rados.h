...
struct RGWAccessListFilterPrefix : public RGWAccessListFilter
{
    ...
    explicit RGWAccessListFilterPrefix(const std::string& _prefix)
        : prefix(_prefix)
    {}
...
private:
    ...
    int pool_iterate(const DoutPrefixProvider* dpp, librados::IoCtx& ioctx, librados::NObjectIterator& iter,
                     uint32_t num, std::vector<rgw_bucket_dir_entry>& objs, RGWAccessListFilter* filter,
                     bool* is_truncated);
...
public:
    ...
    RGWAsyncRadosProcessor* get_async_processor() { return async_processor.get(); }
    ...
    class Pool
    {
        ...
        Pool(RGWSI_RADOS* _rados_svc, const rgw_pool& _pool)
    ...
    public:
        ...
        const rgw_pool& get_pool() { return pool; }

        librados::IoCtx& ioctx() & { return state.ioctx; }

        librados::IoCtx&& ioctx() && { return std::move(state.ioctx); }
    ...
    public:
        ...
        int operate(const DoutPrefixProvider* dpp, librados::ObjectWriteOperation* op, optional_yield y, int flags = 0);
        int operate(const DoutPrefixProvider* dpp, librados::ObjectReadOperation* op, bufferlist* pbl, optional_yield y,
                    int flags = 0);
        int aio_operate(librados::AioCompletion* c, librados::ObjectWriteOperation* op);
        int aio_operate(librados::AioCompletion* c, librados::ObjectReadOperation* op, bufferlist* pbl);
        ...
        const rgw_raw_obj& get_raw_obj() const { return ref.obj; }
    };
    ...
    public:
        ...
        Pool pool(const rgw_pool& p) { return Pool(rados_svc, p); }
    ...
    Obj obj(const rgw_raw_obj& o) { return Obj(this, o); }

    Obj obj(Pool& pool, const std::string& oid) { return Obj(pool, oid); }

    Pool pool() { return Pool(this); }

    Pool pool(const rgw_pool& p) { return Pool(this, p); }
