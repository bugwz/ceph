...
class RGWSI_SysObj_Cache : public RGWSI_SysObj_Core
{
    ...
    void normalize_pool_and_obj(const rgw_pool& src_pool, const std::string& src_obj, rgw_pool& dst_pool,
                                std::string& dst_obj);

protected:
    void init(RGWSI_RADOS* _rados_svc, RGWSI_Zone* _zone_svc, RGWSI_Notify* _notify_svc)
    {
    ...
    int raw_stat(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, uint64_t* psize, real_time* pmtime,
                 std::map<std::string, bufferlist>* attrs, RGWObjVersionTracker* objv_tracker,
                 optional_yield y) override;

    int read(const DoutPrefixProvider* dpp, RGWSI_SysObj_Obj_GetObjState& read_state,
             ...
             rgw_cache_entry_info* cache_info, boost::optional<obj_version>, optional_yield y) override;

    int get_attr(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, const char* name, bufferlist* dest,
                 optional_yield y) override;

    int set_attrs(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, std::map<std::string, bufferlist>& attrs,
                  std::map<std::string, bufferlist>* rmattrs, RGWObjVersionTracker* objv_tracker, bool exclusive,
                  optional_yield y) override;

    int remove(const DoutPrefixProvider* dpp, RGWObjVersionTracker* objv_tracker, const rgw_raw_obj& obj,
               optional_yield y) override;

    int write(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, real_time* pmtime,
              std::map<std::string, bufferlist>& attrs, bool exclusive, const bufferlist& data,
              RGWObjVersionTracker* objv_tracker, real_time set_mtime, optional_yield y) override;

    int write_data(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, const bufferlist& bl, bool exclusive,
                   RGWObjVersionTracker* objv_tracker, optional_yield y);

    int distribute_cache(const DoutPrefixProvider* dpp, const std::string& normal_name, const rgw_raw_obj& obj,
                         ObjectCacheInfo& obj_info, int op, optional_yield y);
...
public:
    RGWSI_SysObj_Cache(const DoutPrefixProvider* dpp, CephContext* cct)
...
template<class T> class RGWChainedCacheImpl : public RGWChainedCache
{
...
public:
    ...
    void unregistered() override { svc = nullptr; }
    ...
    void init(RGWSI_SysObj_Cache* _svc)
    {
        ...
        expiry = std::chrono::seconds(svc->ctx()->_conf.get_val<uint64_t>("rgw_cache_expiry_interval"));
    }
    ...
    boost::optional<T> find(const std::string& key)
    {
        ...
        if (expiry.count() && (ceph::coarse_mono_clock::now() - iter->second.second) > expiry) {
            return boost::none;
        }
