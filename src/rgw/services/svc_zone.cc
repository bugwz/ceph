...
int RGWSI_Zone::search_realm_with_zone(const DoutPrefixProvider* dpp, const rgw_zone_id& zid, RGWRealm* prealm,
                                       RGWPeriod* pperiod, RGWZoneGroup* pzonegroup, bool* pfound, optional_yield y)
{
    auto& found = *pfound;
    ...
    for (auto& realm_name : realms) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "WARNING: can't open realm " << realm_name << ": " << cpp_strerror(-r)
                              << " ... skipping" << dendl;
            continue;
        }

        r = realm.find_zone(dpp, zid, pperiod, pzonegroup, &found, y);
        if (r < 0) {
        ...
        if (found) {
            *prealm = realm;
            ldpp_dout(dpp, 20) << __func__ << "(): found realm_id=" << realm_id << " realm_name=" << realm_name
                               << dendl;
            return 0;
        }
    }
...
int RGWSI_Zone::do_start(optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    if (found_zone && !current_period->get_id().empty()) {
        found_period_conf = current_period->find_zone(dpp, cur_zone_id, zonegroup, y);
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: current_period->find_zone() returned ret=" << ret << dendl;
            return ret;
        }
        if (!found_period_conf) {
            ldpp_dout(dpp, 0) << "period (" << current_period->get_id() << " does not have zone " << cur_zone_id
                              << " configured" << dendl;
        }
    }
    ...
    if (found_zone && !found_period_conf) {
        ldpp_dout(dpp, 20) << "searching for the correct realm" << dendl;
        ret = search_realm_with_zone(dpp, cur_zone_id, realm, current_period, zonegroup, &found_period_conf, y);
        if (ret < 0) {
    ...
    if (!zg_initialized && (!explicit_zg || zonegroup_param == default_zonegroup_name)) {
    ...
    if (explicit_zg && zonegroup->get_name() != zonegroup_param) {
        ldpp_dout(dpp, 0) << "ERROR: incorrect zonegroup: " << zonegroup_param << " (got: " << zonegroup_param
                          << ", expected: " << zonegroup->get_name() << ")" << dendl;
        return -EINVAL;
    }
    ...
    if (!found_zone) {
        if ((!explicit_zone || zone_param == default_zone_name) && zonegroup->get_name() == default_zonegroup_name) {
    ...
    *zone_public_config = zone_iter->second;
    ...
    ldpp_dout(dpp, 4) << "Realm:     " << std::left << setw(20) << realm->get_name() << " (" << realm->get_id() << ")"
                      << dendl;
    ldpp_dout(dpp, 4) << "ZoneGroup: " << std::left << setw(20) << zonegroup->get_name() << " (" << zonegroup->get_id()
                      << ")" << dendl;
    ldpp_dout(dpp, 4) << "Zone:      " << std::left << setw(20) << zone_params->get_name() << " ("
                      << zone_params->get_id() << ")" << dendl;

    if (init_from_period) {
        ldpp_dout(dpp, 4) << "using period configuration: " << current_period->get_id() << ":"
                          << current_period->get_epoch() << dendl;
    ...
    sync_policy_handler->reflect(dpp,
                                 nullptr,
                                 nullptr,
                                 nullptr,
                                 nullptr,
    ...
    for (const auto& ziter : zonegroup->zones) {
        ...
        RGWRESTConn* conn =
            new RGWRESTConn(cct, z.id, z.endpoints, zone_params->system_key, zonegroup->get_id(), zonegroup->api_name);
...
int RGWSI_Zone::init_zg_from_period(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    for (iter = current_period->get_map().zonegroups.begin(); iter != current_period->get_map().zonegroups.end();
        ...
        add_new_connection_to_map(
            zonegroup_conn_map,
            zg,
            new RGWRESTConn(cct, zg.get_id(), endpoints, zone_params->system_key, zonegroup->get_id(), zg.api_name));
...
int RGWSI_Zone::init_zg_from_local(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    if (zonegroup->is_master_zonegroup()) {
        ...
        const auto& endpoints = master->second.endpoints;
        rest_master_conn = new RGWRESTConn(
            cct, zonegroup->get_id(), endpoints, zone_params->system_key, zonegroup->get_id(), zonegroup->api_name);
    }
