...
RGWSI_Zone::RGWSI_Zone(CephContext* cct)
    : RGWServiceInstance(cct)
{}

void RGWSI_Zone::init(RGWSI_SysObj* _sysobj_svc, RGWSI_RADOS* _rados_svc, RGWSI_SyncModules* _sync_modules_svc,
...
int RGWSI_Zone::do_start(optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    if (ret < 0 && ret != -ENOENT) {
        ...
        ret = current_period->init(dpp, cct, sysobj_svc, realm->get_id(), y, realm->get_name());
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 0) << "failed reading current period info: "
    ...
    if (ret < 0) {
        ldpp_dout(dpp, -1) << "failed converting region to zonegroup : ret " << ret << " " << cpp_strerror(-ret)
                           << dendl;
        return ret;
    }
    ...
    if (zone_iter == zonegroup->zones.end()) {
        if (using_local) {
            ldpp_dout(dpp, -1) << "Cannot find zone id=" << zone_params->get_id()
                               << " (name=" << zone_params->get_name() << ")" << dendl;
            return -EINVAL;
        }
        ldpp_dout(dpp, 1) << "Cannot find zone id=" << zone_params->get_id() << " (name=" << zone_params->get_name()
                          << "), switching to local zonegroup configuration" << dendl;
        ret = init_zg_from_local(dpp, &creating_defaults, y);
        if (ret < 0) {
            return ret;
        }
    ...
    for (auto ziter : zonegroup->zones) {
        auto zone_handler =
            std::make_shared<RGWBucketSyncPolicyHandler>(this, sync_modules_svc, bucket_sync_svc, ziter.second.id);
        ret = zone_handler->init(dpp, y);
        if (ret < 0) {
            ldpp_dout(dpp, -1) << "ERROR: could not initialize zone policy handler for zone=" << ziter.second.name
                               << dendl;
            return ret;
        }
    ...
    sync_policy_handler->reflect(nullptr,
                                 nullptr,
                                 nullptr,
                                 nullptr,
                                 &source_zones,
    ...
    if (zone_by_id.find(zone_id()) == zone_by_id.end()) {
        ldpp_dout(dpp, 0) << "WARNING: could not find zone config in zonegroup for local zone (" << zone_id()
                          << "), will use defaults" << dendl;
    }
    ...
    for (const auto& ziter : zonegroup->zones) {
        ...
        if (z.endpoints.empty()) {
            ldpp_dout(dpp, 0) << "WARNING: can't generate connection for zone " << z.id << " id " << z.name
...
int RGWSI_Zone::list_periods(const DoutPrefixProvider* dpp, const string& current_period, list<string>& periods,
                             optional_yield y)
{
    int ret = 0;
...
int RGWSI_Zone::replace_region_with_zonegroup(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(dpp, 0) << __func__ << " failed to read converted: ret " << ret << " " << cpp_strerror(-ret) << dendl;
        return ret;
    }
    else if (ret != -ENOENT) {
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << __func__ << " failed init default region: ret " << ret << " " << cpp_strerror(-ret)
                          << dendl;
        return ret;
    }
    ret = default_zonegroup.read_default_id(dpp, default_region, y, true);
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(dpp, 0) << __func__ << " failed reading old default region: ret " << ret << " " << cpp_strerror(-ret)
                          << dendl;
        return ret;
    }
    ...
    if (ret < 0 && ret != -ENOENT) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 0) << __func__ << ": error initializing default zone params: " << cpp_strerror(-ret)
                              << dendl;
            return ret;
        }
        /* update master zone */
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 0) << __func__ << ": error in initializing default zonegroup: " << cpp_strerror(-ret)
                              << dendl;
            return ret;
        }
    ...
    for (iter = regions.begin(); iter != regions.end(); ++iter) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << __func__ << " failed init zonegroup: ret " << ret << " " << cpp_strerror(-ret)
                              << dendl;
            return ret;
        }
        ...
        if (ret < 0 && ret != -EEXIST) {
            ldpp_dout(dpp, 0) << __func__ << " failed to update zonegroup " << *iter << ": ret " << ret << " "
                              << cpp_strerror(-ret) << dendl;
            return ret;
        }
        ret = zonegroup.update_name(dpp, y);
        if (ret < 0 && ret != -EEXIST) {
            ldpp_dout(dpp, 0) << __func__ << " failed to update_name for zonegroup " << *iter << ": ret " << ret << " "
                              << cpp_strerror(-ret) << dendl;
            return ret;
        }
        if (zonegroup.get_name() == default_region) {
            ...
            if (ret < 0) {
                ldpp_dout(dpp, 0) << __func__ << " failed to set_as_default " << *iter << ": ret " << ret << " "
                                  << cpp_strerror(-ret) << dendl;
                return ret;
            }
        }
        for (auto iter = zonegroup.zones.begin(); iter != zonegroup.zones.end(); ++iter) {
            ...
            if (ret < 0 && ret != -ENOENT) {
                ldpp_dout(dpp, 0) << __func__ << " failed to init zoneparams  " << iter->first << ": "
                                  << cpp_strerror(-ret) << dendl;
                return ret;
            }
            else if (ret == -ENOENT) {
                ldpp_dout(dpp, 0) << __func__ << " zone is part of another cluster " << iter->first << " skipping "
                                  << dendl;
                continue;
            }
            zonegroup.realm_id = realm->get_id();
            ...
            if (ret < 0 && ret != -EEXIST) {
                ldpp_dout(dpp, 0) << __func__ << " failed to update zoneparams " << iter->first << ": "
                                  << cpp_strerror(-ret) << dendl;
                return ret;
            }
        ...
        if (!current_period->get_id().empty()) {
            ...
            if (ret < 0) {
                ldpp_dout(dpp, 0) << __func__ << " failed to add zonegroup to current_period: " << cpp_strerror(-ret)
                                  << dendl;
                return ret;
            }
        }
    ...
    for (auto const& iter : regions) {
        ...
        int ret = zonegroup.init(dpp, cct, sysobj_svc, y, true, true);
        if (ret < 0) {
            ldpp_dout(dpp, 0) << __func__ << " failed init zonegroup" << iter << ": ret " << ret << " "
                              << cpp_strerror(-ret) << dendl;
            return ret;
        }
        ret = zonegroup.delete_obj(dpp, y, true);
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 0) << __func__ << " failed to delete region " << iter << ": ret " << ret << " "
                              << cpp_strerror(-ret) << dendl;
            return ret;
        }
    }
    ...
    /* mark as converted */
    ret = sysobj.wop().set_exclusive(true).write(dpp, bl, y);
    if (ret < 0) {
        ldpp_dout(dpp, 0) << __func__ << " failed to mark cluster as converted: ret " << ret << " "
                          << cpp_strerror(-ret) << dendl;
        return ret;
    }
 ...
 */
static void add_new_connection_to_map(map<string, RGWRESTConn*>& zonegroup_conn_map, const RGWZoneGroup& zonegroup,
                                      RGWRESTConn* new_connection)
{
...
int RGWSI_Zone::init_zg_from_period(const DoutPrefixProvider* dpp, bool* initialized, optional_yield y)
{
    *initialized = false;
    ...
    map<string, RGWZoneGroup>::const_iterator iter = current_period->get_map().zonegroups.find(zonegroup->get_id());
    ...
    if (iter != current_period->get_map().zonegroups.end()) {
        ...
        *zonegroup = iter->second;
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "failed init zonegroup: "
                              << " " << cpp_strerror(-ret) << dendl;
            return ret;
        }
        ret = zone_params->init(dpp, cct, sysobj_svc, y);
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 0) << "failed reading zone params info: "
                              << " " << cpp_strerror(-ret) << dendl;
            return ret;
        }
    ...
    for (iter = current_period->get_map().zonegroups.begin(); iter != current_period->get_map().zonegroups.end();
         ++iter) {
        ...
        if (master == zg.zones.end()) {
            ...
            if (zg.master_zone.empty() && zg.zones.size() == 1) {
                ...
                ldpp_dout(dpp, 0) << "zonegroup " << zg.get_name() << " missing zone for master_zone=" << zg.master_zone
                                  << dendl;
                return -EINVAL;
            }
        }
        ...
        if (!current_period->get_master_zonegroup().empty() && zg.get_id() == current_period->get_master_zonegroup()) {
    ...
    if (zonegroup->is_master_zonegroup()) {
        ...
        if (master == zonegroup->zones.end()) {
            ...
            if (zonegroup->master_zone.empty() && zonegroup->zones.size() == 1) {
                ...
                ldpp_dout(dpp, 0) << "zonegroup " << zonegroup->get_name()
                                  << " missing zone for "
                                     "master_zone="
                                  << zonegroup->master_zone << dendl;
                return -EINVAL;
            }
        }
        const auto& endpoints = master->second.endpoints;
...
int RGWSI_Zone::convert_regionmap(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    for (map<string, RGWZoneGroup>::iterator iter = zonegroupmap.zonegroups.begin();
         iter != zonegroupmap.zonegroups.end();
         ++iter) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            ...
            if (ret < 0) {
                ldpp_dout(dpp, 0) << "Error could not create " << zonegroup.get_name() << ": " << cpp_strerror(-ret)
                                  << dendl;
                return ret;
            }
        }
...
bool RGWSI_Zone::has_zonegroup_api(const std::string& api) const
{
    if (!current_period->get_id().empty()) {
        ...
        if (zonegroups_by_api.find(api) != zonegroups_by_api.end()) return true;
    }
...
{
    return !(zonegroup->master_zone.empty() || !rest_master_conn || current_period->get_id().empty());
}
...
{
    return is_meta_master() && (zonegroup->zones.size() > 1 || current_period->is_multi_zonegroups_with_zones());
}

bool RGWSI_Zone::can_reshard() const
{
    return current_period->get_id().empty() || (zonegroup->zones.size() == 1 && current_period->is_single_zonegroup());
}
...
int RGWSI_Zone::select_new_bucket_location(const DoutPrefixProvider* dpp, const RGWUserInfo& user_info,
                                           const string& zonegroup_id, const rgw_placement_rule& request_rule,
                                           ...
                                           optional_yield y)
{
    ...
    if (!request_rule.name.empty()) {
        ...
        if (titer == zonegroup.placement_targets.end()) {
            ldpp_dout(dpp, 0) << "could not find requested placement id " << request_rule << " within zonegroup "
                              << dendl;
...
int RGWSI_Zone::select_bucket_location_by_rule(const DoutPrefixProvider* dpp, const rgw_placement_rule& location_rule,
                                               RGWZonePlacementInfo* rule_info, optional_yield y)
{
    ...
    if (piter == zone_params->placement_pools.end()) {
        ...
        ldpp_dout(dpp, 0) << "ERROR: This zone does not contain placement rule " << location_rule
...
int RGWSI_Zone::select_bucket_placement(const DoutPrefixProvider* dpp, const RGWUserInfo& user_info,
                                        ...
                                        optional_yield y)
{
    if (!zone_params->placement_pools.empty()) {
        return select_new_bucket_location(dpp, user_info, zonegroup_id, placement_rule, pselected_rule, rule_info, y);
    }
