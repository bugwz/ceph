...
public:
    RGWSI_SysObj_Cache_CB(RGWSI_SysObj_Cache* _svc)
        : svc(_svc)
    {}
    ...
    void set_enabled(bool status) { svc->set_enabled(status); }
};
...
void RGWSI_SysObj_Cache::normalize_pool_and_obj(const rgw_pool& src_pool, const string& src_obj, rgw_pool& dst_pool,
                                                string& dst_obj)
{
    if (src_obj.size()) {
        dst_pool = src_pool;
...
int RGWSI_SysObj_Cache::remove(const DoutPrefixProvider* dpp, RGWSysObjectCtxBase& obj_ctx,
...
int RGWSI_SysObj_Cache::read(const DoutPrefixProvider* dpp, RGWSysObjectCtxBase& obj_ctx,
                             RGWSI_SysObj_Obj_GetObjState& read_state, RGWObjVersionTracker* objv_tracker,
                             const rgw_raw_obj& obj, bufferlist* obl, off_t ofs, off_t end,
                             map<string, bufferlist>* attrs, bool raw_attrs, rgw_cache_entry_info* cache_info,
                             boost::optional<obj_version> refresh_version, optional_yield y)
{
    rgw_pool pool;
    string oid;
    if (ofs != 0) {
        return RGWSI_SysObj_Core::read(dpp,
                                       obj_ctx,
                                       read_state,
                                       objv_tracker,
                                       obj,
    ...
    if (objv_tracker) flags |= CACHE_FLAG_OBJV;
    if (attrs) flags |= CACHE_FLAG_XATTRS;

    int r = cache.get(dpp, name, info, flags, cache_info);
    if (r == 0 && (!refresh_version || !info.version.compare(&(*refresh_version)))) {
        ...
        i.copy_all(*obl);
        if (objv_tracker) objv_tracker->read_version = info.version;
        if (attrs) {
            if (raw_attrs) {
                *attrs = info.xattrs;
            }
    ...
    if (r == -ENODATA) return -ENOENT;

    map<string, bufferlist> unfiltered_attrset;
    r = RGWSI_SysObj_Core::read(dpp,
                                obj_ctx,
                                read_state,
                                objv_tracker,
                                obj,
...
int RGWSI_SysObj_Cache::get_attr(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, const char* attr_name,
...
int RGWSI_SysObj_Cache::set_attrs(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, map<string, bufferlist>& attrs,
                                  map<string, bufferlist>* rmattrs, RGWObjVersionTracker* objv_tracker,
                                  optional_yield y)
{
    rgw_pool pool;
    string oid;
    ...
    if (ret >= 0) {
        ...
        if (r < 0) ldpp_dout(dpp, 0) << "ERROR: failed to distribute cache for " << obj << dendl;
    }
    else {
        cache.remove(dpp, name);
    }
...
int RGWSI_SysObj_Cache::write(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, real_time* pmtime,
                              ...
                              RGWObjVersionTracker* objv_tracker, real_time set_mtime, optional_yield y)
{
    ...
    int ret = RGWSI_SysObj_Core::write(dpp, obj, &result_mtime, attrs, exclusive, data, objv_tracker, set_mtime, y);
    if (pmtime) {
        *pmtime = result_mtime;
    }
    if (objv_tracker && objv_tracker->read_version.ver) {
...
int RGWSI_SysObj_Cache::write_data(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, const bufferlist& data,
...
int RGWSI_SysObj_Cache::raw_stat(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, uint64_t* psize,
                                 real_time* pmtime, uint64_t* pepoch, map<string, bufferlist>* attrs,
                                 bufferlist* first_chunk, RGWObjVersionTracker* objv_tracker, optional_yield y)
{
    ...
    r = RGWSI_SysObj_Core::raw_stat(dpp, obj, &size, &mtime, &epoch, &info.xattrs, first_chunk, objv_tracker, y);
    if (r < 0) {
        if (r == -ENOENT) {
            info.status = r;
            cache.put(dpp, name, info, NULL);
    ...
    if (pepoch) *pepoch = epoch;
    if (attrs) *attrs = info.xattrs;
    return 0;
}

int RGWSI_SysObj_Cache::distribute_cache(const DoutPrefixProvider* dpp, const string& normal_name,
                                         const rgw_raw_obj& obj, ObjectCacheInfo& obj_info, int op, optional_yield y)
{
...
int RGWSI_SysObj_Cache::watch_cb(const DoutPrefixProvider* dpp, uint64_t notify_id, uint64_t cookie,
                                 uint64_t notifier_id, bufferlist& bl)
{
    ...
    switch (info.op) {
    case UPDATE_OBJ: cache.put(dpp, name, info.obj_info, NULL); break;
    case REMOVE_OBJ: cache.remove(dpp, name); break;
    default: ldout(cct, 0) << "WARNING: got unknown notification op: " << info.op << dendl; return -EINVAL;
    }
...
static void cache_list_dump_helper(Formatter* f, const std::string& name, const ceph::real_time mtime,
...
class RGWSI_SysObj_Cache_ASocketHook : public AdminSocketHook
{
    ...
    static constexpr std::string_view admin_commands[][2] = {
        ...
        {"cache inspect name=target,type=CephString,req=true", "cache inspect target: print cache element"},
        {"cache erase name=target,type=CephString,req=true", "cache erase target: erase element from cache"},
        {"cache zap", "cache zap: erase all elements from cache"}};

public:
    RGWSI_SysObj_Cache_ASocketHook(RGWSI_SysObj_Cache* _svc)
        : svc(_svc)
    {}
    ...
    int call(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, std::ostream& ss,
...
int RGWSI_SysObj_Cache_ASocketHook::call(std::string_view command, const cmdmap_t& cmdmap, Formatter* f,
...
RGWSI_SysObj_Cache::ASocketHandler::ASocketHandler(const DoutPrefixProvider* _dpp, RGWSI_SysObj_Cache* _svc)
    : dpp(_dpp)
    , svc(_svc)
{
...
{
    svc->cache.for_each([&filter, f](const string& name, const ObjectCacheEntry& entry) {
        if (!filter || name.find(*filter) != name.npos) {
            cache_list_dump_helper(f, name, entry.info.meta.mtime, entry.info.meta.size);
        }
    });
}
