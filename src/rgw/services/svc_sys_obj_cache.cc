...
int RGWSI_SysObj_Cache::remove(const DoutPrefixProvider* dpp, RGWObjVersionTracker* objv_tracker,
                               const rgw_raw_obj& obj, optional_yield y)
...
int RGWSI_SysObj_Cache::read(const DoutPrefixProvider* dpp, RGWSI_SysObj_Obj_GetObjState& read_state,
                             RGWObjVersionTracker* objv_tracker, const rgw_raw_obj& obj, bufferlist* obl, off_t ofs,
                             off_t end, ceph::real_time* pmtime, uint64_t* psize, map<string, bufferlist>* attrs,
                             bool raw_attrs, rgw_cache_entry_info* cache_info,
                             boost::optional<obj_version> refresh_version, optional_yield y)
{
    ...
    if (pmtime || psize) flags |= CACHE_FLAG_META;
...
int RGWSI_SysObj_Cache::set_attrs(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, map<string, bufferlist>& attrs,
                                  map<string, bufferlist>* rmattrs, RGWObjVersionTracker* objv_tracker, bool exclusive,
                                  optional_yield y)
{
    rgw_pool pool;
    string oid;
...
int RGWSI_SysObj_Cache::raw_stat(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, uint64_t* psize,
                                 real_time* pmtime, map<string, bufferlist>* attrs, RGWObjVersionTracker* objv_tracker,
                                 optional_yield y)
{
    rgw_pool pool;
    string oid;
    ...
    r = RGWSI_SysObj_Core::raw_stat(dpp, obj, &size, &mtime, &info.xattrs, objv_tracker, y);
    if (r < 0) {
        if (r == -ENOENT) {
            info.status = r;
            cache.put(dpp, name, info, NULL);
                                 ...
                                 uint64_t notifier_id, bufferlist& bl)
{
    ...
    case INVALIDATE_OBJ: cache.invalidate_remove(dpp, name); break;
    default: ldpp_dout(dpp, 0) << "WARNING: got unknown notification op: " << info.op << dendl; return -EINVAL;
    }
...
int RGWSI_SysObj_Cache_ASocketHook::call(std::string_view command, const cmdmap_t& cmdmap, const bufferlist&,
