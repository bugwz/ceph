...
public:
    RGWMPObj() {}
    RGWMPObj(const std::string& _oid, const std::string& _upload_id) { init(_oid, _upload_id, _upload_id); }
    ...
    {
        return out << "RGWMPObj:{ prefix=" << std::quoted(obj.prefix) << ", meta=" << std::quoted(obj.meta) << " }";
    }
};   // class RGWMPObj
...
public:
    ...
    void init(RGWSI_Zone* _zone_svc) { zone_svc = _zone_svc; }
