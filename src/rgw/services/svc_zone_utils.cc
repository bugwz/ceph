...
string RGWSI_ZoneUtils::gen_host_id()
{
    ...
    snprintf(charbuf,
             sizeof(charbuf),
             "%llx-%s-%s",
             (unsigned long long)rados_svc->instance_id(),
             zone_name.c_str(),
             zonegroup_name.c_str());
...
string RGWSI_ZoneUtils::unique_id(uint64_t unique_num)
{
    char buf[32];
    snprintf(
        buf, sizeof(buf), ".%llu.%llu", (unsigned long long)rados_svc->instance_id(), (unsigned long long)unique_num);
...
string RGWSI_ZoneUtils::unique_trans_id(const uint64_t unique_num)
{
    ...
    snprintf(buf, sizeof(buf), "tx%021llx-%010llx", (unsigned long long)unique_num, (unsigned long long)timestamp);
