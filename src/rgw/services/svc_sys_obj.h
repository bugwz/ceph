...
public:
    ...
    public:
        ...
        ROp rop() { return ROp(*this); }

        WOp wop() { return WOp(*this); }

        OmapOp omap() { return OmapOp(*this); }

        WNOp wn() { return WNOp(*this); }
    };
    ...
    protected:
        ...
        struct ListCtx
        {
            ceph::static_ptr<ListImplInfo, sizeof(RGWSI_SysObj_Core_PoolListImplInfo)>
                impl; /* update this if creating new backend types */
        };

    public:
        Pool(RGWSI_SysObj_Core* _core_svc, const rgw_pool& _pool)
            : core_svc(_core_svc)
            , pool(_pool)
        {}

        rgw_pool& get_pool() { return pool; }
        ...
        int list_prefixed_objs(const DoutPrefixProvider* dpp, const std::string& prefix, Container* result)
        {
            return list_prefixed_objs(dpp, prefix, [&](const std::string& val) { result->push_back(val); });
        }

        Op op() { return Op(*this); }
    };
...
protected:
    ...
    void init(RGWSI_RADOS* _rados_svc, RGWSI_SysObj_Core* _core_svc)
    {
...
public:
    RGWSI_SysObj(CephContext* cct)
        : RGWServiceInstance(cct)
    {}

    Obj get_obj(const rgw_raw_obj& obj);

    Pool get_pool(const rgw_pool& pool) { return Pool(core_svc, pool); }
