...
public:
    RGWSI_Role_Module(RGWSI_Role_RADOS::Svc& _svc)
        : RGWSI_MBSObj_Handler_Module("roles")
        , svc(_svc)
    ...
    bool is_valid_oid(const std::string& oid) override { return boost::algorithm::starts_with(oid, prefix); }

    std::string key_to_oid(const std::string& key) override { return prefix + key; }

    // This is called after `is_valid_oid` and is assumed to be a valid oid
    std::string oid_to_key(const std::string& oid) override { return oid.substr(prefix.size()); }
...
void RGWSI_Role_RADOS::init(RGWSI_Zone* _zone_svc, RGWSI_Meta* _meta_svc, RGWSI_MetaBackend* _meta_be_svc,
...
int RGWSI_Role_RADOS::do_start(optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    if (r < 0) {
        ldout(ctx(), 0) << "ERROR: failed to create be_handler for Roles: r=" << r << dendl;
        return r;
    }
