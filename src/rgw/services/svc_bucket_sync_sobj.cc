...
public:
    RGWSI_Bucket_Sync_SObj_HintIndexManager(RGWSI_Zone* _zone_svc, RGWSI_SysObj* _sysobj_svc)
    {
        svc.zone = _zone_svc;
    ...
    int update_hints(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info, C1& added_dests,
                     C2& removed_dests, C1& added_sources, C2& removed_sources, optional_yield y);
};

RGWSI_Bucket_Sync_SObj::RGWSI_Bucket_Sync_SObj(CephContext* cct)
    : RGWSI_Bucket_Sync(cct)
{}
RGWSI_Bucket_Sync_SObj::~RGWSI_Bucket_Sync_SObj() {}

void RGWSI_Bucket_Sync_SObj::init(RGWSI_Zone* _zone_svc, RGWSI_SysObj* _sysobj_svc, RGWSI_SysObj_Cache* _cache_svc,
...
void RGWSI_Bucket_Sync_SObj::get_hint_entities(RGWSI_Bucket_X_Ctx& ctx, const std::set<rgw_zone_id>& zones,
                                               const std::set<rgw_bucket>& buckets,
                                               std::set<rgw_sync_bucket_entity>* hint_entities, optional_yield y,
                                               const DoutPrefixProvider* dpp)
{
    ...
    for (auto& b : buckets) {
        ...
        int ret = svc.bucket_sobj->read_bucket_info(ctx, b, &hint_bucket_info, nullptr, nullptr, boost::none, y, dpp);
        if (ret < 0) {
...
int RGWSI_Bucket_Sync_SObj::resolve_policy_hints(
    RGWSI_Bucket_X_Ctx& ctx, rgw_sync_bucket_entity& self_entity, RGWBucketSyncPolicyHandlerRef& handler,
    RGWBucketSyncPolicyHandlerRef& zone_policy_handler,
    std::map<optional_zone_bucket, RGWBucketSyncPolicyHandlerRef>& temp_map, optional_yield y,
    const DoutPrefixProvider* dpp)
{
...
int RGWSI_Bucket_Sync_SObj::do_get_policy_handler(
    RGWSI_Bucket_X_Ctx& ctx, std::optional<rgw_zone_id> zone, std::optional<rgw_bucket> _bucket,
    std::map<optional_zone_bucket, RGWBucketSyncPolicyHandlerRef>& temp_map, RGWBucketSyncPolicyHandlerRef* handler,
    optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    int r = svc.bucket_sobj->read_bucket_instance_info(
        ctx.bi, bucket_key, &bucket_info, nullptr, &attrs, y, dpp, &cache_info);
    if (r < 0) {
        if (r != -ENOENT) {
            ldpp_dout(dpp, 0) << "ERROR: svc.bucket->read_bucket_instance_info(key=" << bucket_key
                              << ") returned r=" << r << dendl;
        }
        return r;
    }
    ...
    r = resolve_policy_hints(ctx, self_entity, e.handler, zone_policy_handler, temp_map, y, dpp);
    if (r < 0) {
        ldpp_dout(dpp, 20) << "ERROR: failed to resolve policy hints: bucket_key=" << bucket_key << ", r=" << r
                           << dendl;
        return r;
    }

    if (!sync_policy_cache->put(dpp, svc.cache, cache_key, &e, {&cache_info})) {
        ldpp_dout(dpp, 20) << "couldn't put bucket_sync_policy cache entry, might have raced with data changes"
                           << dendl;
    }
...
int RGWSI_Bucket_Sync_SObj::get_policy_handler(RGWSI_Bucket_X_Ctx& ctx, std::optional<rgw_zone_id> zone,
                                               std::optional<rgw_bucket> _bucket,
                                               RGWBucketSyncPolicyHandlerRef* handler, optional_yield y,
                                               const DoutPrefixProvider* dpp)
{
...
static bool diff_sets(std::set<rgw_bucket>& orig_set, std::set<rgw_bucket>& new_set, vector<rgw_bucket>* added,
                      vector<rgw_bucket>* removed)
{
    auto oiter = orig_set.begin();
    auto niter = new_set.begin();

    while (oiter != orig_set.end() && niter != new_set.end()) {
        if (*oiter == *niter) {
            ++oiter;
            ++niter;
            continue;
        }
        while (*oiter < *niter && oiter != orig_set.end()) {
            removed->push_back(*oiter);
            ++oiter;
        }
        while (*niter < *oiter && niter != new_set.end()) {
...
public:
    struct bi_entry
    {
        ...
        bool add(const rgw_bucket& info_source, const obj_version& info_source_ver)
        {
            ...
            if (info_source_ver.tag == ver.tag && info_source_ver.ver < ver.ver) {
                return false;
            }
        ...
        bool remove(const rgw_bucket& info_source, const obj_version& info_source_ver)
        {
    ...
    struct single_instance_info
    {
        ...
        bool add_entry(const rgw_bucket& info_source, const obj_version& info_source_ver, const rgw_bucket& bucket)
        {
        ...
        bool remove_entry(const rgw_bucket& info_source, const obj_version& info_source_ver, const rgw_bucket& bucket)
        {
    ...
    struct info_map
    {
        ...
        void get_entities(const rgw_bucket& bucket, std::set<rgw_bucket>* result) const
        {
    ...
    RGWSI_BS_SObj_HintIndexObj(RGWSI_SysObj* _sysobj_svc, const rgw_raw_obj& _obj)
        : cct(_sysobj_svc->ctx())
    ...
    template<typename C1, typename C2>
    int update(const DoutPrefixProvider* dpp, const rgw_bucket& entity, const RGWBucketInfo& info_source, C1* add,
               C2* remove, optional_yield y);

private:
    template<typename C1, typename C2>
    void update_entries(const rgw_bucket& info_source, const obj_version& info_source_ver, C1* add, C2* remove,
...
template<typename C1, typename C2>
int RGWSI_BS_SObj_HintIndexObj::update(const DoutPrefixProvider* dpp, const rgw_bucket& entity,
                                       const RGWBucketInfo& info_source, C1* add, C2* remove, optional_yield y)
{
    int r = 0;
...
#define MAX_RETRIES 25
    ...
    for (int i = 0; i < MAX_RETRIES; ++i) {
        ...
        update_entries(info_source.bucket, info_source_ver, add, remove, &instance);
    ...
    ldpp_dout(dpp, 0) << "ERROR: failed to flush hint index: too many retries (obj=" << obj << "), likely a bug"
                      << dendl;
...
template<typename C1, typename C2>
void RGWSI_BS_SObj_HintIndexObj::update_entries(const rgw_bucket& info_source, const obj_version& info_source_ver,
                                                C1* add, C2* remove, single_instance_info* instance)
{
    if (remove) {
...
int RGWSI_BS_SObj_HintIndexObj::flush(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    if (!info.empty()) {
        ...
        r = sysobj.wop().set_objv_tracker(&ot).remove(dpp, y);
    }
...
rgw_raw_obj RGWSI_Bucket_Sync_SObj_HintIndexManager::get_sources_obj(const rgw_bucket& bucket) const
{
    rgw_bucket b = bucket;
    b.bucket_id.clear();
    return rgw_raw_obj(svc.zone->get_zone_params().log_pool, bucket_sync_sources_oid_prefix + "." + b.get_key());
}
...
rgw_raw_obj RGWSI_Bucket_Sync_SObj_HintIndexManager::get_dests_obj(const rgw_bucket& bucket) const
{
    rgw_bucket b = bucket;
    b.bucket_id.clear();
    return rgw_raw_obj(svc.zone->get_zone_params().log_pool, bucket_sync_targets_oid_prefix + "." + b.get_key());
}

template<typename C1, typename C2>
int RGWSI_Bucket_Sync_SObj_HintIndexManager::update_hints(const DoutPrefixProvider* dpp,
                                                          const RGWBucketInfo& bucket_info, C1& added_dests,
                                                          C2& removed_dests, C1& added_sources, C2& removed_sources,
                                                          optional_yield y)
{
    C1 self_entity = {bucket_info.bucket};

    if (!added_dests.empty() || !removed_dests.empty()) {
        /* update our dests */
        RGWSI_BS_SObj_HintIndexObj index(svc.sysobj, get_dests_obj(bucket_info.bucket));
        int r = index.update(dpp, bucket_info.bucket, bucket_info, &added_dests, &removed_dests, y);
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket=" << bucket_info.bucket << " r=" << r
                              << dendl;
            return r;
        }
        ...
        for (auto& dest_bucket : added_dests) {
            RGWSI_BS_SObj_HintIndexObj dep_index(svc.sysobj, get_sources_obj(dest_bucket));
            int r = dep_index.update(dpp, dest_bucket, bucket_info, &self_entity, static_cast<C2*>(nullptr), y);
            if (r < 0) {
                ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket=" << dest_bucket << " r=" << r
                                  << dendl;
                return r;
            }
        }
        ...
        for (auto& dest_bucket : removed_dests) {
            RGWSI_BS_SObj_HintIndexObj dep_index(svc.sysobj, get_sources_obj(dest_bucket));
            int r = dep_index.update(dpp, dest_bucket, bucket_info, static_cast<C1*>(nullptr), &self_entity, y);
            if (r < 0) {
                ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket=" << dest_bucket << " r=" << r
                                  << dendl;
                return r;
    ...
    if (!added_sources.empty() || !removed_sources.empty()) {
        RGWSI_BS_SObj_HintIndexObj index(svc.sysobj, get_sources_obj(bucket_info.bucket));
        /* update our sources */
        int r = index.update(dpp, bucket_info.bucket, bucket_info, &added_sources, &removed_sources, y);
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket=" << bucket_info.bucket << " r=" << r
                              << dendl;
            return r;
        ...
        for (auto& source_bucket : added_sources) {
            RGWSI_BS_SObj_HintIndexObj dep_index(svc.sysobj, get_dests_obj(source_bucket));
            int r = dep_index.update(dpp, source_bucket, bucket_info, &self_entity, static_cast<C2*>(nullptr), y);
            if (r < 0) {
                ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket=" << source_bucket << " r=" << r
                                  << dendl;
                return r;
            }
        }
        ...
        for (auto& source_bucket : removed_sources) {
            RGWSI_BS_SObj_HintIndexObj dep_index(svc.sysobj, get_dests_obj(source_bucket));
            int r = dep_index.update(dpp, source_bucket, bucket_info, static_cast<C1*>(nullptr), &self_entity, y);
            if (r < 0) {
                ldpp_dout(dpp, 0) << "ERROR: failed to update targets index for bucket=" << source_bucket << " r=" << r
                                  << dendl;
                return r;
...
int RGWSI_Bucket_Sync_SObj::handle_bi_removal(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                              optional_yield y)
{
    ...
    if (bucket_info.sync_policy) {
        bucket_info.sync_policy->get_potential_related_buckets(bucket_info.bucket, &sources_set, &dests_set);
    }
    ...
    return hint_index_mgr->update_hints(
        dpp, bucket_info, added_dests, removed_dests, added_sources, removed_sources, y);
}

int RGWSI_Bucket_Sync_SObj::handle_bi_update(const DoutPrefixProvider* dpp, RGWBucketInfo& bucket_info,
                                             RGWBucketInfo* orig_bucket_info, optional_yield y)
{
    std::set<rgw_bucket> orig_sources;
    std::set<rgw_bucket> orig_dests;

    if (orig_bucket_info && orig_bucket_info->sync_policy) {
        orig_bucket_info->sync_policy->get_potential_related_buckets(bucket_info.bucket, &orig_sources, &orig_dests);
    }

    std::set<rgw_bucket> sources;
    std::set<rgw_bucket> dests;
    if (bucket_info.sync_policy) {
        bucket_info.sync_policy->get_potential_related_buckets(bucket_info.bucket, &sources, &dests);
    }

    std::vector<rgw_bucket> removed_sources;
    std::vector<rgw_bucket> added_sources;
    bool found = diff_sets(orig_sources, sources, &added_sources, &removed_sources);
    ldpp_dout(dpp, 20) << __func__ << "(): bucket=" << bucket_info.bucket << ": orig_sources=" << orig_sources
                       << " new_sources=" << sources << dendl;
    ldpp_dout(dpp, 20) << __func__ << "(): bucket=" << bucket_info.bucket
                       << ":  potential sources added=" << added_sources << " removed=" << removed_sources << dendl;

    std::vector<rgw_bucket> removed_dests;
    std::vector<rgw_bucket> added_dests;
    found = found || diff_sets(orig_dests, dests, &added_dests, &removed_dests);

    ldpp_dout(dpp, 20) << __func__ << "(): bucket=" << bucket_info.bucket << ": orig_dests=" << orig_dests
                       << " new_dests=" << dests << dendl;
    ldpp_dout(dpp, 20) << __func__ << "(): bucket=" << bucket_info.bucket << ":  potential dests added=" << added_dests
                       << " removed=" << removed_dests << dendl;

    if (!found) {
        return 0;
    }

    return hint_index_mgr->update_hints(dpp,
                                        bucket_info,
...
int RGWSI_Bucket_Sync_SObj::get_bucket_sync_hints(const DoutPrefixProvider* dpp, const rgw_bucket& bucket,
                                                  std::set<rgw_bucket>* sources, std::set<rgw_bucket>* dests,
                                                  optional_yield y)
{
    if (!sources && !dests) {
        return 0;
    ...
    if (sources) {
        RGWSI_BS_SObj_HintIndexObj index(svc.sysobj, hint_index_mgr->get_sources_obj(bucket));
    ...
    if (dests) {
        RGWSI_BS_SObj_HintIndexObj index(svc.sysobj, hint_index_mgr->get_dests_obj(bucket));
        int r = index.read(dpp, y);
        if (r < 0) {
