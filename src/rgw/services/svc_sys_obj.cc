...
RGWSI_SysObj::Obj::ROp::ROp(Obj& _source)
    : source(_source)
{
...
int RGWSI_SysObj::Obj::ROp::stat(optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    return svc->stat(source.get_ctx(), *state, obj, attrs, raw_attrs, lastmod, obj_size, objv_tracker, y, dpp);
}
...
int RGWSI_SysObj::Obj::WOp::remove(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    return svc->remove(dpp, source.get_ctx(), objv_tracker, obj, y);
}
...
int RGWSI_SysObj::Obj::WOp::write(const DoutPrefixProvider* dpp, bufferlist& bl, optional_yield y)
{
    ...
    return svc->write(dpp, obj, pmtime, attrs, exclusive, bl, objv_tracker, mtime, y);
}
...
int RGWSI_SysObj::Pool::list_prefixed_objs(const DoutPrefixProvider* dpp, const string& prefix,
...
int RGWSI_SysObj::Obj::OmapOp::get_all(const DoutPrefixProvider* dpp, std::map<string, bufferlist>* m, optional_yield y)
{
    RGWSI_SysObj_Core* svc = source.core_svc;
    rgw_raw_obj& obj = source.obj;
...
int RGWSI_SysObj::Obj::OmapOp::get_vals(const DoutPrefixProvider* dpp, const string& marker, uint64_t count,
                                        std::map<string, bufferlist>* m, bool* pmore, optional_yield y)
{
    RGWSI_SysObj_Core* svc = source.core_svc;
    rgw_raw_obj& obj = source.obj;
...
int RGWSI_SysObj::Obj::WNOp::notify(const DoutPrefixProvider* dpp, bufferlist& bl, uint64_t timeout_ms, bufferlist* pbl,
                                    optional_yield y)
{
    RGWSI_SysObj_Core* svc = source.core_svc;
    rgw_raw_obj& obj = source.obj;
