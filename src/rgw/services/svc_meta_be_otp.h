...
public:
    ...
    RGWSI_MetaBackend::Type get_type() { return MDBE_OTP; }

    static std::string get_meta_key(const rgw_user& user);

    void init(RGWSI_SysObj* _sysobj_svc, RGWSI_MDLog* _mdlog_svc, RGWSI_Cls* _cls_svc)
    {
    ...
    int get_entry(RGWSI_MetaBackend::Context* ctx, const std::string& key, RGWSI_MetaBackend::GetParams& _params,
    ...
    int put_entry(const DoutPrefixProvider* dpp, RGWSI_MetaBackend::Context* ctx, const std::string& key,
                  RGWSI_MetaBackend::PutParams& _params, RGWObjVersionTracker* objv_tracker, optional_yield y);
};
