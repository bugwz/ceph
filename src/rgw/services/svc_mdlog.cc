...
RGWSI_MDLog::RGWSI_MDLog(CephContext* cct, bool _run_sync)
...
int RGWSI_MDLog::do_start(optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    period_history.reset(new RGWPeriodHistory(cct, period_puller.get(), current_period));

    if (run_sync && svc.zone->need_to_sync()) {
...
int RGWSI_MDLog::read_history(RGWMetadataLogHistory* state, RGWObjVersionTracker* objv_tracker, optional_yield y,
                              const DoutPrefixProvider* dpp) const
{
    ...
    if (bl.length() == 0) {
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: meta history is empty, but cannot remove it (" << cpp_strerror(-ret) << ")"
                              << dendl;
            return ret;
        }
        return -ENOENT;
...
int RGWSI_MDLog::write_history(const DoutPrefixProvider* dpp, const RGWMetadataLogHistory& state,
                               RGWObjVersionTracker* objv_tracker, optional_yield y, bool exclusive)
{
    bufferlist bl;
    state.encode(bl);
    ...
    auto& pool = svc.zone->get_zone_params().log_pool;
    const auto& oid = RGWMetadataLogHistory::oid;
    return rgw_put_system_obj(dpp, svc.sysobj, pool, oid, bl, exclusive, objv_tracker, real_time{}, y);
}
...
namespace {
template<class T> class SysObjReadCR : public RGWSimpleCoroutine
{
...
public:
    SysObjReadCR(const DoutPrefixProvider* _dpp, RGWAsyncRadosProcessor* _async_rados, RGWSI_SysObj* _svc,
                 const rgw_raw_obj& _obj, T* _result, bool empty_on_enoent = true,
    ...
    ~SysObjReadCR() override
    {
        try {
            ...
            ldpp_dout(dpp, 0) << "ERROR: " << __func__ << ": reference counted object mismatched, \"" << e.what()
                              << "\"" << dendl;
        }
    }
    ...
    int send_request(const DoutPrefixProvider* dpp)
    {
        req = new RGWAsyncGetSystemObj(
            dpp, this, stack->create_completion_notifier(), svc, objv_tracker, obj, false, false);
...
template<class T> class SysObjWriteCR : public RGWSimpleCoroutine
{
    const DoutPrefixProvider* dpp;
    RGWAsyncRadosProcessor* async_rados;
    RGWSI_SysObj* svc;
...
public:
    SysObjWriteCR(const DoutPrefixProvider* _dpp, RGWAsyncRadosProcessor* _async_rados, RGWSI_SysObj* _svc,
                  const rgw_raw_obj& _obj, const T& _data, RGWObjVersionTracker* objv_tracker = nullptr,
    ...
    {
        req = new RGWAsyncPutSystemObj(
            dpp, this, stack->create_completion_notifier(), svc, objv_tracker, obj, exclusive, std::move(bl));
        async_rados->queue(req);
        return 0;
    }
...
public:
    ReadHistoryCR(const DoutPrefixProvider* dpp, const Svc& svc, Cursor* cursor, RGWObjVersionTracker* objv_tracker)
        : RGWCoroutine(svc.zone->ctx())
        , dpp(dpp)
        , svc(svc)
        , cursor(cursor)
        , objv_tracker(objv_tracker)
    ...
    int operate(const DoutPrefixProvider* dpp)
    {
        reenter(this)
        {
            yield
            {
                rgw_raw_obj obj{svc.zone->get_zone_params().log_pool, RGWMetadataLogHistory::oid};
                constexpr bool empty_on_enoent = false;

                using ReadCR = SysObjReadCR<RGWMetadataLogHistory>;
                call(new ReadCR(dpp, async_processor, svc.sysobj, obj, &state, empty_on_enoent, objv_tracker));
            }
            if (retcode < 0) {
                ldpp_dout(dpp, 1) << "failed to read mdlog history: " << cpp_strerror(retcode) << dendl;
            ...
            *cursor = svc.mdlog->period_history->lookup(state.oldest_realm_epoch);
            ...
            ldpp_dout(dpp, 10) << "read mdlog history with oldest period id=" << state.oldest_period_id
                               << " realm_epoch=" << state.oldest_realm_epoch << dendl;
            return set_cr_done();
        }
        return 0;
    }
...
public:
    WriteHistoryCR(const DoutPrefixProvider* dpp, Svc& svc, const Cursor& cursor, RGWObjVersionTracker* objv)
        : RGWCoroutine(svc.zone->ctx())
        , dpp(dpp)
        , svc(svc)
        , cursor(cursor)
    ...
    int operate(const DoutPrefixProvider* dpp)
    {
        reenter(this)
        {
            ...
            if (retcode < 0) {
                ldpp_dout(dpp, 1) << "failed to write mdlog history: " << cpp_strerror(retcode) << dendl;
                return set_cr_error(retcode);
            }

            ldpp_dout(dpp, 10) << "wrote mdlog history with oldest period id=" << state.oldest_period_id
                               << " realm_epoch=" << state.oldest_realm_epoch << dendl;
            return set_cr_done();
        }
        return 0;
...
Cursor RGWSI_MDLog::init_oldest_log_period(optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    if (period.get_realm_epoch() != state.oldest_realm_epoch) {
        ldpp_dout(dpp, 1) << "inconsistent mdlog history: read period id=" << period.get_id()
                          << " with realm_epoch=" << period.get_realm_epoch()
...
RGWCoroutine* RGWSI_MDLog::read_oldest_log_period_cr(const DoutPrefixProvider* dpp, Cursor* period,
                                                     RGWObjVersionTracker* objv) const
{
    return new mdlog::ReadHistoryCR(dpp, svc, period, objv);
}

RGWCoroutine* RGWSI_MDLog::trim_log_period_cr(const DoutPrefixProvider* dpp, Cursor period,
                                              RGWObjVersionTracker* objv) const
{
...
RGWMetadataLog* RGWSI_MDLog::get_log(const std::string& period)
{
    ...
    auto insert = md_logs.emplace(
        std::piecewise_construct, std::forward_as_tuple(period), std::forward_as_tuple(cct, svc.zone, svc.cls, period));
    return &insert.first->second;
}

int RGWSI_MDLog::add_entry(const DoutPrefixProvider* dpp, const string& hash_key, const string& section,
