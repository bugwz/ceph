...
{
    if (!maybe_insecure_mon_conn || warned_insecure.test_and_set()) {
        return;
    }

    string s = "rgw is configured to optionally allow insecure connections to the monitors (auth_supported, "
               "ms_mon_client_mode), ssl certificates stored at the monitor configuration could leak";
