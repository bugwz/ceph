...
int RGWSI_Cls::MFA::get_mfa_obj(const DoutPrefixProvider* dpp, const rgw_user& user,
...
int RGWSI_Cls::MFA::check_mfa(const DoutPrefixProvider* dpp, const rgw_user& user, const string& otp_id,
                              const string& pin, optional_yield y)
{
    rgw_rados_ref ref;
...
void RGWSI_Cls::MFA::prepare_mfa_write(librados::ObjectWriteOperation* op, RGWObjVersionTracker* objv_tracker,
...
int RGWSI_Cls::MFA::create_mfa(const DoutPrefixProvider* dpp, const rgw_user& user,
                               const rados::cls::otp::otp_info_t& config, RGWObjVersionTracker* objv_tracker,
...
int RGWSI_Cls::MFA::remove_mfa(const DoutPrefixProvider* dpp, const rgw_user& user, const string& id,
...
int RGWSI_Cls::MFA::get_mfa(const DoutPrefixProvider* dpp, const rgw_user& user, const string& id,
...
int RGWSI_Cls::MFA::list_mfa(const DoutPrefixProvider* dpp, const rgw_user& user,
                             list<rados::cls::otp::otp_info_t>* result, optional_yield y)
{
    rgw_rados_ref ref;
...
int RGWSI_Cls::MFA::set_mfa(const DoutPrefixProvider* dpp, const string& oid,
                            const list<rados::cls::otp::otp_info_t>& entries, bool reset_obj,
                            RGWObjVersionTracker* objv_tracker, const real_time& mtime, optional_yield y)
{
    rgw_raw_obj o(zone_svc->get_zone_params().otp_pool, oid);
...
int RGWSI_Cls::MFA::list_mfa(const DoutPrefixProvider* dpp, const string& oid,
                             list<rados::cls::otp::otp_info_t>* result, RGWObjVersionTracker* objv_tracker,
...
void RGWSI_Cls::TimeLog::prepare_entry(cls_log_entry& entry, const real_time& ut, const string& section,
...
int RGWSI_Cls::TimeLog::add(const DoutPrefixProvider* dpp, const string& oid, const real_time& ut,
                            const string& section, const string& key, bufferlist& bl, optional_yield y)
{
    RGWSI_RADOS::Obj obj;
...
int RGWSI_Cls::TimeLog::add(const DoutPrefixProvider* dpp, const string& oid, std::list<cls_log_entry>& entries,
                            librados::AioCompletion* completion, bool monotonic_inc, optional_yield y)
{
    RGWSI_RADOS::Obj obj;
...
int RGWSI_Cls::TimeLog::list(const DoutPrefixProvider* dpp, const string& oid, const real_time& start_time,
                             const real_time& end_time, int max_entries, std::list<cls_log_entry>& entries,
                             const string& marker, string* out_marker, bool* truncated, optional_yield y)
{
    RGWSI_RADOS::Obj obj;
    ...
    cls_log_list(op, st, et, marker, max_entries, entries, out_marker, truncated);
...
int RGWSI_Cls::TimeLog::info(const DoutPrefixProvider* dpp, const string& oid, cls_log_header* header, optional_yield y)
{
    RGWSI_RADOS::Obj obj;
...
int RGWSI_Cls::TimeLog::info_async(const DoutPrefixProvider* dpp, RGWSI_RADOS::Obj& obj, const string& oid,
...
int RGWSI_Cls::TimeLog::trim(const DoutPrefixProvider* dpp, const string& oid, const real_time& start_time,
                             const real_time& end_time, const string& from_marker, const string& to_marker,
...
int RGWSI_Cls::Lock::lock_exclusive(const DoutPrefixProvider* dpp, const rgw_pool& pool, const string& oid,
                                    timespan& duration, string& zone_id, string& owner_id,
...
int RGWSI_Cls::Lock::unlock(const DoutPrefixProvider* dpp, const rgw_pool& pool, const string& oid, string& zone_id,
                            string& owner_id, std::optional<string> lock_name)
{
    auto p = rados_svc->pool(pool);
    int r = p.open(dpp);
    if (r < 0) {
