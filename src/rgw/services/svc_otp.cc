...
public:
    RGW_MB_Handler_Module_OTP(RGWSI_Zone* _zone_svc)
    ...
    bool is_valid_oid(const string& oid) override { return true; }
...
RGWSI_OTP::RGWSI_OTP(CephContext* cct)
    : RGWServiceInstance(cct)
{}

RGWSI_OTP::~RGWSI_OTP() {}

void RGWSI_OTP::init(RGWSI_Zone* _zone_svc, RGWSI_Meta* _meta_svc, RGWSI_MetaBackend* _meta_be_svc)
{
    svc.otp = this;
    svc.zone = _zone_svc;
    svc.meta = _meta_svc;
...
int RGWSI_OTP::read_all(RGWSI_OTP_BE_Ctx& ctx, const string& key, otp_devices_list_t* devices, real_time* pmtime,
...
int RGWSI_OTP::read_all(RGWSI_OTP_BE_Ctx& ctx, const rgw_user& uid, otp_devices_list_t* devices, real_time* pmtime,
                        RGWObjVersionTracker* objv_tracker, optional_yield y, const DoutPrefixProvider* dpp)
{
    return read_all(ctx, uid.to_str(), devices, pmtime, objv_tracker, y, dpp);
}

int RGWSI_OTP::store_all(const DoutPrefixProvider* dpp, RGWSI_OTP_BE_Ctx& ctx, const string& key,
                         const otp_devices_list_t& devices, real_time mtime, RGWObjVersionTracker* objv_tracker,
                         optional_yield y)
{
...
int RGWSI_OTP::store_all(const DoutPrefixProvider* dpp, RGWSI_OTP_BE_Ctx& ctx, const rgw_user& uid,
                         const otp_devices_list_t& devices, real_time mtime, RGWObjVersionTracker* objv_tracker,
                         optional_yield y)
{
...
int RGWSI_OTP::remove_all(const DoutPrefixProvider* dpp, RGWSI_OTP_BE_Ctx& ctx, const string& key,
...
int RGWSI_OTP::remove_all(const DoutPrefixProvider* dpp, RGWSI_OTP_BE_Ctx& ctx, const rgw_user& uid,
                          RGWObjVersionTracker* objv_tracker, optional_yield y)
{
