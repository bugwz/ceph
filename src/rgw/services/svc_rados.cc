...
RGWSI_RADOS::RGWSI_RADOS(CephContext* cct)
...
{
    return rgw_init_ioctx(dpp, get_rados_handle(), pool, io_ctx, params.create, params.mostly_omap);
}

int RGWSI_RADOS::pool_iterate(const DoutPrefixProvider* dpp, librados::IoCtx& io_ctx, librados::NObjectIterator& iter,
                              uint32_t num, vector<rgw_bucket_dir_entry>& objs, RGWAccessListFilter* filter,
                              bool* is_truncated)
{
...
RGWSI_RADOS::Obj::Obj(Pool& pool, const string& oid)
    : rados_svc(pool.rados_svc)
{
    ref.pool = pool;
...
int RGWSI_RADOS::Obj::operate(const DoutPrefixProvider* dpp, librados::ObjectWriteOperation* op, optional_yield y,
                              int flags)
{
    return rgw_rados_operate(dpp, ref.pool.ioctx(), ref.obj.oid, op, y, flags);
}

int RGWSI_RADOS::Obj::operate(const DoutPrefixProvider* dpp, librados::ObjectReadOperation* op, bufferlist* pbl,
...
int RGWSI_RADOS::Obj::aio_operate(librados::AioCompletion* c, librados::ObjectReadOperation* op, bufferlist* pbl)
{
...
int RGWSI_RADOS::Obj::notify(const DoutPrefixProvider* dpp, bufferlist& bl, uint64_t timeout_ms, bufferlist* pbl,
                             optional_yield y)
{
    return rgw_rados_notify(dpp, ref.pool.ioctx(), ref.obj.oid, bl, timeout_ms, pbl, y);
}

void RGWSI_RADOS::Obj::notify_ack(uint64_t notify_id, uint64_t cookie, bufferlist& bl)
{
...
int RGWSI_RADOS::Pool::create(const DoutPrefixProvider* dpp, const vector<rgw_pool>& pools, vector<int>* retcodes)
{
    ...
    for (auto& io_ctx : io_ctxs) {
        librados::PoolAsyncCompletion* c = librados::Rados::pool_async_create_completion();
        completions.push_back(c);
        int ret = io_ctx.application_enable_async(pg_pool_t::APPLICATION_NAME_RGW, false, c);
        ceph_assert(ret == 0);
    }
    ...
    for (auto c : completions) {
        ...
        if (ret == -EOPNOTSUPP) {
            ...
            ldpp_dout(dpp, 0) << "WARNING: async application_enable returned " << ret << dendl;
            error = true;
        }
        c->release();
        retcodes->push_back(ret);
...
int RGWSI_RADOS::Pool::List::get_next(const DoutPrefixProvider* dpp, int max, std::vector<string>* oids,
                                      bool* is_truncated)
{
    if (!ctx.initialized) {
        return -EINVAL;
...
int RGWSI_RADOS::Handle::mon_command(std::string cmd, const bufferlist& inbl, bufferlist* outbl, std::string* outs)
{
    librados::Rados* rad = rados_svc->get_rados_handle();
