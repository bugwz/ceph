...
int RGWSI_RADOS::pool_iterate(librados::IoCtx& io_ctx, librados::NObjectIterator& iter, uint32_t num,
                              vector<rgw_bucket_dir_entry>& objs, RGWAccessListFilter* filter, bool* is_truncated)
{
...
int RGWSI_RADOS::Pool::List::get_next(int max, std::vector<string>* oids, bool* is_truncated)
{
    if (!ctx.initialized) {
        return -EINVAL;
    }
