...
void TempURLApplier::modify_request_state(const DoutPrefixProvider* dpp, req_state* s) const /* in/out */
{
    ...
    ldpp_dout(dpp, 20) << "finished applying changes to req_state for TempURL: "
                       << " content_disp override " << s->content_disp.override << " content_disp fallback "
                       << s->content_disp.fallback << dendl;
}
...
{
    return s->info.args.exists("temp_url_sig") || s->info.args.exists("temp_url_expires");
}

void TempURLEngine::get_owner_info(const DoutPrefixProvider* dpp, const req_state* const s, RGWUserInfo& owner_info,
                                   optional_yield y) const
{
    ...
    int ret = ctl->bucket->read_bucket_info(
        b, &bucket_info, y, dpp, RGWBucketCtl::BucketInstance::GetParams().set_bectx_params(bectx_params));
    if (ret < 0) {
        throw ret;
    }

    ldpp_dout(dpp, 20) << "temp url user (bucket owner): " << bucket_info.owner << dendl;
...
bool TempURLEngine::is_disallowed_header_present(const req_info& info) const
{
    ...
    return std::any_of(
        std::begin(headers), std::end(headers), [&info](const char* header) { return info.env->exists(header); });
}
...
{
private:
    static constexpr uint32_t output_size = CEPH_CRYPTO_HMACSHA1_DIGESTSIZE * 2 + 1;
...
public:
    ...
    const char* calc(const std::string& key, const std::string_view& method, const std::string_view& path,
...
class TempURLEngine::PrefixableSignatureHelper : private TempURLEngine::SignatureHelper
{
...
public:
    PrefixableSignatureHelper(const std::string& _decoded_uri, const std::string& object_name,
                              const boost::optional<const std::string&> prefix)
        ...
        , prefix(prefix)
    {
         ...
         * a temporary. */
        no_obj_uri = decoded_uri.substr(0, decoded_uri.length() - object_name.length());
    }
                     ...
                     const std::string& expires)
    {
        if (!prefix) {
            ...
            const auto prefixed_path = string_cat_reserve("prefix:", no_obj_uri, *prefix);
...
TempURLEngine::result_t TempURLEngine::authenticate(const DoutPrefixProvider* dpp, const req_state* const s,
                                                    optional_yield y) const
{
    if (!is_applicable(s)) {
    ...
    const std::string& temp_url_expires = convert_from_iso8601(s->info.args.get("temp_url_expires"));
     ...
     * container. */
    const boost::optional<const std::string&> temp_url_prefix = s->info.args.get_optional("temp_url_prefix");
    ...
    const std::array<std::string_view, 2> allowed_paths = {ref_uri, ref_uri.substr(pos + 1)};
    ...
    PrefixableSignatureHelper sig_helper{s->decoded_uri, s->object->get_name(), temp_url_prefix};
    ...
    for (const auto& kv : owner_info.temp_url_keys) {
        ...
        for (const auto& path : allowed_paths) {
            for (const auto& method : allowed_methods) {
                const char* const local_sig = sig_helper.calc(temp_url_key, method, path, temp_url_expires);

                ldpp_dout(dpp, 20) << "temp url signature [" << temp_url_key_num << "] (calculated): " << local_sig
                                   << dendl;
                ...
                if (sig_helper.is_equal_to(temp_url_sig)) {
                    ...
                    ldpp_dout(dpp, 5) << "temp url signature mismatch: " << local_sig << " != " << temp_url_sig
                                      << dendl;
                }
            }
        }
...
ExternalTokenEngine::result_t ExternalTokenEngine::authenticate(const DoutPrefixProvider* dpp, const std::string& token,
                                                                const req_state* const s, optional_yield y) const
{
    ...
    try {
        ...
        get_str_vec(validator.get_header_value("X-Auth-Groups"), ",", swift_groups);
    ...
    auto apl = apl_factory->create_apl_local(
        cct, s, tmp_uinfo, extract_swift_subuser(swift_user), boost::none, rgw::auth::LocalApplier::NO_ACCESS_KEY);
    return result_t::grant(std::move(apl));
}

static int build_token(const string& swift_user, const string& key, const uint64_t nonce, const utime_t& expiration,
                       bufferlist& bl)
{
    using ceph::encode;
...
SignedTokenEngine::result_t SignedTokenEngine::authenticate(const DoutPrefixProvider* dpp, const std::string& token,
                                                            const req_state* const s) const
{
    ...
    const utime_t now = ceph_clock_now();
    if (expiration < now) {
        ldpp_dout(dpp, 0) << "NOTICE: old timed out token was used now=" << now << " token.expiration=" << expiration
                          << dendl;
        return result_t::deny(-EPERM);
    }
    ...
    if (local_tok_bl.length() != tok_bl.length()) {
        ldpp_dout(dpp, 0) << "NOTICE: tokens length mismatch:"
                          ...
                          << " local_tok_bl.length()=" << local_tok_bl.length() << dendl;
        return result_t::deny(-EPERM);
    }

    if (memcmp(local_tok_bl.c_str(), tok_bl.c_str(), local_tok_bl.length()) != 0) {
        char buf[local_tok_bl.length() * 2 + 1];

        buf_to_hex(reinterpret_cast<const unsigned char*>(local_tok_bl.c_str()), local_tok_bl.length(), buf);
    ...
    auto apl = apl_factory->create_apl_local(
        cct, s, user_info, extract_swift_subuser(swift_user), boost::none, rgw::auth::LocalApplier::NO_ACCESS_KEY);
    return result_t::grant(std::move(apl));
}
...
void RGW_SWIFT_Auth_Get::execute(optional_yield y)
{
    ...
    if (swift_url.size() == 0) {
        ...
        if (!host) {
            dout(0) << "NOTICE: server is misconfigured, missing rgw_swift_url_prefix or rgw_swift_url, HTTP_HOST is "
                       "not set"
                    << dendl;
            ret = -EINVAL;
            goto done;
    ...
    dump_header(s, "X-Storage-Url", swift_url + swift_prefix + "/v1" + tenant_path);

    using rgw::auth::swift::encode_token;
    if ((ret = encode_token(s->cct, swift_key->id, swift_key->key, bl)) < 0) goto done;

    {
        ...
        buf_to_hex((const unsigned char*)bl.c_str(), bl.length(), token_val + PREFIX_LEN);
...
int RGWHandler_SWIFT_Auth::init(rgw::sal::RGWRadosStore* store, struct req_state* state, rgw::io::BasicClient* cio)
{
