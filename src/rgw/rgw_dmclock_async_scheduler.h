...
public:
    ...
    AsyncScheduler(CephContext* cct, boost::asio::io_context& context, GetClientCounters&& counters,
    ...
    executor_type get_executor() noexcept { return timer.get_executor(); }
    ...
    auto async_request(const client_id& client, const ReqParams& params, const Time& time, Cost cost,
...
private:
    int schedule_request_impl(const client_id& client, const ReqParams& params, const Time& time, const Cost& cost,
    ...
    using Timer = boost::asio::basic_waitable_timer<Clock, boost::asio::wait_traits<Clock>, executor_type>;
...
AsyncScheduler::AsyncScheduler(CephContext* cct, boost::asio::io_context& context, GetClientCounters&& counters,
                               md_config_obs_t* observer, Args&&... args)
...
template<typename CompletionToken>
auto AsyncScheduler::async_request(const client_id& client, const ReqParams& params, const Time& time, Cost cost,
                                   CompletionToken&& token)
{
    ...
    auto completion = Completion::create(ex1, std::move(handler), Request{client, time, cost});
    ...
    if (r == 0) {
        ...
        async::post(std::unique_ptr<Completion>{completion}, ec, PhaseType::priority);
        if (auto c = counters(client)) {
...
private:
    int schedule_request_impl(const client_id&, const ReqParams&, const Time&, const Cost&, optional_yield) override
    {
