...
static int conf_to_uint64(CephContext* cct, const JSONFormattable& config, const string& key, uint64_t* pval)
{
    ...
    if (config.find(key, &sval)) {
        ...
        if (!err.empty()) {
            ldout(cct, 0) << "ERROR: could not parse configurable value for cloud sync module: " << key << ": " << sval
                          << dendl;
            return -EINVAL;
        }
        *pval = val;
...
struct AWSSyncConfig
{
                     ...
                     bool connection_must_exist)
    {
        if (!profile.connection_id.empty()) {
            if (profile.conn_conf) {
                ldout(cct, 0) << "ERROR: ambiguous profile connection configuration, connection_id="
                              << profile.connection_id << dendl;
                return -EINVAL;
            }
            if (connections.find(profile.connection_id) == connections.end()) {
                ldout(cct, 0) << "ERROR: profile configuration reference non-existent connection_id="
                              << profile.connection_id << dendl;
                return -EINVAL;
            }
        ...
        if (!profile.acls_id.empty()) {
            if (!acl_profiles.find(profile.acls_id, &acl_mappings)) {
                ldout(cct, 0) << "ERROR: profile configuration reference non-existent acls id=" << profile.acls_id
                              << dendl;
                return -EINVAL;
            }
    ...
    int init_target(CephContext* cct, const JSONFormattable& profile_conf,
    ...
    void init_conns(RGWDataSyncCtx* sc, const string& id)
    {
        ...
        root_profile->conn.reset(new S3RESTConn(
            sc->cct, sync_env->svc->zone, id, {root_conf->endpoint}, root_conf->key, root_conf->host_style));
...
static int do_decode_rest_obj(CephContext* cct, map<string, bufferlist>& attrs, map<string, string>& headers,
                              rgw_rest_obj* info)
{
...
public:
    RGWRESTStreamGetCRF(CephContext* _cct, RGWCoroutinesEnv* _env, RGWCoroutine* _caller, RGWDataSyncCtx* _sc,
                        RGWRESTConn* _conn, rgw::sal::RGWObject* _src_obj,
                        const rgw_sync_aws_src_obj_properties& _src_properties)
        : RGWStreamReadHTTPResourceCRF(_cct, _env, _caller, _sc->env->http_manager, _src_obj->get_key())
        , sc(_sc)
        , conn(_conn)
        , src_obj(_src_obj)
...
public:
    RGWAWSStreamPutCRF(CephContext* _cct, RGWCoroutinesEnv* _env, RGWCoroutine* _caller, RGWDataSyncCtx* _sc,
                       const rgw_sync_aws_src_obj_properties& _src_properties,
                       std::shared_ptr<AWSSyncConfig_Profile>& _target, rgw::sal::RGWObject* _dest_obj)
...
public:
    RGWAWSStreamObjToCloudPlainCR(RGWDataSyncCtx* _sc, RGWRESTConn* _source_conn, rgw::sal::RGWObject* _src_obj,
                                  const rgw_sync_aws_src_obj_properties& _src_properties,
                                  std::shared_ptr<AWSSyncConfig_Profile> _target, rgw::sal::RGWObject* _dest_obj)
...
public:
    RGWAWSStreamObjToCloudMultipartPartCR(RGWDataSyncCtx* _sc, RGWRESTConn* _source_conn, rgw::sal::RGWObject* _src_obj,
...
public:
    RGWAWSAbortMultipartCR(RGWDataSyncCtx* _sc, RGWRESTConn* _dest_conn, rgw::sal::RGWObject* _dest_obj,
...
public:
    RGWAWSInitMultipartCR(RGWDataSyncCtx* _sc, RGWRESTConn* _dest_conn, rgw::sal::RGWObject* _dest_obj,
                          uint64_t _obj_size, const map<string, string>& _attrs, string* _upload_id)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , dest_conn(_dest_conn)
        , dest_obj(_dest_obj)
...
public:
    RGWAWSCompleteMultipartCR(RGWDataSyncCtx* _sc, RGWRESTConn* _dest_conn, rgw::sal::RGWObject* _dest_obj,
                              string _upload_id, const map<int, rgw_sync_aws_multipart_part_info>& _parts)
        : RGWCoroutine(_sc->cct)
        , sc(_sc)
        , dest_conn(_dest_conn)
        , dest_obj(_dest_obj)
...
public:
    RGWAWSStreamAbortMultipartUploadCR(RGWDataSyncCtx* _sc, RGWRESTConn* _dest_conn, rgw::sal::RGWObject* _dest_obj,
...
public:
    RGWAWSStreamObjToCloudMultipartCR(RGWDataSyncCtx* _sc, rgw_bucket_sync_pipe& _sync_pipe, AWSSyncConfig& _conf,
                                      RGWRESTConn* _source_conn, rgw::sal::RGWObject* _src_obj,
                                      std::shared_ptr<AWSSyncConfig_Profile>& _target, rgw::sal::RGWObject* _dest_obj,
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            yield call(new RGWSimpleRadosReadCR<rgw_sync_aws_multipart_upload_info>(
                dpp, sync_env->async_rados, sync_env->svc->sysobj, status_obj, &status, false));
...
#define MULTIPART_MAX_PARTS 10000
            ...
            for (; (uint32_t)status.cur_part <= status.num_parts; ++status.cur_part) {
                ...
                yield call(new RGWSimpleRadosWriteCR<rgw_sync_aws_multipart_upload_info>(
                    dpp, sync_env->async_rados, sync_env->svc->sysobj, status_obj, status));
                if (retcode < 0) {
                    ldpp_dout(dpp, 0) << "ERROR: failed to store multipart upload state, retcode=" << retcode << dendl;
                    /* continue with upload anyway */
                }
                ldout(sc->cct, 20) << "sync of object=" << src_obj << " via multipart upload, finished sending part #"
        ...
        reenter(this)
        {
            ...
            if (bucket_created.find(target_bucket_name) == bucket_created.end()) {
                yield
                {
                    ...
                    call(new RGWPutRawRESTResourceCR<bufferlist>(
                        sc->cct, target->conn.get(), sync_env->http_manager, target_bucket_name, nullptr, bl, &out_bl));
                }
                if (retcode < 0) {
                    RGWXMLDecoder::XMLParser parser;
                    if (!parser.init()) {
            ...
            yield
            {
                ...
                if (size < instance.conf.s3.multipart_sync_threshold) {
                    call(new RGWAWSStreamObjToCloudPlainCR(
                        sc, source_conn, &src_obj, src_properties, target, &dest_obj));
                }
                else {
                    rgw_rest_obj rest_obj;
                    rest_obj.init(key);
                    if (do_decode_rest_obj(sc->cct, attrs, headers, &rest_obj)) {
                        ldout(sc->cct, 0) << "ERROR: failed to decode rest obj out of headers=" << headers
                                          << ", attrs=" << attrs << dendl;
                        return set_cr_error(-EINVAL);
                    }
...
int RGWAWSSyncModule::create_instance(CephContext* cct, const JSONFormattable& config,
