...
const char* LC_STATUS[] = {"UNINITIAL", "PROCESSING", "FAILED", "COMPLETE"};
...
void RGWLC::initialize(CephContext* _cct, rgw::sal::Driver* _driver)
{
    ...
    if (max_objs > HASH_PRIME) max_objs = HASH_PRIME;
                            ...
                            ceph::real_time* expire_time = nullptr)
{
    ...
    ldpp_dout(dpp, 20) << __func__ << "(): mtime=" << mtime << " days=" << days << " base_time=" << base_time
                       << " timediff=" << timediff << " cmp=" << cmp << " is_expired=" << (timediff >= cmp) << dendl;
...
public:
    ...
    int init(const DoutPrefixProvider* dpp) { return fetch(dpp); }
    ...
    bool get_obj(
        const DoutPrefixProvider* dpp, rgw_bucket_dir_entry** obj,
    ...
    rgw_bucket_dir_entry get_prev_obj() { return pre_obj; }
    ...
    {
        if (obj_iter == list_results.objs.end() || (obj_iter + 1) == list_results.objs.end()) {
...
struct op_env
{
    ...
    op_env(lc_op& _op, rgw::sal::Driver* _driver, LCWorker* _worker, rgw::sal::Bucket* _bucket, LCObjsLister& _ol)
        : op(_op)
        , driver(_driver)
        , worker(_worker)
        , bucket(_bucket)
...
struct lc_op_ctx
{
    ...
    lc_op_ctx(op_env& env, rgw_bucket_dir_entry& o, boost::optional<std::string> next_key_name,
              ceph::real_time effective_mtime, const DoutPrefixProvider* dpp, WorkQ* wq)
        : cct(env.driver->ctx())
        , env(env)
        , o(o)
    ...
    {
        return (next_key_name && key_name.compare(boost::get<std::string>(next_key_name)) == 0);
    }
                              ...
                              rgw::notify::EventType event_type)
{
    ...
    notify = driver->get_notification(dpp,
                                      obj.get(),
                                      nullptr,
                                      event_type,
                                      bucket.get(),
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 1) << "ERROR: notify reservation failed, deferring delete of object k=" << o.key << dendl;
        return ret;
    }
    ...
    if (ret < 0) {
        ...
        (void)notify->publish_commit(
            dpp, obj_state->size, ceph::real_clock::now(), obj_state->attrset[RGW_ATTR_ETAG].to_str(), version_id);
    }
...
public:
    ...
    virtual bool check(lc_op_ctx& oc, ceph::real_time* exp_time, const DoutPrefixProvider* dpp) { return false; }
     ...
     */
    virtual bool should_process() { return true; }
...
public:
    virtual ~LCOpFilter() {}
    virtual bool check(const DoutPrefixProvider* dpp, lc_op_ctx& oc) { return false; }
}; /* LCOpFilter */
...
public:
    LCOpRule(op_env& _env)
        : env(_env)
    {}

    boost::optional<std::string> get_next_key_name() { return next_key_name; }

    std::vector<shared_ptr<LCOpAction>>& get_actions() { return actions; }

    void build();
    void update();
    int process(rgw_bucket_dir_entry& o, const DoutPrefixProvider* dpp, WorkQ* wq);
}; /* LCOpRule */
...
using WorkItem = boost::variant<void*,
                                ...
                                std::tuple<lc_op, rgw_bucket_dir_entry>, rgw_bucket_dir_entry>;
...
public:
    ...
    std::string thr_name() { return std::string{"wp_thrd: "} + std::to_string(wk->ix) + ", " + std::to_string(ix); }

    void setf(work_f _f) { f = _f; }

    void enqueue(WorkItem&& item)
    {
        unique_lock uniq(mtx);
        while ((!wk->get_lc()->going_down()) && (items.size() > qmax)) {
...
private:
    ...
    {
        unique_lock uniq(mtx);
        while ((!wk->get_lc()->going_down()) && (items.size() == 0)) {
...
public:
    WorkPool(RGWLC::LCWorker* wk, uint16_t n_threads, uint32_t qmax)
        : wqs(TVector{n_threads, [&](const size_t ix, auto emplacer) { emplacer.emplace(wk, ix, qmax); }})
        , ix(0)
    {}
...
RGWLC::LCWorker::LCWorker(const DoutPrefixProvider* dpp, CephContext* cct, RGWLC* lc, int ix)
    : dpp(dpp)
    , cct(cct)
    , lc(lc)
    , ix(ix)
...
int RGWLC::handle_multipart_expiration(rgw::sal::Bucket* target, const multimap<string, lc_op>& prefix_map,
                                       LCWorker* worker, time_t stop_at, bool once)
{
        ...
        if (obj_has_expired(this, cct, obj.meta.mtime, rule.mp_expiration)) {
            ...
            if (ret == 0) {
                ...
                if (ret == -ERR_NO_SUCH_UPLOAD) {
                    ldpp_dout(wk->get_lc(), 5) << "ERROR: abort_multipart_upload failed, ret=" << ret
                                               << ", thread:" << wq->thr_name() << ", meta:" << obj.key << dendl;
                }
                else {
                    ldpp_dout(wk->get_lc(), 0) << "ERROR: abort_multipart_upload failed, ret=" << ret
                                               << ", thread:" << wq->thr_name() << ", meta:" << obj.key << dendl;
                }
            } /* abort failed */
        }     /* expired */
    ...
    for (auto prefix_iter = prefix_map.begin(); prefix_iter != prefix_map.end(); ++prefix_iter) {
        ...
        if (worker_should_stop(stop_at, once)) {
            ldpp_dout(this, 5) << __func__ << " interval budget EXPIRED worker " << worker->ix << dendl;
            return 0;
        }
...
{
    return (op.status && (op.expiration > 0 || op.expiration_date != boost::none || op.noncur_expiration > 0 ||
                          op.dm_expiration || !op.transitions.empty() || !op.noncur_transitions.empty()));
}
...
static inline bool has_all_tags(const lc_op& rule_action, const RGWObjTags& object_tags)
{
    if (!rule_action.obj_tags) return false;
    if (object_tags.count() < rule_action.obj_tags->count()) return false;
    size_t tag_count = 0;
    for (const auto& tag : object_tags.get_tags()) {
        ...
        if (iter == rule_tags.end()) continue;
...
static int check_tags(const DoutPrefixProvider* dpp, lc_op_ctx& oc, bool* skip)
{
    ...
    if (op.obj_tags != boost::none) {
        *skip = true;
        ...
        if (ret < 0) {
            if (ret != -ENODATA) {
                ldpp_dout(oc.dpp, 5) << "ERROR: read_obj_tags returned r=" << ret << " " << oc.wq->thr_name() << dendl;
            }
            return 0;
        }
        ...
        try {
            ...
            ldpp_dout(oc.dpp, 0) << "ERROR: caught buffer::error, couldn't decode TagSet " << oc.wq->thr_name()
                                 << dendl;
            return -EIO;
        }
        ...
        if (!has_all_tags(op, dest_obj_tags)) {
            ldpp_dout(oc.dpp, 20) << __func__ << "() skipping obj " << oc.obj
                                  << " as tags do not match in rule: " << op.id << " " << oc.wq->thr_name() << dendl;
            return 0;
        }
    }
    *skip = false;
...
public:
    bool check(const DoutPrefixProvider* dpp, lc_op_ctx& oc) override
    {
        ...
        if (ret < 0) {
            ...
            ldpp_dout(oc.dpp, 0) << "ERROR: check_tags on obj=" << oc.obj << " returned ret=" << ret << " "
...
public:
    ...
    bool check(lc_op_ctx& oc, ceph::real_time* exp_time, const DoutPrefixProvider* dpp) override
    {
        auto& o = oc.o;
        if (!o.is_current()) {
            ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << ": not current, skipping " << oc.wq->thr_name()
                               << dendl;
            return false;
        }
        if (o.is_delete_marker()) {
            if (oc.next_key_name) {
                ...
                if (oc.next_has_same_name(o.key.name)) {
                    ldpp_dout(dpp, 7) << __func__ << "(): dm-check SAME: key=" << o.key << " next_key_name: %%" << nkn
                                      << "%% " << oc.wq->thr_name() << dendl;
                    return false;
                }
                else {
                    ldpp_dout(dpp, 7) << __func__ << "(): dm-check DELE: key=" << o.key << " next_key_name: %%" << nkn
                                      << "%% " << oc.wq->thr_name() << dendl;
        ...
        if (op.expiration <= 0) {
            if (op.expiration_date == boost::none) {
                ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << ": no expiration set in rule, skipping "
                                   << oc.wq->thr_name() << dendl;
                return false;
            }
        ...
        ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << ": is_expired=" << (int)is_expired << " "
                           << oc.wq->thr_name() << dendl;
        return is_expired;
    }
    ...
    int process(lc_op_ctx& oc)
    {
        ...
        int r;
        if (o.is_delete_marker()) {
            r = remove_expired_obj(oc.dpp, oc, true, rgw::notify::ObjectExpirationDeleteMarker);
            if (r < 0) {
                ldpp_dout(oc.dpp, 0) << "ERROR: current is-dm remove_expired_obj " << oc.bucket << ":" << o.key << " "
                                     << cpp_strerror(r) << " " << oc.wq->thr_name() << dendl;
                return r;
            }
            ldpp_dout(oc.dpp, 2) << "DELETED: current is-dm " << oc.bucket << ":" << o.key << " " << oc.wq->thr_name()
                                 << dendl;
        }
        else {
            /* ! o.is_delete_marker() */
            r = remove_expired_obj(oc.dpp, oc, !oc.bucket->versioned(), rgw::notify::ObjectExpirationCurrent);
            if (r < 0) {
                ldpp_dout(oc.dpp, 0) << "ERROR: remove_expired_obj " << oc.bucket << ":" << o.key << " "
                                     << cpp_strerror(r) << " " << oc.wq->thr_name() << dendl;
                return r;
            }
            if (perfcounter) {
            ...
            ldpp_dout(oc.dpp, 2) << "DELETED:" << oc.bucket << ":" << o.key << " " << oc.wq->thr_name() << dendl;
        }
        return 0;
    }
};
...
public:
    ...
    bool check(lc_op_ctx& oc, ceph::real_time* exp_time, const DoutPrefixProvider* dpp) override
    {
        auto& o = oc.o;
        if (o.is_current()) {
            ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << ": current version, skipping " << oc.wq->thr_name()
                               << dendl;
            return false;
        }

        int expiration = oc.op.noncur_expiration;
        bool is_expired = obj_has_expired(dpp, oc.cct, oc.effective_mtime, expiration, exp_time);

        ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << ": is_expired=" << is_expired << " "
                           << oc.wq->thr_name() << dendl;

        return is_expired && pass_object_lock_check(oc.driver, oc.obj.get(), dpp);
    }

    int process(lc_op_ctx& oc)
    {
        auto& o = oc.o;
        int r = remove_expired_obj(oc.dpp, oc, true, rgw::notify::ObjectExpirationNoncurrent);
        if (r < 0) {
            ldpp_dout(oc.dpp, 0) << "ERROR: remove_expired_obj (non-current expiration) " << oc.bucket << ":" << o.key
                                 << " " << cpp_strerror(r) << " " << oc.wq->thr_name() << dendl;
            return r;
        }
        if (perfcounter) {
        ...
        ldpp_dout(oc.dpp, 2) << "DELETED:" << oc.bucket << ":" << o.key << " (non-current expiration) "
                             << oc.wq->thr_name() << dendl;
        return 0;
    }
};
...
public:
    ...
    bool check(lc_op_ctx& oc, ceph::real_time* exp_time, const DoutPrefixProvider* dpp) override
    {
        auto& o = oc.o;
        if (!o.is_delete_marker()) {
            ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << ": not a delete marker, skipping "
                               << oc.wq->thr_name() << dendl;
            return false;
        }
        if (oc.next_has_same_name(o.key.name)) {
            ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << ": next is same object, skipping "
                               << oc.wq->thr_name() << dendl;
            return false;
        }
    ...
    {
        auto& o = oc.o;
        int r = remove_expired_obj(oc.dpp, oc, true, rgw::notify::ObjectExpirationDeleteMarker);
        if (r < 0) {
            ldpp_dout(oc.dpp, 0) << "ERROR: remove_expired_obj (delete marker expiration) " << oc.bucket << ":" << o.key
                                 << " " << cpp_strerror(r) << " " << oc.wq->thr_name() << dendl;
            return r;
        }
        if (perfcounter) {
        ...
        ldpp_dout(oc.dpp, 2) << "DELETED:" << oc.bucket << ":" << o.key << " (delete marker expiration) "
                             << oc.wq->thr_name() << dendl;
        return 0;
    }
};
...
public:
    ...
    bool check(lc_op_ctx& oc, ceph::real_time* exp_time, const DoutPrefixProvider* dpp) override
    {
        ...
        if (transition.days < 0) {
            if (transition.date == boost::none) {
                ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key
                                   << ": no transition day/date set in rule, skipping " << oc.wq->thr_name() << dendl;
                return false;
            }
        ...
        ldpp_dout(oc.dpp, 20) << __func__ << "(): key=" << o.key << ": is_expired=" << is_expired << " "
                              << oc.wq->thr_name() << dendl;

        need_to_process =
            (rgw_placement_rule::get_canonical_storage_class(o.meta.storage_class) != transition.storage_class);

        return is_expired;
    }

    bool should_process() override { return need_to_process; }
    ...
    int delete_tier_obj(lc_op_ctx& oc)
    {
        ...
        if (oc.bucket->versioned() && oc.o.is_current() && !oc.o.is_delete_marker()) {
            ...
            ldpp_dout(oc.dpp, 20) << "delete_tier_obj Object(key:" << oc.o.key << ") current & not delete_marker"
                                  << " versioned_epoch:  " << oc.o.versioned_epoch << "flags: " << oc.o.flags << dendl;
        }
        else {
            ret = remove_expired_obj(oc.dpp, oc, true, rgw::notify::ObjectExpiration);
            ldpp_dout(oc.dpp, 20) << "delete_tier_obj Object(key:" << oc.o.key << ") not current "
                                  << "versioned_epoch:  " << oc.o.versioned_epoch << "flags: " << oc.o.flags << dendl;
        }
        return ret;
    }
    ...
    int transition_obj_to_cloud(lc_op_ctx& oc)
    {
        ...
        bool delete_object = (!oc.tier->retain_head_object() || (oc.o.is_current() && oc.bucket->versioned()));

        int ret = oc.obj->transition_to_cloud(oc.bucket,
                                              oc.tier.get(),
                                              oc.o,
    ...
    int process(lc_op_ctx& oc)
    {
        ...
        if (oc.o.meta.category == RGWObjCategory::CloudTiered) {
            ...
            ldpp_dout(oc.dpp, 30) << "Object(key:" << oc.o.key
                                  << ") is already cloud tiered to cloud-s3 tier: " << oc.o.meta.storage_class << dendl;
            return 0;
        }
        ...
        if (!r && oc.tier->get_tier_type() == "cloud-s3") {
            ...
            if (!oc.o.is_current() && !pass_object_lock_check(oc.driver, oc.obj.get(), oc.dpp)) {
                ...
                ldpp_dout(oc.dpp, 10) << "Object(key:" << oc.o.key
                                      ...
                                      << target_placement.storage_class << dendl;
                return 0;
            }
            ...
            if (!oc.driver->valid_placement(target_placement)) {
                ldpp_dout(oc.dpp, 0) << "ERROR: non existent dest placement: " << target_placement
                                     << " bucket=" << oc.bucket << " rule_id=" << oc.op.id << " " << oc.wq->thr_name()
                                     << dendl;
                return -EINVAL;
            }

            int r =
                oc.obj->transition(oc.bucket, target_placement, o.meta.mtime, o.versioned_epoch, oc.dpp, null_yield);
            if (r < 0) {
                ldpp_dout(oc.dpp, 0) << "ERROR: failed to transition obj " << oc.bucket << ":" << o.key << " -> "
                                     << transition.storage_class << " " << cpp_strerror(r) << " " << oc.wq->thr_name()
                                     << dendl;
                return r;
            }
        }
        ldpp_dout(oc.dpp, 2) << "TRANSITIONED:" << oc.bucket << ":" << o.key << " -> " << transition.storage_class
                             << " " << oc.wq->thr_name() << dendl;
        return 0;
    }
};
...
{
protected:
    bool check_current_state(bool is_current) override { return is_current; }

    ceph::real_time get_effective_mtime(lc_op_ctx& oc) override { return oc.o.meta.mtime; }
...
protected:
    ...
    ceph::real_time get_effective_mtime(lc_op_ctx& oc) override { return oc.effective_mtime; }

public:
    LCOpAction_NonCurrentTransition(op_env& env, const transition_action& _transition)
        : LCOpAction_Transition(_transition)
    {}
    int process(lc_op_ctx& oc)
    {
...
void LCOpRule::build()
{
    ...
    if (op.expiration > 0 || op.expiration_date != boost::none) {
...
int LCOpRule::process(rgw_bucket_dir_entry& o, const DoutPrefixProvider* dpp, WorkQ* wq)
{
    ...
    if (selected && (*selected)->should_process()) {
        ...
        if (!cont) {
            ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << ": no rule match, skipping " << wq->thr_name()
                               << dendl;
            return 0;
        }
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: remove_expired_obj " << env.bucket << ":" << o.key << " " << cpp_strerror(r)
                              << " " << wq->thr_name() << dendl;
            return r;
        }
...
int RGWLC::bucket_lc_process(string& shard_id, LCWorker* worker, time_t stop_at, bool once)
{
    ...
    if (ret < 0) {
        ldpp_dout(this, 0) << "LC:load_bucket for " << bucket_name << " failed" << dendl;
        return ret;
    }

    auto stack_guard = make_scope_guard([&worker] { worker->workpool->drain(); });

    if (bucket->get_marker() != bucket_marker) {
        ldpp_dout(this, 1) << "LC: deleting stale entry found for bucket=" << bucket_tenant << ":" << bucket_name
                           << " cur_marker=" << bucket->get_marker() << " orig_marker=" << bucket_marker << dendl;
        return -ENOENT;
    }

    map<string, bufferlist>::iterator aiter = bucket->get_attrs().find(RGW_ATTR_LC);
    if (aiter == bucket->get_attrs().end()) {
        ldpp_dout(this, 0) << "WARNING: bucket_attrs.find(RGW_ATTR_LC) failed for " << bucket_name
                           << " (terminates bucket_lc_process(...))" << dendl;
        return 0;
    }
    ...
    auto pf = [](RGWLC::LCWorker* wk, WorkQ* wq, WorkItem& wi) {
        ...
        if (ret < 0) {
            ldpp_dout(wk->get_lc(), 20) << "ERROR: orule.process() returned ret=" << ret << "thread:" << wq->thr_name()
                                        << dendl;
        }
    };
    worker->workpool->setf(pf);
    ...
    for (auto prefix_iter = prefix_map.begin(); prefix_iter != prefix_map.end(); ++prefix_iter) {
        ...
        if (!zone_check(op, zone)) {
            ldpp_dout(this, 7) << "LC rule not executable in " << zone->get_tier_type() << " zone, skipping" << dendl;
            continue;
        }
...
int RGWLC::bucket_lc_post(int index, int max_lock_sec, rgw::sal::Lifecycle::LCEntry& entry, int& result,
                          LCWorker* worker)
{
    utime_t lock_duration(cct->_conf->rgw_lc_lock_max_time, 0);

    std::unique_ptr<rgw::sal::LCSerializer> lock = sal_lc->get_serializer(lc_index_lock_name, obj_names[index], cookie);

    ldpp_dout(this, 5) << "RGWLC::bucket_lc_post(): POST " << entry << " index: " << index
    ...
    do {
        ...
        if (ret == -EBUSY || ret == -EEXIST) {
            ...
            ldpp_dout(this, 0) << "RGWLC::bucket_lc_post() failed to acquire lock on " << obj_names[index]
                            ...
                            vector<std::unique_ptr<rgw::sal::Lifecycle::LCEntry>>& progress_map, int& index)
{
    ...
    for (; index < max_objs; index++, marker = "") {
        ...
        if (progress_map.size() > 0) marker = progress_map.back()->get_bucket();

        if (progress_map.size() >= max_entries) break;
    }
    return 0;
}
...
{
    vector<int> v(n, 0);
    std::generate(v.begin(), v.end(), [ix = 0]() mutable { return ix++; });
...
static inline int get_lc_index(CephContext* cct, const std::string& shard_id)
{
...
static inline void get_lc_oid(CephContext* cct, const std::string& shard_id, string* oid)
{
    /* n.b. review hash algo */
...
int RGWLC::process(LCWorker* worker, const std::unique_ptr<rgw::sal::Bucket>& optional_bucket, bool once = false)
{
    int ret = 0;
...
bool RGWLC::expired_session(time_t started)
{
    ...
    time_t interval = (cct->_conf->rgw_lc_debug_interval > 0) ? cct->_conf->rgw_lc_debug_interval : 24 * 60 * 60;

    auto now = time(nullptr);

    ldpp_dout(this, 16) << "RGWLC::expired_session"
                        << " started: " << started << " interval: " << interval << "(*2==" << 2 * interval << ")"
                        << " now: " << now << dendl;
...
{
    uint64_t interval = (cct->_conf->rgw_lc_debug_interval > 0) ? cct->_conf->rgw_lc_debug_interval : 24 * 60 * 60;

    return time(nullptr) + interval;
}

int RGWLC::process_bucket(int index, int max_lock_secs, LCWorker* worker, const std::string& bucket_entry_marker,
                          bool once = false)
{
    ...
    if (ret == -EBUSY || ret == -EEXIST) {
        /* already locked by another lc processor */
        ldpp_dout(this, 0) << "RGWLC::process() failed to acquire lock on " << obj_names[index] << dendl;
        return -EBUSY;
    }
    if (ret < 0) return 0;

    std::unique_lock<rgw::sal::LCSerializer> lock(*(serializer.get()), std::adopt_lock);
        ...
        if (entry->get_status() == lc_processing) {
            if (expired_session(entry->get_start_time())) {
                ldpp_dout(this, 5) << "RGWLC::process_bucket(): STALE lc session found for: " << entry
                                   << " index: " << index << " worker ix: " << worker->ix << " (clearing)" << dendl;
            }
            else {
                ldpp_dout(this, 5) << "RGWLC::process_bucket(): ACTIVE entry: " << entry << " index: " << index
                                   << " worker ix: " << worker->ix << dendl;
                return ret;
            }
        }
    ...
    ldpp_dout(this, 5) << "RGWLC::process_bucket(): START entry 1: " << entry << " index: " << index
                       << " worker ix: " << worker->ix << dendl;

    entry->set_status(lc_processing);
    ret = sal_lc->set_entry(obj_names[index], *entry);
    if (ret < 0) {
        ldpp_dout(this, 0) << "RGWLC::process_bucket() failed to set obj entry " << obj_names[index]
                           << entry->get_bucket() << entry->get_status() << dendl;
        return ret;
    }

    ldpp_dout(this, 5) << "RGWLC::process_bucket(): START entry 2: " << entry << " index: " << index
                       << " worker ix: " << worker->ix << dendl;
...
static inline bool allow_shard_rollover(CephContext* cct, time_t now, time_t shard_rollover_date)
{
     ...
     */
    if (((shard_rollover_date < now) && (now - shard_rollover_date > 24 * 60 * 60)) ||
...
inline int RGWLC::advance_head(const std::string& lc_shard, rgw::sal::Lifecycle::LCHead& head,
                               rgw::sal::Lifecycle::LCEntry& entry, time_t start_date)
{
    int ret{0};
    ...
    if (ret < 0) {
        ldpp_dout(this, 0) << "RGWLC::process() failed to get obj entry " << lc_shard << dendl;
        goto exit;
    }
    ...
    if (ret < 0) {
        ldpp_dout(this, 0) << "RGWLC::process() failed to put head " << lc_shard << dendl;
        goto exit;
    }
exit:
    return ret;
...
int RGWLC::process(int index, int max_lock_secs, LCWorker* worker, bool once = false)
{
    int ret{0};
    const auto& lc_shard = obj_names[index];

    std::unique_ptr<rgw::sal::Lifecycle::LCHead> head;
    std::unique_ptr<rgw::sal::Lifecycle::LCEntry>
        entry;   // string = bucket_name:bucket_id, start_time, int = LC_BUCKET_STATUS
    ...
    if (!shard_lock.wait_backoff(lock_lambda)) {
        ldpp_dout(this, 0) << "RGWLC::process(): failed to aquire lock on " << lc_shard << " after "
                           << shard_lock.get_retries() << dendl;
        return 0;
    }
    ...
    do {
        ...
        if (ret < 0) {
            ldpp_dout(this, 0) << "RGWLC::process() failed to get obj head " << lc_shard << ", ret=" << ret << dendl;
            goto exit;
        }
	...
	allow_shard_rollover(cct, now, head->get_shard_rollover_date()) /* prevent multiple passes by diff.
            ...
            ldpp_dout(this, 5) << "RGWLC::process() process shard rollover lc_shard=" << lc_shard
                               ...
                               << " head.shard_rollover_date=" << head->get_shard_rollover_date() << dendl;
            ...
            ldpp_dout(this, 0) << "RGWLC::process() head.marker !empty() at START for shard==" << lc_shard
                               << " head last stored at " << rgw_to_asctime(utime_t(time_t(head->get_start_date()), 0))
                               << dendl;
        ...
        if (entry && !entry->get_bucket().empty()) {
            if (entry->get_status() == lc_processing) {
                if (expired_session(entry->get_start_time())) {
                    ...
                    ldpp_dout(this, 5) << "RGWLC::process(): ACTIVE entry: " << entry << " index: " << index
                                       << " worker ix: " << worker->ix << dendl;
                    /* skip to next entry */
                    ...
                    if (head->get_marker().empty()) {
                        ldpp_dout(this, 5) << "RGWLC::process() cycle finished lc_shard=" << lc_shard << dendl;
                ...
                if ((entry->get_status() == lc_complete) && already_run_today(cct, entry->get_start_time())) {
                    /* skip to next entry */
                    if (advance_head(lc_shard, *head.get(), *entry.get(), now) < 0) {
                        goto exit;
                    }
            ...
            ldpp_dout(this, 5) << "RGWLC::process() entry.bucket.empty() == true at START 1"
                               ...
                               << " or after removal of an lc_shard object)" << dendl;
            goto exit;
        }
        ...
        ldpp_dout(this, 5) << "RGWLC::process(): START entry 1: " << entry << " index: " << index
                           << " worker ix: " << worker->ix << dendl;
        ...
        if (ret < 0) {
            ldpp_dout(this, 0) << "RGWLC::process() failed to set obj entry " << lc_shard << entry->get_bucket()
                               << entry->get_status() << dendl;
            goto exit;
        }
        ...
        ldpp_dout(this, 5) << "RGWLC::process(): START entry 2: " << entry << " index: " << index
                           << " worker ix: " << worker->ix << dendl;
        ...
        if (ret == -ENOENT) {
            ...
            if (ret < 0) {
                ldpp_dout(this, 0) << "RGWLC::process() failed to remove entry " << lc_shard << " (nonfatal)" << dendl;
            ...
            ret = sal_lc->set_entry(lc_shard, *entry);
            if (ret < 0) {
                ldpp_dout(this, 0) << "RGWLC::process() failed to set entry on " << lc_shard << dendl;
                /* fatal, locked */
                goto exit;
            }
        }
...
template<typename F>
static int guard_lc_modify(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, rgw::sal::Lifecycle* sal_lc,
                           const rgw_bucket& bucket, const string& cookie, const F& f)
{
    ...
    std::unique_ptr<rgw::sal::LCSerializer> lock = sal_lc->get_serializer(lc_index_lock_name, oid, cookie);
    utime_t time(max_lock_secs, 0);
    ...
    do {
        ...
        if (ret == -EBUSY || ret == -EEXIST) {
            ldpp_dout(dpp, 0) << "RGWLC::RGWPutLC() failed to acquire lock on " << oid
                              << ", retry in 100ms, ret=" << ret << dendl;
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "RGWLC::RGWPutLC() failed to acquire lock on " << oid << ", ret=" << ret << dendl;
            break;
        }
        ret = f(sal_lc, oid, *entry.get());
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "RGWLC::RGWPutLC() failed to set entry on " << oid << ", ret=" << ret << dendl;
        }
        break;
    } while (true);
    lock->unlock();
...
int RGWLC::set_bucket_config(rgw::sal::Bucket* bucket, const rgw::sal::Attrs& bucket_attrs,
                             RGWLifecycleConfiguration* config)
{
    ...
    ret = guard_lc_modify(this,
                          driver,
                          sal_lc.get(),
                          b,
                          cookie,
                          [&](rgw::sal::Lifecycle* sal_lc, const string& oid, rgw::sal::Lifecycle::LCEntry& entry) {
...
int RGWLC::remove_bucket_config(rgw::sal::Bucket* bucket, const rgw::sal::Attrs& bucket_attrs, bool merge_attrs)
{
    rgw::sal::Attrs attrs = bucket_attrs;
    rgw_bucket& b = bucket->get_key();
    int ret{0};
    ...
    if (merge_attrs) {
        ...
        if (ret < 0) {
            ldpp_dout(this, 0) << "RGWLC::RGWDeleteLC() failed to set attrs on bucket=" << b.name
                               << " returned err=" << ret << dendl;
            return ret;
        }
    }
...
int fix_lc_shard_entry(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, rgw::sal::Lifecycle* sal_lc,
                       rgw::sal::Bucket* bucket)
{
    if (auto aiter = bucket->get_attrs().find(RGW_ATTR_LC); aiter == bucket->get_attrs().end()) {
    ...
    // 2. entry doesn't exist, which usually happens when reshard has happened prior to update and next LC process has
    // already dropped the update
    ...
    if (ret == -ENOENT) {
        ldpp_dout(dpp, 1) << "No entry for bucket=" << bucket << " creating " << dendl;
        ...
        ret = guard_lc_modify(
            ...
            bucket->get_key(),
            cookie,
            [&lc_oid](rgw::sal::Lifecycle* slc, const string& oid, rgw::sal::Lifecycle::LCEntry& entry) {
...
std::string s3_expiration_header(DoutPrefixProvider* dpp, const rgw_obj_key& obj_key, const RGWObjTags& obj_tagset,
                                 const ceph::real_time& mtime, const std::map<std::string, buffer::list>& bucket_attrs)
{
    ...
    if (aiter == bucket_attrs.end()) return hdr;
    ...
    if (cct->_conf->subsys.should_gather(ceph_subsys_rgw, 16)) {
        for (const auto& elt : obj_tag_map) {
            ldpp_dout(dpp, 16) << __func__ << "() key=" << elt.first << " val=" << elt.second << dendl;
        }
    }
    ...
    for (const auto& ri : rule_map) {
        ...
        ldpp_dout(dpp, 10) << "rule: " << ri.first << " prefix: " << prefix << " expiration: "
                           << " date: " << expiration.get_date() << " days: " << expiration.get_days()
                           << " noncur_expiration: "
                           << " date: " << noncur_expiration.get_date() << " days: " << noncur_expiration.get_days()
                           << dendl;
        ...
        if (!prefix.empty()) {
            if (!boost::starts_with(obj_key.name, prefix)) continue;
        }
        ...
        if (filter.has_tags()) {
            ...
            for (auto& tag : rule_tagset.get_tags()) {
                ...
                if (obj_tag == obj_tag_map.end() || obj_tag->second != tag.second) {
                    ldpp_dout(dpp, 10) << "tag does not match obj_key=" << obj_key << " rule_id=" << id
                                       << " tag=" << tag << dendl;
                    tag_match = false;
                    break;
                }
        ...
        const LCExpiration& rule_expiration = (obj_key.instance.empty()) ? expiration : noncur_expiration;

        if (rule_expiration.has_date()) {
            rule_expiration_date =
                boost::optional<ceph::real_time>(ceph::from_iso_8601(rule.get_expiration().get_date()));
        }
        else {
            if (rule_expiration.has_days()) {
                rule_expiration_date = boost::optional<ceph::real_time>(
                    mtime + make_timespan(double(rule_expiration.get_days()) * 24 * 60 * 60 -
                                          ceph::real_clock::to_time_t(mtime) % (24 * 60 * 60) + 24 * 60 * 60));
            }
        }
    ...
    if (expiration_date && rule_id) {
        ...
        if (std::strftime(exp_buf, sizeof(exp_buf), "%a, %d %b %Y %T %Z", std::gmtime(&exp))) {
            hdr = fmt::format("expiry-date=\"{0}\", rule-id=\"{1}\"", exp_buf, *rule_id);
        }
        else {
...
bool s3_multipart_abort_header(DoutPrefixProvider* dpp, const rgw_obj_key& obj_key, const ceph::real_time& mtime,
                               const std::map<std::string, buffer::list>& bucket_attrs, ceph::real_time& abort_date,
                               std::string& rule_id)
{
    CephContext* cct = dpp->get_cct();
    RGWLifecycleConfiguration config(cct);
    ...
    const auto& aiter = bucket_attrs.find(RGW_ATTR_LC);
    if (aiter == bucket_attrs.end()) return false;
    ...
    for (const auto& ri : rule_map) {
        ...
        if (rule_abort_date) {
            if ((!abort_date_tmp) || (*abort_date_tmp > *rule_abort_date)) {
                abort_date_tmp = std::optional<ceph::real_time>(rule_abort_date);
