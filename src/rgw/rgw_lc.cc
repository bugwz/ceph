...
static bool obj_has_expired(CephContext* cct, ceph::real_time mtime, int days, ceph::real_time* expire_time = nullptr)
{
    ...
    ldout(cct, 20) << __func__ << __func__ << "(): mtime=" << mtime << " days=" << days << " base_time=" << base_time
...
struct op_env
{
    ...
    op_env(lc_op& _op, rgw::sal::RGWRadosStore* _store, LCWorker* _worker, rgw::sal::RGWBucket* _bucket,
           LCObjsLister& _ol)
        : op(_op)
        , store(_store)
        , worker(_worker)
...
static int remove_expired_obj(const DoutPrefixProvider* dpp, lc_op_ctx& oc, bool remove_indeed)
{
    ...
    return obj->delete_object(dpp, &oc.rctx, obj_owner, bucket_owner, meta.mtime, false, 0, version_id, null_yield);
} /* remove_expired_obj */
...
int RGWLC::handle_multipart_expiration(rgw::sal::RGWBucket* target, const multimap<string, lc_op>& prefix_map,
        ...
        if (obj_has_expired(cct, obj.meta.mtime, rule.mp_expiration)) {
            ...
            if (ret == 0) {
                ...
                if (ret == -ERR_NO_SUCH_UPLOAD) {
                    ldpp_dout(wk->get_lc(), 5) << "ERROR: abort_multipart_upload failed, ret=" << ret << wq->thr_name()
                                               << ", meta:" << obj.key << dendl;
                }
                else {
                    ldpp_dout(wk->get_lc(), 0) << "ERROR: abort_multipart_upload failed, ret=" << ret << wq->thr_name()
                                               << ", meta:" << obj.key << dendl;
                }
            } /* abort failed */
        }     /* expired */
...
static int read_obj_tags(const DoutPrefixProvider* dpp, rgw::sal::RGWObject* obj, RGWObjectCtx& ctx,
                         bufferlist& tags_bl)
{
    ...
    if (op.obj_tags != boost::none) {
        *skip = true;
        ...
        if (!has_all_tags(op, dest_obj_tags)) {
            ldout(oc.cct, 20) << __func__ << "() skipping obj " << oc.obj << " as tags do not match in rule: " << op.id
...
public:
    bool check(const DoutPrefixProvider* dpp, lc_op_ctx& oc) override
    {
        ...
        if (ret < 0) {
            ...
            ldout(oc.cct, 0) << "ERROR: check_tags on obj=" << oc.obj << " returned ret=" << ret << " "
                             << oc.wq->thr_name() << dendl;
            return false;
        }
...
public:
    ...
    int process(lc_op_ctx& oc)
    {
        ...
        if (o.is_delete_marker()) {
            ...
            if (r < 0) {
                ldout(oc.cct, 0) << "ERROR: current is-dm remove_expired_obj " << oc.bucket << ":" << o.key << " "
                                 << cpp_strerror(r) << " " << oc.wq->thr_name() << dendl;
                return r;
            }
            ldout(oc.cct, 2) << "DELETED: current is-dm " << oc.bucket << ":" << o.key << " " << oc.wq->thr_name()
                             << dendl;
        }
        else {
            /* ! o.is_delete_marker() */
            ...
            if (r < 0) {
                ldout(oc.cct, 0) << "ERROR: remove_expired_obj " << oc.bucket << ":" << o.key << " " << cpp_strerror(r)
                                 << " " << oc.wq->thr_name() << dendl;
                return r;
            }
            if (perfcounter) {
...
public:
    ...
    bool check(lc_op_ctx& oc, ceph::real_time* exp_time, const DoutPrefixProvider* dpp) override
    {
        ...
        return is_expired && pass_object_lock_check(oc.store, oc.obj.get(), oc.rctx, dpp);
    }
    ...
    int process(lc_op_ctx& oc)
    {
        ...
        if (r < 0) {
            ldout(oc.cct, 0) << "ERROR: remove_expired_obj (non-current expiration) " << oc.bucket << ":" << o.key
                             << " " << cpp_strerror(r) << " " << oc.wq->thr_name() << dendl;
            return r;
        }
        if (perfcounter) {
        ...
        ldout(oc.cct, 2) << "DELETED:" << oc.bucket << ":" << o.key << " (non-current expiration) " << oc.wq->thr_name()
                         << dendl;
        return 0;
    }
};
...
public:
    ...
    int process(lc_op_ctx& oc)
    {
        ...
        int r = remove_expired_obj(oc.dpp, oc, true);
        if (r < 0) {
            ldout(oc.cct, 0) << "ERROR: remove_expired_obj (delete marker expiration) " << oc.bucket << ":" << o.key
                             << " " << cpp_strerror(r) << " " << oc.wq->thr_name() << dendl;
            return r;
        }
        if (perfcounter) {
        ...
        ldout(oc.cct, 2) << "DELETED:" << oc.bucket << ":" << o.key << " (delete marker expiration) "
                         << oc.wq->thr_name() << dendl;
        return 0;
    }
};
...
public:
    ...
    bool check(lc_op_ctx& oc, ceph::real_time* exp_time, const DoutPrefixProvider* dpp) override
    {
        ...
        ldout(oc.cct, 20) << __func__ << "(): key=" << o.key << ": is_expired=" << is_expired << " "
                          << oc.wq->thr_name() << dendl;
    ...
    int process(lc_op_ctx& oc)
    {
        ...
        int r = oc.obj->transition(
            oc.rctx, oc.bucket, target_placement, o.meta.mtime, o.versioned_epoch, oc.dpp, null_yield);
        if (r < 0) {
...
int LCOpRule::process(rgw_bucket_dir_entry& o, const DoutPrefixProvider* dpp, WorkQ* wq)
{
    ...
    if (selected && (*selected)->should_process()) {
        ...
        if (!cont) {
            ldpp_dout(dpp, 20) << __func__ << "(): key=" << o.key << ": no rule match, skipping "
                               << " " << wq->thr_name() << dendl;
            return 0;
        }
                          ...
                          LCWorker* worker)
{
    ...
    rgw::sal::LCSerializer* lock = sal_lc->get_serializer(lc_index_lock_name, obj_names[index], cookie);

    dout(5) << "RGWLC::bucket_lc_post(): POST " << entry << " index: " << index << " worker ix: " << worker->ix
            << dendl;
...
int RGWLC::list_lc_progress(string& marker, uint32_t max_entries, vector<rgw::sal::Lifecycle::LCEntry>& progress_map,
                            int& index)
{
    progress_map.clear();
    for (; index < max_objs; index++, marker = "") {
        ...
        if (progress_map.size() > 0) marker = progress_map.back().bucket;
...
int RGWLC::process(int index, int max_lock_secs, LCWorker* worker, bool once = false)
{
    ...
    rgw::sal::LCSerializer* lock = sal_lc->get_serializer(lc_index_lock_name, obj_names[index], std::string());
    do {
        utime_t now = ceph_clock_now();
        // string = bucket_name:bucket_id, start_time, int = LC_BUCKET_STATUS
        rgw::sal::Lifecycle::LCEntry entry;
        if (max_lock_secs <= 0) return -EAGAIN;
        ...
        if (ret == -EBUSY || ret == -EEXIST) {
            /* already locked by another lc processor */
            ldpp_dout(this, 0) << "RGWLC::process() failed to acquire lock on " << obj_names[index]
                               << ", sleep 5, try again" << dendl;
            sleep(5);
            continue;
        }
        ...
        if (ret < 0) {
            ldpp_dout(this, 0) << "RGWLC::process() failed to get obj head " << obj_names[index] << ", ret=" << ret
                               << dendl;
            goto exit;
        }
            ...
            if (ret >= 0) {
                if (entry.status == lc_processing) {
                    if (expired_session(entry.start_time)) {
                        dout(5) << "RGWLC::process(): STALE lc session found for: " << entry << " index: " << index
                                << " worker ix: " << worker->ix << " (clearing)" << dendl;
                    }
                    else {
                        dout(5) << "RGWLC::process(): ACTIVE entry: " << entry << " index: " << index
        ...
        if (!if_already_run_today(head.start_date) || once) {
            head.start_date = now;
            head.marker.clear();
            ret = bucket_lc_prepare(index, worker);
            if (ret < 0) {
                ldpp_dout(this, 0) << "RGWLC::process() failed to update lc object " << obj_names[index]
                                   << ", ret=" << ret << dendl;
                goto exit;
            }
        }
        ...
        if (entry.bucket.empty()) goto exit;
        ...
        ret = sal_lc->set_entry(obj_names[index], entry);
        if (ret < 0) {
            ldpp_dout(this, 0) << "RGWLC::process() failed to set obj entry " << obj_names[index] << entry.bucket
                               << entry.status << dendl;
            goto exit;
        }
...
static inline void get_lc_oid(CephContext* cct, const string& shard_id, string* oid)
{
...
template<typename F>
static int guard_lc_modify(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, rgw::sal::Lifecycle* sal_lc,
                           const rgw_bucket& bucket, const string& cookie, const F& f)
{
    ...
    rgw::sal::LCSerializer* lock = sal_lc->get_serializer(lc_index_lock_name, oid, cookie);
    utime_t time(max_lock_secs, 0);
    ...
    do {
        ...
        if (ret == -EBUSY || ret == -EEXIST) {
            ldpp_dout(dpp, 0) << "RGWLC::RGWPutLC() failed to acquire lock on " << oid << ", sleep 5, try again"
                              << dendl;
...
int RGWLC::set_bucket_config(RGWBucketInfo& bucket_info, const map<string, bufferlist>& bucket_attrs,
                             RGWLifecycleConfiguration* config)
{
    ...
    ret =
        guard_lc_modify(this,
                        store,
                        sal_lc.get(),
                        bucket,
                        cookie,
                        [&](rgw::sal::Lifecycle* sal_lc, const string& oid, const rgw::sal::Lifecycle::LCEntry& entry) {
...
int RGWLC::remove_bucket_config(RGWBucketInfo& bucket_info, const map<string, bufferlist>& bucket_attrs)
{
    map<string, bufferlist> attrs = bucket_attrs;
    ...
    if (ret < 0) {
        ldpp_dout(this, 0) << "RGWLC::RGWDeleteLC() failed to set attrs on bucket=" << bucket.name
...
int fix_lc_shard_entry(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, rgw::sal::Lifecycle* sal_lc,
                       const RGWBucketInfo& bucket_info, const map<std::string, bufferlist>& battrs)
{
    if (auto aiter = battrs.find(RGW_ATTR_LC); aiter == battrs.end()) {
    ...
    if (ret == -ENOENT) {
        ldpp_dout(dpp, 1) << "No entry for bucket=" << bucket_info.bucket.name << " creating " << dendl;
        ...
        ret = guard_lc_modify(
            ...
            bucket_info.bucket,
            cookie,
            [&lc_oid](rgw::sal::Lifecycle* slc, const string& oid, const rgw::sal::Lifecycle::LCEntry& entry) {
