...
int RGWRole::store_info(const DoutPrefixProvider* dpp, bool exclusive, optional_yield y)
{
    ...
    if (!this->tags.empty()) {
        ...
        return rgw_put_system_obj(
            dpp, obj_ctx, svc->zone->get_zone_params().roles_pool, oid, bl, exclusive, nullptr, real_time(), y, &attrs);
    }

    return rgw_put_system_obj(
        dpp, obj_ctx, svc->zone->get_zone_params().roles_pool, oid, bl, exclusive, NULL, real_time(), y, NULL);
}
...
int RGWRole::create(const DoutPrefixProvider* dpp, bool exclusive, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR:  storing role info in pool: " << pool.name << ": " << id << ": "
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: storing role name in pool: " << pool.name << ": " << name << ": "
                          << cpp_strerror(-ret) << dendl;
        ...
        if (info_ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: cleanup of role id from pool: " << pool.name << ": " << id << ": "
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: storing role path in pool: " << pool.name << ": " << path << ": "
                          << cpp_strerror(-ret) << dendl;
        // Delete the role info that was stored in the previous call
        string oid = get_info_oid_prefix() + id;
        int info_ret = rgw_delete_system_obj(dpp, svc->sysobj, pool, oid, NULL, y);
        ...
        if (name_ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: cleanup of role name from pool: " << pool.name << ": " << name << ": "
...
int RGWRole::delete_obj(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: deleting role id from pool: " << pool.name << ": " << id << ": "
                          << cpp_strerror(-ret) << dendl;
    }
    ...
    ret = rgw_delete_system_obj(dpp, svc->sysobj, pool, oid, NULL, y);
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: deleting role name from pool: " << pool.name << ": " << name << ": "
                          << cpp_strerror(-ret) << dendl;
    }
    ...
    ret = rgw_delete_system_obj(dpp, svc->sysobj, pool, oid, NULL, y);
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: deleting role path from pool: " << pool.name << ": " << path << ": "
                          << cpp_strerror(-ret) << dendl;
    }
    return ret;
}
...
int RGWRole::update(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR:  storing info in pool: " << pool.name << ": " << id << ": " << cpp_strerror(-ret)
                          << dendl;
        return ret;
    }
...
int RGWRole::read_id(const DoutPrefixProvider* dpp, const string& role_name, const string& tenant, string& role_id,
                     optional_yield y)
{
    auto svc = ctl->svc;
    auto& pool = svc->zone->get_zone_params().roles_pool;
...
int RGWRole::read_info(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed reading role info from pool: " << pool.name << ": " << id << ": "
                          << cpp_strerror(-ret) << dendl;
        return ret;
    }
...
int RGWRole::read_name(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    int ret = rgw_get_system_obj(obj_ctx, pool, oid, bl, NULL, NULL, y, dpp);
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed reading role name from pool: " << pool.name << ": " << name << ": "
                          << cpp_strerror(-ret) << dendl;
        return ret;
    }
...
bool RGWRole::validate_input()
{
    ...
    if (max_session_duration < SESSION_DURATION_MIN || max_session_duration > SESSION_DURATION_MAX) {
...
int RGWRole::get_roles_by_path_prefix(const DoutPrefixProvider* dpp, RGWRados* store, CephContext* cct,
                                      const string& path_prefix, const string& tenant, vector<RGWRole>& roles,
                                      optional_yield y)
{
    ...
    do {
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: listing filtered objects failed: " << pool.name << ": " << prefix << ": "
