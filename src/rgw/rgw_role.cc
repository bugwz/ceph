...
RGWRole::RGWRole(std::string name, std::string tenant, std::string path, std::string trust_policy,
                 std::string max_session_duration_str, std::multimap<std::string, std::string> tags)
{
    ...
    if (this->info.path.empty()) this->info.path = "/";
...
{
    if (info.max_session_duration < SESSION_DURATION_MIN || info.max_session_duration > SESSION_DURATION_MAX) {
...
int RGWRole::update(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR:  storing info in Role pool: " << info.id << ": " << cpp_strerror(-ret) << dendl;
        return ret;
    }
...
RGWRoleMetadataHandler::RGWRoleMetadataHandler(Driver* driver, RGWSI_Role_RADOS* role_svc)
{
    this->driver = driver;
    base_init(role_svc->ctx(), role_svc->get_be_handler());
}

RGWMetadataObject* RGWRoleMetadataHandler::get_meta_obj(JSONObj* jo, const obj_version& objv,
...
int RGWRoleMetadataHandler::do_get(RGWSI_MetaBackend_Handler::Op* op, std::string& entry, RGWMetadataObject** obj,
                                   optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    RGWRoleMetadataObject* rdo = new RGWRoleMetadataObject(info, objv_tracker.read_version, mtime, driver);
    *obj = rdo;

    return 0;
}

int RGWRoleMetadataHandler::do_remove(RGWSI_MetaBackend_Handler::Op* op, std::string& entry,
...
public:
    RGWMetadataHandlerPut_Role(RGWRoleMetadataHandler* handler, RGWSI_MetaBackend_Handler::Op* op, std::string& entry,
...
int RGWRoleMetadataHandler::do_put(RGWSI_MetaBackend_Handler::Op* op, std::string& entry, RGWMetadataObject* obj,
