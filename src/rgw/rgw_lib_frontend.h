...
public:
    RGWLibProcess(CephContext* cct, RGWProcessEnv& pe, int num_threads, std::string uri_prefix,
    ...
    void enqueue_req(RGWLibRequest* req)
    {
        ...
        lsubdout(g_ceph_context, rgw, 10)
            << __func__ << " enqueue request req=" << std::hex << req << std::dec << dendl;
...
public:
    ...
    RGWLibProcess* get_process() { return static_cast<RGWLibProcess*>(pprocess); }
    ...
    inline int start_req(RGWLibContinuedReq* req) { return static_cast<RGWLibProcess*>(pprocess)->start_request(req); }
