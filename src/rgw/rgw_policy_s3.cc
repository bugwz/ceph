...
public:
    ...
    bool check(RGWPolicyEnv* env, map<string, bool, ltstr_nocase>& checked_vars, string& err_msg)
    {
        ...
        dout(1) << "policy condition check " << v1 << " [" << rgw::crypt_sanitize::s3_policy{v1, first} << "] " << v2
                << " [" << rgw::crypt_sanitize::s3_policy{v2, second} << "]" << dendl;
...
bool RGWPolicyEnv::match_policy_vars(map<string, bool, ltstr_nocase>& policy_vars, string& err_msg)
{
    ...
    for (iter = vars.begin(); iter != vars.end(); ++iter) {
        ...
        if (strncasecmp(ignore_prefix.c_str(), var.c_str(), ignore_prefix.size()) == 0) continue;
...
{
    struct tm t;
    if (!parse_iso8601(e.c_str(), &t)) return -EINVAL;
