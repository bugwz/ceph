...
void RGWLifecycleConfiguration_S3::decode_xml(XMLObj* obj)
{
    ...
    if (cct->_conf->rgw_lc_max_rules < rule_map.size()) {
        stringstream ss;
        ss << "Warn: The lifecycle config has too many rules, rule number is:" << rule_map.size()
           << ", max number is:" << cct->_conf->rgw_lc_max_rules;
...
void LCRule_S3::decode_xml(XMLObj* obj)
{
    ...
    if (!has_expiration && !has_noncur_expiration && !has_mp_expiration && !has_transition && !has_noncur_transition) {
