...
RGWRealmReloader::RGWRealmReloader(RGWProcessEnv& env, const rgw::auth::ImplicitTenants& implicit_tenants,
                                   std::map<std::string, std::string>& service_map_meta, Pauser* frontends)
    : env(env)
...
void RGWRealmReloader::handle_notify(RGWRealmNotify type, bufferlist::const_iterator& p)
{
    if (!env.driver) {
...
void RGWRealmReloader::reload()
{
    ...
    while (!env.driver) {
        ...
        env.driver = DriverManager::get_storage(&dp,
                                                cct,
                                                cfg,
    ...
    env.auth_registry = rgw::auth::StrategyRegistry::create(cct, implicit_tenants, env.driver);
