...
template<typename Op> Aio::OpFunc aio_abstract(Op&& op)
{
...
template<typename Op> Aio::OpFunc aio_abstract(Op&& op, boost::asio::io_context& context, yield_context yield)
{
    return [op = std::move(op), &context, yield](Aio* aio, AioResult& r) mutable {
        ...
        librados::async_operate(context, ref.pool.ioctx(), ref.obj.oid, &op, 0, bind_executor(ex, Handler{aio, r}));
    };
}


Aio::OpFunc d3n_cache_aio_abstract(const DoutPrefixProvider* dpp, optional_yield y, off_t read_ofs, off_t read_len,
                                   std::string& cache_location)
{
    return [dpp, y, read_ofs, read_len, cache_location](Aio* aio, AioResult& r) mutable {
        ...
        lsubdout(g_ceph_context, rgw_datacache, 20)
            << "D3nDataCache: d3n_cache_aio_abstract(): libaio Read From Cache, oid=" << ref.obj.oid << dendl;
        c->file_aio_read_abstract(
            dpp, y.get_io_context(), y.get_yield_context(), cache_location, read_ofs, read_len, aio, r);
    };
}


template<typename Op> Aio::OpFunc aio_abstract(Op&& op, optional_yield y)
{
    ...
    if (y) {
        return aio_abstract(std::forward<Op>(op), y.get_io_context(), y.get_yield_context());
    }
...
Aio::OpFunc Aio::librados_op(librados::ObjectReadOperation&& op, optional_yield y)
{
    return aio_abstract(std::move(op), y);
}
Aio::OpFunc Aio::librados_op(librados::ObjectWriteOperation&& op, optional_yield y)
{
    return aio_abstract(std::move(op), y);
}

Aio::OpFunc Aio::d3n_cache_op(const DoutPrefixProvider* dpp, optional_yield y, off_t read_ofs, off_t read_len,
                              std::string& cache_location)
{
