...
static std::string get_canon_amz_hdr(const meta_map_t& meta_map)
{
    std::string dest;
 ...
 */
static std::string get_canon_resource(const DoutPrefixProvider* dpp, const char* const request_uri,
 ...
 */
void rgw_create_s3_canonical_header(const DoutPrefixProvider* dpp, const char* const method,
                                    const char* const content_md5, const char* const content_type,
                                    const char* const date, const meta_map_t& meta_map, const meta_map_t& qs_map,
                                    const char* const request_uri,
                                    const std::map<std::string, std::string>& sub_resources, std::string& dest_str)
{
    std::string dest;
 ...
 * compute a request's signature
 */
bool rgw_create_s3_canonical_header(const DoutPrefixProvider* dpp, const req_info& info, utime_t* const header_time,
...
static bool get_next_token(const std::string_view& s, size_t& pos, const char* const delims, std::string_view& token)
{
                                       ...
                                       const DoutPrefixProvider* dpp)
{
    ...
    static const std::array<std::string_view, 3> required_keys = {"Credential", "SignedHeaders", "Signature"};
...
{
    if (op_type == RGW_STS_GET_SESSION_TOKEN || op_type == RGW_STS_ASSUME_ROLE ||
        op_type == RGW_STS_ASSUME_ROLE_WEB_IDENTITY || op_type == RGW_OP_CREATE_ROLE || op_type == RGW_OP_DELETE_ROLE ||
        op_type == RGW_OP_GET_ROLE || op_type == RGW_OP_MODIFY_ROLE_TRUST_POLICY || op_type == RGW_OP_LIST_ROLES ||
        op_type == RGW_OP_PUT_ROLE_POLICY || op_type == RGW_OP_GET_ROLE_POLICY ||
        op_type == RGW_OP_LIST_ROLE_POLICIES || op_type == RGW_OP_DELETE_ROLE_POLICY ||
        op_type == RGW_OP_PUT_USER_POLICY || op_type == RGW_OP_GET_USER_POLICY ||
        op_type == RGW_OP_LIST_USER_POLICIES || op_type == RGW_OP_DELETE_USER_POLICY ||
        op_type == RGW_OP_CREATE_OIDC_PROVIDER || op_type == RGW_OP_DELETE_OIDC_PROVIDER ||
        op_type == RGW_OP_GET_OIDC_PROVIDER || op_type == RGW_OP_LIST_OIDC_PROVIDERS ||
        op_type == RGW_OP_PUBSUB_TOPIC_CREATE || op_type == RGW_OP_PUBSUB_TOPICS_LIST ||
        op_type == RGW_OP_PUBSUB_TOPIC_GET || op_type == RGW_OP_PUBSUB_TOPIC_DELETE || op_type == RGW_OP_TAG_ROLE ||
        op_type == RGW_OP_LIST_ROLE_TAGS || op_type == RGW_OP_UNTAG_ROLE || op_type == RGW_OP_UPDATE_ROLE) {
        return true;
    }
    return false;
}
                         ...
                         const DoutPrefixProvider* dpp)
{
    ...
    int ret;
    if (using_qs) {
        ret = parse_v4_query_string(info, credential, signedheaders, signature, date, session_token);
    }
    else {
        ret = parse_v4_auth_header(info, credential, signedheaders, signature, date, session_token, dpp);
    }
...
string gen_v4_scope(const ceph::real_time& timestamp, const string& region, const string& service)
{
    ...
    return fmt::format(FMT_STRING("{:d}{:02d}{:02d}/{:s}/{:s}/aws4_request"), year, mon, day, region, service);
}
...
std::string get_v4_canonical_qs(const req_info& info, const bool using_qs)
{
    ...
    canonical_qs.append(iter->first).append("=", ::strlen("=")).append(iter->second);

    for (iter++; iter != std::end(canonical_qs_map); iter++) {
        canonical_qs.append("&", ::strlen("&")).append(iter->first).append("=", ::strlen("=")).append(iter->second);
    }

    return canonical_qs;
}

static void add_v4_canonical_params_from_map(const map<string, string>& m, std::map<string, string>* result,
                                             bool is_non_s3_op)
{
    for (auto& entry : m) {
...
std::string get_v4_canonical_method(const req_state* s)
{
    ...
    if (s->op_type == RGW_OP_OPTIONS_CORS) {
        ...
        if (cors_method) {
            ...
            if (!cors_flags) {
                ldpp_dout(s, 1) << "invalid access-control-request-method header = " << cors_method << dendl;
                throw -EINVAL;
            }
...
boost::optional<std::string> get_v4_canonical_headers(const req_info& info, const std::string_view& signedheaders,
                                                      const bool using_qs, const bool force_boto2_compat)
{
    ...
    for (const auto& token : get_str_vec<5>(signedheaders, ";")) {
        ...
        std::transform(std::begin(token), std::end(token), std::back_inserter(token_env), [](const int c) {
        ...
        if (token_env == "HTTP_CONTENT_MD5" &&
            !std::all_of(std::begin(token_value), std::end(token_value), is_base64_for_content_md5)) {
        ...
        if (force_boto2_compat && using_qs && token == "host") {
            ...
            if (!secure_port.empty()) {
                if (secure_port != "443")
                    token_value.append(":", std::strlen(":")).append(secure_port.data(), secure_port.length());
            }
            else if (!port.empty()) {
                if (port != "80") token_value.append(":", std::strlen(":")).append(port.data(), port.length());
            }
        }
                          ...
                          std::map<std::string, std::string>* canonical_hdrs_map)
{
    ...
    if (header == "HTTP_CONTENT_LENGTH") {
        ...
        std::transform(start, std::end(header), std::back_inserter(token), [](const int c) {
...
std::string gen_v4_canonical_headers(const req_info& info, const map<string, string>& extra_headers,
                                     string* signed_hdrs)
{
    std::map<std::string, std::string> canonical_hdrs_map;
 ...
 */
sha256_digest_t get_v4_canon_req_hash(CephContext* cct, const std::string_view& http_verb,
                                      ...
                                      const std::string_view& request_payload_hash, const DoutPrefixProvider* dpp)
{
    ldpp_dout(dpp, 10) << "payload request hash = " << request_payload_hash << dendl;

    const auto canonical_req = string_join_reserve(
        "\n", http_verb, canonical_uri, canonical_qs, canonical_hdrs, signed_hdrs, request_payload_hash);
 ...
 */
AWSEngine::VersionAbstractor::string_to_sign_t get_v4_string_to_sign(
    CephContext* const cct, const std::string_view& algorithm, const std::string_view& request_date,
    const std::string_view& credential_scope, const sha256_digest_t& canonreq_hash, const DoutPrefixProvider* dpp)
{
...
static inline std::vector<unsigned char> transform_secret_key(const std::string_view& secret_access_key)
{
    ...
    for (const auto c : secret_access_key) {
        ...
        secret_key_utf8.insert(std::end(secret_key_utf8), std::begin(buf), std::begin(buf) + n);
    }
 ...
 */
static sha256_digest_t get_v4_signing_key(CephContext* const cct, const std::string_view& credential_scope,
                                          const std::string_view& secret_access_key, const DoutPrefixProvider* dpp)
{
    ...
    /* aws4_request */
    const auto signing_key = calc_hmac_sha256(service_k, std::string_view("aws4_request"));
...
AWSEngine::VersionAbstractor::server_signature_t get_v4_signature(
    const std::string_view& credential_scope, CephContext* const cct, const std::string_view& secret_key,
    const AWSEngine::VersionAbstractor::string_to_sign_t& string_to_sign, const DoutPrefixProvider* dpp)
{
    ...
    srv_signature_t signature(srv_signature_t::initialized_later(), digest.SIZE * 2);
...
std::pair<AWSv4ComplMulti::ChunkMeta, size_t /* consumed */> AWSv4ComplMulti::ChunkMeta::create_next(
    CephContext* const cct, ChunkMeta&& old, const char* const metabuf, const size_t metabuf_len)
{
    ...
    const auto signature = signature_part.substr(eq_sign_pos + 1, data_sep_pos - 1 - eq_sign_pos);
    ...
    ldout(cct, 20) << "parsed new chunk; signature=" << signature << ", data_length=" << data_length
                   << ", data_starts_in_stream=" << data_starts_in_stream << dendl;

    return std::make_pair(ChunkMeta(data_starts_in_stream, data_length, signature), semicolon_pos + 83);
}

std::string AWSv4ComplMulti::calc_chunk_signature(const std::string& payload_hash) const
{
    const auto string_to_sign = string_join_reserve("\n",
...
size_t AWSv4ComplMulti::recv_chunk(char* const buf, const size_t buf_max, bool& eof)
{
    ...
    if (chunk_meta.is_new_chunk_in_stream(stream_pos)) {
        ...
        do {
            ...
            const size_t received = io_base_t::recv_body(parsing_buf.data() + orig_size, to_extract);
        ...
        std::tie(chunk_meta, consumed) =
            ChunkMeta::create_next(cct, std::move(chunk_meta), parsing_buf.data(), parsing_buf.size());
        ...
        parsing_buf.erase(std::begin(parsing_buf), std::begin(parsing_buf) + consumed);
    }

    size_t stream_pos_was = stream_pos - parsing_buf.size();

    size_t to_extract = std::min(chunk_meta.get_data_size(stream_pos_was), buf_max);
...
{
    const char* const decoded_length = s_rw->info.env->get("HTTP_X_AMZ_DECODED_CONTENT_LENGTH");
    ...
    AWS_AUTHv4_IO(s_rw)->add_filter(std::static_pointer_cast<io_base_t>(shared_from_this()));
}
...
rgw::auth::Completer::cmplptr_t AWSv4ComplMulti::create(const req_state* const s, std::string_view date,
                                                        ...
                                                        const boost::optional<std::string>& secret_key)
{
    ...
    const auto signing_key = rgw::auth::s3::get_v4_signing_key(s->cct, credential_scope, *secret_key, s);

    return std::make_shared<AWSv4ComplMulti>(
        s, std::move(date), std::move(credential_scope), std::move(seed_signature), signing_key);
}
...
rgw::auth::Completer::cmplptr_t AWSv4ComplSingle::create(const req_state* const s, const boost::optional<std::string>&)
{
