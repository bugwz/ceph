...
bool parse(CephContext* cct, const std::string& tenant, const std::string& fname, std::istream& in) noexcept
{
    bufferlist bl;
    bl.append(in);
    try {
        auto p = rgw::IAM::Policy(cct, tenant, bl, cct->_conf.get_val<bool>("rgw_policy_reject_invalid_principals"));
    }
...
{
    std::cout << "usage: " << cmdname << " -t <tenant> [filename]" << std::endl;
}
...
int main(int argc, const char** argv)
{
    ...
    for (std::vector<const char*>::iterator i = args.begin(); i != args.end();) {
        ...
        else if (ceph_argparse_witharg(args, i, &val, "--tenant", "-t", (char*)nullptr)) {
            tenant = std::move(val);
        }
        else {
            ++i;
