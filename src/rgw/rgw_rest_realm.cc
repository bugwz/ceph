...
void RGWOp_Period_Post::execute(optional_yield y)
{
    ...
    if (period.get_realm() != store->svc()->zone->get_realm().get_id()) {
        error_stream << "period with realm id " << period.get_realm() << " doesn't match current realm "
                     << store->svc()->zone->get_realm().get_id() << std::endl;
        op_ret = -EINVAL;
        return;
    }
...
{
public:
    RGWHandler_REST* get_handler(rgw::sal::RGWRadosStore* store, struct req_state*,
                                 const rgw::auth::StrategyRegistry& auth_registry, const std::string&) override
    {
...
RGWHandler_REST* RGWRESTMgr_Realm::get_handler(rgw::sal::RGWRadosStore* store, struct req_state*,
