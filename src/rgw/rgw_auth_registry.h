...
class StrategyRegistry
{
    ...
    using s3_strategy_t = rgw::auth::s3::AWSAuthStrategy<AbstractorT, AllowAnonAccessT>;

    struct s3_main_strategy_t : public Strategy
    {
        using s3_main_strategy_plain_t = s3_strategy_t<rgw::auth::s3::AWSGeneralAbstractor, true>;
        using s3_main_strategy_boto2_t = s3_strategy_t<rgw::auth::s3::AWSGeneralBoto2Abstractor>;

        s3_main_strategy_plain_t s3_main_strategy_plain;
        s3_main_strategy_boto2_t s3_main_strategy_boto2;

        s3_main_strategy_t(CephContext* const cct, const ImplicitTenants& implicit_tenant_context,
        ...
        const char* get_name() const noexcept override { return "rgw::auth::StrategyRegistry::s3_main_strategy_t"; }
    } s3_main_strategy;

    using s3_post_strategy_t = s3_strategy_t<rgw::auth::s3::AWSBrowserUploadAbstractor>;
...
public:
    StrategyRegistry(CephContext* const cct, const ImplicitTenants& implicit_tenant_context, rgw::sal::Driver* driver)
    ...
    const s3_main_strategy_t& get_s3_main() const { return s3_main_strategy; }

    const s3_post_strategy_t& get_s3_post() const { return s3_post_strategy; }

    const rgw::auth::swift::DefaultStrategy& get_swift() const { return swift_strategy; }

    const rgw::auth::sts::DefaultStrategy& get_sts() const { return sts_strategy; }

    static std::unique_ptr<StrategyRegistry> create(CephContext* const cct,
