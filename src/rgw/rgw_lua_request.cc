...
int SetAttribute(lua_State* L)
{
    ...
    switch (value_type) {
    case LUA_TSTRING: s->trace->SetAttribute(key, lua_tostring(L, 2)); break;
    ...
    default: luaL_error(L, "unsupported value type for SetAttribute");
    }
    return 0;
}
...
int AddEvent(lua_State* L)
{
    ...
    if (args == 1) {
        ...
        while (lua_next(L, 2) != 0) {
            ...
            switch (value_type) {
            case LUA_TSTRING: event_values.emplace(key, lua_tostring(L, -1)); break;
