...
struct rgw_placement_rule
{
    ...
    rgw_placement_rule() {}
    rgw_placement_rule(const std::string& _n, const std::string& _sc)
        : name(_n)
        , storage_class(_sc)
    {}
    rgw_placement_rule(const rgw_placement_rule& _r, const std::string& _sc)
        : name(_r.name)
    {
        if (!_sc.empty()) {
            storage_class = _sc;
    ...
    bool empty() const { return name.empty() && storage_class.empty(); }
    ...
    const std::string& get_storage_class() const { return get_canonical_storage_class(storage_class); }
    ...
    {
        return (name == r.name && get_storage_class() == r.get_storage_class());
    }

    bool operator!=(const rgw_placement_rule& r) const { return !(*this == r); }
    ...
    std::string to_str_explicit() const { return name + "/" + storage_class; }
    ...
    bool standard_storage_class() const { return storage_class.empty() || storage_class == RGW_STORAGE_CLASS_STANDARD; }
};
