...
public:
    PushAndRetryCR(CephContext* cct, const std::string& zone, RGWRESTConn* conn, RGWHTTPManager* http,
                   RGWPeriod& period)
        : RGWCoroutine(cct)
        , zone(zone)
        , conn(conn)
...
int PushAndRetryCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        for (;;) {
            yield
            {
                ldpp_dout(dpp, 10) << "pushing period " << period.get_id() << " to " << zone << dendl;
                // initialize the http params
                rgw_http_param_pair params[] = {
                    {"period", period.get_id().c_str()}, {"epoch", epoch.c_str()}, {nullptr, nullptr}};
                call(new PushCR(cct, conn, http, "/admin/realm/period", params, period, nullptr));
            }
            ...
            if (++counter < conn->get_endpoint_count()) continue;
            counter = 0;
...
public:
    PushAllCR(CephContext* cct, RGWHTTPManager* http, RGWPeriod&& period, std::map<std::string, RGWRESTConn>&& conns)
        : RGWCoroutine(cct)
        , http(http)
...
int PushAllCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        yield
        {
            ...
            for (auto& c : conns) spawn(new PushAndRetryCR(cct, c.first, &c.second, http, period), false);
        }
...
public:
    CRThread(CephContext* cct, RGWPeriod&& period, std::map<std::string, RGWRESTConn>&& conns)
        : cct(cct)
        , coroutines(cct, NULL)
...
RGWPeriodPusher::RGWPeriodPusher(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, optional_yield y)
    : cct(driver->ctx())
    , driver(driver)
{
    ...
    // XXX dang
    int r = period.init(
        dpp, cct, static_cast<rgw::sal::RadosStore*>(driver)->svc()->sysobj, realm_id, y, zone->get_realm_name());
    if (r < 0) {
...
void RGWPeriodPusher::handle_notify(RGWRealmNotify type, bufferlist::const_iterator& p)
{
    // decode the period
...
void RGWPeriodPusher::handle_notify(RGWZonesNeedPeriod&& period)
{
    if (period.get_realm_epoch() < realm_epoch) {
        ldout(cct, 10) << "period's realm epoch " << period.get_realm_epoch()
                       << " is not newer than current realm epoch " << realm_epoch << ", discarding update" << dendl;
        return;
    }
    if (period.get_realm_epoch() == realm_epoch && period.get_epoch() <= period_epoch) {
    ...
    if (my_zonegroup.master_zone != driver->get_zone()->get_id()) return;
    ...
    if (period.get_map().master_zonegroup == driver->get_zone()->get_zonegroup().get_id()) {
        ...
        for (auto& zg : zonegroups) {
            ...
            if (zonegroup.get_id() == driver->get_zone()->get_zonegroup().get_id()) continue;
            if (zonegroup.endpoints.empty()) continue;
    ...
    for (auto& z : my_zonegroup.zones) {
        auto& zone = z.second;
        if (zone.id == driver->get_zone()->get_id()) continue;
        if (zone.endpoints.empty()) continue;

        hint = conns.emplace_hint(hint,
                                  std::piecewise_construct,
    ...
    ldout(cct, 4) << "Zone master pushing period " << period.get_id() << " epoch " << period_epoch << " to "
