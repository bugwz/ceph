...
RGWPeriodPusher::RGWPeriodPusher(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, optional_yield y)
    : cct(store->ctx())
    , store(store)
{
...
void RGWPeriodPusher::handle_notify(RGWZonesNeedPeriod&& period)
{
    ...
    if (my_zonegroup.master_zone != store->svc()->zone->get_zone_params().get_id()) return;
    ...
    if (period.get_map().master_zonegroup == store->svc()->zone->get_zonegroup().get_id()) {
        ...
        for (auto& zg : zonegroups) {
            ...
            if (zonegroup.get_id() == store->svc()->zone->get_zonegroup().get_id()) continue;
    ...
    for (auto& z : my_zonegroup.zones) {
        auto& zone = z.second;
        if (zone.id == store->svc()->zone->get_zone_params().get_id()) continue;
