...
RGWSyncErrorLogger::RGWSyncErrorLogger(rgw::sal::RGWRadosStore* _store, const string& oid_prefix, int _num_shards)
    : store(_store)
...
void RGWMetaSyncEnv::init(const DoutPrefixProvider* _dpp, CephContext* _cct, rgw::sal::RGWRadosStore* _store,
...
public:
    ...
    RGWAsyncReadMDLogEntries(const DoutPrefixProvider* _dpp, RGWCoroutine* caller, RGWAioCompletionNotifier* cn,
                             rgw::sal::RGWRadosStore* _store, RGWMetadataLog* mdlog, int _shard_id, std::string _marker,
                             int _max_entries)
...
#define NUM_TRANSIENT_ERROR_RETRIES 10
        ...
        for (tries = 0; tries < NUM_TRANSIENT_ERROR_RETRIES; tries++) {
            ...
            if (tries < NUM_TRANSIENT_ERROR_RETRIES - 1) {
                ldpp_dout(dpp, 20) << *this << ": failed to fetch remote metadata: " << section << ":" << key
                                   << ", will retry" << dendl;
                continue;
            }
            ...
            if (sync_status < 0) {
                tn->log(10,
                        SSTR("failed to send read remote metadata entry: section=" << section << " key=" << key
                                                                                   << " status=" << sync_status));
                log_error() << "failed to send read remote metadata entry: section=" << section << " key=" << key
        ...
        for (tries = 0; tries < NUM_TRANSIENT_ERROR_RETRIES; tries++) {
            ...
            if (tries < NUM_TRANSIENT_ERROR_RETRIES - 1) {
                ldpp_dout(dpp, 20) << *this << ": failed to store metadata: " << section << ":" << key
                                   << ", got retcode=" << retcode << dendl;
                continue;
            }
            break;
        }
...
#define INCREMENTAL_MAX_ENTRIES 100
                ldpp_dout(sync_env->dpp, 20)
                    ...
                    << " sync_marker.marker=" << sync_marker.marker << " period_marker=" << period_marker << dendl;
...
public:
    ...
    {
        rgw::sal::RGWRadosStore* store = sync_env->store;
        return new RGWSimpleRadosReadCR<rgw_meta_sync_marker>(sync_env->dpp,
                                                              sync_env->async_rados,
                                                              store->svc()->sysobj,
                                                              rgw_raw_obj(pool, sync_env->shard_obj_name(shard_id)),
                                                              &sync_marker);
    ...
    int operate(const DoutPrefixProvider* dpp) override
    {
        reenter(this)
        {
            ...
            for (;;) {
                ...
                yield call(new RGWSimpleRadosWriteCR<rgw_meta_sync_info>(dpp,
                                                                         sync_env->async_rados,
                                                                         sync_env->store->svc()->sysobj,
...
static RGWPeriodHistory::Cursor get_period_at(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
