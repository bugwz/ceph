...
#define RGW_RWMODE (RGW_RWXMODE & ~(S_IXUSR | S_IXGRP | S_IXOTH))


namespace rgw {

template<typename T> static inline void ignore(T&&) {}
...
struct fh_key
{
    ...
    fh_key(const std::string& _b, const std::string& _o, const std::string& _t /* tenant */)
        : version(0)
    {
        std::string tb = _t + ":" + _b;
        std::string to = _t + ":" + _o;
...
inline bool operator<(const fh_key& lhs, const fh_key& rhs)
{
    return ((lhs.fh_hk.bucket < rhs.fh_hk.bucket) ||
            ((lhs.fh_hk.bucket == rhs.fh_hk.bucket) && (lhs.fh_hk.object < rhs.fh_hk.object)));
}
...
inline bool operator==(const fh_key& lhs, const fh_key& rhs)
{
    return ((lhs.fh_hk.bucket == rhs.fh_hk.bucket) && (lhs.fh_hk.object == rhs.fh_hk.object));
}
...
#define CREATE_FLAGS(x) ((x) & ~(RGWFileHandle::FLAG_CREATE | RGWFileHandle::FLAG_LOCK))
    ...
    static constexpr uint32_t RCB_MASK = RGW_SETATTR_MTIME | RGW_SETATTR_CTIME | RGW_SETATTR_ATIME | RGW_SETATTR_SIZE;
...
private:
    ...
    uint64_t init_fsid(std::string& uid) { return XXH64(uid.c_str(), uid.length(), fh_key::seed); }

    void init_rootfs(std::string& fsid, const std::string& object_name, bool is_bucket)
    {
        /* fh_key */
        fh.fh_hk.bucket = XXH64(fsid.c_str(), fsid.length(), fh_key::seed);
        fh.fh_hk.object = XXH64(object_name.c_str(), object_name.length(), fh_key::seed);
        fhk = fh.fh_hk;
        name = object_name;
    ...
    void decode(bufferlist::const_iterator& bl)
    {
        ...
        uint32_t fh_type;
        decode(fh_type, bl);
        if ((fh.fh_type != fh_type) && (fh_type == RGW_FS_TYPE_SYMBOLIC_LINK)) fh.fh_type = RGW_FS_TYPE_SYMBOLIC_LINK;
        decode(state.dev, bl);
...
public:
    RGWFileHandle(RGWLibFS* _fs, RGWFileHandle* _parent, const fh_key& _fhk, std::string& _name, uint32_t _flags)
        : fs(_fs)
        , bucket(nullptr)
        , parent(_parent)
        , file_ondisk_version(-1)
        ...
        , flags(_flags)
    {
        ...
        case RGW_FS_TYPE_DIRECTORY:
            ...
            advance_mtime();
            break;
        case RGW_FS_TYPE_FILE: state.unix_mode = RGW_RWMODE | S_IFREG; break;
        case RGW_FS_TYPE_SYMBOLIC_LINK: state.unix_mode = RGW_RWMODE | S_IFLNK; break;
        default: break;
        }
    ...
    const fh_key& get_key() const { return fhk; }

    directory* get_directory() { return boost::get<directory>(&variant_type); }

    size_t get_size() const { return state.size; }

    const char* stype() { return is_dir() ? "DIR" : "FILE"; }
    ...
    {
        if (mask & RGW_SETATTR_UID) state.owner_uid = st->st_uid;

        if (mask & RGW_SETATTR_GID) state.owner_gid = st->st_gid;

        if (mask & RGW_SETATTR_MODE) {
            switch (fh.fh_type) {
            case RGW_FS_TYPE_DIRECTORY: state.unix_mode = st->st_mode | S_IFDIR; break;
            case RGW_FS_TYPE_FILE: state.unix_mode = st->st_mode | S_IFREG; break;
            case RGW_FS_TYPE_SYMBOLIC_LINK: state.unix_mode = st->st_mode | S_IFLNK; break;
            default: break;
            }
        }

        if (mask & RGW_SETATTR_ATIME) state.atime = st->st_atim;

        if (mask & RGW_SETATTR_MTIME) {
            if (fh.fh_type != RGW_FS_TYPE_DIRECTORY) state.mtime = st->st_mtim;
        }

        if (mask & RGW_SETATTR_CTIME) state.ctime = st->st_ctim;
    }
    ...
    {
        if (is_root()) return root_name;
        if (is_bucket()) return name;
    ...
    std::string full_object_name(bool omit_bucket = false) const
    {
        ...
        for (auto& s : boost::adaptors::reverse(segments)) {
            if (pos > 1) {
                ...
                if (!omit_bucket && ((path.length() == 0) || (path.front() != '/'))) path += "/";
            }
            path += *s;
            ++pos;
        }
    ...
    inline std::string relative_object_name() const { return full_object_name(true /* omit_bucket */); }
    ...
    inline std::string format_child_name(const std::string& cbasename, bool is_dir) const
    {
        std::string child_name{relative_object_name()};
        if ((child_name.size() > 0) && (child_name.back() != '/')) child_name += "/";
    ...
    void add_marker(uint64_t off, const rgw_obj_key& marker, uint8_t obj_type)
    {
        using std::get;
    ...
    int readdir(rgw_readdir_cb rcb, void* cb_arg, readdir_offset offset, bool* eof, uint32_t flags);
    ...
    void inc_nlink(const uint64_t n) { state.nlink += n; }

    void set_nlink(const uint64_t n) { state.nlink = n; }

    void set_size(const size_t size) { state.size = size; }
    ...
    void set_times(real_time t) { set_times(real_clock::to_timespec(t)); }

    void set_ctime(const struct timespec& ts) { state.ctime = ts; }

    void set_mtime(const struct timespec& ts) { state.mtime = ts; }

    void set_atime(const struct timespec& ts) { state.atime = ts; }

    void set_etag(const ceph::buffer::list& _etag) { etag = _etag; }

    void set_acls(const ceph::buffer::list& _acls) { acls = _acls; }

    void encode_attrs(ceph::buffer::list& ux_key1, ceph::buffer::list& ux_attrs1, bool inc_ov = true);

    DecodeAttrsResult decode_attrs(const ceph::buffer::list* ux_key1, const ceph::buffer::list* ux_attrs1);
    ...
    struct FhLT
    {
        ...
        bool operator()(const fh_key& k, const RGWFileHandle& fh) const { return k < fh.get_key(); }

        bool operator()(const RGWFileHandle& fh, const fh_key& k) const { return fh.get_key() < k; }
    };
    ...
    typedef bi::member_hook<RGWFileHandle, tree_hook_type, &RGWFileHandle::fh_hook> FhHook;
...
#endif
    typedef cohort::lru::TreeX<RGWFileHandle, FhTree, FhLT, FhEQ, fh_key, std::mutex> FHCache;

    ~RGWFileHandle() override;

    friend std::ostream& operator<<(std::ostream& os, RGWFileHandle const& rgw_fh);
    ...
    public:
        ...
        Factory(RGWLibFS* _fs, RGWFileHandle* _parent, const fh_key& _fhk, std::string& _name, uint32_t _flags)
            : fs(_fs)
            , parent(_parent)
            , fhk(_fhk)
            , name(_name)
        ...
        cohort::lru::Object* alloc() override { return new RGWFileHandle(fs, parent, fhk, name, flags); }
    }; /* Factory */
...
inline enum rgw_fh_type fh_type_of(uint32_t flags)
{
    ...
    case RGW_LOOKUP_FLAG_DIR: fh_type = RGW_FS_TYPE_DIRECTORY; break;
    case RGW_LOOKUP_FLAG_FILE: fh_type = RGW_FS_TYPE_FILE; break;
    default: fh_type = RGW_FS_TYPE_NIL;
    };
    return fh_type;
}
...
class RGWLibFS
{
    ...
    friend std::ostream& operator<<(std::ostream& os, RGWLibFS::event const& ev);
    ...
    struct WriteCompletion
    {
        ...
        explicit WriteCompletion(RGWFileHandle& _fh)
            : rgw_fh(_fh)
        {
    ...
    struct State
    {
        ...
        void push_event(const event& ev) { events.push_back(ev); }
    } state;

    uint32_t new_inst() { return ++fs_inst_counter; }
...
public:
    ...
    RGWLibFS(CephContext* _cct, const char* _uid, const char* _user_id, const char* _key, const char* root)
        : cct(_cct)
        , root_fh(this)
        ...
        , shutdown(false)
        , refcnt(1)
        , fh_cache(cct->_conf->rgw_nfs_fhcache_partitions, cct->_conf->rgw_nfs_fhcache_size)
        , fh_lru(cct->_conf->rgw_nfs_lru_lanes, cct->_conf->rgw_nfs_lru_lane_hiwat)
        , uid(_uid)
        , key(_user_id, _key)
    {
    ...
    friend void intrusive_ptr_add_ref(const RGWLibFS* fs) { fs->refcnt.fetch_add(1, std::memory_order_relaxed); }
    ...
    inline void rele() { intrusive_ptr_release(this); }
    ...
    void release_evict(RGWFileHandle* fh)
    {
        ...
        fh_cache.remove(fh->fh.fh_hk.object, fh, RGWFileHandle::FHCache::FLAG_LOCK);
    ...
    int authorize(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver)
    {
        ...
        if (ret == 0) {
            ...
            if (!k || (k->key != key.key)) return -EINVAL;
            if (user->get_info().suspended) return -ERR_USER_SUSPENDED;
        }
        else {
                ...
                if (user->load_user(dpp, null_yield) < 0) {
                    ...
                    if (ret < 0) {
                        lsubdout(get_context(), rgw, 10)
                            << "NOTICE: failed to driver new user's info: ret=" << ret << dendl;
                    }
                }
            } /* auth success */
        }
    ...
    LookupFHResult lookup_fh(const fh_key& fhk, const uint32_t flags = RGWFileHandle::FLAG_NONE)
    {
        using std::get;
    ...
    retry:
        RGWFileHandle* fh = fh_cache.find_latch(fhk.fh_hk.object /* partition selector*/,
                                                fhk /* key */,
                                                lat /* serializer */,
                                                RGWFileHandle::FHCache::FLAG_LOCK);
        /* LATCHED */
        if (fh) {
            if (likely(!fh_locked)) fh->mtx.lock();   // XXX !RAII because may-return-LOCKED
            /* need initial ref from LRU (fast path) */
            if (!fh_lru.ref(fh, cohort::lru::FLAG_INITIAL)) {
                lat.lock->unlock();
                if (likely(!fh_locked)) fh->mtx.unlock();
                goto retry; /* !LATCHED */
            }
            /* LATCHED, LOCKED */
            if (!(flags & RGWFileHandle::FLAG_LOCK)) fh->mtx.unlock(); /* ! LOCKED */
        }
        lat.lock->unlock(); /* !LATCHED */
        get<0>(fhr) = fh;
        if (fh) {
            lsubdout(get_context(), rgw, 17) << __func__ << " 1 " << *fh << dendl;
        }
        return fhr;
    } /* lookup_fh(const fh_key&) */

    /* find or create an RGWFileHandle */
    LookupFHResult lookup_fh(RGWFileHandle* parent, const char* name, const uint32_t flags = RGWFileHandle::FLAG_NONE)
    {
        using std::get;
        ...
        /* mount is stale? */
        if (state.flags & FLAG_CLOSED) return fhr;
        ...
        lsubdout(get_context(), rgw, 10) << __func__ << " called on " << parent->object_name() << " for " << key_name
                                         << " (" << obj_name << ")"
                                         << " -> " << fhk << dendl;
    ...
    retry:
        ...
        if (fh) {
            ...
            if (!(flags & RGWFileHandle::FLAG_LOCK))
                if (likely(!fh_locked)) fh->mtx.unlock(); /* ! LOCKED */
        }
        else {
            /* make or re-use handle */
            RGWFileHandle::Factory prototype(this, parent, fhk, obj_name, CREATE_FLAGS(flags));
            uint32_t iflags{cohort::lru::FLAG_INITIAL};
            fh = static_cast<RGWFileHandle*>(fh_lru.insert(&prototype, cohort::lru::Edge::MRU, iflags));
            if (fh) {
                /* lock fh (LATCHED) */
                if (flags & RGWFileHandle::FLAG_LOCK) fh->mtx.lock();
    ...
    out:
        get<0>(fhr) = fh;
        if (fh) {
            lsubdout(get_context(), rgw, 17) << __func__ << " 2 " << *fh << dendl;
        }
        return fhr;
    ...
    int setattr(RGWFileHandle* rgw_fh, struct stat* st, uint32_t mask, uint32_t flags);

    int getxattrs(RGWFileHandle* rgw_fh, rgw_xattrlist* attrs, rgw_getxattr_cb cb, void* cb_arg, uint32_t flags);

    int lsxattrs(RGWFileHandle* rgw_fh, rgw_xattrstr* filter_prefix, rgw_getxattr_cb cb, void* cb_arg, uint32_t flags);
    ...
    LookupFHResult stat_bucket(RGWFileHandle* parent, const char* path, RGWLibFS::BucketStats& bs, uint32_t flags);

    LookupFHResult fake_leaf(RGWFileHandle* parent, const char* path, enum rgw_fh_type type = RGW_FS_TYPE_NIL,
                             struct stat* st = nullptr, uint32_t mask = 0, uint32_t flags = RGWFileHandle::FLAG_NONE);

    LookupFHResult stat_leaf(RGWFileHandle* parent, const char* path, enum rgw_fh_type type = RGW_FS_TYPE_NIL,
                             uint32_t flags = RGWFileHandle::FLAG_NONE);

    int read(RGWFileHandle* rgw_fh, uint64_t offset, size_t length, size_t* bytes_read, void* buffer, uint32_t flags);

    int readlink(RGWFileHandle* rgw_fh, uint64_t offset, size_t length, size_t* bytes_read, void* buffer,
                 uint32_t flags);

    int rename(RGWFileHandle* old_fh, RGWFileHandle* new_fh, const char* old_name, const char* new_name);

    MkObjResult create(RGWFileHandle* parent, const char* name, struct stat* st, uint32_t mask, uint32_t flags);

    MkObjResult symlink(RGWFileHandle* parent, const char* name, const char* link_path, struct stat* st, uint32_t mask,
                        uint32_t flags);

    MkObjResult mkdir(RGWFileHandle* parent, const char* name, struct stat* st, uint32_t mask, uint32_t flags);
    ...
    RGWFileHandle* lookup_handle(struct rgw_fh_hk fh_hk)
    {
        ...
        if (state.flags & FLAG_CLOSED) return nullptr;
    ...
    retry:
        ...
        if (!fh) {
            ...
            lsubdout(get_context(), rgw, 0) << __func__ << " handle lookup failed " << fhk << dendl;
            goto out;
        }
        fh->mtx.lock();
        if (fh->flags & RGWFileHandle::FLAG_DELETED) {
...
static inline std::string make_uri(const std::string& bucket_name, const std::string& object_name)
{
    std::string uri("/");
...
class RGWListBucketsRequest : public RGWLibRequest, public RGWListBuckets /* RGWOp */
{
public:
    RGWFileHandle* rgw_fh;
    ...
    RGWListBucketsRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, RGWFileHandle* _rgw_fh,
                          rgw_readdir_cb _rcb, void* _cb_arg, RGWFileHandle::readdir_offset& _offset)
        : RGWLibRequest(_cct, std::move(_user))
        , rgw_fh(_rgw_fh)
        , offset(_offset)
        , cb_arg(_cb_arg)
        , rcb(_rcb)
    ...
    void send_response_begin(bool has_buckets) override { sent_data = true; }
        ...
        for (const auto& iter : m) {
            ...
            if (!this->operator()(ent->get_name(), marker)) {
                ...
                lsubdout(cct, rgw, 5) << "ListBuckets rcb failed"
                                      << " dirent=" << ent->get_name() << " call count=" << ix << dendl;
                rcb_eof = true;
                return;
            }
            ++ix;
    ...
    int operator()(const std::string_view& name, const std::string_view& marker)
    {
        ...
        rgw_fh->add_marker(off, rgw_obj_key{marker.data(), ""}, RGW_FS_TYPE_DIRECTORY);
        ++d_count;
    ...
    bool eof()
    {
        ...
        if (unlikely(cct->_conf->subsys.should_gather(ceph_subsys_rgw, 15))) {
            bool is_offset = unlikely(!get<const char*>(&offset)) || !!get<const char*>(offset);
            lsubdout(cct, rgw, 15) << "READDIR offset: " << ((is_offset) ? offset : "(nil)")
...
class RGWReaddirRequest : public RGWLibRequest, public RGWListBucket /* RGWOp */
{
public:
    RGWFileHandle* rgw_fh;
    RGWFileHandle::readdir_offset offset;
    ...
    RGWReaddirRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, RGWFileHandle* _rgw_fh,
                      rgw_readdir_cb _rcb, void* _cb_arg, RGWFileHandle::readdir_offset& _offset)
        : RGWLibRequest(_cct, std::move(_user))
        , rgw_fh(_rgw_fh)
        , offset(_offset)
    ...
    int operator()(const std::string_view name, const rgw_obj_key& marker, const ceph::real_time& t, const uint64_t fsz,
                   uint8_t type)
    {
        ...
        return rcb(name.data(),
                   cb_arg,
                   off,
                   &st,
                   RGWFileHandle::RCB_MASK,
                   (type == RGW_FS_TYPE_DIRECTORY) ? RGW_LOOKUP_FLAG_DIR : RGW_LOOKUP_FLAG_FILE);
    }
    ...
    void send_response() override
    {
        ...
        public:
            DirIterator(std::vector<rgw_bucket_dir_entry>& objs, std::map<string, bool>& common_prefixes)
                : objs(objs)
            ...
            bool is_obj() { return (obj_iter != objs.end()); }

            bool is_cp() { return (cp_iter != common_prefixes.end()); }

            bool eof() { return ((!is_obj()) && (!is_cp())); }
            ...
            void parse_obj()
            {
                if (is_obj()) {
                    ...
                    if (last_del != string::npos) sref.remove_prefix(last_del + 1);
                    obj_sref = sref;
                }
            } /* parse_obj */
            ...
            void parse_cp()
            {
                if (is_cp()) {
                    ...
                    if (cp_iter->first.back() == '/') const_cast<std::string&>(cp_iter->first).pop_back();
            ...
            bool skip_cp() { return _skip_cp; }

            bool entry_is_obj() { return (is_obj() && ((!is_cp()) || (obj_sref.get() < cp_sref.get()))); }

            std::string_view get_obj_sref() { return obj_sref.get(); }

            std::string_view get_cp_sref() { return cp_sref.get(); }

            std::vector<rgw_bucket_dir_entry>::iterator& get_obj_iter() { return obj_iter; }

            std::map<string, bool>::iterator& get_cp_iter() { return cp_iter; }
        ...
        for (;;) {
            ...
            if (di.entry_is_obj()) {
                ...
                if (sref.empty()) {
                    ...
                    lsubdout(cct, rgw, 15) << "RGWReaddirRequest " << __func__ << " "
                                           << "list uri=" << state->relative_uri << " "
                                           << " prefix=" << prefix << " "
                                           << " obj path=" << obj_entry.key.name << " (" << sref << ")"
                                           << ""
                                           << " mtime=" << real_clock::to_time_t(obj_entry.meta.mtime)
                                           << " size=" << obj_entry.meta.accounted_size << dendl;
                    ...
                    if (!this->operator()(
                        ...
                        lsubdout(cct, rgw, 5) << "readdir rcb caller signalled stop"
                                              << " dirent=" << sref.data() << " call count=" << ix << dendl;
                        rcb_eof = true;
                        return;
                    }
                }
                ...
                if (!di.skip_cp()) {
                    ...
                    if (sref.empty()) {
                        ...
                        if (!this->operator()(sref, next_marker, cnow, 0, RGW_FS_TYPE_DIRECTORY)) {
                            /* caller cannot accept more */
                            lsubdout(cct, rgw, 5) << "readdir rcb caller signalled stop"
                                                  << " dirent=" << sref.data() << " call count=" << ix << dendl;
                            rcb_eof = true;
    ...
    virtual void send_versioned_response() { send_response(); }
    ...
    bool eof()
    {
        ...
        if (unlikely(cct->_conf->subsys.should_gather(ceph_subsys_rgw, 15))) {
            bool is_offset = unlikely(!get<const char*>(&offset)) || !!get<const char*>(offset);
            lsubdout(cct, rgw, 15) << "READDIR offset: " << ((is_offset) ? offset : "(nil)")
                                   << " next marker: " << next_marker << " is_truncated: " << is_truncated << dendl;
        }
        return !is_truncated && !rcb_eof;
    }
...
class RGWRMdirCheck : public RGWLibRequest, public RGWListBucket /* RGWOp */
{
public:
    ...
    RGWRMdirCheck(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, const RGWFileHandle* _rgw_fh)
        : RGWLibRequest(_cct, std::move(_user))
        , rgw_fh(_rgw_fh)
        , valid(false)
        , has_children(false)
    ...
    {
        valid = true;
        if ((objs.size() > 1) || (!objs.empty() && (objs.front().key.name != prefix))) {
            has_children = true;
            return;
        }
...
class RGWCreateBucketRequest : public RGWLibRequest, public RGWCreateBucket /* RGWOp */
{
public:
    const std::string& bucket_name;

    RGWCreateBucketRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, std::string& _bname)
        : RGWLibRequest(_cct, std::move(_user))
        , bucket_name(_bname)
    {
        op = this;
...
class RGWDeleteBucketRequest : public RGWLibRequest, public RGWDeleteBucket /* RGWOp */
{
public:
    const std::string& bucket_name;

    RGWDeleteBucketRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, std::string& _bname)
        : RGWLibRequest(_cct, std::move(_user))
        , bucket_name(_bname)
    {
        op = this;
...
/*
  put object
*/
class RGWPutObjRequest : public RGWLibRequest, public RGWPutObj /* RGWOp */
{
public:
    const std::string& bucket_name;
    ...
    RGWPutObjRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, const std::string& _bname,
                     const std::string& _oname, buffer::list& _bl)
        : RGWLibRequest(_cct, std::move(_user))
        , bucket_name(_bname)
        , obj_name(_oname)
        , bl(_bl)
    ...
    {
        if (bl.length() > cct->_conf->rgw_max_put_size) return -ERR_TOO_LARGE;
        return 0;
    }
...
class RGWReadRequest : public RGWLibRequest, public RGWGetObj /* RGWOp */
{
public:
    RGWFileHandle* rgw_fh;
    void* ulp_buffer;
    ...
    RGWReadRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, RGWFileHandle* _rgw_fh, uint64_t off,
                   uint64_t len, void* _ulp_buffer)
        : RGWLibRequest(_cct, std::move(_user))
        , rgw_fh(_rgw_fh)
    ...
    int header_init() override
    {
        ...
        /* XXX derp derp derp */
        state->relative_uri = make_uri(rgw_fh->bucket_name(), rgw_fh->relative_object_name());
    ...
    int send_response_data(ceph::buffer::list& bl, off_t bl_off, off_t bl_len) override
    {
        size_t bytes;
...
class RGWDeleteObjRequest : public RGWLibRequest, public RGWDeleteObj /* RGWOp */
{
public:
    const std::string& bucket_name;
    const std::string& obj_name;

    RGWDeleteObjRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, const std::string& _bname,
...
class RGWStatObjRequest : public RGWLibRequest, public RGWGetObj /* RGWOp */
{
public:
    const std::string& bucket_name;
    const std::string& obj_name;
    ...
    RGWStatObjRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, const std::string& _bname,
                      const std::string& _oname, uint32_t _flags)
        : RGWLibRequest(_cct, std::move(_user))
        , bucket_name(_bname)
        , obj_name(_oname)
        , _size(0)
    ...
    real_time get_mtime() const { return lastmod; }
    ...
    int send_response_data(ceph::buffer::list& _bl, off_t s_off, off_t e_off) override
    {
        /* NOP */
        /* XXX save attrs? */
        return 0;
...
class RGWStatBucketRequest : public RGWLibRequest, public RGWStatBucket /* RGWOp */
{
public:
    std::string uri;
    ...
    RGWStatBucketRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, const std::string& _path,
    ...
    real_time get_ctime() const { return bucket->get_creation_time(); }
    ...
    bool matched() { return (bucket->get_name().length() > 0); }

}; /* RGWStatBucketRequest */

class RGWStatLeafRequest : public RGWLibRequest, public RGWListBucket /* RGWOp */
{
public:
    RGWFileHandle* rgw_fh;
    std::string path;
    ...
    RGWStatLeafRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, RGWFileHandle* _rgw_fh,
    ...
    void send_response() override
    {
        ...
        for (const auto& iter : objs) {
            ...
            lsubdout(cct, rgw, 15) << "RGWStatLeafRequest " << __func__ << " "
                                   << "list uri=" << state->relative_uri << " "
                                   << " prefix=" << prefix << " "
                                   << " obj path=" << name << ""
                                   << " target = " << path << "" << dendl;
...
class RGWWriteRequest : public RGWLibContinuedReq, public RGWPutObj /* RGWOp */
{
public:
    const std::string& bucket_name;
    const std::string& obj_name;
    ...
    RGWWriteRequest(rgw::sal::Driver* driver, const RGWProcessEnv& penv, std::unique_ptr<rgw::sal::User> _user,
    ...
    int verify_params() override { return 0; }
}; /* RGWWriteRequest */

/*
  copy object
*/
class RGWCopyObjRequest : public RGWLibRequest, public RGWCopyObj /* RGWOp */
{
public:
    ...
    RGWCopyObjRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, RGWFileHandle* _src_parent,
                      RGWFileHandle* _dst_parent, const std::string& _src_name, const std::string& _dst_name)
        : RGWLibRequest(_cct, std::move(_user))
        , src_parent(_src_parent)
        , dst_parent(_dst_parent)
        , src_name(_src_name)
        , dst_name(_dst_name)
    {
...
class RGWGetAttrsRequest : public RGWLibRequest, public RGWGetAttrs /* RGWOp */
{
public:
    const std::string& bucket_name;
    const std::string& obj_name;

    RGWGetAttrsRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, const std::string& _bname,
                       const std::string& _oname)
        : RGWLibRequest(_cct, std::move(_user))
        , RGWGetAttrs()
        , bucket_name(_bname)
    ...
    const flat_map<std::string, std::optional<buffer::list>>& get_attrs() { return attrs; }
...
class RGWSetAttrsRequest : public RGWLibRequest, public RGWSetAttrs /* RGWOp */
{
public:
    const std::string& bucket_name;
    const std::string& obj_name;

    RGWSetAttrsRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, const std::string& _bname,
                       const std::string& _oname)
        : RGWLibRequest(_cct, std::move(_user))
        , bucket_name(_bname)
        , obj_name(_oname)
    ...
    const std::map<std::string, buffer::list>& get_attrs() { return attrs; }
...
class RGWRMAttrsRequest : public RGWLibRequest, public RGWRMAttrs /* RGWOp */
{
public:
    const std::string& bucket_name;
    const std::string& obj_name;

    RGWRMAttrsRequest(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, const std::string& _bname,
                      const std::string& _oname)
        : RGWLibRequest(_cct, std::move(_user))
        , RGWRMAttrs()
        , bucket_name(_bname)
    ...
    const rgw::sal::Attrs& get_attrs() { return attrs; }
 ...
 */
class RGWGetClusterStatReq : public RGWLibRequest, public RGWGetClusterStat
{
public:
    struct rados_cluster_stat_t& stats_req;
    RGWGetClusterStatReq(CephContext* _cct, std::unique_ptr<rgw::sal::User> _user, rados_cluster_stat_t& _stats)
        : RGWLibRequest(_cct, std::move(_user))
        , stats_req(_stats)
    {
        op = this;
