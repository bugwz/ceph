...
bool rgw_sync_pipe_filter_tag::operator==(const string& s) const
{
    ...
    return s.compare(0, pos, s) == 0 && s.compare(pos + 1, s.size() - pos - 1, value) == 0;
}
...
void rgw_sync_pipe_filter::set_prefix(std::optional<std::string> opt_prefix, bool prefix_rm)
{
    if (opt_prefix) {
        prefix = *opt_prefix;
    }
...
void rgw_sync_pipe_filter::set_tags(std::list<std::string>& tags_add, std::list<std::string>& tags_rm)
{
    for (auto& t : tags_rm) {
...
void rgw_sync_bucket_entities::set_bucket(std::optional<string> tenant, std::optional<string> bucket_name,
                                          std::optional<string> bucket_id)
{
    ...
    if (bucket->tenant.empty() && bucket->name.empty() && bucket->bucket_id.empty()) {
        bucket.reset();
    }
}

void rgw_sync_bucket_entities::remove_bucket(std::optional<string> tenant, std::optional<string> bucket_name,
                                             std::optional<string> bucket_id)
{
    if (!bucket) {
        return;
...
void rgw_sync_bucket_pipes::get_potential_related_buckets(const rgw_bucket& bucket, std::set<rgw_bucket>* sources,
...
bool rgw_sync_data_flow_group::find_or_create_directional(const rgw_zone_id& source_zone, const rgw_zone_id& dest_zone,
...
void rgw_sync_policy_group::get_potential_related_buckets(const rgw_bucket& bucket, std::set<rgw_bucket>* sources,
                                                          std::set<rgw_bucket>* dests) const
{
    for (auto& pipe : pipes) {
...
void rgw_sync_policy_info::get_potential_related_buckets(const rgw_bucket& bucket, std::set<rgw_bucket>* sources,
                                                         std::set<rgw_bucket>* dests) const
{
