...
static void init_bucket(rgw_bucket* b, const char* t, const char* n, const char* dp, const char* ip, const char* m,
                        const char* id)
{
    b->tenant = t;
    b->name = n;
...
void RGWBucketInfo::generate_test_instances(list<RGWBucketInfo*>& o)
{
    ...
    auto gen_layout = [](rgw::BucketLayout& l) {
        ...
        l.logs.push_back(log_layout_from_index(l.current_index.gen, l.current_index.layout.normal));
    };
