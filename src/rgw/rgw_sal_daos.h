...
inline void DebugBreak()
{
    ...
    if (IsDebuggerAttached()) raise(SIGINT);   // breaks into GDB and stops, can be continued
#endif                                         // DEBUG
}

inline int NotImplementedLog(const DoutPrefixProvider* ldpp, const char* filename, int linenumber,
...
inline int NotImplementedGdbBreak(const DoutPrefixProvider* ldpp, const char* filename, int linenumber,
                                  const char* functionname)
{
...
#define ldpp_dout(dpp, v)                                                                                \
    ...
    dout_impl(pdpp->get_cct(), ceph::dout::need_dynamic(pdpp->get_subsys()), v) pdpp->gen_prefix(*_dout) \
...
public:
    DaosUser(DaosStore* _st, const rgw_user& _u)
        : StoreUser(_u)
        , store(_st)
    {}
    DaosUser(DaosStore* _st, const RGWUserInfo& _i)
        : StoreUser(_i)
        , store(_st)
    {}
    DaosUser(DaosStore* _st)
        : store(_st)
    {}
    DaosUser(DaosUser& _o) = default;
    DaosUser() {}

    virtual std::unique_ptr<User> clone() override { return std::make_unique<DaosUser>(*this); }
    ...
    int read_user(const DoutPrefixProvider* dpp, std::string name, DaosUserInfo* duinfo);

    std::unique_ptr<struct ds3_user_info> get_encoded_info(bufferlist& bl, obj_version& obj_ver);
...
public:
    ...
    virtual std::unique_ptr<Bucket> clone() override { return std::make_unique<DaosBucket>(*this); }
    ...
    std::unique_ptr<struct ds3_bucket_info> get_encoded_info(bufferlist& bl, ceph::real_time mtime);
...
public:
    DaosZoneGroup(DaosStore* _store)
        : store(_store)
        , group()
    {}
    ...
    virtual std::unique_ptr<ZoneGroup> clone() override { return std::make_unique<DaosZoneGroup>(store, group); }
...
public:
    ...
    virtual std::unique_ptr<Zone> clone() override { return std::make_unique<DaosZone>(store); }
...
public:
    ...
    virtual std::unique_ptr<Object> clone() override { return std::make_unique<DaosObject>(*this); }
    ...
    int get_dir_entry_attrs(const DoutPrefixProvider* dpp, rgw_bucket_dir_entry* ent, Attrs* getattrs = nullptr);
    // Set the object's dirent and attrs
    int set_dir_entry_attrs(const DoutPrefixProvider* dpp, rgw_bucket_dir_entry* ent, Attrs* setattrs = nullptr);
    ...
    DaosBucket* get_daos_bucket() { return static_cast<DaosBucket*>(get_bucket()); }
};
...
public:
    DaosAtomicWriter(const DoutPrefixProvider* dpp, optional_yield y, rgw::sal::Object* obj, DaosStore* _store,
...
public:
    DaosMultipartWriter(const DoutPrefixProvider* dpp, optional_yield y, MultipartUpload* _upload,
                        rgw::sal::Object* obj, DaosStore* _store, const rgw_user& owner,
                        const rgw_placement_rule* ptail_placement_rule, uint64_t _part_num,
                        const std::string& part_num_str)
        : StoreWriter(dpp, y)
        , store(_store)
        , upload(_upload)
