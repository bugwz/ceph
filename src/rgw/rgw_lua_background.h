...
struct RGWTable : EmptyMetaTable
{
    ...
    static int NewIndexClosure(lua_State* L)
    {
        ...
        switch (value_type) {
        case LUA_TNIL: map->erase(std::string(index)); return NO_RETURNVAL;
        case LUA_TBOOLEAN:
        ...
        default: l.unlock(); return luaL_error(L, "unsupported value type for RGW table");
        }

        if (len + strnlen(index, MAX_LUA_VALUE_SIZE) > MAX_LUA_VALUE_SIZE) {
...
public:
    Background(rgw::sal::Driver* driver, CephContext* cct, const std::string& luarocks_path,
    ...
    template<typename T> void put_table_value(const std::string& key, T value)
    {
