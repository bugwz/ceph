...
bool operator<(const std::vector<rgw_bucket_shard_sync_info>& lhs, const BucketIndexShardsManager& rhs)
{
...
int bucket_source_sync_checkpoint(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store,
                                  const RGWBucketInfo& bucket_info, const RGWBucketInfo& source_bucket_info,
                                  const rgw_sync_bucket_pipe& pipe, uint64_t latest_gen,
                                  const BucketIndexShardsManager& remote_markers, ceph::timespan retry_delay,
                                  ceph::coarse_mono_time timeout_at)
{
    ...
    while (full_status.incremental_gen < latest_gen) {
        ...
        ldout(store->ctx(), 1) << "waiting to reach latest gen " << latest_gen << ", on " << full_status.incremental_gen
                               << ".." << dendl;
        std::this_thread::sleep_until(delay_until);
...
int source_bilog_info(const DoutPrefixProvider* dpp, RGWSI_Zone* zone_svc, const rgw_sync_bucket_pipe& pipe,
                      rgw_bucket_index_marker_info& info, BucketIndexShardsManager& markers, optional_yield y)
{
    ...
    return rgw_read_remote_bilog_info(dpp, conn->second, *pipe.source.bucket, info, markers, y);
}

}   // anonymous namespace

int rgw_bucket_sync_checkpoint(const DoutPrefixProvider* dpp, rgw::sal::RadosStore* store,
                               const RGWBucketSyncPolicyHandler& policy, const RGWBucketInfo& info,
                               std::optional<rgw_zone_id> opt_source_zone, std::optional<rgw_bucket> opt_source_bucket,
                               ceph::timespan retry_delay, ceph::coarse_mono_time timeout_at)
{
    ...
    for (const auto& [source_zone_id, pipe] : policy.get_all_sources()) {
        ...
        spawn::spawn(ioctx, [&](yield_context yield) {
            ...
            int r = source_bilog_info(dpp, store->svc()->zone, entry.pipe, info, entry.remote_markers, y);
            if (r < 0) {
        ...
        spawn::spawn(ioctx, [&](yield_context yield) {
            ...
            int r = store->getRados()->get_bucket_instance_info(
                *entry.pipe.source.bucket, entry.source_bucket_info, nullptr, nullptr, y, dpp);
            if (r < 0) {
    ...
    for (const auto& e : sources) {
        int r = bucket_source_sync_checkpoint(
            dpp, store, info, e.source_bucket_info, e.pipe, e.latest_gen, e.remote_markers, retry_delay, timeout_at);
        if (r < 0) {
