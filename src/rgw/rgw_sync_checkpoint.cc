...
int bucket_source_sync_checkpoint(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                  const RGWBucketInfo& bucket_info, const RGWBucketInfo& source_bucket_info,
                                  const rgw_sync_bucket_pipe& pipe, const BucketIndexShardsManager& remote_markers,
...
int source_bilog_markers(const DoutPrefixProvider* dpp, RGWSI_Zone* zone_svc, const rgw_sync_bucket_pipe& pipe,
                         BucketIndexShardsManager& remote_markers, optional_yield y)
{
    ...
    return rgw_read_remote_bilog_info(dpp, conn->second, *pipe.source.bucket, remote_markers, y);
}

}   // anonymous namespace

int rgw_bucket_sync_checkpoint(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                               ...
                               ceph::timespan retry_delay, ceph::coarse_mono_time timeout_at)
{
    ...
    for (const auto& [source_zone_id, pipe] : policy.get_all_sources()) {
        ...
        spawn::spawn(ioctx, [&](yield_context yield) {
            ...
            int r = source_bilog_markers(dpp, store->svc()->zone, entry.pipe, entry.remote_markers, y);
            if (r < 0) {
        ...
        spawn::spawn(ioctx, [&](yield_context yield) {
            ...
            int r = store->getRados()->get_bucket_instance_info(
                obj_ctx, *entry.pipe.source.bucket, entry.source_bucket_info, nullptr, nullptr, y, dpp);
            if (r < 0) {
    ...
    for (const auto& [pipe, remote_markers, source_bucket_info] : sources) {
        int r = bucket_source_sync_checkpoint(
            dpp, store, info, source_bucket_info, pipe, remote_markers, retry_delay, timeout_at);
        if (r < 0) {
