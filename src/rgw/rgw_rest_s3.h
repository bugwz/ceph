...
public:
    ...
    int get_decrypt_filter(std::unique_ptr<RGWGetObj_Filter>* filter, RGWGetObj_Filter* cb,
...
public:
    ...
    int get_encrypt_filter(std::unique_ptr<rgw::sal::DataProcessor>* filter, rgw::sal::DataProcessor* cb) override;
    int get_decrypt_filter(std::unique_ptr<RGWGetObj_Filter>* filter, RGWGetObj_Filter* cb,
                           std::map<std::string, bufferlist>& attrs, bufferlist* manifest_bl) override;
};
...
{
public:
    RGWListBucketMultiparts_ObjStore_S3() { default_max = 1000; }
...
public:
    ...
    void send_partial_response(const rgw_obj_key& key, bool delete_marker, const std::string& marker_version_id,
                               int ret, boost::asio::deadline_timer* formatter_flush_cond) override;
...
{
public:
    static int authorize(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver,
                         const rgw::auth::StrategyRegistry& auth_registry, req_state* s, optional_yield y);
};
...
{
protected:
    bool is_usage_op() const { return s->info.args.exists("usage"); }
...
protected:
    bool is_acl_op() const { return s->info.args.exists("acl"); }
    bool is_cors_op() const { return s->info.args.exists("cors"); }
    bool is_lc_op() const { return s->info.args.exists("lifecycle"); }
    bool is_obj_update_op() const override { return is_acl_op() || is_cors_op(); }
    bool is_tagging_op() const { return s->info.args.exists("tagging"); }
    bool is_request_payment_op() const { return s->info.args.exists("requestPayment"); }
    bool is_policy_op() const { return s->info.args.exists("policy"); }
    bool is_object_lock_op() const { return s->info.args.exists("object-lock"); }
    ...
    bool is_replication_op() const { return s->info.args.exists("replication"); }
    bool is_policy_status_op() { return s->info.args.exists("policyStatus"); }
    bool is_block_public_access_op() { return s->info.args.exists("publicAccessBlock"); }
    bool is_bucket_encryption_op() { return s->info.args.exists("encryption"); }
...
protected:
    bool is_acl_op() const { return s->info.args.exists("acl"); }
    bool is_tagging_op() const { return s->info.args.exists("tagging"); }
    bool is_obj_retention_op() const { return s->info.args.exists("retention"); }
    bool is_obj_legal_hold_op() const { return s->info.args.exists("legal-hold"); }

    bool is_select_op() const { return s->info.args.exists("select-type"); }
...
public:
    explicit RGWRESTMgr_S3(bool _enable_s3website = false, bool _enable_sts = false, bool _enable_iam = false,
                           bool _enable_pubsub = false)
...
static inline bool looks_like_ip_address(const char* bucket)
{
    ...
    for (const char* b = bucket; *b; ++b) {
        if (*b == '.') {
            if (!expect_period) return false;
            ++num_periods;
            if (num_periods > 3) return false;
            expect_period = false;
        }
        else if (isdigit(*b)) {
            expect_period = true;
...
inline int valid_s3_bucket_name(const std::string& name, bool relaxed = false)
{
    ...
    if (!(isalpha(name[len - 1]) || isdigit(name[len - 1])))
        if (!relaxed) return -ERR_INVALID_BUCKET_NAME;
    ...
    if (looks_like_ip_address(name.c_str())) return -ERR_INVALID_BUCKET_NAME;
...
public:
    class VersionAbstractor
    {
        ...
        static constexpr size_t SIGNATURE_MAX_SIZE = std::max(DIGEST_SIZE_V2, DIGEST_SIZE_V4) * 2 + sizeof('\0');
    ...
    public:
        ...
        using signature_factory_t = std::function<server_signature_t(CephContext* cct, const std::string& secret_key,
        ...
        using completer_factory_t =
            std::function<rgw::auth::Completer::cmplptr_t(const boost::optional<std::string>& secret_key)>;
...
protected:
    ...
    virtual result_t authenticate(const DoutPrefixProvider* dpp, const std::string_view& access_key_id,
                                  const std::string_view& signature, const std::string_view& session_token,
                                  const string_to_sign_t& string_to_sign, const signature_factory_t& signature_factory,
                                  const completer_factory_t& completer_factory, const req_state* s,
                                  optional_yield y) const = 0;

public:
    result_t authenticate(const DoutPrefixProvider* dpp, const req_state* const s, optional_yield y) const final;
};
...
class AWSGeneralAbstractor : public AWSEngine::VersionAbstractor
{
    ...
    virtual boost::optional<std::string> get_v4_canonical_headers(const req_info& info,
...
class AWSBrowserUploadAbstractor : public AWSEngine::VersionAbstractor
{
    ...
    {
        return std::string(bl.c_str(), static_cast<std::string::size_type>(bl.length()));
    }
...
public:
    AWSSignerV4(const DoutPrefixProvider* _dpp)
        : dpp(_dpp)
        , cct(_dpp->get_cct())
    {}
    ...
    using signature_factory_t = std::function<signature_headers_t(
        const DoutPrefixProvider* dpp, const std::string& secret_key, const prepare_result_t&)>;
    ...
    static prepare_result_t prepare(const DoutPrefixProvider* dpp, const std::string& access_key_id,
                                    const string& region, const string& service, const req_info& info,
                                    const bufferlist* opt_content, bool s3_op);
};


extern AWSSignerV4::signature_headers_t gen_v4_signature(const DoutPrefixProvider* dpp,
...
protected:
    ...
    result_t authenticate(const DoutPrefixProvider* dpp, const std::string_view& access_key_id,
                          const std::string_view& signature, const std::string_view& session_token,
...
public:
    LDAPEngine(CephContext* const cct, rgw::sal::Driver* driver, const VersionAbstractor& ver_abstractor,
               const rgw::auth::RemoteApplier::Factory* const apl_factory)
    ...
    const char* get_name() const noexcept override { return "rgw::auth::s3::LDAPEngine"; }
...
public:
    LocalEngine(CephContext* const cct, rgw::sal::Driver* driver, const VersionAbstractor& ver_abstractor,
                const rgw::auth::LocalApplier::Factory* const apl_factory)
        : AWSEngine(cct, ver_abstractor)
        , driver(driver)
        , apl_factory(apl_factory)
    ...
    const char* get_name() const noexcept override { return "rgw::auth::s3::LocalEngine"; }
};
...
public:
    STSEngine(CephContext* const cct, rgw::sal::Driver* driver, const VersionAbstractor& ver_abstractor,
              const rgw::auth::LocalApplier::Factory* const local_apl_factory,
              const rgw::auth::RemoteApplier::Factory* const remote_apl_factory,
              const rgw::auth::RoleApplier::Factory* const role_apl_factory)
        : AWSEngine(cct, ver_abstractor)
    ...
    const char* get_name() const noexcept override { return "rgw::auth::s3::STSEngine"; }
};
...
public:
    ...
    const char* get_name() const noexcept override { return "rgw::auth::s3::S3AnonymousEngine"; }
};
