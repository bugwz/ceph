...
static boost::optional<Principal> parse_principal(CephContext* cct, TokenID t, string&& s)
{
    // Wildcard!
    if ((t == TokenID::AWS) && (s == "*")) {
    ...
    else if (t == TokenID::CanonicalUser) {}   // AWS and Federated ARNs
        ...
        if (auto a = ARN::parse(s)) {
            ...
            if (std::none_of(s.begin(), s.end(), [](const char& c) { return (c == ':') || (c == '/'); })) {
...
bool ParseState::do_string(CephContext* cct, const char* s, size_t l)
{
    ...
    // Top level!
    if ((w->id == TokenID::Version) && k && k->kind == TokenKind::version_key) {
    ...
    else if ((w->id == TokenID::Effect) && k && k->kind == TokenKind::effect_key) {
        ...
        if (a && (a->account.empty() || a->account == pp->tenant || a->account == "*")) {
...
const char* action_bit_string(uint64_t action)
{
    ...
    case iamModifyRole: return "iam:ModifyRole";
...
Policy::Policy(CephContext* cct, const string& tenant, const bufferlist& _text)
    : text(_text.to_str())
{
