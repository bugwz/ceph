...
int rgw_compression_info_from_attrset(const std::map<std::string, bufferlist>& attrs, bool& need_decompress,
...
public:
    RGWGetObj_Decompress(CephContext* cct_, RGWCompressionInfo* cs_info_, bool partial_content_,
...
public:
    RGWPutObj_Compress(CephContext* cct_, CompressorRef compressor, rgw::sal::DataProcessor* next)
        : Pipe(next)
        , cct(cct_)
