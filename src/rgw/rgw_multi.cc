...
bool RGWMultiPart::xml_end(const char* el)
{
    ...
    if (!num_obj || !etag_obj) return false;
...
{
    XMLObj* obj = NULL;
    if (strcmp(el, "CompleteMultipartUpload") == 0 || strcmp(el, "MultipartUpload") == 0) {
...
int list_multipart_parts(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, RGWBucketInfo& bucket_info,
                         CephContext* cct, const string& upload_id, const string& meta_oid, int num_parts, int marker,
                         map<uint32_t, RGWUploadPartInfo>& parts, int* next_marker, bool* truncated,
                         bool assume_unsorted)
{
    ...
    if (sorted_omap) {
        ...
        ret = sysobj.omap().get_vals(dpp, p, num_parts + 1, &parts_map, nullptr, null_yield);
    }
    else {
    ...
    for (i = 0, iter = parts_map.begin(); (i < num_parts || !sorted_omap) && iter != parts_map.end(); ++iter, ++i) {
        ...
        if (sorted_omap) {
            if (info.num != expected_next) {
                 ...
                 */
                return list_multipart_parts(dpp,
                                            store,
                                            bucket_info,
                                            cct,
                                            upload_id,
        ...
        if (sorted_omap || (int)info.num > marker) {
            parts[info.num] = info;
            last_num = info.num;
        }
    }
...
int list_multipart_parts(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, struct req_state* s,
                         const string& upload_id, const string& meta_oid, int num_parts, int marker,
                         map<uint32_t, RGWUploadPartInfo>& parts, int* next_marker, bool* truncated,
                         bool assume_unsorted)
{
    return list_multipart_parts(dpp,
...
int abort_multipart_upload(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, CephContext* cct,
                           RGWObjectCtx* obj_ctx, RGWBucketInfo& bucket_info, RGWMPObj& mp_obj)
{
    rgw_obj meta_obj;
...
int list_bucket_multiparts(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, RGWBucketInfo& bucket_info,
                           const std::string& prefix,
                           std::string& marker,   // in/out
                           const std::string& delim, const int& max_uploads, std::vector<rgw_bucket_dir_entry>* objs,
...
int abort_bucket_multiparts(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, CephContext* cct,
                            RGWBucketInfo& bucket_info)
{
    ...
    do {
        ret = list_bucket_multiparts(
            dpp, store, bucket_info, empty_prefix, marker, empty_delim, max, &objs, nullptr, &is_truncated);
        if (ret < 0) {
            ldpp_dout(dpp, 0) << __func__ << " ERROR : calling list_bucket_multiparts; ret=" << ret << "; bucket=\""
                              << bucket_info.bucket << "\"" << dendl;
            return ret;
        }
        ldpp_dout(dpp, 20) << __func__ << " INFO: aborting and cleaning up multipart upload(s); bucket=\""
        ...
        if (!objs.empty()) {
            ...
            for (const auto& obj : objs) {
                ...
                if (!mp.from_meta(key.name)) continue;
                ...
                if (ret < 0) {
                    ...
                    if (ret != -ENOENT && ret != -ERR_NO_SUCH_UPLOAD) {
                        ...
                        ldpp_dout(dpp, 10) << __func__
                                           ...
                                           << key.get_oid() << "\" for cleaning up" << dendl;
                    }
                }
                num_deleted++;
            }
