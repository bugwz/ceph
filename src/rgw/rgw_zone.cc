...
rgw_pool fix_zone_pool_dup(const set<rgw_pool>& pools, const string& default_prefix, const string& default_suffix,
...
void add_zone_pools(const RGWZoneParams& info, std::set<rgw_pool>& pools)
{
                       ...
                       std::string_view my_zone_id, std::set<rgw_pool>& pools)
{
    ...
    do {
        int r = cfgstore->list_zone_names(dpp, y, listing.next, zone_names, listing);
        if (r < 0) {
        ...
        for (const auto& name : listing.entries) {
            ...
            if (r < 0) {
                ldpp_dout(dpp, 0) << "failed to load zone " << name << " with " << cpp_strerror(r) << dendl;
                return r;
            }
...
static int get_zones_pool_set(const DoutPrefixProvider* dpp, CephContext* cct, RGWSI_SysObj* sysobj_svc,
                              const list<string>& zone_names, const string& my_zone_id, set<rgw_pool>& pool_names,
                              optional_yield y)
{
    for (const auto& name : zone_names) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "Error: failed to load zone " << name << " with " << cpp_strerror(-r) << dendl;
            return r;
        }
...
void RGWTierACLMapping::dump(Formatter* f) const
{
    string s;
    switch (type) {
    case ACL_TYPE_EMAIL_USER: s = "email"; break;
    case ACL_TYPE_GROUP: s = "uri"; break;
    default: s = "id"; break;
    }
...
bool RGWPeriodMap::find_zone_by_id(const rgw_zone_id& zone_id, RGWZoneGroup* zonegroup, RGWZone* zone) const
{
                         ...
                         RGWZoneParams& info)
{
    ...
    for (auto& [pname, placement] : info.placement_pools) {
        placement.index_pool =
            fix_zone_pool_dup(pools, info.name, "." + default_bucket_index_pool_suffix, placement.index_pool);
        placement.data_extra_pool =
            fix_zone_pool_dup(pools, info.name, "." + default_storage_extra_pool_suffix, placement.data_extra_pool);
                      ...
                      const rgw::zone_features::set& enable_features, const rgw::zone_features::set& disable_features)
{
    ...
    if (!zonegroup.zones.count(zone_id)) {
        for (const auto& [id, zone] : zonegroup.zones) {
            if (zone.name == zone_name) {
                ldpp_dout(dpp, 0) << "ERROR: found existing zone name " << zone_name << " (" << id << ") in zonegroup "
    ...
    if (pis_master) {
        ...
        ldpp_dout(dpp, 0) << "NOTICE: promoted " << zone_name << " as new master_zone of zonegroup " << zonegroup.name
                          << dendl;
        master_zone = zone_id;
    }
    ...
    for (const auto& rm : sync_from_rm) {
        ...
        if (i == zone.sync_from.end()) {
            ldpp_dout(dpp, 1) << "WARNING: zone \"" << rm << "\" was not in sync_from" << dendl;
            continue;
        }
    ...
    for (const auto& feature : disable_features) {
        ...
        if (i == zone.supported_features.end()) {
            ldpp_dout(dpp, 1) << "WARNING: zone feature \"" << feature << "\" was not enabled in zone " << zone.name
                              << dendl;
            continue;
        }
