...
arw::Result<FlightKey> TicketToFlightKey(const flt::Ticket& t)
{
    try {
        ...
        return arw::Status::Invalid("could not convert Ticket containing \"%s\" into a Flight Key", t.ticket);
    }
    catch (const std::out_of_range& ex) {
        return arw::Status::Invalid("could not convert Ticket containing \"%s\" into a Flight Key due to range",
                                    t.ticket);
    }
}
...
FlightData::FlightData(const std::string& _uri, const std::string& _tenant_name, const std::string& _bucket_name,
                       const rgw_obj_key& _object_key, uint64_t _num_records, uint64_t _obj_size,
                       std::shared_ptr<arw::Schema>& _schema,
                       std::shared_ptr<const arw::KeyValueMetadata>& _kv_metadata, rgw_user _user_id)
    : key(++next_flight_key)
    ,
...
arw::Result<FlightData> MemoryFlightStore::get_flight(const FlightKey& key) const
{
    ...
    auto i = map.find(key);
    if (i == map.cend()) {
        return arw::Status::KeyError("could not find Flight with Key %" PRIu32, key);
    }
    else {
        return i->second;
    }
...
FlightServer::FlightServer(RGWProcessEnv& _env, FlightStore* _flight_store, const DoutPrefix& _dp)
    : env(_env)
    , driver(env.driver)
    , dp(_dp)
...
arw::Status FlightServer::ListFlights(const flt::ServerCallContext& context, const flt::Criteria* criteria,
                                      std::unique_ptr<flt::FlightListing>* listings)
{
    ...
    public:
        ...
        arw::Status Next(std::unique_ptr<flt::FlightInfo>* info)
        {
            ...
            if (fd) {
                previous_key = fd->key;
                auto descriptor = flt::FlightDescriptor::Path({fd->tenant_name,
                                                               fd->bucket_name,
                                                               fd->object_key.name,
                                                               fd->object_key.instance,
                                                               fd->object_key.ns});
...
arw::Status FlightServer::GetFlightInfo(const flt::ServerCallContext& context, const flt::FlightDescriptor& request,
...
arw::Status FlightServer::GetSchema(const flt::ServerCallContext& context, const flt::FlightDescriptor& request,
...
public:
    ~OwnedBuffer() override { delete[] buffer; }
    ...
    uint8_t* writeable_data() { return buffer; }
};   // class OwnedBuffer
...
public:
    RandomAccessObject(const FlightData& _flight_data, std::unique_ptr<rgw::sal::Object>& obj, const DoutPrefix _dp)
        : flight_data(_flight_data)
        , dp(_dp)
        , position(-1)
        , is_closed(false)
    ...
    arw::Status Open()
    {
        ...
        if (ret < 0) {
            return arw::Status::IOError("unable to prepare object with error %d", ret);
        }
        INFO << "file opened successfully" << dendl;
        position = 0;
        return arw::Status::OK();
    ...
    arw::Result<int64_t> Read(int64_t nbytes, void* out) override
    {
        ...
        const int64_t bytes_read = op->read(position, end_position, bl, null_yield, &dp);
        if (bytes_read < 0) {
            const int64_t former_position = position;
            position = -1;
            ERROR << "read operation returned " << bytes_read << dendl;
            return arw::Status::IOError(
                "unable to read object at position %" PRId64 ", error code: %" PRId64, former_position, bytes_read);
        }
        ...
        if (nbytes != bytes_read) {
            INFO << "partial read: nbytes=" << nbytes << ", bytes_read=" << bytes_read << dendl;
        }
    ...
    arw::Result<std::shared_ptr<arw::Buffer>> Read(int64_t nbytes) override
    {
        ...
        ARROW_ASSIGN_OR_RAISE(const int64_t bytes_read, Read(nbytes, buffer->writeable_data()));
        buffer->set_size(bytes_read);
    ...
    arw::Result<arw::util::string_view> Peek(int64_t nbytes) override
    {
        ...
        ARROW_ASSIGN_OR_RAISE(OwningStringView buffer, OwningStringView::make(nbytes));

        ARROW_ASSIGN_OR_RAISE(const int64_t bytes_read, Read(nbytes, (void*)buffer.writeable_data()));
    ...
    arw::Result<std::shared_ptr<const arw::KeyValueMetadata>> ReadMetadata() { return flight_data.kv_metadata; }
...
arw::Status FlightServer::DoGet(const flt::ServerCallContext& context, const flt::Ticket& request,
                                std::unique_ptr<flt::FlightDataStream>* stream)
{
    ...
    ret = driver->get_bucket(&dp, &(*user), fd.tenant_name, fd.bucket_name, &bucket, null_yield);
    if (ret < 0) {
    ...
    ARROW_RETURN_NOT_OK(parquet::arrow::OpenFile(input, arw::default_memory_pool(), &reader));
    ...
    ARROW_ASSIGN_OR_RAISE(auto owning_reader, arw::RecordBatchReader::Make(std::move(batches), table->schema()));
    *stream = std::unique_ptr<flt::FlightDataStream>(new flt::RecordBatchStream(owning_reader));
