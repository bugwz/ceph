...
int RGWCreateOIDCProvider::verify_permission(optional_yield y)
{
    ...
    if (!verify_user_permission(this, s, rgw::ARN(idp_url, "oidc-provider", s->user->get_tenant(), true), get_op())) {
        return -EACCES;
    }
    return 0;
}
...
void RGWCreateOIDCProvider::execute(optional_yield y)
{
    ...
    RGWOIDCProvider provider(
        s->cct, store->getRados()->pctl, provider_url, s->user->get_tenant(), client_ids, thumbprints);
