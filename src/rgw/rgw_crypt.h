...
public:
    RGWGetObj_BlockDecrypt(const DoutPrefixProvider* dpp, CephContext* cct, RGWGetObj_Filter* next,
    ...
    static int read_manifest_parts(const DoutPrefixProvider* dpp, const bufferlist& manifest_bl,
...
public:
    RGWPutObj_BlockEncrypt(const DoutPrefixProvider* dpp, CephContext* cct, rgw::sal::DataProcessor* next,
...
int rgw_s3_prepare_decrypt(req_state* s, std::map<std::string, ceph::bufferlist>& attrs,
                           std::unique_ptr<BlockCrypt>* block_crypt,
                           std::map<std::string, std::string>& crypt_http_responses);

static inline void set_attr(std::map<std::string, bufferlist>& attrs, const char* key, std::string_view value)
{
    bufferlist bl;
...
static inline std::string get_str_attribute(std::map<std::string, bufferlist>& attrs, const char* name)
{
