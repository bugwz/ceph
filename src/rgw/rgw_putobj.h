...
public:
    explicit Pipe(rgw::sal::DataProcessor* next)
        : next(next)
    {}
    ...
    int process(bufferlist&& data, uint64_t offset) override { return next->process(std::move(data), offset); }
};
...
public:
    StripeProcessor(rgw::sal::DataProcessor* next, StripeGenerator* gen, uint64_t first_stripe_size)
        : Pipe(next)
        , gen(gen)
