...
bool TokenEngine::is_applicable(const std::string& token) const noexcept
{
    return !token.empty() && !cct->_conf->rgw_keystone_url.empty();
}

boost::optional<TokenEngine::token_envelope_t> TokenEngine::get_from_keystone(const DoutPrefixProvider* dpp,
                                                                              const std::string& token,
                                                                              bool allow_expired) const
{
    /* Unfortunately, we can't use the short form of "using" here. It's because
     * we're aliasing a class' member, not namespace. */
    using RGWValidateKeystoneToken = rgw::keystone::Service::RGWValidateKeystoneToken;
    ...
    if (rgw::keystone::Service::get_admin_token(dpp, cct, token_cache, config, admin_token) < 0) {
        throw -EINVAL;
    }
...
TokenEngine::auth_info_t TokenEngine::get_creds_info(const TokenEngine::token_envelope_t& token,
                                                     const std::vector<std::string>& admin_roles) const noexcept
{
    ...
    return auth_info_t{/* Suggested account name for the authenticated user. */
...
static inline const std::string make_spec_item(const std::string& tenant, const std::string& id)
{
    return tenant + ":" + id;
}

TokenEngine::acl_strategy_t TokenEngine::get_acl_strategy(const TokenEngine::token_envelope_t& token) const
{
...
TokenEngine::result_t TokenEngine::authenticate(const DoutPrefixProvider* dpp, const std::string& token,
                                                const std::string& service_token, const req_state* const s) const
{
    ...
    if (t) {
        ...
        auto apl = apl_factory->create_apl_remote(cct, s, get_acl_strategy(*t), get_creds_info(*t, roles.admin));
    ...
    if (g_conf()->rgw_keystone_service_token_enabled && !service_token.empty()) {
        ...
        if (st) {
            ldpp_dout(dpp, 20) << "cached service_token.project.id=" << st->get_project_id() << dendl;

            /* We found the service token in the cache so we allow using an expired
             * token for this request. */
            allow_expired = true;
            ldpp_dout(dpp, 20) << "allowing expired tokens because service_token_id=" << service_token_id
            ...
            if (st->expired()) {
                ldpp_dout(dpp, 0) << "got expired service token: " << st->get_project_name() << ":"
                                  << st->get_user_name() << " expired " << st->get_expires() << dendl;
    ...
    if (t->expired()) {
        if (allow_expired) {
            ldpp_dout(dpp, 20) << "allowing expired token: " << t->get_project_name() << ":" << t->get_user_name()
                               << " expired: " << t->get_expires() << " because of valid service token" << dendl;
        }
        else {
            ldpp_dout(dpp, 0) << "got expired token: " << t->get_project_name() << ":" << t->get_user_name()
        ...
        if (t->has_role(role) == true) {
            ...
            if (allow_expired) {
                ...
                ldpp_dout(dpp, 20) << "updating expiration of allowed expired token"
                                   ...
                                   << g_conf()->rgw_keystone_expired_token_cache_expiration << " secs = " << new_expires
                                   << dendl;
                t->set_expires(new_expires);
            }
            ldpp_dout(dpp, 0) << "validated token: " << t->get_project_name() << ":" << t->get_user_name()
    ...
    ldpp_dout(dpp, 0) << "user does not hold a matching role; required roles: " << g_conf()->rgw_keystone_accepted_roles
                      << dendl;
 ...
 */
std::pair<boost::optional<rgw::keystone::TokenEnvelope>, int> EC2Engine::get_from_keystone(
    const DoutPrefixProvider* dpp, const std::string_view& access_key_id, const std::string& string_to_sign,
    const std::string_view& signature) const
{
    ...
    std::string admin_token;
    int ret = rgw::keystone::Service::get_admin_token(dpp, cct, token_cache, config, admin_token);
    if (ret < 0) {
    ...
    if (validate.get_http_status() == decltype(validate)::HTTP_STATUS_UNAUTHORIZED) {
        return std::make_pair(boost::none, -ERR_SIGNATURE_NO_MATCH);
    }
    else if (validate.get_http_status() == decltype(validate)::HTTP_STATUS_NOTFOUND) {
...
std::pair<boost::optional<std::string>, int> EC2Engine::get_secret_from_keystone(
    const DoutPrefixProvider* dpp, const std::string& user_id, const std::string_view& access_key_id) const
{
 ...
 */
auto EC2Engine::get_access_token(const DoutPrefixProvider* dpp, const std::string_view& access_key_id,
                                 const std::string& string_to_sign, const std::string_view& signature,
                                 const signature_factory_t& signature_factory) const -> access_token_result
{
...
EC2Engine::acl_strategy_t EC2Engine::get_acl_strategy(const EC2Engine::token_envelope_t&) const
{
...
EC2Engine::auth_info_t EC2Engine::get_creds_info(const EC2Engine::token_envelope_t& token,
    ...
    const req_state* s, optional_yield y) const
{
    ...
    if (!found) {
        ...
        ldpp_dout(dpp, 5) << "s3 keystone: validated token: " << t->get_project_name() << ":" << t->get_user_name()
                          << " expires: " << t->get_expires() << dendl;

        auto apl = apl_factory->create_apl_remote(
            cct, s, get_acl_strategy(*t), get_creds_info(*t, accepted_roles.admin, std::string(access_key_id)));
        return result_t::grant(std::move(apl), completer_factory(secret_key));
    }
}

bool SecretCache::find(const std::string& token_id, SecretCache::token_envelope_t& token, std::string& secret)
{
...
void SecretCache::add(const std::string& token_id, const SecretCache::token_envelope_t& token,
