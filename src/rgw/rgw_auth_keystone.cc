                                                                              ...
                                                                              const std::string& token) const
{
    ...
    if (rgw::keystone::Service::get_admin_token(cct, token_cache, config, admin_token) < 0) {
        throw -EINVAL;
    }
    ...
    const std::string_view& signature) const
{
    ...
    std::string admin_token;
    int ret = rgw::keystone::Service::get_admin_token(cct, token_cache, config, admin_token);
    if (ret < 0) {
 ...
 */
std::pair<boost::optional<rgw::keystone::TokenEnvelope>, int> EC2Engine::get_access_token(
    const DoutPrefixProvider* dpp, const std::string_view& access_key_id, const std::string& string_to_sign,
    const std::string_view& signature, const signature_factory_t& signature_factory, bool ignore_signature) const
{
...
rgw::auth::Engine::result_t EC2Engine::authenticate(
    ...
    const signature_factory_t& signature_factory, const completer_factory_t& completer_factory, const req_state* s,
    optional_yield y) const
{
    ...
    std::tie(t, failure_reason) =
        get_access_token(dpp, access_key_id, string_to_sign, signature, signature_factory, ignore_signature);
    if (!t) {
