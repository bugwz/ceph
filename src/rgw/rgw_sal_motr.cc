...
static std::string motr_global_indices[] = {RGW_MOTR_USERS_IDX_NAME,
                                            RGW_MOTR_BUCKET_INST_IDX_NAME,
                                            RGW_MOTR_BUCKET_HD_IDX_NAME,
                                            RGW_IAM_MOTR_ACCESS_KEY,
                                            RGW_IAM_MOTR_EMAIL_KEY};

void MotrMetaCache::invalid(const DoutPrefixProvider* dpp, const string& name)
{
    cache.invalidate_remove(dpp, name);
}

int MotrMetaCache::put(const DoutPrefixProvider* dpp, const string& name, const bufferlist& data)
{
    ...
    if (rc < 0) ldpp_dout(dpp, 0) << "ERROR: failed to distribute cache for " << name << dendl;

    return 0;
}

int MotrMetaCache::get(const DoutPrefixProvider* dpp, const string& name, bufferlist& data)
{
    ObjectCacheInfo info;
    uint32_t flags = CACHE_FLAG_DATA;
    int rc = cache.get(dpp, name, info, flags, NULL);
    if (rc == 0) {
        if (info.status < 0) return info.status;
    ...
    if (rc == -ENODATA) return -ENOENT;

    return rc;
}

int MotrMetaCache::remove(const DoutPrefixProvider* dpp, const string& name)
...
int MotrMetaCache::distribute_cache(const DoutPrefixProvider* dpp, const string& normal_name, ObjectCacheInfo& obj_info,
                                    int op)
{
    return 0;
}

int MotrMetaCache::watch_cb(const DoutPrefixProvider* dpp, uint64_t notify_id, uint64_t cookie, uint64_t notifier_id,
                            bufferlist& bl)
{
    return 0;
}
...
int MotrUser::list_buckets(const DoutPrefixProvider* dpp, const string& marker, const string& end_marker, uint64_t max,
                           bool need_stats, BucketList& buckets, optional_yield y)
{
    ...
    ldpp_dout(dpp, 20) << __func__ << ": list_user_buckets: marker=" << marker << " end_marker=" << end_marker
                       << " max=" << max << dendl;
    ...
    for (const auto& bl : vals) {
        ...
        if (!end_marker.empty() && end_marker.compare(ent.bucket.marker) <= 0) break;

        buckets.add(std::make_unique<MotrBucket>(this->store, ent, this));
        bcount++;
    }
    if (bcount == max) is_truncated = true;
    buckets.set_truncated(is_truncated);

    return 0;
}

int MotrUser::create_bucket(const DoutPrefixProvider* dpp, const rgw_bucket& b, const std::string& zonegroup_id,
                            ...
                            bool* existed, req_info& req_info, std::unique_ptr<Bucket>* bucket_out, optional_yield y)
{
    ...
    if (!*existed) {
        ...
        if (ret < 0) ldpp_dout(dpp, 0) << "ERROR: failed to create bucket indices! " << ret << dendl;

        // Insert the bucket entry into the user info index.
        ret = mbucket->link_user(dpp, this, y);
        if (ret < 0) ldpp_dout(dpp, 0) << "ERROR: failed to add bucket entry! " << ret << dendl;
    }
    else {
        return -EEXIST;
        // bucket->set_version(ep_objv);
...
int MotrUser::read_stats(const DoutPrefixProvider* dpp, optional_yield y, RGWStorageStats* stats,
...
int MotrUser::load_user_from_idx(const DoutPrefixProvider* dpp, MotrStore* store, RGWUserInfo& info,
                                 map<string, bufferlist>* attrs, RGWObjVersionTracker* objv_tr)
{
    ...
    if (store->get_user_cache()->get(dpp, info.user_id.id, bl)) {
        // Cache misses
        int rc = store->do_idx_op_by_name(RGW_MOTR_USERS_IDX_NAME, M0_IC_GET, info.user_id.to_str(), bl);
...
{
    for (auto& it : new_attrs) attrs[it.first] = it.second;
...
int MotrUser::store_user(const DoutPrefixProvider* dpp, optional_yield y, bool exclusive, RGWUserInfo* old_info)
{
    ...
    if (rc == 0 && obj_ver.ver > 0) {
        if (old_info) *old_info = orig_info;
    ...
    muinfo.encode(bl);
    rc = store->do_idx_op_by_name(RGW_MOTR_USERS_IDX_NAME, M0_IC_PUT, info.user_id.to_str(), bl);
...
int MotrUser::remove_user(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    // Delete email id
    if (!info.user_email.empty()) {
        rc = store->do_idx_op_by_name(RGW_IAM_MOTR_EMAIL_KEY, M0_IC_DEL, info.user_email, bl);
        if (rc < 0 && rc != -ENOENT) {
    ...
    // Delete user from user index
    rc = store->do_idx_op_by_name(RGW_MOTR_USERS_IDX_NAME, M0_IC_DEL, info.user_id.to_str(), bl);
    if (rc < 0) {
...
int MotrBucket::remove_bucket(const DoutPrefixProvider* dpp, bool delete_children, bool forward_to_master,
                              req_info* req_info, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: remove_bucket failed to remove bucket instance from cache rc=" << ret << dendl;
        return ret;
    }

    ret = store->do_idx_op_by_name(RGW_MOTR_BUCKET_INST_IDX_NAME, M0_IC_DEL, info.bucket.name, bl);
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: remove_bucket failed to remove bucket instance rc=" << ret << dendl;
        return ret;
    }
...
int MotrBucket::remove_bucket_bypass_gc(int concurrent_max, bool keep_index_consistent, optional_yield y,
...
int MotrBucket::put_info(const DoutPrefixProvider* dpp, bool exclusive, ceph::real_time _mtime)
{
    ...
    int rc = store->do_idx_op_by_name(RGW_MOTR_BUCKET_INST_IDX_NAME, M0_IC_PUT, info.bucket.name, bl, !exclusive);
    if (rc == 0) store->get_bucket_inst_cache()->put(dpp, info.bucket.name, bl);
...
int MotrBucket::load_bucket(const DoutPrefixProvider* dpp, optional_yield y, bool get_stats)
{
    ...
    if (store->get_bucket_inst_cache()->get(dpp, info.bucket.name, bl)) {
        ...
        int rc = store->do_idx_op_by_name(RGW_MOTR_BUCKET_INST_IDX_NAME, M0_IC_GET, info.bucket.name, bl);
...
int MotrBucket::link_user(const DoutPrefixProvider* dpp, User* new_user, optional_yield y)
{
    ...
    new_bucket.size = 0;
    if (real_clock::is_zero(creation_time)) creation_time = ceph::real_clock::now();
    ...
    return store->do_idx_op_by_name(user_info_idx_name, M0_IC_PUT, info.bucket.name, bl);
}
...
int MotrBucket::unlink_user(const DoutPrefixProvider* dpp, User* new_user, optional_yield y)
{
    ...
    bufferlist bl;
    string user_info_idx_name = "motr.rgw.user.info." + new_user->get_info().user_id.to_str();
    return store->do_idx_op_by_name(user_info_idx_name, M0_IC_DEL, info.bucket.name, bl);
}

/* stats - Not for first pass */
int MotrBucket::read_stats(const DoutPrefixProvider* dpp, const bucket_index_layout_generation& idx_layout,
                           int shard_id, std::string* bucket_ver, std::string* master_ver,
                           std::map<RGWObjCategory, RGWStorageStats>& stats, std::string* max_marker, bool* syncstopped)
{
    return 0;
}
...
int MotrBucket::read_stats_async(const DoutPrefixProvider* dpp, const bucket_index_layout_generation& idx_layout,
...
int MotrBucket::check_quota(const DoutPrefixProvider* dpp, RGWQuota& quota, uint64_t obj_size, optional_yield y,
...
int MotrBucket::check_index(const DoutPrefixProvider* dpp, std::map<RGWObjCategory, RGWStorageStats>& existing_stats,
...
int MotrBucket::list(const DoutPrefixProvider* dpp, ListParams& params, int max, ListResults& results, optional_yield y)
{
    ...
    ldpp_dout(dpp, 20) << "bucket=" << info.bucket.name << " prefix=" << params.prefix << " marker=" << params.marker
                       << " max=" << max << dendl;

    // Retrieve all `max` number of pairs.
    string bucket_index_iname = "motr.rgw.bucket.index." + info.bucket.name;
    keys[0] = params.marker.empty() ? params.prefix : params.marker.get_oid();
    rc = store->next_query_by_name(bucket_index_iname, keys, vals, params.prefix, params.delim);
    if (rc < 0) {
        ldpp_dout(dpp, 0) << "ERROR: NEXT query failed. " << rc << dendl;
        return rc;
    }
    ...
    for (; i < rc; ++i) {
        if (vals[i].length() == 0) {
            ...
            ent.decode(iter);
            if (params.list_versions || ent.is_visible()) results.objs.emplace_back(std::move(ent));
        }
    }
...
int MotrBucket::list_multiparts(const DoutPrefixProvider* dpp, const string& prefix, string& marker,
                                ...
                                bool* is_truncated)
{
    ...
    string bucket_multipart_iname = "motr.rgw.bucket." + this->get_name() + ".multiparts";
    key_vec[0].clear();
    ...
    *is_truncated = false;
    for (const auto& bl : val_vec) {
        ...
        if (prefix.size() && (0 != ent.key.name.compare(0, prefix.size(), prefix))) {
            ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ": skippping \"" << ent.key
...
int MotrZoneGroup::get_placement_tier(const rgw_placement_rule& rule, std::unique_ptr<PlacementTier>* tier)
{
...
int MotrObject::get_obj_state(const DoutPrefixProvider* dpp, RGWObjState** _state, optional_yield y, bool follow_olh)
{
    ...
    if (this->store->get_obj_meta_cache()->get(dpp, this->get_key().get_oid(), bl)) {
        ...
        int rc = this->store->do_idx_op_by_name(bucket_index_iname, M0_IC_GET, this->get_key().get_oid(), bl);
        if (rc < 0) {
...
//  int MotrObject::read_attrs(const DoutPrefixProvider* dpp, Motr::Object::Read &read_op, optional_yield y, rgw_obj*
//  target_obj)
//  {
...
{
    if (this->category == RGWObjCategory::MultiMeta) return 0;
    ...
    ldpp_dout(dpp, 20) << "MotrObject::get_obj_attrs(): " << bname << "/" << key << dendl;
...
int MotrObject::modify_obj_attrs(const char* attr_name, bufferlist& attr_val, optional_yield y,
                                 const DoutPrefixProvider* dpp)
{
...
int MotrObject::omap_get_all(const DoutPrefixProvider* dpp, std::map<std::string, bufferlist>* m, optional_yield y)
{
    return 0;
}
...
std::unique_ptr<MPSerializer> MotrObject::get_serializer(const DoutPrefixProvider* dpp, const std::string& lock_name)
{
    return std::make_unique<MPMotrSerializer>(dpp, store, this, lock_name);
}

int MotrObject::transition(Bucket* bucket, const rgw_placement_rule& placement_rule, const real_time& mtime,
...
int MotrObject::MotrReadOp::prepare(optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    *params.lastmod = ent.meta.mtime;
    ...
    if (source->get_obj_size() == 0) return 0;

    // Open the object here.
    if (source->category == RGWObjCategory::MultiMeta) {
        ldpp_dout(dpp, 20) << __func__ << ": open obj parts..." << dendl;
        rc = source->get_part_objs(dpp, this->part_objs) ?: source->open_part_objs(dpp, this->part_objs);
        return rc;
    }
    else {
...
int MotrObject::MotrReadOp::read(int64_t off, int64_t end, bufferlist& bl, optional_yield y,
                                 const DoutPrefixProvider* dpp)
{
...
int MotrObject::MotrReadOp::iterate(const DoutPrefixProvider* dpp, int64_t off, int64_t end, RGWGetDataCB* cb,
                                    optional_yield y)
{
    int rc;
...
int MotrObject::MotrReadOp::get_attr(const DoutPrefixProvider* dpp, const char* name, bufferlist& dest,
                                     optional_yield y)
{
    // return 0;
    return -ENODATA;
...
{
    ldpp_dout(dpp, 20) << "delete " << source->get_key().get_oid() << " from " << source->get_bucket()->get_name()
                       << dendl;
    ...
    rc = source->store->do_idx_op_by_name(bucket_index_iname, M0_IC_DEL, source->get_key().get_oid(), bl);
    if (rc < 0) {
...
int MotrObject::delete_obj_aio(const DoutPrefixProvider* dpp, RGWObjState* astate, Completions* aio,
...
int MotrObject::copy_object(User* user, req_info* info, const rgw_zone_id& source_zone, rgw::sal::Object* dest_object,
...
int MotrObject::swift_versioning_restore(bool& restored, const DoutPrefixProvider* dpp)
{
    return 0;
}

int MotrObject::swift_versioning_copy(const DoutPrefixProvider* dpp, optional_yield y)
{
    return 0;
}

MotrAtomicWriter::MotrAtomicWriter(const DoutPrefixProvider* dpp, optional_yield y, rgw::sal::Object* obj,
                                   MotrStore* _store, const rgw_user& _owner,
...
int MotrObject::create_mobj(const DoutPrefixProvider* dpp, uint64_t sz)
{
    ...
    m0_op_launch(&op, 1);
    rc = m0_op_wait(op, M0_BITS(M0_OS_FAILED, M0_OS_STABLE), M0_TIME_NEVER) ?: m0_rc(op);
    m0_op_fini(op);
    m0_op_free(op);
    ...
    ldpp_dout(dpp, 20) << __func__ << ": lid=0x" << std::hex << meta.layout_id << std::dec << " rc=" << rc << dendl;
...
int MotrObject::open_mobj(const DoutPrefixProvider* dpp)
{
    ...
    if (meta.layout_id == 0) return -ENOENT;
...
int MotrObject::write_mobj(const DoutPrefixProvider* dpp, bufferlist&& data, uint64_t offset)
{
    ...
    rc = m0_bufvec_empty_alloc(&buf, 1) ?: m0_bufvec_alloc(&attr, 1, 1) ?: m0_indexvec_alloc(&ext, 1);
    if (rc != 0) goto out;
    ...
    for (p = start; left > 0; left -= bs, p += bs, offset += bs) {
        if (left < bs) bs = this->get_optimal_bs(left);
        if (left < bs) {
...
int MotrObject::read_mobj(const DoutPrefixProvider* dpp, int64_t off, int64_t end, RGWGetDataCB* cb)
{
    ...
    ldpp_dout(dpp, 20) << "MotrObject::read_mobj(): off=" << off << " end=" << end << dendl;
    ...
    for (; left > 0; off += actual) {
        if (left < bs) bs = this->get_optimal_bs(left);
        actual = bs;
        if (left < bs) actual = left;
        ldpp_dout(dpp, 20) << "MotrObject::read_mobj(): off=" << off << " actual=" << actual << dendl;
        bufferlist bl;
...
int MotrObject::get_bucket_dir_ent(const DoutPrefixProvider* dpp, rgw_bucket_dir_entry& ent)
{
    ...
    if (this->get_bucket()->get_info().versioning_status() == BUCKET_VERSIONED ||
        ...
        if (this->store->get_obj_meta_cache()->get(dpp, this->get_key().get_oid(), bl)) {
            ...
            rc = this->store->do_idx_op_by_name(bucket_index_iname, M0_IC_GET, this->get_key().get_oid(), bl);
            if (rc < 0) {
                ldpp_dout(dpp, 0) << __func__ << "ERROR: failed to get object's entry from bucket index: rc=" << rc
                                  << dendl;
                return rc;
            }
            this->store->get_obj_meta_cache()->put(dpp, this->get_key().get_oid(), bl);
...
int MotrObject::update_version_entries(const DoutPrefixProvider* dpp)
{
    ...
    for (const auto& bl : vals) {
        ...
        if (0 != ent.key.name.compare(0, this->get_name().size(), this->get_name())) continue;

        if (!ent.is_current()) continue;
        ...
        rc = store->do_idx_op_by_name(bucket_index_iname, M0_IC_PUT, key, ent_bl);
        if (rc < 0) break;
    }
    return rc;
}
...
int MotrObject::get_part_objs(const DoutPrefixProvider* dpp, std::map<int, std::unique_ptr<MotrObject>>& part_objs)
{
    int rc;
    int max_parts = 1000;
    int marker = 0;
    ...
    do {
        ...
        for (auto part_iter = parts.begin(); part_iter != parts.end(); ++part_iter) {
            ...
            string part_obj_name =
                this->get_bucket()->get_name() + "." + this->get_key().get_oid() + ".part." + std::to_string(part_num);
...
int MotrObject::open_part_objs(const DoutPrefixProvider* dpp, std::map<int, std::unique_ptr<MotrObject>>& part_objs)
{
...
int MotrObject::delete_part_objs(const DoutPrefixProvider* dpp)
{
    std::unique_ptr<rgw::sal::MultipartUpload> upload;
    upload = this->get_bucket()->get_multipart_upload(this->get_name(), string());
    std::unique_ptr<rgw::sal::MotrMultipartUpload> mupload(
        static_cast<rgw::sal::MotrMultipartUpload*>(upload.release()));
    return mupload->delete_parts(dpp);
}

int MotrObject::read_multipart_obj(const DoutPrefixProvider* dpp, int64_t off, int64_t end, RGWGetDataCB* cb,
                                   std::map<int, std::unique_ptr<MotrObject>>& part_objs)
{
    ...
    for (auto iter = part_objs.begin(); iter != part_objs.end(); ++iter) {
        ...
        ldpp_dout(dpp, 20) << "read_multipart_obj: part_off=" << part_off << " part_end=" << part_end << dendl;
        if (part_end < off) continue;

        int64_t local_off = cursor - obj->part_off;
        int64_t local_end = part_end < end ? part_size - 1 : end - part_off;
        ldpp_dout(dpp, 20) << "real_multipart_obj: name=" << obj->get_name() << " local_off=" << local_off
...
unsigned MotrObject::get_optimal_bs(unsigned len)
{
    ...
    pver = m0_pool_version_find(&store->instance->m0c_pools_common, &mobj->ob_attr.oa_pver);
    ...
    unsigned max_bs = depth * unit_sz * pa->pa_P * pa->pa_N / (pa->pa_N + pa->pa_K + pa->pa_S);
...
int MotrAtomicWriter::write()
{
    ...
    if (!obj.is_opened()) {
        ...
        if (rc == -EEXIST) rc = obj.open_mobj(dpp);
        if (rc != 0) {
            char fid_str[M0_FID_STR_LEN];
            snprintf(fid_str, ARRAY_SIZE(fid_str), U128X_F, U128_P(&obj.meta.oid));
            ldpp_dout(dpp, 0) << "ERROR: failed to create/open motr object " << fid_str << " ("
                              << obj.get_bucket()->get_name() << "/" << obj.get_key().get_oid() << "): rc=" << rc
                              << dendl;
            goto err;
        }
    }
    ...
    bi = acc_data.begin();
    while (left > 0) {
        if (left < bs) bs = obj.get_optimal_bs(left);
        if (left < bs) {
...
int MotrAtomicWriter::process(bufferlist&& data, uint64_t offset)
{
    if (data.length() == 0) {   // last call, flush data
        int rc = 0;
        if (acc_data.length() != 0) rc = this->write();
        this->cleanup();
        return rc;
    }

    if (acc_data.length() == 0) acc_off = offset;

    acc_data.append(std::move(data));
    if (acc_data.length() < MAX_ACC_SIZE) return 0;

    return this->write();
}

int MotrAtomicWriter::complete(size_t accounted_size, const std::string& etag, ceph::real_time* mtime,
                               ...
                               optional_yield y)
{
    ...
    if (is_versioned) ent.flags = rgw_bucket_dir_entry::FLAG_VER | rgw_bucket_dir_entry::FLAG_CURRENT;
    ldpp_dout(dpp, 20) << __func__ << ": key=" << obj.get_key().get_oid() << " etag: " << etag
                       << " user_data=" << user_data << dendl;
    ...
    encode(attrs, bl);
    obj.meta.encode(bl);
    ldpp_dout(dpp, 20) << __func__ << ": lid=0x" << std::hex << obj.meta.layout_id << dendl;
    if (is_versioned) {
    ...
    rc = store->do_idx_op_by_name(bucket_index_iname, M0_IC_PUT, obj.get_key().get_oid(), bl);
    if (rc == 0) store->get_obj_meta_cache()->put(dpp, obj.get_key().get_oid(), bl);
...
int MotrMultipartUpload::delete_parts(const DoutPrefixProvider* dpp)
{
    ...
    do {
        ...
        for (auto part_iter = parts.begin(); part_iter != parts.end(); ++part_iter) {
            ...
            string part_obj_name = bucket->get_name() + "." + mp_obj.get_key() + ".part." + std::to_string(part_num);
            std::unique_ptr<rgw::sal::Object> obj;
            obj = this->bucket->get_object(rgw_obj_key(part_obj_name));
            std::unique_ptr<rgw::sal::MotrObject> mobj(static_cast<rgw::sal::MotrObject*>(obj.release()));
            mobj->meta = mmpart->meta;
...
int MotrMultipartUpload::abort(const DoutPrefixProvider* dpp, CephContext* cct)
{
    ...
    string bucket_multipart_iname = "motr.rgw.bucket." + meta_obj->get_bucket()->get_name() + ".multiparts";
    rc = store->do_idx_op_by_name(bucket_multipart_iname, M0_IC_GET, meta_obj->get_key().to_str(), bl);
    if (rc < 0) {
    ...
    rc = store->do_idx_op_by_name(bucket_multipart_iname, M0_IC_DEL, meta_obj->get_key().get_oid(), bl);
    return rc;
}
...
int MotrMultipartUpload::init(const DoutPrefixProvider* dpp, optional_yield y, ACLOwner& _owner,
                              rgw_placement_rule& dest_placement, rgw::sal::Attrs& attrs)
{
    ...
    do {
        ...
        string bucket_multipart_iname = "motr.rgw.bucket." + obj->get_bucket()->get_name() + ".multiparts";
        rc = store->do_idx_op_by_name(bucket_multipart_iname, M0_IC_PUT, obj->get_key().get_oid(), bl);
...
int MotrMultipartUpload::list_parts(const DoutPrefixProvider* dpp, CephContext* cct, int num_parts, int marker,
                                    int* next_marker, bool* truncated, bool assume_unsorted)
{
    ...
    for (const auto& bl : val_vec) {
        ...
        ldpp_dout(dpp, 20) << __func__ << ": part_num=" << info.num << " part_size=" << info.size << dendl;
        ldpp_dout(dpp, 20) << __func__ << ": meta:oid=[" << meta.oid.u_hi << "," << meta.oid.u_lo << "], meta:pvid=["
                           << meta.pver.f_container << "," << meta.pver.f_key << "], meta:layout id=" << meta.layout_id
                           << dendl;
    ...
    if (truncated) *truncated = part_cnt < num_parts ? false : true;
    ldpp_dout(dpp, 20) << __func__ << ": truncated=" << *truncated << dendl;

    if (next_marker) *next_marker = last_num;

    return 0;
}

// Heavily copy from rgw_sal_rados.cc
int MotrMultipartUpload::complete(const DoutPrefixProvider* dpp, optional_yield y, CephContext* cct,
                                  map<int, string>& part_etags, list<rgw_obj_index_key>& remove_objs,
                                  uint64_t& accounted_size, bool& compressed, RGWCompressionInfo& cs_info, off_t& off,
                                  std::string& tag, ACLOwner& owner, uint64_t olh_epoch, rgw::sal::Object* target_obj)
{
    ...
    do {
      ...
      ldpp_dout(dpp, 0) << "MotrMultipartUpload::complete(): manifest " << dendl;
            ...
            if ((handled_parts > 0) &&
                ((part_compressed != compressed) || (cs_info.compression_type != part->cs_info.compression_type))) {
            ...
            if (part_compressed) {
                ...
                if (!compressed) cs_info.compression_type = part->cs_info.compression_type;
            ...
            ldpp_dout(dpp, 20) << "MotrMultipartUpload::complete(): off=" << off
    ...
    meta_obj = get_meta_obj();
    string bucket_multipart_iname = "motr.rgw.bucket." + meta_obj->get_bucket()->get_name() + ".multiparts";
    rc = this->store->do_idx_op_by_name(bucket_multipart_iname, M0_IC_GET, meta_obj->get_key().get_oid(), bl);
    ...
    rc = store->do_idx_op_by_name(bucket_index_iname, M0_IC_PUT, target_obj->get_name(), update_bl);
    if (rc < 0) return rc;
    ...
    return store->do_idx_op_by_name(bucket_multipart_iname, M0_IC_DEL, meta_obj->get_key().get_oid(), bl);
}

int MotrMultipartUpload::get_info(const DoutPrefixProvider* dpp, optional_yield y, rgw_placement_rule** rule,
                                  rgw::sal::Attrs* attrs)
{
    if (!rule && !attrs) {
        return 0;
    ...
    bufferlist bl;
    string bucket_multipart_iname = "motr.rgw.bucket." + meta_obj->get_bucket()->get_name() + ".multiparts";
    int rc = this->store->do_idx_op_by_name(bucket_multipart_iname, M0_IC_GET, meta_obj->get_key().get_oid(), bl);
    if (rc < 0) {
...
std::unique_ptr<Writer> MotrMultipartUpload::get_writer(const DoutPrefixProvider* dpp, optional_yield y,
                                                        rgw::sal::Object* obj, const rgw_user& owner,
                                                        const rgw_placement_rule* ptail_placement_rule,
                                                        uint64_t part_num, const std::string& part_num_str)
{
    return std::make_unique<MotrMultipartWriter>(
        dpp, y, this, obj, store, owner, ptail_placement_rule, part_num, part_num_str);
}

int MotrMultipartWriter::prepare(optional_yield y)
{
    string part_obj_name =
        head_obj->get_bucket()->get_name() + "." + head_obj->get_key().get_oid() + ".part." + std::to_string(part_num);
    ...
    if (part_obj == nullptr) return -ENOMEM;
...
int MotrMultipartWriter::complete(size_t accounted_size, const std::string& etag, ceph::real_time* mtime,
                                  ceph::real_time set_mtime, std::map<std::string, bufferlist>& attrs,
                                  ceph::real_time delete_at, const char* if_match, const char* if_nomatch,
                                  const std::string* user_data, rgw_zone_set* zones_trace, bool* canceled,
                                  optional_yield y)
{
    ...
    p.append(buf);
    string obj_part_iname =
        "motr.rgw.object." + head_obj->get_bucket()->get_name() + "." + head_obj->get_key().get_oid() + ".parts";
...
std::unique_ptr<RGWRole> MotrStore::get_role(std::string name, std::string tenant, std::string path,
...
int MotrStore::get_roles(const DoutPrefixProvider* dpp, optional_yield y, const std::string& path_prefix,
...
int MotrStore::get_oidc_providers(const DoutPrefixProvider* dpp, const std::string& tenant,
...
std::unique_ptr<Writer> MotrStore::get_append_writer(const DoutPrefixProvider* dpp, optional_yield y,
                                                     rgw::sal::Object* obj, const rgw_user& owner,
                                                     const rgw_placement_rule* ptail_placement_rule,
...
std::unique_ptr<Writer> MotrStore::get_atomic_writer(const DoutPrefixProvider* dpp, optional_yield y,
                                                     rgw::sal::Object* obj, const rgw_user& owner,
                                                     const rgw_placement_rule* ptail_placement_rule, uint64_t olh_epoch,
                                                     const std::string& unique_tag)
{
    return std::make_unique<MotrAtomicWriter>(dpp, y, obj, this, owner, ptail_placement_rule, olh_epoch, unique_tag);
}
...
int MotrStore::get_user_by_access_key(const DoutPrefixProvider* dpp, const std::string& key, optional_yield y,
                                      std::unique_ptr<User>* user)
{
    ...
    rc = do_idx_op_by_name(RGW_IAM_MOTR_ACCESS_KEY, M0_IC_GET, key, bl);
    if (rc < 0) {
...
int MotrStore::get_user_by_email(const DoutPrefixProvider* dpp, const std::string& email, optional_yield y,
                                 std::unique_ptr<User>* user)
{
    int rc;
    User* u;
    ...
    rc = do_idx_op_by_name(RGW_IAM_MOTR_EMAIL_KEY, M0_IC_GET, email, bl);
    if (rc < 0) {
...
int MotrStore::get_user_by_swift(const DoutPrefixProvider* dpp, const std::string& user_str, optional_yield y,
                                 std::unique_ptr<User>* user)
{
...
int MotrStore::store_access_key(const DoutPrefixProvider* dpp, optional_yield y, MotrAccessKey access_key)
{
    ...
    rc = do_idx_op_by_name(RGW_IAM_MOTR_ACCESS_KEY, M0_IC_PUT, access_key.id, bl);
    if (rc < 0) {
...
int MotrStore::delete_access_key(const DoutPrefixProvider* dpp, optional_yield y, std::string access_key)
{
    int rc;
    bufferlist bl;
    rc = do_idx_op_by_name(RGW_IAM_MOTR_ACCESS_KEY, M0_IC_DEL, access_key, bl);
    if (rc < 0) {
...
int MotrStore::store_email_info(const DoutPrefixProvider* dpp, optional_yield y, MotrEmailInfo& email_info)
{
    ...
    rc = do_idx_op_by_name(RGW_IAM_MOTR_EMAIL_KEY, M0_IC_PUT, email_info.email_id, bl);
    if (rc < 0) {
...
int MotrStore::get_bucket(const DoutPrefixProvider* dpp, User* u, const rgw_bucket& b, std::unique_ptr<Bucket>* bucket,
                          optional_yield y)
{
    int ret;
    Bucket* bp;
...
int MotrStore::get_bucket(const DoutPrefixProvider* dpp, User* u, const std::string& tenant, const std::string& name,
...
int MotrStore::list_all_zones(const DoutPrefixProvider* dpp, std::list<std::string>& zone_ids)
{
...
std::unique_ptr<Notification> MotrStore::get_notification(const DoutPrefixProvider* dpp, Object* obj, Object* src_obj,
...
void MotrStore::get_ratelimit(RGWRateLimitInfo& bucket_ratelimit, RGWRateLimitInfo& user_ratelimit,
...
int MotrStore::get_sync_policy_handler(const DoutPrefixProvider* dpp, std::optional<rgw_zone_id> zone,
...
int MotrStore::meta_list_keys_next(const DoutPrefixProvider* dpp, void* handle, int max, list<string>& keys,
                                   bool* truncated)
{
    return 0;
}
...
int MotrStore::open_idx(struct m0_uint128* id, bool create, struct m0_idx* idx)
{
    ...
    if (!create) return 0;   // nothing to do more
    ...
    if (rc != 0 && rc != -EEXIST) ldout(cctx, 0) << "ERROR: index create failed: " << rc << dendl;
out:
    return rc;
}
...
int MotrStore::do_idx_op(struct m0_idx* idx, enum m0_idx_opcode opcode, vector<uint8_t>& key, vector<uint8_t>& val,
                         bool update)
{
    int rc, rc_i;
    ...
    set_m0bufvec(&k, key);
    if (opcode == M0_IC_PUT) set_m0bufvec(&v, val);

    if (opcode == M0_IC_DEL) vp = nullptr;

    if (opcode == M0_IC_PUT && update) flags |= M0_OIF_OVERWRITE;
...
int MotrStore::do_idx_next_op(struct m0_idx* idx, vector<vector<uint8_t>>& keys, vector<vector<uint8_t>>& vals)
{
    int rc;
    uint32_t i = 0;
    ...
    rc = m0_bufvec_empty_alloc(&k, nr_kvp) ?: m0_bufvec_empty_alloc(&v, nr_kvp);
    if (rc != 0) {
...
int MotrStore::next_query_by_name(string idx_name, vector<string>& key_out, vector<bufferlist>& val_out, string prefix,
                                  string delim)
{
    ...
    if (rc != 0) {
        ldout(cctx, 0) << "ERROR: next_query_by_name(): failed to open index: rc=" << rc << dendl;
        goto out;
    }
    ...
    ldout(cctx, 20) << __func__ << ": next_query_by_name(): index=" << idx_name << " prefix=" << prefix
                    << " delim=" << delim << dendl;
    ...
    for (i = 0; i < (int)val_out.size(); i += k, k = 0) {
        ...
        for (j = 0, k = 0; j < rc; ++j) {
            ...
            if (!delim.empty()) pos = key.find(delim, prefix.length());
            if (pos != std::string::npos) {   // DIR entry
                dir.assign(key, 0, pos + 1);
                if (dir.compare(0, prefix.length(), prefix) != 0) goto out;
                if (i + k == 0 || dir != key_out[i + k - 1])   // a new one
                    key_out[i + k++] = dir;
                continue;
            }
            dir = "";
            if (key.compare(0, prefix.length(), prefix) != 0) goto out;
            key_out[i + k] = key;
...
enum
{
    ...
    M0_DIX_FID_DIX_CONTAINER_MASK = (1ULL << M0_DIX_FID_DEVICE_ID_OFFSET) - 1,
};
...
void MotrStore::index_name_to_motr_fid(string iname, struct m0_uint128* id)
{
    ...
    m0_fid_tset(fid, m0_dix_fid_type.ft_id, fid->f_container & M0_DIX_FID_DIX_CONTAINER_MASK, fid->f_key);
    ldout(cctx, 20) << "converted id = 0x" << std::hex << id->u_hi << ":0x" << std::hex << id->u_lo << dendl;
}

int MotrStore::do_idx_op_by_name(string idx_name, enum m0_idx_opcode opcode, string key_str, bufferlist& bl,
                                 bool update)
{
    struct m0_idx idx;
    ...
    if (opcode == M0_IC_PUT) val.assign(bl.c_str(), bl.c_str() + bl.length());

    ldout(cctx, 20) << __func__ << ": do_idx_op_by_name(): op=" << (opcode == M0_IC_PUT ? "PUT" : "GET")
...
int MotrStore::init_metadata_cache(const DoutPrefixProvider* dpp, CephContext* cct)
{
...
int MotrLuaManager::get_script(const DoutPrefixProvider* dpp, optional_yield y, const std::string& key,
                               std::string& script)
{
    return -ENOENT;
}

int MotrLuaManager::put_script(const DoutPrefixProvider* dpp, optional_yield y, const std::string& key,
