...
template<typename CephTime> void pushtime(lua_State* L, const CephTime& tp)
{
...
struct EmptyMetaTable
{
    ...
    static int NewIndexClosure(lua_State* L) { return luaL_error(L, "trying to write to readonly field"); }

    // by default nothing is iterable
    // to change, overload this function in the derived
    static int PairsClosure(lua_State* L) { return luaL_error(L, "trying to iterate over non-iterable field"); }

    // by default nothing is iterable
    // to change, overload this function in the derived
    static int LenClosure(lua_State* L) { return luaL_error(L, "trying to get length of non-iterable field"); }

    static int error_unknown_field(lua_State* L, const std::string& index, const std::string& table)
    {
        return luaL_error(L, "unknown field name: %s provided to: %s", index.c_str(), table.c_str());
    }
};
...
// package.path = "<install_dir>/share/lua/5.3/?.lua" │                         LuaRocks. package.cpath=
// "<install_dir>/lib/lua/5.3/?.so"
...
template<typename MapType = std::map<std::string, std::string>> int StringMapWriteableNewIndex(lua_State* L)
{
    ...
    if (lua_isnil(L, 3) == 0) {
        ...
        if (strnlen(value, MAX_LUA_VALUE_SIZE) + strnlen(index, MAX_LUA_VALUE_SIZE) > MAX_LUA_VALUE_SIZE) {
...
struct StringMapMetaTable : public EmptyMetaTable
{
    ...
    static int NewIndexClosure(lua_State* L) { return NewIndex(L); }
