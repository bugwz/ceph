...
static const string BI_OLH_ENTRY_NS_START = "\x80"
                                            "1001_";
static const string BI_INSTANCE_ENTRY_NS_START = "\x80"
                                                 "1000_";
 ...
 */
static void parse_bucket(const string& bucket, string* tenant_name, string* bucket_name,
 ...
 */
std::string rgw_make_bucket_entry_name(const std::string& tenant_name, const std::string& bucket_name)
{
 ...
 */
int rgw_parse_url_bucket(const string& bucket, const string& auth_tenant, string& tenant_name, string& bucket_name)
{
 ...
 */
int rgw_read_user_buckets(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, const rgw_user& user_id,
                          rgw::sal::RGWBucketList& buckets, const string& marker, const string& end_marker,
                          uint64_t max, bool need_stats, optional_yield y)
{
    rgw::sal::RGWRadosUser user(store, user_id);
    return user.list_buckets(dpp, marker, end_marker, max, need_stats, buckets, y);
}

int rgw_bucket_parse_bucket_instance(const string& bucket_instance, string* bucket_name, string* bucket_id,
                                     int* shard_id)
{
...
int rgw_bucket_parse_bucket_key(CephContext* cct, const string& key, rgw_bucket* bucket, int* shard_id)
{
...
static void dump_mulipart_index_results(list<rgw_obj_index_key>& objs_to_unlink, Formatter* f)
{
...
void check_bad_user_bucket_mapping(rgw::sal::RGWRadosStore* store, const rgw_user& user_id, bool fix, optional_yield y,
                                   const DoutPrefixProvider* dpp)
{
    ...
    do {
        ...
        for (auto i = buckets.begin(); i != buckets.end(); ++i) {
            ...
            real_time mtime;
            int r = store->getRados()->get_bucket_info(
                store->svc(), user_id.tenant, bucket->get_name(), bucket_info, &mtime, null_yield, dpp);
            if (r < 0) {
            ...
            if (actual_bucket.name.compare(bucket->get_name()) != 0 ||
                ...
                if (fix) {
                    ...
                    r = store->ctl()->bucket->link_bucket(
                        user_id, actual_bucket, bucket_info.creation_time, null_yield, dpp);
                    if (r < 0) {
...
int rgw_remove_object(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, const RGWBucketInfo& bucket_info,
                      const rgw_bucket& bucket, rgw_obj_key& key)
{
...
int rgw_remove_bucket_bypass_gc(rgw::sal::RGWRadosStore* store, rgw_bucket& bucket, int concurrent_max,
                                bool keep_index_consistent, optional_yield y, const DoutPrefixProvider* dpp)
{
    int ret;
    ...
    while (is_truncated) {
        objs.clear();
        ret = list_op.list_objects(dpp, listing_max_entries, &objs, &common_prefixes, &is_truncated, null_yield);
        if (ret < 0) return ret;
        ...
        for (; it != objs.end(); ++it) {
            ...
            if (astate->manifest) {
                ...
                for (; miter != manifest.obj_end(dpp) && max_aio--; ++miter) {
                    if (!max_aio) {
                        ...
                        if (ret < 0) {
                            ldpp_dout(dpp, -1)
                                << "ERROR: could not drain handles as aio completion returned with " << ret << dendl;
                            return ret;
                        }
                ...
                ret = store->getRados()->delete_obj_aio(
                    dpp, head_obj, info, astate, handles, keep_index_consistent, null_yield);
                if (ret < 0) {
                    ldpp_dout(dpp, -1) << "ERROR: delete obj aio failed with " << ret << dendl;
                    return ret;
                }
            ...
            if (!max_aio) {
                ret = drain_handles(handles);
                if (ret < 0) {
                    ldpp_dout(dpp, -1) << "ERROR: could not drain handles as aio completion returned with " << ret
                                       << dendl;
                    return ret;
                }
                max_aio = concurrent_max;
...
{
    if (sink && !msg.empty()) *sink = msg;
}

int RGWBucket::init(rgw::sal::RGWRadosStore* storage, RGWBucketAdminOpState& op_state, optional_yield y,
                    const DoutPrefixProvider* dpp, std::string* err_msg, map<string, bufferlist>* pattrs)
{
    if (!storage) {
    ...
    if (bucket.name.empty() && user_id.empty()) return -EINVAL;
    ...
    if (!bucket.name.empty()) {
        int r = store->ctl()->bucket->read_bucket_info(
            bucket, &bucket_info, y, dpp, RGWBucketCtl::BucketInstance::GetParams().set_attrs(pattrs), &ep_objv);
        if (r < 0) {
                    ...
                    map<string, bufferlist>& attrs, std::string* err_msg)
{
    ...
    if (!bucket_id.empty() && bucket_id != bucket.bucket_id) {
        set_err_msg(err_msg, "specified bucket id does not match " + bucket.bucket_id);
        return -EINVAL;
    }
    ...
    if (aiter == attrs.end()) {
        ...
        set_err_msg(err_msg,
                    ...
                    "  You must sacrifice your ancient bucket " +
                        bucket.bucket_id);
        return -EINVAL;
    }
    ...
    /* link to user */
    r = store->ctl()->bucket->link_bucket(
        user_info.user_id, bucket_info.bucket, ep.creation_time, y, dpp, true, &ep_data);
    if (r < 0) {
    ...
    if (bucket != old_bucket) {
        ...
        r = bucket_ctl->remove_bucket_entrypoint_info(
            old_bucket, y, dpp, RGWBucketCtl::Bucket::RemoveParams().set_objv_tracker(&ep_data.ep_objv));
        if (r < 0) {
            set_err_msg(err_msg, "failed to unlink old bucket endpoint " + old_bucket.tenant + "/" + old_bucket.name);
            return r;
        }

        r = bucket_ctl->remove_bucket_instance_info(
            old_bucket,
            bucket_info,
            y,
            dpp,
            RGWBucketCtl::BucketInstance::RemoveParams().set_objv_tracker(&old_version));
        if (r < 0) {
...
int RGWBucket::chown(RGWBucketAdminOpState& op_state, const string& marker, optional_yield y,
                     const DoutPrefixProvider* dpp, std::string* err_msg)
{
    int ret =
        store->ctl()->bucket->chown(store, bucket_info, user_info.user_id, user_info.display_name, marker, y, dpp);
    if (ret < 0) {
...
int RGWBucket::unlink(RGWBucketAdminOpState& op_state, optional_yield y, const DoutPrefixProvider* dpp,
                      std::string* err_msg)
{
    rgw_bucket bucket = op_state.get_bucket();
...
int RGWBucket::set_quota(RGWBucketAdminOpState& op_state, const DoutPrefixProvider* dpp, std::string* err_msg)
{
    ...
    int r = store->getRados()->get_bucket_info(
        store->svc(), bucket.tenant, bucket.name, bucket_info, NULL, null_yield, dpp, &attrs);
    if (r < 0) {
...
static void dump_index_check(map<RGWObjCategory, RGWStorageStats> existing_stats,
                             map<RGWObjCategory, RGWStorageStats> calculated_stats, Formatter* formatter)
{
...
int RGWBucket::check_bad_index_multipart(RGWBucketAdminOpState& op_state, RGWFormatterFlusher& flusher,
                                         const DoutPrefixProvider* dpp, std::string* err_msg)
{
    ...
    int r =
        store->getRados()->get_bucket_instance_info(obj_ctx, bucket, bucket_info, nullptr, nullptr, null_yield, dpp);
    if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: " << __func__ << "(): get_bucket_instance_info(bucket=" << bucket
                          << ") returned r=" << r << dendl;
        return r;
    }
    ...
    do {
        ...
        int r = list_op.list_objects(dpp, listing_max_entries, &result, &common_prefixes, &is_truncated, null_yield);
        if (r < 0) {
            set_err_msg(err_msg, "failed to list objects in bucket=" + bucket.name + " err=" + cpp_strerror(-r));
    ...
    for (auto aiter = all_objs.begin(); aiter != all_objs.end(); ++aiter) {
        ...
        if (objs_to_unlink.size() > listing_max_entries) {
            if (fix_index) {
                ...
                if (r < 0) {
                    set_err_msg(err_msg, "ERROR: remove_obj_from_index() returned error: " + cpp_strerror(-r));
                    return r;
                }
            }
...
int RGWBucket::check_object_index(const DoutPrefixProvider* dpp, RGWBucketAdminOpState& op_state,
                                  RGWFormatterFlusher& flusher, optional_yield y, std::string* err_msg)
{
    ...
    while (is_truncated) {
        ...
        int r = store->getRados()->cls_bucket_list_ordered(dpp,
                                                           bucket_info,
                                                           RGW_NO_SHARD,
                                                           marker,
                                                           prefix,
        ...
        else if (result.size() > listing_max_entries * 7 / 8 && expansion_factor > 1) {
            --expansion_factor;
        }
 ...
 */
static int check_index_olh(rgw::sal::RGWRadosStore* const rados_store, rgw::sal::RGWRadosBucket* const bucket,
                           const DoutPrefixProvider* dpp, RGWBucketAdminOpState& op_state, RGWFormatterFlusher& flusher,
                           const int shard, uint64_t* const count_out, optional_yield y)
{
        ...
        for (iter = entries.begin(); iter != entries.end(); ++iter) {
            ...
            if (op_state.will_fix_index()) {
                ...
                if (olh_entry.pending_log.empty()) {
                    ...
                    if (ret < 0) {
                        ldpp_dout(dpp, -1) << "ERROR failed to clear olh for: " << olh_entry.key.name
                                           << " clear_olh(): " << cpp_strerror(-ret) << dendl;
                        continue;
                    }
                }
                else {
                    ...
                    if (ret < 0) {
                        ldpp_dout(dpp, -1) << "ERROR failed to get state for: " << olh_entry.key.name
                                           << " get_obj_state(): " << cpp_strerror(-ret) << dendl;
                        continue;
                    }
                    ret = store->update_olh(dpp, obj_ctx, state, bucket->get_info(), obj);
                    if (ret < 0) {
                        ldpp_dout(dpp, -1) << "ERROR failed to update olh for: " << olh_entry.key.name
                                           << " update_olh(): " << cpp_strerror(-ret) << dendl;
                        continue;
                    }
                }
            }
 ...
 */
int RGWBucket::check_index_olh(rgw::sal::RGWRadosStore* const rados_store, const DoutPrefixProvider* dpp,
                               RGWBucketAdminOpState& op_state, RGWFormatterFlusher& flusher)
{
    ...
    const int max_shards = (bucket_info.layout.current_index.layout.normal.num_shards > 0
    ...
    for (int i = 0; i < max_aio; i++) {
        spawn::spawn(context, [&](yield_context yield) {
            while (true) {
                ...
                if (r < 0) {
                    ldpp_dout(dpp, -1) << "NOTICE: error processing shard " << shard << " check_index_olh(): " << r
                                       << dendl;
                }
                count_out += shard_count;
                if (!op_state.hide_progress) {
                    ldpp_dout(dpp, 1) << "NOTICE: finished shard " << shard << " (" << shard_count << " entries "
                                      << verb << ")" << dendl;
                }
            }
        });
    ...
    if (!op_state.hide_progress) {
        ldpp_dout(dpp, 1) << "NOTICE: finished all shards (" << count_out << " entries " << verb << ")" << dendl;
    }
    if (op_state.dump_keys) {
        formatter->close_section();
        flusher.flush();
 ...
 * of a plain entry).
 */
static int is_versioned_instance_listable(const DoutPrefixProvider* dpp, RGWRados::BucketShard& bs,
                                          const cls_rgw_obj_key& key, bool& listable, optional_yield y)
{
    ...
    do {
        ...
        cls_rgw_bucket_list_op(op, marker, key.name, empty_delim, 1000, true, &result);
        bufferlist ibl;
 ...
 * instance entry.
 */
static int check_index_unlinked(rgw::sal::RGWRadosStore* const rados_store, rgw::sal::RGWRadosBucket* const bucket,
                                const DoutPrefixProvider* dpp, RGWBucketAdminOpState& op_state,
                                RGWFormatterFlusher& flusher, const int shard, uint64_t* const count_out,
                                optional_yield y)
{
    ...
    do {
        ...
        for (iter = entries.begin(); iter != entries.end(); ++iter) {
            ...
            try {
                ...
                ldpp_dout(dpp, -1) << "ERROR failed to decode instance entry for key: " << entry.idx << dendl;
                continue;
            }
            ...
            if (ret < 0) {
                ldpp_dout(dpp, -1) << "ERROR is_versioned_instance_listable(key='" << dir_entry.key
            ...
            if (op_state.will_fix_index()) {
                ...
                if (ret < 0) {
                    ldpp_dout(dpp, -1) << "ERROR rgw_remove_obj(key='" << dir_entry.key << "'): " << cpp_strerror(-ret)
                                       << dendl;
                    continue;
                }
            }
 ...
 */
int RGWBucket::check_index_unlinked(rgw::sal::RGWRadosStore* const rados_store, const DoutPrefixProvider* dpp,
                                    RGWBucketAdminOpState& op_state, RGWFormatterFlusher& flusher)
{
    if ((bucket_info.versioning_status() & BUCKET_VERSIONED) == 0) {
        ldpp_dout(dpp, 0) << "WARNING: this command is only applicable to versioned buckets" << dendl;
    ...
    for (int i = 0; i < max_aio; i++) {
        spawn::spawn(context, [&](yield_context yield) {
            while (true) {
                ...
                optional_yield y{context, yield};
                int r =
                    ::check_index_unlinked(rados_store, &rados_bucket, dpp, op_state, flusher, shard, &shard_count, y);
                if (r < 0) {
                    ldpp_dout(dpp, -1) << "ERROR: error processing shard " << shard << " check_index_unlinked(): " << r
                                       << dendl;
                }
                count_out += shard_count;
                if (!op_state.hide_progress) {
...
int RGWBucket::check_index(const DoutPrefixProvider* dpp, RGWBucketAdminOpState& op_state,
                           map<RGWObjCategory, RGWStorageStats>& existing_stats,
                           map<RGWObjCategory, RGWStorageStats>& calculated_stats, std::string* err_msg)
{
    bool fix_index = op_state.will_fix_index();
...
int RGWBucket::sync(RGWBucketAdminOpState& op_state, map<string, bufferlist>* attrs, const DoutPrefixProvider* dpp,
                    std::string* err_msg)
{
    ...
    int shards_num = bucket_info.layout.current_index.layout.normal.num_shards
                         ? bucket_info.layout.current_index.layout.normal.num_shards
                         : 1;
...
int rgw_object_get_attr(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store, const RGWBucketInfo& bucket_info,
                        const rgw_obj& obj, const char* attr_name, bufferlist& out_bl, optional_yield y)
{
    RGWObjectCtx obj_ctx(store);
...
int RGWBucketAdminOp::dump_s3_policy(rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state, ostream& os,
                                     const DoutPrefixProvider* dpp)
{
...
int RGWBucketAdminOp::link(rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state,
                           const DoutPrefixProvider* dpp, string* err)
{
    RGWBucket bucket;
    map<string, bufferlist> attrs;
...
int RGWBucketAdminOp::chown(rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state, const string& marker,
                            const DoutPrefixProvider* dpp, string* err)
{
    RGWBucket bucket;
    map<string, bufferlist> attrs;
...
int RGWBucketAdminOp::check_index_unlinked(rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state,
                                           RGWFormatterFlusher& flusher, const DoutPrefixProvider* dpp)
{
    flusher.start(0);
    RGWBucket bucket;
...
int RGWBucketAdminOp::remove_bucket(rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state, optional_yield y,
                                    const DoutPrefixProvider* dpp, bool bypass_gc, bool keep_index_consistent)
{
    std::unique_ptr<rgw::sal::RGWBucket> bucket;
    std::unique_ptr<rgw::sal::RGWUser> user = store->get_user(op_state.get_user_id());

    int ret = store->get_bucket(dpp, user.get(), user->get_tenant(), op_state.get_bucket_name(), &bucket, y);
    if (ret < 0) return ret;

    if (bypass_gc)
        ret = rgw_remove_bucket_bypass_gc(
            store, bucket->get_key(), op_state.get_max_aio(), keep_index_consistent, y, dpp);
    else
        ret = bucket->remove_bucket(dpp, op_state.will_delete_children(), false, nullptr, y);
...
int RGWBucketAdminOp::sync_bucket(rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state,
                                  const DoutPrefixProvider* dpp, string* err_msg)
{
    RGWBucket bucket;
    map<string, bufferlist> attrs;
...
static int bucket_stats(rgw::sal::RGWRadosStore* store, const std::string& tenant_name, const std::string& bucket_name,
                        Formatter* formatter, const DoutPrefixProvider* dpp)
{
    ...
    real_time mtime;
    int r = store->getRados()->get_bucket_info(
        store->svc(), tenant_name, bucket_name, bucket_info, &mtime, null_yield, dpp, &attrs);
    if (r < 0) {
        return r;
    }
    ...
    string bucket_ver, master_ver;
    string max_marker;
    int ret = store->getRados()->get_bucket_stats(
        dpp, bucket_info, RGW_NO_SHARD, &bucket_ver, &master_ver, stats, &max_marker);
    if (ret < 0) {
...
int RGWBucketAdminOp::limit_check(rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state,
                                  const std::list<std::string>& user_ids, RGWFormatterFlusher& flusher,
                                  optional_yield y, const DoutPrefixProvider* dpp, bool warnings_only)
{
    int ret = 0;
    const size_t max_entries = store->ctx()->_conf->rgw_list_buckets_max_chunk;

    const size_t safe_max_objs_per_shard = store->ctx()->_conf->rgw_safe_max_objects_per_shard;

    uint16_t shard_warn_pct = store->ctx()->_conf->rgw_shard_warning_threshold;
    if (shard_warn_pct > 100) shard_warn_pct = 90;
    ...
    for (const auto& user_id : user_ids) {
        ...
        do {
            ...
            for (const auto& iter : m_buckets) {
                ...
                ret = store->getRados()->get_bucket_info(
                    store->svc(), bucket->get_tenant(), bucket->get_name(), info, nullptr, null_yield, dpp);
                if (ret < 0) continue;

                /* need stats for num_entries */
                string bucket_ver, master_ver;
                std::map<RGWObjCategory, RGWStorageStats> stats;
                ret = store->getRados()->get_bucket_stats(
                ...
                uint64_t objs_per_shard = (num_shards) ? num_objects / num_shards : num_objects;
                {
                    bool warn;
                    stringstream ss;
                           ...
                           RGWFormatterFlusher& flusher, optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    if (op_state.is_user_op()) {
        ...
        do {
            ret = user.list_buckets(dpp, marker, empty_end_marker, max_entries, no_need_stats, buckets, y);
            if (ret < 0) {
                return ret;
            }
        ...
        while (ret == 0 && truncated) {
            ...
            ret = store->ctl()->meta.mgr->list_keys_next(handle, max_keys, buckets, &truncated);
...
static int purge_bucket_instance(rgw::sal::RGWRadosStore* store, const RGWBucketInfo& bucket_info,
                                 const DoutPrefixProvider* dpp)
{
    int max_shards = (bucket_info.layout.current_index.layout.normal.num_shards > 0
                          ? bucket_info.layout.current_index.layout.normal.num_shards
                          : 1);
...
void get_stale_instances(rgw::sal::RGWRadosStore* store, const std::string& bucket_name, const vector<std::string>& lst,
                         bucket_instance_ls& stale_instances, const DoutPrefixProvider* dpp)
{
    ...
    for (const auto& bucket_instance : lst) {
        ...
        int r = store->getRados()->get_bucket_instance_info(
            obj_ctx, bucket_instance, binfo, nullptr, nullptr, null_yield, dpp);
        if (r < 0) {
            // this can only happen if someone deletes us right when we're processing
            ldpp_dout(dpp, -1) << "Bucket instance is invalid: " << bucket_instance << cpp_strerror(-r) << dendl;
            continue;
        }
    ...
    if (r < 0) {
        if (r == -ENOENT) {
            ...
            ldpp_dout(dpp, -1) << "error: reading bucket info for bucket: " << bucket << cpp_strerror(-r) << dendl;
        }
        return;
    }

    // Don't process further in this round if bucket is resharding
    if (cur_bucket_info.reshard_status == cls_rgw_reshard_status::IN_PROGRESS) return;

    other_instances.erase(std::remove_if(other_instances.begin(),
                                         other_instances.end(),
    ...
    // with these
    {
        ...
        if (r < 0) {
            ...
            ldpp_dout(dpp, 5) << __func__ << "failed to take reshard lock; reshard underway likey" << dendl;
            return;
        }
...
static int process_stale_instances(
    rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state, RGWFormatterFlusher& flusher,
    const DoutPrefixProvider* dpp,
    std::function<void(const bucket_instance_ls&, Formatter*, rgw::sal::RGWRadosStore*)> process_f)
{
    std::string marker;
    void* handle;
    Formatter* formatter = flusher.get_formatter();
    ...
    do {
        ...
        if (ret != -ENOENT) {
            ...
            for (auto& key : keys) {
                auto pos = key.find(':');
                if (pos != std::string::npos) bucket_instance_map[key.substr(0, pos)].emplace_back(std::move(key));
            }
...
int RGWBucketAdminOp::list_stale_instances(rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state,
                                           RGWFormatterFlusher& flusher, const DoutPrefixProvider* dpp)
{
    auto process_f = [](const bucket_instance_ls& lst, Formatter* formatter, rgw::sal::RGWRadosStore*) {
        for (const auto& binfo : lst) formatter->dump_string("key", binfo.bucket.get_key());
    };
    return process_stale_instances(store, op_state, flusher, dpp, process_f);
}


int RGWBucketAdminOp::clear_stale_instances(rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state,
                                            RGWFormatterFlusher& flusher, const DoutPrefixProvider* dpp)
{
    auto process_f = [dpp](const bucket_instance_ls& lst, Formatter* formatter, rgw::sal::RGWRadosStore* store) {
...
static int fix_single_bucket_lc(rgw::sal::RGWRadosStore* store, const std::string& tenant_name,
                                const std::string& bucket_name, const DoutPrefixProvider* dpp)
{
    RGWBucketInfo bucket_info;
    map<std::string, bufferlist> bucket_attrs;
    int ret = store->getRados()->get_bucket_info(
        store->svc(), tenant_name, bucket_name, bucket_info, nullptr, null_yield, dpp, &bucket_attrs);
    if (ret < 0) {
    ...
    return rgw::lc::fix_lc_shard_entry(dpp, store, store->get_rgwlc()->get_lc(), bucket_info, bucket_attrs);
}

static void format_lc_status(Formatter* formatter, const std::string& tenant_name, const std::string& bucket_name,
                             int status)
{
...
static void process_single_lc_entry(rgw::sal::RGWRadosStore* store, Formatter* formatter,
...
int RGWBucketAdminOp::fix_lc_shards(rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state,
                                    RGWFormatterFlusher& flusher, const DoutPrefixProvider* dpp)
{
    std::string marker;
    void* handle;
    ...
    bool truncated;
    if (const std::string& bucket_name = op_state.get_bucket_name(); !bucket_name.empty()) {
        const rgw_user user_id = op_state.get_user_id();
...
static bool has_object_expired(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                               const RGWBucketInfo& bucket_info, const rgw_obj_key& key, utime_t& delete_at)
{
...
static int fix_bucket_obj_expiry(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                 const RGWBucketInfo& bucket_info, RGWFormatterFlusher& flusher, bool dry_run)
{
    ...
    do {
        ...
        int ret = list_op.list_objects(dpp, listing_max_entries, &objs, nullptr, &is_truncated, null_yield);
        if (ret < 0) {
...
int RGWBucketAdminOp::fix_obj_expiry(rgw::sal::RGWRadosStore* store, RGWBucketAdminOpState& op_state,
                                     RGWFormatterFlusher& flusher, const DoutPrefixProvider* dpp, bool dry_run)
{
...
public:
    ...
    void init(RGWSI_Bucket* bucket_svc, RGWBucketCtl* bucket_ctl) override
    {
        base_init(bucket_svc->ctx(), bucket_svc->get_ep_be_handler().get());
    ...
    int do_get(RGWSI_MetaBackend_Handler::Op* op, string& entry, RGWMetadataObject** obj, optional_yield y,
               const DoutPrefixProvider* dpp) override
    {
        ...
        RGWBucketEntryMetadataObject* mdo =
            new RGWBucketEntryMetadataObject(be, ot.read_version, mtime, std::move(attrs));
    ...
    int call(std::function<int(RGWSI_Bucket_EP_Ctx& ctx)> f) { return call(nullopt, f); }

    int call(std::optional<RGWSI_MetaBackend_CtxParams> bectx_params, std::function<int(RGWSI_Bucket_EP_Ctx& ctx)> f)
    {
...
public:
    RGWMetadataHandlerPut_Bucket(RGWBucketMetadataHandler* _handler, RGWSI_MetaBackend_Handler::Op* op, string& entry,
                                 RGWMetadataObject* _obj, RGWObjVersionTracker& objv_tracker, optional_yield y,
                                 RGWMDLogSyncType type, bool from_remote_zone)
        : RGWMetadataHandlerPut_SObj(_handler, op, entry, obj, objv_tracker, y, type, from_remote_zone)
        , bhandler(_handler)
    {
    ...
    void encode_obj(bufferlist* bl) override { obj->get_ep().encode(*bl); }

    int put_checked(const DoutPrefixProvider* dpp) override;
    int put_post(const DoutPrefixProvider* dpp) override;
};

int RGWBucketMetadataHandler::do_put(RGWSI_MetaBackend_Handler::Op* op, string& entry, RGWMetadataObject* obj,
                                     RGWObjVersionTracker& objv_tracker, optional_yield y,
                                     const DoutPrefixProvider* dpp, RGWMDLogSyncType type, bool from_remote_zone)
{
...
int RGWMetadataHandlerPut_Bucket::put_checked(const DoutPrefixProvider* dpp)
{
    ...
    return bhandler->svc.bucket->store_bucket_entrypoint_info(
        ctx, entry, be, false, mtime, pattrs, &objv_tracker, y, dpp);
}
...
struct archive_meta_info
{
    ...
    void store_in_attrs(map<string, bufferlist>& attrs) const { encode(attrs[ARCHIVE_META_ATTR]); }
...
public:
                  ...
                  optional_yield y, const DoutPrefixProvider* dpp) override
    {
        ...
        ldpp_dout(dpp, 5) << "SKIP: bucket removal is not allowed on archive zone: bucket:" << entry
        ...
        ret = ctl.bucket->read_bucket_instance_info(
            be.bucket,
            &old_bi,
            y,
            dpp,
            RGWBucketCtl::BucketInstance::GetParams().set_mtime(&orig_mtime).set_attrs(&attrs_m));
        if (ret < 0) {
            return ret;
        }
        ...
        ret = ctl.bucket->store_bucket_instance_info(be.bucket,
                                                     new_bi,
                                                     y,
                                                     dpp,
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: failed to put new bucket instance info for bucket=" << new_bi.bucket
                              << " ret=" << ret << dendl;
            return ret;
        }
        ...
        ret = svc.bucket->store_bucket_entrypoint_info(
            ctx, RGWSI_Bucket::get_entrypoint_meta_key(new_be.bucket), new_be, true, mtime, &attrs, nullptr, y, dpp);
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: failed to put new bucket entrypoint for bucket=" << new_be.bucket
                              << " ret=" << ret << dendl;
            return ret;
        }
        ...
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: failed to link new bucket for bucket=" << new_be.bucket << " ret=" << ret
                              << dendl;
            return ret;
        }
        ...
        ret = svc.bucket->remove_bucket_entrypoint_info(
            ctx, RGWSI_Bucket::get_entrypoint_meta_key(be.bucket), &objv_tracker, y, dpp);
        if (ret < 0) {
            ldpp_dout(dpp, 0) << "ERROR: failed to put new bucket entrypoint for bucket=" << new_be.bucket
                              << " ret=" << ret << dendl;
            return ret;
               ...
               RGWMDLogSyncType type, bool from_remote_zone) override
    {
        ...
        return RGWBucketMetadataHandler::do_put(op, entry, obj, objv_tracker, y, dpp, type, from_remote_zone);
    }
};

class RGWBucketInstanceMetadataHandler : public RGWBucketInstanceMetadataHandlerBase
{
    int read_bucket_instance_entry(RGWSI_Bucket_BI_Ctx& ctx, const string& entry, RGWBucketCompleteInfo* bi,
                                   ceph::real_time* pmtime, optional_yield y, const DoutPrefixProvider* dpp)
    {
        return svc.bucket->read_bucket_instance_info(ctx, entry, &bi->info, pmtime, &bi->attrs, y, dpp);
    }
...
public:
    ...
    void init(RGWSI_Zone* zone_svc, RGWSI_Bucket* bucket_svc, RGWSI_BucketIndex* bi_svc) override
    {
        base_init(bucket_svc->ctx(), bucket_svc->get_bi_be_handler().get());
        svc.zone = zone_svc;
        svc.bucket = bucket_svc;
        svc.bi = bi_svc;
    }
               ...
               const DoutPrefixProvider* dpp) override
    {
        ...
        RGWBucketInstanceMetadataObject* mdo =
            new RGWBucketInstanceMetadataObject(bci, bci.info.objv_tracker.read_version, mtime);
    ...
    int do_put(RGWSI_MetaBackend_Handler::Op* op, string& entry, RGWMetadataObject* _obj,
               RGWObjVersionTracker& objv_tracker, optional_yield y, const DoutPrefixProvider* dpp,
    ...
    int call(std::function<int(RGWSI_Bucket_BI_Ctx& ctx)> f) { return call(nullopt, f); }

    int call(std::optional<RGWSI_MetaBackend_CtxParams> bectx_params, std::function<int(RGWSI_Bucket_BI_Ctx& ctx)> f)
    {
        return be_handler->call(bectx_params, [&](RGWSI_MetaBackend_Handler::Op* op) {
            RGWSI_Bucket_BI_Ctx ctx(op->ctx());
            return f(ctx);
...
public:
    RGWMetadataHandlerPut_BucketInstance(CephContext* _cct, RGWBucketInstanceMetadataHandler* _handler,
                                         RGWSI_MetaBackend_Handler::Op* _op, string& entry, RGWMetadataObject* _obj,
                                         RGWObjVersionTracker& objv_tracker, optional_yield y, RGWMDLogSyncType type,
                                         bool from_remote_zone)
        : RGWMetadataHandlerPut_SObj(_handler, _op, entry, obj, objv_tracker, y, type, from_remote_zone)
        , cct(_cct)
        , bihandler(_handler)
    {
    ...
    void encode_obj(bufferlist* bl) override { obj->get_bucket_info().encode(*bl); }
...
int RGWBucketInstanceMetadataHandler::do_put(RGWSI_MetaBackend_Handler::Op* op, string& entry, RGWMetadataObject* obj,
                                             RGWObjVersionTracker& objv_tracker, optional_yield y,
                                             const DoutPrefixProvider* dpp, RGWMDLogSyncType type,
                                             bool from_remote_zone)
{
    RGWMetadataHandlerPut_BucketInstance put_op(
        svc.bucket->ctx(), this, op, entry, obj, objv_tracker, y, type, from_remote_zone);
    return do_put_operate(&put_op, dpp);
}

void init_default_bucket_layout(CephContext* cct, rgw::BucketLayout& layout, const RGWZone& zone,
                                std::optional<uint32_t> shards, std::optional<rgw::BucketIndexType> type)
{
    layout.current_index.gen = 0;
    layout.current_index.layout.normal.hash_type = rgw::BucketHashType::Mod;

    layout.current_index.layout.type = type.value_or(rgw::BucketIndexType::Normal);
    ...
    if (layout.current_index.layout.type == rgw::BucketIndexType::Normal) {
        layout.logs.push_back(log_layout_from_index(layout.current_index.gen, layout.current_index.layout.normal));
    }
}
...
int RGWMetadataHandlerPut_BucketInstance::put_check(const DoutPrefixProvider* dpp)
{
    ...
    if (from_remote_zone) {
        ...
        if (!exists) {
            ...
            init_default_bucket_layout(cct, bci.info.layout, bihandler->svc.zone->get_zone(), num_shards, index_type);
        }
        else {
...
int RGWMetadataHandlerPut_BucketInstance::put_checked(const DoutPrefixProvider* dpp)
{
    ...
    return bihandler->svc.bucket->store_bucket_instance_info(ctx, entry, info, orig_info, false, mtime, pattrs, y, dpp);
}
...
public:
    ...
    int do_remove(RGWSI_MetaBackend_Handler::Op* op, string& entry, RGWObjVersionTracker& objv_tracker,
                  optional_yield y, const DoutPrefixProvider* dpp) override
    {
        ldpp_dout(dpp, 0) << "SKIP: bucket instance removal is not allowed on archive zone: bucket.instance:" << entry
                          << dendl;
        return 0;
    }
};

RGWBucketCtl::RGWBucketCtl(RGWSI_Zone* zone_svc, RGWSI_Bucket* bucket_svc, RGWSI_Bucket_Sync* bucket_sync_svc,
...
void RGWBucketCtl::init(RGWUserCtl* user_ctl, RGWBucketMetadataHandler* _bm_handler,
                        RGWBucketInstanceMetadataHandler* _bmi_handler, RGWDataChangesLog* datalog,
                        const DoutPrefixProvider* dpp)
{
    ctl.user = user_ctl;
    ...
    datalog->set_bucket_filter([this](const rgw_bucket& bucket, optional_yield y, const DoutPrefixProvider* dpp) {
...
int RGWBucketCtl::read_bucket_entrypoint_info(const rgw_bucket& bucket, RGWBucketEntryPoint* info, optional_yield y,
                                              const DoutPrefixProvider* dpp, const Bucket::GetParams& params)
{
...
int RGWBucketCtl::store_bucket_entrypoint_info(const rgw_bucket& bucket, RGWBucketEntryPoint& info, optional_yield y,
                                               const DoutPrefixProvider* dpp, const Bucket::PutParams& params)
{
...
int RGWBucketCtl::remove_bucket_entrypoint_info(const rgw_bucket& bucket, optional_yield y,
                                                const DoutPrefixProvider* dpp, const Bucket::RemoveParams& params)
{
    return bm_handler->call([&](RGWSI_Bucket_EP_Ctx& ctx) {
        return svc.bucket->remove_bucket_entrypoint_info(
            ctx, RGWSI_Bucket::get_entrypoint_meta_key(bucket), params.objv_tracker, y, dpp);
    });
}

int RGWBucketCtl::read_bucket_instance_info(const rgw_bucket& bucket, RGWBucketInfo* info, optional_yield y,
                                            const DoutPrefixProvider* dpp, const BucketInstance::GetParams& params)
{
...
int RGWBucketCtl::read_bucket_info(const rgw_bucket& bucket, RGWBucketInfo* info, optional_yield y,
                                   const DoutPrefixProvider* dpp, const BucketInstance::GetParams& params,
                                   RGWObjVersionTracker* ep_objv_tracker)
{
    ...
    if (b->bucket_id.empty()) {
        ...
        int r = read_bucket_entrypoint_info(
            *b,
            &(*ep),
            y,
            dpp,
            RGWBucketCtl::Bucket::GetParams().set_bectx_params(params.bectx_params).set_objv_tracker(ep_objv_tracker));
        if (r < 0) {
            return r;
        }
...
int RGWBucketCtl::do_store_bucket_instance_info(RGWSI_Bucket_BI_Ctx& ctx, const rgw_bucket& bucket, RGWBucketInfo& info,
...
int RGWBucketCtl::store_bucket_instance_info(const rgw_bucket& bucket, RGWBucketInfo& info, optional_yield y,
                                             const DoutPrefixProvider* dpp, const BucketInstance::PutParams& params)
{
    return bmi_handler->call(
        [&](RGWSI_Bucket_BI_Ctx& ctx) { return do_store_bucket_instance_info(ctx, bucket, info, y, dpp, params); });
}

int RGWBucketCtl::remove_bucket_instance_info(const rgw_bucket& bucket, RGWBucketInfo& info, optional_yield y,
                                              const DoutPrefixProvider* dpp, const BucketInstance::RemoveParams& params)
{
    if (params.objv_tracker) {
        info.objv_tracker = *params.objv_tracker;
    }
    ...
    return bmi_handler->call([&](RGWSI_Bucket_BI_Ctx& ctx) {
        return svc.bucket->remove_bucket_instance_info(
            ctx, RGWSI_Bucket::get_bi_meta_key(bucket), info, &info.objv_tracker, y, dpp);
    });
}

int RGWBucketCtl::do_store_linked_bucket_info(RGWSI_Bucket_X_Ctx& ctx, RGWBucketInfo& info, RGWBucketInfo* orig_info,
                                              bool exclusive, real_time mtime, obj_version* pep_objv,
                                              map<string, bufferlist>* pattrs, bool create_entry_point,
                                              optional_yield y, const DoutPrefixProvider* dpp)
{
    bool create_head = !info.has_instance_obj || create_entry_point;

    int ret = svc.bucket->store_bucket_instance_info(
        ctx.bi, RGWSI_Bucket::get_bi_meta_key(info.bucket), info, orig_info, exclusive, mtime, pattrs, y, dpp);
    if (ret < 0) {
        return ret;
    }
    ...
    ret = svc.bucket->store_bucket_entrypoint_info(
        ctx.ep, RGWSI_Bucket::get_entrypoint_meta_key(info.bucket), entry_point, exclusive, mtime, pattrs, &ot, y, dpp);
    if (ret < 0) return ret;

    return 0;
}
int RGWBucketCtl::convert_old_bucket_info(RGWSI_Bucket_X_Ctx& ctx, const rgw_bucket& bucket, optional_yield y,
                                          const DoutPrefixProvider* dpp)
{
    RGWBucketEntryPoint entry_point;
    real_time ep_mtime;
    ...
    int ret = svc.bucket->read_bucket_entrypoint_info(
        ctx.ep, RGWSI_Bucket::get_entrypoint_meta_key(bucket), &entry_point, &ot, &ep_mtime, &attrs, y, dpp);
    if (ret < 0) {
...
int RGWBucketCtl::set_bucket_instance_attrs(RGWBucketInfo& bucket_info, map<string, bufferlist>& attrs,
                                            RGWObjVersionTracker* objv_tracker, optional_yield y,
                                            const DoutPrefixProvider* dpp)
{
    return call([&](RGWSI_Bucket_X_Ctx& ctx) {
        ...
        return do_store_bucket_instance_info(
            ctx.bi,
            bucket,
            bucket_info,
            y,
            dpp,
            BucketInstance::PutParams().set_attrs(&attrs).set_objv_tracker(objv_tracker).set_orig_info(&bucket_info));
    });
}


int RGWBucketCtl::link_bucket(const rgw_user& user_id, const rgw_bucket& bucket, ceph::real_time creation_time,
                              optional_yield y, const DoutPrefixProvider* dpp, bool update_entrypoint,
                              rgw_ep_info* pinfo)
{
    return bm_handler->call([&](RGWSI_Bucket_EP_Ctx& ctx) {
        return do_link_bucket(ctx, user_id, bucket, creation_time, update_entrypoint, pinfo, y, dpp);
    });
}

int RGWBucketCtl::do_link_bucket(RGWSI_Bucket_EP_Ctx& ctx, const rgw_user& user_id, const rgw_bucket& bucket,
                                 ceph::real_time creation_time, bool update_entrypoint, rgw_ep_info* pinfo,
                                 optional_yield y, const DoutPrefixProvider* dpp)
{
    int ret;
    ...
    if (update_entrypoint) {
        ...
        if (pinfo) {
            ...
            ret = svc.bucket->read_bucket_entrypoint_info(ctx, meta_key, &ep, &rot, nullptr, &attrs, y, dpp);
            if (ret < 0 && ret != -ENOENT) {
                ldpp_dout(dpp, 0) << "ERROR: store->get_bucket_entrypoint_info() returned: " << cpp_strerror(-ret)
                                  << dendl;
            }
            pattrs = &attrs;
        }
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR: error adding bucket to user directory:"
                          << " user=" << user_id << " bucket=" << bucket << " err=" << cpp_strerror(-ret) << dendl;
        goto done_err;
    }

    if (!update_entrypoint) return 0;

    ep.linked = true;
    ep.owner = user_id;
    ep.bucket = bucket;
    ret = svc.bucket->store_bucket_entrypoint_info(ctx, meta_key, ep, false, real_time(), pattrs, &rot, y, dpp);
    if (ret < 0) goto done_err;
...
done_err:
    ...
    if (r < 0) {
        ldpp_dout(dpp, 0) << "ERROR: failed unlinking bucket on error cleanup: " << cpp_strerror(-r) << dendl;
    }
    return ret;
}

int RGWBucketCtl::unlink_bucket(const rgw_user& user_id, const rgw_bucket& bucket, optional_yield y,
                                const DoutPrefixProvider* dpp, bool update_entrypoint)
{
    return bm_handler->call(
        [&](RGWSI_Bucket_EP_Ctx& ctx) { return do_unlink_bucket(ctx, user_id, bucket, update_entrypoint, y, dpp); });
}

int RGWBucketCtl::do_unlink_bucket(RGWSI_Bucket_EP_Ctx& ctx, const rgw_user& user_id, const rgw_bucket& bucket,
                                   bool update_entrypoint, optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    if (!ep.linked) return 0;

    if (ep.owner != user_id) {
        ldpp_dout(dpp, 0) << "bucket entry point user mismatch, can't unlink bucket: " << ep.owner << " != " << user_id
                          << dendl;
        return -EINVAL;
    }
...
int RGWBucketCtl::set_acl(ACLOwner& owner, rgw_bucket& bucket, RGWBucketInfo& bucket_info, bufferlist& bl,
                          optional_yield y, const DoutPrefixProvider* dpp)
{
    // set owner and acl
    ...
    int r = store_bucket_instance_info(bucket, bucket_info, y, dpp, BucketInstance::PutParams().set_attrs(&attrs));
    if (r < 0) {
...
int RGWBucketCtl::chown(rgw::sal::RGWRadosStore* store, RGWBucketInfo& bucket_info, const rgw_user& user_id,
                        const std::string& display_name, const std::string& marker, optional_yield y,
                        const DoutPrefixProvider* dpp)
{
    std::vector<rgw_bucket_dir_entry> objs;
    map<string, bool> common_prefixes;
    ...
    do {
        ...
        for (const auto& obj : objs) {
            ...
            if (aiter == attrs.end()) {
                ldpp_dout(dpp, 0) << "ERROR: no acls found for object " << obj.key.name
...
int RGWBucketCtl::read_bucket_stats(const rgw_bucket& bucket, RGWBucketEnt* result, optional_yield y,
                                    const DoutPrefixProvider* dpp)
{
    return call([&](RGWSI_Bucket_X_Ctx& ctx) { return svc.bucket->read_bucket_stats(ctx, bucket, result, y, dpp); });
}

int RGWBucketCtl::read_buckets_stats(map<string, RGWBucketEnt>& m, optional_yield y, const DoutPrefixProvider* dpp)
{
    return call([&](RGWSI_Bucket_X_Ctx& ctx) { return svc.bucket->read_buckets_stats(ctx, m, y, dpp); });
}

int RGWBucketCtl::sync_user_stats(const DoutPrefixProvider* dpp, const rgw_user& user_id,
                                  const RGWBucketInfo& bucket_info, optional_yield y, RGWBucketEnt* pent)
{
    RGWBucketEnt ent;
    if (!pent) {
        pent = &ent;
...
int RGWBucketCtl::get_sync_policy_handler(std::optional<rgw_zone_id> zone, std::optional<rgw_bucket> bucket,
                                          RGWBucketSyncPolicyHandlerRef* phandler, optional_yield y,
                                          const DoutPrefixProvider* dpp)
{
    ...
    if (r < 0) {
        ldpp_dout(dpp, 20) << __func__ << "(): failed to get policy handler for bucket=" << bucket << " (r=" << r << ")"
                           << dendl;
        return r;
    }
    return 0;
...
int RGWBucketCtl::bucket_exports_data(const rgw_bucket& bucket, optional_yield y, const DoutPrefixProvider* dpp)
{
...
int RGWBucketCtl::bucket_imports_data(const rgw_bucket& bucket, optional_yield y, const DoutPrefixProvider* dpp)
{
