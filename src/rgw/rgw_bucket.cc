...
void init_bucket(rgw_bucket* b, const char* t, const char* n, const char* dp, const char* ip, const char* m,
                 const char* id)
{
    b->tenant = t;
    b->name = n;
...
int rgw_chown_bucket_and_objects(rgw::sal::Driver* driver, rgw::sal::Bucket* bucket, rgw::sal::User* new_user,
                                 const std::string& marker, std::string* err_msg, const DoutPrefixProvider* dpp,
                                 optional_yield y)
{
    /* Chown on the bucket */
    ...
    do {
        ...
        cerr << count << " objects processed in " << bucket << ". Next marker " << params.marker.name << std::endl;
