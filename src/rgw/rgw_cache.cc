...
int ObjectCache::get(const DoutPrefixProvider* dpp, const string& name, ObjectCacheInfo& info, uint32_t mask,
                     rgw_cache_entry_info* cache_info)
{
    ...
    if (expiry.count() && (ceph::coarse_mono_clock::now() - iter->second.info.time_added) > expiry) {
        ...
        if (iter != cache_map.end()) {
            for (auto& kv : iter->second.chained_entries) kv.first->invalidate(kv.second);
    ...
    if ((src.flags & mask) != mask) {
        ldpp_dout(dpp, 10) << "cache get: name=" << name << " : type miss (requested=0x" << std::hex << mask
                           << ", cached=0x" << src.flags << std::dec << ")" << dendl;
        if (perfcounter) perfcounter->inc(l_rgw_cache_miss);
        return -ENOENT;
    }
    ldpp_dout(dpp, 10) << "cache get: name=" << name << " : hit (requested=0x" << std::hex << mask << ", cached=0x"
                       << src.flags << std::dec << ")" << dendl;
                                    ...
                                    RGWChainedCache::Entry* chained_entry)
{
    ...
    for (auto cache_info : cache_info_entries) {
        ldpp_dout(dpp, 10) << "chain_cache_entry: cache_locator=" << cache_info->cache_locator << dendl;
        ...
        if (entry->gen != cache_info->gen) {
            ldpp_dout(dpp, 20) << "chain_cache_entry: entry.gen (" << entry->gen << ") != cache_info.gen ("
                               << cache_info->gen << ")" << dendl;
            return false;
        }
...
void ObjectCache::put(const DoutPrefixProvider* dpp, const string& name, ObjectCacheInfo& info,
                      rgw_cache_entry_info* cache_info)
{
    std::unique_lock l{lock};
    ...
    ldpp_dout(dpp, 10) << "cache put: name=" << name << " info.flags=0x" << std::hex << info.flags << std::dec << dendl;
    ...
    if (info.flags & CACHE_FLAG_XATTRS) {
        ...
        for (iter = target.xattrs.begin(); iter != target.xattrs.end(); ++iter) {
            ldpp_dout(dpp, 10) << "updating xattr: name=" << iter->first << " bl.length()=" << iter->second.length()
                               << dendl;
        }
    }
    else if (info.flags & CACHE_FLAG_MODIFY_XATTRS) {
        ...
        for (iter = info.xattrs.begin(); iter != info.xattrs.end(); ++iter) {
            ldpp_dout(dpp, 10) << "appending xattr: name=" << iter->first << " bl.length()=" << iter->second.length()
                               << dendl;
            target.xattrs[iter->first] = iter->second;
        }
    }

    if (info.flags & CACHE_FLAG_DATA) target.data = info.data;

    if (info.flags & CACHE_FLAG_OBJV) target.version = info.version;
}
...
void ObjectCache::remove_lru(const string& name, std::list<string>::iterator& lru_iter)
{
    if (lru_iter == lru.end()) return;
...
{
    for (auto iter = entry.chained_entries.begin(); iter != entry.chained_entries.end(); ++iter) {
