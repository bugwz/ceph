...
rgw_bucket::rgw_bucket(const rgw_user& u, const cls_user_bucket& b)
    ...
    , explicit_placement(b.explicit_placement.data_pool, b.explicit_placement.data_extra_pool,
...
std::string rgw_bucket::get_key(char tenant_delim, char id_delim, size_t reserve) const
{
    const size_t max_len =
        tenant.size() + sizeof(tenant_delim) + name.size() + sizeof(id_delim) + bucket_id.size() + reserve;
...
std::string rgw_bucket_shard::get_key(char tenant_delim, char id_delim, char shard_delim, size_t reserve) const
{
