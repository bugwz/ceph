...
static inline int stringtoll(const std::string& s, int64_t* val)
{
    ...
    if (result == LLONG_MAX) return -EINVAL;
...
static inline int stringtoull(const std::string& s, uint64_t* val)
{
    ...
    if (result == ULLONG_MAX) return -EINVAL;
...
static inline int stringtol(const std::string& s, int32_t* val)
{
    ...
    if (result == LONG_MAX) return -EINVAL;
...
static inline int stringtoul(const std::string& s, uint32_t* val)
{
    ...
    if (result == ULONG_MAX) return -EINVAL;
...
static inline boost::container::small_vector<char, N> sview2cstr(const std::string_view& sv)
{
...
template<size_t N> static inline constexpr size_t sarrlen(const char (&arr)[N])
{
    return N - 1;
}
...
static inline constexpr size_t sum()
{
    return 0;
}
template<typename... Args> constexpr size_t sum(size_t v, Args... args)
{
    return v + sum(args...);
}
...
template<> struct string_traits<char*> : string_traits<const char*>
{};
// constexpr specializations for char[]/const char[]
template<std::size_t N> struct string_traits<const char[N]>
{
...
template<std::size_t N> struct string_traits<char[N]> : string_traits<const char[N]>
{};

// helpers for string_cat_reserve()
static inline void append_to(std::string& s) {}
template<typename... Args> void append_to(std::string& s, const std::string_view& v, const Args&... args)
{
...
void join_next(std::string& s, const std::string_view& d, const std::string_view& v, const Args&... args)
{
...
template<typename... Args>
void join(std::string& s, const std::string_view& d, const std::string_view& v, const Args&... args)
{
    s.append(v.begin(), v.end());
    join_next(s, d, args...);
}
...
template<typename T> constexpr size_t string_size(const T& s)
{
...
template<typename... Args> std::string string_cat_reserve(const Args&... args)
{
...
template<typename... Args> std::string string_join_reserve(const std::string_view& delim, const Args&... args)
{
...
template<typename... Args> std::string string_join_reserve(char delim, const Args&... args)
{
...
extern bool match_wildcards(std::string_view pattern, std::string_view input, uint32_t flags = 0);
