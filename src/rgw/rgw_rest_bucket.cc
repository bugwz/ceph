...
public:
    ...
    int check_caps(const RGWUserCaps& caps) override { return caps.check_cap("buckets", RGW_CAP_READ); }
...
public:
    ...
    int check_caps(const RGWUserCaps& caps) override { return caps.check_cap("buckets", RGW_CAP_WRITE); }
...
void RGWOp_Set_Bucket_Quota::execute(optional_yield y)
{
    ...
    if (use_http_params) {
        ...
        op_ret = store->getRados()->get_bucket_info(
            store->svc(), uid.tenant, bucket, bucket_info, NULL, s->yield, s, &attrs);
        if (op_ret < 0) {
            return;
        }
...
RGWOp* RGWHandler_Bucket::op_get()
{
    ...
    if (s->info.args.sub_resource_exists("policy")) return new RGWOp_Get_Policy;

    if (s->info.args.sub_resource_exists("index")) return new RGWOp_Check_Bucket_Index;
...
{
    if (s->info.args.sub_resource_exists("quota")) return new RGWOp_Set_Bucket_Quota;

    if (s->info.args.sub_resource_exists("sync")) return new RGWOp_Sync_Bucket;
...
{
    if (s->info.args.sub_resource_exists("object")) return new RGWOp_Object_Remove;
