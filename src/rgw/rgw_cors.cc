...
{
    if ((s.find("*") != s.end()) || (s.find(h) != s.end())) {
        return true;
    }
    ...
    for (set<string>::iterator it = s.begin(); it != s.end(); ++it) {
        ...
        if ((off = (*it).find("*")) != string::npos) {
            ...
            if (off != 0) {
                ...
                if (!boost::algorithm::starts_with(h, sl)) continue;
                ssplit.pop_front();
            }
            if (off != ((*it).length() - 1)) {
                string sl = ssplit.front();
                dout(10) << "Finding " << sl << ", in " << h << ", at offset not less than " << flen << dendl;
                if (h.size() < sl.size() || h.compare((h.size() - sl.size()), sl.size(), sl) != 0) continue;
                ssplit.pop_front();
            }
            if (!ssplit.empty()) continue;
            return true;
        }
    }
    return false;
...
{
    if (allowed_origins.find("*") != allowed_origins.end()) return true;
...
{
    for (list<RGWCORSRule>::iterator it_r = rules.begin(); it_r != rules.end(); ++it_r) {
        RGWCORSRule& r = (*it_r);
        if (r.is_origin_present(origin)) return &r;
    }
    return NULL;
}
...
void RGWCORSConfiguration::erase_host_name_rule(string& origin)
{
    ...
    for (list<RGWCORSRule>::iterator it_r = rules.begin(); it_r != rules.end(); ++it_r, loop++) {
        RGWCORSRule& r = (*it_r);
...
void RGWCORSConfiguration::dump()
{
    ...
    for (list<RGWCORSRule>::iterator it = rules.begin(); it != rules.end(); ++it, loop++) {
