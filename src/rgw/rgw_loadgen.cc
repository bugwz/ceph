...
int RGWLoadGenRequestEnv::sign(const DoutPrefixProvider* dpp, RGWAccessKey& access_key)
{
    ...
    try {
        ...
        const auto signature =
            static_cast<std::string>(rgw::auth::s3::get_v2_signature(g_ceph_context, canonical_header, access_key.key));
        headers["HTTP_AUTHORIZATION"] = std::string("AWS ") + access_key.id + ":" + signature;
    }
    catch (int ret) {
        return ret;
    }
...
size_t RGWLoadGenIO::write_data(const char* const buf, const size_t len)
{
    return len;
}
...
{
    const size_t read_len = std::min(left_to_read, static_cast<uint64_t>(len));
...
size_t RGWLoadGenIO::send_status(const int status, const char* const status_name)
{
    return 0;
}
...
size_t RGWLoadGenIO::send_header(const std::string_view& name, const std::string_view& value)
{
    return 0;
}
