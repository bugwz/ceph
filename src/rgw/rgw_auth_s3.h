                        ...
                        public rgw::auth::RoleApplier::Factory
{
    ...
    aplptr_t create_apl_remote(CephContext* const cct, const req_state* const s,
                               rgw::auth::RemoteApplier::acl_strategy_t&& acl_alg,
                               const rgw::auth::RemoteApplier::AuthInfo& info) const override
    {
        auto apl = rgw::auth::add_sysreq(cct,
                                         driver,
                                         s,
                                         rgw::auth::RemoteApplier(cct,
                                                                  driver,
                                                                  std::move(acl_alg),
                                                                  info,
    ...
    aplptr_t create_apl_local(CephContext* const cct, const req_state* const s, const RGWUserInfo& user_info,
                              const std::string& subuser, const std::optional<uint32_t>& perm_mask,
                              const std::string& access_key_id) const override
    {
        auto apl = rgw::auth::add_sysreq(
            cct, driver, s, rgw::auth::LocalApplier(cct, user_info, subuser, perm_mask, access_key_id));
        return aplptr_t(new decltype(apl)(std::move(apl)));
    }

    aplptr_t create_apl_role(CephContext* const cct, const req_state* const s, const rgw::auth::RoleApplier::Role& role,
                             const rgw::auth::RoleApplier::TokenAttrs& token_attrs) const override
    {
        auto apl = rgw::auth::add_sysreq(cct, driver, s, rgw::auth::RoleApplier(cct, role, token_attrs));
        return aplptr_t(new decltype(apl)(std::move(apl)));
    }

public:
    STSAuthStrategy(CephContext* const cct, rgw::sal::Driver* driver,
                    const rgw::auth::ImplicitTenants& implicit_tenant_context,
                    AWSEngine::VersionAbstractor* const ver_abstractor)
        : driver(driver)
        , implicit_tenant_context(implicit_tenant_context)
        , sts_engine(cct, driver, *ver_abstractor, static_cast<rgw::auth::LocalApplier::Factory*>(this),
    ...
    const char* get_name() const noexcept override { return "rgw::auth::s3::STSAuthStrategy"; }
};

class ExternalAuthStrategy : public rgw::auth::Strategy, public rgw::auth::RemoteApplier::Factory
{
    typedef rgw::auth::IdentityApplier::aplptr_t aplptr_t;
    rgw::sal::Driver* driver;
    const rgw::auth::ImplicitTenants& implicit_tenant_context;
...
public:
    ExternalAuthStrategy(CephContext* const cct, rgw::sal::Driver* driver,
                         const rgw::auth::ImplicitTenants& implicit_tenant_context,
                         AWSEngine::VersionAbstractor* const ver_abstractor)
        : driver(driver)
        , implicit_tenant_context(implicit_tenant_context)
        , ldap_engine(cct, driver, *ver_abstractor, static_cast<rgw::auth::RemoteApplier::Factory*>(this))
    {

        if (cct->_conf->rgw_s3_auth_use_keystone && !cct->_conf->rgw_keystone_url.empty()) {
    ...
    const char* get_name() const noexcept override { return "rgw::auth::s3::AWSv2ExternalAuthStrategy"; }
};


template<class AbstractorT, bool AllowAnonAccessT = false>
class AWSAuthStrategy : public rgw::auth::Strategy, public rgw::auth::LocalApplier::Factory
{
    typedef rgw::auth::IdentityApplier::aplptr_t aplptr_t;

    static_assert(std::is_base_of<rgw::auth::s3::AWSEngine::VersionAbstractor, AbstractorT>::value,
...
public:
    ...
    void add_engines(const std::vector<std::string>& auth_order, engine_map_t eng_map)
    {
        ...
        for (const auto& eng : auth_order) {
            ...
            if (const auto kv = eng_map.find(eng); kv != eng_map.end()) {
    ...
    auto parse_auth_order(CephContext* const cct)
    {
        ...
        boost::split(result, cct->_conf->rgw_s3_auth_order, boost::is_any_of(", "), boost::token_compress_on);

        if (std::any_of(result.begin(), result.end(), [allowed_auth](std::string_view s) {
    ...
    AWSAuthStrategy(CephContext* const cct, const rgw::auth::ImplicitTenants& implicit_tenant_context,
                    rgw::sal::Driver* driver)
        : driver(driver)
        , ver_abstractor(cct)
        , anonymous_engine(cct, static_cast<rgw::auth::LocalApplier::Factory*>(this))
        ...
        , local_engine(cct, driver, ver_abstractor, static_cast<rgw::auth::LocalApplier::Factory*>(this))
    {
        /* The anonymous auth. */
        if (AllowAnonAccessT) {
    ...
    const char* get_name() const noexcept override { return "rgw::auth::s3::AWSAuthStrategy"; }
};
                        ...
                        public std::enable_shared_from_this<AWSv4ComplMulti>
{
    ...
    class ChunkMeta
    {
        ...
        ChunkMeta(const size_t data_starts_in_stream, const size_t data_length, const std::string_view signature)
    ...
    public:
        ...
        static constexpr size_t META_MIN_SIZE = sarrlen("0;chunk-signature=") + SIG_SIZE + sarrlen("\r\n");
         ...
         * in a stream. */
        static ChunkMeta create_first(const std::string_view& seed_signature) { return ChunkMeta(seed_signature); }
        ...
        static std::pair<ChunkMeta, size_t> create_next(CephContext* cct, ChunkMeta&& prev, const char* metabuf,
                                                        size_t metabuf_len);
    } chunk_meta;
...
public:
    ...
    AWSv4ComplMulti(const req_state* const s, std::string_view date, std::string_view credential_scope,
                    std::string_view seed_signature, const signing_key_t& signing_key)
        : io_base_t(nullptr)
        , cct(s->cct)
        , date(std::move(date))
    ...
    /* Factories. */
    static cmplptr_t create(const req_state* s, std::string_view date, std::string_view credential_scope,
                            std::string_view seed_signature, const boost::optional<std::string>& secret_key);
};
...
public:
    ...
    /* Factories. */
    static cmplptr_t create(const req_state* s, const boost::optional<std::string>&);
};
...
void rgw_create_s3_canonical_header(const DoutPrefixProvider* dpp, const char* method, const char* content_md5,
                                    const char* content_type, const char* date, const meta_map_t& meta_map,
                                    const meta_map_t& qs_map, const char* request_uri,
                                    const std::map<std::string, std::string>& sub_resources, std::string& dest_str);
bool rgw_create_s3_canonical_header(const DoutPrefixProvider* dpp, const req_info& info, utime_t* header_time, /* out */
                                    std::string& dest,                                                         /* out */
                                    bool qsr);
static inline std::tuple<bool, std::string, utime_t> rgw_create_s3_canonical_header(const DoutPrefixProvider* dpp,
                                                                                    const req_info& info,
                                                                                    const bool qsr)
{
    std::string dest;
...
static constexpr char AWS4_EMPTY_PAYLOAD_HASH[] = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";

static constexpr char AWS4_UNSIGNED_PAYLOAD_HASH[] = "UNSIGNED-PAYLOAD";

static constexpr char AWS4_STREAMING_PAYLOAD_HASH[] = "STREAMING-AWS4-HMAC-SHA256-PAYLOAD";
...
static inline bool char_needs_aws4_escaping(const char c, bool encode_slash)
{
    ...
    if (c == '/' && !encode_slash) return false;
...
static inline const char* get_v4_exp_payload_hash(const req_info& info)
{
    ...
    const char* expected_request_payload_hash = info.env->get("HTTP_X_AMZ_CONTENT_SHA256");
...
static inline bool is_v4_payload_empty(const req_state* const s)
{
     ...
     * message-headers." */
    return s->content_length == 0 && s->info.env->get("HTTP_TRANSFER_ENCODING") == nullptr;
}
...
boost::optional<std::string> get_v4_canonical_headers(const req_info& info, const std::string_view& signedheaders,
                                                      bool using_qs, bool force_boto2_compat);

std::string gen_v4_canonical_headers(const req_info& info, const std::map<std::string, std::string>& extra_headers,
                                     string* signed_hdrs);

extern sha256_digest_t get_v4_canon_req_hash(CephContext* cct, const std::string_view& http_verb,
...
extern AWSEngine::VersionAbstractor::server_signature_t get_v4_signature(
...
extern AWSEngine::VersionAbstractor::server_signature_t get_v2_signature(
    CephContext*, const std::string& secret_key, const AWSEngine::VersionAbstractor::string_to_sign_t& string_to_sign);
} /* namespace s3 */
} /* namespace auth */
} /* namespace rgw */
