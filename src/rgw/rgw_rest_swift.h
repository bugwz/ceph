...
public:
    ...
    void set_custom_http_response(const int http_ret) { custom_http_ret = http_ret; }
...
class RGWListBuckets_ObjStore_SWIFT : public RGWListBuckets_ObjStore
{
    ...
    uint64_t get_default_max() const override { return 0; }
...
public:
    RGWListBucket_ObjStore_SWIFT() { default_max = 10000; }
...
public:
    ...
    int get_data(std::list<RGWBulkDelete::acct_path_t>& items, bool* is_truncated) override;
    void send_response() override;
};
...
class RGWFormPost : public RGWPostObj_ObjStore
{
    ...
    bool is_integral();
    bool is_non_expired();
    void get_owner_info(const req_state* s, RGWUserInfo& owner_info) const;
...
public:
    ...
    const char* calc(const std::string& key, const std::string_view& path_info, const std::string_view& redirect,
                     const std::string_view& max_file_size, const std::string_view& max_file_count,
    ...
    const char* get_signature() const { return dest_str; }
...
public:
    RGWSwiftWebsiteHandler(rgw::sal::Driver* const driver, req_state* const s, RGWHandler_REST* const handler)
        : driver(driver)
        , s(s)
        , handler(handler)
    {}

    int error_handler(const int err_no, std::string* const error_content, optional_yield y);
...
protected:
    ...
    virtual bool is_acl_op() const { return false; }

    static int init_from_header(rgw::sal::Driver* driver, req_state* s, const std::string& frontend_prefix);
...
protected:
    bool is_obj_update_op() const override { return s->op == OP_POST; }
...
public:
    ...
    int init(rgw::sal::Driver* const driver, req_state* const s, rgw::io::BasicClient* const cio) override
    {
...
{
protected:
    RGWRESTMgr* get_resource_mgr_as_default(req_state* const s, const std::string& uri,
...
class RGWGetCrossDomainPolicy_ObjStore_SWIFT : public RGWGetCrossDomainPolicy_ObjStore
{
public:
...
class RGWGetHealthCheck_ObjStore_SWIFT : public RGWGetHealthCheck_ObjStore
{
public:
...
public:
    ...
    int init(rgw::sal::Driver* const driver, req_state* const state, rgw::io::BasicClient* const cio) override
    {
        state->dialect = "swift";
...
public:
    ...
    RGWOp* op_get() override { return new RGWInfo_ObjStore_SWIFT(); }
