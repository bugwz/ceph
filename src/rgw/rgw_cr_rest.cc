...
RGWCRHTTPGetDataCB::RGWCRHTTPGetDataCB(RGWCoroutinesEnv* _env, RGWCoroutine* _cr, RGWHTTPStreamRWRequest* _req)
    : env(_env)
    , cr(_cr)
    , req(_req)
{
...
int RGWStreamReadHTTPResourceCRF::decode_rest_obj(const DoutPrefixProvider* dpp, map<string, string>& headers,
...
int RGWStreamReadHTTPResourceCRF::read(const DoutPrefixProvider* dpp, bufferlist* out, uint64_t max_size,
                                       bool* io_pending)
{
    reenter(&read_state)
    {
        io_read_mask = req->get_io_id(RGWHTTPClient::HTTPCLIENT_IO_READ | RGWHTTPClient::HTTPCLIENT_IO_CONTROL);
        while (!req->is_done() || in_cb->has_data()) {
            *io_pending = true;
            if (!in_cb->has_data()) {
...
void RGWStreamWriteHTTPResourceCRF::write_drain_notify(uint64_t pending_size)
{
    ...
    if (is_blocked && (pending_size < PENDING_WRITES_WINDOW / 2)) {
        env->manager->io_complete(
            caller, req->get_io_id(RGWHTTPClient::HTTPCLIENT_IO_WRITE | RGWHTTPClient::HTTPCLIENT_IO_CONTROL));
        is_blocked = false;
    }
}
            ...
            if (req->get_pending_send_size() >= PENDING_WRITES_WINDOW) {
                *io_pending = true;
                ...
                yield caller->io_block(
                    0, req->get_io_id(RGWHTTPClient::HTTPCLIENT_IO_WRITE | RGWHTTPClient::HTTPCLIENT_IO_CONTROL));
            }
...
RGWStreamSpliceCR::RGWStreamSpliceCR(CephContext* _cct, RGWHTTPManager* _mgr,
                                     ...
                                     shared_ptr<RGWStreamWriteHTTPResourceCRF>& _out_crf)
    : RGWCoroutine(_cct)
    , cct(_cct)
    , http_manager(_mgr)
    , in_crf(_in_crf)
