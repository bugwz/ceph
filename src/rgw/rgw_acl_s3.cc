...
void ACLPermission_S3::to_xml(ostream& out)
{
    if ((flags & RGW_PERM_FULL_CONTROL) == RGW_PERM_FULL_CONTROL) {
        ...
        if (flags & RGW_PERM_READ) out << "<Permission>READ</Permission>";
        if (flags & RGW_PERM_WRITE) out << "<Permission>WRITE</Permission>";
        if (flags & RGW_PERM_READ_ACP) out << "<Permission>READ_ACP</Permission>";
        if (flags & RGW_PERM_WRITE_ACP) out << "<Permission>WRITE_ACP</Permission>";
    }
}
...
public:
    static const char* to_string(ACLGranteeType& type)
    {
        ...
        case ACL_TYPE_CANON_USER: return "CanonicalUser";
        case ACL_TYPE_EMAIL_USER: return "AmazonCustomerByEmail";
        case ACL_TYPE_GROUP: return "Group";
...
void ACLOwner_S3::to_xml(ostream& out)
{
    ...
    out << "<Owner>"
        << "<ID>" << s << "</ID>";
    if (!display_name.empty()) out << "<DisplayName>" << display_name << "</DisplayName>";
    out << "</Owner>";
}
...
bool ACLGrant_S3::xml_end(const char* el)
{
    ...
    if (!acl_grantee) return false;
    string type_str;
    if (!acl_grantee->get_attr("xsi:type", type_str)) return false;
    ACLGranteeType_S3::set(type_str.c_str(), type);

    acl_permission = static_cast<ACLPermission_S3*>(find_first("Permission"));
    if (!acl_permission) return false;
...
void ACLGrant_S3::to_xml(CephContext* cct, ostream& out)
{
    ...
    if (!(perm.get_permissions() & RGW_PERM_ALL_S3)) return;

    string uri;

    out << "<Grant>"
        << "<Grantee xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:type=\""
        << ACLGranteeType_S3::to_string(type) << "\">";
    switch (type.get_type()) {
    case ACL_TYPE_CANON_USER:
        out << "<ID>" << id << "</ID>";
        if (name.size()) {
        ...
        break;
    case ACL_TYPE_EMAIL_USER: out << "<EmailAddress>" << email << "</EmailAddress>"; break;
    case ACL_TYPE_GROUP:
...
{
    switch (group) {
    case ACL_GROUP_ALL_USERS: uri = rgw_uri_all_users; return true;
    case ACL_GROUP_AUTHENTICATED_USERS: uri = rgw_uri_auth_users; return true;
...
static const char* get_acl_header(const RGWEnv* env, const struct s3_acl_header* perm)
{
                             ...
                             const struct s3_acl_header* perm, ACLGrant& grant)
{
    ...
    if (strcasecmp(id_type.c_str(), "emailAddress") == 0) {
        ...
        if (gid == ACL_GROUP_NONE) return -EINVAL;
...
static int parse_acl_header(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, const RGWEnv* env,
...
int RGWAccessControlList_S3::create_canned(ACLOwner& owner, ACLOwner& bucket_owner, const string& canned_acl)
{
    ...
    if (canned_acl.compare("public-read") == 0) {
        ...
        if (bid.compare(owner.get_id()) != 0) add_grant(&bucket_owner_grant);
    }
...
{
    RGWAccessControlList_S3* s3acl = static_cast<RGWAccessControlList_S3*>(find_first("AccessControlList"));
    if (!s3acl) return false;

    acl = *s3acl;

    ACLOwner* owner_p = static_cast<ACLOwner_S3*>(find_first("Owner"));
    if (!owner_p) return false;
    owner = *owner_p;
    return true;
}
...
static const s3_acl_header acl_header_perms[] = {{RGW_PERM_READ, "HTTP_X_AMZ_GRANT_READ"},
...
int RGWAccessControlPolicy_S3::create_from_headers(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver,
 ...
 */
int RGWAccessControlPolicy_S3::rebuild(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, ACLOwner* owner,
                                       RGWAccessControlPolicy& dest, std::string& err_msg)
{
    ...
    if (requested_owner) {
        ...
        if (!requested_id.empty() && requested_id.compare(owner->get_id()) != 0) return -EPERM;
    }
...
{
    switch (group) {
    case ACL_GROUP_ALL_USERS: return (id.compare(RGW_USER_ANON_ID) == 0);
    case ACL_GROUP_AUTHENTICATED_USERS: return (id.compare(rgw_uri_auth_users) == 0);
