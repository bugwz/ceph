...
void AsyncScheduler::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    if (observer) {
...
int AsyncScheduler::schedule_request_impl(const client_id& client, const ReqParams& params, const Time& time,
                                          const Cost& cost, optional_yield yield_ctx)
{
...
void AsyncScheduler::cancel()
{
    ...
    queue.remove_by_req_filter([&](RequestRef&& request) {
        ...
        Completion::dispatch(
            std::unique_ptr<Completion>{c}, boost::asio::error::operation_aborted, PhaseType::priority);
        return true;
    });
    timer.cancel();
...
void AsyncScheduler::process(const Time& now)
{
    ...
    while (outstanding_requests < max_requests) {
        ...
        Completion::post(std::unique_ptr<Completion>{c}, boost::system::error_code{}, phase);
