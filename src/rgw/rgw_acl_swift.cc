...
static ACLGrant user_to_grant(const DoutPrefixProvider* dpp, CephContext* const cct, RGWUserCtl* const user_ctl,
...
int RGWAccessControlPolicy_SWIFT::add_grants(const DoutPrefixProvider* dpp, RGWUserCtl* const user_ctl,
...
int RGWAccessControlPolicy_SWIFT::create(const DoutPrefixProvider* dpp, RGWUserCtl* const user_ctl, const rgw_user& id,
...
void RGWAccessControlPolicy_SWIFTAcct::add_grants(const DoutPrefixProvider* dpp, RGWUserCtl* const user_ctl,
                                                  const std::vector<std::string>& uids, const uint32_t perm)
{
    for (const auto& uid : uids) {
        ACLGrant grant;
...
bool RGWAccessControlPolicy_SWIFTAcct::create(const DoutPrefixProvider* dpp, RGWUserCtl* const user_ctl,
