...
static int parse_list(const char* uid_list, std::vector<std::string>& uids) /* out */
{
...
{
    return designator.compare(".r") == 0 || designator.compare(".ref") == 0 || designator.compare(".referer") == 0 ||
           designator.compare(".referrer") == 0;
}

static bool uid_is_public(const string& uid)
{
    if (uid[0] != '.' || uid[1] != 'r') return false;

    int pos = uid.find(':');
    if (pos < 0 || pos == (int)uid.size()) return false;

    string sub = uid.substr(0, pos);
    string after = uid.substr(pos + 1);

    if (after.compare("*") != 0) return false;

    return is_referrer(sub);
}

static boost::optional<ACLGrant> referrer_to_grant(std::string url_spec, const uint32_t perm)
{
...
static ACLGrant user_to_grant(const DoutPrefixProvider* dpp, CephContext* const cct, rgw::sal::Driver* driver,
                              const std::string& uid, const uint32_t perm)
{
...
int RGWAccessControlPolicy_SWIFT::add_grants(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver,
                                             const std::vector<std::string>& uids, const uint32_t perm)
{
...
int RGWAccessControlPolicy_SWIFT::create(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, const rgw_user& id,
                                         const std::string& name, const char* read_list, const char* write_list,
                                         uint32_t& rw_mask)
{
    ...
    if (read_list) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: add_grants for read returned r=" << r << dendl;
            return r;
        }
    ...
    if (write_list) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, 0) << "ERROR: add_grants for write returned r=" << r << dendl;
            return r;
        }
...
void RGWAccessControlPolicy_SWIFT::filter_merge(uint32_t rw_mask, RGWAccessControlPolicy_SWIFT* old)
{
...
void RGWAccessControlPolicy_SWIFTAcct::add_grants(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver,
                                                  const std::vector<std::string>& uids, const uint32_t perm)
{
    for (const auto& uid : uids) {
        ACLGrant grant;
...
bool RGWAccessControlPolicy_SWIFTAcct::create(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver,
                                              const rgw_user& id, const std::string& name, const std::string& acl_str)
{
    acl.create_default(id, name);
    owner.set_id(id);
    owner.set_name(name);
