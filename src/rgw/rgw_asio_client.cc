...
ClientIO::ClientIO(parser_type& parser, bool is_ssl, const endpoint_type& local_endpoint,
...
static size_t dump_date_header(char (&timestr)[TIME_BUF_SIZE])
{
    ...
    return strftime(timestr, sizeof(timestr), "Date: %a, %d %b %Y %H:%M:%S %Z\r\n", tmp);
}
...
size_t ClientIO::send_header(const std::string_view& name, const std::string_view& value)
{
