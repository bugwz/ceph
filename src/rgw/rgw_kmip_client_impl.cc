...
struct RGWKmipWorker : public Thread
{
    ...
    RGWKmipWorker(RGWKMIPManagerImpl& m)
        : m(m)
    {}
    void* entry() override;
    void signal()
...
{
    if (kmip->encoding) {
        kmip_free_buffer(kmip->kmip_ctx, kmip->encoding, kmip->buffer_total_size);
        kmip_set_buffer(kmip->kmip_ctx, NULL, 0);
    }
    if (kmip->need_to_free_kmip) kmip_destroy(kmip->kmip_ctx);
    if (kmip->bio) BIO_free_all(kmip->bio);
    if (kmip->ctx) SSL_CTX_free(kmip->ctx);
}
...
public:
    RGWKmipHandleBuilder(CephContext* cct)
        : cct(cct){};
...
static int kmip_write_an_error_helper(const char* s, size_t l, void* u)
{
...
RGWKmipHandle* RGWKmipHandleBuilder::build() const
{
    int failed = 1;
    ...
    if (!clientcert)
        ...
        lderr(cct) << "ERROR: can't load client cert from " << clientcert << dendl;
    ...
    if (!clientkey)
        ;
    else if (SSL_CTX_use_PrivateKey_file(r->ctx, clientkey, SSL_FILETYPE_PEM) != 1) {
        lderr(cct) << "ERROR: can't load client key from " << clientkey << dendl;
        ERR_print_errors_ceph(cct);
        goto Done;
    }
    ...
    if (!capath)
        ...
        lderr(cct) << "ERROR: can't load cacert from " << capath << dendl;
        ERR_print_errors_ceph(cct);
        goto Done;
    }
    ...
    if (BIO_do_connect(r->bio) != 1) {
        lderr(cct) << "BIO_do_connect failed to " << host << ":" << portstring << dendl;
        ERR_print_errors_ceph(cct);
        goto Done;
    }
    ...
    r->encoding =
        static_cast<uint8*>(r->kmip_ctx->calloc_func(r->kmip_ctx->state, r->buffer_blocks, r->buffer_block_size));
    if (!r->encoding) {
        lderr(cct) << "kmip buffer alloc failed: " << r->buffer_blocks << " * " << r->buffer_block_size << dendl;
        goto Done;
    }
...
RGWKmipHandle* RGWKmipHandles::get_kmip_handle()
{
...
void* RGWKmipHandles::entry()
{
    ...
    for (;;) {
        ...
        while (!saved_kmip.empty()) {
            ...
            --cend;
            kmip = *cend;
            if (!cleaner_shutdown && now - kmip->lastuse < std::chrono::seconds(MAXIDLE)) break;
...
void RGWKmipHandles::flush_kmip_handles()
{
    stop();
    join();
    if (!saved_kmip.empty()) {
...
int RGWKMIPManagerImpl::add_request(RGWKMIPTransceiver* req)
{
    ...
    if (going_down) return -ECANCELED;
    requests.push_back(*new Request{*req});
    l.unlock();
    if (worker) worker->signal();
    return 0;
}
...
int RGWKmipHandles::do_one_entry(RGWKMIPTransceiver& element)
{
    ...
    memset(a, 0, sizeof *a);
    for (i = 0; i < (int)(sizeof a / sizeof *a); ++i) kmip_init_attribute(a + i);
    ap = a;
    ...
    case RGWKMIPTransceiver::GET:
        if (element.unique_id) u->get_req->unique_identifier = uvalue;
    ...
    for (;;) {
        ...
        ++h->buffer_blocks;
        h->encoding =
            static_cast<uint8*>(h->kmip_ctx->calloc_func(h->kmip_ctx->state, h->buffer_blocks, h->buffer_block_size));
        if (!h->encoding) {
            lderr(cct) << "kmip buffer alloc failed: " << h->buffer_blocks << " * " << h->buffer_block_size << dendl;
            element.ret = -ENOMEM;
            goto Done;
        }
    ...
    if (i != KMIP_OK) {
        lderr(cct) << " Failed to encode " << what << " request; err=" << i << " ctx error message "
                   << h->kmip_ctx->error_message << dendl;
        element.ret = -EINVAL;
        goto Done;
    }
    i = kmip_bio_send_request_encoding(
        h->kmip_ctx, h->bio, (char*)h->encoding, h->kmip_ctx->index - h->kmip_ctx->buffer, &response, &response_size);
    if (i < 0) {
        lderr(cct) << "Problem sending request to " << what << " " << i << " context error message "
                   << h->kmip_ctx->error_message << dendl;
        element.ret = -EINVAL;
        goto Done;
    }
    kmip_free_buffer(h->kmip_ctx, h->encoding, h->buffer_total_size);
    h->encoding = 0;
    kmip_set_buffer(h->kmip_ctx, response, response_size);
    need_to_free_response = 1;
    i = kmip_decode_response_message(h->kmip_ctx, resp_m);
    if (i != KMIP_OK) {
        lderr(cct) << "Failed to decode " << what << " " << i << " context error message " << h->kmip_ctx->error_message
                   << dendl;
        element.ret = -EINVAL;
        goto Done;
    }
    ...
    if (req->operation != rbi->operation) {
        lderr(cct) << "Failed; response operation mismatch, got " << req->operation << " expected " << rbi->operation
                   << dendl;
        element.ret = -EINVAL;
        goto Done;
    }
    ...
    element.ret = 0;
Done:
    if (need_to_free_response) kmip_free_response_message(h->kmip_ctx, resp_m);
    element.done = true;
