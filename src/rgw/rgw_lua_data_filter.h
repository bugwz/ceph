...
public:
    RGWObjFilter(req_state* s, const std::string& script)
        : s(s)
        , script(script)
    {}
...
public:
    RGWGetObjFilter(req_state* s, const std::string& script, RGWGetObj_Filter* next)
...
public:
    RGWPutObjFilter(req_state* s, const std::string& script, rgw::sal::DataProcessor* next)
        : rgw::putobj::Pipe(next)
        , filter(s, script)
    {}
