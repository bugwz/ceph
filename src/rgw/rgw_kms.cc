...
protected:
    ...
    FORTEST_VIRTUAL
    int send_request(const char* method, std::string_view infix, std::string_view key_id, const std::string& postdata,
                     bufferlist& secret_bl)
    {
        ...
        ldout(cct, 20) << "Request to Vault returned " << res << " and HTTP status " << secret_req.get_http_status()
                       << dendl;
...
public:
    VaultSecretEngine(CephContext* cct) { this->cct = cct; }
};
...
public:
    TransitSecretEngine(CephContext* cct, EngineParmMap parms)
    ...
    int get_key(std::string_view key_id, std::string& actual_key)
    {
        ...
        int res =
            send_request("GET", compat == COMPAT_ONLY_OLD ? "" : "/export/encryption-key", key_id, string{}, secret_bl);
        if (res < 0) {
            return res;
        }
    ...
    int make_actual_key(map<string, bufferlist>& attrs, std::string& actual_key)
    {
        ...
        */
        ...
        int res = send_request("POST", "/datakey/plaintext/", key_id, post_data, secret_bl);
        if (res < 0) {
            return res;
        }
    ...
    int reconstitute_actual_key(map<string, bufferlist>& attrs, std::string& actual_key)
    {
        ...
        */
        ...
        int res = send_request("POST", "/decrypt/", key_id, post_data, secret_bl);
        if (res < 0) {
            return res;
        }
...
public:
    KvSecretEngine(CephContext* cct, EngineParmMap parms)
        : VaultSecretEngine(cct)
    {
        if (!parms.empty()) {
...
static int get_actual_key_from_conf(CephContext* cct, std::string_view key_id, std::string_view key_selector,
                                    std::string& actual_key)
{
    ...
    if (it == str_map.end()) return -ERR_INVALID_ACCESS_KEY;
...
static int request_key_from_barbican(CephContext* cct, std::string_view key_id, const std::string& barbican_token,
                                     std::string& actual_key)
{
    int res;
...
static int get_actual_key_from_barbican(CephContext* cct, std::string_view key_id, std::string& actual_key)
{
    int res = 0;
    std::string token;
...
static int get_actual_key_from_vault(CephContext* cct, map<string, bufferlist>& attrs, std::string& actual_key,
                                     bool make_it)
{
    ...
    if (RGW_SSE_KMS_VAULT_SE_KV == secret_engine) {
        ...
        std::string key_id = get_str_attribute(attrs, RGW_ATTR_CRYPT_KEYID);
        return make_it ? engine.make_actual_key(attrs, actual_key) : engine.reconstitute_actual_key(attrs, actual_key);
    }
    else {
...
static int make_actual_key_from_vault(CephContext* cct, map<string, bufferlist>& attrs, std::string& actual_key)
{
    return get_actual_key_from_vault(cct, attrs, actual_key, true);
}


static int reconstitute_actual_key_from_vault(CephContext* cct, map<string, bufferlist>& attrs, std::string& actual_key)
{
    return get_actual_key_from_vault(cct, attrs, actual_key, false);
}


static int get_actual_key_from_kmip(CephContext* cct, std::string_view key_id, std::string& actual_key)
{
...
int reconstitute_actual_key_from_kms(CephContext* cct, map<string, bufferlist>& attrs, std::string& actual_key)
{
    std::string key_id = get_str_attribute(attrs, RGW_ATTR_CRYPT_KEYID);
...
int make_actual_key_from_kms(CephContext* cct, map<string, bufferlist>& attrs, std::string& actual_key)
{
    std::string kms_backend{cct->_conf->rgw_crypt_s3_kms_backend};
    if (RGW_SSE_KMS_BACKEND_VAULT == kms_backend) return make_actual_key_from_vault(cct, attrs, actual_key);
