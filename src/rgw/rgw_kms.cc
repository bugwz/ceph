...
public:
    ...
    static void Free(void* p) { ceph_assert(0 == "Free should not be called"); }
...
typedef rapidjson::GenericDocument<rapidjson::UTF8<>, ZeroPoolAllocator, rapidjson::CrtAllocator> ZeroPoolDocument;
 ...
 */
static bool string_ends_maybe_slash(std::string_view hay, std::string_view needle)
{
...
static inline void add_name_val_to_obj(std::string& n, std::string& v, rapidjson::GenericValue<E, A>& d, A& allocator)
{
...
template<typename E, typename A = ZeroPoolAllocator>
static inline void add_name_val_to_obj(std::string& n, bool v, rapidjson::GenericValue<E, A>& d, A& allocator)
{
    rapidjson::GenericValue<E, A> name, val;
    name.SetString(n.c_str(), n.length(), allocator);
    val.SetBool(v);
...
template<typename E, typename A = ZeroPoolAllocator>
static inline void add_name_val_to_obj(const char* n, std::string& v, rapidjson::GenericValue<E, A>& d, A& allocator)
{
...
template<typename E, typename A = ZeroPoolAllocator>
static inline void add_name_val_to_obj(const char* n, bool v, rapidjson::GenericValue<E, A>& d, A& allocator)
{
    std::string ns{n, strlen(n)};
    add_name_val_to_obj(ns, v, d, allocator);
}
...
protected:
    ...
    FORTEST_VIRTUAL
    int send_request(const DoutPrefixProvider* dpp, const char* method, std::string_view infix, std::string_view key_id,
                     const std::string& postdata, bufferlist& secret_bl)
    {
        int res;
        string vault_token = "";
        ...
        if (secret_req.get_http_status() == RGWHTTPTransceiver::HTTP_STATUS_UNAUTHORIZED) {
        ...
        ldpp_dout(dpp, 20) << "Request to Vault returned " << res << " and HTTP status " << secret_req.get_http_status()
                           << dendl;
...
public:
    VaultSecretEngine(CephContext* _c, SSEContext& _k)
        : cct(_c)
        , kctx(_k)
    {}
};
...
public:
    TransitSecretEngine(CephContext* cct, SSEContext& kctx, EngineParmMap parms)
        : VaultSecretEngine(cct, kctx)
        , parms(parms)
    {
        compat = COMPAT_UNSET;
        for (auto& e : parms) {
            if (e.first == "compat") {
                if (e.second.empty()) {
                    ...
                    if (ep != e.second.length()) {
                        lderr(cct) << "warning: vault transit secrets engine : compat=" << e.second
                                   << " trailing junk? (ignored)" << dendl;
                    }
                }
                continue;
            }
            lderr(cct) << "ERROR: vault transit secrets engine : parameter " << e.first << "=" << e.second << " ignored"
                       << dendl;
        }
    ...
    int get_key(const DoutPrefixProvider* dpp, std::string_view key_id, std::string& actual_key)
    {
        ...
        int res = send_request(
            dpp, "GET", compat == COMPAT_ONLY_OLD ? "" : "/export/encryption-key", key_id, string{}, secret_bl);
        if (res < 0) {
            return res;
        }
    ...
    int make_actual_key(const DoutPrefixProvider* dpp, map<string, bufferlist>& attrs, std::string& actual_key)
    {
        ...
        */
        ...
        int res = send_request(dpp, "POST", "/datakey/plaintext/", key_id, post_data, secret_bl);
        if (res < 0) {
            return res;
        }
    ...
    int reconstitute_actual_key(const DoutPrefixProvider* dpp, map<string, bufferlist>& attrs, std::string& actual_key)
    {
        ...
        */
        ...
        int res = send_request(dpp, "POST", "/decrypt/", key_id, post_data, secret_bl);
        if (res < 0) {
            return res;
        }
    ...
    int create_bucket_key(const DoutPrefixProvider* dpp, const std::string& key_name)
    {
        ...
        */
        ...
        int res = send_request(dpp, "POST", "/keys/", key_name, post_data, dummy_bl);
        if (res < 0) {
            return res;
        }
        if (dummy_bl.length() != 0) {
            ldpp_dout(dpp, 0) << "ERROR: unexpected response from Vault making a key: " << dummy_bl << dendl;
        }
        return 0;
    }
    ...
    int delete_bucket_key(const DoutPrefixProvider* dpp, const std::string& key_name)
    {
        ...
        */
        ...
        int res = send_request(dpp, "POST", "", config_path, post_data, dummy_bl);
        if (res < 0) {
            return res;
        }
        if (dummy_bl.length() != 0) {
            ldpp_dout(dpp, 0) << "ERROR: unexpected response from Vault marking key to delete: " << dummy_bl << dendl;
            return -EINVAL;
        }

        res = send_request(dpp, "DELETE", "", delete_path, string{}, dummy_bl);
        if (res < 0) {
            return res;
        }
        if (dummy_bl.length() != 0) {
            ldpp_dout(dpp, 0) << "ERROR: unexpected response from Vault deleting key: " << dummy_bl << dendl;
            return -EINVAL;
        }
        return 0;
    }
...
public:
    KvSecretEngine(CephContext* cct, SSEContext& kctx, EngineParmMap parms)
        : VaultSecretEngine(cct, kctx)
    {
        if (!parms.empty()) {
...
KmipGetTheKey& KmipGetTheKey::keyid_to_keyname(std::string_view key_id)
{
...
KmipGetTheKey& KmipGetTheKey::get_uniqueid_for_keyname()
{
    ...
    if (ret < 0) {
        ...
        ret = -ENOENT;
        lderr(cct) << "error: locate returned no results for " << secret_req.name << dendl;
        failed = true;
    }
    else if (secret_req.outlist->string_count != 1) {
        ret = -EINVAL;
        lderr(cct) << "error: locate found " << secret_req.outlist->string_count << " results for " << secret_req.name
                   << dendl;
        failed = true;
    }
    else {
...
int KmipGetTheKey::get_key_for_uniqueid(std::string& actual_key)
{
    ...
    if (ret < 0) {
        ...
        actual_key = std::string((char*)(secret_req.outkey->data), secret_req.outkey->keylen);
    }
    return ret;
}
...
public:
    KmipSecretEngine(CephContext* cct) { this->cct = cct; }

    int get_key(const DoutPrefixProvider* dpp, std::string_view key_id, std::string& actual_key)
    {
        int r;
        r = KmipGetTheKey{cct}.keyid_to_keyname(key_id).get_uniqueid_for_keyname().get_key_for_uniqueid(actual_key);
        return r;
    }
};

static int get_actual_key_from_conf(const DoutPrefixProvider* dpp, CephContext* cct, std::string_view key_id,
                                    std::string_view key_selector, std::string& actual_key)
{
    int res = 0;

    static map<string, string> str_map = get_str_map(cct->_conf->rgw_crypt_s3_kms_encryption_keys);

    map<string, string>::iterator it = str_map.find(std::string(key_id));
    if (it == str_map.end()) return -EINVAL;
    ...
    try {
        ...
        ldpp_dout(dpp, 5) << "ERROR: get_actual_key_from_conf invalid encryption key id "
                          << "which contains character that is not base64 encoded." << dendl;
        return -EINVAL;
    }

    if (master_key.length() == AES_256_KEYSIZE) {
        uint8_t _actual_key[AES_256_KEYSIZE];
        if (AES_256_ECB_encrypt(dpp,
                                cct,
...
static int request_key_from_barbican(const DoutPrefixProvider* dpp, CephContext* cct, std::string_view key_id,
                                     const std::string& barbican_token, std::string& actual_key)
{
    int res;
    ...
    if (secret_req.get_http_status() >= 200 && secret_req.get_http_status() < 300 &&
...
static int get_actual_key_from_barbican(const DoutPrefixProvider* dpp, CephContext* cct, std::string_view key_id,
...
std::string config_to_engine_and_parms(CephContext* cct, const char* which, std::string& secret_engine_str,
                                       EngineParmMap& secret_engine_parms)
{
    ...
    if (meta_errors.length()) {
        ...
        lderr(cct) << "ERROR: while expanding " << which << ": " << meta_errors << dendl;
    }
...
static int get_actual_key_from_vault(const DoutPrefixProvider* dpp, CephContext* cct, SSEContext& kctx,
                                     map<string, bufferlist>& attrs, std::string& actual_key, bool make_it)
{
    ...
    if (RGW_SSE_KMS_VAULT_SE_KV == secret_engine) {
        ...
        std::string key_id = get_str_attribute(attrs, RGW_ATTR_CRYPT_KEYID);
        return make_it ? engine.make_actual_key(dpp, attrs, actual_key)
...
static int make_actual_key_from_vault(const DoutPrefixProvider* dpp, CephContext* cct, SSEContext& kctx,
                                      map<string, bufferlist>& attrs, std::string& actual_key)
{
    return get_actual_key_from_vault(dpp, cct, kctx, attrs, actual_key, true);
}


static int reconstitute_actual_key_from_vault(const DoutPrefixProvider* dpp, CephContext* cct, SSEContext& kctx,
                                              map<string, bufferlist>& attrs, std::string& actual_key)
{
...
static int get_actual_key_from_kmip(const DoutPrefixProvider* dpp, CephContext* cct, std::string_view key_id,
                                    std::string& actual_key)
{
...
public:
    KMSContext(CephContext* _cct)
        : cct{_cct} {};
    ~KMSContext() override{};
    const std::string& backend() override { return cct->_conf->rgw_crypt_s3_kms_backend; };
    const std::string& addr() override { return cct->_conf->rgw_crypt_vault_addr; };
    const std::string& auth() override { return cct->_conf->rgw_crypt_vault_auth; };
    const std::string& k_namespace() override { return cct->_conf->rgw_crypt_vault_namespace; };
    const std::string& prefix() override { return cct->_conf->rgw_crypt_vault_prefix; };
    const std::string& secret_engine() override { return cct->_conf->rgw_crypt_vault_secret_engine; };
    const std::string& ssl_cacert() override { return cct->_conf->rgw_crypt_vault_ssl_cacert; };
    const std::string& ssl_clientcert() override { return cct->_conf->rgw_crypt_vault_ssl_clientcert; };
    const std::string& ssl_clientkey() override { return cct->_conf->rgw_crypt_vault_ssl_clientkey; };
    const std::string& token_file() override { return cct->_conf->rgw_crypt_vault_token_file; };
    const bool verify_ssl() override { return cct->_conf->rgw_crypt_vault_verify_ssl; };
};
...
public:
    ...
    SseS3Context(CephContext* _cct)
        : cct{_cct} {};
    ~SseS3Context(){};
    const std::string& backend() override { return cct->_conf->rgw_crypt_sse_s3_backend; };
    const std::string& addr() override { return cct->_conf->rgw_crypt_sse_s3_vault_addr; };
    const std::string& auth() override { return cct->_conf->rgw_crypt_sse_s3_vault_auth; };
    const std::string& k_namespace() override { return cct->_conf->rgw_crypt_sse_s3_vault_namespace; };
    const std::string& prefix() override { return cct->_conf->rgw_crypt_sse_s3_vault_prefix; };
    const std::string& secret_engine() override { return cct->_conf->rgw_crypt_sse_s3_vault_secret_engine; };
    const std::string& ssl_cacert() override { return cct->_conf->rgw_crypt_sse_s3_vault_ssl_cacert; };
    const std::string& ssl_clientcert() override { return cct->_conf->rgw_crypt_sse_s3_vault_ssl_clientcert; };
    const std::string& ssl_clientkey() override { return cct->_conf->rgw_crypt_sse_s3_vault_ssl_clientkey; };
    const std::string& token_file() override { return cct->_conf->rgw_crypt_sse_s3_vault_token_file; };
    const bool verify_ssl() override { return cct->_conf->rgw_crypt_sse_s3_vault_verify_ssl; };
};

int reconstitute_actual_key_from_kms(const DoutPrefixProvider* dpp, CephContext* cct, map<string, bufferlist>& attrs,
                                     std::string& actual_key)
{
    std::string key_id = get_str_attribute(attrs, RGW_ATTR_CRYPT_KEYID);
    KMSContext kctx{cct};
...
int make_actual_key_from_kms(const DoutPrefixProvider* dpp, CephContext* cct, map<string, bufferlist>& attrs,
                             std::string& actual_key)
{
    KMSContext kctx{cct};
    const std::string& kms_backend{kctx.backend()};
    if (RGW_SSE_KMS_BACKEND_VAULT == kms_backend) return make_actual_key_from_vault(dpp, cct, kctx, attrs, actual_key);
    return reconstitute_actual_key_from_kms(dpp, cct, attrs, actual_key);
}

int reconstitute_actual_key_from_sse_s3(const DoutPrefixProvider* dpp, CephContext* cct, map<string, bufferlist>& attrs,
                                        std::string& actual_key)
{
    std::string key_id = get_str_attribute(attrs, RGW_ATTR_CRYPT_KEYID);
...
int make_actual_key_from_sse_s3(const DoutPrefixProvider* dpp, CephContext* cct, map<string, bufferlist>& attrs,
                                std::string& actual_key)
{
    SseS3Context kctx{cct};
    const std::string kms_backend{kctx.backend()};
...
int create_sse_s3_bucket_key(const DoutPrefixProvider* dpp, CephContext* cct, const std::string& bucket_key)
{
    SseS3Context kctx{cct};
    ...
    auto secret_engine{config_to_engine_and_parms(
        cct, "rgw_crypt_sse_s3_vault_secret_engine", secret_engine_str, secret_engine_parms)};
...
int remove_sse_s3_bucket_key(const DoutPrefixProvider* dpp, CephContext* cct, const std::string& bucket_key)
{
    SseS3Context kctx{cct};
    std::string secret_engine_str = kctx.secret_engine();
    EngineParmMap secret_engine_parms;
