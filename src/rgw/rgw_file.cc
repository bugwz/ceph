...
LookupFHResult RGWLibFS::stat_leaf(RGWFileHandle* parent, const char* path, enum rgw_fh_type type, uint32_t flags)
{
    ...
    for (auto ix : {0, 1, 2}) {
        ...
        case 0:
        {
            ...
            RGWStatObjRequest req(cct, user->clone(), parent->bucket_name(), obj_path, RGWStatObjRequest::FLAG_NONE);
            int rc = g_rgwlib->get_fe()->execute_req(&req);
        ...
        case 2:
        {
            ...
            RGWStatLeafRequest req(cct, user->clone(), parent, object_name);
            int rc = g_rgwlib->get_fe()->execute_req(&req);
            if ((rc == 0) && (req.get_ret() == 0)) {
                if (req.matched) {
...
bool RGWFileHandle::has_children() const
{
    ...
    RGWRMdirCheck req(fs->get_context(), g_rgwlib->get_driver()->get_user(fs->get_user()->user_id), this);
    int rc = g_rgwlib->get_fe()->execute_req(&req);
    if (!rc) {
...
int RGWFileHandle::readdir(rgw_readdir_cb rcb, void* cb_arg, readdir_offset offset, bool* eof, uint32_t flags)
{
    ...
    if (is_root()) {
        RGWListBucketsRequest req(
            cct, g_rgwlib->get_driver()->get_user(fs->get_user()->user_id), this, rcb, cb_arg, offset);
        rc = g_rgwlib->get_fe()->execute_req(&req);
        if (!rc) {
            (void)clock_gettime(CLOCK_MONOTONIC_COARSE, &now); /* !LOCKED */
            lock_guard guard(mtx);
...
int RGWFileHandle::write(uint64_t off, size_t len, size_t* bytes_written, void* buffer)
{
    ...
    if (!f->write_req) {
        ...
        f->write_req = new RGWWriteRequest(g_rgwlib->get_driver(),
                                           penv,
...
int RGWWriteRequest::exec_start()
{
    ...
    auto compression_type = get_driver()->get_compression_type(state->bucket->get_placement_rule());
    ...
    processor = get_driver()->get_atomic_writer(this,
                                                state->yield,
                                                state->object.get(),
...
int rgw_statfs(struct rgw_fs* rgw_fs, struct rgw_file_handle* parent_fh, struct rgw_statvfs* vfs_st, uint32_t flags)
{
    ...
    RGWGetClusterStatReq req(fs->get_context(), g_rgwlib->get_driver()->get_user(fs->get_user()->user_id), stats);
    int rc = g_rgwlib->get_fe()->execute_req(&req);
    if (rc < 0) {
...
int rgw_writev(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, rgw_uio* uio, uint32_t flags)
{
    ...
    RGWPutObjRequest req(
        cct, g_rgwlib->get_driver()->get_user(fs->get_user()->user_id), rgw_fh->bucket_name(), oname, bl);
