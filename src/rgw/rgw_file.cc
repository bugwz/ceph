...
ceph::timer<ceph::mono_clock> RGWLibFS::write_timer{ceph::construct_suspended};
...
inline int valid_fs_object_name(const string& name)
{
    ...
    if (rc != 0) {
        if (name.size() > 1024) return -ENAMETOOLONG;
        return -EINVAL;
    }
    return 0;
}
...
{
public:
    std::size_t operator()(const rgw_xattrstr& att) const noexcept { return XXH64(att.val, att.len, 5882300); }
};
...
public:
    ...
    {
        return ((lhs.len == rhs.len) && (strncmp(lhs.val, rhs.val, lhs.len) == 0));
    }
};

/* well-known attributes */
static const std::unordered_set<rgw_xattrstr, XattrHash, XattrEqual> rgw_exposed_attrs = {
...
LookupFHResult RGWLibFS::stat_bucket(RGWFileHandle* parent, const char* path, RGWLibFS::BucketStats& bs, uint32_t flags)
{
    ...
    if ((rc == 0) && (req.get_ret() == 0) && (req.matched())) {
        fhr = lookup_fh(parent,
                        path,
                        (flags & RGWFileHandle::FLAG_LOCKED) | RGWFileHandle::FLAG_CREATE | RGWFileHandle::FLAG_BUCKET);
        if (get<0>(fhr)) {
...
LookupFHResult RGWLibFS::fake_leaf(RGWFileHandle* parent, const char* path, enum rgw_fh_type type, struct stat* st,
                                   uint32_t st_mask, uint32_t flags)
{
    ...
    switch (type) {
    case RGW_FS_TYPE_DIRECTORY: flags |= RGWFileHandle::FLAG_DIRECTORY; break;
    default:
        /* file */
        break;
    };
...
LookupFHResult RGWLibFS::stat_leaf(RGWFileHandle* parent, const char* path, enum rgw_fh_type type, uint32_t flags)
{
    ...
    for (auto ix : {0, 1, 2}) {
        ...
        {
            /* type hint */
            if (type == RGW_FS_TYPE_DIRECTORY) continue;

            RGWStatObjRequest req(cct,
                                  rgwlib.get_store()->get_user(user.user_id),
                                  parent->bucket_name(),
                                  obj_path,
                                  RGWStatObjRequest::FLAG_NONE);
            int rc = rgwlib.get_fe()->execute_req(&req);
            if ((rc == 0) && (req.get_ret() == 0)) {
                ...
                if (get<0>(fhr)) {
                    ...
                    if (!(flags & RGWFileHandle::FLAG_IN_CB) && ux_key && ux_attrs) {
                        DecodeAttrsResult dar = rgw_fh->decode_attrs(ux_key, ux_attrs);
                        if (get<0>(dar) || get<1>(dar)) {
                            update_fh(rgw_fh);
                        }
        ...
        case 1:
        {
            /* try dir form */
            /* type hint */
            if (type == RGW_FS_TYPE_FILE) continue;
        ...
        case 2:
        {
            ...
            RGWStatLeafRequest req(cct, rgwlib.get_store()->get_user(user.user_id), parent, object_name);
            int rc = rgwlib.get_fe()->execute_req(&req);
            if ((rc == 0) && (req.get_ret() == 0)) {
                if (req.matched) {
                    /* we need rgw object's key name equal to file name, if
                     * not return NULL */
                    if ((flags & RGWFileHandle::FLAG_EXACT_MATCH) && !req.exact_matched) {
                        lsubdout(get_context(), rgw, 15)
                            << __func__ << ": stat leaf not exact match file name = " << path << dendl;
                        goto done;
                    }
                    fhr = lookup_fh(parent,
                                    path,
                                    RGWFileHandle::FLAG_CREATE |
                                        ((req.is_dir) ? RGWFileHandle::FLAG_DIRECTORY : RGWFileHandle::FLAG_NONE));
...
int RGWLibFS::read(RGWFileHandle* rgw_fh, uint64_t offset, size_t length, size_t* bytes_read, void* buffer,
                   uint32_t flags)
{
    if (!rgw_fh->is_file()) return -EINVAL;

    if (rgw_fh->deleted()) return -ESTALE;

    RGWReadRequest req(get_context(), rgwlib.get_store()->get_user(user.user_id), rgw_fh, offset, length, buffer);

    int rc = rgwlib.get_fe()->execute_req(&req);
    if ((rc == 0) && ((rc = req.get_ret()) == 0)) {
        lock_guard guard(rgw_fh->mtx);
...
int RGWLibFS::readlink(RGWFileHandle* rgw_fh, uint64_t offset, size_t length, size_t* bytes_read, void* buffer,
                       uint32_t flags)
{
    if (!rgw_fh->is_link()) return -EINVAL;
...
int RGWLibFS::unlink(RGWFileHandle* rgw_fh, const char* name, uint32_t flags)
{
    ...
    if (parent->is_root()) {
         ...
         * for and delete it */
        LookupFHResult fhr;
        fhr = stat_bucket(parent, name, bs, (rgw_fh) ? RGWFileHandle::FLAG_LOCKED : RGWFileHandle::FLAG_NONE);
        bkt_fh = get<0>(fhr);
        if (unlikely(!bkt_fh)) {
        ...
        if (bs.num_entries > 1) {
            ...
            string oname{"/"};
            RGWDeleteObjRequest req(cct, rgwlib.get_store()->get_user(user.user_id), bkt_fh->bucket_name(), oname);
        ...
        if (!rgw_fh) {
            ...
            rc =
                rgw_lookup(get_fs(), parent->get_fh(), name, &fh, nullptr /* st */, 0 /* mask */, RGW_LOOKUP_FLAG_NONE);
            if (!!rc) return rc;
        ...
        RGWDeleteObjRequest req(cct, rgwlib.get_store()->get_user(user.user_id), parent->bucket_name(), oname);
        rc = rgwlib.get_fe()->execute_req(&req);
        if (!rc) {
            rc = req.get_ret();
        }
    ...
    if (!rc || rc == -ENOENT) {
        ...
        fh_cache.remove(rgw_fh->fh.fh_hk.object, rgw_fh, RGWFileHandle::FHCache::FLAG_LOCK);
    }
...
int RGWLibFS::rename(RGWFileHandle* src_fh, RGWFileHandle* dst_fh, const char* _src_name, const char* _dst_name)

{
    ...
    if (rgw_fh->is_dir()) {
        ldout(get_context(), 12) << __func__
                                 << " rejecting attempt to rename directory path=" << rgw_fh->full_object_name()
                                 << dendl;
        rc = -EPERM;
        goto unlock;
    }
    ...
    if (rgw_fh->is_open()) {
        ldout(get_context(), 12) << __func__
                                 << " rejecting attempt to rename open file path=" << rgw_fh->full_object_name()
                                 << dendl;
        rc = -EPERM;
        goto unlock;
    }
    ...
    for (int ix : {0, 1}) {
        ...
        case 0:
        {
            RGWCopyObjRequest req(cct, rgwlib.get_store()->get_user(user.user_id), src_fh, dst_fh, src_name, dst_name);
            int rc = rgwlib.get_fe()->execute_req(&req);
            if ((rc != 0) || ((rc = req.get_ret()) != 0)) {
                ldout(get_context(), 1) << __func__ << " rename step 0 failed src=" << src_fh->full_object_name() << " "
                                        << src_name << " dst=" << dst_fh->full_object_name() << " " << dst_name << "rc "
                                        << rc << dendl;
                goto unlock;
            }
            ldout(get_context(), 12) << __func__ << " rename step 0 success src=" << src_fh->full_object_name() << " "
                                     << src_name << " dst=" << dst_fh->full_object_name() << " " << dst_name << " rc "
                                     << rc << dendl;
        ...
        case 1:
        {
            rc = this->unlink(rgw_fh /* LOCKED */, _src_name, RGWFileHandle::FLAG_UNLINK_THIS);
            /* !LOCKED, -ref */
            if (!rc) {
                ldout(get_context(), 12) << __func__ << " rename step 1 success src=" << src_fh->full_object_name()
                                         << " " << src_name << " dst=" << dst_fh->full_object_name() << " " << dst_name
                                         << " rc " << rc << dendl;
                /* update src change id */
                src_fh->set_times(t);
            }
            else {
                ldout(get_context(), 1) << __func__ << " rename step 1 failed src=" << src_fh->full_object_name() << " "
                                        << src_name << " dst=" << dst_fh->full_object_name() << " " << dst_name
                                        << " rc " << rc << dendl;
            }
        }
...
MkObjResult RGWLibFS::mkdir(RGWFileHandle* parent, const char* name, struct stat* st, uint32_t mask, uint32_t flags)
{
    int rc, rc2;
    rgw_file_handle* lfh;

    rc = rgw_lookup(get_fs(), parent->get_fh(), name, &lfh, nullptr /* st */, 0 /* mask */, RGW_LOOKUP_FLAG_NONE);
    if (!rc) {
        /* conflict! */
    ...
    fhr =
        lookup_fh(parent, name, RGWFileHandle::FLAG_CREATE | RGWFileHandle::FLAG_DIRECTORY | RGWFileHandle::FLAG_LOCK);
    rgw_fh = get<0>(fhr);
    if (rgw_fh) {
        ...
        if (st) rgw_fh->stat(st, RGWFileHandle::FLAG_LOCKED);
        get<0>(mkr) = rgw_fh;
    }
    else {
        get<1>(mkr) = -EIO;
    ...
    if (parent->is_root()) {
        ...
        RGWCreateBucketRequest req(get_context(), rgwlib.get_store()->get_user(user.user_id), bname);
        ...
        RGWPutObjRequest req(
            get_context(), rgwlib.get_store()->get_user(user.user_id), parent->bucket_name(), dir_name, bl);
...
MkObjResult RGWLibFS::create(RGWFileHandle* parent, const char* name, struct stat* st, uint32_t mask, uint32_t flags)
{
    int rc, rc2;
    ...
    /* create it */
    buffer::list bl;
    RGWPutObjRequest req(cct, rgwlib.get_store()->get_user(user.user_id), parent->bucket_name(), obj_name, bl);
    MkObjResult mkr{nullptr, -EINVAL};
    ...
    if ((rc == 0) && (rc2 == 0)) {
        /* XXX atomicity */
        LookupFHResult fhr = lookup_fh(parent, name, RGWFileHandle::FLAG_CREATE | RGWFileHandle::FLAG_LOCK);
        RGWFileHandle* rgw_fh = get<0>(fhr);
        if (rgw_fh) {
            ...
            if (st) (void)rgw_fh->stat(st, RGWFileHandle::FLAG_LOCKED);
    ...
    if (rc2 < 0) get<1>(mkr) = rc2;

    return mkr;
} /* RGWLibFS::create */

MkObjResult RGWLibFS::symlink(RGWFileHandle* parent, const char* name, const char* link_path, struct stat* st,
                              uint32_t mask, uint32_t flags)
{
    ...
    fhr = lookup_fh(
        parent, name, RGWFileHandle::FLAG_CREATE | RGWFileHandle::FLAG_SYMBOLIC_LINK | RGWFileHandle::FLAG_LOCK);
    rgw_fh = get<0>(fhr);
    if (rgw_fh) {
        rgw_fh->create_stat(st, mask);
        rgw_fh->set_times(real_clock::now());
    ...
    bl.push_back(buffer::copy(link_path, strlen(link_path)));
#endif

    RGWPutObjRequest req(
        get_context(), rgwlib.get_store()->get_user(user.user_id), parent->bucket_name(), obj_name, bl);
...
int RGWLibFS::setattr(RGWFileHandle* rgw_fh, struct stat* st, uint32_t mask, uint32_t flags)
{
    int rc, rc2;
    buffer::list ux_key, ux_attrs;
    ...
    if (rgw_fh->is_dir() && (likely(!rgw_fh->is_bucket()))) {
        obj_name += "/";
    }

    RGWSetAttrsRequest req(cct, rgwlib.get_store()->get_user(user.user_id), rgw_fh->bucket_name(), obj_name);
    ...
    if (rc == -ENOENT) {
        ...
        buffer::list bl;
        RGWPutObjRequest req(
            get_context(), rgwlib.get_store()->get_user(user.user_id), rgw_fh->bucket_name(), obj_name, bl);
...
int RGWLibFS::getxattrs(RGWFileHandle* rgw_fh, rgw_xattrlist* attrs, rgw_getxattr_cb cb, void* cb_arg, uint32_t flags)
{
    /* cannot store on fs_root, should not on buckets? */
    if ((rgw_fh->is_bucket()) || (rgw_fh->is_root())) {
        return -EINVAL;
    }
    ...
    RGWGetAttrsRequest req(cct, rgwlib.get_store()->get_user(user.user_id), rgw_fh->bucket_name(), obj_name);
    ...
    for (uint32_t ix = 0; ix < attrs->xattr_cnt; ++ix) {
        ...
        std::string k =
            is_exposed_attr(xattr.key) ? std::string{xattr.key.val, xattr.key.len} : prefix_xattr_keystr(xattr.key);
    ...
    if (rc3 == 0) {
        ...
        for (const auto& attr : attrs) {
            ...
            if (!attr.second.has_value()) continue;
            ...
            std::string_view svk = is_exposed_attr(rgw_xattrstr{const_cast<char*>(k.c_str()), uint32_t(k.length())})
                                       ? k
                                       : unprefix_xattr_keystr(k);

            /* skip entries not matching prefix */
            if (svk.empty()) continue;

            rgw_xattrstr xattr_k = {const_cast<char*>(svk.data()), uint32_t(svk.length())};
            rgw_xattrstr xattr_v = {const_cast<char*>(const_cast<buffer::list&>(v).c_str()), uint32_t(v.length())};
...
int RGWLibFS::lsxattrs(RGWFileHandle* rgw_fh, rgw_xattrstr* filter_prefix, rgw_getxattr_cb cb, void* cb_arg,
                       uint32_t flags)
{
    /* cannot store on fs_root, should not on buckets? */
    if ((rgw_fh->is_bucket()) || (rgw_fh->is_root())) {
    ...
    if (rc3 == 0) {
        ...
        for (const auto& k : keys) {
            ...
            /* return exposed attr keys as given, else unprefix */
            std::string_view svk =
                is_exposed_attr(rgw_xattrstr{const_cast<char*>(k.first.c_str()), uint32_t(k.first.length())})
                    ? k.first
            ...
            if (cbr & RGW_LSXATTR_FLAG_STOP) break;
        }
    }
...
int RGWLibFS::setxattrs(RGWFileHandle* rgw_fh, rgw_xattrlist* attrs, uint32_t flags)
{
    /* cannot store on fs_root, should not on buckets? */
    if ((rgw_fh->is_bucket()) || (rgw_fh->is_root())) {
        return -EINVAL;
    ...
    for (uint32_t ix = 0; ix < attrs->xattr_cnt; ++ix) {
        ...
        if (!(xattr.key.len > 0)) continue;

        /* reject lexical match with any exposed attr */
        if (is_exposed_attr(xattr.key)) continue;
...
int RGWLibFS::rmxattrs(RGWFileHandle* rgw_fh, rgw_xattrlist* attrs, uint32_t flags)
{
    /* cannot store on fs_root, should not on buckets? */
    if ((rgw_fh->is_bucket()) || (rgw_fh->is_root())) {
        return -EINVAL;
    ...
    RGWRMAttrsRequest req(cct, rgwlib.get_store()->get_user(user.user_id), rgw_fh->bucket_name(), obj_name);
...
void RGWLibFS::close()
{
    ...
    public:
        explicit ObjUnref(RGWLibFS* _fs)
            : fs(_fs)
        {}
        void operator()(RGWFileHandle* fh) const
        {
            lsubdout(fs->get_context(), rgw, 5)
                << __PRETTY_FUNCTION__ << fh->name << " before ObjUnref refs=" << fh->get_refcnt() << dendl;
            fs->unref(fh);
        }
    };

    /* force cache drain, forces objects to evict */
    fh_cache.drain(ObjUnref(this), RGWFileHandle::FHCache::FLAG_LOCK);
...
std::ostream& operator<<(std::ostream& os, RGWLibFS::event const& ev)
{
    os << "<event:";
    switch (ev.t) {
    case RGWLibFS::event::type::READDIR: os << "type=READDIR;"; break;
    default: os << "type=UNKNOWN;"; break;
    };
    os << "fid=" << ev.fhk.fh_hk.bucket << ":" << ev.fhk.fh_hk.object << ";ts=" << ev.ts << ">";
    return os;
}
...
void RGWLibFS::gc()
{
    ...
    auto expire_s = get_context()->_conf->rgw_nfs_namespace_expire_secs;
    ...
    do {
        ...
        lsubdout(get_context(), rgw, 15) << "GC: top of expire loop"
                                         << " now=" << now << " expire_s=" << expire_s << dendl;
        {
            lock_guard guard(state.mtx); /* LOCKED */
            lsubdout(get_context(), rgw, 15) << "GC: processing"
                                             << " count=" << events.size() << " events" << dendl;
            /* just return if no events */
            if (events.empty()) {
                return;
            }
            uint32_t _max_ev = (events.size() < 500) ? max_ev : (events.size() / 4);
                ...
                if (rgw_fh) {
                    ...
                    if (unlikely(!rgw_fh->is_dir())) {
                        lsubdout(get_context(), rgw, 0)
                            << __func__ << " BUG non-directory found with READDIR event "
                            << "(" << rgw_fh->bucket_name() << "," << rgw_fh->object_name() << ")" << dendl;
                        goto rele;
                    }
                    /* maybe clear state */
                    ...
                    if (d) {
                        ...
                        if (unlikely(ev_ts < d_last_readdir)) {
                            ...
                            lsubdout(get_context(), rgw, 15)
                                << "GC: delay expiration for " << rgw_fh->object_name() << " ev.ts=" << ev_ts
                                << " last_readdir=" << d_last_readdir << dendl;
                            continue;
                        }
                        else {
...
std::ostream& operator<<(std::ostream& os, RGWFileHandle const& rgw_fh)
{
    ...
    os << "addr=" << &rgw_fh << ";";
    switch (fh->fh_type) {
    case RGW_FS_TYPE_DIRECTORY: os << "type=DIRECTORY;"; break;
    case RGW_FS_TYPE_FILE: os << "type=FILE;"; break;
    default: os << "type=UNKNOWN;"; break;
    };
...
void RGWFileHandle::encode_attrs(ceph::buffer::list& ux_key1, ceph::buffer::list& ux_attrs1, bool inc_ov)
{
    using ceph::encode;
    fh_key fhk(this->fh.fh_hk);
    encode(fhk, ux_key1);
    bool need_ondisk_version = (fh.fh_type == RGW_FS_TYPE_FILE || fh.fh_type == RGW_FS_TYPE_SYMBOLIC_LINK);
    if (need_ondisk_version && file_ondisk_version < 0) {
...
DecodeAttrsResult RGWFileHandle::decode_attrs(const ceph::buffer::list* ux_key1, const ceph::buffer::list* ux_attrs1)
{
    using ceph::decode;
    ...
    // file handle
    if (!((fh.fh_type == RGW_FS_TYPE_FILE || fh.fh_type == RGW_FS_TYPE_SYMBOLIC_LINK) &&
...
{
    if (unlikely(!is_dir())) return false;

    RGWRMdirCheck req(fs->get_context(), rgwlib.get_store()->get_user(fs->get_user()->user_id), this);
    int rc = rgwlib.get_fe()->execute_req(&req);
    if (!rc) {
...
std::ostream& operator<<(std::ostream& os, RGWFileHandle::readdir_offset const& offset)
{
    using boost::get;
...
int RGWFileHandle::readdir(rgw_readdir_cb rcb, void* cb_arg, readdir_offset offset, bool* eof, uint32_t flags)
{
    ...
    if (is_root()) {
        RGWListBucketsRequest req(
            cct, rgwlib.get_store()->get_user(fs->get_user()->user_id), this, rcb, cb_arg, offset);
        rc = rgwlib.get_fe()->execute_req(&req);
        if (!rc) {
            (void)clock_gettime(CLOCK_MONOTONIC_COARSE, &now); /* !LOCKED */
            lock_guard guard(mtx);
            state.atime = now;
            if (initial_off) set_nlink(2);
            inc_nlink(req.d_count);
            *eof = req.eof();
        }
    }
    else {
        RGWReaddirRequest req(cct, rgwlib.get_store()->get_user(fs->get_user()->user_id), this, rcb, cb_arg, offset);
        rc = rgwlib.get_fe()->execute_req(&req);
        if (!rc) {
            (void)clock_gettime(CLOCK_MONOTONIC_COARSE, &now); /* !LOCKED */
            lock_guard guard(mtx);
...
int RGWFileHandle::write(uint64_t off, size_t len, size_t* bytes_written, void* buffer)
{
    using std::get;
    ...
    if (deleted()) {
        lsubdout(fs->get_context(), rgw, 5)
            << __func__ << " write attempted on deleted object " << this->object_name() << dendl;
    ...
    if (!f->write_req) {
        ...
        if (off != 0) {
            lsubdout(fs->get_context(), rgw, 5) << __func__ << " " << object_name() << " non-0 initial write position "
                                                << off << " (mounting with -o sync required)" << dendl;
            return -EIO;
        }

        /* start */
        std::string object_name = relative_object_name();
        f->write_req = new RGWWriteRequest(rgwlib.get_store(),
        ...
        if (rc < 0) {
            lsubdout(fs->get_context(), rgw, 5)
                << __func__ << this->object_name() << " write start failed " << off << " (" << rc << ")" << dendl;
            ...
            if (stateless_open()) {
                /* start write timer */
                f->write_req->timer_id = RGWLibFS::write_timer.add_event(
                    std::chrono::seconds(RGWLibFS::write_completion_interval_s), WriteCompletion(*this));
            }
        }
    }

    int overlap = 0;
    if ((static_cast<off_t>(off) < f->write_req->real_ofs) && ((f->write_req->real_ofs - off) <= len)) {
    ...
    bl.push_back(
      buffer::create_static(len, static_cast<char*>(buffer)));
#else
    bl.push_back(buffer::copy(static_cast<char*>(buffer), len));
#endif
    ...
    if (rc == 0) {
        ...
        if (min_size > get_size()) set_size(min_size);
        if (stateless_open()) {
            /* bump write timer */
            RGWLibFS::write_timer.adjust_event(f->write_req->timer_id, std::chrono::seconds(10));
        }
    }
    else {
        /* continuation failed (e.g., non-contiguous write position) */
        lsubdout(fs->get_context(), rgw, 5) << __func__ << object_name() << " failed write at position " << off
                                            << " (fails write transaction) " << dendl;
        /* zap failed write transaction */
        delete f->write_req;
        f->write_req = nullptr;
        rc = -EIO;
...
void RGWFileHandle::advance_mtime(uint32_t flags)
{
    ...
    cmptime.tv_sec += fs->get_context()->_conf->rgw_nfs_namespace_expire_secs;
...
int RGWWriteRequest::exec_start()
{
    ...
    auto compression_type =
        get_store()->svc()->zone->get_zone_params().get_compression_type(state->bucket->get_placement_rule());
...
int RGWWriteRequest::exec_continue()
{
    ...
    if (eio) {
        ldout(state->cct, 5) << " chunks arrived in wrong order"
                             << " (mounting with -o sync required)" << dendl;
        return -EIO;
    }

    op_ret = state->bucket->check_quota(user_quota, bucket_quota, real_ofs, null_yield, true);
    /* max_size exceed */
    if (op_ret < 0) return -EIO;
...
int RGWWriteRequest::exec_finish()
{
    ...
    for (iter = state->generic_attrs.begin(); iter != state->generic_attrs.end(); ++iter) {
    ...
    op_ret = processor->complete(state->obj_size,
                                 etag,
                                 &mtime,
                                 real_time(),
                                 attrs,
...
{
    if (major) *major = LIBRGW_FILE_VER_MAJOR;
    if (minor) *minor = LIBRGW_FILE_VER_MINOR;
    if (extra) *extra = LIBRGW_FILE_VER_EXTRA;
}

/*
 attach rgw namespace
*/
int rgw_mount(librgw_t rgw, const char* uid, const char* acc_key, const char* sec_key, struct rgw_fs** rgw_fs,
              uint32_t flags)
{
    int rc = 0;

    /* stash access data for "mount" */
    RGWLibFS* new_fs = new RGWLibFS(static_cast<CephContext*>(rgw), uid, acc_key, sec_key, "/");
    ceph_assert(new_fs);
...
int rgw_mount2(librgw_t rgw, const char* uid, const char* acc_key, const char* sec_key, const char* root,
               struct rgw_fs** rgw_fs, uint32_t flags)
{
    int rc = 0;
    ...
    if (root && (!strcmp(root, ""))) {
        /* stash access data for "mount" */
        new_fs = new RGWLibFS(static_cast<CephContext*>(rgw), uid, acc_key, sec_key, "/");
    }
    else {
        /* stash access data for "mount" */
        new_fs = new RGWLibFS(static_cast<CephContext*>(rgw), uid, acc_key, sec_key, root);
    }
...
*/
int rgw_register_invalidate(struct rgw_fs* rgw_fs, rgw_fh_callback_t cb, void* arg, uint32_t flags)
...
int rgw_statfs(struct rgw_fs* rgw_fs, struct rgw_file_handle* parent_fh, struct rgw_statvfs* vfs_st, uint32_t flags)
{
    ...
    RGWGetClusterStatReq req(fs->get_context(), rgwlib.get_store()->get_user(fs->get_user()->user_id), stats);
    int rc = rgwlib.get_fe()->execute_req(&req);
    if (rc < 0) {
        lderr(fs->get_context()) << "ERROR: getting total cluster usage" << cpp_strerror(-rc) << dendl;
        return rc;
    }
...
*/
int rgw_create(struct rgw_fs* rgw_fs, struct rgw_file_handle* parent_fh, const char* name, struct stat* st,
               uint32_t mask, struct rgw_file_handle** fh, uint32_t posix_flags, uint32_t flags)
{
    using std::get;
    ...
    if ((!parent) || (parent->is_root()) || (parent->is_file())) {
        /* bad parent */
        return -EINVAL;
    }
 ...
 */
int rgw_symlink(struct rgw_fs* rgw_fs, struct rgw_file_handle* parent_fh, const char* name, const char* link_path,
                struct stat* st, uint32_t mask, struct rgw_file_handle** fh, uint32_t posix_flags, uint32_t flags)
{
    using std::get;
...
*/
int rgw_mkdir(struct rgw_fs* rgw_fs, struct rgw_file_handle* parent_fh, const char* name, struct stat* st,
              uint32_t mask, struct rgw_file_handle** fh, uint32_t flags)
{
    using std::get;
...
/*
  rename object
*/
int rgw_rename(struct rgw_fs* rgw_fs, struct rgw_file_handle* src, const char* src_name, struct rgw_file_handle* dst,
               const char* dst_name, uint32_t flags)
{
    RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
...
*/
int rgw_unlink(struct rgw_fs* rgw_fs, struct rgw_file_handle* parent_fh, const char* name, uint32_t flags)
{
    RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
    RGWFileHandle* parent = get_rgwfh(parent_fh);
...
*/
int rgw_lookup(struct rgw_fs* rgw_fs, struct rgw_file_handle* parent_fh, const char* path, struct rgw_file_handle** fh,
               struct stat* st, uint32_t mask, uint32_t flags)
{
    ...
    if (parent->is_root()) {
        ...
        if (unlikely((strcmp(path, "..") == 0))) {
            ...
            uint32_t sl_flags =
                (flags & RGW_LOOKUP_FLAG_RCB) ? RGWFileHandle::FLAG_IN_CB : RGWFileHandle::FLAG_EXACT_MATCH;
...
*/
int rgw_lookup_handle(struct rgw_fs* rgw_fs, struct rgw_fh_hk* fh_hk, struct rgw_file_handle** fh, uint32_t flags)
{
    RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
 ...
 * release file handle
 */
int rgw_fh_rele(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, uint32_t flags)
{
    RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
...
*/
int rgw_getattr(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, struct stat* st, uint32_t flags)
{
    RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
    RGWFileHandle* rgw_fh = get_rgwfh(fh);
...
*/
int rgw_setattr(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, struct stat* st, uint32_t mask, uint32_t flags)
{
    RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
    RGWFileHandle* rgw_fh = get_rgwfh(fh);
...
/*
   truncate file
*/
int rgw_truncate(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, uint64_t size, uint32_t flags)
{
    return 0;
}
...
int rgw_open(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, uint32_t posix_flags, uint32_t flags)
{
     ...
     */
    if (!rgw_fh->is_file()) return -EISDIR;
...
/*
   close file
*/
int rgw_close(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, uint32_t flags)
{
    RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
    RGWFileHandle* rgw_fh = get_rgwfh(fh);
    int rc = rgw_fh->close(/* XXX */);

    if (flags & RGW_CLOSE_FLAG_RELE) fs->unref(rgw_fh);

    return rc;
}

int rgw_readdir(struct rgw_fs* rgw_fs, struct rgw_file_handle* parent_fh, uint64_t* offset, rgw_readdir_cb rcb,
                void* cb_arg, bool* eof, uint32_t flags)
{
    RGWFileHandle* parent = get_rgwfh(parent_fh);
    if (!parent) {
        /* bad parent */
...
int rgw_readdir2(struct rgw_fs* rgw_fs, struct rgw_file_handle* parent_fh, const char* name, rgw_readdir_cb rcb,
                 void* cb_arg, bool* eof, uint32_t flags)
{
    RGWFileHandle* parent = get_rgwfh(parent_fh);
    if (!parent) {
    ...
    if ((!name) && (flags & RGW_READDIR_FLAG_DOTDOT)) {
        /* send '.' and '..' with their NFS-defined offsets */
        rcb(".", cb_arg, 1, nullptr, 0, RGW_LOOKUP_FLAG_DIR);
        rcb("..", cb_arg, 2, nullptr, 0, RGW_LOOKUP_FLAG_DIR);
    }
...
int rgw_dirent_offset(struct rgw_fs* rgw_fs, struct rgw_file_handle* parent_fh, const char* name, int64_t* offset,
                      uint32_t flags)
{
    RGWFileHandle* parent = get_rgwfh(parent_fh);
    if ((!parent)) {
...
/*
   read data from file
*/
int rgw_read(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, uint64_t offset, size_t length, size_t* bytes_read,
...
/*
   read symbolic link
*/
int rgw_readlink(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, uint64_t offset, size_t length, size_t* bytes_read,
                 void* buffer, uint32_t flags)
{
    RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
    RGWFileHandle* rgw_fh = get_rgwfh(fh);
...
/*
   write data to file
*/
int rgw_write(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, uint64_t offset, size_t length, size_t* bytes_written,
              void* buffer, uint32_t flags)
{
    RGWFileHandle* rgw_fh = get_rgwfh(fh);
    int rc;
...
public:
    RGWReadV(buffer::list& _bl, rgw_vio* _vio)
        : vio(_vio)
    {
        bl = std::move(_bl);
    }
...
int rgw_readv(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, rgw_uio* uio, uint32_t flags)
{
#if 0 /* XXX */
  CephContext* cct = static_cast<CephContext*>(rgw_fs->rgw);
  RGWLibFS *fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
...
*/
int rgw_writev(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, rgw_uio* uio, uint32_t flags)
{
    ...
    for (unsigned int ix = 0; ix < uio->uio_cnt; ++ix) {
        rgw_vio* vio = &(uio->uio_vio[ix]);
        bl.push_back(buffer::create_static(vio->vio_len, static_cast<char*>(vio->vio_base)));
    }

    std::string oname = rgw_fh->relative_object_name();
    RGWPutObjRequest req(cct, rgwlib.get_store()->get_user(fs->get_user()->user_id), rgw_fh->bucket_name(), oname, bl);
...
/*
   sync written data
*/
int rgw_fsync(struct rgw_fs* rgw_fs, struct rgw_file_handle* handle, uint32_t flags)
{
    return 0;
}
...
int rgw_getxattrs(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, rgw_xattrlist* attrs, rgw_getxattr_cb cb,
...
int rgw_lsxattrs(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, rgw_xattrstr* filter_prefix /* ignored */,
...
int rgw_setxattrs(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, rgw_xattrlist* attrs, uint32_t flags)
{
    RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
    RGWFileHandle* rgw_fh = get_rgwfh(fh);
...
int rgw_rmxattrs(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, rgw_xattrlist* attrs, uint32_t flags)
{
    RGWLibFS* fs = static_cast<RGWLibFS*>(rgw_fs->fs_private);
    RGWFileHandle* rgw_fh = get_rgwfh(fh);
