...
namespace {
boost::optional<Partition> to_partition(const smatch::value_type& p, bool wildcards)
{
    if (p == "aws") {
...
boost::optional<Service> to_service(const smatch::value_type& s, bool wildcards)
{
    static const unordered_map<string, Service> services = {
        ...
        {"aws-marketplace-management", Service::aws_marketplace_management},
...
ARN::ARN(const std::string& resource_name, const std::string& type, const std::string& tenant, bool has_path)
    ...
    , resource(type)
{
    if (!has_path) resource.push_back('/');
...
boost::optional<ARN> ARN::parse(const std::string& s, bool wildcards)
{
    ...
    static const std::regex rx_no_wild("arn:([^:*]*):([^:*]*):([^:*]*):([^:*]*):(.*)",
                                       std::regex_constants::ECMAScript | std::regex_constants::optimize);
    ...
    else if (regex_match(s, match, wildcards ? rx_wild : rx_no_wild) && match.size() == 6) {
...
{
    return ((l.partition == r.partition) && (l.service == r.service) && (l.region == r.region) &&
            (l.account == r.account) && (l.resource == r.resource));
}
bool operator<(const ARN& l, const ARN& r)
{
    return ((l.partition < r.partition) || (l.service < r.service) || (l.region < r.region) ||
...
bool ARN::match(const ARN& candidate) const
{
    if ((candidate.partition == Partition::wildcard) ||
        (partition != candidate.partition && partition != Partition::wildcard)) {
        return false;
    }

    if ((candidate.service == Service::wildcard) || (service != candidate.service && service != Service::wildcard)) {
        return false;
    }
