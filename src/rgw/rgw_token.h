...
public:
    ...
    {
        if (boost::iequals(s, "ad")) return TOKEN_AD;
        if (boost::iequals(s, "ldap")) return TOKEN_LDAP;
        if (boost::iequals(s, "keystone")) return TOKEN_KEYSTONE;
        return TOKEN_NONE;
    }
    ...
    static const char* from_type(enum token_type type)
    {
        ...
        case TOKEN_LDAP: return "ldap";
        case TOKEN_KEYSTONE: return "keystone";
        default: return "none";
        };
    }
    ...
    bool valid() const { return ((type != TOKEN_NONE) && (!id.empty()) && (!key.empty())); }

    RGWToken()
        : type(TOKEN_NONE){};

    RGWToken(enum token_type _type, const std::string& _id, const std::string& _key)
        : type(_type)
        , id(_id)
        , key(_key){};
...
{
    os << "<<RGWToken"
       << " type=" << RGWToken::from_type(token.type) << " id=" << token.id << " key=" << token.key << ">>";
    return os;
}
