...
int RGWRESTSimpleRequest::execute(const DoutPrefixProvider* dpp, RGWAccessKey& key, const char* _method,
                                  const char* resource, optional_yield y)
{
    method = _method;
    string new_url = url;
...
int RGWRESTSimpleRequest::forward_request(const DoutPrefixProvider* dpp, RGWAccessKey& key, req_info& info,
...
void RGWRESTGenerateHTTPHeaders::init(const string& _method, const string& _url, const string& resource,
...
static std::set<string> keep_headers = {"content-type", "content-encoding", "content-disposition", "content-language"};
...
int RGWRESTStreamS3PutObj::send_ready(const DoutPrefixProvider* dpp, RGWAccessKey& key,
                                      map<string, bufferlist>& rgw_attrs, bool send)
{
...
int RGWRESTStreamS3PutObj::send_ready(const DoutPrefixProvider* dpp, RGWAccessKey& key,
                                      const map<string, string>& http_attrs, RGWAccessControlPolicy& policy, bool send)
{
...
int RGWRESTStreamS3PutObj::put_obj_init(const DoutPrefixProvider* dpp, RGWAccessKey& key, rgw::sal::RGWObject* obj,
                                        uint64_t obj_size, map<string, bufferlist>& attrs, bool send)
{
    send_init(obj);
...
int RGWRESTStreamRWRequest::complete_request(optional_yield y, string* etag, real_time* mtime, uint64_t* psize,
    ...
    if (status >= 0) {
        ...
        if (psize) {
            ...
            *psize = strict_strtoll(size_str.c_str(), 10, &err);
            if (!err.empty()) {
                ldout(cct, 0) << "ERROR: failed parsing embedded metadata object size (" << size_str << ") to int "
                              << dendl;
                return -EIO;
            }
        }
...
public:
    explicit StreamIntoBufferlist(bufferlist& _bl)
        : bl(_bl)
    {}
