...
{
    if (!send_iter) return 0;

    if (len > send_iter->get_remaining()) len = send_iter->get_remaining();
...
int RGWHTTPSimpleRequest::receive_data(void* ptr, size_t len, bool* pause)
{
    ...
    if (left_len == 0) return 0; /* don't read extra data */
...
static int sign_request_v2(const DoutPrefixProvider* dpp, const RGWAccessKey& key, const string& region,
                           const string& service, RGWEnv& env, req_info& info, const bufferlist* opt_content)
{
    ...
    if (cct->_conf->subsys.should_gather<ceph_subsys_rgw, 20>()) {
        for (const auto& i : env.get_map()) {
            ldpp_dout(dpp, 20) << __func__ << "():> " << i.first << " -> "
                               << rgw::crypt_sanitize::x_meta_map{i.first, i.second} << dendl;
        }
    }
...
static int sign_request_v4(const DoutPrefixProvider* dpp, const RGWAccessKey& key, const string& region,
                           const string& service, RGWEnv& env, req_info& info, const bufferlist* opt_content)
{
    /* don't sign if no key is provided */
    if (key.key.empty()) {
...
static int sign_request(const DoutPrefixProvider* dpp, const RGWAccessKey& key, const string& region,
                        const string& service, RGWEnv& env, req_info& info, const bufferlist* opt_content)
{
...
static bool identify_scope(const DoutPrefixProvider* dpp, CephContext* cct, const string& host, string* region,
                           string& service)
{
...
static void scope_from_api_name(const DoutPrefixProvider* dpp, CephContext* cct, const string& host,
                                std::optional<string> api_name, string* region, string& service)
{
...
int RGWRESTSimpleRequest::forward_request(const DoutPrefixProvider* dpp, const RGWAccessKey& key, req_info& info,
...
public:
    explicit RGWRESTStreamOutCB(RGWRESTStreamS3PutObj* _req)
        : req(_req)
    {}
    int handle_data(bufferlist& bl, off_t bl_ofs,
                    off_t bl_len) override; /* callback for object iteration when sending data */
};

int RGWRESTStreamOutCB::handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
    dout(20) << "RGWRESTStreamOutCB::handle_data bl.length()=" << bl.length() << " bl_ofs=" << bl_ofs
             << " bl_len=" << bl_len << dendl;
...
static void grants_by_type_add_one_grant(map<int, string>& grants_by_type, int perm, ACLGrant& grant)
{
    ...
    case ACL_TYPE_GROUP: id_type_str = "uri"; break;
    case ACL_TYPE_EMAIL_USER: id_type_str = "emailAddress"; break;
    default: id_type_str = "id";
    }
    rgw_user id;
    grant.get_id(id);
...
struct grant_type_to_header grants_headers_def[] = {{RGW_PERM_FULL_CONTROL, "x-amz-grant-full-control"},
...
static void grants_by_type_add_perm(map<int, string>& grants_by_type, int perm, ACLGrant& grant)
{
    ...
    for (t = grants_headers_def; t->header; t++) {
        if (grants_by_type_check_perm(grants_by_type, perm, grant, t->type)) return;
    }
}
...
void RGWRESTStreamS3PutObj::send_init(const rgw_obj& obj)
{
    ...
    if (new_url[new_url.size() - 1] != '/') new_url.append("/");
...
void RGWRESTStreamS3PutObj::send_ready(const DoutPrefixProvider* dpp, RGWAccessKey& key,
                                       const map<string, string>& http_attrs, RGWAccessControlPolicy& policy)
{
...
void RGWRESTStreamS3PutObj::put_obj_init(const DoutPrefixProvider* dpp, RGWAccessKey& key, const rgw_obj& obj,
...
int RGWRESTStreamRWRequest::send_request(const DoutPrefixProvider* dpp, RGWAccessKey& key,
                                         map<string, string>& extra_headers, const rgw_obj& obj, RGWHTTPManager* mgr)
{
    string resource;
...
int RGWRESTStreamRWRequest::send_prepare(const DoutPrefixProvider* dpp, RGWAccessKey& key,
                                         map<string, string>& extra_headers, const rgw_obj& obj)
{
    string resource;
    send_prepare_convert(obj, &resource);
...
int RGWRESTStreamRWRequest::send_prepare(const DoutPrefixProvider* dpp, RGWAccessKey* key,
                                         map<string, string>& extra_headers, const string& resource,
...
int RGWRESTStreamRWRequest::do_send_prepare(const DoutPrefixProvider* dpp, RGWAccessKey* key,
                                            map<string, string>& extra_headers, const string& resource,
                                            bufferlist* send_data)
{
    string new_url = url;
    if (!new_url.empty() && new_url.back() != '/') new_url.append("/");
...
int RGWHTTPStreamRWRequest::complete_request(optional_yield y, string* etag, real_time* mtime, uint64_t* psize,
                                             map<string, string>* pattrs, map<string, string>* pheaders)
{
    int ret = wait(y);
    if (ret < 0) {
        return ret;
    ...
    if (status >= 0) {
        ...
        if (psize) {
            ...
            *psize = strict_strtoll(size_str.c_str(), 10, &err);
            if (!err.empty()) {
                ldpp_dout(this, 0) << "ERROR: failed parsing embedded metadata object size (" << size_str << ") to int "
                                   << dendl;
                return -EIO;
            }
        }
