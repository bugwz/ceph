...
{
    if (tag_map.size() == max_obj_tags || key.size() > max_tag_key_size || val.size() > max_tag_val_size ||
        key.size() == 0) {
...
int RGWObjTags::set_from_string(const string& input)
{
    ...
    for (const auto& kv : kvs) {
        ...
        string key, val;
        if (p != string::npos) {
            ret = check_and_add_tag(url_decode(kv.substr(0, p)), url_decode(kv.substr(p + 1)));
        }
        else {
