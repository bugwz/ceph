...
int RGWRadosUser::list_buckets(const DoutPrefixProvider* dpp, const string& marker, const string& end_marker,
                               uint64_t max, bool need_stats, RGWBucketList& buckets, optional_yield y)
{
...
RGWBucket* RGWRadosUser::create_bucket(rgw_bucket& bucket, ceph::real_time creation_time)
{
    return NULL;
}
...
int RGWRadosBucket::remove_bucket(const DoutPrefixProvider* dpp, bool delete_children, bool forward_to_master,
                                  req_info* req_info, optional_yield y)
{
    ...
    (void)store->getRados()->get_lc()->remove_bucket_config(this->info, get_attrs());
...
int RGWRadosBucket::get_bucket_info(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    int ret = store->ctl()->bucket->read_bucket_info(
        info.bucket,
        &info,
        y,
        dpp,
        RGWBucketCtl::BucketInstance::GetParams().set_mtime(&mtime).set_attrs(&attrs).set_bectx_params(bectx_params),
        &ep_ot);
    if (ret == 0) {
...
int RGWRadosBucket::load_by_name(const DoutPrefixProvider* dpp, const std::string& tenant,
                                 const std::string& bucket_name, const std::string bucket_instance_id,
                                 RGWSysObjectCtx* rctx, optional_yield y)
{
                                     ...
                                     bool* syncstopped)
{
    return store->getRados()->get_bucket_stats(
        dpp, bucket_info, shard_id, bucket_ver, master_ver, stats, max_marker, syncstopped);
}
...
int RGWRadosBucket::link(const DoutPrefixProvider* dpp, RGWUser* new_user, optional_yield y)
{
    ...
    return store->ctl()->bucket->link_bucket(
        new_user->get_user(), info.bucket, ceph::real_time(), y, dpp, true, &ep_data);
}
...
int RGWRadosBucket::chown(RGWUser* new_user, RGWUser* old_user, optional_yield y, const DoutPrefixProvider* dpp)
{
    ...
    return store->ctl()->bucket->chown(
        store, info, new_user->get_user(), old_user->get_display_name(), obj_marker, y, dpp);
}
...
{
    return store->getRados()->check_quota(
        owner->get_user(), get_key(), user_quota, bucket_quota, obj_size, y, check_size_only);
}

int RGWRadosBucket::set_instance_attrs(const DoutPrefixProvider* dpp, RGWAttrs& attrs, optional_yield y)
{
    return store->ctl()->bucket->set_bucket_instance_attrs(get_info(), attrs, &get_info().objv_tracker, y, dpp);
}
...
int RGWRadosBucket::list(const DoutPrefixProvider* dpp, ListParams& params, int max, ListResults& results,
                         optional_yield y)
{
...
int RGWRadosObject::get_obj_state(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, RGWBucket& bucket,
                                  RGWObjState** state, optional_yield y, bool follow_olh)
{
...
int RGWRadosObject::read_attrs(RGWRados::Object::Read& read_op, optional_yield y, const DoutPrefixProvider* dpp,
                               rgw_obj* target_obj)
{
...
int RGWRadosObject::set_obj_attrs(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, RGWAttrs* setattrs,
                                  RGWAttrs* delattrs, optional_yield y, rgw_obj* target_obj)
{
    RGWAttrs empty;
    rgw_obj target = get_obj();

    if (!target_obj) target_obj = &target;

    return store->getRados()->set_attrs(
        dpp, rctx, bucket->get_info(), *target_obj, setattrs ? *setattrs : empty, delattrs ? delattrs : nullptr, y);
}

int RGWRadosObject::get_obj_attrs(RGWObjectCtx* rctx, optional_yield y, const DoutPrefixProvider* dpp,
                                  rgw_obj* target_obj)
{
...
int RGWRadosObject::delete_obj_attrs(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, const char* attr_name,
                                     optional_yield y)
{
    RGWAttrs rmattr;
    bufferlist bl;
...
int RGWRadosObject::copy_obj_data(RGWObjectCtx& rctx, RGWBucket* dest_bucket, RGWObject* dest_obj, uint16_t olh_epoch,
                                  std::string* petag, const DoutPrefixProvider* dpp, optional_yield y)
{
    RGWAttrs attrset;
    ...
    return store->getRados()->copy_obj_data(rctx,
                                            dest_bucket,
                                            dest_bucket->get_info().placement_rule,
                                            read_op,
                                            obj_size - 1,
                                            dest_obj,
                                            NULL,
...
int RGWRadosObject::omap_get_vals_by_keys(const DoutPrefixProvider* dpp, const std::string& oid,
                                          const std::set<std::string>& keys, RGWAttrs* vals)
{
    int ret;
...
int RGWRadosObject::transition(RGWObjectCtx& rctx, RGWBucket* bucket, const rgw_placement_rule& placement_rule,
...
int RGWRadosObject::get_max_chunk_size(const DoutPrefixProvider* dpp, rgw_placement_rule placement_rule,
...
void RGWRadosObject::get_max_aligned_size(uint64_t size, uint64_t alignment, uint64_t* max_size)
{
...
int RGWRadosObject::RadosReadOp::get_attr(const DoutPrefixProvider* dpp, const char* name, bufferlist& dest,
                                          optional_yield y)
{
    return parent_op.get_attr(dpp, name, dest, y);
}

int RGWRadosObject::delete_object(const DoutPrefixProvider* dpp, RGWObjectCtx* obj_ctx, ACLOwner obj_owner,
                                  ACLOwner bucket_owner, ceph::real_time unmod_since, bool high_precision_time,
                                  uint64_t epoch, std::string& version_id, optional_yield y, bool prevent_versioning)
{
    int ret = 0;
...
int RGWRadosObject::copy_object(RGWObjectCtx& obj_ctx, RGWUser* user, req_info* info, const rgw_zone_id& source_zone,
                                rgw::sal::RGWObject* dest_object, rgw::sal::RGWBucket* dest_bucket,
                                rgw::sal::RGWBucket* src_bucket, const rgw_placement_rule& dest_placement,
                                ...
                                const char* if_nomatch, AttrsMod attrs_mod, bool copy_if_newer, RGWAttrs& attrs,
                                RGWObjCategory category, uint64_t olh_epoch, boost::optional<ceph::real_time> delete_at,
                                string* version_id, string* tag, string* etag, void (*progress_cb)(off_t, void*),
...
int RGWRadosObject::RadosReadOp::iterate(const DoutPrefixProvider* dpp, int64_t ofs, int64_t end, RGWGetDataCB* cb,
                                         optional_yield y)
{
...
int RGWRadosObject::RadosWriteOp::write_meta(const DoutPrefixProvider* dpp, uint64_t size, uint64_t accounted_size,
                                             optional_yield y)
{
...
int RGWRadosObject::swift_versioning_restore(RGWObjectCtx* obj_ctx, bool& restored, const DoutPrefixProvider* dpp)
{
    return store->getRados()->swift_versioning_restore(
        *obj_ctx, bucket->get_owner()->get_id(), bucket, this, restored, dpp);
}

int RGWRadosObject::swift_versioning_copy(RGWObjectCtx* obj_ctx, const DoutPrefixProvider* dpp, optional_yield y)
{
    return store->getRados()->swift_versioning_copy(*obj_ctx, bucket->get_info().owner, bucket, this, dpp, y);
}

int RGWRadosStore::get_bucket(const DoutPrefixProvider* dpp, RGWUser* u, const rgw_bucket& b,
                              std::unique_ptr<RGWBucket>* bucket, optional_yield y)
{
    int ret;
    RGWBucket* bp;
...
int RGWRadosStore::get_bucket(const DoutPrefixProvider* dpp, RGWUser* u, const std::string& tenant,
                              const std::string& name, std::unique_ptr<RGWBucket>* bucket, optional_yield y)
{
    rgw_bucket b;
...
static int rgw_op_get_bucket_policy_from_attr(const DoutPrefixProvider* dpp, RGWRadosStore* store, RGWUser& user,
                                              RGWAttrs& bucket_attrs, RGWAccessControlPolicy* policy, optional_yield y)
{
...
int RGWRadosStore::defer_gc(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, RGWBucket* bucket, RGWObject* obj,
                            optional_yield y)
{
...
int RGWRadosStore::create_bucket(const DoutPrefixProvider* dpp, RGWUser& u, const rgw_bucket& b,
                                 const string& zonegroup_id, rgw_placement_rule& placement_rule,
                                 string& swift_ver_location, const RGWQuotaInfo* pquota_info,
                                 const RGWAccessControlPolicy& policy, RGWAttrs& attrs, RGWBucketInfo& info,
                                 obj_version& ep_objv, bool exclusive, bool obj_lock_enabled, bool* existed,
                                 req_info& req_info, std::unique_ptr<RGWBucket>* bucket_out, optional_yield y)
{
    int ret;
    bufferlist in_data;
    ...
    if (ret != -ENOENT) {
        ...
        *existed = true;
        ...
        int r = rgw_op_get_bucket_policy_from_attr(dpp, this, u, bucket->get_attrs(), &old_policy, y);
    ...
    if (*existed) {
        ...
        ret = svc()->zone->select_bucket_placement(
            dpp, u.get_info(), zid, placement_rule, &selected_placement_rule, nullptr, y);
        ...
        ret = getRados()->create_bucket(u.get_info(),
                                        bucket->get_key(),
                                        zid,
                                        placement_rule,
                                        swift_ver_location,
...
MPRadosSerializer::MPRadosSerializer(const DoutPrefixProvider* dpp, RGWRadosStore* store, RGWRadosObject* obj,
...
LCRadosSerializer::LCRadosSerializer(RGWRadosStore* store, const std::string& _oid, const std::string& lock_name,
...
int RadosLifecycle::get_entry(const string& oid, const std::string& marker, LCEntry& entry)
{
...
int RadosLifecycle::get_next_entry(const string& oid, std::string& marker, LCEntry& entry)
{
    cls_rgw_lc_entry cls_entry;
...
int RadosLifecycle::list_entries(const string& oid, const string& marker, uint32_t max_entries,
                                 vector<LCEntry>& entries)
{
    entries.clear();
...
LCSerializer* RadosLifecycle::get_serializer(const std::string& lock_name, const std::string& oid,
                                             const std::string& cookie)
{
...
rgw::sal::RGWRadosStore* RGWStoreManager::init_storage_provider(const DoutPrefixProvider* dpp, CephContext* cct,
                                                                bool use_gc_thread, bool use_lc_thread,
...
int rgw::sal::RGWRadosStore::get_obj_head_ioctx(const DoutPrefixProvider* dpp, const RGWBucketInfo& bucket_info,
                                                const rgw_obj& obj, librados::IoCtx* ioctx)
{
