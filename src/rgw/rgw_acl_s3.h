...
{
public:
    explicit RGWAccessControlList_S3(CephContext* _cct)
...
{
public:
    explicit RGWAccessControlPolicy_S3(CephContext* _cct)
    ...
    void to_xml(std::ostream& out);
    int rebuild(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, ACLOwner* owner, RGWAccessControlPolicy& dest,
                std::string& err_msg);
    ...
    int create_from_headers(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, const RGWEnv* env,
                            ACLOwner& _owner);
};
