...
// the parser will parse the input and store info, but will not generate the target object. The object can be allocated
// outside of the parser (stack or heap), and require to implement the decode_xml() API for the values to be populated.
...
// API that should create a set of classes derived from XMLObj implementing xml_end() to create the actual target
// objects
//
...
protected:
    ...
    virtual XMLObj* alloc_obj(const char* el) { return nullptr; }
...
template<class T> bool decode_xml(const char* name, T& val, XMLObj* obj, bool mandatory = false);

template<class T> bool decode_xml(const char* name, std::vector<T>& v, XMLObj* obj, bool mandatory = false);

template<class C>
bool decode_xml(const char* name, C& container, void (*cb)(C&, XMLObj* obj), XMLObj* obj, bool mandatory = false);

template<class T> void decode_xml(const char* name, T& val, T& default_val, XMLObj* obj);
}   // namespace RGWXMLDecoder
...
template<class T> void decode_xml_obj(T& val, XMLObj* obj)
{
    val.decode_xml(obj);
}
...
template<class T> void decode_xml_obj(std::optional<T>& val, XMLObj* obj)
{
    val.emplace();
    decode_xml_obj(*val, obj);
}

template<class T> void do_decode_xml_obj(std::list<T>& l, const std::string& name, XMLObj* obj)
{
    l.clear();
...
template<class T> bool RGWXMLDecoder::decode_xml(const char* name, T& val, XMLObj* obj, bool mandatory)
{
    XMLObjIter iter = obj->find(name);
...
template<class T> bool RGWXMLDecoder::decode_xml(const char* name, std::vector<T>& v, XMLObj* obj, bool mandatory)
{
    XMLObjIter iter = obj->find(name);
    XMLObj* o = iter.get_next();
...
template<class T> void RGWXMLDecoder::decode_xml(const char* name, T& val, T& default_val, XMLObj* obj)
{
    XMLObjIter iter = obj->find(name);
    XMLObj* o = iter.get_next();
    if (!o) {
...
template<class T> static void encode_xml(const char* name, const T& val, ceph::Formatter* f)
{
...
template<class T> static void encode_xml(const char* name, const char* ns, const T& val, ceph::Formatter* f)
{
...
template<class T> static void encode_xml(const char* name, const std::vector<T>& l, ceph::Formatter* f)
{
...
template<class T> static void encode_xml(const char* name, const std::optional<T>& o, ceph::Formatter* f)
{
    if (!o) {
        return;
    }
