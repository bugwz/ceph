...
public:
    RGWCORSRule_S3(const DoutPrefixProvider* dpp)
        : dpp(dpp)
    {}
...
public:
    RGWCORSConfiguration_S3(const DoutPrefixProvider* dpp)
        : dpp(dpp)
    {}
...
public:
    explicit RGWCORSXMLParser_S3(const DoutPrefixProvider* _dpp, CephContext* _cct)
        : dpp(_dpp)
        , cct(_cct)
    {}
};
