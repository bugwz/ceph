...
void LogStatusDump::dump(Formatter* f) const
{
    string s;
    switch (status) {
    case MDLOG_STATUS_WRITE: s = "write"; break;
    case MDLOG_STATUS_SETATTRS: s = "set_attrs"; break;
    case MDLOG_STATUS_REMOVE: s = "remove"; break;
    case MDLOG_STATUS_COMPLETE: s = "complete"; break;
    case MDLOG_STATUS_ABORT: s = "abort"; break;
...
int RGWMetadataLog::add_entry(const DoutPrefixProvider* dpp, const string& hash_key, const string& section,
                              const string& key, bufferlist& bl)
{
    if (!svc.zone->need_to_log_metadata()) return 0;
...
int RGWMetadataLog::store_entries_in_shard(const DoutPrefixProvider* dpp, list<cls_log_entry>& entries, int shard_id,
...
int RGWMetadataLog::list_entries(const DoutPrefixProvider* dpp, void* handle, int max_entries,
                                 list<cls_log_entry>& entries, string* last_marker, bool* truncated)
{
    LogListCtx* ctx = static_cast<LogListCtx*>(handle);
    ...
    int ret = svc.cls->timelog.list(dpp,
                                    ctx->cur_oid,
                                    ctx->from_time,
                                    ctx->end_time,
                                    max_entries,
...
RGWMetadataLogInfoCompletion::RGWMetadataLogInfoCompletion(info_callback_t cb)
    : completion(librados::Rados::aio_create_completion((void*)this, _mdlog_info_completion))
    , callback(cb)
{}
...
int RGWMetadataLog::get_info_async(const DoutPrefixProvider* dpp, int shard_id,
                                   RGWMetadataLogInfoCompletion* completion)
{
    string oid;
    get_shard_oid(shard_id, oid);
    ...
    return svc.cls->timelog.info_async(
        dpp, completion->get_io_obj(), oid, &completion->get_header(), completion->get_completion());
}

int RGWMetadataLog::trim(const DoutPrefixProvider* dpp, int shard_id, const real_time& from_time,
                         const real_time& end_time, const string& start_marker, const string& end_marker)
{
    string oid;
    get_shard_oid(shard_id, oid);

    return svc.cls->timelog.trim(dpp, oid, from_time, end_time, start_marker, end_marker, nullptr, null_yield);
}

int RGWMetadataLog::lock_exclusive(const DoutPrefixProvider* dpp, int shard_id, timespan duration, string& zone_id,
                                   string& owner_id)
{
    string oid;
    get_shard_oid(shard_id, oid);
...
public:
    RGWMetadataTopHandler(RGWSI_Meta* meta_svc, RGWMetadataManager* _mgr)
        : mgr(_mgr)
    {
    ...
    int put(string& entry, RGWMetadataObject* obj, RGWObjVersionTracker& objv_tracker, optional_yield y,
            const DoutPrefixProvider* dpp, RGWMDLogSyncType type, bool from_remote_zone) override
    {
        return -ENOTSUP;
    }
    ...
    int mutate(const string& entry, const ceph::real_time& mtime, RGWObjVersionTracker* objv_tracker, optional_yield y,
               const DoutPrefixProvider* dpp, RGWMDLogStatus op_type, std::function<int()> f)
    {
        return -ENOTSUP;
    }
...
RGWMetadataHandler_GenericMetaBE::Put::Put(RGWMetadataHandler_GenericMetaBE* _handler,
                                           RGWSI_MetaBackend_Handler::Op* _op, string& _entry, RGWMetadataObject* _obj,
...
RGWMetadataHandlerPut_SObj::RGWMetadataHandlerPut_SObj(RGWMetadataHandler_GenericMetaBE* handler,
    ...
    : Put(handler, op, entry, obj, objv_tracker, y, type, from_remote_zone)
{}
...
int RGWMetadataHandlerPut_SObj::put_pre(const DoutPrefixProvider* dpp)
{
    ...
    if (!handler->check_versions(
            exists, old_ver, old_mtime, objv_tracker.write_version, obj->get_mtime(), apply_type)) {
...
{
    return be_handler->call([&](RGWSI_MetaBackend_Handler::Op* op) { return do_get(op, entry, obj, y, dpp); });
}

int RGWMetadataHandler_GenericMetaBE::put(string& entry, RGWMetadataObject* obj, RGWObjVersionTracker& objv_tracker,
                                          optional_yield y, const DoutPrefixProvider* dpp, RGWMDLogSyncType type,
                                             ...
                                             const DoutPrefixProvider* dpp)
{
    return be_handler->call(
        [&](RGWSI_MetaBackend_Handler::Op* op) { return do_remove(op, entry, objv_tracker, y, dpp); });
}

int RGWMetadataHandler_GenericMetaBE::mutate(const string& entry, const ceph::real_time& mtime,
                                             ...
                                             std::function<int()> f)
{
    return be_handler->call([&](RGWSI_MetaBackend_Handler::Op* op) {
        ...
        return op->mutate(entry, params, objv_tracker, y, f, dpp);
    });
}

int RGWMetadataHandler_GenericMetaBE::get_shard_id(const string& entry, int* shard_id)
{
    return be_handler->call([&](RGWSI_MetaBackend_Handler::Op* op) { return op->get_shard_id(entry, shard_id); });
}

int RGWMetadataHandler_GenericMetaBE::list_keys_init(const DoutPrefixProvider* dpp, const string& marker,
                                                     void** phandle)
{
...
int RGWMetadataManager::register_handler(RGWMetadataHandler* handler)
{
    ...
    if (handlers.find(type) != handlers.end()) return -EEXIST;
...
int RGWMetadataManager::find_handler(const string& metadata_key, RGWMetadataHandler** handler, string& entry)
{
    ...
    map<string, RGWMetadataHandler*>::iterator iter = handlers.find(type);
    if (iter == handlers.end()) return -ENOENT;
...
int RGWMetadataManager::put(string& metadata_key, bufferlist& bl, optional_yield y, const DoutPrefixProvider* dpp,
                            RGWMDLogSyncType sync_type, bool from_remote_zone, obj_version* existing_version)
{
    RGWMetadataHandler* handler;
    string entry;
...
int RGWMetadataManager::mutate(const string& metadata_key, const ceph::real_time& mtime,
                               RGWObjVersionTracker* objv_tracker, optional_yield y, const DoutPrefixProvider* dpp,
                               RGWMDLogStatus op_type, std::function<int()> f)
{
    RGWMetadataHandler* handler;
    string entry;
...
int RGWMetadataManager::list_keys_init(const DoutPrefixProvider* dpp, const string& section, const string& marker,
                                       void** handle)
{
    string entry;
    RGWMetadataHandler* handler;
...
void RGWMetadataManager::dump_log_entry(cls_log_entry& entry, Formatter* f)
{
    ...
    try {
        ...
        lderr(cct) << "failed to decode log entry: " << entry.section << ":" << entry.name << " ts=" << entry.timestamp
                   << dendl;
    }
    f->close_section();
}
