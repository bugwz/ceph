...
public:
    WaitContext(RGWCompletionManager* _cm, void* _opaque)
        : manager(_cm)
        , opaque(_opaque)
    {}
    void finish(int r) override { manager->_wakeup(opaque); }
};

RGWCompletionManager::RGWCompletionManager(CephContext* _cct)
    : cct(_cct)
    , timer(cct, lock)
{
    timer.init();
...
RGWCoroutinesStack::RGWCoroutinesStack(CephContext* _cct, RGWCoroutinesManager* _ops_mgr, RGWCoroutine* start)
    : cct(_cct)
    , ops_mgr(_ops_mgr)
    , done_flag(false)
    , error_flag(false)
    , blocked_flag(false)
    , sleep_flag(false)
    , interval_wait_flag(false)
    , is_scheduled(false)
    , is_waiting_for_child(false)
    , retcode(0)
    , run_count(0)
    , env(NULL)
    , parent(NULL)
...
bool RGWCoroutinesStack::collect(RGWCoroutine* op, int* ret, RGWCoroutinesStack* skip_stack,
    ...
    *ret = 0;
    ...
    for (vector<RGWCoroutinesStack*>::iterator iter = s->entries.begin(); iter != s->entries.end(); ++iter) {
        ...
        if (stack == skip_stack || !stack->is_done()) {
            ...
            if (!stack->is_done()) {
                ldout(cct, 20) << "collect(): s=" << (void*)this << " stack=" << (void*)stack << " is still running"
                               << dendl;
            }
            else if (stack == skip_stack) {
                ldout(cct, 20) << "collect(): s=" << (void*)this << " stack=" << (void*)stack
...
bool RGWCoroutinesStack::collect_next(
    RGWCoroutine* op, int* ret, RGWCoroutinesStack** collected_stack) /* returns true if found a stack to collect */
{
    rgw_spawned_stacks* s = (op ? &op->spawned : &spawned);
    *ret = 0;
...
bool RGWCoroutinesStack::collect(int* ret, RGWCoroutinesStack* skip_stack,
                                 uint64_t* stack_id) /* returns true if needs to be called again */
{
...
RGWAioCompletionNotifier::RGWAioCompletionNotifier(RGWCompletionManager* _mgr, const rgw_io_id& _io_id,
                                                   void* _user_data)
    : completion_mgr(_mgr)
    , io_id(_io_id)
...
void RGWCoroutinesManager::handle_unblocked_stack(set<RGWCoroutinesStack*>& context_stacks,
...
int RGWCoroutinesManager::run(const DoutPrefixProvider* dpp, list<RGWCoroutinesStack*>& stacks)
{
    ...
    for (list<RGWCoroutinesStack*>::iterator iter = scheduled_stacks.begin();
         iter != scheduled_stacks.end() && !going_down;) {
        ...
        if (stack->is_io_blocked()) {
            ldout(cct, 20) << __func__ << ":"
                           << " stack=" << (void*)stack << " is io blocked" << dendl;
             ...
             */
            ldout(cct, 20) << __func__ << ":"
                           << " stack=" << (void*)stack << " is_blocked_by_stack()=" << stack->is_blocked_by_stack()
                           << " is_sleeping=" << stack->is_sleeping()
                           << " waiting_for_child()=" << stack->waiting_for_child() << dendl;
        }
        else if (stack->is_done()) {
            ldout(cct, 20) << __func__ << ":"
                           << " stack=" << (void*)stack << " is done" << dendl;
    ...
    for (auto stack : context_stacks) {
        ldout(cct, 20) << "clearing stack on run() exit: stack=" << (void*)stack << " nref=" << stack->get_nref()
                       << dendl;
        stack->cancel();
    }
...
int RGWCoroutinesManagerRegistry::hook_to_admin_command(const string& command)
{
    ...
    int r = admin_socket->register_command(admin_command, this, "dump current coroutines stack state");
    if (r < 0) {
...
int RGWCoroutinesManagerRegistry::call(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, std::ostream& ss,
                                       bufferlist& out)
{
    std::shared_lock rl{lock};
...
bool RGWCoroutine::collect(int* ret, RGWCoroutinesStack* skip_stack,
                           uint64_t* stack_id) /* returns true if needs to be called again */
{
    return stack->collect(this, ret, skip_stack, stack_id);
}

bool RGWCoroutine::collect_next(int* ret,
...
bool RGWCoroutine::drain_children(int num_cr_left, std::optional<std::function<int(uint64_t stack_id, int ret)> > cb)
{
    bool done = false;
    ceph_assert(num_cr_left >= 0);
