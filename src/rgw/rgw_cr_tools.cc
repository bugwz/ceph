...
template<> int RGWUserCreateCR::Request::_send_request(const DoutPrefixProvider* dpp)
{
    CephContext* cct = store->ctx();

    const int32_t default_max_buckets = cct->_conf.get_val<int64_t>("rgw_user_max_buckets");
...
template<> int RGWGetUserInfoCR::Request::_send_request(const DoutPrefixProvider* dpp)
{
    return store->ctl()->user->get_info_by_uid(dpp, params.user, result.get(), null_yield);
}

template<> int RGWGetBucketInfoCR::Request::_send_request(const DoutPrefixProvider* dpp)
{
    return store->getRados()->get_bucket_info(store->svc(),
                                              params.tenant,
                                              params.bucket_name,
                                              result->bucket_info,
                                              &result->mtime,
...
template<> int RGWBucketCreateLocalCR::Request::_send_request(const DoutPrefixProvider* dpp)
{
    CephContext* cct = store->ctx();
    ...
    if (!placement_rule.empty() && !zone_svc->get_zone_params().valid_placement(placement_rule)) {
    ...
    int ret = store->getRados()->get_bucket_info(
        store->svc(), user.tenant, bucket_name, bucket_info, nullptr, null_yield, dpp, &bucket_attrs);
    ...
    if (bucket_exists) {
        ret = rgw_op_get_bucket_policy_from_attr(dpp, cct, store, bucket_info, bucket_attrs, &old_policy, null_yield);
        if (ret >= 0) {
    ...
    if (bucket_exists) {
        ...
        ret = zone_svc->select_bucket_placement(
            dpp, *user_info, zonegroup_id, placement_rule, &selected_placement_rule, nullptr, null_yield);
    ...
    ret = store->getRados()->create_bucket(*user_info,
                                           bucket,
                                           zonegroup_id,
                                           placement_rule,
    ...
    if (existed) {
        if (info.owner != user) {
            ldpp_dout(dpp, 20) << "NOTICE: bucket already exists under a different user (bucket=" << bucket
                               << " user=" << user << " bucket_owner=" << info.owner << dendl;
            return -EEXIST;
        }
        bucket = info.bucket;
    }
...
template<> int RGWObjectSimplePutCR::Request::_send_request(const DoutPrefixProvider* dpp)
{
...
template<> int RGWBucketLifecycleConfigCR::Request::_send_request(const DoutPrefixProvider* dpp)
{
    CephContext* cct = store->ctx();
    ...
    int ret = lc->set_bucket_config(params.bucket_info, params.bucket_attrs, &params.config);
    if (ret < 0) {
...
template<> int RGWBucketGetSyncPolicyHandlerCR::Request::_send_request(const DoutPrefixProvider* dpp)
{
