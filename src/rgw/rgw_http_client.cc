...
struct rgw_http_req_data : public RefCountedObject
{
    ...
    void finish(int r, long http_status = -1)
    {
        ...
        ret = r;
        if (curl_handle) do_curl_easy_cleanup(curl_handle);
...
struct RGWCurlHandle
{
    ...
    explicit RGWCurlHandle(CURL* h)
        : uses(0)
        , h(h){};
    CURL* operator*() { return this->h; }
};
...
void* RGWCurlHandles::entry()
{
    ...
    for (;;) {
        ...
        while (!saved_curl.empty()) {
            ...
            --cend;
            curl = *cend;
            if (!cleaner_shutdown && now - curl->lastuse < std::chrono::seconds(MAXIDLE)) break;
 ...
 */
size_t RGWHTTPClient::receive_http_header(void* const ptr, const size_t size, const size_t nmemb, void* const _info)
{
...
size_t RGWHTTPClient::receive_http_data(void* const ptr, const size_t size, const size_t nmemb, void* const _info)
{
    rgw_http_req_data* req_data = static_cast<rgw_http_req_data*>(_info);
    size_t len = size * nmemb;
...
size_t RGWHTTPClient::send_http_data(void* const ptr, const size_t size, const size_t nmemb, void* const _info)
{
    rgw_http_req_data* req_data = static_cast<rgw_http_req_data*>(_info);
    ...
    if (!verify_ssl) {
        ...
        if (!client_cert.empty()) {
            if (!client_key.empty()) {
                ...
                dout(20) << "using custom client cert " << client_cert.c_str() << " and private key "
                         << client_key.c_str() << dendl;
            }
            else {
...
int RGWHTTPHeadersCollector::receive_header(void* const ptr, const size_t len)
{
    ...
    if (std::string_view::npos == val_loc_s || std::string_view::npos == val_loc_e) {
        /* Empty value case. */
        found_headers.emplace(name, header_value_t());
    }
    else {
        found_headers.emplace(name, header_value_t(value_part.substr(val_loc_s, val_loc_e - val_loc_s)));
    }
...
static int detect_curl_multi_wait_bug(CephContext* cct, CURLM* handle, int write_fd, int read_fd)
{
    int ret = 0;
 ...
 */
RGWHTTPManager::RGWHTTPManager(CephContext* _cct, RGWCompletionManager* _cm)
    : cct(_cct)
    , completion_mgr(_cm)
{
...
{
    stop();
    if (multi_handle) curl_multi_cleanup((CURLM*)multi_handle);
}
...
void RGWHTTPManager::register_request(rgw_http_req_data* req_data)
{
    ...
    num_reqs++;
    ldout(cct, 20) << __func__ << " mgr=" << this << " req_data->id=" << req_data->id
                   << ", curl_handle=" << req_data->curl_handle << dendl;
}
...
{
    ldout(cct, 20) << __func__ << " req_data=" << req_data << " req_data->id=" << req_data->id
                   << ", curl_handle=" << req_data->curl_handle << dendl;
...
int RGWHTTPManager::set_request_state(RGWHTTPClient* client, RGWHTTPRequestSetState state)
{
    ...
    switch (state) {
    case SET_WRITE_PAUSED: suggested_wr_paused = true; break;
    case SET_WRITE_RESUME: suggested_wr_paused = false; break;
    case SET_READ_PAUSED: suggested_rd_paused = true; break;
    case SET_READ_RESUME: suggested_rd_paused = false; break;
    default:
        /* shouldn't really be here */
        return -EIO;
    }
    if (suggested_wr_paused == req_data->write_paused && suggested_rd_paused == req_data->read_paused) {
        return 0;
    }
...
void* RGWHTTPManager::reqs_thread_entry()
{
    ...
    while (!going_down) {
        ...
        switch (mstatus) {
        case CURLM_OK:
        case CURLM_CALL_MULTI_PERFORM: break;
        default: dout(10) << "curl_multi_perform returned: " << mstatus << dendl; break;
        }
        int msgs_left;
        CURLMsg* msg;
            ...
            if (msg->msg == CURLMSG_DONE) {
                ...
                if (!req_data->user_ret) {
                    ...
                    if (result != CURLE_OK && status == 0) {
                        dout(0) << "ERROR: curl error: " << curl_easy_strerror((CURLcode)result)
                ...
                case CURLE_OPERATION_TIMEDOUT:
                    dout(0) << "WARNING: curl operation timed out, network average transfer speed less than "
                            << cct->_conf->rgw_curl_low_speed_limit << " Bytes per second during "
                            << cct->_conf->rgw_curl_low_speed_time << " seconds." << dendl;
                default:
                    dout(20) << "ERROR: msg->data.result=" << result << " req_data->id=" << id
                             << " http_status=" << http_status << dendl;
                    dout(20) << "ERROR: curl error: " << curl_easy_strerror((CURLcode)result)
                             << " req_data->error_buf=" << req_data->error_buf << dendl;
                    break;
                }
            }
        }
