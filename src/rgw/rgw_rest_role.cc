...
int RGWRestRole::verify_permission(optional_yield y)
{
    ...
    std::unique_ptr<rgw::sal::RGWRole> role = driver->get_role(role_name, s->user->get_tenant());
    ...
    uint64_t op = get_op();
    if (!verify_user_permission(this, s, rgw::ARN(resource_name, "role", s->user->get_tenant(), true), op)) {
        return -EACCES;
    }
...
int RGWRestRole::parse_tags()
{
    ...
    for (auto& v : val_map) {
        ...
        for (sregex_iterator it = sregex_iterator(v.first.begin(), v.first.end(), pattern_key); it != sregex_iterator();
             it++) {
            smatch match;
            match = *it;
        ...
        for (sregex_iterator it = sregex_iterator(v.first.begin(), v.first.end(), pattern_value);
             it != sregex_iterator();
             it++) {
            smatch match;
            match = *it;
...
int RGWCreateRole::verify_permission(optional_yield y)
{
    ...
    if (!verify_user_permission(this, s, rgw::ARN(resource_name, "role", s->user->get_tenant(), true), get_op())) {
        return -EACCES;
    }
    return 0;
}
...
int RGWCreateRole::get_params()
{
    ...
    if (role_name.empty() || trust_policy.empty()) {
        ldpp_dout(this, 20) << "ERROR: one of role name or assume role policy document is empty" << dendl;
        return -EINVAL;
    }
...
void RGWCreateRole::execute(optional_yield y)
{
    ...
    std::unique_ptr<rgw::sal::RGWRole> role =
        driver->get_role(role_name, user_tenant, role_path, trust_policy, max_session_duration, tags);
    if (!user_tenant.empty() && role->get_tenant() != user_tenant) {
        ldpp_dout(this, 20) << "ERROR: the tenant provided in the role name does not match with the tenant of the user "
                               "creating the role"
                            << dendl;
        op_ret = -EINVAL;
        return;
...
int RGWListRoles::verify_permission(optional_yield y)
{
    ...
    if (!verify_user_permission(this, s, rgw::ARN(), get_op())) {
        return -EACCES;
    }
