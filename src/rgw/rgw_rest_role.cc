...
void RGWCreateRole::execute(optional_yield y)
{
    ...
    RGWRole role(
        s->cct, store->getRados()->pctl, role_name, role_path, trust_policy, user_tenant, max_session_duration, tags);
...
void RGWListRoles::execute(optional_yield y)
{
    ...
    op_ret =
        RGWRole::get_roles_by_path_prefix(s, store->getRados(), s->cct, path_prefix, s->user->get_tenant(), result, y);
