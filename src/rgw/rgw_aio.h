...
template<typename T, typename... Args> struct OwningList : boost::intrusive::list<T, Args...>
{
    OwningList() = default;
...
public:
    ...
    virtual AioResultList get(const RGWSI_RADOS::Obj& obj, OpFunc&& f, uint64_t cost, uint64_t id) = 0;
    ...
    static OpFunc librados_op(librados::ObjectReadOperation&& op, optional_yield y);
    static OpFunc librados_op(librados::ObjectWriteOperation&& op, optional_yield y);
    static OpFunc d3n_cache_op(const DoutPrefixProvider* dpp, optional_yield y, off_t read_ofs, off_t read_len,
                               std::string& location);
};
