...
public:
    void set_arn(std::string _arn) { arn = _arn; }
    void set_url(std::string _provider_url) { provider_url = _provider_url; }
    void set_tenant(std::string _tenant) { tenant = _tenant; }
    void set_client_ids(std::vector<std::string>& _client_ids) { client_ids = std::move(_client_ids); }
    void set_thumbprints(std::vector<std::string>& _thumbprints) { thumbprints = std::move(_thumbprints); }

    RGWOIDCProvider(std::string provider_url, std::string tenant, std::vector<std::string> client_ids,
                    std::vector<std::string> thumbprints)
    ...
    RGWOIDCProvider(std::string arn, std::string tenant)
        : arn(std::move(arn))
        , tenant(std::move(tenant))
    {}
