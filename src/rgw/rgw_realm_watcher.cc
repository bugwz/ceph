    ...
    : cct(cct)
{
    ...
    if (r < 0) {
        ldpp_dout(dpp, -1) << "Failed to establish a watch on RGWRealm, "
                              "disabling dynamic reconfiguration."
                           << dendl;
        return;
    }
}
...
void RGWRealmWatcher::handle_notify(uint64_t notify_id, uint64_t cookie, uint64_t notifier_id, bufferlist& bl)
{
    if (cookie != watch_handle) return;
    ...
    try {
        ...
        while (!p.end()) {
            ...
            if (watcher == watchers.end()) {
                lderr(cct) << "Failed to find a watcher for notify type " << static_cast<int>(notify) << dendl;
                break;
            }
...
int RGWRealmWatcher::watch_start(const DoutPrefixProvider* dpp, const RGWRealm& realm)
{
    ...
    if (r < 0) {
        ldpp_dout(dpp, -1) << "Rados client initialization failed with " << cpp_strerror(-r) << dendl;
        return r;
    }
    r = rados.connect();
    if (r < 0) {
        ldpp_dout(dpp, -1) << "Rados client connection failed with " << cpp_strerror(-r) << dendl;
        return r;
    }
    ...
    if (r < 0) {
        ldpp_dout(dpp, -1) << "Failed to open pool " << pool << " with " << cpp_strerror(-r) << dendl;
        rados.shutdown();
        return r;
    }
    ...
    if (r < 0) {
        ldpp_dout(dpp, -1) << "Failed to watch " << oid << " with " << cpp_strerror(-r) << dendl;
        pool_ctx.close();
        rados.shutdown();
        return r;
    }
...
int RGWRealmWatcher::watch_restart()
{
    ...
    if (r < 0) {
        lderr(cct) << "Failed to unwatch on " << watch_oid << " with " << cpp_strerror(-r) << dendl;
    }
    r = pool_ctx.watch2(watch_oid, &watch_handle, this);
    if (r < 0) {
        lderr(cct) << "Failed to restart watch on " << watch_oid << " with " << cpp_strerror(-r) << dendl;
        pool_ctx.close();
        watch_oid.clear();
    }
    return r;
