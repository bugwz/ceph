...
int rgw_compression_info_from_attr(const bufferlist& attr, bool& need_decompress, RGWCompressionInfo& cs_info)
{
...
int rgw_compression_info_from_attrset(const map<string, bufferlist>& attrs, bool& need_decompress,
    ...
    if (in.length() > 0) {
        ...
        if ((logical_offset > 0 && compressed) ||   // if previous part was compressed
            ...
            if (cr < 0) {
                ...
                compressed = false;
                ldout(cct, 5) << "Compression failed with exit code " << cr << " for first part, storing uncompressed"
                              << dendl;
                out = std::move(in);
            }
            else {
...
RGWGetObj_Decompress::RGWGetObj_Decompress(CephContext* cct_, RGWCompressionInfo* cs_info_, bool partial_content_,
    ...
    , cur_ofs(0)
{
    ...
    if (!compressor.get()) lderr(cct) << "Cannot load compressor of type " << cs_info->compression_type << dendl;
}
...
int RGWGetObj_Decompress::handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len)
{
    ...
    while (first_block <= last_block) {
        ...
        ++first_block;
        while (out_bl.length() - q_ofs >= static_cast<off_t>(cct->_conf->rgw_max_chunk_size)) {
...
int RGWGetObj_Decompress::fixup_range(off_t& ofs, off_t& end)
{
    if (partial_content) {
        ...
        first_block = cs_info->blocks.begin();
        last_block = cs_info->blocks.begin();
        if (cs_info->blocks.size() > 1) {
            ...
            fb = upper_bound(cs_info->blocks.begin() + 1, cs_info->blocks.end(), ofs, cmp_u);
            first_block = fb - 1;
            lb = lower_bound(fb, cs_info->blocks.end(), end, cmp_l);
            last_block = lb - 1;
        }
    }
    else {
        first_block = cs_info->blocks.begin();
        last_block = cs_info->blocks.end() - 1;
    }
