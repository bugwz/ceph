...
{
public:
    TempURLApplier(CephContext* const cct, const RGWUserInfo& user_info)
    ...
    {
        virtual ~Factory() {}
        virtual aplptr_t create_apl_turl(CephContext* cct, const req_state* s, const RGWUserInfo& user_info) const = 0;
    };
};
...
class TempURLEngine : public rgw::auth::Engine
{
    ...
    /* Helper methods. */
    void get_owner_info(const DoutPrefixProvider* dpp, const req_state* s, RGWUserInfo& owner_info,
...
public:
    TempURLEngine(CephContext* const cct, rgw::sal::Driver* _driver, const TempURLApplier::Factory* const apl_factory)
        : cct(cct)
        , driver(_driver)
    ...
    const char* get_name() const noexcept override { return "rgw::auth::swift::TempURLEngine"; }
...
class SignedTokenEngine : public rgw::auth::Engine
{
    ...
    result_t authenticate(const DoutPrefixProvider* dpp, const std::string& token, const req_state* s) const;

public:
    SignedTokenEngine(CephContext* const cct, rgw::sal::Driver* _driver,
                      const rgw::auth::TokenExtractor* const extractor,
                      const rgw::auth::LocalApplier::Factory* const apl_factory)
        : cct(cct)
        , driver(_driver)
    ...
    const char* get_name() const noexcept override { return "rgw::auth::swift::SignedTokenEngine"; }
...
class ExternalTokenEngine : public rgw::auth::Engine
{
    ...
    bool is_applicable(const std::string& token) const noexcept;
    result_t authenticate(const DoutPrefixProvider* dpp, const std::string& token, const req_state* s,
                          optional_yield y) const;

public:
    ExternalTokenEngine(CephContext* const cct, rgw::sal::Driver* _driver,
                        const rgw::auth::TokenExtractor* const extractor,
                        const rgw::auth::LocalApplier::Factory* const apl_factory)
        : cct(cct)
        , driver(_driver)
    ...
    const char* get_name() const noexcept override { return "rgw::auth::swift::ExternalTokenEngine"; }
...
{
public:
    SwiftAnonymousApplier(CephContext* const cct, const RGWUserInfo& user_info)
        : LocalApplier(cct, user_info, LocalApplier::NO_SUBUSER, std::nullopt, LocalApplier::NO_ACCESS_KEY)
    {}
...
class SwiftAnonymousEngine : public rgw::auth::AnonymousEngine
{
    ...
    bool is_applicable(const req_state* s) const noexcept override { return extractor->get_token(s).empty(); }

public:
    SwiftAnonymousEngine(CephContext* const cct, const SwiftAnonymousApplier::Factory* const apl_factory,
                         const rgw::auth::TokenExtractor* const extractor)
        : AnonymousEngine(cct, apl_factory)
        , extractor(extractor)
    {}

    const char* get_name() const noexcept override { return "rgw::auth::swift::SwiftAnonymousEngine"; }
};
                        ...
                        public rgw::auth::swift::TempURLApplier::Factory
{
    ...
    aplptr_t create_apl_remote(CephContext* const cct, const req_state* const s, acl_strategy_t&& extra_acl_strategy,
                               const rgw::auth::RemoteApplier::AuthInfo& info) const override
    {
        auto apl = rgw::auth::add_3rdparty(
            driver,
            rgw_user(s->account_name),
    ...
    aplptr_t create_apl_turl(CephContext* const cct, const req_state* const s,
...
public:
    DefaultStrategy(CephContext* const cct, const ImplicitTenants& implicit_tenant_context, rgw::sal::Driver* _driver)
        : driver(_driver)
        , implicit_tenant_context(implicit_tenant_context)
        , tempurl_engine(cct, driver, static_cast<rgw::auth::swift::TempURLApplier::Factory*>(this))
        , signed_engine(cct, driver, static_cast<rgw::auth::TokenExtractor*>(&auth_token_extractor),
                        static_cast<rgw::auth::LocalApplier::Factory*>(this))
        , external_engine(cct, driver, static_cast<rgw::auth::TokenExtractor*>(&auth_token_extractor),
                          static_cast<rgw::auth::LocalApplier::Factory*>(this))
        , anon_engine(cct, static_cast<SwiftAnonymousApplier::Factory*>(this),
    ...
    const char* get_name() const noexcept override { return "rgw::auth::swift::DefaultStrategy"; }
};
...
public:
    ...
    RGWRESTMgr* get_resource_mgr(req_state* const s, const std::string& uri, std::string* const out_uri) override
    {
        return this;
    }
