...
int rgw_process_authenticated(RGWHandler_REST* const handler, RGWOp*& op, RGWRequest* const req, req_state* const s,
                              optional_yield y, const bool skip_retarget)
{
...
int process_request(rgw::sal::RGWRadosStore* const store, RGWREST* const rest, RGWRequest* const req,
                    ...
                    RGWRestfulIO* const client_io, OpsLogSink* const olog, optional_yield yield,
                    rgw::dmclock::Scheduler* scheduler, string* user, ceph::coarse_real_clock::duration* latency,
                    int* http_ret)
{
    ...
    RGWHandler_REST* handler =
        rest->get_handler(store, s, auth_registry, frontend_prefix, client_io, &mgr, &init_error);
