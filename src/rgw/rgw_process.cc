...
auto schedule_request(Scheduler* scheduler, req_state* s, RGWOp* op)
{
    ...
    if (!scheduler) return std::make_pair(0, SchedulerCompleter{});

    const auto client = op->dmclock_client();
    const auto cost = op->dmclock_cost();
    if (s->cct->_conf->subsys.should_gather(ceph_subsys_rgw, 10)) {
        ldpp_dout(op, 10) << "scheduling with " << s->cct->_conf.get_val<std::string>("rgw_scheduler_type")
                          << " client=" << static_cast<int>(client) << " cost=" << cost << dendl;
    }
    return scheduler->schedule_request(client, {}, req_state::Clock::to_double(s->time), cost, s->yield);
}
...
{
    if (process->m_req_queue.empty()) return NULL;
...
bool rate_limit(rgw::sal::Driver* driver, req_state* s)
{
    ...
    if ((s->op_type == RGW_OP_GET_HEALTH_CHECK) || is_admin_or_system.admin || is_admin_or_system.system) return false;
    std::string userfind;
    ...
    if (iter != s->user->get_attrs().end()) {
        try {
            ...
            if (user_ratelimit_temp.enabled) *user_ratelimit = user_ratelimit_temp;
        }
    ...
    if (!rgw::sal::Bucket::empty(s->bucket.get())) {
        ...
        if (iter != s->bucket->get_attrs().end()) {
            try {
                ...
                // override global rate limiting only if local rate limiting is enabled
                if (bucket_ratelimit_temp.enabled) *bucket_ratelimit = bucket_ratelimit_temp;
            }
            catch (buffer::error& err) {
                ldpp_dout(s, 0) << "ERROR: failed to decode rate limit" << dendl;
                return -EIO;
        ...
        if (!limit_user) {
            limit_bucket =
                s->ratelimit_data->should_rate_limit(method, s->ratelimit_bucket_marker, s->time, bucket_ratelimit);
        }
    }
...
int rgw_process_authenticated(RGWHandler_REST* const handler, RGWOp*& op, RGWRequest* const req, req_state* const s,
                              optional_yield y, rgw::sal::Driver* driver, const bool skip_retarget)
{
...
int process_request(const RGWProcessEnv& penv, RGWRequest* const req, const std::string& frontend_prefix,
                    RGWRestfulIO* const client_io, optional_yield yield, rgw::dmclock::Scheduler* scheduler,
                    string* user, ceph::coarse_real_clock::duration* latency, int* http_ret)
{
    ...
    RGWHandler_REST* handler =
        rest->get_handler(driver, s, *penv.auth_registry, frontend_prefix, client_io, &mgr, &init_error);
    ...
    }
    {
        ...
        std::string script;
        auto rc = rgw::lua::read_script(
            s, penv.lua.manager.get(), s->bucket_tenant, s->yield, rgw::lua::context::preRequest, script);
        if (rc == -ENOENT) {
    ...
    if (op) {
        ...
        std::string script;
        auto rc = rgw::lua::read_script(
            s, penv.lua.manager.get(), s->bucket_tenant, s->yield, rgw::lua::context::postRequest, script);
        if (rc == -ENOENT) {
            // no script, nothing to do
        }
        else if (rc < 0) {
    ...
    dout(1) << "====== req done req=" << hex << req << dec << " op status=" << op_ret
            << " http_status=" << s->err.http_ret << " latency=" << lat << " ======" << dendl;
