...
struct connection_t
{
    ...
    bool is_ok() const { return (producer != nullptr && !marked_for_deletion); }
    ...
    ~connection_t() { destroy(STATUS_CONNECTION_CLOSED); }
    ...
    if (conn->use_ssl) {
        if (!conn->user.empty()) {
            ...
            if (rd_kafka_conf_set(conn->temp_conf, "security.protocol", "SASL_SSL", errstr, sizeof(errstr)) !=
                    RD_KAFKA_CONF_OK ||
                rd_kafka_conf_set(conn->temp_conf, "sasl.mechanism", "PLAIN", errstr, sizeof(errstr)) !=
                    RD_KAFKA_CONF_OK ||
...
connection_ptr_t create_new_connection(const std::string& broker, CephContext* cct, bool use_ssl, bool verify_ssl,
                                       boost::optional<const std::string&> ca_location, const std::string& user,
                                       const std::string& password)
{
...
private:
    ...
    void publish_internal(message_wrapper_t* message)
    {
        ...
        if (!conn->is_ok()) {
            ...
            ldout(conn->cct, 1) << "Kafka publish: connection had an issue while message was in the queue. error: "
