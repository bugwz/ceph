...
struct connection_t
{
    ...
    void destroy(int s)
    {
        ...
        std::for_each(callbacks.begin(), callbacks.end(), [this](auto& cb_tag) {
            cb_tag.cb(status);
            ldout(cct, 20) << "Kafka destroy: invoking callback with tag=" << cb_tag.tag << " for: " << broker << dendl;
        });
        callbacks.clear();
        delivery_tag = 1;
    ...
    bool is_ok() const { return (producer != nullptr); }

    // ctor for setting immutable values
    connection_t(CephContext* _cct, const std::string& _broker, bool _use_ssl, bool _verify_ssl,
                 const boost::optional<const std::string&>& _ca_location, const std::string& _user,
                 const std::string& _password, const boost::optional<const std::string&>& _mechanism)
        : cct(_cct)
        , broker(_broker)
        , use_ssl(_use_ssl)
...
std::string status_to_string(int s)
{
    ...
    case STATUS_CONNECTION_CLOSED: return "RGW_KAFKA_STATUS_CONNECTION_CLOSED";
    case STATUS_QUEUE_FULL: return "RGW_KAFKA_STATUS_QUEUE_FULL";
    case STATUS_MAX_INFLIGHT: return "RGW_KAFKA_STATUS_MAX_INFLIGHT";
    case STATUS_MANAGER_STOPPED: return "RGW_KAFKA_STATUS_MANAGER_STOPPED";
    case STATUS_CONF_ALLOC_FAILED: return "RGW_KAFKA_STATUS_CONF_ALLOC_FAILED";
    case STATUS_CONF_REPLCACE: return "RGW_KAFKA_STATUS_CONF_REPLCACE";
    case STATUS_CONNECTION_IDLE: return "RGW_KAFKA_STATUS_CONNECTION_IDLE";
    }
...
void message_callback(rd_kafka_t* rk, const rd_kafka_message_t* rkmessage, void* opaque)
{
    ...
    if (tag_it != callbacks_end) {
        ldout(conn->cct, 20) << "Kafka run: n/ack received, invoking callback with tag=" << *tag
                             << " and result=" << rd_kafka_err2str(result) << dendl;
        tag_it->cb(result);
...
bool new_producer(connection_t* conn)
{
    ...
    if (rd_kafka_conf_set(conn->temp_conf, "bootstrap.servers", conn->broker.c_str(), errstr, sizeof(errstr)) !=
        RD_KAFKA_CONF_OK)
        goto conf_error;
    ...
    if (conn->use_ssl) {
        if (!conn->user.empty()) {
            // use SSL+SASL
            if (rd_kafka_conf_set(conn->temp_conf, "security.protocol", "SASL_SSL", errstr, sizeof(errstr)) !=
                    RD_KAFKA_CONF_OK ||
                rd_kafka_conf_set(conn->temp_conf, "sasl.username", conn->user.c_str(), errstr, sizeof(errstr)) !=
                    RD_KAFKA_CONF_OK ||
                rd_kafka_conf_set(conn->temp_conf, "sasl.password", conn->password.c_str(), errstr, sizeof(errstr)) !=
                    RD_KAFKA_CONF_OK)
                goto conf_error;
            ...
            if (conn->mechanism) {
                if (rd_kafka_conf_set(
                        conn->temp_conf, "sasl.mechanism", conn->mechanism->c_str(), errstr, sizeof(errstr)) !=
                    RD_KAFKA_CONF_OK)
                    goto conf_error;
                ldout(conn->cct, 20) << "Kafka connect: successfully configured SASL mechanism" << dendl;
            }
            else {
                if (rd_kafka_conf_set(conn->temp_conf, "sasl.mechanism", "PLAIN", errstr, sizeof(errstr)) !=
                    RD_KAFKA_CONF_OK)
                    goto conf_error;
            ...
            // use only SSL
            if (rd_kafka_conf_set(conn->temp_conf, "security.protocol", "SSL", errstr, sizeof(errstr)) !=
                RD_KAFKA_CONF_OK)
                goto conf_error;
        ...
        if (conn->ca_location) {
            if (rd_kafka_conf_set(
                    conn->temp_conf, "ssl.ca.location", conn->ca_location->c_str(), errstr, sizeof(errstr)) !=
                RD_KAFKA_CONF_OK)
                goto conf_error;
        ...
        // Note: when librdkafka.1.0 is available the following line could be uncommented instead of the callback
        // setting call if (rd_kafka_conf_set(conn->temp_conf, "enable.ssl.certificate.verification", "0", errstr,
        ...
        // use SASL+PLAINTEXT
        if (rd_kafka_conf_set(conn->temp_conf, "security.protocol", "SASL_PLAINTEXT", errstr, sizeof(errstr)) !=
                RD_KAFKA_CONF_OK ||
            rd_kafka_conf_set(conn->temp_conf, "sasl.username", conn->user.c_str(), errstr, sizeof(errstr)) !=
                RD_KAFKA_CONF_OK ||
            rd_kafka_conf_set(conn->temp_conf, "sasl.password", conn->password.c_str(), errstr, sizeof(errstr)) !=
    ...
    if (conn->producer) {
        ldout(conn->cct, 5)
            << "Kafka connect: producer already exists. detroying the existing before creating a new one" << dendl;
...
struct message_wrapper_t
{
    ...
    message_wrapper_t(const std::string& _conn_name, const std::string& _topic, const std::string& _message,
...
private:
    ...
    void publish_internal(message_wrapper_t* message)
    {
        ...
        if (conn_it == connections.end()) {
            ldout(cct, 1) << "Kafka publish: connection was deleted while message was in the queue. error: "
                          << STATUS_CONNECTION_CLOSED << dendl;
            if (message->cb) {
        ...
        if (!conn->is_ok()) {
            ...
            ldout(conn->cct, 1) << "Kafka publish: producer was closed while message was in the queue. error: "
                                << status_to_string(conn->status) << dendl;
            if (message->cb) {
        ...
        if (topic_it == conn->topics.end()) {
            ...
            if (!topic) {
                ...
                ldout(conn->cct, 1) << "Kafka publish: failed to create topic: " << message->topic
                                    << " error: " << status_to_string(err) << dendl;
                if (message->cb) {
                    message->cb(err);
                }
                conn->destroy(err);
        ...
        const auto rc = rd_kafka_produce(topic,
        ...
        if (tag) {
            ...
            if (q_len < max_inflight) {
                ldout(conn->cct, 20) << "Kafka publish (with callback, tag=" << *tag << "): OK. Queue has: " << q_len
                                     << " callbacks" << dendl;
    ...
    void run() noexcept
    {
        while (!stopped) {
            ...
            const auto send_count =
                messages.consume_all(std::bind(&Manager::publish_internal, this, std::placeholders::_1));
            dequeued += send_count;
            ...
            for (; conn_it != end_it;) {
                ...
                if (conn->timestamp.sec() + max_idle_time < ceph_clock_now()) {
                    ldout(conn->cct, 20) << "kafka run: deleting a connection due to idle behaviour: "
                ...
                if (!conn->is_ok()) {
                    ldout(conn->cct, 10) << "Kafka run: connection status is: " << status_to_string(conn->status)
                                         << dendl;
...
public:
    Manager(size_t _max_connections, size_t _max_inflight, size_t _max_queue, int _read_timeout_ms, CephContext* _cct)
                 ...
                 boost::optional<const std::string&> ca_location, boost::optional<const std::string&> mechanism)
    {
        ...
        auto conn = connections
                        .emplace(broker,
                                 std::make_unique<connection_t>(
                                     cct, broker, use_ssl, verify_ssl, ca_location, user, password, mechanism))
                        .first->second.get();
        if (!new_producer(conn)) {
            ldout(cct, 10) << "Kafka connect: new connection is created. But producer creation failed. will retry"
                           << dendl;
        }
        return true;
    }
...
bool init(CephContext* cct)
{
    ...
    s_manager =
        new Manager(MAX_CONNECTIONS_DEFAULT, MAX_INFLIGHT_DEFAULT, MAX_QUEUE_DEFAULT, READ_TIMEOUT_MS_DEFAULT, cct);
    return true;
}
