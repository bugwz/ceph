...
int Service::get_admin_token(CephContext* const cct, TokenCache& token_cache, const Config& config, std::string& token)
{
...
int Service::issue_admin_token_request(CephContext* const cct, const Config& config, TokenEnvelope& t)
{
    ...
    if (t.parse(cct, token_req.get_subject_token(), token_bl, keystone_version) != 0) {
        return -EINVAL;
    }
...
int Service::get_keystone_barbican_token(CephContext* const cct, std::string& token)
{
...
int TokenEnvelope::parse(CephContext* const cct, const std::string& token_str, ceph::bufferlist& bl,
