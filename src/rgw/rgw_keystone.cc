...
ApiVersion CephCtxConfig::get_api_version() const noexcept
{
    ...
    case 3: return ApiVersion::VER_3;
    case 2: return ApiVersion::VER_2;
    default:
...
int Service::get_admin_token(const DoutPrefixProvider* dpp, CephContext* const cct, TokenCache& token_cache,
                             const Config& config, std::string& token)
{
...
int Service::issue_admin_token_request(const DoutPrefixProvider* dpp, CephContext* const cct, const Config& config,
                                       TokenEnvelope& t)
{
    ...
    if (token_req.get_http_status() == RGWGetKeystoneAdminToken::HTTP_STATUS_UNAUTHORIZED) {
        return -EACCES;
    }

    if (t.parse(dpp, cct, token_req.get_subject_token(), token_bl, keystone_version) != 0) {
        return -EINVAL;
    }
...
int Service::get_keystone_barbican_token(const DoutPrefixProvider* dpp, CephContext* const cct, std::string& token)
{
    ...
    /* Detect rejection earlier than during the token parsing step. */
    if (token_req.get_http_status() == RGWKeystoneHTTPTransceiver::HTTP_STATUS_UNAUTHORIZED) {
        return -EACCES;
    }
...
int TokenEnvelope::parse(const DoutPrefixProvider* dpp, CephContext* const cct, const std::string& token_str,
                         ceph::bufferlist& bl, const ApiVersion version)
{
    JSONParser parser;
...
bool TokenCache::find(const std::string& token_id, rgw::keystone::TokenEnvelope& token)
{
    std::lock_guard l{lock};
    return find_locked(token_id, token, tokens, tokens_lru);
}

bool TokenCache::find_service(const std::string& token_id, rgw::keystone::TokenEnvelope& token)
{
    std::lock_guard l{lock};
...
void TokenCache::add(const std::string& token_id, const rgw::keystone::TokenEnvelope& token)
{
    std::lock_guard l{lock};
    add_locked(token_id, token, tokens, tokens_lru);
}

void TokenCache::add_service(const std::string& token_id, const rgw::keystone::TokenEnvelope& token)
{
    std::lock_guard l{lock};
