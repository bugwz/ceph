...
RGWRESTConn::RGWRESTConn(CephContext* _cct, RGWSI_Zone* zone_svc, const string& _remote_id,
                         const list<string>& remote_endpoints, HostStyle _host_style)
    : cct(_cct)
...
RGWRESTConn::RGWRESTConn(CephContext* _cct, RGWSI_Zone* zone_svc, const string& _remote_id,
                         const list<string>& remote_endpoints, RGWAccessKey _cred, HostStyle _host_style)
    : cct(_cct)
    , endpoints(remote_endpoints.begin(), remote_endpoints.end())
    , key(std::move(_cred))
    , remote_id(_remote_id)
...
public:
    explicit StreamObjData(rgw_obj& _obj)
        : obj(_obj)
    {}
};

int RGWRESTConn::put_obj_send_init(rgw::sal::RGWObject* obj, const rgw_http_param_pair* extra_params,
...
int RGWRESTConn::put_obj_async(const DoutPrefixProvider* dpp, const rgw_user& uid, rgw::sal::RGWObject* obj,
...
int RGWRESTConn::get_obj(const DoutPrefixProvider* dpp, const rgw_user& uid, req_info* info /* optional */,
                         const rgw::sal::RGWObject* obj, const real_time* mod_ptr, const real_time* unmod_ptr,
                         uint32_t mod_zone_id, uint64_t mod_pg_ver, bool prepend_metadata, bool get_op, bool rgwx_stat,
                         bool sync_manifest, bool skip_decrypt, bool send, RGWHTTPStreamRWRequest::ReceiveCB* cb,
...
int RGWRESTConn::get_obj(const DoutPrefixProvider* dpp, const rgw::sal::RGWObject* obj, const get_obj_params& in_params,
                         bool send, RGWRESTStreamRWRequest** req)
{
    string url;
    int ret = get_url(url);
    if (ret < 0) return ret;
