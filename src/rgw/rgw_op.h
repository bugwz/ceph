...
int rgw_op_get_bucket_policy_from_attr(const DoutPrefixProvider* dpp, CephContext* cct, rgw::sal::Driver* driver,
                                       RGWBucketInfo& bucket_info, std::map<std::string, bufferlist>& bucket_attrs,
...
public:
    ...
    virtual int init(rgw::sal::Driver* driver, req_state* _s, rgw::io::BasicClient* cio);

    virtual int init_permissions(RGWOp*, optional_yield y) { return 0; }
    ...
    virtual bool supports_quota() { return true; }
};
...
std::tuple<int, bufferlist> rgw_rest_read_all_input(req_state* s, const uint64_t max_len,
                                                    const bool allow_chunked = true);

template<class T> int rgw_rest_get_json_input(CephContext* cct, req_state* s, T& out, uint64_t max_len, bool* empty)
{
...
protected:
    ...
    std::tuple<int, bufferlist> read_all_input(req_state* s, const uint64_t max_len, const bool allow_chunked = true)
    {
    ...
    template<class T> int get_json_input(CephContext* cct, req_state* s, T& out, uint64_t max_len, bool* empty)
    {
...
public:
    ...
    bool generate_cors_headers(std::string& origin, std::string& method, std::string& headers, std::string& exp_headers,
                               unsigned* max_age);
...
public:
    ...
    void set_get_data(bool get_data) { this->get_data = get_data; }

    int verify_permission(optional_yield y) override;
    void pre_exec() override;
    void execute(optional_yield y) override;
    int parse_range();
    int read_user_manifest_part(rgw::sal::Bucket* bucket, const rgw_bucket_dir_entry& ent,
                                RGWAccessControlPolicy* const bucket_acl,
                                const boost::optional<rgw::IAM::Policy>& bucket_policy, const off_t start_ofs,
     ...
     */
    virtual int get_decrypt_filter(std::unique_ptr<RGWGetObj_Filter>* filter, RGWGetObj_Filter* cb,
    ...
    int get_lua_filter(std::unique_ptr<RGWGetObj_Filter>* filter, RGWGetObj_Filter* cb);
...
public:
    ...
    public:
        ...
        const std::list<fail_desc_t> get_failures() const { return failures; }

        bool verify_permission(RGWBucketInfo& binfo, std::map<std::string, bufferlist>& battrs,
...
protected:
    ...
    boost::optional<std::pair<std::string, rgw_obj_key>> parse_path(const std::string_view& path);

    std::pair<std::string, std::string> handle_upload_path(req_state* s);

    bool handle_file_verify_permission(RGWBucketInfo& binfo, const rgw_obj& obj,
                                       std::map<std::string, ceph::bufferlist>& battrs,
                                       ACLOwner& bucket_owner /* out */, optional_yield y);
    int handle_file(std::string_view path, size_t size, AlignedStreamGetter& body, optional_yield y);
...
public:
    ...
    void init(rgw::sal::Driver* const driver, req_state* const s, RGWHandler* const h) override;
    ...
    RGWOpType get_type() override { return RGW_OP_BULK_UPLOAD; }
...
protected:
    StreamGetter& get_decoratee() { return decoratee; }
...
class RGWBulkUploadOp::AlignedStreamGetter : public RGWBulkUploadOp::DecoratedStreamGetter
{
    size_t position;
    size_t length;
    size_t alignment;
...
public:
    template<typename U>
    AlignedStreamGetter(const size_t position, const size_t length, const size_t alignment, U&& decoratee)
...
protected:
    ...
    virtual uint64_t get_default_max() const { return 1000; }
...
public:
    ...
    void init(rgw::sal::Driver* driver, req_state* s, RGWHandler* h) override { RGWOp::init(driver, s, h); }
    virtual int get_params(optional_yield y) = 0;
    void send_response() override = 0;
...
public:
    RGWCreateBucket()
        : has_cors(false)
        , relaxed_region_enforcement(false)
    ...
    void init(rgw::sal::Driver* driver, req_state* s, RGWHandler* h) override
    {
        ...
        relaxed_region_enforcement = s->cct->_conf.get_val<bool>("rgw_relaxed_region_enforcement");
    }
    virtual int get_params(optional_yield y) { return 0; }
    void send_response() override = 0;
...
public:
    ...
    virtual int get_decrypt_filter(std::unique_ptr<RGWGetObj_Filter>* filter, RGWGetObj_Filter* cb,
                                   std::map<std::string, bufferlist>& attrs, bufferlist* manifest_bl)
    {
        *filter = nullptr;
        return 0;
    }
    virtual int get_encrypt_filter(std::unique_ptr<rgw::sal::DataProcessor>* filter, rgw::sal::DataProcessor* cb)
    {
        return 0;
    }

    // get lua script to run as a "put object" filter
    int get_lua_filter(std::unique_ptr<rgw::sal::DataProcessor>* filter, rgw::sal::DataProcessor* cb);
...
public:
    ...
    static bool parse_copy_location(const std::string_view& src, std::string& bucket_name, rgw_obj_key& object,
                                    req_state* s);

    void emplace_attr(std::string&& key, buffer::list&& bl) { attrs.emplace(std::move(key), std::move(bl)); }
    ...
    virtual int check_storage_class(const rgw_placement_rule& src_placement) { return 0; }
...
public:
    ...
    void execute(optional_yield) override { op_ret = 0; }

    const char* name() const override { return "get_crossdomain_policy"; }

    RGWOpType get_type() override { return RGW_OP_GET_CROSS_DOMAIN_POLICY; }
...
public:
    ...
    RGWOpType get_type() override { return RGW_OP_GET_HEALTH_CHECK; }
...
class RGWDeleteMultiObj : public RGWOp
{
     ...
     */
    void wait_flush(optional_yield y, boost::asio::deadline_timer* formatter_flush_cond,
...
public:
    ...
    virtual void send_partial_response(const rgw_obj_key& key, bool delete_marker, const std::string& marker_version_id,
                                       int ret, boost::asio::deadline_timer* formatter_flush_cond) = 0;
...
extern int rgw_build_bucket_policies(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, req_state* s,
                                     optional_yield y);
extern int rgw_build_object_policies(const DoutPrefixProvider* dpp, rgw::sal::Driver* driver, req_state* s,
                                     bool prefetch_data, optional_yield y);
extern void rgw_build_iam_environment(rgw::sal::Driver* driver, req_state* s);
extern std::vector<rgw::IAM::Policy> get_iam_user_policy_from_attr(CephContext* cct,
                                                                   std::map<std::string, bufferlist>& attrs,
                                                                   const std::string& tenant);

inline int get_system_versioning_params(req_state* s, uint64_t* olh_epoch, std::string* version_id)
{
    if (!s->system_request) {
        return 0;
    }
    ...
    if (olh_epoch) {
        ...
        if (!epoch_str.empty()) {
            ...
            *olh_epoch = strict_strtol(epoch_str.c_str(), 10, &err);
            if (!err.empty()) {
                ldpp_subdout(s, rgw, 0) << "failed to parse versioned-epoch param" << dendl;
                return -EINVAL;
            }
        }
    }
 ...
 *
 */
inline int rgw_get_request_metadata(const DoutPrefixProvider* dpp, CephContext* const cct, struct req_info& info,
                                    std::map<std::string, ceph::bufferlist>& attrs, const bool allow_empty_attrs = true)
{
    static const std::set<std::string> blocklisted_headers = {"x-amz-server-side-encryption-customer-algorithm",
...
inline void encode_delete_at_attr(boost::optional<ceph::real_time> delete_at, std::map<std::string, bufferlist>& attrs)
{
...
inline int encode_dlo_manifest_attr(const char* const dlo_manifest, std::map<std::string, bufferlist>& attrs)
{
...
public:
    ...
    void emplace_key(std::string&& key) { attrs.emplace(std::move(key), std::nullopt); }
...
public:
    ...
    void emplace_key(std::string&& key) { attrs.emplace(std::move(key), buffer::list()); }
...
public:
    ...
    int check_caps(RGWUserCaps& caps) { return caps.check_cap("admin", RGW_CAP_READ); }
    int verify_permission(optional_yield) override { return check_caps(s->user->get_info().caps); }
    void pre_exec() override;
    void execute(optional_yield y) override;
...
public:
    ...
    RGWOpType get_type() override { return RGW_OP_GET_BUCKET_POLICY; }
};
...
public:
    ...
    RGWOpType get_type() override { return RGW_OP_DELETE_BUCKET_POLICY; }
};
...
inline int parse_value_and_bound(const std::string& input, int& output, const long lower_bound, const long upper_bound,
...
int rgw_policy_from_attrset(const DoutPrefixProvider* dpp, CephContext* cct, std::map<std::string, bufferlist>& attrset,
                            RGWAccessControlPolicy* policy);
