...
int RGWOrphanStore::init(const DoutPrefixProvider* dpp)
{
    ...
    int r =
        rgw_init_ioctx(dpp, static_cast<rgw::sal::RadosStore*>(store)->getRados()->get_rados_handle(), log_pool, ioctx);
    if (r < 0) {
...
int RGWOrphanStore::store_entries(const DoutPrefixProvider* dpp, const string& oid,
...
int RGWOrphanStore::read_entries(const string& oid, const string& marker, map<string, bufferlist>* entries,
                                 bool* truncated)
{
...
int RGWOrphanSearch::init(const DoutPrefixProvider* dpp, const string& job_name, RGWOrphanSearchInfo* info,
                          bool _detailed_mode)
{
    ...
    max_list_bucket_entries = std::max(store->ctx()->_conf->rgw_list_bucket_min_readahead, MAX_LIST_OBJS_ENTRIES);
...
int RGWOrphanSearch::build_all_oids_index(const DoutPrefixProvider* dpp)
{
    ...
    int ret = rgw_init_ioctx(
        dpp, static_cast<rgw::sal::RadosStore*>(store)->getRados()->get_rados_handle(), search_info.pool, ioctx);
    if (ret < 0) {
...
int RGWOrphanSearch::handle_stat_result(const DoutPrefixProvider* dpp, map<int, list<string>>& oids,
                                        RGWRados::Object::Stat::Result& result)
{
    ...
    if (!result.manifest) { /* a very very old object, or part of a multipart upload during upload */
        ...
        if (!detailed_mode && manifest.get_obj_size() <= manifest.get_head_size()) {
...
int RGWOrphanSearch::build_linked_oids_for_bucket(const DoutPrefixProvider* dpp, const string& bucket_instance_id,
                                                  map<int, list<string>>& oids)
{
    RGWObjectCtx obj_ctx(store);
    rgw_bucket orphan_bucket;
    int shard_id;
    int ret = rgw_bucket_parse_bucket_key(store->ctx(), bucket_instance_id, &orphan_bucket, &shard_id);
    if (ret < 0) {
        ldpp_dout(dpp, 0) << __func__ << " failed to parse bucket instance: " << bucket_instance_id << " skipping"
                          << dendl;
        return ret;
    }
    ...
    if (cur_bucket->get_bucket_id() != orphan_bucket.bucket_id) {
        ldpp_dout(dpp, 0) << __func__ << ": Skipping stale bucket instance: " << orphan_bucket.name << ": "
                          << orphan_bucket.bucket_id << dendl;
        return 0;
    }

    if (cur_bucket->get_info().layout.resharding != rgw::BucketReshardState::None) {
        ldpp_dout(dpp, 0) << __func__ << ": reshard in progress. Skipping " << orphan_bucket.name << ": "
                          << orphan_bucket.bucket_id << dendl;
        return 0;
    }
    ...
    do {
        ...
        ret = list_op.list_objects(dpp, max_list_bucket_entries, &result, nullptr, &truncated, null_yield);
        if (ret < 0) {
        ...
        for (vector<rgw_bucket_dir_entry>::iterator iter = result.begin(); iter != result.end(); ++iter) {
            ...
            ldpp_dout(dpp, 20) << __func__ << ": entry.key.name=" << entry.key.name
                               << " entry.key.instance=" << entry.key.instance << dendl;

            if (!detailed_mode && entry.meta.accounted_size <= (uint64_t)store->ctx()->_conf->rgw_max_chunk_size) {
                ldpp_dout(dpp, 5) << __func__ << "skipping stat as the object " << entry.key.name << "fits in a head"
                                  << dendl;
                continue;
            }
...
int RGWOrphanSearch::build_linked_oids_index(const DoutPrefixProvider* dpp)
{
    ...
    for (; iter != buckets_instance_index.end(); ++iter) {
        ldpp_dout(dpp, 0) << "building linked oids index: " << iter->first << "/" << buckets_instance_index.size()
                          << dendl;
        bool truncated;
        ...
        do {
            ...
            if (ret < 0) {
                ldpp_dout(dpp, -1) << __func__ << ": ERROR: read_entries() oid=" << oid << " returned ret=" << ret
                                   << dendl;
                return ret;
            }
            ...
            for (map<string, bufferlist>::iterator eiter = entries.begin(); eiter != entries.end(); ++eiter) {
                ...
                if (ret < 0) {
                    ldpp_dout(dpp, -1) << __func__
                                       << ": ERROR: build_linked_oids_for_bucket() indexed entry=" << eiter->first
...
public:
    OMAPReader(librados::IoCtx& _ioctx, const string& _oid)
        : ioctx(_ioctx)
        , oid(_oid)
        , truncated(true)
    {
...
int RGWOrphanSearch::compare_oid_indexes(const DoutPrefixProvider* dpp)
{
    ...
    int ret = rgw_init_ioctx(
        dpp, static_cast<rgw::sal::RadosStore*>(store)->getRados()->get_rados_handle(), search_info.pool, data_ioctx);
    if (ret < 0) {
        ldpp_dout(dpp, -1) << __func__ << ": rgw_init_ioctx() returned ret=" << ret << dendl;
        return ret;
    }
    ...
    for (; liter != linked_objs_index.end(); ++liter, ++aiter) {
        ...
        do {
            ...
            if (stale_secs && (uint64_t)mtime >= time_threshold) {
                ldpp_dout(dpp, 20) << "skipping: " << key << " (mtime=" << mtime << " threshold=" << time_threshold
                                   << ")" << dendl;
                continue;
            }
...
int RGWOrphanSearch::finish()
{
    ...
    if (r < 0) {
        ldout(store->ctx(), 0) << "ERROR: could not remove job name (" << search_info.job_name << ") ret=" << r
                               << dendl;
    }
...
int RGWRadosList::handle_stat_result(const DoutPrefixProvider* dpp, RGWRados::Object::Stat::Result& result,
                                     std::string& bucket_name, rgw_obj_key& obj_key, std::set<string>& obj_oids)
{
    obj_oids.clear();

    rgw_bucket& bucket = result.obj.bucket;

    ldpp_dout(dpp, 20) << "RGWRadosList::" << __func__ << " bucket=" << bucket
                       << ", has_manifest=" << result.manifest.has_value() << dendl;
    ...
    ldpp_dout(dpp, 20) << "radoslist processing object=\"" << oid << "\"" << dendl;
    if (visited_oids.find(oid) != visited_oids.end()) {
        // apparently we hit a loop; don't continue with this oid
        ldpp_dout(dpp, 15) << "radoslist stopped loop at already visited object=\"" << oid << "\"" << dendl;
        return 0;
    }
    ...
    if (!result.manifest) {
    ...
    else if ((attr_it = result.attrs.find(RGW_ATTR_USER_MANIFEST)) != result.attrs.end()) {
        // *** handle DLO object ***

        obj_oids.insert(oid);
        visited_oids.insert(oid);   // prevent dlo loops
        ldpp_dout(dpp, 15) << "radoslist added to visited list DLO=\"" << oid << "\"" << dendl;
        ...
        ldpp_dout(dpp, 25) << "radoslist DLO oid=\"" << oid << "\" added bucket=\"" << bucket_name << "\" prefix=\""
        ...
        ldpp_dout(dpp, 15) << "radoslist added to visited list SLO=\"" << oid << "\"" << dendl;
        ...
        try {
            ...
            ldpp_dout(dpp, 0) << "ERROR: failed to decode slo manifest for " << oid << dendl;
            return -EIO;
        }
        ...
        for (const auto& iter : slo_info.entries) {
            ...
            ldpp_dout(dpp, 25) << "radoslist SLO oid=\"" << oid << "\" added bucket=\"" << bucket_name
                               << "\" obj_key=\"" << obj_key << "\" to process list" << dendl;
        }
    }
    else {
        RGWObjManifest& manifest = *result.manifest;
        ...
        if (0 == manifest.get_max_head_size() || manifest.obj_begin(dpp) == manifest.obj_end(dpp)) {
            obj_oids.insert(oid);
            // first_insert = true;
        }
...
int RGWRadosList::pop_and_handle_stat_op(const DoutPrefixProvider* dpp, RGWObjectCtx& obj_ctx,
                                         std::deque<RGWRados::Object::Stat>& ops)
{
    std::string bucket_name;
    rgw_obj_key obj_key;
    ...
    if (ret < 0) {
        ldpp_dout(dpp, -1) << "ERROR: handle_stat_result() returned error: " << cpp_strerror(-ret) << dendl;
    }

    // output results
    for (const auto& o : obj_oids) {
        if (include_rgw_obj_name) {
            std::cout << o << field_separator << bucket_name << field_separator << obj_key << std::endl;
        }
        else {
            std::cout << o << std::endl;
        }
...
int RGWRadosList::process_bucket(const DoutPrefixProvider* dpp, const std::string& bucket_instance_id,
                                 const std::string& prefix, const std::set<rgw_obj_key>& entries_filter)
{
    ldpp_dout(dpp, 10) << "RGWRadosList::" << __func__ << " bucket_instance_id=" << bucket_instance_id
                       << ", prefix=" << prefix << ", entries_filter.size=" << entries_filter.size() << dendl;

    RGWBucketInfo bucket_info;
    int ret =
        store->getRados()->get_bucket_instance_info(bucket_instance_id, bucket_info, nullptr, nullptr, null_yield, dpp);
    if (ret < 0) {
        if (ret == -ENOENT) {
            // probably raced with bucket removal
            return 0;
    ...
    do {
        ...
        ret = list_op.list_objects(dpp, LIST_OBJS_MAX_ENTRIES, &result, NULL, &truncated, null_yield);
        if (ret == -ENOENT) {
            ...
            std::cerr << "ERROR: store->list_objects(): " << cpp_strerror(-ret) << std::endl;
            return ret;
        }

        for (std::vector<rgw_bucket_dir_entry>::iterator iter = result.begin(); iter != result.end(); ++iter) {
            rgw_bucket_dir_entry& entry = *iter;
            ...
            if (!entries_filter.empty() && entries_filter.find(entry.key) == entries_filter.cend()) {
                continue;
            }
            ...
            auto do_stat_key = [&](const rgw_obj_key& key) -> int {
                int ret;
                ...
                if (stat_ops.size() >= max_concurrent_ios) {
                    ...
                    if (ret < 0) {
                        if (ret != -ENOENT) {
                            ldpp_dout(dpp, -1)
                                << "ERROR: pop_and_handle_stat_op() returned error: " << cpp_strerror(-ret) << dendl;
                        }
            ...
            if (!entry.key.instance.empty() && entry.key.name != prev_versioned_key_name) {
...
int RGWRadosList::run(const DoutPrefixProvider* dpp, const bool yes_i_really_mean_it)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, -1) << "RGWRadosList::" << __func__ << " ERROR: list_keys_init returned " << cpp_strerror(-ret)
                           << dendl;
        return ret;
    }
            ...
            if (ret == -ENOENT) {
                ...
                if (!warned_indexless) {
                    if (yes_i_really_mean_it) {
                        std::cerr
                            << "WARNING: because there is at least one indexless bucket (" << bucket_id
                        ...
                        std::cerr
                            << "ERROR: because there is at least one indexless bucket (" << bucket_id
...
int RGWRadosList::run(const DoutPrefixProvider* dpp, const std::string& start_bucket_name, const bool silent_indexless)
{
    int ret;
    ...
    while (!bucket_process_map.empty()) {
        ...
        if (ret == -ENOENT) {
            std::cerr << "WARNING: bucket " << bucket_name
                      << " does not exist; could it have been deleted very recently?" << std::endl;
            continue;
        }
        else if (ret < 0) {
            std::cerr << "ERROR: could not get info for bucket " << bucket_name << " -- " << cpp_strerror(-ret)
                      << std::endl;
            return ret;
        }
        else if (bucket->get_info().is_indexless()) {
            if (!silent_indexless) {
                std::cerr << "ERROR: unable to run radoslist on indexless bucket " << bucket_name << std::endl;
            }
            return -EINVAL;
        }
        ...
        auto do_process_bucket = [dpp, &bucket_id, this](const std::string& prefix,
            ...
            if (ret < 0) {
                ldpp_dout(dpp, -1) << "RGWRadosList::" << __func__
                                   << ": ERROR: process_bucket(); bucket_id=" << bucket_id << " returned ret=" << ret
                                   << dendl;
            }
    ...
    if (ret == -ENOENT) {
        ...
        ldpp_dout(dpp, -1) << "RGWRadosList::" << __func__ << ": ERROR: get_bucket_info returned ret=" << ret << dendl;
        return ret;
    }

    ret = do_incomplete_multipart(dpp, bucket.get());
    if (ret < 0) {
        ldpp_dout(dpp, -1) << "RGWRadosList::" << __func__ << ": ERROR: do_incomplete_multipart returned ret=" << ret
                           << dendl;
        return ret;
    }
...
int RGWRadosList::do_incomplete_multipart(const DoutPrefixProvider* dpp, rgw::sal::Bucket* bucket)
{
        ...
        if (ret == -ENOENT) {
            ...
            ldpp_dout(dpp, 5) << "RGWRadosList::" << __func__
                              << ": WARNING: call to list_objects of multipart namespace got ENOENT; "
                                 "assuming bucket removal race"
                              << dendl;
            break;
        }
        else if (ret < 0) {
            ldpp_dout(dpp, -1) << "RGWRadosList::" << __func__ << ": ERROR: list_objects op returned ret=" << ret
                               << dendl;
            return ret;
        }
        ...
        if (!uploads.empty()) {
            ...
            for (const auto& upload : uploads) {
                ...
                do {   // while (is_parts_truncated);
                    ret = upload->list_parts(
                        dpp, store->ctx(), max_parts, parts_marker, &parts_marker, &is_parts_truncated);
                    if (ret == -ENOENT) {
                        ldpp_dout(dpp, 5) << "RGWRadosList::" << __func__
                                          << ": WARNING: list_multipart_parts returned ret=-ENOENT "
                                             "for "
                                          << upload->get_upload_id() << ", moving on" << dendl;
                        break;
                    }
                    else if (ret < 0) {
                        ldpp_dout(dpp, -1) << "RGWRadosList::" << __func__
                    ...
                    for (auto& p : upload->get_parts()) {
                        ...
                        for (auto obj_it = manifest.obj_begin(dpp); obj_it != manifest.obj_end(dpp); ++obj_it) {
                            const rgw_raw_obj& loc = obj_it.get_location().get_raw_obj(store->getRados());
...
void RGWOrphanSearchStage::dump(Formatter* f) const
{
    ...
    string s;
    switch (stage) {
    case ORPHAN_SEARCH_STAGE_INIT: s = "init"; break;
    case ORPHAN_SEARCH_STAGE_LSPOOL: s = "lspool"; break;
    case ORPHAN_SEARCH_STAGE_LSBUCKETS: s = "lsbuckets"; break;
    case ORPHAN_SEARCH_STAGE_ITERATE_BI: s = "iterate_bucket_index"; break;
    case ORPHAN_SEARCH_STAGE_COMPARE: s = "comparing"; break;
    default: s = "unknown";
    }
