...
int RGWOrphanSearch::log_oids(const DoutPrefixProvider* dpp, map<int, string>& log_shards,
                                                  ...
                                                  map<int, list<string> >& oids)
{
    ...
    ret = store->getRados()->get_bucket_info(
        store->svc(), orphan_bucket.tenant, orphan_bucket.name, cur_bucket_info, nullptr, null_yield, dpp);
    if (ret < 0) {
        if (ret == -ENOENT) {
    ...
    ret = store->getRados()->get_bucket_instance_info(
        sysobj_ctx, bucket_instance_id, bucket_info, nullptr, nullptr, null_yield, dpp);
    if (ret < 0) {
        if (ret == -ENOENT) {
            /* probably raced with bucket removal */
            return 0;
                                     ...
                                     std::string& bucket_name, rgw_obj_key& obj_key, std::set<string>& obj_oids)
{
    ...
    else if ((attr_it = result.attrs.find(RGW_ATTR_SLO_MANIFEST)) != result.attrs.end()) {
        // *** handle SLO object ***
                                 ...
                                 const std::string& prefix, const std::set<rgw_obj_key>& entries_filter)
{
    ...
    int ret = store->getRados()->get_bucket_instance_info(
        sys_obj_ctx, bucket_instance_id, bucket_info, nullptr, nullptr, null_yield, dpp);
    if (ret < 0) {
        if (ret == -ENOENT) {
            // probably raced with bucket removal
            return 0;
...
int RGWRadosList::run(const DoutPrefixProvider* dpp, const std::string& start_bucket_name)
{
    ...
    while (!bucket_process_map.empty()) {
        ...
        RGWBucketInfo bucket_info;
        ret = store->getRados()->get_bucket_info(
            store->svc(), tenant_name, bucket_name, bucket_info, nullptr, null_yield, dpp);
        if (ret == -ENOENT) {
    ...
    ret = store->getRados()->get_bucket_info(
        store->svc(), tenant_name, start_bucket_name, bucket_info, nullptr, null_yield, dpp);
    if (ret == -ENOENT) {
        // bucket deletion race?
        return 0;
    }
...
int RGWRadosList::do_incomplete_multipart(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                          RGWBucketInfo& bucket_info)
{
    ...
    do {
        ...
        ret = list_op.list_objects(dpp, max_uploads, &objs, &common_prefixes, &is_listing_truncated, null_yield);
        if (ret == -ENOENT) {
        ...
        if (!objs.empty()) {
            ...
            for (const rgw_bucket_dir_entry& obj : objs) {
                ...
                ldpp_dout(dpp, 20) << "RGWRadosList::" << __func__ << " processing incomplete multipart entry " << entry
                                   << dendl;
            }
                    ...
                    if (ret == -ENOENT) {
                        ldpp_dout(dpp, 5) << "RGWRadosList::" << __func__
                                          << ": WARNING: list_multipart_parts returned ret=-ENOENT "
                                             "for "
                                          << mp.get_upload_id() << ", moving on" << dendl;
                        break;
                    }
                    else if (ret < 0) {
                        ldpp_dout(dpp, -1) << "RGWRadosList::" << __func__
