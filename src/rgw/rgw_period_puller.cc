...
int pull_period(const DoutPrefixProvider* dpp, RGWRESTConn* conn, const std::string& period_id,
                const std::string& realm_id, RGWPeriod& period, optional_yield y)
{
    rgw_user user;
    RGWEnv env;
...
#define MAX_REST_RESPONSE (128 * 1024)
    ...
    try {
        ...
        lderr(conn->get_ctx()) << "failed to decode JSON input: " << e.what() << dendl;
        return -EINVAL;
    }
    return 0;
}
                          ...
                          optional_yield y)
{
    ...
    if (r < 0) {
        ...
        ldpp_dout(dpp, 14) << "pulling period " << period_id << " from master" << dendl;
        // request the period from the master zone
        r = pull_period(dpp, svc.zone->get_master_conn(), period_id, svc.zone->get_realm().get_id(), period, y);
        if (r < 0) {
        ...
        if (r < 0) {
            ldpp_dout(dpp, -1) << "failed to update latest_epoch for period " << period_id << dendl;
            return r;
        }
        ...
        ldpp_dout(dpp, 14) << "period " << period_id << " pulled and written to local storage" << dendl;
    }
    else {
        ldpp_dout(dpp, 14) << "found period " << period_id << " in local storage" << dendl;
    }
    return 0;
}
