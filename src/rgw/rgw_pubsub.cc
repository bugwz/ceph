...
bool match(const rgw_s3_key_value_filter& filter, const KeyMultiValueMap& kv)
{
    ...
    for (auto& filter : filter.kv) {
        ...
        if (std::any_of(result.first, result.second, [&filter](const std::pair<std::string, std::string>& p) {
...
int RGWPubSub::read_topics(const DoutPrefixProvider* dpp, rgw_pubsub_topics& result, RGWObjVersionTracker* objv_tracker,
...
int RGWPubSub::get_topic(const DoutPrefixProvider* dpp, const std::string& name, rgw_pubsub_topic& result,
                         optional_yield y) const
{
...
auto find_unique_topic(const rgw_pubsub_bucket_topics& bucket_topics, const std::string& notification_id)
{
    auto it = std::find_if(bucket_topics.topics.begin(), bucket_topics.topics.end(), [&](const auto& val) {
        return notification_id == val.second.s3_id;
    });
                                           ...
                                           const std::string& notif_name, optional_yield y) const
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 1) << "ERROR: failed to read topics from bucket '" << bucket->get_name() << "': ret=" << ret
                          << dendl;
        return ret;
    }
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 1) << "ERROR: failed to write topics to bucket '" << bucket->get_name() << "': ret=" << ret
                          << dendl;
        return ret;
    }

    ldpp_dout(dpp, 20) << "successfully wrote " << bucket_topics.topics.size() << " topics to bucket '"
                       << bucket->get_name() << "'" << dendl;

    return 0;
}

int RGWPubSub::Bucket::remove_notification(const DoutPrefixProvider* dpp, const std::string& topic_name,
                                           optional_yield y) const
{
...
int RGWPubSub::Bucket::remove_notifications(const DoutPrefixProvider* dpp, optional_yield y) const
{
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(dpp, 1) << "ERROR: failed to get list of topics from bucket '" << bucket->get_name()
                          << "', ret=" << ret << dendl;
        return ret;
    }
