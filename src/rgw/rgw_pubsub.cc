...
{
    return key_filter.has_content() || metadata_filter.has_content() || tag_filter.has_content();
}
...
bool match(const rgw_s3_key_value_filter& filter, const KeyMultiValueMap& kv)
{
    ...
    for (auto& filter : filter.kv) {
        ...
        if (std::any_of(result.first, result.second, [&filter](const pair<string, string>& p) {
                return p.second == filter.second;
            }))
            continue;
        else
            return false;
...
rgw_pubsub_s3_notification::rgw_pubsub_s3_notification(const rgw_pubsub_topic_filter& topic_filter)
    : id(topic_filter.s3_id)
    , events(topic_filter.events)
    , topic_arn(topic_filter.topic.arn)
    , filter(topic_filter.s3_filter)
{}
...
int RGWPubSub::remove(const DoutPrefixProvider* dpp, const rgw_raw_obj& obj, RGWObjVersionTracker* objv_tracker,
                      optional_yield y)
{
...
int RGWPubSub::Bucket::create_notification(const DoutPrefixProvider* dpp, const string& topic_name,
                                           const rgw::notify::EventTypeList& events, optional_yield y)
{
...
int RGWPubSub::Bucket::create_notification(const DoutPrefixProvider* dpp, const string& topic_name,
                                           ...
                                           const std::string& notif_name, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 1) << "ERROR: failed to read topics from bucket '" << bucket.name << "': ret=" << ret << dendl;
        return ret;
    }
    ldpp_dout(dpp, 20) << "successfully read " << bucket_topics.topics.size() << " topics from bucket '" << bucket.name
                       << "'" << dendl;
    ...
    ldpp_dout(dpp, 20) << "successfully wrote " << bucket_topics.topics.size() << " topics to bucket '" << bucket.name
                       << "'" << dendl;
...
int RGWPubSub::Bucket::remove_notifications(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    if (ret < 0 && ret != -ENOENT) {
        ldpp_dout(dpp, 1) << "ERROR: failed to get list of topics from bucket '" << bucket.name << "', ret=" << ret
                          << dendl;
        return ret;
    }
    ...
    for (const auto& topic : bucket_topics.topics) {
        ...
        if (ret < 0 && ret != -ENOENT) {
            ldpp_dout(dpp, 5) << "WARNING: failed to remove auto-generated topic '" << topic_name << "', ret=" << ret
                              << dendl;
        }
    }
...
int RGWPubSub::create_topic(const DoutPrefixProvider* dpp, const string& name, const rgw_pubsub_sub_dest& dest,
                            const std::string& arn, const std::string& opaque_data, optional_yield y)
{
    RGWObjVersionTracker objv_tracker;
    rgw_pubsub_topics topics;
...
int RGWPubSub::Sub::write_sub(const DoutPrefixProvider* dpp, const rgw_pubsub_sub_config& sub_conf,
                              RGWObjVersionTracker* objv_tracker, optional_yield y)
{
...
int RGWPubSub::Sub::remove_sub(const DoutPrefixProvider* dpp, RGWObjVersionTracker* objv_tracker, optional_yield y)
{
...
int RGWPubSub::Sub::subscribe(const DoutPrefixProvider* dpp, const string& topic, const rgw_pubsub_sub_dest& dest,
                              optional_yield y, const std::string& s3_id)
{
    RGWObjVersionTracker objv_tracker;
    rgw_pubsub_topics topics;
...
template<typename EventType> void RGWPubSub::SubWithEvents<EventType>::list_events_result::dump(Formatter* f) const
{
...
int RGWPubSub::SubWithEvents<EventType>::list_events(const DoutPrefixProvider* dpp, const string& marker,
                                                     int max_events)
{
    ...
    string tenant;
    ret = store->get_bucket_info(
        &store->svc, tenant, sub_conf.dest.bucket_name, bucket_info, nullptr, null_yield, nullptr);
    if (ret == -ENOENT) {
        list.is_truncated = false;
        return 0;
    }
    if (ret < 0) {
        ldpp_dout(dpp, 1) << "ERROR: failed to read bucket info for events bucket: bucket=" << sub_conf.dest.bucket_name
                          << " ret=" << ret << dendl;
        return ret;
    }
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 1) << "ERROR: failed to list bucket: bucket=" << sub_conf.dest.bucket_name << " ret=" << ret
                          << dendl;
        return ret;
    }
...
template<typename EventType> void RGWPubSub::SubWithEvents<EventType>::dump(Formatter* f) const
{
    list.dump(f);
}
