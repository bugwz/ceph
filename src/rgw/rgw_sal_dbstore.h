...
public:
    DBUser(DBStore* _st, const rgw_user& _u)
        : StoreUser(_u)
        , store(_st)
    {}
    DBUser(DBStore* _st, const RGWUserInfo& _i)
        : StoreUser(_i)
        , store(_st)
    {}
    DBUser(DBStore* _st)
    ...
    virtual std::unique_ptr<User> clone() override { return std::unique_ptr<User>(new DBUser(*this)); }
...
public:
    ...
    virtual std::unique_ptr<Bucket> clone() override { return std::make_unique<DBBucket>(*this); }
    virtual std::unique_ptr<MultipartUpload> get_multipart_upload(
        const std::string& oid, std::optional<std::string> upload_id, ACLOwner owner = {},
        ceph::real_time mtime = ceph::real_clock::now()) override;
...
public:
    DBPlacementTier(DBStore* _store, const RGWZoneGroupPlacementTier& _tier)
        : store(_store)
        , tier(_tier)
    {}
...
public:
    DBZoneGroup(DBStore* _store, std::unique_ptr<RGWZoneGroup> _group)
        : store(_store)
    ...
    virtual int get_placement_tier(const rgw_placement_rule& rule, std::unique_ptr<PlacementTier>* tier) { return -1; }
...
public:
    ...
    virtual std::unique_ptr<Zone> clone() override { return std::make_unique<DBZone>(store); }
...
public:
    DBMPObj() {}
    DBMPObj(const std::string& _oid, const std::string& _upload_id) { init(_oid, _upload_id, _upload_id); }
    ...
    void init(const std::string& _oid, const std::string& _upload_id) { init(_oid, _upload_id, _upload_id); }
    ...
    const std::string& get_key() const { return oid; }
...
public:
    DBMultipartUpload(DBStore* _store, Bucket* _bucket, const std::string& oid, std::optional<std::string> upload_id,
                      ACLOwner _owner, ceph::real_time _mtime)
...
public:
    ...
    virtual std::unique_ptr<Object> clone() override { return std::unique_ptr<Object>(new DBObject(*this)); }
...
private:
    int read_attrs(const DoutPrefixProvider* dpp, DB::Object::Read& read_op, optional_yield y,
...
public:
    DBAtomicWriter(const DoutPrefixProvider* dpp, optional_yield y, rgw::sal::Object* obj, DBStore* _store,
...
public:
    DBMultipartWriter(const DoutPrefixProvider* dpp, optional_yield y, MultipartUpload* upload, rgw::sal::Object* obj,
                      DBStore* _store, const rgw_user& owner, const rgw_placement_rule* ptail_placement_rule,
...
public:
    ...
    virtual const std::string get_name() const override { return "dbstore"; }
