...
template<class T> class RGWQuotaCache
{
protected:
    ...
    public:
        ...
        {
            if (entry->async_refresh_time.sec() == 0) return false;
    ...
    virtual int fetch_stats_from_storage(const rgw_user& user, const rgw_bucket& bucket, RGWStorageStats& stats,
    ...
    virtual bool map_find_and_update(const rgw_user& user, const rgw_bucket& bucket,
                                     typename lru_map<T, RGWQuotaCacheStats>::UpdateContext* ctx) = 0;
...
public:
    RGWQuotaCache(rgw::sal::RGWRadosStore* _store, int size)
        : store(_store)
        , stats_map(size)
    {
        async_refcount = new RefCountedWaitObject;
    }
    virtual ~RGWQuotaCache() { async_refcount->put_wait(); /* wait for all pending async requests to complete */ }

    int get_stats(const rgw_user& user, const rgw_bucket& bucket, RGWStorageStats& stats, optional_yield y,
                  const DoutPrefixProvider* dpp);
    void adjust_stats(const rgw_user& user, rgw_bucket& bucket, int objs_delta, uint64_t added_bytes,
                      uint64_t removed_bytes);
    ...
    public:
        AsyncRefreshHandler(rgw::sal::RGWRadosStore* _store, RGWQuotaCache<T>* _cache)
            : store(_store)
            , cache(_cache)
        {}
...
template<class T> void RGWQuotaCache<T>::async_refresh_fail(const rgw_user& user, rgw_bucket& bucket)
{
...
template<class T>
void RGWQuotaCache<T>::set_stats(const rgw_user& user, const rgw_bucket& bucket, RGWQuotaCacheStats& qs,
                                 RGWStorageStats& stats)
{
    qs.stats = stats;
...
template<class T> class RGWQuotaStatsUpdate : public lru_map<T, RGWQuotaCacheStats>::UpdateContext
{
...
public:
    RGWQuotaStatsUpdate(const int objs_delta, const uint64_t added_bytes, const uint64_t removed_bytes)
...
template<class T>
void RGWQuotaCache<T>::adjust_stats(const rgw_user& user, rgw_bucket& bucket, int objs_delta, uint64_t added_bytes,
                                    uint64_t removed_bytes)
{
...
class BucketAsyncRefreshHandler : public RGWQuotaCache<rgw_bucket>::AsyncRefreshHandler, public RGWGetBucketStats_CB
{
    rgw_user user;

public:
    BucketAsyncRefreshHandler(rgw::sal::RGWRadosStore* _store, RGWQuotaCache<rgw_bucket>* _cache, const rgw_user& _user,
                              const rgw_bucket& _bucket)
        : RGWQuotaCache<rgw_bucket>::AsyncRefreshHandler(_store, _cache)
        , RGWGetBucketStats_CB(_bucket)
        , user(_user)
    {}
...
protected:
    ...
    bool map_find_and_update(const rgw_user& user, const rgw_bucket& bucket,
                             lru_map<rgw_bucket, RGWQuotaCacheStats>::UpdateContext* ctx) override
    {
    ...
    int fetch_stats_from_storage(const rgw_user& user, const rgw_bucket& bucket, RGWStorageStats& stats,
                                 optional_yield y, const DoutPrefixProvider* dpp) override;

public:
    explicit RGWBucketStatsCache(rgw::sal::RGWRadosStore* _store)
        : RGWQuotaCache<rgw_bucket>(_store, _store->ctx()->_conf->rgw_bucket_quota_cache_size)
    {}
...
int RGWBucketStatsCache::fetch_stats_from_storage(const rgw_user& user, const rgw_bucket& bucket,
                                                  RGWStorageStats& stats, optional_yield y,
                                                  const DoutPrefixProvider* dpp)
{
    RGWBucketInfo bucket_info;
    ...
    r = store->getRados()->get_bucket_stats(
        dpp, bucket_info, RGW_NO_SHARD, &bucket_ver, &master_ver, bucket_stats, nullptr);
    if (r < 0) {
        ldpp_dout(dpp, 0) << "could not get bucket stats for bucket=" << bucket.name << dendl;
        return r;
    }
...
class UserAsyncRefreshHandler : public RGWQuotaCache<rgw_user>::AsyncRefreshHandler, public RGWGetUserStats_CB
{
    const DoutPrefixProvider* dpp;
    rgw_bucket bucket;

public:
    UserAsyncRefreshHandler(const DoutPrefixProvider* _dpp, rgw::sal::RGWRadosStore* _store,
                            RGWQuotaCache<rgw_user>* _cache, const rgw_user& _user, const rgw_bucket& _bucket)
    ...
    public:
        ...
        void* entry() override
        {
            ...
            do {
                ...
                if (stats->going_down()) break;

                std::unique_lock locker{lock};
                cond.wait_for(locker, std::chrono::seconds(cct->_conf->rgw_user_quota_bucket_sync_interval));
...
protected:
    ...
    bool map_find_and_update(const rgw_user& user, const rgw_bucket& bucket,
                             lru_map<rgw_user, RGWQuotaCacheStats>::UpdateContext* ctx) override
    {
...
public:
    ...
    ~RGWUserStatsCache() override { stop(); }
                                   ...
                                   const DoutPrefixProvider* dpp)
{
    ...
    if (r < 0) {
        ldout(store->ctx(), 0) << "ERROR: sync_user_stats() for user=" << user << ", bucket=" << bucket << " returned "
                               << r << dendl;
        return r;
    }
...
int RGWUserStatsCache::sync_user(const DoutPrefixProvider* dpp, const rgw_user& user, optional_yield y)
{
    ...
    if (!store->ctx()->_conf->rgw_user_quota_sync_idle_users && last_stats_update < last_stats_sync) {
...
int RGWUserStatsCache::sync_all_users(const DoutPrefixProvider* dpp, optional_yield y)
{
    ...
    do {
        ...
        for (list<string>::iterator iter = keys.begin(); iter != keys.end() && !going_down(); ++iter) {
            rgw_user user(*iter);
...
public:
    ...
    virtual bool is_size_exceeded(const char* const entity, const RGWQuotaInfo& qinfo, const RGWStorageStats& stats,
                                  const uint64_t size) const = 0;

    virtual bool is_num_objs_exceeded(const char* const entity, const RGWQuotaInfo& qinfo, const RGWStorageStats& stats,
...
{
public:
    bool is_size_exceeded(const char* const entity, const RGWQuotaInfo& qinfo, const RGWStorageStats& stats,
                          const uint64_t size) const override;

    bool is_num_objs_exceeded(const char* const entity, const RGWQuotaInfo& qinfo, const RGWStorageStats& stats,
...
bool RGWQuotaInfoDefApplier::is_size_exceeded(const char* const entity, const RGWQuotaInfo& qinfo,
                                              const RGWStorageStats& stats, const uint64_t size) const
{
    if (qinfo.max_size < 0) {
    ...
    if (cur_size + new_size > static_cast<uint64_t>(qinfo.max_size)) {
        dout(10) << "quota exceeded: stats.size_rounded=" << stats.size_rounded << " size=" << new_size << " " << entity
                 << "_quota.max_size=" << qinfo.max_size << dendl;
        return true;
    }
...
bool RGWQuotaInfoDefApplier::is_num_objs_exceeded(const char* const entity, const RGWQuotaInfo& qinfo,
                                                  const RGWStorageStats& stats, const uint64_t num_objs) const
{
    ...
    if (stats.num_objects + num_objs > static_cast<uint64_t>(qinfo.max_objects)) {
        dout(10) << "quota exceeded: stats.num_objects=" << stats.num_objects << " " << entity
                 << "_quota.max_objects=" << qinfo.max_objects << dendl;
        return true;
    }
...
bool RGWQuotaInfoRawApplier::is_size_exceeded(const char* const entity, const RGWQuotaInfo& qinfo,
                                              const RGWStorageStats& stats, const uint64_t size) const
{
    if (qinfo.max_size < 0) {
        /* The limit is not enabled. */
    ...
    if (cur_size + size > static_cast<uint64_t>(qinfo.max_size)) {
        dout(10) << "quota exceeded: stats.size=" << stats.size << " size=" << size << " " << entity
                 << "_quota.max_size=" << qinfo.max_size << dendl;
        return true;
    }
...
bool RGWQuotaInfoRawApplier::is_num_objs_exceeded(const char* const entity, const RGWQuotaInfo& qinfo,
                                                  const RGWStorageStats& stats, const uint64_t num_objs) const
{
    if (qinfo.max_objects < 0) {
        /* The limit is not enabled. */
...
const RGWQuotaInfoApplier& RGWQuotaInfoApplier::get_instance(const RGWQuotaInfo& qinfo)
{
...
class RGWQuotaHandlerImpl : public RGWQuotaHandler
{
    ...
    int check_quota(const char* const entity, const RGWQuotaInfo& quota, const RGWStorageStats& stats,
                    const uint64_t num_objs, const uint64_t size)
    {
        if (!quota.enabled) {
            return 0;
        }
        ...
        ldout(store->ctx(), 20) << entity << " quota OK:"
                                << " stats.num_objects=" << stats.num_objects << " stats.size=" << stats.size << dendl;
        return 0;
    }

public:
    RGWQuotaHandlerImpl(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* _store, bool quota_threads)
        : store(_store)
        , bucket_stats_cache(_store)
        , user_stats_cache(dpp, _store, quota_threads)
    {}

    int check_quota(const rgw_user& user, rgw_bucket& bucket, RGWQuotaInfo& user_quota, RGWQuotaInfo& bucket_quota,
                    uint64_t num_objs, uint64_t size, optional_yield y) override
    {
    ...
    void update_stats(const rgw_user& user, rgw_bucket& bucket, int obj_delta, uint64_t added_bytes,
    ...
    void check_bucket_shards(uint64_t max_objs_per_shard, uint64_t num_shards, uint64_t num_objs, bool& need_resharding,
...
RGWQuotaHandler* RGWQuotaHandler::generate_handler(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                                                   bool quota_threads)
{
