...
void RGWOp_BILog_List::execute(optional_yield y)
{
    ...
    if (!bucket_instance.empty()) {
        ...
        op_ret =
            store->getRados()->get_bucket_instance_info(*s->sysobj_ctx, b, bucket_info, NULL, NULL, s->yield, this);
        if (op_ret < 0) {
            ldpp_dout(this, 5) << "could not get bucket instance info for bucket instance id=" << bucket_instance
                               << dendl;
            return;
        }
    }
    else { /* !bucket_name.empty() */
        op_ret = store->getRados()->get_bucket_info(
            store->svc(), tenant_name, bucket_name, bucket_info, NULL, s->yield, NULL);
        if (op_ret < 0) {
    ...
    do {
        ...
        int ret = store->svc()->bilog_rados->log_list(
            s, bucket_info, shard_id, marker, max_entries - count, entries, &truncated);
        if (ret < 0) {
...
void RGWOp_BILog_Info::execute(optional_yield y)
{
    ...
    int ret = store->getRados()->get_bucket_stats(
        s, bucket_info, shard_id, &bucket_ver, &master_ver, stats, &max_marker, &syncstopped);
    if (ret < 0 && ret != -ENOENT) {
        op_ret = ret;
        return;
    }
...
void RGWOp_DATALog_List::execute(optional_yield y)
{
    ...
    // entry listed
    op_ret = store->svc()->datalog_rados->list_entries(
        this, shard_id, max_entries, entries, marker, &last_marker, &truncated);
}
...
void RGWOp_BILog_Status::execute(optional_yield y)
{
    ...
    if (!merge) {
        ...
        if (op_ret < 0) {
            ldpp_dout(this, -1) << "ERROR: rgw_bucket_sync_status() on pipe=" << pipe << " returned ret=" << op_ret
                                << dendl;
        }
        return;
    }
    ...
    for (auto& entry : local_dests) {
        ...
        if (*pipe.dest.bucket != info.bucket) {
            ...
            op_ret = store->ctl()->bucket->read_bucket_info(
                *pipe.dest.bucket, pinfo, s->yield, s, RGWBucketCtl::BucketInstance::GetParams(), nullptr);
            if (op_ret < 0) {
        ...
        if (status.empty()) {
            ...
            if (current_status.size() != status.size()) {
                op_ret = -EINVAL;
                ldpp_dout(this, -1) << "ERROR: different number of shards for sync status of buckets syncing from the "
                                       "same source: status.size()= "
                                    << status.size() << " current_status.size()=" << current_status.size() << dendl;
                return;
            }
            auto m = status.begin();
...
RGWOp* RGWHandler_Log::op_post()
{
    ...
    if (type.compare("metadata") == 0) {
        ...
        if (s->info.args.exists("notify")) return new RGWOp_DATALog_Notify;
    }
    return NULL;
}
