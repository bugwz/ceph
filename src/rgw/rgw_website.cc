...
{
    return (key.size() >= key_prefix_equals.size() && key.compare(0, key_prefix_equals.size(), key_prefix_equals) == 0);
}


void RGWBWRoutingRule::apply_rule(const string& default_protocol, const string& default_hostname, const string& key,
                                  string* new_url, int* redirect_code)
{
    ...
    *new_url = protocol + "://" + hostname + "/";
    ...
    if (redirect.http_redirect_code > 0) *redirect_code = redirect.http_redirect_code;
}
...
void RGWBWRedirectInfo::decode_xml(XMLObj* obj)
{
    ...
    if (has_http_redirect_code && !(code > WEBSITE_HTTP_REDIRECT_CODE_MIN && code < WEBSITE_HTTP_REDIRECT_CODE_MAX)) {
...
void RGWBWRoutingRuleCondition::decode_xml(XMLObj* obj)
{
    ...
    if (has_http_error_code_returned_equals &&
        !(code >= WEBSITE_HTTP_ERROR_CODE_RETURNED_EQUALS_MIN && code < WEBSITE_HTTP_ERROR_CODE_RETURNED_EQUALS_MAX)) {
