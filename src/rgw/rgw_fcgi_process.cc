...
void RGWFCGXProcess::handle_request(RGWRequest* r)
{
    ...
    auto real_client_io = rgw::io::add_reordering(rgw::io::add_buffering(cct, rgw::io::add_chunking(&fcgxfe)));
    RGWRestfulIO client_io(cct, &real_client_io);


    int ret = process_request(store, rest, req, uri_prefix, *auth_registry, &client_io, olog, null_yield, nullptr);
    if (ret < 0) {
