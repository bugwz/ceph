...
public:
    ...
    explicit RGWHTTPClient(CephContext* cct, const std::string& _method, const std::string& _url);
    ...
    void set_send_data_hint(bool hint) { send_data_hint = hint; }

    long get_http_status() const { return http_status; }

    void set_http_status(long _http_status) { http_status = _http_status; }

    void set_verify_ssl(bool flag) { verify_ssl = flag; }

    // set request timeout in seconds
    // zero (default) mean that request will never timeout
    void set_req_timeout(long timeout) { req_timeout = timeout; }
    ...
    void set_io_user_info(void* _user_info) override { user_info = _user_info; }

    void* get_io_user_info() override { return user_info; }

    void set_ca_path(const std::string& _ca_path) { ca_path = _ca_path; }

    void set_client_cert(const std::string& _client_cert) { client_cert = _client_cert; }

    void set_client_key(const std::string& _client_key) { client_key = _client_key; }
};
...
public:
    ...
    RGWHTTPHeadersCollector(CephContext* const cct, const std::string& method, const std::string& url,
                            const header_spec_t& relevant_headers)
        : RGWHTTPClient(cct, method, url)
        , relevant_headers(relevant_headers)
    {}

    std::map<header_name_t, header_value_t, ltstr_nocase> get_headers() const { return found_headers; }

    /* Throws std::out_of_range */
    const header_value_t& get_header_value(const header_name_t& name) const { return found_headers.at(name); }
...
public:
    RGWHTTPTransceiver(CephContext* const cct, const std::string& method, const std::string& url,
                       bufferlist* const read_bl, const header_spec_t intercept_headers = {})
    ...
    RGWHTTPTransceiver(CephContext* const cct, const std::string& method, const std::string& url,
                       bufferlist* const read_bl, const bool verify_ssl, const header_spec_t intercept_headers = {})
        : RGWHTTPHeadersCollector(cct, method, url, intercept_headers)
        , read_bl(read_bl)
        , post_data_index(0)
    {
    ...
    void set_post_data(const std::string& _post_data) { this->post_data = _post_data; }
...
class RGWHTTPManager
{
    struct set_state
    {
        ...
        set_state(rgw_http_req_data* _req, int _bitmask)
            : req(_req)
            , bitmask(_bitmask)
        {}
    };
    ...
    public:
        explicit ReqsThread(RGWHTTPManager* _m)
            : manager(_m)
        {}
        void* entry() override;
    };
