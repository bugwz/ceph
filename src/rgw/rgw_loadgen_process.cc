                                    ...
                                    std::atomic<bool>* fail_flag)
{
    RGWLoadGenRequest* req =
        new RGWLoadGenRequest(env.driver->get_new_req_id(), method, resource, content_length, fail_flag);
...
void RGWLoadGenProcess::handle_request(const DoutPrefixProvider* dpp, RGWRequest* r)
{
    ...
    int ret = process_request(env, req, uri_prefix, &client_io, null_yield, nullptr, nullptr, nullptr);
    if (ret < 0) {
