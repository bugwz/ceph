                                    ...
                                    std::atomic<bool>* fail_flag)
{
    RGWLoadGenRequest* req =
        new RGWLoadGenRequest(store->getRados()->get_new_req_id(), method, resource, content_length, fail_flag);
...
void RGWLoadGenProcess::handle_request(const DoutPrefixProvider* dpp, RGWRequest* r)
{
    ...
    int ret = process_request(
        store, rest, req, uri_prefix, *auth_registry, &client_io, olog, null_yield, nullptr, nullptr, nullptr);
    if (ret < 0) {
