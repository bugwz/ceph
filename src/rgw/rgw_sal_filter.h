...
public:
    FilterCompletions(std::unique_ptr<Completions> _next)
        : next(std::move(_next))
    {}
...
public:
    FilterPlacementTier(std::unique_ptr<PlacementTier> _next)
        : next(std::move(_next))
    {}
...
public:
    FilterZoneGroup(std::unique_ptr<ZoneGroup> _next)
        : next(std::move(_next))
    {}
    virtual ~FilterZoneGroup() = default;
    virtual const std::string& get_id() const override { return next->get_id(); }
    virtual const std::string& get_name() const override { return next->get_name(); }
    virtual int equals(const std::string& other_zonegroup) const override { return next->equals(other_zonegroup); }
    virtual const std::string& get_endpoint() const override { return next->get_endpoint(); }
    virtual bool placement_target_exists(std::string& target) const override
    {
        return next->placement_target_exists(target);
    }
    virtual bool is_master_zonegroup() const override { return next->is_master_zonegroup(); }
    virtual const std::string& get_api_name() const override { return next->get_api_name(); }
    ...
    virtual int get_hostnames(std::list<std::string>& names) const override { return next->get_hostnames(names); }
    virtual int get_s3website_hostnames(std::list<std::string>& names) const override
    {
        return next->get_s3website_hostnames(names);
    }
    virtual int get_zone_count() const override { return next->get_zone_count(); }
    ...
    virtual int list_zones(std::list<std::string>& zone_ids) override { return next->list_zones(zone_ids); }
    bool supports(std::string_view feature) const override { return next->supports(feature); }
...
public:
    FilterZone(std::unique_ptr<Zone> _next)
        : next(std::move(_next))
    {
    ...
    virtual const std::string& get_id() override { return next->get_id(); }
    virtual const std::string& get_name() const override { return next->get_name(); }
    virtual bool is_writeable() override { return next->is_writeable(); }
    virtual bool get_redirect_endpoint(std::string* endpoint) override { return next->get_redirect_endpoint(endpoint); }
    virtual bool has_zonegroup_api(const std::string& api) const override { return next->has_zonegroup_api(api); }
    virtual const std::string& get_current_period_id() override { return next->get_current_period_id(); }
    virtual const RGWAccessKey& get_system_key() override { return next->get_system_key(); }
    virtual const std::string& get_realm_name() override { return next->get_realm_name(); }
    virtual const std::string& get_realm_id() override { return next->get_realm_id(); }
    virtual const std::string_view get_tier_type() override { return next->get_tier_type(); }
    virtual RGWBucketSyncPolicyHandlerRef get_sync_policy_handler() override { return next->get_sync_policy_handler(); }
};
...
public:
    ...
    virtual void register_admin_apis(RGWRESTMgr* mgr) override { return next->register_admin_apis(mgr); }
};
...
public:
    FilterUser(std::unique_ptr<User> _next)
        : next(std::move(_next))
    {}
    FilterUser(FilterUser& u)
        : next(u.next->clone()){};
    virtual ~FilterUser() = default;

    virtual std::unique_ptr<User> clone() override { return std::make_unique<FilterUser>(*this); }
    virtual int list_buckets(const DoutPrefixProvider* dpp, const std::string& marker, const std::string& end_marker,
...
public:
    ...
    int write_topics(const rgw_pubsub_bucket_topics& notifications, RGWObjVersionTracker* obj_tracker, optional_yield y,
                     const DoutPrefixProvider* dpp) override
    {
...
public:
    struct FilterReadOp : ReadOp
    {
        ...
        FilterReadOp(std::unique_ptr<ReadOp> _next)
            : next(std::move(_next))
        {}
    ...
    struct FilterDeleteOp : DeleteOp
    {
        ...
        FilterDeleteOp(std::unique_ptr<DeleteOp> _next)
            : next(std::move(_next))
        {}
    ...
    FilterObject(std::unique_ptr<Object> _next)
        : next(std::move(_next))
    {}
    ...
    int range_to_ofs(uint64_t obj_size, int64_t& ofs, int64_t& end) { return next->range_to_ofs(obj_size, ofs, end); };
    ...
    virtual std::unique_ptr<Object> clone() override { return std::make_unique<FilterObject>(*this); }
...
public:
    FilterMultipartPart(std::unique_ptr<MultipartPart> _next)
        : next(std::move(_next))
    {}
...
public:
    FilterMPSerializer(std::unique_ptr<MPSerializer> _next)
        : next(std::move(_next))
    {}
...
public:
    FilterLCSerializer(std::unique_ptr<LCSerializer> _next)
        : next(std::move(_next))
    {}
...
public:
    struct FilterLCHead : LCHead
    {
        ...
        FilterLCHead(std::unique_ptr<LCHead> _next)
            : next(std::move(_next))
        {}
    ...
    struct FilterLCEntry : LCEntry
    {
        ...
        FilterLCEntry(std::unique_ptr<LCEntry> _next)
            : next(std::move(_next))
        {}
    ...
    FilterLifecycle(std::unique_ptr<Lifecycle> _next)
        : next(std::move(_next))
    {}
...
public:
    FilterNotification(std::unique_ptr<Notification> _next)
        : next(std::move(_next))
    {}
...
public:
    FilterLuaManager(std::unique_ptr<LuaManager> _next)
        : next(std::move(_next))
    {}
