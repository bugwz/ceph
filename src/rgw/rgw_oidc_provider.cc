...
int RGWOIDCProvider::store_url(const DoutPrefixProvider* dpp, const string& url, bool exclusive, optional_yield y)
{
    using ceph::encode;
    ...
    return rgw_put_system_obj(
        dpp, obj_ctx, svc->zone->get_zone_params().oidc_pool, oid, bl, exclusive, NULL, real_time(), y);
}
...
int RGWOIDCProvider::create(const DoutPrefixProvider* dpp, bool exclusive, optional_yield y)
{
    ...
    if (ret < 0) {
        ldpp_dout(dpp, 0) << "ERROR:  storing role info in pool: " << pool.name << ": " << provider_url << ": "
...
int RGWOIDCProvider::get_providers(const DoutPrefixProvider* dpp, RGWRados* store, const string& tenant,
