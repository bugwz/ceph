...
public:
    ...
    virtual size_t send_header(const std::string_view& name, const std::string_view& value) = 0;
...
template<typename DecorateeT> class DecoratedRestfulClient : public RestfulClient
{
    template<typename T> friend class DecoratedRestfulClient;
    friend RGWRestfulIO;
...
protected:
    ...
    int init_env(CephContext* cct) override { return get_decoratee().init_env(cct); }
...
public:
    ...
    size_t send_100_continue() override { return get_decoratee().send_100_continue(); }
    ...
    size_t send_content_length(const uint64_t len) override { return get_decoratee().send_content_length(len); }

    size_t send_chunked_transfer_encoding() override { return get_decoratee().send_chunked_transfer_encoding(); }

    size_t complete_header() override { return get_decoratee().complete_header(); }

    size_t recv_body(char* const buf, const size_t max) override { return get_decoratee().recv_body(buf, max); }

    size_t send_body(const char* const buf, const size_t len) override { return get_decoratee().send_body(buf, len); }

    void flush() override { return get_decoratee().flush(); }

    RGWEnv& get_env() noexcept override { return get_decoratee().get_env(); }

    size_t complete_request() override { return get_decoratee().complete_request(); }
...
template<size_t BufferSizeV = 4096> class StaticOutputBufferer : public std::streambuf
{
    ...
    int sync() override
    {
        const auto len = static_cast<size_t>(std::streambuf::pptr() - std::streambuf::pbase());
