...
rgw::sal::Driver* DriverManager::init_storage_provider(const DoutPrefixProvider* dpp, CephContext* cct,
                                                       const Config& cfg, bool use_gc_thread, bool use_lc_thread,
                                                       ...
                                                       bool run_reshard_thread, bool use_cache, bool use_gc)
{
    ...
    if (cfg.store_name.compare("rados") == 0) {
        ...
        lsubdout(cct, rgw, 1) << "rgw_d3n: rgw_d3n_l1_datacache_size=" << cct->_conf->rgw_d3n_l1_datacache_size
                              << dendl;
        ...
        lsubdout(cct, rgw, 1) << "rgw_d3n: rgw_d3n_l1_eviction_policy=" << cct->_conf->rgw_d3n_l1_eviction_policy
                              << dendl;
    }
#ifdef WITH_RADOSGW_DBSTORE
        ...
        if ((*(rgw::sal::DBStore*)driver).set_run_lc_thread(use_lc_thread).initialize(cct, dpp) < 0) {
            delete driver;
            return nullptr;
        }
    }
    ...
    if (config_store == "rados") {
        ...
        if (!admin && d3n) {
            if (g_conf().get_val<Option::size_t>("rgw_max_chunk_size") !=
                ...
                lsubdout(cct, rgw_datacache, 0) << "rgw_d3n:  WARNING: D3N DataCache disabling (D3N requires that the "
                                                   "chunk_size equals stripe_size)"
                                                << dendl;
            }
            else if (!g_conf().get_val<bool>("rgw_beast_enable_async")) {
                lsubdout(cct, rgw_datacache, 0) << "rgw_d3n:  WARNING: D3N DataCache disabling (D3N requires yield "
...
auto DriverManager::create_config_store(const DoutPrefixProvider* dpp, std::string_view type)
    -> std::unique_ptr<rgw::sal::ConfigStore>
{
    try {
        if (type == "rados") {
            ...
            ldpp_dout(dpp, -1) << "ERROR: unrecognized config store type '" << type << "'" << dendl;
            return nullptr;
        }
    }
    catch (const std::exception& e) {
        ldpp_dout(dpp, -1) << "ERROR: failed to initialize config store '" << type << "': " << e.what() << dendl;
    }
    return nullptr;
}
