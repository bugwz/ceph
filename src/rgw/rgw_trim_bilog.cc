...
public:
    BucketTrimWatcher(rgw::sal::RGWRadosStore* store, const rgw_raw_obj& obj, TrimCounters::Server* counters)
        : store(store)
        , obj(obj)
    {
...
template<typename Iter> int take_min_status(CephContext* cct, Iter first, Iter last, std::vector<std::string>* status)
{
...
public:
    BucketTrimShardCollectCR(const DoutPrefixProvider* dpp, rgw::sal::RGWRadosStore* store,
                             const RGWBucketInfo& bucket_info, const std::vector<std::string>& markers)
    ...
    while (i < markers.size()) {
        ...
        if (!marker.empty()) {
            ldpp_dout(dpp, 10) << "trimming bilog shard " << shard_id << " of " << bucket_info.bucket << " at marker "
                               << marker << dendl;
            spawn(new RGWRadosBILogTrimCR(dpp, store, bucket_info, shard_id, std::string{}, marker), false);
            return true;
        }
    }
    return false;
...
public:
    BucketTrimInstanceCR(rgw::sal::RGWRadosStore* store, RGWHTTPManager* http, BucketTrimObserver* observer,
...
int BucketTrimInstanceCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        retcode = take_min_status(cct, peer_status.begin(), peer_status.end(), &min_markers);
        if (retcode < 0) {
...
public:
    BucketTrimInstanceCollectCR(rgw::sal::RGWRadosStore* store, RGWHTTPManager* http, BucketTrimObserver* observer,
...
public:
    BucketTrimCR(rgw::sal::RGWRadosStore* store, RGWHTTPManager* http, const BucketTrimConfig& config,
...
int BucketTrimCR::operate(const DoutPrefixProvider* dpp)
{
    reenter(this)
    {
        ...
        if (buckets.size() < config.buckets_per_interval) {
            ...
            yield call(new ReadStatus(
                dpp, store->svc()->rados->get_async_processor(), store->svc()->sysobj, obj, &status, true, &objv));
            if (retcode < 0) {
        ...
        if (!last_cold_marker.empty() && status.marker != last_cold_marker) {
            ...
            yield call(new WriteStatus(
                dpp, store->svc()->rados->get_async_processor(), store->svc()->sysobj, obj, status, &objv));
            if (retcode < 0) {
...
public:
    BucketTrimPollCR(rgw::sal::RGWRadosStore* store, RGWHTTPManager* http, const BucketTrimConfig& config,
                     BucketTrimObserver* observer, const rgw_raw_obj& obj, const DoutPrefixProvider* dpp)
        : RGWCoroutine(store->ctx())
        , store(store)
        , http(http)
...
BucketTrimManager::BucketTrimManager(rgw::sal::RGWRadosStore* store, const BucketTrimConfig& config)
