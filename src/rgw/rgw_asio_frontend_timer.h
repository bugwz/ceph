...
template<typename Stream> struct timeout_handler
{
...
template<typename Clock, typename Executor, typename Stream> class basic_timeout_timer
{
public:
    ...
    explicit basic_timeout_timer(const executor_type& ex, duration dur, boost::intrusive_ptr<Stream> stream)
        : timer(ex)
        , dur(dur)
        , stream(std::move(stream))
    {}
...
private:
    using Timer = boost::asio::basic_waitable_timer<clock_type, boost::asio::wait_traits<clock_type>, executor_type>;
    Timer timer;
    duration dur;
    boost::intrusive_ptr<Stream> stream;
};
