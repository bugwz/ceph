...
{   // cls_rgw_obj_key now aliases this type
    ...
    std::string to_string() const { return fmt::format("{}({})", name, instance); }
    ...
    bool operator<=(const rgw_obj_index_key& k) const { return !(k < *this); }
    ...
    size_t estimate_encoded_size() const
    {
        ...
        return start_overhead + string_overhead + name.size() + string_overhead + instance.size();
    }
};
...
struct rgw_obj_key
{
    ...
    void set_instance(const std::string& i) { instance = i; }

    const std::string& get_instance() const { return instance; }

    void set_ns(const std::string& _ns) { ns = _ns; }

    const std::string& get_ns() const { return ns; }
    ...
    bool have_null_instance() const { return instance == "null"; }

    bool have_instance() const { return !instance.empty(); }

    bool need_to_encode_instance() const { return have_instance() && !have_null_instance(); }
    ...
    bool operator<=(const rgw_obj_key& k) const { return !(k < *this); }
...
{
    template<typename FormatContext> auto format(const rgw_obj_key& key, FormatContext& ctx) const
    {
...
struct rgw_raw_obj
{
    ...
    rgw_raw_obj() {}
    rgw_raw_obj(const rgw_pool& _pool, const std::string& _oid) { init(_pool, _oid); }
    rgw_raw_obj(const rgw_pool& _pool, const std::string& _oid, const std::string& _loc)
        : loc(_loc)
    {
        init(_pool, _oid);
    }
    ...
    bool empty() const { return oid.empty(); }
    ...
    bool operator==(const rgw_raw_obj& o) const { return (pool == o.pool && oid == o.oid && loc == o.loc); }
...
struct rgw_obj
{
    ...
    void set_key(const rgw_obj_key& k) { key = k; }

    std::string get_oid() const { return key.get_oid(); }

    const std::string& get_hash_object() const { return index_hash_source.empty() ? key.name : index_hash_source; }
    ...
    void decode(bufferlist::const_iterator& bl)
    {
        ...
        if (struct_v < 6) {
            ...
            if (struct_v >= 4) decode(key.instance, bl);
    ...
    bool operator==(const rgw_obj& o) const { return (key == o.key) && (bucket == o.bucket); }
