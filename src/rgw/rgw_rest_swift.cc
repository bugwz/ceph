...
static void dump_account_metadata(struct req_state* const s, const RGWUsageStats& global_stats,
                                  const std::map<std::string, RGWUsageStats>& policies_stats,
                                  /* const */ map<string, bufferlist>& attrs, const RGWQuotaInfo& quota,
                                  const RGWAccessControlPolicy_SWIFTAcct& policy)
{
    ...
    for (const auto& kv : policies_stats) {
        ...
        dump_header_infixed(
            s, "X-Account-Storage-Policy-", policy_name, "-Container-Count", policy_stats.buckets_count);
        dump_header_infixed(s, "X-Account-Storage-Policy-", policy_name, "-Object-Count", policy_stats.objects_count);
        dump_header_infixed(s, "X-Account-Storage-Policy-", policy_name, "-Bytes-Used", policy_stats.bytes_used);
        dump_header_infixed(
            s, "X-Account-Storage-Policy-", policy_name, "-Bytes-Used-Actual", policy_stats.bytes_used_rounded);
    }
    ...
    for (iter = attrs.lower_bound(RGW_ATTR_PREFIX); iter != attrs.end(); ++iter) {
        ...
        if (geniter != rgw_to_http_attrs.end()) {
            ...
            dump_header_prefixed(s, "X-Account-Meta-", camelcase_dash_http_attr(name + PREFIX_LEN), iter->second);
        }
    }
...
void RGWListBuckets_ObjStore_SWIFT::send_response_data(rgw::sal::RGWBucketList& buckets)
{
    ...
    for (auto iter = m.lower_bound(prefix); iter != m.end() && boost::algorithm::starts_with(iter->first, prefix);
         ++iter) {
...
void RGWListBuckets_ObjStore_SWIFT::send_response_data_reversed(rgw::sal::RGWBucketList& buckets)
{
    ...
    auto iter = m.rbegin();
    for (/* initialized above */; iter != m.rend() && !boost::algorithm::starts_with(iter->first, prefix); ++iter) {
        /* NOP */;
    }

    for (/* iter carried */; iter != m.rend() && boost::algorithm::starts_with(iter->first, prefix); ++iter) {
        dump_bucket_entry(*iter->second);
    }
}
...
int RGWListBucket_ObjStore_SWIFT::get_params(optional_yield y)
{
    ...
    if (max > default_max) return -ERR_PRECONDITION_FAILED;
    ...
    if (s->info.args.exists("path")) {   // should handle empty path
        ...
        path = prefix;
        if (path.size() && path[path.size() - 1] != '/') path.append("/");

        int len = prefix.size();
        int delim_size = delimiter.size();

        if (len >= delim_size) {
            if (prefix.substr(len - delim_size).compare(delimiter) != 0) prefix.append(delimiter);
        }
    }
...
static void dump_container_metadata(struct req_state*, const rgw::sal::RGWBucket*, const RGWQuotaInfo&,
...
void RGWListBucket_ObjStore_SWIFT::send_response()
{
    ...
    dump_start(s);
    dump_container_metadata(s, s->bucket.get(), bucket_quota, s->bucket->get_info().website_conf);

    s->formatter->open_array_section_with_attrs("container",
                                                FormatterAttrs("name", s->bucket->get_name().c_str(), NULL));
    ...
    while (iter != objs.end() || pref_iter != common_prefixes.end()) {
        ...
        if (do_objs && (allow_unordered || marker.empty() || marker < key)) {
            if (key.name.compare(path) == 0) goto next;
            ...
            if (!iter->meta.user_data.empty()) s->formatter->dump_string("user_custom_data", iter->meta.user_data);
            ...
            if (iter->meta.content_type.size()) {
                ...
                if (pos > 0) {
                    ++pos;
                    while (single_content_type[pos] == ' ') ++pos;
        ...
        if (do_pref && (marker.empty() || pref_iter->first.compare(marker.name) > 0)) {
            ...
            if (name.compare(delimiter) == 0) goto next;

            s->formatter->open_object_section_with_attrs("subdir", FormatterAttrs("name", name.c_str(), NULL));

            /* swift is a bit inconsistent here */
            switch (s->format) {
            case RGW_FORMAT_XML: s->formatter->dump_string("name", name); break;
            default: s->formatter->dump_string("subdir", name);
            }
            s->formatter->close_section();
        }
    next:
...
static void dump_container_metadata(struct req_state* s, const rgw::sal::RGWBucket* bucket, const RGWQuotaInfo& quota,
                                    const RGWBucketWebsiteConf& ws_conf)
{
    ...
    if (rgw::sal::RGWObject::empty(s->object.get())) {
        auto swift_policy = static_cast<RGWAccessControlPolicy_SWIFT*>(s->bucket_acl.get());
        ...
        const size_t PREFIX_LEN = sizeof(RGW_ATTR_META_PREFIX) - 1;
        map<string, bufferlist>::iterator iter;
        for (iter = s->bucket_attrs.lower_bound(RGW_ATTR_PREFIX); iter != s->bucket_attrs.end(); ++iter) {
            const char* name = iter->first.c_str();
            map<string, string>::const_iterator geniter = rgw_to_http_attrs.find(name);
            ...
            if (geniter != rgw_to_http_attrs.end()) {
                ...
                dump_header_prefixed(s, "X-Container-Meta-", camelcase_dash_http_attr(name + PREFIX_LEN), iter->second);
            }
        }
    }
    ...
    if (!s->bucket->get_info().swift_ver_location.empty()) {
        dump_header(s, "X-Versions-Location", url_encode(s->bucket->get_info().swift_ver_location));
    }
...
void RGWStatBucket_ObjStore_SWIFT::send_response()
{
    if (op_ret >= 0) {
        op_ret = STATUS_NO_CONTENT;
        dump_container_metadata(s, bucket.get(), bucket_quota, s->bucket->get_info().website_conf);
    }
...
static int get_swift_container_settings(req_state* const s, rgw::sal::RGWRadosStore* const store,
                                        RGWAccessControlPolicy* const policy, bool* const has_policy, uint32_t* rw_mask,
                                        RGWCORSConfiguration* const cors_config, bool* const has_cors)
{
    ...
    *has_policy = false;
    ...
    if (read_list || write_list) {
        ...
        const auto r = swift_policy.create(
            s, store->ctl()->user, s->user->get_id(), s->user->get_display_name(), read_list, write_list, *rw_mask);
        if (r < 0) {
            return r;
        }
...
static void get_rmattrs_from_headers(const req_state* const s, const char* const put_prefix,
                                     const char* const del_prefix, set<string>& rmattr_names)
{
    ...
    for (const auto& kv : s->info.env->get_map()) {
        ...
        else if ((strncasecmp(p, put_prefix, put_prefix_len) == 0) && kv.second.empty()) {
...
static int get_swift_versioning_settings(req_state* const s, boost::optional<std::string>& swift_ver_location)
{
    ...
    const std::string vlocdel = s->info.env->get("HTTP_X_REMOVE_VERSIONS_LOCATION", "");
    if (vlocdel.size()) {
...
int RGWCreateBucket_ObjStore_SWIFT::get_params(optional_yield y)
{
    ...
    int r = get_swift_container_settings(s, store, &policy, &has_policy, &policy_rw_mask, &cors_config, &has_cors);
    if (r < 0) {
        return r;
    }
...
static inline int handle_metadata_errors(req_state* const s, const int op_ret)
{
    if (op_ret == -EFBIG) {
        ...
        const auto error_message = boost::str(boost::format("Metadata value longer than %lld") %
                                              s->cct->_conf.get_val<Option::size_t>("rgw_max_attr_size"));
        set_req_state_err(s, EINVAL, error_message);
        return -EINVAL;
    }
    else if (op_ret == -E2BIG) {
        const auto error_message = boost::str(boost::format("Too many metadata items; max %lld") %
...
int RGWPutObj_ObjStore_SWIFT::update_slo_segment_size(rgw_slo_entry& entry)
{
    ...
    if (bucket_name.compare(s->bucket->get_name()) != 0) {
        ...
        r = store->getRados()->get_bucket_info(
            store->svc(), s->user->get_id().tenant, bucket_name, bucket_info, nullptr, s->yield, s, &bucket_attrs);
        if (r < 0) {
     ...
     */
    if (entry.size_bytes && (entry.size_bytes != size_bytes)) {
        return -EINVAL;
    }
...
#define MAX_SLO_ENTRY_SIZE (1024 + 128)   // 1024 - max obj name, 128 - enough extra for other info
        ...
        for (auto& entry : slo_info->entries) {
            ...
            ldpp_dout(this, 20) << "slo_part: " << entry.path << " size=" << entry.size_bytes << " etag=" << entry.etag
                                << dendl;
        }
...
static int get_swift_account_settings(req_state* const s, rgw::sal::RGWRadosStore* const store,
                                      RGWAccessControlPolicy_SWIFTAcct* const policy, bool* const has_policy)
{
    *has_policy = false;
    ...
    if (acl_attr) {
        ...
        const bool r = swift_acct_policy.create(
            s, store->ctl()->user, s->user->get_id(), s->user->get_display_name(), string(acl_attr));
        if (r != true) {
            return -EINVAL;
        }
...
static void bulkdelete_respond(const unsigned num_deleted, const unsigned int num_unfound,
                               const std::list<RGWBulkDelete::fail_desc_t>& failures, const int prot_flags, /* in  */
...
static void get_contype_from_attrs(map<string, bufferlist>& attrs, string& content_type)
{
                                 ...
                                 const map<string, bufferlist>& attrs)
{
    ...
    for (auto kv : attrs) {
        ...
        if (aiter != std::end(rgw_to_http_attrs)) {
            ...
            dump_header_prefixed(s, "X-Object-Meta-", camelcase_dash_http_attr(name), kv.second);
        }
    }
    ...
    if (cditer != std::end(rgw_to_http_attrs)) {
        ...
        else if (!s->content_disp.fallback.empty() && response_attrs.find(name) == std::end(response_attrs)) {
...
{
    if (!sent_header) {
        if (!op_ret) op_ret = STATUS_CREATED;
        set_req_state_err(s, op_ret);
        dump_errno(s);
        end_header(s, this);
...
{
    /* Dump X-Copied-From. */
    dump_header(s, "X-Copied-From", url_encode(src_bucket->get_name()) + "/" + url_encode(src_object->get_name()));
...
int RGWGetObj_ObjStore_SWIFT::send_response_data(bufferlist& bl, const off_t bl_ofs, const off_t bl_len)
{
    string content_type;
...
int RGWBulkDelete_ObjStore_SWIFT::get_data(list<RGWBulkDelete::acct_path_t>& items, bool* const is_truncated)
{
    constexpr size_t MAX_LINE_SIZE = 2048;

    RGWClientIOStreamBuf ciosb(static_cast<RGWRestfulIO&>(*(s->cio)), size_t(s->cct->_conf->rgw_max_chunk_size));
    istream cioin(&ciosb);
    ...
    while (cioin.getline(buf, sizeof(buf))) {
        ...
        if (string::npos != start_pos) {
            ...
            if (string::npos != sep_pos) {
                path.bucket_name = url_decode(path_str.substr(start_pos, sep_pos - start_pos));
...
void RGWBulkDelete_ObjStore_SWIFT::send_response()
{
    ...
    bulkdelete_respond(
        deleter->get_num_deleted(), deleter->get_num_unfound(), deleter->get_failures(), s->prot_flags, *s->formatter);
    rgw_flush_formatter_and_reset(s, s->formatter);
}


std::unique_ptr<RGWBulkUploadOp::StreamGetter> RGWBulkUploadOp_ObjStore_SWIFT::create_stream()
{
    ...
    public:
        ...
        ssize_t get_at_most(size_t want, ceph::bufferlist& dst) override
        {
            ...
            /* RadosGW's limit. */
            const size_t max_chunk_size = static_cast<size_t>(s->cct->_conf->rgw_max_chunk_size);
            const size_t max_to_read = std::min({want, conlen - curpos, max_chunk_size});

            ldpp_dout(dpp, 20) << "bulk_upload: get_at_most max_to_read=" << max_to_read
            ...
            curpos += read_len;
            return curpos > s->cct->_conf->rgw_max_put_size ? -ERR_TOO_LARGE : read_len;
        }
...
void RGWInfo_ObjStore_SWIFT::execute(optional_yield y)
{
    ...
    if (!swiftinfo_sig.empty() && !swiftinfo_expires.empty() && !is_expired(swiftinfo_expires, this)) {
    ...
    for (const auto& pair : swift_info) {
        if (!is_admin_info_enabled && pair.second.is_admin_info) continue;
...
void RGWInfo_ObjStore_SWIFT::list_swift_data(Formatter& formatter, const ConfigProxy& config, RGWRados& store)
{
    ...
    const size_t max_attr_name_len = g_conf().get_val<Option::size_t>("rgw_max_attr_name_len");
    if (max_attr_name_len) {
        const size_t meta_name_limit = max_attr_name_len - strlen(RGW_ATTR_PREFIX RGW_AMZ_META_PREFIX);
    ...
    const size_t meta_num_limit = g_conf().get_val<uint64_t>("rgw_max_attrs_num_in_req");
    if (meta_num_limit) {
...
void RGWInfo_ObjStore_SWIFT::list_tempauth_data(Formatter& formatter, const ConfigProxy& config, RGWRados& store)
{
    formatter.open_object_section("tempauth");
    formatter.dump_bool("account_acls", true);
    formatter.close_section();
}
void RGWInfo_ObjStore_SWIFT::list_tempurl_data(Formatter& formatter, const ConfigProxy& config, RGWRados& store)
{
...
void RGWInfo_ObjStore_SWIFT::list_slo_data(Formatter& formatter, const ConfigProxy& config, RGWRados& store)
{
...
bool RGWInfo_ObjStore_SWIFT::is_expired(const std::string& expires, const DoutPrefixProvider* dpp)
{
    ...
    const uint64_t expiration = (uint64_t)strict_strtoll(expires.c_str(), 10, &err);
    if (!err.empty()) {
...
void RGWFormPost::init(rgw::sal::RGWRadosStore* const store, req_state* const s, RGWHandler* const dialect_handler)
{
...
std::size_t RGWFormPost::get_max_file_size() /*const*/
{
    ...
    std::string err;
    const std::size_t max_file_size = static_cast<uint64_t>(strict_strtoll(max_str.c_str(), 10, &err));
...
bool RGWFormPost::is_non_expired()
{
    ...
    std::string err;
    const uint64_t expires_timestamp = static_cast<uint64_t>(strict_strtoll(expires.c_str(), 10, &err));
...
void RGWFormPost::get_owner_info(const req_state* const s, RGWUserInfo& owner_info) const
{
    ...
    RGWBucketInfo bucket_info;
    int ret =
        store->getRados()->get_bucket_info(store->svc(), bucket_tenant, bucket_name, bucket_info, nullptr, s->yield, s);
    if (ret < 0) {
        throw ret;
    }

    ldpp_dout(this, 20) << "temp url user (bucket owner): " << bucket_info.owner << dendl;
...
int RGWFormPost::get_params(optional_yield y)
{
    ...
    do {
        ...
        if (std::end(part.fields) != field_iter &&
            ...
            bool boundary;
            ret = read_data(part.data, s->cct->_conf->rgw_max_chunk_size, boundary, stream_done);
            if (ret < 0) {
                return ret;
            }
            else if (!boundary) {
...
int RGWFormPost::get_data(ceph::bufferlist& bl, bool& again)
{
    ...
    int r = read_data(bl, s->cct->_conf->rgw_max_chunk_size, boundary, stream_done);
    if (r < 0) {
        return r;
    }
...
bool RGWFormPost::is_formpost_req(req_state* const s)
{
    ...
    parse_boundary_params(s->info.env->get("CONTENT_TYPE", ""), content_type, params);

    return boost::algorithm::iequals(content_type, "multipart/form-data") && params.count("boundary") > 0;
}
...
int RGWSwiftWebsiteHandler::serve_errordoc(const int http_ret, const std::string error_doc, optional_yield y)
{
    ...
    {
    public:
        RGWGetErrorPage(rgw::sal::RGWRadosStore* const store, RGWHandler_REST* const handler, req_state* const s,
                        const int http_ret)
        {
        ...
        int error_handler(const int err_no, std::string* const error_content, optional_yield y) override
        {
...
int RGWSwiftWebsiteHandler::error_handler(const int err_no, std::string* const error_content, optional_yield y)
{
    if (!s->bucket.get()) {
...
RGWOp* RGWSwiftWebsiteHandler::get_ws_redirect_op()
{
    ...
    public:
        ...
        const char* name() const override { return "RGWMovedPermanently"; }
    };
...
RGWOp* RGWSwiftWebsiteHandler::get_ws_index_op()
{
    ...
    if (!s->object->empty()) {
        s->object->set_name(s->object->get_name() + s->bucket->get_info().website_conf.get_index_doc());
    }
    else {
...
RGWOp* RGWSwiftWebsiteHandler::get_ws_listing_op()
{
    class RGWWebsiteListing : public RGWListBucket_ObjStore_SWIFT
    {
        ...
        void send_response() override
        {
            ...
            const auto& ws_conf = s->bucket->get_info().website_conf;
            htmler.generate_header(s->decoded_uri, ws_conf.listing_css_doc);
...
bool RGWSwiftWebsiteHandler::is_web_dir() const
{
    ...
    const auto& ws_conf = s->bucket->get_info().website_conf;
    const std::string subdir_marker = ws_conf.subdir_marker.empty() ? "application/directory" : ws_conf.subdir_marker;
...
int RGWHandler_REST_SWIFT::postauth_init(optional_yield y)
{
    ...
    if (g_conf()->rgw_swift_account_in_url && s->user->get_id().id == RGW_USER_ANON_ID) {
    ...
    ldpp_dout(s, 10) << "s->object=" << (!s->object->empty() ? s->object->get_key() : rgw_obj_key("<NULL>"))
...
int RGWHandler_REST_SWIFT::validate_bucket_name(const string& bucket)
{
    ...
    if (len > MAX_BUCKET_NAME_LEN) {
        ...
        const auto msg = boost::str(boost::format("Container name length of %lld longer than %lld") % len %
                                    int(MAX_BUCKET_NAME_LEN));
    ...
    if (bucket[0] == '.') return -ERR_INVALID_BUCKET_NAME;

    if (check_utf8(bucket.c_str(), len)) return -ERR_INVALID_UTF8;

    const char* s = bucket.c_str();

    for (size_t i = 0; i < len; ++i, ++s) {
        if (*(unsigned char*)s == 0xff) return -ERR_INVALID_BUCKET_NAME;
        if (*(unsigned char*)s == '/') return -ERR_INVALID_BUCKET_NAME;
    }
...
int RGWHandler_REST_SWIFT::init_from_header(rgw::sal::RGWRadosStore* store, struct req_state* const s,
                                            const std::string& frontend_prefix)
{
    ...
    if (g_conf()->rgw_swift_url_prefix == "/") {
        ...
        blen = sprintf(buf, "/%s/v1%s", g_conf()->rgw_swift_url_prefix.c_str(), tenant_path.c_str());
    }
...
int RGWHandler_REST_SWIFT::init(rgw::sal::RGWRadosStore* store, struct req_state* s, rgw::io::BasicClient* cio)
{
    struct req_init_state* t = &s->init_state;
    ...
    if (!copy_source.empty()) {
        ...
        if (!result) return -ERR_BAD_URL;
        s->src_object = store->get_object(key);
        if (!s->src_object) return -ERR_BAD_URL;
    }

    if (s->op == OP_COPY) {
        std::string req_dest = s->info.env->get("HTTP_DESTINATION", "");
        if (req_dest.empty()) return -ERR_BAD_URL;

        std::string dest_bucket_name;
        rgw_obj_key dest_obj_key;
        bool result = RGWCopyObj::parse_copy_location(req_dest, dest_bucket_name, dest_obj_key, s);
        if (!result) return -ERR_BAD_URL;
...
RGWHandler_REST* RGWRESTMgr_SWIFT::get_handler(rgw::sal::RGWRadosStore* store, struct req_state* const s,
...
RGWHandler_REST* RGWRESTMgr_SWIFT_Info::get_handler(rgw::sal::RGWRadosStore* store, struct req_state* const s,
                                                    const rgw::auth::StrategyRegistry& auth_registry,
                                                    const std::string& frontend_prefix)
{
    s->prot_flags |= RGW_REST_SWIFT;
