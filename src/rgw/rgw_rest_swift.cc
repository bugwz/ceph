...
static void dump_account_metadata(req_state* const s, const RGWUsageStats& global_stats,
...
static void dump_container_metadata(req_state*, const rgw::sal::Bucket*, const RGWQuotaInfo&,
...
void RGWListBucket_ObjStore_SWIFT::send_response()
{
    ...
    dump_start(s);
    dump_container_metadata(s, s->bucket.get(), quota.bucket_quota, s->bucket->get_info().website_conf);
    ...
    while (iter != objs.end() || pref_iter != common_prefixes.end()) {
        ...
        if (do_pref && (marker.empty() || pref_iter->first.compare(marker.name) > 0)) {
            ...
            switch (s->format) {
            case RGWFormat::XML: s->formatter->dump_string("name", name); break;
...
static void dump_container_metadata(req_state* s, const rgw::sal::Bucket* bucket, const RGWQuotaInfo& quota,
...
void RGWStatBucket_ObjStore_SWIFT::send_response()
{
    if (op_ret >= 0) {
        op_ret = STATUS_NO_CONTENT;
        dump_container_metadata(s, bucket.get(), quota.bucket_quota, s->bucket->get_info().website_conf);
    }
...
static int get_swift_container_settings(req_state* const s, rgw::sal::Driver* const driver,
    ...
    *has_policy = false;
    ...
    if (read_list || write_list) {
        ...
        const auto r = swift_policy.create(
            s, driver, s->user->get_id(), s->user->get_display_name(), read_list, write_list, *rw_mask);
        if (r < 0) {
            return r;
        }
...
int RGWCreateBucket_ObjStore_SWIFT::get_params(optional_yield y)
{
    ...
    int r = get_swift_container_settings(s, driver, &policy, &has_policy, &policy_rw_mask, &cors_config, &has_cors);
    if (r < 0) {
        return r;
    }
...
static int get_swift_account_settings(req_state* const s, rgw::sal::Driver* const driver,
                                      RGWAccessControlPolicy_SWIFTAcct* const policy, bool* const has_policy)
{
    *has_policy = false;
    ...
    if (acl_attr) {
        ...
        const bool r =
            swift_acct_policy.create(s, driver, s->user->get_id(), s->user->get_display_name(), string(acl_attr));
        if (r != true) {
            return -EINVAL;
        }
...
{
    /* Dump X-Copied-From. */
    dump_header(s, "X-Copied-From", url_encode(src_bucket->get_name()) + "/" + url_encode(s->src_object->get_name()));
...
void RGWInfo_ObjStore_SWIFT::list_swift_data(Formatter& formatter, const ConfigProxy& config, rgw::sal::Driver* driver)
{
...
void RGWInfo_ObjStore_SWIFT::list_tempauth_data(Formatter& formatter, const ConfigProxy& config,
...
void RGWInfo_ObjStore_SWIFT::list_tempurl_data(Formatter& formatter, const ConfigProxy& config,
                                               rgw::sal::Driver* driver)
{
...
void RGWInfo_ObjStore_SWIFT::list_slo_data(Formatter& formatter, const ConfigProxy& config, rgw::sal::Driver* driver)
{
...
void RGWFormPost::init(rgw::sal::Driver* const driver, req_state* const s, RGWHandler* const dialect_handler)
{
...
int RGWSwiftWebsiteHandler::serve_errordoc(const int http_ret, const std::string error_doc, optional_yield y)
{
    ...
    {
    public:
        RGWGetErrorPage(rgw::sal::Driver* const driver, RGWHandler_REST* const handler, req_state* const s,
                        const int http_ret)
        {
...
int RGWHandler_REST_SWIFT::init_from_header(rgw::sal::Driver* driver, req_state* const s,
...
int RGWHandler_REST_SWIFT::init(rgw::sal::Driver* driver, req_state* s, rgw::io::BasicClient* cio)
{
    struct req_init_state* t = &s->init_state;
...
RGWHandler_REST* RGWRESTMgr_SWIFT::get_handler(rgw::sal::Driver* driver, req_state* const s,
...
RGWHandler_REST* RGWRESTMgr_SWIFT_Info::get_handler(rgw::sal::Driver* driver, req_state* const s,
                                                    const rgw::auth::StrategyRegistry& auth_registry,
                                                    const std::string& frontend_prefix)
{
    s->prot_flags |= RGW_REST_SWIFT;
