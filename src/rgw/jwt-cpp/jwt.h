...
inline std::string extract_pubkey_from_cert(const std::string& certstr, const std::string& pw = "")
{
    ...
    std::unique_ptr<BIO, decltype(&BIO_free_all)> certbio(
        BIO_new_mem_buf(const_cast<char*>(certstr.data()), certstr.size()), BIO_free_all);
#else
...
inline std::shared_ptr<EVP_PKEY> load_public_key_from_string(const std::string& key, const std::string& password = "")
{
    ...
    std::shared_ptr<EVP_PKEY> pkey(PEM_read_bio_PUBKEY(pubkey_bio.get(), nullptr, nullptr, (void*)password.c_str()),
                                   EVP_PKEY_free);
    if (!pkey)
...
inline std::shared_ptr<EVP_PKEY> load_private_key_from_string(const std::string& key, const std::string& password = "")
{
    ...
    std::shared_ptr<EVP_PKEY> pkey(
        ...
        EVP_PKEY_free);
    if (!pkey) throw rsa_exception("failed to load private key: PEM_read_bio_PrivateKey failed");
    return pkey;
}
}   // namespace helper
...
struct none
{
    ...
    {
        if (!signature.empty()) throw signature_verification_exception();
    }
    /// Get algorithm name
...
struct hmacsha
{
    ...
    void verify(const std::string& data, const std::string& signature) const
    {
        try {
            ...
            if (!matched) throw signature_verification_exception();
        }
...
struct rsa
{
    ...
    std::string sign(const std::string& data) const
    {
...
#endif
        if (!ctx) throw signature_generation_exception("failed to create signature: could not create context");
        ...
        if (!EVP_SignUpdate(ctx.get(), data.data(), data.size())) throw signature_generation_exception();
    ...
    void verify(const std::string& data, const std::string& signature) const
    {
        ...
        std::unique_ptr<EVP_MD_CTX, decltype(&EVP_MD_CTX_free)> ctx(EVP_MD_CTX_create(), EVP_MD_CTX_free);
#endif
        if (!ctx) throw signature_verification_exception("failed to verify signature: could not create context");
...
struct ecdsa
{
    ...
    ecdsa(const std::string& public_key, const std::string& private_key, const std::string& public_key_password,
          const std::string& private_key_password, const EVP_MD* (*md)(), const std::string& name, size_t siglen)
        : md(md)
        , alg_name(name)
        , signature_length(siglen)
    {
        if (!public_key.empty()) {
            ...
            pkey.reset(PEM_read_bio_EC_PUBKEY(pubkey_bio.get(), nullptr, nullptr, (void*)public_key_password.c_str()),
                       EC_KEY_free);
            if (!pkey)
        ...
        if (!private_key.empty()) {
            ...
            pkey.reset(PEM_read_bio_ECPrivateKey(
                           privkey_bio.get(), nullptr, nullptr, const_cast<char*>(private_key_password.c_str())),
                       EC_KEY_free);
            if (!pkey) throw rsa_exception("failed to load private key: PEM_read_bio_ECPrivateKey failed");
            size_t keysize = EC_GROUP_get_degree(EC_KEY_get0_group(pkey.get()));
            if (keysize != signature_length * 4 && (signature_length != 132 || keysize != 521))
                throw ecdsa_exception("invalid key size");
        }
        if (!pkey) throw rsa_exception("at least one of public or private key need to be present");

        if (EC_KEY_check_key(pkey.get()) == 0) throw ecdsa_exception("failed to load key: key is invalid");
    }
    /**
     * Sign jwt data
     * \param data The data to sign
    ...
    std::string sign(const std::string& data) const
    {
        ...
        std::unique_ptr<ECDSA_SIG, decltype(&ECDSA_SIG_free)> sig(
            ECDSA_do_sign((const unsigned char*)hash.data(), hash.size(), pkey.get()), ECDSA_SIG_free);
...
#endif
        if (EVP_DigestInit(ctx.get(), md()) == 0) throw signature_generation_exception("EVP_DigestInit failed");
...
struct pss
{
    ...
    std::string sign(const std::string& data) const
    {
        ...
        if (RSA_private_encrypt(
                size, (const unsigned char*)padded.data(), (unsigned char*)res.data(), key.get(), RSA_NO_PADDING) < 0)
    ...
    void verify(const std::string& data, const std::string& signature) const
    {
        ...
        if (!RSA_public_decrypt(signature.size(),
...
public:
    ...
    const std::set<std::string> as_set() const
    {
        ...
        for (auto& e : as_array()) {
            if (!e.is<std::string>()) throw std::bad_cast();
...
public:
    ...
    {
        if (!has_payload_claim(name)) throw std::runtime_error("claim not found");
...
public:
    ...
    {
        if (!has_header_claim(name)) throw std::runtime_error("claim not found");
...
public:
        ...
        : token(token)
    {
        ...
        auto fix_padding = [](std::string& str) {
            switch (str.size() % 4) {
            case 1: str += alphabet::base64url::fill();
#ifdef __has_cpp_attribute
#if __has_cpp_attribute(fallthrough)
                [[fallthrough]];
#endif
#endif
            case 2: str += alphabet::base64url::fill();
#ifdef __has_cpp_attribute
#if __has_cpp_attribute(fallthrough)
                [[fallthrough]];
#endif
#endif
            case 3: str += alphabet::base64url::fill();
#ifdef __has_cpp_attribute
#if __has_cpp_attribute(fallthrough)
                [[fallthrough]];
#endif
        ...
        auto parse_claims = [](const std::string& str) {
            ...
            picojson::value val;
            if (!picojson::parse(val, str).empty()) throw std::runtime_error("Invalid json");
...
public:
     ...
     */
    template<typename T> std::string sign(const T& algo) const
    {
...
template<typename Clock> class verifier
{
    ...
    template<typename T> struct algo : public algo_base
    {
        ...
        explicit algo(T a)
            : alg(a)
        {}
        virtual void verify(const std::string& data, const std::string& sig) override { alg.verify(data, sig); }
    };
...
public:
    ...
    void verify(const decoded_jwt& jwt) const
    {
        ...
        if (algs.count(algo) == 0) throw token_verification_exception("wrong algorithm");
        ...
        auto assert_claim_eq = [](const decoded_jwt& jwt, const std::string& key, const claim& c) {
            ...
            if (jc.get_type() != c.get_type()) throw token_verification_exception("claim " + key + " type mismatch");
        ...
        if (jwt.has_expires_at()) {
            auto leeway = claims.count("exp") == 1 ? std::chrono::system_clock::to_time_t(claims.at("exp").as_date())
                                                   : default_leeway;
            auto exp = jwt.get_expires_at();
            if (time > exp + std::chrono::seconds(leeway)) throw token_verification_exception("token expired");
        }
        if (jwt.has_issued_at()) {
            auto leeway = claims.count("iat") == 1 ? std::chrono::system_clock::to_time_t(claims.at("iat").as_date())
                                                   : default_leeway;
            auto iat = jwt.get_issued_at();
            if (time < iat - std::chrono::seconds(leeway)) throw token_verification_exception("token expired");
        }
        if (jwt.has_not_before()) {
            auto leeway = claims.count("nbf") == 1 ? std::chrono::system_clock::to_time_t(claims.at("nbf").as_date())
                                                   : default_leeway;
            auto nbf = jwt.get_not_before();
            if (time < nbf - std::chrono::seconds(leeway)) throw token_verification_exception("token expired");
        }
        for (auto& c : claims) {
...
{
    std::chrono::system_clock::time_point now() const { return std::chrono::system_clock::now(); }
};
