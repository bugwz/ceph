...
void dump_header(struct req_state* const s, const std::string_view& name, const std::string_view& val)
{
    try {
...
void dump_header(struct req_state* const s, const std::string_view& name, ceph::buffer::list& bl)
{
    return dump_header(s, name, rgw_sanitized_hdrval(bl));
}

void dump_header(struct req_state* const s, const std::string_view& name, const long long val)
{
    char buf[32];
...
void dump_header(struct req_state* const s, const std::string_view& name, const utime_t& ut)
{
    char buf[32];
    const auto len =
...
void dump_etag(struct req_state* const s, const std::string_view& etag, const bool quoted)
{
    if (etag.empty()) {
        return;
    }
...
void dump_owner(struct req_state* s, const rgw_user& id, string& name, const char* section)
{
...
void dump_access_control(struct req_state* s, const char* origin, const char* meth, const char* hdr,
                ...
                bool force_content_type, bool force_no_error)
{
    ...
    if (force_content_type || (!content_type && s->formatter->get_len() != 0) || s->is_err()) {
        switch (s->format) {
        case RGW_FORMAT_XML: ctype = "application/xml"; break;
        case RGW_FORMAT_JSON: ctype = "application/json"; break;
        case RGW_FORMAT_HTML: ctype = "text/html"; break;
...
void abort_early(struct req_state* s, RGWOp* op, int err_no, RGWHandler* handler, optional_yield y)
{
...
void dump_range(struct req_state* const s, const uint64_t ofs, const uint64_t end, const uint64_t total)
{
...
int dump_body(struct req_state* const s, const char* const buf, const size_t len)
{
    try {
...
std::tuple<int, bufferlist> rgw_rest_read_all_input(struct req_state* s, const uint64_t max_len,
...
int RGWListBucketMultiparts_ObjStore::get_params(optional_yield y)
{
    ...
    if (!key_marker.empty()) marker.init(key_marker, upload_id_marker);
...
int RGWHandler_REST::allocate_formatter(struct req_state* s, int default_type, bool configurable)
{
...
int RGWHandler_REST::reallocate_formatter(struct req_state* s, int type)
{
    ...
    case RGW_FORMAT_JSON: s->formatter = new JSONFormatter(false); break;
    case RGW_FORMAT_HTML: s->formatter = new HTMLFormatter(s->prot_flags & RGW_REST_WEBSITE); break;
...
RGWRESTMgr* RGWRESTMgr::get_resource_mgr(struct req_state* const s, const std::string& uri, std::string* const out_uri)
{
    *out_uri = uri;
...
RGWHandler_REST* RGWREST::get_handler(rgw::sal::RGWRadosStore* const store, struct req_state* const s,
