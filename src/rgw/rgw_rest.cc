                                     ...
                                     const set<string>& valid_hostnames_set)
{
    ...
    for (iter = valid_hostnames_set.begin(); iter != valid_hostnames_set.end(); ++iter) {
        size_t pos;
        if (!str_ends_with_nocase(host, *iter, &pos)) continue;
...
static void dump_status(req_state* s, int status, const char* status_name)
{
...
void dump_header(req_state* const s, const std::string_view& name, const std::string_view& val)
{
    try {
...
void dump_header(req_state* const s, const std::string_view& name, ceph::buffer::list& bl)
{
    return dump_header(s, name, rgw_sanitized_hdrval(bl));
}

void dump_header(req_state* const s, const std::string_view& name, const long long val)
{
    char buf[32];
...
void dump_header(req_state* const s, const std::string_view& name, const utime_t& ut)
{
    char buf[32];
    const auto len =
        snprintf(buf, sizeof(buf), "%lld.%05d", static_cast<long long>(ut.sec()), static_cast<int>(ut.usec() / 10));
...
void dump_etag(req_state* const s, const std::string_view& etag, const bool quoted)
{
    if (etag.empty()) {
        return;
    }
...
void dump_bucket_from_state(req_state* s)
{
    if (g_conf()->rgw_expose_bucket && !s->bucket_name.empty()) {
        if (!s->bucket_tenant.empty()) {
            dump_header(s, "Bucket", url_encode(s->bucket_tenant + "/" + s->bucket_name));
        }
        else {
...
static size_t dump_time_header_impl(char (&timestr)[TIME_BUF_SIZE], const real_time t)
{
    const utime_t ut(t);
    ...
    return strftime(timestr, sizeof(timestr), "%a, %d %b %Y %H:%M:%S %Z", tmp);
}
...
void dump_epoch_header(req_state* s, const char* name, real_time t)
{
    utime_t ut(t);
    char buf[65];
    const auto len = snprintf(buf, sizeof(buf), "%lld.%09lld", (long long)ut.sec(), (long long)ut.nsec());
...
void dump_owner(req_state* s, const rgw_user& id, const string& name, const char* section)
{
    if (!section) section = "Owner";
...
void dump_access_control(req_state* s, const char* origin, const char* meth, const char* hdr, const char* exp_hdr,
                         uint32_t max_age)
{
...
void dump_access_control(req_state* s, RGWOp* op)
{
    ...
    if (!op->generate_cors_headers(origin, method, header, exp_header, &max_age)) return;
...
void end_header(req_state* s, RGWOp* op, const char* content_type, const int64_t proposed_content_length,
                bool force_content_type, bool force_no_error)
{
    ...
    if ((!s->is_err()) && s->bucket && (s->bucket->get_info().owner != s->user->get_id()) &&
    ...
    if (force_content_type || (!content_type && s->formatter->get_len() != 0) || s->is_err()) {
        ...
        if (s->prot_flags & RGW_REST_SWIFT) ctype.append("; charset=utf-8");
...
void abort_early(req_state* s, RGWOp* op, int err_no, RGWHandler* handler, optional_yield y)
{
...
void dump_range(req_state* const s, const uint64_t ofs, const uint64_t end, const uint64_t total)
{
    ...
    if (!total) {
        len = snprintf(range_buf, sizeof(range_buf), "bytes */%lld", static_cast<long long>(total));
    }
    else {
        len = snprintf(range_buf,
                       sizeof(range_buf),
                       "bytes %lld-%lld/%lld",
...
int dump_body(req_state* const s, const char* const buf, const size_t len)
{
    bool healthchk = false;
    // we dont want to limit health checks
    if (s->op_type == RGW_OP_GET_HEALTH_CHECK) healthchk = true;
    if (len > 0 && !healthchk) {
...
int recv_body(req_state* const s, char* const buf, const size_t max)
{
    int len;
    try {
...
int RESTArgs::get_string(req_state* s, const string& name, const string& def_val, string* val, bool* existed)
{
    bool exists;
...
int RESTArgs::get_uint64(req_state* s, const string& name, uint64_t def_val, uint64_t* val, bool* existed)
{
    bool exists;
...
int RESTArgs::get_int64(req_state* s, const string& name, int64_t def_val, int64_t* val, bool* existed)
{
    bool exists;
    string sval = s->info.args.get(name, &exists);
...
int RESTArgs::get_uint32(req_state* s, const string& name, uint32_t def_val, uint32_t* val, bool* existed)
{
    bool exists;
    string sval = s->info.args.get(name, &exists);
...
int RESTArgs::get_int32(req_state* s, const string& name, int32_t def_val, int32_t* val, bool* existed)
{
    bool exists;
    string sval = s->info.args.get(name, &exists);
...
int RESTArgs::get_time(req_state* s, const string& name, const utime_t& def_val, utime_t* val, bool* existed)
{
    bool exists;
    string sval = s->info.args.get(name, &exists);
...
int RESTArgs::get_bool(req_state* s, const string& name, bool def_val, bool* val, bool* existed)
{
    ...
    if (sval.empty() || strcasecmp(str, "true") == 0 || sval.compare("1") == 0) {
        *val = true;
        return 0;
    }

    if (strcasecmp(str, "false") != 0 && sval.compare("0") != 0) {
        *val = def_val;
        return -EINVAL;
    }
...
int RGWPutObj_ObjStore::get_data(bufferlist& bl)
{
    ...
    if (s->length) {
        ...
        if (cl > chunk_size) cl = chunk_size;
    }
    else {
        cl = chunk_size;
    }
 ...
 */
void RGWPostObj_ObjStore::parse_boundary_params(const std::string& params_str, std::string& first,
...
int RGWPostObj_ObjStore::parse_part_field(const std::string& line, std::string& field_name, /* out */
 ...
 */
static int index_of(ceph::bufferlist& bl, uint64_t max_len, const std::string& str, const bool check_crlf,
                    bool& reached_boundary, int& skip)
{
    ...
    if (bl.length() < str.size()) return -1;

    const char* buf = bl.c_str();
    const char* s = str.c_str();

    if (max_len > bl.length()) max_len = bl.length();

    for (uint64_t i = 0; i < max_len; i++, buf++) {
        if (check_crlf && i >= 1 && is_crlf(buf - 1)) {
            return i + 1;   // skip the crlf
        }
        if ((i < max_len - str.size() + 1) && (buf[0] == s[0] && buf[1] == s[1]) &&
...
int RGWPostObj_ObjStore::read_with_boundary(ceph::bufferlist& bl, uint64_t max, const bool check_crlf,
                                            bool& reached_boundary, bool& done)
{
    ...
    int skip;
    const int index = index_of(in_data, cl, boundary, check_crlf, reached_boundary, skip);
    if (index >= 0) {
        max = index;
    }
...
int RGWPostObj_ObjStore::read_line(ceph::bufferlist& bl, const uint64_t max, bool& reached_boundary, bool& done)
{
    return read_with_boundary(bl, max, true, reached_boundary, done);
}

int RGWPostObj_ObjStore::read_data(ceph::bufferlist& bl, const uint64_t max, bool& reached_boundary, bool& done)
{
    return read_with_boundary(bl, max, false, reached_boundary, done);
}


int RGWPostObj_ObjStore::read_form_part_header(struct post_form_part* const part, bool& done)
{
    bufferlist bl;
...
bool RGWPostObj_ObjStore::part_str(parts_collection_t& parts, const std::string& name, std::string* val)
{
...
std::string RGWPostObj_ObjStore::get_part_str(parts_collection_t& parts, const std::string& name,
...
bool RGWPostObj_ObjStore::part_bl(parts_collection_t& parts, const std::string& name, ceph::bufferlist* pbl)
{
    const auto iter = parts.find(name);
    if (std::end(parts) == iter) {
        return false;
...
#define MAX_READ_CHUNK (128 * 1024)
    ...
    do {
        ...
        if (read_len == need_to_read) {
            if (need_to_read < MAX_READ_CHUNK) need_to_read *= 2;
...
std::tuple<int, bufferlist> rgw_rest_read_all_input(req_state* s, const uint64_t max_len, const bool allow_chunked)
{
    size_t cl = 0;
    int len = 0;
    bufferlist bl;
    ...
    if (cl) {
        ...
        if (!encoding || strcmp(encoding, "chunked") != 0) return std::make_tuple(-ERR_LENGTH_REQUIRED, std::move(bl));

        int ret = 0;
        std::tie(ret, bl) = read_all_chunked_input(s, max_len);
        if (ret < 0) return std::make_tuple(ret, std::move(bl));
    }
...
int RGWListMultipart_ObjStore::get_params(optional_yield y)
{
    ...
    op_ret = parse_value_and_bound(str, max_parts, 0, g_conf().get_val<uint64_t>("rgw_max_listing_results"), max_parts);
...
int RGWListBucketMultiparts_ObjStore::get_params(optional_yield y)
{
    ...
    op_ret =
        parse_value_and_bound(str, max_uploads, 0, g_conf().get_val<uint64_t>("rgw_max_listing_results"), default_max);
    if (op_ret < 0) {
        return op_ret;
    }

    if (auto encoding_type = s->info.args.get_optional("encoding-type"); encoding_type != boost::none) {
    ...
    if (!key_marker.empty()) {
        ...
        upload = s->bucket->get_multipart_upload(key_marker, upload_id_marker);
...
RGWOp* RGWHandler_REST::get_op(void)
{
    RGWOp* op;
    switch (s->op) {
    case OP_GET: op = op_get(); break;
    case OP_PUT: op = op_put(); break;
    case OP_DELETE: op = op_delete(); break;
    case OP_HEAD: op = op_head(); break;
    case OP_POST: op = op_post(); break;
    case OP_COPY: op = op_copy(); break;
    case OP_OPTIONS: op = op_options(); break;
    default: return NULL;
    }
...
int RGWHandler_REST::allocate_formatter(req_state* s, RGWFormat default_type, bool configurable)
{
...
int RGWHandler_REST::reallocate_formatter(req_state* s, const RGWFormat type)
{
    ...
    const bool swift_bulkupload = s->prot_flags & RGW_REST_SWIFT && s->info.args.exists("extract-archive");
    switch (s->format) {
    case RGWFormat::PLAIN:
    {
        const bool use_kv_syntax = s->info.args.exists("bulk-delete") || multipart_delete || swift_bulkupload;
        s->formatter = new RGWFormatter_Plain(use_kv_syntax);
        break;
    }
    case RGWFormat::XML:
    {
        const bool lowercase_underscore = s->info.args.exists("bulk-delete") || multipart_delete || swift_bulkupload;

        s->formatter = new XMLFormatter(false, lowercase_underscore);
        break;
    }
    case RGWFormat::JSON: s->formatter = new JSONFormatter(false); break;
    case RGWFormat::HTML: s->formatter = new HTMLFormatter(s->prot_flags & RGW_REST_WEBSITE); break;
...
{
    if (!method) return OP_UNKNOWN;
    if (strcmp(method, "GET") == 0) return OP_GET;
    if (strcmp(method, "PUT") == 0) return OP_PUT;
    if (strcmp(method, "DELETE") == 0) return OP_DELETE;
    if (strcmp(method, "HEAD") == 0) return OP_HEAD;
    if (strcmp(method, "POST") == 0) return OP_POST;
    if (strcmp(method, "COPY") == 0) return OP_COPY;
    if (strcmp(method, "OPTIONS") == 0) return OP_OPTIONS;
        ...
        if (!s->user->get_id().empty() && s->auth.identity->get_identity_type() != TYPE_ROLE) {
            try {
                if (auto ret = s->user->read_attrs(s, y); !ret) {
                    auto user_policies =
                        get_iam_user_policy_from_attr(s->cct, s->user->get_attrs(), s->user->get_tenant());
...
int RGWHandler_REST::read_permissions(RGWOp* op_obj, optional_yield y)
{
    ...
    switch (s->op) {
    case OP_HEAD:
    case OP_GET: only_bucket = false; break;
    case OP_PUT:
    case OP_POST:
    case OP_COPY:
        ...
        if (op_obj->get_type() == RGW_OP_CREATE_BUCKET) return 0;
    ...
    case OP_DELETE:
        ...
        break;
    case OP_OPTIONS: only_bucket = true; break;
    default: return -EINVAL;
    }
...
RGWRESTMgr* RGWRESTMgr::get_resource_mgr(req_state* const s, const std::string& uri, std::string* const out_uri)
{
    *out_uri = uri;
    ...
    for (iter = resources_by_size.rbegin(); iter != resources_by_size.rend(); ++iter) {
        ...
        if (uri.compare(0, iter->first, resource) == 0 && (uri.size() == iter->first || uri[iter->first] == '/')) {
    ...
    if (info.host.size()) {
        ...
        if (s3website_enabled) {
            in_hosted_domain_s3website =
                rgw_find_host_in_domains(info.host, &s3website_domain, &s3website_subdomain, hostnames_s3website_set);
        ...
        ldpp_dout(s, 20) << "subdomain=" << subdomain << " domain=" << domain
                         << " in_hosted_domain=" << in_hosted_domain
                         << " in_hosted_domain_s3website=" << in_hosted_domain_s3website << dendl;
        ...
        if (g_conf()->rgw_resolve_cname && !in_hosted_domain && !in_hosted_domain_s3website) {
            ...
            if (found) {
                ...
                if (s3website_enabled && !in_hosted_domain_s3website) {
                    in_hosted_domain_s3website = rgw_find_host_in_domains(
                        cname, &s3website_domain, &s3website_subdomain, hostnames_s3website_set);
                    if (in_hosted_domain_s3website) {
        ...
        if (subdomain.empty() && (domain.empty() || domain != info.host) && !looks_like_ip_address(info.host.c_str()) &&
        ...
        if (in_hosted_domain && !subdomain.empty()) {
            ...
            if (s->info.request_uri[0] != '/') encoded_bucket.append("/");
        ...
        ldpp_dout(s, 20) << "final domain/bucket"
                         << " subdomain=" << subdomain << " domain=" << domain
                         << " in_hosted_domain=" << in_hosted_domain
                         << " in_hosted_domain_s3website=" << in_hosted_domain_s3website
                         << " s->info.domain=" << s->info.domain << " s->info.request_uri=" << s->info.request_uri
                         << dendl;
    }
...
RGWHandler_REST* RGWREST::get_handler(rgw::sal::Driver* const driver, req_state* const s,
                                      const rgw::auth::StrategyRegistry& auth_registry,
                                      const std::string& frontend_prefix, RGWRestfulIO* const rio,
                                      RGWRESTMgr** const pmgr, int* const init_error)
{
