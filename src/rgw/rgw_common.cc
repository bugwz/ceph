...
{
    if (s->format != RGW_FORMAT_HTML) s->formatter->open_object_section("Error");
    ...
    if (s->format != RGW_FORMAT_HTML) s->formatter->close_section();
}
    ...
    for (const auto& kv : env->get_map()) {
        ...
        for (int prefix_num = 0; (prefix = meta_prefixes[prefix_num].str) != NULL; prefix_num++) {
            ...
            if (strncmp(p, prefix, len) == 0) {
                ...
                if (found_bad_meta && strncmp(name, "_META_", name_len) == 0) *found_bad_meta = true;
...
Effect eval_or_pass(const boost::optional<Policy>& policy, const rgw::IAM::Environment& env,
                    boost::optional<const rgw::auth::Identity&> id, const uint64_t op, const ARN& arn,
...
Effect eval_identity_or_session_policies(const vector<Policy>& policies, const rgw::IAM::Environment& env,
                                         boost::optional<const rgw::auth::Identity&> id, const uint64_t op,
                                         const ARN& arn)
{
...
bool verify_user_permission(const DoutPrefixProvider* dpp, perm_state_base* const s,
                            ...
                            const rgw::ARN& res, const uint64_t op)
{
...
bool verify_user_permission(const DoutPrefixProvider* dpp, struct req_state* const s, const rgw::ARN& res,
                            const uint64_t op)
{
...
bool verify_user_permission_no_policy(const DoutPrefixProvider* dpp, struct req_state* const s, const int perm)
{
    perm_state_from_req_state ps(s);
                              ...
                              const vector<Policy>& session_policies, const uint64_t op)
{
    ...
    auto identity_policy_res =
        eval_identity_or_session_policies(identity_policies, s->env, boost::none, op, ARN(bucket));
    if (identity_policy_res == Effect::Deny) return false;

    rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
    auto r = eval_or_pass(bucket_policy, s->env, *s->identity, op, ARN(bucket), princ_type);
    ...
    if (!session_policies.empty()) {
        auto session_policy_res =
            eval_identity_or_session_policies(session_policies, s->env, boost::none, op, ARN(bucket));
...
bool verify_bucket_permission(const DoutPrefixProvider* dpp, struct req_state* const s, const rgw_bucket& bucket,
                              RGWAccessControlPolicy* const user_acl, RGWAccessControlPolicy* const bucket_acl,
...
int verify_bucket_owner_or_policy(struct req_state* const s, const uint64_t op)
{
    auto identity_policy_res =
        eval_identity_or_session_policies(s->iam_user_policies, s->env, boost::none, op, ARN(s->bucket->get_key()));
    if (identity_policy_res == Effect::Deny) {
        return -EACCES;
    }

    rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
    auto e = eval_or_pass(s->iam_policy, s->env, *s->auth.identity, op, ARN(s->bucket->get_key()), princ_type);
    if (e == Effect::Deny) {
        return -EACCES;
    }
    ...
    if (!s->session_policies.empty()) {
        auto session_policy_res =
            eval_identity_or_session_policies(s->session_policies, s->env, boost::none, op, ARN(s->bucket->get_key()));
        if (session_policy_res == Effect::Deny) {
            return -EACCES;
        }
        if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
                              ...
                              const uint64_t op)
{
    ...
    if (!session_policies.empty()) {
        auto session_policy_res =
            eval_identity_or_session_policies(session_policies, s->env, boost::none, op, ARN(obj));
        if (session_policy_res == Effect::Deny) {
            return false;
        }
        if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
...
bool verify_object_permission(const DoutPrefixProvider* dpp, struct req_state* const s, const rgw_obj& obj,
                              RGWAccessControlPolicy* const user_acl, RGWAccessControlPolicy* const bucket_acl,
                              RGWAccessControlPolicy* const object_acl, const boost::optional<Policy>& bucket_policy,
                              const vector<Policy>& identity_policies, const vector<Policy>& session_policies,
                              const uint64_t op)
...
int verify_object_lock(const DoutPrefixProvider* dpp, const rgw::sal::RGWAttrs& attrs, const bool bypass_perm,
