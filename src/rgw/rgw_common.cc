...
rgw_http_errors rgw_http_swift_errors({
    ...
    {ENOTEMPTY,
     {409,
      "There was a conflict when trying "
...
req_info::req_info(CephContext* cct, const class RGWEnv* env)
    : env(env)
{
...
req_state::req_state(CephContext* _cct, const RGWProcessEnv& penv, RGWEnv* e, uint64_t id)
    : cct(_cct)
    , penv(penv)
    , info(_cct, e)
    , id(id)
...
std::ostream& req_state::gen_prefix(std::ostream& out) const
{
    ...
    return out << "req " << id << ' ' << std::setprecision(3) << std::fixed << time_elapsed()   // '0.123s'
                       ...
                       const int prot_flags) /* in  */
{
    ...
    if (prot_flags & RGW_REST_SWIFT) {
        if (search_err(rgw_http_swift_errors, err_no, err.http_ret, err.err_code)) return;
    }

    if (prot_flags & RGW_REST_STS) {
        if (search_err(rgw_http_sts_errors, err_no, err.http_ret, err.err_code)) return;
    }

    if (prot_flags & RGW_REST_IAM) {
        if (search_err(rgw_http_iam_errors, err_no, err.http_ret, err.err_code)) return;
    }

    // Default to searching in s3 errors
    if (search_err(rgw_http_s3_errors, err_no, err.http_ret, err.err_code)) return;
...
{
    if (s->format != RGWFormat::HTML) s->formatter->open_object_section("Error");
    if (!s->err.err_code.empty()) s->formatter->dump_string("Code", s->err.err_code);
    ...
    if (s->format != RGWFormat::HTML) s->formatter->close_section();
}
...
void req_info::init_meta_info(const DoutPrefixProvider* dpp, bool* found_bad_meta)
{
    ...
    for (const auto& kv : env->get_map()) {
        ...
        for (int prefix_num = 0; (prefix = meta_prefixes[prefix_num].str) != NULL; prefix_num++) {
            ...
            if (strncmp(p, prefix, len) == 0) {
                ...
                if (found_bad_meta && strncmp(name, "META_", name_len) == 0) *found_bad_meta = true;

                char name_low[meta_prefixes[0].len + name_len + 1];
                snprintf(name_low,
                         meta_prefixes[0].len - 5 + name_len + 1,
                         "%s%s",
                         meta_prefixes[0].str + 5 /* skip HTTP_ */,
                         name);   // normalize meta prefix
                int j;
    ...
    for (const auto& kv : x_meta_map) {
        ldpp_dout(dpp, 10) << "x>> " << kv.first << ":" << rgw::crypt_sanitize::x_meta_map{kv.first, kv.second}
                           << dendl;
    }
}
...
void rgw_add_amz_meta_header(meta_map_t& x_meta_map, const std::string& k, const std::string& v)
{
...
bool rgw_set_amz_meta_header(meta_map_t& x_meta_map, const std::string& k, const std::string& v,
                             rgw_set_action_if_set a)
{
    auto it{x_meta_map.find(k)};
    bool r{it != x_meta_map.end()};
    switch (a) {
    default: ceph_assert(a == 0);
    case DISCARD: break;
    case APPEND:
        if (r) {
            std::string old{it->second};
...
static bool check_gmt_end(const char* s)
{
    ...
    if ((strncmp(s, "GMT", 3) != 0) && (strncmp(s, "UTC", 3) != 0)) {
        return false;
    }
...
bool parse_iso8601(const char* s, struct tm* t, uint32_t* pns, bool extended_format)
{
    ...
    if (len == 0 || (len == 1 && str[0] == 'Z')) return true;

    if (str[0] != '.' || str[len - 1] != 'Z') return false;
    ...
    uint64_t mul_table[] = {0, 100000000LL, 10000000LL, 1000000LL, 100000LL, 10000LL, 1000LL, 100LL, 10LL, 1};
...
boost::optional<std::pair<std::string_view, std::string_view>> parse_key_value(const std::string_view& in_str,
...
void rgw_to_iso8601(const real_time& t, char* dest, int buf_size)
{
    ...
    if (strftime(buf, sizeof(buf), "%Y-%m-%dT%T", tmp) == 0) return;
 ...
 */
void calc_hmac_sha1(const char* key, int key_len, const char* msg, int msg_len, char* dest)
 ...
 */
void calc_hmac_sha256(const char* key, int key_len, const char* msg, int msg_len, char* dest)
{
...
void RGWHTTPArgs::append(const string& name, const string& val)
{
    ...
    // when sub_resources exclusive by object are added, please remember to update obj_sub_resource in
    // RGWHTTPArgs::exist_obj_excl_sub_resource().
    if ((name.compare("acl") == 0) || (name.compare("cors") == 0) || (name.compare("notification") == 0) ||
        (name.compare("location") == 0) || (name.compare("logging") == 0) || (name.compare("usage") == 0) ||
        (name.compare("lifecycle") == 0) || (name.compare("delete") == 0) || (name.compare("uploads") == 0) ||
        (name.compare("partNumber") == 0) || (name.compare("uploadId") == 0) || (name.compare("versionId") == 0) ||
        (name.compare("start-date") == 0) || (name.compare("end-date") == 0) || (name.compare("versions") == 0) ||
        (name.compare("versioning") == 0) || (name.compare("website") == 0) || (name.compare("requestPayment") == 0) ||
        (name.compare("torrent") == 0) || (name.compare("tagging") == 0) || (name.compare("append") == 0) ||
        (name.compare("position") == 0) || (name.compare("policyStatus") == 0) ||
        (name.compare("publicAccessBlock") == 0)) {
        sub_resources[name] = val;
    }
    else if (name[0] == 'r') {   // root of all evil
        if ((name.compare("response-content-type") == 0) || (name.compare("response-content-language") == 0) ||
            (name.compare("response-expires") == 0) || (name.compare("response-cache-control") == 0) ||
            (name.compare("response-content-disposition") == 0) || (name.compare("response-content-encoding") == 0)) {
            sub_resources[name] = val;
            has_resp_modifier = true;
        }
    }
    else if ((name.compare("subuser") == 0) || (name.compare("key") == 0) || (name.compare("caps") == 0) ||
             (name.compare("index") == 0) || (name.compare("policy") == 0) || (name.compare("quota") == 0) ||
             (name.compare("list") == 0) || (name.compare("object") == 0) || (name.compare("sync") == 0)) {
...
const string& RGWHTTPArgs::get(const string& name, bool* exists) const
{
    ...
    if (exists) *exists = e;
    if (e) return iter->second;
    return empty_str;
}

boost::optional<const std::string&> RGWHTTPArgs::get_optional(const std::string& name) const
{
    bool exists;
...
{
    bool exists = false;
    if ((get_bool(name, val, &exists) < 0) || !exists) {
        *val = def_val;
    }
}
...
struct perm_state_from_req_state : public perm_state_base
{
    ...
    perm_state_from_req_state(req_state* const _s)
        : perm_state_base(_s->cct, _s->env, _s->auth.identity.get(),
                          _s->bucket.get() ? _s->bucket->get_info() : RGWBucketInfo(), _s->perm_mask,
                          _s->defer_to_bucket_acls, _s->bucket_access_conf)
        , s(_s)
    {}
    ...
    const char* get_referer() const override { return s->info.env->get("HTTP_REFERER"); }
};

Effect eval_or_pass(const DoutPrefixProvider* dpp, const boost::optional<Policy>& policy,
                    const rgw::IAM::Environment& env, boost::optional<const rgw::auth::Identity&> id, const uint64_t op,
                    const ARN& resource, boost::optional<rgw::IAM::PolicyPrincipal&> princ_type = boost::none)
{
    if (!policy)
        return Effect::Pass;
    else
...
Effect eval_identity_or_session_policies(const DoutPrefixProvider* dpp, const vector<Policy>& policies,
                                         const rgw::IAM::Environment& env, const uint64_t op, const ARN& arn)
{
...
bool verify_user_permission(const DoutPrefixProvider* dpp, perm_state_base* const s,
                            RGWAccessControlPolicy* const user_acl, const vector<rgw::IAM::Policy>& user_policies,
                            const vector<rgw::IAM::Policy>& session_policies, const rgw::ARN& res, const uint64_t op,
...
bool verify_user_permission_no_policy(const DoutPrefixProvider* dpp, struct perm_state_base* const s,
                                      RGWAccessControlPolicy* const user_acl, const int perm)
{
    if (s->identity->get_identity_type() == TYPE_ROLE) return false;
...
bool verify_user_permission(const DoutPrefixProvider* dpp, req_state* const s, const rgw::ARN& res, const uint64_t op,
                            bool mandatory_policy)
{
    perm_state_from_req_state ps(s);
    return verify_user_permission(
        dpp, &ps, s->user_acl.get(), s->iam_user_policies, s->session_policies, res, op, mandatory_policy);
}

bool verify_user_permission_no_policy(const DoutPrefixProvider* dpp, req_state* const s, const int perm)
{
    perm_state_from_req_state ps(s);
...
{
    if (!s->bucket_info.requester_pays) return true;

    if (s->identity->is_owner_of(s->bucket_info.owner)) return true;
...
bool verify_bucket_permission(const DoutPrefixProvider* dpp, struct perm_state_base* const s, const rgw_bucket& bucket,
                              RGWAccessControlPolicy* const user_acl, RGWAccessControlPolicy* const bucket_acl,
                              const boost::optional<Policy>& bucket_policy, const vector<Policy>& identity_policies,
                              const vector<Policy>& session_policies, const uint64_t op)
{
    if (!verify_requester_payer_permission(s)) return false;

    auto identity_policy_res = eval_identity_or_session_policies(dpp, identity_policies, s->env, op, ARN(bucket));
    if (identity_policy_res == Effect::Deny) return false;

    rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
    if (bucket_policy) {
        ldpp_dout(dpp, 16) << __func__ << ": policy: " << bucket_policy.get() << "resource: " << ARN(bucket) << dendl;
    }
    auto r = eval_or_pass(dpp, bucket_policy, s->env, *s->identity, op, ARN(bucket), princ_type);
    ...
    if (!session_policies.empty()) {
        ...
        if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
            ...
            if (session_policy_res == Effect::Allow && identity_policy_res == Effect::Allow) return true;
        }
        return false;
    }
...
bool verify_bucket_permission(const DoutPrefixProvider* dpp, req_state* const s, const rgw_bucket& bucket,
                              RGWAccessControlPolicy* const user_acl, RGWAccessControlPolicy* const bucket_acl,
                              const boost::optional<Policy>& bucket_policy, const vector<Policy>& user_policies,
                              const vector<Policy>& session_policies, const uint64_t op)
{
    perm_state_from_req_state ps(s);
    return verify_bucket_permission(
        dpp, &ps, bucket, user_acl, bucket_acl, bucket_policy, user_policies, session_policies, op);
}

bool verify_bucket_permission_no_policy(const DoutPrefixProvider* dpp, struct perm_state_base* const s,
                                        RGWAccessControlPolicy* const user_acl,
                                        RGWAccessControlPolicy* const bucket_acl, const int perm)
{
    if (!bucket_acl) return false;

    if ((perm & (int)s->perm_mask) != perm) return false;

    if (bucket_acl->verify_permission(dpp,
                                      *s->identity,
                                      perm,
                                      perm,
                                      s->get_referer(),
                                      s->bucket_access_conf && s->bucket_access_conf->ignore_public_acls()))
        return true;

    if (!user_acl) return false;
...
{
    perm_state_from_req_state ps(s);
    return verify_bucket_permission_no_policy(dpp, &ps, user_acl, bucket_acl, perm);
}
...
bool verify_bucket_permission_no_policy(const DoutPrefixProvider* dpp, req_state* const s, const int perm)
{
    ...
    if (!verify_requester_payer_permission(&ps)) return false;

    return verify_bucket_permission_no_policy(dpp, &ps, s->user_acl.get(), s->bucket_acl.get(), perm);
}
...
int verify_bucket_owner_or_policy(req_state* const s, const uint64_t op)
{
    auto identity_policy_res =
        eval_identity_or_session_policies(s, s->iam_user_policies, s->env, op, ARN(s->bucket->get_key()));
    if (identity_policy_res == Effect::Deny) {
        return -EACCES;
    }

    rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
    auto e = eval_or_pass(s, s->iam_policy, s->env, *s->auth.identity, op, ARN(s->bucket->get_key()), princ_type);
    if (e == Effect::Deny) {
        return -EACCES;
    }
    ...
    if (!s->session_policies.empty()) {
        auto session_policy_res =
            eval_identity_or_session_policies(s, s->session_policies, s->env, op, ARN(s->bucket->get_key()));
        if (session_policy_res == Effect::Deny) {
            return -EACCES;
        }
        if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
    ...
    if (e == Effect::Allow || identity_policy_res == Effect::Allow ||
        (e == Effect::Pass && identity_policy_res == Effect::Pass &&
...
static inline bool check_deferred_bucket_perms(const DoutPrefixProvider* dpp, struct perm_state_base* const s,
                                               const rgw_bucket& bucket, RGWAccessControlPolicy* const user_acl,
                                               RGWAccessControlPolicy* const bucket_acl,
                                               const boost::optional<Policy>& bucket_policy,
                                               const vector<Policy>& identity_policies,
                                               const vector<Policy>& session_policies, const uint8_t deferred_check,
                                               const uint64_t op)
{
    return (s->defer_to_bucket_acls == deferred_check &&
            verify_bucket_permission(
                dpp, s, bucket, user_acl, bucket_acl, bucket_policy, identity_policies, session_policies, op));
}

static inline bool check_deferred_bucket_only_acl(const DoutPrefixProvider* dpp, struct perm_state_base* const s,
                                                  RGWAccessControlPolicy* const user_acl,
                                                  RGWAccessControlPolicy* const bucket_acl,
                                                  const uint8_t deferred_check, const int perm)
{
    return (s->defer_to_bucket_acls == deferred_check &&
            verify_bucket_permission_no_policy(dpp, s, user_acl, bucket_acl, perm));
}

bool verify_object_permission(const DoutPrefixProvider* dpp, struct perm_state_base* const s, const rgw_obj& obj,
                              RGWAccessControlPolicy* const user_acl, RGWAccessControlPolicy* const bucket_acl,
                              RGWAccessControlPolicy* const object_acl, const boost::optional<Policy>& bucket_policy,
                              const vector<Policy>& identity_policies, const vector<Policy>& session_policies,
                              const uint64_t op)
{
    if (!verify_requester_payer_permission(s)) return false;
    ...
    if (check_deferred_bucket_perms(dpp,
                                    s,
                                    obj.bucket,
                                    user_acl,
                                    bucket_acl,
                                    ...
                                    op) ||
        check_deferred_bucket_perms(dpp,
                                    s,
                                    obj.bucket,
                                    user_acl,
                                    bucket_acl,
    ...
    bool ret = object_acl->verify_permission(dpp,
                                             *s->identity,
                                             s->perm_mask,
                                             perm,
    ...
    if (!s->cct->_conf->rgw_enforce_swift_acls) return ret;

    if ((perm & (int)s->perm_mask) != perm) return false;

    int swift_perm = 0;
    if (perm & (RGW_PERM_READ | RGW_PERM_READ_ACP)) swift_perm |= RGW_PERM_READ_OBJS;
    if (perm & RGW_PERM_WRITE) swift_perm |= RGW_PERM_WRITE_OBJS;

    if (!swift_perm) return false;

    /* we already verified the user mask above, so we pass swift_perm as the mask here,
       otherwise the mask might not cover the swift permissions bits */
    if (bucket_acl->verify_permission(dpp, *s->identity, swift_perm, swift_perm, s->get_referer())) return true;
...
bool verify_object_permission(const DoutPrefixProvider* dpp, req_state* const s, const rgw_obj& obj,
                              RGWAccessControlPolicy* const user_acl, RGWAccessControlPolicy* const bucket_acl,
                              RGWAccessControlPolicy* const object_acl, const boost::optional<Policy>& bucket_policy,
                              const vector<Policy>& identity_policies, const vector<Policy>& session_policies,
                              const uint64_t op)
{
    perm_state_from_req_state ps(s);
    return verify_object_permission(
        dpp, &ps, obj, user_acl, bucket_acl, object_acl, bucket_policy, identity_policies, session_policies, op);
}

bool verify_object_permission_no_policy(const DoutPrefixProvider* dpp, struct perm_state_base* const s,
                                        RGWAccessControlPolicy* const user_acl,
                                        RGWAccessControlPolicy* const bucket_acl,
                                        RGWAccessControlPolicy* const object_acl, const int perm)
{
    if (check_deferred_bucket_only_acl(dpp, s, user_acl, bucket_acl, RGW_DEFER_TO_BUCKET_ACLS_RECURSE, perm) ||
        check_deferred_bucket_only_acl(
            dpp, s, user_acl, bucket_acl, RGW_DEFER_TO_BUCKET_ACLS_FULL_CONTROL, RGW_PERM_FULL_CONTROL)) {
        return true;
    }
...
bool verify_object_permission_no_policy(const DoutPrefixProvider* dpp, req_state* s, int perm)
{
    ...
    return verify_object_permission_no_policy(
...
int verify_object_lock(const DoutPrefixProvider* dpp, const rgw::sal::Attrs& attrs, const bool bypass_perm,
...
public:
    HexTable()
    {
        ...
        for (i = 'A'; i <= 'F'; i++) table[i] = i - 'A' + 0xa;
        for (i = 'a'; i <= 'f'; i++) table[i] = i - 'a' + 0xa;
    }

    char to_num(char c) { return table[(int)c]; }
};
...
static struct rgw_name_to_flag cap_names[] = {
    {"*", RGW_CAP_ALL}, {"read", RGW_CAP_READ}, {"write", RGW_CAP_WRITE}, {NULL, 0}};

static int rgw_parse_list_of_flags(struct rgw_name_to_flag* mapping, const string& str, uint32_t* perm)
{
    list<string> strs;
    ...
    for (iter = strs.begin(); iter != strs.end(); ++iter) {
        ...
        for (int i = 0; mapping[i].type_name; i++) {
            if (s.compare(mapping[i].type_name) == 0) v |= mapping[i].flag;
        }
    }
...
int RGWUserCaps::get_cap(const string& cap, string& type, uint32_t* pperm)
{
    ...
    if (!is_valid_cap_type(type)) return -ERR_INVALID_CAP;
...
int RGWUserCaps::remove_cap(const string& cap)
{
    ...
    if (iter == caps.end()) return 0;

    uint32_t& old_perm = iter->second;
    old_perm &= ~perm;
    if (!old_perm) caps.erase(iter);
    ...
    for (iter = caps.begin(); iter != caps.end(); ++iter) {
        ...
        for (int i = 0; cap_names[i].type_name; i++) {
            if ((perm & cap_names[i].flag) == cap_names[i].flag) {
                if (perm_str.size()) perm_str.append(", ");

                perm_str.append(cap_names[i].type_name);
                perm &= ~cap_names[i].flag;
            }
        }
        if (perm_str.empty()) perm_str = "<none>";
...
int RGWUserCaps::check_cap(const string& cap, uint32_t perm) const
{
    ...
    if ((iter == caps.end()) || (iter->second & perm) != perm) {
        return -EPERM;
    }
...
bool match_policy(std::string_view pattern, std::string_view input, uint32_t flag)
{
    const uint32_t flag2 = flag & (MATCH_POLICY_ACTION | MATCH_POLICY_ARN) ? MATCH_CASE_INSENSITIVE : 0;
    const bool colonblocks = !(flag & (MATCH_POLICY_RESOURCE | MATCH_POLICY_STRING));
    ...
    while (true) {
        ...
        auto cur_pos_pattern = colonblocks ? pattern.find(":", last_pos_pattern) : npos;
        ...
        if (!match_wildcards(substr_pattern, substr_input, flag2)) return false;

        if (cur_pos_pattern == npos) return cur_pos_input == npos;
        if (cur_pos_input == npos) return false;
...
void RGWBucketInfo::decode(bufferlist::const_iterator& bl)
{
    ...
    if (struct_v >= 3) decode(flags, bl);
    if (struct_v >= 5) decode(zonegroup, bl);
    if (struct_v >= 6) {
        uint64_t ct;
        decode(ct, bl);
        if (struct_v < 17) creation_time = ceph::real_clock::from_time_t((time_t)ct);
    }
    if (struct_v >= 7) decode(placement_rule, bl);
    if (struct_v >= 8) decode(has_instance_obj, bl);
    if (struct_v >= 9) decode(quota, bl);
    static constexpr uint8_t new_layout_v = 22;
    if (struct_v >= 10 && struct_v < new_layout_v) decode(layout.current_index.layout.normal.num_shards, bl);
    if (struct_v >= 11 && struct_v < new_layout_v) decode(layout.current_index.layout.normal.hash_type, bl);
    if (struct_v >= 12) decode(requester_pays, bl);
    if (struct_v >= 13) decode(owner.tenant, bl);
    if (struct_v >= 14) {
    ...
    if (layout.logs.empty() && layout.current_index.layout.type == rgw::BucketIndexType::Normal) {
...
void RGWBucketInfo::generate_test_instances(list<RGWBucketInfo*>& o)
{
    ...
    auto gen_layout = [](rgw::BucketLayout& l) {
        ...
        l.logs.push_back(log_layout_from_index(l.current_index.gen, l.current_index));
    };
...
template<class T> static void mask_to_str(T* mask_list, uint32_t mask, char* buf, int len)
{
    const char* sep = "";
    int pos = 0;
    if (!mask) {
...
static struct rgw_flags_desc op_type_flags[] = {
    {RGW_OP_TYPE_READ, "read"}, {RGW_OP_TYPE_WRITE, "write"}, {RGW_OP_TYPE_DELETE, "delete"}, {0, NULL}};
...
void RGWSubUser::decode_json(JSONObj* obj)
{
    ...
    int pos = uid.find(':');
    if (pos >= 0) name = uid.substr(pos + 1);
    string perm_str;
...
boost::intrusive_ptr<CephContext> rgw_global_init(const std::map<std::string, std::string>* defaults,
                                                  ...
                                                  code_environment_t code_env, int flags)
{
    ...
    if ((config_store == "dbstore") || (config_store == "motr") || (config_store == "daos")) {
