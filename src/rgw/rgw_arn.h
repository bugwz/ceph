...
struct ARN
{
    ...
    ARN()
        : partition(Partition::wildcard)
        , service(Service::wildcard)
    {}
    ARN(Partition partition, Service service, std::string region, std::string account, std::string resource)
        : partition(partition)
        , service(service)
    ...
    static boost::optional<ARN> parse(const std::string& s, bool wildcard = false);
...
struct ARNResource
{
    ...
    ARNResource(const std::string& _resource_type, const std::string& _resource, const std::string& _qualifier)
        : resource_type(std::move(_resource_type))
...
namespace std {
template<> struct hash<::rgw::Service>
{
