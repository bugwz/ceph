...
#define BUFFER_MEMORY_WEIGHT \
    CEPH_PAGE_SHIFT   // memory usage of BufferHead, count in (1<<n)
...
public:
    bufferlist bl;
    C_ReadFinish(ObjectCacher* c, Object* ob, ceph_tid_t t, loff_t s, uint64_t l, const ZTracer::Trace& trace)
        : oc(c)
        , poolid(ob->oloc.pool)
        , oid(ob->get_soid())
        , start(s)
    ...
    void finish(int r) override
    {
        ...
        if (set_item.is_on_list()) set_item.remove_myself();
    }

    void distrust_enoent() { trust_enoent = false; }
};
...
public:
    C_RetryRead(ObjectCacher* _oc, OSDRead* r, ObjectSet* os, Context* c, const ZTracer::Trace& trace)
        : oc(_oc)
        , rd(r)
        , oset(os)
        , onfinish(c)
...
ObjectCacher::BufferHead* ObjectCacher::Object::split(BufferHead* left, loff_t off)
{
    ...
    if (!left->waitfor_read.empty()) {
        ...
        while (start_remove != left->waitfor_read.end() && start_remove->first < right->start()) ++start_remove;
...
void ObjectCacher::Object::merge_left(BufferHead* left, BufferHead* right)
{
    ...
    // waiters
    for (auto p = right->waitfor_read.begin(); p != right->waitfor_read.end(); ++p)
        left->waitfor_read[p->first].splice(left->waitfor_read[p->first].begin(), p->second);
...
{
    if (left->end() != right->start() || left->get_state() != right->get_state() || !left->can_merge_journal(right))
        return false;
    if (left->is_tx() && left->last_write_tid != right->last_write_tid) return false;
    return true;
}
...
void ObjectCacher::Object::try_merge_bh(BufferHead* bh)
{
    ...
    ceph_assert(p->second == bh);
    ++p;
    if (p != data.end() && can_merge_bh(bh, p->second)) merge_left(bh, p->second);
...
{
    auto& bl = bh->bl;
    if (bl.get_num_buffers() <= 1) return;

    auto wasted = bl.get_wasted_space();
    if (wasted * 2 > bl.length() && wasted > (1U << BUFFER_MEMORY_WEIGHT)) bl.rebuild();
}
 ...
 */
int ObjectCacher::Object::map_read(ObjectExtent& ex, map<loff_t, BufferHead*>& hits, map<loff_t, BufferHead*>& missing,
                                   map<loff_t, BufferHead*>& rx, map<loff_t, BufferHead*>& errors)
{
    ceph_assert(ceph_mutex_is_locked(oc->lock));
    ...
    for (auto it = data.begin(); it != data.end(); ++it) {
        if (it->first != it->second->start()) {
            lderr(oc->cct) << "AUDIT FAILURE: map position " << it->first
                           << " does not match bh start position: " << *it->second << dendl;
            ceph_assert(it->first == it->second->start());
        }
        if (it->first < offset) {
            lderr(oc->cct) << "AUDIT FAILURE: " << it->first << " " << *it->second << " overlaps with previous bh "
                           << *((--it)->second) << dendl;
            ceph_assert(it->first >= offset);
        }
        BufferHead* bh = it->second;
        for (auto w_it = bh->waitfor_read.begin(); w_it != bh->waitfor_read.end(); ++w_it) {
            if (w_it->first < bh->start() || w_it->first >= bh->start() + bh->length()) {
                lderr(oc->cct) << "AUDIT FAILURE: waiter at " << w_it->first << " is not within bh " << *bh << dendl;
 ...
 */
ObjectCacher::BufferHead* ObjectCacher::Object::map_write(ObjectExtent& ex, ceph_tid_t tid)
{
    ceph_assert(ceph_mutex_is_locked(oc->lock));
    BufferHead* final = 0;
...
void ObjectCacher::Object::replace_journal_tid(BufferHead* bh, ceph_tid_t tid)
{
    ...
    if (bh_tid != 0 && bh_tid != tid) {
        ...
        oc->writeback_handler.overwrite_extent(get_oid(), bh->start(), bh->length(), bh_tid, tid);
    }
...
void ObjectCacher::Object::discard(loff_t off, loff_t len, C_GatherBuilder* commit_gather)
{
    ceph_assert(ceph_mutex_is_locked(oc->lock));
...
ObjectCacher::ObjectCacher(CephContext* cct_, string name, WritebackHandler& wb, ceph::mutex& l,
                           flush_set_callback_t flush_callback, void* flush_callback_arg, uint64_t max_bytes,
                           uint64_t max_objects, uint64_t max_dirty, uint64_t target_dirty, double max_dirty_age,
...
ObjectCacher::~ObjectCacher()
{
    ...
    perf_stop();
    // we should be empty.
    for (auto i = objects.begin(); i != objects.end(); ++i) ceph_assert(i->empty());
...
void ObjectCacher::perf_start()
{
    ...
    plb.add_u64_counter(l_objectcacher_cache_bytes_hit, "cache_bytes_hit", "Hit data", NULL, 0, unit_t(UNIT_BYTES));
    plb.add_u64_counter(l_objectcacher_cache_bytes_miss, "cache_bytes_miss", "Miss data", NULL, 0, unit_t(UNIT_BYTES));
    ...
    plb.add_u64_counter(
        l_objectcacher_overwritten_in_flush, "data_overwritten_while_flushing", "Data overwritten while flushing");
    plb.add_u64_counter(
        l_objectcacher_write_ops_blocked, "write_ops_blocked", "Write operations, delayed due to dirty limits");
    plb.add_u64_counter(l_objectcacher_write_bytes_blocked,
                        ...
                        unit_t(UNIT_BYTES));
    plb.add_time(
        l_objectcacher_write_time_blocked, "write_time_blocked", "Time spent blocking a write due to dirty limits");
...
/* private */
ObjectCacher::Object* ObjectCacher::get_object(sobject_t oid, uint64_t object_no, ObjectSet* oset, object_locator_t& l,
                                               uint64_t truncate_size, uint64_t truncate_seq)
{
    ...
    // create it.
    Object* o = new Object(this, oid, object_no, oset, l, truncate_size, truncate_seq);
...
void ObjectCacher::bh_read(BufferHead* bh, int op_flags, const ZTracer::Trace& parent_trace)
{
    ceph_assert(ceph_mutex_is_locked(lock));
    ...
    // finisher
    C_ReadFinish* onfinish = new C_ReadFinish(this, bh->ob, bh->last_read_tid, bh->start(), bh->length(), trace);
    // go
    writeback_handler.read(bh->ob->get_oid(),
                           bh->ob->get_object_number(),
                           bh->ob->get_oloc(),
                           bh->start(),
                           bh->length(),
                           bh->ob->get_snap(),
...
void ObjectCacher::bh_read_finish(int64_t poolid, sobject_t oid, ceph_tid_t tid, loff_t start, uint64_t length,
                                  bufferlist& bl, int r, bool trust_enoent)
{
    ceph_assert(ceph_mutex_is_locked(lock));
    ldout(cct, 7) << "bh_read_finish " << oid << " tid " << tid << " " << start << "~" << length << " (bl is "
                  << bl.length() << ")"
                  << " returned " << r << " outstanding reads " << reads_outstanding << dendl;

    if (r >= 0 && bl.length() < length) {
        ldout(cct, 7) << "bh_read_finish " << oid << " padding " << start << "~" << length << " with "
                      << length - bl.length() << " bytes of zeroes" << dendl;
    ...
    if (objects[poolid].count(oid) == 0) {
        ...
        if (r == -ENOENT && !ob->complete) {
            ...
            for (auto p = ob->data.begin(); p != ob->data.end(); ++p) {
                BufferHead* bh = p->second;
                for (auto p = bh->waitfor_read.begin(); p != bh->waitfor_read.end(); ++p)
                    ls.splice(ls.end(), p->second);
                bh->waitfor_read.clear();
                if (!bh->is_zero() && !bh->is_rx()) allzero = false;
            }
        ...
        while (true) {
            ...
            if (p == ob->data.end()) break;
            if (opos >= start + (loff_t)length) {
                ldout(cct, 20) << "break due to opos " << opos << " >= start+length " << start << "+" << length << "="
                               << start + (loff_t)length << dendl;
                break;
            }
            ...
            if (bh->last_read_tid != tid) {
                ldout(cct, 10) << "bh_read_finish bh->last_read_tid " << bh->last_read_tid << " != tid " << tid
                               << ", skipping" << dendl;
                opos = bh->end();
                continue;
            }
            ...
            if (r < 0) {
                ...
                bh->bl.substr_of(bl, bh->start() - start, bh->length());
                mark_clean(bh);
            }
...
void ObjectCacher::bh_write_adjacencies(BufferHead* bh, ceph::real_time cutoff, int64_t* max_amount, int* max_count)
{
    ...
    for (set<BufferHead*, BufferHead::ptr_lt>::iterator p = it; p != dirty_or_tx_bh.end(); ++p) {
        BufferHead* obh = *p;
        if (obh->ob != bh->ob) break;
        if (obh->is_dirty() && obh->last_write <= cutoff) {
            blist.push_back(obh);
            ++count;
            total_len += obh->length();
            if ((max_count && count > *max_count) || (max_amount && total_len > *max_amount)) break;
        }
    }
    ...
    if (max_amount) *max_amount -= total_len;
...
public:
    ceph_tid_t tid = 0;
    C_WriteCommit(ObjectCacher* c, int64_t _poolid, sobject_t o, loff_t s, uint64_t l, const ZTracer::Trace& trace)
        : oc(c)
        , poolid(_poolid)
        , oid(o)
        , trace(trace)
    ...
    C_WriteCommit(ObjectCacher* c, int64_t _poolid, sobject_t o, vector<pair<loff_t, uint64_t>>& _ranges)
        : oc(c)
        , poolid(_poolid)
        , oid(o)
        , tid(0)
...
void ObjectCacher::bh_write_scattered(list<BufferHead*>& blist)
{
    ...
    for (list<BufferHead*>::iterator p = blist.begin(); p != blist.end(); ++p) {
        ...
        total_len += bh->length();
        if (bh->snapc.seq > snapc.seq) snapc = bh->snapc;
        if (bh->last_write > last_write) last_write = bh->last_write;
    }

    C_WriteCommit* oncommit = new C_WriteCommit(this, ob->oloc.pool, ob->get_soid(), ranges);

    ceph_tid_t tid = writeback_handler.write(
        ob->get_oid(), ob->get_oloc(), io_vec, snapc, last_write, ob->truncate_size, ob->truncate_seq, oncommit);
    oncommit->tid = tid;
    ...
    if (perfcounter) perfcounter->inc(l_objectcacher_data_flushed, total_len);
}
...
void ObjectCacher::bh_write(BufferHead* bh, const ZTracer::Trace& parent_trace)
{
    ...
    // finishers
    C_WriteCommit* oncommit =
        new C_WriteCommit(this, bh->ob->oloc.pool, bh->ob->get_soid(), bh->start(), bh->length(), trace);
    // go
...
void ObjectCacher::bh_write_commit(int64_t poolid, sobject_t oid, vector<pair<loff_t, uint64_t>>& ranges,
                                   ceph_tid_t tid, int r)
{
    ...
    for (vector<pair<loff_t, uint64_t>>::iterator p = ranges.begin(); p != ranges.end(); ++p) {
        ...
        // apply to bh's!
        for (map<loff_t, BufferHead*>::const_iterator p = ob->data_lower_bound(start); p != ob->data.end(); ++p) {
            BufferHead* bh = p->second;
            ...
            if (r >= 0) {
                ...
                if (bh->get_nocache()) bh_lru_rest.lru_bottouch(bh);
                hit.push_back(make_pair(bh->start(), bh));
                ldout(cct, 10) << "bh_write_commit clean " << *bh << dendl;
            }
            else {
                mark_dirty(bh);
                ldout(cct, 10) << "bh_write_commit marking dirty again due to error " << *bh << " r = " << r << " "
        ...
        for (auto& p : hit) {
            ...
            if (ob->data.count(p.first)) ob->try_merge_bh(p.second);
        }
    }
    ...
    if (flush_set_callback && was_dirty_or_tx > 0 && oset->dirty_or_tx == 0) {   // nothing dirty/tx
        flush_set_callback(flush_set_callback_arg, oset);
    }

    if (!ls.empty()) finish_contexts(cct, ls, r);
}
...
{
    ceph_assert(ceph_mutex_is_locked(lock));
    ldout(cct, 10) << "trim  start: bytes: max " << max_size << "  clean " << get_stat_clean() << ", objects: max "
                   << max_objects << " current " << ob_lru.lru_get_size() << dendl;
    ...
    ldout(cct, 10) << "trim finish:  max " << max_size << "  clean " << get_stat_clean() << ", objects: max "
                   << max_objects << " current " << ob_lru.lru_get_size() << dendl;
}
...
{
    ceph_assert(ceph_mutex_is_locked(lock));
    for (vector<ObjectExtent>::iterator ex_it = extents.begin(); ex_it != extents.end(); ++ex_it) {
        ...
        if (!o) return false;
        if (!o->is_cached(ex_it->offset, ex_it->length)) return false;
    }
    return true;
}
 ...
 */
int ObjectCacher::readx(OSDRead* rd, ObjectSet* oset, Context* onfinish, ZTracer::Trace* parent_trace)
{
    ZTracer::Trace trace;
...
int ObjectCacher::_readx(OSDRead* rd, ObjectSet* oset, Context* onfinish, bool external_call, ZTracer::Trace* trace)
{
    ceph_assert(trace != nullptr);
    ceph_assert(ceph_mutex_is_locked(lock));
    bool success = true;
    ...
    for (vector<ObjectExtent>::iterator ex_it = rd->extents.begin(); ex_it != rd->extents.end(); ++ex_it) {
        ...
        if (oset->return_enoent && !o->exists) {
            ...
            if (writeback_handler.may_copy_on_write(soid.oid, ex_it->offset, ex_it->length, soid.snap)) {
                ...
                bool wait = false;
                list<BufferHead*> blist;
                for (map<loff_t, BufferHead*>::iterator bh_it = o->data.begin(); bh_it != o->data.end(); ++bh_it) {
                ...
                if (scattered_write && !blist.empty()) bh_write_scattered(blist);
                if (wait) {
                    ldout(cct, 10) << "readx  waiting on tid " << o->last_write_tid << " on " << *o << dendl;
                    o->waitfor_commit[o->last_write_tid].push_back(new C_RetryRead(this, rd, oset, onfinish, *trace));
                    // FIXME: perfcounter!
                    return 0;
                }
            }
        ...
        if (!missing.empty() || !rx.empty()) {
            ...
            for (map<loff_t, BufferHead*>::iterator bh_it = missing.begin(); bh_it != missing.end(); ++bh_it) {
                ...
                if (!waitfor_read.empty() || (stat_rx > 0 && rx_bytes > max_size)) {
                    ...
                    if (success) {
                        ldout(cct, 10) << "readx missed, waiting on cache to complete " << waitfor_read.size()
                                       << " blocked reads, " << (std::max(rx_bytes, max_size) - max_size)
                                       << " read bytes" << dendl;
                    ...
                    if ((success && onfinish) || last != missing.end()) last = bh_it;
                }
                success = false;
            }
            ...
            if (last != missing.end()) {
                ...
                last->second->waitfor_read[last->first].push_back(new C_RetryRead(this, rd, oset, onfinish, *trace));
            }
            ...
            for (map<loff_t, BufferHead*>::iterator bh_it = hits.begin(); bh_it != hits.end(); ++bh_it) {
                ...
                if (bh->is_error() && bh->error) error = bh->error;
                ...
                if (dontneed &&
                    ...
                    if (bh->is_clean()) bh_lru_rest.lru_bottouch(bh);
                }
            }
            ...
            if (!error) {
                ...
                while (1) {
                    ...
                    ldout(cct, 10) << "readx rmap opos " << opos << ": " << *bh << " +" << bhoff << " frag "
                                   << f_it->first << "~" << f_it->second << " +" << foff << "~" << len << dendl;
            ...
            if (dontneed && o->include_all_cached_data(ex_it->offset, ex_it->length)) bottouch_ob(o);
        }
    }
...
int ObjectCacher::writex(OSDWrite* wr, ObjectSet* oset, Context* onfreespace, ZTracer::Trace* parent_trace)
{
    ceph_assert(ceph_mutex_is_locked(lock));
    ...
    for (vector<ObjectExtent>::iterator ex_it = wr->extents.begin(); ex_it != wr->extents.end(); ++ex_it) {
        // get object cache
        ...
        for (vector<pair<uint64_t, uint64_t>>::iterator f_it = ex_it->buffer_extents.begin();
             ...
             ++f_it) {
            ldout(cct, 10) << "writex writing " << f_it->first << "~" << f_it->second << " into " << *bh << " at "
                           << opos << dendl;
    ...
    if (perfcounter) {
        ...
        if (bytes_written_in_flush) {
            perfcounter->inc(l_objectcacher_overwritten_in_flush, bytes_written_in_flush);
        }
    }
...
{
public:
    C_WaitForWrite(ObjectCacher* oc, uint64_t len, const ZTracer::Trace& trace, Context* onfinish)
        : m_oc(oc)
        , m_len(len)
        , m_trace(trace)
        , m_onfinish(onfinish)
...
void ObjectCacher::_maybe_wait_for_writeback(uint64_t len, ZTracer::Trace* trace)
{
    ceph_assert(ceph_mutex_is_locked(lock));
    ...
    while (get_stat_dirty() + get_stat_tx() > 0 &&
           (((uint64_t)(get_stat_dirty() + get_stat_tx()) >= max_dirty + get_stat_dirty_waiting()) ||
            (dirty_or_tx_bh.size() >= max_dirty_bh + get_stat_nr_dirty_waiters()))) {

        if (blocked == 0) {
            trace->event("start wait for writeback");
        }
        ldout(cct, 10) << __func__ << " waiting for dirty|tx " << (get_stat_dirty() + get_stat_tx()) << " >= max "
                       << max_dirty << " + dirty_waiting " << get_stat_dirty_waiting() << dendl;
...
int ObjectCacher::_wait_for_write(OSDWrite* wr, uint64_t len, ObjectSet* oset, ZTracer::Trace* trace,
                                  Context* onfreespace)
{
    ceph_assert(ceph_mutex_is_locked(lock));
    ceph_assert(trace != nullptr);
    ...
    if (max_dirty > 0 && !(wr->fadvise_flags & LIBRADOS_OP_FLAG_FADVISE_FUA)) {
        if (block_writes_upfront) {
            ...
            if (onfreespace) onfreespace->complete(0);
        }
        else {
        ...
        if (block_writes_upfront) {
            ...
            if (onfreespace) onfreespace->complete(ret);
        }
    }

    // start writeback anyway?
    if (get_stat_dirty() > 0 && (uint64_t)get_stat_dirty() > target_dirty) {
        ldout(cct, 10) << "wait_for_write " << get_stat_dirty() << " > target " << target_dirty << ", nudging flusher"
                       << dendl;
        flusher_cond.notify_all();
    }
    return ret;
...
void ObjectCacher::flusher_entry()
{
    ...
    while (!flusher_stop) {
        ...
        ldout(cct, 11) << "flusher " << all << " / " << max_size << ":  " << get_stat_tx() << " tx, " << get_stat_rx()
                       << " rx, " << get_stat_clean() << " clean, " << get_stat_dirty() << " dirty (" << target_dirty
                       << " target, " << max_dirty << " max)" << dendl;
        ...
        if (actual > 0 && (uint64_t)actual > target_dirty) {
            ...
            ldout(cct, 10) << "flusher " << get_stat_dirty() << " dirty + " << get_stat_dirty_waiting()
                           << " dirty_waiting > target " << target_dirty << ", flushing some dirty bhs" << dendl;
...
{
    ceph_assert(ceph_mutex_is_locked(lock));
    if (oset->objects.empty()) return true;
...
{
    ceph_assert(ceph_mutex_is_locked(lock));
    if (oset->objects.empty()) return false;

    for (xlist<Object*>::iterator p = oset->objects.begin(); !p.end(); ++p) {
        Object* ob = *p;
        for (map<loff_t, BufferHead*>::iterator q = ob->data.begin(); q != ob->data.end(); ++q) {
            BufferHead* bh = q->second;
            if (!bh->is_dirty() && !bh->is_tx()) return true;
        }
    }
...
bool ObjectCacher::set_is_dirty_or_committing(ObjectSet* oset)
{
    ...
    for (xlist<Object*>::iterator i = oset->objects.begin(); !i.end(); ++i) {
        Object* ob = *i;
...
bool ObjectCacher::flush(Object* ob, loff_t offset, loff_t length, ZTracer::Trace* trace)
{
    ceph_assert(trace != nullptr);
    ceph_assert(ceph_mutex_is_locked(lock));
    list<BufferHead*> blist;
    ...
    for (map<loff_t, BufferHead*>::const_iterator p = ob->data_lower_bound(offset); p != ob->data.end(); ++p) {
        BufferHead* bh = p->second;
...
bool ObjectCacher::flush_set(ObjectSet* oset, Context* onfinish)
{
    ...
    for (set<Object*>::iterator i = waitfor_commit.begin(); i != waitfor_commit.end(); ++i) {
        Object* ob = *i;

        // we'll need to gather...
        ldout(cct, 10) << "flush_set " << oset << " will wait for ack tid " << ob->last_write_tid << " on " << *ob
                       << dendl;
...
// flush.  non-blocking, takes callback.
// returns true if already flushed
bool ObjectCacher::flush_set(ObjectSet* oset, vector<ObjectExtent>& exv, ZTracer::Trace* trace, Context* onfinish)
{
    ceph_assert(ceph_mutex_is_locked(lock));
    ceph_assert(trace != nullptr);
    ceph_assert(onfinish != NULL);
    ...
    for (vector<ObjectExtent>::iterator p = exv.begin(); p != exv.end(); ++p) {
        ObjectExtent& ex = *p;
        sobject_t soid(ex.oid, CEPH_NOSNAP);
        if (objects[oset->poolid].count(soid) == 0) continue;
...
loff_t ObjectCacher::release(Object* ob)
{
    ...
    for (list<BufferHead*>::iterator p = clean.begin(); p != clean.end(); ++p) {
        bh_remove(ob, *p);
        delete *p;
    }
...
loff_t ObjectCacher::release_set(ObjectSet* oset)
{
    ...
    for (xlist<Object*>::iterator p = oset->objects.begin(); !p.end();) {
        q = p;
        ++q;
        Object* ob = *p;
...
uint64_t ObjectCacher::release_all()
{
    ...
    while (i != objects.end()) {
        ...
        while (p != i->end()) {
            ...
            if (o_unclean) ldout(cct, 10) << "release_all " << *ob << " has " << o_unclean << " bytes left" << dendl;
            p = n;
        }
        ++i;
    }
...
void ObjectCacher::clear_nonexistence(ObjectSet* oset)
{
    ...
    for (xlist<Object*>::iterator p = oset->objects.begin(); !p.end(); ++p) {
        ...
        for (xlist<C_ReadFinish*>::iterator q = ob->reads.begin(); !q.end(); ++q) {
            C_ReadFinish* comp = *q;
 ...
 */
void ObjectCacher::discard_writeback(ObjectSet* oset, const vector<ObjectExtent>& exls, Context* on_finish)
{
    ceph_assert(ceph_mutex_is_locked(lock));
    bool was_dirty = oset->dirty_or_tx > 0;
    ...
    if (gather.has_subs()) {
        ...
        gather.set_finisher(new LambdaContext([this, oset, flushed, on_finish](int) {
            ceph_assert(ceph_mutex_is_locked(lock));
            if (flushed && flush_set_callback) flush_set_callback(flush_set_callback_arg, oset);
            if (on_finish) on_finish->complete(0);
        }));
        gather.activate();
        return;
    }
...
void ObjectCacher::_discard(ObjectSet* oset, const vector<ObjectExtent>& exls, C_GatherBuilder* gather)
{
    if (oset->objects.empty()) {
...
void ObjectCacher::_discard_finish(ObjectSet* oset, bool was_dirty, Context* on_finish)
{
    ceph_assert(ceph_mutex_is_locked(lock));
...
void ObjectCacher::verify_stats() const
{
    ...
    for (vector<ceph::unordered_map<sobject_t, Object*>>::const_iterator i = objects.begin(); i != objects.end(); ++i) {
        for (ceph::unordered_map<sobject_t, Object*>::const_iterator p = i->begin(); p != i->end(); ++p) {
            Object* ob = p->second;
            for (map<loff_t, BufferHead*>::const_iterator q = ob->data.begin(); q != ob->data.end(); ++q) {
                BufferHead* bh = q->second;
                switch (bh->get_state()) {
                case BufferHead::STATE_MISSING: missing += bh->length(); break;
                case BufferHead::STATE_CLEAN: clean += bh->length(); break;
                case BufferHead::STATE_ZERO: zero += bh->length(); break;
                case BufferHead::STATE_DIRTY: dirty += bh->length(); break;
                case BufferHead::STATE_TX: tx += bh->length(); break;
                case BufferHead::STATE_RX: rx += bh->length(); break;
                case BufferHead::STATE_ERROR: error += bh->length(); break;
                default: ceph_abort();
                }
            }
        }
    ...
    ldout(cct, 10) << " clean " << clean << " rx " << rx << " tx " << tx << " dirty " << dirty << " missing " << missing
                   << " error " << error << dendl;
...
void ObjectCacher::bh_stat_add(BufferHead* bh)
{
    ...
    case BufferHead::STATE_ZERO: stat_zero += bh->length(); break;
    ...
    case BufferHead::STATE_ERROR: stat_error += bh->length(); break;
    default: ceph_abort_msg("bh_stat_add: invalid bufferhead state");
    }
    if (get_stat_dirty_waiting() > 0) stat_cond.notify_all();
}
...
void ObjectCacher::bh_stat_sub(BufferHead* bh)
{
    ...
    case BufferHead::STATE_RX: stat_rx -= bh->length(); break;
    case BufferHead::STATE_ERROR: stat_error -= bh->length(); break;
    default: ceph_abort_msg("bh_stat_sub: invalid bufferhead state");
    }
}
...
void ObjectCacher::bh_set_state(BufferHead* bh, int s)
{
    ...
    if ((s == BufferHead::STATE_TX || s == BufferHead::STATE_DIRTY) && state != BufferHead::STATE_TX &&
        state != BufferHead::STATE_DIRTY) {
        dirty_or_tx_bh.insert(bh);
    }
    else if ((state == BufferHead::STATE_TX || state == BufferHead::STATE_DIRTY) && s != BufferHead::STATE_TX &&
             s != BufferHead::STATE_DIRTY) {
        dirty_or_tx_bh.erase(bh);
    }

    if (s != BufferHead::STATE_ERROR && state == BufferHead::STATE_ERROR) {
        bh->error = 0;
    }
