...
public:
    ...
    C_Probe(Filer* f, Probe* p, object_t o)
        : filer(f)
        , probe(p)
        , oid(o)
        , size(0)
...
int Filer::probe(inodeno_t ino, const file_layout_t* layout, snapid_t snapid, uint64_t start_from,
                 ...
                 ceph::real_time* pmtime, bool fwd, int flags, Context* onfinish)
{
    ldout(cct, 10) << "probe " << (fwd ? "fwd " : "bwd ") << hex << ino << dec << " starting from " << start_from
                   << dendl;
...
int Filer::probe_impl(Probe* probe, const file_layout_t* layout, uint64_t start_from, uint64_t* end)   // LB, when !fwd
{
    ...
    if (probe->fwd) {
        if (start_from % period) probe->probing_len += period - (start_from % period);
    }
    else {
...
void Filer::_probe(Probe* probe, Probe::unique_lock& pl)
{
    ...
    ldout(cct, 10) << "_probe " << hex << probe->ino << dec << " " << probe->probing_off << "~" << probe->probing_len
                   << dendl;
    ...
    Striper::file_to_extents(
        cct, probe->ino, &probe->layout, probe->probing_off, probe->probing_len, 0, probe->probing);
    ...
    pl.unlock();
    for (std::vector<ObjectExtent>::iterator i = stat_extents.begin(); i != stat_extents.end(); ++i) {
 ...
 */
bool Filer::_probed(Probe* probe, const object_t& oid, uint64_t size, ceph::real_time mtime, Probe::unique_lock& pl)
{
    ceph_assert(pl.owns_lock() && pl.mutex() == &probe->lock);

    ldout(cct, 10) << "_probed " << probe->ino << " object " << oid << " has size " << size << " mtime " << mtime
                   << dendl;

    probe->known_size[oid] = size;
    if (mtime > probe->max_mtime) probe->max_mtime = mtime;
    ...
    for (auto p = probe->probing.begin(); p != probe->probing.end(); ++p) {
        ...
        ldout(cct, 10) << "_probed  " << probe->ino << " object " << hex << p->oid << dec << " should be " << shouldbe
                       << ", actual is " << probe->known_size[p->oid] << dendl;
        ...
        if (!probe->found_size) {
            ...
            for (auto i = p->buffer_extents.begin(); i != p->buffer_extents.end(); ++i) {
                if (oleft <= (uint64_t)i->second) {
                    end = probe->probing_off + i->first + oleft;
                    ldout(cct, 10) << "_probed  end is in buffer_extent " << i->first << "~" << i->second << " off "
                                   << oleft << ", from was " << probe->probing_off << ", end is " << end << dendl;
    ...
    if (!probe->found_size || (probe->probing_off && (probe->pmtime || probe->pumtime))) {
        // keep probing!
...
struct PurgeRange
{
    ...
    int uncommitted;
    int err = 0;
    PurgeRange(inodeno_t i, const file_layout_t& l, const SnapContext& sc, uint64_t fo, uint64_t no, ceph::real_time t,
               int fl, Context* fin)
        : ino(i)
        , layout(l)
        , snapc(sc)
...
int Filer::purge_range(inodeno_t ino, const file_layout_t* layout, const SnapContext& snapc, uint64_t first_obj,
                       uint64_t num_obj, ceph::real_time mtime, int flags, Context* oncommit)
{
    ...
    PurgeRange* pr = new PurgeRange(ino, *layout, snapc, first_obj, num_obj, mtime, flags, oncommit);
...
struct C_PurgeRange : public Context
{
    Filer* filer;
    PurgeRange* pr;
    C_PurgeRange(Filer* f, PurgeRange* p)
        : filer(f)
        , pr(p)
    {}
    void finish(int r) override { filer->_do_purge_range(pr, 1, r); }
};

void Filer::_do_purge_range(PurgeRange* pr, int fin, int err)
{
    PurgeRange::unique_lock prl(pr->lock);
    if (err && err != -ENOENT) pr->err = err;
    pr->uncommitted -= fin;
    ldout(cct, 10) << "_do_purge_range " << pr->ino << " objects " << pr->first << "~" << pr->num << " uncommitted "
                   << pr->uncommitted << dendl;
    ...
    for (const auto& oid : remove_oids) {
        ...
        objecter->remove(
            oid, oloc, pr->snapc, pr->mtime, pr->flags, new C_OnFinisher(new C_PurgeRange(this, pr), finisher));
    }
}
...
struct TruncRange
{
    ...
    TruncRange(inodeno_t i, const file_layout_t& l, const SnapContext& sc, ceph::real_time t, int fl, Context* fin,
...
void Filer::truncate(inodeno_t ino, const file_layout_t* layout, const SnapContext& snapc, uint64_t offset,
                     uint64_t len, __u32 truncate_seq, ceph::real_time mtime, int flags, Context* oncommit)
{
    uint64_t period = layout->get_period();
    ...
    if (num_objs == 1) {
        ...
        objecter->_modify(extents[0].oid, extents[0].oloc, ops, mtime, snapc, flags, oncommit);
        return;
    }

    if (len > 0 && (offset + len) % period) len += period - ((offset + len) % period);

    TruncRange* tr = new TruncRange(ino, *layout, snapc, mtime, flags, oncommit, offset, len, truncate_seq);
...
struct C_TruncRange : public Context
{
    Filer* filer;
    TruncRange* tr;
    C_TruncRange(Filer* f, TruncRange* t)
        : filer(f)
        , tr(t)
    {}
    void finish(int r) override { filer->_do_truncate_range(tr, 1); }
};
...
void Filer::_do_truncate_range(TruncRange* tr, int fin)
{
    ...
    tr->uncommitted -= fin;
    ldout(cct, 10) << "_do_truncate_range " << tr->ino << " objects " << tr->offset << "~" << tr->length
                   << " uncommitted " << tr->uncommitted << dendl;
    ...
    if (max > 0 && tr->length > 0) {
        ...
        if (len > tr->length) len = tr->length;
