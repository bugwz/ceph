...
public:
    ...
    OSDRead* prepare_read(snapid_t snap, ceph::buffer::list* b, int f) const { return new OSDRead(snap, b, f); }
    ...
    struct OSDWrite
    {
        ...
        OSDWrite(const SnapContext& sc, const ceph::buffer::list& b, ceph::real_time mt, int f, ceph_tid_t _journal_tid)
            : snapc(sc)
            , bl(b)
            , mtime(mt)
            , fadvise_flags(f)
    ...
    OSDWrite* prepare_write(const SnapContext& sc, const ceph::buffer::list& b, ceph::real_time mt, int f,
    ...
    public:
        ...
        inline void set_error(int _error) { error = _error; }

        inline ceph_tid_t get_journal_tid() const { return journal_tid; }
        inline void set_journal_tid(ceph_tid_t _journal_tid) { journal_tid = _journal_tid; }
        ...
        void set_dontneed(bool v) { dontneed = v; }
        bool get_dontneed() const { return dontneed; }

        void set_nocache(bool v) { nocache = v; }
        bool get_nocache() const { return nocache; }

        inline bool can_merge_journal(BufferHead* bh) const { return (get_journal_tid() == bh->get_journal_tid()); }
        ...
        struct ptr_lt
        {
            bool operator()(const BufferHead* l, const BufferHead* r) const
            {
                ...
                if (loset != roset) return loset < roset;
                if (lob != rob) return lob < rob;
    ...
    public:
        ...
        Object(ObjectCacher* _oc, sobject_t o, uint64_t ono, ObjectSet* os, object_locator_t& l, uint64_t ts,
               uint64_t tq)
            : ref(0)
            , oc(_oc)
            , oid(o)
        ...
        std::map<loff_t, BufferHead*>::const_iterator data_lower_bound(loff_t offset) const
        {
            ...
            if (p != data.begin() && (p == data.end() || p->first > offset)) {
                --p;                                                 // might overlap!
                if (p->first + p->second->length() <= offset) ++p;   // doesn't overlap.
            }
            return p;
        }
        ...
        int map_read(ObjectExtent& ex, std::map<loff_t, BufferHead*>& hits, std::map<loff_t, BufferHead*>& missing,
                     std::map<loff_t, BufferHead*>& rx, std::map<loff_t, BufferHead*>& errors);
...
private:
    ...
    public:
        explicit FlusherThread(ObjectCacher* o)
            : oc(o)
        {}
        void* entry() override
        {
    ...
    {
        // have it?
        if (((uint32_t)l.pool < objects.size()) && (objects[l.pool].count(oid))) return objects[l.pool][oid];
        return NULL;
    }

    Object* get_object(sobject_t oid, uint64_t object_no, ObjectSet* oset, object_locator_t& l, uint64_t truncate_size,
    ...
    void touch_ob(Object* ob) { ob_lru.lru_touch(ob); }
    void bottouch_ob(Object* ob) { ob_lru.lru_bottouch(ob); }

    // bh states
    void bh_set_state(BufferHead* bh, int s);
    void copy_bh_state(BufferHead* bh1, BufferHead* bh2) { bh_set_state(bh2, bh1->get_state()); }
    ...
    // io
    void bh_read(BufferHead* bh, int op_flags, const ZTracer::Trace& parent_trace);
    void bh_write(BufferHead* bh, const ZTracer::Trace& parent_trace);
    void bh_write_scattered(std::list<BufferHead*>& blist);
    void bh_write_adjacencies(BufferHead* bh, ceph::real_time cutoff, int64_t* amount, int* max_count);
     ...
     */
    bool flush(Object* o, loff_t off, loff_t len, ZTracer::Trace* trace);
    ...
    int _readx(OSDRead* rd, ObjectSet* oset, Context* onfinish, bool external_call, ZTracer::Trace* trace);
    void retry_waiting_reads();

public:
    void bh_read_finish(int64_t poolid, sobject_t oid, ceph_tid_t tid, loff_t offset, uint64_t length,
                        ceph::buffer::list& bl, int r, bool trust_enoent);
    void bh_write_commit(int64_t poolid, sobject_t oid, std::vector<std::pair<loff_t, uint64_t> >& ranges, ceph_tid_t t,
                         int r);
    ...
    ObjectCacher(CephContext* cct_, std::string name, WritebackHandler& wb, ceph::mutex& l,
                 ...
                 uint64_t max_objects, uint64_t max_dirty, uint64_t target_dirty, double max_age,
                 bool block_writes_upfront);
    ~ObjectCacher();

    void start() { flusher_thread.create("flusher"); }
    void stop()
    {
     ...
     */
    int readx(OSDRead* rd, ObjectSet* oset, Context* onfinish, ZTracer::Trace* parent_trace = nullptr);
    int writex(OSDWrite* wr, ObjectSet* oset, Context* onfreespace, ZTracer::Trace* parent_trace = nullptr);
    bool is_cached(ObjectSet* oset, std::vector<ObjectExtent>& extents, snapid_t snapid);

private:
    // write blocking
    int _wait_for_write(OSDWrite* wr, uint64_t len, ObjectSet* oset, ZTracer::Trace* trace, Context* onfreespace);
    void _maybe_wait_for_writeback(uint64_t len, ZTracer::Trace* trace);
    bool _flush_set_finish(C_GatherBuilder* gather, Context* onfinish);

    void _discard(ObjectSet* oset, const std::vector<ObjectExtent>& exls, C_GatherBuilder* gather);
...
public:
    ...
    bool flush_set(ObjectSet* oset, std::vector<ObjectExtent>& ex, ZTracer::Trace* trace, Context* onfinish = 0);
    ...
    void discard_writeback(ObjectSet* oset, const std::vector<ObjectExtent>& ex, Context* on_finish);
    ...
    // cache sizes
    void set_max_dirty(uint64_t v) { max_dirty = v; }
    void set_target_dirty(int64_t v) { target_dirty = v; }
    void set_max_size(int64_t v) { max_size = v; }
    void set_max_dirty_age(double a) { max_dirty_age = ceph::make_timespan(a); }
    void set_max_objects(int64_t v) { max_objects = v; }


    // file functions

    /*** async+caching (non-blocking) file interface ***/
    int file_is_cached(ObjectSet* oset, file_layout_t* layout, snapid_t snapid, loff_t offset, uint64_t len)
    {
        std::vector<ObjectExtent> extents;
    ...
    int file_read(ObjectSet* oset, file_layout_t* layout, snapid_t snapid, loff_t offset, uint64_t len,
                  ceph::buffer::list* bl, int flags, Context* onfinish)
    {
    ...
    int file_write(ObjectSet* oset, file_layout_t* layout, const SnapContext& snapc, loff_t offset, uint64_t len,
    ...
    bool file_flush(ObjectSet* oset, file_layout_t* layout, const SnapContext& snapc, loff_t offset, uint64_t len,
                    Context* onfinish)
    {
        std::vector<ObjectExtent> extents;
        Striper::file_to_extents(cct, oset->ino, layout, offset, len, oset->truncate_size, extents);
...
inline std::ostream& operator<<(std::ostream& out, const ObjectCacher::BufferHead& bh)
{
    out << "bh[ " << &bh << " " << bh.start() << "~" << bh.length() << " " << bh.ob << " (" << bh.bl.length() << ")"
        << " v " << bh.last_write_tid;
...
inline std::ostream& operator<<(std::ostream& out, const ObjectCacher::ObjectSet& os)
{
    return out << "objectset[" << os.ino << " ts " << os.truncate_seq << "/" << os.truncate_size << " objects "
               << os.objects.size() << " dirty_or_tx " << os.dirty_or_tx << "]";
}

inline std::ostream& operator<<(std::ostream& out, const ObjectCacher::Object& ob)
{
    out << "object[" << ob.get_soid() << " oset " << ob.oset << std::dec << " wr " << ob.last_write_tid << "/"
        << ob.last_commit_tid;

    if (ob.complete) out << " COMPLETE";
    if (!ob.exists) out << " !EXISTS";
