...
public:
    ...
    uint32_t get_stream_format() const { return stream_format; }
...
public:
    Journaler(const std::string& name_, inodeno_t ino_, int64_t pool, const char* mag, Objecter* obj, PerfCounters* l,
              int lkey, Finisher* f)
        : last_committed(mag)
        , cct(obj->cct)
        , name(name_)
    ...
    void set_write_iohint(uint32_t iohint_flags) { write_iohint = iohint_flags; }
    /**
...
public:
    ...
    uint64_t get_layout_period() const { return layout.get_period(); }
    ...
    size_t get_journal_envelope_size() const { return journal_stream.get_envelope_size(); }
