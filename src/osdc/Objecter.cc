...
public:
    ...
    int call(std::string_view command, const cmdmap_t& cmdmap, const bufferlist&, Formatter* f, std::ostream& ss,
             cb::list& out) override;
};

std::unique_lock<std::mutex> Objecter::OSDSession::get_lock(object_t& oid)
{
    if (oid.name.empty()) return {};
...
{
    static const char* config_keys[] = {"crush_location", "rados_mon_op_timeout", "rados_osd_op_timeout", NULL};
    return config_keys;
}


void Objecter::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
...
void Objecter::init()
{
    ...
    if (!logger) {
        ...
        pcb.add_u64(l_osdc_op_active, "op_active", "Operations active", "actv", PerfCountersBuilder::PRIO_CRITICAL);
        ...
        pcb.add_u64_counter(l_osdc_op_r, "op_r", "Read operations", "rd", PerfCountersBuilder::PRIO_CRITICAL);
        pcb.add_u64_counter(l_osdc_op_w, "op_w", "Write operations", "wr", PerfCountersBuilder::PRIO_CRITICAL);
        pcb.add_u64_counter(
            l_osdc_op_rmw, "op_rmw", "Read-modify-write operations", "rdwr", PerfCountersBuilder::PRIO_INTERESTING);
        ...
        pcb.add_u64_counter(l_osdc_osdop_writesame, "osdop_writesame", "Write same operations");
        ...
        pcb.add_u64_counter(
            l_osdc_osdop_src_cmpxattr, "osdop_src_cmpxattr", "Extended attribute comparison in multi operations");
    ...
    int ret =
        admin_socket->register_command("objecter_requests", m_request_state_hook, "show in-progress osd requests");
...
void Objecter::_send_linger(LingerOp* info, ceph::shunique_lock<ceph::shared_mutex>& sul)
{
    ...
    if (info->registered && info->is_watch) {
        ...
        oncommit = [c = std::move(c)](bs::error_code ec) mutable { std::move (*c)(ec); };
    }
    watchl.unlock();
    auto o = new Op(info->target.base_oid,
                    info->target.base_oloc,
                    std::move(opv),
...
void Objecter::_linger_commit(LingerOp* info, bs::error_code ec, cb::list& outbl)
{
    std::unique_lock wl(info->watch_lock);
    ldout(cct, 10) << "_linger_commit " << info->linger_id << dendl;
    if (info->on_reg_commit) {
        info->on_reg_commit->defer(std::move(info->on_reg_commit), ec, cb::list{});
        info->on_reg_commit.reset();
    }
    if (ec && info->on_notify_finish) {
        info->on_notify_finish->defer(std::move(info->on_notify_finish), ec, cb::list{});
...
public:
    CB_DoWatchError(Objecter* o, Objecter::LingerOp* i, bs::error_code ec)
        : objecter(o)
        , info(i)
        , ec(ec)
    {
...
bs::error_code Objecter::_normalize_watch_error(bs::error_code ec)
{
    ...
    if (ec == bs::errc::no_such_file_or_directory) ec = bs::error_code(ENOTCONN, osd_category());
    return ec;
}

void Objecter::_linger_reconnect(LingerOp* info, bs::error_code ec)
{
    ldout(cct, 10) << __func__ << " " << info->linger_id << " = " << ec << " (last_error " << info->last_error << ")"
                   << dendl;
    std::unique_lock wl(info->watch_lock);
    if (ec) {
        if (!info->last_error) {
...
void Objecter::_send_linger_ping(LingerOp* info)
{
    ...
    Op* o = new Op(info->target.base_oid,
                   info->target.base_oloc,
                   std::move(opv),
                   info->target.flags | CEPH_OSD_FLAG_READ,
...
void Objecter::_linger_ping(LingerOp* info, bs::error_code ec, ceph::coarse_mono_time sent, uint32_t register_gen)
{
    std::unique_lock l(info->watch_lock);
    ldout(cct, 10) << __func__ << " " << info->linger_id << " sent " << sent << " gen " << register_gen << " = " << ec
                   << " (last_error " << info->last_error << " register_gen " << info->register_gen << ")" << dendl;
...
tl::expected<ceph::timespan, bs::error_code> Objecter::linger_check(LingerOp* info)
{
    std::shared_lock l(info->watch_lock);

    ceph::coarse_mono_time stamp = info->watch_valid_thru;
    if (!info->watch_pending_async.empty()) stamp = std::min(info->watch_valid_thru, info->watch_pending_async.front());
    auto age = ceph::coarse_mono_clock::now() - stamp;

    ldout(cct, 10) << __func__ << " " << info->linger_id << " err " << info->last_error << " age " << age << dendl;
    if (info->last_error) return tl::unexpected(info->last_error);
...
Objecter::LingerOp* Objecter::linger_register(const object_t& oid, const object_locator_t& oloc, int flags)
{
    ...
    if (info->target.base_oloc.key == oid) info->target.base_oloc.key.clear();
    info->target.flags = flags;
    info->watch_valid_thru = ceph::coarse_mono_clock::now();
    ldout(cct, 10) << __func__ << " info " << info << " linger_id " << info->linger_id << " cookie "
                   << info->get_cookie() << dendl;
...
ceph_tid_t Objecter::linger_watch(LingerOp* info, ObjectOperation& op, const SnapContext& snapc, real_time mtime,
                                  cb::list& inbl, decltype(info->on_reg_commit)&& oncommit, version_t* objver)
{
    info->is_watch = true;
    info->snapc = snapc;
    info->mtime = mtime;
...
ceph_tid_t Objecter::linger_notify(LingerOp* info, ObjectOperation& op, snapid_t snap, cb::list& inbl,
...
void Objecter::_linger_submit(LingerOp* info, ceph::shunique_lock<ceph::shared_mutex>& sul)
{
    ceph_assert(sul.owns_lock() && sul.mutex() == &rwlock);
    ...
    switch (r) {
    case RECALC_OP_TARGET_POOL_EIO: _check_linger_pool_eio(info); return;
    }
...
struct CB_DoWatchNotify
{
    ...
    void operator()() { objecter->_do_watch_notify(std::move(info), std::move(msg)); }
};
...
void Objecter::handle_watch_notify(MWatchNotify* m)
{
    ...
    if (m->opcode == CEPH_WATCH_EVENT_DISCONNECT) {
        if (!info->last_error) {
            ...
            if (info->handle) {
                boost::asio::defer(finish_strand, CB_DoWatchError(this, info, info->last_error));
            }
        }
    }
        ...
        if (info->notify_id && info->notify_id != m->notify_id) {
            ldout(cct, 10) << __func__ << " reply notify " << m->notify_id << " != " << info->notify_id << ", ignoring"
                           << dendl;
        }
        else if (info->on_notify_finish) {
            info->on_notify_finish->defer(
                std::move(info->on_notify_finish), osdcode(m->return_code), std::move(m->get_data()));
...
void Objecter::_do_watch_notify(boost::intrusive_ptr<LingerOp> info, boost::intrusive_ptr<MWatchNotify> m)
{
    ...
    switch (m->opcode) {
    case CEPH_WATCH_EVENT_NOTIFY: info->handle({}, m->notify_id, m->cookie, m->notifier_gid, std::move(m->bl)); break;
    }
...
bool Objecter::ms_dispatch(Message* m)
{
    ...
    case CEPH_MSG_OSD_OPREPLY: handle_osd_op_reply(static_cast<MOSDOpReply*>(m)); return true;

    case CEPH_MSG_OSD_BACKOFF: handle_osd_backoff(static_cast<MOSDBackoff*>(m)); return true;
    ...
    case MSG_GETPOOLSTATSREPLY: handle_get_pool_stats_reply(static_cast<MGetPoolStatsReply*>(m)); return true;

    case CEPH_MSG_POOLOP_REPLY: handle_pool_op_reply(static_cast<MPoolOpReply*>(m)); return true;
    ...
    case CEPH_MSG_STATFS_REPLY:
        ...
        // MDS, OSD
    case CEPH_MSG_OSD_MAP: handle_osd_map(static_cast<MOSDMap*>(m)); return false;
    }
    return false;
}

void Objecter::_scan_requests(OSDSession* s, bool skipped_map, bool cluster_full, map<int64_t, bool>* pool_full_map,
                              map<ceph_tid_t, Op*>& need_resend, list<LingerOp*>& need_resend_linger,
                              ...
                              ceph::shunique_lock<ceph::shared_mutex>& sul)
{
    ...
    while (lp != s->linger_ops.end()) {
        ...
        if (pool_full_map) force_resend_writes = force_resend_writes || (*pool_full_map)[op->target.base_oloc.pool];
        switch (r) {
        case RECALC_OP_TARGET_NO_ACTION:
            if (!skipped_map && !force_resend_writes) break;
            // -- fall-thru --
    ...
    while (p != s->ops.end()) {
        ...
        if (pool_full_map) force_resend_writes = force_resend_writes || (*pool_full_map)[op->target.base_oloc.pool];
        int r = _calc_target(&op->target, op->session ? op->session->con.get() : nullptr);
        switch (r) {
        case RECALC_OP_TARGET_NO_ACTION:
            if (!skipped_map && !(force_resend_writes && op->target.respects_full())) break;
            // -- fall-thru --
        case RECALC_OP_TARGET_NEED_RESEND:
            ...
            break;
        case RECALC_OP_TARGET_POOL_DNE: _check_op_pool_dne(op, &sl); break;
        case RECALC_OP_TARGET_POOL_EIO: _check_op_pool_eio(op, &sl); break;
        }
    }
    ...
    while (cp != s->command_ops.end()) {
        ...
        bool force_resend_writes = cluster_full;
        if (pool_full_map) force_resend_writes = force_resend_writes || (*pool_full_map)[c->target_pg.pool()];
        ...
        case RECALC_OP_TARGET_OSD_DOWN: _check_command_map_dne(c); break;
        }
    }

    sl.unlock();

    for (auto iter = unregister_lingers.begin(); iter != unregister_lingers.end(); ++iter) {
        _linger_cancel(*iter);
        (*iter)->put();
    }
}
...
void Objecter::handle_osd_map(MOSDMap* m)
{
    ...
    for (auto it = osdmap->get_pools().begin(); it != osdmap->get_pools().end(); ++it)
    ...
    if (m->get_last() <= osdmap->get_epoch()) {
        ldout(cct, 3) << "handle_osd_map ignoring epochs [" << m->get_first() << "," << m->get_last()
                      << "] <= " << osdmap->get_epoch() << dendl;
    }
    else {
        ldout(cct, 3) << "handle_osd_map got epochs [" << m->get_first() << "," << m->get_last() << "] > "
                      << osdmap->get_epoch() << dendl;
        ...
        if (osdmap->get_epoch()) {
            ...
            for (epoch_t e = osdmap->get_epoch() + 1; e <= m->get_last(); e++) {
                ...
                if (osdmap->get_epoch() == e - 1 && m->incremental_maps.count(e)) {
                    ...
                    ldout(cct, 3) << "handle_osd_map missing epoch " << osdmap->get_epoch() + 1 << ", jumping to "
                ...
                _scan_requests(homeless_session,
                               skipped_map,
                               cluster_full,
                               &pool_full_map,
                               need_resend,
                ...
                for (auto p = osd_sessions.begin(); p != osd_sessions.end();) {
                    ...
                    if (!osdmap->is_up(s->osd) || (s->con && s->con->get_peer_addrs() != osdmap->get_addrs(s->osd))) {
                        close_session(s);
                    }
                }
            ...
            if (m->maps.count(m->get_last())) {
                for (auto p = osd_sessions.begin(); p != osd_sessions.end(); ++p) {
                    ...
                    _scan_requests(s, false, false, NULL, need_resend, need_resend_linger, need_resend_command, sul);
                }
                ...
                _scan_requests(
                    homeless_session, false, false, NULL, need_resend, need_resend_linger, need_resend_command, sul);
            }
            else {
    ...
    for (auto p = need_resend_linger.begin(); p != need_resend_linger.end(); ++p) {
        LingerOp* op = *p;
    ...
    for (auto p = need_resend_command.begin(); p != need_resend_command.end(); ++p) {
        auto c = p->second;
        if (c->target.osd >= 0) {
...
void Objecter::emit_blocklist_events(const OSDMap& old_osd_map, const OSDMap& new_osd_map)
{
    if (!blocklist_events_enabled) {
        return;
    }
    ...
    std::set_difference(
        new_set.begin(), new_set.end(), old_set.begin(), old_set.end(), std::inserter(delta_set, delta_set.begin()));
    std::set_difference(new_range_set.begin(),
                        new_range_set.end(),
                        old_range_set.begin(),
                        old_range_set.end(),
...
void Objecter::CB_Op_Map_Latest::operator()(bs::error_code e, version_t latest, version_t)
{
    if (e == bs::errc::resource_unavailable_try_again || e == bs::errc::operation_canceled) return;

    lgeneric_subdout(objecter->cct, objecter, 10)
        << "op_map_latest r=" << e << " tid=" << tid << " latest " << latest << dendl;
    ...
    if (op->map_dne_bound == 0) op->map_dne_bound = latest;
...
int Objecter::pool_snap_by_name(int64_t poolid, const char* snap_name, snapid_t* snap) const
{
    shared_lock rl(rwlock);
    ...
    for (auto p = pg_pool.snaps.begin(); p != pg_pool.snaps.end(); ++p) {
...
int Objecter::pool_snap_get_info(int64_t poolid, snapid_t snap, pool_snap_info_t* info) const
{
    shared_lock rl(rwlock);
...
int Objecter::pool_snap_list(int64_t poolid, vector<uint64_t>* snaps)
{
    ...
    if (!pi) return -ENOENT;
    for (auto p = pi->snaps.begin(); p != pi->snaps.end(); ++p) {
...
void Objecter::_check_op_pool_dne(Op* op, std::unique_lock<std::shared_mutex>* sl)
{
    ...
    if (op->target.pool_ever_existed) {
        ...
        ldout(cct, 10) << "check_op_pool_dne tid " << op->tid << " current " << osdmap->get_epoch() << " map_dne_bound "
                       << op->map_dne_bound << dendl;
    }
    if (op->map_dne_bound > 0) {
        if (osdmap->get_epoch() >= op->map_dne_bound) {
            // we had a new enough map
            ldout(cct, 10) << "check_op_pool_dne tid " << op->tid << " concluding pool " << op->target.base_pgid.pool()
                           << " dne" << dendl;
...
void Objecter::_check_op_pool_eio(Op* op, std::unique_lock<std::shared_mutex>* sl)
{
    ...
    // we had a new enough map
    ldout(cct, 10) << "check_op_pool_eio tid " << op->tid << " concluding pool " << op->target.base_pgid.pool()
                   << " has eio" << dendl;
    if (op->has_completion()) {
        num_in_flight--;
...
void Objecter::CB_Linger_Map_Latest::operator()(bs::error_code e, version_t latest, version_t)
{
    ...
    *need_unregister = false;
    ...
    if (op->register_gen > 0) {
        ldout(cct, 10) << "_check_linger_pool_dne linger_id " << op->linger_id
                       << " pool previously existed but now does not" << dendl;
        op->map_dne_bound = osdmap->get_epoch();
    }
    else {
        ldout(cct, 10) << "_check_linger_pool_dne linger_id " << op->linger_id << " current " << osdmap->get_epoch()
                       << " map_dne_bound " << op->map_dne_bound << dendl;
    }
    if (op->map_dne_bound > 0) {
        if (osdmap->get_epoch() >= op->map_dne_bound) {
            std::unique_lock wl{op->watch_lock};
            if (op->on_reg_commit) {
                op->on_reg_commit->defer(std::move(op->on_reg_commit), osdc_errc::pool_dne, cb::list{});
                op->on_reg_commit = nullptr;
            }
            if (op->on_notify_finish) {
                op->on_notify_finish->defer(std::move(op->on_notify_finish), osdc_errc::pool_dne, cb::list{});
...
void Objecter::CB_Command_Map_Latest::operator()(bs::error_code e, version_t latest, version_t)
{
    if (e == bs::errc::resource_unavailable_try_again || e == bs::errc::operation_canceled) {
        // ignore callback; we will retry in resend_mon_ops()
        return;
    ...
    if (c->map_dne_bound == 0) c->map_dne_bound = latest;
...
void Objecter::_check_command_map_dne(CommandOp* c)
{
    ...
    ldout(cct, 10) << "_check_command_map_dne tid " << c->tid << " current " << osdmap->get_epoch() << " map_dne_bound "
                   << c->map_dne_bound << dendl;
    if (c->map_dne_bound > 0) {
        if (osdmap->get_epoch() >= c->map_dne_bound) {
            _finish_command(c, osdcode(c->map_check_error), std::move(c->map_check_error_str), {});
        }
    }
    else {
...
void Objecter::close_session(OSDSession* s)
{
    ...
    // Assign any leftover ops to the homeless session
    {
        ...
        for (auto i = homeless_lingers.begin(); i != homeless_lingers.end(); ++i) {
            _session_linger_op_assign(homeless_session, *i);
        }
        for (auto i = homeless_ops.begin(); i != homeless_ops.end(); ++i) {
            _session_op_assign(homeless_session, *i);
        }
        for (auto i = homeless_commands.begin(); i != homeless_commands.end(); ++i) {
...
void Objecter::wait_for_osd_map(epoch_t e)
{
    ...
    waiting_for_map[e].emplace_back(OpCompletion::create(service.get_executor(), w.ref()), bs::error_code{});
    l.unlock();
    w.wait();
}

void Objecter::_get_latest_version(epoch_t oldest, epoch_t newest, std::unique_ptr<OpCompletion> fin,
...
void Objecter::_maybe_request_map()
{
    // rwlock is locked
    int flag = 0;
    if (_osdmap_full_flag() || osdmap->test_flag(CEPH_OSDMAP_PAUSERD) || osdmap->test_flag(CEPH_OSDMAP_PAUSEWR)) {
        ldout(cct, 10) << "_maybe_request_map subscribing (continuous) to next "
                          "osd map (FULL flag is set)"
                       << dendl;
    }
    else {
...
void Objecter::_wait_for_new_map(std::unique_ptr<OpCompletion> c, epoch_t epoch, bs::error_code ec)
{
    // rwlock is locked unique
...
void Objecter::_kick_requests(OSDSession* session, map<uint64_t, LingerOp*>& lresend)
{
    ...
    for (auto p = session->ops.begin(); p != session->ops.end();) {
        ...
        ++p;
        if (op->should_resend) {
            if (!op->target.paused) resend[op->tid] = op;
        }
        else {
            _op_cancel_map_check(op);
            _cancel_linger_op(op);
    ...
    for (auto j = session->linger_ops.begin(); j != session->linger_ops.end(); ++j) {
    ...
    for (auto k = session->command_ops.begin(); k != session->command_ops.end(); ++k) {
...
void Objecter::_linger_ops_resend(map<uint64_t, LingerOp*>& lresend, unique_lock<ceph::shared_mutex>& ul)
{
...
void Objecter::start_tick()
{
    ...
    tick_event = timer.add_event(ceph::make_timespan(cct->_conf->objecter_tick_interval), &Objecter::tick, this);
}
...
void Objecter::tick()
{
    ...
    for (auto siter = osd_sessions.begin(); siter != osd_sessions.end(); ++siter) {
        ...
        for (auto p = s->linger_ops.begin(); p != s->linger_ops.end(); ++p) {
            ...
            std::unique_lock wl(op->watch_lock);
            ceph_assert(op->session);
            ldout(cct, 10) << " pinging osd that serves lingering tid " << p->first << " (osd." << op->session->osd
                           << ")" << dendl;
            found = true;
        ...
        for (auto p = s->command_ops.begin(); p != s->command_ops.end(); ++p) {
            auto op = p->second;
            ceph_assert(op->session);
            ldout(cct, 10) << " pinging osd that serves command tid " << p->first << " (osd." << op->session->osd << ")"
                           << dendl;
            found = true;
        }
        if (found) toping.insert(s);
    }
    ...
    if (initialized) {
        tick_event = timer.reschedule_me(ceph::make_timespan(cct->_conf->objecter_tick_interval));
    }
}
...
void Objecter::resend_mon_ops()
{
    ...
    for (auto p = check_latest_map_ops.begin(); p != check_latest_map_ops.end(); ++p) {
        monc->get_version("osdmap", CB_Op_Map_Latest(this, p->second->tid));
    }

    for (auto p = check_latest_map_lingers.begin(); p != check_latest_map_lingers.end(); ++p) {
        monc->get_version("osdmap", CB_Linger_Map_Latest(this, p->second->linger_id));
    }

    for (auto p = check_latest_map_commands.begin(); p != check_latest_map_commands.end(); ++p) {
...
void Objecter::_op_submit_with_budget(Op* op, shunique_lock<ceph::shared_mutex>& sul, ceph_tid_t* ptid, int* ctx_budget)
{
    ...
    if (osd_timeout > timespan(0)) {
        if (op->tid == 0) op->tid = ++last_tid;
        auto tid = op->tid;
        op->ontimeout = timer.add_event(osd_timeout, [this, tid]() { op_cancel(tid, -ETIMEDOUT); });
    }
...
void Objecter::_send_op_account(Op* op)
{
    ...
    if (op->target.flags & CEPH_OSD_FLAG_PGOP) logger->inc(l_osdc_op_pg);
...
void Objecter::_op_submit(Op* op, shunique_lock<ceph::shared_mutex>& sul, ceph_tid_t* ptid)
{
    ...
    int r = _calc_target(&op->target, nullptr);
    switch (r) {
    case RECALC_OP_TARGET_POOL_DNE: check_for_latest_map = true; break;
    case RECALC_OP_TARGET_POOL_EIO:
        if (op->has_completion()) {
            op->complete(osdc_errc::pool_eio, -EIO);
        }
    ...
    if (r == -EAGAIN || (check_for_latest_map && sul.owns_lock_shared()) ||
    ...
    ldout(cct, 10) << "_op_submit oid " << op->target.base_oid << " '" << op->target.base_oloc << "' '"
                   << op->target.target_oloc << "' " << op->ops << " tid " << op->tid << " osd."
                   << (!s->is_homeless() ? s->osd : -1) << dendl;
...
epoch_t Objecter::op_cancel_writes(int r, int64_t pool)
{
    ...
    for (auto siter = osd_sessions.begin(); siter != osd_sessions.end(); ++siter) {
        ...
        if (!found && to_cancel.size()) found = true;
        to_cancel.clear();
    }
...
bool Objecter::is_pg_changed(int oldprimary, const vector<int>& oldacting, int newprimary, const vector<int>& newacting,
                             bool any_change)
{
    if (OSDMap::primary_changed_broken(   // https://tracker.ceph.com/issues/43213
            ...
            newacting))
        return true;
    if (any_change && oldacting != newacting) return true;
...
bool Objecter::target_should_be_paused(op_target_t* t)
{
    ...
    return (t->flags & CEPH_OSD_FLAG_READ && pauserd) || (t->flags & CEPH_OSD_FLAG_WRITE && pausewr) ||
...
{
    for (auto it = osdmap->get_pools().begin(); it != osdmap->get_pools().end(); ++it) {
        if (_osdmap_pool_full(it->second)) return true;
    }
    return false;
}
...
{
    for (map<int64_t, pg_pool_t>::const_iterator it = osdmap->get_pools().begin(); it != osdmap->get_pools().end();
         ++it) {
...
int64_t Objecter::get_object_hash_position(int64_t pool, const string& key, const string& ns)
{
    shared_lock rl(rwlock);
...
int64_t Objecter::get_object_pg_hash_position(int64_t pool, const string& key, const string& ns)
{
    shared_lock rl(rwlock);
    const pg_pool_t* p = osdmap->get_pg_pool(pool);
    if (!p) return -ENOENT;
...
void Objecter::_prune_snapc(const mempool::osdmap::map<int64_t, snap_interval_set_t>& new_removed_snaps, Op* op)
{
    bool match = false;
    ...
    if (i != new_removed_snaps.end()) {
        ...
        if (match) {
            ...
            ldout(cct, 10) << __func__ << " op " << op->tid << " snapc " << op->snapc << " (was " << new_snaps << ")"
                           << dendl;
        }
    }
}
...
int Objecter::_calc_target(op_target_t* t, Connection* con, bool any_change)
{
    ...
    ldout(cct, 20) << __func__ << " epoch " << t->epoch << " base " << t->base_oid << " " << t->base_oloc
                   << " precalc_pgid " << (int)t->precalc_pgid << " pgid " << t->base_pgid
                   << (is_read ? " is_read" : "") << (is_write ? " is_write" : "") << dendl;
    ...
    if ((t->flags & CEPH_OSD_FLAG_IGNORE_OVERLAY) == 0) {
        if (is_read && pi->has_read_tier()) t->target_oloc.pool = pi->read_tier;
        if (is_write && pi->has_write_tier()) t->target_oloc.pool = pi->write_tier;
    ...
    if (!lookup_pg_mapping(actual_pgid, osdmap->get_epoch(), &up, &up_primary, &acting, &acting_primary)) {
        osdmap->pg_to_up_acting_osds(actual_pgid, &up, &up_primary, &acting, &acting_primary);
        pg_mapping_t pg_mapping(osdmap->get_epoch(), up, up_primary, acting, acting_primary);
    ...
    if (any_change && PastIntervals::is_new_interval(t->acting_primary,
                                                     acting_primary,
                                                     t->acting,
                                                     acting,
                                                     t->up_primary,
    ...
    if (t->pg_num) {
        split_or_merge = prev_pgid.is_split(t->pg_num, pg_num, nullptr) ||
    ...
    if (legacy_change || split_or_merge || force_resend) {
        ...
        ldout(cct, 10) << __func__ << " "
                       << " raw pgid " << pgid << " -> actual " << t->actual_pgid << " acting " << t->acting
                       << " primary " << acting_primary << dendl;
        t->used_replica = false;
        if ((t->flags & (CEPH_OSD_FLAG_BALANCE_READS | CEPH_OSD_FLAG_LOCALIZE_READS)) && !is_write &&
            pi->is_replicated() && t->acting.size() > 1) {
            int osd;
            ceph_assert(is_read && t->acting[0] == acting_primary);
            if (t->flags & CEPH_OSD_FLAG_BALANCE_READS) {
                int p = rand() % t->acting.size();
                if (p) t->used_replica = true;
                osd = t->acting[p];
                ldout(cct, 10) << " chose random osd." << osd << " of " << t->acting << dendl;
            }
            else {
                ...
                for (unsigned i = 0; i < t->acting.size(); ++i) {
                    int locality = osdmap->crush->get_common_ancestor_distance(cct, t->acting[i], crush_location);
                    ldout(cct, 20) << __func__ << " localize: rank " << i << " osd." << t->acting[i] << " locality "
                                   << locality << dendl;
                    if (i == 0 || (locality >= 0 && best_locality >= 0 && locality < best_locality) ||
                        (best_locality < 0 && locality >= 0)) {
                        best = i;
                        best_locality = locality;
                        if (i) t->used_replica = true;
                    }
                }
                ceph_assert(best >= 0);
                osd = t->acting[best];
    ...
    if (split_or_merge && (osdmap->require_osd_release >= ceph_release_t::luminous ||
                           HAVE_FEATURE(osdmap->get_xinfo(acting_primary).features, RESEND_ON_SPLIT))) {
        return RECALC_OP_TARGET_NEED_RESEND;
    }
    return RECALC_OP_TARGET_NO_ACTION;
}

int Objecter::_map_session(op_target_t* target, OSDSession** s, shunique_lock<ceph::shared_mutex>& sul)
{
...
int Objecter::_recalc_linger_op_target(LingerOp* linger_op, shunique_lock<ceph::shared_mutex>& sul)
{
    // rwlock is locked unique

    int r = _calc_target(&linger_op->target, nullptr, true);
    if (r == RECALC_OP_TARGET_NEED_RESEND) {
        ldout(cct, 10) << "recalc_linger_op_target tid " << linger_op->linger_id << " pgid " << linger_op->target.pgid
...
void Objecter::_finish_op(Op* op, int r)
{
    ...
    if (op->ontimeout && r != -ETIMEDOUT) timer.cancel_event(op->ontimeout);
...
Objecter::MOSDOp* Objecter::_prepare_osd_op(Op* op)
{
    ...
    if (!honor_pool_full) flags |= CEPH_OSD_FLAG_FULL_FORCE;

    op->target.paused = false;
    op->stamp = ceph::coarse_mono_clock::now();

    hobject_t hobj = op->target.get_hobj();
    auto m = new MOSDOp(client_inc, op->tid, hobj, op->target.actual_pgid, osdmap->get_epoch(), flags, op->features);
...
void Objecter::_send_op(Op* op)
{
    ...
    if (p != op->session->backoffs.end()) {
        ...
        if (q != p->second.end()) {
            ldout(cct, 20) << __func__ << " ? " << q->first << " [" << q->second.begin << "," << q->second.end << ")"
                           << dendl;
            int r = cmp(hoid, q->second.begin);
            if (r == 0 || (r > 0 && hoid < q->second.end)) {
                ldout(cct, 10) << __func__ << " backoff " << op->target.actual_pgid << " id " << q->second.id << " on "
                               << hoid << ", queuing " << op << " tid " << op->tid << dendl;
                return;
            }
        }
    }
    ...
    if (op->target.actual_pgid != m->get_spg()) {
        ldout(cct, 10) << __func__ << " " << op->tid << " pgid change from " << m->get_spg() << " to "
                       << op->target.actual_pgid << ", updating and reencoding" << dendl;
        m->set_spg(op->target.actual_pgid);
        m->clear_payload();   // reencode
    }

    ldout(cct, 15) << "_send_op " << op->tid << " to " << op->target.actual_pgid << " on osd." << op->session->osd
                   << dendl;
    ...
    for (auto i = ops.begin(); i != ops.end(); ++i) {
        if (i->op.op & CEPH_OSD_OP_MODE_WR) {
            ...
            if (ceph_osd_op_uses_extent(i->op.op)) {
                if ((int64_t)i->op.extent.length > 0) op_budget += (int64_t)i->op.extent.length;
            }
...
void Objecter::_throttle_op(Op* op, shunique_lock<ceph::shared_mutex>& sul, int op_budget)
{
    ceph_assert(sul && sul.mutex() == &rwlock);
    bool locked_for_write = sul.owns_lock();

    if (!op_budget) op_budget = calc_op_budget(op->ops);
...
void Objecter::handle_osd_op_reply(MOSDOpReply* m)
{
    ...
    if (iter == s->ops.end()) {
        ldout(cct, 7) << "handle_osd_op_reply " << tid
                      << (m->is_ondisk() ? " ondisk" : (m->is_onnvram() ? " onnvram" : " ack")) << " ... stray"
                      << dendl;
        sl.unlock();
        m->put();
        return;
    ...
    ldout(cct, 7) << "handle_osd_op_reply " << tid
                  << (m->is_ondisk() ? " ondisk" : (m->is_onnvram() ? " onnvram" : " ack")) << " uv "
                  << m->get_user_version() << " in " << m->get_pg() << " attempt " << m->get_retry_attempt() << dendl;
    Op* op = iter->second;
    op->trace.event("osd op reply");

    if (retry_writes_after_first_reply && op->attempts == 1 && (op->target.flags & CEPH_OSD_FLAG_WRITE)) {
    ...
    if (m->get_retry_attempt() >= 0) {
        if (m->get_retry_attempt() != (op->attempts - 1)) {
            ldout(cct, 7) << " ignoring reply from attempt " << m->get_retry_attempt() << " from "
                          << m->get_source_inst() << "; last attempt " << (op->attempts - 1) << " sent to "
    ...
    if (m->is_redirect_reply()) {
        ...
        op->tid = 0;
        m->get_redirect().combine_with_locator(op->target.target_oloc, op->target.target_oid.name);
        op->target.flags |= (CEPH_OSD_FLAG_REDIRECTED | CEPH_OSD_FLAG_IGNORE_CACHE | CEPH_OSD_FLAG_IGNORE_OVERLAY);
        _op_submit(op, sul, NULL);
        m->put();
        return;
    }
    ...
    if (op->objver) *op->objver = m->get_user_version();
    if (op->reply_epoch) *op->reply_epoch = m->get_map_epoch();
    if (op->data_offset) *op->data_offset = m->get_header().data_off;
...
#endif
        auto& bl = m->get_data();
        if (op->outbl->length() == bl.length() && bl.get_num_buffers() <= 1) {
            ...
            ldout(cct, 10) << __func__ << " copying resulting " << bl.length()
                           << " into existing ceph::buffer of length " << op->outbl->length() << dendl;
            cb::list t;
            t = std::move(*op->outbl);
    ...
    if (out_ops.size() != op->ops.size())
        ldout(cct, 0) << "WARNING: tid " << op->tid << " reply ops " << out_ops << " != request ops " << op->ops
    ...
    for (unsigned i = 0; p != out_ops.end() && pb != op->out_bl.end(); ++i, ++p, ++pb, ++pr, ++pe, ++ph) {
        ...
        if (*pr) **pr = ceph_to_hostos_errno(p->rval);
        if (*pe) **pe = p->rval < 0 ? bs::error_code(-p->rval, osd_category()) : bs::error_code();
        if (*ph) {
            std::move((*ph))(
                p->rval < 0 ? bs::error_code(-p->rval, osd_category()) : bs::error_code(), p->rval, p->outdata);
        }
    }
...
void Objecter::handle_osd_backoff(MOSDBackoff* m)
{
    ...
    case CEPH_OSD_BACKOFF_OP_BLOCK:
    {
        ...
        auto r = new MOSDBackoff(m->pgid, m->map_epoch, CEPH_OSD_BACKOFF_OP_ACK_BLOCK, m->id, m->begin, m->end);
    ...
    case CEPH_OSD_BACKOFF_OP_UNBLOCK:
    {
        ...
        if (p != s->backoffs_by_id.end()) {
            ...
            if (b->begin != m->begin && b->end != m->end) {
                lderr(cct) << __func__ << " got " << m->pgid << " id " << m->id << " unblock on [" << m->begin << ","
                           << m->end << ") but backoff is [" << b->begin << "," << b->end << ")" << dendl;
                // hrmpf, unblock it anyway.
            }
            ldout(cct, 10) << __func__ << " unblock backoff " << b->pgid << " id " << b->id << " [" << b->begin << ","
                           << b->end << ")" << dendl;
            ...
            for (auto& q : s->ops) {
                if (q.second->target.actual_pgid == m->pgid) {
                    ...
                    ldout(cct, 20) << __func__ << " contained_by " << r << " on " << q.second->target.get_hobj()
                                   << dendl;
                    if (r) {
                        _send_op(q.second);
                    }
            ...
            lderr(cct) << __func__ << " " << m->pgid << " id " << m->id << " unblock on [" << m->begin << "," << m->end
                       << ") but backoff dne" << dendl;
        }
    } break;

    default: ldout(cct, 10) << __func__ << " unrecognized op " << (int)m->op << dendl;
    }
...
uint32_t Objecter::list_nobjects_seek(NListContext* list_context, uint32_t pos)
{
    shared_lock rl(rwlock);
    list_context->pos = hobject_t(object_t(), string(), CEPH_NOSNAP, pos, list_context->pool_id, string());
...
uint32_t Objecter::list_nobjects_seek(NListContext* list_context, const hobject_t& cursor)
{
    shared_lock rl(rwlock);
...
void Objecter::list_nobjects_get_cursor(NListContext* list_context, hobject_t* cursor)
{
    shared_lock rl(rwlock);
    if (list_context->list.empty()) {
        ...
        *cursor =
            hobject_t(entry.oid, entry.locator, list_context->pool_snap_seq, h, list_context->pool_id, entry.nspace);
    }
}

void Objecter::list_nobjects(NListContext* list_context, Context* onfinish)
{
    ldout(cct, 10) << __func__ << " pool_id " << list_context->pool_id << " pool_snap_seq "
                   << list_context->pool_snap_seq << " max_entries " << list_context->max_entries << " list_context "
                   << list_context << " onfinish " << onfinish << " current_pg " << list_context->current_pg << " pos "
    ...
    if (list_context->sort_bitwise != sort_bitwise) {
        list_context->pos =
            hobject_t(object_t(), string(), CEPH_NOSNAP, list_context->current_pg, list_context->pool_id, string());
        list_context->sort_bitwise = sort_bitwise;
    ...
    ObjectOperation op;
    op.pg_nls(list_context->max_entries, list_context->filter, list_context->pos, osdmap->get_epoch());
    ...
    pg_read(list_context->current_pg, oloc, op, &list_context->bl, 0, onack, &onack->epoch, &list_context->ctx_budget);
}

void Objecter::_nlist_reply(NListContext* list_context, int r, Context* final_finish, epoch_t reply_epoch)
{
    ...
    if ((response.handle.is_max() || r == 1) && !list_context->sort_bitwise) {
    ...
    ldout(cct, 20) << " response.entries.size " << response_size << ", response.entries " << response.entries
                   << ", handle " << response.handle << ", tentative new pos " << list_context->pos << dendl;
    if (response_size) {
        std::move(response.entries.begin(), response.entries.end(), std::back_inserter(list_context->list));
...
void Objecter::create_pool_snap(int64_t pool, std::string_view snap_name, decltype(PoolOp::onfinish)&& onfinish)
{
    unique_lock wl(rwlock);
    ...
    if (p->snap_exists(snap_name)) {
        onfinish->defer(std::move(onfinish), osdc_errc::snapshot_exists, cb::list{});
        return;
    }
                                         ...
                                         std::unique_ptr<ca::Completion<void(bs::error_code, snapid_t)>> onfinish)
{
    ...
    op->tid = ++last_tid;
    op->pool = pool;
    op->onfinish = PoolOp::OpComp::create(service.get_executor(), CB_SelfmanagedSnap(std::move(onfinish)));
...
void Objecter::delete_pool_snap(int64_t pool, std::string_view snap_name, decltype(PoolOp::onfinish)&& onfinish)
{
    unique_lock wl(rwlock);
...
void Objecter::delete_selfmanaged_snap(int64_t pool, snapid_t snap, decltype(PoolOp::onfinish)&& onfinish)
{
    unique_lock wl(rwlock);
    ldout(cct, 10) << "delete_selfmanaged_snap; pool: " << pool << "; snap: " << snap << dendl;
    auto op = new PoolOp;
    op->tid = ++last_tid;
    op->pool = pool;
    op->onfinish = std::move(onfinish);
...
void Objecter::create_pool(std::string_view name, decltype(PoolOp::onfinish)&& onfinish, int crush_rule)
{
    unique_lock wl(rwlock);
...
void Objecter::delete_pool(int64_t pool, decltype(PoolOp::onfinish)&& onfinish)
{
    unique_lock wl(rwlock);
...
void Objecter::delete_pool(std::string_view pool_name, decltype(PoolOp::onfinish)&& onfinish)
{
    unique_lock wl(rwlock);
...
void Objecter::_do_delete_pool(int64_t pool, decltype(PoolOp::onfinish)&& onfinish)
...
void Objecter::pool_op_submit(PoolOp* op)
{
    ...
    if (mon_timeout > timespan(0)) {
        op->ontimeout = timer.add_event(mon_timeout, [this, op]() { pool_op_cancel(op->tid, -ETIMEDOUT); });
    }
    _pool_op_submit(op);
}
...
void Objecter::_pool_op_submit(PoolOp* op)
{
    ...
    auto m = new MPoolOp(monc->get_fsid(), op->tid, op->pool, op->name, op->pool_op, last_seen_osdmap_version);
...
void Objecter::handle_pool_op_reply(MPoolOpReply* m)
{
    ...
    if (iter != pool_ops.end()) {
        ...
        if (m->version > last_seen_osdmap_version) last_seen_osdmap_version = m->version;
...
int Objecter::pool_op_cancel(ceph_tid_t tid, int r)
{
    ...
    PoolOp* op = it->second;
    if (op->onfinish) op->onfinish->defer(std::move(op->onfinish), osdcode(r), cb::list{});
...
void Objecter::get_pool_stats(const std::vector<std::string>& pools, decltype(PoolStatOp::onfinish)&& onfinish)
{
    ...
    op->onfinish = std::move(onfinish);
    if (mon_timeout > timespan(0)) {
        op->ontimeout = timer.add_event(mon_timeout, [this, op]() { pool_stat_op_cancel(op->tid, -ETIMEDOUT); });
    }
    else {
        op->ontimeout = 0;
    }
...
void Objecter::handle_get_pool_stats_reply(MGetPoolStatsReply* m)
{
    ...
    if (iter != poolstat_ops.end()) {
        ...
        op->onfinish->defer(std::move(op->onfinish), bs::error_code{}, std::move(m->pool_stats), m->per_pool);
...
int Objecter::pool_stat_op_cancel(ceph_tid_t tid, int r)
{
    ...
    auto op = it->second;
    if (op->onfinish)
        op->onfinish->defer(std::move(op->onfinish), osdcode(r), bc::flat_map<std::string, pool_stat_t>{}, false);
...
void Objecter::get_fs_stats(std::optional<int64_t> poolid, decltype(StatfsOp::onfinish)&& onfinish)
{
    ...
    op->onfinish = std::move(onfinish);
    if (mon_timeout > timespan(0)) {
        op->ontimeout = timer.add_event(mon_timeout, [this, op]() { statfs_op_cancel(op->tid, -ETIMEDOUT); });
    }
    else {
        op->ontimeout = 0;
    }
...
void Objecter::_fs_stats_submit(StatfsOp* op)
{
    ...
    monc->send_mon_message(new MStatfs(monc->get_fsid(), op->tid, op->data_pool, last_seen_pgmap_version));
    op->last_submit = ceph::coarse_mono_clock::now();
...
void Objecter::handle_fs_stats_reply(MStatfsReply* m)
{
    ...
    if (statfs_ops.count(tid)) {
        ...
        ldout(cct, 10) << "have request " << tid << " at " << op << dendl;
        if (m->h.version > last_seen_pgmap_version) last_seen_pgmap_version = m->h.version;
...
int Objecter::statfs_op_cancel(ceph_tid_t tid, int r)
{
    ...
    auto op = it->second;
    if (op->onfinish) op->onfinish->defer(std::move(op->onfinish), osdcode(r), ceph_statfs{});
...
void Objecter::_sg_read_finish(vector<ObjectExtent>& extents, vector<cb::list>& resultbl, cb::list* bl,
                               Context* onfinish)
{
    // all done
    ...
    if (extents.size() > 1) {
        ...
        for (auto eit = extents.begin(); eit != extents.end(); ++eit, ++bit) {
...
void Objecter::ms_handle_connect(Connection* con)
{
    ...
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_MON) resend_mon_ops();
}
...
void Objecter::_dump_active(OSDSession* s)
{
    for (auto p = s->ops.begin(); p != s->ops.end(); ++p) {
        Op* op = p->second;
        ldout(cct, 20) << op->tid << "\t" << op->target.pgid << "\tosd." << (op->session ? op->session->osd : -1)
                       << "\t" << op->target.base_oid << "\t" << op->ops << dendl;
    }
}
...
void Objecter::_dump_command_ops(const OSDSession* s, Formatter* fmt)
{
    for (auto p = s->command_ops.begin(); p != s->command_ops.end(); ++p) {
        ...
        for (auto q = op->cmd.begin(); q != op->cmd.end(); ++q) fmt->dump_string("word", *q);
        fmt->close_section();
        if (op->target_osd >= 0)
...
int Objecter::RequestStateHook::call(std::string_view command, const cmdmap_t& cmdmap, const bufferlist&, Formatter* f,
                                     std::ostream& ss, cb::list& out)
{
...
void Objecter::handle_command_reply(MCommandReply* m)
{
    ...
    if (!c->session || m->get_connection() != c->session->con) {
    ...
    _finish_command(c,
                    m->r < 0 ? bs::error_code(-m->r, osd_category()) : bs::error_code(),
                    std::move(m->rs),
                    std::move(m->get_data()));
    sul.unlock();
...
Objecter::LingerOp::LingerOp(Objecter* o, uint64_t linger_id)
    : objecter(o)
    , linger_id(linger_id)
    , watch_lock(ceph::make_shared_mutex(fmt::format("LingerOp::watch_lock #{}", linger_id)))
{}
...
void Objecter::submit_command(CommandOp* c, ceph_tid_t* ptid)
{
    ...
    if (osd_timeout > timespan(0)) {
        c->ontimeout = timer.add_event(osd_timeout,
                                       [this, c, tid]() { command_op_cancel(c->session, tid, osdc_errc::timed_out); });
    }
    ...
    if (c->map_check_error) _send_command_map_check(c);
    if (ptid) *ptid = tid;

    logger->inc(l_osdc_command_active);
}

int Objecter::_calc_command_target(CommandOp* c, shunique_lock<ceph::shared_mutex>& sul)
{
    ceph_assert(sul.owns_lock() && sul.mutex() == &rwlock);
...
void Objecter::_assign_command_session(CommandOp* c, shunique_lock<ceph::shared_mutex>& sul)
{
    ceph_assert(sul.owns_lock() && sul.mutex() == &rwlock);
...
int Objecter::command_op_cancel(OSDSession* s, ceph_tid_t tid, bs::error_code ec)
{
    ceph_assert(initialized);
...
void Objecter::_finish_command(CommandOp* c, bs::error_code ec, string&& rs, cb::list&& bl)
{
    // rwlock is locked unique
    ...
    if (c->onfinish) c->onfinish->defer(std::move(c->onfinish), ec, std::move(rs), std::move(bl));

    if (c->ontimeout && ec != bs::errc::timed_out) timer.cancel_event(c->ontimeout);
...
Objecter::Objecter(CephContext* cct, Messenger* m, MonClient* mc, boost::asio::io_context& service)
    : Dispatcher(cct)
    , messenger(m)
    , monc(mc)
    , service(service)
...
void Objecter::set_epoch_barrier(epoch_t epoch)
{
    ...
    ldout(cct, 7) << __func__ << ": barrier " << epoch << " (was " << epoch_barrier << ") current epoch "
                  << osdmap->get_epoch() << dendl;
...
private:
    fu2::unique_function<void(bs::error_code, std::vector<T>, hobject_t) &&> on_finish;

public:
    epoch_t epoch = 0;
    int budget = -1;

    EnumerationContext(Objecter* objecter, hobject_t end, cb::list filter, uint32_t max, object_locator_t oloc,
    ...
    void operator()(bs::error_code ec, std::vector<T> v, hobject_t h) &&
    {
        if (budget >= 0) {
...
template<typename T> struct CB_EnumerateReply
{
    ...
    CB_EnumerateReply(Objecter* objecter, std::unique_ptr<EnumerationContext<T>>&& ctx)
        : objecter(objecter)
        , ctx(std::move(ctx))
    {}

    void operator()(bs::error_code ec) { objecter->_enumerate_reply(std::move(bl), ec, std::move(ctx)); }
};

template<typename T>
void Objecter::enumerate_objects(int64_t pool_id, std::string_view ns, hobject_t start, hobject_t end,
                                 const uint32_t max, const cb::list& filter_bl,
                                 fu2::unique_function<void(bs::error_code, std::vector<T>, hobject_t) &&> on_finish)
{
    ...
    if (!p) {
        lderr(cct) << __func__ << ": pool " << pool_id << " DNE in osd epoch " << osdmap->get_epoch() << dendl;
        rl.unlock();
    ...
    _issue_enumerate(start,
                     std::make_unique<EnumerationContext<T>>(
                         this, std::move(end), filter_bl, max, object_locator_t{pool_id, ns}, std::move(on_finish)));
}

template void Objecter::enumerate_objects<librados::ListObjectImpl>(
    int64_t pool_id, std::string_view ns, hobject_t start, hobject_t end, const uint32_t max, const cb::list& filter_bl,
    fu2::unique_function<void(bs::error_code, std::vector<librados::ListObjectImpl>, hobject_t) &&> on_finish);

template void Objecter::enumerate_objects<neorados::Entry>(
    int64_t pool_id, std::string_view ns, hobject_t start, hobject_t end, const uint32_t max, const cb::list& filter_bl,
    fu2::unique_function<void(bs::error_code, std::vector<neorados::Entry>, hobject_t) &&> on_finish);



template<typename T> void Objecter::_issue_enumerate(hobject_t start, std::unique_ptr<EnumerationContext<T>> ctx)
{
    ObjectOperation op;
    auto c = ctx.get();
    ...
    pg_read(
        ...
        pbl,
        0,
        Op::OpComp::create(service.get_executor(), [c = std::move(on_ack)](bs::error_code ec) mutable { (*c)(ec); }),
        epoch,
        budget);
}

template void Objecter::_issue_enumerate<librados::ListObjectImpl>(
    hobject_t start, std::unique_ptr<EnumerationContext<librados::ListObjectImpl>> ctx);
template void Objecter::_issue_enumerate<neorados::Entry>(hobject_t start,
                                                          std::unique_ptr<EnumerationContext<neorados::Entry>> ctx);

template<typename T>
void Objecter::_enumerate_reply(cb::list&& bl, bs::error_code ec, std::unique_ptr<EnumerationContext<T>>&& ctx)
{
    if (ec) {
        std::move (*ctx)(ec, {}, {});
        return;
    ...
    if ((response.handle <= ctx->end)) {
        ...
        while (!response.entries.empty()) {
            uint32_t hash = response.entries.back().locator.empty()
                                ? pool->hash_key(response.entries.back().oid, response.entries.back().nspace)
                                : pool->hash_key(response.entries.back().locator, response.entries.back().nspace);
    ...
    if (response.entries.size() <= ctx->max) {
        ...
        std::move(response.entries.begin(), response.entries.end(), std::back_inserter(ctx->ls));
    }
    else {
        ...
        uint32_t hash = i->locator.empty() ? pool->hash_key(i->oid, i->nspace) : pool->hash_key(i->locator, i->nspace);

        next = hobject_t{i->oid, i->locator, CEPH_NOSNAP, hash, ctx->oloc.get_pool(), i->nspace};
    }
...
template void Objecter::_enumerate_reply<librados::ListObjectImpl>(
    cb::list&& bl, bs::error_code ec, std::unique_ptr<EnumerationContext<librados::ListObjectImpl>>&& ctx);

template void Objecter::_enumerate_reply<neorados::Entry>(cb::list&& bl, bs::error_code ec,
                                                          std::unique_ptr<EnumerationContext<neorados::Entry>>&& ctx);

namespace {
using namespace librados;

template<typename T> void do_decode(std::vector<T>& items, std::vector<cb::list>& bls)
{
    for (auto bl : bls) {
        auto p = bl.cbegin();
        T t;
...
struct C_ObjectOperation_scrub_ls : public Context
{
    ...
    C_ObjectOperation_scrub_ls(uint32_t* interval, std::vector<inconsistent_obj_t>* objects, int* rval)
        : interval(interval)
        , objects(objects)
        , rval(rval)
    {}
    C_ObjectOperation_scrub_ls(uint32_t* interval, std::vector<inconsistent_snapset_t>* snapsets, int* rval)
        : interval(interval)
        , snapsets(snapsets)
        , rval(rval)
    {}
...
template<typename T>
void do_scrub_ls(::ObjectOperation* op, const scrub_ls_arg_t& arg, std::vector<T>* items, uint32_t* interval, int* rval)
{
...
void ::ObjectOperation::scrub_ls(const librados::object_id_t& start_after, uint64_t max_to_get,
...
void ::ObjectOperation::scrub_ls(const librados::object_id_t& start_after, uint64_t max_to_get,
                                 std::vector<librados::inconsistent_snapset_t>* snapsets, uint32_t* interval, int* rval)
{
