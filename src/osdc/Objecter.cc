...
public:
    ...
    int call(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, std::ostream& ss, cb::list& out) override;
};
...
void Objecter::_op_submit(Op* op, shunique_lock<ceph::shared_mutex>& sul, ceph_tid_t* ptid)
{
    ...
    bool check_for_latest_map = _calc_target(&op->target, nullptr) == RECALC_OP_TARGET_POOL_DNE;
        ...
        if ((t->flags & (CEPH_OSD_FLAG_BALANCE_READS | CEPH_OSD_FLAG_LOCALIZE_READS)) && !is_write &&
            ...
            if (t->flags & CEPH_OSD_FLAG_BALANCE_READS) {
                ...
                for (unsigned i = 0; i < acting.size(); ++i) {
                    ...
                    ldout(cct, 20) << __func__ << " localize: rank " << i << " osd." << acting[i] << " locality "
                                   << locality << dendl;
...
void Objecter::get_fs_stats(boost::optional<int64_t> poolid, decltype(StatfsOp::onfinish)&& onfinish)
{
...
int Objecter::RequestStateHook::call(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, std::ostream& ss,
                                     cb::list& out)
{
