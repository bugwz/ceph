...
#undef dout_prefix
#define dout_prefix \
    *_dout << objecter->messenger->get_myname() << ".journaler." << name << (readonly ? "(ro) " : "(rw) ")
...
public:
    explicit C_DelayFlush(Journaler* j)
        : journaler(j)
    {}
    void finish(int r) override { journaler->_do_delayed_flush(); }
};
...
void Journaler::create(file_layout_t* l, stream_format_t const sf)
{
    ...
    prezeroing_pos = prezero_pos = write_pos = flush_pos = safe_pos = read_pos = requested_pos = received_pos =
        expire_pos = trimming_pos = trimmed_pos = next_safe_pos = layout.get_period();

    ldout(cct, 1) << "created blank journal at inode 0x" << std::hex << ino << std::dec << ", format=" << stream_format
                  << dendl;
}
...
{
    return out << "loghead(trim " << h.trimmed_pos << ", expire " << h.expire_pos << ", write " << h.write_pos
...
public:
    bufferlist bl;
    explicit C_ReadHead(Journaler* l)
        : ls(l)
    {}
    void finish(int r) override { ls->_finish_read_head(r, bl); }
};
...
public:
    bufferlist bl;
    C_RereadHead(Journaler* l, Context* onfinish_)
        : ls(l)
        , onfinish(onfinish_)
    {}
    void finish(int r) override { ls->_finish_reread_head(r, bl, onfinish); }
};
...
public:
    uint64_t end;
    explicit C_ProbeEnd(Journaler* l)
        : ls(l)
        , end(-1)
    {}
    void finish(int r) override { ls->_finish_probe_end(r, end); }
};
...
public:
    ...
    C_ReProbe(Journaler* l, C_OnFinisher* onfinish_)
        ...
        , end(0)
    {}
    void finish(int r) override { ls->_finish_reprobe(r, end, onfinish); }
};
...
void Journaler::recover(Context* onread)
{
    ...
    if (onread) waitfor_recover.push_back(wrap_finisher(onread));
...
void Journaler::_finish_reread_head(int r, bufferlist& bl, Context* finish)
{
    ...
    if (r == 0) {
        ...
        prezeroing_pos = prezero_pos = write_pos = flush_pos = safe_pos = next_safe_pos = h.write_pos;
...
void Journaler::_probe(Context* finish, uint64_t* end)
{
    ...
    // probe the log
    filer.probe(ino, &layout, CEPH_NOSNAP, write_pos, end, true, 0, wrap_finisher(finish));
}
...
void Journaler::_finish_probe_end(int r, uint64_t end)
{
    ...
    if (((int64_t)end) == -1) {
        ...
        ldout(cct, 1) << "_finish_probe_end write_pos = " << end << " (header had " << write_pos
                      << "). log was empty. recovered." << dendl;
        ceph_abort();   // hrm.
    }
    else {
        ceph_assert(end >= write_pos);
        ldout(cct, 1) << "_finish_probe_end write_pos = " << end << " (header had " << write_pos << "). recovered."
                      << dendl;
    }
...
public:
    C_RereadHeadProbe(Journaler* l, C_OnFinisher* finish)
        ...
        , final_finish(finish)
    {}
    void finish(int r) override { ls->_finish_reread_head_and_probe(r, final_finish); }
};
...
public:
    ...
    C_WriteHead(Journaler* l, Header& h_, C_OnFinisher* c)
        : ls(l)
        , h(h_)
        , oncommit(c)
    {}
    void finish(int r) override { ls->_finish_write_head(r, h, oncommit); }
};
...
public:
    C_Flush(Journaler* l, int64_t s, ceph::real_time st)
        ...
        , stamp(st)
    {}
    void finish(int r) override { ls->_finish_flush(r, start, stamp); }
};
...
void Journaler::_finish_flush(int r, uint64_t start, ceph::real_time stamp)
{
    ...
    ldout(cct, 10) << "_finish_flush safe from " << start << ", pending_safe " << pending_safe
...
uint64_t Journaler::append_entry(bufferlist& bl)
{
    ...
    if (write_obj != flush_obj) {
        ldout(cct, 10) << " flushing completed object(s) (su " << su << " wro " << write_obj << " flo " << flush_obj
                       << ")" << dendl;
        ...
        if (write_buf.length() > 0 && write_buf.length() <= wrote) {   // the unflushed data are within this entry
...
void Journaler::_do_flush(unsigned amount)
{
    ...
    if (amount && amount < len) len = amount;
    ...
    if (flush_pos + len + 2 * period > prezero_pos) {
        ...
        if (static_cast<uint64_t>(newlen) < len) {
            ldout(cct, 10) << "_do_flush wanted to do " << flush_pos << "~" << len << " but hit prezero_pos "
                           << prezero_pos << ", will do " << flush_pos << "~" << newlen << dendl;
            waiting_for_zero_pos = flush_pos + len;
            len = newlen;
        }
    }
    ...
    if (len == write_buf.length()) {
        ...
        if (p != waitfor_safe.end()) {
            if (p->first > flush_pos + len && p != waitfor_safe.begin()) --p;
            if (p->first <= flush_pos + len && p->first > next_safe_pos) next_safe_pos = p->first;
        }
    }

    filer.write(
        ino, &layout, snapc, flush_pos, len, write_bl, ceph::real_clock::now(), 0, wrap_finisher(onsafe), write_iohint);
...
void Journaler::wait_for_flush(Context* onsafe)
{
    lock_guard l(lock);
    if (is_stopping()) {
        if (onsafe) onsafe->complete(-EAGAIN);
        return;
    }
...
void Journaler::_wait_for_flush(Context* onsafe)
{
    ...
    if (write_pos == safe_pos) {
        ...
        ldout(cct, 10) << "flush nothing to flush, (prezeroing/prezero)/write/flush/safe "
                          "pointers at "
                       << "(" << prezeroing_pos << "/" << prezero_pos << ")/" << write_pos << "/" << flush_pos << "/"
                       << safe_pos << dendl;
        if (onsafe) {
...
struct C_Journaler_Prezero : public Context
{
    ...
    C_Journaler_Prezero(Journaler* j, uint64_t f, uint64_t l)
        ...
        , len(l)
    {}
    void finish(int r) override { journaler->_finish_prezero(r, from, len); }
};
...
void Journaler::_issue_prezero()
{
    ...
    while (prezeroing_pos < to) {
        ...
        if (prezeroing_pos % period == 0) {
            len = period;
            ldout(cct, 10) << "_issue_prezero removing " << prezeroing_pos << "~" << period << " (full period)"
                           << dendl;
        }
        else {
        ...
        filer.zero(ino, &layout, snapc, prezeroing_pos, len, ceph::real_clock::now(), 0, c);
        prezeroing_pos += len;
    }
}
...
void Journaler::_finish_prezero(int r, uint64_t start, uint64_t len)
{
    ...
    ldout(cct, 10) << "_prezeroed to " << start << "~" << len << ", prezeroing/prezero was " << prezeroing_pos << "/"
                   << prezero_pos << ", pending " << pending_zero << dendl;
    if (r < 0 && r != -ENOENT) {
    ...
    if (start == prezero_pos) {
        prezero_pos += len;
        while (!pending_zero.empty() && pending_zero.begin().get_start() == prezero_pos) {
        ...
        if (prezero_pos == prezeroing_pos && !waitfor_prezero.empty()) {
            list<Context*> ls;
    ...
    ldout(cct, 10) << "_prezeroed prezeroing/prezero now " << prezeroing_pos << "/" << prezero_pos << ", pending "
                   << pending_zero << dendl;
}
...
public:
    bufferlist bl;
    C_Read(Journaler* j, uint64_t o, uint64_t l)
        : ls(j)
        , offset(o)
        , length(l)
    {}
    void finish(int r) override { ls->_finish_read(r, offset, length, bl); }
};
...
public:
    explicit C_RetryRead(Journaler* l)
        : ls(l)
    {}
...
void Journaler::_assimilate_prefetch()
{
    ...
    if (got_any) {
        ldout(cct, 10) << "_assimilate_prefetch read_buf now " << read_pos << "~" << read_buf.length()
                       ...
                       << " requested_pos=" << requested_pos << dendl;
    ...
    if ((got_any && !was_readable && readable) || read_pos == write_pos) {
        // readable!
        ldout(cct, 10) << "_finish_read now readable (or at journal end) readable=" << readable
                       << " read_pos=" << read_pos << " write_pos=" << write_pos << dendl;
        if (on_readable) {
            C_OnFinisher* f = on_readable;
            on_readable = 0;
            f->complete(0);
...
void Journaler::_issue_read(uint64_t len)
{
    ...
    // go.
    ldout(cct, 10) << "_issue_read reading " << requested_pos << "~" << len << ", read pointers read_pos=" << read_pos
                   << " received_pos=" << received_pos << " requested_pos+len=" << (requested_pos + len) << dendl;
...
void Journaler::_prefetch()
{
    ...
    if (target > write_pos) target = write_pos;
    ...
    if (requested_pos < target) {
        ...
        if (pending_safe.empty() && write_pos > safe_pos) {
            ...
            ldout(cct, 10) << "_prefetch: requested_pos=" << requested_pos << ", read_pos=" << read_pos
                           << ", write_pos=" << write_pos << ", safe_pos=" << safe_pos << dendl;
            _do_flush();
        }
...
bool Journaler::_have_next_entry()
{
    ...
    ldout(cct, 10) << "_have_next_entry read_buf.length() == " << read_buf.length() << ", but need " << need
    ...
    if (need > fetch_len) {
        temp_fetch_len = need;
        ldout(cct, 10) << "_have_next_entry noting temp_fetch_len " << temp_fetch_len << dendl;
    }
...
public:
    C_EraseFinish(Journaler* j, C_OnFinisher* c)
        : journaler(j)
        , completion(c)
    {}
    void finish(int r) override { journaler->_finish_erase(r, completion); }
};
...
void Journaler::_finish_erase(int data_result, C_OnFinisher* completion)
{
    ...
    if (data_result == 0) {
        ...
        filer.purge_range(ino, &layout, SnapContext(), 0, 1, ceph::real_clock::now(), 0, wrap_finisher(completion));
    }
    else {
...
bool Journaler::try_read_entry(bufferlist& bl)
{
    ...
    ldout(cct, 10) << "try_read_entry at " << read_pos << " read " << read_pos << "~" << consumed << " (have "
    ...
    if (bl.get_num_buffers() > 1) bl.rebuild();
    return true;
}
...
public:
    C_Trim(Journaler* l, int64_t t)
        : ls(l)
        , to(t)
    {}
    void finish(int r) override { ls->_finish_trim(r, to); }
};
...
void Journaler::_trim()
{
    ...
    ldout(cct, 10) << "trim trimming to " << trim_to << ", trimmed/trimming/expire are " << trimmed_pos << "/"
                   << trimming_pos << "/" << expire_pos << dendl;
    ...
    SnapContext snapc;
    filer.purge_range(
        ino, &layout, snapc, first, num, ceph::real_clock::now(), 0, wrap_finisher(new C_Trim(this, trim_to)));
...
void Journaler::_finish_trim(int r, uint64_t to)
{
    ...
    ceph_assert(!readonly);
    ldout(cct, 10) << "_finish_trim trimmed_pos was " << trimmed_pos << ", trimmed/trimming/expire now " << to << "/"
                   << trimming_pos << "/" << expire_pos << dendl;
    if (r < 0 && r != -ENOENT) {
...
void Journaler::check_isreadable()
{
    ...
    while (!_is_readable() && get_read_pos() < get_write_pos() && !get_error()) {
