...
struct C_HandleComplete : public Context
{
    ...
    explicit C_HandleComplete(ReplayHandler* r)
        : replay_handler(std::move(r))
    {}
    ~C_HandleComplete() override {}
    void finish(int r) override { replay_handler->handle_complete(r); }
};
...
struct C_HandleEntriesAvailable : public Context
{
    ...
    explicit C_HandleEntriesAvailable(ReplayHandler* r)
        : replay_handler(std::move(r))
    {}
    ~C_HandleEntriesAvailable() override {}
    void finish(int r) override { replay_handler->handle_entries_available(); }
};

}   // anonymous namespace

JournalPlayer::JournalPlayer(librados::IoCtx& ioctx, std::string_view object_oid_prefix,
                             ceph::ref_t<JournalMetadata> journal_metadata, ReplayHandler* replay_handler,
    ...
    , m_cache_rebalance_handler(this)
{
    ...
    if (m_cache_manager_handler != nullptr) {
        m_cache_name = "JournalPlayer/" + stringify(m_ioctx.get_id()) + "/" + m_object_oid_prefix;
        ...
        m_cache_manager_handler->register_cache(m_cache_name, min_size, max_size, &m_cache_rebalance_handler);
        m_max_fetch_bytes = 0;
    }
    else {
...
void JournalPlayer::prefetch()
{
    ...
    ldout(m_cct, 10) << __func__ << ": prefetching " << prefetch_object_numbers.size() << " "
                     << "objects" << dendl;
...
void JournalPlayer::shut_down(Context* on_finish)
{
    ...
    on_finish = utils::create_async_context_callback(m_journal_metadata, on_finish);
    ...
    if (m_watch_scheduled) {
        ...
        case WATCH_STEP_FETCH_FIRST:
            ...
            // fallthrough
        case WATCH_STEP_FETCH_CURRENT: object_player->unwatch(); break;
        case WATCH_STEP_ASSERT_ACTIVE: break;
        }
    }
...
bool JournalPlayer::try_pop_front(Entry* entry, uint64_t* commit_tid)
{
    ...
    if (m_journal_metadata->get_last_allocated_entry_tid(entry->get_tag_tid(), &last_entry_tid) &&
    ...
    *commit_tid = m_journal_metadata->allocate_commit_tid(
        object_player->get_object_number(), entry->get_tag_tid(), entry->get_entry_tid());
    return true;
}
...
void JournalPlayer::process_state(uint64_t object_number, int r)
{
    ...
    if (r >= 0) {
        ...
        case STATE_PLAYBACK:
            ...
            break;
        case STATE_ERROR: ldout(m_cct, 10) << "ERROR" << dendl; break;
        default:
...
int JournalPlayer::process_prefetch(uint64_t object_number)
{
    ...
    PrefetchSplayOffsets::iterator it = m_prefetch_splay_offsets.find(splay_offset);
    ...
    if (m_fetch_object_numbers.count(object_player->get_object_number()) == 0) {
        ...
        if (m_commit_positions.count(splay_offset) != 0 && !object_player->empty()) {
            ObjectPosition& position = m_commit_positions[splay_offset];

            ldout(m_cct, 15) << "seeking known commit position " << position << " in " << object_player->get_oid()
                             << dendl;
            ...
            while (!object_player->empty()) {
                ...
                if (entry.get_tag_tid() == position.tag_tid && entry.get_entry_tid() == position.entry_tid) {
                    found_commit = true;
                }
        ...
        if (!object_player->empty()) {
            ...
            if (!m_active_tag_tid) {
                ldout(m_cct, 10) << __func__ << ": "
                                 ...
                                 << "initial tag=" << entry.get_tag_tid() << dendl;
...
ceph::ref_t<ObjectPlayer> JournalPlayer::get_object_player() const
{
    ...
    SplayedObjectPlayers::const_iterator it = m_object_players.find(m_splay_offset);
...
void JournalPlayer::fetch(uint64_t object_num)
{
    ...
    auto object_player = ceph::make_ref<ObjectPlayer>(m_ioctx,
                                                      m_object_oid_prefix,
                                                      object_num,
                                                      m_journal_metadata->get_timer(),
                                                      m_journal_metadata->get_timer_lock(),
                                                      m_journal_metadata->get_order(),
                                                      m_max_fetch_bytes);
...
{
    ldout(m_cct, 10) << __func__ << ": " << utils::get_object_name(m_object_oid_prefix, object_num) << ": r=" << r
                     << dendl;
...
void JournalPlayer::schedule_watch(bool immediate)
{
    ...
    if (m_watch_step == WATCH_STEP_ASSERT_ACTIVE) {
        ...
        auto ctx = new LambdaContext([this](int r) { handle_watch_assert_active(r); });
    ...
    case WATCH_STEP_FETCH_CURRENT:
    {
        ...
        if (immediate || (object_player->get_refetch_state() == ObjectPlayer::REFETCH_STATE_IMMEDIATE) ||
    ...
    Context* ctx =
        utils::create_async_context_callback(m_journal_metadata, new C_Watch(this, object_player->get_object_number()));
...
void JournalPlayer::handle_watch(uint64_t object_num, int r)
{
    ...
    uint8_t splay_width = m_journal_metadata->get_splay_width();
    if (m_watch_step == WATCH_STEP_FETCH_CURRENT && object_player->get_object_number() % splay_width != 0) {
...
void JournalPlayer::handle_cache_rebalanced(uint64_t new_cache_bytes)
{
    ...
    ldout(m_cct, 10) << __func__ << ": new_cache_bytes=" << new_cache_bytes << ", max_fetch_bytes=" << m_max_fetch_bytes
                     << dendl;
    ...
    if (m_max_fetch_bytes < min_bytes) {
        lderr(m_cct) << __func__ << ": can't allocate enough memory from cache" << dendl;
        m_state = STATE_ERROR;
