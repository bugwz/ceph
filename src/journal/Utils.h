...
template<typename M> struct C_AsyncCallback : public Context
{
    M journal_metadata;
    Context* on_finish;
    ...
    C_AsyncCallback(M journal_metadata, Context* on_finish)
        ...
        , on_finish(on_finish)
    {}
    void finish(int r) override { journal_metadata->queue(on_finish, r); }
};

}   // namespace detail

template<typename T, void (T::*MF)(int)> void rados_state_callback(rados_completion_t c, void* arg)
{
...
template<typename M> Context* create_async_context_callback(M journal_metadata, Context* on_finish)
{
