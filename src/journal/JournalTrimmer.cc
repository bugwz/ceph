...
struct JournalTrimmer::C_RemoveSet : public Context
{
    ...
    C_RemoveSet(JournalTrimmer* _journal_trimmer, uint64_t _object_set, uint8_t _splay_width);
...
JournalTrimmer::JournalTrimmer(librados::IoCtx& ioctx, const std::string& object_oid_prefix,
...
void JournalTrimmer::committed(uint64_t commit_tid)
{
    ...
    m_journal_metadata->committed(commit_tid, m_create_commit_position_safe_context);
}
...
void JournalTrimmer::remove_set(uint64_t object_set)
{
    ...
    for (uint64_t object_number = object_set * splay_width; object_number < (object_set + 1) * splay_width;
         ++object_number) {
        ...
        int r = m_ioctx.aio_remove(oid, comp, CEPH_OSD_FLAG_FULL_FORCE | CEPH_OSD_FLAG_FULL_TRY);
        ceph_assert(r == 0);
        comp->release();
    }
}
...
JournalTrimmer::C_RemoveSet::C_RemoveSet(JournalTrimmer* _journal_trimmer, uint64_t _object_set, uint8_t _splay_width)
