...
struct C_GetClient : public Context
{
    ...
    C_GetClient(CephContext* cct, librados::IoCtx& ioctx, const std::string& oid, AsyncOpTracker& async_op_tracker,
                const std::string& client_id, cls::journal::Client* client, Context* on_finish)
        : cct(cct)
        , ioctx(ioctx)
        , oid(oid)
    ...
    ~C_GetClient() override { async_op_tracker.finish_op(); }

    virtual void send() { send_get_client(); }
    ...
    void send_get_client()
    {
        ...
        librados::AioCompletion* comp = librados::Rados::aio_create_completion(
            this, &utils::rados_state_callback<C_GetClient, &C_GetClient::handle_get_client>);
...
struct C_AllocateTag : public Context
{
    ...
    C_AllocateTag(CephContext* cct, librados::IoCtx& ioctx, const std::string& oid, AsyncOpTracker& async_op_tracker,
                  uint64_t tag_class, const bufferlist& data, Tag* tag, Context* on_finish)
        : cct(cct)
        , ioctx(ioctx)
        , oid(oid)
        , async_op_tracker(async_op_tracker)
    ...
    ~C_AllocateTag() override { async_op_tracker.finish_op(); }

    void send() { send_get_next_tag_tid(); }
    ...
    void send_tag_create()
    {
        ...
        librados::AioCompletion* comp = librados::Rados::aio_create_completion(
            this, &utils::rados_state_callback<C_AllocateTag, &C_AllocateTag::handle_tag_create>);
    ...
    void send_get_tag()
    {
        ...
        librados::AioCompletion* comp = librados::Rados::aio_create_completion(
            this, &utils::rados_state_callback<C_AllocateTag, &C_AllocateTag::handle_get_tag>);
...
struct C_GetTag : public Context
{
    ...
    C_GetTag(CephContext* cct, librados::IoCtx& ioctx, const std::string& oid, AsyncOpTracker& async_op_tracker,
             uint64_t tag_tid, JournalMetadata::Tag* tag, Context* on_finish)
        : cct(cct)
        , ioctx(ioctx)
        , oid(oid)
        , async_op_tracker(async_op_tracker)
    ...
    ~C_GetTag() override { async_op_tracker.finish_op(); }

    void send() { send_get_tag(); }
    ...
    void send_get_tag()
    {
        ...
        librados::AioCompletion* comp = librados::Rados::aio_create_completion(
            this, &utils::rados_state_callback<C_GetTag, &C_GetTag::handle_get_tag>);
...
struct C_GetTags : public Context
{
    ...
    C_GetTags(CephContext* cct, librados::IoCtx& ioctx, const std::string& oid, const std::string& client_id,
              AsyncOpTracker& async_op_tracker, uint64_t start_after_tag_tid,
              const boost::optional<uint64_t>& tag_class, JournalMetadata::Tags* tags, Context* on_finish)
        : cct(cct)
        , ioctx(ioctx)
        , oid(oid)
        , client_id(client_id)
        , async_op_tracker(async_op_tracker)
        , start_after_tag_tid(start_after_tag_tid)
        , tag_class(tag_class)
        , tags(tags)
        , on_finish(on_finish)
    {
    ...
    ~C_GetTags() override { async_op_tracker.finish_op(); }

    void send() { send_tag_list(); }

    void send_tag_list()
    {
        librados::ObjectReadOperation op;
        client::tag_list_start(&op, start_after_tag_tid, MAX_RETURN, client_id, tag_class);

        librados::AioCompletion* comp = librados::Rados::aio_create_completion(
            this, &utils::rados_state_callback<C_GetTags, &C_GetTags::handle_tag_list>);
...
struct C_FlushCommitPosition : public Context
{
    ...
    C_FlushCommitPosition(Context* commit_position_ctx, Context* on_finish)
        : commit_position_ctx(commit_position_ctx)
        , on_finish(on_finish)
    {}
...
struct C_AssertActiveTag : public Context
{
    ...
    C_AssertActiveTag(CephContext* cct, librados::IoCtx& ioctx, const std::string& oid,
                      AsyncOpTracker& async_op_tracker, const std::string& client_id, uint64_t tag_tid,
                      Context* on_finish)
        : cct(cct)
        , ioctx(ioctx)
        , oid(oid)
    ...
    ~C_AssertActiveTag() override { async_op_tracker.finish_op(); }
    ...
    void send()
    {
        ...
        librados::AioCompletion* comp = librados::Rados::aio_create_completion(
            this, &utils::rados_state_callback<C_AssertActiveTag, &C_AssertActiveTag::handle_send>);
...
JournalMetadata::JournalMetadata(ContextWQ* work_queue, SafeTimer* timer, ceph::mutex* timer_lock,
                                 librados::IoCtx& ioctx, const std::string& oid, const std::string& client_id,
...
void JournalMetadata::init(Context* on_finish)
{
    ...
    on_finish = utils::create_async_context_callback(this, on_finish);
    on_finish = new C_ImmutableMetadata(this, on_finish);
    on_finish = new LambdaContext([this, on_finish](int r) {
        if (r < 0) {
            lderr(m_cct) << __func__ << ": failed to watch journal" << cpp_strerror(r) << dendl;
            std::lock_guard locker{m_lock};
            m_watch_handle = 0;
            on_finish->complete(r);
            return;
    ...
    librados::AioCompletion* comp = librados::Rados::aio_create_completion(on_finish, utils::rados_ctx_callback);
...
void JournalMetadata::shut_down(Context* on_finish)
{
    ...
    on_finish = new LambdaContext([this, on_finish](int r) { flush_commit_position(on_finish); });
    if (watch_handle != 0) {
        librados::AioCompletion* comp = librados::Rados::aio_create_completion(on_finish, utils::rados_ctx_callback);
...
void JournalMetadata::get_immutable_metadata(uint8_t* order, uint8_t* splay_width, int64_t* pool_id, Context* on_finish)
{
    client::get_immutable_metadata(m_ioctx, m_oid, order, splay_width, pool_id, on_finish);
}

void JournalMetadata::get_mutable_metadata(uint64_t* minimum_set, uint64_t* active_set, RegisteredClients* clients,
                                           Context* on_finish)
{
    client::get_mutable_metadata(m_ioctx, m_oid, minimum_set, active_set, clients, on_finish);
}

void JournalMetadata::register_client(const bufferlist& data, Context* on_finish)
{
    ...
    librados::AioCompletion* comp = librados::Rados::aio_create_completion(ctx, utils::rados_ctx_callback);
    int r = m_ioctx.aio_operate(m_oid, comp, &op);
    ceph_assert(r == 0);
    comp->release();
}

void JournalMetadata::update_client(const bufferlist& data, Context* on_finish)
{
    ldout(m_cct, 10) << __func__ << ": " << m_client_id << dendl;
    librados::ObjectWriteOperation op;
...
void JournalMetadata::allocate_tag(uint64_t tag_class, const bufferlist& data, Tag* tag, Context* on_finish)
{
    on_finish = new C_NotifyUpdate(this, on_finish);
    C_AllocateTag* ctx = new C_AllocateTag(m_cct, m_ioctx, m_oid, m_async_op_tracker, tag_class, data, tag, on_finish);
    ctx->send();
}

void JournalMetadata::get_client(const std::string& client_id, cls::journal::Client* client, Context* on_finish)
{
    C_GetClient* ctx = new C_GetClient(m_cct, m_ioctx, m_oid, m_async_op_tracker, client_id, client, on_finish);
    ctx->send();
}

void JournalMetadata::get_tag(uint64_t tag_tid, Tag* tag, Context* on_finish)
{
    C_GetTag* ctx = new C_GetTag(m_cct, m_ioctx, m_oid, m_async_op_tracker, tag_tid, tag, on_finish);
    ctx->send();
}

void JournalMetadata::get_tags(uint64_t start_after_tag_tid, const boost::optional<uint64_t>& tag_class, Tags* tags,
                               Context* on_finish)
{
    C_GetTags* ctx = new C_GetTags(
        m_cct, m_ioctx, m_oid, m_client_id, m_async_op_tracker, start_after_tag_tid, tag_class, tags, on_finish);
    ctx->send();
}
...
void JournalMetadata::add_listener(JournalMetadataListener* listener)
{
    ...
    m_update_cond.wait(locker, [this] { return m_update_notifications <= 0; });
...
void JournalMetadata::assert_active_tag(uint64_t tag_tid, Context* on_finish)
{
    ...
    C_AssertActiveTag* ctx =
        new C_AssertActiveTag(m_cct, m_ioctx, m_oid, m_async_op_tracker, m_client_id, tag_tid, on_finish);
    ctx->send();
}
...
bool JournalMetadata::get_last_allocated_entry_tid(uint64_t tag_tid, uint64_t* entry_tid) const
{
    std::lock_guard locker{m_lock};
...
{
    if (r < 0) {
        lderr(m_cct) << "failed to initialize immutable metadata: " << cpp_strerror(r) << dendl;
        on_init->complete(r);
        return;
    }
...
void JournalMetadata::refresh(Context* on_complete)
{
    ...
    get_mutable_metadata(&refresh->minimum_set, &refresh->active_set, &refresh->registered_clients, refresh);
}
...
void JournalMetadata::schedule_commit_task()
{
    ...
    if (m_commit_position_task_ctx == nullptr) {
        m_commit_position_task_ctx =
            m_timer->add_event_after(m_settings.commit_interval, new C_CommitPositionTask(this));
    }
}
...
void JournalMetadata::handle_commit_position_task()
{
    ...
    ctx = new LambdaContext([this, ctx](int r) { schedule_laggy_clients_disconnect(ctx); });
...
uint64_t JournalMetadata::allocate_commit_tid(uint64_t object_num, uint64_t tag_tid, uint64_t entry_tid)
{
    std::lock_guard locker{m_lock};
    uint64_t commit_tid = ++m_commit_tid;
    m_pending_commit_tids[commit_tid] = CommitEntry(object_num, tag_tid, entry_tid);

    ldout(m_cct, 20) << "allocated commit tid: commit_tid=" << commit_tid << " ["
                     << "object_num=" << object_num << ", "
                     << "tag_tid=" << tag_tid << ", "
                     << "entry_tid=" << entry_tid << "]" << dendl;
    return commit_tid;
}

void JournalMetadata::overflow_commit_tid(uint64_t commit_tid, uint64_t object_num)
{
    std::lock_guard locker{m_lock};
...
void JournalMetadata::get_commit_entry(uint64_t commit_tid, uint64_t* object_num, uint64_t* tag_tid,
                                       uint64_t* entry_tid)
{
    std::lock_guard locker{m_lock};
...
void JournalMetadata::committed(uint64_t commit_tid, const CreateContext& create_context)
{
    ...
    Context* stale_ctx = nullptr;
    {
        ...
        while (!m_pending_commit_tids.empty()) {
            ...
            commit_position.object_positions.emplace_front(
                commit_entry.object_num, commit_entry.tag_tid, commit_entry.entry_tid);
...
void JournalMetadata::schedule_laggy_clients_disconnect(Context* on_finish)
{
    ...
    Context* ctx = on_finish;
    {
        ...
        for (auto& c : m_registered_clients) {
            if (c.state == cls::journal::CLIENT_STATE_DISCONNECTED || c.id == m_client_id ||
            ...
            if (m_active_set > object_set + m_settings.max_concurrent_object_sets) {
                ...
                ctx = new LambdaContext([this, client_id, ctx](int r1) {
                    ...
                    librados::ObjectWriteOperation op;
                    client::client_update_state(&op, client_id, cls::journal::CLIENT_STATE_DISCONNECTED);
...
std::ostream& operator<<(std::ostream& os, const JournalMetadata::RegisteredClients& clients)
{
    os << "[";
    for (JournalMetadata::RegisteredClients::const_iterator c = clients.begin(); c != clients.end(); ++c) {
...
std::ostream& operator<<(std::ostream& os, const JournalMetadata& jm)
{
