...
bool advance_to_last_pad_byte(uint32_t off, bufferlist::const_iterator* iter, uint32_t* pad_len, bool* partial_entry)
{
...
ObjectPlayer::ObjectPlayer(librados::IoCtx& ioctx, const std::string& object_oid_prefix, uint64_t object_num,
                           SafeTimer& timer, ceph::mutex& timer_lock, uint8_t order, uint64_t max_fetch_bytes)
...
void ObjectPlayer::fetch(Context* on_finish)
{
    ...
    auto rados_completion = librados::Rados::aio_create_completion(context, utils::rados_ctx_callback);
...
int ObjectPlayer::handle_fetch_complete(int r, const bufferlist& bl, bool* refetch)
{
    ...
    while (!iter.end()) {
        ...
        if (!Entry::is_readable(iter, &bytes_needed)) {
            if (bytes_needed != 0) {
                ...
                partial_entry = true;
                if (full_fetch) {
                    lderr(m_cct) << ": partial record at offset " << invalid_start_off << dendl;
                }
                else {
            ...
            if (!advance_to_last_pad_byte(m_read_bl_off + iter.get_off(), &iter, &pad_len, &partial_entry)) {
                invalid_start_off = m_read_bl_off + bl_off;
                invalid = true;
                if (partial_entry) {
                    if (full_fetch) {
                        lderr(m_cct) << ": partial pad at offset " << invalid_start_off << dendl;
                    }
                    else {
                    ...
                    lderr(m_cct) << ": detected corrupt journal entry at offset " << invalid_start_off << dendl;
                }
                break;
            }
            ++iter;
        ...
        if (invalid) {
            ...
            lderr(m_cct) << ": corruption range [" << invalid_start_off << ", " << invalid_end_off << ")" << dendl;
            m_invalid_ranges.insert(invalid_start_off, invalid_end_off - invalid_start_off);
            invalid = false;

            m_read_bl_off = invalid_end_off;
        }

        EntryKey entry_key(std::make_pair(entry.get_tag_tid(), entry.get_entry_tid()));
        ...
        bufferlist sub_bl;
        sub_bl.substr_of(m_read_bl, iter.get_off(), m_read_bl.length() - iter.get_off());
...
void ObjectPlayer::schedule_watch()
{
    ...
    m_watch_task = m_timer.add_event_after(m_watch_interval, new LambdaContext([this](int) { handle_watch_task(); }));
}
