...
public:
    ...
    inline uint64_t get_object_number() const { return m_object_number; }
    ...
    inline CephContext* cct() const { return m_cct; }
...
private:
    ...
    ObjectRecorder(librados::IoCtx& ioctx, std::string_view oid, uint64_t object_number, ceph::mutex* lock,
    ...
    struct FlushHandler : public FutureImpl::FlushHandler
    {
        ...
        virtual void flush(const ceph::ref_t<FutureImpl>& future) override { object_recorder->flush(future); }
        FlushHandler(ceph::ref_t<ObjectRecorder> o)
    ...
    struct C_AppendFlush : public Context
    {
        ...
        C_AppendFlush(ceph::ref_t<ObjectRecorder> o, uint64_t _tid)
            ...
            , tid(_tid)
        {}
        void finish(int r) override { object_recorder->handle_append_flushed(tid, r); }
    };
    ...
    void notify_handler_unlock(std::unique_lock<ceph::mutex>& locker, bool notify_overflowed);
};
