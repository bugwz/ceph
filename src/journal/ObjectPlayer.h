...
public:
    ...
    inline uint64_t get_object_number() const { return m_object_num; }
    ...
    inline bool refetch_required() const { return (get_refetch_state() != REFETCH_STATE_NONE); }
    inline RefetchState get_refetch_state() const { return m_refetch_state; }
    inline void set_refetch_state(RefetchState refetch_state) { m_refetch_state = refetch_state; }
...
private:
    ...
    ObjectPlayer(librados::IoCtx& ioctx, const std::string& object_oid_prefix, uint64_t object_num, SafeTimer& timer,
                 ceph::mutex& timer_lock, uint8_t order, uint64_t max_fetch_bytes);
