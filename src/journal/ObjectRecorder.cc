...
#undef dout_prefix
#define dout_prefix *_dout << "ObjectRecorder: " << this << " " << __func__ << " (" << m_oid << "): "
...
ObjectRecorder::ObjectRecorder(librados::IoCtx& ioctx, std::string_view oid, uint64_t object_number, ceph::mutex* lock,
                               ContextWQ* work_queue, Handler* handler, uint8_t order, int32_t max_in_flight_appends)
    : m_oid(oid)
...
void ObjectRecorder::set_append_batch_options(int flush_interval, uint64_t flush_bytes, double flush_age)
{
...
void ObjectRecorder::flush(Context* on_safe)
{
    ...
    if (future.is_valid()) {
        ...
        m_op_work_queue->queue(new LambdaContext([future, on_safe](int r) mutable { future.flush(on_safe); }));
    }
    else {
        on_safe->complete(0);
    }
...
void ObjectRecorder::claim_append_buffers(AppendBuffers* append_buffers)
{
    ...
    append_buffers->splice(
        append_buffers->end(), m_pending_buffers, m_pending_buffers.begin(), m_pending_buffers.end());
}
...
void ObjectRecorder::append_overflowed()
{
    ...
    for (InFlightAppends::iterator it = in_flight_appends.begin(); it != in_flight_appends.end(); ++it) {
        restart_append_buffers.insert(restart_append_buffers.end(), it->second.begin(), it->second.end());
    }

    restart_append_buffers.splice(
        restart_append_buffers.end(), m_pending_buffers, m_pending_buffers.begin(), m_pending_buffers.end());
...
bool ObjectRecorder::send_appends(bool force, ceph::ref_t<FutureImpl> flush_future)
{
    ...
    if (!force &&
        ((m_flush_interval > 0 && m_pending_buffers.size() >= m_flush_interval) ||
         ...
         (m_flush_age > 0 && !m_last_flush_time.is_zero() && m_last_flush_time + m_flush_age <= ceph_clock_now()))) {
    ...
    if (append_bytes > 0) {
        ...
        auto rados_completion =
            librados::Rados::aio_create_completion(new C_AppendFlush(this, append_tid), utils::rados_ctx_callback);
...
void ObjectRecorder::notify_handler_unlock(std::unique_lock<ceph::mutex>& locker, bool notify_overflowed)
{
    ceph_assert(ceph_mutex_is_locked(*m_lock));
