...
std::string Journaler::object_oid_prefix(int pool_id, const std::string& journal_id)
{
...
Journaler::Threads::Threads(CephContext* cct)
{
    ...
    work_queue = new ContextWQ("Journaler::work_queue", ceph::make_timespan(60), thread_pool);
...
Journaler::Journaler(librados::IoCtx& header_ioctx, const std::string& journal_id, const std::string& client_id,
                     const Settings& settings, CacheManagerHandler* cache_manager_handler)
    : m_threads(new Threads(reinterpret_cast<CephContext*>(header_ioctx.cct())))
    , m_client_id(client_id)
    , m_cache_manager_handler(cache_manager_handler)
{
    set_up(m_threads->work_queue, m_threads->timer, &m_threads->timer_lock, header_ioctx, journal_id, settings);
}

Journaler::Journaler(ContextWQ* work_queue, SafeTimer* timer, ceph::mutex* timer_lock, librados::IoCtx& header_ioctx,
                     const std::string& journal_id, const std::string& client_id, const Settings& settings,
                     CacheManagerHandler* cache_manager_handler)
    : m_client_id(client_id)
    , m_cache_manager_handler(cache_manager_handler)
{
    set_up(work_queue, timer, timer_lock, header_ioctx, journal_id, settings);
}

void Journaler::set_up(ContextWQ* work_queue, SafeTimer* timer, ceph::mutex* timer_lock, librados::IoCtx& header_ioctx,
                       const std::string& journal_id, const Settings& settings)
{
    ...
    m_metadata = ceph::make_ref<JournalMetadata>(
        work_queue, timer, timer_lock, m_header_ioctx, m_header_oid, m_client_id, settings);
}
...
void Journaler::exists(Context* on_finish) const
{
    ...
    librados::AioCompletion* comp = librados::Rados::aio_create_completion(on_finish, rados_ctx_callback);
...
void Journaler::shut_down(Context* on_finish)
{
    ...
    on_finish = new LambdaContext([metadata, on_finish](int r) { on_finish->complete(0); });
...
void Journaler::get_immutable_metadata(uint8_t* order, uint8_t* splay_width, int64_t* pool_id, Context* on_finish)
{
    m_metadata->get_immutable_metadata(order, splay_width, pool_id, on_finish);
}

void Journaler::get_mutable_metadata(uint64_t* minimum_set, uint64_t* active_set, RegisteredClients* clients,
                                     Context* on_finish)
{
    m_metadata->get_mutable_metadata(minimum_set, active_set, clients, on_finish);
}

void Journaler::create(uint8_t order, uint8_t splay_width, int64_t pool_id, Context* on_finish)
{
...
void Journaler::remove(bool force, Context* on_finish)
{
    ...
    on_finish = new LambdaContext([this, force, on_finish](int r) { m_trimmer->remove_objects(force, on_finish); });
...
void Journaler::get_client(const std::string& client_id, cls::journal::Client* client, Context* on_finish)
{
    m_metadata->get_client(client_id, client, on_finish);
}

int Journaler::get_cached_client(const std::string& client_id, cls::journal::Client* client)
{
...
void Journaler::allocate_tag(const bufferlist& data, cls::journal::Tag* tag, Context* on_finish)
{
    m_metadata->allocate_tag(cls::journal::Tag::TAG_CLASS_NEW, data, tag, on_finish);
}

void Journaler::allocate_tag(uint64_t tag_class, const bufferlist& data, cls::journal::Tag* tag, Context* on_finish)
{
...
void Journaler::get_tags(uint64_t start_after_tag_tid, uint64_t tag_class, Tags* tags, Context* on_finish)
{
...
void Journaler::start_live_replay(ReplayHandler* replay_handler, double interval)
{
    create_player(replay_handler);
    m_player->prefetch_and_watch(interval);
}

bool Journaler::try_pop_front(ReplayEntry* replay_entry, uint64_t* tag_tid)
{
    ceph_assert(m_player != nullptr);
...
void Journaler::stop_replay(Context* on_finish)
{
    ...
    auto f = [player = std::move(player), on_finish](int r) { on_finish->complete(r); };
...
void Journaler::start_append(uint64_t max_in_flight_appends)
{
    ...
    m_recorder =
        std::make_unique<JournalRecorder>(m_data_ioctx, m_object_oid_prefix, m_metadata, max_in_flight_appends);
}

void Journaler::set_append_batch_options(int flush_interval, uint64_t flush_bytes, double flush_age)
{
    ceph_assert(m_recorder != nullptr);
...
void Journaler::stop_append(Context* on_safe)
{
    ...
    on_safe = new LambdaContext([recorder = std::move(recorder), on_safe](int r) { on_safe->complete(r); });
    recorderp->shut_down(on_safe);
}

uint64_t Journaler::get_max_append_size() const
{
    uint64_t max_payload_size = m_metadata->get_object_size() - Entry::get_fixed_size();
    if (m_metadata->get_settings().max_payload_bytes > 0) {
        max_payload_size = std::min(max_payload_size, m_metadata->get_settings().max_payload_bytes);
    }
...
{
    ceph_assert(m_player == nullptr);
    m_player = std::make_unique<JournalPlayer>(
        m_data_ioctx, m_object_oid_prefix, m_metadata, replay_handler, m_cache_manager_handler);
}

void Journaler::get_metadata(uint8_t* order, uint8_t* splay_width, int64_t* pool_id)
{
...
std::ostream& operator<<(std::ostream& os, const Journaler& journaler)
{
    os << "[metadata=";
