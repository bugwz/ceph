...
public:
    ...
    void get_immutable_metadata(uint8_t* order, uint8_t* splay_width, int64_t* pool_id, Context* on_finish);

    void get_mutable_metadata(uint64_t* minimum_set, uint64_t* active_set, RegisteredClients* clients,
                              Context* on_finish);
    ...
    void allocate_tag(uint64_t tag_class, const bufferlist& data, Tag* tag, Context* on_finish);
    void get_tag(uint64_t tag_tid, Tag* tag, Context* on_finish);
    void get_tags(uint64_t start_after_tag_tid, const boost::optional<uint64_t>& tag_class, Tags* tags,
                  Context* on_finish);

    inline const Settings& get_settings() const { return m_settings; }
    inline const std::string& get_client_id() const { return m_client_id; }
    inline uint8_t get_order() const { return m_order; }
    inline uint64_t get_object_size() const { return 1 << m_order; }
    inline uint8_t get_splay_width() const { return m_splay_width; }
    inline int64_t get_pool_id() const { return m_pool_id; }

    inline void queue(Context* on_finish, int r) { m_work_queue->queue(on_finish, r); }

    inline ContextWQ* get_work_queue() { return m_work_queue; }

    inline SafeTimer& get_timer() { return *m_timer; }
    inline ceph::mutex& get_timer_lock() { return *m_timer_lock; }
    ...
    uint64_t allocate_commit_tid(uint64_t object_num, uint64_t tag_tid, uint64_t entry_tid);
    void overflow_commit_tid(uint64_t commit_tid, uint64_t object_num);
    void get_commit_entry(uint64_t commit_tid, uint64_t* object_num, uint64_t* tag_tid, uint64_t* entry_tid);
...
private:
    ...
    JournalMetadata(ContextWQ* work_queue, SafeTimer* timer, ceph::mutex* timer_lock, librados::IoCtx& ioctx,
                    const std::string& oid, const std::string& client_id, const Settings& settings);
    ...
    struct C_WatchCtx : public librados::WatchCtx2
    {
        ...
        void handle_error(uint64_t cookie, int err) override { journal_metadata->handle_watch_error(err); }
    };
    ...
    struct C_WatchReset : public Context
    {
        ...
        ~C_WatchReset() override { journal_metadata->m_async_op_tracker.finish_op(); }
        void finish(int r) override { journal_metadata->handle_watch_reset(); }
    };
    ...
    struct C_CommitPositionTask : public Context
    {
        ...
        ~C_CommitPositionTask() override { journal_metadata->m_async_op_tracker.finish_op(); }
    ...
    struct C_AioNotify : public Context
    {
        ...
        ~C_AioNotify() override { journal_metadata->m_async_op_tracker.finish_op(); }
        void finish(int r) override
        {
    ...
    struct C_NotifyUpdate : public Context
    {
        ...
        ~C_NotifyUpdate() override { journal_metadata->m_async_op_tracker.finish_op(); }
        void finish(int r) override
        {
            if (r == 0) {
    ...
    struct C_ImmutableMetadata : public Context
    {
        ...
        ~C_ImmutableMetadata() override { journal_metadata->m_async_op_tracker.finish_op(); }
        void finish(int r) override { journal_metadata->handle_immutable_metadata(r, on_finish); }
    };
    ...
    struct C_Refresh : public Context
    {
        ...
        ~C_Refresh() override { journal_metadata->m_async_op_tracker.finish_op(); }
        void finish(int r) override { journal_metadata->handle_refresh_complete(this, r); }
    };
    ...
    friend std::ostream& operator<<(std::ostream& os, const JournalMetadata& journal_metadata);
};

std::ostream& operator<<(std::ostream& os, const JournalMetadata::RegisteredClients& clients);

std::ostream& operator<<(std::ostream& os, const JournalMetadata& journal_metadata);
