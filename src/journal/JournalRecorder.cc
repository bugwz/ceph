...
struct C_Flush : public Context
{
    ...
    C_Flush(ceph::ref_t<JournalMetadata> _journal_metadata, Context* _on_finish, size_t _pending_flushes)
...
JournalRecorder::JournalRecorder(librados::IoCtx& ioctx, std::string_view object_oid_prefix,
                                 ceph::ref_t<JournalMetadata> journal_metadata, uint64_t max_in_flight_appends)
    ...
    , m_object_locks{
          ceph::make_lock_container<ceph::mutex>(m_journal_metadata->get_splay_width(), [](const size_t splay_offset) {
              return ceph::make_mutex("ObjectRecorder::m_lock::" + std::to_string(splay_offset));
          })}
{
    ...
    for (uint8_t splay_offset = 0; splay_offset < splay_width; ++splay_offset) {
        ...
        m_object_ptrs[splay_offset] = create_object_recorder(object_number, &m_object_locks[splay_offset]);
    }
    ...
    on_safe = new LambdaContext([this, on_safe](int r) {
        Context* ctx = nullptr;
        {
            ...
            if (m_in_flight_advance_sets != 0) {
                ...
                m_on_object_set_advanced = new LambdaContext([on_safe, r](int) { on_safe->complete(r); });
            }
            else {
                ctx = on_safe;
            }
...
void JournalRecorder::set_append_batch_options(int flush_interval, uint64_t flush_bytes, double flush_age)
{
    ...
    for (uint8_t splay_offset = 0; splay_offset < splay_width; ++splay_offset) {
        ...
        object_recorder->set_append_batch_options(flush_interval, flush_bytes, flush_age);
    }
}

Future JournalRecorder::append(uint64_t tag_tid, const bufferlist& payload_bl)
{
    ...
    uint64_t commit_tid = m_journal_metadata->allocate_commit_tid(object_ptr->get_object_number(), tag_tid, entry_tid);
    ...
    bufferlist entry_bl;
    encode(Entry(future->get_tag_tid(), future->get_entry_tid(), payload_bl), entry_bl);
...
void JournalRecorder::advance_object_set()
{
    ...
    m_journal_metadata->set_active_set(m_current_set, new C_AdvanceObjectSet(this));
}
...
void JournalRecorder::handle_advance_object_set(int r)
{
    Context* on_object_set_advanced = nullptr;
    {
        ...
        if (r < 0 && r != -ESTALE) {
            lderr(m_cct) << "failed to advance object set: " << cpp_strerror(r) << dendl;
        }
...
ceph::ref_t<ObjectRecorder> JournalRecorder::create_object_recorder(uint64_t object_number, ceph::mutex* lock)
{
    ldout(m_cct, 10) << "object_number=" << object_number << dendl;
    auto object_recorder = ceph::make_ref<ObjectRecorder>(m_ioctx,
                                                          utils::get_object_name(m_object_oid_prefix, object_number),
                                                          object_number,
                                                          lock,
                                                          m_journal_metadata->get_work_queue(),
                                                          &m_object_handler,
                                                          m_journal_metadata->get_order(),
                                                          m_max_in_flight_appends);
    object_recorder->set_append_batch_options(m_flush_interval, m_flush_bytes, m_flush_age);
    return object_recorder;
}

bool JournalRecorder::create_next_object_recorder(ceph::ref_t<ObjectRecorder> object_recorder)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ...
    auto new_object_recorder =
        create_object_recorder((m_current_set * splay_width) + splay_offset, &m_object_locks[splay_offset]);
    ...
    for (auto& append_buffer : append_buffers) {
        m_journal_metadata->overflow_commit_tid(append_buffer.first->get_commit_tid(),
