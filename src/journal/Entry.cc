...
void Entry::decode(bufferlist::const_iterator& iter)
{
    ...
    if (bl_preamble != preamble) {
        throw buffer::malformed_input("incorrect preamble: " + stringify(bl_preamble));
    }
    ...
    if (crc != actual_crc) {
        throw buffer::malformed_input("crc mismatch: " + stringify(crc) + " != " + stringify(actual_crc));
    }
}
...
bool Entry::operator==(const Entry& rhs) const
{
    return (m_tag_tid == rhs.m_tag_tid && m_entry_tid == rhs.m_entry_tid &&
            const_cast<bufferlist&>(m_data).contents_equal(const_cast<bufferlist&>(rhs.m_data)));
}
