...
RADOS::RADOS(boost::asio::io_context& ioctx, boost::intrusive_ptr<CephContext> cct)
    : Dispatcher(cct.get())
    , ioctx(ioctx)
    , cct(cct)
    ...
    , mgrclient(cct.get(), nullptr, &monclient.monmap)
{
    ...
    if (err < 0) throw std::system_error(ceph::to_error_code(err));

    messenger.reset(Messenger::create_client_messenger(cct.get(), "radosclient"));
    if (!messenger) throw std::bad_alloc();
...
bool RADOS::ms_dispatch(Message* m)
{
    switch (m->get_type()) {
    // OSD
    case CEPH_MSG_OSD_MAP: m->put(); return true;
    }
    return false;
}
