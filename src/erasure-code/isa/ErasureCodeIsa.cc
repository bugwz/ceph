...
unsigned int ErasureCodeIsa::get_chunk_size(unsigned int object_size) const
{
...
int ErasureCodeIsa::encode_chunks(const set<int>& want_to_encode, map<int, bufferlist>* encoded)
{
    char* chunks[k + m];
...
int ErasureCodeIsa::decode_chunks(const set<int>& want_to_read, const map<int, bufferlist>& chunks,
...
void ErasureCodeIsaDefault::isa_encode(char** data, char** coding, int blocksize)
{
    ...
    if (m == 1)
        ...
        ec_encode_data(blocksize, k, m, encode_tbls, (unsigned char**)data, (unsigned char**)coding);
}
...
int ErasureCodeIsaDefault::isa_decode(int* erasures, char** data, char** coding, int blocksize)
{
    int nerrs = 0;
    int i, r, s;
    ...
    for (i = 0, r = 0; i < k; i++, r++) {
        char id[128];
        while (erasure_contains(erasures, r)) r++;
    ...
    if (!tcache.getDecodingTableFromCache(erasure_signature, p_tbls, matrixtype, k, m)) {
        ...
        for (i = 0; i < k; i++) {
            r = decode_index[i];
            for (j = 0; j < k; j++) b[k * i + j] = encode_coeff[k * r + j];
        }
        // ---------------------------------------------
        ...
        for (int p = 0; p < nerrs; p++) {
            if (erasures[p] < k) {
                ...
                for (i = 0; i < k; i++) {
                    int s = 0;
                    for (j = 0; j < k; j++) s ^= gf_mul(d[j * k + i], encode_coeff[k * erasures[p] + j]);
    ...
    ec_encode_data(blocksize, k, nerrs, decode_tbls, recover_source, recover_target);
...
unsigned ErasureCodeIsaDefault::get_alignment() const
{
...
int ErasureCodeIsaDefault::parse(ErasureCodeProfile& profile, ostream* ss)
{
    ...
    if (matrixtype == kVandermonde) {
        ...
        // full erasures
        if (k > 32) {
            *ss << "Vandermonde: m=" << m << " should be less/equal than 32 : revert to k=32" << std::endl;
            k = 32;
            err = -EINVAL;
        }

        if (m > 4) {
            *ss << "Vandermonde: m=" << m << " should be less than 5 to guarantee an MDS codec:"
        ...
        case 4:
            if (k > 21) {
                *ss << "Vandermonde: k=" << k << " should be less than 22 to guarantee an MDS"
...
void ErasureCodeIsaDefault::prepare()
{
    // setup shared encoding table and coefficients
    unsigned char** p_enc_table = tcache.getEncodingTable(matrixtype, k, m);

    unsigned char** p_enc_coeff = tcache.getEncodingCoefficient(matrixtype, k, m);
    ...
    if (!*p_enc_coeff) {
        ...
        if (matrixtype == kVandermonde) gf_gen_rs_matrix(encode_coeff, k + m, k);
        if (matrixtype == kCauchy) gf_gen_cauchy1_matrix(encode_coeff, k + m, k);
    ...
    unsigned memory_lru_cache = k * (m + k) * 32 * tcache.decoding_tables_lru_length;

    dout(10) << "[ cache memory ] = " << memory_lru_cache << " bytes"
             << " [ matrix ] = " << ((matrixtype == kVandermonde) ? "Vandermonde" : "Cauchy") << dendl;
