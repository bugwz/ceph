...
public:
    ...
    ErasureCodeIsa(const char* _technique, ErasureCodeIsaTableCache& _tcache)
        : k(0)
        , m(0)
        , w(0)
        , tcache(_tcache)
    ...
    virtual void isa_encode(char** data, char** coding, int blocksize) = 0;


    virtual int isa_decode(int* erasures, char** data, char** coding, int blocksize) = 0;
...
private:
    virtual int parse(ceph::ErasureCodeProfile& profile, std::ostream* ss) = 0;
};
...
public:
    ...
    ErasureCodeIsaDefault(ErasureCodeIsaTableCache& _tcache, int matrix = kVandermonde)
        :
    ...
    void isa_encode(char** data, char** coding, int blocksize) override;

    virtual bool erasure_contains(int* erasures, int i);

    int isa_decode(int* erasures, char** data, char** coding, int blocksize) override;
