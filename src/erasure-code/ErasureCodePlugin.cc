...
ErasureCodePluginRegistry::~ErasureCodePluginRegistry()
{
    ...
    for (std::map<std::string, ErasureCodePlugin*>::iterator i = plugins.begin(); i != plugins.end(); ++i) {
...
int ErasureCodePluginRegistry::remove(const std::string& name)
{
    ...
    if (plugins.find(name) == plugins.end()) return -ENOENT;
...
{
    ceph_assert(ceph_mutex_is_locked(lock));
    if (plugins.find(name) != plugins.end()) return -EEXIST;
    plugins[name] = plugin;
    return 0;
}
...
int ErasureCodePluginRegistry::factory(const std::string& plugin_name, const std::string& directory,
                                       ErasureCodeProfile& profile, ErasureCodeInterfaceRef* erasure_code, ostream* ss)
{
    ...
    if (profile != (*erasure_code)->get_profile()) {
        *ss << __func__ << " profile " << profile << " != get_profile() " << (*erasure_code)->get_profile()
            << std::endl;
        return -EINVAL;
    }
    return 0;
...
int ErasureCodePluginRegistry::load(const std::string& plugin_name, const std::string& directory,
                                    ErasureCodePlugin** plugin, ostream* ss)
{
    ceph_assert(ceph_mutex_is_locked(lock));
    std::string fname = directory + "/" PLUGIN_PREFIX + plugin_name + PLUGIN_SUFFIX;
    ...
    const char* (*erasure_code_version)() = (const char* (*)())dlsym(library, PLUGIN_VERSION_FUNCTION);
    if (erasure_code_version == NULL) erasure_code_version = an_older_version;
    if (erasure_code_version() != string(CEPH_GIT_NICE_VER)) {
        *ss << "expected plugin " << fname << " version " << CEPH_GIT_NICE_VER << " but it claims to be "
            << erasure_code_version() << " instead";
        dlclose(library);
        return -EXDEV;
    }
    ...
    if (erasure_code_init) {
        ...
        if (r != 0) {
            *ss << "erasure_code_init(" << plugin_name << "," << directory << "): " << cpp_strerror(r);
            dlclose(library);
            return r;
        }
    }
    else {
        *ss << "load dlsym(" << fname << ", " << PLUGIN_INIT_FUNCTION << "): " << dlerror();
        dlclose(library);
        return -ENOENT;
    }
...
int ErasureCodePluginRegistry::preload(const std::string& plugins, const std::string& directory, ostream* ss)
{
    std::lock_guard l{lock};
    list<string> plugins_list;
    get_str_list(plugins, plugins_list);
    for (list<string>::iterator i = plugins_list.begin(); i != plugins_list.end(); ++i) {
        ErasureCodePlugin* plugin;
