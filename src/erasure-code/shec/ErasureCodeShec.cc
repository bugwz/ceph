...
int ErasureCodeShec::init(ErasureCodeProfile& profile, ostream* ss)
{
    int err = 0;
    err |= parse(profile);
    if (err) return err;
...
int ErasureCodeShec::_minimum_to_decode(const set<int>& want_to_read, const set<int>& available_chunks,
                                        set<int>* minimum_chunks)
{
    ...
    for (set<int>::const_iterator i = available_chunks.begin(); i != available_chunks.end(); ++i) {
        avails[*i] = 1;
    }
...
int ErasureCodeShec::minimum_to_decode_with_cost(const set<int>& want_to_read, const map<int, int>& available,
                                                 set<int>* minimum_chunks)
{
...
int ErasureCodeShec::encode(const set<int>& want_to_encode, const bufferlist& in, map<int, bufferlist>* encoded)
{
    ...
    for (unsigned int i = 0; i < k + m; i++) {
        if (want_to_encode.count(i) == 0) encoded->erase(i);
    }
    return 0;
}

int ErasureCodeShec::encode_chunks(const set<int>& want_to_encode, map<int, bufferlist>* encoded)
{
    char* chunks[k + m];
    for (int i = 0; i < k + m; i++) {
...
int ErasureCodeShec::_decode(const set<int>& want_to_read, const map<int, bufferlist>& chunks,
...
int ErasureCodeShec::decode_chunks(const set<int>& want_to_read, const map<int, bufferlist>& chunks,
                                   map<int, bufferlist>* decoded)
{
    unsigned blocksize = (*chunks.begin()).second.length();
    int erased[k + m];
...
void ErasureCodeShecReedSolomonVandermonde::shec_encode(char** data, char** coding, int blocksize)
{
    jerasure_matrix_encode(k, m, w, matrix, data, coding, blocksize);
}

int ErasureCodeShecReedSolomonVandermonde::shec_decode(int* erased, int* avails, char** data, char** coding,
                                                       int blocksize)
{
...
int ErasureCodeShecReedSolomonVandermonde::parse(const ErasureCodeProfile& profile)
{
    int err = 0;
    // k, m, c
    if (profile.find("k") == profile.end() && profile.find("m") == profile.end() &&
        profile.find("c") == profile.end()) {
        dout(10) << "(k, m, c) default to "
                 << "(" << DEFAULT_K << ", " << DEFAULT_M << ", " << DEFAULT_C << ")" << dendl;
        k = DEFAULT_K;
        m = DEFAULT_M;
        c = DEFAULT_C;
    }
    else if (profile.find("k") == profile.end() || profile.find("m") == profile.end() ||
             profile.find("c") == profile.end()) {
        ...
        for (c1 = 0; c1 <= c / 2; c1++) {
            for (m1 = 0; m1 <= m; m1++) {
                ...
                if (true) {
                    ...
                    if (min_r_e1 - r_e1 > std::numeric_limits<double>::epsilon() && r_e1 < min_r_e1) {
                        min_r_e1 = r_e1;
                        c1_best = c1;
                        m1_best = m1;
                    }
...
int ErasureCodeShec::shec_make_decoding_matrix(bool prepare, int* want_, int* avails, int* decoding_matrix, int* dm_row,
                                               int* dm_column, int* minimum)
{
    ...
    if (tcache.getDecodingTableFromCache(
            decoding_matrix, dm_row, dm_column, minimum, technique, k, m, c, w, want, avails)) {
        return 0;
    }
    ...
    tcache.putDecodingTableToCache(decoding_matrix, dm_row, dm_column, minimum, technique, k, m, c, w, want, avails);

    return ret;
}

int ErasureCodeShec::shec_matrix_decode(int* want, int* avails, char** data_ptrs, char** coding_ptrs, int size)
{
    int decoding_matrix[k * k];
    ...
    if (shec_make_decoding_matrix(false, want, avails, decoding_matrix, dm_row, dm_column, minimum) < 0) {
        return -1;
    }
    ...
    for (int i = 0; i < dm_size; i++) {
        if (!avails[dm_column[i]]) {
            jerasure_matrix_dotprod(
                dm_size, w, decoding_matrix + (i * dm_size), dm_row, i, dm_data_ptrs, coding_ptrs, size);
        }
    }
