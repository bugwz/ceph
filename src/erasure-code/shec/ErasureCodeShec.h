...
public:
    ...
    int _minimum_to_decode(const std::set<int>& want_to_read, const std::set<int>& available_chunks,
                           std::set<int>* minimum);

    int minimum_to_decode_with_cost(const std::set<int>& want_to_read, const std::map<int, int>& available,
                                    std::set<int>* minimum) override;

    int encode(const std::set<int>& want_to_encode, const ceph::buffer::list& in,
    ...
    int _decode(const std::set<int>& want_to_read, const std::map<int, ceph::buffer::list>& chunks,
    ...
    virtual int shec_decode(int* erasures, int* avails, char** data, char** coding, int blocksize) = 0;
    virtual unsigned get_alignment() const = 0;
    virtual void prepare() = 0;

    virtual int shec_matrix_decode(int* erased, int* avails, char** data_ptrs, char** coding_ptrs, int size);
...
private:
    ...
    virtual int shec_make_decoding_matrix(bool prepare, int* want, int* avails, int* decoding_matrix, int* dm_row,
...
public:
    ...
    int shec_decode(int* erasures, int* avails, char** data, char** coding, int blocksize) override;
