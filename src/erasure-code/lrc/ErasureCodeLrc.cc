...
int ErasureCodeLrc::create_rule(const string& name, CrushWrapper& crush, ostream* ss) const
{
    ...
    if (rule_device_class.size()) {
        ...
        if (crush.class_bucket.count(root) == 0 || crush.class_bucket[root].count(c) == 0) {
            *ss << "root item " << rule_root << " has no devices with class " << rule_device_class;
            return -EINVAL;
        }
    ...
    for (rno = 0; rno < crush.get_max_rules(); rno++) {
        if (!crush.rule_exists(rno) && !crush.ruleset_exists(rno)) break;
    }
    ...
    int ret;
    ret = crush.add_rule(rno, steps, pg_pool_t::TYPE_ERASURE, min_rep, max_rep);
    ceph_assert(ret == rno);
    int step = 0;
    ...
    // [ [ "choose", "rack", 2 ],
    //   [ "chooseleaf", "host", 5 ] ]
    for (vector<Step>::const_iterator i = rule_steps.begin(); i != rule_steps.end(); ++i) {
        int op = i->op == "chooseleaf" ? CRUSH_RULE_CHOOSELEAF_INDEP : CRUSH_RULE_CHOOSE_INDEP;
...
int ErasureCodeLrc::layers_description(const ErasureCodeProfile& profile, json_spirit::mArray* description,
                                       ostream* ss) const
{
    ...
    try {
        ...
        if (json.type() != json_spirit::array_type) {
            *ss << "layers='" << str << "' must be a JSON array but is of type " << json.type() << " instead"
                << std::endl;
        ...
        *ss << "failed to parse layers='" << str << "'"
            << " at line " << e.line_ << ", column " << e.column_ << " : " << e.reason_ << std::endl;
        return ERROR_LRC_PARSE_JSON;
    }
    return 0;
}

int ErasureCodeLrc::layers_parse(const string& description_string, json_spirit::mArray description, ostream* ss)
{
    int position = 0;
    for (vector<json_spirit::mValue>::iterator i = description.begin(); i != description.end(); ++i, position++) {
        if (i->type() != json_spirit::array_type) {
            stringstream json_string;
            json_spirit::write(*i, json_string);
            *ss << "each element of the array " << description_string << " must be a JSON array but "
                << json_string.str() << " at position " << position << " is of type " << i->type() << " instead"
                << std::endl;
            return ERROR_LRC_ARRAY;
        }
        ...
        int index = 0;
        for (vector<json_spirit::mValue>::iterator j = layer_json.begin(); j != layer_json.end(); ++j, ++index) {
            if (index == 0) {
                if (j->type() != json_spirit::str_type) {
                    stringstream element;
                    json_spirit::write(*j, element);
                    *ss << "the first element of the entry " << element.str() << " (first is zero) " << position
                        << " in " << description_string << " is of type " << (*j).type() << " instead of string"
                        << std::endl;
                ...
                Layer& layer = layers.back();
                if (j->type() != json_spirit::str_type && j->type() != json_spirit::obj_type) {
                    stringstream element;
                    json_spirit::write(*j, element);
                    *ss << "the second element of the entry " << element.str() << " (first is zero) " << position
                        << " in " << description_string << " is of type " << (*j).type()
                        << " instead of string or object" << std::endl;
                ...
                if (j->type() == json_spirit::str_type) {
                    ...
                    for (map<string, json_spirit::mValue>::iterator i = o.begin(); i != o.end(); ++i) {
...
int ErasureCodeLrc::layers_init(ostream* ss)
{
    ...
    for (unsigned int i = 0; i < layers.size(); i++) {
        ...
        int position = 0;
        for (std::string::iterator it = layer.chunks_map.begin(); it != layer.chunks_map.end(); ++it) {
            if (*it == 'D') layer.data.push_back(position);
            if (*it == 'c') layer.coding.push_back(position);
            if (*it == 'c' || *it == 'D') layer.chunks_as_set.insert(position);
            position++;
        }
        layer.chunks = layer.data;
        layer.chunks.insert(layer.chunks.end(), layer.coding.begin(), layer.coding.end());
        if (layer.profile.find("k") == layer.profile.end()) layer.profile["k"] = stringify(layer.data.size());
        if (layer.profile.find("m") == layer.profile.end()) layer.profile["m"] = stringify(layer.coding.size());
        if (layer.profile.find("plugin") == layer.profile.end()) layer.profile["plugin"] = "jerasure";
        if (layer.profile.find("technique") == layer.profile.end()) layer.profile["technique"] = "reed_sol_van";
        int err = registry.factory(layer.profile["plugin"], directory, layer.profile, &layer.erasure_code, ss);
        if (err) return err;
    }
    return 0;
}

int ErasureCodeLrc::layers_sanity_checks(const string& description_string, ostream* ss) const
{
    int position = 0;

    if (layers.size() < 1) {
        *ss << "layers parameter has " << layers.size() << " which is less than the minimum of one. "
    ...
    for (vector<Layer>::const_iterator layer = layers.begin(); layer != layers.end(); ++layer) {
        if (chunk_count != layer->chunks_map.length()) {
            *ss << "the first element of the array at position " << position << " (starting from zero) "
                << " is the string '" << layer->chunks_map << " found in the layers parameter " << description_string
                << ". It is expected to be " << chunk_count << " characters long but is " << layer->chunks_map.length()
                << " characters long instead " << std::endl;
...
int ErasureCodeLrc::parse(ErasureCodeProfile& profile, ostream* ss)
{
...
int ErasureCodeLrc::parse_kml(ErasureCodeProfile& profile, ostream* ss)
{
    ...
    if (k == DEFAULT_INT && m == DEFAULT_INT && l == DEFAULT_INT) return err;

    if ((k != DEFAULT_INT || m != DEFAULT_INT || l != DEFAULT_INT) &&
        (k == DEFAULT_INT || m == DEFAULT_INT || l == DEFAULT_INT)) {
        *ss << "All of k, m, l must be set or none of them in " << profile << std::endl;
        return ERROR_LRC_ALL_OR_NOTHING;
    }

    const char* generated[] = {"mapping", "layers", "crush-steps"};
    ...
    if (l == 0 || (k + m) % l) {
        *ss << "k + m must be a multiple of l in " << profile << std::endl;
    ...
    if (k % local_group_count) {
        *ss << "k must be a multiple of (k + m) / l in " << profile << std::endl;
        return ERROR_LRC_K_MODULO;
    }

    if (m % local_group_count) {
        *ss << "m must be a multiple of (k + m) / l in " << profile << std::endl;
        return ERROR_LRC_M_MODULO;
    }

    string mapping;
    for (int i = 0; i < local_group_count; i++) {
        mapping += string(k / local_group_count, 'D') + string(m / local_group_count, '_') + "_";
    }
    ...
    layers += " [ \"";
    for (int i = 0; i < local_group_count; i++) {
        layers += string(k / local_group_count, 'D') + string(m / local_group_count, 'c') + "_";
    }
    layers += "\", \"\" ],";
    ...
    if (parameter != profile.end()) rule_locality = parameter->second;
    string rule_failure_domain = "host";
    parameter = profile.find("crush-failure-domain");
    if (parameter != profile.end()) rule_failure_domain = parameter->second;

    if (rule_locality != "") {
        rule_steps.clear();
        rule_steps.push_back(Step("choose", rule_locality, local_group_count));
        rule_steps.push_back(Step("chooseleaf", rule_failure_domain, l + 1));
    }
...
int ErasureCodeLrc::parse_rule(ErasureCodeProfile& profile, ostream* ss)
{
    int err = 0;
    err |= to_string("crush-root", profile, &rule_root, "default", ss);
    err |= to_string("crush-device-class", profile, &rule_device_class, "", ss);
    ...
    if (profile.count("crush-steps") != 0) {
        ...
        try {
            ...
            if (json.type() != json_spirit::array_type) {
                *ss << "crush-steps='" << str << "' must be a JSON array but is of type " << json.type() << " instead"
                    << std::endl;
                return ERROR_LRC_ARRAY;
            }
            description = json.get_array();
        ...
        for (vector<json_spirit::mValue>::iterator i = description.begin(); i != description.end(); ++i, position++) {
            if (i->type() != json_spirit::array_type) {
                stringstream json_string;
                json_spirit::write(*i, json_string);
                *ss << "element of the array " << str << " must be a JSON array but " << json_string.str()
                    << " at position " << position << " is of type " << i->type() << " instead" << std::endl;
                return ERROR_LRC_ARRAY;
            }
...
int ErasureCodeLrc::parse_rule_step(const string& description_string, json_spirit::mArray description, ostream* ss)
{
    stringstream json_string;
    ...
    int position = 0;
    for (vector<json_spirit::mValue>::iterator i = description.begin(); i != description.end(); ++i, position++) {
        if ((position == 0 || position == 1) && i->type() != json_spirit::str_type) {
            *ss << "element " << position << " of the array " << json_string.str() << " found in " << description_string
                << " must be a JSON string but is of type " << i->type() << " instead" << std::endl;
            return position == 0 ? ERROR_LRC_RULE_OP : ERROR_LRC_RULE_TYPE;
        }
        if (position == 2 && i->type() != json_spirit::int_type) {
            *ss << "element " << position << " of the array " << json_string.str() << " found in " << description_string
                << " must be a JSON int but is of type " << i->type() << " instead" << std::endl;
            return ERROR_LRC_RULE_N;
        }

        if (position == 0) op = i->get_str();
        if (position == 1) type = i->get_str();
        if (position == 2) n = i->get_int();
    }
    rule_steps.push_back(Step(op, type, n));
    return 0;
}

int ErasureCodeLrc::init(ErasureCodeProfile& profile, ostream* ss)
{
    int r;
    ...
    //
    if (profile.find("l") != profile.end() && profile.find("l")->second != DEFAULT_KML) {
...
set<int> ErasureCodeLrc::get_erasures(const set<int>& want, const set<int>& available) const
{
    set<int> result;
    set_difference(want.begin(), want.end(), available.begin(), available.end(), inserter(result, result.end()));
    return result;
}
...
int ErasureCodeLrc::_minimum_to_decode(const set<int>& want_to_read, const set<int>& available_chunks,
                                       set<int>* minimum)
{
    dout(20) << __func__ << " want_to_read " << want_to_read << " available_chunks " << available_chunks << dendl;
    {
        ...
        for (unsigned int i = 0; i < get_chunk_count(); ++i) {
            if (available_chunks.count(i) == 0) {
                ...
                if (want_to_read.count(i) != 0) erasures_want.insert(i);
            }
        }
        ...
        for (vector<Layer>::reverse_iterator i = layers.rbegin(); i != layers.rend(); ++i) {
            ...
            //
            set<int> layer_want;
            set_intersection(want_to_read.begin(),
                             want_to_read.end(),
                             i->chunks_as_set.begin(),
                             i->chunks_as_set.end(),
                             inserter(layer_want, layer_want.end()));
            if (layer_want.empty()) continue;
            //
            // Are some of the chunks we want missing ?
            //
            set<int> layer_erasures;
            set_intersection(layer_want.begin(),
                             layer_want.end(),
                             erasures_want.begin(),
                             erasures_want.end(),
            ...
            if (layer_erasures.empty()) {
                ...
                set<int> erasures;
                set_intersection(i->chunks_as_set.begin(),
                                 i->chunks_as_set.end(),
                                 erasures_not_recovered.begin(),
                                 erasures_not_recovered.end(),
                ...
                if (erasures.size() > i->erasure_code->get_coding_chunk_count()) {
                    ...
                    // them.
                    //
                    for (set<int>::const_iterator j = erasures.begin(); j != erasures.end(); ++j) {
        ...
        if (erasures_want.empty()) {
            ...
            for (set<int>::const_iterator i = erasures_total.begin(); i != erasures_total.end(); ++i) {
                if (minimum->count(*i)) minimum->erase(*i);
            }
...

    {
        ...
        set<int> erasures_total;
        for (unsigned int i = 0; i < get_chunk_count(); ++i) {
            if (available_chunks.count(i) == 0) erasures_total.insert(i);
        }
        ...
        for (vector<Layer>::reverse_iterator i = layers.rbegin(); i != layers.rend(); ++i) {
            ...
            set_intersection(i->chunks_as_set.begin(),
                             i->chunks_as_set.end(),
                             erasures_total.begin(),
                             erasures_total.end(),
                             inserter(layer_erasures, layer_erasures.end()));
            //
            // If this layer has no erasure, skip it
            //
            if (layer_erasures.empty()) continue;

            if (layer_erasures.size() > 0 && layer_erasures.size() <= i->erasure_code->get_coding_chunk_count()) {
                //
                // chunks recovered by this layer are removed from the list of
                // erasures so that upper levels know they can rely on their
                // availability
                //
                for (set<int>::const_iterator j = layer_erasures.begin(); j != layer_erasures.end(); ++j) {
...
int ErasureCodeLrc::encode_chunks(const set<int>& want_to_encode, map<int, bufferlist>* encoded)
{
    unsigned int top = layers.size();
    for (vector<Layer>::reverse_iterator i = layers.rbegin(); i != layers.rend(); ++i) {
        --top;
        if (includes(i->chunks_as_set.begin(), i->chunks_as_set.end(), want_to_encode.begin(), want_to_encode.end()))
            break;
    }
    ...
    for (unsigned int i = top; i < layers.size(); ++i) {
        ...
        for (const auto& c : layer.chunks) {
            ...
            if (want_to_encode.find(c) != want_to_encode.end()) layer_want_to_encode.insert(j);
            j++;
        }
        int err = layer.erasure_code->encode_chunks(layer_want_to_encode, &layer_encoded);
        j = 0;
        for (const auto& c : layer.chunks) {
            std::swap(layer_encoded[j++], (*encoded)[c]);
        }
        if (err) {
            derr << __func__ << " layer " << layer.chunks_map << " failed with " << err << " trying to encode "
...
int ErasureCodeLrc::decode_chunks(const set<int>& want_to_read, const map<int, bufferlist>& chunks,
                                  map<int, bufferlist>* decoded)
{
    ...
    for (vector<Layer>::reverse_iterator layer = layers.rbegin(); layer != layers.rend(); ++layer) {
        set<int> layer_erasures;
        set_intersection(layer->chunks_as_set.begin(),
                         layer->chunks_as_set.end(),
                         erasures.begin(),
                         erasures.end(),
                         inserter(layer_erasures, layer_erasures.end()));

        if (layer_erasures.size() > layer->erasure_code->get_coding_chunk_count()) {
            ...
            int j = 0;
            for (vector<int>::const_iterator c = layer->chunks.begin(); c != layer->chunks.end(); ++c) {
                //
                ...
                //
                if (erasures.count(*c) == 0) layer_chunks[j] = (*decoded)[*c];
                if (want_to_read.count(*c) != 0) layer_want_to_read.insert(j);
                layer_decoded[j] = (*decoded)[*c];
                ++j;
            }
            int err = layer->erasure_code->decode_chunks(layer_want_to_read, layer_chunks, &layer_decoded);
            if (err) {
                derr << __func__ << " layer " << layer->chunks_map << " failed with " << err << " trying to decode "
            ...
            set_intersection(erasures.begin(),
                             erasures.end(),
                             want_to_read.begin(),
                             want_to_read.end(),
                             inserter(want_to_read_erasures, want_to_read_erasures.end()));
            if (want_to_read_erasures.size() == 0) break;
        }
    }

    if (want_to_read_erasures.size() > 0) {
        derr << __func__ << " want to read " << want_to_read << " with available_chunks = " << available_chunks
