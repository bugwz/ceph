...
bool QccCrypto::init()
{
    ...
    qcc_os_mem_alloc((void**)&qcc_inst->cy_inst_handles, ((int)qcc_inst->num_instances * sizeof(CpaInstanceHandle)));
    ...
    qcc_os_mem_alloc((void**)&qcc_inst->is_polled, ((int)qcc_inst->num_instances * sizeof(CpaBoolean)));
    CpaInstanceInfo2 info;
    for (iter = 0; iter < qcc_inst->num_instances; iter++) {
        qcc_inst->is_polled[iter] = cpaCyInstanceGetInfo2(qcc_inst->cy_inst_handles[iter], &info) == CPA_STATUS_SUCCESS
                                        ? info.isPolled
                                        : CPA_FALSE;
    }

    // Allocate memory structures for all instances
    qcc_os_mem_alloc((void**)&qcc_sess, ((int)qcc_inst->num_instances * sizeof(QCCSESS)));
    if (qcc_sess == NULL) {
    ...
    qcc_os_mem_alloc((void**)&qcc_op_mem, ((int)qcc_inst->num_instances * sizeof(QCCOPMEM)));
    if (qcc_sess == NULL) {
    ...
    qcc_os_mem_alloc((void**)&cypollthreads, ((int)qcc_inst->num_instances * sizeof(pthread_t)));
    ...
    for (iter = 0; iter < qcc_inst->num_instances; iter++) {
        stat = cpaCySetAddressTranslation(qcc_inst->cy_inst_handles[iter], qaeVirtToPhysNUMA);
        if (stat == CPA_STATUS_SUCCESS) {
            ...
            qcc_op_mem[iter].src_buff_meta = qcc_op_mem[iter].src_buff = qcc_op_mem[iter].iv_buff = NULL;
...
bool QccCrypto::perform_op(unsigned char* out, const unsigned char* in, size_t size, uint8_t* iv, uint8_t* key,
                           CpaCySymCipherDirection op_type)
{
    ...
    if (!is_init) {
        dout(10) << "QAT not initialized in this instance or init failed with possible error " << (int)init_stat
                 << dendl;
        return is_init;
    }
    ...
    if (qcc_op_mem[avail_inst].is_mem_alloc == false) {
        ...
        qcc_sess[avail_inst].sess_stp_data.cipherSetupData.cipherAlgorithm = CPA_CY_SYM_CIPHER_AES_CBC;
        qcc_sess[avail_inst].sess_stp_data.cipherSetupData.cipherKeyLenInBytes = AES_256_KEY_SIZE;
        ...
        stat = cpaCyBufferListGetMetaSize(qcc_inst->cy_inst_handles[avail_inst],
                                          ...
                                          &(qcc_op_mem[avail_inst].buff_meta_size));
        if (stat != CPA_STATUS_SUCCESS) {
        ...
        stat = qcc_contig_mem_alloc(
            (void**)&(qcc_op_mem[avail_inst].src_buff_meta), qcc_op_mem[avail_inst].buff_meta_size, 1);
        if (stat != CPA_STATUS_SUCCESS) {
        ...
        // Setup OpData
        stat = qcc_contig_mem_alloc((void**)&(qcc_op_mem[avail_inst].sym_op_data), sizeof(CpaCySymOpData));
        if (stat != CPA_STATUS_SUCCESS) {
        ...
        // exit this block
        qcc_sess[avail_inst].sess_stp_data.cipherSetupData.cipherDirection = CPA_CY_SYM_CIPHER_DIRECTION_ENCRYPT;
        ...
        stat = qcc_contig_mem_alloc((void**)&(qcc_sess[avail_inst].sess_ctx), qcc_sess[avail_inst].sess_ctx_sz);
        if (stat != CPA_STATUS_SUCCESS) {
    ...
    if (qcc_op_mem[avail_inst].buff_size != (Cpa32U)size) {
        ...
        stat = qcc_contig_mem_alloc((void**)&(qcc_op_mem[avail_inst].src_buff), qcc_op_mem[avail_inst].buff_size);
        if (stat != CPA_STATUS_SUCCESS) {
            derr << "Unable to allocate contig memory" << dendl;
            return false;
        }
