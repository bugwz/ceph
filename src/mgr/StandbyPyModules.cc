...
StandbyPyModules::StandbyPyModules(const MgrMap& mgr_map_, PyModuleConfig& module_config, LogChannelRef clog_,
                                   MonClient& monc_, Finisher& f)
...
bool StandbyPyModule::get_config(const std::string& key, std::string* value) const
{
...
bool StandbyPyModule::get_store(const std::string& key, std::string* value) const
{

    const std::string global_key = PyModule::mgr_store_prefix + get_name() + "/" + key;
    ...
    if (r == -ENOENT) {
        ...
        derr << __func__ << " error fetching store key '" << global_key << "': " << cpp_strerror(r) << " " << outs
             << dendl;
        return false;
    }
    else {
