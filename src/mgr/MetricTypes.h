...
typedef boost::variant<OSDMetricPayload, MDSMetricPayload, UnknownMetricPayload> MetricPayload;
...
public:
    ...
    template<typename MetricPayload> inline void operator()(MetricPayload& payload) const
    {
        using ceph::decode;
...
struct MetricReportMessage
{
    ...
    {
        if (!HAVE_FEATURE(features, SERVER_PACIFIC) && boost::get<MDSMetricPayload>(&payload)) {
            return false;
        }
        return true;
    }

    void encode(ceph::buffer::list& bl) const { boost::apply_visitor(EncodeMetricPayloadVisitor(bl), payload); }
    ...
    void decode(ceph::buffer::list::const_iterator& iter)
    {
        ...
        case MetricReportType::METRIC_REPORT_TYPE_OSD: payload = OSDMetricPayload(); break;
        case MetricReportType::METRIC_REPORT_TYPE_MDS: payload = MDSMetricPayload(); break;
        default: payload = UnknownMetricPayload(); break;
        }
...
typedef boost::variant<OSDConfigPayload, MDSConfigPayload, UnknownConfigPayload> ConfigPayload;
...
public:
    ...
    template<typename ConfigPayload> inline void operator()(const ConfigPayload& payload) const
    {
        using ceph::encode;
...
public:
    ...
    template<typename ConfigPayload> inline void operator()(ConfigPayload& payload) const
    {
        using ceph::decode;
        decode(payload, m_iter);
    }
...
struct MetricConfigMessage
{
    ...
    bool should_encode(uint64_t features) const
    {
        if (!HAVE_FEATURE(features, SERVER_PACIFIC) && boost::get<MDSConfigPayload>(&payload)) {
            return false;
        }
        return true;
    }

    void encode(ceph::buffer::list& bl) const { boost::apply_visitor(EncodeConfigPayloadVisitor(bl), payload); }
    ...
    void decode(ceph::buffer::list::const_iterator& iter)
    {
        ...
        case MetricConfigType::METRIC_CONFIG_TYPE_OSD: payload = OSDConfigPayload(); break;
        case MetricConfigType::METRIC_CONFIG_TYPE_MDS: payload = MDSConfigPayload(); break;
        default: payload = UnknownConfigPayload(); break;
        }
