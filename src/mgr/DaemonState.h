...
public:
    const boost::circular_buffer<DataPoint>& get_data() const { return buffer; }
    const DataPoint& get_latest_data() const { return buffer.back(); }
    const boost::circular_buffer<AvgDataPoint>& get_data_avg() const { return avg_buffer; }
    const AvgDataPoint& get_latest_data_avg() const { return avg_buffer.back(); }
...
struct DeviceState : public RefCountedObject
{
    ...
    bool empty() const { return daemons.empty() && metadata.empty(); }
...
{
private:
    mutable ceph::shared_mutex lock = ceph::make_shared_mutex("DaemonStateIndex", true, true, true);
    ...
    void _erase_device(const ceph::ref_t<DeviceState>& d) { devices.erase(d->devid); }
...
public:
    ...
    template<typename Callback, typename... Args> void with_devices(Callback&& cb, Args&&... args) const
    {
        std::shared_lock l{lock};
        for (auto& i : devices) {
    ...
    void update_metadata(DaemonStatePtr state, const std::map<std::string, std::string>& meta)
    {
