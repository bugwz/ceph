...
typedef struct
{
    PyObject_HEAD OSDMap* osdmap;
} BasePyOSDMap;

typedef struct
{
    PyObject_HEAD OSDMap::Incremental* inc;
} BasePyOSDMapIncremental;

typedef struct
{
    PyObject_HEAD std::shared_ptr<CrushWrapper> crush;
} BasePyCRUSH;
...
static PyObject* osdmap_new_incremental(BasePyOSDMap* self, PyObject* obj)
{
    ...
    return construct_with_capsule("mgr_module", "OSDMapIncremental", (void*)(inc));
}

static PyObject* osdmap_apply_incremental(BasePyOSDMap* self, BasePyOSDMapIncremental* incobj)
{
...
{
    return construct_with_capsule("mgr_module", "CRUSHMap", (void*)(&(self->osdmap->crush)));
}

static PyObject* osdmap_get_pools_by_take(BasePyOSDMap* self, PyObject* args)
{
    int take;
    if (!PyArg_ParseTuple(args, "i:get_pools_by_take", &take)) {
        return nullptr;
    }
...
static PyObject* osdmap_calc_pg_upmaps(BasePyOSDMap* self, PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "OiiO:calc_pg_upmaps", &incobj, &max_deviation, &max_iterations, &pool_list)) {
        return nullptr;
    }
    ...
    for (auto i = 0; i < PyList_Size(pool_list); ++i) {
        ...
        auto pool_id = self->osdmap->lookup_pg_pool_name(PyUnicode_AsUTF8(pool_name));
        if (pool_id < 0) {
    ...
    dout(10) << __func__ << " osdmap " << self->osdmap << " inc " << incobj->inc << " max_deviation " << max_deviation
             << " max_iterations " << max_iterations << " pools " << pools << dendl;
    PyThreadState* tstate = PyEval_SaveThread();
    int r = self->osdmap->calc_pg_upmaps(g_ceph_context, max_deviation, max_iterations, pools, incobj->inc);
...
{
    int poolid;
    if (!PyArg_ParseTuple(args, "i:map_pool_pgs_up", &poolid)) {
        return nullptr;
    }
...
static int BasePyOSDMap_init(BasePyOSDMap* self, PyObject* args, PyObject* kwds)
{
    PyObject* osdmap_capsule = nullptr;
    static const char* kwlist[] = {"osdmap_capsule", NULL};

    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", const_cast<char**>(kwlist), &osdmap_capsule)) {
        return -1;
    }
    if (!PyObject_TypeCheck(osdmap_capsule, &PyCapsule_Type)) {
        PyErr_Format(PyExc_TypeError, "Expected a PyCapsule_Type, not %s", Py_TYPE(osdmap_capsule)->tp_name);
        return -1;
    }

    self->osdmap = (OSDMap*)PyCapsule_GetPointer(osdmap_capsule, nullptr);
...
static void BasePyOSDMap_dealloc(BasePyOSDMap* self)
{
    if (self->osdmap) {
        delete self->osdmap;
        self->osdmap = nullptr;
...
static PyObject* osdmap_pg_to_up_acting_osds(BasePyOSDMap* self, PyObject* args)
{
    int pool_id = 0;
    int ps = 0;
    if (!PyArg_ParseTuple(args, "ii:pg_to_up_acting_osds", &pool_id, &ps)) {
        return nullptr;
    }
    ...
    self->osdmap->pg_to_up_acting_osds(pg_id, &up, &up_primary, &acting, &acting_primary);
...
{
    int pool_id = 0;
    if (!PyArg_ParseTuple(args, "i:pool_raw_used_rate", &pool_id)) {
        return nullptr;
    }
...
static PyObject* osdmap_build_simple(PyObject* cls, PyObject* args, PyObject* kwargs)
{
    ...
    int num_osd = -1;
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "izi", const_cast<char**>(kwlist), &epoch, &uuid_str, &num_osd)) {
        Py_RETURN_NONE;
    }
    uuid_d uuid;
    if (uuid_str) {
...
PyMethodDef BasePyOSDMap_methods[] = {
    ...
    {"_get_crush_version", (PyCFunction)osdmap_get_crush_version, METH_NOARGS, "Get CRUSH version"},
    {"_dump", (PyCFunction)osdmap_dump, METH_NOARGS, "Dump OSDMap::Incremental"},
    {"_new_incremental", (PyCFunction)osdmap_new_incremental, METH_NOARGS, "Create OSDMap::Incremental"},
    {"_apply_incremental",
     (PyCFunction)osdmap_apply_incremental,
     METH_O,
    ...
    {"_get_pools_by_take",
     (PyCFunction)osdmap_get_pools_by_take,
     METH_VARARGS,
     "Get pools that have CRUSH rules that TAKE the given root"},
    {"_calc_pg_upmaps", (PyCFunction)osdmap_calc_pg_upmaps, METH_VARARGS, "Calculate new pg-upmap values"},
    {"_map_pool_pgs_up",
     (PyCFunction)osdmap_map_pool_pgs_up,
     METH_VARARGS,
     "Calculate up set mappings for all PGs in a pool"},
    {"_pg_to_up_acting_osds",
     (PyCFunction)osdmap_pg_to_up_acting_osds,
     METH_VARARGS,
     "Calculate up+acting OSDs for a PG ID"},
    {"_pool_raw_used_rate",
     (PyCFunction)osdmap_pool_raw_used_rate,
     METH_VARARGS,
     "Get raw space to logical space ratio"},
    {"_build_simple", (PyCFunction)osdmap_build_simple, METH_VARARGS | METH_CLASS, "Create a simple OSDMap"},
    {NULL, NULL, 0, NULL}};

PyTypeObject BasePyOSDMapType = {
    PyVarObject_HEAD_INIT(NULL, 0) "ceph_module.BasePyOSDMap", /* tp_name */
...
static int BasePyOSDMapIncremental_init(BasePyOSDMapIncremental* self, PyObject* args, PyObject* kwds)
{
    PyObject* inc_capsule = nullptr;
    static const char* kwlist[] = {"inc_capsule", NULL};

    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", const_cast<char**>(kwlist), &inc_capsule)) {
        ceph_abort();
        return -1;
    }
    ceph_assert(PyObject_TypeCheck(inc_capsule, &PyCapsule_Type));

    self->inc = (OSDMap::Incremental*)PyCapsule_GetPointer(inc_capsule, nullptr);
    ceph_assert(self->inc);

    return 0;
}

static void BasePyOSDMapIncremental_dealloc(BasePyOSDMapIncremental* self)
{
    if (self->inc) {
        delete self->inc;
        self->inc = nullptr;
...
static PyObject* osdmap_inc_get_epoch(BasePyOSDMapIncremental* self, PyObject* obj)
{
    return PyLong_FromLong(self->inc->epoch);
}

static PyObject* osdmap_inc_dump(BasePyOSDMapIncremental* self, PyObject* obj)
{
    PyFormatter f;
    self->inc->dump(&f);
    return f.get();
...
static PyObject* osdmap_inc_set_osd_reweights(BasePyOSDMapIncremental* self, PyObject* weightobj)
{
    map<int, double> wm;
...
static PyObject* osdmap_inc_set_compat_weight_set_weights(BasePyOSDMapIncremental* self, PyObject* weightobj)
{
    map<int, double> wm;
    if (get_int_float_map(weightobj, &wm) < 0) {
        return nullptr;
    ...
    for (auto i : wm) {
        crush.choose_args_adjust_item_weightf(
            g_ceph_context, crush.choose_args_get(CrushWrapper::DEFAULT_CHOOSE_ARGS), i.first, {i.second}, nullptr);
    }
...
PyMethodDef BasePyOSDMapIncremental_methods[] = {
    {"_get_epoch", (PyCFunction)osdmap_inc_get_epoch, METH_NOARGS, "Get OSDMap::Incremental epoch"},
    {"_dump", (PyCFunction)osdmap_inc_dump, METH_NOARGS, "Dump OSDMap::Incremental"},
    {"_set_osd_reweights", (PyCFunction)osdmap_inc_set_osd_reweights, METH_O, "Set osd reweight values"},
    {"_set_crush_compat_weight_set_weights",
     (PyCFunction)osdmap_inc_set_compat_weight_set_weights,
     METH_O,
     "Set weight values in the pending CRUSH compat weight-set"},
    {NULL, NULL, 0, NULL}};

PyTypeObject BasePyOSDMapIncrementalType = {
    PyVarObject_HEAD_INIT(NULL, 0) "ceph_module.BasePyOSDMapIncremental", /* tp_name */
...
static int BasePyCRUSH_init(BasePyCRUSH* self, PyObject* args, PyObject* kwds)
{
    PyObject* crush_capsule = nullptr;
    static const char* kwlist[] = {"crush_capsule", NULL};

    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", const_cast<char**>(kwlist), &crush_capsule)) {
        ceph_abort();
        return -1;
    }
    ceph_assert(PyObject_TypeCheck(crush_capsule, &PyCapsule_Type));

    auto ptr_ref = (std::shared_ptr<CrushWrapper>*)(PyCapsule_GetPointer(crush_capsule, nullptr));
...
static void BasePyCRUSH_dealloc(BasePyCRUSH* self)
{
    self->crush.reset();
    Py_TYPE(self)->tp_free(self);
}
...
{
    int root;
    if (!PyArg_ParseTuple(args, "i:get_take_weight_osd_map", &root)) {
        return nullptr;
    }
    map<int, float> wmap;
...
PyMethodDef BasePyCRUSH_methods[] = {
    ...
    {"_get_item_name", (PyCFunction)crush_get_item_name, METH_VARARGS, "Get item name"},
    {"_get_item_weight", (PyCFunction)crush_get_item_weight, METH_VARARGS, "Get item weight"},
    {"_find_roots", (PyCFunction)crush_find_roots, METH_NOARGS, "Find all tree roots"},
    {"_find_takes", (PyCFunction)crush_find_takes, METH_NOARGS, "Find distinct TAKE roots"},
    {"_get_take_weight_osd_map",
     (PyCFunction)crush_get_take_weight_osd_map,
     METH_VARARGS,
     "Get OSD weight map for a given TAKE root node"},
    {NULL, NULL, 0, NULL}};

PyTypeObject BasePyCRUSHType = {
    PyVarObject_HEAD_INIT(NULL, 0) "ceph_module.BasePyCRUSH", /* tp_name */
