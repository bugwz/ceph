...
Mgr::Mgr(MonClient* monc_, const MgrMap& mgrmap, PyModuleRegistry* py_module_registry_, Messenger* clientm_,
         Objecter* objecter_, Client* client_, LogChannelRef clog_, LogChannelRef audit_clog_)
    : monc(monc_)
    , objecter(objecter_)
    , client(client_)
    ...
    , server(monc, finisher, daemon_state, cluster_state, *py_module_registry, clog_, audit_clog_)
    , clog(clog_)
...
void MetadataUpdate::finish(int r)
{
    ...
    if (r == 0) {
        if (key.type == "mds" || key.type == "osd" || key.type == "mgr" || key.type == "mon") {
            ...
            if (json_result.type() != json_spirit::obj_type) {
                dout(1) << "mon returned valid JSON " << key << " but not an object: '" << outbl.to_str() << "'"
                        << dendl;
                return;
            }
...
void Mgr::init()
{
    ...
    monc->with_monmap([&](const MonMap& monmap) {
        if (monmap.get_required_features().contains_all(ceph::features::mon::FEATURE_PACIFIC)) {
    ...
    epoch_t e;
    cluster_state.with_mgrmap([&e](const MgrMap& m) { e = m.last_failure_osd_epoch; });
    ...
    cluster_state.with_osdmap_and_pgmap(
        [this](const OSDMap& osd_map, const PGMap& pg_map) { cluster_state.notify_osdmap(osd_map); });
    ...
    // own modules.
    for (auto p = pre_init_store.lower_bound("device/"); p != pre_init_store.end() && p->first.find("device/") == 0;
         ++p) {
        ...
        if (r < 0) {
            ...
            daemon_state.with_device_create(devid, [&meta](DeviceState& dev) { dev.set_metadata(std::move(meta)); });
        }
    }

    // assume finisher already initialized in background_init
    dout(4) << "starting python modules..." << dendl;
    py_module_registry->active_start(daemon_state,
                                     cluster_state,
                                     pre_init_store,
                                     mon_allows_kv_sub,
                                     *monc,
    ...
    r = admin_socket->register_command("mgr_status", this, "Dump mgr status");
    ceph_assert(r == 0);
...
void Mgr::load_all_metadata()
{
    ...
    for (auto& metadata_val : mds_cmd.json_result.get_array()) {
        ...
        dm->key = DaemonKey{"mds", daemon_meta.at("name").get_str()};
    ...
    for (auto& metadata_val : mon_cmd.json_result.get_array()) {
        ...
        DaemonStatePtr dm = std::make_shared<DaemonState>(daemon_state.types);
        dm->key = DaemonKey{"mon", daemon_meta.at("name").get_str()};
        dm->hostname = daemon_meta.at("hostname").get_str();
    ...
    for (auto& osd_metadata_val : osd_cmd.json_result.get_array()) {
        ...
        DaemonStatePtr dm = std::make_shared<DaemonState>(daemon_state.types);
        dm->key = DaemonKey{"osd", stringify(osd_metadata.at("id").get_int())};
...
void Mgr::handle_osd_map()
{
     ...
     * reload the metadata.
     */
    cluster_state.with_osdmap_and_pgmap([this, &names_exist](const OSDMap& osd_map, const PGMap& pg_map) {
        for (int osd_id = 0; osd_id < osd_map.get_max_osd(); ++osd_id) {
            ...
            if (daemon_state.exists(k)) {
                if (osd_map.get_up_from(osd_id) == osd_map.get_epoch()) {
                    dout(4) << "Mgr::handle_osd_map: osd." << osd_id << " joined cluster at "
                            << "e" << osd_map.get_epoch() << dendl;
                    update_meta = true;
                }
            }
            else {
            ...
            if (update_meta) {
                ...
                cmd << "{\"prefix\": \"osd metadata\", \"id\": " << osd_id << "}";
                monc->start_mon_command({cmd.str()}, {}, &c->outbl, &c->outs, c);
            }
        }
...
void Mgr::handle_mon_map()
{
    ...
    for (const auto& name : names_exist) {
        ...
        monc->start_mon_command({fmt::format(cmd, name)}, {}, &c->outbl, &c->outs, c);
    }
...
bool Mgr::ms_dispatch2(const ref_t<Message>& m)
{
    ...
    case MSG_KV_DATA:
    {
        ...
        if (!msg->data.empty()) {
            if (initialized) {
                py_module_registry->update_kv_data(msg->prefix, msg->incremental, msg->data);
            }
            else {
...
void Mgr::handle_fs_map(ref_t<MFSMap> m)
{
    ...
    for (const auto& i : mds_info) {
        ...
        if (daemon_state.exists(k)) {
            ...
            if (metadata->metadata.empty() || metadata->metadata.count("addr") == 0) {
                update = true;
            }
            else {
        ...
        if (update) {
            ...
            std::ostringstream cmd;
            cmd << "{\"prefix\": \"mds metadata\", \"who\": \"" << info.name << "\"}";
            monc->start_mon_command({cmd.str()}, {}, &c->outbl, &c->outs, c);
        }
    }
...
bool Mgr::got_mgr_map(const MgrMap& m)
{
    ...
    cluster_state.with_mgrmap([&](const MgrMap& m) { old_modules = m.modules; });
...
int Mgr::call(std::string_view admin_command, const cmdmap_t& cmdmap, const bufferlist&, Formatter* f,
              std::ostream& errss, bufferlist& out)
{
    try {
        if (admin_command == "mgr_status") {
            f->open_object_section("mgr_status");
            cluster_state.with_mgrmap([f](const MgrMap& mm) { f->dump_unsigned("mgrmap_epoch", mm.get_epoch()); });
