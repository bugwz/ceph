...
typedef struct
{
    PyObject_HEAD ActivePyModules* py_modules;
...
public:
    ...
    MonCommandCompletion(ActivePyModules* py_modules_, PyObject* ev, const std::string& tag_, PyThreadState* ts_)
...
static PyObject* ceph_send_command(BaseMgrModule* self, PyObject* args)
{
    // Like mon, osd, mds
    char* type = nullptr;
    ...
    if (!PyArg_ParseTuple(
            args, "Ossssz#:ceph_send_command", &completion, &type, &name, &cmd_json, &tag, &inbuf_ptr, &inbuf_len)) {
        return nullptr;
    }
    ...
    MonCommandCompletion* command_c = new MonCommandCompletion(self->py_modules, completion, tag, PyThreadState_Get());
    ...
    if (std::string(type) == "mon") {
        ...
        auto c = new LambdaContext([command_c, self](int command_r) {
            self->py_modules->get_objecter().wait_for_latest_osdmap(
                [command_c, command_r](boost::system::error_code) { command_c->complete(command_r); });
        });

        self->py_modules->get_monc().start_mon_command(name,
                                                       {cmd_json},
                                                       inbuf,
                                                       &command_c->outbl,
                                                       &command_c->outs,
        ...
        self->py_modules->get_objecter().osd_command(
            ...
            &tid,
            [command_c,
             f = &self->py_modules->cmd_finisher](boost::system::error_code ec, std::string s, ceph::buffer::list bl) {
        ...
        int r =
            self->py_modules->get_client().mds_command(name,
                                                       {cmd_json},
                                                       inbuf,
                                                       &command_c->outbl,
                                                       &command_c->outs,
...
static PyObject* ceph_set_health_checks(BaseMgrModule* self, PyObject* args)
{
            ...
            if (ks == "severity") {
                ...
                for (int k = 0; k < PyList_Size(v); ++k) {
                    ...
                    if (!PyUnicode_Check(di)) {
                        derr << __func__ << " check " << check_name << " detail item " << k << " not a [unicode] string"
                             << dendl;
                        continue;
                    }
                    else {
    ...
    jf.flush(*_dout);
    *_dout << dendl;
    without_gil([&] { self->py_modules->set_health_checks(self->this_module->get_name(), std::move(out_checks)); });
    Py_RETURN_NONE;
}
...
static PyObject* ceph_get_server(BaseMgrModule* self, PyObject* args)
{
    char* hostname = NULL;
...
static PyObject* ceph_get_mgr_id(BaseMgrModule* self, PyObject* args)
{
    return PyUnicode_FromString(g_conf()->name.get_id().c_str());
}

static PyObject* ceph_option_get(BaseMgrModule* self, PyObject* args)
{
    char* what = nullptr;
    ...
    if (opt) {
        ...
        case -ENOMEM: PyErr_NoMemory(); return nullptr;
        case -ENAMETOOLONG: PyErr_SetString(PyExc_ValueError, "value too long"); return nullptr;
        default: ceph_assert(r == 0); break;
        }
...
static PyObject* ceph_foreign_option_get(BaseMgrModule* self, PyObject* args)
{
    char* who = nullptr;
    char* what = nullptr;
...
static PyObject* ceph_get_module_option(BaseMgrModule* self, PyObject* args)
{
    char* module = nullptr;
    char* key = nullptr;
    char* prefix = nullptr;
    if (!PyArg_ParseTuple(args, "ss|s:ceph_get_module_option", &module, &key, &prefix)) {
        derr << "Invalid args!" << dendl;
        return nullptr;
    }
...
static PyObject* ceph_store_get_prefix(BaseMgrModule* self, PyObject* args)
{
    char* prefix = nullptr;
    ...
    return self->py_modules->get_store_prefix(self->this_module->get_name(), prefix);
}

static PyObject* ceph_set_module_option(BaseMgrModule* self, PyObject* args)
{
    char* module = nullptr;
    char* key = nullptr;
    char* value = nullptr;
    if (!PyArg_ParseTuple(args, "ssz:ceph_set_module_option", &module, &key, &value)) {
        derr << "Invalid args!" << dendl;
        return nullptr;
    }
    ...
    auto [ret, msg] = without_gil([&] { return self->py_modules->set_config(module, key, val); });
    if (ret) {
        PyErr_SetString(PyExc_ValueError, msg.c_str());
        return nullptr;
    }
...
static PyObject* ceph_store_get(BaseMgrModule* self, PyObject* args)
{
    char* what = nullptr;
    ...
    std::string value;
    bool found = self->py_modules->get_store(self->this_module->get_name(), what, &value);
    if (found) {
...
static PyObject* ceph_store_set(BaseMgrModule* self, PyObject* args)
{
    ...
    without_gil([&] { self->py_modules->set_store(self->this_module->get_name(), key, val); });
    Py_RETURN_NONE;
}

static PyObject* get_metadata(BaseMgrModule* self, PyObject* args)
{
    char* svc_name = NULL;
    char* svc_id = NULL;
...
static PyObject* get_daemon_status(BaseMgrModule* self, PyObject* args)
{
    char* svc_name = NULL;
    char* svc_id = NULL;
    if (!PyArg_ParseTuple(args, "ss:get_daemon_status", &svc_name, &svc_id)) {
        return nullptr;
    }
    return self->py_modules->get_daemon_status_python(svc_name, svc_id);
}

static PyObject* ceph_log(BaseMgrModule* self, PyObject* args)
{
    char* record = nullptr;
...
static PyObject* ceph_cluster_log(BaseMgrModule* self, PyObject* args)
{
    int prio = 0;
    ...
    without_gil([&] { self->py_modules->cluster_log(channel, (clog_type)prio, message); });
    Py_RETURN_NONE;
}

static PyObject* ceph_get_version(BaseMgrModule* self, PyObject* args)
{
    return PyUnicode_FromString(pretty_version_to_str().c_str());
}

static PyObject* ceph_get_ceph_conf_path(BaseMgrModule* self, PyObject* args)
{
    return PyUnicode_FromString(g_conf().get_conf_path().c_str());
}

static PyObject* ceph_get_release_name(BaseMgrModule* self, PyObject* args)
{
    return PyUnicode_FromString(ceph_release_to_str());
}

static PyObject* ceph_lookup_release_name(BaseMgrModule* self, PyObject* args)
{
    int major = 0;
...
static PyObject* ceph_get_context(BaseMgrModule* self)
{
    return self->py_modules->get_context();
}

static PyObject* get_counter(BaseMgrModule* self, PyObject* args)
{
    char* svc_name = nullptr;
    char* svc_id = nullptr;
    char* counter_path = nullptr;
    if (!PyArg_ParseTuple(args, "sss:get_counter", &svc_name, &svc_id, &counter_path)) {
        return nullptr;
    }
    return self->py_modules->get_counter_python(svc_name, svc_id, counter_path);
}

static PyObject* get_latest_counter(BaseMgrModule* self, PyObject* args)
{
    char* svc_name = nullptr;
    char* svc_id = nullptr;
    char* counter_path = nullptr;
    ...
    return self->py_modules->get_latest_counter_python(svc_name, svc_id, counter_path);
}
...
static PyObject* ceph_get_rocksdb_version(BaseMgrModule* self)
{
    return self->py_modules->get_rocksdb_version();
}


static PyObject* ceph_get_osdmap(BaseMgrModule* self, PyObject* args)
{
    return self->py_modules->get_osdmap();
}

static PyObject* ceph_set_uri(BaseMgrModule* self, PyObject* args)
{
    char* svc_str = nullptr;
    if (!PyArg_ParseTuple(args, "s:ceph_advertize_service", &svc_str)) {
        return nullptr;
    }
    // We call down into PyModules even though we have a MgrPyModule
    // reference here, because MgrPyModule's fields are protected
    // by PyModules' lock.
    without_gil([&] { self->py_modules->set_uri(self->this_module->get_name(), svc_str); });
    Py_RETURN_NONE;
}

static PyObject* ceph_set_wear_level(BaseMgrModule* self, PyObject* args)
{
    char* devid = nullptr;
    float wear_level;
    if (!PyArg_ParseTuple(args, "sf:ceph_set_wear_level", &devid, &wear_level)) {
        return nullptr;
    }
    without_gil([&] { self->py_modules->set_device_wear_level(devid, wear_level); });
    Py_RETURN_NONE;
}

static PyObject* ceph_have_mon_connection(BaseMgrModule* self, PyObject* args)
{
...
static PyObject* ceph_update_progress_event(BaseMgrModule* self, PyObject* args)
{
    char* evid = nullptr;
    char* desc = nullptr;
    float progress = 0.0;
    bool add_to_ceph_s = false;
    if (!PyArg_ParseTuple(args, "ssfb:ceph_update_progress_event", &evid, &desc, &progress, &add_to_ceph_s)) {
        return nullptr;
    }
    without_gil([&] { self->py_modules->update_progress_event(evid, desc, progress, add_to_ceph_s); });
    Py_RETURN_NONE;
}

static PyObject* ceph_complete_progress_event(BaseMgrModule* self, PyObject* args)
{
    char* evid = nullptr;
    if (!PyArg_ParseTuple(args, "s:ceph_complete_progress_event", &evid)) {
        return nullptr;
    }
    without_gil([&] { self->py_modules->complete_progress_event(evid); });
    Py_RETURN_NONE;
}

static PyObject* ceph_clear_all_progress_events(BaseMgrModule* self, PyObject* args)
{
    without_gil([&] { self->py_modules->clear_all_progress_events(); });
    Py_RETURN_NONE;
}



static PyObject* ceph_dispatch_remote(BaseMgrModule* self, PyObject* args)
{
    char* other_module = nullptr;
    char* method = nullptr;
    PyObject* remote_args = nullptr;
    PyObject* remote_kwargs = nullptr;
    if (!PyArg_ParseTuple(args, "ssOO:ceph_dispatch_remote", &other_module, &method, &remote_args, &remote_kwargs)) {
        return nullptr;
    }
    ...
    std::string err;
    auto result = self->py_modules->dispatch_remote(other_module, method, remote_args, remote_kwargs, &err);
...
static PyObject* ceph_add_osd_perf_query(BaseMgrModule* self, PyObject* args)
{
    ...
    static const std::string NAME_COUNTERS_DESCRIPTORS = "performance_counter_descriptors";
        ...
        if (query_param_name == NAME_KEY_DESCRIPTOR) {
            ...
            for (int j = 0; j < PyList_Size(query_param_val); j++) {
                ...
                for (int k = 0; k < PyList_Size(sub_key_params); ++k) {
                    ...
                    if (!PyTuple_Check(pair)) {
                        derr << __func__ << " query " << query_param_name << " item " << j << " pair " << k
                             << " not a tuple" << dendl;
                        Py_RETURN_NONE;
                    }
                    ...
                    if (param_name == NAME_SUB_KEY_TYPE) {
                        ...
                        if (d.regex.mark_count() == 0) {
                            derr << __func__ << " query " << query_param_name << " item " << j << " regex "
                                 << d.regex_str << ": no capturing groups" << dendl;
                            Py_RETURN_NONE;
                        }
                    }
                    else {
                        derr << __func__ << " query " << query_param_name << " item " << j << " contains invalid param "
                             << param_name << dendl;
                        Py_RETURN_NONE;
                    }
                }
                if (d.type == static_cast<OSDPerfMetricSubKeyType>(-1) || d.regex_str.empty()) {
            ...
            for (int j = 0; j < PyList_Size(query_param_val); j++) {
                ...
                if (it == counter_types.end()) {
                    derr << __func__ << " query " << query_param_name << " item " << type << " is not valid type"
                         << dendl;
                    Py_RETURN_NONE;
                }
            ...
            for (int j = 0; j < PyList_Size(limit_params); ++j) {
                ...
                if (!PyArg_ParseTuple(kv, "sO:pair", &limit_param_name, &limit_param_val)) {
    ...
    if (query.key_descriptor.empty() || query.performance_counter_descriptors.empty()) {
    ...
    if (limit) {
        ...
        if (std::find(ds.begin(), ds.end(), limit->order_by) == ds.end()) {
            derr << __func__ << " limit order_by " << limit->order_by << " not in performance_counter_descriptors"
                 << dendl;
            Py_RETURN_NONE;
        }
    }
...
static PyObject* ceph_remove_osd_perf_query(BaseMgrModule* self, PyObject* args)
{
...
static PyObject* ceph_get_osd_perf_counters(BaseMgrModule* self, PyObject* args)
{
    MetricQueryID query_id;
...
static PyObject* ceph_add_mds_perf_query(BaseMgrModule* self, PyObject* args)
{
    static const std::string NAME_KEY_DESCRIPTOR = "key_descriptor";
    static const std::string NAME_COUNTERS_DESCRIPTORS = "performance_counter_descriptors";
    static const std::string NAME_LIMIT = "limit";
    ...
    for (int i = 0; i < PyList_Size(query_params); ++i) {
        ...
        if (query_param_name == NAME_KEY_DESCRIPTOR) {
            ...
            for (int j = 0; j < PyList_Size(query_param_val); j++) {
                ...
                if (d.type == static_cast<MDSPerfMetricSubKeyType>(-1) || d.regex_str.empty()) {
                    derr << __func__ << " query " << query_param_name << " item " << i << " invalid" << dendl;
                    Py_RETURN_NONE;
                }
                query.key_descriptor.push_back(d);
...
static PyObject* ceph_remove_mds_perf_query(BaseMgrModule* self, PyObject* args)
{
    MetricQueryID query_id;
...
static PyObject* ceph_reregister_mds_perf_queries(BaseMgrModule* self, PyObject* args)
{
    self->py_modules->reregister_mds_perf_queries();
    Py_RETURN_NONE;
}

static PyObject* ceph_get_mds_perf_counters(BaseMgrModule* self, PyObject* args)
{
    MetricQueryID query_id;
...
static PyObject* ceph_is_authorized(BaseMgrModule* self, PyObject* args)
{
    ...
    bool r = without_gil([&] { return self->this_module->is_authorized(arguments); });
...
static PyObject* ceph_register_client(BaseMgrModule* self, PyObject* args)
{
    ...
    without_gil([&] { self->py_modules->register_client(name, addrs, replace); });
    Py_RETURN_NONE;
}

static PyObject* ceph_unregister_client(BaseMgrModule* self, PyObject* args)
{
    const char* _name = nullptr;
    char* addrs = nullptr;
    ...
    auto name = _name ? std::string(_name) : std::string(self->this_module->get_name());
    without_gil([&] { self->py_modules->unregister_client(name, addrs); });
    Py_RETURN_NONE;
}

static PyObject* ceph_get_daemon_health_metrics(BaseMgrModule* self, PyObject* args)
{
    return self->py_modules->get_daemon_health_metrics();
}

PyMethodDef BaseMgrModule_methods[] = {
    {"_ceph_get", (PyCFunction)ceph_state_get, METH_VARARGS, "Get a cluster object"},

    {"_ceph_get_server", (PyCFunction)ceph_get_server, METH_VARARGS, "Get a server object"},

    {"_ceph_get_metadata", (PyCFunction)get_metadata, METH_VARARGS, "Get a service's metadata"},

    {"_ceph_get_daemon_status", (PyCFunction)get_daemon_status, METH_VARARGS, "Get a service's status"},

    {"_ceph_send_command", (PyCFunction)ceph_send_command, METH_VARARGS, "Send a mon command"},

    {"_ceph_set_health_checks", (PyCFunction)ceph_set_health_checks, METH_VARARGS, "Set health checks for this module"},

    {"_ceph_get_mgr_id",
     (PyCFunction)ceph_get_mgr_id,
     METH_NOARGS,
     "Get the name of the Mgr daemon where we are running"},

    {"_ceph_get_ceph_conf_path", (PyCFunction)ceph_get_ceph_conf_path, METH_NOARGS, "Get path to ceph.conf"},

    {"_ceph_get_option", (PyCFunction)ceph_option_get, METH_VARARGS, "Get a native configuration option value"},

    {"_ceph_get_foreign_option",
     (PyCFunction)ceph_foreign_option_get,
     METH_VARARGS,
     "Get a native configuration option value for another entity"},

    {"_ceph_get_module_option",
     (PyCFunction)ceph_get_module_option,
     METH_VARARGS,
     "Get a module configuration option value"},

    {"_ceph_get_store_prefix",
     (PyCFunction)ceph_store_get_prefix,
     METH_VARARGS,
     "Get all KV store values with a given prefix"},

    {"_ceph_set_module_option",
     (PyCFunction)ceph_set_module_option,
     METH_VARARGS,
     "Set a module configuration option value"},

    {"_ceph_get_store", (PyCFunction)ceph_store_get, METH_VARARGS, "Get a stored field"},

    {"_ceph_set_store", (PyCFunction)ceph_store_set, METH_VARARGS, "Set a stored field"},

    {"_ceph_get_counter", (PyCFunction)get_counter, METH_VARARGS, "Get a performance counter"},

    {"_ceph_get_latest_counter", (PyCFunction)get_latest_counter, METH_VARARGS, "Get the latest performance counter"},

    {"_ceph_get_perf_schema", (PyCFunction)get_perf_schema, METH_VARARGS, "Get the performance counter schema"},

    {"_ceph_get_rocksdb_version",
     (PyCFunction)ceph_get_rocksdb_version,
     METH_NOARGS,
     "Get the current RocksDB version number"},

    {"_ceph_log", (PyCFunction)ceph_log, METH_VARARGS, "Emit a (local) log message"},

    {"_ceph_cluster_log", (PyCFunction)ceph_cluster_log, METH_VARARGS, "Emit a cluster log message"},

    {"_ceph_get_version", (PyCFunction)ceph_get_version, METH_NOARGS, "Get the ceph version of this process"},

    {"_ceph_get_release_name",
     (PyCFunction)ceph_get_release_name,
     METH_NOARGS,
     "Get the ceph release name of this process"},

    {"_ceph_lookup_release_name",
     (PyCFunction)ceph_lookup_release_name,
     METH_VARARGS,
     "Get the ceph release name for a given major number"},

    {"_ceph_get_context", (PyCFunction)ceph_get_context, METH_NOARGS, "Get a CephContext* in a python capsule"},

    {"_ceph_get_osdmap", (PyCFunction)ceph_get_osdmap, METH_NOARGS, "Get an OSDMap* in a python capsule"},

    {"_ceph_set_uri", (PyCFunction)ceph_set_uri, METH_VARARGS, "Advertize a service URI served by this module"},

    {"_ceph_set_device_wear_level", (PyCFunction)ceph_set_wear_level, METH_VARARGS, "Set device wear_level value"},

    {"_ceph_have_mon_connection",
     (PyCFunction)ceph_have_mon_connection,
     METH_NOARGS,
     "Find out whether this mgr daemon currently has "
     "a connection to a monitor"},

    {"_ceph_update_progress_event",
     (PyCFunction)ceph_update_progress_event,
     METH_VARARGS,
     "Update status of a progress event"},
    {"_ceph_complete_progress_event",
     (PyCFunction)ceph_complete_progress_event,
     METH_VARARGS,
    ...
    {"_ceph_dispatch_remote", (PyCFunction)ceph_dispatch_remote, METH_VARARGS, "Dispatch a call to another module"},

    {"_ceph_add_osd_perf_query", (PyCFunction)ceph_add_osd_perf_query, METH_VARARGS, "Add an osd perf query"},

    {"_ceph_remove_osd_perf_query", (PyCFunction)ceph_remove_osd_perf_query, METH_VARARGS, "Remove an osd perf query"},

    {"_ceph_get_osd_perf_counters", (PyCFunction)ceph_get_osd_perf_counters, METH_VARARGS, "Get osd perf counters"},

    {"_ceph_add_mds_perf_query", (PyCFunction)ceph_add_mds_perf_query, METH_VARARGS, "Add an mds perf query"},
    ...
    {"_ceph_reregister_mds_perf_queries",
     (PyCFunction)ceph_reregister_mds_perf_queries,
     METH_NOARGS,
     "Re-register mds perf queries"},

    {"_ceph_get_mds_perf_counters", (PyCFunction)ceph_get_mds_perf_counters, METH_VARARGS, "Get mds perf counters"},

    {"_ceph_is_authorized", (PyCFunction)ceph_is_authorized, METH_VARARGS, "Verify the current session caps are valid"},

    {"_ceph_register_client",
     (PyCFunction)ceph_register_client,
     METH_VARARGS,
     "Register RADOS instance for potential blocklisting"},

    {"_ceph_unregister_client",
     (PyCFunction)ceph_unregister_client,
     METH_VARARGS,
     "Unregister RADOS instance for potential blocklisting"},

    {"_ceph_get_daemon_health_metrics",
     (PyCFunction)ceph_get_daemon_health_metrics,
     METH_VARARGS,
...
static PyObject* BaseMgrModule_new(PyTypeObject* type, PyObject* args, PyObject* kwds)
{
    BaseMgrModule* self;
...
static int BaseMgrModule_init(BaseMgrModule* self, PyObject* args, PyObject* kwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwds, "OO", const_cast<char**>(kwlist), &py_modules_capsule, &this_module_capsule)) {
        return -1;
    }

    self->py_modules = static_cast<ActivePyModules*>(PyCapsule_GetPointer(py_modules_capsule, nullptr));
    ceph_assert(self->py_modules);
    self->this_module = static_cast<ActivePyModule*>(PyCapsule_GetPointer(this_module_capsule, nullptr));
    ceph_assert(self->this_module);
...
PyTypeObject BaseMgrModuleType = {
    PyVarObject_HEAD_INIT(NULL, 0) "ceph_module.BaseMgrModule", /* tp_name */
