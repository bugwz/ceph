...
template<typename Query, typename Limit, typename Key, typename Report>
int MetricCollector<Query, Limit, Key, Report>::get_counters_generic(MetricQueryID query_id,
...
template<typename Query, typename Limit, typename Key, typename Report>
void MetricCollector<Query, Limit, Key, Report>::process_reports_generic(const std::map<Query, Report>& reports,
                                                                         UpdateCallback callback)
{
    ...
    for (auto& [query, report] : reports) {
        dout(10) << "report for " << query << " query: " << report.group_packed_performance_counters.size()
                 << " records" << dendl;
        ...
        for (auto& [key, bl] : report.group_packed_performance_counters) {
            ...
            for (auto& p : queries[query]) {
                ...
                if (key_counters.empty()) {
                    key_counters.resize(query.performance_counter_descriptors.size(), {0, 0});
                }
            }
...
template class MetricCollector<OSDPerfMetricQuery, OSDPerfMetricLimit, OSDPerfMetricKey, OSDPerfMetricReport>;
