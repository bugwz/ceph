...
void MgrClient::shutdown()
{
    ...
    // (e.g., by control-C)
    command_table.clear();
    if (service_daemon && session && session->con && HAVE_FEATURE(session->con->get_features(), SERVER_MIMIC)) {
        ...
        auto timeout = ceph::make_timespan(cct->_conf.get_val<double>("mgr_client_service_daemon_unregister_timeout"));
...
bool MgrClient::ms_dispatch2(const ref_t<Message>& m)
{
    ...
    switch (m->get_type()) {
    case MSG_MGR_MAP: return handle_mgr_map(ref_cast<MMgrMap>(m));
    case MSG_MGR_CONFIGURE: return handle_mgr_configure(ref_cast<MMgrConfigure>(m));
    case MSG_MGR_CLOSE: return handle_mgr_close(ref_cast<MMgrClose>(m));
    ...
    default: ldout(cct, 30) << "Not handling " << *m << dendl; return false;
    }
}
...
void MgrClient::reconnect()
{
    ...
    if (!clock_t::is_zero(last_connect_attempt)) {
        ...
        auto when =
            last_connect_attempt + ceph::make_timespan(cct->_conf.get_val<double>("mgr_connect_retry_interval"));
        if (now < when) {
            if (!connect_retry_callback) {
                connect_retry_callback = timer.add_event_at(when, new LambdaContext([this](int r) {
                                                                connect_retry_callback = nullptr;
                                                                reconnect();
                                                            }));
            }
    ...
    session->con = msgr->connect_to(CEPH_ENTITY_TYPE_MGR, map.get_active_addrs());
    ...
    while (p != command_table.get_commands().end()) {
        ...
        if (op.tell) {
            ...
            m = op.get_message({}, HAVE_FEATURE(map.active_mgr_features, SERVER_OCTOPUS));
        }
        ceph_assert(session);
...
bool MgrClient::handle_mgr_map(ref_t<MMgrMap> m)
{
    ...
    // Reset session?
    if (!session || session->con->get_peer_addrs() != map.get_active_addrs()) {
        reconnect();
    }
...
void MgrClient::_send_report()
{
    ...
    pcc->with_counters([this, report](const PerfCountersCollectionImpl::CounterMap& by_path) {
        ...
        ldout(cct, 20) << "sending " << session->declared.size()
                       << " counters ("
                          "of possible "
                       << by_path.size() << "), " << report->declare_types.size() << " new, "
                       << report->undeclare_types.size() << " removed" << dendl;
    });
    ...
    cct->_conf.get_config_bl(last_config_bl_version, &report->config_bl, &last_config_bl_version);
...
int MgrClient::start_command(const vector<string>& cmd, const bufferlist& inbl, bufferlist* outbl, string* outs,
                             Context* onfinish)
{
    std::lock_guard l(lock);
    ...
    if (session && session->con) {
        ...
        auto m = op.get_message({}, HAVE_FEATURE(map.active_mgr_features, SERVER_OCTOPUS));
        session->con->send_message2(std::move(m));
    }
    else {
...
int MgrClient::start_tell_command(const string& name, const vector<string>& cmd, const bufferlist& inbl,
...
bool MgrClient::handle_command_reply(uint64_t tid, bufferlist& data, const std::string& rs, int r)
{
    ceph_assert(ceph_mutex_is_locked_by_me(lock));
...
int MgrClient::update_daemon_metadata(const std::string& service, const std::string& name,
                                      const std::map<std::string, std::string>& metadata)
{
    ...
    if (need_metadata_update && !daemon_metadata.empty()) {
        _send_update();
...
int MgrClient::service_daemon_register(const std::string& service, const std::string& name,
                                       const std::map<std::string, std::string>& metadata)
{
    std::lock_guard l(lock);
    if (service_daemon) {
...
int MgrClient::service_daemon_update_status(std::map<std::string, std::string>&& status)
{
    std::lock_guard l(lock);
...
int MgrClient::service_daemon_update_task_status(std::map<std::string, std::string>&& status)
{
    std::lock_guard l(lock);
    ldout(cct, 10) << status << dendl;
