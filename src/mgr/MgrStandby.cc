...
MgrStandby::MgrStandby(int argc, const char** argv)
    ...
    , client_messenger(Messenger::create(g_ceph_context, cct->_conf.get_val<std::string>("ms_type"),
                                         entity_name_t::MGR(), "mgr", Messenger::get_pid_nonce()))
...
{
    static const char* KEYS[] = {// clog & admin clog
                                 "clog_to_monitors",
                                 "clog_to_syslog",
...
void MgrStandby::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    ...
    if (changed.count("mgr_standby_modules") && !active_mgr) {
        if (g_conf().get_val<bool>("mgr_standby_modules") != py_module_registry.have_standby_modules()) {
            dout(1) << "mgr_standby_modules now " << (int)g_conf().get_val<bool>("mgr_standby_modules")
                    << ", standby modules are " << (py_module_registry.have_standby_modules() ? "" : "not ")
                    << "active, respawning" << dendl;
            respawn();
        }
    }
}
...
int MgrStandby::init()
{
    ...
    monc.register_config_notify_callback([this]() { py_module_registry.handle_config_notify(); });
...
void MgrStandby::tick()
{
    ...
    timer.add_event_after(g_conf().get_val<std::chrono::seconds>("mgr_tick_period").count(),
                          new LambdaContext([this](int r) { tick(); }));
}
...
void MgrStandby::handle_mgr_map(ref_t<MMgrMap> mmap)
{
    ...
    if (active_in_map) {
        if (!active_mgr) {
            ...
            active_mgr.reset(
                new Mgr(&monc, map, &py_module_registry, client_messenger.get(), &objecter, &client, clog, audit_clog));
            active_mgr->background_init(new LambdaContext([this](int r) {
        ...
        if (map.active_gid != 0 && map.active_name != g_conf()->name.get_id()) {
            ...
            if (!py_module_registry.is_standby_running() && g_conf().get_val<bool>("mgr_standby_modules")) {
