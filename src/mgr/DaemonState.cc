...
void DeviceState::print(ostream& out) const
{
    out << "device " << devid << "\n";
    for (auto& i : attachments) {
        out << "attachment " << std::get<0>(i) << " " << std::get<1>(i) << " " << std::get<2>(i) << "\n";
        out << "\n";
    }
    std::copy(std::begin(daemons), std::end(daemons), std::experimental::make_ostream_joiner(out, ","));
    out << '\n';
    if (life_expectancy.first != utime_t()) {
        out << "life_expectancy " << life_expectancy.first << " to " << life_expectancy.second << " (as of "
...
void DaemonState::set_metadata(const std::map<std::string, std::string>& m)
{
    ...
    if (auto found = m.find("device_ids"); found != m.end()) {
        ...
        for_each_pair(device_ids, ",; ", [&paths, this](std::string_view devname, std::string_view id) {
            // skip blank ids
            if (id.empty()) {
                return;
            }
...
void DaemonStateIndex::_insert(DaemonStatePtr dm)
{
    ...
    for (auto& i : dm->devices) {
        ...
        if (p != dm->devices_bypath.end()) {
            ...
            d->attachments.insert(std::make_tuple(dm->hostname, i.second, std::string()));
        }
    }
}
...
DaemonStateCollection DaemonStateIndex::get_by_service(const std::string& svc) const
{
...
void DaemonStateIndex::cull(const std::string& svc_name, const std::set<std::string>& names_exist)
{
    ...
    for (auto& i = begin; i != end; ++i) {
        ...
        if (daemon_key.type != svc_name) break;
...
{
    dout(20) << "loading " << report.declare_types.size() << " new types, " << report.undeclare_types.size()
             << " old types, had " << types.size() << " types, got " << report.packed.length() << " bytes of data"
             << dendl;
...
void PerfCounterInstance::push_avg(utime_t t, uint64_t const& s, uint64_t const& c)
{
