...
template<class Key, class Value> void TTLCacheBase<Key, Value>::insert(Key key, Value value)
{
    auto now = std::chrono::steady_clock::now();

    if (!ttl) return;
    int16_t random_ttl_offset = ttl * ttl_spread_ratio * (2l * rand() / float(RAND_MAX) - 1);
...
template<class Key, class Value> void TTLCacheBase<Key, Value>::erase(Key key)
{
    cache::erase(key);
}
...
template<class Key, class Value> bool TTLCacheBase<Key, Value>::expired(Key key)
{
...
template<class Key, class Value> Value TTLCacheBase<Key, Value>::get_value(Key key, bool count_hit)
{
...
template<class Key, class Value> ttl_time_point TTLCacheBase<Key, Value>::get_value_time_point(Key key)
{
...
template<class Key, class Value> void TTLCacheBase<Key, Value>::set_ttl(uint16_t ttl)
{
    this->ttl = ttl;
}

template<class Key, class Value> bool TTLCacheBase<Key, Value>::exists(Key key)
{
    return cache::exists(key);
}

template<class Key, class Value> void TTLCacheBase<Key, Value>::throw_key_not_found(Key key)
{
