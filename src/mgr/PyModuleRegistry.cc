...
bool PyModuleRegistry::handle_mgr_map(const MgrMap& mgr_map_)
{
    ...
    if (mgr_map.epoch == 0) {
        ...
        bool modules_changed =
            mgr_map_.modules != mgr_map.modules || mgr_map_.always_on_modules != mgr_map.always_on_modules;
        mgr_map = mgr_map_;
...
void PyModuleRegistry::standby_start(MonClient& mc, Finisher& f)
{
    ...
    standby_modules.reset(new StandbyPyModules(mgr_map, module_config, clog, mc, f));
    ...
    for (const auto& i : modules) {
        ...
        if (i.second->pStandbyClass) {
            ...
            dout(4) << "skipping module '" << i.second->get_name()
                    << "' because "
                       "it does not implement a standby mode"
                    << dendl;
        }
    }
...
void PyModuleRegistry::active_start(DaemonStateIndex& ds, ClusterState& cs,
                                    const std::map<std::string, std::string>& kv_store, bool mon_provides_kv_sub,
                                    MonClient& mc, LogChannelRef clog_, LogChannelRef audit_clog_, Objecter& objecter_,
...
int PyModuleRegistry::handle_command(const ModuleCommand& module_command, const MgrSession& session,
                                     const cmdmap_t& cmdmap, const bufferlist& inbuf, std::stringstream* ds,
                                     std::stringstream* ss)
{
    if (active_modules) {
        return active_modules->handle_command(module_command, session, cmdmap, inbuf, ds, ss);
    }
    else {
...
std::vector<MonCommand> PyModuleRegistry::get_commands() const
{
    ...
    for (auto& pyc : commands) {
        ...
        result.push_back({pyc.cmdstring, pyc.helpstring, "mgr", pyc.perm, flags});
    }
    return result;
}
...
void PyModuleRegistry::get_health_checks(health_check_map_t* checks)
{
    ...
    if (active_modules) {
        ...
        for (const auto& i : modules) {
            ...
            else if ((module->is_enabled() && !module->is_loaded()) || (module->is_failed() && module->get_can_run())) {
        ...
        if (!dependency_modules.empty()) {
            ...
            if (dependency_modules.size() == 1) {
                ...
                ss << "Module '" << iter->first << "' has failed dependency: " << iter->second;
            }
            else if (dependency_modules.size() > 1) {
                ss << dependency_modules.size() << " mgr modules have failed dependencies";
            }
            auto& d = checks->add("MGR_MODULE_DEPENDENCY", HEALTH_WARN, ss.str(), dependency_modules.size());
        ...
        if (!failed_modules.empty()) {
            ...
            auto& d = checks->add("MGR_MODULE_ERROR", HEALTH_ERR, ss.str(), failed_modules.size());
