...
std::string handle_pyerror(bool generate_crash_dump = false, std::string module = {}, std::string caller = {});
...
private:
    ...
    int walk_dict_list(const std::string& attr_name, std::function<int(PyObject*)> fn);
...
public:
    ...
    const std::map<std::string, MgrMap::ModuleOption>& get_options() const { return options; }

    PyObject* get_typed_option_value(const std::string& option, const std::string& value);
    ...
    void set_always_on(const bool always_on_) { always_on = always_on_; }
    ...
    bool should_notify(const std::string& notify_type) const { return notify_types.count(notify_type); }
...
public:
    ...
    std::pair<int, std::string> set_config(MonClient* monc, const std::string& module_name, const std::string& key,
