...
std::string handle_pyerror(bool crash_dump, std::string module, std::string caller)
{
...
static PyMethodDef log_methods[] = {{"write", log_write, METH_VARARGS, "write stdout and stderr"},
...
std::pair<int, std::string> PyModuleConfig::set_config(MonClient* monc, const std::string& module_name,
                                                       const std::string& key, const std::optional<std::string>& val)
{
    ...
    if (set_cmd.r == 0) {
        ...
        if (val) {
            dout(0) << "`config set mgr " << global_key << " " << val << "` failed: " << cpp_strerror(set_cmd.r)
                    << dendl;
        }
        else {
...
PyObject* PyModule::init_ceph_module()
{
    ...
    static PyModuleDef ceph_module_def = {
        PyModuleDef_HEAD_INIT, "ceph_module", nullptr, -1, module_methods, nullptr, nullptr, nullptr, nullptr};
    PyObject* ceph_module = PyModule_Create(&ceph_module_def);
    ceph_assert(ceph_module != nullptr);
    std::map<const char*, PyTypeObject*> classes{{{"BaseMgrModule", &BaseMgrModuleType},
...
int PyModule::walk_dict_list(const std::string& attr_name, std::function<int(PyObject*)> fn)
{
...
{
    PyObject* pRegCmd =
        PyObject_CallMethod(cls, const_cast<char*>("_register_options"), const_cast<char*>("(s)"), module_name.c_str());
    if (pRegCmd != nullptr) {
        Py_DECREF(pRegCmd);
    }
    else {
...
int PyModule::load_notify_types()
{
    ...
    for (size_t i = 0; i < list_size; ++i) {
        ...
        if (!PyObject_TypeCheck(notify_type, &PyUnicode_Type)) {
            derr << "Module " << get_name() << " has non-string entry in NOTIFY_TYPES list" << dendl;
            return -EINVAL;
        }
...
int PyModule::load_commands()
{
    PyObject* pRegCmd = PyObject_CallMethod(
        pClass, const_cast<char*>("_register_commands"), const_cast<char*>("(s)"), module_name.c_str());
    if (pRegCmd != nullptr) {
        Py_DECREF(pRegCmd);
    }
    else {
    ...
    int r = walk_dict_list("COMMANDS", [this](PyObject* pCommand) -> int {
        ...
        if (PyObject* pPoll = PyDict_GetItemString(pCommand, "poll"); pPoll && PyObject_IsTrue(pPoll)) {
...
PyObject* PyModule::get_typed_option_value(const std::string& name, const std::string& value)
{
    ...
    while (PyDict_Next(locals, &pos, &key, &value)) {
        ...
        if (*py_class) {
            derr << __func__ << ": ignoring '" << module_name << "." << class_name << "'"
                 << ": only one '" << base_class << "' class is loaded from each plugin" << dendl;
            continue;
        }
        *py_class = value;
