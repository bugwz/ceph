...
struct MDSPerfMetricSubKeyDescriptor
{
    ...
    bool is_supported() const
    {
        ...
        case MDSPerfMetricSubKeyType::CLIENT_ID: return true;
...
template<> struct denc_traits<MDSPerfMetricKeyDescriptor>
{
    ...
    static void encode(const MDSPerfMetricKeyDescriptor& v, ceph::buffer::list::contiguous_appender& p)
    {
    ...
    static void decode(MDSPerfMetricKeyDescriptor& v, ceph::buffer::ptr::const_iterator& p)
    {
        unsigned num;
        denc_varint(num, p);
        v.clear();
...
struct MDSPerformanceCounterDescriptor
{
    ...
    bool is_supported() const
    {
        ...
        case MDSPerformanceCounterType::STDEV_METADATA_LATENCY_METRIC: return true;
        default: return false;
        }
    }

    MDSPerformanceCounterDescriptor() {}
    MDSPerformanceCounterDescriptor(MDSPerformanceCounterType type)
        : type(type)
    {}

    bool operator<(const MDSPerformanceCounterDescriptor& other) const { return type < other.type; }
...
template<> struct denc_traits<MDSPerformanceCounterDescriptors>
{
    static constexpr bool supported = true;
    static constexpr bool bounded = false;
    static constexpr bool featured = false;
    ...
    static void encode(const MDSPerformanceCounterDescriptors& v, ceph::buffer::list::contiguous_appender& p)
    {
        denc_varint(v.size(), p);
        for (auto& i : v) {
            denc(i, p);
    ...
    static void decode(MDSPerformanceCounterDescriptors& v, ceph::buffer::ptr::const_iterator& p)
    {
        unsigned num;
        denc_varint(num, p);
        v.clear();
...
struct MDSPerfMetricQuery
{
    ...
    template<typename L> bool get_key(L&& get_sub_key, MDSPerfMetricKey* key) const
    {
