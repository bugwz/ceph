...
std::string ServiceMap::Service::get_summary() const
{
    ...
    for (auto& d : daemons) {
        ++num;
        if (auto p = d.second.metadata.find("daemon_type"); p != d.second.metadata.end()) {
            type = p->second;
        }
        for (auto k : {std::make_pair("zone", "zone_id"), std::make_pair("host", "hostname")}) {
...
{
    return std::any_of(daemons.begin(), daemons.end(), [](auto& daemon) { return !daemon.second.task_status.empty(); });
}
...
std::string ServiceMap::Service::get_task_summary(const std::string_view task_prefix) const
{
    ...
    for (const auto& [service_id, daemon] : daemons) {
        for (const auto& [task_name, status] : daemon.task_status) {
            by_task[task_name].emplace(fmt::format("{}.{}", task_prefix, service_id), status);
        }
    }
    std::stringstream ss;
...
void ServiceMap::Service::count_metadata(const std::string& field, std::map<std::string, int>* out) const
{
    for (auto& p : daemons) {
