...
struct OSDPerfMetricSubKeyDescriptor
{
    ...
    bool is_supported() const
    {
        ...
        case OSDPerfMetricSubKeyType::SNAP_ID: return true;
    ...
    OSDPerfMetricSubKeyDescriptor(OSDPerfMetricSubKeyType type, const std::string regex)
        : type(type)
        , regex_str(regex)
    {}
...
template<> struct denc_traits<OSDPerfMetricKeyDescriptor>
{
    ...
    static void encode(const OSDPerfMetricKeyDescriptor& v, ceph::buffer::list::contiguous_appender& p)
    {
    ...
    static void decode(OSDPerfMetricKeyDescriptor& v, ceph::buffer::ptr::const_iterator& p)
    {
        unsigned num;
        denc_varint(num, p);
        v.clear();
...
struct PerformanceCounterDescriptor
{
    ...
    bool is_supported() const
    {
        ...
        case PerformanceCounterType::READ_LATENCY: return true;
        default: return false;
        }
    }
    ...
    PerformanceCounterDescriptor(PerformanceCounterType type)
        : type(type)
    {}

    bool operator<(const PerformanceCounterDescriptor& other) const { return type < other.type; }
...
template<> struct denc_traits<PerformanceCounterDescriptors>
{
    static constexpr bool supported = true;
    static constexpr bool bounded = false;
    static constexpr bool featured = false;
    ...
    static void encode(const PerformanceCounterDescriptors& v, ceph::buffer::list::contiguous_appender& p)
    {
        denc_varint(v.size(), p);
        for (auto& i : v) {
            denc(i, p);
    ...
    static void decode(PerformanceCounterDescriptors& v, ceph::buffer::ptr::const_iterator& p)
    {
        unsigned num;
        denc_varint(num, p);
        v.clear();
...
struct OSDPerfMetricLimit
{
    ...
    OSDPerfMetricLimit(const PerformanceCounterDescriptor& order_by, uint64_t max_count)
        : order_by(order_by)
        , max_count(max_count)
    {}
...
struct OSDPerfMetricQuery
{
    ...
    OSDPerfMetricQuery(const OSDPerfMetricKeyDescriptor& key_descriptor,
    ...
    template<typename L> bool get_key(L&& get_sub_key, OSDPerfMetricKey* key) const
    {
    ...
    void get_performance_counter_descriptors(PerformanceCounterDescriptors* descriptors) const
    {
        *descriptors = performance_counter_descriptors;
    }

    template<typename L> void update_counters(L&& update_counter, PerformanceCounters* counters) const
    {
