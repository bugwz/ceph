...
ClusterState::ClusterState(MonClient* monc_, Objecter* objecter_, const MgrMap& mgrmap)
    : monc(monc_)
    , objecter(objecter_)
    , mgr_map(mgrmap)
    , asok_hook(NULL)
...
void ClusterState::ingest_pgstats(ref_t<MPGStats> stats)
{
    ...
    bool is_in = with_osdmap([from](const OSDMap& osdmap) { return osdmap.is_in(from); });
    ...
    for (auto p : stats->pg_stat) {
        ...
        if (r == existing_pools.end()) {
            dout(15) << " got " << pgid << " reported at " << pg_stats.reported_epoch << ":" << pg_stats.reported_seq
                     << " state " << pg_state_string(pg_stats.state) << " but pool not in " << existing_pools << dendl;
            continue;
        }
        if (pgid.ps() >= r->second) {
            dout(15) << " got " << pgid << " reported at " << pg_stats.reported_epoch << ":" << pg_stats.reported_seq
                     << " state " << pg_state_string(pg_stats.state) << " but > pg_num " << r->second << dendl;
            continue;
        }
        // In case we already heard about more recent stats from this PG
        // from another OSD
        const auto q = pg_map.pg_stat.find(pgid);
        if (q != pg_map.pg_stat.end() && q->second.get_version_pair() > pg_stats.get_version_pair()) {
            dout(15) << " had " << pgid << " from " << q->second.reported_epoch << ":" << q->second.reported_seq
                     << dendl;
            continue;
        }
...
void ClusterState::notify_osdmap(const OSDMap& osd_map)
{
    ...
    PGMapUpdater::check_down_pgs(osd_map, pg_map, true, need_check_down_pg_osds, &pending_inc);
...
public:
    explicit ClusterSocketHook(ClusterState* o)
        : cluster_state(o)
    {}
    int call(std::string_view admin_command, const cmdmap_t& cmdmap, Formatter* f, std::ostream& errss,
...
bool ClusterState::asok_command(std::string_view admin_command, const cmdmap_t& cmdmap, Formatter* f, ostream& ss)
{
    std::lock_guard l(lock);
        ...
        for (auto i : pg_map.osd_stat) {
            for (auto j : i.second.hb_pingtime) {
                ...
                if (now.sec() - j.second.last_update > stale_time) {
                    dout(20) << __func__ << " time out heartbeat for osd " << i.first << " last_update "
                             << j.second.last_update << dendl;
                    continue;
                }
