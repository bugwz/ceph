...
void ActivePyModule::notify(const std::string& notify_type, const std::string& notify_id)
{
    ...
    auto pValue = PyObject_CallMethod(
        pClassInstance, const_cast<char*>("notify"), const_cast<char*>("(ss)"), notify_type.c_str(), notify_id.c_str());
...
void ActivePyModule::notify_clog(const LogEntry& log_entry)
{
    ...
    // Execute
    auto pValue = PyObject_CallMethod(
        pClassInstance, const_cast<char*>("notify"), const_cast<char*>("(sN)"), "clog", py_log_entry);
...
PyObject* ActivePyModule::dispatch_remote(const std::string& method, PyObject* args, PyObject* kwargs, std::string* err)
{
...
void ActivePyModule::config_notify()
{
    ...
    auto remoteResult = PyObject_CallMethod(pClassInstance, const_cast<char*>("_config_notify"), (char*)NULL);
    if (remoteResult != nullptr) {
        Py_DECREF(remoteResult);
    }
}

int ActivePyModule::handle_command(const ModuleCommand& module_command, const MgrSession& session,
                                   ...
                                   std::stringstream* ss)
{
    ...
    auto pResult = PyObject_CallMethod(pClassInstance,
                                       const_cast<char*>("_handle_command"),
                                       const_cast<char*>("s#O"),
                                       instr.c_str(),
                                       instr.length(),
                                       py_cmd);
...
bool ActivePyModule::is_authorized(const std::map<std::string, std::string>& arguments) const
{
    ...
    return m_session->caps.is_capable(nullptr,
                                      m_session->entity_name,
                                      "py",
                                      py_module->get_name(),
                                      "",
                                      arguments,
