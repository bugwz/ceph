...
void ActivePyModules::notify_all(const LogEntry& log_entry)
{
    ...
    for (auto& [name, module] : modules) {
        ...
        // workaround for https://bugs.llvm.org/show_bug.cgi?id=35984
        finisher.queue(new LambdaContext([module = module, log_entry](int r) { module->notify_clog(log_entry); }));
    }
}
...
void ActivePyModules::config_notify()
{
    ...
    for (auto& [name, module] : modules) {
        ...
        // workaround for https://bugs.llvm.org/show_bug.cgi?id=35984
        finisher.queue(new LambdaContext([module = module](int r) { module->config_notify(); }));
    }
}
