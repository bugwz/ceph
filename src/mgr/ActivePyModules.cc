...
ActivePyModules::ActivePyModules(PyModuleConfig& module_config_, std::map<std::string, std::string> store_data,
                                 bool mon_provides_kv_sub, DaemonStateIndex& ds, ClusterState& cs, MonClient& mc,
...
void ActivePyModules::dump_server(const std::string& hostname, const DaemonStateCollection& dmc, Formatter* f)
{
...
PyObject* ActivePyModules::list_servers_python()
{
    ...
    return daemon_state.with_daemons_by_server(
        [this, &no_gil](const std::map<std::string, DaemonStateCollection>& all) {
            no_gil.acquire_gil();
...
PyObject* ActivePyModules::get_metadata_python(const std::string& svc_type, const std::string& svc_id)
{
    ...
    auto l = without_gil([&] { return std::lock_guard(lock); });
    PyFormatter f;
...
PyObject* ActivePyModules::get_daemon_status_python(const std::string& svc_type, const std::string& svc_id)
{
    auto metadata = daemon_state.get(DaemonKey{svc_type, svc_id});
    if (metadata == nullptr) {
        derr << "Requested missing service " << svc_type << "." << svc_id << dendl;
...
PyObject* ActivePyModules::get_python(const std::string& what)
{
    ...
    if (what == "fs_map") {
        ...
        cluster_state.with_osdmap(
            [&](const OSDMap& osd_map) { osd_map.crush->encode(rdata, CEPH_FEATURES_SUPPORTED_DEFAULT); });
        ...
        without_gil_t no_gil;
        cluster_state.with_pgmap([&f, &no_gil](const PGMap& pg_map) {
        ...
        daemon_state.with_devices2(
            ...
            [&](const DeviceState& dev) { with_gil(no_gil, [&] { f.dump_object("device", dev); }); });
        with_gil(no_gil, [&] { f.close_section(); });
    }
    else if (what.size() > 7 && what.substr(0, 7) == "device ") {
        without_gil_t no_gil;
        string devid = what.substr(7);
        if (!daemon_state.with_device(devid, [&](const DeviceState& dev) {
        ...
        without_gil_t no_gil;
        cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osd_map, const PGMap& pg_map) {
            no_gil.acquire_gil();
    ...
    // self.get('health') 调用
        ...
        without_gil_t no_gil;
        cluster_state.with_mon_status([&](const ceph::bufferlist& mon_status_json) {
            no_gil.acquire_gil();
...
void ActivePyModules::start_one(PyModuleRef py_module)
{
    ...
    finisher.queue(new LambdaContext([this, active_module, name](int) {
        ...
        if (r != 0) {
            ...
            dout(4) << "Starting active module " << name << " finisher thread " << active_module->get_fin_thread_name()
                    << dendl;
...
void ActivePyModules::notify_all(const std::string& notify_type, const std::string& notify_id)
{
    std::lock_guard l(lock);
    ...
    for (auto& [name, module] : modules) {
        ...
        mod_finisher.queue(new LambdaContext(
            [module = module, notify_type, notify_id](int r) { module->notify(notify_type, notify_id); }));
    }
}
...
void ActivePyModules::notify_all(const LogEntry& log_entry)
{
    ...
    for (auto& [name, module] : modules) {
        ...
        Finisher& mod_finisher = py_module_registry.get_active_module_finisher(name);
        // workaround for https://bugs.llvm.org/show_bug.cgi?id=35984
        mod_finisher.queue(new LambdaContext([module = module, log_entry](int r) { module->notify_clog(log_entry); }));
    }
}

bool ActivePyModules::get_store(const std::string& module_name, const std::string& key, std::string* val) const
{
    without_gil_t no_gil;
    std::lock_guard l(lock);

    const std::string global_key = PyModule::mgr_store_prefix + module_name + "/" + key;
...
PyObject* ActivePyModules::dispatch_remote(const std::string& other_module, const std::string& method, PyObject* args,
...
bool ActivePyModules::get_config(const std::string& module_name, const std::string& key, std::string* val) const
{
...
PyObject* ActivePyModules::get_typed_config(const std::string& module_name, const std::string& key,
...
PyObject* ActivePyModules::get_store_prefix(const std::string& module_name, const std::string& prefix) const
{
    without_gil_t no_gil;
    std::lock_guard l(lock);
    std::lock_guard lock(module_config.lock);
    ...
    const std::string base_prefix = PyModule::mgr_store_prefix + module_name + "/";
    const std::string global_prefix = base_prefix + prefix;
    dout(4) << __func__ << " prefix: " << global_prefix << dendl;

    PyFormatter f;
    for (auto p = store_cache.lower_bound(global_prefix); p != store_cache.end() && p->first.find(global_prefix) == 0;
         ++p) {
...
void ActivePyModules::set_store(const std::string& module_name, const std::string& key,
...
std::pair<int, std::string> ActivePyModules::set_config(const std::string& module_name, const std::string& key,
                                                        const std::optional<std::string>& val)
{
...
void ActivePyModules::update_kv_data(const std::string prefix, bool incremental,
...
void ActivePyModules::_refresh_config_map()
{
    dout(10) << dendl;
    config_map.clear();
    for (auto p = store_cache.lower_bound("config/"); p != store_cache.end() && p->first.find("config/") == 0; ++p) {
        ...
        if (!opt) {
            config_map.stray_options.push_back(
                std::unique_ptr<Option>(new Option(name, Option::TYPE_STR, Option::LEVEL_UNKNOWN)));
        ...
        if (r < 0) {
            dout(10) << __func__ << " pre-validate failed on '" << name << "' = '" << value << "' for " << name
                     << dendl;
        }
...
PyObject* ActivePyModules::with_perf_counters(
    ...
    const std::string& svc_name, const std::string& svc_id, const std::string& path) const
{
    PyFormatter f;
...
PyObject* ActivePyModules::get_counter_python(const std::string& svc_name, const std::string& svc_id,
                                              const std::string& path)
{
    auto extract_counters = [](PerfCounterInstance& counter_instance, PerfCounterType& counter_type, PyFormatter& f) {
...
PyObject* ActivePyModules::get_latest_counter_python(const std::string& svc_name, const std::string& svc_id,
                                                     const std::string& path)
{
    auto extract_latest_counters =
        [](PerfCounterInstance& counter_instance, PerfCounterType& counter_type, PyFormatter& f) {
            if (counter_type.type & PERFCOUNTER_LONGRUNAVG) {
...
PyObject* ActivePyModules::get_perf_schema_python(const std::string& svc_type, const std::string& svc_id)
{
    without_gil_t no_gil;
    std::lock_guard l(lock);
...
{
    std::string version =
        std::to_string(ROCKSDB_MAJOR) + "." + std::to_string(ROCKSDB_MINOR) + "." + std::to_string(ROCKSDB_PATCH);
 ...
 */
PyObject* construct_with_capsule(const std::string& module_name, const std::string& clsname, void* wrapped)
{
    // Look up the OSDMap type which we will construct
    PyObject* module = PyImport_ImportModule(module_name.c_str());
    if (!module) {
        derr << "Failed to import python module:" << dendl;
        derr << handle_pyerror(true, module_name, "construct_with_capsule "s + module_name + " " + clsname) << dendl;
    }
    ceph_assert(module);

    PyObject* wrapper_type = PyObject_GetAttrString(module, (const char*)clsname.c_str());
    if (!wrapper_type) {
...
PyObject* ActivePyModules::get_osdmap()
{
    auto newmap = without_gil([&] {
        ...
        cluster_state.with_osdmap([&](const OSDMap& o) { newmap->deepish_copy_from(o); });
        return newmap;
    });
    return construct_with_capsule("mgr_module", "OSDMap", (void*)newmap);
}

PyObject* ActivePyModules::get_foreign_config(const std::string& who, const std::string& name)
{
    ...
    if (!have_local_config_map) {
        ...
        Command cmd;
        {
            std::lock_guard l(lock);
            cmd.run(&monc,
                    "{\"prefix\": \"config get\","s + "\"who\": \""s + who + "\","s + "\"key\": \""s + name + "\"}");
        }
        cmd.wait();
        dout(10) << "ceph_foreign_option_get (mon command) " << who << " " << name << " = " << cmd.outbl.to_str()
                 << dendl;
        no_gil.acquire_gil();
    ...
    cluster_state.with_osdmap([&](const OSDMap& osdmap) {
        ...
        config = config_map.generate_entity_map(entity, crush_location, osdmap.crush.get(), device_class, &src);
    });
...
void ActivePyModules::set_health_checks(const std::string& module_name, health_check_map_t&& checks)
{
    bool changed = false;
    ...
    // DaemonServer::lock.
    if (changed) server.schedule_tick(0);
}

int ActivePyModules::handle_command(const ModuleCommand& module_command, const MgrSession& session,
                                    const cmdmap_t& cmdmap, const bufferlist& inbuf, std::stringstream* ds,
                                    std::stringstream* ss)
{
    lock.lock();
    ...
    lock.unlock();
    return mod_iter->second->handle_command(module_command, session, cmdmap, inbuf, ds, ss);
}
...
void ActivePyModules::update_progress_event(const std::string& evid, const std::string& desc, float progress,
                                            bool add_to_ceph_s)
{
    std::lock_guard l(lock);
...
void ActivePyModules::config_notify()
{
    ...
    for (auto& [name, module] : modules) {
        ...
        Finisher& mod_finisher = py_module_registry.get_active_module_finisher(name);
        // workaround for https://bugs.llvm.org/show_bug.cgi?id=35984
        mod_finisher.queue(new LambdaContext([module = module](int r) { module->config_notify(); }));
    }
}

void ActivePyModules::set_uri(const std::string& module_name, const std::string& uri)
{
    std::lock_guard l(lock);
...
void ActivePyModules::set_device_wear_level(const std::string& devid, float wear_level)
{
    // update mgr state
    map<string, string> meta;
    daemon_state.with_device(devid, [wear_level, &meta](DeviceState& dev) {
...
MetricQueryID ActivePyModules::add_osd_perf_query(const OSDPerfMetricQuery& query,
...
MetricQueryID ActivePyModules::add_mds_perf_query(const MDSPerfMetricQuery& query,
...
void ActivePyModules::cluster_log(const std::string& channel, clog_type prio, const std::string& message)
{
    std::lock_guard l(lock);
...
{
    without_gil_t no_gil;
    return daemon_state.with_daemons_by_server([&no_gil](const std::map<std::string, DaemonStateCollection>& all) {
        no_gil.acquire_gil();
        PyFormatter f;
        for (const auto& [hostname, daemon_state] : all) {
