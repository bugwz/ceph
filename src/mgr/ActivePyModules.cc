...
void ActivePyModules::set_config(const std::string& module_name, const std::string& key,
                                 const boost::optional<std::string>& val)
{
...
PyObject* ActivePyModules::get_foreign_config(const std::string& who, const std::string& name)
{
    ...
    if (p != config.end()) {
        ...
        if (!entity.is_client() && !boost::get<boost::blank>(&opt->daemon_value)) {
...
void ActivePyModules::cluster_log(const std::string& channel, clog_type prio, const std::string& message)
{
    ...
    if (parse_log_client_options(g_ceph_context,
                                 ...
                                 fsid,
                                 host) == 0)
        cl->update_config(log_to_monitors,
                          log_to_syslog,
                          log_channel,
                          log_prio,
                          log_to_graylog,
