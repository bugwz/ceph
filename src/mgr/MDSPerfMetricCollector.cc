...
MDSPerfMetricCollector::MDSPerfMetricCollector(MetricListener& listener)
    : MetricCollector<MDSPerfMetricQuery, MDSPerfMetricLimit, MDSPerfMetricKey, MDSPerfMetrics>(listener)
{}
...
void MDSPerfMetricCollector::process_reports(const MetricPayload& payload)
{
    ...
    process_reports_generic(metric_report.reports, [](PerformanceCounter* counter, const PerformanceCounter& update) {
