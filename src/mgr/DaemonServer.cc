...
DaemonServer::DaemonServer(MonClient* monc_, Finisher& finisher_, DaemonStateIndex& daemon_state_,
                           ClusterState& cluster_state_, PyModuleRegistry& py_modules_, LogChannelRef clog_,
                           LogChannelRef audit_clog_)
    : Dispatcher(g_ceph_context)
    , client_byte_throttler(
          new Throttle(g_ceph_context, "mgr_client_bytes", g_conf().get_val<Option::size_t>("mgr_client_bytes")))
    , client_msg_throttler(
          new Throttle(g_ceph_context, "mgr_client_messages", g_conf().get_val<uint64_t>("mgr_client_messages")))
    , osd_byte_throttler(
          new Throttle(g_ceph_context, "mgr_osd_bytes", g_conf().get_val<Option::size_t>("mgr_osd_bytes")))
    , osd_msg_throttler(
          new Throttle(g_ceph_context, "mgr_osd_messsages", g_conf().get_val<uint64_t>("mgr_osd_messages")))
    , mds_byte_throttler(
          new Throttle(g_ceph_context, "mgr_mds_bytes", g_conf().get_val<Option::size_t>("mgr_mds_bytes")))
    , mds_msg_throttler(
          new Throttle(g_ceph_context, "mgr_mds_messsages", g_conf().get_val<uint64_t>("mgr_mds_messages")))
    , mon_byte_throttler(
          new Throttle(g_ceph_context, "mgr_mon_bytes", g_conf().get_val<Option::size_t>("mgr_mon_bytes")))
    , mon_msg_throttler(
          new Throttle(g_ceph_context, "mgr_mon_messsages", g_conf().get_val<uint64_t>("mgr_mon_messages")))
    , msgr(nullptr)
    , monc(monc_)
    , finisher(finisher_)
...
int DaemonServer::init(uint64_t gid, entity_addrvec_t client_addrs)
{
    ...
    std::string public_msgr_type =
        g_conf()->ms_public_type.empty() ? g_conf().get_val<std::string>("ms_type") : g_conf()->ms_public_type;
    msgr =
        Messenger::create(g_ceph_context, public_msgr_type, entity_name_t::MGR(gid), "mgr", Messenger::get_pid_nonce());
    msgr->set_default_policy(Messenger::Policy::stateless_server(0));
    // throttle policy
    msgr->set_policy(entity_name_t::TYPE_OSD, Messenger::Policy::stateless_server(CEPH_FEATURE_SERVER_LUMINOUS));
    msgr->set_policy(entity_name_t::TYPE_MON, Messenger::Policy::lossy_client(CEPH_FEATURE_UID | CEPH_FEATURE_PGID64));
    msgr->set_policy(entity_name_t::TYPE_MDS, Messenger::Policy::stateless_server(CEPH_FEATURE_SERVER_LUMINOUS));

    msgr->set_auth_client(monc);

    // throttle clients
    msgr->set_policy_throttlers(entity_name_t::TYPE_CLIENT, client_byte_throttler.get(), client_msg_throttler.get());

    // servers
    msgr->set_policy_throttlers(entity_name_t::TYPE_OSD, osd_byte_throttler.get(), osd_msg_throttler.get());
    msgr->set_policy_throttlers(entity_name_t::TYPE_MDS, mds_byte_throttler.get(), mds_msg_throttler.get());
    msgr->set_policy_throttlers(entity_name_t::TYPE_MON, mon_byte_throttler.get(), mon_msg_throttler.get());
    ...
    schedule_tick_locked(g_conf().get_val<std::chrono::seconds>("mgr_tick_period").count());
...
int DaemonServer::ms_handle_fast_authentication(Connection* con)
{
    ...
    dout(10) << __func__ << " new session " << s << " con " << con << " entity " << con->peer_name << " addr "
...
bool DaemonServer::ms_dispatch2(const ref_t<Message>& m)
{
    ...
    case MSG_PGSTATS:
        ...
        return true;
    case MSG_MGR_REPORT: return handle_report(ref_cast<MMgrReport>(m));
    case MSG_MGR_OPEN: return handle_open(ref_cast<MMgrOpen>(m));
    case MSG_MGR_UPDATE: return handle_update(ref_cast<MMgrUpdate>(m));
    case MSG_MGR_CLOSE: return handle_close(ref_cast<MMgrClose>(m));
    case MSG_COMMAND: return handle_command(ref_cast<MCommand>(m));
    case MSG_MGR_COMMAND: return handle_command(ref_cast<MMgrCommand>(m));
    default: dout(1) << "Unhandled message type " << m->get_type() << dendl; return false;
    };
}
...
void DaemonServer::maybe_ready(int32_t osd_id)
{
    if (pgmap_ready.load()) {
        ...
        if (reported_osds.find(osd_id) == reported_osds.end()) {
            ...
            cluster_state.with_osdmap([&](const OSDMap& osdmap) { osdmap.get_up_osds(up_osds); });

            std::set<int32_t> unreported_osds;
            std::set_difference(up_osds.begin(),
                                up_osds.end(),
                                reported_osds.begin(),
                                reported_osds.end(),
            ...
            if (unreported_osds.size() == 0) {
                ...
                dout(4) << "still waiting for " << unreported_osds.size()
                        << " osds"
                           " to report in before PGMap is ready"
                        << dendl;
            }
        }
    }
...
void DaemonServer::schedule_tick_locked(double delay_sec)
{
    ...
    tick_event = timer.add_event_after(delay_sec, new LambdaContext([this](int r) { tick(); }));
}
...
static DaemonKey key_from_service(const std::string& service_name, int peer_type, const std::string& daemon_name)
{
...
void DaemonServer::fetch_missing_metadata(const DaemonKey& key, const entity_addr_t& addr)
{
    if (!daemon_state.is_updating(key) && (key.type == "osd" || key.type == "mds" || key.type == "mon")) {
        std::ostringstream oss;
        auto c = new MetadataUpdate(daemon_state, key);
        if (key.type == "osd") {
            oss << "{\"prefix\": \"osd metadata\", \"id\": " << key.name << "}";
        }
        else if (key.type == "mds") {
            c->set_default("addr", stringify(addr));
            oss << "{\"prefix\": \"mds metadata\", \"who\": \"" << key.name << "\"}";
        }
        else if (key.type == "mon") {
            oss << "{\"prefix\": \"mon metadata\", \"id\": \"" << key.name << "\"}";
        }
        else {
            ceph_abort();
        }
...
bool DaemonServer::handle_open(const ref_t<MMgrOpen>& m)
{
    ...
    DaemonKey key = key_from_service(m->service_name, m->get_connection()->get_peer_type(), m->daemon_name);
    ...
    if (daemon) {
        ...
        if (m->service_daemon) {
            ...
            auto [d, added] = pending_service_map.get_daemon(m->service_name, m->daemon_name);
    ...
    if (con->get_peer_type() != entity_name_t::TYPE_CLIENT && m->service_name.empty()) {
...
bool DaemonServer::handle_update(const ref_t<MMgrUpdate>& m)
{
    ...
    if (m->get_connection()->get_peer_type() == entity_name_t::TYPE_CLIENT && m->service_name.empty()) {
        // Clients should not be sending us update request
        dout(10) << "rejecting update request from non-daemon client " << m->daemon_name << dendl;
        clog->warn() << "rejecting report from non-daemon client " << m->daemon_name << " at "
                     << m->get_connection()->get_peer_addrs();
...

    {
        ...
        if (daemon_state.exists(key)) {
            ...
            daemon = daemon_state.get(key);
            if (m->need_metadata_update && !m->daemon_metadata.empty()) {
                daemon_state.update_metadata(daemon, m->daemon_metadata);
            }
        }
    }
...
void DaemonServer::update_task_status(DaemonKey key, const std::map<std::string, std::string>& task_status)
{
    dout(10) << "got task status from " << key << dendl;

    [[maybe_unused]] auto [daemon, added] = pending_service_map.get_daemon(key.type, key.name);
...
bool DaemonServer::handle_report(const ref_t<MMgrReport>& m)
{
...

    {
        ...
        if (daemon = daemon_state.get(key); daemon != nullptr) {
            ...
            dout(10) << "unregistering osd." << session->osd_id << "  session " << session << " con "
...
void DaemonServer::_generate_command_map(cmdmap_t& cmdmap, map<string, string>& param_str_map)
{
...
const MonCommand* DaemonServer::_get_mgrcommand(const string& cmd_prefix, const std::vector<MonCommand>& cmds)
{
...
bool DaemonServer::_allowed_command(MgrSession* s, const string& service, const string& module, const string& prefix,
                                    const cmdmap_t& cmdmap, const map<string, string>& param_str_map,
                                    const MonCommand* this_cmd)
{
    ...
    bool capable = s->caps.is_capable(g_ceph_context,
                                      s->entity_name,
                                      service,
                                      module,
                                      prefix,
...
public:
    ...
    void reply(int r, const std::stringstream& ss) { reply(r, ss.str()); }

    void reply(int r, const std::string& rs)
    {
        // Let the connection drop as soon as we've sent our response
        ConnectionRef con = m_tell ? m_tell->get_connection() : m_mgr->get_connection();
        if (con) {
            con->mark_disposable();
        }
...
void DaemonServer::log_access_denied(std::shared_ptr<CommandContext>& cmdctx, MgrSession* session,
                                     std::stringstream& ss)
{
...
void DaemonServer::_check_offlines_pgs(const set<int>& osds, const OSDMap& osdmap, const PGMap& pgmap,
                                       offline_pg_report* report)
{
    ...
    *report = offline_pg_report();
    ...
    dout(20) << osds << " -> " << report->ok.size() << " ok, " << report->not_ok.size() << " not ok, "
             << report->unknown.size() << " unknown" << dendl;
}

void DaemonServer::_maximize_ok_to_stop_set(const set<int>& orig_osds, unsigned max, const OSDMap& osdmap,
                                            const PGMap& pgmap, offline_pg_report* out_report)
{
...
bool DaemonServer::_handle_command(std::shared_ptr<CommandContext>& cmdctx)
{
    MessageRef m;
    ...
    if (admin_socket_cmd) {
        ...
        is_allowed =
            _allowed_command(session, "py", py_command.module_name, prefix, cmdctx->cmdmap, param_str_map, &pyc);
    }
    else {
        // validate user's permissions for requested command
        is_allowed = _allowed_command(session, mgr_cmd->module, "", prefix, cmdctx->cmdmap, param_str_map, mgr_cmd);
    }
    ...
    audit_clog->debug() << "from='" << session->inst << "' "
                        << "entity='" << session->entity_name << "' "
    ...
    if (prefix == "service dump") {
        ...
        cluster_state.with_servicemap(
            [&](const ServiceMap& service_map) { f->dump_object("service_map", service_map); });
        f->flush(cmdctx->odata);
        cmdctx->reply(0, ss);
        return true;
    }
    ...
    if (prefix == "pg scrub" || prefix == "pg repair" || prefix == "pg deep-scrub") {
        ...
        cluster_state.with_osdmap([&](const OSDMap& osdmap) { pg_exists = osdmap.pg_exists(pgid); });
        if (!pg_exists) {
        ...
        if (p == osd_cons.end()) {
            ss << "pg " << pgid << " primary osd." << acting_primary << " is not currently connected";
            cmdctx->reply(-EAGAIN, ss);
            return true;
        }
        for (auto& con : p->second) {
            if (HAVE_FEATURE(con->get_features(), SERVER_MIMIC)) {
                ...
                con->send_message(
                    new MOSDScrub2(monc->get_fsid(), epoch, pgs, scrubop == "repair", scrubop == "deep-scrub"));
            }
            else {
                vector<pg_t> pgs = {pgid};
                con->send_message(new MOSDScrub(monc->get_fsid(), pgs, scrubop == "repair", scrubop == "deep-scrub"));
            }
        }
        ss << "instructing pg " << spgid << " on osd." << acting_primary << " to " << scrubop;
        cmdctx->reply(0, ss);
        return true;
    }
        ...
        for (auto osd : osds) {
            ...
            if (p == osd_cons.end()) {
                ...
                for (auto& con : p->second) {
                    if (HAVE_FEATURE(con->get_features(), SERVER_MIMIC)) {
                        con->send_message(new MOSDScrub2(
                            monc->get_fsid(), epoch, spgs, pvec.back() == "repair", pvec.back() == "deep-scrub"));
                    }
                    else {
                        con->send_message(
                            new MOSDScrub(monc->get_fsid(), pvec.back() == "repair", pvec.back() == "deep-scrub"));
                    }
                }
            }
        }
        if (failed_osds.size() == osds.size()) {
            ss << "failed to instruct osd(s) " << osds << " to " << pvec.back() << " (not connected)";
            r = -EAGAIN;
        }
        else {
    ...
    else if (prefix == "osd pool scrub" || prefix == "osd pool deep-scrub" || prefix == "osd pool repair") {
        ...
        for (auto& it : spgs_by_primary) {
            ...
            for (auto& con : p->second) {
                if (HAVE_FEATURE(con->get_features(), SERVER_MIMIC)) {
                    ...
                    con->send_message(new MOSDScrub(
                        monc->get_fsid(), q->second, prefix == "osd pool repair", prefix == "osd pool deep-scrub"));
                }
            }
        }
        cmdctx->reply(0, "");
    ...
    else if (prefix == "osd reweight-by-pg" || prefix == "osd reweight-by-utilization" ||
             prefix == "osd test-reweight-by-pg" || prefix == "osd test-reweight-by-utilization") {
        bool by_pg = prefix == "osd reweight-by-pg" || prefix == "osd test-reweight-by-pg";
        bool dry_run = prefix == "osd test-reweight-by-pg" || prefix == "osd test-reweight-by-utilization";
        int64_t oload;
        ...
        r = cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osdmap, const PGMap& pgmap) {
            return reweight::by_utilization(osdmap,
                                            pgmap,
                                            oload,
                                            max_change,
                                            max_osds,
        ...
        if (r < 0) {
            ...
            for (const auto& osd_weight : new_weights) {
                json_spirit::Config::add(
                    json_object, std::to_string(osd_weight.first), std::to_string(osd_weight.second));
            }
            ...
            monc->start_mon_command({cmd}, {}, &on_finish->from_mon, &on_finish->outs, on_finish);
            return true;
        }
    }
        ...
        r = cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osdmap, const PGMap& pgmap) {
            ...
            if (!filter.empty() && osdmap.lookup_pg_pool_name(filter) < 0 && !osdmap.crush->class_exists(filter) &&
                !osdmap.crush->name_exists(filter)) {
                rs << "'" << filter << "' not a pool, crush node or device class name";
                return -EINVAL;
            }
            print_osd_utilization(osdmap, pgmap, ss, f.get(), method == "tree", filter);
    ...
    else if (prefix == "osd safe-to-destroy" || prefix == "osd destroy" || prefix == "osd purge") {
        set<int> osds;
        int r = 0;
        if (prefix == "osd safe-to-destroy") {
            vector<string> ids;
            cmd_getval(cmdctx->cmdmap, "ids", ids);
            cluster_state.with_osdmap([&](const OSDMap& osdmap) { r = osdmap.parse_osd_id_list(ids, &osds, &ss); });
            if (!r && osds.empty()) {
        ...
        if (!r && (!active_osds.empty() || !missing_stats.empty() || !stored_pgs.empty())) {
            if (!safe_to_destroy.empty()) {
                ss << "OSD(s) " << safe_to_destroy << " are safe to destroy without reducing data durability. ";
            }
            if (!active_osds.empty()) {
                ss << "OSD(s) " << active_osds << " have " << affected_pgs << " pgs currently mapped to them. ";
            }
        ...
        if (prefix == "osd safe-to-destroy") {
            ...
            if (f) {
                ...
                for (auto i : safe_to_destroy) f->dump_int("osd", i);
                f->close_section();
                f->open_array_section("active");
                for (auto i : active_osds) f->dump_int("osd", i);
                f->close_section();
                f->open_array_section("missing_stats");
                for (auto i : missing_stats) f->dump_int("osd", i);
                f->close_section();
                f->open_array_section("stored_pgs");
                for (auto i : stored_pgs) f->dump_int("osd", i);
                f->close_section();
        ...
        cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osdmap, const PGMap& pg_map) {
            _maximize_ok_to_stop_set(osds, max, osdmap, pg_map, &out_report);
        });
        if (!f) {
        ...
        if (!out_report.ok_to_stop()) {
            ss << "unsafe to stop osd(s) at this time (" << out_report.not_ok.size()
    ...
    else if (prefix == "pg force-recovery" || prefix == "pg force-backfill" || prefix == "pg cancel-force-recovery" ||
             prefix == "pg cancel-force-backfill" || prefix == "osd pool force-recovery" ||
             prefix == "osd pool force-backfill" || prefix == "osd pool cancel-force-recovery" ||
        ...
        if (granularity == "pg") {
            ...
            cluster_state.with_osdmap([&](const OSDMap& osdmap) {
                for (auto& pool_name : pool_names) {
                    ...
                    auto pool_pg_num = osdmap.get_pg_num(pool_id);
                    for (int i = 0; i < pool_pg_num; i++) candidates.insert({(unsigned int)i, (uint64_t)pool_id});
                }
            });
            if (r < 0) {
                cmdctx->reply(r, ss);
        ...
        cluster_state.with_pgmap([&](const PGMap& pg_map) {
            for (auto& i : candidates) {
                ...
                switch (actual_op) {
                case OFR_RECOVERY:
                    if ((state & (PG_STATE_DEGRADED | PG_STATE_RECOVERY_WAIT | PG_STATE_RECOVERING)) == 0) {
                    ...
                    break;
                case OFR_BACKFILL:
                    if ((state & (PG_STATE_DEGRADED | PG_STATE_BACKFILL_WAIT | PG_STATE_BACKFILLING)) == 0) {
                ...
                case OFR_RECOVERY | OFR_CANCEL:
                    ...
                    break;
                default: ceph_abort_msg("actual_op value is not supported");
                }
                pgs.push_back(i);
            }   // for
        });
        ...
        cluster_state.with_osdmap([&](const OSDMap& osdmap) {
            ...
            for (auto& i : osdpgs) {
                if (osdmap.is_up(i.first)) {
                    ...
                    for (auto& con : p->second) {
                        con->send_message(new MOSDForceRecovery(monc->get_fsid(), i.second, actual_op));
                    }
    ...
    else if (prefix == "config show" || prefix == "config show-with-defaults") {
        string who;
        ...
        if (cmd_getval(cmdctx->cmdmap, "key", name)) {
            ...
            if (p != daemon->config.end() && !p->second.empty()) {
            ...
            if (prefix == "config show") {
                ...
                for (auto& i : daemon->config) {
                    ...
                    if (f) {
                        ...
                        f->dump_string("source", ceph_conf_level_name(i.second.rbegin()->first));
                        if (i.second.size() > 1) {
                        ...
                        if (i.second.size() > 1) {
                            ...
                            for (--j; j != i.second.rbegin(); --j) {
                                if (j->second == i.second.rbegin()->second) {
                                    ov.push_front(string("(") + ceph_conf_level_name(j->first) + string("[") +
                                                  j->second + string("]") + string(")"));
                                }
                                else {
                                    ov.push_front(ceph_conf_level_name(j->first) + string("[") + j->second +
                                                  string("]"));
                                }
                            }
                            tbl << ov;
                ...
                for (auto& i : defaults) {
                    ...
                    if (j != daemon->config.end() && !j->second.empty()) {
                        ...
                        if (f) {
                            ...
                            f->dump_string("source", ceph_conf_level_name(j->second.rbegin()->first));
                            if (j->second.size() > 1) {
                                f->open_array_section("overrides");
                            ...
                            if (j->second.size() > 1) {
                                ...
                                for (--k; k != j->second.rbegin(); --k) {
                                    if (k->second == j->second.rbegin()->second) {
                                        ov.push_front(string("(") + ceph_conf_level_name(k->first) + string("[") +
                                                      k->second + string("]") + string(")"));
                                    }
                                    else {
                                        ov.push_front(ceph_conf_level_name(k->first) + string("[") + k->second +
                                                      string("]"));
                                    }
                                }
                                tbl << ov;
        ...
        if (f) {
            ...
            daemon_state.with_devices([&f](const DeviceState& dev) { f->dump_object("device", dev); });
            f->close_section();
            f->flush(cmdctx->odata);
        }
        else {
            ...
            daemon_state.with_devices([&tbl, now](const DeviceState& dev) {
                ...
                if (dev.wear_level >= 0) {
                    snprintf(wear_level_str, sizeof(wear_level_str) - 1, "%d%%", (int)(100.1 * dev.wear_level));
                }
                tbl << dev.devid << h << d << wear_level_str << dev.get_life_expectancy_str(now) << TextTable::endrow;
            });
            cmdctx->odata.append(stringify(tbl));
        }
        cmdctx->reply(0, ss);
            ...
            if (dm) {
                if (f) {
                    ...
                    for (auto& i : dm->devices) {
                        daemon_state.with_device(i.first,
                                                 [&f](const DeviceState& dev) { f->dump_object("device", dev); });
                    }
                    f->close_section();
                    f->flush(cmdctx->odata);
                }
                    ...
                    auto now = ceph_clock_now();
                    for (auto& i : dm->devices) {
                        daemon_state.with_device(i.first, [&tbl, now](const DeviceState& dev) {
                            string h;
                            for (auto& i : dev.attachments) {
                                if (h.size()) {
                                    h += " ";
                            ...
                            tbl << dev.devid << h << dev.get_life_expectancy_str(now) << TextTable::endrow;
                        });
                    }
                    cmdctx->odata.append(stringify(tbl));
                }
        ...
        if (f) {
            ...
            for (auto& devid : devids) {
                daemon_state.with_device(devid, [&f](const DeviceState& dev) { f->dump_object("device", dev); });
            }
            f->close_section();
            f->flush(cmdctx->odata);
        }
            ...
            auto now = ceph_clock_now();
            for (auto& devid : devids) {
                daemon_state.with_device(devid, [&tbl, &host, now](const DeviceState& dev) {
                    string n;
                    ...
                    tbl << dev.devid << n << d << dev.get_life_expectancy_str(now) << TextTable::endrow;
                });
            }
            cmdctx->odata.append(stringify(tbl));
        }
        ...
        if (!from.parse(from_str)) {
            ...
            daemon_state.with_device_create(devid, [from, to, &meta](DeviceState& dev) {
        ...
        r = cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osdmap, const PGMap& pg_map) {
            return process_pg_map_command(prefix, cmdctx->cmdmap, pg_map, osdmap, f.get(), &ss, &cmdctx->odata);
        });
    ...
    if (!py_modules.is_module_active(mod_name)) {
        ss << "Module '" << mod_name
           << "' is not enabled/loaded (required by "
              "command '"
           << prefix << "'): use `ceph mgr module enable " << mod_name << "` to enable it";
        dout(4) << ss.str() << dendl;
    ...
    mod_finisher.queue(new LambdaContext([this, cmdctx, session, py_command, prefix](int r_) mutable {
        std::stringstream ss;
        ...
        if (!module->is_enabled()) {
            ss << "Module '" << py_handler_name
               << "' is not enabled (required by "
                  "command '"
               << prefix << "'): use `ceph mgr module enable " << py_handler_name << "` to enable it";
            dout(4) << ss.str() << dendl;
            cmdctx->reply(-EOPNOTSUPP, ss);
            return;
        }
        ...
        int r = py_modules.handle_command(py_command, *session, cmdctx->cmdmap, inbl, &ds, &ss);
        if (r == -EACCES) {
            log_access_denied(cmdctx, session, ss);
        }
    ...
    while (p != pending_service_map.services.end()) {
        ...
        while (q != p->second.daemons.end()) {
            ...
            if (daemon->last_service_beacon < cutoff) {
                dout(10) << "pruning stale " << p->first << "." << q->first << " last_beacon "
                         << daemon->last_service_beacon << dendl;
                q = p->second.daemons.erase(q);
                pending_service_map_dirty = pending_service_map.epoch;
            }
            else {
...
void DaemonServer::send_report()
{
    ...
    cluster_state.with_mutable_pgmap([&](PGMap& pg_map) {
        ...
        cluster_state.with_osdmap([&](const OSDMap& osdmap) {
            ...
            pg_map.get_health_checks(g_ceph_context, osdmap, &m->health_checks);
        ...
        for (const auto& [key, state] : daemons) {
            ...
            for (const auto& metric : state->daemon_health_metrics) {
                ...
                if (acc == accumulated.end()) {
                    ...
                    if (!collector) {
                        derr << __func__ << " " << key << " sent me an unknown health metric: " << std::hex
                             << static_cast<uint8_t>(metric.get_type()) << std::dec << dendl;
                        continue;
                    }
                    tie(acc, std::ignore) = accumulated.emplace(metric.get_type(), std::move(collector));
                }
...
void DaemonServer::adjust_pgs()
{
    ...
    cluster_state.with_osdmap_and_pgmap([&](const OSDMap& osdmap, const PGMap& pg_map) {
        ...
        for (auto& i : pg_map.num_pg_by_state) {
            if ((i.first & (PG_STATE_CREATING)) || i.first == 0) {
        ...
        dout(10) << "creating_or_unknown " << creating_or_unknown << " max_creating " << max << " left " << left
                 << dendl;
        ...
        pg_map.get_recovery_stats(&misplaced_ratio, &degraded_ratio, &inactive_pgs_ratio, &unknown_pgs_ratio);
        dout(20) << "misplaced_ratio " << misplaced_ratio << " degraded_ratio " << degraded_ratio
                 << " inactive_pgs_ratio " << inactive_pgs_ratio << " unknown_pgs_ratio " << unknown_pgs_ratio
                 << "; target_max_misplaced_ratio " << max_misplaced << dendl;
        ...
        for (auto& i : osdmap.get_pools()) {
            ...
            if (p.get_pg_num_target() != p.get_pg_num()) {
                dout(20) << "pool " << i.first << " pg_num " << p.get_pg_num() << " target " << p.get_pg_num_target()
                         << dendl;
                if (p.has_flag(pg_pool_t::FLAG_CREATING)) {
                    dout(10) << "pool " << i.first << " pg_num_target " << p.get_pg_num_target() << " pg_num "
                             << p.get_pg_num() << " - still creating initial pgs" << dendl;
                }
                    ...
                    if (p.get_pg_num() != p.get_pg_num_pending()) {
                        dout(10) << "pool " << i.first << " pg_num_target " << p.get_pg_num_target() << " pg_num "
                                 << p.get_pg_num() << " - decrease and pg_num_pending != pg_num, waiting" << dendl;
                        ok = false;
                    }
                    else if (p.get_pg_num() == p.get_pgp_num()) {
                        dout(10) << "pool " << i.first << " pg_num_target " << p.get_pg_num_target() << " pg_num "
                                 << p.get_pg_num() << " - decrease blocked by pgp_num " << p.get_pgp_num() << dendl;
                        ok = false;
                    }
                    ...
                    for (auto& merge_participant : {merge_source, merge_target}) {
                        ...
                        if (osdmap.have_pg_upmaps(merge_participant)) {
                            dout(10) << "pool " << i.first << " pg_num_target " << p.get_pg_num_target() << " pg_num "
                                     << p.get_pg_num() << (is_merge_source ? " - merge source " : " - merge target ")
                                     << merge_participant << " has upmap" << dendl;
                        ...
                        if (q == pg_map.pg_stat.end()) {
                            dout(10) << "pool " << i.first << " pg_num_target " << p.get_pg_num_target() << " pg_num "
                                     << p.get_pg_num() << " - no state for " << merge_participant
                                     << (is_merge_source ? " (merge source)" : " (merge target)") << dendl;
                            ok = false;
                        }
                            ...
                            dout(10) << "pool " << i.first << " pg_num_target " << p.get_pg_num_target() << " pg_num "
                                     << p.get_pg_num() << (is_merge_source ? " - merge source " : " - merge target ")
                                     << merge_participant << " not clean (" << pg_state_string(q->second.state) << ")"
                                     << dendl;
                            ok = false;
                        }
                        if (is_merge_source) {
                            ...
                            dout(10) << "pool " << i.first << " pg_num_target " << p.get_pg_num_target() << " pg_num "
                                     << p.get_pg_num() << (is_merge_source ? " - merge source " : " - merge target ")
                                     << merge_participant << " acting does not match (source " << source_acting
                                     << " != target " << q->second.acting << ")" << dendl;
                            ok = false;
                        }
                    }
                    ...
                    if (ok) {
                        ...
                        dout(10) << "pool " << i.first << " pg_num_target " << p.get_pg_num_target() << " pg_num "
                                 << p.get_pg_num() << " -> " << target << " (merging " << merge_source << " and "
                                 << merge_target << ")" << dendl;
                    ...
                    if (q != pg_map.num_pg_by_pool_state.end()) {
                        for (auto& j : q->second) {
                            if ((j.first & (PG_STATE_ACTIVE | PG_STATE_PEERED)) == 0) {
                                dout(20) << "pool " << i.first << " has " << j.second << " pgs in "
                                         << pg_state_string(j.first) << dendl;
                                active = false;
                                break;
                            }
                        }
                    ...
                    if (!active) {
                        dout(10) << "pool " << i.first << " pg_num_target " << p.get_pg_num_target() << " pg_num "
                                 << p.get_pg_num() << " - not all pgs active" << dendl;
                    }
                    else if (pg_gap >= max_jump) {
                        dout(10) << "pool " << i.first << " pg_num " << p.get_pg_num() << " - pgp_num "
                                 << p.get_pgp_num() << " gap >= max_pg_num_change " << max_jump
                                 << " - must scale pgp_num first" << dendl;
                    }
                    else {
                        unsigned add =
                            std::min(std::min(left, max_jump - pg_gap), p.get_pg_num_target() - p.get_pg_num());
                        unsigned target = p.get_pg_num() + add;
                        left -= add;
                        dout(10) << "pool " << i.first << " pg_num_target " << p.get_pg_num_target() << " pg_num "
                                 << p.get_pg_num() << " -> " << target << dendl;
                        pg_num_to_set[osdmap.get_pool_name(i.first)] = target;
                    }
                }
            }
            ...
            // adjust pgp_num?
            unsigned target = std::min(p.get_pg_num_pending(), p.get_pgp_num_target());
            if (target != p.get_pgp_num()) {
                dout(20) << "pool " << i.first << " pgp_num_target " << p.get_pgp_num_target() << " pgp_num "
                         << p.get_pgp_num() << " -> " << target << dendl;
                if (target > p.get_pgp_num() && p.get_pgp_num() == p.get_pg_num()) {
                    dout(10) << "pool " << i.first << " pgp_num_target " << p.get_pgp_num_target() << " pgp_num "
                             << p.get_pgp_num() << " - increase blocked by pg_num " << p.get_pg_num() << dendl;
                }
                else if (!aggro && (inactive_pgs_ratio > 0 || degraded_ratio > 0 || unknown_pgs_ratio > 0)) {
                    dout(10) << "pool " << i.first << " pgp_num_target " << p.get_pgp_num_target() << " pgp_num "
                             << p.get_pgp_num() << " - inactive|degraded|unknown pgs, deferring pgp_num"
                             << " update" << dendl;
                }
                else if (!aggro && (misplaced_ratio > max_misplaced)) {
                    dout(10) << "pool " << i.first << " pgp_num_target " << p.get_pgp_num_target() << " pgp_num "
                             << p.get_pgp_num() << " - misplaced_ratio " << misplaced_ratio << " > max "
                             << max_misplaced << ", deferring pgp_num update" << dendl;
                }
                else {
                    ...
                    if (aggro ||
                        ...
                        (p.get_pgp_num_target() > p.get_pgp_num() &&
                         s.stats.sum.num_objects <= (MAX_NUM_OBJECTS_PER_PG_FOR_LEAP * p.get_pgp_num_target()))) {
                        next = target;
                    }
                    else {
                        double room = std::min<double>(max_misplaced - misplaced_ratio, max_misplaced / 2.0);
                        unsigned estmax = std::max<unsigned>((double)p.get_pg_num() * room, 1u);
                        unsigned next_min = 0;
                        if (p.get_pgp_num() > estmax) {
                            next_min = p.get_pgp_num() - estmax;
                        }
                        next = std::clamp(target, next_min, p.get_pgp_num() + estmax);
                        dout(20) << " room " << room << " estmax " << estmax << " delta " << (target - p.get_pgp_num())
                                 << " next " << next << dendl;
                        if (p.get_pgp_num_target() == p.get_pg_num_target() &&
                            ...
                            // per-osd pg max.
                            unsigned max_outpace_merges = std::max<unsigned>(8, p.get_pg_num() * max_misplaced);
                            if (next + max_outpace_merges < p.get_pg_num()) {
                                next = p.get_pg_num() - max_outpace_merges;
                                dout(10) << "  using next " << next << " to avoid outpacing merges (max_outpace_merges "
...
void DaemonServer::got_mgr_map()
{
    ...
    cluster_state.with_mgrmap([&](const MgrMap& mgrmap) {
        auto md_update = [&](DaemonKey key) {
            ...
            oss << "{\"prefix\": \"mgr metadata\", \"who\": \"" << key.name << "\", \"id\": \"" << key.name << "\"}";
            monc->start_mon_command({oss.str()}, {}, &c->outbl, &c->outs, c);
        };
...
{
    static const char* KEYS[] = {"mgr_stats_threshold", "mgr_stats_period", nullptr};

    return KEYS;
}

void DaemonServer::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
...
void DaemonServer::_send_configure(ConnectionRef c)
{
    ...
    if (c->peer_is_osd()) {
        configure->osd_perf_metric_queries = osd_perf_metric_collector.get_queries();
    }
...
MetricQueryID DaemonServer::add_osd_perf_query(const OSDPerfMetricQuery& query,
...
MetricQueryID DaemonServer::add_mds_perf_query(const MDSPerfMetricQuery& query,
