...
template<typename Query, typename Limit, typename Key, typename Report> class MetricCollector
{
public:
...
private:
    ...
    {
        return std::any_of(begin(limits), end(limits), [](auto& limits) { return limits.second.has_value(); });
    }
};
