...
protected:
    ...
    Cache(unsigned int size = UINT16_MAX)
        : hits{0}
        , misses{0}
        , capacity{size} {};
...
public:
    ...
    std::pair<uint64_t, uint64_t> get_hit_miss_ratio() { return std::make_pair(hits.load(), misses.load()); }
...
template<class Key, class Value> class TTLCacheBase : public Cache<Key, std::pair<Value, ttl_time_point>>
{
private:
    uint16_t ttl;
...
public:
    TTLCacheBase(uint16_t ttl_ = 0, uint16_t size = UINT16_MAX, float spread = 0.25)
        : Cache<Key, value_type>(size)
        , ttl{ttl_}
        , ttl_spread_ratio{spread}
    {}
    ~TTLCacheBase(){};
    void insert(Key key, Value value);
    Value get(Key key);
...
template<class Key, class Value> class TTLCache : public TTLCacheBase<Key, Value>
{
public:
...
template<class Key> class TTLCache<Key, PyObject*> : public TTLCacheBase<Key, PyObject*>
{
public:
    TTLCache(uint16_t ttl_ = 0, uint16_t size = UINT16_MAX, float spread = 0.25)
