...
struct ServiceMap
{
    ...
    struct Service
    {
        ...
        void count_metadata(const std::string& field, std::map<std::string, int>* out) const;
    };
    ...
    std::pair<Daemon*, bool> get_daemon(const std::string& service, const std::string& daemon)
    {
    ...
    bool rm_daemon(const std::string& service, const std::string& daemon)
    {
    ...
    {
        if (type == "osd" || type == "client" || type == "mon" || type == "mds" || type == "mgr") {
            return true;
        }
