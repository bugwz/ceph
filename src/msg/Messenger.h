...
public:
     ...
     */
    static Messenger* create(CephContext* cct, const std::string& type, entity_name_t name, std::string lname,
                             uint64_t nonce);
     ...
     */
    entity_addr_t get_myaddr_legacy() { return my_addrs->as_legacy_addr(); }
    ...
    void set_auth_client(AuthClient* ac) { auth_client = ac; }
    void set_auth_server(AuthServer* as) { auth_server = as; }
...
public:
     ...
     */
    const ZTracer::Endpoint* get_trace_endpoint() const { return &trace_endpoint; }
     ...
     */
    void set_socket_priority(int prio) { socket_priority = prio; }
    /**
     * Get the socket priority
     *
     * @return the socket priority
     */
    int get_socket_priority() { return socket_priority; }
    /**
    ...
    void add_dispatcher_head(Dispatcher* d)
    {
        ...
        if (d->ms_can_fast_dispatch_any()) fast_dispatchers.push_front(d);
        if (first) ready();
    }
    /**
     ...
     */
    virtual int bind(const entity_addr_t& bind_addr, std::optional<entity_addrvec_t> public_addrs = std::nullopt) = 0;

    virtual int bindv(const entity_addrvec_t& bind_addrs, std::optional<entity_addrvec_t> public_addrs = std::nullopt);
    ...
    virtual bool should_use_msgr2() { return false; }
    ...
    int send_to_mon(Message* m, const entity_addrvec_t& addrs) { return send_to(m, CEPH_ENTITY_TYPE_MON, addrs); }
    int send_to_mds(Message* m, const entity_addrvec_t& addrs) { return send_to(m, CEPH_ENTITY_TYPE_MDS, addrs); }
     ...
     */
    virtual ConnectionRef connect_to(int type, const entity_addrvec_t& dest, bool anon = false,
                                     bool not_local_dest = false) = 0;
    ConnectionRef connect_to_mon(const entity_addrvec_t& dest, bool anon = false, bool not_local_dest = false)
    {
        return connect_to(CEPH_ENTITY_TYPE_MON, dest, anon, not_local_dest);
    }
    ConnectionRef connect_to_mds(const entity_addrvec_t& dest, bool anon = false, bool not_local_dest = false)
    {
        return connect_to(CEPH_ENTITY_TYPE_MDS, dest, anon, not_local_dest);
    }
    ConnectionRef connect_to_osd(const entity_addrvec_t& dest, bool anon = false, bool not_local_dest = false)
    {
        return connect_to(CEPH_ENTITY_TYPE_OSD, dest, anon, not_local_dest);
    }
    ConnectionRef connect_to_mgr(const entity_addrvec_t& dest, bool anon = false, bool not_local_dest = false)
    {
    ...
    virtual void mark_down_addrs(const entity_addrvec_t& a) { mark_down(a.legacy_addr()); }
    /**
...
#define MSGR_SIGPIPE_STOPPER
    ...
    bool ms_can_fast_dispatch(const ceph::cref_t<Message>& m)
    {
        for (const auto& dispatcher : fast_dispatchers) {
            if (dispatcher->ms_can_fast_dispatch2(m)) return true;
        }
        return false;
    }
    ...
    void ms_fast_dispatch(Message* m) { return ms_fast_dispatch(ceph::ref_t<Message>(m, false)); /* consume ref */ }
    /**
     *
     */
    ...
    void ms_deliver_dispatch(const ceph::ref_t<Message>& m)
    {
        ...
        for (const auto& dispatcher : dispatchers) {
            if (dispatcher->ms_dispatch2(m)) return;
        }
    ...
    {
        for (const auto& dispatcher : dispatchers) {
            if (dispatcher->ms_handle_reset(con)) return;
        }
    }
    /**
    ...
    {
        for (const auto& dispatcher : dispatchers) {
            if (dispatcher->ms_handle_refused(con)) return;
        }
    }

    void set_require_authorizer(bool b) { require_authorizer = b; }
