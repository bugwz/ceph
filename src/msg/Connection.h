...
public:
    ...
    virtual bool is_msgr2() const { return false; }

    bool is_anon() const { return anon; }
    ...
    AuthCapsInfo& get_peer_caps_info() { return peer_caps_info; }
    const EntityName& get_peer_entity_name() { return peer_name; }
    uint64_t get_peer_global_id() { return peer_global_id; }
    ...
    entity_addr_t get_peer_addr() const { return peer_addrs->front(); }
    const entity_addrvec_t& get_peer_addrs() const { return *peer_addrs; }
    void set_peer_addr(const entity_addr_t& a) { peer_addrs = entity_addrvec_t(a); }
    ...
    bool has_features(uint64_t f) const { return (features & f) == f; }
    void set_features(uint64_t f) { features = f; }
    void set_feature(uint64_t f) { features |= f; }

    virtual int get_con_mode() const { return CEPH_CON_MODE_CRC; }
