...
bool entity_addr_t::parse(const char* s, const char** end, int default_type)
{
    *this = entity_addr_t();
    ...
    char* o = buf4;
    const char* p = start;
    while (o < buf4 + sizeof(buf4) && *p && ((*p == '.') || (*p >= '0' && *p <= '9'))) {
        *o++ = *p++;
    }
    *o = 0;
    ...
    o = buf6;
    p = start;
    while (o < buf6 + sizeof(buf6) && *p &&
           ((*p == ':') || (*p >= '0' && *p <= '9') || (*p >= 'a' && *p <= 'f') || (*p >= 'A' && *p <= 'F'))) {
        *o++ = *p++;
    }
    *o = 0;
...
std::ostream& operator<<(std::ostream& out, const sockaddr* psa)
{
    ...
    default: return out << "(unrecognized address family " << psa->sa_family << ")";
    }
}
...
void entity_addrvec_t::decode(ceph::buffer::list::const_iterator& bl)
{
    ...
    if (marker > 2) throw ceph::buffer::malformed_input("entity_addrvec_marker > 2");
    decode(v, bl);
}
...
std::string entity_addr_t::ip_only_to_str() const
{
    ...
    switch (get_family()) {
    case AF_INET: host_ip = inet_ntop(AF_INET, &in4_addr().sin_addr, addr_buf, INET_ADDRSTRLEN); break;
    case AF_INET6: host_ip = inet_ntop(AF_INET6, &in6_addr().sin6_addr, addr_buf, INET6_ADDRSTRLEN); break;
    default: break;
    }
