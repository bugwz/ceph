...
{
    return *_dout << "--1- " << messenger->get_myaddrs() << " >> " << *connection->peer_addrs << " conn(" << connection
                  << " " << this << " :" << connection->port << " s=" << get_state_name(state)
                  << " pgs=" << peer_global_seq << " cs=" << connect_seq << " l=" << connection->policy.lossy << ").";
}
...
void ProtocolV1::fault()
{
    ...
    if (!once_ready && out_q.empty() && state >= START_ACCEPT && state <= ACCEPTING_WAIT_CONNECT_MSG_AUTH &&
        !replacing) {
    ...
    if (connection->policy.standby && out_q.empty() && !keepalive && state != WAIT) {
    ...
    if ((state >= START_CONNECT && state <= CONNECTING_SEND_CONNECT_MSG) || state == WAIT) {
        // backoff!
        if (state == WAIT) {
...
void ProtocolV1::send_message(Message* m)
{
    ...
    if (can_fast_prepare && (can_write == WriteStatus::NOWRITE || connection->get_features() != f)) {
        // ensure the correctness of message encoding
        bl.clear();
        m->clear_payload();
        ldout(cct, 5) << __func__ << " clear encoded buffer previous " << f << " != " << connection->get_features()
                      << dendl;
    }
...
void ProtocolV1::prepare_send_message(uint64_t features, Message* m, ceph::buffer::list& bl)
{
...
void ProtocolV1::read_event()
{
    ldout(cct, 20) << __func__ << dendl;
    switch (state) {
    case START_CONNECT: CONTINUATION_RUN(CONTINUATION(send_client_banner)); break;
    case START_ACCEPT: CONTINUATION_RUN(CONTINUATION(send_server_banner)); break;
    case OPENED: CONTINUATION_RUN(CONTINUATION(wait_message)); break;
    case THROTTLE_MESSAGE: CONTINUATION_RUN(CONTINUATION(throttle_message)); break;
    case THROTTLE_BYTES: CONTINUATION_RUN(CONTINUATION(throttle_bytes)); break;
    case THROTTLE_DISPATCH_QUEUE: CONTINUATION_RUN(CONTINUATION(throttle_dispatch_queue)); break;
    default: break;
    }
}
    ...
    if (can_write == WriteStatus::CANWRITE) {
        ...
        do {
            ...
            if (m->queue_start != ceph::mono_time()) {
                connection->logger->tinc(l_msgr_send_messages_queue_lat, ceph::mono_clock::now() - m->queue_start);
            }
...
CtPtr ProtocolV1::read(CONTINUATION_RX_TYPE<ProtocolV1>& next, int len, char* buffer)
{
    if (!buffer) {
        buffer = temp_buffer;
    }
    ssize_t r = connection->read(len, buffer, [&next, this](char* buffer, int r) {
...
CtPtr ProtocolV1::write(CONTINUATION_TX_TYPE<ProtocolV1>& next, ceph::buffer::list& buffer)
{
...
CtPtr ProtocolV1::handle_message_header(char* buffer, int r)
{
    ...
    ldout(cct, 20) << __func__ << " got envelope type=" << current_header.type << " src "
                   ...
                   << " data=" << current_header.data_len << " off " << current_header.data_off << dendl;

    if (messenger->crcflags & MSG_CRC_HEADER) {
        __u32 header_crc = 0;
        header_crc =
            ceph_crc32c(0, (unsigned char*)&current_header, sizeof(current_header) - sizeof(current_header.crc));
        // verify header crc
...
CtPtr ProtocolV1::read_message_middle()
{
    ...
    if (current_header.middle_len) {
        ...
        return READB(current_header.middle_len, middle.c_str(), handle_message_middle);
    }
...
CtPtr ProtocolV1::handle_message_data(char* buffer, int r)
{
    ...
    auto bp = data_blp.get_current_ptr();
    unsigned read_len = std::min(bp.length(), msg_left);
    ceph_assert(read_len < static_cast<unsigned>(std::numeric_limits<int>::max()));
    data_blp += read_len;
...
CtPtr ProtocolV1::handle_message_footer(char* buffer, int r)
{
    ...
    if (aborted) {
        ldout(cct, 0) << __func__ << " got " << front.length() << " + " << middle.length() << " + " << data.length()
                      << " byte message.. ABORTED" << dendl;
        return _fault();
    }

    ldout(cct, 20) << __func__ << " got " << front.length() << " + " << middle.length() << " + " << data.length()
                   << " byte message" << dendl;
    Message* message =
        decode_message(cct, messenger->crcflags, current_header, footer, front, middle, data, connection);
    if (!message) {
    ...
    ldout(cct, 5) << " rx " << message->get_source() << " seq " << message->get_seq() << " " << message << " "
                  << *message << dendl;
    ...
    connection->logger->inc(l_msgr_recv_bytes, cur_msg_size + sizeof(ceph_msg_header) + sizeof(ceph_msg_footer));
...
ssize_t ProtocolV1::write_message(Message* m, ceph::buffer::list& bl, bool more)
{
    ...
    ldout(cct, 20) << __func__ << " sending message type=" << header.type << " src " << entity_name_t(header.src)
                   << " front=" << header.front_len << " data=" << header.data_len << " off " << header.data_off
                   << dendl;
    ...
    if (connection->has_feature(CEPH_FEATURE_MSG_AUTH)) {
        ...
        old_footer.data_crc = messenger->crcflags & MSG_CRC_DATA ? footer.data_crc : 0;
...
uint64_t ProtocolV1::discard_requeued_up_to(uint64_t out_seq, uint64_t seq)
{
    ...
    while (!rq.empty()) {
        ...
        ldout(cct, 10) << __func__ << " " << *(p.second) << " for resend seq " << p.second->get_seq() << " <= " << seq
                       << ", discarding" << dendl;
        p.second->put();
        rq.pop_front();
        count++;
...
void ProtocolV1::discard_out_queue()
{
    ...
    sent.clear();
    for (map<int, list<pair<ceph::buffer::list, Message*> > >::iterator p = out_q.begin(); p != out_q.end(); ++p) {
        for (list<pair<ceph::buffer::list, Message*> >::iterator r = p->second.begin(); r != p->second.end(); ++r) {
...
void ProtocolV1::reset_recv_state()
{
    ...
    if (state > THROTTLE_MESSAGE && state <= READ_FOOTER_AND_DISPATCH && connection->policy.throttler_messages) {
...
Message* ProtocolV1::_get_next_outgoing(ceph::buffer::list* bl)
{
    Message* m = 0;
    if (!out_q.empty()) {
        map<int, list<pair<ceph::buffer::list, Message*> > >::reverse_iterator it = out_q.rbegin();
...
CtPtr ProtocolV1::handle_server_banner_and_identify(char* buffer, int r)
{
    ...
    if (memcmp(buffer, CEPH_BANNER, banner_len)) {
        ldout(cct, 0) << __func__ << " connect protocol error (bad banner) on peer " << connection->get_peer_addr()
                      << dendl;
        return _fault();
    }
    ...
    if (peer_addr != paddr) {
        if (paddr.is_blank_ip() && peer_addr.get_port() == paddr.get_port() &&
            ...
            ldout(cct, 0) << __func__ << " connect claims to be " << paddr << " not " << peer_addr
    ...
    if (messenger->get_myaddrs().empty() || messenger->get_myaddrs().front().is_blank_ip()) {
        ...
        if (cct->_conf->ms_learn_addr_from_peer) {
            ldout(cct, 1) << __func__ << " peer " << connection->target_addr << " says I am " << peer_addr_for_me
...
CtPtr ProtocolV1::send_connect_message()
{
    ...
    if (connection->peer_type != CEPH_ENTITY_TYPE_MON || messenger->get_myname().type() == CEPH_ENTITY_TYPE_MON) {
        if (authorizer_more.length()) {
            ...
            int r = messenger->auth_client->get_auth_request(
                connection, am.get(), &am->auth_method, &preferred_modes, &auth_bl);
            connection->lock.lock();
            if (r < 0) {
                return _fault();
            }
    ...
    connect.protocol_version = messenger->get_proto_version(connection->peer_type, true);
    if (auth_bl.length()) {
        ldout(cct, 10) << __func__ << " connect_msg.authorizer_len=" << auth_bl.length()
    ...
    ldout(cct, 10) << __func__ << " connect sending gseq=" << global_seq << " cseq=" << connect_seq
...
CtPtr ProtocolV1::handle_connect_reply_1(char* buffer, int r)
{
    ...
    ldout(cct, 20) << __func__ << " connect got reply tag " << (int)connect_reply.tag << " connect_seq "
                   << connect_reply.connect_seq << " global_seq " << connect_reply.global_seq << " proto "
                   << connect_reply.protocol_version << " flags " << (int)connect_reply.flags << " features "
...
CtPtr ProtocolV1::handle_connect_reply_auth(char* buffer, int r)
{
    ...
    if (connection->peer_type != CEPH_ENTITY_TYPE_MON || messenger->get_myname().type() == CEPH_ENTITY_TYPE_MON) {
        ...
        connection->lock.unlock();
        if (more) {
            r = messenger->auth_client->handle_auth_reply_more(connection, am.get(), authorizer_reply, &auth_retry_bl);
        }
        else {
            ...
            r = messenger->auth_client->handle_auth_done(
                connection, am.get(), 0 /* global id */, 0 /* con mode */, authorizer_reply, &skey, &con_secret);
        }
        connection->lock.lock();
        if (state != CONNECTING_SEND_CONNECT_MSG) {
            ldout(cct, 1) << __func__ << " state changed" << dendl;
...
CtPtr ProtocolV1::handle_connect_reply_2()
{
    ...
    if (connect_reply.tag == CEPH_MSGR_TAG_FEATURES) {
        ldout(cct, 0) << __func__ << " connect protocol feature mismatch, my " << std::hex
                      << connection->policy.features_supported << " < peer " << connect_reply.features << " missing "
                      << (connect_reply.features & ~connection->policy.features_supported) << std::dec << dendl;
        return _fault();
    }
    ...
    if (connect_reply.tag == CEPH_MSGR_TAG_RETRY_GLOBAL) {
        ...
        ldout(cct, 5) << __func__ << " connect got RETRY_GLOBAL " << connect_reply.global_seq << " chose new "
                      << global_seq << dendl;
        return CONTINUE(send_connect_message);
    }
    ...
    if (connect_reply.tag == CEPH_MSGR_TAG_RETRY_SESSION) {
        ...
        ldout(cct, 5) << __func__ << " connect got RETRY_SESSION " << connect_seq << " -> " << connect_reply.connect_seq
                      << dendl;
    ...
    feat_missing = connection->policy.features_required & ~(uint64_t)connect_reply.features;
    if (feat_missing) {
...
CtPtr ProtocolV1::client_ready()
{
    ...
    backoff = utime_t();
    connection->set_features((uint64_t)connect_reply.features & (uint64_t)connection->policy.features_supported);
    ...
    if (auth_meta->authorizer) {
        ...
        session_security.reset(get_auth_session_handler(
            cct, auth_meta->authorizer->protocol, auth_meta->session_key, connection->get_features()));
    }
    else {
...
CtPtr ProtocolV1::send_server_banner()
{
    ...
    ldout(cct, 1) << __func__ << " sd=" << connection->cs.fd() << " legacy " << legacy << " socket_addr "
                  << connection->socket_addr << " target_addr " << connection->target_addr << dendl;
...
CtPtr ProtocolV1::wait_client_banner()
{
    ...
    return READ(strlen(CEPH_BANNER) + sizeof(ceph_entity_addr), handle_client_banner);
}
...
CtPtr ProtocolV1::handle_client_banner(char* buffer, int r)
{
    ...
    if (memcmp(buffer, CEPH_BANNER, strlen(CEPH_BANNER))) {
        ldout(cct, 1) << __func__ << " accept peer sent bad banner '" << buffer << "' (should be '" << CEPH_BANNER
                      << "')" << dendl;
        return _fault();
    }
    ...
    if (peer_addr.is_blank_ip()) {
        ...
        ldout(cct, 0) << __func__ << " accept peer addr is really " << peer_addr << " (socket is "
...
CtPtr ProtocolV1::wait_connect_message_auth()
{
    ...
    return READB(connect_msg.authorizer_len, authorizer_buf.c_str(), handle_connect_message_auth);
}
...
CtPtr ProtocolV1::handle_connect_message_2()
{
    ...
    ldout(cct, 20) << __func__ << " accept got peer connect_seq " << connect_msg.connect_seq << " global_seq "
    ...
    ldout(cct, 10) << __func__ << " accept of host_type " << connect_msg.host_type
                   << ", policy.lossy=" << connection->policy.lossy << " policy.server=" << connection->policy.server
                   << " policy.standby=" << connection->policy.standby
                   << " policy.resetcheck=" << connection->policy.resetcheck << " features 0x" << std::hex
                   << (uint64_t)connect_msg.features << std::dec << dendl;
    ...
    reply.protocol_version = messenger->get_proto_version(connection->peer_type, false);

    // mismatch?
    ldout(cct, 10) << __func__ << " accept my proto " << reply.protocol_version << ", their proto "
                   << connect_msg.protocol_version << dendl;

    if (connect_msg.protocol_version != reply.protocol_version) {
        return send_connect_message_reply(CEPH_MSGR_TAG_BADPROTOVER, reply, authorizer_reply);
    }

    // require signatures for cephx?
    if (connect_msg.authorizer_protocol == CEPH_AUTH_CEPHX) {
        if (connection->peer_type == CEPH_ENTITY_TYPE_OSD || connection->peer_type == CEPH_ENTITY_TYPE_MDS ||
            connection->peer_type == CEPH_ENTITY_TYPE_MGR) {
            if (cct->_conf->cephx_require_signatures || cct->_conf->cephx_cluster_require_signatures) {
            ...
            if (cct->_conf->cephx_require_version >= 2 || cct->_conf->cephx_cluster_require_version >= 2) {
            ...
            if (cct->_conf->cephx_require_signatures || cct->_conf->cephx_service_require_signatures) {
                ldout(cct, 10) << __func__ << " using cephx, requiring MSG_AUTH feature bit for service" << dendl;
                connection->policy.features_required |= CEPH_FEATURE_MSG_AUTH;
            }
            if (cct->_conf->cephx_require_version >= 2 || cct->_conf->cephx_service_require_version >= 2) {
    ...
    uint64_t feat_missing = connection->policy.features_required & ~(uint64_t)connect_msg.features;
    if (feat_missing) {
        ldout(cct, 1) << __func__ << " peer missing required features " << std::hex << feat_missing << std::dec
                      << dendl;
        return send_connect_message_reply(CEPH_MSGR_TAG_FEATURES, reply, authorizer_reply);
    }
    ...
    connection->lock.unlock();
    ldout(cct, 10) << __func__ << " authorizor_protocol " << connect_msg.authorizer_protocol << " len "
                   << auth_bl_copy.length() << dendl;
    bool more = (bool)auth_meta->authorizer_challenge;
    int r = messenger->auth_server->handle_auth_request(
        connection, am.get(), more, am->auth_method, auth_bl_copy, &authorizer_reply);
    if (r < 0) {
        connection->lock.lock();
        ...
        session_security.reset();
        return send_connect_message_reply(CEPH_MSGR_TAG_BADAUTHORIZER, reply, authorizer_reply);
    }
    if (r == 0) {
        connection->lock.lock();
        if (state != ACCEPTING_WAIT_CONNECT_MSG_AUTH) {
        ...
        return send_connect_message_reply(CEPH_MSGR_TAG_CHALLENGE_AUTHORIZER, reply, authorizer_reply);
    }
    ...
    if (existing) {
        ...
        if (exproto->replacing) {
            ldout(cct, 1) << __func__ << " existing racing replace happened while replacing."
                          << " existing_state=" << connection->get_state_name(existing->state) << dendl;
            reply.global_seq = exproto->peer_global_seq;
            existing->lock.unlock();
            return send_connect_message_reply(CEPH_MSGR_TAG_RETRY_GLOBAL, reply, authorizer_reply);
        }

        if (connect_msg.global_seq < exproto->peer_global_seq) {
            ldout(cct, 10) << __func__ << " accept existing " << existing << ".gseq " << exproto->peer_global_seq
            ...
            ldout(cct, 10) << __func__ << " accept existing " << existing << ".gseq " << exproto->peer_global_seq
                           << " <= " << connect_msg.global_seq << ", looks ok" << dendl;
        }

        if (existing->policy.lossy) {
            ldout(cct, 0) << __func__
                          << " accept replacing existing (lossy) channel (new one lossy=" << connection->policy.lossy
                          << ")" << dendl;
            exproto->session_reset();
            return replace(existing, reply, authorizer_reply);
        }

        ldout(cct, 1) << __func__ << " accept connect_seq " << connect_msg.connect_seq
        ...
        if (connect_msg.connect_seq < exproto->connect_seq) {
            ...
            ldout(cct, 10) << __func__ << " accept existing " << existing << ".cseq " << exproto->connect_seq << " > "
                           << connect_msg.connect_seq << ", RETRY_SESSION" << dendl;
            reply.connect_seq = exproto->connect_seq + 1;
            existing->lock.unlock();
            return send_connect_message_reply(CEPH_MSGR_TAG_RETRY_SESSION, reply, authorizer_reply);
        }
        ...
        if (connect_msg.connect_seq == exproto->connect_seq) {
            ...
            if (exproto->state == OPENED || exproto->state == STANDBY) {
                ldout(cct, 10) << __func__ << " accept connection race, existing " << existing << ".cseq "
            ...
            // connection race?
            if (connection->peer_addrs->legacy_addr() < messenger->get_myaddr_legacy() || existing->policy.server) {
                // incoming wins
                ldout(cct, 10) << __func__ << " accept connection race, existing " << existing << ".cseq "
                               << exproto->connect_seq << " == " << connect_msg.connect_seq
                ...
                ldout(messenger->cct, 10)
                    << __func__ << " accept connection race, existing " << existing << ".cseq " << exproto->connect_seq
                    << " == " << connect_msg.connect_seq << ", sending WAIT" << dendl;
                ceph_assert(connection->peer_addrs->legacy_addr() > messenger->get_myaddr_legacy());
                existing->lock.unlock();
                ...
                return send_connect_message_reply(CEPH_MSGR_TAG_WAIT, reply, authorizer_reply);
            }
        }
        ...
        if (connection->policy.resetcheck &&   // RESETSESSION only used by servers;
            ...
            ldout(cct, 0) << __func__ << " accept we reset (peer sent cseq " << connect_msg.connect_seq << ", "
                          << existing << ".cseq = " << exproto->connect_seq << "), sending RESETSESSION " << dendl;
            existing->lock.unlock();
            return send_connect_message_reply(CEPH_MSGR_TAG_RESETSESSION, reply, authorizer_reply);
        }

        // reconnect
        ldout(cct, 10) << __func__ << " accept peer sent cseq " << connect_msg.connect_seq << " > "
                       << exproto->connect_seq << dendl;
        return replace(existing, reply, authorizer_reply);
    }   // existing
...
CtPtr ProtocolV1::send_connect_message_reply(char tag, ceph_msg_connect_reply& reply,
                                             ceph::buffer::list& authorizer_reply)
{
    ldout(cct, 20) << __func__ << dendl;
    ...
    reply.tag = tag;
    reply.features =
        ((uint64_t)connect_msg.features & connection->policy.features_supported) | connection->policy.features_required;
    reply.authorizer_len = authorizer_reply.length();
    reply_bl.append((char*)&reply, sizeof(reply));

    ldout(cct, 10) << __func__ << " reply features 0x" << std::hex << reply.features << " = (policy sup 0x"
                   << connection->policy.features_supported << " & connect 0x" << (uint64_t)connect_msg.features
                   << ") | policy req 0x" << connection->policy.features_required << dendl;
...
CtPtr ProtocolV1::replace(const AsyncConnectionRef& existing, ceph_msg_connect_reply& reply,
                          ceph::buffer::list& authorizer_reply)
{
    ...
    if (existing->policy.lossy) {
        ...
        auto deactivate_existing = std::bind(
            [existing, new_worker, new_center, exproto, reply, authorizer_reply](ConnectedSocket& cs) mutable {
                ...
                auto transfer_existing = [existing, exproto, reply, authorizer_reply]() mutable {
                    std::lock_guard<std::mutex> l(existing->lock);
...
CtPtr ProtocolV1::open(ceph_msg_connect_reply& reply, ceph::buffer::list& authorizer_reply)
{
    ldout(cct, 20) << __func__ << dendl;

    connect_seq = connect_msg.connect_seq + 1;
    peer_global_seq = connect_msg.global_seq;
    ldout(cct, 10) << __func__ << " accept success, connect_seq = " << connect_seq << " in_seq=" << in_seq
                   << ", sending READY" << dendl;

    // if it is a hard reset from peer, we don't need a round-trip to negotiate
    // in/out sequence
    if ((connect_msg.features & CEPH_FEATURE_RECONNECT_SEQ) && !is_reset_from_peer) {
    ...
    connection->set_features((uint64_t)reply.features & (uint64_t)connect_msg.features);
    ldout(cct, 10) << __func__ << " accept features " << connection->get_features() << " authorizer_protocol "
                   << connect_msg.authorizer_protocol << dendl;

    session_security.reset(
        get_auth_session_handler(cct, auth_meta->auth_method, auth_meta->session_key, connection->get_features()));
    ...
    if (r < 0) {
        ldout(cct, 1) << __func__
                      << " existing race replacing process for addr = " << connection->peer_addrs->legacy_addr()
