...
class Packet
{
    ...
    struct impl
    {
        ...
        static std::unique_ptr<impl> copy(impl* old) { return copy(old, old->_nr_frags); }
        ...
        void operator delete(void* ptr, size_t nr_frags) { return ::operator delete(ptr); }
        ...
        {
            return _nr_frags && frags[0].base >= data && frags[0].base < data + internal_data_size;
        }
        ...
        void copy_internal_fragment_to(impl* to)
        {
            ...
            std::copy(frags[0].base, frags[0].base + frags[0].size, to->frags[0].base);
        }
    };
    explicit Packet(std::unique_ptr<impl>&& impl)
        : _impl(std::move(impl))
    {}
...
public:
    ...
    Packet free_on_cpu(
        EventCenter* c, std::function<void()> cb = [] {});
    ...
    std::optional<uint32_t> rss_hash() { return _impl->rss_hash; }
    void set_rss_hash(uint32_t hash) { _impl->rss_hash = hash; }
...
inline Packet::Packet(size_t nr_frags)
    : _impl(impl::allocate(nr_frags))
{}

inline Packet::Packet(fragment frag)
    : _impl(new impl(frag))
{}
...
template<typename Iterator> inline Packet::Packet(Iterator begin, Iterator end, deleter del)
{
...
inline bool Packet::allocate_headroom(size_t size)
{
    if (_impl->headroom >= size) {
        ...
        if (!_impl->using_internal_data()) {
            ...
            std::copy_backward(_impl->frags, _impl->frags + _impl->_nr_frags, _impl->frags + _impl->_nr_frags + 1);
...
inline Packet::Packet(Packet&& x, deleter d)
    : _impl(std::move(x._impl))
{
...
inline void Packet::append(Packet&& p)
{
    ...
    std::copy(p._impl->frags, p._impl->frags + p._impl->_nr_frags, _impl->frags + _impl->_nr_frags);
