...
class ipv4;
template<ip_protocol_num ProtoNum> class ipv4_l4;
...
template<typename InetTraits> struct l4connid
{
...
public:
    ...
    explicit icmp(CephContext* c, inet_type& inet)
        : cct(c)
        , _inet(inet)
        , _queue_space(c, "DPDK::icmp::_queue_space", 212992)
    {
...
public:
    ipv4_icmp(CephContext* c, ipv4& inet)
        : cct(c)
        , _inet_l4(inet)
        , _icmp(c, _inet_l4)
    {}
...
struct ipv4_frag_id
{
    ...
    {
        return src_ip == x.src_ip && dst_ip == x.dst_ip && identification == x.identification && protocol == x.protocol;
    }
};

struct ipv4_frag_id::hash : private std::hash<ipv4_address>, private std::hash<uint16_t>, private std::hash<uint8_t>
{
...
private:
    ...
    bool in_my_netmask(ipv4_address a) const { return !((a.ip ^ _host_address.ip) & _netmask.ip); }
    void frag_limit_mem();
...
public:
    ...
    ~ipv4() { delete frag_handler; }
    void set_host_address(ipv4_address ip)
    {
        _host_address = ip;
        _arp.set_self_addr(ip);
    }
    ipv4_address host_address() { return _host_address; }
    void set_gw_address(ipv4_address ip) { _gw_address = ip; }
    ipv4_address gw_address() const { return _gw_address; }
    void set_netmask_address(ipv4_address ip) { _netmask = ip; }
    ipv4_address netmask_address() const { return _netmask; }
    interface* netif() const { return _netif; }
    ...
    void set_packet_filter(ip_packet_filter* f) { _packet_filter = f; }
    ip_packet_filter* packet_filter() const { return _packet_filter; }
    ...
    {
        if (p.len() + ipv4_hdr_len_min <= hw_features.mtu) return false;

        if ((proto_num == ip_protocol_num::tcp && hw_features.tx_tso)) return false;

        return true;
    }
    void learn(ethernet_address l2, ipv4_address l3) { _arp.learn(l2, l3); }
    void register_packet_provider(ipv4_traits::packet_provider_type&& func)
    {
