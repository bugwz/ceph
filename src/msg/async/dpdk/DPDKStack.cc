...
{
    static enum { WAIT_DEVICE_STAGE, WAIT_PORT_FIN_STAGE, DONE } create_stage = WAIT_DEVICE_STAGE;
    ...
    if (i == 0) {
        ...
        std::unique_ptr<DPDKDevice> dev = create_dpdk_net_device(
            cct, cores, cct->_conf->ms_dpdk_port_id, cct->_conf->ms_dpdk_lro, cct->_conf->ms_dpdk_hw_flow_control);
    ...
    if (i < sdev->hw_queues_count()) {
        ...
        for (unsigned j = sdev->hw_queues_count() + i % sdev->hw_queues_count(); j < cores;
             j += sdev->hw_queues_count())
            cpu_weights[i] = 1;
    ...
    sdev->workers[i] = this;
    _impl = std::unique_ptr<DPDKWorker::Impl>(new DPDKWorker::Impl(cct, i, &center, sdev));
    {
        std::lock_guard l{lock};
        if (!--queue_init_done) {
...
static bool parse_available_address(const std::string& ips, const std::string& gates, const std::string& masks,
                                    std::vector<AvailableIPAddress>& res)
{
    ...
    if (ip_vec.empty() || ip_vec.size() != gate_vec.size() || ip_vec.size() != mask_vec.size()) return false;
...
static bool match_available_address(const std::vector<AvailableIPAddress>& avails, const entity_addr_t& ip, int& res)
{
    for (size_t i = 0; i < avails.size(); ++i) {
        entity_addr_t addr;
        auto a = std::get<0>(avails[i]).c_str();
        if (!addr.parse(a)) continue;
    ...
    , _inet(cct, c, &_netif)
{
    ...
    bool parsed = parse_available_address(cct->_conf.get_val<std::string>("ms_dpdk_host_ipv4_addr"),
                                          ...
                                          tuples);
    if (!parsed) {
        lderr(cct) << __func__ << " no available address " << cct->_conf.get_val<std::string>("ms_dpdk_host_ipv4_addr")
                   << ", " << cct->_conf.get_val<std::string>("ms_dpdk_gateway_ipv4_addr") << ", "
                   << cct->_conf.get_val<std::string>("ms_dpdk_netmask_ipv4_addr") << ", " << dendl;
        ceph_abort();
    }
...
int DPDKWorker::listen(entity_addr_t& sa, unsigned addr_slot, const SocketOptions& opt, ServerSocket* sock)
{
    ...
    return tcpv4_listen(_impl->_inet.get_tcp(), sa.get_port(), opt, sa.get_type(), addr_slot, sock);
}
...
{
    eal.execute_on_master([&]() { rte_eal_wait_lcore(i + 1); });
    if (i + 1 == get_num_worker()) eal.stop();
}
