...
private:
    ...
    public:
        ...
        void do_request(uint64_t r)
        {
            ...
            res._waiters.clear();
            res.timeout_fd = arp->center->create_time_event(1 * 1000 * 1000, this);
        }
    };
...
public:
    ...
    explicit arp_for(CephContext* c, arp& a, EventCenter* cen)
        : arp_for_protocol(a, L3::arp_protocol_type())
        , cct(c)
        , center(cen)
    {
    ...
    ~arp_for()
    {
        for (auto&& p : _in_progress) center->delete_time_event(p.second.timeout_fd);
    }
...
template<typename L3> void arp_for<L3>::learn(l2addr hwaddr, l3addr paddr)
{
    _table[paddr] = hwaddr;
...
template<typename L3> void arp_for<L3>::wait(const l3addr& paddr, Packet p, resolution_cb cb)
{
    ...
    if (first_request) {
        res.timeout_fd =
            center->create_time_event(1 * 1000 * 1000, new C_handle_arp_timeout(this, paddr, first_request));
        send_query(paddr);
    }
...
template<typename L3> int arp_for<L3>::received(Packet p)
{
    ...
    switch (h.oper) {
    case op_request: return handle_request(&h);
    case op_reply: _arp._netif->arp_learn(h.sender_hwaddr, h.sender_paddr); return 0;
    default: return 0;
    }
}
