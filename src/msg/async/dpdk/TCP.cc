...
void tcp_option::parse(uint8_t* beg, uint8_t* end)
{
    while (beg < end) {
        ...
        case option_kind::sack:
            ...
            beg += option_len::sack;
            break;
        case option_kind::nop: beg += option_len::nop; break;
        case option_kind::eol: return;
        default:
...
ipv4_tcp::ipv4_tcp(ipv4& inet, EventCenter* c)
    : _inet_l4(inet)
    , _tcp(std::unique_ptr<tcp<ipv4_traits>>(new tcp<ipv4_traits>(inet.cct, _inet_l4, c)))
{}
...
int tcpv4_listen(tcp<ipv4_traits>& tcpv4, uint16_t port, const SocketOptions& opts, int type, unsigned addr_slot,
                 ServerSocket* sock)
{
    auto p = new DPDKServerSocketImpl<tcp<ipv4_traits>>(tcpv4, port, opts, type, addr_slot);
    int r = p->listen();
    if (r < 0) {
        delete p;
        return r;
...
int tcpv4_connect(tcp<ipv4_traits>& tcpv4, const entity_addr_t& addr, ConnectedSocket* sock)
{
    auto conn = tcpv4.connect(addr);
    *sock = ConnectedSocket(
        std::unique_ptr<ConnectedSocketImpl>(new NativeConnectedSocketImpl<tcp<ipv4_traits>>(std::move(conn))));
    return 0;
}

template<typename InetTraits> void tcp<InetTraits>::respond_with_reset(tcp_hdr* rth, ipaddr local_ip, ipaddr foreign_ip)
{
...
template<typename InetTraits> std::ostream& tcp<InetTraits>::tcb::_prefix(std::ostream* _dout)
{
    return *_dout << "tcp " << _local_ip << ":" << _local_port << " -> " << _foreign_ip << ":" << _foreign_port
                  << " tcb(" << this << " fd=" << fd << " s=" << _state << ").";
}

template<typename InetTraits> void tcp<InetTraits>::tcb::input_handle_listen_state(tcp_hdr* th, Packet p)
{
...
template<typename InetTraits> void tcp<InetTraits>::tcb::input_handle_syn_sent_state(tcp_hdr* th, Packet p)
{
    auto opt_len = th->data_offset * 4 - sizeof(tcp_hdr);
    auto opt_start = reinterpret_cast<uint8_t*>(p.get_header(0, th->data_offset * 4)) + sizeof(tcp_hdr);
    auto opt_end = opt_start + opt_len;
...
template<typename InetTraits> void tcp<InetTraits>::tcb::input_handle_other_state(tcp_hdr* th, Packet p)
{
    p.trim_front(th->data_offset * 4);
    ...
    ldout(_tcp.cct, 20) << __func__ << " tcp header seq " << seg_seq.raw << " ack " << seg_ack.raw << " snd next "
                        << _snd.next.raw << " unack " << _snd.unacknowledged.raw << " rcv next " << _rcv.next.raw
                        << " len " << seg_len << " fin=" << bool(th->f_fin) << " syn=" << bool(th->f_syn) << dendl;
    ...
    if (seg_seq != _rcv.next) {
        ldout(_tcp.cct, 10) << __func__ << " out of order, expect " << _rcv.next.raw << " actual " << seg_seq.raw
                            << " out of order size " << _rcv.out_of_order.map.size() << dendl;
    ...
    if (!th->f_ack) {
        ...
        auto update_window = [this, th, seg_seq, seg_ack] {
            ldout(_tcp.cct, 20) << __func__ << " window update seg_seq=" << seg_seq << " seg_ack=" << seg_ack
                                << " old window=" << th->window << " new window=" << int(_snd.window_scale) << dendl;
        ...
        if (in_state(ESTABLISHED | CLOSE_WAIT)) {
            ...
            else if (!_snd.data.empty() && seg_len == 0 && th->f_fin == 0 && th->f_syn == 0 &&
                     th->ack == _snd.unacknowledged && uint32_t(th->window << _snd.window_scale) == _snd.window) {
                // Note:
                // RFC793 states:
...
template<typename InetTraits> void tcp<InetTraits>::tcb::connect()
{
...
template<typename InetTraits> void tcp<InetTraits>::tcb::close_final_cleanup()
{
...
template<typename InetTraits> void tcp<InetTraits>::tcb::retransmit()
{
    ...
    ldout(_tcp.cct, 20) << __func__ << " unack data size " << _snd.data.size() << " nr=" << unacked_seg.nr_transmits
                        << dendl;
...
template<typename InetTraits> void tcp<InetTraits>::tcb::persist()
{
