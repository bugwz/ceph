...
public:
    ...
    void proxy_send(Packet p) { _proxy_packetq.push_back(std::move(p)); }
    void register_packet_provider(packet_provider_type func) { _pkt_providers.push_back(std::move(func)); }
    bool poll_tx();
    ...
    public:
        static tx_buf* me(rte_mbuf* mbuf) { return reinterpret_cast<tx_buf*>(mbuf); }
    ...
    private:
         ...
         *         failure
         */
        static tx_buf* from_packet_zc(CephContext* cct, Packet&& p, DPDKQueuePair& qp);
        ...
        static bool do_one_frag(DoOneBufFunc do_one_buf, DPDKQueuePair& qp, fragment& frag, rte_mbuf*& head,
         ...
         * @return TRUE in case of success
         */
        static bool translate_one_frag(DPDKQueuePair& qp, fragment& frag, rte_mbuf*& head, rte_mbuf*& last_seg,
                                       unsigned& nsegs)
        {
         ...
         * @return TRUE in case of success
         */
        static bool copy_one_frag(DPDKQueuePair& qp, fragment& frag, rte_mbuf*& head, rte_mbuf*& last_seg,
                                  unsigned& nsegs)
        {
         ...
         */
        static size_t set_one_data_buf(DPDKQueuePair& qp, rte_mbuf*& m, char* va, size_t buf_len)
        {
            ...
            if (!pa) return copy_one_data_buf(qp, m, va, buf_len);
         ...
         */
        static size_t copy_one_data_buf(DPDKQueuePair& qp, rte_mbuf*& m, char* data, size_t buf_len);
        ...
        static bool check_frag0(Packet& p)
        {
            ...
            // go.
            //
            if (p.frag(0).size < 128) return false;

            return true;
        }

    public:
        tx_buf(tx_buf_factory& fc)
            : _fc(fc)
        {
        ...
        void set_packet(Packet&& p) { _p = std::move(p); }
    ...
    public:
        ...
        unsigned ring_size() const { return _ring.size(); }
    ...
    private:
         ...
         */
        tx_buf* get_one_completed() { return tx_buf::me(rte_pktmbuf_alloc(_pool)); }
...
public:
    ...
    void rx_start() { _rx_poller.emplace(this); }

    uint32_t send(circular_buffer<Packet>& pb)
    {
        // Zero-copy send
        return _send(pb, [&](Packet&& p) { return tx_buf::from_packet_zc(cct, std::move(p), *this); });
    }
...
private:
    template<class Func> uint32_t _send(circular_buffer<Packet>& pb, Func&& packet_to_tx_buf_p)
    {
        if (_tx_burst.size() == 0) {
            for (auto&& p : pb) {
                // TODO: ceph_assert() in a fast path! Remove me ASAP!
        ...
        uint16_t sent =
            rte_eth_tx_burst(_dev_port_idx, _qid, _tx_burst.data() + _tx_burst_idx, _tx_burst.size() - _tx_burst_idx);
     ...
     */
    static bool refill_rx_mbuf(rte_mbuf* m, size_t size, std::vector<void*>& datas)
    {
...
private:
    ...
    public:
        explicit DPDKTXPoller(DPDKQueuePair* qp)
            : EventCenter::Poller(qp->center, "DPDK::DPDKTXPoller")
            , qp(qp)
        {}

        virtual int poll() { return qp->poll_tx(); }
    } _tx_poller;
    ...
    public:
        explicit DPDKRXGCPoller(DPDKQueuePair* qp)
            : EventCenter::Poller(qp->center, "DPDK::DPDKRXGCPoller")
            , qp(qp)
        {}

        virtual int poll() { return qp->rx_gc(); }
    } _rx_gc_poller;
    ...
    public:
        explicit DPDKRXPoller(DPDKQueuePair* qp)
            : EventCenter::Poller(qp->center, "DPDK::DPDKRXPoller")
            , qp(qp)
        {}

        virtual int poll() { return qp->poll_rx_once(); }
    };
    ...
    public:
        explicit DPDKTXGCPoller(DPDKQueuePair* qp)
            : EventCenter::Poller(qp->center, "DPDK::DPDKTXGCPoller")
            , qp(qp)
        {}

        virtual int poll() { return qp->_tx_buf_factory.gc(); }
    } _tx_gc_poller;
...
public:
        ...
        , _enable_fc(enable_fc)
    {
        ...
        plb.add_u64_counter(
            l_dpdk_dev_rx_mcast, "dpdk_device_receive_multicast_packets", "DPDK received multicast packets");
        plb.add_u64_counter(
            l_dpdk_dev_rx_badcrc_errors, "dpdk_device_receive_badcrc_errors", "DPDK received bad crc errors");

        plb.add_u64_counter(
            l_dpdk_dev_rx_total_errors, "dpdk_device_receive_total_errors", "DPDK received total_errors");
        plb.add_u64_counter(l_dpdk_dev_tx_total_errors, "dpdk_device_send_total_errors", "DPDK sendd total_errors");
        plb.add_u64_counter(
            l_dpdk_dev_rx_dropped_errors, "dpdk_device_receive_dropped_errors", "DPDK received dropped errors");
        plb.add_u64_counter(l_dpdk_dev_rx_nombuf_errors,
                            "dpdk_device_receive_nombuf_errors",
                            "DPDK received RX mbuf allocation errors");
    ...
    ~DPDKDevice()
    {
        ...
        dfx_hook.reset();
        if (_flow) rte_flow_destroy(_port_idx, _flow, nullptr);
        rte_eth_dev_stop(_port_idx);
    }

    DPDKQueuePair& queue_for_cpu(unsigned cpu) { return *_queues[cpu]; }
    void l2receive(int qid, Packet p) { _queues[qid]->_rx_stream.produce(std::move(p)); }
    ...
    hw_features get_hw_features() { return _hw_features; }
    const rss_key_type& rss_key() const { return _rss_key; }
    uint16_t hw_queues_count() { return _num_queues; }
    std::unique_ptr<DPDKQueuePair> init_local_queue(CephContext* c, EventCenter* center, std::string hugepages,
                                                    uint16_t qid)
    {
    ...
    template<typename Func> unsigned forward_dst(unsigned src_cpuid, Func&& hashfn)
    {
        auto& qp = queue_for_cpu(src_cpuid);
        if (!qp._sw_reta) return src_cpuid;
    ...
    const rte_eth_rxconf* def_rx_conf() const { return &_dev_info.default_rxconf; }

    const rte_eth_txconf* def_tx_conf() const { return &_dev_info.default_txconf; }
    ...
    bool is_i40e_device() const { return _is_i40e_device; }
    bool is_vmxnet3_device() const { return _is_vmxnet3_device; }
};


std::unique_ptr<DPDKDevice> create_dpdk_net_device(CephContext* c, unsigned cores, uint8_t port_idx = 0,
