...
{
    auto ip = a.ip;
    return os << ((ip >> 24) & 0xff) << "." << ((ip >> 16) & 0xff) << "." << ((ip >> 8) & 0xff) << "."
              << ((ip >> 0) & 0xff);
}
...
public:
    C_handle_frag_timeout(ipv4* i)
        : _ipv4(i)
    {}
    void do_request(uint64_t fd_or_id) { _ipv4->frag_timeout(); }
};
...
ipv4::ipv4(CephContext* c, EventCenter* cen, interface* netif)
    ...
    , _rx_packets(_l3.receive(
          [this](Packet p, ethernet_address ea) { return handle_received_packet(std::move(p), ea); },
          [this](forward_hash& out_hash_data, Packet& p, size_t off) { return forward(out_hash_data, p, off); }))
    , _tcp(*this, cen)
    , _icmp(c, *this)
    ...
    , _packet_filter(nullptr)
{
    ...
    plb.add_u64_counter(
        l_dpdk_total_linearize_operations, "dpdk_ip_linearize_operations", "DPDK IP Packet linearization operations");
...
int ipv4::handle_received_packet(Packet p, ethernet_address from)
{
    ...
    ldout(cct, 10) << __func__ << " get " << std::hex << int(h.ip_proto) << std::dec << " packet from " << h.src_ip
                   << " -> " << h.dst_ip << " id=" << h.id << " ip_len=" << ip_len << " ip_hdr_len=" << ip_hdr_len
    ...
    if (mf == true || offset != 0) {
        ...
        if (frag.is_complete()) {
            ...
            perf_logger->set(l_dpdk_total_linearize_operations, ipv4_packet_merger::linearizations());
        }
        else {
...
void ipv4::send(ipv4_address to, ip_protocol_num proto_num, Packet p, ethernet_address e_dst)
{
    ...
    auto send_pkt = [this, to, proto_num, needs_frag, e_dst](Packet& pkt, uint16_t remaining, uint16_t offset) mutable {
        ...
        iph->dst_ip = to;
        ldout(cct, 20) << " ipv4::send "
                       << " id=" << iph->id << " " << _host_address << " -> " << to << " len " << pkt.len() << dendl;
        *iph = iph->hton();
