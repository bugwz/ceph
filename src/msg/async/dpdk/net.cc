...
interface::interface(CephContext* cct, std::shared_ptr<DPDKDevice> dev, EventCenter* center)
    : cct(cct)
    , _dev(dev)
    , _rx(_dev->receive(center->get_id(), [center, this](Packet p) { return dispatch_packet(center, std::move(p)); }))
    ...
    , _hw_features(_dev->get_hw_features())
{
    ...
    dev->queue_for_cpu(center->get_id()).register_packet_provider([this, idx, qid]() mutable {
        ...
        for (size_t i = 0; i < _pkt_providers.size(); i++) {
            ...
            if (idx == _pkt_providers.size()) idx = 0;
            if (l3p) {
                ...
                *eh = eh->hton();
                ldout(this->cct, 10) << "=== tx === proto " << std::hex << uint16_t(l3pv.proto_num) << " "
                                     << _hw_address << " -> " << l3pv.to << " length " << std::dec << l3pv.p.len()
                                     << dendl;
                p = std::move(l3pv.p);
                return p;
            }
...
subscription<Packet, ethernet_address> interface::register_l3(
    eth_protocol_num proto_num, std::function<int(Packet p, ethernet_address from)> next,
    std::function<bool(forward_hash&, Packet& p, size_t)> forward)
{
    auto i = _proto_map.emplace(
        std::piecewise_construct, std::make_tuple(uint16_t(proto_num)), std::forward_as_tuple(std::move(forward)));
    ...
    if (eh) {
        ...
        if (hwrss) {
            ldout(cct, 10) << __func__ << " === rx === proto " << std::hex << ::ntoh(eh->eth_proto) << " "
                           << eh->src_mac.ntoh() << " -> " << eh->dst_mac.ntoh() << " length " << std::dec << p.len()
                           << " rss_hash " << *p.rss_hash() << dendl;
        }
        else {
            ldout(cct, 10) << __func__ << " === rx === proto " << std::hex << ::ntoh(eh->eth_proto) << " "
                           << eh->src_mac.ntoh() << " -> " << eh->dst_mac.ntoh() << " length " << std::dec << p.len()
