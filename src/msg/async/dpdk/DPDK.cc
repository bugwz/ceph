...
uint32_t qp_mempool_obj_size()
{
    ...
    // Rx
    mp_size +=
        align_up(rte_mempool_calc_obj_size(mbuf_overhead, 0, &mp_obj_sz) + sizeof(struct rte_pktmbuf_pool_private),
                 huge_page_size);

    // Tx
    std::memset(&mp_obj_sz, 0, sizeof(mp_obj_sz));
    mp_size +=
        align_up(rte_mempool_calc_obj_size(inline_mbuf_size, 0, &mp_obj_sz) + sizeof(struct rte_pktmbuf_pool_private),
                 huge_page_size);
    return mp_size;
}
...
int DPDKDevice::init_port_start()
{
    ...
    const uint64_t tx_offloads_wanted =
        DEV_TX_OFFLOAD_VLAN_INSERT | DEV_TX_OFFLOAD_IPV4_CKSUM | DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM |
        DEV_TX_OFFLOAD_SCTP_CKSUM | DEV_TX_OFFLOAD_TCP_TSO | DEV_TX_OFFLOAD_UDP_TSO | DEV_TX_OFFLOAD_OUTER_IPV4_CKSUM |
        DEV_TX_OFFLOAD_QINQ_INSERT | DEV_TX_OFFLOAD_VXLAN_TNL_TSO | DEV_TX_OFFLOAD_GRE_TNL_TSO |
        DEV_TX_OFFLOAD_IPIP_TNL_TSO | DEV_TX_OFFLOAD_GENEVE_TNL_TSO | DEV_TX_OFFLOAD_MACSEC_INSERT;
    ...
    ldout(cct, 5) << __func__ << " Port " << int(_port_idx) << ": max_rx_queues " << _dev_info.max_rx_queues
                  << "  max_tx_queues " << _dev_info.max_tx_queues << dendl;
    ...
    if (_num_queues > 1) {
        if (_dev_info.hash_key_size == 40) {
            ...
            lderr(cct) << "Port " << int(_port_idx) << ": We support only 40 or 52 bytes RSS hash keys, "
                       << int(_dev_info.hash_key_size) << " bytes key requested" << dendl;
            return -EINVAL;
        }
        else {
            _rss_key = default_rsskey_40bytes;
    ...
    if (_num_queues > 1) {
        if (_dev_info.reta_size) {
            ...
            ldout(cct, 5) << __func__ << " Port " << int(_port_idx) << ": RSS table size is " << _dev_info.reta_size
                          << dendl;
        }
        else {
    ...
    if ((_dev_info.tx_offload_capa & DEV_TX_OFFLOAD_TCP_TSO) && cct->_conf.get_val<bool>("ms_dpdk_enable_tso")) {
    ...
    if ((retval = rte_eth_dev_configure(_port_idx, _num_queues, _num_queues, &port_conf)) != 0) {
        lderr(cct) << __func__ << " failed to configure port " << (int)_port_idx << " rx/tx queues " << _num_queues
                   << " error " << cpp_strerror(retval) << dendl;
        return retval;
    }
...
public:
    explicit XstatSocketHook(DPDKDevice* dev)
        : dev(dev)
    {}
...
int DPDKDevice::init_port_fini()
{
    ...
    if (_num_queues > 1) set_rss_table();
    ...
    int r = admin_socket->register_command("show_pmd_stats", dfx_hook.get(), "show pmd stats statistics");
    ceph_assert(r == 0);
    r = admin_socket->register_command("show_pmd_xstats", dfx_hook.get(), "show pmd xstats statistics");
    ceph_assert(r == 0);
    return 0;
}
...
bool DPDKQueuePair::init_rx_mbuf_pool()
{
    ...
    if (!_pktmbuf_pool_rx) {
        ldout(cct, 1) << __func__ << " Creating Rx mbuf pool '" << name.c_str() << "' [" << mbufs_per_queue_rx
                      << " mbufs] ..." << dendl;
        ...
        _pktmbuf_pool_rx = rte_mempool_create(name.c_str(),
                                              mbufs_per_queue_rx,
        ...
        const struct rte_memzone* mz = rte_memzone_reserve_aligned(
            mz_name.c_str(), mbuf_data_size * bufs_count, _pktmbuf_pool_rx->socket_id, mz_flags, mbuf_data_size);
        ceph_assert(mz);
        void* m = mz->addr;
        ...
        if (rte_eth_rx_queue_setup(_dev_port_idx,
                                   _qid,
                                   default_ring_size,
...
int DPDKDevice::check_port_link_status()
{
    ...
    while (true) {
        ...
        if (true) {
            if (link.link_status) {
                ldout(cct, 5) << __func__ << " done port " << static_cast<unsigned>(_port_idx) << " link Up - speed "
...
public:
    C_handle_dev_stats(DPDKQueuePair* qp)
        : _qp(qp)
    {}
    void do_request(uint64_t id) { _qp->handle_stats(); }
};

DPDKQueuePair::DPDKQueuePair(CephContext* c, EventCenter* cen, DPDKDevice* dev, uint8_t qid)
    : cct(c)
    , _dev(dev)
    , _dev_port_idx(dev->port_idx())
    , center(cen)
    , _qid(qid)
    , _tx_poller(this)
    , _rx_gc_poller(this)
    , _tx_buf_factory(c, dev, qid)
    , _tx_gc_poller(this)
{
    ...
    static_assert(offsetof(tx_buf, private_end) - offsetof(tx_buf, private_start) <= RTE_PKTMBUF_HEADROOM,
    ...
    plb.add_u64_counter(
        l_dpdk_qp_rx_bad_checksum_errors, "dpdk_receive_bad_checksum_errors", "DPDK received bad checksum packets");
    plb.add_u64_counter(
        l_dpdk_qp_rx_no_memory_errors, "dpdk_receive_no_memory_errors", "DPDK received no memory packets");
    ...
    plb.add_u64_counter(
        l_dpdk_qp_rx_copy_bytes, "dpdk_receive_copy_bytes", "DPDK received copy bytes", NULL, 0, unit_t(UNIT_BYTES));
    plb.add_u64_counter(
        l_dpdk_qp_tx_copy_bytes, "dpdk_send_copy_bytes", "DPDK send copy bytes", NULL, 0, unit_t(UNIT_BYTES));
    ...
    if (!_qid) device_stat_time_fd = center->create_time_event(1000 * 1000, new C_handle_dev_stats(this));
}
...
void DPDKDevice::nic_stats_dump(Formatter* f)
{
    ...
    uint64_t diff_pkts_rx = (stats.ipackets > prev_pkts_rx[_port_idx]) ? (stats.ipackets - prev_pkts_rx[_port_idx]) : 0;
    uint64_t diff_pkts_tx = (stats.opackets > prev_pkts_tx[_port_idx]) ? (stats.opackets - prev_pkts_tx[_port_idx]) : 0;
...
inline std::optional<Packet> DPDKQueuePair::from_mbuf_lro(rte_mbuf* m)
{
    ...
    auto del = std::bind(
        ...
        std::move(_bufs));
    return Packet(_frags.begin(), _frags.end(), make_deleter(std::move(del)));
}
...
bool DPDKQueuePair::rx_gc(bool force)
{
    if (_num_rx_free_segs >= rx_gc_thresh || force) {
        ldout(cct, 10) << __func__ << " free segs " << _num_rx_free_segs << " thresh " << rx_gc_thresh << " free pkts "
                       << _rx_free_pkts.size() << dendl;
        ...
        if (_rx_free_bufs.size()) {
            ...
            // TODO: ceph_assert() in a fast path! Remove me ASAP!
            ceph_assert((_rx_free_pkts.empty() && !_num_rx_free_segs) || (!_rx_free_pkts.empty() && _num_rx_free_segs));
        }
    }
...
void DPDKQueuePair::process_packets(struct rte_mbuf** bufs, uint16_t count)
{
    ...
    for (uint16_t i = 0; i < count; i++) {
        ...
        if ((_dev->_dev_info.rx_offload_capa & DEV_RX_OFFLOAD_VLAN_STRIP) && (m->ol_flags & PKT_RX_VLAN_STRIPPED)) {
...
bool DPDKQueuePair::poll_rx_once()
{
    ...
    uint64_t start = Cycles::rdtsc();
#endif
    uint16_t count = rte_eth_rx_burst(_dev_port_idx, _qid, buf, packet_read_size);
    ...
    if (likely(count > 0)) {
        ...
        if (rx_count > 10000 && tx_count) {
            ldout(cct, 0) << __func__ << " rx count=" << rx_count
                          << " avg rx=" << Cycles::to_nanoseconds(rx_cycles) / rx_count << "ns "
                          << " tx count=" << tx_count << " avg tx=" << Cycles::to_nanoseconds(tx_cycles) / tx_count
                          << "ns" << dendl;
...
DPDKQueuePair::tx_buf_factory::tx_buf_factory(CephContext* c, DPDKDevice* dev, uint8_t qid)
    : cct(c)
{
    ...
    if (!_pool) {
        ldout(cct, 0) << __func__ << " Creating Tx mbuf pool '" << name.c_str() << "' [" << mbufs_per_queue_tx
                      << " mbufs] ..." << dendl;
        //
        ...
        if (rte_eth_tx_queue_setup(
                dev->port_idx(), qid, default_ring_size, rte_eth_dev_socket_id(dev->port_idx()), dev->def_tx_conf()) <
            0) {
...
DPDKQueuePair::tx_buf* DPDKQueuePair::tx_buf::from_packet_zc(CephContext* cct, Packet&& p, DPDKQueuePair& qp)
{
...
build_mbuf_cluster:
    ...
    //
    if (head->nb_segs > max_frags || (p.nr_frags() > 1 && qp.port().is_i40e_device() && i40e_should_linearize(head)) ||
...
void DPDKQueuePair::tx_buf::copy_packet_to_cluster(const Packet& p, rte_mbuf* head)
{
    ...
    while (true) {
        size_t to_copy = std::min(p.frag(cur_frag_idx).size - cur_frag_offset, inline_mbuf_data_size - cur_seg_offset);
...
size_t DPDKQueuePair::tx_buf::copy_one_data_buf(DPDKQueuePair& qp, rte_mbuf*& m, char* data, size_t buf_len)
{
    tx_buf* buf = qp.get_tx_buf();
    if (!buf) {
        return 0;
...
std::unique_ptr<DPDKDevice> create_dpdk_net_device(CephContext* cct, unsigned cores, uint8_t port_idx, bool use_lro,
                                                   bool enable_fc)
{
    ...
    return std::unique_ptr<DPDKDevice>(new DPDKDevice(cct, port_idx, cores, use_lro, enable_fc));
}
