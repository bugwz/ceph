...
inline tcp_sequence make_seq(uint32_t raw)
{
    return tcp_sequence{raw};
}
inline tcp_sequence& operator+=(tcp_sequence& s, int32_t n)
{
    s.raw += n;
    return s;
}
...
inline int32_t operator-(tcp_sequence s, tcp_sequence q)
{
    return s.raw - q.raw;
}
inline bool operator==(tcp_sequence s, tcp_sequence q)
{
    return s.raw == q.raw;
}
inline bool operator!=(tcp_sequence s, tcp_sequence q)
...
private:
    ...
    public:
        C_all_data_acked(tcb* t)
            : tc(t)
        {}
        void do_request(uint64_t fd_or_id) { tc->close_final_cleanup(); }
    };
    ...
    public:
        C_actual_remove_tcb(tcb* t)
            : tc(t->shared_from_this())
        {}
        void do_request(uint64_t r) { delete this; }
    };
    ...
    class tcb : public enable_lw_shared_from_this<tcb>
    {
        ...
        struct send
        {
            ...
            send(CephContext* c)
                : user_queue_space(c, "DPDK::tcp::tcb::user_queue_space", 81920)
            {}
        } _snd;
        struct receive
        {
    ...
    public:
        ...
        void output()
        {
            if (!_poll_active) {
                ...
                _tcp._inet.wait_l2_dst_address(
                    _foreign_ip, Packet(), [tcb](const ethernet_address& dst, Packet p, int r) {
                        if (r == 0) {
        ...
        int16_t get_errno() const { return _errno; }

        tcp_state& state() { return _state; }

        uint64_t peek_sent_available()
        {
            if (!in_state(ESTABLISHED)) return 0;
    ...
    private:
        ...
        {
            if (retransmit_fd) center->delete_time_event(*retransmit_fd);
        ...
        {
            if (persist_fd) center->delete_time_event(*persist_fd);
        ...
        uint16_t local_mss() { return _tcp.get_hw_features().mtu - tcp_hdr_len_min - InetTraits::ip_hdr_len_min; }
        void queue_packet(Packet p) { _packetq.emplace_back(typename InetTraits::l4packet{_foreign_ip, std::move(p)}); }
        void signal_data_received() { manager.notify(fd, EVENT_READABLE); }
        ...
        void do_reset()
        {
            ...
            if (_snd._all_data_acked_fd >= 0) manager.notify(_snd._all_data_acked_fd, EVENT_READABLE);
        }
        void do_time_wait()
        {
        ...
        bool syn_needs_on() { return in_state(SYN_SENT | SYN_RECEIVED); }
        bool fin_needs_on()
        {
            return in_state(FIN_WAIT_1 | CLOSING | LAST_ACK) && _snd.closed && _snd.unsent_len == 0 &&
                   _snd.queued_len == 0;
        }
        bool ack_needs_on() { return !in_state(CLOSED | LISTEN | SYN_SENT); }
        bool foreign_will_not_send() { return in_state(CLOSING | TIME_WAIT | CLOSE_WAIT | LAST_ACK | CLOSED); }
        bool in_state(tcp_state state) { return uint16_t(_state) & uint16_t(state); }
...
public:
    ...
    public:
        explicit connection(lw_shared_ptr<tcb> tcbp)
            : _tcb(std::move(tcbp))
        {
            _tcb->_conn = this;
        }
        ...
        int fd() const { return _tcb->fd; }
        int send(Packet p) { return _tcb->send(std::move(p)); }
        std::optional<Packet> read() { return _tcb->read(); }
        int16_t get_errno() const { return _tcb->get_errno(); }
        void close_read();
        void close_write();
        entity_addr_t remote_addr() const
        {
            ...
            memcpy((void*)&addr.in4_addr().sin_addr.s_addr, &net_ip, sizeof(addr.in4_addr().sin_addr.s_addr));
            addr.set_family(AF_INET);
            return addr;
        }
        uint64_t peek_sent_available() { return _tcb->peek_sent_available(); }
    ...
    public:
        ...
        listener(listener&& x)
            ...
            , _q(std::move(x._q))
        {
            if (_fd >= 0) _tcp._listening[_port] = this;
        }
        ~listener() { abort_accept(); }
        int listen()
        {
            if (_tcp._listening.find(_port) != _tcp._listening.end()) return -EADDRINUSE;
        ...
        int16_t get_errno() const { return _errno; }
        bool full() const { return _q.size() == _q_max_length; }
...
public:
    ...
    void poll_tcb(const ethernet_address& dst, lw_shared_ptr<tcb> tcb) { _poll_tcbs.emplace_back(std::move(tcb), dst); }
...
tcp<InetTraits>::tcp(CephContext* c, inet_type& inet, EventCenter* cen)
    ...
    , _e(_rd())
    , _queue_space(cct, "DPDK::tcp::queue_space", 81920)
{
    int tcb_polled = 0u;
...
template<typename InetTraits> auto tcp<InetTraits>::listen(uint16_t port, size_t queue_length) -> listener
{
    return listener(*this, port, queue_length);
}

template<typename InetTraits> typename tcp<InetTraits>::connection tcp<InetTraits>::connect(const entity_addr_t& addr)
{
    uint16_t src_port;
    connid id;
...
template<typename InetTraits> bool tcp<InetTraits>::forward(forward_hash& out_hash_data, Packet& p, size_t off)
{
...
template<typename InetTraits> void tcp<InetTraits>::received(Packet p, ipaddr from, ipaddr to)
{
...
template<typename InetTraits> void tcp<InetTraits>::send_packet_without_tcb(ipaddr from, ipaddr to, Packet p)
{
    if (_queue_space.get_or_fail(p.len())) {   // drop packets that do not fit the queue
        _inet.wait_l2_dst_address(to, std::move(p), [this, to](const ethernet_address& e_dst, Packet p, int r) mutable {
            if (r == 0) _packetq.emplace_back(ipv4_traits::l4packet{to, std::move(p), e_dst, ip_protocol_num::tcp});
        });
    }
}

template<typename InetTraits> tcp<InetTraits>::connection::~connection()
{
    if (_tcb) {
        _tcb->_conn = nullptr;
        close_read();
...
template<typename InetTraits> tcp<InetTraits>::tcb::~tcb()
{
    if (_delayed_ack_fd) center->delete_time_event(*_delayed_ack_fd);
    if (retransmit_fd) center->delete_time_event(*retransmit_fd);
    if (persist_fd) center->delete_time_event(*persist_fd);
...
template<typename InetTraits> void tcp<InetTraits>::tcb::respond_with_reset(tcp_hdr* rth)
{
    _tcp.respond_with_reset(rth, _local_ip, _foreign_ip);
}

template<typename InetTraits> uint32_t tcp<InetTraits>::tcb::data_segment_acked(tcp_sequence seg_ack)
{
...
template<typename InetTraits> bool tcp<InetTraits>::tcb::segment_acceptable(tcp_sequence seg_seq, unsigned seg_len)
{
...
template<typename InetTraits> Packet tcp<InetTraits>::tcb::get_transmit_packet()
{
...
template<typename InetTraits> void tcp<InetTraits>::tcb::output_one(bool data_retransmit)
{
    if (in_state(CLOSED)) {
        return;
    }
...
template<typename InetTraits> bool tcp<InetTraits>::tcb::is_all_data_acked()
{
...
template<typename InetTraits> std::optional<Packet> tcp<InetTraits>::tcb::read()
{
    std::optional<Packet> p;
    if (_rcv.data.empty()) return p;
...
template<typename InetTraits> int tcp<InetTraits>::tcb::send(Packet p)
{
    // We can not send after the connection is closed
    ceph_assert(!_snd.closed);

    if (in_state(CLOSED)) return -ECONNRESET;
...
template<typename InetTraits> void tcp<InetTraits>::tcb::close()
{
...
template<typename InetTraits> bool tcp<InetTraits>::tcb::should_send_ack(uint16_t seg_len)
{
...
template<typename InetTraits> void tcp<InetTraits>::tcb::clear_delayed_ack()
{
    if (_delayed_ack_fd) {
        center->delete_time_event(*_delayed_ack_fd);
        _delayed_ack_fd.reset();
...
template<typename InetTraits> bool tcp<InetTraits>::tcb::merge_out_of_order()
{
    bool merged = false;
...
template<typename InetTraits> void tcp<InetTraits>::tcb::insert_out_of_order(tcp_sequence seg, Packet p)
{
    _rcv.out_of_order.merge(seg, std::move(p));
}

template<typename InetTraits> void tcp<InetTraits>::tcb::trim_receive_data_after_window()
{
    abort();
}

template<typename InetTraits> void tcp<InetTraits>::tcb::fast_retransmit()
{
    if (!_snd.data.empty()) {
        auto& unacked_seg = _snd.data.front();
...
template<typename InetTraits> void tcp<InetTraits>::tcb::update_rto(clock_type::time_point tx_time)
{
...
template<typename InetTraits> void tcp<InetTraits>::tcb::update_cwnd(uint32_t acked_bytes)
{
...
template<typename InetTraits> void tcp<InetTraits>::tcb::cleanup()
{
    manager.notify(fd, EVENT_READABLE);
    _snd.closed = true;
    _snd.unsent.clear();
...
template<typename InetTraits> tcp_sequence tcp<InetTraits>::tcb::get_isn()
{
...
template<typename InetTraits> std::optional<typename InetTraits::l4packet> tcp<InetTraits>::tcb::get_packet()
{
    _poll_active = false;
    if (_packetq.empty()) {
        output_one();
...
template<typename InetTraits> void tcp<InetTraits>::connection::close_read()
{
    // do nothing
    // _tcb->manager.notify(_tcb->fd, EVENT_READABLE);
}

template<typename InetTraits> void tcp<InetTraits>::connection::close_write()
{
    _tcb->close();
}

template<typename InetTraits> constexpr uint16_t tcp<InetTraits>::tcb::_max_nr_retransmit;

template<typename InetTraits> constexpr std::chrono::microseconds tcp<InetTraits>::tcb::_rto_min;

template<typename InetTraits> constexpr std::chrono::microseconds tcp<InetTraits>::tcb::_rto_max;

template<typename InetTraits> constexpr std::chrono::microseconds tcp<InetTraits>::tcb::_rto_clk_granularity;

template<typename InetTraits> typename tcp<InetTraits>::tcb::isn_secret tcp<InetTraits>::tcb::_isn_secret;
