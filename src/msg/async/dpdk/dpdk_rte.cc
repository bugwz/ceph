...
int eal::init(CephContext* c)
{
    ...
    if (coremaskbit <= 0 || static_cast<uint64_t>(coremaskbit) < c->_conf->ms_async_op_threads) return -EINVAL;
    ...
    t = std::thread([&]() {
        ...
        std::vector<std::vector<char>> args{
            ...
            string2vector("-c"),
            string2vector(c->_conf.get_val<std::string>("ms_dpdk_coremask")),
            string2vector("-n"),
            string2vector(c->_conf->ms_dpdk_memory_channel),
        };
