...
public:
    ...
    double get_ratio() const { return get_initial_size() / (double)get_final_size(); }

    uint64_t get_initial_size() const { return m_init_onwire_size; }

    uint64_t get_final_size() const { return m_onwire_size; }
...
struct rxtx_t
{
    ...
    static rxtx_t create_handler_pair(CephContext* ctx, const CompConnectionMeta& comp_meta,
