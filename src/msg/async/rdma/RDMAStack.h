...
class RDMADispatcher
{
    ...
    ceph::mutex w_lock = ceph::make_mutex("RDMADispatcher::for worker pending list");
    // fixme: lockfree
...
class RDMAWorker : public Worker
{
    ...
    public:
        explicit C_handle_cq_tx(RDMAWorker* w)
            : worker(w)
        {}
...
private:
    ...
    public:
        C_handle_cm_connection(RDMAIWARPConnectedSocketImpl* w)
            : csi(w)
        {}
...
{
public:
    RDMAIWARPServerSocketImpl(CephContext* cct, std::shared_ptr<Infiniband>& ib,
                              std::shared_ptr<RDMADispatcher>& rdma_dispatcher, RDMAWorker* w, entity_addr_t& addr,
                              unsigned addr_slot);
