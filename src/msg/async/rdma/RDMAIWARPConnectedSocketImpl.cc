...
RDMAIWARPConnectedSocketImpl::RDMAIWARPConnectedSocketImpl(CephContext* cct, std::shared_ptr<Infiniband>& ib,
    ...
    : RDMAConnectedSocketImpl(cct, ib, rdma_dispatcher, w)
    , cm_con_handler(new C_handle_cm_connection(this))
{
    status = IDLE;
...
void RDMAIWARPConnectedSocketImpl::handle_cm_connection()
{
    ...
    ldout(cct, 20) << __func__ << " event name: " << rdma_event_str(event->event) << " (cm id: " << cm_id << ")"
                   << dendl;
    ...
    default: ceph_abort_msg("unhandled event"); break;
    }
...
{
    ldout(cct, 30) << __func__ << dendl;
    qp = ib->create_queue_pair(cct, dispatcher->get_tx_cq(), dispatcher->get_rx_cq(), IBV_QPT_RC, cm_id);
    if (!qp) {
        return -1;
    }
