...
RDMAIWARPServerSocketImpl::RDMAIWARPServerSocketImpl(CephContext* cct, std::shared_ptr<Infiniband>& ib,
...
int RDMAIWARPServerSocketImpl::listen(entity_addr_t& sa, const SocketOptions& opt)
{
    ...
    if (rc < 0) {
        rc = -errno;
        ldout(cct, 10) << __func__ << " unable to bind to " << sa.get_sockaddr() << " on port " << sa.get_port() << ": "
    ...
    if (rc < 0) {
        rc = -errno;
        ldout(cct, 10) << __func__ << " unable to listen to " << sa.get_sockaddr() << " on port " << sa.get_port()
                       << ": " << cpp_strerror(errno) << dendl;
        goto err;
    }
