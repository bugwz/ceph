...
RDMAServerSocketImpl::RDMAServerSocketImpl(CephContext* cct, std::shared_ptr<Infiniband>& ib,
...
int RDMAServerSocketImpl::listen(entity_addr_t& sa, const SocketOptions& opt)
{
    ...
    if (server_setup_socket < 0) {
        rc = -errno;
        lderr(cct) << __func__ << " failed to create server socket: " << cpp_strerror(errno) << dendl;
        return rc;
    }
