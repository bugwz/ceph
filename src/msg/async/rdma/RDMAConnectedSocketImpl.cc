...
public:
    C_handle_connection_established(RDMAConnectedSocketImpl* w)
        : csi(w)
    {}
...
public:
    explicit C_handle_connection_read(RDMAConnectedSocketImpl* w)
        : csi(w)
    {}
    void do_request(uint64_t fd) final
    {
...
{
    ldout(cct, 20) << __func__ << " nonblock:" << opts.nonblock << ", nodelay:" << opts.nodelay
                   << ", rbuf_size: " << opts.rcbuf_size << dendl;
...
ssize_t RDMAConnectedSocketImpl::read(char* buf, size_t len)
{
    ...
    if (is_server && connected == 0) {
        ldout(cct, 20) << __func__ << " we do not need last handshake, QP: " << local_qpn << " peer QP: " << peer_qpn
                       << dendl;
...
void RDMAConnectedSocketImpl::buffer_prefetch(void)
{
    ...
    for (size_t i = 0; i < cqe.size(); ++i) {
        ...
        if (chunk->get_size() == 0) {
            ...
            ldout(cct, 25) << __func__ << " buffers add a chunk: " << chunk->get_offset() << ":" << chunk->get_bound()
                           << dendl;
        }
    }
...
ssize_t RDMAConnectedSocketImpl::read_buffers(char* buf, size_t len)
{
    ...
    while (pchunk != buffers.end()) {
        ...
        read_size += tmp;
        ldout(cct, 25) << __func__ << " read chunk " << *pchunk << " bytes length" << tmp
                       << " offset: " << (*pchunk)->get_offset() << " ,bound: " << (*pchunk)->get_bound() << dendl;
...
size_t RDMAConnectedSocketImpl::tx_copy_chunk(std::vector<Chunk*>& tx_buffers, size_t req_copy_len,
...
int RDMAConnectedSocketImpl::post_work_request(std::vector<Chunk*>& tx_buffers)
{
    ...
    while (current_buffer != tx_buffers.end()) {
        ...
        ldout(cct, 25) << __func__ << " sending buffer: " << *current_buffer << " length: " << isge[current_sge].length
                       << dendl;
...
void RDMAConnectedSocketImpl::fin()
{
    ...
    if (ibv_post_send(qp->get_qp(), &wr, &bad_tx_work_request)) {
        ldout(cct, 1) << __func__ << " failed to send message="
                      << " ibv_post_send failed(most probably should be peer not ready): " << cpp_strerror(errno)
                      << dendl;
        worker->perf_logger->inc(l_msgr_rdma_tx_failed);
        return;
    }
...
void RDMAConnectedSocketImpl::cleanup()
{
    if (read_handler && tcp_fd >= 0) {
        ...
        worker->center.submit_to(
            ...
            [this]() { worker->center.delete_file_event(tcp_fd, EVENT_READABLE | EVENT_WRITABLE); },
            false);
        delete read_handler;
...
void RDMAConnectedSocketImpl::set_accept_fd(int sd)
{
    ...
    worker->center.submit_to(
        worker->center.get_id(),
        [this]() { worker->center.create_file_event(tcp_fd, EVENT_READABLE, read_handler); },
        true);
}
