...
Port::Port(CephContext* cct, struct ibv_context* ictxt, uint8_t ipn)
    : ctxt(ictxt)
    , port_num(ipn)
    , gid_idx(cct->_conf.get_val<int64_t>("ms_async_rdma_gid_idx"))
{
    ...
    ldout(cct, 1) << __func__ << " looking for local GID " << (cct->_conf->ms_async_rdma_local_gid) << " of type "
                  << (cct->_conf->ms_async_rdma_roce_ver) << dendl;
    ...
    for (gid_idx = 0; gid_idx < port_attr.gid_tbl_len; gid_idx++) {
        ...
        if (r) {
            lderr(cct) << __func__ << " query gid of port " << port_num << " index " << gid_idx << " failed  "
                       << cpp_strerror(errno) << dendl;
            ceph_abort();
        }
        r = ibv_exp_query_gid_attr(ctxt, port_num, gid_idx, &gid_attr);
        if (r) {
            lderr(cct) << __func__ << " query gid attributes of port " << port_num << " index " << gid_idx
...
Device::Device(CephContext* cct, ibv_device* ib_dev)
    : device(ib_dev)
    , active_port(nullptr)
{
    ceph_assert(device);
...
Device::Device(CephContext* cct, struct ibv_context* ib_ctx)
    : device(ib_ctx->device)
    , active_port(nullptr)
{
    ceph_assert(device);
    ctxt = ib_ctx;
...
void Device::binding_port(CephContext* cct, int port_num)
{
    ...
    for (uint8_t port_id = 1; port_id <= port_cnt; ++port_id) {
        ...
        if (port_id == port_num && port->get_port_attr()->state == IBV_PORT_ACTIVE) {
            ...
            ldout(cct, 10) << __func__ << " port " << port_id
                           << " is not what we want. state: " << ibv_port_state_str(port->get_port_attr()->state)
                           << dendl;
            delete port;
        }
    }
...
Infiniband::QueuePair::QueuePair(CephContext* c, Infiniband& infiniband, ibv_qp_type type, int port, ibv_srq* srq,
...
int Infiniband::QueuePair::modify_qp_to_rtr(void)
{
    ...
    ldout(cct, 20) << __func__ << " Choosing gid_index " << (int)qpa.ah_attr.grh.sgid_index << ", sl "
                   << (int)qpa.ah_attr.sl << dendl;
...
int Infiniband::QueuePair::init()
{
    ...
    if (!cct->_conf->ms_async_rdma_cm) {
        ...
        if (rdma_create_qp(cm_id, pd, &qpia)) {
            lderr(cct) << __func__ << " failed to create queue pair with rdmacm library" << cpp_strerror(errno)
                       << dendl;
            return -1;
        }
        qp = cm_id->qp;
    ...
    if (!srq) {
        ...
        ldout(cct, 20) << __func__ << " initialize no SRQ Queue Pair, qp number: " << qp->qp_num << " post SQ WR "
                       << rq_wrs << dendl;
    }
    return 0;
}
...
int Infiniband::QueuePair::recv_cm_meta(CephContext* cct, int socket_fd)
{
    ...
    if (r < 0) {
        r = -errno;
        lderr(cct) << __func__ << " got error " << r << ": " << cpp_strerror(r) << dendl;
    }
        ...
        sscanf(msg,
               "%hx:%x:%x:%x:%s",
               &(peer_cm_meta.lid),
               &(peer_cm_meta.local_qpn),
               &(peer_cm_meta.psn),
               &(peer_cm_meta.peer_qpn),
               gid);
...
retry:
    ...
    sprintf(msg,
            "%04x:%08x:%08x:%08x:%s",
            local_cm_meta.lid,
            local_cm_meta.local_qpn,
            local_cm_meta.psn,
            local_cm_meta.peer_qpn,
            gid);
    ...
    if ((size_t)r != sizeof(msg)) {
        ...
        if (r < 0)
            lderr(cct) << __func__ << " send returned error " << errno << ": " << cpp_strerror(errno) << dendl;
        else
...
int Infiniband::QueuePair::to_dead()
{
    ...
    ldout(cct, 20) << __func__ << " trigger error state Queue Pair, qp number: " << local_cm_meta.local_qpn
                   << " Beacon sent " << dendl;
    dead = true;
...
int Infiniband::QueuePair::get_remote_qp_number(uint32_t* rqp) const
{
    ...
    if (r) {
        lderr(cct) << __func__ << " failed to query qp: " << cpp_strerror(errno) << dendl;
        return -1;
    }
...
int Infiniband::QueuePair::get_state() const
{
    ...
    if (r) {
        lderr(cct) << __func__ << " failed to get state: " << cpp_strerror(errno) << dendl;
        return -1;
    }
    return qpa.qp_state;
}
...
Infiniband::CompletionChannel::~CompletionChannel()
{
    if (channel) {
        ...
        if (r < 0) lderr(cct) << __func__ << " failed to destroy cc: " << cpp_strerror(errno) << dendl;
        ceph_assert(r == 0);
    }
}
...
int Infiniband::CompletionChannel::init()
{
    ...
    if (!channel) {
        lderr(cct) << __func__ << " failed to create receive completion channel: " << cpp_strerror(errno) << dendl;
        return -1;
    }
...
bool Infiniband::CompletionChannel::get_cq_event()
{
    ...
    if (ibv_get_cq_event(channel, &cq, &ev_ctx)) {
        if (errno != EAGAIN && errno != EINTR)
            lderr(cct) << __func__ << " failed to retrieve CQ event: " << cpp_strerror(errno) << dendl;
        return false;
    }
...
Infiniband::CompletionQueue::~CompletionQueue()
{
    if (cq) {
        ...
        if (r < 0) lderr(cct) << __func__ << " failed to destroy cq: " << cpp_strerror(errno) << dendl;
        ceph_assert(r == 0);
    }
}
...
int Infiniband::CompletionQueue::init()
{
    ...
    if (!cq) {
        lderr(cct) << __func__ << " failed to create receive completion queue: " << cpp_strerror(errno) << dendl;
        return -1;
    }
...
int Infiniband::CompletionQueue::rearm_notify(bool solicite_only)
{
    ...
    if (r < 0) lderr(cct) << __func__ << " failed to notify cq: " << cpp_strerror(errno) << dendl;
    return r;
}
...
int Infiniband::CompletionQueue::poll_cq(int num_entries, ibv_wc* ret_wc_array)
{
    ...
    if (r < 0) {
        lderr(cct) << __func__ << " poll_completion_queue occur met error: " << cpp_strerror(errno) << dendl;
        return -1;
    }
    return r;
}
...
Infiniband::MemoryManager::Chunk::Chunk(ibv_mr* m, uint32_t bytes, char* buffer, uint32_t offset, uint32_t bound,
                                        uint32_t lkey, QueuePair* qp)
    : mr(m)
    , qp(qp)
    , lkey(lkey)
    , bytes(bytes)
...
bool Infiniband::MemoryManager::MemPoolContext::can_alloc(unsigned nbufs)
{
    ...
    if (n_bufs_allocated + nbufs > (unsigned)manager->cct->_conf->ms_async_rdma_receive_buffers) {
        lderr(manager->cct) << __func__ << " WARNING: OUT OF RX BUFFERS: allocated: " << n_bufs_allocated
                            << " requested: " << nbufs
...
char* Infiniband::MemoryManager::PoolAllocator::malloc(const size_type block_size)
{
    ...
    if (!minfo) {
        lderr(cct) << __func__ << " failed to allocate " << chunk_buffer_number
                   << " buffers "
                      " Its block size is : "
    ...
    minfo->mr =
        ibv_reg_mr(manager->pd->pd, minfo->chunks, block_size, IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE);
    if (minfo->mr == NULL) {
        lderr(cct) << __func__ << " failed to do rdma memory registration " << block_size
                   << " bytes. "
...
void* Infiniband::MemoryManager::huge_pages_malloc(size_t size)
{
    ...
    char* ptr = (char*)mmap(
        NULL, real_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_POPULATE | MAP_HUGETLB, -1, 0);
    if (ptr == MAP_FAILED) {
...
void Infiniband::verify_prereq(CephContext* cct)
{
    ...
    ldout(cct, 20) << __func__
                   << " ms_async_rdma_enable_hugepage value is: " << cct->_conf->ms_async_rdma_enable_hugepage << dendl;
    if (cct->_conf->ms_async_rdma_enable_hugepage) {
        rc = setenv("RDMAV_HUGEPAGES_SAFE", "1", 1);
        ldout(cct, 0) << __func__ << " RDMAV_HUGEPAGES_SAFE is set as: " << getenv("RDMAV_HUGEPAGES_SAFE") << dendl;
        if (rc) {
            lderr(cct) << __func__
                       << " failed to export RDMA_HUGEPAGES_SAFE. On RDMA must be exported before using huge pages. "
                          "Application aborts."
                       << dendl;
            ceph_abort();
        }
    ...
    if (rc) {
        lderr(cct) << __func__
                   << " failed to call ibv_for_init(). On RDMA must be called before fork. Application aborts."
                   << dendl;
        ceph_abort();
    }
    ...
    if (limit.rlim_cur != RLIM_INFINITY || limit.rlim_max != RLIM_INFINITY) {
        lderr(cct) << __func__
                   << "!!! WARNING !!! For RDMA to work properly user memlock (ulimit -l) must be big enough to allow "
                      "large amount of registered memory."
...
void Infiniband::init()
{
    ...
    if (cct->_conf->ms_async_rdma_receive_buffers > 0 &&
        ...
        lderr(cct) << __func__ << " rdma_receive_queue_len (" << rx_queue_len << ") > ms_async_rdma_receive_buffers("
 ...
 */
Infiniband::QueuePair* Infiniband::create_queue_pair(CephContext* cct, CompletionQueue* tx, CompletionQueue* rx,
                                                     ibv_qp_type type, struct rdma_cm_id* cm_id)
{
    Infiniband::QueuePair* qp =
        new QueuePair(cct, *this, type, ib_physical_port, srq, tx, rx, tx_queue_len, rx_queue_len, cm_id);
    if (qp->init()) {
        delete qp;
        return NULL;
    }
...
int Infiniband::post_chunks_to_rq(int rq_wr_num, QueuePair* qp)
{
    ...
    while (i < rq_wr_num) {
        ...
        if (chunk == nullptr) {
            lderr(cct) << __func__ << " WARNING: out of memory. Request " << rq_wr_num << " rx buffers. Only get " << i
                       << " rx buffers." << dendl;
            if (i == 0) {
...
{
    Infiniband::CompletionQueue* cq = new Infiniband::CompletionQueue(cct, *this, CQ_DEPTH, cc);
    if (cq->init()) {
        delete cq;
        return NULL;
    }
...
{
    ldout(cct, 20) << __func__ << " destroy Queue Pair, qp number: " << qp->qp_num << " left SQ WR "
                   << recv_queue.size() << dendl;
    if (qp) {
...
{
    static const char* lookup[] = {"SUCCESS",          "LOC_LEN_ERR",       "LOC_QP_OP_ERR",     "LOC_EEC_OP_ERR",
                                   "LOC_PROT_ERR",     "WR_FLUSH_ERR",      "MW_BIND_ERR",       "BAD_RESP_ERR",
                                   "LOC_ACCESS_ERR",   "REM_INV_REQ_ERR",   "REM_ACCESS_ERR",    "REM_OP_ERR",
                                   "RETRY_EXC_ERR",    "RNR_RETRY_EXC_ERR", "LOC_RDD_VIOL_ERR",  "REM_INV_RD_REQ_ERR",
                                   "REM_ABORT_ERR",    "INV_EECN_ERR",      "INV_EEC_STATE_ERR", "FATAL_ERR",
                                   "RESP_TIMEOUT_ERR", "GENERAL_ERR"};
