...
public:
    ...
    void stop();
    int bind(const entity_addrvec_t& bind_addrs, const std::set<int>& avoid_ports, entity_addrvec_t* bound_addrs);
    void start();
    void accept();
};
...
public:
     ...
     */
    AsyncMessenger(CephContext* cct, entity_name_t name, const std::string& type, std::string mname, uint64_t _nonce);
    ...
    int get_dispatch_queue_len() override { return dispatch_queue.get_queue_len(); }

    double get_dispatch_queue_max_age(utime_t now) override { return dispatch_queue.get_max_age(now); }
    /** @} Accessors */
    ...
    int bind(const entity_addr_t& bind_addr, std::optional<entity_addrvec_t> public_addrs = std::nullopt) override;
    int rebind(const std::set<int>& avoid_ports) override;
    int bindv(const entity_addrvec_t& bind_addrs, std::optional<entity_addrvec_t> public_addrs = std::nullopt) override;
     ...
     * @{
     */
    ConnectionRef connect_to(int type, const entity_addrvec_t& addrs, bool anon, bool not_local_dest = false) override;
    ConnectionRef get_loopback_connection() override;
    void mark_down(const entity_addr_t& addr) override { mark_down_addrs(entity_addrvec_t(addr)); }
...
public:
    ...
    void add_accept(Worker* w, ConnectedSocket cli_socket, const entity_addr_t& listen_addr,
                    const entity_addr_t& peer_addr);
    NetworkStack* get_stack() { return stack; }
    ...
    __u32 get_global_seq(__u32 old = 0)
    {
        ...
        if (old > global_seq) global_seq = old;
