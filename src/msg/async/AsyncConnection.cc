...
{
    return *_dout << "-- " << async_msgr->get_myaddrs() << " >> " << *peer_addrs << " conn(" << this
                  << (msgr2 ? " msgr2=" : " legacy=") << protocol.get() << " "
                  << ceph_con_mode_name(protocol->auth_meta->con_mode) << " :" << port << " s=" << get_state_name(state)
                  << " l=" << policy.lossy << ").";
}
...
public:
    explicit C_time_wakeup(AsyncConnectionRef c)
        : conn(c)
    {}
    void do_request(uint64_t fd_or_id) override { conn->wakeup_from(fd_or_id); }
};
...
public:
    explicit C_handle_read(AsyncConnectionRef c)
        : conn(c)
    {}
...
public:
    explicit C_handle_write(AsyncConnectionRef c)
        : conn(c)
    {}
...
public:
    explicit C_handle_write_callback(AsyncConnectionRef c)
        : conn(c)
    {}
...
public:
    explicit C_clean_handler(AsyncConnectionRef c)
        : conn(c)
    {}
...
public:
    explicit C_tick_wakeup(AsyncConnectionRef c)
        : conn(c)
    {}
    void do_request(uint64_t fd_or_id) override { conn->tick(fd_or_id); }
};


AsyncConnection::AsyncConnection(CephContext* cct, AsyncMessenger* m, DispatchQueue* q, Worker* w, bool m2, bool local)
    : Connection(cct, m)
    , delay_state(NULL)
    , async_msgr(m)
    , conn_id(q->get_id())
...
{
    if (!delay_state) {
        async_msgr->cct->_conf.with_val<std::string>("ms_inject_delay_type", [this](const std::string& s) {
...
ssize_t AsyncConnection::read(unsigned len, char* buffer, std::function<void(char*, ssize_t)> callback)
{
...
ssize_t AsyncConnection::read_until(unsigned len, char* p)
{
    ...
    if (left > (uint64_t)recv_max_prefetch) {
        ...
        do {
            ...
            ldout(async_msgr->cct, 25) << __func__ << " read_bulk recv_end is " << recv_end << " left is " << left
                                       << " got " << r << dendl;
            if (r < 0) {
                ldout(async_msgr->cct, 1) << __func__ << " read failed" << dendl;
                return -1;
    ...
    ldout(async_msgr->cct, 25) << __func__ << " need len " << len << " remaining " << len - state_offset << " bytes"
                               << dendl;
...
again:
    ...
    if (nread < 0) {
        if (nread == -EAGAIN) {
            ...
            ldout(async_msgr->cct, 1) << __func__ << " reading from fd=" << cs.fd() << " : " << nread << " "
...
ssize_t AsyncConnection::write(ceph::buffer::list& bl, std::function<void(ssize_t)> callback, bool more)
{
...
ssize_t AsyncConnection::_try_send(bool more)
{
    ...
    ldout(async_msgr->cct, 10) << __func__ << " sent bytes " << r << " remaining bytes " << outgoing_bl.length()
                               << dendl;
...
void AsyncConnection::inject_delay()
{
    if (async_msgr->cct->_conf->ms_inject_internal_delays) {
        ldout(async_msgr->cct, 10) << __func__ << " sleep for " << async_msgr->cct->_conf->ms_inject_internal_delays
                                   << dendl;
        utime_t t;
...
void AsyncConnection::process()
{
    ...
    case STATE_CONNECTING:
    {
        ...
        if (async_msgr->cct->_conf->mon_use_min_delay_socket) {
            if (async_msgr->get_mytype() == CEPH_ENTITY_TYPE_MON && peer_is_mon()) {
...
void AsyncConnection::accept(ConnectedSocket socket, const entity_addr_t& listen_addr, const entity_addr_t& peer_addr)
{
    ldout(async_msgr->cct, 10) << __func__ << " sd=" << socket.fd() << " listen_addr " << listen_addr << " peer_addr "
                               << peer_addr << dendl;
...
int AsyncConnection::send_message(Message* m)
{
    ...
    lgeneric_subdout(async_msgr->cct, ms, 1) << "-- " << async_msgr->get_myaddrs() << " --> " << get_peer_addrs()
                                             << " -- " << *m << " -- " << m << " con " << this << dendl;

    if (is_blackhole()) {
        lgeneric_subdout(async_msgr->cct, ms, 0)
            << __func__ << ceph_entity_type_name(peer_type) << " blackhole " << *m << dendl;
        m->put();
        return 0;
    }
...
void AsyncConnection::fault()
{
    ...
    if (delay_state) delay_state->flush();
...
void AsyncConnection::DelayedDelivery::do_request(uint64_t id)
{
    Message* m = nullptr;
    {
        ...
        if (stop_dispatch) return;
        if (delay_queue.empty()) return;
        m = delay_queue.front();
...
void AsyncConnection::DelayedDelivery::discard()
{
    ...
    center->submit_to(
        ...
        [this]() mutable {
            ...
            for (auto i : register_time_events) center->delete_time_event(i);
            register_time_events.clear();
            stop_dispatch = false;
        },
        true);
...
void AsyncConnection::tick(uint64_t id)
{
    ...
    if (!is_connected()) {
        if (connect_timeout_us <=
            (uint64_t)std::chrono::duration_cast<std::chrono::microseconds>(now - last_connect_started).count()) {
            ldout(async_msgr->cct, 1) << __func__ << " see no progress in more than " << connect_timeout_us
                                      << " us during connecting to " << target_addr << ", fault." << dendl;
            protocol->fault();
        ...
        if (inactive_timeout_us < (uint64_t)idle_period) {
            ldout(async_msgr->cct, 1) << __func__ << " idle (" << idle_period << ") for more than "
                                      << inactive_timeout_us << " us, fault." << dendl;
            protocol->fault();
