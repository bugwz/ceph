...
int KqueueDriver::restore_events()
{
    ...
    for (i = 0; i < size; i++) {
        int num = 0;
        if (sav_events[i].mask == 0) continue;
        ldout(cct, 30) << __func__ << " restore kqfd = " << kqfd << " fd = " << i << " mask " << sav_events[i].mask
                       << dendl;
        if (sav_events[i].mask & EVENT_READABLE) EV_SET(&ke[num++], i, EVFILT_READ, EV_ADD, 0, 0, NULL);
        if (sav_events[i].mask & EVENT_WRITABLE) EV_SET(&ke[num++], i, EVFILT_WRITE, EV_ADD, 0, 0, NULL);
        if (num) {
...
int KqueueDriver::test_thread_change(const char* funcname)
{
    ...
    if (kqfd == -1) {
        ...
        if (kqfd < 0) {
            lderr(cct) << funcname << " unable to do kqueue: " << cpp_strerror(errno) << dendl;
            return -errno;
        }
        if (restore_events() < 0) {
            lderr(cct) << funcname << " unable restore all events " << cpp_strerror(errno) << dendl;
            return -errno;
        }
    }
    return 0;
...
int KqueueDriver::init(EventCenter* c, int nevent)
{
    ...
    if (!res_events) {
        lderr(cct) << __func__ << " unable to malloc memory: " << cpp_strerror(errno) << dendl;
        return -ENOMEM;
    }
...
int KqueueDriver::add_event(int fd, int cur_mask, int add_mask)
{
    ...
    ldout(cct, 30) << __func__ << " add event kqfd = " << kqfd << " fd = " << fd << " cur_mask = " << cur_mask
                   << " add_mask = " << add_mask << dendl;

    int r = test_thread_change(__func__);
    if (r < 0) return r;

    if (add_mask & EVENT_READABLE) EV_SET(&ke[num++], fd, EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0, NULL);
    if (add_mask & EVENT_WRITABLE) EV_SET(&ke[num++], fd, EVFILT_WRITE, EV_ADD | EV_CLEAR, 0, 0, NULL);

    if (num) {
        if (kevent(kqfd, ke, num, NULL, 0, KEVENT_NOWAIT) == -1) {
            lderr(cct) << __func__ << " unable to add event: " << cpp_strerror(errno) << dendl;
            return -errno;
        }
    }
    // keep what we set
    if (fd >= sav_max) resize_events(sav_max + 5000);
...
int KqueueDriver::del_event(int fd, int cur_mask, int del_mask)
{
    ...
    ldout(cct, 30) << __func__ << " delete event kqfd = " << kqfd << " fd = " << fd << " cur_mask = " << cur_mask
                   << " del_mask = " << del_mask << dendl;

    int r = test_thread_change(__func__);
    if (r < 0) return r;

    if (mask & EVENT_READABLE) EV_SET(&ke[num++], fd, EVFILT_READ, EV_DELETE, 0, 0, NULL);
    if (mask & EVENT_WRITABLE) EV_SET(&ke[num++], fd, EVFILT_WRITE, EV_DELETE, 0, 0, NULL);

    if (num) {
        int r = 0;
        if ((r = kevent(kqfd, ke, num, NULL, 0, KEVENT_NOWAIT)) < 0) {
            lderr(cct) << __func__ << " kevent: delete fd=" << fd << " mask=" << mask << " failed."
...
int KqueueDriver::event_wait(std::vector<FiredFileEvent>& fired_events, struct timeval* tvp)
{
    ...
    if (tvp != NULL) {
        ...
        ldout(cct, 30) << __func__ << " event_wait: "
                       << " NULL" << dendl;
    ...
    if (retval < 0) {
        lderr(cct) << __func__ << " kqueue error: " << cpp_strerror(errno) << dendl;
        return -errno;
    }
    else if (retval == 0) {
        ldout(cct, 5) << __func__ << " Hit timeout(" << timeout.tv_sec << " sec " << timeout.tv_nsec << " nsec"
                      << ")." << dendl;
    }
    else {
        int j;
