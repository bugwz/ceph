...
int NetHandler::create_socket(int domain, bool reuse_addr)
{
    ...
    if (reuse_addr) {
        ...
        if (::setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (SOCKOPT_VAL_TYPE)&on, sizeof(on)) == -1) {
            r = ceph_sock_errno();
            lderr(cct) << __func__ << " setsockopt SO_REUSEADDR failed: " << strerror(r) << dendl;
...
int NetHandler::set_nonblock(int sd)
{
    ...
    if (r) {
        lderr(cct) << __func__ << " ioctlsocket(FIONBIO) failed: " << r << " " << WSAGetLastError() << dendl;
        return -r;
    }
#else
    int flags;
...
void NetHandler::set_priority(int sd, int prio, int domain)
{
    ...
    switch (domain) {
    case AF_INET: r = ::setsockopt(sd, IPPROTO_IP, IP_TOS, (SOCKOPT_VAL_TYPE)&iptos, sizeof(iptos)); break;
    case AF_INET6: r = ::setsockopt(sd, IPPROTO_IPV6, IPV6_TCLASS, (SOCKOPT_VAL_TYPE)&iptos, sizeof(iptos)); break;
    default:
...
int NetHandler::generic_connect(const entity_addr_t& addr, const entity_addr_t& bind_addr, bool nonblock)
{
...

    {
        ...
        if (cct->_conf->ms_bind_before_connect && (!addr.is_blank_ip())) {
            ...
            if (ret < 0) {
                ret = ceph_sock_errno();
                ldout(cct, 2) << __func__ << " client bind error "
