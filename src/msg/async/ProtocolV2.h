...
private:
    ...
    Ct<ProtocolV2>* read(CONTINUATION_RXBPTR_TYPE<ProtocolV2>& next, rx_buffer_t&& buffer);
    template<class F> Ct<ProtocolV2>* write(const std::string& desc, CONTINUATION_TYPE<ProtocolV2>& next, F& frame);
    Ct<ProtocolV2>* write(const std::string& desc, CONTINUATION_TYPE<ProtocolV2>& next, ceph::bufferlist& buffer);

    template<class F> bool append_frame(F& frame);
...
private:
    ...
    Ct<ProtocolV2>* reuse_connection(const AsyncConnectionRef& existing, ProtocolV2* exproto);
