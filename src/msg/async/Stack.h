...
public:
    ...
    /// peer failure.
    void shutdown() { return _csi->shutdown(); }
    ...
    /// Get file descriptor
    int fd() const { return _csi->fd(); }

    void set_priority(int sd, int prio, int domain) { _csi->set_priority(sd, prio, domain); }

    explicit operator bool() const { return _csi.get(); }
};
/// @}
...
public:
    ...
    unsigned get_addr_slot() { return _ssi->addr_slot; }
...
public:
        ...
        , center(c)
    {
        ...
        plb.add_u64_counter(
            l_msgr_recv_bytes, "msgr_recv_bytes", "Network received bytes", NULL, 0, unit_t(UNIT_BYTES));
        ...
        plb.add_time(
            l_msgr_running_fast_dispatch_time, "msgr_running_fast_dispatch_time", "The total time of fast dispatch");
        ...
        plb.add_u64_counter(l_msgr_recv_encrypted_bytes,
                            "msgr_recv_encrypted_bytes",
                            "Network received encrypted bytes",
                            NULL,
                            0,
                            unit_t(UNIT_BYTES));
        plb.add_u64_counter(l_msgr_send_encrypted_bytes,
                            "msgr_send_encrypted_bytes",
                            "Network sent encrypted bytes",
                            NULL,
                            0,
                            unit_t(UNIT_BYTES));
        ...
        std::string labels = ceph::perf_counters::key_create(name_prefix, {{"id", std::to_string(id)}});
        PerfCountersBuilder plb_labeled(cct, labels, l_msgr_labeled_first, l_msgr_labeled_last);

        plb_labeled.add_u64_counter(l_msgr_connection_ready_timeouts,
                                    "msgr_connection_ready_timeouts",
                                    "Number of not yet ready connections declared as dead",
                                    NULL,
                                    PerfCountersBuilder::PRIO_USEFUL);
        plb_labeled.add_u64_counter(l_msgr_connection_idle_timeouts,
    ...
    virtual int listen(entity_addr_t& addr, unsigned addr_slot, const SocketOptions& opts, ServerSocket*) = 0;
...
public:
    ...
    Worker* get_worker(unsigned worker_id) { return workers[worker_id]; }
    void drain();
    unsigned get_num_worker() const { return workers.size(); }
