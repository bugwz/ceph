...
struct nonce_t
{
    ...
    bool operator==(const nonce_t& rhs) const { return !memcmp(this, &rhs, sizeof(*this)); }
...
public:
    AES128GCM_OnWireTxHandler(CephContext* const cct, const key_t& key, const nonce_t& nonce, bool new_nonce_format)
        : cct(cct)
        ...
        , new_nonce_format(new_nonce_format)
    {
        ...
        if (1 != EVP_EncryptInit_ex(ectx.get(), EVP_aes_128_gcm(), nullptr, nullptr, nullptr)) {
            throw std::runtime_error("EVP_EncryptInit_ex failed");
        }

        if (1 != EVP_EncryptInit_ex(ectx.get(), nullptr, nullptr, key.data(), nullptr)) {
            throw std::runtime_error("EVP_EncryptInit_ex failed");
        }
    }
...
void AES128GCM_OnWireTxHandler::reset_tx_handler(const uint32_t* first, const uint32_t* last)
{
    ...
    if (1 !=
        EVP_EncryptInit_ex(ectx.get(), nullptr, nullptr, nullptr, reinterpret_cast<const unsigned char*>(&nonce))) {
        throw std::runtime_error("EVP_EncryptInit_ex failed");
    }
...
void AES128GCM_OnWireTxHandler::authenticated_encrypt_update(const ceph::bufferlist& plaintext)
{
    ceph_assert(buffer.get_append_buffer_unused_tail_length() >= plaintext.length());
    ...
    ldout(cct, 15) << __func__ << " plaintext.length()=" << plaintext.length() << " buffer.length()=" << buffer.length()
                   << dendl;
}
...
{
    int final_len = 0;
    ceph_assert(buffer.get_append_buffer_unused_tail_length() == AESGCM_BLOCK_LEN);
    auto filler = buffer.append_hole(AESGCM_BLOCK_LEN);
    if (1 != EVP_EncryptFinal_ex(ectx.get(), reinterpret_cast<unsigned char*>(filler.c_str()), &final_len)) {
        throw std::runtime_error("EVP_EncryptFinal_ex failed");
    }
    ceph_assert_always(final_len == 0);

    static_assert(AESGCM_BLOCK_LEN == AESGCM_TAG_LEN);
    if (1 != EVP_CIPHER_CTX_ctrl(ectx.get(), EVP_CTRL_GCM_GET_TAG, AESGCM_TAG_LEN, filler.c_str())) {
...
public:
    AES128GCM_OnWireRxHandler(CephContext* const cct, const key_t& key, const nonce_t& nonce, bool new_nonce_format)
        : ectx(EVP_CIPHER_CTX_new(), EVP_CIPHER_CTX_free)
        , nonce(nonce)
        , new_nonce_format(new_nonce_format)
    {
        ...
        if (1 != EVP_DecryptInit_ex(ectx.get(), EVP_aes_128_gcm(), nullptr, nullptr, nullptr)) {
            throw std::runtime_error("EVP_DecryptInit_ex failed");
        }

        if (1 != EVP_DecryptInit_ex(ectx.get(), nullptr, nullptr, key.data(), nullptr)) {
            throw std::runtime_error("EVP_DecryptInit_ex failed");
        }
    }

    ~AES128GCM_OnWireRxHandler() override { ::TOPNSPC::crypto::zeroize_for_security(&nonce, sizeof(nonce)); }
...
{
    if (1 !=
        EVP_DecryptInit_ex(ectx.get(), nullptr, nullptr, nullptr, reinterpret_cast<const unsigned char*>(&nonce))) {
        throw std::runtime_error("EVP_DecryptInit_ex failed");
    }
...
void AES128GCM_OnWireRxHandler::authenticated_decrypt_update(ceph::bufferlist& bl)
{
...
void AES128GCM_OnWireRxHandler::authenticated_decrypt_update_final(ceph::bufferlist& bl)
{
    ...
    if (1 != EVP_CIPHER_CTX_ctrl(ectx.get(), EVP_CTRL_GCM_SET_TAG, AESGCM_TAG_LEN, auth_tag.c_str())) {
        throw std::runtime_error("EVP_CIPHER_CTX_ctrl failed");
    }
...
ceph::crypto::onwire::rxtx_t ceph::crypto::onwire::rxtx_t::create_handler_pair(CephContext* cct,
                                                                               const AuthConnectionMeta& auth_meta,
                                                                               bool new_nonce_format, bool crossed)
{
    if (auth_meta.is_mode_secure()) {
        ceph_assert_always(auth_meta.connection_secret.length() >= sizeof(key_t) + 2 * sizeof(nonce_t));
        ...
        return {std::make_unique<AES128GCM_OnWireRxHandler>(cct, key, crossed ? tx_nonce : rx_nonce, new_nonce_format),
                std::make_unique<AES128GCM_OnWireTxHandler>(cct, key, crossed ? rx_nonce : tx_nonce, new_nonce_format)};
    }
    else {
