...
public:
    C_handle_notify(EventCenter* c, CephContext* cc)
        : center(c)
        , cct(cc)
    {}
        ...
        do {
            ...
            if (r < 0) {
                if (ceph_sock_errno() != EAGAIN)
                    ldout(cct, 1) << __func__ << " read notify pipe failed: " << cpp_strerror(ceph_sock_errno())
                                  << dendl;
            }
        } while (r > 0);
    }
...
int EventCenter::create_file_event(int fd, int mask, EventCallbackRef ctxt)
{
    ...
    ldout(cct, 20) << __func__ << " create event started fd=" << fd << " mask=" << mask << " original mask is "
                   << event->mask << dendl;
    ...
    if (r < 0) {
        ...
        lderr(cct) << __func__ << " add event failed, ret=" << r << " fd=" << fd << " mask=" << mask
    ...
    ldout(cct, 20) << __func__ << " create event end fd=" << fd << " mask=" << mask << " current mask is "
                   << event->mask << dendl;
    return 0;
}
...
void EventCenter::delete_file_event(int fd, int mask)
{
    ...
    EventCenter::FileEvent* event = _get_file_event(fd);
    ldout(cct, 30) << __func__ << " delete event started fd=" << fd << " mask=" << mask << " original mask is "
                   << event->mask << dendl;
    ...
    ldout(cct, 30) << __func__ << " delete event end fd=" << fd << " mask=" << mask << " current mask is "
                   << event->mask << dendl;
}
...
int EventCenter::process_events(unsigned timeout_microseconds, ceph::timespan* working_dur)
{
    ...
    for (int event_id = 0; event_id < numevents; event_id++) {
        ...
        ldout(cct, 30) << __func__ << " event_wq process is " << fired_events[event_id].fd << " mask is "
                       << fired_events[event_id].mask << dendl;
    }
