...
const uint64_t msgr2_frame_assumed = msgr2_required | CEPH_FEATUREMASK_SERVER_NAUTILUS;
...
static constexpr uint32_t FRAME_PREAMBLE_WITH_INLINE_SIZE = sizeof(preamble_block_t) + FRAME_PREAMBLE_INLINE_SIZE;
...
public:
    ...
    FrameAssembler(const ceph::crypto::onwire::rxtx_t* crypto, bool is_rev1, bool with_data_crc,
    ...
    bool get_is_rev1() { return m_is_rev1; }
    ...
    uint32_t get_epilogue_onwire_len() const
    {
        ...
        if (m_crypto->rx) {
            return (m_is_rev1 ? sizeof(epilogue_secure_rev1_block_t) : sizeof(epilogue_secure_rev0_block_t)) +
                   get_auth_tag_len();
        }
        return m_is_rev1 ? sizeof(epilogue_crc_rev1_block_t) : sizeof(epilogue_crc_rev0_block_t);
    }

    uint64_t get_frame_logical_len() const;
    uint64_t get_frame_onwire_len() const;

    bufferlist assemble_frame(Tag tag, bufferlist segment_bls[], const uint16_t segment_aligns[], size_t segment_count);

    Tag disassemble_preamble(bufferlist& preamble_bl);

    bool disassemble_segments(bufferlist& preamble_bl, bufferlist segments_bls[], bufferlist& epilogue_bl) const;
...
private:
    ...
    {
        return p2roundup<uint32_t>(m_descs[seg_idx].logical_len, CRYPTO_BLOCK_SIZE);
    }

    uint32_t get_auth_tag_len() const { return m_crypto->rx->get_extra_size_at_final(); }

    bool is_compressed() const { return m_flags & FRAME_EARLY_DATA_COMPRESSED; }

    void asm_compress(bufferlist segment_bls[]);

    bufferlist asm_crc_rev0(const preamble_block_t& preamble, bufferlist segment_bls[]) const;
    bufferlist asm_secure_rev0(const preamble_block_t& preamble, bufferlist segment_bls[]) const;
    bufferlist asm_crc_rev1(const preamble_block_t& preamble, bufferlist segment_bls[]) const;
    bufferlist asm_secure_rev1(const preamble_block_t& preamble, bufferlist segment_bls[]) const;
    ...
    // and must be dropped.
    void disassemble_first_segment(bufferlist& preamble_bl, bufferlist& segment_bl) const;
    bool disassemble_remaining_segments(bufferlist segment_bls[], bufferlist& epilogue_bl) const;
    void disassemble_decompress(bufferlist segment_bls[]) const;

    bool disasm_all_crc_rev0(bufferlist segment_bls[], bufferlist& epilogue_bl) const;
    bool disasm_all_secure_rev0(bufferlist segment_bls[], bufferlist& epilogue_bl) const;
    void disasm_first_crc_rev1(bufferlist& preamble_bl, bufferlist& segment_bl) const;
    bool disasm_remaining_crc_rev1(bufferlist segment_bls[], bufferlist& epilogue_bl) const;
    void disasm_first_secure_rev1(bufferlist& preamble_bl, bufferlist& segment_bl) const;
    bool disasm_remaining_secure_rev1(bufferlist segment_bls[], bufferlist& epilogue_bl) const;

    void fill_preamble(Tag tag, preamble_block_t& preamble) const;
    friend std::ostream& operator<<(std::ostream& os, const FrameAssembler& frame_asm);
...
template<class T, uint16_t... SegmentAlignmentVs> struct Frame
{
...
private:
    static constexpr std::array<uint16_t, SegmentsNumV> alignments{SegmentAlignmentVs...};

public:
    ceph::bufferlist get_buffer(FrameAssembler& tx_frame_asm)
    {
        auto bl = tx_frame_asm.assemble_frame(T::tag, segments.data(), alignments.data(), SegmentsNumV);
...
{
protected:
    ceph::bufferlist& get_payload_segment() { return this->segments[SegmentIndex::Control::PAYLOAD]; }
    ...
    template<typename T> inline void _encode_payload_each(T& t)
    {
    ...
    template<typename T> inline void _decode_payload_each(T& t, bufferlist::const_iterator& ti) const
    {
    ...
    inline void _decode_payload(bufferlist::const_iterator& ti, std::index_sequence<Is...>) const
    {
        (_decode_payload_each((Args&)std::get<Is>(_values), ti), ...);
    }

    template<std::size_t N> inline decltype(auto) get_val() { return std::get<N>(_values); }

    ControlFrame()
        : Frame<C, segment_t::DEFAULT_ALIGNMENT /* single segment */>()
    {}

    void _encode(const Args&... args) { (_encode_payload_each(args), ...); }
...
struct AuthReplyMoreFrame : public ControlFrame<AuthReplyMoreFrame, bufferlist>
{   // auth payload
...
struct AuthRequestMoreFrame : public ControlFrame<AuthRequestMoreFrame, bufferlist>
{   // auth payload
...
struct AuthSignatureFrame : public ControlFrame<AuthSignatureFrame, sha256_digest_t>
{
...
struct ClientIdentFrame : public ControlFrame<ClientIdentFrame,
...
struct ServerIdentFrame : public ControlFrame<ServerIdentFrame,
                                              entity_addrvec_t,   // my addresses
                                              int64_t,            // global_id
                                              uint64_t,           // global seq
                                              uint64_t,           // supported features
...
struct ReconnectFrame : public ControlFrame<ReconnectFrame,
                                            entity_addrvec_t,   // my addresses
...
struct ResetFrame : public ControlFrame<ResetFrame, bool>
{   // full reset
...
struct RetryFrame : public ControlFrame<RetryFrame, uint64_t>
{   // connection seq
...
struct RetryGlobalFrame : public ControlFrame<RetryGlobalFrame, uint64_t>
{   // global seq
...
struct ReconnectOkFrame : public ControlFrame<ReconnectOkFrame, uint64_t>
{   // message seq
...
struct IdentMissingFeaturesFrame : public ControlFrame<IdentMissingFeaturesFrame, uint64_t>
...
struct KeepAliveFrame : public ControlFrame<KeepAliveFrame, utime_t>
{   // timestamp
    static const Tag tag = Tag::KEEPALIVE2;
    using ControlFrame::Encode;
    using ControlFrame::Decode;

    static KeepAliveFrame Encode() { return KeepAliveFrame::Encode(ceph_clock_now()); }
...
struct KeepAliveFrameAck : public ControlFrame<KeepAliveFrameAck, utime_t>
{   // ack timestamp
...
struct AckFrame : public ControlFrame<AckFrame, uint64_t>
{   // message sequence
...
using segment_bls_t = boost::container::static_vector<bufferlist, MAX_NUM_SEGMENTS>;
                                   ...
                                   segment_t::DEFAULT_ALIGNMENT, segment_t::PAGE_SIZE_ALIGNMENT>
{
    ...
    static MessageFrame Encode(const ceph_msg_header2& msg_header, const ceph::bufferlist& front,
                               const ceph::bufferlist& middle, const ceph::bufferlist& data)
    {
        MessageFrame f;
        f.segments[SegmentIndex::Msg::HEADER].append(reinterpret_cast<const char*>(&msg_header), sizeof(msg_header));
    ...
    ceph::bufferlist& front() { return segments[SegmentIndex::Msg::FRONT]; }

    ceph::bufferlist& middle() { return segments[SegmentIndex::Msg::MIDDLE]; }

    ceph::bufferlist& data() { return segments[SegmentIndex::Msg::DATA]; }

    uint32_t front_len() const { return segments[SegmentIndex::Msg::FRONT].length(); }

    uint32_t middle_len() const { return segments[SegmentIndex::Msg::MIDDLE].length(); }

    uint32_t data_len() const { return segments[SegmentIndex::Msg::DATA].length(); }
