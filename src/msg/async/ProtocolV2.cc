...
std::ostream& ProtocolV2::_conn_prefix(std::ostream* _dout)
{
    return *_dout << "--2- " << messenger->get_myaddrs() << " >> " << *connection->peer_addrs << " conn(" << connection
                  ...
                  << " rx=" << session_stream_handlers.rx.get() << " tx=" << session_stream_handlers.tx.get() << ").";
}
...
ssize_t ProtocolV2::write_message(Message* m, bool more)
{
    ...
    if (rc < 0) {
        ...
        connection->logger->inc(l_msgr_send_bytes, total_send_size - connection->outgoing_bl.length());
...
CtPtr ProtocolV2::handle_read_frame_dispatch()
{
    ...
    case Tag::ACK:
    case Tag::WAIT: return handle_frame_payload();
...
CtPtr ProtocolV2::_handle_read_frame_epilogue_main()
{
    ...
    try {
        ...
        aborted = !rx_frame_asm.disassemble_remaining_segments(rx_segments_data.data(), rx_epilogue);
    }
    catch (FrameError& e) {
        ldout(cct, 1) << __func__ << " " << e.what() << dendl;
        return _fault();
...
CtPtr ProtocolV2::handle_message()
{
    ...
    ceph_msg_header header{current_header.seq,
                           ...
                           init_le32(0)};
    ceph_msg_footer footer{init_le32(0), init_le32(0), init_le32(0), init_le64(0), current_header.flags};
...
CtPtr ProtocolV2::reuse_connection(const AsyncConnectionRef& existing, ProtocolV2* exproto)
{
    ...
    auto deactivate_existing = std::bind(
        [existing,
         ...
         temp_stream_handlers = std::move(temp_stream_handlers)](ConnectedSocket& cs) mutable {
