...
{
    return *_dout << "--2- " << messenger->get_myaddrs() << " >> " << *connection->peer_addrs << " conn(" << connection
                  << " " << this << " " << ceph_con_mode_name(auth_meta->con_mode) << " :" << connection->port
                  << " s=" << get_state_name(state) << " pgs=" << peer_global_seq << " cs=" << connect_seq
                  << " l=" << connection->policy.lossy
                  << " rev1=" << HAVE_MSGR2_FEATURE(peer_supported_features, REVISION_1)
                  << " crypto rx=" << session_stream_handlers.rx.get() << " tx=" << session_stream_handlers.tx.get()
...
void ProtocolV2::run_continuation(CtRef continuation)
{
    try {
        ...
        lderr(cct) << __func__ << " failed decoding of frame header: " << e.what() << dendl;
        _fault();
    }
...
ProtocolV2::ProtocolV2(AsyncConnection* connection)
    ...
    , tx_frame_asm(&session_stream_handlers, false, cct->_conf->ms_crc_data, &session_compression_handlers)
    , rx_frame_asm(&session_stream_handlers, false, cct->_conf->ms_crc_data, &session_compression_handlers)
...
void ProtocolV2::requeue_sent()
{
    ...
    while (!sent.empty()) {
        Message* m = sent.back();
        sent.pop_back();
        ldout(cct, 5) << __func__ << " requeueing message m=" << m << " seq=" << m->get_seq()
...
uint64_t ProtocolV2::discard_requeued_up_to(uint64_t out_seq, uint64_t seq)
{
    ...
    while (!rq.empty()) {
        ...
        ldout(cct, 5) << __func__ << " discarding message m=" << m << " seq=" << m->get_seq() << " ack_seq=" << seq
                      << " " << *m << dendl;
        m->put();
        rq.pop_front();
        count++;
...
void ProtocolV2::reset_recv_state()
{
    ...
    if (!connection->center->in_thread()) {
        ...
        connection->center->submit_to(
            connection->center->get_id(),
            [this] {
                ldout(cct, 5) << "reset_recv_state (warped) reseting crypto and compression handlers" << dendl;
...
{
    if (state > THROTTLE_MESSAGE && state <= THROTTLE_DONE && connection->policy.throttler_messages) {
        ldout(cct, 10) << __func__ << " releasing " << 1 << " message to policy throttler "
                       << connection->policy.throttler_messages->get_current() << "/"
                       << connection->policy.throttler_messages->get_max() << dendl;
    ...
    if (state > THROTTLE_BYTES && state <= THROTTLE_DONE) {
        if (connection->policy.throttler_bytes) {
            ...
            ldout(cct, 10) << __func__ << " releasing " << cur_msg_size << " bytes to policy throttler "
    ...
    if (state > THROTTLE_DISPATCH_QUEUE && state <= THROTTLE_DONE) {
        const size_t cur_msg_size = get_current_msg_size();
        ldout(cct, 10) << __func__ << " releasing " << cur_msg_size << " bytes to dispatch_queue throttler "
...
CtPtr ProtocolV2::_fault()
{
    ...
    if (connection->policy.lossy && !(state >= START_CONNECT && state <= SESSION_RECONNECTING)) {
    ...
    if (out_queue.empty() && state >= START_ACCEPT && state <= SESSION_ACCEPTING && !replacing) {
    ...
    if (connection->policy.standby && out_queue.empty() && !keepalive && state != WAIT) {
    ...
    if (!(state >= START_CONNECT && state <= SESSION_RECONNECTING) && state != WAIT &&
        ...
        if (state == WAIT) {
            ...
            backoff += backoff;
            if (backoff > cct->_conf->ms_max_backoff) backoff.set_from_double(cct->_conf->ms_max_backoff);
        }
        ...
        connection->register_time_events.insert(
            connection->center->create_time_event(backoff.to_nsec() / 1000, connection->wakeup_handler));
    }
    return nullptr;
}
...
void ProtocolV2::send_message(Message* m)
{
    ...
    if (can_fast_prepare && (!can_write || connection->get_features() != f)) {
        ...
        m->clear_payload();
        is_prepared = false;
        ldout(cct, 10) << __func__ << " clear encoded buffer previous " << f << " != " << connection->get_features()
                       << dendl;
    }
    if (state == CLOSED) {
...
void ProtocolV2::read_event()
{
    ...
    switch (state) {
    case START_CONNECT: run_continuation(CONTINUATION(start_client_banner_exchange)); break;
    case START_ACCEPT: run_continuation(CONTINUATION(start_server_banner_exchange)); break;
    case READY: run_continuation(CONTINUATION(read_frame)); break;
    case THROTTLE_MESSAGE: run_continuation(CONTINUATION(throttle_message)); break;
    case THROTTLE_BYTES: run_continuation(CONTINUATION(throttle_bytes)); break;
    case THROTTLE_DISPATCH_QUEUE: run_continuation(CONTINUATION(throttle_dispatch_queue)); break;
    default: break;
    }
}
...
ssize_t ProtocolV2::write_message(Message* m, bool more)
{
    ...
    ceph_msg_header2 header2{header.seq,
                             header.tid,
                             header.type,
                             header.priority,
                             header.version,
    ...
    ldout(cct, 20) << __func__ << " sending m=" << m << " seq=" << m->get_seq()
                   << " src=" << entity_name_t(messenger->get_myname()) << " off=" << header2.data_off << dendl;
...
template<class F> bool ProtocolV2::append_frame(F& frame)
{
    ceph::bufferlist bl;
    try {
...
void ProtocolV2::handle_message_ack(uint64_t seq)
{
    ...
    while (!sent.empty() && sent.front()->get_seq() <= seq && i < max_pending) {
        ...
        sent.pop_front();
        pending[i++] = m;
        ldout(cct, 10) << __func__ << " got ack seq " << seq << " >= " << m->get_seq() << " on " << m << " " << *m
                       << dendl;
    }
    connection->write_lock.unlock();
...
void ProtocolV2::write_event()
{
    ...
    if (can_write) {
        ...
        else if (connection->cs && state != NONE && state != CLOSED && state != START_CONNECT) {
            r = connection->_try_send();
            if (r < 0) {
...
CtPtr ProtocolV2::read(CONTINUATION_RXBPTR_TYPE<ProtocolV2>& next, rx_buffer_t&& buffer)
{
    ...
    ssize_t r = connection->read(len, buf, [&next, this](char* buffer, int r) {
        if (unlikely(pre_auth.enabled) && r >= 0) {
            pre_auth.rxbuf.append(*next.node);
            ceph_assert(!cct->_conf->ms_die_on_bug || pre_auth.rxbuf.length() < 20000000);
        }
        next.r = r;
...
template<class F> CtPtr ProtocolV2::write(const std::string& desc, CONTINUATION_TYPE<ProtocolV2>& next, F& frame)
{
    ceph::bufferlist bl;
    try {
        bl = frame.get_buffer(tx_frame_asm);
...
CtPtr ProtocolV2::write(const std::string& desc, CONTINUATION_TYPE<ProtocolV2>& next, ceph::bufferlist& buffer)
{
    if (unlikely(pre_auth.enabled)) {
        pre_auth.txbuf.append(buffer);
        ceph_assert(!cct->_conf->ms_die_on_bug || pre_auth.txbuf.length() < 20000000);
    }

    ssize_t r = connection->write(buffer, [&next, desc, this](int r) {
        if (r < 0) {
            ldout(cct, 1) << __func__ << " " << desc << " write failed r=" << r << " (" << cpp_strerror(r) << ")"
                          << dendl;
...
CtPtr ProtocolV2::_handle_peer_banner(rx_buffer_t&& buffer, int r)
{
    ...
    if (memcmp(buffer->c_str(), CEPH_BANNER_V2_PREFIX, banner_prefix_len)) {
        if (memcmp(buffer->c_str(), CEPH_BANNER, strlen(CEPH_BANNER)) == 0) {
            lderr(cct) << __func__ << " peer " << *connection->peer_addrs << " is using msgr V1 protocol" << dendl;
            return _fault();
        }
...
CtPtr ProtocolV2::_handle_peer_banner_payload(rx_buffer_t&& buffer, int r)
{
    ...
    if (r < 0) {
        ldout(cct, 1) << __func__ << " read peer banner payload failed r=" << r << " (" << cpp_strerror(r) << ")"
                      << dendl;
        return _fault();
    }
    ...
    ldout(cct, 1) << __func__ << " supported=" << std::hex << peer_supported_features << " required=" << std::hex
    ...
    if ((required_features & peer_supported_features) != required_features) {
        ldout(cct, 1) << __func__ << " peer does not support all required features"
                      << " required=" << std::hex << required_features << " supported=" << std::hex
                      << peer_supported_features << std::dec << dendl;
        stop();
        connection->dispatch_queue->queue_reset(connection);
        return nullptr;
    }
    if ((supported_features & peer_required_features) != peer_required_features) {
        ldout(cct, 1) << __func__ << " we do not support all peer required features"
                      << " required=" << std::hex << peer_required_features << " supported=" << supported_features
                      << std::dec << dendl;
        stop();
        connection->dispatch_queue->queue_reset(connection);
        return nullptr;
    ...
    auto hello = HelloFrame::Encode(messenger->get_mytype(), connection->target_addr);
...
CtPtr ProtocolV2::handle_hello(ceph::bufferlist& payload)
{
    ...
    ldout(cct, 5) << __func__ << " received hello:"
                  << " peer_type=" << (int)hello.entity_type() << " peer_addr_for_me=" << hello.peer_addr() << dendl;

    sockaddr_storage ss;
    socklen_t len = sizeof(ss);
    getsockname(connection->cs.fd(), (sockaddr*)&ss, &len);
    ldout(cct, 5) << __func__ << " getsockname says I am " << (sockaddr*)&ss << " when talking to "
    ...
    if (connection->get_peer_type() == -1) {
        ...
        ldout(cct, 10) << __func__ << " accept of host_type " << (int)hello.entity_type()
        ...
        if (connection->get_peer_type() != hello.entity_type()) {
            ldout(cct, 1) << __func__ << " connection peer type does not match what"
                          << " peer advertises " << connection->get_peer_type() << " != " << (int)hello.entity_type()
                          << dendl;
            stop();
            connection->dispatch_queue->queue_reset(connection);
            return nullptr;
    ...
    if (messenger->get_myaddrs().empty() || messenger->get_myaddrs().front().is_blank_ip()) {
        entity_addr_t a;
        if (cct->_conf->ms_learn_addr_from_peer) {
            ldout(cct, 1) << __func__ << " peer " << connection->target_addr << " says I am " << hello.peer_addr()
                          << " (socket says " << (sockaddr*)&ss << ")" << dendl;
            a = hello.peer_addr();
        }
        else {
            ldout(cct, 1) << __func__ << " socket to  " << connection->target_addr << " says I am " << (sockaddr*)&ss
        ...
        if (cct->_conf->ms_inject_internal_delays && cct->_conf->ms_inject_socket_failures) {
        ...
        if (state != HELLO_CONNECTING) {
            ldout(cct, 1) << __func__ << " state changed while learned_addr, mark_down or "
...
CtPtr ProtocolV2::read_frame()
{
    ...
    return READ(rx_frame_asm.get_preamble_onwire_len(), handle_read_frame_preamble_main);
}
...
CtPtr ProtocolV2::handle_read_frame_preamble_main(rx_buffer_t&& buffer, int r)
{
    ...
    if (r < 0) {
        ldout(cct, 1) << __func__ << " read frame preamble failed r=" << r << dendl;
        return _fault();
    }
...
CtPtr ProtocolV2::handle_read_frame_dispatch()
{
    ...
    case Tag::COMPRESSION_DONE: return handle_frame_payload();
    case Tag::MESSAGE: return handle_message();
    default:
    {
        lderr(cct) << __func__ << " received unknown tag=" << static_cast<uint32_t>(next_tag) << dendl;
        return _fault();
    }
    }
...
CtPtr ProtocolV2::read_frame_segment()
{
    ...
    try {
        rx_buffer = ceph::buffer::ptr_node::create(ceph::buffer::create_aligned(onwire_len, align));
    }
        ...
        ldout(cct, 1) << __func__ << " can't allocate aligned rx_buffer"
                      << " len=" << onwire_len << " align=" << align << dendl;
        return _fault();
    }
...
CtPtr ProtocolV2::handle_frame_payload()
{
    ...
    *_dout << dendl;
    ...
    switch (next_tag) {
    case Tag::HELLO: return handle_hello(payload);
    case Tag::AUTH_REQUEST: return handle_auth_request(payload);
    case Tag::AUTH_BAD_METHOD: return handle_auth_bad_method(payload);
    case Tag::AUTH_REPLY_MORE: return handle_auth_reply_more(payload);
    case Tag::AUTH_REQUEST_MORE: return handle_auth_request_more(payload);
    case Tag::AUTH_DONE: return handle_auth_done(payload);
    case Tag::AUTH_SIGNATURE: return handle_auth_signature(payload);
    case Tag::CLIENT_IDENT: return handle_client_ident(payload);
    case Tag::SERVER_IDENT: return handle_server_ident(payload);
    case Tag::IDENT_MISSING_FEATURES: return handle_ident_missing_features(payload);
    case Tag::SESSION_RECONNECT: return handle_reconnect(payload);
    case Tag::SESSION_RESET: return handle_session_reset(payload);
    case Tag::SESSION_RETRY: return handle_session_retry(payload);
    case Tag::SESSION_RETRY_GLOBAL: return handle_session_retry_global(payload);
    case Tag::SESSION_RECONNECT_OK: return handle_reconnect_ok(payload);
    case Tag::KEEPALIVE2: return handle_keepalive2(payload);
    case Tag::KEEPALIVE2_ACK: return handle_keepalive2_ack(payload);
    case Tag::ACK: return handle_message_ack(payload);
    case Tag::WAIT: return handle_wait(payload);
    case Tag::COMPRESSION_REQUEST: return handle_compression_request(payload);
    case Tag::COMPRESSION_DONE: return handle_compression_done(payload);
    default: ceph_abort();
    }
    return nullptr;
}
...
CtPtr ProtocolV2::ready()
{
    ...
    connection->last_tick_id =
        connection->center->create_time_event(connection->inactive_timeout_us, connection->tick_handler);
    ...
    state = READY;
    ldout(cct, 1) << __func__ << " entity=" << peer_name << " client_cookie=" << std::hex << client_cookie
                  << " server_cookie=" << server_cookie << std::dec << " in_seq=" << in_seq << " out_seq=" << out_seq
                  << dendl;
...
CtPtr ProtocolV2::handle_message()
{
    ...
    ldout(cct, 5) << __func__ << " got " << msg_frame.front_len() << " + " << msg_frame.middle_len() << " + "
                  << msg_frame.data_len() << " byte message."
                  << " envelope type=" << current_header.type << " src " << peer_name << " off "
                  << current_header.data_off << dendl;
    ...
    ceph_msg_header header{current_header.seq,
                           ...
                           ceph_le32(0)};
    ceph_msg_footer footer{ceph_le32(0), ceph_le32(0), ceph_le32(0), ceph_le64(0), current_header.flags};

    Message* message =
        decode_message(cct, 0, header, footer, msg_frame.front(), msg_frame.middle(), msg_frame.data(), connection);
    if (!message) {
    ...
    if (message->get_seq() <= cur_seq) {
        ldout(cct, 0) << __func__ << " got old message " << message->get_seq() << " <= " << cur_seq << " " << message
                      << " " << *message << ", discarding" << dendl;
        message->put();
        if (connection->has_feature(CEPH_FEATURE_RECONNECT_SEQ) && cct->_conf->ms_die_on_old_message) {
            ceph_assert(0 == "old msgs despite reconnect_seq feature");
        }
        return nullptr;
    }
    if (message->get_seq() > cur_seq + 1) {
        ldout(cct, 0) << __func__ << " missed message?  skipped from seq " << cur_seq << " to " << message->get_seq()
                      << dendl;
    ...
    if (message->get_type() == CEPH_MSG_OSD_OP || message->get_type() == CEPH_MSG_OSD_OPREPLY) {
        ...
        double usecs_elapsed = ((double)(ltt_processed_stamp.to_nsec() - recv_stamp.to_nsec())) / 1000;
        ostringstream buf;
    ...
    ldout(cct, 5) << __func__ << " received message m=" << message << " seq=" << message->get_seq()
                  << " from=" << message->get_source() << " type=" << header.type << " " << *message << dendl;
    ...
    connection->logger->inc(l_msgr_recv_bytes, rx_frame_asm.get_frame_onwire_len());
    if (session_stream_handlers.rx) {
        connection->logger->inc(l_msgr_recv_encrypted_bytes, rx_frame_asm.get_frame_onwire_len());
    }

    messenger->ms_fast_preprocess(message);
    fast_dispatch_time = ceph::mono_clock::now();
    connection->logger->tinc(l_msgr_running_recv_time, fast_dispatch_time - connection->recv_start_time);
    if (connection->delay_state) {
        double delay_period = 0;
        if (rand() % 10000 < cct->_conf->ms_inject_delay_probability * 10000.0) {
            delay_period = cct->_conf->ms_inject_delay_max * (double)(rand() % 10000) / 10000.0;
            ldout(cct, 1) << "queue_received will delay after " << (ceph_clock_now() + delay_period) << " on "
        ...
        connection->logger->tinc(l_msgr_running_fast_dispatch_time, connection->recv_start_time - fast_dispatch_time);
        connection->lock.lock();
        ...
        connection->dispatch_queue->enqueue(message, message->get_priority(), connection->conn_id);
    }
...
CtPtr ProtocolV2::throttle_message()
{
    ...
    if (connection->policy.throttler_messages) {
        ldout(cct, 10) << __func__ << " wants " << 1 << " message from policy throttler "
                       << connection->policy.throttler_messages->get_current() << "/"
                       << connection->policy.throttler_messages->get_max() << dendl;
        if (!connection->policy.throttler_messages->get_or_fail()) {
            ldout(cct, 1) << __func__ << " wants 1 message from policy throttle "
                          << connection->policy.throttler_messages->get_current() << "/"
                          << connection->policy.throttler_messages->get_max() << " failed, just wait." << dendl;
            // following thread pool deal with th full message queue isn't a
            // short time, so we can wait a ms.
            if (connection->register_time_events.empty()) {
                connection->register_time_events.insert(
                    connection->center->create_time_event(1000, connection->wakeup_handler));
            }
            return nullptr;
        }
    }
...
CtPtr ProtocolV2::throttle_bytes()
{
    ...
    if (cur_msg_size) {
        if (connection->policy.throttler_bytes) {
            ldout(cct, 10) << __func__ << " wants " << cur_msg_size << " bytes from policy throttler "
                           << connection->policy.throttler_bytes->get_current() << "/"
                           << connection->policy.throttler_bytes->get_max() << dendl;
            if (!connection->policy.throttler_bytes->get_or_fail(cur_msg_size)) {
                ldout(cct, 1) << __func__ << " wants " << cur_msg_size << " bytes from policy throttler "
                              << connection->policy.throttler_bytes->get_current() << "/"
                              << connection->policy.throttler_bytes->get_max() << " failed, just wait." << dendl;
                // following thread pool deal with th full message queue isn't a
                // short time, so we can wait a ms.
                if (connection->register_time_events.empty()) {
                    connection->register_time_events.insert(
...
CtPtr ProtocolV2::throttle_dispatch_queue()
{
    ...
    if (cur_msg_size) {
        if (!connection->dispatch_queue->dispatch_throttler.get_or_fail(cur_msg_size)) {
            ldout(cct, 1) << __func__ << " wants " << cur_msg_size << " bytes from dispatch throttle "
                          << connection->dispatch_queue->dispatch_throttler.get_current() << "/"
                          << connection->dispatch_queue->dispatch_throttler.get_max() << " failed, just wait." << dendl;
            // following thread pool deal with th full message queue isn't a
            // short time, so we can wait a ms.
            if (connection->register_time_events.empty()) {
                connection->register_time_events.insert(
...
CtPtr ProtocolV2::send_auth_request(std::vector<uint32_t>& allowed_methods)
{
    ...
    ldout(cct, 20) << __func__ << " peer_type " << (int)connection->peer_type << " auth_client "
                   << messenger->auth_client << dendl;

    ceph::bufferlist bl;
    std::vector<uint32_t> preferred_modes;
    auto am = auth_meta;
    connection->lock.unlock();
    int r = messenger->auth_client->get_auth_request(connection, am.get(), &am->auth_method, &preferred_modes, &bl);
    connection->lock.lock();
...
CtPtr ProtocolV2::handle_auth_bad_method(ceph::bufferlist& payload)
{
    ...
    ldout(cct, 1) << __func__ << " method=" << bad_method.method() << " result " << cpp_strerror(bad_method.result())
                  << ", allowed methods=" << bad_method.allowed_methods()
                  << ", allowed modes=" << bad_method.allowed_modes() << dendl;
    ceph_assert(messenger->auth_client);
    auto am = auth_meta;
    connection->lock.unlock();
    int r = messenger->auth_client->handle_auth_bad_method(connection,
                                                           am.get(),
                                                           bad_method.method(),
                                                           bad_method.result(),
...
CtPtr ProtocolV2::handle_auth_reply_more(ceph::bufferlist& payload)
{
    ...
    auto am = auth_meta;
    connection->lock.unlock();
    int r = messenger->auth_client->handle_auth_reply_more(connection, am.get(), auth_more.auth_payload(), &reply);
    connection->lock.lock();
    if (state != AUTH_CONNECTING) {
        ldout(cct, 1) << __func__ << " state changed!" << dendl;
        return _fault();
    ...
    if (r < 0) {
        lderr(cct) << __func__ << " auth_client handle_auth_reply_more returned " << r << dendl;
        return _fault();
    }
...
CtPtr ProtocolV2::handle_auth_done(ceph::bufferlist& payload)
{
    ...
    auto am = auth_meta;
    connection->lock.unlock();
    int r = messenger->auth_client->handle_auth_done(connection,
                                                     am.get(),
                                                     auth_done.global_id(),
                                                     auth_done.con_mode(),
    ...
    const auto sig =
        auth_meta->session_key.empty() ? sha256_digest_t() : auth_meta->session_key.hmac_sha256(cct, pre_auth.rxbuf);
...
CtPtr ProtocolV2::send_client_ident()
{
    ...
    auto client_ident = ClientIdentFrame::Encode(messenger->get_myaddrs(),
    ...
    ldout(cct, 5) << __func__ << " sending identification: "
                  << "addrs=" << messenger->get_myaddrs() << " target=" << connection->target_addr
                  << " gid=" << messenger->get_myname().num() << " global_seq=" << global_seq
                  << " features_supported=" << std::hex << connection->policy.features_supported
                  << " features_required=" << (connection->policy.features_required | msgr2_required)
                  << " flags=" << flags << " cookie=" << client_cookie << std::dec << dendl;
...
CtPtr ProtocolV2::send_reconnect()
{
    ...
    auto reconnect =
        ReconnectFrame::Encode(messenger->get_myaddrs(), client_cookie, server_cookie, global_seq, connect_seq, in_seq);

    ldout(cct, 5) << __func__ << " reconnect to session: client_cookie=" << std::hex << client_cookie
                  << " server_cookie=" << server_cookie << std::dec << " gs=" << global_seq << " cs=" << connect_seq
                  << " ms=" << in_seq << dendl;
...
CtPtr ProtocolV2::handle_ident_missing_features(ceph::bufferlist& payload)
{
    ...
    auto ident_missing = IdentMissingFeaturesFrame::Decode(payload);
    lderr(cct) << __func__ << " client does not support all server features: " << std::hex << ident_missing.features()
               << std::dec << dendl;
...
CtPtr ProtocolV2::handle_session_retry(ceph::bufferlist& payload)
{
    ...
    ldout(cct, 1) << __func__ << " received session retry connect_seq=" << retry.connect_seq()
...
CtPtr ProtocolV2::handle_session_retry_global(ceph::bufferlist& payload)
{
    ...
    ldout(cct, 1) << __func__ << " received session retry global global_seq=" << retry.global_seq()
                  << ", choose new gs=" << global_seq << dendl;
...
{
    ldout(cct, 20) << __func__ << " received WAIT (connection race)"
...
CtPtr ProtocolV2::handle_reconnect_ok(ceph::bufferlist& payload)
{
    ...
    backoff = utime_t();
    ldout(cct, 10) << __func__ << " reconnect success " << connect_seq << ", lossy = " << connection->policy.lossy
                   << ", features " << connection->get_features() << dendl;
...
CtPtr ProtocolV2::handle_server_ident(ceph::bufferlist& payload)
{
    ...
    ldout(cct, 5) << __func__ << " received server identification:"
                  ...
                  << " global_seq=" << server_ident.global_seq() << " features_supported=" << std::hex
                  << server_ident.supported_features() << " features_required=" << server_ident.required_features()
                  << " flags=" << server_ident.flags() << " cookie=" << server_ident.cookie() << std::dec << dendl;
    ...
    if (!server_ident.addrs().contains(connection->target_addr)) {
        ldout(cct, 1) << __func__ << " peer identifies as " << server_ident.addrs() << ", does not include "
                      << connection->target_addr << dendl;
        return _fault();
    }
    ...
    connection->set_features(server_ident.supported_features() & connection->policy.features_supported);
    peer_global_seq = server_ident.global_seq();

    connection->policy.lossy = server_ident.flags() & CEPH_MSG_CONNECT_LOSSY;

    backoff = utime_t();
    ldout(cct, 10) << __func__ << " connect success " << connect_seq << ", lossy = " << connection->policy.lossy
                   << ", features " << connection->get_features() << dendl;
...
CtPtr ProtocolV2::handle_auth_request(ceph::bufferlist& payload)
{
    ...
    ldout(cct, 10) << __func__ << " AuthRequest(method=" << request.method()
                   ...
                   << ", payload_len=" << request.auth_payload().length() << ")" << dendl;
    auth_meta->auth_method = request.method();
    auth_meta->con_mode = messenger->auth_server->pick_con_mode(
        connection->get_peer_type(), auth_meta->auth_method, request.preferred_modes());
...
CtPtr ProtocolV2::_auth_bad_method(int r)
{
    ...
    messenger->auth_server->get_supported_auth_methods(connection->get_peer_type(), &allowed_methods, &allowed_modes);
    ldout(cct, 1) << __func__ << " auth_method " << auth_meta->auth_method << " r " << cpp_strerror(r)
                  << ", allowed_methods " << allowed_methods << ", allowed_modes " << allowed_modes << dendl;
    auto bad_method = AuthBadMethodFrame::Encode(auth_meta->auth_method, r, allowed_methods, allowed_modes);
...
CtPtr ProtocolV2::_handle_auth_request(ceph::bufferlist& auth_payload, bool more)
{
    ...
    connection->lock.unlock();
    int r =
        messenger->auth_server->handle_auth_request(connection, am.get(), more, am->auth_method, auth_payload, &reply);
    connection->lock.lock();
    ...
    if (r == 1) {
        ...
        auto auth_done = AuthDoneFrame::Encode(connection->peer_global_id, auth_meta->con_mode, reply);
...
CtPtr ProtocolV2::handle_auth_signature(ceph::bufferlist& payload)
{
    ...
    if (state != AUTH_ACCEPTING_SIGN && state != AUTH_CONNECTING_SIGN) {
        lderr(cct) << __func__ << " pre-auth verification signature seen in wrong state!" << dendl;
        return _fault();
    }
    ...
    const auto actual_tx_sig =
        auth_meta->session_key.empty() ? sha256_digest_t() : auth_meta->session_key.hmac_sha256(cct, pre_auth.txbuf);
    if (sig_frame.signature() != actual_tx_sig) {
        ldout(cct, 2) << __func__ << " pre-auth signature mismatch"
                      << " actual_tx_sig=" << actual_tx_sig << " sig_frame.signature()=" << sig_frame.signature()
                      << dendl;
        return _fault();
    }
    else {
        ldout(cct, 20) << __func__ << " pre-auth signature success"
                       << " sig_frame.signature()=" << sig_frame.signature() << dendl;
        pre_auth.txbuf.clear();
    }
...
CtPtr ProtocolV2::handle_client_ident(ceph::bufferlist& payload)
{
    ...
    ldout(cct, 5) << __func__ << " received client identification:"
                  << " addrs=" << client_ident.addrs() << " target=" << client_ident.target_addr()
                  << " gid=" << client_ident.gid() << " global_seq=" << client_ident.global_seq()
                  << " features_supported=" << std::hex << client_ident.supported_features()
                  << " features_required=" << client_ident.required_features() << " flags=" << client_ident.flags()
                  << " cookie=" << client_ident.cookie() << std::dec << dendl;

    if (client_ident.addrs().empty() || client_ident.addrs().front() == entity_addr_t()) {
    ...
    if (!messenger->get_myaddrs().contains(client_ident.target_addr())) {
        ldout(cct, 5) << __func__ << " peer is trying to reach " << client_ident.target_addr() << " which is not us ("
    ...
    uint64_t feat_missing =
        (connection->policy.features_required | msgr2_required) & ~(uint64_t)client_ident.supported_features();
    if (feat_missing) {
        ldout(cct, 1) << __func__ << " peer missing required features " << std::hex << feat_missing << std::dec
                      << dendl;
    ...
    connection_features = client_ident.supported_features() & connection->policy.features_supported;

    peer_global_seq = client_ident.global_seq();

    if (connection->policy.server && connection->policy.lossy && !connection->policy.register_lossy_clients) {
        ...
        if (existing && existing->protocol->proto_type != 2) {
            ldout(cct, 1) << __func__ << " existing " << existing << " proto " << existing->protocol.get()
                          << " version is " << existing->protocol->proto_type << ", marking down" << dendl;
            existing->mark_down();
            existing = nullptr;
        }
...
CtPtr ProtocolV2::handle_reconnect(ceph::bufferlist& payload)
{
    ...
    ldout(cct, 5) << __func__ << " received reconnect:"
                  << " client_cookie=" << std::hex << reconnect.client_cookie()
                  << " server_cookie=" << reconnect.server_cookie() << std::dec << " gs=" << reconnect.global_seq()
                  << " cs=" << reconnect.connect_seq() << " ms=" << reconnect.msg_seq() << dendl;
    ...
    if (existing && existing->protocol->proto_type != 2) {
        ldout(cct, 1) << __func__ << " existing " << existing << " proto " << existing->protocol.get() << " version is "
    ...
    if (exproto->replacing) {
        ldout(cct, 1) << __func__ << " existing racing replace happened while replacing."
    ...
    if (exproto->client_cookie != reconnect.client_cookie()) {
        ldout(cct, 1) << __func__ << " existing=" << existing << " client cookie mismatch, I must have reseted:"
                      << " cc=" << std::hex << exproto->client_cookie << " rcc=" << reconnect.client_cookie()
                      << ", reseting client." << std::dec << dendl;
    ...
    if (exproto->peer_global_seq > reconnect.global_seq()) {
        ldout(cct, 5) << __func__ << " stale global_seq: sgs=" << exproto->peer_global_seq
                      << " cgs=" << reconnect.global_seq() << ", ask client to retry global" << dendl;
        auto retry = RetryGlobalFrame::Encode(exproto->peer_global_seq);
    ...
    if (exproto->connect_seq > reconnect.connect_seq()) {
        ldout(cct, 5) << __func__ << " stale connect_seq scs=" << exproto->connect_seq
                      << " ccs=" << reconnect.connect_seq() << " , ask client to retry" << dendl;
    ...
    if (exproto->connect_seq == reconnect.connect_seq()) {
        ...
        if (existing->peer_addrs->msgr2_addr() > messenger->get_myaddrs().msgr2_addr() && !existing->policy.server) {
            // the existing connection wins
            ldout(cct, 1) << __func__ << " reconnect race detected, this connection loses to existing=" << existing
                          << dendl;
            ...
            ldout(cct, 1) << __func__ << " reconnect race detected, replacing existing=" << existing
...
CtPtr ProtocolV2::handle_existing_connection(const AsyncConnectionRef& existing)
{
    ...
    if (exproto->peer_global_seq > peer_global_seq) {
        ldout(cct, 1) << __func__ << " this is a stale connection, peer_global_seq=" << peer_global_seq
                      << " existing->peer_global_seq=" << exproto->peer_global_seq << ", stopping this connection."
                      << dendl;
        stop();
        connection->dispatch_queue->queue_reset(connection);
        return nullptr;
    ...
    if (existing->policy.lossy) {
        ...
        ldout(cct, 1) << __func__ << " existing=" << existing
                      << " is a lossy channel. Stopping existing in favor of this connection" << dendl;
    ...
    if (exproto->server_cookie && exproto->client_cookie && exproto->client_cookie != client_cookie) {
        // Found previous session
        // peer has reseted and we're going to reuse the existing connection
        // by replacing the communication socket
        ldout(cct, 1) << __func__ << " found previous session existing=" << existing << ", peer must have reseted."
                      << dendl;
    ...
    if (connection->peer_addrs->msgr2_addr() < messenger->get_myaddrs().msgr2_addr() || existing->policy.server) {
        // this connection wins
        ldout(cct, 1) << __func__ << " connection race detected, replacing existing=" << existing
                      << " socket by this connection's socket" << dendl;
        return reuse_connection(existing, exproto);
    }
    else {
        // the existing connection wins
        ldout(cct, 1) << __func__ << " connection race detected, this connection loses to existing=" << existing
                      << dendl;
        ceph_assert(connection->peer_addrs->msgr2_addr() > messenger->get_myaddrs().msgr2_addr());
...
CtPtr ProtocolV2::reuse_connection(const AsyncConnectionRef& existing, ProtocolV2* exproto)
{
    ...
    connection->center->delete_file_event(connection->cs.fd(), EVENT_READABLE | EVENT_WRITABLE);
    ...
    auto deactivate_existing = std::bind(
        [existing,
         ...
         temp_compression_handlers = std::move(temp_compression_handlers)](ConnectedSocket& cs) mutable {
            // we need to delete time event in original thread
            {
                ...
                if (exproto->state == NONE) {
                    ...
                    if (existing->delay_state) existing->delay_state->set_center(new_center);
                }
                else if (exproto->state == CLOSED) {
                    auto back_to_close = std::bind([](ConnectedSocket& cs) mutable { cs.close(); }, std::move(cs));
                    new_center->submit_to(new_center->get_id(), std::move(back_to_close), true);
                    return;
                }
                else {
                    ceph_abort();
            ...
            auto transfer_existing = [existing, exproto]() mutable {
                ...
                existing->last_tick_id =
                    existing->center->create_time_event(existing->connect_timeout_us, existing->tick_handler);
            ...
            if (existing->center->in_thread())
                ...
                existing->center->submit_to(existing->center->get_id(), std::move(transfer_existing), true);
        },
        std::move(temp_cs));

    existing->center->submit_to(existing->center->get_id(), std::move(deactivate_existing), true);
    return nullptr;
}
...
CtPtr ProtocolV2::send_server_ident()
{
    ...
    auto server_ident = ServerIdentFrame::Encode(messenger->get_myaddrs(),
                                                 messenger->get_myname().num(),
                                                 gs,
                                                 connection->policy.features_supported,
                                                 connection->policy.features_required | msgr2_required,
    ...
    ldout(cct, 5) << __func__ << " sending identification:"
                  << " addrs=" << messenger->get_myaddrs() << " gid=" << messenger->get_myname().num()
                  << " global_seq=" << gs << " features_supported=" << std::hex << connection->policy.features_supported
                  << " features_required=" << (connection->policy.features_required | msgr2_required)
                  << " flags=" << flags << " cookie=" << server_cookie << std::dec << dendl;
    ...
    if (r < 0) {
        ldout(cct, 1) << __func__
                      << " existing race replacing process for addr = " << connection->peer_addrs->msgr2_addr()
...
CtPtr ProtocolV2::handle_compression_request(ceph::bufferlist& payload)
{
    ...
    if (Compressor::CompressionMode mode = messenger->comp_registry.get_mode(peer_type, auth_meta->is_mode_secure());
        ...
        ldout(cct, 10) << __func__
                       << " Compressor(pick_method=" << Compressor::get_comp_alg_name(comp_meta.get_method()) << ")"
                       << dendl;
