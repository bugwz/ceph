...
protected:
    ...
    CtPtr read(CONTINUATION_RX_TYPE<ProtocolV1>& next, int len, char* buffer = nullptr);
...
protected:
    ...
    WRITE_HANDLER_CONTINUATION_DECL(ProtocolV1, handle_ready_connect_message_reply_write);
    ...
    CtPtr send_connect_message_reply(char tag, ceph_msg_connect_reply& reply, ceph::buffer::list& authorizer_reply);
