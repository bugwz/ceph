...
public:
    explicit C_processor_accept(Processor* p)
        : pro(p)
    {}
...
int Processor::bind(const entity_addrvec_t& bind_addrs, const std::set<int>& avoid_ports, entity_addrvec_t* bound_addrs)
{
    ...
    *bound_addrs = bind_addrs;
    ...
    for (unsigned k = 0; k < bind_addrs.v.size(); ++k) {
        ...
        for (int i = 0; i < conf->ms_bind_retry_count; i++) {
            if (i > 0) {
                lderr(msgr->cct) << __func__ << " was unable to bind. Trying again in " << conf->ms_bind_retry_delay
                                 << " seconds " << dendl;
            ...
            if (listen_addr.get_port()) {
                worker->center.submit_to(
                    ...
                    false);
                if (r < 0) {
                    lderr(msgr->cct) << __func__ << " unable to bind to " << listen_addr << ": " << cpp_strerror(r)
                                     << dendl;
                    continue;
                }
            }
                ...
                for (int port = msgr->cct->_conf->ms_bind_port_min; port <= msgr->cct->_conf->ms_bind_port_max;
                     port++) {
                    if (avoid_ports.count(port)) continue;
                ...
                if (r < 0) {
                    lderr(msgr->cct) << __func__ << " unable to bind to " << listen_addr << " on any port in range "
                                     << msgr->cct->_conf->ms_bind_port_min << "-" << msgr->cct->_conf->ms_bind_port_max
            ...
            for (auto& listen_socket : listen_sockets) {
                if (listen_socket) {
                    if (listen_socket.fd() == -1) {
                        ldout(msgr->cct, 1)
                            << __func__ << " Error: processor restart after listen_socket.fd closed. " << this << dendl;
                        return;
                    }
            ...
            if (r == 0) {
                ...
                if (r == -EINTR) {
                    ...
                    ldout(msgr->cct, 0) << __func__
                                        << " it was closed because of rst arrived sd = " << listen_socket.fd()
                                        << " errno " << r << " " << cpp_strerror(r) << dendl;
                    continue;
                }
                else {
...
struct StackSingleton
{
    ...
    explicit StackSingleton(CephContext* c)
        : cct(c)
    {}
...
public:
    explicit C_handle_reap(AsyncMessenger* m)
        : msgr(m)
    {}
...
AsyncMessenger::AsyncMessenger(CephContext* cct, entity_name_t name, const std::string& type, std::string mname,
                               uint64_t _nonce)
    ...
    , nonce(_nonce)
{
    ...
    local_connection = ceph::make_ref<AsyncConnection>(cct, this, &dispatch_queue, local_worker, true, true);
    init_local_connection();
    reap_handler = new C_handle_reap(this);
    unsigned processor_num = 1;
    if (stack->support_local_listen_table()) processor_num = stack->get_num_worker();
    for (unsigned i = 0; i < processor_num; ++i) processors.push_back(new Processor(this, stack->get_worker(i), cct));
}
...
int AsyncMessenger::bind(const entity_addr_t& bind_addr, std::optional<entity_addrvec_t> public_addrs)
{
    ldout(cct, 10) << __func__ << " " << bind_addr << " public " << public_addrs << dendl;
...
int AsyncMessenger::bindv(const entity_addrvec_t& bind_addrs, std::optional<entity_addrvec_t> public_addrs)
{
    lock.lock();
    ...
    ldout(cct, 10) << __func__ << " " << bind_addrs << " public " << public_addrs << dendl;
...
{
    if (!cct->_conf->ms_bind_before_connect) return 0;
    std::lock_guard l{lock};
    if (did_bind) {
        return 0;
    }
...
void AsyncMessenger::_finish_bind(const entity_addrvec_t& bind_addrs, const entity_addrvec_t& listen_addrs)
{
...
{
    {
        ...
        if (!stopped) stop_cond.wait(locker);
    }
...
void AsyncMessenger::add_accept(Worker* w, ConnectedSocket cli_socket, const entity_addr_t& listen_addr,
...
int AsyncMessenger::send_to(Message* m, int type, const entity_addrvec_t& addrs)
{
    ...
    ldout(cct, 1) << __func__ << "--> " << ceph_entity_type_name(type) << " " << addrs << " -- " << *m << " -- ?+"
...
{
    if (!not_local_dest) {
        if (*my_addrs == addrs || (addrs.v.size() == 1 && my_addrs->contains(addrs.front()))) {
            // local
            return local_connection;
        }
    }
...
{
    std::lock_guard l{lock};
    if (conn->policy.server && conn->policy.lossy && !conn->policy.register_lossy_clients) {
        anon_conns.insert(conn);
...
bool AsyncMessenger::learned_addr(const entity_addr_t& peer_addr_for_me)
{
    ...
    if (need_addr) {
        if (my_addrs->empty()) {
            ...
            entity_addrvec_t newaddrs = *my_addrs;
            for (auto& a : newaddrs.v) {
                if (a.is_blank_ip() && a.get_family() == peer_addr_for_me.get_family()) {
        ...
        ldout(cct, 1) << __func__ << " learned my addr " << *my_addrs << " (peer_addr_for_me " << peer_addr_for_me
                      << ")" << dendl;
        _init_local_connection();
        need_addr = false;
        return true;
...
void AsyncMessenger::reap_dead()
{
...

    {
        ...
        for (auto& c : deleted_conns) {
            ...
            if (conns_it != conns.end() && conns_it->second == c) conns.erase(conns_it);
