...
int PollDriver::poll_ctl(int fd, int op, int events)
{
    ...
    if (op == POLL_ADD) {
        ...
        if (max_pfds < hard_max_pfds) {
            ...
            if (!pfds) {
                lderr(cct) << __func__ << " unable to realloc for more pollfd slots" << dendl;
                return -ENOMEM;
            }
...
#ifdef _WIN32
    retval = WSAPoll(pfds, max_pfds, tvp ? (tvp->tv_sec * 1000 + tvp->tv_usec / 1000) : -1);
#else
    retval = poll(pfds, max_pfds, tvp ? (tvp->tv_sec * 1000 + tvp->tv_usec / 1000) : -1);
#endif
    if (retval > 0) {
