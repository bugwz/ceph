...
{
    static const char* keys[] = {"ms_osd_compress_mode",
...
void CompressorRegistry::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
...
void CompressorRegistry::_refresh_config()
{
    ...
    if (c_mode) {
        ...
        ldout(cct, 1) << __func__ << " failed to identify ms_osd_compress_mode " << ms_osd_compress_mode << dendl;
    ...
    ldout(cct, 10) << __func__ << " ms_osd_compression_mode " << ms_osd_compress_mode << " ms_osd_compression_methods "
                   << ms_osd_compression_methods << " ms_osd_compress_above_min_size " << ms_osd_compress_min_size
                                                                 ...
                                                                 const std::vector<uint32_t>& preferred_methods)
{
    ...
    auto preferred = std::find_first_of(
        preferred_methods.begin(), preferred_methods.end(), allowed_methods.begin(), allowed_methods.end());
    if (preferred == preferred_methods.end()) {
        ldout(cct, 1) << "failed to pick compression method from client's " << preferred_methods << " and our "
                      << allowed_methods << dendl;
...
{
    std::scoped_lock l(lock);
    ldout(cct, 20) << __func__ << " peer_type " << peer_type << " is_secure " << is_secure << dendl;
    ...
    switch (peer_type) {
    case CEPH_ENTITY_TYPE_OSD: return static_cast<Compressor::CompressionMode>(ms_osd_compress_mode);
    default: return Compressor::COMP_NONE;
    }
}
