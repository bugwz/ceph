...
public:
    ...
    public:
        explicit CompletionHook(Message* _m)
            : m(_m)
        {}
    ...
    typedef boost::intrusive::list<
        Message, boost::intrusive::member_hook<Message, boost::intrusive::list_member_hook<>, &Message::dispatch_q>>
        Queue;
...
protected:
    ~Message() override
    {
        if (byte_throttler) byte_throttler->put(payload.length() + middle.length() + data.length());
        release_message_throttle();
        trace.event("message destructed");
        /* call completion hooks (if any) */
        if (completion_hook) completion_hook->complete(0);
    }
...
public:
    ...
    void clear_data()
    {
        if (byte_throttler) byte_throttler->put(data.length());
        data.clear();
        clear_buffers();   // let subclass drop buffers as well
    }
    void release_message_throttle()
    {
        if (msg_throttler) msg_throttler->put();
    ...
    {
        if (byte_throttler) byte_throttler->put(payload.length());
        payload = std::move(bl);
        if (byte_throttler) byte_throttler->take(payload.length());
    }

    void set_middle(ceph::buffer::list& bl)
    {
        if (byte_throttler) byte_throttler->put(middle.length());
        middle = std::move(bl);
        if (byte_throttler) byte_throttler->take(middle.length());
    }
    ...
    void set_data(const ceph::buffer::list& bl)
    {
        if (byte_throttler) byte_throttler->put(data.length());
        data.share(bl);
        if (byte_throttler) byte_throttler->take(data.length());
    }
    ...
    {
        header.crc = ceph_crc32c(0, (unsigned char*)&header, sizeof(header) - sizeof(header.crc));
    }
    void calc_front_crc()
    {
    ...
    // source/dest
    entity_inst_t get_source_inst() const { return entity_inst_t(get_source(), get_source_addr()); }
    entity_name_t get_source() const { return entity_name_t(header.src); }
    entity_addr_t get_source_addr() const
    {
        if (connection) return connection->get_peer_addr();
        return entity_addr_t();
    }
    entity_addrvec_t get_source_addrs() const
    {
        if (connection) return connection->get_peer_addrs();
    ...
    virtual void print(std::ostream& out) const { out << get_type_name() << " magic: " << magic; }
...
extern Message* decode_message(CephContext* cct, int crcflags, ceph_msg_header& header, ceph_msg_footer& footer,
                               ceph::buffer::list& front, ceph::buffer::list& middle, ceph::buffer::list& data,
                               Message::ConnectionRef conn);
inline std::ostream& operator<<(std::ostream& out, const Message& m)
{
    m.print(out);
    if (m.get_header().version) out << " v" << m.get_header().version;
    return out;
}

extern void encode_message(Message* m, uint64_t features, ceph::buffer::list& bl);
extern Message* decode_message(CephContext* cct, int crcflags, ceph::buffer::list::const_iterator& bl);
...
public:
    ...
    bool is_a_client() const { return get_connection()->get_peer_type() == CEPH_ENTITY_TYPE_CLIENT; }
...
namespace ceph {
template<class T, typename... Args> ceph::ref_t<T> make_message(Args&&... args)
{
...
namespace crimson {
template<class T, typename... Args> MURef<T> make_message(Args&&... args)
{
