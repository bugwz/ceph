...
{
    ldout(cct, 1) << "<== " << m->get_source_inst() << " " << m->get_seq() << " ==== " << *m
                  << " ==== " << m->get_payload().length() << "+" << m->get_middle().length() << "+"
                  << m->get_data().length() << " (" << ceph_con_mode_name(m->get_connection()->get_con_mode()) << " "
                  << m->get_footer().front_crc << " " << m->get_footer().middle_crc << " " << m->get_footer().data_crc
                  << ")"
                  << " " << m << " con " << m->get_connection() << dendl;
...
void DispatchQueue::local_delivery(const ref_t<Message>& m, int priority)
{
    ...
    if (local_messages.empty()) local_delivery_cond.notify_all();
...
{
    if (msize) {
        ldout(cct, 10) << __func__ << " " << msize << " to dispatch throttler " << dispatch_throttler.get_current()
...
void DispatchQueue::entry()
{
    ...
    while (true) {
        while (!mqueue.empty()) {
            ...
            if (!qitem.is_code()) remove_arrival(qitem.get_message());
            l.unlock();

            if (qitem.is_code()) {
                if (cct->_conf->ms_inject_internal_delays && cct->_conf->ms_inject_delay_probability &&
                ...
                case D_BAD_REMOTE_RESET: msgr->ms_deliver_handle_remote_reset(qitem.get_connection()); break;
                case D_CONNECT: msgr->ms_deliver_handle_connect(qitem.get_connection()); break;
                case D_ACCEPT: msgr->ms_deliver_handle_accept(qitem.get_connection()); break;
                case D_BAD_RESET: msgr->ms_deliver_handle_reset(qitem.get_connection()); break;
                case D_CONN_REFUSED: msgr->ms_deliver_handle_refused(qitem.get_connection()); break;
                default: ceph_abort();
                }
            }
            else {
