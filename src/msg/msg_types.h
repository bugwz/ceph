...
public:
    ...
    const char* type_str() const { return ceph_entity_type_name(type()); }
...
{
    size_t operator()(const entity_name_t& m) const { return rjhash32(m.type() ^ m.num()); }
};
}   // namespace std
...
static inline void encode(const sockaddr_storage& a, ceph::buffer::list& bl)
{
    ...
    const auto copy_size = std::min((unsigned char*)(&a + 1) - src, (unsigned char*)(&ss + 1) - dst);
...
static inline void decode(sockaddr_storage& a, ceph::buffer::list::const_iterator& bl)
{
#if defined(__linux__)
    ...
    auto const copy_size = std::min((unsigned char*)(&ss + 1) - src, (unsigned char*)(&a + 1) - dst);
    ::memcpy(dst, src, copy_size);
#elif defined(_WIN32)
    ceph_sockaddr_storage ss{};
    decode(ss, bl);
...
struct entity_addr_t
{
    ...
    bool set_sockaddr(const struct sockaddr* sa)
    {
        ...
        case AF_INET6:
            ...
            break;
        case AF_UNSPEC: memset(&u, 0, sizeof(u)); break;
    ...
    {
        switch (u.sa.sa_family) {
        case AF_INET: u.sin.sin_port = htons(port); break;
        case AF_INET6: u.sin6.sin6_port = htons(port); break;
        default: ceph_abort();
        }
    }
    int get_port() const
    {
        switch (u.sa.sa_family) {
        case AF_INET: return ntohs(u.sin.sin_port);
        case AF_INET6: return ntohs(u.sin6.sin6_port);
        }
        return 0;
    }
    ...
    bool is_same_host(const entity_addr_t& o) const
    {
        ...
        if (u.sa.sa_family == AF_INET6)
            return memcmp(u.sin6.sin6_addr.s6_addr, o.u.sin6.sin6_addr.s6_addr, sizeof(u.sin6.sin6_addr.s6_addr)) == 0;
        return false;
    }
    ...
    {
        switch (u.sa.sa_family) {
        case AF_INET: return u.sin.sin_addr.s_addr == INADDR_ANY;
        case AF_INET6: return memcmp(&u.sin6.sin6_addr, &in6addr_any, sizeof(in6addr_any)) == 0;
        default: return true;
        }
    }
    ...
    void decode(ceph::buffer::list::const_iterator& bl)
    {
        ...
        if (marker != 1) throw ceph::buffer::malformed_input("entity_addr_t marker != 1");
        DECODE_START(1, bl);
        decode(type, bl);
        decode(nonce, bl);
        __u32 elen;
...
struct entity_addrvec_t
{
    ...
    entity_addrvec_t() {}
    explicit entity_addrvec_t(const entity_addr_t& a)
        : v({a})
    {}
    ...
    entity_addr_t legacy_addr() const { return addr_of_type(entity_addr_t::TYPE_LEGACY); }
    ...
    std::string get_legacy_str() const { return legacy_or_front_addr().get_legacy_str(); }

    entity_addr_t msgr2_addr() const { return addr_of_type(entity_addr_t::TYPE_MSGR2); }
    ...
    entity_addr_t pick_addr(uint32_t type) const
    {
        ...
        switch (type) {
        case entity_addr_t::TYPE_LEGACY: [[fallthrough]];
        case entity_addr_t::TYPE_MSGR2: picked_addr = addr_of_type(type); break;
        case entity_addr_t::TYPE_ANY: return front();
        default: return {};
        }
    ...
    bool legacy_equals(const entity_addrvec_t& o) const
    {
        ...
        if (v.size() == 1 && front().is_legacy() && front() == o.legacy_addr()) {
            return true;
        }
        if (o.v.size() == 1 && o.front().is_legacy() && o.front() == legacy_addr()) {
            return true;
        }
        return false;
    }
    ...
    friend bool operator==(const entity_addrvec_t& l, const entity_addrvec_t& r) { return l.v == r.v; }
    friend bool operator!=(const entity_addrvec_t& l, const entity_addrvec_t& r) { return l.v != r.v; }
...
struct entity_inst_t
{
    ...
    entity_addr_t addr;
    entity_inst_t() {}
    entity_inst_t(entity_name_t n, const entity_addr_t& a)
        : name(n)
        , addr(a)
    {}
