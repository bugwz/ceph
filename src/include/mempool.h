...
#define DEFINE_MEMORY_POOLS_HELPER(f)                                                                              \
    f(bloom_filter) f(bluestore_alloc) f(bluestore_cache_data) f(bluestore_cache_onode) f(bluestore_cache_meta)    \
        f(bluestore_cache_other) f(bluestore_cache_buffer) f(bluestore_extent) f(bluestore_blob)                   \
            f(bluestore_shared_blob) f(bluestore_inline_bl) f(bluestore_fsck) f(bluestore_txc)                     \
                f(bluestore_writing_deferred) f(bluestore_writing) f(bluefs) f(bluefs_file_reader)                 \
                    f(bluefs_file_writer) f(buffer_anon) f(buffer_meta) f(osd) f(osd_mapbl) f(osd_pglog) f(osdmap) \
                        f(osdmap_mapping) f(pgmap) f(mds_co) f(unittest_1) f(unittest_2)
...
{
    std::size_t operator()(const std::type_info& k) const { return k.hash_code(); }
};
...
public:
    ...
    template<typename U> pool_allocator(const pool_allocator<pool_ix, U>&) { init(false); }
...
#define P(x)                                                                                                      \
    ...
    template<typename v> using pool_allocator = mempool::pool_allocator<id, v>;                                   \
                                                                                                                  \
    ...
    using map = std::map<k, v, cmp, pool_allocator<std::pair<const k, v>>>;                                       \
                                                                                                                  \
    template<typename k, typename v, typename cmp = std::less<k>>                                                 \
    using compact_map = compact_map<k, v, cmp, pool_allocator<std::pair<const k, v>>>;                            \
                                                                                                                  \
    template<typename k, typename v, typename cmp = std::less<k>>                                                 \
    using compact_multimap = compact_multimap<k, v, cmp, pool_allocator<std::pair<const k, v>>>;                  \
                                                                                                                  \
    template<typename k, typename cmp = std::less<k>> using compact_set = compact_set<k, cmp, pool_allocator<k>>; \
                                                                                                                  \
    template<typename k, typename v, typename cmp = std::less<k>>                                                 \
    using multimap = std::multimap<k, v, cmp, pool_allocator<std::pair<const k, v>>>;                             \
                                                                                                                  \
    template<typename k, typename cmp = std::less<k>> using set = std::set<k, cmp, pool_allocator<k>>;            \
                                                                                                                  \
    template<typename k, typename cmp = std::less<k>>                                                             \
    using flat_set = boost::container::flat_set<k, cmp, pool_allocator<k>>;                                       \
                                                                                                                  \
    template<typename k, typename v, typename cmp = std::less<k>>                                                 \
    using flat_map = boost::container::flat_map<k, v, cmp, pool_allocator<std::pair<k, v>>>;                      \
                                                                                                                  \
    template<typename v> using list = std::list<v, pool_allocator<v>>;                                            \
                                                                                                                  \
    template<typename v> using vector = std::vector<v, pool_allocator<v>>;                                        \
                                                                                                                  \
    template<typename k, typename v, typename h = std::hash<k>, typename eq = std::equal_to<k>>                   \
    using unordered_map = std::unordered_map<k, v, h, eq, pool_allocator<std::pair<const k, v>>>;                 \
                                                                                                                  \
...
#define MEMPOOL_CLASS_HELPERS()                \
    ...
    void operator delete[](void*)              \
    {                                          \
        ceph_abort_msg("no array delete");     \
    }
