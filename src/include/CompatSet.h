...
struct CompatSet
{
    ...
    public:
        ...
        bool contains(const Feature& f) const { return names.count(f.id); }
        bool contains(uint64_t f) const { return names.count(f); }
        /**
        ...
        void remove(const Feature& f) { remove(f.id); }
    ...
    {
        return readable(other) && !((other.ro_compat.mask ^ ro_compat.mask) & other.ro_compat.mask);
    }
    ...
    {
        if ((other.compat.mask == compat.mask) && (other.ro_compat.mask == ro_compat.mask) &&
        ...
        if (writeable(other) && !((other.compat.mask ^ compat.mask) & other.compat.mask)) return 1;
    ...
    {
        CompatSet diff;
        uint64_t other_compat = ((other.compat.mask ^ compat.mask) & other.compat.mask);
        uint64_t other_ro_compat = ((other.ro_compat.mask ^ ro_compat.mask) & other.ro_compat.mask);
        uint64_t other_incompat = ((other.incompat.mask ^ incompat.mask) & other.incompat.mask);
    ...
    bool merge(CompatSet const& other)
    {
        ...
        uint64_t other_ro_compat = ((other.ro_compat.mask ^ ro_compat.mask) & other.ro_compat.mask);
        uint64_t other_incompat = ((other.incompat.mask ^ incompat.mask) & other.incompat.mask);
        if (!other_compat && !other_ro_compat && !other_incompat) return false;
        for (int id = 1; id < 64; ++id) {
            uint64_t mask = (uint64_t)1 << id;
            if (mask & other_compat) {
...
{
    return out << "compat=" << compat.compat << ",rocompat=" << compat.ro_compat << ",incompat=" << compat.incompat;
}
