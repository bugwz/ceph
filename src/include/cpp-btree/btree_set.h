...
class btree_set : public internal::btree_set_container<
                      internal::btree<internal::set_params<Key, Compare, Alloc, /*TargetNodeSize=*/256,
                                                           /*Multi=*/false>>>
{
...
template<typename K, typename C, typename A, typename Pred> void erase_if(btree_set<K, C, A>& set, Pred pred)
{
...
//
template<typename Key, typename Compare = std::less<Key>, typename Alloc = std::allocator<Key>>
class btree_multiset : public internal::btree_multiset_container<
                           internal::btree<internal::set_params<Key, Compare, Alloc, /*TargetNodeSize=*/256,
                                                                /*Multi=*/true>>>
{
...
//
// Erases all elements that satisfy the predicate pred from the container.
template<typename K, typename C, typename A, typename Pred> void erase_if(btree_multiset<K, C, A>& set, Pred pred)
{
    for (auto it = set.begin(); it != set.end();) {
        if (pred(*it)) {
            it = set.erase(it);
