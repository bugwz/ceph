...
using btree_is_key_compare_to = std::is_signed<std::invoke_result_t<Compare, T, T>>;

template<typename T> using compare_to_t = decltype(std::declval<T&>().compare(std::declval<const T&>()));
template<typename T> inline constexpr bool has_compare_to = std::experimental::is_detected_v<compare_to_t, T>;
...
// default.
template<typename Compare, typename = void> struct key_compare_to_adapter
{
    using type = Compare;
};

template<typename K> struct key_compare_to_adapter<std::less<K>, std::enable_if_t<has_compare_to<K>>>
{
    struct type
    {
        inline int operator()(const K& lhs, const K& rhs) const noexcept { return lhs.compare(rhs); }
    };
};

template<typename K> struct key_compare_to_adapter<std::less<K>, std::enable_if_t<std::is_signed_v<K>>>
{
    struct type
    {
        inline K operator()(const K& lhs, const K& rhs) const noexcept { return lhs - rhs; }
    };
};

template<typename K> struct key_compare_to_adapter<std::less<K>, std::enable_if_t<std::is_unsigned_v<K>>>
{
    struct type
    {
...
template<typename Key, typename Compare, typename Alloc, int TargetNodeSize, int ValueSize, bool Multi>
struct common_params
{
    ...
    // would be 0.
    constexpr static int kNodeValueSpace = TargetNodeSize - /*minimum overhead=*/(sizeof(void*) + 4);

    // This is an integral type large enough to hold as many
    // ValueSize-values as will fit a node of TargetNodeSize bytes.
    using node_count_type =
        std::conditional_t<(kNodeValueSpace / ValueSize > (std::numeric_limits<uint8_t>::max)()), uint16_t, uint8_t>;
};
...
template<class K, class V> void swap(map_slot_type<K, V>& lhs, map_slot_type<K, V>& rhs)
{
...
struct map_params : common_params<Key, Compare, Alloc, TargetNodeSize, sizeof(Key) + sizeof(Data), Multi>
{
    ...
    template<class... Args> static void construct(Alloc* alloc, slot_type* slot, Args&&... args)
    {
        std::allocator_traits<Alloc>::construct(*alloc, &slot->mutable_value, std::forward<Args>(args)...);
    }
    // Construct this slot by moving from another slot.
    static void construct(Alloc* alloc, slot_type* slot, slot_type* other)
    {
        emplace(slot);
        std::allocator_traits<Alloc>::construct(*alloc, &slot->value, std::move(other->value));
    }
...
struct set_params : public common_params<Key, Compare, Alloc, TargetNodeSize, sizeof(Key), Multi>
{
    ...
    static void move(Alloc* alloc, slot_type* src, slot_type* dest) { *dest = std::move(*src); }
    static void destroy(Alloc* alloc, slot_type* slot) { std::allocator_traits<Alloc>::destroy(*alloc, slot); }
...
template<typename Result> constexpr bool compare_result_as_less_than(const Result r)
{
...
template<typename Compare> struct upper_bound_adapter
{
    explicit upper_bound_adapter(const Compare& c)
        : comp(c)
    {}
    template<typename K, typename LK> bool operator()(const K& a, const LK& b) const
    {
...
template<typename V, bool IsCompareTo> struct SearchResult
{
    V value;
    MatchKind match;
...
// useful information.
template<typename V> struct SearchResult<V, false>
{
    V value;
...
public:
    ...
    using use_linear_search = std::integral_constant<bool, std::is_arithmetic_v<key_type> &&
...
private:
    ...
    constexpr static size_type MinimumOverhead() { return SizeWithNValues(1) - sizeof(value_type); }
    ...
    constexpr static size_type LeafSize(const int max_values = kNodeValues) { return SizeWithNValues(max_values); }
    constexpr static size_type InternalSize() { return sizeof(internal_fields); }

    template<auto MemPtr> auto& GetField() { return reinterpret_cast<internal_fields*>(this)->*MemPtr; }

    template<auto MemPtr> auto& GetField() const { return reinterpret_cast<const internal_fields*>(this)->*MemPtr; }
...
public:
    ...
    {
        static_assert(alignof(leaf_fields) == alignof(internal_fields), "Alignment of all nodes must be equal.");
    ...
    field_type max_count() const
    {
        ...
        return max_count == field_type{kInternalNodeMaxCount} ? field_type{kNodeValues} : max_count;
    }
    ...
    template<typename K> int upper_bound(const K& k, const key_compare& comp) const
    {
    ...
    template<typename... Args> void emplace_value(size_type i, allocator_type* alloc, Args&&... args);
    ...
    void remove_values_ignore_children(int i, int to_erase, allocator_type* alloc);

    // Rebalances a node with its right sibling.
    void rebalance_right_to_left(const int to_move, btree_node* right, allocator_type* alloc);
    void rebalance_left_to_right(const int to_move, btree_node* right, allocator_type* alloc);
    ...
    static btree_node* init_leaf(btree_node* n, btree_node* parent, int max_count)
    {
        n->set_parent(parent);
        n->set_position(0);
        n->set_count(0);
...
private:
    template<typename... Args> void value_init(const size_type i, allocator_type* alloc, Args&&... args)
    {
        params_type::construct(alloc, slot(i), std::forward<Args>(args)...);
    }
    void value_destroy(const size_type i, allocator_type* alloc) { params_type::destroy(alloc, slot(i)); }
...
private:
    template<typename P> friend class btree;
    template<typename N, typename R, typename P> friend struct btree_iterator;
};

template<typename Node, typename Reference, typename Pointer> struct btree_iterator
{
private:
...
private:
    template<typename Params> friend class btree;
    template<typename Tree> friend class btree_container;
    template<typename Tree> friend class btree_set_container;
    template<typename Tree> friend class btree_map_container;
    template<typename Tree> friend class btree_multiset_container;
    template<typename N, typename R, typename P> friend struct btree_iterator;
...
template<size_t Alignment, class Alloc> class AlignedAlloc
{
    struct alignas(Alignment) M
    {};
    using alloc_t = typename std::allocator_traits<Alloc>::template rebind_alloc<M>;
    using traits_t = typename std::allocator_traits<Alloc>::template rebind_traits<M>;
    static constexpr size_t num_aligned_objects(size_t size) { return (size + sizeof(M) - 1) / sizeof(M); }

public:
    static void* allocate(Alloc* alloc, size_t size)
    {
        alloc_t aligned_alloc(*alloc);
        void* p = traits_t::allocate(aligned_alloc, num_aligned_objects(size));
    ...
    {
        alloc_t aligned_alloc(*alloc);
        traits_t::deallocate(aligned_alloc, static_cast<M*>(p), num_aligned_objects(size));
    }
};

template<typename Params> class btree
{
    ...
    template<typename Base1, typename Base2, typename Data> struct empty_base_handle : public Base1, Base2
    {
...
private:
    ...
    template<typename Btree> void copy_or_move_values_in_order(Btree* x);
...
public:
    ...
    template<typename K> iterator lower_bound(const K& key) { return internal_end(internal_lower_bound(key)); }
    ...
    template<typename K> iterator upper_bound(const K& key) { return internal_end(internal_upper_bound(key)); }
    ...
    template<typename... Args> std::pair<iterator, bool> insert_unique(const key_type& key, Args&&... args);
    ...
    template<typename InputIterator> void insert_iterator_unique(InputIterator b, InputIterator e);

    // Inserts a value into the btree.
    template<typename ValueType> iterator insert_multi(const key_type& key, ValueType&& v);

    // Inserts a value into the btree.
    template<typename ValueType> iterator insert_multi(ValueType&& v)
    {
    ...
    template<typename ValueType> iterator insert_hint_multi(iterator position, ValueType&& v);

    // Insert a range of values into the btree.
    template<typename InputIterator> void insert_iterator_multi(InputIterator b, InputIterator e);
    ...
    template<typename K> size_type erase_unique(const K& key);

    // Erases all of the entries matching the specified key from the
    // btree. Returns the number of elements erased.
    template<typename K> size_type erase_multi(const K& key);

    // Finds the iterator corresponding to a key or returns end() if the key is
    // not present.
    template<typename K> iterator find(const K& key) { return internal_end(internal_find(key)); }
    template<typename K> const_iterator find(const K& key) const { return internal_end(internal_find(key)); }

    // Returns a count of the number of times the key appears in the btree.
    template<typename K> size_type count_unique(const K& key) const
    {
    ...
    // Returns a count of the number of times the key appears in the btree.
    template<typename K> size_type count_multi(const K& key) const
    {
    ...
    const key_compare& key_comp() const noexcept { return *static_cast<const key_compare*>(&root_); }
...
private:
    ...
    key_compare* mutable_key_comp() noexcept { return static_cast<key_compare*>(&root_); }

    node_type* rightmost() { return rightmost_; }
    const node_type* rightmost() const { return rightmost_; }
    ...
    // Allocator routines.
    allocator_type* mutable_allocator() noexcept { return static_cast<allocator_type*>(&root_); }
    const allocator_type& allocator() const noexcept { return *static_cast<const allocator_type*>(&root_); }

    node_type* allocate(const size_type size)
    {
        using aligned_alloc_t = AlignedAlloc<node_type::Alignment(), allocator_type>;
        return static_cast<node_type*>(aligned_alloc_t::allocate(mutable_allocator(), size));
    }
    ...
    template<typename K> SearchResult<iterator, is_key_compare_to::value> internal_locate(const K& key) const;
    ...
    template<typename K> iterator internal_lower_bound(const K& key) const;

    // Internal routine which implements upper_bound().
    template<typename K> iterator internal_upper_bound(const K& key) const;

    // Internal routine which implements find().
    template<typename K> iterator internal_find(const K& key) const;
...
inline void btree_node<P>::emplace_value(const size_type i, allocator_type* alloc, Args&&... args)
{
    ...
    if (i < count()) {
        ...
        std::copy_backward(std::make_move_iterator(slot(i)), std::make_move_iterator(slot(count() - 1)), slot(count()));
        value_destroy(i, alloc);
    }
...
template<typename P> inline void btree_node<P>::remove_value(const int i, allocator_type* alloc)
{
    if (!leaf() && count() > i + 1) {
...
template<typename P>
inline void btree_node<P>::remove_values_ignore_children(const int i, const int to_erase, allocator_type* alloc)
{
    assert(to_erase >= 0);
    std::copy(std::make_move_iterator(slot(i + to_erase)), std::make_move_iterator(slot(count())), slot(i));
...
template<typename P> void btree_node<P>::split(const int insert_position, btree_node* dest, allocator_type* alloc)
{
    assert(dest->count() == 0);
...
template<typename P> void btree_node<P>::merge(btree_node* src, allocator_type* alloc)
{
...
template<typename P> void btree_node<P>::swap(btree_node* x, allocator_type* alloc)
{
    using std::swap;
    assert(leaf() == x->leaf());
    ...
    // Swap the values.
    std::swap_ranges(smaller->slot(0), smaller->slot(smaller->count()), larger->slot(0));
...
template<typename N, typename R, typename P> void btree_iterator<N, R, P>::increment_slow()
{
    if (node->leaf()) {
...
template<typename N, typename R, typename P> void btree_iterator<N, R, P>::decrement_slow()
{
    if (node->leaf()) {
        assert(position <= -1);
        btree_iterator save(*this);
...
////
// btree methods
template<typename P> template<typename Btree> void btree<P>::copy_or_move_values_in_order(Btree* x)
{
...
template<typename P> constexpr bool btree<P>::static_assert_validation()
{
    ...
    static_assert(std::is_trivially_copyable_v<iterator>, "iterator not trivially copyable.");
    ...
    static_assert(std::is_same_v<compare_result_type, bool> || std::is_signed_v<compare_result_type>,
...
template<typename P> auto btree<P>::operator=(const btree& x) -> btree&
{
    if (this != &x) {
        clear();
...
template<typename P> auto btree<P>::operator=(btree&& x) noexcept -> btree&
{
    if (this != &x) {
        clear();
...
template<typename P> auto btree<P>::erase(iterator iter) -> iterator
{
...
template<typename P> auto btree<P>::rebalance_after_delete(iterator iter) -> iterator
{
...
template<typename P> auto btree<P>::erase(iterator begin, iterator end) -> std::pair<size_type, iterator>
{
    ...
    while (size_ > target_size) {
        if (begin.node->leaf()) {
            ...
            begin = erase_from_leaf_node(begin, std::min(remaining_to_erase, remaining_in_node));
        }
        else {
            begin = erase(begin);
        }
...
template<typename P> void btree<P>::erase_same_node(iterator begin, iterator end)
{
    ...
    node->remove_values_ignore_children(begin.position, to_erase, mutable_allocator());
...
template<typename P> auto btree<P>::erase_from_leaf_node(iterator begin, size_type to_erase) -> iterator
{
    node_type* node = begin.node;
    assert(node->leaf());
...
template<typename P> template<typename K> auto btree<P>::erase_unique(const K& key) -> size_type
{
...
template<typename P> template<typename K> auto btree<P>::erase_multi(const K& key) -> size_type
{
...
template<typename P> void btree<P>::swap(btree& x)
{
    using std::swap;
...
template<typename P> void btree<P>::merge_nodes(node_type* left, node_type* right)
{
...
template<typename P> template<typename IterType> inline IterType btree<P>::internal_last(IterType iter)
{
...
inline auto btree<P>::internal_emplace(iterator iter, Args&&... args) -> iterator
{
    ...
    if (iter.node->count() == max_count) {
        ...
        if (max_count < kNodeValues) {
            ...
            iter.node = new_leaf_root_node(std::min(kNodeValues, 2 * max_count));
...
template<typename P> template<typename K> auto btree<P>::internal_lower_bound(const K& key) const -> iterator
{
    iterator iter(const_cast<node_type*>(root()), 0);
    for (;;) {
        iter.position = iter.node->lower_bound(key, key_comp()).value;
...
template<typename P> template<typename K> auto btree<P>::internal_upper_bound(const K& key) const -> iterator
{
    iterator iter(const_cast<node_type*>(root()), 0);
    for (;;) {
...
template<typename P> template<typename K> auto btree<P>::internal_find(const K& key) const -> iterator
{
    auto res = internal_locate(key);
    if constexpr (res.has_match) {
        if (res.IsEq()) {
