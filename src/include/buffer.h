...
template<class T> struct nop_delete
{
    void operator()(T*) {}
};
...
template<class T> struct unique_leakable_ptr : public std::unique_ptr<T, ceph::nop_delete<T>>
{
...
private:
    ...
    template<bool is_const> class iterator_impl
    {
        ...
        iterator_impl(typename std::conditional<is_const, const ptr*, ptr*>::type p, size_t offset, bool d)
            : bp(p)
    ...
    public:
        ...
        const char* get_pos() { return pos; }
        const char* get_end() { return end_ptr; }

        size_t get_offset() { return pos - start; }

        bool end() const { return pos == end_ptr; }
    };
...
public:
    ...
    iterator begin(size_t offset = 0) { return iterator(this, offset, false); }
    const_iterator begin(size_t offset = 0) const { return const_iterator(this, offset, false); }
    const_iterator cbegin() const { return begin(); }
    const_iterator begin_deep(size_t offset = 0) const { return const_iterator(this, offset, true); }

    // misc
    bool is_aligned(unsigned align) const { return ((uintptr_t)c_str() & (align - 1)) == 0; }
    bool is_page_aligned() const { return is_aligned(CEPH_PAGE_SIZE); }
    bool is_n_align_sized(unsigned align) const { return (length() % align) == 0; }
    bool is_n_page_sized() const { return is_n_align_sized(CEPH_PAGE_SIZE); }
    bool is_partial() const { return have_raw() && (start() > 0 || end() < raw_length()); }
...
#if __cplusplus >= 201703L
    inline unsigned append(std::string_view s) { return append(s.data(), s.length()); }
#endif   // __cplusplus >= 201703L
...
public:
    ...
    template<class... Args> static std::unique_ptr<ptr_node, disposer> create(Args&&... args)
    {
...
public:
    ...
    public:
        template<class T> class buffers_iterator
        {
            typename std::conditional<std::is_const<T>::value, const ptr_hook*, ptr_hook*>::type cur;
        ...
        public:
            ...
            T& operator*() const { return *reinterpret_cast<T*>(cur); }
            T* operator->() const { return reinterpret_cast<T*>(cur); }
            ...
            template<class U> buffers_iterator& operator=(buffers_iterator<U>& other)
            {
                cur = other.cur;
                return *this;
            }

            bool operator==(const buffers_iterator& rhs) const { return cur == rhs.cur; }
            bool operator!=(const buffers_iterator& rhs) const { return !(*this == rhs); }
        };
        ...
        const_iterator begin() const { return _root.next; }
        const_iterator before_begin() const { return &_root; }
        const_iterator end() const { return &_root; }
        iterator begin() { return _root.next; }
        iterator before_begin() { return &_root; }
        iterator end() { return &_root; }

        reference front() { return reinterpret_cast<reference>(*_root.next); }
        reference back() { return reinterpret_cast<reference>(*_tail); }
        const_reference front() const { return reinterpret_cast<const_reference>(*_root.next); }
        const_reference back() const { return reinterpret_cast<const_reference>(*_tail); }
        ...
        void swap(buffers_t& other)
        {
            ...
            _root.next = other._root.next == &other._root ? &this->_root : other._root.next;
            other._root.next = copy_root.next == &_root ? &other._root : copy_root.next;
...
private:
    ...
    template<bool is_const> class CEPH_BUFFER_API iterator_impl
    {
    protected:
        ...
        typedef
            typename std::conditional<is_const, typename buffers_t::const_iterator, typename buffers_t::iterator>::type
                list_iter_t;
        bl_t* bl;
...
public:
    ...
    public:
        ~contiguous_appender() { flush_and_continue(); }

        size_t get_out_of_band_offset() const { return out_of_band_offset; }
        ...
        size_t get_logical_offset() const { return out_of_band_offset + (pos - space.bp_data); }
    };
    ...
    class contiguous_filler
    {
        ...
        contiguous_filler(char* const pos)
            : pos(pos)
        {}

    public:
        void advance(const unsigned len) { pos += len; }
        void copy_in(const unsigned len, const char* const src)
        {
            memcpy(pos, src, len);
            advance(len);
        }
        char* c_str() { return pos; }
    };
    // The contiguous_filler is supposed to be not costlier than a single
    // pointer. Keep it dumb, please.
    static_assert(sizeof(contiguous_filler) == sizeof(char*), "contiguous_filler should be no costlier than pointer");
    ...
    class page_aligned_appender
    {
        ...
        template<class Func> void _append_common(size_t len, Func&& impl_f)
        {
    ...
    public:
        ...
        {
            _append_common(entire_len, [buf, this](const size_t chunk_len) mutable {
        ...
        {
            _append_common(entire_len, [this](const size_t chunk_len) { bl.append_zero(chunk_len); });
        }
...
private:
    ...
    ptr& get_append_buffer() { return *_carriage; }
...
public:
    ...
    ~list() { _buffers.clear_and_dispose(); }
    ...
    size_t get_append_buffer_unused_tail_length() const { return _carriage->unused_tail_length(); }
    ...
    bool is_aligned_size_and_memory(unsigned align_size, unsigned align_memory) const;
    ...
    bool rebuild_aligned_size_and_memory(unsigned align_size, unsigned align_memory, unsigned max_buffers = 0);
    bool rebuild_page_aligned();

    void reserve(size_t prealloc);

    [[deprecated("in favor of operator=(list&&)")]] void claim(list& bl) { *this = std::move(bl); }
    void claim_append(list& bl);
    void claim_append(list&& bl) { claim_append(bl); }
    ...
    iterator begin(size_t offset = 0) { return iterator(this, offset); }
    iterator end() { return iterator(this, _len, _buffers.end(), 0); }

    const_iterator begin(size_t offset = 0) const { return const_iterator(this, offset); }
    const_iterator cbegin(size_t offset = 0) const { return begin(offset); }
    const_iterator end() const { return const_iterator(this, _len, _buffers.end(), 0); }

    void append(char c);
    void append(const char* data, unsigned len);
    void append(std::string s) { append(s.data(), s.length()); }
#if __cplusplus >= 201703L
    // To forcibly disambiguate between string and string_view in the
    // case of arrays
    template<std::size_t N> void append(const char (&s)[N]) { append(s, N); }
    void append(const char* s) { append(s, strlen(s)); }
    void append(std::string_view s) { append(s.data(), s.length()); }
#endif   // __cplusplus >= 201703L
    ...
    template<typename VectorT> void prepare_iov(VectorT* piov) const
    {
#ifdef __CEPH__
...
public:
    ...
    void update(const buffer::list& bl) { crc = bl.crc32c(crc); }
