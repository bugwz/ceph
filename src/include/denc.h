...
template<typename T, typename = void> struct denc_traits
{
...
template<typename T> inline constexpr bool denc_supported = denc_traits<T>::supported;
...
{
public:
    DencDumper(const char* name, const ceph::bufferlist::contiguous_appender& appender)
        : name{name}
        , appender{appender}
...
private:
    static bool do_sample()
    {
        ...
        i++;
        int bits = 0;
        for (unsigned t = i; t; bits++) t &= t - 1;
        return bits <= 2;
    }
    size_t space_size() const { return appender.get_logical_offset() - appender.get_out_of_band_offset(); }
    void dump() const
    {
        char fn[PATH_MAX];
        ::snprintf(fn, sizeof(fn), ENCODE_STRINGIFY(ENCODE_DUMP_PATH) "/%s__%d.%x", name, getpid(), i++);
...
#define DENC_DUMP_PRE(Type) DencDumper<Type> _denc_dumper{#Type, p};
#else
...
template<typename T> struct underlying_type<T, std::enable_if_t<std::is_enum_v<T>>>
{
...
template<typename T, is_const_iterator It> const T& get_pos_add(It& i)
{
...
template<typename T>
    requires _denc::is_any_of<_denc::underlying_type_t<T>, ceph_le64, ceph_le32, ceph_le16, uint8_t
...
struct denc_traits<T>
{
    ...
    static void bound_encode(const T& o, size_t& p, uint64_t f = 0) { p += sizeof(T); }
    template<class It>
        requires(!is_const_iterator<It>)
    ...
    template<is_const_iterator It> static void decode(T& o, It& p, uint64_t f = 0) { o = get_pos_add<T>(p); }
    static void decode(T& o, ceph::buffer::list::const_iterator& p) { p.copy(sizeof(T), reinterpret_cast<char*>(&o)); }
};
...
template<typename T>
    requires _denc::is_any_of<T, int16_t, uint16_t>
struct ExtType<T>
{
...
template<typename T>
    requires _denc::is_any_of<T, int32_t, uint32_t>
struct ExtType<T>
{
...
template<typename T>
    requires _denc::is_any_of<T, int64_t, uint64_t>
struct ExtType<T>
{
...
template<typename T> using ExtType_t = typename ExtType<T>::type;
}   // namespace _denc
...
struct denc_traits<T>
{
    ...
    static void bound_encode(const T& o, size_t& p, uint64_t f = 0) { p += sizeof(etype); }
    template<class It>
        requires(!is_const_iterator<It>)
    static void encode(const T& o, It& p, uint64_t f = 0)
    {
    ...
    template<is_const_iterator It> static void decode(T& o, It& p, uint64_t f = 0) { o = get_pos_add<etype>(p); }
...
template<typename T> inline void denc_varint(T v, size_t& p)
{
    p += sizeof(T) + 1;
}

template<typename T> inline void denc_varint(T v, ceph::buffer::list::contiguous_appender& p)
{
    uint8_t byte = v & 0x7f;
    v >>= 7;
    while (v) {
...
template<typename T> inline void denc_varint(T& v, ceph::buffer::ptr::const_iterator& p)
{
...
template<typename T, is_const_iterator It> inline void denc_signed_varint(T& v, It& p)
{
    int64_t i = 0;
    denc_varint(i, p);
    if (i & 1) {
...
inline void denc_varint_lowz(uint64_t v, ceph::buffer::list::contiguous_appender& p)
{
    int lowznib = v ? (std::countr_zero(v) / 4) : 0;
    if (lowznib > 3) lowznib = 3;
    v >>= lowznib * 4;
    v <<= 2;
    v |= lowznib;
    denc_varint(v, p);
...
template<typename T> inline void denc_varint_lowz(T& v, ceph::buffer::ptr::const_iterator& p)
{
    uint64_t i = 0;
    denc_varint(i, p);
    int lowznib = (i & 3);
...
template<typename T, is_const_iterator It> inline void denc_signed_varint_lowz(T& v, It& p)
{
    int64_t i = 0;
    denc_varint(i, p);
    int lowznib = (i & 6) >> 1;
...
template<is_const_iterator It> inline void denc_lba(uint64_t& v, It& p)
{
    ...
    case 3:
        ...
        shift = 20 + 28;
        break;
    case 7: v = (uint64_t)(word & 0x7ffffff8) >> 3; shift = 28;
    }
...
inline std::enable_if_t<traits::supported> denc(const T& o, size_t& p, uint64_t f = 0)
{
...
inline void denc(const T& o, It& p, uint64_t features = 0)
{
    if constexpr (traits::featured) {
...
inline void denc(T& o, It& p, uint64_t features = 0)
{
    if constexpr (traits::featured) {
...
namespace _denc {
template<typename T, typename = void> struct has_legacy_denc : std::false_type
{};
template<typename T>
struct has_legacy_denc<T, decltype(std::declval<T&>().decode(std::declval<ceph::buffer::list::const_iterator&>()))>
    : std::true_type
{
    static void decode(T& v, ceph::buffer::list::const_iterator& p) { v.decode(p); }
};
template<typename T> struct has_legacy_denc<T, std::enable_if_t<!denc_traits<T>::need_contiguous>> : std::true_type
{
    static void decode(T& v, ceph::buffer::list::const_iterator& p) { denc_traits<T>::decode(v, p); }
};
}   // namespace _denc

template<typename T, typename traits = denc_traits<T>, typename has_legacy_denc = _denc::has_legacy_denc<T>>
inline std::enable_if_t<traits::supported && has_legacy_denc::value> denc(T& o, ceph::buffer::list::const_iterator& p)
{
...
// std::string
//
template<typename A> struct denc_traits<std::basic_string<char, std::char_traits<char>, A>>
{
private:
...
public:
    ...
    static void bound_encode(const value_type& s, size_t& p, uint64_t f = 0) { p += sizeof(uint32_t) + s.size(); }
    template<class It> static void encode(const value_type& s, It& p, uint64_t f = 0)
    {
        denc((uint32_t)s.size(), p);
        memcpy(p.get_pos_add(s.size()), s.data(), s.size());
    }
    template<class It> static void decode(value_type& s, It& p, uint64_t f = 0)
    {
        uint32_t len;
        denc(len, p);
    ...
    template<class It> static void decode_nohead(size_t len, value_type& s, It& p)
    {
        s.clear();
        if (len) {
    ...
    static void decode_nohead(size_t len, value_type& s, ceph::buffer::list::const_iterator& p)
    {
        if (len) {
    ...
    template<class It>
        requires(!is_const_iterator<It>)
    static void encode_nohead(const value_type& s, It& p)
    {
        auto len = s.length();
...
// ceph::buffer::ptr
//
template<> struct denc_traits<ceph::buffer::ptr>
{
    static constexpr bool supported = true;
    static constexpr bool featured = false;
    static constexpr bool bounded = false;
    ...
    template<class It>
        requires(!is_const_iterator<It>)
    static void encode(const ceph::buffer::ptr& v, It& p, uint64_t f = 0)
    {
        denc((uint32_t)v.length(), p);
        p.append(v);
    }
    template<is_const_iterator It> static void decode(ceph::buffer::ptr& v, It& p, uint64_t f = 0)
    {
        uint32_t len;
        denc(len, p);
        v = p.get_ptr(len);
...
// ceph::buffer::list
//
template<> struct denc_traits<ceph::buffer::list>
{
    static constexpr bool supported = true;
    static constexpr bool featured = false;
    static constexpr bool bounded = false;
    ...
    static void encode(const ceph::buffer::list& v, ceph::buffer::list::contiguous_appender& p, uint64_t f = 0)
    {
        denc((uint32_t)v.length(), p);
        p.append(v);
    }
    ...
    static void encode_nohead(const ceph::buffer::list& v, ceph::buffer::list::contiguous_appender& p) { p.append(v); }
    static void decode_nohead(size_t len, ceph::buffer::list& v, ceph::buffer::ptr::const_iterator& p)
    {
        v.clear();
        if (len) {
    ...
    static void decode_nohead(size_t len, ceph::buffer::list& v, ceph::buffer::list::const_iterator& p)
    {
        v.clear();
        p.copy(len, v);
    }
...
struct denc_traits<std::pair<A, B>, std::enable_if_t<denc_supported<std::remove_const_t<A>> && denc_supported<B>>>
{
    ...
    static constexpr bool need_contiguous = (a_traits::need_contiguous || b_traits::need_contiguous);
    ...
    static void encode(const std::pair<A, B>& v, ceph::buffer::list::contiguous_appender& p, uint64_t f = 0)
    {
        if constexpr (featured) {
            denc(v.first, p, f);
            denc(v.second, p, f);
    ...
    static std::enable_if_t<!!sizeof(AA) && !need_contiguous> decode(std::pair<A, B>& v,
...
namespace _denc {
template<template<class...> class C, typename Details, typename... Ts> struct container_base
{
private:
...
public:
    ...
    template<typename U = T> static void bound_encode(const container& s, size_t& p, uint64_t f = 0)
    {
        p += sizeof(uint32_t);
    ...
    static void encode(const container& s, ceph::buffer::list::contiguous_appender& p, uint64_t f = 0)
    {
        denc((uint32_t)s.size(), p);
        if constexpr (traits::featured) {
            encode_nohead(s, p, f);
    ...
    static void decode(container& s, ceph::buffer::ptr::const_iterator& p, uint64_t f = 0)
    {
        uint32_t num;
        denc(num, p);
    ...
    template<typename U = T>
    static std::enable_if_t<!!sizeof(U) && !need_contiguous> decode(container& s, ceph::buffer::list::const_iterator& p)
    {
        uint32_t num;
        denc(num, p);
    ...
    // nohead
    static void encode_nohead(const container& s, ceph::buffer::list::contiguous_appender& p, uint64_t f = 0)
    {
        for (const T& e : s) {
            if constexpr (traits::featured) {
                denc(e, p, f);
    ...
    static void decode_nohead(size_t num, container& s, ceph::buffer::ptr::const_iterator& p, uint64_t f = 0)
    {
        s.clear();
    ...
    template<typename U = T>
    static std::enable_if_t<!!sizeof(U) && !need_contiguous> decode_nohead(size_t num, container& s,
                                                                           ceph::buffer::list::const_iterator& p)
    {
        s.clear();
        Details::reserve(s, num);
...
template<typename T> class container_has_reserve
{
    template<typename U, U> struct SFINAE_match;
    template<typename U> static std::true_type test(SFINAE_match<T (*)(typename T::size_type), &U::reserve>*);

    template<typename U> static std::false_type test(...);

public:
    static constexpr bool value = decltype(test<denc_traits<T>>(0))::value;
};
template<typename T> inline constexpr bool container_has_reserve_v = container_has_reserve<T>::value;


template<typename Container> struct container_details_base
{
...
template<typename Container> struct pushback_details : public container_details_base<Container>
{
    template<typename... Args> static void insert(Container& c, Args&&... args)
    {
...
struct denc_traits<std::list<T, Ts...>, typename std::enable_if_t<denc_traits<T>::supported>>
    : public _denc::container_base<std::list, _denc::pushback_details<std::list<T, Ts...>>, T, Ts...>
{};

template<typename T, typename... Ts>
struct denc_traits<std::vector<T, Ts...>, typename std::enable_if_t<denc_traits<T>::supported>>
    : public _denc::container_base<std::vector, _denc::pushback_details<std::vector<T, Ts...>>, T, Ts...>
{};

template<typename T, std::size_t N, typename... Ts>
struct denc_traits<boost::container::small_vector<T, N, Ts...>, typename std::enable_if_t<denc_traits<T>::supported>>
{
private:
...
namespace _denc {
template<typename Container> struct setlike_details : public container_details_base<Container>
{
    using T = typename Container::value_type;
    template<typename... Args> static void insert(Container& c, Args&&... args)
    {
...
template<typename T, typename... Ts>
struct denc_traits<std::set<T, Ts...>, std::enable_if_t<denc_traits<T>::supported>>
    : public _denc::container_base<std::set, _denc::setlike_details<std::set<T, Ts...>>, T, Ts...>
{};

template<typename T, typename... Ts>
struct denc_traits<boost::container::flat_set<T, Ts...>, std::enable_if_t<denc_traits<T>::supported>>
    : public _denc::container_base<boost::container::flat_set,
                                   _denc::setlike_details<boost::container::flat_set<T, Ts...>>, T, Ts...>
{};

namespace _denc {
template<typename Container> struct maplike_details : public container_details_base<Container>
{
    using T = typename Container::value_type;
    template<typename... Args> static void insert(Container& c, Args&&... args)
    {
...
struct denc_traits<std::map<A, B, Ts...>, std::enable_if_t<denc_traits<A>::supported && denc_traits<B>::supported>>
    : public _denc::container_base<std::map, _denc::maplike_details<std::map<A, B, Ts...>>, A, B, Ts...>
{};

template<typename A, typename B, typename... Ts>
struct denc_traits<boost::container::flat_map<A, B, Ts...>,
                   std::enable_if_t<denc_traits<A>::supported && denc_traits<B>::supported>>
    : public _denc::container_base<boost::container::flat_map,
                                   _denc::maplike_details<boost::container::flat_map<A, B, Ts...>>, A, B, Ts...>
{};

template<typename T, size_t N> struct denc_traits<std::array<T, N>, std::enable_if_t<denc_traits<T>::supported>>
{
private:
...
template<typename... Ts> struct denc_traits<std::tuple<Ts...>, std::enable_if_t<(denc_traits<Ts>::supported && ...)>>
{

private:
    static_assert(sizeof...(Ts) > 0, "Zero-length tuples are not supported.");
...
public:
    ...
    static constexpr bool need_contiguous = (denc_traits<Ts>::need_contiguous || ...);

    template<typename U = container>
    static std::enable_if_t<denc_traits<U>::featured> bound_encode(const container& s, size_t& p, uint64_t f)
    {
    ...
    template<typename U = container>
    static std::enable_if_t<!denc_traits<U>::featured> bound_encode(const container& s, size_t& p)
    {
        ceph::for_each(s, [&p](const auto& e) { denc(e, p); });
    }

    template<typename U = container>
    static std::enable_if_t<denc_traits<U>::featured> encode(const container& s,
                                                             ceph::buffer::list::contiguous_appender& p, uint64_t f)
    {
        ceph::for_each(s, [&p, f](const auto& e) {
            if constexpr (denc_traits<std::decay_t<decltype(e)>>::featured) {
                denc(e, p, f);
    ...
    template<typename U = container>
    static std::enable_if_t<!denc_traits<U>::need_contiguous> decode(container& s,
                                                                     ceph::buffer::list::const_iterator& p,
                                                                     uint64_t f = 0)
    {
        ceph::for_each(s, [&p](auto& e) { denc(e, p); });
...
// boost::optional<T>
//
template<typename T> struct denc_traits<boost::optional<T>, std::enable_if_t<denc_traits<T>::supported>>
{
    using traits = denc_traits<T>;
    ...
    static void bound_encode(const boost::optional<T>& v, size_t& p, uint64_t f = 0)
    {
        p += sizeof(bool);
        if (v) {
            if constexpr (featured) {
    ...
    static void encode(const boost::optional<T>& v, ceph::buffer::list::contiguous_appender& p, uint64_t f = 0)
    {
        denc((bool)v, p);
        if (v) {
            if constexpr (featured) {
    ...
    static void decode(boost::optional<T>& v, ceph::buffer::ptr::const_iterator& p, uint64_t f = 0)
    {
        bool x;
        denc(x, p, f);
        if (x) {
    ...
    template<typename U = T>
    static std::enable_if_t<!!sizeof(U) && !need_contiguous> decode(boost::optional<T>& v,
                                                                    ceph::buffer::list::const_iterator& p)
    {
        bool x;
        denc(x, p);
    ...
    template<typename U = T>
    static void encode_nohead(const boost::optional<T>& v, ceph::buffer::list::contiguous_appender& p, uint64_t f = 0)
    {
        if (v) {
            if constexpr (featured) {
                denc(*v, p, f);
    ...
    static void decode_nohead(bool num, boost::optional<T>& v, ceph::buffer::ptr::const_iterator& p, uint64_t f = 0)
    {
        if (num) {
            v = T();
            denc(*v, p, f);
...
template<> struct denc_traits<boost::none_t>
{
    static constexpr bool supported = true;
    static constexpr bool featured = false;
    static constexpr bool bounded = true;
    ...
    static void bound_encode(const boost::none_t& v, size_t& p) { p += sizeof(bool); }

    static void encode(const boost::none_t& v, ceph::buffer::list::contiguous_appender& p) { denc(false, p); }
};

//
// std::optional<T>
//
template<typename T> struct denc_traits<std::optional<T>, std::enable_if_t<denc_traits<T>::supported>>
{
    using traits = denc_traits<T>;
    ...
    static void bound_encode(const std::optional<T>& v, size_t& p, uint64_t f = 0)
    {
        p += sizeof(bool);
        if (v) {
            if constexpr (featured) {
    ...
    static void encode(const std::optional<T>& v, ceph::buffer::list::contiguous_appender& p, uint64_t f = 0)
    {
        denc((bool)v, p);
        if (v) {
            if constexpr (featured) {
    ...
    static void decode(std::optional<T>& v, ceph::buffer::ptr::const_iterator& p, uint64_t f = 0)
    {
        bool x;
        denc(x, p, f);
        if (x) {
    ...
    template<typename U = T>
    static std::enable_if_t<!!sizeof(U) && !need_contiguous> decode(std::optional<T>& v,
                                                                    ceph::buffer::list::const_iterator& p)
    {
        bool x;
        denc(x, p);
    ...
    static void encode_nohead(const std::optional<T>& v, ceph::buffer::list::contiguous_appender& p, uint64_t f = 0)
    {
        if (v) {
            if constexpr (featured) {
                denc(*v, p, f);
    ...
    static void decode_nohead(bool num, std::optional<T>& v, ceph::buffer::ptr::const_iterator& p, uint64_t f = 0)
    {
        if (num) {
            v = T();
            denc(*v, p, f);
...
template<> struct denc_traits<std::nullopt_t>
{
    static constexpr bool supported = true;
    static constexpr bool featured = false;
    static constexpr bool bounded = true;
    ...
    static void bound_encode(const std::nullopt_t& v, size_t& p) { p += sizeof(bool); }

    static void encode(const std::nullopt_t& v, ceph::buffer::list::contiguous_appender& p) { denc(false, p); }
};
...
#define _DECLARE_CLASS_DENC(T, b)                                                                    \
    ...
    {                                                                                                \
        ...
        static void encode(const T& v, ::ceph::buffer::list::contiguous_appender& p, uint64_t f = 0) \
        {                                                                                            \
            v.encode(p);                                                                             \
        }                                                                                            \
...
#define _DECLARE_CLASS_DENC_FEATURED(T, b)                                                       \
    ...
    {                                                                                            \
        ...
        static void encode(const T& v, ::ceph::buffer::list::contiguous_appender& p, uint64_t f) \
        {                                                                                        \
            v.encode(p, f);                                                                      \
        }                                                                                        \
        static void decode(T& v, ::ceph::buffer::ptr::const_iterator& p, uint64_t f = 0)         \
...
template<typename T, typename traits = denc_traits<T>>
constexpr std::enable_if_t<traits::supported && traits::bounded, size_t> encoded_sizeof_bounded()
{
    size_t p = 0;
...
template<typename T, typename traits = denc_traits<T>>
std::enable_if_t<traits::supported, size_t> encoded_sizeof(const T& t)
{
    size_t p = 0;
    traits::bound_encode(t, p);
    return p;
...
namespace ceph {
template<typename T, typename traits = denc_traits<T>>
inline std::enable_if_t<traits::supported && !traits::featured> encode(const T& o, ceph::buffer::list& bl,
...
template<typename T, typename traits = denc_traits<T>>
inline std::enable_if_t<traits::supported && !traits::need_contiguous> decode(T& o,
                                                                              ::ceph::buffer::list::const_iterator& p)
{
    if (p.end()) throw ::ceph::buffer::end_of_buffer();
...
// nohead variants
template<typename T, typename traits = denc_traits<T>>
inline std::enable_if_t<traits::supported && !traits::featured> encode_nohead(const T& o, ceph::buffer::list& bl)
{
    size_t len = 0;
    traits::bound_encode(o, len);
    auto a = bl.get_contiguous_appender(len);
...
template<typename T, typename traits = denc_traits<T>>
inline std::enable_if_t<traits::supported && !traits::featured> decode_nohead(size_t num, T& o,
                                                                              ceph::buffer::list::const_iterator& p)
{
    if (!num) return;
    if (p.end()) throw ceph::buffer::end_of_buffer();
...
#define DENC_HELPERS                                                                                 \
    /* bound_encode */                                                                               \
    static void _denc_start(size_t& p, __u8* struct_v, __u8* struct_compat, char**, uint32_t*)       \
    {                                                                                                \
        p += 2 + 4;                                                                                  \
    }                                                                                                \
    static void _denc_finish(size_t& p, __u8* struct_v, __u8* struct_compat, char**, uint32_t*) {}   \
    /* encode */                                                                                     \
    ...
    {                                                                                                \
        *(ceph_le32*)*len_pos =                                                                      \
            p.get_pos() - *len_pos - sizeof(uint32_t) + p.get_out_of_band_offset() - *start_oob_off; \
    }                                                                                                \
    /* decode */                                                                                     \
...
#define DENC(Type, v, p)                                            \
    ...
    friend std::enable_if_t<std::is_same_v<T, Type> || std::is_same_v<T, const Type>> _denc_friend(T& v, P& p)
...
#define DENC_FEATURED(Type, v, p, f)                                                                \
    ...
    template<typename T, typename P>                                                                \
    friend std::enable_if_t<std::is_same_v<T, Type> || std::is_same_v<T, const Type>> _denc_friend( \
        T& v, P& p, uint64_t f)
