...
template<class Key, class T, class Map> class compact_map_base
{
protected:
    std::unique_ptr<Map> map;
    void alloc_internal()
    {
        if (!map) map.reset(new Map);
    }
    void free_internal() { map.reset(); }
    template<class It> class const_iterator_base
    {
        ...
        const_iterator_base()
            : map(0)
        {}
        const_iterator_base(const compact_map_base* m)
            : map(m)
        {}
        const_iterator_base(const compact_map_base* m, const It& i)
            : map(m)
            , it(i)
        {}
    ...
    public:
        ...
        bool operator==(const const_iterator_base& o) const { return (map == o.map) && (!map->map || it == o.it); }
        ...
        const std::pair<const Key, T>& operator*() { return *it; }
        const std::pair<const Key, T>* operator->() { return it.operator->(); }
    };
    template<class It> class iterator_base
    {
    private:
        const compact_map_base* map;
        It it;
        iterator_base()
            : map(0)
        {}
        iterator_base(compact_map_base* m)
            : map(m)
        {}
        iterator_base(compact_map_base* m, const It& i)
            : map(m)
            , it(i)
        {}
        friend class compact_map_base;
    ...
    public:
        ...
        bool operator==(const iterator_base& o) const { return (map == o.map) && (!map->map || it == o.it); }
        ...
        std::pair<const Key, T>& operator*() { return *it; }
        std::pair<const Key, T>* operator->() { return it.operator->(); }
        operator const_iterator_base<It>() const { return const_iterator_base<It>(map, it); }
    };
...
public:
    ...
    public:
        ...
        {}
        iterator(compact_map_base* m)
            : iterator_base<typename Map::iterator>(m)
        {}
    ...
    public:
        ...
        {}
        const_iterator(const compact_map_base* m)
            : const_iterator_base<typename Map::const_iterator>(m)
        {}
    ...
    public:
        ...
        {}
        reverse_iterator(compact_map_base* m)
            : iterator_base<typename Map::reverse_iterator>(m)
        {}
    ...
    public:
        ...
        {}
        const_reverse_iterator(const compact_map_base* m)
            : const_iterator_base<typename Map::const_reverse_iterator>(m)
        {}
    ...
    bool operator!=(const compact_map_base& o) const { return !(*this == o); }
    size_t count(const Key& k) const { return map ? map->count(k) : 0; }
    ...
    size_t erase(const Key& k)
    {
        ...
        if (map->empty()) free_internal();
        return r;
    }
    void clear() { free_internal(); }
    void swap(compact_map_base& o) { map.swap(o.map); }
    ...
    {
        if (!map) return reverse_iterator(this);
    ...
    {
        if (!map) return const_iterator(this);
    ...
    {
        if (!map) return const_reverse_iterator(this);
...
template<class Key, class T, class Map>
inline void encode(const compact_map_base<Key, T, Map>& m, ceph::buffer::list& bl, uint64_t features)
{
