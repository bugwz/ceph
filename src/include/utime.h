...
public:
    ...
    utime_t(const struct ceph_timespec& v) { decode_timeval(&v); }
    ...
    template<typename Clock, typename std::enable_if_t<ceph::converts_to_timespec_v<Clock>>* = nullptr>
    explicit utime_t(const std::chrono::time_point<Clock>& t)
        : utime_t(Clock::to_timespec(t))
    {}   // forward to timespec ctor

    template<class Rep, class Period> explicit utime_t(const std::chrono::duration<Rep, Period>& dur)
    {
        ...
        tv.tv_nsec = std::max<common_t>(
            (std::chrono::duration_cast<std::chrono::nanoseconds>(dur) % std::chrono::seconds(1)).count(), 0);
    }
#if defined(WITH_SEASTAR)
    explicit utime_t(const seastar::lowres_system_clock::time_point& t)
    {
        ...
        tv.tv_nsec =
            std::chrono::duration_cast<std::chrono::nanoseconds>(t.time_since_epoch() % std::chrono::seconds(1))
                .count();
    }
    explicit operator seastar::lowres_system_clock::time_point() const noexcept
    {
        using clock_t = seastar::lowres_system_clock;
        return clock_t::time_point{std::chrono::duration_cast<clock_t::duration>(std::chrono::seconds{tv.tv_sec} +
    ...
    void padding_check()
    {
        static_assert(sizeof(utime_t) == sizeof(tv.tv_sec) + sizeof(tv.tv_nsec), "utime_t have padding");
    }
    ...
    std::ostream& gmtime(std::ostream& out, bool legacy_form = false) const
    {
        ...
        if (sec() < ((time_t)(60 * 60 * 24 * 365 * 10))) {
            ...
            out << std::setw(2) << bdt.tm_hour << ':' << std::setw(2) << bdt.tm_min << ':' << std::setw(2)
                << bdt.tm_sec;
            out << "." << std::setw(6) << usec();
            out << "Z";
        }
    ...
    std::ostream& gmtime_nsec(std::ostream& out) const
    {
        ...
        if (sec() < ((time_t)(60 * 60 * 24 * 365 * 10))) {
            ...
            gmtime_r(&tt, &bdt);
            out << std::setw(4) << (bdt.tm_year + 1900)   // 2007 -> '07'
                << '-' << std::setw(2) << (bdt.tm_mon + 1) << '-' << std::setw(2) << bdt.tm_mday << 'T' << std::setw(2)
                << bdt.tm_hour << ':' << std::setw(2) << bdt.tm_min << ':' << std::setw(2) << bdt.tm_sec;
            out << "." << std::setw(9) << nsec();
            out << "Z";
        }
        out.fill(oldfill);
    ...
    static int parse_date(const std::string& date, uint64_t* epoch, uint64_t* nsec, std::string* out_date = nullptr,
