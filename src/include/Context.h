 ...
 */
template<typename T> class GenContext
{
...
public:
    ...
    template<typename C> void complete(C&& t)
    {
        finish(std::forward<C>(t));
        delete this;
    }

    template<typename C> void operator()(C&& t) noexcept { complete(std::forward<C>(t)); }

    template<typename U = T>
    auto operator()() noexcept -> typename std::enable_if<std::is_default_constructible<U>::value, void>::type
    {
        complete(T{});
    }


    std::reference_wrapper<GenContext> func() { return std::ref(*this); }
};

template<typename T> using GenContextURef = std::unique_ptr<GenContext<T>>;
...
protected:
    ...
    // return true.
    virtual bool sync_finish(int r) { return false; }
...
public:
    ...
    void complete(boost::system::error_code ec) { complete(ceph::from_error_code(ec)); }
    void operator()(boost::system::error_code ec) noexcept { complete(ec); }

    void operator()() noexcept { complete({}); }

    std::reference_wrapper<Context> func() { return std::ref(*this); }
};

/**
 * Simple context holding a single object
 */
template<class T> class ContainerContext : public Context
{
    T obj;

public:
    ContainerContext(T& obj)
        : obj(obj)
    {}
    void finish(int r) override {}
};
template<typename T> ContainerContext<T>* make_container_context(T&& t)
{
    return new ContainerContext<T>(std::forward<T>(t));
}

template<class T> struct Wrapper : public Context
{
    Context* to_run;
    T val;
    Wrapper(Context* to_run, T val)
        : to_run(to_run)
        , val(val)
    {}
    void finish(int r) override
    {
        if (to_run) to_run->complete(r);
    }
};
struct RunOnDelete
{
    Context* to_run;
    RunOnDelete(Context* to_run)
        : to_run(to_run)
    {}
    ~RunOnDelete()
    {
        if (to_run) to_run->complete(0);
    }
};
typedef std::shared_ptr<RunOnDelete> RunOnDeleteRef;

template<typename T> class LambdaContext : public Context
{
public:
    LambdaContext(T&& t)
        : t(std::forward<T>(t))
...
template<typename T> LambdaContext<T>* make_lambda_context(T&& t)
{
    return new LambdaContext<T>(std::move(t));
}

template<typename F, typename T> struct LambdaGenContext : GenContext<T>
{
    F f;
    LambdaGenContext(F&& f)
        : f(std::forward<F>(f))
    {}
    void finish(T t) override { f(std::forward<T>(t)); }
};
template<typename T, typename F> GenContextURef<T> make_gen_lambda_context(F&& f)
{
 ...
 */
template<class C> inline void finish_contexts(CephContext* cct, C& finished, int result = 0)
{
    ...
    if (cct) mydout(cct, 10) << ls.size() << " contexts to finish with " << result << dendl;
    for (Context* c : ls) {
        if (cct) mydout(cct, 10) << "---- " << c << dendl;
        c->complete(result);
    }
}
...
struct C_Lock : public Context
{
    ceph::mutex* lock;
    Context* fin;
    C_Lock(ceph::mutex* l, Context* c)
        : lock(l)
        , fin(c)
    {}
    ~C_Lock() override { delete fin; }
    void finish(int r) override
    {
        if (fin) {
...
public:
    ...
    void add(ContextType* c) { contexts.push_back(c); }
    ...
    void finish(int r) override { finish_contexts(cct, contexts, r); }
    bool empty() { return contexts.empty(); }

    template<class C> static ContextType* list_to_context(C& cs)
    {
        if (cs.size() == 0) {
            return 0;
        }
            ...
            C_ContextsBase<ContextType, ContextInstanceType>* c(
                new C_ContextsBase<ContextType, ContextInstanceType>(0));
            c->take(cs);
            return c;
        }
    }
...
private:
    ...
    public:
        ...
        {
            if (gather) gather->sub_finish(this, 0);
        }
    };
...
public:
    ...
    ~C_GatherBase() { mydout(cct, 10) << "C_GatherBase " << this << ".delete" << dendl; }
...
public:
    ...
    {
        finisher = finisher_;
        if (c_gather) c_gather->set_finisher(finisher);
    }
...
template<class ContextType> class ContextFactory
{
public:
...
{
    return [fin = std::unique_ptr<Context>(c)](boost::system::error_code ec) mutable {
