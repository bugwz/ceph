                 ...
                 private detail::expected_default_ctor_base<T, E>
{
    ...
    template<class U = T, detail::enable_if_t<!std::is_void<U>::value>* = nullptr> U& val() { return this->m_val; }
    unexpected<E>& err() { return this->m_unexpect; }

    template<class U = T, detail::enable_if_t<!std::is_void<U>::value>* = nullptr> const U& val() const
    {
        return this->m_val;
    }
...
public:
    ...
    /// \group and_then
    /// \synopsis template <class F>\nconstexpr auto and_then(F &&f) const &;
    template<class F> constexpr auto and_then(F&& f) const& -> decltype(and_then_impl(*this, std::forward<F>(f)))
    {
        return and_then_impl(*this, std::forward<F>(f));
    }
    ...
    /// \requires a value is stored
    /// \group deref
    template<class U = T, detail::enable_if_t<!std::is_void<U>::value>* = nullptr> constexpr const U& operator*() const&
    {
        return val();
    }
    /// \group deref
    ...
    /// \group value
    template<class U = T, detail::enable_if_t<!std::is_void<U>::value>* = nullptr> TL_EXPECTED_11_CONSTEXPR U& value() &
    {
        if (!has_value()) detail::throw_exception(bad_expected_access<E>(err().value()));
        return val();
    }
...
constexpr auto and_then_impl(Exp&& exp, F&& f)
{
    ...
    return exp.has_value() ? detail::invoke(std::forward<F>(f), *std::forward<Exp>(exp)) : Ret(unexpect, exp.error());
}
