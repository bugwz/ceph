...
template<bool Owning, bool Copyable, std::size_t Capacity> struct config
{
...
constexpr auto invoke(Callable&& callable,
                      Args&&... args) noexcept(noexcept(std::forward<Callable>(callable)(std::forward<Args>(args)...)))
...
/// SFINAE here.
template<typename T, typename Args, typename = void> struct can_invoke : std::false_type
{};
...
struct can_invoke<Pointer, identity<T&, Args...>,
                  decltype((void)((std::declval<T&>().*std::declval<Pointer>())(std::declval<Args>()...)))>
    : std::true_type
{};
template<typename Pointer, typename T, typename... Args>
struct can_invoke<Pointer, identity<T&&, Args...>,
...
struct can_invoke<Pointer, identity<T&>, decltype((void)(std::declval<T&>().*std::declval<Pointer>()))> : std::true_type
{};
template<typename Pointer, typename T>
...
template<bool RequiresNoexcept, typename T, typename Args> struct is_noexcept_correct : std::true_type
{};
template<typename T, typename... Args>
struct is_noexcept_correct<true, T, identity<Args...>>
    : std::integral_constant<bool, noexcept(invoke(std::declval<T>(), std::declval<Args>()...))>
{};
}   // end namespace invocation

namespace overloading {
template<typename... Args> struct overload_impl;
...
template<typename Current> struct overload_impl<Current> : Current
{
...
template<typename... T> constexpr auto overload(T&&... callables)
{
...
template<typename T, typename = void> struct address_taker
{
    template<typename O> static void* take(O&& obj) { return std::addressof(obj); }
    static T& restore(void* ptr) { return *static_cast<T*>(ptr); }
    static T const& restore(void const* ptr) { return *static_cast<T const*>(ptr); }
    static T volatile& restore(void volatile* ptr) { return *static_cast<T volatile*>(ptr); }
    static T const volatile& restore(void const volatile* ptr) { return *static_cast<T const volatile*>(ptr); }
};
/// Specialization to work with addresses of raw function pointers
template<typename T> struct address_taker<T, std::enable_if_t<std::is_pointer<T>::value>>
{
    template<typename O> static void* take(O&& obj) { return reinterpret_cast<void*>(obj); }
    template<typename O> static T restore(O ptr) { return reinterpret_cast<T>(const_cast<void*>(ptr)); }
};

template<typename Box> struct box_factory;
/// Store the allocator inside the box
template<bool IsCopyable, typename T, typename Allocator> struct box : private Allocator
{
...
template<typename T, typename Allocator> struct box<false, T, Allocator> : private Allocator
{
    friend box_factory<box>;
...
template<bool IsCopyable, typename T, typename Allocator> struct box_factory<box<IsCopyable, T, Allocator>>
{
...
template<bool IsCopyable, typename T, typename Allocator = std::allocator<std::decay_t<T>>>
auto make_box(std::integral_constant<bool, IsCopyable>, T&& value, Allocator&& allocator = Allocator{})
{
    return box<IsCopyable, std::decay_t<T>, std::decay_t<Allocator>>{std::forward<T>(value),
                                                                     std::forward<Allocator>(allocator)};
}

template<typename T> struct is_box : std::false_type
{};
template<bool IsCopyable, typename T, typename Allocator> struct is_box<box<IsCopyable, T, Allocator>> : std::true_type
{};
...
constexpr auto retrieve(std::true_type /*is_inplace*/, Accessor from, std::size_t from_capacity)
{
...
struct bad_function_call : std::exception
{
    ...
    char const* what() const noexcept override { return "bad function call"; }
};
#elif
...
#define FU2_DEFINE_FUNCTION_TRAIT(CONST, VOLATILE, NOEXCEPT, OVL_REF, REF)                                       \
    template<typename Ret, typename... Args> struct function_trait<Ret(Args...) CONST VOLATILE OVL_REF NOEXCEPT> \
    {                                                                                                            \
        using pointer_type = Ret (*)(data_accessor CONST VOLATILE*, std::size_t capacity, Args...);              \
        template<typename T, bool IsInplace> struct internal_invoker                                             \
        {                                                                                                        \
            ...
            {                                                                                                    \
                ...
                return invocation::invoke(                                                                       \
                    ...
                    std::forward<Args>(args)...);                                                                \
                                                                                                                 ...
                                                                                                                 \
        template<typename T> struct view_invoker                                                                 \
        {                                                                                                        \
            ...
            {                                                                                                    \
                ...
                return invocation::invoke(address_taker<T>::restore(ptr), std::forward<Args>(args)...);          \
                                                                                                                 ...
                                                                                                                 \
        template<typename T> using callable = T CONST VOLATILE REF;                                              \
                                                                                                                 \
        using arguments = identity<Args...>;                                                                     \
                                                                                                                 \
        using is_noexcept = is_noexcept_##NOEXCEPT;                                                              \
                                                                                                                 \
        template<bool Throws> struct empty_invoker                                                               \
        {                                                                                                        \
            static Ret invoke(data_accessor CONST VOLATILE* /*data*/, std::size_t /*capacity*/,                  \
                              Args... /*args*/) NOEXCEPT                                                         \
            {                                                                                                    \
...
template<typename Signature> using function_pointer_of = typename function_trait<Signature>::pointer_type;

template<typename... Args> struct invoke_table;

/// We optimize the vtable_t in case there is a single function overload
template<typename First> struct invoke_table<First>
{
    using type = function_pointer_of<First>;

    /// Return the function pointer itself
    template<std::size_t Index> static constexpr auto fetch(type pointer) noexcept
    {
    ...
    template<typename T, bool IsInplace> static constexpr type get_invocation_table_of() noexcept
    {
        return &function_trait<First>::template internal_invoker<T, IsInplace>::invoke;
    }
    /// Returns the thunk of an single overloaded callable
    template<typename T> static constexpr type get_invocation_view_table_of() noexcept
    {
        return &function_trait<First>::template view_invoker<T>::invoke;
    }
    /// Returns the thunk of an empty single overloaded callable
    template<bool IsThrowing> static constexpr type get_empty_invocation_table() noexcept
    {
...
template<typename First, typename Second, typename... Args> struct invoke_table<First, Second, Args...>
{
    using type =
        std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...> const*;

    /// Return the function pointer at the particular index
    template<std::size_t Index> static constexpr auto fetch(type table) noexcept { return std::get<Index>(*table); }
        ...
        : public std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...>
    {
        constexpr invocation_vtable() noexcept
            : std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...>(
                  std::make_tuple(&function_trait<First>::template internal_invoker<T, IsInplace>::invoke,
    ...
    template<typename T, bool IsInplace> static type get_invocation_table_of() noexcept
    {
        ...
        : public std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...>
    {
        constexpr invocation_view_vtable() noexcept
            : std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...>(
                  std::make_tuple(&function_trait<First>::template view_invoker<T>::invoke,
    ...
    /// Returns the thunk of an multi overloaded callable
    template<typename T> static type get_invocation_view_table_of() noexcept
    {
        ...
        : public std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...>
    {
        constexpr empty_vtable() noexcept
            : std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...>(
                  std::make_tuple(&function_trait<First>::template empty_invoker<IsThrowing>::invoke,
    ...
    template<bool IsThrowing> static type get_empty_invocation_table() noexcept
    {
...
#define FU2_DEFINE_FUNCTION_TRAIT(CONST, VOLATILE, NOEXCEPT, OVL_REF, REF)                                        \
    ...
    {                                                                                                             \
                                                                                                                  \
        template<std::size_t, typename, typename...> friend class operator_impl;                                  \
                                                                                                                  \
    protected:                                                                                                    \
        ...
        {                                                                                                         \
            ...
            using erasure_t = std::decay_t<decltype(parent->erasure_)>;                                           \
                                                                                                                  \
            return erasure_t::template invoke<Index>(static_cast<erasure_t CONST VOLATILE REF>(parent->erasure_), \
                                                     std::forward<Args>(args)...);                                \
        }                                                                                                         \
    };                                                                                                            \
...
/// optimized
template<typename Property> class vtable;
...
{
    using command_function_t = void (*)(vtable* /*this*/, opcode /*op*/, data_accessor* /*from*/,
    ...
    template<typename T> struct trait
    {
        ...
        static void process_cmd(vtable* to_table, opcode op, data_accessor* from, std::size_t from_capacity,
                                data_accessor* to, std::size_t to_capacity)
        {
            ...
            {
                auto box =
                    static_cast<T const*>(retrieve<T>(std::integral_constant<bool, IsInplace>{}, from, from_capacity));
                assert(box && "The object must not be over aligned or null!");
        ...
        static void construct(std::true_type /*apply*/, Box&& box, vtable* to_table, data_accessor* to,
        ...
        template<typename Box>
        static void construct(std::false_type /*apply*/, Box&& /*box*/, vtable* /*to_table*/, data_accessor* /*to*/,
                              std::size_t /*to_capacity*/) noexcept(HasStrongExceptGuarantee)
        {}
    };

    /// The command table
    static void empty_cmd(vtable* to_table, opcode op, data_accessor* /*from*/, std::size_t /*from_capacity*/,
                          data_accessor* to, std::size_t /*to_capacity*/)
    {
...
public:
    ...
    template<typename T> static void init(vtable& table, T&& object, data_accessor* to, std::size_t to_capacity)
    {
    ...
    void move(vtable& to_table, data_accessor* from, std::size_t from_capacity, data_accessor* to,
              std::size_t to_capacity) noexcept(HasStrongExceptGuarantee)
    {
    ...
    void copy(vtable& to_table, data_accessor const* from, std::size_t from_capacity, data_accessor* to,
    ...
    template<std::size_t Index, typename... Args> constexpr auto invoke(Args&&... args) const
    {
        auto thunk = invoke_table_t::template fetch<Index>(vtable_);
        return thunk(std::forward<Args>(args)...);
    }
    /// Invoke the function at the given index
    template<std::size_t Index, typename... Args> constexpr auto invoke(Args&&... args) const volatile
    {
        auto thunk = invoke_table_t::template fetch<Index>(vtable_);
        return thunk(std::forward<Args>(args)...);
    }

    template<typename T> void set_inplace() noexcept
    {
    ...
    template<typename T> void set_allocated() noexcept
    {
        using type = std::decay_t<T>;
...
/// control and vtable.
template<std::size_t Capacity, typename = void> struct internal_capacity
{
...
template<std::size_t Capacity> struct internal_capacity<Capacity, std::enable_if_t<(Capacity < sizeof(void*))>>
{
    typedef struct
    {
        /// Tag to access the structure in a type-safe way
...
template<std::size_t Capacity> class internal_capacity_holder
{
    // Tag to access the structure in a type-safe way
...
public:
    ...
    constexpr data_accessor* opaque_ptr() noexcept { return &storage_.accessor_; }
    constexpr data_accessor const* opaque_ptr() const noexcept { return &storage_.accessor_; }
    constexpr data_accessor volatile* opaque_ptr() volatile noexcept { return &storage_.accessor_; }
    constexpr data_accessor const volatile* opaque_ptr() const volatile noexcept { return &storage_.accessor_; }

    static constexpr std::size_t capacity() noexcept { return sizeof(storage_); }
};
...
{
    template<bool, typename, typename> friend class erasure;
    template<std::size_t, typename, typename...> friend class operator_impl;
...
public:
    ...
    static constexpr std::size_t capacity() noexcept { return internal_capacity_holder<Config::capacity>::capacity(); }

    constexpr erasure() noexcept { vtable_.set_empty(); }

    constexpr erasure(std::nullptr_t) noexcept { vtable_.set_empty(); }

    constexpr erasure(erasure&& right) noexcept(Property::is_strong_exception_guaranteed)
    {
    ...
    constexpr erasure(erasure<true, OtherConfig, Property> right) noexcept(Property::is_strong_exception_guaranteed)
    {
        right.vtable_.move(vtable_, right.opaque_ptr(), right.capacity(), this->opaque_ptr(), capacity());
    }
    ...
    constexpr erasure(T&& callable, Allocator&& allocator = Allocator{})
    {
        vtable_t::init(vtable_,
                       type_erasure::make_box(std::integral_constant<bool, Config::is_copyable>{},
    ...
    ~erasure() { vtable_.weak_destroy(this->opaque_ptr(), capacity()); }

    constexpr erasure& operator=(std::nullptr_t) noexcept(Property::is_strong_exception_guaranteed)
    {
        vtable_.destroy(this->opaque_ptr(), capacity());
        return *this;
    }

    constexpr erasure& operator=(erasure&& right) noexcept(Property::is_strong_exception_guaranteed)
    {
    ...
    template<typename OtherConfig>
    constexpr erasure&
    operator=(erasure<true, OtherConfig, Property> right) noexcept(Property::is_strong_exception_guaranteed)
    {
        vtable_.weak_destroy(this->opaque_ptr(), capacity());
        right.vtable_.move(vtable_, right.opaque_ptr(), right.capacity(), this->opaque_ptr(), capacity());
        return *this;
    ...
    template<typename T> constexpr erasure& operator=(T&& callable)
    {
        vtable_.weak_destroy(this->opaque_ptr(), capacity());
        vtable_t::init(
            vtable_,
            type_erasure::make_box(std::integral_constant<bool, Config::is_copyable>{}, std::forward<T>(callable)),
            this->opaque_ptr(),
            capacity());
        return *this;
    }

    template<typename T, typename Allocator> void assign(T&& callable, Allocator&& allocator)
    {
        vtable_.weak_destroy(this->opaque_ptr(), capacity());
        vtable_t::init(vtable_,
                       type_erasure::make_box(std::integral_constant<bool, Config::is_copyable>{},
...
public:
    ...
    template<typename OtherConfig> constexpr erasure& operator=(erasure<true, OtherConfig, property_t> right) noexcept
    {
        invoke_table_ = right.invoke_table_;
        view_ = right.view_;
        return *this;
    ...
    template<typename T> constexpr erasure& operator=(T&& object)
    {
        invoke_table_ = invoke_table_t::template get_invocation_view_table_of<std::decay_t<T>>();
    ...
    /// Returns true when the erasure doesn't hold any erased object
    constexpr bool empty() const noexcept { return view_.ptr_ == nullptr; }
...
struct accepts_one
    : std::integral_constant<
          bool, invocation::can_invoke<typename Trait::template callable<T>, typename Trait::arguments>::value &&
                    invocation::is_noexcept_correct<Trait::is_noexcept::value, typename Trait::template callable<T>,
                                                    typename Trait::arguments>::value>
{};

/// Deduces to a true_type if the type T provides all signatures
template<typename T, typename Signatures, typename = void> struct accepts_all : std::false_type
{};
...
{
    static_assert(!Config::is_copyable || std::is_copy_constructible<std::decay_t<T>>::value,
...
class function<Config, property<IsThrowing, HasStrongExceptGuarantee, Args...>>
    : type_erasure::invocation_table::operator_impl<
          0U, function<Config, property<IsThrowing, HasStrongExceptGuarantee, Args...>>, Args...>
{

    template<typename, typename> friend class function;

    template<std::size_t, typename, typename...> friend class type_erasure::invocation_table::operator_impl;
    ...
    template<typename Function, typename = void> struct is_convertible_to_this : std::false_type
    {};
    ...
    template<typename T> using enable_if_owning_t = std::enable_if_t<std::is_same<T, T>::value && Config::is_owning>;
...
public:
    ...
    template<typename RightConfig, std::enable_if_t<RightConfig::is_copyable>* = nullptr,
    ...
    template<typename RightConfig, enable_if_copyable_correct_t<Config, RightConfig>* = nullptr,
             enable_if_owning_correct_t<Config, RightConfig>* = nullptr>
    ...
    template<typename T,   //
             enable_if_not_convertible_to_this<T>* = nullptr, enable_if_can_accept_all_t<T>* = nullptr,
             assert_wrong_copy_assign_t<T>* = nullptr, assert_no_strong_except_guarantee_t<T>* = nullptr>
    constexpr function(T&& callable)
        : erasure_(std::forward<T>(callable))
    {}
    template<typename T, typename Allocator,   //
             enable_if_not_convertible_to_this<T>* = nullptr, enable_if_can_accept_all_t<T>* = nullptr,
             enable_if_owning_t<T>* = nullptr, assert_wrong_copy_assign_t<T>* = nullptr,
    ...
    constexpr function(std::nullptr_t np)
        : erasure_(np)
    {}
    ...
    bool empty() const noexcept { return erasure_.empty(); }
    ...
    friend void swap(function& left, function& right) noexcept(HasStrongExceptGuarantee) { left.swap(right); }
...
template<typename Config, typename Property> bool operator==(function<Config, Property> const& f, std::nullptr_t)
{
    return !bool(f);
}
...
template<typename Config, typename Property> bool operator==(std::nullptr_t, function<Config, Property> const& f)
{
    return !bool(f);
}
...
using default_capacity = std::integral_constant<std::size_t, object_size::value - (2 * sizeof(void*))>;
}   // namespace detail
}   // namespace abi_310
...
template<
    ...
    typename... Signatures>
using function_base = detail::function<detail::config<IsOwning, IsCopyable, Capacity>,
...
using function = function_base<true, true, detail::default_capacity::value, true, false, Signatures...>;

/// An owning non copyable function wrapper for arbitrary callable types.
template<typename... Signatures>
using unique_function = function_base<true, false, detail::default_capacity::value, true, false, Signatures...>;

/// A non owning copyable function wrapper for arbitrary callable types.
template<typename... Signatures>
using function_view = function_base<false, true, detail::default_capacity::value, true, false, Signatures...>;
