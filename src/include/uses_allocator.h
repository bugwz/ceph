...
namespace internal {
template<class T, class Tuple, std::size_t... Indexes> T make_from_tuple_imp(Tuple&& t, std::index_sequence<Indexes...>)
{
    return T(std::get<Indexes>(std::forward<Tuple>(t))...);
}
}   // namespace internal

template<class T, class Tuple> T make_from_tuple(Tuple&& args_tuple)
{
...
// Forward declaration
template<class T, class Alloc, class... Args> auto uses_allocator_construction_args(const Alloc& a, Args&&... args);

namespace internal {

template<class T, class A> struct has_allocator : std::uses_allocator<T, A>
{};

// Specialization of `has_allocator` for `std::pair`
template<class T1, class T2, class A>
struct has_allocator<std::pair<T1, T2>, A>
    : std::integral_constant<bool, has_allocator<T1, A>::value || has_allocator<T2, A>::value>
{};
...
auto uses_allocator_args_imp(Unused1 /* is_pair */, std::false_type /* has_allocator */,
                             Unused2 /* uses prefix allocator arg */, const Alloc& /* ignored */, Args&&... args)
{
...
auto uses_allocator_args_imp(std::false_type /* is_pair */, std::true_type /* has_allocator */,
                             std::true_type /* uses prefix allocator arg */, const Alloc& a, Args&&... args)
{
    // Allocator added to front of argument list, after `allocator_arg`.
    return std::tuple<std::allocator_arg_t, const Alloc&, Args&&...>(
        std::allocator_arg, a, std::forward<Args>(args)...);
}
...
auto uses_allocator_args_imp(std::false_type /* is_pair */, std::true_type /* has_allocator */,
                             std::false_type /* prefix allocator arg */, const Alloc& a, Args&&... args)
{
...
auto uses_allocator_args_imp(std::true_type /* is_pair */, std::true_type /* has_allocator */,
                             std::false_type /* prefix allocator arg */, const Alloc& a, std::piecewise_construct_t,
                             Tuple1&& x, Tuple2&& y)
{
    ...
    return std::make_tuple(
        ...
        std::apply(
            [&a](auto&&... args1) -> auto {
                return uses_allocator_construction_args<T1>(a, std::forward<decltype(args1)>(args1)...);
            },
            std::forward<Tuple1>(x)),
        std::apply(
            [&a](auto&&... args2) -> auto {
                return uses_allocator_construction_args<T2>(a, std::forward<decltype(args2)>(args2)...);
            },
...
auto uses_allocator_args_imp(std::true_type /* is_pair */, std::true_type /* has_allocator */,
                             std::false_type /* prefix allocator arg */, const Alloc& a)
{
    // using T1 = typename T::first_type;
    // using T2 = typename T::second_type;
    ...
    return uses_allocator_construction_args<T>(a, std::piecewise_construct, std::tuple<>{}, std::tuple<>{});
}
...
auto uses_allocator_args_imp(std::true_type /* is_pair */, std::true_type /* has_allocator */,
                             std::false_type /* prefix allocator arg */, const Alloc& a, const std::pair<U1, U2>& arg)
{
    // using T1 = typename T::first_type;
    // using T2 = typename T::second_type;
    ...
    return uses_allocator_construction_args<T>(
        a, std::piecewise_construct, std::forward_as_tuple(arg.first), std::forward_as_tuple(arg.second));
}
...
template<class T, class Alloc, class U1, class U2>
auto uses_allocator_args_imp(std::true_type /* is_pair */, std::true_type /* has_allocator */,
                             std::false_type /* prefix allocator arg */, const Alloc& a, std::pair<U1, U2>&& arg)
{
    // using T1 = typename T::first_type;
    // using T2 = typename T::second_type;
    ...
    return uses_allocator_construction_args<T>(a,
                                               std::piecewise_construct,
...
template<class T, class Alloc, class U1, class U2>
auto uses_allocator_args_imp(std::true_type /* is_pair */, std::true_type /* has_allocator */,
                             std::false_type /* prefix allocator arg */, const Alloc& a, U1&& arg1, U2&& arg2)
{
    // using T1 = typename T::first_type;
    // using T2 = typename T::second_type;
...
template<class T, class Alloc, class... Args> T make_obj_using_allocator(const Alloc& a, Args&&... args)
{
    return make_from_tuple<T>(uses_allocator_construction_args<T>(a, std::forward<Args>(args)...));
}

template<class T, class Alloc, class... Args>
T* uninitialized_construct_using_allocator(T* p, const Alloc& a, Args&&... args)
{
    return std::apply(
        [p](auto&&... args2) { return ::new (static_cast<void*>(p)) T(std::forward<decltype(args2)>(args2)...); },
