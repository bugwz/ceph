...
template<class T> class alloc_ptr
{
public:
    ...
    alloc_ptr(alloc_ptr<pointer>&& rhs)
        : ptr(std::move(rhs.ptr))
    {}
    alloc_ptr(const alloc_ptr<pointer>& rhs) = delete;
    alloc_ptr& operator=(const alloc_ptr<pointer>&& rhs) { ptr = rhs.ptr; }
    alloc_ptr& operator=(const alloc_ptr<pointer>& rhs) { ptr = rhs.ptr; }

    void swap(alloc_ptr<pointer>& rhs) { ptr.swap(rhs.ptr); }
    element_type* release() { return ptr.release(); }
    void reset(element_type* p = nullptr) { ptr.reset(p); }
    element_type* get() const
    {
        if (!ptr) ptr.reset(new element_type);
        return ptr.get();
    }
    ...
    friend bool operator<(const alloc_ptr& lhs, const alloc_ptr& rhs) { return std::less<element_type>(*lhs, *rhs); }
    friend bool operator<=(const alloc_ptr& lhs, const alloc_ptr& rhs)
    {
        return std::less_equal<element_type>(*lhs, *rhs);
    }
    friend bool operator>(const alloc_ptr& lhs, const alloc_ptr& rhs) { return std::greater<element_type>(*lhs, *rhs); }
    friend bool operator>=(const alloc_ptr& lhs, const alloc_ptr& rhs)
    {
        return std::greater_equal<element_type>(*lhs, *rhs);
    }
    friend bool operator==(const alloc_ptr& lhs, const alloc_ptr& rhs) { return *lhs == *rhs; }
    friend bool operator!=(const alloc_ptr& lhs, const alloc_ptr& rhs) { return *lhs != *rhs; }
