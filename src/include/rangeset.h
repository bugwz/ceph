...
template<class T> struct _rangeset_base
{
    ...
    mapit get_range_for(T val)
    {
        ...
        if (it == ranges.end()) {
            ...
            if (it->first <= val && it->second >= val) return ranges.find(it->first);
            return ranges.end();
        }
        else {
            if (it->first == val) return it--;
            if (it->first <= val && it->second >= val) return it;
            return ranges.end();
        }
    }
};
...
template<class T> class rangeset_iterator : public std::iterator<std::input_iterator_tag, T>
{
    // typedef typename map<T,T>::iterator mapit;
...
public:
    ...
    rangeset_iterator(typename map<T, T>::iterator& it, map<T, T>& ranges)
    {
        this->ranges = ranges;
        this->it = it;
        if (this->it != ranges.end()) current = it->first;
    }

    bool operator==(rangeset_iterator<T> rit) { return (it == rit.it && rit.current == current); }
    bool operator!=(rangeset_iterator<T> rit) { return (it != rit.it) || (rit.current != current); }
    ...
    rangeset_iterator<T> operator++(int)
    {
        if (current < it->second)
            ...
            it++;
            if (it != ranges.end()) current = it->first;
        }
...
public:
    ...
    map_iterator map_begin() { return theset.ranges.begin(); }
    map_iterator map_end() { return theset.ranges.end(); }
    int map_size() { return theset.ranges.size(); }
    ...
    void insert(T val)
    {
        ...
        if (left != theset.ranges.end() && right != theset.ranges.end()) {
            // join!
    ...
    void dump()
    {
        for (typename map<T, T>::iterator it = theset.ranges.begin(); it != theset.ranges.end(); it++) {
