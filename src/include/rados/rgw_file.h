...
#define LIBRGW_FILE_VERSION_CODE \
    LIBRGW_FILE_VERSION(LIBRGW_FILE_VER_MAJOR, LIBRGW_FILE_VER_MINOR, LIBRGW_FILE_VER_EXTRA)
...
#define RGW_LOOKUP_TYPE_FLAGS (RGW_LOOKUP_FLAG_DIR | RGW_LOOKUP_FLAG_FILE)
...
int rgw_mount(librgw_t rgw, const char* uid, const char* key, const char* secret, struct rgw_fs** rgw_fs,
              uint32_t flags);

int rgw_mount2(librgw_t rgw, const char* uid, const char* key, const char* secret, const char* root,
...
int rgw_rename(struct rgw_fs* rgw_fs, struct rgw_file_handle* olddir, const char* old_name,
               struct rgw_file_handle* newdir, const char* new_name, uint32_t flags);
...
int rgw_unlink(struct rgw_fs* rgw_fs, struct rgw_file_handle* parent_fh, const char* path, uint32_t flags);

/*
    read  directory content
*/
typedef int (*rgw_readdir_cb)(const char* name, void* arg, uint64_t offset, struct stat* st, uint32_t mask,
                              uint32_t flags);
...
int rgw_fsync(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh, uint32_t flags);
...
typedef int (*rgw_getxattr_cb)(rgw_xattrlist* attrs, void* arg, uint32_t flags);
...
int rgw_lsxattrs(struct rgw_fs* rgw_fs, struct rgw_file_handle* fh,
                 rgw_xattrstr* filter_prefix /* unimplemented for now */, rgw_getxattr_cb cb, void* cb_arg,
                 uint32_t flags);
