...
struct frag_info_t : public scatter_info_t
{
    ...
    void add_delta(const frag_info_t& cur, const frag_info_t& acc, bool* touched_mtime = 0, bool* touched_chattr = 0)
    {
        if (cur.mtime > mtime) {
            mtime = cur.mtime;
            if (touched_mtime) *touched_mtime = true;
        }
        if (cur.change_attr > change_attr) {
            change_attr = cur.change_attr;
            if (touched_chattr) *touched_chattr = true;
        }
    ...
    {
        if (other.mtime > mtime) mtime = other.mtime;
        if (other.change_attr > change_attr) change_attr = other.change_attr;
        nfiles += other.nfiles;
...
struct nest_info_t : public scatter_info_t
{
    ...
    void zero() { *this = nest_info_t(); }

    void sub(const nest_info_t& other) { add(other, -1); }
    void add(const nest_info_t& other, int fac = 1)
    {
        if (other.rctime > rctime) rctime = other.rctime;
    ...
    {
        if (cur.rctime > rctime) rctime = cur.rctime;
    ...
    {
        return rctime <= o.rctime && rbytes == o.rbytes && rfiles == o.rfiles && rsubdirs == o.rsubdirs &&
               rsnaps == o.rsnaps;
    }
...
struct quota_info_t
{
    ...
    bool is_valid() const { return max_bytes >= 0 && max_files >= 0; }
    bool is_enabled(quota_max_t type = QUOTA_ANY) const
    {
        switch (type) {
        case QUOTA_MAX_FILES: return !!max_files;
        case QUOTA_MAX_BYTES: return !!max_bytes;
        case QUOTA_ANY:
        default: return !!max_bytes || !!max_files;
        }
    }
    void decode_json(JSONObj* obj);
...
{
    return l.range.first == r.range.first && l.range.last == r.range.last && l.follows == r.follows;
}
...
public:
    ...
    {
        if (!blp) blp.reset(new ceph::buffer::list);
        *blp = bl;
    }
    ...
    {
        return length() == o.length() && (length() == 0 || (*const_cast<ceph::buffer::list*>(blp.get()) ==
                                                            *const_cast<ceph::buffer::list*>(o.blp.get())));
    }
    bool operator!=(const inline_data_t& o) const { return !(*this == o); }
    void encode(ceph::buffer::list& bl) const;
    void decode(ceph::buffer::list::const_iterator& bl);
...
template<template<typename> class Allocator = std::allocator> struct inode_t
{
    /**
     * ***************
    ...
    void truncate(uint64_t old_size, uint64_t new_size)
    {
        ...
        if (old_size > max_size_ever) max_size_ever = old_size;
    ...
    bool has_layout() const { return layout != file_layout_t(); }

    void clear_layout() { layout = file_layout_t(); }

    uint64_t get_layout_size_increment() const { return layout.get_period(); }
    ...
    uint64_t get_max_size() const
    {
        ...
        for (std::map<client_t, client_writeable_range_t>::const_iterator p = client_ranges.begin();
             p != client_ranges.end();
             ++p)
            if (p->second.range.last > max) max = p->second.range.last;
        return max;
    }
    ...
    bool is_backtrace_updated() const { return backtrace_version == version; }
    void update_backtrace(version_t pv = 0) { backtrace_version = pv ? pv : version; }
...
template<template<typename> class Allocator> void inode_t<Allocator>::decode(ceph::buffer::list::const_iterator& p)
{
    ...
    if (struct_v >= 3) {
        ...
        decode(m, p);
        for (auto q = m.begin(); q != m.end(); ++q) client_ranges[q->first].range = q->second;
    }
    ...
    if (struct_v >= 2) decode(backtrace_version, p);
    if (struct_v >= 7) decode(old_pools, p);
    if (struct_v >= 8) decode(max_size_ever, p);
    if (struct_v >= 9) {
    ...
    if (struct_v < 10) backtrace_version = 0;   // force update backtrace
    if (struct_v >= 11) decode(quota, p);
...
template<template<typename> class Allocator> void inode_t<Allocator>::dump(ceph::Formatter* f) const
{
...
template<template<typename> class Allocator> void inode_t<Allocator>::decode_json(JSONObj* obj)
{
...
template<template<typename> class Allocator> void inode_t<Allocator>::generate_test_instances(std::list<inode_t*>& ls)
{
...
int inode_t<Allocator>::compare(const inode_t<Allocator>& other, bool* divergent) const
{
    ...
    *divergent = false;
    if (version == other.version) {
        if (rdev != other.rdev || ctime != other.ctime || btime != other.btime || mode != other.mode ||
            uid != other.uid || gid != other.gid || nlink != other.nlink ||
            memcmp(&dir_layout, &other.dir_layout, sizeof(dir_layout)) || layout != other.layout ||
            old_pools != other.old_pools || size != other.size || max_size_ever != other.max_size_ever ||
            truncate_seq != other.truncate_seq || truncate_size != other.truncate_size ||
            truncate_from != other.truncate_from || truncate_pending != other.truncate_pending ||
            change_attr != other.change_attr || mtime != other.mtime || atime != other.atime ||
            time_warp_seq != other.time_warp_seq || inline_data != other.inline_data ||
            client_ranges != other.client_ranges || !(dirstat == other.dirstat) || !(rstat == other.rstat) ||
            !(accounted_rstat == other.accounted_rstat) || file_data_version != other.file_data_version ||
            xattr_version != other.xattr_version || backtrace_version != other.backtrace_version) {
            *divergent = true;
        }
        return 0;
    }
...
{
    if (max_size_ever < other.max_size_ever || truncate_seq < other.truncate_seq ||
        time_warp_seq < other.time_warp_seq || inline_data.version < other.inline_data.version ||
        dirstat.version < other.dirstat.version || rstat.version < other.rstat.version ||
        accounted_rstat.version < other.accounted_rstat.version || file_data_version < other.file_data_version ||
        xattr_version < other.xattr_version || backtrace_version < other.backtrace_version) {
        return false;
    }
    return true;
...
template<typename Iterator> struct keys_and_values : qi::grammar<Iterator, std::map<std::string, std::string>()>
{
    keys_and_values()
