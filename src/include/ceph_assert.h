...
extern void __ceph_assert_fail(const assert_data& ctx) __attribute__((__noreturn__));

extern void __ceph_assertf_fail(const char* assertion, const char* file, int line, const char* function,
                                const char* msg, ...) __attribute__((__noreturn__));
extern void __ceph_assert_warn(const char* assertion, const char* file, int line, const char* function);

[[noreturn]] void __ceph_abort(const char* file, int line, const char* func, const std::string& msg);

[[noreturn]] void __ceph_abortf(const char* file, int line, const char* func, const char* msg, ...);

#define _CEPH_ASSERT_VOID_CAST static_cast<void>

#define assert_warn(expr)               \
    ((expr) ? _CEPH_ASSERT_VOID_CAST(0) \
 ...
 */
#define ceph_abort(msg, ...) ::ceph::__ceph_abort(__FILE__, __LINE__, __CEPH_ASSERT_FUNCTION, "abort() called")

#define ceph_abort_msg(msg) ::ceph::__ceph_abort(__FILE__, __LINE__, __CEPH_ASSERT_FUNCTION, msg)

#define ceph_abort_msgf(...) ::ceph::__ceph_abortf(__FILE__, __LINE__, __CEPH_ASSERT_FUNCTION, __VA_ARGS__)
...
#define ceph_assert(expr)                                                                                              \
    do {                                                                                                               \
        static const ceph::assert_data assert_data_ctx = {__STRING(expr), __FILE__, __LINE__, __CEPH_ASSERT_FUNCTION}; \
        ((expr) ? _CEPH_ASSERT_VOID_CAST(0) : ::ceph::__ceph_assert_fail(assert_data_ctx));                            \
    } while (false)
#endif
