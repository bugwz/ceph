...
template<class T, class Set> class compact_set_base
{
protected:
    std::unique_ptr<Set> set;
    void alloc_internal()
    {
        if (!set) set.reset(new Set);
    }
    void free_internal() { set.reset(); }
    ...
    private:
        ...
        iterator_base()
            : set(0)
        {}
        iterator_base(const compact_set_base* s)
            : set(s)
        {}
        iterator_base(const compact_set_base* s, const It& i)
            : set(s)
            , it(i)
        {}
    ...
    public:
        ...
        bool operator==(const iterator_base& o) const { return (set == o.set) && (!set->set || it == o.it); }
...
public:
    ...
    public:
        ...
        {}
        const_iterator(const compact_set_base* s)
            : iterator_base<typename Set::const_iterator>(s)
        {}
    ...
    public:
        ...
        {}
        iterator(compact_set_base* s)
            : iterator_base<typename Set::iterator>(s)
        {}
        iterator(compact_set_base* s, const typename Set::iterator& i)
            : iterator_base<typename Set::iterator>(s, i)
        {}
        operator const_iterator() const { return const_iterator(this->set, this->it); }
    };
    ...
    public:
        ...
        {}
        const_reverse_iterator(const compact_set_base* s)
            : iterator_base<typename Set::const_reverse_iterator>(s)
        {}
    ...
    public:
        ...
        {}
        reverse_iterator(compact_set_base* s)
            : iterator_base<typename Set::reverse_iterator>(s)
        {}
    ...
    bool operator!=(const compact_set_base& o) const { return !(*this == o); }
    size_t count(const T& t) const { return set ? set->count(t) : 0; }
    ...
    size_t erase(const T& t)
    {
        ...
        if (set->empty()) free_internal();
        return r;
    }
    void clear() { free_internal(); }
    void swap(compact_set_base& o) { set.swap(o.set); }
    ...
    {
        if (!set) return reverse_iterator(this);
    ...
    {
        if (!set) return const_iterator(this);
    ...
    {
        if (!set) return const_reverse_iterator(this);
...
template<class T, class Set> inline void encode(const compact_set_base<T, Set>& m, ceph::buffer::list& bl)
{
    m.encode(bl);
}
template<class T, class Set> inline void decode(compact_set_base<T, Set>& m, ceph::buffer::list::const_iterator& p)
{
    m.decode(p);
}
