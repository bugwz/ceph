...
class PrCtl
{
    ...
    static int get_dumpable()
    {
        ...
        if (r == -1) {
            r = errno;
            std::cerr << "warning: unable to get dumpable flag: " << cpp_strerror(r) << std::endl;
        }
        return r;
    }
    static int set_dumpable(bool new_state)
    {
        ...
        if (r) {
            r = -errno;
            std::cerr << "warning: unable to " << (new_state ? "set" : "unset") << " dumpable flag: " << cpp_strerror(r)
                      << std::endl;
        }
        return r;
    }
...
class PrCtl
{
    ...
    static int get_dumpable(rlimit* saved)
    {
        ...
        if (r) {
            r = errno;
            std::cerr << "warning: unable to getrlimit(): " << cpp_strerror(r) << std::endl;
        }
        return r;
    }
    static void set_dumpable(const rlimit& rlim)
    {
        ...
        if (r) {
            r = -errno;
            std::cerr << "warning: unable to setrlimit(): " << cpp_strerror(r) << std::endl;
        }
    }
