...
template<typename T> inline void op_func(const op o, void* p) noexcept
{
    static const std::type_info& type = typeid(T);
    switch (o) {
    case op::type: *(reinterpret_cast<const std::type_info**>(p)) = &type; break;
    case op::destroy: reinterpret_cast<T*>(p)->~T(); break;
    }
}
...
// *horrible*.
//
template<typename D, typename storage_t> class base
{
    ...
    {
        static_assert(noexcept(static_cast<const D*>(this)->ptr()) &&
    ...
    {
        static_assert(std::is_same_v<decltype(static_cast<D*>(this)->alloc_storage(z)), void*>,
    ...
    {
        static_assert(noexcept(static_cast<D*>(this)->free_storage()) &&
    ...
    //
    template<class T> struct is_in_place_type_helper : std::false_type
    {};
    template<class T> struct is_in_place_type_helper<std::in_place_type_t<T>> : std::true_type
    {};

    template<class T> static constexpr bool is_in_place_type_v = is_in_place_type_helper<std::decay_t<T>>::value;
    ...
    //
    template<typename T>
    using value_condition_t =
        std::enable_if_t<!std::is_same_v<std::decay_t<T>, D> && !is_in_place_type_v<std::decay_t<T>>>;
    ...
    //
    template<typename T>
    static constexpr bool value_noexcept_v = std::is_nothrow_constructible_v<std::decay_t<T>, T> && capacity != dynamic;

    // SFINAE condition for in-place constructors/assigners
    //
    template<typename T, typename... Args>
    using in_place_condition_t = std::enable_if_t<std::is_constructible_v<std::decay_t<T>, Args...>>;
    ...
    static constexpr bool in_place_noexcept_v =
        std::is_nothrow_constructible_v<std::decay_t<T>, Args...> && capacity != dynamic;
...
private:
    ...
    //
    template<typename T, typename... Args> std::decay_t<T>& construct(Args&&... args)
    {
        ...
        try {
            func = &op_func<Td>;
            return *new (reinterpret_cast<Td*>(alloc_storage(sizeof(Td)))) Td(std::forward<Args>(args)...);
        }
        catch (...) {
            reset();
            throw;
...
public:
    ...
    // ----------------------
    //
    template<typename T, typename = value_condition_t<T>> base(T&& t) noexcept(value_noexcept_v<T>)
    {
    ...
    //
    template<typename T, typename = value_condition_t<T>> base& operator=(T&& t) noexcept(value_noexcept_v<T>)
    {
        reset();
        construct<T>(std::forward<T>(t));
        return *this;
    ...
    base(std::in_place_type_t<T>, Args&&... args) noexcept(in_place_noexcept_v<T, Args...>)
    {
    ...
    std::decay_t<T>& emplace(Args&&... args) noexcept(in_place_noexcept_v<T, Args...>)
    {
        reset();
    ...
    base(std::in_place_type_t<T>, std::initializer_list<U> i,
         Args&&... args) noexcept(in_place_noexcept_v<T, std::initializer_list<U>, Args...>)
    {
    ...
    template<typename U = storage_t, typename = std::enable_if<std::is_swappable_v<storage_t>>> void swap(base& rhs)
    {
        using std::swap;
        swap(func, rhs.func);
    ...
    //
    bool has_value() const noexcept { return !!func; }
    ...
    template<typename T, typename U, typename V> friend inline void* cast_helper(const base<U, V>& b) noexcept;
};
...
//
template<typename T, typename U, typename V> inline void* cast_helper(const base<U, V>& b) noexcept
{
...
// The pointer pair!
//
template<typename T, typename U, typename V> inline T* any_cast(_any::base<U, V>* a) noexcept
{
    if (a) {
...
template<typename T, typename U, typename V> inline const T* any_cast(const _any::base<U, V>* a) noexcept
{
    if (a) {
        return static_cast<T*>(_any::cast_helper<std::decay_t<T>>(*a));
    }
...
//
template<typename T, typename U, typename V> inline T any_cast(_any::base<U, V>& a)
{
...
template<typename T, typename U, typename V> inline T any_cast(const _any::base<U, V>& a)
{
    static_assert(std::is_reference_v<T> || std::is_copy_constructible_v<T>,
                  "The supplied type must be either a reference or "
                  "copy constructible.");
...
inline std::enable_if_t<
    (std::is_move_constructible_v<T> || std::is_copy_constructible_v<T>)&&!std::is_rvalue_reference_v<T>, T>
...
template<typename T, typename U, typename V>
inline std::enable_if_t<std::is_rvalue_reference_v<T>, T> any_cast(_any::base<U, V>&& a)
{
    auto p = any_cast<std::decay_t<T>>(&a);
    if (p) {
        return static_cast<T>(*p);
...
//
template<std::size_t S> class immobile_any : public _any::base<immobile_any<S>, std::aligned_storage_t<S>>
{
    ...
    void* ptr() const noexcept { return const_cast<void*>(static_cast<const void*>(&storage)); }
    void* alloc_storage(std::size_t) noexcept { return ptr(); }
    void free_storage() noexcept {}

    static_assert(capacity != _any::dynamic, "That is not a valid size for an immobile_any.");
...
template<typename T, std::size_t S, typename... Args> inline immobile_any<S> make_immobile_any(Args&&... args)
{
...
class unique_any : public _any::base<unique_any, std::unique_ptr<std::byte[]>>
{
    ...
    void free_storage() noexcept { storage.reset(); }
...
template<typename T, typename... Args> inline unique_any make_unique_any(Args&&... args)
{
...
class shared_any : public _any::base<shared_any, boost::shared_ptr<std::byte[]>>
{
    ...
    //
    static constexpr std::size_t capacity = _any::dynamic;
    void* ptr() const noexcept { return static_cast<void*>(storage.get()); }
...
template<typename T, typename... Args> inline shared_any make_shared_any(Args&&... args)
{
