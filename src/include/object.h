...
struct object_t
{
    ...
    void swap(object_t& o) { name.swap(o.name); }
...
struct file_object_t
{
    ...
    {
        if (!buf[0]) snprintf(buf, sizeof(buf), "%llx.%08llx", (long long unsigned)ino, (long long unsigned)bno);
        return buf;
    }
...
template<> struct denc_traits<snapid_t>
{
    static constexpr bool supported = true;
    static constexpr bool featured = false;
    static constexpr bool bounded = true;
    static constexpr bool need_contiguous = true;
    static void bound_encode(const snapid_t& o, size_t& p) { denc(o.val, p); }
    static void encode(const snapid_t& o, ceph::buffer::list::contiguous_appender& p) { denc(o.val, p); }
    static void decode(snapid_t& o, ceph::buffer::ptr::const_iterator& p) { denc(o.val, p); }
};
