...
public:
    ...
    frag_t() = default;
    frag_t(unsigned v, unsigned b)
        : _enc(ceph_frag_make(b, v))
    {}
    frag_t(_frag_t e)
        : _enc(e)
    {}
    ...
    template<typename T> void split(int nb, T& fragments) const
    {
        ceph_assert(nb > 0);
        unsigned nway = 1 << nb;
        for (unsigned i = 0; i < nway; i++) fragments.push_back(make_child(i, nb));
    }
    ...
    void encode(ceph::buffer::list& bl) const { ceph::encode_raw(_enc, bl); }
...
inline std::ostream& operator<<(std::ostream& out, const frag_t& hb)
{
    ...
    if (num) {
        ...
        for (unsigned bit = 23; num; num--, bit--) out << ((val & (1 << bit)) ? '1' : '0');
    }
    return out << '*';
}
...
public:
    // -------------
    // basics
    void swap(fragtree_t& other) { _splits.swap(other._splits); }
    void clear() { _splits.clear(); }

    // -------------
    // accessors
    bool empty() const { return _splits.empty(); }
     ...
     */
    template<typename T> void get_leaves(T& c) const { return get_leaves_under_split(frag_t(), c); }

    /**
     * get_leaves_under_split -- list all leaves under a known split point (or root)
     */
    template<typename T> void get_leaves_under_split(frag_t under, T& c) const
    {
        frag_vec_t s;
        s.push_back(under);
        while (!s.empty()) {
     ...
     */
    template<typename T> void get_leaves_under(frag_t x, T& c) const
    {
        frag_vec_t s;
    ...
    void split(frag_t x, int b, bool simplify = true)
    {
        ...
        if (simplify) try_assimilate_children(get_branch_above(x));
    }
    ...
    void try_assimilate_children(frag_t x)
    {
        ...
        for (auto& frag : children) _splits.erase(frag);
        _splits[x] += childbits;
    }

    bool force_to_leaf(CephContext* cct, frag_t x)
    {
        if (is_leaf(x)) return false;
    ...
    {
        using ceph::encode;
        for (compact_map<frag_t, int32_t>::const_iterator p = _splits.begin(); p != _splits.end(); ++p) {
            encode(p->first, bl);
            encode(p->second, bl);
        }
    }
...
inline std::ostream& operator<<(std::ostream& out, const fragtree_t& ft)
{
    ...
    for (compact_map<frag_t, int32_t>::const_iterator p = ft._splits.begin(); p != ft._splits.end(); ++p) {
        if (p != ft._splits.begin()) out << " ";
...
public:
    ...
    void insert_raw(frag_t f) { _set.insert(f); }
    ...
    void simplify()
    {
        auto it = _set.begin();
        while (it != _set.end()) {
            if (!it->is_root() && _set.count(it->get_sibling())) {
    ...
    void encode(ceph::buffer::list& bl) const { ceph::encode(_set, bl); }
    void decode(ceph::buffer::list::const_iterator& p) { ceph::decode(_set, p); }
};
