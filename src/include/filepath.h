...
class filepath
{
    ...
    void parse_bits() const
    {
        ...
        while (off < (int)path.length()) {
            ...
            if (nextslash < 0) nextslash = path.length();   // no more slashes
...
public:
    filepath() = default;
    filepath(std::string_view p, inodeno_t i)
        : ino(i)
        , path(p)
    {}
    ...
    {
        filepath t(ino);
        for (int i = 0; i < s; i++) t.push_dentry(bits[i]);
        return t;
    }
    filepath postfixpath(int s) const
    {
        filepath t;
        for (unsigned i = s; i < bits.size(); i++) t.push_dentry(bits[i]);
        return t;
    }
    ...
    void push_dentry(const std::string& s) { push_dentry(std::string_view(s)); }
    void push_dentry(const char* cs) { push_dentry(std::string_view(cs, strlen(cs))); }
    ...
    void append(const filepath& a)
    {
        ...
        for (unsigned i = 0; i < a.depth(); i++) push_dentry(a[i]);
    }
