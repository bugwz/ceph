...
void dump_services(ceph::Formatter* f, const std::map<std::string, std::list<int> >& services, const char* type);
...
/// @param type the service type of given @p services, for example @p osd or @p mon.
void dump_services(ceph::Formatter* f, const std::map<std::string, std::list<std::string> >& services,
                   const char* type);
...
template<typename... XS> bool match_str(const std::string& s, const XS&... xs)
{
    return ((s == xs) || ...);
}
