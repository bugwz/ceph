...
template<typename T, template<typename, typename, typename...> class C = std::map> class interval_set
{
public:
    using Map = C<T, T>;
    ...
    public:
        ...
        offset_type get_start() const { return _iter->first; }

        // Return the interval length.
        length_type get_len() const { return _iter->second; }

        offset_type get_end() const { return _iter->first + _iter->second; }

        // Set the interval length.
        void set_len(const length_type& len) { _iter->second = len; }
    ...
    size_type num_intervals() const { return m.size(); }

    iterator begin() { return iterator(m.begin()); }

    iterator lower_bound(T start) { return iterator(find_inc_m(start)); }

    iterator end() { return iterator(m.end()); }

    const_iterator begin() const { return const_iterator(m.begin()); }

    const_iterator lower_bound(T start) const { return const_iterator(find_inc(start)); }
...
private:
    auto find_inc(T start) const
    {
        ...
        if (p != m.begin() && (p == m.end() || p->first > start)) {
            --p;                                      // might overlap?
            if (p->first + p->second <= start) ++p;   // it doesn't.
        }
        return p;
    }
    ...
    void intersection_size_asym(const interval_set& s, const interval_set& l)
    {
        ...
        while (1) {
            ...
            if (pl == l.m.end()) break;
            while (ps != s.m.end() && ps->first + ps->second <= pl->first) ++ps;
            if (ps == s.m.end()) break;
    ...
    bool subset_size_sym(const interval_set& b) const
    {
        ...
        while (pa != a_end && pb != b_end) {
            ...
            if (pa->first + pa->second > pb->first + pb->second) return false;
...
public:
    bool operator==(const interval_set& other) const { return _size == other._size && m == other.m; }

    uint64_t size() const { return _size; }

    void bound_encode(size_t& p) const { denc_traits<Map>::bound_encode(m, p); }
    void encode(ceph::buffer::list::contiguous_appender& p) const { denc(m, p); }
    ...
    void encode_nohead(ceph::buffer::list::contiguous_appender& p) const { denc_traits<Map>::encode_nohead(m, p); }
    ...
    bool contains(T i, T* pstart = 0, T* plen = 0) const
    {
        ...
        if (pstart) *pstart = p->first;
        if (plen) *plen = p->second;
        return true;
    }
        ...
        if (p == m.end()) {
            ...
            if (p->first < start) {
                ...
                if (p->first + p->second != start) {
                    // cout << "p is " << p->first << "~" << p->second << ", start is " << start << ", len is " << len
                    // << endl;
                    ceph_abort();
                }
    ...
    bool subset_of(const interval_set& big) const
    {
        ...
        if (range_end() > big.range_end()) return false;
         ...
         * ratios where sequential search performs better.
         */
        if (big.size() / size() < 10) return subset_size_sym(big);
    ...
    void span_of(const interval_set& other, T start, T len)
    {
        ...
        if (p == other.m.end()) return;
        if (p->first < start) {
            if (p->first + p->second < start) return;
...
template<typename T, template<typename, typename, typename...> class C> struct denc_traits<interval_set<T, C>>
{
private:
...
public:
    ...
    static void bound_encode(const container_t& v, size_t& p) { v.bound_encode(p); }
    static void encode(const container_t& v, ceph::buffer::list::contiguous_appender& p) { v.encode(p); }
    static void decode(container_t& v, ceph::buffer::ptr::const_iterator& p) { v.decode(p); }
    template<typename U = T>
    static typename std::enable_if<sizeof(U) && !need_contiguous>::type decode(container_t& v,
                                                                               ceph::buffer::list::iterator& p)
    {
        v.decode(p);
    }
    static void encode_nohead(const container_t& v, ceph::buffer::list::contiguous_appender& p) { v.encode_nohead(p); }
    static void decode_nohead(size_t n, container_t& v, ceph::buffer::ptr::const_iterator& p) { v.decode_nohead(n, p); }
};
