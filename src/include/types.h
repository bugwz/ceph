...
namespace std {
template<class A, class B> inline std::ostream& operator<<(std::ostream& out, const std::pair<A, B>& v);
template<class A, class Alloc> inline std::ostream& operator<<(std::ostream& out, const std::vector<A, Alloc>& v);
...
template<typename... Ts> inline std::ostream& operator<<(std::ostream& out, const std::tuple<Ts...>& t);
template<typename T> inline std::ostream& operator<<(std::ostream& out, const std::optional<T>& t);
template<class A, class Alloc> inline std::ostream& operator<<(std::ostream& out, const std::list<A, Alloc>& ilist);
template<class A, class Comp, class Alloc>
...
namespace boost {
template<typename... Ts> inline std::ostream& operator<<(std::ostream& out, const boost::tuple<Ts...>& t);
...
template<class A, class Alloc> inline std::ostream& operator<<(std::ostream& out, const std::deque<A, Alloc>& v)
{
    out << "<";
...
{
    return out << boost::get<0>(t) << "," << boost::get<1>(t) << "," << boost::get<2>(t);
}
}   // namespace tuples
namespace container {
template<class A, class Comp, class Alloc>
...
namespace {
inline std::ostream& format_u(std::ostream& out, const uint64_t v, const uint64_t n, const int index,
                              const uint64_t mult, const char* u)
{
    char buffer[32];
    ...
    if (index == 0) {
        ...
        int i;
        for (i = 2; i >= 0; i--) {
            if (snprintf(buffer, sizeof(buffer), "%.*f%s", i, static_cast<double>(v) / mult, u) <= 7) break;
        }
    }
...
{
    return out << (long)i.start << ((i.flags & CEPH_SUBSCRIBE_ONETIME) ? "" : "+");
}
...
struct shard_id_t
{
    ...
    shard_id_t()
        : id(0)
    {}
    explicit shard_id_t(int8_t _id)
        : id(_id)
    {}
...
#if defined(__sun) || defined(_AIX) || defined(__APPLE__) || defined(__FreeBSD__) || defined(_WIN32)
extern "C" {
...
struct errorcode32_t
{
    ...
    {}
    // cppcheck-suppress noExplicitConstructor
    explicit errorcode32_t(int32_t i)
        : code(i)
    {}
...
template<uint8_t S> struct sha_digest_t
{
    ...
    bool operator==(const sha_digest_t& r) const { return ::memcmp(v, r.v, SIZE) == 0; }
    bool operator!=(const sha_digest_t& r) const { return ::memcmp(v, r.v, SIZE) != 0; }
...
template<uint8_t S> inline std::ostream& operator<<(std::ostream& out, const sha_digest_t<S>& b)
{
