...
struct uuid_d
{
    ...
    bool is_zero() const { return uuid.is_nil(); }
    ...
    void print(char* s) const { memcpy(s, boost::uuids::to_string(uuid).c_str(), 37); }

    std::string to_string() const { return boost::uuids::to_string(uuid); }

    const char* bytes() const { return (const char*)uuid.data; }
    ...
    void bound_encode(size_t& p) const { p += sizeof(uuid); }
