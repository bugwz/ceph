...
static ostream& _prefix(std::ostream* _dout, Notify* notify)
{
    return notify->gen_dbg_prefix(*_dout);
}

Notify::Notify(ConnectionRef client, uint64_t client_gid, bufferlist& payload, uint32_t timeout, uint64_t cookie,
...
NotifyRef Notify::makeNotifyRef(ConnectionRef client, uint64_t client_gid, bufferlist& payload, uint32_t timeout,
                                uint64_t cookie, uint64_t notify_id, uint64_t version, OSDService* osd)
{
    NotifyRef ret(new Notify(client, client_gid, payload, timeout, cookie, notify_id, version, osd));
    ret->set_self(ret);
    return ret;
}
...
public:
    explicit NotifyTimeoutCB(NotifyRef notif)
        : notif(notif)
        , canceled(false)
    {}
...
void Notify::complete_watcher(WatchRef watch, bufferlist& reply_bl)
{
    ...
    notify_replies.insert(make_pair(make_pair(watch->get_watcher_gid(), watch->get_cookie()), reply_bl));
    maybe_complete_notify();
}
...
void Notify::maybe_complete_notify()
{
    ...
    if (watchers.empty() || timed_out) {
        ...
        for (auto& watcher : watchers) {
            missed.emplace_back(watcher->get_watcher_gid(), watcher->get_cookie());
        }
        encode(missed, bl);

        bufferlist empty;
        auto* const reply =
            new MWatchNotify(cookie, version, notify_id, CEPH_WATCH_EVENT_NOTIFY_COMPLETE, empty, client_gid);
        reply->set_data(bl);
        if (timed_out) reply->return_code = -ETIMEDOUT;
...
static ostream& _prefix(std::ostream* _dout, Watch* watch)
{
...
public:
    ...
    explicit HandleWatchTimeout(WatchRef watch)
        : watch(watch)
        , canceled(false)
    {}
    ...
    void complete(int) override
    {
        ...
        pg->lock();
        watch->cb = nullptr;
        if (!watch->is_discarded() && !canceled) watch->pg->handle_watch_timeout(watch);
...
public:
    bool canceled;
    explicit HandleDelayedWatchTimeout(WatchRef watch)
        : watch(watch)
        , canceled(false)
    {}
    void cancel() override { canceled = true; }
...
{
    return pg->gen_prefix(out) << " -- Watch(" << make_pair(cookie, entity) << ") ";
}

Watch::Watch(PrimaryLogPG* pg, OSDService* osd, ObjectContextRef obc, uint32_t timeout, uint64_t cookie,
             entity_name_t entity, const entity_addr_t& addr)
    : cb(NULL)
    , osd(osd)
    , pg(pg)
    , obc(obc)
...
void Watch::connect(ConnectionRef con, bool _will_ping)
{
    ...
    if (priv) {
        ...
        priv.reset();
        for (auto i = in_progress_notifies.begin(); i != in_progress_notifies.end(); ++i) {
...
void Watch::remove(bool send_disconnect)
{
    ...
    if (send_disconnect && is_connected()) {
        bufferlist empty;
        MWatchNotify* reply(new MWatchNotify(cookie, 0, 0, CEPH_WATCH_EVENT_DISCONNECT, empty));
...
{
    ceph_assert(in_progress_notifies.find(notif->notify_id) == in_progress_notifies.end());
    if (will_ping) {
        utime_t cutoff = ceph_clock_now();
        cutoff.sec_ref() -= timeout;
        if (last_ping < cutoff) {
            dout(10) << __func__ << " " << notif->notify_id << " last_ping " << last_ping << " < cutoff " << cutoff
                     << ", disconnecting" << dendl;
            disconnect();
            return;
        }
    ...
    if (is_connected()) send_notify(notif);
}
...
void Watch::send_notify(NotifyRef notif)
{
    ...
    MWatchNotify* notify_msg = new MWatchNotify(
        cookie, notif->version, notif->notify_id, CEPH_WATCH_EVENT_NOTIFY, notif->payload, notif->client_gid);
...
WatchRef Watch::makeWatchRef(PrimaryLogPG* pg, OSDService* osd, ObjectContextRef obc, uint32_t timeout, uint64_t cookie,
                             entity_name_t entity, const entity_addr_t& addr)
{
...
void WatchConState::reset(Connection* con)
{
    ...
    for (set<WatchRef>::iterator i = _watches.begin(); i != _watches.end(); ++i) {
        boost::intrusive_ptr<PrimaryLogPG> pg((*i)->get_pg());
        pg->lock();
        if (!(*i)->is_discarded()) {
