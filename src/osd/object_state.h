...
struct ObjectState
{
    ...
    ObjectState(object_info_t&& oi_, bool exists_)
        ...
        , exists(exists_)
    {}
    ObjectState(const hobject_t& obj)
        : oi(obj)
        , exists(false)
    {}
};
...
struct RWState
{
    ...
    bool get_read_lock()
    {
        ...
        case RWNONE:
            ...
            state = RWREAD;
            // fall through
        case RWREAD: count++; return true;
        case RWWRITE: return false;
        case RWEXCL: return false;
        default: ceph_abort_msg("unhandled case"); return false;
        }
    }
    ...
    bool get_write_lock(bool greedy = false)
    {
        if (!greedy) {
            // don't starve anybody!
            if (waiters > 0 || recovery_read_marker) {
                return false;
            }
        }
        switch (state) {
        case RWNONE:
            ...
            state = RWWRITE;
            // fall through
        case RWWRITE: count++; return true;
        case RWREAD: return false;
        case RWEXCL: return false;
        default: ceph_abort_msg("unhandled case"); return false;
        }
    }
    ...
    void inc_waiters() { ++waiters; }
    void release_waiters() { waiters = 0; }
    ...
    bool get_snaptrimmer_write(bool mark_if_unsuccessful)
    {
        if (get_write_lock()) {
            ...
            if (mark_if_unsuccessful) snaptrimmer_write_marker = true;
            return false;
        }
    }
...
{
    return out << "rwstate(" << rw.get_state_name() << " n=" << rw.count << " w=" << rw.waiters << ")";
}
