...
{
    return out << "ssc(" << ssc.oid << " snapset: " << ssc.snapset << " ref: " << ssc.ref
               << " registered: " << ssc.registered << " exists: " << ssc.exists << ")";
}
...
public:
    ...
    {
        switch (type) {
        case RWState::RWWRITE: return get_write(op);
        case RWState::RWREAD: return get_read(op);
        case RWState::RWEXCL: return get_excl(op);
        default: ceph_abort_msg("invalid lock type"); return true;
        }
    }
    bool get_write_greedy(OpRequestRef& op) { return get_write(op, true); }
    bool get_snaptrimmer_write(bool mark_if_unsuccessful)
    {
        return rwstate.get_snaptrimmer_write(mark_if_unsuccessful);
    }
    bool get_recovery_read() { return rwstate.get_recovery_read(); }
    bool try_get_read_lock() { return rwstate.get_read_lock(); }
    ...
    void put_lock_type(RWState::State type, std::list<OpRequestRef>* to_wake, bool* requeue_recovery,
                       bool* requeue_snaptrimmer)
    {
        switch (type) {
        case RWState::RWWRITE: put_write(to_wake); break;
        case RWState::RWREAD: put_read(to_wake); break;
        case RWState::RWEXCL: put_excl(to_wake); break;
        default: ceph_abort_msg("invalid lock type");
        }
    ...
    bool is_request_pending() { return !rwstate.empty(); }
    ...
    ~ObjectContext()
    {
        ...
        if (destructor_callback) destructor_callback->complete(0);
    }
...
class ObcLockManager
{
    struct ObjectLockState
    {
        ...
        RWState::State type;
        ObjectLockState(ObjectContextRef obc, RWState::State type)
            : obc(std::move(obc))
            , type(type)
        {}
    };
...
public:
    ...
    bool get_lock_type(RWState::State type, const hobject_t& hoid, ObjectContextRef& obc, OpRequestRef& op)
    {
    ...
    bool take_write_lock(const hobject_t& hoid, ObjectContextRef obc)
    {
        ...
        if (obc->rwstate.take_write_lock()) {
            locks.insert(std::make_pair(hoid, ObjectLockState(obc, RWState::RWWRITE)));
            return true;
        }
        else {
            return false;
    ...
    bool get_snaptrimmer_write(const hobject_t& hoid, ObjectContextRef obc, bool mark_if_unsuccessful)
    {
        ceph_assert(locks.find(hoid) == locks.end());
    ...
    /// try get read lock
    bool try_get_read_lock(const hobject_t& hoid, ObjectContextRef obc)
    {
        ceph_assert(locks.find(hoid) == locks.end());
        if (obc->try_get_read_lock()) {
            locks.insert(std::make_pair(hoid, ObjectLockState(obc, RWState::RWREAD)));
            return true;
        }
        else {
            return false;
    ...
    void put_locks(std::list<std::pair<ObjectContextRef, std::list<OpRequestRef>>>* to_requeue, bool* requeue_recovery,
                   bool* requeue_snaptrimmer)
    {
        for (auto& p : locks) {
            ...
            if (to_requeue) {
                ...
                to_requeue->emplace_back(std::move(p.second.obc), std::move(_to_requeue));
            }
        }
        locks.clear();
    }
    ~ObcLockManager() { ceph_assert(locks.empty()); }
};
