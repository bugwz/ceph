...
{
    DECLARE_LOCALS;   // 'scrbr' & 'pg_id' aliases
    dout(10) << "WaitPushes::react(const ActivePushesUpd&) pending_active_pushes: " << scrbr->pending_active_pushes()
             << dendl;
...
sc::result DrainReplMaps::react(const GotReplicas&)
{
    ...
    dout(15) << "DrainReplMaps::react(const GotReplicas&): still draining incoming maps: " << scrbr->dump_awaited_maps()
             << dendl;
    return discard_event();
}
...
{
    DECLARE_LOCALS;   // 'scrbr' & 'pg_id' aliases
    dout(10) << "ActiveReplica::react(const SchedReplica&). is_preemptable? " << scrbr->get_preemptor().is_preemptable()
             << dendl;
