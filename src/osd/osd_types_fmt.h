...
template<> struct fmt::formatter<osd_reqid_t>
{
    ...
    template<typename FormatContext> auto format(const osd_reqid_t& req_id, FormatContext& ctx) const
    {
...
template<> struct fmt::formatter<pg_shard_t>
{
    ...
    template<typename FormatContext> auto format(const pg_shard_t& shrd, FormatContext& ctx) const
    {
...
template<> struct fmt::formatter<eversion_t>
{
    ...
    template<typename FormatContext> auto format(const eversion_t& ev, FormatContext& ctx) const
    {
        return fmt::format_to(ctx.out(), "{}'{}", ev.epoch, ev.version);
    }
};

template<> struct fmt::formatter<chunk_info_t>
{
    constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

    template<typename FormatContext> auto format(const chunk_info_t& ci, FormatContext& ctx)
    {
        return fmt::format_to(ctx.out(),
...
template<> struct fmt::formatter<object_manifest_t>
{
    ...
    template<typename FormatContext> auto format(const object_manifest_t& om, FormatContext& ctx) const
    {
...
template<> struct fmt::formatter<object_info_t>
{
    ...
    template<typename FormatContext> auto format(const object_info_t& oi, FormatContext& ctx) const
    {
        ...
        fmt::format_to(
            ctx.out(), " alloc_hint [{} {} {}]", oi.expected_object_size, oi.expected_write_size, oi.alloc_hint_flags);
...
template<> struct fmt::formatter<pg_t>
{
    constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

    template<typename FormatContext> auto format(const pg_t& pg, FormatContext& ctx) const
    {
...
template<> struct fmt::formatter<spg_t>
{
    constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

    template<typename FormatContext> auto format(const spg_t& spg, FormatContext& ctx) const
    {
...
template<> struct fmt::formatter<pg_history_t>
{
    constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

    template<typename FormatContext> auto format(const pg_history_t& pgh, FormatContext& ctx) const
    {
        fmt::format_to(ctx.out(),
...
template<> struct fmt::formatter<pg_info_t>
{
    constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

    template<typename FormatContext> auto format(const pg_info_t& pgi, FormatContext& ctx)
    {
        ...
        fmt::format_to(ctx.out(), " local-lis/les={}/{}", pgi.last_interval_started, pgi.last_epoch_started);
        return fmt::format_to(ctx.out(), " n={} {})", pgi.stats.stats.sum.num_objects, pgi.history);
    }
};

// snaps and snap-sets

template<> struct fmt::formatter<SnapSet>
{
    ...
    template<typename FormatContext> auto format(const SnapSet& snps, FormatContext& ctx)
    {
        if (verbose) {
            // similar to SnapSet::dump()
            fmt::format_to(ctx.out(), "snaps{{{}: clns ({}): ", snps.seq, snps.clones.size());
            ...
            return fmt::format_to(ctx.out(), "{}={}:{}", snps.seq, snps.snaps, snps.clone_snaps);
        }
    }
...
template<> struct fmt::formatter<ScrubMap::object>
{
    constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

    ///\todo: consider passing the 'D" flag to control snapset dump
    template<typename FormatContext> auto format(const ScrubMap::object& so, FormatContext& ctx)
    {
        fmt::format_to(ctx.out(), "so{{ sz:{} dd:{} od:{} ", so.size, so.digest, so.digest_present);
...
template<> struct fmt::formatter<ScrubMap>
{
    template<typename ParseContext> constexpr auto parse(ParseContext& ctx)
    {
        auto it = ctx.begin();
    ...
    template<typename FormatContext> auto format(const ScrubMap& smap, FormatContext& ctx)
    {
        fmt::format_to(
            ctx.out(), "smap{{ valid:{} incr-since:{} #:{}", smap.valid_through, smap.incr_since, smap.objects.size());
        if (debug_log) {
