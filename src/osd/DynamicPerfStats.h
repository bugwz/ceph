...
public:
    ...
    void merge(const DynamicPerfStats& dps)
    {
        for (auto& query_it : dps.data) {
            ...
            for (auto& key_it : query_it.second) {
                ...
                auto update_counter_fnc = [&counter_it](const PerformanceCounterDescriptor& d, PerformanceCounter* c) {
    ...
    {
        std::map<OSDPerfMetricQuery, std::map<OSDPerfMetricKey, PerformanceCounters>> new_data;
        for (auto& query : queries) {
             ...
             const utime_t& latency)
    {
        ...
        auto update_counter_fnc = [&op, inb, outb, &latency](const PerformanceCounterDescriptor& d,
            ...
            switch (d.type) {
            case PerformanceCounterType::OPS: c->first++; return;
            ...
            case PerformanceCounterType::READ_OPS:
                ...
                return;
            case PerformanceCounterType::BYTES: c->first += inb + outb; return;
        ...
        auto get_subkey_fnc = [&osd, &pg_info, &op](const OSDPerfMetricSubKeyDescriptor& d,
            ...
            switch (d.type) {
            case OSDPerfMetricSubKeyType::CLIENT_ID: match_string = stringify(m->get_reqid().name); break;
            case OSDPerfMetricSubKeyType::CLIENT_ADDRESS:
                match_string = stringify(m->get_connection()->get_peer_addr());
                break;
            case OSDPerfMetricSubKeyType::POOL_ID: match_string = stringify(m->get_spg().pool()); break;
            case OSDPerfMetricSubKeyType::NAMESPACE: match_string = m->get_hobj().nspace; break;
            case OSDPerfMetricSubKeyType::OSD_ID: match_string = stringify(osd->get_nodeid()); break;
            case OSDPerfMetricSubKeyType::PG_ID: match_string = stringify(pg_info.pgid); break;
            case OSDPerfMetricSubKeyType::OBJECT_NAME: match_string = m->get_oid().name; break;
            case OSDPerfMetricSubKeyType::SNAP_ID: match_string = stringify(m->get_snapid()); break;
            default: ceph_abort_msg("unknown counter type");
            }
    ...
    void add_to_reports(const std::map<OSDPerfMetricQuery, OSDPerfMetricLimits>& limits,
                        std::map<OSDPerfMetricQuery, OSDPerfMetricReport>* reports)
    {
        for (auto& it : data) {
            ...
            for (auto& limit : query_limits) {
                ...
                typedef std::map<OSDPerfMetricKey, PerformanceCounters>::iterator Iterator;
                ...
                for (; it_counters != counters.end(); it_counters++) {
                    wsum += it_counters->second[index].first;
                    if (ceph::util::generate_random_number(0, wsum) <= it_counters->second[index].first) {
...
private:
    static bool is_limited(const OSDPerfMetricLimits& limits, size_t counters_size)
    {
        if (limits.empty()) {
            return false;
        }
    ...
    std::map<OSDPerfMetricQuery, std::map<OSDPerfMetricKey, PerformanceCounters>> data;
};
