...
static std::map<int, int> max_prio_map = {{OSD_BACKFILL_PRIORITY_BASE, OSD_BACKFILL_DEGRADED_PRIORITY_BASE - 1},
...
typedef interval_set<snapid_t, mempool::osdmap::flat_map> snap_interval_set_t;
...
struct pg_shard_t
{
    ...
    {}
    explicit pg_shard_t(int osd)
        : osd(osd)
        , shard(shard_id_t::NO_SHARD)
    {}
    pg_shard_t(int osd, shard_id_t shard)
        : osd(osd)
        , shard(shard)
    {}
...
inline bool operator<(const osd_reqid_t& l, const osd_reqid_t& r)
{
    return (l.name < r.name) || (l.inc < r.inc) || (l.name == r.name && l.inc == r.inc && l.tid < r.tid);
}
inline bool operator<=(const osd_reqid_t& l, const osd_reqid_t& r)
{
    return (l.name < r.name) || (l.inc < r.inc) || (l.name == r.name && l.inc == r.inc && l.tid <= r.tid);
...
struct object_locator_t
{
    ...
    int64_t get_pool() const { return pool; }
    ...
    bool empty() const { return pool == -1; }
...
{
    out << "@" << loc.pool;
    if (loc.nspace.length()) out << ";" << loc.nspace;
    if (loc.key.length()) out << ":" << loc.key;
    return out;
}
...
public:
    ...
    {}
    explicit request_redirect_t(const object_locator_t& orig, const std::string& robj)
        : redirect_locator(orig)
        , redirect_object(robj)
    {}

    bool empty() const { return redirect_locator.empty() && redirect_object.empty(); }

    void combine_with_locator(object_locator_t& orig, std::string& obj) const
    {
        orig = redirect_locator;
        if (!redirect_object.empty()) obj = redirect_object;
    }
...
{
    ceph_pg v;
    void encode(ceph::buffer::list& bl) const { ceph::encode_raw(v, bl); }
    void decode(ceph::buffer::list::const_iterator& bl) { ceph::decode_raw(v, bl); }
};
...
struct pg_t
{
    ...
    // cppcheck-suppress noExplicitConstructor
    pg_t(const old_pg_t& opg) { *this = opg.v; }
    ...
    ps_t ps() const { return m_seed; }
    int64_t pool() const { return m_pool; }

    static const uint8_t calc_name_buf_size =
    ...
    void set_ps(ps_t p) { m_seed = p; }
    void set_pool(uint64_t p) { m_pool = p; }
    ...
    {
        return (int64_t)m_pool == oid.hobj.get_logical_pool() && oid.match(bits, ps());
    }
    bool contains(int bits, const hobject_t& oid) const
    {
        return (int64_t)m_pool == oid.get_logical_pool() && oid.match(bits, ps());
    }
...
struct spg_t
{
    ...
    spg_t()
        : shard(shard_id_t::NO_SHARD)
    {}
    spg_t(pg_t pgid, shard_id_t shard)
        : pgid(pgid)
        , shard(shard)
    {}
    explicit spg_t(pg_t pgid)
        : pgid(pgid)
        , shard(shard_id_t::NO_SHARD)
    {}
    auto operator<=>(const spg_t&) const = default;
    unsigned get_split_bits(unsigned pg_num) const { return pgid.get_split_bits(pg_num); }
    spg_t get_parent() const { return spg_t(pgid.get_parent(), shard); }
    ps_t ps() const { return pgid.ps(); }
    uint64_t pool() const { return pgid.pool(); }
    void reset_shard(shard_id_t s) { shard = s; }
    ...
    spg_t get_ancestor(unsigned old_pg_num) const { return spg_t(pgid.get_ancestor(old_pg_num), shard); }

    bool is_split(unsigned old_pg_num, unsigned new_pg_num, std::set<spg_t>* pchildren) const
    {
        ...
        if (pchildren && is_split) {
            for (std::set<pg_t>::iterator i = _children.begin(); i != _children.end(); ++i) {
    ...
    bool is_merge_source(unsigned old_pg_num, unsigned new_pg_num, spg_t* parent) const
    {
        spg_t out = *this;
    ...
    bool is_no_shard() const { return shard == shard_id_t::NO_SHARD; }

    ghobject_t make_pgmeta_oid() const { return ghobject_t::make_pgmeta(pgid.pool(), pgid.ps(), shard); }
    ...
    {
        return ghobject_t(
            hobject_t(object_t(name), "", CEPH_NOSNAP, pgid.ps(), hobject_t::get_temp_pool(pgid.pool()), ""),
            ghobject_t::NO_GEN,
            shard);
    }

    unsigned hash_to_shard(unsigned num_shards) const { return ps() % num_shards; }
};
...
public:
    ...
    // named constructors
    static coll_t meta() { return coll_t(); }
    static coll_t pg(spg_t p) { return coll_t(p); }

    const std::string to_str() const { return std::string(_str); }
    const char* c_str() const { return _str; }

    bool parse(const std::string& s);

    int operator<(const coll_t& rhs) const { return type < rhs.type || (type == rhs.type && pgid < rhs.pgid); }

    bool is_meta() const { return type == TYPE_META; }
    ...
    bool is_pg() const { return type == TYPE_PG; }
    ...
    bool is_temp() const { return type == TYPE_PG_TEMP; }
    ...
    int64_t pool() const { return pgid.pool(); }
    ...
    inline bool operator==(const coll_t& rhs) const
    {
        ...
        if (type == TYPE_META) return true;
        return type == rhs.type && pgid == rhs.pgid;
    }
    inline bool operator!=(const coll_t& rhs) const { return !(*this == rhs); }
    ...
    ghobject_t get_min_hobj() const
    {
        ...
        case TYPE_PG:
            ...
            break;
        case TYPE_META: o.hobj.pool = -1; break;
        default: break;
        }
        return o;
    }
    ...
    {
        if (type == TYPE_PG) return pgid.hash_to_shard(num_shards);
        return 0;   // whatever.
    }
...
struct denc_coll_t
{
    ...
    explicit denc_coll_t(const coll_t& coll)
        : coll(coll)
    {}
    operator coll_t() const { return coll; }

    bool operator<(const denc_coll_t& rhs) const { return coll < rhs.coll; }
...
public:
    ...
    eversion_t()
        ...
        , __pad(0)
    {}
    eversion_t(epoch_t e, version_t v)
        : version(v)
        , epoch(e)
        , __pad(0)
    {}
    ...
    explicit eversion_t(ceph::buffer::list& bl)
        : __pad(0)
    {
        decode(bl);
    }
...
struct objectstore_perf_stat_t
{
    ...
    objectstore_perf_stat_t()
        : os_commit_latency_ns(0)
        , os_apply_latency_ns(0)
    {}

    bool operator==(const objectstore_perf_stat_t& r) const
    {
        return os_commit_latency_ns == r.os_commit_latency_ns && os_apply_latency_ns == r.os_apply_latency_ns;
    }
...
public:
    ...
    struct opt_desc_t
    {
        ...
        opt_desc_t(key_t k, type_t t)
            : key(k)
            , type(t)
        {}

        bool operator==(const opt_desc_t& rhs) const { return key == rhs.key && type == rhs.type; }
    };
    ...
    template<typename T> void set(key_t key, const T& val)
    {
        value_t value = val;
        opts[key] = value;
    }

    template<typename T> bool get(key_t key, T* val) const
    {
    ...
    template<typename T> T value_or(key_t key, T&& default_value) const
    {
...
struct pg_pool_t
{
    ...
    std::string_view get_type_name() const { return get_type_name(type); }
    ...
    {
        if (name == "hashpspool") return FLAG_HASHPSPOOL;
        if (name == "full") return FLAG_FULL;
        if (name == "ec_overwrites") return FLAG_EC_OVERWRITES;
        if (name == "incomplete_clones") return FLAG_INCOMPLETE_CLONES;
        if (name == "nodelete") return FLAG_NODELETE;
        if (name == "nopgchange") return FLAG_NOPGCHANGE;
        if (name == "nosizechange") return FLAG_NOSIZECHANGE;
        if (name == "write_fadvise_dontneed") return FLAG_WRITE_FADVISE_DONTNEED;
        if (name == "noscrub") return FLAG_NOSCRUB;
        if (name == "nodeep-scrub") return FLAG_NODEEP_SCRUB;
        if (name == "full_quota") return FLAG_FULL_QUOTA;
        if (name == "nearfull") return FLAG_NEARFULL;
        if (name == "backfillfull") return FLAG_BACKFILLFULL;
        if (name == "selfmanaged_snaps") return FLAG_SELFMANAGED_SNAPS;
        if (name == "pool_snaps") return FLAG_POOL_SNAPS;
        if (name == "creating") return FLAG_CREATING;
        if (name == "eio") return FLAG_EIO;
        if (name == "bulk") return FLAG_BULK;
        if (name == "crimson") return FLAG_CRIMSON;
        return 0;
    }
    ...
    {
        if (s == "none") return CACHEMODE_NONE;
        if (s == "writeback") return CACHEMODE_WRITEBACK;
        if (s == "forward") return CACHEMODE_FORWARD;
        if (s == "readonly") return CACHEMODE_READONLY;
        if (s == "readforward") return CACHEMODE_READFORWARD;
        if (s == "readproxy") return CACHEMODE_READPROXY;
        if (s == "proxy") return CACHEMODE_PROXY;
        return (cache_mode_t)-1;
    }
    const char* get_cache_mode_name() const { return get_cache_mode_name(cache_mode); }
    bool cache_mode_requires_hit_set() const
    {
        ...
        case CACHEMODE_PROXY: return false;
        case CACHEMODE_WRITEBACK:
        case CACHEMODE_READFORWARD:
        case CACHEMODE_READPROXY: return true;
        default: ceph_abort_msg("implement me");
        }
    }
...
public:
    ...
    {
        if (cache_mode != CACHEMODE_NONE) flags |= FLAG_INCOMPLETE_CLONES;
    ...
    bool has_snaps() const { return snaps.size() > 0; }

    bool is_stretch_pool() const { return peering_crush_bucket_count != 0; }

    bool stretch_set_can_peer(const std::set<int>& want, const OSDMap& osdmap, std::ostream* out) const;
    bool stretch_set_can_peer(const std::vector<int>& want, const OSDMap& osdmap, std::ostream* out) const
    {
    ...
    {
        if (s == "none") return TYPE_FINGERPRINT_NONE;
        if (s == "sha1") return TYPE_FINGERPRINT_SHA1;
        if (s == "sha256") return TYPE_FINGERPRINT_SHA256;
        if (s == "sha512") return TYPE_FINGERPRINT_SHA512;
    ...
    {
        if (s == "none") return TYPE_DEDUP_CHUNK_NONE;
        if (s == "fastcdc") return TYPE_DEDUP_CHUNK_FASTCDC;
        if (s == "fixed") return TYPE_DEDUP_CHUNK_FIXEDCDC;
...
public:
    ...
    {
        if (grade_table.size() <= i) return 0;
    ...
    bool require_rollback() const { return is_erasure(); }

    /// true if incomplete clones may be present
    bool allow_incomplete_clones() const { return cache_mode != CACHEMODE_NONE || has_flag(FLAG_INCOMPLETE_CLONES); }
    ...
    const char* get_object_hash_name() const { return ceph_str_hash_name(get_object_hash()); }
    epoch_t get_last_change() const { return last_change; }
    epoch_t get_last_force_op_resend() const { return last_force_op_resend; }
    epoch_t get_last_force_op_resend_prenautilus() const { return last_force_op_resend_prenautilus; }
    epoch_t get_last_force_op_resend_preluminous() const { return last_force_op_resend_preluminous; }
    ...
    bool supports_omap() const { return !(get_type() == TYPE_ERASURE); }
    ...
    bool allows_ecoverwrites() const { return has_flag(FLAG_EC_OVERWRITES); }

    bool is_crimson() const { return has_flag(FLAG_CRIMSON); }

    bool can_shift_osds() const
    {
        switch (get_type()) {
        case TYPE_REPLICATED: return true;
        case TYPE_ERASURE: return false;
        default: ceph_abort_msg("unhandled pool type");
        }
    }
    ...
    void set_pg_num_target(int p) { pg_num_target = p; }
    void set_pgp_num_target(int p) { pgp_num_target = p; }
    void dec_pg_num(pg_t source_pgid, epoch_t ready_epoch, eversion_t source_version, eversion_t target_version,
                    epoch_t last_epoch_started, epoch_t last_epoch_clean)
    {
        --pg_num;
    ...
    void set_quota_max_bytes(uint64_t m) { quota_max_bytes = m; }
    uint64_t get_quota_max_bytes() { return quota_max_bytes; }

    void set_quota_max_objects(uint64_t m) { quota_max_objects = m; }
    uint64_t get_quota_max_objects() { return quota_max_objects; }
...
#define SPLIT_PRESERVE_NONZERO(PARAM)              \
    for (unsigned i = 0; i < out.size(); ++i) {    \
        ...
        for (unsigned i = 0; i < out.size(); ++i) {
            out[i].num_scrub_errors = out[i].num_shallow_scrub_errors + out[i].num_deep_scrub_errors;
        }
...
#undef SPLIT_PRESERVE_NONZERO
    ...
    void calc_copies(int nrep) { num_object_copies = nrep * num_objects; }

    bool is_zero() const { return mem_is_zero((char*)this, sizeof(*this)); }
    ...
    void padding_check()
    {
        static_assert(
            sizeof(object_stat_sum_t) ==
                sizeof(num_bytes) + sizeof(num_objects) + sizeof(num_object_clones) + sizeof(num_object_copies) +
                    sizeof(num_objects_missing_on_primary) + sizeof(num_objects_degraded) +
                    sizeof(num_objects_unfound) + sizeof(num_rd) + sizeof(num_rd_kb) + sizeof(num_wr) +
                    sizeof(num_wr_kb) + sizeof(num_scrub_errors) + sizeof(num_large_omap_objects) +
                    sizeof(num_objects_manifest) + sizeof(num_omap_bytes) + sizeof(num_omap_keys) +
                    sizeof(num_objects_repaired) + sizeof(num_objects_recovered) + sizeof(num_bytes_recovered) +
                    sizeof(num_keys_recovered) + sizeof(num_shallow_scrub_errors) + sizeof(num_deep_scrub_errors) +
                    sizeof(num_objects_dirty) + sizeof(num_whiteouts) + sizeof(num_objects_omap) +
                    sizeof(num_objects_hit_set_archive) + sizeof(num_objects_misplaced) +
                    sizeof(num_bytes_hit_set_archive) + sizeof(num_flush) + sizeof(num_flush_kb) + sizeof(num_evict) +
                    sizeof(num_evict_kb) + sizeof(num_promote) + sizeof(num_flush_mode_high) +
                    sizeof(num_flush_mode_low) + sizeof(num_evict_mode_some) + sizeof(num_evict_mode_full) +
                    sizeof(num_objects_pinned) + sizeof(num_objects_missing) + sizeof(num_legacy_snapsets),
...
struct object_stat_collection_t
{
    ...
    void calc_copies(int nrep) { sum.calc_copies(nrep); }
    ...
    bool is_zero() const { return sum.is_zero(); }

    void clear() { sum.clear(); }

    void floor(int64_t f) { sum.floor(f); }

    void add(const object_stat_sum_t& o) { sum.add(o); }

    void add(const object_stat_collection_t& o) { sum.add(o.sum); }
    void sub(const object_stat_collection_t& o) { sum.sub(o.sum); }
};
WRITE_CLASS_ENCODER(object_stat_collection_t)

inline bool operator==(const object_stat_collection_t& l, const object_stat_collection_t& r)
{
    return l.sum == r.sum;
}

enum class scrub_level_t : bool
{
    shallow = false,
    deep = true
};
enum class scrub_type_t : bool
{
    not_repair = false,
    do_repair = true
};
...
struct pg_stat_t
{
    ...
    std::pair<epoch_t, version_t> get_version_pair() const { return {reported_epoch, reported_seq}; }

    void floor(int64_t f)
    {
        stats.floor(f);
        if (log_size < f) log_size = f;
        if (ondisk_log_size < f) ondisk_log_size = f;
        if (snaptrimq_len < f) snaptrimq_len = f;
    }
    ...
    void add(const pg_stat_t& o)
    {
        ...
        snaptrimq_len = std::min((uint64_t)snaptrimq_len + o.snaptrimq_len, (uint64_t)(1ull << 31));
...
struct store_statfs_t
{
    ...
    void reset() { *this = store_statfs_t(); }
    void floor(int64_t f)
    {
#define FLOOR(x) \
    if (int64_t(x) < f) x = f
...
#undef FLOOR
    ...
    bool is_zero() const { return *this == store_statfs_t(); }

    uint64_t get_used() const { return total - available - internally_reserved; }

    // this accumulates both actually used and statfs's internally_reserved
    uint64_t get_used_raw() const { return total - available; }
    ...
    uint64_t kb_avail() const { return available >> 10; }
    uint64_t kb() const { return total >> 10; }
    uint64_t kb_used() const { return (total - available - internally_reserved) >> 10; }
    uint64_t kb_used_raw() const { return get_used_raw() >> 10; }

    uint64_t kb_used_data() const { return allocated >> 10; }
    uint64_t kb_used_omap() const { return omap_allocated >> 10; }

    uint64_t kb_used_internal_metadata() const { return internal_metadata >> 10; }
...
{
    return l.statfs == r.statfs && l.snap_trim_queue_len == r.snap_trim_queue_len &&
           l.num_snap_trimming == r.num_snap_trimming && l.num_shards_repaired == r.num_shards_repaired &&
           l.hb_peers == r.hb_peers && l.op_queue_age_hist == r.op_queue_age_hist && l.os_perf_stat == r.os_perf_stat &&
           l.num_pgs == r.num_pgs && l.num_osds == r.num_osds && l.num_per_pool_osds == r.num_per_pool_osds &&
...
inline std::ostream& operator<<(std::ostream& out, const osd_stat_t& s)
{
    return out << "osd_stat(" << s.statfs << ", "
               << "peers " << s.hb_peers << " op hist " << s.op_queue_age_hist.h << ")";
}
...
struct pool_stat_t
{
    ...
    void floor(int64_t f)
    {
        ...
        if (ondisk_log_size < f) ondisk_log_size = f;
        if (up < f) up = f;
        if (acting < f) acting = f;
        if (num_store_stats < f) num_store_stats = f;
    }
    ...
    bool is_zero() const
    {
        return (stats.is_zero() && store_stats.is_zero() && log_size == 0 && ondisk_log_size == 0 && up == 0 &&
                acting == 0 && num_store_stats == 0);
    }
...
struct pg_hit_set_info_t
{
    ...
    friend bool operator==(const pg_hit_set_info_t& l, const pg_hit_set_info_t& r)
    {
        return l.begin == r.begin && l.end == r.end && l.version == r.version && l.using_gmt == r.using_gmt;
    }
...
struct pg_hit_set_history_t
{
    ...
    friend bool operator==(const pg_hit_set_history_t& l, const pg_hit_set_history_t& r)
    {
        return l.current_last_update == r.current_last_update && l.history == r.history;
    }
...
struct pg_history_t
{
    ...
    {
        return l.epoch_created == r.epoch_created && l.epoch_pool_created == r.epoch_pool_created &&
               l.last_epoch_started == r.last_epoch_started && l.last_interval_started == r.last_interval_started &&
               l.last_epoch_clean == r.last_epoch_clean && l.last_interval_clean == r.last_interval_clean &&
               l.last_epoch_split == r.last_epoch_split && l.last_epoch_marked_full == r.last_epoch_marked_full &&
               l.same_up_since == r.same_up_since && l.same_interval_since == r.same_interval_since &&
               l.same_primary_since == r.same_primary_since && l.last_scrub == r.last_scrub &&
               l.last_deep_scrub == r.last_deep_scrub && l.last_scrub_stamp == r.last_scrub_stamp &&
    ...
    ceph::signedspan refresh_prior_readable_until_ub(ceph::signedspan now,   ///< now, relative to osd startup_time
                                                     ceph::signedspan ub)
...
{
    out << "ec=" << h.epoch_created << "/" << h.epoch_pool_created << " lis/c=" << h.last_interval_started << "/"
        << h.last_interval_clean << " les/c/f=" << h.last_epoch_started << "/" << h.last_epoch_clean << "/"
        << h.last_epoch_marked_full << " sis=" << h.same_interval_since;
...
struct pg_info_t
{
    ...
    {
        return l.pgid == r.pgid && l.last_update == r.last_update && l.last_complete == r.last_complete &&
               l.last_epoch_started == r.last_epoch_started && l.last_interval_started == r.last_interval_started &&
               l.last_user_version == r.last_user_version && l.log_tail == r.log_tail &&
               l.last_backfill == r.last_backfill && l.purged_snaps == r.purged_snaps && l.stats == r.stats &&
               l.history == r.history && l.hit_set == r.hit_set;
    }
    ...
    void set_last_backfill(hobject_t pos) { last_backfill = pos; }
...
inline std::ostream& operator<<(std::ostream& out, const pg_info_t& pgi)
{
    ...
    if (pgi.is_empty())
        ...
        if (pgi.last_complete != pgi.last_update) out << " lc " << pgi.last_complete;
        out << " (" << pgi.log_tail << "," << pgi.last_update << "]";
    }
    if (pgi.is_incomplete()) out << " lb " << pgi.last_backfill;
    // out << " c " << pgi.epoch_created;
    out << " local-lis/les=" << pgi.last_interval_started << "/" << pgi.last_epoch_started;
...
struct pg_fast_info_t
{
    ...
    {
        if (last_update <= info->last_update) return false;
...
public:
    struct pg_interval_t
    {
        ...
        pg_interval_t(std::vector<int32_t>&& up, std::vector<int32_t>&& acting, epoch_t first, epoch_t last,
                      bool maybe_went_rw, int32_t primary, int32_t up_primary)
            : up(up)
            , acting(acting)
            , first(first)
            , last(last)
    ...
    public:
        ...
        virtual std::set<pg_shard_t> get_all_participants(bool ec_pool) const = 0;
        ...
        virtual void iterate_mayberw_back_to(epoch_t les,
                                             std::function<void(epoch_t, const std::set<pg_shard_t>&)>&& f) const = 0;

        virtual bool has_full_intervals() const { return false; }
        virtual void iterate_all_intervals(std::function<void(const pg_interval_t&)>&& f) const
        {
...
private:
    ...
    explicit PastIntervals(interval_rep* rep)
        : past_intervals(rep)
    {}
...
public:
     ...
     */
    static bool is_new_interval(int old_acting_primary, int new_acting_primary, const std::vector<int>& old_acting,
                                const std::vector<int>& new_acting, int old_up_primary, int new_up_primary,
                                const std::vector<int>& old_up, const std::vector<int>& new_up, int old_size,
                                int new_size, int old_min_size, int new_min_size, unsigned old_pg_num,
                                unsigned new_pg_num, unsigned old_pg_num_pending, unsigned new_pg_num_pending,
                                bool old_sort_bitwise, bool new_sort_bitwise, bool old_recovery_deletes,
                                bool new_recovery_deletes, uint32_t old_crush_count, uint32_t new_crush_count,
                                uint32_t old_crush_target, uint32_t new_crush_target, uint32_t old_crush_barrier,
                                uint32_t new_crush_barrier, int32_t old_crush_member, int32_t new_crush_member,
                                pg_t pgid);

    /**
     * Determines whether there is an interval change
     */
    static bool is_new_interval(int old_acting_primary,               ///< [in] primary as of lastmap
    ...
    {
        return check_new_interval(old_acting_primary,
                                  new_acting_primary,
                                  old_acting,
                                  new_acting,
                                  old_up_primary,
    ...
    template<typename F> void iterate_mayberw_back_to(epoch_t les, F&& f) const
    {
        ceph_assert(past_intervals);
     ...
     */
    std::set<pg_shard_t> get_might_have_unfound(pg_shard_t pg_whoami, bool ec_pool) const
    {
        ceph_assert(past_intervals);
     ...
     */
    std::set<pg_shard_t> get_all_probe(bool ec_pool) const
    {
        ceph_assert(past_intervals);
    ...
    struct PriorSet
    {
        ...
        std::map<int, epoch_t> blocked_by;   ///< current lost_at values for any OSDs in cur set for which (re)marking
                                             ///< them lost would affect cur set
        ...
        {
            return (ec_pool == rhs.ec_pool) && (probe == rhs.probe) && (down == rhs.down) &&
                   (blocked_by == rhs.blocked_by) && (pg_down == rhs.pg_down);
        }

        bool affected_by_map(const OSDMap& osdmap, const DoutPrefixProvider* dpp) const;

        // For verifying tests
        PriorSet(bool ec_pool, std::set<pg_shard_t> probe, std::set<int> down, std::map<int, epoch_t> blocked_by,
                 bool pg_down, const IsPGRecoverablePredicate* pcontdec)
            : ec_pool(ec_pool)
            , probe(probe)
            , down(down)
    ...
    private:
        template<typename F>
        PriorSet(const PastIntervals& past_intervals, bool ec_pool, epoch_t last_epoch_started,
                 const IsPGRecoverablePredicate* c, F f, const std::vector<int>& up, const std::vector<int>& acting,
                 const DoutPrefixProvider* dpp);

        friend class PastIntervals;
    };

    template<typename... Args> PriorSet get_prior_set(Args&&... args) const
    {
...
template<typename F>
PastIntervals::PriorSet::PriorSet(const PastIntervals& past_intervals, bool ec_pool, epoch_t last_epoch_started,
                                  const IsPGRecoverablePredicate* c, F f, const std::vector<int>& up,
                                  const std::vector<int>& acting, const DoutPrefixProvider* dpp)
    : ec_pool(ec_pool)
    , pg_down(false)
    , pcontdec(c)
{
    ...
    past_intervals.iterate_mayberw_back_to(last_epoch_started, [&](epoch_t start, const std::set<pg_shard_t>& acting) {
        ldpp_dout(dpp, 10) << "build_prior maybe_rw interval:" << start << ", acting: " << acting << dendl;
        ...
        for (auto&& so : acting) {
            ...
            case DNE:
            {
                ldpp_dout(dpp, 10) << "build_prior  prior osd." << so.osd << " no longer exists" << dendl;
                break;
            }
            case LOST:
            {
                ldpp_dout(dpp, 10) << "build_prior  prior osd." << so.osd << " is down, but lost_at " << lost_at
                                   << dendl;
                up_now.insert(so);
                break;
            }
            case DOWN:
            {
                ldpp_dout(dpp, 10) << "build_prior  prior osd." << so.osd << " is down" << dendl;
    ...
    ldpp_dout(dpp, 10) << "build_prior final: probe " << probe << " down " << down << " blocked_by " << blocked_by
                       << (pg_down ? " pg_down" : "") << dendl;
}
...
struct pg_notify_t
{
    ...
    {}
    pg_notify_t(shard_id_t to, shard_id_t from, epoch_t query_epoch, epoch_t epoch_sent, const pg_info_t& info,
...
struct pg_query_t
{
    ...
    pg_query_t()
        ...
        , from(shard_id_t::NO_SHARD)
    {}
    pg_query_t(int t, shard_id_t to, shard_id_t from, const pg_history_t& h, epoch_t epoch_sent)
        : type(t)
        , history(h)
        , epoch_sent(epoch_sent)
        , to(to)
    ...
    pg_query_t(int t, shard_id_t to, shard_id_t from, eversion_t s, const pg_history_t& h, epoch_t epoch_sent)
        : type(t)
        , since(s)
        , history(h)
        , epoch_sent(epoch_sent)
...
inline std::ostream& operator<<(std::ostream& out, const pg_query_t& q)
{
    ...
    if (q.type == pg_query_t::LOG) out << " " << q.history;
...
struct pg_lease_t
{
    ...
    pg_lease_t() {}
    pg_lease_t(ceph::signedspan ru, ceph::signedspan ruub, ceph::signedspan i)
        : readable_until(ru)
    ...
    {
        return out << "pg_lease(ru " << l.readable_until << " ub " << l.readable_until_ub << " int " << l.interval
                   << ")";
    }
};
...
public:
    ...
    public:
         ...
         */
        virtual void try_rmobject(version_t old_version) { rmobject(old_version); }
        virtual void create() {}
        virtual void update_snaps(const std::set<snapid_t>& old_snaps) {}
        virtual void rollback_extents(version_t gen, const std::vector<std::pair<uint64_t, uint64_t>>& extents) {}
        virtual ~Visitor() {}
    };
    ...
    {
        if (!can_local_rollback || rollback_info_completed) return;
    ...
    {
        if (!can_local_rollback || rollback_info_completed) return false;
        ENCODE_START(1, 1, bl);
        append_id(DELETE);
    ...
    void rollback_extents(version_t gen, const std::vector<std::pair<uint64_t, uint64_t>>& extents)
    {
        ceph_assert(can_local_rollback);
        ceph_assert(!rollback_info_completed);
        if (max_required_version < 2) max_required_version = 2;
        ENCODE_START(2, 2, bl);
    ...
    bool can_rollback() const { return can_local_rollback; }
    bool empty() const { return can_local_rollback && (bl.length() == 0); }

    bool requires_kraken() const { return max_required_version >= 2; }
...
struct OSDOp
{
     ...
     */
    template<typename V> static void split_osd_op_vector_in_data(V& ops, ceph::buffer::list& in)
    {
     ...
     */
    template<typename V> static void merge_osd_op_vector_in_data(V& ops, ceph::buffer::list& out)
    {
        for (unsigned i = 0; i < ops.size(); i++) {
     ...
     * @param ops [in] vector of OSDOps
     */
    template<typename V> static void clear_data(V& ops)
    {
        for (unsigned i = 0; i < ops.size(); i++) {
            OSDOp& op = ops[i];
            op.outdata.clear();
            if (ceph_osd_op_type_attr(op.op.op) && op.op.xattr.name_len && op.indata.length() >= op.op.xattr.name_len) {
                ceph::buffer::list bl;
            ...
            else if (ceph_osd_op_type_exec(op.op.op) && op.op.cls.class_len &&
                     op.indata.length() > (op.op.cls.class_len + op.op.cls.method_len)) {
...
struct pg_log_op_return_item_t
{
    ...
    friend bool operator==(const pg_log_op_return_item_t& lhs, const pg_log_op_return_item_t& rhs)
    {
        return lhs.rval == rhs.rval && lhs.bl.contents_equal(rhs.bl);
    }
    friend bool operator!=(const pg_log_op_return_item_t& lhs, const pg_log_op_return_item_t& rhs)
    {
        return !(lhs == rhs);
...
struct pg_log_entry_t
{
    ...
    static const char* get_op_name(int op)
    {
        ...
        case LOST_REVERT: return "l_revert";
        case LOST_DELETE: return "l_delete";
        case LOST_MARK: return "l_mark";
        case CLEAN: return "clean";
        case ERROR: return "error";
        default: return "unknown";
        }
    }
    const char* get_op_name() const { return get_op_name(op); }
    ...
    pg_log_entry_t(int _op, const hobject_t& _soid, const eversion_t& v, const eversion_t& pv, version_t uv,
                   const osd_reqid_t& rid, const utime_t& mt, int return_code)
        : soid(_soid)
        , reqid(rid)
        , version(v)
        , prior_version(pv)
    ...
    {
        return is_clone() || is_modify() || is_promote() || is_clean() || is_lost_revert() || is_lost_mark();
    }
    bool is_delete() const { return op == DELETE || op == LOST_DELETE; }

    bool can_rollback() const { return mod_desc.can_rollback(); }

    void mark_unrollbackable() { mod_desc.mark_unrollbackable(); }

    bool requires_kraken() const { return mod_desc.requires_kraken(); }
    ...
    bool object_is_indexed() const { return !is_error(); }

    bool reqid_is_indexed() const { return reqid != osd_reqid_t() && (op == MODIFY || op == DELETE || op == ERROR); }
...
struct pg_log_dup_t
{
    ...
    {}
    pg_log_dup_t(const eversion_t& v, version_t uv, const osd_reqid_t& rid, int return_code)
        : reqid(rid)
        , version(v)
        , user_version(uv)
        , return_code(return_code)
    ...
    {
        return reqid == rhs.reqid && version == rhs.version && user_version == rhs.user_version &&
               return_code == rhs.return_code && op_returns == rhs.op_returns;
    }
    bool operator!=(const pg_log_dup_t& rhs) const { return !(*this == rhs); }
...
public:
    ...
    pg_log_t() = default;
    pg_log_t(const eversion_t& last_update, const eversion_t& log_tail, const eversion_t& can_rollback_to,
             const eversion_t& rollback_info_trimmed_to, mempool::osd_pglog::list<pg_log_entry_t>&& entries,
    ...
    {}
    pg_log_t(const eversion_t& last_update, const eversion_t& log_tail, const eversion_t& can_rollback_to,
             const eversion_t& rollback_info_trimmed_to, const std::list<pg_log_entry_t>& entries,
    ...
    eversion_t get_rollback_info_trimmed_to() const { return rollback_info_trimmed_to; }
    eversion_t get_can_rollback_to() const { return can_rollback_to; }
    ...
    pg_log_t split_out_child(pg_t child_pgid, unsigned split_bits)
    {
        ...
        for (auto i = oldlog.begin(); i != oldlog.end();) {
        ...
        return pg_log_t(
            head, tail, can_rollback_to, rollback_info_trimmed_to, std::move(childlog), std::move(childdups));
    }
    ...
    mempool::osd_pglog::list<pg_log_entry_t> rewind_from_head(eversion_t newhead)
    {
        ...
        if (can_rollback_to > newhead) can_rollback_to = newhead;

        if (rollback_info_trimmed_to > newhead) rollback_info_trimmed_to = newhead;
    ...
    bool null() const { return head.version == 0 && head.epoch == 0; }

    uint64_t approx_size() const { return head.version - tail.version; }

    static void filter_log(spg_t import_pgid, const OSDMap& curmap, const std::string& hit_set_namespace,
                           const pg_log_t& in, pg_log_t& out, pg_log_t& reject);
...
{
    out << "log((" << log.tail << "," << log.head << "], crt=" << log.get_can_rollback_to() << ")";
    return out;
}
...
struct pg_missing_item
{
    ...
    pg_missing_item()
        : flags(FLAG_NONE)
    {}
    explicit pg_missing_item(eversion_t n)
        : need(n)
        , flags(FLAG_NONE)
    {}   // have no old version
        ...
        , have(h)
    {
        ...
        if (old_style) clean_regions.mark_fully_dirty();
    }
    ...
    void set_delete(bool is_delete) { flags = is_delete ? FLAG_DELETE : FLAG_NONE; }

    bool is_delete() const { return (flags & FLAG_DELETE) == FLAG_DELETE; }
    ...
    bool operator!=(const pg_missing_item& rhs) const { return !(*this == rhs); }
};
...
{
public:
    virtual const std::map<hobject_t, pg_missing_item>& get_items() const = 0;
...
template<bool Track> class ChangeTracker
{
public:
    void changed(const hobject_t& obj) {}
    template<typename F> void get_changed(F&& f) const {}
    void flush() {}
    bool is_clean() const { return true; }
};
template<> class ChangeTracker<true>
{
    std::set<hobject_t> _changed;

public:
    void changed(const hobject_t& obj) { _changed.insert(obj); }
    template<typename F> void get_changed(F&& f) const
    {
    ...
    void flush() { _changed.clear(); }
    bool is_clean() const { return _changed.empty(); }
};

template<bool TrackChanges> class pg_missing_set : public pg_missing_const_i
{
...
public:
    ...
    template<typename missing_type> pg_missing_set(const missing_type& m)
    {
        missing = m.get_items();
        rmissing = m.get_rmissing();
        may_include_deletes = m.get_may_include_deletes();
        for (auto&& i : missing) tracker.changed(i.first);
    }

    bool may_include_deletes = false;

    const std::map<hobject_t, item>& get_items() const override { return missing; }
    const std::map<version_t, hobject_t>& get_rmissing() const override { return rmissing; }
    bool get_may_include_deletes() const override { return may_include_deletes; }
    unsigned int num_missing() const override { return missing.size(); }
    bool have_missing() const override { return !missing.empty(); }
    ...
    bool is_missing(const hobject_t& oid, pg_missing_item* out = nullptr) const override
    {
        ...
        if (iter == missing.end()) return false;
        if (out) *out = iter->second;
        return true;
    }
    bool is_missing(const hobject_t& oid, eversion_t v) const override
    {
        std::map<hobject_t, item>::const_iterator m = missing.find(oid);
        if (m == missing.end()) return false;
        const item& item(m->second);
        if (item.need > v) return false;
        return true;
    }
    ...
    void rm(const hobject_t& oid, eversion_t v)
    {
        ...
        if (p != missing.end() && p->second.need <= v) rm(p);
    }
    ...
    void split_into(pg_t child_pgid, unsigned split_bits, pg_missing_set* omissing)
    {
        omissing->may_include_deletes = may_include_deletes;
        unsigned mask = ~((~0) << split_bits);
        for (std::map<hobject_t, item>::iterator i = missing.begin(); i != missing.end();) {
            if ((i->first.get_hash() & mask) == child_pgid.m_seed) {
                omissing->add(i->first, i->second.need, i->second.have, i->second.is_delete());
                rm(i++);
            }
            else {
                ++i;
    ...
    void clear()
    {
        for (auto const& i : missing) tracker.changed(i.first);
        missing.clear();
        rmissing.clear();
    }
    ...
    void decode(ceph::buffer::list::const_iterator& bl, int64_t pool = -1)
    {
        ...
        for (std::map<hobject_t, item>::iterator it = missing.begin(); it != missing.end(); ++it)
    ...
    void dump(ceph::Formatter* f) const
    {
        ...
        for (std::map<hobject_t, item>::const_iterator p = missing.begin(); p != missing.end(); ++p) {
    ...
    template<typename F> void filter_objects(F&& f)
    {
    ...
    template<typename F> void get_changed(F&& f) const { tracker.get_changed(f); }
    void flush() { tracker.flush(); }
    bool is_clean() const { return tracker.is_clean(); }
    template<typename missing_t> bool debug_verify_from_init(const missing_t& init_missing, std::ostream* oss) const
    {
        if (!TrackChanges) return true;
        ...
        if (check_missing.size() != missing.size()) {
            if (oss) {
                *oss << "Size mismatch, check: " << check_missing.size() << ", actual: " << missing.size() << "\n";
            }
            ok = false;
        }
        for (auto& i : missing) {
            if (!check_missing.count(i.first)) {
                if (oss) *oss << "check_missing missing " << i.first << "\n";
                ok = false;
            }
            else if (check_missing[i.first] != i.second) {
                if (oss)
                    *oss << "check_missing missing item mismatch on " << i.first
                         << ", check: " << check_missing[i.first] << ", actual: " << i.second << "\n";
                ok = false;
            }
        }
        if (oss && !ok) {
...
void encode(const pg_missing_set<TrackChanges>& c, ceph::buffer::list& bl, uint64_t features = 0)
{
    ENCODE_DUMP_PRE();
    c.encode(bl, features);
    ENCODE_DUMP_POST(cl);
}
template<bool TrackChanges> void decode(pg_missing_set<TrackChanges>& c, ceph::buffer::list::const_iterator& p)
{
    c.decode(p);
}
template<bool TrackChanges> std::ostream& operator<<(std::ostream& out, const pg_missing_set<TrackChanges>& missing)
{
    out << "missing(" << missing.num_missing() << " may_include_deletes = " << missing.may_include_deletes;
...
template<typename T> struct pg_nls_response_template
{
...
struct object_copy_cursor_t
{
    ...
    bool is_initial() const { return !attr_complete && data_offset == 0 && omap_offset.empty(); }
    bool is_complete() const { return attr_complete && data_complete && omap_complete; }
...
public:
    ...
    std::vector<std::pair<uint64_t, uint64_t>>
        buffer_extents;   // off -> len.  extents in buffer being mapped (may be fragmented bc of striping!)
...
{
    return out << "extent(" << ex.oid << " (" << ex.objectno << ") in " << ex.oloc << " " << ex.offset << "~"
               << ex.length << " -> " << ex.buffer_extents << ")";
}
...
{
    return out << "sb(" << sb.cluster_fsid << " osd." << sb.whoami << " " << sb.osd_fsid << " e" << sb.current_epoch
...
struct SnapSet
{
    ...
    SnapContext get_ssc_as_of(snapid_t as_of) const
    {
        SnapContext out;
        out.seq = as_of;
        for (auto p = clone_snaps.rbegin(); p != clone_snaps.rend(); ++p) {
...
{
    return l.cookie == r.cookie && l.timeout_seconds == r.timeout_seconds && l.addr == r.addr;
}
...
{
    return out << "notify(cookie " << n.cookie << " notify" << n.notify_id << " " << n.timeout << "s)";
}
...
public:
    ...
    void dec_ref(const hobject_t& hoid, unsigned num = 1) { mut_ref(hoid, -num); }
    void inc_ref(const hobject_t& hoid, unsigned num = 1) { mut_ref(hoid, num); }
    void mut_ref(const hobject_t& hoid, int num)
    {
        [[maybe_unused]] auto [iter, _] = ref_delta.try_emplace(hoid, 0);
        iter->second += num;
        if (iter->second == 0) ref_delta.erase(iter);
    }
    ...
    bool operator==(const object_ref_delta_t& rhs) const { return ref_delta == rhs.ref_delta; }
    bool operator!=(const object_ref_delta_t& rhs) const { return !(*this == rhs); }
    bool is_empty() { return ref_delta.empty(); }
    uint64_t size() { return ref_delta.size(); }
...
struct chunk_info_t
{
    ...
    static std::string get_flag_string(uint64_t flags)
    {
        ...
        if (r.length()) return r.substr(1);
        return r;
    }
    bool test_flag(cflag_t f) const { return (flags & f) == f; }
    void set_flag(cflag_t f) { flags = (cflag_t)(flags | f); }
    void set_flags(cflag_t f) { flags = f; }
    void clear_flag(cflag_t f) { flags = (cflag_t)(flags & ~f); }
    void clear_flags() { flags = (cflag_t)0; }
    bool is_dirty() const { return test_flag(FLAG_DIRTY); }
    bool is_missing() const { return test_flag(FLAG_MISSING); }
    bool has_reference() const { return test_flag(FLAG_HAS_REFERENCE); }
    bool has_fingerprint() const { return test_flag(FLAG_HAS_FINGERPRINT); }
    void encode(ceph::buffer::list& bl) const;
    void decode(ceph::buffer::list::const_iterator& bl);
    void dump(ceph::Formatter* f) const;
    friend std::ostream& operator<<(std::ostream& out, const chunk_info_t& ci);
    bool operator==(const chunk_info_t& cit) const;
    bool operator!=(const chunk_info_t& cit) const { return !(cit == *this); }
};
...
struct object_manifest_t
{
    ...
    bool is_redirect() const { return type == TYPE_REDIRECT; }
    bool is_chunked() const { return type == TYPE_CHUNKED; }
     ...
     *
     */
    void calc_refs_to_inc_on_set(const object_manifest_t* g,   ///< [in] manifest for clone > *this
     ...
     *
     */
    void calc_refs_to_drop_on_modify(const object_manifest_t* l,                ///< [in] manifest for previous clone
     ...
     */
    void calc_refs_to_drop_on_removal(const object_manifest_t* g,   ///< [in] manifest for clone > *this
                                      const object_manifest_t* l,   ///< [in] manifest for clone < *this
                                      object_ref_delta_t& delta     ///< [out] set of refs to drop
    ) const;
...
struct object_info_t
{
    ...
    static std::string get_flag_string(flag_t flags)
    {
        ...
        if (s.length()) return s.substr(1);
        return s;
    }
    static std::vector<std::string> get_flag_vector(flag_t flags)
    {
        std::vector<std::string> sv;
        if (flags & FLAG_LOST) sv.insert(sv.end(), "lost");
        if (flags & FLAG_WHITEOUT) sv.insert(sv.end(), "whiteout");
        if (flags & FLAG_DIRTY) sv.insert(sv.end(), "dirty");
        if (flags & FLAG_USES_TMAP) sv.insert(sv.end(), "uses_tmap");
        if (flags & FLAG_OMAP) sv.insert(sv.end(), "omap");
        if (flags & FLAG_DATA_DIGEST) sv.insert(sv.end(), "data_digest");
        if (flags & FLAG_OMAP_DIGEST) sv.insert(sv.end(), "omap_digest");
        if (flags & FLAG_CACHE_PIN) sv.insert(sv.end(), "cache_pin");
        if (flags & FLAG_MANIFEST) sv.insert(sv.end(), "manifest");
        if (flags & FLAG_REDIRECT_HAS_REFERENCE) sv.insert(sv.end(), "redirect_has_reference");
        return sv;
    }
    std::string get_flag_string() const { return get_flag_string(flags); }
    ...
    bool test_flag(flag_t f) const { return (flags & f) == f; }
    void set_flag(flag_t f) { flags = (flag_t)(flags | f); }
    void clear_flag(flag_t f) { flags = (flag_t)(flags & ~f); }
    bool is_lost() const { return test_flag(FLAG_LOST); }
    bool is_whiteout() const { return test_flag(FLAG_WHITEOUT); }
    bool is_dirty() const { return test_flag(FLAG_DIRTY); }
    bool is_omap() const { return test_flag(FLAG_OMAP); }
    bool is_data_digest() const { return test_flag(FLAG_DATA_DIGEST); }
    bool is_omap_digest() const { return test_flag(FLAG_OMAP_DIGEST); }
    bool is_cache_pinned() const { return test_flag(FLAG_CACHE_PIN); }
    bool has_manifest() const { return test_flag(FLAG_MANIFEST); }
    ...
    explicit object_info_t(const ceph::buffer::list& bl) { decode(bl); }
...
struct ObjectRecoveryProgress
{
    ...
    {
        return (data_recovered_to >= (info.copy_subset.empty() ? 0 : info.copy_subset.range_end())) && omap_complete;
    }
...
struct ScrubMap
{
    ...
    void clear_from(const hobject_t& start) { objects.erase(objects.lower_bound(start), objects.end()); }
    void insert(const ScrubMap& r) { objects.insert(r.objects.begin(), r.objects.end()); }
...
struct ScrubMapBuilder
{
    ...
    bool done() { return pos >= ls.size(); }
    void reset() { *this = ScrubMapBuilder(); }

    bool data_done() { return data_pos < 0; }
...
struct watch_item_t
{
    ...
    watch_item_t()
        ...
        , timeout_seconds(0)
    {}
    watch_item_t(entity_name_t name, uint64_t cookie, uint32_t timeout, const entity_addr_t& addr)
        : name(name)
        , cookie(cookie)
...
struct clone_info
{
    ...
    void dump(ceph::Formatter* f) const
    {
        ...
        for (std::vector<std::pair<uint64_t, uint64_t>>::const_iterator q = overlap.begin(); q != overlap.end(); ++q) {
...
struct pool_pg_num_history_t
{
    ...
    void log_pg_num_change(epoch_t epoch, int64_t pool, uint32_t pg_num) { pg_nums[pool][epoch] = pg_num; }
    void log_pool_delete(epoch_t epoch, int64_t pool) { deleted_pools.insert(std::make_pair(epoch, pool)); }
    ...
    {
        return out << "pg_num_history(e" << h.epoch << " pg_nums " << h.pg_nums << " deleted_pools " << h.deleted_pools
                   << ")";
    }
};
...
int prepare_info_keymap(CephContext* cct, std::map<std::string, ceph::buffer::list>* km, std::string* key_to_remove,
                        epoch_t epoch, pg_info_t& info, pg_info_t& last_written_info, PastIntervals& past_intervals,
                        bool dirty_big_info, bool dirty_epoch, bool try_fast_info, PerfCounters* logger = nullptr,
...
void create_pg_collection(ceph::os::Transaction& t, spg_t pgid, int bits);

void init_pg_ondisk(ceph::os::Transaction& t, spg_t pgid, const pg_pool_t* pool);
...
public:
    PGLSFilter();
    virtual ~PGLSFilter();
    virtual bool filter(const hobject_t& obj, const ceph::buffer::list& xattr_data) const = 0;
...
public:
    ...
    bool filter(const hobject_t& obj, const ceph::buffer::list& xattr_data) const override;
};

// alias name for this structure:
using missing_map_t = std::map<hobject_t, std::pair<std::optional<uint32_t>, std::optional<uint32_t>>>;
