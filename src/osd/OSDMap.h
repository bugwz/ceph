...
struct PGTempMap
{
    ...
    {
        return l.map.size() == r.map.size() && l.data.contents_equal(r.data);
    }
    ...
    public:
        iterator(map_t::const_iterator p, map_t::const_iterator e)
            : it(p)
            , end(e)
        {
            init_current();
        ...
        const std::pair<pg_t, std::vector<int32_t>>& operator*() const { return current; }
        const std::pair<pg_t, std::vector<int32_t>>* operator->() const { return &current; }
        friend bool operator==(const iterator& l, const iterator& r) { return l.it == r.it; }
        friend bool operator!=(const iterator& l, const iterator& r) { return l.it != r.it; }
        iterator& operator++()
        {
            ++it;
            if (it != end) init_current();
            return *this;
        }
    ...
    iterator begin() const { return iterator(map.begin(), map.end()); }
    iterator end() const { return iterator(map.end(), map.end()); }
    iterator find(pg_t pgid) const { return iterator(map.find(pgid), map.end()); }
    size_t size() const { return map.size(); }
    size_t count(pg_t pgid) const { return map.count(pgid); }
    void erase(pg_t pgid) { map.erase(pgid); }
    void clear()
    {
        map.clear();
        data.clear();
    ...
    void encode(ceph::buffer::list& bl) const { encode(pg_temp, bl); }
    void decode(ceph::buffer::list::const_iterator& p) { decode(pg_temp, p); }
    friend bool operator==(const PGTempMap& l, const PGTempMap& r)
    {
        return l.pg_temp.size() == r.pg_temp.size() && l.pg_temp == r.pg_temp;
    }
    ...
    public:
        iterator(mempool::osdmap::map<pg_t, mempool::osdmap::vector<int32_t>>::const_iterator p)
            : it(p)
        {}

        std::pair<pg_t, const mempool::osdmap::vector<int32_t>&> operator*() const { return *it; }
        const std::pair<const pg_t, mempool::osdmap::vector<int32_t>>* operator->() const { return &*it; }
        friend bool operator==(const iterator& l, const iterator& r) { return l.it == r.it; }
        friend bool operator!=(const iterator& l, const iterator& r) { return l.it != r.it; }
        iterator& operator++()
        {
    ...
    iterator begin() const { return iterator(pg_temp.cbegin()); }
    iterator end() const { return iterator(pg_temp.cend()); }
    iterator find(pg_t pgid) const { return iterator(pg_temp.find(pgid)); }
    size_t size() const { return pg_temp.size(); }
    size_t count(pg_t pgid) const { return pg_temp.count(pgid); }
    void erase(pg_t pgid) { pg_temp.erase(pgid); }
    void clear() { pg_temp.clear(); }
    void set(pg_t pgid, const mempool::osdmap::vector<int32_t>& v) { pg_temp[pgid] = v; }
    const mempool::osdmap::vector<int32_t>& get(pg_t pgid) { return pg_temp.at(pgid); }
#endif
    void dump(ceph::Formatter* f) const
    {
        for (const auto& pg : *this) {
            ...
            for (const auto osd : pg.second) f->dump_int("osd", osd);
            f->close_section();
            f->close_section();
        }
    }
...
public:
    ...
    public:
        ...
        explicit Incremental(ceph::buffer::list::const_iterator& p) { decode(p); }

        pg_pool_t* get_new_pool(int64_t pool, const pg_pool_t* orig)
        {
            if (new_pools.count(pool) == 0) new_pools[pool] = *orig;
        ...
        void set_erasure_code_profile(const std::string& name, const std::map<std::string, std::string>& profile)
        {
        ...
        {
            if (pending_osd_has_state(osd, state)) return false;
            new_state[osd] |= state;
            return true;
        }
...
private:
    ...
    static constexpr uint64_t SIGNIFICANT_FEATURES =
        CEPH_FEATUREMASK_PGID64 | CEPH_FEATUREMASK_PGPOOL3 | CEPH_FEATUREMASK_OSDENC | CEPH_FEATUREMASK_OSDMAP_ENC |
        CEPH_FEATUREMASK_OSD_POOLRESEND | CEPH_FEATUREMASK_NEW_OSDOP_ENCODING | CEPH_FEATUREMASK_MSG_ADDR2 |
        CEPH_FEATUREMASK_CRUSH_TUNABLES5 | CEPH_FEATUREMASK_CRUSH_CHOOSE_ARGS | CEPH_FEATUREMASK_SERVER_LUMINOUS |
        CEPH_FEATUREMASK_SERVER_MIMIC | CEPH_FEATUREMASK_SERVER_NAUTILUS | CEPH_FEATUREMASK_SERVER_OCTOPUS |
    ...
    mempool::osdmap::map<pg_t, mempool::osdmap::vector<std::pair<int32_t, int32_t>>>
        pg_upmap_items;                                       ///< remap osds in up set
    ...
    class range_bits
    {
        ...
        bool ipv6;
        static void get_ipv6_bytes(unsigned const char* addr, uint64_t* upper, uint64_t* lower);
...
public:
    ...
    static uint64_t get_significant_features(uint64_t features) { return SIGNIFICANT_FEATURES & features; }
    ...
    uint32_t get_crush_version() const { return crush_version; }
    ...
    void get_blocklist(std::set<entity_addr_t>* bl, std::set<entity_addr_t>* rl) const;

    std::string get_cluster_snapshot() const
    {
        if (cluster_snapshot_epoch == epoch) return cluster_snapshot;
        return std::string();
    }

    float get_full_ratio() const { return full_ratio; }
    float get_backfillfull_ratio() const { return backfillfull_ratio; }
    float get_nearfull_ratio() const { return nearfull_ratio; }
    void get_full_pools(CephContext* cct, std::set<int64_t>* full, std::set<int64_t>* backfillfull,
    ...
    unsigned get_num_osds() const { return num_osd; }
    unsigned get_num_up_osds() const { return num_up_osd; }
    unsigned get_num_in_osds() const { return num_in_osd; }
    ...
    unsigned get_num_pg_temp() const { return pg_temp->size(); }
    ...
    void set_weight(int o, unsigned w)
    {
        ceph_assert(o < max_osd);
        osd_weight[o] = w;
        if (w) osd_state[o] |= CEPH_OSD_EXISTS;
    }
    ...
    float get_weightf(int o) const { return (float)get_weight(o) / (float)CEPH_OSD_IN; }
    ...
    {
        ceph_assert(o < max_osd);
        if (!osd_primary_affinity) return CEPH_OSD_DEFAULT_PRIMARY_AFFINITY;
    ...
    int get_erasure_code_profile_default(CephContext* cct, std::map<std::string, std::string>& profile_map,
                                         std::ostream* ss);
    void set_erasure_code_profile(const std::string& name, const std::map<std::string, std::string>& profile)
    {
        erasure_code_profiles[name] = profile;
    }
    const std::map<std::string, std::string>& get_erasure_code_profile(const std::string& name) const
    {
    ...
    bool get_allow_crimson() const { return allow_crimson; }
    ...
    bool is_destroyed(int osd) const { return exists(osd) && (osd_state[osd] & CEPH_OSD_DESTROYED); }

    bool is_up(int osd) const { return exists(osd) && (osd_state[osd] & CEPH_OSD_UP); }

    bool has_been_up_since(int osd, epoch_t epoch) const { return is_up(osd) && get_up_from(osd) <= epoch; }

    bool is_down(int osd) const { return !is_up(osd); }

    bool is_stop(int osd) const { return exists(osd) && is_down(osd) && (osd_state[osd] & CEPH_OSD_STOP); }

    bool is_out(int osd) const { return !exists(osd) || get_weight(osd) == CEPH_OSD_OUT; }

    bool is_in(int osd) const { return !is_out(osd); }
    ...
    bool is_noup_by_osd(int osd) const { return exists(osd) && (osd_state[osd] & CEPH_OSD_NOUP); }

    bool is_nodown_by_osd(int osd) const { return exists(osd) && (osd_state[osd] & CEPH_OSD_NODOWN); }

    bool is_noin_by_osd(int osd) const { return exists(osd) && (osd_state[osd] & CEPH_OSD_NOIN); }

    bool is_noout_by_osd(int osd) const { return exists(osd) && (osd_state[osd] & CEPH_OSD_NOOUT); }
    ...
    bool is_noup(int osd) const
    {
        ...
        if (get_osd_crush_node_flags(osd) & CEPH_OSD_NOUP)   // by crush-node?
            return true;
        if (auto class_id = crush->get_item_class_id(osd);
            class_id >= 0 && get_device_class_flags(class_id) & CEPH_OSD_NOUP)   // by device-class?
            return true;
        return false;
    }
    ...
    {
        if (test_flag(CEPH_OSDMAP_NODOWN)) return true;
        if (is_nodown_by_osd(osd)) return true;
        if (get_osd_crush_node_flags(osd) & CEPH_OSD_NODOWN) return true;
        if (auto class_id = crush->get_item_class_id(osd);
            class_id >= 0 && get_device_class_flags(class_id) & CEPH_OSD_NODOWN)
            return true;
        return false;
    }
    ...
    {
        if (test_flag(CEPH_OSDMAP_NOIN)) return true;
        if (is_noin_by_osd(osd)) return true;
        if (get_osd_crush_node_flags(osd) & CEPH_OSD_NOIN) return true;
        if (auto class_id = crush->get_item_class_id(osd);
            class_id >= 0 && get_device_class_flags(class_id) & CEPH_OSD_NOIN)
            return true;
        return false;
    }
    ...
    {
        if (test_flag(CEPH_OSDMAP_NOOUT)) return true;
        if (is_noout_by_osd(osd)) return true;
        if (get_osd_crush_node_flags(osd) & CEPH_OSD_NOOUT) return true;
        if (auto class_id = crush->get_item_class_id(osd);
            class_id >= 0 && get_device_class_flags(class_id) & CEPH_OSD_NOOUT)
            return true;
        return false;
    }
    ...
    bool subtree_type_is_down(CephContext* cct, int id, int subtree_type, std::set<int>* down_in_osds,
                              std::set<int>* up_in_osds, std::set<int>* subtree_up,
                              std::unordered_map<int, std::set<int>>* subtree_type_down) const;
    ...
    bool have_addr(const entity_addr_t& addr) const { return identify_osd(addr) >= 0; }
    int find_osd_on_ip(const entity_addr_t& ip) const;

    const entity_addrvec_t& get_addrs(int osd) const
    {
        ceph_assert(exists(osd));
        return osd_addrs->client_addrs[osd] ? *osd_addrs->client_addrs[osd] : _blank_addrvec;
    }
    const entity_addrvec_t& get_most_recent_addrs(int osd) const { return get_addrs(osd); }
    const entity_addrvec_t& get_cluster_addrs(int osd) const
    {
        ceph_assert(exists(osd));
        return osd_addrs->cluster_addrs[osd] ? *osd_addrs->cluster_addrs[osd] : _blank_addrvec;
    }
    ...
    {
        ceph_assert(exists(osd));
        return osd_addrs->hb_front_addrs[osd] ? *osd_addrs->hb_front_addrs[osd] : _blank_addrvec;
    }
    ...
    {
        if (get_max_osd() == 0) return -1;
        for (int i = n + 1; i != n; ++i) {
            if (i >= get_max_osd()) i = 0;
            if (i == n) break;
            if (is_up(i)) return i;
        }
        return -1;
    ...
    void get_random_up_osds_by_subtree(int n,   // whoami
                                       std::string& subtree,
                                       int limit,   // how many
                                       std::set<int> skip, std::set<int>* want) const;
    ...
    bool check_pg_upmaps(CephContext* cct, const std::vector<pg_t>& to_check, std::vector<pg_t>* to_cancel,
                         std::map<pg_t, mempool::osdmap::vector<std::pair<int, int>>>* to_remap) const;
    void clean_pg_upmaps(CephContext* cct, Incremental* pending_inc, const std::vector<pg_t>& to_cancel,
    ...
    static void clean_temps(CephContext* cct, const OSDMap& oldmap, const OSDMap& nextmap, Incremental* pending_inc);
...
public:
    ...
    /****   mapping facilities   ****/
    int map_to_pg(int64_t pool, const std::string& name, const std::string& key, const std::string& nspace,
                  pg_t* pg) const;
    ...
    ceph_object_layout make_object_layout(object_t oid, int pg_pool, std::string nspace) const;
...
private:
    /// pg -> (raw osd std::list)
    void _pg_to_raw_osds(const pg_pool_t& pool, pg_t pg, std::vector<int>* osds, ps_t* ppps) const;
    ...
    void _apply_primary_affinity(ps_t seed, const pg_pool_t& pool, std::vector<int>* osds, int* primary) const;

    /// apply pg_upmap[_items] mappings
    void _apply_upmap(const pg_pool_t& pi, pg_t pg, std::vector<int>* raw) const;

    /// pg -> (up osd std::list)
    void _raw_to_up_osds(const pg_pool_t& pool, const std::vector<int>& raw, std::vector<int>* up) const;
     ...
     */
    void _get_temp_osds(const pg_pool_t& pool, pg_t pg, std::vector<int>* temp_pg, int* temp_primary) const;

    /**
     *  map to up and acting. Fills in whatever fields are non-NULL.
     */
    void _pg_to_up_acting_osds(const pg_t& pg, std::vector<int>* up, int* up_primary, std::vector<int>* acting,
                               int* acting_primary, bool raw_pg_to_pg = true) const;
...
public:
    ...
    void pg_to_raw_upmap(pg_t pg, std::vector<int>* raw, std::vector<int>* raw_upmap) const;
    /// map a pg to its acting set. @return acting set size
    void pg_to_acting_osds(const pg_t& pg, std::vector<int>* acting, int* acting_primary) const
    {
        _pg_to_up_acting_osds(pg, NULL, NULL, acting, acting_primary);
    }
    void pg_to_acting_osds(pg_t pg, std::vector<int>& acting) const { return pg_to_acting_osds(pg, &acting, NULL); }
    /**
     ...
     */
    void pg_to_up_acting_osds(pg_t pg, std::vector<int>* up, int* up_primary, std::vector<int>* acting,
    ...
    const mempool::osdmap::map<int64_t, snap_interval_set_t>& get_removed_snaps_queue() const
    {
        return removed_snaps_queue;
    }
    const mempool::osdmap::map<int64_t, snap_interval_set_t>& get_new_removed_snaps() const
    {
        return new_removed_snaps;
    }
    const mempool::osdmap::map<int64_t, snap_interval_set_t>& get_new_purged_snaps() const { return new_purged_snaps; }

    int64_t lookup_pg_pool_name(std::string_view name) const
    {
        auto p = name_pool.find(name);
        if (p == name_pool.end()) return -ENOENT;
        return p->second;
    }

    int64_t get_pool_max() const { return pool_max; }
    const mempool::osdmap::map<int64_t, pg_pool_t>& get_pools() const { return pools; }
    mempool::osdmap::map<int64_t, pg_pool_t>& get_pools() { return pools; }
    ...
    void get_pool_ids_by_osd(CephContext* cct, int osd, std::set<int64_t>* pool_ids) const;
    ...
    const mempool::osdmap::map<int64_t, std::string>& get_pool_names() const { return pool_name; }
    bool have_pg_pool(int64_t p) const { return pools.count(p); }
    const pg_pool_t* get_pg_pool(int64_t p) const
    {
        auto i = pools.find(p);
        if (i != pools.end()) return &i->second;
        return NULL;
    }
    ...
    bool is_up_acting_osd_shard(spg_t pg, int osd) const
    {
        ...
        if (calc_pg_role(pg_shard_t(osd, pg.shard), acting) >= 0 || calc_pg_role(pg_shard_t(osd, pg.shard), up) >= 0) {
            return true;
        }
        return false;
    }
    ...
    static bool primary_changed_broken(int oldprimary, const std::vector<int>& oldacting, int newprimary,
    ...
    bool try_pg_upmap(CephContext* cct,
    ...
    int balance_primaries(CephContext* cct, int64_t pid, Incremental* pending_inc, OSDMap& tmp_osd_map) const;
    ...
    int calc_pg_upmaps(CephContext* cct,
    ...
    std::map<uint64_t, std::set<pg_t>> get_pgs_by_osd(
        CephContext* cct, int64_t pid, std::map<uint64_t, std::set<pg_t>>* p_primaries_by_osd = nullptr,
        std::map<uint64_t, std::set<pg_t>>* p_acting_primaries_by_osd =
            nullptr) const;   // used in calc_desired_primary_distribution()

private:   // Bunch of internal functions used only by calc_pg_upmaps (result of code refactoring)
    float get_osds_weight(CephContext* cct, const OSDMap& tmp_osd_map, int64_t pid,
                          std::map<int, float>& osds_weight) const;

    float build_pool_pgs_info(CephContext* cct,
                              const std::set<int64_t>& pools,   ///< [optional] restrict to pool
                              const OSDMap& tmp_osd_map, int& total_pgs, std::map<int, std::set<pg_t>>& pgs_by_osd,
                              std::map<int, float>& osds_weight);   // return total weight of all OSDs

    float calc_deviations(CephContext* cct, const std::map<int, std::set<pg_t>>& pgs_by_osd,
                          const std::map<int, float>& osd_weight, float pgs_per_weight,
                          std::map<int, float>& osd_deviation, std::multimap<float, int>& deviation_osd,
                          float& stddev);   // return current max deviation

    void fill_overfull_underfull(CephContext* cct, const std::multimap<float, int>& deviation_osd, int max_deviation,
                                 std::set<int>& overfull, std::set<int>& more_overfull, std::vector<int>& underfull,
                                 std::vector<int>& more_underfull);

    int pack_upmap_results(CephContext* cct, const std::set<pg_t>& to_unmap,
                           const std::map<pg_t, mempool::osdmap::vector<std::pair<int, int>>>& to_upmap,
                           OSDMap& tmp_osd_map, OSDMap::Incremental* pending_inc);

    std::default_random_engine get_random_engine(CephContext* cct, std::random_device::result_type* p_seed);

    bool try_drop_remap_overfull(CephContext* cct, const std::vector<pg_t>& pgs, const OSDMap& tmp_osd_map, int osd,
                                 std::map<int, std::set<pg_t>>& temp_pgs_by_osd, std::set<pg_t>& to_unmap,
                                 std::map<pg_t, mempool::osdmap::vector<std::pair<int32_t, int32_t>>>& to_upmap);

    typedef std::vector<std::pair<pg_t, mempool::osdmap::vector<std::pair<int, int>>>> candidates_t;

    bool try_drop_remap_underfull(CephContext* cct, const candidates_t& candidates, int osd,
                                  std::map<int, std::set<pg_t>>& temp_pgs_by_osd, std::set<pg_t>& to_unmap,
                                  std::map<pg_t, mempool::osdmap::vector<std::pair<int32_t, int32_t>>>& to_upmap);

    void add_remap_pair(CephContext* cct, int orig, int out, pg_t pg, size_t pg_pool_size, int osd,
                        std::set<int>& existing, std::map<int, std::set<pg_t>>& temp_pgs_by_osd,
                        mempool::osdmap::vector<std::pair<int32_t, int32_t>> new_upmap_items,
                        std::map<pg_t, mempool::osdmap::vector<std::pair<int32_t, int32_t>>>& to_upmap);

    int find_best_remap(CephContext* cct, const std::vector<int>& orig, const std::vector<int>& out,
                        const std::set<int>& existing, const std::map<int, float> osd_deviation);

    candidates_t build_candidates(CephContext* cct, const OSDMap& tmp_osd_map, const std::set<pg_t> to_skip,
                                  const std::set<int64_t>& only_pools, bool aggressive,
...
public:
    ...
    //
    int calc_read_balance_score(CephContext* cct, int64_t pool_id, read_balance_info_t* p_rb_info) const;

private:
    float rbi_round(float f) const { return (f > 0.0) ? floor(f * 100 + 0.5) / 100 : ceil(f * 100 - 0.5) / 100; }

    int64_t has_zero_pa_pgs(CephContext* cct, int64_t pool_id) const;

    void zero_rbi(read_balance_info_t& rbi) const;

    int set_rbi(CephContext* cct, read_balance_info_t& rbi, int64_t pool_id, float total_w_pa, float pa_sum,
                int num_osds, int osd_pa_count, float total_osd_weight, uint max_prims_per_osd,
                uint max_acting_prims_per_osd, float avg_prims_per_osd, bool prim_on_zero_pa, bool acting_on_zero_pa,
                float max_osd_score) const;

public:
    int get_osds_by_bucket_name(const std::string& name, std::set<int>* osds) const;

    bool have_pg_upmaps(pg_t pg) const { return pg_upmap.count(pg) || pg_upmap_items.count(pg); }

    bool check_full(const std::set<pg_shard_t>& missing_on) const
    {
        for (auto shard : missing_on) {
            if (get_state(shard.osd) & CEPH_OSD_FULL) return true;
        }
        return false;
    }
     ...
     */
private:
    int build_simple_optioned(CephContext* cct, epoch_t e, uuid_d& fsid, int num_osd, int pg_bits, int pgp_bits,
                              bool default_pool);

public:
    int build_simple(CephContext* cct, epoch_t e, uuid_d& fsid, int num_osd)
    {
        return build_simple_optioned(cct, e, fsid, num_osd, 0, 0, false);
    }
    int build_simple_with_pool(CephContext* cct, epoch_t e, uuid_d& fsid, int num_osd, int pg_bits, int pgp_bits)
    {
        return build_simple_optioned(cct, e, fsid, num_osd, pg_bits, pgp_bits, true);
    }
    static int _build_crush_types(CrushWrapper& crush);
    static int build_simple_crush_map(CephContext* cct, CrushWrapper& crush, int num_osd, std::ostream* ss);
    static int build_simple_crush_map_from_conf(CephContext* cct, CrushWrapper& crush, std::ostream* ss);
    static int build_simple_crush_rules(CephContext* cct, CrushWrapper& crush, const std::string& root,
                                        std::ostream* ss);
...
public:
    ...
    int summarize_mapping_stats(OSDMap* newmap, const std::set<int64_t>* pools, std::string* out,
    ...
    static void dump_erasure_code_profiles(
        const mempool::osdmap::map<std::string, std::map<std::string, std::string>>& profiles, ceph::Formatter* f);
    ...
    int parse_osd_id_list(const std::vector<std::string>& ls, std::set<int>* out, std::ostream* ss) const;
...
void print_osd_utilization(const OSDMap& osdmap, const PGMap& pgmap, std::ostream& out, ceph::Formatter* f, bool tree,
