...
public:
    ...
    void sub_write_committed(ceph_tid_t tid, eversion_t version, eversion_t last_complete, const ZTracer::Trace& trace);
    void handle_sub_write(pg_shard_t from, OpRequestRef msg, ECSubWrite& op, const ZTracer::Trace& trace);
    void handle_sub_read(pg_shard_t from, const ECSubRead& op, ECSubReadReply* reply, const ZTracer::Trace& trace);
    void handle_sub_write_reply(pg_shard_t from, const ECSubWriteReply& op, const ZTracer::Trace& trace);
    void handle_sub_read_reply(pg_shard_t from, ECSubReadReply& op, RecoveryMessages* m, const ZTracer::Trace& trace);
    ...
    void objects_read_and_reconstruct(
        const std::map<hobject_t, std::list<boost::tuple<uint64_t, uint64_t, uint32_t>>>& reads, bool fast_read,
    ...
    struct ClientAsyncReadStatus
    {
        ...
        explicit ClientAsyncReadStatus(unsigned objects_to_read,
            ...
            , func(std::move(func))
        {}
        void complete_object(const hobject_t& hoid, int err, extent_map&& buffers)
        {
        ...
        bool is_complete() const { return objects_to_read == 0; }
        void run() { func.release()->complete(std::move(results)); }
    };
    ...
    void objects_read_async_no_cache(const std::map<hobject_t, extent_set>& to_read, Func&& on_complete)
    {
        ...
        objects_read_and_reconstruct(_to_read,
                                     false,
                                     make_gen_lambda_context<std::map<hobject_t, std::pair<int, extent_map>>&&, Func>(
                                         std::forward<Func>(on_complete)));
    }
    void kick_reads()
    {
        while (in_progress_client_reads.size() && in_progress_client_reads.front().is_complete()) {
...
private:
    ...
    {
        return round_up_to(cct->_conf->osd_recovery_max_chunk, sinfo.get_stripe_width());
    }
    ...
    struct RecoveryOp
    {
        ...
        {
            switch (state) {
            case ECBackend::RecoveryOp::IDLE: return "IDLE";
    ...
    void handle_recovery_read_complete(
        const hobject_t& hoid, boost::tuple<uint64_t, uint64_t, std::map<pg_shard_t, ceph::buffer::list>>& to_read,
        std::optional<std::map<std::string, ceph::buffer::list, std::less<>>> attrs, RecoveryMessages* m);
    void handle_recovery_push(const PushOp& op, RecoveryMessages* m, bool is_repair);
    void handle_recovery_push_reply(const PushReplyOp& op, pg_shard_t from, RecoveryMessages* m);
    void get_all_avail_shards(const hobject_t& hoid, const std::set<pg_shard_t>& error_shards, std::set<int>& have,
                              std::map<shard_id_t, pg_shard_t>& shards, bool for_recovery);
...
public:
    ...
    struct read_result_t
    {
        ...
        std::list<boost::tuple<uint64_t, uint64_t, std::map<pg_shard_t, ceph::buffer::list>>> returned;
        read_result_t()
            : r(0)
        {}
    };
    struct read_request_t
    {
        ...
        read_request_t(const std::list<boost::tuple<uint64_t, uint64_t, uint32_t>>& to_read,
                       const std::map<pg_shard_t, std::vector<std::pair<int, int>>>& need, bool want_attrs,
                       GenContext<std::pair<RecoveryMessages*, read_result_t&>&>* cb)
            : to_read(to_read)
            , need(need)
    ...
    struct ReadOp
    {
        ...
        ReadOp(int priority, ceph_tid_t tid, bool do_redundant_reads, bool for_recovery, OpRequestRef op,
               std::map<hobject_t, std::set<int>>&& _want_to_read, std::map<hobject_t, read_request_t>&& _to_read)
            : priority(priority)
            , tid(tid)
            , op(op)
            ...
            , to_read(std::move(_to_read))
        {
            for (auto&& hpair : to_read) {
                ...
                for (auto&& extent : hpair.second.to_read) {
                    returned.push_back(boost::make_tuple(
                        extent.get<0>(), extent.get<1>(), std::map<pg_shard_t, ceph::buffer::list>()));
                }
            }
        }
        ReadOp() = delete;
    ...
    void start_read_op(int priority, std::map<hobject_t, std::set<int>>& want_to_read,
                       std::map<hobject_t, read_request_t>& to_read, OpRequestRef op, bool do_redundant_reads,
                       bool for_recovery);
    ...
    struct Op : boost::intrusive::list_base_hook<>
    {
        ...
        bool read_in_progress() const { return !remote_read.empty() && remote_read_result.empty(); }
        ...
        bool write_in_progress() const { return !pending_commit.empty() || !pending_apply.empty(); }
    ...
    public:
        bool caching_enabled() const { return pipeline_state == CACHE_VALID; }
        bool cache_invalid() const { return !caching_enabled(); }
        void invalidate() { pipeline_state = CACHE_INVALID; }
        void clear() { pipeline_state = CACHE_VALID; }
    ...
    public:
        explicit ECRecPred(ceph::ErasureCodeInterfaceRef ec_impl)
            : ec_impl(ec_impl)
        {
        ...
        {
            std::set<int> have;
            for (std::set<pg_shard_t>::const_iterator i = _have.begin(); i != _have.end(); ++i) {
                have.insert(i->shard);
            }
    ...
    IsPGRecoverablePredicate* get_is_recoverable_predicate() const override { return new ECRecPred(ec_impl); }

    int get_ec_data_chunk_count() const override { return ec_impl->get_data_chunk_count(); }
    int get_ec_stripe_chunk_size() const override { return sinfo.get_chunk_size(); }
    ...
    public:
        ECReadPred(pg_shard_t whoami, ceph::ErasureCodeInterfaceRef ec_impl)
            : whoami(whoami)
            , rec_pred(ec_impl)
        {}
        bool operator()(const std::set<pg_shard_t>& _have) const override
...
public:
    ECBackend(PGBackend::Listener* pg, const coll_t& coll, ObjectStore::CollectionHandle& ch, ObjectStore* store,
              CephContext* cct, ceph::ErasureCodeInterfaceRef ec_impl, uint64_t stripe_width);
    ...
    int get_min_avail_to_read_shards(
        ...
        std::map<pg_shard_t, std::vector<std::pair<int, int>>>*
            to_read   ///< [out] shards, corresponding subchunks to read
    );                ///< @return error code, 0 on success

    int get_remaining_shards(const hobject_t& hoid, const std::set<int>& avail, const std::set<int>& want,
                             const read_result_t& result,
                             std::map<pg_shard_t, std::vector<std::pair<int, int>>>* to_read, bool for_recovery);

    int objects_get_attrs(const hobject_t& hoid, std::map<std::string, ceph::buffer::list, std::less<>>* out) override;

    void rollback_append(const hobject_t& hoid, uint64_t old_size, ObjectStore::Transaction* t) override;
    ...
    void _failed_push(const hobject_t& hoid, std::pair<RecoveryMessages*, ECBackend::read_result_t&>& in);
};
