...
class Notify
{
    ...
    bool is_discarded() { return discarded || complete; }
    ...
    Notify(ConnectionRef client, uint64_t client_gid, ceph::buffer::list& payload, uint32_t timeout, uint64_t cookie,
...
public:
    ...
    void set_self(NotifyRef _self) { self = _self; }
    static NotifyRef makeNotifyRef(ConnectionRef client, uint64_t client_gid, ceph::buffer::list& payload,
                                   uint32_t timeout, uint64_t cookie, uint64_t notify_id, uint64_t version,
                                   OSDService* osd);
...
class Watch
{
    ...
    Watch(PrimaryLogPG* pg, OSDService* osd, std::shared_ptr<ObjectContext> obc, uint32_t timeout, uint64_t cookie,
...
public:
    ...
    bool is_connected() const { return conn.get() != NULL; }
    bool is_connected(Connection* con) const { return conn.get() == con; }

    /// NOTE: must be called with pg lock held
    ~Watch();

    uint64_t get_watcher_gid() const { return entity.num(); }

    std::ostream& gen_dbg_prefix(std::ostream& out);
    static WatchRef makeWatchRef(PrimaryLogPG* pg, OSDService* osd, std::shared_ptr<ObjectContext> obc,
                                 uint32_t timeout, uint64_t cookie, entity_name_t entity, const entity_addr_t& addr);
    void set_self(WatchRef _self) { self = _self; }
...
public:
    CephContext* cct;
    explicit WatchConState(CephContext* cct)
        : cct(cct)
    {}

    /// Add a watch
    void addWatch(WatchRef watch   ///< [in] Ref to new watch object
    );
