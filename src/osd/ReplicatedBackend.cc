...
public:
    PG_SendMessageOnConn(PGBackend::Listener* pg, Message* reply, ConnectionRef conn)
        : pg(pg)
        , reply(reply)
        , conn(conn)
    {}
    void finish(int) override { pg->send_message_osd_cluster(MessageRef(reply, false), conn.get()); }
};
...
public:
    PG_RecoveryQueueAsync(PGBackend::Listener* pg, GenContext<ThreadPool::TPHandle&>* c, uint64_t cost)
        : pg(pg)
        , c(c)
        , cost(cost)
    {}
    void finish(int) override { pg->schedule_recovery_work(c.release(), cost); }
};
}   // namespace
...
struct ReplicatedBackend::C_OSD_RepModifyCommit : public Context
{
    ...
    C_OSD_RepModifyCommit(ReplicatedBackend* pg, RepModifyRef r)
        ...
        , rm(r)
    {}
    void finish(int r) override { pg->repop_commit(rm); }
};

static void log_subop_stats(PerfCounters* logger, OpRequestRef op, int subop)
{
...
ReplicatedBackend::ReplicatedBackend(PGBackend::Listener* pg, const coll_t& coll, ObjectStore::CollectionHandle& c,
                                     ObjectStore* store, CephContext* cct)
...
int ReplicatedBackend::recover_object(const hobject_t& hoid, eversion_t v, ObjectContextRef head, ObjectContextRef obc,
                                      RecoveryHandle* _h)
{
...
{
    for (map<pg_shard_t, set<hobject_t>>::iterator i = pull_from_peer.begin(); i != pull_from_peer.end();) {
        if (osdmap->is_down(i->first.osd)) {
            dout(10) << "check_recovery_sources resetting pulls from osd." << i->first << ", osdmap has it marked down"
                     << dendl;
            for (set<hobject_t>::iterator j = i->second.begin(); j != i->second.end(); ++j) {
...
bool ReplicatedBackend::can_handle_while_inactive(OpRequestRef op)
{
    ...
    case MSG_OSD_PG_PULL: return true;
...
bool ReplicatedBackend::_handle_message(OpRequestRef op)
{
    dout(10) << __func__ << ": " << *op->get_req() << dendl;
    switch (op->get_req()->get_type()) {
    case MSG_OSD_PG_PUSH: do_push(op); return true;

    case MSG_OSD_PG_PULL: do_pull(op); return true;

    case MSG_OSD_PG_PUSH_REPLY: do_push_reply(op); return true;
...
int ReplicatedBackend::objects_read_sync(const hobject_t& hoid, uint64_t off, uint64_t len, uint32_t op_flags,
                                         bufferlist* bl)
{
    return store->read(ch, ghobject_t(hoid), off, len, *bl, op_flags);
}

int ReplicatedBackend::objects_readv_sync(const hobject_t& hoid, map<uint64_t, uint64_t>&& m, uint32_t op_flags,
                                          bufferlist* bl)
{
...
public:
    C_OSD_OnOpCommit(ReplicatedBackend* pg, ceph::ref_t<ReplicatedBackend::InProgressOp> op)
        ...
        , op(std::move(op))
    {}
    void finish(int) override { pg->op_commit(op); }
};

void generate_transaction(PGTransactionUPtr& pgt, const coll_t& coll, vector<pg_log_entry_t>& log_entries,
                          ObjectStore::Transaction* t, set<hobject_t>* added, set<hobject_t>* removed,
                          const ceph_release_t require_osd_release = ceph_release_t::unknown)
{
    ...
    pgt->safe_create_traverse([&](pair<const hobject_t, PGTransaction::ObjectOperation>& obj_op) {
        const hobject_t& oid = obj_op.first;
        const ghobject_t goid = ghobject_t(oid, ghobject_t::NO_GEN, shard_id_t::NO_SHARD);
        ...
        match(
            ...
            [&](const PGTransaction::ObjectOperation::Init::Clone& op) {
                t->clone(coll, ghobject_t(op.source, ghobject_t::NO_GEN, shard_id_t::NO_SHARD), goid);
            },
            [&](const PGTransaction::ObjectOperation::Init::Rename& op) {
                ceph_assert(op.source.is_temp());
                t->collection_move_rename(
                    coll, ghobject_t(op.source, ghobject_t::NO_GEN, shard_id_t::NO_SHARD), coll, goid);
            });

        if (op.truncate) {
            t->truncate(coll, goid, op.truncate->first);
            if (op.truncate->first != op.truncate->second) t->truncate(coll, goid, op.truncate->second);
        }
        ...
        if (op.clear_omap) t->omap_clear(coll, goid);
        if (op.omap_header) t->omap_setheader(coll, goid, *(op.omap_header));

        for (auto&& up : op.omap_updates) {
            using UpdateType = PGTransaction::ObjectOperation::OmapUpdateType;
            switch (up.first) {
            case UpdateType::Remove: t->omap_rmkeys(coll, goid, up.second); break;
            case UpdateType::Insert: t->omap_setkeys(coll, goid, up.second); break;
            case UpdateType::RemoveRange: t->omap_rmkeyrange(coll, goid, up.second); break;
            }
        }
        ...
        if (op.alloc_hint) {
            ...
            t->set_alloc_hint(coll, goid, hint.expected_object_size, hint.expected_write_size, hint.flags);
        }
        ...
        for (auto&& extent : op.buffer_updates) {
            ...
            match(
                ...
                [&](const BufferUpdate::Write& op) {
                    t->write(coll, goid, extent.get_off(), extent.get_len(), op.buffer, op.fadvise_flags);
                },
                [&](const BufferUpdate::Zero& op) { t->zero(coll, goid, extent.get_off(), extent.get_len()); },
...
void ReplicatedBackend::submit_transaction(const hobject_t& soid, const object_stat_sum_t& delta_stats,
                                           const eversion_t& at_version, PGTransactionUPtr&& _t,
                                           const eversion_t& trim_to, const eversion_t& min_last_complete_ondisk,
                                           ...
                                           ceph_tid_t tid, osd_reqid_t reqid, OpRequestRef orig_op)
{
    ...
    generate_transaction(t, coll, log_entries, &op_t, &added, &removed, get_osdmap()->require_osd_release);
    ceph_assert(added.size() <= 1);
    ceph_assert(removed.size() <= 1);

    auto insert_res =
        in_progress_ops.insert(make_pair(tid, ceph::make_ref<InProgressOp>(tid, on_all_commit, orig_op, at_version)));
    ...
    parent->log_operation(
        std::move(log_entries), hset_history, trim_to, at_version, min_last_complete_ondisk, true, op_t);
...
void ReplicatedBackend::do_repop_reply(OpRequestRef op)
{
    ...
    if (iter != in_progress_ops.end()) {
        ...
        if (ip_op.op) m = ip_op.op->get_req<MOSDOp>();
...
int ReplicatedBackend::be_deep_scrub(const hobject_t& poid, ScrubMap& map, ScrubMapBuilder& pos, ScrubMap::object& o)
{
    dout(10) << __func__ << " " << poid << " pos " << pos << dendl;
    int r;
    uint32_t fadvise_flags =
        CEPH_OSD_OP_FLAG_FADVISE_SEQUENTIAL | CEPH_OSD_OP_FLAG_FADVISE_DONTNEED | CEPH_OSD_OP_FLAG_BYPASS_CLEAN_CACHE;
    ...
    if (!pos.data_done()) {
        ...
        if (static_cast<uint64_t>(r) == stride) {
            dout(20) << __func__ << "  " << poid << " more data, digest so far 0x" << std::hex << pos.data_hash.digest()
                     << std::dec << dendl;
            return -EINPROGRESS;
        }
        // done with bytes
        ...
        dout(20) << __func__ << "  " << poid << " done with data, digest 0x" << std::hex << o.digest << std::dec
                 << dendl;
    }
    ...
    if (pos.omap_pos.empty()) {
        ...
        r = store->omap_get_header(
            ch, ghobject_t(poid, ghobject_t::NO_GEN, get_parent()->whoami_shard().shard), &hdrbl, true);
        if (r == -EIO) {
    ...
    ObjectMap::ObjectMapIterator iter =
        store->get_omap_iterator(ch, ghobject_t(poid, ghobject_t::NO_GEN, get_parent()->whoami_shard().shard));
    ceph_assert(iter);
    ...
    if (pos.omap_keys > cct->_conf->osd_deep_scrub_large_omap_object_key_threshold ||
        ...
        dout(25) << __func__ << " " << poid << " large omap object detected. Object has " << pos.omap_keys
...
void ReplicatedBackend::_do_push(OpRequestRef op)
{
    ...
    for (vector<PushOp>::const_iterator i = m->pushes.begin(); i != m->pushes.end(); ++i) {
    ...
    t.register_on_complete(new PG_SendMessageOnConn(get_parent(), reply, m->get_connection()));
    ...
    void finish(ThreadPool::TPHandle& handle) override
    {
        ...
        for (auto&& i : to_continue) {
            ...
            if (started < 0) {
                ...
                bc->get_parent()->on_failed_pull({bc->get_parent()->whoami_shard()}, i.hoid, obc->obs.oi.version);
            }
            else if (!started) {
                bc->get_parent()->on_global_recover(i.hoid, i.stat, false);
            }
...
void ReplicatedBackend::_do_pull_response(OpRequestRef op)
{
    ...
    for (vector<PushOp>::const_iterator i = m->pushes.begin(); i != m->pushes.end(); ++i) {
        ...
        if (more) replies.push_back(PullOp());
    }
    if (!to_continue.empty()) {
        C_ReplicatedBackend_OnPullComplete* c =
            new C_ReplicatedBackend_OnPullComplete(this, m->get_priority(), std::move(to_continue));
        t.register_on_complete(new PG_RecoveryQueueAsync(
            get_parent(), get_parent()->bless_unlocked_gencontext(c), std::max<uint64_t>(1, c->estimate_push_costs())));
    }
...
void ReplicatedBackend::do_push_reply(OpRequestRef op)
{
    ...
    for (vector<PushReplyOp>::const_iterator i = m->replies.begin(); i != m->replies.end(); ++i) {
        bool more = handle_push_reply(from, *i, &(replies.back()));
        if (more) replies.push_back(PushOp());
    }
    replies.erase(replies.end() - 1);
...
Message* ReplicatedBackend::generate_subop(const hobject_t& soid, const eversion_t& at_version, ceph_tid_t tid,
                                           osd_reqid_t reqid, eversion_t pg_trim_to,
                                           eversion_t min_last_complete_ondisk, hobject_t new_temp_oid,
                                           hobject_t discard_temp_oid, const bufferlist& log_entries,
                                           std::optional<pg_hit_set_history_t>& hset_hist,
                                           ObjectStore::Transaction& op_t, pg_shard_t peer, const pg_info_t& pinfo)
{
...
void ReplicatedBackend::issue_op(const hobject_t& soid, const eversion_t& at_version, ceph_tid_t tid, osd_reqid_t reqid,
                                 eversion_t pg_trim_to, eversion_t min_last_complete_ondisk, hobject_t new_temp_oid,
                                 hobject_t discard_temp_oid, const vector<pg_log_entry_t>& log_entries,
                                 std::optional<pg_hit_set_history_t>& hset_hist, InProgressOp* op,
                                 ObjectStore::Transaction& op_t)
{
    if (parent->get_acting_recovery_backfill_shards().size() > 1) {
        ...
        for (const auto& shard : get_parent()->get_acting_recovery_backfill_shards()) {
            ...
            wr = generate_subop(soid,
                                ...
                                shard,
                                pinfo);
            if (op->op && op->op->pg_trace) wr->trace.init("replicated op", nullptr, &op->op->pg_trace);
...
void ReplicatedBackend::do_repop(OpRequestRef op)
{
    ...
    dout(10) << __func__ << " " << soid << " v " << m->version
             << (m->logbl.length() ? " (transaction)" : " (parallel exec") << " " << m->logbl.length() << dendl;
    ...
    if (m->discard_temp_oid != hobject_t()) {
        ...
        if (rm->opt.empty()) {
            dout(10) << __func__ << ": removing object " << m->discard_temp_oid << " since we won't get the transaction"
                     << dendl;
...
void ReplicatedBackend::repop_commit(RepModifyRef rm)
{
    ...
    MOSDRepOpReply* reply = new MOSDRepOpReply(
        m, get_parent()->whoami_shard(), 0, get_osdmap_epoch(), m->get_min_epoch(), CEPH_OSD_FLAG_ONDISK);
...
void ReplicatedBackend::calc_head_subsets(ObjectContextRef obc, SnapSet& snapset, const hobject_t& head,
                                          const pg_missing_t& missing, const hobject_t& last_backfill,
                                          ...
                                          ObcLockManager& manager)
{
    ...
    if (size) data_subset.insert(0, size);
    ...
    hobject_t c = head;
    if (size) prev.insert(0, size);

    for (int j = snapset.clones.size() - 1; j >= 0; j--) {
        c.snap = snapset.clones[j];
        prev.intersection_of(snapset.clone_overlap[snapset.clones[j]]);
        if (!missing.is_missing(c) && c < last_backfill && get_parent()->try_lock_for_read(c, manager)) {
    ...
    dout(10) << "calc_head_subsets " << head << "  data_subset " << data_subset << "  clone_subsets " << clone_subsets
             << dendl;
}

void ReplicatedBackend::calc_clone_subsets(SnapSet& snapset, const hobject_t& soid, const pg_missing_t& missing,
                                           const hobject_t& last_backfill, interval_set<uint64_t>& data_subset,
                                           map<hobject_t, interval_set<uint64_t>>& clone_subsets,
                                           ObcLockManager& manager)
{
    ...
    for (i = 0; i < snapset.clones.size(); i++)
        if (snapset.clones[i] == soid.snap) break;
    ...
    dout(10) << "calc_clone_subsets " << soid << "  data_subset " << data_subset << "  clone_subsets " << clone_subsets
             << dendl;
}

void ReplicatedBackend::prepare_pull(eversion_t v, const hobject_t& soid, ObjectContextRef headctx, RPGHandle* h)
{
    ...
    ceph_assert(_v == v);
    const map<hobject_t, set<pg_shard_t>>& missing_loc(get_parent()->get_missing_loc_shards());
    ...
    if (p == q->second.end()) {
        ...
        p = q->second.begin();
        std::advance(p, ceph::util::generate_random_number<int>(0, q->second.size() - 1));
    }
    ...
    if (pmissing.is_missing(soid, v)) {
        ...
        dout(10) << "pulling soid " << soid << " from osd " << fromshard << " at version "
                 << pmissing.get_items().find(soid)->second.have << " rather than at version " << v << dendl;
    ...
    if (soid.is_snap()) {
        ...
        calc_clone_subsets(ssc->snapset,
                           soid,
 ...
 */
int ReplicatedBackend::prep_push_to_replica(ObjectContextRef obc, const hobject_t& soid, pg_shard_t peer, PushOp* pop,
                                            bool cache_dont_need)
{
    ...
    if (soid.snap && soid.snap < CEPH_NOSNAP) {
        ...
        calc_clone_subsets(
            ssc->snapset, soid, pm->second, pi->second.last_backfill, data_subset, clone_subsets, lock_manager);
    }
        ...
        ceph_assert(ssc);
        dout(15) << "push_to_replica snapset is " << ssc->snapset << dendl;
        calc_head_subsets(obc,
                          ssc->snapset,
                          soid,
                          get_parent()->get_shard_missing().find(peer)->second,
    ...
    return prep_push(
        obc, soid, peer, oi.version, data_subset, clone_subsets, pop, cache_dont_need, std::move(lock_manager));
}

int ReplicatedBackend::prep_push(ObjectContextRef obc, const hobject_t& soid, pg_shard_t peer, PushOp* pop,
                                 bool cache_dont_need)
{
    interval_set<uint64_t> data_subset;
    if (obc->obs.oi.size) data_subset.insert(0, obc->obs.oi.size);
    map<hobject_t, interval_set<uint64_t>> clone_subsets;

    return prep_push(
        obc, soid, peer, obc->obs.oi.version, data_subset, clone_subsets, pop, cache_dont_need, ObcLockManager());
}

int ReplicatedBackend::prep_push(ObjectContextRef obc, const hobject_t& soid, pg_shard_t peer, eversion_t version,
                                 interval_set<uint64_t>& data_subset,
                                 map<hobject_t, interval_set<uint64_t>>& clone_subsets, PushOp* pop,
                                 bool cache_dont_need, ObcLockManager&& lock_manager)
{
    ...
    int r = build_push_op(pi.recovery_info, pi.recovery_progress, &new_progress, pop, &(pi.stat), cache_dont_need);
    if (r < 0) return r;
    pi.recovery_progress = new_progress;
    return 0;
}

void ReplicatedBackend::submit_push_data(const ObjectRecoveryInfo& recovery_info, bool first, bool complete,
                                         bool clear_omap, bool cache_dont_need, interval_set<uint64_t>& data_zeros,
                                         const interval_set<uint64_t>& intervals_included, bufferlist data_included,
                                         bufferlist omap_header, const map<string, bufferlist, less<>>& attrs,
                                         const map<string, bufferlist>& omap_entries, ObjectStore::Transaction* t)
{
    ...
    if (first && complete) {
        ...
        target_oid = get_parent()->get_temp_recovery_object(recovery_info.soid, recovery_info.version);
        if (first) {
    ...
    if (first) {
        if (!complete) {
            ...
            t->set_alloc_hint(
                coll, ghobject_t(target_oid), oi.expected_object_size, oi.expected_write_size, oi.alloc_hint_flags);
        }
        else {
            ...
            if (clear_omap) t->omap_clear(coll, ghobject_t(target_oid));
        }

        t->truncate(coll, ghobject_t(target_oid), recovery_info.size);
        if (omap_header.length()) t->omap_setheader(coll, ghobject_t(target_oid), omap_header);

        struct stat st;
        int r = store->stat(ch, ghobject_t(recovery_info.soid), &st);
        if (get_parent()->pg_is_remote_backfilling()) {
            uint64_t size = 0;
            if (r == 0) size = st.st_size;
            ...
            if (size != recovery_info.oi.size) {
                ...
                dout(10) << __func__ << " " << recovery_info.soid << " backfill size " << recovery_info.oi.size
                         << " previous size " << size << " net size " << recovery_info.oi.size - size << dendl;
            }
        }
        if (!complete) {
            ...
            if (recovery_info.object_exist) {
                ...
                for (interval_set<uint64_t>::const_iterator q = local_intervals_included.begin();
                     ...
                     ++q) {
                    dout(15) << " clone_range " << recovery_info.soid << " " << q.get_start() << "~" << q.get_len()
                             << dendl;
                    t->clone_range(coll,
    ...
    if (cache_dont_need) fadvise_flags |= CEPH_OSD_OP_FLAG_FADVISE_DONTNEED;
    ...
    if (data_zeros.size() > 0) {
        ...
        dout(20) << __func__ << " recovering object " << recovery_info.soid
                 << " copy_subset: " << recovery_info.copy_subset << " intervals_included: " << intervals_included
    ...
    for (interval_set<uint64_t>::const_iterator p = intervals_included.begin(); p != intervals_included.end(); ++p) {
        bufferlist bit;
        bit.substr_of(data_included, off, p.get_len());
        t->write(coll, ghobject_t(target_oid), p.get_start(), p.get_len(), bit, fadvise_flags);
        off += p.get_len();
    }

    if (!omap_entries.empty()) t->omap_setkeys(coll, ghobject_t(target_oid), omap_entries);
    if (!attrs.empty()) t->setattrs(coll, ghobject_t(target_oid), attrs);
    ...
    if (complete) {
        if (!first) {
            ...
            t->collection_move_rename(coll, ghobject_t(target_oid), coll, ghobject_t(recovery_info.soid));
        }
...
void ReplicatedBackend::submit_push_complete(const ObjectRecoveryInfo& recovery_info, ObjectStore::Transaction* t)
{
    for (map<hobject_t, interval_set<uint64_t>>::const_iterator p = recovery_info.clone_subset.begin();
         p != recovery_info.clone_subset.end();
         ++p) {
        for (interval_set<uint64_t>::const_iterator q = p->second.begin(); q != p->second.end(); ++q) {
            dout(15) << " clone_range " << p->first << " " << q.get_start() << "~" << q.get_len() << dendl;
            t->clone_range(
                coll, ghobject_t(p->first), ghobject_t(recovery_info.soid), q.get_start(), q.get_len(), q.get_start());
        }
    }
}

ObjectRecoveryInfo ReplicatedBackend::recalc_subsets(const ObjectRecoveryInfo& recovery_info, SnapSetContext* ssc,
                                                     ObcLockManager& manager)
{
    if (!recovery_info.soid.snap || recovery_info.soid.snap >= CEPH_NOSNAP) return recovery_info;
    ...
    calc_clone_subsets(ssc->snapset,
                       ...
                       new_info.clone_subset,
                       manager);
    return new_info;
}

bool ReplicatedBackend::handle_pull_response(pg_shard_t from, const PushOp& pop, PullOp* response,
                                             list<pull_complete_info>* to_continue, ObjectStore::Transaction* t)
{
    interval_set<uint64_t> data_included = pop.data_included;
    bufferlist data;
    data = pop.data;
    dout(10) << "handle_pull_response " << pop.recovery_info << pop.after_progress << " data.size() is "
             << data.length() << " data_included: " << data_included << dendl;
    ...
    ceph_assert((data_included.empty() && data.length() == 0) || (!data_included.empty() && data.length() > 0));
    ...
    trim_pushed_data(pi.recovery_info.copy_subset, data_included, data, &usable_intervals, &usable_data);
    ...
    if (z_length) data_zeros.insert(z_offset, z_length);
...
void ReplicatedBackend::handle_push(pg_shard_t from, const PushOp& pop, PushReplyOp* response,
...
int ReplicatedBackend::build_push_op(const ObjectRecoveryInfo& recovery_info, const ObjectRecoveryProgress& progress,
                                     ObjectRecoveryProgress* out_progress, PushOp* out_op, object_stat_sum_t* stat,
                                     bool cache_dont_need)
{
    ObjectRecoveryProgress _new_progress;
    if (!out_progress) out_progress = &_new_progress;
    ObjectRecoveryProgress& new_progress = *out_progress;
    new_progress = progress;

    dout(7) << __func__ << " " << recovery_info.soid << " v " << recovery_info.version << " size " << recovery_info.size
            << " recovery_info: " << recovery_info << dendl;
    ...
    if (!progress.omap_complete) {
        ObjectMap::ObjectMapIterator iter = store->get_omap_iterator(ch, ghobject_t(recovery_info.soid));
        ceph_assert(iter);
        for (iter->lower_bound(progress.omap_recovered_to); iter->valid(); iter->next()) {
    ...
    if (available > 0) {
        if (!recovery_info.copy_subset.empty()) {
            ...
            int r = store->fiemap(ch, ghobject_t(recovery_info.soid), 0, copy_subset.range_end(), m);
            if (r >= 0) {
            ...
            if (out_op->data_included.empty() || out_op->data_included.range_end() == copy_subset.range_end())
    ...
    bufferlist bit;
    int r = store->readv(ch,
                         ghobject_t(recovery_info.soid),
                         out_op->data_included,
                         bit,
    ...
    if (out_op->data_included.size() != origin_size) {
        dout(10) << __func__ << " some extents get pruned " << out_op->data_included.size() << "/" << origin_size
                 << dendl;
        new_progress.data_complete = true;
    }
    out_op->data.claim_append(bit);
    if (progress.first && !out_op->data_included.empty() && out_op->data_included.begin().get_start() == 0 &&
        out_op->data.length() == oi.size && oi.is_data_digest()) {
        uint32_t crc = out_op->data.crc32c(-1);
        if (oi.data_digest != crc) {
            dout(0) << __func__ << " " << coll << std::hex << " full-object read crc 0x" << crc << " != expected 0x"
                    << oi.data_digest << std::dec << " on " << recovery_info.soid << dendl;
            return -EIO;
        }
    }
    ...
    if (new_progress.is_complete(recovery_info)) {
        ...
        if (stat) {
            ...
            if (get_parent()->pg_is_repair()) stat->num_objects_repaired++;
        }
    }
...
bool ReplicatedBackend::handle_push_reply(pg_shard_t peer, const PushReplyOp& op, PushOp* reply)
{
    ...
    if (pushing.count(soid) == 0) {
        ...
        if (!pi->recovery_progress.data_complete && !error) {
            dout(10) << " pushing more from, " << pi->recovery_progress.data_recovered_to << " of "
                     << pi->recovery_info.copy_subset << dendl;
            ObjectRecoveryProgress new_progress;
            int r = build_push_op(pi->recovery_info, pi->recovery_progress, &new_progress, reply, &(pi->stat));
            ...
            if (!error) get_parent()->on_peer_recover(peer, soid, pi->recovery_info);
            ...
            if (pushing[soid].empty()) {
                if (!error)
                    ...
                    get_parent()->on_failed_pull(std::set<pg_shard_t>{get_parent()->whoami_shard()}, soid, v);
                pushing.erase(soid);
            }
            else {
                // This looks weird, but we erased the current peer and need to remember
                // the error on any other one, while getting more acks.
                if (error) pushing[soid].begin()->second.recovery_progress.error = true;
                dout(10) << "pushed " << soid << ", still waiting for push ack from " << pushing[soid].size()
                         << " others" << dendl;
            }
            return false;
        }
...
void ReplicatedBackend::handle_pull(pg_shard_t peer, PullOp& op, PushOp* reply)
{
    ...
    if (r != 0) {
        get_parent()->clog_error() << get_info().pgid << " " << peer << " tried to pull " << soid << " but got "
                                   << cpp_strerror(-r);
        ...
        if (r < 0) prep_push_op_blank(soid, reply);
    }
}
 ...
 */
void ReplicatedBackend::trim_pushed_data(const interval_set<uint64_t>& copy_subset,
                                         const interval_set<uint64_t>& intervals_received, bufferlist data_received,
                                         interval_set<uint64_t>* intervals_usable, bufferlist* data_usable)
{
    ...
    uint64_t off = 0;
    for (interval_set<uint64_t>::const_iterator p = intervals_received.begin(); p != intervals_received.end(); ++p) {
        interval_set<uint64_t> x;
        x.insert(p.get_start(), p.get_len());
        x.intersection_of(copy_subset);
        for (interval_set<uint64_t>::const_iterator q = x.begin(); q != x.end(); ++q) {
            bufferlist sub;
...
void ReplicatedBackend::_failed_pull(pg_shard_t from, const hobject_t& soid)
{
    ...
    get_parent()->on_failed_pull({from}, soid, it->second.recovery_info.version);

    clear_pull(it);
}

void ReplicatedBackend::clear_pull_from(map<hobject_t, PullInfo>::iterator piter)
{
    auto from = piter->second.from;
    pull_from_peer[from].erase(piter->second.soid);
    if (pull_from_peer[from].empty()) pull_from_peer.erase(from);
}

void ReplicatedBackend::clear_pull(map<hobject_t, PullInfo>::iterator piter, bool clear_pull_from_peer)
{
...
int ReplicatedBackend::start_pushes(const hobject_t& soid, ObjectContextRef obc, RPGHandle* h)
{
    ...
    for (set<pg_shard_t>::iterator i = get_parent()->get_acting_recovery_backfill_shards().begin();
    ...
    for (auto j : shards) {
        ...
        int r = prep_push_to_replica(obc, soid, peer, &(h->pushes[peer].back()), cache);
        if (r < 0) {
