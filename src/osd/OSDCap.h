...
struct OSDCapSpec
{
    ...
    bool allow_all() const { return allow == OSD_CAP_ANY; }
};
...
struct OSDCapPoolNamespace
{
    ...
    OSDCapPoolNamespace() {}
    OSDCapPoolNamespace(const std::string& pool_name, const boost::optional<std::string>& nspace = boost::none)
        : pool_name(pool_name)
        , nspace(nspace)
    {}
...
struct OSDCapPoolTag
{
    ...
    OSDCapPoolTag() {}
    OSDCapPoolTag(const std::string& application, const std::string& key, const std::string& value)
...
BOOST_FUSION_ADAPT_STRUCT(OSDCapPoolTag, (std::string, application)(std::string, key)(std::string, value))
...
struct OSDCapMatch
{
    ...
    OSDCapMatch(const std::string& pl, const std::string& pre)
        ...
        , object_prefix(pre)
    {}
    OSDCapMatch(const std::string& pl, const std::string& ns, const std::string& pre)
        : pool_namespace(pl, ns)
        , object_prefix(pre)
    {}
    OSDCapMatch(const std::string& dummy, const std::string& app, const std::string& key, const std::string& val)
     ...
     */
    bool is_match(const std::string& pool_name, const std::string& nspace_name, const app_map_t& app_map,
...
struct OSDCapProfile
{
    ...
    OSDCapProfile() {}
    OSDCapProfile(const std::string& name, const std::string& pool_name,
                  const boost::optional<std::string>& nspace = boost::none)
        : name(name)
        , pool_namespace(pool_name, nspace)
    {}
...
struct OSDCapGrant
{
    ...
    OSDCapGrant() {}
    OSDCapGrant(const OSDCapMatch& m, const OSDCapSpec& s, boost::optional<std::string> n = {})
        : match(m)
        , spec(s)
    {
        if (n) {
    ...
    explicit OSDCapGrant(const OSDCapProfile& profile, boost::optional<std::string> n = {})
        : profile(profile)
    {
        if (n) {
            set_network(*n);
    ...
    bool is_capable(const std::string& pool_name, const std::string& ns,
                    const OSDCapPoolTag::app_map_t& application_metadata, const std::string& object, bool op_may_read,
                    bool op_may_write, const std::vector<OpInfo::ClassInfo>& classes, const entity_addr_t& addr,
...
struct OSDCap
{
    ...
    bool is_capable(const std::string& pool_name, const std::string& ns,
                    const OSDCapPoolTag::app_map_t& application_metadata, const std::string& object, bool op_may_read,
                    bool op_may_write, const std::vector<OpInfo::ClassInfo>& classes, const entity_addr_t& addr) const;
};
