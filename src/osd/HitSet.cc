    ...
    : sealed(false)
{
    ...
    default: assert(0 == "unknown HitSet type");
    }
}
...
void HitSet::decode(ceph::buffer::list::const_iterator& bl)
{
    ...
    case TYPE_EXPLICIT_HASH: impl.reset(new ExplicitHashHitSet); break;
    case TYPE_EXPLICIT_OBJECT: impl.reset(new ExplicitObjectHitSet); break;
    case TYPE_BLOOM: impl.reset(new BloomHitSet); break;
    case TYPE_NONE: impl.reset(NULL); break;
    default: throw ceph::buffer::malformed_input("unrecognized HitMap type");
    }
    if (impl) impl->decode(bl);
    DECODE_FINISH(bl);
}
...
{
    switch ((impl_type_t)type) {
    case TYPE_EXPLICIT_HASH: impl.reset(new ExplicitHashHitSet::Params); break;
    case TYPE_EXPLICIT_OBJECT: impl.reset(new ExplicitObjectHitSet::Params); break;
    case TYPE_BLOOM: impl.reset(new BloomHitSet::Params); break;
    case TYPE_NONE: impl.reset(NULL); break;
...
void HitSet::Params::decode(ceph::buffer::list::const_iterator& bl)
{
    ...
    __u8 type;
    decode(type, bl);
    if (!create_impl((impl_type_t)type)) throw ceph::buffer::malformed_input("unrecognized HitMap type");
    if (impl) impl->decode(bl);
    DECODE_FINISH(bl);
}
...
#define loop_hitset_params(kind)                                                                                    \
    {                                                                                                               \
        ...
        for (list<kind::Params*>::iterator i = params.begin(); i != params.end(); ++i) o.push_back(new Params(*i)); \
    }
