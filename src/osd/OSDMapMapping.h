...
public:
    struct Job
    {
        ...
        Job(const OSDMap* om)
            ...
            , osdmap(om)
        {}
        virtual ~Job() { ceph_assert(shards == 0); }
        ...
        utime_t get_duration() { return finish - start; }
        void wait()
        {
...
protected:
    ...
    struct Item
    {
        ...
        Item(Job* j, std::vector<pg_t> pgs)
            : job(j)
            , pgs(pgs)
        {}
    ...
    struct WQ : public ThreadPool::WorkQueue<Item>
    {
        ...
        void _dequeue(Item* i) override { ceph_abort(); }
        ...
        void _clear() override { ceph_assert(_empty()); }

        bool _empty() override { return m->q.empty(); }
    } wq;
...
public:
    ...
    void queue(Job* job, unsigned pgs_per_item, const std::vector<pg_t>& input_pgs);
...
private:
    struct PoolMapping
    {
        ...
        void get(size_t ps, std::vector<int>* up, int* up_primary, std::vector<int>* acting, int* acting_primary) const
        {
        ...
        void set(size_t ps, const std::vector<int>& up, int up_primary, const std::vector<int>& acting,
                 int acting_primary)
        {
    ...
    mempool::osdmap_mapping::vector<mempool::osdmap_mapping::vector<pg_t>> acting_rmap;   // osd -> pg
    // unused: mempool::osdmap_mapping::vector<std::vector<pg_t>> up_rmap;  // osd -> pg
    epoch_t epoch = 0;
    uint64_t num_pgs = 0;

    void _init_mappings(const OSDMap& osdmap);
    void _update_range(const OSDMap& map, int64_t pool, unsigned pg_begin, unsigned pg_end);

    void _build_rmap(const OSDMap& osdmap);

    void _start(const OSDMap& osdmap) { _init_mappings(osdmap); }
    ...
    struct MappingJob : public ParallelPGMapper::Job
    {
        ...
        void complete() override { mapping->_finish(*osdmap); }
    };
...
public:
    void get(pg_t pgid, std::vector<int>* up, int* up_primary, std::vector<int>* acting, int* acting_primary) const
    {
    ...
    std::unique_ptr<MappingJob> start_update(const OSDMap& map, ParallelPGMapper& mapper, unsigned pgs_per_item)
    {
    ...
    uint64_t get_num_pgs() const { return num_pgs; }
};
