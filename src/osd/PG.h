...
public:
    ...
    void dump(ostream& out)
    {
        ...
        for (std::map<const char*, per_state_info>::iterator p = info.begin(); p != info.end(); ++p) {
            ...
            out << i.enter << "\t" << i.exit << "\t" << i.events << "\t" << i.event_time << "\t" << i.total_time << "\t"
                << i.min_time << "\t" << i.max_time << "\t" << p->first << "\n";
        }
    }
    ...
    void dump_formatted(ceph::Formatter* f)
    {
        ...
        for (std::map<const char*, per_state_info>::iterator p = info.begin(); p != info.end(); ++p) {
            ...
            for (std::vector<std::string>::iterator st = states.begin(); st != states.end(); ++st) {
    ...
    void log_exit(const char* s, utime_t dur, uint64_t events, utime_t event_dur)
    {
        ...
        i.exit++;
        i.total_time += dur;
        if (dur > i.max_time) i.max_time = dur;
        if (dur < i.min_time || i.min_time == utime_t()) i.min_time = dur;
        i.events += events;
...
public:
    ...
    const requested_scrub_t& get_planned_scrub() const { return m_planned_scrub; }
...
public:
    ...
    const char* const get_current_state() const { return recovery_state.get_current_state(); }
    ...
    epoch_t get_osdmap_epoch() const override final { return recovery_state.get_osdmap()->get_epoch(); }
    ...
    void log_state_exit(const char* state_name, utime_t enter_time, uint64_t events, utime_t event_dur) override;
    ...
    const PGPool& get_pgpool() const final { return pool; }
    uint64_t get_last_user_version() const { return info.last_user_version; }
    const pg_history_t& get_history() const { return info.history; }
    bool get_need_up_thru() const { return recovery_state.get_need_up_thru(); }
    epoch_t get_same_interval_since() const { return info.history.same_interval_since; }

    bool is_waiting_for_unreadable_object() const final { return !waiting_for_unreadable_object.empty(); }

    static void set_last_scrub_stamp(utime_t t, pg_history_t& history, pg_stat_t& stats)
    {
    ...
    {
        recovery_state.update_stats([t](auto& history, auto& stats) {
            set_last_scrub_stamp(t, history, stats);
            return true;
        });
    }

    static void set_last_deep_scrub_stamp(utime_t t, pg_history_t& history, pg_stat_t& stats)
    {
    ...
    static void add_objects_scrubbed_count(int64_t count, pg_stat_t& stats) { stats.objects_scrubbed += count; }

    void add_objects_scrubbed_count(int64_t count)
    {
        recovery_state.update_stats([count](auto& history, auto& stats) {
            add_objects_scrubbed_count(count, stats);
            return true;
        });
    }

    static void reset_objects_scrubbed(pg_stat_t& stats) { stats.objects_scrubbed = 0; }
    ...
    bool is_deleting() const { return recovery_state.is_deleting(); }
    bool is_deleted() const { return recovery_state.is_deleted(); }
    bool is_nonprimary() const { return recovery_state.is_nonprimary(); }
    bool is_primary() const { return recovery_state.is_primary(); }
    ...
    bool is_ec_pg() const { return recovery_state.is_ec_pg(); }
    int get_role() const { return recovery_state.get_role(); }
    const std::vector<int> get_acting() const { return recovery_state.get_acting(); }
    const std::set<pg_shard_t>& get_actingset() const { return recovery_state.get_actingset(); }
    int get_acting_primary() const { return recovery_state.get_acting_primary(); }
    pg_shard_t get_primary() const final { return recovery_state.get_primary(); }
    const std::vector<int> get_up() const { return recovery_state.get_up(); }
    int get_up_primary() const { return recovery_state.get_up_primary(); }
    const PastIntervals& get_past_intervals() const { return recovery_state.get_past_intervals(); }
    bool is_acting_recovery_backfill(pg_shard_t osd) const { return recovery_state.is_acting_recovery_backfill(osd); }
    const std::set<pg_shard_t>& get_acting_recovery_backfill() const
    {
        return recovery_state.get_acting_recovery_backfill();
    }
    bool is_acting(pg_shard_t osd) const { return recovery_state.is_acting(osd); }
    bool is_up(pg_shard_t osd) const { return recovery_state.is_up(osd); }
    ...
    void init(int role, const std::vector<int>& up, int up_primary, const std::vector<int>& acting, int acting_primary,
              const pg_history_t& history, const PastIntervals& pim, ObjectStore::Transaction& t);

    /// read existing pg state off disk
    void read_state(ObjectStore* store);
    static int peek_map_epoch(ObjectStore* store, spg_t pgid, epoch_t* pepoch);

    static int get_latest_struct_v() { return pg_latest_struct_v; }
    static int get_compat_struct_v() { return pg_compat_struct_v; }
    static int read_info(ObjectStore* store, spg_t pgid, const coll_t& coll, pg_info_t& info,
                         PastIntervals& past_intervals, __u8&);
    static bool _has_removal_flag(ObjectStore* store, spg_t pgid);

    void rm_backoff(const ceph::ref_t<Backoff>& b);

    void update_snap_mapper_bits(uint32_t bits) { snap_mapper.update_bits(bits); }
    void start_split_stats(const std::set<spg_t>& childpgs, std::vector<object_stat_sum_t>* v);
    virtual void split_colls(spg_t child, int split_bits, int seed, const pg_pool_t* pool,
                             ObjectStore::Transaction& t) = 0;
    void split_into(pg_t child_pgid, PG* child, unsigned split_bits);
    void merge_from(std::map<spg_t, PGRef>& sources, PeeringCtx& rctx, unsigned split_bits,
    ...
    {
        // a new scrub
        forward_scrub_event(&ScrubPgIF::initiate_scrub_after_repair, queued, "AfterRepairScrub");
    }

    void replica_scrub(epoch_t queued, Scrub::act_token_t act_token, ThreadPool::TPHandle& handle);

    void replica_scrub_resched(epoch_t queued, Scrub::act_token_t act_token, ThreadPool::TPHandle& handle)
    {
        forward_scrub_event(&ScrubPgIF::send_sched_replica, queued, act_token, "SchedReplica");
    }
    ...
    {
        forward_scrub_event(&ScrubPgIF::send_reservation_failure, queued, "ReservationFailure");
    }
    ...
    {
        forward_scrub_event(&ScrubPgIF::active_pushes_notification, queued, "ActivePushesUpd");
    }

    void scrub_send_applied_update(epoch_t queued, ThreadPool::TPHandle& handle)
    {
        forward_scrub_event(&ScrubPgIF::update_applied_notification, queued, "UpdatesApplied");
    }
    ...
    {
        forward_scrub_event(&ScrubPgIF::send_replica_pushes_upd, queued, "ReplicaPushesUpd");
    }
    ...
    uint64_t get_snap_trimq_size() const override { return snap_trimq.size(); }

    static void add_objects_trimmed_count(int64_t count, pg_stat_t& stats) { stats.objects_trimmed += count; }

    void add_objects_trimmed_count(int64_t count)
    {
        recovery_state.update_stats_wo_resched(
            [count](auto& history, auto& stats) { add_objects_trimmed_count(count, stats); });
    }

    static void reset_objects_trimmed(pg_stat_t& stats) { stats.objects_trimmed = 0; }

    void reset_objects_trimmed()
    {
        recovery_state.update_stats_wo_resched([](auto& history, auto& stats) { reset_objects_trimmed(stats); });
    }

    utime_t snaptrim_begin_stamp;

    void set_snaptrim_begin_stamp() { snaptrim_begin_stamp = ceph_clock_now(); }
    ...
    void set_snaptrim_duration()
    {
        ...
        recovery_state.update_stats_wo_resched(
            [duration](auto& history, auto& stats) { stats.snaptrim_duration = double(duration); });
    }
    ...
    void schedule_event_after(PGPeeringEventRef event, float delay) override;
    void request_local_background_io_reservation(unsigned priority, PGPeeringEventURef on_grant,
                                                 PGPeeringEventURef on_preempt) override;
    void update_local_background_io_priority(unsigned priority) override;
    void cancel_local_background_io_reservation() override;

    void request_remote_recovery_reservation(unsigned priority, PGPeeringEventURef on_grant,
                                             PGPeeringEventURef on_preempt) override;
    void cancel_remote_recovery_reservation() override;

    void schedule_event_on_commit(ObjectStore::Transaction& t, PGPeeringEventRef on_commit) override;
    ...
    bool is_forced_recovery_or_backfill() const { return recovery_state.is_forced_recovery_or_backfill(); }

    PGLog::LogEntryHandlerRef get_log_handler(ObjectStore::Transaction& t) override
    {
        return std::make_unique<PG::PGLogEntryHandler>(this, &t);
    }

    std::pair<ghobject_t, bool> do_delete_work(ObjectStore::Transaction& t, ghobject_t _next) override;
    ...
    void handle_advance_map(OSDMapRef osdmap, OSDMapRef lastmap, std::vector<int>& newup, int up_primary,
                            std::vector<int>& newacting, int acting_primary, PeeringCtx& rctx);
     ...
     */
    virtual bool start_recovery_ops(uint64_t max, ThreadPool::TPHandle& handle, uint64_t* ops_begun) = 0;
...
private:
    ...
    bool is_time_for_deep(bool allow_deep_scrub, bool allow_shallow_scrub, bool has_deep_errors,
    ...
    std::optional<requested_scrub_t> validate_periodic_mode(bool allow_deep_scrub, bool try_to_auto_repair,
                                                            bool allow_shallow_scrub, bool time_for_deep,
                                                            bool has_deep_errors,
                                                            const requested_scrub_t& planned) const;

    std::optional<requested_scrub_t> validate_initiated_scrub(bool allow_deep_scrub, bool try_to_auto_repair,
    ...
    using ScrubSafeAPI = void (ScrubPgIF::*)(epoch_t epoch_queued, Scrub::act_token_t act_token);
    void forward_scrub_event(ScrubSafeAPI fn, epoch_t epoch_queued, Scrub::act_token_t act_token,
...
public:
    ...
    virtual void do_command(const std::string_view& prefix, const cmdmap_t& cmdmap, const ceph::buffer::list& idata,
    ...
    struct C_DeleteMore : public Context
    {
        PGRef pg;
        epoch_t epoch;
        C_DeleteMore(PG* p, epoch_t e)
            : pg(p)
            , epoch(e)
        {}
        void finish(int r) override { ceph_abort(); }
        void complete(int r) override;
    };

    virtual void set_dynamic_perf_stats_queries(const std::list<OSDPerfMetricQuery>& queries) {}
    ...
    // ctor
    PG(OSDService* o, OSDMapRef curmap, const PGPool& pool, spg_t p);
    ~PG() override;
...
private:
    friend void intrusive_ptr_add_ref(PG* pg) { pg->get("intptr"); }
    friend void intrusive_ptr_release(PG* pg) { pg->put("intptr"); }
...
public:
    ...
    void send_cluster_message(int osd, MessageRef m, epoch_t epoch, bool share_map_update) override;

protected:
    epoch_t get_last_peering_reset() const { return recovery_state.get_last_peering_reset(); }

    /* heartbeat peers */
    void set_probe_targets(const std::set<pg_shard_t>& probe_set) override;
    void clear_probe_targets() override;

    ceph::mutex heartbeat_peer_lock = ceph::make_mutex("PG::heartbeat_peer_lock");
...
public:
    ...
    bool is_remote_backfilling() { return primary_num_bytes.load() > 0; }
    ...
    void add_num_bytes(int64_t num_bytes)
    {
        ...
        if (num_bytes) {
            recovery_state.update_stats([num_bytes](auto& history, auto& stats) {
    ...
    int64_t get_stats_num_bytes()
    {
        ...
        if (lnb && lnb != num_bytes) {
            lgeneric_dout(cct, 0) << this << " " << info.pgid << " num_bytes mismatch " << lnb << " vs stats "
                                  << info.stats.stats.sum.num_bytes << " / chunk "
                                  << get_pgbackend()->get_ec_data_chunk_count() << dendl;
        }
        return num_bytes;
    }
...
protected:
    ...
    std::map<hobject_t, std::list<OpRequestRef>> waiting_for_unreadable_object, waiting_for_degraded_object,
        waiting_for_blocked_object;
    ...
    std::map<eversion_t, std::list<std::tuple<OpRequestRef, version_t, int, std::vector<pg_log_op_return_item_t>>>>
        waiting_for_ondisk;
    ...
    // publish stats
    ceph::mutex pg_stats_publish_lock = ceph::make_mutex("PG::pg_stats_publish_lock");
    std::optional<pg_stat_t> pg_stats_publish;

    friend class TestOpsSocketHook;
    void publish_stats_to_osd() override;

    bool needs_recovery() const { return recovery_state.needs_recovery(); }
    bool needs_backfill() const { return recovery_state.needs_backfill(); }
    ...
    struct PGLogEntryHandler : public PGLog::LogEntryHandler
    {
        ...
        PGLogEntryHandler(PG* pg, ObjectStore::Transaction* t)
            : pg(pg)
            , t(t)
        {}

        // LogEntryHandler
        void remove(const hobject_t& hoid) override { pg->get_pgbackend()->remove(hoid, t); }
        void try_stash(const hobject_t& hoid, version_t v) override { pg->get_pgbackend()->try_stash(hoid, v, t); }
        void rollback(const pg_log_entry_t& entry) override
        {
            ceph_assert(entry.can_rollback());
            pg->get_pgbackend()->rollback(entry, t);
        }
        void rollforward(const pg_log_entry_t& entry) override { pg->get_pgbackend()->rollforward(entry, t); }
        void trim(const pg_log_entry_t& entry) override { pg->get_pgbackend()->trim(entry, t); }
    };

    void update_object_snap_mapping(ObjectStore::Transaction* t, const hobject_t& soid,
                                    const std::set<snapid_t>& snaps);
    void clear_object_snap_mapping(ObjectStore::Transaction* t, const hobject_t& soid);
    void remove_snap_mapped_object(ObjectStore::Transaction& t, const hobject_t& soid);

    bool have_unfound() const { return recovery_state.have_unfound(); }
    uint64_t get_num_unfound() const { return recovery_state.get_num_unfound(); }
    ...
    {
        PGRef pg;
        explicit C_PG_FinishRecovery(PG* p)
            : pg(p)
        {}
        void finish(int r) override { pg->_finish_recovery(this); }
    };
    ...
    void release_backoffs(const hobject_t& o) { release_backoffs(o, o); }
    void clear_backoffs();
...
protected:
     ...
     */
    virtual bool _range_available_for_scrub(const hobject_t& begin, const hobject_t& end) = 0;
    ...
    struct QueuePeeringEvt : Context
    {
        ...
        QueuePeeringEvt(PG* pg, epoch_t epoch, EVT evt)
            : pg(pg)
            , evt(std::make_shared<PGPeeringEvent>(epoch, epoch, evt))
        {}
...
protected:
    ...
    bool is_complete() const { return recovery_state.is_complete(); }
    bool should_send_notify() const { return recovery_state.should_send_notify(); }
...
public:
    void prepare_write(pg_info_t& info, pg_info_t& last_written_info, PastIntervals& past_intervals, PGLog& pglog,
                       bool dirty_info, bool dirty_big_info, bool need_write_epoch,
                       ObjectStore::Transaction& t) override;

    void write_if_dirty(PeeringCtx& rctx) { write_if_dirty(rctx.transaction); }

protected:
    void write_if_dirty(ObjectStore::Transaction& t) { recovery_state.write_if_dirty(t); }

    PGLog::IndexedLog projected_log;
    bool check_in_progress_op(const osd_reqid_t& r, eversion_t* version, version_t* user_version, int* return_code,
                              std::vector<pg_log_op_return_item_t>* op_returns) const;
    eversion_t projected_last_update;
    eversion_t get_next_version() const
    {
        eversion_t at_version(get_osdmap_epoch(), projected_last_update.version + 1);
    ...
    void update_snap_map(const std::vector<pg_log_entry_t>& log_entries, ObjectStore::Transaction& t);
    ...
    void update_history(const pg_history_t& history) { recovery_state.update_history(history); }
    ...
    template<typename T, int MSGTYPE> bool can_discard_replica_op(OpRequestRef& op);
    ...
    bool have_same_or_newer_map(epoch_t e) { return e <= get_osdmap_epoch(); }
...
public:
    ...
    requested_scrub_t& get_planned_scrub(ScrubberPasskey) { return m_planned_scrub; }

    void force_object_missing(ScrubberPasskey, const std::set<pg_shard_t>& peer, const hobject_t& oid,
