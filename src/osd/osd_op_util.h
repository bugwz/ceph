...
public:
    struct ClassInfo
    {
        ClassInfo(std::string&& class_name, std::string&& method_name, bool read, bool write, bool allowed)
...
private:
    ...
    void add_class(std::string&& class_name, std::string&& method_name, bool read, bool write, bool allowed)
    {
        classes.emplace_back(std::move(class_name), std::move(method_name), read, write, allowed);
    }

public:
    void clear() { rmw_flags = 0; }

    uint64_t get_flags() const { return rmw_flags; }
    ...
    int set_from_op(const std::vector<OSDOp>& ops, const pg_t& pg, const OSDMap& osdmap);

    std::vector<ClassInfo> get_classes() const { return classes; }
};
