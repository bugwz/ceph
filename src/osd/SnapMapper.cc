...
int OSDriver::get_keys(const std::set<std::string>& keys, std::map<std::string, bufferlist>* out)
{
    return os->omap_get_values(ch, hoid, keys, out);
}

int OSDriver::get_next(const std::string& key, pair<std::string, bufferlist>* next)
{
...
string SnapMapper::get_prefix(int64_t pool, snapid_t snap)
{
    char buf[100];
    int len =
        snprintf(buf, sizeof(buf), "%lld_%.*X_", (long long)pool, (int)(sizeof(snap) * 2), static_cast<unsigned>(snap));
    return MAPPING_PREFIX + string(buf, len);
}

string SnapMapper::to_raw_key(const pair<snapid_t, hobject_t>& in)
{
    return get_prefix(in.second.pool, in.first) + shard_prefix + in.second.to_str();
}

pair<string, bufferlist> SnapMapper::to_raw(const pair<snapid_t, hobject_t>& in)
{
    bufferlist bl;
...
pair<snapid_t, hobject_t> SnapMapper::from_raw(const pair<std::string, bufferlist>& image)
{
    using ceph::decode;
    Mapping map;
...
int SnapMapper::get_snaps(const hobject_t& oid, object_snaps* out)
{
...
void SnapMapper::clear_snaps(const hobject_t& oid, MapCacher::Transaction<std::string, bufferlist>* t)
{
...
int SnapMapper::remove_oid(const hobject_t& oid, MapCacher::Transaction<std::string, bufferlist>* t)
{
    dout(20) << __func__ << " " << oid << dendl;
    ceph_assert(check(oid));
...
int SnapMapper::_remove_oid(const hobject_t& oid, MapCacher::Transaction<std::string, bufferlist>* t)
{
    dout(20) << __func__ << " " << oid << dendl;
    object_snaps out;
    int r = get_snaps(oid, &out);
...
{
    char k[80];
    snprintf(k, sizeof(k), "%s_%llu_%016llx", PURGED_SNAP_PREFIX, (unsigned long long)pool, (unsigned long long)last);
    return k;
}

void SnapMapper::make_purged_snap_key_value(int64_t pool, snapid_t begin, snapid_t end, map<string, bufferlist>* m)
{
...
int SnapMapper::_lookup_purged_snap(CephContext* cct, ObjectStore* store, ObjectStore::CollectionHandle& ch,
                                    const ghobject_t& hoid, int64_t pool, snapid_t snap, snapid_t* begin, snapid_t* end)
{
    ...
    if (it->key().find(PURGED_SNAP_PREFIX) != 0) {
        dout(20) << __func__ << " pool " << pool << " snap " << snap << " key '" << k
                 << "' lower_bound got mismatched prefix '" << it->key() << "'" << dendl;
        return -ENOENT;
    }
...
void SnapMapper::record_purged_snaps(CephContext* cct, ObjectStore* store, ObjectStore::CollectionHandle& ch,
                                     ghobject_t hoid, ObjectStore::Transaction* t,
    ...
    for (auto& [epoch, bypool] : purged_snaps) {
        ...
        for (auto& [pool, snaps] : bypool) {
            for (auto i = snaps.begin(); i != snaps.end(); ++i) {
                ...
                int b = _lookup_purged_snap(cct, store, ch, hoid, pool, begin - 1, &before_begin, &before_end);
                int a = _lookup_purged_snap(cct, store, ch, hoid, pool, end, &after_begin, &after_end);
                if (!b && !a) {
...
{
    char buf[100];
    int len = snprintf(buf, sizeof(buf), "%.*X_", (int)(sizeof(snap) * 2), static_cast<unsigned>(snap));
 ...
 */
std::string SnapMapper::convert_legacy_key(const std::string& old_key, const bufferlist& value)
{
