...
const char* osd_error_category::message(int ev, char* buf, std::size_t len) const noexcept
{
    if (ev == 0) return "No error";

    switch (static_cast<osd_errc>(ev)) {
    case osd_errc::old_snapc: return "ORDERSNAP flag set; writer has old snapc";
    case osd_errc::blocklisted: return "Blocklisted";
    }
...
{
    if (ev == static_cast<int>(osd_errc::old_snapc) || ev == static_cast<int>(osd_errc::blocklisted))
        return {ev, *this};
    else
...
{
    switch (static_cast<osd_errc>(ev)) {
    case osd_errc::old_snapc: return c == boost::system::errc::invalid_argument;
    case osd_errc::blocklisted: return c == boost::system::errc::operation_not_permitted;
    }
