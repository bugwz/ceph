...
void OpInfo::set_returnvec()
{
    set_rmw_flags(CEPH_OSD_RMW_FLAG_RETURNVEC);
}
...
int OpInfo::set_from_op(const std::vector<OSDOp>& ops, const pg_t& pg, const OSDMap& osdmap)
{
    ...
    for (iter = ops.begin(); iter != ops.end(); ++iter) {
        if ((iter->op.op == CEPH_OSD_OP_WATCH && iter->op.watch.op == CEPH_OSD_WATCH_OP_PING)) {
            ...
            if (ceph_osd_op_mode_modify(iter->op.op)) set_write();
        }
        if (ceph_osd_op_mode_read(iter->op.op)) set_read();

        // set READ flag if there are src_oids
        if (iter->soid.oid.name.length()) set_read();

        // set PGOP flag if there are PG ops
        if (ceph_osd_op_type_pg(iter->op.op)) set_pg_op();

        if (ceph_osd_op_mode_cache(iter->op.op)) set_cache();
        ...
        if (pool && pool->is_tier()) {
            ...
            if (base_pool && base_pool->require_rollback()) {
                if ((iter->op.op != CEPH_OSD_OP_READ) && (iter->op.op != CEPH_OSD_OP_CHECKSUM) &&
                    (iter->op.op != CEPH_OSD_OP_CMPEXT) && (iter->op.op != CEPH_OSD_OP_STAT) &&
                    (iter->op.op != CEPH_OSD_OP_ISDIRTY) && (iter->op.op != CEPH_OSD_OP_UNDIRTY) &&
                    (iter->op.op != CEPH_OSD_OP_GETXATTR) && (iter->op.op != CEPH_OSD_OP_GETXATTRS) &&
                    (iter->op.op != CEPH_OSD_OP_CMPXATTR) && (iter->op.op != CEPH_OSD_OP_ASSERT_VER) &&
                    (iter->op.op != CEPH_OSD_OP_LIST_WATCHERS) && (iter->op.op != CEPH_OSD_OP_LIST_SNAPS) &&
                    (iter->op.op != CEPH_OSD_OP_SETALLOCHINT) && (iter->op.op != CEPH_OSD_OP_WRITEFULL) &&
                    (iter->op.op != CEPH_OSD_OP_ROLLBACK) && (iter->op.op != CEPH_OSD_OP_CREATE) &&
                    (iter->op.op != CEPH_OSD_OP_DELETE) && (iter->op.op != CEPH_OSD_OP_SETXATTR) &&
                    (iter->op.op != CEPH_OSD_OP_RMXATTR) && (iter->op.op != CEPH_OSD_OP_STARTSYNC) &&
                    (iter->op.op != CEPH_OSD_OP_COPY_GET) && (iter->op.op != CEPH_OSD_OP_COPY_FROM) &&
        ...
        case CEPH_OSD_OP_CALL:
        {
            ...
            if (is_read) set_class_read();
            if (is_write) set_class_write();
            if (is_promote) set_promote();
            add_class(std::move(cname), std::move(mname), is_read, is_write, cls->allowed);
            break;
        }
        ...
        case CEPH_OSD_OP_DELETE:
            ...
            if (iter == ops.begin() && iter->op.flags == CEPH_OSD_OP_FLAG_FAILOK) {
        ...
        case CEPH_OSD_OP_WRITEFULL:
            if (ops.size() == 1 && (iter->op.flags & CEPH_OSD_OP_FLAG_FADVISE_NOCACHE ||
        ...
        case CEPH_OSD_OP_CACHE_PIN: set_promote(); break;

        default: break;
        }
    }

    if (rmw_flags == 0) return -EINVAL;
...
{
    out << "class " << i.class_name << " method " << i.method_name << " rd " << i.read << " wr " << i.write
        << " allowed " << i.allowed;
    return out;
}
