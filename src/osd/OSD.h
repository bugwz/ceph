...
public:
    ...
    void maybe_inject_dispatch_delay()
    {
        if (g_conf()->osd_debug_inject_dispatch_delay_probability > 0) {
            if (rand() % 10000 < g_conf()->osd_debug_inject_dispatch_delay_probability * 10000) {
                utime_t t;
...
public:
    ...
    void maybe_share_map(Connection* con, const OSDMapRef& osdmap, epoch_t peer_epoch_lb = 0);
    ...
    MOSDMap* build_incremental_map_msg(epoch_t from, epoch_t to, OSDSuperblock& superblock);
    ...
    void send_message_osd_cluster(MessageRef m, Connection* con) { con->send_message2(std::move(m)); }
    void send_message_osd_cluster(Message* m, const ConnectionRef& con) { con->send_message(m); }
    void send_message_osd_client(Message* m, const ConnectionRef& con) { con->send_message(m); }
...
public:
     ...
     */
    Scrub::schedule_result_t initiate_a_scrub(spg_t pgid, bool allow_requested_repair_only) final;
...
private:
    ...
    {
        OSDService* osd;
        explicit AgentThread(OSDService* o)
            : osd(o)
        {}
        void* entry() override
        {
...
public:
    ...
    void _enqueue(PG* pg, uint64_t priority)
    {
        ...
        if (nq.empty()) agent_cond.notify_all();
        nq.insert(pg);
    }
    ...
    void _dequeue(PG* pg, uint64_t old_priority)
    {
        ...
        if (p == agent_queue_pos) ++agent_queue_pos;
        oq.erase(p);
        if (oq.empty()) {
            if (agent_queue.rbegin()->first == old_priority) agent_valid_iterator = false;
...
public:
    bool promote_throttle()
    {
        ...
        if ((unsigned)rand() % 1000 > promote_probability_millis) return true;   // yes throttle (no promote)
        if (promote_max_objects && promote_counter.objects > promote_max_objects) return true;   // yes throttle
        if (promote_max_bytes && promote_counter.bytes > promote_max_bytes) return true;         // yes throttle
        return false;   //  no throttle (promote)
    }
    void promote_finish(uint64_t bytes) { promote_counter.finish(bytes); }
    void promote_throttle_recalibrate();
    unsigned get_num_shards() const { return m_objecter_finishers; }
    Finisher* get_objecter_finisher(int shard) { return objecter_finishers[shard].get(); }
    ...
    ceph_tid_t get_tid() { return (ceph_tid_t)last_tid++; }
    ...
    void set_ready_to_merge_source(PG* pg, eversion_t version);
    void set_ready_to_merge_target(PG* pg, eversion_t version, epoch_t last_epoch_started, epoch_t last_epoch_clean);
...
public:
    void queue_want_pg_temp(pg_t pgid, const std::vector<int>& want, bool forced = false);
    ...
    void queue_recovery_context(PG* pg, GenContext<ThreadPool::TPHandle&>* c, uint64_t cost, int priority);
...
private:
    ...
    template<class MSG_TYPE> void queue_scrub_event_msg(PG* pg, Scrub::scrub_prio_t with_priority);
...
public:
    ...
    void clear_queued_recovery(PG* pg)
    {
        ...
        awaiting_throttle.remove_if(
            [pg](decltype(awaiting_throttle)::const_reference awaiting) { return awaiting.pg.get() == pg; });
    }

    unsigned get_target_pg_log_entries() const;

    // delayed pg activation
    void queue_for_recovery(PG* pg, uint64_t cost_per_object, int priority)
    {
        std::lock_guard l(recovery_lock);

        if (pg->is_forced_recovery_or_backfill()) {
            awaiting_throttle.emplace_front(
                pg_awaiting_throttle_t{pg->get_osdmap()->get_epoch(), pg, cost_per_object, priority});
        }
        else {
    ...
    void queue_recovery_after_sleep(PG* pg, epoch_t queued, uint64_t reserved_pushes, int priority)
    {
        std::lock_guard l(recovery_lock);
        ...
        _queue_for_recovery(pg_awaiting_throttle_t{queued, pg, 1, priority}, reserved_pushes);
    }

    void queue_check_readable(spg_t spgid, epoch_t lpr, ceph::signedspan delay = ceph::signedspan::zero());
    ...
    void identify_splits_and_merges(OSDMapRef old_map, OSDMapRef new_map, spg_t pgid,
    ...
    void set_statfs(const struct store_statfs_t& stbuf, osd_alert_list_t& alerts);
...
public:
     ...
     */
    void retrieve_epochs(epoch_t* _boot_epoch, epoch_t* _up_epoch, epoch_t* _bind_epoch) const;
    /**
     * Std::set the boot, up, and bind epochs. Any NULL params will not be std::set.
     */
    void set_epochs(const epoch_t* _boot_epoch, const epoch_t* _up_epoch, const epoch_t* _bind_epoch);
...
struct OSDShard
{
    ...
    {
        bool operator()(const OSDShardPGSlot& l, const OSDShardPGSlot& r) const { return l.epoch < r.epoch; }
    };

    /// maintain an ordering of pg slots by pg epoch
    boost::intrusive::multiset<OSDShardPGSlot,
                               boost::intrusive::member_hook<OSDShardPGSlot, boost::intrusive::set_member_hook<>,
                                                             &OSDShardPGSlot::pg_epoch_item>,
                               boost::intrusive::compare<pg_slot_compare_by_epoch>>
        pg_slots_by_epoch;
    ...
    void consume_map(const OSDMapRef& osdmap, unsigned* pushes_to_free);

    int _wake_pg_slot(spg_t pgid, OSDShardPGSlot* slot);

    void identify_splits_and_merges(const OSDMapRef& as_of_osdmap, std::set<std::pair<spg_t, epoch_t>>* split_children,
                                    std::set<std::pair<spg_t, epoch_t>>* merge_pgs);
    void _prime_splits(std::set<std::pair<spg_t, epoch_t>>* pgids);
    void prime_splits(const OSDMapRef& as_of_osdmap, std::set<std::pair<spg_t, epoch_t>>* pgids);
    void prime_merges(const OSDMapRef& as_of_osdmap, std::set<std::pair<spg_t, epoch_t>>* merge_pgs);
...
protected:
    ...
    PGRefOrError locate_asok_target(const cmdmap_t& cmdmap, std::stringstream& ss, bool only_primary);
    int asok_route_to_pg(bool only_primary, std::string_view prefix, cmdmap_t cmdmap, Formatter* f,
                         std::stringstream& ss, const bufferlist& inbl, bufferlist& outbl,
                         std::function<void(int, const std::string&, bufferlist&)> on_finish);
    void asok_command(std::string_view prefix, const cmdmap_t& cmdmap, ceph::Formatter* f,
...
public:
    ...
    static ghobject_t make_snapmapper_oid() { return ghobject_t(hobject_t(sobject_t(object_t("snapmapper"), 0))); }
    static ghobject_t make_purged_snaps_oid() { return ghobject_t(hobject_t(sobject_t(object_t("purged_snaps"), 0))); }
    ...
    {
        return ghobject_t(hobject_t(sobject_t(object_t(std::string("final_pool_") + stringify(pool)), CEPH_NOSNAP)));
    }
    ...
    static void recursive_remove_collection(CephContext* cct, ObjectStore* store, spg_t pgid, coll_t tmp);
...
public:
    int get_state() const { return state; }
    void set_state(int s) { state = s; }
    bool is_initializing() const { return state == STATE_INITIALIZING; }
    bool is_preboot() const { return state == STATE_PREBOOT; }
    bool is_booting() const { return state == STATE_BOOTING; }
    bool is_active() const { return state == STATE_ACTIVE; }
    bool is_stopping() const { return state == STATE_STOPPING; }
    bool is_waiting_for_healthy() const { return state == STATE_WAITING_FOR_HEALTHY; }
...
private:
    ...
    void dispatch_sessions_waiting_on_map()
    {
        ...
        for (auto i = sessions_to_check.begin(); i != sessions_to_check.end(); sessions_to_check.erase(i++)) {
...
private:
    ...
    bool heartbeat_peers_need_update() { return heartbeat_need_update.load(); }
    void heartbeat_set_peers_need_update() { heartbeat_need_update.store(true); }
    void heartbeat_clear_peers_need_update() { heartbeat_need_update.store(false); }
    void heartbeat();
...
public:
    ...
    {
        OSD* osd;
        explicit HeartbeatDispatcher(OSD* o)
            : Dispatcher(o->cct)
            , osd(o)
        {}
        ...
        bool ms_can_fast_dispatch(const Message* m) const override
        {
            switch (m->get_type()) {
            case CEPH_MSG_PING:
            case MSG_OSD_PING: return true;
            default: return false;
            }
        }
        void ms_fast_dispatch(Message* m) override { osd->heartbeat_dispatch(m); }
        bool ms_dispatch(Message* m) override { return osd->heartbeat_dispatch(m); }
        bool ms_handle_reset(Connection* con) override { return osd->heartbeat_reset(con); }
        void ms_handle_remote_reset(Connection* con) override {}
        bool ms_handle_refused(Connection* con) override { return osd->ms_handle_refused(con); }
        int ms_handle_fast_authentication(Connection* con) override { return true; }
...
protected:
    ...
    class ShardedOpWQ : public ShardedThreadPool::ShardedWQ<OpSchedulerItem>
    {
        OSD* osd;
        bool m_fast_shutdown = false;

    public:
        ShardedOpWQ(OSD* o, ceph::timespan ti, ceph::timespan si, ShardedThreadPool* tp)
            : ShardedThreadPool::ShardedWQ<OpSchedulerItem>(ti, si, tp)
            , osd(o)
        {}

        void _add_slot_waiter(spg_t token, OSDShardPGSlot* slot, OpSchedulerItem&& qi);
    ...
    void dequeue_op(PGRef pg, OpRequestRef op, ThreadPool::TPHandle& handle);

    void enqueue_peering_evt(spg_t pgid, PGPeeringEventRef ref);
    void dequeue_peering_evt(OSDShard* sdata, PG* pg, PGPeeringEventRef ref, ThreadPool::TPHandle& handle);

    void dequeue_delete(OSDShard* sdata, PG* pg, epoch_t epoch, ThreadPool::TPHandle& handle);
...
protected:
    ...
    OSDMapRef _osdmap;
    void set_osdmap(OSDMapRef osdmap) { std::atomic_store(&_osdmap, osdmap); }
    OSDMapRef get_osdmap() const { return std::atomic_load(&_osdmap); }
    ...
    bool advance_pg(epoch_t advance_to, PG* pg, ThreadPool::TPHandle& handle, PeeringCtx& rctx);
    void consume_map();
    void activate_map();

    // osd map cache (past osd maps)
    OSDMapRef get_map(epoch_t e) { return service.get_map(e); }
    OSDMapRef add_map(OSDMap* o) { return service.add_map(o); }
    bool get_map_bl(epoch_t e, ceph::buffer::list& bl) { return service.get_map_bl(e, bl); }
...
public:
    ...
    void inc_num_pgs() { ++num_pgs; }
    void dec_num_pgs() { --num_pgs; }
    int get_num_pgs() const { return num_pgs; }
...
protected:
    ...
    bool add_merge_waiter(OSDMapRef nextmap, spg_t target, PGRef source, unsigned need);
...
protected:
    ...
    bool maybe_wait_for_max_pg(const OSDMapRef& osdmap, spg_t pgid, bool is_mon_create);
    ...
    void split_pgs(PG* parent, const std::set<spg_t>& childpgids, std::set<PGRef>* out_pgs, OSDMapRef curmap,
                   OSDMapRef nextmap, PeeringCtx& rctx);
    ...
    ceph_tid_t get_tid() { return service.get_tid(); }

    // -- generic pg peering --
    void dispatch_context(PeeringCtx& ctx, PG* pg, OSDMapRef curmap, ThreadPool::TPHandle* handle = NULL);
...
protected:
    ...
    // -- pg recovery --
    void do_recovery(PG* pg, epoch_t epoch_queued, uint64_t pushes_reserved, int priority,
...
private:
    ...
    bool ms_can_fast_dispatch(const Message* m) const override
    {
        ...
        case MSG_OSD_PG_LEASE_ACK: return true;
        default: return false;
        }
    }
...
public:
    ...
    OSD(CephContext* cct_, std::unique_ptr<ObjectStore> store_, int id, Messenger* internal, Messenger* external,
        Messenger* hb_front_client, Messenger* hb_back_client, Messenger* hb_front_server, Messenger* hb_back_server,
        Messenger* osdc_messenger, MonClient* mc, const std::string& dev, const std::string& jdev,
        ceph::async::io_context_pool& poolctx);
    ~OSD() override;

    // static bits
    static int mkfs(CephContext* cct, std::unique_ptr<ObjectStore> store, uuid_d fsid, int whoami,
                    std::string osdspec_affinity);

    /* remove any non-user xattrs from a std::map of them */
    void filter_xattrs(std::map<std::string, ceph::buffer::ptr>& attrs)
    {
        for (std::map<std::string, ceph::buffer::ptr>::iterator iter = attrs.begin(); iter != attrs.end();) {
...
private:
    ...
    static int write_meta(CephContext* cct, ObjectStore* store, uuid_d& cluster_fsid, uuid_d& osd_fsid, int whoami,
                          std::string& osdspec_affinity);
    ...
    bool maybe_override_options_for_qos(const std::set<std::string>* changed = nullptr);
