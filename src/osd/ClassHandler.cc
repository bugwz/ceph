    ...
    if (cls->status == ClassData::CLASS_UNKNOWN || cls->status == ClassData::CLASS_MISSING) {
        ...
        if (cls_deps) {
            ...
            while (deps) {
                ...
                if (cls_dep->status != ClassData::CLASS_OPEN) cls->missing_dependencies.insert(cls_dep);
                deps++;
            }
        }
    }
                                                                    ...
                                                                    cls_method_call_t func)
{
    ...
    if (!flags) {
        lderr(handler->cct) << "register_method " << name << "." << mname << " flags " << flags << " " << (void*)func
                            << " FAILED -- flags must be non-zero" << dendl;
        return NULL;
    }
    ldout(handler->cct, 10) << "register_method " << name << "." << mname << " flags " << flags << " " << (void*)func
                            << dendl;
...
ClassHandler::ClassMethod* ClassHandler::ClassData::_get_method(const std::string& mname)
{
...
void ClassHandler::ClassData::unregister_method(ClassHandler::ClassMethod* method)
{
    ...
    if (iter == methods_map.end()) return;
...
void ClassHandler::ClassData::unregister_filter(ClassHandler::ClassFilter* filter)
{
    ...
    if (iter == filters_map.end()) return;
