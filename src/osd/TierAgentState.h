...
struct TierAgentState
{
    ...
    const char* get_flush_mode_name() const { return get_flush_mode_name(flush_mode); }
    ...
    const char* get_evict_mode_name() const { return get_evict_mode_name(evict_mode); }
    ...
    bool is_idle() const { return delaying || (flush_mode == FLUSH_MODE_IDLE && evict_mode == EVICT_MODE_IDLE); }

    /// add archived HitSet
    void add_hit_set(time_t start, HitSetRef hs) { hit_set_map.insert(std::make_pair(start, hs)); }
    ...
    void discard_hit_sets() { hit_set_map.clear(); }
