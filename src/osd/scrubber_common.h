...
struct PgScrubBeListener
{
    ...
    virtual void force_object_missing(ScrubberPasskey, const std::set<pg_shard_t>& peer, const hobject_t& oid,
...
template<> struct fmt::formatter<requested_scrub_t>
{
    ...
    template<typename FormatContext> auto format(const requested_scrub_t& rs, FormatContext& ctx)
    {
...
struct ScrubPgIF
{
    ...
    virtual void dump_scrubber(ceph::Formatter* f, const requested_scrub_t& request_flags) const = 0;
    ...
    virtual unsigned int scrub_requeue_priority(Scrub::scrub_prio_t with_priority) const = 0;
    ...
    virtual void stats_of_handled_objects(const object_stat_sum_t& delta_stats, const hobject_t& soid) = 0;
     ...
     */
    virtual void update_scrub_stats(ceph::coarse_real_clock::time_point now_is) = 0;
    ...
    virtual void scrub_requested(scrub_level_t scrub_level, scrub_type_t scrub_type, requested_scrub_t& req_flags) = 0;

    // --------------- debugging via the asok ------------------------------

    virtual int asok_debug(std::string_view cmd, std::string param, Formatter* f, std::stringstream& ss) = 0;
};
