...
{
    PrimaryLogPG::OpContext* ctx = *(PrimaryLogPG::OpContext**)hctx;
    if (!ctx->new_obs.exists || (ctx->new_obs.oi.is_whiteout() && ctx->obc->ssc->snapset.clones.empty())) {
        return -ENOENT;
    }
                   ...
                   const char* cls, const char* method, bufferlist& inbl)
{
    ...
    auto [iter, inserted] =
        (*pctx)->op_finishers.emplace(std::make_pair(subop_num, std::make_unique<GatherFinisher>(osd_op)));
    assert(inserted);
