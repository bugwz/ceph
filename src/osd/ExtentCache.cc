...
void ExtentCache::extent::link(object_extent_set& extent_set, pin_state& pin_state)
{
    ceph_assert(!parent_extent_set);
...
ExtentCache::object_extent_set& ExtentCache::get_or_create(const hobject_t& oid)
{
...
std::pair<ExtentCache::object_extent_set::set::iterator, ExtentCache::object_extent_set::set::iterator>
ExtentCache::object_extent_set::get_containing_range(uint64_t off, uint64_t len)
{
    // fst is first iterator with end after off (may be end)
    auto fst = extent_set.upper_bound(off, uint_cmp());
    if (fst != extent_set.begin()) --fst;
    if (fst != extent_set.end() && off >= (fst->offset + fst->get_length())) ++fst;
...
extent_set ExtentCache::reserve_extents_for_rmw(const hobject_t& oid, write_pin& pin, const extent_set& to_write,
                                                const extent_set& to_read)
{
    ...
    for (auto&& res : to_write) {
        eset.traverse_update(pin,
                             res.first,
                             res.second,
                             [&](uint64_t off, uint64_t len, extent* ext, object_extent_set::update_action* action) {
...
extent_map ExtentCache::get_remaining_extents_for_rmw(const hobject_t& oid, write_pin& pin, const extent_set& to_get)
{
    if (to_get.empty()) {
        return extent_map();
    }
...
void ExtentCache::present_rmw_update(const hobject_t& oid, write_pin& pin, const extent_map& extents)
{
    if (extents.empty()) {
        return;
    }
    ...
    for (auto&& res : extents) {
        eset.traverse_update(pin,
                             ...
                             [&](uint64_t off, uint64_t len, extent* ext, object_extent_set::update_action* action) {
                                 ...
                                 action->bl->substr_of(res.get_val(), off - res.get_off(), len);
                             });
    }
}
...
ostream& ExtentCache::print(ostream& out) const
{
    ...
    for (auto esiter = per_object_caches.begin(); esiter != per_object_caches.end(); ++esiter) {
        out << "  Extents(" << esiter->oid << ")[" << std::endl;
        for (auto exiter = esiter->extent_set.begin(); exiter != esiter->extent_set.end(); ++exiter) {
            out << "    Extent(" << exiter->offset << "~" << exiter->get_length() << ":" << exiter->pin_tid() << ")"
                << std::endl;
        }
    }
    return out << ")" << std::endl;
