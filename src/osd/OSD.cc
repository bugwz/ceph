...
CompatSet OSD::get_osd_initial_compat_set()
{
    ...
    return CompatSet(ceph_osd_feature_compat, ceph_osd_feature_ro_compat, ceph_osd_feature_incompat);
}
...
OSDService::OSDService(OSD* osd, ceph::async::io_context_pool& poolctx)
    ...
    , promote_max_bytes(0)
    , poolctx(poolctx)
    , objecter(make_unique<Objecter>(osd->client_messenger->cct, osd->objecter_messenger, osd->monc, poolctx))
    ...
    , local_reserver(cct, &reserver_finisher, cct->_conf->osd_max_backfills, cct->_conf->osd_min_recovery_priority)
    , remote_reserver(cct, &reserver_finisher, cct->_conf->osd_max_backfills, cct->_conf->osd_min_recovery_priority)
    , snap_reserver(cct, &reserver_finisher, cct->_conf->osd_max_trimming_pgs)
...
void OSDService::dump_live_pgids()
{
    ...
    for (map<spg_t, int>::const_iterator i = pgid_tracker.cbegin(); i != pgid_tracker.cend(); ++i) {
        derr << "\t" << *i << dendl;
...
void OSDService::identify_splits_and_merges(OSDMapRef old_map, OSDMapRef new_map, spg_t pgid,
                                            set<pair<spg_t, epoch_t>>* split_children,
                                            set<pair<spg_t, epoch_t>>* merge_pgs)
{
    dout(20) << __func__ << " " << pgid << " e" << old_map->get_epoch() << " to e" << new_map->get_epoch() << dendl;
    if (!old_map->have_pg_pool(pgid.pool())) {
        dout(20) << __func__ << " " << pgid << " pool " << pgid.pool() << " does not exist in old map" << dendl;
        return;
    }
    ...
    if (p == osd->pg_num_history.pg_nums.end()) {
        dout(20) << __func__ << " " << pgid << " pool " << pgid.pool() << " has no history" << dendl;
        return;
    }
    dout(20) << __func__ << " " << pgid << " e" << old_map->get_epoch() << " to e" << new_map->get_epoch()
    ...
    while (!queue.empty()) {
        ...
        for (auto q = p->second.lower_bound(old_map->get_epoch());
             q != p->second.end() && q->first <= new_map->get_epoch();
             ++q) {
            if (pgnum < q->second) {
                // split?
                if (cur.ps() < pgnum) {
                    ...
                    if (cur.is_split(pgnum, q->second, &children)) {
                        dout(20) << __func__ << " " << cur << " e" << q->first << " pg_num " << pgnum << " -> "
                                 << q->second << " children " << children << dendl;
                        for (auto i : children) {
                            split_children->insert(make_pair(i, q->first));
                            if (!did.count(i)) queue.push_back(i);
                        }
                    }
                }
                else if (cur.ps() < q->second) {
                    dout(20) << __func__ << " " << cur << " e" << q->first << " pg_num " << pgnum << " -> " << q->second
                             << " is a child" << dendl;
                ...
                if (cur.ps() >= q->second) {
                    if (cur.ps() < pgnum) {
                        ...
                        if (cur.is_merge_source(pgnum, q->second, &parent)) {
                            ...
                            dout(20) << __func__ << " " << cur << " e" << q->first << " pg_num " << pgnum << " -> "
                                     << q->second << " is merge source, target " << parent << ", source(s) " << children
                                     << dendl;
                            ...
                            for (auto c : children) {
                                ...
                                if (!did.count(c)) queue.push_back(c);
                            }
                        }
                    }
                    else {
                        dout(20) << __func__ << " " << cur << " e" << q->first << " pg_num " << pgnum << " -> "
                                 << q->second << " is beyond old pgnum, skipping" << dendl;
                    }
                }
                else {
                    set<spg_t> children;
                    if (cur.is_split(q->second, pgnum, &children)) {
                        dout(20) << __func__ << " " << cur << " e" << q->first << " pg_num " << pgnum << " -> "
                                 << q->second << " is merge target, source " << children << dendl;
                        for (auto c : children) {
                            merge_pgs->insert(make_pair(c, q->first));
                            if (!did.count(c)) queue.push_back(c);
                        }
...
{
    osd->enqueue_peering_evt(spgid, PGPeeringEventRef(std::make_shared<PGPeeringEvent>(epoch, epoch, RenewLease())));
}
...
void OSDService::init()
{
    ...
    if (cct->_conf->osd_recovery_delay_start) defer_recovery(cct->_conf->osd_recovery_delay_start);
}
...
OSDMapRef OSDService::get_nextmap_reserved()
{
    ...
    std::map<epoch_t, unsigned>::iterator i = map_reservations.insert(std::make_pair(e, 0)).first;
    i->second++;
...
void OSDService::release_map(OSDMapRef osdmap)
{
    ...
    std::map<epoch_t, unsigned>::iterator i = map_reservations.find(osdmap->get_epoch());
...
void OSDService::await_reserved_maps()
{
    ...
    pre_publish_cond.wait(l, [this] {
        ...
        return (i == map_reservations.cend() || i->first >= next_osdmap->get_epoch());
    });
    pre_publish_waiter--;
...
public:
    explicit AgentTimeoutCB(PGRef _pg)
        : pg(_pg)
    {}
...
void OSDService::agent_entry()
{
    ...
    while (!agent_stop_flag) {
        ...
        dout(10) << __func__ << " tiers " << agent_queue.size() << ", top is " << level << " with pgs " << top.size()
        ...
        if (!flush_mode_high_count) agent_flush_quota = cct->_conf->osd_agent_max_low_ops - agent_ops;
        ...
        dout(10) << "high_count " << flush_mode_high_count << " agent_ops " << agent_ops << " flush_quota "
                 << agent_flush_quota << dendl;
        agent_locker.unlock();
        if (!pg->agent_work(max, agent_flush_quota)) {
            dout(10) << __func__ << " " << pg->pg_id << " no agent_work, delay for " << cct->_conf->osd_agent_delay_time
                     << " seconds" << dendl;
...
void OSDService::promote_throttle_recalibrate()
{
    ...
    dout(10) << __func__ << " " << attempts << " attempts, promoted " << obj << " objects and " << byte_u_t(bytes)
             << "; target " << target_obj_sec << " obj/sec or " << byte_u_t(target_bytes_sec) << "/sec" << dendl;

    // calculate what the probability *should* be, given the targets
    unsigned new_prob;
    if (attempts && dur > 0) {
        uint64_t avg_size = 1;
        if (obj) avg_size = std::max<uint64_t>(bytes / obj, 1);
        unsigned po = (double)target_obj_sec * dur * 1000.0 / (double)attempts;
        unsigned pb = (double)target_bytes_sec / (double)avg_size * dur * 1000.0 / (double)attempts;
        dout(20) << __func__ << "  po " << po << " pb " << pb << " avg_size " << avg_size << dendl;
    ...
    dout(10) << __func__ << "  actual " << actual << ", actual/prob ratio " << ratio << ", adjusted new_prob "
             << new_prob << ", prob " << promote_probability_millis << " -> " << prob << dendl;
...
OSDService::s_names OSDService::recalc_full_state(float ratio, float pratio, string& inject)
{
    ...
    else if (full_ratio <= 0 || backfillfull_ratio <= 0 || nearfull_ratio <= 0) {
...
void OSDService::check_full_status(float ratio, float pratio)
{
    ...
    dout(20) << __func__ << " cur ratio " << ratio << ", physical ratio " << pratio << ", new state "
             << get_full_state_name(new_state) << " " << inject << dendl;

    // warn
    if (cur_state != new_state) {
        dout(10) << __func__ << " " << get_full_state_name(cur_state) << " -> " << get_full_state_name(new_state)
                 << dendl;
        if (new_state == FAILSAFE) {
            clog->error() << "full status failsafe engaged, dropping updates, now " << (int)roundf(ratio * 100)
                          << "% full";
        }
...
bool OSDService::_check_inject_full(DoutPrefixProvider* dpp, s_names type) const
{
    if (injectfull && injectfull_state >= type) {
        ...
        if (injectfull > 0) --injectfull;
        ldpp_dout(dpp, 10) << __func__ << " Injected " << get_full_state_name(type) << " OSD ("
                           << (injectfull < 0 ? "set" : std::to_string(injectfull)) << ")" << dendl;
        return true;
    }
    return false;
}
...
bool OSDService::_check_full(DoutPrefixProvider* dpp, s_names type) const
{
    ...
    if (_check_inject_full(dpp, type)) return true;
...
{
    ldpp_dout(dpp, 20) << __func__ << " type " << get_full_state_name(type) << " adjust_used " << (adjust_used >> 10)
                       << "KiB" << dendl;
    {
        std::lock_guard l(full_status_lock);
    ...
    if (tentative_state >= type) ldpp_dout(dpp, 10) << __func__ << " tentative usage is " << ratio << dendl;
...
void OSDService::set_statfs(const struct store_statfs_t& stbuf, osd_alert_list_t& alerts)
{
    ...
    if (cct->_conf->fake_statfs_for_testing) {
        ...
        if (total_num_bytes < bytes)
            ...
            avail = 0;
        dout(0) << __func__ << " fake total " << cct->_conf->fake_statfs_for_testing << " adjust available " << avail
                << dendl;
        used = bytes - avail;
    }
...
osd_stat_t OSDService::set_osd_stat(vector<int>& hb_peers, int num_pgs)
{
    utime_t now = ceph_clock_now();
    ...
    for (auto i : osd_stat.hb_pingtime) {
        if (i.second.last_update == 0) continue;
        if (stale_time && now.sec() - i.second.last_update > stale_time) {
            dout(20) << __func__ << " time out heartbeat for osd " << i.first << " last_update " << i.second.last_update
                     << dendl;
...
float OSDService::compute_adjusted_ratio(osd_stat_t new_stat, float* pratio, uint64_t adjust_used)
{
    *pratio = ((float)new_stat.statfs.get_used_raw()) / ((float)new_stat.statfs.total);
...
{
    dout(20) << __func__ << " " << m->get_type_name() << " to osd." << peer << " from_epoch " << from_epoch << dendl;
    OSDMapRef next_map = get_nextmap_reserved();
    // service map is always newer/newest
    ceph_assert(from_epoch <= next_map->get_epoch());

    if (next_map->is_down(peer) || next_map->get_info(peer).up_from > from_epoch) {
        m->put();
    ...
    if (peer == whoami) {
        ...
        peer_con = osd->cluster_messenger->connect_to_osd(next_map->get_cluster_addrs(peer), false, true);
    }
...
{
    dout(20) << __func__ << " to osd." << peer << " from_epoch " << from_epoch << dendl;
    OSDMapRef next_map = get_nextmap_reserved();
    // service map is always newer/newest
    ceph_assert(from_epoch <= next_map->get_epoch());
    ...
    if (peer == whoami) {
        ...
        con = osd->cluster_messenger->connect_to_osd(next_map->get_cluster_addrs(peer), false, true);
    }
    release_map(next_map);
    return con;
}
...
pair<ConnectionRef, ConnectionRef> OSDService::get_con_osd_hb(int peer, epoch_t from_epoch)
{
    ...
    ret.first = osd->hb_back_client_messenger->connect_to_osd(next_map->get_hb_back_addrs(peer));
    ret.second = osd->hb_front_client_messenger->connect_to_osd(next_map->get_hb_front_addrs(peer));
    release_map(next_map);
    return ret;
}
...
void OSDService::queue_want_pg_temp(pg_t pgid, const vector<int>& want, bool forced)
{
    std::lock_guard l(pg_temp_lock);
    auto p = pg_temp_pending.find(pgid);
    if (p == pg_temp_pending.end() || p->second.acting != want || forced) {
...
#else
    pg_temp_pending.insert(make_move_iterator(begin(pg_temp_wanted)), make_move_iterator(end(pg_temp_wanted)));
#endif
...
std::ostream& operator<<(std::ostream& out, const OSDService::pg_temp_t& pg_temp)
{
    out << pg_temp.acting;
    if (pg_temp.forced) {
        out << " (forced)";
...
{
    std::lock_guard l(pg_temp_lock);
    if (pg_temp_wanted.empty()) return;
...
void OSDService::retrieve_epochs(epoch_t* _boot_epoch, epoch_t* _up_epoch, epoch_t* _bind_epoch) const
{
    std::lock_guard l(epoch_lock);
    if (_boot_epoch) *_boot_epoch = boot_epoch;
    if (_up_epoch) *_up_epoch = up_epoch;
    if (_bind_epoch) *_bind_epoch = bind_epoch;
}

void OSDService::set_epochs(const epoch_t* _boot_epoch, const epoch_t* _up_epoch, const epoch_t* _bind_epoch)
{
    std::lock_guard l(epoch_lock);
    if (_boot_epoch) {
...
MOSDMap* OSDService::build_incremental_map_msg(epoch_t since, epoch_t to, OSDSuperblock& sblock)
{
    MOSDMap* m = new MOSDMap(monc->get_fsid(), osdmap->get_encoding_features());
    ...
    if (since < m->cluster_osdmap_trim_lower_bound) {
        ...
        // full map.
        bufferlist bl;
        dout(10) << __func__ << " cluster osdmap lower bound " << sblock.cluster_osdmap_trim_lower_bound << " > since "
...
panic:
    if (!m->maps.empty() || !m->incremental_maps.empty()) {
...
void OSDService::send_incremental_map(epoch_t since, Connection* con, const OSDMapRef& osdmap)
{
    epoch_t to = osdmap->get_epoch();
    dout(10) << "send_incremental_map " << since << " -> " << to << " to " << con << " " << con->get_peer_addr()
             << dendl;
...
bool OSDService::_get_map_bl(epoch_t e, bufferlist& bl)
{
    ...
    found = store->read(meta_ch, OSD::get_osdmap_pobject_name(e), 0, 0, bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED) >= 0;
    if (found) {
        _add_map_bl(e, bl);
    }
    return found;
...
bool OSDService::get_inc_map_bl(epoch_t e, bufferlist& bl)
{
    ...
    logger->inc(l_osd_map_bl_cache_miss);
    found = store->read(meta_ch, OSD::get_inc_osdmap_pobject_name(e), 0, 0, bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED) >= 0;
    if (found) {
                                ...
                                vector<pg_log_op_return_item_t> op_returns)
{
    ...
    MOSDOpReply* reply = new MOSDOpReply(m, err, osdmap->get_epoch(), flags, !m->has_flag(CEPH_OSD_FLAG_RETURNVEC));
...
void OSDService::handle_misdirected_op(PG* pg, OpRequestRef op)
{
    ...
    if (pg->is_ec_pg()) {
        ...
        if (!opmap) {
            dout(7) << __func__ << ": " << *pg << " no longer have map for " << m->get_map_epoch() << ", dropping"
                    << dendl;
            return;
        }
        pg_t _pgid = m->get_raw_pg();
        spg_t pgid;
        if ((m->get_flags() & CEPH_OSD_FLAG_PGOP) == 0) _pgid = opmap->raw_pg_to_pg(_pgid);
        if (opmap->get_primary_shard(_pgid, &pgid) && pgid.shard != pg->pg_id.shard) {
            dout(7) << __func__ << ": " << *pg << " primary changed since " << m->get_map_epoch() << ", dropping"
                    << dendl;
            return;
        }
    }
    ...
    clog->warn() << m->get_source_inst() << " misdirected " << m->get_reqid() << " pg " << m->get_raw_pg() << " to osd."
                 << whoami << " not " << pg->get_acting() << " in e" << m->get_map_epoch() << "/"
                 << osdmap->get_epoch();
}
...
void OSDService::queue_recovery_context(PG* pg, GenContext<ThreadPool::TPHandle&>* c, uint64_t cost, int priority)
{
    ...
    enqueue_back(
        OpSchedulerItem(unique_ptr<OpSchedulerItem::OpQueueable>(new PGRecoveryContext(pg->get_pgid(), c, e, priority)),
                        cost_for_queue,
...
void OSDService::queue_for_snap_trim(PG* pg)
{
    ...
    enqueue_back(OpSchedulerItem(
        unique_ptr<OpSchedulerItem::OpQueueable>(new PGSnapTrim(pg->get_pgid(), pg->get_osdmap_epoch())),
...
void OSDService::queue_scrub_event_msg(PG* pg, Scrub::scrub_prio_t with_priority, unsigned int qu_priority,
                                       Scrub::act_token_t act_token)
{
    const auto epoch = pg->get_osdmap_epoch();
    auto msg = new MSG_TYPE(pg->get_pgid(), epoch, act_token);
    dout(15) << "queue a scrub event (" << *msg << ") for " << *pg << ". Epoch: " << epoch << " token: " << act_token
             << dendl;
    enqueue_back(OpSchedulerItem(unique_ptr<OpSchedulerItem::OpQueueable>(msg),
                                 get_scrub_cost(),
                                 pg->scrub_requeue_priority(with_priority, qu_priority),
                                 ceph_clock_now(),
                                 0,
                                 epoch));
}

template<class MSG_TYPE> void OSDService::queue_scrub_event_msg(PG* pg, Scrub::scrub_prio_t with_priority)
{
    const auto epoch = pg->get_osdmap_epoch();
...
void OSDService::queue_for_rep_scrub(PG* pg, Scrub::scrub_prio_t with_priority, unsigned int qu_priority,
                                     Scrub::act_token_t act_token)
{
    queue_scrub_event_msg<PGRepScrub>(pg, with_priority, qu_priority, act_token);
}

void OSDService::queue_for_rep_scrub_resched(PG* pg, Scrub::scrub_prio_t with_priority, unsigned int qu_priority,
                                             Scrub::act_token_t act_token)
{
    // Resulting scrub event: 'SchedReplica'
    queue_scrub_event_msg<PGRepScrubResched>(pg, with_priority, qu_priority, act_token);
}
...
void OSDService::queue_for_pg_delete(spg_t pgid, epoch_t e)
{
    ...
    enqueue_back(OpSchedulerItem(unique_ptr<OpSchedulerItem::OpQueueable>(new PGDelete(pgid, e)),
...
void OSDService::set_ready_to_merge_target(PG* pg, eversion_t version, epoch_t last_epoch_started,
                                           epoch_t last_epoch_clean)
{
    std::lock_guard l(merge_lock);
    dout(10) << __func__ << " " << pg->pg_id << dendl;
    ready_to_merge_target.insert(make_pair(pg->pg_id.pgid, make_tuple(version, last_epoch_started, last_epoch_clean)));
...
{
    dout(20) << __func__ << " ready_to_merge_source " << ready_to_merge_source << " not_ready_to_merge_source "
             << not_ready_to_merge_source << " ready_to_merge_target " << ready_to_merge_target
             << " not_ready_to_merge_target " << not_ready_to_merge_target << " sent_ready_to_merge_source "
             << sent_ready_to_merge_source << dendl;
    for (auto src : not_ready_to_merge_source) {
        if (sent_ready_to_merge_source.count(src) == 0) {
            monc->send_mon_message(new MOSDPGReadyToMerge(src, {}, {}, 0, 0, false, osdmap->get_epoch()));
            sent_ready_to_merge_source.insert(src);
        }
    }
    for (auto p : not_ready_to_merge_target) {
        if (sent_ready_to_merge_source.count(p.second) == 0) {
            monc->send_mon_message(new MOSDPGReadyToMerge(p.second, {}, {}, 0, 0, false, osdmap->get_epoch()));
            sent_ready_to_merge_source.insert(p.second);
        }
    }
    for (auto src : ready_to_merge_source) {
        if (not_ready_to_merge_source.count(src.first) || not_ready_to_merge_target.count(src.first.get_parent())) {
            continue;
        }
        auto p = ready_to_merge_target.find(src.first.get_parent());
        if (p != ready_to_merge_target.end() && sent_ready_to_merge_source.count(src.first) == 0) {
            monc->send_mon_message(new MOSDPGReadyToMerge(src.first,                // source pgid
...
void OSDService::_queue_for_recovery(pg_awaiting_throttle_t p, uint64_t reserved_pushes)
{
    ...
    enqueue_back(OpSchedulerItem(unique_ptr<OpSchedulerItem::OpQueueable>(
                                     new PGRecovery(p.pg->get_pgid(), p.epoch_queued, reserved_pushes, p.priority)),
                                 cost_for_queue,
                                 cct->_conf->osd_recovery_priority,
                                 ceph_clock_now(),
                                 0,
...
int heap(CephContext& cct, const cmdmap_t& cmdmap, std::ostream& outos, std::ostream& erros);

}   // namespace ceph::osd_cmds

int OSD::mkfs(CephContext* cct, std::unique_ptr<ObjectStore> store, uuid_d fsid, int whoami, string osdspec_affinity)
{
    int ret;
    ...
    auto umount_store = make_scope_guard([&] { store->umount(); });
    ...
    if (ch) {
        ...
        ObjectStore::CollectionHandle ch = store->create_new_collection(coll_t::meta());
                    ...
                    string& osdspec_affinity)
{
    ...
    if (key.size()) {
        ...
        if (!keyfile.empty()) {
            ...
            if (r < 0) {
                derr << __func__ << " failed to read keyfile " << keyfile << ": " << err << ": " << cpp_strerror(r)
                     << dendl;
                return r;
            }
...
OSD::OSD(CephContext* cct_, std::unique_ptr<ObjectStore> store_, int id, Messenger* internal_messenger,
         Messenger* external_messenger, Messenger* hb_client_front, Messenger* hb_client_back,
         Messenger* hb_front_serverm, Messenger* hb_back_serverm, Messenger* osdc_messenger, MonClient* mc,
         const std::string& dev, const std::string& jdev, ceph::async::io_context_pool& poolctx)
    : Dispatcher(cct_)
    ...
    , asok_hook(NULL)
    , m_osd_pg_epoch_max_lag_factor(cct->_conf.get_val<double>("osd_pg_epoch_max_lag_factor"))
    ...
    , op_tracker(cct, cct->_conf->osd_enable_op_tracker, cct->_conf->osd_num_op_tracker_shard)
    , test_ops_hook(NULL)
    , op_shardedwq(this, ceph::make_timespan(cct->_conf->osd_op_thread_timeout),
                   ceph::make_timespan(cct->_conf->osd_op_thread_suicide_timeout), &osd_op_tp)
    ...
    , service(this, poolctx)
{
    ...
    if (!gss_ktfile_client.empty()) {
        ...
        */
        const int32_t set_result(setenv("KRB5_CLIENT_KTNAME", gss_ktfile_client.c_str(), 1));
        ceph_assert(set_result == 0);
    }

    monc->set_messenger(client_messenger);
    op_tracker.set_complaint_and_threshold(cct->_conf->osd_op_complaint_time, cct->_conf->osd_op_log_threshold);
    op_tracker.set_history_size_and_duration(cct->_conf->osd_op_history_size, cct->_conf->osd_op_history_duration);
...
double OSD::get_tick_interval() const
{
    ...
    return (OSD_TICK_INTERVAL * ceph::util::generate_random_number(1.0 - delta, 1.0 + delta));
}
...
int OSD::set_numa_affinity()
{
    ...
    string front_iface = pick_iface(cct, client_messenger->get_myaddrs().front().get_sockaddr_storage());
    string back_iface = pick_iface(cct, cluster_messenger->get_myaddrs().front().get_sockaddr_storage());
    ...
    if (r >= 0 && front_node >= 0) {
        ...
        if (r >= 0 && back_node >= 0) {
            ...
            if (front_node == back_node && front_node == store_node) {
    ...
    if (numa_node >= 0) {
        ...
        if (r < 0) {
            ...
            dout(1) << __func__ << " setting numa affinity to node " << numa_node << " cpus "
...
public:
    explicit OSDSocketHook(OSD* o)
        : osd(o)
    {}
    int call(std::string_view prefix, const cmdmap_t& cmdmap, const bufferlist& inbl, Formatter* f, std::ostream& ss,
             bufferlist& out) override
    {
        ceph_abort("should use async hook");
    }
    void call_async(std::string_view prefix, const cmdmap_t& cmdmap, Formatter* f, const bufferlist& inbl,
...
OSD::PGRefOrError OSD::locate_asok_target(const cmdmap_t& cmdmap, stringstream& ss, bool only_primary)
{
    string pgidstr;
...
int OSD::asok_route_to_pg(bool only_primary, std::string_view prefix, cmdmap_t cmdmap, Formatter* f, stringstream& ss,
...
void OSD::asok_command(std::string_view prefix, const cmdmap_t& cmdmap, Formatter* f, const bufferlist& inbl,
                       std::function<void(int, const std::string&, bufferlist&)> on_finish)
{
    int ret = 0;
    ...
    if (prefix == "pg" || prefix == "query" || prefix == "log" || prefix == "mark_unfound_lost" ||
        prefix == "list_unfound" || prefix == "scrub" || prefix == "deep_scrub") {
        string pgidstr;
        pg_t pgid;
        if (!cmd_getval(cmdmap, "pgid", pgidstr)) {
            ss << "no pgid specified";
    ...
    // --- OSD commands follow ---
        ...
        f->dump_unsigned("cluster_osdmap_trim_lower_bound", superblock.cluster_osdmap_trim_lower_bound);
    ...
    else if (prefix == "dump_ops_in_flight" || prefix == "ops" || prefix == "dump_blocked_ops" ||
             prefix == "dump_blocked_ops_count" || prefix == "dump_historic_ops" ||
             prefix == "dump_historic_ops_by_duration" || prefix == "dump_historic_slow_ops") {
...
will start to track new ops received afterwards.";
        ...
        if (cmd_getval(cmdmap, "filterstr", filter_str)) {
            copy(filter_str.begin(), filter_str.end(), inserter(filters, filters.end()));
        }

        if (prefix == "dump_ops_in_flight" || prefix == "ops") {
        ...
        for (list<pair<entity_addr_t, utime_t>>::iterator it = bl.begin(); it != bl.end(); ++it) {
        ...
        for (list<pair<entity_addr_t, utime_t>>::iterator it = rbl.begin(); it != rbl.end(); ++it) {
            f->open_object_section("entry");
            f->open_object_section("entity_addr_t");
            it->first.dump(f);
            f->close_section();   // entity_addr_t
        ...
        for (list<obj_watch_item_t>::iterator it = watchers.begin(); it != watchers.end(); ++it) {
        ...
        for (vector<string>::iterator a = ++msg.begin(); a != msg.end(); ++a) message += " " + *a;
        string lvl;
        ...
        if (ret != 0) {
            ss << "kick_recovery_wq: error setting "
               << "osd_recovery_delay_start to '" << delay << "': error " << ret;
            goto out;
        }
        ...
        cct->get_perfcounters_collection()->dump_formatted_histograms(f, false, logger, counter);
    }
        ...
        if (!(cmd_getval(cmdmap, "value", value))) {
            ...
            value = static_cast<double>(g_conf().get_val<double>("mon_warn_on_slow_ping_time")) * 1000;
            if (value == 0) {
        ...
        struct osd_ping_time_t
        {
            ...
            {
                if (pingtime < rhs.pingtime) return true;
                if (pingtime > rhs.pingtime) return false;
        ...
        for (auto j : *pingtimes) {
            if (j.second.last_update == 0) continue;
            ...
            if (j.second.front_last == 0) continue;
...
int OSD::run_osd_bench_test(int64_t count, int64_t bsize, int64_t osize, int64_t onum, double* elapsed, ostream& ss)
{
    int ret = 0;
    ...
    if (bsize > (int64_t)cct->_conf->osd_bench_max_block_size) {
        ...
        ss << "block 'size' values are capped at " << byte_u_t(cct->_conf->osd_bench_max_block_size)
           << ". If you wish to use"
        ...
        int64_t max_count = bsize * duration * cct->_conf->osd_bench_small_size_max_iops;
        if (count > max_count) {
            ss << "'count' values greater than " << max_count << " for a block size of " << byte_u_t(bsize)
               << ", assuming " << cct->_conf->osd_bench_small_size_max_iops << " IOPS,"
        ...
        int64_t max_count = cct->_conf->osd_bench_large_size_max_throughput * duration;
        if (count > max_count) {
            ss << "'count' values greater than " << max_count << " for a block size of " << byte_u_t(bsize)
               << ", assuming " << byte_u_t(cct->_conf->osd_bench_large_size_max_throughput) << "/s,"
               << " for " << duration << " seconds,"
               << " can cause ill effects on osd. "
...
public:
    TestOpsSocketHook(OSDService* s, ObjectStore* st)
        : service(s)
        , store(st)
    {}
    int call(std::string_view command, const cmdmap_t& cmdmap, const bufferlist&, Formatter* f, std::ostream& errss,
             bufferlist& out) override
    {
        int r = 0;
        stringstream outss;
    ...
    void test_ops(OSDService* service, ObjectStore* store, std::string_view command, const cmdmap_t& cmdmap,
                  ostream& ss);
};
...
public:
    explicit C_Tick(OSD* o)
        : osd(o)
    {}
    void finish(int r) override { osd->tick(); }
};
...
public:
    explicit C_Tick_WithoutOSDLock(OSD* o)
        : osd(o)
    {}
    void finish(int r) override { osd->tick_without_osd_lock(); }
};
...
{
    if (cct->_conf->osd_op_num_shards) return cct->_conf->osd_op_num_shards;
...
{
    if (cct->_conf->osd_op_num_threads_per_shard) return get_num_op_shards() * cct->_conf->osd_op_num_threads_per_shard;
    if (store_is_rotational)
...
{
    if (cct->_conf->osd_recovery_sleep) return cct->_conf->osd_recovery_sleep;
...
float OSD::get_osd_delete_sleep()
{
    ...
    if (osd_delete_sleep > 0) return osd_delete_sleep;
    if (!store_is_rotational && !journal_is_rotational) return cct->_conf.get_val<double>("osd_delete_sleep_ssd");
    if (store_is_rotational && !journal_is_rotational) return cct->_conf.get_val<double>("osd_delete_sleep_hybrid");
    return cct->_conf.get_val<double>("osd_delete_sleep_hdd");
}

int OSD::get_recovery_max_active()
{
    if (cct->_conf->osd_recovery_max_active) return cct->_conf->osd_recovery_max_active;
    if (store_is_rotational)
...
float OSD::get_osd_snap_trim_sleep()
{
    ...
    if (osd_snap_trim_sleep > 0) return osd_snap_trim_sleep;
    if (!store_is_rotational && !journal_is_rotational) return cct->_conf.get_val<double>("osd_snap_trim_sleep_ssd");
    if (store_is_rotational && !journal_is_rotational) return cct->_conf.get_val<double>("osd_snap_trim_sleep_hybrid");
...
int OSD::init()
{
    ...
    auto rotating_auth_timeout = g_conf().get_val<int64_t>("rotating_keys_bootstrap_timeout");
    ...
    if (!service.meta_ch) {
        derr << "OSD:init: unable to open meta collection" << dendl;
        r = -ENOENT;
        goto out;
    }
    ...
    if (cct->_conf->osd_open_classes_on_start) {
        ...
        if (r) dout(1) << "warning: got an error loading one or more classes: " << cpp_strerror(r) << dendl;
    }
    ...
    for (auto m : {client_messenger, cluster_messenger, hb_front_server_messenger, hb_back_server_messenger}) {
        m->set_auth_server(monc);
    }
    monc->set_handle_authentication_dispatcher(this);

    monc->set_want_keys(CEPH_ENTITY_TYPE_MON | CEPH_ENTITY_TYPE_OSD | CEPH_ENTITY_TYPE_MGR);
    r = monc->init();
    if (r < 0) goto out;

    mgrc.set_pgstats_cb([this]() { return collect_pg_stats(); });
    mgrc.set_perf_metric_query_cb([this](const ConfigPayload& config_payload) { set_perf_queries(config_payload); },
                                  [this] { return get_perf_reports(); });
    mgrc.init();
    ...
    for (auto& shard : shards) {
        ...
        for (auto pg : pgs) {
            ...
            set<pair<spg_t, epoch_t>> merge_pgs;
            service.identify_splits_and_merges(pg->get_osdmap(), osdmap, pg->pg_id, &new_children, &merge_pgs);
    ...
    tick_timer.add_event_after(get_tick_interval(), new C_Tick(this));
    {
        ...
        tick_timer_without_osd_lock.add_event_after(get_tick_interval(), new C_Tick_WithoutOSDLock(this));
    }
    ...
    r = monc->authenticate();
    if (r < 0) {
        derr << __func__ << " authentication failed: " << cpp_strerror(r) << dendl;
        exit(1);
    }
...
void OSD::final_init()
{
    ...
    int r = admin_socket->register_command("status", asok_hook, "high-level status of OSD");
    ceph_assert(r == 0);
    r = admin_socket->register_command("flush_journal", asok_hook, "flush the journal to permanent store");
    ceph_assert(r == 0);
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command("dump_op_pq_state", asok_hook, "dump op queue state");
    ceph_assert(r == 0);
    r = admin_socket->register_command("dump_blocklist", asok_hook, "dump blocklisted clients and times");
    ceph_assert(r == 0);
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command("dump_recovery_reservations", asok_hook, "show recovery reservations");
    ceph_assert(r == 0);
    r = admin_socket->register_command("dump_scrub_reservations", asok_hook, "show scrub reservations");
    ceph_assert(r == 0);
    ...
    r = admin_socket->register_command(
        "dump_objectstore_kv_stats", asok_hook, "print statistics of kvdb which used by bluestore");
    ceph_assert(r == 0);

    r = admin_socket->register_command("dump_scrubs", asok_hook, "print scheduled scrubs");
    ceph_assert(r == 0);
    ...
    r = admin_socket->register_command("flush_store_cache", asok_hook, "Flush bluestore internal cache");
    ceph_assert(r == 0);
    r = admin_socket->register_command("rotate-stored-key", asok_hook, "Update the stored osd_key");
    ceph_assert(r == 0);
    r = admin_socket->register_command("dump_pgstate_history", asok_hook, "show recent state history");
    ceph_assert(r == 0);
    ...
    r = admin_socket->register_command("get_mapped_pools", asok_hook, "dump pools whose PG(s) are mapped to this OSD.");

    ceph_assert(r == 0);

    r = admin_socket->register_command(
        "smart name=devid,type=CephString,req=false", asok_hook, "probe OSD devices for SMART data.");

    ceph_assert(r == 0);

    r = admin_socket->register_command("list_devices", asok_hook, "list OSD devices.");
    r = admin_socket->register_command("send_beacon", asok_hook, "send OSD beacon to mon immediately");

    r = admin_socket->register_command(
        "dump_osd_network name=value,type=CephInt,req=false", asok_hook, "Dump osd heartbeat network ping times");
    ceph_assert(r == 0);

    r = admin_socket->register_command(
        "dump_pool_statfs name=poolid,type=CephInt,req=true", asok_hook, "Dump store's statistics for the given pool");
    ceph_assert(r == 0);
    ...
    r = admin_socket->register_command("setomapval "
    ...
    r = admin_socket->register_command("getomap "
                                       "name=pool,type=CephString "
                                       "name=objname,type=CephObjectname",
                                       test_ops_hook,
    ...
    r = admin_socket->register_command("truncobj "
                                       "name=pool,type=CephString "
                                       "name=objname,type=CephObjectname "
    ...
    r = admin_socket->register_command("injectdataerr "
                                       "name=pool,type=CephString "
                                       "name=objname,type=CephObjectname "
    ...
    r = admin_socket->register_command("injectmdataerr "
                                       "name=pool,type=CephString "
                                       "name=objname,type=CephObjectname "
                                       "name=shardid,type=CephInt,req=false,range=0|255",
                                       test_ops_hook,
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command("cluster_log "
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command("flush_pg_stats", asok_hook, "flush pg stats");
    ceph_assert(r == 0);
    r = admin_socket->register_command(
        "heap "
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command("debug dump_missing "
                                       "name=filename,type=CephFilepath",
                                       asok_hook,
                                       "dump missing objects to a named file");
    ceph_assert(r == 0);
    r = admin_socket->register_command("debug kick_recovery_wq "
                                       "name=delay,type=CephInt,range=0",
                                       asok_hook,
                                       "set osd_recovery_delay_start to <val>");
    ceph_assert(r == 0);
    r = admin_socket->register_command("cpu_profiler "
                                       "name=arg,type=CephChoices,strings=status|flush",
                                       asok_hook,
                                       "run cpu profiling on daemon");
    ceph_assert(r == 0);
    r = admin_socket->register_command("dump_pg_recovery_stats", asok_hook, "dump pg recovery statistics");
    ceph_assert(r == 0);
    r = admin_socket->register_command("reset_pg_recovery_stats", asok_hook, "reset pg recovery statistics");
    ceph_assert(r == 0);
    r = admin_socket->register_command("cache drop", asok_hook, "Drop all OSD caches");
    ceph_assert(r == 0);
    r = admin_socket->register_command("cache status", asok_hook, "Get OSD caches statistics");
    ceph_assert(r == 0);
    r = admin_socket->register_command("scrub_purged_snaps", asok_hook, "Scrub purged_snaps vs snapmapper index");
    ceph_assert(r == 0);
    r = admin_socket->register_command("scrubdebug "
    ...
    r = admin_socket->register_command("pg "
                                       "name=pgid,type=CephPgid "
    ...
    r = admin_socket->register_command("query", asok_hook, "show details of a specific pg");
    ceph_assert(r == 0);
    r = admin_socket->register_command("log", asok_hook, "dump pg_log of a specific pg");
    ceph_assert(r == 0);
    r = admin_socket->register_command("mark_unfound_lost "
                                       "name=pgid,type=CephPgid,req=false "
                                       "name=mulcmd,type=CephChoices,strings=revert|delete",
                                       asok_hook,
                                       "mark all unfound objects in this pg as lost, either removing or reverting to a "
                                       "prior version if one is available");
    ceph_assert(r == 0);
    r = admin_socket->register_command("list_unfound "
                                       "name=pgid,type=CephPgid,req=false "
                                       "name=offset,type=CephString,req=false",
                                       asok_hook,
                                       "list unfound objects on this pg, perhaps starting at an offset given in JSON");
    ceph_assert(r == 0);
    r = admin_socket->register_command("scrub "
                                       "name=pgid,type=CephPgid,req=false "
                                       "name=time,type=CephInt,req=false",
                                       asok_hook,
                                       "Trigger a scheduled scrub ");
    ceph_assert(r == 0);
    r = admin_socket->register_command("deep_scrub "
                                       "name=pgid,type=CephPgid,req=false "
                                       "name=time,type=CephInt,req=false",
                                       asok_hook,
...
int OSD::shutdown()
{
    ...
    dout(0) << "Fast Shutdown: - cct->_conf->osd_fast_shutdown = " << cct->_conf->osd_fast_shutdown
    ...
    if (cct->_conf->osd_fast_shutdown) {
        ...
        if (cct->_conf->osd_fast_shutdown_notify_mon) service.prepare_to_stop();
    ...
    if (cct->_conf->osd_fast_shutdown) {
        ...
        dout(0) << "Fast Shutdown duration osd_drain :" << start_time_umount - start_time_osd_drain << " seconds"
                << dendl;
        dout(0) << "Fast Shutdown duration umount    :" << end_time - start_time_umount << " seconds" << dendl;
        dout(0) << "Fast Shutdown duration timer     :" << start_time_osd_drain - start_time_timer << " seconds"
                << dendl;
        cct->_log->flush();
...
int OSD::mon_cmd_maybe_osd_create(string& cmd)
{
    ...
    while (true) {
        ...
        if (r < 0) {
            if (r == -ENOENT && !created) {
                string newcmd = "{\"prefix\": \"osd create\", \"id\": " + stringify(whoami) + ", \"uuid\": \"" +
                                stringify(superblock.osd_fsid) + "\"}";
...
int OSD::update_crush_location()
{
    ...
    if (cct->_conf->osd_crush_initial_weight >= 0) {
        ...
        snprintf(weight, sizeof(weight), "%.4lf", std::max(.00001, double(st.total) / double(1ull << 40 /* TB */)));
    }

    dout(10) << __func__ << " crush location is " << cct->crush_location << dendl;

    string cmd = string("{\"prefix\": \"osd crush create-or-move\", ") + string("\"id\": ") + stringify(whoami) + ", " +
                 string("\"weight\":") + weight + ", " + string("\"args\": [") + stringify(cct->crush_location) + "]}";
...
int OSD::update_crush_device_class()
{
    ...
    string cmd = string("{\"prefix\": \"osd crush set-device-class\", ") + string("\"class\": \"") + device_class +
                 string("\", ") + string("\"ids\": [\"") + stringify(whoami) + string("\"]}");
...
void OSD::clear_temp_objects()
{
    ...
    for (vector<coll_t>::iterator p = ls.begin(); p != ls.end(); ++p) {
        ...
        while (1) {
            ...
            ceph_assert(ch);
            store->collection_list(ch, next, ghobject_t::get_max(), store->get_ideal_list_max(), &objects, &next);
...
void OSD::recursive_remove_collection(CephContext* cct, ObjectStore* store, spg_t pgid, coll_t tmp)
{
    OSDriver driver(store, coll_t(), make_snapmapper_oid());
    ...
    while (true) {
        objects.clear();
        store->collection_list(ch, next, ghobject_t::get_max(), max, &objects, &next);
        generic_dout(10) << __func__ << " " << objects << dendl;
        if (objects.empty()) break;
        for (auto& p : objects) {
            OSDriver::OSTransaction _t(driver.get_transaction(&t));
            int r = mapper.remove_oid(p.hobj, &_t);
            if (r != 0 && r != -ENOENT) ceph_abort();
            t.remove(tmp, p);
        }
...
PG* OSD::_make_pg(OSDMapRef createmap, spg_t pgid)
{
    ...
    PG* pg;
    if (pi.type == pg_pool_t::TYPE_REPLICATED || pi.type == pg_pool_t::TYPE_ERASURE)
...
void OSD::_get_pgs(vector<PGRef>* v, bool clear_too)
{
    ...
    for (auto& s : shards) {
        ...
        for (auto& j : s->pg_slots) {
            if (j.second->pg && !j.second->pg->is_deleted()) {
...
bool OSD::try_finish_pg_delete(PG* pg, unsigned old_pg_num)
{
    ...
    ceph_assert(sdata);
    {
        ...
        if (p == sdata->pg_slots.end() || !p->second->pg) {
...
void OSD::load_pgs()
{
    ...
    for (vector<coll_t>::iterator it = ls.begin(); it != ls.end(); ++it) {
        spg_t pgid;
        if (it->is_temp(&pgid) || (it->is_pg(&pgid) && PG::_has_removal_flag(store.get(), pgid))) {
        ...
        if (map_epoch > 0) {
            ...
            if (!pgosdmap) {
                if (!get_osdmap()->have_pg_pool(pgid.pool())) {
                    derr << __func__ << ": could not find map for epoch " << map_epoch << " on pg " << pgid
...
PGRef OSD::handle_pg_create_info(const OSDMapRef& osdmap, const PGCreateInfo* info)
{
    ...
    if (info->by_mon) {
        ...
        if (osdmap->require_osd_release >= ceph_release_t::nautilus && !pool->has_flag(pg_pool_t::FLAG_CREATING)) {
    ...
    startmap->pg_to_up_acting_osds(pgid.pgid, &up, &up_primary, &acting, &acting_primary);

    const pg_pool_t* pp = startmap->get_pg_pool(pgid.pool());
    if (pp->has_flag(pg_pool_t::FLAG_EC_OVERWRITES) && store->get_type() != "bluestore") {
        clog->warn() << "pg " << pgid << " is at risk of silent data corruption: "
                     << "the pool allows ec overwrites but is not stored in "
                     << "bluestore, so deep scrubbing will not detect bitrot";
    }
    PeeringCtx rctx;
    create_pg_collection(rctx.transaction, pgid, pgid.get_split_bits(pp->get_pg_num()));
    ...
    pg->init(role, up, up_primary, acting, acting_primary, info->history, info->past_intervals, rctx.transaction);
...
bool OSD::maybe_wait_for_max_pg(const OSDMapRef& osdmap, spg_t pgid, bool is_mon_create)
{
    const auto max_pgs_per_osd = (cct->_conf.get_val<uint64_t>("mon_max_pg_per_osd") *
...
void OSD::resume_creating_pg()
{
    ...
    bool have_pending_creates = false;
    {
        ...
        have_pending_creates = (pending_creates_from_mon > 0 || !pending_creates_from_osd.empty());
    }
...
void OSD::_add_heartbeat_peer(int p)
{
    ...
    if (i == heartbeat_peers.end()) {
        ...
        dout(10) << "_add_heartbeat_peer: new peer osd." << p << " " << hi->con_back->get_peer_addr() << " "
                 << hi->con_front->get_peer_addr() << dendl;
    }
    else {
        hi = &i->second;
    }
...
void OSD::_remove_heartbeat_peer(int n)
{
    ...
    dout(20) << " removing heartbeat peer osd." << n << " " << q->second.con_back->get_peer_addr() << " "
             << (q->second.con_front ? q->second.con_front->get_peer_addr() : entity_addr_t()) << dendl;
...
void OSD::maybe_update_heartbeat_peers()
{
    ...
    if (!heartbeat_peers_need_update()) return;
    ...
    if (next >= 0) want.insert(next);
    int prev = get_osdmap()->get_previous_up_osd_before(whoami);
    if (prev >= 0 && prev != next) want.insert(prev);
    ...
    get_osdmap()->get_random_up_osds_by_subtree(whoami, subtree, limit, want, &want);
    ...
    // too few?
    for (int n = next; n >= 0;) {
        if ((int)heartbeat_peers.size() >= cct->_conf->osd_heartbeat_min_peers) break;
        ...
        if (n == next) break;   // came full circle; stop
    }
...
void OSD::handle_osd_ping(MOSDPing* m)
{
    if (superblock.cluster_fsid != m->fsid) {
        dout(20) << "handle_osd_ping from " << m->get_source_inst() << " bad fsid " << m->fsid
                 << " != " << superblock.cluster_fsid << dendl;
        m->put();
        return;
    }
    ...
    case MOSDPing::PING:
    {
        if (cct->_conf->osd_debug_drop_ping_probability > 0) {
            ...
            if (heartbeat_drop != debug_heartbeat_drops_remaining.end()) {
                if (heartbeat_drop->second == 0) {
                    ...
                    dout(5) << "Dropping heartbeat from " << from << ", " << heartbeat_drop->second
                            << " remaining to drop" << dendl;
                    break;
                }
            }
            else if (cct->_conf->osd_debug_drop_ping_probability > ((((double)(rand() % 100)) / 100.0))) {
                heartbeat_drop = debug_heartbeat_drops_remaining
                                     .insert(std::make_pair(from, cct->_conf->osd_debug_drop_ping_duration))
                                     .first;
                dout(5) << "Dropping heartbeat from " << from << ", " << heartbeat_drop->second << " remaining to drop"
                        << dendl;
                break;
            }
        }
        ...
        s->stamps->got_ping(m->up_from, mnow, m->mono_send_stamp, m->delta_ub, &sender_delta_ub);
        ...
        if (curmap->is_up(from)) {
            if (is_active()) {
                ConnectionRef cluster_con = service.get_con_osd_cluster(from, curmap->get_epoch());
                if (cluster_con) {
        ...
        if (i != heartbeat_peers.end()) {
            ...
            if (acked != i->second.ping_history.end()) {
                ...
                if (con == i->second.con_back) {
                    dout(25) << "handle_osd_ping got reply from osd." << from << " first_tx " << i->second.first_tx
                             << " last_tx " << i->second.last_tx << " last_rx_back " << i->second.last_rx_back << " -> "
                             << now << " last_rx_front " << i->second.last_rx_front << dendl;
                    ...
                    dout(25) << "handle_osd_ping got reply from osd." << from << " first_tx " << i->second.first_tx
                             << " last_tx " << i->second.last_tx << " last_rx_back " << i->second.last_rx_back
                             << " last_rx_front " << i->second.last_rx_front << " -> " << now << dendl;
                    i->second.last_rx_front = now;
                    ceph_assert(unacknowledged > 0);
                    --unacknowledged;
                }
                ...
                if (unacknowledged == 0) {
                    ...
                    dout(25) << "handle_osd_ping got all replies from osd." << from << " , erase pending ping(sent at "
                             << m->ping_stamp << ")"
                             << " and older pending ping(s)" << dendl;
...
#define ROUND_S_TO_USEC(sec) (uint32_t)((sec)*1000 * 1000 + 0.5)
                    ...
                    if (back_pingtime < i->second.hb_min_back) i->second.hb_min_back = back_pingtime;
                    if (back_pingtime > i->second.hb_max_back) i->second.hb_max_back = back_pingtime;
                    uint32_t front_pingtime = ROUND_S_TO_USEC(i->second.last_rx_front - m->ping_stamp);
                    i->second.hb_total_front += front_pingtime;
                    if (front_pingtime < i->second.hb_min_front) i->second.hb_min_front = front_pingtime;
                    if (front_pingtime > i->second.hb_max_front) i->second.hb_max_front = front_pingtime;

                    ceph_assert(i->second.hb_interval_start != utime_t());
                    if (i->second.hb_interval_start == utime_t()) i->second.hb_interval_start = now;
                    ...
                    if (now - i->second.hb_interval_start >= utime_t(hb_avg_time_period, 0)) {
...

                        {
                            ...
                            for (int32_t k = size - 1; k >= 0; --k) {
                                ...
                                if (i->second.hb_back_min[index] < min) min = i->second.hb_back_min[index];
                                if (i->second.hb_back_max[index] > max) max = i->second.hb_back_max[index];
                            ...
                            if (i->second.con_front != NULL) {
                                ...
                                for (int32_t k = size - 1; k >= 0; --k) {
                                    ...
                                    if (i->second.hb_front_min[index] < min) min = i->second.hb_front_min[index];
                                    if (i->second.hb_front_max[index] > max) max = i->second.hb_front_max[index];
                                    if (count == 1 || count == 5 || count == 15) {
                        ...
                        std::lock_guard l(service.stat_lock);
                        service.osd_stat.hb_pingtime[from].back_last = back_pingtime;
                        if (i->second.con_front != NULL) service.osd_stat.hb_pingtime[from].front_last = front_pingtime;
                    }
                ...
                if (i->second.is_healthy(now)) {
                    ...
                    if (failure_pending_entry != failure_pending.end()) {
                        dout(10) << "handle_osd_ping canceling in-flight "
                                 << "failure report for osd." << from << dendl;
                        send_still_alive(curmap->get_epoch(), from, failure_pending_entry->second.second);
        ...
        if (m->map_epoch && curmap->is_up(from)) {
            if (is_active()) {
                ConnectionRef cluster_con = service.get_con_osd_cluster(from, curmap->get_epoch());
                if (cluster_con) {
                    service.maybe_share_map(cluster_con.get(), curmap, m->map_epoch);
        ...
        s->stamps->got_ping_reply(mnow, m->mono_send_stamp, m->delta_ub);
        dout(20) << __func__ << " new stamps " << *s->stamps << dendl;
    } break;
...
void OSD::heartbeat_check()
{
    ...
    for (map<int, HeartbeatInfo>::iterator p = heartbeat_peers.begin(); p != heartbeat_peers.end(); ++p) {
        ...
        dout(25) << "heartbeat_check osd." << p->first << " first_tx " << p->second.first_tx << " last_tx "
                 << p->second.last_tx << " last_rx_back " << p->second.last_rx_back << " last_rx_front "
                 << p->second.last_rx_front << dendl;
        if (p->second.is_unhealthy(now)) {
            ...
            if (p->second.last_rx_back == utime_t() || p->second.last_rx_front == utime_t()) {
                derr << "heartbeat_check: no reply from " << p->second.con_front->get_peer_addr().get_sockaddr()
                     << " osd." << p->first << " ever on either front or back, first ping sent " << p->second.first_tx
                ...
                derr << "heartbeat_check: no reply from " << p->second.con_front->get_peer_addr().get_sockaddr()
                     << " osd." << p->first << " since back " << p->second.last_rx_back << " front "
                     << p->second.last_rx_front << " (oldest deadline " << oldest_deadline << ")" << dendl;
                // fail
...
void OSD::heartbeat()
{
    ...
    for (map<int, HeartbeatInfo>::iterator i = heartbeat_peers.begin(); i != heartbeat_peers.end(); ++i) {
        ...
        if (i->second.first_tx == utime_t()) i->second.first_tx = now;
...
bool OSD::heartbeat_reset(Connection* con)
{
    ...
    if (s) {
        ...
        if (p != heartbeat_peers.end() && (p->second.con_back == con || p->second.con_front == con)) {
            dout(10) << "heartbeat_reset failed hb con " << con << " for osd." << p->second.peer << ", reopening"
                     << dendl;
...
void OSD::tick()
{
    ...
    while (!osd_markdown_log.empty() && osd_markdown_log.front() + grace < now) osd_markdown_log.pop_front();
    ...
    // scrub purged_snaps every deep scrub interval
    {
        ...
        next += cct->_conf->osd_scrub_min_interval * cct->_conf->osd_scrub_interval_randomize_ratio * r;
...
void OSD::tick_without_osd_lock()
{
    ...
    if (is_active() || is_waiting_for_healthy()) {
        ...
        // mon report?
        utime_t now = ceph_clock_now();
        if (service.need_fullness_update() || now - last_mon_report > cct->_conf->osd_mon_report_interval) {
            last_mon_report = now;
            send_full_update();
            send_failures();
        }
...
void TestOpsSocketHook::test_ops(OSDService* service, ObjectStore* store, std::string_view command,
                                 const cmdmap_t& cmdmap, ostream& ss)
{
    ...
    if (command == "setomapval" || command == "rmomapkey" || command == "setomapheader" || command == "getomap" ||
        command == "truncobj" || command == "injectmdataerr" || command == "injectdataerr") {
        pg_t rawpg;
        int64_t pool;
        OSDMapRef curmap = service->get_osdmap();
        int r = -1;
        ...
        if (pool < 0 && isdigit(poolstr[0])) pool = atoll(poolstr.c_str());
        if (pool < 0) {
        ...
        if (command == "setomapval") {
            ...
            if (!ch) {
                ...
                if (r >= 0) {
                    ...
                    for (map<string, bufferlist>::iterator it = keyvals.begin(); it != keyvals.end(); ++it)
                        ss << " key=" << (*it).first << " val=" << string((*it).second.c_str(), (*it).second.length());
                }
                else {
                    ss << "error=" << r;
                }
    ...
    if (command == "set_recovery_delay") {
        ...
        ostringstream oss;
        oss << delay;
        int r = service->cct->_conf.set_val("osd_recovery_delay_start", oss.str().c_str());
        if (r != 0) {
            ss << "set_recovery_delay: error setting "
               << "osd_recovery_delay_start to '" << delay << "': error " << r;
            return;
        }
...
{
    if (con->get_peer_type() != CEPH_ENTITY_TYPE_MON && con->get_peer_type() != CEPH_ENTITY_TYPE_MGR) {
        if (auto s = ceph::ref_cast<Session>(con->get_priv()); !s) {
            s = ceph::make_ref<Session>(cct, con);
            con->set_priv(s);
            dout(10) << " new session (outgoing) " << s << " con=" << s->con << " addr=" << s->con->get_peer_addr()
                     << dendl;
...
void OSD::ms_handle_fast_accept(Connection* con)
{
    if (con->get_peer_type() != CEPH_ENTITY_TYPE_MON && con->get_peer_type() != CEPH_ENTITY_TYPE_MGR) {
        if (auto s = ceph::ref_cast<Session>(con->get_priv()); !s) {
            s = ceph::make_ref<Session>(cct, con);
            con->set_priv(s);
            dout(10) << "new session (incoming)" << s << " con=" << con << " addr=" << con->get_peer_addr()
...
{
    OSD* osd;
    explicit CB_OSD_GetVersion(OSD* o)
        : osd(o)
    {}
    void operator()(boost::system::error_code ec, version_t newest, version_t oldest)
    {
        if (!ec) osd->_got_mon_epochs(oldest, newest);
    }
};
...
void OSD::start_boot()
{
    if (!_is_healthy()) {
        ...
        if (!is_waiting_for_healthy()) start_waiting_for_healthy();
...
void OSD::_preboot(epoch_t oldest, epoch_t newest)
{
    ...
    if (osdmap->get_epoch() == 0) {
        ...
        derr << "osdmap SORTBITWISE OSDMap flag is NOT set; please set it" << dendl;
    }
        ...
        dout(10) << __func__ << " purged_snaps_last " << superblock.purged_snaps_last << " < newest_map "
        ...
        // make progress.
        boot_finisher.queue(new LambdaContext([this](int r) {
...
void OSD::_get_purged_snaps()
{
    ...
    dout(10) << __func__ << " purged_snaps_last " << superblock.purged_snaps_last << ", newest_map "
             << superblock.current_epoch << dendl;
    MMonGetPurgedSnaps* m = new MMonGetPurgedSnaps(superblock.purged_snaps_last + 1, superblock.current_epoch + 1);
    monc->send_mon_message(m);
}
...
void OSD::handle_get_purged_snaps_reply(MMonGetPurgedSnapsReply* m)
{
    ...
    if (!is_preboot() || m->last < superblock.purged_snaps_last) {
        ...
        SnapMapper::record_purged_snaps(cct, osdriver, osdriver.get_transaction(&t), m->purged_snaps);
    }
...
{
    if (!service.need_fullness_update()) return;
    unsigned state = 0;
    if (service.is_full()) {
        state = CEPH_OSD_FULL;
    }
    ...
    if (is_waiting_for_healthy()) {
        ...
        if (osd_markdown_log.empty()) {
            dout(5) << __func__ << " force returning true since last markdown"
                    << " was " << cct->_conf->osd_max_markdown_period << "s ago" << dendl;
            return true;
        }
        std::lock_guard l(heartbeat_lock);
        int num = 0, up = 0;
        for (map<int, HeartbeatInfo>::iterator p = heartbeat_peers.begin(); p != heartbeat_peers.end(); ++p) {
            if (p->second.is_healthy(now)) ++up;
            ++num;
        }
...
void OSD::_send_boot()
{
    ...
    dout(20) << " initial client_addrs " << client_addrs << ", cluster_addrs " << cluster_addrs << ", hb_back_addrs "
             << hb_back_addrs << ", hb_front_addrs " << hb_front_addrs << dendl;
    ...
    MOSDBoot* mboot = new MOSDBoot(superblock,
                                   ...
                                   cluster_addrs,
                                   CEPH_FEATURES_ALL);
    dout(10) << " final client_addrs " << client_addrs << ", cluster_addrs " << cluster_addrs << ", hb_back_addrs "
             << hb_back_addrs << ", hb_front_addrs " << hb_front_addrs << dendl;
...
void OSD::_collect_metadata(map<string, string>* pm)
{
    ...
    (*pm)["front_iface"] = pick_iface(cct, client_messenger->get_myaddrs().front().get_sockaddr_storage());
    (*pm)["back_iface"] = pick_iface(cct, cluster_messenger->get_myaddrs().front().get_sockaddr_storage());
    ...
    if (numa_node >= 0) {
        ...
        (*pm)["numa_node_cpus"] = cpu_set_to_str_list(numa_cpu_set_size, &numa_cpu_set);
    }
...
void OSD::queue_want_up_thru(epoch_t want)
{
    ...
    if (want > up_thru_wanted) {
        ...
        dout(10) << "queue_want_up_thru want " << want << " <= queued " << up_thru_wanted << ", currently " << cur
                 << dendl;
    }
}
...
void OSD::send_alive()
{
    ...
    const auto osdmap = get_osdmap();
    if (!osdmap->exists(whoami)) return;
...
{
    dout(10) << __func__ << " " << first << ".." << last << ", previously requested " << requested_full_first << ".."
...
void OSD::got_full_map(epoch_t e)
{
    ...
    if (e < requested_full_first) {
        dout(10) << __func__ << " " << e << ", requested " << requested_full_first << ".." << requested_full_last
                 << ", ignoring" << dendl;
        return;
    }
...
{
    MOSDFailure* m = new MOSDFailure(monc->get_fsid(), osd, addrs, 0, epoch, MOSDFailure::FLAG_ALIVE);
    monc->send_mon_message(m);
}
...
void OSD::send_beacon(const ceph::coarse_mono_clock::time_point& now)
{
    ...
    if (monmap.epoch > 0 && monmap.get_required_features().contains_all(ceph::features::mon::FEATURE_LUMINOUS)) {
...
void OSD::scrub_purged_snaps()
{
    dout(10) << __func__ << dendl;
    ceph_assert(ceph_mutex_is_locked(osd_lock));
    SnapMapper::Scrubber s(cct, store.get(), service.meta_ch, make_snapmapper_oid(), make_purged_snaps_oid());
...
void OSD::probe_smart(const string& only_devid, ostream& ss)
{
    ...
    for (auto dev : devnames) {
        ...
        if (block_device_get_metrics(dev, smart_timeout, &smart_json)) {
...
bool OSD::heartbeat_dispatch(Message* m)
{
    ...
    case MSG_OSD_PING: handle_osd_ping(static_cast<MOSDPing*>(m)); break;

    default: dout(0) << "dropping unexpected message " << *m << " from " << m->get_source_inst() << dendl; m->put();
    }
...
void OSDService::maybe_share_map(Connection* con, const OSDMapRef& osdmap, epoch_t peer_epoch_lb)
{
    ...
    if (peer_epoch_lb > session->last_sent_epoch) {
        dout(10) << __func__ << " con " << con << " " << con->get_peer_addr() << " map epoch "
                 << session->last_sent_epoch << " -> " << peer_epoch_lb << " (as per caller)" << dendl;
    ...
    if (session->last_sent_epoch < last_sent_epoch) {
        dout(10) << __func__ << " con " << con << " " << con->get_peer_addr() << " map epoch "
                 << session->last_sent_epoch << " -> " << last_sent_epoch << " (shared)" << dendl;
...
void OSD::ms_fast_dispatch(Message* m)
{
    ...
    case CEPH_MSG_PING:
        ...
        m->put();
        return;
    case MSG_OSD_FORCE_RECOVERY: handle_fast_force_recovery(static_cast<MOSDForceRecovery*>(m)); return;
    case MSG_OSD_SCRUB2: handle_fast_scrub(static_cast<MOSDScrub2*>(m)); return;
    case MSG_OSD_PG_CREATE2: return handle_fast_pg_create(static_cast<MOSDPGCreate2*>(m));
    case MSG_OSD_PG_NOTIFY: return handle_fast_pg_notify(static_cast<MOSDPGNotify*>(m));
    ...
    if (m->trace) op->osd_trace.init("osd op", &trace_endpoint, &m->trace);
    ...
    if (m->get_connection()->has_features(CEPH_FEATUREMASK_RESEND_ON_SPLIT) || m->get_type() != CEPH_MSG_OSD_OP) {
        // queue it directly
...
int OSD::ms_handle_fast_authentication(Connection* con)
{
    ...
    if (!s) {
        ...
        dout(10) << __func__ << " new session " << s << " con " << s->con << " entity " << s->entity_name << " addr "
                 << con->get_peer_addrs() << dendl;
    }
    else {
        dout(10) << __func__ << " existing session " << s << " con " << s->con << " entity " << s->entity_name
    ...
    if (caps_info.allow_all) {
        ...
        try {
            ...
            dout(10) << __func__ << " session " << s << " " << s->entity_name << " failed to decode caps string"
                     << dendl;
            ret = -EACCES;
        }
        if (!ret) {
            ...
            if (success) {
                dout(10) << __func__ << " session " << s << " " << s->entity_name << " has caps " << s->caps << " '"
                         << str << "'" << dendl;
                ret = 1;
            }
            else {
                dout(10) << __func__ << " session " << s << " " << s->entity_name << " failed to parse caps '" << str
                         << "'" << dendl;
                ret = -EACCES;
            }
        }
...
void OSD::_dispatch(Message* m)
{
    ...
    switch (m->get_type()) {
        ...
        // map and replication
    case CEPH_MSG_OSD_MAP: handle_osd_map(static_cast<MOSDMap*>(m)); break;
    case MSG_MON_GET_PURGED_SNAPS_REPLY:
        handle_get_purged_snaps_reply(static_cast<MMonGetPurgedSnapsReply*>(m));
        break;

        // osd
    case MSG_COMMAND: handle_command(static_cast<MCommand*>(m)); return;
    }
}
    ...
    for (auto pgid : m->scrub_pgs) {
        enqueue_peering_evt(pgid,
                            PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                                m->epoch, m->epoch, PeeringState::RequestScrub(m->deep, m->repair))));
    }
    m->put();
}
...
void OSD::sched_scrub()
{
    ...
    if (auto blocked_pgs = scrub_scheduler.get_blocked_pgs_count(); blocked_pgs > 0) {
        ...
        dout(10) << fmt::format(
                        "{}: PGs are blocked while scrubbing due to locked objects ({} PGs)", __func__, blocked_pgs)
                 << dendl;
    }
    ...
    dout(20) << "sched_scrub done (" << ScrubQueue::attempt_res_text(was_started) << ")" << dendl;
}

Scrub::schedule_result_t OSDService::initiate_a_scrub(spg_t pgid, bool allow_requested_repair_only)
{
    ...
    if (allow_requested_repair_only && !pg->get_planned_scrub().must_repair) {
        pg->unlock();
        dout(10) << __func__ << " skip " << pgid << " because repairing is not explicitly requested on it" << dendl;
...
vector<DaemonHealthMetric> OSD::get_health_metrics()
{
    vector<DaemonHealthMetric> metrics;
    {
        ...
        bool log_aggregated_slow_op = cct->_conf.get_val<bool>("osd_aggregated_slow_ops_logging");
        auto count_slow_ops = [&](TrackedOp& op) {
            if (op.get_initiated() < too_old) {
                stringstream ss;
                ss << "slow request " << op.get_desc() << " initiated " << op.get_initiated() << " currently "
                   << op.state_string();
        ...
        if (op_tracker.visit_ops_in_flight(&oldest_secs, count_slow_ops)) {
            if (slow) {
                ...
                if (log_aggregated_slow_op && slow_op_types.size() > 0) {
                    stringstream ss;
                    ss << slow << " slow requests (by type [ ";
                    for (const auto& [op_type, count] : slow_op_types) {
                        ss << "'" << OpRequest::get_state_string(op_type) << "' : " << count << " ";
                    }
                    auto slow_pool_it = std::max_element(
                        slow_op_pools.begin(),
                        slow_op_pools.end(),
                    ...
                    if (osdmap->get_pools().find(slow_pool_it->first) != osdmap->get_pools().end()) {
                        ...
                        ss << "] most affected pool [ '" << pool_name << "' : " << slow_pool_it->second << " ])";
                    }
                    else {
                        ss << "])";
                    }
...
void OSD::osdmap_subscribe(version_t epoch, bool force_request)
{
    ...
    if (latest_subscribed_epoch >= epoch && !force_request) return;

    latest_subscribed_epoch = std::max<uint64_t>(epoch, latest_subscribed_epoch);

    if (monc->sub_want_increment("osdmap", epoch, CEPH_SUBSCRIBE_ONETIME) || force_request) {
        monc->renew_subs();
    }
}
...
void OSD::trim_maps(epoch_t oldest, int nreceived, bool skip_maps)
{
    ...
    if (min <= superblock.oldest_map) return;
...
void OSD::handle_osd_map(MOSDMap* m)
{
    // wait for pgs to catch up
    {
        ...
        // up with.
        epoch_t max_lag = cct->_conf->osd_map_cache_size * m_osd_pg_epoch_max_lag_factor;
        ...
        if (osd_min > 0 && osdmap_epoch > max_lag && osdmap_epoch - max_lag > osd_min) {
            epoch_t need = osdmap_epoch - max_lag;
            dout(10) << __func__ << " waiting for pgs to catch up (need " << need << " max_lag " << max_lag << ")"
                     << dendl;
            for (auto shard : shards) {
                epoch_t min = shard->get_min_pg_epoch();
                if (need > min) {
                    dout(10) << __func__ << " waiting for pgs to consume " << need << " (shard " << shard->shard_id
                             << " min " << min << ", map cache is " << cct->_conf->osd_map_cache_size
                             << ", max_lag_factor " << m_osd_pg_epoch_max_lag_factor << ")" << dendl;
    ...
    if (session && !(session->entity_name.is_mon() || session->entity_name.is_osd())) {
        // not enough perms!
        dout(10) << "got osd map from Session " << session << " which we can't take maps from (not a mon or osd)"
                 << dendl;
        m->put();
        return;
    }
    ...
    if (!is_preboot()) service.objecter->handle_osd_map(m);

    epoch_t first = m->get_first();
    epoch_t last = m->get_last();
    dout(3) << "handle_osd_map epochs [" << first << "," << last << "], i have " << superblock.newest_map
            << ", src has [" << m->cluster_osdmap_trim_lower_bound << "," << m->newest_map << "]" << dendl;

    logger->inc(l_osd_map);
    logger->inc(l_osd_mape, last - first + 1);
    if (first <= superblock.newest_map) logger->inc(l_osd_mape_dup, superblock.newest_map - first + 1);
    ...
    if (superblock.cluster_osdmap_trim_lower_bound < m->cluster_osdmap_trim_lower_bound) {
        ...
        ceph_assert(superblock.cluster_osdmap_trim_lower_bound >= superblock.oldest_map);
    }
    ...
    for (epoch_t e = start; e <= last; e++) {
        ...
        if (p != m->incremental_maps.end()) {
            ...
            if ((inc.have_crc && o->get_crc() != inc.full_crc) || injected_failure) {
                dout(2) << "got incremental " << e << " but failed to encode full with correct crc; requesting"
                        << dendl;
    ...
    if (!m->maps.empty() && requested_full_first) {
        dout(10) << __func__ << " still missing full maps " << requested_full_first << ".." << requested_full_last
                 << dendl;
        rerequest_full_maps();
    }
    ...
    if (superblock.oldest_map) {
        ...
        trim_maps(m->cluster_osdmap_trim_lower_bound, last - first + 1, skip_maps);
        pg_num_history.prune(superblock.oldest_map);
    }

    if (!superblock.oldest_map || skip_maps) superblock.oldest_map = first;
    ...
    for (auto& i : added_maps) {
        if (!lastmap) {
            if (!(lastmap = service.try_get_map(i.first - 1))) {
                dout(10) << __func__ << " can't get previous map " << i.first - 1 << " probably first start of this osd"
                         << dendl;
                continue;
            }
        }
        ...
        for (auto& j : lastmap->get_pools()) {
            if (!i.second->have_pg_pool(j.first)) {
                ...
                if (lastmap->get_pg_pool(j.first)->is_erasure()) {
                    profile = lastmap->get_erasure_code_profile(lastmap->get_pg_pool(j.first)->erasure_code_profile);
                }
                encode(profile, bl);
                t.write(coll_t::meta(), obj, 0, bl.length(), bl);
            }
            else if (unsigned new_pg_num = i.second->get_pg_num(j.first); new_pg_num != j.second.get_pg_num()) {
                dout(10) << __func__ << " recording pool " << j.first << " pg_num " << j.second.get_pg_num() << " -> "
                         << new_pg_num << dendl;
        ...
        for (auto& j : i.second->get_pools()) {
            if (!lastmap->have_pg_pool(j.first)) {
                dout(10) << __func__ << " recording new pool " << j.first << " pg_num " << j.second.get_pg_num()
                         << dendl;
                pg_num_history.log_pg_num_change(i.first, j.first, j.second.get_pg_num());
            }
        }
        lastmap = i.second;
    }
    ...
    if (superblock.purged_snaps_last == start - 1) {
        OSDriver osdriver{store.get(), service.meta_ch, make_purged_snaps_oid()};
        SnapMapper::record_purged_snaps(cct, osdriver, osdriver.get_transaction(&t), purged_snaps);
        superblock.purged_snaps_last = last;
    }
    else {
        dout(10) << __func__ << " superblock purged_snaps_last is " << superblock.purged_snaps_last
...
void OSD::_committed_osd_maps(epoch_t first, epoch_t last, MOSDMap* m)
{
    ...
    for (epoch_t cur = first; cur <= last; cur++) {
        dout(10) << " advance to epoch " << cur << " (<= last " << last << " <= newest_map " << superblock.newest_map
                 << ")" << dendl;
        ...
        for (set<int>::iterator p = old.begin(); p != old.end(); ++p) {
            if (*p != whoami && osdmap->is_up(*p) &&   // in old map
            ...
            else if (*p != whoami && osdmap->is_down(*p) && newmap->is_up(*p)) {
                note_up_osd(*p);
            }
        }
        ...
        if (!up_epoch && osdmap->is_up(whoami) && osdmap->get_addrs(whoami) == client_messenger->get_myaddrs()) {
    ...
    if (osdmap->is_up(whoami) && osdmap->get_addrs(whoami).legacy_equals(client_messenger->get_myaddrs()) &&
    ...
    if (osdmap->get_epoch() > 0 && is_active()) {
        if (!osdmap->exists(whoami)) {
        ...
        else if (!osdmap->is_up(whoami) || !osdmap->get_addrs(whoami).legacy_equals(client_messenger->get_myaddrs()) ||
                 !osdmap->get_cluster_addrs(whoami).legacy_equals(cluster_messenger->get_myaddrs()) ||
                 !osdmap->get_hb_back_addrs(whoami).legacy_equals(hb_back_server_messenger->get_myaddrs()) ||
                 !osdmap->get_hb_front_addrs(whoami).legacy_equals(hb_front_server_messenger->get_myaddrs())) {
            if (!osdmap->is_up(whoami)) {
                if (service.is_preparing_to_stop() || service.is_stopping()) {
                    service.got_stop_ack();
                }
                else {
                    clog->warn() << "Monitor daemon marked osd." << whoami
                                 << " down, "
                                    "but it is still running";
                    clog->debug() << "map e" << osdmap->get_epoch() << " wrongly marked me down at e"
                                  << osdmap->get_down_at(whoami);
                }
                if (monc->monmap.min_mon_release >= ceph_release_t::octopus) {
                    // note that this is best-effort...
                    monc->send_mon_message(new MOSDMarkMeDead(monc->get_fsid(), whoami, osdmap->get_epoch()));
                }
            }
            else if (!osdmap->get_addrs(whoami).legacy_equals(client_messenger->get_myaddrs())) {
                clog->error() << "map e" << osdmap->get_epoch() << " had wrong client addr ("
                              << osdmap->get_addrs(whoami) << " != my " << client_messenger->get_myaddrs() << ")";
            }
            else if (!osdmap->get_cluster_addrs(whoami).legacy_equals(cluster_messenger->get_myaddrs())) {
                clog->error() << "map e" << osdmap->get_epoch() << " had wrong cluster addr ("
                              << osdmap->get_cluster_addrs(whoami) << " != my " << cluster_messenger->get_myaddrs()
                              << ")";
            }
            else if (!osdmap->get_hb_back_addrs(whoami).legacy_equals(hb_back_server_messenger->get_myaddrs())) {
                clog->error() << "map e" << osdmap->get_epoch() << " had wrong heartbeat back addr ("
                              << osdmap->get_hb_back_addrs(whoami) << " != my "
                              << hb_back_server_messenger->get_myaddrs() << ")";
            }
            else if (!osdmap->get_hb_front_addrs(whoami).legacy_equals(hb_front_server_messenger->get_myaddrs())) {
                clog->error() << "map e" << osdmap->get_epoch() << " had wrong heartbeat front addr ("
                              << osdmap->get_hb_front_addrs(whoami) << " != my "
            ...
            if (!service.is_stopping()) {
                ...
                if ((int)osd_markdown_log.size() > cct->_conf->osd_max_markdown_count) {
                    derr << __func__ << " marked down " << osd_markdown_log.size() << " > osd_max_markdown_count "
                         << cct->_conf->osd_max_markdown_count << " in last " << grace << " seconds, shutting down"
                         << dendl;
                    do_restart = false;
                    do_shutdown = true;
                }
    ...
    if (is_active() || is_waiting_for_healthy()) maybe_update_heartbeat_peers();
...
void OSD::check_osdmap_features()
{
    ...
    }
    {
        ...
        if ((p.features_required & mask) != features) {
            dout(0) << "crush map has features " << features << " was " << p.features_required
    ...
    if (osdmap->require_osd_release != last_require_osd_release) {
        ...
        store->write_meta("require_osd_release", stringify((int)osdmap->require_osd_release));
...
struct C_FinishSplits : public Context
{
    ...
    C_FinishSplits(OSD* osd, const set<PGRef>& in)
        ...
        , pgs(in)
    {}
    void finish(int r) override { osd->_finish_splits(pgs); }
};
...
void OSD::_finish_splits(set<PGRef>& pgs)
{
    ...
    if (is_stopping()) return;
    for (set<PGRef>::iterator i = pgs.begin(); i != pgs.end(); ++i) {
        PG* pg = i->get();
...
bool OSD::add_merge_waiter(OSDMapRef nextmap, spg_t target, PGRef src, unsigned need)
{
    std::lock_guard l(merge_lock);
    auto& p = merge_waiters[nextmap->get_epoch()][target];
    p[src->pg_id] = src;
    dout(10) << __func__ << " added merge_waiter " << src->pg_id << " for " << target << ", have " << p.size() << "/"
             << need << dendl;
    return p.size() == need;
}
...
bool OSD::advance_pg(epoch_t osd_epoch, PG* pg, ThreadPool::TPHandle& handle, PeeringCtx& rctx)
{
    ...
    unsigned old_pg_num = lastmap->have_pg_pool(pg->pg_id.pool()) ? lastmap->get_pg_num(pg->pg_id.pool()) : 0;
    for (epoch_t next_epoch = pg->get_osdmap_epoch() + 1; next_epoch <= osd_epoch; ++next_epoch) {
        ...
        unsigned new_pg_num =
            (old_pg_num && nextmap->have_pg_pool(pg->pg_id.pool())) ? nextmap->get_pg_num(pg->pg_id.pool()) : 0;
            ...
            if (nextmap->have_pg_pool(pg->pg_id.pool())) {
                ...
                if (pg->pg_id.is_merge_source(old_pg_num, new_pg_num, &parent)) {
                    ...
                    if (!sources.empty()) {
                        ...
                        pg->merge_from(
                            sources, rctx, split_bits, nextmap->get_pg_pool(pg->pg_id.pool())->last_pg_merge_meta);
        ...
        int up_primary, acting_primary;
        nextmap->pg_to_up_acting_osds(pg->pg_id.pgid, &newup, &up_primary, &newacting, &acting_primary);
...
void OSD::consume_map()
{
...

    {
        ...
        std::lock_guard l(pending_creates_lock);
        for (auto pg = pending_creates_from_osd.begin(); pg != pending_creates_from_osd.end();) {
    ...
    for (auto pgid : pgids) {
        enqueue_peering_evt(
            pgid,
            PGPeeringEventRef(std::make_shared<PGPeeringEvent>(osdmap->get_epoch(), osdmap->get_epoch(), NullEvt())));
    }
...
bool OSD::require_mon_peer(const Message* m)
{
    if (!m->get_connection()->peer_is_mon()) {
        dout(0) << "require_mon_peer received from non-mon " << m->get_connection()->get_peer_addr() << " " << *m
                << dendl;
        return false;
    }
    return true;
...
{
    if (!m->get_connection()->peer_is_mon() && !m->get_connection()->peer_is_mgr()) {
        dout(0) << "require_mon_or_mgr_peer received from non-mon, non-mgr " << m->get_connection()->get_peer_addr()
                << " " << *m << dendl;
        return false;
    }
    return true;
...
bool OSD::require_osd_peer(const Message* m)
{
    if (!m->get_connection()->peer_is_osd()) {
        dout(0) << "require_osd_peer received from non-osd " << m->get_connection()->get_peer_addr() << " " << *m
                << dendl;
        return false;
    }
    return true;
...
void OSD::split_pgs(PG* parent, const set<spg_t>& childpgids, set<PGRef>* out_pgs, OSDMapRef curmap, OSDMapRef nextmap,
                    PeeringCtx& rctx)
{
    ...
    for (set<spg_t>::const_iterator i = childpgids.begin(); i != childpgids.end(); ++i, ++stat_iter) {
        ...
        parent->split_colls(*i, split_bits, i->ps(), &child->get_pgpool().info, rctx.transaction);
        parent->split_into(i->pgid, child, split_bits);
...
void OSD::dispatch_context(PeeringCtx& ctx, PG* pg, OSDMapRef curmap, ThreadPool::TPHandle* handle)
{
    if (!service.get_osdmap()->is_up(whoami)) {
        ...
        for (auto& [osd, ls] : ctx.message_map) {
            ...
            ConnectionRef con = service.get_con_osd_cluster(osd, curmap->get_epoch());
            if (!con) {
    ...
    if ((!ctx.transaction.empty() || ctx.transaction.has_contexts()) && pg) {
        int tr = store->queue_transaction(pg->ch, std::move(ctx.transaction), TrackedOpRef(), handle);
        ceph_assert(tr == 0);
    }
}
...
void OSD::handle_fast_pg_create(MOSDPGCreate2* m)
{
    ...
    for (auto& p : m->pgs) {
        ...
        if (q == m->pg_extra.end()) {
            clog->error() << __func__ << " " << pgid << " e" << created << "@" << created_stamp
                          << " with no history or past_intervals"
                          << ", this should be impossible after octopus.  Ignoring.";
        }
        else {
            dout(20) << __func__ << " " << pgid << " e" << created << "@" << created_stamp << " history "
                     << q->second.first << " pi " << q->second.second << dendl;
            if (!q->second.second.empty() && m->epoch < q->second.second.get_bounds().second) {
                clog->error() << "got pg_create on " << pgid << " epoch " << m->epoch
                              << " and unmatched past_intervals " << q->second.second << " (history " << q->second.first
                              << ")";
            }
            else {
                enqueue_peering_evt(pgid,
                                    PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                                        ...
                                        NullEvt(),
                                        true,
                                        new PGCreateInfo(pgid, m->epoch, q->second.first, q->second.second, true))));
            }
        }
    }
    ...
    for (auto& p : m->get_pg_list()) {
        ...
        enqueue_peering_evt(pgid,
                            PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                                p.epoch_sent,
                                p.query_epoch,
                                MNotifyRec(pgid, pg_shard_t(from, p.from), p, m->get_connection()->get_features()),
                                true,
                                new PGCreateInfo(pgid, p.query_epoch, p.info.history, p.past_intervals, false))));
    }
    m->put();
}
...
void OSD::handle_fast_pg_info(MOSDPGInfo* m)
{
    ...
    for (auto& p : m->pg_list) {
        enqueue_peering_evt(
            ...
            PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                p.epoch_sent, p.query_epoch, MInfoRec(pg_shard_t(from, p.from), p.info, p.epoch_sent))));
    }
    m->put();
}
...
void OSD::handle_fast_pg_remove(MOSDPGRemove* m)
{
    ...
    for (auto& pgid : m->pg_list) {
        enqueue_peering_evt(pgid,
                            PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                                m->get_epoch(), m->get_epoch(), PeeringState::DeleteStart())));
    }
    m->put();
}
    ...
    for (auto pgid : m->forced_pgs) {
        if (m->options & OFR_BACKFILL) {
            if (m->options & OFR_CANCEL) {
                enqueue_peering_evt(pgid,
                                    PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                                        epoch, epoch, PeeringState::UnsetForceBackfill())));
            }
            else {
                enqueue_peering_evt(pgid,
                                    PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                                        epoch, epoch, PeeringState::SetForceBackfill())));
            }
        }
            ...
            if (m->options & OFR_CANCEL) {
                enqueue_peering_evt(pgid,
                                    PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                                        epoch, epoch, PeeringState::UnsetForceRecovery())));
            }
            else {
                enqueue_peering_evt(pgid,
                                    PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                                        epoch, epoch, PeeringState::SetForceRecovery())));
            }
        }
    }
    m->put();
...
void OSD::handle_pg_query_nopg(const MQuery& q)
{
    ...
    OSDMapRef osdmap = get_osdmap();
    if (!osdmap->have_pg_pool(pgid.pool())) return;
    ...
    if (con) {
        Message* m;
        if (q.query.type == pg_query_t::LOG || q.query.type == pg_query_t::FULLLOG) {
            m = new MOSDPGLog(q.query.from, q.query.to, osdmap->get_epoch(), empty, q.query.epoch_sent);
        }
        else {
            pg_notify_t notify{
                q.query.from, q.query.to, q.query.epoch_sent, osdmap->get_epoch(), empty, PastIntervals()};
            m = new MOSDPGNotify2(spg_t{pgid.pgid, q.query.from}, std::move(notify));
        }
...
void OSDService::queue_check_readable(spg_t spgid, epoch_t lpr, ceph::signedspan delay)
{
    if (delay == ceph::signedspan::zero()) {
        osd->enqueue_peering_evt(
            spgid, PGPeeringEventRef(std::make_shared<PGPeeringEvent>(lpr, lpr, PeeringState::CheckReadable())));
    }
    else {
        mono_timer.add_event(delay, [this, spgid, lpr]() { queue_check_readable(spgid, lpr); });
    }
}
...
void OSDService::_maybe_queue_recovery()
{
    ...
    while (!awaiting_throttle.empty() && _recover_now(&available_pushes)) {
        ...
        dout(10) << __func__ << " starting " << to_start << ", recovery_ops_reserved " << recovery_ops_reserved
...
{
    if (available_pushes) *available_pushes = 0;
    ...
    if (max <= recovery_ops_active + recovery_ops_reserved) {
        dout(15) << __func__ << " active " << recovery_ops_active << " + reserved " << recovery_ops_reserved
                 << " >= max " << max << dendl;
        return false;
    }

    if (available_pushes) *available_pushes = max - recovery_ops_active - recovery_ops_reserved;
...
unsigned OSDService::get_target_pg_log_entries() const
{
    ...
    if (num_pgs > 0 && target > 0) {
        ...
        // will work out.
        return std::max<unsigned>(std::min<unsigned>(target / num_pgs, cct->_conf->osd_max_pg_log_entries),
...
void OSD::do_recovery(PG* pg, epoch_t queued, uint64_t reserved_pushes, int priority, ThreadPool::TPHandle& handle)
{
    uint64_t started = 0;
    ...
    float recovery_sleep = get_osd_recovery_sleep();
    {
        ...
        if (recovery_sleep > 0 && service.recovery_needs_sleep) {
            PGRef pgref(pg);
            auto recovery_requeue_callback = new LambdaContext([this, pgref, queued, reserved_pushes, priority](int r) {
            ...
            service.sleep_timer.add_event_at(service.recovery_schedule_time, recovery_requeue_callback);
...
void OSDService::start_recovery_op(PG* pg, const hobject_t& soid)
{
    ...
    dout(10) << "start_recovery_op " << *pg << " " << soid << " (" << recovery_ops_active << "/"
...
{
    std::lock_guard l(recovery_lock);
    dout(10) << "finish_recovery_op " << *pg << " " << soid << " dequeue=" << dequeue << " (" << recovery_ops_active
             << "/" << osd->get_recovery_max_active() << " rops)" << dendl;
...
{
    std::lock_guard l(recovery_lock);
    dout(10) << __func__ << "(" << pushes << "), recovery_ops_reserved " << recovery_ops_reserved << " -> "
             << (recovery_ops_reserved - pushes) << dendl;
...
void OSD::enqueue_op(spg_t pg, OpRequestRef&& op, epoch_t epoch)
{
    ...
    dout(15) << "enqueue_op " << *op->get_req() << " prio " << priority << " type " << type << " cost " << cost
    ...
    enqueue_span->AddEvent(
        __func__, {{"priority", priority}, {"cost", cost}, {"epoch", epoch}, {"owner", owner}, {"type", type}});

    op->mark_queued_for_pg();
    logger->tinc(l_osd_op_before_queue_op_lat, latency);
    if (PGRecoveryMsg::is_recovery_msg(op)) {
        op_shardedwq.queue(
            OpSchedulerItem(unique_ptr<OpSchedulerItem::OpQueueable>(new PGRecoveryMsg(pg, std::move(op))),
                            cost,
                            priority,
                            stamp,
                            owner,
        ...
        op_shardedwq.queue(OpSchedulerItem(unique_ptr<OpSchedulerItem::OpQueueable>(new PGOpItem(pg, std::move(op))),
                                           cost,
                                           priority,
                                           stamp,
                                           owner,
...
void OSD::enqueue_peering_evt(spg_t pgid, PGPeeringEventRef evt)
{
    ...
    op_shardedwq.queue(OpSchedulerItem(unique_ptr<OpSchedulerItem::OpQueueable>(new PGPeeringItem(pgid, evt)),
                                       10,
...
void OSD::dequeue_op(PGRef pg, OpRequestRef op, ThreadPool::TPHandle& handle)
{
    ...
    dout(10) << "dequeue_op " << *op->get_req() << " prio " << m->get_priority() << " cost " << m->get_cost()
...
void OSD::dequeue_peering_evt(OSDShard* sdata, PG* pg, PGPeeringEventRef evt, ThreadPool::TPHandle& handle)
{
    ...
    else if (PeeringCtx rctx; advance_pg(curmap->get_epoch(), pg, handle, rctx)) {
...
void OSD::dequeue_delete(OSDShard* sdata, PG* pg, epoch_t e, ThreadPool::TPHandle& handle)
{
    dequeue_peering_evt(
        sdata, pg, PGPeeringEventRef(std::make_shared<PGPeeringEvent>(e, e, PeeringState::DeleteSome())), handle);
}
...
{
    static const char* KEYS[] = {"osd_max_backfills",
...
void OSD::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    std::lock_guard l{osd_lock};

    if (changed.count("osd_max_backfills") || changed.count("osd_recovery_max_active") ||
        changed.count("osd_recovery_max_active_hdd") || changed.count("osd_recovery_max_active_ssd")) {
        if (!maybe_override_options_for_qos(&changed) && changed.count("osd_max_backfills")) {
            // Scheduler is not "mclock". Fallback to earlier behavior
            service.local_reserver.set_max(cct->_conf->osd_max_backfills);
            service.remote_reserver.set_max(cct->_conf->osd_max_backfills);
        }
    }
    if (changed.count("osd_delete_sleep") || changed.count("osd_delete_sleep_hdd") ||
        changed.count("osd_delete_sleep_ssd") || changed.count("osd_delete_sleep_hybrid") ||
        changed.count("osd_snap_trim_sleep") || changed.count("osd_snap_trim_sleep_hdd") ||
        changed.count("osd_snap_trim_sleep_ssd") || changed.count("osd_snap_trim_sleep_hybrid") ||
        changed.count("osd_scrub_sleep") || changed.count("osd_recovery_sleep") ||
        changed.count("osd_recovery_sleep_hdd") || changed.count("osd_recovery_sleep_ssd") ||
    ...
    if (changed.count("osd_op_complaint_time") || changed.count("osd_op_log_threshold")) {
        op_tracker.set_complaint_and_threshold(cct->_conf->osd_op_complaint_time, cct->_conf->osd_op_log_threshold);
    }
    if (changed.count("osd_op_history_size") || changed.count("osd_op_history_duration")) {
        op_tracker.set_history_size_and_duration(cct->_conf->osd_op_history_size, cct->_conf->osd_op_history_duration);
    }
    if (changed.count("osd_op_history_slow_op_size") || changed.count("osd_op_history_slow_op_threshold")) {
        op_tracker.set_history_slow_op_size_and_threshold(cct->_conf->osd_op_history_slow_op_size,
                                                          cct->_conf->osd_op_history_slow_op_threshold);
    }
    ...
    if (changed.count("clog_to_monitors") || changed.count("clog_to_syslog") || changed.count("clog_to_syslog_level") ||
        changed.count("clog_to_syslog_facility") || changed.count("clog_to_graylog") ||
        changed.count("clog_to_graylog_host") || changed.count("clog_to_graylog_port") || changed.count("host") ||
        changed.count("fsid")) {
        update_log_config();
    }
    if (changed.count("osd_pg_epoch_max_lag_factor")) {
        m_osd_pg_epoch_max_lag_factor = conf.get_val<double>("osd_pg_epoch_max_lag_factor");
    }
...
void OSD::maybe_override_max_osd_capacity_for_qos()
{
    ...
    if ((cct->_conf.get_val<std::string>("osd_op_queue") == "mclock_scheduler") &&
        (cct->_conf.get_val<bool>("osd_mclock_skip_benchmark") == false) && (!unsupported_objstore_for_qos())) {
        std::string max_capacity_iops_config;
        bool force_run_benchmark = cct->_conf.get_val<bool>("osd_mclock_force_run_benchmark_on_init");
        ...
        if (!force_run_benchmark) {
            ...
            if (!val.has_value()) {
                derr << __func__ << " Unable to determine default value of " << max_capacity_iops_config << dendl;
            ...
            if (!force_run_benchmark && (default_iops != cur_iops)) {
                dout(1) << __func__ << std::fixed << std::setprecision(2) << " default_iops: " << default_iops
                        << " cur_iops: " << cur_iops << ". Skip OSD benchmark test." << dendl;
                return;
            }
        }
        ...
        if (ret != 0) {
            derr << __func__ << " osd bench err: " << ret << " osd bench errstr: " << ss.str() << dendl;
            return;
        }

        double rate = count / elapsed;
        iops = rate / bsize;
        dout(1) << __func__ << " osd bench result -" << std::fixed << std::setprecision(3)
                << " bandwidth (MiB/sec): " << rate / (1024 * 1024) << " iops: " << iops << " elapsed_sec: " << elapsed
                << dendl;

        // Get the threshold IOPS set for the underlying hdd/ssd.
        double threshold_iops = 0.0;
        if (store_is_rotational) {
            threshold_iops = cct->_conf.get_val<double>("osd_mclock_iops_capacity_threshold_hdd");
        }
        else {
            threshold_iops = cct->_conf.get_val<double>("osd_mclock_iops_capacity_threshold_ssd");
        }
        ...
        if (iops > threshold_iops) {
            clog->warn() << "OSD bench result of " << std::to_string(iops) << " IOPS exceeded the threshold limit of "
                         << std::to_string(threshold_iops) << " IOPS for osd." << std::to_string(whoami)
...
bool OSD::maybe_override_options_for_qos(const std::set<std::string>* changed)
{
    ...
    if (cct->_conf.get_val<std::string>("osd_op_queue") == "mclock_scheduler" && !unsupported_objstore_for_qos()) {
        ...
        if (changed != nullptr) {
            if (cct->_conf.get_val<bool>("osd_mclock_override_recovery_settings")) {
                if (changed->count("osd_max_backfills")) {
                    dout(1) << __func__ << " Set local and remote max backfills to " << cct->_conf->osd_max_backfills
                            << dendl;
                    service.local_reserver.set_max(cct->_conf->osd_max_backfills);
                    service.remote_reserver.set_max(cct->_conf->osd_max_backfills);
                }
                ...
                if (val != recovery_qos_defaults.end() && cct->_conf.get_val<uint64_t>(key) != val->second) {
                    static const std::vector<std::string> osds = {"osd", "osd." + std::to_string(whoami)};
                    ...
                    for (auto osd : osds) {
                        ...
                        dout(1) << __func__ << " Removing Key: " << key << " for " << osd << " from Mon db" << dendl;
                    ...
                    clog->warn() << "Change to " << key << " on osd." << std::to_string(whoami)
                                 << " did not take effect."
            ...
            for (auto opt : recovery_qos_defaults) {
                ...
                dout(1) << __func__ << " " << opt.first << " set to " << opt_val << dendl;
...
public:
    explicit MonCmdSetConfigOnFinish(OSD* o, CephContext* cct, const std::string& k, const std::string& v, const bool s)
        : osd(o)
        , cct(cct)
        , key(k)
        , val(v)
...
void OSD::mon_cmd_set_config(const std::string& key, const std::string& val)
{
    ...
    static const std::vector<std::string> shard_option = {"osd_mclock_max_capacity_iops_hdd",
                                                          "osd_mclock_max_capacity_iops_ssd"};
    const bool update_shard = std::find(shard_option.begin(), shard_option.end(), key) != shard_option.end();

    auto on_finish = new MonCmdSetConfigOnFinish(this, cct, key, val, update_shard);
...
bool OSD::unsupported_objstore_for_qos()
{
    ...
    return std::find(unsupported_objstores.begin(), unsupported_objstores.end(), store->get_type()) !=
...
void OSD::check_config()
{
    ...
    if (cct->_conf->osd_map_cache_size <= (int)cct->_conf->osd_pg_epoch_persisted_max_stale + 2) {
        clog->warn() << "osd_map_cache_size (" << cct->_conf->osd_map_cache_size << ")"
                     << " is not > osd_pg_epoch_persisted_max_stale (" << cct->_conf->osd_pg_epoch_persisted_max_stale
                     << ")";
    }
    if (cct->_conf->osd_object_clean_region_max_num_intervals < 0) {
        clog->warn() << "osd_object_clean_region_max_num_intervals ("
                     << cct->_conf->osd_object_clean_region_max_num_intervals << ") is < 0";
    }
}
...
void OSDShard::update_pg_epoch(OSDShardPGSlot* slot, epoch_t e)
{
    ...
    dout(30) << "min was " << pg_slots_by_epoch.begin()->epoch << " on " << pg_slots_by_epoch.begin()->pg->pg_id
             << dendl;
    pg_slots_by_epoch.erase(pg_slots_by_epoch.iterator_to(*slot));
    dout(20) << slot->pg->pg_id << " " << slot->epoch << " -> " << e << dendl;
    slot->epoch = e;
    pg_slots_by_epoch.insert(*slot);
    dout(30) << "min is now " << pg_slots_by_epoch.begin()->epoch << " on " << pg_slots_by_epoch.begin()->pg->pg_id
             << dendl;
    if (waiting_for_min_pg_epoch) {
        min_pg_epoch_cond.notify_all();
    }
...
void OSDShard::consume_map(const OSDMapRef& new_osdmap, unsigned* pushes_to_free)
{
    std::lock_guard l(shard_lock);
    ...
    while (p != pg_slots.end()) {
        ...
        if (slot->waiting_for_merge_epoch > new_osdmap->get_epoch()) {
            dout(20) << __func__ << "  " << pgid << " waiting for merge by epoch " << slot->waiting_for_merge_epoch
                     << dendl;
            ++p;
            continue;
        }
        if (!slot->waiting_peering.empty()) {
            ...
            if (first <= new_osdmap->get_epoch()) {
                dout(20) << __func__ << "  " << pgid << " pending_peering first epoch " << first
        ...
        if (!slot->waiting.empty()) {
            ...
            while (!slot->waiting.empty() && slot->waiting.front().get_map_epoch() <= new_osdmap->get_epoch()) {
                auto& qi = slot->waiting.front();
                dout(20) << __func__ << "  " << pgid << " waiting item " << qi << " epoch " << qi.get_map_epoch()
                         << " <= " << new_osdmap->get_epoch() << ", "
                         << (qi.get_map_epoch() < new_osdmap->get_epoch() ? "stale" : "misdirected") << ", dropping"
                         << dendl;
                *pushes_to_free += qi.get_reserved_pushes();
                slot->waiting.pop_front();
            }
        }
        if (slot->waiting.empty() && slot->num_running == 0 && slot->waiting_for_split.empty() && !slot->pg) {
...
int OSDShard::_wake_pg_slot(spg_t pgid, OSDShardPGSlot* slot)
{
    int count = 0;
    dout(20) << __func__ << " " << pgid << " to_process " << slot->to_process << " waiting " << slot->waiting
             << " waiting_peering " << slot->waiting_peering << dendl;
    for (auto i = slot->to_process.rbegin(); i != slot->to_process.rend(); ++i) {
        scheduler->enqueue_front(std::move(*i));
        count++;
    }
    slot->to_process.clear();
    for (auto i = slot->waiting.rbegin(); i != slot->waiting.rend(); ++i) {
        scheduler->enqueue_front(std::move(*i));
        count++;
    }
    slot->waiting.clear();
    for (auto i = slot->waiting_peering.rbegin(); i != slot->waiting_peering.rend(); ++i) {
...
void OSDShard::identify_splits_and_merges(const OSDMapRef& as_of_osdmap, set<pair<spg_t, epoch_t>>* split_pgs,
                                          set<pair<spg_t, epoch_t>>* merge_pgs)
{
    std::lock_guard l(shard_lock);
    ...
    if (shard_osdmap) {
        for (auto& i : pg_slots) {
            ...
            if (slot->pg) {
                osd->service.identify_splits_and_merges(shard_osdmap, as_of_osdmap, pgid, split_pgs, merge_pgs);
            }
            else if (!slot->waiting_for_split.empty()) {
                osd->service.identify_splits_and_merges(shard_osdmap, as_of_osdmap, pgid, split_pgs, nullptr);
            }
            else {
    ...
    dout(20) << __func__ << " " << split_pgs->size() << " splits, " << merge_pgs->size() << " merges" << dendl;
}

void OSDShard::prime_splits(const OSDMapRef& as_of_osdmap, set<pair<spg_t, epoch_t>>* pgids)
{
    std::lock_guard l(shard_lock);
    _prime_splits(pgids);
    if (shard_osdmap->get_epoch() > as_of_osdmap->get_epoch()) {
        ...
        for (auto i : *pgids) {
            osd->service.identify_splits_and_merges(as_of_osdmap, shard_osdmap, i.first, &newer_children, nullptr);
        }
        newer_children.insert(pgids->begin(), pgids->end());
        dout(10) << "as_of_osdmap " << as_of_osdmap->get_epoch() << " < shard " << shard_osdmap->get_epoch()
                 << ", new children " << newer_children << dendl;
...
void OSDShard::prime_merges(const OSDMapRef& as_of_osdmap, set<pair<spg_t, epoch_t>>* merge_pgs)
{
    std::lock_guard l(shard_lock);
    ...
    while (p != merge_pgs->end()) {
        ...
        if (slot->pg) {
        ...
        else if (!slot->waiting_for_split.empty() && *slot->waiting_for_split.begin() < epoch) {
            dout(20) << __func__ << "  pending split on merge participant pg " << pgid << " " << slot->waiting_for_split
                     << dendl;
        }
        else {
            ...
            PGCreateInfo cinfo(pgid, epoch - 1, history, PastIntervals(), false);
...
void OSDShard::register_and_wake_split_child(PG* pg)
{
    ...
    epoch_t epoch;
    {
        ...
        dout(20) << __func__ << ": " << pg->pg_id << " waiting_for_split " << slot->waiting_for_split << dendl;
        ceph_assert(!slot->pg);
    ...
    osd->enqueue_peering_evt(pg->pg_id, PGPeeringEventRef(std::make_shared<PGPeeringEvent>(epoch, epoch, NullEvt())));
...
void OSDShard::unprime_split_children(spg_t parent, unsigned old_pg_num)
{
    ...
    for (auto& i : pg_slots) {
        if (i.first != parent && i.first.get_ancestor(old_pg_num) == parent) {
...
OSDShard::OSDShard(int id, CephContext* cct, OSD* osd)
    : shard_id(id)
    , cct(cct)
    , osd(osd)
    ...
    , scheduler(ceph::osd::scheduler::make_scheduler(cct, osd->whoami, osd->num_shards, id, osd->store->is_rotational(),
...
void OSD::ShardedOpWQ::_add_slot_waiter(spg_t pgid, OSDShardPGSlot* slot, OpSchedulerItem&& qi)
{
    if (qi.is_peering()) {
        dout(20) << __func__ << " " << pgid << " peering, item epoch is " << qi.get_map_epoch() << ", will wait on "
                 << qi << dendl;
        slot->waiting_peering[qi.get_map_epoch()].push_back(std::move(qi));
    }
    else {
        dout(20) << __func__ << " " << pgid << " item epoch is " << qi.get_map_epoch() << ", will wait on " << qi
                 << dendl;
...
void OSD::ShardedOpWQ::_process(uint32_t thread_index, heartbeat_handle_d* hb)
{
    ...
    dout(20) << __func__ << " " << token << (r.second ? " (new)" : "") << " to_process " << slot->to_process
             << " waiting " << slot->waiting << " waiting_peering " << slot->waiting_peering << dendl;
...
retry_pg:
    ...
    if (pg) {
        ...
        if (requeue_seq != slot->requeue_seq) {
            dout(20) << __func__ << " " << token << " requeue_seq " << slot->requeue_seq << " > our " << requeue_seq
                     << ", we raced with _wake_pg_slot" << dendl;
            pg->unlock();
            sdata->shard_lock.unlock();
            handle_oncommits(oncommits);
            return;
    ...
    dout(20) << __func__ << " " << token << " to_process " << slot->to_process << " waiting " << slot->waiting
             << " waiting_peering " << slot->waiting_peering << dendl;
    ...
    while (!pg) {
        ...
        if (!slot->waiting_for_split.empty()) {
            ...
            dout(20) << __func__ << " " << token << " map " << qi.get_map_epoch() << " > " << osdmap->get_epoch()
                     << dendl;
            _add_slot_waiter(token, slot, std::move(qi));
        }
            ...
            if (!qi.peering_requires_pg()) {
                ...
                if (create_info) {
                    if (create_info->by_mon && osdmap->get_pg_acting_primary(token.pgid) != osd->whoami) {
                        dout(20) << __func__ << " " << token << " no pg, no longer primary, ignoring mon create on "
                                 << qi << dendl;
                    }
                    else {
                ...
                dout(20) << __func__ << " " << token << " no pg, peering, doesn't map here e" << osdmap->get_epoch()
                         << ", discarding " << qi << dendl;
            }
        }
        else if (osdmap->is_up_acting_osd_shard(token, osd->whoami)) {
            dout(20) << __func__ << " " << token << " no pg, should exist e" << osdmap->get_epoch() << ", will wait on "
                     << qi << dendl;
            _add_slot_waiter(token, slot, std::move(qi));
        }
        else {
            dout(20) << __func__ << " " << token << " no pg, shouldn't exist e" << osdmap->get_epoch() << ", dropping "
                     << qi << dendl;
            // share map with client?
            if (std::optional<OpRequestRef> _op = qi.maybe_get_op()) {
                osd->service.maybe_share_map(
                    (*_op)->get_req()->get_connection().get(), sdata->shard_osdmap, (*_op)->sent_epoch);
            }
