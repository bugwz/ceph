...
bool OSDService::can_inc_scrubs()
{
    ...
    if (scrubs_local + scrubs_remote < cct->_conf->osd_max_scrubs) {
        dout(20) << __func__ << " == true " << scrubs_local << " local + " << scrubs_remote << " remote < max "
                 << cct->_conf->osd_max_scrubs << dendl;
        can_inc = true;
    }
    else {
        dout(20) << __func__ << " == false " << scrubs_local << " local + " << scrubs_remote << " remote >= max "
                 << cct->_conf->osd_max_scrubs << dendl;
    }
...
bool OSDService::inc_scrubs_local()
{
    ...
    std::lock_guard l{sched_scrub_lock};
    if (scrubs_local + scrubs_remote < cct->_conf->osd_max_scrubs) {
        dout(20) << __func__ << " " << scrubs_local << " -> " << (scrubs_local + 1) << " (max "
                 << cct->_conf->osd_max_scrubs << ", remote " << scrubs_remote << ")" << dendl;
        result = true;
        ++scrubs_local;
    }
    else {
        dout(20) << __func__ << " " << scrubs_local << " local + " << scrubs_remote << " remote >= max "
                 << cct->_conf->osd_max_scrubs << dendl;
    }
    return result;
}
...
bool OSDService::inc_scrubs_remote()
{
    ...
    std::lock_guard l{sched_scrub_lock};
    if (scrubs_local + scrubs_remote < cct->_conf->osd_max_scrubs) {
        dout(20) << __func__ << " " << scrubs_remote << " -> " << (scrubs_remote + 1) << " (max "
                 << cct->_conf->osd_max_scrubs << ", local " << scrubs_local << ")" << dendl;
        result = true;
        ++scrubs_remote;
    }
    else {
...
void OSDService::queue_recovery_context(PG* pg, GenContext<ThreadPool::TPHandle&>* c)
{
    epoch_t e = get_osdmap_epoch();
    enqueue_back(OpSchedulerItem(unique_ptr<OpSchedulerItem::OpQueueable>(new PGRecoveryContext(pg->get_pgid(), c, e)),
...
void OSDService::_queue_for_recovery(std::pair<epoch_t, PGRef> p, uint64_t reserved_pushes)
{
    ceph_assert(ceph_mutex_is_locked_by_me(recovery_lock));
    enqueue_back(OpSchedulerItem(
        unique_ptr<OpSchedulerItem::OpQueueable>(new PGRecovery(p.second->get_pgid(), p.first, reserved_pushes)),
        cct->_conf->osd_recovery_cost,
        cct->_conf->osd_recovery_priority,
        ceph_clock_now(),
        0,
...
public:
    explicit OSDSocketHook(OSD* o)
        : osd(o)
    {}
    int call(std::string_view prefix, const cmdmap_t& cmdmap, Formatter* f, std::ostream& ss, bufferlist& out) override
    {
                       ...
                       std::function<void(int, const std::string&, bufferlist&)> on_finish)
{
    ...
    if (prefix == "pg" || prefix == "query" || prefix == "mark_unfound_lost" || prefix == "list_unfound" ||
        prefix == "scrub" || prefix == "deep_scrub") {
        string pgidstr;
        pg_t pgid;
    ...
    // --- OSD commands follow ---
        ...
        store->flush_journal();
             ...
             prefix == "dump_historic_ops" || prefix == "dump_historic_ops_by_duration" ||
...
public:
    TestOpsSocketHook(OSDService* s, ObjectStore* st)
        ...
        , store(st)
    {}
    int call(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, std::ostream& errss,
...
void OSD::final_init()
{
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command("dump_op_pq_state", asok_hook, "dump op priority queue state");
    ceph_assert(r == 0);
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command(
        "reset_purged_snaps_last", asok_hook, "Reset the superblock's purged_snaps_last");
    ceph_assert(r == 0);
...
int OSD::shutdown()
{
    ...
    if (!service.prepare_to_stop()) return 0;   // already shutting down
    osd_lock.lock();
    if (is_stopping()) {
        osd_lock.unlock();
        return 0;
...
PGRef OSD::handle_pg_create_info(const OSDMapRef& osdmap, const PGCreateInfo* info)
{
    ...
    pg->init(
        role, up, up_primary, acting, acting_primary, info->history, info->past_intervals, false, rctx.transaction);
...
void OSD::build_initial_pg_history(spg_t pgid, epoch_t created, utime_t created_stamp, pg_history_t* h,
                                   PastIntervals* pi)
{
    ...
    *h = pg_history_t(created, created_stamp);
    ...
    int up_primary, acting_primary;
    vector<int> up, acting;
    lastmap->pg_to_up_acting_osds(pgid.pgid, &up, &up_primary, &acting, &acting_primary);
    ...
    for (epoch_t e = created + 1; e <= get_osdmap_epoch(); ++e) {
        ...
        osdmap->pg_to_up_acting_osds(pgid.pgid, &new_up, &new_up_primary, &new_acting, &new_acting_primary);
        ...
        bool new_interval = PastIntervals::check_new_interval(acting_primary,
                                                              new_acting_primary,
                                                              acting,
                                                              new_acting,
                                                              up_primary,
        ...
        if (new_interval) {
            ...
            if (pgid.pgid.is_split(
                    lastmap->get_pg_num(pgid.pgid.pool()), osdmap->get_pg_num(pgid.pgid.pool()), nullptr)) {
    ...
    dout(10) << __func__ << " " << *h << " " << *pi << " ["
             << (pi->empty() ? pair<epoch_t, epoch_t>(0, 0) : pi->get_bounds()) << ")" << dendl;
}
...
void OSD::handle_get_purged_snaps_reply(MMonGetPurgedSnapsReply* m)
{
    ...
    SnapMapper::record_purged_snaps(cct, store, service.meta_ch, make_purged_snaps_oid(), &t, m->purged_snaps);
...
void OSD::scrub_purged_snaps()
{
    dout(10) << __func__ << dendl;
    ceph_assert(ceph_mutex_is_locked(osd_lock));
    SnapMapper::Scrubber s(cct, store, service.meta_ch, make_snapmapper_oid(), make_purged_snaps_oid());
...
void OSD::ms_fast_dispatch(Message* m)
{
    ...
    case MSG_OSD_PG_QUERY: return handle_fast_pg_query(static_cast<MOSDPGQuery*>(m));
...
void OSD::dispatch_op(OpRequestRef op)
{
    ...
    case MSG_OSD_PG_CREATE: handle_pg_create(op); break;
    }
}
    ...
    for (auto pgid : spgs) {
        enqueue_peering_evt(
            ...
            PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                get_osdmap_epoch(), get_osdmap_epoch(), PeeringState::RequestScrub(m->deep, m->repair))));
    }
...
OSDService::ScrubJob::ScrubJob(CephContext* cct, const spg_t& pg, const utime_t& timestamp,
                               double pool_scrub_min_interval, double pool_scrub_max_interval, bool must)
    : cct(cct)
    , pgid(pg)
    ...
    , deadline(timestamp)
{
    ...
    if (!must) {
        double scrub_min_interval =
            pool_scrub_min_interval > 0 ? pool_scrub_min_interval : cct->_conf->osd_scrub_min_interval;
        double scrub_max_interval =
            pool_scrub_max_interval > 0 ? pool_scrub_max_interval : cct->_conf->osd_scrub_max_interval;
...
{
    if (sched_time < rhs.sched_time) return true;
    if (sched_time > rhs.sched_time) return false;
...
bool OSD::scrub_time_permit(utime_t now)
{
    ...
    if (!day_permit) {
        dout(20) << __func__ << " should run between week day " << cct->_conf->osd_scrub_begin_week_day << " - "
                 << cct->_conf->osd_scrub_end_week_day << " now " << bdt.tm_wday << " = no" << dendl;
        return false;
    }
...
bool OSD::scrub_load_below_threshold()
{
    ...
    if (loadavgs[0] < daily_loadavg && loadavgs[0] < loadavgs[2]) {
        dout(20) << __func__ << " loadavg " << loadavgs[0] << " < daily_loadavg " << daily_loadavg << " and < 15m avg "
                 << loadavgs[2] << " = yes" << dendl;
        return true;
    }

    dout(20) << __func__ << " loadavg " << loadavgs[0] << " >= max " << cct->_conf->osd_scrub_load_threshold
             << " and ( >= daily_loadavg " << daily_loadavg << " or >= 15m avg " << loadavgs[2] << ") = no" << dendl;
    return false;
}
    ...
    if (service.first_scrub_stamp(&scrub_job)) {
        do {
            ...
            if (scrub_job.sched_time > now) {
                ...
                dout(20) << "sched_scrub " << scrub_job.pgid << " scheduled at " << scrub_job.sched_time << " > " << now
                         << dendl;
                break;
            }
...
void OSD::handle_osd_map(MOSDMap* m)
{
    ...
    if (superblock.purged_snaps_last == start - 1) {
        SnapMapper::record_purged_snaps(cct, store, service.meta_ch, make_purged_snaps_oid(), &t, purged_snaps);
...
bool OSD::require_same_peer_instance(const Message* m, const OSDMapRef& map, bool is_fast_dispatch)
{
    int from = m->get_source().num();

    if (map->is_down(from) || (map->get_cluster_addrs(from) != m->get_source_addrs())) {
        dout(5) << "from dead osd." << from << ", marking down, "
                << " msg was " << m->get_source_inst().addr << " expected "
                << (map->is_up(from) ? map->get_cluster_addrs(from) : entity_addrvec_t()) << dendl;
        ConnectionRef con = m->get_connection();
        con->mark_down();
        if (auto s = ceph::ref_cast<Session>(con->get_priv()); s) {
            if (!is_fast_dispatch) s->session_dispatch_lock.lock();
            clear_session_waiting_on_map(s);
            con->set_priv(nullptr);   // break ref <-> session cycle, if any
            s->con.reset();
            if (!is_fast_dispatch) s->session_dispatch_lock.unlock();
        }
        return false;
    }
    return true;
 ...
 */
bool OSD::require_same_or_newer_map(OpRequestRef& op, epoch_t epoch, bool is_fast_dispatch)
{
                    ...
                    PeeringCtx& rctx)
{
    ...
    for (set<spg_t>::const_iterator i = childpgids.begin(); i != childpgids.end(); ++i, ++stat_iter) {
        ...
        parent->split_colls(*i, split_bits, i->ps(), &child->get_pool().info, rctx.transaction);
...
void OSD::handle_pg_create(OpRequestRef op)
{
    ...
    if (!require_same_or_newer_map(op, m->epoch, false)) return;

    op->mark_started();

    const auto osdmap = get_osdmap();
    map<pg_t, utime_t>::const_iterator ci = m->ctimes.begin();
    for (map<pg_t, pg_create_t>::const_iterator p = m->mkpg.begin(); p != m->mkpg.end(); ++p, ++ci) {
        ...
        if (acting_primary != whoami) {
            dout(10) << "mkpg " << on << "  not acting_primary (" << acting_primary << "), my role=" << role
                     << ", skipping" << dendl;
            continue;
        }
        ...
        if (history.same_primary_since > m->epoch) {
            dout(10) << __func__ << ": got obsolete pg create on pgid " << pgid << " from epoch " << m->epoch
                     << ", primary changed in " << history.same_primary_since << dendl;
            continue;
        }
        enqueue_peering_evt(pgid,
                            PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                                ...
                                NullEvt(),
                                true,
                                new PGCreateInfo(pgid, osdmap->get_epoch(), history, pi, true))));
    }
        ...
        if (q == m->pg_extra.end()) {
            ...
            enqueue_peering_evt(
                ...
                PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                    ...
                    NullEvt(),
                    true,
                    new PGCreateInfo(pgid, created, pg_history_t(created, created_stamp), PastIntervals(), true))));
        }
        else {
...
void OSD::handle_fast_pg_query(MOSDPGQuery* m)
{
    ...
    for (auto& p : m->pg_list) {
        enqueue_peering_evt(p.first,
                            PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                                p.second.epoch_sent,
                                p.second.epoch_sent,
                                MQuery(p.first, pg_shard_t(from, p.second.from), p.second, p.second.epoch_sent),
                                false)));
    }
    m->put();
}
...
void OSD::handle_pg_query_nopg(const MQuery& q)
{
    ...
    if (con) {
        ...
        if (q.query.type == pg_query_t::LOG || q.query.type == pg_query_t::FULLLOG) {
            ...
            ls.push_back(
                pg_notify_t(q.query.from, q.query.to, q.query.epoch_sent, osdmap->get_epoch(), empty, PastIntervals()));
...
void OSD::enqueue_op(spg_t pg, OpRequestRef&& op, epoch_t epoch)
{
    ...
    dout(15) << "enqueue_op " << op << " prio " << priority << " type " << type << " cost " << cost << " latency "
             << latency << " epoch " << epoch << " " << *(op->get_req()) << dendl;
    ...
    if (op->osd_parent_span) {
        ...
        enqueue_span->Log({{"priority", priority}, {"cost", cost}, {"epoch", epoch}, {"owner", owner}, {"type", type}});
    }
#endif
...
void OSD::dequeue_op(PGRef pg, OpRequestRef op, ThreadPool::TPHandle& handle)
{
    ...
    dout(10) << "dequeue_op " << op << " prio " << m->get_priority() << " cost " << m->get_cost() << " latency "
             << latency << " " << *m << " pg " << *pg << dendl;
...
void OSD::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    ...
    if (changed.count("osd_max_backfills") || changed.count("osd_delete_sleep") ||
        changed.count("osd_delete_sleep_hdd") || changed.count("osd_delete_sleep_ssd") ||
        changed.count("osd_delete_sleep_hybrid") || changed.count("osd_snap_trim_sleep") ||
        changed.count("osd_snap_trim_sleep_hdd") || changed.count("osd_snap_trim_sleep_ssd") ||
        changed.count("osd_snap_trim_sleep_hybrid") || changed.count("osd_scrub_sleep") ||
        changed.count("osd_recovery_sleep") || changed.count("osd_recovery_sleep_hdd") ||
        changed.count("osd_recovery_sleep_ssd") || changed.count("osd_recovery_sleep_hybrid") ||
        changed.count("osd_recovery_max_active") || changed.count("osd_recovery_max_active_hdd") ||
        changed.count("osd_recovery_max_active_ssd")) {
        if (!maybe_override_options_for_qos() && changed.count("osd_max_backfills")) {
...
bool OSD::maybe_override_options_for_qos()
{
    ...
    if (cct->_conf.get_val<std::string>("osd_op_queue") == "mclock_scheduler") {
        dout(1) << __func__ << ": Changing recovery/backfill/sleep settings for QoS" << dendl;
        ...
        cct->_conf.set_val("osd_recovery_max_active_hdd", std::to_string(rec_max_active));
        cct->_conf.set_val("osd_recovery_max_active_ssd", std::to_string(rec_max_active));
...
int OSD::mon_cmd_set_config(const std::string& key, const std::string& val)
{
    ...
    int r = cond.wait();
    if (r < 0) {
        derr << __func__ << " Failed to set config key " << key << " err: " << cpp_strerror(r) << " errstr: " << outs
             << dendl;
        return r;
    }
...
void OSD::update_log_config()
{
    ...
    if (parse_log_client_options(cct,
                                 log_to_monitors,
                                 log_to_syslog,
                                 log_channel,
                                 log_prio,
                                 ...
                                 fsid,
                                 host) == 0)
        clog->update_config(log_to_monitors,
                            log_to_syslog,
                            log_channel,
                            log_prio,
                            log_to_graylog,
...
OSDShard::OSDShard(int id, CephContext* cct, OSD* osd)
    ...
    , scheduler(ceph::osd::scheduler::make_scheduler(cct, osd->num_shards, osd->store->is_rotational()))
