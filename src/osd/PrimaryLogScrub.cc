...
bool PrimaryLogScrub::get_store_errors(const scrub_ls_arg_t& arg, scrub_ls_result_t& res_inout) const
{
    if (!m_store) {
        return false;
    }
    ...
    if (arg.get_snapsets) {
        res_inout.vals = m_store->get_snap_errors(m_pg->get_pgid().pool(), arg.start_after, arg.max_return);
    }
    else {
        res_inout.vals = m_store->get_object_errors(m_pg->get_pgid().pool(), arg.start_after, arg.max_return);
    }
    return true;
}
...
void PrimaryLogScrub::_scrub_finish()
{
    ...
    if (info.stats.stats_invalid) {
        ...
        if (m_pl_pg->agent_state) m_pl_pg->agent_choose_mode();
    }

    dout(10) << m_mode_desc << " got " << m_scrub_cstat.sum.num_objects << "/" << info.stats.stats.sum.num_objects
             << " objects, " << m_scrub_cstat.sum.num_object_clones << "/" << info.stats.stats.sum.num_object_clones
             << " clones, " << m_scrub_cstat.sum.num_objects_dirty << "/" << info.stats.stats.sum.num_objects_dirty
             << " dirty, " << m_scrub_cstat.sum.num_objects_omap << "/" << info.stats.stats.sum.num_objects_omap
             << " omap, " << m_scrub_cstat.sum.num_objects_pinned << "/" << info.stats.stats.sum.num_objects_pinned
             << " pinned, " << m_scrub_cstat.sum.num_objects_hit_set_archive << "/"
             << info.stats.stats.sum.num_objects_hit_set_archive << " hit_set_archive, " << m_scrub_cstat.sum.num_bytes
             << "/" << info.stats.stats.sum.num_bytes << " bytes, " << m_scrub_cstat.sum.num_objects_manifest << "/"
             << info.stats.stats.sum.num_objects_manifest << " manifest objects, "
             << m_scrub_cstat.sum.num_bytes_hit_set_archive << "/" << info.stats.stats.sum.num_bytes_hit_set_archive
    ...
    if (m_scrub_cstat.sum.num_objects != info.stats.stats.sum.num_objects ||
        ...
        (m_scrub_cstat.sum.num_objects_hit_set_archive != info.stats.stats.sum.num_objects_hit_set_archive &&
         !info.stats.hitset_stats_invalid) ||
        (m_scrub_cstat.sum.num_bytes_hit_set_archive != info.stats.stats.sum.num_bytes_hit_set_archive &&
         !info.stats.hitset_bytes_stats_invalid) ||
        (m_scrub_cstat.sum.num_objects_manifest != info.stats.stats.sum.num_objects_manifest &&
        ...
        m_osds->clog->error() << info.pgid << " " << m_mode_desc << " : stat mismatch, got "
                              << m_scrub_cstat.sum.num_objects << "/" << info.stats.stats.sum.num_objects
                              << " objects, " << m_scrub_cstat.sum.num_object_clones << "/"
                              ...
                              << m_scrub_cstat.sum.num_whiteouts << "/" << info.stats.stats.sum.num_whiteouts
                              << " whiteouts, " << m_scrub_cstat.sum.num_bytes << "/" << info.stats.stats.sum.num_bytes
    ...
    if (m_is_repair) m_pl_pg->object_contexts.clear();
}

static bool doing_clones(const std::optional<SnapSet>& snapset, const vector<snapid_t>::reverse_iterator& curclone)
{
    return snapset && curclone != snapset->clones.rend();
}

void PrimaryLogScrub::log_missing(int missing, const std::optional<hobject_t>& head, LogChannelRef clog,
                                  const spg_t& pgid, const char* func, bool allow_incomplete_clones)
{
    ceph_assert(head);
    if (allow_incomplete_clones) {
        dout(20) << func << " " << m_mode_desc << " " << pgid << " " << *head << " skipped " << missing
                 << " clone(s) in cache tier" << dendl;
    }
    else {
        clog->info() << m_mode_desc << " " << pgid << " " << *head << " : " << missing << " missing clone(s)";
    }
}

int PrimaryLogScrub::process_clones_to(const std::optional<hobject_t>& head, const std::optional<SnapSet>& snapset,
                                       LogChannelRef clog, const spg_t& pgid, bool allow_incomplete_clones,
                                       ...
                                       inconsistent_snapset_wrapper& e)
{
    ...
    while (doing_clones(snapset, *curclone) && (!target || **curclone > *target)) {
        ...
        if (!allow_incomplete_clones) {
            ...
            clog->error() << m_mode_desc << " " << pgid << " " << *head << " : expected clone " << next_clone << " "
                          << m_missing << " missing";
            ++m_shallow_errors;
 ...
 */
void PrimaryLogScrub::scrub_snapshot_metadata(ScrubMap& scrubmap, const missing_map_t& missing_digest)
{
    ...
    for (auto p = scrubmap.objects.rbegin(); p != scrubmap.objects.rend(); ++p) {
        ...
        if (soid.nspace == m_pl_pg->cct->_conf->osd_hit_set_namespace) stat.num_objects_hit_set_archive++;
        ...
        if (p->second.attrs.count(OI_ATTR) == 0) {
            oi = std::nullopt;
            m_osds->clog->error() << m_mode_desc << " " << info.pgid << " " << soid << " : no '" << OI_ATTR << "' attr";
            ++m_shallow_errors;
            ...
            try {
                ...
                oi = std::nullopt;
                m_osds->clog->error() << m_mode_desc << " " << info.pgid << " " << soid << " : can't decode '"
                                      << OI_ATTR << "' attr " << e.what();
                ++m_shallow_errors;
        ...
        if (oi) {
            if (m_pl_pg->pgbackend->be_get_ondisk_size(oi->size) != p->second.size) {
                m_osds->clog->error() << m_mode_desc << " " << info.pgid << " " << soid << " : on disk size ("
                                      << p->second.size << ") does not match object info size (" << oi->size
            ...
            if (soid.nspace == m_pl_pg->cct->_conf->osd_hit_set_namespace) stat.num_bytes_hit_set_archive += oi->size;
        ...
        if (doing_clones(snapset, curclone)) {
            ...
            missing += process_clones_to(
                head, snapset, m_osds->clog, info.pgid, allow_incomplete_clones, target, &curclone, head_error);
        }
        ...
        if (!expected) {
            ...
            if (head && !snapset) {
                ...
                m_osds->clog->error() << m_mode_desc << " " << info.pgid << " " << soid << " : is an unexpected clone";
            }
            ++m_shallow_errors;
        ...
        if (soid.has_snapset()) {
            ...
            if (missing) {
                log_missing(missing, head, m_osds->clog, info.pgid, __func__, pool.info.allow_incomplete_clones());
            }

            // Save previous head error information
            if (head && (head_error.errors || soid_error_count)) m_store->add_snap_error(pool.id, head_error);
            ...
            if (p->second.attrs.count(SS_ATTR) == 0) {
                m_osds->clog->error() << m_mode_desc << " " << info.pgid << " " << soid << " : no '" << SS_ATTR
                                      << "' attr";
                ++m_shallow_errors;
                snapset = std::nullopt;
            ...
            if (snapset->clone_size.count(soid.snap) == 0) {
                ...
                if (oi && oi->size != snapset->clone_size[soid.snap]) {
                    m_osds->clog->error() << m_mode_desc << " " << info.pgid << " " << soid << " : size " << oi->size
                ...
                if (snapset->clone_overlap.count(soid.snap) == 0) {
                    m_osds->clog->error()
                        << m_mode_desc << " " << info.pgid << " " << soid << " : is missing in clone_overlap";
                    ++m_shallow_errors;
                    soid_error.set_size_mismatch();
                }
                else {
                    ...
                    if (bad_interval_set) {
                        m_osds->clog->error()
                            << m_mode_desc << " " << info.pgid << " " << soid << " : bad interval_set in clone_overlap";
                        ++m_shallow_errors;
                        soid_error.set_size_mismatch();
                    }
                    else {
    ...
    if (doing_clones(snapset, curclone)) {
        dout(10) << __func__ << " " << m_mode_desc << " " << info.pgid << " No more objects while processing " << *head
                 << dendl;

        missing += process_clones_to(
            head, snapset, m_osds->clog, info.pgid, allow_incomplete_clones, all_clones, &curclone, head_error);
    }

    // There could be missing found by the test above or even
    // before dropping out of the loop for the last head.
    if (missing) {
        log_missing(missing, head, m_osds->clog, info.pgid, __func__, allow_incomplete_clones);
    }
    if (head && (head_error.errors || soid_error_count)) m_store->add_snap_error(pool.id, head_error);
    ...
    for (auto p = missing_digest.begin(); p != missing_digest.end(); ++p) {
        ...
        if (!obc) {
            m_osds->clog->error() << info.pgid << " " << m_mode_desc << " cannot get object context for object "
                                  << p->first;
            continue;
        }
...
void PrimaryLogScrub::stats_of_handled_objects(const object_stat_sum_t& delta_stats, const hobject_t& soid)
{
