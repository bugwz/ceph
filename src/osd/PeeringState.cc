...
{
    send_osd_message(to, TOPNSPC::make_message<MOSDPGInfo2>(to_spgid, info, cur_epoch, min_epoch, lease, lease_ack));
}
...
void PeeringState::purge_strays()
{
    ...
    for (auto p = stray_set.begin(); p != stray_set.end(); ++p) {
        ...
        if (get_osdmap()->is_up(p->osd)) {
            ...
            auto m = TOPNSPC::make_message<MOSDPGRemove>(get_osdmap_epoch(), to_remove);
...
void PeeringState::send_lease()
{
    ...
    for (auto peer : actingset) {
        ...
        pl->send_cluster_message(
            peer.osd, TOPNSPC::make_message<MOSDPGLease>(epoch, spg_t(spgid.pgid, peer.shard), get_lease()), epoch);
    }
}
    ...
    if (is_primary()) {
        ...
        for (auto i = acting_recovery_backfill.begin(); i != acting_recovery_backfill.end(); ++i) {
            ...
            if (pi.last_update == info.last_update) {
                ...
                m = TOPNSPC::make_message<MOSDPGLog>(i->shard,
                                                     pg_whoami.shard,
                                                     get_osdmap_epoch(),
                                                     info,
...
void PeeringState::fulfill_log(pg_shard_t from, const pg_query_t& query, epoch_t query_epoch)
{
    ...
    auto mlog = TOPNSPC::make_message<MOSDPGLog>(from.shard, pg_whoami.shard, get_osdmap_epoch(), info, query_epoch);
...
std::optional<pg_stat_t> PeeringState::prepare_stats_for_publish(const std::optional<pg_stat_t>& pg_stats_publish,
                                                                 const object_stat_collection_t& unstable_stats)
{
    ...
    if (pg_stats_publish && pre_publish == *pg_stats_publish && info.stats.last_fresh > cutoff) {
...
void PeeringState::init(int role, const vector<int>& newup, int new_up_primary, const vector<int>& newacting,
                        int new_acting_primary, const pg_history_t& history, const PastIntervals& pi,
                        ObjectStore::Transaction& t)
{
...
void PeeringState::update_stats_wo_resched(std::function<void(pg_history_t&, pg_stat_t&)> f)
{
...
boost::statechart::result PeeringState::WaitRemoteBackfillReserved::react(const RemoteBackfillReserved& evt)
{
    ...
    if (backfill_osd_it != context<Active>().remote_shards_to_reserve_backfill.end()) {
        ...
        pl->send_cluster_message(
            backfill_osd_it->osd,
            TOPNSPC::make_message<MBackfillReserve>(MBackfillReserve::REQUEST,
...
void PeeringState::WaitRemoteBackfillReserved::retry()
{
    ...
    for (it = begin; it != backfill_osd_it; ++it) {
        ...
        pl->send_cluster_message(
            it->osd,
            TOPNSPC::make_message<MBackfillReserve>(MBackfillReserve::RELEASE,
...
boost::statechart::result PeeringState::ReplicaActive::react(const MLease& l)
{
    ...
    pl->send_cluster_message(
        ps->get_primary().osd,
        TOPNSPC::make_message<MOSDPGLeaseAck>(epoch, spg_t(spgid.pgid, ps->get_primary().shard), ps->get_lease_ack()),
        epoch);
    return discard_event();
}
