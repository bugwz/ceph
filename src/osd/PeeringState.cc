...
BufferedRecoveryMessages::BufferedRecoveryMessages(ceph_release_t r, PeeringCtx& ctx)
...
void BufferedRecoveryMessages::send_query(int to, spg_t to_spgid, const pg_query_t& q)
{
    if (require_osd_release >= ceph_release_t::octopus) {
        send_osd_message(to, make_message<MOSDPGQuery2>(to_spgid, q));
    }
    else {
        auto m = make_message<MOSDPGQuery>(q.epoch_sent, MOSDPGQuery::pg_list_t{{to_spgid, q}});
        send_osd_message(to, m);
    }
}

void BufferedRecoveryMessages::send_info(int to, spg_t to_spgid, epoch_t min_epoch, epoch_t cur_epoch,
                                         const pg_info_t& info, std::optional<pg_lease_t> lease,
                                         std::optional<pg_lease_ack_t> lease_ack)
{
    if (require_osd_release >= ceph_release_t::octopus) {
        send_osd_message(to, make_message<MOSDPGInfo2>(to_spgid, info, cur_epoch, min_epoch, lease, lease_ack));
    }
    else {
        send_osd_message(
            to,
            make_message<MOSDPGInfo>(
                cur_epoch,
                vector{pg_notify_t{to_spgid.shard, info.pgid.shard, min_epoch, cur_epoch, info, PastIntervals{}}}));
    }
}
...
void PGPool::update(OSDMapRef map)
{
    ...
    bool updated = false;
    if ((map->get_epoch() != cached_epoch + 1) || (pi->get_snap_epoch() == map->get_epoch())) {
        updated = true;
    }
...
PeeringState::PeeringState(CephContext* cct, pg_shard_t pg_whoami, spg_t spgid, const PGPool& _pool, OSDMapRef curmap,
                           DoutPrefixProvider* dpp, PeeringListener* pl)
    : state_history(*pl)
    , cct(cct)
    , spgid(spgid)
    , dpp(dpp)
...
void PeeringState::begin_block_outgoing()
{
    ...
    ceph_assert(orig_ctx);
    ceph_assert(rctx);
    messages_pending_flush = BufferedRecoveryMessages(orig_ctx->require_osd_release);
...
void PeeringState::check_recovery_sources(const OSDMapRef& osdmap)
{
    ...
    for (auto i = peer_missing_requested.begin(); i != peer_missing_requested.end();) {
        if (!osdmap->is_up(i->osd)) {
...
void PeeringState::update_history(const pg_history_t& new_history)
{
    ...
    if (info.history.merge(new_history)) {
        ...
        if (prior_readable_until_ub != ceph::signedspan::zero()) {
            dout(20) << __func__ << " prior_readable_until_ub " << prior_readable_until_ub << " (mnow " << mnow << " + "
...
void PeeringState::purge_strays()
{
    ...
    for (auto p = stray_set.begin(); p != stray_set.end(); ++p) {
        ...
        if (get_osdmap()->is_up(p->osd)) {
            ...
            auto m = make_message<MOSDPGRemove>(get_osdmap_epoch(), to_remove);
    ...
    if (removed) update_heartbeat_peers();
...
void PeeringState::query_unfound(Formatter* f, string state)
{
    psdout(20) << "Enter PeeringState common QueryUnfound" << dendl;
    {
        ...
        for (auto p = might_have_unfound.begin(); p != might_have_unfound.end(); ++p) {
...
bool PeeringState::proc_replica_info(pg_shard_t from, const pg_info_t& oinfo, epoch_t send_epoch)
{
    ...
    if (p == peer_info.end()) update_heartbeat_peers();
...
void PeeringState::update_heartbeat_peers()
{
    ...
    for (unsigned i = 0; i < acting.size(); i++) {
        if (acting[i] != CRUSH_ITEM_NONE) new_peers.insert(acting[i]);
    }
    for (unsigned i = 0; i < up.size(); i++) {
        if (up[i] != CRUSH_ITEM_NONE) new_peers.insert(up[i]);
    }
...
void PeeringState::advance_map(OSDMapRef osdmap, OSDMapRef lastmap, vector<int>& newup, int up_primary,
                               vector<int>& newacting, int acting_primary, PeeringCtx& rctx)
{
    ceph_assert(lastmap == osdmap_ref);
    psdout(10) << "handle_advance_map " << newup << "/" << newacting << " -- " << up_primary << "/" << acting_primary
               << dendl;

    update_osdmap_ref(osdmap);
    pool.update(osdmap);

    AdvMap evt(osdmap, lastmap, newup, up_primary, newacting, acting_primary);
...
void PeeringState::activate_map(PeeringCtx& rctx)
{
    ...
    ActMap evt;
    handle_event(evt, &rctx);
    if (osdmap_ref->get_epoch() - last_persisted_osdmap > cct->_conf->osd_pg_epoch_persisted_max_stale) {
        psdout(20) << __func__ << ": Dirtying info: last_persisted is " << last_persisted_osdmap << " while current is "
                   << osdmap_ref->get_epoch() << dendl;
        dirty_info = true;
    }
    else {
        psdout(20) << __func__ << ": Not dirtying info: last_persisted is " << last_persisted_osdmap
...
bool PeeringState::should_restart_peering(int newupprimary, int newactingprimary, const vector<int>& newup,
                                          const vector<int>& newacting, OSDMapRef lastmap, OSDMapRef osdmap)
{
    if (PastIntervals::is_new_interval(primary.osd,
                                       newactingprimary,
                                       acting,
                                       newacting,
                                       up_primary.osd,
...
void PeeringState::start_peering_interval(const OSDMapRef lastmap, const vector<int>& newup, int new_up_primary,
                                          ...
                                          ObjectStore::Transaction& t)
{
    ...
    if (info.stats.up != up || info.stats.acting != acting || info.stats.up_primary != new_up_primary ||
    ...
    if (!lastmap) {
        ...
        bool new_interval = PastIntervals::check_new_interval(old_acting_primary.osd,
                                                              new_acting_primary,
                                                              oldacting,
                                                              newacting,
                                                              old_up_primary.osd,
        ...
        if (new_interval) {
            ...
            if (osdmap->have_pg_pool(info.pgid.pgid.pool()) &&
                info.pgid.pgid.is_split(
                    lastmap->get_pg_num(info.pgid.pgid.pool()), osdmap->get_pg_num(info.pgid.pgid.pool()), nullptr)) {
    ...
    psdout(1) << __func__ << " up " << oldup << " -> " << up << ", acting " << oldacting << " -> " << acting
              << ", acting_primary " << old_acting_primary << " -> " << new_acting_primary << ", up_primary "
              << old_up_primary << " -> " << new_up_primary << ", role " << oldrole << " -> " << role
              << ", features acting " << acting_features << " upacting " << upacting_features << dendl;
    ...
    if (role != oldrole || was_old_primary != is_primary()) {
        ...
        if (get_primary() != old_acting_primary) {
            psdout(10) << oldacting << " -> " << acting << ", acting primary " << old_acting_primary << " -> "
                       << get_primary() << dendl;
        }
        else {
            // primary is the same.
...
void PeeringState::on_new_interval()
{
    ...
    psdout(20) << __func__ << " upacting_features 0x" << std::hex << upacting_features << std::dec << " from " << acting
               << "+" << up << dendl;

    psdout(20) << __func__ << " checking missing set deletes flag. missing = " << get_pg_log().get_missing() << dendl;

    if (!pg_log.get_missing().may_include_deletes && !perform_deletes_during_peering()) {
        pl->rebuild_missing_set_with_deletes(pg_log);
    }
    ceph_assert(pg_log.get_missing().may_include_deletes == !perform_deletes_during_peering());

    init_hb_stamps();

    // update lease bounds for a new interval
    auto mnow = pl->get_mnow();
    prior_readable_until_ub = std::max(prior_readable_until_ub, readable_until_ub);
    prior_readable_until_ub = info.history.refresh_prior_readable_until_ub(mnow, prior_readable_until_ub);
    psdout(10) << __func__ << " prior_readable_until_ub " << prior_readable_until_ub << " (mnow " << mnow << " + "
               << info.history.prior_readable_until_ub << ")" << dendl;
    prior_readable_down_osds.clear();   // we populate this when we build the priorset

    readable_until = readable_until_ub = readable_until_ub_sent = readable_until_ub_from_primary =
...
void PeeringState::init_primary_up_acting(const vector<int>& newup, const vector<int>& newacting, int new_up_primary,
                                          int new_acting_primary)
{
    ...
    for (uint8_t i = 0; i < acting.size(); ++i) {
        if (acting[i] != CRUSH_ITEM_NONE)
            actingset.insert(pg_shard_t(acting[i], pool.info.is_erasure() ? shard_id_t(i) : shard_id_t::NO_SHARD));
    }
    upset.clear();
    up = newup;
    for (uint8_t i = 0; i < up.size(); ++i) {
        if (up[i] != CRUSH_ITEM_NONE)
            upset.insert(pg_shard_t(up[i], pool.info.is_erasure() ? shard_id_t(i) : shard_id_t::NO_SHARD));
    }
...
static pair<epoch_t, epoch_t> get_required_past_interval_bounds(const pg_info_t& info, epoch_t oldest_map)
{
    epoch_t start = std::max(
        info.history.last_epoch_clean ? info.history.last_epoch_clean : info.history.epoch_pool_created, oldest_map);
    epoch_t end = std::max(info.history.same_interval_since, info.history.epoch_pool_created);
...
void PeeringState::check_past_interval_bounds() const
{
    ...
    auto rpib = get_required_past_interval_bounds(info, oldest_epoch);
    if (rpib.first >= rpib.second) {
        ...
        if (!past_intervals.empty() && rpib.first > oldest_epoch) {
            pl->get_clog_error() << info.pgid << " required past_interval bounds are"
                                 << " empty [" << rpib << ") but past_intervals is not: " << past_intervals;
            derr << info.pgid << " required past_interval bounds are"
                 << " empty [" << rpib << ") but past_intervals is not: " << past_intervals << dendl;
        }
    }
    else {
        if (past_intervals.empty()) {
            pl->get_clog_error() << info.pgid << " required past_interval bounds are"
                                 << " not empty [" << rpib << ") but past_intervals " << past_intervals << " is empty";
            derr << info.pgid << " required past_interval bounds are"
                 << " not empty [" << rpib << ") but past_intervals " << past_intervals << " is empty" << dendl;
        ...
        if (apib.first > rpib.first) {
            pl->get_clog_error() << info.pgid << " past_intervals [" << apib
                                 ...
                                 << " bound [" << rpib << ") start";
            derr << info.pgid << " past_intervals [" << apib << ") start interval does not contain the required"
                 << " bound [" << rpib << ") start" << dendl;
            ceph_abort_msg("past_interval start interval mismatch");
        }
        if (apib.second != rpib.second) {
            pl->get_clog_error() << info.pgid << " past_interal bound [" << apib << ") end does not match required ["
                                 << rpib << ") end";
            derr << info.pgid << " past_interal bound [" << apib << ") end does not match required [" << rpib << ") end"
                 << dendl;
...
int PeeringState::clamp_recovery_priority(int priority, int pool_recovery_priority, int max)
{
    ...
    if (pool_recovery_priority > OSD_POOL_PRIORITY_MAX) pool_recovery_priority = OSD_POOL_PRIORITY_MAX;
    if (pool_recovery_priority < OSD_POOL_PRIORITY_MIN) pool_recovery_priority = OSD_POOL_PRIORITY_MIN;
    // Shift range from min to max to 0 to max - min
    pool_recovery_priority += (0 - OSD_POOL_PRIORITY_MIN);
    ceph_assert(pool_recovery_priority >= 0 &&
                pool_recovery_priority <= (OSD_POOL_PRIORITY_MAX - OSD_POOL_PRIORITY_MIN));
...
unsigned PeeringState::get_delete_priority()
{
    ...
    if (state & (CEPH_OSD_BACKFILLFULL | CEPH_OSD_FULL)) {
...
bool PeeringState::set_force_recovery(bool b)
{
    ...
    if (b) {
        if (!(state & PG_STATE_FORCED_RECOVERY) &&
            (state & (PG_STATE_DEGRADED | PG_STATE_RECOVERY_WAIT | PG_STATE_RECOVERING))) {
...
bool PeeringState::set_force_backfill(bool b)
{
    ...
    if (b) {
        if (!(state & PG_STATE_FORCED_BACKFILL) &&
            (state & (PG_STATE_DEGRADED | PG_STATE_BACKFILL_WAIT | PG_STATE_BACKFILLING))) {
...
{
    pl->schedule_renew_lease(last_peering_reset, readable_interval / 2);
}
...
void PeeringState::send_lease()
{
    ...
    for (auto peer : actingset) {
        ...
        pl->send_cluster_message(
            peer.osd, make_message<MOSDPGLease>(epoch, spg_t(spgid.pgid, peer.shard), get_lease()), epoch);
    }
}
...
void PeeringState::proc_lease(const pg_lease_t& l)
{
    if (!HAVE_FEATURE(upacting_features, SERVER_OCTOPUS)) {
        psdout(20) << __func__ << " no-op, upacting_features 0x" << std::hex << upacting_features << std::dec
    ...
    if (l.readable_until != ceph::signedspan::zero() && hb_stamps[0]->peer_clock_delta_ub) {
...
void PeeringState::recalc_readable_until()
{
    ...
    dout(20) << __func__ << " readable_until[_ub] " << readable_until << " (sent " << readable_until_ub_sent << ")"
             << dendl;
}
...
bool PeeringState::check_prior_readable_down_osds(const OSDMapRef& map)
{
    ...
    while (p != prior_readable_down_osds.end()) {
        if (map->is_dead(*p)) {
            dout(10) << __func__ << " prior_readable_down_osds osd." << *p << " is dead as of epoch "
                     << map->get_epoch() << dendl;
...
PastIntervals::PriorSet PeeringState::build_prior()
{
    ...
    PastIntervals::PriorSet prior = past_intervals.get_prior_set(
        ...
        [&](epoch_t start, int osd, epoch_t* lost_at) {
            ...
            if (osdmap.exists(osd)) {
                ...
                if (lost_at) *lost_at = pinfo->lost_at;
            }
    ...
    if (get_osdmap()->get_up_thru(pg_whoami.osd) < info.history.same_interval_since) {
        psdout(10) << "up_thru " << get_osdmap()->get_up_thru(pg_whoami.osd) << " < same_since "
                   << info.history.same_interval_since << ", must notify monitor" << dendl;
        need_up_thru = true;
    }
    else {
        psdout(10) << "up_thru " << get_osdmap()->get_up_thru(pg_whoami.osd) << " >= same_since "
                   << info.history.same_interval_since << ", all is well" << dendl;
        need_up_thru = false;
    }
 ...
 */
bool PeeringState::all_unfound_are_queried_or_lost(const OSDMapRef osdmap) const
{
    ceph_assert(is_primary());
    ...
    for (; peer != mend; ++peer) {
        ...
        if (iter != peer_info.end() && (iter->second.is_empty() || iter->second.dne())) continue;
        if (!osdmap->exists(peer->osd)) continue;
    ...
    psdout(10) << "all_unfound_are_queried_or_lost all of might_have_unfound " << might_have_unfound
...
void PeeringState::reject_reservation()
{
    ...
    pl->send_cluster_message(
        ...
        make_message<MBackfillReserve>(
            MBackfillReserve::REJECT_TOOFULL, spg_t(info.pgid.pgid, primary.shard), get_osdmap_epoch()),
        get_osdmap_epoch());
}
 ...
 */
map<pg_shard_t, pg_info_t>::const_iterator PeeringState::find_best_info(const map<pg_shard_t, pg_info_t>& infos,
                                                                        ...
                                                                        bool* history_les_bound) const
{
    ...
    for (auto i = infos.begin(); i != infos.end(); ++i) {
        if (max_last_epoch_started_found <= i->second.last_epoch_started) {
            if (min_last_update_acceptable > i->second.last_update) min_last_update_acceptable = i->second.last_update;
        }
    }
    if (min_last_update_acceptable == eversion_t::max()) return infos.end();
    ...
    for (auto p = infos.begin(); p != infos.end(); ++p) {
        if (restrict_to_up_acting && !is_up(p->first) && !is_acting(p->first)) continue;
        // Only consider peers with last_update >= min_last_update_acceptable
        if (p->second.last_update < min_last_update_acceptable) continue;
        // Disqualify anyone with a too old last_epoch_started
        if (p->second.last_epoch_started < max_last_epoch_started_found) continue;
        // Disqualify anyone who is incomplete (not fully backfilled)
        if (p->second.is_incomplete()) continue;
        if (best == infos.end()) {
            best = p;
            continue;
        }
        ...
        if (pool.info.require_rollback()) {
            if (p->second.last_update > best->second.last_update) continue;
        ...
        if (!p->second.has_missing() && best->second.has_missing()) {
            psdout(10) << __func__ << " prefer osd." << p->first << " because it is complete while best has missing"
                       << dendl;
            best = p;
            continue;
        }
        else if (p->second.has_missing() && !best->second.has_missing()) {
            psdout(10) << __func__ << " skipping osd." << p->first << " because it has missing while best is complete"
                       << dendl;
            continue;
        }
        else {
...
void PeeringState::calc_ec_acting(map<pg_shard_t, pg_info_t>::const_iterator auth_log_shard, unsigned size,
                                  const vector<int>& acting, const vector<int>& up,
                                  const map<pg_shard_t, pg_info_t>& all_info, bool restrict_to_up_acting,
                                  vector<int>* _want, set<pg_shard_t>* backfill, set<pg_shard_t>* acting_backfill,
                                  ostream& ss)
{
    vector<int> want(size, CRUSH_ITEM_NONE);
    map<shard_id_t, set<pg_shard_t>> all_info_by_shard;
    for (auto i = all_info.begin(); i != all_info.end(); ++i) {
    ...
    for (uint8_t i = 0; i < want.size(); ++i) {
        ...
        if (up.size() > (unsigned)i && up[i] != CRUSH_ITEM_NONE &&
            ...
            all_info.find(pg_shard_t(up[i], shard_id_t(i)))->second.last_update >= auth_log_shard->second.log_tail) {
        ...
        if (acting.size() > (unsigned)i && acting[i] != CRUSH_ITEM_NONE &&
            ...
            for (auto j = all_info_by_shard[shard_id_t(i)].begin(); j != all_info_by_shard[shard_id_t(i)].end(); ++j) {
            ...
            if (want[i] == CRUSH_ITEM_NONE) ss << " failed to fill position " << (int)i << std::endl;
        }
    }
...
std::pair<map<pg_shard_t, pg_info_t>::const_iterator, eversion_t> PeeringState::select_replicated_primary(
    map<pg_shard_t, pg_info_t>::const_iterator auth_log_shard, uint64_t force_auth_primary_missing_objects,
    const std::vector<int>& up, pg_shard_t up_primary, const map<pg_shard_t, pg_info_t>& all_info,
    const OSDMapRef osdmap, ostream& ss)
{
    pg_shard_t auth_log_shard_id = auth_log_shard->first;

    ss << __func__ << " newest update on osd." << auth_log_shard_id << " with " << auth_log_shard->second << std::endl;
        ...
        if (HAVE_FEATURE(osdmap->get_up_osd_features(), SERVER_NAUTILUS)) {
            ...
            if ((uint64_t)approx_missing_objects > force_auth_primary_missing_objects) {
                ...
                ss << "up_primary: " << up_primary << ") has approximate " << approx_missing_objects << "(>"
                   ...
                   << "missing objects, osd." << auth_log_shard_id << " selected as primary instead" << std::endl;
            }
            else {
    ...
    ss << __func__ << " primary is osd." << primary->first << " with " << primary->second << std::endl;
    ...
    eversion_t oldest_auth_log_entry = std::min(primary->second.log_tail, auth_log_shard->second.log_tail);
...
void PeeringState::calc_replicated_acting(map<pg_shard_t, pg_info_t>::const_iterator primary,
                                          eversion_t oldest_auth_log_entry, unsigned size, const vector<int>& acting,
                                          const vector<int>& up, pg_shard_t up_primary,
                                          const map<pg_shard_t, pg_info_t>& all_info, bool restrict_to_up_acting,
                                          vector<int>* want, set<pg_shard_t>* backfill,
                                          set<pg_shard_t>* acting_backfill, const OSDMapRef osdmap, const PGPool& pool,
                                          ostream& ss)
{
    ...
    for (auto i : up) {
        ...
        if (up_cand == primary->first) continue;
    ...
    for (auto i : acting) {
        ...
        if (acting_cand == primary->first) continue;
    ...
    auto sort_by_eversion = [](const std::pair<eversion_t, int>& lhs, const std::pair<eversion_t, int>& rhs) {
    ...
    for (auto& i : all_info) {
        // skip up osds we already considered above
        if (i.first == primary->first) continue;
...
public:
    ...
    osd_ord_t get_ord() const { return osds.empty() ? std::make_tuple(false, eversion_t()) : osds.front().first; }

    bool is_empty() const { return osds.empty(); }

    bool operator<(const bucket_candidates_t& rhs) const
    {
        return std::make_tuple(-selected, get_ord()) < std::make_tuple(-rhs.selected, rhs.get_ord());
    }
...
class bucket_heap_t
{
    ...
    struct comp
    {
        bool operator()(const elem_t& lhs, const elem_t& rhs) { return lhs.get() < rhs.get(); }
    };
 ...
 */
void PeeringState::calc_replicated_acting_stretch(map<pg_shard_t, pg_info_t>::const_iterator primary,
                                                  eversion_t oldest_auth_log_entry, unsigned size,
                                                  const vector<int>& acting, const vector<int>& up,
                                                  pg_shard_t up_primary, const map<pg_shard_t, pg_info_t>& all_info,
                                                  bool restrict_to_up_acting, vector<int>* want,
                                                  set<pg_shard_t>* backfill, set<pg_shard_t>* acting_backfill,
                                                  const OSDMapRef osdmap, const PGPool& pool, ostream& ss)
{
    ceph_assert(want);
    ...
    auto used = [want](int osd) { return std::find(want->begin(), want->end(), osd) != want->end(); };

    auto usable_info = [&](const auto& cur_info) mutable {
        return !(cur_info.is_incomplete() || cur_info.last_update < oldest_auth_log_entry);
    };
    ...
    auto usable_osd = [&](int osd) mutable { return usable_info(osd_info(osd)); };

    std::map<int, bucket_candidates_t> ancestors;
    auto get_ancestor = [&](int osd) mutable {
        int ancestor =
            osdmap->crush->get_parent_of_type(osd, pool.info.peering_crush_bucket_barrier, pool.info.crush_rule);
    ...
    auto add_required = [&](int osd) {
        if (!used(osd)) {
            want->push_back(osd);
            acting_backfill->insert(pg_shard_t(osd, shard_id_t::NO_SHARD));
            get_ancestor(osd)->inc_selected();
        }
    };
    add_required(primary->first.osd);
    ss << " osd " << primary->first.osd << " primary accepted " << osd_info(primary->first.osd) << std::endl;
    for (auto upcand : up) {
     ...
     */
    {
        ...
        auto get_osd_ord = [&](bool is_acting, const pg_info_t& info) -> osd_ord_t {
            return std::make_tuple(!is_acting /* acting should sort first */, info.last_update);
        };
        ...
        if (!restrict_to_up_acting) {
            for (auto& [cand, info] : all_info) {
                if (!used(cand.osd) && usable_info(info) &&
                    (std::find(acting.begin(), acting.end(), cand.osd) == acting.end())) {
                    ss << " other candidate " << cand << " " << info << std::endl;
                    candidates.push_back(std::make_pair(get_osd_ord(false, info), cand.osd));
                }
            }
        }
    ...
    if (pool.info.peering_crush_mandatory_member != CRUSH_ITEM_NONE) {
        ...
        if (aiter != ancestors.end() && !aiter->second.get_num_selected()) {
    ...
    bucket_heap_t aheap;
    std::for_each(ancestors.begin(), ancestors.end(), [&](auto& anc) { aheap.push_if_nonempty(anc.second); });
...
bool PeeringState::recoverable(const vector<int>& want) const
{
    ...
    for (int i = 0; i < (int)want.size(); ++i) {
        if (want[i] != CRUSH_ITEM_NONE) {
            ++num_want_acting;
            have.insert(pg_shard_t(want[i], pool.info.is_erasure() ? shard_id_t(i) : shard_id_t::NO_SHARD));
        }
    }

    if (num_want_acting < pool.info.min_size) {
        const bool recovery_ec_pool_below_min_size = HAVE_FEATURE(get_osdmap()->get_up_osd_features(), SERVER_OCTOPUS);
...
void PeeringState::choose_async_recovery_ec(const map<pg_shard_t, pg_info_t>& all_info, const pg_info_t& auth_info,
                                            vector<int>* want, set<pg_shard_t>* async_recovery,
                                            const OSDMapRef osdmap) const
{
    set<pair<int, pg_shard_t>> candidates_by_cost;
    unsigned want_acting_size = 0;
    for (uint8_t i = 0; i < want->size(); ++i) {
        if ((*want)[i] == CRUSH_ITEM_NONE) continue;
        ++want_acting_size;
        ...
        if (stray_set.find(shard_i) != stray_set.end()) continue;
        ...
        if (HAVE_FEATURE(osdmap->get_up_osd_features(), SERVER_NAUTILUS)) {
            auto approx_missing_objects = shard_info.stats.stats.sum.num_objects_missing;
    ...
    for (auto rit = candidates_by_cost.rbegin(); rit != candidates_by_cost.rend(); ++rit) {
        ...
        --want_acting_size;
        if ((want_acting_size >= pool.info.min_size) && recoverable(candidate_want)) {
...
void PeeringState::choose_async_recovery_replicated(const map<pg_shard_t, pg_info_t>& all_info,
                                                    const pg_info_t& auth_info, vector<int>* want,
                                                    set<pg_shard_t>* async_recovery, const OSDMapRef osdmap) const
{
    set<pair<int, pg_shard_t>> candidates_by_cost;
 ...
 * TODO!
 */
bool PeeringState::choose_acting(pg_shard_t& auth_log_shard_id, bool restrict_to_up_acting, bool* history_les_bound,
                                 bool request_pg_temp_change_only)
{
    ...
    if (pool.info.is_replicated()) {
        auto [primary_shard, oldest_log] =
            select_replicated_primary(auth_log_shard,
    ...
    if (request_pg_temp_change_only) return true;
    want_acting.clear();
...
void PeeringState::log_weirdness()
{
    if (pg_log.get_tail() != info.log_tail)
        pl->get_clog_error() << info.pgid << " info mismatch, log.tail " << pg_log.get_tail() << " != info.log_tail "
                             << info.log_tail;
    if (pg_log.get_head() != info.last_update)
        pl->get_clog_error() << info.pgid << " info mismatch, log.head " << pg_log.get_head() << " != info.last_update "
                             << info.last_update;

    if (!pg_log.get_log().empty()) {
        // sloppy check
        if ((pg_log.get_log().log.begin()->version <= pg_log.get_tail()))
            pl->get_clog_error() << info.pgid << " log bound mismatch, info (tail,head] (" << pg_log.get_tail() << ","
                                 << pg_log.get_head() << "]"
                                 << " actual [" << pg_log.get_log().log.begin()->version << ","
                                 << pg_log.get_log().log.rbegin()->version << "]";
    }

    if (pg_log.get_log().caller_ops.size() > pg_log.get_log().log.size()) {
        pl->get_clog_error() << info.pgid << " caller_ops.size " << pg_log.get_log().caller_ops.size() << " > log size "
                             << pg_log.get_log().log.size();
    }
}
 ...
 */
bool PeeringState::search_for_missing(const pg_info_t& oinfo, const pg_missing_t& omissing, pg_shard_t from,
                                      PeeringCtxWrapper& ctx)
{
    uint64_t num_unfound_before = missing_loc.num_unfound();
    bool found_missing = missing_loc.add_source_info(from, oinfo, omissing, ctx.handle);
    if (found_missing && num_unfound_before != missing_loc.num_unfound()) pl->publish_stats_to_osd();
    ...
    if (found_missing && oinfo.last_update != eversion_t()) {
        pg_info_t tinfo(oinfo);
        tinfo.pgid.shard = pg_whoami.shard;
        ctx.send_info(from.osd,
...
bool PeeringState::discover_all_missing(BufferedRecoveryMessages& rctx)
{
    auto& missing = pg_log.get_missing();
    ...
    for (; m != mend; ++m) {
        ...
        auto iter = peer_info.find(peer);
        if (iter != peer_info.end() && (iter->second.is_empty() || iter->second.dne())) {
            // ignore empty peers
            continue;
        }
        ...
        rctx.send_query(peer.osd,
                        spg_t(info.pgid.pgid, peer.shard),
                        pg_query_t(pg_query_t::FULLLOG, peer.shard, pg_whoami.shard, info.history, get_osdmap_epoch()));
        any = true;
    }
    return any;
}
...
void PeeringState::build_might_have_unfound()
{
    ...
    might_have_unfound = past_intervals.get_might_have_unfound(pg_whoami, pool.info.is_erasure());

    // include any (stray) peers
    for (auto p = peer_info.begin(); p != peer_info.end(); ++p) might_have_unfound.insert(p->first);
...
void PeeringState::activate(ObjectStore::Transaction& t, epoch_t activation_epoch, PeeringCtxWrapper& ctx)
{
    ...
    pl->schedule_event_on_commit(
        ...
        std::make_shared<PGPeeringEvent>(
            get_osdmap_epoch(), get_osdmap_epoch(), ActivateCommitted(get_osdmap_epoch(), activation_epoch)));

    // init complete pointer
    if (missing.num_missing() == 0) {
        psdout(10) << "activate - no missing, moving last_complete " << info.last_complete << " -> " << info.last_update
                   << dendl;
    ...
    if (is_primary()) {
        ...
        for (auto i = acting_recovery_backfill.begin(); i != acting_recovery_backfill.end(); ++i) {
            ...
            if (pi.last_update == info.last_update) {
                ...
                if (!pi.last_backfill.is_max())
                    pl->get_clog_info() << info.pgid << " continuing backfill to osd." << peer << " from ("
                                        << pi.log_tail << "," << pi.last_update << "] " << pi.last_backfill << " to "
                                        << info.last_update;
                if (!pi.is_empty()) {
                    psdout(10) << "activate peer osd." << peer << " is up to date, queueing in pending_activators"
                               << dendl;
                 ...
                 */
                // backfill
                pl->get_clog_debug() << info.pgid << " starting backfill to osd." << peer << " from (" << pi.log_tail
                ...
                m = make_message<MOSDPGLog>(
                    i->shard, pg_whoami.shard, get_osdmap_epoch(), pi, last_peering_reset /* epoch to create pg at */);
            ...
            if (m && needs_past_intervals) m->past_intervals = past_intervals;
        ...
        for (auto i = acting_recovery_backfill.begin(); i != acting_recovery_backfill.end(); ++i) {
            ...
            if (*i == get_primary()) {
                ...
                if (!missing.have_missing()) complete_shards.insert(*i);
            }
            else {
                ...
                if (!peer_missing_entry->second.have_missing() && peer_info[*i].last_backfill.is_max())
        ...
        if (needs_recovery()) {
            ...
            for (auto i = peer_missing.begin(); i != peer_missing.end(); ++i) {
                if (is_acting_recovery_backfill(i->first)) continue;
    ...
    for (auto pg_shard : acting_recovery_backfill) {
        ...
        if (last_require_osd_release >= ceph_release_t::octopus) {
            ...
            m = make_message<MOSDPGInfo>(
                get_osdmap_epoch(),
                MOSDPGInfo::pg_list_t{pg_notify_t{
                    pg_shard.shard, pg_whoami.shard, get_osdmap_epoch(), get_osdmap_epoch(), info, past_intervals}});
        }
        pl->send_cluster_message(pg_shard.osd, m, get_osdmap_epoch());
    }
}

void PeeringState::merge_log(ObjectStore::Transaction& t, pg_info_t& oinfo, pg_log_t&& olog, pg_shard_t from)
{
    PGLog::LogEntryHandlerRef rollbacker{pl->get_log_handler(t)};
    pg_log.merge_log(oinfo, std::move(olog), from, info, rollbacker.get(), dirty_info, dirty_big_info);
}

void PeeringState::rewind_divergent_log(ObjectStore::Transaction& t, eversion_t newhead)
{
    PGLog::LogEntryHandlerRef rollbacker{pl->get_log_handler(t)};
    pg_log.rewind_divergent_log(newhead, info, rollbacker.get(), dirty_info, dirty_big_info);
}


void PeeringState::proc_primary_info(ObjectStore::Transaction& t, const pg_info_t& oinfo)
{
    ceph_assert(!is_primary());
...
void PeeringState::proc_master_log(ObjectStore::Transaction& t, pg_info_t& oinfo, pg_log_t&& olog,
...
void PeeringState::proc_replica_log(pg_info_t& oinfo, const pg_log_t& olog, pg_missing_t&& omissing, pg_shard_t from)
{
    ...
    for (auto i = omissing.get_items().begin(); i != omissing.get_items().end(); ++i) {
        psdout(20) << " after missing " << i->first << " need " << i->second.need << " have " << i->second.have
                   << dendl;
    }
    peer_missing[from].claim(std::move(omissing));
}

void PeeringState::fulfill_info(pg_shard_t from, const pg_query_t& query, pair<pg_shard_t, pg_info_t>& notify_info)
{
...
void PeeringState::fulfill_log(pg_shard_t from, const pg_query_t& query, epoch_t query_epoch)
{
    psdout(10) << "log request from " << from << dendl;
    ceph_assert(from == primary);
    ceph_assert(query.type != pg_query_t::INFO);

    auto mlog = make_message<MOSDPGLog>(from.shard, pg_whoami.shard, get_osdmap_epoch(), info, query_epoch);
    ...
    if (query.type == pg_query_t::LOG) {
        ...
        if (query.since != eversion_t() && query.since < pg_log.get_tail()) {
            pl->get_clog_error() << info.pgid << " got broken pg_query_t::LOG since " << query.since
...
void PeeringState::fulfill_query(const MQuery& query, PeeringCtxWrapper& rctx)
{
    if (query.query.type == pg_query_t::INFO) {
        ...
        rctx.send_notify(notify_info.first.osd,
                         pg_notify_t(notify_info.first.shard,
                                     pg_whoami.shard,
                                     query.query_epoch,
                                     get_osdmap_epoch(),
                                     notify_info.second,
            ...
            if (pool.info.is_pending_merge(info.pgid.pgid, &target)) {
                if (target) {
                    ...
                    pl->set_ready_to_merge_target(
                        info.last_update, info.history.last_epoch_started, info.history.last_epoch_clean);
                }
                else {
...
void PeeringState::split_into(pg_t child_pgid, PeeringState* child, unsigned split_bits)
{
    ...
    get_osdmap()->pg_to_up_acting_osds(child->info.pgid.pgid, &newup, &up_primary, &newacting, &primary);
    child->init_primary_up_acting(newup, newacting, up_primary, primary);
    child->role = OSDMap::calc_pg_role(pg_whoami, child->acting);

    // this comparison includes primary rank via pg_shard_t
    if (get_primary() != child->get_primary()) child->info.history.same_primary_since = get_osdmap_epoch();
...
void PeeringState::merge_from(map<spg_t, PeeringState*>& sources, PeeringCtx& rctx, unsigned split_bits,
                              const pg_merge_meta_t& last_pg_merge_meta)
{
    bool incomplete = false;
    if (info.last_complete != info.last_update || info.is_incomplete() || info.dne()) {
    ...
    if (last_pg_merge_meta.source_pgid != pg_t()) {
        if (info.pgid.pgid != last_pg_merge_meta.source_pgid.get_parent()) {
            psdout(10) << __func__ << " target doesn't match expected parent "
                       << last_pg_merge_meta.source_pgid.get_parent() << " of source_pgid "
                       << last_pg_merge_meta.source_pgid << dendl;
            incomplete = true;
        }
        if (info.last_update != last_pg_merge_meta.target_version) {
            psdout(10) << __func__ << " target version doesn't match expected " << last_pg_merge_meta.target_version
                       << dendl;
            incomplete = true;
        }
    }
    ...
    for (auto& i : sources) {
        ...
        if (source->info.last_complete != source->info.last_update || source->info.is_incomplete() ||
            source->info.dne()) {
        ...
        if (last_pg_merge_meta.source_pgid != pg_t()) {
            if (source->info.pgid.pgid != last_pg_merge_meta.source_pgid) {
                dout(10) << __func__ << " source " << source->info.pgid.pgid << " doesn't match expected source pgid "
                         << last_pg_merge_meta.source_pgid << dendl;
                incomplete = true;
            }
            if (source->info.last_update != last_pg_merge_meta.source_version) {
                dout(10) << __func__ << " source version doesn't match expected " << last_pg_merge_meta.target_version
                         << dendl;
                incomplete = true;
            }
        }
        ...
        // prepare log
        PGLog::LogEntryHandlerRef handler{source->pl->get_log_handler(rctx.transaction)};
    ...
    if (info.history.epoch_created == 0) {
        ...
        psdout(10) << __func__ << " set les/c to " << last_pg_merge_meta.last_epoch_started << "/"
                   << last_pg_merge_meta.last_epoch_clean << " from pool last_dec_*, source pg history was "
                   << sources.begin()->second->info.history << dendl;
        ...
        if (info.history.epoch_created == 0) {
            dout(10) << __func__ << " both merge target and source are placeholders,"
                     << " set sis to lec " << info.history.last_epoch_clean << dendl;
        ...
        if (info.history.last_epoch_clean < pib.first) {
            psdout(10) << __func__ << " last_epoch_clean " << info.history.last_epoch_clean << " < past_interval start "
        ...
        if (info.history.last_epoch_started < info.history.same_interval_since) {
            psdout(10) << __func__ << " last_epoch_started " << info.history.last_epoch_started
            ...
            info.history.same_up_since = std::min(info.history.same_up_since, info.history.same_interval_since);
            info.history.same_primary_since =
                std::min(info.history.same_primary_since, info.history.same_interval_since);
        }
    }
...
void PeeringState::start_split_stats(const set<spg_t>& childpgs, vector<object_stat_sum_t>* out)
{
    out->resize(childpgs.size() + 1);
    info.stats.stats.sum.split(*out);
}

void PeeringState::finish_split_stats(const object_stat_sum_t& stats, ObjectStore::Transaction& t)
{
...
void PeeringState::update_calc_stats()
{
    ...
    if (info.stats.stats.sum.num_objects < 0) {
        psdout(0) << __func__ << " negative num_objects = " << info.stats.stats.sum.num_objects << " setting it to 0 "
                  << dendl;
    ...
    if ((is_remapped() || is_undersized() || !is_clean()) && (is_peered() || is_activating())) {
        psdout(20) << __func__ << " actingset " << actingset << " upset " << upset << " acting_recovery_backfill "
...

        {
            ...
            if (missing == 0) info.stats.avail_no_missing.push_back(pg_whoami);
            psdout(20) << __func__ << " shard " << pg_whoami << " primary objects " << num_objects << " missing "
                       << missing << dendl;
        }
        ...
        for (auto& peer : peer_info) {
            ...
            int64_t missing = 0;
            int64_t peer_num_objects = std::max((int64_t)0, peer.second.stats.stats.sum.num_objects);
            ...
            if (missing == 0) info.stats.avail_no_missing.push_back(peer.first);
            psdout(20) << __func__ << " shard " << peer.first << " objects " << peer_num_objects << " missing "
                       << missing << dendl;
        }
        ...
        for (auto& ml : missing_loc.get_missing_locs()) {
            ...
            psdout(30) << __func__ << " " << ml.first << " object_location_counts[" << ml.second
                       << "]=" << info.stats.object_location_counts[ml.second] << dendl;
        }
        ...
        psdout(30) << __func__ << " object_location_counts[" << upset
                   << "]=" << info.stats.object_location_counts[upset] << dendl;
            ...
            for (auto& sml : missing_loc.get_missing_by_count()) {
                for (auto& ml : sml.second) {
                    ...
                    if (sml.first == shard_id_t::NO_SHARD) {
                        psdout(20) << __func__ << " ml " << ml.second << " upset size " << upset.size() << " up "
                                   << ml.first.up << dendl;
                        missing_shards = (int)upset.size() - ml.first.up;
                    }
                    else {
                        // Handle shards not even in upset below
                        if (!find_shard(upset, sml.first)) continue;
                        missing_shards = std::max(0, 1 - ml.first.up);
                        psdout(20) << __func__ << " shard " << sml.first << " ml " << ml.second << " missing shards "
                                   << missing_shards << dendl;
                    }
        ...
        if (pool.info.is_replicated()) {
            ...
            if (needed) missing_target_objects.emplace(num_objects * needed, pg_shard_t(pg_shard_t::NO_OSD));
        }
        else {
            for (unsigned i = 0; i < num_shards; ++i) {
                shard_id_t shard(i);
                ...
                if (!found) missing_target_objects.emplace(num_objects, pg_shard_t(pg_shard_t::NO_OSD, shard));
            }
        }

        for (const auto& item : missing_target_objects)
            psdout(20) << __func__ << " missing shard " << std::get<1>(item) << " missing= " << std::get<0>(item)
                       << dendl;
        for (const auto& item : acting_source_objects)
            psdout(20) << __func__ << " acting shard " << std::get<1>(item) << " missing= " << std::get<0>(item)
                       << dendl;

        // Handle all objects not in missing for remapped
        // or backfill
        for (auto m = missing_target_objects.rbegin(); m != missing_target_objects.rend(); ++m) {
...
std::optional<pg_stat_t> PeeringState::prepare_stats_for_publish(bool pg_stats_publish_valid,
                                                                 const pg_stat_t& pg_stats_publish,
                                                                 const object_stat_collection_t& unstable_stats)
{
    if (info.stats.stats.sum.num_scrub_errors) {
        psdout(10) << __func__ << " inconsistent due to " << info.stats.stats.sum.num_scrub_errors << " scrub errors"
                   << dendl;
        state_set(PG_STATE_INCONSISTENT);
    }
    else {
    ...
    if (info.stats.state != state) {
        ...
        if (!(state & PG_STATE_ACTIVE) && (info.stats.state & PG_STATE_ACTIVE)) info.stats.last_active = now;

        if ((state & PG_STATE_ACTIVE) && !(info.stats.state & PG_STATE_ACTIVE)) info.stats.last_became_active = now;
        if ((state & (PG_STATE_ACTIVE | PG_STATE_PEERED)) && !(info.stats.state & (PG_STATE_ACTIVE | PG_STATE_PEERED)))
    ...
    if (pg_stats_publish_valid && pre_publish == pg_stats_publish && info.stats.last_fresh > cutoff) {
        psdout(15) << "publish_stats_to_osd " << pg_stats_publish.reported_epoch << ": no change since "
                   << info.stats.last_fresh << dendl;
        return std::nullopt;
    }
    else {
        ...
        if (info.stats.state & PG_STATE_CLEAN) info.stats.last_clean = now;
        if (info.stats.state & PG_STATE_ACTIVE) info.stats.last_active = now;
        if (info.stats.state & (PG_STATE_ACTIVE | PG_STATE_PEERED)) info.stats.last_peered = now;
        info.stats.last_unstale = now;
        if ((info.stats.state & PG_STATE_DEGRADED) == 0) info.stats.last_undegraded = now;
        if ((info.stats.state & PG_STATE_UNDERSIZED) == 0) info.stats.last_fullsized = now;

        psdout(15) << "publish_stats_to_osd " << pg_stats_publish.reported_epoch << ":" << pg_stats_publish.reported_seq
                   << dendl;
        return std::make_optional(std::move(pre_publish));
    }
}

void PeeringState::init(int role, const vector<int>& newup, int new_up_primary, const vector<int>& newacting,
                        int new_acting_primary, const pg_history_t& history, const PastIntervals& pi, bool backfill,
                        ObjectStore::Transaction& t)
{
    psdout(10) << "init role " << role << " up " << newup << " acting " << newacting << " history " << history
               << " past_intervals " << pi << dendl;
...
void PeeringState::dump_peering_state(Formatter* f)
{
    ...
    for (auto p = up.begin(); p != up.end(); ++p) f->dump_unsigned("osd", *p);
    f->close_section();
    f->open_array_section("acting");
    for (auto p = acting.begin(); p != acting.end(); ++p) f->dump_unsigned("osd", *p);
    f->close_section();
    if (!backfill_targets.empty()) {
        f->open_array_section("backfill_targets");
        for (auto p = backfill_targets.begin(); p != backfill_targets.end(); ++p) f->dump_stream("shard") << *p;
        f->close_section();
    }
    if (!async_recovery_targets.empty()) {
        f->open_array_section("async_recovery_targets");
        for (auto p = async_recovery_targets.begin(); p != async_recovery_targets.end(); ++p)
            f->dump_stream("shard") << *p;
        f->close_section();
    }
    if (!acting_recovery_backfill.empty()) {
        f->open_array_section("acting_recovery_backfill");
        for (auto p = acting_recovery_backfill.begin(); p != acting_recovery_backfill.end(); ++p)
            f->dump_stream("shard") << *p;
        f->close_section();
    }
...
void PeeringState::update_stats(std::function<bool(pg_history_t&, pg_stat_t&)> f, ObjectStore::Transaction* t)
{
...
bool PeeringState::append_log_entries_update_missing(const mempool::osd_pglog::list<pg_log_entry_t>& entries,
                                                     ...
                                                     std::optional<eversion_t> roll_forward_to)
{
    ...
    PGLog::LogEntryHandlerRef rollbacker{pl->get_log_handler(t)};
    bool invalidate_stats = pg_log.append_new_log_entries(info.last_backfill, entries, rollbacker.get());
    ...
    if (trim_to) pg_log.trim(*trim_to, info);
    dirty_info = true;
    write_if_dirty(t);
    return invalidate_stats;
}

void PeeringState::merge_new_log_entries(const mempool::osd_pglog::list<pg_log_entry_t>& entries,
                                         ObjectStore::Transaction& t, std::optional<eversion_t> trim_to,
                                         std::optional<eversion_t> roll_forward_to)
{
    ...
    for (auto i = acting_recovery_backfill.begin(); i != acting_recovery_backfill.end(); ++i) {
        ...
        bool invalidate_stats =
            PGLog::append_log_entries_update_missing(pinfo.last_backfill, entries, true, NULL, pmissing, NULL, dpp);
    ...
    for (auto&& i : entries) {
        missing_loc.rebuild(
            i.soid, pg_whoami, acting_recovery_backfill, info, pg_log.get_missing(), peer_missing, peer_info);
    }
}
...
void PeeringState::add_log_entry(const pg_log_entry_t& e, bool applied)
{
    ...
    if (info.last_complete == info.last_update) info.last_complete = e.version;
    ...
    if (e.user_version > info.last_user_version) info.last_user_version = e.user_version;
...
void PeeringState::append_log(vector<pg_log_entry_t>&& logv, eversion_t trim_to, eversion_t roll_forward_to,
                              eversion_t mlcod, ObjectStore::Transaction& t, bool transaction_applied, bool async)
{
    ...
    for (auto p = logv.begin(); p != logv.end(); ++p) {
         ...
         * above */
        if (transaction_applied && p->soid > info.last_backfill) {
            pg_log.roll_forward(handler.get());
        }
    }
    if (transaction_applied && roll_forward_to > pg_log.get_can_rollback_to()) {
        pg_log.roll_forward_to(roll_forward_to, handler.get());
        last_rollback_info_trimmed_to_applied = roll_forward_to;
    }

    psdout(10) << __func__ << " approx pg log length =  " << pg_log.get_log().approx_size() << dendl;
    psdout(10) << __func__ << " dups pg log length =  " << pg_log.get_log().dups.size() << dendl;
    ...
    if (!is_primary()) min_last_complete_ondisk = mlcod;
}

void PeeringState::recover_got(const hobject_t& oid, eversion_t v, bool is_delete, ObjectStore::Transaction& t)
{
    ...
    if (pg_log.get_log().log.empty()) {
        ...
        psdout(10) << "last_complete now " << info.last_complete << " log.complete_to "
    ...
    if (is_primary()) {
        ...
        if (!is_delete) missing_loc.add_location(oid, pg_whoami);
    }
...
void PeeringState::update_backfill_progress(const hobject_t& updated_backfill, const pg_stat_t& updated_stats,
                                            bool preserve_local_num_bytes, ObjectStore::Transaction& t)
{
    info.set_last_backfill(updated_backfill);
    if (preserve_local_num_bytes) {
        psdout(25) << __func__ << " primary " << updated_stats.stats.sum.num_bytes << " local "
                   << info.stats.stats.sum.num_bytes << dendl;
        int64_t bytes = info.stats.stats.sum.num_bytes;
        info.stats = updated_stats;
        info.stats.stats.sum.num_bytes = bytes;
    }
    else {
        psdout(20) << __func__ << " final " << updated_stats.stats.sum.num_bytes << " replaces local "
                   << info.stats.stats.sum.num_bytes << dendl;
        info.stats = updated_stats;
    }
...
void PeeringState::adjust_purged_snaps(std::function<void(interval_set<snapid_t>& snaps)> f)
{
    f(info.purged_snaps);
    dirty_info = true;
    dirty_big_info = true;
...
void PeeringState::on_peer_recover(pg_shard_t peer, const hobject_t& soid, const eversion_t& version)
{
    pl->publish_stats_to_osd();
    // done!
...
void PeeringState::begin_peer_recover(pg_shard_t peer, const hobject_t soid)
{
    peer_missing[peer].revise_have(soid, eversion_t());
}

void PeeringState::force_object_missing(const set<pg_shard_t>& peers, const hobject_t& soid, eversion_t version)
{
    for (auto&& peer : peers) {
        if (peer != primary) {
    ...
    missing_loc.rebuild(soid, pg_whoami, acting_recovery_backfill, info, pg_log.get_missing(), peer_missing, peer_info);
}

void PeeringState::pre_submit_op(const hobject_t& hoid, const vector<pg_log_entry_t>& logv, eversion_t at_version)
{
    if (at_version > eversion_t()) {
        for (auto&& i : get_acting_recovery_backfill()) {
            ...
            if (pinfo.last_complete == pinfo.last_update) pinfo.last_complete = at_version;
    ...
    for (auto&& i : get_async_recovery_targets()) {
        if (i == primary || !get_peer_missing(i).is_missing(hoid)) continue;
    ...
    if (requires_missing_loc) {
        for (auto&& entry : logv) {
            ...
            for (auto& i : get_actingset()) {
                if (!get_peer_missing(i).is_missing(entry.soid)) missing_loc.add_location(entry.soid, i);
            }
        ...
        if (!is_primary()) {
            ...
            pl->send_cluster_message(
                ...
                make_message<MOSDPGTrim>(
                    get_osdmap_epoch(), spg_t(info.pgid.pgid, primary.shard), last_complete_ondisk),
                get_osdmap_epoch());
        }
        else {
...
void PeeringState::calc_trim_to()
{
    ...
    eversion_t limit = std::min(min_last_complete_ondisk, pg_log.get_can_rollback_to());
    if (limit != eversion_t() && limit != pg_trim_to && pg_log.get_log().approx_size() > target) {
        size_t num_to_trim = std::min(pg_log.get_log().approx_size() - target, cct->_conf->osd_pg_log_trim_max);
...
void PeeringState::calc_trim_to_aggressive()
{
    ...
    eversion_t limit = std::min({pg_log.get_head(), pg_log.get_can_rollback_to(), last_update_ondisk});
    psdout(10) << __func__ << " limit = " << limit << dendl;

    if (limit != eversion_t() && limit != pg_trim_to && pg_log.get_log().approx_size() > target) {
        psdout(10) << __func__ << " approx pg log length =  " << pg_log.get_log().approx_size() << dendl;
        uint64_t num_to_trim =
            std::min<uint64_t>(pg_log.get_log().approx_size() - target, cct->_conf->osd_pg_log_trim_max);
        ...
        for (size_t i = 0; it != pg_log.get_log().log.end(); ++it, ++rit) {
            ...
            if (by_n_to_keep != eversion_t() && by_n_to_trim != eversion_t::max()) {
                break;
            }
        }
...
void PeeringState::apply_op_stats(const hobject_t& soid, const object_stat_sum_t& delta_stats)
{
    info.stats.stats.add(delta_stats);
    info.stats.stats.floor(0);

    for (auto i = get_backfill_targets().begin(); i != get_backfill_targets().end(); ++i) {
        pg_shard_t bt = *i;
        pg_info_t& pinfo = peer_info[bt];
        if (soid <= pinfo.last_backfill) pinfo.stats.stats.add(delta_stats);
    }
}

void PeeringState::update_complete_backfill_object_stats(const hobject_t& hoid, const pg_stat_t& stats)
{
    for (auto&& bt : get_backfill_targets()) {
        pg_info_t& pinfo = peer_info[bt];
        // Add stats to all peers that were missing object
        if (hoid > pinfo.last_backfill) pinfo.stats.add(stats);
    }
}

void PeeringState::update_peer_last_backfill(pg_shard_t peer, const hobject_t& new_last_backfill)
{
    pg_info_t& pinfo = peer_info[peer];
...
void PeeringState::set_revert_with_targets(const hobject_t& soid, const set<pg_shard_t>& good_peers)
{
...
void PeeringState::prepare_backfill_for_missing(const hobject_t& soid, const eversion_t& version,
...
#undef dout_prefix
#define dout_prefix (context<PeeringMachine>().dpp->gen_prefix(*_dout) << "state<" << get_state_name() << ">: ")
#undef psdout
...
{
    DECLARE_LOCALS;
    ps->proc_replica_info(notify.from, notify.notify.info, notify.notify.epoch_sent);
...
boost::statechart::result PeeringState::Started::react(const IntervalFlush&)
{
...
boost::statechart::result PeeringState::Started::react(const AdvMap& advmap)
{
    ...
    if (ps->should_restart_peering(
            advmap.up_primary, advmap.acting_primary, advmap.newup, advmap.newacting, advmap.lastmap, advmap.osdmap)) {
...
boost::statechart::result PeeringState::Reset::react(const IntervalFlush&)
{
    psdout(10) << "Ending blocked outgoing recovery messages" << dendl;
    context<PeeringMachine>().state->end_block_outgoing();
    return discard_event();
...
boost::statechart::result PeeringState::Reset::react(const AdvMap& advmap)
{
    ...
    if (ps->should_restart_peering(
        ...
        ps->start_peering_interval(advmap.lastmap,
                                   advmap.newup,
                                   advmap.up_primary,
                                   advmap.newacting,
...
boost::statechart::result PeeringState::Reset::react(const ActMap&)
{
    ...
    if (ps->should_send_notify() && ps->get_primary().osd >= 0) {
        ps->info.history.refresh_prior_readable_until_ub(pl->get_mnow(), ps->prior_readable_until_ub);
        context<PeeringMachine>().send_notify(ps->get_primary().osd,
                                              pg_notify_t(ps->get_primary().shard,
                                                          ps->pg_whoami.shard,
                                                          ps->get_osdmap_epoch(),
                                                          ps->get_osdmap_epoch(),
                                                          ps->info,
...
boost::statechart::result PeeringState::Primary::react(const MNotifyRec& notevt)
{
    ...
    ps->proc_replica_info(notevt.from, notevt.notify.info, notevt.notify.epoch_sent);
    return discard_event();
}
...
boost::statechart::result PeeringState::Primary::react(const SetForceRecovery&)
{
    DECLARE_LOCALS;
...
boost::statechart::result PeeringState::Primary::react(const UnsetForceRecovery&)
{
    DECLARE_LOCALS;
...
boost::statechart::result PeeringState::Primary::react(const RequestScrub& evt)
{
    DECLARE_LOCALS;
    if (ps->is_primary()) {
...
boost::statechart::result PeeringState::Primary::react(const SetForceBackfill&)
{
    DECLARE_LOCALS;
...
boost::statechart::result PeeringState::Primary::react(const UnsetForceBackfill&)
{
    DECLARE_LOCALS;
...
boost::statechart::result PeeringState::Peering::react(const QueryState& q)
{
    ...
    for (auto p = prior_set.probe.begin(); p != prior_set.probe.end(); ++p) q.f->dump_stream("osd") << *p;
    q.f->close_section();

    if (prior_set.pg_down) q.f->dump_string("blocked", "peering is blocked due to down osds");

    q.f->open_array_section("down_osds_we_would_probe");
    for (auto p = prior_set.down.begin(); p != prior_set.down.end(); ++p) q.f->dump_int("osd", *p);
    q.f->close_section();

    q.f->open_array_section("peering_blocked_by");
    for (auto p = prior_set.blocked_by.begin(); p != prior_set.blocked_by.end(); ++p) {
...
void PeeringState::Backfilling::backfill_release_reservations()
{
    ...
    for (auto it = ps->backfill_targets.begin(); it != ps->backfill_targets.end(); ++it) {
        ceph_assert(*it != ps->pg_whoami);
        pl->send_cluster_message(
            it->osd,
            make_message<MBackfillReserve>(
                MBackfillReserve::RELEASE, spg_t(ps->info.pgid.pgid, it->shard), ps->get_osdmap_epoch()),
            ps->get_osdmap_epoch());
    }
}
...
boost::statechart::result PeeringState::Backfilling::react(const Backfilled& c)
{
    backfill_release_reservations();
    return transit<Recovered>();
}

boost::statechart::result PeeringState::Backfilling::react(const DeferBackfill& c)
{
    DECLARE_LOCALS;
    ...
    pl->schedule_event_after(
        std::make_shared<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), RequestBackfill()), c.delay);
    return transit<NotBackfilling>();
}

boost::statechart::result PeeringState::Backfilling::react(const UnfoundBackfill& c)
{
    DECLARE_LOCALS;
...
boost::statechart::result PeeringState::Backfilling::react(const RemoteReservationRevokedTooFull&)
{
    DECLARE_LOCALS;
    ...
    pl->schedule_event_after(
        std::make_shared<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), RequestBackfill()),
        ps->cct->_conf->osd_backfill_retry_interval);

    return transit<NotBackfilling>();
}

boost::statechart::result PeeringState::Backfilling::react(const RemoteReservationRevoked&)
{
    DECLARE_LOCALS;
    ps->state_set(PG_STATE_BACKFILL_WAIT);
    cancel_backfill();
...
boost::statechart::result PeeringState::WaitRemoteBackfillReserved::react(const RemoteBackfillReserved& evt)
{
    DECLARE_LOCALS;
    ...
    if (backfill_osd_it != context<Active>().remote_shards_to_reserve_backfill.end()) {
        ...
        pl->send_cluster_message(
            backfill_osd_it->osd,
            make_message<MBackfillReserve>(MBackfillReserve::REQUEST,
...
void PeeringState::WaitRemoteBackfillReserved::retry()
{
    ...
    for (it = begin; it != backfill_osd_it; ++it) {
        // The primary never backfills itself
        ceph_assert(*it != ps->pg_whoami);
        pl->send_cluster_message(it->osd,
                                 make_message<MBackfillReserve>(MBackfillReserve::RELEASE,
...
boost::statechart::result PeeringState::WaitRemoteBackfillReserved::react(const RemoteReservationRejectedTooFull& evt)
{
    DECLARE_LOCALS;
    ps->state_set(PG_STATE_BACKFILL_TOOFULL);
    retry();
...
boost::statechart::result PeeringState::WaitRemoteBackfillReserved::react(const RemoteReservationRevoked& evt)
{
    retry();
    return transit<NotBackfilling>();
}
    ...
    , NamedState(context<PeeringMachine>().state_history, "Started/Primary/Active/WaitLocalBackfillReserved")
{
    ...
    pl->request_local_background_io_reservation(
        ps->get_backfill_priority(),
        std::make_unique<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), LocalBackfillReserved()),
        std::make_unique<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DeferBackfill(0.0)));
    pl->publish_stats_to_osd();
}
...
boost::statechart::result PeeringState::NotBackfilling::react(const RemoteBackfillReserved& evt)
{
    return discard_event();
}

boost::statechart::result PeeringState::NotBackfilling::react(const RemoteReservationRejectedTooFull& evt)
{
    return discard_event();
}
...
boost::statechart::result PeeringState::RepNotRecovering::react(const RejectTooFullRemoteReservation& evt)
{
    DECLARE_LOCALS;
...
boost::statechart::result PeeringState::RepWaitRecoveryReserved::react(const RemoteRecoveryReserved& evt)
{
    DECLARE_LOCALS;
    pl->send_cluster_message(
        ps->primary.osd,
        make_message<MRecoveryReserve>(
            MRecoveryReserve::GRANT, spg_t(ps->info.pgid.pgid, ps->primary.shard), ps->get_osdmap_epoch()),
        ps->get_osdmap_epoch());
    return transit<RepRecovering>();
}

boost::statechart::result PeeringState::RepWaitRecoveryReserved::react(const RemoteReservationCanceled& evt)
{
    DECLARE_LOCALS;
    pl->unreserve_recovery_space();
...
boost::statechart::result PeeringState::RepNotRecovering::react(const RequestBackfillPrio& evt)
{

    DECLARE_LOCALS;

    if (!pl->try_reserve_recovery_space(evt.primary_num_bytes, evt.local_num_bytes)) {
        ...
        if (HAVE_FEATURE(ps->upacting_features, RECOVERY_RESERVATION_2)) {
            ...
            preempt = std::make_unique<PGPeeringEvent>(
                pl->get_osdmap_epoch(), pl->get_osdmap_epoch(), RemoteBackfillPreempted());
        }
        pl->request_remote_recovery_reservation(
            evt.priority,
            std::make_unique<PGPeeringEvent>(pl->get_osdmap_epoch(), pl->get_osdmap_epoch(), RemoteBackfillReserved()),
            std::move(preempt));
    }
    return transit<RepWaitBackfillReserved>();
}

boost::statechart::result PeeringState::RepNotRecovering::react(const RequestRecoveryPrio& evt)
{
    DECLARE_LOCALS;
    ...
    if (HAVE_FEATURE(ps->upacting_features, RECOVERY_RESERVATION_2)) {
        ...
        preempt =
            std::make_unique<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), RemoteRecoveryPreempted());
    }

    pl->request_remote_recovery_reservation(
        prio,
        std::make_unique<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), RemoteRecoveryReserved()),
        std::move(preempt));
...
boost::statechart::result PeeringState::RepWaitBackfillReserved::react(const RemoteBackfillReserved& evt)
{
    DECLARE_LOCALS;
    ...
    pl->send_cluster_message(
        ps->primary.osd,
        make_message<MBackfillReserve>(
            MBackfillReserve::GRANT, spg_t(ps->info.pgid.pgid, ps->primary.shard), ps->get_osdmap_epoch()),
        ps->get_osdmap_epoch());
    return transit<RepRecovering>();
}

boost::statechart::result PeeringState::RepWaitBackfillReserved::react(const RejectTooFullRemoteReservation& evt)
{
    DECLARE_LOCALS;
    ps->reject_reservation();
    post_event(RemoteReservationRejectedTooFull());
...
boost::statechart::result PeeringState::RepWaitBackfillReserved::react(const RemoteReservationRejectedTooFull& evt)
{
    DECLARE_LOCALS;
    pl->unreserve_recovery_space();
...
boost::statechart::result PeeringState::RepWaitBackfillReserved::react(const RemoteReservationCanceled& evt)
{
    DECLARE_LOCALS;
    pl->unreserve_recovery_space();
...
boost::statechart::result PeeringState::RepRecovering::react(const RemoteRecoveryPreempted&)
{
    DECLARE_LOCALS;
    ...
    pl->send_cluster_message(
        ps->primary.osd,
        make_message<MRecoveryReserve>(
            MRecoveryReserve::REVOKE, spg_t(ps->info.pgid.pgid, ps->primary.shard), ps->get_osdmap_epoch()),
        ps->get_osdmap_epoch());
    return discard_event();
}

boost::statechart::result PeeringState::RepRecovering::react(const BackfillTooFull&)
{
    DECLARE_LOCALS;
    ...
    pl->send_cluster_message(
        ps->primary.osd,
        make_message<MBackfillReserve>(
            MBackfillReserve::REVOKE_TOOFULL, spg_t(ps->info.pgid.pgid, ps->primary.shard), ps->get_osdmap_epoch()),
        ps->get_osdmap_epoch());
    return discard_event();
}

boost::statechart::result PeeringState::RepRecovering::react(const RemoteBackfillPreempted&)
{
    DECLARE_LOCALS;
    ...
    pl->send_cluster_message(
        ps->primary.osd,
        make_message<MBackfillReserve>(
            MBackfillReserve::REVOKE, spg_t(ps->info.pgid.pgid, ps->primary.shard), ps->get_osdmap_epoch()),
        ps->get_osdmap_epoch());
    return discard_event();
}
    ...
    , NamedState(context<PeeringMachine>().state_history, "Started/Primary/Active/WaitLocalRecoveryReserved")
{
    ...
    pl->request_local_background_io_reservation(
        ...
        std::make_unique<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), LocalRecoveryReserved()),
        std::make_unique<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DeferRecovery(0.0)));
    pl->publish_stats_to_osd();
}

boost::statechart::result PeeringState::WaitLocalRecoveryReserved::react(const RecoveryTooFull& evt)
{
    DECLARE_LOCALS;
    ps->state_set(PG_STATE_RECOVERY_TOOFULL);
    pl->schedule_event_after(
        std::make_shared<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DoRecovery()),
...
boost::statechart::result PeeringState::WaitRemoteRecoveryReserved::react(const RemoteRecoveryReserved& evt)
{
    DECLARE_LOCALS;
    ...
    if (remote_recovery_reservation_it != context<Active>().remote_shards_to_reserve_recovery.end()) {
        ...
        pl->send_cluster_message(remote_recovery_reservation_it->osd,
                                 make_message<MRecoveryReserve>(
...
void PeeringState::Recovering::release_reservations(bool cancel)
{
    ...
    for (auto i = context<Active>().remote_shards_to_reserve_recovery.begin();
        ...
        pl->send_cluster_message(
            i->osd,
            make_message<MRecoveryReserve>(
                MRecoveryReserve::RELEASE, spg_t(ps->info.pgid.pgid, i->shard), ps->get_osdmap_epoch()),
            ps->get_osdmap_epoch());
    }
}

boost::statechart::result PeeringState::Recovering::react(const AllReplicasRecovered& evt)
{
    DECLARE_LOCALS;
...
boost::statechart::result PeeringState::Recovering::react(const RequestBackfill& evt)
{
    DECLARE_LOCALS;
...
boost::statechart::result PeeringState::Recovering::react(const DeferRecovery& evt)
{
    DECLARE_LOCALS;
    ...
    pl->schedule_event_after(
        std::make_shared<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DoRecovery()), evt.delay);
    return transit<NotRecovering>();
}

boost::statechart::result PeeringState::Recovering::react(const UnfoundRecovery& evt)
{
    DECLARE_LOCALS;
    ...
    , NamedState(context<PeeringMachine>().state_history, "Started/Primary/Active/Recovered")
{
    ...
    if (ps->get_osdmap()->get_pg_size(context<PeeringMachine>().spgid.pgid) <= ps->acting_recovery_backfill.size()) {
        ps->state_clear(PG_STATE_FORCED_BACKFILL | PG_STATE_FORCED_RECOVERY);
        pl->publish_stats_to_osd();
    }
    ...
    bool history_les_bound = false;
    if (ps->acting != ps->up && !ps->choose_acting(auth_log_shard, true, &history_les_bound)) {
    ...
    if (context<Active>().all_replicas_activated && ps->async_recovery_targets.empty()) post_event(GoClean());
}
    ...
    , NamedState(context<PeeringMachine>().state_history, "Started/Primary/Active/Clean")
{
    ...
    context<PeeringMachine>().get_cur_transaction().register_on_commit(pl->on_clean());
}
...
template<typename T> set<pg_shard_t> unique_osd_shard_set(const pg_shard_t& skip, const T& in)
{
    set<int> osds_found;
    set<pg_shard_t> out;
...
PeeringState::Active::Active(my_context ctx)
    ...
    , remote_shards_to_reserve_recovery(unique_osd_shard_set(context<PeeringMachine>().state->pg_whoami,
                                                             context<PeeringMachine>().state->acting_recovery_backfill))
    , remote_shards_to_reserve_backfill(unique_osd_shard_set(context<PeeringMachine>().state->pg_whoami,
    ...
    , all_replicas_activated(false)
{
    ...
    ps->blocked_by.clear();
    for (auto p = ps->acting_recovery_backfill.begin(); p != ps->acting_recovery_backfill.end(); ++p) {
...
boost::statechart::result PeeringState::Active::react(const AdvMap& advmap)
{
    ...
    if (advmap.lastmap->get_pg_size(ps->info.pgid.pgid) != ps->get_osdmap()->get_pg_size(ps->info.pgid.pgid)) {
        if (ps->get_osdmap()->get_pg_size(ps->info.pgid.pgid) <= ps->actingset.size()) {
    ...
    if (need_publish) pl->publish_stats_to_osd();
...
boost::statechart::result PeeringState::Active::react(const ActMap&)
{
    ...
    if (unfound > 0 && ps->all_unfound_are_queried_or_lost(ps->get_osdmap())) {
...
boost::statechart::result PeeringState::Active::react(const MNotifyRec& notevt)
{
    ...
    if (ps->peer_info.count(notevt.from)) {
        psdout(10) << "Active: got notify from " << notevt.from << ", already have info from that osd, ignoring"
                   << dendl;
    }
        ...
        psdout(10) << "Active: got notify from " << notevt.from
                   << ", calling proc_replica_info and discover_all_missing" << dendl;
        ps->proc_replica_info(notevt.from, notevt.notify.info, notevt.notify.epoch_sent);
        ...
        if (!ps->want_acting.empty() && ps->want_acting != ps->acting) {
            psdout(10) << "Active: got notify from previous acting member " << notevt.from
...
boost::statechart::result PeeringState::Active::react(const MTrim& trim)
{
    ...
    ps->update_peer_last_complete_ondisk(pg_shard_t{trim.from, trim.shard}, trim.trim_to);
...
boost::statechart::result PeeringState::Active::react(const MInfoRec& infoevt)
{
    ...
    if (ps->is_acting_recovery_backfill(infoevt.from) && ps->peer_activated.count(infoevt.from) == 0) {
...
boost::statechart::result PeeringState::Active::react(const MLogRec& logevt)
{
    ...
    ps->proc_replica_log(logevt.msg->info, logevt.msg->log, std::move(logevt.msg->missing), logevt.from);
    bool got_missing = ps->search_for_missing(ps->peer_info[logevt.from],
...
boost::statechart::result PeeringState::Active::react(const QueryState& q)
{
...

    {
        ...
        for (auto p = ps->might_have_unfound.begin(); p != ps->might_have_unfound.end(); ++p) {
            q.f->open_object_section("osd");
    ...
    }
    {
        ...
        for (auto p = ps->backfill_targets.begin(); p != ps->backfill_targets.end(); ++p)
...
boost::statechart::result PeeringState::Active::react(const ActivateCommitted& evt)
{
    DECLARE_LOCALS;
    ceph_assert(!ps->peer_activated.count(ps->pg_whoami));
    ps->peer_activated.insert(ps->pg_whoami);
    psdout(10) << "_activate_committed " << evt.epoch << " peer_activated now " << ps->peer_activated
               << " last_interval_started " << ps->info.history.last_interval_started << " last_epoch_started "
               << ps->info.history.last_epoch_started << " same_interval_since " << ps->info.history.same_interval_since
               << dendl;
    ceph_assert(!ps->acting_recovery_backfill.empty());
    if (ps->peer_activated.size() == ps->acting_recovery_backfill.size()) all_activated_and_committed();
    return discard_event();
}
...
boost::statechart::result PeeringState::Active::react(const AllReplicasActivated& evt)
{
    ...
    if (ps->prior_readable_until_ub > mnow) {
        psdout(10) << " waiting for prior_readable_until_ub " << ps->prior_readable_until_ub << " > mnow " << mnow
                   << dendl;
...
boost::statechart::result PeeringState::ReplicaActive::react(const Activate& actevt)
{
    DECLARE_LOCALS;
...
boost::statechart::result PeeringState::ReplicaActive::react(const ActivateCommitted& evt)
{
    DECLARE_LOCALS;
    ...
    rctx.send_info(ps->get_primary().osd,
...
boost::statechart::result PeeringState::ReplicaActive::react(const MLease& l)
{
    ...
    pl->send_cluster_message(
        ps->get_primary().osd,
        make_message<MOSDPGLeaseAck>(epoch, spg_t(spgid.pgid, ps->get_primary().shard), ps->get_lease_ack()),
        epoch);
    return discard_event();
}
...
{
    DECLARE_LOCALS;
    ps->proc_primary_info(context<PeeringMachine>().get_cur_transaction(), infoevt.info);
    return discard_event();
}
...
boost::statechart::result PeeringState::ReplicaActive::react(const MQuery& query)
{
    DECLARE_LOCALS;
...
PeeringState::WaitDeleteReserved::WaitDeleteReserved(my_context ctx)
    : my_base(ctx)
    , NamedState(context<PeeringMachine>().state_history, "Started/ToDelete/WaitDeleteReseved")
{
    context<PeeringMachine>().log_enter(state_name);
    DECLARE_LOCALS;
    ...
    pl->request_local_background_io_reservation(
        ...
        std::make_unique<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DeleteReserved()),
        std::make_unique<PGPeeringEvent>(ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DeleteInterrupted()));
}

boost::statechart::result PeeringState::ToDelete::react(const ActMap& evt)
{
    DECLARE_LOCALS;
...
boost::statechart::result PeeringState::Deleting::react(const DeleteSome& evt)
{
    ...
    p = pl->do_delete_work(context<PeeringMachine>().get_cur_transaction(), next);
    next = p.first;
    ...
    , NamedState(context<PeeringMachine>().state_history, "Started/Primary/Peering/GetInfo")
{
    ...
    if (ps->prior_readable_down_osds.empty()) {
        psdout(10) << " no prior_set down osds, will clear prior_readable_until_ub before activating" << dendl;
    }
    ...
    for (auto it = prior_set.probe.begin(); it != prior_set.probe.end(); ++it) {
        ...
        if (peer_info_requested.count(peer)) {
            ...
            context<PeeringMachine>().send_query(
                peer.osd,
                pg_query_t(pg_query_t::INFO, it->shard, ps->pg_whoami.shard, ps->info.history, ps->get_osdmap_epoch()));
...
boost::statechart::result PeeringState::GetInfo::react(const MNotifyRec& infoevt)
{
    ...
    if (ps->proc_replica_info(infoevt.from, infoevt.notify.info, infoevt.notify.epoch_sent)) {
        ...
        psdout(20) << "Adding osd: " << infoevt.from.osd << " peer features: " << hex << infoevt.features << dec
                   << dendl;
...
boost::statechart::result PeeringState::GetInfo::react(const QueryState& q)
{
    ...
    for (auto p = peer_info_requested.begin(); p != peer_info_requested.end(); ++p) {
        q.f->open_object_section("osd");
        q.f->dump_stream("osd") << *p;
    ...
    , msg(0)
{
    ...
    for (auto p = ps->acting_recovery_backfill.begin(); p != ps->acting_recovery_backfill.end(); ++p) {
        ...
        if (ri.last_update < ps->info.log_tail && ri.last_update >= best.log_tail && ri.last_update < request_log_from)
...
boost::statechart::result PeeringState::Down::react(const QueryState& q)
{
    ...
    q.f->dump_string("name", state_name);
    q.f->dump_stream("enter_time") << enter_time;
    q.f->dump_string("comment", "not enough up instances of this PG to go active");
    q.f->close_section();
    return forward_event();
}
...
boost::statechart::result PeeringState::Incomplete::react(const AdvMap& advmap)
{
    ...
    if (!advmap.osdmap->have_pg_pool(poolnum) || advmap.lastmap->get_pools().find(poolnum)->second.min_size >
...
boost::statechart::result PeeringState::Incomplete::react(const MNotifyRec& notevt)
{
    DECLARE_LOCALS;
    psdout(7) << "handle_pg_notify from osd." << notevt.from << dendl;
    if (ps->proc_replica_info(notevt.from, notevt.notify.info, notevt.notify.epoch_sent)) {
...
boost::statechart::result PeeringState::Incomplete::react(const QueryState& q)
{
    q.f->open_object_section("state");
    q.f->dump_string("name", state_name);
    q.f->dump_stream("enter_time") << enter_time;
    ...
    , NamedState(context<PeeringMachine>().state_history, "Started/Primary/Peering/GetMissing")
{
    ...
    ceph_assert(!ps->acting_recovery_backfill.empty());
    eversion_t since;
    for (auto i = ps->acting_recovery_backfill.begin(); i != ps->acting_recovery_backfill.end(); ++i) {
        ...
        if (pi.is_empty()) continue;   // no pg data, nothing divergent
        ...
        if (pi.log_tail <= since) {
            ...
            context<PeeringMachine>().send_query(
                i->osd,
                pg_query_t(
                    pg_query_t::LOG, i->shard, ps->pg_whoami.shard, since, ps->info.history, ps->get_osdmap_epoch()));
        }
        else {
            psdout(10) << " requesting fulllog+missing from osd." << *i << " (want since " << since << " < log.tail "
                       << pi.log_tail << ")" << dendl;
            context<PeeringMachine>().send_query(
                i->osd,
                pg_query_t(
                    pg_query_t::FULLLOG, i->shard, ps->pg_whoami.shard, ps->info.history, ps->get_osdmap_epoch()));
        }
...
boost::statechart::result PeeringState::GetMissing::react(const QueryState& q)
{
    ...
    for (auto p = peer_missing_requested.begin(); p != peer_missing_requested.end(); ++p) {
        q.f->open_object_section("osd");
        q.f->dump_stream("osd") << *p;
        if (ps->peer_missing.count(*p)) {
...
{
    out << "pg[" << ps.info << " " << pg_vector_string(ps.up);
    if (ps.acting != ps.up) out << "/" << pg_vector_string(ps.acting);
    if (ps.is_ec_pg()) out << "p" << ps.get_primary();
    if (!ps.async_recovery_targets.empty()) out << " async=[" << ps.async_recovery_targets << "]";
    if (!ps.backfill_targets.empty()) out << " backfill=[" << ps.backfill_targets << "]";
    out << " r=" << ps.get_role();
    out << " lpr=" << ps.get_last_peering_reset();

    if (ps.deleting) out << " DELETING";

    if (!ps.past_intervals.empty()) {
        out << " pi=[" << ps.past_intervals.get_bounds() << ")/" << ps.past_intervals.size();
    }

    if (ps.is_peered()) {
        if (ps.last_update_ondisk != ps.info.last_update) out << " luod=" << ps.last_update_ondisk;
        if (ps.last_update_applied != ps.info.last_update) out << " lua=" << ps.last_update_applied;
    }

    if (ps.pg_log.get_tail() != ps.info.log_tail || ps.pg_log.get_head() != ps.info.last_update)
        out << " (info mismatch, " << ps.pg_log.get_log() << ")";

    if (!ps.pg_log.get_log().empty()) {
        if ((ps.pg_log.get_log().log.begin()->version <= ps.pg_log.get_tail())) {
            out << " (log bound mismatch, actual=[" << ps.pg_log.get_log().log.begin()->version << ","
    ...
    if (ps.last_complete_ondisk != ps.info.last_complete) out << " lcod " << ps.last_complete_ondisk;

    out << " mlcod " << ps.min_last_complete_ondisk;

    out << " " << pg_state_string(ps.get_state());
    if (ps.should_send_notify()) out << " NOTIFY";

    if (ps.prior_readable_until_ub != ceph::signedspan::zero()) {
        out << " pruub " << ps.prior_readable_until_ub << "@" << ps.get_prior_readable_down_osds();
    }
    return out;
}
