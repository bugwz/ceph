...
{
    out << "up_from " << info.up_from << " up_thru " << info.up_thru << " down_at " << info.down_at
        << " last_clean_interval [" << info.last_clean_begin << "," << info.last_clean_end << ")";
    if (info.lost_at) out << " lost_at " << info.lost_at;
    return out;
}
...
{
    return out << "down_stamp " << xi.down_stamp << " laggy_probability " << xi.laggy_probability << " laggy_interval "
               << xi.laggy_interval << " old_weight " << xi.old_weight << " last_purged_snaps_scrub "
               << xi.last_purged_snaps_scrub << " dead_epoch " << xi.dead_epoch;
}
...
int OSDMap::Incremental::identify_osd(uuid_d u) const
{
    for (auto& uuid : new_uuid)
        if (uuid.second == u) return uuid.first;
    return -1;
}

int OSDMap::Incremental::propagate_base_properties_to_tiers(CephContext* cct, const OSDMap& osdmap)
{
    ...
    for (auto& new_pool : new_pools) {
        if (!new_pool.second.tiers.empty()) {
            ...
            for (const auto& tier_pool : base.tiers) {
                ...
                tier->flags |= base.flags & (pg_pool_t::FLAG_SELFMANAGED_SNAPS | pg_pool_t::FLAG_POOL_SNAPS);
...
{
    if (id >= 0) return is_down(id);

    if (down_cache && down_cache->count(id)) {
        return true;
    }
...
bool OSDMap::containing_subtree_is_down(CephContext* cct, int id, int subtree_type, set<int>* down_cache) const
{
    ...
    while (true) {
        ...
        if (type >= subtree_type) {
            ldout(cct, 30) << "containing_subtree_is_down(" << id << ") = true ... " << type << " >= " << subtree_type
                           << dendl;
            return true;
        }
...
bool OSDMap::subtree_type_is_down(CephContext* cct, int id, int subtree_type, set<int>* down_in_osds,
                                  set<int>* up_in_osds, set<int>* subtree_up,
                                  unordered_map<int, set<int>>* subtree_type_down) const
{
    ...
    if (subtree_type_down && (*subtree_type_down)[subtree_type].count(id)) {
        return true;
    }
    ...
    for (const auto& child : children) {
        if (!subtree_type_is_down(
                cct, child, crush->get_bucket_type(child), down_in_osds, up_in_osds, subtree_up, subtree_type_down)) {
...
template<class T> static void encode_addrvec_map_as_addr(const T& m, ceph::buffer::list& bl, uint64_t f)
{
    uint32_t n = m.size();
    encode(n, bl);
    for (auto& i : m) {
...
template<class T> static void encode_addrvec_pvec_as_addr(const T& m, ceph::buffer::list& bl, uint64_t f)
{
    uint32_t n = m.size();
    encode(n, bl);
    for (auto& i : m) {
...
void OSDMap::Incremental::encode(ceph::buffer::list& bl, uint64_t features) const
{
...

    {
        ...
        if (!new_range_blocklist.empty() || !old_range_blocklist.empty()) {
...
void OSDMap::Incremental::decode_classic(ceph::buffer::list::const_iterator& p)
{
    ...
    if (v < 5) decode(new_pool_names, p);
    decode(new_up_thru, p);
    decode(new_last_clean_interval, p);
    decode(new_lost, p);
    decode(new_blocklist, p);
    decode(old_blocklist, p);
    if (ev >= 6) decode(new_up_cluster, p);
    if (ev >= 7) decode(cluster_snapshot, p);
    if (ev >= 8) decode(new_uuid, p);
    if (ev >= 9) decode(new_xinfo, p);
    if (ev >= 10) decode(new_hb_front_up, p);
}
...
void OSDMap::Incremental::dump(Formatter* f) const
{
    ...
    for (const auto& old_pool : old_pools) f->dump_int("pool", old_pool);
    f->close_section();
    ...
    for (const auto& upclient : new_up_client) {
        ...
        if (auto p = new_up_cluster.find(upclient.first); p != new_up_cluster.end()) {
            f->dump_stream("cluster_addr") << p->second.legacy_addr();
            f->dump_object("cluster_addrs", p->second);
        }
        if (auto p = new_hb_back_up.find(upclient.first); p != new_hb_back_up.end()) {
            f->dump_object("heartbeat_back_addrs", p->second);
        }
        if (auto p = new_hb_front_up.find(upclient.first); p != new_hb_front_up.end()) {
    ...
    for (const auto& ns : new_state) {
        ...
        for (auto& state : st) f->dump_string("state", state);
        f->close_section();
        f->close_section();
    }
    f->close_section();
    ...
    for (const auto& pg_temp : new_pg_temp) {
        ...
        for (const auto& osd : pg_temp.second) f->dump_int("osd", osd);
        f->close_section();
        f->close_section();
    }
    f->close_section();
    ...
    for (const auto& blist : old_blocklist) f->dump_stream("addr") << blist;
    f->close_section();
    ...
    for (const auto& blist : old_range_blocklist) f->dump_stream("addr") << blist;
    f->close_section();
    ...
    if (cluster_snapshot.size()) f->dump_string("cluster_snapshot", cluster_snapshot);
...
{
    epoch = e;
    for (auto& pool : pools) pool.second.last_change = e;
}

OSDMap::range_bits::range_bits()
    : ipv6(false)
{
    memset(&bits, 0, sizeof(bits));
}

OSDMap::range_bits::range_bits(const entity_addr_t& addr)
    : ipv6(false)
{
    memset(&bits, 0, sizeof(bits));
    parse(addr);
...
void OSDMap::range_bits::get_ipv6_bytes(unsigned const char* addr, uint64_t* upper, uint64_t* lower)
{
    *upper = ((uint64_t)(ntohl(*(uint32_t*)(addr)))) << 32 | ((uint64_t)(ntohl(*(uint32_t*)(&addr[4]))));
    *lower = ((uint64_t)(ntohl(*(uint32_t*)(&addr[8])))) << 32 | ((uint64_t)(ntohl(*(uint32_t*)(&addr[12]))));
}
...
void OSDMap::range_bits::parse(const entity_addr_t& addr)
{
    ...
    if (addr.is_ipv6()) {
        get_ipv6_bytes(addr.in6_addr().sin6_addr.s6_addr, &bits.ipv6.upper_64_bits, &bits.ipv6.lower_64_bits);
        int32_t lower_shift = std::min(128 - static_cast<int32_t>(addr.get_nonce()), 64);
        int32_t upper_shift = std::max(64 -   //(128-b.first.get_nonce())-64
                                           static_cast<int32_t>(addr.get_nonce()),
                                       0);
...
bool OSDMap::range_bits::matches(const entity_addr_t& addr) const
{
    if (addr.is_ipv4() && !ipv6) {
        return ((ntohl(addr.in4_addr().sin_addr.s_addr) & bits.ipv4.mask) == (bits.ipv4.ip_32_bits & bits.ipv4.mask));
    }
    else if (addr.is_ipv6() && ipv6) {
        uint64_t upper_64, lower_64;
        get_ipv6_bytes(addr.in6_addr().sin6_addr.s6_addr, &upper_64, &lower_64);
        return (((upper_64 & bits.ipv6.upper_mask) == (bits.ipv6.upper_64_bits & bits.ipv6.upper_mask)) &&
                ((lower_64 & bits.ipv6.lower_mask) == (bits.ipv6.lower_64_bits & bits.ipv6.lower_mask)));
    }
    return false;
}
...
{
    if (blocklist.empty() && range_blocklist.empty()) return false;
                           ...
                           std::list<std::pair<entity_addr_t, utime_t>>* rl) const
{
    ...
    std::copy(range_blocklist.begin(), range_blocklist.end(), std::back_inserter(*rl));
}

void OSDMap::get_blocklist(std::set<entity_addr_t>* bl, std::set<entity_addr_t>* rl) const
{
...
void OSDMap::set_max_osd(int m)
{
    ...
    if (osd_primary_affinity) osd_primary_affinity->resize(max_osd, CEPH_OSD_DEFAULT_PRIMARY_AFFINITY);
...
void OSDMap::get_full_pools(CephContext* cct, set<int64_t>* full, set<int64_t>* backfillfull,
...
void OSDMap::get_full_osd_counts(set<int>* full, set<int>* backfill, set<int>* nearfull) const
{
    full->clear();
    backfill->clear();
    nearfull->clear();
...
{
    for (int i = 0; i < max_osd; i++)
        if (exists(i)) ls.insert(i);
}

void OSDMap::get_up_osds(set<int32_t>& ls) const
{
    for (int i = 0; i < max_osd; i++) {
        if (is_up(i)) ls.insert(i);
    }
}
...
{
    for (int i = 0; i < max_osd; i++) {
        if (exists(i) && get_weight(i) == CEPH_OSD_OUT) ls.insert(i);
    }
}
...
void OSDMap::adjust_osd_weights(const map<int, double>& weights, Incremental& inc) const
{
    ...
    for (const auto& weight : weights) {
        if (weight.second > max) max = weight.second;
    }
...
{
    for (int i = 0; i < max_osd; i++)
        if (exists(i) && (get_addrs(i).contains(addr) || get_cluster_addrs(i).contains(addr))) return i;
    return -1;
}
...
{
    for (int i = 0; i < max_osd; i++)
        if (exists(i) && get_uuid(i) == u) return i;
    return -1;
}
...
{
    for (int i = 0; i < max_osd; i++)
        if (exists(i) && (get_addrs(i).contains(addr) || get_cluster_addrs(i).contains(addr) ||
                          get_hb_back_addrs(i).contains(addr) || get_hb_front_addrs(i).contains(addr)))
            return i;
    return -1;
}
...
{
    for (int i = 0; i < max_osd; i++)
        if (exists(i) && (get_addrs(i).is_same_host(ip) || get_cluster_addrs(i).is_same_host(ip))) return i;
    return -1;
}
...
uint64_t OSDMap::get_features(int entity_type, uint64_t* pmask) const
{
    ...
    if (crush->has_nondefault_tunables()) features |= CEPH_FEATURE_CRUSH_TUNABLES;
    if (crush->has_nondefault_tunables2()) features |= CEPH_FEATURE_CRUSH_TUNABLES2;
    if (crush->has_nondefault_tunables3()) features |= CEPH_FEATURE_CRUSH_TUNABLES3;
    if (crush->has_v4_buckets()) features |= CEPH_FEATURE_CRUSH_V4;
    if (crush->has_nondefault_tunables5()) features |= CEPH_FEATURE_CRUSH_TUNABLES5;
    ...
    for (auto& pool : pools) {
        ...
        if (!pool.second.tiers.empty() || pool.second.is_tier()) {
            features |= CEPH_FEATURE_OSD_CACHEPOOL;
        }
        int ruleid = pool.second.get_crush_rule();
        if (ruleid >= 0) {
            if (crush->is_v2_rule(ruleid)) features |= CEPH_FEATURE_CRUSH_V2;
            if (crush->is_v3_rule(ruleid)) features |= CEPH_FEATURE_CRUSH_TUNABLES3;
            if (crush->is_v5_rule(ruleid)) features |= CEPH_FEATURE_CRUSH_TUNABLES5;
        }
    }
    ...
    if (entity_type == CEPH_ENTITY_TYPE_OSD) {
        ...
        const uint64_t kraken_features = CEPH_FEATUREMASK_SERVER_KRAKEN | CEPH_FEATURE_MSG_ADDR2;
    ...
    else if (require_osd_release >= ceph_release_t::nautilus && entity_type == CEPH_ENTITY_TYPE_OSD) {
...
void OSDMap::_calc_up_osd_features()
{
    ...
    for (int osd = 0; osd < max_osd; ++osd) {
        ...
        if (xi.features == 0) continue;   // bogus xinfo, maybe #20751 or similar, skipping
        if (first) {
...
{
    using ceph::encode;
    if (o->epoch == n->epoch) return;

    int diff = 0;

    // do addrs match?
    if (o->max_osd != n->max_osd) diff++;
    ...
    // does pg_temp match?
    if (*o->pg_temp == *n->pg_temp) n->pg_temp = o->pg_temp;

    // does primary_temp match?
    if (o->primary_temp->size() == n->primary_temp->size()) {
        if (*o->primary_temp == *n->primary_temp) n->primary_temp = o->primary_temp;
    }

    // do uuids match?
    if (o->osd_uuid->size() == n->osd_uuid->size() && *o->osd_uuid == *n->osd_uuid) n->osd_uuid = o->osd_uuid;
}

void OSDMap::clean_temps(CephContext* cct, const OSDMap& oldmap, const OSDMap& nextmap, Incremental* pending_inc)
{
    ...
    for (auto pg : *nextmap.pg_temp) {
        ...
        if (!nextmap.have_pg_pool(pg.first.pool())) {
            ldout(cct, 10) << __func__ << " removing pg_temp " << pg.first << " for nonexistent pool "
                           << pg.first.pool() << dendl;
            pending_inc->new_pg_temp[pg.first].clear();
            continue;
        }
        if (!nextmap.pg_exists(pg.first)) {
            ldout(cct, 10) << __func__ << " removing pg_temp " << pg.first << " for nonexistent pg " << dendl;
            pending_inc->new_pg_temp[pg.first].clear();
            continue;
        }
        // all osds down?
        ...
        if (num_up == 0) {
            ldout(cct, 10) << __func__ << "  removing pg_temp " << pg.first << " with all down osds" << pg.second
                           << dendl;
            pending_inc->new_pg_temp[pg.first].clear();
            continue;
        }
        ...
        bool remove = false;
        if (raw_up == pg.second) {
            ldout(cct, 10) << __func__ << "  removing pg_temp " << pg.first << " " << pg.second
                           << " that matches raw_up mapping" << dendl;
            remove = true;
        }
        // oversized pg_temp?
        if (pg.second.size() > nextmap.get_pg_pool(pg.first.pool())->get_size()) {
            ldout(cct, 10) << __func__ << "  removing pg_temp " << pg.first << " " << pg.second << " exceeds pool size"
                           << dendl;
            remove = true;
        }
        if (remove) {
    ...
    for (auto& pg : *nextmap.primary_temp) {
        ...
        if (real_primary == templess_primary) {
            ldout(cct, 10) << __func__ << "  removing primary_temp " << pgid << " -> " << real_primary
...
void OSDMap::get_upmap_pgs(vector<pg_t>* upmap_pgs) const
{
    ...
    for (auto& p : pg_upmap) upmap_pgs->push_back(p.first);
    for (auto& p : pg_upmap_items) upmap_pgs->push_back(p.first);
}

bool OSDMap::check_pg_upmaps(CephContext* cct, const vector<pg_t>& to_check, vector<pg_t>* to_cancel,
                             map<pg_t, mempool::osdmap::vector<pair<int, int>>>* to_remap) const
{
    ...
    for (auto& pg : to_check) {
        ...
        auto r = crush->verify_upmap(cct, crush_rule, get_pg_pool_size(pg), up);
        if (r < 0) {
        ...
        for (auto osd : up) {
            ...
            if (it == weight_map.end()) {
                ldout(cct, 10) << __func__ << " pg " << pg << ": osd " << osd << " is gone or has "
                               << "been moved out of the specific crush-tree" << dendl;
                to_cancel->push_back(pg);
                break;
            }
        ...
        if (!to_cancel->empty() && to_cancel->back() == pg) continue;
        // okay, upmap is valid
        // continue to check if it is still necessary
        auto i = pg_upmap.find(pg);
        if (i != pg_upmap.end()) {
            if (i->second == raw) {
                ldout(cct, 10) << __func__ << "removing redundant pg_upmap " << i->first << " " << i->second << dendl;
                to_cancel->push_back(pg);
                continue;
            }
            if ((int)i->second.size() != get_pg_pool_size(pg)) {
                ldout(cct, 10) << __func__ << "removing pg_upmap " << i->first << " " << i->second << " != pool size "
                               << get_pg_pool_size(pg) << dendl;
                to_cancel->push_back(pg);
                continue;
            }
        }
        ...
        if (j != pg_upmap_items.end()) {
            ...
            for (auto& p : j->second) {
                ...
                if (std::find(raw.begin(), raw.end(), osd_from) == raw.end()) {
                    ...
                    ldout(cct, 20) << __func__ << " pg_upmap_items (source osd does not exist) " << pg_upmap_items
                                   << dendl;
                    continue;
                }
                if (osd_to != CRUSH_ITEM_NONE && osd_to < max_osd && osd_to >= 0 && osd_weight[osd_to] == 0) {
            ...
            if (newmap.empty()) {
                ldout(cct, 10) << __func__ << " removing no-op pg_upmap_items " << j->first << " " << j->second
                               << dendl;
                to_cancel->push_back(pg);
            }
            else if (newmap != j->second) {
                // check partial no-op here.
                ldout(cct, 10) << __func__ << " simplifying partially no-op pg_upmap_items " << j->first << " "
                               << j->second << " -> " << newmap << dendl;
...
void OSDMap::clean_pg_upmaps(CephContext* cct, Incremental* pending_inc, const vector<pg_t>& to_cancel,
    ...
    for (auto& pg : to_cancel) {
        ...
        if (i != pending_inc->new_pg_upmap.end()) {
            ldout(cct, 10) << __func__ << " cancel invalid pending "
                           << "pg_upmap entry " << i->first << "->" << i->second << dendl;
        ...
        if (p != pending_inc->new_pg_upmap_items.end()) {
            ldout(cct, 10) << __func__ << " cancel invalid pending "
                           << "pg_upmap_items entry " << p->first << "->" << p->second << dendl;
        ...
        if (q != pg_upmap_items.end()) {
            ldout(cct, 10) << __func__ << " cancel invalid "
                           << "pg_upmap_items entry " << q->first << "->" << q->second << dendl;
            pending_inc->old_pg_upmap_items.insert(pg);
        }
    }
    for (auto& i : to_remap) pending_inc->new_pg_upmap_items[i.first] = i.second;
}

bool OSDMap::clean_pg_upmaps(CephContext* cct, Incremental* pending_inc) const
{
    ldout(cct, 10) << __func__ << dendl;
...
int OSDMap::apply_incremental(const Incremental& inc)
{
    ...
    if (inc.new_max_osd >= 0) set_max_osd(inc.new_max_osd);

    if (inc.new_pool_max != -1) pool_max = inc.new_pool_max;
    ...
    for (const auto& profile : inc.old_erasure_code_profiles) erasure_code_profiles.erase(profile);
    ...
    for (const auto& state : inc.new_state) {
        ...
        if ((osd_state[osd] & CEPH_OSD_UP) && (s & CEPH_OSD_UP)) {
            osd_info[osd].down_at = epoch;
            osd_xinfo[osd].down_stamp = modified;
        }
        if ((osd_state[osd] & CEPH_OSD_EXISTS) && (s & CEPH_OSD_EXISTS)) {
    ...
    for (const auto& client : inc.new_up_client) {
        ...
        osd_addrs->client_addrs[client.first].reset(new entity_addrvec_t(client.second));
    ...
    for (const auto& cluster : inc.new_up_cluster)
        osd_addrs->cluster_addrs[cluster.first].reset(new entity_addrvec_t(cluster.second));

    // info
    for (const auto& thru : inc.new_up_thru) osd_info[thru.first].up_thru = thru.second;
    ...
    for (const auto& lost : inc.new_lost) osd_info[lost.first].lost_at = lost.second;

    // xinfo
    for (const auto& xinfo : inc.new_xinfo) osd_xinfo[xinfo.first] = xinfo.second;

    // uuid
    for (const auto& uuid : inc.new_uuid) (*osd_uuid)[uuid.first] = uuid.second;
    ...
    for (const auto& addr : inc.old_blocklist) blocklist.erase(addr);
    ...
    if (inc.crush.length()) {
        ...
        for (auto it = device_class_flags.begin(); it != device_class_flags.end();) {
    ...
    case Incremental::mutate_allow_crimson_t::NONE: break;
    case Incremental::mutate_allow_crimson_t::SET: allow_crimson = true; break;
    case Incremental::mutate_allow_crimson_t::CLEAR: allow_crimson = false; break;
    }
...
ceph_object_layout OSDMap::make_object_layout(object_t oid, int pg_pool, string nspace) const
{
...
void OSDMap::_remove_nonexistent_osds(const pg_pool_t& pool, vector<int>& osds) const
{
    if (pool.can_shift_osds()) {
        ...
        if (removed) osds.resize(osds.size() - removed);
    }
    else {
        for (auto& osd : osds) {
            if (!exists(osd)) osd = CRUSH_ITEM_NONE;
        }
    }
}
...
void OSDMap::_apply_upmap(const pg_pool_t& pi, pg_t raw_pg, vector<int>* raw) const
{
    ...
    if (p != pg_upmap.end()) {
        ...
        for (auto osd : p->second) {
            if (osd != CRUSH_ITEM_NONE && osd < max_osd && osd >= 0 && osd_weight[osd] == 0) {
    ...
    if (q != pg_upmap_items.end()) {
        ...
        for (auto& [osd_from, osd_to] : q->second) {
            ...
            for (unsigned i = 0; i < raw->size(); ++i) {
                ...
                if (osd == osd_from && pos < 0 &&
                    !(osd_to != CRUSH_ITEM_NONE && osd_to < max_osd && osd_to >= 0 && osd_weight[osd_to] == 0)) {
                    pos = i;
                }
            }
            if (!exists && pos >= 0) {
    ...
    if (r != pg_upmap_primaries.end()) {
        ...
        if (new_prim != CRUSH_ITEM_NONE && new_prim < max_osd && new_prim >= 0 && osd_weight[new_prim] != 0) {
            int new_prim_idx = 0;
...
// pg -> (up osd list)
void OSDMap::_raw_to_up_osds(const pg_pool_t& pool, const vector<int>& raw, vector<int>* up) const
{
    if (pool.can_shift_osds()) {
        // shift left
        up->clear();
...
void OSDMap::_apply_primary_affinity(ps_t seed, const pg_pool_t& pool, vector<int>* osds, int* primary) const
{
    // do we have any non-default primary_affinity values for these osds?
    if (!osd_primary_affinity) return;

    bool any = false;
    for (const auto osd : *osds) {
        if (osd != CRUSH_ITEM_NONE && (*osd_primary_affinity)[osd] != CEPH_OSD_DEFAULT_PRIMARY_AFFINITY) {
            any = true;
            break;
        }
    }
    ...
    for (unsigned i = 0; i < osds->size(); ++i) {
        int o = (*osds)[i];
        if (o == CRUSH_ITEM_NONE) continue;
        unsigned a = (*osd_primary_affinity)[o];
        if (a < CEPH_OSD_MAX_PRIMARY_AFFINITY && (crush_hash32_2(CRUSH_HASH_RJENKINS1, seed, o) >> 16) >= a) {
...
void OSDMap::_get_temp_osds(const pg_pool_t& pool, pg_t pg, vector<int>* temp_pg, int* temp_primary) const
{
...
void OSDMap::pg_to_raw_upmap(pg_t pg, vector<int>* raw, vector<int>* raw_upmap) const
{
...
void OSDMap::_pg_to_up_acting_osds(const pg_t& pg, vector<int>* up, int* up_primary, vector<int>* acting,
                                   int* acting_primary, bool raw_pg_to_pg) const
{
    const pg_pool_t* pool = get_pg_pool(pg.pool());
    if (!pool || (!raw_pg_to_pg && pg.ps() >= pool->get_pg_num())) {
        if (up) up->clear();
        if (up_primary) *up_primary = -1;
        if (acting) acting->clear();
        if (acting_primary) *acting_primary = -1;
        return;
    }
    vector<int> raw;
    vector<int> _up;
    ...
    if (_acting.empty() || up || up_primary) {
        ...
        if (up) up->swap(_up);
        if (up_primary) *up_primary = _up_primary;
    }

    if (acting) acting->swap(_acting);
    if (acting_primary) *acting_primary = _acting_primary;
}
...
bool OSDMap::primary_changed_broken(int oldprimary, const vector<int>& oldacting, int newprimary,
                                    const vector<int>& newacting)
{
    if (oldacting.empty() && newacting.empty()) return false;   // both still empty
    if (oldacting.empty() ^ newacting.empty()) return true;     // was empty, now not, or vice versa
    if (oldprimary != newprimary) return true;                  // primary changed
    if (calc_pg_role_broken(oldprimary, oldacting) != calc_pg_role_broken(newprimary, newacting)) return true;
...
uint64_t OSDMap::get_encoding_features() const
{
    ...
    if (require_osd_release < ceph_release_t::luminous) {
        f &= ~(CEPH_FEATURE_SERVER_LUMINOUS | CEPH_FEATURE_CRUSH_CHOOSE_ARGS);
    }
    if (require_osd_release < ceph_release_t::kraken) {
        f &= ~(CEPH_FEATURE_SERVER_KRAKEN | CEPH_FEATURE_MSG_ADDR2);
    }
    if (require_osd_release < ceph_release_t::jewel) {
...
void OSDMap::encode(ceph::buffer::list& bl, uint64_t features) const
{
...

    {
        ...
        encode(osd_info, bl);
        {
            ...
            for (const auto& addr : blocklist) blocklist_map.insert(make_pair(addr.first, addr.second));
...
void OSDMap::decode(ceph::buffer::list::const_iterator& bl)
{
     ...
     */
    {
        ...
        if (struct_v >= 2) {
            ...
            if (osd_primary_affinity->empty()) osd_primary_affinity.reset();
        }
        else {
            osd_primary_affinity.reset();
        }
...

    {
        ...
        if (struct_v == 4) {
            string r;
            decode(r, bl);
            if (r.length()) require_min_compat_client = ceph_release_from_name(r.c_str());
        }
        if (struct_v >= 5) {
...
void OSDMap::dump_erasure_code_profiles(const mempool::osdmap::map<string, map<string, string>>& profiles, Formatter* f)
{
...
void OSDMap::dump_osd(int id, Formatter* f) const
{
    ...
    f->dump_stream("heartbeat_back_addr") << get_hb_back_addrs(id).get_legacy_str();
    f->dump_stream("heartbeat_front_addr") << get_hb_front_addrs(id).get_legacy_str();

    set<string> st;
    get_state(id, st);
    f->open_array_section("state");
    for (const auto& state : st) f->dump_string("state", state);
    f->close_section();

    f->close_section();
}

void OSDMap::dump_pool(CephContext* cct, int64_t pid, const pg_pool_t& pdata, ceph::Formatter* f) const
{
    std::string name("<unknown>");
    const auto& pni = pool_name.find(pid);
    if (pni != pool_name.end()) name = pni->second;
    f->open_object_section("pool");
    f->dump_int("pool", pid);
...
void OSDMap::dump_read_balance_score(CephContext* cct, int64_t pid, const pg_pool_t& pdata, ceph::Formatter* f) const
{
...
void OSDMap::dump(Formatter* f, CephContext* cct) const
{
    ...
    f->dump_string("require_min_compat_client", to_string(require_min_compat_client));
    f->dump_string("min_compat_client", to_string(get_min_compat_client()));
    f->dump_string("require_osd_release", to_string(require_osd_release));
    ...
    for (auto& i : crush_node_flags) {
        string s = crush->item_exists(i.first) ? crush->get_item_name(i.first) : stringify(i.first);
...
{
    string s;
    if (f & CEPH_OSDMAP_PAUSERD) s += ",pauserd";
    if (f & CEPH_OSDMAP_PAUSEWR) s += ",pausewr";
    if (f & CEPH_OSDMAP_PAUSEREC) s += ",pauserec";
    if (f & CEPH_OSDMAP_NOUP) s += ",noup";
    if (f & CEPH_OSDMAP_NODOWN) s += ",nodown";
    if (f & CEPH_OSDMAP_NOOUT) s += ",noout";
    if (f & CEPH_OSDMAP_NOIN) s += ",noin";
    if (f & CEPH_OSDMAP_NOBACKFILL) s += ",nobackfill";
    if (f & CEPH_OSDMAP_NOREBALANCE) s += ",norebalance";
    if (f & CEPH_OSDMAP_NORECOVER) s += ",norecover";
    if (f & CEPH_OSDMAP_NOSCRUB) s += ",noscrub";
    if (f & CEPH_OSDMAP_NODEEP_SCRUB) s += ",nodeep-scrub";
    if (f & CEPH_OSDMAP_NOTIERAGENT) s += ",notieragent";
    if (f & CEPH_OSDMAP_NOSNAPTRIM) s += ",nosnaptrim";
    if (f & CEPH_OSDMAP_SORTBITWISE) s += ",sortbitwise";
    if (f & CEPH_OSDMAP_REQUIRE_JEWEL) s += ",require_jewel_osds";
    if (f & CEPH_OSDMAP_REQUIRE_KRAKEN) s += ",require_kraken_osds";
    if (f & CEPH_OSDMAP_REQUIRE_LUMINOUS) s += ",require_luminous_osds";
    if (f & CEPH_OSDMAP_RECOVERY_DELETES) s += ",recovery_deletes";
    if (f & CEPH_OSDMAP_PURGED_SNAPDIRS) s += ",purged_snapdirs";
    if (f & CEPH_OSDMAP_PGLOG_HARDLIMIT) s += ",pglog_hardlimit";
    if (f & CEPH_OSDMAP_NOAUTOSCALE) s += ",noautoscale";
...
void OSDMap::print_pools(CephContext* cct, ostream& out) const
{
    for (const auto& [pid, pdata] : pools) {
        ...
        if (pdata.is_replicated()) {
            ...
            if (rc >= 0)
                snprintf(rb_score_str, sizeof(rb_score_str), " read_balance_score %.2f", rb_info.acting_adj_score);
        }

        out << "pool " << pid << " '" << name << "' " << pdata << rb_score_str << "\n";
        if (rb_info.err_msg.length() > 0) {
...
public:
    ...
    OSDTreePlainDumper(const CrushWrapper* crush, const OSDMap* osdmap_, unsigned f)
    ...
    bool should_dump_leaf(int i) const override
    {
        ...
        if (((filter & OSDMap::DUMP_UP) && osdmap->is_up(i)) || ((filter & OSDMap::DUMP_DOWN) && osdmap->is_down(i)) ||
            ((filter & OSDMap::DUMP_IN) && osdmap->is_in(i)) || ((filter & OSDMap::DUMP_OUT) && osdmap->is_out(i)) ||
    ...
    bool should_dump_empty_bucket() const override { return !filter; }
...
protected:
    void dump_item(const CrushTreeDumper::Item& qi, TextTable* tbl) override
    {
        ...
        if (!c) c = "";
        *tbl << qi.id << c << weightf_t(qi.weight);

        ostringstream name;
        for (int k = 0; k < qi.depth; k++) name << "    ";
        if (qi.is_bucket()) {
            name << crush->get_type_name(crush->get_bucket_type(qi.id)) << " " << crush->get_item_name(qi.id);
        }
        else {
            name << "osd." << qi.id;
        }
        *tbl << name.str();
        ...
        if (!qi.is_bucket()) {
            if (!osdmap->exists(qi.id)) {
                ...
                *tbl << s << weightf_t(osdmap->get_weightf(qi.id)) << weightf_t(osdmap->get_primary_affinityf(qi.id));
            }
        }
...
public:
    ...
    OSDTreeFormattingDumper(const CrushWrapper* crush, const OSDMap* osdmap_, unsigned f)
        : Parent(crush, osdmap_->get_pool_names())
        , osdmap(osdmap_)
        , filter(f)
    {}
...
void OSDMap::print_summary(Formatter* f, ostream& out, const string& prefix, bool extra) const
{
    if (f) {
        f->dump_int("epoch", get_epoch());
        ...
        out << get_num_osds() << " osds: " << get_num_up_osds() << " up";
        ...
        if (extra) out << "; epoch: e" << get_epoch();
        if (get_num_pg_temp()) out << "; " << get_num_pg_temp() << " remapped pgs";
        out << "\n";
        uint64_t important_flags = flags & ~CEPH_OSDMAP_SEMIHIDDEN_FLAGS;
        if (important_flags) out << prefix << "flags " << get_flag_string(important_flags) << "\n";
    }
}

void OSDMap::print_oneline_summary(ostream& out) const
{
    out << "e" << get_epoch() << ": " << get_num_osds() << " total, " << get_num_up_osds() << " up, "
        << get_num_in_osds() << " in";
}
...
{
    for (const auto& pool : pools) {
        if (pool.second.crush_rule == rule_id) return true;
    }
    return false;
}

int OSDMap::validate_crush_rules(CrushWrapper* newcrush, ostream* ss) const
{
    for (auto& i : pools) {
        auto& pool = i.second;
        int ruleno = pool.get_crush_rule();
        if (!newcrush->rule_exists(ruleno)) {
            *ss << "pool " << i.first << " references crush_rule " << ruleno << " but it is not present";
            return -EINVAL;
        }
...
int OSDMap::build_simple_optioned(CephContext* cct, epoch_t e, uuid_d& fsid, int nosd, int pg_bits, int pgp_bits,
                                  bool default_pool)
{
    ...
    if (nosd >= 0) {
        ...
        for (auto& section : sections) {
            if (section.find("osd.") != 0) continue;
    ...
    if (default_pool) {
        // pgp_num <= pg_num
        if (pgp_bits > pg_bits) pgp_bits = pg_bits;
        ...
        for (auto& plname : pool_names) {
            ...
            if (cct->_conf->osd_pool_default_flag_hashpspool) pools[pool].set_flag(pg_pool_t::FLAG_HASHPSPOOL);
            if (cct->_conf->osd_pool_default_flag_nodelete) pools[pool].set_flag(pg_pool_t::FLAG_NODELETE);
            if (cct->_conf->osd_pool_default_flag_nopgchange) pools[pool].set_flag(pg_pool_t::FLAG_NOPGCHANGE);
            if (cct->_conf->osd_pool_default_flag_nosizechange) pools[pool].set_flag(pg_pool_t::FLAG_NOSIZECHANGE);
            if (cct->_conf->osd_pool_default_flag_bulk) pools[pool].set_flag(pg_pool_t::FLAG_BULK);
            pools[pool].size = cct->_conf.get_val<uint64_t>("osd_pool_default_size");
            pools[pool].min_size = cct->_conf.get_osd_pool_default_min_size(pools[pool].size);
            ...
            pools[pool].application_metadata.insert({pg_pool_t::APPLICATION_NAME_RBD, {}});
...
int OSDMap::get_erasure_code_profile_default(CephContext* cct, map<string, string>& profile_map, ostream* ss)
{
    int r = get_json_str_map(cct->_conf.get_val<string>("osd_pool_default_erasure_code_profile"), *ss, &profile_map);
    return r;
}
...
int OSDMap::build_simple_crush_map(CephContext* cct, CrushWrapper& crush, int nosd, ostream* ss)
{
    crush.create();
    ...
    int rootid;
    int r = crush.add_bucket(0, 0, CRUSH_HASH_DEFAULT, root_type, 0, NULL, NULL, &rootid);
    ceph_assert(r == 0);
    crush.set_item_name(rootid, "default");

    map<string, string> loc{{"host", "localhost"}, {"rack", "localrack"}, {"root", "default"}};
...
int OSDMap::build_simple_crush_map_from_conf(CephContext* cct, CrushWrapper& crush, ostream* ss)
{
    const auto& conf = cct->_conf;
    ...
    for (auto& section : sections) {
        ...
        if (host.length() == 0) host = "unknownhost";
        if (rack.length() == 0) rack = "unknownrack";

        map<string, string> loc;
        loc["host"] = host;
        loc["rack"] = rack;
        if (row.size()) loc["row"] = row;
        if (room.size()) loc["room"] = room;
        if (dc.size()) loc["datacenter"] = dc;
        loc["root"] = "default";
...
int OSDMap::build_simple_crush_rules(CephContext* cct, CrushWrapper& crush, const string& root, ostream* ss)
{
    int crush_rule = crush.get_osd_pool_default_crush_replicated_rule(cct);
    string failure_domain = crush.get_type_name(cct->_conf->osd_crush_chooseleaf_type);

    int r;
    r = crush.add_simple_rule_at(
        "replicated_rule", root, failure_domain, "", "firstn", pg_pool_t::TYPE_REPLICATED, crush_rule, ss);
    if (r < 0) return r;
...
int OSDMap::summarize_mapping_stats(OSDMap* newmap, const set<int64_t>* pools, std::string* out, Formatter* f) const
{
    set<int64_t> ls;
    if (pools) {
        ls = *pools;
        ...
        for (auto& p : get_pools()) ls.insert(p.first);
    }
    ...
    for (int64_t pool_id : ls) {
        ...
        for (unsigned ps = 0; ps < pi->get_pg_num(); ++ps) {
            ...
            for (int osd : up) {
                if (osd >= 0 && osd < get_max_osd()) ++base_by_osd[osd];
            }
            if (newmap) {
                newmap->pg_to_up_acting_osds(pgid, &up2, &up_primary, nullptr, nullptr);
                for (int osd : up2) {
                    if (osd >= 0 && osd < get_max_osd()) ++new_by_osd[osd];
                }
                if (pi->is_erasure()) {
    ...
    for (int osd = 0; osd < get_max_osd(); ++osd) {
        if (is_up(osd) && is_in(osd)) ++num_up_in;
    }
    if (!num_up_in) {
        return -EINVAL;
    }
    ...
    ostringstream ss;
    if (f) f->open_object_section("utilization");
    if (newmap) {
        if (f) {
            ...
            float percent = 0;
            if (total_pg) percent = (float)moved_pg * 100.0 / (float)total_pg;
            ss << "moved " << moved_pg << " / " << total_pg << " (" << percent << "%)\n";
        }
    }
    if (f) {
        ...
        if (newmap) f->dump_float("new_std_dev", new_stddev);
    }
    else {
        ss << "avg " << avg_pg << "\n";
        ss << "stddev " << base_stddev;
        if (newmap) ss << " -> " << new_stddev;
    ...
    if (min >= 0) {
        if (f) {
            ...
            if (newmap) f->dump_unsigned("new_min_osd_pgs", min_new_pg);
        }
        else {
            ss << "min osd." << min << " with " << min_base_pg;
            if (newmap) ss << " -> " << min_new_pg;
            ss << " pgs (" << (float)min_base_pg / avg_pg;
            if (newmap) ss << " -> " << (float)min_new_pg / avg_pg;
            ss << " * mean)\n";
        }
    }
    if (max >= 0) {
        if (f) {
            ...
            if (newmap) f->dump_unsigned("new_max_osd_pgs", max_new_pg);
        }
        else {
            ss << "max osd." << max << " with " << max_base_pg;
            if (newmap) ss << " -> " << max_new_pg;
            ss << " pgs (" << (float)max_base_pg / avg_pg;
            if (newmap) ss << " -> " << (float)max_new_pg / avg_pg;
            ss << " * mean)\n";
        }
    }
...
bool OSDMap::try_pg_upmap(CephContext* cct,
                          ...
                          vector<int>* out)   ///< resulting alternative mapping
{
    ...
    int r = crush->try_remap_rule(cct, rule, pool->get_size(), overfull, underfull, more_underfull, *orig, out);
    if (r < 0) return false;
...
int OSDMap::balance_primaries(CephContext* cct, int64_t pid, OSDMap::Incremental* pending_inc,
                              OSDMap& tmp_osd_map) const
{
    ...
    if (!pool->is_replicated()) {
        ldout(cct, 10) << __func__ << " skipping erasure pool " << get_pool_name(pid) << dendl;
        return -EINVAL;
    }
        ...
        for (const auto& [pg, mapped] : prim_pgs_to_check) {
            ...
            for (auto potential_osd : up_osds) {
                ...
                if ((prim_score > 0) &&        // taking 1 pg from the prim would not make its score worse
                    ...
                    (desired_prim_dist[potential_osd] >
            ...
            auto legal_swap = crush->verify_upmap(cct, crush_rule, pool_size, {(int)curr_best_osd});
            if (legal_swap >= 0 && ((int)curr_best_osd != up_primary)) {
...
int OSDMap::calc_desired_primary_distribution(CephContext* cct, int64_t pid, const vector<uint64_t>& osds,
                                              std::map<uint64_t, float>& desired_primary_distribution) const
{
    ...
    const pg_pool_t* pool = get_pg_pool(pid);
    if (pool->is_replicated()) {
        ldout(cct, 20) << __func__ << " calculating distribution for replicated pool " << get_pool_name(pid) << dendl;
        ...
        for (const auto& osd : osds) {
            float osd_primary_count =
                ((float)pgs_by_osd[osd].size() / (float)replica_count) * get_primary_affinityf(osd);
...
int OSDMap::calc_pg_upmaps(CephContext* cct, uint32_t max_deviation, int max, const set<int64_t>& only_pools,
                           OSDMap::Incremental* pending_inc, std::random_device::result_type* p_seed)
{
    ldout(cct, 10) << __func__ << " pools " << only_pools << dendl;
    OSDMap tmp_osd_map;
    // Can't be less than 1 pg
    if (max_deviation < 1) max_deviation = 1;
    ...
    float cur_max_deviation =
        calc_deviations(cct, pgs_by_osd, osd_weight, pgs_per_weight, osd_deviation, deviation_osd, stddev);
    ...
    auto fast_aggressive = aggressive && cct->_conf.get_val<bool>("osd_calc_pg_upmaps_aggressively_fast");
    ...
    while (max--) {
        ...
        fill_overfull_underfull(cct, deviation_osd, max_deviation, overfull, more_overfull, underfull, more_underfull);
    ...
    retry:
        ...
        for (auto p = deviation_osd.rbegin(); p != deviation_osd.rend(); ++p) {
            ...
            if (deviation < 0) {
                ldout(cct, 10) << " hitting underfull osds now"
                               << " when trying to remap overfull osds" << dendl;
                break;
            }
            float target = osd_weight[osd] * pgs_per_weight;
            ldout(cct, 10) << " Overfull search osd." << osd << " target " << target << " deviation " << deviation
                           << dendl;
            ceph_assert(target > 0);
            if (!using_more_overfull && deviation <= max_deviation) {
                ldout(cct, 10) << " osd." << osd << " target " << target << " deviation " << deviation
                               << " < max deviation " << max_deviation << dendl;
                break;
            }
            ...
            if (try_drop_remap_overfull(cct, pgs, tmp_osd_map, osd, temp_pgs_by_osd, to_unmap, to_upmap))
                goto test_change;
            ...
            for (auto pg : pgs) {
                ...
                if (it != tmp_osd_map.pg_upmap_items.end()) {
                    ...
                    if (um_items.size() >= (size_t)pg_pool_size) {
                        ldout(cct, 10) << " " << pg << " already has full-size pg_upmap_items " << um_items
                                       << ", skipping" << dendl;
                        continue;
                    }
                    else {
                        ldout(cct, 10) << " " << pg << " already has pg_upmap_items " << um_items << dendl;
        ...
        for (auto& [deviation, osd] : deviation_osd) {
            if (std::find(underfull.begin(), underfull.end(), osd) == underfull.end()) break;
            float target = osd_weight[osd] * pgs_per_weight;
            ceph_assert(target > 0);
            if (fabsf(deviation) < max_deviation) {
                // respect max_deviation too
                ldout(cct, 10) << " osd." << osd << " target " << target << " deviation " << deviation
                               << " -> absolute " << fabsf(deviation) << " < max " << max_deviation << dendl;
                break;
            }
            // look for remaps we can un-remap
            candidates_t candidates = build_candidates(cct, tmp_osd_map, to_skip, only_pools, aggressive, p_seed);
            if (try_drop_remap_underfull(cct, candidates, osd, temp_pgs_by_osd, to_unmap, to_upmap)) {
                goto test_change;
            }
        }
    ...
    test_change:
        ...
        float cur_max_deviation = calc_deviations(
            cct, temp_pgs_by_osd, osd_weight, pgs_per_weight, temp_osd_deviation, temp_deviation_osd, new_stddev);
        ldout(cct, 10) << " stddev " << stddev << " -> " << new_stddev << dendl;
        if (new_stddev >= stddev) {
            if (!aggressive) {
                ldout(cct, 10) << " break because stddev is not decreasing"
                               << " and aggressive mode is not enabled" << dendl;
                break;
            }
            ...
            for (auto& i : to_unmap) to_skip.insert(i);
            for (auto& i : to_upmap) to_skip.insert(i.first);
...
map<uint64_t, set<pg_t>> OSDMap::get_pgs_by_osd(CephContext* cct, int64_t pid,
                                                ...
                                                map<uint64_t, set<pg_t>>* p_acting_primaries_by_osd) const
{
    ...
    for (unsigned ps = 0; ps < pool->get_pg_num(); ++ps) {
        ...
        if (cct != nullptr)
            ldout(cct, 20) << __func__ << " " << pg << " up " << up << " primary " << primary << " acting_primary "
                           << acting_prim << dendl;

        if (!up.empty()) {   // up can be empty is test generated files
                             // in this case, we return empty result
            for (auto osd : up) {
                if (osd != CRUSH_ITEM_NONE) pgs_by_osd[osd].insert(pg);
            }
            if (p_primaries_by_osd != nullptr) {
                if (primary != CRUSH_ITEM_NONE) (*p_primaries_by_osd)[primary].insert(pg);
            }
            if (p_acting_primaries_by_osd != nullptr) {
                if (acting_prim != CRUSH_ITEM_NONE) (*p_acting_primaries_by_osd)[acting_prim].insert(pg);
            }
        }
    }
    return pgs_by_osd;
...
float OSDMap::get_osds_weight(CephContext* cct, const OSDMap& tmp_osd_map, int64_t pid,
                              map<int, float>& osds_weight) const
{
    ...
    ldout(cct, 20) << __func__ << " pool " << pid << " ruleno " << ruleno << " weight-map " << pmap << dendl;
...
float OSDMap::build_pool_pgs_info(CephContext* cct,
                                  const std::set<int64_t>& only_pools,   ///< [optional] restrict to pool
                                  const OSDMap& tmp_osd_map, int& total_pgs, map<int, set<pg_t>>& pgs_by_osd,
                                  map<int, float>& osds_weight)
{
    ...
    for (auto& [pid, pdata] : pools) {
        if (!only_pools.empty() && !only_pools.count(pid)) continue;
    ...
    for (auto& [oid, oweight] : osds_weight) {
        ...
        ldout(cct, 20) << " osd." << oid << " weight " << oweight << " pgs " << pgs << dendl;
    }
    return osds_weight_total;

}   // return total weight of all OSDs

float OSDMap::calc_deviations(CephContext* cct, const map<int, set<pg_t>>& pgs_by_osd,
                              const map<int, float>& osd_weight, float pgs_per_weight, map<int, float>& osd_deviation,
                              ...
                              float& stddev)   // return current max deviation
{
    ...
    for (auto& [oid, opgs] : pgs_by_osd) {
        ...
        ldout(cct, 20) << " osd." << oid << "\tpgs " << opgs.size() << "\ttarget " << target << "\tdeviation "
                       << deviation << dendl;
        osd_deviation[oid] = deviation;
        deviation_osd.insert(make_pair(deviation, oid));
        stddev += deviation * deviation;
        if (fabsf(deviation) > cur_max_deviation) cur_max_deviation = fabsf(deviation);
    }
    return cur_max_deviation;
}

void OSDMap::fill_overfull_underfull(CephContext* cct, const std::multimap<float, int>& deviation_osd,
                                     int max_deviation, std::set<int>& overfull, std::set<int>& more_overfull,
                                     std::vector<int>& underfull, std::vector<int>& more_underfull)
{
    //
...
int OSDMap::pack_upmap_results(CephContext* cct, const std::set<pg_t>& to_unmap,
                               const std::map<pg_t, mempool::osdmap::vector<std::pair<int, int>>>& to_upmap,
                               OSDMap& tmp_osd_map, OSDMap::Incremental* pending_inc)
{
    //
    ...
    for (auto& [pg, um_items] : to_upmap) {
        ldout(cct, 10) << " upmap pg " << pg << " new pg_upmap_items " << um_items << dendl;
...
std::default_random_engine OSDMap::get_random_engine(CephContext* cct, std::random_device::result_type* p_seed)
{
    //
    ...
    if (p_seed == nullptr) {
        ...
        ldout(cct, 30) << " Starting random engine with seed " << seed << dendl;
        seed_set++;
    }
    return std::default_random_engine{seed};
}

bool OSDMap::try_drop_remap_overfull(CephContext* cct, const std::vector<pg_t>& pgs, const OSDMap& tmp_osd_map, int osd,
                                     map<int, std::set<pg_t>>& temp_pgs_by_osd, set<pg_t>& to_unmap,
                                     map<pg_t, mempool::osdmap::vector<pair<int32_t, int32_t>>>& to_upmap)
{
    //
    ...
    for (auto pg : pgs) {
        ...
        if (p == tmp_osd_map.pg_upmap_items.end()) continue;
        mempool::osdmap::vector<pair<int32_t, int32_t>> new_upmap_items;
        ...
        for (auto um_pair : pg_upmap_items) {
            ...
            if (um_to == osd) {
                ldout(cct, 10) << " will try dropping existing"
                               << " remapping pair " << um_from << " -> " << um_to << " which remapped " << pg
                               << " into overfull osd." << osd << dendl;
        ...
        if (new_upmap_items.empty()) {
            // drop whole item
            ldout(cct, 10) << " existing pg_upmap_items " << pg_upmap_items << " remapped " << pg
                           << " into overfull osd." << osd << ", will try cancelling it entirely" << dendl;
            to_unmap.insert(pg);
            return true;
        }
            ...
            ldout(cct, 10) << " existing pg_upmap_items " << pg_upmap_items << " remapped " << pg
                           << " into overfull osd." << osd << ", new_pg_upmap_items now " << new_upmap_items << dendl;
...
bool OSDMap::try_drop_remap_underfull(CephContext* cct, const candidates_t& candidates, int osd,
                                      map<int, std::set<pg_t>>& temp_pgs_by_osd, set<pg_t>& to_unmap,
    ...
    for (auto& [pg, um_pairs] : candidates) {
        ...
        for (auto& ump : um_pairs) {
            ...
            if (um_from == osd) {
                ldout(cct, 10) << " will try dropping existing"
                               << " remapping pair " << um_from << " -> " << um_to << " which remapped " << pg
                               << " out from underfull osd." << osd << dendl;
                temp_pgs_by_osd[um_to].erase(pg);
                temp_pgs_by_osd[um_from].insert(pg);
            }
            else {
        ...
        if (new_upmap_items.empty()) {
            // drop whole item
            ldout(cct, 10) << " existing pg_upmap_items " << um_pairs << " remapped " << pg
                           << " out from underfull osd." << osd << ", will try cancelling it entirely" << dendl;
            to_unmap.insert(pg);
            return true;
        }
            ...
            ldout(cct, 10) << " existing pg_upmap_items " << um_pairs << " remapped " << pg
                           << " out from underfull osd." << osd << ", new_pg_upmap_items now " << new_upmap_items
                           << dendl;
            to_upmap[pg] = new_upmap_items;
            return true;
        }
...
void OSDMap::add_remap_pair(CephContext* cct, int orig, int out, pg_t pg, size_t pg_pool_size, int osd,
                            set<int>& existing, map<int, set<pg_t>>& temp_pgs_by_osd,
                            mempool::osdmap::vector<pair<int32_t, int32_t>> new_upmap_items,
                            map<pg_t, mempool::osdmap::vector<pair<int32_t, int32_t>>>& to_upmap)
{
    //
    ...
    //
    ldout(cct, 10) << " will try adding new remapping pair " << orig << " -> " << out << " for " << pg
                   << (orig != osd ? " NOT selected osd" : "") << dendl;
...
int OSDMap::find_best_remap(CephContext* cct, const vector<int>& orig, const vector<int>& out, const set<int>& existing,
                            const map<int, float> osd_deviation)
{
    ...
    for (unsigned i = 0; i < out.size(); ++i) {
        if (orig[i] == out[i]) continue;                                   // skip invalid remappings
        if (existing.count(orig[i]) || existing.count(out[i])) continue;   // we want new remappings only!
...
OSDMap::candidates_t OSDMap::build_candidates(CephContext* cct, const OSDMap& tmp_osd_map, const set<pg_t> to_skip,
                                              const set<int64_t>& only_pools, bool aggressive,
                                              std::random_device::result_type* p_seed)
{
    ...
    for (auto& [pg, um_pair] : tmp_osd_map.pg_upmap_items) {
        if (to_skip.count(pg)) continue;
        if (!only_pools.empty() && !only_pools.count(pg.pool())) continue;
...
int OSDMap::set_rbi(CephContext* cct, read_balance_info_t& rbi, int64_t pool_id, float total_w_pa, float pa_sum,
                    int num_osds, int osd_pa_count, float total_osd_weight, uint max_prims_per_osd,
                    uint max_acting_prims_per_osd, float avg_prims_per_osd, bool prim_on_zero_pa,
                    bool acting_on_zero_pa, float max_osd_score) const
{
    ...
    if (total_w_pa / total_osd_weight < 1. / float(pool->get_size())) {
        ...
        rbi.err_msg =
            fmt::format("pool {} average primary affinity is lower than {:.2f}, read balance score is not reliable",
                        pool_id,
                        1. / float(pool->get_size()));
        return -EINVAL;
    }
    ...
    if (rbi.pa_avg != 0.) {
        ...
        if ((zpg = has_zero_pa_pgs(cct, pool_id)) >= 0) {
            ...
            ldout(cct, 10) << __func__ << " pool " << pool_id
                           << " has some PGs where all OSDs are with primary_affinity 0 (" << pg << ",...)" << dendl;
            rbi.err_msg = fmt::format("pool {} has some PGs where all OSDs are with primary_affinity 0 (at least pg "
                                      "{}), read balance score may not be reliable",
                                      pool_id,
                                      ss.str());
            return -EINVAL;
        }
        ...
        // We should never get here - this condition is checked before calling this function - this is just sanity check
        // code.
        rbi.err_msg = fmt::format(
            "pool {} all OSDs have zero primary affinity, can't calculate a reliable read balance score", pool_id);
        return -EINVAL;
    }
...
int OSDMap::calc_read_balance_score(CephContext* cct, int64_t pool_id, read_balance_info_t* p_rbi) const
{
    // BUG: wrong score with one PG replica 3 and 4 OSDs
    if (cct != nullptr) ldout(cct, 20) << __func__ << " pool " << get_pool_name(pool_id) << dendl;

    OSDMap tmp_osd_map;
    tmp_osd_map.deepish_copy_from(*this);
    if (p_rbi == nullptr) {
        // The only case where error message is not set - this is not tested in the unit test.
        if (cct != nullptr) ldout(cct, 30) << __func__ << " p_rbi is nullptr." << dendl;
        return -EINVAL;
    }

    if (tmp_osd_map.pools.count(pool_id) == 0) {
        if (cct != nullptr) ldout(cct, 30) << __func__ << " pool " << pool_id << " not found." << dendl;
        zero_rbi(*p_rbi);
    ...
    if (cct != nullptr) ldout(cct, 30) << __func__ << " Primaries for pool: " << prim_pgs_by_osd << dendl;
    ...
    if (cct != nullptr) {
        for (auto& [osd, pgs] : prim_pgs_by_osd) {
            ldout(cct, 20) << __func__ << " Pool " << pool_id << " OSD." << osd << " has " << pgs.size()
                           << " primary PGs, " << acting_prims_by_osd[osd].size() << " acting primaries." << dendl;
        }
    }
    ...
    if (cct != nullptr) {
        ldout(cct, 20) << __func__ << " pool " << pool_id << " ruleno " << ruleno << " weight-map " << osds_crush_weight
                       << dendl;
    }
    uint osd_pa_count = 0;
    ...
    for (auto [osd, oweight] : osds_crush_weight) {   // loop over all OSDs
        ...
        if (cct != nullptr) {
            ...
            ldout(cct, 30) << __func__ << " OSD." << osd << " info: "
                           << " num_primaries " << np << " num_acting_prims " << nap << " prim_affinity "
                           << tmp_osd_map.get_primary_affinityf(osd) << " weight " << wt << dendl;
        }
    }
    if (cct != nullptr) {
        ldout(cct, 30) << __func__ << " pool " << pool_id << " total_osd_weight " << total_osd_weight
                       << " total_weighted_pa " << total_weighted_pa << dendl;
    }

    if (prim_affinity_sum == 0.0) {
        if (cct != nullptr) {
            ldout(cct, 10) << __func__ << " pool " << pool_id << " has primary_affinity set to zero on all OSDs"
                           << dendl;
        }
        zero_rbi(*p_rbi);
    ...
    if (cct != nullptr) {
        ldout(cct, 30) << __func__ << " pool " << get_pool_name(pool_id) << " pa_avg " << p_rbi->pa_avg
                       << " pa_weighted " << p_rbi->pa_weighted << " pa_weighted_avg " << p_rbi->pa_weighted_avg
                       << " optimal_score " << p_rbi->optimal_score << " adjusted_score " << p_rbi->adjusted_score
                       << " acting_adj_score " << p_rbi->acting_adj_score << dendl;
        ldout(cct, 20) << __func__ << " pool " << get_pool_name(pool_id) << " raw_score: " << p_rbi->raw_score
                       << " acting_raw_score: " << p_rbi->acting_raw_score << dendl;
        ldout(cct, 10) << __func__ << " pool " << get_pool_name(pool_id) << " wl_score: " << p_rbi->acting_adj_score
                       << dendl;
    }
...
void OSDMap::get_pool_ids_by_osd(CephContext* cct, int osd, set<int64_t>* pool_ids) const
{
    ceph_assert(pool_ids);
    set<int> raw_rules;
    int r = crush->get_rules_by_osd(osd, &raw_rules);
    if (r < 0) {
        lderr(cct) << __func__ << " get_rules_by_osd failed: " << cpp_strerror(r) << dendl;
        ceph_assert(r >= 0);
    }
    set<int> rules;
...
template<typename F> class OSDUtilizationDumper : public CrushTreeDumper::Dumper<F>
{
public:
    typedef CrushTreeDumper::Dumper<F> Parent;

    OSDUtilizationDumper(const CrushWrapper* crush, const OSDMap* osdmap_, const PGMap& pgmap_, bool tree_,
                         const string& filter)
        : Parent(crush, osdmap_->get_pool_names())
        , osdmap(osdmap_)
        , pgmap(pgmap_)
        ...
        , sum(0)
    {
        ...
        else if (auto pool_id = osdmap->lookup_pg_pool_name(filter); pool_id >= 0) {
            // filter by pool
            auto crush_rule = osdmap->get_pool_crush_rule(pool_id);
            set<int> roots;
            osdmap->crush->find_takes_by_rule(crush_rule, &roots);
            allowed = roots;
            for (auto r : roots) osdmap->crush->get_all_children(r, &allowed);
        }
...
protected:
    ...
    {
        for (int i = 0; i < osdmap->get_max_osd(); i++) {
            if (osdmap->exists(i) && !this->is_touched(i)) dump_item(CrushTreeDumper::Item(i, 0, 0, 0), f);
        }
    }

    void dump_item(const CrushTreeDumper::Item& qi, F* f) override
    {
        if (!tree && (qi.is_bucket() || dumped_osds.count(qi.id))) return;
        if (!should_dump(qi.id)) return;

        if (!qi.is_bucket()) dumped_osds.insert(qi.id);
        float reweight = qi.is_bucket() ? -1 : osdmap->get_weightf(qi.id);
        int64_t kb = 0, kb_used = 0, kb_used_data = 0, kb_used_omap = 0, kb_used_meta = 0, kb_avail = 0;
        double util = 0;
        if (get_bucket_utilization(qi.id, &kb, &kb_used, &kb_used_data, &kb_used_omap, &kb_used_meta, &kb_avail))
            if (kb_used && kb) util = 100.0 * (double)kb_used / (double)kb;

        double var = 1.0;
        if (average_util) var = util / average_util;

        size_t num_pgs = qi.is_bucket() ? 0 : pgmap.get_num_pg_by_osd(qi.id);

        dump_item(qi, reweight, kb, kb_used, kb_used_data, kb_used_omap, kb_used_meta, kb_avail, util, var, num_pgs, f);
    ...
    virtual void dump_item(const CrushTreeDumper::Item& qi, float& reweight, int64_t kb, int64_t kb_used,
                           int64_t kb_used_data, int64_t kb_used_omap, int64_t kb_used_meta, int64_t kb_avail,
                           double& util, double& var, const size_t num_pgs, F* f) = 0;

    double dev() { return sum > 0 ? sqrt(stddev / sum) : 0; }

    double average_utilization()
    {
        int64_t kb = 0, kb_used = 0;
        for (int i = 0; i < osdmap->get_max_osd(); i++) {
            if (!osdmap->exists(i) || osdmap->get_weight(i) == 0 || !should_dump(i)) continue;
            int64_t kb_i, kb_used_i, kb_used_data_i, kb_used_omap_i, kb_used_meta_i, kb_avail_i;
            if (get_osd_utilization(
                    i, &kb_i, &kb_used_i, &kb_used_data_i, &kb_used_omap_i, &kb_used_meta_i, &kb_avail_i)) {
                kb += kb_i;
                kb_used += kb_used_i;
            }
        }
    ...
    bool get_osd_utilization(int id, int64_t* kb, int64_t* kb_used, int64_t* kb_used_data, int64_t* kb_used_omap,
                             int64_t* kb_used_meta, int64_t* kb_avail) const
    {
    ...
    bool get_bucket_utilization(int id, int64_t* kb, int64_t* kb_used, int64_t* kb_used_data, int64_t* kb_used_omap,
                                int64_t* kb_used_meta, int64_t* kb_avail) const
    {
        if (id >= 0) {
            ...
            return get_osd_utilization(id, kb, kb_used, kb_used_data, kb_used_omap, kb_used_meta, kb_avail);
        }
        ...
        *kb_avail = 0;
        ...
        for (int k = osdmap->crush->get_bucket_size(id) - 1; k >= 0; k--) {
            ...
            int64_t kb_i = 0, kb_used_i = 0, kb_used_data_i = 0, kb_used_omap_i = 0, kb_used_meta_i = 0, kb_avail_i = 0;
            if (!get_bucket_utilization(
                    item, &kb_i, &kb_used_i, &kb_used_data_i, &kb_used_omap_i, &kb_used_meta_i, &kb_avail_i))
                return false;
            *kb += kb_i;
            *kb_used += kb_used_i;
...
public:
    ...
    OSDUtilizationPlainDumper(const CrushWrapper* crush, const OSDMap* osdmap, const PGMap& pgmap, bool tree,
                              const string& filter)
    ...
    void dump(TextTable* tbl)
    {
        ...
        tbl->define_column("STATUS", TextTable::LEFT, TextTable::RIGHT);
        if (tree) tbl->define_column("TYPE NAME", TextTable::LEFT, TextTable::LEFT);
        ...
        *tbl << ""
             << ""
             << ""
             << "TOTAL" << byte_u_t(sum.statfs.total) << byte_u_t(sum.statfs.get_used_raw())
             << byte_u_t(sum.statfs.allocated) << byte_u_t(sum.statfs.omap_allocated)
             << byte_u_t(sum.statfs.internal_metadata) << byte_u_t(sum.statfs.available) << lowprecision_t(average_util)
             << "" << TextTable::endrow;
    }
...
protected:
    ...
    {
        float v;
        explicit lowprecision_t(float _v)
            : v(_v)
        {}
    };
    ...
    void dump_item(const CrushTreeDumper::Item& qi, float& reweight, int64_t kb, int64_t kb_used, int64_t kb_used_data,
                   int64_t kb_used_omap, int64_t kb_used_meta, int64_t kb_avail, double& util, double& var,
                   const size_t num_pgs, TextTable* tbl) override
    {
        const char* c = crush->get_item_class(qi.id);
        if (!c) c = "";
        *tbl << qi.id << c << weightf_t(qi.weight) << weightf_t(reweight) << byte_u_t(kb << 10)
             << byte_u_t(kb_used << 10) << byte_u_t(kb_used_data << 10) << byte_u_t(kb_used_omap << 10)
             << byte_u_t(kb_used_meta << 10) << byte_u_t(kb_avail << 10) << lowprecision_t(util) << lowprecision_t(var);
        ...
        if (tree) {
            ...
            if (qi.is_bucket()) {
                ...
                name << crush->get_type_name(type) << " " << crush->get_item_name(qi.id);
            }
            else {
                name << "osd." << qi.id;
            }
...
public:
    ...
    {
        ostringstream out;
        out << "MIN/MAX VAR: " << lowprecision_t(min_var) << "/" << lowprecision_t(max_var) << "  "
            << "STDDEV: " << lowprecision_t(dev());
        return out.str();
    }
};

ostream& operator<<(ostream& out, const OSDUtilizationPlainDumper::lowprecision_t& v)
{
    if (v.v < -0.01) {
        return out << "-";
    }
...
public:
    ...
    OSDUtilizationFormatDumper(const CrushWrapper* crush, const OSDMap* osdmap, const PGMap& pgmap, bool tree,
                               const string& filter)
        : Parent(crush, osdmap, pgmap, tree, filter)
    {}
...
protected:
    ...
    void dump_item(const CrushTreeDumper::Item& qi, float& reweight, int64_t kb, int64_t kb_used, int64_t kb_used_data,
                   int64_t kb_used_omap, int64_t kb_used_meta, int64_t kb_avail, double& util, double& var,
                   const size_t num_pgs, Formatter* f) override
    {
...
void print_osd_utilization(const OSDMap& osdmap, const PGMap& pgmap, ostream& out, Formatter* f, bool tree,
                           const string& filter)
{
...
void OSDMap::check_health(CephContext* cct, health_check_map_t* checks) const
{
    ...
    if (num_osds >= 0) {
        ...
        for (int i = 0; i < get_max_osd(); i++) {
            ...
            if (is_out(i) || (osd_state[i] & CEPH_OSD_NEW)) continue;
            ++num_in_osds;
            if (down_in_osds.count(i) || up_in_osds.count(i)) continue;
            if (!is_up(i)) {
                down_in_osds.insert(i);
                int parent_id = 0;
                int current = i;
                for (int type = 0; type <= max_type; type++) {
                    if (!crush->get_type_name(type)) continue;
                    int r = crush->get_immediate_parent_id(current, &parent_id);
                    if (r == -ENOENT) break;
                    // break early if this parent is already marked as up
                    if (subtree_up.count(parent_id)) break;
                    type = crush->get_bucket_type(parent_id);
                    if (!subtree_type_is_down(
                            cct, parent_id, type, &down_in_osds, &up_in_osds, &subtree_up, &subtree_type_down))
                        break;
                    current = parent_id;
                }
            }
        ...
        if (num_down_in_osds > 0) {
            ...
            for (int type = max_type; type > 0; type--) {
                ...
                if (subtree_type_down[type].size() > 0) {
                    ostringstream ss;
                    ss << subtree_type_down[type].size() << " " << crush->get_type_name(type);
                    ...
                    string err = string("OSD_") + string(crush->get_type_name(type)) + "_DOWN";
                    boost::to_upper(err);
                    auto& d = checks->add(err, HEALTH_WARN, ss.str(), subtree_type_down[type].size());
            ...
            auto& d = checks->add("OSD_DOWN", HEALTH_WARN, ss.str(), down_in_osds.size());
        ...
        if (!osds.empty()) {
            ...
            auto& d = checks->add("OSD_ORPHAN", HEALTH_WARN, ss.str(), osds.size());
            for (auto osd : osds) {
                ostringstream ss;
    ...
    if (noscrub || nodeepscrub) {
        ...
        auto& d = checks->add("POOL_SCRUB_FLAGS", HEALTH_OK, "Some pool(s) have the " + out + " flag(s) set", 0);
    ...
    // OSD_OUT_OF_ORDER_FULL
    {
        ...
        if (!detail.empty()) {
            auto& d = checks->add("OSD_OUT_OF_ORDER_FULL", HEALTH_ERR, "full ratio(s) out of order", 0);
    ...
    // OSD_FAILSAFE_FULL
    {
        ...
        if (backfillfull.size()) {
            ...
            auto& d = checks->add("OSD_BACKFILLFULL", HEALTH_WARN, ss.str(), backfillfull.size());
    ...
    {
        // warn about flags
        uint64_t warn_flags = CEPH_OSDMAP_PAUSERD | CEPH_OSDMAP_PAUSEWR | CEPH_OSDMAP_PAUSEREC | CEPH_OSDMAP_NOUP |
                              CEPH_OSDMAP_NODOWN | CEPH_OSDMAP_NOIN | CEPH_OSDMAP_NOOUT | CEPH_OSDMAP_NOBACKFILL |
                              CEPH_OSDMAP_NORECOVER | CEPH_OSDMAP_NOSCRUB | CEPH_OSDMAP_NODEEP_SCRUB |
                              CEPH_OSDMAP_NOTIERAGENT | CEPH_OSDMAP_NOSNAPTRIM | CEPH_OSDMAP_NOREBALANCE;
        if (test_flag(warn_flags)) {
            ostringstream ss;
            string s = get_flag_string(get_flags() & warn_flags);
            ss << s << " flag(s) set";
            checks->add("OSDMAP_FLAGS", HEALTH_WARN, ss.str(), s.size() /* kludgey but sufficient */);
        }
    }
    ...
    {
        list<string> detail;
        const unsigned flags = CEPH_OSD_NOUP | CEPH_OSD_NOIN | CEPH_OSD_NODOWN | CEPH_OSD_NOOUT;
        for (int i = 0; i < max_osd; ++i) {
        ...
        for (auto& i : crush_node_flags) {
            if (i.second && crush->item_exists(i.first)) {
                ...
                ss << (tn ? tn : "node") << " " << crush->get_item_name(i.first) << " has flags " << states;
                detail.push_back(ss.str());
            }
        }
        for (auto& i : device_class_flags) {
    ...
    if (cct->_conf->mon_warn_on_legacy_crush_tunables) {
        ...
        if (min < cct->_conf->mon_crush_min_required_version) {
            ostringstream ss;
            ss << "crush map has legacy tunables (require " << min << ", min is "
               << cct->_conf->mon_crush_min_required_version << ")";
    ...
    if (cct->_conf->mon_warn_on_cache_pools_without_hit_sets) {
        ...
        for (auto p = pools.cbegin(); p != pools.cend(); ++p) {
            ...
            if (info.cache_mode_requires_hit_set() && info.hit_set_params.get_type() == HitSet::TYPE_NONE) {
                ostringstream ss;
                ss << "pool '" << get_pool_name(p->first) << "' with cache_mode " << info.get_cache_mode_name()
        ...
        if (!detail.empty()) {
            ...
            auto& d = checks->add("CACHE_POOL_NO_HIT_SET", HEALTH_WARN, ss.str(), detail.size());
            d.detail.swap(detail);
        }
    }
    ...
    // POOL_NEARFULL/BACKFILLFULL/FULL
    {
        ...
        if (!backfillfull_detail.empty()) {
            ...
            auto& d = checks->add("POOL_BACKFILLFULL", HEALTH_WARN, ss.str(), backfillfull_detail.size());
            d.detail.swap(backfillfull_detail);
        }
        if (!nearfull_detail.empty()) {
            ostringstream ss;
            ss << nearfull_detail.size() << " pool(s) nearfull";
            auto& d = checks->add("POOL_NEARFULL", HEALTH_WARN, ss.str(), nearfull_detail.size());
    ...
    if (cct->_conf.get_val<bool>("mon_warn_on_pool_pg_num_not_power_of_two")) {
        ...
        for (auto it : get_pools()) {
            if (!std::has_single_bit(it.second.get_pg_num_target())) {
                ostringstream ss;
                ss << "pool '" << get_pool_name(it.first) << "' pg_num " << it.second.get_pg_num_target()
        ...
        if (!detail.empty()) {
            ...
            auto& d = checks->add("POOL_PG_NUM_NOT_POWER_OF_TWO", HEALTH_WARN, ss.str(), detail.size());
            d.detail.swap(detail);
        }
    }
    ...
    if (cct->_conf.get_val<bool>("mon_warn_on_pool_no_redundancy")) {
        ...
        for (auto it : get_pools()) {
            if (it.second.get_size() == 1) {
                ostringstream ss;
                ss << "pool '" << get_pool_name(it.first) << "' has no replicas configured";
                detail.push_back(ss.str());
            }
        }
        if (!detail.empty()) {
            ...
            auto& d = checks->add("POOL_NO_REDUNDANCY", HEALTH_WARN, ss.str(), detail.size());
            d.detail.swap(detail);
        }
    }
    ...
    if (cct->_conf.get_val<bool>("mon_warn_on_degraded_stretch_mode")) {
        if (recovering_stretch_mode) {
            stringstream ss;
            ss << "We are recovering stretch mode buckets, only requiring " << degraded_stretch_mode << " of "
               << stretch_bucket_count << " buckets to peer";
            checks->add("RECOVERING_STRETCH_MODE", HEALTH_WARN, ss.str(), 0);
        }
        else if (degraded_stretch_mode) {
            stringstream ss;
            ss << "We are missing stretch mode buckets, only requiring " << degraded_stretch_mode << " of "
               << stretch_bucket_count << " buckets to peer";
            checks->add("DEGRADED_STRETCH_MODE", HEALTH_WARN, ss.str(), 0);
        }
    }
    // UNEQUAL_WEIGHT
    if (stretch_mode_enabled) {
...
int OSDMap::parse_osd_id_list(const vector<string>& ls, set<int>* out, ostream* ss) const
{
    out->clear();
    for (auto i = ls.begin(); i != ls.end(); ++i) {
        if (i == ls.begin() && (*i == "any" || *i == "all" || *i == "*")) {
            get_all_osds(*out);
            break;
        }
...
void OSDMap::get_random_up_osds_by_subtree(int n,   // whoami
                                           string& subtree,
                                           int limit,   // how many
                                           set<int> skip, set<int>* want) const
{
    if (limit <= 0) return;
    int subtree_type = crush->get_type_id(subtree);
    if (subtree_type < 1) return;
    vector<int> subtrees;
    ...
    for (auto s : subtrees) {
        if (limit <= 0) break;
        if (crush->subtree_contains(s, n)) continue;
        vector<int> osds;
        crush->get_children_of_type(s, 0, &osds);
        if (osds.empty()) continue;
        vector<int> up_osds;
        for (auto o : osds) {
            if (is_up(o) && !skip.count(o)) up_osds.push_back(o);
        }
        if (up_osds.empty()) continue;
...
float OSDMap::pool_raw_used_rate(int64_t poolid) const
{
    ...
    case pg_pool_t::TYPE_REPLICATED: return pool->get_size();
    ...
    default: ceph_abort_msg("unrecognized pool type");
    }
}
...
unsigned OSDMap::get_crush_node_flags(int id) const
{
    ...
    if (it != crush_node_flags.end()) flags = it->second;
    return flags;
}
...
unsigned OSDMap::get_device_class_flags(int id) const
{
    ...
    if (it != device_class_flags.end()) flags = it->second;
    return flags;
}

std::optional<std::string> OSDMap::pending_require_osd_release() const
{
    if (HAVE_FEATURE(get_up_osd_features(), SERVER_QUINCY) && require_osd_release < ceph_release_t::quincy) {
        return "quincy";
    }
    if (HAVE_FEATURE(get_up_osd_features(), SERVER_PACIFIC) && require_osd_release < ceph_release_t::pacific) {
        return "pacific";
    }
    if (HAVE_FEATURE(get_up_osd_features(), SERVER_OCTOPUS) && require_osd_release < ceph_release_t::octopus) {
        return "octopus";
    }
    if (HAVE_FEATURE(get_up_osd_features(), SERVER_NAUTILUS) && require_osd_release < ceph_release_t::nautilus) {
        return "nautilus";
    }
