...
uint64_t OSDMap::get_features(int entity_type, uint64_t* pmask) const
{
    ...
    if (!pg_upmap.empty() || !pg_upmap_items.empty()) features |= CEPH_FEATUREMASK_OSDMAP_PG_UPMAP;
    ...
    for (auto& pool : pools) {
        ...
        int ruleid = crush->find_rule(pool.second.get_crush_rule(), pool.second.get_type(), pool.second.get_size());
        if (ruleid >= 0) {
    ...
    for (auto& pg : to_check) {
        ...
        if (i != pg_upmap.end()) {
            ...
            if ((int)i->second.size() != get_pg_pool_size(pg)) {
                ldout(cct, 10) << "removing pg_upmap " << i->first << " " << i->second << " != pool size "
        ...
        if (j != pg_upmap_items.end()) {
            ...
            for (auto& p : j->second) {
                ...
                if (p.second != CRUSH_ITEM_NONE && p.second < max_osd && p.second >= 0 && osd_weight[p.second] == 0) {
            ...
            if (newmap.empty()) {
                ...
                ldout(cct, 10) << " simplifying partially no-op pg_upmap_items " << j->first << " " << j->second
                               << " -> " << newmap << dendl;
    ...
    if (q != pg_upmap_items.end()) {
        ...
        for (auto& r : q->second) {
            ...
            for (unsigned i = 0; i < raw->size(); ++i) {
                ...
                if (osd == r.first && pos < 0 &&
                    !(r.second != CRUSH_ITEM_NONE && r.second < max_osd && r.second >= 0 &&
...
void OSDMap::print_pools(ostream& out) const
{
    for (const auto& pool : pools) {
        ...
        const auto& pni = pool_name.find(pool.first);
        if (pni != pool_name.end()) name = pni->second;
        out << "pool " << pool.first << " '" << name << "' " << pool.second << "\n";
    ...
    for (auto& i : pools) {
        ...
        if (poolsize < newcrush->get_rule_mask_min_size(ruleno) ||
            ...
            *ss << "pool " << i.first << " size " << poolsize << " does not"
                << " fall within rule " << ruleno << " min_size " << newcrush->get_rule_mask_min_size(ruleno)
                          ...
                          vector<int>* out)   ///< resulting alternative mapping
{
    ...
    int rule = crush->find_rule(pool->get_crush_rule(), pool->get_type(), pool->get_size());
                           ...
                           OSDMap::Incremental* pending_inc)
{
    ...
    for (auto& i : pools) {
        if (!only_pools.empty() && !only_pools.count(i.first)) continue;
        ...
        map<int, float> pmap;
        int ruleno = tmp.crush->find_rule(i.second.get_crush_rule(), i.second.get_type(), i.second.get_size());
    ...
    for (auto& i : pgs_by_osd) {
        ...
        ldout(cct, 20) << " osd." << i.first << "\tpgs " << i.second.size() << "\ttarget " << target << "\tdeviation "
                       << deviation << dendl;
    ...
    retry:
        ...
        for (auto p = deviation_osd.rbegin(); p != deviation_osd.rend(); ++p) {
            ...
            for (auto pg : pgs) {
                ...
                if (p == tmp.pg_upmap_items.end()) continue;
                mempool::osdmap::vector<pair<int32_t, int32_t>> new_upmap_items;
                for (auto q : p->second) {
                    if (q.second == osd) {
                        ldout(cct, 10) << " will try dropping existing"
                                       << " remapping pair " << q.first << " -> " << q.second << " which remapped "
                                       << pg << " into overfull osd." << osd << dendl;
                ...
                if (new_upmap_items.empty()) {
                    // drop whole item
                    ldout(cct, 10) << " existing pg_upmap_items " << p->second << " remapped " << pg
                    ...
                    ldout(cct, 10) << " existing pg_upmap_items " << p->second << " remapped " << pg
                                   << " into overfull osd." << osd << ", new_pg_upmap_items now " << new_upmap_items
                                   << dendl;
            ...
            for (auto pg : pgs) {
                ...
                if (it != tmp.pg_upmap_items.end() && it->second.size() >= (size_t)pg_pool_size) {
                    ldout(cct, 10) << " " << pg << " already has full-size pg_upmap_items " << it->second
                                   << ", skipping" << dendl;
                    continue;
                }
                ...
                for (unsigned i = 0; i < out.size(); ++i) {
                    ...
                    if (osd_deviation[orig[i]] > max_dev) {
                        ...
                        pos = i;
                        ldout(cct, 30) << "Max osd." << orig[i] << " pos " << i << " dev " << osd_deviation[orig[i]]
                                       << dendl;
                    }
                }
                if (pos != -1) {
                    int i = pos;
                    ldout(cct, 10) << " will try adding new remapping pair " << orig[i] << " -> " << out[i] << " for "
                                   << pg << (orig[i] != osd ? " NOT selected osd" : "") << dendl;
        ...
        for (auto& p : deviation_osd) {
            if (std::find(underfull.begin(), underfull.end(), p.second) == underfull.end()) break;
            int osd = p.second;
            float deviation = p.first;
            float target = osd_weight[osd] * pgs_per_weight;
            ceph_assert(target > 0);
            ...
            // look for remaps we can un-remap
            vector<pair<pg_t, mempool::osdmap::vector<pair<int32_t, int32_t>>>> candidates;
            candidates.reserve(tmp.pg_upmap_items.size());
            for (auto& i : tmp.pg_upmap_items) {
                if (to_skip.count(i.first)) continue;
                if (!only_pools.empty() && !only_pools.count(i.first.pool())) continue;
            ...
            for (auto& i : candidates) {
                ...
                for (auto& j : i.second) {
                    if (j.first == osd) {
                        ldout(cct, 10) << " will try dropping existing"
                                       << " remapping pair " << j.first << " -> " << j.second << " which remapped "
                                       << pg << " out from underfull osd." << osd << dendl;
                ...
                if (new_upmap_items.empty()) {
                    // drop whole item
                    ldout(cct, 10) << " existing pg_upmap_items " << i.second << " remapped " << pg
                    ...
                    ldout(cct, 10) << " existing pg_upmap_items " << i.second << " remapped " << pg
                                   << " out from underfull osd." << osd << ", new_pg_upmap_items now "
                                   << new_upmap_items << dendl;
                    to_upmap[pg] = new_upmap_items;
                    goto test_change;
                }
            }
    ...
    test_change:
        ...
        for (auto& i : temp_pgs_by_osd) {
            ...
            float target = osd_weight[i.first] * pgs_per_weight;
            float deviation = (float)i.second.size() - target;
            ldout(cct, 20) << " osd." << i.first << "\tpgs " << i.second.size() << "\ttarget " << target
