...
constexpr double default_max = std::numeric_limits<double>::is_iec559 ? std::numeric_limits<double>::infinity()
...
struct client_profile_id_t
{
    ...
    friend std::ostream& operator<<(std::ostream& out, const client_profile_id_t& client_profile)
    {
        out << " client_id: " << client_profile.client_id << " profile_id: " << client_profile.profile_id;
        return out;
    }
};
...
struct scheduler_id_t
{
    ...
    friend std::ostream& operator<<(std::ostream& out, const scheduler_id_t& sched_id)
    {
...
class mClockScheduler : public OpScheduler, md_config_obs_t
{
    ...
    {
        std::array<crimson::dmclock::ClientInfo, static_cast<size_t>(op_scheduler_class::immediate)>
            internal_client_infos = {// Placeholder, gets replaced with configured values
    ...
    public:
         ...
         * and client).
         */
        void update_from_config(const ConfigProxy& conf, double capacity_per_shard);
        const crimson::dmclock::ClientInfo* get_info(const scheduler_id_t& id) const;
    } client_registry;

    using mclock_queue_t = crimson::dmclock::PullPriorityQueue<scheduler_id_t, OpSchedulerItem, true, true, 2>;
    using priority_t = unsigned;
    using SubQueue = std::map<priority_t, std::list<OpSchedulerItem>, std::greater<priority_t>>;
    ...
    {
        return scheduler_id_t{item.get_scheduler_class(), client_profile_id_t()};
    }
...
public:
    mClockScheduler(CephContext* cct, int whoami, uint32_t num_shards, int shard_id, bool is_rotational,
                    MonClient* monc);
    ...
    bool empty() const final { return scheduler.empty() && high_priority.empty(); }
