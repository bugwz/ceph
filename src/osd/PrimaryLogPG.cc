...
void PrimaryLogPG::schedule_recovery_work(GenContext<ThreadPool::TPHandle&>* c)
{
...
{
    if (!HAVE_FEATURE(recovery_state.get_min_upacting_features(), SERVER_OCTOPUS)) {
                                                                        ...
                                                                        ObjectContextRef obc)
{
    ...
    for (vector<OSDOp>::iterator p = ops.begin(); p != ops.end(); ++p) {
        ...
        if (op.op == CEPH_OSD_OP_SET_REDIRECT || op.op == CEPH_OSD_OP_SET_CHUNK ||
            ...
            op.op == CEPH_OSD_OP_TIER_FLUSH || op.op == CEPH_OSD_OP_TIER_EVICT) {
            return cache_result_t::NOOP;
        }
    }
...
struct C_SetManifestRefCountDone : public Context
{
    ...
    hobject_t soid;
    C_SetManifestRefCountDone(PrimaryLogPG* p, PrimaryLogPG::ManifestOpRef mop, hobject_t soid)
        : pg(p)
        , mop(mop)
        , soid(soid)
    {}
...
bool PrimaryLogPG::inc_refcount_by_set(OpContext* ctx, object_manifest_t& set_chunk, OSDOp& osd_op)
{
    ...
    if (!refs.is_empty()) {
        ...
        if (inc_ref_count > 0) {
            ...
            ceph_tid_t tid =
                refcount_manifest(ctx->obs->oi.soid, p->first, refcount_t::INCREMENT_REF, fin, std::nullopt);
...
int PrimaryLogPG::do_xattr_cmp_u64(int op, __u64 v1, bufferlist& xattr)
{
    ...
    switch (op) {
    case CEPH_OSD_CMPXATTR_OP_EQ: return (v1 == v2);
    case CEPH_OSD_CMPXATTR_OP_NE: return (v1 != v2);
    case CEPH_OSD_CMPXATTR_OP_GT: return (v1 > v2);
    case CEPH_OSD_CMPXATTR_OP_GTE: return (v1 >= v2);
    case CEPH_OSD_CMPXATTR_OP_LT: return (v1 < v2);
    case CEPH_OSD_CMPXATTR_OP_LTE: return (v1 <= v2);
...
int PrimaryLogPG::do_xattr_cmp_str(int op, string& v1s, bufferlist& xattr)
{
    ...
    switch (op) {
    case CEPH_OSD_CMPXATTR_OP_EQ: return (v1s.compare(v2s) == 0);
    case CEPH_OSD_CMPXATTR_OP_NE: return (v1s.compare(v2s) != 0);
    case CEPH_OSD_CMPXATTR_OP_GT: return (v1s.compare(v2s) > 0);
    case CEPH_OSD_CMPXATTR_OP_GTE: return (v1s.compare(v2s) >= 0);
    case CEPH_OSD_CMPXATTR_OP_LT: return (v1s.compare(v2s) < 0);
    case CEPH_OSD_CMPXATTR_OP_LTE: return (v1s.compare(v2s) <= 0);
    default: return -EINVAL;
    }
}
...
int PrimaryLogPG::do_sparse_read(OpContext* ctx, OSDOp& osd_op)
{
    ...
    if (pool.info.is_erasure()) {
        ...
        int r = osd->store->fiemap(
            ch, ghobject_t(soid, ghobject_t::NO_GEN, info.pgid.shard), op.extent.offset, op.extent.length, m);
        if (r < 0) {
            return r;
        }
        ...
        dout(10) << " sparse_read got " << m.size() << " extents and " << r << " bytes from object " << soid << dendl;
    }
...
void PrimaryLogPG::_make_clone(OpContext* ctx, PGTransaction* t, ObjectContextRef obc, const hobject_t& head,
...
void PrimaryLogPG::make_writeable(OpContext* ctx)
{
    ...
    if ((ctx->obs->exists && !ctx->obs->oi.is_whiteout()) &&                // head exist(ed)
        ...
        for (unsigned i = 0; i < l; i++) snaps[i] = snapc.snaps[i];
        ...
        if (ctx->obs->oi.size) ctx->new_snapset.clone_overlap[coid.snap].insert(0, ctx->obs->oi.size);
...
void PrimaryLogPG::finish_ctx(OpContext* ctx, int log_op_type, int result)
{
    ...
    if (ctx->new_obs.oi.is_dirty() && (ctx->obs->oi.has_manifest() && ctx->obs->oi.manifest.is_chunked()) &&
        ...
        !ctx->delta_stats.num_object_clones && ctx->new_obs.oi.size != 0 &&   // missing, redirect and delete
...
int PrimaryLogPG::start_dedup(OpRequestRef op, ObjectContextRef obc)
{
    ...
    for (auto p : chunks) {
        ...
        ceph_tid_t tid = refcount_manifest(soid, target, refcount_t::CREATE_OR_GET_REF, fin, move(chunks[p.first]));
...
int PrimaryLogPG::finish_set_dedup(hobject_t oid, int r, ceph_tid_t tid, uint64_t offset)
{
    ...
    if (!obc) {
        if (mop->op) osd->reply_op_error(mop->op, -EINVAL);
        return -EINVAL;
    }
    ceph_assert(obc->is_blocked());
    obc->stop_block();
...
PrimaryLogPG::RepGather* PrimaryLogPG::new_repop(OpContext* ctx, ObjectContextRef obc, ceph_tid_t rep_tid)
{
    if (ctx->op)
                                                  ...
                                                  const map<string, bufferlist>* attrs)
{
    ...
    dout(10) << __func__ << ": " << obc << " " << soid << " " << obc->rwstate << " oi: " << obc->obs.oi
             << " exists: " << (int)obc->obs.exists << " ssc: " << obc->ssc << " snapset: " << obc->ssc->snapset
             << dendl;
    return obc;
}
...
SnapSetContext* PrimaryLogPG::get_snapset_context(const hobject_t& oid, bool can_create,
                                                  const map<string, bufferlist>* attrs, bool oid_existed)
{
...
void PrimaryLogPG::remove_missing_object(const hobject_t& soid, eversion_t v, Context* on_complete)
{
    ...
    epoch_t cur_epoch = get_osdmap_epoch();
    t.register_on_complete(new LambdaContext([=](int) {
...
void PrimaryLogPG::do_update_log_missing(OpRequestRef& op)
{
    ...
    Context* complete = new LambdaContext([=](int) {
...
void PrimaryLogPG::setattrs_maybe_cache(ObjectContextRef obc, PGTransaction* t, map<string, bufferlist>& attrs)
{
...
int PrimaryLogPG::getattrs_maybe_cache(ObjectContextRef obc, map<string, bufferlist>* out)
{
    int r = 0;
    ceph_assert(out);
    if (pool.info.is_erasure()) {
    ...
    for (map<string, bufferlist>::iterator i = out->begin(); i != out->end(); ++i) {
        if (i->first.size() > 1 && i->first[0] == '_') tmp[i->first.substr(1, i->first.size())] = std::move(i->second);
    }
    tmp.swap(*out);
    return r;
}
