...
template<typename T> static ostream& _prefix(std::ostream* _dout, T* pg)
{
...
template<typename T> class PrimaryLogPG::BlessedGenContext : public GenContext<T>
{
    PrimaryLogPGRef pg;
...
GenContext<ThreadPool::TPHandle&>* PrimaryLogPG::bless_gencontext(GenContext<ThreadPool::TPHandle&>* c)
{
    return new BlessedGenContext<ThreadPool::TPHandle&>(this, c, get_osdmap_epoch());
}

template<typename T> class PrimaryLogPG::UnlockedBlessedGenContext : public GenContext<T>
{
    PrimaryLogPGRef pg;
    unique_ptr<GenContext<T>> c;
    epoch_t e;
...
GenContext<ThreadPool::TPHandle&>* PrimaryLogPG::bless_unlocked_gencontext(GenContext<ThreadPool::TPHandle&>* c)
{
    return new UnlockedBlessedGenContext<ThreadPool::TPHandle&>(this, c, get_osdmap_epoch());
}
...
public:
    C_PG_ObjectContext(PrimaryLogPG* p, ObjectContext* o)
        ...
        , obc(o)
    {}
    void finish(int r) override { pg->object_context_destructor_callback(obc); }
};
...
struct OnReadComplete : public Context
{
    ...
    OnReadComplete(PrimaryLogPG* pg, PrimaryLogPG::OpContext* ctx)
        : pg(pg)
        , opcontext(ctx)
    {}
    void finish(int r) override { opcontext->finish_read(pg); }
...
public:
    C_OSD_CommittedPushedObject(PrimaryLogPG* p, epoch_t epoch, eversion_t lc)
        : pg(p)
        , epoch(epoch)
        , last_complete(lc)
    {}
    void finish(int r) override { pg->_committed_pushed_object(epoch, last_complete); }
};
...
{
    inflightreads = 1;
    list<pair<boost::tuple<uint64_t, uint64_t, unsigned>, pair<bufferlist*, Context*>>> in;
    in.swap(pending_async_reads);
    pg->pgbackend->objects_read_async(obc->obs.oi.soid, in, new OnReadComplete(pg, this), pg->get_pool().fast_read);
}
    ...
    void finish(PrimaryLogPG::CopyCallbackResults results_) override
    {
        ...
        if (r >= 0) {
            ...
            if (r != -ECANCELED) {   // on cancel just toss it out; client resends
                if (ctx->op) ctx->pg->osd->reply_op_error(ctx->op, r);
            }
            else if (results->should_requeue) {
                if (ctx->op) ctx->pg->requeue_op(ctx->op);
            }
...
void PrimaryLogPG::on_local_recover(const hobject_t& hoid, const ObjectRecoveryInfo& _recovery_info,
                                    ObjectContextRef obc, bool is_delete, ObjectStore::Transaction* t)
{
    ...
    if (!is_delete && recovery_info.soid.is_snap()) {
        ...
        if (p != recovery_info.ss.clone_snaps.end()) {
            ...
            snap_mapper.add_oid(recovery_info.soid, snaps, &_t);
        }
        else {
    ...
    if (!is_delete && recovery_state.get_pg_log().get_missing().is_missing(recovery_info.soid) &&
        recovery_state.get_pg_log().get_missing().get_items().find(recovery_info.soid)->second.need >
            recovery_info.version) {
        ceph_assert(is_primary());
        const pg_log_entry_t* latest = recovery_state.get_pg_log().get_log().objects.find(recovery_info.soid)->second;
        if (latest->op == pg_log_entry_t::LOST_REVERT && latest->reverting_to == recovery_info.version) {
            ...
            bufferlist bl;
            encode(recovery_info.oi, bl, get_osdmap()->get_features(CEPH_ENTITY_TYPE_OSD, nullptr));
            ceph_assert(!pool.info.is_erasure());
            t->setattr(coll, ghobject_t(recovery_info.soid), OI_ATTR, bl);
            if (obc) obc->attr_cache[OI_ATTR] = bl;
        }
    }
    ...
    recovery_state.recover_got(recovery_info.soid, recovery_info.version, is_delete, *t);
    ...
    if (is_primary()) {
        ...
        t->register_on_applied(new C_OSD_AppliedRecoveredObjectReplica(this));
    }

    t->register_on_commit(new C_OSD_CommittedPushedObject(this, get_osdmap_epoch(), info.last_complete));
}

void PrimaryLogPG::on_global_recover(const hobject_t& soid, const object_stat_sum_t& stat_diff, bool is_delete)
{
...
void PrimaryLogPG::schedule_recovery_work(GenContext<ThreadPool::TPHandle&>* c, uint64_t cost)
{
    osd->queue_recovery_context(this, c, cost, recovery_state.get_recovery_op_priority());
}

void PrimaryLogPG::replica_clear_repop_obc(const vector<pg_log_entry_t>& logv, ObjectStore::Transaction& t)
{
    for (auto&& e : logv) {
        /* Have to blast all clones, they share a snapset */
        object_contexts.clear_range(e.soid.get_object_boundary(), e.soid.get_head());
        ceph_assert(snapset_contexts.find(e.soid.get_head()) == snapset_contexts.end());
    }
}

bool PrimaryLogPG::should_send_op(pg_shard_t peer, const hobject_t& hoid)
{
    if (peer == get_primary()) return true;
    ceph_assert(recovery_state.has_peer_info(peer));
    bool should_send = hoid.pool != (int64_t)info.pgid.pool() || hoid <= last_backfill_started ||
                       hoid <= recovery_state.get_peer_info(peer).last_backfill;
    if (!should_send) {
        ceph_assert(is_backfill_target(peer));
        dout(10) << __func__ << " issue_repop shipping empty opt to osd." << peer << ", object " << hoid
                 << " beyond std::max(last_backfill_started "
                 << ", peer_info[peer].last_backfill " << recovery_state.get_peer_info(peer).last_backfill << ")"
                 << dendl;
        return should_send;
    }
    if (is_async_recovery_target(peer) && recovery_state.get_peer_missing(peer).is_missing(hoid)) {
        should_send = false;
        dout(10) << __func__ << " issue_repop shipping empty opt to osd." << peer << ", object " << hoid
...
ConnectionRef PrimaryLogPG::get_con_osd_cluster(int peer, epoch_t from_epoch)
{
...
void PrimaryLogPG::maybe_kick_recovery(const hobject_t& soid)
{
    eversion_t v;
    bool work_started = false;
    if (!recovery_state.get_missing_loc().needs_recovery(soid, &v)) return;
...
void PrimaryLogPG::wait_for_unreadable_object(const hobject_t& soid, OpRequestRef op)
{
...
bool PrimaryLogPG::is_degraded_or_backfilling_object(const hobject_t& soid)
{
     ...
     */
    if (waiting_for_degraded_object.count(soid)) return true;
    if (recovery_state.get_pg_log().get_missing().get_items().count(soid)) return true;
    ...
    for (set<pg_shard_t>::iterator i = get_acting_recovery_backfill().begin();
        ...
        if (is_backfill_target(peer) && recovery_state.get_peer_info(peer).last_backfill <= soid &&
            last_backfill_started >= soid && backfills_in_flight.count(soid))
            return true;
    }
    return false;
}
...
void PrimaryLogPG::block_write_on_full_cache(const hobject_t& _oid, OpRequestRef op)
{
...
void PrimaryLogPG::block_for_clean(const hobject_t& oid, OpRequestRef op)
{
...
void PrimaryLogPG::block_write_on_snap_rollback(const hobject_t& oid, ObjectContextRef obc, OpRequestRef op)
{
    dout(20) << __func__ << ": blocking object " << oid.get_head() << " on snap promotion " << obc->obs.oi.soid
             << dendl;
...
void PrimaryLogPG::block_write_on_degraded_snap(const hobject_t& snap, OpRequestRef op)
{
...
bool PrimaryLogPG::maybe_await_blocked_head(const hobject_t& hoid, OpRequestRef op)
{
    ObjectContextRef obc;
...
void PrimaryLogPG::maybe_force_recovery()
{
    ...
    if (!is_degraded() && !state_test(PG_STATE_RECOVERING | PG_STATE_RECOVERY_WAIT | PG_STATE_BACKFILLING |
                                      PG_STATE_BACKFILL_WAIT | PG_STATE_BACKFILL_TOOFULL))
        return;

    if (recovery_state.get_pg_log().get_log().approx_size() <
        cct->_conf->osd_max_pg_log_entries * cct->_conf->osd_force_recovery_pg_log_entries_factor)
        return;
    ...
    for (set<pg_shard_t>::iterator it = get_acting_recovery_backfill().begin();
        ...
        if (it_missing != recovery_state.get_peer_missing().end() && !it_missing->second.get_rmissing().empty()) {
            const auto& min_obj = recovery_state.get_peer_missing(peer).get_rmissing().begin();
            dout(20) << __func__ << " peer " << peer << " min_version " << min_obj->first << " oid " << min_obj->second
                     << dendl;
    ...
    // recover it
    if (soid != hobject_t()) maybe_kick_recovery(soid);
}

bool PrimaryLogPG::check_laggy(OpRequestRef& op)
{
    assert(HAVE_FEATURE(recovery_state.get_min_upacting_features(), SERVER_OCTOPUS));
...
void PrimaryLogPG::recheck_readable()
{
    ...
    if (is_wait()) {
        ...
        if (mnow < prior_readable_until_ub) {
            dout(10) << __func__ << " still wait (mnow " << mnow << " < prior_readable_until_ub "
                     << prior_readable_until_ub << ")" << dendl;
        }
        else {
            dout(10) << __func__ << " no longer wait (mnow " << mnow << " >= prior_readable_until_ub "
                     << prior_readable_until_ub << ")" << dendl;
...
bool PrimaryLogPG::pgls_filter(const PGLSFilter& filter, const hobject_t& sobj)
{
    ...
    if (!filter.get_xattr().empty()) {
        int ret = pgbackend->objects_get_attr(sobj, filter.get_xattr(), &bl);
...
std::pair<int, std::unique_ptr<const PGLSFilter>> PrimaryLogPG::get_pgls_filter(bufferlist::const_iterator& iter)
{
    string type;
...
void PrimaryLogPG::do_command(const string_view& orig_prefix, const cmdmap_t& cmdmap, const bufferlist& idata,
                              std::function<void(int, const std::string&, bufferlist&)> on_finish)
{
    ...
    if (prefix == "query") {
        ...
        if (agent_state) agent_state->dump(f.get());
        f->close_section();
        ...
        ceph_assert(mode == pg_log_entry_t::LOST_REVERT || mode == pg_log_entry_t::LOST_DELETE);
        ...
        if (!recovery_state.all_unfound_are_queried_or_lost(get_osdmap())) {
            ss << "pg has " << unfound << " unfound objects but we haven't probed all sources, not marking lost";
            ret = -EINVAL;
            goto out;
        }
        ...
        if (cmd_getval(cmdmap, "offset", offset_json)) {
            json_spirit::Value v;
            try {
                if (!json_spirit::read(offset_json, v)) throw std::runtime_error("bad json");
                offset.decode(v);
            }
        ...
        auto& needs_recovery_map = recovery_state.get_missing_loc().get_needs_recovery();
        f->dump_int("num_missing", needs_recovery_map.size());
        f->dump_int("num_unfound", get_num_unfound());
        map<hobject_t, pg_missing_item>::const_iterator p = needs_recovery_map.upper_bound(offset);
        {
            ...
            for (; p != needs_recovery_map.end() && num < cct->_conf->osd_command_max_records; ++p) {
                if (recovery_state.get_missing_loc().is_unfound(p->first)) {
                    ...
                    p->second.dump(f.get());   // have, need keys
                    {
                        ...
                        for (auto&& r : recovery_state.get_missing_loc().get_locations(p->first)) {
        ...
        if (is_primary()) {
            ...
            if (deep) {
                ...
                scrub_max_interval =
                    pool_scrub_max_interval > 0 ? pool_scrub_max_interval : g_conf()->osd_deep_scrub_interval;
            }
            else {
                p->opts.get(pool_opts_t::SCRUB_MAX_INTERVAL, &pool_scrub_max_interval);
                scrub_max_interval =
                    pool_scrub_max_interval > 0 ? pool_scrub_max_interval : g_conf()->osd_scrub_max_interval;
            }
    ...
    else if (prefix == "block" || prefix == "unblock" || prefix == "set" || prefix == "unset") {
        string value;
...
void PrimaryLogPG::do_pg_op(OpRequestRef op)
{
    ...
    for (vector<OSDOp>::iterator p = ops.begin(); p != ops.end(); ++p) {
        ...
        case CEPH_OSD_OP_PGNLS:
            ...
            if (get_osdmap()->raw_pg_to_pg(m->get_pg()) != info.pgid.pgid) {
                dout(10) << " pgnls pg=" << m->get_pg() << " " << get_osdmap()->raw_pg_to_pg(m->get_pg())
                         << " != " << info.pgid << dendl;
                result = 0;   // hmm?
            }
            else {
                unsigned list_size = std::min<uint64_t>(cct->_conf->osd_max_pgls, p->op.pgls.count);
                ...
                int r = pgbackend->objects_list_partial(current, list_size, list_size, &sentries, &next);
                if (r != 0) {
                    result = -EINVAL;
                    break;
                }
                ...
                while (1) {
                    const hobject_t& mcand = missing_iter == recovery_state.get_pg_log().get_missing().get_items().end()
                                                 ? _max
                                                 : missing_iter->first;
                    const hobject_t& lcand = ls_iter == sentries.end() ? _max : *ls_iter;
                    ...
                    dout(10) << " pgnls candidate 0x" << std::hex << candidate.get_hash() << " vs lower bound 0x"
                             << lower_bound.get_hash() << std::dec << dendl;
                    ...
                    if (candidate.snap != CEPH_NOSNAP) continue;

                    // skip internal namespace
                    if (candidate.get_namespace() == cct->_conf->osd_hit_set_namespace) continue;

                    if (recovery_state.get_missing_loc().is_deleted(candidate)) continue;
                    ...
                    if (filter && !pgls_filter(*filter, candidate)) continue;

                    dout(20) << "pgnls item 0x" << std::hex << candidate.get_hash() << ", rev 0x"
                             << hobject_t::_reverse_bits(candidate.get_hash()) << std::dec << " " << candidate.oid.name
                             << dendl;
                ...
                if (next.is_max() && missing_iter == recovery_state.get_pg_log().get_missing().get_items().end() &&
        ...
        case CEPH_OSD_OP_PGLS:
            ...
            if (get_osdmap()->raw_pg_to_pg(m->get_pg()) != info.pgid.pgid) {
                dout(10) << " pgls pg=" << m->get_pg() << " " << get_osdmap()->raw_pg_to_pg(m->get_pg())
                         << " != " << info.pgid << dendl;
                result = 0;   // hmm?
            }
            else {
                ...
                while (1) {
                    ...
                    // skip wrong namespace
                    if (candidate.get_namespace() != m->get_hobj().nspace) continue;

                    if (recovery_state.get_missing_loc().is_deleted(candidate)) continue;

                    if (filter && !pgls_filter(*filter, candidate)) continue;

                    response.entries.push_back(make_pair(candidate.oid, candidate.get_key()));
                }
                if (next.is_max() && missing_iter == recovery_state.get_pg_log().get_missing().get_items().end() &&
                    ls_iter == sentries.end()) {
                    result = 1;
        ...
        case CEPH_OSD_OP_PG_HITSET_LS:
        {
            ...
            if (hit_set) ls.push_back(make_pair(hit_set_start_stamp, utime_t()));
        ...
        case CEPH_OSD_OP_SCRUBLS: result = do_scrub_ls(m, &osd_op); break;
    ...
    // reply
    MOSDOpReply* reply = new MOSDOpReply(m, 0, get_osdmap_epoch(), CEPH_OSD_FLAG_ACK | CEPH_OSD_FLAG_ONDISK, false);
...
bool PrimaryLogPG::get_rw_locks(bool write_ordered, OpContext* ctx)
{
    ...
    if (ctx->head_obc) {
        ...
        if (!ctx->lock_manager.get_lock_type(ctx->lock_type, ctx->head_obc->obs.oi.soid, ctx->head_obc, ctx->op)) {
 ...
 */
void PrimaryLogPG::release_object_locks(ObcLockManager& lock_manager)
{
    ...
    if (requeue_recovery) queue_recovery();
    if (requeue_snaptrim) snap_trimmer_machine.process_event(TrimWriteUnblocked());
    ...
    if (!to_req.empty()) {
        ...
        for (auto&& p : to_req) {
            if (m_scrubber->write_blocked_by_scrub(p.first->obs.oi.soid.get_head())) {
                ...
                waiting_for_scrub.splice(waiting_for_scrub.begin(), p.second, p.second.begin(), p.second.end());
            }
            else if (is_laggy()) {
                for (auto& op : p.second) {
                    op->mark_delayed("waiting for readable");
                }
                waiting_for_readable.splice(waiting_for_readable.begin(), p.second, p.second.begin(), p.second.end());
            }
            else {
...
PrimaryLogPG::PrimaryLogPG(OSDService* o, OSDMapRef curmap, const PGPool& _pool, const map<string, string>& ec_profile,
                           spg_t p)
    : PG(o, curmap, _pool, p)
    , pgbackend(PGBackend::build_pg_backend(_pool.info, ec_profile, this, coll_t(p), ch, o->store, cct))
    , object_contexts(o->cct, o->cct->_conf->osd_pg_object_context_cache_count)
    , new_backfill(false)
    , temp_seq(0)
    , snap_trimmer_machine(this)
{
    recovery_state.set_backend_predicates(pgbackend->get_is_readable_predicate(),
...
{
    src_oloc = oloc;
    if (oloc.key.empty()) src_oloc.key = oid.name;
}
...
void PrimaryLogPG::do_request(OpRequestRef& op, ThreadPool::TPHandle& handle)
{
    ...
    if (pgbackend->handle_message(op)) return;

    switch (msg_type) {
    case CEPH_MSG_OSD_OP:
    case CEPH_MSG_OSD_BACKOFF:
        if (!is_active()) {
            dout(20) << " peered, not active, waiting for active on " << *op->get_req() << dendl;
    ...
    case MSG_OSD_PG_SCAN: do_scan(op, handle); break;

    case MSG_OSD_PG_BACKFILL: do_backfill(op); break;

    case MSG_OSD_PG_BACKFILL_REMOVE: do_backfill_remove(op); break;
    ...
    case MSG_OSD_SCRUB_RESERVE:
    {
        ...
        switch (m->type) {
        case MOSDScrubReserve::REQUEST: m_scrubber->handle_scrub_reserve_request(op); break;
        case MOSDScrubReserve::GRANT: m_scrubber->handle_scrub_reserve_grant(op, m->from); break;
        case MOSDScrubReserve::REJECT: m_scrubber->handle_scrub_reserve_reject(op, m->from); break;
        case MOSDScrubReserve::RELEASE: m_scrubber->handle_scrub_reserve_release(op); break;
        }
    } break;

    case MSG_OSD_REP_SCRUB: replica_scrub(op, handle); break;

    case MSG_OSD_REP_SCRUBMAP: do_replica_scrub_map(op); break;

    case MSG_OSD_PG_UPDATE_LOG_MISSING: do_update_log_missing(op); break;

    case MSG_OSD_PG_UPDATE_LOG_MISSING_REPLY: do_update_log_missing_reply(op); break;

    default: ceph_abort_msg("bad message type in do_request");
    }
}
...
void PrimaryLogPG::do_op(OpRequestRef& op)
{
    ...
    if (!info.pgid.pgid.contains(info.pgid.pgid.get_split_bits(pool.info.get_pg_num()), head)) {
        derr << __func__ << " " << info.pgid.pgid << " does not contain " << head << " pg_num "
             << pool.info.get_pg_num() << " hash " << std::hex << head.get_hash() << std::dec << dendl;
    ...
    if ((m->get_flags() & (CEPH_OSD_FLAG_BALANCE_READS | CEPH_OSD_FLAG_LOCALIZE_READS)) && op->may_read() &&
    ...
    if (write_ordered &&
        !(m->get_source().is_mds() || m->has_flag(CEPH_OSD_FLAG_FULL_TRY) || m->has_flag(CEPH_OSD_FLAG_FULL_FORCE)) &&
    ...
    if (write_ordered && osd->check_failsafe_full(get_dpp()) && !m->has_flag(CEPH_OSD_FLAG_FULL_TRY)) {
    ...
    dout(10) << "do_op " << *m << (op->may_write() ? " may_write" : "") << (op->may_read() ? " may_read" : "")
             << (op->may_cache() ? " may_cache" : "") << " -> " << (write_ordered ? "write-ordered" : "read-ordered")
             << " flags " << ceph_osd_flag_string(m->get_flags()) << dendl;
    ...
    if (is_unreadable_object(head)) {
        ...
        if (can_backoff && (g_conf()->osd_backoff_on_degraded ||
                            (g_conf()->osd_backoff_on_unfound && recovery_state.get_missing_loc().is_unfound(head)))) {
    ...
    if (op->may_write() || op->may_cache()) {
        ...
        bool got = check_in_progress_op(m->get_reqid(), &version, &user_version, &return_code, &op_returns);
        if (got) {
    ...
    if (hit_set) {
        if (obc.get()) {
            if (obc->obs.oi.soid != hobject_t() && hit_set->contains(obc->obs.oi.soid)) in_hit_set = true;
        }
        else {
            if (missing_oid != hobject_t() && hit_set->contains(missing_oid)) in_hit_set = true;
        }
    ...
    if (agent_state) {
        if (agent_choose_mode(false, op)) return;
    }
    ...
    if (obc.get() && obc->obs.exists) {
        ...
        if (maybe_handle_manifest(op, write_ordered, obc)) return;
    }

    if (maybe_handle_cache(op, write_ordered, obc, r, missing_oid, false, in_hit_set)) return;
...
PrimaryLogPG::cache_result_t PrimaryLogPG::maybe_handle_manifest_detail(OpRequestRef op, bool write_ordered,
                                                                        ObjectContextRef obc)
{
    if (!obc) {
    ...
    if (!obc->obs.oi.has_manifest()) {
        dout(20) << __func__ << ": " << obc->obs.oi.soid << " is not manifest object " << dendl;
        return cache_result_t::NOOP;
    }
    ...
    for (vector<OSDOp>::iterator p = ops.begin(); p != ops.end(); ++p) {
        ...
        if (op.op == CEPH_OSD_OP_SET_REDIRECT || op.op == CEPH_OSD_OP_SET_CHUNK ||
            op.op == CEPH_OSD_OP_UNSET_MANIFEST || op.op == CEPH_OSD_OP_TIER_PROMOTE ||
            op.op == CEPH_OSD_OP_TIER_FLUSH || op.op == CEPH_OSD_OP_TIER_EVICT || op.op == CEPH_OSD_OP_ISDIRTY) {
            return cache_result_t::NOOP;
        }
    }
    ...
    default: ceph_abort_msg("unrecognized manifest type");
    }

    return cache_result_t::NOOP;
}

void PrimaryLogPG::record_write_error(OpRequestRef op, const hobject_t& soid, MOSDOpReply* orig_reply, int r,
                                      OpContext* ctx_for_op_returns)
{
    ...
    entries.push_back(
        pg_log_entry_t(pg_log_entry_t::ERROR, soid, get_next_version(), eversion_t(), 0, reqid, utime_t(), r));
    ...
    struct OnComplete
    {
        ...
        int r;
        OnComplete(PrimaryLogPG* pg, OpRequestRef op, MOSDOpReply* orig_reply, int r)
            : pg(pg)
            , op(op)
            , orig_reply(orig_reply, false /* take over ref */)
            , r(r)
        {}
        void operator()()
        {
    ...
    submit_log_entries(entries,
                       std::move(lock_manager),
                       std::optional<std::function<void(void)>>(OnComplete(this, op, orig_reply, r)),
                       op,
                       r);
}
                                                                     ...
                                                                     ObjectContextRef* promote_obc)
{
    ...
    if (pool.info.cache_mode == pg_pool_t::CACHEMODE_NONE) return cache_result_t::NOOP;

    if (op && op->get_req() && op->get_req()->get_type() == CEPH_MSG_OSD_OP &&
        (op->get_req<MOSDOp>()->get_flags() & CEPH_OSD_FLAG_IGNORE_CACHE)) {
    ...
    case pg_pool_t::CACHEMODE_WRITEBACK:
        ...
        if (op->may_write() || op->may_cache()) {
            ...
            // Promote too?
            if (!op->need_skip_promote() && maybe_promote(obc,
                                                          missing_oid,
                                                          oloc,
                                                          in_hit_set,
            ...
            if (obc.get() && obc->is_blocked()) {
                if (promote_obc) *promote_obc = obc;
                return cache_result_t::BLOCKED_PROMOTE;
            }

            // Promote too?
            if (!op->need_skip_promote()) {
                (void)maybe_promote(obc,
                                    missing_oid,
                                    oloc,
                                    in_hit_set,
    ...
    default: ceph_abort_msg("unrecognized cache_mode");
    }
    return cache_result_t::NOOP;
}

bool PrimaryLogPG::maybe_promote(ObjectContextRef obc, const hobject_t& missing_oid, const object_locator_t& oloc,
                                 bool in_hit_set, uint32_t recency, OpRequestRef promote_op,
                                 ObjectContextRef* promote_obc)
{
    ...
    default:
    {
        ...
        if (count) {
            ...
            for (map<time_t, HitSetRef>::reverse_iterator itor = agent_state->hit_set_map.rbegin();
...
void PrimaryLogPG::do_cache_redirect(OpRequestRef op)
{
    ...
    dout(10) << "sending redirect to pool " << pool.info.tier_of << " for op " << *op->get_req() << dendl;
...
struct C_ProxyRead : public Context
{
    ...
    utime_t start;
    C_ProxyRead(PrimaryLogPG* p, hobject_t o, epoch_t lpr, const PrimaryLogPG::ProxyReadOpRef& prd)
        : pg(p)
        , oid(o)
...
struct C_ProxyChunkRead : public Context
{
    ...
    C_ProxyChunkRead(PrimaryLogPG* p, hobject_t o, epoch_t lpr, const PrimaryLogPG::ProxyReadOpRef& prd)
        : pg(p)
        , oid(o)
        , last_peering_reset(lpr)
        , tid(0)
    ...
    void finish(int r) override
    {
        ...
        if (last_peering_reset == pg->get_last_peering_reset()) {
            if (r >= 0) {
                ...
                prdop->ops[op_index]
                    .outdata.begin(copy_offset)
                    .copy_in(obj_op->ops[0].outdata.length(), obj_op->ops[0].outdata.c_str());
            }
...
void PrimaryLogPG::do_proxy_read(OpRequestRef op, ObjectContextRef obc)
{
    ...
    flags |= m->get_flags() & (CEPH_OSD_FLAG_RWORDERED | CEPH_OSD_FLAG_ORDERSNAP | CEPH_OSD_FLAG_ENFORCE_SNAPC |
    ...
    ceph_tid_t tid = osd->objecter->read(soid.oid,
                                         oloc,
                                         obj_op,
                                         m->get_snapid(),
                                         NULL,
...
void PrimaryLogPG::finish_proxy_read(hobject_t oid, ceph_tid_t tid, int r)
{
    ...
    else if (std::find(q->second.begin(), q->second.end(), prdop->op) != q->second.end()) {
        /* multiple read case */
...
void PrimaryLogPG::kick_proxy_ops_blocked(hobject_t& soid)
{
    ...
    if (p == in_progress_proxy_ops.end()) return;
...
void PrimaryLogPG::cancel_proxy_read(ProxyReadOpRef prdop, vector<ceph_tid_t>* tids)
{
...
void PrimaryLogPG::cancel_proxy_ops(bool requeue, vector<ceph_tid_t>* tids)
{
    ...
    if (requeue) {
        map<hobject_t, list<OpRequestRef>>::iterator p = in_progress_proxy_ops.begin();
...
struct C_ProxyWrite_Commit : public Context
{
    ...
    C_ProxyWrite_Commit(PrimaryLogPG* p, hobject_t o, epoch_t lpr, const PrimaryLogPG::ProxyWriteOpRef& pw)
        : pg(p)
        , oid(o)
        , last_peering_reset(lpr)
        , tid(0)
...
void PrimaryLogPG::do_proxy_write(OpRequestRef op, ObjectContextRef obc)
{
    ...
    C_ProxyWrite_Commit* fin = new C_ProxyWrite_Commit(this, soid, get_last_peering_reset(), pwop);
    ceph_tid_t tid = osd->objecter->mutate(soid.oid,
                                           oloc,
                                           obj_op,
                                           snapc,
...
void PrimaryLogPG::do_proxy_chunked_op(OpRequestRef op, const hobject_t& missing_oid, ObjectContextRef obc,
                                       bool write_ordered)
{
    MOSDOp* m = static_cast<MOSDOp*>(op->get_nonconst_req());
    OSDOp* osd_op = NULL;
    for (unsigned int i = 0; i < m->ops.size(); i++) {
        ...
        for (auto& p : chunk_read) {
            ...
            dout(20) << __func__ << " chunk_index: " << chunks->first << " next_length: " << chunks->second
                     << " cursor: " << p.first << dendl;
...
public:
    ...
    void set_requeue(bool rq) { requeue = rq; }
};
...
struct SetManifestFinisher : public PrimaryLogPG::OpFinisher
{
    ...
    explicit SetManifestFinisher(OSDOp& osd_op)
        : osd_op(osd_op)
    {}

    int execute() override { return osd_op.rval; }
};
...
struct C_SetManifestRefCountDone : public Context
{
    ...
    uint64_t offset;
    ceph_tid_t tid = 0;
    C_SetManifestRefCountDone(PrimaryLogPG* p, hobject_t soid, uint64_t offset)
        : pg(p)
        , soid(soid)
        , offset(offset)
    {}
...
int PrimaryLogPG::get_manifest_ref_count(ObjectContextRef obc, std::string& fp_oid, OpRequestRef op)
{
    ...
    for (vector<snapid_t>::const_reverse_iterator p = ss.clones.rbegin(); p != ss.clones.rend(); ++p) {
        ...
        if (is_unreadable_object(clone_oid)) {
            dout(10) << __func__ << ": " << clone_oid << " is unreadable. Need to wait for recovery" << dendl;
        ...
        clone_obc->obs.oi.manifest.calc_refs_to_inc_on_set(obc_g ? &(obc_g->obs.oi.manifest) : nullptr, nullptr, refs);
...
bool PrimaryLogPG::recover_adjacent_clones(ObjectContextRef obc, OpRequestRef op)
{
    ...
    bool has_manifest_op = std::any_of(
        begin(m->ops), end(m->ops), [](const auto& osd_op) { return osd_op.op.op == CEPH_OSD_OP_SET_CHUNK; });
...
{
    auto s = std::find(obc->ssc->snapset.clones.begin(), obc->ssc->snapset.clones.end(), obc->obs.oi.soid.snap);
...
void PrimaryLogPG::dec_refcount(const hobject_t& soid, const object_ref_delta_t& refs)
{
    for (auto p = refs.begin(); p != refs.end(); ++p) {
        ...
        while (dec_ref_count < 0) {
            ...
            refcount_manifest(soid, p->first, refcount_t::DECREMENT_REF, NULL, std::nullopt);
            dec_ref_count++;
        }
    }
}
...
void PrimaryLogPG::get_adjacent_clones(ObjectContextRef src_obc, ObjectContextRef& _l, ObjectContextRef& _g)
{
    const SnapSet& snapset = src_obc->ssc->snapset;
    const object_info_t& oi = src_obc->obs.oi;

    auto get_context = [this, &oi, &snapset](auto iter) -> ObjectContextRef {
...
bool PrimaryLogPG::inc_refcount_by_set(OpContext* ctx, object_manifest_t& set_chunk, OSDOp& osd_op)
{
    object_ref_delta_t refs;
    ObjectContextRef obc_l, obc_g;
    get_adjacent_clones(ctx->obc, obc_l, obc_g);
    set_chunk.calc_refs_to_inc_on_set(
        obc_l ? &(obc_l->obs.oi.manifest) : nullptr, obc_g ? &(obc_g->obs.oi.manifest) : nullptr, refs);
    ...
    if (!refs.is_empty()) {
        ...
        for (auto c : set_chunk.chunk_map) {
            ...
            if (inc_ref_count > 0) {
                ...
                ceph_tid_t tid =
                    refcount_manifest(ctx->obs->oi.soid, target_oid, refcount_t::INCREMENT_REF, fin, std::nullopt);
                fin->tid = tid;
                ...
                ctx->register_on_commit(
                    [src, tgt, this]() { refcount_manifest(src, tgt, refcount_t::DECREMENT_REF, NULL, std::nullopt); });
            }
        }
        if (mop->tids.size()) {
...
void PrimaryLogPG::dec_refcount_by_dirty(OpContext* ctx)
{
    ...
    obc->obs.oi.manifest.calc_refs_to_drop_on_modify(cobc ? &cobc->obs.oi.manifest : nullptr, ctx->clean_regions, refs);
    if (!refs.is_empty()) {
        hobject_t soid = obc->obs.oi.soid;
        ctx->register_on_commit([soid, this, refs]() { dec_refcount(soid, refs); });
    }
}
...
void PrimaryLogPG::dec_all_refcount_manifest(const object_info_t& oi, OpContext* ctx)
{
    ...
    else if (oi.manifest.is_redirect() && oi.test_flag(object_info_t::FLAG_REDIRECT_HAS_REFERENCE)) {
        ctx->register_on_commit([oi, this]() {
            refcount_manifest(oi.soid, oi.manifest.redirect_target, refcount_t::DECREMENT_REF, NULL, std::nullopt);
        });
    }
}

ceph_tid_t PrimaryLogPG::refcount_manifest(hobject_t src_soid, hobject_t tgt_soid, refcount_t type, Context* cb,
                                           std::optional<bufferlist> chunk)
{
    ...
    ceph_assert(src_obc);
    auto tid = osd->objecter->mutate(tgt_soid.oid,
                                     oloc,
                                     obj_op,
                                     SnapContext(),
...
void PrimaryLogPG::do_proxy_chunked_read(OpRequestRef op, ObjectContextRef obc, int op_index, uint64_t chunk_index,
                                         uint64_t req_offset, uint64_t req_length, uint64_t req_total_len,
                                         bool write_ordered)
{
    MOSDOp* m = static_cast<MOSDOp*>(op->get_nonconst_req());
    object_manifest_t* manifest = &obc->obs.oi.manifest;
    ...
    dout(10) << __func__ << " Start do chunk proxy read for " << *m << " index: " << op_index
             << " oid: " << soid.oid.name << " req_offset: " << req_offset << " req_length: " << req_length << dendl;
    ...
    C_ProxyChunkRead* fin = new C_ProxyChunkRead(this, ori_soid, get_last_peering_reset(), prdop);
    fin->obj_op = pobj_op;
...
void PrimaryLogPG::finish_proxy_write(hobject_t oid, ceph_tid_t tid, int r)
{
    ...
    list<OpRequestRef>::iterator it = std::find(in_progress_op.begin(), in_progress_op.end(), pwop->op);
    ceph_assert(it != in_progress_op.end());
    in_progress_op.erase(it);
    if (in_progress_op.size() == 0) {
        in_progress_proxy_ops.erase(oid);
    }
    else if (std::find(in_progress_op.begin(), in_progress_op.end(), pwop->op) != in_progress_op.end()) {
        if (pwop->ctx) delete pwop->ctx;
        pwop->ctx = NULL;
        dout(20) << __func__ << " " << oid << " tid " << tid << " in_progress_op size: " << in_progress_op.size()
                 << dendl;
        return;
    }
    ...
    if (!pwop->sent_reply) {
        ...
        MOSDOpReply* reply = new MOSDOpReply(m, r, get_osdmap_epoch(), 0, true /* we claim it below */);
...
void PrimaryLogPG::cancel_proxy_write(ProxyWriteOpRef pwop, vector<ceph_tid_t>* tids)
{
                                  ...
                                  OpRequestRef op, ObjectContextRef* promote_obc)
{
    ...
    if (op) wait_for_blocked_object(obc->obs.oi.soid, op);
...
void PrimaryLogPG::execute_ctx(OpContext* ctx)
{
    ...
    if (op->may_write() || op->may_cache()) {
        ...
        dout(10) << __func__ << " " << soid << " " << *ctx->ops << " ov " << obc->obs.oi.version << " av "
                 << ctx->at_version << " snapc " << ctx->snapc << " snapset " << obc->ssc->snapset << dendl;
    }
    else {
        dout(10) << __func__ << " " << soid << " " << *ctx->ops << " ov " << obc->obs.oi.version << dendl;
    }

    if (!ctx->user_at_version) ctx->user_at_version = obc->obs.oi.user_version;
    ...
    if ((ctx->op_t->empty() || result < 0) && !ctx->update_log_only) {
        // finish side-effects
        if (result >= 0) do_osd_op_effects(ctx, m->get_connection());
    ...
    ctx->register_on_commit([m, ctx, this]() {
        if (ctx->op) log_op_stats(*ctx->op, ctx->bytes_written, ctx->bytes_read);
    ...
    ctx->register_on_success(
        [ctx, this]() { do_osd_op_effects(ctx, ctx->op ? ctx->op->get_req()->get_connection() : ConnectionRef()); });
...
{
    if (ctx->op) osd->reply_op_error(ctx->op, r);
    close_op_ctx(ctx);
}

void PrimaryLogPG::log_op_stats(const OpRequest& op, const uint64_t inb, const uint64_t outb)
{
    auto m = op.get_req<MOSDOp>();
...
void PrimaryLogPG::set_dynamic_perf_stats_queries(const std::list<OSDPerfMetricQuery>& queries)
{
    ...
    case MOSDPGScan::OP_SCAN_GET_DIGEST:
    {
        ...
        if (osd->check_backfill_full(dpp)) {
            ...
            queue_peering_event(PGPeeringEventRef(std::make_shared<PGPeeringEvent>(
                get_osdmap_epoch(), get_osdmap_epoch(), PeeringState::BackfillTooFull())));
            return;
        }
        ...
        // past m->begin
        scan_range(cct->_conf->osd_backfill_scan_min, cct->_conf->osd_backfill_scan_max, &bi, handle);
    ...
    case MOSDPGScan::OP_SCAN_DIGEST:
    {
        ...
        if (waiting_on_backfill.erase(from)) {
            if (waiting_on_backfill.empty()) {
                ceph_assert(peer_backfill_info.size() == get_backfill_targets().size());
...
void PrimaryLogPG::do_backfill(OpRequestRef op)
{
    ...
    case MOSDPGBackfill::OP_BACKFILL_FINISH:
    {
        ...
        queue_peering_event(PGPeeringEventRef(
            std::make_shared<PGPeeringEvent>(get_osdmap_epoch(), get_osdmap_epoch(), RecoveryDone())));
    }
        // fall-thru
...
{
    const MOSDPGBackfillRemove* m = static_cast<const MOSDPGBackfillRemove*>(op->get_req());
        ...
        if (is_remote_backfilling()) {
            ...
            if (r == 0) {
                ...
                if (pool.info.is_erasure()) {
                    bufferlist bv;
                    int r =
                        osd->store->getattr(ch, ghobject_t(p.first, ghobject_t::NO_GEN, pg_whoami.shard), OI_ATTR, bv);
                    if (r >= 0) {
                        object_info_t oi(bv);
                ...
                dout(10) << __func__ << " " << ghobject_t(p.first, ghobject_t::NO_GEN, pg_whoami.shard)
                         << " sub actual data by " << st.st_size << " sub num_bytes by " << usersize << dendl;
            }
        }
...
int PrimaryLogPG::trim_object(bool first, const hobject_t& coid, snapid_t snap_to_trim,
                              PrimaryLogPG::OpContextUPtr* ctxp)
{
    *ctxp = NULL;
    ...
    if (!obc || !obc->ssc || !obc->ssc->exists) {
        osd->clog->error() << __func__ << ": Can not trim " << coid << " repair needed "
                           << (obc ? "(no obc->ssc or !exists)" : "(no obc)");
        return -ENOENT;
    }
    ...
    if (!head_obc) {
        osd->clog->error() << __func__ << ": Can not trim " << coid << " repair needed, no snapset obc for "
                           << head_oid;
        return -ENOENT;
    }
    ...
    const OSDMapRef& osdmap = get_osdmap();
    for (set<snapid_t>::iterator i = old_snaps.begin(); i != old_snaps.end(); ++i) {
        if (!osdmap->in_removed_snaps_queue(info.pgid.pgid.pool(), *i) && *i != snap_to_trim) {
            new_snaps.insert(*i);
        }
    }
    ...
    if (!ctx->lock_manager.get_snaptrimmer_write(head_oid, head_obc, first)) {
        close_op_ctx(ctx.release());
    ...
    if (new_snaps.empty()) {
        ...
        if (p != snapset.clones.begin()) {
            ...
            if (adjust_prev_bytes) ctx->delta_stats.num_bytes -= snapset.get_clone_bytes(*n);
        ...
        if (coi.is_dirty()) ctx->delta_stats.num_objects_dirty--;
        if (coi.is_omap()) ctx->delta_stats.num_objects_omap--;
        if (coi.is_whiteout()) {
            dout(20) << __func__ << " trimming whiteout on " << coid << dendl;
            ctx->delta_stats.num_whiteouts--;
        }
        ctx->delta_stats.num_object_clones--;
        if (coi.is_cache_pinned()) ctx->delta_stats.num_objects_pinned--;
        if (coi.has_manifest()) {
        ...
        ctx->log.push_back(pg_log_entry_t(
            pg_log_entry_t::DELETE, coid, ctx->at_version, ctx->obs->oi.version, 0, osd_reqid_t(), ctx->mtime, 0));
        t->remove(coid);
        t->update_snaps(coid, old_snaps, new_snaps);
        ...
        snapset.clone_snaps[coid.snap] = vector<snapid_t>(new_snaps.rbegin(), new_snaps.rend());
        ...
        ctx->log.push_back(pg_log_entry_t(
            pg_log_entry_t::MODIFY, coid, coi.version, coi.prior_version, 0, osd_reqid_t(), ctx->mtime, 0));
        ctx->at_version.version++;
    ...
    if (snapset.clones.empty() &&
        (head_obc->obs.oi.is_whiteout() && !(head_obc->obs.oi.is_dirty() && pool.info.is_tier()) &&
        ...
        ctx->log.push_back(pg_log_entry_t(pg_log_entry_t::DELETE,
                                          head_oid,
                                          ctx->at_version,
        ...
        if (oi.is_omap()) ctx->delta_stats.num_objects_omap--;
        if (oi.is_whiteout()) {
        ...
        bl.clear();
        encode(head_obc->obs.oi, bl, get_osdmap()->get_features(CEPH_ENTITY_TYPE_OSD, nullptr));
    ...
    if (num_objects_before_trim > ctx->delta_stats.num_objects) {
        int64_t num_objects_trimmed = num_objects_before_trim - ctx->delta_stats.num_objects;
...
void PrimaryLogPG::kick_snap_trim()
{
    ...
    ceph_assert(is_primary());
    if (is_clean() && !state_test(PG_STATE_PREMERGE) && !snap_trimq.empty()) {
...
template<typename U, typename V> int do_cmp_xattr(int op, const U& lhs, const V& rhs)
{
    switch (op) {
    case CEPH_OSD_CMPXATTR_OP_EQ: return lhs == rhs;
    case CEPH_OSD_CMPXATTR_OP_NE: return lhs != rhs;
    case CEPH_OSD_CMPXATTR_OP_GT: return lhs > rhs;
    case CEPH_OSD_CMPXATTR_OP_GTE: return lhs >= rhs;
    case CEPH_OSD_CMPXATTR_OP_LT: return lhs < rhs;
    case CEPH_OSD_CMPXATTR_OP_LTE: return lhs <= rhs;
...
int PrimaryLogPG::do_xattr_cmp_u64(int op, uint64_t v1, bufferlist& xattr)
{
    ...
    if (xattr.length()) {
        ...
        if (auto [p, ec] = std::from_chars(first, first + xattr.length(), v2); ec != std::errc()) {
            return -EINVAL;
        }
    }
    else {
...
int PrimaryLogPG::do_writesame(OpContext* ctx, OSDOp& osd_op)
{
    ...
    if (!op.writesame.data_length || write_length % op.writesame.data_length) return -EINVAL;

    if (op.writesame.data_length != osd_op.indata.length()) {
        derr << "invalid length ws data length " << op.writesame.data_length << " actual len " << osd_op.indata.length()
             << dendl;
        return -EINVAL;
    }
    ...
    if (result < 0) derr << "do_writesame do_osd_ops failed " << result << dendl;
...
int PrimaryLogPG::do_tmap2omap(OpContext* ctx, unsigned flags)
{
    ...
    if (r < 0) {
        if (r == -ENODATA && (flags & CEPH_OSD_TMAP2OMAP_NULLOK)) r = 0;
        return r;
    }
...
int PrimaryLogPG::do_tmapup_slow(OpContext* ctx, bufferlist::const_iterator& bp, OSDOp& osd_op, bufferlist& bl)
{
    // decode
    bufferlist header;
...
static int check_offset_and_length(uint64_t offset, uint64_t length, uint64_t max, DoutPrefixProvider* dpp)
{
    if (offset >= max || length > max || offset + length > max) {
        ldpp_dout(dpp, 10) << __func__ << " "
                           << "osd_max_object_size: " << max << "; Hard limit of object size is 4GB." << dendl;
        return -EFBIG;
    }
...
struct FillInVerifyExtent : public Context
{
    ...
    hobject_t soid;
    uint32_t flags;
    FillInVerifyExtent(ceph_le64* r, int32_t* rv, bufferlist* blp, std::optional<uint32_t> mc, uint64_t size,
    ...
    void finish(int len) override
    {
        ...
        *rval = 0;
        ...
        if (maybe_crc && *r == size) {
            ...
            if (maybe_crc != crc) {
                osd->clog->error() << std::hex << " full-object read crc 0x" << crc << " != expected 0x" << *maybe_crc
                                   << std::dec << " on " << soid;
...
struct ToSparseReadResult : public Context
{
    ...
    ceph_le64* len;
    ToSparseReadResult(int* result, bufferlist* bl, uint64_t offset, ceph_le64* len)
        : result(result)
        , data_bl(bl)
        , data_offset(offset)
        , len(len)
...
template<typename V> static string list_keys(const map<string, V>& m)
{
    string s;
...
template<typename T> static string list_entries(const T& m)
{
    string s;
...
void PrimaryLogPG::maybe_create_new_object(OpContext* ctx, bool ignore_transaction)
{
    ...
    if (!obs.exists) {
        ...
        obs.oi.new_object();
        if (!ignore_transaction) ctx->op_t->create(obs.oi.soid);
    }
...
struct ReadFinisher : public PrimaryLogPG::OpFinisher
{
    ...
    explicit ReadFinisher(OSDOp& osd_op)
        : osd_op(osd_op)
    {}
...
struct C_ChecksumRead : public Context
{
    ...
    C_ChecksumRead(PrimaryLogPG* primary_log_pg, OSDOp& osd_op, Checksummer::CSumType csum_type,
                   bufferlist&& init_value_bl, std::optional<uint32_t> maybe_crc, uint64_t size, OSDService* osd,
                   hobject_t soid, uint32_t flags)
        : primary_log_pg(primary_log_pg)
        , osd_op(osd_op)
        , csum_type(csum_type)
        , init_value_bl(std::move(init_value_bl))
        , fill_extent_ctx(
              new FillInVerifyExtent(&read_length, &osd_op.rval, &read_bl, maybe_crc, size, osd, soid, flags))
    {}
    ~C_ChecksumRead() override { delete fill_extent_ctx; }
    ...
    void finish(int r) override
    {
        ...
        if (osd_op.rval >= 0) {
            ...
            osd_op.rval = primary_log_pg->finish_checksum(osd_op, csum_type, &init_value_bl_it, read_bl);
        }
    }
};

int PrimaryLogPG::do_checksum(OpContext* ctx, OSDOp& osd_op, bufferlist::const_iterator* bl_it)
{
    ...
    if (op.checksum.offset == 0 && op.checksum.length == 0) {
        ...
        if (op.checksum.chunk_size > 0 && op.checksum.length % op.checksum.chunk_size != 0) {
            dout(10) << __func__ << ": length (trimmed to 0x" << std::hex << op.checksum.length
                     << ") not aligned to chunk size 0x" << op.checksum.chunk_size << std::dec << dendl;
            return -EINVAL;
        }
    }
    ...
    case CEPH_OSD_CHECKSUM_OP_TYPE_XXHASH32: csum_type = Checksummer::CSUM_XXHASH32; break;
    case CEPH_OSD_CHECKSUM_OP_TYPE_XXHASH64: csum_type = Checksummer::CSUM_XXHASH64; break;
    case CEPH_OSD_CHECKSUM_OP_TYPE_CRC32C: csum_type = Checksummer::CSUM_CRC32C; break;
    default:
    ...
    bufferlist init_value_bl;
    init_value_bl.substr_of(bl_it->get_bl(), bl_it->get_off(), csum_init_value_size);
    *bl_it += csum_init_value_size;
    ...
    if (pool.info.is_erasure() && op.checksum.length > 0) {
        ...
        std::optional<uint32_t> maybe_crc;
        if (oi.is_data_digest() && op.checksum.offset == 0 && op.checksum.length >= oi.size) {
            maybe_crc = oi.data_digest;
        }

        // async read
        auto& soid = oi.soid;
        auto checksum_ctx = new C_ChecksumRead(
            this, osd_op, csum_type, std::move(init_value_bl), maybe_crc, oi.size, osd, soid, op.flags);

        ctx->pending_async_reads.push_back(
            {{op.checksum.offset, op.checksum.length, op.flags}, {&checksum_ctx->read_bl, checksum_ctx}});

        dout(10) << __func__ << ": async_read noted for " << soid << dendl;
        ctx->op_finishers[ctx->current_osd_subop_num].reset(new ReadFinisher(osd_op));
        return -EINPROGRESS;
    }
    ...
    bufferlist::const_iterator init_value_bl_it = init_value_bl.begin();
    return finish_checksum(osd_op, csum_type, &init_value_bl_it, read_op.outdata);
}

int PrimaryLogPG::finish_checksum(OSDOp& osd_op, Checksummer::CSumType csum_type,
                                  bufferlist::const_iterator* init_value_bl_it, const bufferlist& read_bl)
{
    dout(20) << __func__ << dendl;
    ...
    if (csum_count > 0) {
        ...
        case Checksummer::CSUM_XXHASH32:
        {
            ...
            Checksummer::calculate<Checksummer::xxhash32>(
                init_value, csum_chunk_size, 0, read_bl.length(), read_bl, &csum_data);
        } break;
...
struct C_ExtentCmpRead : public Context
{
    ...
    C_ExtentCmpRead(PrimaryLogPG* primary_log_pg, OSDOp& osd_op, std::optional<uint32_t> maybe_crc, uint64_t size,
                    OSDService* osd, hobject_t soid, uint32_t flags)
        : primary_log_pg(primary_log_pg)
        , osd_op(osd_op)
        , fill_extent_ctx(
              new FillInVerifyExtent(&read_length, &osd_op.rval, &read_bl, maybe_crc, size, osd, soid, flags))
    {}
    ~C_ExtentCmpRead() override { delete fill_extent_ctx; }
...
int PrimaryLogPG::do_extent_cmp(OpContext* ctx, OSDOp& osd_op)
{
    ...
    if ((oi.truncate_seq < op.extent.truncate_seq) && (op.extent.offset + op.extent.length > op.extent.truncate_size)) {
    ...
    if (op.extent.length == 0) {
        ...
        // async read
        auto& soid = oi.soid;
        auto extent_cmp_ctx = new C_ExtentCmpRead(this, osd_op, maybe_crc, oi.size, osd, soid, op.flags);
        ctx->pending_async_reads.push_back(
            {{op.extent.offset, op.extent.length, op.flags}, {&extent_cmp_ctx->read_bl, extent_cmp_ctx}});
...
int PrimaryLogPG::do_read(OpContext* ctx, OSDOp& osd_op)
{
    ...
    if ((seq < op.extent.truncate_seq) && (op.extent.offset + op.extent.length > op.extent.truncate_size) &&
        (size > op.extent.truncate_size))
        size = op.extent.truncate_size;
    ...
    if (trimmed_read && op.extent.length == 0) {
        ...
        if (oi.is_data_digest() && op.extent.offset == 0 && op.extent.length >= oi.size) maybe_crc = oi.data_digest;
        ctx->pending_async_reads.push_back(make_pair(
            boost::make_tuple(op.extent.offset, op.extent.length, op.flags),
            make_pair(&osd_op.outdata,
                      new FillInVerifyExtent(
                          &op.extent.length, &osd_op.rval, &osd_op.outdata, maybe_crc, oi.size, osd, soid, op.flags))));
        ...
        // whole object?  can we verify the checksum?
        if (r >= 0 && op.extent.offset == 0 && (uint64_t)r == oi.size && oi.is_data_digest()) {
            uint32_t crc = osd_op.outdata.crc32c(-1);
            if (oi.data_digest != crc) {
                osd->clog->error() << info.pgid << std::hex << " full-object read crc 0x" << crc << " != expected 0x"
                                   << oi.data_digest << std::dec << " on " << soid;
...
int PrimaryLogPG::do_sparse_read(OpContext* ctx, OSDOp& osd_op)
{
    ...
    if (pool.info.is_erasure()) {
        ...
        if (length > 0) {
            ctx->pending_async_reads.push_back(
                make_pair(boost::make_tuple(offset, length, op.flags),
                          make_pair(&osd_op.outdata,
                                    new ToSparseReadResult(&osd_op.rval, &osd_op.outdata, offset, &op.extent.length))));
        ...
        int r = osd->store->fiemap(ch, ghobject_t(soid, ghobject_t::NO_GEN, info.pgid.shard), offset, length, m);
        if (r < 0) {
            return r;
        }
...
int PrimaryLogPG::do_osd_ops(OpContext* ctx, vector<OSDOp>& ops)
{
    ...
    const bool skip_data_digest = osd->store->has_builtin_csum() && osd->osd_skip_data_digest;
    ...
    for (auto p = ops.begin(); p != ops.end(); ++p, ctx->current_osd_subop_num++, ctx->processed_subop_count++) {
        ...
        case CEPH_OSD_OP_TIER_EVICT: break;
        default:
            if (op.op & CEPH_OSD_OP_MODE_WR) ctx->user_modify = true;
        }

        // munge -1 truncate to 0 truncate
        if (ceph_osd_op_uses_extent(op.op) && op.extent.truncate_seq == 1 && op.extent.truncate_size == (-1ULL)) {
        ...
        if (op.op == CEPH_OSD_OP_ZERO && obs.exists &&
            op.extent.offset < static_cast<Option::size_t>(osd->osd_max_object_size) && op.extent.length >= 1 &&
        ...
        case CEPH_OSD_OP_MAPEXT:
            tracepoint(
                osd, do_osd_op_pre_mapext, soid.oid.name.c_str(), soid.snap.val, op.extent.offset, op.extent.length);
            if (pool.info.is_erasure()) {
                result = -EOPNOTSUPP;
                break;
            }
            ++ctx->num_read;
            {
                // read into a buffer
                bufferlist bl;
                int r = osd->store->fiemap(
                    ch, ghobject_t(soid, ghobject_t::NO_GEN, info.pgid.shard), op.extent.offset, op.extent.length, bl);
        ...
        case CEPH_OSD_OP_CALL:
        {
            ...
            if (flags & CLS_METHOD_WR) ctx->user_modify = true;
        ...
        case CEPH_OSD_OP_GETXATTRS:
            ++ctx->num_read;
            {
                ...
                result = getattrs_maybe_cache(ctx->obc, &out);
        ...
        case CEPH_OSD_OP_CMPXATTR:
            ++ctx->num_read;
            {
                ...
                if (result < 0 && result != -EEXIST && result != -ENODATA) break;
                ...
                case CEPH_OSD_CMPXATTR_MODE_STRING:
                {
                    ...
                    dout(10) << "CEPH_OSD_OP_CMPXATTR name=" << name << " val=" << val << " op=" << (int)op.xattr.cmp_op
                ...
                default: dout(10) << "bad cmp mode " << (int)op.xattr.cmp_mode << dendl; result = -EINVAL;
                }
        ...
        case CEPH_OSD_OP_LIST_WATCHERS:
            ++ctx->num_read;
            {
                ...
                for (oi_iter = oi.watchers.begin(); oi_iter != oi.watchers.end(); ++oi_iter) {
                    dout(20) << "key cookie=" << oi_iter->first.first << " entity=" << oi_iter->first.second << " "
        ...
        case CEPH_OSD_OP_LIST_SNAPS:
            ++ctx->num_read;
            {
                ...
                for (auto clone_iter = ssc->snapset.clones.begin(); clone_iter != ssc->snapset.clones.end();
                     ++clone_iter) {
                    clone_info ci;
                    ...
                    if (p == ssc->snapset.clone_snaps.end()) {
                        osd->clog->error() << "osd." << osd->whoami << ": inconsistent clone_snaps found for oid "
                                           << soid << " clone " << *clone_iter << " snapset " << ssc->snapset;
                        result = -EINVAL;
                        break;
                    }
        ...
        case CEPH_OSD_OP_NOTIFY:
            ++ctx->num_read;
            {
                ...
                if (!timeout) timeout = cct->_conf->osd_default_notify_timeout;
        ...
        case CEPH_OSD_OP_NOTIFY_ACK:
            ++ctx->num_read;
            {
                try {
                    ...
                    tracepoint(
                        osd, do_osd_op_pre_notify_ack, soid.oid.name.c_str(), soid.snap.val, op.watch.cookie, 0, "N");
        ...
        case CEPH_OSD_OP_SETALLOCHINT:
            ...
            result = 0;
            {
                ...
                t->set_alloc_hint(
                    soid, op.alloc_hint.expected_object_size, op.alloc_hint.expected_write_size, op.alloc_hint.flags);
            }
            break;
        ...
        case CEPH_OSD_OP_WRITE:
            ...
            {   // write
                ...
                else if (op.extent.offset != oi.size && pool.info.requires_aligned_append()) {
                    result = -EOPNOTSUPP;
                    break;
                }
                ...
                if (op.extent.truncate_seq > seq) {
                    ...
                    if (obs.exists && !oi.is_whiteout()) {
                        ...
                        if (oi.size > op.extent.truncate_size) {
                            ...
                            trim.insert(op.extent.truncate_size, oi.size - op.extent.truncate_size);
                            ctx->modified_ranges.union_of(trim);
                            ctx->clean_regions.mark_data_region_dirty(op.extent.truncate_size,
                                                                      oi.size - op.extent.truncate_size);
                            oi.clear_data_digest();
                        }
                        if (op.extent.truncate_size != oi.size) {
                            truncate_update_size_and_usage(ctx->delta_stats, oi, op.extent.truncate_size);
                        }
                    }
                    else {
                        dout(10) << " truncate_seq " << op.extent.truncate_seq << " > current " << seq
                ...
                result = check_offset_and_length(op.extent.offset,
                                                 op.extent.length,
                                                 static_cast<Option::size_t>(osd->osd_max_object_size),
                                                 get_dpp());
                if (result < 0) break;
                ...
                if (op.extent.length == 0) {
                    if (op.extent.offset > oi.size) {
                        ...
                        truncate_update_size_and_usage(ctx->delta_stats, oi, op.extent.offset);
                    }
                    else {
                        t->nop(soid);
                    }
                ...
                write_update_size_and_usage(
                    ctx->delta_stats, oi, ctx->modified_ranges, op.extent.offset, op.extent.length);
        ...
        case CEPH_OSD_OP_WRITEFULL:
            ...
            {   // write full object
                tracepoint(
                    osd, do_osd_op_pre_writefull, soid.oid.name.c_str(), soid.snap.val, oi.size, 0, op.extent.length);
                ...
                result = check_offset_and_length(
                    0, op.extent.length, static_cast<Option::size_t>(osd->osd_max_object_size), get_dpp());
                if (result < 0) break;
                ...
                ctx->clean_regions.mark_data_region_dirty(0, std::max((uint64_t)op.extent.length, oi.size));
                write_update_size_and_usage(ctx->delta_stats, oi, ctx->modified_ranges, 0, op.extent.length, true);
            }
            break;
        ...
        case CEPH_OSD_OP_ZERO:
            tracepoint(
                osd, do_osd_op_pre_zero, soid.oid.name.c_str(), soid.snap.val, op.extent.offset, op.extent.length);
        ...
        case CEPH_OSD_OP_CREATE:
            ...
            result = 0;
            {
                ...
                if (obs.exists && !oi.is_whiteout() && (op.flags & CEPH_OSD_OP_FLAG_EXCL)) {
        ...
        case CEPH_OSD_OP_WATCH:
            ...
            {
                tracepoint(
                    osd, do_osd_op_pre_watch, soid.oid.name.c_str(), soid.snap.val, op.watch.cookie, op.watch.op);
                if (!obs.exists) {
                    result = -ENOENT;
                    break;
                }
                ...
                dout(10) << "watch " << ceph_osd_watch_op_name(op.watch.op) << ": ctx->obc=" << (void*)obc.get()
                         << " cookie=" << cookie << " oi.version=" << oi.version.version
        ...
        case CEPH_OSD_OP_CACHE_PIN:
            ...
            if ((!pool.info.is_tier() || pool.info.cache_mode == pg_pool_t::CACHEMODE_NONE)) {
                result = -EINVAL;
        ...
        case CEPH_OSD_OP_SET_REDIRECT:
            ...
            result = 0;
            {
                ...
                hobject_t target(
                    target_name, target_oloc.key, target_snapid, raw_pg.ps(), raw_pg.pool(), target_oloc.nspace);
                if (target == soid) {
                ...
                if (op_finisher == nullptr && need_reference) {
                    // start
                    ctx->op_finishers[ctx->current_osd_subop_num].reset(new SetManifestFinisher(osd_op));
                    ...
                    ceph_tid_t tid = refcount_manifest(soid, target, refcount_t::INCREMENT_REF, fin, std::nullopt);
                    fin->tid = tid;
                    mop->num_chunks++;
                    mop->tids[0] = tid;
                    ...
                    if (!oi.has_manifest() && !oi.manifest.is_redirect()) ctx->delta_stats.num_objects_manifest++;
                    ...
                    write_update_size_and_usage(ctx->delta_stats, oi, ctx->modified_ranges, 0, oi.size, false);
        ...
        case CEPH_OSD_OP_SET_CHUNK:
            ...
            result = 0;
            {
                ...
                hobject_t target(tgt_name, tgt_oloc.key, snapid_t(), raw_pg.ps(), raw_pg.pool(), tgt_oloc.nspace);
                ...
                if (!oi.has_manifest() && !oi.manifest.is_chunked()) ctx->delta_stats.num_objects_manifest++;
                oi.set_flag(object_info_t::FLAG_MANIFEST);
        ...
        case CEPH_OSD_OP_TIER_PROMOTE:
            ...
            result = 0;
            {
                ...
                if (op_finisher == nullptr) {
                    ...
                    ctx->op_finishers[ctx->current_osd_subop_num].reset(new PromoteFinisher(cb));
                    unsigned flags = CEPH_OSD_COPY_FROM_FLAG_IGNORE_OVERLAY | CEPH_OSD_COPY_FROM_FLAG_IGNORE_CACHE |
                                     CEPH_OSD_COPY_FROM_FLAG_MAP_SNAP_CLONE | CEPH_OSD_COPY_FROM_FLAG_RWORDERED;
                    unsigned src_fadvise_flags = LIBRADOS_OP_FLAG_FADVISE_SEQUENTIAL;
                    start_copy(cb,
        ...
        case CEPH_OSD_OP_SETXATTR:
            ...
            result = 0;
            {
                if (cct->_conf->osd_max_attr_size > 0 && op.xattr.value_len > cct->_conf->osd_max_attr_size) {
                    tracepoint(osd, do_osd_op_pre_setxattr, soid.oid.name.c_str(), soid.snap.val, "???");
                    result = -EFBIG;
                    break;
                }
                unsigned max_name_len =
                    std::min<uint64_t>(osd->store->get_max_attr_name_length(), cct->_conf->osd_max_attr_name_len);
        ...
        case CEPH_OSD_OP_OMAPGETKEYS:
            ++ctx->num_read;
            {
                ...
                if (oi.is_omap()) {
                    ObjectMap::ObjectMapIterator iter = osd->store->get_omap_iterator(ch, ghobject_t(soid));
                    ceph_assert(iter);
        ...
        case CEPH_OSD_OP_OMAPGETVALS:
            ++ctx->num_read;
            {
                ...
                if (oi.is_omap()) {
                    ...
                    for (num = 0; iter->valid() && iter->key().substr(0, filter_prefix.size()) == filter_prefix;
                         ++num, iter->next()) {
        ...
        case CEPH_OSD_OP_OMAPGETVALSBYKEYS:
            ++ctx->num_read;
            {
                ...
                tracepoint(osd,
                           ...
                           soid.oid.name.c_str(),
                           soid.snap.val,
                           list_entries(keys_to_get).c_str());
        ...
        case CEPH_OSD_OP_OMAP_CMP:
            ++ctx->num_read;
            {
                ...
                tracepoint(
                    osd, do_osd_op_pre_omap_cmp, soid.oid.name.c_str(), soid.snap.val, list_keys(assertions).c_str());

                map<string, bufferlist> out;

                if (oi.is_omap()) {
                    set<string> to_get;
                    for (map<string, pair<bufferlist, int>>::iterator i = assertions.begin(); i != assertions.end();
                         ++i)
                        to_get.insert(i->first);
                    int r = osd->store->omap_get_values(ch, ghobject_t(soid), to_get, &out);
                    if (r < 0) {
                        result = r;
                        break;
                    }
        ...
        case CEPH_OSD_OP_COPY_FROM2:
            ...
            result = 0;
            {
                ...
                if ((op.op == CEPH_OSD_OP_COPY_FROM2) && (op.copy_from.flags & ~CEPH_OSD_COPY_FROM_FLAGS)) {
                    dout(20) << "invalid copy-from2 flags 0x" << std::hex << (int)op.copy_from.flags << std::dec
                             << dendl;
                    result = -EINVAL;
                    break;
                }
                ...
                if (op_finisher == nullptr) {
                    ...
                    hobject_t src(src_name, src_oloc.key, src_snapid, raw_pg.ps(), raw_pg.pool(), src_oloc.nspace);
                    if (src == soid) {
                    ...
                    if (have_truncate) cb->set_truncate(truncate_seq, truncate_size);
                    ctx->op_finishers[ctx->current_osd_subop_num].reset(new CopyFromFinisher(cb));
                    start_copy(cb,
                               ctx->obc,
                               src,
                               src_oloc,
        ...
        default:
            tracepoint(
                osd, do_osd_op_pre_unknown, soid.oid.name.c_str(), soid.snap.val, op.op, ceph_osd_op_name(op.op));
    ...
    fail:
        ...
        tracepoint(osd,
                   ...
                   op.flags,
                   result);
        if (result < 0 && (op.flags & CEPH_OSD_OP_FLAG_FAILOK) && result != -EAGAIN && result != -EINPROGRESS)
            result = 0;
...
int PrimaryLogPG::_verify_no_head_clones(const hobject_t& soid, const SnapSet& ss)
{
    // verify that all clones have been evicted
    dout(20) << __func__ << " verifying clones are absent " << ss << dendl;
    for (vector<snapid_t>::const_iterator p = ss.clones.begin(); p != ss.clones.end(); ++p) {
        hobject_t clone_oid = soid;
        clone_oid.snap = *p;
        if (is_missing_object(clone_oid)) return -EBUSY;
        ObjectContextRef clone_obc = get_object_context(clone_oid, false);
...
inline int PrimaryLogPG::_delete_oid(OpContext* ctx,
                                     ...
                                     bool try_no_whiteout)   // try not to whiteout
{
    ...
    if (!snapset.clones.empty() || (!ctx->snapc.snaps.empty() && ctx->snapc.snaps[0] > snapset.seq)) {
        if (no_whiteout) {
    ...
    dout(20) << __func__ << " " << soid << " whiteout=" << (int)whiteout << " no_whiteout=" << (int)no_whiteout
             << " try_no_whiteout=" << (int)try_no_whiteout << dendl;
    if (!obs.exists || (obs.oi.is_whiteout() && whiteout)) return -ENOENT;
    ...
    for (map<pair<uint64_t, entity_name_t>, watch_info_t>::iterator p = oi.watchers.begin(); p != oi.watchers.end();
         ++p) {
        dout(20) << __func__ << " will disconnect watcher " << p->first << dendl;
        ctx->watch_disconnects.push_back(watch_disconnect_t(p->first.first, p->first.second, true));
    }
    oi.watchers.clear();
    ...
    // delete the head
    ctx->delta_stats.num_objects--;
    if (soid.is_snap()) ctx->delta_stats.num_object_clones--;
    if (oi.is_whiteout()) {
...
int PrimaryLogPG::_rollback_to(OpContext* ctx, OSDOp& op)
{
    ...
    int ret = find_object_context(
        hobject_t(soid.oid, soid.get_key(), snapid, soid.get_hash(), info.pgid.pool(), soid.get_namespace()),
        &rollback_to,
        false,
        false,
        &missing_oid);
    if (ret == -EAGAIN) {
        ...
        ceph_assert(is_degraded_or_backfilling_object(missing_oid) ||
                    is_degraded_on_async_recovery_target(missing_oid));
        dout(20) << "_rollback_to attempted to roll back to a missing or backfilling clone " << missing_oid
                 << " (requested snapid: ) " << snapid << dendl;
    ...
    }
    {
        ...
        if (obs.exists && obs.oi.has_manifest()) {
            ...
            tier_mode_result = maybe_handle_manifest_detail(ctx->op, true, rollback_to);
        }
        else {
            tier_mode_result =
                maybe_handle_cache_detail(ctx->op, true, rollback_to, ret, missing_oid, true, false, &promote_obc);
        }
        switch (tier_mode_result) {
        case cache_result_t::NOOP: break;
        case cache_result_t::BLOCKED_PROMOTE:
            ceph_assert(promote_obc);
            block_write_on_snap_rollback(soid, promote_obc, ctx->op);
            return -EAGAIN;
        case cache_result_t::BLOCKED_FULL: block_write_on_full_cache(soid, ctx->op); return -EAGAIN;
        case cache_result_t::REPLIED_WITH_EAGAIN: ceph_abort_msg("this can't happen, no rollback on replica");
        default: ceph_abort_msg("must promote was set, other values are not valid"); return -EAGAIN;
        }
    }
    ...
    if (ret == -ENOENT || (rollback_to && rollback_to->obs.oi.is_whiteout())) {
        ...
        if (is_degraded_or_backfilling_object(rollback_to_sobject) ||
            ...
            dout(20) << "_rollback_to attempted to roll back to a degraded object " << rollback_to_sobject
                     << " (requested snapid: ) " << snapid << dendl;
            ...
            if (rollback_to->obs.oi.has_manifest() && rollback_to->obs.oi.manifest.is_chunked()) {
                ...
                if (!op_finisher) {
                    ...
                    if (need_inc_ref) {
                        ...
                        ctx->op_finishers[ctx->current_osd_subop_num].reset(new SetManifestFinisher(op));
                        return -EINPROGRESS;
                    }
                }
                else {
...
void PrimaryLogPG::_do_rollback_to(OpContext* ctx, ObjectContextRef rollback_to, OSDOp& op)
{
    SnapSet& snapset = ctx->new_snapset;
    ObjectState& obs = ctx->new_obs;
    object_info_t& oi = obs.oi;
    ...
    dout(10) << "_do_rollback_to deleting " << soid.oid << " and rolling back to old snap" << dendl;
    ...
    map<snapid_t, interval_set<uint64_t>>::iterator iter = snapset.clone_overlap.lower_bound(snapid);
    ceph_assert(iter != snapset.clone_overlap.end());
    interval_set<uint64_t> overlaps = iter->second;
    for (; iter != snapset.clone_overlap.end(); ++iter) overlaps.intersection_of(iter->second);
...
void PrimaryLogPG::_make_clone(OpContext* ctx, PGTransaction* t, ObjectContextRef clone_obc, const hobject_t& head,
                               const hobject_t& coid, object_info_t* poi)
{
    bufferlist bv;
...
void PrimaryLogPG::make_writeable(OpContext* ctx)
{
    ...
    if ((ctx->new_obs.exists && ctx->new_obs.oi.is_omap()) && (!ctx->obc->obs.exists || !ctx->obc->obs.oi.is_omap())) {
    ...
    if ((ctx->obs->exists && !ctx->obs->oi.is_whiteout()) &&                // head exist(ed)
        ...
        !ctx->cache_operation && snapc.snaps[0] > ctx->new_snapset.seq) {   // existing object is old
        // clone
        hobject_t coid = soid;
        coid.snap = snapc.seq;
        ...
        const auto snaps = [&] {
            auto last = find_if_not(
                begin(snapc.snaps), end(snapc.snaps), [&](snapid_t snap_id) { return snap_id > ctx->new_snapset.seq; });
        ...
        if (is_primary()) {
            ...
            if (pool.info.is_erasure()) ctx->clone_obc->attr_cache = ctx->obc->attr_cache;
        ...
        if (snap_oi->is_omap()) ctx->delta_stats.num_objects_omap++;
        if (snap_oi->is_cache_pinned()) ctx->delta_stats.num_objects_pinned++;
        if (snap_oi->has_manifest()) ctx->delta_stats.num_objects_manifest++;
        ctx->delta_stats.num_object_clones++;
        ...
        // log clone
        dout(10) << " cloning v " << ctx->obs->oi.version << " to " << coid << " v " << ctx->at_version
                 << " snaps=" << snaps << " snapset=" << ctx->new_snapset << dendl;
                                               ...
                                               bool write_full)
{
    interval_set<uint64_t> ch;
    if (write_full) {
        if (oi.size) ch.insert(0, oi.size);
    }
    else if (length)
        ch.insert(offset, length);
    modified.union_of(ch);
    if (write_full || (offset + length > oi.size && length)) {
...
void PrimaryLogPG::truncate_update_size_and_usage(object_stat_sum_t& delta_stats, object_info_t& oi,
                                                  uint64_t truncate_size)
{
...
void PrimaryLogPG::complete_disconnect_watches(ObjectContextRef obc, const list<watch_disconnect_t>& to_disconnect)
{
    for (list<watch_disconnect_t>::const_iterator i = to_disconnect.begin(); i != to_disconnect.end(); ++i) {
...
void PrimaryLogPG::do_osd_op_effects(OpContext* ctx, const ConnectionRef& conn)
{
    ...
    for (list<pair<watch_info_t, bool>>::iterator i = ctx->watch_connects.begin(); i != ctx->watch_connects.end();
         ++i) {
        pair<uint64_t, entity_name_t> watcher(i->first.cookie, entity);
        dout(15) << "do_osd_op_effects applying watch connect on session " << session.get() << " watcher " << watcher
                 << dendl;
        WatchRef watch;
        if (ctx->obc->watchers.count(watcher)) {
            ...
            watch = Watch::makeWatchRef(
                this, osd, ctx->obc, i->first.timeout_seconds, i->first.cookie, entity, conn->get_peer_addr());
            ctx->obc->watchers.insert(make_pair(watcher, watch));
        }
        watch->connect(conn, i->second);
    }

    for (list<notify_info_t>::iterator p = ctx->notifies.begin(); p != ctx->notifies.end(); ++p) {
        dout(10) << "do_osd_op_effects, notify " << *p << dendl;
        ConnectionRef conn(ctx->op->get_req()->get_connection());
        NotifyRef notif(Notify::makeNotifyRef(conn,
                                              ctx->reqid.name.num(),
                                              p->bl,
                                              p->timeout,
                                              p->cookie,
    ...
    for (list<OpContext::NotifyAck>::iterator p = ctx->notify_acks.begin(); p != ctx->notify_acks.end(); ++p) {
        if (p->watch_cookie)
        ...
        for (map<pair<uint64_t, entity_name_t>, WatchRef>::iterator i = ctx->obc->watchers.begin();
            ...
            if (p->watch_cookie && *(p->watch_cookie) != i->first.first) continue;
...
hobject_t PrimaryLogPG::get_temp_recovery_object(const hobject_t& target, eversion_t version)
{
    ostringstream ss;
    ss << "temp_recovering_" << info.pgid   // (note this includes the shardid)
       << "_" << version << "_" << info.history.same_interval_since << "_" << target.snap;
...
int PrimaryLogPG::prepare_transaction(OpContext* ctx)
{
    ...
    const hobject_t& soid = ctx->obs->oi.soid;
    // clone, if necessary
    if (soid.snap == CEPH_NOSNAP) make_writeable(ctx);
...
void PrimaryLogPG::finish_ctx(OpContext* ctx, int log_op_type, int result)
{
    ...
    if (ctx->new_obs.oi.is_dirty() && (ctx->obs->oi.has_manifest() && ctx->obs->oi.manifest.is_chunked()) &&
        !ctx->cache_operation && log_op_type != pg_log_entry_t::PROMOTE) {
    ...
    if (ctx->user_modify) {
        ...
        if (ctx->at_version.version > ctx->user_at_version) ctx->user_at_version = ctx->at_version.version;
    ...
    if (ctx->new_obs.exists) {
        ...
        encode(ctx->new_obs.oi, bv, get_osdmap()->get_features(CEPH_ENTITY_TYPE_OSD, nullptr));
...
void PrimaryLogPG::apply_stats(const hobject_t& soid, const object_stat_sum_t& delta_stats)
{

    recovery_state.apply_op_stats(soid, delta_stats);
    for (set<pg_shard_t>::const_iterator i = get_backfill_targets().begin(); i != get_backfill_targets().end(); ++i) {
        pg_shard_t bt = *i;
...
void PrimaryLogPG::complete_read_ctx(int result, OpContext* ctx)
{
    ...
    for (auto p = ctx->ops->begin(); p != ctx->ops->end() && result >= 0; ++p) {
...
struct C_Copyfrom : public Context
{
    ...
    C_Copyfrom(PrimaryLogPG* p, hobject_t o, epoch_t lpr, const PrimaryLogPG::CopyOpRef& c)
        : pg(p)
        , oid(o)
        , last_peering_reset(lpr)
        , tid(0)
...
struct C_CopyChunk : public Context
{
    ...
    PrimaryLogPG::CopyOpRef cop;   // used for keeping the cop alive
    uint64_t offset = 0;
    C_CopyChunk(PrimaryLogPG* p, hobject_t o, epoch_t lpr, const PrimaryLogPG::CopyOpRef& c)
        : pg(p)
        , oid(o)
        , last_peering_reset(lpr)
        , tid(0)
...
int PrimaryLogPG::do_copy_get(OpContext* ctx, bufferlist::const_iterator& bp, OSDOp& osd_op, ObjectContextRef& obc)
{
    ...
    if (!cursor.attr_complete) {
        result = getattrs_maybe_cache(ctx->obc, &out_attrs);
        if (result < 0) {
            if (cb) {
                delete cb;
            }
    ...
    if (left > 0 && !cursor.data_complete) {
        if (cursor.data_offset < oi.size) {
            ...
            if (cb) {
                ...
                ctx->pending_async_reads.push_back(
                    make_pair(boost::make_tuple(cursor.data_offset, max_read, osd_op.op.flags), make_pair(&bl, cb)));
                cb->len = max_read;
                ...
                result = pgbackend->objects_read_sync(oi.soid, cursor.data_offset, max_read, osd_op.op.flags, &bl);
    ...
    if (!pool.info.supports_omap() || !oi.is_omap()) {
        ...
        if (left > 0 && !cursor.omap_complete) {
            ...
            if (cursor.omap_offset.empty()) {
                osd->store->omap_get_header(ch, ghobject_t(oi.soid), &reply_obj.omap_header);
            }
            bufferlist omap_data;
            ObjectMap::ObjectMapIterator iter = osd->store->get_omap_iterator(ch, ghobject_t(oi.soid));
            ceph_assert(iter);
    ...
    if (cursor.is_complete()) {
        ...
        // but it works...
        recovery_state.get_pg_log().get_log().get_object_reqids(
            ctx->obc->obs.oi.soid, 10, &reply_obj.reqids, &reply_obj.reqid_return_codes);
        dout(20) << " got reqids" << dendl;
    }

    dout(20) << " cursor.is_complete=" << cursor.is_complete() << " " << out_attrs.size() << " attrs"
             << " " << bl.length() << " bytes"
             << " " << reply_obj.omap_header.length() << " omap header bytes"
             << " " << reply_obj.omap_data.length() << " omap data bytes in " << omap_keys << " keys"
             << " " << reply_obj.reqids.size() << " reqids" << dendl;
...
void PrimaryLogPG::fill_in_copy_get_noent(OpRequestRef& op, hobject_t oid, OSDOp& osd_op)
{
    const MOSDOp* m = static_cast<const MOSDOp*>(op->get_req());
    uint64_t features = m->get_features();
    object_copy_data_t reply_obj;

    recovery_state.get_pg_log().get_log().get_object_reqids(oid, 10, &reply_obj.reqids, &reply_obj.reqid_return_codes);
...
void PrimaryLogPG::start_copy(CopyCallback* cb, ObjectContextRef obc, hobject_t src, object_locator_t oloc,
                              version_t version, unsigned flags, bool mirror_snapset, unsigned src_obj_fadvise_flags,
                              unsigned dest_obj_fadvise_flags)
{
    const hobject_t& dest = obc->obs.oi.soid;
    dout(10) << __func__ << " " << dest << " from " << src << " " << oloc << " v" << version << " flags " << flags
             << (mirror_snapset ? " mirror_snapset" : "") << dendl;
    ...
    CopyOpRef cop(std::make_shared<CopyOp>(
        cb, obc, src, oloc, version, flags, mirror_snapset, src_obj_fadvise_flags, dest_obj_fadvise_flags));
    copy_ops[dest] = cop;
...
void PrimaryLogPG::_copy_some(ObjectContextRef obc, CopyOpRef cop)
{
    ...
    unsigned flags = 0;
    if (cop->flags & CEPH_OSD_COPY_FROM_FLAG_FLUSH) flags |= CEPH_OSD_FLAG_FLUSH;
    if (cop->flags & CEPH_OSD_COPY_FROM_FLAG_IGNORE_CACHE) flags |= CEPH_OSD_FLAG_IGNORE_CACHE;
    if (cop->flags & CEPH_OSD_COPY_FROM_FLAG_IGNORE_OVERLAY) flags |= CEPH_OSD_FLAG_IGNORE_OVERLAY;
    if (cop->flags & CEPH_OSD_COPY_FROM_FLAG_MAP_SNAP_CLONE) flags |= CEPH_OSD_FLAG_MAP_SNAP_CLONE;
    if (cop->flags & CEPH_OSD_COPY_FROM_FLAG_RWORDERED) flags |= CEPH_OSD_FLAG_RWORDERED;
    ...
    if (cop->cursor.is_initial() && cop->mirror_snapset) {
        ...
        ceph_tid_t tid =
            osd->objecter->read(cop->src.oid, cop->oloc, op, CEPH_SNAPDIR, NULL, flags, gather.new_sub(), NULL);
    ...
    op.copy_get(&cop->cursor,
                get_copy_chunk_size(),
                &cop->results.object_size,
                &cop->results.mtime,
                &cop->attrs,
                &cop->data,
                &cop->omap_header,
    ...
    C_Copyfrom* fin = new C_Copyfrom(this, obc->obs.oi.soid, get_last_peering_reset(), cop);
    gather.set_finisher(new C_OnFinisher(fin, osd->get_objecter_finisher(get_pg_shard())));

    ceph_tid_t tid = osd->objecter->read(cop->src.oid,
                                         cop->oloc,
                                         op,
                                         cop->src.snap,
                                         NULL,
...
void PrimaryLogPG::_copy_some_manifest(ObjectContextRef obc, CopyOpRef cop, uint64_t start_offset)
{
    ...
    dout(20) << __func__ << " oid " << obc->obs.oi.soid << " num_chunks: " << num_chunks
             << " start_offset: " << start_offset << " chunks_size: " << chunks_size << " last_offset: " << last_offset
             << dendl;
    ...
    for (; iter != manifest->chunk_map.end(); ++iter) {
        ...
        CopyOpRef sub_cop(std::make_shared<CopyOp>(cb,
                                                   ObjectContextRef(),
                                                   cop->src,
                                                   oloc,
        ...
        C_CopyChunk* fin = new C_CopyChunk(this, obc->obs.oi.soid, get_last_peering_reset(), cop);
        ...
        dout(20) << __func__ << " tgt_oid: " << soid.oid << " tgt_offset: " << manifest->chunk_map[iter->first].offset
                 << " length: " << length << " pool id: " << oloc.pool << " tid: " << tid << dendl;
...
void PrimaryLogPG::process_copy_chunk(hobject_t oid, ceph_tid_t tid, int r)
{
    ...
    if (cop->omap_data.length() || cop->omap_header.length()) cop->results.has_omap = true;

    if (r >= 0 && !pool.info.supports_omap() && (cop->omap_data.length() || cop->omap_header.length())) {
        r = -EOPNOTSUPP;
    }
    cop->objecter_tid = 0;
    ...
    if (oid.snap < CEPH_NOSNAP && !cop->results.snaps.empty()) {
        ...
        while (p != cop->results.snaps.end()) {
            ...
            if (get_osdmap()->in_removed_snaps_queue(info.pgid.pgid.pool(), *p)) {
                ...
                for (vector<snapid_t>::iterator q = p + 1; q != cop->results.snaps.end(); ++q) *(q - 1) = *q;
    ...
    if (pool.info.supports_omap() && !cop->temp_cursor.omap_complete) {
        if (cop->omap_header.length()) {
            cop->results.omap_digest = cop->omap_header.crc32c(cop->results.omap_digest);
        }
    ...
    if (cop->results.is_data_digest() || cop->results.is_omap_digest()) {
        dout(20) << __func__ << std::hex << " got digest: rx data 0x" << cop->results.data_digest << " omap 0x"
                 << cop->results.omap_digest << ", source: data 0x" << cop->results.source_data_digest << " omap 0x"
                 << cop->results.source_omap_digest << std::dec << " flags " << cop->results.flags << dendl;
    }
    if (cop->results.is_data_digest() && cop->results.data_digest != cop->results.source_data_digest) {
        derr << __func__ << std::hex << " data digest 0x" << cop->results.data_digest << " != source 0x"
             << cop->results.source_data_digest << std::dec << dendl;
        osd->clog->error() << info.pgid << " copy from " << cop->src << " to " << cop->obc->obs.oi.soid << std::hex
                           << " data digest 0x" << cop->results.data_digest << " != source 0x"
                           << cop->results.source_data_digest << std::dec;
        r = -EIO;
        goto out;
    }
    if (cop->results.is_omap_digest() && cop->results.omap_digest != cop->results.source_omap_digest) {
        derr << __func__ << std::hex << " omap digest 0x" << cop->results.omap_digest << " != source 0x"
             << cop->results.source_omap_digest << std::dec << dendl;
        osd->clog->error() << info.pgid << " copy from " << cop->src << " to " << cop->obc->obs.oi.soid << std::hex
                           << " omap digest 0x" << cop->results.omap_digest << " != source 0x"
                           << cop->results.source_omap_digest << std::dec;
        r = -EIO;
        goto out;
    }
    ...
    cop->results.fill_in_final_tx =
        std::function<void(PGTransaction*)>([this, &cop /* avoid ref cycle */](PGTransaction* t) {
...
void PrimaryLogPG::process_copy_chunk_manifest(hobject_t oid, ceph_tid_t tid, int r, uint64_t offset)
{
    ...
    if (tid != chunk_cop->objecter_tid) {
        dout(10) << __func__ << " tid " << tid << " != cop " << chunk_cop << " tid " << chunk_cop->objecter_tid
                 << dendl;
        return;
    }
...

    {
        ...
        if (!ctx->lock_manager.take_write_lock(obj_cop->obc->obs.oi.soid, obj_cop->obc)) {
...
{
    vector<ceph_tid_t> tids;
    for (map<ceph_tid_t, ProxyReadOpRef>::iterator it = proxyread_ops.begin(); it != proxyread_ops.end();) {
    ...
    for (map<ceph_tid_t, ProxyWriteOpRef>::iterator it = proxywrite_ops.begin(); it != proxywrite_ops.end();) {
        if (it->second->soid == oid) {
...
{
    dout(20) << __func__ << " " << cop << " " << cop->attrs.size() << " attrs"
             << " " << cop->data.length() << " bytes"
             << " " << cop->omap_header.length() << " omap header bytes"
             << " " << cop->omap_data.length() << " omap data bytes" << dendl;
    if (!cop->temp_cursor.attr_complete) {
        t->create(cop->results.temp_oid);
    }
    if (!cop->temp_cursor.data_complete) {
        ceph_assert(cop->data.length() + cop->temp_cursor.data_offset == cop->cursor.data_offset);
        if (pool.info.required_alignment() && !cop->cursor.data_complete) {
            /**
             * Trim off the unaligned bit at the end, we'll adjust cursor.data_offset
             * to pick it up on the next pass.
             */
            ceph_assert(cop->temp_cursor.data_offset % pool.info.required_alignment() == 0);
            if (cop->data.length() % pool.info.required_alignment() != 0) {
                uint64_t to_trim = cop->data.length() % pool.info.required_alignment();
                bufferlist bl;
    ...
    if (pool.info.supports_omap()) {
        if (!cop->temp_cursor.omap_complete) {
            if (cop->omap_header.length()) {
                t->omap_setheader(cop->results.temp_oid, cop->omap_header);
...
void PrimaryLogPG::finish_copyfrom(CopyFromCallback* cb)
{
    ...
    interval_set<uint64_t> ch;
    if (obs.oi.size > 0) ch.insert(0, obs.oi.size);
    ctx->modified_ranges.union_of(ch);
...
void PrimaryLogPG::finish_promote(int r, CopyResults* results, ObjectContextRef obc)
{
    const hobject_t& soid = obc->obs.oi.soid;
    ...
    if (r != -ENOENT && soid.is_snap()) {
        if (results->snaps.empty()) {
            ...
            SnapSet& snapset = obc->ssc->snapset;
            for (auto p = snapset.clone_snaps.rbegin(); p != snapset.clone_snaps.rend(); ++p) {
        ...
        if (results->snaps.empty()) {
            dout(20) << __func__ << " snaps are empty, clone is invalid,"
    ...
    if (r == -ENOENT && soid.is_snap()) {
        dout(10) << __func__ << ": enoent while trying to promote clone, " << soid
                 << " must have been trimmed, removing from snapset" << dendl;
        ...
        for (vector<snapid_t>::iterator i = tctx->new_snapset.clones.begin(); i != tctx->new_snapset.clones.end();
             ++i) {
            if (*i != soid.snap) {
        ...
        if (!tctx->lock_manager.take_write_lock(head, obc)) {
    ...
    if (r < 0 && !whiteout) {
        ...
        map<hobject_t, list<OpRequestRef>>::iterator blocked_iter = waiting_for_blocked_object.find(soid);
    ...
    if (soid.snap < CEPH_NOSNAP) ++tctx->delta_stats.num_object_clones;
    ...
    if (whiteout) {
        ...
        if (results->object_size) tctx->clean_regions.mark_data_region_dirty(0, results->object_size);
        ...
        if (results->has_omap) tctx->clean_regions.mark_omap_dirty();
        ...
        if (soid.snap != CEPH_NOSNAP) {
            ...
            ceph_assert(obc->ssc->snapset.clone_size[soid.snap] == results->object_size);
    ...
    if (results->mirror_snapset) {
        ...
        tctx->new_snapset.from_snap_set(results->snapset, get_osdmap()->require_osd_release < ceph_release_t::luminous);
    }
    dout(20) << __func__ << " new_snapset " << tctx->new_snapset << dendl;

    // take RWWRITE lock for duration of our local write.  ignore starvation.
    if (!tctx->lock_manager.take_write_lock(obc->obs.oi.soid, obc)) {
        ceph_abort_msg("problem!");
    }
    dout(20) << __func__ << " took lock on obc, " << obc->rwstate << dendl;
    ...
    if (agent_state && agent_state->is_idle()) agent_choose_mode();
}

void PrimaryLogPG::finish_promote_manifest(int r, CopyResults* results, ObjectContextRef obc)
{
    const hobject_t& soid = obc->obs.oi.soid;
    dout(10) << __func__ << " " << soid << " r=" << r << " uv" << results->user_version << dendl;
...
void PrimaryLogPG::cancel_copy(CopyOpRef cop, bool requeue, vector<ceph_tid_t>* tids)
{
    dout(10) << __func__ << " " << cop->obc->obs.oi.soid << " from " << cop->src << " " << cop->oloc << " v"
             << cop->results.user_version << dendl;
...
int PrimaryLogPG::start_cls_gather(OpContext* ctx, std::map<std::string, bufferlist>* src_obj_buffs,
                                   const std::string& pool, const char* cls, const char* method, bufferlist& inbl)
{
    OpRequestRef op = ctx->op;
    MOSDOp* m = static_cast<MOSDOp*>(op->get_nonconst_req());
    ...
    for (std::map<std::string, bufferlist>::iterator it = src_obj_buffs->begin(); it != src_obj_buffs->end(); it++) {
        ...
        uint32_t flags = 0;
        ceph_tid_t tid =
            osd->objecter->read(object_t(oid), oloc, obj_op, m->get_snapid(), &it->second, flags, gather.new_sub());
...
int PrimaryLogPG::start_dedup(OpRequestRef op, ObjectContextRef obc)
{
    ...
    for (auto p : chunks) {
        ...
        ceph_tid_t tid =
            refcount_manifest(soid, target, refcount_t::CREATE_OR_GET_REF, fin, std::move(chunks[p.first]));
        mop->chunks[target] = make_pair(p.first, p.second.length());
        mop->num_chunks++;
        mop->tids[p.first] = tid;
        fin->tid = tid;
        dout(10) << __func__ << " oid: " << soid << " tid: " << tid << " target: " << target << " offset: " << p.first
...
int PrimaryLogPG::do_cdc(const object_info_t& oi, std::map<uint64_t, chunk_info_t>& chunk_map,
                         std::map<uint64_t, bufferlist>& chunks)
{
     ...
     */
    int r = pgbackend->objects_read_sync(oi.soid, 0, oi.size, 0, &bl);
    if (r < 0) {
        dout(0) << __func__ << " read fail " << oi.soid << " len: " << oi.size << " r: " << r << dendl;
        return r;
    }
    if (bl.length() != oi.size) {
        dout(0) << __func__ << " bl.length: " << bl.length() << " != oi.size: " << oi.size << " during chunking "
                << dendl;
        return -EIO;
    }

    dout(10) << __func__ << " oid: " << oi.soid << " len: " << bl.length() << " oi.size: " << oi.size
...
std::pair<int, hobject_t> PrimaryLogPG::get_fpoid_from_chunk(const hobject_t soid, bufferlist& chunk)
{
    ...
    object_t fp_oid = [&fp_algo, &chunk]() -> string {
        switch (fp_algo) {
        case pg_pool_t::TYPE_FINGERPRINT_SHA1: return ceph::crypto::digest<ceph::crypto::SHA1>(chunk).to_str();
        case pg_pool_t::TYPE_FINGERPRINT_SHA256: return ceph::crypto::digest<ceph::crypto::SHA256>(chunk).to_str();
        case pg_pool_t::TYPE_FINGERPRINT_SHA512: return ceph::crypto::digest<ceph::crypto::SHA512>(chunk).to_str();
        default: assert(0 == "unrecognized fingerprint type"); return {};
        }
    }();
    ...
    hobject_t target(fp_oid, oloc.key, snapid_t(), raw_pg.ps(), raw_pg.pool(), oloc.nspace);
...
int PrimaryLogPG::finish_set_dedup(hobject_t oid, int r, ceph_tid_t tid, uint64_t offset)
{
    ...
    if (mop->chunks.size()) {
        OpContextUPtr ctx = simple_opc_create(obc);
        ceph_assert(ctx);
        if (ctx->lock_manager.get_lock_type(RWState::RWWRITE, oid, obc, mop->op)) {
        ...
        ctx->obs->oi.manifest.calc_refs_to_drop_on_removal(
            obc_l ? &(obc_l->obs.oi.manifest) : nullptr, obc_g ? &(obc_g->obs.oi.manifest) : nullptr, refs);
        if (!refs.is_empty()) {
            ctx->register_on_commit([oid, this, refs]() { dec_refcount(oid, refs); });
        }
    ...
    if (mop->op) osd->reply_op_error(mop->op, r);
...
int PrimaryLogPG::start_flush(OpRequestRef op, ObjectContextRef obc, bool blocking, hobject_t* pmissing,
                              std::optional<std::function<void()>>&& on_flush, bool force_dedup)
{
    const object_info_t& oi = obc->obs.oi;
    const hobject_t& soid = oi.soid;
    dout(10) << __func__ << " " << soid << " v" << oi.version << " uv" << oi.user_version << " "
             << (blocking ? "blocking" : "non-blocking/best-effort") << dendl;

    bool preoctopus_compat = get_osdmap()->require_osd_release < ceph_release_t::octopus;
    SnapSet snapset;
    ...
    if ((obc->obs.oi.has_manifest() && obc->obs.oi.manifest.is_chunked()) || force_dedup) {
    ...
    // verify there are no (older) check for dirty clones
    {
        ...
        while (p != snapset.clones.rend() && *p >= soid.snap) ++p;
        if (p != snapset.clones.rend()) {
            ...
            if (older_obc) {
                ...
                dout(20) << __func__ << " next oldest clone " << next << " is not present; implicitly clean" << dendl;
            }
        }
        else {
    ...
    if (p != flush_ops.end()) {
        ...
        if (fop->flushed_version == obc->obs.oi.user_version && (fop->blocking || !blocking)) {
            ...
            if (op) fop->dup_ops.push_back(op);
        ...
        if (fop->op) osd->reply_op_error(fop->op, -EBUSY);
    ...
    if (snapset.seq != 0) {
        ...
        for (vector<snapid_t>::reverse_iterator citer = snapset.clones.rbegin(); citer != snapset.clones.rend();
             ++citer) {
            if (*citer < soid.snap) {
                prev_snapc = *citer;
                break;
    ...
    if (oi.is_whiteout()) {
        ...
        o.copy_from(soid.oid.name,
                    ...
                    oloc,
                    oi.user_version,
                    CEPH_OSD_COPY_FROM_FLAG_FLUSH | CEPH_OSD_COPY_FROM_FLAG_IGNORE_OVERLAY |
                        CEPH_OSD_COPY_FROM_FLAG_IGNORE_CACHE | CEPH_OSD_COPY_FROM_FLAG_MAP_SNAP_CLONE,
    ...
    recovery_state.update_stats([&oi](auto& history, auto& stats) {
...
int PrimaryLogPG::try_flush_mark_clean(FlushOpRef fop)
{
    ...
    if (fop->flushed_version != obc->obs.oi.user_version || !obc->obs.exists) {
        if (obc->obs.exists)
            dout(10) << __func__ << " flushed_version " << fop->flushed_version << " != current "
                     << obc->obs.oi.user_version << dendl;
        else
    ...
    if (!fop->blocking && m_scrubber->write_blocked_by_scrub(oid)) {
        if (fop->op) {
    ...
    if (!obc->obs.oi.has_manifest() && !fop->op && agent_state &&
        agent_state->evict_mode != TierAgentState::EVICT_MODE_IDLE && agent_maybe_evict(obc, true)) {
        osd->logger->inc(l_osd_tier_clean);
        if (fop->on_flush) {
            (*(fop->on_flush))();
            fop->on_flush = std::nullopt;
    ...
    // have a ctx yet.
    if (ctx->lock_manager.get_lock_type(RWState::RWWRITE, oid, obc, fop->op)) {
        dout(20) << __func__ << " took write lock" << dendl;
    }
    else if (fop->op) {
        ...
        for (auto op : fop->dup_ops) {
            bool locked = ctx->lock_manager.get_lock_type(RWState::RWWRITE, oid, obc, op);
            ceph_assert(!locked);
        }
        close_op_ctx(ctx.release());
        return -EAGAIN;   // will retry
    ...
    if (fop->obc->obs.oi.has_manifest()) {
        ...
        if (obc->obs.oi.size == chunks_size) {
            ...
            ctx->modified_ranges.union_of(trim);
            truncate_update_size_and_usage(ctx->delta_stats, ctx->new_obs.oi, 0);
...
void PrimaryLogPG::cancel_flush(FlushOpRef fop, bool requeue, vector<ceph_tid_t>* tids)
{
...
{
    if (!pool.info.allow_incomplete_clones()) return true;
    if (is_missing_object(coid)) return true;
...
void PrimaryLogPG::cancel_cls_gather(map<hobject_t, CLSGatherOp>::iterator iter, bool requeue, vector<ceph_tid_t>* tids)
{
    auto& cgop = iter->second;
    ...
    if (repop->all_committed) {
        ...
        if (it != waiting_for_ondisk.end()) {
            ...
            for (auto& i : it->second) {
                ...
                osd->reply_op_error(std::get<0>(i), return_code, repop->v, std::get<1>(i), std::get<3>(i));
            }
        ...
        if (repop_queue.front() == repop) {
            ...
            while (!repop_queue.empty() && (to_remove = repop_queue.front())->all_committed) {
                repop_queue.pop_front();
                for (auto p = to_remove->on_success.begin(); p != to_remove->on_success.end();
...
PrimaryLogPG::RepGather* PrimaryLogPG::new_repop(OpContext* ctx, ceph_tid_t rep_tid)
{
    if (ctx->op)
...
boost::intrusive_ptr<PrimaryLogPG::RepGather> PrimaryLogPG::new_repop(
    eversion_t version, int r, ObcLockManager&& manager, OpRequestRef&& op,
    std::optional<std::function<void(void)>>&& on_complete)
{
    RepGather* repop =
        new RepGather(std::move(manager), std::move(op), std::move(on_complete), osd->get_tid(), info.last_complete, r);
    repop->v = version;
...
void PrimaryLogPG::remove_repop(RepGather* repop)
{
    ...
    for (auto p = repop->on_finish.begin(); p != repop->on_finish.end(); repop->on_finish.erase(p++)) {
        (*p)();
    }
...
void PrimaryLogPG::submit_log_entries(const mempool::osd_pglog::list<pg_log_entry_t>& entries, ObcLockManager&& manager,
                                      std::optional<std::function<void(void)>>&& _on_complete, OpRequestRef op, int r)
{
    ...
    if (get_osdmap()->require_osd_release >= ceph_release_t::jewel) {
        repop = new_repop(version, r, std::move(manager), std::move(op), std::move(_on_complete));
    }
    else {
        on_complete = std::move(_on_complete);
    }

    pgbackend->call_write_ordered([this, entries, repop, on_complete]() {
        ObjectStore::Transaction t;
        eversion_t old_last_update = info.last_update;
        recovery_state.merge_new_log_entries(
            entries, t, recovery_state.get_pg_trim_to(), recovery_state.get_min_last_complete_ondisk());
        ...
        for (set<pg_shard_t>::const_iterator i = get_acting_recovery_backfill().begin();
            ...
            if (get_osdmap()->require_osd_release >= ceph_release_t::jewel) {
                ceph_assert(repop);
                MOSDPGUpdateLogMissing* m = new MOSDPGUpdateLogMissing(entries,
                ...
                MOSDPGLog* m =
                    new MOSDPGLog(peer.shard, pg_whoami.shard, info.last_update.epoch, info, get_last_peering_reset());
                m->log.log = entries;
        ...
        log_entry_update_waiting_on.insert(make_pair(rep_tid, LogUpdateCtx{std::move(repop), std::move(waiting_on)}));
        ...
        t.register_on_commit(new OnComplete{this, rep_tid, get_osdmap_epoch()});
        int r = osd->store->queue_transaction(ch, std::move(t), NULL);
        ceph_assert(r == 0);
...
{
    for (map<pair<uint64_t, entity_name_t>, WatchRef>::iterator j = obc->watchers.begin(); j != obc->watchers.end();
         ++j) {
        ...
        dout(30) << "watch: Found oid=" << owi.obj << " addr=" << owi.wi.addr << " name=" << owi.wi.name
...
void PrimaryLogPG::check_blocklisted_watchers()
{
    ...
    pair<hobject_t, ObjectContextRef> i;
    while (object_contexts.get_next(i.first, &i)) check_blocklisted_obc_watchers(i.second);
}
...
void PrimaryLogPG::check_blocklisted_obc_watchers(ObjectContextRef obc)
{
    ...
    for (map<pair<uint64_t, entity_name_t>, WatchRef>::iterator k = obc->watchers.begin(); k != obc->watchers.end();) {
...
void PrimaryLogPG::populate_obc_watchers(ObjectContextRef obc)
{
    ...
    ceph_assert(
        (recovering.count(obc->obs.oi.soid) || !is_missing_object(obc->obs.oi.soid)) ||
        (it_objects !=
             recovery_state.get_pg_log().get_log().objects.end() &&   // or this is a revert... see recover_primary()
         it_objects->second->op == pg_log_entry_t::LOST_REVERT &&
         it_objects->second->reverting_to == obc->obs.oi.version));

    dout(10) << "populate_obc_watchers " << obc->obs.oi.soid << dendl;
    ceph_assert(obc->watchers.empty());
    // populate unconnected_watchers
    for (map<pair<uint64_t, entity_name_t>, watch_info_t>::iterator p = obc->obs.oi.watchers.begin();
        ...
        WatchRef watch(Watch::makeWatchRef(
            this, osd, obc, p->second.timeout_seconds, p->first.first, p->first.second, p->second.addr));
        watch->disconnect();
        obc->watchers.insert(make_pair(make_pair(p->first.first, p->first.second), watch));
    }
...
void PrimaryLogPG::handle_watch_timeout(WatchRef watch)
{
    ...
    if (is_degraded_or_backfilling_object(obc->obs.oi.soid)) {
        callbacks_for_degraded_object[obc->obs.oi.soid].push_back(watch->get_delayed_cb());
    ...
    if (m_scrubber->write_blocked_by_scrub(obc->obs.oi.soid)) {
        ...
        m_scrubber->add_callback(watch->get_delayed_cb()   // This callback!
        );
        return;
    }
    ...
    oi.watchers.erase(make_pair(watch->get_cookie(), watch->get_entity()));

    list<watch_disconnect_t> watch_disconnects = {watch_disconnect_t(watch->get_cookie(), watch->get_entity(), true)};
    ctx->register_on_success([this, obc, watch_disconnects]() { complete_disconnect_watches(obc, watch_disconnects); });


    PGTransaction* t = ctx->op_t.get();
    ctx->log.push_back(pg_log_entry_t(
        pg_log_entry_t::MODIFY, obc->obs.oi.soid, ctx->at_version, oi.version, 0, osd_reqid_t(), ctx->mtime, 0));
...
ObjectContextRef PrimaryLogPG::create_object_context(const object_info_t& oi, SnapSetContext* ssc)
{
    ...
    obc->obs.exists = false;
    obc->ssc = ssc;
    if (ssc) register_snapset_context(ssc);
    dout(10) << "create_object_context " << (void*)obc.get() << " " << oi.soid << " " << dendl;
    if (is_active()) populate_obc_watchers(obc);
    return obc;
}

ObjectContextRef PrimaryLogPG::get_object_context(const hobject_t& soid, bool can_create,
                                                  const map<string, bufferlist, less<>>* attrs)
{
    auto it_objects = recovery_state.get_pg_log().get_log().objects.find(soid);
    ceph_assert(attrs || !recovery_state.get_pg_log().get_missing().is_missing(soid) ||
    ...
    if (obc) {
        ...
        if (attrs) {
            ...
            if (r < 0) {
                ...
                dout(10) << __func__ << ": " << *obc << " oi: " << obc->obs.oi << " " << *obc->ssc << dendl;
                return obc;
            }
        }
        ...
        if (is_primary() && is_active()) populate_obc_watchers(obc);
    ...
    dout(10) << __func__ << ": " << *obc << " oi: " << obc->obs.oi << " exists: " << (int)obc->obs.exists << " "
             << *obc->ssc << dendl;
    return obc;
}
...
void PrimaryLogPG::context_registry_on_change()
{
    ...
    while (object_contexts.get_next(i.first, &i)) {
        ObjectContextRef obc(i.second);
        if (obc) {
            for (map<pair<uint64_t, entity_name_t>, WatchRef>::iterator j = obc->watchers.begin();
                 j != obc->watchers.end();
 ...
 */
int PrimaryLogPG::find_object_context(const hobject_t& oid, ObjectContextRef* pobc, bool can_create,
                                      bool map_snapid_to_clone, hobject_t* pmissing)
{
    FUNCTRACE(cct);
    ...
    if (!ssc || !(ssc->exists || can_create)) {
        ...
        if (pmissing) *pmissing = head;   // start by getting the head
        if (ssc) put_snapset_context(ssc);
        return -ENOENT;
    }

    if (map_snapid_to_clone) {
        dout(10) << __func__ << " " << oid << " @" << oid.snap << " snapset " << ssc->snapset
                 << " map_snapid_to_clone=true" << dendl;
        if (oid.snap > ssc->snapset.seq) {
            // already must be readable
            ObjectContextRef obc = get_object_context(head, false);
            dout(10) << __func__ << " " << oid << " @" << oid.snap << " snapset " << ssc->snapset << " maps to head"
                     << dendl;
            *pobc = obc;
            ...
            vector<snapid_t>::const_iterator citer =
                std::find(ssc->snapset.clones.begin(), ssc->snapset.clones.end(), oid.snap);
            ...
            dout(10) << __func__ << " " << oid << " @" << oid.snap << " snapset " << ssc->snapset << " maps to " << oid
                     << dendl;

            if (recovery_state.get_pg_log().get_missing().is_missing(oid)) {
                dout(10) << __func__ << " " << oid << " @" << oid.snap << " snapset " << ssc->snapset << " " << oid
                         << " is missing" << dendl;
                if (pmissing) *pmissing = oid;
                put_snapset_context(ssc);
                return -EAGAIN;
    ...
    if (oid.snap > ssc->snapset.seq) {
        ObjectContextRef obc = get_object_context(head, false);
        dout(10) << __func__ << " " << head << " want " << oid.snap << " > snapset seq " << ssc->snapset.seq
                 << " -- HIT " << obc->obs << dendl;
        if (!obc->ssc)
            obc->ssc = ssc;
        else {
    ...
    unsigned k = 0;
    while (k < ssc->snapset.clones.size() && ssc->snapset.clones[k] < oid.snap) k++;
    ...
    hobject_t soid(
        oid.oid, oid.get_key(), ssc->snapset.clones[k], oid.get_hash(), info.pgid.pool(), oid.get_namespace());

    if (recovery_state.get_pg_log().get_missing().is_missing(soid)) {
        dout(20) << __func__ << " " << soid << " missing, try again later" << dendl;
        if (pmissing) *pmissing = soid;
        put_snapset_context(ssc);
        return -EAGAIN;
    }
    ...
    if (std::find(p->second.begin(), p->second.end(), oid.snap) == p->second.end()) {
        dout(20) << __func__ << " " << soid << " clone_snaps " << p->second << " does not contain " << oid.snap
                 << " -- DNE" << dendl;
        return -ENOENT;
    }
    if (get_osdmap()->in_removed_snaps_queue(info.pgid.pgid.pool(), oid.snap)) {
        dout(20) << __func__ << " " << soid << " snap " << oid.snap << " in removed_snaps_queue"
                 << " -- DNE" << dendl;
        return -ENOENT;
    }
    dout(20) << __func__ << " " << soid << " clone_snaps " << p->second << " contains " << oid.snap << " -- HIT "
             << obc->obs << dendl;
    *pobc = obc;
    return 0;
}
...
{
    if (obc->ssc) put_snapset_context(obc->ssc);
}
...
void PrimaryLogPG::add_object_context_to_pg_stat(ObjectContextRef obc, pg_stat_t* pgstat)
{
    ...
    stat.num_objects++;
    if (oi.is_dirty()) stat.num_objects_dirty++;
    if (oi.is_whiteout()) stat.num_whiteouts++;
    if (oi.is_omap()) stat.num_objects_omap++;
    if (oi.is_cache_pinned()) stat.num_objects_pinned++;
    if (oi.has_manifest()) stat.num_objects_manifest++;

    if (oi.soid.is_snap()) {
        stat.num_object_clones++;

        if (!obc->ssc) obc->ssc = get_snapset_context(oi.soid, false);
        ceph_assert(obc->ssc);
...
void PrimaryLogPG::kick_object_context_blocked(ObjectContextRef obc)
{
    ...
    map<hobject_t, ObjectContextRef>::iterator i = objects_blocked_on_snap_promotion.find(obc->obs.oi.soid.get_head());
...
SnapSetContext* PrimaryLogPG::get_snapset_context(const hobject_t& oid, bool can_create,
                                                  const map<string, bufferlist, less<>>* attrs, bool oid_existed)
{
...
void PrimaryLogPG::put_snapset_context(SnapSetContext* ssc)
{
    ...
    --ssc->ref;
    if (ssc->ref == 0) {
        if (ssc->registered) snapset_contexts.erase(ssc->oid);
        delete ssc;
    }
}
...
int PrimaryLogPG::recover_missing(const hobject_t& soid, eversion_t v, int priority, PGBackend::RecoveryHandle* h)
{
        ...
        remove_missing_object(soid, v, new LambdaContext([=, this](int) {
                                  ...
                                  if (!pg_has_reset_since(cur_epoch)) {
                                      ...
                                      if (!object_missing) {
                                          ...
                                          if (scrub_after_recovery) stat_diff.num_objects_repaired = 1;
        ...
        if (recovery_state.get_pg_log().get_missing().is_missing(head)) {
            if (recovering.count(head)) {
                ...
                int r = recover_missing(
                    head, recovery_state.get_pg_log().get_missing().get_items().find(head)->second.need, priority, h);
                if (r != PULL_NONE) return PULL_HEAD;
                return PULL_NONE;
            }
        }
        head_obc = get_object_context(head, false, 0);
        ceph_assert(head_obc);
    }
    start_recovery_op(soid);
    ceph_assert(!recovering.count(soid));
    recovering.insert(make_pair(soid, obc));
    int r = pgbackend->recover_object(soid, v, head_obc, obc, h);
    // This is only a pull which shouldn't return an error
    ceph_assert(r >= 0);
    return PULL_YES;
}

void PrimaryLogPG::remove_missing_object(const hobject_t& soid, eversion_t v, Context* on_complete)
{
    ...
    epoch_t cur_epoch = get_osdmap_epoch();
    t.register_on_complete(new LambdaContext([=, this](int) {
...
void PrimaryLogPG::finish_degraded_object(const hobject_t oid)
{
    ...
    if (callbacks_for_degraded_object.count(oid)) {
        ...
        for (list<Context*>::iterator i = contexts.begin(); i != contexts.end(); ++i) {
            (*i)->complete(0);
        }
    }
    map<hobject_t, snapid_t>::iterator i = objects_blocked_on_degraded_snap.find(oid.get_head());
    if (i != objects_blocked_on_degraded_snap.end() && i->second == oid.snap) objects_blocked_on_degraded_snap.erase(i);
}

void PrimaryLogPG::_committed_pushed_object(epoch_t epoch, eversion_t last_complete)
{
    std::scoped_lock locker{*this};
...
void PrimaryLogPG::_applied_recovered_object(ObjectContextRef obc)
{
    ...
    if (!recovery_state.is_deleting() && active_pushes == 0 && is_scrub_active()) {
...
void PrimaryLogPG::on_failed_pull(const set<pg_shard_t>& from, const hobject_t& soid, const eversion_t& v)
{
    ...
    dout(0) << __func__ << " " << soid << " from shard " << from << ", reps on "
            << recovery_state.get_missing_loc().get_locations(soid) << " unfound? "
            << recovery_state.get_missing_loc().is_unfound(soid) << dendl;
...
{
    const MOSDPGUpdateLogMissing* m = static_cast<const MOSDPGUpdateLogMissing*>(op->get_req());
    ceph_assert(m->get_type() == MSG_OSD_PG_UPDATE_LOG_MISSING);
    ObjectStore::Transaction t;
    std::optional<eversion_t> op_trim_to, op_roll_forward_to;
    if (m->pg_trim_to != eversion_t()) op_trim_to = m->pg_trim_to;
    if (m->pg_roll_forward_to != eversion_t()) op_roll_forward_to = m->pg_roll_forward_to;

    dout(20) << __func__ << " op_trim_to = " << op_trim_to << " op_roll_forward_to = " << op_roll_forward_to << dendl;

    recovery_state.append_log_entries_update_missing(m->entries, t, op_trim_to, op_roll_forward_to);
    eversion_t new_lcod = info.last_complete;

    Context* complete = new LambdaContext([=, this](int) {
        const MOSDPGUpdateLogMissing* msg = static_cast<const MOSDPGUpdateLogMissing*>(op->get_req());
        std::scoped_lock locker{*this};
        if (!pg_has_reset_since(msg->get_epoch())) {
            update_last_complete_ondisk(new_lcod);
            MOSDPGUpdateLogMissingReply* reply =
                new MOSDPGUpdateLogMissingReply(spg_t(info.pgid.pgid, primary_shard().shard),
                                                pg_whoami.shard,
                                                msg->get_epoch(),
                                                msg->min_epoch,
                                                msg->get_tid(),
    ...
    int tr = osd->store->queue_transaction(ch, std::move(t), nullptr);
    ceph_assert(tr == 0);
    op_applied(info.last_update);
}
...
{
    const MOSDPGUpdateLogMissingReply* m = static_cast<const MOSDPGUpdateLogMissingReply*>(op->get_req());
    ...
    if (it != log_entry_update_waiting_on.end()) {
        if (it->second.waiting_on.count(m->get_from())) {
            ...
            osd->clog->error() << info.pgid << " got reply " << *m << " from shard we are not waiting for "
                               << m->get_from();
        }
        ...
        osd->clog->error() << info.pgid << " got reply " << *m << " on unknown tid " << m->get_tid();
    }
}

/* Mark all unfound objects as lost.
 */
void PrimaryLogPG::mark_all_unfound_lost(int what, std::function<void(int, const std::string&, bufferlist&)> on_finish)
{
    ...
    *_dout << dendl;
    ...
    map<hobject_t, pg_missing_item>::const_iterator m = recovery_state.get_missing_loc().get_needs_recovery().begin();
    map<hobject_t, pg_missing_item>::const_iterator mend = recovery_state.get_missing_loc().get_needs_recovery().end();
    ...
    while (m != mend) {
        ...
        switch (what) {
        case pg_log_entry_t::LOST_MARK: ceph_abort_msg("actually, not implemented yet!"); break;

        case pg_log_entry_t::LOST_REVERT:
            prev = pick_newest_available(oid);
            if (prev > eversion_t()) {
                // log it
                pg_log_entry_t e(pg_log_entry_t::LOST_REVERT, oid, v, m->second.need, 0, osd_reqid_t(), mtime, 0);
                e.reverting_to = prev;
        ...
        {
            pg_log_entry_t e(pg_log_entry_t::LOST_DELETE, oid, v, m->second.need, 0, osd_reqid_t(), mtime, 0);
            if (get_osdmap()->require_osd_release >= ceph_release_t::jewel) {
    ...
    submit_log_entries(log_entries,
                       std::move(manager),
                       std::optional<std::function<void(void)>>([this, oids, num_unfound, on_finish]() {
                           ...
                           stringstream ss;
                           ss << "pg has " << num_unfound << " objects unfound and apparently lost marking";
                           string rs = ss.str();
...
void PrimaryLogPG::clear_async_reads()
{
    ...
    for (auto& i : in_progress_async_reads) {
        dout(10) << "clear ctx: "
                 << "OpRequestRef " << i.first << " OpContext " << i.second << dendl;
...
void PrimaryLogPG::on_activate_complete()
{
    ...
    if (needs_recovery()) {
        ...
        queue_peering_event(PGPeeringEventRef(
            std::make_shared<PGPeeringEvent>(get_osdmap_epoch(), get_osdmap_epoch(), PeeringState::DoRecovery())));
    }
    else if (needs_backfill()) {
        dout(10) << "activate queueing backfill" << dendl;
        queue_peering_event(PGPeeringEventRef(
            std::make_shared<PGPeeringEvent>(get_osdmap_epoch(), get_osdmap_epoch(), PeeringState::RequestBackfill())));
    }
    else {
    ...
    if (get_backfill_targets().size()) {
        ...
        for (set<pg_shard_t>::const_iterator i = get_backfill_targets().begin(); i != get_backfill_targets().end();
             ++i) {
...
void PrimaryLogPG::on_change(ObjectStore::Transaction& t)
{
    ...
    for (auto p = waiting_for_blocked_object.begin(); p != waiting_for_blocked_object.end();
         waiting_for_blocked_object.erase(p++)) {
        if (is_primary())
            requeue_ops(p->second);
        else
    ...
    for (list<pair<OpRequestRef, OpContext*>>::iterator i = in_progress_async_reads.begin();
         i != in_progress_async_reads.end();
         in_progress_async_reads.erase(i++)) {
        close_op_ctx(i->second);
        if (is_primary()) requeue_op(i->first);
    }
...
void PrimaryLogPG::plpg_on_pool_change()
{
    ...
    // ordered).
    if (is_active() && pool.info.cache_mode != pg_pool_t::CACHEMODE_WRITEBACK && !waiting_for_cache_not_full.empty()) {
...
void PrimaryLogPG::_clear_recovery_state()
{
    ...
    list<OpRequestRef> blocked_ops;
    for (map<hobject_t, ObjectContextRef>::iterator i = recovering.begin(); i != recovering.end();
         recovering.erase(i++)) {
        if (i->second) {
...
void PrimaryLogPG::cancel_pull(const hobject_t& soid)
{
    ...
    if (is_missing_object(soid)) recovery_state.set_last_requested(0);
    finish_degraded_object(soid);
}
...
bool PrimaryLogPG::start_recovery_ops(uint64_t max, ThreadPool::TPHandle& handle, uint64_t* ops_started)
{
    ...
    if (started || recovery_started) work_in_progress = true;

    bool deferred_backfill = false;
    if (recovering.empty() && state_test(PG_STATE_BACKFILLING) && !get_backfill_targets().empty() && started < max &&
    ...
    if (missing.num_missing() > 0) {
        ...
        osd->clog->error() << info.pgid << " Unexpected Error: recovery ending with " << missing.num_missing() << ": "
                           << missing.get_items();
        return false;
    }
    ...
    if (needs_recovery()) {
        ...
        osd->clog->error() << info.pgid << " Unexpected Error: recovery ending with missing replicas";
        return false;
    }
    ...
    if (state_test(PG_STATE_RECOVERING)) {
        ...
        queue_peering_event(PGPeeringEventRef(
            std::make_shared<PGPeeringEvent>(get_osdmap_epoch(), get_osdmap_epoch(), PeeringState::Backfilled())));
    }
...
uint64_t PrimaryLogPG::recover_primary(uint64_t max, ThreadPool::TPHandle& handle)
{
    ...
    while (p != missing.get_rmissing().end()) {
        ...
        dout(10) << __func__ << " " << soid << " " << item.need << (missing.is_missing(soid) ? " (missing)" : "")
            ...
            case pg_log_entry_t::LOST_REVERT:
            {
                if (item.have == latest->reverting_to) {
                    ...
                    if (obc->obs.oi.version == latest->version) {
                        ...
                        ObjectStore::Transaction t;
                        bufferlist b2;
                        obc->obs.oi.encode(b2, get_osdmap()->get_features(CEPH_ENTITY_TYPE_OSD, nullptr));
                        ...
                        t.register_on_commit(
                            new C_OSD_CommittedPushedObject(this, get_osdmap_epoch(), info.last_complete));
                    ...
                    for (auto p = recovery_state.get_peer_missing().begin();
                         ...
                         ++p) {
                        if (p->second.is_missing(soid, need) && p->second.get_items().at(soid).have == alternate_need) {
        ...
        if (!recovering.count(soid)) {
            if (recovering.count(head)) {
                ...
                int r = recover_missing(soid, need, recovery_state.get_recovery_op_priority(), h);
                switch (r) {
                case PULL_YES: ++started; break;
                case PULL_HEAD: ++started;
                case PULL_NONE: ++skipped; break;
                default: ceph_abort();
                }
        ...
        if (!skipped) recovery_state.set_last_requested(v);
    }
...
bool PrimaryLogPG::primary_error(const hobject_t& soid, eversion_t v)
{
...
int PrimaryLogPG::prep_object_replica_deletes(const hobject_t& soid, eversion_t v, PGBackend::RecoveryHandle* h,
                                              bool* work_started)
{
    ceph_assert(is_primary());
    ...
    if (obc) {
        if (!obc->get_recovery_read()) {
            dout(20) << "replica delete delayed on " << soid << "; could not get rw_manager lock" << dendl;
            *work_started = true;
            return 0;
        }
        else {
...
int PrimaryLogPG::prep_object_replica_pushes(const hobject_t& soid, eversion_t v, PGBackend::RecoveryHandle* h,
                                             bool* work_started)
{
    ceph_assert(is_primary());
    dout(10) << __func__ << ": on " << soid << dendl;
    ...
    int r = pgbackend->recover_object(soid,
                                      v,
                                      ObjectContextRef(),
...
uint64_t PrimaryLogPG::recover_replicas(uint64_t max, ThreadPool::TPHandle& handle, bool* work_started)
{
    ...
    std::vector<std::pair<unsigned int, pg_shard_t>> replicas_by_num_missing, async_by_num_missing;
    ...
    auto func = [](const std::pair<unsigned int, pg_shard_t>& lhs, const std::pair<unsigned int, pg_shard_t>& rhs) {
    ...
    replicas_by_num_missing.insert(
        replicas_by_num_missing.end(), async_by_num_missing.begin(), async_by_num_missing.end());
    for (auto& replica : replicas_by_num_missing) {
        ...
        for (map<version_t, hobject_t>::const_iterator p = m.get_rmissing().begin();
            ...
            if (soid.is_snap() && recovery_state.get_pg_log().get_missing().is_missing(soid.get_head())) {
...
bool PrimaryLogPG::all_peer_done() const
{
    ...
    for (const pg_shard_t& bt : get_backfill_targets()) {
        ...
        if (!pbi.extends_to_end() || !pbi.empty()) return false;
    }
    return true;
}
...
uint64_t PrimaryLogPG::recover_backfill(uint64_t max, ThreadPool::TPHandle& handle, bool* work_started)
{
    dout(10) << __func__ << " (" << max << ")"
             << " bft=" << get_backfill_targets() << " last_backfill_started " << last_backfill_started
             << (new_backfill ? " new_backfill" : "") << dendl;
    ...
    for (set<pg_shard_t>::const_iterator i = get_backfill_targets().begin(); i != get_backfill_targets().end(); ++i) {
        dout(10) << "peer osd." << *i << " info " << recovery_state.get_peer_info(*i) << " interval "
                 << peer_backfill_info[*i].begin << "-" << peer_backfill_info[*i].end << " "
                 << peer_backfill_info[*i].objects.size() << " objects" << dendl;
    }
    ...
    for (set<pg_shard_t>::const_iterator i = get_backfill_targets().begin(); i != get_backfill_targets().end(); ++i) {
        peer_backfill_info[*i].trim_to(std::max(recovery_state.get_peer_info(*i).last_backfill, last_backfill_started));
    }
    ...
    while (ops < max) {
        ...
        if (check < backfill_info.begin) {
            ...
            for (set<pg_shard_t>::const_iterator i = get_backfill_targets().begin(); i != get_backfill_targets().end();
                ...
                pg_shard_t bt = *i;
                BackfillInterval& pbi = peer_backfill_info[bt];
                if (pbi.begin == check) check_targets.insert(bt);
            }
            ...
            if (!keep_ver_targs.empty()) {
                ...
                dout(20) << " BACKFILL keeping " << check << " with ver " << obj_v << " on peers " << keep_ver_targs
                         << dendl;
            ...
            if (!need_ver_targs.empty() || !missing_targs.empty()) {
                ...
                if (obc->get_recovery_read()) {
                    if (!need_ver_targs.empty()) {
                        dout(20) << " BACKFILL replacing " << check << " with ver " << obj_v << " to peers "
                                 << need_ver_targs << dendl;
                    }
                    if (!missing_targs.empty()) {
                        dout(20) << " BACKFILL pushing " << backfill_info.begin << " with ver " << obj_v << " to peers "
                                 << missing_targs << dendl;
                    }
                    ...
                    *work_started = true;
                    dout(20) << "backfill blocking on " << backfill_info.begin << "; could not get rw_manager lock"
                             << dendl;
                    break;
                }
            }
            dout(20) << "need_ver_targs=" << need_ver_targs << " keep_ver_targs=" << keep_ver_targs << dendl;
            dout(20) << "backfill_targets=" << get_backfill_targets() << " missing_targs=" << missing_targs
    ...
    for (unsigned i = 0; i < to_remove.size(); ++i) {
        ...
        if (it != reqs.end()) {
            ...
            m = reqs[peer] = new MOSDPGBackfillRemove(spg_t(info.pgid.pgid, peer.shard), get_osdmap_epoch());
            if (cct->_conf->osd_op_queue == "mclock_scheduler") {
                /* This guard preserves legacy WeightedPriorityQueue behavior for
                 * now, but should be removed after Reef */
                m->set_priority(recovery_state.get_recovery_op_priority());
        ...
        if (oid <= last_backfill_started) pending_backfill_updates[oid];   // add empty stat!
    }
    for (auto p : reqs) {
    ...
    for (set<hobject_t>::iterator i = backfills_in_flight.begin(); i != backfills_in_flight.end(); ++i) {
        ...
        if (new_last_backfill > pinfo.last_backfill) {
            ...
            if (pinfo.last_backfill.is_max()) {
                m = new MOSDPGBackfill(
                    MOSDPGBackfill::OP_BACKFILL_FINISH, e, get_last_peering_reset(), spg_t(info.pgid.pgid, bt.shard));
                // Use default priority here, must match sub_op priority
                start_recovery_op(hobject_t::get_max());
            }
            else {
                m = new MOSDPGBackfill(
                    MOSDPGBackfill::OP_BACKFILL_PROGRESS, e, get_last_peering_reset(), spg_t(info.pgid.pgid, bt.shard));
                // Use default priority here, must match sub_op priority
            }
    ...
    if (ops) *work_started = true;
    return ops;
}

int PrimaryLogPG::prep_backfill_object_push(hobject_t oid, eversion_t v, ObjectContextRef obc, vector<pg_shard_t> peers,
                                            PGBackend::RecoveryHandle* h)
{
    ...
    int r = pgbackend->recover_object(oid, v, ObjectContextRef(), obc, h);
    if (r < 0) {
...
void PrimaryLogPG::update_range(BackfillInterval* bi, ThreadPool::TPHandle& handle)
{
    ...
    if (bi->version >= projected_last_update) {
        ...
        dout(10) << __func__ << ": bi is old, (" << bi->version << ") can be updated with log to projected_last_update "
...
void PrimaryLogPG::scan_range(int min, int max, BackfillInterval* bi, ThreadPool::TPHandle& handle)
{
    ...
    for (vector<hobject_t>::iterator p = ls.begin(); p != ls.end(); ++p) {
        handle.reset_tp_timeout();
        ObjectContextRef obc;
        if (is_primary()) obc = object_contexts.lookup(*p);
        if (obc) {
            if (!obc->obs.exists) {
...
void PrimaryLogPG::check_local()
{
    ...
    ceph_assert(info.last_update >= recovery_state.get_pg_log().get_tail());   // otherwise we need some help!

    if (!cct->_conf->osd_debug_verify_stray_on_activate) return;
    ...
    for (list<pg_log_entry_t>::const_reverse_iterator p = recovery_state.get_pg_log().get_log().log.rbegin();
        ...
        if (p->is_delete() && !is_missing_object(p->soid)) {
            ...
            struct stat st;
            int r = osd->store->stat(ch, ghobject_t(p->soid, ghobject_t::NO_GEN, pg_whoami.shard), &st);
            if (r != -ENOENT) {
...
hobject_t PrimaryLogPG::get_hit_set_current_object(utime_t stamp)
{
    ...
    hobject_t hoid(
        sobject_t(ss.str(), CEPH_NOSNAP), "", info.pgid.ps(), info.pgid.pool(), cct->_conf->osd_hit_set_namespace);
    dout(20) << __func__ << " " << hoid << dendl;
    return hoid;
}

hobject_t PrimaryLogPG::get_hit_set_archive_object(utime_t start, utime_t end, bool using_gmt)
{
    ostringstream ss;
...
void PrimaryLogPG::hit_set_setup()
{
    ...
    if (is_active() && is_primary() &&
        (!pool.info.hit_set_count || !pool.info.hit_set_period ||
...
void PrimaryLogPG::hit_set_remove_all()
{
    ...
    for (auto p = info.hit_set.history.begin(); p != info.hit_set.history.end(); ++p) {
        hobject_t aoid = get_hit_set_archive_object(p->begin, p->end, p->using_gmt);

        // Once we hit a degraded object just skip
        if (is_degraded_or_backfilling_object(aoid)) return;
        if (m_scrubber->write_blocked_by_scrub(aoid)) return;
    }
...
void PrimaryLogPG::hit_set_create()
{
    ...
    if (pool.info.hit_set_params.get_type() == HitSet::TYPE_BLOOM) {
        ...
        if (p->get_fpp() <= 0.0) p->set_fpp(.01);   // fpp cannot be zero!
        ...
        if (p->target_size == 0 && hit_set) {
            ...
            dout(20) << __func__ << " previous set had approx " << unique << " unique items over " << dur << " seconds"
                     << dendl;
            p->target_size = (double)unique * (double)pool.info.hit_set_period / (double)dur;
        }
        if (p->target_size < static_cast<uint64_t>(cct->_conf->osd_hit_set_min_size))
...
bool PrimaryLogPG::hit_set_apply_log()
{
    ...
    list<pg_log_entry_t>::const_reverse_iterator p = recovery_state.get_pg_log().get_log().log.rbegin();
    while (p != recovery_state.get_pg_log().get_log().log.rend() && p->version > to) ++p;
...
void PrimaryLogPG::hit_set_persist()
{
    ...
    for (set<pg_shard_t>::const_iterator p = get_backfill_targets().begin(); p != get_backfill_targets().end(); ++p) {
        const pg_info_t& pi = recovery_state.get_peer_info(*p);
        if (pi.last_backfill == hobject_t() || pi.last_backfill.get_hash() == info.pgid.ps()) {
            dout(10) << __func__ << " backfill target osd." << *p << " last_backfill has not progressed past pgid ps"
                     << dendl;
            return;
        }
    }
    ...
    new_hset.end = now;
    oid = get_hit_set_archive_object(new_hset.begin, new_hset.end, new_hset.using_gmt);

    // If the current object is degraded we skip this persist request
    if (m_scrubber->write_blocked_by_scrub(oid)) return;
    ...
    encode(ctx->new_obs.oi, boi, get_osdmap()->get_features(CEPH_ENTITY_TYPE_OSD, nullptr));

    ctx->op_t->create(oid);
    if (bl.length()) {
        ctx->op_t->write(oid, 0, bl.length(), bl, 0);
        write_update_size_and_usage(ctx->delta_stats, obc->obs.oi, ctx->modified_ranges, 0, bl.length());
        ctx->clean_regions.mark_data_region_dirty(0, bl.length());
    }
    map<string, bufferlist, std::less<>> attrs = {{OI_ATTR, std::move(boi)}, {SS_ATTR, std::move(bss)}};
    setattrs_maybe_cache(ctx->obc, ctx->op_t.get(), attrs);
    ctx->log.push_back(
        pg_log_entry_t(pg_log_entry_t::MODIFY, oid, ctx->at_version, eversion_t(), 0, osd_reqid_t(), ctx->mtime, 0));
    ctx->log.back().clean_regions = ctx->clean_regions;
...
void PrimaryLogPG::hit_set_trim(OpContextUPtr& ctx, unsigned max)
{
    ...
    for (unsigned num = updated_hit_set_hist.history.size(); num > max; --num) {
        ...
        ++ctx->at_version.version;
        ctx->log.push_back(
            pg_log_entry_t(pg_log_entry_t::DELETE, oid, ctx->at_version, p->version, 0, osd_reqid_t(), ctx->mtime, 0));
...
{
    ceph_assert(is_locked());
    if (!is_active() || !is_primary() || state_test(PG_STATE_PREMERGE) ||
        pool.info.cache_mode == pg_pool_t::CACHEMODE_NONE || pool.info.tier_of < 0 ||
    ...
    if (!agent_state) {
        ...
        agent_state->position.set_hash(pool.info.get_random_pg_position(info.pgid.pgid, rand()));
...
bool PrimaryLogPG::agent_work(int start_max, int agent_flush_quota)
{
    ...
    dout(10) << __func__ << " max " << start_max << ", flush " << agent_state->get_flush_mode_name() << ", evict "
             << agent_state->get_evict_mode_name() << ", pos " << agent_state->position << dendl;
    ceph_assert(is_primary());
    ceph_assert(is_active());
    ...
    for (vector<hobject_t>::iterator p = ls.begin(); p != ls.end(); ++p) {
        ...
        if (m_scrubber->range_intersects_scrub(obc->obs.oi.soid, obc->obs.oi.soid.get_head())) {
    ...
    dout(20) << __func__ << " start pos " << agent_state->position << " next start pos " << next << " started "
             << total_started << dendl;
...
bool PrimaryLogPG::agent_maybe_flush(ObjectContextRef& obc)
{
    ...
    if (!evict_mode_full && obc->obs.oi.soid.snap == CEPH_NOSNAP &&   // snaps immutable; don't delay
...
bool PrimaryLogPG::agent_maybe_evict(ObjectContextRef& obc, bool after_flush)
{
    ...
    if (agent_state->evict_mode != TierAgentState::EVICT_MODE_FULL) {
        ...
        if (hit_set) agent_estimate_temp(soid, &temp);
        agent_state->temp_hist.add(temp);
        agent_state->temp_hist.get_position_micro(temp, &temp_lower, &temp_upper);

        dout(20) << __func__ << " temp " << temp << " pos " << temp_lower << "-" << temp_upper << ", evict_effort "
        ...
        *_dout << dendl;
        ...
        if (1000000 - temp_upper >= agent_state->evict_effort) return false;
    }
    ...
    if (!ctx->lock_manager.get_lock_type(RWState::RWWRITE, obc->obs.oi.soid, obc, null_op_req)) {
        close_op_ctx(ctx.release());
    ...
    if (obc->obs.oi.is_omap()) ctx->delta_stats.num_objects_omap--;
    ctx->delta_stats.num_evict++;
    ctx->delta_stats.num_evict_kb += shift_round_up(obc->obs.oi.size, 10);
    if (obc->obs.oi.is_dirty()) --ctx->delta_stats.num_objects_dirty;
    ceph_assert(r == 0);
...
bool PrimaryLogPG::agent_choose_mode(bool restart, OpRequestRef op)
{
...

    {
        ...
        const pg_pool_t* base_pool = get_osdmap()->get_pg_pool(pool.info.tier_of);
        ceph_assert(base_pool);
        if (!base_pool->supports_omap()) unflushable += info.stats.stats.sum.num_objects_omap;
        ...
        if (pool.info.target_max_bytes && num_user_objects > 0) {
            ...
            full_micro =
                num_user_objects * avg_size * 1000000 / std::max<uint64_t>(pool.info.target_max_bytes / divisor, 1);
        }
        if (pool.info.target_max_objects > 0) {
            uint64_t dirty_objects_micro =
                num_dirty * 1000000 / std::max<uint64_t>(pool.info.target_max_objects / divisor, 1);
            if (dirty_objects_micro > dirty_micro) dirty_micro = dirty_objects_micro;
            uint64_t full_objects_micro =
                num_user_objects * 1000000 / std::max<uint64_t>(pool.info.target_max_objects / divisor, 1);
            if (full_objects_micro > full_micro) full_micro = full_objects_micro;
        }
        dout(20) << __func__ << " dirty " << ((float)dirty_micro / 1000000.0) << " full "
                 << ((float)full_micro / 1000000.0) << dendl;
        ...
        if (full_micro > 1000000) {
            ...
            evict_effort =
                std::max(over * 1000000 / span, uint64_t(1000000.0 * cct->_conf->osd_agent_min_evict_effort));
            ...
            if (evict_effort < inc) evict_effort = inc;
            ceph_assert(evict_effort >= inc && evict_effort <= 1000000);
            dout(30) << __func__ << " evict_effort " << was << " quantized by " << inc << " to " << evict_effort
                     << dendl;
        }
    }
...
skip_calc:
    ...
    if (flush_mode != agent_state->flush_mode) {
        dout(5) << __func__ << " flush_mode " << TierAgentState::get_flush_mode_name(agent_state->flush_mode) << " -> "
                << TierAgentState::get_flush_mode_name(flush_mode) << dendl;
        recovery_state.update_stats([=, this](auto& history, auto& stats) {
    ...
    if (evict_mode != agent_state->evict_mode) {
        dout(5) << __func__ << " evict_mode " << TierAgentState::get_evict_mode_name(agent_state->evict_mode) << " -> "
    ...
    if (evict_effort != agent_state->evict_effort) {
        dout(5) << __func__ << " evict_effort " << ((float)agent_state->evict_effort / 1000000.0) << " -> "
...
void PrimaryLogPG::agent_estimate_temp(const hobject_t& oid, int* temp)
{
    ...
    ceph_assert(temp);
    *temp = 0;
    if (hit_set->contains(oid)) *temp = 1000000;
    unsigned i = 0;
    int last_n = pool.info.hit_set_search_last_n;
    for (map<time_t, HitSetRef>::reverse_iterator p = agent_state->hit_set_map.rbegin();
         last_n > 0 && p != agent_state->hit_set_map.rend();
         ++p, ++i) {
...
{
    return pg->is_clean() && !pg->is_scrub_queued_or_active() && !pg->snap_trimq.empty();
}

/*---SnapTrimmer states---*/
#undef dout_prefix
#define dout_prefix (context<SnapTrimmer>().pg->gen_prefix(*_dout) << "SnapTrimmer state<" << get_state_name() << ">: ")
...
boost::statechart::result PrimaryLogPG::NotTrimming::react(const KickTrim&)
{
    ...
    if (!pg->is_clean() || pg->snap_trimq.empty()) {
...
boost::statechart::result PrimaryLogPG::AwaitAsyncWork::react(const DoSnapWork&)
{
    ...
    ceph_assert(max > 0);
    to_trim.reserve(max);
    int r = pg->snap_mapper.get_next_objects_to_trim(snap_to_trim, max, &to_trim);
    if (r != 0 && r != -ENOENT) {
        lderr(pg->cct) << "get_next_objects_to_trim returned " << cpp_strerror(r) << dendl;
        ...
        if (pg->snap_trimq_repeat.count(snap_to_trim)) {
            ...
            pg->recovery_state.adjust_purged_snaps(
                [snap_to_trim](auto& purged_snaps) { purged_snaps.insert(snap_to_trim); });
            pg->write_if_dirty(t);

            ldout(pg->cct, 10) << "purged_snaps now " << pg->info.purged_snaps << ", snap_trimq now " << pg->snap_trimq
                               << dendl;
    ...
    for (auto&& object : to_trim) {
        ...
        ctx->register_on_success([pg, object, &in_flight]() {
...
void PrimaryLogPG::setattr_maybe_cache(ObjectContextRef obc, PGTransaction* t, const string& key, bufferlist& val)
{
    t->setattr(obc->obs.oi.soid, key, val);
}

void PrimaryLogPG::setattrs_maybe_cache(ObjectContextRef obc, PGTransaction* t, map<string, bufferlist, less<>>& attrs)
{
    t->setattrs(obc->obs.oi.soid, attrs);
}

void PrimaryLogPG::rmattr_maybe_cache(ObjectContextRef obc, PGTransaction* t, const string& key)
{
    t->rmattr(obc->obs.oi.soid, key);
}

int PrimaryLogPG::getattr_maybe_cache(ObjectContextRef obc, const string& key, bufferlist* val)
{
    if (pool.info.is_erasure()) {
...
int PrimaryLogPG::getattrs_maybe_cache(ObjectContextRef obc, map<string, bufferlist, less<>>* out)
{
    int r = 0;
    ceph_assert(out);
    if (pool.info.is_erasure()) {
