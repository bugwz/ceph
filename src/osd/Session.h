...
struct Backoff : public RefCountedObject
{
    ...
    bool is_new() const { return state.load() == STATE_NEW; }
    bool is_acked() const { return state.load() == STATE_ACKED; }
    bool is_deleting() const { return state.load() == STATE_DELETING; }
    ...
    {
        return out << "Backoff(" << &b << " " << b.pgid << " " << b.id << " " << b.get_state_name() << " [" << b.begin
                   << "," << b.end << ") "
                   << " session " << b.session << " pg " << b.pg << ")";
    }

private:
    FRIEND_MAKE_REF(Backoff);
    Backoff(spg_t pgid, PGRef pg, ceph::ref_t<Session> s, uint64_t i, const hobject_t& b, const hobject_t& e)
...
struct Session : public RefCountedObject
{
    ...
    entity_addr_t& get_peer_socket_addr() { return socket_addr; }

    void ack_backoff(CephContext* cct, spg_t pgid, uint64_t id, const hobject_t& start, const hobject_t& end);
    ...
    bool check_backoff(CephContext* cct, spg_t pgid, const hobject_t& oid, const Message* m);
