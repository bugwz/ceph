...
{
    if (with_priority == Scrub::scrub_prio_t::high_priority) {
        suggested_priority = std::max(suggested_priority, (unsigned int)m_pg->cct->_conf->osd_client_op_priority);
    }
...
void PgScrubber::reg_next_scrub(const requested_scrub_t& request_flags)
{
    ...
    dout(10) << __func__ << " planned: must? " << request_flags.must_scrub << " need-auto? " << request_flags.need_auto
    ...
    else if (m_pg->info.stats.stats_invalid && m_pg->cct->_conf->osd_scrub_invalid_stats) {
    ...
    dout(15) << __func__ << " pg(" << m_pg_id << ") must: " << must << " required:" << m_flags.required
             << " flags: " << request_flags << " stamp: " << reg_stamp << dendl;

    const double scrub_min_interval = m_pg->pool.info.opts.value_or(pool_opts_t::SCRUB_MIN_INTERVAL, 0.0);
    const double scrub_max_interval = m_pg->pool.info.opts.value_or(pool_opts_t::SCRUB_MAX_INTERVAL, 0.0);

    // note the sched_time, so we can locate this scrub, and remove it later
    m_scrub_reg_stamp = m_osds->reg_pg_scrub(m_pg->info.pgid, reg_stamp, scrub_min_interval, scrub_max_interval, must);
    dout(15) << __func__ << " pg(" << m_pg_id << ") register next scrub, scrub time " << m_scrub_reg_stamp
             << ", must = " << (int)must << dendl;
}
...
void PgScrubber::scrub_requested(scrub_level_t scrub_level, scrub_type_t scrub_type, requested_scrub_t& req_flags)
{
    dout(10) << __func__ << (scrub_level == scrub_level_t::deep ? " deep " : " shallow ")
             ...
             << " prev stamp: " << m_scrub_reg_stamp << " " << is_scrub_registered() << dendl;
...
bool PgScrubber::select_range()
{
     ...
     */
    int min_idx = std::max<int64_t>(3, m_pg->get_cct()->_conf->osd_scrub_chunk_min / preemption_data.chunk_divisor());

    int max_idx =
        std::max<int64_t>(min_idx, m_pg->get_cct()->_conf->osd_scrub_chunk_max / preemption_data.chunk_divisor());
    ...
    int ret = m_pg->get_pgbackend()->objects_list_partial(start, min_idx, max_idx, &objects, &candidate_end);
    ceph_assert(ret >= 0);
        ...
        if (hoid.snap < CEPH_MAXSNAP) {
            ...
            if (r == -ENOENT || cur_snaps != obj_snaps) {
                ...
                if (r == 0) {
                    ...
                    m_pg->osd->clog->error() << "osd." << m_pg->osd->whoami << " found snap mapper error on pg "
                                             << m_pg->info.pgid << " oid " << hoid << " snaps in mapper: " << cur_snaps
                                             << ", oi: " << obj_snaps << "...repaired";
                }
                else {
                    m_pg->osd->clog->error()
                        << "osd." << m_pg->osd->whoami << " found snap mapper error on pg " << m_pg->info.pgid
                        << " oid " << hoid << " snaps missing in mapper"
                        << ", should be: " << obj_snaps << " was " << cur_snaps << " r " << r << "...repaired";
                }
...
int PgScrubber::build_primary_map_chunk()
{
    ...
    auto ret = build_scrub_map_chunk(m_primary_scrubmap, m_primary_scrubmap_pos, m_start, m_end, m_is_deep);
...
void PgScrubber::replica_scrub_op(OpRequestRef op)
{
    ...
    if (is_queued_or_active()) {
        ...
        //  - reset the FSM.
        m_osds->clog->warn() << __func__ << ": error: a second scrub-op received while handling the previous one";

        scrub_clear_state();
        m_osds->clog->warn() << __func__ << ": after a reset. Now handling the new OP";
    }
...
void PgScrubber::scrub_compare_maps()
{
    ...
    if (m_pg->recovery_state.get_actingset().size() > 1) {
        ...
        dout(2) << __func__ << ": primary (" << m_pg->get_primary() << ") has " << m_primary_scrubmap.objects.size()
                << " items" << dendl;
...
{
    dout(10) << __func__ << ": checking authoritative (mode=" << m_mode_desc
             << ", auth remaining #: " << m_authoritative.size() << ")" << dendl;

    // authoritative only store objects which are missing or inconsistent.
    if (!m_authoritative.empty()) {

        stringstream ss;
        ss << m_pg->info.pgid << " " << m_mode_desc << " " << m_missing.size() << " missing, " << m_inconsistent.size()
...

    {
        ...
        m_pg->recovery_state.update_stats(
            [this](auto& history, auto& stats) {
                ...
                if (m_is_deep) {
                    if ((m_shallow_errors == 0) && (m_deep_errors == 0)) history.last_clean_scrub_stamp = now;
                    ...
                    if (m_shallow_errors == 0) history.last_clean_scrub_stamp = now;
                }
...
void ReplicaReservations::release_replica(pg_shard_t peer, epoch_t epoch)
{
    auto m = new MOSDScrubReserve(
        spg_t(m_pg->info.pgid.pgid, peer.shard), epoch, MOSDScrubReserve::RELEASE, m_pg->pg_whoami);
    ...
    , m_pending{static_cast<int>(m_acting_set.size()) - 1}
{
...

    {
        ...
        prefix << "osd." << m_osds->whoami << " ep: " << epoch << " scrubber::ReplicaReservations pg[" << pg->pg_id
               << "]: ";
    ...
    if (m_pending <= 0) {
        ...
        for (auto p : m_acting_set) {
            ...
            auto m = new MOSDScrubReserve(
                spg_t(m_pg->info.pgid.pgid, p.shard), epoch, MOSDScrubReserve::REQUEST, m_pg->pg_whoami);
