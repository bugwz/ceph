...
void OSDMapMapping::_init_mappings(const OSDMap& osdmap)
{
    ...
    for (auto& p : osdmap.get_pools()) {
        ...
        if (q != pools.end() && q->first == p.first) {
            if (q->second.pg_num != p.second.get_pg_num() || q->second.size != p.second.get_size()) {
                // pg_num changed
                q = pools.erase(q);
            }
            else {
        ...
        pools.emplace(p.first, PoolMapping(p.second.get_size(), p.second.get_pg_num(), p.second.is_erasure()));
    }
...
void OSDMapMapping::_dump()
{
    for (auto& p : pools) {
        ...
        for (unsigned i = 0; i < p.second.table.size(); ++i) {
            ...
            if (i % p.second.row_size() == p.second.row_size() - 1) std::cout << std::endl;
        }
    }
}

void OSDMapMapping::_update_range(const OSDMap& osdmap, int64_t pool, unsigned pg_begin, unsigned pg_end)
{
    ...
    for (unsigned ps = pg_begin; ps < pg_end; ++ps) {
        ...
        osdmap.pg_to_up_acting_osds(pg_t(ps, pool), &up, &up_primary, &acting, &acting_primary);
...
{
    ldout(m->cct, 20) << __func__ << " " << i->job << " pool " << i->pool << " [" << i->begin << "," << i->end << ")"
                      << " pgs " << i->pgs << dendl;
    if (!i->pgs.empty())
        i->job->process(i->pgs);
    else
...
void ParallelPGMapper::queue(Job* job, unsigned pgs_per_item, const vector<pg_t>& input_pgs)
{
    bool any = false;
    if (!input_pgs.empty()) {
        unsigned i = 0;
