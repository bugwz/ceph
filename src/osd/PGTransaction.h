...
public:
    ...
    public:
        ...
        bool is_delete() const { return boost::get<Init::None>(&init_type) != nullptr && delete_first; }
        bool is_none() const { return boost::get<Init::None>(&init_type) != nullptr && !delete_first; }
        bool is_fresh_object() const { return boost::get<Init::None>(&init_type) == nullptr; }
        bool is_rename() const { return boost::get<Init::Rename>(&init_type) != nullptr; }
        bool has_source(hobject_t* source = nullptr) const
        {
            return match(
                init_type,
                [&](const Init::Clone& op) -> bool {
                    if (source) *source = op.source;
                    return true;
                },
    ...
    private:
        struct SplitMerger
        {
            BufferUpdateType split(uint64_t offset, uint64_t len, const BufferUpdateType& bu) const
            {
                return match(
                    bu,
                    ...
                    [&](const BufferUpdate::Zero&) -> BufferUpdateType { return BufferUpdate::Zero{len}; },
            ...
            uint64_t length(const BufferUpdateType& left) const
            {
                return match(
                    left,
                    [&](const BufferUpdate::Write& w) -> uint64_t { return w.buffer.length(); },
                    [&](const BufferUpdate::Zero& z) -> uint64_t { return z.len; },
                    [&](const BufferUpdate::CloneRange& c) -> uint64_t { return c.len; });
            }
            bool can_merge(const BufferUpdateType& left, const BufferUpdateType& right) const
            {
                return match(
                    ...
                    [&](const BufferUpdate::CloneRange& c) -> bool { return false; });
            }
...
private:
    ...
    ObjectOperation& get_object_op(const hobject_t& hoid) { return op_map[hoid]; }
...
public:
    ...
    void clone(const hobject_t& target,   ///< [in] obj to clone to
    ...
    void rename(const hobject_t& target,   ///< [in] to, must not exist, be non-temp
    ...
    )
    {
        ...
        op.updated_snaps = make_pair(old_snaps, new_snaps);
    }

    /// Clears, truncates
    void omap_clear(const hobject_t& hoid   ///< [in] object to clear omap
    )
    {
        auto& op = get_object_op_for_modify(hoid);
        op.clear_omap = true;
    ...
    void truncate(const hobject_t& hoid,   ///< [in] object
    ...
    /// Attr ops
    void setattrs(const hobject_t& hoid,                                          ///< [in] object to write
    ...
    void setattr(const hobject_t& hoid,         ///< [in] object to write
    ...
    void rmattr(const hobject_t& hoid,        ///< [in] object to write
    ...
    /// set alloc hint
    void set_alloc_hint(const hobject_t& hoid,           ///< [in] object (must exist)
                        uint64_t expected_object_size,   ///< [in]
                        uint64_t expected_write_size, uint32_t flags)
    {
        auto& op = get_object_op_for_modify(hoid);
        op.alloc_hint = ObjectOperation::alloc_hint_t{expected_object_size, expected_write_size, flags};
    }

    /// Buffer updates
    void write(const hobject_t& hoid,       ///< [in] object to write
    ...
    )
    {
        ...
        op.buffer_updates.insert(off, len, ObjectOperation::BufferUpdate::Write{bl, fadvise_flags});
    }
    void clone_range(const hobject_t& from,   ///< [in] from
    ...
    )
    {
        ...
        ceph_assert(!op.updated_snaps);
        op.buffer_updates.insert(tooff, len, ObjectOperation::BufferUpdate::CloneRange{from, fromoff, len});
    }
    void zero(const hobject_t& hoid,   ///< [in] object
    ...
    /// Omap updates
    void omap_setkeys(const hobject_t& hoid,        ///< [in] object to write
                      ceph::buffer::list& keys_bl   ///< [in] encoded map<string, ceph::buffer::list>
    )
    {
        auto& op = get_object_op_for_modify(hoid);
        op.omap_updates.emplace_back(std::make_pair(ObjectOperation::OmapUpdateType::Insert, keys_bl));
    }
    void omap_setkeys(const hobject_t& hoid,                            ///< [in] object to write
    ...
    void omap_rmkeys(const hobject_t& hoid,        ///< [in] object to write
                     ceph::buffer::list& keys_bl   ///< [in] encode set<string>
    )
    {
        auto& op = get_object_op_for_modify(hoid);
        op.omap_updates.emplace_back(std::make_pair(ObjectOperation::OmapUpdateType::Remove, keys_bl));
    }
    void omap_rmkeys(const hobject_t& hoid,        ///< [in] object to write
    ...
    void omap_rmkeyrange(const hobject_t& hoid,         ///< [in] object to write
                         ceph::buffer::list& range_bl   ///< [in] encode string[2]
    )
    {
        auto& op = get_object_op_for_modify(hoid);
        op.omap_updates.emplace_back(std::make_pair(ObjectOperation::OmapUpdateType::RemoveRange, range_bl));
    }
    void omap_rmkeyrange(const hobject_t& hoid,    ///< [in] object to write
    ...
    void omap_setheader(const hobject_t& hoid,       ///< [in] object to write
    ...
    bool empty() const { return op_map.empty(); }
    ...
    void nop(const hobject_t& hoid   ///< [in] obj to which we are doing nothing
    )
    {
     ...
     */
    template<typename T> void safe_create_traverse(T&& t)
    {
