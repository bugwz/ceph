...
struct PGCreateInfo
{
    ...
    bool by_mon;
    PGCreateInfo(spg_t p, epoch_t e, const pg_history_t& h, const PastIntervals& pi, bool mon)
        : pgid(p)
        , epoch(e)
        , history(h)
        , past_intervals(pi)
...
public:
    ...
    template<class T>
    PGPeeringEvent(epoch_t epoch_sent, epoch_t epoch_requested, const T& evt_, bool req = true, PGCreateInfo* ci = 0)
    ...
    epoch_t get_epoch_sent() const { return epoch_sent; }
    epoch_t get_epoch_requested() const { return epoch_requested; }
    const boost::statechart::event_base& get_event() const { return *evt; }
...
struct MInfoRec : boost::statechart::event<MInfoRec>
{
    ...
    MInfoRec(pg_shard_t from, const pg_info_t& info, epoch_t msg_epoch, std::optional<pg_lease_t> l = {},
...
struct MNotifyRec : boost::statechart::event<MNotifyRec>
{
    ...
    void print(std::ostream* out) const
    {
        *out << "MNotifyRec " << pgid << " from " << from << " notify: " << notify << " features: 0x" << std::hex
             << features << std::dec;
    }
};
...
struct MQuery : boost::statechart::event<MQuery>
{
    ...
    void print(std::ostream* out) const
    {
        *out << "MQuery " << pgid << " from " << from << " query_epoch " << query_epoch << " query: " << query;
    }
};
...
struct MTrim : boost::statechart::event<MTrim>
{
    ...
    void print(std::ostream* out) const
    {
        *out << "MTrim epoch " << epoch << " from " << from << " shard " << shard << " trim_to " << trim_to;
    }
};
...
struct MLease : boost::statechart::event<MLease>
{
    ...
    MLease(epoch_t epoch, int from, pg_lease_t l)
        ...
        , lease(l)
    {}
    void print(std::ostream* out) const { *out << "MLease epoch " << epoch << " from osd." << from << " " << lease; }
};
...
struct MLeaseAck : boost::statechart::event<MLeaseAck>
{
    ...
    void print(std::ostream* out) const
    {
        *out << "MLeaseAck epoch " << epoch << " from osd." << from << " " << lease_ack;
    }
};
...
struct RequestBackfillPrio : boost::statechart::event<RequestBackfillPrio>
{
    ...
    void print(std::ostream* out) const
    {
        *out << "RequestBackfillPrio: priority " << priority << " primary bytes " << primary_num_bytes
...
struct RequestRecoveryPrio : boost::statechart::event<RequestRecoveryPrio>
{
    ...
    explicit RequestRecoveryPrio(unsigned prio)
        ...
        , priority(prio)
    {}
    void print(std::ostream* out) const { *out << "RequestRecoveryPrio: priority " << priority; }
};

#define TrivialEvent(T)                     \
    struct T : boost::statechart::event<T>  \
    {                                       \
        T()                                 \
            : boost::statechart::event<T>() \
        {}                                  \
        void print(std::ostream* out) const \
        {                                   \
            *out << #T;                     \
...
TrivialEvent(NullEvt) TrivialEvent(RemoteBackfillReserved) TrivialEvent(RemoteReservationRejectedTooFull)
    TrivialEvent(RemoteReservationRevokedTooFull) TrivialEvent(RemoteReservationRevoked)
        TrivialEvent(RemoteReservationCanceled) TrivialEvent(RemoteRecoveryReserved) TrivialEvent(RecoveryDone)

            struct DeferRecovery : boost::statechart::event<DeferRecovery>
{
    float delay;
    explicit DeferRecovery(float delay)
        : delay(delay)
    {}
    void print(std::ostream* out) const { *out << "DeferRecovery: delay " << delay; }
};

struct DeferBackfill : boost::statechart::event<DeferBackfill>
{
    float delay;
    explicit DeferBackfill(float delay)
        : delay(delay)
    {}
    void print(std::ostream* out) const { *out << "DeferBackfill: delay " << delay; }
};
