...
std::ostream& ScrubBackend::logger_prefix(std::ostream* out, const ScrubBackend* t)
{
...
// for a Primary
ScrubBackend::ScrubBackend(ScrubBeListener& scrubber, PgScrubBeListener& pg, pg_shard_t i_am, bool repair,
                           scrub_level_t shallow_or_deep, const std::set<pg_shard_t>& acting)
    : m_scrubber{scrubber}
    , m_pg{pg}
    , m_pg_whoami{i_am}
    , m_repair{repair}
    ...
    , clog{m_scrubber.get_logger()}
{
    ...
    std::copy_if(acting.begin(), acting.end(), std::back_inserter(m_acting_but_me), [i_am](const pg_shard_t& shard) {
        return shard != i_am;
    });

    m_is_replicated = m_pool.info.is_replicated();
    m_mode_desc = (m_repair ? "repair"sv : (m_depth == scrub_level_t::deep ? "deep-scrub"sv : "scrub"sv));
}
...
{
    dout(15) << __func__ << ": repair state set to :" << (should_repair ? "true" : "false") << dendl;
...
void ScrubBackend::merge_to_authoritative_set()
{
    ...
    ceph_assert(this_chunk->authoritative_set.empty() && "the scrubber-backend should be empty");

    if (g_conf()->subsys.should_gather<ceph_subsys_osd, 15>()) {
        for (const auto& rpl : m_acting_but_me) {
            dout(15) << fmt::format(
                            "{}: replica {} has {} items", __func__, rpl, this_chunk->received_maps[rpl].objects.size())
                     << dendl;
        }
    }
    ...
    for (const auto& map : this_chunk->received_maps) {
        std::transform(map.second.objects.begin(),
                       ...
                       std::inserter(this_chunk->authoritative_set, this_chunk->authoritative_set.end()),
...
void ScrubBackend::decode_received_map(pg_shard_t from, const MOSDRepScrubMap& msg)
{
    ...
    dout(15) << __func__ << ": decoded map from : " << from
             << ": versions: " << this_chunk->received_maps[from].valid_through << " / " << msg.get_map_epoch()
             << dendl;
}


std::vector<snap_mapper_fix_t> ScrubBackend::replica_clean_meta(ScrubMap& repl_map, bool max_reached,
                                                                const hobject_t& start, SnapMapReaderI& snaps_getter)
{
    dout(15) << __func__ << ": REPL META # " << m_cleaned_meta_map.objects.size() << " objects" << dendl;
...
objs_fix_list_t ScrubBackend::scrub_compare_maps(bool max_reached, SnapMapReaderI& snaps_getter)
{
    ...
    return objs_fix_list_t{std::move(this_chunk->m_inconsistent_objs), scan_snaps(for_meta_scrub, snaps_getter)};
}

void ScrubBackend::omap_checks()
{
    const bool needs_omap_check =
        std::any_of(this_chunk->received_maps.begin(), this_chunk->received_maps.end(), [](const auto& m) -> bool {
...
void ScrubBackend::update_authoritative()
{
    ...
    for (const auto& [obj, peers] : this_chunk->authoritative) {
        ...
        m_cleaned_meta_map.objects.insert(*(this_chunk->received_maps[peers.back()].objects.find(obj)));
    }
}
...
{
    dout(20) << fmt::format(
                    "{}: {} (m_repair:{}) good peers tbl #: {}", __func__, m_mode_desc, m_repair, m_auth_peers.size())
             << dendl;
...
void ScrubBackend::repair_object(const hobject_t& soid, const auth_peers_t& ok_peers, const set<pg_shard_t>& bad_peers)
{
    if (g_conf()->subsys.should_gather<ceph_subsys_osd, 20>()) {
        ...
        dout(10) << fmt::format(
                        "repair_object {} bad_peers osd.{{{}}}, ok_peers osd.{{{}}}", soid, bad_peers, ok_shards)
                 << dendl;
    }
...
auth_selection_t ScrubBackend::select_auth_object(const hobject_t& ho, stringstream& errstream)
{
    ...
    for (auto& l : shards) {
        ...
        if (auth_version != eversion_t() && ret_auth.auth->second.objects[ho].digest_present &&
            shard_ret.digest.has_value() && ret_auth.auth->second.objects[ho].digest != *shard_ret.digest) {

            ret_auth.digest_match = false;
            dout(10) << fmt::format("{}: digest_match = false, {} data_digest 0x{:x} != "
                                    "data_digest 0x{:x}",
                                    __func__,
                                    ho,
        ...
        dout(20) << fmt::format("{}: {} shard {} got:{:D}", __func__, ho, l, shard_ret) << dendl;
        ...
        if (shard_ret.possible_auth == shard_as_auth_t::usable_t::not_usable) {
            ...
            errstream << m_pg_id.pgid << " shard " << l << " soid " << ho << " : " << shard_ret.error_text << "\n";
        }
        else if (shard_ret.possible_auth == shard_as_auth_t::usable_t::not_found) {

            // do not emit the returned error message to the log
            dout(15) << fmt::format("{}: {} not found on shard {}", __func__, ho, l) << dendl;
        }
        else {
            ...
            if (auth_version == eversion_t() || shard_ret.oi.version > auth_version ||
                (shard_ret.oi.version == auth_version && dcount(shard_ret.oi) > dcount(ret_auth.auth_oi))) {
    ...
    dout(10) << fmt::format(
                    "{}: selecting osd {} for obj {} with oi {}", __func__, ret_auth.auth_shard, ho, ret_auth.auth_oi)
             << dendl;
...
static inline bool dup_error_cond(bool& prev_err, bool continue_on_err, bool pred, shard_info_wrapper& si,
                                  set_sinfo_err_t sete, std::string_view msg, stringstream& errstream)
{
    if (pred) {
        (si.*sete)();
 ...
 */
static inline bool test_error_cond(bool error_pred, shard_info_wrapper& si, set_sinfo_err_t sete)
{
    if (error_pred) {
        (si.*sete)();
    }
...
shard_as_auth_t ScrubBackend::possible_auth_shard(const hobject_t& obj, const pg_shard_t& srd,
                                                  shard_info_map_t& shard_map)
{
    ...
    if (obj.is_head()) {
        ...
        if (dup_error_cond(err,
            ...
            // debug@dev only
            dout(30) << fmt::format("{} missing snap addr: {:p} shard_info: {:p} er: {:x}",
                                    __func__,
                                    (void*)&smap_obj,
                                    (void*)&shard_info,
                                    shard_info.errors)
    ...
    if (test_error_cond(smap_obj.size != logical_to_ondisk_size(oi.size),
        ...
        errstream << sep(err) << "candidate size " << smap_obj.size << " info size " << logical_to_ondisk_size(oi.size)
                  << " mismatch";
    }
...
{
    dout(10) << __func__ << ": authoritative-set #: " << this_chunk->authoritative_set.size() << dendl;

    std::for_each(this_chunk->authoritative_set.begin(), this_chunk->authoritative_set.end(), [this](const auto& ho) {
...
std::optional<std::string> ScrubBackend::compare_obj_in_maps(const hobject_t& ho)
{
    ...
    auto opt_ers = for_empty_auth_list(std::move(auths), std::move(objerrs), auth, ho, errstream);
...
std::optional<ScrubBackend::auth_and_obj_errs_t> ScrubBackend::for_empty_auth_list(std::list<pg_shard_t>&& auths,
                                                                                   ...
                                                                                   stringstream& errstream)
{
    if (auths.empty()) {
        if (obj_errors.empty()) {
            errstream << m_pg_id << " soid " << ho << " : failed to pick suitable auth object\n";
            return std::nullopt;
        }
    ...
    return ScrubBackend::auth_and_obj_errs_t{std::move(auths), std::move(obj_errors)};
}
...
void ScrubBackend::inconsistents(const hobject_t& ho, ScrubMap::object& auth_object, object_info_t& auth_oi,
                                 auth_and_obj_errs_t&& auth_n_errs, stringstream& errstream)
{
    ...
    dout(15) << fmt::format("{}: object errors #: {}  auth list #: {}  cur_missing #: {}  "
                            "cur_incon #: {}",
                            __func__,
                            object_errors.size(),
                            auth_list.size(),
    ...
    if (this_chunk->fix_digest) {
        ...
        this_chunk->missing_digest.push_back(make_pair(ho, make_pair(data_digest, omap_digest)));
    }
    ...
    if (!this_chunk->cur_inconsistent.empty() || !this_chunk->cur_missing.empty()) {
        ...
        case digest_fixing_t::if_aged:
        {
            ...
            if (age <= age_limit) {
                dout(20) << __func__ << ": missing digest but age (" << age << ") < conf (" << age_limit << ") on "
                         << ho << dendl;
                break;
            }
        }
...
ScrubBackend::digest_fixing_t ScrubBackend::should_fix_digest(const hobject_t& ho, const ScrubMap::object& auth_object,
                                                              const object_info_t& auth_oi, bool repair_flag,
                                                              stringstream& errstream)
{
    ...
    if (auth_oi.is_data_digest() && auth_object.digest_present && auth_oi.data_digest != auth_object.digest) {
        errstream << m_pg_id << " recorded data digest 0x" << std::hex << auth_oi.data_digest << " != on disk 0x"
                  << auth_object.digest << std::dec << " on " << auth_oi.soid << "\n";
        if (repair_flag) update = digest_fixing_t::force;
    }

    if (auth_oi.is_omap_digest() && auth_object.omap_digest_present && auth_oi.omap_digest != auth_object.omap_digest) {
        errstream << m_pg_id << " recorded omap digest 0x" << std::hex << auth_oi.omap_digest << " != on disk 0x"
...
ScrubBackend::auth_and_obj_errs_t ScrubBackend::match_in_shards(const hobject_t& ho, auth_selection_t& auth_sel,
    ...
    for (auto& [srd, smap] : this_chunk->received_maps) {
        ...
        if (smap.objects.count(ho)) {
            ...
            dout(20) << fmt::format(
                            ...
                            (auth_sel.shard_map[srd].only_data_digest_mismatch_info() ? "'info mismatch info'" : ""))
                     << dendl;
            if (discrep_found) {
            ...
            if (m_repair && m_is_replicated && (srd == auth_sel.auth_shard) && auth_sel.shard_map.size() > 1 &&
                auth_sel.digest_match && auth_sel.shard_map[srd].only_data_digest_mismatch_info() &&
        ...
        dout(20) << __func__ << ": (debug) soid " << ho << " : " << errstream.str() << dendl;
    }
...
bool ScrubBackend::compare_obj_details(pg_shard_t auth_shard, const ScrubMap::object& auth,
                                       const object_info_t& auth_oi, const ScrubMap::object& candidate,
                                       shard_info_wrapper& shard_result, inconsistent_obj_wrapper& obj_result,
                                       stringstream& errstream, bool has_snapset)
{
    ...
    if (auth.digest_present && candidate.digest_present && auth.digest != candidate.digest) {
    ...
    if (auth.omap_digest_present && candidate.omap_digest_present && auth.omap_digest != candidate.omap_digest) {
        fmt::format_to(std::back_inserter(out),
    ...
    // for replicated:
    if (m_is_replicated) {
        if (auth_oi.is_data_digest() && candidate.digest_present && auth_oi.data_digest != candidate.digest) {
            fmt::format_to(std::back_inserter(out),
        ...
        // for replicated:
        if (auth_oi.is_omap_digest() && candidate.omap_digest_present && auth_oi.omap_digest != candidate.omap_digest) {
            fmt::format_to(std::back_inserter(out),
...
{
    dout(10) << __func__ << " num stat obj " << m_pg.get_pg_info(ScrubberPasskey{}).stats.stats.sum.num_objects
             << dendl;
    ...
    for (auto p = map.objects.rbegin(); p != map.objects.rend(); ++p) {
        ...
        if (soid.nspace == m_conf->osd_hit_set_namespace) stat.num_objects_hit_set_archive++;
        ...
        if (!p->second.attrs.count(OI_ATTR)) {
            oi = std::nullopt;
            clog.error() << m_mode_desc << " " << m_pg_id << " " << soid << " : no '" << OI_ATTR << "' attr";
            this_chunk->m_error_counts.shallow_errors++;
            ...
            try {
                ...
                oi = std::nullopt;
                clog.error() << m_mode_desc << " " << m_pg_id << " " << soid << " : can't decode '" << OI_ATTR
                             << "' attr " << e.what();
                this_chunk->m_error_counts.shallow_errors++;
        ...
        if (oi) {
            if (logical_to_ondisk_size(oi->size) != p->second.size) {
                clog.error() << m_mode_desc << " " << m_pg_id << " " << soid << " : on disk size (" << p->second.size
                             << ") does not match object info size (" << oi->size << ") adjusted for ondisk to ("
            ...
            if (soid.nspace == m_conf->osd_hit_set_namespace) stat.num_bytes_hit_set_archive += oi->size;
        ...
        if (doing_clones(snapset, curclone)) {
            ...
            if (soid.has_snapset() || soid.get_head() != head->get_head()) {
                ...
                dout(10) << __func__ << " " << m_mode_desc << " " << m_pg_id << " new object " << soid
            ...
            missing += process_clones_to(head, snapset, target, &curclone, head_error);
        }
        ...
        if (!expected) {
            ...
            if (head && !snapset) {
                ...
                clog.error() << m_mode_desc << " " << m_pg_id << " " << soid << " : is an unexpected clone";
            }
            this_chunk->m_error_counts.shallow_errors++;
            soid_error.set_headless();
            this_chunk->m_inconsistent_objs.push_back(std::move(soid_error));
            ++soid_error_count;
            if (head && soid.get_head() == head->get_head()) head_error.set_clone(soid.snap);
            continue;
        }
        ...
        if (soid.has_snapset()) {
            ...
            if (p->second.attrs.count(SS_ATTR) == 0) {
                clog.error() << m_mode_desc << " " << m_pg_id << " " << soid << " : no '" << SS_ATTR << "' attr";
                this_chunk->m_error_counts.shallow_errors++;
                snapset = std::nullopt;
                ...
                try {
                    ...
                    snapset = std::nullopt;
                    clog.error() << m_mode_desc << " " << m_pg_id << " " << soid << " : can't decode '" << SS_ATTR
                                 << "' attr " << e.what();
                    this_chunk->m_error_counts.shallow_errors++;
            ...
            if (snapset) {
                ...
                if (!snapset->clones.empty()) {
                    ...
                    if (snapset->seq == 0) {
                        clog.error() << m_mode_desc << " " << m_pg_id << " " << soid << " : snaps.seq not set";
                        this_chunk->m_error_counts.shallow_errors++;
            ...
            if (snapset->clone_size.count(soid.snap) == 0) {
                clog.error() << m_mode_desc << " " << m_pg_id << " " << soid << " : is missing in clone_size";
                this_chunk->m_error_counts.shallow_errors++;
                soid_error.set_size_mismatch();
            }
            else {
                if (oi && oi->size != snapset->clone_size[soid.snap]) {
                    clog.error() << m_mode_desc << " " << m_pg_id << " " << soid << " : size " << oi->size
                ...
                if (snapset->clone_overlap.count(soid.snap) == 0) {
                    clog.error() << m_mode_desc << " " << m_pg_id << " " << soid << " : is missing in clone_overlap";
                    this_chunk->m_error_counts.shallow_errors++;
                    soid_error.set_size_mismatch();
                }
                else {
                    ...
                    const interval_set<uint64_t>& overlap = snapset->clone_overlap.find(soid.snap)->second;
                    bool bad_interval_set = false;
                    for (interval_set<uint64_t>::const_iterator i = overlap.begin(); i != overlap.end(); ++i) {
                        if (size < i.get_len()) {
    ...
    if (doing_clones(snapset, curclone)) {
        dout(10) << __func__ << " " << m_mode_desc << " " << m_pg_id << " No more objects while processing " << *head
                 << dendl;

        missing += process_clones_to(head, snapset, all_clones, &curclone, head_error);
    }
...
int ScrubBackend::process_clones_to(const std::optional<hobject_t>& head, const std::optional<SnapSet>& snapset,
                                    std::optional<snapid_t> target, vector<snapid_t>::reverse_iterator* curclone,
                                    inconsistent_snapset_wrapper& e)
{
    ...
    while (doing_clones(snapset, *curclone) && (!target || **curclone > *target)) {
        ...
        if (!m_incomplete_clones_allowed) {
            ...
            clog.error() << m_mode_desc << " " << m_pg_id << " " << *head << " : expected clone " << next_clone << " "
                         << m_missing << " missing";
            this_chunk->m_error_counts.shallow_errors++;
...
void ScrubBackend::log_missing(int missing, const std::optional<hobject_t>& head, const char* logged_func_name)
{
    ceph_assert(head);
    if (m_incomplete_clones_allowed) {
        dout(20) << logged_func_name << " " << m_mode_desc << " " << m_pg_id << " " << *head << " skipped " << missing
                 << " clone(s) in cache tier" << dendl;
    }
    else {
        clog.info() << m_mode_desc << " " << m_pg_id << " " << *head << " : " << missing << " missing clone(s)";
    }
}
...
std::vector<snap_mapper_fix_t> ScrubBackend::scan_snaps(ScrubMap& smap, SnapMapReaderI& snaps_getter)
{
    ...
    for (auto i = smap.objects.rbegin(); i != smap.objects.rend(); ++i) {
        ...
        if (hoid.is_head()) {
            ...
            try {
                ...
                dout(20) << fmt::format("{}: failed to decode the snapset ({})", __func__, hoid) << dendl;
                continue;
            }
            head = hoid.get_head();
            continue;
...
std::optional<snap_mapper_fix_t> ScrubBackend::scan_object_snaps(const hobject_t& hoid, const SnapSet& snapset,
  ...
  // There is the "happy path": cur_snaps == obj_snaps. Nothing to do there.
    ...
    if (!cur_snaps) {
        ...
        case result_t::code_t::backend_error:
            derr << __func__ << ": get_snaps returned " << cpp_strerror(e.backend_error) << " for " << hoid << dendl;
            ceph_abort();
        ...
        case result_t::code_t::inconsistent:
            dout(10) << __func__ << ": inconsistent snapmapper data for " << hoid << ". Recreating." << dendl;
            return snap_mapper_fix_t{snap_mapper_op_t::overwrite, hoid, obj_snaps, {}};
        default:
            dout(10) << __func__ << ": error (" << cpp_strerror(e.backend_error) << ") fetching snapmapper data for "
                     << hoid << ". Recreating." << dendl;
            return snap_mapper_fix_t{snap_mapper_op_t::overwrite, hoid, obj_snaps, {}};
        }
    ...
    if (*cur_snaps == obj_snaps) {
        dout(20) << fmt::format("{}: {}: snapset match SnapMapper's ({})", __func__, hoid, obj_snaps) << dendl;
        return std::nullopt;
    }
    ...
    dout(20) << fmt::format("{}: obj {}: was: {} updating to: {}", __func__, hoid, *cur_snaps, obj_snaps) << dendl;
    return snap_mapper_fix_t{snap_mapper_op_t::update, hoid, obj_snaps, *cur_snaps};
}
