...
#define MEV(E)                                  \
    ...
    {                                           \
        ...
        E()                                     \
        {                                       \
            if (!actv++) on_event_creation(#E); \
        }                                       \
        ~E()                                    \
        {                                       \
            if (!--actv) on_event_discard(#E);  \
        }                                       \
...
struct NotActive : sc::state<NotActive, ScrubMachine>, NamedSimply
{
    ...
    using reactions = mpl::list<sc::custom_reaction<StartScrub>,
                                ...
                                sc::custom_reaction<AfterRepairScrub>, sc::transition<StartReplica, ReplicaWaitUpdates>,
...
struct ReservingReplicas : sc::state<ReservingReplicas, ScrubMachine>, NamedSimply
{
    ...
    ~ReservingReplicas();
    using reactions =
        mpl::list<sc::custom_reaction<FullReset>,
                  // all replicas granted our resources request
                  sc::transition<RemotesReserved, ActiveScrubbing>, sc::custom_reaction<ReservationFailure>>;
...
struct ActiveScrubbing : sc::state<ActiveScrubbing, ScrubMachine, PendingTimer>, NamedSimply
{

    explicit ActiveScrubbing(my_context ctx);
    ~ActiveScrubbing();

    using reactions = mpl::list<sc::custom_reaction<InternalError>, sc::custom_reaction<FullReset>>;
...
struct NewChunk : sc::state<NewChunk, ActiveScrubbing>, NamedSimply
{
    ...
    using reactions = mpl::list<sc::transition<ChunkIsBusy, RangeBlocked>, sc::custom_reaction<SelectedChunkFree>>;
...
struct BuildMap : sc::state<BuildMap, ActiveScrubbing>, NamedSimply
{
    ...
    using reactions = mpl::list<sc::transition<IntBmPreempted, DrainReplMaps>,
                                ...
                                sc::transition<InternalSchedScrub, BuildMap>, sc::custom_reaction<IntLocalMapDone>>;
...
struct WaitReplicas : sc::state<WaitReplicas, ActiveScrubbing>, NamedSimply
{
    ...
    using reactions = mpl::list<
        // all replicas are accounted for:
        sc::custom_reaction<GotReplicas>, sc::custom_reaction<DigestUpdate>>;
...
struct WaitDigestUpdate : sc::state<WaitDigestUpdate, ActiveScrubbing>, NamedSimply
{
    ...
    using reactions = mpl::list<sc::custom_reaction<DigestUpdate>, sc::custom_reaction<ScrubFinished>,
...
struct ReplicaWaitUpdates : sc::state<ReplicaWaitUpdates, ScrubMachine>, NamedSimply
{
    ...
    using reactions = mpl::list<sc::custom_reaction<ReplicaPushesUpd>, sc::custom_reaction<FullReset>>;
...
struct ActiveReplica : sc::state<ActiveReplica, ScrubMachine>, NamedSimply
{
    ...
    using reactions = mpl::list<sc::custom_reaction<SchedReplica>, sc::custom_reaction<FullReset>>;
