...
{
    out << sjob.pgid << ",  " << sjob.schedule.scheduled_at << " dead: " << sjob.schedule.deadline << " - "
        << sjob.registration_state() << " / failure: " << sjob.resources_failure
        << " / pen. t.o.: " << sjob.penalty_timeout << " / queue state: " << ScrubQueue::qu_state_text(sjob.state);

    return out;
}

void ScrubQueue::ScrubJob::update_schedule(const ScrubQueue::scrub_schedule_t& adjusted)
{
    schedule = adjusted;
    ...
    updated = true;
    dout(10) << fmt::format(
                    "{}: pg[{}] adjusted: {:s} ({})", __func__, pgid, schedule.scheduled_at, registration_state())
             << dendl;
}

std::string ScrubQueue::ScrubJob::scheduling_state(utime_t now_is, bool is_deep_expected) const
{
    ...
    return fmt::format("{}scrub scheduled @ {:s}", (is_deep_expected ? "deep " : ""), schedule.scheduled_at);
}
...
#define dout_context (cct)
#undef dout_prefix
#define dout_prefix *_dout << "osd." << osd_service.get_nodeid() << " scrub-queue::" << __func__ << " "
...
{
    dout(15) << "removing pg[" << scrub_job->pgid << "] from OSD scrub queue" << dendl;

    qu_state_t expected_state{qu_state_t::registered};
    auto ret = scrub_job->state.compare_exchange_strong(expected_state, qu_state_t::unregistering);

    if (ret) {

        dout(10) << "pg[" << scrub_job->pgid << "] sched-state changed from " << qu_state_text(expected_state) << " to "
                 << qu_state_text(scrub_job->state) << dendl;
    }
    else {

        // job wasn't in state 'registered' coming in
        dout(5) << "removing pg[" << scrub_job->pgid << "] failed. State was: " << qu_state_text(expected_state)
                << dendl;
    }
}

void ScrubQueue::register_with_osd(ScrubJobRef scrub_job, const ScrubQueue::sched_params_t& suggested)
{
    qu_state_t state_at_entry = scrub_job->state.load();
    dout(20) << fmt::format("pg[{}] state at entry: <{:.14}>", scrub_job->pgid, state_at_entry) << dendl;
    ...
    dout(10) << fmt::format("pg[{}] sched-state changed from <{:.14}> to <{:.14}> (@{:s})",
                            scrub_job->pgid,
                            state_at_entry,
...
// look mommy - no locks!
void ScrubQueue::update_job(ScrubJobRef scrub_job, const ScrubQueue::sched_params_t& suggested)
{
...
ScrubQueue::sched_params_t ScrubQueue::determine_scrub_time(const requested_scrub_t& request_flags,
                                                            ...
                                                            const pool_opts_t& pool_conf) const
{
    ...
    dout(15) << fmt::format("suggested: {:s} hist: {:s} v:{}/{} must:{} pool-min:{} {}",
                            res.proposed_time,
...
void ScrubQueue::move_failed_pgs(utime_t now_is)
{
    ...
    for (auto job = to_scrub.begin(); job != to_scrub.end();) {
        if ((*job)->resources_failure) {
            ...
            dout(15) << "moving " << sjob->pgid << " state: " << ScrubQueue::qu_state_text(sjob->state) << dendl;
...
{
    dout(10) << " reg./pen. sizes: " << to_scrub.size() << " / " << penalized.size() << dendl;
    ...
    std::for_each(to_scrub.begin(), to_scrub.end(), [](const auto& jobref) -> void { jobref->updated = false; });
    ...
    if (res == Scrub::schedule_result_t::none_ready && !penalized_copy.empty()) {
        ...
        dout(10) << "tried the penalized. Res: " << ScrubQueue::attempt_res_text(res) << dendl;
...
{
    bool operator()(const ScrubQueue::ScrubJobRef& lhs, const ScrubQueue::ScrubJobRef& rhs) const
    {
...
// called under lock
ScrubQueue::ScrubQContainer ScrubQueue::collect_ripe_jobs(ScrubQContainer& group, utime_t time_now)
{
    ...
    std::copy_if(group.begin(), group.end(), std::back_inserter(ripes), [time_now](const auto& jobref) -> bool {
    ...
    if (g_conf()->subsys.should_gather<ceph_subsys_osd, 20>()) {
        for (const auto& jobref : group) {
            if (jobref->schedule.scheduled_at > time_now) {
                dout(20) << " not ripe: " << jobref->pgid << " @ " << jobref->schedule.scheduled_at << dendl;
            }
        }
    }
...
Scrub::schedule_result_t ScrubQueue::select_from_group(ScrubQContainer& group, const Scrub::ScrubPreconds& preconds,
                                                       utime_t now_is)
{
    ...
    for (auto& candidate : group) {
        ...
        if (preconds.only_deadlined &&
            (candidate->schedule.deadline.is_zero() || candidate->schedule.deadline >= now_is)) {
        ...
        // scrub.
        switch (osd_service.initiate_a_scrub(candidate->pgid, preconds.allow_requested_repair_only)) {
...
ScrubQueue::scrub_schedule_t ScrubQueue::adjust_target_time(const sched_params_t& times) const
{
    ScrubQueue::scrub_schedule_t sched_n_dead{times.proposed_time, times.proposed_time};

    if (times.is_must == ScrubQueue::must_scrub_t::not_mandatory) {
        // unless explicitly requested, postpone the scrub with a random delay
        double scrub_min_interval = times.min_interval > 0 ? times.min_interval : conf()->osd_scrub_min_interval;
        double scrub_max_interval = times.max_interval > 0 ? times.max_interval : conf()->osd_scrub_max_interval;

        sched_n_dead.scheduled_at += scrub_min_interval;
        double r = rand() / (double)RAND_MAX;
        sched_n_dead.scheduled_at += scrub_min_interval * conf()->osd_scrub_interval_randomize_ratio * r;
        ...
        dout(20) << fmt::format("not-must. Was:{:s} {{min:{}/{} max:{}/{} ratio:{}}} "
                                "Adjusted:{:s} ({:s})",
                                times.proposed_time,
                                fmt::group_digits(times.min_interval),
...
bool ScrubQueue::scrub_load_below_threshold() const
{
    ...
    if (loadavg_per_cpu < conf()->osd_scrub_load_threshold) {
        dout(20) << "loadavg per cpu " << loadavg_per_cpu << " < max " << conf()->osd_scrub_load_threshold << " = yes"
                 << dendl;
        return true;
    }
    ...
    if (loadavgs[0] < daily_loadavg && loadavgs[0] < loadavgs[2]) {
        dout(20) << "loadavg " << loadavgs[0] << " < daily_loadavg " << daily_loadavg << " and < 15m avg "
                 << loadavgs[2] << " = yes" << dendl;
        return true;
    }

    dout(20) << "loadavg " << loadavgs[0] << " >= max " << conf()->osd_scrub_load_threshold
...
void ScrubQueue::scan_penalized(bool forgive_all, utime_t time_now)
{
    ...
    if (forgive_all) {
        ...
        auto forgiven_last = std::partition(penalized.begin(), penalized.end(), [time_now](const auto& e) {
...
bool ScrubQueue::scrub_time_permit(utime_t now) const
{
    ...
    bool day_permit = isbetween_modulo(conf()->osd_scrub_begin_week_day, conf()->osd_scrub_end_week_day, bdt.tm_wday);
    if (!day_permit) {
        dout(20) << "should run between week day " << conf()->osd_scrub_begin_week_day << " - "
                 << conf()->osd_scrub_end_week_day << " now " << bdt.tm_wday << " - no" << dendl;
        return false;
    }

    bool time_permit = isbetween_modulo(conf()->osd_scrub_begin_hour, conf()->osd_scrub_end_hour, bdt.tm_hour);
    dout(20) << "should run between " << conf()->osd_scrub_begin_hour << " - " << conf()->osd_scrub_end_hour << " now ("
...
void ScrubQueue::dump_scrubs(ceph::Formatter* f) const
{
    ...
    std::for_each(to_scrub.cbegin(), to_scrub.cend(), [&f](const ScrubJobRef& j) { j->dump(f); });

    std::for_each(penalized.cbegin(), penalized.cend(), [&f](const ScrubJobRef& j) { j->dump(f); });
...
ScrubQueue::ScrubQContainer ScrubQueue::list_registered_jobs() const
{
    ...
    std::copy_if(to_scrub.begin(), to_scrub.end(), std::back_inserter(all_jobs), registered_job);
    std::copy_if(penalized.begin(), penalized.end(), std::back_inserter(all_jobs), registered_job);
...
bool ScrubQueue::can_inc_scrubs() const
{
    ...
    dout(20) << " == false. " << scrubs_local << " local + " << scrubs_remote << " remote >= max "
             << conf()->osd_max_scrubs << dendl;
    return false;
}
...
bool ScrubQueue::inc_scrubs_local()
{
    ...
    dout(20) << ": " << scrubs_local << " local + " << scrubs_remote << " remote >= max " << conf()->osd_max_scrubs
             << dendl;
    return false;
}
...
{
    std::lock_guard lck{resource_lock};
    dout(20) << ": " << scrubs_local << " -> " << (scrubs_local - 1) << " (max " << conf()->osd_max_scrubs
             << ", remote " << scrubs_remote << ")" << dendl;
...
bool ScrubQueue::inc_scrubs_remote()
{
    ...
    if (scrubs_local + scrubs_remote < conf()->osd_max_scrubs) {
        dout(20) << ": " << scrubs_remote << " -> " << (scrubs_remote + 1) << " (max " << conf()->osd_max_scrubs
...
{
    dout(5) << fmt::format(": pg {} is blocked on an object", blocked_pg) << dendl;
    ++blocked_scrubs_cnt;
}
