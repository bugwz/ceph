...
{
    if (sf.auto_repair) out << " AUTO_REPAIR";
    if (sf.check_repair) out << " CHECK_REPAIR";
    if (sf.deep_scrub_on_error) out << " DEEP_SCRUB_ON_ERROR";
    if (sf.required) out << " REQ_SCRUB";
...
{
    if (sf.must_repair) out << " MUST_REPAIR";
    if (sf.auto_repair) out << " planned AUTO_REPAIR";
    if (sf.check_repair) out << " planned CHECK_REPAIR";
    if (sf.deep_scrub_on_error) out << " planned DEEP_SCRUB_ON_ERROR";
    if (sf.must_deep_scrub) out << " MUST_DEEP_SCRUB";
    if (sf.must_scrub) out << " MUST_SCRUB";
    if (sf.time_for_deep) out << " TIME_FOR_DEEP";
    if (sf.need_auto) out << " NEED_AUTO";
    if (sf.req_scrub) out << " planned REQ_SCRUB";
...
bool PgScrubber::verify_against_abort(epoch_t epoch_to_verify)
{
    ...
    dout(10) << __func__ << " aborting. incoming epoch: " << epoch_to_verify << " vs last-aborted: " << m_last_aborted
             << dendl;
...
bool PgScrubber::should_abort() const
{
    ...
    else if (get_osdmap()->test_flag(CEPH_OSDMAP_NOSCRUB) || m_pg->pool.info.has_flag(pg_pool_t::FLAG_NOSCRUB)) {
...
{
    dout(10) << "scrubber event -->> " << __func__ << " epoch: " << epoch_queued << " token: " << token << dendl;
    if (is_primary()) {
...
unsigned int PgScrubber::scrub_requeue_priority(Scrub::scrub_prio_t with_priority) const
{
    ...
    if (with_priority == Scrub::scrub_prio_t::high_priority) {
        qu_priority = std::max(qu_priority, (unsigned int)m_pg->get_cct()->_conf->osd_client_op_priority);
    }
    return qu_priority;
}
...
void PgScrubber::on_primary_change(std::string_view caller, const requested_scrub_t& request_flags)
{
    if (!m_scrub_job) {
        // we won't have a chance to see more logs from this function, thus:
        dout(10) << fmt::format("{}: (from {}& w/{}) {}.Reg-state:{:.7}. No scrub-job",
                                __func__,
                                caller,
                                request_flags,
    ...
    if (is_primary()) {
        auto suggested =
            m_osds->get_scrub_services().determine_scrub_time(request_flags, m_pg->info, m_pg->get_pgpool().info.opts);
    ...
    if (is_primary() && is_scrub_active()) {
        if (m_interval_start < m_pg->get_same_interval_since()) {
            dout(10) << fmt::format("{}: interval changed ({} -> {}). Aborting active scrub.",
                                    __func__,
                                    m_interval_start,
    ...
    dout(10) << fmt::format("{} (from {} {}): {}. <{:.5}>&<{:.10}> --> <{:.5}>&<{:.14}>",
                            __func__,
                            caller,
                            request_flags,
                            (is_primary() ? "Primary" : "Replica/other"),
...
void PgScrubber::scrub_requested(scrub_level_t scrub_level, scrub_type_t scrub_type, requested_scrub_t& req_flags)
{
    dout(10) << __func__ << (scrub_level == scrub_level_t::deep ? " deep " : " shallow ")
             << (scrub_type == scrub_type_t::do_repair ? " repair-scrub " : " not-repair ")
             << " prev stamp: " << m_scrub_job->get_sched_time() << " registered? " << registration_state() << dendl;

    req_flags.must_scrub = true;
    req_flags.must_deep_scrub = (scrub_level == scrub_level_t::deep) || (scrub_type == scrub_type_t::do_repair);
...
void PgScrubber::on_applied_when_primary(const eversion_t& applied_version)
{
    ...
    if (m_fsm->is_accepting_updates() && (applied_version >= m_subset_last_update)) {
        ...
        dout(15) << __func__ << " update: " << applied_version << " vs. required: " << m_subset_last_update << dendl;
    }
}
 ...
 */
int size_from_conf(bool is_deep, const ceph::common::ConfigProxy& conf, std::string_view deep_opt,
...
bool PgScrubber::select_range()
{
    ...
    dout(20) << fmt::format("{} {} mins: {}d {}s, max: {}d {}s",
                            __func__,
                            (m_is_deep ? "D" : "S"),
    ...
    const int min_from_conf = size_from_conf(m_is_deep, conf, "osd_scrub_chunk_min", "osd_shallow_scrub_chunk_min");
    const int max_from_conf = size_from_conf(m_is_deep, conf, "osd_scrub_chunk_max", "osd_shallow_scrub_chunk_max");
    ...
    dout(10) << fmt::format("{}: Min: {} Max: {} Div: {}", __func__, min_chunk_sz, max_chunk_sz, divisor) << dendl;

    hobject_t start = m_start;
    hobject_t candidate_end;
    std::vector<hobject_t> objects;
    int ret = m_pg->get_pgbackend()->objects_list_partial(start, min_chunk_sz, max_chunk_sz, &objects, &candidate_end);
    ceph_assert(ret >= 0);
    ...
    m_end = candidate_end;
    if (m_end > m_max_end) m_max_end = m_end;
...
bool PgScrubber::write_blocked_by_scrub(const hobject_t& soid)
{
    ...
    dout(20) << __func__ << " " << soid << " can preempt? " << preemption_data.is_preemptable()
             << " already preempted? " << preemption_data.was_preempted() << dendl;
...
Scrub::BlockedRangeWarning PgScrubber::acquire_blocked_alarm()
{
    ...
    if (grace == 0) {
        ...
        dout(10) << __func__ << ": blocked-alarm disabled ('osd_blocked_scrub_grace_period' set to 0)" << dendl;
        return nullptr;
    }
    ceph::timespan grace_period{m_debug_blockrange ? 4s : seconds{grace}};
    dout(20) << fmt::format(": timeout:{}", std::chrono::duration_cast<seconds>(grace_period)) << dendl;
    return std::make_unique<blocked_range_t>(m_osds, grace_period, *this, m_pg_id);
}
...
void PgScrubber::add_delayed_scheduling()
{
    ...
    if (sleep_time.count()) {
        ...
        auto callbk = new LambdaContext([osds = m_osds, pgid, scrbr = this]([[maybe_unused]] int r) mutable {
            ...
            lgeneric_dout(scrbr->get_pg_cct(), 7)
                << "scrub_requeue_callback: slept for " << ceph_clock_now() - scrbr->m_sleep_started_at
...
eversion_t PgScrubber::search_log_for_updates() const
{
    ...
    auto pi = find_if(projected.crbegin(), projected.crend(), [this](const auto& e) -> bool {
        return e.soid >= m_start && e.soid < m_end;
    });

    if (pi != projected.crend()) return pi->version;

    // there was no relevant update entry in the log

    auto& log = m_pg->recovery_state.get_pg_log().get_log().log;
    auto p = find_if(
        log.crbegin(), log.crend(), [this](const auto& e) -> bool { return e.soid >= m_start && e.soid < m_end; });
...
{
    dout(10) << __func__ << " started in epoch/interval: " << m_epoch_start << "/" << m_interval_start
...
void PgScrubber::update_op_mode_text()
{
    ...
    dout(10) << __func__ << ": repair: visible: " << (visible_repair ? "true" : "false")
             << ", internal: " << (m_is_repair ? "true" : "false") << ". Displayed: " << m_mode_desc << dendl;
}

void PgScrubber::_request_scrub_map(pg_shard_t replica, eversion_t version, hobject_t start, hobject_t end, bool deep,
...
void PgScrubber::on_init()
{
    ...
    m_be = std::make_unique<ScrubBackend>(*this,
                                          *m_pg,
                                          m_pg_whoami,
                                          m_is_repair,
...
{
    dout(10) << __func__ << " called with 'active' " << (m_active ? "set" : "cleared") << dendl;
    if (!m_active) {
        m_be = std::make_unique<ScrubBackend>(
            *this, *m_pg, m_pg_whoami, m_is_repair, m_is_deep ? scrub_level_t::deep : scrub_level_t::shallow);
        m_active = true;
        ++m_sessions_counter;
    }
}
...
int PgScrubber::build_primary_map_chunk()
{
    ...
    auto ret = build_scrub_map_chunk(m_be->get_primary_scrubmap(), m_primary_scrubmap_pos, m_start, m_end, m_is_deep);
...
{
    dout(10) << __func__ << " interval start: " << m_interval_start << " current token: " << m_current_token
             << " epoch: " << m_epoch_start << " deep: " << m_is_deep << dendl;

    ceph_assert(m_be);

    auto ret = build_scrub_map_chunk(replica_scrubmap, replica_scrubmap_pos, m_start, m_end, m_is_deep);
    ...
    case -EINPROGRESS:
        ...
        m_osds->queue_for_rep_scrub_resched(m_pg, m_replica_request_priority, m_flags.priority, m_current_token);
        break;
    ...
    case 0:
    {
        ...
        auto required_fixes =
            m_be->replica_clean_meta(replica_scrubmap, m_end.is_max(), m_start, get_snap_mapper_accessor());
...
int PgScrubber::build_scrub_map_chunk(ScrubMap& map, ScrubMapBuilder& pos, hobject_t start, hobject_t end, bool deep)
{
...
void PgScrubber::repair_oinfo_oid(ScrubMap& smap)
{
    for (auto i = smap.objects.rbegin(); i != smap.objects.rend(); ++i) {
        ...
        if (oi.soid != hoid) {
            ...
            m_osds->clog->error() << "osd." << m_pg_whoami << " found object info error on pg " << m_pg_id << " oid "
...
void PgScrubber::apply_snap_mapper_fixes(const std::vector<snap_mapper_fix_t>& fix_list)
{
    ...
    for (auto& [fix_op, hoid, snaps, bogus_snaps] : fix_list) {
        ...
        if (fix_op != snap_mapper_op_t::add) {
            ...
            m_osds->clog->error() << fmt::format("osd.{} found snap mapper error on pg {} oid {} snaps in mapper: {}, "
                                                 "oi: "
                                                 "{} ...repaired",
                                                 m_pg_whoami,
                                                 m_pg_id,
            ...
            m_osds->clog->error() << fmt::format("osd.{} found snap mapper error on pg {} oid {} snaps missing in "
    ...
    // wait for repair to apply to avoid confusing other bits of the system.
    {
        ...
        if (e = m_pg->osd->store->queue_transaction(m_pg->ch, std::move(t)); e != 0) {
...
void PgScrubber::maps_compare_n_cleanup()
{
    ...
    auto required_fixes = m_be->scrub_compare_maps(m_end.is_max(), get_snap_mapper_accessor());
...
void PgScrubber::replica_scrub_op(OpRequestRef op)
{
    ...
    if (is_queued_or_active()) {
        ...
        //  - reset the FSM.
        m_osds->clog->warn() << fmt::format("{}: error: a second scrub-op received while handling the previous one",
                                            __func__);

        scrub_clear_state();
        m_osds->clog->warn() << fmt::format("{}: after a reset. Now handling the new OP", __func__);
    }
    ...
    m_replica_request_priority =
        msg->high_priority ? Scrub::scrub_prio_t::high_priority : Scrub::scrub_prio_t::low_priority;
...
void PgScrubber::set_op_parameters(const requested_scrub_t& request)
{
    ...
    m_flags.priority = (request.must_scrub || request.need_auto) ? get_pg_cct()->_conf->osd_requested_scrub_priority
...
ScrubMachineListener::MsgAndEpoch PgScrubber::prep_replica_map_msg(PreemptionNoted was_preempted)
{
    dout(10) << __func__ << " min epoch:" << m_replica_min_epoch << dendl;

    auto reply = make_message<MOSDRepScrubMap>(
        spg_t(m_pg->info.pgid.pgid, m_pg->get_primary().shard), m_replica_min_epoch, m_pg_whoami);
...
{
    m_pg->send_cluster_message(m_pg->get_primary().osd, preprepared.m_msg, preprepared.m_epoch, false);
}
...
void PgScrubber::handle_scrub_reserve_request(OpRequestRef op)
{
    ...
    dout(20) << fmt::format("{}: request_ep:{} recovery:{}", __func__, request_ep, m_osds->is_recovery_active())
             << dendl;
    ...
    else if (m_pg->cct->_conf->osd_scrub_during_recovery || !m_osds->is_recovery_active()) {
    ...
    Message* reply = new MOSDScrubReserve(spg_t(m_pg->info.pgid.pgid, m_pg->get_primary().shard),
                                          request_ep,
...
void PgScrubber::handle_scrub_reserve_grant(OpRequestRef op, pg_shard_t from)
{
    ...
    if (m_reservations.has_value()) {
        ...
        dout(20) << __func__ << ": late/unsolicited reservation grant from osd " << from << " (" << op << ")" << dendl;
    }
}
...
void PgScrubber::message_all_replicas(int32_t opcode, std::string_view op_text)
{
    ...
    for (auto& p : m_pg->get_actingset()) {
        ...
        if (p == m_pg_whoami) continue;

        dout(10) << "scrub requesting " << op_text << " from osd." << p << " Epoch: " << epch << dendl;
        Message* m = new MOSDScrubReserve(spg_t(m_pg->info.pgid.pgid, p.shard), epch, opcode, m_pg_whoami);
...
void PgScrubber::scrub_finish()
{
    ...
    if (m_is_repair && m_flags.auto_repair &&
        m_be->authoritative_peers_count() > static_cast<int>(m_pg->cct->_conf->osd_scrub_auto_repair_num_errors)) {
...

    {
        ...
        if (m_is_repair) oss << ", " << m_fixed_count << " fixed";
        if (total_errors)
    ...
    if (m_is_repair) {
        dout(15) << fmt::format(
                        "{}: {} errors. {} errors fixed", __func__, m_shallow_errors + m_deep_errors, m_fixed_count)
                 << dendl;
        if (m_fixed_count == m_shallow_errors + m_deep_errors) {
            ...
            dout(20) << __func__ << " Current 'required': " << m_flags.required
                     << " Planned 'req_scrub': " << m_planned_scrub.req_scrub << dendl;
        }
...

    {
        ...
        m_pg->recovery_state.update_stats(
            [this](auto& history, auto& stats) {
                dout(10) << "m_pg->recovery_state.update_stats() errors:" << m_shallow_errors << "/" << m_deep_errors
 ...
 * thus transitory.
 */
void PgScrubber::dump_scrubber(ceph::Formatter* f, const requested_scrub_t& request_flags) const
{
    ...
    if (m_active) {   // TBD replace with PR#42780's test
        ...
        f->dump_bool("must_scrub", (m_planned_scrub.must_scrub || m_flags.required));
        ...
        bool deep_expected = (ceph_clock_now() >= m_pg->next_deepscrub_interval()) || request_flags.must_deep_scrub ||
                             request_flags.need_auto;
        auto sched_state = m_scrub_job->scheduling_state(ceph_clock_now(), deep_expected);
...
pg_scrubbing_status_t PgScrubber::get_schedule() const
{
    ...
    dout(25) << fmt::format("{}: active:{} blocked:{}", __func__, m_active, m_scrub_job->blocked) << dendl;
    ...
    if (m_active) {
        ...
        if (m_scrub_job->blocked) {
            ...
            int32_t blocked_for = (utime_t{now_is} - m_scrub_job->blocked_since).sec();
    ...
    if (m_scrub_job->state != ScrubQueue::qu_state_t::registered) {
        return pg_scrubbing_status_t{
            utime_t{}, 0, pg_scrub_sched_status_t::not_queued, false, scrub_level_t::shallow, false};
    }

    // Will next scrub surely be a deep one? note that deep-scrub might be
    // selected even if we report a regular scrub here.
    bool deep_expected =
        (now_is >= m_pg->next_deepscrub_interval()) || m_planned_scrub.must_deep_scrub || m_planned_scrub.need_auto;
    scrub_level_t expected_level = deep_expected ? scrub_level_t::deep : scrub_level_t::shallow;
    bool periodic = !m_planned_scrub.must_scrub && !m_planned_scrub.need_auto && !m_planned_scrub.must_deep_scrub;

    // are we ripe for scrubbing?
    if (now_is > m_scrub_job->schedule.scheduled_at) {
        // we are waiting for our turn at the OSD.
        return pg_scrubbing_status_t{
            m_scrub_job->schedule.scheduled_at, 0, pg_scrub_sched_status_t::queued, false, expected_level, periodic};
    }

    return pg_scrubbing_status_t{
        m_scrub_job->schedule.scheduled_at, 0, pg_scrub_sched_status_t::scheduled, false, expected_level, periodic};
}
    ...
    , preemption_data{pg}
{
    ...
    m_scrub_job = ceph::make_ref<ScrubQueue::ScrubJob>(m_osds->cct, m_pg->pg_id, m_osds->get_nodeid());
}

void PgScrubber::set_scrub_begin_time()
{
    scrub_begin_stamp = ceph_clock_now();
    m_osds->clog->debug() << fmt::format("{} {} starts", m_pg->info.pgid.pgid, m_mode_desc);
}
...
{
    dout(10) << __func__ << dendl;
    m_reservations.emplace(m_pg, m_pg_whoami, m_scrub_job, m_pg->get_cct()->_conf);
}
...
bool PgScrubber::is_token_current(Scrub::act_token_t received_token)
{
    ...
    dout(5) << __func__ << " obsolete token (" << received_token << " vs current " << m_current_token << dendl;
...
LoggerSinkSet& PgScrubber::get_logger() const
{
    return *m_osds->clog.get();
}
...
int PgScrubber::asok_debug(std::string_view cmd, std::string param, Formatter* f, stringstream& ss)
{
...
void PgScrubber::update_scrub_stats(ceph::coarse_real_clock::time_point now_is)
{
    ...
    const seconds period_active = seconds(m_pg->get_cct()->_conf.get_val<int64_t>("osd_stats_update_period_scrubbing"));
    if (!period_active.count()) {
        // a way for the operator to disable these stats updates
        return;
    }
    const seconds period_inactive = seconds(
        m_pg->get_cct()->_conf.get_val<int64_t>("osd_stats_update_period_not_scrubbing") + m_pg_id.pgid.m_seed % 30);
    ...
    if (g_conf()->subsys.should_gather<ceph_subsys_osd, 20>()) {
        ...
        dout(20) << fmt::format("{}: period: {}/{}-> {} last:{}", __func__, period_active, period_inactive, period, buf)
                 << dendl;
    }
...
PgScrubber::preemption_data_t::preemption_data_t(PG* pg)
    : m_pg{pg}
{
    m_left = static_cast<int>(m_pg->get_cct()->_conf.get_val<uint64_t>("osd_scrub_max_preemptions"));
}
...
void PgScrubber::preemption_data_t::reset()
{
    ...
    m_preemptable = false;
    m_preempted = false;
    m_left = static_cast<int>(m_pg->cct->_conf.get_val<uint64_t>("osd_scrub_max_preemptions"));
    m_size_divisor = 1;
}
...
{
    auto m =
        new MOSDScrubReserve(spg_t(m_pg_info.pgid.pgid, peer.shard), epoch, MOSDScrubReserve::RELEASE, m_pg->pg_whoami);
    ...
    , m_conf{conf}
{
    ...
    m_timeout = conf.get_val<std::chrono::milliseconds>("osd_scrub_slow_reservation_response");
    m_log_msg_prefix =
        fmt::format("osd.{} ep: {} scrubber::ReplicaReservations pg[{}]: ", m_osds->whoami, epoch, pg->pg_id);
    ...
    if (m_pending <= 0) {
        ...
        m_no_reply = make_unique<ReplicaReservations::no_reply_t>(m_osds, m_conf, *this, m_log_msg_prefix);

        // send the reservation requests
        for (auto p : m_acting_set) {
            if (p == whoami) continue;
            auto m = new MOSDScrubReserve(
                spg_t(m_pg_info.pgid.pgid, p.shard), epoch, MOSDScrubReserve::REQUEST, m_pg->pg_whoami);
 ...
 */
std::optional<ReplicaReservations::tpoint_t> ReplicaReservations::update_latecomers(tpoint_t now_is)
{
...
void ReplicaReservations::handle_reserve_grant(OpRequestRef op, pg_shard_t from)
{
...

    {
        ...
        if (w != m_waited_for_peers.end()) m_waited_for_peers.erase(w);
    }

    // are we forced to reject the reservation?
    if (m_had_rejections) {

        dout(10) << __func__ << ": rejecting late-coming reservation from " << from << dendl;
        release_replica(from, m_pg->get_osdmap_epoch());
    }
    else if (std::find(m_reserved_peers.begin(), m_reserved_peers.end(), from) != m_reserved_peers.end()) {

        dout(10) << __func__ << ": already had osd." << from << " reserved" << dendl;
    }
    else {

        dout(10) << __func__ << ": osd." << from << " scrub reserve = success" << dendl;
        ...
        if (m_timeout_point && (now_is > *m_timeout_point)) {
            m_osds->clog->warn() << fmt::format(
                "osd.{} scrubber pg[{}]: late reservation from osd.{}", m_osds->whoami, m_pg->pg_id, from);
...
void ReplicaReservations::handle_reserve_reject(OpRequestRef op, pg_shard_t from)
{
    ...
    if (m_had_rejections) {
        ...
        dout(15) << __func__ << ": ignoring late-coming rejection from " << from << dendl;
    }
    else if (std::find(m_reserved_peers.begin(), m_reserved_peers.end(), from) != m_reserved_peers.end()) {
...
{
    dout(1) << fmt::format("{}: timeout! no reply from {}", __func__, m_waited_for_peers) << dendl;
...
ReplicaReservations::no_reply_t::no_reply_t(OSDService* osds, const ConfigProxy& conf, ReplicaReservations& parent,
    ...
    , m_log_prfx{log_prfx}
{
    using namespace std::chrono;
    auto now_is = clock::now();
    auto timeout = conf.get_val<std::chrono::milliseconds>("osd_scrub_reservation_timeout");

    m_abort_callback = new LambdaContext([this, now_is]([[maybe_unused]] int r) {
        // behave as if a REJECT was received
        m_osds->clog->warn() << fmt::format("{} timeout on replica reservations (since {})", m_log_prfx, now_is);
...
ReservedByRemotePrimary::ReservedByRemotePrimary(const PgScrubber* scrubber, PG* pg, OSDService* osds, epoch_t epoch)
    : m_scrubber{scrubber}
    , m_pg{pg}
    , m_osds{osds}
    , m_reserved_at{epoch}
...
auto MapsCollectionStatus::mark_arriving_map(pg_shard_t from) -> std::tuple<bool, std::string_view>
{
...
blocked_range_t::blocked_range_t(OSDService* osds, ceph::timespan waittime, ScrubMachineListener& scrubber, spg_t pg_id)
    : m_osds{osds}
    , m_scrubber{scrubber}
    , m_pgid{pg_id}
{
    ...
    m_callbk = new LambdaContext([this, now_is]([[maybe_unused]] int r) {
        ...
        lgeneric_subdout(g_ceph_context, osd, 10)
            << "PgScrubber: " << m_pgid << " blocked on an object for too long (since " << buf << ")" << dendl;
        m_osds->clog->warn() << "osd." << m_osds->whoami << " PgScrubber: " << m_pgid
