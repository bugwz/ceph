...
public:
     ...
     * reason.
     */
    virtual schedule_result_t initiate_a_scrub(spg_t pgid, bool allow_requested_repair_only) = 0;
...
public:
    ...
    struct ScrubJob final : public RefCountedObject
    {
         ...
         */
        std::string_view state_desc() const { return ScrubQueue::qu_state_text(state.load(std::memory_order_relaxed)); }
        ...
        {
            return in_queues.load(std::memory_order_relaxed) ? "in-queue" : "not-queued";
        }
    ...
    sched_params_t determine_scrub_time(const requested_scrub_t& request_flags, const pg_info_t& pg_info,
...
private:
     ...
     */
    scrub_schedule_t adjust_target_time(const sched_params_t& recomputed_params) const;
    ...
    Scrub::schedule_result_t select_from_group(ScrubQContainer& group, const Scrub::ScrubPreconds& preconds,
                                               utime_t now_is);
...
template<> struct fmt::formatter<ScrubQueue::qu_state_t> : fmt::formatter<std::string_view>
{
    template<typename FormatContext> auto format(const ScrubQueue::qu_state_t& s, FormatContext& ctx)
    {
        auto out = ctx.out();
        out = fmt::formatter<string_view>::format(std::string{ScrubQueue::qu_state_text(s)}, ctx);
        return out;
    }
};

template<> struct fmt::formatter<ScrubQueue::ScrubJob>
{
    constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

    template<typename FormatContext> auto format(const ScrubQueue::ScrubJob& sjob, FormatContext& ctx)
    {
