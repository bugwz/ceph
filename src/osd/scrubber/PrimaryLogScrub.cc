...
void PrimaryLogScrub::submit_digest_fixes(const digests_fixes_t& fixes)
{
    ...
    dout(10) << __func__ << ": num_digest_updates_pending: " << num_digest_updates_pending << dendl;

    for (auto& [obj, dgs] : fixes) {

        ObjectContextRef obc = m_pl_pg->get_object_context(obj, false);
        if (!obc) {
            m_osds->clog->error() << m_pg_id << " " << m_mode_desc << " cannot get object context for object " << obj;
            num_digest_updates_pending--;
            continue;
        }
        dout(15) << fmt::format("{}: {}, pg[{}] {}/{}", __func__, num_digest_updates_pending, m_pg_id, obj, dgs)
                 << dendl;
        if (obc->obs.oi.soid != obj) {
            m_osds->clog->error() << m_pg_id << " " << m_mode_desc << " " << obj
...
void PrimaryLogScrub::_scrub_finish()
{
    ...
    dout(10) << __func__ << " info stats: " << (info.stats.stats_invalid ? "invalid" : "valid")
