...
class ReplicaReservations
{
    ...
    struct no_reply_t
    {
        explicit no_reply_t(OSDService* osds, const ConfigProxy& conf, ReplicaReservations& parent,
...
public:
    ...
    ReplicaReservations(PG* pg, pg_shard_t whoami, ScrubQueue::ScrubJobRef scrubjob, const ConfigProxy& conf);
...
public:
    [[nodiscard]] bool are_all_maps_available() const { return m_local_map_ready && m_maps_awaited_for.empty(); }

    void mark_local_map_ready() { m_local_map_ready = true; }

    void mark_replica_map_request(pg_shard_t from_whom) { m_maps_awaited_for.push_back(from_whom); }
 ...
 */
class PgScrubber : public ScrubPgIF, public ScrubMachineListener, public ScrubBeListener
{
public:
    ...
    void scrub_requested(scrub_level_t scrub_level, scrub_type_t scrub_type, requested_scrub_t& req_flags) final;
    ...
    void dump_scrubber(ceph::Formatter* f, const requested_scrub_t& request_flags) const final;
    ...
    Scrub::scrub_prio_t replica_op_priority() const final { return m_replica_request_priority; };

    unsigned int scrub_requeue_priority(Scrub::scrub_prio_t with_priority, unsigned int suggested_priority) const final;
     ...
     */
    void stats_of_handled_objects(const object_stat_sum_t& delta_stats, const hobject_t& soid) override
    {
        ceph_assert(false);
    }
    ...
    bool get_store_errors(const scrub_ls_arg_t& arg, scrub_ls_result_t& res_inout) const override { return false; }

    void update_scrub_stats(ceph::coarse_real_clock::time_point now_is) final;

    int asok_debug(std::string_view cmd, std::string param, Formatter* f, std::stringstream& ss) override;
    ...
    void set_state_name(const char* name) final { m_fsm_state_name = name; }
    ...
    eversion_t get_last_update_applied() const final { return m_pg->recovery_state.get_last_update_applied(); }
    ...
    ScrubMachineListener::MsgAndEpoch prep_replica_map_msg(Scrub::PreemptionNoted was_preempted) final;
    ...
    Scrub::SnapMapReaderI& get_snap_mapper_accessor() { return m_pg->snap_mapper; }
...
protected:
    ...
    int build_scrub_map_chunk(ScrubMap& map,   // primary or replica?
                              ScrubMapBuilder& pos, hobject_t start, hobject_t end, bool deep);
...
private:
    ...
    void _request_scrub_map(pg_shard_t replica, eversion_t version, hobject_t start, hobject_t end, bool deep,
    ...
    void apply_snap_mapper_fixes(const std::vector<Scrub::snap_mapper_fix_t>& fix_list);
    ...
    public:
        ...
        {
            if (m_preempted || !m_preemptable) return false;

            std::lock_guard<ceph::mutex> lk{m_preemption_lock};
            if (!m_preemptable) return false;
