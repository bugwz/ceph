...
void PG::send_cluster_message(int target, MessageRef m, epoch_t epoch, bool share_map_update = false)
{
...
void PG::publish_stats_to_osd()
{
    ...
    auto stats = recovery_state.prepare_stats_for_publish(pg_stats_publish_valid, pg_stats_publish, unstable_stats);
    if (stats) {
...
{
    recovery_state.init(role, newup, new_up_primary, newacting, new_acting_primary, history, pi, backfill, t);
}
...
bool PG::is_time_for_deep(bool allow_deep_scrub, bool allow_scrub, bool has_deep_errors,
...
bool PG::verify_periodic_scrub_mode(bool allow_deep_scrub, bool try_to_auto_repair, bool allow_regular_scrub,
                                    bool has_deep_errors, requested_scrub_t& planned) const

{
    ...
    if (allow_deep_scrub) {
        ...
        planned.time_for_deep = is_time_for_deep(allow_deep_scrub, allow_regular_scrub, has_deep_errors, planned);
...
std::optional<requested_scrub_t> PG::verify_scrub_mode() const
{
    ...
    bool try_to_auto_repair = (cct->_conf->osd_scrub_auto_repair && get_pgbackend()->auto_repair_supported());
    ...
    if (!upd_flags.must_scrub) {
        ...
        bool can_start_periodic = verify_periodic_scrub_mode(
            allow_deep_scrub, try_to_auto_repair, allow_regular_scrub, has_deep_errors, upd_flags);
    ...
    bool prevented_by_recovery = osd->is_recovery_active() && !cct->_conf->osd_scrub_during_recovery &&
...
{
    dout(20) << __func__ << ": scrub flags on new interval: " << m_planned_scrub << dendl;
...
{
    for (map<hobject_t, ScrubMap::object>::reverse_iterator i = smap.objects.rbegin(); i != smap.objects.rend(); ++i) {
        ...
        if (oi.soid != hoid) {
            ObjectStore::Transaction t;
            OSDriver::OSTransaction _t(osdriver.get_transaction(&t));
            osd->clog->error() << "osd." << osd->whoami << " found object info error on pg " << info.pgid << " oid "
                               << hoid << " oid in object info: " << oi.soid << "...repaired";
            // Fix object info
            oi.soid = hoid;
            bl.clear();
...
void PG::repair_object(const hobject_t& soid, const list<pair<ScrubMap::object, pg_shard_t>>& ok_peers,
