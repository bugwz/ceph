...
void PG::get(const char* tag)
{
    ...
    lgeneric_subdout(cct, refs, 5) << "PG::get " << this << " "
                                   << "tag " << (tag ? tag : "(none") << " " << (after - 1) << " -> " << after << dendl;
#ifdef PG_DEBUG_REFS
...
void PG::put(const char* tag)
{
    ...
    lgeneric_subdout(local_cct, refs, 5) << "PG::put " << this << " "
                                         << "tag " << (tag ? tag : "(none") << " " << (after + 1) << " -> " << after
                                         << dendl;
    if (after == 0) delete this;
}
...
uint64_t PG::get_with_id()
{
    ...
    stringstream ss;
    bt.print(ss);
    lgeneric_subdout(cct, refs, 5) << "PG::get " << this << " " << info.pgid << " got id " << id << " " << (ref - 1)
                                   << " -> " << ref << dendl;
...
{
    int newref = --ref;
    lgeneric_subdout(cct, refs, 5) << "PG::put " << this << " " << info.pgid << " put id " << id << " " << (newref + 1)
                                   << " -> " << newref << dendl;
    {
        std::lock_guard l(_ref_id_lock);
        ceph_assert(_live_ids.count(id));
...
void PG::dump_live_ids()
{
    ...
    for (map<uint64_t, string>::iterator i = _live_ids.begin(); i != _live_ids.end(); ++i) {
        dout(0) << "\t\tid: " << *i << dendl;
    }
    dout(0) << "\t" << __func__ << ": " << info.pgid << " live tags:" << dendl;
    for (map<string, uint64_t>::iterator i = _tag_counts.begin(); i != _tag_counts.end(); ++i) {
        dout(0) << "\t\tid: " << *i << dendl;
    }
}
#endif
...
PG::PG(OSDService* o, OSDMapRef curmap, const PGPool& _pool, spg_t p)
    ...
    , snap_mapper(cct, &osdriver, p.ps(), p.get_split_bits(_pool.info.get_pg_num()), _pool.id, p.shard)
    ...
    , active_pushes(0)
    , recovery_state(o->cct, pg_whoami, p, _pool, curmap, this, this)
...
#endif
        out << "osd." << osd->whoami << " pg_epoch: " << (mapref ? mapref->get_epoch() : 0) << " " << *this << " ";
    }
    else {
        out << "osd." << osd->whoami << " pg_epoch: " << (mapref ? mapref->get_epoch() : 0) << " pg[" << pg_id.pgid
            << "(unlocked)] ";
    }
    return out;
}
...
void PG::log_state_exit(const char* state_name, utime_t enter_time, uint64_t events, utime_t event_dur)
{
    osd->pg_recovery_stats.log_exit(state_name, ceph_clock_now() - enter_time, events, event_dur);
}

/********* PG **********/

void PG::remove_snap_mapped_object(ObjectStore::Transaction& t, const hobject_t& soid)
{
    t.remove(coll, ghobject_t(soid, ghobject_t::NO_GEN, pg_whoami.shard));
    clear_object_snap_mapping(&t, soid);
}

void PG::clear_object_snap_mapping(ObjectStore::Transaction* t, const hobject_t& soid)
{
...
void PG::update_object_snap_mapping(ObjectStore::Transaction* t, const hobject_t& soid, const set<snapid_t>& snaps)
{
    OSDriver::OSTransaction _t(osdriver.get_transaction(t));
...
{
    // only check MOSDOp
    if (op->get_req()->get_type() != CEPH_MSG_OSD_OP) return true;
    ...
    const string& key = req->get_hobj().get_key().empty() ? req->get_oid().name : req->get_hobj().get_key();

    bool cap = caps.is_capable(pool.name,
                               req->get_hobj().nspace,
    ...
    dout(20) << "op_has_sufficient_caps "
             << "session=" << session << " pool=" << pool.id << " (" << pool.name << " " << req->get_hobj().nspace
             << ")"
             << " pool_app_metadata=" << pool.info.application_metadata << " need_read_cap=" << op->need_read_cap()
             << " need_write_cap=" << op->need_write_cap() << " classes=" << op->classes() << " -> "
             << (cap ? "yes" : "NO") << dendl;
    return cap;
}
...
void PG::queue_recovery()
{
    if (!is_primary() || !is_peered()) {
        ...
        auto num_bytes = static_cast<uint64_t>(std::max<int64_t>(0,   // ensure bytes is non-negative
                                                                 info.stats.stats.sum.num_bytes));
        auto num_objects = static_cast<uint64_t>(std::max<int64_t>(1,   // ensure objects is non-negative and non-zero
                                                                   info.stats.stats.sum.num_objects));
        uint64_t cost_per_object = std::max<uint64_t>(num_bytes / num_objects, 1);
        osd->queue_for_recovery(this, cost_per_object, recovery_state.get_recovery_op_priority());
    }
}
...
void PG::queue_scrub_after_repair()
{
    ...
    if (is_scrub_queued_or_active()) {
        dout(10) << __func__ << ": scrubbing already (" << (is_scrubbing() ? "active)" : "queued)") << dendl;
        return;
    }
...
unsigned PG::get_scrub_priority()
{
    ...
    int64_t pool_scrub_priority = pool.info.opts.value_or(pool_opts_t::SCRUB_PRIORITY, (int64_t)0);
...
void PG::merge_from(map<spg_t, PGRef>& sources, PeeringCtx& rctx, unsigned split_bits,
...
void PG::add_backoff(const ceph::ref_t<Session>& s, const hobject_t& begin, const hobject_t& end)
{
    ...
    con->send_message(new MOSDBackoff(info.pgid, get_osdmap_epoch(), CEPH_OSD_BACKOFF_OP_BLOCK, b->id, begin, end));
}
...
void PG::release_backoffs(const hobject_t& begin, const hobject_t& end)
{
    ...
    for (auto b : bv) {
        ...
        if (b->session) {
            ...
            if (con) {   // OSD::ms_handle_reset clears s->con without a lock
                con->send_message(new MOSDBackoff(
                    info.pgid, get_osdmap_epoch(), CEPH_OSD_BACKOFF_OP_UNBLOCK, b->id, b->begin, b->end));
            }
            if (b->is_new()) {
...
void PG::send_cluster_message(int target, MessageRef m, epoch_t epoch, bool share_map_update)
{
    ConnectionRef con = osd->get_con_osd_cluster(target, get_osdmap_epoch());
    if (!con) {
        return;
    }
...
void PG::update_heartbeat_peers(set<int> new_peers)
{
    ...
    if (need_update) osd->need_heartbeat_peer_update();
}


bool PG::check_in_progress_op(const osd_reqid_t& r, eversion_t* version, version_t* user_version, int* return_code,
                              vector<pg_log_op_return_item_t>* op_returns) const
{
    return (projected_log.get_request(r, version, user_version, return_code, op_returns) ||
            recovery_state.get_pg_log().get_log().get_request(r, version, user_version, return_code, op_returns));
}
...
void PG::publish_stats_to_osd()
{
    ...
    recovery_state.update_stats_wo_resched(
        [scrubber = m_scrubber.get()](pg_history_t& hist, pg_stat_t& info) mutable -> void {
            info.scrub_sched_status = scrubber->get_schedule();
        });

    std::lock_guard l{pg_stats_publish_lock};
    auto stats = recovery_state.prepare_stats_for_publish(pg_stats_publish, unstable_stats);
    if (stats) {
 ...
 */
void PG::init(int role, const vector<int>& newup, int new_up_primary, const vector<int>& newacting,
              int new_acting_primary, const pg_history_t& history, const PastIntervals& pi, ObjectStore::Transaction& t)
{
    recovery_state.init(role, newup, new_up_primary, newacting, new_acting_primary, history, pi, t);
}
...
void PG::prepare_write(pg_info_t& info, pg_info_t& last_written_info, PastIntervals& past_intervals, PGLog& pglog,
                       bool dirty_info, bool dirty_big_info, bool need_write_epoch, ObjectStore::Transaction& t)
{
    ...
    if (!km.empty()) t.omap_setkeys(coll, pgmeta_oid, km);
    if (!key_to_remove.empty()) t.omap_rmkey(coll, pgmeta_oid, key_to_remove);
}
...
bool PG::_has_removal_flag(ObjectStore* store, spg_t pgid)
{
    coll_t coll(pgid);
    ...
    ceph_assert(ch);
    if (store->omap_get_values(ch, pgmeta_oid, keys, &values) == 0 && values.size() == 1) return true;

    return false;
}

int PG::peek_map_epoch(ObjectStore* store, spg_t pgid, epoch_t* pepoch)
{
    coll_t coll(pgid);
    ghobject_t pgmeta_oid(pgid.make_pgmeta_oid());
    epoch_t cur_epoch = 0;
...
int PG::read_info(ObjectStore* store, spg_t pgid, const coll_t& coll, pg_info_t& info, PastIntervals& past_intervals,
                  __u8& struct_v)
{
    set<string> keys;
    keys.insert(string(infover_key));
    ...
    int r = store->omap_get_values(ch, pgmeta_oid, keys, &values);
    ceph_assert(r == 0);
    ceph_assert(values.size() == 3 || values.size() == 4);
...
void PG::read_state(ObjectStore* store)
{
    ...
    recovery_state.init_from_disk_state(
        std::move(info_from_disk), std::move(past_intervals_from_disk), [this, store](PGLog& pglog) {
            ostringstream oss;
            pglog.read_log_and_missing(store,
                                       ch,
                                       pgmeta_oid,
                                       info,
                                       oss,
            ...
            if (oss.tellp()) osd->clog->error() << oss.str();
            return 0;
        });
...
void PG::update_snap_map(const vector<pg_log_entry_t>& log_entries, ObjectStore::Transaction& t)
{
    for (auto i = log_entries.cbegin(); i != log_entries.cend(); ++i) {
        ...
        if (i->soid.snap < CEPH_MAXSNAP) {
            if (i->is_delete()) {
                int r = snap_mapper.remove_oid(i->soid, &_t);
                if (r) derr << __func__ << " remove_oid " << i->soid << " failed with " << r << dendl;
                ...
                if (i->is_clone() || i->is_promote()) {
                    snap_mapper.add_oid(i->soid, _snaps, &_t);
                }
                else if (i->is_modify()) {
                    int r = snap_mapper.update_snaps(i->soid, _snaps, 0, &_t);
                    ceph_assert(r == 0);
                }
                else {
...
void PG::filter_snapc(vector<snapid_t>& snaps)
{
    ...
    if (snap_trimq.empty() && info.purged_snaps.empty()) return;

    bool filtering = false;
    vector<snapid_t> newsnaps;
    for (vector<snapid_t>::iterator p = snaps.begin(); p != snaps.end(); ++p) {
        if (snap_trimq.contains(*p) || info.purged_snaps.contains(*p)) {
            ...
            if (filtering) newsnaps.push_back(*p);   // continue building new vector
        }
    }
    if (filtering) {
        snaps.swap(newsnaps);
...
{
    for (auto it = m.begin(); it != m.end(); ++it) requeue_ops(it->second);
    m.clear();
}
...
void PG::requeue_op(OpRequestRef op)
{
    ...
    if (p != waiting_for_map.end()) {
        dout(20) << __func__ << " " << *op->get_req() << " (waiting_for_map " << p->first << ")" << dendl;
        p->second.push_front(op);
    }
    else {
        dout(20) << __func__ << " " << *op->get_req() << dendl;
        osd->enqueue_front(OpSchedulerItem(unique_ptr<OpSchedulerItem::OpQueueable>(new PGOpItem(info.pgid, op)),
                                           op->get_req()->get_cost(),
...
{
    for (list<OpRequestRef>::reverse_iterator i = ls.rbegin(); i != ls.rend(); ++i) {
        requeue_op(*i);
    }
    ls.clear();
}
...
void PG::requeue_map_waiters()
{
    ...
    while (p != waiting_for_map.end()) {
        if (epoch < p->second.front()->min_epoch) {
            dout(20) << __func__ << " " << p->first << " front op " << p->second.front()
                     << " must still wait, doing nothing" << dendl;
            ++p;
        }
        else {
            ...
            for (auto q = p->second.rbegin(); q != p->second.rend(); ++q) {
                auto req = *q;
                osd->enqueue_front(
                    OpSchedulerItem(unique_ptr<OpSchedulerItem::OpQueueable>(new PGOpItem(info.pgid, req)),
...
{
    double deep_scrub_interval = pool.info.opts.value_or(pool_opts_t::DEEP_SCRUB_INTERVAL, 0.0);
    if (deep_scrub_interval <= 0.0) deep_scrub_interval = cct->_conf->osd_deep_scrub_interval;
    return info.history.last_deep_scrub_stamp + deep_scrub_interval;
}

bool PG::is_time_for_deep(bool allow_deep_scrub, bool allow_shallow_scrub, bool has_deep_errors,
                          const requested_scrub_t& planned) const
{
    dout(10) << fmt::format("{}: need-auto? {} allowed? {}/{} deep-errors? {} "
    ...
    if (!allow_deep_scrub) return false;
    ...
    if (ceph_clock_now() >= next_deepscrub_interval()) {
        dout(20) << __func__ << ": now (" << ceph_clock_now() << ") >= time for deep (" << next_deepscrub_interval()
                 << ")" << dendl;
        return true;
    }
    ...
    if (allow_shallow_scrub) {
        const bool deep_coin_flip = (rand() % 100) < cct->_conf->osd_deep_scrub_randomize_ratio * 100;

        dout(15) << __func__ << ": time_for_deep=" << planned.time_for_deep << " deep_coin_flip=" << deep_coin_flip
                 << dendl;

        if (deep_coin_flip) return true;
    }
...
/*
 ...
 clang-format on
*/
std::optional<requested_scrub_t> PG::validate_initiated_scrub(bool allow_deep_scrub, bool try_to_auto_repair,
                                                              bool time_for_deep, bool has_deep_errors,
                                                              const requested_scrub_t& planned) const
{
    ...
    if (upd_flags.must_deep_scrub) {
        ...
        if (has_deep_errors) {
            osd->clog->error() << fmt::format(
                "osd.{} pg {} Regular scrub request, deep-scrub details will be lost", osd->whoami, info.pgid);
        }
    }
...
/*
 ...
 clang-format on
*/
std::optional<requested_scrub_t> PG::validate_periodic_mode(bool allow_deep_scrub, bool try_to_auto_repair,
                                                            bool allow_shallow_scrub, bool time_for_deep,
    ...
    if (allow_shallow_scrub) {
        if (!upd_flags.time_for_deep || !allow_deep_scrub) {
            if (try_to_auto_repair) {
                dout(10) << __func__ << ": auto repair with scrubbing, rescrub if errors found" << dendl;
                upd_flags.deep_scrub_on_error = true;
            }
            dout(20) << __func__ << " will do shallow scrub (time_for_deep = " << upd_flags.time_for_deep << ")"
                     << dendl;
            return upd_flags;
        }
...
std::optional<requested_scrub_t> PG::validate_scrub_mode() const
{
    const bool allow_shallow_scrub =
        !(get_osdmap()->test_flag(CEPH_OSDMAP_NOSCRUB) || pool.info.has_flag(pg_pool_t::FLAG_NOSCRUB));
    const bool allow_deep_scrub =
        !(get_osdmap()->test_flag(CEPH_OSDMAP_NODEEP_SCRUB) || pool.info.has_flag(pg_pool_t::FLAG_NODEEP_SCRUB));
    const bool has_deep_errors = (info.stats.stats.sum.num_deep_scrub_errors > 0);
    const bool try_to_auto_repair = (cct->_conf->osd_scrub_auto_repair && get_pgbackend()->auto_repair_supported());

    dout(10) << __func__ << " pg: " << info.pgid << " allow: " << allow_shallow_scrub << "/" << allow_deep_scrub
             << " deep errs: " << has_deep_errors << " auto-repair: " << try_to_auto_repair << " ("
             << cct->_conf->osd_scrub_auto_repair << ")" << dendl;

    //  scrubbing while recovering?
    const bool prevented_by_recovery = osd->is_recovery_active() && !cct->_conf->osd_scrub_during_recovery &&
    ...
    const bool time_for_deep =
        is_time_for_deep(allow_deep_scrub, allow_shallow_scrub, has_deep_errors, m_planned_scrub);
    std::optional<requested_scrub_t> upd_flags;

    if (m_planned_scrub.must_scrub) {
        upd_flags = validate_initiated_scrub(
            allow_deep_scrub, try_to_auto_repair, time_for_deep, has_deep_errors, m_planned_scrub);
    }
    else {
        ceph_assert(!m_planned_scrub.must_deep_scrub);
        upd_flags = validate_periodic_mode(
            allow_deep_scrub, try_to_auto_repair, allow_shallow_scrub, time_for_deep, has_deep_errors, m_planned_scrub);
        if (!upd_flags) {
    ...
    dout(10) << fmt::format("{}: next scrub flags: {}", __func__, *upd_flags) << dendl;
...
void PG::on_new_interval()
{
    ...
    dout(20) << __func__ << (is_scrub_queued_or_active() ? " scrubbing " : " ") << "flags: " << m_planned_scrub
             << dendl;
...
void PG::schedule_event_after(PGPeeringEventRef event, float delay)
{
    std::lock_guard lock(osd->recovery_request_lock);
    osd->recovery_request_timer.add_event_after(delay, new QueuePeeringEvt(this, std::move(event)));
}

void PG::request_local_background_io_reservation(unsigned priority, PGPeeringEventURef on_grant,
                                                 PGPeeringEventURef on_preempt)
{
    osd->local_reserver.request_reservation(pg_id,
                                            on_grant ? new QueuePeeringEvt(this, std::move(on_grant)) : nullptr,
                                            priority,
                                            on_preempt ? new QueuePeeringEvt(this, std::move(on_preempt)) : nullptr);
}

void PG::update_local_background_io_priority(unsigned priority)
{
    osd->local_reserver.update_priority(pg_id, priority);
}

void PG::cancel_local_background_io_reservation()
{
    osd->local_reserver.cancel_reservation(pg_id);
}

void PG::request_remote_recovery_reservation(unsigned priority, PGPeeringEventURef on_grant,
                                             PGPeeringEventURef on_preempt)
{
    osd->remote_reserver.request_reservation(pg_id,
                                             on_grant ? new QueuePeeringEvt(this, std::move(on_grant)) : nullptr,
                                             priority,
                                             on_preempt ? new QueuePeeringEvt(this, std::move(on_preempt)) : nullptr);
}
...
{
    osd->remote_reserver.cancel_reservation(pg_id);
}

void PG::schedule_event_on_commit(ObjectStore::Transaction& t, PGPeeringEventRef on_commit)
{
    ...
    if (j != new_purged_snaps.end()) {
        ...
        for (auto k : j->second) {
            if (!recovery_state.get_info().purged_snaps.contains(k.first, k.second)) {
                ...
                recovery_state.adjust_purged_snaps([&overlap](auto& purged_snaps) { purged_snaps.subtract(overlap); });
                ...
                recovery_state.adjust_purged_snaps([&k](auto& purged_snaps) { purged_snaps.erase(k.first, k.second); });
            }
        }
        dout(10) << __func__ << " new purged_snaps " << j->second << ", now " << recovery_state.get_info().purged_snaps
                 << dendl;
        ceph_assert(!bad || !cct->_conf->osd_debug_verify_cached_snaps);
    }
}
...
void PG::queue_snap_retrim(snapid_t snap)
{
    ...
    if (!snap_trimq.contains(snap)) {
        ...
        dout(20) << __func__ << " snap " << snap << ", trimq now " << snap_trimq << ", repeat " << snap_trimq_repeat
                 << dendl;
        kick_snap_trim();
    }
    else {
...
{
    if (cct->_conf->osd_check_for_log_corruption) check_log_for_corruption(osd->store);
    ...
    if (recovery_state.is_peered() && !recovery_state.is_clean() &&
        !recovery_state.get_osdmap()->test_flag(CEPH_OSDMAP_NOBACKFILL) &&
        (!recovery_state.get_osdmap()->test_flag(CEPH_OSDMAP_NOREBALANCE) || recovery_state.is_degraded())) {
        queue_recovery();
    }
}
...
void PG::schedule_renew_lease(epoch_t lpr, ceph::timespan delay)
{
    ...
    auto o = osd;
    osd->mono_timer.add_event(delay, [o, lpr, spgid]() { o->queue_renew_lease(lpr, spgid); });
}
...
{
    pglog.rebuild_missing_set_with_deletes(osd->store, ch, recovery_state.get_info());
}
...
bool PG::try_reserve_recovery_space(int64_t primary_bytes, int64_t local_bytes)
{
    ...
    if (primary_bytes) {
        ...
        if (pool.info.is_erasure()) {
            ...
            primary_bytes += get_pgbackend()->get_ec_stripe_chunk_size() * info.stats.stats.sum.num_objects;
            local_bytes /= (int)get_pgbackend()->get_ec_data_chunk_count();
            local_bytes += get_pgbackend()->get_ec_stripe_chunk_size() * info.stats.stats.sum.num_objects;
        }
        pending_adjustment = pending_backfill(cct, primary_bytes, local_bytes);
        dout(10) << __func__ << " primary_bytes " << (primary_bytes >> 10) << "KiB"
                 << " local " << (local_bytes >> 10) << "KiB"
                 << " pending_adjustments " << (pending_adjustment >> 10) << "KiB" << dendl;
    }
    ...
    if (cct->_conf->osd_debug_reject_backfill_probability > 0 &&
        ...
        // backfill anyway.
        if (primary_bytes && !cct->_conf->osd_debug_skip_full_check_in_backfill_reservation) {
...
void PG::_scan_rollback_obs(const vector<ghobject_t>& rollback_obs)
{
    ...
    for (vector<ghobject_t>::const_iterator i = rollback_obs.begin(); i != rollback_obs.end(); ++i) {
        if (i->generation < trimmed_to.version) {
            dout(10) << __func__ << "osd." << osd->whoami << " pg " << info.pgid << " found obsolete rollback obj "
                     << *i << " generation < trimmed_to " << trimmed_to << "...repaired" << dendl;
            t.remove(coll, *i);
        }
    }
    if (!t.empty()) {
...
void PG::forward_scrub_event(ScrubSafeAPI fn, epoch_t epoch_queued, Scrub::act_token_t act_token, std::string_view desc)
{
    dout(20) << __func__ << ": " << desc << " queued: " << epoch_queued << " token: " << act_token << dendl;
    ceph_assert(m_scrubber);
    if (is_active()) {
...
void PG::replica_scrub(epoch_t epoch_queued, Scrub::act_token_t act_token,
                       [[maybe_unused]] ThreadPool::TPHandle& handle)
{
    dout(10) << __func__ << " queued at: " << epoch_queued << (is_primary() ? " (primary)" : " (replica)") << dendl;
    forward_scrub_event(&ScrubPgIF::send_start_replica, epoch_queued, act_token, "StartReplica/nw");
}
...
{
    return waiting_for_scrub.empty() ? Scrub::scrub_prio_t::low_priority : Scrub::scrub_prio_t::high_priority;
}

bool PG::old_peering_msg(epoch_t reply_epoch, epoch_t query_epoch)
{
    if (auto last_reset = get_last_peering_reset(); last_reset > reply_epoch || last_reset > query_epoch) {
        dout(10) << "old_peering_msg reply_epoch " << reply_epoch << " query_epoch " << query_epoch
                 << " last_peering_reset " << last_reset << dendl;
        return true;
    }
    return false;
}
...
struct FlushState
{
    PGRef pg;
    epoch_t epoch;
    FlushState(PG* pg, epoch_t epoch)
        : pg(pg)
        , epoch(epoch)
    {}
    ~FlushState()
...
void PG::start_flush_on_transaction(ObjectStore::Transaction& t)
{
    ...
    FlushStateRef flush_trigger(std::make_shared<FlushState>(this, get_osdmap_epoch()));
...
{
    Context* c = new QueuePeeringEvt(this, get_osdmap_epoch(), PeeringState::IntervalFlush());
...
ostream& operator<<(ostream& out, const PG& pg)
{
    ...
    if (pg.recovery_ops_active) out << " rops=" << pg.recovery_ops_active;
...
bool PG::can_discard_op(OpRequestRef& op)
{
    ...
    if ((m->get_flags() & (CEPH_OSD_FLAG_BALANCE_READS | CEPH_OSD_FLAG_LOCALIZE_READS)) && !is_primary() &&
    ...
    if (m->get_connection()->has_feature(CEPH_FEATURE_RESEND_ON_SPLIT)) {
        ...
        if (m->get_connection()->has_feature(CEPH_FEATURE_SERVER_NAUTILUS)) {
            ...
            if (m->get_map_epoch() < pool.info.get_last_force_op_resend()) {
                dout(7) << __func__ << " sent before last_force_op_resend " << pool.info.last_force_op_resend
                        << ", dropping" << *m << dendl;
                return true;
            }
        }
            ...
            if (m->get_map_epoch() < pool.info.get_last_force_op_resend_prenautilus()) {
                dout(7) << __func__ << " sent before last_force_op_resend_prenautilus "
                        << pool.info.last_force_op_resend_prenautilus << ", dropping" << *m << dendl;
                return true;
            }
        }
        ...
        if (m->get_map_epoch() < pool.info.get_last_force_op_resend_preluminous()) {
            dout(7) << __func__ << " sent before last_force_op_resend_preluminous "
                    << pool.info.last_force_op_resend_preluminous << ", dropping" << *m << dendl;
            return true;
        }
    }
...
template<typename T, int MSGTYPE> bool PG::can_discard_replica_op(OpRequestRef& op)
{
    auto m = op->get_req<T>();
    ...
    if (old_peering_msg(m->map_epoch, m->map_epoch)) {
        dout(10) << "can_discard_replica_op pg changed " << info.history << " after " << m->map_epoch << ", dropping"
                 << dendl;
        return true;
    }
    return false;
...
bool PG::can_discard_request(OpRequestRef& op)
{
    ...
    case CEPH_MSG_OSD_OP: return can_discard_op(op);
    case CEPH_MSG_OSD_BACKOFF: return false;   // never discard
    case MSG_OSD_REPOP: return can_discard_replica_op<MOSDRepOp, MSG_OSD_REPOP>(op);
    case MSG_OSD_PG_PUSH: return can_discard_replica_op<MOSDPGPush, MSG_OSD_PG_PUSH>(op);
    case MSG_OSD_PG_PULL: return can_discard_replica_op<MOSDPGPull, MSG_OSD_PG_PULL>(op);
    case MSG_OSD_PG_PUSH_REPLY: return can_discard_replica_op<MOSDPGPushReply, MSG_OSD_PG_PUSH_REPLY>(op);
    case MSG_OSD_REPOPREPLY: return can_discard_replica_op<MOSDRepOpReply, MSG_OSD_REPOPREPLY>(op);
    ...
    case MSG_OSD_EC_WRITE: return can_discard_replica_op<MOSDECSubOpWrite, MSG_OSD_EC_WRITE>(op);
    case MSG_OSD_EC_WRITE_REPLY: return can_discard_replica_op<MOSDECSubOpWriteReply, MSG_OSD_EC_WRITE_REPLY>(op);
    case MSG_OSD_EC_READ: return can_discard_replica_op<MOSDECSubOpRead, MSG_OSD_EC_READ>(op);
    case MSG_OSD_EC_READ_REPLY: return can_discard_replica_op<MOSDECSubOpReadReply, MSG_OSD_EC_READ_REPLY>(op);
    case MSG_OSD_REP_SCRUB: return can_discard_replica_op<MOSDRepScrub, MSG_OSD_REP_SCRUB>(op);
    case MSG_OSD_SCRUB_RESERVE: return can_discard_replica_op<MOSDScrubReserve, MSG_OSD_SCRUB_RESERVE>(op);
    case MSG_OSD_REP_SCRUBMAP: return can_discard_replica_op<MOSDRepScrubMap, MSG_OSD_REP_SCRUBMAP>(op);
    case MSG_OSD_PG_UPDATE_LOG_MISSING:
        return can_discard_replica_op<MOSDPGUpdateLogMissing, MSG_OSD_PG_UPDATE_LOG_MISSING>(op);
    case MSG_OSD_PG_UPDATE_LOG_MISSING_REPLY:
        return can_discard_replica_op<MOSDPGUpdateLogMissingReply, MSG_OSD_PG_UPDATE_LOG_MISSING_REPLY>(op);

    case MSG_OSD_PG_SCAN: return can_discard_scan(op);
    case MSG_OSD_PG_BACKFILL: return can_discard_backfill(op);
    case MSG_OSD_PG_BACKFILL_REMOVE:
        return can_discard_replica_op<MOSDPGBackfillRemove, MSG_OSD_PG_BACKFILL_REMOVE>(op);
    }
    return true;
}
...
{
    dout(10) << "null" << dendl;
    queue_peering_event(PGPeeringEventRef(std::make_shared<PGPeeringEvent>(msg_epoch, query_epoch, NullEvt())));
}
...
void PG::find_unfound(epoch_t queued, PeeringCtx& rctx)
{
    ...
    if (!recovery_state.discover_all_missing(rctx)) {
        ...
        if (state_test(PG_STATE_BACKFILLING)) {
            ...
            auto evt = PGPeeringEventRef(new PGPeeringEvent(queued, queued, PeeringState::UnfoundRecovery()));
            queue_peering_event(evt);
            action = "in recovery";
        }
        else {
...
void PG::handle_advance_map(OSDMapRef osdmap, OSDMapRef lastmap, vector<int>& newup, int up_primary,
                            vector<int>& newacting, int acting_primary, PeeringCtx& rctx)
{
    ...
    recovery_state.advance_map(osdmap, lastmap, newup, up_primary, newacting, acting_primary, rctx);
}
...
std::pair<ghobject_t, bool> PG::do_delete_work(ObjectStore::Transaction& t, ghobject_t _next)
{
    dout(10) << __func__ << dendl;

    {
        ...
        if (osd_delete_sleep > 0 && delete_needs_sleep) {
            ...
            auto delete_requeue_callback = new LambdaContext([this, pgref, e](int r) {
                dout(20) << "do_delete_work() [cb] wake up at " << ceph_clock_now() << ", re-queuing delete" << dendl;
                std::scoped_lock locker{*this};
            ...
            osd->sleep_timer.add_event_at(delete_schedule_time, delete_requeue_callback);
    ...
    int max = std::min(osd->store->get_ideal_list_max(), (int)cct->_conf->osd_target_transaction_size);

    osd->store->collection_list(ch, _next, ghobject_t::get_max(), max, &olist, &next);
    ...
    if (_next != ghobject_t() && olist.empty()) {
        next = ghobject_t();
        osd->store->collection_list(ch, next, ghobject_t::get_max(), max, &olist, &next);
        for (auto& oid : olist) {
            if (oid == pgmeta_oid) {
                ...
                dout(0) << __func__ << " additional unexpected onode"
                        << " new onode has appeared since PG removal started" << oid << dendl;
            }
        }
    }
    ...
    for (auto& oid : olist) {
        ...
        if (oid.is_pgmeta()) {
            osd->clog->warn() << info.pgid << " found stray pgmeta-like " << oid << " during PG removal";
        }
    ...
    if (num) {
        ...
        if (!osd->try_finish_pg_delete(this, pool.info.get_pg_num())) {
            ...
            create_pg_collection(t, info.pgid, info.pgid.get_split_bits(pool.info.get_pg_num()));
...
void PG::dump_missing(Formatter* f)
{
    for (auto& i : recovery_state.get_pg_log().get_missing().get_items()) {
        ...
        if (recovery_state.get_missing_loc().needs_recovery(i.first)) {
            f->dump_bool("unfound", recovery_state.get_missing_loc().is_unfound(i.first));
