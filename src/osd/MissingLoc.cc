...
bool MissingLoc::readable_with_acting(const hobject_t& hoid, const set<pg_shard_t>& acting, eversion_t* v) const
{
    if (!needs_recovery(hoid, v)) return true;
    if (is_deleted(hoid)) return false;
    ...
    for (auto i = locs.begin(); i != locs.end(); ++i) {
        if (acting.count(*i)) have_acting.insert(*i);
    }
    return (*is_readable)(have_acting);
}

void MissingLoc::add_batch_sources_info(const set<pg_shard_t>& sources, HBHandle* handle)
{
    ldout(cct, 10) << __func__ << ": adding sources in batch " << sources.size() << dendl;
    unsigned loop = 0;
    bool sources_updated = false;
    for (auto i = needs_recovery_map.begin(); i != needs_recovery_map.end(); ++i) {
...
bool MissingLoc::add_source_info(pg_shard_t fromosd, const pg_info_t& oinfo, const pg_missing_t& omissing,
                                 HBHandle* handle)
{
    ...
    bool sources_updated = false;
    // found items?
    for (auto p = needs_recovery_map.begin(); p != needs_recovery_map.end(); ++p) {
