...
#undef dout_prefix
#define dout_prefix \
    *_dout << "client." << ioctx.get_instance_id() << ": SimpleRADOSStriper: " << __func__ << ": " << oid << ": "
...
{
    d(10) << " new_size: " << new_size << " update_size: " << update_size << " allocated: " << allocated
    ...
    while (!shutdown) {
        ...
        if (since >= lock_keeper_interval && locked) {
            ...
            int rc = ioctx.lock_exclusive(
                ext.soid, biglock, cookie.to_string(), lockdesc, &tv, LIBRADOS_LOCK_FLAG_MUST_RENEW);
            if (rc) {
        ...
        for (auto& a : addrv.v) {
            ...
            *css << "\"}";
            ...
            if (int rc = R.mon_command(css->str(), bufferlist(), nullptr, &out); rc < 0) {
                ...
                d(-1) << "Cannot proceed with recovery because I have failed to blocklist the old client: "
