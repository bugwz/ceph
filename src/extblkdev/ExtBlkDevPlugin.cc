...
int detect_device(CephContext* cct, const std::string& logdevname, ExtBlkDevInterfaceRef& ebd_impl)
{
    int rc = -ENOENT;
    std::string plg_name;
    auto registry = cct->get_plugin_registry();
    ...
    for (auto& it : ptype->second) {
        ...
        dout(10) << __func__ << " Trying to detect block device " << logdevname << " using plugin " << it.first
                 << dendl;
        auto ebdplugin = dynamic_cast<ExtBlkDevPlugin*>(it.second);
        if (ebdplugin == nullptr) {
            derr << __func__ << " Is not an extblkdev plugin: " << it.first << dendl;
    ...
    if (rc == 0) {
        dout(1) << __func__ << " using plugin " << plg_name << ", "
                << "volume " << ebd_impl->get_devname() << " maps to " << logdevname << dendl;
    }
    else {
