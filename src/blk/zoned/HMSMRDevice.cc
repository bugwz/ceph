...
bool HMSMRDevice::set_smr_params(const std::string& path)
{
    ...
    dout(10) << __func__ << " setting zone size to " << zone_size << " and conventional region size to "
             << conventional_region_size << dendl;
...
void HMSMRDevice::_aio_log_start(IOContext* ioc, uint64_t offset, uint64_t length)
{
...
void HMSMRDevice::_aio_log_finish(IOContext* ioc, uint64_t offset, uint64_t length)
{
...
int HMSMRDevice::_sync_write(uint64_t off, bufferlist& bl, bool buffered, int write_hint)
{
    ...
    if (buffered) {
        ...
        r = ::sync_file_range(fd_buffereds[WRITE_LIFE_NOT_SET],
                              off,
                              len,
...
int HMSMRDevice::write(uint64_t off, bufferlist& bl, bool buffered, int write_hint)
{
    uint64_t len = bl.length();
...
int HMSMRDevice::aio_write(uint64_t off, bufferlist& bl, IOContext* ioc, bool buffered, int write_hint)
{
    uint64_t len = bl.length();
    dout(20) << __func__ << " 0x" << std::hex << off << "~" << len << std::dec
             << (buffered ? " (buffered)" : " (direct)") << dendl;
...
int HMSMRDevice::read(uint64_t off, uint64_t len, bufferlist* pbl, IOContext* ioc, bool buffered)
{
    dout(5) << __func__ << " 0x" << std::hex << off << "~" << len << std::dec
            << (buffered ? " (buffered)" : " (direct)") << dendl;
    ceph_assert(is_valid_io(off, len));
    ...
    int r = ::pread(buffered ? fd_buffereds[WRITE_LIFE_NOT_SET] : fd_directs[WRITE_LIFE_NOT_SET], p->c_str(), len, off);
    auto age = cct->_conf->bdev_debug_aio_log_age;
    if (mono_clock::now() - start1 >= make_timespan(age)) {
        derr << __func__ << " stalled read "
             << " 0x" << std::hex << off << "~" << len << std::dec << (buffered ? " (buffered)" : " (direct)")
             << " since " << start1 << ", timeout is " << age << "s" << dendl;
    }
...
int HMSMRDevice::aio_read(uint64_t off, uint64_t len, bufferlist* pbl, IOContext* ioc)
{
...
int HMSMRDevice::read_random(uint64_t off, uint64_t len, char* buf, bool buffered)
{
    ...
    if (buffered) {
        ...
        if (r < 0) {
            ...
            derr << __func__ << " direct_aligned_read"
                 << " 0x" << std::hex << off << "~" << left << std::dec << " error: " << cpp_strerror(r) << dendl;
            goto out;
        }
        ceph_assert((uint64_t)r == len);
    }
