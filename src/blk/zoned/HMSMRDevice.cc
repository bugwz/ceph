...
int HMSMRDevice::_post_open()
{
    ...
    if (zbd_fd < 0) {
        r = errno;
        derr << __func__ << " zbd_open failed on " << path << ": " << cpp_strerror(r) << dendl;
        return -r;
    }
    ...
    if (zbd_report_nr_zones(zbd_fd, 0, 0, ZBD_RO_NOT_WP, &nr_zones) != 0) {
        r = -errno;
        derr << __func__ << " zbd_report_nr_zones failed on " << path << ": " << cpp_strerror(r) << dendl;
        goto fail;
    }
...
void HMSMRDevice::reset_zone(uint64_t zone)
{
    ...
    if (zbd_reset_zones(zbd_fd, zone * zone_size, zone_size) != 0) {
        derr << __func__ << " resetting zone failed for zone 0x" << std::hex << zone << std::dec << dendl;
...
std::vector<uint64_t> HMSMRDevice::get_zones()
{
    ...
    if (r != 0) {
        derr << __func__ << " zbd_report_zones failed on " << path << ": " << cpp_strerror(errno) << dendl;
