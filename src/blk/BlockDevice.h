...
public:
    ...
    bool skip_cache() const { return flags & FLAG_DONT_CACHE; }
};
...
private:
    ...
    static BlockDevice* create_with_type(block_device_t device_type, CephContext* cct, const std::string& path,
...
public:
    ...
    static BlockDevice* create(CephContext* cct, const std::string& path, aio_callback_t cb, void* cbpriv,
    ...
    virtual void reset_all_zones() { ceph_assert(is_smr()); }
    virtual void reset_zone(uint64_t zone) { ceph_assert(is_smr()); }
    ...
    virtual int get_ebd_state(ExtBlkDevState& state) const { return -ENOENT; }
    ...
    virtual int read(uint64_t off, uint64_t len, ceph::buffer::list* pbl, IOContext* ioc, bool buffered) = 0;
    virtual int read_random(uint64_t off, uint64_t len, char* buf, bool buffered) = 0;
    virtual int write(uint64_t off, ceph::buffer::list& bl, bool buffered, int write_hint = WRITE_LIFE_NOT_SET) = 0;

    virtual int aio_read(uint64_t off, uint64_t len, ceph::buffer::list* pbl, IOContext* ioc) = 0;
    virtual int aio_write(uint64_t off, ceph::buffer::list& bl, IOContext* ioc, bool buffered,
