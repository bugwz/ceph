...
public:
    ...
    SharedDriverData(unsigned id_, const spdk_nvme_transport_id& trid_, spdk_nvme_ctrlr* c, spdk_nvme_ns* ns_)
        : id(id_)
        , trid(trid_)
        , ctrlr(c)
        , ns(ns_)
    ...
    void register_device(NVMEDevice* device) { registered_devices.push_back(device); }

    void remove_device(NVMEDevice* device)
    {
        std::vector<NVMEDevice*> new_devices;
        for (auto&& it : registered_devices) {
            if (it != device) new_devices.push_back(it);
        }
...
static int data_buf_next_sge(void* cb_arg, void** address, uint32_t* length)
{
    ...
    addr = t->io_request.extra_segs ? t->io_request.extra_segs[t->io_request.cur_seg_idx]
...
int SharedDriverQueueData::alloc_buf_from_pool(Task* t, bool write)
{
    ...
    if (t->len % data_buffer_size) ++count;
    void** segs;
    if (count > data_buf_list.size()) return -ENOMEM;
        ...
        for (; t; t = t->next) {
            ...
            case IOCommand::WRITE_COMMAND:
            {
                ...
                r = spdk_nvme_ns_cmd_writev(
                    ns, qpair, lba_off, lba_count, io_complete, t, 0, data_buf_reset_sgl, data_buf_next_sge);
                if (r < 0) {
...
public:
    ...
    ~NVMEManager()
    {
        if (!dpdk_thread.joinable()) return;
        {
...
int NVMEManager::try_get(const spdk_nvme_transport_id& trid, SharedDriverData** driver)
{
    ...
    if (!dpdk_thread.joinable()) {
        dpdk_thread = std::thread([this, coremask_arg, m_core_arg, mem_size_arg, trid]() {
            ...
            for (auto p : probe_queue) p->done = true;
            probe_queue_cond.notify_all();
        });
    }
    ...
    ProbeContext ctx{trid, this, nullptr, false};
    {
        ...
        while (!ctx.done) probe_queue_cond.wait(l);
    }
    if (!ctx.driver) return -1;
    *driver = ctx.driver;
...
void io_complete(void* t, const struct spdk_nvme_cpl* completion)
{
    ...
    if (task->command == IOCommand::WRITE_COMMAND) {
        ...
        dout(20) << __func__ << " write/zero op successfully, left " << queue->queue_op_seq - queue->completed_op_seq
                 << dendl;
        ...
        if (!task->return_code) {
            ...
            if (Task* primary = task->primary; primary != nullptr) {
                delete task;
                if (!primary->ref) primary->return_code = 0;
            }
            else {
                task->return_code = 0;
            }
...
int NVMEDevice::open(const string& p)
{
    ...
    if (int r = manager.try_get(trid, &driver); r < 0) {
        derr << __func__ << " failed to get nvme device with transport address " << trid.traddr << " type "
             << trid.trtype << dendl;
        return r;
    }
    ...
    dout(1) << __func__ << " size " << size << " (" << byte_u_t(size) << ")"
            << " block_size " << block_size << " (" << byte_u_t(block_size) << ")" << dendl;
...
static void ioc_append_task(IOContext* ioc, Task* t)
{
    ...
    if (last) last->next = t;
    if (!first) ioc->nvme_task_first = t;
    ioc->nvme_task_last = t;
    ++ioc->num_pending;
}

static void write_split(NVMEDevice* dev, uint64_t off, bufferlist& bl, IOContext* ioc)
{
...
static void make_read_tasks(NVMEDevice* dev, uint64_t aligned_off, IOContext* ioc, char* buf, uint64_t aligned_len,
                            Task* primary, uint64_t orig_off, uint64_t orig_len)
{
    // This value may need to be got from configuration later.
    uint64_t split_size = 131072;   // 128KB.
    ...
    for (; begin < aligned_end; begin += split_size) {
        ...
        // TODO: if upper layer alloc memory with known physical address,
        // we can reduce this copy
        t->fill_cb = [buf, t, tmp_off, tmp_len] { t->copy_to_buf(buf, tmp_off, tmp_len); };
...
int NVMEDevice::aio_write(uint64_t off, bufferlist& bl, IOContext* ioc, bool buffered, int write_hint)
{
...
int NVMEDevice::read(uint64_t off, uint64_t len, bufferlist* pbl, IOContext* ioc, bool buffered)
{
...
int NVMEDevice::aio_read(uint64_t off, uint64_t len, bufferlist* pbl, IOContext* ioc)
{
