...
static int pmem_check_file_type(int fd, const char* pmem_file, uint64_t* total_size)
{
    ...
    fs::path char_dir = fmt::format("/sys/dev/char/{}:{}", major(file_stat.st_rdev), minor(file_stat.st_rdev));
    // Need to check if it is a DAX device
    if (auto subsys_path = char_dir / "subsystem"; fs::read_symlink(subsys_path).filename().string() != "dax") {
        return -EINVAL;
    }
...
int PMEMDevice::open(const std::string& p)
{
    ...
    size_t map_len;
    addr = (char*)pmem_map_file(path.c_str(), 0, devdax_device ? 0 : PMEM_FILE_EXCL, O_RDWR, &map_len, NULL);
    if (addr == NULL) {
...
int PMEMDevice::write(uint64_t off, bufferlist& bl, bool buffered, int write_hint)
{
    ...
    *_dout << dendl;
    ...
    while (len) {
        ...
        auto result =
            dml::execute<execution_path>(dml::mem_move, dml::make_view(data, l), dml::make_view(addr + off1, l));
...
int PMEMDevice::aio_write(uint64_t off, bufferlist& bl, IOContext* ioc, bool buffered, int write_hint)
{
    return write(off, bl, buffered);
}


int PMEMDevice::read(uint64_t off, uint64_t len, bufferlist* pbl, IOContext* ioc, bool buffered)
{
...
#if defined(HAVE_LIBDML)
    auto result =
        dml::execute<execution_path>(dml::mem_move, dml::make_view(addr + off, len), dml::make_view(p.c_str(), len));
    ceph_assert(result.status == dml::status_code::ok);
#else
...
int PMEMDevice::aio_read(uint64_t off, uint64_t len, bufferlist* pbl, IOContext* ioc)
{
...
#if defined(HAVE_LIBDML)
    auto result =
        dml::execute<execution_path>(dml::mem_move, dml::make_view(addr + off, len), dml::make_view(buf, len));
    ceph_assert(result.status == dml::status_code::ok);
#else
