...
int KernelDevice::_lock()
{
    ...
    for (;;) {
        ...
        if (const uint64_t max_retry = cct->_conf.get_val<uint64_t>("bdev_flock_retry");
...
int KernelDevice::open(const string& p)
{
    ...
    if (block_size != (unsigned)st.st_blksize) {
        dout(1) << __func__ << " backing device/file reports st_blksize " << st.st_blksize << ", using bdev_block_size "
    ...
    dout(1) << __func__ << " size " << size << " (0x" << std::hex << size << std::dec << ", " << byte_u_t(size) << ")"
            << " block_size " << block_size << " (" << byte_u_t(block_size) << ")"
            << " " << (rotational ? "rotational device," : "non-rotational device,") << " discard "
...
{
#if defined(F_SET_FILE_RW_HINT)
    if (!enable_wrt) write_hint = WRITE_LIFE_NOT_SET;
#else
...
static bool is_expected_ioerr(const int r)
{
    ...
    return (r == -EOPNOTSUPP || r == -ETIMEDOUT || r == -ENOSPC || r == -ENOLINK || r == -EREMOTEIO || r == -EAGAIN ||
            r == -EIO || r == -ENODATA || r == -EILSEQ || r == -ENOMEM ||
        ...
        if (r > 0) {
            ...
            for (int i = 0; i < r; ++i) {
                ...
                if (r < 0) {
                    ...
                    derr << "aio to 0x" << std::hex << aio[i]->offset << "~" << aio[i]->length << std::dec
                         << " but returned: " << r << dendl;
                    ceph_abort_msg("unexpected aio return value: does not match length");
                }

                dout(10) << __func__ << " finished aio " << aio[i] << " r " << r << " ioc " << ioc << " with "
                         << (ioc->num_running.load() - 1) << " aios left" << dendl;
                ...
                if (debug_stall_since == utime_t()) {
                    ...
                    if (cct->_conf->bdev_debug_aio_suicide_timeout) {
                        ...
                        if (debug_stall_since < cutoff) {
                            derr << __func__ << " stalled aio " << debug_oldest << " since " << debug_stall_since
                                 << ", timeout is " << cct->_conf->bdev_debug_aio_suicide_timeout << "s, suicide"
                                 << dendl;
...
int KernelDevice::_queue_discard(interval_set<uint64_t>& to_release)
{
    ...
    if (!discard_thread.is_started()) return -1;
...
{
    if (!support_discard || !cct->_conf->bdev_enable_discard) return false;
...
void KernelDevice::_aio_log_start(IOContext* ioc, uint64_t offset, uint64_t length)
{
    ...
    if (cct->_conf->bdev_debug_inflight_ios) {
        ...
        if (debug_inflight.intersects(offset, length)) {
            derr << __func__ << " inflight overlap of 0x" << std::hex << offset << "~" << length << std::dec << " with "
                 << debug_inflight << dendl;
            ceph_abort();
        }
...
void KernelDevice::_aio_log_finish(IOContext* ioc, uint64_t offset, uint64_t length)
{
...
void KernelDevice::aio_submit(IOContext* ioc)
{
    ...
    if (retries) derr << __func__ << " retries " << retries << dendl;
    if (r < 0) {
...
int KernelDevice::_sync_write(uint64_t off, bufferlist& bl, bool buffered, int write_hint)
{
    ...
    dout(5) << __func__ << " 0x" << std::hex << off << "~" << len << std::dec << " " << buffermode(buffered) << dendl;
    ...
    if (buffered) {
        ...
        auto r = ::sync_file_range(fd_buffereds[WRITE_LIFE_NOT_SET],
                                   off,
                                   len,
...
int KernelDevice::write(uint64_t off, bufferlist& bl, bool buffered, int write_hint)
{
    uint64_t len = bl.length();
    dout(20) << __func__ << " 0x" << std::hex << off << "~" << len << std::dec << " " << buffermode(buffered) << dendl;
...
int KernelDevice::aio_write(uint64_t off, bufferlist& bl, IOContext* ioc, bool buffered, int write_hint)
{
    ...
    *_dout << dendl;
    ...
    if (aio && dio && !buffered) {
        if (cct->_conf->bdev_inject_crash && rand() % cct->_conf->bdev_inject_crash == 0) {
            derr << __func__ << " bdev_inject_crash: dropping io 0x" << std::hex << off << "~" << len << std::dec
                 << dendl;
            ...
            aio.bl.push_back(ceph::buffer::ptr_node::create(ceph::buffer::create_small_page_aligned(len)));
            ...
            if (bl.length() <= RW_IO_MAX) {
                ...
                while (prev_len < bl.length()) {
                    ...
                    dout(30) << aio << dendl;
                    dout(5) << __func__ << " 0x" << std::hex << off + prev_len << "~" << len << std::dec << " aio "
                            << &aio << " (piece)" << dendl;
                    prev_len += len;
                }
            }
...
struct ExplicitHugePagePool
{
    ...
    using instrumented_raw = ceph::buffer_instrumentation::instrumented_raw<BlockDevice::hugepaged_raw_marker_t>;
    ...
    ceph::unique_leakable_ptr<buffer::raw> try_create()
    {
        if (void* mmaped_region; region_q.pop(mmaped_region)) {
            return ceph::unique_leakable_ptr<buffer::raw>{new mmaped_buffer_raw(mmaped_region, *this)};
        }
        else {
            // oops, empty queue.
            return nullptr;
...
struct HugePagePoolOfPools
{
    HugePagePoolOfPools(const std::map<size_t, size_t> conf)
        : pools(conf.size(), [conf](size_t index, auto emplacer) {
            ...
            const auto [buffer_size, buffers_in_pool] = *std::next(std::begin(conf), index);
    ...
    ceph::unique_leakable_ptr<buffer::raw> try_create(const size_t size)
    {
        ...
        if (auto iter = std::find_if(std::begin(pools),
                                     std::end(pools),
                                     [size](const auto& pool) { return size == pool.get_buffer_size(); });
            iter != std::end(pools)) {
...
ceph::unique_leakable_ptr<buffer::raw> KernelDevice::create_custom_aligned(const size_t len, IOContext* const ioc) const
{
    ...
    if (len < CEPH_PAGE_SIZE) {
        ...
        static HugePagePoolOfPools hp_pools =
            HugePagePoolOfPools::from_desc(cct->_conf.get_val<std::string>("bdev_read_preallocated_huge_buffers"));
        if (auto lucky_raw = hp_pools.try_create(len); lucky_raw) {
            dout(20) << __func__ << " allocated from huge pool"
                     << " lucky_raw.data=" << (void*)lucky_raw->get_data() << " bdev_read_preallocated_huge_buffers="
                     << cct->_conf.get_val<std::string>("bdev_read_preallocated_huge_buffers") << dendl;
...
int KernelDevice::read(uint64_t off, uint64_t len, bufferlist* pbl, IOContext* ioc, bool buffered)
{
    ...
    int r = ::pread(choose_fd(buffered, WRITE_LIFE_NOT_SET), p->c_str(), len, off);
    auto age = cct->_conf->bdev_debug_aio_log_age;
    if (mono_clock::now() - start1 >= make_timespan(age)) {
        derr << __func__ << " stalled read "
             << " 0x" << std::hex << off << "~" << len << std::dec << " " << buffermode(buffered) << " since " << start1
             << ", timeout is " << age << "s" << dendl;
    }
    if (r < 0) {
        ...
        derr << __func__ << " 0x" << std::hex << off << "~" << std::left << std::dec << " error: " << cpp_strerror(r)
             << dendl;
        goto out;
    }
...
int KernelDevice::aio_read(uint64_t off, uint64_t len, bufferlist* pbl, IOContext* ioc)
{
    ...
    if (aio && dio) {
        ...
        aio.bl.push_back(ceph::buffer::ptr_node::create(create_custom_aligned(len, ioc)));
        aio.bl.prepare_iov(&aio.iov);
        aio.preadv(off, len);
        dout(30) << aio << dendl;
        pbl->append(aio.bl);
...
int KernelDevice::direct_read_unaligned(uint64_t off, uint64_t len, char* buf)
{
    ...
    if (mono_clock::now() - start1 >= make_timespan(age)) {
        derr << __func__ << " stalled read "
             << " 0x" << std::hex << off << "~" << len << std::dec << " since " << start1 << ", timeout is " << age
             << "s" << dendl;
    }

    if (r < 0) {
        r = -errno;
        derr << __func__ << " 0x" << std::hex << off << "~" << len << std::dec << " error: " << cpp_strerror(r)
             << dendl;
        goto out;
    }
...
int KernelDevice::read_random(uint64_t off, uint64_t len, char* buf, bool buffered)
{
    ...
    if (!buffered && ((off % block_size != 0) || (len % block_size != 0) || (uintptr_t(buf) % CEPH_PAGE_SIZE != 0)))
    ...
    if (buffered) {
        ...
        while (left > 0) {
            ...
            if (r < 0) {
                r = -errno;
                derr << __func__ << " 0x" << std::hex << off << "~" << left << std::dec << " error: " << cpp_strerror(r)
                     << dendl;
                goto out;
            }
            off += r;
        ...
        if (mono_clock::now() - start1 >= make_timespan(age)) {
            derr << __func__ << " stalled read "
                 << " 0x" << std::hex << off0 << "~" << len << std::dec << " (buffered) since " << start1
                 << ", timeout is " << age << "s" << dendl;
        }
    }
    else {
        ...
        if (mono_clock::now() - start1 >= make_timespan(age)) {
            derr << __func__ << " stalled read "
                 << " 0x" << std::hex << off << "~" << len << std::dec << " (direct) since " << start1
                 << ", timeout is " << age << "s" << dendl;
        }
        if (r < 0) {
            r = -errno;
            derr << __func__ << " direct_aligned_read"
                 << " 0x" << std::hex << off << "~" << std::left << std::dec << " error: " << cpp_strerror(r) << dendl;
            goto out;
        }
        ceph_assert((uint64_t)r == len);
    }
