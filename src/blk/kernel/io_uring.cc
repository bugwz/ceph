...
static int ioring_get_cqe(struct ioring_data* d, unsigned int max, struct aio_t** paio)
{
...
static int find_fixed_fd(struct ioring_data* d, int real_fd)
{
    ...
    if (it == d->fixed_fds_map.end()) return -1;

    return it->second;
}

static void init_sqe(struct ioring_data* d, struct io_uring_sqe* sqe, struct aio_t* io)
{
...
static int ioring_queue(struct ioring_data* d, void* priv, list<aio_t>::iterator beg, list<aio_t>::iterator end)
{
    struct io_uring* ring = &d->io_uring;
    ...
    if (!io) /* Queue is full, go and reap something first */
        return 0;
...
int ioring_queue_t::init(std::vector<int>& fds)
{
    ...
    if (hipri) flags |= IORING_SETUP_IOPOLL;
    if (sq_thread) flags |= IORING_SETUP_SQPOLL;
...
int ioring_queue_t::submit_batch(aio_iter beg, aio_iter end, uint16_t aios_size, void* priv, int* retries)
{
    (void)aios_size;
    (void)retries;
