...
{
    if (cmount->is_mounted()) return -EISCONN;
    delete cmount;
    cmount = nullptr;
    return 0;
}
...
{
    if (!cmount->is_initialized()) return -ENOTCONN;
...
{
    if (!cmount->is_mounted()) return -ENOTCONN;
...
{
    if (!cmount->is_mounted()) return -ENOTCONN;
    if (flags & ~CEPH_REQ_FLAG_MASK) return -EINVAL;
...
extern "C" int ceph_get_file_pool_name(struct ceph_mount_info* cmount, int fh, char* buf, size_t len)
{
    ...
    if (name.length() > len) return -ERANGE;
                                         ...
                                         int* osds, int nosds)
{
    ...
    if ((int)vosds.size() > nosds) return -ERANGE;
                                            ...
                                            struct sockaddr_storage* addr, int naddr)
{
    ...
    if (naddr && (address.size() > (unsigned)naddr)) return -ERANGE;
...
extern "C" int ceph_ll_create(class ceph_mount_info* cmount, Inode* parent, const char* name, mode_t mode, int oflags,
                              ...
                              const UserPerm* perms)
{
    if (lflags & ~CEPH_REQ_FLAG_MASK) return -EINVAL;
