...
template<typename I> std::ostream& operator<<(std::ostream& os, const typename SnapshotProtectRequest<I>::State& state)
{
    switch (state) {
    case SnapshotProtectRequest<I>::STATE_PROTECT_SNAP: os << "PROTECT_SNAP"; break;
    }
    return os;
}
...
template<typename I>
SnapshotProtectRequest<I>::SnapshotProtectRequest(I& image_ctx, Context* on_finish,
...
template<typename I> bool SnapshotProtectRequest<I>::should_complete(int r)
{
...
template<typename I> void SnapshotProtectRequest<I>::send_protect_snap()
{
    I& image_ctx = this->m_image_ctx;
...
template<typename I> int SnapshotProtectRequest<I>::verify_and_send_protect_snap()
{
    I& image_ctx = this->m_image_ctx;
    ...
    cls_client::set_protection_status(&op, snap_id, RBD_PROTECTION_STATUS_PROTECTED);

    librados::AioCompletion* rados_completion = this->create_callback_completion();
    r = image_ctx.md_ctx.aio_operate(image_ctx.header_oid, rados_completion, &op);
    ceph_assert(r == 0);
