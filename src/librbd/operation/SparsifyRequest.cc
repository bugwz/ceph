...
bool may_be_trimmed(const std::map<uint64_t, uint64_t>& extent_map, const bufferlist& bl, size_t sparse_size,
...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::operation::SparsifyObject: " << this << " " << m_oid << " " << __func__ << ": "

template<typename I> class C_SparsifyObject : public C_AsyncObjectThrottle<I>
{
public:
    /**
     * @verbatim
    ...
    C_SparsifyObject(AsyncObjectThrottle<I>& throttle, I* image_ctx, uint64_t object_no, size_t sparse_size)
    ...
    int send() override
    {
...

        {
            ...
            ceph_assert(r == 0);
            if (raw_overlap > 0) {
                auto [parent_extents, area] =
                    io::util::object_to_area_extents(&image_ctx, m_object_no, {{0, image_ctx.layout.object_size}});
    ...
    void send_sparsify()
    {
        ...
        auto comp = create_rados_callback<C_SparsifyObject, &C_SparsifyObject::handle_sparsify>(this);
    ...
    void send_pre_update_object_map()
    {
        ...
        if (m_trying_trim) {
            if (!m_remove_empty || m_new_end != 0 || !image_ctx.test_features(RBD_FEATURE_OBJECT_MAP)) {
                send_trim();
                return;
            }
        }
        else if (!m_remove_empty || !image_ctx.test_features(RBD_FEATURE_OBJECT_MAP)) {
            finish_op(0);
            return;
        }
        ...
        auto ctx =
            create_context_callback<C_SparsifyObject<I>, &C_SparsifyObject<I>::handle_pre_update_object_map>(this);

        bool sent = image_ctx.object_map->template aio_update<Context, &Context::complete>(
            CEPH_NOSNAP, m_object_no, OBJECT_PENDING, OBJECT_EXISTS, {}, false, ctx);
    ...
    void send_check_exists()
    {
        ...
        m_bl.clear();
        auto comp = create_rados_callback<C_SparsifyObject, &C_SparsifyObject::handle_check_exists>(this);
    ...
    void send_post_update_object_map(bool exists)
    {
        ...
        auto ctx =
            create_context_callback<C_SparsifyObject<I>, &C_SparsifyObject<I>::handle_post_update_object_map>(this);
        bool sent;
        {
            ...
            sent = image_ctx.object_map->template aio_update<Context, &Context::complete>(
                CEPH_NOSNAP, m_object_no, exists ? OBJECT_EXISTS : OBJECT_NONEXISTENT, OBJECT_PENDING, {}, false, ctx);
        }
        if (!sent) {
            ctx->complete(0);
        }
    ...
    void send_read()
    {
        ...
        librados::ObjectReadOperation op;
        m_bl.clear();
        op.sparse_read(0, image_ctx.layout.object_size, &m_extent_map, &m_bl, nullptr);
        auto comp = create_rados_callback<C_SparsifyObject, &C_SparsifyObject::handle_read>(this);
        int r = image_ctx.data_ctx.aio_operate(m_oid, comp, &op, &m_bl);
        ceph_assert(r == 0);
        comp->release();
    }
    ...
    void send_trim()
    {
        ...
        auto comp = create_rados_callback<C_SparsifyObject, &C_SparsifyObject::handle_trim>(this);
        int r = image_ctx.data_ctx.aio_operate(m_oid, comp, &op);
        ceph_assert(r == 0);
        comp->release();
    }
...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::operation::SparsifyRequest: " << this << " " << __func__ << ": "

template<typename I> bool SparsifyRequest<I>::should_complete(int r)
{
    I& image_ctx = this->m_image_ctx;
...
template<typename I> void SparsifyRequest<I>::send_op()
{
    sparsify_objects();
}

template<typename I> void SparsifyRequest<I>::sparsify_objects()
{
    I& image_ctx = this->m_image_ctx;
    ceph_assert(ceph_mutex_is_locked(image_ctx.owner_lock));
    ...
    auto ctx = create_context_callback<SparsifyRequest<I>, &SparsifyRequest<I>::handle_sparsify_objects>(this);
    ...
    AsyncObjectThrottle<I>* throttle =
        new AsyncObjectThrottle<I>(this, image_ctx, context_factory, ctx, &m_prog_ctx, 0, objects);
    throttle->start_ops(image_ctx.config.template get_val<uint64_t>("rbd_concurrent_management_ops"));
}

template<typename I> void SparsifyRequest<I>::handle_sparsify_objects(int r)
{
    I& image_ctx = this->m_image_ctx;
    CephContext* cct = image_ctx.cct;
    ldout(cct, 5) << "r=" << r << dendl;
