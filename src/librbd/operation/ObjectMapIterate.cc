...
template<typename I> class C_VerifyObjectCallback : public C_AsyncObjectThrottle<I>
{
public:
    C_VerifyObjectCallback(AsyncObjectThrottle<I>& throttle, I* image_ctx, uint64_t snap_id, uint64_t object_no,
                           ObjectIterateWork<I> handle_mismatch, std::atomic_flag* invalidate)
...
private:
    ...
    uint8_t get_object_state()
    {
        ...
        for (std::vector<librados::clone_info_t>::const_iterator r = m_snap_set.clones.begin();
             r != m_snap_set.clones.end();
             ++r) {
            ...
            if ((image_ctx.features & RBD_FEATURE_FAST_DIFF) != 0 && from_snap_id != m_snap_id) {
    ...
    uint64_t next_valid_snap_id(uint64_t snap_id)
    {
        ...
        std::map<librados::snap_t, SnapInfo>::iterator it = image_ctx.snap_info.lower_bound(snap_id);
    ...
    bool object_map_action(uint8_t new_state)
    {
        ...
        ldout(cct, 10) << "C_VerifyObjectCallback::object_map_action"
                       << " object " << image_ctx.get_object_name(m_object_no) << " state " << (int)state
        ...
        if (state != new_state) {
            ...
            if (r) {
                lderr(cct) << "object map error: object " << image_ctx.get_object_name(m_object_no) << " marked as "
                           << (int)state << ", but should be " << (int)new_state << dendl;
                m_invalidate->test_and_set();
            }
            else {
                ldout(cct, 1) << "object map inconsistent: object " << image_ctx.get_object_name(m_object_no)
                              << " marked as " << (int)state << ", but should be " << (int)new_state << dendl;
            }
        }
...
template<typename I> void ObjectMapIterateRequest<I>::send()
{
...
template<typename I> bool ObjectMapIterateRequest<I>::should_complete(int r)
{
    CephContext* cct = m_image_ctx.cct;
    ldout(cct, 5) << this << " should_complete: "
                  << " r=" << r << dendl;
    ...
    if (r < 0) {
        lderr(cct) << "object map operation encountered an error: " << cpp_strerror(r) << dendl;
    }
...
template<typename I> void ObjectMapIterateRequest<I>::send_verify_objects()
{
    ...
    uint64_t num_objects;
    {
        ...
        num_objects = Striper::get_num_objects(m_image_ctx.layout, m_image_ctx.get_image_size(snap_id));
    }
    ...
    AsyncObjectThrottle<I>* throttle = new AsyncObjectThrottle<I>(
        this, m_image_ctx, context_factory, this->create_callback_context(), &m_prog_ctx, 0, num_objects);
    throttle->start_ops(m_image_ctx.config.template get_val<uint64_t>("rbd_concurrent_management_ops"));
}

template<typename I> uint64_t ObjectMapIterateRequest<I>::get_image_size() const
{
...
template<typename I> void ObjectMapIterateRequest<I>::send_invalidate_object_map()
{
    CephContext* cct = m_image_ctx.cct;
    ...
    object_map::InvalidateRequest<I>* req = object_map::InvalidateRequest<I>::create(
        m_image_ctx, m_image_ctx.snap_id, true, this->create_callback_context());
