...
public:
    ...
    static GetMetadataRequest* create(IoCtx& io_ctx, const std::string& oid, bool filter_internal,
                                      ...
                                      uint32_t max_results, KeyValues* key_values, Context* on_finish)
    {
        return new GetMetadataRequest(
            io_ctx, oid, filter_internal, filter_key_prefix, last_key, max_results, key_values, on_finish);
    }

    GetMetadataRequest(IoCtx& io_ctx, const std::string& oid, bool filter_internal,
                       const std::string& filter_key_prefix, const std::string& last_key, uint32_t max_results,
