...
template<typename I>
SetFlagsRequest<I>::SetFlagsRequest(I* image_ctx, uint64_t flags, uint64_t mask, Context* on_finish)
...
template<typename I> void SetFlagsRequest<I>::send_set_flags()
{
    ...
    Context* ctx = create_context_callback<SetFlagsRequest<I>, &SetFlagsRequest<I>::handle_set_flags>(this);
...
template<typename I> Context* SetFlagsRequest<I>::handle_set_flags(int* result)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << __func__ << ": r=" << *result << dendl;

    if (*result < 0) {
        lderr(cct) << "set_flags failed: " << cpp_strerror(*result) << dendl;
    }
    return m_on_finish;
}
