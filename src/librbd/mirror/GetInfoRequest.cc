...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::mirror::GetInfoRequest: " << this << " " << __func__ << ": "
...
template<typename I>
GetInfoRequest<I>::GetInfoRequest(librados::IoCtx& io_ctx, asio::ContextWQ* op_work_queue, const std::string& image_id,
                                  cls::rbd::MirrorImage* mirror_image, PromotionState* promotion_state,
...
template<typename I>
GetInfoRequest<I>::GetInfoRequest(I& image_ctx, cls::rbd::MirrorImage* mirror_image, PromotionState* promotion_state,
                                  std::string* primary_mirror_uuid, Context* on_finish)
...
template<typename I> void GetInfoRequest<I>::get_mirror_image()
{
    ...
    librados::AioCompletion* comp =
        create_rados_callback<GetInfoRequest<I>, &GetInfoRequest<I>::handle_get_mirror_image>(this);
    int r = m_io_ctx.aio_operate(RBD_MIRRORING, comp, &op, &m_out_bl);
    ceph_assert(r == 0);
    comp->release();
}

template<typename I> void GetInfoRequest<I>::handle_get_mirror_image(int r)
{
    ...
    *m_promotion_state = PROMOTION_STATE_NON_PRIMARY;
    ...
    if (r == -ENOENT) {
        ...
        lderr(m_cct) << "failed to retrieve mirroring state: " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
...
template<typename I> void GetInfoRequest<I>::get_journal_tag_owner()
{
    ldout(m_cct, 20) << dendl;

    auto ctx = create_context_callback<GetInfoRequest<I>, &GetInfoRequest<I>::handle_get_journal_tag_owner>(this);
    Journal<I>::get_tag_owner(m_io_ctx, m_image_id, &m_mirror_uuid, m_op_work_queue, ctx);
}

template<typename I> void GetInfoRequest<I>::handle_get_journal_tag_owner(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;

    if (r < 0) {
        lderr(m_cct) << "failed to determine tag ownership: " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
...
template<typename I> void GetInfoRequest<I>::get_snapcontext()
{
    ...
    librados::AioCompletion* comp =
        create_rados_callback<GetInfoRequest<I>, &GetInfoRequest<I>::handle_get_snapcontext>(this);
    m_out_bl.clear();
    int r = m_io_ctx.aio_operate(util::header_name(m_image_id), comp, &op, &m_out_bl);
    ceph_assert(r == 0);
    comp->release();
}

template<typename I> void GetInfoRequest<I>::handle_get_snapcontext(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;
    ...
    if (r == -ENOENT && m_mirror_image->state == cls::rbd::MIRROR_IMAGE_STATE_CREATING) {
...
template<typename I> void GetInfoRequest<I>::get_snapshots()
{
    ldout(m_cct, 20) << dendl;
    ...
    librados::AioCompletion* comp =
        create_rados_callback<GetInfoRequest<I>, &GetInfoRequest<I>::handle_get_snapshots>(this);
    m_out_bl.clear();
    int r = m_io_ctx.aio_operate(util::header_name(m_image_id), comp, &op, &m_out_bl);
    ceph_assert(r == 0);
    comp->release();
...
template<typename I> void GetInfoRequest<I>::handle_get_snapshots(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;
    ...
    for (auto snap_id : m_snapc.snaps) {
        ...
        snap_info.emplace(snap_id, SnapInfo(snap.name, snap.snapshot_namespace, 0, {}, 0, 0, {}));
    }
...
template<typename I> void GetInfoRequest<I>::calc_promotion_state(const std::map<librados::snap_t, SnapInfo>& snap_info)
{
    *m_promotion_state = PROMOTION_STATE_UNKNOWN;
    *m_primary_mirror_uuid = "";

    for (auto it = snap_info.rbegin(); it != snap_info.rend(); it++) {
        auto mirror_ns = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&it->second.snap_namespace);

        if (mirror_ns != nullptr) {
            switch (mirror_ns->state) {
            case cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY: *m_promotion_state = PROMOTION_STATE_PRIMARY; break;
