...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::mirror::snapshot::GetImageStateRequest: " << this << " " << __func__ << ": "
...
template<typename I> void GetImageStateRequest<I>::read_object()
{
    ...
    librados::AioCompletion* comp =
        create_rados_callback<GetImageStateRequest<I>, &GetImageStateRequest<I>::handle_read_object>(this);
    int r = m_image_ctx->md_ctx.aio_operate(oid, comp, &op, nullptr);
    ceph_assert(r == 0);
    comp->release();
}

template<typename I> void GetImageStateRequest<I>::handle_read_object(int r)
{
    CephContext* cct = m_image_ctx->cct;
...
template<typename I> void GetImageStateRequest<I>::finish(int r)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 15) << "r=" << r << dendl;
