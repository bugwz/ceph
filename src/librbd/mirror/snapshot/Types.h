...
struct ImageStateHeader
{
    ...
    ImageStateHeader() {}
    ImageStateHeader(uint32_t object_count)
...
struct SnapState
{
    ...
    {
        return snap_namespace == rhs.snap_namespace && name == rhs.name && protection_status == rhs.protection_status;
    }
...
struct ImageState
{
    ...
    ImageState(const std::string& name, uint64_t features, uint64_t snap_limit,
               const std::map<uint64_t, SnapState>& snapshots, const std::map<std::string, bufferlist>& metadata)
        : name(name)
        , features(features)
    ...
    {
        return name == rhs.name && features == rhs.features && snap_limit == rhs.snap_limit &&
               snapshots == rhs.snapshots;
    }
