...
template<typename ImageCtxT = librbd::ImageCtx> class CreateNonPrimaryRequest
{
public:
    static CreateNonPrimaryRequest* create(ImageCtxT* image_ctx, bool demoted, const std::string& primary_mirror_uuid,
                                           ...
                                           const ImageState& image_state, uint64_t* snap_id, Context* on_finish)
    {
        return new CreateNonPrimaryRequest(
            image_ctx, demoted, primary_mirror_uuid, primary_snap_id, snap_seqs, image_state, snap_id, on_finish);
    }

    CreateNonPrimaryRequest(ImageCtxT* image_ctx, bool demoted, const std::string& primary_mirror_uuid,
                            uint64_t primary_snap_id, const SnapSeqs& snap_seqs, const ImageState& image_state,
...
private:
    ...
    bool is_orphan() const { return m_primary_mirror_uuid.empty(); }
