...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::mirror::PromoteRequest: " << this << " " << __func__ << ": "
...
template<typename I> void PromoteRequest<I>::get_info()
{
    ...
    auto ctx = create_context_callback<PromoteRequest<I>, &PromoteRequest<I>::handle_get_info>(this);
    auto req = GetInfoRequest<I>::create(m_image_ctx, &m_mirror_image, &m_promotion_state, &m_primary_mirror_uuid, ctx);
    req->send();
}

template<typename I> void PromoteRequest<I>::handle_get_info(int r)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    if (r < 0) {
        ...
        lderr(cct) << "image is primary within a remote cluster or demotion is not propagated yet" << dendl;
        finish(-EBUSY);
        return;
    }
...
template<typename I> void PromoteRequest<I>::promote()
{
    ...
    auto ctx = create_context_callback<PromoteRequest<I>, &PromoteRequest<I>::handle_promote>(this);
