...
template<typename ImageCtxT = ImageCtx> class DisableRequest
{
public:
    static DisableRequest* create(ImageCtxT* image_ctx, bool force, bool remove, Context* on_finish)
    {
        return new DisableRequest(image_ctx, force, remove, on_finish);
    }

    DisableRequest(ImageCtxT* image_ctx, bool force, bool remove, Context* on_finish);
...
private:
    ...
    mutable ceph::mutex m_lock = ceph::make_mutex("mirror::DisableRequest::m_lock");
    ...
    void send_remove_snap(const std::string& client_id, const cls::rbd::SnapshotNamespace& snap_namespace,
    ...
    Context* create_context_callback(Context* (DisableRequest<ImageCtxT>::*handle)(int*, const std::string& client_id),
