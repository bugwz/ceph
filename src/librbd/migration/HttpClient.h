...
template<typename ImageCtxT> class HttpClient
{
public:
    ...
    static HttpClient* create(ImageCtxT* image_ctx, const std::string& url) { return new HttpClient(image_ctx, url); }
    ...
    void set_ignore_self_signed_cert(bool ignore) { m_ignore_self_signed_cert = ignore; }

    void set_http_processor(HttpProcessorInterface* http_processor) { m_http_processor = http_processor; }

    template<class Body, typename Completion>
    void issue(boost::beast::http::request<Body>&& request, Completion&& completion)
    {
        struct WorkImpl : Work
        {
            ...
            WorkImpl(HttpClient* http_client, boost::beast::http::request<Body>&& request, Completion&& completion)
            ...
            bool need_eof() const override { return request.need_eof(); }

            bool header_only() const override { return (request.method() == boost::beast::http::verb::head); }

            void complete(int r, Response&& response) override { completion(r, std::move(response)); }
            ...
            void operator()(boost::beast::tcp_stream& stream) override
            {
                ...
                boost::beast::http::async_write(
                    ...
                    [http_session = http_client->m_http_session.get(),
                     work = this->shared_from_this()](boost::beast::error_code ec, std::size_t) mutable {
                        http_session->handle_issue(ec, std::move(work));
                    });
            }

            void operator()(boost::beast::ssl_stream<boost::beast::tcp_stream>& stream) override
            {
                preprocess_request();
        ...
        issue(std::make_shared<WorkImpl>(this, std::move(request), std::move(completion)));
    }
...
private:
    ...
    struct HttpSessionInterface
    {
        ...
        virtual void handle_issue(boost::system::error_code ec, std::shared_ptr<Work>&& work) = 0;
    };
    ...
    struct Work : public std::enable_shared_from_this<Work>
    {
        ...
        virtual void operator()(boost::beast::ssl_stream<boost::beast::tcp_stream>& stream) = 0;
    };
    ...
    template<typename Fields> void initialize_default_fields(Fields& fields) const
    {
        fields.target(m_url_spec.path);
        fields.set(boost::beast::http::field::host, m_url_spec.host);
        fields.set(boost::beast::http::field::user_agent, BOOST_BEAST_VERSION_STRING);
    }

    void handle_get_size(int r, Response&& response, uint64_t* size, Context* on_finish);

    void handle_read(int r, Response&& response, uint64_t byte_offset, uint64_t byte_length, bufferlist* data,
                     Context* on_finish);
