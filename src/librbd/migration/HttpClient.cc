...
#undef dout_prefix
#define dout_prefix                             \
    *_dout << "librbd::migration::HttpClient::" \
...
template<typename I> template<typename D> class HttpClient<I>::HttpSession : public HttpSessionInterface
{
public:
    ...
    void handle_issue(boost::system::error_code ec, std::shared_ptr<Work>&& work) override
    {
        ceph_assert(m_http_client->m_strand.running_in_this_thread());
        ...
        if (ec) {
            if (ec == boost::asio::error::bad_descriptor || ec == boost::asio::error::broken_pipe ||
                ec == boost::asio::error::connection_reset || ec == boost::asio::error::operation_aborted ||
                ec == boost::asio::ssl::error::stream_truncated || ec == boost::beast::http::error::end_of_stream ||
...
protected:
    ...
    HttpSession(HttpClient* http_client)
        ...
        , m_resolver(http_client->m_strand)
    {}

    virtual void connect(boost::asio::ip::tcp::resolver::results_type results, Context* on_finish) = 0;
...
private:
    ...
    void resolve_host(Context* on_finish)
    {
        ...
        shutdown_socket();
        m_resolver.async_resolve(m_http_client->m_url_spec.host,
                                 m_http_client->m_url_spec.port,
                                 [this, on_finish](boost::system::error_code ec, auto results) {
                                     handle_resolve_host(ec, results, on_finish);
                                 });
    }

    void handle_resolve_host(boost::system::error_code ec, boost::asio::ip::tcp::resolver::results_type results,
                             Context* on_finish)
    {
        auto cct = m_http_client->m_cct;
        int r = -ec.value();
        ...
        if (ec) {
            ...
            lderr(cct) << "failed to resolve host '" << m_http_client->m_url_spec.host << "': " << cpp_strerror(r)
                       << dendl;
            advance_state(STATE_UNINITIALIZED, r, on_finish);
            return;
        }

        connect(results, new LambdaContext([this, on_finish](int r) { handle_connect(r, on_finish); }));
    }
    ...
    void handle_connect(int r, Context* on_finish)
    {
        ...
        if (r < 0) {
            lderr(cct) << "failed to connect to host '" << m_http_client->m_url_spec.host << "': " << cpp_strerror(r)
                       << dendl;
            advance_state(STATE_UNINITIALIZED, r, on_finish);
            return;
        }
    ...
    bool maybe_finalize_reset()
    {
        ...
        m_issue_queue.insert(m_issue_queue.begin(), m_receive_queue.begin(), m_receive_queue.end());
    ...
    int shutdown_socket()
    {
        if (!boost::beast::get_lowest_layer(derived().stream()).socket().is_open()) {
            return 0;
        }
    ...
    void receive(std::shared_ptr<Work>&& work)
    {
        ...
        if (work->header_only()) {
            ...
            boost::beast::http::async_read_header(
                ...
                m_buffer,
                *m_header_parser,
                [this, work = std::move(work)](boost::beast::error_code ec, std::size_t) mutable {
    ...
    void handle_receive(boost::system::error_code ec, std::shared_ptr<Work>&& work)
    {
        auto cct = m_http_client->m_cct;
        ...
        else if (boost::beast::http::to_status_class(result) != boost::beast::http::status_class::successful) {
...
template<typename I> class HttpClient<I>::PlainHttpSession : public HttpSession<PlainHttpSession>
{
public:
    ...
    inline boost::beast::tcp_stream& stream() { return m_stream; }

protected:
    void connect(boost::asio::ip::tcp::resolver::results_type results, Context* on_finish) override
    {
        ...
        m_stream.async_connect(
            results, asio::util::get_callback_adapter([on_finish](int r, auto endpoint) { on_finish->complete(r); }));
    }

    void disconnect(Context* on_finish) override { on_finish->complete(0); }
...
template<typename I> class HttpClient<I>::SslHttpSession : public HttpSession<SslHttpSession>
{
public:
    ...
    inline boost::beast::ssl_stream<boost::beast::tcp_stream>& stream() { return m_stream; }
...
protected:
    void connect(boost::asio::ip::tcp::resolver::results_type results, Context* on_finish) override
    {
        ...
        boost::beast::get_lowest_layer(m_stream).async_connect(
            results, asio::util::get_callback_adapter([this, on_finish](int r, auto endpoint) {
    ...
    void disconnect(Context* on_finish) override
    {
        ...
        m_stream.async_shutdown(asio::util::get_callback_adapter([this, on_finish](int r) { shutdown(r, on_finish); }));
    }
...
private:
    ...
    void handle_connect(int r, Context* on_finish)
    {
        ...
        if (r < 0) {
            lderr(cct) << "failed to connect to host '" << http_client->m_url_spec.host << "': " << cpp_strerror(r)
                       << dendl;
            on_finish->complete(r);
            return;
        }
    ...
    void handshake(Context* on_finish)
    {
        ...
        m_stream.set_verify_mode(boost::asio::ssl::verify_peer | boost::asio::ssl::verify_fail_if_no_peer_cert);
        m_stream.set_verify_callback([host,
                                      next = boost::asio::ssl::host_name_verification(host),
        ...
        if (!SSL_set_tlsext_host_name(m_stream.native_handle(), http_client->m_url_spec.host.c_str())) {
            int r = -::ERR_get_error();
            lderr(cct) << "failed to initialize SNI hostname: " << cpp_strerror(r) << dendl;
            on_finish->complete(r);
            return;
        }
        ...
        m_stream.async_handshake(
            ...
            asio::util::get_callback_adapter([this, on_finish](int r) { handle_handshake(r, on_finish); }));
    }
    ...
    void handle_handshake(int r, Context* on_finish)
    {
        ...
        if (r < 0) {
            lderr(cct) << "failed to complete handshake: " << cpp_strerror(r) << dendl;
            disconnect(new LambdaContext([r, on_finish](int) { on_finish->complete(r); }));
            return;
        }
...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::HttpClient: " << this << " " << __func__ << ": "
...
template<typename I> void HttpClient<I>::open(Context* on_finish)
{
    ldout(m_cct, 10) << "url=" << m_url << dendl;

    int r = util::parse_url(m_cct, m_url, &m_url_spec);
    if (r < 0) {
        lderr(m_cct) << "failed to parse url '" << m_url << "': " << cpp_strerror(r) << dendl;
        on_finish->complete(-EINVAL);
        return;
    }

    boost::asio::post(m_strand, [this, on_finish]() mutable { create_http_session(on_finish); });
}

template<typename I> void HttpClient<I>::close(Context* on_finish)
{
    boost::asio::post(m_strand, [this, on_finish]() mutable { shut_down_http_session(on_finish); });
}

template<typename I> void HttpClient<I>::get_size(uint64_t* size, Context* on_finish)
{
...
template<typename I> void HttpClient<I>::handle_get_size(int r, Response&& response, uint64_t* size, Context* on_finish)
{
...
template<typename I> void HttpClient<I>::read(io::Extents&& byte_extents, bufferlist* data, Context* on_finish)
{
    ldout(m_cct, 20) << dendl;

    auto aio_comp =
        io::AioCompletion::create_and_start(on_finish, librbd::util::get_image_ctx(m_image_ctx), io::AIO_TYPE_READ);
    ...
    for (auto [byte_offset, byte_length] : byte_extents) {
        auto ctx = new io::ReadResult::C_ImageReadRequest(aio_comp, buffer_offset, {{byte_offset, byte_length}});
        ...
        issue(std::move(req),
              [this, byte_offset = byte_offset, byte_length = byte_length, ctx](int r, Response&& response) {
                  handle_read(r, std::move(response), byte_offset, byte_length, &ctx->bl, ctx);
              });
    }
}

template<typename I>
void HttpClient<I>::handle_read(int r, Response&& response, uint64_t byte_offset, uint64_t byte_length,
                                bufferlist* data, Context* on_finish)
{
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to read requested byte range: " << cpp_strerror(r) << dendl;
        on_finish->complete(r);
        return;
    }
    else if (response.result() != boost::beast::http::status::partial_content) {
        lderr(m_cct) << "failed to retrieve requested byte range: HTTP " << response.result() << dendl;
...
template<typename I> void HttpClient<I>::issue(std::shared_ptr<Work>&& work)
{
    boost::asio::post(m_strand, [this, work = std::move(work)]() mutable { m_http_session->issue(std::move(work)); });
}

template<typename I> void HttpClient<I>::create_http_session(Context* on_finish)
{
    ldout(m_cct, 15) << dendl;

    ceph_assert(m_http_session == nullptr);
    switch (m_url_spec.scheme) {
    case URL_SCHEME_HTTP: m_http_session = std::make_unique<PlainHttpSession>(this); break;
    case URL_SCHEME_HTTPS: m_http_session = std::make_unique<SslHttpSession>(this); break;
    default: ceph_assert(false); break;
    }

    m_http_session->init(on_finish);
}

template<typename I> void HttpClient<I>::shut_down_http_session(Context* on_finish)
{
    ldout(m_cct, 15) << dendl;
