...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::RawSnapshot::OpenRequest " << this << " " << __func__ << ": "

template<typename I> struct RawSnapshot<I>::OpenRequest
{
    ...
    void open_stream()
    {
        ...
        auto ctx = util::create_context_callback<OpenRequest, &OpenRequest::handle_open_stream>(this);
    ...
    void get_image_size()
    {
        ...
        auto ctx = util::create_context_callback<OpenRequest, &OpenRequest::handle_get_image_size>(this);
...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::RawSnapshot: " << this << " " << __func__ << ": "

template<typename I>
RawSnapshot<I>::RawSnapshot(I* image_ctx, const json_spirit::mObject& json_object,
                            const SourceSpecBuilder<I>* source_spec_builder, uint64_t index)
...
template<typename I> void RawSnapshot<I>::open(SnapshotInterface* previous_snapshot, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
...
template<typename I> void RawSnapshot<I>::close(Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;
...
template<typename I>
void RawSnapshot<I>::read(io::AioCompletion* aio_comp, io::Extents&& image_extents, io::ReadResult&& read_result,
...
template<typename I>
void RawSnapshot<I>::list_snap(io::Extents&& image_extents, int list_snaps_flags, io::SparseExtents* sparse_extents,
                               const ZTracer::Trace& parent_trace, Context* on_finish)
{
    ...
    for (auto& [image_offset, image_length] : image_extents) {
        sparse_extents->insert(image_offset, image_length, {io::SPARSE_EXTENT_STATE_DATA, image_length});
    }
