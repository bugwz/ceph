    ...
    if (*previous_size && **previous_size > new_size) {
        ...
        for (auto& image_extent : image_extents) {
            ...
            for (auto [image_offset, image_length] : image_interval) {
                ldout(cct, 20) << "zeroing extent " << image_offset << "~" << image_length << " at snapshot " << snap_id
                               << dendl;
                sparse_extents->insert(image_offset, image_length, {io::SPARSE_EXTENT_STATE_ZEROED, image_length});
            }
        }
    }
    ...
    for (auto& [snap_key, sparse_extents] : orig_snapshot_delta) {
        ...
        for (auto prev_sparse_extents : pending_sparse_extents) {
            for (auto& sparse_extent : sparse_extents) {
                prev_sparse_extents->erase(sparse_extent.get_off(), sparse_extent.get_len());
            }
        }
