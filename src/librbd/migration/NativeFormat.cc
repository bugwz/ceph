...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::NativeFormat: " << this << " " << __func__ << ": "
...
template<typename I>
std::string NativeFormat<I>::build_source_spec(int64_t pool_id, const std::string& pool_namespace,
...
template<typename I>
NativeFormat<I>::NativeFormat(I* image_ctx, const json_spirit::mObject& json_object, bool import_only)
    : m_image_ctx(image_ctx)
    , m_json_object(json_object)
    , m_import_only(import_only)
{}

template<typename I> void NativeFormat<I>::open(Context* on_finish)
{
    ...
    if (snap_id_val.type() != json_spirit::null_type && m_snap_id == CEPH_NOSNAP) {
    ...
    librados::IoCtx io_ctx;
    int r = util::create_ioctx(m_image_ctx->md_ctx, "source image", m_pool_id, m_pool_namespace, &io_ctx);
    if (r < 0) {
        on_finish->complete(r);
        return;
    }
...
template<typename I> void NativeFormat<I>::handle_open(int r, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ...
    if (!m_snap_name.empty()) {
        ...
        m_snap_id = m_image_ctx->get_snap_id(cls::rbd::UserSnapshotNamespace{}, m_snap_name);
    }

    if (m_snap_id == CEPH_NOSNAP) {
        lderr(cct) << "failed to locate snapshot " << m_snap_name << dendl;
        on_finish = new LambdaContext([on_finish](int) { on_finish->complete(-ENOENT); });
        m_image_ctx->state->close(on_finish);
        return;
    }

    on_finish = new LambdaContext([this, on_finish](int r) { handle_snap_set(r, on_finish); });
    m_image_ctx->state->snap_set(m_snap_id, on_finish);
}

template<typename I> void NativeFormat<I>::handle_snap_set(int r, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << "r=" << r << dendl;

    if (r < 0) {
        lderr(cct) << "failed to set snapshot " << m_snap_id << ": " << cpp_strerror(r) << dendl;
...
template<typename I> void NativeFormat<I>::close(Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;
...
template<typename I> void NativeFormat<I>::get_snapshots(SnapInfos* snap_infos, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;
...
template<typename I> void NativeFormat<I>::get_image_size(uint64_t snap_id, uint64_t* size, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;
...
template<typename I>
void NativeFormat<I>::list_snaps(io::Extents&& image_extents, io::SnapIds&& snap_ids, int list_snaps_flags,
                                 ...
                                 Context* on_finish)
{
    ...
    auto aio_comp =
        io::AioCompletion::create_and_start(on_finish, util::get_image_ctx(m_image_ctx), io::AIO_TYPE_GENERIC);
