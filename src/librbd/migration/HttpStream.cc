...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::HttpStream: " << this << " " << __func__ << ": "
...
template<typename I> void HttpStream<I>::open(Context* on_finish)
{
...
template<typename I> void HttpStream<I>::close(Context* on_finish)
{
...
template<typename I> void HttpStream<I>::get_size(uint64_t* size, Context* on_finish)
{
    ldout(m_cct, 10) << dendl;
...
template<typename I> void HttpStream<I>::read(io::Extents&& byte_extents, bufferlist* data, Context* on_finish)
{
