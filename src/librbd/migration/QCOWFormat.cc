...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::QCOWFormat: " << __func__ << ": "
...
struct ClusterExtent
{
    ...
    ClusterExtent(uint64_t cluster_offset, uint64_t cluster_length, uint64_t intra_cluster_offset,
                  uint64_t image_offset, uint64_t buffer_offset)
        : cluster_offset(cluster_offset)
        , cluster_length(cluster_length)
        , intra_cluster_offset(intra_cluster_offset)
...
void populate_cluster_extents(CephContext* cct, uint64_t cluster_size, const io::Extents& image_extents,
                              ClusterExtents* cluster_extents)
{
    ...
    for (auto [image_offset, image_length] : image_extents) {
        while (image_length > 0) {
            ...
            cluster_extents->emplace_back(0, cluster_length, intra_cluster_offset, image_offset, buffer_offset);
...
template<typename I> struct QCOWFormat<I>::Cluster
{
    const uint64_t cluster_offset;
    bufferlist cluster_data_bl;

    Cluster(uint64_t cluster_offset)
        : cluster_offset(cluster_offset)
    {}
};

#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::QCOWFormat::ClusterCache: " << this << " " << __func__ << ": "

template<typename I> class QCOWFormat<I>::ClusterCache
{
public:
    ...
    void get_cluster(uint64_t cluster_offset, uint64_t cluster_length, uint64_t intra_cluster_offset, bufferlist* bl,
                     Context* on_finish)
    {
        ...
        boost::asio::dispatch(m_strand, [this, cluster_offset, cluster_length, intra_cluster_offset, bl, on_finish]() {
            execute_get_cluster(cluster_offset, cluster_length, intra_cluster_offset, bl, on_finish);
        });
    }
...
private:
    ...
    void execute_get_cluster(uint64_t cluster_offset, uint64_t cluster_length, uint64_t intra_cluster_offset,
                             bufferlist* bl, Context* on_finish)
    {
        ...
        if (cluster && cluster->cluster_offset == cluster_offset) {
            ...
            bl->substr_of(cluster->cluster_data_bl, intra_cluster_offset, cluster_length);
            boost::asio::post(*qcow_format->m_image_ctx->asio_engine, [on_finish]() { on_finish->complete(0); });
            return;
        }

        // record callback for cluster
        bool new_request = (cluster_completions.count(cluster_offset) == 0);
        cluster_completions[cluster_offset].emplace_back(intra_cluster_offset, cluster_length, bl, on_finish);
        if (new_request) {
    ...
    void read_cluster(std::shared_ptr<Cluster> cluster)
    {
        ...
        if ((cluster->cluster_offset & QCOW_OFLAG_COMPRESSED) != 0) {
            ...
            stream_offset = cluster->cluster_offset & qcow_format->m_cluster_offset_mask;
            stream_length =
                (cluster->cluster_offset >> (63 - qcow_format->m_cluster_bits)) & (qcow_format->m_cluster_size - 1);
        }
        ...
        auto ctx = new LambdaContext([this, cluster](int r) {
            boost::asio::post(m_strand, [this, cluster, r]() { handle_read_cluster(r, cluster); });
        });
        qcow_format->m_stream->read({{stream_offset, stream_length}}, &cluster->cluster_data_bl, ctx);
    }
    ...
    void handle_read_cluster(int r, std::shared_ptr<Cluster> cluster)
    {
        ...
        if (r < 0) {
            lderr(cct) << "failed to read cluster offset " << cluster->cluster_offset << ": " << cpp_strerror(r)
                       << dendl;
        }
        else {
            if ((cluster->cluster_offset & QCOW_OFLAG_COMPRESSED) != 0) {
                ...
                // TODO
                lderr(cct) << "support for compressed clusters is not available" << dendl;
                r = -EINVAL;
            }
            else {
        ...
        boost::asio::post(*qcow_format->m_image_ctx->asio_engine, [r, cluster, completions = std::move(completions)]() {
            for (auto completion : completions) {
                if (r >= 0) {
                    std::get<2>(completion)
                        ->substr_of(cluster->cluster_data_bl, std::get<0>(completion), std::get<1>(completion));
                }
...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::QCOWFormat::L2TableCache: " << this << " " << __func__ << ": "

template<typename I> class QCOWFormat<I>::L2TableCache
{
public:
                      ...
                      std::shared_ptr<const LookupTable>* l2_table, Context* on_finish)
    {
        ...
        boost::asio::dispatch(m_strand,
                              [this, request = std::move(request)]() { requests.push_back(std::move(request)); });
        dispatch_request();
    }

    void get_cluster_offset(const LookupTable* l1_table, uint64_t image_offset, uint64_t* cluster_offset,
                            Context* on_finish)
    {
        auto cct = qcow_format->m_image_ctx->cct;
        uint32_t l1_table_index = image_offset >> qcow_format->m_l1_shift;
        uint64_t l2_table_offset = l1_table->cluster_offsets[std::min<uint32_t>(l1_table_index, l1_table->size - 1)] &
                                   qcow_format->m_cluster_mask;
        uint32_t l2_table_index = (image_offset >> qcow_format->m_cluster_bits) & (qcow_format->m_l2_size - 1);
        ldout(cct, 20) << "image_offset=" << image_offset << ", "
        ...
        // cache state machine runs in a single strand thread
        Request request{l1_table, l2_table_offset, l2_table_index, cluster_offset, on_finish};
        boost::asio::dispatch(m_strand,
                              [this, request = std::move(request)]() { requests.push_back(std::move(request)); });
        dispatch_request();
    }
...
private:
    ...
    struct Request
    {
        ...
        Request(const LookupTable* l1_table, uint64_t l2_table_offset, uint32_t l2_table_index,
            ...
            , on_finish(on_finish)
        {}
        Request(const LookupTable* l1_table, uint64_t l2_table_offset, std::shared_ptr<const LookupTable>* l2_table,
                Context* on_finish)
            : l1_table(l1_table)
            , l2_table_offset(l2_table_offset)
            , l2_table(l2_table)
    ...
    void execute_request()
    {
        ...
        if (r < 0) {
            lderr(cct) << "failed to load L2 table: l2_table_offset=" << request.l2_table_offset << ": "
        ...
        boost::asio::post(*qcow_format->m_image_ctx->asio_engine, [r, ctx = request.on_finish]() { ctx->complete(r); });
        requests.pop_front();
    ...
    int l2_table_lookup(uint64_t l2_offset, std::shared_ptr<LookupTable>* l2_table)
    {
        auto cct = qcow_format->m_image_ctx->cct;
        ...
        for (auto idx = 0U; idx < l2_cache_entries.size(); ++idx) {
            ...
            if (l2_cache.l2_offset == l2_offset) {
                ...
                if (l2_cache.ret_val < 0) {
                    ldout(cct, 20) << "l2_offset=" << l2_offset << ", "
                                   << "index=" << idx << " (error): " << cpp_strerror(l2_cache.ret_val) << dendl;
        ...
        auto ctx = new LambdaContext([this, index = min_idx, l2_offset](int r) {
            boost::asio::post(m_strand, [this, index, l2_offset, r]() { handle_l2_table_lookup(r, index, l2_offset); });
        });
        qcow_format->m_stream->read(
            {{l2_offset, qcow_format->m_l2_size * sizeof(uint64_t)}}, &l2_cache.l2_table->bl, ctx);
        return 0;
    }
    ...
    void handle_l2_table_lookup(int r, uint32_t index, uint64_t l2_offset)
    {
        ...
        if (r < 0) {
            lderr(cct) << "failed to load L2 table: "
                       << "l2_offset=" << l2_cache.l2_offset << ": " << cpp_strerror(r) << dendl;
            l2_cache.ret_val = r;
        }
        else {
...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::QCOWFormat::ReadRequest: " << this << " " << __func__ << ": "

template<typename I> class QCOWFormat<I>::ReadRequest
{
public:
    ReadRequest(QCOWFormat* qcow_format, io::AioCompletion* aio_comp, const LookupTable* l1_table,
...
private:
    ...
    void get_cluster_offsets()
    {
        ...
        ldout(cct, 20) << dendl;
        auto ctx = new LambdaContext([this](int r) { handle_get_cluster_offsets(r); });
        auto gather_ctx = new C_Gather(cct, ctx);

        for (auto& cluster_extent : cluster_extents) {
            auto sub_ctx = new LambdaContext([this, &cluster_extent, on_finish = gather_ctx->new_sub()](int r) {
                handle_get_cluster_offset(r, cluster_extent, on_finish);
            });
            qcow_format->m_l2_table_cache->get_cluster_offset(
                l1_table, cluster_extent.image_offset, &cluster_extent.cluster_offset, sub_ctx);
        }

        gather_ctx->activate();
    }

    void handle_get_cluster_offset(int r, const ClusterExtent& cluster_extent, Context* on_finish)
    {
        auto cct = qcow_format->m_image_ctx->cct;
        ldout(cct, 20) << "r=" << r << ", "
                       << "image_offset=" << cluster_extent.image_offset << ", "
                       << "cluster_offset=" << cluster_extent.cluster_offset << dendl;
        ...
        if (r == -ENOENT) {
            ...
            lderr(cct) << "failed to map image offset " << cluster_extent.image_offset << ": " << cpp_strerror(r)
                       << dendl;
        }
    ...
    void handle_get_cluster_offsets(int r)
    {
        ...
        if (r < 0) {
            lderr(cct) << "failed to retrieve cluster extents: " << cpp_strerror(r) << dendl;
            aio_comp->fail(r);
            delete this;
            return;
        }
    ...
    void read_clusters()
    {
        ...
        for (auto& cluster_extent : cluster_extents) {
            auto read_ctx = new io::ReadResult::C_ImageReadRequest(
                aio_comp, cluster_extent.buffer_offset, {{cluster_extent.image_offset, cluster_extent.cluster_length}});
            ...
            auto log_ctx = new LambdaContext(
                [this,
                 ...
                 ctx = read_ctx](int r) { handle_read_cluster(cct, r, image_offset, image_length, ctx); });
            ...
            if (cluster_extent.cluster_offset == 0) {
                ...
                qcow_format->m_cluster_cache->get_cluster(cluster_extent.cluster_offset,
                                                          cluster_extent.cluster_length,
                                                          cluster_extent.intra_cluster_offset,
                                                          &read_ctx->bl,
                                                          log_ctx);
            }
        }
    ...
    void handle_read_cluster(CephContext* cct, int r, uint64_t image_offset, uint64_t image_length,
                             Context* on_finish) const
    {
        ...
        if (r != -ENOENT && r < 0) {
            lderr(cct) << "failed to read image extent " << image_offset << "~" << image_length << ": "
                       << cpp_strerror(r) << dendl;
        }
...
#undef dout_prefix
#define dout_prefix                             \
    *_dout << "librbd::migration::QCOWFormat::" \
           << "ListSnapsRequest: " << this << " " << __func__ << ": "

template<typename I> class QCOWFormat<I>::ListSnapsRequest
{
public:
    ListSnapsRequest(QCOWFormat* qcow_format, uint32_t l1_table_index, ClusterExtents&& cluster_extents,
        ...
        , snap_id_to_l1_table(snap_id_to_l1_table)
        , snapshot_delta(snapshot_delta)
        , on_finish(on_finish)
    {}

    void send() { get_l2_table(); }
...
private:
    ...
    void get_l2_table()
    {
        ...
        auto ctx = new LambdaContext([this, snap_id = snap_id](int r) {
            boost::asio::post(qcow_format->m_strand, [this, snap_id, r]() { handle_get_l2_table(r, snap_id); });
        });

        if (l1_table_index >= l1_table->size || l1_table->cluster_offsets[l1_table_index] == 0) {
        ...
        uint64_t l2_table_offset = l1_table->cluster_offsets[l1_table_index] & qcow_format->m_cluster_mask;

        ldout(cct, 20) << "l1_table_index=" << l1_table_index << ", "
                       << "snap_id=" << snap_id << ", "
                       << "l2_table_offset=" << l2_table_offset << dendl;
        qcow_format->m_l2_table_cache->get_l2_table(l1_table, l2_table_offset, &l2_table, ctx);
    }
    ...
    void handle_get_l2_table(int r, uint64_t snap_id)
    {
        ...
        if (r == -ENOENT) {
            ...
            lderr(cct) << "failed to retrieve L2 table for snapshot " << snap_id << ": " << cpp_strerror(r) << dendl;
            finish(r);
            return;
        }
        ...
        for (auto& cluster_extent : cluster_extents) {
            uint32_t l2_table_index =
                (cluster_extent.image_offset >> qcow_format->m_cluster_bits) & (qcow_format->m_l2_size - 1);

            std::optional<uint64_t> cluster_offset;
            if (l2_table && l2_table_index < l2_table->size) {
                cluster_offset = l2_table->cluster_offsets[l2_table_index] & qcow_format->m_cluster_offset_mask;
            }

            std::optional<uint64_t> prev_cluster_offset;
            if (previous_l2_table && l2_table_index < previous_l2_table->size) {
                prev_cluster_offset =
                    previous_l2_table->cluster_offsets[l2_table_index] & qcow_format->m_cluster_offset_mask;
            }
            ...
            else if ((prev_cluster_offset && !cluster_offset) || *cluster_offset == QCOW_OFLAG_ZERO) {
                // explicitly zeroed or deallocated
                state = io::SPARSE_EXTENT_STATE_ZEROED;
            }

            sparse_extents.insert(
                cluster_extent.image_offset, cluster_extent.cluster_length, {state, cluster_extent.cluster_length});
        }
        ...
        boost::asio::post(*qcow_format->m_image_ctx->asio_engine, [this]() { get_l2_table(); });
    }
...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::QCOWFormat: " << this << " " << __func__ << ": "

template<typename I>
QCOWFormat<I>::QCOWFormat(I* image_ctx, const json_spirit::mObject& json_object,
...
template<typename I> void QCOWFormat<I>::open(Context* on_finish)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to build migration stream handler" << cpp_strerror(r) << dendl;
        on_finish->complete(r);
        return;
    }

    auto ctx = new LambdaContext([this, on_finish](int r) { handle_open(r, on_finish); });
    m_stream->open(ctx);
}

template<typename I> void QCOWFormat<I>::handle_open(int r, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << "r=" << r << dendl;

    if (r < 0) {
        lderr(cct) << "failed to open QCOW image: " << cpp_strerror(r) << dendl;
        on_finish->complete(r);
        return;
    }
...
template<typename I> void QCOWFormat<I>::probe(Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;

    auto ctx = new LambdaContext([this, on_finish](int r) { handle_probe(r, on_finish); });
    m_bl.clear();
    m_stream->read({{0, 8}}, &m_bl, ctx);
}

template<typename I> void QCOWFormat<I>::handle_probe(int r, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << "r=" << r << dendl;

    if (r < 0) {
        lderr(cct) << "failed to probe QCOW image: " << cpp_strerror(r) << dendl;
        on_finish->complete(r);
        return;
    }

    auto header_probe = *reinterpret_cast<QCowHeaderProbe*>(m_bl.c_str());
    ...
    if (header_probe.version == 1) {
        ...
        lderr(cct) << "invalid QCOW header version " << header_probe.version << dendl;
        on_finish->complete(-EINVAL);
        return;
    }
}
...
template<typename I> void QCOWFormat<I>::read_v1_header(Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;

    auto ctx = new LambdaContext([this, on_finish](int r) { handle_read_v1_header(r, on_finish); });
    m_bl.clear();
    m_stream->read({{0, sizeof(QCowHeaderV1)}}, &m_bl, ctx);
}

template<typename I> void QCOWFormat<I>::handle_read_v1_header(int r, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << "r=" << r << dendl;
    ...
    if (header.l2_bits < (QCOW_MIN_CLUSTER_BITS - 3) || header.l2_bits > (QCOW_MAX_CLUSTER_BITS - 3)) {
    ...
    if (m_size > (std::numeric_limits<uint64_t>::max() - (1ULL << m_l1_shift)) ||
        m_l1_table.size > (std::numeric_limits<int32_t>::max() / sizeof(uint64_t))) {
...
template<typename I> void QCOWFormat<I>::read_v2_header(Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;

    auto ctx = new LambdaContext([this, on_finish](int r) { handle_read_v2_header(r, on_finish); });
    m_bl.clear();
    m_stream->read({{0, sizeof(QCowHeader)}}, &m_bl, ctx);
}

template<typename I> void QCOWFormat<I>::handle_read_v2_header(int r, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << "r=" << r << dendl;
    ...
    if ((header.compression_type != 0) || ((header.incompatible_features & QCOW2_INCOMPAT_COMPRESSION) != 0)) {
    ...
    if (m_size > (std::numeric_limits<uint64_t>::max() - (1ULL << m_l1_shift)) ||
        ...
        lderr(cct) << "invalid L1 table size in header (" << header.l1_size << " < " << m_l1_table.size << ")" << dendl;
        on_finish->complete(-EINVAL);
        return;
    }
...
template<typename I> void QCOWFormat<I>::read_snapshot(Context* on_finish)
{
    ...
    auto ctx = new LambdaContext([this, on_finish](int r) { handle_read_snapshot(r, on_finish); });
    m_bl.clear();
    m_stream->read({{m_snapshots_offset, sizeof(QCowSnapshotHeader)}}, &m_bl, ctx);
}

template<typename I> void QCOWFormat<I>::handle_read_snapshot(int r, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << "r=" << r << ", "
    ...
    if (r < 0) {
        lderr(cct) << "failed to read QCOW2 snapshot header: " << cpp_strerror(r) << dendl;
        on_finish->complete(r);
        return;
    }
...
template<typename I> void QCOWFormat<I>::read_snapshot_extra(Context* on_finish)
{
    ceph_assert(!m_snapshots.empty());
    auto& snapshot = m_snapshots.rbegin()->second;

    uint32_t length = snapshot.extra_data_size + snapshot.id.size() + snapshot.name.size();
    if (length == 0) {
        uuid_d uuid_gen;
    ...
    auto ctx = new LambdaContext([this, on_finish](int r) { handle_read_snapshot_extra(r, on_finish); });
    m_bl.clear();
    m_stream->read({{offset, length}}, &m_bl, ctx);
}

template<typename I> void QCOWFormat<I>::handle_read_snapshot_extra(int r, Context* on_finish)
{
    ceph_assert(!m_snapshots.empty());
    auto& snapshot = m_snapshots.rbegin()->second;
    ...
    if (r < 0) {
        lderr(cct) << "failed to read QCOW2 snapshot header extra: " << cpp_strerror(r) << dendl;
        on_finish->complete(r);
        return;
    }

    if (snapshot.extra_data_size >= offsetof(QCowSnapshotExtraData, disk_size) + sizeof(uint64_t)) {
...
template<typename I> void QCOWFormat<I>::read_snapshot_l1_table(Context* on_finish)
{
    ceph_assert(!m_snapshots.empty());
    auto& snapshot = m_snapshots.rbegin()->second;
    ...
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << "snap_id=" << m_snapshots.size() << ", "
                   << "l1_table_offset=" << snapshot.l1_table_offset << dendl;

    auto ctx = new LambdaContext([this, on_finish](int r) { handle_read_snapshot_l1_table(r, on_finish); });
    m_stream->read({{snapshot.l1_table_offset, snapshot.l1_table.size * sizeof(uint64_t)}}, &snapshot.l1_table.bl, ctx);
}

template<typename I> void QCOWFormat<I>::handle_read_snapshot_l1_table(int r, Context* on_finish)
{
    ceph_assert(!m_snapshots.empty());
    auto& snapshot = m_snapshots.rbegin()->second;
    ...
    if (r < 0) {
        lderr(cct) << "failed to read snapshot L1 table: " << cpp_strerror(r) << dendl;
        on_finish->complete(r);
        return;
    }
...
template<typename I> void QCOWFormat<I>::read_l1_table(Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;

    auto ctx = new LambdaContext([this, on_finish](int r) { handle_read_l1_table(r, on_finish); });
    m_stream->read({{m_l1_table_offset, m_l1_table.size * sizeof(uint64_t)}}, &m_l1_table.bl, ctx);
}

template<typename I> void QCOWFormat<I>::handle_read_l1_table(int r, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << "r=" << r << dendl;
...
template<typename I> void QCOWFormat<I>::read_backing_file(Context* on_finish)
{
...
template<typename I> void QCOWFormat<I>::close(Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;
...
template<typename I> void QCOWFormat<I>::get_snapshots(SnapInfos* snap_infos, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;
    ...
    for (auto& [snap_id, snapshot] : m_snapshots) {
        SnapInfo snap_info(
            snapshot.name, cls::rbd::UserSnapshotNamespace{}, snapshot.size, {}, 0, 0, snapshot.timestamp);
...
template<typename I> void QCOWFormat<I>::get_image_size(uint64_t snap_id, uint64_t* size, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
...
template<typename I>
bool QCOWFormat<I>::read(io::AioCompletion* aio_comp, uint64_t snap_id, io::Extents&& image_extents,
                         io::ReadResult&& read_result, int op_flags, int read_flags, const ZTracer::Trace& parent_trace)
{
    auto cct = m_image_ctx->cct;
    ...
    auto read_request = new ReadRequest(this, aio_comp, l1_table, std::move(image_extents));
    read_request->send();
...
template<typename I>
void QCOWFormat<I>::list_snaps(io::Extents&& image_extents, io::SnapIds&& snap_ids, int list_snaps_flags,
                               ...
                               Context* on_finish)
{
    ...
    ClusterExtents cluster_extents;
    populate_cluster_extents(cct, m_cluster_size, image_extents, &cluster_extents);
    ...
    on_finish = new LambdaContext(
        [this, image_extents, snap_ids = std::move(snap_ids), snapshot_delta, on_finish](int r) mutable {
            handle_list_snaps(r, std::move(image_extents), std::move(snap_ids), snapshot_delta, on_finish);
        });

    auto gather_ctx = new C_Gather(cct, on_finish);

    for (auto& [l1_table_index, cluster_extents] : l1_cluster_extents) {
        auto list_snaps_request = new ListSnapsRequest(this,
                                                       l1_table_index,
...
template<typename I>
void QCOWFormat<I>::handle_list_snaps(int r, io::Extents&& image_extents, io::SnapIds&& snap_ids,
                                      io::SnapshotDelta* snapshot_delta, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << ", "
    ...
    for (auto& [snap_id, snapshot] : m_snapshots) {
        auto sparse_extents = &(*snapshot_delta)[{snap_id, snap_id}];
        util::zero_shrunk_snapshot(cct, image_extents, snap_id, snapshot.size, &previous_size, sparse_extents);
    }

    auto sparse_extents = &(*snapshot_delta)[{CEPH_NOSNAP, CEPH_NOSNAP}];
    util::zero_shrunk_snapshot(cct, image_extents, CEPH_NOSNAP, m_size, &previous_size, sparse_extents);
