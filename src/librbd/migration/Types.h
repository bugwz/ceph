...
{
    UrlSpec() {}
    UrlSpec(UrlScheme scheme, const std::string& host, const std::string& port, const std::string& path)
        : scheme(scheme)
        , host(host)
        , port(port)
        , path(path)
...
{
    return (lhs.scheme == rhs.scheme && lhs.host == rhs.host && lhs.port == rhs.port && lhs.path == rhs.path);
}
