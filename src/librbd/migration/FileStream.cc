...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::FileStream::ReadRequest " << this << " " << __func__ << ": "

template<typename I> struct FileStream<I>::ReadRequest
{
    ...
    ReadRequest(FileStream* file_stream, io::Extents&& byte_extents, bufferlist* data, Context* on_finish)
    ...
    void read()
    {
        ...
        auto ptr = buffer::ptr_node::create(buffer::create_small_page_aligned(byte_extent.second));
        auto buffer = boost::asio::mutable_buffer(ptr->c_str(), byte_extent.second);
        ...
        size_t bytes_read = boost::asio::read(*file_stream->m_stream_descriptor, std::move(buffer), ec);
        r = -ec.value();
        if (r < 0 && r != -ENOENT) {
            lderr(cct) << "failed to read from file stream: " << cpp_strerror(r) << dendl;
            finish(r);
            return;
        }
...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::FileStream: " << this << " " << __func__ << ": "
...
template<typename I> FileStream<I>::~FileStream()
{
    if (m_file_no != -1) {
        ::close(m_file_no);
    }
...
template<typename I> void FileStream<I>::open(Context* on_finish)
{
    ...
    if (m_file_no < 0) {
        int r = -errno;
        lderr(m_cct) << "failed to open file stream '" << file_path << "': " << cpp_strerror(r) << dendl;
        on_finish->complete(r);
        return;
    }

    m_stream_descriptor = std::make_optional<boost::asio::posix::stream_descriptor>(m_strand, m_file_no);
    on_finish->complete(0);
}

template<typename I> void FileStream<I>::close(Context* on_finish)
{
...
template<typename I> void FileStream<I>::get_size(uint64_t* size, Context* on_finish)
{
    ldout(m_cct, 10) << dendl;

    // execute IO operations in a single strand to prevent seek races
    boost::asio::post(m_strand, [this, size, on_finish]() {
        auto offset = lseek64(m_file_no, 0, SEEK_END);
        if (offset == -1) {
            int r = -errno;
            lderr(m_cct) << "failed to seek to file end: " << cpp_strerror(r) << dendl;
            on_finish->complete(r);
            return;
        }
...
template<typename I> void FileStream<I>::read(io::Extents&& byte_extents, bufferlist* data, Context* on_finish)
{
