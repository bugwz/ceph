...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::RawFormat: " << this << " " << __func__ << ": "

template<typename I>
RawFormat<I>::RawFormat(I* image_ctx, const json_spirit::mObject& json_object,
...
template<typename I> void RawFormat<I>::open(Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;

    on_finish = new LambdaContext([this, on_finish](int r) { handle_open(r, on_finish); });

    // treat the base image as a HEAD-revision snapshot
    Snapshots snapshots;
    int r = m_source_spec_builder->build_snapshot(m_json_object, CEPH_NOSNAP, &snapshots[CEPH_NOSNAP]);
    if (r < 0) {
        lderr(cct) << "failed to build HEAD revision handler: " << cpp_strerror(r) << dendl;
    ...
    if (snapshots_val.type() == json_spirit::array_type) {
        ...
        for (auto& snapshot_val : snapshots_arr) {
            ...
            if (snapshot_val.type() != json_spirit::obj_type) {
                lderr(cct) << "invalid snapshot " << index << " JSON: " << cpp_strerror(r) << dendl;
                on_finish->complete(-EINVAL);
                return;
            }

            auto& snapshot_obj = snapshot_val.get_obj();
            r = m_source_spec_builder->build_snapshot(snapshot_obj, index, &snapshots[index]);
            if (r < 0) {
                lderr(cct) << "failed to build snapshot " << index << " handler: " << cpp_strerror(r) << dendl;
                on_finish->complete(r);
                return;
            }
        }
...
template<typename I> void RawFormat<I>::handle_open(int r, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ...
    if (r < 0) {
        ...
        m_image_ctx->state->close(
            new LambdaContext([r, on_finish = gather_ctx->new_sub()](int _) { on_finish->complete(r); }));
...
template<typename I> void RawFormat<I>::get_snapshots(SnapInfos* snap_infos, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;
...
template<typename I> void RawFormat<I>::get_image_size(uint64_t snap_id, uint64_t* size, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << dendl;
...
template<typename I>
bool RawFormat<I>::read(io::AioCompletion* aio_comp, uint64_t snap_id, io::Extents&& image_extents,
...
template<typename I>
void RawFormat<I>::list_snaps(io::Extents&& image_extents, io::SnapIds&& snap_ids, int list_snaps_flags,
                              io::SnapshotDelta* snapshot_delta, const ZTracer::Trace& parent_trace, Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << "image_extents=" << image_extents << dendl;

    on_finish = new LambdaContext([this, snap_ids = std::move(snap_ids), snapshot_delta, on_finish](int r) mutable {
    ...
    for (auto& [snap_id, snapshot] : m_snapshots) {
        ...
        util::zero_shrunk_snapshot(cct, image_extents, snap_id, snap_info.size, &previous_size, &sparse_extents);

        // build set of data/zeroed extents for the current snapshot
        snapshot->list_snap(
            io::Extents{image_extents}, list_snaps_flags, &sparse_extents, parent_trace, gather_ctx->new_sub());
    }
...
template<typename I>
void RawFormat<I>::handle_list_snaps(int r, io::SnapIds&& snap_ids, io::SnapshotDelta* snapshot_delta,
                                     Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << ", "
