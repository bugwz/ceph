...
template<typename I> struct S3Stream<I>::HttpProcessor : public HttpProcessorInterface
{
    S3Stream* s3stream;

    HttpProcessor(S3Stream* s3stream)
        : s3stream(s3stream)
    {}

    void process_request(EmptyRequest& request) override { s3stream->process_request(request); }
};

#define dout_subsys ceph_subsys_rbd
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::S3Stream: " << this << " " << __func__ << ": "
...
template<typename I> S3Stream<I>::~S3Stream() {}

template<typename I> void S3Stream<I>::open(Context* on_finish)
{
    ...
    if (util::is_config_key_uri(m_access_key)) {
        ...
        if (r < 0) {
            lderr(m_cct) << "failed to retrieve access key from config: " << cpp_strerror(r) << dendl;
    ...
    if (util::is_config_key_uri(m_secret_key)) {
        ...
        if (r < 0) {
            lderr(m_cct) << "failed to retrieve secret key from config: " << cpp_strerror(r) << dendl;
            on_finish->complete(r);
            return;
        }
    }
...
template<typename I> void S3Stream<I>::close(Context* on_finish)
{
...
template<typename I> void S3Stream<I>::get_size(uint64_t* size, Context* on_finish)
{
    ldout(m_cct, 10) << dendl;
...
template<typename I> void S3Stream<I>::read(io::Extents&& byte_extents, bufferlist* data, Context* on_finish)
{
...
template<typename I> void S3Stream<I>::process_request(HttpRequest& http_request)
{
    ...
    ceph::crypto::HMACSHA1 hmac(reinterpret_cast<const unsigned char*>(m_secret_key.data()), m_secret_key.size());
    hmac.Update(reinterpret_cast<const unsigned char*>(string_to_sign.data()), string_to_sign.size());
    hmac.Final(reinterpret_cast<unsigned char*>(digest.v));

    // base64 encode the result
    char buf[64];
    int r = ceph_armor(std::begin(buf),
                       std::begin(buf) + sizeof(buf),
