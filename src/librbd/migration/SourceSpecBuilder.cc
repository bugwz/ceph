...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::migration::SourceSpecBuilder: " << this << " " << __func__ << ": "
...
template<typename I>
int SourceSpecBuilder<I>::parse_source_spec(const std::string& source_spec,
...
template<typename I>
int SourceSpecBuilder<I>::build_format(const json_spirit::mObject& source_spec_object, bool import_only,
                                       std::unique_ptr<FormatInterface>* format) const
{
    ...
    if (type_value_it == source_spec_object.end() || type_value_it->second.type() != json_spirit::str_type) {
    ...
    if (type == "native") {
        format->reset(NativeFormat<I>::create(m_image_ctx, source_spec_object, import_only));
    }
    else if (type == "qcow") {
        ...
        lderr(cct) << "unknown or unsupported format type '" << type << "'" << dendl;
        return -ENOSYS;
    }
    return 0;
}
...
template<typename I>
int SourceSpecBuilder<I>::build_snapshot(const json_spirit::mObject& source_spec_object, uint64_t index,
                                         std::shared_ptr<SnapshotInterface>* snapshot) const
{
    ...
    auto& type = type_value_it->second.get_str();
    if (type == "raw") {
        snapshot->reset(RawSnapshot<I>::create(m_image_ctx, source_spec_object, this, index));
    }
    else {
        lderr(cct) << "unknown or unsupported format type '" << type << "'" << dendl;
        return -ENOSYS;
...
template<typename I>
int SourceSpecBuilder<I>::build_stream(const json_spirit::mObject& source_spec_object,
                                       std::shared_ptr<StreamInterface>* stream) const
{
    ...
    if (stream_value_it == source_spec_object.end() || stream_value_it->second.type() != json_spirit::obj_type) {
    ...
    if (type_value_it == stream_obj.end() || type_value_it->second.type() != json_spirit::str_type) {
    ...
    if (type == "file") {
        ...
        lderr(cct) << "unknown or unsupported stream type '" << type << "'" << dendl;
        return -ENOSYS;
    }
