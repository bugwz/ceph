...
{
public:
    explicit ImageUpdateWatchers(CephContext* cct)
        : m_cct(cct)
    ...
    void send_notify(uint64_t handle, UpdateWatchCtx* watcher)
    {
        ...
        ldout(m_cct, 20) << "ImageUpdateWatchers::" << __func__ << ": handle=" << handle << ", watcher=" << watcher
                         << dendl;

        m_in_flight.insert(handle);

        Context* ctx = new LambdaContext([this, handle, watcher](int r) { handle_notify(handle, watcher); });
...
private:
    ...
    public:
        explicit ThreadPoolSingleton(CephContext* cct)
            : ThreadPool(cct, "librbd::ImageUpdateWatchers::thread_pool", "tp_librbd", 1)
        {
            start();
        }
        ~ThreadPoolSingleton() override { stop(); }
    };
    ...
    void create_work_queue()
    {
        ...
        auto& thread_pool = m_cct->lookup_or_create_singleton_object<ThreadPoolSingleton>(
            "librbd::ImageUpdateWatchers::thread_pool", false, m_cct);
        m_work_queue = new ContextWQ("librbd::ImageUpdateWatchers::work_queue",
                                     ceph::make_timespan(m_cct->_conf.get_val<uint64_t>("rbd_op_thread_timeout")),
                                     &thread_pool);
    }
...
public:
    explicit QuiesceWatchers(CephContext* cct, asio::ContextWQ* work_queue)
        ...
        , m_lock(ceph::make_mutex(util::unique_lock_name("librbd::QuiesceWatchers::m_lock", this)))
    {}
...
private:
    ...
    void notify(EventType event_type, Context* on_finish)
    {
        ...
        if (event_type == QUIESCE) {
            ...
            ctx = create_async_context_callback(
                m_work_queue,
                create_context_callback<QuiesceWatchers, &QuiesceWatchers::handle_notify_unquiesce>(this));
        }
    ...
    void send_notify(uint64_t handle, QuiesceWatchCtx* watcher, EventType event_type, Context* on_finish)
    {
        auto ctx = new LambdaContext([this, handle, watcher, event_type, on_finish](int) {
            ldout(m_cct, 20) << "QuiesceWatchers::" << __func__ << ": handle=" << handle
                             << ", event_type=" << event_type << dendl;
            switch (event_type) {
            case QUIESCE: watcher->handle_quiesce(); break;
            case UNQUIESCE:
            {
                std::lock_guard locker{m_lock};
                ...
                break;
            default: ceph_abort_msgf("invalid event_type %d", event_type);
            }
...
ImageState<I>::ImageState(I* image_ctx)
    ...
    , m_quiesce_watchers(new QuiesceWatchers(image_ctx->cct, image_ctx->asio_engine->get_work_queue()))
{}

template<typename I> ImageState<I>::~ImageState()
{
...
template<typename I> int ImageState<I>::open(uint64_t flags)
{
    C_SaferCond ctx;
    open(flags, &ctx);
...
template<typename I> void ImageState<I>::open(uint64_t flags, Context* on_finish)
{
...
template<typename I> int ImageState<I>::close()
{
    C_SaferCond ctx;
    close(&ctx);
...
template<typename I> void ImageState<I>::close(Context* on_finish)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << __func__ << dendl;
...
template<typename I> void ImageState<I>::handle_update_notification()
{
    std::lock_guard locker{m_lock};
    ++m_refresh_seq;
    ...
    case STATE_CLOSED:
    case STATE_OPENING:
    case STATE_CLOSING: ldout(cct, 5) << "dropping update notification to watchers" << dendl; return;
    default: break;
    }
...
template<typename I> bool ImageState<I>::is_refresh_required() const
{
    std::lock_guard locker{m_lock};
    return (m_last_refresh != m_refresh_seq || find_pending_refresh() != nullptr);
}

template<typename I> int ImageState<I>::refresh()
{
...
template<typename I> void ImageState<I>::refresh(Context* on_finish)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << __func__ << dendl;
...
template<typename I> int ImageState<I>::refresh_if_required()
{
    C_SaferCond ctx;
    {
        m_lock.lock();
...
template<typename I> const typename ImageState<I>::Action* ImageState<I>::find_pending_refresh() const
{
    ceph_assert(ceph_mutex_is_locked(m_lock));

    auto it =
        std::find_if(m_actions_contexts.rbegin(), m_actions_contexts.rend(), [](const ActionContexts& action_contexts) {
...
template<typename I> void ImageState<I>::snap_set(uint64_t snap_id, Context* on_finish)
{
    CephContext* cct = m_image_ctx->cct;
...
template<typename I> void ImageState<I>::prepare_lock(Context* on_ready)
{
    CephContext* cct = m_image_ctx->cct;
...
template<typename I> void ImageState<I>::handle_prepare_lock_complete()
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 10) << __func__ << dendl;
...
template<typename I> int ImageState<I>::register_update_watcher(UpdateWatchCtx* watcher, uint64_t* handle)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << __func__ << dendl;
...
template<typename I> void ImageState<I>::unregister_update_watcher(uint64_t handle, Context* on_finish)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << __func__ << ": handle=" << handle << dendl;
...
template<typename I> int ImageState<I>::unregister_update_watcher(uint64_t handle)
{
    C_SaferCond ctx;
...
template<typename I> void ImageState<I>::flush_update_watchers(Context* on_finish)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << __func__ << dendl;
...
template<typename I> void ImageState<I>::shut_down_update_watchers(Context* on_finish)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << __func__ << dendl;
...
template<typename I> bool ImageState<I>::is_transition_state() const
{
    switch (m_state) {
    case STATE_UNINITIALIZED:
    case STATE_OPEN:
    case STATE_CLOSED: return false;
    case STATE_OPENING:
    case STATE_CLOSING:
    case STATE_REFRESHING:
    case STATE_SETTING_SNAP:
    case STATE_PREPARING_LOCK: break;
    }
    return true;
}

template<typename I> bool ImageState<I>::is_closed() const
{
    ceph_assert(ceph_mutex_is_locked(m_lock));

    return ((m_state == STATE_CLOSED) ||
            (!m_actions_contexts.empty() && m_actions_contexts.back().first.action_type == ACTION_TYPE_CLOSE));
}

template<typename I> void ImageState<I>::append_context(const Action& action, Context* context)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
...
template<typename I> void ImageState<I>::execute_next_action_unlock()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ceph_assert(!m_actions_contexts.empty());
    switch (m_actions_contexts.front().first.action_type) {
    case ACTION_TYPE_OPEN: send_open_unlock(); return;
    case ACTION_TYPE_CLOSE: send_close_unlock(); return;
    case ACTION_TYPE_REFRESH: send_refresh_unlock(); return;
    case ACTION_TYPE_SET_SNAP: send_set_snap_unlock(); return;
    case ACTION_TYPE_LOCK: send_prepare_lock_unlock(); return;
    }
    ceph_abort();
}

template<typename I> void ImageState<I>::execute_action_unlock(const Action& action, Context* on_finish)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
...
template<typename I> void ImageState<I>::complete_action_unlock(State next_state, int r)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ceph_assert(!m_actions_contexts.empty());
    ...
    if (next_state == STATE_CLOSED || (next_state == STATE_UNINITIALIZED && r < 0)) {
        // the ImageCtx must be deleted outside the scope of its callback threads
        auto ctx = new LambdaContext([image_ctx = m_image_ctx, contexts = std::move(action_contexts.second)](int r) {
            delete image_ctx;
...
template<typename I> void ImageState<I>::send_open_unlock()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    CephContext* cct = m_image_ctx->cct;
    ...
    Context* ctx = create_context_callback<ImageState<I>, &ImageState<I>::handle_open>(this);
...
template<typename I> void ImageState<I>::handle_open(int r)
{
    CephContext* cct = m_image_ctx->cct;
...
template<typename I> void ImageState<I>::send_close_unlock()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 10) << this << " " << __func__ << dendl;
    ...
    Context* ctx = create_context_callback<ImageState<I>, &ImageState<I>::handle_close>(this);
...
template<typename I> void ImageState<I>::handle_close(int r)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 10) << this << " " << __func__ << ": r=" << r << dendl;

    if (r < 0) {
        lderr(cct) << "error occurred while closing image: " << cpp_strerror(r) << dendl;
    }
...
template<typename I> void ImageState<I>::send_refresh_unlock()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 10) << this << " " << __func__ << dendl;
    ...
    Context* ctx = create_async_context_callback(
        *m_image_ctx, create_context_callback<ImageState<I>, &ImageState<I>::handle_refresh>(this));
    image::RefreshRequest<I>* req = image::RefreshRequest<I>::create(*m_image_ctx, false, false, ctx);

    m_lock.unlock();
    req->send();
}

template<typename I> void ImageState<I>::handle_refresh(int r)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 10) << this << " " << __func__ << ": r=" << r << dendl;
...
template<typename I> void ImageState<I>::send_set_snap_unlock()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ...
    Context* ctx = create_async_context_callback(
        *m_image_ctx, create_context_callback<ImageState<I>, &ImageState<I>::handle_set_snap>(this));
    image::SetSnapRequest<I>* req = image::SetSnapRequest<I>::create(*m_image_ctx, action_contexts.first.snap_id, ctx);

    m_lock.unlock();
    req->send();
}

template<typename I> void ImageState<I>::handle_set_snap(int r)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 10) << this << " " << __func__ << " r=" << r << dendl;
...
template<typename I> void ImageState<I>::send_prepare_lock_unlock()
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 10) << this << " " << __func__ << dendl;
...
template<typename I> int ImageState<I>::register_quiesce_watcher(QuiesceWatchCtx* watcher, uint64_t* handle)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << __func__ << dendl;
...
template<typename I> int ImageState<I>::unregister_quiesce_watcher(uint64_t handle)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << __func__ << ": handle=" << handle << dendl;
...
template<typename I> void ImageState<I>::notify_quiesce(Context* on_finish)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << __func__ << dendl;
...
template<typename I> void ImageState<I>::notify_unquiesce(Context* on_finish)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << __func__ << dendl;
...
template<typename I> void ImageState<I>::quiesce_complete(uint64_t handle, int r)
{
    CephContext* cct = m_image_ctx->cct;
