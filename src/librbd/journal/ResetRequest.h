...
template<typename ImageCtxT = ImageCtx> class ResetRequest
{
public:
    ...
    {
        return new ResetRequest(io_ctx, image_id, client_id, mirror_uuid, op_work_queue, on_finish);
    }

    ResetRequest(librados::IoCtx& io_ctx, const std::string& image_id, const std::string& client_id,
                 const std::string& mirror_uuid, ContextWQ* op_work_queue, Context* on_finish)
        : m_io_ctx(io_ctx)
        , m_image_id(image_id)
