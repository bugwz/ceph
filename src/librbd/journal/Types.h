...
struct AioDiscardEvent
{
    ...
    AioDiscardEvent() {}
    AioDiscardEvent(uint64_t _offset, uint64_t _length, uint32_t discard_granularity_bytes)
        : offset(_offset)
        , length(_length)
...
struct AioWriteSameEvent
{
    ...
    AioWriteSameEvent()
        ...
        , length(0)
    {}
    AioWriteSameEvent(uint64_t _offset, uint64_t _length, const bufferlist& _data)
        : offset(_offset)
        , length(_length)
        , data(_data)
    {}
...
struct AioCompareAndWriteEvent
{
    ...
    AioCompareAndWriteEvent()
        ...
        , length(0)
    {}
    AioCompareAndWriteEvent(uint64_t _offset, uint64_t _length, const bufferlist& _cmp_data,
                            const bufferlist& _write_data)
        : offset(_offset)
        , length(_length)
        , cmp_data(_cmp_data)
...
struct OpFinishEvent : public OpEventBase
{
    ...
    OpFinishEvent()
        : r(0)
    {}
    OpFinishEvent(uint64_t op_tid, int r)
        : OpEventBase(op_tid)
        , r(r)
    {}
...
protected:
    SnapEventBase() {}
    SnapEventBase(uint64_t op_tid, const cls::rbd::SnapshotNamespace& _snap_namespace, const std::string& _snap_name)
        : OpEventBase(op_tid)
...
struct SnapCreateEvent : public SnapEventBase
{
    ...
    SnapCreateEvent() {}
    SnapCreateEvent(uint64_t op_tid, const cls::rbd::SnapshotNamespace& snap_namespace, const std::string& snap_name)
...
struct SnapRemoveEvent : public SnapEventBase
{
    ...
    SnapRemoveEvent() {}
    SnapRemoveEvent(uint64_t op_tid, const cls::rbd::SnapshotNamespace& snap_namespace, const std::string& snap_name)
        : SnapEventBase(op_tid, snap_namespace, snap_name)
    {}
...
struct SnapRenameEvent : public OpEventBase
{
    ...
    SnapRenameEvent()
        : snap_id(CEPH_NOSNAP)
    {}
    SnapRenameEvent(uint64_t op_tid, uint64_t src_snap_id, const std::string& src_snap_name,
...
struct SnapProtectEvent : public SnapEventBase
{
    ...
    SnapProtectEvent() {}
    SnapProtectEvent(uint64_t op_tid, const cls::rbd::SnapshotNamespace& snap_namespace, const std::string& snap_name)
        : SnapEventBase(op_tid, snap_namespace, snap_name)
    {}
...
struct SnapUnprotectEvent : public SnapEventBase
{
    ...
    SnapUnprotectEvent() {}
    SnapUnprotectEvent(uint64_t op_tid, const cls::rbd::SnapshotNamespace& snap_namespace, const std::string& snap_name)
        : SnapEventBase(op_tid, snap_namespace, snap_name)
    {}
...
struct SnapRollbackEvent : public SnapEventBase
{
    ...
    SnapRollbackEvent() {}
    SnapRollbackEvent(uint64_t op_tid, const cls::rbd::SnapshotNamespace& snap_namespace, const std::string& snap_name)
        : SnapEventBase(op_tid, snap_namespace, snap_name)
    {}
...
{
    static const EventType TYPE = static_cast<EventType>(EVENT_TYPE_DEMOTE_PROMOTE);
...
typedef boost::mpl::vector<AioDiscardEvent, AioWriteEvent, AioFlushEvent, OpFinishEvent, SnapCreateEvent,
                           SnapRemoveEvent, SnapRenameEvent, SnapProtectEvent, SnapUnprotectEvent, SnapRollbackEvent,
                           RenameEvent, ResizeEvent, FlattenEvent, DemotePromoteEvent, SnapLimitEvent,
                           UpdateFeaturesEvent, MetadataSetEvent, MetadataRemoveEvent, AioWriteSameEvent,
                           AioCompareAndWriteEvent, UnknownEvent>
    EventVector;
typedef boost::make_variant_over<EventVector>::type Event;

struct EventEntry
{
    static uint32_t get_fixed_size() { return EVENT_FIXED_SIZE + METADATA_FIXED_SIZE; }
...
struct ImageClientMeta
{
    ...
    ImageClientMeta() {}
    ImageClientMeta(uint64_t tag_class)
        : tag_class(tag_class)
    {}
...
struct MirrorPeerSyncPoint
{
    ...
    MirrorPeerSyncPoint()
        : MirrorPeerSyncPoint({}, "", "", boost::none)
    {}
    MirrorPeerSyncPoint(const cls::rbd::SnapshotNamespace& snap_namespace, const std::string& snap_name,
                        const ObjectNumber& object_number)
        : MirrorPeerSyncPoint(snap_namespace, snap_name, "", object_number)
    {}
    MirrorPeerSyncPoint(const cls::rbd::SnapshotNamespace& snap_namespace, const std::string& snap_name,
                        const std::string& from_snap_name, const ObjectNumber& object_number)
        : snap_namespace(snap_namespace)
        , snap_name(snap_name)
        , from_snap_name(from_snap_name)
    ...
    inline bool operator==(const MirrorPeerSyncPoint& sync) const
    {
        return (snap_name == sync.snap_name && from_snap_name == sync.from_snap_name &&
                object_number == sync.object_number && snap_namespace == sync.snap_namespace);
    }
...
struct MirrorPeerClientMeta
{
    ...
    MirrorPeerClientMeta(const std::string& image_id, const SyncPoints& sync_points = SyncPoints(),
    ...
    {
        return (image_id == meta.image_id && state == meta.state && sync_object_count == meta.sync_object_count &&
                sync_points == meta.sync_points && snap_seqs == meta.snap_seqs);
    }
...
typedef boost::variant<ImageClientMeta, MirrorPeerClientMeta, CliClientMeta, UnknownClientMeta> ClientMeta;

struct ClientData
{
    ClientData() {}
    ClientData(const ClientMeta& client_meta)
...
struct TagPredecessor
{
    ...
    TagPredecessor() {}
    TagPredecessor(const std::string& mirror_uuid, bool commit_valid, uint64_t tag_tid, uint64_t entry_tid)
    ...
    {
        return (mirror_uuid == rhs.mirror_uuid && commit_valid == rhs.commit_valid && tag_tid == rhs.tag_tid &&
...
struct TagData
{
    ...
    TagData(const std::string& mirror_uuid)
        : mirror_uuid(mirror_uuid)
    {}
    TagData(const std::string& mirror_uuid, const std::string& predecessor_mirror_uuid, bool predecessor_commit_valid,
            uint64_t predecessor_tag_tid, uint64_t predecessor_entry_tid)
        : mirror_uuid(mirror_uuid)
        , predecessor(predecessor_mirror_uuid, predecessor_commit_valid, predecessor_tag_tid, predecessor_entry_tid)
    {}
