...
template<typename I, typename E> struct ExecuteOp : public Context
{
    I& image_ctx;
    E event;
    ...
    {
        image_ctx.operations->execute_snap_remove(event.snap_namespace, event.snap_name, on_op_complete);
    }

    void execute(const journal::SnapRenameEvent& _)
    {
        image_ctx.operations->execute_snap_rename(event.snap_id, event.dst_snap_name, on_op_complete);
    }

    void execute(const journal::SnapProtectEvent& _)
    {
        image_ctx.operations->execute_snap_protect(event.snap_namespace, event.snap_name, on_op_complete);
    }

    void execute(const journal::SnapUnprotectEvent& _)
    {
        image_ctx.operations->execute_snap_unprotect(event.snap_namespace, event.snap_name, on_op_complete);
    }

    void execute(const journal::SnapRollbackEvent& _)
    {
        image_ctx.operations->execute_snap_rollback(
            event.snap_namespace, event.snap_name, no_op_progress_callback, on_op_complete);
    }

    void execute(const journal::RenameEvent& _)
    {
        image_ctx.operations->execute_rename(event.image_name, on_op_complete);
    }

    void execute(const journal::ResizeEvent& _)
    {
        image_ctx.operations->execute_resize(event.size, true, no_op_progress_callback, on_op_complete, event.op_tid);
    }

    void execute(const journal::FlattenEvent& _)
    {
        image_ctx.operations->execute_flatten(no_op_progress_callback, on_op_complete);
    }
    ...
    {
        image_ctx.operations->execute_update_features(event.features, event.enabled, on_op_complete, event.op_tid);
    }

    void execute(const journal::MetadataSetEvent& _)
    {
        image_ctx.operations->execute_metadata_set(event.key, event.value, on_op_complete);
    }
    ...
    void finish(int r) override
    {
        ...
        if (image_ctx.exclusive_lock == nullptr || !image_ctx.exclusive_lock->accept_ops()) {
...
template<typename I> struct C_RefreshIfRequired : public Context
{
    I& image_ctx;
    Context* on_finish;
    ...
    C_RefreshIfRequired(I& image_ctx, Context* on_finish)
        ...
        , on_finish(on_finish)
    {}
    ~C_RefreshIfRequired() override { delete on_finish; }
...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::journal::Replay: " << this << " " << __func__
...
template<typename I> Replay<I>::~Replay()
{
...
template<typename I> int Replay<I>::decode(bufferlist::const_iterator* it, EventEntry* event_entry)
{
    try {
        using ceph::decode;
...
template<typename I> void Replay<I>::process(const EventEntry& event_entry, Context* on_ready, Context* on_safe)
{
    ...
    if (m_image_ctx.exclusive_lock == nullptr || !m_image_ctx.exclusive_lock->accept_ops()) {
...
template<typename I> void Replay<I>::shut_down(bool cancel_ops, Context* on_finish)
{
    CephContext* cct = m_image_ctx.cct;
    ldout(cct, 20) << dendl;
...

    {
        ...
        for (auto& op_event_pair : m_op_events) {
            ...
            if (cancel_ops) {
                ...
                if (op_event.on_start_ready == nullptr && op_event.on_op_finish_event != nullptr) {
    ...
    if (flush_comp != nullptr) {
        ...
        io::ImageRequest<I>::aio_flush(&m_image_ctx, flush_comp, io::FLUSH_SOURCE_INTERNAL, {});
    }
...
template<typename I> void Replay<I>::flush(Context* on_finish)
{
    io::AioCompletion* aio_comp;
    {
        std::lock_guard locker{m_lock};
        aio_comp = create_aio_flush_completion(util::create_async_context_callback(m_image_ctx, on_finish));
        if (aio_comp == nullptr) {
            return;
        }
    }
    ...
    std::shared_lock owner_locker{m_image_ctx.owner_lock};
    io::ImageRequest<I>::aio_flush(&m_image_ctx, aio_comp, io::FLUSH_SOURCE_INTERNAL, {});
}

template<typename I> void Replay<I>::replay_op_ready(uint64_t op_tid, Context* on_resume)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    ceph_assert(op_event.op_in_progress && op_event.on_op_finish_event == nullptr &&
                op_event.on_finish_ready == nullptr && op_event.on_finish_safe == nullptr);
    ...
    // is processed
    op_event.on_op_finish_event = new LambdaContext([on_resume](int r) { on_resume->complete(r); });
...
template<typename I>
void Replay<I>::handle_event(const journal::AioDiscardEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ldout(cct, 20) << ": AIO discard event" << dendl;

    bool flush_required;
    auto aio_comp = create_aio_modify_completion(on_ready, on_safe, io::AIO_TYPE_DISCARD, &flush_required, {});
    if (aio_comp == nullptr) {
        return;
    }

    if (!clipped_io(event.offset, aio_comp)) {
        io::ImageRequest<I>::aio_discard(
            &m_image_ctx, aio_comp, {{event.offset, event.length}}, event.discard_granularity_bytes, {});
    }
...
template<typename I>
void Replay<I>::handle_event(const journal::AioWriteEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    bool flush_required;
    auto aio_comp = create_aio_modify_completion(on_ready, on_safe, io::AIO_TYPE_WRITE, &flush_required, {});
    if (aio_comp == nullptr) {
        return;
    }

    if (!clipped_io(event.offset, aio_comp)) {
        io::ImageRequest<I>::aio_write(&m_image_ctx, aio_comp, {{event.offset, event.length}}, std::move(data), 0, {});
    }
...
template<typename I>
void Replay<I>::handle_event(const journal::AioFlushEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
...
template<typename I>
void Replay<I>::handle_event(const journal::AioWriteSameEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    bufferlist data = event.data;
    bool flush_required;
    auto aio_comp = create_aio_modify_completion(on_ready, on_safe, io::AIO_TYPE_WRITESAME, &flush_required, {});
    if (aio_comp == nullptr) {
        return;
    }

    if (!clipped_io(event.offset, aio_comp)) {
        io::ImageRequest<I>::aio_writesame(
            &m_image_ctx, aio_comp, {{event.offset, event.length}}, std::move(data), 0, {});
    }
...
template<typename I>
void Replay<I>::handle_event(const journal::AioCompareAndWriteEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    bool flush_required;
    auto aio_comp =
        create_aio_modify_completion(on_ready, on_safe, io::AIO_TYPE_COMPARE_AND_WRITE, &flush_required, {-EILSEQ});

    if (!clipped_io(event.offset, aio_comp)) {
        io::ImageRequest<I>::aio_compare_and_write(&m_image_ctx,
                                                   aio_comp,
...
template<typename I>
void Replay<I>::handle_event(const journal::OpFinishEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
...
template<typename I>
void Replay<I>::handle_event(const journal::SnapCreateEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    std::lock_guard locker{m_lock};
    OpEvent* op_event;
    Context* on_op_complete = create_op_context_callback(event.op_tid, on_ready, on_safe, &op_event);
    ...
    m_image_ctx.op_work_queue->queue(
        new C_RefreshIfRequired<I>(m_image_ctx,
                                   new ExecuteOp<I, journal::SnapCreateEvent>(m_image_ctx, event, on_op_complete)),
        0);
...
template<typename I>
void Replay<I>::handle_event(const journal::SnapRemoveEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    op_event->on_op_finish_event = new C_RefreshIfRequired<I>(
        m_image_ctx, new ExecuteOp<I, journal::SnapRemoveEvent>(m_image_ctx, event, on_op_complete));
...
template<typename I>
void Replay<I>::handle_event(const journal::SnapRenameEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    op_event->on_op_finish_event = new C_RefreshIfRequired<I>(
        m_image_ctx, new ExecuteOp<I, journal::SnapRenameEvent>(m_image_ctx, event, on_op_complete));
...
template<typename I>
void Replay<I>::handle_event(const journal::SnapProtectEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    op_event->on_op_finish_event = new C_RefreshIfRequired<I>(
        m_image_ctx, new ExecuteOp<I, journal::SnapProtectEvent>(m_image_ctx, event, on_op_complete));
...
template<typename I>
void Replay<I>::handle_event(const journal::SnapUnprotectEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    op_event->on_op_finish_event = new C_RefreshIfRequired<I>(
        m_image_ctx, new ExecuteOp<I, journal::SnapUnprotectEvent>(m_image_ctx, event, on_op_complete));
...
template<typename I>
void Replay<I>::handle_event(const journal::SnapRollbackEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    op_event->on_op_finish_event = new C_RefreshIfRequired<I>(
        m_image_ctx, new ExecuteOp<I, journal::SnapRollbackEvent>(m_image_ctx, event, on_op_complete));

    on_ready->complete(0);
}

template<typename I>
void Replay<I>::handle_event(const journal::RenameEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    op_event->on_op_finish_event = new C_RefreshIfRequired<I>(
        m_image_ctx, new ExecuteOp<I, journal::RenameEvent>(m_image_ctx, event, on_op_complete));
...
template<typename I>
void Replay<I>::handle_event(const journal::ResizeEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    m_image_ctx.op_work_queue->queue(
        new C_RefreshIfRequired<I>(m_image_ctx,
                                   new ExecuteOp<I, journal::ResizeEvent>(m_image_ctx, event, on_op_complete)),
        0);
...
template<typename I>
void Replay<I>::handle_event(const journal::FlattenEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    op_event->on_op_finish_event = new C_RefreshIfRequired<I>(
        m_image_ctx, new ExecuteOp<I, journal::FlattenEvent>(m_image_ctx, event, on_op_complete));
...
template<typename I>
void Replay<I>::handle_event(const journal::DemotePromoteEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
...
template<typename I>
void Replay<I>::handle_event(const journal::SnapLimitEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    op_event->on_op_finish_event = new C_RefreshIfRequired<I>(
        m_image_ctx, new ExecuteOp<I, journal::SnapLimitEvent>(m_image_ctx, event, on_op_complete));
...
template<typename I>
void Replay<I>::handle_event(const journal::UpdateFeaturesEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    m_image_ctx.op_work_queue->queue(
        new C_RefreshIfRequired<I>(m_image_ctx,
                                   new ExecuteOp<I, journal::UpdateFeaturesEvent>(m_image_ctx, event, on_op_complete)),
        0);
...
template<typename I>
void Replay<I>::handle_event(const journal::MetadataSetEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    op_event->on_op_finish_event = util::create_async_context_callback(
        m_image_ctx, new ExecuteOp<I, journal::MetadataSetEvent>(m_image_ctx, event, on_op_complete));

    on_ready->complete(0);
}

template<typename I>
void Replay<I>::handle_event(const journal::MetadataRemoveEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    on_op_complete = new C_RefreshIfRequired<I>(m_image_ctx, on_op_complete);
    op_event->on_op_finish_event = util::create_async_context_callback(
        m_image_ctx, new ExecuteOp<I, journal::MetadataRemoveEvent>(m_image_ctx, event, on_op_complete));
...
template<typename I>
void Replay<I>::handle_event(const journal::UnknownEvent& event, Context* on_ready, Context* on_safe)
{
    CephContext* cct = m_image_ctx.cct;
...
template<typename I>
void Replay<I>::handle_aio_modify_complete(Context* on_ready, Context* on_safe, int r, std::set<int>& filters)
{
    std::lock_guard locker{m_lock};
    CephContext* cct = m_image_ctx.cct;
    ...
    if (filters.find(r) != filters.end()) r = 0;
...
template<typename I> void Replay<I>::handle_aio_flush_complete(Context* on_flush_safe, Contexts& on_safe_ctxs, int r)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    Context* on_flush = nullptr;
    {
        ...
        if (m_in_flight_op_events == 0 && (m_in_flight_aio_flush + m_in_flight_aio_modify) == 0) {
...
template<typename I>
Context* Replay<I>::create_op_context_callback(uint64_t op_tid, Context* on_ready, Context* on_safe, OpEvent** op_event)
{
    CephContext* cct = m_image_ctx.cct;
    if (m_shut_down) {
...
template<typename I> void Replay<I>::handle_op_complete(uint64_t op_tid, int r)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    if (op_event.on_start_ready != nullptr) {
        ...
        ceph_assert(op_event.on_finish_ready == nullptr && op_event.on_finish_safe == nullptr);

        op_event.on_start_ready->complete(0);
    }
    else {
        // event kicked off by OpFinishEvent
        ceph_assert((op_event.on_finish_ready != nullptr && op_event.on_finish_safe != nullptr) || shutting_down);
    }
...
template<typename I>
io::AioCompletion* Replay<I>::create_aio_modify_completion(Context* on_ready, Context* on_safe, io::aio_type_t aio_type,
                                                           bool* flush_required, std::set<int>&& filters)
{
    std::lock_guard locker{m_lock};
    CephContext* cct = m_image_ctx.cct;
    ...
    *flush_required = (m_aio_modify_unsafe_contexts.size() == IN_FLIGHT_IO_LOW_WATER_MARK);
    if (*flush_required) {
...
template<typename I> io::AioCompletion* Replay<I>::create_aio_flush_completion(Context* on_safe)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ...
    auto aio_comp = io::AioCompletion::create_and_start<Context>(
        new C_AioFlushComplete(this, on_safe, std::move(m_aio_modify_unsafe_contexts)),
        util::get_image_ctx(&m_image_ctx),
        io::AIO_TYPE_FLUSH);
...
template<typename I> bool Replay<I>::clipped_io(uint64_t image_offset, io::AioCompletion* aio_comp)
{
    CephContext* cct = m_image_ctx.cct;
