...
struct C_DecodeTag : public Context
{
    ...
    C_DecodeTag(CephContext* cct, ceph::mutex* lock, uint64_t* tag_tid, TagData* tag_data, Context* on_finish)
        : cct(cct)
        , lock(lock)
        , tag_tid(tag_tid)
        , tag_data(tag_data)
...
struct C_DecodeTags : public Context
{
    ...
    C_DecodeTags(CephContext* cct, ceph::mutex* lock, uint64_t* tag_tid, TagData* tag_data, Context* on_finish)
        : cct(cct)
        , lock(lock)
        , tag_tid(tag_tid)
        , tag_data(tag_data)
