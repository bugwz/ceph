...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::journal::PromoteRequest: " << this << " " << __func__ << ": "
...
template<typename I> void PromoteRequest<I>::send_open()
{
    ...
    Context* ctx = create_async_context_callback(
        *m_image_ctx, create_context_callback<PromoteRequest<I>, &PromoteRequest<I>::handle_open>(this));
    auto open_req =
        OpenRequest<I>::create(m_image_ctx, m_journaler, &m_lock, &m_client_meta, &m_tag_tid, &m_tag_data, ctx);
    open_req->send();
}
...
template<typename I> void PromoteRequest<I>::allocate_tag()
{
    ...
    if (!m_force && m_tag_data.mirror_uuid == Journal<>::ORPHAN_MIRROR_UUID) {
        ...
        predecessor = TagPredecessor{Journal<>::ORPHAN_MIRROR_UUID, true, m_tag_tid, 1};
    }
    else {
        // forced promotion -- create an epoch no peers can link against
        predecessor = TagPredecessor{Journal<>::LOCAL_MIRROR_UUID, true, m_tag_tid, 0};
    }
    ...
    Context* ctx = create_context_callback<PromoteRequest<I>, &PromoteRequest<I>::handle_allocate_tag>(this);
    m_journaler->allocate_tag(m_client_meta.tag_class, tag_bl, &m_tag, ctx);
}

template<typename I> void PromoteRequest<I>::handle_allocate_tag(int r)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << dendl;
...
template<typename I> void PromoteRequest<I>::append_event()
{
    ...
    auto ctx = create_context_callback<PromoteRequest<I>, &PromoteRequest<I>::handle_append_event>(this);
    m_future.flush(ctx);
}

template<typename I> void PromoteRequest<I>::handle_append_event(int r)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << dendl;
    ...
    if (r < 0) {
        m_ret_val = r;
        lderr(cct) << "failed to append promotion journal event: " << cpp_strerror(r) << dendl;
        stop_append();
        return;
    }
...
template<typename I> void PromoteRequest<I>::commit_event()
{
    ...
    auto ctx = create_context_callback<PromoteRequest<I>, &PromoteRequest<I>::handle_commit_event>(this);
    m_journaler->flush_commit_position(ctx);
}

template<typename I> void PromoteRequest<I>::handle_commit_event(int r)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << dendl;
    ...
    if (r < 0) {
        m_ret_val = r;
        lderr(cct) << "failed to flush promote commit position: " << cpp_strerror(r) << dendl;
    }
...
template<typename I> void PromoteRequest<I>::stop_append()
{
    ...
    auto ctx = create_context_callback<PromoteRequest<I>, &PromoteRequest<I>::handle_stop_append>(this);
    m_journaler->stop_append(ctx);
}

template<typename I> void PromoteRequest<I>::handle_stop_append(int r)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << dendl;
...
template<typename I> void PromoteRequest<I>::shut_down()
{
    ...
    Context* ctx = create_async_context_callback(
        *m_image_ctx, create_context_callback<PromoteRequest<I>, &PromoteRequest<I>::handle_shut_down>(this));
    m_journaler->shut_down(ctx);
}

template<typename I> void PromoteRequest<I>::handle_shut_down(int r)
{
    CephContext* cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << dendl;
