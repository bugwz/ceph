...
struct ImageAddedPayload
{
    ...
    ImageAddedPayload() {}
    ImageAddedPayload(const std::string& image_id, const cls::rbd::TrashImageSpec& trash_image_spec)
        : image_id(image_id)
        , trash_image_spec(trash_image_spec)
    {}
...
typedef boost::variant<ImageAddedPayload, ImageRemovedPayload, UnknownPayload> Payload;
