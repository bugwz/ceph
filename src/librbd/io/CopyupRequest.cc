...
#undef dout_prefix
#define dout_prefix                                                            \
    *_dout << "librbd::io::CopyupRequest: " << this << " " << __func__ << ": " \
...
template<typename I> class C_UpdateObjectMap : public C_AsyncObjectThrottle<I>
{
public:
    C_UpdateObjectMap(AsyncObjectThrottle<I>& throttle, I* image_ctx, uint64_t object_no, uint8_t head_object_map_state,
                      const std::vector<uint64_t>* snap_ids, bool first_snap_is_clean, const ZTracer::Trace& trace,
                      size_t snap_id_idx)
        : C_AsyncObjectThrottle<I>(throttle, *image_ctx)
        , m_object_no(object_no)
        , m_head_object_map_state(head_object_map_state)
        , m_snap_ids(*snap_ids)
        , m_first_snap_is_clean(first_snap_is_clean)
        , m_trace(trace)
    ...
    int update_head()
    {
        ...
        bool sent = image_ctx.object_map->template aio_update<Context>(
            CEPH_NOSNAP, m_object_no, m_head_object_map_state, {}, m_trace, false, this);
        return (sent ? 0 : 1);
    }
    ...
    int update_snapshot(uint64_t snap_id)
    {
        ...
        bool sent =
            image_ctx.object_map->template aio_update<Context>(snap_id, m_object_no, state, {}, m_trace, true, this);
        ceph_assert(sent);
        return 0;
    }
...
template<typename I>
CopyupRequest<I>::CopyupRequest(I* ictx, uint64_t objectno, Extents&& image_extents, ImageArea area,
...
template<typename I> CopyupRequest<I>::~CopyupRequest()
{
...
template<typename I>
void CopyupRequest<I>::append_request(AbstractObjectWriteRequest<I>* req, const Extents& object_extents)
{
...
template<typename I> void CopyupRequest<I>::send()
{
    read_from_parent();
}

template<typename I> void CopyupRequest<I>::read_from_parent()
{
    auto cct = m_image_ctx->cct;
    ...
    if (m_image_ctx->parent == nullptr) {
        ...
        m_image_ctx->asio_engine->post([this]() { handle_read_from_parent(-ENOENT); });
        return;
    }
    ...
    auto comp = AioCompletion::create_and_start<CopyupRequest<I>, &CopyupRequest<I>::handle_read_from_parent>(
        this, librbd::util::get_image_ctx(m_image_ctx->parent), AIO_TYPE_READ);

    ldout(cct, 20) << "completion=" << comp << " image_extents=" << m_image_extents << " area=" << m_image_area
                   << dendl;
    auto req = io::ImageDispatchSpec::create_read(*m_image_ctx->parent,
                                                  io::IMAGE_DISPATCH_LAYER_INTERNAL_START,
                                                  comp,
                                                  std::move(m_image_extents),
                                                  m_image_area,
                                                  ReadResult{&m_copyup_extent_map, &m_copyup_data},
                                                  m_image_ctx->parent->get_data_io_context(),
                                                  0,
                                                  0,
                                                  m_trace);
    req->send();
...
template<typename I> void CopyupRequest<I>::handle_read_from_parent(int r)
{
    auto cct = m_image_ctx->cct;
    ...
    if (r < 0) {
        ...
        lderr(m_image_ctx->cct) << "failed to prepare copyup data: " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
    ...
    // zeros.
    if (!m_copyup_is_zero) {
        m_snap_ids.insert(m_snap_ids.end(), m_image_ctx->snaps.rbegin(), m_image_ctx->snaps.rend());
    }
...
template<typename I> void CopyupRequest<I>::deep_copy()
{
    auto cct = m_image_ctx->cct;
    ...
    auto ctx = librbd::util::create_context_callback<CopyupRequest<I>, &CopyupRequest<I>::handle_deep_copy>(this);
    auto req = deep_copy::ObjectCopyRequest<I>::create(
        m_image_ctx->parent, m_image_ctx, 0, 0, m_image_ctx->migration_info.snap_map, m_object_no, flags, nullptr, ctx);

    req->send();
}

template<typename I> void CopyupRequest<I>::handle_deep_copy(int r)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << dendl;
    ...
    if (r < 0 && r != -ENOENT) {
        ...
        lderr(cct) << "error encountered during deep-copy: " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
...
template<typename I> void CopyupRequest<I>::update_object_maps()
{
    ...
    if (copy_on_read && !m_snap_ids.empty() &&
        m_image_ctx->test_features(RBD_FEATURE_FAST_DIFF, m_image_ctx->image_lock)) {
    ...
    typename AsyncObjectThrottle<I>::ContextFactory context_factory(
        boost::lambda::bind(boost::lambda::new_ptr<C_UpdateObjectMap<I>>(),
                            ...
                            boost::lambda::_2));
    auto ctx =
        librbd::util::create_context_callback<CopyupRequest<I>, &CopyupRequest<I>::handle_update_object_maps>(this);
    auto throttle =
        new AsyncObjectThrottle<I>(nullptr, *m_image_ctx, context_factory, ctx, nullptr, 0, m_snap_ids.size());
    throttle->start_ops(m_image_ctx->config.template get_val<uint64_t>("rbd_concurrent_management_ops"));
}

template<typename I> void CopyupRequest<I>::handle_update_object_maps(int r)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << dendl;

    if (r < 0) {
        lderr(m_image_ctx->cct) << "failed to update object map: " << cpp_strerror(r) << dendl;
...
template<typename I> void CopyupRequest<I>::copyup()
{
    auto cct = m_image_ctx->cct;
    m_image_ctx->image_lock.lock_shared();
    ...
    if (m_image_ctx->enable_sparse_copyup) {
        ...
        destriper.add_partial_sparse_result(
            cct, std::move(m_copyup_data), m_copyup_extent_map, 0, {{0, m_image_ctx->layout.object_size}});
    ...
    if (copyup_op.size() > 0) {
        ...
        m_image_ctx->rados_api.execute(object,
                                       copyup_io_context,
                                       std::move(copyup_op),
                                       librbd::asio::util::get_callback_adapter([this](int r) { handle_copyup(r); }),
                                       nullptr,
    ...
    if (write_op.size() > 0) {
        ...
        ldout(cct, 20) << (!deep_copyup && write_op.size() > 2 ? "copyup + ops"
                           : !deep_copyup                      ? "copyup"
                                                               : "ops")
...
template<typename I> void CopyupRequest<I>::handle_copyup(int r)
{
    auto cct = m_image_ctx->cct;
    ...
    if (pending_copyups == 0) {
        if (copyup_ret_val < 0 && copyup_ret_val != -ENOENT) {
            lderr(cct) << "failed to copyup object: " << cpp_strerror(copyup_ret_val) << dendl;
...
template<typename I> void CopyupRequest<I>::finish(int r)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << dendl;
...
template<typename I> void CopyupRequest<I>::complete_requests(bool override_restart_retval, int r)
{
    auto cct = m_image_ctx->cct;
    remove_from_list();
...
template<typename I> void CopyupRequest<I>::disable_append_requests()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    m_append_request_permitted = false;
}

template<typename I> void CopyupRequest<I>::remove_from_list()
{
...
template<typename I> bool CopyupRequest<I>::is_copyup_required()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
...
template<typename I> bool CopyupRequest<I>::is_deep_copy() const
{
    ceph_assert(ceph_mutex_is_locked(m_image_ctx->image_lock));
    return !m_image_ctx->migration_info.empty();
}

template<typename I> bool CopyupRequest<I>::is_update_object_map_required(int r)
{
    ceph_assert(ceph_mutex_is_locked(m_image_ctx->image_lock));
...
template<typename I> void CopyupRequest<I>::compute_deep_copy_snap_ids()
{
    ceph_assert(ceph_mutex_is_locked(m_image_ctx->image_lock));
    ...
    std::copy_if(m_image_ctx->snaps.rbegin(),
                 ...
                 [this, cct = m_image_ctx->cct, &deep_copied](uint64_t snap_id) {
                     ...
                     if (r < 0) {
                         ldout(cct, 5) << "failed getting parent overlap for snap_id: " << snap_id << ": "
                         ...
                         object_overlap = m_image_ctx->prune_parent_extents(parent_extents, area, raw_overlap, false);
                     }
                     return object_overlap > 0;
                 });
}

template<typename I> void CopyupRequest<I>::convert_copyup_extent_map()
{
    auto cct = m_image_ctx->cct;
    ...
    for (auto [image_offset, image_length] : image_extent_map) {
        ...
        util::area_to_object_extents(m_image_ctx, image_offset, image_length, m_image_area, 0, &object_extents);
        for (auto& object_extent : object_extents) {
            m_copyup_extent_map.emplace_back(object_extent.offset, object_extent.length);
        }
    }
...
template<typename I> int CopyupRequest<I>::prepare_copyup_data()
{
    ceph_assert(ceph_mutex_is_locked(m_image_ctx->image_lock));
    auto cct = m_image_ctx->cct;
    ...
    if (copy_on_read || maybe_deep_copyup) {
        ...
        for (auto [object_offset, object_length] : m_copyup_extent_map) {
            ...
            sparse_bufferlist.insert(
                object_offset, object_length, {SPARSE_EXTENT_STATE_DATA, object_length, std::move(sub_bl)});
        }
    }
    else {
        ...
        for (auto [object_offset, object_length] : m_copyup_extent_map) {
            ...
            intersection.intersection_of(copyup_object_extents, m_write_object_extents);
            ...
            for (auto [copyup_offset, copyup_length] : copyup_object_extents) {
                bufferlist sub_bl;
                sub_bl.substr_of(m_copyup_data, buffer_offset + (copyup_offset - object_offset), copyup_length);
                ceph_assert(sub_bl.length() == copyup_length);

                sparse_bufferlist.insert(
                    copyup_offset, copyup_length, {SPARSE_EXTENT_STATE_DATA, copyup_length, std::move(sub_bl)});
            }
            buffer_offset += object_length;
        }
    ...
    auto r = m_image_ctx->io_object_dispatcher->prepare_copyup(m_object_no, &snapshot_sparse_bufferlist);
    if (r < 0) {
        return r;
    }
