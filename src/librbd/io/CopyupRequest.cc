...
template<typename I>
CopyupRequest<I>::CopyupRequest(I* ictx, uint64_t objectno, Extents&& image_extents, const ZTracer::Trace& parent_trace)
    : m_image_ctx(ictx)
...
template<typename I> void CopyupRequest<I>::compute_deep_copy_snap_ids()
{
    ...
    std::copy_if(m_image_ctx->snaps.rbegin(),
                 ...
                 [this, cct = m_image_ctx->cct, &deep_copied](uint64_t snap_id) {
                     ...
                     util::extent_to_file(m_image_ctx, m_object_no, 0, m_image_ctx->layout.object_size, extents);
                     auto overlap = m_image_ctx->prune_parent_extents(extents, parent_overlap);
                     return overlap > 0;
                 });
}
...
template<typename I> void CopyupRequest<I>::convert_copyup_extent_map()
{
    ...
    for (auto [image_offset, image_length] : image_extent_map) {
        ...
        util::file_to_extents(m_image_ctx, image_offset, image_length, 0, &object_extents);
