...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::io::QosImageDispatch: " << this << " " << __func__ << ": "
    ...
    , m_flush_tracker(new FlushTracker<I>(image_ctx))
{
    ...
    for (auto [flag, name] : throttle_flags) {
        m_throttles.emplace_back(flag, new TokenBucketThrottle(cct, name, 0, 0, timer, timer_lock));
    }
}
...
template<typename I> void QosImageDispatch<I>::apply_qos_schedule_tick_min(uint64_t tick)
{
...
template<typename I>
void QosImageDispatch<I>::apply_qos_limit(uint64_t flag, uint64_t limit, uint64_t burst, uint64_t burst_seconds)
{
    auto cct = m_image_ctx->cct;
...
template<typename I> void QosImageDispatch<I>::apply_qos_exclude_ops(uint64_t exclude_ops)
{
    m_qos_exclude_ops = exclude_ops;
}

template<typename I>
bool QosImageDispatch<I>::read(AioCompletion* aio_comp, Extents&& image_extents, ReadResult&& read_result,
                               ...
                               DispatchResult* dispatch_result, Context** on_finish, Context* on_dispatched)
{
    ...
    if (needs_throttle(true, image_extents, tid, image_dispatch_flags, dispatch_result, on_finish, on_dispatched)) {
        return true;
    }
...
template<typename I>
bool QosImageDispatch<I>::write(AioCompletion* aio_comp, Extents&& image_extents, bufferlist&& bl, int op_flags,
                                const ZTracer::Trace& parent_trace, uint64_t tid,
                                std::atomic<uint32_t>* image_dispatch_flags, DispatchResult* dispatch_result,
                                Context** on_finish, Context* on_dispatched)
{
    ...
    if (needs_throttle(false, image_extents, tid, image_dispatch_flags, dispatch_result, on_finish, on_dispatched)) {
        return true;
    }
...
template<typename I>
bool QosImageDispatch<I>::discard(AioCompletion* aio_comp, Extents&& image_extents, uint32_t discard_granularity_bytes,
                                  const ZTracer::Trace& parent_trace, uint64_t tid,
                                  std::atomic<uint32_t>* image_dispatch_flags, DispatchResult* dispatch_result,
                                  Context** on_finish, Context* on_dispatched)
{
...
template<typename I>
bool QosImageDispatch<I>::write_same(AioCompletion* aio_comp, Extents&& image_extents, bufferlist&& bl, int op_flags,
                                     const ZTracer::Trace& parent_trace, uint64_t tid,
                                     std::atomic<uint32_t>* image_dispatch_flags, DispatchResult* dispatch_result,
                                     Context** on_finish, Context* on_dispatched)
{
...
template<typename I>
bool QosImageDispatch<I>::compare_and_write(AioCompletion* aio_comp, Extents&& image_extents, bufferlist&& cmp_bl,
...
template<typename I>
bool QosImageDispatch<I>::flush(AioCompletion* aio_comp, FlushSource flush_source, const ZTracer::Trace& parent_trace,
                                uint64_t tid, std::atomic<uint32_t>* image_dispatch_flags,
                                DispatchResult* dispatch_result, Context** on_finish, Context* on_dispatched)
{
    auto cct = m_image_ctx->cct;
...
template<typename I> void QosImageDispatch<I>::handle_finished(int r, uint64_t tid)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << "tid=" << tid << dendl;
...
template<typename I>
bool QosImageDispatch<I>::set_throttle_flag(std::atomic<uint32_t>* image_dispatch_flags, uint32_t flag)
{
    ...
    return ((desired & IMAGE_DISPATCH_FLAG_QOS_MASK) == IMAGE_DISPATCH_FLAG_QOS_MASK);
}

template<typename I>
bool QosImageDispatch<I>::needs_throttle(bool read_op, const Extents& image_extents, uint64_t tid,
                                         std::atomic<uint32_t>* image_dispatch_flags, DispatchResult* dispatch_result,
                                         Context** on_finish, Context* on_dispatched)
{
    auto cct = m_image_ctx->cct;
    ...
    *dispatch_result = DISPATCH_RESULT_CONTINUE;
    ...
    for (auto [flag, throttle] : m_throttles) {
        ...
        if (tokens > 0 && throttle->get(tokens,
                                        this,
                                        &QosImageDispatch<I>::handle_throttle_ready,
                                        Tag{image_dispatch_flags, on_dispatched},
                                        flag)) {
...
template<typename I> void QosImageDispatch<I>::handle_throttle_ready(Tag&& tag, uint64_t flag)
{
    auto cct = m_image_ctx->cct;
