...
static_assert((RBD_IO_OPERATION_COMPARE_AND_WRITE << 1) > RBD_IO_OPERATIONS_ALL, "new RBD io operation added");

std::string rbd_io_operations_to_string(uint64_t operations, std::ostream* err)
{
    std::string r;
    ...
    if (err && operations) {
        *err << "ignoring unknown io operation mask 0x" << std::hex << operations << std::dec;
    }
    return r;
}

uint64_t rbd_io_operations_from_string(const std::string& orig_value, std::ostream* err)
{
    ...
    try {
        ...
        if (unsupported_operations != 0ull) {
            ...
            if (err) {
                *err << "ignoring unknown operation mask 0x" << std::hex << unsupported_operations << std::dec;
            }
        }
    }
