...
template<typename I> inline bool is_copy_on_read(I* ictx, const IOContext& io_context)
{
    std::shared_lock image_locker{ictx->image_lock};
    return (ictx->clone_copy_on_read && !ictx->read_only &&
            io_context->read_snap().value_or(CEPH_NOSNAP) == CEPH_NOSNAP &&
            (ictx->exclusive_lock == nullptr || ictx->exclusive_lock->is_lock_owner()));
}

template<typename S, typename D> void convert_snap_set(const S& src_snap_set, D* dst_snap_set)
{
...
template<typename I>
ObjectRequest<I>* ObjectRequest<I>::create_write(I* ictx, uint64_t object_no, uint64_t object_off,
                                                 ceph::bufferlist&& data, IOContext io_context, int op_flags,
                                                 int write_flags, std::optional<uint64_t> assert_version,
                                                 const ZTracer::Trace& parent_trace, Context* completion)
{
    return new ObjectWriteRequest<I>(ictx,
                                     object_no,
                                     object_off,
                                     std::move(data),
                                     io_context,
...
template<typename I>
ObjectRequest<I>* ObjectRequest<I>::create_discard(I* ictx, uint64_t object_no, uint64_t object_off,
                                                   uint64_t object_len, IOContext io_context, int discard_flags,
                                                   const ZTracer::Trace& parent_trace, Context* completion)
{
    return new ObjectDiscardRequest<I>(
        ictx, object_no, object_off, object_len, io_context, discard_flags, parent_trace, completion);
}

template<typename I>
ObjectRequest<I>* ObjectRequest<I>::create_write_same(I* ictx, uint64_t object_no, uint64_t object_off,
                                                      uint64_t object_len, ceph::bufferlist&& data,
                                                      IOContext io_context, int op_flags,
                                                      const ZTracer::Trace& parent_trace, Context* completion)
{
    return new ObjectWriteSameRequest<I>(
        ictx, object_no, object_off, object_len, std::move(data), io_context, op_flags, parent_trace, completion);
}

template<typename I>
ObjectRequest<I>* ObjectRequest<I>::create_compare_and_write(I* ictx, uint64_t object_no, uint64_t object_off,
                                                             ceph::bufferlist&& cmp_data, ceph::bufferlist&& write_data,
                                                             IOContext io_context, uint64_t* mismatch_offset,
                                                             int op_flags, const ZTracer::Trace& parent_trace,
                                                             Context* completion)
{
    return new ObjectCompareAndWriteRequest<I>(ictx,
                                               object_no,
                                               object_off,
                                               std::move(cmp_data),
                                               std::move(write_data),
...
template<typename I>
ObjectRequest<I>::ObjectRequest(I* ictx, uint64_t objectno, IOContext io_context, const char* trace_name,
    ...
    , m_trace(create_trace(*ictx, "", trace))
{
    ...
    if (m_trace.valid()) {
        m_trace.copy_name(trace_name + std::string(" ") + data_object_name(ictx, objectno));
        m_trace.event("start");
    }
}

template<typename I> void ObjectRequest<I>::add_write_hint(I& image_ctx, neorados::WriteOp* wr)
{
    auto alloc_hint_flags = static_cast<neorados::alloc_hint::alloc_hint_t>(image_ctx.alloc_hint_flags);
    if (image_ctx.enable_alloc_hint) {
        wr->set_alloc_hint(image_ctx.get_object_size(), image_ctx.get_object_size(), alloc_hint_flags);
    }
...
template<typename I>
bool ObjectRequest<I>::compute_parent_extents(Extents* parent_extents, ImageArea* area, bool read_request)
{
    ...
    *area = ImageArea::DATA;
    ...
    int r = m_ictx->get_parent_overlap(m_io_context->read_snap().value_or(CEPH_NOSNAP), &raw_overlap);
    if (r < 0) {
        // NOTE: it's possible for a snapshot to be deleted while we are
        // still reading from it
        lderr(m_ictx->cct) << "failed to retrieve parent overlap: " << cpp_strerror(r) << dendl;
        return false;
    }
    ...
    std::tie(*parent_extents, *area) =
        io::util::object_to_area_extents(m_ictx, m_object_no, {{0, m_ictx->layout.object_size}});
    uint64_t object_overlap = m_ictx->prune_parent_extents(*parent_extents, *area, raw_overlap, migration_write);
    if (object_overlap > 0) {
        m_has_parent = true;
        return true;
    }
...
template<typename I>
ObjectReadRequest<I>::ObjectReadRequest(I* ictx, uint64_t objectno, ReadExtents* extents, IOContext io_context,
                                        int op_flags, int read_flags, const ZTracer::Trace& parent_trace,
                                        uint64_t* version, Context* completion)
    : ObjectRequest<I>(ictx, objectno, io_context, "read", parent_trace, completion)
    , m_extents(extents)
    , m_op_flags(op_flags)
...
template<typename I> void ObjectReadRequest<I>::read_object()
{
    ...
    util::apply_op_flags(m_op_flags, image_ctx->get_read_flags(read_snap_id), &read_op);

    image_ctx->rados_api.execute({data_object_name(this->m_ictx, this->m_object_no)},
                                 *this->m_io_context,
                                 std::move(read_op),
                                 nullptr,
                                 librbd::asio::util::get_callback_adapter([this](int r) { handle_read_object(r); }),
                                 m_version,
...
template<typename I> void ObjectReadRequest<I>::handle_read_object(int r)
{
    ...
    if (r == -ENOENT) {
        ...
        lderr(image_ctx->cct) << "failed to read from object: " << cpp_strerror(r) << dendl;
        this->finish(r);
        return;
    }
...
template<typename I> void ObjectReadRequest<I>::read_parent()
{
    ...
    auto ctx = create_context_callback<ObjectReadRequest<I>, &ObjectReadRequest<I>::handle_read_parent>(this);

    io::util::read_parent<I>(image_ctx,
                             this->m_object_no,
                             this->m_extents,
...
template<typename I> void ObjectReadRequest<I>::handle_read_parent(int r)
{
    ...
    if (r == -ENOENT) {
        ...
        lderr(image_ctx->cct) << "failed to read parent extents: " << cpp_strerror(r) << dendl;
        this->finish(r);
        return;
    }
...
template<typename I> void ObjectReadRequest<I>::copyup()
{
    ...
    if (!this->compute_parent_extents(&parent_extents, &area, true) ||
        (image_ctx->exclusive_lock != nullptr && !image_ctx->exclusive_lock->is_lock_owner())) {
    ...
    if (it == image_ctx->copyup_list.end()) {
        ...
        auto new_req =
            CopyupRequest<I>::create(image_ctx, this->m_object_no, std::move(parent_extents), area, this->m_trace);
...
template<typename I>
AbstractObjectWriteRequest<I>::AbstractObjectWriteRequest(I* ictx, uint64_t object_no, uint64_t object_off,
                                                          uint64_t len, IOContext io_context, const char* trace_name,
                                                          const ZTracer::Trace& parent_trace, Context* completion)
    : ObjectRequest<I>(ictx, object_no, io_context, trace_name, parent_trace, completion)
    , m_object_off(object_off)
    , m_object_len(len)
{
    if (this->m_object_off == 0 && this->m_object_len == ictx->get_object_size()) {
        m_full_object = true;
    }
...
template<typename I> void AbstractObjectWriteRequest<I>::compute_parent_info()
{
    I* image_ctx = this->m_ictx;
    std::shared_lock image_locker{image_ctx->image_lock};
    ...
    if (!this->has_parent() ||
        (m_full_object && !this->m_io_context->write_snap_context() && !is_post_copyup_write_required())) {
        m_copyup_enabled = false;
    }
}

template<typename I> void AbstractObjectWriteRequest<I>::add_write_hint(neorados::WriteOp* wr)
{
    I* image_ctx = this->m_ictx;
    std::shared_lock image_locker{image_ctx->image_lock};
    if (image_ctx->object_map == nullptr || !this->m_object_may_exist || image_ctx->alloc_hint_flags != 0U) {
...
template<typename I> void AbstractObjectWriteRequest<I>::send()
{
    ...
    ldout(image_ctx->cct, 20) << this->get_op_type() << " " << this->m_object_off << "~" << this->m_object_len << dendl;
    {
        ...
        if (image_ctx->object_map == nullptr) {
            ...
            m_object_may_exist = image_ctx->object_map->object_may_exist(this->m_object_no);
        }
    }
...
template<typename I> void AbstractObjectWriteRequest<I>::pre_write_object_map_update()
{
    I* image_ctx = this->m_ictx;
    ...
    if (image_ctx->object_map->template aio_update<AbstractObjectWriteRequest<I>,
                                                   &AbstractObjectWriteRequest<I>::handle_pre_write_object_map_update>(
            CEPH_NOSNAP, this->m_object_no, new_state, {}, this->m_trace, false, this)) {
        image_ctx->image_lock.unlock_shared();
        return;
    }
...
template<typename I> void AbstractObjectWriteRequest<I>::handle_pre_write_object_map_update(int r)
{
    I* image_ctx = this->m_ictx;
    ldout(image_ctx->cct, 20) << "r=" << r << dendl;
    if (r < 0) {
        lderr(image_ctx->cct) << "failed to update object map: " << cpp_strerror(r) << dendl;
        this->finish(r);
        return;
    }
...
template<typename I> void AbstractObjectWriteRequest<I>::write_object()
{
    ...
    if (m_copyup_enabled) {
        if (m_guarding_migration_write) {
            auto snap_seq =
                (this->m_io_context->write_snap_context() ? this->m_io_context->write_snap_context()->first : 0);
            ldout(image_ctx->cct, 20) << "guarding write: snap_seq=" << snap_seq << dendl;

            cls_client::assert_snapc_seq(&write_op, snap_seq, cls::rbd::ASSERT_SNAPC_SEQ_LE_SNAPSET_SEQ);
        }
        else {
...
template<typename I> void AbstractObjectWriteRequest<I>::handle_write_object(int r)
{
    I* image_ctx = this->m_ictx;
    ldout(image_ctx->cct, 20) << "r=" << r << dendl;
    ...
    if (r == -ENOENT) {
        ...
        lderr(image_ctx->cct) << "failed to write object: " << cpp_strerror(r) << dendl;
        this->finish(r);
        return;
    }
...
template<typename I> void AbstractObjectWriteRequest<I>::copyup()
{
    ...
    if (it == image_ctx->copyup_list.end()) {
        auto new_req = CopyupRequest<I>::create(
            image_ctx, this->m_object_no, std::move(this->m_parent_extents), m_image_area, this->m_trace);
...
template<typename I> void AbstractObjectWriteRequest<I>::handle_copyup(int r)
{
    ...
    if (r < 0 && r != -ERESTART) {
        lderr(image_ctx->cct) << "failed to copyup object: " << cpp_strerror(r) << dendl;
        this->finish(r);
        return;
    }
...
template<typename I> void AbstractObjectWriteRequest<I>::post_write_object_map_update()
{
    I* image_ctx = this->m_ictx;
    ...
    if (image_ctx->object_map->template aio_update<AbstractObjectWriteRequest<I>,
            ...
            CEPH_NOSNAP, this->m_object_no, OBJECT_NONEXISTENT, OBJECT_PENDING, this->m_trace, false, this)) {
        image_ctx->image_lock.unlock_shared();
        return;
    }
...
template<typename I> void AbstractObjectWriteRequest<I>::handle_post_write_object_map_update(int r)
{
    I* image_ctx = this->m_ictx;
    ldout(image_ctx->cct, 20) << "r=" << r << dendl;
    if (r < 0) {
...
template<typename I> void ObjectWriteRequest<I>::add_write_hint(neorados::WriteOp* wr)
{
...
template<typename I> void ObjectWriteRequest<I>::add_write_ops(neorados::WriteOp* wr)
{
...
template<typename I> void ObjectDiscardRequest<I>::add_write_ops(neorados::WriteOp* wr)
{
...
template<typename I> void ObjectWriteSameRequest<I>::add_write_ops(neorados::WriteOp* wr)
{
    wr->writesame(this->m_object_off, this->m_object_len, bufferlist{m_write_data});
    util::apply_op_flags(m_op_flags, 0U, wr);
}

template<typename I> void ObjectCompareAndWriteRequest<I>::add_write_ops(neorados::WriteOp* wr)
{
...
template<typename I> int ObjectCompareAndWriteRequest<I>::filter_write_result(int r) const
{
    if (r <= -MAX_ERRNO) {
        I* image_ctx = this->m_ictx;
        ...
        auto [image_extents, _] =
            io::util::object_to_area_extents(image_ctx, this->m_object_no, {{offset, this->m_object_len}});
...
template<typename I>
ObjectListSnapsRequest<I>::ObjectListSnapsRequest(I* ictx, uint64_t objectno, Extents&& object_extents,
                                                  SnapIds&& snap_ids, int list_snaps_flags,
                                                  const ZTracer::Trace& parent_trace, SnapshotDelta* snapshot_delta,
                                                  Context* completion)
    : ObjectRequest<I>(ictx, objectno, ictx->duplicate_data_io_context(), "snap_list", parent_trace, completion)
...
template<typename I> void ObjectListSnapsRequest<I>::list_snaps()
{
    ...
    image_ctx->rados_api.execute({data_object_name(this->m_ictx, this->m_object_no)},
                                 *this->m_io_context,
                                 std::move(read_op),
                                 nullptr,
                                 librbd::asio::util::get_callback_adapter([this](int r) { handle_list_snaps(r); }),
                                 nullptr,
                                 (this->m_trace.valid() ? this->m_trace.get_info() : nullptr));
}
...
template<typename I> void ObjectListSnapsRequest<I>::handle_list_snaps(int r)
{
    ...
    if (r == -ENOENT) {
        ...
        lderr(cct) << "failed to retrieve object snapshot list: " << cpp_strerror(r) << dendl;
        this->finish(r);
        return;
    }
    ...
    for (auto end_snap_id : m_snap_ids) {
        ...
        if (read_whole_object || (!diff.empty() && ((m_list_snaps_flags & LIST_SNAPS_FLAG_WHOLE_OBJECT) != 0))) {
        ...
        if (exists && start_snap_id == 0 && (!diff_interval.empty() || !zero_interval.empty())) {
        ...
        if (exists) {
            for (auto& interval : diff_interval) {
                snapshot_delta[{end_snap_id, clone_end_snap_id}].insert(
                    interval.first, interval.second, SparseExtent(SPARSE_EXTENT_STATE_DATA, interval.second));
            }
        }
        else {
        ...
        if ((m_list_snaps_flags & LIST_SNAPS_FLAG_IGNORE_ZEROED_EXTENTS) == 0) {
            for (auto& interval : zero_interval) {
                snapshot_delta[{end_snap_id, end_snap_id}].insert(
                    interval.first, interval.second, SparseExtent(SPARSE_EXTENT_STATE_ZEROED, interval.second));
            }
        }
    }
...
template<typename I> void ObjectListSnapsRequest<I>::list_from_parent()
{
    ...
    if (raw_overlap > 0) {
        ...
        std::tie(parent_extents, m_image_area) =
            io::util::object_to_area_extents(image_ctx, this->m_object_no, m_object_extents);
        object_overlap = image_ctx->prune_parent_extents(parent_extents, m_image_area, raw_overlap, false);
    }
    if (object_overlap == 0) {
        image_locker.unlock();
    ...
    auto ctx =
        create_context_callback<ObjectListSnapsRequest<I>, &ObjectListSnapsRequest<I>::handle_list_from_parent>(this);
    auto aio_comp =
        AioCompletion::create_and_start(ctx, librbd::util::get_image_ctx(image_ctx->parent), AIO_TYPE_GENERIC);
    ldout(cct, 20) << "completion=" << aio_comp << " parent_extents=" << parent_extents << " area=" << m_image_area
                   << dendl;

    auto list_snaps_flags = (m_list_snaps_flags | LIST_SNAPS_FLAG_IGNORE_ZEROED_EXTENTS);

    ImageListSnapsRequest<I> req(*image_ctx->parent,
                                 aio_comp,
                                 std::move(parent_extents),
                                 m_image_area,
                                 {0, image_ctx->parent->snap_id},
                                 list_snaps_flags,
...
template<typename I> void ObjectListSnapsRequest<I>::handle_list_from_parent(int r)
{
    I* image_ctx = this->m_ictx;
    auto cct = image_ctx->cct;

    ldout(cct, 20) << "r=" << r << ", "
                   << "parent_snapshot_delta=" << m_parent_snapshot_delta << dendl;
    ...
    for (auto& [key, image_extents] : m_parent_snapshot_delta) {
        for (auto image_extent : image_extents) {
            ...
            io::util::area_to_object_extents(
                image_ctx, image_extent.get_off(), image_extent.get_len(), m_image_area, 0, &object_extents);
            for (auto& object_extent : object_extents) {
                ceph_assert(object_extent.object_no == this->m_object_no);
                intervals.insert(object_extent.offset, object_extent.length, {state, object_extent.length});
            }
        }
    }
...
template<typename I> void ObjectListSnapsRequest<I>::zero_extent(uint64_t snap_id, bool dne)
{
    I* image_ctx = this->m_ictx;
    auto cct = image_ctx->cct;
    ...
    if ((m_list_snaps_flags & LIST_SNAPS_FLAG_IGNORE_ZEROED_EXTENTS) == 0) {
        ...
        for (auto [offset, length] : interval) {
            ldout(cct, 20) << "snapshot " << snap_id << ": " << (dne ? "DNE" : "zeroed") << " extent " << offset << "~"
                           << length << dendl;
            (*m_snapshot_delta)[{snap_id, snap_id}].insert(
                offset, length, SparseExtent((dne ? SPARSE_EXTENT_STATE_DNE : SPARSE_EXTENT_STATE_ZEROED), length));
        }
    }
}
