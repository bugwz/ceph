...
template<typename I> bool ObjectRequest<I>::compute_parent_extents(Extents* parent_extents, bool read_request)
{
    ...
    int r = m_ictx->get_parent_overlap(m_io_context->read_snap().value_or(CEPH_NOSNAP), &parent_overlap);
    if (r < 0) {
    ...
    io::util::extent_to_file(m_ictx, m_object_no, 0, m_ictx->layout.object_size, *parent_extents);
    uint64_t object_overlap = m_ictx->prune_parent_extents(*parent_extents, parent_overlap);
    if (object_overlap > 0) {
...
template<typename I> void ObjectReadRequest<I>::copyup()
{
    ...
    if (it == image_ctx->copyup_list.end()) {
        ...
        auto new_req = CopyupRequest<I>::create(image_ctx, this->m_object_no, std::move(parent_extents), this->m_trace);
...
template<typename I> void AbstractObjectWriteRequest<I>::copyup()
{
    ...
    if (it == image_ctx->copyup_list.end()) {
        auto new_req =
            CopyupRequest<I>::create(image_ctx, this->m_object_no, std::move(this->m_parent_extents), this->m_trace);
...
template<typename I> int ObjectCompareAndWriteRequest<I>::filter_write_result(int r) const
{
    if (r <= -MAX_ERRNO) {
        ...
        io::util::extent_to_file(image_ctx, this->m_object_no, offset, this->m_object_len, image_extents);
...
template<typename I> void ObjectListSnapsRequest<I>::list_from_parent()
{
    ...
    for (auto [object_off, object_len] : m_object_extents) {
        io::util::extent_to_file(image_ctx, this->m_object_no, object_off, object_len, parent_image_extents);
    }
    ...
    if (r == 0) {
        object_overlap = image_ctx->prune_parent_extents(parent_image_extents, parent_overlap);
    }
    ...
    *m_snapshot_delta = {};
    ...
    for (auto& [key, image_extents] : m_parent_snapshot_delta) {
        for (auto image_extent : image_extents) {
            ...
            io::util::file_to_extents(image_ctx, image_extent.get_off(), image_extent.get_len(), 0, &object_extents);
