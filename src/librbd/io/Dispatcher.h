...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::io::Dispatcher: " << this << " " << __func__ << ": "

namespace librbd {
namespace io {

template<typename ImageCtxT, typename DispatchInterfaceT> class Dispatcher : public DispatchInterfaceT
{
public:
    ...
    Dispatcher(ImageCtxT* image_ctx)
        ...
        , m_lock(ceph::make_shared_mutex(librbd::util::unique_lock_name("librbd::io::Dispatcher::lock", this)))
    {}
    ...
    void shut_down_dispatch(DispatchLayer dispatch_layer, Context* on_finish) override
    {
        auto cct = m_image_ctx->cct;
...
protected:
    ...
    virtual bool send_dispatch(Dispatch* dispatch, DispatchSpec* dispatch_spec) = 0;
...
protected:
    struct C_LayerIterator : public Context
    {
        ...
        C_LayerIterator(Dispatcher* dispatcher, DispatchLayer start_layer, Context* on_finish)
            : dispatcher(dispatcher)
            , on_finish(on_finish)
    ...
    struct C_InvalidateCache : public C_LayerIterator
    {
        ...
        bool execute(Dispatch* dispatch, Context* on_finish) override { return dispatch->invalidate_cache(on_finish); }
    };
...
private:
    void shut_down_dispatch(DispatchMeta& dispatch_meta, Context** on_finish)
    {
        ...
        ctx = new LambdaContext([dispatch, ctx](int r) { dispatch->shut_down(ctx); });
        *on_finish = new LambdaContext([async_op_tracker, ctx](int r) { async_op_tracker->wait_for_ops(ctx); });
    }
};
