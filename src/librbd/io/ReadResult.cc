...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::io::ReadResult: " << this << " " << __func__ << ": "
...
struct ReadResult::SetImageExtentsVisitor : public boost::static_visitor<void>
{
    ...
    void operator()(SparseBufferlist& sbl) const { sbl.image_extents = image_extents; }
...
struct ReadResult::AssembleResultVisitor : public boost::static_visitor<void>
{
    ...
    void operator()(Empty& empty) const { ldout(cct, 20) << "dropping read result" << dendl; }
    ...
    void operator()(Bufferlist& bufferlist) const
    {
        ...
        ldout(cct, 20) << "moved resulting " << bufferlist.bl->length() << " "
                       << "bytes to bl " << reinterpret_cast<void*>(bufferlist.bl) << dendl;
    }
    ...
    void operator()(SparseBufferlist& sparse_bufferlist) const
    {
        ...
        auto buffer_extents_length = destriper.assemble_result(cct, &buffer_extent_map, sparse_bufferlist.bl);

        ldout(cct, 20) << "image_extents=" << sparse_bufferlist.image_extents << ", "
        ...
        for (auto [image_offset, image_length] : sparse_bufferlist.image_extents) {
            while (bem_it != buffer_extent_map.end()) {
                ...
                ceph_assert(buffer_offset <= buffer_extent_offset &&
                            buffer_offset + image_length >= buffer_extent_offset + buffer_extent_length);
                auto image_extent_offset = image_offset + (buffer_extent_offset - buffer_offset);
                ldout(cct, 20) << "mapping buffer extent " << buffer_extent_offset << "~" << buffer_extent_length
                               << " to image extent " << image_extent_offset << "~" << buffer_extent_length << dendl;
                sparse_bufferlist.extent_map->emplace_back(image_extent_offset, buffer_extent_length);
                ++bem_it;
            }
        ...
        ldout(cct, 20) << "moved resulting " << *sparse_bufferlist.extent_map << " extents of total "
                       << sparse_bufferlist.bl->length() << " bytes to bl "
                       << reinterpret_cast<void*>(sparse_bufferlist.bl) << dendl;
    }
};

ReadResult::C_ImageReadRequest::C_ImageReadRequest(AioCompletion* aio_completion, uint64_t buffer_offset,
...
void ReadResult::C_ImageReadRequest::finish(int r)
{
    ...
    if (r >= 0 || (ignore_enoent && r == -ENOENT)) {
        ...
        aio_completion->read_result.m_destriper.add_partial_result(cct, std::move(bl), buffer_extents);
...
ReadResult::C_ObjectReadRequest::C_ObjectReadRequest(AioCompletion* aio_completion, ReadExtents&& extents)
    : aio_completion(aio_completion)
...
void ReadResult::C_ObjectReadRequest::finish(int r)
{
    ...
    if (r >= 0) {
        ...
        for (auto& extent : extents) {
            ldout(cct, 10) << " got " << extent.extent_map << " for " << extent.buffer_extents << " bl "
                           << extent.bl.length() << dendl;

            aio_completion->read_result.m_destriper.add_partial_sparse_result(
                cct, std::move(extent.bl), extent.extent_map, extent.offset, extent.buffer_extents);
...
ReadResult::C_ObjectReadMergedExtents::C_ObjectReadMergedExtents(CephContext* cct, ReadExtents* extents,
                                                                 Context* on_finish)
    : cct(cct)
    , extents(extents)
    , on_finish(on_finish)
