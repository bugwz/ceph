...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::io::AioCompletion: " << this << " " << __func__ << ": "
...
void AioCompletion::complete()
{
    ...
    if ((aio_type == AIO_TYPE_CLOSE) || (aio_type == AIO_TYPE_OPEN && r < 0)) {
        ...
        if (ictx->perfcounter != nullptr) {
            ...
            case AIO_TYPE_OPEN: break;
            case AIO_TYPE_READ: ictx->perfcounter->tinc(l_librbd_rd_latency, elapsed); break;
            case AIO_TYPE_WRITE: ictx->perfcounter->tinc(l_librbd_wr_latency, elapsed); break;
            case AIO_TYPE_DISCARD: ictx->perfcounter->tinc(l_librbd_discard_latency, elapsed); break;
            case AIO_TYPE_FLUSH: ictx->perfcounter->tinc(l_librbd_flush_latency, elapsed); break;
            case AIO_TYPE_WRITESAME: ictx->perfcounter->tinc(l_librbd_ws_latency, elapsed); break;
            case AIO_TYPE_COMPARE_AND_WRITE: ictx->perfcounter->tinc(l_librbd_cmp_latency, elapsed); break;
            default: lderr(cct) << "completed invalid aio_type: " << aio_type << dendl; break;
            }
        }
    }
...
void AioCompletion::queue_complete()
{
    ...
    boost::asio::post(ictx->asio_engine->get_api_strand(), [this]() { complete_request(0); });
}
