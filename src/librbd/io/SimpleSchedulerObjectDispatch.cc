...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::io::SimpleSchedulerObjectDispatch: " << this << " " << __func__ << ": "
...
public:
    ...
    bool is_ready() const { return rolling_count(m_acc) == LATENCY_STATS_WINDOW_SIZE; }
...
template<typename I>
bool SimpleSchedulerObjectDispatch<I>::ObjectRequests::try_delay_request(uint64_t object_off, ceph::bufferlist&& data,
                                                                         IOContext io_context, int op_flags,
                                                                         int object_dispatch_flags,
                                                                         Context* on_dispatched)
{
    if (!m_delayed_requests.empty()) {
        if (!m_io_context || *m_io_context != *io_context || op_flags != m_op_flags || data.length() == 0 ||
    ...
    if (!m_delayed_requests.empty()) {
        ...
        if (iter != m_delayed_requests.begin() && (iter == m_delayed_requests.end() || iter->first > object_off)) {
            iter--;
        }
        if (iter != m_delayed_requests.end() && iter->first + iter->second.data.length() == object_off) {
    ...
    typename std::map<uint64_t, MergedRequests>::iterator& iter2)
{
    ...
    iter1->second.requests.insert(
        iter1->second.requests.end(), iter2->second.requests.begin(), iter2->second.requests.end());
    m_delayed_requests.erase(iter2);
}

template<typename I>
void SimpleSchedulerObjectDispatch<I>::ObjectRequests::dispatch_delayed_requests(I* image_ctx,
                                                                                 ...
                                                                                 ceph::mutex* latency_stats_lock)
{
    for (auto& it : m_delayed_requests) {
        ...
        auto ctx = new LambdaContext([requests = std::move(merged_requests.requests),
                                      latency_stats,
                                      latency_stats_lock,
                                      start_time = ceph_clock_now()](int r) {
            if (latency_stats) {
...
SimpleSchedulerObjectDispatch<I>::SimpleSchedulerObjectDispatch(I* image_ctx)
    ...
    , m_lock(ceph::make_mutex(librbd::util::unique_lock_name("librbd::io::SimpleSchedulerObjectDispatch::lock", this)))
    , m_max_delay(image_ctx->config.template get_val<uint64_t>("rbd_io_scheduler_simple_max_delay"))
{
...
template<typename I> SimpleSchedulerObjectDispatch<I>::~SimpleSchedulerObjectDispatch()
{
...
template<typename I> void SimpleSchedulerObjectDispatch<I>::shut_down(Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 5) << dendl;
...
template<typename I>
bool SimpleSchedulerObjectDispatch<I>::read(uint64_t object_no, ReadExtents* extents, IOContext io_context,
...
template<typename I>
bool SimpleSchedulerObjectDispatch<I>::discard(uint64_t object_no, uint64_t object_off, uint64_t object_len,
...
template<typename I>
bool SimpleSchedulerObjectDispatch<I>::write(uint64_t object_no, uint64_t object_off, ceph::bufferlist&& data,
                                             ...
                                             Context* on_dispatched)
{
    ...
    if (assert_version.has_value() || (write_flags & OBJECT_WRITE_FLAG_CREATE_EXCLUSIVE) != 0) {
        dispatch_delayed_requests(object_no);
        return false;
    }

    if (try_delay_write(
            object_no, object_off, std::move(data), io_context, op_flags, *object_dispatch_flags, on_dispatched)) {

        auto dispatch_seq = ++m_dispatch_seq;
        m_flush_tracker->start_io(dispatch_seq);
        *on_finish = new LambdaContext([this, dispatch_seq, ctx = *on_finish](int r) {
            ctx->complete(r);
...
template<typename I>
bool SimpleSchedulerObjectDispatch<I>::write_same(uint64_t object_no, uint64_t object_off, uint64_t object_len,
...
template<typename I>
bool SimpleSchedulerObjectDispatch<I>::compare_and_write(uint64_t object_no, uint64_t object_off,
...
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << data_object_name(m_image_ctx, object_no) << " " << object_off << "~" << cmp_data.length()
                   << dendl;
...
template<typename I>
bool SimpleSchedulerObjectDispatch<I>::flush(FlushSource flush_source, const ZTracer::Trace& parent_trace,
                                             uint64_t* journal_tid, DispatchResult* dispatch_result,
                                             Context** on_finish, Context* on_dispatched)
{
    auto cct = m_image_ctx->cct;
...
template<typename I>
bool SimpleSchedulerObjectDispatch<I>::intersects(uint64_t object_no, uint64_t object_off, uint64_t len) const
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    auto cct = m_image_ctx->cct;

    auto it = m_requests.find(object_no);
    bool intersects = (it != m_requests.end()) && it->second->intersects(object_off, len);
...
template<typename I>
bool SimpleSchedulerObjectDispatch<I>::try_delay_write(uint64_t object_no, uint64_t object_off, ceph::bufferlist&& data,
                                                       ...
                                                       Context* on_dispatched)
{
    ...
    bool delayed = object_requests->try_delay_request(
        object_off, std::move(data), io_context, op_flags, object_dispatch_flags, on_dispatched);
...
template<typename I> void SimpleSchedulerObjectDispatch<I>::dispatch_all_delayed_requests()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << dendl;
...
template<typename I>
void SimpleSchedulerObjectDispatch<I>::register_in_flight_request(uint64_t object_no, const utime_t& start_time,
                                                                  Context** on_finish)
{
    auto res = m_requests.insert({object_no, std::make_shared<ObjectRequests>(object_no)});
    ...
    *on_finish = new LambdaContext([this, object_no, dispatch_seq, start_time, ctx = *on_finish](int r) {
        ctx->complete(r);
        ...
        auto it = m_requests.find(object_no);
        if (it == m_requests.end() || it->second->get_dispatch_seq() != dispatch_seq) {
...
template<typename I> void SimpleSchedulerObjectDispatch<I>::dispatch_delayed_requests(uint64_t object_no)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    auto cct = m_image_ctx->cct;
...
template<typename I> void SimpleSchedulerObjectDispatch<I>::dispatch_delayed_requests(ObjectRequestsRef object_requests)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    auto cct = m_image_ctx->cct;
    ...
    object_requests->dispatch_delayed_requests(m_image_ctx, m_latency_stats.get(), &m_lock);
...
template<typename I> void SimpleSchedulerObjectDispatch<I>::schedule_dispatch_delayed_requests()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    auto cct = m_image_ctx->cct;
    ...
    m_timer_task = new LambdaContext([this, object_no = object_requests->get_object_no()](int r) {
