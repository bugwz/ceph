...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::io::FlushTracker: " << this << " " << __func__ << ": "
...
FlushTracker<I>::FlushTracker(I* image_ctx)
    ...
    , m_lock(ceph::make_shared_mutex(util::unique_lock_name("librbd::io::FlushTracker::m_lock", this)))
{}

template<typename I> FlushTracker<I>::~FlushTracker()
{
    std::unique_lock locker{m_lock};
    ceph_assert(m_flush_contexts.empty());
}

template<typename I> void FlushTracker<I>::shut_down()
{
...
template<typename I> uint64_t FlushTracker<I>::start_io(uint64_t tid)
{
    auto cct = m_image_ctx->cct;
...
template<typename I> void FlushTracker<I>::finish_io(uint64_t tid)
{
    auto cct = m_image_ctx->cct;
...
template<typename I> void FlushTracker<I>::flush(Context* on_finish)
{
    auto cct = m_image_ctx->cct;
