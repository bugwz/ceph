...
static_assert((RBD_FEATURE_DIRTY_CACHE << 1) > RBD_FEATURES_ALL, "new RBD feature added");
...
std::string rbd_features_to_string(uint64_t features, std::ostream* err)
{
    ...
    if (err && features) {
        *err << "ignoring unknown feature mask 0x" << std::hex << features << std::dec;
    }
    return r;
}

uint64_t rbd_features_from_string(const std::string& orig_value, std::ostream* err)
{
    uint64_t features = 0;
    ...
    try {
        ...
        if (unsupported_features != 0ull) {
            ...
            if (err) {
                *err << "ignoring unknown feature mask 0x" << std::hex << unsupported_features << std::dec;
            }
        }

        uint64_t ignore_features_mask = (RBD_FEATURES_INTERNAL | RBD_FEATURES_MUTABLE_INTERNAL);
