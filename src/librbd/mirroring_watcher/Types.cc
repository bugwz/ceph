...
void ImageUpdatedPayload::decode(__u8 version, bufferlist::const_iterator& iter)
{
    ...
    mirror_image_state = static_cast<cls::rbd::MirrorImageState>(mirror_image_state_decode);
...
void NotifyMessage::decode(bufferlist::const_iterator& iter)
{
    ...
    switch (notify_op) {
    case NOTIFY_OP_MODE_UPDATED: payload = ModeUpdatedPayload(); break;
    case NOTIFY_OP_IMAGE_UPDATED: payload = ImageUpdatedPayload(); break;
...
void NotifyMessage::generate_test_instances(std::list<NotifyMessage*>& o)
{
    ...
    o.push_back(
        new NotifyMessage(ImageUpdatedPayload(cls::rbd::MIRROR_IMAGE_STATE_DISABLING, "image id", "global image id")));
}

std::ostream& operator<<(std::ostream& out, const NotifyOp& op)
{
    switch (op) {
    case NOTIFY_OP_MODE_UPDATED: out << "ModeUpdated"; break;
    case NOTIFY_OP_IMAGE_UPDATED: out << "ImageUpdated"; break;
