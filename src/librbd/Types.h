...
struct SnapInfo
{
    ...
    uint64_t flags;
    utime_t timestamp;
    SnapInfo(std::string _name, const cls::rbd::SnapshotNamespace& _snap_namespace, uint64_t _size,
             const ParentImageInfo& _parent, uint8_t _protection_status, uint64_t _flags, utime_t _timestamp)
        : name(_name)
...
struct MigrationInfo
{
    ...
    MigrationInfo() {}
    MigrationInfo(int64_t pool_id, const std::string& pool_namespace, const std::string& image_name,
                  const std::string& image_id, const std::string& source_spec, const deep_copy::SnapMap& snap_map,
                  uint64_t overlap, bool flatten)
        : pool_id(pool_id)
    ...
    bool empty() const { return (pool_id == -1 && source_spec.empty()); }
};
