...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::BlockGuard: " << this << " " << __func__ << ": "
...
struct BlockExtent
{
    ...
    {
        os << "[block_start=" << block_extent.block_start << ", block_end=" << block_extent.block_end << "]";
        return os;
    }
};
 ...
 */
template<typename BlockOperation> class BlockGuard
{
private:
...
public:
    ...
    int detain(const BlockExtent& block_extent, BlockOperation* block_operation, BlockGuardCell** cell)
    {
        ...
        ldout(m_cct, 20) << block_extent << ", free_slots=" << m_free_detained_block_extents.size() << dendl;
        ...
        if (it != m_detained_block_extents.end()) {
            ...
            if (block_operation != nullptr) {
                detained_block_extent->block_operations.emplace_back(std::move(*block_operation));
            }
    ...
    void release(BlockGuardCell* cell, BlockOperations* block_operations)
    {
        ...
        ldout(m_cct, 20) << detained_block_extent.block_extent
                         << ", pending_ops=" << detained_block_extent.block_operations.size() << dendl;
...
private:
    struct DetainedBlockExtent : public boost::intrusive::list_base_hook<>, public boost::intrusive::set_base_hook<>
    {
    ...
    struct DetainedBlockExtentKey
    {
        ...
        const BlockExtent& operator()(const DetainedBlockExtent& value) { return value.block_extent; }
    };

    struct DetainedBlockExtentCompare
    {
        bool operator()(const BlockExtent& lhs, const BlockExtent& rhs) const
        {
    ...
    typedef boost::intrusive::set<DetainedBlockExtent, boost::intrusive::compare<DetainedBlockExtentCompare>,
