...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::cache::WriteAroundObjectDispatch: " << this << " " << __func__ << ": "
...
template<typename I>
WriteAroundObjectDispatch<I>::WriteAroundObjectDispatch(I* image_ctx, size_t max_dirty, bool writethrough_until_flush)
    : m_image_ctx(image_ctx)
    , m_init_max_dirty(max_dirty)
    , m_max_dirty(max_dirty)
    , m_lock(ceph::make_mutex(util::unique_lock_name("librbd::cache::WriteAroundObjectDispatch::lock", this)))
{
...
template<typename I> WriteAroundObjectDispatch<I>::~WriteAroundObjectDispatch() {}

template<typename I> void WriteAroundObjectDispatch<I>::init()
{
...
template<typename I> void WriteAroundObjectDispatch<I>::shut_down(Context* on_finish)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 5) << dendl;
...
template<typename I>
bool WriteAroundObjectDispatch<I>::read(uint64_t object_no, io::ReadExtents* extents, IOContext io_context,
                                        ...
                                        Context* on_dispatched)
{
    ...
    for (auto& extent : *extents) {
        handled |= dispatch_unoptimized_io(object_no, extent.offset, extent.length, dispatch_result, on_dispatched);
    }
    return handled;
}

template<typename I>
bool WriteAroundObjectDispatch<I>::discard(uint64_t object_no, uint64_t object_off, uint64_t object_len,
                                           ...
                                           Context* on_dispatched)
{
    ...
    return dispatch_io(object_no, object_off, object_len, 0, dispatch_result, on_finish, on_dispatched);
}

template<typename I>
bool WriteAroundObjectDispatch<I>::write(uint64_t object_no, uint64_t object_off, ceph::bufferlist&& data,
                                         ...
                                         Context* on_dispatched)
{
    ...
    return dispatch_io(object_no, object_off, data.length(), op_flags, dispatch_result, on_finish, on_dispatched);
}

template<typename I>
bool WriteAroundObjectDispatch<I>::write_same(uint64_t object_no, uint64_t object_off, uint64_t object_len,
                                              ...
                                              Context* on_dispatched)
{
    ...
    return dispatch_io(object_no, object_off, object_len, op_flags, dispatch_result, on_finish, on_dispatched);
}

template<typename I>
bool WriteAroundObjectDispatch<I>::compare_and_write(uint64_t object_no, uint64_t object_off,
                                                     ...
                                                     Context* on_dispatched)
{
    return dispatch_unoptimized_io(object_no, object_off, cmp_data.length(), dispatch_result, on_dispatched);
}
                                         ...
                                         Context** on_finish, Context* on_dispatched)
{
    ...
    *on_finish = new LambdaContext([this, tid](int r) { handle_in_flight_flush_complete(r, tid); });
...
template<typename I>
bool WriteAroundObjectDispatch<I>::dispatch_unoptimized_io(uint64_t object_no, uint64_t object_off, uint64_t object_len,
                                                           io::DispatchResult* dispatch_result, Context* on_dispatched)
{
    ...
    *dispatch_result = io::DISPATCH_RESULT_CONTINUE;
    m_blocked_unoptimized_ios[object_no].emplace(tid, BlockedIO{object_off, object_len, nullptr, on_dispatched});
    m_lock.unlock();
...
template<typename I>
bool WriteAroundObjectDispatch<I>::dispatch_io(uint64_t object_no, uint64_t object_off, uint64_t object_len,
                                               ...
                                               Context* on_dispatched)
{
    ...
    if ((op_flags & LIBRADOS_OP_FLAG_FADVISE_FUA) != 0) {
        ...
        m_lock.unlock();
        return dispatch_unoptimized_io(object_no, object_off, object_len, dispatch_result, on_dispatched);
    }
    ...
    *dispatch_result = io::DISPATCH_RESULT_CONTINUE;
    *on_finish = new LambdaContext([this, tid, object_no, object_off, object_len](int r) {
        handle_in_flight_io_complete(r, tid, object_no, object_off, object_len);
    });

    bool blocked = block_overlapping_io(&m_in_flight_extents[object_no], object_off, object_len);
    if (blocked) {
        ldout(cct, 20) << "blocked on overlap: tid=" << tid << dendl;
        m_queued_or_blocked_io_tids.insert(tid);
        m_blocked_ios[object_no].emplace(tid, BlockedIO{object_off, object_len, ctx, on_dispatched});
        m_lock.unlock();
    }
...
template<typename I>
bool WriteAroundObjectDispatch<I>::block_overlapping_io(InFlightObjectExtents* in_flight_object_extents,
...
template<typename I>
void WriteAroundObjectDispatch<I>::unblock_overlapping_ios(uint64_t object_no, uint64_t object_off, uint64_t object_len,
                                                           Contexts* unoptimized_io_dispatches)
{
    ...
    if (blocked_unoptimized_ios_it != m_blocked_unoptimized_ios.end()) {
        ...
        for (auto it = blocked_unoptimized_object_ios.begin(); it != blocked_unoptimized_object_ios.end();) {
            auto& blocked_io = it->second;
            if (!in_flight_object_extents.intersects(blocked_io.offset, blocked_io.length)) {
                unoptimized_io_dispatches->emplace(it->first, blocked_io.on_dispatched);
                it = blocked_unoptimized_object_ios.erase(it);
            }
            else {
                blocked_unoptimized_ios.union_insert(blocked_io.offset, blocked_io.length);
                ++it;
            }
        }
    ...
    if (blocked_io_it != m_blocked_ios.end()) {
        ...
        while (blocked_object_ios_it != blocked_object_ios.end()) {
            ...
            if (blocked_unoptimized_ios.intersects(blocked_io.offset, blocked_io.length) ||
                block_overlapping_io(&in_flight_object_extents, blocked_io.offset, blocked_io.length)) {
                break;
            }
...
template<typename I> bool WriteAroundObjectDispatch<I>::can_dispatch_io(uint64_t tid, uint64_t length)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
...
template<typename I>
void WriteAroundObjectDispatch<I>::handle_in_flight_io_complete(int r, uint64_t tid, uint64_t object_no,
                                                                uint64_t object_off, uint64_t object_len)
{
    auto cct = m_image_ctx->cct;
    ...
    if (r < 0) {
        lderr(cct) << "IO error encountered: tid=" << tid << ": " << cpp_strerror(r) << dendl;
    ...
    Contexts unoptimized_io_dispatches;
    unblock_overlapping_ios(object_no, object_off, object_len, &unoptimized_io_dispatches);
...
template<typename I> void WriteAroundObjectDispatch<I>::handle_in_flight_flush_complete(int r, uint64_t tid)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << "r=" << r << ", tid=" << tid << dendl;
...
template<typename I> typename WriteAroundObjectDispatch<I>::QueuedIOs WriteAroundObjectDispatch<I>::collect_ready_ios()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ...
    while (true) {
        ...
        if (it == m_queued_ios.end() || !can_dispatch_io(it->first, it->second.length)) {
            break;
        }
...
template<typename I>
typename WriteAroundObjectDispatch<I>::Contexts WriteAroundObjectDispatch<I>::collect_ready_flushes()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ...
    while (true) {
        ...
        if (it == m_queued_flushes.end() ||
            (io_tid_it != m_queued_or_blocked_io_tids.end() && *io_tid_it < it->first)) {
            break;
        }
...
template<typename I>
typename WriteAroundObjectDispatch<I>::Contexts WriteAroundObjectDispatch<I>::collect_finished_flushes()
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ...
    while (true) {
        ...
        if (it == m_pending_flushes.end() || (io_tid_it != m_in_flight_io_tids.end() && *io_tid_it < it->first)) {
            break;
        }
