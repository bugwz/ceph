 ...
 */
template<typename T> class LogMapEntry
{
public:
    ...
    template<typename U> friend std::ostream& operator<<(std::ostream& os, LogMapEntry<U>& e);
};

template<typename T> using LogMapEntries = std::list<LogMapEntry<T>>;

template<typename T> class LogMap
{
public:
