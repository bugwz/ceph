 ...
 */
template<typename T> class C_BlockIORequest : public Context
{
public:
    T& pwl;
    ...
    C_BlockIORequest(T& pwl, const utime_t arrived, io::Extents&& extents, bufferlist&& bl, const int fadvise_flags,
                     Context* user_req);
    ...
    virtual const char* get_name() const { return "C_BlockIORequest"; }

    uint64_t get_image_extents_size() { return image_extents.size(); }

    std::vector<WriteBufferAllocation>& get_resources_buffers() { return m_resources.buffers; }
    ...
    virtual void setup_buffer_resources(uint64_t* bytes_cached, uint64_t* bytes_dirtied, uint64_t* bytes_allocated,
...
private:
    ...
    template<typename U> friend std::ostream& operator<<(std::ostream& os, const C_BlockIORequest<U>& req);
};
 ...
 * overlapping writes).
 */
template<typename T> class C_WriteRequest : public C_BlockIORequest<T>
{
public:
    ...
    C_WriteRequest(T& pwl, const utime_t arrived, io::Extents&& image_extents, bufferlist&& bl, const int fadvise_flags,
                   ceph::mutex& lock, PerfCounters* perfcounter, Context* user_req);

    C_WriteRequest(T& pwl, const utime_t arrived, io::Extents&& image_extents, bufferlist&& cmp_bl, bufferlist&& bl,
                   uint64_t* mismatch_offset, int fadvise_flags, ceph::mutex& lock, PerfCounters* perfcounter,
                   Context* user_req);
    ...
    const char* get_name() const override { return "C_WriteRequest"; }
...
private:
    ...
    bool m_queued = false;
    ceph::mutex& m_lock;
    template<typename U> friend std::ostream& operator<<(std::ostream& os, const C_WriteRequest<U>& req);
};
 ...
 */
template<typename T> class C_FlushRequest : public C_BlockIORequest<T>
{
public:
    using C_BlockIORequest<T>::pwl;
    bool internal = false;
    ...
    C_FlushRequest(T& pwl, const utime_t arrived, io::Extents&& image_extents, bufferlist&& bl, const int fadvise_flags,
                   ceph::mutex& lock, PerfCounters* perfcounter, Context* user_req);
    ...
    const char* get_name() const override { return "C_FlushRequest"; }

    void setup_buffer_resources(uint64_t* bytes_cached, uint64_t* bytes_dirtied, uint64_t* bytes_allocated,
                                uint64_t* number_lanes, uint64_t* number_log_entries,
...
private:
    ...
    void finish_req(int r) override;
    void deferred_handler() override { m_perfcounter->inc(l_librbd_pwl_aio_flush_def, 1); }

    template<typename U> friend std::ostream& operator<<(std::ostream& os, const C_FlushRequest<U>& req);
};
 ...
 * everywhere.
 */
template<typename T> class C_DiscardRequest : public C_BlockIORequest<T>
{
public:
    using C_BlockIORequest<T>::pwl;
    std::shared_ptr<DiscardLogOperation> op;

    C_DiscardRequest(T& pwl, const utime_t arrived, io::Extents&& image_extents, uint32_t discard_granularity_bytes,
                     ceph::mutex& lock, PerfCounters* perfcounter, Context* user_req);
    ...
    const char* get_name() const override { return "C_DiscardRequest"; }
    void setup_buffer_resources(uint64_t* bytes_cached, uint64_t* bytes_dirtied, uint64_t* bytes_allocated,
                                uint64_t* number_lanes, uint64_t* number_log_entries,
                                uint64_t* number_unpublished_reserves) override;
...
private:
    ...
    ceph::mutex& m_lock;
    PerfCounters* m_perfcounter = nullptr;
    template<typename U> friend std::ostream& operator<<(std::ostream& os, const C_DiscardRequest<U>& req);
};
 ...
 */
template<typename T> class C_WriteSameRequest : public C_WriteRequest<T>
{
public:
    using C_BlockIORequest<T>::pwl;
    C_WriteSameRequest(T& pwl, const utime_t arrived, io::Extents&& image_extents, bufferlist&& bl,
                       const int fadvise_flags, ceph::mutex& lock, PerfCounters* perfcounter, Context* user_req);
    ...
    const char* get_name() const override { return "C_WriteSameRequest"; }

    template<typename U> friend std::ostream& operator<<(std::ostream& os, const C_WriteSameRequest<U>& req);
};
...
struct BlockGuardReqState
{
    ...
    {
        os << "barrier=" << r.barrier << ", current_barrier=" << r.current_barrier << ", detained=" << r.detained
           << ", queued=" << r.queued;
        return os;
    }
};
...
public:
    ...
    GuardedRequest(const BlockExtent block_extent, GuardedRequestFunctionContext* on_guard_acquire,
                   bool barrier = false)
        : block_extent(block_extent)
        , guard_ctx(on_guard_acquire)
    {
        guard_ctx->state.barrier = barrier;
    }
    friend std::ostream& operator<<(std::ostream& os, const GuardedRequest& r)
    {
        os << "guard_ctx->state=[" << r.guard_ctx->state << "], block_extent.block_start=" << r.block_extent.block_start
