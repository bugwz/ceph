...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::cache::pwl::ssd::WriteLog: " << this << " " << __func__ << ": "
...
{
    return root.pool_size % MIN_WRITE_ALLOC_SSD_SIZE == 0 && root.first_valid_entry >= DATA_RING_BUFFER_OFFSET &&
           root.first_valid_entry < root.pool_size && root.first_valid_entry % MIN_WRITE_ALLOC_SSD_SIZE == 0 &&
           root.first_free_entry >= DATA_RING_BUFFER_OFFSET && root.first_free_entry < root.pool_size &&
           root.first_free_entry % MIN_WRITE_ALLOC_SSD_SIZE == 0;
}

template<typename I> Builder<AbstractWriteLog<I>>* WriteLog<I>::create_builder()
{
...
template<typename I>
WriteLog<I>::WriteLog(I& image_ctx, librbd::cache::pwl::ImageCacheState<I>* cache_state,
                      cache::ImageWritebackInterface& image_writeback, plugin::Api<I>& plugin_api)
    : AbstractWriteLog<I>(image_ctx, cache_state, create_builder(), image_writeback, plugin_api)
{}

template<typename I> WriteLog<I>::~WriteLog()
{
    delete m_builderobj;
}

template<typename I>
void WriteLog<I>::collect_read_extents(uint64_t read_buffer_offset, LogMapEntry<GenericWriteLogEntry> map_entry,
                                       ...
                                       Extent hit_extent, pwl::C_ReadRequest* read_ctx)
{
    ...
    auto hit_extent_buf = std::make_shared<ImageExtentBuf>(hit_extent, hit_bl, true, read_buffer_offset, writesame);
...
template<typename I>
void WriteLog<I>::complete_read(std::vector<std::shared_ptr<GenericWriteLogEntry>>& log_entries_to_read,
...
template<typename I> int WriteLog<I>::create_and_open_bdev()
{
    CephContext* cct = m_image_ctx.cct;

    bdev = BlockDevice::create(cct, this->m_log_pool_name, aio_cache_cb, nullptr, nullptr, nullptr);
    ...
    if (bdev->get_size() != this->m_log_pool_size) {
        lderr(cct) << "size mismatch: bdev size " << bdev->get_size() << " (block size " << bdev->get_block_size()
...
template<typename I> bool WriteLog<I>::initialize_pool(Context* on_finish, pwl::DeferredContexts& later)
{
    int r;
    CephContext* cct = m_image_ctx.cct;
    ...
    if (access(this->m_log_pool_name.c_str(), F_OK) != 0) {
        ...
        bool succeed = true;
        if (fd >= 0) {
            if (truncate(this->m_log_pool_name.c_str(), this->m_log_pool_size) != 0) {
                succeed = false;
            }
            ::close(fd);
        }
        ...
        this->m_bytes_allocated_cap = this->m_log_pool_size - DATA_RING_BUFFER_OFFSET - MIN_WRITE_ALLOC_SSD_SIZE;
        /* Log ring empty */
        ...
        if (pool_root.layout_version != SSD_LAYOUT_VERSION) {
            lderr(cct) << "pool layout version is " << pool_root.layout_version << " expected " << SSD_LAYOUT_VERSION
                       << dendl;
            goto err_close_bdev;
        }
        if (pool_root.block_size != MIN_WRITE_ALLOC_SSD_SIZE) {
            lderr(cct) << "pool block size is " << pool_root.block_size << " expected " << MIN_WRITE_ALLOC_SSD_SIZE
                       << dendl;
            goto err_close_bdev;
        }
...
template<typename I> void WriteLog<I>::remove_pool_file()
{
    ceph_assert(bdev);
    bdev->close();
    delete bdev;
    ...
    if (m_cache_state->clean) {
        ...
        if (remove(this->m_log_pool_name.c_str()) != 0) {
            lderr(m_image_ctx.cct) << "failed to remove empty pool \"" << this->m_log_pool_name << "\": " << dendl;
        }
        else {
            m_cache_state->present = false;
        }
...
template<typename I> void WriteLog<I>::load_existing_entries(pwl::DeferredContexts& later)
{
    CephContext* cct = m_image_ctx.cct;
    ...
    for (uint64_t next_log_pos = this->m_first_valid_entry; next_log_pos != this->m_first_free_entry;) {
        // read the entries from SSD cache and decode
        bufferlist bl_entries;
        ::IOContext ioctx_entry(cct, nullptr);
        bdev->read(next_log_pos, MIN_WRITE_ALLOC_SSD_SIZE, &bl_entries, &ioctx_entry, false);
        ...
        for (auto it = ssd_log_entries.begin(); it != ssd_log_entries.end(); ++it) {
            this->update_entries(&log_entry, &*it, missing_sync_points, sync_point_entries, curr_log_pos);
    ...
    if (m_first_valid_entry > m_first_free_entry) {
        m_bytes_allocated = this->m_log_pool_size - m_first_valid_entry + m_first_free_entry - DATA_RING_BUFFER_OFFSET;
    }
    else {
...
template<typename I> void WriteLog<I>::inc_allocated_cached_bytes(std::shared_ptr<pwl::GenericLogEntry> log_entry)
{
...
template<typename I> bool WriteLog<I>::alloc_resources(C_BlockIORequestT* req)
{
    ...
    req->setup_buffer_resources(
        &bytes_cached, &bytes_dirtied, &bytes_allocated, &num_lanes, &num_log_entries, &num_unpublished_reserves);
    ...
    alloc_succeeds = this->check_allocation(
        req, bytes_cached, bytes_dirtied, bytes_allocated, num_lanes, num_log_entries, num_unpublished_reserves);
    req->set_allocated(alloc_succeeds);
    return alloc_succeeds;
}

template<typename I> bool WriteLog<I>::has_sync_point_logs(GenericLogOperations& ops)
{
    for (auto& op : ops) {
...
template<typename I> void WriteLog<I>::enlist_op_appender()
{
    this->m_async_append_ops++;
    this->m_async_op_tracker.start_op();
    Context* append_ctx = new LambdaContext([this](int r) { append_scheduled_ops(); });
 ...
 */
template<typename I> void WriteLog<I>::schedule_append_ops(GenericLogOperations& ops, C_BlockIORequestT* req)
{
    ...
    std::copy(std::begin(ops), std::end(ops), std::back_inserter(appending));
    {
        ...
        need_finisher = !this->m_appending &&
                        ((this->m_ops_to_append.size() >= CONTROL_BLOCK_MAX_LOG_ENTRIES) || !persist_on_flush);
...
template<typename I>
void WriteLog<I>::setup_schedule_append(pwl::GenericLogOperationsVector& ops, bool do_early_flush,
                                        C_BlockIORequestT* req)
{
    this->schedule_append(ops, req);
}

template<typename I> void WriteLog<I>::append_scheduled_ops(void)
{
 ...
 */
template<typename I> void WriteLog<I>::append_op_log_entries(GenericLogOperations& ops)
{
    ...
    Context* ctx = new LambdaContext([this, ops](int r) {
        ...
        bool need_finisher = false;
        {
            ...
            bool persist_on_flush = this->get_persist_on_flush();
            need_finisher = ((this->m_ops_to_append.size() >= CONTROL_BLOCK_MAX_LOG_ENTRIES) || !persist_on_flush);
...
template<typename I> void WriteLog<I>::release_ram(std::shared_ptr<GenericLogEntry> log_entry)
{
    log_entry->remove_cache_bl();
}

template<typename I> void WriteLog<I>::alloc_op_log_entries(GenericLogOperations& ops)
{
...
template<typename I>
void WriteLog<I>::construct_flush_entries(pwl::GenericLogEntries entries_to_flush, DeferredContexts& post_unlock,
    ...
    if (invalidating || !has_write_entry) {
        for (auto& log_entry : entries_to_flush) {
            GuardedRequestFunctionContext* guarded_ctx = new GuardedRequestFunctionContext(
                [this, log_entry, invalidating](GuardedRequestFunctionContext& guard_ctx) {
                    ...
                    if (!invalidating) {
                        ctx = new LambdaContext([this, log_entry, ctx](int r) {
                            m_image_ctx.op_work_queue->queue(new LambdaContext([this, log_entry, ctx](int r) {
                                                                 ldout(m_image_ctx.cct, 15)
                                                                     << "flushing:" << log_entry << " " << *log_entry
                                                                     << dendl;
        ...
        Context* ctx = new LambdaContext([this, entries_to_flush, read_bls](int r) {
            int i = 0;
                ...
                if (log_entry->is_write_entry()) {
                    ...
                    guarded_ctx = new GuardedRequestFunctionContext([this, log_entry, captured_entry_bl](
                        ...
                        m_image_ctx.op_work_queue->queue(
                            new LambdaContext([this, log_entry, entry_bl = std::move(captured_entry_bl), ctx](int r) {
                                auto captured_entry_bl = std::move(entry_bl);
                                ldout(m_image_ctx.cct, 15) << "flushing:" << log_entry << " " << *log_entry << dendl;
                                log_entry->writeback_bl(this->m_image_writeback, ctx, std::move(captured_entry_bl));
                            }),
                            0);
                    });
                }
                    ...
                    guarded_ctx =
                        new GuardedRequestFunctionContext([this, log_entry](GuardedRequestFunctionContext& guard_ctx) {
                            log_entry->m_cell = guard_ctx.cell;
                            Context* ctx = this->construct_flush_entry(log_entry, false);
                            m_image_ctx.op_work_queue->queue(new LambdaContext([this, log_entry, ctx](int r) {
                                                                 ldout(m_image_ctx.cct, 15)
...
template<typename I> void WriteLog<I>::process_work()
{
    CephContext* cct = m_image_ctx.cct;
    ...
    do {
        ...
        if (this->m_alloc_failed_since_retire || (this->m_shutting_down) || this->m_invalidating ||
            ...
            ldout(m_image_ctx.cct, 10) << "alloc_fail=" << this->m_alloc_failed_since_retire
                                       << ", allocated > high_water=" << (m_bytes_allocated > high_water_bytes)
                                       << dendl;
            retire_entries(
                (this->m_shutting_down || this->m_invalidating || m_bytes_allocated > aggressive_high_water_bytes)
 ...
 *
 */
template<typename I> bool WriteLog<I>::retire_entries(const unsigned long int frees_per_tx)
{
    CephContext* cct = m_image_ctx.cct;
        ...
        while (retiring_entries.size() < frees_per_tx && !m_log_entries.empty()) {
            ...
            for (auto it = m_log_entries.begin(); it != m_log_entries.end(); ++it) {
                if (this->can_retire_entry(*it)) {
                    ...
                    if ((*it)->log_entry_index != control_block_pos) {
                        ldout(cct, 20) << "Old log_entry_index is " << control_block_pos << ",New log_entry_index is "
                                       << (*it)->log_entry_index << ",data length is " << data_length << dendl;
            ...
            if (!retiring_subentries.empty()) {
                ...
                ldout(cct, 20) << "span with " << retiring_subentries.size()
                               << " entries: control_block_pos=" << control_block_pos << " data_length=" << data_length
                               << dendl;
                retiring_entries.insert(retiring_entries.end(), retiring_subentries.begin(), retiring_subentries.end());

                first_valid_entry = control_block_pos + data_length + MIN_WRITE_ALLOC_SSD_SIZE;
                if (first_valid_entry >= this->m_log_pool_size) {
                    first_valid_entry = first_valid_entry % this->m_log_pool_size + DATA_RING_BUFFER_OFFSET;
                }
            }
            else {
                break;
    ...
    if (retiring_entries.size()) {
        ...
        Context* ctx = new LambdaContext([this, first_valid_entry, initial_first_valid_entry, retiring_entries](int r) {
            ...
            bool need_update_state = false;
            {
                ...
                ldout(m_image_ctx.cct, 20)
                    << "Finished root update: initial_first_valid_entry=" << initial_first_valid_entry
                    << ", m_first_valid_entry=" << m_first_valid_entry << ", release space = " << allocated_bytes
                    << ", m_bytes_allocated=" << m_bytes_allocated << ", release cached space=" << cached_bytes
...
template<typename I>
void WriteLog<I>::append_ops(GenericLogOperations& ops, Context* ctx, uint64_t* new_first_free_entry)
{
    ...
    *new_first_free_entry = pool_root.first_free_entry;
    ...
    for (auto& operation : ops) {
        ...
        if (log_entries.size() == CONTROL_BLOCK_MAX_LOG_ENTRIES || span_payload_len >= SPAN_MAX_DATA_LEN) {
...
template<typename I>
void WriteLog<I>::write_log_entries(GenericLogEntriesVector log_entries, AioTransContext* aio, uint64_t* pos)
{
    CephContext* cct = m_image_ctx.cct;
    ldout(m_image_ctx.cct, 20) << "pos=" << *pos << dendl;
    ceph_assert(*pos >= DATA_RING_BUFFER_OFFSET && *pos < this->m_log_pool_size &&
    ...
    *pos += MIN_WRITE_ALLOC_SSD_SIZE;
    ...
    if (control_block_pos + bl.length() > this->m_log_pool_size) {
        ...
        ldout(cct, 20) << "write " << control_block_pos << "~" << size << " spans boundary, split into "
                       << control_block_pos << "~" << bl1.length() << " and " << DATA_RING_BUFFER_OFFSET << "~"
                       << bl.length() << dendl;
        bdev->aio_write(control_block_pos, bl1, &aio->ioc, false, WRITE_LIFE_NOT_SET);
        bdev->aio_write(DATA_RING_BUFFER_OFFSET, bl, &aio->ioc, false, WRITE_LIFE_NOT_SET);
    }
    else {
        ldout(cct, 20) << "write " << control_block_pos << "~" << bl.length() << dendl;
        bdev->aio_write(control_block_pos, bl, &aio->ioc, false, WRITE_LIFE_NOT_SET);
    }
}

template<typename I> void WriteLog<I>::schedule_update_root(std::shared_ptr<WriteLogPoolRoot> root, Context* ctx)
{
    CephContext* cct = m_image_ctx.cct;
    ldout(cct, 15) << "New root: pool_size=" << root->pool_size << " first_valid_entry=" << root->first_valid_entry
                   << " first_free_entry=" << root->first_free_entry << " flushed_sync_gen=" << root->flushed_sync_gen
                   << dendl;
...
template<typename I> void WriteLog<I>::enlist_op_update_root()
{
    Context* append_ctx = new LambdaContext([this](int r) { update_root_scheduled_ops(); });
    this->m_work_queue.queue(append_ctx);
}

template<typename I> void WriteLog<I>::update_root_scheduled_ops()
{
    ldout(m_image_ctx.cct, 20) << dendl;
...
template<typename I> void WriteLog<I>::update_pool_root(std::shared_ptr<WriteLogPoolRoot> root, AioTransContext* aio)
{
    bufferlist bl;
    SuperBlock superblock;
...
template<typename I> int WriteLog<I>::update_pool_root_sync(std::shared_ptr<WriteLogPoolRoot> root)
{
    bufferlist bl;
    SuperBlock superblock;
    superblock.root = *root;
...
template<typename I>
void WriteLog<I>::aio_read_data_block(std::shared_ptr<GenericWriteLogEntry> log_entry, bufferlist* bl, Context* ctx)
{
...
template<typename I>
void WriteLog<I>::aio_read_data_blocks(std::vector<std::shared_ptr<GenericWriteLogEntry>>& log_entries,
                                       std::vector<bufferlist*>& bls, Context* ctx)
{
    ceph_assert(log_entries.size() == bls.size());

    // get the valid part
    Context* read_ctx = new LambdaContext([log_entries, bls, ctx](int r) {
    ...
    for (unsigned int i = 0; i < log_entries.size(); i++) {
        ...
        uint64_t len = log_entry->is_write() ? log_entry->write_bytes : log_entry->ws_datalen;
        uint64_t align_len = round_up_to(len, MIN_WRITE_ALLOC_SSD_SIZE);

        ldout(cct, 20) << "entry i=" << i << " " << log_entry->write_data_pos << "~" << len << dendl;
        ...
        if (log_entry->write_data_pos + align_len > pool_root.pool_size) {
            ...
            ldout(cct, 20) << "read " << log_entry->write_data_pos << "~" << align_len << " spans boundary, split into "
                           << log_entry->write_data_pos << "~" << len1 << " and " << DATA_RING_BUFFER_OFFSET << "~"
                           << len2 << dendl;
            bdev->aio_read(log_entry->write_data_pos, len1, bls[i], &aio->ioc);
            bdev->aio_read(DATA_RING_BUFFER_OFFSET, len2, bls[i], &aio->ioc);
        }
        else {
            ldout(cct, 20) << "read " << log_entry->write_data_pos << "~" << align_len << dendl;
...
template<typename I> void WriteLog<I>::complete_user_request(Context*& user_req, int r)
{
