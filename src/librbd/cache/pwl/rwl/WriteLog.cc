...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::cache::pwl::rwl::WriteLog: " << this << " " << __func__ << ": "
...
WriteLog<I>::WriteLog(I& image_ctx, librbd::cache::pwl::ImageCacheState<I>* cache_state,
                      ImageWritebackInterface& image_writeback, plugin::Api<I>& plugin_api)
 ...
 */
template<typename I> int WriteLog<I>::append_op_log_entries(GenericLogOperations& ops)
{
    ...
    for (auto& operation : ops) {
        ...
        ldout(m_image_ctx.cct, 20) << "Copying entry for operation at index="
                                   ...
                                   << "operation=[" << *operation << "]" << dendl;
        ldout(m_image_ctx.cct, 05) << "APPENDING: index=" << operation->get_log_entry()->log_entry_index << " "
                                   << "operation=[" << *operation << "]" << dendl;
        operation->log_append_start_time = now;
        *operation->get_log_entry()->cache_entry = operation->get_log_entry()->ram_entry;
        ldout(m_image_ctx.cct, 20) << "APPENDING: index=" << operation->get_log_entry()->log_entry_index << " "
                                   << "pmem_entry=[" << *operation->get_log_entry()->cache_entry << "]" << dendl;
    ...
    TX_ONABORT
    {
        lderr(cct) << "failed to commit " << ops.size() << " log entries (" << this->m_log_pool_name << ")" << dendl;
        ceph_assert(false);
        ret = -EIO;
    }
    TX_FINALLY {}
    ...
    m_perfcounter->hinc(l_librbd_pwl_append_tx_t_hist, utime_t(tx_end - tx_start).to_nsec(), ops.size());
    for (auto& operation : ops) {
 ...
 */
template<typename I> void WriteLog<I>::flush_op_log_entries(GenericLogOperationsVector& ops)
{
    if (ops.empty()) {
        return;
    }
    ...
    ldout(m_image_ctx.cct, 20) << "entry count=" << ops.size() << " "
                               << "start address=" << ops.front()->get_log_entry()->cache_entry << " "
                               << "bytes=" << ops.size() * sizeof(*(ops.front()->get_log_entry()->cache_entry))
                               << dendl;
...
template<typename I> bool WriteLog<I>::initialize_pool(Context* on_finish, pwl::DeferredContexts& later)
{
    ...
    if (access(this->m_log_pool_name.c_str(), F_OK) != 0) {
        if ((m_log_pool = pmemobj_create(this->m_log_pool_name.c_str(),
                                         this->m_pwl_pool_layout_name,
                                         this->m_log_pool_size,
                                         (S_IWUSR | S_IRUSR))) == NULL) {
            lderr(cct) << "failed to create pool (" << this->m_log_pool_name << ")" << pmemobj_errormsg() << dendl;
            m_cache_state->present = false;
        ...
        TX_BEGIN(m_log_pool)
        {
            ...
            D_RW(pool_root)->log_entries =
                TX_ZALLOC(struct WriteLogCacheEntry, sizeof(struct WriteLogCacheEntry) * num_small_writes);
        ...
        /* Open existing pool */
        if ((m_log_pool = pmemobj_open(this->m_log_pool_name.c_str(), this->m_pwl_pool_layout_name)) == NULL) {
            lderr(cct) << "failed to open pool (" << this->m_log_pool_name << "): " << pmemobj_errormsg() << dendl;
            on_finish->complete(-errno);
            return false;
        }
        pool_root = POBJ_ROOT(m_log_pool, struct WriteLogPoolRoot);
        if (D_RO(pool_root)->header.layout_version != RWL_LAYOUT_VERSION) {
            ...
            lderr(cct) << "Pool layout version is " << D_RO(pool_root)->header.layout_version << " expected "
                       << RWL_LAYOUT_VERSION << dendl;
            on_finish->complete(-EINVAL);
            return false;
        }
        if (D_RO(pool_root)->block_size != MIN_WRITE_ALLOC_SIZE) {
            lderr(cct) << "Pool block size is " << D_RO(pool_root)->block_size << " expected " << MIN_WRITE_ALLOC_SIZE
                       << dendl;
            on_finish->complete(-EINVAL);
            return false;
        }
...
template<typename I> void WriteLog<I>::load_existing_entries(DeferredContexts& later)
{
    TOID(struct WriteLogPoolRoot) pool_root;
    pool_root = POBJ_ROOT(m_log_pool, struct WriteLogPoolRoot);
    struct WriteLogCacheEntry* pmem_log_entries = D_RW(D_RW(pool_root)->log_entries);
    ...
    while (entry_index != m_first_free_entry) {
        ...
        this->update_entries(&log_entry, pmem_entry, missing_sync_points, sync_point_entries, entry_index);
...
template<typename I>
void WriteLog<I>::write_data_to_buffer(std::shared_ptr<pwl::WriteLogEntry> ws_entry, WriteLogCacheEntry* pmem_entry)
{
...
template<typename I> bool WriteLog<I>::retire_entries(const unsigned long int frees_per_tx)
{
    ...
    ldout(cct, 20) << "Look for entries to retire" << dendl;
    {
        ...
        while (!m_log_entries.empty() && retiring_entries.size() < frees_per_tx && this->can_retire_entry(entry)) {
            if (entry->log_entry_index != first_valid_entry) {
                lderr(cct) << "Retiring entry index (" << entry->log_entry_index
                           << ") and first valid log entry index (" << first_valid_entry << ") must be ==." << dendl;
            }
    ...
    if (retiring_entries.size()) {
        ...
        /* Advance first valid entry and release buffers */
        {
            ...
            TX_BEGIN(m_log_pool)
            {
                if (D_RO(pool_root)->flushed_sync_gen < flushed_sync_gen) {
                    ldout(m_image_ctx.cct, 20)
                        << "flushed_sync_gen in log updated from " << D_RO(pool_root)->flushed_sync_gen << " to "
            ...
            TX_ONABORT
            {
                lderr(cct) << "failed to commit free of" << retiring_entries.size() << " log entries ("
        ...
        m_perfcounter->hinc(
            l_librbd_pwl_retire_tx_t_hist, utime_t(tx_end - tx_start).to_nsec(), retiring_entries.size());
 ...
 */
template<typename I> void WriteLog<I>::flush_then_append_scheduled_ops(void)
{
    ...
    do {
        {
            ...
            if (m_ops_to_flush.size()) {
                ...
                ldout(m_image_ctx.cct, 20)
                    << "flushing " << ops.size() << ", " << m_ops_to_flush.size() << " remain" << dendl;
            }
            else {
                ops_remain = false;
            }
...
template<typename I> void WriteLog<I>::enlist_op_flusher()
{
 ...
 */
template<typename I> void WriteLog<I>::schedule_flush_and_append(GenericLogOperationsVector& ops)
{
...
template<typename I> void WriteLog<I>::process_work()
{
    ...
    do {
        ...
        if (this->m_alloc_failed_since_retire || this->m_invalidating || this->m_bytes_allocated > high_water_bytes ||
            (m_log_entries.size() > high_water_entries)) {
            int retired = 0;
            utime_t started = ceph_clock_now();
            ldout(m_image_ctx.cct, 10) << "alloc_fail=" << this->m_alloc_failed_since_retire
                                       << ", allocated > high_water=" << (this->m_bytes_allocated > high_water_bytes)
                                       << ", allocated_entries > high_water="
                                       << (m_log_entries.size() > high_water_entries) << dendl;
            while (this->m_alloc_failed_since_retire || this->m_invalidating ||
                   (this->m_bytes_allocated > high_water_bytes) || (m_log_entries.size() > high_water_entries) ||
 ...
 */
template<typename I> template<typename V> void WriteLog<I>::flush_pmem_buffer(V& ops)
{
    utime_t now = ceph_clock_now();
    for (auto& operation : ops) {
        if (operation->reserved_allocated()) {
 ...
 */
template<typename I> void WriteLog<I>::persist_last_flushed_sync_gen()
{
    TOID(struct WriteLogPoolRoot) pool_root;
    pool_root = POBJ_ROOT(m_log_pool, struct WriteLogPoolRoot);
    uint64_t flushed_sync_gen;
    ...
    if (D_RO(pool_root)->flushed_sync_gen < flushed_sync_gen) {
        ldout(m_image_ctx.cct, 15) << "flushed_sync_gen in log updated from " << D_RO(pool_root)->flushed_sync_gen
                                   << " to " << flushed_sync_gen << dendl;
        TX_BEGIN(m_log_pool)
        {
            D_RW(pool_root)->flushed_sync_gen = flushed_sync_gen;
        }
...
template<typename I> void WriteLog<I>::reserve_cache(C_BlockIORequestT* req, bool& alloc_succeeds, bool& no_space)
{
    std::vector<WriteBufferAllocation>& buffers = req->get_resources_buffers();
    for (auto& buffer : buffers) {
        utime_t before_reserve = ceph_clock_now();
        buffer.buffer_oid =
            pmemobj_reserve(m_log_pool, &buffer.buffer_alloc_action, buffer.allocation_size, 0 /* Object type */);
        ...
        if (TOID_IS_NULL(buffer.buffer_oid)) {
            ...
            ldout(m_image_ctx.cct, 5) << "can't allocate all data buffers: " << pmemobj_errormsg() << ". " << *req
                                      << dendl;
...
template<typename I>
void WriteLog<I>::copy_bl_to_buffer(WriteRequestResources* resources, std::unique_ptr<WriteLogOperationSet>& op_set)
{
