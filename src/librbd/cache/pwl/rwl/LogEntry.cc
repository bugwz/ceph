...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::cache::pwl::rwl::WriteLogEntry: " << this << " " << __func__ << ": "
...
void WriteLogEntry::writeback(librbd::cache::ImageWritebackInterface& image_writeback, Context* ctx)
{
    ...
    image_writeback.aio_write({{ram_entry.image_offset_bytes, ram_entry.write_bytes}}, std::move(entry_bl), 0, ctx);
}

void WriteLogEntry::init_cache_bp()
{
    ceph_assert(!this->cache_bp.have_raw());
    cache_bp = buffer::ptr(buffer::create_static(this->write_bytes(), (char*)this->cache_buffer));
}
...
void WriteLogEntry::init_cache_buffer(std::vector<WriteBufferAllocation>::iterator allocation)
{
...
void WriteSameLogEntry::writeback(librbd::cache::ImageWritebackInterface& image_writeback, Context* ctx)
{
    bufferlist entry_bl;
    buffer::list entry_bl_copy;
    copy_cache_bl(&entry_bl_copy);
    entry_bl_copy.begin(0).copy(write_bytes(), entry_bl);
    image_writeback.aio_writesame(ram_entry.image_offset_bytes, ram_entry.write_bytes, std::move(entry_bl), 0, ctx);
}
