...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::cache::pwl::LogOperation: " << this << " " << __func__ << ": "

namespace librbd {
namespace cache {
namespace pwl {

GenericLogOperation::GenericLogOperation(utime_t dispatch_time, PerfCounters* perfcounter)
...
{
    os << "dispatch_time=[" << dispatch_time << "], buf_persist_start_time=[" << buf_persist_start_time
       << "], buf_persist_comp_time=[" << buf_persist_comp_time << "], log_append_start_time=[" << log_append_start_time
       << "], log_append_comp_time=[" << log_append_comp_time << "]";
    return os;
}

std::ostream& operator<<(std::ostream& os, const GenericLogOperation& op)
{
    return op.format(os);
}

SyncPointLogOperation::SyncPointLogOperation(ceph::mutex& lock, std::shared_ptr<SyncPoint> sync_point,
...
std::ostream& operator<<(std::ostream& os, const SyncPointLogOperation& op)
{
    return op.format(os);
}
...
GenericWriteLogOperation::GenericWriteLogOperation(std::shared_ptr<SyncPoint> sync_point, utime_t dispatch_time,
                                                   PerfCounters* perfcounter, CephContext* cct)
    : GenericLogOperation(dispatch_time, perfcounter)
    , m_lock(ceph::make_mutex(pwl::unique_lock_name("librbd::cache::pwl::GenericWriteLogOperation::m_lock", this)))
    , m_cct(cct)
    , sync_point(sync_point)
{}
...
std::ostream& operator<<(std::ostream& os, const GenericWriteLogOperation& op)
{
    return op.format(os);
}
...
WriteLogOperation::WriteLogOperation(WriteLogOperationSet& set, uint64_t image_offset_bytes, uint64_t write_bytes,
                                     CephContext* cct, std::shared_ptr<WriteLogEntry> write_log_entry)
...
WriteLogOperation::WriteLogOperation(WriteLogOperationSet& set, uint64_t image_offset_bytes, uint64_t write_bytes,
                                     uint32_t data_len, CephContext* cct,
                                     std::shared_ptr<WriteLogEntry> writesame_log_entry)
    : WriteLogOperation(set, image_offset_bytes, write_bytes, cct, writesame_log_entry)
{
    is_writesame = true;
}
...
void WriteLogOperation::init(bool has_data, std::vector<WriteBufferAllocation>::iterator allocation,
                             uint64_t current_sync_gen, uint64_t last_op_sequence_num, bufferlist& write_req_bl,
                             uint64_t buffer_offset, bool persist_on_flush)
{
    ...
    log_entry->init_cache_bl(write_req_bl, buffer_offset, log_entry->write_bytes());
}
...
std::ostream& operator<<(std::ostream& os, const WriteLogOperation& op)
{
    return op.format(os);
}
...
void WriteLogOperation::complete(int result)
{
    ...
    m_perfcounter->tinc(l_librbd_pwl_log_op_dis_to_buf_t, buf_persist_start_time - dispatch_time);
    ...
    m_perfcounter->hinc(
        l_librbd_pwl_log_op_buf_to_bufc_t_hist, buf_persist_lat.to_nsec(), log_entry->ram_entry.write_bytes);
    m_perfcounter->tinc(l_librbd_pwl_log_op_buf_to_app_t, log_append_start_time - buf_persist_start_time);
}

WriteLogOperationSet::WriteLogOperationSet(utime_t dispatched, PerfCounters* perfcounter,
                                           std::shared_ptr<SyncPoint> sync_point, bool persist_on_flush,
                                           CephContext* cct, Context* on_finish)
    : m_cct(cct)
    ...
    , sync_point(sync_point)
{
    ...
    extent_ops_persist = new C_Gather(m_cct, new LambdaContext([this](int r) {
                                          ldout(this->m_cct, 20)
                                              << __func__ << " " << this << " m_extent_ops_persist completed" << dendl;
                                          if (on_ops_persist) {
    ...
    extent_ops_appending = new C_Gather(m_cct, new LambdaContext([this, appending_persist_sub](int r) {
                                            ldout(this->m_cct, 20) << __func__ << " " << this
                                                                   << " m_extent_ops_appending completed" << dendl;
...
std::ostream& operator<<(std::ostream& os, const WriteLogOperationSet& s)
{
    os << "cell=" << (void*)s.cell << ", extent_ops_appending=" << s.extent_ops_appending
       << ", extent_ops_persist=" << s.extent_ops_persist;
    return os;
}

DiscardLogOperation::DiscardLogOperation(std::shared_ptr<SyncPoint> sync_point, uint64_t image_offset_bytes,
...
std::ostream& operator<<(std::ostream& os, const DiscardLogOperation& op)
{
    return op.format(os);
}
