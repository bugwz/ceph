...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::cache::pwl::SyncPoint: " << this << " " << __func__ << ": "
...
std::ostream& operator<<(std::ostream& os, const SyncPoint& p)
{
    os << "log_entry=[" << *p.log_entry << "], earlier_sync_point=" << p.earlier_sync_point
       << ", later_sync_point=" << p.later_sync_point << ", m_final_op_sequence_num=" << p.m_final_op_sequence_num
       ...
       << ", m_append_scheduled=" << p.m_append_scheduled << ", appending=" << p.appending
...
void SyncPoint::prior_persisted_gather_set_finisher()
{
    ...
    m_prior_log_entries_persisted->set_finisher(new LambdaContext([this, sp, sync_point_persist_ready](int r) {
...
void SyncPoint::setup_earlier_sync_point(std::shared_ptr<SyncPoint> sync_point, uint64_t last_op_sequence_num)
{
