...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::cache::pwl::LogMap: " << this << " " << __func__ << ": "
template<typename T> std::ostream& operator<<(std::ostream& os, LogMapEntry<T>& e)
{
...
LogMapEntry<T>::LogMapEntry(std::shared_ptr<T> log_entry)
    : block_extent(log_entry->block_extent())
    , log_entry(log_entry)
{}
...
LogMap<T>::LogMap(CephContext* cct)
    : m_cct(cct)
    , m_lock(ceph::make_mutex(pwl::unique_lock_name("librbd::cache::pwl::LogMap::m_lock", this)))
{}
 ...
 * this.
 */
template<typename T> void LogMap<T>::add_log_entry(std::shared_ptr<T> log_entry)
{
    std::lock_guard locker(m_lock);
    add_log_entry_locked(log_entry);
}

template<typename T> void LogMap<T>::add_log_entries(std::list<std::shared_ptr<T>>& log_entries)
{
    std::lock_guard locker(m_lock);
 ...
 * entry.
 */
template<typename T> void LogMap<T>::remove_log_entry(std::shared_ptr<T> log_entry)
{
    std::lock_guard locker(m_lock);
    remove_log_entry_locked(log_entry);
}

template<typename T> void LogMap<T>::remove_log_entries(std::list<std::shared_ptr<T>>& log_entries)
{
    std::lock_guard locker(m_lock);
    ldout(m_cct, 20) << dendl;
    for (auto& log_entry : log_entries) {
 ...
 */
template<typename T> std::list<std::shared_ptr<T>> LogMap<T>::find_log_entries(BlockExtent block_extent)
{
    std::lock_guard locker(m_lock);
    ldout(m_cct, 20) << dendl;
 ...
 */
template<typename T> LogMapEntries<T> LogMap<T>::find_map_entries(BlockExtent block_extent)
{
    std::lock_guard locker(m_lock);
    ldout(m_cct, 20) << dendl;
...
template<typename T> void LogMap<T>::add_log_entry_locked(std::shared_ptr<T> log_entry)
{
    ...
    for (auto& entry : overlap_entries) {
        ...
        if (map_entry.block_extent.block_start <= entry.block_extent.block_start) {
            if (map_entry.block_extent.block_end >= entry.block_extent.block_end) {
                ...
                BlockExtent adjusted_extent(map_entry.block_extent.block_end, entry.block_extent.block_end);
            ...
            if (map_entry.block_extent.block_end >= entry.block_extent.block_end) {
                ...
                BlockExtent adjusted_extent(entry.block_extent.block_start, map_entry.block_extent.block_start);
                adjust_map_entry_locked(entry, adjusted_extent);
            }
            else {
...
template<typename T> void LogMap<T>::remove_log_entry_locked(std::shared_ptr<T> log_entry)
{
...
template<typename T> void LogMap<T>::add_map_entry_locked(LogMapEntry<T>& map_entry)
{
...
template<typename T> void LogMap<T>::remove_map_entry_locked(LogMapEntry<T>& map_entry)
{
...
template<typename T> void LogMap<T>::adjust_map_entry_locked(LogMapEntry<T>& map_entry, BlockExtent& new_extent)
{
    auto it = m_block_to_log_entry_map.find(map_entry);
    ceph_assert(it != m_block_to_log_entry_map.end());
...
template<typename T> void LogMap<T>::split_map_entry_locked(LogMapEntry<T>& map_entry, BlockExtent& removed_extent)
{
    auto it = m_block_to_log_entry_map.find(map_entry);
    ceph_assert(it != m_block_to_log_entry_map.end());
    ...
    BlockExtent left_extent(split.block_extent.block_start, removed_extent.block_start);
    m_block_to_log_entry_map.insert(LogMapEntry<T>(left_extent, split.log_entry));

    BlockExtent right_extent(removed_extent.block_end, split.block_extent.block_end);
    m_block_to_log_entry_map.insert(LogMapEntry<T>(right_extent, split.log_entry));

    split.log_entry->inc_map_ref();
}

template<typename T> std::list<std::shared_ptr<T>> LogMap<T>::find_log_entries_locked(const BlockExtent& block_extent)
{
 ...
 */
template<typename T> LogMapEntries<T> LogMap<T>::find_map_entries_locked(const BlockExtent& block_extent)
{
 ...
 */
template<typename T>
bool LogMap<T>::LogMapEntryCompare::operator()(const LogMapEntry<T>& lhs, const LogMapEntry<T>& rhs) const
{
