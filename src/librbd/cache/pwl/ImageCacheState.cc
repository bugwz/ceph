...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::cache::pwl::ImageCacheState: " << __func__ << ": "

namespace librbd {
namespace cache {
namespace pwl {

template<typename I> void ImageCacheState<I>::init_from_config()
{
...
template<typename I> bool ImageCacheState<I>::init_from_metadata(json_spirit::mValue& json_root)
{
    ldout(m_image_ctx->cct, 20) << dendl;
    ...
    try {
        ...
        lderr(m_image_ctx->cct) << "failed to parse cache state: " << e.what() << dendl;
        return false;
    }
...
template<typename I>
void ImageCacheState<I>::write_image_cache_state(std::unique_lock<ceph::mutex>& locker, Context* on_finish)
{
    ...
    m_plugin_api.execute_image_metadata_set(m_image_ctx, PERSISTENT_CACHE_STATE, image_state_json, on_finish);
}

template<typename I> void ImageCacheState<I>::clear_image_cache_state(Context* on_finish)
{
    std::shared_lock owner_lock{m_image_ctx->owner_lock};
    ldout(m_image_ctx->cct, 20) << __func__ << " Remove state: " << dendl;
    m_plugin_api.execute_image_metadata_remove(m_image_ctx, PERSISTENT_CACHE_STATE, on_finish);
}

template<typename I>
ImageCacheState<I>* ImageCacheState<I>::create_image_cache_state(I* image_ctx, plugin::Api<I>& plugin_api, int& r)
{
    ...
    if (dirty_cache) {
        cls_client::metadata_get(&image_ctx->md_ctx, image_ctx->header_oid, PERSISTENT_CACHE_STATE, &cache_state_str);
    }
    ...
    if (!dirty_cache && !cache_desired) {
        ...
        lderr(image_ctx->cct) << "There's a dirty cache, but RWL cache is disabled." << dendl;
        r = -EINVAL;
    }
...
template<typename I>
ImageCacheState<I>* ImageCacheState<I>::get_image_cache_state(I* image_ctx, plugin::Api<I>& plugin_api)
{
    ImageCacheState<I>* cache_state = nullptr;
...
template<typename I> bool ImageCacheState<I>::is_valid()
{
    if (this->present && (host.compare(ceph_get_short_hostname()) != 0)) {
        auto cleanstring = "dirty";
        if (this->clean) {
            cleanstring = "clean";
        }
        lderr(m_image_ctx->cct) << "An image cache (RWL) remains on another host " << host << " which is "
                                << cleanstring << ". Flush/close the image there to remove the "
                                << "image cache" << dendl;
        return false;
    }
    return true;
