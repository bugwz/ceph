    ...
    GuardedRequestFunctionContext* guarded_ctx =
        new GuardedRequestFunctionContext([this, cw_req](GuardedRequestFunctionContext& guard_ctx) {
            ...
            auto read_complete_ctx = new LambdaContext([this, cw_req](int r) {
                ...
                if (cw_req->cmp_bl.contents_equal(cw_req->read_bl) || cw_req->cmp_bl.contents_equal(aligned_read_bl)) {
...
void AbstractWriteLog<I>::append_scheduled(GenericLogOperations& ops, bool& ops_remain, bool& appending, bool isRWL)
{
    const unsigned long int OPS_APPENDED = isRWL ? MAX_ALLOC_PER_TRANSACTION : MAX_WRITES_PER_SYNC_POINT;
    {
        ...
        if (m_ops_to_append.size()) {
            ...
            ldout(m_image_ctx.cct, 20) << "appending " << ops.size() << ", " << m_ops_to_append.size() << " remain"
                                       << dendl;
        }
        else if (isRWL) {
            ops_remain = false;
    ...
    bool no_space = false;
    {
        ...
        if (m_bytes_allocated + bytes_allocated > m_bytes_allocated_cap) {
            if (!req->has_io_waited_for_buffers()) {
                ...
                ldout(m_image_ctx.cct, 5)
                    << "Waiting for allocation cap (cap=" << m_bytes_allocated_cap
