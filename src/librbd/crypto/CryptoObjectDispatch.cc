...
template<typename I> struct C_AlignedObjectReadRequest : public Context
{
    ...
    C_AlignedObjectReadRequest(I* image_ctx, ceph::ref_t<CryptoInterface> crypto, uint64_t object_no,
                               io::ReadExtents* extents, IOContext io_context, int op_flags, int read_flags,
                               const ZTracer::Trace& parent_trace, uint64_t* version, int* object_dispatch_flags,
        ...
        if (r == 0) {
            for (auto& extent : *extents) {
                auto crypto_ret = crypto->decrypt_aligned_extent(
                    extent, io::util::get_file_offset(image_ctx, object_no, extent.offset));
                if (crypto_ret != 0) {
...
template<typename I> struct C_UnalignedObjectReadRequest : public Context
{
    ...
    C_UnalignedObjectReadRequest(I* image_ctx, ceph::ref_t<CryptoInterface> crypto, uint64_t object_no,
                                 io::ReadExtents* extents, IOContext io_context, int op_flags, int read_flags,
                                 const ZTracer::Trace& parent_trace, uint64_t* version, int* object_dispatch_flags,
                                 Context* on_dispatched)
        : cct(image_ctx->cct)
...
template<typename I> struct C_UnalignedObjectWriteRequest : public Context
{
    ...
    C_UnalignedObjectWriteRequest(I* image_ctx, ceph::ref_t<CryptoInterface> crypto, uint64_t object_no,
                                  uint64_t object_off, ceph::bufferlist&& data, ceph::bufferlist&& cmp_data,
                                  uint64_t* mismatch_offset, IOContext io_context, int op_flags, int write_flags,
...
template<typename I>
CryptoObjectDispatch<I>::CryptoObjectDispatch(I* image_ctx, ceph::ref_t<CryptoInterface> crypto)
                                    ...
                                    io::DispatchResult* dispatch_result, Context** on_finish, Context* on_dispatched)
{
    ...
    if (m_crypto->is_aligned(object_off, data.length())) {
        auto r = m_crypto->encrypt(&data, io::util::get_file_offset(m_image_ctx, object_no, object_off));
                                            ...
                                            io::SnapshotSparseBufferlist* snapshot_sparse_bufferlist)
{
    ...
    for (auto& [key, extent_map] : *snapshot_sparse_bufferlist) {
        ...
        for (auto& extent : extent_map) {
            ...
            io::util::extent_to_file(m_image_ctx, object_no, aligned_off, aligned_len, image_extents);
