...
template<typename T>
CryptoContextPool<T>::CryptoContextPool(DataCryptor<T>* data_cryptor, uint32_t pool_size)
    : m_data_cryptor(data_cryptor)
    , m_encrypt_contexts(pool_size)
    , m_decrypt_contexts(pool_size)
{}

template<typename T> CryptoContextPool<T>::~CryptoContextPool()
{
    T* ctx;
...
template<typename T> T* CryptoContextPool<T>::get_context(CipherMode mode)
{
    T* ctx;
...
template<typename T> void CryptoContextPool<T>::return_context(T* ctx, CipherMode mode)
{
