...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::crypto::luks::Header: " << this << " " << __func__ << ": "
...
{
    switch (level) {
    case CRYPT_LOG_NORMAL: ldout(m_cct, 5) << "[libcryptsetup] " << msg << dendl; break;
    case CRYPT_LOG_ERROR: lderr(m_cct) << "[libcryptsetup] " << msg << dendl; break;
    case CRYPT_LOG_VERBOSE: ldout(m_cct, 10) << "[libcryptsetup] " << msg << dendl; break;
    case CRYPT_LOG_DEBUG: ldout(m_cct, 20) << "[libcryptsetup] " << msg << dendl; break;
    }
}
...
int Header::init()
{
    ...
    if (m_fd == -1) {
        lderr(m_cct) << "error creating anonymous file: " << cpp_strerror(-errno) << dendl;
        return -errno;
    }
    std::string path = "/proc/" + std::to_string(getpid()) + "/fd/" + std::to_string(m_fd);
...
ssize_t Header::read(ceph::bufferlist* bl)
{
    ...
    if (r < 0) {
        r = -errno;
        lderr(m_cct) << "failed to stat anonymous file: " << cpp_strerror(r) << dendl;
        return r;
    }
...
int Header::format(const char* type, const char* alg, const char* key, size_t key_size, const char* cipher_mode,
                   uint32_t sector_size, uint32_t data_alignment, bool insecure_fast_mode)
{
    ...
    if (ftruncate(m_fd, 4096) != 0) {
        lderr(m_cct) << "failed to truncate anonymous file: " << cpp_strerror(-errno) << dendl;
        return -errno;
    }
    ...
    if (insecure_fast_mode) {
        ...
        if (r != 0) {
            lderr(m_cct) << "crypt_set_pbkdf_type failed: " << cpp_strerror(r) << dendl;
            return r;
        }
    }

    auto r = crypt_format(m_cd, type, alg, cipher_mode, NULL, key, key_size, params);
    if (r != 0) {
...
int Header::add_keyslot(const char* passphrase, size_t passphrase_size)
{
    ...
    auto r = crypt_keyslot_add_by_volume_key(m_cd, CRYPT_ANY_SLOT, NULL, 0, passphrase, passphrase_size);
    if (r < 0) {
        lderr(m_cct) << "crypt_keyslot_add_by_volume_key failed: " << cpp_strerror(r) << dendl;
        return r;
    }
...
int Header::read_volume_key(const char* passphrase, size_t passphrase_size, char* volume_key, size_t* volume_key_size)
{
    ceph_assert(m_cd != nullptr);

    auto r = crypt_volume_key_get(m_cd, CRYPT_ANY_SLOT, volume_key, volume_key_size, passphrase, passphrase_size);
    if (r < 0) {
        ldout(m_cct, 20) << "crypt_volume_key_get failed: " << cpp_strerror(r) << dendl;
        return r;
    }
