...
int Magic::read(ceph::bufferlist& bl, uint32_t bl_off, uint32_t read_size, char* result)
{
...
int Magic::cmp(ceph::bufferlist& bl, uint32_t bl_off, const std::string& cmp_str)
{
...
int Magic::replace_magic(CephContext* cct, ceph::bufferlist& bl)
{
    ...
    case 2:
    {
        ...
        // read header size
        uint64_t hdr_size;
        r = read(bl, MAGIC_LENGTH + sizeof(version), sizeof(hdr_size), (char*)&hdr_size);
        if (r < 0) {
        ...
        auto is_secondary_header_magic_valid = !memcmp(secondary_header_magic, old_magic->c_str(), MAGIC_LENGTH);
