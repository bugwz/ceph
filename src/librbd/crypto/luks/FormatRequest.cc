...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::crypto::luks::FormatRequest: " << this << " " << __func__ << ": "
...
template<typename I>
FormatRequest<I>::FormatRequest(I* image_ctx, encryption_format_t format, encryption_algorithm_t alg,
...
template<typename I> void FormatRequest<I>::send()
{
    ...
    case RBD_ENCRYPTION_ALGORITHM_AES256:
        ...
        break;
    default:
        lderr(m_image_ctx->cct) << "unsupported cipher algorithm: " << m_alg << dendl;
        finish(-EINVAL);
        return;
    }
    ...
    if (RAND_bytes((unsigned char*)key, key_size) != 1) {
        lderr(m_image_ctx->cct) << "cannot generate random encryption key" << dendl;
        finish(-EAGAIN);
        return;
    }
    ...
    if (m_header.get_data_offset() > image_size) {
        lderr(m_image_ctx->cct) << "image is too small, format requires " << m_header.get_data_offset() << " bytes"
                                << dendl;
        finish(-ENOSPC);
        return;
    }
    ...
    r = util::build_crypto(
        m_image_ctx->cct, key, key_size, m_header.get_sector_size(), m_header.get_data_offset(), m_result_crypto);
    ...
    if (m_image_ctx->parent != nullptr) {
        ...
        if (r < 0) {
            lderr(m_image_ctx->cct) << "error replacing LUKS magic: " << cpp_strerror(r) << dendl;
            finish(r);
            return;
        }
    }
    ...
    auto ctx = create_context_callback<FormatRequest<I>, &FormatRequest<I>::handle_write_header>(this);
    auto aio_comp =
...
template<typename I> void FormatRequest<I>::handle_write_header(int r)
{
