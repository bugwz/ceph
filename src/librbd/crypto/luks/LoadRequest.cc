...
template<typename I>
LoadRequest<I>::LoadRequest(I* image_ctx, encryption_format_t format, std::string&& passphrase,
...
template<typename I> void LoadRequest<I>::read_volume_key()
{
    ...
    auto r = m_header.read_volume_key(
        m_passphrase.c_str(), m_passphrase.size(), reinterpret_cast<char*>(volume_key), &volume_key_size);
    if (r != 0) {
