...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::crypto::luks::LoadRequest: " << this << " " << __func__ << ": "
...
template<typename I>
LoadRequest<I>::LoadRequest(I* image_ctx, encryption_format_t format, std::string_view passphrase,
...
template<typename I> void LoadRequest<I>::set_initial_read_size(uint64_t read_size)
{
    m_initial_read_size = read_size;
}

template<typename I> void LoadRequest<I>::send()
{
    auto ctx = create_context_callback<LoadRequest<I>, &LoadRequest<I>::handle_read_header>(this);
    read(m_initial_read_size, ctx);
}

template<typename I> void LoadRequest<I>::read(uint64_t end_offset, Context* on_finish)
{
...
template<typename I> bool LoadRequest<I>::handle_read(int r)
{
    ...
    if (m_offset == 0) {
        ...
        if (m_image_ctx->parent != nullptr && Magic::is_rbd_clone(m_bl) > 0) {
            ...
            if (r < 0) {
                ...
                lderr(m_image_ctx->cct) << "error replacing rbd clone magic: " << cpp_strerror(r) << dendl;
                finish(r);
                return false;
            }
        }
...
template<typename I> void LoadRequest<I>::handle_read_header(int r)
{
    ldout(m_image_ctx->cct, 20) << "r=" << r << dendl;
    ...
    const char* type;
    switch (m_format) {
    case RBD_ENCRYPTION_FORMAT_LUKS: type = CRYPT_LUKS; break;
    case RBD_ENCRYPTION_FORMAT_LUKS1: type = CRYPT_LUKS1; break;
    case RBD_ENCRYPTION_FORMAT_LUKS2: type = CRYPT_LUKS2; break;
    default:
    ...
    *m_detected_format_name = m_header.get_format_name();
    ...
    if (strcmp(cipher_mode, "xts-plain64") != 0) {
        lderr(m_image_ctx->cct) << "unsupported cipher mode: " << cipher_mode << dendl;
        finish(-ENOTSUP);
        return;
    }
    ...
    if (m_header.get_data_offset() > image_size) {
        lderr(m_image_ctx->cct) << "image is too small, data offset " << m_header.get_data_offset() << dendl;
        finish(-EINVAL);
        return;
    }
    ...
    if (m_header.get_data_offset() % stripe_period != 0) {
        lderr(m_image_ctx->cct) << "incompatible stripe pattern, data offset " << m_header.get_data_offset() << dendl;
        finish(-EINVAL);
        return;
    }
...
template<typename I> void LoadRequest<I>::handle_read_keyslots(int r)
{
    ldout(m_image_ctx->cct, 20) << "r=" << r << dendl;
...
template<typename I> void LoadRequest<I>::read_volume_key()
{
    char volume_key[64];
    size_t volume_key_size = sizeof(volume_key);

    auto r = m_header.read_volume_key(
        m_passphrase.data(), m_passphrase.size(), reinterpret_cast<char*>(volume_key), &volume_key_size);
    if (r != 0) {
        ...
        if (m_offset < keyslots_end_offset) {
            ...
            auto ctx = create_context_callback<LoadRequest<I>, &LoadRequest<I>::handle_read_keyslots>(this);
    ...
    r = util::build_crypto(m_image_ctx->cct,
                           reinterpret_cast<unsigned char*>(volume_key),
                           volume_key_size,
                           m_header.get_sector_size(),
                           m_header.get_data_offset(),
                           m_result_crypto);
    ceph_memzero_s(volume_key, 64, 64);
    finish(r);
}

template<typename I> void LoadRequest<I>::finish(int r)
{
    ldout(m_image_ctx->cct, 20) << "r=" << r << dendl;
