...
template<typename I> void set_crypto(I* image_ctx, decltype(I::encryption_format) encryption_format)
{
...
int build_crypto(CephContext* cct, const unsigned char* key, uint32_t key_length, uint64_t block_size,
                 uint64_t data_offset, std::unique_ptr<CryptoInterface>* result_crypto)
{
    ...
    result_crypto->reset(BlockCrypto<EVP_CIPHER_CTX>::create(cct, data_cryptor, block_size, data_offset));
    return 0;
}
...
template void librbd::crypto::util::set_crypto(librbd::ImageCtx* image_ctx,
