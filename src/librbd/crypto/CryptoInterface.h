...
public:
    ...
    inline std::pair<uint64_t, uint64_t> get_pre_and_post_align(uint64_t off, uint64_t len)
    {
        if (len == 0) {
            return std::make_pair(0, 0);
        }
        auto block_size = get_block_size();
        return std::make_pair(p2phase(off, block_size), p2nphase(off + len, block_size));
    }

    inline std::pair<uint64_t, uint64_t> align(uint64_t off, uint64_t len)
    {
        auto aligns = get_pre_and_post_align(off, len);
        return std::make_pair(off - aligns.first, len + aligns.first + aligns.second);
    }
    ...
    inline void align_extents(const io::ReadExtents& extents, io::ReadExtents* aligned_extents)
    {
        for (const auto& extent : extents) {
    ...
    inline int decrypt_aligned_extent(io::ReadExtent& extent, uint64_t image_offset)
    {
        ...
        extent.extent_map.emplace_back(extent.offset + extent.length + get_block_size(), 0);
        ...
        for (auto [off, len] : extent.extent_map) {
            ...
            if (aligned_off > curr_block_end_offset) {
                ...
                if (curr_block_length > 0) {
                    auto r = decrypt(&curr_block_bl, image_offset + curr_block_start_offset - extent.offset);
                    if (r != 0) {
                        return r;
                    }

                    curr_block_bl.splice(0, curr_block_length, &result_bl);
                    result_extent_map.emplace_back(curr_block_start_offset, curr_block_length);
                }
