...
template<typename T>
BlockCrypto<T>::BlockCrypto(CephContext* cct, DataCryptor<T>* data_cryptor, uint64_t block_size, uint64_t data_offset)
    : m_cct(cct)
...
template<typename T> BlockCrypto<T>::~BlockCrypto()
{
...
template<typename T> int BlockCrypto<T>::crypt(ceph::bufferlist* data, uint64_t image_offset, CipherMode mode)
{
    if (image_offset % m_block_size != 0) {
        lderr(m_cct) << "image offset: " << image_offset << " not aligned to block size: " << m_block_size << dendl;
        return -EINVAL;
    }
    if (data->length() % m_block_size != 0) {
        lderr(m_cct) << "data length: " << data->length() << " not aligned to block size: " << m_block_size << dendl;
        return -EINVAL;
    }
    ...
    auto sg = make_scope_guard([&] { m_data_cryptor->return_context(ctx, mode); });
    ...
    for (auto buf = src.buffers().begin(); buf != src.buffers().end(); ++buf) {
        ...
        while (remaining_buf_bytes > 0) {
            if (leftover_size == 0) {
                ...
                out_buf_ptr = reinterpret_cast<unsigned char*>(appender.get_pos_add(m_block_size));
                sector_number += m_block_size / 512;
            }

            if (leftover_size > 0 || remaining_buf_bytes < m_block_size) {
                auto copy_size = std::min((uint32_t)m_block_size - leftover_size, remaining_buf_bytes);
            ...
            if (leftover_size == 0) {
                crypto_output_length = m_data_cryptor->update_context(ctx, in_buf_ptr, out_buf_ptr, m_block_size);

                in_buf_ptr += m_block_size;
                remaining_buf_bytes -= m_block_size;
            }
            else if (leftover_size == m_block_size) {
                crypto_output_length = m_data_cryptor->update_context(ctx, leftover_block, out_buf_ptr, m_block_size);
                leftover_size = 0;
            }
...
template<typename T> int BlockCrypto<T>::decrypt(ceph::bufferlist* data, uint64_t image_offset)
{
