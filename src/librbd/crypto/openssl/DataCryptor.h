...
public:
    DataCryptor(CephContext* cct)
        : m_cct(cct){};
    ~DataCryptor();

    int init(const char* cipher_name, const unsigned char* key, uint16_t key_length);
    ...
    int init_context(EVP_CIPHER_CTX* ctx, const unsigned char* iv, uint32_t iv_length) const override;
    int update_context(EVP_CIPHER_CTX* ctx, const unsigned char* in, unsigned char* out, uint32_t len) const override;
