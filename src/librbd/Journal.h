...
template<typename ImageCtxT = ImageCtx> class Journal : public RefCountedObject
{
public:
    /**
     * @verbatim
    ...
    static int create(librados::IoCtx& io_ctx, const std::string& image_id, uint8_t order, uint8_t splay_width,
                      const std::string& object_pool);
    static int remove(librados::IoCtx& io_ctx, const std::string& image_id);
    static int reset(librados::IoCtx& io_ctx, const std::string& image_id);

    static void is_tag_owner(ImageCtxT* image_ctx, bool* is_tag_owner, Context* on_finish);
    static void is_tag_owner(librados::IoCtx& io_ctx, std::string& image_id, bool* is_tag_owner,
                             asio::ContextWQ* op_work_queue, Context* on_finish);
    static void get_tag_owner(librados::IoCtx& io_ctx, std::string& image_id, std::string* mirror_uuid,
                              asio::ContextWQ* op_work_queue, Context* on_finish);
    ...
    void allocate_tag(const std::string& mirror_uuid, const journal::TagPredecessor& predecessor, Context* on_finish);

    void flush_commit_position(Context* on_finish);

    void user_flushed();

    uint64_t append_write_event(uint64_t offset, size_t length, const bufferlist& bl, bool flush_entry);
    uint64_t append_compare_and_write_event(uint64_t offset, size_t length, const bufferlist& cmp_bl,
    ...
    void commit_io_event_extent(uint64_t tid, uint64_t offset, uint64_t length, int r);

    void append_op_event(uint64_t op_tid, journal::EventEntry&& event_entry, Context* on_safe);
    ...
    void start_external_replay(journal::Replay<ImageCtxT>** journal_replay, Context* on_start);
...
private:
    ...
    struct Event
    {
        ...
        Event() {}
        Event(const Futures& _futures, uint64_t offset, size_t length, int filter_ret_val)
            : futures(_futures)
    ...
    struct C_IOEventSafe : public Context
    {
        ...
        void finish(int r) override { journal->handle_io_event_safe(r, tid); }
    };
    ...
    struct C_OpEventSafe : public Context
    {
        ...
        C_OpEventSafe(Journal* journal, uint64_t tid, const Future& op_start_future, const Future& op_finish_future,
                      Context* on_safe)
            : journal(journal)
            , tid(tid)
        ...
        {
            journal->handle_op_event_safe(r, tid, op_start_future, op_finish_future, on_safe);
        }
    };
    ...
    struct C_ReplayProcessSafe : public Context
    {
        ...
        {}
        void finish(int r) override { journal->handle_replay_process_safe(replay_entry, r); }
    };

    struct ReplayHandler : public ::journal::ReplayHandler
    {
        Journal* journal;
        ReplayHandler(Journal* _journal)
            : journal(_journal)
        {}
    ...
    struct MetadataListener : public ::journal::JournalMetadataListener
    {
        ...
        MetadataListener(Journal<ImageCtxT>* journal)
            : journal(journal)
        {}
    ...
    uint64_t append_io_events(journal::EventType event_type, const Bufferlists& bufferlists, uint64_t offset,
    ...
    void handle_start_external_replay(int r, journal::Replay<ImageCtxT>** journal_replay, Context* on_finish);
    ...
    void handle_op_event_safe(int r, uint64_t tid, const Future& op_start_future, const Future& op_finish_future,
                              Context* on_safe);
    ...
    void handle_refresh_metadata(uint64_t refresh_sequence, uint64_t tag_tid, journal::TagData tag_data, int r);
};
