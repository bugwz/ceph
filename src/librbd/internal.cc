...
int validate_pool(IoCtx& io_ctx, CephContext* cct)
{
    ...
    if (r == -EINVAL) {
        lderr(cct) << "pool not configured for self-managed RBD snapshot support" << dendl;
        return r;
    }
    else if (r < 0) {
        lderr(cct) << "failed to allocate self-managed snapshot: " << cpp_strerror(r) << dendl;
        return r;
    }

    r = io_ctx.selfmanaged_snap_remove(snap_id);
    if (r < 0) {
        lderr(cct) << "failed to release self-managed snapshot " << snap_id << ": " << cpp_strerror(r) << dendl;
    }
    return 0;
}
...
int detect_format(IoCtx& io_ctx, const string& name, bool* old_format, uint64_t* size)
{
    CephContext* cct = (CephContext*)io_ctx.cct();
    if (old_format) *old_format = true;
    int r = io_ctx.stat(util::old_header_name(name), size, NULL);
    if (r == -ENOENT) {
        if (old_format) *old_format = false;
    ...
    ldout(cct, 20) << "detect format of " << name << " : "
                   << (old_format ? (*old_format ? "old" : "new") : "don't care") << dendl;
    return 0;
}
...
void init_rbd_header(struct rbd_obj_header_ondisk& ondisk, uint64_t size, int order, uint64_t bid)
{
    uint32_t hi = bid >> 32;
    ...
    snprintf(ondisk.block_name, sizeof(ondisk.block_name), "rb.%x.%x.%x", hi, lo, extra);
...
void image_info(ImageCtx* ictx, image_info_t& info, size_t infosize)
{
    ...
    strncpy(info.block_name_prefix, ictx->object_prefix.c_str(), RBD_MAX_BLOCK_NAME_SIZE);
...
void trim_image(ImageCtx* ictx, uint64_t newsize, ProgressContext& prog_ctx)
{
    ...
    ceph_assert(ictx->exclusive_lock == nullptr || ictx->exclusive_lock->is_lock_owner());

    C_SaferCond ctx;
    ictx->image_lock.lock_shared();
    operation::TrimRequest<>* req = operation::TrimRequest<>::create(*ictx, &ctx, ictx->size, newsize, prog_ctx);
    ictx->image_lock.unlock_shared();
    req->send();

    int r = ctx.wait();
    if (r < 0) {
        lderr(ictx->cct) << "warning: failed to remove some object(s): " << cpp_strerror(r) << dendl;
    }
}

int read_header_bl(IoCtx& io_ctx, const string& header_oid, bufferlist& header, uint64_t* ver)
{
    int r;
    uint64_t off = 0;
#define READ_SIZE 4096
    ...
    static_assert(sizeof(RBD_HEADER_TEXT) == sizeof(RBD_MIGRATE_HEADER_TEXT), "length of rbd headers must be the same");

    if (header.length() < sizeof(RBD_HEADER_TEXT) ||
        (memcmp(RBD_HEADER_TEXT, header.c_str(), sizeof(RBD_HEADER_TEXT)) != 0 &&
         memcmp(RBD_MIGRATE_HEADER_TEXT, header.c_str(), sizeof(RBD_MIGRATE_HEADER_TEXT)) != 0)) {
        CephContext* cct = (CephContext*)io_ctx.cct();
        lderr(cct) << "unrecognized header format" << dendl;
        return -ENXIO;
    }

    if (ver) *ver = io_ctx.get_last_version();

    return 0;
}

int read_header(IoCtx& io_ctx, const string& header_oid, struct rbd_obj_header_ondisk* header, uint64_t* ver)
{
    bufferlist header_bl;
    int r = read_header_bl(io_ctx, header_oid, header_bl, ver);
    if (r < 0) return r;
    if (header_bl.length() < (int)sizeof(*header)) return -EIO;
...
{
    switch (optname) {
    case RBD_IMAGE_OPTION_FORMAT: return "format";
    case RBD_IMAGE_OPTION_FEATURES: return "features";
    case RBD_IMAGE_OPTION_ORDER: return "order";
    case RBD_IMAGE_OPTION_STRIPE_UNIT: return "stripe_unit";
    case RBD_IMAGE_OPTION_STRIPE_COUNT: return "stripe_count";
    case RBD_IMAGE_OPTION_JOURNAL_ORDER: return "journal_order";
    case RBD_IMAGE_OPTION_JOURNAL_SPLAY_WIDTH: return "journal_splay_width";
    case RBD_IMAGE_OPTION_JOURNAL_POOL: return "journal_pool";
    case RBD_IMAGE_OPTION_FEATURES_SET: return "features_set";
    case RBD_IMAGE_OPTION_FEATURES_CLEAR: return "features_clear";
    case RBD_IMAGE_OPTION_DATA_POOL: return "data_pool";
    case RBD_IMAGE_OPTION_FLATTEN: return "flatten";
    case RBD_IMAGE_OPTION_CLONE_FORMAT: return "clone_format";
    case RBD_IMAGE_OPTION_MIRROR_IMAGE_MODE: return "mirror_image_mode";
    default: return "unknown (" + stringify(optname) + ")";
    }
}
...
void image_options_create_ref(rbd_image_options_t* opts, rbd_image_options_t orig)
{
...
void image_options_copy(rbd_image_options_t* opts, const ImageOptions& orig)
{
    image_options_ref* opts_ = new image_options_ref(new image_options_t());
...
int image_options_set(rbd_image_options_t opts, int optname, const std::string& optval)
{
    image_options_ref* opts_ = static_cast<image_options_ref*>(opts);

    std::map<int, image_option_type_t>::const_iterator i = IMAGE_OPTIONS_TYPE_MAPPING.find(optname);
...
int image_options_get(rbd_image_options_t opts, int optname, std::string* optval)
{
    image_options_ref* opts_ = static_cast<image_options_ref*>(opts);
...
int image_options_is_set(rbd_image_options_t opts, int optname, bool* is_set)
{
    if (IMAGE_OPTIONS_TYPE_MAPPING.find(optname) == IMAGE_OPTIONS_TYPE_MAPPING.end()) {
        return -EINVAL;
    }
...
int create_v1(IoCtx& io_ctx, const char* imgname, uint64_t size, int order)
{
    ...
    ldout(cct, 20) << __func__ << " " << &io_ctx << " name = " << imgname << " size = " << size << " order = " << order
                   << dendl;
    ...
    if (r < 0) {
        lderr(cct) << "Error writing image header: " << cpp_strerror(r) << dendl;
        int remove_r = tmap_rm(io_ctx, imgname);
        if (remove_r < 0) {
            lderr(cct) << "Could not remove image from directory after "
                       << "header creation failed: " << cpp_strerror(remove_r) << dendl;
        }
        return r;
    }
...
int create(IoCtx& io_ctx, const char* imgname, uint64_t size, bool old_format, uint64_t features, int* order,
           uint64_t stripe_unit, uint64_t stripe_count)
{
    if (!order) return -EINVAL;
...
int create(IoCtx& io_ctx, const std::string& image_name, const std::string& image_id, uint64_t size, ImageOptions& opts,
           const std::string& non_primary_global_image_id, const std::string& primary_mirror_uuid,
           bool skip_mirror_enable)
{
    ...
    if (opts.get(RBD_IMAGE_OPTION_CLONE_FORMAT, &option) == 0) {
        lderr(cct) << "create does not support 'clone_format' image option" << dendl;
        return -EINVAL;
    }
    ...
    uint64_t format;
    if (opts.get(RBD_IMAGE_OPTION_FORMAT, &format) != 0) format = cct->_conf.get_val<uint64_t>("rbd_default_format");
    ...
    if (r != -ENOENT) {
        if (r) {
            lderr(cct) << "Could not tell if " << image_name << " already exists" << dendl;
            return r;
        }
    ...
    if (old_format) {
        ...
        else if (opts.get(RBD_IMAGE_OPTION_MIRROR_IMAGE_MODE, &mirror_image_mode) == 0) {
            create_flags = image::CREATE_FLAG_FORCE_MIRROR_ENABLE;
        }

        C_SaferCond cond;
        image::CreateRequest<>* req =
            image::CreateRequest<>::create(config,
                                           io_ctx,
                                           image_name,
                                           id,
                                           size,
 ...
 */
int clone(IoCtx& p_ioctx, const char* p_name, const char* p_snap_name, IoCtx& c_ioctx, const char* c_name,
          uint64_t features, int* c_order, uint64_t stripe_unit, int stripe_count)
{
    ...
    int r = clone(p_ioctx, nullptr, p_name, p_snap_name, c_ioctx, nullptr, c_name, opts, "", "");
    opts.get(RBD_IMAGE_OPTION_ORDER, &order);
    *c_order = order;
    return r;
}

int clone(IoCtx& p_ioctx, const char* p_id, const char* p_name, const char* p_snap_name, IoCtx& c_ioctx,
          const char* c_id, const char* c_name, ImageOptions& c_opts, const std::string& non_primary_global_image_id,
          const std::string& primary_mirror_uuid)
{
    ...
    if (p_id == nullptr) {
        r = cls_client::dir_get_id(&p_ioctx, RBD_DIRECTORY, p_name, &parent_id);
        if (r < 0) {
            if (r != -ENOENT) {
                lderr(cct) << "failed to retrieve parent image id: " << cpp_strerror(r) << dendl;
            }
            return r;
        }
    }
    ...
    C_SaferCond cond;
    auto* req = image::CloneRequest<>::create(config,
                                              p_ioctx,
                                              parent_id,
                                              p_snap_name,
...
int set_image_notification(ImageCtx* ictx, int fd, int type)
{
    ...
    if (ictx->event_socket.is_valid()) return -EINVAL;
...
int lock_acquire(ImageCtx* ictx, rbd_lock_mode_t lock_mode)
{
    ...
    C_SaferCond lock_ctx;
    {
        ...
        if (ictx->get_exclusive_lock_policy()->may_auto_request_lock()) {
            ictx->set_exclusive_lock_policy(new exclusive_lock::StandardPolicy(ictx));
        }
    ...
    int r = lock_ctx.wait();
    if (r < 0) {
        lderr(cct) << "failed to request exclusive lock: " << cpp_strerror(r) << dendl;
        return r;
    }
...
int lock_release(ImageCtx* ictx)
{
    ...
    int r = lock_ctx.wait();
    if (r < 0) {
        lderr(cct) << "failed to release exclusive lock: " << cpp_strerror(r) << dendl;
        return r;
    }
    return 0;
}

int lock_get_owners(ImageCtx* ictx, rbd_lock_mode_t* lock_mode, std::list<std::string>* lock_owners)
{
    CephContext* cct = ictx->cct;
    ldout(cct, 20) << __func__ << ": ictx=" << ictx << dendl;
    ...
    if (r == -ENOENT) {
        ...
        lderr(cct) << "failed to determine current lock owner: " << cpp_strerror(r) << dendl;
        return r;
    }
...
int lock_break(ImageCtx* ictx, rbd_lock_mode_t lock_mode, const std::string& lock_owner)
{
    CephContext* cct = ictx->cct;
    ldout(cct, 20) << __func__ << ": ictx=" << ictx << ", "
                   << "lock_mode=" << lock_mode << ", "
...
int copy(ImageCtx* src, IoCtx& dest_md_ctx, const char* destname, ImageOptions& opts, ProgressContext& prog_ctx,
         size_t sparse_size)
{
    ...
    if (opts.get(RBD_IMAGE_OPTION_CLONE_FORMAT, &option) == 0) {
        lderr(cct) << "copy does not support 'clone_format' image option" << dendl;
        return -EINVAL;
    }

    ldout(cct, 20) << "copy " << src->name << (src->snap_name.length() ? "@" + src->snap_name : "") << " -> "
    ...
    ImageCtx* dest = new librbd::ImageCtx(destname, "", nullptr, dest_md_ctx, false);
    r = dest->state->open(0);
    if (r < 0) {
...
{
public:
    C_CopyRead(SimpleThrottle* throttle, ImageCtx* dest, uint64_t offset, bufferlist* bl, size_t sparse_size)
        : m_throttle(throttle)
        , m_dest(dest)
        , m_offset(offset)
        , m_bl(bl)
    ...
    {
        if (r < 0) {
            lderr(m_dest->cct) << "error reading from source image at offset " << m_offset << ": " << cpp_strerror(r)
                               << dendl;
            delete m_bl;
            m_throttle->end_op(r);
            return;
        ...
        auto* end_op_ctx = new LambdaContext([throttle](int r) { throttle->end_op(r); });
        ...
        while (offset < length) {
            if (util::calc_sparse_extent(m_ptr, m_sparse_size, length, &write_offset, &write_length, &offset)) {
                bufferlist* write_bl = new bufferlist();
                write_bl->push_back(buffer::ptr_node::create(m_ptr, write_offset, write_length));
                ...
                api::Io<>::aio_write(*m_dest,
                                     comp,
                                     m_offset + write_offset,
                                     write_length,
                                     std::move(*write_bl),
...
int copy(ImageCtx* src, ImageCtx* dest, ProgressContext& prog_ctx, size_t sparse_size)
{
    ...
    if (dest_size < src_size) {
        lderr(cct) << " src size " << src_size << " > dest size " << dest_size << dendl;
        return -EINVAL;
    }
    ...
    {
        auto aio_comp = io::AioCompletion::create_and_start(&flush_ctx, src, io::AIO_TYPE_FLUSH);
        auto req = io::ImageDispatchSpec::create_flush(
            *src, io::IMAGE_DISPATCH_LAYER_INTERNAL_START, aio_comp, io::FLUSH_SOURCE_INTERNAL, {});
        req->send();
    }
    ...
    C_SaferCond ctx;
    auto req = deep_copy::MetadataCopyRequest<>::create(src, dest, &ctx);
    req->send();
    ...
    for (uint64_t offset = 0; offset < src_size; offset += period) {
...

        {
            ...
            if (src->object_map != nullptr) {
                ...
                for (uint64_t i = 0; i < src->stripe_count; i++) {
                    if (object_id < src->object_map->size() && src->object_map->object_may_exist(object_id)) {
                        skip = false;
                    }
                    ++object_id;
                }
        ...
        auto comp = io::AioCompletion::create_and_start<Context>(ctx, src, io::AIO_TYPE_READ);
        auto req = io::ImageDispatchSpec::create_read(*src,
                                                      io::IMAGE_DISPATCH_LAYER_NONE,
                                                      comp,
                                                      {{offset, len}},
                                                      io::ReadResult{bl},
                                                      src->get_data_io_context(),
                                                      fadvise_flags,
                                                      0,
                                                      trace);
    ...
    if (r >= 0) prog_ctx.update_progress(src_size, src_size);
    return r;
}

int list_lockers(ImageCtx* ictx, std::list<locker_t>* lockers, bool* exclusive, string* tag)
{
    ...
    std::shared_lock locker{ictx->image_lock};
    if (exclusive) *exclusive = ictx->exclusive_locked;
    if (tag) *tag = ictx->lock_tag;
    if (lockers) {
        lockers->clear();
        map<rados::cls::lock::locker_id_t, rados::cls::lock::locker_info_t>::const_iterator it;
...
int lock(ImageCtx* ictx, bool exclusive, const string& cookie, const string& tag)
{
    ldout(ictx->cct, 20) << "lock image " << ictx << " exclusive=" << exclusive << " cookie='" << cookie << "' tag='"
                         << tag << "'" << dendl;
    ...
    {
        std::shared_lock locker{ictx->image_lock};
        r = rados::cls::lock::lock(&ictx->md_ctx,
                                   ictx->header_oid,
                                   RBD_LOCK_NAME,
...
int unlock(ImageCtx* ictx, const string& cookie)
{
    ...
    {
        std::shared_lock locker{ictx->image_lock};
        r = rados::cls::lock::unlock(&ictx->md_ctx, ictx->header_oid, RBD_LOCK_NAME, cookie);
        if (r < 0) {
            return r;
        }
    }
...
int break_lock(ImageCtx* ictx, const string& client, const string& cookie)
{
    ldout(ictx->cct, 20) << "break_lock image " << ictx << " client='" << client << "' cookie='" << cookie << "'"
                         << dendl;
    ...
    if (!lock_client.parse(client)) {
        lderr(ictx->cct) << "Unable to parse client '" << client << "'" << dendl;
        return -EINVAL;
    }

    if (ictx->config.get_val<bool>("rbd_blocklist_on_break_lock")) {
        typedef std::map<rados::cls::lock::locker_id_t, rados::cls::lock::locker_info_t> Lockers;
        Lockers lockers;
        ClsLockType lock_type;
        std::string lock_tag;
        r = rados::cls::lock::get_lock_info(
            &ictx->md_ctx, ictx->header_oid, RBD_LOCK_NAME, &lockers, &lock_type, &lock_tag);
        if (r < 0) {
            lderr(ictx->cct) << "unable to retrieve lock info: " << cpp_strerror(r) << dendl;
            return r;
        }

        std::string client_address;
        for (Lockers::iterator it = lockers.begin(); it != lockers.end(); ++it) {
        ...
        r = rados.blocklist_add(client_address, ictx->config.get_val<uint64_t>("rbd_blocklist_expire_seconds"));
        if (r < 0) {
            lderr(ictx->cct) << "unable to blocklist client: " << cpp_strerror(r) << dendl;
            return r;
        }
    }

    r = rados::cls::lock::break_lock(&ictx->md_ctx, ictx->header_oid, RBD_LOCK_NAME, cookie, lock_client);
    if (r < 0) return r;
    ictx->notify_update();
    return 0;
}
...
int64_t read_iterate(ImageCtx* ictx, uint64_t off, uint64_t len, int (*cb)(uint64_t, size_t, const char*, void*),
                     void* arg)
{
    ...
    while (left > 0) {
        ...
        C_SaferCond ctx;
        auto c = io::AioCompletion::create_and_start(&ctx, ictx, io::AIO_TYPE_READ);
        auto req = io::ImageDispatchSpec::create_read(*ictx,
                                                      io::IMAGE_DISPATCH_LAYER_NONE,
                                                      c,
                                                      {{off, read_len}},
                                                      io::ReadResult{&bl},
                                                      ictx->get_data_io_context(),
                                                      0,
                                                      0,
                                                      trace);
        req->send();
...
int clip_io(ImageCtx* ictx, uint64_t off, uint64_t* len)
{
    ...
    if (ictx->snap_id != CEPH_NOSNAP && ictx->get_snap_info(ictx->snap_id) == nullptr) {
        return -ENOENT;
    }
    ...
    if (off >= image_size) return -EINVAL;

    // clip requests that extend past end to just end
    if ((off + *len) > image_size) *len = (size_t)(image_size - off);
...
int invalidate_cache(ImageCtx* ictx)
{
    ...
    if ((!ictx->exclusive_lock || !ictx->exclusive_lock->is_lock_owner()) &&
...
int metadata_list(ImageCtx* ictx, const string& start, uint64_t max, map<string, bufferlist>* pairs)
{
    ...
    C_SaferCond ctx;
    auto req = image::GetMetadataRequest<>::create(ictx->md_ctx, ictx->header_oid, false, "", start, max, pairs, &ctx);
    req->send();
