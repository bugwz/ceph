...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::managed_lock::GetLockerRequest: " << this << " " << __func__ << ": "
...
template<typename I>
GetLockerRequest<I>::GetLockerRequest(librados::IoCtx& ioctx, const std::string& oid, bool exclusive, Locker* locker,
                                      Context* on_finish)
    : m_ioctx(ioctx)
...
template<typename I> void GetLockerRequest<I>::send()
{
    send_get_lockers();
}

template<typename I> void GetLockerRequest<I>::send_get_lockers()
{
    ...
    librados::AioCompletion* rados_completion = create_rados_callback<klass, &klass::handle_get_lockers>(this);
    m_out_bl.clear();
...
template<typename I> void GetLockerRequest<I>::handle_get_lockers(int r)
{
    ...
    if (r == 0) {
        ...
        r = rados::cls::lock::get_lock_info_finish(&it, &lockers, &lock_type, &lock_tag);
    }
    ...
    std::map<rados::cls::lock::locker_id_t, rados::cls::lock::locker_info_t>::iterator iter = lockers.begin();
...
template<typename I> void GetLockerRequest<I>::finish(int r)
{
    ldout(m_cct, 10) << "r=" << r << dendl;
