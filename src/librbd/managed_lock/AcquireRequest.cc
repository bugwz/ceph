...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::managed_lock::AcquireRequest: " << this << " " << __func__ << ": "
...
template<typename I>
AcquireRequest<I>* AcquireRequest<I>::create(librados::IoCtx& ioctx, Watcher* watcher, AsioEngine& asio_engine,
                                             const string& oid, const string& cookie, bool exclusive,
                                             bool blocklist_on_break_lock, uint32_t blocklist_expire_seconds,
                                             Context* on_finish)
{
    return new AcquireRequest(ioctx,
                              watcher,
                              asio_engine,
                              oid,
                              cookie,
...
template<typename I>
AcquireRequest<I>::AcquireRequest(librados::IoCtx& ioctx, Watcher* watcher, AsioEngine& asio_engine, const string& oid,
                                  const string& cookie, bool exclusive, bool blocklist_on_break_lock,
    ...
    , m_on_finish(new C_AsyncCallback<asio::ContextWQ>(asio_engine.get_work_queue(), on_finish))
{}
...
template<typename I> void AcquireRequest<I>::send_get_locker()
{
    ...
    Context* ctx = create_context_callback<AcquireRequest<I>, &AcquireRequest<I>::handle_get_locker>(this);
    auto req = GetLockerRequest<I>::create(m_ioctx, m_oid, m_exclusive, &m_locker, ctx);
    req->send();
}

template<typename I> void AcquireRequest<I>::handle_get_locker(int r)
{
...
template<typename I> void AcquireRequest<I>::send_lock()
{
    ...
    librados::AioCompletion* rados_completion = create_rados_callback<klass, &klass::handle_lock>(this);
...
template<typename I> void AcquireRequest<I>::send_break_lock()
{
    ...
    Context* ctx = create_context_callback<AcquireRequest<I>, &AcquireRequest<I>::handle_break_lock>(this);
    auto req = BreakRequest<I>::create(m_ioctx,
                                       m_asio_engine,
                                       m_oid,
                                       m_locker,
                                       m_exclusive,
...
template<typename I> void AcquireRequest<I>::handle_break_lock(int r)
{
    ldout(m_cct, 10) << "r=" << r << dendl;
