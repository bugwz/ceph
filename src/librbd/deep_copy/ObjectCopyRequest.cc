...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::deep_copy::ObjectCopyRequest: " << this << " " << __func__ << ": "
...
template<typename I>
ObjectCopyRequest<I>::ObjectCopyRequest(I* src_image_ctx, I* dst_image_ctx, librados::snap_t src_snap_id_start,
                                        librados::snap_t dst_snap_id_start, const SnapMap& snap_map,
                                        uint64_t dst_object_number, uint32_t flags, Handler* handler,
                                        Context* on_finish)
...
template<typename I> void ObjectCopyRequest<I>::send()
{
    send_list_snaps();
}

template<typename I> void ObjectCopyRequest<I>::send_list_snaps()
{
    // image extents are consistent across src and dst so compute once
    io::util::extent_to_file(
        m_dst_image_ctx, m_dst_object_number, 0, m_dst_image_ctx->layout.object_size, m_image_extents);
    ldout(m_cct, 20) << "image_extents=" << m_image_extents << dendl;

    auto ctx = create_async_context_callback(
        *m_src_image_ctx, create_context_callback<ObjectCopyRequest, &ObjectCopyRequest<I>::handle_list_snaps>(this));
    ...
    auto aio_comp = io::AioCompletion::create_and_start(ctx, get_image_ctx(m_src_image_ctx), io::AIO_TYPE_GENERIC);
    auto req = io::ImageDispatchSpec::create_list_snaps(*m_src_image_ctx,
...
template<typename I> void ObjectCopyRequest<I>::handle_list_snaps(int r)
{
...
template<typename I> void ObjectCopyRequest<I>::send_read()
{
    ...
    io::Extents image_extents{read_op.image_interval.begin(), read_op.image_interval.end()};
    io::ReadResult read_result{&read_op.image_extent_map, &read_op.out_bl};

    ldout(m_cct, 20) << "read: src_snap_seq=" << index.second << ", "
                     << "image_extents=" << image_extents << dendl;

    int op_flags = (LIBRADOS_OP_FLAG_FADVISE_SEQUENTIAL | LIBRADOS_OP_FLAG_FADVISE_NOCACHE);
    ...
    auto ctx = create_context_callback<ObjectCopyRequest<I>, &ObjectCopyRequest<I>::handle_read>(this);
    auto aio_comp = io::AioCompletion::create_and_start(ctx, get_image_ctx(m_src_image_ctx), io::AIO_TYPE_READ);

    auto req = io::ImageDispatchSpec::create_read(*m_src_image_ctx,
...
template<typename I> void ObjectCopyRequest<I>::handle_read(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;

    if (r < 0) {
        lderr(m_cct) << "failed to read from source object: " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
...
template<typename I> void ObjectCopyRequest<I>::send_update_object_map()
{
    if (!m_dst_image_ctx->test_features(RBD_FEATURE_OBJECT_MAP) || m_dst_object_state.empty()) {
        process_copyup();
        return;
    }
    ...
    ldout(m_cct, 20) << "dst_snap_id=" << dst_snap_id << ", object_state=" << static_cast<uint32_t>(object_state)
                     << dendl;
    ...
    bool sent = dst_image_ctx->object_map->template aio_update<Context, &Context::complete>(
        dst_snap_id, m_dst_object_number, object_state, {}, {}, false, ctx);
...
template<typename I> void ObjectCopyRequest<I>::handle_update_object_map(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;
...
template<typename I> void ObjectCopyRequest<I>::process_copyup()
{
    ...
    auto r = m_dst_image_ctx->io_object_dispatcher->prepare_copyup(m_dst_object_number, &m_snapshot_sparse_bufferlist);
    if (r < 0) {
        lderr(m_cct) << "failed to prepare copyup data: " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
...
template<typename I> void ObjectCopyRequest<I>::send_write_object()
{
    ...
    bool hide_parent = (m_src_snap_id_start == 0 && m_src_image_ctx->parent != nullptr);
    ...
    if (src_snap_seq != 0) {
        ...
        if (!dst_may_exist_it->second && !sparse_bufferlist.empty()) {
            ...
            ceph_assert(sparse_bufferlist.begin().get_val().state == io::SPARSE_EXTENT_STATE_ZEROED &&
                        sparse_bufferlist.begin().get_off() == 0 &&
                        sparse_bufferlist.begin().get_len() == m_dst_image_ctx->layout.object_size);
        }
    ...
    if (migration) {
        ...
        cls_client::assert_snapc_seq(&op, dst_snap_seq, cls::rbd::ASSERT_SNAPC_SEQ_GT_SNAPSET_SEQ);
    }
    ...
    for (auto& sbe : sparse_bufferlist) {
        ...
        case io::SPARSE_EXTENT_STATE_DATA:
            ...
            op.set_op_flags2(LIBRADOS_OP_FLAG_FADVISE_SEQUENTIAL | LIBRADOS_OP_FLAG_FADVISE_NOCACHE);
            break;
        case io::SPARSE_EXTENT_STATE_ZEROED:
            if (sbe.get_off() + sbe.get_len() == m_dst_image_ctx->layout.object_size) {
                if (sbe.get_off() == 0) {
                    if (hide_parent) {
    ...
    r = m_dst_io_ctx.aio_operate(m_dst_oid, comp, &op, dst_snap_seq, dst_snap_ids, nullptr);
    ceph_assert(r == 0);
    comp->release();
}

template<typename I> void ObjectCopyRequest<I>::handle_write_object(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to write to destination object: " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
...
template<typename I> Context* ObjectCopyRequest<I>::start_lock_op(ceph::shared_mutex& owner_lock, int* r)
{
...
template<typename I> void ObjectCopyRequest<I>::compute_read_ops()
{
    ldout(m_cct, 20) << dendl;

    m_src_image_ctx->image_lock.lock_shared();
    bool read_from_parent = (m_src_snap_id_start == 0 && m_src_image_ctx->parent != nullptr);
    m_src_image_ctx->image_lock.unlock_shared();
    ...
    for (auto& [key, image_intervals] : m_snapshot_delta) {
        ...
        for (auto& image_interval : image_intervals) {
            ...
            case io::SPARSE_EXTENT_STATE_DNE:
                if (write_read_snap_ids == io::INITIAL_WRITE_READ_SNAP_IDS && read_from_parent) {
                    // special-case for DNE initial object-extents since when flattening
                    // we need to read data from the parent images extents
                    ldout(m_cct, 20) << "DNE extent: " << image_interval.get_off() << "~" << image_interval.get_len()
                                     << dendl;
                    dne_image_interval.insert(image_interval.get_off(), image_interval.get_len());
                }
                break;
            case io::SPARSE_EXTENT_STATE_ZEROED: only_dne_extents = false; break;
            case io::SPARSE_EXTENT_STATE_DATA:
                ldout(m_cct, 20) << "read op: "
                                 << "snap_ids=" << write_read_snap_ids << " " << image_interval.get_off() << "~"
                                 << image_interval.get_len() << dendl;
                m_read_ops[write_read_snap_ids].image_interval.union_insert(image_interval.get_off(),
                                                                            image_interval.get_len());
    ...
    if (!dne_image_interval.empty() && (!only_dne_extents || flatten)) {
        ...
        int r = m_src_image_ctx->get_parent_overlap(src_snap_seq, &src_parent_overlap);
        m_src_image_ctx->image_lock.unlock_shared();

        if (r < 0) {
            ldout(m_cct, 5) << "failed getting parent overlap for snap_id: " << src_snap_seq << ": " << cpp_strerror(r)
                            << dendl;
        }
        else {
            ldout(m_cct, 20) << "parent overlap=" << src_parent_overlap << dendl;
            for (auto& [image_offset, image_length] : dne_image_interval) {
                auto end_image_offset = std::min(image_offset + image_length, src_parent_overlap);
                ...
                ldout(m_cct, 20) << "parent read op: "
                                 << "snap_ids=" << write_read_snap_ids << " " << image_offset << "~" << image_length
                                 << dendl;
                m_read_ops[write_read_snap_ids].image_interval.union_insert(image_offset, image_length);
            }
        }
    }
...
template<typename I> void ObjectCopyRequest<I>::merge_write_ops()
{
    ldout(m_cct, 20) << dendl;
    ...
    for (auto& [write_read_snap_ids, read_op] : m_read_ops) {
        ...
        for (auto& [image_offset, image_length] : read_op.image_interval) {
            ldout(m_cct, 20) << "src_snap_seq=" << src_snap_seq << ", "
                             << "inserting sparse-read zero " << image_offset << "~" << image_length << dendl;
            m_dst_zero_interval[src_snap_seq].union_insert(image_offset, image_length);
        }
        ...
        for (auto [image_offset, image_length] : read_op.image_extent_map) {
            ...
            io::util::file_to_extents(m_dst_image_ctx, image_offset, image_length, buffer_offset, &object_extents);
            for (auto& object_extent : object_extents) {
                ...
                m_snapshot_sparse_bufferlist[src_snap_seq].insert(
                    ...
                    {io::SPARSE_EXTENT_STATE_DATA, object_extent.length, std::move(sub_bl)});
...
template<typename I> void ObjectCopyRequest<I>::compute_zero_ops()
{
    ldout(m_cct, 20) << dendl;
    ...
    bool object_exists =
        (m_src_snap_id_start > 0 && m_snapshot_delta.count({m_src_snap_id_start, m_src_snap_id_start}) == 0);
    ...
    for (auto& it : m_dst_zero_interval) {
        ...
        if (hide_parent) {
            ...
            if (r < 0) {
                ldout(m_cct, 5) << "failed getting parent overlap for snap_id: " << dst_snap_seq << ": "
            ...
            if (parent_overlap == 0) {
                ...
                uint64_t overlap = m_dst_image_ctx->prune_parent_extents(image_extents, parent_overlap);
                if (overlap == 0) {
                    ldout(m_cct, 20) << "no parent overlap" << dendl;
                    hide_parent = false;
                }
        ...
        auto snapshot_delta_it =
            m_snapshot_delta.lower_bound({(hide_parent && src_snap_seq == first_src_snap_id ? 0 : src_snap_seq), 0});
        for (; snapshot_delta_it != m_snapshot_delta.end() && snapshot_delta_it->first.first <= src_snap_seq;
             ++snapshot_delta_it) {
                ...
                case io::SPARSE_EXTENT_STATE_ZEROED:
                    if (write_read_snap_ids != io::INITIAL_WRITE_READ_SNAP_IDS) {
                        ldout(m_cct, 20) << "zeroed extent: "
                                         << "src_snap_seq=" << src_snap_seq << " " << image_interval.get_off() << "~"
                                         << image_interval.get_len() << dendl;
                        zero_interval.union_insert(image_interval.get_off(), image_interval.get_len());
                    }
                    else if (hide_parent && write_read_snap_ids == io::INITIAL_WRITE_READ_SNAP_IDS) {
                    ...
                    break;
                case io::SPARSE_EXTENT_STATE_DNE:
                case io::SPARSE_EXTENT_STATE_DATA: break;
                default: ceph_abort(); break;
                }
            }
        }
        ...
        if (iter != m_snapshot_sparse_bufferlist.end()) {
            for (auto& sparse_bufferlist : iter->second) {
                object_exists = true;
                end_size = std::max(end_size, sparse_bufferlist.get_off() + sparse_bufferlist.get_len());
            }
        }
        ...
        for (auto z = zero_interval.begin(); z != zero_interval.end(); ++z) {
            // convert image extents back to object extents for the write op
            striper::LightweightObjectExtents object_extents;
            io::util::file_to_extents(m_dst_image_ctx, z.get_start(), z.get_len(), 0, &object_extents);
            for (auto& object_extent : object_extents) {
                ceph_assert(object_extent.offset + object_extent.length <= m_dst_image_ctx->layout.object_size);

                if (object_extent.offset + object_extent.length >= end_size) {
                    // zero interval at the object end
                    if ((object_extent.offset == 0 && hide_parent) || (object_extent.offset < prev_end_size)) {
                        ldout(m_cct, 20) << "truncate " << object_extent.offset << dendl;
                        auto length = m_dst_image_ctx->layout.object_size - object_extent.offset;
                        m_snapshot_sparse_bufferlist[src_snap_seq].insert(
                            object_extent.offset, length, {io::SPARSE_EXTENT_STATE_ZEROED, length});
                    }
        ...
        ldout(m_cct, 20) << "dst_snap_seq=" << dst_snap_seq << ", "
                         << "end_size=" << end_size << ", "
                         << "dst_object_map_state=" << static_cast<uint32_t>(dst_object_map_state) << dendl;
        prev_end_size = end_size;
    }
}

template<typename I> void ObjectCopyRequest<I>::finish(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;
...
template<typename I> void ObjectCopyRequest<I>::compute_dst_object_may_exist()
{
    std::shared_lock image_locker{m_dst_image_ctx->image_lock};
    ...
    for (auto snap_id : snap_ids) {
        m_dst_object_may_exist[snap_id] = (m_dst_object_number < m_dst_image_ctx->get_object_count(snap_id));
    }
