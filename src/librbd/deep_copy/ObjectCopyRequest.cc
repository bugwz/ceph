...
template<typename I> void ObjectCopyRequest<I>::send_list_snaps()
{
    ...
    ldout(m_cct, 20) << "image_extents=" << m_image_extents << " area=" << m_image_area << dendl;
...
template<typename I> void ObjectCopyRequest<I>::compute_read_ops()
{
    ...
    if (!dne_image_interval.empty() && (!only_dne_extents || flatten)) {
        ...
        if (r < 0) {
            ...
            m_src_image_ctx->prune_parent_extents(parent_extents, m_image_area, raw_overlap, false);
    ...
    for (auto& [write_read_snap_ids, read_op] : m_read_ops) {
        ...
        for (auto [image_offset, image_length] : read_op.image_extent_map) {
            ...
            io::util::area_to_object_extents(
                m_dst_image_ctx, image_offset, image_length, m_image_area, buffer_offset, &object_extents);
    ...
    for (auto& it : m_dst_zero_interval) {
        ...
        if (hide_parent) {
            ...
            if (r < 0) {
                ...
                object_overlap =
                    m_dst_image_ctx->prune_parent_extents(parent_extents, m_image_area, raw_overlap, false);
            }
            if (object_overlap == 0) {
        ...
        for (auto z = zero_interval.begin(); z != zero_interval.end(); ++z) {
            ...
            striper::LightweightObjectExtents object_extents;
            io::util::area_to_object_extents(
                m_dst_image_ctx, z.get_start(), z.get_len(), m_image_area, 0, &object_extents);
            for (auto& object_extent : object_extents) {
