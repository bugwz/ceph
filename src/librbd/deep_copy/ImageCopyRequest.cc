...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::deep_copy::ImageCopyRequest: " << this << " " << __func__ << ": "
...
template<typename I>
ImageCopyRequest<I>::ImageCopyRequest(I* src_image_ctx, I* dst_image_ctx, librados::snap_t src_snap_id_start,
                                      librados::snap_t src_snap_id_end, librados::snap_t dst_snap_id_start,
                                      bool flatten, const ObjectNumber& object_number, const SnapSeqs& snap_seqs,
...
template<typename I> void ImageCopyRequest<I>::send()
{
    m_dst_image_ctx->image_lock.lock_shared();
    util::compute_snap_map(
        m_dst_image_ctx->cct, m_src_snap_id_start, m_src_snap_id_end, m_dst_image_ctx->snaps, m_snap_seqs, &m_snap_map);
...
template<typename I> void ImageCopyRequest<I>::cancel()
{
...
template<typename I> void ImageCopyRequest<I>::map_src_objects(uint64_t dst_object, std::set<uint64_t>* src_objects)
{
    std::vector<std::pair<uint64_t, uint64_t>> image_extents;
    Striper::extent_to_file(
        m_cct, &m_dst_image_ctx->layout, dst_object, 0, m_dst_image_ctx->layout.object_size, image_extents);

    for (auto& e : image_extents) {
        std::map<object_t, std::vector<ObjectExtent>> src_object_extents;
        Striper::file_to_extents(
            m_cct, m_src_image_ctx->format_string, &m_src_image_ctx->layout, e.first, e.second, 0, src_object_extents);
...
template<typename I> void ImageCopyRequest<I>::compute_diff()
{
    if (m_flatten) {
        send_object_copies();
        return;
    ...
    auto ctx = create_context_callback<ImageCopyRequest<I>, &ImageCopyRequest<I>::handle_compute_diff>(this);
    auto req = object_map::DiffRequest<I>::create(
        m_src_image_ctx, m_src_snap_id_start, m_src_snap_id_end, 0, UINT64_MAX, &m_object_diff_state, ctx);
    req->send();
}

template<typename I> void ImageCopyRequest<I>::handle_compute_diff(int r)
{
...
template<typename I> void ImageCopyRequest<I>::send_object_copies()
{
    m_object_no = 0;
    if (m_object_number) {
    ...
    {
        std::lock_guard locker{m_lock};
        auto max_ops = m_src_image_ctx->config.template get_val<uint64_t>("rbd_concurrent_management_ops");
...
template<typename I> void ImageCopyRequest<I>::send_next_object_copy()
{
    ...
    Context* ctx = new LambdaContext([this, ono](int r) { handle_object_copy(ono, r); });
    ...
    auto req = ObjectCopyRequest<I>::create(m_src_image_ctx,
                                            m_dst_image_ctx,
                                            m_src_snap_id_start,
                                            m_dst_snap_id_start,
                                            m_snap_map,
...
template<typename I> void ImageCopyRequest<I>::handle_object_copy(uint64_t object_no, int r)
{
    ...
    bool complete;
    {
        ...
        if (r < 0 && r != -ENOENT) {
            ...
            while (!m_updating_progress && !m_copied_objects.empty() &&
                   m_copied_objects.top() == (m_object_number ? *m_object_number + 1 : 0)) {
...
template<typename I> void ImageCopyRequest<I>::finish(int r)
{
