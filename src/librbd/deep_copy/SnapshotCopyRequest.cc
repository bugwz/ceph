...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::deep_copy::SnapshotCopyRequest: " << this << " " << __func__ << ": "
...
template<typename I> const std::string& get_snapshot_name(I* image_ctx, librados::snap_t snap_id)
{
    auto snap_it = std::find_if(
...
template<typename I>
SnapshotCopyRequest<I>::SnapshotCopyRequest(I* src_image_ctx, I* dst_image_ctx, librados::snap_t src_snap_id_start,
                                            librados::snap_t src_snap_id_end, librados::snap_t dst_snap_id_start,
                                            bool flatten, asio::ContextWQ* work_queue, SnapSeqs* snap_seqs,
                                            Context* on_finish)
    ...
    , m_lock(ceph::make_mutex(unique_lock_name("SnapshotCopyRequest::m_lock", this)))
{
    ...
    m_src_snap_ids.insert(src_image_ctx->snaps.begin(), src_image_ctx->snaps.end());
    m_src_image_ctx->image_lock.unlock_shared();

    m_dst_image_ctx->image_lock.lock_shared();
    m_dst_snap_ids.insert(dst_image_ctx->snaps.begin(), dst_image_ctx->snaps.end());
    m_dst_image_ctx->image_lock.unlock_shared();

    if (m_src_snap_id_end != CEPH_NOSNAP) {
        m_src_snap_ids.erase(m_src_snap_ids.upper_bound(m_src_snap_id_end), m_src_snap_ids.end());
    }
}

template<typename I> void SnapshotCopyRequest<I>::send()
{
...
template<typename I> void SnapshotCopyRequest<I>::cancel()
{
...
template<typename I> void SnapshotCopyRequest<I>::send_snap_unprotect()
{
    ...
    for (; snap_id_it != m_dst_snap_ids.end(); ++snap_id_it) {
        ...
        if (r < 0) {
            lderr(m_cct) << "failed to retrieve destination snap unprotect status: " << cpp_strerror(r) << dendl;
            m_dst_image_ctx->image_lock.unlock_shared();
            finish(r);
            return;
        }
        ...
        auto snap_seq_it =
            std::find_if(m_snap_seqs.begin(), m_snap_seqs.end(), [dst_snap_id](const SnapSeqs::value_type& pair) {
        ...
        if (snap_seq_it != m_snap_seqs.end()) {
            ...
            r = m_src_image_ctx->is_snap_unprotected(snap_seq_it->first, &src_unprotected);
            ldout(m_cct, 20) << "m_src_image_ctx->is_snap_unprotected(" << snap_seq_it->first << "): r=" << r
            ...
            if (r < 0) {
                lderr(m_cct) << "failed to retrieve source snap unprotect status: " << cpp_strerror(r) << dendl;
                m_src_image_ctx->image_lock.unlock_shared();
                finish(r);
                return;
            }
    ...
    m_dst_image_ctx->operations->execute_snap_unprotect(cls::rbd::UserSnapshotNamespace(), m_snap_name.c_str(), ctx);
}

template<typename I> void SnapshotCopyRequest<I>::handle_snap_unprotect(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;

    if (r < 0) {
        lderr(m_cct) << "failed to unprotect snapshot '" << m_snap_name << "': " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
...
template<typename I> void SnapshotCopyRequest<I>::send_snap_remove()
{
    SnapIdSet::iterator snap_id_it = m_dst_snap_ids.begin();
    if (m_prev_snap_id != CEPH_NOSNAP) {
        snap_id_it = m_dst_snap_ids.upper_bound(m_prev_snap_id);
    ...
    for (; snap_id_it != m_dst_snap_ids.end(); ++snap_id_it) {
        ...
        m_dst_image_ctx->image_lock.unlock_shared();
        if (r < 0) {
            lderr(m_cct) << "failed to retrieve destination snap namespace: " << m_snap_name << dendl;
            finish(r);
            return;
        }
    ...
    std::shared_lock owner_locker{m_dst_image_ctx->owner_lock};
    m_dst_image_ctx->operations->execute_snap_remove(cls::rbd::UserSnapshotNamespace(), m_snap_name.c_str(), ctx);
}

template<typename I> void SnapshotCopyRequest<I>::handle_snap_remove(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;

    if (r < 0) {
        lderr(m_cct) << "failed to remove snapshot '" << m_snap_name << "': " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
    if (handle_cancellation()) {
...
template<typename I> void SnapshotCopyRequest<I>::send_snap_create()
{
    ...
    for (; snap_id_it != m_src_snap_ids.end(); ++snap_id_it) {
        ...
        m_src_image_ctx->image_lock.unlock_shared();
        if (r < 0) {
            lderr(m_cct) << "failed to retrieve source snap namespace: " << m_snap_name << dendl;
            finish(r);
            return;
        }
    ...
    if (snap_info_it == m_src_image_ctx->snap_info.end()) {
        m_src_image_ctx->image_lock.unlock_shared();
        lderr(m_cct) << "failed to retrieve source snap info: " << m_snap_name << dendl;
        finish(-ENOENT);
        return;
    }
    ...
    SnapshotCreateRequest<I>* req = SnapshotCreateRequest<I>::create(
        m_dst_image_ctx, m_snap_name, m_snap_namespace, size, parent_spec, parent_overlap, ctx);
    req->send();
}

template<typename I> void SnapshotCopyRequest<I>::handle_snap_create(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;

    if (r < 0) {
        lderr(m_cct) << "failed to create snapshot '" << m_snap_name << "': " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
    if (handle_cancellation()) {
    ...
    auto snap_it = m_dst_image_ctx->snap_ids.find({cls::rbd::UserSnapshotNamespace(), m_snap_name});
...
template<typename I> void SnapshotCopyRequest<I>::send_snap_protect()
{
    SnapIdSet::iterator snap_id_it = m_src_snap_ids.begin();
    if (m_prev_snap_id != CEPH_NOSNAP) {
        snap_id_it = m_src_snap_ids.upper_bound(m_prev_snap_id);
    ...
    for (; snap_id_it != m_src_snap_ids.end(); ++snap_id_it) {
        ...
        if (r < 0) {
            lderr(m_cct) << "failed to retrieve source snap protect status: " << cpp_strerror(r) << dendl;
            m_src_image_ctx->image_lock.unlock_shared();
            finish(r);
            return;
        }
        ...
        if (r < 0) {
            lderr(m_cct) << "failed to retrieve destination snap protect status: " << cpp_strerror(r) << dendl;
            m_dst_image_ctx->image_lock.unlock_shared();
            finish(r);
            return;
        }
    ...
    std::shared_lock owner_locker{m_dst_image_ctx->owner_lock};
    m_dst_image_ctx->operations->execute_snap_protect(cls::rbd::UserSnapshotNamespace(), m_snap_name.c_str(), ctx);
}

template<typename I> void SnapshotCopyRequest<I>::handle_snap_protect(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;

    if (r < 0) {
        lderr(m_cct) << "failed to protect snapshot '" << m_snap_name << "': " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
    if (handle_cancellation()) {
...
template<typename I> void SnapshotCopyRequest<I>::send_set_head()
{
    auto snap_seq_it = m_snap_seqs.find(m_src_snap_id_end);
    if (m_src_snap_id_end != CEPH_NOSNAP && (snap_seq_it == m_snap_seqs.end() || snap_seq_it->second != CEPH_NOSNAP)) {
    ...
    auto ctx = create_context_callback<SnapshotCopyRequest<I>, &SnapshotCopyRequest<I>::handle_set_head>(this);
    auto req = SetHeadRequest<I>::create(m_dst_image_ctx, size, parent_spec, parent_overlap, ctx);
    req->send();
}

template<typename I> void SnapshotCopyRequest<I>::handle_set_head(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;
...
template<typename I> void SnapshotCopyRequest<I>::send_resize_object_map()
{
    int r = 0;
    ...
    if (m_dst_image_ctx->test_features(RBD_FEATURE_OBJECT_MAP)) {
        ...
        if (m_dst_image_ctx->object_map != nullptr &&
            Striper::get_num_objects(m_dst_image_ctx->layout, m_dst_image_ctx->size) !=
            ...
            if (finish_op_ctx != nullptr) {
                ...
                m_dst_image_ctx->object_map->aio_resize(m_dst_image_ctx->size, OBJECT_NONEXISTENT, ctx);
                return;
            }
...
template<typename I> void SnapshotCopyRequest<I>::handle_resize_object_map(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;

    if (r < 0) {
        lderr(m_cct) << "failed to resize object map: " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
...
template<typename I> bool SnapshotCopyRequest<I>::handle_cancellation()
{
    {
        std::lock_guard locker{m_lock};
        if (!m_canceled) {
...
template<typename I> void SnapshotCopyRequest<I>::error(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;
...
template<typename I> int SnapshotCopyRequest<I>::validate_parent(I* image_ctx, cls::rbd::ParentImageSpec* spec)
{
...
template<typename I> Context* SnapshotCopyRequest<I>::start_lock_op(int* r)
{
    std::shared_lock owner_locker{m_dst_image_ctx->owner_lock};
    return start_lock_op(m_dst_image_ctx->owner_lock, r);
}

template<typename I> Context* SnapshotCopyRequest<I>::start_lock_op(ceph::shared_mutex& owner_lock, int* r)
{
...
template<typename I> void SnapshotCopyRequest<I>::finish(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;
