...
template<typename ImageCtxT = ImageCtx> class ImageCopyRequest : public RefCountedObject
{
public:
    static ImageCopyRequest* create(ImageCtxT* src_image_ctx, ImageCtxT* dst_image_ctx,
    ...
    {
        return new ImageCopyRequest(src_image_ctx,
                                    dst_image_ctx,
                                    src_snap_id_start,
                                    src_snap_id_end,
                                    dst_snap_id_start,
    ...
    ImageCopyRequest(ImageCtxT* src_image_ctx, ImageCtxT* dst_image_ctx, librados::snap_t src_snap_id_start,
                     librados::snap_t src_snap_id_end, librados::snap_t dst_snap_id_start, bool flatten,
                     const ObjectNumber& object_number, const SnapSeqs& snap_seqs, Handler* handler,
                     Context* on_finish);
...
private:
    ...
    std::priority_queue<uint64_t, std::vector<uint64_t>, std::greater<uint64_t>> m_copied_objects;
