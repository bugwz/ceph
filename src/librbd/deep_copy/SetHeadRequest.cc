...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::deep_copy::SetHeadRequest: " << this << " " << __func__ << ": "
...
template<typename I>
SetHeadRequest<I>::SetHeadRequest(I* image_ctx, uint64_t size, const cls::rbd::ParentImageSpec& spec,
...
template<typename I> void SetHeadRequest<I>::send()
{
    send_set_size();
}

template<typename I> void SetHeadRequest<I>::send_set_size()
{
...
template<typename I> void SetHeadRequest<I>::handle_set_size(int r)
{
...

    {
        ...
        if (m_image_ctx->size > m_size) {
            if (m_image_ctx->parent_md.spec.pool_id != -1 && m_image_ctx->parent_md.overlap > m_size) {
...
template<typename I> void SetHeadRequest<I>::send_detach_parent()
{
    m_image_ctx->image_lock.lock_shared();
    if (m_image_ctx->parent_md.spec.pool_id == -1 ||
        (m_image_ctx->parent_md.spec == m_parent_spec && m_image_ctx->parent_md.overlap == m_parent_overlap)) {
        m_image_ctx->image_lock.unlock_shared();
        send_attach_parent();
        return;
    }
...
template<typename I> void SetHeadRequest<I>::handle_detach_parent(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;
...
template<typename I> void SetHeadRequest<I>::send_attach_parent()
{
    m_image_ctx->image_lock.lock_shared();
    ...
    auto req = image::AttachParentRequest<I>::create(*m_image_ctx, m_parent_spec, m_parent_overlap, false, ctx);
    req->send();
}

template<typename I> void SetHeadRequest<I>::handle_attach_parent(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;
...
template<typename I> Context* SetHeadRequest<I>::start_lock_op(int* r)
{
...
template<typename I> void SetHeadRequest<I>::finish(int r)
{
    ldout(m_cct, 20) << "r=" << r << dendl;
