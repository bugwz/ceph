...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::image_watcher::NotifyLockOwner: " << this << " " << __func__
...
NotifyLockOwner::NotifyLockOwner(ImageCtx& image_ctx, watcher::Notifier& notifier, bufferlist&& bl, Context* on_finish)
...
void NotifyLockOwner::send_notify()
{
    ...
    m_notifier.notify(
        m_bl, &m_notify_response, create_context_callback<NotifyLockOwner, &NotifyLockOwner::handle_notify>(this));
}
...
void NotifyLockOwner::handle_notify(int r)
{
    ...
    if (r < 0 && r != -ETIMEDOUT) {
        lderr(cct) << ": lock owner notification failed: " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
