...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::watcher::Notifier: " << this << " " << __func__ << ": "
...
Notifier::C_AioNotify::C_AioNotify(Notifier* notifier, NotifyResponse* response, Context* on_finish)
    : notifier(notifier)
    , response(response)
    , on_finish(on_finish)
{}
...
Notifier::Notifier(asio::ContextWQ* work_queue, IoCtx& ioctx, const std::string& oid)
    : m_work_queue(work_queue)
    , m_ioctx(ioctx)
    , m_oid(oid)
    , m_aio_notify_lock(
          ceph::make_mutex(util::unique_lock_name("librbd::object_watcher::Notifier::m_aio_notify_lock", this)))
{
...
void Notifier::notify(bufferlist& bl, NotifyResponse* response, Context* on_finish)
{
    {
        std::lock_guard aio_notify_locker{m_aio_notify_lock};
