...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::watcher::RewatchRequest: " << this << " " << __func__ << " "
...
RewatchRequest::RewatchRequest(librados::IoCtx& ioctx, const string& oid, ceph::shared_mutex& watch_lock,
                               librados::WatchCtx2* watch_ctx, uint64_t* watch_handle, Context* on_finish)
    : m_ioctx(ioctx)
    , m_oid(oid)
...
void RewatchRequest::unwatch()
{
    ...
    librados::AioCompletion* aio_comp = create_rados_callback<RewatchRequest, &RewatchRequest::handle_unwatch>(this);
...
void RewatchRequest::rewatch()
{
    ...
    librados::AioCompletion* aio_comp = create_rados_callback<RewatchRequest, &RewatchRequest::handle_rewatch>(this);
...
void RewatchRequest::handle_rewatch(int r)
{
    ...
    ldout(cct, 10) << "r=" << r << dendl;
    if (r < 0) {
        lderr(cct) << "failed to watch object: " << cpp_strerror(r) << dendl;
        m_rewatch_handle = 0;
    }
