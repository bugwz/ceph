...
template<typename ImageCtxT = librbd::ImageCtx> class ManagedLock
{
private:
...
public:
    static ManagedLock* create(librados::IoCtx& ioctx, AsioEngine& asio_engine, const std::string& oid,
                               Watcher* watcher, managed_lock::Mode mode, bool blocklist_on_break_lock,
                               uint32_t blocklist_expire_seconds)
    {
        return new ManagedLock(
            ioctx, asio_engine, oid, watcher, mode, blocklist_on_break_lock, blocklist_expire_seconds);
    }
    void destroy() { delete this; }

    ManagedLock(librados::IoCtx& ioctx, AsioEngine& asio_engine, const std::string& oid, Watcher* watcher,
                managed_lock::Mode mode, bool blocklist_on_break_lock, uint32_t blocklist_expire_seconds);
    virtual ~ManagedLock();
...
protected:
    ...
    virtual void pre_release_lock_handler(bool shutting_down, Context* on_finish);
    virtual void post_release_lock_handler(bool shutting_down, int r, Context* on_finish);
...
private:
    ...
    struct C_ShutDownRelease : public Context
    {
        ...
        C_ShutDownRelease(ManagedLock* lock)
            : lock(lock)
        {}
        void finish(int r) override { lock->send_shutdown_release(); }
    };
