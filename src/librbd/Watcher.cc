...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::Watcher::C_NotifyAck " << this << " " << __func__ << ": "

Watcher::C_NotifyAck::C_NotifyAck(Watcher* watcher, uint64_t notify_id, uint64_t handle)
    : watcher(watcher)
    , cct(watcher->m_cct)
    , notify_id(notify_id)
    , handle(handle)
{
    ldout(cct, 10) << "id=" << notify_id << ", "
                   << "handle=" << handle << dendl;
}
...
Watcher::Watcher(librados::IoCtx& ioctx, asio::ContextWQ* work_queue, const string& oid)
    : m_ioctx(ioctx)
...
void Watcher::register_watch(Context* on_finish)
{
    ...
    librados::AioCompletion* aio_comp = create_rados_callback(new C_RegisterWatch(this, on_finish));
...
void Watcher::unregister_watch(Context* on_finish)
{
...

    {
        ...
        if (m_watch_state != WATCH_STATE_IDLE) {
            ...
            m_unregister_watch_ctx = new LambdaContext([this, on_finish](int r) { unregister_watch(on_finish); });
            return;
        }
        else if (is_registered(m_watch_lock)) {
            librados::AioCompletion* aio_comp = create_rados_callback(new C_UnwatchAndFlush(m_ioctx, on_finish));
...
void Watcher::handle_error(uint64_t handle, int err)
{
    ...
    if (is_registered(m_watch_lock)) {
        ...
        auto ctx = new LambdaContext(boost::bind(&Watcher::rewatch, this));
        m_work_queue->queue(ctx);
    }
}

void Watcher::acknowledge_notify(uint64_t notify_id, uint64_t handle, bufferlist& out)
{
...
void Watcher::rewatch()
{
    ...
    Context* unregister_watch_ctx = nullptr;
    {
        ...
        if (m_unregister_watch_ctx != nullptr) {
            ...
            m_watch_error = false;
            auto ctx = create_context_callback<Watcher, &Watcher::handle_rewatch>(this);
            auto req = RewatchRequest::create(m_ioctx, m_oid, m_watch_lock, &m_watch_ctx, &m_watch_handle, ctx);
            req->send();
            return;
        }
    }
...
void Watcher::handle_rewatch(int r)
{
    ...
    auto ctx = create_context_callback<Watcher, &Watcher::handle_rewatch_callback>(this);
...
void Watcher::send_notify(bufferlist& payload, watcher::NotifyResponse* response, Context* on_finish)
{
    m_notifier.notify(payload, response, on_finish);
}

void Watcher::WatchCtx::handle_notify(uint64_t notify_id, uint64_t handle, uint64_t notifier_id, bufferlist& bl)
{
