...
bool is_metadata_config_override(const std::string& metadata_key, std::string* config_key)
{
    ...
    if (metadata_key.size() > prefix_len &&
        metadata_key.compare(0, prefix_len, librbd::ImageCtx::METADATA_CONF_PREFIX) == 0) {
        *config_key = metadata_key.substr(prefix_len, metadata_key.size() - prefix_len);
        return true;
    }
    return false;
}
                 ...
                 const std::optional<std::string>& pool_namespace, librados::IoCtx* dst_io_ctx)
{
    ...
    if (r == -ENOENT) {
        ...
        lderr(cct) << "error accessing " << pool_desc << " pool " << pool_id << dendl;
        return r;
    }

    dst_io_ctx->set_namespace(pool_namespace ? *pool_namespace : src_io_ctx.get_namespace());
...
int snap_create_flags_api_to_internal(CephContext* cct, uint32_t api_flags, uint64_t* internal_flags)
{
    *internal_flags = 0;
    ...
    if (api_flags != 0) {
        lderr(cct) << "invalid snap create flags: " << std::bitset<32>(api_flags) << dendl;
        return -EINVAL;
    }
...
{
    auto mode = ictx->config.get_val<std::string>("rbd_default_snapshot_quiesce_mode");
...
int get_config_key(librados::Rados& rados, const std::string& uri, std::string* value)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to retrieve MON config key " << key << ": " << cpp_strerror(r) << dendl;
        return r;
    }
