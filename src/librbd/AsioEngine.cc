...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::AsioEngine: " << this << " " << __func__ << ": "

namespace librbd {

AsioEngine::AsioEngine(std::shared_ptr<librados::Rados> rados)
    : m_rados_api(std::make_shared<neorados::RADOS>(neorados::RADOS::make_with_librados(*rados)))
    , m_cct(m_rados_api->cct())
    , m_io_context(m_rados_api->get_io_context())
    , m_api_strand(std::make_unique<boost::asio::io_context::strand>(m_io_context))
    , m_context_wq(std::make_unique<asio::ContextWQ>(m_cct, m_io_context))
{
    ...
    if (rbd_threads > rados_threads) {
        ...
        m_cct->_conf.set_val_or_die("librados_thread_count", std::to_string(rbd_threads));
