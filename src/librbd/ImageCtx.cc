...
uint64_t ImageCtx::get_effective_image_size(snap_t in_snap_id) const
{
    ...
    io_image_dispatcher->remap_extents(extents, io::IMAGE_EXTENTS_MAP_TYPE_PHYSICAL_TO_LOGICAL);
...
uint64_t ImageCtx::prune_parent_extents(vector<pair<uint64_t, uint64_t> >& objectx, uint64_t overlap)
{
    // drop extents completely beyond the overlap
    while (!objectx.empty() && objectx.back().first >= overlap) objectx.pop_back();
    ...
    uint64_t len = 0;
    for (vector<pair<uint64_t, uint64_t> >::iterator p = objectx.begin(); p != objectx.end(); ++p) len += p->second;
    ldout(cct, 10) << "prune_parent_extents image overlap " << overlap << ", object overlap " << len
