...
ImageCtx::ImageCtx(const string& image_name, const string& image_id, const char* snap, IoCtx& p, bool ro)
...
ImageCtx::ImageCtx(const string& image_name, const string& image_id, uint64_t snap_id, IoCtx& p, bool ro)
...
void ImageCtx::init()
{
    ...
    string pname = string("librbd-") + id + string("-") + md_ctx.get_pool_name() + string("-") + name;
    if (!snap_name.empty()) {
        pname += "-";
        pname += snap_name;
    }
...
void ImageCtx::init_layout(int64_t pool_id)
{
    ...
    ldout(cct, 10) << "init_layout stripe_unit " << stripe_unit << " stripe_count " << stripe_count << " object_size "
                   << layout.object_size << " prefix " << object_prefix << " format " << format_string << dendl;
}
...
void ImageCtx::perf_start(string name)
{
    ...
    plb.add_u64_counter(l_librbd_rd_bytes, "rd_bytes", "Data size in reads", "rb", perf_prio, unit_t(UNIT_BYTES));
    plb.add_time_avg(l_librbd_rd_latency, "rd_latency", "Latency of reads", "rl", perf_prio);
    plb.add_u64_counter(l_librbd_wr, "wr", "Writes", "w", perf_prio);
    plb.add_u64_counter(l_librbd_wr_bytes, "wr_bytes", "Written data", "wb", perf_prio, unit_t(UNIT_BYTES));
    plb.add_time_avg(l_librbd_wr_latency, "wr_latency", "Write latency", "wl", perf_prio);
    ...
    plb.add_u64_counter(
        l_librbd_readahead_bytes, "readahead_bytes", "Data size in read ahead", NULL, 0, unit_t(UNIT_BYTES));
    plb.add_u64_counter(l_librbd_invalidate_cache, "invalidate_cache", "Cache invalidates");

    plb.add_time(l_librbd_opened_time, "opened_time", "Opened time", "ots", perf_prio);
    plb.add_time(l_librbd_lock_acquired_time, "lock_acquired_time", "Lock acquired time", "lats", perf_prio);
...
int ImageCtx::get_read_flags(snap_t snap_id)
{
    ...
    if (snap_id == LIBRADOS_SNAP_HEAD) return flags;
...
snap_t ImageCtx::get_snap_id(const cls::rbd::SnapshotNamespace& in_snap_namespace, const string& in_snap_name) const
{
...
{
    ceph_assert(ceph_mutex_is_locked(image_lock));
    map<snap_t, SnapInfo>::const_iterator it = snap_info.find(in_snap_id);
    if (it != snap_info.end()) return &it->second;
    return nullptr;
}

int ImageCtx::get_snap_name(snap_t in_snap_id, string* out_snap_name) const
{
    ceph_assert(ceph_mutex_is_locked(image_lock));
...
int ImageCtx::get_snap_namespace(snap_t in_snap_id, cls::rbd::SnapshotNamespace* out_snap_namespace) const
{
    ceph_assert(ceph_mutex_is_locked(image_lock));
    const SnapInfo* info = get_snap_info(in_snap_id);
    if (info) {
...
int ImageCtx::get_parent_spec(snap_t in_snap_id, cls::rbd::ParentImageSpec* out_pspec) const
{
    const SnapInfo* info = get_snap_info(in_snap_id);
    if (info) {
...
int ImageCtx::is_snap_protected(snap_t in_snap_id, bool* is_protected) const
{
    ceph_assert(ceph_mutex_is_locked(image_lock));
    const SnapInfo* info = get_snap_info(in_snap_id);
    if (info) {
        *is_protected = (info->protection_status == RBD_PROTECTION_STATUS_PROTECTED);
        return 0;
    }
    return -ENOENT;
}

int ImageCtx::is_snap_unprotected(snap_t in_snap_id, bool* is_unprotected) const
{
    ceph_assert(ceph_mutex_is_locked(image_lock));
    const SnapInfo* info = get_snap_info(in_snap_id);
    if (info) {
        *is_unprotected = (info->protection_status == RBD_PROTECTION_STATUS_UNPROTECTED);
        return 0;
    }
    return -ENOENT;
}

void ImageCtx::add_snap(cls::rbd::SnapshotNamespace in_snap_namespace, string in_snap_name, snap_t id, uint64_t in_size,
                        const ParentImageInfo& parent, uint8_t protection_status, uint64_t flags, utime_t timestamp)
{
    ceph_assert(ceph_mutex_is_wlocked(image_lock));
    snaps.push_back(id);
    SnapInfo info(in_snap_name, in_snap_namespace, in_size, parent, protection_status, flags, timestamp);
    snap_info.insert({id, info});
    snap_ids.insert({{in_snap_namespace, in_snap_name}, id});
}

void ImageCtx::rm_snap(cls::rbd::SnapshotNamespace in_snap_namespace, string in_snap_name, snap_t id)
{
    ceph_assert(ceph_mutex_is_wlocked(image_lock));
...
uint64_t ImageCtx::get_image_size(snap_t in_snap_id) const
{
    ...
    if (in_snap_id == CEPH_NOSNAP) {
        if (!resize_reqs.empty() && resize_reqs.front()->shrinking()) {
...
uint64_t ImageCtx::get_area_size(io::ImageArea area) const
{
    ...
    switch (area) {
    case io::ImageArea::DATA: return size.first;
...
bool ImageCtx::test_features(uint64_t in_features, const ceph::shared_mutex& in_image_lock) const
{
    ceph_assert(ceph_mutex_is_locked(image_lock));
...
bool ImageCtx::test_op_features(uint64_t in_op_features, const ceph::shared_mutex& in_image_lock) const
{
    ceph_assert(ceph_mutex_is_locked(image_lock));
...
int ImageCtx::test_flags(librados::snap_t in_snap_id, uint64_t flags, bool* flags_set) const
{
    std::shared_lock l{image_lock};
    return test_flags(in_snap_id, flags, image_lock, flags_set);
}

int ImageCtx::test_flags(librados::snap_t in_snap_id, uint64_t flags, const ceph::shared_mutex& in_image_lock,
...
{
    ceph_assert(ceph_mutex_is_locked(image_lock));
    if (in_snap_id == CEPH_NOSNAP) return &parent_md;
    const SnapInfo* info = get_snap_info(in_snap_id);
...
int64_t ImageCtx::get_parent_pool_id(snap_t in_snap_id) const
{
    ...
    if (info) return info->spec.pool_id;
    return -1;
}
...
{
    const auto info = get_parent_info(in_snap_id);
    if (info) return info->spec.image_id;
    return "";
}
...
{
    const auto info = get_parent_info(in_snap_id);
    if (info) return info->spec.snap_id;
    return CEPH_NOSNAP;
}

int ImageCtx::get_parent_overlap(snap_t in_snap_id, uint64_t* raw_overlap) const
{
    const auto info = get_parent_info(in_snap_id);
    if (info) {
...
std::pair<uint64_t, io::ImageArea> ImageCtx::reduce_parent_overlap(uint64_t raw_overlap, bool migration_write) const
{
    ceph_assert(ceph_mutex_is_locked(image_lock));
    if (migration_write) {
    ...
    overlap.first = std::min(overlap.first, parent->get_area_size(overlap.second));
    return overlap;
}

uint64_t ImageCtx::prune_parent_extents(io::Extents& image_extents, io::ImageArea area, uint64_t raw_overlap,
                                        bool migration_write) const
{
    ceph_assert(ceph_mutex_is_locked(image_lock));
    ldout(cct, 10) << __func__ << ": image_extents=" << image_extents << " area=" << area
                   << " raw_overlap=" << raw_overlap << " migration_write=" << migration_write << dendl;
    if (raw_overlap == 0) {
        image_extents.clear();
        return 0;
    }
    ...
    if (area == overlap.second) {
        ...
        while (!image_extents.empty() && image_extents.back().first >= overlap.first) {
    ...
    else if (area == io::ImageArea::DATA && overlap.second == io::ImageArea::CRYPTO_HEADER) {
        // all extents completely beyond the overlap
        image_extents.clear();
    }
    else {
        // all extents completely within the overlap
        ceph_assert(area == io::ImageArea::CRYPTO_HEADER && overlap.second == io::ImageArea::DATA);
    }
    ...
    ldout(cct, 10) << __func__ << ": overlap=" << overlap.first << "/" << overlap.second
                   << " got overlap_bytes=" << overlap_bytes << " at " << image_extents << dendl;
...
void ImageCtx::apply_metadata(const std::map<std::string, bufferlist>& meta, bool thread_safe)
{
        ...
        if (config.find_option(key) != nullptr) {
            ...
            if (r >= 0) {
                ...
                lderr(cct) << __func__ << ": failed to set config " << key << " "
                           << "with value " << val << ": " << cpp_strerror(r) << dendl;
            }
        }
    }
...
#define ASSIGN_OPTION(param, type) param = config.get_val<type>("rbd_" #param)
...
#undef ASSIGN_OPTION
    ...
    io_image_dispatcher->apply_qos_schedule_tick_min(config.get_val<uint64_t>("rbd_qos_schedule_tick_min"));

    io_image_dispatcher->apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_IOPS_THROTTLE,
    ...
    io_image_dispatcher->apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_BPS_THROTTLE,
    ...
    io_image_dispatcher->apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_READ_IOPS_THROTTLE,
    ...
    io_image_dispatcher->apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_WRITE_IOPS_THROTTLE,
    ...
    io_image_dispatcher->apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_READ_BPS_THROTTLE,
    ...
    io_image_dispatcher->apply_qos_limit(io::IMAGE_DISPATCH_FLAG_QOS_WRITE_BPS_THROTTLE,
    ...
    io_image_dispatcher->apply_qos_exclude_ops(
        librbd::io::rbd_io_operations_from_string(config.get_val<std::string>("rbd_qos_exclude_ops"), nullptr));

    if (!disable_zero_copy && config.get_val<bool>("rbd_disable_zero_copy_writes")) {
...
{
    auto ctx = std::make_shared<neorados::IOContext>(data_ctx.get_id(), data_ctx.get_namespace());
    if (snap_id != CEPH_NOSNAP) {
        ctx->read_snap(snap_id);
    }
    if (!snapc.snaps.empty()) {
        ctx->write_snap_context({{snapc.seq, {snapc.snaps.begin(), snapc.snaps.end()}}});
    }
...
void ImageCtx::get_timer_instance(CephContext* cct, SafeTimer** timer, ceph::mutex** timer_lock)
{
    auto safe_timer_singleton =
        &cct->lookup_or_create_singleton_object<SafeTimerSingleton>("librbd::journal::safe_timer", false, cct);
