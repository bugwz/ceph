...
template<typename ImageCtxT = librbd::ImageCtx> class MoveRequest
{
public:
    ...
    MoveRequest(librados::IoCtx& io_ctx, const std::string& image_id, const cls::rbd::TrashImageSpec& trash_image_spec,
                Context* on_finish)
        : m_io_ctx(io_ctx)
        , m_image_id(image_id)
