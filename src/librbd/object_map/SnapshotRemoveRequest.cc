...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::object_map::SnapshotRemoveRequest: " << this << " " << __func__ << ": "
...
void SnapshotRemoveRequest::load_map()
{
    ...
    auto rados_completion =
        librbd::util::create_rados_callback<SnapshotRemoveRequest, &SnapshotRemoveRequest::handle_load_map>(this);
    int r = m_image_ctx.md_ctx.aio_operate(snap_oid, rados_completion, &op, &m_out_bl);
    ceph_assert(r == 0);
...
void SnapshotRemoveRequest::handle_load_map(int r)
{
    ...
    if (r == -ENOENT) {
        ...
        lderr(cct) << "failed to load object map " << oid << ": " << cpp_strerror(r) << dendl;
...
void SnapshotRemoveRequest::remove_snapshot()
{
    ...
    auto rados_completion =
        librbd::util::create_rados_callback<SnapshotRemoveRequest, &SnapshotRemoveRequest::handle_remove_snapshot>(
            this);
...
void SnapshotRemoveRequest::handle_remove_snapshot(int r)
{
    ...
    if (r < 0 && r != -ENOENT) {
        std::string oid(ObjectMap<>::object_map_name(m_image_ctx.id, m_next_snap_id));
        lderr(cct) << "failed to remove object map snapshot " << oid << ": " << cpp_strerror(r) << dendl;
...
void SnapshotRemoveRequest::invalidate_next_map()
{
    ...
    auto ctx = librbd::util::create_context_callback<SnapshotRemoveRequest,
                                                     &SnapshotRemoveRequest::handle_invalidate_next_map>(this);
    InvalidateRequest<>* req = new InvalidateRequest<>(m_image_ctx, m_next_snap_id, true, ctx);
    req->send();
}
...
void SnapshotRemoveRequest::handle_invalidate_next_map(int r)
{
    ...
    if (r < 0) {
        std::string oid(ObjectMap<>::object_map_name(m_image_ctx.id, m_next_snap_id));
        lderr(cct) << "failed to invalidate object map " << oid << ": " << cpp_strerror(r) << dendl;
        complete(r);
        return;
    }
...
void SnapshotRemoveRequest::remove_map()
{
    ...
    auto rados_completion =
        librbd::util::create_rados_callback<SnapshotRemoveRequest, &SnapshotRemoveRequest::handle_remove_map>(this);
    int r = m_image_ctx.md_ctx.aio_operate(oid, rados_completion, &op);
    ceph_assert(r == 0);
    rados_completion->release();
}
...
void SnapshotRemoveRequest::handle_remove_map(int r)
{
    ...
    if (r < 0 && r != -ENOENT) {
        std::string oid(ObjectMap<>::object_map_name(m_image_ctx.id, m_snap_id));
        lderr(cct) << "failed to remove object map " << oid << ": " << cpp_strerror(r) << dendl;
        complete(r);
        return;
    }
...
void SnapshotRemoveRequest::compute_next_snap_id()
{
    ...
    std::map<librados::snap_t, SnapInfo>::const_iterator it = m_image_ctx.snap_info.find(m_snap_id);
...
void SnapshotRemoveRequest::update_object_map()
{
    ...
    if (m_next_snap_id == m_image_ctx.snap_id && m_next_snap_id == CEPH_NOSNAP) {
        ...
        uint64_t i = 0;
        for (; it != end_it; ++it) {
            if (*it == OBJECT_EXISTS_CLEAN && (i >= m_snap_object_map.size() || *snap_it == OBJECT_EXISTS)) {
                *it = OBJECT_EXISTS;
            }
