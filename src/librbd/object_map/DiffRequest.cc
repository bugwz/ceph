...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::object_map::DiffRequest: " << this << " " << __func__ << ": "
...
template<typename I>
DiffRequest<I>::DiffRequest(I* image_ctx, uint64_t snap_id_start, uint64_t snap_id_end, uint64_t start_object_no,
                            uint64_t end_object_no, BitVector<2>* object_diff_state, Context* on_finish)
    ...
    , m_on_finish(on_finish)
{
    ...
    ldout(cct, 10) << "snap_id_start=" << m_snap_id_start << ", snap_id_end=" << m_snap_id_end
                   << ", start_object_no=" << m_start_object_no << ", end_object_no=" << m_end_object_no << dendl;
}

template<typename I> bool DiffRequest<I>::is_diff_iterate() const
{
    return m_start_object_no != 0 || m_end_object_no != UINT64_MAX;
}

template<typename I> int DiffRequest<I>::prepare_for_object_map()
{
...
template<typename I> int DiffRequest<I>::process_object_map(const BitVector<2>& object_map)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 20) << "snap_id=" << m_current_snap_id << dendl;

    uint64_t num_objs = Striper::get_num_objects(m_image_ctx->layout, m_current_size);
    if (object_map.size() < num_objs) {
        ldout(cct, 1) << "object map too small: " << object_map.size() << " < " << num_objs << dendl;
        return -EINVAL;
    }
    ...
    uint64_t overlap = std::min(m_object_diff_state->size(), prev_object_diff_state_size);
    ...
    for (; ono < start_object_no + overlap; ++diff_it, ++ono) {
        ...
        ldout(cct, 20) << "object state: " << ono << " " << static_cast<uint32_t>(prev_object_diff_state) << "->"
                       << static_cast<uint32_t>(*diff_it) << " (" << static_cast<uint32_t>(object_map_state) << ")"
                       << dendl;
    }
    ldout(cct, 20) << "computed overlap diffs" << dendl;

    ceph_assert(diff_it == m_object_diff_state->end() || end_object_no <= num_objs);
    for (; ono < end_object_no; ++it, ++diff_it, ++ono) {
        ...
        ldout(cct, 20) << "object state: " << ono << " "
                       << "->" << static_cast<uint32_t>(*diff_it) << " (" << static_cast<uint32_t>(*it) << ")" << dendl;
    }
...
template<typename I> void DiffRequest<I>::send()
{
    auto cct = m_image_ctx->cct;
    ...
    if (is_diff_iterate() && m_snap_id_start == 0 && m_snap_id_end == m_image_ctx->snap_id &&
        ...
        if (r == 0) {
            r = m_image_ctx->object_map->with_object_map(
                [this](const BitVector<2>& object_map) { return process_object_map(object_map); });
        }
        image_locker.unlock();
...
template<typename I> void DiffRequest<I>::load_object_map(std::shared_lock<ceph::shared_mutex>* image_locker)
{
    ceph_assert(ceph_mutex_is_locked(m_image_ctx->image_lock));
    ...
    m_ignore_enoent = (m_current_snap_id != m_snap_id_start && m_current_snap_id != m_snap_id_end);
    ...
    std::string oid(ObjectMap<>::object_map_name(m_image_ctx->id, m_current_snap_id));

    librados::ObjectReadOperation op;
    cls_client::object_map_load_start(&op);

    m_out_bl.clear();
    auto aio_comp = create_rados_callback<DiffRequest<I>, &DiffRequest<I>::handle_load_object_map>(this);
    r = m_image_ctx->md_ctx.aio_operate(oid, aio_comp, &op, &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
}

template<typename I> void DiffRequest<I>::handle_load_object_map(int r)
{
    auto cct = m_image_ctx->cct;
...
template<typename I> void DiffRequest<I>::finish(int r)
{
    auto cct = m_image_ctx->cct;
    ldout(cct, 10) << "r=" << r << dendl;
