...
bool Request::should_complete(int r)
{
    ...
    switch (m_state) {
    case STATE_REQUEST:
        if (r == -ETIMEDOUT && !cct->_conf.get_val<bool>("rbd_invalidate_object_map_on_timeout")) {
    ...
    case STATE_INVALIDATE:
        ...
        if (r < 0) {
            lderr(cct) << "failed to invalidate object map: " << cpp_strerror(r) << dendl;
        }
        return true;
...
{
    bool flags_set;
    int r = m_image_ctx.test_flags(m_snap_id, RBD_FLAG_OBJECT_MAP_INVALID, &flags_set);
    if (r < 0 || flags_set) {
        return true;
    }
    ...
    InvalidateRequest<>* req = new InvalidateRequest<>(m_image_ctx, m_snap_id, true, create_callback_context());
    req->send();
    return false;
}
