...
const std::list<std::string> POOL_OBJECTS{
    RBD_CHILDREN, RBD_GROUP_DIRECTORY, RBD_INFO, RBD_MIRRORING, RBD_TASK, RBD_TRASH, RBD_DIRECTORY};

}   // anonymous namespace

template<typename I> int Namespace<I>::create(librados::IoCtx& io_ctx, const std::string& name)
{
    ...
    if (r < 0) {
        lderr(cct) << "failed to retrieve min OSD release: " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    r = cls_client::dir_state_set(&ns_ctx, RBD_DIRECTORY, cls::rbd::DIRECTORY_STATE_READY);
    if (r < 0) {
        lderr(cct) << "failed to initialize image directory: " << cpp_strerror(r) << dendl;
        goto rollback;
    }
...
template<typename I> int Namespace<I>::remove(librados::IoCtx& io_ctx, const std::string& name)
{
    CephContext* cct = (CephContext*)io_ctx.cct();
    ldout(cct, 5) << "name=" << name << dendl;
    ...
    librbd::cls_client::dir_state_set(&dir_op, cls::rbd::DIRECTORY_STATE_ADD_DISABLED);
    dir_op.remove();
    ...
    if (r == -EBUSY) {
        ...
        lderr(cct) << "failed to disable the namespace: " << cpp_strerror(r) << dendl;
        return r;
    }

    r = cls_client::trash_list(&ns_ctx, "", 1, &trash_entries);
    if (r < 0 && r != -ENOENT) {
        lderr(cct) << "failed to list trash directory: " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    for (auto& oid : POOL_OBJECTS) {
        r = ns_ctx.remove(oid);
        if (r < 0 && r != -ENOENT) {
            lderr(cct) << "failed to remove object '" << oid << "': " << cpp_strerror(r) << dendl;
            return r;
        }
    }
...
rollback:
    ...
    r = librbd::cls_client::dir_state_set(&ns_ctx, RBD_DIRECTORY, cls::rbd::DIRECTORY_STATE_READY);
    if (r < 0) {
        lderr(cct) << "failed to restore directory state: " << cpp_strerror(r) << dendl;
    }

    return -EBUSY;
}

template<typename I> int Namespace<I>::list(IoCtx& io_ctx, std::vector<std::string>* names)
{
    CephContext* cct = (CephContext*)io_ctx.cct();
    ldout(cct, 5) << dendl;
    ...
    do {
        ...
        r = cls_client::namespace_list(&default_ns_ctx, last_read, max_read, &name_list);
        if (r == -ENOENT) {
            return 0;
        }
        else if (r < 0) {
...
template<typename I> int Namespace<I>::exists(librados::IoCtx& io_ctx, const std::string& name, bool* exists)
{
    CephContext* cct = (CephContext*)io_ctx.cct();
    ldout(cct, 5) << "name=" << name << dendl;
    ...
    *exists = false;
    ...
    int r = librbd::cls_client::dir_state_assert(&ns_ctx, RBD_DIRECTORY, cls::rbd::DIRECTORY_STATE_READY);
    if (r == 0) {
        *exists = true;
    }
    else if (r != -ENOENT) {
