...
inline bool operator==(const linked_image_spec_t& rhs, const linked_image_spec_t& lhs)
{
    bool result =
        (rhs.pool_id == lhs.pool_id && rhs.pool_namespace == lhs.pool_namespace && rhs.image_id == lhs.image_id);
    return result;
}
...
{
public:
    MigrationProgressContext(librados::IoCtx& io_ctx, const std::string& header_oid, cls::rbd::MigrationState state,
        ...
        , m_lock(ceph::make_mutex(util::unique_lock_name("librbd::api::MigrationProgressContext", this)))
    {
...
private:
    ...
    void set_state_description()
    {
        ...
        librados::AioCompletion* comp = create_rados_callback<klass, &klass::handle_set_state_description>(this);
    ...
    void handle_set_state_description(int r)
    {
        ...
        if (r < 0) {
            lderr(m_cct) << "failed to update migration state: " << cpp_strerror(r) << dendl;
        }
...
int trash_search(librados::IoCtx& io_ctx, rbd_trash_image_source_t source, const std::string& image_name,
                 std::string* image_id)
{
...
template<typename I>
int open_images(librados::IoCtx& io_ctx, const std::string& image_name, I** src_image_ctx, I** dst_image_ctx,
                cls::rbd::MigrationSpec* src_migration_spec, cls::rbd::MigrationSpec* dst_migration_spec,
                bool skip_open_dst_image)
{
    ...
    *dst_image_ctx = nullptr;
    ...
    if (r == -ENOENT) {
        ...
        r = trash_search(io_ctx, RBD_TRASH_IMAGE_SOURCE_MIGRATION, image_name, &src_image_id);
        if (r < 0) {
            lderr(cct) << "failed to determine image id: " << cpp_strerror(r) << dendl;
            return r;
        }
    ...
    r = cls_client::migration_get(&image_ctx->md_ctx, image_ctx->header_oid, &migration_spec);
    if (r < 0) {
        lderr(cct) << "failed retrieving migration header: " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    if (migration_spec.header_type == cls::rbd::MIGRATION_HEADER_TYPE_SRC) {
        ...
        *src_migration_spec = migration_spec;
        ...
        if (!skip_open_dst_image) {
            ...
            if (r < 0) {
                image_ctx = nullptr;
                lderr(cct) << "failed to re-open destination image: " << cpp_strerror(r) << dendl;
                return r;
            }
        }
        ...
        *dst_migration_spec = migration_spec;
        ...
        lderr(cct) << "unexpected migration header type: " << migration_spec.header_type << dendl;
        r = -EINVAL;
        return r;
    }
    ...
    if (*src_image_ctx == nullptr && dst_migration_spec->source_spec.empty()) {
        r = util::create_ioctx(
            io_ctx, "source image", migration_spec.pool_id, migration_spec.pool_namespace, &other_io_ctx);
        if (r < 0) {
            return r;
        }
        ...
        if (other_image_id.empty()) {
            ldout(cct, 20) << "trying to open v1 image by name " << other_io_ctx.get_pool_name() << "/"
                           << other_image_name << dendl;
            flags |= OPEN_FLAG_OLD_FORMAT;
        }
        else {
            ldout(cct, 20) << "trying to open v2 image by id " << other_io_ctx.get_pool_name() << "/" << other_image_id
                           << dendl;
        }

        *src_image_ctx = I::create(other_image_name, other_image_id, nullptr, other_io_ctx, false);
    }
    else if (*dst_image_ctx == nullptr) {
        r = util::create_ioctx(
            io_ctx, "destination image", migration_spec.pool_id, migration_spec.pool_namespace, &other_io_ctx);
        if (r < 0) {
            return r;
        }
        ...
        *dst_image_ctx = I::create(other_image_name, other_image_id, nullptr, other_io_ctx, false);
    }

    if (other_image_ctx != nullptr) {
        r = (*other_image_ctx)->state->open(flags);
        if (r < 0) {
            lderr(cct) << "failed to open " << other_image_type << " image " << other_io_ctx.get_pool_name() << "/"
                       << (other_image_id.empty() ? other_image_name : other_image_id) << ": " << cpp_strerror(r)
                       << dendl;
        ...
        r = cls_client::migration_get(
            &(*other_image_ctx)->md_ctx, (*other_image_ctx)->header_oid, other_migration_spec);
        if (r < 0) {
            lderr(cct) << "failed retrieving migration header: " << cpp_strerror(r) << dendl;
            return r;
        }
...
public:
    ...
    {
        return m_progress_ctx->update_progress(object_number + (object_count * (m_current_step - 1)),
...
template<typename I>
int Migration<I>::prepare(librados::IoCtx& io_ctx, const std::string& image_name, librados::IoCtx& dest_io_ctx,
                          const std::string& dest_image_name_, ImageOptions& opts)
{
    CephContext* cct = reinterpret_cast<CephContext*>(io_ctx.cct());

    std::string dest_image_name = dest_image_name_.empty() ? image_name : dest_image_name_;

    ldout(cct, 10) << io_ctx.get_pool_name() << "/" << image_name << " -> " << dest_io_ctx.get_pool_name() << "/"
    ...
    int flags =
        librbd::image::LIST_WATCHERS_FILTER_OUT_MY_INSTANCE | librbd::image::LIST_WATCHERS_FILTER_OUT_MIRROR_INSTANCES;
    C_SaferCond on_list_watchers;
    auto list_watchers_request =
        librbd::image::ListWatchersRequest<I>::create(*src_image_ctx, flags, &watchers, &on_list_watchers);
    ...
    auto dst_image_ctx = I::create(dest_image_name, util::generate_image_id(dest_io_ctx), nullptr, dest_io_ctx, false);
    src_image_ctx->image_lock.lock_shared();
    cls::rbd::MigrationSpec dst_migration_spec{cls::rbd::MIGRATION_HEADER_TYPE_DST,
    ...
    Migration migration(src_image_ctx, dst_image_ctx, dst_migration_spec, opts, nullptr);
    r = migration.prepare();
...
template<typename I>
int Migration<I>::prepare_import(const std::string& source_spec, librados::IoCtx& dest_io_ctx,
                                 const std::string& dest_image_name, ImageOptions& opts)
{
    if (source_spec.empty() || !dest_io_ctx.is_valid() || dest_image_name.empty()) {
        return -EINVAL;
    }

    auto cct = reinterpret_cast<CephContext*>(dest_io_ctx.cct());
    ldout(cct, 10) << source_spec << " -> " << dest_io_ctx.get_pool_name() << "/" << dest_image_name
                   << ", opts=" << opts << dendl;

    I* src_image_ctx = nullptr;
    C_SaferCond open_ctx;
    auto req = migration::OpenSourceImageRequest<I>::create(
        dest_io_ctx, nullptr, CEPH_NOSNAP, {-1, "", "", "", source_spec, {}, 0, false}, &src_image_ctx, &open_ctx);
    req->send();
    ...
    if (image_format != 2) {
        lderr(cct) << "unsupported destination image format: " << image_format << dendl;
        return -EINVAL;
    }
...
template<typename I>
int Migration<I>::execute(librados::IoCtx& io_ctx, const std::string& image_name, ProgressContext& prog_ctx)
{
    CephContext* cct = reinterpret_cast<CephContext*>(io_ctx.cct());
    ...
    int r = open_images(
        io_ctx, image_name, &src_image_ctx, &dst_image_ctx, &src_migration_spec, &dst_migration_spec, false);
    if (r < 0) {
        return r;
    }
    ...
    if (r < 0) {
        lderr(cct) << "failed to refresh destination image: " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    if (dst_migration_spec.state != cls::rbd::MIGRATION_STATE_PREPARED &&
        ...
        lderr(cct) << "current migration state is '" << dst_migration_spec.state << "' (should be 'prepared')" << dendl;
        return -EINVAL;
    }
    ...
    if (!dst_migration_spec.source_spec.empty()) {
        ...
        *_dout << src_image_ctx->md_ctx.get_pool_name() << "/" << src_image_ctx->name;
    }
    *_dout << " -> " << dst_image_ctx->md_ctx.get_pool_name() << "/" << dst_image_ctx->name << dendl;

    ImageOptions opts;
    Migration migration(src_image_ctx, dst_image_ctx, dst_migration_spec, opts, &prog_ctx);
    r = migration.execute();
    if (r < 0) {
        return r;
    }
...
template<typename I>
int Migration<I>::abort(librados::IoCtx& io_ctx, const std::string& image_name, ProgressContext& prog_ctx)
{
    CephContext* cct = reinterpret_cast<CephContext*>(io_ctx.cct());
    ...
    cls::rbd::MigrationSpec dst_migration_spec;
    int r =
        open_images(io_ctx, image_name, &src_image_ctx, &dst_image_ctx, &src_migration_spec, &dst_migration_spec, true);
    if (r < 0) {
        return r;
    }
...
template<typename I>
int Migration<I>::commit(librados::IoCtx& io_ctx, const std::string& image_name, ProgressContext& prog_ctx)
{
    CephContext* cct = reinterpret_cast<CephContext*>(io_ctx.cct());
    ...
    if (dst_migration_spec.state != cls::rbd::MIGRATION_STATE_EXECUTED) {
        lderr(cct) << "current migration state is '" << dst_migration_spec.state << "' (should be 'executed')" << dendl;
        dst_image_ctx->state->close();
        if (src_image_ctx != nullptr) {
            src_image_ctx->state->close();
        }
...
template<typename I>
int Migration<I>::status(librados::IoCtx& io_ctx, const std::string& image_name, image_migration_status_t* status)
{
    CephContext* cct = reinterpret_cast<CephContext*>(io_ctx.cct());
...
template<typename I> int Migration<I>::get_source_spec(I* image_ctx, std::string* source_spec)
{
    ...
    if (migration_info.empty()) {
        ...
        cls::rbd::MigrationSpec migration_spec;
        int r = cls_client::migration_get(&image_ctx->md_ctx, image_ctx->header_oid, &migration_spec);
        if (r == -ENOENT) {
            return r;
        }
        else if (r < 0) {
        ...
        migration_info = {migration_spec.pool_id,
    ...
    if (!migration_info.source_spec.empty()) {
        ...
        *source_spec = migration::NativeFormat<I>::build_source_spec(
            migration_info.pool_id, migration_info.pool_namespace, migration_info.image_name, migration_info.image_id);
    }
...
template<typename I>
Migration<I>::Migration(ImageCtx* src_image_ctx, ImageCtx* dst_image_ctx,
                        const cls::rbd::MigrationSpec& dst_migration_spec, ImageOptions& opts,
                        ProgressContext* prog_ctx)
    ...
    , m_prog_ctx(prog_ctx)
    , m_src_migration_spec(cls::rbd::MIGRATION_HEADER_TYPE_SRC, m_dst_io_ctx.get_id(), m_dst_io_ctx.get_namespace(),
                           m_dst_image_name, m_dst_image_id, "", {}, 0, m_mirroring, m_mirror_image_mode, m_flatten,
                           dst_migration_spec.state, dst_migration_spec.state_description)
    , m_dst_migration_spec(dst_migration_spec)
{
    m_dst_io_ctx.dup(dst_image_ctx->md_ctx);
}

template<typename I> int Migration<I>::prepare()
{
...
template<typename I> int Migration<I>::prepare_import()
{
    ldout(m_cct, 10) << dendl;
...
template<typename I> int Migration<I>::execute()
{
    ldout(m_cct, 10) << dendl;
...

    {
        MigrationProgressContext dst_prog_ctx(
            m_dst_image_ctx->md_ctx, m_dst_image_ctx->header_oid, cls::rbd::MIGRATION_STATE_EXECUTING, m_prog_ctx);
        std::optional<MigrationProgressContext> src_prog_ctx;
        if (m_src_image_ctx != nullptr) {
            src_prog_ctx.emplace(m_src_image_ctx->md_ctx,
        ...
        while (true) {
            r = m_dst_image_ctx->operations->migrate(*(src_prog_ctx ? &src_prog_ctx.value() : &dst_prog_ctx));
            if (r == -EROFS) {
                std::shared_lock owner_locker{m_dst_image_ctx->owner_lock};
                if (m_dst_image_ctx->exclusive_lock != nullptr && !m_dst_image_ctx->exclusive_lock->accept_ops()) {
...
template<typename I> int Migration<I>::abort()
{
    ldout(m_cct, 10) << dendl;
    ...
    if (m_src_image_ctx != nullptr) {
        ...
        if (m_src_image_ctx->exclusive_lock != nullptr && !m_src_image_ctx->exclusive_lock->is_lock_owner()) {
            C_SaferCond ctx;
            m_src_image_ctx->exclusive_lock->acquire_lock(&ctx);
            m_src_image_ctx->owner_lock.unlock_shared();
            r = ctx.wait();
            if (r < 0) {
                lderr(m_cct) << "error acquiring exclusive lock: " << cpp_strerror(r) << dendl;
                return r;
            }
        }
        else {
    ...
    if (r < 0) {
        ...
        C_SaferCond on_list_watchers;
        auto list_watchers_request =
            librbd::image::ListWatchersRequest<I>::create(*m_dst_image_ctx, flags, &watchers, &on_list_watchers);
        list_watchers_request->send();
        r = on_list_watchers.wait();
        if (r < 0) {
        ...
        SteppedProgressContext progress_ctx(m_prog_ctx, (m_src_image_ctx != nullptr ? 2 : 1));
        if (m_src_image_ctx != nullptr) {
        ...
        for (auto& snap : snaps) {
            ...
            int r = Snapshot<I>::remove(m_dst_image_ctx, snap.name.c_str(), RBD_SNAP_REMOVE_UNPROTECT, prog_ctx);
            if (r < 0) {
                lderr(m_cct) << "failed removing snapshot: " << cpp_strerror(r) << dendl;
                return r;
            }
        }
        ...
        auto req = librbd::image::RemoveRequest<>::create(
            dst_io_ctx, m_dst_image_ctx, false, false, progress_ctx, asio_engine->get_work_queue(), &on_remove);
        req->send();
        r = on_remove.wait();
        ...
        if (r < 0) {
            lderr(m_cct) << "failed removing destination image '" << dst_io_ctx.get_pool_name() << "/"
                         << m_dst_image_name << " (" << m_dst_image_id << ")': " << cpp_strerror(r) << dendl;
            return r;
        }
    }
...
template<typename I> int Migration<I>::commit()
{
    ldout(m_cct, 10) << dendl;
...
template<typename I> int Migration<I>::status(image_migration_status_t* status)
{
    ldout(m_cct, 10) << dendl;
    ...
    case cls::rbd::MIGRATION_STATE_ERROR: status->state = RBD_IMAGE_MIGRATION_STATE_ERROR; break;
    case cls::rbd::MIGRATION_STATE_PREPARING: status->state = RBD_IMAGE_MIGRATION_STATE_PREPARING; break;
    case cls::rbd::MIGRATION_STATE_PREPARED: status->state = RBD_IMAGE_MIGRATION_STATE_PREPARED; break;
    case cls::rbd::MIGRATION_STATE_EXECUTING: status->state = RBD_IMAGE_MIGRATION_STATE_EXECUTING; break;
    case cls::rbd::MIGRATION_STATE_EXECUTED: status->state = RBD_IMAGE_MIGRATION_STATE_EXECUTED; break;
    default: status->state = RBD_IMAGE_MIGRATION_STATE_UNKNOWN; break;
    }
...
template<typename I>
int Migration<I>::set_state(I* image_ctx, const std::string& image_description, cls::rbd::MigrationState state,
                            const std::string& description)
{
    int r = cls_client::migration_set_state(&image_ctx->md_ctx, image_ctx->header_oid, state, description);
    if (r < 0) {
...
template<typename I> int Migration<I>::set_state(cls::rbd::MigrationState state, const std::string& description)
{
    int r;
    if (m_src_image_ctx != nullptr) {
...
template<typename I> int Migration<I>::list_src_snaps(I* image_ctx, std::vector<librbd::snap_info_t>* snaps)
{
    ldout(m_cct, 10) << dendl;
    ...
    for (auto& snap : *snaps) {
        ...
        if (r < 0) {
            lderr(m_cct) << "error getting snap namespace type: " << cpp_strerror(r) << dendl;
            return r;
        }
        ...
        if (namespace_type != RBD_SNAP_NAMESPACE_TYPE_USER) {
            if (namespace_type == RBD_SNAP_NAMESPACE_TYPE_TRASH) {
                lderr(m_cct) << "image has snapshots with linked clones that must be "
                             << "deleted or flattened before the image can be migrated" << dendl;
            }
            else {
...
template<typename I> int Migration<I>::validate_src_snaps(I* image_ctx)
{
    ldout(m_cct, 10) << dendl;
    ...
    for (auto& snap : snaps) {
        ...
        cls::rbd::ParentImageSpec parent_spec{
            image_ctx->md_ctx.get_id(), image_ctx->md_ctx.get_namespace(), image_ctx->id, snap.id};
...
template<typename I> int Migration<I>::set_src_migration(I* image_ctx)
{
    ldout(m_cct, 10) << dendl;

    image_ctx->ignore_migrating = true;

    int r = cls_client::migration_set(&image_ctx->md_ctx, image_ctx->header_oid, m_src_migration_spec);
    if (r < 0) {
        lderr(m_cct) << "failed to set source migration header: " << cpp_strerror(r) << dendl;
        return r;
    }
...
template<typename I> int Migration<I>::remove_migration(I* image_ctx)
{
    ldout(m_cct, 10) << dendl;
    ...
    if (r < 0) {
        lderr(m_cct) << "failed removing migration header: " << cpp_strerror(r) << dendl;
        return r;
    }
...
template<typename I> int Migration<I>::unlink_src_image(I* image_ctx)
{
...
template<typename I> int Migration<I>::v1_unlink_src_image(I* image_ctx)
{
    ldout(m_cct, 10) << dendl;
    ...
    if (r < 0) {
        lderr(m_cct) << "failed removing " << image_ctx->name << " from tmap: " << cpp_strerror(r) << dendl;
        return r;
    }
...
template<typename I> int Migration<I>::v2_unlink_src_image(I* image_ctx)
{
    ldout(m_cct, 10) << dendl;

    image_ctx->owner_lock.lock_shared();
    if (image_ctx->exclusive_lock != nullptr && image_ctx->exclusive_lock->is_lock_owner()) {
        C_SaferCond ctx;
        image_ctx->exclusive_lock->release_lock(&ctx);
        image_ctx->owner_lock.unlock_shared();
        int r = ctx.wait();
        if (r < 0) {
            lderr(m_cct) << "error releasing exclusive lock: " << cpp_strerror(r) << dendl;
            return r;
        }
    }
    else {
    ...
    int r = Trash<I>::move(image_ctx->md_ctx, RBD_TRASH_IMAGE_SOURCE_MIGRATION, image_ctx->name, 0);
    if (r < 0) {
        lderr(m_cct) << "failed moving image to trash: " << cpp_strerror(r) << dendl;
        return r;
    }
...
template<typename I> int Migration<I>::relink_src_image(I* image_ctx)
{
    if (image_ctx->old_format) {
...
template<typename I> int Migration<I>::v1_relink_src_image(I* image_ctx)
{
    ldout(m_cct, 10) << dendl;
    ...
    if (r < 0) {
        lderr(m_cct) << "failed adding " << image_ctx->name << " to tmap: " << cpp_strerror(r) << dendl;
        return r;
    }
...
template<typename I> int Migration<I>::v2_relink_src_image(I* image_ctx)
{
    ldout(m_cct, 10) << dendl;

    std::shared_lock image_locker{image_ctx->image_lock};
    int r =
        Trash<I>::restore(image_ctx->md_ctx, {cls::rbd::TRASH_IMAGE_SOURCE_MIGRATION}, image_ctx->id, image_ctx->name);
    if (r < 0) {
        lderr(m_cct) << "failed restoring image from trash: " << cpp_strerror(r) << dendl;
        return r;
    }
...
template<typename I> int Migration<I>::create_dst_image(I** image_ctx)
{
    ldout(m_cct, 10) << dendl;
    ...
    if (m_image_options.get(RBD_IMAGE_OPTION_MIRROR_IMAGE_MODE, &mirror_image_mode) == 0) {
        m_mirroring = true;
        m_mirror_image_mode = static_cast<cls::rbd::MirrorImageMode>(mirror_image_mode);
    ...
    if (parent_spec.pool_id == -1) {
        auto* req = image::CreateRequest<I>::create(config,
                                                    m_dst_io_ctx,
                                                    m_dst_image_name,
                                                    m_dst_image_id,
                                                    size,
                                                    ...
                                                    "",
                                                    "",
                                                    m_src_image_ctx->op_work_queue,
                                                    &on_create);
        req->send();
    }
    else {
        r = util::create_ioctx(
            m_src_image_ctx->md_ctx, "parent image", parent_spec.pool_id, parent_spec.pool_namespace, &parent_io_ctx);
        if (r < 0) {
            return r;
        }

        auto* req = image::CloneRequest<I>::create(config,
                                                   parent_io_ctx,
                                                   parent_spec.image_id,
                                                   "",
                                                   {},
                                                   parent_spec.snap_id,
                                                   m_dst_io_ctx,
    ...
    *image_ctx = nullptr;   // prevent prepare from cleaning up the ImageCtx
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to open newly created header: " << cpp_strerror(r) << dendl;
        return r;
    }
...

    {
        ...
        r = dst_image_ctx->operations->prepare_image_update(exclusive_lock::OPERATION_REQUEST_TYPE_GENERAL, true);
        if (r < 0) {
    ...
    auto snapshot_copy_req = librbd::deep_copy::SnapshotCopyRequest<I>::create(m_src_image_ctx,
                                                                               dst_image_ctx,
                                                                               0,
                                                                               CEPH_NOSNAP,
                                                                               0,
    ...
    if (!m_src_image_ctx->header_oid.empty()) {
        ...
        auto metadata_copy_req =
            librbd::deep_copy::MetadataCopyRequest<I>::create(m_src_image_ctx, dst_image_ctx, &on_metadata_copy);
    ...
    r = cls_client::migration_set(&m_dst_io_ctx, m_dst_header_oid, m_dst_migration_spec);
    if (r < 0) {
        lderr(m_cct) << "failed to set migration header: " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    if (m_dst_migration_spec.source_spec.empty()) {
        ...
        r = set_state(m_src_image_ctx, "source", cls::rbd::MIGRATION_STATE_PREPARED, "");
        if (r < 0) {
            return r;
        }
    }

    r = set_state(dst_image_ctx, "destination", cls::rbd::MIGRATION_STATE_PREPARED, "");
    if (r < 0) {
        return r;
    }
    ...
    if (m_dst_migration_spec.source_spec.empty()) {
        r = dst_image_ctx->state->refresh();
        if (r < 0) {
            lderr(m_cct) << "failed to refresh destination image: " << cpp_strerror(r) << dendl;
            return r;
        }
...
template<typename I> int Migration<I>::remove_group(I* image_ctx, group_info_t* group_info)
{
    ...
    IoCtx group_ioctx;
    r = util::create_ioctx(image_ctx->md_ctx, "group", group_info->pool, {}, &group_ioctx);
    if (r < 0) {
        return r;
    }

    r = librbd::api::Group<I>::image_remove_by_id(
        group_ioctx, group_info->name.c_str(), image_ctx->md_ctx, image_ctx->id.c_str());
    if (r < 0) {
        lderr(m_cct) << "failed to remove image from group: " << cpp_strerror(r) << dendl;
        return r;
    }
...
template<typename I> int Migration<I>::add_group(I* image_ctx, group_info_t& group_info)
{
    if (group_info.pool == -1) {
        return 0;
    }
    ...
    IoCtx group_ioctx;
    int r = util::create_ioctx(image_ctx->md_ctx, "group", group_info.pool, {}, &group_ioctx);
    if (r < 0) {
        return r;
    }

    r = librbd::api::Group<I>::image_add(
        group_ioctx, group_info.name.c_str(), image_ctx->md_ctx, image_ctx->name.c_str());
    if (r < 0) {
        lderr(m_cct) << "failed to add image to group: " << cpp_strerror(r) << dendl;
        return r;
    }
...
template<typename I> int Migration<I>::update_group(I* from_image_ctx, I* to_image_ctx)
{
    ldout(m_cct, 10) << dendl;
...
template<typename I>
int Migration<I>::disable_mirroring(I* image_ctx, bool* was_enabled, cls::rbd::MirrorImageMode* mirror_image_mode)
{
    *was_enabled = false;

    cls::rbd::MirrorImage mirror_image;
    int r = cls_client::mirror_image_get(&image_ctx->md_ctx, image_ctx->id, &mirror_image);
    if (r == -ENOENT) {
    ...
    if (r < 0) {
        lderr(m_cct) << "failed to retrieve mirror image: " << cpp_strerror(r) << dendl;
        return r;
    }
...
template<typename I>
int Migration<I>::enable_mirroring(I* image_ctx, bool was_enabled, cls::rbd::MirrorImageMode mirror_image_mode)
{
    cls::rbd::MirrorMode mirror_mode;
    int r = cls_client::mirror_mode_get(&image_ctx->md_ctx, &mirror_mode);
    if (r < 0 && r != -ENOENT) {
        lderr(m_cct) << "failed to retrieve mirror mode: " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    C_SaferCond ctx;
    auto req = mirror::EnableRequest<I>::create(image_ctx, mirror_image_mode, "", false, &ctx);
    req->send();
    r = ctx.wait();
    if (r < 0) {
...
template<typename I> int Migration<I>::relink_children(I* from_image_ctx, I* to_image_ctx)
{
    ldout(m_cct, 10) << dendl;
    ...
    std::vector<librbd::snap_info_t> snaps;
    int r = list_src_snaps(migration_abort ? to_image_ctx : from_image_ctx, &snaps);
    if (r < 0) {
        return r;
    }
    ...
    for (auto it = snaps.begin(); it != snaps.end(); it++) {
        ...
        if (from_image_ctx != m_src_image_ctx) {
            ...
            cls::rbd::ParentImageSpec src_parent_spec{
                to_image_ctx->md_ctx.get_id(), to_image_ctx->md_ctx.get_namespace(), to_image_ctx->id, snap.id};
            r = api::Image<I>::list_children(to_image_ctx, src_parent_spec, &src_child_images);
            if (r < 0) {
                lderr(m_cct) << "failed listing children: " << cpp_strerror(r) << dendl;
                return r;
            }
            ...
            snap.id = from_image_ctx->get_snap_id(cls::rbd::UserSnapshotNamespace(), snap.name);
        ...
        std::vector<librbd::linked_image_spec_t> child_images;
        {
            std::shared_lock image_locker{from_image_ctx->image_lock};
            cls::rbd::ParentImageSpec parent_spec{
                from_image_ctx->md_ctx.get_id(), from_image_ctx->md_ctx.get_namespace(), from_image_ctx->id, snap.id};
            r = api::Image<I>::list_children(from_image_ctx, parent_spec, &child_images);
            if (r < 0) {
                lderr(m_cct) << "failed listing children: " << cpp_strerror(r) << dendl;
                return r;
            }
        ...
        for (auto& child_image : child_images) {
            r = relink_child(from_image_ctx, to_image_ctx, snap, child_image, migration_abort, true);
            if (r < 0) {
                return r;
            }

            src_child_images.erase(std::remove(src_child_images.begin(), src_child_images.end(), child_image),
...
template<typename I>
int Migration<I>::relink_child(I* from_image_ctx, I* to_image_ctx, const librbd::snap_info_t& from_snap,
                               const librbd::linked_image_spec_t& child_image, bool migration_abort,
                               bool reattach_child)
{
    ldout(m_cct, 10) << from_snap.name << " " << child_image.pool_name << "/" << child_image.pool_namespace << "/"
                     << child_image.image_name << " (migration_abort=" << migration_abort
                     << ", reattach_child=" << reattach_child << ")" << dendl;

    librados::snap_t to_snap_id;
    {
        std::shared_lock image_locker{to_image_ctx->image_lock};
        to_snap_id = to_image_ctx->get_snap_id(cls::rbd::UserSnapshotNamespace(), from_snap.name);
        if (to_snap_id == CEPH_NOSNAP) {
            lderr(m_cct) << "no snapshot " << from_snap.name << " on destination image" << dendl;
            return -ENOENT;
        }
    }
    ...
    I* child_image_ctx = I::create("", child_image.image_id, nullptr, child_io_ctx, false);
    ...
    if (migration_abort && parent_spec.pool_id == to_image_ctx->md_ctx.get_id() &&
        ...
        parent_spec.image_id == to_image_ctx->id && parent_spec.snap_id == to_snap_id) {
        ...
        if (parent_spec.pool_id != from_image_ctx->md_ctx.get_id() ||
            ...
            parent_spec.image_id != from_image_ctx->id || parent_spec.snap_id != from_snap.id) {
            lderr(m_cct) << "parent is not source image: " << parent_spec.pool_id << "/" << parent_spec.pool_namespace
                         << "/" << parent_spec.image_id << "@" << parent_spec.snap_id << dendl;
            return -ESTALE;
        }
    ...
    if (reattach_child) {
        ...
        auto reattach_child_req = image::AttachChildRequest<I>::create(
            child_image_ctx, to_image_ctx, to_snap_id, from_image_ctx, from_snap.id, clone_format, &on_reattach_child);
...
template<typename I> int Migration<I>::remove_src_image(I** image_ctx)
{
    ldout(m_cct, 10) << dendl;
    ...
    for (auto it = snaps.rbegin(); it != snaps.rend(); it++) {
        ...
        librbd::NoOpProgressContext prog_ctx;
        int r = Snapshot<I>::remove(src_image_ctx, snap.name.c_str(), RBD_SNAP_REMOVE_UNPROTECT, prog_ctx);
        if (r < 0) {
            lderr(m_cct) << "failed removing source image snapshot '" << snap.name << "': " << cpp_strerror(r) << dendl;
            return r;
        }
    }
    ...
    auto req = librbd::image::RemoveRequest<I>::create(
        src_io_ctx, src_image_ctx, false, true, *m_prog_ctx, asio_engine->get_work_queue(), &on_remove);
    req->send();
    r = on_remove.wait();
    ...
    *image_ctx = nullptr;
    ...
    if (r < 0 && r != -ENOENT) {
        lderr(m_cct) << "failed removing source image: " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    if (!src_image_id.empty()) {
        ...
        if (r < 0 && r != -ENOENT) {
            lderr(m_cct) << "error removing image " << src_image_id << " from rbd_trash object" << dendl;
        }
    }
...
template<typename I> int Migration<I>::revert_data(I* src_image_ctx, I* dst_image_ctx, ProgressContext* prog_ctx)
{
    ldout(m_cct, 10) << dendl;

    cls::rbd::MigrationSpec migration_spec;
    int r = cls_client::migration_get(&src_image_ctx->md_ctx, src_image_ctx->header_oid, &migration_spec);

    if (r < 0) {
        lderr(m_cct) << "failed retrieving migration header: " << cpp_strerror(r) << dendl;
        return r;
    }

    if (migration_spec.header_type != cls::rbd::MIGRATION_HEADER_TYPE_DST) {
        lderr(m_cct) << "unexpected migration header type: " << migration_spec.header_type << dendl;
        return -EINVAL;
    }
    ...
    auto request = deep_copy::ImageCopyRequest<I>::create(src_image_ctx,
                                                          dst_image_ctx,
                                                          src_snap_id_start,
                                                          src_snap_id_end,
                                                          dst_snap_id_start,
