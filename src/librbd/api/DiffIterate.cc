...
template<typename I> int DiffIterate<I>::execute()
{
    ...
    if (m_whole_object) {
        C_SaferCond ctx;
        auto req =
            object_map::DiffRequest<I>::create(&m_image_ctx, from_snap_id, end_snap_id, &object_diff_state, &ctx);
        req->send();
        ...
        if (r < 0) {
            ...
            if (m_include_parent && from_snap_id == 0) {
                ...
                if (overlap.first > 0 && overlap.second == io::ImageArea::DATA) {
                    ...
                    DiffIterate diff_parent(
                        *m_image_ctx.parent, {}, nullptr, 0, overlap.first, true, true, &simple_diff_cb, &parent_diff);
