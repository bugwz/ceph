...
struct DiffContext
{
    ...
    template<typename I>
    DiffContext(I& image_ctx, DiffIterate<>::Callback callback, void* callback_arg, bool _whole_object,
                bool _include_parent, uint64_t _from_snap_id, uint64_t _end_snap_id)
        : callback(callback)
        , callback_arg(callback_arg)
        , whole_object(_whole_object)
...
template<typename I> class C_DiffObject : public Context
{
public:
    C_DiffObject(I& image_ctx, DiffContext& diff_context, uint64_t image_offset, uint64_t image_length)
        : m_image_ctx(image_ctx)
        , m_cct(image_ctx.cct)
        , m_diff_context(diff_context)
    ...
    void send()
    {
        ...
        auto aio_comp =
            io::AioCompletion::create_and_start(ctx, util::get_image_ctx(&m_image_ctx), io::AIO_TYPE_GENERIC);
        ...
        auto req = io::ImageDispatchSpec::create_list_snaps(m_image_ctx,
                                                            io::IMAGE_DISPATCH_LAYER_INTERNAL_START,
                                                            aio_comp,
                                                            {{m_image_offset, m_image_length}},
...
protected:
    ...
    void finish(int r) override
    {
        ...
        if (r < 0) {
            ldout(cct, 20) << "list_snaps failed: " << m_image_offset << "~" << m_image_length << ": "
                           << cpp_strerror(r) << dendl;
        }

        Diffs diffs;
        ldout(cct, 20) << "image extent " << m_image_offset << "~" << m_image_length << ": list_snaps complete"
                       << dendl;

        compute_diffs(&diffs);
        for (Diffs::const_iterator d = diffs.begin(); d != diffs.end(); ++d) {
            r = m_diff_context.callback(d->get<0>(), d->get<1>(), d->get<2>(), m_diff_context.callback_arg);
            if (r < 0) {
                break;
            }
        }
...
private:
    ...
    void compute_diffs(Diffs* diffs)
    {
        ...
        for (auto& [key, snapshot_extents] : m_snapshot_delta) {
            for (auto& snapshot_extent : snapshot_extents) {
                ...
                if ((state == io::SPARSE_EXTENT_STATE_DNE) ||
                    (key == io::INITIAL_WRITE_READ_SNAP_IDS && state == io::SPARSE_EXTENT_STATE_ZEROED)) {
                    continue;
                }

                aggregate_snapshot_extents.insert(
                    snapshot_extent.get_off(), snapshot_extent.get_len(), {state, snapshot_extent.get_len()});
            }
        }
        ...
        for (auto& snapshot_extent : aggregate_snapshot_extents) {
            ...
            diffs->emplace_back(snapshot_extent.get_off(),
                                snapshot_extent.get_len(),
                                snapshot_extent.get_val().state == io::SPARSE_EXTENT_STATE_DATA);
        }
    }
};

template<typename I> bool should_try_acquire_lock(I* image_ctx)
{
...
template<typename I>
int DiffIterate<I>::diff_iterate(I* ictx, const cls::rbd::SnapshotNamespace& from_snap_namespace,
                                 const char* fromsnapname, uint64_t off, uint64_t len, bool include_parent,
                                 bool whole_object, int (*cb)(uint64_t, size_t, int, void*), void* arg)
{
    ldout(ictx->cct, 10) << "from_snap_namespace=" << from_snap_namespace << ", fromsnapname=" << (fromsnapname ?: "")
                         << ", off=" << off << ", len=" << len << ", include_parent=" << include_parent
                         << ", whole_object=" << whole_object << dendl;
    ...
    C_SaferCond flush_ctx;
    {
        ...
        auto req = io::ImageDispatchSpec::create_flush(
            *ictx, io::IMAGE_DISPATCH_LAYER_INTERNAL_START, aio_comp, io::FLUSH_SOURCE_INTERNAL, {});
        req->send();
    }
...

    {
        ...
        if (fromsnapname == nullptr && whole_object && should_try_acquire_lock(ictx)) {
    ...
    DiffIterate command(*ictx, from_snap_namespace, fromsnapname, off, len, include_parent, whole_object, cb, arg);
    r = command.execute();
    return r;
}

template<typename I> std::pair<uint64_t, uint64_t> DiffIterate<I>::calc_object_diff_range()
{
    ...
    if (first_period_off != last_period_off) {
        ...
        Striper::file_to_extents(m_image_ctx.cct,
                                 &m_image_ctx.layout,
                                 m_offset,
        ...
        Striper::file_to_extents(m_image_ctx.cct, &m_image_ctx.layout, m_offset, m_length, 0, 0, &object_extents);
    }
    return {object_extents.front().object_no, object_extents.back().object_no + 1};
}

template<typename I> int DiffIterate<I>::execute()
{
    ...
    if (m_whole_object) {
        ...
        auto req = object_map::DiffRequest<I>::create(
            &m_image_ctx, from_snap_id, end_snap_id, start_object_no, end_object_no, &object_diff_state, &ctx);
        req->send();
        r = ctx.wait();
        if (r < 0) {
    ...
    ldout(cct, 5) << "diff_iterate from " << from_snap_id << " to " << end_snap_id << " size from " << from_size
                  << " to " << end_size << dendl;
    DiffContext diff_context(
        m_image_ctx, m_callback, m_callback_arg, m_whole_object, m_include_parent, from_snap_id, end_snap_id);
    ...
    while (left > 0) {
        ...
        if (fast_diff_enabled) {
            ...
            Striper::file_to_extents(
                cct, m_image_ctx.format_string, &m_image_ctx.layout, off, read_len, 0, object_extents, 0);
            ...
            for (auto& [object, extents] : object_extents) {
                ...
                ldout(cct, 20) << "object " << object << ": diff_state=" << (int)diff_state << dendl;

                if (diff_state == object_map::DIFF_STATE_HOLE && from_snap_id == 0 && !parent_diff.empty()) {
                    ...
                    for (auto& oe : extents) {
                        for (auto& be : oe.buffer_extents) {
                            ...
                            for (auto e = o.begin(); e != o.end(); ++e) {
                                aggregate_sparse_extents.insert(
                                    e.get_start(), e.get_len(), {io::SPARSE_EXTENT_STATE_DATA, e.get_len()});
                            }
                        }
                    }
                }
                    ...
                    auto state = (diff_state == object_map::DIFF_STATE_HOLE_UPDATED ? io::SPARSE_EXTENT_STATE_ZEROED
                                                                                    : io::SPARSE_EXTENT_STATE_DATA);
                    for (auto& oe : extents) {
                        for (auto& be : oe.buffer_extents) {
                            aggregate_sparse_extents.insert(off + be.first, be.second, {state, be.second});
                        }
                    }
                }
            }
            ...
            for (const auto& se : aggregate_sparse_extents) {
                ldout(cct, 20) << "off=" << se.get_off() << ", len=" << se.get_len() << ", state=" << se.get_val().state
                               << dendl;
                r = m_callback(
                    se.get_off(), se.get_len(), se.get_val().state == io::SPARSE_EXTENT_STATE_DATA, m_callback_arg);
                if (r < 0) {
                    return r;
                }
            }
            ...
            auto diff_object = new C_DiffObject<I>(m_image_ctx, diff_context, off, read_len);
            diff_object->send();
