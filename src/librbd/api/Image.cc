...
bool compare_by_pool(const librbd::linked_image_spec_t& lhs, const librbd::linked_image_spec_t& rhs)
{
...
bool compare(const librbd::linked_image_spec_t& lhs, const librbd::linked_image_spec_t& rhs)
{
...
template<typename I> int pre_remove_image(librados::IoCtx& io_ctx, const std::string& image_id)
{
...
template<typename I> int64_t Image<I>::get_data_pool_id(I* ictx)
{
...
template<typename I> int Image<I>::get_op_features(I* ictx, uint64_t* op_features)
{
    CephContext* cct = ictx->cct;
...
template<typename I> int Image<I>::list_images(librados::IoCtx& io_ctx, std::vector<image_spec_t>* images)
{
...
template<typename I> int Image<I>::list_images_v2(librados::IoCtx& io_ctx, ImageNameToIds* images)
{
    ...
    do {
        ...
        r = cls_client::dir_list(&io_ctx, RBD_DIRECTORY, last_read, max_read, &images_page);
        if (r < 0 && r != -ENOENT) {
            lderr(cct) << "error listing image in directory: " << cpp_strerror(r) << dendl;
            return r;
        }
        else if (r == -ENOENT) {
            break;
        }
        for (map<string, string>::const_iterator it = images_page.begin(); it != images_page.end(); ++it) {
            images->insert(*it);
        }
...
template<typename I>
int Image<I>::get_parent(I* ictx, librbd::linked_image_spec_t* parent_image, librbd::snap_spec_t* parent_snap)
{
    auto cct = ictx->cct;
    ldout(cct, 20) << "image_ctx=" << ictx << dendl;
    ...
    if (parent->snap_id != CEPH_NOSNAP) {
        ...
        if (snap_info == nullptr) {
            lderr(cct) << "error finding parent snap name: " << cpp_strerror(r) << dendl;
            return -ENOENT;
        }

        parent_snap->namespace_type =
            static_cast<snap_namespace_type_t>(cls::rbd::get_snap_namespace_type(snap_info->snap_namespace));
...
template<typename I> int Image<I>::list_children(I* ictx, std::vector<librbd::linked_image_spec_t>* images)
{
    images->clear();
...
template<typename I>
int Image<I>::list_children(I* ictx, const cls::rbd::ParentImageSpec& parent_spec,
...
template<typename I>
int Image<I>::list_descendants(librados::IoCtx& io_ctx, const std::string& image_id,
                               const std::optional<size_t>& max_level, std::vector<librbd::linked_image_spec_t>* images)
{
    ...
    if (r < 0) {
        ...
        lderr(cct) << "failed to open descendant " << image_id << " from pool " << io_ctx.get_pool_name() << ":"
    ...
    if (r1 < 0) {
        lderr(cct) << "error when closing descendant " << image_id << " from pool " << io_ctx.get_pool_name() << ":"
...
template<typename I>
int Image<I>::list_descendants(I* ictx, const std::optional<size_t>& max_level,
                               std::vector<librbd::linked_image_spec_t>* images)
{
    ...
    for (auto snap_id : snap_ids) {
        cls::rbd::ParentImageSpec parent_spec{ictx->md_ctx.get_id(), ictx->md_ctx.get_namespace(), ictx->id, snap_id};
...
template<typename I>
int Image<I>::list_descendants(I* ictx, const cls::rbd::ParentImageSpec& parent_spec,
                               const std::optional<size_t>& max_level, std::vector<librbd::linked_image_spec_t>* images)
{
    ...
    for (auto& it : pools) {
        ...
        if (r == -ENOENT) {
            ...
            lderr(cct) << "error retrieving base tier for pool " << it.second << dendl;
            return r;
        }
        ...
        IoCtx ioctx;
        r = librbd::util::create_ioctx(ictx->md_ctx, "child image", it.first, {}, &ioctx);
        if (r == -ENOENT) {
            continue;
        }
        else if (r < 0) {
        ...
        if (r < 0 && r != -ENOENT) {
            lderr(cct) << "error reading list of children from pool " << it.second << dendl;
            return r;
        }

        for (auto& image_id : image_ids) {
            images->push_back({it.first, "", ictx->md_ctx.get_namespace(), image_id, "", false});
    ...
    r = librbd::util::create_ioctx(
        ictx->md_ctx, "parent image", parent_spec.pool_id, parent_spec.pool_namespace, &parent_io_ctx);
    if (r < 0) {
        return r;
    }
    ...
    r = cls_client::children_list(
        &parent_io_ctx, librbd::util::header_name(parent_spec.image_id), parent_spec.snap_id, &child_images);
    ...
    for (auto& child_image : child_images) {
        images->push_back({child_image.pool_id, "", child_image.pool_namespace, child_image.image_id, "", false});
        if (!child_max_level || *child_max_level > 0) {
            IoCtx ioctx;
            r = librbd::util::create_ioctx(
                ictx->md_ctx, "child image", child_image.pool_id, child_image.pool_namespace, &ioctx);
            if (r == -ENOENT) {
                continue;
            }
            else if (r < 0) {
    ...
    for (auto& image : *images) {
        if (child_pool_id == -1 || child_pool_id != image.pool_id ||
            ...
            r = librbd::util::create_ioctx(
                ictx->md_ctx, "child image", image.pool_id, image.pool_namespace, &child_io_ctx);
            if (r == -ENOENT) {
                image.pool_name = "";
                image.image_name = "";
                continue;
            ...
            for (auto& it : trash_images) {
                child_image_id_to_info.insert(
                    {it.id, {it.name, it.source == RBD_TRASH_IMAGE_SOURCE_REMOVING ? false : true}});
            }
        }

        auto it = child_image_id_to_info.find(image.image_id);
        if (it == child_image_id_to_info.end()) {
            lderr(cct) << "error looking up name for image id " << image.image_id << " in pool "
                       << child_io_ctx.get_pool_name()
                       << (image.pool_namespace.empty() ? "" : "/" + image.pool_namespace) << dendl;
            return -ENOENT;
        }
...
template<typename I>
int Image<I>::deep_copy(I* src, librados::IoCtx& dest_md_ctx, const char* destname, ImageOptions& opts,
                        ProgressContext& prog_ctx)
{
    CephContext* cct = (CephContext*)dest_md_ctx.cct();
    ldout(cct, 20) << src->name << (src->snap_name.length() ? "@" + src->snap_name : "") << " -> " << destname
                   << " opts = " << opts << dendl;
    ...
    if (parent_spec.pool_id == -1) {
        ...
        r = librbd::util::create_ioctx(
            src->md_ctx, "parent image", parent_spec.pool_id, parent_spec.pool_namespace, &parent_io_ctx);
        if (r < 0) {
            return r;
        }
    ...
    C_SaferCond lock_ctx;
    {
        ...
        if (dest->exclusive_lock == nullptr || dest->exclusive_lock->is_lock_owner()) {
            lock_ctx.complete(0);
        }
        else {
...
template<typename I> int Image<I>::deep_copy(I* src, I* dest, bool flatten, ProgressContext& prog_ctx)
{
    ...
    auto req = DeepCopyRequest<I>::create(src,
                                          dest,
                                          snap_id_start,
                                          snap_id_end,
                                          0U,
...
template<typename I>
int Image<I>::snap_set(I* ictx, const cls::rbd::SnapshotNamespace& snap_namespace, const char* snap_name)
{
...
template<typename I> int Image<I>::snap_set(I* ictx, uint64_t snap_id)
{
...
template<typename I> int Image<I>::remove(IoCtx& io_ctx, const std::string& image_name, ProgressContext& prog_ctx)
{
    CephContext* cct((CephContext*)io_ctx.cct());
    ...
    if (r == -ENOENT) {
        ...
        for (auto& entry : trash_entries) {
            if (entry.name == image_name && entry.source == RBD_TRASH_IMAGE_SOURCE_REMOVING) {
                cls::rbd::TrashImageSpec spec;
                r = cls_client::trash_get(&io_ctx, entry.id, &spec);
                if (r < 0) {
                    lderr(cct) << "error getting image id " << entry.id << " info from trash: " << cpp_strerror(r)
                               << dendl;
                    return r;
                }
        ...
        rbd_trash_image_source_t trash_image_source = RBD_TRASH_IMAGE_SOURCE_REMOVING;
        ...
        if (config.get_val<bool>("rbd_move_to_trash_on_remove")) {
            ...
            expire_seconds = config.get_val<uint64_t>("rbd_move_to_trash_on_remove_expire_seconds");
        }
        else {
        ...
        r = Trash<I>::move(io_ctx, trash_image_source, image_name, image_id, expire_seconds);
        if (r >= 0) {
            if (trash_image_source == RBD_TRASH_IMAGE_SOURCE_REMOVING) {
                ...
                if (r == -ENOTEMPTY || r == -EBUSY || r == -EMLINK) {
                    ...
                    Trash<I>::restore(io_ctx, {cls::rbd::TRASH_IMAGE_SOURCE_REMOVING}, image_id, image_name);
                }
            }
            return r;
        }
    ...
    auto req = librbd::image::RemoveRequest<I>::create(
        io_ctx, image_name, "", false, false, prog_ctx, asio_engine.get_work_queue(), &cond);
    req->send();

    return cond.wait();
}

template<typename I> int Image<I>::flatten_children(I* ictx, const char* snap_name, ProgressContext& pctx)
{
    CephContext* cct = ictx->cct;
    ...
    for (auto& child_image : child_images) {
        ...
        ImageCtx* imctx = new ImageCtx("", child_image.image_id, nullptr, child_io_ctx, false);
        ...
        if ((imctx->features & RBD_FEATURE_DEEP_FLATTEN) == 0 && !imctx->snaps.empty()) {
            lderr(cct) << "snapshot in-use by " << pool << "/" << imctx->name << dendl;
            imctx->state->close();
            return -EBUSY;
        }
        ...
        if (r < 0) {
            lderr(cct) << "error flattening image: " << pool << "/"
                       << (child_image.pool_namespace.empty() ? "" : "/" + child_image.pool_namespace)
...
template<typename I>
int Image<I>::encryption_format(I* ictx, encryption_format_t format, encryption_options_t opts, size_t opts_size,
                                bool c_api)
{
    crypto::EncryptionFormat<I>* result_format;
    auto r = util::create_encryption_format(ictx->cct, format, opts, opts_size, c_api, &result_format);
    if (r != 0) {
        return r;
    }
...
template<typename I>
int Image<I>::encryption_load(I* ictx, const encryption_spec_t* specs, size_t spec_count, bool c_api)
{
    ...
    for (size_t i = 0; i < spec_count; ++i) {
        ...
        auto r = util::create_encryption_format(
            ictx->cct, specs[i].format, specs[i].opts, specs[i].opts_size, c_api, &result_format);
        if (r != 0) {
            return r;
        }
    ...
    C_SaferCond cond;
    auto req = librbd::crypto::LoadRequest<I>::create(ictx, std::move(formats), &cond);
    req->send();
    return cond.wait();
}
