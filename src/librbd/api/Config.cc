...
static std::set<std::string_view> EXCLUDE_OPTIONS{"rbd_auto_exclusive_lock_until_manual_request",
                                                  "rbd_default_format",
                                                  "rbd_default_pool",
...
static std::set<std::string_view> EXCLUDE_IMAGE_OPTIONS{"rbd_default_clone_format",
...
struct Options : Parent
{
    ...
    Options(librados::IoCtx& io_ctx, bool image_apply_only_options)
    {
        ...
        for (auto& pair : schema) {
            ...
            else if (image_apply_only_options && EXCLUDE_IMAGE_OPTIONS.count(pair.first) != 0) {
                continue;
            }
            else if (image_apply_only_options && boost::starts_with(pair.first, rbd_mirror_key_prefix)) {
                continue;
            }
    ...
    int init()
    {
        ...
        while (more_results) {
            ...
            int r = librbd::api::PoolMetadata<>::list(m_io_ctx, last_key, MAX_KEYS, &pairs);
            if (r < 0) {
                return r;
            }
            ...
            for (auto kv : pairs) {
                ...
                auto it = find(key);
                if (it != end()) {
                    it->second = {{kv.second.c_str(), kv.second.length()}, RBD_CONFIG_SOURCE_POOL};
                }
            }
        }
        return 0;
...
template<typename I> bool Config<I>::is_option_name(librados::IoCtx& io_ctx, const std::string& name)
{
...
template<typename I> int Config<I>::list(librados::IoCtx& io_ctx, std::vector<config_option_t>* options)
{
    Options opts(io_ctx, false);
...
template<typename I> bool Config<I>::is_option_name(I* image_ctx, const std::string& name)
{
...
template<typename I> int Config<I>::list(I* image_ctx, std::vector<config_option_t>* options)
{
    ...
    std::map<std::string, bufferlist> pairs;
    C_SaferCond ctx;
    auto req = image::GetMetadataRequest<I>::create(image_ctx->md_ctx,
    ...
    for (auto kv : pairs) {
        ...
        if (it != opts.end()) {
            it->second = {{kv.second.c_str(), kv.second.length()}, RBD_CONFIG_SOURCE_IMAGE};
        }
    }
...
template<typename I> void Config<I>::apply_pool_overrides(librados::IoCtx& io_ctx, ConfigProxy* config)
{
    ...
    int r = opts.init();
    if (r < 0) {
        lderr(cct) << "failed to read pool config overrides: " << cpp_strerror(r) << dendl;
        return;
    }
    ...
    for (auto& [k, v] : opts) {
        if (v.second == RBD_CONFIG_SOURCE_POOL) {
            ...
            if (r < 0) {
                lderr(cct) << "failed to override pool config " << k << "=" << v.first << ": " << cpp_strerror(r)
                           << dendl;
            }
        }
    }
