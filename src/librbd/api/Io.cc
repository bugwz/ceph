...
template<typename I> ssize_t Io<I>::write(I& image_ctx, uint64_t off, uint64_t len, bufferlist&& bl, int op_flags)
{
    ...
    int r = clip_io(util::get_image_ctx(&image_ctx), off, &len, io::ImageArea::DATA);
                      ...
                      int op_flags, bool native_async)
{
    ...
    auto req = io::ImageDispatchSpec::create_write(image_ctx,
                                                   io::IMAGE_DISPATCH_LAYER_API_START,
                                                   aio_comp,
                                                   {{off, len}},
                                                   io::ImageArea::DATA,
                        ...
                        uint32_t discard_granularity_bytes, bool native_async)
{
    ...
    auto req = io::ImageDispatchSpec::create_discard(image_ctx,
                                                     io::IMAGE_DISPATCH_LAYER_API_START,
                                                     aio_comp,
                                                     {{off, len}},
                                                     io::ImageArea::DATA,
