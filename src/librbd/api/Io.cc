...
template<typename I> bool is_valid_io(I& image_ctx, io::AioCompletion* aio_comp)
{
...
template<typename I>
ssize_t Io<I>::read(I& image_ctx, uint64_t off, uint64_t len, io::ReadResult&& read_result, int op_flags)
{
    auto cct = image_ctx.cct;
    ...
    aio_read(image_ctx, aio_comp, off, len, std::move(read_result), op_flags, false);
    return ctx.wait();
}

template<typename I> ssize_t Io<I>::write(I& image_ctx, uint64_t off, uint64_t len, bufferlist&& bl, int op_flags)
{
    auto cct = image_ctx.cct;
    ldout(cct, 20) << "ictx=" << &image_ctx << ", off=" << off << ", "
                   << "len = " << len << dendl;
...
template<typename I>
ssize_t Io<I>::discard(I& image_ctx, uint64_t off, uint64_t len, uint32_t discard_granularity_bytes)
{
    auto cct = image_ctx.cct;
    ldout(cct, 20) << "ictx=" << &image_ctx << ", off=" << off << ", "
                   << "len = " << len << dendl;
...
template<typename I> ssize_t Io<I>::write_same(I& image_ctx, uint64_t off, uint64_t len, bufferlist&& bl, int op_flags)
{
    auto cct = image_ctx.cct;
    ldout(cct, 20) << "ictx=" << &image_ctx << ", off=" << off << ", "
...
template<typename I> ssize_t Io<I>::write_zeroes(I& image_ctx, uint64_t off, uint64_t len, int zero_flags, int op_flags)
{
    auto cct = image_ctx.cct;
    ldout(cct, 20) << "ictx=" << &image_ctx << ", off=" << off << ", "
                   << "len = " << len << dendl;
...
template<typename I>
ssize_t Io<I>::compare_and_write(I& image_ctx, uint64_t off, uint64_t len, bufferlist&& cmp_bl, bufferlist&& bl,
                                 uint64_t* mismatch_off, int op_flags)
{
    ...
    aio_compare_and_write(
        image_ctx, aio_comp, off, len, std::move(cmp_bl), std::move(bl), mismatch_off, op_flags, false);
...
template<typename I>
void Io<I>::aio_read(I& image_ctx, io::AioCompletion* aio_comp, uint64_t off, uint64_t len,
                     io::ReadResult&& read_result, int op_flags, bool native_async)
{
    auto cct = image_ctx.cct;
    FUNCTRACE(cct);
    ZTracer::Trace trace;
    ...
    auto req = io::ImageDispatchSpec::create_read(image_ctx,
...
template<typename I>
void Io<I>::aio_write(I& image_ctx, io::AioCompletion* aio_comp, uint64_t off, uint64_t len, bufferlist&& bl,
                      int op_flags, bool native_async)
{
    ...
    auto req = io::ImageDispatchSpec::create_write(
        image_ctx, io::IMAGE_DISPATCH_LAYER_API_START, aio_comp, {{off, len}}, std::move(bl), op_flags, trace);
    req->send();
}

template<typename I>
void Io<I>::aio_discard(I& image_ctx, io::AioCompletion* aio_comp, uint64_t off, uint64_t len,
                        uint32_t discard_granularity_bytes, bool native_async)
{
    auto cct = image_ctx.cct;
    FUNCTRACE(cct);
    ZTracer::Trace trace;
    ...
    auto req = io::ImageDispatchSpec::create_discard(
        image_ctx, io::IMAGE_DISPATCH_LAYER_API_START, aio_comp, off, len, discard_granularity_bytes, trace);
    req->send();
}

template<typename I>
void Io<I>::aio_write_same(I& image_ctx, io::AioCompletion* aio_comp, uint64_t off, uint64_t len, bufferlist&& bl,
                           int op_flags, bool native_async)
{
    auto cct = image_ctx.cct;
    FUNCTRACE(cct);
...
template<typename I>
void Io<I>::aio_write_zeroes(I& image_ctx, io::AioCompletion* aio_comp, uint64_t off, uint64_t len, int zero_flags,
                             int op_flags, bool native_async)
{
    auto cct = image_ctx.cct;
    FUNCTRACE(cct);
    ...
    if (io_type == io::AIO_TYPE_WRITESAME) {
        ...
        if (prepend_length > 0) {
            ...
            auto prepend_aio_comp = io::AioCompletion::create_and_start(prepend_ctx, &image_ctx, io::AIO_TYPE_WRITE);
            auto prepend_req = io::ImageDispatchSpec::create_write(image_ctx,
                                                                   io::IMAGE_DISPATCH_LAYER_API_START,
                                                                   prepend_aio_comp,
        ...
        if (append_length > 0) {
            ...
            auto append_aio_comp = io::AioCompletion::create_and_start(append_ctx, &image_ctx, io::AIO_TYPE_WRITE);
            auto append_req = io::ImageDispatchSpec::create_write(image_ctx,
                                                                  io::IMAGE_DISPATCH_LAYER_API_START,
                                                                  append_aio_comp,
        ...
        auto write_same_aio_comp =
            io::AioCompletion::create_and_start(write_same_ctx, &image_ctx, io::AIO_TYPE_WRITESAME);
        auto req = io::ImageDispatchSpec::create_write_same(image_ctx,
                                                            io::IMAGE_DISPATCH_LAYER_API_START,
                                                            write_same_aio_comp,
                                                            write_same_offset,
                                                            write_same_length,
...
template<typename I>
void Io<I>::aio_compare_and_write(I& image_ctx, io::AioCompletion* aio_comp, uint64_t off, uint64_t len,
                                  bufferlist&& cmp_bl, bufferlist&& bl, uint64_t* mismatch_off, int op_flags,
                                  bool native_async)
{
    auto cct = image_ctx.cct;
    FUNCTRACE(cct);
    ...
    aio_comp->init_time(util::get_image_ctx(&image_ctx), io::AIO_TYPE_COMPARE_AND_WRITE);
    ldout(cct, 20) << "ictx=" << &image_ctx << ", "
                   << "completion=" << aio_comp << ", off=" << off << ", "
                   << "len=" << len << dendl;
    ...
    auto req = io::ImageDispatchSpec::create_compare_and_write(image_ctx,
                                                               io::IMAGE_DISPATCH_LAYER_API_START,
                                                               aio_comp,
                                                               {{off, len}},
                                                               std::move(cmp_bl),
...
template<typename I> void Io<I>::aio_flush(I& image_ctx, io::AioCompletion* aio_comp, bool native_async)
{
    auto cct = image_ctx.cct;
    FUNCTRACE(cct);
    ZTracer::Trace trace;
    ...
    auto req = io::ImageDispatchSpec::create_flush(
        image_ctx, io::IMAGE_DISPATCH_LAYER_API_START, aio_comp, io::FLUSH_SOURCE_USER, trace);
    req->send();
}
