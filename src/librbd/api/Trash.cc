                   ...
                   uint64_t delay)
{
    ...
    if (r == -ENOENT) {
        ...
        if (auto found_image = std::find_if(trash_image_specs.begin(),
                                            trash_image_specs.end(),
                                            [&](const auto& pair) {
...
template<typename I>
int Trash<I>::list(IoCtx& io_ctx, std::vector<trash_image_info_t>& entries, bool exclude_user_remove_source)
{
    CephContext* cct((CephContext*)io_ctx.cct());
    ldout(cct, 20) << __func__ << " " << &io_ctx << dendl;
