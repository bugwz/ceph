...
template<typename I>
const typename Trash<I>::TrashImageSources Trash<I>::ALLOWED_RESTORE_SOURCES{cls::rbd::TRASH_IMAGE_SOURCE_USER,
                                                                             cls::rbd::TRASH_IMAGE_SOURCE_MIRRORING,
                                                                             cls::rbd::TRASH_IMAGE_SOURCE_USER_PARENT};

namespace {

template<typename I> int disable_mirroring(I* ictx)
{
    ...
    int r = ctx.wait();
    if (r < 0) {
        lderr(ictx->cct) << "failed to disable mirroring: " << cpp_strerror(r) << dendl;
        return r;
    }
...
template<typename I> int enable_mirroring(IoCtx& io_ctx, const std::string& image_id)
{
    auto cct = reinterpret_cast<CephContext*>(io_ctx.cct());

    uint64_t features;
    uint64_t incompatible_features;
    int r = cls_client::get_features(&io_ctx, util::header_name(image_id), true, &features, &incompatible_features);
    if (r < 0) {
    ...
    auto req = mirror::EnableRequest<I>::create(
        io_ctx, image_id, cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "", false, asio_engine.get_work_queue(), &ctx);
    req->send();
    r = ctx.wait();
    if (r < 0) {
...
int list_trash_image_specs(librados::IoCtx& io_ctx, std::map<std::string, cls::rbd::TrashImageSpec>* trash_image_specs,
                           bool exclude_user_remove_source)
{
    ...
    do {
        ...
        if (r < 0 && r != -ENOENT) {
            lderr(cct) << "error listing rbd trash entries: " << cpp_strerror(r) << dendl;
            return r;
        }
        else if (r == -ENOENT) {
            break;
        ...
        for (const auto& entry : trash_entries) {
            if (exclude_user_remove_source && entry.second.source == cls::rbd::TRASH_IMAGE_SOURCE_REMOVING) {
                continue;
            }
...
template<typename I>
int Trash<I>::move(librados::IoCtx& io_ctx, rbd_trash_image_source_t source, const std::string& image_name,
                   const std::string& image_id, uint64_t delay)
{
    ...
    if (r == 0) {
        ...
        int mirror_r = cls_client::mirror_image_get(&ictx->md_ctx, ictx->id, &mirror_image);
        if (mirror_r == -ENOENT) {
            ldout(ictx->cct, 10) << "mirroring is not enabled for this image" << dendl;
        }
        else if (mirror_r < 0) {
            lderr(ictx->cct) << "failed to retrieve mirror image: " << cpp_strerror(mirror_r) << dendl;
            return mirror_r;
        }
        ...
        if (ictx->exclusive_lock != nullptr) {
            ...
            r = ictx->operations->prepare_image_update(exclusive_lock::OPERATION_REQUEST_TYPE_GENERAL, true);
            if (r < 0) {
        ...
        if (mirror_r >= 0 && mirror_image.mode != cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT) {
            r = disable_mirroring<I>(ictx);
            if (r < 0) {
                ictx->state->close();
                return r;
    ...
    cls::rbd::TrashImageSpec trash_image_spec{
        static_cast<cls::rbd::TrashImageSource>(source), image_name, delete_time, deferment_end_time};

    trash_image_spec.state = cls::rbd::TRASH_IMAGE_STATE_MOVING;
    C_SaferCond ctx;
    auto req = trash::MoveRequest<I>::create(io_ctx, image_id, trash_image_spec, &ctx);
    req->send();

    r = ctx.wait();
    trash_image_spec.state = cls::rbd::TRASH_IMAGE_STATE_NORMAL;
    int ret =
        cls_client::trash_state_set(&io_ctx, image_id, trash_image_spec.state, cls::rbd::TRASH_IMAGE_STATE_MOVING);
    if (ret < 0 && ret != -EOPNOTSUPP) {
        lderr(cct) << "error setting trash image state: " << cpp_strerror(ret) << dendl;
        return ret;
    }
    if (r < 0) {
        return r;
    ...
    TrashWatcher<I>::notify_image_added(io_ctx, image_id, trash_image_spec, &notify_ctx);
    r = notify_ctx.wait();
    if (r < 0) {
                   ...
                   uint64_t delay)
{
    ...
    if (r == -ENOENT) {
        ...
        std::experimental::erase_if(trash_image_specs, [image_name](const auto& pair) {
            const auto& spec = pair.second;
            return (spec.source != cls::rbd::TRASH_IMAGE_SOURCE_USER ||
                    spec.state != cls::rbd::TRASH_IMAGE_STATE_MOVING || spec.name != image_name);
        });
...
template<typename I> int Trash<I>::get(IoCtx& io_ctx, const std::string& id, trash_image_info_t* info)
{
    CephContext* cct((CephContext*)io_ctx.cct());
    ...
    rbd_trash_image_source_t source = static_cast<rbd_trash_image_source_t>(spec.source);
    *info = trash_image_info_t{id, spec.name, source, spec.deletion_time.sec(), spec.deferment_end_time.sec()};
    return 0;
}

template<typename I>
int Trash<I>::list(IoCtx& io_ctx, vector<trash_image_info_t>& entries, bool exclude_user_remove_source)
{
    CephContext* cct((CephContext*)io_ctx.cct());
    ldout(cct, 20) << __func__ << " " << &io_ctx << dendl;

    std::map<std::string, cls::rbd::TrashImageSpec> trash_image_specs;
    int r = list_trash_image_specs(io_ctx, &trash_image_specs, exclude_user_remove_source);
    if (r < 0) {
        return r;
    }
    ...
    for (const auto& [image_id, spec] : trash_image_specs) {
        rbd_trash_image_source_t source = static_cast<rbd_trash_image_source_t>(spec.source);
        entries.push_back({image_id, spec.name, source, spec.deletion_time.sec(), spec.deferment_end_time.sec()});
    }

    return 0;
}

template<typename I> int Trash<I>::purge(IoCtx& io_ctx, time_t expire_ts, float threshold, ProgressContext& pctx)
{
    ...
    trash_entries.erase(std::remove_if(trash_entries.begin(),
                                       trash_entries.end(),
    ...
    if (threshold != -1) {
        ...
        rados.mon_command(R"({"prefix": "df", "format": "json"})", inbl, &outbl, nullptr);
        ...
        std::sort(
            trash_entries.begin(), trash_entries.end(), [](librbd::trash_image_info_t a, librbd::trash_image_info_t b) {
                return a.deferment_end_time < b.deferment_end_time;
            });

        for (const auto& entry : trash_entries) {
            int64_t data_pool_id = -1;
            r = cls_client::get_data_pool(&io_ctx, util::header_name(entry.id), &data_pool_id);
        ...
        for (uint8_t i = 0; i < arr.size(); ++i) {
            ...
            if (img != datapools.end()) {
                ...
                pool_total_bytes = stats["max_avail"].get_uint64() + stats["bytes_used"].get_uint64();

                auto bytes_threshold = (uint64_t)(pool_total_bytes * (pool_percent_used - threshold));
                ...
                for (const auto& it : img->second) {
                    ...
                    if (r == -ENOENT) {
                        ...
                        lderr(cct) << "failed to open image " << it << ": " << cpp_strerror(r) << dendl;
                    }
                    ...
                    ictx->state->close();
                    if (r < 0) {
                        lderr(cct) << "failed to calculate disk usage for image " << it << ": " << cpp_strerror(r)
                                   << dendl;
                        continue;
                    }
    ...
    while (!to_be_removed.empty() && remove_err == 1) {
        ...
        for (auto it = to_be_removed.begin(); it != to_be_removed.end();) {
            ...
            if (r == -ENOENT) {
                ...
                lderr(cct) << "error getting image id " << *it << " info: " << cpp_strerror(r) << dendl;
                return r;
            }
            ...
            if (r == -ENOTEMPTY || r == -EBUSY || r == -EMLINK || r == -EUCLEAN) {
                ...
                lderr(cct) << "error removing image id " << *it << ": " << cpp_strerror(r) << dendl;
                return r;
            }
            pctx.update_progress(++i, list_size);
            it = to_be_removed.erase(it);
    ...
    if (!to_be_removed.empty()) {
        ...
        ldout(cct, 10) << "couldn't remove " << to_be_removed.size() << " expired images" << dendl;
        return remove_err;
    }
...
template<typename I>
int Trash<I>::remove(IoCtx& io_ctx, const std::string& image_id, bool force, ProgressContext& prog_ctx)
{
    CephContext* cct((CephContext*)io_ctx.cct());
    ...
    if (r < 0) {
        lderr(cct) << "error getting image id " << image_id << " info from trash: " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    C_SaferCond cond;
    auto req =
        librbd::trash::RemoveRequest<I>::create(io_ctx, image_id, asio_engine.get_work_queue(), force, prog_ctx, &cond);
    req->send();
...
template<typename I>
int Trash<I>::restore(librados::IoCtx& io_ctx, const TrashImageSources& trash_image_sources,
                      const std::string& image_id, const std::string& image_new_name)
{
    CephContext* cct((CephContext*)io_ctx.cct());
    ...
    if (trash_spec.state != cls::rbd::TRASH_IMAGE_STATE_NORMAL &&
        ...
        lderr(cct) << "error restoring image id " << image_id << ", which is pending deletion" << dendl;
        return -EBUSY;
    }
    r = cls_client::trash_state_set(
        &io_ctx, image_id, cls::rbd::TRASH_IMAGE_STATE_RESTORING, cls::rbd::TRASH_IMAGE_STATE_NORMAL);
    if (r < 0 && r != -EOPNOTSUPP) {
        lderr(cct) << "error setting trash image state: " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    if (r < 0 && r != -ENOENT) {
        lderr(cct) << "error checking if image " << image_name << " exists: " << cpp_strerror(r) << dendl;
        int ret = cls_client::trash_state_set(
            &io_ctx, image_id, cls::rbd::TRASH_IMAGE_STATE_NORMAL, cls::rbd::TRASH_IMAGE_STATE_RESTORING);
        if (ret < 0 && ret != -EOPNOTSUPP) {
            lderr(cct) << "error setting trash image state: " << cpp_strerror(ret) << dendl;
        }
        return r;
        ...
        if (existing_id != image_id) {
            ...
            if (r2 < 0 && r2 != -EOPNOTSUPP) {
                lderr(cct) << "error setting trash image state: " << cpp_strerror(r2) << dendl;
            }
            return -EEXIST;
        }
        create_id_obj = false;
    ...
    if (create_id_obj) {
        ...
        if (r < 0) {
            lderr(cct) << "error adding id object for image " << image_name << ": " << cpp_strerror(r) << dendl;
            return r;
        }
    }
    ...
    if (r < 0 && r != -EEXIST) {
        lderr(cct) << "error adding image to v2 directory: " << cpp_strerror(r) << dendl;
        return r;
    }
    ...
    if (r < 0 && r != -ENOENT) {
        lderr(cct) << "error removing image id " << image_id << " from trash: " << cpp_strerror(r) << dendl;
        return r;
    }
