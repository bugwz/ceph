...
public:
    ...
    Watcher(librados::IoCtx& ioctx, asio::ContextWQ* work_queue, const std::string& oid);
    virtual ~Watcher();
...
protected:
    ...
    void send_notify(bufferlist& payload, watcher::NotifyResponse* response = nullptr, Context* on_finish = nullptr);
...
private:
    ...
    struct C_RegisterWatch : public Context
    {
        ...
        C_RegisterWatch(Watcher* watcher, Context* on_finish)
            ...
            , on_finish(on_finish)
        {}
        void finish(int r) override { watcher->handle_register_watch(r, on_finish); }
    };
