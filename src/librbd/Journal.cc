...
template<typename I>
uint64_t Journal<I>::append_write_event(uint64_t offset, size_t length, const bufferlist& bl, bool flush_entry)
{
    ceph_assert(m_max_append_size > journal::AioWriteEvent::get_fixed_size());
    uint64_t max_write_data_size = m_max_append_size - journal::AioWriteEvent::get_fixed_size();
    ...
    return append_io_events(journal::EVENT_TYPE_AIO_WRITE, bufferlists, offset, length, flush_entry, 0);
}

template<typename I>
uint64_t Journal<I>::append_compare_and_write_event(uint64_t offset, size_t length, const bufferlist& cmp_bl,
                                                    const bufferlist& write_bl, bool flush_entry)
{
    ceph_assert(m_max_append_size > journal::AioCompareAndWriteEvent::get_fixed_size());
    uint64_t max_compare_and_write_data_size = m_max_append_size - journal::AioCompareAndWriteEvent::get_fixed_size();
    ...
    uint64_t event_offset = 0;
    do {
        uint64_t event_length = std::min(bytes_remaining, max_compare_and_write_data_size);
        ...
        journal::EventEntry event_entry(
            journal::AioCompareAndWriteEvent(offset + event_offset, event_length, event_cmp_bl, event_write_bl),
            ceph_clock_now());
    ...
    return append_io_events(
        journal::EVENT_TYPE_AIO_COMPARE_AND_WRITE, bufferlists, offset, length, flush_entry, -EILSEQ);
}
...
template<typename I>
uint64_t Journal<I>::append_io_events(journal::EventType event_type, const Bufferlists& bufferlists, uint64_t offset,
                                      size_t length, bool flush_entry, int filter_ret_val)
{
