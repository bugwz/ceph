...
static auto create_write_raw(librbd::ImageCtx* ictx, const char* buf, size_t len,
                             librbd::io::AioCompletion* aio_completion)
{
    ...
    return ceph::unique_leakable_ptr<ceph::buffer::raw>(
        buffer::claim_buffer(len, const_cast<char*>(buf), deleter(new UserBufferDeleter(ictx->cct, aio_completion))));
}
...
static bufferlist iovec_to_bufferlist(librbd::ImageCtx* ictx, const struct iovec* iov, int iovcnt,
                                      librbd::io::AioCompletion* aio_completion)
{
    bufferlist bl;
    for (int i = 0; i < iovcnt; ++i) {
        const struct iovec& io = iov[i];
        bl.push_back(create_write_raw(ictx, static_cast<char*>(io.iov_base), io.iov_len, aio_completion));
    }
    return bl;
}
...
struct C_AioCompletion : public Context
{
    ...
    C_AioCompletion(librbd::ImageCtx* ictx, librbd::io::aio_type_t aio_type, librbd::io::AioCompletion* aio_comp)
        : cct(ictx->cct)
        , aio_type(aio_type)
        , aio_comp(aio_comp)
    {
...
struct C_OpenComplete : public C_AioCompletion
{
    ...
    void** ictxp;
    C_OpenComplete(librbd::ImageCtx* ictx, librbd::io::AioCompletion* comp, void** ictxp)
...
struct C_OpenAfterCloseComplete : public Context
{
    ...
    void** ictxp;
    C_OpenAfterCloseComplete(librbd::ImageCtx* ictx, librbd::io::AioCompletion* comp, void** ictxp)
        : ictx(ictx)
        , comp(comp)
        , ictxp(ictxp)
    {}
...
struct C_QuiesceWatchCB : public librbd::QuiesceWatchCtx
{
    ...
    C_QuiesceWatchCB(rbd_update_callback_t quiesce_cb, rbd_update_callback_t unquiesce_cb, void* arg)
        : quiesce_cb(quiesce_cb)
...
void group_image_status_cpp_to_c(const librbd::group_image_info_t& cpp_info, rbd_group_image_info_t* c_info)
{
...
void group_info_cpp_to_c(const librbd::group_info_t& cpp_info, rbd_group_info_t* c_info)
{
    c_info->name = strdup(cpp_info.name.c_str());
    c_info->pool = cpp_info.pool;
}

void group_snap_info_cpp_to_c(const librbd::group_snap_info_t& cpp_info, rbd_group_snap_info_t* c_info)
{
    c_info->name = strdup(cpp_info.name.c_str());
    c_info->state = cpp_info.state;
}

void mirror_image_info_cpp_to_c(const librbd::mirror_image_info_t& cpp_info, rbd_mirror_image_info_t* c_info)
{
...
int get_local_mirror_image_site_status(const librbd::mirror_image_global_status_t& status,
                                       librbd::mirror_image_site_status_t* local_status)
{
    auto it = std::find_if(
        status.site_statuses.begin(), status.site_statuses.end(), [](const librbd::mirror_image_site_status_t& s) {
            return (s.mirror_uuid == cls::rbd::MirrorImageSiteStatus::LOCAL_MIRROR_UUID);
        });
...
int mirror_image_global_status_cpp_to_c(const librbd::mirror_image_global_status_t& cpp_status,
...
void mirror_image_global_status_cpp_to_c(const librbd::mirror_image_global_status_t& cpp_status,
                                         rbd_mirror_image_global_status_t* c_status)
{
    ...
    c_status->site_statuses = (rbd_mirror_image_site_status_t*)calloc(cpp_status.site_statuses.size(),
                                                                      sizeof(rbd_mirror_image_site_status_t));

    auto idx = 0U;
    for (auto it = cpp_status.site_statuses.begin(); it != cpp_status.site_statuses.end(); ++it) {
...
void trash_image_info_cpp_to_c(const librbd::trash_image_info_t& cpp_info, rbd_trash_image_info_t* c_info)
{
...
void config_option_cpp_to_c(const librbd::config_option_t& cpp_option, rbd_config_option_t* c_option)
{
...
struct C_MirrorImageGetInfo : public Context
{
    ...
    C_MirrorImageGetInfo(rbd_mirror_image_info_t* mirror_image_info, Context* on_finish)
...
struct C_MirrorImageGetGlobalStatus : public Context
{
    ...
    C_MirrorImageGetGlobalStatus(rbd_mirror_image_global_status_t* mirror_image_global_status, Context* on_finish)
    ...
    void finish(int r) override
    {
        ...
        mirror_image_global_status_cpp_to_c(cpp_mirror_image_global_status, mirror_image_global_status);
        on_finish->complete(0);
    }
};
...
struct C_MirrorImageGetStatus : public Context
{
    ...
    C_MirrorImageGetStatus(rbd_mirror_image_status_t* mirror_image_status, Context* on_finish)
        : mirror_image_status(mirror_image_status)
        , on_finish(on_finish)
    {}
    C_MirrorImageGetStatus(librbd::mirror_image_status_t* mirror_image_status, Context* on_finish)
        : mirror_image_status_cpp(mirror_image_status)
        , on_finish(on_finish)
    {}
    ...
    void finish(int r) override
    {
        ...
        if (mirror_image_status != nullptr) {
            r = mirror_image_global_status_cpp_to_c(cpp_mirror_image_global_status, mirror_image_status);
        }
        else if (mirror_image_status_cpp != nullptr) {
            librbd::mirror_image_site_status_t local_status;
            r = get_local_mirror_image_site_status(cpp_mirror_image_global_status, &local_status);
            if (r >= 0) {
...
public:
    CProgressContext(librbd_progress_fn_t fn, void* data)
        ...
        , m_data(data)
    {}
    int update_progress(uint64_t offset, uint64_t src_size) override { return m_fn(offset, src_size, m_data); }
...
int RBD::open(IoCtx& io_ctx, Image& image, const char* name, const char* snap_name)
{
    ...
    tracepoint(
        librbd, open_image_enter, ictx, ictx->name.c_str(), ictx->id.c_str(), ictx->snap_name.c_str(), ictx->read_only);
...
int RBD::open_by_id(IoCtx& io_ctx, Image& image, const char* id, const char* snap_name)
{
    ImageCtx* ictx = new ImageCtx("", id, snap_name, io_ctx, false);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, open_image_by_id_enter, ictx, ictx->id.c_str(), ictx->snap_name.c_str(), ictx->read_only);
...
int RBD::aio_open(IoCtx& io_ctx, Image& image, const char* name, const char* snap_name, RBD::AioCompletion* c)
{
    ImageCtx* ictx = new ImageCtx(name, "", snap_name, io_ctx, false);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd,
    ...
    if (image.ctx != NULL) {
        ...
        ictx->state->open(0, new C_OpenComplete(ictx, get_aio_completion(c), &image.ctx));
    }
    tracepoint(librbd, aio_open_image_exit, 0);
    return 0;
}

int RBD::aio_open_by_id(IoCtx& io_ctx, Image& image, const char* id, const char* snap_name, RBD::AioCompletion* c)
{
    ImageCtx* ictx = new ImageCtx("", id, snap_name, io_ctx, false);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(
        librbd, aio_open_image_by_id_enter, ictx, ictx->id.c_str(), ictx->snap_name.c_str(), ictx->read_only, c->pc);
...
int RBD::open_read_only(IoCtx& io_ctx, Image& image, const char* name, const char* snap_name)
{
...
int RBD::open_by_id_read_only(IoCtx& io_ctx, Image& image, const char* id, const char* snap_name)
{
...
int RBD::aio_open_read_only(IoCtx& io_ctx, Image& image, const char* name, const char* snap_name, RBD::AioCompletion* c)
{
    ImageCtx* ictx = new ImageCtx(name, "", snap_name, io_ctx, true);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd,
...
int RBD::aio_open_by_id_read_only(IoCtx& io_ctx, Image& image, const char* id, const char* snap_name,
                                  RBD::AioCompletion* c)
{
    ImageCtx* ictx = new ImageCtx("", id, snap_name, io_ctx, true);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
int RBD::create2(IoCtx& io_ctx, const char* name, uint64_t size, uint64_t features, int* order)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
int RBD::create3(IoCtx& io_ctx, const char* name, uint64_t size, uint64_t features, int* order, uint64_t stripe_unit,
                 uint64_t stripe_count)
{
    ...
    tracepoint(librbd,
               ...
               *order,
               stripe_unit,
               stripe_count);
    int r = librbd::create(io_ctx, name, size, false, features, order, stripe_unit, stripe_count);
    tracepoint(librbd, create3_exit, r, *order);
    return r;
}

int RBD::create4(IoCtx& io_ctx, const char* name, uint64_t size, ImageOptions& opts)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
int RBD::clone(IoCtx& p_ioctx, const char* p_name, const char* p_snap_name, IoCtx& c_ioctx, const char* c_name,
               uint64_t features, int* c_order)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioctx));
    tracepoint(librbd,
               clone_enter,
               p_ioctx.get_pool_name().c_str(),
               p_ioctx.get_id(),
               p_name,
               p_snap_name,
               c_ioctx.get_pool_name().c_str(),
               c_ioctx.get_id(),
               c_name,
               features);
    int r = librbd::clone(p_ioctx, p_name, p_snap_name, c_ioctx, c_name, features, c_order, 0, 0);
    tracepoint(librbd, clone_exit, r, *c_order);
    return r;
}

int RBD::clone2(IoCtx& p_ioctx, const char* p_name, const char* p_snap_name, IoCtx& c_ioctx, const char* c_name,
                uint64_t features, int* c_order, uint64_t stripe_unit, int stripe_count)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioctx));
    tracepoint(librbd,
               clone2_enter,
               ...
               stripe_unit,
               stripe_count);
    int r = librbd::clone(p_ioctx, p_name, p_snap_name, c_ioctx, c_name, features, c_order, stripe_unit, stripe_count);
    tracepoint(librbd, clone2_exit, r, *c_order);
    return r;
}

int RBD::clone3(IoCtx& p_ioctx, const char* p_name, const char* p_snap_name, IoCtx& c_ioctx, const char* c_name,
                ImageOptions& c_opts)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioctx));
    tracepoint(librbd,
               ...
               c_name,
               c_opts.opts);
    int r = librbd::clone(p_ioctx, nullptr, p_name, p_snap_name, c_ioctx, nullptr, c_name, c_opts, "", "");
...
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, trash_move_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), name);
    int r = librbd::api::Trash<>::move(io_ctx, RBD_TRASH_IMAGE_SOURCE_USER, name, delay);
...
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, trash_list_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id());
...
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, trash_remove_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), image_id, force);
    librbd::NoOpProgressContext prog_ctx;
...
int RBD::trash_remove_with_progress(IoCtx& io_ctx, const char* image_id, bool force, ProgressContext& pctx)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, trash_remove_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), image_id, force);
...
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, trash_undelete_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), id, name);
    int r = librbd::api::Trash<>::restore(io_ctx, librbd::api::Trash<>::ALLOWED_RESTORE_SOURCES, id, name);
...
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, trash_purge_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), expire_ts, threshold);
...
int RBD::trash_purge_with_progress(IoCtx& io_ctx, time_t expire_ts, float threshold, ProgressContext& pctx)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, trash_purge_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), expire_ts, threshold);
...
int RBD::namespace_list(IoCtx& io_ctx, std::vector<std::string>* namespace_names)
{
    return librbd::api::Namespace<>::list(io_ctx, namespace_names);
}

int RBD::namespace_exists(IoCtx& io_ctx, const char* namespace_name, bool* exists)
{
...
{
    auto pool_stat_options = reinterpret_cast<librbd::api::Pool<>::StatOptions*>(stats->pool_stats);
...
int RBD::migration_prepare(IoCtx& io_ctx, const char* image_name, IoCtx& dest_io_ctx, const char* dest_image_name,
                           ImageOptions& opts)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd,
               ...
               io_ctx.get_id(),
               image_name,
               dest_io_ctx.get_pool_name().c_str(),
               dest_io_ctx.get_id(),
               dest_image_name,
               opts.opts);
    int r = librbd::api::Migration<>::prepare(io_ctx, image_name, dest_io_ctx, dest_image_name, opts);
    tracepoint(librbd, migration_prepare_exit, r);
    return r;
}

int RBD::migration_prepare_import(const char* source_spec, IoCtx& dest_io_ctx, const char* dest_image_name,
                                  ImageOptions& opts)
{
    return librbd::api::Migration<>::prepare_import(source_spec, dest_io_ctx, dest_image_name, opts);
}

int RBD::migration_execute(IoCtx& io_ctx, const char* image_name)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, migration_execute_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), image_name);
    librbd::NoOpProgressContext prog_ctx;
...
int RBD::migration_execute_with_progress(IoCtx& io_ctx, const char* image_name, librbd::ProgressContext& prog_ctx)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, migration_execute_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), image_name);
    int r = librbd::api::Migration<>::execute(io_ctx, image_name, prog_ctx);
...
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, migration_abort_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), image_name);
    librbd::NoOpProgressContext prog_ctx;
...
int RBD::migration_abort_with_progress(IoCtx& io_ctx, const char* image_name, librbd::ProgressContext& prog_ctx)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, migration_abort_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), image_name);
    int r = librbd::api::Migration<>::abort(io_ctx, image_name, prog_ctx);
...
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, migration_commit_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), image_name);
    librbd::NoOpProgressContext prog_ctx;
...
int RBD::migration_commit_with_progress(IoCtx& io_ctx, const char* image_name, librbd::ProgressContext& prog_ctx)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, migration_commit_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), image_name);
    int r = librbd::api::Migration<>::commit(io_ctx, image_name, prog_ctx);
...
int RBD::migration_status(IoCtx& io_ctx, const char* image_name, image_migration_status_t* status, size_t status_size)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, migration_status_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), image_name);
...
int RBD::mirror_site_name_set(librados::Rados& rados, const std::string& site_name)
{
...
int RBD::mirror_peer_bootstrap_import(IoCtx& io_ctx, rbd_mirror_peer_direction_t direction, const std::string& token)
{
    return librbd::api::Mirror<>::peer_bootstrap_import(io_ctx, direction, token);
}

int RBD::mirror_peer_site_add(IoCtx& io_ctx, std::string* uuid, mirror_peer_direction_t direction,
                              const std::string& site_name, const std::string& client_name)
{
    return librbd::api::Mirror<>::peer_site_add(io_ctx, uuid, direction, site_name, client_name);
}
...
int RBD::mirror_peer_site_list(IoCtx& io_ctx, std::vector<mirror_peer_site_t>* peer_sites)
{
    return librbd::api::Mirror<>::peer_site_list(io_ctx, peer_sites);
}

int RBD::mirror_peer_site_set_client_name(IoCtx& io_ctx, const std::string& uuid, const std::string& client_name)
{
    return librbd::api::Mirror<>::peer_site_set_client(io_ctx, uuid, client_name);
}

int RBD::mirror_peer_site_set_name(IoCtx& io_ctx, const std::string& uuid, const std::string& site_name)
{
    return librbd::api::Mirror<>::peer_site_set_name(io_ctx, uuid, site_name);
}

int RBD::mirror_peer_site_set_direction(IoCtx& io_ctx, const std::string& uuid, mirror_peer_direction_t direction)
{
    return librbd::api::Mirror<>::peer_site_set_direction(io_ctx, uuid, direction);
}

int RBD::mirror_peer_site_get_attributes(IoCtx& io_ctx, const std::string& uuid,
                                         std::map<std::string, std::string>* key_vals)
{
    return librbd::api::Mirror<>::peer_site_get_attributes(io_ctx, uuid, key_vals);
}

int RBD::mirror_peer_site_set_attributes(IoCtx& io_ctx, const std::string& uuid,
                                         const std::map<std::string, std::string>& key_vals)
{
    return librbd::api::Mirror<>::peer_site_set_attributes(io_ctx, uuid, key_vals);
}

int RBD::mirror_image_global_status_list(IoCtx& io_ctx, const std::string& start_id, size_t max,
                                         std::map<std::string, mirror_image_global_status_t>* global_statuses)
{
    return librbd::api::Mirror<>::image_global_status_list(io_ctx, start_id, max, global_statuses);
}

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"

int RBD::mirror_peer_add(IoCtx& io_ctx, std::string* uuid, const std::string& cluster_name,
...
int RBD::mirror_peer_list(IoCtx& io_ctx, std::vector<mirror_peer_t>* peers)
{
    ...
    for (auto& peer_site : peer_sites) {
        peers->push_back({peer_site.uuid, peer_site.site_name, peer_site.client_name});
    }
    return 0;
}

int RBD::mirror_peer_set_client(IoCtx& io_ctx, const std::string& uuid, const std::string& client_name)
{
    return librbd::api::Mirror<>::peer_site_set_client(io_ctx, uuid, client_name);
}

int RBD::mirror_peer_set_cluster(IoCtx& io_ctx, const std::string& uuid, const std::string& cluster_name)
{
    return librbd::api::Mirror<>::peer_site_set_name(io_ctx, uuid, cluster_name);
}

int RBD::mirror_peer_get_attributes(IoCtx& io_ctx, const std::string& uuid,
                                    std::map<std::string, std::string>* key_vals)
{
    return librbd::api::Mirror<>::peer_site_get_attributes(io_ctx, uuid, key_vals);
}

int RBD::mirror_peer_set_attributes(IoCtx& io_ctx, const std::string& uuid,
                                    const std::map<std::string, std::string>& key_vals)
{
    return librbd::api::Mirror<>::peer_site_set_attributes(io_ctx, uuid, key_vals);
}

int RBD::mirror_image_status_list(IoCtx& io_ctx, const std::string& start_id, size_t max,
                                  std::map<std::string, mirror_image_status_t>* images)
{
    std::map<std::string, mirror_image_global_status_t> global_statuses;

    int r = librbd::api::Mirror<>::image_global_status_list(io_ctx, start_id, max, &global_statuses);
    if (r < 0) {
        return r;
    }
    ...
    for (auto& [id, global_status] : global_statuses) {
        if (global_status.site_statuses.empty() ||
            global_status.site_statuses[0].mirror_uuid != cls::rbd::MirrorImageSiteStatus::LOCAL_MIRROR_UUID) {
            continue;
        }

        auto& site_status = global_status.site_statuses[0];
        (*images)[id] = mirror_image_status_t{global_status.name,
                                              global_status.info,
                                              site_status.state,
                                              site_status.description,
                                              site_status.last_update,
                                              site_status.up};
    }
...
int RBD::mirror_image_status_summary(IoCtx& io_ctx, std::map<mirror_image_status_state_t, int>* states)
{
    return librbd::api::Mirror<>::image_status_summary(io_ctx, states);
}

int RBD::mirror_image_instance_id_list(IoCtx& io_ctx, const std::string& start_id, size_t max,
                                       std::map<std::string, std::string>* instance_ids)
{
    return librbd::api::Mirror<>::image_instance_id_list(io_ctx, start_id, max, instance_ids);
}

int RBD::mirror_image_info_list(IoCtx& io_ctx, mirror_image_mode_t* mode_filter, const std::string& start_id,
                                size_t max,
                                std::map<std::string, std::pair<mirror_image_mode_t, mirror_image_info_t>>* entries)
{
    return librbd::api::Mirror<>::image_info_list(io_ctx, mode_filter, start_id, max, entries);
}

int RBD::group_create(IoCtx& io_ctx, const char* group_name)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, group_create_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), group_name);
...
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, group_remove_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), group_name);
...
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, group_list_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id());
...
int RBD::group_rename(IoCtx& io_ctx, const char* src_name, const char* dest_name)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, group_rename_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), src_name, dest_name);
    int r = librbd::api::Group<>::rename(io_ctx, src_name, dest_name);
    tracepoint(librbd, group_rename_exit, r);
    return r;
}

int RBD::group_image_add(IoCtx& group_ioctx, const char* group_name, IoCtx& image_ioctx, const char* image_name)
{
    ...
    tracepoint(librbd,
               ...
               image_ioctx.get_id(),
               image_name);
    int r = librbd::api::Group<>::image_add(group_ioctx, group_name, image_ioctx, image_name);
    tracepoint(librbd, group_image_add_exit, r);
    return r;
}

int RBD::group_image_remove(IoCtx& group_ioctx, const char* group_name, IoCtx& image_ioctx, const char* image_name)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
    tracepoint(librbd,
               ...
               image_ioctx.get_id(),
               image_name);
    int r = librbd::api::Group<>::image_remove(group_ioctx, group_name, image_ioctx, image_name);
    tracepoint(librbd, group_image_remove_exit, r);
    return r;
}

int RBD::group_image_remove_by_id(IoCtx& group_ioctx, const char* group_name, IoCtx& image_ioctx, const char* image_id)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
    tracepoint(librbd,
               ...
               image_ioctx.get_id(),
               image_id);
    int r = librbd::api::Group<>::image_remove_by_id(group_ioctx, group_name, image_ioctx, image_id);
    tracepoint(librbd, group_image_remove_by_id_exit, r);
    return r;
}

int RBD::group_image_list(IoCtx& group_ioctx, const char* group_name, std::vector<group_image_info_t>* images,
                          size_t group_image_info_size)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
    tracepoint(librbd, group_image_list_enter, group_ioctx.get_pool_name().c_str(), group_ioctx.get_id(), group_name);
...
int RBD::group_snap_create(IoCtx& group_ioctx, const char* group_name, const char* snap_name)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
    tracepoint(librbd,
               ...
               group_name,
               snap_name);
    int r = librbd::api::Group<>::snap_create(group_ioctx, group_name, snap_name, 0);
    tracepoint(librbd, group_snap_create_exit, r);
    return r;
}

int RBD::group_snap_create2(IoCtx& group_ioctx, const char* group_name, const char* snap_name, uint32_t flags)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
    tracepoint(librbd,
               group_snap_create_enter,
               ...
               group_name,
               snap_name);
    int r = librbd::api::Group<>::snap_create(group_ioctx, group_name, snap_name, flags);
    tracepoint(librbd, group_snap_create_exit, r);
    return r;
}

int RBD::group_snap_remove(IoCtx& group_ioctx, const char* group_name, const char* snap_name)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
    tracepoint(librbd,
...
int RBD::group_snap_list(IoCtx& group_ioctx, const char* group_name, std::vector<group_snap_info_t>* snaps,
                         size_t group_snap_info_size)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
    tracepoint(librbd, group_snap_list_enter, group_ioctx.get_pool_name().c_str(), group_ioctx.get_id(), group_name);
...
int RBD::group_snap_rename(IoCtx& group_ioctx, const char* group_name, const char* old_snap_name,
                           const char* new_snap_name)
{
    ...
    tracepoint(librbd,
               ...
               old_snap_name,
               new_snap_name);
    int r = librbd::api::Group<>::snap_rename(group_ioctx, group_name, old_snap_name, new_snap_name);
    tracepoint(librbd, group_snap_list_exit, r);
    return r;
}

int RBD::group_snap_rollback(IoCtx& group_ioctx, const char* group_name, const char* snap_name)
{
    TracepointProvider::initialize<tracepoint_traits>(get_cct(group_ioctx));
    tracepoint(librbd,
               ...
               snap_name);
    librbd::NoOpProgressContext prog_ctx;
    int r = librbd::api::Group<>::snap_rollback(group_ioctx, group_name, snap_name, prog_ctx);
    tracepoint(librbd, group_snap_rollback_exit, r);
    return r;
}

int RBD::group_snap_rollback_with_progress(IoCtx& group_ioctx, const char* group_name, const char* snap_name,
...
int RBD::pool_metadata_get(IoCtx& ioctx, const std::string& key, std::string* value)
{
    int r = librbd::api::PoolMetadata<>::get(ioctx, key, value);
    return r;
}

int RBD::pool_metadata_set(IoCtx& ioctx, const std::string& key, const std::string& value)
{
...
int RBD::pool_metadata_list(IoCtx& ioctx, const std::string& start, uint64_t max, map<string, bufferlist>* pairs)
{
...
int Image::aio_close(RBD::AioCompletion* c)
{
    ...
    ictx->state->close(new C_AioCompletion(ictx, librbd::io::AIO_TYPE_CLOSE, get_aio_completion(c)));
    ctx = NULL;
...
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(librbd, get_create_timestamp_enter, ictx, ictx->name.c_str(), ictx->read_only);
...
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(librbd, get_access_timestamp_enter, ictx, ictx->name.c_str(), ictx->read_only);
    {
...
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(librbd, get_modify_timestamp_enter, ictx, ictx->name.c_str(), ictx->read_only);
    {
        std::shared_lock timestamp_locker{ictx->timestamp_lock};
...
int Image::parent_info(string* parent_pool_name, string* parent_name, string* parent_snap_name)
{
...
int Image::parent_info2(string* parent_pool_name, string* parent_name, string* parent_id, string* parent_snap_name)
{
    librbd::linked_image_spec_t parent_image;
    librbd::snap_spec_t parent_snap;
    int r = get_parent(&parent_image, &parent_snap);
...
int Image::get_parent(linked_image_spec_t* parent_image, snap_spec_t* parent_snap)
{
...
int Image::lock_get_owners(rbd_lock_mode_t* lock_mode, std::list<std::string>* lock_owners)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
...
int Image::lock_break(rbd_lock_mode_t lock_mode, const std::string& lock_owner)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
...
int Image::copy2(Image& dest)
{
    ...
    tracepoint(librbd,
               ...
               srcctx,
               srcctx->name.c_str(),
               srcctx->snap_name.c_str(),
               srcctx->read_only,
               destctx,
               destctx->name.c_str(),
               destctx->snap_name.c_str(),
               destctx->read_only);
    librbd::NoOpProgressContext prog_ctx;
...
int Image::copy_with_progress(IoCtx& dest_io_ctx, const char* destname, librbd::ProgressContext& pctx)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(librbd,
               copy_enter,
...
int Image::copy_with_progress3(IoCtx& dest_io_ctx, const char* destname, ImageOptions& opts,
...
int Image::copy_with_progress4(IoCtx& dest_io_ctx, const char* destname, ImageOptions& opts,
                               librbd::ProgressContext& pctx, size_t sparse_size)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(librbd,
               copy4_enter,
...
int Image::deep_copy(IoCtx& dest_io_ctx, const char* destname, ImageOptions& opts)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(librbd,
               deep_copy_enter,
               ...
               opts.opts);
    librbd::NoOpProgressContext prog_ctx;
    int r = librbd::api::Image<>::deep_copy(ictx, dest_io_ctx, destname, opts, prog_ctx);
    tracepoint(librbd, deep_copy_exit, r);
    return r;
}

int Image::deep_copy_with_progress(IoCtx& dest_io_ctx, const char* destname, ImageOptions& opts,
...
int Image::encryption_format(encryption_format_t format, encryption_options_t opts, size_t opts_size)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    return librbd::api::Image<>::encryption_format(ictx, format, opts, opts_size, false);
}

int Image::encryption_load(encryption_format_t format, encryption_options_t opts, size_t opts_size)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
...
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    return librbd::api::Image<>::encryption_load(ictx, specs, spec_count, false);
}
...
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(librbd, sparsify_enter, ictx, ictx->name.c_str(), sparse_size, ictx->id.c_str());
    librbd::NoOpProgressContext prog_ctx;
...
int Image::sparsify_with_progress(size_t sparse_size, librbd::ProgressContext& prog_ctx)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(librbd, sparsify_enter, ictx, ictx->name.c_str(), sparse_size, ictx->id.c_str());
    int r = ictx->operations->sparsify(sparse_size, prog_ctx);
...
int Image::list_lockers(std::list<librbd::locker_t>* lockers, bool* exclusive, string* tag)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    ...
    if (r >= 0) {
        for (std::list<librbd::locker_t>::const_iterator it = lockers->begin(); it != lockers->end(); ++it) {
...
int Image::unlock(const string& cookie)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, unlock_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, cookie.c_str());
...
int Image::snap_create(const char* snap_name)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, snap_create_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, snap_name);
...
int Image::snap_create2(const char* snap_name, uint32_t flags, ProgressContext& prog_ctx)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, snap_create_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, snap_name);
...
int Image::snap_remove(const char* snap_name)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, snap_remove_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, snap_name);
    librbd::NoOpProgressContext prog_ctx;
...
int Image::snap_rollback(const char* snap_name)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, snap_rollback_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, snap_name);
    librbd::NoOpProgressContext prog_ctx;
...
int Image::snap_protect(const char* snap_name)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, snap_protect_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, snap_name);
...
int Image::snap_unprotect(const char* snap_name)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, snap_unprotect_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, snap_name);
...
int Image::snap_is_protected(const char* snap_name, bool* is_protected)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, snap_is_protected_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, snap_name);
...
bool Image::snap_exists(const char* snap_name)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, snap_exists_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, snap_name);
    bool exists;
...
int Image::snap_get_namespace_type(uint64_t snap_id, snap_namespace_type_t* namespace_type)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
...
int Image::snap_get_group_namespace(uint64_t snap_id, snap_group_namespace_t* group_snap, size_t group_snap_size)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    ...
    int r = librbd::api::Snapshot<>::get_group_namespace(ictx, snap_id, group_snap);
    tracepoint(librbd, snap_get_group_namespace_exit, r);
    return r;
}

int Image::snap_get_trash_namespace(uint64_t snap_id, std::string* original_name)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    return librbd::api::Snapshot<>::get_trash_namespace(ictx, snap_id, original_name);
}

int Image::snap_get_mirror_namespace(uint64_t snap_id, snap_mirror_namespace_t* mirror_snap, size_t mirror_snap_size)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    ...
    int r = librbd::api::Snapshot<>::get_mirror_namespace(ictx, snap_id, mirror_snap);
    return r;
}
...
ssize_t Image::read2(uint64_t ofs, size_t len, bufferlist& bl, int op_flags)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, read2_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, ofs, len, op_flags);
...
int64_t Image::read_iterate(uint64_t ofs, size_t len, int (*cb)(uint64_t, size_t, const char*, void*), void* arg)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, read_iterate_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, ofs, len);
...
int Image::read_iterate2(uint64_t ofs, uint64_t len, int (*cb)(uint64_t, size_t, const char*, void*), void* arg)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, read_iterate2_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, ofs, len);
...
int Image::diff_iterate(const char* fromsnapname, uint64_t ofs, uint64_t len, int (*cb)(uint64_t, size_t, int, void*),
                        void* arg)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(librbd,
    ...
    int r = librbd::api::DiffIterate<>::diff_iterate(
        ictx, cls::rbd::UserSnapshotNamespace(), fromsnapname, ofs, len, true, false, cb, arg);
    tracepoint(librbd, diff_iterate_exit, r);
    return r;
}

int Image::diff_iterate2(const char* fromsnapname, uint64_t ofs, uint64_t len, bool include_parent, bool whole_object,
                         int (*cb)(uint64_t, size_t, int, void*), void* arg)
{
    ...
    tracepoint(librbd,
               ...
               whole_object);
    int r = librbd::api::DiffIterate<>::diff_iterate(
        ictx, cls::rbd::UserSnapshotNamespace(), fromsnapname, ofs, len, include_parent, whole_object, cb, arg);
    tracepoint(librbd, diff_iterate_exit, r);
    return r;
}
...
ssize_t Image::writesame(uint64_t ofs, size_t len, bufferlist& bl, int op_flags)
{
    ...
    tracepoint(librbd,
               ...
               ofs,
               len,
               bl.length() == 0 ? NULL : bl.c_str(),
               bl.length(),
               op_flags);
    if (bl.length() == 0 || len % bl.length() || len > static_cast<size_t>(std::numeric_limits<int>::max())) {
...
ssize_t Image::write_zeroes(uint64_t ofs, size_t len, int zero_flags, int op_flags)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    return api::Io<>::write_zeroes(*ictx, ofs, len, zero_flags, op_flags);
}

ssize_t Image::compare_and_write(uint64_t ofs, size_t len, ceph::bufferlist& cmp_bl, ceph::bufferlist& bl,
                                 uint64_t* mismatch_off, int op_flags)
{
    ...
    int r = api::Io<>::compare_and_write(*ictx, ofs, len, bufferlist{cmp_bl}, bufferlist{bl}, mismatch_off, op_flags);
...
int Image::aio_write(uint64_t off, size_t len, bufferlist& bl, RBD::AioCompletion* c)
{
    ...
    api::Io<>::aio_write(*ictx, get_aio_completion(c), off, len, bufferlist{bl}, 0, true);
...
int Image::aio_write2(uint64_t off, size_t len, bufferlist& bl, RBD::AioCompletion* c, int op_flags)
{
    ...
    api::Io<>::aio_write(*ictx, get_aio_completion(c), off, len, bufferlist{bl}, op_flags, true);
...
int Image::aio_read(uint64_t off, size_t len, bufferlist& bl, RBD::AioCompletion* c)
{
    ...
    tracepoint(librbd,
               ...
               bl.c_str(),
               c->pc);
    ldout(ictx->cct, 10) << "Image::aio_read() buf=" << (void*)bl.c_str() << "~" << (void*)(bl.c_str() + len - 1)
                         << dendl;

    api::Io<>::aio_read(*ictx, get_aio_completion(c), off, len, io::ReadResult{&bl}, 0, true);
...
int Image::aio_read2(uint64_t off, size_t len, bufferlist& bl, RBD::AioCompletion* c, int op_flags)
{
    ...
    api::Io<>::aio_read(*ictx, get_aio_completion(c), off, len, io::ReadResult{&bl}, op_flags, true);
    tracepoint(librbd, aio_read_exit, 0);
    return 0;
}
...
int Image::aio_discard(uint64_t off, uint64_t len, RBD::AioCompletion* c)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(
        librbd, aio_discard_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, off, len, c->pc);
    api::Io<>::aio_discard(*ictx, get_aio_completion(c), off, len, ictx->discard_granularity_bytes, true);
    tracepoint(librbd, aio_discard_exit, 0);
    return 0;
}

int Image::aio_writesame(uint64_t off, size_t len, bufferlist& bl, RBD::AioCompletion* c, int op_flags)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(librbd,
               aio_writesame_enter,
    ...
    bool discard_zero = ictx->config.get_val<bool>("rbd_discard_on_zeroed_write_same");
    if (discard_zero && bl.is_zero()) {
        api::Io<>::aio_write_zeroes(*ictx, get_aio_completion(c), off, len, 0U, op_flags, true);
        tracepoint(librbd, aio_writesame_exit, 0);
        return 0;
    }

    api::Io<>::aio_write_same(*ictx, get_aio_completion(c), off, len, bufferlist{bl}, op_flags, true);
    tracepoint(librbd, aio_writesame_exit, 0);
    return 0;
}

int Image::aio_write_zeroes(uint64_t off, size_t len, RBD::AioCompletion* c, int zero_flags, int op_flags)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    api::Io<>::aio_write_zeroes(*ictx, get_aio_completion(c), off, len, zero_flags, op_flags, true);
    return 0;
}

int Image::aio_compare_and_write(uint64_t off, size_t len, ceph::bufferlist& cmp_bl, ceph::bufferlist& bl,
                                 RBD::AioCompletion* c, uint64_t* mismatch_off, int op_flags)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    tracepoint(librbd,
    ...
    api::Io<>::aio_compare_and_write(
        *ictx, get_aio_completion(c), off, len, bufferlist{cmp_bl}, bufferlist{bl}, mismatch_off, op_flags, false);
...
int Image::poll_io_events(RBD::AioCompletion** comps, int numcomp)
{
    ...
    if (r > 0) {
        for (int i = 0; i < r; ++i) comps[i] = (RBD::AioCompletion*)cs[i]->rbd_comp;
    }
    return r;
}
...
int Image::mirror_image_get_info(mirror_image_info_t* mirror_image_info, size_t info_size)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
...
int Image::mirror_image_get_global_status(mirror_image_global_status_t* mirror_image_global_status, size_t status_size)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    ...
    return librbd::api::Mirror<>::image_get_global_status(ictx, mirror_image_global_status);
}
...
int Image::mirror_image_get_status(mirror_image_status_t* mirror_image_status, size_t status_size)
{
    ...
    int r = librbd::api::Mirror<>::image_get_global_status(ictx, &mirror_image_global_status);
    if (r < 0) {
        return r;
    }

    librbd::mirror_image_site_status_t local_status;
    r = get_local_mirror_image_site_status(mirror_image_global_status, &local_status);
    if (r < 0) {
        return r;
    }
...
int Image::aio_mirror_image_promote(bool force, RBD::AioCompletion* c)
{
    ...
    librbd::api::Mirror<>::image_promote(
        ictx, force, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(c)));
    return 0;
}
...
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    librbd::api::Mirror<>::image_demote(ictx,
                                        new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(c)));
    return 0;
}

int Image::aio_mirror_image_get_info(mirror_image_info_t* mirror_image_info, size_t info_size, RBD::AioCompletion* c)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    ...
    librbd::api::Mirror<>::image_get_info(
        ictx, mirror_image_info, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(c)));
    return 0;
}

int Image::aio_mirror_image_get_mode(mirror_image_mode_t* mode, RBD::AioCompletion* c)
{
    ImageCtx* ictx = (ImageCtx*)ctx;

    librbd::api::Mirror<>::image_get_mode(
        ictx, mode, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(c)));
    return 0;
}
                                              ...
                                              RBD::AioCompletion* c)
{
    ...
    librbd::api::Mirror<>::image_get_global_status(
        ictx, status, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(c)));
    return 0;
}
...
int Image::aio_mirror_image_get_status(mirror_image_status_t* status, size_t status_size, RBD::AioCompletion* c)
{
    ImageCtx* ictx = (ImageCtx*)ctx;
    ...
    auto ctx = new C_MirrorImageGetStatus(
        status, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(c)));
    librbd::api::Mirror<>::image_get_global_status(ictx, &ctx->cpp_mirror_image_global_status, ctx);
    return 0;
}

#pragma GCC diagnostic pop

int Image::aio_mirror_image_create_snapshot(uint32_t flags, uint64_t* snap_id, RBD::AioCompletion* c)
{
    ImageCtx* ictx = (ImageCtx*)ctx;

    librbd::api::Mirror<>::image_snapshot_create(
        ictx, flags, snap_id, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(c)));
    return 0;
}
...
{
    if (major) *major = LIBRBD_VER_MAJOR;
    if (minor) *minor = LIBRBD_VER_MINOR;
    if (extra) *extra = LIBRBD_VER_EXTRA;
}
...
extern "C" int rbd_image_options_set_string(rbd_image_options_t opts, int optname, const char* optval)
{
    return librbd::image_options_set(opts, optname, optval);
}

extern "C" int rbd_image_options_set_uint64(rbd_image_options_t opts, int optname, uint64_t optval)
{
    return librbd::image_options_set(opts, optname, optval);
}

extern "C" int rbd_image_options_get_string(rbd_image_options_t opts, int optname, char* optval, size_t maxlen)
{
    std::string optval_;
...
extern "C" int rbd_image_options_get_uint64(rbd_image_options_t opts, int optname, uint64_t* optval)
{
    return librbd::image_options_get(opts, optname, optval);
}

extern "C" int rbd_image_options_is_set(rbd_image_options_t opts, int optname, bool* is_set)
{
    return librbd::image_options_is_set(opts, optname, is_set);
}
...
/* pool mirroring */
extern "C" int rbd_mirror_site_name_get(rados_t cluster, char* name, size_t* max_len)
{
...
extern "C" int rbd_mirror_mode_get(rados_ioctx_t p, rbd_mirror_mode_t* mirror_mode)
{
...
extern "C" int rbd_mirror_mode_set(rados_ioctx_t p, rbd_mirror_mode_t mirror_mode)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    return librbd::api::Mirror<>::mode_set(io_ctx, mirror_mode);
...
extern "C" int rbd_mirror_uuid_get(rados_ioctx_t p, char* mirror_uuid, size_t* max_len)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_mirror_peer_bootstrap_create(rados_ioctx_t p, char* token, size_t* max_len)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_mirror_peer_bootstrap_import(rados_ioctx_t p, rbd_mirror_peer_direction_t direction,
                                                const char* token)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_mirror_peer_site_add(rados_ioctx_t p, char* uuid, size_t uuid_max_length,
                                        ...
                                        const char* client_name)
{
    ...
    std::string uuid_str;
    int r = librbd::api::Mirror<>::peer_site_add(io_ctx, &uuid_str, direction, site_name, client_name);
    if (r >= 0) {
...
extern "C" int rbd_mirror_peer_site_list(rados_ioctx_t p, rbd_mirror_peer_site_t* peers, int* max_peers)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" void rbd_mirror_peer_site_list_cleanup(rbd_mirror_peer_site_t* peers, int max_peers)
{
...
extern "C" int rbd_mirror_peer_site_set_client_name(rados_ioctx_t p, const char* uuid, const char* client_name)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    return librbd::api::Mirror<>::peer_site_set_client(io_ctx, uuid, client_name);
...
extern "C" int rbd_mirror_peer_site_set_name(rados_ioctx_t p, const char* uuid, const char* site_name)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    return librbd::api::Mirror<>::peer_site_set_name(io_ctx, uuid, site_name);
...
extern "C" int rbd_mirror_peer_site_set_direction(rados_ioctx_t p, const char* uuid,
...
extern "C" int rbd_mirror_peer_site_get_attributes(rados_ioctx_t p, const char* uuid, char* keys, size_t* max_key_len,
                                                   char* values, size_t* max_val_len, size_t* key_value_count)
{
    ...
    int r = librbd::api::Mirror<>::peer_site_get_attributes(io_ctx, uuid, &attributes);
    if (r < 0) {
        return r;
    }
    ...
    bool too_short = ((*max_key_len < key_total_len) || (*max_val_len < val_total_len));
...
extern "C" int rbd_mirror_peer_site_set_attributes(rados_ioctx_t p, const char* uuid, const char* keys,
                                                   const char* values, size_t count)
{
    ...
    return librbd::api::Mirror<>::peer_site_set_attributes(io_ctx, uuid, attributes);
}

extern "C" int rbd_mirror_image_global_status_list(rados_ioctx_t p, const char* start_id, size_t max, char** image_ids,
                                                   rbd_mirror_image_global_status_t* images, size_t* len)
{
    ...
    int r = librbd::api::Mirror<>::image_global_status_list(io_ctx, start_id, max, &cpp_images);
    if (r < 0) {
        return r;
    }
...
extern "C" void rbd_mirror_image_global_status_cleanup(rbd_mirror_image_global_status_t* global_status)
{
...
extern "C" void rbd_mirror_image_global_status_list_cleanup(char** image_ids, rbd_mirror_image_global_status_t* images,
                                                            size_t len)
{
...
extern "C" int rbd_mirror_peer_add(rados_ioctx_t p, char* uuid, size_t uuid_max_length, const char* cluster_name,
                                   const char* client_name)
{
    return rbd_mirror_peer_site_add(
        p, uuid, uuid_max_length, RBD_MIRROR_PEER_DIRECTION_RX_TX, cluster_name, client_name);
}
...
extern "C" int rbd_mirror_peer_list(rados_ioctx_t p, rbd_mirror_peer_t* peers, int* max_peers)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" void rbd_mirror_peer_list_cleanup(rbd_mirror_peer_t* peers, int max_peers)
{
    for (int i = 0; i < max_peers; ++i) {
        free(peers[i].uuid);
...
extern "C" int rbd_mirror_peer_set_client(rados_ioctx_t p, const char* uuid, const char* client_name)
{
    return rbd_mirror_peer_site_set_client_name(p, uuid, client_name);
}

extern "C" int rbd_mirror_peer_set_cluster(rados_ioctx_t p, const char* uuid, const char* cluster_name)
{
    return rbd_mirror_peer_site_set_name(p, uuid, cluster_name);
}

extern "C" int rbd_mirror_peer_get_attributes(rados_ioctx_t p, const char* uuid, char* keys, size_t* max_key_len,
                                              char* values, size_t* max_val_len, size_t* key_value_count)
{
    return rbd_mirror_peer_site_get_attributes(p, uuid, keys, max_key_len, values, max_val_len, key_value_count);
}

extern "C" int rbd_mirror_peer_set_attributes(rados_ioctx_t p, const char* uuid, const char* keys, const char* values,
                                              size_t count)
{
    return rbd_mirror_peer_site_set_attributes(p, uuid, keys, values, count);
}

extern "C" int rbd_mirror_image_status_list(rados_ioctx_t p, const char* start_id, size_t max, char** image_ids,
...
extern "C" void rbd_mirror_image_status_list_cleanup(char** image_ids, rbd_mirror_image_status_t* images, size_t len)
{
    for (size_t i = 0; i < len; i++) {
        free(image_ids[i]);
        free(images[i].name);
...
extern "C" int rbd_mirror_image_status_summary(rados_ioctx_t p, rbd_mirror_image_status_state_t* states, int* counts,
                                               size_t* maxlen)
{
...
extern "C" int rbd_mirror_image_instance_id_list(rados_ioctx_t p, const char* start_id, size_t max, char** image_ids,
                                                 char** instance_ids, size_t* len)
{
    ...
    int r = librbd::api::Mirror<>::image_instance_id_list(io_ctx, start_id, max, &cpp_instance_ids);
    if (r < 0) {
        return r;
    }
...
extern "C" void rbd_mirror_image_instance_id_list_cleanup(char** image_ids, char** instance_ids, size_t len)
{
    for (size_t i = 0; i < len; i++) {
        free(image_ids[i]);
        free(instance_ids[i]);
...
extern "C" int rbd_mirror_image_info_list(rados_ioctx_t p, rbd_mirror_image_mode_t* mode_filter, const char* start_id,
                                          size_t max, char** image_ids, rbd_mirror_image_mode_t* mode_entries,
                                          rbd_mirror_image_info_t* info_entries, size_t* num_entries)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    std::map<std::string, std::pair<librbd::mirror_image_mode_t, librbd::mirror_image_info_t>> cpp_entries;

    int r = librbd::api::Mirror<>::image_info_list(io_ctx, mode_filter, start_id, max, &cpp_entries);
    if (r < 0) {
        return r;
    }
...
extern "C" void rbd_mirror_image_info_list_cleanup(char** image_ids, rbd_mirror_image_info_t* info_entries,
                                                   size_t num_entries)
{
...
extern "C" void rbd_image_spec_list_cleanup(rbd_image_spec_t* images, size_t num_images)
{
...
extern "C" void rbd_linked_image_spec_list_cleanup(rbd_linked_image_spec_t* images, size_t num_images)
{
    for (size_t idx = 0; idx < num_images; ++idx) {
...
extern "C" int rbd_list2(rados_ioctx_t p, rbd_image_spec_t* images, size_t* size)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_create2(rados_ioctx_t p, const char* name, uint64_t size, uint64_t features, int* order)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int rbd_create3(rados_ioctx_t p, const char* name, uint64_t size, uint64_t features, int* order,
...
extern "C" int rbd_create4(rados_ioctx_t p, const char* name, uint64_t size, rbd_image_options_t opts)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int rbd_clone(rados_ioctx_t p_ioctx, const char* p_name, const char* p_snap_name, rados_ioctx_t c_ioctx,
                         const char* c_name, uint64_t features, int* c_order)
{
    ...
    tracepoint(librbd,
               clone_enter,
               p_ioc.get_pool_name().c_str(),
               p_ioc.get_id(),
               p_name,
               p_snap_name,
               c_ioc.get_pool_name().c_str(),
               c_ioc.get_id(),
               c_name,
               features);
    int r = librbd::clone(p_ioc, p_name, p_snap_name, c_ioc, c_name, features, c_order, 0, 0);
    tracepoint(librbd, clone_exit, r, *c_order);
    return r;
}

extern "C" int rbd_clone2(rados_ioctx_t p_ioctx, const char* p_name, const char* p_snap_name, rados_ioctx_t c_ioctx,
                          const char* c_name, uint64_t features, int* c_order, uint64_t stripe_unit, int stripe_count)
{
    librados::IoCtx p_ioc, c_ioc;
    librados::IoCtx::from_rados_ioctx_t(p_ioctx, p_ioc);
    librados::IoCtx::from_rados_ioctx_t(c_ioctx, c_ioc);
    ...
    tracepoint(librbd,
               ...
               stripe_unit,
               stripe_count);
    int r = librbd::clone(p_ioc, p_name, p_snap_name, c_ioc, c_name, features, c_order, stripe_unit, stripe_count);
    tracepoint(librbd, clone2_exit, r, *c_order);
    return r;
}

extern "C" int rbd_clone3(rados_ioctx_t p_ioctx, const char* p_name, const char* p_snap_name, rados_ioctx_t c_ioctx,
                          const char* c_name, rbd_image_options_t c_opts)
{
    ...
    int r = librbd::clone(p_ioc, nullptr, p_name, p_snap_name, c_ioc, nullptr, c_name, c_opts_, "", "");
    tracepoint(librbd, clone3_exit, r);
    return r;
}
...
extern "C" int rbd_remove_with_progress(rados_ioctx_t p, const char* name, librbd_progress_fn_t cb, void* cbdata)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int rbd_trash_move(rados_ioctx_t p, const char* name, uint64_t delay)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int rbd_trash_get(rados_ioctx_t io, const char* id, rbd_trash_image_info_t* info)
{
    librados::IoCtx io_ctx;
...
extern "C" int rbd_trash_list(rados_ioctx_t p, rbd_trash_image_info_t* entries, size_t* num_entries)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" void rbd_trash_list_cleanup(rbd_trash_image_info_t* entries, size_t num_entries)
{
    for (size_t i = 0; i < num_entries; i++) {
...
extern "C" int rbd_trash_purge(rados_ioctx_t io, time_t expire_ts, float threshold)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int rbd_trash_purge_with_progress(rados_ioctx_t io, time_t expire_ts, float threshold,
...
extern "C" int rbd_trash_remove(rados_ioctx_t p, const char* image_id, bool force)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int rbd_trash_remove_with_progress(rados_ioctx_t p, const char* image_id, bool force,
                                              librbd_progress_fn_t cb, void* cbdata)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_trash_restore(rados_ioctx_t p, const char* id, const char* name)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int rbd_namespace_create(rados_ioctx_t io, const char* namespace_name)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
...
extern "C" int rbd_namespace_remove(rados_ioctx_t io, const char* namespace_name)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
...
extern "C" int rbd_namespace_exists(rados_ioctx_t io, const char* namespace_name, bool* exists)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
...
{
    *stats = reinterpret_cast<rbd_pool_stats_t>(new librbd::api::Pool<>::StatOptions{});
}

extern "C" void rbd_pool_stats_destroy(rbd_pool_stats_t stats)
{
    auto pool_stat_options = reinterpret_cast<librbd::api::Pool<>::StatOptions*>(stats);
    delete pool_stat_options;
}

extern "C" int rbd_pool_stats_option_add_uint64(rbd_pool_stats_t stats, int stat_option, uint64_t* stat_val)
{
    auto pool_stat_options = reinterpret_cast<librbd::api::Pool<>::StatOptions*>(stats);
    return librbd::api::Pool<>::add_stat_option(
        pool_stat_options, static_cast<rbd_pool_stat_option_t>(stat_option), stat_val);
}

extern "C" int rbd_pool_stats_get(rados_ioctx_t io, rbd_pool_stats_t pool_stats)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(io, io_ctx);

    auto pool_stat_options = reinterpret_cast<librbd::api::Pool<>::StatOptions*>(pool_stats);
    return librbd::api::Pool<>::get_stats(io_ctx, pool_stat_options);
}
...
extern "C" int rbd_copy3(rbd_image_t image, rados_ioctx_t dest_p, const char* destname, rbd_image_options_t c_opts)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librados::IoCtx dest_io_ctx;
    librados::IoCtx::from_rados_ioctx_t(dest_p, dest_io_ctx);
...
extern "C" int rbd_copy4(rbd_image_t image, rados_ioctx_t dest_p, const char* destname, rbd_image_options_t c_opts,
                         size_t sparse_size)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librados::IoCtx dest_io_ctx;
...
extern "C" int rbd_copy_with_progress(rbd_image_t image, rados_ioctx_t dest_p, const char* destname,
...
extern "C" int rbd_copy_with_progress2(rbd_image_t srcp, rbd_image_t destp, librbd_progress_fn_t fn, void* data)
{
    librbd::ImageCtx* src = (librbd::ImageCtx*)srcp;
    librbd::ImageCtx* dest = (librbd::ImageCtx*)destp;
    tracepoint(librbd,
...
extern "C" int rbd_copy_with_progress3(rbd_image_t image, rados_ioctx_t dest_p, const char* destname,
                                       rbd_image_options_t dest_opts, librbd_progress_fn_t fn, void* data)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librados::IoCtx dest_io_ctx;
    librados::IoCtx::from_rados_ioctx_t(dest_p, dest_io_ctx);
...
extern "C" int rbd_copy_with_progress4(rbd_image_t image, rados_ioctx_t dest_p, const char* destname,
                                       rbd_image_options_t dest_opts, librbd_progress_fn_t fn, void* data,
                                       size_t sparse_size)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
...
extern "C" int rbd_deep_copy(rbd_image_t image, rados_ioctx_t dest_p, const char* destname, rbd_image_options_t c_opts)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librados::IoCtx dest_io_ctx;
    librados::IoCtx::from_rados_ioctx_t(dest_p, dest_io_ctx);
...
extern "C" int rbd_deep_copy_with_progress(rbd_image_t image, rados_ioctx_t dest_p, const char* destname,
                                           rbd_image_options_t dest_opts, librbd_progress_fn_t fn, void* data)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librados::IoCtx dest_io_ctx;
    ...
    librbd::CProgressContext prog_ctx(fn, data);
    int ret = librbd::api::Image<>::deep_copy(ictx, dest_io_ctx, destname, opts, prog_ctx);
    tracepoint(librbd, deep_copy_exit, ret);
    return ret;
}

extern "C" int rbd_encryption_format(rbd_image_t image, rbd_encryption_format_t format, rbd_encryption_options_t opts,
                                     size_t opts_size)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    return librbd::api::Image<>::encryption_format(ictx, format, opts, opts_size, true);
}

extern "C" int rbd_encryption_load(rbd_image_t image, rbd_encryption_format_t format, rbd_encryption_options_t opts,
                                   size_t opts_size)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
...
extern "C" int rbd_encryption_load2(rbd_image_t image, const rbd_encryption_spec_t* specs, size_t spec_count)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
...
extern "C" int rbd_flatten_with_progress(rbd_image_t image, librbd_progress_fn_t cb, void* cbdata)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, flatten_enter, ictx, ictx->name.c_str(), ictx->id.c_str());
    librbd::CProgressContext prog_ctx(cb, cbdata);
...
extern "C" int rbd_sparsify_with_progress(rbd_image_t image, size_t sparse_size, librbd_progress_fn_t cb, void* cbdata)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, sparsify_enter, ictx, ictx->name.c_str(), sparse_size, ictx->id.c_str());
    librbd::CProgressContext prog_ctx(cb, cbdata);
...
extern "C" int rbd_migration_prepare(rados_ioctx_t p, const char* image_name, rados_ioctx_t dest_p,
...
extern "C" int rbd_migration_prepare_import(const char* source_spec, rados_ioctx_t dest_p, const char* dest_image_name,
...
extern "C" int rbd_migration_execute_with_progress(rados_ioctx_t p, const char* name, librbd_progress_fn_t fn,
                                                   void* data)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, migration_execute_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), name);
    librbd::CProgressContext prog_ctx(fn, data);
...
extern "C" int rbd_migration_abort_with_progress(rados_ioctx_t p, const char* name, librbd_progress_fn_t fn, void* data)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, migration_abort_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), name);
    librbd::CProgressContext prog_ctx(fn, data);
...
extern "C" int rbd_migration_commit_with_progress(rados_ioctx_t p, const char* name, librbd_progress_fn_t fn,
                                                  void* data)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, migration_commit_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), name);
    librbd::CProgressContext prog_ctx(fn, data);
...
extern "C" int rbd_migration_status(rados_ioctx_t p, const char* image_name, rbd_image_migration_status_t* status,
                                    size_t status_size)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    ...
    if (r >= 0) {
        ...
        status->source_pool_namespace = strdup(cpp_status.source_pool_namespace.c_str());
        ...
        status->dest_pool_namespace = strdup(cpp_status.dest_pool_namespace.c_str());
...
extern "C" int rbd_pool_metadata_get(rados_ioctx_t p, const char* key, char* value, size_t* vallen)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    string val_s;
...
extern "C" int rbd_pool_metadata_set(rados_ioctx_t p, const char* key, const char* value)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_pool_metadata_list(rados_ioctx_t p, const char* start, uint64_t max, char* key, size_t* key_len,
...
extern "C" int rbd_open_by_id(rados_ioctx_t p, const char* id, rbd_image_t* image, const char* snap_name)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    librbd::ImageCtx* ictx = new librbd::ImageCtx("", id, snap_name, io_ctx, false);
    tracepoint(
        librbd, open_image_enter, ictx, ictx->name.c_str(), ictx->id.c_str(), ictx->snap_name.c_str(), ictx->read_only);
...
extern "C" int rbd_aio_open(rados_ioctx_t p, const char* name, rbd_image_t* image, const char* snap_name,
                            rbd_completion_t c)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    ...
    tracepoint(librbd,
               ...
               ictx->read_only,
               comp->pc);
    ictx->state->open(0, new C_OpenComplete(ictx, get_aio_completion(comp), image));
    tracepoint(librbd, aio_open_image_exit, 0);
    return 0;
}

extern "C" int rbd_aio_open_by_id(rados_ioctx_t p, const char* id, rbd_image_t* image, const char* snap_name,
                                  rbd_completion_t c)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_open_read_only(rados_ioctx_t p, const char* name, rbd_image_t* image, const char* snap_name)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    librbd::ImageCtx* ictx = new librbd::ImageCtx(name, "", snap_name, io_ctx, true);
    tracepoint(
        librbd, open_image_enter, ictx, ictx->name.c_str(), ictx->id.c_str(), ictx->snap_name.c_str(), ictx->read_only);
...
extern "C" int rbd_open_by_id_read_only(rados_ioctx_t p, const char* id, rbd_image_t* image, const char* snap_name)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    librbd::ImageCtx* ictx = new librbd::ImageCtx("", id, snap_name, io_ctx, true);
    tracepoint(
        librbd, open_image_enter, ictx, ictx->name.c_str(), ictx->id.c_str(), ictx->snap_name.c_str(), ictx->read_only);
...
extern "C" int rbd_aio_open_read_only(rados_ioctx_t p, const char* name, rbd_image_t* image, const char* snap_name,
                                      rbd_completion_t c)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_aio_open_by_id_read_only(rados_ioctx_t p, const char* id, rbd_image_t* image, const char* snap_name,
                                            rbd_completion_t c)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_aio_close(rbd_image_t image, rbd_completion_t c)
{
    ...
    ictx->state->close(new C_AioCompletion(ictx, librbd::io::AIO_TYPE_CLOSE, get_aio_completion(comp)));
    tracepoint(librbd, aio_close_image_exit, 0);
    return 0;
}
...
extern "C" int rbd_resize2(rbd_image_t image, uint64_t size, bool allow_shrink, librbd_progress_fn_t cb, void* cbdata)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, resize_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, size);
    librbd::CProgressContext prog_ctx(cb, cbdata);
...
extern "C" int rbd_resize_with_progress(rbd_image_t image, uint64_t size, librbd_progress_fn_t cb, void* cbdata)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, resize_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, size);
    librbd::CProgressContext prog_ctx(cb, cbdata);
...
extern "C" int rbd_update_features(rbd_image_t image, uint64_t features, uint8_t enabled)
{
...
extern "C" int rbd_get_create_timestamp(rbd_image_t image, struct timespec* timestamp)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, get_create_timestamp_enter, ictx, ictx->name.c_str(), ictx->read_only);
    utime_t time = ictx->get_create_timestamp();
...
extern "C" int rbd_get_access_timestamp(rbd_image_t image, struct timespec* timestamp)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, get_access_timestamp_enter, ictx, ictx->name.c_str(), ictx->read_only);
    utime_t time = ictx->get_access_timestamp();
...
extern "C" int rbd_get_modify_timestamp(rbd_image_t image, struct timespec* timestamp)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, get_modify_timestamp_enter, ictx, ictx->name.c_str(), ictx->read_only);
    utime_t time = ictx->get_modify_timestamp();
...
extern "C" int rbd_get_block_name_prefix(rbd_image_t image, char* prefix, size_t prefix_len)
{
    librbd::ImageCtx* ictx = reinterpret_cast<librbd::ImageCtx*>(image);
...
extern "C" int rbd_get_parent_info(rbd_image_t image, char* parent_pool_name, size_t ppool_namelen, char* parent_name,
...
extern "C" int rbd_get_parent_info2(rbd_image_t image, char* parent_pool_name, size_t ppool_namelen, char* parent_name,
                                    size_t pnamelen, char* parent_id, size_t pidlen, char* parent_snap_name,
...
extern "C" int rbd_get_parent(rbd_image_t image, rbd_linked_image_spec_t* parent_image, rbd_snap_spec_t* parent_snap)
{
    auto ictx = reinterpret_cast<librbd::ImageCtx*>(image);
    tracepoint(librbd, get_parent_info_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only);
    ...
    int r = librbd::api::Image<>::get_parent(ictx, &cpp_parent_image, &cpp_parent_snap);
    if (r < 0) {
...
extern "C" int rbd_get_migration_source_spec(rbd_image_t image, char* source_spec, size_t* max_len)
{
    auto ictx = reinterpret_cast<librbd::ImageCtx*>(image);
...
extern "C" int rbd_get_group(rbd_image_t image, rbd_group_info_t* group_info, size_t group_info_size)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, image_get_group_enter, ictx->name.c_str());
...
extern "C" int rbd_lock_get_owners(rbd_image_t image, rbd_lock_mode_t* lock_mode, char** lock_owners,
...
extern "C" void rbd_lock_get_owners_cleanup(char** lock_owners, size_t lock_owner_count)
{
...
extern "C" int rbd_lock_break(rbd_image_t image, rbd_lock_mode_t lock_mode, const char* lock_owner)
{
    librbd::ImageCtx* ictx = reinterpret_cast<librbd::ImageCtx*>(image);
...
extern "C" int rbd_rebuild_object_map(rbd_image_t image, librbd_progress_fn_t cb, void* cbdata)
{
    librbd::ImageCtx* ictx = reinterpret_cast<librbd::ImageCtx*>(image);
    librbd::CProgressContext prog_ctx(cb, cbdata);
    return ictx->operations->rebuild_object_map(prog_ctx);
...
extern "C" int rbd_snap_create2(rbd_image_t image, const char* snap_name, uint32_t flags, librbd_progress_fn_t cb,
                                void* cbdata)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(
...
extern "C" int rbd_snap_remove2(rbd_image_t image, const char* snap_name, uint32_t flags, librbd_progress_fn_t cb,
                                void* cbdata)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd,
...
extern "C" int rbd_snap_rollback_with_progress(rbd_image_t image, const char* snap_name, librbd_progress_fn_t cb,
                                               void* cbdata)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(
...
extern "C" int rbd_snap_list(rbd_image_t image, rbd_snap_info_t* snaps, int* max_snaps)
{
    ...
    for (i = 0; i < (int)cpp_snaps.size(); i++) {
        ...
        if (!snaps[i].name) {
            for (int j = 0; j < i; j++) free((void*)snaps[j].name);
...
extern "C" int rbd_snap_is_protected(rbd_image_t image, const char* snap_name, int* is_protected)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(
        librbd, snap_is_protected_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, snap_name);
...
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, snap_exists_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, snapname);
...
extern "C" ssize_t rbd_list_children(rbd_image_t image, char* pools, size_t* pools_len, char* images,
                                     size_t* images_len)
{
    auto ictx = reinterpret_cast<librbd::ImageCtx*>(image);
    tracepoint(librbd, list_children_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only);
    ...
    if (pools_total > *pools_len) too_short = true;
    if (images_total > *images_len) too_short = true;
...
extern "C" int rbd_list_children2(rbd_image_t image, rbd_child_info_t* children, int* max_children)
{
    auto ictx = reinterpret_cast<librbd::ImageCtx*>(image);
    tracepoint(librbd, list_children_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only);
    // FIPS zeroization audit 20191117: this memset is not security related.
...
extern "C" void rbd_list_children_cleanup(rbd_child_info_t* children, size_t num_children)
{
...
extern "C" int rbd_list_children3(rbd_image_t image, rbd_linked_image_spec_t* images, size_t* max_images)
{
    auto ictx = reinterpret_cast<librbd::ImageCtx*>(image);
    tracepoint(librbd, list_children_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only);
    // FIPS zeroization audit 20191117: this memset is not security related.
    ...
    *max_images = cpp_children.size();
    for (size_t idx = 0; idx < cpp_children.size(); ++idx) {
        ...
        tracepoint(librbd, list_children_entry, images[idx].pool_name, images[idx].image_name);
    }
    return 0;
}

extern "C" int rbd_list_descendants(rbd_image_t image, rbd_linked_image_spec_t* images, size_t* max_images)
{
    auto ictx = reinterpret_cast<librbd::ImageCtx*>(image);
    // FIPS zeroization audit 20191117: this memset is not security related.
    memset(images, 0, sizeof(*images) * *max_images);
...
extern "C" ssize_t rbd_list_lockers(rbd_image_t image, int* exclusive, char* tag, size_t* tag_len, char* clients,
                                    size_t* clients_len, char* cookies, size_t* cookies_len, char* addrs,
                                    size_t* addrs_len)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, list_lockers_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only);
    ...
    *exclusive = (int)exclusive_bool;
    ...
    for (list<librbd::locker_t>::const_iterator it = lockers.begin(); it != lockers.end(); ++it) {
        clients_total += it->client.length() + 1;
        cookies_total += it->cookie.length() + 1;
        addrs_total += it->address.length() + 1;
    }

    bool too_short = ((clients_total > *clients_len) || (cookies_total > *cookies_len) || (addrs_total > *addrs_len) ||
...
extern "C" int rbd_lock_exclusive(rbd_image_t image, const char* cookie)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(
        librbd, lock_exclusive_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, cookie);
    int r = librbd::lock(ictx, true, cookie ? cookie : "", "");
    tracepoint(librbd, lock_exclusive_exit, r);
    return r;
}

extern "C" int rbd_lock_shared(rbd_image_t image, const char* cookie, const char* tag)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(
        librbd, lock_shared_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, cookie, tag);
...
extern "C" int rbd_break_lock(rbd_image_t image, const char* client, const char* cookie)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(
        librbd, break_lock_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, client, cookie);
...
extern "C" ssize_t rbd_read(rbd_image_t image, uint64_t ofs, size_t len, char* buf)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, read_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, ofs, len);
    int r = librbd::api::Io<>::read(*ictx, ofs, len, librbd::io::ReadResult{buf, len}, 0);
    tracepoint(librbd, read_exit, r);
    return r;
}

extern "C" ssize_t rbd_read2(rbd_image_t image, uint64_t ofs, size_t len, char* buf, int op_flags)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(
        librbd, read2_enter, ictx, ictx->name.c_str(), ictx->snap_name.c_str(), ictx->read_only, ofs, len, op_flags);
    int r = librbd::api::Io<>::read(*ictx, ofs, len, librbd::io::ReadResult{buf, len}, op_flags);
    tracepoint(librbd, read_exit, r);
    return r;
}
...
extern "C" int rbd_diff_iterate(rbd_image_t image, const char* fromsnapname, uint64_t ofs, uint64_t len,
                                int (*cb)(uint64_t, size_t, int, void*), void* arg)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd,
...
extern "C" int rbd_diff_iterate2(rbd_image_t image, const char* fromsnapname, uint64_t ofs, uint64_t len,
                                 uint8_t include_parent, uint8_t whole_object, int (*cb)(uint64_t, size_t, int, void*),
                                 void* arg)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd,
...
extern "C" ssize_t rbd_write2(rbd_image_t image, uint64_t ofs, size_t len, const char* buf, int op_flags)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd,
               write2_enter,
...
extern "C" int rbd_discard(rbd_image_t image, uint64_t ofs, uint64_t len)
{
    ...
    int r = librbd::api::Io<>::discard(*ictx, ofs, len, ictx->discard_granularity_bytes);
    tracepoint(librbd, discard_exit, r);
    return r;
}

extern "C" ssize_t rbd_writesame(rbd_image_t image, uint64_t ofs, size_t len, const char* buf, size_t data_len,
                                 int op_flags)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd,
    ...
    if (data_len == 0 || len % data_len || len > static_cast<uint64_t>(std::numeric_limits<int>::max())) {
        tracepoint(librbd, writesame_exit, -EINVAL);
        return -EINVAL;
    }
    ...
    int r = librbd::api::Io<>::write_same(*ictx, ofs, len, std::move(bl), op_flags);
    tracepoint(librbd, writesame_exit, r);
    return r;
}

extern "C" ssize_t rbd_write_zeroes(rbd_image_t image, uint64_t ofs, size_t len, int zero_flags, int op_flags)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    return librbd::api::Io<>::write_zeroes(*ictx, ofs, len, zero_flags, op_flags);
}

extern "C" ssize_t rbd_compare_and_write(rbd_image_t image, uint64_t ofs, size_t len, const char* cmp_buf,
                                         const char* buf, uint64_t* mismatch_off, int op_flags)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd,
               compare_and_write_enter,
    ...
    int r =
        librbd::api::Io<>::compare_and_write(*ictx, ofs, len, std::move(cmp_bl), std::move(bl), mismatch_off, op_flags);
    tracepoint(librbd, compare_and_write_exit, r);
    return r;
}
...
extern "C" int rbd_aio_write(rbd_image_t image, uint64_t off, size_t len, const char* buf, rbd_completion_t c)
{
    ...
    librbd::api::Io<>::aio_write(*ictx, aio_completion, off, len, std::move(bl), 0, true);
    tracepoint(librbd, aio_write_exit, 0);
    return 0;
}

extern "C" int rbd_aio_write2(rbd_image_t image, uint64_t off, size_t len, const char* buf, rbd_completion_t c,
                              int op_flags)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;
    ...
    bufferlist bl;
    bl.push_back(create_write_raw(ictx, buf, len, aio_completion));
    librbd::api::Io<>::aio_write(*ictx, aio_completion, off, len, std::move(bl), op_flags, true);
    tracepoint(librbd, aio_write_exit, 0);
    return 0;
}
...
extern "C" int rbd_aio_read(rbd_image_t image, uint64_t off, size_t len, char* buf, rbd_completion_t c)
{
    ...
    tracepoint(librbd,
               ...
               buf,
               comp->pc);
    librbd::api::Io<>::aio_read(*ictx, get_aio_completion(comp), off, len, librbd::io::ReadResult{buf, len}, 0, true);
    tracepoint(librbd, aio_read_exit, 0);
    return 0;
}

extern "C" int rbd_aio_read2(rbd_image_t image, uint64_t off, size_t len, char* buf, rbd_completion_t c, int op_flags)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;
    tracepoint(librbd,
    ...
    librbd::api::Io<>::aio_read(
        *ictx, get_aio_completion(comp), off, len, librbd::io::ReadResult{buf, len}, op_flags, true);
    tracepoint(librbd, aio_read_exit, 0);
    return 0;
}

extern "C" int rbd_aio_readv(rbd_image_t image, const struct iovec* iov, int iovcnt, uint64_t off, rbd_completion_t c)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;
    ...
    if (r == 0) {
        ...
        if (iovcnt == 1) {
            read_result = librbd::io::ReadResult(static_cast<char*>(iov[0].iov_base), iov[0].iov_len);
        }
        else {
            read_result = librbd::io::ReadResult(iov, iovcnt);
        }
        librbd::api::Io<>::aio_read(*ictx, get_aio_completion(comp), off, len, std::move(read_result), 0, true);
    }
...
extern "C" int rbd_aio_discard(rbd_image_t image, uint64_t off, uint64_t len, rbd_completion_t c)
{
    ...
    tracepoint(librbd,
               ...
               len,
               comp->pc);
    librbd::api::Io<>::aio_discard(*ictx, get_aio_completion(comp), off, len, ictx->discard_granularity_bytes, true);
    tracepoint(librbd, aio_discard_exit, 0);
    return 0;
}

extern "C" int rbd_aio_writesame(rbd_image_t image, uint64_t off, size_t len, const char* buf, size_t data_len,
                                 rbd_completion_t c, int op_flags)
{
    ...
    bool discard_zero = ictx->config.get_val<bool>("rbd_discard_on_zeroed_write_same");
    if (discard_zero && mem_is_zero(buf, data_len)) {
        librbd::api::Io<>::aio_write_zeroes(*ictx, get_aio_completion(comp), off, len, 0, op_flags, true);
        tracepoint(librbd, aio_writesame_exit, 0);
        return 0;
    }
    ...
    librbd::api::Io<>::aio_write_same(*ictx, aio_completion, off, len, std::move(bl), op_flags, true);
    tracepoint(librbd, aio_writesame_exit, 0);
    return 0;
}

extern "C" int rbd_aio_write_zeroes(rbd_image_t image, uint64_t off, size_t len, rbd_completion_t c, int zero_flags,
                                    int op_flags)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;

    librbd::api::Io<>::aio_write_zeroes(*ictx, get_aio_completion(comp), off, len, zero_flags, op_flags, true);
    return 0;
}

extern "C" ssize_t rbd_aio_compare_and_write(rbd_image_t image, uint64_t off, size_t len, const char* cmp_buf,
                                             const char* buf, rbd_completion_t c, uint64_t* mismatch_off, int op_flags)
{
    ...
    librbd::api::Io<>::aio_compare_and_write(
        *ictx, aio_completion, off, len, std::move(cmp_bl), std::move(bl), mismatch_off, op_flags, false);

    tracepoint(librbd, aio_compare_and_write_exit, 0);
    return 0;
}

extern "C" ssize_t rbd_aio_compare_and_writev(rbd_image_t image, uint64_t off, const struct iovec* cmp_iov,
                                              int cmp_iovcnt, const struct iovec* iov, int iovcnt, rbd_completion_t c,
                                              uint64_t* mismatch_off, int op_flags)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;
    ...
    auto bl = iovec_to_bufferlist(ictx, iov, iovcnt, aio_completion);
    librbd::api::Io<>::aio_compare_and_write(
        *ictx, aio_completion, off, cmp_len, std::move(cmp_bl), std::move(bl), mismatch_off, op_flags, false);
...
extern "C" int rbd_poll_io_events(rbd_image_t image, rbd_completion_t* comps, int numcomp)
{
    ...
    tracepoint(librbd, poll_io_events_exit, r);
    if (r > 0) {
        for (int i = 0; i < r; ++i) comps[i] = cs[i]->rbd_comp;
    }
    return r;
}
...
extern "C" int rbd_metadata_list(rbd_image_t image, const char* start, uint64_t max, char* key, size_t* key_len,
                                 char* value, size_t* val_len)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    tracepoint(librbd, metadata_list_enter, ictx);
    ...
    if (*key_len < key_total_len || *val_len < val_total_len) too_short = true;
    *key_len = key_total_len;
    *val_len = val_total_len;
    if (too_short) {
...
extern "C" int rbd_mirror_image_enable2(rbd_image_t image, rbd_mirror_image_mode_t mode)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    return librbd::api::Mirror<>::image_enable(ictx, mode, false);
}
...
extern "C" int rbd_mirror_image_create_snapshot(rbd_image_t image, uint64_t* snap_id)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    auto flags = librbd::util::get_default_snap_create_flags(ictx);
    return librbd::api::Mirror<>::image_snapshot_create(ictx, flags, snap_id);
...
extern "C" int rbd_mirror_image_create_snapshot2(rbd_image_t image, uint32_t flags, uint64_t* snap_id)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    return librbd::api::Mirror<>::image_snapshot_create(ictx, flags, snap_id);
}

extern "C" int rbd_mirror_image_get_info(rbd_image_t image, rbd_mirror_image_info_t* mirror_image_info,
                                         size_t info_size)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
...
extern "C" void rbd_mirror_image_get_info_cleanup(rbd_mirror_image_info_t* mirror_image_info)
{
    free(mirror_image_info->global_id);
}

extern "C" int rbd_mirror_image_get_mode(rbd_image_t image, rbd_mirror_image_mode_t* mode)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
...
extern "C" int rbd_mirror_image_get_global_status(rbd_image_t image, rbd_mirror_image_global_status_t* status,
                                                  size_t status_size)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
...
extern "C" int rbd_mirror_image_get_instance_id(rbd_image_t image, char* instance_id, size_t* instance_id_max_length)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
...
extern "C" int rbd_aio_mirror_image_promote(rbd_image_t image, bool force, rbd_completion_t c)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;
    librbd::api::Mirror<>::image_promote(
        ictx, force, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(comp)));
    return 0;
}

extern "C" int rbd_aio_mirror_image_demote(rbd_image_t image, rbd_completion_t c)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;
    librbd::api::Mirror<>::image_demote(
        ictx, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(comp)));
    return 0;
}

extern "C" int rbd_aio_mirror_image_get_info(rbd_image_t image, rbd_mirror_image_info_t* info, size_t info_size,
                                             rbd_completion_t c)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;
    ...
    auto ctx = new C_MirrorImageGetInfo(
        info, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(comp)));
    librbd::api::Mirror<>::image_get_info(ictx, &ctx->cpp_mirror_image_info, ctx);
    return 0;
}

extern "C" int rbd_aio_mirror_image_get_mode(rbd_image_t image, rbd_mirror_image_mode_t* mode, rbd_completion_t c)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;

    librbd::api::Mirror<>::image_get_mode(
        ictx, mode, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(comp)));
    return 0;
}

extern "C" int rbd_aio_mirror_image_get_global_status(rbd_image_t image, rbd_mirror_image_global_status_t* status,
                                                      size_t status_size, rbd_completion_t c)
{
    ...
    auto ctx = new C_MirrorImageGetGlobalStatus(
        status, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(comp)));
    librbd::api::Mirror<>::image_get_global_status(ictx, &ctx->cpp_mirror_image_global_status, ctx);
    return 0;
}
...
extern "C" int rbd_aio_mirror_image_create_snapshot(rbd_image_t image, uint32_t flags, uint64_t* snap_id,
                                                    rbd_completion_t c)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
    librbd::RBD::AioCompletion* comp = (librbd::RBD::AioCompletion*)c;
    ...
    librbd::api::Mirror<>::image_snapshot_create(
        ictx, flags, snap_id, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC, get_aio_completion(comp)));
    return 0;
}

extern "C" int rbd_update_watch(rbd_image_t image, uint64_t* handle, rbd_update_callback_t watch_cb, void* arg)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
...
extern "C" int rbd_group_create(rados_ioctx_t p, const char* name)
{
    ...
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, group_create_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), name);
...
extern "C" int rbd_group_remove(rados_ioctx_t p, const char* name)
{
    ...
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
    tracepoint(librbd, group_remove_enter, io_ctx.get_pool_name().c_str(), io_ctx.get_id(), name);
...
extern "C" int rbd_group_rename(rados_ioctx_t p, const char* src_name, const char* dest_name)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
    TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int rbd_group_image_add(rados_ioctx_t group_p, const char* group_name, rados_ioctx_t image_p,
...
extern "C" int rbd_group_image_remove(rados_ioctx_t group_p, const char* group_name, rados_ioctx_t image_p,
                                      const char* image_name)
{
    librados::IoCtx group_ioctx;
    librados::IoCtx image_ioctx;
...
extern "C" int rbd_group_image_remove_by_id(rados_ioctx_t group_p, const char* group_name, rados_ioctx_t image_p,
                                            const char* image_id)
{
    librados::IoCtx group_ioctx;
    librados::IoCtx image_ioctx;
...
extern "C" int rbd_group_image_list(rados_ioctx_t group_p, const char* group_name, rbd_group_image_info_t* images,
                                    size_t group_image_info_size, size_t* image_size)
{
    librados::IoCtx group_ioctx;
    librados::IoCtx::from_rados_ioctx_t(group_p, group_ioctx);
    ...
    int r = librbd::api::Group<>::image_list(group_ioctx, group_name, &cpp_images);
...
extern "C" int rbd_group_info_cleanup(rbd_group_info_t* group_info, size_t group_info_size)
{
    if (group_info_size != sizeof(rbd_group_info_t)) {
        return -ERANGE;
    }
...
extern "C" int rbd_group_image_list_cleanup(rbd_group_image_info_t* images, size_t group_image_info_size, size_t len)
{
    if (group_image_info_size != sizeof(rbd_group_image_info_t)) {
        return -ERANGE;
    }
...
extern "C" int rbd_group_snap_create(rados_ioctx_t group_p, const char* group_name, const char* snap_name)
{
    librados::IoCtx group_ioctx;
    librados::IoCtx::from_rados_ioctx_t(group_p, group_ioctx);
...
extern "C" int rbd_group_snap_create2(rados_ioctx_t group_p, const char* group_name, const char* snap_name,
                                      uint32_t flags)
{
    librados::IoCtx group_ioctx;
    librados::IoCtx::from_rados_ioctx_t(group_p, group_ioctx);
...
extern "C" int rbd_group_snap_remove(rados_ioctx_t group_p, const char* group_name, const char* snap_name)
{
    librados::IoCtx group_ioctx;
    librados::IoCtx::from_rados_ioctx_t(group_p, group_ioctx);
...
extern "C" int rbd_group_snap_rename(rados_ioctx_t group_p, const char* group_name, const char* old_snap_name,
                                     const char* new_snap_name)
{
    librados::IoCtx group_ioctx;
    librados::IoCtx::from_rados_ioctx_t(group_p, group_ioctx);
...
extern "C" int rbd_group_snap_list(rados_ioctx_t group_p, const char* group_name, rbd_group_snap_info_t* snaps,
...
extern "C" int rbd_group_snap_list_cleanup(rbd_group_snap_info_t* snaps, size_t group_snap_info_size, size_t len)
{
    if (group_snap_info_size != sizeof(rbd_group_snap_info_t)) {
        return -ERANGE;
    }
...
extern "C" int rbd_group_snap_rollback(rados_ioctx_t group_p, const char* group_name, const char* snap_name)
{
    librados::IoCtx group_ioctx;
    librados::IoCtx::from_rados_ioctx_t(group_p, group_ioctx);
...
extern "C" int rbd_group_snap_rollback_with_progress(rados_ioctx_t group_p, const char* group_name,
                                                     const char* snap_name, librbd_progress_fn_t cb, void* cbdata)
{
    librados::IoCtx group_ioctx;
    librados::IoCtx::from_rados_ioctx_t(group_p, group_ioctx);
...
extern "C" int rbd_snap_get_namespace_type(rbd_image_t image, uint64_t snap_id,
...
extern "C" int rbd_snap_get_group_namespace(rbd_image_t image, uint64_t snap_id, rbd_snap_group_namespace_t* group_snap,
                                            size_t snap_group_namespace_size)
{
    ...
    int r = librbd::api::Snapshot<>::get_group_namespace(ictx, snap_id, &group_namespace);
    if (r >= 0) {
        group_snap->group_pool = group_namespace.group_pool;
        group_snap->group_name = strdup(group_namespace.group_name.c_str());
        group_snap->group_snap_name = strdup(group_namespace.group_snap_name.c_str());
    }
...
extern "C" int rbd_snap_get_trash_namespace(rbd_image_t image, uint64_t snap_id, char* original_name, size_t max_length)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;

    std::string cpp_original_name;
    int r = librbd::api::Snapshot<>::get_trash_namespace(ictx, snap_id, &cpp_original_name);
    if (r < 0) {
        return r;
    }
...
extern "C" int rbd_snap_get_mirror_namespace(rbd_image_t image, uint64_t snap_id,
                                             rbd_snap_mirror_namespace_t* mirror_snap, size_t mirror_snap_size)
{
    ...
    int r = librbd::api::Snapshot<>::get_mirror_namespace(ictx, snap_id, &mirror_namespace);
    if (r < 0) {
        return r;
    }

    mirror_snap->state = mirror_namespace.state;
    mirror_snap->primary_mirror_uuid = strdup(mirror_namespace.primary_mirror_uuid.c_str());
    mirror_snap->primary_snap_id = mirror_namespace.primary_snap_id;
    mirror_snap->mirror_peer_uuids_count = mirror_namespace.mirror_peer_uuids.size();
    size_t len = 0;
    ...
    mirror_snap->last_copied_object_number = mirror_namespace.last_copied_object_number;

    return 0;
}

extern "C" int rbd_snap_mirror_namespace_cleanup(rbd_snap_mirror_namespace_t* mirror_snap, size_t mirror_snap_size)
{
    if (mirror_snap_size != sizeof(rbd_snap_mirror_namespace_t)) {
        return -ERANGE;
    }
...
extern "C" int rbd_watchers_list(rbd_image_t image, rbd_image_watcher_t* watchers, size_t* max_watchers)
{
...
extern "C" void rbd_watchers_list_cleanup(rbd_image_watcher_t* watchers, size_t num_watchers)
{
...
extern "C" int rbd_config_image_list(rbd_image_t image, rbd_config_option_t* options, int* max_options)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
...
extern "C" int rbd_quiesce_watch(rbd_image_t image, rbd_update_callback_t quiesce_cb,
                                 rbd_update_callback_t unquiesce_cb, void* arg, uint64_t* handle)
{
    librbd::ImageCtx* ictx = (librbd::ImageCtx*)image;
