...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::ConfigWatcher: " << __func__ << ": "

namespace librbd {

template<typename I> struct ConfigWatcher<I>::Observer : public md_config_obs_t
{
...
template<typename I> ConfigWatcher<I>::~ConfigWatcher()
{
    ceph_assert(m_observer == nullptr);
}

template<typename I> void ConfigWatcher<I>::init()
{
...
template<typename I> void ConfigWatcher<I>::shut_down()
{
    auto cct = m_image_ctx.cct;
    ldout(cct, 10) << dendl;
...
template<typename I> void ConfigWatcher<I>::handle_global_config_change(std::set<std::string> changed_keys)
{
