...
template<typename ImageCtxT = ImageCtx> class ObjectMap : public RefCountedObject
{
public:
    static ObjectMap* create(ImageCtxT& image_ctx, uint64_t snap_id) { return new ObjectMap(image_ctx, snap_id); }
    ...
    static std::string object_map_name(const std::string& image_id, uint64_t snap_id);
    ...
    inline void set_state(uint64_t object_no, uint8_t new_state, const boost::optional<uint8_t>& current_state)
    {
    ...
    void aio_resize(uint64_t new_size, uint8_t default_object_state, Context* on_finish);
    ...
    bool aio_update(uint64_t snap_id, uint64_t start_object_no, uint8_t new_state,
                    ...
                    bool ignore_enoent, T* callback_object)
    {
        return aio_update<T, MF>(snap_id,
                                 start_object_no,
                                 start_object_no + 1,
                                 new_state,
                                 current_state,
    ...
    template<typename T, void (T::*MF)(int) = &T::complete>
    bool aio_update(uint64_t snap_id, uint64_t start_object_no, uint64_t end_object_no, uint8_t new_state,
                    const boost::optional<uint8_t>& current_state, const ZTracer::Trace& parent_trace,
                    bool ignore_enoent, T* callback_object)
    {
        ...
        if (snap_id == CEPH_NOSNAP) {
            ...
            UpdateOperation update_operation(start_object_no,
                                             end_object_no,
                                             new_state,
                                             current_state,
                                             parent_trace,
...
private:
    struct UpdateOperation
    {
        ...
        UpdateOperation(uint64_t start_object_no, uint64_t end_object_no, uint8_t new_state,
                        const boost::optional<uint8_t>& current_state, const ZTracer::Trace& parent_trace,
                        bool ignore_enoent, Context* on_finish)
            : start_object_no(start_object_no)
    ...
    void handle_detained_aio_update(BlockGuardCell* cell, int r, Context* on_finish);

    void aio_update(uint64_t snap_id, uint64_t start_object_no, uint64_t end_object_no, uint8_t new_state,
                    const boost::optional<uint8_t>& current_state, const ZTracer::Trace& parent_trace,
                    bool ignore_enoent, Context* on_finish);
    bool update_required(const ceph::BitVector<2>::Iterator& it, uint8_t new_state);
};
