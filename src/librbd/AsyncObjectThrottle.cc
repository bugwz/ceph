...
template<typename T>
AsyncObjectThrottle<T>::AsyncObjectThrottle(const AsyncRequest<T>* async_request, T& image_ctx,
...
template<typename T> void AsyncObjectThrottle<T>::start_ops(uint64_t max_concurrent)
{
...
template<typename T> void AsyncObjectThrottle<T>::finish_op(int r)
{
    bool complete;
    {
...
template<typename T> void AsyncObjectThrottle<T>::start_next_op()
{
    bool done = false;
    while (!done) {
        if (m_async_request != NULL && m_async_request->is_canceled() && m_ret == 0) {
