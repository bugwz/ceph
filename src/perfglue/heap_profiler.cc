...
#pragma GCC diagnostic ignored "-Wformat-truncation"
    ...
    if (last_slash == NULL) {
        snprintf(profile_name, profile_name_len, "./%s.profile", g_conf()->name.to_cstr());
    }
    else {
        last_slash[1] = '\0';
        snprintf(profile_name, profile_name_len, "%s/%s.profile", path, g_conf()->name.to_cstr());
    }
#if __GNUC__ && __GNUC__ >= 8
...
void ceph_heap_profiler_handle_command(const std::vector<std::string>& cmd, std::ostream& out)
{
    ...
    if (cmd.size() == 1 && cmd[0] == "dump") {
        ...
        out << g_conf()->name << " release rate: " << std::setprecision(4) << ceph_heap_get_release_rate() << "\n";
    }
    else if (cmd.size() == 2 && cmd[0] == "set_release_rate") {
        try {
            double val = std::stod(cmd[1]);
            ceph_heap_set_release_rate(val);
            out << g_conf()->name << " release rate changed to: " << std::setprecision(4)
