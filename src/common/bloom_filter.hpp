...
public:
    ...
    inline virtual std::size_t size() const { return table_size_ * CHAR_BIT; }
...
protected:
    virtual std::pair<size_t /* bit_index */, size_t /* bit */> compute_indices(const bloom_type& hash) const
    {
    ...
    static std::pair<std::size_t /* salt_count */, std::size_t /* table_size */> find_optimal_parameters(
        std::size_t target_insert_count, double target_fpp)
    {
        /*
          Note:
...
public:
    ...
    inline std::size_t size() const override { return size_list.back() * CHAR_BIT; }

    inline bool compress(const double& target_ratio)
    {
        if (bit_table_.empty()) return false;
...
private:
    std::pair<size_t /* bit_index */, size_t /* bit */> compute_indices(const bloom_type& hash) const final
    {
