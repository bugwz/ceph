...
public:
    ...
    bloom_filter(const std::size_t& predicted_inserted_element_count, const double& false_positive_probability,
        ...
        , random_seed_((random_seed) ? random_seed : 0xA5A5A5A5)
    {
        ...
        find_optimal_parameters(
            predicted_inserted_element_count, false_positive_probability, &salt_count_, &table_size_);
        init();
    }

    bloom_filter(const std::size_t& salt_count, std::size_t table_size, const std::size_t& random_seed,
    ...
    virtual ~bloom_filter() { mempool::bloom_filter::alloc_byte.deallocate(bit_table_, table_size_); }

    inline bool operator!() const { return (0 == table_size_); }

    inline void clear()
    {
        if (bit_table_) std::fill_n(bit_table_, table_size_, 0x00);
        insert_count_ = 0;
    }
    ...
    template<typename InputIterator> inline void insert(const InputIterator begin, const InputIterator end)
    {
    ...
    inline virtual std::size_t size() const { return table_size_ * bits_per_char; }

    inline std::size_t element_count() const { return insert_count_; }

    inline bool is_full() const { return insert_count_ >= target_element_count_; }
    ...
    inline const cell_type* table() const { return bit_table_; }
...
protected:
    ...
    static void find_optimal_parameters(std::size_t target_insert_count, double target_fpp, std::size_t* salt_count,
...
public:
    ...
    compressible_bloom_filter(const std::size_t& salt_count, std::size_t table_size, const std::size_t& random_seed,
    ...
    inline std::size_t size() const override { return size_list.back() * bits_per_char; }
    ...
    inline bool compress(const double& target_ratio)
    {
        ...
        while (end != itr) {
            *(itr_tmp++) |= (*itr++);
            if (itr_tmp == itr_end) itr_tmp = tmp;
        }
