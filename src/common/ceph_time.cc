...
void real_clock::to_ceph_timespec(const time_point& t, struct ceph_timespec& ts)
{
...
real_clock::time_point real_clock::from_ceph_timespec(const struct ceph_timespec& ts)
{
    return time_point(seconds(ts.tv_sec) + nanoseconds(ts.tv_nsec));
}

void coarse_real_clock::to_ceph_timespec(const time_point& t, struct ceph_timespec& ts)
{
    ts.tv_sec = to_time_t(t);
    ts.tv_nsec = (t.time_since_epoch() % seconds(1)).count();
}
struct ceph_timespec coarse_real_clock::to_ceph_timespec(const time_point& t)
{
    struct ceph_timespec ts;
    to_ceph_timespec(t, ts);
    return ts;
}
coarse_real_clock::time_point coarse_real_clock::from_ceph_timespec(const struct ceph_timespec& ts)
{
    return time_point(seconds(ts.tv_sec) + nanoseconds(ts.tv_nsec));
}
...
std::ostream& operator<<(std::ostream& m, const std::chrono::time_point<Clock>& t)
{
    ...
    m << std::setw(4) << (bdt.tm_year + 1900)   // 2007 -> '07'
      ...
      << bdt.tm_hour << ':' << std::setw(2) << bdt.tm_min << ':' << std::setw(2) << bdt.tm_sec << "." << std::setw(6)
...
template ostream& operator<< <chrono::milliseconds::rep, chrono::milliseconds::period>(ostream&,
