...
double bloom_filter::density() const
{
    ...
    unsigned set =
        std::accumulate(bit_table_.begin(), bit_table_.begin() + table_size_, 0u, [](unsigned set, cell_type cell) {
...
void bloom_filter::dump(Formatter* f) const
{
    ...
    for (std::vector<bloom_type>::const_iterator i = salt_.begin(); i != salt_.end(); ++i) f->dump_unsigned("salt", *i);
    f->close_section();
...
void compressible_bloom_filter::encode(bufferlist& bl) const
{
    ...
    encode(s, bl);
    for (std::vector<size_t>::const_iterator p = size_list.begin(); p != size_list.end(); ++p) encode((uint64_t)*p, bl);
...
void compressible_bloom_filter::dump(Formatter* f) const
{
    ...
    for (std::vector<size_t>::const_iterator p = size_list.begin(); p != size_list.end(); ++p)
