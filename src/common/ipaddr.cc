...
void netmask_ipv4(const struct in_addr* addr, unsigned int prefix_len, struct in_addr* out)
{
    uint32_t mask;
...
bool matches_ipv4_in_subnet(const struct ifaddrs& addrs, const struct sockaddr_in* net, unsigned int prefix_len)
{
    if (addrs.ifa_addr == nullptr) return false;

    if (addrs.ifa_addr->sa_family != net->sin_family) return false;
    struct in_addr want;
...
void netmask_ipv6(const struct in6_addr* addr, unsigned int prefix_len, struct in6_addr* out)
{
    if (prefix_len > 128) prefix_len = 128;

    memcpy(out->s6_addr, addr->s6_addr, prefix_len / 8);
    if (prefix_len < 128) out->s6_addr[prefix_len / 8] = addr->s6_addr[prefix_len / 8] & ~(0xFF >> (prefix_len % 8));
    if (prefix_len / 8 < 15) memset(out->s6_addr + prefix_len / 8 + 1, 0, 16 - prefix_len / 8 - 1);
}

bool matches_ipv6_in_subnet(const struct ifaddrs& addrs, const struct sockaddr_in6* net, unsigned int prefix_len)
{
    if (addrs.ifa_addr == nullptr) return false;

    if (addrs.ifa_addr->sa_family != net->sin6_family) return false;
    ...
    if (IN6_IS_ADDR_LINKLOCAL(cur)) return false;
...
bool parse_network(const char* s, entity_addr_t* network, unsigned int* prefix_len)
{
    sockaddr_storage ss;
...
bool network_contains(const struct entity_addr_t& network, unsigned int prefix_len, const struct entity_addr_t& addr)
{
    ...
    {
        struct in6_addr a, b;
        netmask_ipv6(&((const sockaddr_in6*)network.get_sockaddr())->sin6_addr, prefix_len, &a);
        netmask_ipv6(&((const sockaddr_in6*)addr.get_sockaddr())->sin6_addr, prefix_len, &b);
        if (memcmp(&a, &b, sizeof(a)) == 0) {
            return true;
        }
    } break;
