...
class Finisher
{
    ...
    {
        Finisher* fin;
        explicit FinisherThread(Finisher* f)
            : fin(f)
        {}
...
public:
    ...
    void queue(Context* c, int r = 0)
    {
        ...
        if (logger) logger->inc(l_finisher_queue_len);
    }
    ...
    {
        {
            ...
            if (logger) logger->inc(l_finisher_queue_len, ls.size());
        }
        ls.clear();
    }
    ...
    Finisher(CephContext* cct_, std::string name, std::string tn)
        ...
        , finisher_thread(this)
    {
        PerfCountersBuilder b(cct, std::string("finisher-") + name, l_finisher_first, l_finisher_last);
...
public:
    C_OnFinisher(Context* c, Finisher* f)
        : con(c)
        , fin(f)
    {
...
public:
    ContextQueue(ceph::mutex& mut, ceph::condition_variable& con)
        : mutex(mut)
        , cond(con)
    {}
