...
void SloppyCRCMap::write(uint64_t offset, uint64_t len, const bufferlist& bl, std::ostream* out)
{
    int64_t left = len;
    uint64_t pos = offset;
    ...
    if (o) {
        ...
        if (out) *out << "write invalidate " << (offset - o) << "\n";
        pos += (block_size - o);
        left -= (block_size - o);
    }
    while (left >= block_size) {
        ...
        if (out) *out << "write set " << pos << " " << crc_map[pos] << "\n";
        pos += block_size;
        left -= block_size;
    }
    if (left > 0) {
        crc_map.erase(pos);
        if (out) *out << "write invalidate " << pos << "\n";
    }
}

int SloppyCRCMap::read(uint64_t offset, uint64_t len, const bufferlist& bl, std::ostream* err)
{
    int errors = 0;
    int64_t left = len;
    uint64_t pos = offset;
    ...
    while (left >= block_size) {
        ...
        if (p != crc_map.end()) {
            ...
            if (p->second != crc) {
                errors++;
                if (err)
                    *err << "offset " << pos << " len " << block_size << " has crc " << crc << " expected " << p->second
                         << "\n";
            }
        }
        pos += block_size;
...
void SloppyCRCMap::truncate(uint64_t offset)
{
    ...
    while (p != crc_map.end()) crc_map.erase(p++);
}
...
void SloppyCRCMap::zero(uint64_t offset, uint64_t len)
{
    ...
    if (left > 0) crc_map.erase(pos);
}

void SloppyCRCMap::clone_range(uint64_t offset, uint64_t len, uint64_t srcoff, const SloppyCRCMap& src,
                               std::ostream* out)
{
    int64_t left = len;
    uint64_t pos = offset;
    ...
    if (o) {
        ...
        srcpos += (block_size - o);
        left -= (block_size - o);
        if (out) *out << "clone_range invalidate " << (offset - o) << "\n";
    }
    while (left >= block_size) {
        ...
        if (block_size == src.block_size) {
            ...
            if (p != src.crc_map.end()) {
                ...
                if (out) *out << "clone_range copy " << pos << " " << p->second << "\n";
            }
            else {
                crc_map.erase(pos);
                if (out) *out << "clone_range invalidate " << pos << "\n";
            }
        }
        else {
            crc_map.erase(pos);
