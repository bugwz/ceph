...
template<typename K, typename V, typename VToK> struct intrusive_lru_config
{
    using key_type = K;
    using value_type = V;
...
template<typename Config> class intrusive_lru_base;

template<typename Config> void intrusive_ptr_add_ref(intrusive_lru_base<Config>* p);

template<typename Config> void intrusive_ptr_release(intrusive_lru_base<Config>* p);
...
template<typename Config> class intrusive_lru
{
    ...
    using lru_set_option_t =
        boost::intrusive::member_hook<base_t, boost::intrusive::set_member_hook<>, &base_t::set_hook>;

    using VToK = typename Config::key_of_value;
    struct VToKWrapped
    {
        using type = typename VToK::type;
        const type& operator()(const base_t& obc) { return VToK()(static_cast<const T&>(obc)); }
    };
    using lru_set_t = boost::intrusive::set<base_t, lru_set_option_t, boost::intrusive::key_of_value<VToKWrapped>>;
    lru_set_t lru_set;

    using lru_list_t = boost::intrusive::list<
        base_t, boost::intrusive::member_hook<base_t, boost::intrusive::list_member_hook<>, &base_t::list_hook>>;
    ...
    void evict()
    {
        while (!unreferenced_list.empty() && lru_set.size() > lru_target_size) {
            auto& b = unreferenced_list.front();
            assert(!b.lru);
            unreferenced_list.pop_front();
            lru_set.erase_and_dispose(lru_set.iterator_to(b), [](auto* p) { delete p; });
        }
    }
...
public:
    ...
    std::pair<TRef, bool> get_or_create(const K& k)
    {
        ...
        auto [iter, missing] = lru_set.insert_check(k, icd);
        if (missing) {
            auto ret = new T(k);
     ...
     */
    void clear_range(const K& from, const K& to)
    {
        ...
        for (auto i = from_iter; i != to_iter;) {
            if (!(*i).lru) {
                ...
                i = lru_set.erase_and_dispose(i, [](auto* p) { delete p; });
            }
            else {
                i++;
            }
    ...
    ~intrusive_lru() { set_target_size(0); }
