...
size_t escape_xml_attr_len(const char* buf)
{
    ...
    for (b = buf; *b; ++b) {
        unsigned char c = *b;
        switch (c) {
        case '<': ret += SSTRL(LESS_THAN_XESCAPE); break;
        case '&': ret += SSTRL(AMPERSAND_XESCAPE); break;
        case '>': ret += SSTRL(GREATER_THAN_XESCAPE); break;
        case '\'': ret += SSTRL(SGL_QUOTE_XESCAPE); break;
        case '"': ret += SSTRL(DBL_QUOTE_XESCAPE); break;
        default:
...
std::ostream& operator<<(std::ostream& out, const xml_stream_escaper& e)
{
    ...
    for (unsigned char c : e.str) {
        switch (c) {
        case '<': out << LESS_THAN_XESCAPE; break;
        case '&': out << AMPERSAND_XESCAPE; break;
        case '>': out << GREATER_THAN_XESCAPE; break;
        case '\'': out << SGL_QUOTE_XESCAPE; break;
        case '"': out << DBL_QUOTE_XESCAPE; break;
        default:
            // Escape control characters.
            if (((c < 0x20) && (c != 0x09) && (c != 0x0a)) || (c == 0x7f)) {
                if (!fmt) {
...
size_t escape_json_attr_len(const char* buf, size_t src_len)
{
    ...
    for (i = 0, b = buf; i < src_len; ++i, ++b) {
        unsigned char c = *b;
        switch (c) {
        case '"': ret += SSTRL(DBL_QUOTE_JESCAPE); break;
        case '\\': ret += SSTRL(BACKSLASH_JESCAPE); break;
        case '\t': ret += SSTRL(TAB_JESCAPE); break;
        case '\n': ret += SSTRL(NEWLINE_JESCAPE); break;
        default:
            // Escape control characters.
            if ((c < 0x20) || (c == 0x7f)) {
                ret += 6;
...
std::ostream& operator<<(std::ostream& out, const json_stream_escaper& e)
{
    ...
    for (unsigned char c : e.str) {
        switch (c) {
        case '"': out << DBL_QUOTE_JESCAPE; break;
        case '\\': out << BACKSLASH_JESCAPE; break;
        case '\t': out << TAB_JESCAPE; break;
        case '\n': out << NEWLINE_JESCAPE; break;
        default:
            // Escape control characters.
            if ((c < 0x20) || (c == 0x7f)) {
                if (!fmt) {
