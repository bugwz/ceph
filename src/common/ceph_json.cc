...
vector<string> JSONObj::get_array_elements()
{
    ...
    if (data.type() == array_type) temp_array = data.get_array();
...
{
    return (boost::iequals(value.str, "true") || boost::iequals(value.str, "on") || boost::iequals(value.str, "yes") ||
...
struct field_entity
{
    ...
    field_entity() {}
    explicit field_entity(const string& n)
        : is_obj(true)
        , name(n)
    {}
    explicit field_entity(int i)
        : is_obj(false)
        , index(i)
    {}
};
...
{
    switch (type) {
    case JSONFormattable::FMT_VALUE: ::encode_json(name, value, f); break;
    case JSONFormattable::FMT_ARRAY: ::encode_json(name, arr, f); break;
    case JSONFormattable::FMT_OBJ:
        ...
        f->close_section();
        break;
    case JSONFormattable::FMT_NONE: break;
    }
}
...
bool JSONFormattable::handle_open_section(std::string_view name, const char* ns, bool section_is_array)
{
    if (cur_enc->is_array()) {
        cur_enc->arr.push_back(JSONFormattable());
