...
public:
    ...
    virtual int res_nquery(res_state s, const char* hostname, int cls, int type, u_char* buf, int bufsz);

    virtual int res_nsearch(res_state s, const char* hostname, int cls, int type, u_char* buf, int bufsz);
#else
    virtual int res_query(const char* hostname, int cls, int type, u_char* buf, int bufsz);

    virtual int res_search(const char* hostname, int cls, int type, u_char* buf, int bufsz);
#endif
};
...
public:
    ...
    int resolve_cname(CephContext* cct, const std::string& hostname, std::string* cname, bool* found);
     ...
     */
    int resolve_ip_addr(CephContext* cct, const std::string& hostname, entity_addr_t* addr);
     ...
     * @returns 0 on success, negative error code on failure
     */
    int resolve_srv_hosts(CephContext* cct, const std::string& service_name, const SRV_Protocol trans_protocol,
     ...
     */
    int resolve_srv_hosts(CephContext* cct, const std::string& service_name, const SRV_Protocol trans_protocol,
                          const std::string& domain, std::map<std::string, Record>* srv_hosts);
...
private:
     ...
     */
    int resolve_ip_addr(CephContext* cct, res_state* res, const std::string& hostname, entity_addr_t* addr);
#endif

    std::string srv_protocol_to_str(SRV_Protocol proto)
    {
        switch (proto) {
        case SRV_Protocol::TCP: return "tcp";
        case SRV_Protocol::UDP: return "udp";
        }
        return "";
    }
};
