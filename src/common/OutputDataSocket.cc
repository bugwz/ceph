...
static void remove_cleanup_file(const char* file)
{
    ...
    for (std::vector<const char*>::iterator i = cleanup_files.begin(); i != cleanup_files.end(); ++i) {
        if (strcmp(file, *i) == 0) {
            free((void*)*i);
...
void* OutputDataSocket::entry()
{
    ...
    while (true) {
        ...
        if (ret < 0) {
            ...
            lderr(m_cct) << "OutputDataSocket: poll(2) error: '" << cpp_strerror(err) << dendl;
            return PFL_FAIL;
        }
...
bool OutputDataSocket::do_accept()
{
    ...
    if (connection_fd < 0) {
        int err = errno;
        lderr(m_cct) << "OutputDataSocket: do_accept error: '" << cpp_strerror(err) << dendl;
        return false;
    }
