...
#define bdout                                           \
    {                                                   \
        std::lock_guard<ceph::spinlock> lg(debug_lock); \
        std::cout
#define bendl  \
    std::endl; \
    }
...
public:
    raw_combined(char* dataptr, unsigned l, unsigned align, int mempool)
        : raw(dataptr, l, mempool)
        , alignment(align)
    {}
    raw* clone_empty() override { return create(len, alignment).release(); }

    static ceph::unique_leakable_ptr<buffer::raw> create(unsigned len, unsigned align,
                                                         int mempool = mempool::mempool_buffer_anon)
    {
        ...
        return ceph::unique_leakable_ptr<buffer::raw>(new (ptr + datalen) raw_combined(ptr, len, align, mempool));
    }
...
public:
    ...
    raw* clone_empty() override { return new raw_malloc(len); }
};
...
public:
    ...
    raw_posix_aligned(unsigned l, unsigned _align)
        : raw(l)
    {
        // posix_memalign() requires a multiple of sizeof(void *)
...
#endif /* DARWIN */
        if (!data) throw bad_alloc();
        bdout << "raw_posix_aligned " << this << " alloc " << (void*)data << " l=" << l << ", align=" << align << bendl;
    }
    ...
    raw* clone_empty() override { return new raw_posix_aligned(len, align); }
};
#endif
...
public:
    raw_hack_aligned(unsigned l, unsigned _align)
        : raw(l)
    {
        align = _align;
    ...
    ~raw_hack_aligned() { delete[] realdata; }
    raw* clone_empty() { return new raw_hack_aligned(len, align); }
};
#endif
...
public:
    ...
    raw* clone_empty() override { return new raw_char(len); }
};
...
public:
    ...
    explicit raw_claimed_char(unsigned l, char* b)
        : raw(b, l)
    {
        bdout << "raw_claimed_char " << this << " alloc " << (void*)data << " " << l << bendl;
    }
    ~raw_claimed_char() override { bdout << "raw_claimed_char " << this << " free " << (void*)data << bendl; }
    raw* clone_empty() override { return new raw_char(len); }
};
...
public:
    ...
    {}
    ~raw_static() override {}
    raw* clone_empty() override { return new buffer::raw_char(len); }
};
...
ceph::unique_leakable_ptr<buffer::raw> buffer::claim_char(unsigned len, char* buf)
{
    return ceph::unique_leakable_ptr<buffer::raw>(new raw_claimed_char(len, buf));
}
...
ceph::unique_leakable_ptr<buffer::raw> buffer::claim_malloc(unsigned len, char* buf)
{
    return ceph::unique_leakable_ptr<buffer::raw>(new raw_malloc(len, buf));
}
ceph::unique_leakable_ptr<buffer::raw> buffer::create_static(unsigned len, char* buf)
{
    return ceph::unique_leakable_ptr<buffer::raw>(new raw_static(buf, len));
}
ceph::unique_leakable_ptr<buffer::raw> buffer::claim_buffer(unsigned len, char* buf, deleter del)
{
    return ceph::unique_leakable_ptr<buffer::raw>(new raw_claim_buffer(buf, len, std::move(del)));
}

ceph::unique_leakable_ptr<buffer::raw> buffer::create_aligned_in_mempool(unsigned len, unsigned align, int mempool)
{
    ...
    // size passes 8KB.
    if ((align & ~CEPH_PAGE_MASK) == 0 || len >= CEPH_PAGE_SIZE * 2) {
#ifndef __CYGWIN__
...
ceph::unique_leakable_ptr<buffer::raw> buffer::create_aligned(unsigned len, unsigned align)
{
    return create_aligned_in_mempool(len, align, mempool::mempool_buffer_anon);
}
...
void buffer::ptr::release()
{
    ...
    // checks.
    if (auto* const cached_raw = std::exchange(_raw, nullptr); cached_raw) {
        ...
        const bool last_one = (1 == cached_raw->nref.load(std::memory_order_acquire));
        if (likely(last_one) || --cached_raw->nref == 0) {
            bdout << "deleting raw " << static_cast<void*>(cached_raw) << " len " << cached_raw->get_len() << bendl;
...
{
    ceph_assert(_raw);
    if (o + l > _len) throw end_of_buffer();
...
void buffer::ptr::copy_in(unsigned o, unsigned l, const char* src, bool crc_reset)
{
    ...
    if (crc_reset) _raw->invalidate_crc();
...
template<bool B> buffer::ptr::iterator_impl<B>& buffer::ptr::iterator_impl<B>::operator+=(size_t len)
{
    pos += len;
    if (pos > end_ptr) throw end_of_buffer();
    return *this;
}

template buffer::ptr::iterator_impl<false>& buffer::ptr::iterator_impl<false>::operator+=(size_t len);
template buffer::ptr::iterator_impl<true>& buffer::ptr::iterator_impl<true>::operator+=(size_t len);
...
template<bool is_const> auto buffer::list::iterator_impl<is_const>::operator+=(unsigned o) -> iterator_impl&
{
...
template<bool is_const> void buffer::list::iterator_impl<is_const>::seek(unsigned o)
{
    p = ls->begin();
    off = p_off = 0;
    *this += o;
...
template<bool is_const> char buffer::list::iterator_impl<is_const>::operator*() const
{
    if (p == ls->end()) throw end_of_buffer();
    return (*p)[p_off];
}

template<bool is_const> buffer::list::iterator_impl<is_const>& buffer::list::iterator_impl<is_const>::operator++()
{
    if (p == ls->end()) throw end_of_buffer();
    *this += 1;
    return *this;
...
template<bool is_const> buffer::ptr buffer::list::iterator_impl<is_const>::get_current_ptr() const
{
    if (p == ls->end()) throw end_of_buffer();
    return ptr(*p, p_off, p->length() - p_off);
}

template<bool is_const> bool buffer::list::iterator_impl<is_const>::is_pointing_same_raw(const ptr& other) const
{
    if (p == ls->end()) throw end_of_buffer();
...
template<bool is_const> void buffer::list::iterator_impl<is_const>::copy(unsigned len, char* dest)
{
...
template<bool is_const> void buffer::list::iterator_impl<is_const>::copy(unsigned len, ptr& dest)
{
    copy_deep(len, dest);
}

template<bool is_const> void buffer::list::iterator_impl<is_const>::copy_deep(unsigned len, ptr& dest)
{
    if (!len) {
        return;
    }
...
template<bool is_const> void buffer::list::iterator_impl<is_const>::copy_shallow(unsigned len, ptr& dest)
{
    if (!len) {
        return;
    }
...
template<bool is_const> void buffer::list::iterator_impl<is_const>::copy(unsigned len, list& dest)
{
    if (p == ls->end()) seek(off);
    while (len > 0) {
        if (p == ls->end()) throw end_of_buffer();
...
template<bool is_const> void buffer::list::iterator_impl<is_const>::copy(unsigned len, std::string& dest)
{
    if (p == ls->end()) seek(off);
    while (len > 0) {
        if (p == ls->end()) throw end_of_buffer();
...
template<bool is_const> void buffer::list::iterator_impl<is_const>::copy_all(list& dest)
{
    if (p == ls->end()) seek(off);
    while (1) {
        if (p == ls->end()) return;
...
template<bool is_const>
size_t buffer::list::iterator_impl<is_const>::get_ptr_and_advance(size_t want, const char** data)
{
    if (p == ls->end()) {
        seek(off);
        if (p == ls->end()) {
...
template<bool is_const> uint32_t buffer::list::iterator_impl<is_const>::crc32c(size_t length, uint32_t crc)
{
...
bool buffer::list::contents_equal(const ceph::buffer::list& other) const
{
    ...
    if (true) {
        ...
        while (a != std::cend(_buffers)) {
            ...
            if (len > b->length() - boff) len = b->length() - boff;
            if (memcmp(a->c_str() + aoff, b->c_str() + boff, len) != 0) return false;
            aoff += len;
            if (aoff == a->length()) {
                aoff = 0;
                ++a;
...
bool buffer::list::contents_equal(const void* const other, size_t length) const
{
...
bool buffer::list::is_aligned_size_and_memory(const unsigned align_size, const unsigned align_memory) const
{
    for (const auto& node : _buffers) {
...
{
    if (_num == 1) return _buffers.back().wasted();

    std::vector<const raw*> raw_vec;
    raw_vec.reserve(_num);
    for (const auto& p : _buffers) raw_vec.push_back(p._raw);
...
void buffer::list::rebuild(std::unique_ptr<buffer::ptr_node, buffer::ptr_node::disposer> nb)
{
    unsigned pos = 0;
...
bool buffer::list::rebuild_aligned_size_and_memory(unsigned align_size, unsigned align_memory, unsigned max_buffers)
{
        ...
        do {
            ...
            */
            ...
            p = p_after;
                 ...
                 (!p->is_aligned(align_memory) || !p->is_n_align_sized(align_size) || (offset % align_size)));
        if (!(unaligned.is_contiguous() && unaligned._buffers.front().is_aligned(align_memory))) {
            unaligned.rebuild(ptr_node::create(buffer::create_aligned(unaligned._len, align_memory)));
            had_to_rebuild = true;
        }
...
void buffer::list::append(char c)
{
    ...
    if (!gap) {
        // make a new buffer!
        auto buf = ptr_node::create(raw_combined::create(CEPH_BUFFER_APPEND_SIZE, 0, get_mempool()));
...
buffer::list::reserve_t buffer::list::obtain_contiguous_space(const unsigned len)
{
...
{
    if (_buffers.empty()) return 0;   // no buffers
...
{
    if (off + len > other.length()) throw end_of_buffer();
...
{   // fixme?
    ...
    if (off >= length()) throw end_of_buffer();
    ...
    if (off) {
        ...
        _buffers.insert_after(curbuf_prev, *ptr_node::create(*curbuf, 0, off).release());
        _len += off;
        _num += 1;
        ++curbuf_prev;
    }
    ...
    while (len > 0) {
        ...
        if (const auto to_drop = off + len; to_drop < curbuf->length()) {
            ...
            if (claim_by) claim_by->append(*curbuf, off, len);
        ...
        if (claim_by) claim_by->append(*curbuf, off, howmuch);
        _len -= curbuf->length();
        if (curbuf == _carriage) {
...
ssize_t buffer::list::pread_file(const char* fn, uint64_t off, uint64_t len, std::string* error)
{
    ...
    if (::fstat(fd, &st) < 0) {
        int err = errno;
        std::ostringstream oss;
        oss << "bufferlist::read_file(" << fn << "): stat error: " << cpp_strerror(err);
        *error = oss.str();
    ...
    if (ret < 0) {
        std::ostringstream oss;
        oss << "bufferlist::read_file(" << fn << "): read error:" << cpp_strerror(ret);
        *error = oss.str();
        VOID_TEMP_FAILURE_RETRY(::close(fd));
        return ret;
    }
...
int buffer::list::write_file(const char* fn, int mode)
{
    ...
    if (fd < 0) {
        int err = errno;
        cerr << "bufferlist::write_file(" << fn << "): failed to open file: " << cpp_strerror(err) << std::endl;
        return -err;
    }
    int ret = write_fd(fd);
    if (ret) {
        cerr << "bufferlist::write_fd(" << fn << "): write_fd error: " << cpp_strerror(ret) << std::endl;
        VOID_TEMP_FAILURE_RETRY(::close(fd));
        return ret;
    }
    if (TEMP_FAILURE_RETRY(::close(fd))) {
        int err = errno;
        cerr << "bufferlist::write_file(" << fn << "): close error: " << cpp_strerror(err) << std::endl;
        return -err;
    }
    return 0;
}
...
int buffer::list::write_fd(int fd) const
{
    ...
    while (p != std::cend(_buffers)) {
        ...
        if (iovlen == IOV_MAX || p == _buffers.end()) {
            iovec* start = iov;
            int num = iovlen;
            ssize_t wrote;
        retry:
...
__u32 buffer::list::crc32c(__u32 crc) const
{
    ...
    if (buffer_track_crc) {
        if (cache_adjusts) buffer_cached_crc_adjusted += cache_adjusts;
        if (cache_hits) buffer_cached_crc += cache_hits;
        if (cache_misses) buffer_missed_crc += cache_misses;
    }
...
#define BUF_OFFSETOF(type, field) (reinterpret_cast<std::uintptr_t>(&(((type*)1024)->field)) - 1024u)
...
bool buffer::ptr_node::dispose_if_hypercombined(buffer::ptr_node* const delete_this)
{
    ...
    const std::uintptr_t bptr =
        (reinterpret_cast<std::uintptr_t>(delete_this->_raw) + BUF_OFFSETOF(buffer::raw, bptr_storage));
    const bool is_hypercombined = reinterpret_cast<std::uintptr_t>(delete_this) == bptr;
    if (is_hypercombined) {
...
std::unique_ptr<buffer::ptr_node, buffer::ptr_node::disposer> buffer::ptr_node::create_hypercombined(
    ceph::unique_leakable_ptr<buffer::raw> r)
{
...
buffer::ptr_node* buffer::ptr_node::cloner::operator()(const buffer::ptr_node& clone_this)
{
...
{
    return out << "buffer::raw(" << (void*)r.get_data() << " len " << r.get_len() << " nref " << r.nref.load() << ")";
}

std::ostream& buffer::operator<<(std::ostream& out, const buffer::ptr& bp)
{
    if (bp.have_raw())
        out << "buffer::ptr(" << bp.offset() << "~" << bp.length() << " " << (void*)bp.c_str() << " in raw "
            << (void*)bp.raw_c_str() << " len " << bp.raw_length() << " nref " << bp.raw_nref() << ")";
    else
...
{
    const size_t alloc = std::max((size_t)min_alloc, (len + CEPH_PAGE_SIZE - 1) & CEPH_PAGE_MASK);
    auto new_back = ptr_node::create(buffer::create_page_aligned(alloc));
    new_back->set_length(0);   // unused, so far.
...
public:
    ...
    bool equivalent(int ev, const boost::system::error_condition& c) const noexcept override;
...
const char* buffer_error_category::message(int ev, char*, std::size_t) const noexcept
{
    ...
    case errc::end_of_buffer: return "End of buffer";

    case errc::malformed_input: return "Malformed input";
    }
...
boost::system::error_condition buffer_error_category::default_error_condition(int ev) const noexcept
{
    using ceph::buffer::errc;
    switch (static_cast<errc>(ev)) {
    case errc::bad_alloc: return boost::system::errc::not_enough_memory;
    case errc::end_of_buffer:
    case errc::malformed_input: return boost::system::errc::io_error;
    }
    return {ev, *this};
}
...
int buffer_error_category::from_code(int ev) const noexcept
{
    using ceph::buffer::errc;
    switch (static_cast<errc>(ev)) {
    case errc::bad_alloc: return -ENOMEM;

    case errc::end_of_buffer: return -EIO;

    case errc::malformed_input: return -EIO;
    }
    return -EDOM;
}
