...
#define CEPH_BUFFER_ALLOC_UNIT_MAX \
    std::size_t                    \
    {                              \
        256 * 1024                 \
    }
...
public:
                                                         ...
                                                         int mempool = mempool::mempool_buffer_anon)
    {
        ...
        return ceph::unique_leakable_ptr<buffer::raw>(new (ptr + datalen) raw_combined(ptr, len, mempool));
    }
...
public:
    ...
    raw_posix_aligned(unsigned l, unsigned align)
        : raw(l)
    {
        // posix_memalign() requires a multiple of sizeof(void *)
        align = std::max<unsigned>(align, sizeof(void*));
...
public:
    raw_hack_aligned(unsigned l, unsigned align)
        : raw(l)
    {
...
buffer::list::iov_vec_t buffer::list::prepare_iovs() const
{
    ...
    for (auto& bp : _buffers) {
        if (index == 0) {
            ...
            it->iov.resize(std::min(_num - IOV_MAX * nr_iov_created, (size_t)IOV_MAX));
        }
...
void ceph::buffer::list::page_aligned_appender::_refill(size_t len)
{
    const unsigned alloc =
        std::max(min_alloc, shift_round_up(static_cast<unsigned>(len), static_cast<unsigned>(CEPH_PAGE_SHIFT)));
