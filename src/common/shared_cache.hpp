...
private:
    ...
    public:
        SharedLRU<K, V>* cache;
        K key;
        Cleanup(SharedLRU<K, V>* cache, K key)
            : cache(cache)
            , key(key)
        {}
        void operator()(V* ptr)
...
public:
    ...
    void dump_weak_refs(std::ostream& out)
    {
        for (const auto& [key, ref] : weak_refs) {
            out << __func__ << " " << this << " weak_refs: " << key << " = " << ref.second << " with "
                << ref.first.use_count() << " refs" << std::endl;
        }
    }
    ...
    bool get_next(const K& key, std::pair<K, VPtr>* next)
    {
        std::pair<K, VPtr> r;
        {
            ...
            while (i != weak_refs.end() && !(next_val = i->second.first.lock())) ++i;

            if (i == weak_refs.end()) return false;

            if (next) r = make_pair(i->first, next_val);
        }
        if (next) *next = r;
        return true;
    }
