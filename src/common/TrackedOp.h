...
public:
    OpHistory(CephContext* c)
        ...
        , opsvc(this)
    {
        PerfCountersBuilder b(cct, "osd-slow-ops", l_osd_slow_op_first, l_osd_slow_op_last);
        b.add_u64_counter(l_osd_slow_op_count, "slow_ops_count", "Number of operations taking over ten second");
...
public:
    ...
    void set_tracking(bool enable) { tracking_enabled = enable; }
    ...
    bool dump_ops_in_flight(ceph::Formatter* f, bool print_only_blocked = false, std::set<std::string> filters = {""},
                            bool count_only = false, dumper lambda = default_dumper);
     ...
     */
    bool visit_ops_in_flight(utime_t* oldest_secs, std::function<bool(TrackedOp&)>&& visit);
    /**
     ...
     * @return True if there are any Ops to warn on, false otherwise
     */
    bool with_slow_ops_in_flight(utime_t* oldest_secs, int* num_slow_ops, int* num_warned_ops,
     ...
     * @return True if there are any Ops to warn on, false otherwise.
     */
    bool check_ops_in_flight(std::string* summary, std::vector<std::string>& warning_strings, int* slow = nullptr);

    void on_shutdown() { history.on_shutdown(); }
    ~OpTracker();

    template<typename T, typename U> typename T::Ref create_request(U params)
    {
...
public:
    typedef boost::intrusive::list<
        TrackedOp,
        boost::intrusive::member_hook<TrackedOp, boost::intrusive::list_member_hook<>, &TrackedOp::tracker_item>>
        tracked_op_list_t;
    ...
    struct Putter
    {
        void operator()(TrackedOp* op) { op->put(); }
    };
...
protected:
    ...
    struct Event
    {
        ...
        Event(utime_t t, std::string_view s)
            : stamp(t)
            , str(s)
        {}
        ...
        if (nref_snap == 1) {
            ...
            case STATE_LIVE:
                ...
                if (!tracker->is_tracking()) {
                    ...
                    tracker->record_history_op(TrackedOpRef(this, /* add_ref = */ false));
                }
                break;

            case STATE_HISTORY: delete this; break;
...
public:
    void reset_desc() { want_new_desc = true; }

    void dump_type(Formatter* f) const { return _dump(f); }

    const utime_t& get_initiated() const { return initiated_at; }
    ...
    void mark_nowarn() { warn_interval_multiplier = 0; }
    ...
    friend void intrusive_ptr_add_ref(TrackedOp* o) { o->get(); }
    friend void intrusive_ptr_release(TrackedOp* o) { o->put(); }
