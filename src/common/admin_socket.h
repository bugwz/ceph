...
public:
     ...
     */
    virtual int call(std::string_view command, const cmdmap_t& cmdmap, const ceph::buffer::list& inbl,
                     ceph::Formatter* f, std::ostream& errss, ceph::buffer::list& out) = 0;
     ...
     * @note If @c out is empty, then admin socket will try to flush @c f to out.
     */
    virtual void call_async(std::string_view command, const cmdmap_t& cmdmap, ceph::Formatter* f,
...
public:
     ...
     */
    int register_command(std::string_view cmddesc, AdminSocketHook* hook, std::string_view help);
    ...
    /// execute (async)
    void execute_command(const std::vector<std::string>& cmd, const ceph::buffer::list& inbl,
                         std::function<void(int, const std::string&, ceph::buffer::list&)> on_fin);

    /// execute (blocking)
    int execute_command(const std::vector<std::string>& cmd, const ceph::buffer::list& inbl, std::ostream& errss,
...
private:
    ...
    struct hook_info
    {
        ...
        hook_info(AdminSocketHook* hook, std::string_view desc, std::string_view help)
            : hook(hook)
            , desc(desc)
            , help(help)
        {}
    ...
    std::pair<int, AdminSocketHook*> find_matched_hook(std::string& prefix, const cmdmap_t& cmdmap);
