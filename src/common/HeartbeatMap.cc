...
heartbeat_handle_d* HeartbeatMap::add_worker(const string& name, pthread_t thread_id)
{
    ...
    ANNOTATE_BENIGN_RACE_SIZED(&h->timeout, sizeof(h->timeout), "heartbeat_handle_d timeout");
    ANNOTATE_BENIGN_RACE_SIZED(&h->suicide_timeout, sizeof(h->suicide_timeout), "heartbeat_handle_d suicide_timeout");
...
bool HeartbeatMap::_check(const heartbeat_handle_d* h, const char* who, ceph::coarse_mono_time now)
{
    bool healthy = true;
    if (auto was = h->timeout.load(std::memory_order_relaxed); !clock::is_zero(was) && was < now) {
        ldout(m_cct, 1) << who << " '" << h->name << "'"
                        << " had timed out after " << h->grace << dendl;
        healthy = false;
    }
    if (auto was = h->suicide_timeout.load(std::memory_order_relaxed); !clock::is_zero(was) && was < now) {
        ldout(m_cct, 1) << who << " '" << h->name << "'"
...
void HeartbeatMap::reset_timeout(heartbeat_handle_d* h, ceph::timespan grace, ceph::timespan suicide_grace)
{
...
bool HeartbeatMap::is_healthy()
{
    ...
    if (m_cct->_conf->heartbeat_inject_failure) {
        ldout(m_cct, 0) << "is_healthy injecting failure for next " << m_cct->_conf->heartbeat_inject_failure
                        << " seconds" << dendl;
    ...
    ldout(m_cct, 20) << "is_healthy = " << (healthy ? "healthy" : "NOT HEALTHY") << ", total workers: " << total
                     << ", number of unhealthy: " << unhealthy << dendl;
    return healthy;
}
