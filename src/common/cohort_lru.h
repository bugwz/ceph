...
private:
    ...
    typedef bi::list<Object, bi::member_hook<Object, bi::list_member_hook<link_mode>, &Object::lru_hook>,
                     bi::constant_time_size<true>>
        Queue;

    bi::slist_member_hook<link_mode> q2_hook;

    typedef bi::slist<Object, bi::member_hook<Object, bi::slist_member_hook<link_mode>, &Object::q2_hook>,
                      bi::constant_time_size<true>>
        Queue2;
...
private:
    template<typename LK> friend class LRU;

    template<typename T, typename TTree, typename CLT, typename CEQ, typename K, typename LK> friend class TreeX;
};
...
template<typename LK> class LRU
{
private:
    struct Lane
    {
    ...
    Lane& lane_of(void* addr) { return qlane[(uint64_t)(addr) % n_lanes]; }

    uint32_t next_evict_lane() { return (evict_lane++ % n_lanes); }

    bool can_reclaim(Object* o) { return ((o->lru_refcnt == SENTINEL_REFCNT) && (!(o->lru_flags & FLAG_EVICTING))); }

    Object* evict_block(const ObjectFactory* newobj_fac)
    {
        uint32_t lane_ix = next_evict_lane();
        for (int ix = 0; ix < n_lanes; ++ix, lane_ix = next_evict_lane()) {
...
public:
    ...
    Object* insert(ObjectFactory* fac, Edge edge, uint32_t& flags)
    {
        ...
        lane.lock.lock();
        switch (edge) {
        case Edge::MRU: lane.q.push_front(*o); break;
        case Edge::LRU: lane.q.push_back(*o); break;
...
template<typename T, typename TTree, typename CLT, typename CEQ, typename K, typename LK> class TreeX
{
public:
    ...
    Partition& partition_of_scalar(uint64_t x) { return part[x % n_part]; }

    Partition& get(uint8_t x) { return part[x]; }

    Partition*& get() { return part; }

    void lock()
    {
        std::for_each(locks.begin(), locks.end(), [](LK* lk) { lk->lock(); });
    }

    void unlock()
    {
        std::for_each(locks.begin(), locks.end(), [](LK* lk) { lk->unlock(); });
    }
    ...
    T* find(uint64_t hk, const K& k, uint32_t flags)
    {
        ...
        if (csz) { /* template specialize? */
            ...
            if (v) {
                if (CEQ()(*v, k)) {
                    if (flags & FLAG_LOCK) lat.lock->unlock();
                    return v;
                }
                v = nullptr;
            }
    ...
    T* find_latch(uint64_t hk, const K& k, Latch& lat, uint32_t flags)
    {
        ...
        lat.p = &(partition_of_scalar(hk));
        lat.lock = &lat.p->lock;
        if (flags & FLAG_LOCK) lat.lock->lock();
        if (csz) { /* template specialize? */
            slot = hk % csz;
            v = lat.p->cache[slot];
            if (v) {
                if (CEQ()(*v, k)) {
                    if ((flags & FLAG_LOCK) && (flags & FLAG_UNLOCK)) lat.lock->unlock();
                    return v;
                }
                v = nullptr;
            }
        ...
        check_result r = lat.p->tr.insert_unique_check(k, CLT(), lat.commit_data);
    ...
    void remove(uint64_t hk, T* v, uint32_t flags)
    {
        ...
        if (csz) { /* template specialize? */
            ...
            if (v == v2) p.cache[slot] = nullptr;
        }
        if (flags & FLAG_LOCK) p.lock.unlock();
    } /* remove */

    void drain(std::function<void(T*)> uref, uint32_t flags = FLAG_NONE)
    {
