...
{
    return (g_lockdep_ceph_ctx != NULL && g_lockdep_ceph_ctx->_conf->lockdep_force_backtrace);
}
...
void lockdep_register_ceph_context(CephContext* cct)
{
    ...
    if (g_lockdep_ceph_ctx == NULL) {
        ANNOTATE_BENIGN_RACE_SIZED(&g_lockdep_ceph_ctx, sizeof(g_lockdep_ceph_ctx), "lockdep cct");
        ANNOTATE_BENIGN_RACE_SIZED(&g_lockdep, sizeof(g_lockdep), "lockdep enabled");
        g_lockdep = true;
...
void lockdep_unregister_ceph_context(CephContext* cct)
{
    ...
    if (cct == g_lockdep_ceph_ctx) {
        ...
        lock_names.clear();
        lock_ids.clear();
        std::for_each(follows.begin(), std::next(follows.begin(), current_maxid), [](auto& follow) { follow.reset(); });
        std::for_each(
            follows_bt.begin(), std::next(follows_bt.begin(), current_maxid), [](auto& follow_bt) { follow_bt = {}; });
    }
    pthread_mutex_unlock(&lockdep_mutex);
}
...
static bool does_follow(int a, int b)
{
    if (follows[a].test(b)) {
        ...
        *_dout << "------------------------------------"
               << "\n";
        *_dout << "existing dependency " << lock_names[a] << " (" << a << ") -> " << lock_names[b] << " (" << b
               << ") at:\n";
        if (follows_bt[a][b]) {
    ...
    for (unsigned i = 0; i < current_maxid; i++) {
        if (follows[a].test(i) && does_follow(i, b)) {
            lockdep_dout(0) << "existing intermediate dependency " << lock_names[a] << " (" << a << ") -> "
                            << lock_names[i] << " (" << i << ") at:\n";
            if (follows_bt[a][i]) {
...
int lockdep_will_lock(const char* name, int id, bool force_backtrace, bool recursive)
{
    ...
    if (id < 0) id = _lockdep_register(name);
    ...
    for (auto p = m.begin(); p != m.end(); ++p) {
        if (p->first == id) {
            ...
            if (does_follow(id, p->first)) {
                auto bt = new ceph::ClibBackTrace(BACKTRACE_SKIP);
                lockdep_dout(0) << "new dependency " << lock_names[p->first] << " (" << p->first << ") -> " << name
                                << " (" << id << ")"
                                << " creates a cycle at\n";
                bt->print(*_dout);
                *_dout << dendl;
