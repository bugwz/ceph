...
template<typename T, typename K> class mClockQueue : public OpQueue<T, K>
{
    ...
    static void filter_list_pairs(ListPairs* l, std::function<bool(T&&)> f)
    {
    ...
    public:
        ...
        void set_max_tokens(unsigned mt) { max_tokens = mt; }

        unsigned get_max_tokens() const { return max_tokens; }

        unsigned num_tokens() const { return tokens; }
...
public:
    mClockQueue(const typename Queue::ClientInfoFunc& info_func, double anticipation_timeout = 0.0)
    ...
    {
        queue.remove_by_req_filter([&](std::unique_ptr<T>&& r) { return filter_accum(std::move(*r)); }, true);
        ...
        for (typename SubQueues::iterator i = high_queue.begin(); i != high_queue.end();
             /* no-inc */) {
    ...
    {
        if (out) {
            queue.remove_by_client(k, true, [&out](std::unique_ptr<T>&& t) { out->push_front(std::move(*t)); });
        }
        else {
    ...
    bool empty() const override final { return queue.empty() && high_queue.empty() && queue_front.empty(); }
    ...
    void dump(ceph::Formatter* f) const override final
    {
        ...
        for (typename SubQueues::const_iterator p = high_queue.begin(); p != high_queue.end(); ++p) {
    ...
    void print(std::ostream& os) const final { os << "mClockPriorityQueue"; }
};
