...
void* ObjBencher::status_printer(void* _bencher)
{
    ...
    if (formatter) formatter->open_array_section("datas");
    while (!data.done) {
        ...
        if (i % 20 == 0 && !formatter) {
            if (i > 0)
                t.localtime(cout) << " min lat: " << data.min_latency << " max lat: " << data.max_latency
                                  << " avg lat: " << data.avg_latency << std::endl;
            // I'm naughty and don't reset the fill
            bencher->out(cout, t) << setfill(' ') << setw(5) << "sec" << setw(8) << "Cur ops" << setw(10) << "started"
                                  << setw(10) << "finished" << setw(10) << "avg MB/s" << setw(10) << "cur MB/s"
                                  << setw(12) << "last lat(s)" << setw(12) << "avg lat(s)" << std::endl;
        }
        if (cycleSinceChange)
            bandwidth = (double)(data.finished - previous_writes) * (data.op_size) / (1024 * 1024) / cycleSinceChange;
        else
            bandwidth = -1;

        if (!std::isnan(bandwidth) && bandwidth > -1) {
            if (bandwidth > data.idata.max_bandwidth) data.idata.max_bandwidth = bandwidth;
            if (bandwidth < data.idata.min_bandwidth) data.idata.min_bandwidth = bandwidth;
        ...
        if (cycleSinceChange)
            iops = (double)(data.finished - previous_writes) / cycleSinceChange;
        else
            iops = -1;

        if (!std::isnan(iops) && iops > -1) {
            if (iops > data.idata.max_iops) data.idata.max_iops = iops;
            if (iops < data.idata.min_iops) data.idata.min_iops = iops;
        ...
        if (formatter) formatter->open_object_section("data");

        // elapsed will be in seconds, by default
        std::chrono::duration<double> elapsed = cur_time - data.start_time;
        double avg_bandwidth = (double)(data.op_size) * (data.finished) / elapsed.count() / (1024 * 1024);
        if (previous_writes != data.finished) {
            previous_writes = data.finished;
            cycleSinceChange = 0;
            if (!formatter) {
                bencher->out(cout, t) << setfill(' ') << setw(5) << i << ' ' << setw(7) << data.in_flight << ' '
                                      << setw(9) << data.started << ' ' << setw(9) << data.finished << ' ' << setw(9)
                                      << avg_bandwidth << ' ' << setw(9) << bandwidth << ' ' << setw(11)
                                      << (double)data.cur_latency.count() << ' ' << setw(11) << data.avg_latency
                                      << std::endl;
            }
            else {
            ...
            if (!formatter) {
                bencher->out(cout, t) << setfill(' ') << setw(5) << i << ' ' << setw(7) << data.in_flight << ' '
                                      << setw(9) << data.started << ' ' << setw(9) << data.finished << ' ' << setw(9)
                                      << avg_bandwidth << ' ' << setw(9) << '0' << ' ' << setw(11) << '-' << ' '
    ...
    if (formatter) formatter->close_section();   // datas
    if (iops < 0) {
...
int ObjBencher::aio_bench(int operation, int secondsToRun, int concurrentios, uint64_t op_size, uint64_t object_size,
                          unsigned max_objects, bool cleanup, bool hints, const std::string& run_name, bool reuse_bench,
                          bool no_verify)
{

    if (concurrentios <= 0) return -EINVAL;
    ...
    if (operation != OP_WRITE || reuse_bench) {
        ...
        r = fetch_bench_metadata(run_name_meta, &prev_op_size, &prev_object_size, &num_ops, &num_objects, &prev_pid);
        if (r < 0) {
            if (r == -ENOENT) {
                if (reuse_bench)
    ...
    if (formatter) formatter->open_object_section("bench");
    ...
    if (OP_WRITE == operation && cleanup) {
        r = fetch_bench_metadata(run_name_meta, &op_size, &object_size, &num_ops, &num_objects, &prev_pid);
        if (r < 0) {
            if (r == -ENOENT) cerr << "Should never happen: bench metadata missing for current run!" << std::endl;
            goto out;
        }
...
struct lock_cond
{
    explicit lock_cond(ceph::mutex* _lock)
        : lock(_lock)
    {}
    ceph::mutex* lock;
    ceph::condition_variable cond;
...
int ObjBencher::fetch_bench_metadata(const std::string& metadata_file, uint64_t* op_size, uint64_t* object_size,
                                     int* num_ops, int* num_objects, int* prevPid)
{
    int r = 0;
    bufferlist object_data;

    r = sync_read(metadata_file, object_data, sizeof(int) * 2 + sizeof(size_t) * 2);
    if (r <= 0) {
...
int ObjBencher::write_bench(int secondsToRun, int concurrentios, const string& run_name_meta, unsigned max_objects,
                            int prev_pid)
{
    if (concurrentios <= 0) return -EINVAL;

    if (!formatter) {
        out(cout) << "Maintaining " << concurrentios << " concurrent writes of " << data.op_size
                  << " bytes to objects of size " << data.object_size << " for up to " << secondsToRun << " seconds or "
                  << max_objects << " objects" << std::endl;
    }
    else {
    ...
    if (data.op_size) writes_per_object = data.object_size / data.op_size;
    ...
    for (int i = 0; i < concurrentios; ++i) {
        ...
        if (r < 0) goto ERR;
        r = aio_write(name[i], i, *contents[i], data.op_size, data.op_size * (i % writes_per_object));
        if (r < 0) {
            goto ERR;
        }
        locker.lock();
    ...
    while (data.finished < data.started) {
        ...
        if (data.cur_latency.count() > data.max_latency) data.max_latency = data.cur_latency.count();
        if (data.cur_latency.count() < data.min_latency) data.min_latency = data.cur_latency.count();
        ++data.finished;
        ...
        if (data.op_size && max_objects &&
            data.started >= (int)((data.object_size * max_objects + data.op_size - 1) / data.op_size)) {
            locker.lock();
            continue;
        }
        ...
        if (r < 0) goto ERR;
        r = aio_write(newName, slot, *newContents, data.op_size, data.op_size * (data.started % writes_per_object));
        if (r < 0) {
            goto ERR;
        }
        name[slot] = newName;
...
int ObjBencher::seq_read_bench(int seconds_to_run, int num_ops, int num_objects, int concurrentios, int pid,
                               bool no_verify)
{
    ...
    if (data.op_size) reads_per_object = data.object_size / data.op_size;
    ...
    for (int i = 0; i < concurrentios; ++i) {
        ...
        r = aio_read(name[i], i, contents[i].get(), data.op_size, data.op_size * (i % reads_per_object));
        if (r < 0) {
            cerr << "r = " << r << std::endl;
            goto ERR;
        }
    ...
    while (data.finished < data.started) {
        ...
        bool start_new_read = (seconds_to_run && mono_clock::now() < finish_time) && num_ops > data.started;
        if (start_new_read) {
        ...
        if (!start_new_read) continue;

        // start new read and check data if requested
        start_times[slot] = mono_clock::now();
        create_completion(slot, _aio_cb, (void*)&lc);
        r = aio_read(
            newName, slot, contents[slot].get(), data.op_size, data.op_size * (data.started % reads_per_object));
        if (r < 0) {
            goto ERR;
        }
        locker.lock();
...
int ObjBencher::rand_read_bench(int seconds_to_run, int num_ops, int num_objects, int concurrentios, int pid,
                                bool no_verify)
{
    ...
    while (data.finished < data.started) {
        ...
        if (!seconds_to_run || mono_clock::now() >= finish_time) continue;
        ...
        start_times[slot] = mono_clock::now();
        create_completion(slot, _aio_cb, (void*)&lc);
        r = aio_read(newName, slot, contents[slot].get(), data.op_size, data.op_size * (rand_id % reads_per_object));
        if (r < 0) {
            goto ERR;
        }
        locker.lock();
...
int ObjBencher::clean_up(int num_objects, int prevPid, int concurrentios)
{
    ...
    while (data.finished < data.started) {
        ...
        if (data.started >= num_objects) continue;
...
bool ObjBencher::more_objects_matching_prefix(const std::string& prefix, std::list<Object>* objects)
{
    ...
    while (objects->empty()) {
        bool objects_remain = get_objects(&unfiltered_objects, 20);
        if (!objects_remain) return false;
