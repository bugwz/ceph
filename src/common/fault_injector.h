...
public:
    ...
    [[nodiscard]] constexpr int check(const Key& location) const
    {
        struct visitor
        {
            ...
            constexpr int operator()(const std::monostate&) const { return 0; }
            ...
            {
                if (check_location == this_location) {
                    ldpp_dout(e.dpp, -1) << "Injecting error=" << e.error << " at location=" << this_location << dendl;
                    return e.error;
                }
                return 0;
            }
            ...
            {
                if (check_location == this_location) {
                    ldpp_dout(e.dpp, -1) << "Injecting delay=" << e.duration << " at location=" << this_location
                                         << dendl;
...
private:
    // Key requirements:
    static_assert(std::is_default_constructible_v<Key>, "Key must be default-constrible");
    static_assert(std::is_move_constructible_v<Key>, "Key must be move-constructible");
    static_assert(std::is_move_assignable_v<Key>, "Key must be move-assignable");
    static_assert(boost::has_equal_to<Key, Key, bool>::value, "Key must be equality-comparable");
