...
public:
    ...
    template<class TAutoVector, class TValueType> class iterator_impl
    {
    public:
        // -- iterator traits
        ...
        self_type operator-(difference_type len) const { return self_type(vect_, index_ - len); }
    ...
    template<class... Args> void emplace_back(Args&&... args) { push_back(value_type(args...)); }
...
template<class T, size_t kSize> autovector<T, kSize>& autovector<T, kSize>::assign(const autovector& other)
{
