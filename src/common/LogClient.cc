...
{
    ldout(cct, 20) << __func__ << " log_to_monitors " << conf_strings.log_to_monitors << " log_to_syslog "
                   << conf_strings.log_to_syslog << " log_channels " << conf_strings.log_channels << " log_prios "
                   << conf_strings.log_prios << dendl;
    ...
    ldout(cct, 10) << __func__ << " to_monitors: " << (to_monitors ? "true" : "false")
                   ...
                   << " syslog_facility: " << conf_strings.log_channels << " prio: " << conf_strings.log_prios
                   << " to_graylog: " << (to_graylog ? "true" : "false")
                   << " graylog_host: " << conf_strings.log_to_graylog_host << " graylog_port: " << graylog_port << ")"
                   << dendl;
}
...
clog_targets_conf_t LogChannel::parse_log_client_options(CephContext* cct)
{
    ...
    targets.log_to_monitors =
        get_value_via_strmap(cct->_conf.get_val<string>("clog_to_monitors"), log_channel, CLOG_CONFIG_DEFAULT_KEY);
    targets.log_to_syslog =
        get_value_via_strmap(cct->_conf.get_val<string>("clog_to_syslog"), log_channel, CLOG_CONFIG_DEFAULT_KEY);
    targets.log_channels = get_value_via_strmap(
        cct->_conf.get_val<string>("clog_to_syslog_facility"), log_channel, CLOG_CONFIG_DEFAULT_KEY);
    targets.log_prios =
        get_value_via_strmap(cct->_conf.get_val<string>("clog_to_syslog_level"), log_channel, CLOG_CONFIG_DEFAULT_KEY);
    targets.log_to_graylog =
        get_value_via_strmap(cct->_conf.get_val<string>("clog_to_graylog"), log_channel, CLOG_CONFIG_DEFAULT_KEY);
    targets.log_to_graylog_host =
        get_value_via_strmap(cct->_conf.get_val<string>("clog_to_graylog_host"), log_channel, CLOG_CONFIG_DEFAULT_KEY);
    targets.log_to_graylog_port =
        get_value_via_strmap(cct->_conf.get_val<string>("clog_to_graylog_port"), log_channel, CLOG_CONFIG_DEFAULT_KEY);
