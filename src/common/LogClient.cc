...
int parse_log_client_options(CephContext* cct, map<string, string>& log_to_monitors, map<string, string>& log_to_syslog,
                             map<string, string>& log_channels, map<string, string>& log_prios,
                             map<string, string>& log_to_graylog, map<string, string>& log_to_graylog_host,
                             map<string, string>& log_to_graylog_port, uuid_d& fsid, string& host)
{
    ostringstream oss;

    int r = get_conf_str_map_helper(
        cct->_conf.get_val<string>("clog_to_monitors"), oss, &log_to_monitors, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
    ...
    r = get_conf_str_map_helper(
        cct->_conf.get_val<string>("clog_to_syslog"), oss, &log_to_syslog, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
    ...
    r = get_conf_str_map_helper(
        cct->_conf.get_val<string>("clog_to_syslog_facility"), oss, &log_channels, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
    ...
    r = get_conf_str_map_helper(
        cct->_conf.get_val<string>("clog_to_syslog_level"), oss, &log_prios, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
    ...
    r = get_conf_str_map_helper(
        cct->_conf.get_val<string>("clog_to_graylog"), oss, &log_to_graylog, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
    ...
    r = get_conf_str_map_helper(
        cct->_conf.get_val<string>("clog_to_graylog_host"), oss, &log_to_graylog_host, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
    ...
    r = get_conf_str_map_helper(
        cct->_conf.get_val<string>("clog_to_graylog_port"), oss, &log_to_graylog_port, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
...
LogChannel::LogChannel(CephContext* cct, LogClient* lc, const string& channel, const string& facility,
                       const string& prio)
    : cct(cct)
    , parent(lc)
    , log_channel(channel)
...
LogClient::LogClient(CephContext* cct, Messenger* m, MonMap* mm, enum logclient_flag_t flags)
    : cct(cct)
    , messenger(m)
    , monmap(mm)
    , is_mon(flags & FLAG_MON)
    , last_log_sent(0)
    , last_log(0)
{}
...
void LogChannel::update_config(map<string, string>& log_to_monitors, map<string, string>& log_to_syslog,
                               map<string, string>& log_channels, map<string, string>& log_prios,
                               map<string, string>& log_to_graylog, map<string, string>& log_to_graylog_host,
                               map<string, string>& log_to_graylog_port, uuid_d& fsid, string& host)
{
    ldout(cct, 20) << __func__ << " log_to_monitors " << log_to_monitors << " log_to_syslog " << log_to_syslog
                   << " log_channels " << log_channels << " log_prios " << log_prios << dendl;
    bool to_monitors = (get_str_map_key(log_to_monitors, log_channel, &CLOG_CONFIG_DEFAULT_KEY) == "true");
    bool to_syslog = (get_str_map_key(log_to_syslog, log_channel, &CLOG_CONFIG_DEFAULT_KEY) == "true");
    string syslog_facility = get_str_map_key(log_channels, log_channel, &CLOG_CONFIG_DEFAULT_KEY);
    string prio = get_str_map_key(log_prios, log_channel, &CLOG_CONFIG_DEFAULT_KEY);
    bool to_graylog = (get_str_map_key(log_to_graylog, log_channel, &CLOG_CONFIG_DEFAULT_KEY) == "true");
    string graylog_host = get_str_map_key(log_to_graylog_host, log_channel, &CLOG_CONFIG_DEFAULT_KEY);
    ...
    ldout(cct, 10) << __func__ << " to_monitors: " << (to_monitors ? "true" : "false")
                   << " to_syslog: " << (to_syslog ? "true" : "false") << " syslog_facility: " << syslog_facility
                   << " prio: " << prio << " to_graylog: " << (to_graylog ? "true" : "false")
                   << " graylog_host: " << graylog_host << " graylog_port: " << graylog_port << ")" << dendl;
}
...
ceph::ref_t<Message> LogClient::get_mon_log_message(bool flush)
{
    ...
    if (flush) {
        if (log_queue.empty()) return nullptr;
        // reset session
...
ceph::ref_t<Message> LogClient::_get_mon_log_message()
{
    ...
    if (last_log_sent == last_log) return {};
    ...
    ldout(cct, 10) << " log_queue is " << log_queue.size() << " last_log " << last_log << " sent " << last_log_sent
                   << " num " << log_queue.size() << " unsent " << num_unsent << " sending " << num_send << dendl;
    ...
    return ceph::make_message<MLog>(monmap->get_fsid(), std::move(o));
}
