...
bool matches_with_net(const ifaddrs& ifa, const sockaddr* net, unsigned int prefix_len, unsigned ipv)
{
...
bool matches_with_net(CephContext* cct, const ifaddrs& ifa, const std::string& s, unsigned ipv)
{
...
const struct sockaddr* find_ip_in_subnet_list(CephContext* cct, const struct ifaddrs* ifa, unsigned ipv,
                                              const std::string& networks, const std::string& interfaces, int numa_node)
{
    ...
    for (const auto* addr = ifa; addr != nullptr; addr = addr->ifa_next) {
        if (!ifs.empty() && std::none_of(std::begin(ifs), std::end(ifs), [&](const auto& if_name) {
                return matches_with_name(*addr, if_name);
            })) {
            continue;
        }
        if (!nets.empty() && std::none_of(std::begin(nets), std::end(nets), [&](const auto& net) {
...
struct Observer : public md_config_obs_t
{
    ...
    const char** get_tracked_conf_keys() const override { return (const char**)keys; }
...
static void fill_in_one_address(CephContext* cct, const struct ifaddrs* ifa, const string& networks,
                                const string& interfaces, const char* conf_var, int numa_node = -1)
{
    const struct sockaddr* found = find_ip_in_subnet_list(
        cct, ifa, CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_IPV6, networks, interfaces, numa_node);
    if (!found) {
        lderr(cct) << "unable to find any IP address in networks '" << networks << "' interfaces '" << interfaces << "'"
                   << dendl;
        exit(1);
    }
    ...
    err = getnameinfo(found,
                      (found->sa_family == AF_INET) ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6),
...
void pick_addresses(CephContext* cct, int needs)
{
    ...
    auto public_network_interface = cct->_conf.get_val<std::string>("public_network_interface");
    ...
    auto cluster_network_interface = cct->_conf.get_val<std::string>("cluster_network_interface");
    ...
    if ((needs & CEPH_PICK_ADDRESS_PUBLIC) && public_addr.is_blank_ip() && !public_network.empty()) {
        fill_in_one_address(cct, ifa, public_network, public_network_interface, "public_addr");
    }

    if ((needs & CEPH_PICK_ADDRESS_CLUSTER) && cluster_addr.is_blank_ip()) {
        if (!cluster_network.empty()) {
            fill_in_one_address(cct, ifa, cluster_network, cluster_network_interface, "cluster_addr");
        }
        else {
            if (!public_network.empty()) {
                ...
                fill_in_one_address(cct, ifa, public_network, public_network_interface, "cluster_addr");
            }
        }
    }
}
...
static int fill_in_one_address(CephContext* cct, const struct ifaddrs* ifa, unsigned ipv, const string& networks,
                               const string& interfaces, entity_addrvec_t* addrs, int numa_node = -1)
{
    const struct sockaddr* found = find_ip_in_subnet_list(cct, ifa, ipv, networks, interfaces, numa_node);
    if (!found) {
        std::string ip_type = "";
        ...
        lderr(cct) << "unable to find any " << ip_type << " address in networks '" << networks << "' interfaces '"
                   << interfaces << "'" << dendl;
        return -1;
    }
...
int pick_addresses(CephContext* cct, unsigned flags, struct ifaddrs* ifa, entity_addrvec_t* addrs,
                   int preferred_numa_node)
{
    addrs->v.clear();

    unsigned addrt = (flags & (CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_CLUSTER));
    if (addrt == 0 || addrt == (CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_CLUSTER)) {
        return -EINVAL;
    }
    unsigned msgrv = flags & (CEPH_PICK_ADDRESS_MSGR1 | CEPH_PICK_ADDRESS_MSGR2);
    if (msgrv == 0) {
    ...
    unsigned ipv = flags & (CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_IPV6);
    if (ipv == 0) {
    ...
    if (addrt & CEPH_PICK_ADDRESS_PUBLIC) {
        ...
        interfaces = cct->_conf.get_val<std::string>("public_network_interface");
    }
    else {
        addr = cct->_conf.get_val<entity_addr_t>("cluster_addr");
        networks = cct->_conf.get_val<std::string>("cluster_network");
        interfaces = cct->_conf.get_val<std::string>("cluster_network_interface");
        if (networks.empty()) {
    ...
    if (addr.is_blank_ip() && !networks.empty()) {
        ...
        if ((ipv & CEPH_PICK_ADDRESS_IPV4) && (flags & CEPH_PICK_ADDRESS_PREFER_IPV4)) {
            ipv4_r =
                fill_in_one_address(cct, ifa, CEPH_PICK_ADDRESS_IPV4, networks, interfaces, addrs, preferred_numa_node);
        }
        if (ipv & CEPH_PICK_ADDRESS_IPV6) {
            ipv6_r =
                fill_in_one_address(cct, ifa, CEPH_PICK_ADDRESS_IPV6, networks, interfaces, addrs, preferred_numa_node);
        }
        if ((ipv & CEPH_PICK_ADDRESS_IPV4) && !(flags & CEPH_PICK_ADDRESS_PREFER_IPV4)) {
            ipv4_r =
                fill_in_one_address(cct, ifa, CEPH_PICK_ADDRESS_IPV4, networks, interfaces, addrs, preferred_numa_node);
...
int pick_addresses(CephContext* cct, unsigned flags, entity_addrvec_t* addrs, int preferred_numa_node)
{
    struct ifaddrs* ifa;
    int r = getifaddrs(&ifa);
    if (r < 0) {
        ...
        lderr(cct) << "unable to fetch interfaces and addresses: " << cpp_strerror(r) << dendl;
        return r;
    }
...
std::string pick_iface(CephContext* cct, const struct sockaddr_storage& network)
{
    ...
    for (auto addr = ifa; addr != nullptr; addr = addr->ifa_next) {
        if (matches_with_net(
                *ifa, (const struct sockaddr*)&network, prefix_len, CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_IPV6)) {
