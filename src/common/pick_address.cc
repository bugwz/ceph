...
static std::optional<entity_addr_t> get_one_address(CephContext* cct, const struct ifaddrs* ifa, unsigned ipv,
                                                    const string& networks, const string& interfaces,
                                                    int numa_node = -1)
{
                   ...
                   int preferred_numa_node)
{
    ...
    unsigned addrt = (flags & (CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_PUBLIC_BIND | CEPH_PICK_ADDRESS_CLUSTER));
    ...
    if (addr.is_blank_ip() && !networks.empty()) {
        ...
        for (auto pick_mask : {CEPH_PICK_ADDRESS_IPV4, CEPH_PICK_ADDRESS_IPV6}) {
            if (ipv & pick_mask) {
                auto ip_addr = get_one_address(cct, ifa, pick_mask, networks, interfaces, preferred_numa_node);
                if (ip_addr) {
    ...
    std::sort(addrs->v.begin(), addrs->v.end(), [flags](entity_addr_t& lhs, entity_addr_t& rhs) {
