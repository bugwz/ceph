...
enum
{
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_val
    // ceph_throttle_msgr_dispatch_throttler_mon_val
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_val
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_max
    // ceph_throttle_msgr_dispatch_throttler_mon_max
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_max
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_get_started
    // ceph_throttle_msgr_dispatch_throttler_mon_get_started
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_get_started
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_get
    // ceph_throttle_msgr_dispatch_throttler_mon_get
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_get
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_get_sum
    // ceph_throttle_msgr_dispatch_throttler_mon_get_sum
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_get_sum
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_get_or_fail_fail
    // ceph_throttle_msgr_dispatch_throttler_mon_get_or_fail_fail
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_get_or_fail_fail
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_get_or_fail_success
    // ceph_throttle_msgr_dispatch_throttler_mon_get_or_fail_success
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_get_or_fail_success
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_take
    // ceph_throttle_msgr_dispatch_throttler_mon_take
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_take
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_take_sum
    // ceph_throttle_msgr_dispatch_throttler_mon_take_sum
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_take_sum
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_put
    // ceph_throttle_msgr_dispatch_throttler_mon_put
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_put
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_put_sum
    // ceph_throttle_msgr_dispatch_throttler_mon_put_sum
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_put_sum
    ...
    // ceph_throttle_msgr_dispatch_throttler_mds_wait_sum
    // ceph_throttle_msgr_dispatch_throttler_mds_wait_count
    // ceph_throttle_msgr_dispatch_throttler_mon_wait_sum
    // ceph_throttle_msgr_dispatch_throttler_mon_wait_count
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_wait_sum
    // ceph_throttle_msgr_dispatch_throttler_mon_mgrc_wait_count
...
void Throttle::_reset_max(int64_t m)
{
    // lock must be held.
    if (max == m) return;
    if (!conds.empty()) conds.front().notify_one();
    if (logger) logger->set(l_throttle_max, m);
    max = m;
}
    ...
    if (_should_wait(c) || !conds.empty()) {   // always wait behind other waiters.
        {
            ...
            auto w = make_scope_guard([this, cv]() { conds.erase(cv); });
            waited = true;
            ldout(cct, 2) << "_wait waiting..." << dendl;
            if (logger) start = mono_clock::now();

            cv->wait(l, [this, c, cv]() { return (!_should_wait(c) && cv == conds.begin()); });
...
bool Throttle::get_or_fail(int64_t c)
{
    ...
    bool result = false;
    {
        ...
        if (_should_wait(c) || !conds.empty()) {
            ...
            ldout(cct, 10) << "get_or_fail " << c << " success (" << count.load() << " -> " << (count.load() + c) << ")"
                           << dendl;
            count += c;
            result = true;
        }
...
bool BackoffThrottle::set_params(double _low_threshold, double _high_threshold, double _expected_throughput,
                                 double _high_multiple, double _max_multiple, uint64_t _throttle_max,
                                 ostream* errstream)
{
    bool valid = true;
    if (_low_threshold > _high_threshold) {
        valid = false;
        if (errstream) {
            *errstream << "low_threshold (" << _low_threshold << ") > high_threshold (" << _high_threshold << ")"
                       << std::endl;
        }
    }
    ...
    if (_high_multiple > _max_multiple) {
        valid = false;
        if (errstream) {
            *errstream << "_high_multiple (" << _high_multiple << ") > _max_multiple (" << _max_multiple << ")"
                       << std::endl;
        }
    }
    ...
    if (_low_threshold > 1 || _low_threshold < 0) {
        valid = false;
        if (errstream) {
            *errstream << "invalid low_threshold (" << _low_threshold << ")" << std::endl;
        }
    }
    ...
    if (_high_threshold > 1 || _high_threshold < 0) {
        valid = false;
        if (errstream) {
            *errstream << "invalid high_threshold (" << _high_threshold << ")" << std::endl;
        }
    }
    ...
    if (_max_multiple < 0) {
        valid = false;
        if (errstream) {
            *errstream << "invalid _max_multiple (" << _max_multiple << ")" << std::endl;
        }
    }
    ...
    if (_high_multiple < 0) {
        valid = false;
        if (errstream) {
            *errstream << "invalid _high_multiple (" << _high_multiple << ")" << std::endl;
        }
    }
    ...
    if (_expected_throughput < 0) {
        valid = false;
        if (errstream) {
            *errstream << "invalid _expected_throughput(" << _expected_throughput << ")" << std::endl;
        }
    }
    ...
    if (logger) logger->set(l_backoff_throttle_max, max);
    ...
    if (1 - high_threshold > 0) {
        s1 = (max_delay_per_count - high_delay_per_count) / (1 - high_threshold);
    }
    else {
        high_threshold = 1;
        s1 = 0;
...
{
    if (max == 0) return ceph::timespan(0);
    ...
    if (r < low_threshold) {
        ...
        return c * ceph::make_timespan(high_delay_per_count + ((r - high_threshold) * s1));
    }
}
...
ceph::timespan BackoffThrottle::get(uint64_t c)
{
    ...
    // fast path
    if (delay.count() == 0 && waiters.empty() && ((max == 0) || (current == 0) || ((current + c) <= max))) {
        current += c;
...
void SimpleThrottle::end_op(int r)
{
    std::lock_guard l(m_lock);
    --m_current;
    if (r < 0 && !m_ret && !(r == -ENOENT && m_ignore_enoent)) m_ret = r;
    m_cond.notify_all();
}
...
void OrderedThrottle::complete_pending_ops(std::unique_lock<std::mutex>& l)
{
    while (true) {
        ...
        if (it == m_tid_result.end() || it->first != m_complete_tid || !it->second.finished) {
            break;
        }
...
#undef dout_prefix
#define dout_prefix *_dout << "TokenBucketThrottle(" << m_name << " " << (void*)this << ") "
...
TokenBucketThrottle::TokenBucketThrottle(CephContext* cct, const std::string& name, uint64_t burst, uint64_t avg,
...
void TokenBucketThrottle::add_tokens()
{
    list<Blocker> tmp_blockers;
    {
        ...
        if (0 == m_avg || 0 == m_throttle.max) tmp_blockers.swap(m_blockers);
...
{
    m_token_ctx = new LambdaContext([this](int r) { schedule_timer(); });
