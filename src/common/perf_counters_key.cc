...
{
    return l.first.size() + sizeof(DELIMITER) + l.second.size() + sizeof(DELIMITER);
}

// an output iterator that writes label_pairs to a flat buffer
template<std::contiguous_iterator Iterator> class label_insert_iterator
{
...
std::string create(std::string_view counter_name, label_pair* begin, label_pair* end)
{
    ...
    auto size =
        std::accumulate(begin, end, counter_name.size() + sizeof(DELIMITER), [](std::size_t sum, const label_pair& l) {
...
std::string insert(const char* begin1, const char* end1, label_pair* begin2, label_pair* end2)
{
    // sort the input labels and remove duplicate keys
    ...
    size =
        std::accumulate(begin2, end2, size, [](std::size_t sum, const label_pair& l) { return sum + label_size(l); });
    std::string result;
    result.resize(size);
...
auto label_iterator::make_state(base_iterator begin, base_iterator end) -> std::optional<iterator_state>
{
