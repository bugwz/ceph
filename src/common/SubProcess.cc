...
protected:
    ...
    std::streamsize xsputn(const char* s, std::streamsize count) override { return write(fd, s, count); }
};
...
int SubProcess::spawn()
{
    ...
    if ((stdin_op == PIPE && pipe_cloexec(ipipe, 0) == -1) || (stdout_op == PIPE && pipe_cloexec(opipe, 0) == -1) ||
    ...
    if (pid == 0) {   // Child
        ...
        if (maxfd == -1) maxfd = 16384;
        for (int fd = 0; fd <= maxfd; fd++) {
            if (fd == STDIN_FILENO && stdin_op != CLOSE) continue;
            if (fd == STDOUT_FILENO && stdout_op != CLOSE) continue;
            if (fd == STDERR_FILENO && stderr_op != CLOSE) continue;
            ::close(fd);
        }
...
void SubProcess::exec()
{
    ...
    for (std::vector<std::string>::iterator i = cmd_args.begin(); i != cmd_args.end(); i++) {
...
int SubProcess::join()
{
    ...
    while (waitpid(pid, &status, 0) == -1) ceph_assert(errno == EINTR);

    pid = -1;

    if (WIFEXITED(status)) {
        if (WEXITSTATUS(status) != EXIT_SUCCESS) errstr << cmd << ": exit status: " << WEXITSTATUS(status);
...
void SubProcessTimed::exec()
{
    ...
    for (;;) {
        ...
        case SIGALRM:
            ...
            continue;
        default: std::cerr << cmd << ": sigwait: invalid signal: " << signo << "\n"; goto fail_exit;
        }
    }
