...
static inline int fork_function(int timeout, std::ostream& errstr, std::function<int8_t(void)> f)
{
    ...
    if (sigprocmask(SIG_SETMASK, &mask, &oldmask) == -1) {
        std::cerr << ": sigprocmask failed: " << cpp_strerror(errno) << "\n";
        goto fail_exit;
    }
    ...
    for (;;) {
        ...
        case SIGALRM:
            ...
            _exit(-ETIMEDOUT);
        default: std::cerr << ": sigwait: invalid signal: " << signo << "\n"; goto fail_exit;
        }
    }
    return 0;
fail_exit:
