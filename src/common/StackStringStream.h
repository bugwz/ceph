...
template<std::size_t SIZE> class StackStringBuf : public std::basic_streambuf<char>
{
public:
    StackStringBuf()
    ...
    std::string_view strv() const { return std::string_view(pbase(), pptr() - pbase()); }
...
template<std::size_t SIZE> class StackStringStream : public std::basic_ostream<char>
{
public:
    StackStringStream()
        : basic_ostream<char>(&ssb)
        , default_fmtflags(flags())
    {}
    ...
    std::string_view strv() const { return ssb.strv(); }
    std::string str() const { return std::string(ssb.strv()); }
...
public:
    ...
    sss& operator*() { return *osp; }
    sss const& operator*() const { return *osp; }
    sss* operator->() { return osp.get(); }
    sss const* operator->() const { return osp.get(); }

    sss const* get() const { return osp.get(); }
    sss* get() { return osp.get(); }
