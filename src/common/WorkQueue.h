...
public:
    ...
    public:
        TPHandle(CephContext* cct, ceph::heartbeat_handle_d* hb, ceph::timespan grace, ceph::timespan suicide_grace)
            : cct(cct)
            , hb(hb)
            , grace(grace)
...
protected:
    ...
    struct WorkQueue_
    {
        ...
        void set_timeout(time_t ti) { timeout_interval = ceph::make_timespan(ti); }
        void set_suicide_timeout(time_t sti) { suicide_interval = ceph::make_timespan(sti); }
    };
    ...
    const char** get_tracked_conf_keys() const override { return _conf_keys; }
...
public:
    ...
    template<typename T, typename U = T> class WorkQueueVal : public WorkQueue_
    {
     ...
     * destruction. */
    template<class T> class WorkQueue : public WorkQueue_
    {
        ThreadPool* pool;
        ...
        void* _void_dequeue() override { return (void*)_dequeue(); }
        void _void_process(void* p, TPHandle& handle) override { _process(static_cast<T*>(p), handle); }
        void _void_process_finish(void* p) override { _process_finish(static_cast<T*>(p)); }
    ...
    public:
        ...
        ~WorkQueue() override { pool->remove_work_queue(this); }
        ...
        void _wake() { pool->_wake(); }
        void _wait() { pool->_wait(); }
        void drain() { pool->drain(this); }
    };

    template<typename T> class PointerWQ : public WorkQueue_
    {
    public:
        ~PointerWQ() override
        {
    ...
    protected:
        PointerWQ(std::string n, ceph::timespan ti, ceph::timespan sti, ThreadPool* p)
            : WorkQueue_(std::move(n), ti, sti)
            , m_pool(p)
            , m_processing(0)
        {}
        void register_work_queue() { m_pool->add_work_queue(this); }
        void _clear() override
        {
        ...
        void _void_process(void* item, ThreadPool::TPHandle& handle) override { process(reinterpret_cast<T*>(item)); }
        ...
        ceph::mutex& get_pool_lock() { return m_pool->_lock; }
...
public:
    GenContextWQ(const std::string& name, ceph::timespan ti, ThreadPool* tp)
        : ThreadPool::WorkQueueVal<GenContext<ThreadPool::TPHandle&>*>(name, ti, ti * 10, tp)
    {}

    void _enqueue(GenContext<ThreadPool::TPHandle&>* c) override { _queue.push_back(c); }
    void _enqueue_front(GenContext<ThreadPool::TPHandle&>* c) override { _queue.push_front(c); }
    bool _empty() override { return _queue.empty(); }
    ...
    void _process(GenContext<ThreadPool::TPHandle&>* c, ThreadPool::TPHandle& tp) override { c->complete(tp); }
};
...
public:
    C_QueueInWQ(GenContextWQ* wq, GenContext<ThreadPool::TPHandle&>* c)
        ...
        , c(c)
    {}
    void finish(int) override { wq->queue(c); }
};
...
protected:
    ...
    void process(Context* ctx) override
    {
        ...
        {
            std::lock_guard locker(m_lock);
            ceph::unordered_map<Context*, int>::iterator it = m_context_results.find(ctx);
...
public:
    ...
    template<typename T> class ShardedWQ : public BaseShardedWQ
    {
    ...
    public:
        ShardedWQ(ceph::timespan ti, ceph::timespan sti, ShardedThreadPool* tp)
            : BaseShardedWQ(ti, sti)
            , sharded_pool(tp)
        {
            tp->set_wq(this);
        }
        ...
        void queue(T&& item) { _enqueue(std::move(item)); }
        void queue_front(T&& item) { _enqueue_front(std::move(item)); }
        void drain() { sharded_pool->drain(); }
    };
...
private:
    ...
    void set_wq(BaseShardedWQ* swq) { wq = swq; }
