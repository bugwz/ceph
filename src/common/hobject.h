...
public:
    ...
    bool match(uint32_t bits, uint32_t match) const { return match_hash(hash, bits, match); }

    bool is_temp() const { return is_temp_pool(pool) && pool != INT64_MIN; }
    bool is_meta() const { return is_meta_pool(pool); }
    ...
    static hobject_t_max get_max() { return hobject_t_max(); }

    hobject_t(const object_t& oid, const std::string& key, snapid_t snap, uint32_t hash, int64_t pool,
    ...
    hobject_t(const sobject_t& soid, const std::string& key, uint32_t hash, int64_t pool, const std::string& nspace)
        : oid(soid.oid)
        , snap(soid.snap)
        , hash(hash)
        , max(false)
    ...
    // used by Crimson
    hobject_t(const std::string& key, snapid_t snap, uint32_t reversed_hash, int64_t pool, const std::string& nspace)
        : oid(key)
        , snap(snap)
        , max(false)
        , pool(pool)
    ...
    bool is_snapdir() const { return snap == CEPH_SNAPDIR; }

    /// @return true if object is head
    bool is_head() const { return snap == CEPH_NOSNAP; }

    /// @return true if object is neither head nor snapdir nor max
    bool is_snap() const { return !is_max() && !is_head() && !is_snapdir(); }

    /// @return true iff the object should have a snapset in it's attrs
    bool has_snapset() const { return is_head() || is_snapdir(); }
    ...
    static uint32_t _reverse_bits(uint32_t v) { return reverse_bits(v); }
    static uint32_t _reverse_nibbles(uint32_t retval) { return reverse_nibbles(retval); }
     ...
     */
    static std::set<std::string> get_prefixes(uint32_t bits, uint32_t mask, int64_t pool);
    ...
    uint64_t get_nibblewise_key() const { return max ? 0x100000000ull : nibblewise_key_cache; }
    ...
    uint64_t get_bitwise_key() const { return max ? 0x100000000ull : hash_reverse_bits; }
    ...
    {
        return hobject_t(object_t(name), "", CEPH_NOSNAP, hash, get_temp_pool(pool), "");
    }
    ...
    const std::string& get_namespace() const { return nspace; }
    ...
    bool operator==(const hobject_t& rhs) const noexcept { return operator<=>(rhs) == 0; }
...
template<typename T> inline bool operator==(const hobject_t& lhs, const T&)
{
    static_assert(always_false<T>::value::value, "Do not compare to get_max()");
    return lhs.is_max();
}
template<typename T> inline bool operator==(const T&, const hobject_t& rhs)
{
    static_assert(always_false<T>::value::value, "Do not compare to get_max()");
    return rhs.is_max();
}
...
template<typename T> static inline int cmp(const hobject_t& l, const T&)
{
    static_assert(always_false<T>::value::value, "Do not compare to get_max()");
    return l.is_max() ? 0 : -1;
}
template<typename T> static inline int cmp(const T&, const hobject_t& r)
{
    static_assert(always_false<T>::value::value, "Do not compare to get_max()");
    return r.is_max() ? 0 : 1;
}
...
struct ghobject_t
{
    ...
    // used by Crimson
    ghobject_t(shard_id_t shard, int64_t pool, uint32_t reversed_hash, const std::string& nspace,
               const std::string& oid, snapid_t snap, gen_t gen)
        : shard_id(shard)
    ...
    bool match(uint32_t bits, uint32_t match) const { return hobj.match_hash(hobj.hash, bits, match); }
    /// @return min ghobject_t ret s.t. ret.hash == this->hash
    ghobject_t get_boundary() const
    {
        if (hobj.is_max()) return *this;
        ghobject_t ret;
    ...
    uint32_t get_nibblewise_key_u32() const { return hobj.get_nibblewise_key_u32(); }
    uint32_t get_nibblewise_key() const { return hobj.get_nibblewise_key(); }

    bool is_degenerate() const { return generation == NO_GEN && shard_id == shard_id_t::NO_SHARD; }

    bool is_no_gen() const { return generation == NO_GEN; }

    bool is_no_shard() const { return shard_id == shard_id_t::NO_SHARD; }

    void set_shard(shard_id_t s) { shard_id = s; }
    ...
    bool is_min() const { return *this == ghobject_t(); }
