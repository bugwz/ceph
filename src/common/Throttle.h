...
public:
     ...
     */
    int64_t get_current() const { return count; }
     ...
     */
    bool past_midpoint() const { return count >= max / 2; }
...
class BackoffThrottle
{
    ...
    std::list<std::condition_variable*>::iterator _push_waiter()
    {
        ...
        if (next_cond == conds.size()) next_cond = 0;
        return waiters.insert(waiters.end(), &(conds[next]));
    }

    void _kick_waiters()
    {
        if (!waiters.empty()) waiters.front()->notify_all();
    }
...
public:
     ...
     * explanation.
     */
    bool set_params(double _low_threshold, double _high_threshold, double expected_throughput, double high_multiple,
                    double max_multiple, uint64_t throttle_max, std::ostream* errstream);

    ceph::timespan get(uint64_t c = 1);
    ceph::timespan wait() { return get(0); }
...
public:
    C_OrderedThrottle(OrderedThrottle* ordered_throttle, uint64_t tid)
        : m_ordered_throttle(ordered_throttle)
        , m_tid(tid)
    {}
...
class TokenBucketThrottle
{
    ...
    struct Blocker
    {
        ...
        Blocker(uint64_t _tokens_requested, Context* _ctx)
            : tokens_requested(_tokens_requested)
            , ctx(_ctx)
        {}
    };
...
public:
    TokenBucketThrottle(CephContext* cct, const std::string& name, uint64_t burst, uint64_t avg, SafeTimer* timer,
    ...
    template<typename T, typename MF, typename I> void add_blocker(uint64_t c, T&& t, MF&& mf, I&& item, uint64_t flag)
    {
        auto ctx = new LambdaContext(
            [t, mf, item = std::forward<I>(item), flag](int) mutable { (t->*mf)(std::forward<I>(item), flag); });
        m_blockers.emplace_back(c, ctx);
    }

    template<typename T, typename MF, typename I> bool get(uint64_t c, T&& t, MF&& mf, I&& item, uint64_t flag)
    {
        bool wait = false;
        uint64_t got = 0;
        ...
        if (!m_blockers.empty()) {
            ...
            if (0 == m_throttle.max || 0 == m_avg) return false;
        ...
        if (wait) {
            add_blocker(c - got, std::forward<T>(t), std::forward<MF>(mf), std::forward<I>(item), flag);
        }
