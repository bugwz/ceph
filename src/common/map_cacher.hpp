 ...
 */
template<typename K, typename V> class Transaction
{
public:
    /// Std::set keys according to map
    virtual void set_keys(const std::map<K, V>& keys   ///< [in] keys/values to std::set
                          ) = 0;

    /// Remove keys
    virtual void remove_keys(const std::set<K>& to_remove   ///< [in] keys to remove
                             ) = 0;
 ...
 */
template<typename K, typename V> class StoreDriver
{
public:
    /// Returns requested key values
    virtual int get_keys(const std::set<K>& keys,   ///< [in] keys requested
                         std::map<K, V>* got        ///< [out] values for keys obtained
                         ) = 0;                     ///< @return error value

    /// Returns next key
    virtual int get_next(const K& key,           ///< [in] key after which to get next
    ...
    virtual int get_next_or_current(
        ...
        std::pair<K, V>* next_or_current) = 0;   ///< @return 0 on success, -ENOENT if there is no next
...
public:
    MapCacher(StoreDriver<K, V>* driver)
        : driver(driver)
    {}
    ...
    )
    {
        while (true) {
            ...
            if (!got_cached && !got_store) {
                ...
                if (cached.second) {
                    if (next) *next = make_pair(cached.first, cached.second.get());
                    return 0;
                }
                else {
                    key = cached.first;
    ...
    )
    {
        ...
        for (auto i = keys_to_get.begin(); i != keys_to_get.end(); ++i) {
            ...
            if (val) {
                if (*val) got->insert(make_pair(*i, val->get()));
