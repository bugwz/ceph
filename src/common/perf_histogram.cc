...
void PerfHistogramCommon::dump_formatted_axis(ceph::Formatter* f, const PerfHistogramCommon::axis_config_d& ac)
{
    ...
    case SCALE_LINEAR: f->dump_string("scale_type", "linear"); break;
    case SCALE_LOG2: f->dump_string("scale_type", "log2"); break;
    default: ceph_assert(false && "Invalid scale type");
    }
...
{
    switch (st) {
    case PerfHistogramCommon::SCALE_LINEAR: return i;
    case PerfHistogramCommon::SCALE_LOG2: return int64_t(1) << (i - 1);
    }
    ceph_assert(false && "Invalid scale type");
}

int64_t PerfHistogramCommon::get_bucket_for_axis(int64_t value, const PerfHistogramCommon::axis_config_d& ac)
{
    if (value < ac.m_min) {
        return 0;
    }
    ...
    switch (ac.m_scale_type) {
    case SCALE_LINEAR: return std::min<int64_t>(value + 1, ac.m_buckets - 1);
...
std::vector<std::pair<int64_t, int64_t>> PerfHistogramCommon::get_axis_bucket_ranges(
