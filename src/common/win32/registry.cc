...
RegistryKey::RegistryKey(CephContext* cct_, HKEY hRootKey, LPCTSTR strKey, bool create_value)
    : cct(cct_)
{
    ...
    if (ERROR_SUCCESS != status) {
        if (ERROR_FILE_NOT_FOUND == status) {
            ...
            lderr(cct_) << "Error: " << win32_strerror(status) << ". Could not open registry key: " << strKey << dendl;
        }
    }
}
...
int RegistryKey::remove(CephContext* cct_, HKEY hRootKey, LPCTSTR strKey)
{
    ...
    if (ERROR_SUCCESS != status) {
        lderr(cct_) << "Error: " << win32_strerror(status) << ". Could not delete registry key: " << strKey << dendl;
        return -EINVAL;
    }
