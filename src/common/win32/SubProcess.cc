...
int SubProcess::join()
{
    ...
    if (WaitForSingleObject(proc_handle, INFINITE) != WAIT_FAILED) {
        if (!GetExitCodeProcess(proc_handle, &status)) {
            errstr << cmd << ": Could not get exit code: " << pid << ". Error code: " << GetLastError();
            status = -ECHILD;
        }
        else if (status) {
        ...
        errstr << cmd << ": Waiting for child process failed: " << pid << ". Error code: " << GetLastError();
        status = -ECHILD;
    }
...
int SubProcess::spawn()
{
    ...
    HANDLE stdin_r = INVALID_HANDLE_VALUE, stdin_w = INVALID_HANDLE_VALUE, stdout_r = INVALID_HANDLE_VALUE,
           stdout_w = INVALID_HANDLE_VALUE, stderr_r = INVALID_HANDLE_VALUE, stderr_w = INVALID_HANDLE_VALUE;
    ...
    if (stdin_op == PIPE && stdin_pipe_out_fd == -1 || stdout_op == PIPE && stdout_pipe_in_fd == -1 ||
