...
int ServiceBase::initialize(ServiceBase* service)
{
    ...
    SERVICE_TABLE_ENTRY service_table[] = {{"", (LPSERVICE_MAIN_FUNCTION)run}, {NULL, NULL}};
...
void WINAPI ServiceBase::run()
{
    ...
    s_service->hstatus = RegisterServiceCtrlHandler("", (LPHANDLER_FUNCTION)control_handler);
    ...
    if (err) {
        lderr(s_service->cct) << "Failed to start service. Error code: " << err << dendl;
...
void ServiceBase::set_status(DWORD current_state, DWORD exit_code)
{
    ...
    if (hstatus) {
        dout(5) << "Updating service service status (" << current_state << ") and exit code(" << exit_code << ")."
                << dendl;
        ::SetServiceStatus(hstatus, &status);
    }
    else {
        derr << "Service control handler not initialized. Cannot "
             << "update service status (" << current_state << ") and exit code(" << exit_code << ")." << dendl;
    }
}
