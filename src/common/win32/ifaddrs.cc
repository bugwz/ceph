...
int getifaddrs(struct ifaddrs** ifap)
{
    ...
    DWORD size, res = 0;
    res = GetAdaptersAddresses(AF_UNSPEC, GAA_FLAG_INCLUDE_PREFIX, NULL, NULL, &size);
    ...
    res = GetAdaptersAddresses(AF_UNSPEC, GAA_FLAG_INCLUDE_PREFIX, NULL, adapter_addrs, &size);
    ...
    for (PIP_ADAPTER_ADDRESSES curr_addrs = adapter_addrs; curr_addrs != NULL; curr_addrs = curr_addrs->Next) {
        if (curr_addrs->OperStatus != 1) continue;

        for (PIP_ADAPTER_UNICAST_ADDRESS unicast_addrs = curr_addrs->FirstUnicastAddress; unicast_addrs != NULL;
             unicast_addrs = unicast_addrs->Next) {
            SOCKADDR* unicast_sockaddr = unicast_addrs->Address.lpSockaddr;
            if (unicast_sockaddr->sa_family != AF_INET && unicast_sockaddr->sa_family != AF_INET6) continue;
            ...
            if (curr_addrs->IfType == IF_TYPE_SOFTWARE_LOOPBACK) out_list_curr->ifa_flags |= IFF_LOOPBACK;
            ...
            sprintf_s(out_list_curr->ad_name, sizeof(out_list_curr->ad_name), curr_addrs->AdapterName);
        }
    }
    ret = 0;
out:
