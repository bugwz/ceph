...
///       });
/// \endcode
template<std::size_t Count> std::string key_create(std::string_view counter_name, label_pair (&&labels)[Count]);
...
template<std::size_t Count> std::string key_insert(std::string_view key, label_pair (&&labels)[Count]);
...
private:
    ...
    static auto make_state(base_iterator begin, base_iterator end) -> std::optional<iterator_state>;
};
...
template<std::size_t Count> std::string key_insert(std::string_view key, label_pair (&&labels)[Count])
{
    return detail::insert(key.begin(), key.end(), std::begin(labels), std::end(labels));
}
