 ...
 */
template<typename Key, typename Count> class BoundedKeyCounter
{
    ...
    static bool value_greater(const value_type* lhs, const value_type* rhs) { return lhs->second > rhs->second; }
    ...
    struct const_pointer_iterator : public map_type::const_iterator
    {
        ...
        reference operator*() const { return &map_type::const_iterator::operator*(); }
    };
...
public:
    ...
    Count insert(const Key& key, Count n = 1)
    {
        ...
        sorted_position = std::lower_bound(sorted.begin(), sorted_position, &*i, &value_greater);
    ...
    template<typename Callback> void get_highest(size_t count, Callback&& cb)
    {
        if (sorted.empty()) {
            // initialize the vector with pointers to all key-value pairs
            sorted.assign(const_pointer_iterator{counters.cbegin()}, const_pointer_iterator{counters.cend()});
        ...
        if (sorted_count < count) {
            ...
            std::partial_sort(sorted.begin(), sorted_position, sorted.end(), &value_greater);
        }
