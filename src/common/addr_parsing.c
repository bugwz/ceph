...
char* resolve_addrs(const char* orig_str)
{
    ...
    while (tok) {
        ...
        if (port_str && !*port_str) port_str = NULL;
        ...
        while (res) {
            ...
            getnameinfo(
                res->ai_addr, res->ai_addrlen, host, sizeof(host), port, sizeof(port), NI_NUMERICSERV | NI_NUMERICHOST);
          ...
          res->ai_canonname);*/
            if (res->ai_family == AF_INET6) brackets = 1; /* always surround ipv6 addrs with brackets */
            if (brackets) pos = safe_cat(&new_str, &len, pos, "[");
            ...
            res = res->ai_next;
            if (res) pos = safe_cat(&new_str, &len, pos, ",");
        }
        freeaddrinfo(ores);

        tok = strtok_r(NULL, delim, &saveptr);
        if (tok) pos = safe_cat(&new_str, &len, pos, ",");
    }
