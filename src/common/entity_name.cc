...
int EntityName::set(std::string_view type_, std::string_view id_)
{
    ...
    if (t == CEPH_ENTITY_TYPE_ANY) return -EINVAL;
    set(t, id_);
    return 0;
}
...
int EntityName::set_type(std::string_view type_)
{
    return set(type_, id);
}
...
uint32_t EntityName::str_to_ceph_entity_type(std::string_view s)
{
    size_t i;
    for (i = 0; i < STR_TO_ENTITY_TYPE.size(); ++i) {
        if (s == STR_TO_ENTITY_TYPE[i].str) return STR_TO_ENTITY_TYPE[i].type;
    }
