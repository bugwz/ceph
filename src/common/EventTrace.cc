...
void EventTrace::set_message_attrs(const Message* m, string& oid, string& context, bool incl_oid)
{
    ...
    if (m && (m->get_type() == CEPH_MSG_OSD_OP || m->get_type() == CEPH_MSG_OSD_OPREPLY)) {
        ...
        ostringstream buf;
        buf << m->get_source() << "!" << m->get_source_addr() << "!" << m->get_tid() << "!" << m->get_seq() << "!"
            << m->get_type();
        context = buf.str();
    }
}
...
void EventTrace::trace_oid_event(const char* oid, const char* event, const char* context, const char* file,
                                 const char* func, int line)
{
    if (unlikely(!g_ceph_context)) return;
    init_tp(g_ceph_context);
    tracepoint(eventtrace, oid_event, oid, event, context, file, func, line);
}

void EventTrace::trace_oid_event(const Message* m, const char* event, const char* file, const char* func, int line,
                                 bool incl_oid)
{
    string oid, context;
...
void EventTrace::trace_oid_elapsed(const char* oid, const char* event, const char* context, double elapsed,
...
void EventTrace::trace_oid_elapsed(const Message* m, const char* event, double elapsed, const char* file,
