...
public:
    ...
    MessageRef get_message(const uuid_d& fsid, bool mgr = false) const
    {
        if (mgr) {
...
public:
    ...
    ~CommandTable() { ceph_assert(commands.empty()); }
    ...
    const std::map<ceph_tid_t, T>& get_commands() const { return commands; }

    bool exists(ceph_tid_t tid) const { return commands.count(tid) > 0; }

    T& get_command(ceph_tid_t tid) { return commands.at(tid); }

    void erase(ceph_tid_t tid) { commands.erase(tid); }
