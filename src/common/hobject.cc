...
set<string> hobject_t::get_prefixes(uint32_t bits, uint32_t mask, int64_t pool)
{
    uint32_t len = bits;
...
void hobject_t::generate_test_instances(list<hobject_t*>& o)
{
    ...
    o.push_back(new hobject_t(object_t("oname2"), string("okey"), CEPH_NOSNAP, 67, 0, "n1"));
    o.push_back(new hobject_t(object_t("oname3"), string("oname3"), CEPH_SNAPDIR, 910, 1, "n2"));
}
...
{
    if (o == hobject_t()) return out << "MIN";
    if (o.is_max()) return out << "MAX";
    out << o.pool << ':';
    out << std::hex;
    out.width(8);
    out.fill('0');
...
int cmp(const hobject_t& l, const hobject_t& r)
{
    ...
    if (l.max > r.max) return 1;
    if (l.pool < r.pool) return -1;
    if (l.pool > r.pool) return 1;
    if (l.get_bitwise_key() < r.get_bitwise_key()) return -1;
    if (l.get_bitwise_key() > r.get_bitwise_key()) return 1;
    if (l.nspace < r.nspace) return -1;
    if (l.nspace > r.nspace) return 1;
...
void ghobject_t::decode(bufferlist::const_iterator& bl)
{
    ...
    if (struct_v >= 1) decode(hobj.key, bl);
    decode(hobj.oid, bl);
    decode(hobj.snap, bl);
    decode(hobj.hash, bl);
    if (struct_v >= 2)
    ...
    if (struct_v >= 4) {
        ...
        if (hobj.pool == -1 && hobj.snap == 0 && hobj.hash == 0 && !hobj.max && hobj.oid.name.empty()) {
            hobj.pool = INT64_MIN;
...
{
    hobj.dump(f);
    if (generation != NO_GEN) f->dump_int("generation", generation);
    if (shard_id != shard_id_t::NO_SHARD) f->dump_int("shard_id", shard_id);
    f->dump_int("max", (int)max);
}
...
void ghobject_t::generate_test_instances(list<ghobject_t*>& o)
{
    ...
    o.push_back(
        new ghobject_t(hobject_t(object_t("oname2"), string("okey"), CEPH_NOSNAP, 67, 0, "n1"), 1, shard_id_t(0)));
    o.push_back(
        new ghobject_t(hobject_t(object_t("oname2"), string("okey"), CEPH_NOSNAP, 67, 0, "n1"), 1, shard_id_t(1)));
    o.push_back(
        new ghobject_t(hobject_t(object_t("oname2"), string("okey"), CEPH_NOSNAP, 67, 0, "n1"), 1, shard_id_t(2)));
    o.push_back(
        new ghobject_t(hobject_t(object_t("oname3"), string("oname3"), CEPH_SNAPDIR, 910, 1, "n2"), 1, shard_id_t(0)));
    o.push_back(
        new ghobject_t(hobject_t(object_t("oname3"), string("oname3"), CEPH_SNAPDIR, 910, 1, "n2"), 2, shard_id_t(0)));
    o.push_back(
        new ghobject_t(hobject_t(object_t("oname3"), string("oname3"), CEPH_SNAPDIR, 910, 1, "n2"), 3, shard_id_t(0)));
    o.push_back(
        new ghobject_t(hobject_t(object_t("oname3"), string("oname3"), CEPH_SNAPDIR, 910, 1, "n2"), 3, shard_id_t(1)));
    o.push_back(
        new ghobject_t(hobject_t(object_t("oname3"), string("oname3"), CEPH_SNAPDIR, 910, 1, "n2"), 3, shard_id_t(2)));
}

ostream& operator<<(ostream& out, const ghobject_t& o)
{
    if (o == ghobject_t()) return out << "GHMIN";
    if (o.is_max()) return out << "GHMAX";
    if (o.shard_id != shard_id_t::NO_SHARD) out << std::hex << o.shard_id << std::dec;
    out << '#' << o.hobj << '#';
    if (o.generation != ghobject_t::NO_GEN) out << std::hex << (unsigned long long)(o.generation) << std::dec;
    return out;
}
...
int cmp(const ghobject_t& l, const ghobject_t& r)
{
    if (l.max < r.max) return -1;
    if (l.max > r.max) return 1;
    if (l.shard_id < r.shard_id) return -1;
    if (l.shard_id > r.shard_id) return 1;
