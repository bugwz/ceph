...
Graylog::Graylog(const SubsystemMap* const s, const std::string& logger)
    ...
    , m_ostream_compressed(std::stringstream::in | std::stringstream::out | std::stringstream::binary)
{
...
void Graylog::log_entry(const Entry& e)
{
    if (m_log_dst_valid) {
        ...
        if (m_subs != NULL) m_formatter->dump_string("_subsys_name", m_subs->get_name(e.m_subsys));
