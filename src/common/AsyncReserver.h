 ...
 * over all time.
 */
template<typename T, typename F> class AsyncReserver
{
    CephContext* cct;
    F* f;
    ...
    struct Reservation
    {
        ...
        {
            return out << r.item << "(prio " << r.prio << " grant " << r.grant << " preempt " << r.preempt << ")";
        }
    };
    ...
    void do_queues()
    {
        ...
        *_dout << dendl;
        ...
        while (!preempt_by_prio.empty() &&
               (in_progress.size() > max_allowed || preempt_by_prio.begin()->first < min_priority)) {
            preempt_one();
        }
...
public:
    AsyncReserver(CephContext* cct, F* f, unsigned max_allowed, unsigned min_priority = 0)
        : cct(cct)
        , f(f)
    ...
    void update_priority(T item, unsigned newprio)
    {
        ...
        if (i != queue_pointers.end()) {
            ...
            queue_pointers.insert(std::make_pair(item, std::make_pair(newprio, --(queues[newprio]).end())));
        }
        else {
            auto p = in_progress.find(item);
            if (p != in_progress.end()) {
                if (p->second.prio == newprio) return;
                rdout(10) << __func__ << " update " << p->second << " (in progress)" << dendl;
     ...
     * reservation slot.
     */
    void request_reservation(T item,                   ///< [in] reservation key
    ...
    )
    {
        ...
        queue_pointers.insert(std::make_pair(item, std::make_pair(prio, --(queues[prio]).end())));
        do_queues();
    }
     ...
     */
    void cancel_reservation(T item   ///< [in] key for reservation to cancel
    )
    {
        std::lock_guard l(lock);
        auto i = queue_pointers.find(item);
        if (i != queue_pointers.end()) {
            ...
            auto p = in_progress.find(item);
            if (p != in_progress.end()) {
                rdout(10) << __func__ << " cancel " << p->second << " (was in progress)" << dendl;
                if (p->second.preempt) {
                    preempt_by_prio.erase(std::make_pair(p->second.prio, p->second.item));
