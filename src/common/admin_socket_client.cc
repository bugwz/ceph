...
static std::string asok_connect(const std::string& path, int* fd)
{
    ...
    if (::setsockopt(socket_fd, SOL_SOCKET, SO_RCVTIMEO, (SOCKOPT_VAL_TYPE)&timer, sizeof(timer))) {
        int err = ceph_sock_errno();
        ostringstream oss;
        oss << "setsockopt(" << socket_fd << ", SO_RCVTIMEO) failed: " << cpp_strerror(err);
        compat_closesocket(socket_fd);
        return oss.str();
    }
    timer.tv_sec = 10;
    ...
    if (::setsockopt(socket_fd, SOL_SOCKET, SO_SNDTIMEO, (SOCKOPT_VAL_TYPE)&timer, sizeof(timer))) {
        int err = ceph_sock_errno();
        ostringstream oss;
        oss << "setsockopt(" << socket_fd << ", SO_SNDTIMEO) failed: " << cpp_strerror(err);
        compat_closesocket(socket_fd);
        return oss.str();
    }
...
static std::string asok_request(int socket_fd, std::string request)
{
    ...
    if (res < 0) {
        int err = res;
        ostringstream oss;
        oss << "safe_write(" << socket_fd << ") failed to write request code: " << cpp_strerror(err);
        return oss.str();
    }
    return "";
}
...
std::string AdminSocketClient::do_request(std::string request, std::string* result)
{
    ...
    res = safe_recv_exact(socket_fd, &message_size_raw, sizeof(message_size_raw));
    if (res < 0) {
        int e = res;
        ostringstream oss;
        oss << "safe_recv(" << socket_fd << ") failed to read message size: " << cpp_strerror(e);
        err = oss.str();
        goto done;
    }
