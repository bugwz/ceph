...
class condition_variable_debug
{
    ...
    condition_variable_debug& operator=(const condition_variable_debug&) = delete;
...
public:
    ...
    template<class Predicate> void wait(std::unique_lock<mutex_debug>& lock, Predicate pred)
    {
        while (!pred()) {
            wait(lock);
        }
    ...
    std::cv_status wait_until(std::unique_lock<mutex_debug>& lock, const std::chrono::time_point<Clock, Duration>& when)
    {
    ...
    std::cv_status wait_for(std::unique_lock<mutex_debug>& lock, const std::chrono::duration<Rep, Period>& awhile)
    {
    ...
    bool wait_for(std::unique_lock<mutex_debug>& lock, const std::chrono::duration<Rep, Period>& awhile, Pred pred)
    {
        ceph::real_time when{ceph::real_clock::now()};
        when += awhile;
        timespec ts = ceph::real_clock::to_timespec(when);
