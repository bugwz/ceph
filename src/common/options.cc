...
public:
    explicit printer(ostream& os)
        : out(os)
    {}
    template<typename T> void operator()(const T& v) const { out << v; }
    void operator()(boost::blank blank) const { return; }
    void operator()(bool v) const { out << (v ? "true" : "false"); }
    void operator()(double v) const { out << std::fixed << v << std::defaultfloat; }
    void operator()(const Option::size_t& v) const { out << v.value; }
    void operator()(const std::chrono::seconds v) const { out << v.count(); }
    void operator()(const std::chrono::milliseconds v) const { out << v.count(); }
};
}   // namespace
...
void Option::dump_value(const char* field_name, const Option::value_t& v, Formatter* f) const
{
    ...
    switch (type) {
    case TYPE_INT: f->dump_int(field_name, boost::get<int64_t>(v)); break;
    case TYPE_UINT: f->dump_unsigned(field_name, boost::get<uint64_t>(v)); break;
    case TYPE_STR: f->dump_string(field_name, boost::get<std::string>(v)); break;
    case TYPE_FLOAT: f->dump_float(field_name, boost::get<double>(v)); break;
    case TYPE_BOOL: f->dump_bool(field_name, boost::get<bool>(v)); break;
    default: f->dump_stream(field_name) << v; break;
    }
}
...
int Option::validate(const Option::value_t& new_value, std::string* err) const
{
    ...
    if (!enum_allowed.empty() && type == Option::TYPE_STR) {
        auto found = std::find(enum_allowed.begin(), enum_allowed.end(), boost::get<std::string>(new_value));
        if (found == enum_allowed.end()) {
            std::ostringstream oss;
            oss << "'" << new_value
                << "' is not one of the permitted "
                   "values: "
                << joinify(enum_allowed.begin(), enum_allowed.end(), std::string(", "));
            *err = oss.str();
            return -EINVAL;
        }
    }
...
int Option::parse_value(const std::string& raw_val, value_t* out, std::string* error_message,
    ...
    return std::vector<Option>({
        Option("host", Option::TYPE_STR, Option::LEVEL_BASIC)
            ...
            Option("public_network_interface", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .add_tag("network")
                .set_flag(Option::FLAG_STARTUP)
                .set_description("Interface name(s) from which to choose an address from a public_network to bind to; "
                                 "public_network must also be specified.")
            ...
            Option("cluster_network_interface", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_flag(Option::FLAG_STARTUP)
                .add_tag("network")
                .set_description("Interface name(s) from which to choose an address from a cluster_network to bind to; "
                                 "cluster_network must also be specified.")
            ...
            Option("setuser_match_path", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "If setuser or setgroup are specified, and this option is non-empty, then the uid/gid of the "
                    "daemon will only be changed if the file or directory specified by this option has a matching uid "
                    "and/or gid.  This exists primarily to allow switching to user ceph for OSDs to be conditional on "
                    "whether the osd data contents have also been chowned after an upgrade.  This is normally "
                    "specified by the systemd unit file.")
                .add_service({"mon", "mgr", "osd", "mds"})
                .add_tag("service")
            ...
            Option("log_max_recent", Option::TYPE_INT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "The purpose of this option is to log at a higher debug level only to the in-memory buffer, and "
                    "write out the detailed log messages only if there is a crash.  Only log entries below the lower "
                    "log level will be written unconditionally to the log.  For example, debug_osd=1/5 will write "
                    "everything <= 1 to the log unconditionally but keep entries at levels 2-5 in memory.  If there is "
                    "a seg fault or assertion failure, all entries will be dumped to the log."),
            ...
            Option("log_stderr_prefix", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "This is useful in container environments when combined with mon_cluster_log_to_stderr.  The mon "
                    "log prefixes each line with the channel name (e.g., 'default', 'audit'), while log_stderr_prefix "
                    "can be set to 'debug '.")
            ...
            Option("mon_cluster_log_file", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("This can either be a simple file name to receive all messages, or a list of "
                                      "key/value pairs where the key is the log channel and the value is the filename, "
            ...
            Option(
                "enable_experimental_unrecoverable_data_corrupting_features", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                .set_flag(Option::FLAG_RUNTIME)
                .set_default("")
                .set_description("Enable named (or all with '*') experimental features that may be untested, "
            ...
            Option("openssl_engine_opts", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("Pass opts in this way: "
                                      "engine_id=engine1,dynamic_path=/some/path/"
                                      "engine1.so,default_algorithms=DIGESTS:engine_id=engine2,dynamic_path=/some/path/"
            ...
            Option("thp", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description("Ceph is known to suffer from memory fragmentation due to THP use. This is "
                                      "indicated by RSS usage above configured memory targets. Enabling THP is "
                                      "currently discouraged until selective use of THP by Ceph is implemented."),

            Option("key", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                .set_default("")
                .set_description("Authentication key")
                .set_long_description("A CephX authentication key, base64 encoded.  It normally looks something like "
                                      "'AQAtut9ZdMbNJBAAHz6yBAWyJyz2yYRyeMWDag=='.")
                .set_flag(Option::FLAG_STARTUP)
                .set_flag(Option::FLAG_NO_MON_UPDATE)
            ...
            Option("keyfile", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("The file should contain a CephX authentication key and optionally a trailing "
            ...
            Option("keyring", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                .set_default("/etc/ceph/$cluster.$name.keyring,/etc/ceph/$cluster.keyring,"
                ...
                .set_long_description("A keyring file is an INI-style formatted file where the section names are "
                                      "client or daemon names (e.g., 'osd.0') and each section contains a 'key' "
                                      "property with CephX authentication key as the value.")
                .set_flag(Option::FLAG_STARTUP)
                .set_flag(Option::FLAG_NO_MON_UPDATE)
                .add_see_also("key")
                .add_see_also("keyfile"),
            ...
            Option("heartbeat_file", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "If set, this file will be touched every time an internal heartbeat check succeeds.")
                .add_see_also("heartbeat_interval"),

            Option("heartbeat_inject_failure", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),
            ...
            Option("ms_mon_client_mode", Option::TYPE_STR, Option::LEVEL_BASIC)
                ...
                .set_flag(Option::FLAG_STARTUP)
                .set_description(
                    "Connection modes (crc, secure) for connections from clients to monitors in order of preference")
                .add_see_also("ms_mon_service_mode")
                .add_see_also("ms_mon_cluster_mode")
                .add_see_also("ms_service_mode")
                .add_see_also("ms_cluster_mode")
            ...
            Option("ms_learn_addr_from_peer", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("Use the IP address our first peer (usually a monitor) sees that we are "
            ...
            Option("ms_pq_min_cost", Option::TYPE_SIZE, Option::LEVEL_DEV).set_default(65536).set_description(""),
            ...
            Option("ms_blackhole_osd", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

            Option("ms_blackhole_mon", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

            Option("ms_blackhole_mds", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

            Option("ms_blackhole_mgr", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

            Option("ms_blackhole_client", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),
            ...
            Option("ms_async_rdma_sl", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(3).set_description(""),

            Option("ms_async_rdma_dscp", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(96).set_description(""),
            ...
            Option("ms_dpdk_port_id", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(0).set_description(""),
            ...
            Option("ms_dpdk_hugepages", Option::TYPE_STR, Option::LEVEL_ADVANCED).set_default("").set_description(""),

            Option("ms_dpdk_pmd", Option::TYPE_STR, Option::LEVEL_ADVANCED).set_default("").set_description(""),
            ...
            Option("ms_dpdk_lro", Option::TYPE_BOOL, Option::LEVEL_ADVANCED).set_default(true).set_description(""),
            ...
            Option("mon_memory_target", Option::TYPE_SIZE, Option::LEVEL_BASIC)
                ...
                .set_flag(Option::FLAG_RUNTIME)
                .add_service("mon")
                .set_description("The amount of bytes pertaining to osd monitor caches and kv cache to be kept mapped "
                                 "in memory with cache auto-tuning enabled"),
            ...
            Option("mon_osd_max_initial_pgs", Option::TYPE_INT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("If the user specifies more PGs than this, the cluster will subsequently split "
                                      "PGs after the pool is created in order to reach the target."),
            ...
            Option("mon_osd_laggy_weight", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "1.0 means that an OSD marking itself back up (because it was marked down but not actually dead) "
                    "means a 100% laggy_probability; 0.0 effectively disables tracking of laggy_probability."),
            ...
            Option("mon_osd_adjust_heartbeat_grace", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "If an OSD is marked down but then marks itself back up, it implies it wasn't actually down but "
                    "was unable to respond to heartbeats.  If this option is true, we can use the laggy_probability "
                    "and laggy_interval values calculated to model this situation to increase the heartbeat grace "
                    "period for this OSD so that it isn't marked down again.  laggy_probability is an estimated "
                    "probability that the given OSD is down because it is laggy (not actually down), and "
                    "laggy_interval is an estiate on how long it stays down when it is laggy.")
            ...
            Option("mon_osd_warn_num_repaired", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                ...
                .add_service("mon")
                .set_description(
                    "issue OSD_TOO_MANY_REPAIRS health warning if an OSD has more than this many read repairs"),
            ...
            Option("mon_osd_prime_pg_temp_max_estimate", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .add_service("mon")
                .set_description(
                    "calculate all PG mappings if estimated fraction of PGs that change is above this amount"),
            ...
            Option("mon_stat_smooth_intervals", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                ...
                .add_service("mgr")
                .set_description(
                    "number of PGMaps stats over which we calc the average read/write throughput of the whole cluster"),
            ...
            Option("mon_lease", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("This setting controls how sensitive your mon quorum is to intermittent network "
                                      "issues or other failures."),
            ...
            Option("mon_lease_renew_interval_factor", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("Leases must be renewed before they time out.  A smaller value means frequent "
            ...
            Option("mon_accept_timeout_factor", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                .set_default(2.0)
                .add_service("mon")
                .set_description("multiple of mon_lease for follower mons to accept proposed state changes before "
            ...
            Option("mon_elector_ignore_propose_margin", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .add_service("mon")
                .set_description(
                    "The difference in connection score allowed before a peon stops ignoring out-of-quorum PROPOSEs"),
            ...
            Option("mon_stretch_recovery_min_wait", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                .set_default(15.0)
                .add_service("mon")
                .set_description("how long the monitors wait before considering fully-healthy PGs as evidence the "
                                 "stretch mode is repaired")
                .set_min(1.0),
            ...
            Option("mon_timecheck_skew_interval", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                .set_default(30.0)
                .add_service("mon")
                .set_description("frequency of clock synchronization (re)checks between monitors while clocks are "
                                 "believed to be skewed (seconds)")
            ...
            Option("mon_cache_target_full_warn_ratio", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .set_flag(Option::FLAG_NO_MON_UPDATE)
                .set_flag(Option::FLAG_CLUSTER_CREATE)
                .set_description("issue CACHE_POOL_NEAR_FULL health warning when cache pool utilization exceeds this "
                                 "ratio of usable space"),
            ...
            Option("mon_globalid_prealloc", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("This setting caps how many new clients can authenticate with the cluster before "
                                      "the monitors have to perform a write to preallocate more.  Large values burn "
                                      "through the 64-bit ID space more quickly."),
            ...
            Option("mon_warn_on_insecure_global_id_reclaim", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                .set_default(true)
                .add_service("mon")
                .set_description("issue AUTH_INSECURE_GLOBAL_ID_RECLAIM health warning if any connected clients are "
                                 "insecurely reclaiming global_id")
            ...
            Option("mon_warn_on_insecure_global_id_reclaim_allowed", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                .set_default(true)
                .add_service("mon")
                .set_description("issue AUTH_INSECURE_GLOBAL_ID_RECLAIM_ALLOWED health warning if insecure global_id "
                                 "reclaim is allowed")
            ...
            Option("mon_warn_on_msgr2_not_enabled", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                .set_default(true)
                .add_service("mon")
                .set_description("issue MON_MSGR2_NOT_ENABLED health warning if monitors are all running Nautilus but "
                                 "not all binding to a msgr2 port")
                .add_see_also("ms_bind_msgr2"),

            Option("mon_warn_on_legacy_crush_tunables", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                .set_default(true)
                .add_service("mgr")
                .set_description("issue OLD_CRUSH_TUNABLES health warning if CRUSH tunables are older than "
            ...
            Option("mon_warn_on_crush_straw_calc_version_zero", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .add_service("mgr")
                .set_description(
                    "issue OLD_CRUSH_STRAW_CALC_VERSION health warning if the CRUSH map's straw_calc_version is zero"),
            ...
            Option("mon_warn_on_osd_down_out_interval_zero", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "Having mon_osd_down_out_interval set to 0 means that down OSDs are not marked out automatically "
                    "and the cluster does not heal itself without administrator intervention.")
                .add_see_also("mon_osd_down_out_interval"),
            ...
            Option("mon_warn_on_cache_pools_without_hit_sets", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .add_service("mgr")
                .set_description(
                    "issue CACHE_POOL_NO_HIT_SET health warning for cache pools that do not have hit sets configured"),
            ...
            Option("mon_warn_on_pool_pg_num_not_power_of_two", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .add_service("mon")
                .set_description(
                    "issue POOL_PG_NUM_NOT_POWER_OF_TWO warning if pool has a non-power-of-two pg_num value"),
            ...
            Option("mon_warn_on_slow_ping_ratio", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .add_service("mgr")
                .set_description(
                    "Issue a health warning if heartbeat ping longer than percentage of osd_heartbeat_grace")
            ...
            Option("mon_scrub_timeout", Option::TYPE_INT, Option::LEVEL_ADVANCED)
                ...
                .add_service("mon")
                .set_description(
                    "timeout to restart scrub of mon quorum participant does not respond for the latest chunk"),
            ...
            Option("mon_osd_snap_trim_queue_warn_on", Option::TYPE_INT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("Warn when snap trim queue length for at least one PG crosses this value, as "
                                      "this is indicator of snap trimmer not keeping up, wasting disk space"),
            ...
            Option("mds_beacon_mon_down_grace", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "The interval without beacons before Ceph declares an MDS laggy when a monitor is down."),
            ...
            Option("mon_debug_extra_checks", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description("Enable some additional monitor checks that would be too expensive "
            ...
            Option("mon_debug_block_osdmap_trim", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description("Blocking OSDMap trimming may be quite helpful to easily reproduce "
            ...
            Option("mon_warn_on_older_version", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .add_service("mon")
                .set_description(
                    "issue DAEMON_OLD_VERSION health warning if daemons are not all running the same version"),
            ...
            Option("paxos_service_trim_max_multiplier", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                ...
                .set_min(0)
                .add_service("mon")
                .set_description("factor by which paxos_service_trim_max will be multiplied to get a new upper bound "
                                 "when trim sizes are high  (0 disables it)")
                .set_flag(Option::FLAG_RUNTIME),
            ...
            Option("auth_allow_insecure_global_id_reclaim", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                .set_default(true)
                .set_description("Allow reclaiming global_id without presenting a valid ticket proving previous "
                                 "possession of that global_id")
                .set_long_description(
                    "Allowing unauthorized global_id (re)use poses a security risk.  Unfortunately, older clients may "
                    "omit their ticket on reconnects and therefore rely on this being allowed for preserving their "
                    "global_id for the lifetime of the client instance.  Setting this value to false would immediately "
                    "prevent new connections from those clients (assuming auth_expose_insecure_global_id_reclaim set "
                    "to true) and eventually break existing sessions as well (regardless of "
                    "auth_expose_insecure_global_id_reclaim setting).")
                .add_see_also("mon_warn_on_insecure_global_id_reclaim")
                .add_see_also("mon_warn_on_insecure_global_id_reclaim_allowed")
                .add_see_also("auth_expose_insecure_global_id_reclaim"),
            ...
            Option("auth_expose_insecure_global_id_reclaim", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                .set_default(true)
                .set_description("Force older clients that may omit their ticket on reconnects to reconnect as part of "
                ...
                .set_long_description(
                    ...
                    "(auth_allow_insecure_global_id_reclaim set to false), this is a fail-fast mechanism: don't "
                    "establish a session that will almost inevitably be broken later.")
                .add_see_also("mon_warn_on_insecure_global_id_reclaim")
                .add_see_also("mon_warn_on_insecure_global_id_reclaim_allowed")
                .add_see_also("auth_allow_insecure_global_id_reclaim"),

            Option("auth_debug", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),
            ...
            Option("crush_location", Option::TYPE_STR, Option::LEVEL_ADVANCED).set_default("").set_description(""),

            Option("crush_location_hook", Option::TYPE_STR, Option::LEVEL_ADVANCED).set_default("").set_description(""),
            ...
            Option("objecter_timeout", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                .set_default(10.0)
                .set_description(
                    "Seconds before in-flight op is considered 'laggy' and we query mon for the latest OSDMap"),
            ...
            Option("filer_max_truncate_ops", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                .set_default(128)
                .set_description(
                    "Max in-flight operations for truncating/deleting a striped sequence (e.g., MDS journal)"),
            ...
            Option("osd_max_backfills", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("There can be osd_max_backfills local reservations AND the same remote "
            ...
            Option("osd_min_recovery_priority", Option::TYPE_INT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("The purpose here is to prevent the cluster from doing *any* lower priority work "
                                      "(e.g., rebalancing) below this threshold and focus solely on higher priority "
                                      "work (e.g., replicating degraded objects)."),

            Option("osd_backfill_retry_interval", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                .set_default(30.0)
                .set_description(
                    "how frequently to retry backfill reservations after being denied (e.g., due to a full OSD)"),

            Option("osd_recovery_retry_interval", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                .set_default(30.0)
                .set_description(
                    "how frequently to retry recovery reservations after being denied (e.g., due to a full OSD)"),
            ...
            Option("osd_find_best_info_ignore_history_les", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    ...
                    "It makes peering ignore the last_epoch_started value when peering, which can allow the OSD to "
                    "believe an OSD has an authoritative view of a PG's contents even when it is in fact old and "
                    "stale, typically leading to data loss (by believing a stale PG is up to date)."),
            ...
            Option("osd_client_message_size_cap", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("If this value is exceeded, the OSD will not read any new client data off of the "
                                      "network until memory is freed."),
            ...
            Option("osd_crush_update_weight_set", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "If this setting is true, we will update the weight-set weights when adjusting an item's weight, "
                    "effectively making changes take effect immediately, and discarding any previous optimization in "
                    "the weight-set value.  Setting this value to false will leave it to the balancer to (slowly, "
                    "presumably) adjust weights to approach the new target value."),
            ...
            Option("osd_pool_default_min_size", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                ...
                .set_flag(Option::FLAG_RUNTIME)
                .set_description(
                    "the minimal number of copies allowed to write to a degraded pool for new replicated pools")
            ...
            Option("osd_pool_default_read_lease_ratio", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                ...
                .set_long_description("This should be <= 1.0 so that the read lease will have expired by the time we "
                                      "decide to mark a peer OSD down.")
                .add_see_also("osd_heartbeat_grace"),
            ...
            Option("osd_map_dedup", Option::TYPE_BOOL, Option::LEVEL_ADVANCED).set_default(true).set_description(""),

            Option("osd_map_cache_size", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(50).set_description(""),
            ...
            Option("osd_skip_data_digest", Option::TYPE_BOOL, Option::LEVEL_DEV)
                .set_default(false)
                .set_description("Do not store full-object checksums if the backend (bluestore) does its own "
            ...
            Option("osd_op_queue_cut_off", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "the threshold between high priority ops that use strict priority ordering and low priority ops "
                    "that use a fairness algorithm that may or may not incorporate priority")
            ...
            Option("osd_mclock_cost_per_io_usec", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                ...
                .set_long_description("This option specifies the cost factor to consider in usec per OSD. This is "
                                      "considered by the mclock scheduler to set an additional cost factor in QoS "
            ...
            Option("osd_mclock_cost_per_io_usec_hdd", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "This option specifies the cost factor to consider in usec per OSD for rotational device type. "
                    "This is considered by the mclock_scheduler to set an additional cost factor in QoS calculations. "
            ...
            Option("osd_mclock_cost_per_io_usec_ssd", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "This option specifies the cost factor to consider in usec per OSD for solid state device type. "
                    "This is considered by the mclock_scheduler to set an additional cost factor in QoS calculations. "
                    "Only considered for osd_op_queue = mclock_scheduler")
                .set_flag(Option::FLAG_RUNTIME),
            ...
            Option("osd_mclock_cost_per_byte_usec", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                ...
                .set_long_description("This option specifies the cost per byte to consider in microseconds per OSD. "
                                      "This is considered by the mclock scheduler to set an additional cost factor in "
            ...
            Option("osd_mclock_cost_per_byte_usec_hdd", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "This option specifies the cost per byte to consider in microseconds per OSD for rotational device "
                    "type. This is considered by the mclock_scheduler to set an additional cost factor in QoS "
                    "calculations. Only considered for osd_op_queue = mclock_scheduler")
                .set_flag(Option::FLAG_RUNTIME),
            ...
            Option("osd_mclock_cost_per_byte_usec_ssd", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "This option specifies the cost per byte to consider in microseconds per OSD for solid state "
                    "device type. This is considered by the mclock_scheduler to set an additional cost factor in QoS "
                    "calculations. Only considered for osd_op_queue = mclock_scheduler")
                .set_flag(Option::FLAG_RUNTIME),
            ...
            Option("osd_mclock_max_capacity_iops_hdd", Option::TYPE_FLOAT, Option::LEVEL_BASIC)
                ...
                .set_long_description(
                    "This option specifies the max OSD capacity in iops per OSD. Helps in QoS calculations when "
                    "enabling a dmclock profile. Only considered for osd_op_queue = mclock_scheduler")
                .set_flag(Option::FLAG_RUNTIME),
            ...
            Option("osd_mclock_force_run_benchmark_on_init", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "This option specifies whether the OSD benchmark must be run during the OSD boot-up sequence even "
                    "if historical data about the OSD iops capacity is available in the MON config store. Enable this "
                    "to refresh the OSD iops capacity if the underlying device's performance characteristics have "
                    "changed significantly. Only considered for osd_op_queue = mclock_scheduler.")
                .set_flag(Option::FLAG_STARTUP)
            ...
            Option("osd_mclock_skip_benchmark", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description("This option specifies whether the OSD benchmark must be skipped during the OSD "
                                      "boot-up sequence. Only considered for osd_op_queue = mclock_scheduler.")
                .set_flag(Option::FLAG_RUNTIME)
                .add_see_also("osd_mclock_max_capacity_iops_hdd")
                .add_see_also("osd_mclock_max_capacity_iops_ssd"),
            ...
            Option("osd_mclock_profile", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "This option specifies the mclock profile to enable - one among the set of built-in profiles or a "
                    "custom profile. Only considered for osd_op_queue = mclock_scheduler")
                .set_flag(Option::FLAG_RUNTIME)
                .add_see_also("osd_op_queue"),
            ...
            Option("osd_recovery_sleep_hybrid", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                .set_default(0.025)
                .set_flag(Option::FLAG_RUNTIME)
                .set_description("Time in seconds to sleep before next recovery or backfill op when data is on HDD and "
                                 "journal is on SSD")
                .add_see_also("osd_recovery_sleep"),
            ...
            Option("osd_snap_trim_sleep_hybrid", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .set_flag(Option::FLAG_RUNTIME)
                .set_description(
                    "Time in seconds to sleep before next snap trim when data is on HDD and journal is on SSD"),
            ...
            Option("osd_heartbeat_grace", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(20).set_description(""),
            ...
            Option("osd_heartbeat_min_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
                .set_default(2000)
                .set_description("Minimum heartbeat packet size in bytes. Will add dummy payload if heartbeat packet "
            ...
            Option("osd_recovery_max_active_ssd", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                .set_default(10)
                .set_flag(Option::FLAG_RUNTIME)
                .set_description("Number of simultaneous active recovery operations per OSD (for non-rotational solid "
                                 "state devices)")
                .add_see_also("osd_recovery_max_active")
                .add_see_also("osd_recovery_max_active_hdd"),
            ...
            Option("osd_scrub_begin_week_day", Option::TYPE_INT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("0 = Sunday, 1 = Monday, etc. Use osd_scrub_begin_week_day=0 "
                                      "osd_scrub_end_week_day=0 for the entire week.")
            ...
            Option("osd_scrub_interval_randomize_ratio", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("This prevents a scrub 'stampede' by randomly varying the scrub intervals so "
                                      "that they are soon uniformly distributed over the week")
                .add_see_also("osd_scrub_min_interval"),

            Option("osd_scrub_backoff_ratio", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                .set_default(.66)
                .set_long_description("This is the precentage of ticks that do NOT schedule scrubs, 66% means that 1 "
                                      "out of 3 ticks will schedule scrubs")
            ...
            Option("osd_scrub_max_preemptions", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                .set_default(5)
                .set_min_max(0, 30)
                .set_description("Set the maximum number of times we will preempt a deep scrub due to a client "
            ...
            Option("osd_deep_scrub_randomize_ratio", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("This prevents a deep scrub 'stampede' by spreading deep scrubs so they are "
            ...
            Option("osd_class_load_list", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                .set_default("cephfs hello journal lock log numops "
                             "otp rbd refcount rgw rgw_gc timeindex user version cas cmpomap queue 2pc_queue fifo")
                .set_description(""),
            ...
            Option("osd_kill_backfill_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),
            ...
            Option("osd_target_pg_log_entries_per_osd", Option::TYPE_UINT, Option::LEVEL_DEV)
                ...
                .set_description(
                    "target number of PG entries total on an OSD - limited per pg by the min and max options below")
            ...
            Option("osd_pg_log_dups_tracked", Option::TYPE_UINT, Option::LEVEL_DEV)
                .set_default(3000)
                .set_description("how many versions back to track in order to detect duplicate ops; this is combined "
                                 "with both the regular pg log entries and additional minimal dup detection entries")
                .add_service("osd")
                .add_see_also("osd_min_pg_log_entries")
                .add_see_also("osd_max_pg_log_entries"),
            ...
            Option("osd_object_clean_region_max_num_intervals", Option::TYPE_INT, Option::LEVEL_DEV)
                ...
                .set_long_description("partial recovery uses multiple intervals to record the clean part of the object"
                                      "when the number of intervals is greater than "
                                      "osd_object_clean_region_max_num_intervals, minimum interval will be trimmed"
            ...
            Option("osd_pg_log_trim_min", Option::TYPE_UINT, Option::LEVEL_DEV)
                .set_default(100)
                .set_description("Minimum number of log entries to trim at once. This lets us trim in larger batches "
                                 "rather than with each write.")
                .add_see_also("osd_max_pg_log_entries")
                .add_see_also("osd_min_pg_log_entries"),
            ...
            Option("osd_force_auth_primary_missing_objects", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                .set_default(100)
                .set_description(
                    "Approximate missing objects above which to force auth_log_shard to be primary temporarily"),
            ...
            Option("osd_max_pg_per_osd_hard_ratio", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "OSD will refuse to instantiate PG if the number of PG it serves exceeds this number.")
            ...
            Option("osd_op_log_threshold", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(5).set_description(""),
            ...
            Option("osd_debug_op_order", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),
            ...
            Option("osd_debug_no_acting_change", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false),
            Option("osd_debug_no_purge_strays", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false),
            ...
            Option("osd_delete_sleep_hybrid", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                .set_default(1)
                .set_flag(Option::FLAG_RUNTIME)
                .set_description("Time in seconds to sleep before next removal transaction when data is on HDD and "
                                 "journal is on SSD"),
            ...
            Option("osd_fast_shutdown", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "Setting this to false makes the OSD do a slower teardown of all state when it receives a SIGINT "
                    "or SIGTERM or when shutting down for any other reason.  That slow shutdown is primarilyy useful "
                    "for doing memory leak checking with valgrind."),
            ...
            Option("osd_fast_shutdown_notify_mon", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("Tell the monitor the OSD is shutting down on immediate shutdown. This helps "
                                      "with cluster log messages from other OSDs reporting it immediately failed.")
            ...
            Option("osd_tracing", Option::TYPE_BOOL, Option::LEVEL_ADVANCED).set_default(false).set_description(""),

            Option("osd_function_tracing", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                .set_default(false)
                .set_description(""),

            Option("osd_fast_info", Option::TYPE_BOOL, Option::LEVEL_ADVANCED).set_default(true).set_description(""),
            ...
            Option("leveldb_block_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED).set_default(0).set_description(""),

            Option("leveldb_bloom_size", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(0).set_description(""),
            ...
            Option("rocksdb_perf", Option::TYPE_BOOL, Option::LEVEL_ADVANCED).set_default(false).set_description(""),
            ...
            Option("rocksdb_bloom_bits_per_key", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "RocksDB bloom filters can be used to quickly answer the question of whether or not a key may "
                    "exist or definitely does not exist in a given RocksDB SST file without having to read all keys "
                    "into memory.  Using a higher bit value decreases the likelihood of false positives at the expense "
                    "of additional disk space and memory consumption when the filter is loaded into RAM.  The current "
                    "default value of 20 was found to provide significant performance gains when getattr calls are "
                    "made (such as during new object creation in bluestore) without significant memory overhead or "
                    "cache pollution when combined with rocksdb partitioned index filters.  See: "
                    "https://github.com/facebook/rocksdb/wiki/Partitioned-Index-Filters for more information."),
            ...
            Option("rocksdb_cache_index_and_filter_blocks", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "By default RocksDB will load an SST file's index and bloom filters into memory when it is opened "
                    "and remove them from memory when an SST file is closed.  Thus, memory consumption by indices and "
                    "bloom filters is directly tied to the number of concurrent SST files allowed to be kept open.  "
                    "This option instead stores cached indicies and filters in the block cache where they directly "
                    "compete with other cached data.  By default we set this option to true to better account for and "
                    "bound rocksdb memory usage and keep filters in memory even when an SST file is closed."),
            ...
            Option("rocksdb_cache_index_and_filter_blocks_with_high_priority", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "A downside of setting rocksdb_cache_index_and_filter_blocks to true is that regular data can push "
                    "indices and filters out of memory.  Setting this option to true means they are cached with higher "
                    "priority than other data and should typically stay in the block cache."),
            ...
            Option("rocksdb_pin_l0_filter_and_index_blocks_in_cache", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "A downside of setting rocksdb_cache_index_and_filter_blocks to true is that regular data can push "
                    "indices and filters out of memory.  Setting this option to true means that level 0 SST files will "
                    "always have their indices and filters pinned in the block cache."),
            ...
            Option("rocksdb_index_type", Option::TYPE_STR, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "This option controls the table index type.  binary_search is a space efficient index block that "
                    "is optimized for block-search-based index. hash_search may improve prefix lookup performance at "
                    "the expense of higher disk and memory usage and potentially slower compactions.  two_level is an "
                    "experimental index type that uses two binary search indexes and works in conjunction with "
                    "partition filters.  See: http://rocksdb.org/blog/2017/05/12/partitioned-index-filter.html"),
            ...
            Option("rocksdb_partition_filters", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "This is an experimental option for rocksdb that works in conjunction with two_level indices to "
                    "avoid having to keep the entire filter/index in cache when cache_index_and_filter_blocks is true. "
                    " The idea is to keep a much smaller top-level index in heap/cache and then opportunistically "
                    "cache the lower level indices.  See: "
            ...
            Option("rocksdb_cf_compact_on_deletion", Option::TYPE_BOOL, Option::LEVEL_DEV)
                .set_default(false)
                .set_description(
                    "Compact the column family when a certain number of tombstones are observed within a given window.")
                .set_long_description(
                    "This setting instructs RocksDB to compact a column family when a certain number of tombstones are "
                    "observed during iteration within a certain sliding window. For instance if "
                    "rocksdb_cf_compact_on_deletion_sliding_window is 8192 and rocksdb_cf_compact_on_deletion_trigger "
                    "is 4096,  then once 4096 tombstones are observed after iteration over 8192 entries, the column "
                    "family will be compacted.")
                .add_see_also(
                    {"rocksdb_cf_compact_on_deletion_sliding_window", "rocksdb_cf_compact_on_deletion_trigger"}),
            ...
            Option("osd_max_attr_size", Option::TYPE_UINT, Option::LEVEL_ADVANCED).set_default(0).set_description(""),
            ...
            Option("osd_max_write_op_reply_len", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("This value caps the amount of data (per op; a request may have many ops) that "
                                      "will be sent back to the client and recorded in the PG log."),
            ...
            Option("osd_bench_duration", Option::TYPE_UINT, Option::LEVEL_ADVANCED).set_default(30).set_description(""),
            ...
            Option("osd_memory_target", Option::TYPE_SIZE, Option::LEVEL_BASIC)
                ...
                .set_description(
                    "When tcmalloc and cache autotuning is enabled, try to keep this many bytes mapped in memory.")
                .set_long_description(
            ...
            Option("osd_memory_target_cgroup_limit_ratio", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .add_see_also("osd_memory_target")
                .set_description(
                    "Set the default value for osd_memory_target to the cgroup memory limit (if set) times this value")
            ...
            Option("osd_memory_base", Option::TYPE_SIZE, Option::LEVEL_DEV)
                ...
                .set_flag(Option::FLAG_RUNTIME)
                .add_see_also("bluestore_cache_autotune")
                .set_description("When tcmalloc and cache autotuning is enabled, estimate the minimum amount of memory "
                                 "in bytes the OSD will need."),
            ...
            Option("osd_memory_expected_fragmentation", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                ...
                .add_see_also("bluestore_cache_autotune")
                .set_description(
                    "When tcmalloc and cache autotuning is enabled, estimate the percent of memory fragmentation."),
            ...
            Option("osd_memory_cache_min", Option::TYPE_SIZE, Option::LEVEL_DEV)
                ...
                .add_see_also("bluestore_cache_autotune")
                .set_description(
                    "When tcmalloc and cache autotuning is enabled, set the minimum amount of memory used for caches."),

            Option("osd_memory_cache_resize_interval", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                .set_default(1)
                .add_see_also("bluestore_cache_autotune")
                .set_description(
                    "When tcmalloc and cache autotuning is enabled, wait this many seconds between resizing caches."),
            ...
            Option("bdev_inject_crash", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

            Option("bdev_inject_crash_flush_delay", Option::TYPE_INT, Option::LEVEL_DEV)
                .set_default(2)
                .set_description(""),

            Option("bdev_aio", Option::TYPE_BOOL, Option::LEVEL_ADVANCED).set_default(true).set_description(""),

            Option("bdev_aio_poll_ms", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(250).set_description(""),

            Option("bdev_aio_max_queue_depth", Option::TYPE_INT, Option::LEVEL_ADVANCED)
                .set_default(1024)
                .set_description(""),

            Option("bdev_aio_reap_max", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(16).set_description(""),

            Option("bdev_block_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED).set_default(4_K).set_description(""),

            Option("bdev_debug_aio", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),
            ...
            Option("bdev_flock_retry", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("The number of times to retry on getting the block device lock. "
            ...
            Option("bluefs_failed_shared_alloc_cooldown", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                .set_default(600)
                .set_description("Duration(in seconds) until the next attempt to use "
                                 "'bluefs_shared_alloc_size' after facing ENOSPC failure.")
                .set_long_description("Cooldown period(in seconds) when BlueFS uses shared/slow device "
            ...
            Option("bluefs_buffered_io", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "When this option is enabled, bluefs will in some cases perform buffered reads.  This allows the "
                    "kernel page cache to act as a secondary cache for things like RocksDB compaction.  For example, "
                    "if the rocksdb block cache isn't large enough to hold blocks from the compressed SST files "
                    "itself, they can be read from page cache instead of from the disk."),
            ...
            Option("bluefs_replay_recovery", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "If BlueFS log grows to extreme sizes (200GB+) it is likely that it becames unreadable. "
            ...
            Option("bluefs_check_for_zeros", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "Looks into data read to check if there is a 4K block entirely filled with zeros. "
            ...
            Option("bluefs_check_volume_selector_often", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "Periodically checks if current volume selector does not diverge from the valid state. "
                    "Reference is constructed from bluefs inode table. Asserts on inconsistency. "
            ...
            Option("bluestore_bluefs", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "BlueFS allows rocksdb to share the same physical device(s) as the rest of BlueStore.  It should "
                    "be used in all cases unless testing/developing an alternative metadata database for BlueStore."),
            ...
            Option("bluestore_spdk_mem", Option::TYPE_SIZE, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "If running multiple SPDK instances per node, you must specify the amount of dpdk memory size in "
                    "MB each instance will use, to make sure each instance uses its own dpdk memory"),

            Option("bluestore_spdk_coremask", Option::TYPE_STR, Option::LEVEL_DEV)
                .set_default("0x1")
                .set_description("A hexadecimal bit mask of the cores to run on. Note the core numbering can change "
                                 "between platforms and should be determined beforehand"),

            Option("bluestore_spdk_max_io_completion", Option::TYPE_UINT, Option::LEVEL_DEV)
                .set_default(0)
                .set_description("Maximal I/Os to be batched completed while checking queue pair completions, 0 means "
                                 "let spdk library determine it"),
            ...
            Option("bluestore_block_create", Option::TYPE_BOOL, Option::LEVEL_DEV)
                ...
                .add_see_also("bluestore_block_path")
                .add_see_also("bluestore_block_size"),
            ...
            Option("bluestore_csum_type", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("crc32c, xxhash32, and xxhash64 are available.  The _16 and _8 variants use only "
                                      "a subset of the bits for more compact (but less reliable) checksumming."),
            ...
            Option("bluestore_retry_disk_reads", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("Retries to read data from the disk this many times when checksum validation "
            ...
            Option("bluestore_min_alloc_size", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "A smaller allocation size generally means less data is read and then rewritten when a "
                    "copy-on-write operation is triggered (e.g., when writing to something that was recently "
                    "snapshotted).  Similarly, less data is journaled before performing an overwrite (writes smaller "
                    "than min_alloc_size must first pass through the BlueStore journal).  Larger values of "
                    "min_alloc_size reduce the amount of metadata required to describe the on-disk layout and reduce "
            ...
            Option("bluestore_prefer_deferred_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
                ...
                .set_flag(Option::FLAG_RUNTIME)
                .set_description(
                    "Writes smaller than this size will be written to the journal and then asynchronously written to "
                    "the device.  This can be beneficial when using rotational media where seeks are expensive, and is "
                    "helpful both with and without solid state journal/wal devices."),
            ...
            Option("bluestore_compression_mode", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("'none' means never use compression.  'passive' means use compression when "
                                      "clients hint that data is compressible.  'aggressive' means use compression "
                                      "unless clients hint that data is not compressible.  This option is used when "
                                      "the per-pool property for the compression mode is not present."),
            ...
            Option("bluestore_compression_algorithm", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("This controls the default compressor to use (if any) if the per-pool property "
            ...
            Option("bluestore_compression_min_blob_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
                ...
                .set_flag(Option::FLAG_RUNTIME)
                .set_description(
                    "Maximum chunk size to apply compression to when random access is expected for an object.")
            ...
            Option("bluestore_compression_max_blob_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
                ...
                .set_flag(Option::FLAG_RUNTIME)
                .set_description(
                    "Maximum chunk size to apply compression to when non-random access is expected for an object.")
                .set_long_description("Chunks larger than this are broken into smaller chunks before being compressed"),
            ...
            Option("bluestore_compression_max_blob_size_ssd", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
                ...
                .set_flag(Option::FLAG_RUNTIME)
                .set_description(
                    "Default value of bluestore_compression_max_blob_size for non-rotational (solid state) media")
                .add_see_also("bluestore_compression_max_blob_size"),
            ...
            Option("bluestore_max_blob_size", Option::TYPE_SIZE, Option::LEVEL_DEV)
                ...
                .set_flag(Option::FLAG_RUNTIME)
                .set_description("")
                .set_long_description("Bluestore blobs are collections of extents (ie on-disk data) originating from "
                                      "one or more objects.  Blobs can be compressed, typically have checksum data, "
                                      "may be overwritten, may be shared (with an extent ref map), or split.  This "
                                      "setting controls the maximum size a blob is allowed to be."),
            ...
            Option("bluestore_compression_required_ratio", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("If we compress data and get less than this we discard the result and store the "
            ...
            Option("bluestore_extent_map_shard_target_size_slop", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                .set_default(.2)
                .set_description(
                    "Ratio above/below target for a shard when trying to align to an existing extent or blob boundary"),
            ...
            Option("bluestore_cache_size", Option::TYPE_SIZE, Option::LEVEL_DEV)
                ...
                .set_long_description("This includes data and metadata cached by BlueStore as well as memory devoted "
                                      "to rocksdb's cache(s)."),
            ...
            Option("bluestore_max_deferred_txc", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
                .set_default(32)
                .set_description(
                    "Max transactions with deferred writes that can accumulate before we force flush deferred writes"),
            ...
            Option("bluestore_rocksdb_options", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                .set_default(
                    "compression=kNoCompression,max_write_buffer_number=4,min_write_buffer_number_to_merge=1,recycle_"
                    "log_file_num=4,write_buffer_size=268435456,writable_file_max_buffer_size=0,compaction_readahead_"
                    "size=2097152,max_background_compactions=2,max_total_wal_size=1073741824")
            ...
            Option("bluestore_rocksdb_options_annex", Option::TYPE_STR, Option::LEVEL_ADVANCED)
                .set_default("")
                .set_description("An addition to bluestore_rocksdb_options. Allows setting rocksdb options without "
            ...
            Option("bluestore_rocksdb_cfs", Option::TYPE_STR, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "Space separated list of elements: column_def [ '=' rocksdb_options ]. "
            ...
            Option("bluestore_fsck_shared_blob_tracker_size", Option::TYPE_FLOAT, Option::LEVEL_DEV)
                ...
                .set_flag(Option::FLAG_RUNTIME)
                .set_description(
                    "Size(a fraction of osd_memory_target, defaults to 128MB) of a hash table to track shared blobs "
                    "ref counts. Higher the size, more precise is the tracker -> less overhead during the repair.")
                .add_see_also("osd_memory_target"),
            ...
            Option("bluestore_clone_cow", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .set_flag(Option::FLAG_RUNTIME)
                .set_description(
                    "Use copy-on-write when cloning objects (versus reading and rewriting them at clone time)"),
            ...
            Option("bluestore_fsck_error_on_no_per_pool_stats", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                .set_default(false)
                .set_description(
                    "Make fsck error (instead of warn) when bluestore lacks per-pool stats, e.g., after an upgrade"),
            ...
            Option("bluestore_debug_enforce_settings", Option::TYPE_STR, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "'hdd' enforces settings intended for BlueStore above a rotational drive. 'ssd' enforces settings "
                    "intended for BlueStore above a solid drive. 'default' - using settings for the actual hardware."),

            Option("bluestore_avl_alloc_ff_max_search_count", Option::TYPE_UINT, Option::LEVEL_DEV)
                .set_default(100)
                .set_description("Search for this many ranges in first-fit mode before switching over to to best-fit "
                                 "mode. 0 to iterate through all ranges for required chunk."),

            Option("bluestore_avl_alloc_ff_max_search_bytes", Option::TYPE_SIZE, Option::LEVEL_DEV)
                .set_default(16_M)
                .set_description("Maximum distance to search in first-fit mode before switching over to to best-fit "
                                 "mode. 0 to iterate through all ranges for required chunk."),

            Option("bluestore_avl_alloc_bf_threshold", Option::TYPE_UINT, Option::LEVEL_DEV)
                .set_default(131072)
                .set_description(
                    "Sets threshold at which shrinking max free chunk size triggers enabling best-fit mode.")
                .set_long_description(
                    "AVL allocator works in two modes: near-fit and best-fit. By default, it uses very fast near-fit "
                    "mode, "
                    "in which it tries to fit a new block near the last allocated block of similar size. The second "
                    "mode "
                    "is much slower best-fit mode, in which it tries to find an exact match for the requested "
                    "allocation. "
                    "This mode is used when either the device gets fragmented or when it is low on free space. "
                    "When the largest free block is smaller than 'bluestore_avl_alloc_bf_threshold', best-fit mode is "
                    "used.")
            ...
            Option("bluestore_avl_alloc_bf_free_pct", Option::TYPE_UINT, Option::LEVEL_DEV)
                .set_default(4)
                .set_description(
                    "Sets threshold at which shrinking free space (in %, integer) triggers enabling best-fit mode.")
                .set_long_description(
                    "AVL allocator works in two modes: near-fit and best-fit. By default, it uses very fast near-fit "
                    "mode, "
                    "in which it tries to fit a new block near the last allocated block of similar size. The second "
            ...
            Option("bluestore_volume_selection_policy", Option::TYPE_STR, Option::LEVEL_DEV)
                ...
                .set_long_description(
                    "Determines bluefs volume selection policy. 'use_some_extra' policy allows to override RocksDB "
                    "level "
                    "granularity and put high level's data to faster device even when the level doesn't completely fit "
                    "there. "
            ...
            Option("bluestore_volume_selection_reserved_factor", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
                .set_flag(Option::FLAG_STARTUP)
                .set_default(2.0)
                .set_description("DB level size multiplier. Determines amount of space at DB device to bar from the "
                                 "usage when 'use some extra' policy is in action. Reserved size is determined as "
            ...
            Option("bluestore_volume_selection_reserved", Option::TYPE_INT, Option::LEVEL_ADVANCED)
                ...
                .set_default(0)
                .set_description(
                    "Space reserved at DB device and not allowed for 'use some extra' policy usage. Overrides "
                    "'bluestore_volume_selection_reserved_factor' setting and introduces straightforward limit."),
            ...
            Option("kstore_max_ops", Option::TYPE_UINT, Option::LEVEL_ADVANCED).set_default(512).set_description(""),

            Option("kstore_max_bytes", Option::TYPE_SIZE, Option::LEVEL_ADVANCED).set_default(64_M).set_description(""),
            ...
            Option("filestore_sloppy_crc", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),
            ...
            Option("filestore_btrfs_snap", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(true).set_description(""),
            ...
            Option("filestore_zfs_snap", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),
            ...
            Option("filestore_split_multiple", Option::TYPE_INT, Option::LEVEL_DEV).set_default(2).set_description(""),

            Option("filestore_split_rand_factor", Option::TYPE_UINT, Option::LEVEL_DEV)
                .set_default(20)
                .set_description(""),

            Option("filestore_update_to", Option::TYPE_INT, Option::LEVEL_DEV).set_default(1000).set_description(""),

            Option("filestore_blackhole", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

            Option("filestore_fd_cache_size", Option::TYPE_INT, Option::LEVEL_DEV).set_default(128).set_description(""),
            ...
            Option("filestore_dump_file", Option::TYPE_STR, Option::LEVEL_DEV).set_default("").set_description(""),

            Option("filestore_kill_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

            Option("filestore_inject_stall", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

            Option("filestore_fail_eio", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(true).set_description(""),

            Option("filestore_debug_verify_split", Option::TYPE_BOOL, Option::LEVEL_DEV)
                .set_default(false)
                .set_description(""),

            Option("journal_dio", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(true).set_description(""),

            Option("journal_aio", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(true).set_description(""),

            Option("journal_force_aio", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

            Option("journal_block_size", Option::TYPE_SIZE, Option::LEVEL_DEV).set_default(4_K).set_description(""),

            Option("journal_block_align", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(true).set_description(""),
            ...
            Option("journal_replay_from", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),
            ...
            Option("journal_discard", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

            Option("fio_dir", Option::TYPE_STR, Option::LEVEL_ADVANCED).set_default("/tmp/fio").set_description(""),
            ...
            Option("rados_tracing", Option::TYPE_BOOL, Option::LEVEL_ADVANCED).set_default(false).set_description(""),
            ...
            Option("mgr_client_bytes", Option::TYPE_SIZE, Option::LEVEL_DEV).set_default(128_M).add_service("mgr"),

            Option("mgr_client_messages", Option::TYPE_UINT, Option::LEVEL_DEV).set_default(512).add_service("mgr"),

            Option("mgr_osd_bytes", Option::TYPE_SIZE, Option::LEVEL_DEV).set_default(512_M).add_service("mgr"),

            Option("mgr_osd_messages", Option::TYPE_UINT, Option::LEVEL_DEV).set_default(8192).add_service("mgr"),

            Option("mgr_mds_bytes", Option::TYPE_SIZE, Option::LEVEL_DEV).set_default(128_M).add_service("mgr"),

            Option("mgr_mds_messages", Option::TYPE_UINT, Option::LEVEL_DEV).set_default(128).add_service("mgr"),

            Option("mgr_mon_bytes", Option::TYPE_SIZE, Option::LEVEL_DEV).set_default(128_M).add_service("mgr"),

            Option("mgr_mon_messages", Option::TYPE_UINT, Option::LEVEL_DEV).set_default(128).add_service("mgr"),
            ...
            Option("event_tracing", Option::TYPE_BOOL, Option::LEVEL_ADVANCED).set_default(false).set_description(""),
            ...
            Option("debug_asserts_on_shutdown", Option::TYPE_BOOL, Option::LEVEL_DEV)
                .set_default(false)
                .set_description("Enable certain asserts to check for refcounting bugs on shutdown; see "
                                 "http://tracker.ceph.com/issues/21738"),
            ...
            Option("device_failure_prediction_mode", Option::TYPE_STR, Option::LEVEL_BASIC)
                ...
                .set_long_description("To disable prediction, use 'none',  'local' uses a prediction model that runs "
                                      "inside the mgr daemon.  'cloud' will share metrics with a cloud service and "
            ...
            Option("cephsqlite_lock_renewal_timeout", Option::TYPE_MILLISECS, Option::LEVEL_ADVANCED)
                ...
                .set_long_description("The amount of time before a running libcephsqlite VFS connection has to renew a "
                                      "lock on the database before the lock is automatically lost. If the lock is "
                                      "lost, the VFS will abort the process to prevent database corruption.")
                .set_min(100),
            ...
            Option("cephsqlite_blocklist_dead_locker", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
                ...
                .set_long_description(
                    "Require that the Ceph SQLite VFS blocklist the last dead owner of the database when cleanup was "
                    "incomplete. DO NOT CHANGE THIS UNLESS YOU UNDERSTAND THE RAMIFICATIONS. CORRUPTION MAY RESULT."),
...
std::vector<Option> get_rgw_options()
{
    return std::vector<Option>({
        ...
        Option("rgw_max_chunk_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The chunk size is the size of RADOS I/O requests that RGW sends when accessing "
        ...
        Option("rgw_max_put_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Plain object upload is capped at this amount of data. In order to upload larger "
        ...
        Option("rgw_enable_quota_threads", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The quota maintenance thread is responsible for quota related maintenance work. "
        ...
        Option("rgw_enable_gc_threads", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The garbage collection maintenance thread is responsible for garbage collector "
        ...
        Option("rgw_enable_lc_threads", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            .set_default(true)
            .set_description(
                "Enables the lifecycle maintenance thread. This is required on at least one rgw for each zone.")
            .set_long_description("The lifecycle maintenance thread is responsible for lifecycle related maintenance "
        ...
        Option("rgw_data", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("If this is set, the different Ceph system configurables (such as the keyring file "
        ...
        Option("rgw_cache_enabled", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The metadata cache holds metadata entries that RGW requires for processing "
        ...
        Option("rgw_dns_name", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("This is Needed for virtual hosting of buckets to work properly, unless configured "
        ...
        Option("rgw_dns_s3website_name", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("This is needed for virtual hosting of buckets, unless configured via zonegroup "
                                  "configuration."),
        ...
        Option("rgw_service_provider_name", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("As S3 or other cloud storage providers do, http response headers should contain the "
        ...
        Option("rgw_content_length_compat", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Try to handle requests with abiguous multiple content length headers "
        ...
        Option("rgw_relaxed_region_enforcement", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Enable requests such as bucket creation to succeed irrespective of region "
        ...
        Option("rgw_lc_lock_max_time", Option::TYPE_INT, Option::LEVEL_DEV).set_default(90).set_description(""),

        Option("rgw_lc_thread_delay", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            .set_default(0)
            .set_description(
                "Delay after processing of bucket listing chunks (i.e., per 1000 entries) in milliseconds"),
        ...
        Option("rgw_lc_max_worker", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Number of LCWorker tasks that will run in parallel--used to permit >1 "
        ...
        Option("rgw_lc_max_wp_worker", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Number of threads in per-LCWorker workpools--used to accelerate "
        ...
        Option("rgw_lc_max_objs", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Number of RADOS objects to use for storing lifecycle index. This "
        ...
        Option("rgw_lc_debug_interval", Option::TYPE_INT, Option::LEVEL_DEV)
            ...
            .set_long_description(
                "For debugging RGW LifeCycle, the number of seconds that are equivalent to "
        ...
        Option("rgw_mp_lock_max_time", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Time length to allow completion of a multipart upload operation. This is done "
                                  "to prevent concurrent completions on the same object with the same upload id."),

        Option("rgw_script_uri", Option::TYPE_STR, Option::LEVEL_DEV).set_default("").set_description(""),

        Option("rgw_request_uri", Option::TYPE_STR, Option::LEVEL_DEV).set_default("").set_description(""),
        ...
        Option("rgw_swift_url", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Used in conjunction with rgw internal swift authentication. This affects the "
        ...
        Option("rgw_swift_auth_url", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Default url to which RGW connects and verifies tokens for v1 auth (if not using "
                                  "internal swift auth)."),
        ...
        Option("rgw_swift_enforce_content_length", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Whether content length header is needed when listing containers. When this is "
        ...
        Option("rgw_keystone_admin_token_path", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            .set_default("")
            .set_description(
                "Path to a file containing the admin token (shared secret) that is used for the Keystone requests."),
        ...
        Option("rgw_keystone_token_cache_size", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Max number of Keystone tokens that will be cached. Token that is not cached "
        ...
        Option("rgw_keystone_implicit_tenants", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Implicitly create new users in their own tenant with the same name when "
        ...
        Option("rgw_s3_auth_order", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Order of authentication strategies to try for s3 authentication, the allowed "
        ...
        Option("rgw_enforce_swift_acls", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Should RGW enforce special Swift-only ACLs. Swift has a special ACL that gives "
        ...
        Option("rgw_print_continue", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Should RGW explicitly send 100 (continue) responses. This is mainly relevant when "
        ...
        Option("rgw_print_prohibited_content_length", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Specifies whether RGW violates RFC 7230 and sends Content-Length with 204 or 304 "
                                  "statuses."),
        ...
        Option("rgw_remote_addr_param", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("RGW will use this header to extract requests origin. When RGW runs behind "
        ...
        Option("rgw_op_thread_suicide_timeout", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

        Option("rgw_thread_pool_size", Option::TYPE_INT, Option::LEVEL_BASIC)
            .set_default(512)
            .set_description("RGW requests handling thread pool size.")
            .set_long_description("This parameter determines the number of concurrent requests RGW can process "
        ...
        Option("rgw_num_control_oids", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("RGW uses certain control objects to send messages between different RGW "
        ...
        Option("rgw_num_rados_handles", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("This param affects the number of separate librados handles it uses to "
        ...
        Option("rgw_verify_ssl", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("RGW can send requests to other RGW servers (e.g., in multi-site sync work). "
        ...
        Option("rgw_nfs_lru_lanes", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(5).set_description(""),

        Option("rgw_nfs_lru_lane_hiwat", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(911).set_description(""),

        Option("rgw_nfs_fhcache_partitions", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            .set_default(3)
            .set_description(""),

        Option("rgw_nfs_fhcache_size", Option::TYPE_INT, Option::LEVEL_ADVANCED).set_default(2017).set_description(""),
        ...
        Option("rgw_realm", Option::TYPE_STR, Option::LEVEL_ADVANCED).set_default("").set_description(""),
        ...
        Option("rgw_log_nonexistent_bucket", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("This config option applies to the ops log. When this option is set, the ops log "
        ...
        Option("rgw_log_object_name", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Defines the format of the RADOS objects names that ops log uses to store ops "
                                  "log data")
            .add_see_also("rgw_enable_ops_log"),
        ...
        Option("rgw_log_object_name_utc", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("If set, the names of the RADOS objects that hold the ops log data will be based "
        ...
        Option("rgw_usage_max_shards", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The number of RADOS objects that RGW will use in order to store the usage log "
                                  "data.")
        ...
        Option("rgw_enable_ops_log", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .add_see_also(
                {"rgw_log_nonexistent_bucket", "rgw_log_object_name", "rgw_ops_log_rados", "rgw_ops_log_socket_path"}),
        ...
        Option("rgw_ops_log_rados", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("If set, RGW will store ops log information in RADOS.")
            .add_see_also({"rgw_enable_ops_log"}),

        Option("rgw_ops_log_socket_path", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            .set_default("")
            .set_description("Unix domain socket path for ops log.")
            .set_long_description("Path to unix domain socket that RGW will listen for connection on. When connected, "
        ...
        Option("rgw_ops_log_data_backlog", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Maximum amount of data backlog that RGW can keep when ops log is configured to "
        ...
        Option("rgw_fcgi_socket_backlog", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Size of FastCGI connection backlog. This reflects the maximum number of new "
        ...
        Option("rgw_usage_log_flush_threshold", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("This is the max number of entries that will be held in the usage log, before it "
        ...
        Option("rgw_usage_log_tick_interval", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The number of seconds between consecutive usage log flushes. The usage log will "
        ...
        Option("rgw_init_timeout", Option::TYPE_INT, Option::LEVEL_BASIC)
            ...
            .set_long_description("The time length (in seconds) that RGW will allow for its initialization. RGW "
        ...
        Option("rgw_mime_types_file", Option::TYPE_STR, Option::LEVEL_BASIC)
            ...
            .set_long_description("The mime types file is needed in Swift when uploading an object. If object's "
        ...
        Option("rgw_gc_max_objs", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The number of garbage collector data shards, is the number of RADOS objects that "
        ...
        Option("rgw_gc_obj_min_wait", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The length of time (in seconds) that the RGW collector will wait before purging "
        ...
        Option("rgw_gc_processor_period", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The amount of time between the start of consecutive runs of the garbage collector "
        ...
        Option("rgw_gc_max_concurrent_io", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The maximum number of concurrent IO operations that the RGW garbage collection "
        ...
        Option("rgw_gc_max_trim_chunk", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .add_see_also(
                {"rgw_gc_max_objs", "rgw_gc_obj_min_wait", "rgw_gc_processor_max_time", "rgw_gc_max_concurrent_io"}),
        ...
        Option("rgw_gc_max_queue_size", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The maximum allowed size of each gc queue, and its value should not "
        ...
        Option("rgw_s3_success_create_obj_status", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("If not zero, this is the HTTP return code that will be returned on a successful S3 "
                                  "object creation."),

        Option("rgw_resolve_cname", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            .set_default(false)
            .set_description("Support vanity domain names via CNAME")
            .set_long_description("If true, RGW will query DNS when detecting that it's serving a request that was "
        ...
        Option("rgw_obj_stripe_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The size of an object stripe for RGW objects. This is the maximum size a backing "
        ...
        Option("rgw_extended_http_attrs", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Add new set of attributes that could be set on an object. These extra attributes "
        ...
        Option("rgw_defer_to_bucket_acls", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("If not empty, a string that selects that mode of operation. 'recurse' will use "
        ...
        Option("rgw_list_buckets_max_chunk", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("When RGW fetches lists of user's buckets from the backend, this is the max number "
        ...
        Option("rgw_md_log_max_shards", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The number of shards the RGW metadata log entries will reside in. This affects "
        ...
        Option("rgw_curl_buffersize", Option::TYPE_INT, Option::LEVEL_DEV)
            ...
            .set_long_description("Pass a long specifying your preferred size (in bytes) for the receive"
                                  "buffer in libcurl. "
                                  "See: https://curl.se/libcurl/c/CURLOPT_BUFFERSIZE.html"),

        Option("rgw_curl_wait_timeout_ms", Option::TYPE_INT, Option::LEVEL_DEV).set_default(1000).set_description(""),

        Option("rgw_curl_low_speed_limit", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            .set_default(1024)
            .set_long_description("It contains the average transfer speed in bytes per second that the "
        ...
        Option("rgw_curl_low_speed_time", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            .set_default(300)
            .set_long_description("It contains the time in number seconds that the transfer speed should be below "
        ...
        Option("rgw_obj_tombstone_cache_size", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The tombstone cache is used when doing a multi-zone data sync. RGW keeps "
        ...
        Option("rgw_data_log_window", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The data log keeps information about buckets that have objectst that were "
        ...
        Option("rgw_data_log_changes_size", Option::TYPE_INT, Option::LEVEL_DEV)
            ...
            .set_long_description("RGW will trigger update to the data log if the number of pending entries reached "
                                  "this number."),

        Option("rgw_data_log_num_shards", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            .set_default(128)
            .set_description("Number of data log shards")
            .set_long_description("The number of shards the RGW data log entries will reside in. This affects the "
        ...
        Option("rgw_bucket_default_quota_max_objects", Option::TYPE_INT, Option::LEVEL_BASIC)
            ...
            .set_long_description("The default quota configuration for max number of objects in a bucket. A "
        ...
        Option("rgw_bucket_default_quota_max_size", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The default quota configuration for total size of objects in a bucket. A "
                                  "negative number means 'unlimited'."),

        Option("rgw_expose_bucket", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            .set_default(false)
            .set_description("Send Bucket HTTP header with the response")
            .set_long_description("If true, RGW will send a Bucket HTTP header with the responses. The header will "
        ...
        Option("rgw_frontends", Option::TYPE_STR, Option::LEVEL_BASIC)
            ...
            .set_long_description("A comma delimited list of frontends configuration. Each configuration contains "
        ...
        Option("rgw_frontend_defaults", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            .set_default("beast ssl_certificate=config://rgw/cert/$realm/$zone.crt "
                         "ssl_private_key=config://rgw/cert/$realm/$zone.key")
        ...
        Option("rgw_user_default_quota_max_objects", Option::TYPE_INT, Option::LEVEL_BASIC)
            ...
            .set_long_description("The default quota configuration for total number of objects for a single user. A "
                                  "negative number means 'unlimited'."),

        Option("rgw_user_default_quota_max_size", Option::TYPE_INT, Option::LEVEL_BASIC)
            .set_default(-1)
            .set_description("User quota max size")
            .set_long_description("The default quota configuration for total size of objects for a single user. A "
                                  "negative number means 'unlimited'."),
        ...
        Option("rgw_multipart_min_part_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("When doing a multipart upload, each part (other than the last part) should be "
                                  "at least this size."),
        ...
        Option("rgw_olh_pending_timeout_sec", Option::TYPE_INT, Option::LEVEL_DEV)
            ...
            .set_long_description("OLH is a versioned object's logical head. Operations on it are journaled and "
        ...
        Option("rgw_user_max_buckets", Option::TYPE_INT, Option::LEVEL_BASIC)
            ...
            .set_long_description("A user can create at most this number of buckets. Zero means "
        ...
        Option("rgw_objexp_chunk_size", Option::TYPE_UINT, Option::LEVEL_DEV).set_default(100).set_description(""),

        Option("rgw_enable_static_website", Option::TYPE_BOOL, Option::LEVEL_BASIC)
            .set_default(false)
            .set_description("Enable static website APIs")
            .set_long_description("This configurable controls whether RGW handles the website control APIs. RGW can "
        ...
        Option("rgw_user_unique_email", Option::TYPE_BOOL, Option::LEVEL_BASIC)
            ...
            .set_long_description("Enforce builtin user accounts to have unique email addresses.  This "
        ...
        Option("rgw_log_http_headers", Option::TYPE_STR, Option::LEVEL_BASIC)
            ...
            .set_long_description("A comma delimited list of HTTP headers to log when seen, ignores case (e.g., "
        ...
        Option("rgw_num_async_rados_threads", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The number of concurrent RADOS IO operations that will be triggered for handling "
        ...
        Option("rgw_md_notify_interval_msec", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Length of time (in milliseconds) in which the master zone aggregates all the "
        ...
        Option("rgw_sync_lease_period", Option::TYPE_INT, Option::LEVEL_DEV).set_default(120).set_description(""),
        ...
        Option("rgw_sync_log_trim_max_buckets", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The maximum number of buckets to consider for bucket index log trimming each trim "
                                  "interval, regardless of the number of bucket index shards. Priority is given to "
                                  "buckets with the most sync activity over the last trim interval.")
        ...
        Option("rgw_sync_log_trim_min_cold_buckets", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description(
                "Of the `rgw_sync_log_trim_max_buckets` selected for bucket index log trimming each trim interval, at "
                "least this many of them must be 'cold' buckets. These buckets are selected in order from the list of "
                "all bucket instances, to guarantee that all buckets will be visited eventually.")
            .add_see_also("rgw_sync_log_trim_interval")
        ...
        Option("rgw_period_push_interval_max", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The max number of seconds to wait before retrying 'period push' after exponential "
                                  "backoff."),
        ...
        Option("rgw_safe_max_objects_per_shard", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("This is the max number of objects per bucket index shard that RGW considers "
        ...
        Option("rgw_shard_warning_threshold", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Warn if number of objects per shard in a specific bucket passed this percentage "
                                  "of the safe number.")
        ...
        Option("rgw_swift_custom_header", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("If not empty, specifies a name of HTTP header that can include custom data. When "
        ...
        Option("rgw_reshard_bucket_lock_duration", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            .set_default(360)
            .set_min(30)
            .set_description("Number of seconds the timeout on the reshard locks (bucket reshard lock and reshard log "
                             "lock) are set to. As a reshard proceeds these locks can be renewed/extended. If too "
                             "short, reshards cannot complete and will fail, causing a future reshard attempt. If too "
                             "long a hung or crashed reshard attempt will keep the bucket locked for an extended "
                             "period, not allowing RGW to detect the failed reshard attempt and recover.")
            .add_tag("performance")
            .add_service("rgw"),
        ...
        Option("rgw_reshard_batch_size", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            ...
            .set_min(8)
            .set_description(
                "Number of reshard entries to batch together before sending the operations to the CLS back-end")
            .add_tag("performance")
            .add_service("rgw"),
        ...
        Option("rgw_reshard_max_aio", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            ...
            .set_min(16)
            .set_description(
                "Maximum number of outstanding asynchronous I/O operations to allow at a time during resharding")
            .add_tag("performance")
            .add_service("rgw"),
        ...
        Option("rgw_trust_forwarded_https", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("When a proxy in front of radosgw is used for ssl termination, radosgw "
        ...
        Option("rgw_crypt_s3_kms_backend", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_description("Where the SSE-KMS encryption keys are stored. Supported KMS "
        ...
        Option("rgw_crypt_vault_token_file", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            .set_default("")
            .set_description("If authentication method is 'token', provide a path to the token file, "
        ...
        Option("rgw_dynamic_resharding", Option::TYPE_BOOL, Option::LEVEL_BASIC)
            ...
            .set_long_description("If true, RGW will dynamicall increase the number of shards in buckets that have "
        ...
        Option("rgw_max_objs_per_shard", Option::TYPE_UINT, Option::LEVEL_BASIC)
            ...
            .set_long_description("This is the max number of objects per bucket index shard that RGW will "
        ...
        Option("rgw_max_dynamic_shards", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("This is the maximum number of bucket index shards that dynamic "
        ...
        Option("rgw_max_concurrent_requests", Option::TYPE_INT, Option::LEVEL_BASIC)
            ...
            .set_long_description("Maximum number of concurrent HTTP requests that the beast frontend "
        ...
        Option("rgw_default_data_log_backing", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Whether to use the older OMAP backing store or the high performance "
...
static std::vector<Option> get_rbd_options()
{
    return std::vector<Option>({
        ...
        Option("rbd_default_features", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("RBD features are only applicable for v2 images. This setting accepts "
        ...
        Option("rbd_concurrent_management_ops", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            ...
            .set_min(1)
            .set_description(
                "how many operations can be in flight for a management operation like deleting or resizing an image"),
        ...
        Option("rbd_enable_alloc_hint", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
            .set_default(true)
            .set_description(
                "when writing a object, it will issue a hint to osd backend to indicate the expected size object need"),
        ...
        Option("rbd_journal_max_concurrent_object_sets", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            .set_default(0)
            .set_description(
                "maximum number of object sets a journal client can be behind before it is automatically unregistered"),
        ...
        Option("rbd_mtime_update_interval", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            ...
            .set_min(0)
            .set_description(
                "RBD Image modify timestamp refresh interval. Set to 0 to disable modify timestamp update."),

        Option("rbd_atime_update_interval", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            .set_default(60)
            .set_min(0)
            .set_description(
                "RBD Image access timestamp refresh interval. Set to 0 to disable access timestamp update."),
        ...
        Option("rbd_io_scheduler_simple_max_delay", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            .set_default(0)
            .set_min(0)
            .set_description("maximum io delay (in milliseconds) for simple io scheduler (if set to 0 dalay is "
                             "calculated based on latency stats)"),
        ...
        Option("rbd_persistent_cache_path", Option::TYPE_STR, Option::LEVEL_ADVANCED)
            .set_default("/tmp")
            .set_description(
                "location of the persistent write back cache in a DAX-enabled filesystem on persistent memory"),
...
static std::vector<Option> get_rbd_mirror_options()
{
    return std::vector<Option>({
        ...
        Option("rbd_mirror_image_policy_rebalance_timeout", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
            .set_default(0)
            .set_description(
                "number of seconds policy should be idle before trigerring reshuffle (rebalance) of images"),
        ...
        Option("rbd_mirror_perf_stats_prio", Option::TYPE_INT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("The daemon will send perf counter data to the "
                                  ...
                                  "mgr_stats_threshold.")
            .set_min_max((int64_t)PerfCountersBuilder::PRIO_DEBUGONLY, (int64_t)PerfCountersBuilder::PRIO_CRITICAL + 1),
        ...
        Option("rbd_mirror_memory_base", Option::TYPE_SIZE, Option::LEVEL_DEV)
            .set_default(768_M)
            .add_see_also("rbd_mirror_memory_autotune")
            .set_description("When tcmalloc and cache autotuning is enabled, estimate the minimum amount of memory in "
                             "bytes the rbd-mirror daemon will need."),
        ...
        Option("rbd_mirror_memory_cache_min", Option::TYPE_SIZE, Option::LEVEL_DEV)
            ...
            .add_see_also("rbd_mirror_memory_autotune")
            .set_description(
                "When tcmalloc and cache autotuning is enabled, set the minimum amount of memory used for cache."),
    ...
    return std::vector<Option>(
        {Option("mds_alternate_name_max", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
         ...
         Option("mds_join_fs", Option::TYPE_STR, Option::LEVEL_BASIC)
             ...
             .set_long_description("This setting indicates which file system name the MDS should prefer to join "
                                   "(affinity). The monitors will try to have the MDS cluster safely reach a state "
                                   "where all MDS have strong affinity, even via failovers to a standby.")
             .set_flag(Option::FLAG_RUNTIME),
         ...
         Option("mds_cache_memory_limit", Option::TYPE_SIZE, Option::LEVEL_BASIC)
             ...
             .set_flag(Option::FLAG_RUNTIME)
             .set_long_description(
                 "This sets a target maximum memory usage of the MDS cache and is the primary tunable to limit the MDS "
                 "memory usage. The MDS will try to stay under a reservation of this limit (by default 95%; 1 - "
                 "mds_cache_reservation) by trimming unused metadata in its cache and recalling cached items in the "
                 "client caches. It is possible for the MDS to exceed this limit due to slow recall from clients. The "
                 "mds_health_cache_threshold (150%) sets a cache full threshold for when the MDS signals a cluster "
                 "health warning."),
         ...
         Option("mds_session_cache_liveness_decay_rate", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
             ...
             .set_flag(Option::FLAG_RUNTIME)
             .set_long_description(
                 "This determines how long a session needs to be quiescent before the MDS begins preemptively "
                 "recalling capabilities. The default of 5 minutes will cause 10 halvings of the decay counter after 1 "
                 "hour, or 1/1024. The default magnitude of 10 (1^10 or 1024) is chosen so that the MDS considers a "
                 "previously chatty session (approximately) to be quiescent after 1 hour."),
         ...
         Option("mds_session_cache_liveness_magnitude", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
             ...
             .set_flag(Option::FLAG_RUNTIME)
             .set_long_description(
                 "This is the order of magnitude difference (in base 2) of the internal liveness decay counter and the "
                 "number of capabilities the session holds. When this difference occurs, the MDS treats the session as "
         ...
         Option("mds_session_cap_acquisition_decay_rate", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
             ...
             .set_flag(Option::FLAG_RUNTIME)
             .set_long_description("The half-life for the session cap acquisition counter of caps acquired by readdir. "
                                   "This is used for throttling readdir requests from clients slow to release caps."),
         ...
         Option("mds_session_max_caps_throttle_ratio", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
             .set_default(1.1)
             .set_description("ratio of mds_max_maps_per_client that client must exceed before readdir may be "
                              "throttled by cap acquisition throttle"),

         Option("mds_cap_acquisition_throttle_retry_request_timeout", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
             .set_default(0.5)
             .set_description(
                 "timeout in seconds after which a client request is retried due to cap acquisition throttling"),

         Option("mds_freeze_tree_timeout", Option::TYPE_FLOAT, Option::LEVEL_DEV).set_default(30).set_description(""),
         ...
         Option("mds_reconnect_timeout", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
             .set_default(45)
             .set_description(
                 "timeout in seconds to wait for clients to reconnect during MDS reconnect recovery state"),
         ...
         Option("mds_dirstat_min_interval", Option::TYPE_FLOAT, Option::LEVEL_DEV).set_default(1).set_description(""),
         ...
         Option("mds_replay_unsafe_with_closed_session", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
             ...
             .set_flag(Option::FLAG_STARTUP)
             .set_description(
                 "complete all the replay request when mds is restarted, no matter the session is closed or not"),
         ...
         Option("mds_log_pause", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),
         ...
         Option("mds_log_warn_factor", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
             ...
             .set_min(1.0)
             .set_flag(Option::FLAG_RUNTIME)
             .set_description("trigger MDS_HEALTH_TRIM warning when the mds log is longer than mds_log_max_segments * "
                              "mds_log_warn_factor"),
         ...
         Option("mds_export_ephemeral_random", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
             ...
             .set_long_description("probabilistically pin the loaded directory inode and the subtree beneath it to an "
                                   "MDS based on the consistent hash of the inode number. The higher this value the "
                                   "more likely the loaded subtrees get pinned"),
         ...
         Option("mds_export_ephemeral_distributed", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
             ...
             .set_long_description("pin the immediate child directories of the loaded directory inode based on the "
                                   "consistent hash of the child's inode number. "),
         ...
         Option("mds_bal_fragment_dirs", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
             ...
             .set_long_description(
                 "Directory fragmentation is a standard feature of CephFS that allows sharding directories across "
                 "multiple objects for performance and stability. Additionally, this allows fragments to be "
                 "distributed across multiple active MDSs to increase throughput. Disabling (new) fragmentation should "
                 "only be done in exceptional circumstances and may lead to performance issues."),
         ...
         Option("mds_bal_max", Option::TYPE_INT, Option::LEVEL_DEV).set_default(-1).set_description(""),

         Option("mds_bal_max_until", Option::TYPE_INT, Option::LEVEL_DEV).set_default(-1).set_description(""),

         Option("mds_bal_mode", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),
         ...
         Option("mds_bal_min_start", Option::TYPE_FLOAT, Option::LEVEL_DEV).set_default(.2).set_description(""),

         Option("mds_bal_need_min", Option::TYPE_FLOAT, Option::LEVEL_DEV).set_default(.8).set_description(""),

         Option("mds_bal_need_max", Option::TYPE_FLOAT, Option::LEVEL_DEV).set_default(1.2).set_description(""),

         Option("mds_bal_midchunk", Option::TYPE_FLOAT, Option::LEVEL_DEV).set_default(.3).set_description(""),

         Option("mds_bal_minchunk", Option::TYPE_FLOAT, Option::LEVEL_DEV).set_default(.001).set_description(""),
         ...
         Option("mds_shutdown_check", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_thrash_exports", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_thrash_fragments", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_dump_cache_on_map", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

         Option("mds_dump_cache_after_rejoin", Option::TYPE_BOOL, Option::LEVEL_DEV)
             .set_default(false)
             .set_description(""),

         Option("mds_verify_scatter", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

         Option("mds_debug_scatterstat", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

         Option("mds_debug_frag", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

         Option("mds_debug_auth_pins", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

         Option("mds_debug_subtrees", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),
         ...
         Option("mds_kill_mdstable_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_max_export_size", Option::TYPE_SIZE, Option::LEVEL_DEV).set_default(20_M).set_description(""),

         Option("mds_kill_export_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_kill_import_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_kill_link_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_kill_rename_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_kill_openc_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_kill_journal_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_kill_journal_expire_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_kill_journal_replay_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_journal_format", Option::TYPE_UINT, Option::LEVEL_DEV).set_default(1).set_description(""),

         Option("mds_kill_create_at", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_inject_health_dummy", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false).set_description(""),

         Option("mds_kill_skip_replaying_inotable", Option::TYPE_BOOL, Option::LEVEL_DEV)
             .set_default(false)
             .set_description("Ceph will skip replaying the inotable when replaying the journal, and the premary MDS "
                              "will crash, while the replacing MDS won't. (for testing only)"),
         ...
         Option("mds_wipe_sessions", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_wipe_ino_prealloc", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(0).set_description(""),

         Option("mds_skip_ino", Option::TYPE_INT, Option::LEVEL_DEV).set_default(0).set_description(""),
         ...
         Option("mds_op_log_threshold", Option::TYPE_INT, Option::LEVEL_DEV).set_default(5).set_description(""),
         ...
         Option("mds_verify_backtrace", Option::TYPE_UINT, Option::LEVEL_DEV).set_default(1).set_description(""),
         ...
         Option("mds_hack_allow_loading_invalid_metadata", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
             .set_default(0)
             .set_description("INTENTIONALLY CAUSE DATA LOSS by bypasing checks for invalid metadata on disk. Allows "
                              "testing repair tools."),

         Option("mds_defer_session_stale", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(true),

         Option("mds_inject_migrator_session_race", Option::TYPE_BOOL, Option::LEVEL_DEV).set_default(false),
         ...
         Option("mds_cap_revoke_eviction_timeout", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
             .set_default(0)
             .set_description("number of seconds after which clients which have not responded to cap revoke messages "
                              "by the MDS are evicted."),
         ...
         Option("mds_dump_cache_threshold_formatter", Option::TYPE_SIZE, Option::LEVEL_DEV)
             ...
             .set_long_description("Disallow MDS from dumping caches to formatter via \"dump cache\" command if cache "
         ...
         Option("mds_dump_cache_threshold_file", Option::TYPE_SIZE, Option::LEVEL_DEV)
             ...
             .set_long_description("Disallow MDS from dumping caches to file via \"dump cache\" command if cache usage "
         ...
         Option("mds_ping_grace", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
             ...
             .set_long_description("timeout for replying to a ping message sent by rank 0 after which an active MDS "
                                   "considered laggy (delayed metrics) by rank 0."),
         ...
         Option("defer_client_eviction_on_laggy_osds", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
             ...
             .set_long_description("Laggy OSD(s) can make clients laggy or unresponsive, this can lead to their "
                                   "eviction, this option once enabled can help defer client eviction."),
         ...
         Option("mds_session_metadata_threshold", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
             ...
             .set_long_description("Evict clients which are not advancing their request tids which causes a large "
                                   "buildup of session metadata (`completed_requests`) in the MDS causing the MDS to "
                                   "go read-only since the RADOS operation exceeds the size threashold. This config is "
                                   "the maximum size (in bytes) that a session metadata (encoded) can grow.")});
}
...
std::vector<Option> get_mds_client_options()
{
    return std::vector<Option>(
        {Option("client_cache_size", Option::TYPE_SIZE, Option::LEVEL_BASIC)
         ...
         /* RADOS client option */
         Option("client_notify_timeout", Option::TYPE_INT, Option::LEVEL_DEV).set_default(10).set_description(""),

         /* RADOS client option */
         Option("osd_client_watch_timeout", Option::TYPE_INT, Option::LEVEL_DEV).set_default(30).set_description(""),

         Option("client_caps_release_delay", Option::TYPE_SECS, Option::LEVEL_DEV).set_default(5).set_description(""),
         ...
         Option("client_max_inline_size", Option::TYPE_SIZE, Option::LEVEL_DEV).set_default(4_K).set_description(""),
         ...
         Option("client_dirsize_rbytes", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
             ...
             .set_long_description(
                 "This option enables a CephFS feature that stores the recursive directory size (the bytes used by "
                 "files in the directory and its descendents) in the st_size field of the stat structure."),
         ...
         Option("client_max_retries_on_remount_failure", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
             .set_default(5)
             .set_description("number of consecutive failed remount attempts for invalidating kernel dcache after "
                              "which client would abort."),
         ...
         Option("fuse_max_write", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
             ...
             .set_long_description(
                 "Set the maximum number of bytes in a single write operation that may pass atomically through FUSE. "
                 "The FUSE default is 128kB and may be indicated by setting this option to 0."),
         ...
         Option("client_die_on_failed_dentry_invalidate", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
             ...
             .set_long_description("The CephFS client requires a mechanism to invalidate dentries in the caller (e.g. "
                                   "the kernel for ceph-fuse) when capabilities must be recalled. If the client cannot "
                                   "do this then the MDS cache cannot shrink which can cause the MDS to fail."),
...
std::vector<Option> get_cephfs_mirror_options()
{
    return std::vector<Option>({
        Option("cephfs_mirror_max_concurrent_directory_syncs", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("maximum number of directory snapshots that can be synchronized concurrently by "
        ...
        Option("cephfs_mirror_restart_mirror_on_blocklist_interval", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Interval in seconds to restart blocklisted mirror instances. Setting to zero (0) "
        ...
        Option("cephfs_mirror_max_consecutive_failures_per_directory", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
            ...
            .set_min(0)
            .set_description(
                "consecutive failed directory synchronization attempts before marking a directory as \"failed\"")
            .set_long_description("number of consecutive snapshot synchronization failues to mark a directory as "
        ...
        Option("cephfs_mirror_restart_mirror_on_failure_interval", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Interval in seconds to restart failed mirror instances. Setting to zero (0) "
        ...
        Option("cephfs_mirror_mount_timeout", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
            ...
            .set_long_description("Timeout in seconds for mounting primary or secondary (remote) ceph file system by "
                                  "the cephfs-mirror daemon. Setting this to a higher value could result in the mirror "
                                  "daemon getting stalled when mounting a file system if the cluster is not reachable. "
                                  "This option is used to override the usual client_mount_timeout."),
    });
}
