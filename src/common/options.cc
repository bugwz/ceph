...
void Option::dump_value(const char* field_name, const Option::value_t& v, Formatter* f) const
{
    ...
    switch (type) {
    case TYPE_INT: f->dump_int(field_name, std::get<int64_t>(v)); break;
    case TYPE_UINT: f->dump_unsigned(field_name, std::get<uint64_t>(v)); break;
    case TYPE_STR: f->dump_string(field_name, std::get<std::string>(v)); break;
    case TYPE_FLOAT: f->dump_float(field_name, std::get<double>(v)); break;
    case TYPE_BOOL: f->dump_bool(field_name, std::get<bool>(v)); break;
...
int Option::validate(const Option::value_t& new_value, std::string* err) const
{
    ...
    if (!enum_allowed.empty() && type == Option::TYPE_STR) {
        auto found = std::find(enum_allowed.begin(), enum_allowed.end(), std::get<std::string>(new_value));
