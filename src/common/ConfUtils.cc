...
conf_line_t::conf_line_t(const std::string& key, const std::string& val)
    ...
    , val{boost::algorithm::trim_copy_if(val, [](unsigned char c) { return std::isspace(c); })}
{}
...
conf_section_t::conf_section_t(const std::string& heading, const std::vector<conf_line_t>& lines)
    : heading{heading}
{
    for (auto& line : lines) {
 ...
 */
int ConfFile::parse_file(const std::string& fname, std::ostream* warnings)
{
    clear();
    try {
        if (auto file_size = fs::file_size(fname); file_size > MAX_CONFIG_FILE_SZ) {
            *warnings << __func__ << ": config file '" << fname << "' is " << file_size << " bytes, "
...
template<typename Iterator, typename Skipper> struct IniGrammer : qi::grammar<Iterator, ConfFile(), Skipper>
{
    struct error_handler_t
    {
        std::ostream& os;
        template<typename Iter>
        auto operator()(Iter first, Iter last, Iter where, const boost::spirit::info& what) const
        {
            auto line_start = boost::spirit::get_line_start(first, where);
            os << "parse error: expected '" << what << "' in line " << boost::spirit::get_line(where) << " at position "
        ...
        , report_error{error_handler_t{err}}
    {
        ...
        qi::on_error<qi::fail>(conf_file, report_error(qi::_1, qi::_2, qi::_3, qi::_4));
...
int ConfFile::parse_bufferlist(ceph::bufferlist* bl, std::ostream* warnings)
{
    clear();
    ostringstream oss;
    if (!warnings) {
...
int ConfFile::read(std::string_view section_name, std::string_view key, std::string& val) const
{
...
void ConfFile::check_old_style_section_names(const std::vector<std::string>& prefixes, std::ostream& os)
{
