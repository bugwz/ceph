...
void condition_variable_debug::notify_one()
{
    ...
    ceph_assert(waiter_mutex == nullptr || waiter_mutex->is_locked());
...
void condition_variable_debug::notify_all(bool sloppy)
{
    if (!sloppy) {
        // make sure signaler is holding the waiter's lock.
        ceph_assert(waiter_mutex == NULL || waiter_mutex->is_locked());
    }
...
std::cv_status condition_variable_debug::_wait_until(mutex_debug* mutex, timespec* ts)
{
    // make sure this cond is used with one mutex only
    ...
    waiter_mutex->_post_lock();
    switch (r) {
    case 0: return std::cv_status::no_timeout;
    case ETIMEDOUT: return std::cv_status::timeout;
