...
#define CRC32C3X8_ZERO __asm__("crc32cx %w[c0], %w[c0], xzr" : [c0] "+r"(crc0));
...
#define CRC32C3X8_ZERO crc0 = __crc32cd(crc0, (const uint64_t)0);

#endif /* HAVE_ARMV8_CRC_CRYPTO_INTRINSICS */

#define CRC32C7X3X8(ITR)       \
    do {                       \
        CRC32C3X8((ITR)*7 + 0) \
        CRC32C3X8((ITR)*7 + 1) \
        CRC32C3X8((ITR)*7 + 2) \
...
uint32_t ceph_crc32c_aarch64(uint32_t crc, unsigned char const* buffer, unsigned len)
{
    ...
    if (buffer) {
        ...
        if (ceph_arch_aarch64_pmull) {
            ...
            while ((length -= 1024) >= 0) {
                ...
                __asm__("crc32cx %w[c0], %w[c], %x[v]\n\t"
                        : [c0] "=r"(crc0)
                        : [c] "r"(crc), [v] "r"(*(const uint64_t*)buffer)
                        :);
                crc1 = 0;
                crc2 = 0;
                buffer += sizeof(uint64_t);
        ...
        if (length & sizeof(uint8_t)) CRC32CB(crc, *buffer);
    }
    else {
#ifdef HAVE_ARMV8_CRYPTO
        if (ceph_arch_aarch64_pmull) {
...
#endif /* HAVE_ARMV8_CRYPTO */
        while ((length -= sizeof(uint64_t)) >= 0) CRC32CX(crc, 0);

        /* The following is more efficient than the straight loop */
        if (length & sizeof(uint32_t)) CRC32CW(crc, 0);

        if (length & sizeof(uint16_t)) CRC32CH(crc, 0);

        if (length & sizeof(uint8_t)) CRC32CB(crc, 0);
    }
    return crc;
}
