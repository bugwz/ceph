...
inconsistent_obj_wrapper::inconsistent_obj_wrapper(const hobject_t& hoid)
    : inconsistent_obj_t{librados::object_id_t{hoid.oid.name, hoid.nspace, hoid.get_key(), hoid.snap}}
{}

void inconsistent_obj_wrapper::add_shard(const pg_shard_t& pgs, const shard_info_wrapper& shard)
{
    union_shards.errors |= shard.errors;
    shards.emplace(osd_shard_t{pgs.osd, int8_t(pgs.shard)}, shard);
}

void inconsistent_obj_wrapper::set_auth_missing(const hobject_t& hoid, const map<pg_shard_t, ScrubMap>& maps,
                                                map<pg_shard_t, shard_info_wrapper>& shard_map, int& shallow_errors,
                                                int& deep_errors, const pg_shard_t& primary)
{
...
inconsistent_snapset_wrapper::inconsistent_snapset_wrapper(const hobject_t& hoid)
    : inconsistent_snapset_t{object_id_t{hoid.oid.name, hoid.nspace, hoid.get_key(), hoid.snap}}
{}
