...
public:
    raw_seastar_foreign_ptr(temporary_buffer&& buf)
        : raw(buf.get_write(), buf.size())
        , ptr(std::move(buf))
    {}
    raw* clone_empty() override { return create(len).release(); }
};
...
{
    return ceph::unique_leakable_ptr<buffer::raw>(new raw_seastar_foreign_ptr(std::move(buf)));
}

ceph::unique_leakable_ptr<buffer::raw> create(temporary_buffer&& buf)
{
    return ceph::unique_leakable_ptr<buffer::raw>(new raw_seastar_local_ptr(std::move(buf)));
}
...
public:
    raw_seastar_local_shared_ptr(temporary_buffer& buf)
        ...
        , buf(buf.share())
    {}
    raw* clone_empty() override { return ceph::buffer::create(len).release(); }
};
}   // namespace

buffer::ptr seastar_buffer_iterator::get_ptr(size_t len)
{
    buffer::ptr p{ceph::unique_leakable_ptr<buffer::raw>(new raw_seastar_local_shared_ptr{buf})};
    p.set_length(len);
    return p;
}
