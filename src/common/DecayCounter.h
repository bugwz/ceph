...
public:
    ...
    void set_halflife(double hl) { k = log(.5) / hl; }
    double get_halflife() const { return log(.5) / k; }
...
public:
    ...
    double get_last() const { return val; }

    time get_last_decay() const { return last_decay; }
    ...
    void adjust(double v = 1.0) { decay(v); }

    void scale(double f) { val *= f; }
