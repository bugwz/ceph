...
int get_fs_stats(ceph_data_stats_t& stats, const char* path)
{
    ...
    if (!GetDiskFreeSpaceExA(path, &avail_bytes, &total_bytes, &total_free_bytes)) {
        return -EINVAL;
    }
...
static bool value_set(char* buf, const char* prefix, map<string, string>* pm, const char* key)
{
...
static bool os_release_parse(map<string, string>* m, CephContext* cct)
{
    ...
    static const map<string, string> kvm = {
        {"distro", "ID="}, {"distro_description", "PRETTY_NAME="}, {"distro_version", "VERSION_ID="}};
...
int get_windows_version(POSVERSIONINFOEXW ver)
{
    ...
    auto get_version_func = (get_version_func_t)GetProcAddress(ntdll_lib, "RtlGetVersion");
    int ret = 0;
...
void collect_sys_info(map<string, string>* m, CephContext* cct)
{
    ...
    char version_str[64];
    snprintf(version_str, 64, "%lu.%lu (%lu)", ver.dwMajorVersion, ver.dwMinorVersion, ver.dwBuildNumber);
    ...
    if (get_cgroup_memory_limit(&cgroup_limit) == 0 && cgroup_limit > 0) {
...
string cleanbin(bufferlist& bl, bool& base64, bool show)
{
    ...
    if (show) encoded = "Base64:" + encoded;
    base64 = true;
    return encoded;
}
