...
template<typename Duration> struct timeval to_timeval(Duration d)
{
    struct timeval tv;
...
public:
    ...
    static void to_timeval(const time_point& t, struct timeval& tv)
    {
        ...
        tv.tv_usec =
            std::chrono::duration_cast<std::chrono::microseconds>(t.time_since_epoch() % std::chrono::seconds(1))
                .count();
    }
    ...
    {
        return time_point(std::chrono::duration_cast<duration>(std::chrono::duration<double>(d)));
    }
};
...
inline auto difference(typename std::chrono::time_point<Clock, Duration1> minuend,
                       typename std::chrono::time_point<Clock, Duration2> subtrahend) ->
    typename std::common_type<
        std::chrono::duration<typename std::make_signed<typename Duration1::rep>::type, typename Duration1::period>,
        std::chrono::duration<typename std::make_signed<typename Duration2::rep>::type,
...
{
    return std::chrono::duration_cast<timespan>(std::chrono::duration<double>(d));
}
...
inline signedspan operator-(real_time minuend, real_time subtrahend)
{
    return time_detail::difference(minuend, subtrahend);
}

inline signedspan operator-(coarse_real_time minuend, coarse_real_time subtrahend)
{
    return time_detail::difference(minuend, subtrahend);
}

inline signedspan operator-(mono_time minuend, mono_time subtrahend)
{
    return time_detail::difference(minuend, subtrahend);
}

inline signedspan operator-(coarse_mono_time minuend, coarse_mono_time subtrahend)
{
    return time_detail::difference(minuend, subtrahend);
}
...
{
    return z > signedspan::zero() ? std::chrono::duration_cast<timespan>(z) : timespan(-z.count());
}
...
template<typename Clock, typename = std::void_t<>> struct converts_to_timespec : std::false_type
{};

template<typename Clock>
struct converts_to_timespec<
    Clock, std::void_t<decltype(Clock::from_timespec(Clock::to_timespec(std::declval<typename Clock::time_point>())))>>
    : std::true_type
{};

template<typename Clock> constexpr bool converts_to_timespec_v = converts_to_timespec<Clock>::value;

template<typename Rep, typename T> static Rep to_seconds(T t)
{
    return std::chrono::duration_cast<std::chrono::duration<Rep>>(t).count();
}

template<typename Rep, typename T> static Rep to_microseconds(T t)
{
...
namespace std {
template<typename Rep, typename Period> ostream& operator<<(ostream& m, const chrono::duration<Rep, Period>& t);
}

#if FMT_VERSION >= 90000
template<typename Clock> struct fmt::formatter<std::chrono::time_point<Clock>> : fmt::ostream_formatter
{};
#endif
