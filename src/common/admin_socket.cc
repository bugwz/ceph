...
template<typename F, typename... Args> inline int retry_sys_call(F f, Args... args)
{
    int r;
    do {
        r = f(args...);
...
static void remove_cleanup_file(std::string_view file)
{
    ...
    if (auto i = std::find(cleanup_files.cbegin(), cleanup_files.cend(), file); i != cleanup_files.cend()) {
...
std::string AdminSocket::bind_and_listen(const std::string& sock_path, int* fd)
{
    ...
    if (sock_path.size() > sizeof(address.sun_path) - 1) {
        ...
        oss << "AdminSocket::bind_and_listen: "
            ...
            << "maximum length on this system is " << (sizeof(address.sun_path) - 1);
        return oss.str();
    }
    ...
    snprintf(address.sun_path, sizeof(address.sun_path), "%s", sock_path.c_str());
    if (::bind(sock_fd, (struct sockaddr*)&address, sizeof(struct sockaddr_un)) != 0) {
        ...
        if (err != 0) {
            ostringstream oss;
            oss << "AdminSocket::bind_and_listen: "
                << "failed to bind the UNIX domain socket to '" << sock_path << "': " << cpp_strerror(err);
            close(sock_fd);
            return oss.str();
        }
    }
...
void AdminSocket::chown(uid_t uid, gid_t gid)
{
    if (m_sock_fd >= 0) {
        ...
        if (r < 0) {
            r = -errno;
            lderr(m_cct) << "AdminSocket: failed to chown socket: " << cpp_strerror(r) << dendl;
        }
    }
}
...
void AdminSocket::chmod(mode_t mode)
{
    if (m_sock_fd >= 0) {
        ...
        if (r < 0) {
            r = -errno;
            lderr(m_cct) << "AdminSocket: failed to chmod socket: " << cpp_strerror(r) << dendl;
        }
    }
}
...
void AdminSocket::do_accept()
{
    ...
    int connection_fd = accept_cloexec(m_sock_fd, (struct sockaddr*)&address, &address_length);
    if (connection_fd < 0) {
        int err = ceph_sock_errno();
        lderr(m_cct) << "AdminSocket: do_accept error: '" << cpp_strerror(err) << dendl;
        return;
    }
    ...
    while (1) {
        ...
        if (ret <= 0) {
            if (ret < 0) {
                lderr(m_cct) << "AdminSocket: error reading request code: " << cpp_strerror(ret) << dendl;
            }
        ...
        if (cmd[0] == '\0') {
            ...
            if (pos == 3 && cmd[0] == '\0') {
                switch (cmd[3]) {
                case 0: c = "0"; break;
                case 1: c = "perfcounters_dump"; break;
                case 2: c = "perfcounters_schema"; break;
                default: c = "foo"; break;
                }
    ...
    if (ret < 0) {
        lderr(m_cct) << "AdminSocket: error writing response length " << cpp_strerror(ret) << dendl;
    }
    else {
        int r = out.send_fd(connection_fd);
        if (r < 0) {
            lderr(m_cct) << "AdminSocket: error writing response payload " << cpp_strerror(ret) << dendl;
        }
    }
    retry_sys_call(::compat_closesocket, connection_fd);
}
...
int AdminSocket::execute_command(const std::vector<std::string>& cmd, const bufferlist& inbl, std::ostream& errss,
                                 bufferlist* outbl)
{
#ifdef WITH_SEASTAR
    ...
    execute_command(cmd, inbl, [&errss, outbl, &fin](int r, const std::string& err, bufferlist& out) {
        errss << err;
        *outbl = std::move(out);
        fin.finish(r);
    });
...
void AdminSocket::execute_command(const std::vector<std::string>& cmdvec, const bufferlist& inbl,
                                  std::function<void(int, const std::string&, bufferlist&)> on_finish)
{
    ...
    try {
        ...
        return on_finish(-EINVAL, "invalid json, missing format and/or prefix", empty);
    }
    ...
    switch (retval) {
    case ENOENT:
        lderr(m_cct) << "AdminSocket: request '" << cmdvec << "' not defined" << dendl;
        delete f;
        return on_finish(-EINVAL, "unknown command prefix "s + prefix, empty);
    case EINVAL: delete f; return on_finish(-EINVAL, "invalid command json", empty);
    default: assert(retval == 0);
    }

    hook->call_async(prefix, cmdmap, f, inbl, [f, on_finish](int r, const std::string& err, bufferlist& out) {
...
std::pair<int, AdminSocketHook*> AdminSocket::find_matched_hook(std::string& prefix, const cmdmap_t& cmdmap)
{
    std::unique_lock l(lock);
...
int AdminSocket::register_command(std::string_view cmddesc, AdminSocketHook* hook, std::string_view help)
{
    int ret;
    std::unique_lock l(lock);
    ...
    if (i != hooks.cend() && i->second.desc == cmddesc) {
        ldout(m_cct, 5) << "register_command " << prefix << " cmddesc " << cmddesc << " hook " << hook << " EEXIST"
                        << dendl;
        ret = -EEXIST;
    }
    else {
        ...
        hooks.emplace_hint(
            i, std::piecewise_construct, std::forward_as_tuple(prefix), std::forward_as_tuple(hook, cmddesc, help));
        ret = 0;
    }
    return ret;
}
...
public:
    explicit HelpHook(AdminSocket* as)
        : m_as(as)
    {}
    int call(std::string_view command, const cmdmap_t& cmdmap, const bufferlist&, Formatter* f, std::ostream& errss,
             bufferlist& out) override
    {
        ...
        for (const auto& [command, info] : m_as->hooks) {
            if (info.help.length()) f->dump_string(command.c_str(), info.help);
        }
        f->close_section();
        return 0;
    }
...
public:
    explicit GetdescsHook(AdminSocket* as)
        : m_as(as)
    {}
    int call(std::string_view command, const cmdmap_t& cmdmap, const bufferlist&, Formatter* f, std::ostream& errss,
             bufferlist& out) override
    {
        ...
        for (const auto& [command, info] : m_as->hooks) {
            ...
            dump_cmd_and_help_to_json(f, CEPH_FEATURES_ALL, secname.str().c_str(), info.desc, info.help);
            cmdnum++;
        }
...
bool AdminSocket::init(const std::string& path)
{
    ...
    if (std::tie(ver.dwMajorVersion, ver.dwMinorVersion, ver.dwBuildNumber) < std::make_tuple(10, 0, 17063)) {
    ...
    register_command("git_version", version_hook.get(), "get git sha1");
    help_hook = std::make_unique<HelpHook>(this);
    register_command("help", help_hook.get(), "list available commands");
    getdescs_hook = std::make_unique<GetdescsHook>(this);
    register_command("get_command_descriptions", getdescs_hook.get(), "list available commands");
