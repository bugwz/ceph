...
int get_json_str_map(const string& str, ostream& ss, map<string, string>* str_map, bool fallback_to_plain)
{
...
{
    return boost::algorithm::trim_copy_if(str, [](unsigned char c) { return std::isspace(c); });
}

int get_str_map(const string& str, map<string, string>* str_map, const char* delims)
{
    list<string> pairs;
...
string get_str_map_value(const map<string, string>& str_map, const string& key, const string* def_val)
{
...
string get_str_map_key(const map<string, string>& str_map, const string& key, const string* fallback_key)
{
    map<string, string>::const_iterator p = str_map.find(key);
...
// original "m = { 'VALUE' : '' }".
int get_conf_str_map_helper(const string& str, ostringstream& oss, map<string, string>* m, const string& def_key)
{
