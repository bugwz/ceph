...
int get_json_str_map(const string& str, ostream& ss, str_map_t* str_map, bool fallback_to_plain)
{
    ...
    try {
        ...
        if (json.type() != json_spirit::obj_type) {
            ss << str << " must be a JSON object but is of type " << json.type() << " instead";
            return -EINVAL;
        }
...
{
    for_each_pair(str, delims, [str_map](std::string_view key, std::string_view val) {
...
string get_str_map_value(const str_map_t& str_map, const string& key, const string* def_val)
{
    ...
    if (def_val != nullptr) return *def_val;

    // key DNE in str_map, no def_val was specified
    return string();
}

string get_str_map_key(const str_map_t& str_map, const string& key, const string* fallback_key)
{
    auto p = str_map.find(key);
    if (p != str_map.end()) return p->second;
...
// original "m = { 'VALUE' : '' }".
int get_conf_str_map_helper(const string& str, ostringstream& oss, str_map_t* str_map, const string& default_key)
{
    get_str_map(str, str_map);
...
std::string get_value_via_strmap(const string& conf_string, std::string_view default_key)
{
...
std::string get_value_via_strmap(const string& conf_string, const string& key, std::string_view default_key)
{
    auto mp = get_str_map(conf_string);
    if (mp.size() != 1) {
        return std::string{};
