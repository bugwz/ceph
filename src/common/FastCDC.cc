...
void FastCDC::calc_chunks(const bufferlist& bl, std::vector<std::pair<uint64_t, uint64_t>>* chunks) const
{
    if (bl.length() == 0) {
        return;
    }
    ...
    while (pos < len) {
        ...
        if (
            ...
            _scan(&p, &pp, &pe, pos, std::min(len, cstart + (1 << max_bits)), fp, large_mask, table))
            ;
