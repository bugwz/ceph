...
Formatter* Formatter::create(std::string_view type, std::string_view default_type, std::string_view fallback)
{
...
void JSONFormatter::print_comma(json_formatter_stack_entry_d& entry)
{
    if (entry.size) {
        if (m_pretty) {
            m_ss << ",\n";
            for (unsigned i = 1; i < m_stack.size(); i++) m_ss << "    ";
        }
        else {
            m_ss << ",";
        }
    ...
    if (m_pretty && entry.is_array) m_ss << "    ";
}
...
void JSONFormatter::close_section()
{
    ...
    m_ss << (entry.is_array ? ']' : '}');
    m_stack.pop_back();
    if (m_pretty && m_stack.empty()) m_ss << "\n";
}
...
template<class T> void JSONFormatter::add_value(std::string_view name, T val)
{
    std::stringstream ss;
...
const char* XMLFormatter::XML_1_DTD = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
...
void XMLFormatter::close_section()
{
    ...
    std::transform(
        section.begin(), section.end(), section.begin(), [this](char c) { return this->to_lower_underscore(c); });
    m_sections.pop_back();
    print_spaces();
    m_ss << "</" << section << ">";
    if (m_pretty) m_ss << "\n";
...
template<class T> void XMLFormatter::add_value(std::string_view name, T val)
{
    std::string e(name);
    std::transform(e.begin(), e.end(), e.begin(), [this](char c) { return this->to_lower_underscore(c); });
...
void XMLFormatter::finish_pending_string()
{
    if (!m_pending_string_name.empty()) {
        m_ss << xml_stream_escaper(m_pending_string.str()) << "</" << m_pending_string_name << ">";
        m_pending_string_name.clear();
...
void TableFormatter::flush(std::ostream& os)
{
    ...
    for (size_t i = 0; i < m_vec.size(); i++) {
        for (size_t j = 0; j < m_vec[i].size(); j++) {
            ...
            if (m_vec[i][j].second.length() > column_size[j]) column_size[j] = m_vec[i][j].second.length();
            if (m_vec[i][j].first.length() > column_size[j]) column_size[j] = m_vec[i][j].first.length();
        }
    }
        ...
        if (i == 0) {
            if (need_header_set.count(i)) {
                ...
                if (!m_keyval) {
                    os << "+";
                    for (size_t j = 0; j < m_vec[i].size(); j++) {
                        for (size_t v = 0; v < m_column_size[j] + 3; v++) os << "-";
                        os << "+";
                    }
                    os << "\n";
                    os << "|";
...
template<class T> void TableFormatter::add_value(std::string_view name, T val)
{
    finish_pending_string();
...
void TableFormatter::dump_format_va(std::string_view name, const char* ns, bool quoted, const char* fmt, va_list ap)
{
    finish_pending_string();
    char buf[LARGE_SIZE];
    vsnprintf(buf, LARGE_SIZE, fmt, ap);
