...
template<class T, int NUM_BUCKETS = 128> class LRUSet
{
    /// internal node
    struct Node : boost::intrusive::unordered_set_base_hook<>
    {
        // actual payload
        T value;
        ...
        friend std::size_t hash_value(const Node& node) { return std::hash<T>{}(node.value); }
    ...
    boost::intrusive::list<Node,
                           boost::intrusive::member_hook<Node, boost::intrusive::list_member_hook<>, &Node::lru_item> >
        lru;
...
public:
    LRUSet()
        : set(typename boost::intrusive::unordered_set<Node>::bucket_traits(base_buckets, NUM_BUCKETS))
    {}
    ~LRUSet() { clear(); }
    ...
    bool contains(const T& item) const { return set.count(item) > 0; }
