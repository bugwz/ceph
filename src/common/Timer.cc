...
template<class Mutex> class CommonSafeTimerThread : public Thread
{
...
template<class Mutex> CommonSafeTimer<Mutex>::~CommonSafeTimer()
{
    ceph_assert(thread == NULL);
}

template<class Mutex> void CommonSafeTimer<Mutex>::init()
{
...
template<class Mutex> void CommonSafeTimer<Mutex>::shutdown()
{
...
template<class Mutex> void CommonSafeTimer<Mutex>::timer_thread()
{
    ...
    while (!stopping) {
        ...
        if (!safe_callbacks && stopping) break;
...
template<class Mutex> Context* CommonSafeTimer<Mutex>::add_event_after(double seconds, Context* callback)
{
    return add_event_after(ceph::make_timespan(seconds), callback);
}

template<class Mutex> Context* CommonSafeTimer<Mutex>::add_event_after(ceph::timespan duration, Context* callback)
{
    ceph_assert(ceph_mutex_is_locked(lock));
...
Context* CommonSafeTimer<Mutex>::add_event_at(CommonSafeTimer<Mutex>::clock_t::time_point when, Context* callback)
{
     ...
     * adjust our timeout. */
    if (i == schedule.begin()) cond.notify_all();
    return callback;
}
...
Context* CommonSafeTimer<Mutex>::add_event_at(ceph::real_clock::time_point when, Context* callback)
{
    ...
    const auto mono_atime = (mono_now + std::chrono::ceil<clock_t::duration>(delta));
    return add_event_at(mono_atime, callback);
}

template<class Mutex> bool CommonSafeTimer<Mutex>::cancel_event(Context* callback)
{
    ceph_assert(ceph_mutex_is_locked(lock));
...
template<class Mutex> void CommonSafeTimer<Mutex>::cancel_all_events()
{
...
template<class Mutex> void CommonSafeTimer<Mutex>::dump(const char* caller) const
{
    if (!caller) caller = "";
    ldout(cct, 10) << "dump " << caller << dendl;

    for (scheduled_map_t::const_iterator s = schedule.begin(); s != schedule.end(); ++s)
