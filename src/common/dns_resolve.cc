...
int ResolvHWrapper::res_nquery(res_state s, const char* hostname, int cls, int type, u_char* buf, int bufsz)
{
    return ::res_nquery(s, hostname, cls, type, buf, bufsz);
}

int ResolvHWrapper::res_nsearch(res_state s, const char* hostname, int cls, int type, u_char* buf, int bufsz)
{
...
int ResolvHWrapper::res_query(const char* hostname, int cls, int type, u_char* buf, int bufsz)
{
    return ::res_query(hostname, cls, type, buf, bufsz);
}

int ResolvHWrapper::res_search(const char* hostname, int cls, int type, u_char* buf, int bufsz)
{
...
int DNSResolver::resolve_cname(CephContext* cct, const string& hostname, string* cname, bool* found)
{
    *found = false;
    ...
    auto put_state = make_scope_guard([res, this] { this->put_state(res); });
#endif
...
#define MAX_FQDN_SIZE 255
    ...
    if (type != ns_t_cname) {
        lderr(cct) << "ERROR: failed response type: type=" << type << " (was expecting " << ns_t_cname << ")" << dendl;
        return -EIO;
    }
...
int DNSResolver::resolve_ip_addr(CephContext* cct, const string& hostname, entity_addr_t* addr)
{
...
int DNSResolver::resolve_ip_addr(CephContext* cct, res_state* res, const string& hostname, entity_addr_t* addr)
{
    ...
    if (!addr->parse(addr_buf)) {
        lderr(cct) << "failed to parse address '" << (const char*)ns_rr_rdata(rr) << "'" << dendl;
        return -1;
    }
...
int DNSResolver::resolve_srv_hosts(CephContext* cct, const string& service_name, const SRV_Protocol trans_protocol,
...
int DNSResolver::resolve_srv_hosts(CephContext* cct, const string& service_name, const SRV_Protocol trans_protocol,
                                   const string& domain, map<string, DNSResolver::Record>* srv_hosts)
{
    ...
    string query_str = "_" + service_name + "._" + proto_str + (domain.empty() ? "" : "." + domain);
    int len;

#ifdef HAVE_RES_NQUERY
    len = resolv_h->res_nsearch(res, query_str.c_str(), ns_c_in, ns_t_srv, nsbuf, sizeof(nsbuf));
#else
    {
#ifndef HAVE_THREAD_SAFE_RES_QUERY
        std::lock_guard l(lock);
#endif
        len = resolv_h->res_search(query_str.c_str(), ns_c_in, ns_t_srv, nsbuf, sizeof(nsbuf));
    }
#endif
    if (len < 0) {
    ...
    for (int i = 0; i < num_hosts; i++) {
        ...
        string srv_domain = full_srv_name.substr(full_srv_name.find(protocol) + protocol.length());

        auto rdata = ns_rr_rdata(rr);
        uint16_t priority = ns_get16(rdata);
        rdata += NS_INT16SZ;
        uint16_t weight = ns_get16(rdata);
        rdata += NS_INT16SZ;
        uint16_t port = ns_get16(rdata);
        rdata += NS_INT16SZ;
        // FIPS zeroization audit 20191115: this memset is not security related.
        memset(full_target, 0, sizeof(full_target));
        ns_name_uncompress(ns_msg_base(handle), ns_msg_end(handle), rdata, full_target, sizeof(full_target));
        ...
        if (r == 0) {
            ...
            if (end == target.npos) {
                lderr(cct) << "resolved target not in search domain: " << target << " / " << srv_domain << dendl;
                return -EINVAL;
            }
