...
template<uint8_t _bit_count> class BitVector
{
private:
    ...
    template<typename DataIterator> class ReferenceImpl
    {
    protected:
        ...
        ReferenceImpl(const DataIterator& data_iterator, uint64_t shift)
            : m_data_iterator(data_iterator)
            , m_shift(shift)
        {}
        ReferenceImpl(DataIterator&& data_iterator, uint64_t shift)
            : m_data_iterator(std::move(data_iterator))
            , m_shift(shift)
        {}

    public:
        inline operator uint8_t() const { return (*m_data_iterator >> m_shift) & MASK; }
    };
...
public:
    ...
    private:
        ...
        ConstReference(const bufferlist::const_iterator& data_iterator, uint64_t shift)
        ...
        ConstReference(bufferlist::const_iterator&& data_iterator, uint64_t shift)
            : ReferenceImpl<bufferlist::const_iterator>(std::move(data_iterator), shift)
        {}
    };
...
public:
    template<typename BitVectorT, typename DataIterator> class IteratorImpl
    {
    private:
        friend class BitVector;
    ...
    public:
        ...
        inline ConstReference operator*() const { return ConstReference(m_data_iterator, m_shift); }
        inline Reference operator*() { return Reference(m_data_iterator, m_shift); }
    };

    typedef IteratorImpl<const BitVector, bufferlist::const_iterator> ConstIterator;
    ...
    inline ConstIterator end() const { return ConstIterator(this, m_size); }
    inline Iterator begin() { return Iterator(this, 0); }
    inline Iterator end() { return Iterator(this, m_size); }

    void set_crc_enabled(bool enabled) { m_crc_enabled = enabled; }
    void clear();
    ...
    void encode_data(bufferlist& bl, uint64_t data_byte_offset, uint64_t byte_length) const;
    void decode_data(bufferlist::const_iterator& it, uint64_t data_byte_offset);
    void get_data_extents(uint64_t offset, uint64_t length, uint64_t* data_byte_offset, uint64_t* object_byte_offset,
    ...
    void get_header_crc_extents(uint64_t* byte_offset, uint64_t* byte_length) const;

    void encode_data_crcs(bufferlist& bl, uint64_t offset, uint64_t length) const;
    void decode_data_crcs(bufferlist::const_iterator& it, uint64_t offset);
    void get_data_crcs_extents(uint64_t offset, uint64_t length, uint64_t* byte_offset, uint64_t* byte_length) const;
...
template<uint8_t _b> const uint32_t BitVector<_b>::BLOCK_SIZE = 4096;

template<uint8_t _b>
BitVector<_b>::BitVector()
    : m_size(0)
    , m_crc_enabled(true)
    , m_header_crc(0)
{}

template<uint8_t _b> void BitVector<_b>::clear()
{
    m_data.clear();
    m_data_crcs.clear();
    m_size = 0;
...
template<uint8_t _b> void BitVector<_b>::resize(uint64_t size)
{
    resize(size, true);
}

template<uint8_t _b> void BitVector<_b>::resize(uint64_t size, bool zero)
{
...
template<uint8_t _b> uint64_t BitVector<_b>::size() const
{
    return m_size;
}

template<uint8_t _b> const bufferlist& BitVector<_b>::get_data() const
{
    return m_data;
}

template<uint8_t _b> void BitVector<_b>::compute_index(uint64_t offset, uint64_t* index, uint64_t* shift)
{
    *index = offset / ELEMENTS_PER_BLOCK;
    *shift = ((ELEMENTS_PER_BLOCK - 1) - (offset % ELEMENTS_PER_BLOCK)) * _b;
}

template<uint8_t _b> void BitVector<_b>::encode_header(bufferlist& bl) const
{
...
template<uint8_t _b> void BitVector<_b>::decode_header(bufferlist::const_iterator& it)
{
    using ceph::decode;
    bufferlist header_bl;
    decode(header_bl, it);
...
template<uint8_t _b> uint64_t BitVector<_b>::get_header_length() const
{
...
template<uint8_t _b>
void BitVector<_b>::encode_data(bufferlist& bl, uint64_t data_byte_offset, uint64_t byte_length) const
{
    ceph_assert(data_byte_offset % BLOCK_SIZE == 0);
    ceph_assert(data_byte_offset + byte_length == m_data.length() || byte_length % BLOCK_SIZE == 0);
...
template<uint8_t _b> void BitVector<_b>::decode_data(bufferlist::const_iterator& it, uint64_t data_byte_offset)
{
    ceph_assert(data_byte_offset % BLOCK_SIZE == 0);
    if (it.end()) {
        return;
    ...
    while (data_byte_offset < end_offset) {
        ...
        bufferlist bit;
        bit.append(ptr);
        if (m_crc_enabled && m_data_crcs[data_byte_offset / BLOCK_SIZE].val != bit.crc32c(0)) {
...
template<uint8_t _b>
void BitVector<_b>::get_data_extents(uint64_t offset, uint64_t length, uint64_t* data_byte_offset,
                                     uint64_t* object_byte_offset, uint64_t* byte_length) const
{
...
template<uint8_t _b> void BitVector<_b>::encode_footer(bufferlist& bl) const
{
    using ceph::encode;
...
template<uint8_t _b> void BitVector<_b>::decode_footer(bufferlist::const_iterator& it)
{
    using ceph::decode;
    bufferlist footer_bl;
    decode(footer_bl, it);
...
template<uint8_t _b> uint64_t BitVector<_b>::get_footer_offset() const
{
    return get_header_length() + m_data.length();
}

template<uint8_t _b> void BitVector<_b>::decode_header_crc(bufferlist::const_iterator& it)
{
...
template<uint8_t _b> void BitVector<_b>::get_header_crc_extents(uint64_t* byte_offset, uint64_t* byte_length) const
{
...
template<uint8_t _b> void BitVector<_b>::encode_data_crcs(bufferlist& bl, uint64_t offset, uint64_t length) const
{
    if (length == 0) {
        return;
    }
...
template<uint8_t _b> void BitVector<_b>::decode_data_crcs(bufferlist::const_iterator& it, uint64_t offset)
{
    if (it.end()) {
        return;
    }
...
template<uint8_t _b>
void BitVector<_b>::get_data_crcs_extents(uint64_t offset, uint64_t length, uint64_t* byte_offset,
                                          uint64_t* byte_length) const
{
    ...
    *byte_offset += sizeof(__u32);
    ...
    uint64_t shift;
    compute_index(offset, &index, &shift);
    uint64_t start_byte_offset = *byte_offset + ((index / BLOCK_SIZE) * sizeof(__u32));

    compute_index(offset + length, &index, &shift);
    uint64_t end_byte_offset = *byte_offset + (((index / BLOCK_SIZE) + 1) * sizeof(__u32));
...
template<uint8_t _b> void BitVector<_b>::encode(bufferlist& bl) const
{
    encode_header(bl);
...
template<uint8_t _b> void BitVector<_b>::decode(bufferlist::const_iterator& it)
{
    decode_header(it);
...
template<uint8_t _b> void BitVector<_b>::dump(Formatter* f) const
{
...
template<uint8_t _b> bool BitVector<_b>::operator==(const BitVector& b) const
{
    return (this->m_size == b.m_size && this->m_data == b.m_data);
}

template<uint8_t _b> typename BitVector<_b>::Reference BitVector<_b>::operator[](uint64_t offset)
{
    uint64_t index;
    uint64_t shift;
    compute_index(offset, &index, &shift);
...
template<uint8_t _b> typename BitVector<_b>::ConstReference BitVector<_b>::operator[](uint64_t offset) const
{
    uint64_t index;
    uint64_t shift;
    compute_index(offset, &index, &shift);
...
template<uint8_t _b> typename BitVector<_b>::Reference& BitVector<_b>::Reference::operator=(uint8_t v)
{
    uint8_t mask = MASK << this->m_shift;
    char packed_value = (*this->m_data_iterator & ~mask) | ((v << this->m_shift) & mask);
    bufferlist::iterator it(this->m_data_iterator);
    it.copy_in(1, &packed_value, true);
    return *this;
}

template<uint8_t _b> void BitVector<_b>::generate_test_instances(std::list<BitVector*>& o)
{
...
template<uint8_t _b> inline std::ostream& operator<<(std::ostream& out, const ceph::BitVector<_b>& b)
{
    out << "ceph::BitVector<" << _b << ">(size=" << b.size() << ", data=" << b.get_data() << ")";
    return out;
}
}   // namespace ceph
