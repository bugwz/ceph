...
template<typename T> void HTMLFormatter::dump_template(std::string_view name, T arg)
{
    print_spaces();
    m_ss << "<li>" << name << ": " << arg << "</li>";
    if (m_pretty) m_ss << "\n";
...
void HTMLFormatter::dump_format_va(std::string_view name, const char* ns, bool quoted, const char* fmt, va_list ap)
{
    ...
    print_spaces();
    if (ns) {
        m_ss << "<li xmlns=\"" << ns << "\">" << e << ": " << xml_stream_escaper(std::string_view(buf, len)) << "</li>";
    }
    else {
        m_ss << "<li>" << e << ": " << xml_stream_escaper(std::string_view(buf, len)) << "</li>";
    }
