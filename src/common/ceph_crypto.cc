...
static auto ssl_mutexes = ceph::make_lock_container<ceph::shared_mutex>(
    ...
    [](const size_t i) { return ceph::make_shared_mutex(std::string("ssl-mutex-") + std::to_string(i)); });
...
static void ssl_locking_callback(const int mode, const int mutex_num, [[maybe_unused]] const char* file,
...
static unsigned long ssl_get_thread_id(void)
{
...
{
    if (flags == EVP_MD_CTX_FLAG_NON_FIPS_ALLOW && OpenSSL_version_num() >= 0x30000000L && mpType == EVP_md5() &&
        !mpType_FIPS) {
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
