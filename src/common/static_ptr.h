...
template<typename T> static std::size_t op_fun(op oper, void* p1, void* p2)
{
    auto me = static_cast<T*>(p1);

    switch (oper) {
    case op::move: new (p2) T(std::move(*me)); break;

    case op::destroy: me->~T(); break;

    case op::size: return sizeof(T);
    }
    return 0;
}
}   // namespace _mem
...
//
template<typename Base, std::size_t Size = sizeof(Base)> class static_ptr
{
    ...
    //
    template<typename T, std::size_t S> constexpr static int create_ward() noexcept
    {
        static_assert(std::is_void_v<Base> || std::is_base_of_v<Base, std::decay_t<T>>,
                      "Value to store must be a derivative of the base.");
        static_assert(S <= Size, "Value too large.");
        static_assert(std::is_void_v<Base> || !std::is_const<Base>{} || std::is_const_v<T>,
...
public:
    ...
    static_ptr() noexcept
        : operate(nullptr)
    {}
    static_ptr(std::nullptr_t) noexcept
        : operate(nullptr)
    {}
    ...
    static_ptr(static_ptr&& rhs) noexcept(std::is_nothrow_move_constructible_v<Base>)
        : operate(rhs.operate)
    {
        if (operate) {
    ...
    static_ptr(static_ptr<U, S>&& rhs) noexcept(std::is_nothrow_move_constructible_v<U>)
        : operate(rhs.operate)
    {
        create_ward<U, S>();
        if (operate) {
    ...
    static_ptr& operator=(static_ptr&& rhs) noexcept(std::is_nothrow_move_constructible_v<Base>)
    {
        reset();
        if (rhs) {
            operate = rhs.operate;
    ...
    template<typename U, std::size_t S>
    static_ptr& operator=(static_ptr<U, S>&& rhs) noexcept(std::is_nothrow_move_constructible_v<U>)
    {
        create_ward<U, S>();
        reset();
        if (rhs) {
    ...
    bool operator==(std::nullptr_t) const { return !operate; }
    ...
    static_ptr(std::in_place_type_t<T>, Args&&... args) noexcept(std::is_nothrow_constructible_v<T, Args...>)
        : operate(&_mem::op_fun<T>)
    {
        static_assert((!std::is_nothrow_copy_constructible_v<Base> || std::is_nothrow_copy_constructible_v<T>)&&(
                          !std::is_nothrow_move_constructible_v<Base> || std::is_nothrow_move_constructible_v<T>),
    ...
    // Access!
    Base* get() const noexcept { return operate ? reinterpret_cast<Base*>(&buf) : nullptr; }
    template<typename U = Base> std::enable_if_t<!std::is_void_v<U>, Base*> operator->() const noexcept
    {
        return get();
    }
    template<typename U = Base> std::enable_if_t<!std::is_void_v<U>, Base&> operator*() const noexcept
    ...
    operator bool() const noexcept { return !!operate; }
...
{
    static_assert(Z >= S, "Value too large.");
    static_ptr<U, Z> r;
...
template<typename U, std::size_t Z, typename T, std::size_t S> static_ptr<U, Z> const_pointer_cast(static_ptr<T, S>&& p)
{
    static_assert(Z >= S, "Value too large.");
    static_ptr<U, Z> r;
...
{
    static_assert(std::is_same_v<U, T>, "resize_pointer_cast only changes size, not type.");
    static_ptr<U, Z> r;
...
// lead.
//
template<typename Base, typename Derived = Base, std::size_t Size = sizeof(Derived), typename... Args>
