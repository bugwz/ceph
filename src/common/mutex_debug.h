...
protected:
    ...
    bool _enable_lockdep() const { return lockdep && g_lockdep; }
    void _register();
...
template<bool Recursive> class mutex_debug_impl : public mutex_debugging_base
{
private:
    pthread_mutex_t m;
...
public:
    ...
    bool try_lock_impl()
    {
        ...
        default: throw std::system_error(r, std::generic_category());
        }
    }
    ...
    void _post_lock()
    {
        if (!recursive) ceph_assert(nlock == 0);
    ...
    void _pre_unlock()
    {
        ...
        if (nlock == 1) locked_by = std::thread::id();
    ...
    bool try_lock(bool no_lockdep = false)
    {
        ...
        if (locked) {
            if (enable_lockdep(no_lockdep)) _locked();
            _post_lock();
        }
        return locked;
    }
    ...
    {
        if (enable_lockdep(no_lockdep)) _will_lock(recursive);

        if (try_lock(no_lockdep)) return;
    ...
    {
        _pre_unlock();
        if (enable_lockdep(no_lockdep)) _will_unlock();
        unlock_impl();
    }
};
