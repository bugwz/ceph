...
void dump_cmd_to_json(Formatter* f, uint64_t features, const string& cmd)
{
...
void dump_cmd_and_help_to_json(Formatter* jf, uint64_t features, const string& secname, const string& cmdsig,
...
void dump_cmddesc_to_json(Formatter* jf, uint64_t features, const string& secname, const string& cmdsig,
                          const string& helptext, const string& module, const string& perm, uint64_t flags)
{
    jf->open_object_section(secname);
    jf->open_array_section("sig");
    dump_cmd_to_json(jf, features, cmdsig);
...
void cmdmap_dump(const cmdmap_t& cmdmap, Formatter* f)
{
    ...
    public:
        ...
        void operator()(const std::string& operand) const { f->dump_string(key, operand); }

        void operator()(const bool& operand) const { f->dump_bool(key, operand); }

        void operator()(const int64_t& operand) const { f->dump_int(key, operand); }

        void operator()(const double& operand) const { f->dump_float(key, operand); }
...
bool cmdmap_from_json(const vector<string>& cmd, cmdmap_t* mapp, std::ostream& ss)
{
    json_spirit::mValue v;

    string fullcmd;
    // First, join all cmd strings
    for (auto& c : cmd) fullcmd += c;

    try {
        if (!json_spirit::read(fullcmd, v)) throw std::runtime_error("unparseable JSON " + fullcmd);
        if (v.type() != json_spirit::obj_type) throw std::runtime_error("not JSON object " + fullcmd);
        ...
        for (auto it = o.begin(); it != o.end(); ++it) {
            ...
            default: throw std::runtime_error("JSON array/object not allowed " + fullcmd); break;
            ...
            case json_spirit::str_type: (*mapp)[it->first] = it->second.get_str(); break;

            case json_spirit::bool_type: (*mapp)[it->first] = it->second.get_bool(); break;

            case json_spirit::int_type: (*mapp)[it->first] = it->second.get_int64(); break;

            case json_spirit::real_type: (*mapp)[it->first] = it->second.get_real(); break;
            }
        }
        return true;
    }
...
{
public:
    template<typename T> string operator()(T& operand) const
    {
        ostringstream oss;
        oss << operand;
        return oss.str();
...
string cmd_vartype_stringify(const cmd_vartype& v)
{
    return boost::apply_visitor(stringify_visitor(), v);
}


void handle_bad_get(CephContext* cct, const string& k, const char* tname)
{
    ostringstream errstr;
    int status;
    const char* typestr = abi::__cxa_demangle(tname, 0, 0, &status);
    if (status != 0) typestr = tname;
...
{
    if (*s == '-' || *s == '+') {
        if (pss) *pss << "expected numerical value, got: " << s;
        return -EINVAL;
    }
    ...
    if (r < 0) {
        if (pss) *pss << "unable to parse positive integer '" << s << "'";
        return -1;
    }
    return r;
}
...
namespace {
template<typename Func> bool find_first_in(std::string_view s, const char* delims, Func&& f)
{
...
template<typename T> bool arg_in_range(T value, const arg_desc_t& desc, std::ostream& os)
{
...
bool validate_str_arg(std::string_view value, std::string_view type, const arg_desc_t& desc, std::ostream& os)
{
    if (type == "CephIPAddr") {
        entity_addr_t addr;
        ...
        if (find_first_in(strings, "|", [=](auto choice) { return (value == choice); })) {
            return true;
        }
        else {
...
template<bool is_vector, typename T, typename Value = std::conditional_t<is_vector, vector<T>, T>>
bool validate_arg(CephContext* cct, const cmdmap_t& cmdmap, const arg_desc_t& desc, const std::string_view name,
                  const std::string_view type, std::ostream& os)
{
    Value v;
    try {
        if (!cmd_getval(cmdmap, string(name), v)) {
            if constexpr (is_vector) {
                ...
                if (auto req = desc.find("req"); req != end(desc) && req->second == "false") {
                    return true;
                }
                else {
...
bool validate_cmd(CephContext* cct, const std::string& desc, const cmdmap_t& cmdmap, std::ostream& os)
{
    return !find_first_in(desc, " ", [&](auto desc) {
        ...
        if (arg_desc.count("n")) {
            if (type == "CephInt") {
                return !validate_arg<true, int64_t>(cct, cmdmap, arg_desc, name, type, os);
            }
            else if (type == "CephFloat") {
                return !validate_arg<true, double>(cct, cmdmap, arg_desc, name, type, os);
            }
            else {
                return !validate_arg<true, string>(cct, cmdmap, arg_desc, name, type, os);
            }
        }
        else {
            if (type == "CephInt") {
                return !validate_arg<false, int64_t>(cct, cmdmap, arg_desc, name, type, os);
            }
            else if (type == "CephFloat") {
                return !validate_arg<false, double>(cct, cmdmap, arg_desc, name, type, os);
            }
            else {
                return !validate_arg<false, string>(cct, cmdmap, arg_desc, name, type, os);
            }
        }
    });
}

bool cmd_getval(const cmdmap_t& cmdmap, const std::string& k, bool& val)
{
    /*
