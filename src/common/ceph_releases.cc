...
bool can_upgrade_from(ceph_release_t from_release, std::string_view from_release_name, std::ostream& err)
{
    ...
    if (cutoff < to_release) {
        err << "recorded " << from_release_name << " " << to_integer<int>(from_release) << " (" << from_release << ") "
            << "is more than two releases older than installed " << to_integer<int>(to_release) << " (" << to_release
            << "); "
