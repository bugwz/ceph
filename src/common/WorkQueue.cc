...
ThreadPool::ThreadPool(CephContext* cct_, std::string nm, std::string tn, int n, const char* option)
    ...
    , lockname(name + "::lock")
    , _lock(ceph::make_mutex(lockname))
...
void ThreadPool::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
...
void ThreadPool::worker(WorkThread* wt)
{
    ...
    while (!_stop) {
        ...
        if (_threads.size() > _num_threads) {
            ldout(cct, 1) << " worker shutting down; too many threads (" << _threads.size() << " > " << _num_threads
                          << ")" << dendl;
            _threads.erase(wt);
        ...
        if (work_queues.empty()) {
            ...
            while (tries--) {
                ...
                if (item) {
                    processing++;
                    ldout(cct, 12) << "worker wq " << wq->name << " start processing " << item << " (" << processing
                                   << " active)" << dendl;
                    ul.unlock();
                    ...
                    processing--;
                    ldout(cct, 15) << "worker wq " << wq->name << " done processing " << item << " (" << processing
                                   << " active)" << dendl;
                    if (_pause || _draining) _wait_cond.notify_all();
                    did = true;
                    break;
                }
            }
        ...
        cct->get_heartbeat_map()->reset_timeout(
            hb, ceph::make_timespan(cct->_conf->threadpool_default_timeout), ceph::make_timespan(0));
        auto wait = std::chrono::seconds(cct->_conf->threadpool_empty_queue_max_wait);
...
void ThreadPool::stop(bool clear_after)
{
    ...
    _threads.clear();
    _lock.lock();
    for (unsigned i = 0; i < work_queues.size(); i++) work_queues[i]->_clear();
    _stop = false;
    _lock.unlock();
    ...
    while (!stop_threads) {
        if (pause_threads) {
            ...
            while (pause_threads) {
                ...
                shardedpool_cond.wait_for(ul, std::chrono::seconds(cct->_conf->threadpool_empty_queue_max_wait));
            }
            --num_paused;
        }
        if (drain_threads) {
...
void ShardedThreadPool::stop()
{
    ...
    for (auto p = threads_shardedpool.begin(); p != threads_shardedpool.end(); ++p) {
        (*p)->join();
        delete *p;
    }
