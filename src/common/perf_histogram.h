...
template<int DIM = 2> class PerfHistogram : public PerfHistogramCommon
{
public:
    ...
    template<typename... T> void inc(T... axis)
    {
    ...
    template<typename... T> void inc_bucket(T... bucket)
    {
    ...
    template<typename... T> uint64_t read_bucket(T... bucket) const
    {
        auto index = get_raw_index_for_bucket(bucket...);
...
protected:
    ...
    template<typename... T> int64_t get_raw_index_for_value(T... axes) const
    {
    ...
    template<typename... T> int64_t get_raw_index_for_bucket(T... buckets) const
    {
        static_assert(sizeof...(T) == DIM, "Incorrect number of arguments");
    ...
    template<int level, typename F> int64_t get_raw_index_internal(F, int64_t startIndex) const
    {
