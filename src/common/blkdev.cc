...
int get_device_by_path(const char* path, char* partition, char* device, size_t max)
{
 ...
 */
int64_t BlkDev::get_string_property(const char* prop, char* val, size_t maxlen) const
{
    ...
    if (snprintf(filename, sizeof(filename), "%s/block/%s/%s", sysfsdir(), dev, prop) >=
...
int _get_vdo_stats_handle(const char* devname, std::string* vdo_name)
{
    ...
    while ((de = ::readdir(dir))) {
        ...
        if (r < 0 || r >= (int)sizeof(target)) continue;
        target[r] = 0;
        if (expect == target) {
...
bool get_vdo_utilization(int fd, uint64_t* total, uint64_t* avail)
{
    ...
    if (!block_size || !physical_blocks || !overhead_blocks_used || !data_blocks_used) {
        return false;
    }
    int64_t avail_blocks = physical_blocks - overhead_blocks_used - data_blocks_used;
...
std::string get_device_id(const std::string& devname, std::string* err)
{
    ...
    if (!dev) {
        if (err) {
            *err = std::string("udev_device_new_from_subsystem_sysname failed on '") + devname + "'";
        }
        udev_unref(udev);
        return {};
    }
    ...
    // ID_VENDOR is sometimes $vendor and ID_MODEL just $model and ID_SCSI_SERIAL the real serial number, with ID_SERIAL
    // and ID_SERIAL_SHORT gibberish (ick)
    ...
    if (err) {
        if (model.empty() && serial.empty()) {
            ...
            *err = std::string("fallback method has serial '") + serial + "' but no model'";
            return {};
        }
        else if (serial.empty()) {
            *err = std::string("fallback method has model '") + model + "' but no serial'";
            return {};
        }
    }
...
static std::string get_device_vendor(const std::string& devname)
{
    ...
    std::transform(id_vendor.begin(), id_vendor.end(), id_vendor.begin(), ::tolower);
...
static int block_device_run_vendor_nvme(const string& devname, const string& vendor, int timeout, std::string* result)
{
    string device = "/dev/" + devname;

    SubProcessTimed nvmecli("sudo", SubProcess::CLOSE, SubProcess::PIPE, SubProcess::CLOSE, timeout);
    nvmecli.add_cmd_args("nvme", vendor.c_str(), "smart-log-add", "--json", device.c_str(), NULL);
...
std::string get_device_path(const std::string& devname, std::string* err)
{
    ...
    for (auto& i : links) {
        ...
        if (r < 0 || r >= (int)sizeof(target)) continue;
        target[r] = 0;
        if ((unsigned)r > devname.size() + 1 && strncmp(target + r - devname.size(), devname.c_str(), r) == 0 &&
...
static int block_device_run_smartctl(const string& devname, int timeout, std::string* result)
{
    string device = "/dev/" + devname;
    ...
    smartctl.add_cmd_args("smartctl",
                          //"-a",    // all SMART info
    ...
    // (see '-n' option above). Bit 2: Some SMART or other ATA command to the disk failed, or there was a checksum error
    // in a SMART data structure (see '-b' option above). Bit 3: SMART status check returned "DISK FAILING". Bit 4: We
    // found prefail Attributes <= threshold. Bit 5: SMART status check returned "DISK OK" but we found that some (usage
    // or prefail) Attributes have been <= threshold at some time in the past. Bit 6: The device error log contains
    // records of errors. Bit 7: The device self-test log contains records of errors.  [ATA only] Failed self-tests
    // outdated by a newer successful extended self-test are ignored.
    if (joinerr & 3) {
        *result = "smartctl returned an error ("s + stringify(joinerr) + "): stderr:\n"s + smartctl.err() +
                  "\nstdout:\n"s + *result;
        return -EINVAL;
    }
...
int block_device_get_metrics(const string& devname, int timeout, json_spirit::mValue* result)
{
    std::string s;
    ...
    if (vendor.size()) {
        ...
        if (int r = block_device_run_vendor_nvme(devname, vendor, timeout, &s); r == 0) {
            if (json_spirit::read(s, nvme_json) != 0) {
                base["nvme_smart_health_information_add_log"] = nvme_json;
            }
            else {
                base["nvme_smart_health_information_add_log_error"] = "bad json output: " + s;
            }
        }
        else {
...
int BlkDev::get_size(int64_t* psize) const
{
    ...
    if (!ret) {
        ...
        if (!ret) *psize = (int64_t)nblocks * blocksize;
    }
    if (ret < 0) ret = -errno;
    return ret;
}
...
int BlkDev::serial(char* serial, size_t max) const
{
    ...
    if (ioctl(fd, DIOCGIDENT, ident) < 0) return -errno;
...
int block_device_run_smartctl(const char* device, int timeout, std::string* result)
{
    // FIXME: implement me for freebsd
    return -EOPNOTSUPP;
}
...
int block_device_run_nvme(const char* device, const char* vendor, int timeout, std::string* result)
{
    return -EOPNOTSUPP;
}
...
static int block_device_devname(int fd, char* devname, size_t max)
{
    ...
    arg.buf = devname;
    arg.len = max;
    if (ioctl(fd, FIODGNAME, &arg) < 0) return -errno;
    return 0;
}
...
int BlkDev::partition(char* partition, size_t max) const
{
    ...
    if (block_device_devname(fd, devname, sizeof(devname)) < 0) return -errno;
...
void get_device_metadata(const std::set<std::string>& devnames, std::map<std::string, std::string>* pm,
