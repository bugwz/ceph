...
public:
    ...
    void add_u64(int key, const char* name, const char* description = NULL, const char* nick = NULL, int prio = 0,
                 int unit = UNIT_NONE);
    void add_u64_counter(int key, const char* name, const char* description = NULL, const char* nick = NULL,
    ...
    void add_u64_counter_histogram(int key, const char* name, PerfHistogramCommon::axis_config_d x_axis_config,
                                   PerfHistogramCommon::axis_config_d y_axis_config, const char* description = NULL,
...
private:
    ...
    void add_impl(int idx, const char* name, const char* description, const char* nick, int prio, int ty,
                  int unit = UNIT_NONE, std::unique_ptr<PerfHistogram<>> histogram = nullptr);
...
public:
    ...
    void reset();
    void dump_formatted(ceph::Formatter* f, bool schema, bool dump_labeled, const std::string& counter = "") const
    {
    ...
    {
        return std::max(std::min(p + prio_adjust, (int)PerfCountersBuilder::PRIO_CRITICAL), 0);
    }
...
private:
    ...
    void dump_formatted_generic(ceph::Formatter* f, bool schema, bool histograms, bool dump_labeled,
...
public:
    ...
    void dump_formatted(ceph::Formatter* f, bool schema, bool dump_labeled, const std::string& logger = "",
                        const std::string& counter = "") const
    {
        dump_formatted_generic(f, schema, false, dump_labeled, logger, counter);
    }

    void dump_formatted_histograms(ceph::Formatter* f, bool schema, const std::string& logger = "",
                                   const std::string& counter = "") const
    {
...
public:
    PerfGuard(PerfCounters* const counters, const int event)
