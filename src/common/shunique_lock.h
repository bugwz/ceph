...
template<typename Mutex> class shunique_lock
{
public:
    ...
    shunique_lock(mutex_type& m, AcquireType at, const std::chrono::time_point<Clock, Duration>& t)
        : m(&m)
        , o(ownership::none)
    {
        try_lock_until(at, t);
    ...
    shunique_lock(mutex_type& m, AcquireType at, const std::chrono::duration<Rep, Period>& dur)
        : m(&m)
        , o(ownership::none)
    {
        try_lock_for(at, dur);
    ...
    {
        switch (o) {
        case ownership::none: return;
        case ownership::unique: m->unlock(); break;
        case ownership::shared: m->unlock_shared(); break;
        }
    }
    ...
    void lock(ceph::acquire_unique_t) { lock(); }

    void lock(ceph::acquire_shared_t) { lock_shared(); }
    ...
    bool try_lock(ceph::acquire_unique_t) { return try_lock(); }

    bool try_lock(ceph::acquire_shared_t) { return try_lock_shared(); }

    template<typename Rep, typename Period> bool try_lock_for(const std::chrono::duration<Rep, Period>& dur)
    {
        lockable();
    ...
    template<typename Rep, typename Period> bool try_lock_shared_for(const std::chrono::duration<Rep, Period>& dur)
    {
        lockable();
    ...
    bool try_lock_for(ceph::acquire_unique_t, const std::chrono::duration<Rep, Period>& dur)
    {
        return try_lock_for(dur);
    }

    template<typename Rep, typename Period>
    bool try_lock_for(ceph::acquire_shared_t, const std::chrono::duration<Rep, Period>& dur)
    {
    ...
    template<typename Clock, typename Duration>
    bool try_lock_shared_until(const std::chrono::time_point<Clock, Duration>& time)
    {
        lockable();
    ...
    template<typename Clock, typename Duration>
    bool try_lock_until(ceph::acquire_unique_t, const std::chrono::time_point<Clock, Duration>& time)
    {
        return try_lock_until(time);
    }

    template<typename Clock, typename Duration>
    bool try_lock_until(ceph::acquire_shared_t, const std::chrono::time_point<Clock, Duration>& time)
    {
    ...
    void unlock()
    {
        switch (o) {
        case ownership::none:
            throw std::system_error((int)std::errc::resource_deadlock_would_occur, std::generic_category());
            break;
    ...
    unique_lock_type release_to_unique()
    {
        ...
        throw std::system_error((int)std::errc::operation_not_permitted, std::generic_category());
    }
    ...
    // false for shared
    bool owns_lock() const noexcept { return o == ownership::unique; }

    bool owns_lock_shared() const noexcept { return o == ownership::shared; }

    // If you want to make sure you have a lock of some sort on the
    // mutex, just treat as a bool.
    explicit operator bool() const noexcept { return o != ownership::none; }

    mutex_type* mutex() const noexcept { return m; }

private:
    void lockable() const
    {
        if (m == nullptr) throw std::system_error((int)std::errc::operation_not_permitted, std::generic_category());
        if (o != ownership::none)
            throw std::system_error((int)std::errc::resource_deadlock_would_occur, std::generic_category());
    }
...
namespace std {
template<typename Mutex> void swap(ceph::shunique_lock<Mutex> sh1, ceph::shunique_lock<Mutex> sha)
{
    sh1.swap(sha);
}
}   // namespace std
