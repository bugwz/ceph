        ...
        {
            {
                ...
                typename std::map<K, std::pair<WeakVPtr, V*>, C>::iterator i = parent->contents.find(key);
...
public:
    ...
    bool get_next(const K& key, std::pair<K, VPtr>* next)
    {
        std::pair<K, VPtr> r;
        {
            std::lock_guard l(lock);
            VPtr next_val;
            typename std::map<K, std::pair<WeakVPtr, V*>, C>::iterator i = contents.upper_bound(key);
            while (i != contents.end() && !(next_val = i->second.first.lock())) ++i;
            if (i == contents.end()) return false;
            if (next) r = std::make_pair(i->first, next_val);
        }
        if (next) *next = r;
        return true;
    }
    ...
    bool get_next(const K& key, std::pair<K, V>* next)
    {
        ...
        while (i != contents.end() && !(next_val = i->second.first.lock())) ++i;
        if (i == contents.end()) return false;
        if (next) *next = std::make_pair(i->first, *next_val);
        return true;
    }
