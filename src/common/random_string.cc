...
int gen_rand_base64(CephContext* cct, char* dest, size_t size) /* size should be the required string size + 1 */
{
    ...
    ret = ceph_armor(
        tmp_dest, &tmp_dest[sizeof(tmp_dest)], (const char*)buf, ((const char*)buf) + ((size - 1) * 3 + 4 - 1) / 4);
    if (ret < 0) {
...
static void choose_from(CryptoRandom* random, std::string_view table, char* dest, size_t size)
{
...
void gen_rand_alphanumeric_lower(CephContext* cct, char* dest,
                                 size_t size) /* size should be the required string size + 1 */
{
...
void gen_rand_alphanumeric_upper(CephContext* cct, char* dest,
                                 size_t size) /* size should be the required string size + 1 */
{
    static constexpr char table[] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    choose_from(cct->random(), table, dest, size - 1);
...
void gen_rand_alphanumeric_plain(CephContext* cct, char* dest,
                                 size_t size) /* size should be the required string size + 1 */
{
    static constexpr char table[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
    choose_from(cct->random(), table, dest, size - 1);
