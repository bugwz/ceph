...
int ceph_resolve_file_search(const std::string& filename_list, std::string& result)
{
    list<string> ls;
    ...
    : is_daemon(is_daemon)
{
    ...
    for (unsigned i = 0; i < values.subsys.get_num(); ++i) {
        ...
        opt.set_long_description(
            "The value takes the form 'N' or 'N/M' where N and M are values between 0 and 99.  N is the debug level to "
            "log (all values below this are included), and M is the level to gather and buffer in memory.  In the "
            "event of a crash, the most recent items <= M are dumped to the log file.");
        opt.set_subsys(i);
    ...
    // members if present.
    legacy_values = {
#define OPTION(name, type)      {STRINGIFY(name), &ConfigValues::name},
    ...
    for (const auto& i : schema) {
        ...
        if (opt.type == Option::TYPE_STR) {
            ...
            if (opt.pre_validate(&val, &err) != 0) {
                std::cerr << "Default value " << opt.name << "=" << *def_str
                          << " is "
                             "invalid: "
                          << err << std::endl;
    ...
    for (const auto& i : schema) {
        ...
        for (const auto& see_also_key : opt.see_also) {
            if (schema.count(see_also_key) == 0) {
                std::cerr << "Non-existent see-also key '" << see_also_key << "' on option '" << opt.name << "'"
                          << std::endl;
                ceph_abort();
            }
        }
...
void md_config_t::set_val_default(ConfigValues& values, const ConfigTracker& tracker, const string_view name,
...
int md_config_t::set_mon_vals(CephContext* cct, ConfigValues& values, const ConfigTracker& tracker,
                              const map<string, string, less<>>& kv, config_callback config_cb)
{
    ...
    for (auto& i : kv) {
        ...
        else if (r == ConfigValues::SET_NO_CHANGE || r == ConfigValues::SET_NO_EFFECT) {
...
int md_config_t::parse_config_files(ConfigValues& values, const ConfigTracker& tracker, const char* conf_files_str,
                                    std::ostream* warnings, int flags)
{
    if (safe_to_start_threads) return -ENOSYS;
...
int md_config_t::parse_buffer(ConfigValues& values, const ConfigTracker& tracker, const char* buf, size_t len,
                              std::ostream* warnings)
{
    ...
    for (const auto& i : schema) {
        ...
        if (_set_val(values, tracker, val, opt, CONF_FILE, &error_message) < 0) {
            if (warnings != nullptr) {
                *warnings << "parse error setting " << std::quoted(opt.name) << " to " << std::quoted(val);
...
std::list<std::string> md_config_t::get_conffile_paths(const ConfigValues& values, const char* conf_files_str,
                                                       std::ostream* warnings, int flags) const
{
    if (!conf_files_str) {
        ...
        if (c) {
            ...
            if (flags & CINIT_FLAG_NO_DEFAULT_CONFIG_FILE) return {};
...
void md_config_t::parse_env(unsigned entity_type, ConfigValues& values, const ConfigTracker& tracker,
                            const char* args_var)
{
    if (safe_to_start_threads) return;
    if (!args_var) {
        args_var = "CEPH_ARGS";
    }
    ...
    if (auto pod_lim = getenv("POD_MEMORY_LIMIT"); pod_lim) {
        ...
        if (v) {
            ...
            {
                double cgroup_ratio = get_val<double>(values, "osd_memory_target_cgroup_limit_ratio");
                if (cgroup_ratio > 0.0) {
                    ...
                    set_val_default(values, tracker, "osd_memory_target", stringify(pod_limit));
                }
            }
            }
        }
    ...
    if (pod_request) {
        ...
        switch (entity_type) {
        case CEPH_ENTITY_TYPE_OSD:
            _set_val(values, tracker, stringify(pod_request), *find_option("osd_memory_target"), CONF_ENV, &err);
            break;
        }
    }
...
void md_config_t::show_config(const ConfigValues& values, std::ostream& out) const
{
    _show_config(values, &out, nullptr);
}

void md_config_t::show_config(const ConfigValues& values, Formatter* f) const
{
...
void md_config_t::_show_config(const ConfigValues& values, std::ostream* out, Formatter* f) const
{
    if (out) {
...
int md_config_t::parse_argv(ConfigValues& values, const ConfigTracker& tracker, std::vector<const char*>& args,
                            int level)
{
...
void md_config_t::do_argv_commands(const ConfigValues& values) const
{
    ...
    if (do_show_config_value.size()) {
        string val;
        int r = conf_stringify(_get_val(values, do_show_config_value, 0, &cerr), &val);
        if (r < 0) {
            if (r == -ENOENT)
                std::cerr << "failed to get config option '" << do_show_config_value << "': option not found"
                          << std::endl;
            else
                std::cerr << "failed to get config option '" << do_show_config_value << "': " << cpp_strerror(r)
                          << std::endl;
            _exit(1);
        }
...
int md_config_t::parse_option(ConfigValues& values, const ConfigTracker& tracker, std::vector<const char*>& args,
                              std::vector<const char*>::iterator& i, ostream* oss, int level)
{
    int ret = 0;
    size_t o = 0;
    std::string val;
    ...
    for (const auto& opt_iter : schema) {
        ...
        if (ceph_argparse_witharg(args, i, &val, err, string(string("--default-") + opt.name).c_str(), (char*)NULL)) {
            if (!err.str().empty()) {
            ...
            int res;
            if (ceph_argparse_binary_flag(args, i, &res, oss, as_option.c_str(), (char*)NULL)) {
                if (res == 0)
        ...
        else if (ceph_argparse_witharg(args, i, &val, err, as_option.c_str(), (char*)NULL)) {
            if (!err.str().empty()) {
                error_message = err.str();
                ret = -EINVAL;
                break;
    ...
    if (ret < 0 || !error_message.empty()) {
        ...
        if (oss) {
            *oss << "Parse error setting " << option_name << " to '" << val << "' using injectargs";
            if (!error_message.empty()) {
                *oss << " (" << error_message << ")";
            }
            *oss << ".\n";
            ...
            cerr << "parse error setting '" << option_name << "' to '" << val << "'";
            if (!error_message.empty()) {
                cerr << " (" << error_message << ")";
            }
            cerr << "\n" << std::endl;
...
int md_config_t::parse_injectargs(ConfigValues& values, const ConfigTracker& tracker, std::vector<const char*>& args,
                                  std::ostream* oss)
{
    int ret = 0;
    for (std::vector<const char*>::iterator i = args.begin(); i != args.end();) {
...
int md_config_t::injectargs(ConfigValues& values, const ConfigTracker& tracker, const std::string& s, std::ostream* oss)
{
    int ret;
    char b[s.length() + 1];
    strcpy(b, s.c_str());
    ...
    if (!nargs.empty()) {
        *oss << " failed to parse arguments: ";
        std::string prefix;
        for (std::vector<const char*>::const_iterator i = nargs.begin(); i != nargs.end(); ++i) {
            *oss << prefix << *i;
            prefix = ",";
        }
        *oss << "\n";
...
void md_config_t::set_val_or_die(ConfigValues& values, const ConfigTracker& tracker, const std::string_view key,
                                 const std::string& val)
{
...
int md_config_t::set_val(ConfigValues& values, const ConfigTracker& tracker, const std::string_view key,
                         const char* val, std::stringstream* err_ss)
{
    if (key.empty()) {
...
void md_config_t::get_config_bl(const ConfigValues& values, uint64_t have_version, bufferlist* bl,
...
int md_config_t::get_val(const ConfigValues& values, const std::string_view key, char** buf, int len) const
{
    string k(ConfFile::normalize_key_name(key));
    return _get_val_cstr(values, k, buf, len);
}

int md_config_t::get_val(const ConfigValues& values, const std::string_view key, std::string* val) const
{
    return conf_stringify(get_val_generic(values, key), val);
}

Option::value_t md_config_t::get_val_generic(const ConfigValues& values, const std::string_view key) const
{
    return _get_val(values, key);
}

Option::value_t md_config_t::_get_val(const ConfigValues& values, const std::string_view key, expand_stack_t* stack,
...
Option::value_t md_config_t::_get_val(const ConfigValues& values, const Option& o, expand_stack_t* stack,
                                      std::ostream* err) const
{
...
Option::value_t md_config_t::_get_val_nometa(const ConfigValues& values, const Option& o) const
{
...
void md_config_t::early_expand_meta(const ConfigValues& values, std::string& val, std::ostream* err) const
{
...
Option::value_t md_config_t::_expand_meta(const ConfigValues& values, const Option::value_t& in, const Option* o,
                                          expand_stack_t* stack, std::ostream* err) const
{
            ...
            if (var == "type") {
                ...
                if (!o) {
                    ...
                    auto match = std::find_if(
                        stack->begin(), stack->end(), [o](pair<const Option*, const Option::value_t*>& item) {
                            return item.first == o;
                        });
                    if (match != stack->end()) {
                        // substitution loop; break the cycle
                        if (err) {
                            *err << "variable expansion loop at " << var << "=" << Option::to_str(*match->second)
                                 << "\n"
                                 << "expansion stack:\n";
...
int md_config_t::_get_val_cstr(const ConfigValues& values, const std::string& key, char** buf, int len) const
{
 ...
 */
std::vector<std::string> md_config_t::get_my_sections(const ConfigValues& values) const
{
    return {values.name.to_str(), values.name.get_type_name().data(), "global"};
}
...
int md_config_t::get_val_from_conf_file(const ConfigValues& values, const std::vector<std::string>& sections,
...
int md_config_t::_get_val_from_conf_file(const std::vector<std::string>& sections, const std::string_view key,
...
int md_config_t::_set_val(ConfigValues& values, const ConfigTracker& observers, const std::string& raw_val,
                          const Option& opt, int level, std::string* error_message)
{
    ...
    if (!opt.can_update_at_runtime() && safe_to_start_threads && !observers.is_tracking(opt.name)) {
    ...
    switch (result) {
    case ConfigValues::SET_NO_CHANGE: break;
    case ConfigValues::SET_NO_EFFECT: values_bl.clear(); break;
...
int md_config_t::_rm_val(ConfigValues& values, const std::string_view key, int level)
{
...
namespace {
template<typename Size> struct get_size_visitor : public boost::static_visitor<Size>
{
    get_size_visitor() {}

    template<typename T> Size operator()(const T&) const { return -1; }
    Size operator()(const Option::size_t& sz) const { return static_cast<Size>(sz.value); }
    Size operator()(const Size& v) const { return v; }
};

/**
 * Handles assigning from a variant-of-types to a variant-of-pointers-to-types
 */
template<class Config> class assign_visitor : public boost::static_visitor<>
{
    Config* conf;
    Option::value_t val;
...
public:
    ...
    template<typename T> void operator()(T Config::*ptr) const
    {
...
void md_config_t::update_legacy_val(ConfigValues& values, const Option& opt, md_config_t::member_ptr_t member_ptr)
{
...
void md_config_t::diff(const ConfigValues& values, Formatter* f, string name) const
{
    values.for_each([this, f, &values](auto& name, auto& configs) {
        ...
        const Option* o = find_option(name);
        if (configs.size() && configs.begin()->first != CONF_DEFAULT) {
