...
public:
    SloppyCRCMap(uint32_t b = 0) { set_block_size(b); }
    ...
    void write(uint64_t offset, uint64_t len, const ceph::buffer::list& bl, std::ostream* out = NULL);
    ...
    /// update based on a zero/punch_hole
    void clone_range(uint64_t offset, uint64_t len, uint64_t srcoff, const SloppyCRCMap& src, std::ostream* out = NULL);
