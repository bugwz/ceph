...
template<typename Handler> struct ForwardingHandler
{
    ...
    template<typename... Args> void operator()(Args&&... args) { std::move(handler)(std::forward<Args>(args)...); }
...
struct associated_executor<ceph::async::ForwardingHandler<Handler>, Executor>
{
    ...
    static type get(const ceph::async::ForwardingHandler<Handler>& handler, const Executor& ex = Executor()) noexcept
    {
