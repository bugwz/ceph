...
public:
    ...
    template<typename Mutex, typename CompletionToken> auto async_lock(Mutex& mtx, CompletionToken&& token);
    void lock();
    void lock(boost::system::error_code& ec);
    bool try_lock();
    void unlock();
    template<typename Mutex, typename CompletionToken> auto async_lock_shared(Mutex& mtx, CompletionToken&& token);
    void lock_shared();
...
template<typename Mutex, template<typename> typename Lock> class AsyncRequest : public LockRequest
{
    Mutex& mutex;   //< mutex argument for lock guard
public:
    explicit AsyncRequest(Mutex& mutex)
        : mutex(mutex)
    {}
    ...
    void complete(boost::system::error_code ec) override
    {
        ...
        post(std::unique_ptr<LockCompletion>{r}, ec, ec ? Lock{mutex, std::defer_lock} : Lock{mutex, std::adopt_lock});
    }
    void destroy() override { delete static_cast<LockCompletion*>(this); }
};
...
template<typename Mutex, typename CompletionToken> auto SharedMutexImpl::async_lock(Mutex& mtx, CompletionToken&& token)
{
    ...
    auto ex1 = mtx.get_executor();
    {
        ...
        if (state == Unlocked) {
            ...
            auto b = bind_handler(std::move(handler), ec, std::unique_lock{mtx, std::adopt_lock});
    ...
    RequestList granted;
    {
        ...
        if (!exclusive_queue.empty()) {
            ...
            if (state > MaxShared) {
                ...
                granted.splice(granted.end(), shared_queue, shared_queue.begin(), end, MaxShared);
            }
            else {
...
auto SharedMutexImpl::async_lock_shared(Mutex& mtx, CompletionToken&& token)
{
    ...
    auto ex1 = mtx.get_executor();
    {
        ...
        if (exclusive_queue.empty() && state < MaxShared) {
            ...
            auto ex2 = boost::asio::get_associated_executor(handler, ex1);
            auto alloc2 = boost::asio::get_associated_allocator(handler);
            auto b = bind_handler(std::move(handler), ec, std::shared_lock{mtx, std::adopt_lock});
            ex2.post(forward_handler(std::move(b)), alloc2);
        }
        else {
            using LockCompletion = typename Request::LockCompletion;
...
inline void SharedMutexImpl::unlock_shared()
{
    ...
    else if (state == MaxShared && !shared_queue.empty() && exclusive_queue.empty()) {
...
void SharedMutexImpl::complete(RequestList&& requests, boost::system::error_code ec)
{
