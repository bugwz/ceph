...
template<typename Executor> class unique_lock<ceph::async::SharedMutex<Executor>>
{
public:
    ...
    void lock()
    {
        if (!impl) throw system_error(make_error_code(errc::operation_not_permitted));
        if (locked) throw system_error(make_error_code(errc::resource_deadlock_would_occur));
        impl->lock();
        locked = true;
    }
    bool try_lock()
    ...
    void unlock()
    {
        if (!impl || !locked) throw system_error(make_error_code(errc::operation_not_permitted));
        impl->unlock();
        locked = false;
    }
...
template<typename Executor> class shared_lock<ceph::async::SharedMutex<Executor>>
{
public:
    using mutex_type = boost::intrusive_ptr<ceph::async::detail::SharedMutexImpl>;
    ...
    ~shared_lock()
    {
        if (impl && locked) impl->unlock_shared();
    }
