 ...
 * @see bind_handler
 */
template<typename Handler, typename Tuple> struct CompletionHandler
{
    Handler handler;
    Tuple args;
    ...
    void operator()() & { std::apply(handler, args); }
    void operator()() const& { std::apply(handler, args); }
    void operator()() && { std::apply(std::move(handler), std::move(args)); }
 ...
 */
template<typename Handler, typename... Args> auto bind_handler(Handler&& h, Args&&... args)
{
    return CompletionHandler{std::forward<Handler>(h), std::make_tuple(std::forward<Args>(args)...)};
}
