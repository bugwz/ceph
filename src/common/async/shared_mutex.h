 ...
 *   context.run();
 */
template<typename Executor> class SharedMutex
{
public:
    ...
    template<typename CompletionToken> auto async_lock(CompletionToken&& token);
    ...
    template<typename CompletionToken> auto async_lock_shared(CompletionToken&& token);
...
SharedMutex<Executor>::SharedMutex(const Executor& ex)
    : ex(ex)
    , impl(new detail::SharedMutexImpl)
{}

template<typename Executor> SharedMutex<Executor>::~SharedMutex()
{
    try {
        impl->cancel();
    }
...
template<typename Executor> void SharedMutex<Executor>::lock()
{
    impl->lock();
}

template<typename Executor> void SharedMutex<Executor>::lock(boost::system::error_code& ec)
{
    impl->lock(ec);
}

template<typename Executor> bool SharedMutex<Executor>::try_lock()
{
    return impl->try_lock();
}

template<typename Executor> void SharedMutex<Executor>::unlock()
{
    impl->unlock();
}
...
template<typename Executor> void SharedMutex<Executor>::lock_shared()
{
    impl->lock_shared();
}

template<typename Executor> void SharedMutex<Executor>::lock_shared(boost::system::error_code& ec)
{
    impl->lock_shared(ec);
}

template<typename Executor> bool SharedMutex<Executor>::try_lock_shared()
{
    return impl->try_lock_shared();
}

template<typename Executor> void SharedMutex<Executor>::unlock_shared()
{
    impl->unlock_shared();
}

template<typename Executor> void SharedMutex<Executor>::cancel()
{
    impl->cancel();
}
