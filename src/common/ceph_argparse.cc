...
std::pair<std::vector<const char*>, std::vector<const char*>> split_dashdash(const std::vector<const char*>& args)
{
    auto dashdash = std::find_if(args.begin(), args.end(), [](const char* arg) { return strcmp(arg, "--") == 0; });
...
void vec_to_argv(const char* argv0, std::vector<const char*>& args, int* argc, const char*** argv)
{
    ...
    *argc = 1;
    ...
    for (unsigned i = 0; i < args.size(); i++) (*argv)[(*argc)++] = args[i];
}
...
static bool va_ceph_argparse_binary_flag(std::vector<const char*>& args, std::vector<const char*>::iterator& i,
                                         int* ret, std::ostream* oss, va_list ap)
{
    const char* first = *i;
    char tmp[strlen(first) + 1];
    dashes_to_underscores(first, tmp);
    ...
    while (1) {
        ...
        if (strncmp(a2, first, strlen(a2)) == 0) {
            if (first[strlen_a] == '=') {
                ...
                if (oss) {
                    (*oss) << "Parse error parsing binary flag  " << a << ". Expected true or false, but got '" << val
                           << "'\n";
                }
                *ret = -EINVAL;
                return true;
...
bool ceph_argparse_binary_flag(std::vector<const char*>& args, std::vector<const char*>::iterator& i, int* ret,
                               std::ostream* oss, ...)
{
    bool r;
    va_list ap;
...
CephInitParameters ceph_argparse_early_args(std::vector<const char*>& args, uint32_t module_type, std::string* cluster,
            ...
            *cluster = val;
            ...
            if (!iparams.name.from_str(val)) {
                std::cerr << "error parsing '" << val << "': expected string of the form TYPE.ID, "
                          << "valid types are: " << EntityName::get_valid_types_as_str() << std::endl;
                _exit(1);
            }
        }
            ...
            for (std::vector<const char*>::iterator ci = orig_args.begin(); ci != orig_args.end(); ++ci) {
                if (ci != orig_args.begin()) std::cout << " ";
                std::cout << *ci;
            }
            std::cout << std::endl;
        }
...
{
    std::cout << "  --conf/-c FILE    read configuration from the given configuration file" << std::endl
              << (is_server ? "  --id/-i ID        set ID portion of my name"
    ...
    if (is_server) {
        std::cout << "  -d                run in foreground, log to stderr" << std::endl
