...
{
    str_list.clear();
    for_each_substr(str, delims, [&str_list](auto token) { str_list.emplace_back(token.begin(), token.end()); });
}
...
{
    str_vec.clear();
    for_each_substr(str, delims, [&str_vec](auto token) { str_vec.emplace_back(token.begin(), token.end()); });
}
...
vector<string> get_str_vec(std::string_view str, const char* delims)
{
    ...
    for_each_substr(str, delims, [&result](auto token) { result.emplace_back(token.begin(), token.end()); });
    return result;
}
