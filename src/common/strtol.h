...
template<typename T>
auto parse(std::string_view s, int base = 10) -> std::enable_if_t<std::is_integral_v<T>, std::optional<T>>
{
    T t;
...
template<typename T>
auto consume(std::string_view& s, int base = 10) -> std::enable_if_t<std::is_integral_v<T>, std::optional<T>>
{
    T t;
    auto r = std::from_chars(s.data(), s.data() + s.size(), t, base);
 ...
 */
template<typename T, const unsigned base = 10, const unsigned width = 1> static inline char* ritoa(T u, char* buf)
{
