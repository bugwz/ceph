...
{
    size_t operator()(const LogEntryKey& r) const { return r.get_hash(); }
};
}   // namespace std
...
struct LogSummary
{
    ...
    bool contains(const LogEntryKey& k) const { return keys.count(k) || recent_keys.contains(k); }
...
{
    switch (t) {
    case CLOG_DEBUG: return out << "[DBG]";
    case CLOG_INFO: return out << "[INF]";
    case CLOG_SEC: return out << "[SEC]";
    case CLOG_WARN: return out << "[WRN]";
    case CLOG_ERROR: return out << "[ERR]";
    default: return out << "[???]";
    }
}
...
{
    return out << e.stamp << " " << e.name << " (" << e.rank << ") " << e.seq << " : " << e.channel << " " << e.prio
               << " " << e.msg;
}

template<> struct fmt::formatter<EntityName> : fmt::formatter<std::string_view>
{
    template<typename FormatContext> auto format(const EntityName& e, FormatContext& ctx)
    {
...
{
    template<typename FormatContext> auto format(const LogEntry& e, FormatContext& ctx)
    {
        return fmt::format_to(
            ctx.out(), "{} {} ({}) {} : {} {} {}", e.stamp, e.name, e.rank, e.seq, e.channel, e.prio, e.msg);
    }
};
