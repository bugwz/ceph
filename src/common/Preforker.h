...
public:
    ...
    int get_signal_fd() const { return forked ? fd[1] : 0; }

    bool is_child() { return childpid == 0; }

    bool is_parent() { return childpid != 0; }
    ...
    int parent_wait(std::string& err_msg)
    {
        ...
        if (err == 0 && r == -1) {
            ...
            if (err < 0) {
                oss << "[" << getpid() << "]"
                    << " waitpid error: " << cpp_strerror(err);
            }
                ...
                if (err != 0)
                    oss << "[" << getpid() << "]"
                        << " returned exit_status " << cpp_strerror(err);
            }
        }
        err_msg = oss.str();
        return err;
