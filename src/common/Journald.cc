...
{
    if (ceph_prio < 0) return LOG_ERR;
    if (ceph_prio == 0) return LOG_WARNING;
    if (ceph_prio < 5) return LOG_NOTICE;
    if (ceph_prio < 10) return LOG_INFO;
    return LOG_DEBUG;
}
}   // namespace
...
public:
    ...
    constexpr std::size_t iovec_len() { return sizeof(m_msg_vec) / sizeof(m_msg_vec[0]); }
...
int JournaldClient::open_mem_file()
{
    ...
    case MemFileMode::MEMFD_CREATE: return memfd_create("ceph-journald", MFD_ALLOW_SEALING | MFD_CLOEXEC);
    case MemFileMode::OPEN_TMPFILE: return open(mem_file_dir, O_TMPFILE | O_EXCL | O_CLOEXEC, S_IRUSR | S_IWUSR);
...
int JournaldClient::send()
{
    ...
    if (ret < 0) {
        /* Fail silently if the journal is not available */
        if (errno == ENOENT) goto err_close_buffer_fd;
