...
{
    template<class LockT> void wait(LockT&&)
    {
...
template<typename... Args> dummy_mutex make_mutex(Args&&... args)
{
    return {};
}

template<typename... Args> recursive_mutex make_recursive_mutex(Args&&... args)
{
    return {};
}

template<typename... Args> shared_mutex make_shared_mutex(Args&&... args)
{
    return {};
}
...
template<typename... Args> mutex make_mutex(Args&&... args)
{
...
template<class LockT, class LockFactoryT>
ceph::containers::tiny_vector<LockT> make_lock_container(const std::size_t num_instances, LockFactoryT&& lock_factory)
{
    return {num_instances, [&](const std::size_t i, auto emplacer) {
