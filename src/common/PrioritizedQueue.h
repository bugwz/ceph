 ...
 */
template<typename T, typename K> class PrioritizedQueue : public OpQueue<T, K>
{
    ...
    void distribute_tokens(unsigned cost)
    {
        ...
        for (typename SubQueues::iterator i = queue.begin(); i != queue.end(); ++i) {
...
public:
    ...
    {
        unsigned total = 0;
        for (typename SubQueues::const_iterator i = queue.begin(); i != queue.end(); ++i) {
            ceph_assert(i->second.length());
            total += i->second.length();
        }
        for (typename SubQueues::const_iterator i = high_queue.begin(); i != high_queue.end(); ++i) {
            ceph_assert(i->second.length());
            total += i->second.length();
        }
        return total;
    ...
    void enqueue(K cl, unsigned priority, unsigned cost, T&& item) final
    {
        ...
        if (cost > max_tokens_per_subqueue) cost = max_tokens_per_subqueue;
    ...
    void dump(ceph::Formatter* f) const final
    {
        ...
        for (typename SubQueues::const_iterator p = queue.begin(); p != queue.end(); ++p) {
            f->open_object_section("subqueue");
            f->dump_int("priority", p->first);
            p->second.dump(f);
            f->close_section();
    ...
    void print(std::ostream& ostream) const final { ostream << "PrioritizedQueue"; }
};
