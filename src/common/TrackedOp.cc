...
void OpHistory::_insert_delayed(const utime_t& now, TrackedOpRef op)
{
    ...
    if (opduration >= history_slow_op_threshold.load()) slow_op.insert(make_pair(op->get_initiated(), op));
    cleanup(now);
}

void OpHistory::cleanup(utime_t now)
{
    while (arrived.size() && (now - arrived.begin()->first > (double)(history_duration.load()))) {
        duration.erase(make_pair(arrived.begin()->second->get_duration(), arrived.begin()->second));
        arrived.erase(arrived.begin());
    }

    while (duration.size() > history_size.load()) {
        arrived.erase(make_pair(duration.begin()->second->get_initiated(), duration.begin()->second));
        duration.erase(duration.begin());
    }

    while (slow_op.size() > history_slow_op_size.load()) {
        slow_op.erase(make_pair(slow_op.begin()->second->get_initiated(), slow_op.begin()->second));
    }
}
    ...
    f->dump_int("duration", history_duration.load());
    {
        ...
        auto dump_fn = [&f, &now, &filters](auto begin_iter, auto end_iter) {
            for (auto i = begin_iter; i != end_iter; ++i) {
                if (!i->second->filter_out(filters)) continue;
...
void OpHistory::dump_slow_ops(utime_t now, Formatter* f, set<string> filters)
{
    ...
    {
        f->open_array_section("Ops");
        for (set<pair<utime_t, TrackedOpRef>>::const_iterator i = slow_op.begin(); i != slow_op.end(); ++i) {
            if (!i->second->filter_out(filters)) continue;
            f->open_object_section("Op");
            i->second->dump(now, f);
            f->close_section();
...
bool OpTracker::dump_ops_in_flight(Formatter* f, bool print_only_blocked, set<string> filters)
{
    ...
    for (uint32_t i = 0; i < num_optracker_shards; i++) {
        ...
        for (auto& op : sdata->ops_in_flight_sharded) {
            if (print_only_blocked && (now - op.get_initiated() <= complaint_time)) break;
            if (!op.filter_out(filters)) continue;
            f->open_object_section("op");
            op.dump(now, f);
...
bool OpTracker::visit_ops_in_flight(utime_t* oldest_secs, std::function<bool(TrackedOp&)>&& visit)
{
    if (!tracking_enabled) return false;
    ...
    if (ops_in_flight.empty()) return false;
    *oldest_secs = now - oldest_op;
    dout(10) << "ops_in_flight.size: " << ops_in_flight.size() << "; oldest is " << *oldest_secs << " seconds old"
             << dendl;

    if (*oldest_secs < complaint_time) return false;
...
bool OpTracker::with_slow_ops_in_flight(utime_t* oldest_secs, int* num_slow_ops, int* num_warned_ops,
                                        std::function<void(TrackedOp&)>&& on_warn)
{
    ...
    auto check = [&](TrackedOp& op) {
        ...
        if (!op.warn_interval_multiplier) return true;
        slow++;
        if (warned >= log_threshold) {
            // enough samples of slow ops
            return true;
        }
        auto time_to_complain = (op.get_initiated() + complaint_time * op.warn_interval_multiplier);
...
bool OpTracker::check_ops_in_flight(std::string* summary, std::vector<string>& warnings, int* num_slow_ops)
{
    const utime_t now = ceph_clock_now();
    auto too_old = now;
    too_old -= complaint_time;
    ...
    auto warn_on_slow_op = [&](TrackedOp& op) {
        ...
        ss << "slow request " << age << " seconds old, received at " << op.get_initiated() << ": " << op.get_desc()
           << " currently " << op.state_string();
        warnings.push_back(ss.str());
        // only those that have been shown will backoff
        op.warn_interval_multiplier *= 2;
    };
    int slow = 0;
    if (with_slow_ops_in_flight(&oldest_secs, &slow, &warned, warn_on_slow_op) && slow > 0) {
        stringstream ss;
        ss << slow << " slow requests, " << warned << " included below; oldest blocked for > " << oldest_secs
           << " secs";
        *summary = ss.str();
        if (num_slow_ops) {
            *num_slow_ops = slow;
...
void TrackedOp::mark_event(std::string_view event, utime_t stamp)
{
    ...
    dout(6) << " seq: " << seq << ", time: " << stamp << ", event: " << event << ", op: " << get_desc() << dendl;
    _event_marked();
}
