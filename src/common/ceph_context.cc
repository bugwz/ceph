...
public:
    explicit LockdepObs(CephContext* cct)
        : m_cct(cct)
        , m_registered(false)
        , lock(ceph::make_mutex("lock_dep_obs"))
    {}
    ~LockdepObs() override
    {
        if (m_registered) {
...
public:
        ...
        , lock(ceph::make_mutex("mem_pool_obs"))
    {
        ...
        int r = cct->get_admin_socket()->register_command("dump_mempools", this, "get mempool stats");
        ceph_assert(r == 0);
    }
    ~MempoolObs() override
    {
    ...
    const char** get_tracked_conf_keys() const override
    {
        static const char* KEYS[] = {"mempool_debug", NULL};
        return KEYS;
    }
    ...
    // AdminSocketHook
    int call(std::string_view command, const cmdmap_t& cmdmap, const bufferlist& inbl, ceph::Formatter* f,
...
public:
    ...
    const char** get_tracked_conf_keys() const override
    {
        static const char* KEYS[] = {"log_file",
                                     "log_max_new",
                                     "log_max_recent",
                                     "log_to_file",
                                     "log_to_syslog",
...
public:
    explicit CephContextObs(CephContext* cct)
        : cct(cct)
    {}

    const char** get_tracked_conf_keys() const override
    {
        static const char* KEYS[] = {"enable_experimental_unrecoverable_data_corrupting_features",
                                     "crush_location",
    ...
    void handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed) override
    {
        if (changed.count("enable_experimental_unrecoverable_data_corrupting_features")) {
            ...
            for_each_substr(
                conf->enable_experimental_unrecoverable_data_corrupting_features, ";,= \t", add_experimental_feature);
...
public:
    explicit CephContextHook(CephContext* cct)
        : m_cct(cct)
    {}

    int call(std::string_view command, const cmdmap_t& cmdmap, const bufferlist& inbl, Formatter* f,
             std::ostream& errss, bufferlist& out) override
    {
        try {
...
bool CephContext::check_experimental_feature_enabled(const std::string& feat, std::ostream* message)
{
    std::unique_lock<ceph::spinlock> lg(_feature_lock);

    bool enabled = (_experimental_features.count(feat) || _experimental_features.count("*"));
...
int CephContext::do_command(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, std::ostream& ss,
                            bufferlist* out)
{
    try {
...
int CephContext::_do_command(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, std::ostream& ss,
                             bufferlist* out)
{
    int r = 0;
    ...
    if (command == "leak_some_memory") {
    ...
    else if (command == "perfcounters_dump" || command == "1" || command == "perf dump") {
        std::string logger;
        std::string counter;
    ...
    else if (command == "perfcounters_schema" || command == "2" || command == "perf schema") {
        ...
        cmd_getval(cmdmap, "logger", logger);
        cmd_getval(cmdmap, "counter", counter);
        _perf_counters_collection->dump_formatted_histograms(f, false, logger, counter);
    }
        ...
        if (command == "config show") {
            ...
            if (!(cmd_getval(cmdmap, "var", var))) {
                ...
                if (r < 0 && r != -ENOENT) {
                    ss << "error unsetting '" << var << "': " << cpp_strerror(r);
                }
                else {
            ...
            if (!(cmd_getval(cmdmap, "var", var)) || !(cmd_getval(cmdmap, "val", val))) {
                r = -EINVAL;
            }
            else {
                ...
                if (r < 0) {
                    ss << "error setting '" << var << "' to '" << valstr << "': " << cpp_strerror(r);
                }
                else {
                    std::stringstream ss;
                    _conf.apply_changes(&ss);
    ...
    lgeneric_dout(this, 1) << "do_command '" << command << "' '" << cmdmap << "' result is " << out->length()
                           << " bytes" << dendl;
    return r;
}
...
CephContext::CephContext(uint32_t module_type_, const create_options& options)
    : nref(1)
    ...
    crush_location(this)
{
    ...
    _admin_socket->register_command(
        "perf dump name=logger,type=CephString,req=false name=counter,type=CephString,req=false",
        _admin_hook,
        "dump non-labeled counters and their values");
    _admin_socket->register_command("perfcounters_schema", _admin_hook, "");
    _admin_socket->register_command(
        "perf histogram dump name=logger,type=CephString,req=false name=counter,type=CephString,req=false",
        _admin_hook,
    ...
    _admin_socket->register_command(
        "counter dump", _admin_hook, "dump all labeled and non-labeled counters and their values");
    ...
    _admin_socket->register_command("perf reset name=var,type=CephString",
                                    _admin_hook,
    ...
    _admin_socket->register_command(
        "config help name=var,type=CephString,req=false", _admin_hook, "get config setting schema and descriptions");
    _admin_socket->register_command("config set name=var,type=CephString name=val,type=CephString,n=N",
                                    _admin_hook,
                                    "config set <field> <val> [<val> ...]: set a config variable");
    _admin_socket->register_command(
        "config unset name=var,type=CephString", _admin_hook, "config unset <field>: unset a config variable");
    _admin_socket->register_command(
        "config get name=var,type=CephString", _admin_hook, "config get <field>: get the config value");
    _admin_socket->register_command("config diff", _admin_hook, "dump diff of current config and default config");
    _admin_socket->register_command("config diff get name=var,type=CephString",
                                    _admin_hook,
                                    "dump diff get <field>: dump diff of current and default config setting <field>");
    _admin_socket->register_command("injectargs name=injected_args,type=CephString,n=N",
                                    _admin_hook,
...
void CephContext::put()
{
    if (--nref == 0) {
        ...
        if (g_ceph_context == this) g_ceph_context = nullptr;
        delete this;
    }
    else {
...
void CephContext::start_service_thread()
{
    ...
    if (!(get_init_flags() & CINIT_FLAG_NO_CCT_PERF_COUNTERS)) _enable_perf_counter();

    // make logs flush on_exit()
    if (_conf->log_flush_on_exit) _log->set_flush_on_exit();
    ...
    // start admin socket
    if (_conf->admin_socket.length()) _admin_socket->init(_conf->admin_socket);
}

void CephContext::reopen_logs()
{
    std::lock_guard lg(_service_thread_lock);
    if (_service_thread) _service_thread->reopen_logs();
}
...
void CephContext::join_service_thread()
{
    ...
    if (!(get_init_flags() & CINIT_FLAG_NO_CCT_PERF_COUNTERS)) _disable_perf_counter();
}
...
void CephContext::_enable_perf_counter()
{
    ...
    for (unsigned i = 0; i < mempool::num_pools; ++i) {
        ...
        _mempool_perf_descriptions.push_back("mempool "s + n + " total bytes");
        _mempool_perf_names.push_back(n + "_items"s);
        _mempool_perf_descriptions.push_back("mempool "s + n + " total items"s);
    }

    PerfCountersBuilder plb2(this, "mempool", l_mempool_first, l_mempool_first + 1 + 2 * mempool::num_pools);
    unsigned l = l_mempool_first + 1;
    for (unsigned i = 0; i < mempool::num_pools; ++i) {
        plb2.add_u64(l++, _mempool_perf_names[i * 2].c_str(), _mempool_perf_descriptions[i * 2].c_str());
        plb2.add_u64(l++, _mempool_perf_names[i * 2 + 1].c_str(), _mempool_perf_descriptions[i * 2 + 1].c_str());
    }
...
{
    switch (type) {
    case CEPH_CRYPTO_NONE: return _crypto_none;
    case CEPH_CRYPTO_AES: return _crypto_aes;
    default: return NULL;
    }
}
