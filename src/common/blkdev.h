...
extern std::string get_device_id(const std::string& devname, std::string* err = 0);

// get /dev/disk/by-path/... style device id that is stable for a disk slot across reboots etc
extern std::string get_device_path(const std::string& devname, std::string* err = 0);

// populate daemon metadata map with device info
extern void get_device_metadata(const std::set<std::string>& devnames, std::map<std::string, std::string>* pm,
                                std::map<std::string, std::string>* errs);

extern void get_dm_parents(const std::string& dev, std::set<std::string>* ls);
extern int block_device_get_metrics(const std::string& devname, int timeout, json_spirit::mValue* result);
...
extern void get_raw_devices(const std::string& in, std::set<std::string>* ls);
...
protected:
    ...
    int64_t get_string_property(const char* prop, char* val, size_t maxlen) const;
