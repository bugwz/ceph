...
public:
    ...
    template<class T> static bool decode_json(const char* name, T& val, JSONObj* obj, bool mandatory = false);

    template<class C>
    static bool decode_json(const char* name, C& container, void (*cb)(C&, JSONObj* obj), JSONObj* obj,
                            bool mandatory = false);

    template<class T> static void decode_json(const char* name, T& val, const T& default_val, JSONObj* obj);
...
template<class T> void decode_json_obj(T& val, JSONObj* obj)
{
    val.decode_json(obj);
}
...
template<class T> void decode_json_obj(std::list<T>& l, JSONObj* obj)
{
    l.clear();
...
template<class T> void decode_json_obj(std::deque<T>& l, JSONObj* obj)
{
    l.clear();
...
template<class T> void decode_json_obj(std::set<T>& l, JSONObj* obj)
{
    l.clear();
...
template<class T> void decode_json_obj(std::vector<T>& l, JSONObj* obj)
{
    l.clear();
...
template<class K, class V, class C = std::less<K>> void decode_json_obj(std::map<K, V, C>& m, JSONObj* obj)
{
    m.clear();
...
template<class K, class V> void decode_json_obj(std::multimap<K, V>& m, JSONObj* obj)
{
    m.clear();
...
template<class K, class V> void decode_json_obj(boost::container::flat_map<K, V>& m, JSONObj* obj)
{
    m.clear();
...
template<class C> void decode_json_obj(C& container, void (*cb)(C&, JSONObj* obj), JSONObj* obj)
{
    container.clear();
...
template<class T> bool JSONDecoder::decode_json(const char* name, T& val, JSONObj* obj, bool mandatory)
{
...
template<class T> void JSONDecoder::decode_json(const char* name, T& val, const T& default_val, JSONObj* obj)
{
    JSONObjIter iter = obj->find_first(name);
    if (iter.end()) {
        val = default_val;
...
template<class T> bool JSONDecoder::decode_json(const char* name, boost::optional<T>& val, JSONObj* obj, bool mandatory)
{
    JSONObjIter iter = obj->find_first(name);
    if (iter.end()) {
        if (mandatory) {
...
template<class T> bool JSONDecoder::decode_json(const char* name, std::optional<T>& val, JSONObj* obj, bool mandatory)
{
    JSONObjIter iter = obj->find_first(name);
    if (iter.end()) {
        if (mandatory) {
...
public:
    ...
    template<class T> class Handler : public HandlerBase
    {
    public:
        virtual ~Handler() {}

        std::type_index get_type() override { return std::type_index(typeid(const T&)); }
    };
...
public:
    void register_type(HandlerBase* h) { handlers[h->get_type()] = h; }

    template<class T> bool encode_json(const char* name, const T& val, ceph::Formatter* f)
    {
...
template<class T> static void encode_json_impl(const char* name, const T& val, ceph::Formatter* f)
{
...
template<class T> static void encode_json(const char* name, const T& val, ceph::Formatter* f)
{
    JSONEncodeFilter* filter = static_cast<JSONEncodeFilter*>(f->get_external_feature_handler("JSONEncodeFilter"));

    if (!filter || !filter->encode_json(name, val, f)) {
...
template<class T> static void encode_json(const char* name, const std::list<T>& l, ceph::Formatter* f)
{
...
template<class T> static void encode_json(const char* name, const std::deque<T>& l, ceph::Formatter* f)
{
    f->open_array_section(name);
    for (auto iter = l.cbegin(); iter != l.cend(); ++iter) {
        encode_json("obj", *iter, f);
...
template<class T, class Compare, class Alloc>
static void encode_json(const char* name, const boost::container::flat_set<T, Compare, Alloc>& l, ceph::Formatter* f)
{
    f->open_array_section(name);
    for (auto iter = l.cbegin(); iter != l.cend(); ++iter) {
        encode_json("obj", *iter, f);
...
template<class T> static void encode_json(const char* name, const std::vector<T>& l, ceph::Formatter* f)
{
    f->open_array_section(name);
    for (auto iter = l.cbegin(); iter != l.cend(); ++iter) {
        encode_json("obj", *iter, f);
...
template<class K, class V> static void encode_json(const char* name, const std::multimap<K, V>& m, ceph::Formatter* f)
{
    f->open_array_section(name);
...
template<class K, class V> void encode_json_map(const char* name, const std::map<K, V>& m, ceph::Formatter* f)
{
    f->open_array_section(name);
...
template<class K, class V>
void encode_json_map(const char* name, const char* index_name, const char* object_name, const char* value_name,
                     void (*cb)(const char*, const V&, ceph::Formatter*, void*), void* parent, const std::map<K, V>& m,
                     ceph::Formatter* f)
{
    f->open_array_section(name);
    for (auto iter = m.cbegin(); iter != m.cend(); ++iter) {
...
template<class K, class V>
void encode_json_map(const char* name, const char* index_name, const char* value_name, const std::map<K, V>& m,
                     ceph::Formatter* f)
{
    encode_json_map<K, V>(name, index_name, NULL, value_name, NULL, NULL, m, f);
}

template<class T> static void encode_json(const char* name, const std::optional<T>& o, ceph::Formatter* f)
{
    if (!o) {
        return;
    }
...
public:
    ...
    const std::string& val() const { return value.str; }
    ...
    const std::map<std::string, JSONFormattable> object() const { return obj; }

    const std::vector<JSONFormattable>& array() const { return arr; }
    ...
    operator std::string() const { return value.str; }
    ...
    template<class T> T operator[](const std::string& name) const { return this->operator[](name)(T()); }

    template<class T> T operator[](const std::string& name) { return this->operator[](name)(T()); }
