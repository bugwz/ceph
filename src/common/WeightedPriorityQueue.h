...
{
public:
    bool operator()(const S i, const T& k) const { return i < k.key; }
    bool operator()(const T& k, const S i) const { return k.key < i; }
};

template<typename T> class DelItem
{
public:
    void operator()(T* delete_this) { delete delete_this; }
};
...
private:
    ...
    public:
        ...
        Klass(K& k)
            : key(k)
        {}
        ~Klass() { lp.clear_and_dispose(DelItem<ListPair>()); }
        friend bool operator<(const Klass& a, const Klass& b) { return a.key < b.key; }
        friend bool operator>(const Klass& a, const Klass& b) { return a.key > b.key; }
        friend bool operator==(const Klass& a, const Klass& b) { return a.key == b.key; }
        ...
        bool empty() const { return lp.empty(); }
        unsigned get_size() const { return lp.size(); }
    ...
    public:
        ...
        SubQueue(unsigned& p)
            ...
            , next(klasses.begin())
        {}
        ~SubQueue() { klasses.clear_and_dispose(DelItem<Klass>()); }
        friend bool operator<(const SubQueue& a, const SubQueue& b) { return a.key < b.key; }
        friend bool operator>(const SubQueue& a, const SubQueue& b) { return a.key > b.key; }
        friend bool operator==(const SubQueue& a, const SubQueue& b) { return a.key == b.key; }
        bool empty() const { return klasses.empty(); }
        void insert(K cl, unsigned cost, T&& item, bool front = false)
        {
            typename Klasses::insert_commit_data insert_data;
            std::pair<Kit, bool> ret = klasses.insert_unique_check(cl, MapKey<Klass, K>(), insert_data);
            if (ret.second) {
    ...
    public:
        Queue()
            ...
            , max_cost(0)
        {}
        ~Queue() { queues.clear_and_dispose(DelItem<SubQueue>()); }
        ...
        T pop(bool strict = false)
        {
            ...
            if (queues.size() > 1) {
                while (true) {
                    ...
                    if (max_cost == 0 || rand() % max_cost <= (max_cost - ((i->get_cost() * 9) / 10))) {
                        break;
                    }
                    i = --queues.end();
                }
        ...
        void dump(ceph::Formatter* f) const
        {
            for (typename SubQueues::const_iterator i = queues.begin(); i != queues.end(); ++i) {
...
public:
    ...
    bool empty() const final { return strict.empty() && normal.empty(); }
    void enqueue_strict(K cl, unsigned p, T&& item) final { strict.insert(p, cl, 0, std::move(item)); }
    void enqueue_strict_front(K cl, unsigned p, T&& item) final { strict.insert(p, cl, 0, std::move(item), true); }
    void enqueue(K cl, unsigned p, unsigned cost, T&& item) final { normal.insert(p, cl, cost, std::move(item)); }
    ...
    unsigned get_size_slow() { return strict.get_size_slow() + normal.get_size_slow(); }
    ...
    void print(std::ostream& ostream) const final { ostream << "WeightedPriorityQueue"; }
};
