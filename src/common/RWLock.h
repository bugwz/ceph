...
public:
    ...
    ~RWLock()
    {
        ...
        if (track) ceph_assert(!is_locked());
    ...
    void unlock(bool lockdep = true) const
    {
        ...
        if (lockdep && this->lockdep && g_lockdep) id = lockdep_will_unlock(name.c_str(), id);
    ...
    void put_read() const { unlock(); }
    void lock_shared() { get_read(); }
    void unlock_shared() { put_read(); }
    // write
    void get_write(bool lockdep = true)
    {
        if (lockdep && this->lockdep && g_lockdep) id = lockdep_will_lock(name.c_str(), id);
        int r = pthread_rwlock_wrlock(&L);
        ceph_assert(r == 0);
        if (lockdep && this->lockdep && g_lockdep) id = lockdep_locked(name.c_str(), id);
        if (track) nwlock++;
    }
...
public:
    ...
    public:
        explicit RLocker(const RWLock& lock)
            : m_lock(lock)
        {
            m_lock.get_read();
            locked = true;
    ...
    public:
        explicit Context(RWLock& l)
            : lock(l)
            , state(Untaken)
        {}
        Context(RWLock& l, LockState s)
            : lock(l)
            , state(s)
        {}
        ...
        void set_state(LockState s) { state = s; }

        bool is_locked() { return (state != Untaken); }

        bool is_rlocked() { return (state == TakenForRead); }

        bool is_wlocked() { return (state == TakenForWrite); }
    };
};
