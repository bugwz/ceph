...
public:
    enum CSumType
    {
        CSUM_NONE = 1,   // intentionally set to 1 to be aligned with OSDMnitor's pool_opts_t handling - it treats 0 as
                         // unset while we need to distinguish none and unset cases
        CSUM_XXHASH32 = 2,
        CSUM_XXHASH64 = 3,
        CSUM_CRC32C = 4,
    ...
    {
        if (s == "none") return CSUM_NONE;
        if (s == "xxhash32") return CSUM_XXHASH32;
        if (s == "xxhash64") return CSUM_XXHASH64;
        if (s == "crc32c") return CSUM_CRC32C;
        if (s == "crc32c_16") return CSUM_CRC32C_16;
        if (s == "crc32c_8") return CSUM_CRC32C_8;
        return -EINVAL;
    }
    ...
    struct crc32c
    {
        ...
        static init_value_t calc(state_t state, init_value_t init_value, size_t len,
    ...
    struct xxhash32
    {
        ...
        static void init(state_t* s) { *s = XXH32_createState(); }
        static void fini(state_t* s) { XXH32_freeState(*s); }
    ...
    struct xxhash64
    {
        ...
        static void init(state_t* s) { *s = XXH64_createState(); }
        static void fini(state_t* s) { XXH64_freeState(*s); }
    ...
    template<class Alg>
    static int calculate(size_t csum_block_size, size_t offset, size_t length, const ceph::buffer::list& bl,
    ...
    template<class Alg>
    static int calculate(typename Alg::init_value_t init_value, size_t csum_block_size, size_t offset, size_t length,
                         const ceph::buffer::list& bl, ceph::buffer::ptr* csum_data)
    {
        ...
        ceph_assert(csum_data->length() >= (offset + length) / csum_block_size * sizeof(typename Alg::value_t));

        typename Alg::value_t* pv = reinterpret_cast<typename Alg::value_t*>(csum_data->c_str());
    ...
    template<class Alg>
    static int verify(size_t csum_block_size, size_t offset, size_t length, const ceph::buffer::list& bl,
                      const ceph::buffer::ptr& csum_data, uint64_t* bad_csum = 0)
    {
        ceph_assert(length % csum_block_size == 0);
        ceph::buffer::list::const_iterator p = bl.begin();
        ceph_assert(bl.length() >= length);
        ...
        const typename Alg::value_t* pv = reinterpret_cast<const typename Alg::value_t*>(csum_data.c_str());
        pv += offset / csum_block_size;
        size_t pos = offset;
        while (length > 0) {
