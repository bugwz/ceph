...
optional<real_time> calculate(const tm& t, uint32_t n = 0)
{
    ...
    return boost::make_optional<real_time>(real_clock::from_time_t(tt) + nanoseconds(n));
}
}   // namespace

optional<real_time> from_iso_8601(const string_view s, const bool ws_terminates) noexcept
{
    auto end = s.cend();
    ...
    auto partial_date = [end, ws_terminates](sriter& c) { return (c == end || (ws_terminates && std::isspace(*c))); };
    auto time_end = [end, ws_terminates](sriter& c) {
        return (c != end && *c == 'Z' && ((c + 1) == end || (ws_terminates && std::isspace(*(c + 1)))));
    };
...
string to_iso_8601(const real_time t, const iso_8601_format f, std::string_view date_separator,
                   std::string_view time_separator) noexcept
{
    ...
    auto nsec = duration_cast<nanoseconds>(t.time_since_epoch() % seconds(1)).count();
