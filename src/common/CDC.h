...
public:
    ...
    virtual void calc_chunks(const bufferlist& inputdata, std::vector<std::pair<uint64_t, uint64_t>>* chunks) const = 0;
    ...
    static std::unique_ptr<CDC> create(const std::string& type, int bits, int windowbits = 0);
};
