...
#ifndef _WIN32
    if (fcntl(sv[0], F_SETFD, FD_CLOEXEC) < 0) goto fail;

    if (fcntl(sv[1], F_SETFD, FD_CLOEXEC) < 0) goto fail;
#endif
...
int _win_socketpair(int socks[2])
{
    ...
    do {
        if (setsockopt(listener, SOL_SOCKET, SO_REUSEADDR, (char*)&reuse, (socklen_t)sizeof(reuse)) == -1) break;
        ...
        if (s[0] == INVALID_SOCKET) break;
        if (connect(s[0], &a.addr, sizeof(a.inaddr)) == SOCKET_ERROR) break;
        s[1] = accept(listener, NULL, NULL);
        if (s[1] == INVALID_SOCKET) break;
