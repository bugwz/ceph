...
void PerfCountersCollection::dump_formatted(ceph::Formatter* f, bool schema, const std::string& logger,
                                            const std::string& counter)
{
    std::lock_guard lck(m_lock);
    perf_impl.dump_formatted(f, schema, logger, counter);
}
void PerfCountersCollection::dump_formatted_histograms(ceph::Formatter* f, bool schema, const std::string& logger,
                                                       const std::string& counter)
{
    std::lock_guard lck(m_lock);
...
{
    if (cct) cct->get_perfcounters_collection()->remove(p);
    delete p;
}
