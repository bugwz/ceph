...
long long strict_strtoll(std::string_view str, int base, std::string* err)
{
    ...
    if (endptr == str.data() || endptr != str.data() + str.size()) {
        *err = (std::string{"Expected option value to be integer, got '"} + std::string{str} + "'");
        return 0;
    }
    if (errno) {
        *err = (std::string{"The option value '"} + std::string{str} + "' seems to be invalid");
        return 0;
    }
    *err = "";
    return ret;
...
double strict_strtod(std::string_view str, std::string* err)
{
    ...
    if (errno == ERANGE) {
        ostringstream oss;
        oss << "strict_strtod: floating point overflow or underflow parsing '" << str << "'";
        *err = oss.str();
        return 0.0;
    }
    if (endptr == str) {
...
template<typename T> T strict_iec_cast(std::string_view str, std::string* err)
{
    if (str.empty()) {
    ...
    if (u != std::string_view::npos) {
        ...
        case 'E': m = 60; break;
        case 'B': break;
        default: *err = "strict_iecstrtoll: unit prefix not recognized"; return 0;
        }
    }
...
template<typename T> T strict_iec_cast(const char* str, std::string* err)
{
...
template<typename T> T strict_si_cast(std::string_view str, std::string* err)
{
    if (str.empty()) {
    ...
    if (str.find_first_not_of("0123456789+-") != std::string_view::npos) {
        ...
        if (m >= 3) n = str.substr(0, str.length() - 1);
    }
    ...
    if (v != std::clamp(v,
                        (static_cast<promoted_t>(std::numeric_limits<T>::min()) / coefficient),
                        (static_cast<promoted_t>(std::numeric_limits<T>::max()) / coefficient))) {
...
template<typename T> T strict_si_cast(const char* str, std::string* err)
{
