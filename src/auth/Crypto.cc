...
CryptoRandom::CryptoRandom()
    : fd(getentropy_works() ? -1 : open_urandom())
{}
...
void CryptoRandom::get_bytes(char* buf, int len)
{
    auto ret = BCryptGenRandom(NULL, (unsigned char*)buf, len, BCRYPT_USE_SYSTEM_PREFERRED_RNG);
    if (ret != 0) {
                                      ...
                                      const CryptoKeyHandler::out_slice_t& out) const
{
    ...
    const auto todo_len = std::min<std::size_t>(ciphertext.length(), out.max_length);
                                      ...
                                      const CryptoKeyHandler::out_slice_t& out) const
{
    ...
    // we need to specify the template parameter explicitly as ::length()
    // returns unsigned int, not size_t.
    const auto todo_len = std::min<std::size_t>(plaintext.length(), out.max_length);
    memcpy(out.buf, plaintext.c_str(), todo_len);

    return todo_len;
}

sha256_digest_t CryptoKeyHandler::hmac_sha256(const ceph::bufferlist& in) const
{
...
public:
    ...
    int encrypt(const bufferlist& in, bufferlist& out, std::string* error) const override
    {
        out = in;
        return 0;
    }
    int decrypt(const bufferlist& in, bufferlist& out, std::string* error) const override
    {
        out = in;
        return 0;
    }
...
public:
    CryptoNone() {}
    ~CryptoNone() override {}
    int get_type() const override { return CEPH_CRYPTO_NONE; }
    int create(CryptoRandom* random, bufferptr& secret) override { return 0; }
...
public:
    CryptoAES() {}
    ~CryptoAES() override {}
    int get_type() const override { return CEPH_CRYPTO_AES; }
...
public:
    ...
    int init(const bufferptr& s, ostringstream& err)
    {
        ...
        const int enc_key_ret =
            AES_set_encrypt_key((const unsigned char*)secret.c_str(), AES_KEY_LEN * CHAR_BIT, &enc_key);
        if (enc_key_ret != 0) {
        ...
        const int dec_key_ret =
            AES_set_decrypt_key((const unsigned char*)secret.c_str(), AES_KEY_LEN * CHAR_BIT, &dec_key);
        if (dec_key_ret != 0) {
    ...
    int encrypt(const ceph::bufferlist& in, ceph::bufferlist& out, std::string* /* unused */) const override
    {
        ...
        ceph::bufferptr out_tmp{
            static_cast<unsigned>(AES_BLOCK_LEN + p2align<std::size_t>(in.length(), AES_BLOCK_LEN))};
        ...
        AES_cbc_encrypt(
            in_buf, reinterpret_cast<unsigned char*>(out_tmp.c_str()), out_tmp.length(), &enc_key, iv, AES_ENCRYPT);

        out.append(out_tmp);
        return 0;
    }

    int decrypt(const ceph::bufferlist& in, ceph::bufferlist& out, std::string* /* unused */) const override
    {
        ...
        AES_cbc_encrypt(
            in_buf, reinterpret_cast<unsigned char*>(out_tmp.c_str()), in.length(), &dec_key, iv, AES_DECRYPT);
        ...
        const auto pad_len = std::min<std::uint8_t>(out_tmp[in.length() - 1], AES_BLOCK_LEN);
    ...
    std::size_t encrypt(const in_slice_t& in, const out_slice_t& out) const override
    {
        if (out.buf == nullptr) {
            // 16 + p2align(10, 16) -> 16
            // 16 + p2align(16, 16) -> 32
        ...
        const std::size_t main_encrypt_size = std::min(in.length - tail_len, out.max_length);
        AES_cbc_encrypt(in.buf, out.buf, main_encrypt_size, &enc_key, iv.data(), AES_ENCRYPT);

        const std::size_t tail_encrypt_size = std::min(AES_BLOCK_LEN, out.max_length - main_encrypt_size);
        AES_cbc_encrypt(
            last_block.data(), out.buf + main_encrypt_size, tail_encrypt_size, &enc_key, iv.data(), AES_ENCRYPT);

        return main_encrypt_size + tail_encrypt_size;
    }

    std::size_t decrypt(const in_slice_t& in, const out_slice_t& out) const override
    {
        ...
        AES_cbc_encrypt(in.buf, out.buf, in.length, &dec_key, iv.data(), AES_DECRYPT);
        ...
        const auto pad_len = std::min<std::uint8_t>(out.buf[in.length - 1], AES_BLOCK_LEN);
...
CryptoKeyHandler* CryptoAES::get_key_handler(const bufferptr& secret, string& error)
{
...
void CryptoKey::decode(bufferlist::const_iterator& bl)
{
    ...
    if (_set_secret(type, tmp) < 0) throw ceph::buffer::malformed_input("malformed secret");
}
...
int CryptoKey::create(CephContext* cct, int t)
{
    CryptoHandler* ch = CryptoHandler::create(t);
    if (!ch) {
        if (cct) lderr(cct) << "ERROR: cct->get_crypto_handler(type=" << t << ") returned NULL" << dendl;
        return -EOPNOTSUPP;
    }
    bufferptr s;
...
{
    switch (type) {
    case CEPH_CRYPTO_NONE: return new CryptoNone;
    case CEPH_CRYPTO_AES: return new CryptoAES;
    default: return NULL;
    }
}
