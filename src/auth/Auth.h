...
struct AuthAuthorizer
{
    ...
    explicit AuthAuthorizer(__u32 p)
        : protocol(p)
    {}
    virtual ~AuthAuthorizer() {}
    virtual bool verify_reply(ceph::buffer::list::const_iterator& reply, std::string* connection_secret) = 0;
    virtual bool add_challenge(CephContext* cct, const ceph::buffer::list& challenge) = 0;
};
...
struct AuthConnectionMeta
{
    ...
    bool is_mode_crc() const { return con_mode == CEPH_CON_MODE_CRC; }
    bool is_mode_secure() const { return con_mode == CEPH_CON_MODE_SECURE; }
    ...
    size_t get_connection_secret_length() const
    {
        ...
        case CEPH_CON_MODE_SECURE: return 16 * 4;
        }
        return 0;
    }
...
struct RotatingSecrets
{
    ...
    {
        secrets[++max_ver] = key;
        while (secrets.size() > KEY_ROTATE_NUM) secrets.erase(secrets.begin());
        return max_ver;
    }

    bool need_new_secrets() const { return secrets.size() < KEY_ROTATE_NUM; }
    bool need_new_secrets(const utime_t& now) const
    {
        return secrets.size() < KEY_ROTATE_NUM || current().expiration <= now;
    }

    ExpiringCryptoKey& previous() { return secrets.begin()->second; }
    ...
    ExpiringCryptoKey& next() { return secrets.rbegin()->second; }
    bool empty() { return secrets.empty(); }
...
public:
    ...
    virtual bool get_service_secret(uint32_t service_id, uint64_t secret_id, CryptoKey& secret) const = 0;
};
