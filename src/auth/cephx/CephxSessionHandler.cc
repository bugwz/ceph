...
int CephxSessionHandler::_calc_signature(Message* m, uint64_t* psig)
{
    ...
    if (!HAVE_FEATURE(features, CEPHX_V2)) {
        ...
        struct
        {
            ...
            ceph_le32 data_crc;
        } __attribute__((packed)) sigblock = {1,
                                              init_le64(AUTH_ENC_MAGIC),
                                              init_le32(4 * 4),
        ...
        try {
            const CryptoKey::in_slice_t in{sizeof(sigblock), reinterpret_cast<const unsigned char*>(&sigblock)};
            const CryptoKey::out_slice_t out{sizeof(exp_buf), reinterpret_cast<unsigned char*>(&exp_buf)};
        ...
        *psig = *reinterpret_cast<ceph_le64*>(exp_buf);
        ...
        } __attribute__((packed)) sigblock = {init_le32(header.crc),
                                              init_le32(footer.front_crc),
    ...
    ldout(cct, 10) << __func__ << " seq " << m->get_seq() << " front_crc_ = " << footer.front_crc
                   << " middle_crc = " << footer.middle_crc << " data_crc = " << footer.data_crc << " sig = " << *psig
                   << dendl;
    return 0;
}
