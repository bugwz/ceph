...
int CephxClientHandler::build_request(bufferlist& bl) const
{
    ...
    if (need & CEPH_ENTITY_TYPE_AUTH) {
        ...
        std::string error;
        cephx_calc_client_server_challenge(cct, secret, server_challenge, req.client_challenge, &req.key, error);
        if (!error.empty()) {
        ...
        ldout(cct, 10) << "get auth session key: client_challenge " << std::hex << req.client_challenge << std::dec
                       << dendl;
        return 0;
    }
...
int CephxClientHandler::handle_response(int ret, bufferlist::const_iterator& indata, CryptoKey* session_key,
                                        std::string* connection_secret)
{
    ldout(cct, 10) << this << " handle_response ret = " << ret << dendl;

    if (ret < 0) return ret;   // hrm!
    ...
    case CEPHX_GET_AUTH_SESSION_KEY:
    {
        ...
        if (!indata.end()) {
            ...
            ldout(cct, 10) << " got connection bl " << cbl.length() << " and extra tickets " << extra_tickets.length()
                           << dendl;
            ...
            if (session_key && connection_secret) {
                ...
                if (cbl.length() && connection_secret) {
                    ...
                    if (decode_decrypt(cct, *connection_secret, *session_key, p, err)) {
                        lderr(cct) << __func__ << " failed to decrypt connection_secret" << dendl;
                    }
                    else {
    ...
    default: ldout(cct, 0) << " unknown request_type " << header.request_type << dendl; ceph_abort();
    }
    return ret;
}
