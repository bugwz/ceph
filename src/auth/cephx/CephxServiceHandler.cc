                                        ...
                                        std::string* pconnection_secret)
{
    ...
    case CEPHX_GET_AUTH_SESSION_KEY:
    {
        ...
        std::string error;
        cephx_calc_client_server_challenge(cct, secret, server_challenge, req.client_challenge, &expected_key, error);
        if (!error.empty()) {
        ...
        if (!cephx_build_service_ticket_reply(
                cct, eauth.key, info_vec, should_enc_ticket, old_ticket_info.session_key, *result_bl)) {
            ret = -EIO;
            break;
        }
