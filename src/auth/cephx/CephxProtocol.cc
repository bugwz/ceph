                                        ...
                                        uint64_t client_challenge, uint64_t* key, std::string& error)
{
    ...
    for (int pos = 0; pos + sizeof(k) <= enc.length(); pos += sizeof(k), p++) k ^= *p;
    *key = k;
}
...
bool cephx_build_service_ticket_blob(CephContext* cct, CephXSessionAuthInfo& info, CephXTicketBlob& blob)
{
    ...
    ldout(cct, 10) << "build_service_ticket service " << ceph_entity_type_name(info.service_id) << " secret_id "
                   << info.secret_id << " ticket_info.ticket.name=" << ticket_info.ticket.name.to_str()
 ...
 */
bool cephx_build_service_ticket_reply(CephContext* cct, CryptoKey& principal_secret,
                                      vector<CephXSessionAuthInfo> ticket_info_vec, bool should_encrypt_ticket,
                                      CryptoKey& ticket_enc_key, bufferlist& reply)
{
    ...
    encode(num, reply);
    ldout(cct, 10) << "build_service_ticket_reply encoding " << num << " tickets with secret " << principal_secret
                   << dendl;

    for (auto ticket_iter = ticket_info_vec.begin(); ticket_iter != ticket_info_vec.end(); ++ticket_iter) {
 ...
 */
bool CephXTicketHandler::verify_service_ticket_reply(CryptoKey& secret, bufferlist::const_iterator& indata)
{
    using ceph::decode;
    try {
        ...
        ldout(cct, 10) << __func__ << " service " << ceph_entity_type_name(service_id) << " secret_id "
                       << ticket.secret_id << " session_key " << msg_a.session_key << " validity=" << msg_a.validity
                       << dendl;
...
bool CephXTicketManager::have_key(uint32_t service_id)
{
    ...
    if (iter == tickets_map.end()) return false;
...
{
    auto iter = tickets_map.find(service_id);
    if (iter == tickets_map.end()) return true;
...
{
    auto iter = tickets_map.find(service_id);
    if (iter != tickets_map.end()) iter->second.invalidate_ticket();
}
 ...
 * this ServiceTicket with the result.
 */
bool CephXTicketManager::verify_service_ticket_reply(CryptoKey& secret, bufferlist::const_iterator& indata)
{
...
bool cephx_decode_ticket(CephContext* cct, KeyStore* keys, uint32_t service_id, const CephXTicketBlob& ticket_blob,
                         CephXServiceTicketInfo& ticket_info)
{
 ...
 */
bool cephx_verify_authorizer(CephContext* cct, const KeyStore& keys, bufferlist::const_iterator& indata,
                             size_t connection_secret_required_len, CephXServiceTicketInfo& ticket_info,
                             std::unique_ptr<AuthAuthorizerChallenge>* challenge, std::string* connection_secret,
                             bufferlist* reply_bl)
{
    __u8 authorizer_v;
    uint32_t service_id;
    ...
    ldout(cct, 10) << "verify_authorizer decrypted service " << ceph_entity_type_name(service_id)
    ...
    if (ticket.secret_id == (uint64_t)-1) {
        ...
        if (!keys.get_service_secret(service_id, ticket.secret_id, service_secret)) {
            ldout(cct, 0) << "verify_authorizer could not get service secret for service "
                          << ceph_entity_type_name(service_id) << " secret_id=" << ticket.secret_id << dendl;
            if (cct->_conf->auth_debug && ticket.secret_id == 0) ceph_abort_msg("got secret_id=0");
            return false;
        }
    }
    std::string error;
    ...
    if (challenge) {
        ...
        ldout(cct, 10) << __func__ << " got server_challenge+1 " << auth_msg.server_challenge_plus_one << " expecting "
                       << c->server_challenge + 1 << dendl;
    ...
    if (connection_secret) {
        ...
        if (connection_secret_required_len) {
            ...
            std::generate_n(
                connection_secret->data(), connection_secret_required_len, std::default_random_engine{rd()});
#else
            cct->random()->get_bytes(connection_secret->data(), connection_secret_required_len);
#endif
        }
...
bool CephXAuthorizer::verify_reply(bufferlist::const_iterator& indata, std::string* connection_secret)
{
    CephXAuthorizeReply reply;
    ...
    if (connection_secret && reply.connection_secret.size()) {
        *connection_secret = reply.connection_secret;
    }
    return true;
}

bool CephXAuthorizer::add_challenge(CephContext* cct, const bufferlist& challenge)
{
    bl = base_bl;
