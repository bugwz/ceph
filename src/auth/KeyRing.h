...
class KeyRing : public KeyStore
{
    ...
    int set_modifier(const char* type, const char* val, EntityName& name,
...
public:
    ...
    bool get_service_secret(uint32_t service_id, uint64_t secret_id, CryptoKey& secret) const override { return false; }
    ...
    // modifiers
    void add(const EntityName& name, const EntityAuth& a) { keys[name] = a; }
    ...
    void remove(const EntityName& name) { keys.erase(name); }
    void set_caps(EntityName& name, std::map<std::string, ceph::buffer::list>& caps) { keys[name].caps = caps; }
    void set_key(EntityName& ename, CryptoKey& key) { keys[ename].key = key; }
