...
int KeyRing::from_ceph_context(CephContext* cct)
{
    ...
    if (!ret) {
        ...
        if (ret < 0) lderr(cct) << "failed to load " << filename << ": " << cpp_strerror(ret) << dendl;
    }
    else if (conf->key.empty() && conf->keyfile.empty()) {
        lderr(cct) << "unable to find a keyring on " << conf->keyring << ": " << cpp_strerror(ret) << dendl;
    }
...
void KeyRing::encode_formatted(string label, Formatter* f, bufferlist& bl)
{
    ...
    for (map<EntityName, EntityAuth>::iterator p = keys.begin(); p != keys.end(); ++p) {
        ...
        for (map<string, bufferlist>::iterator q = p->second.caps.begin(); q != p->second.caps.end(); ++q) {
...
void KeyRing::decode_plaintext(bufferlist::const_iterator& bli)
{
    ...
    for (auto& [name, section] : cf) {
        ...
        for (auto& [k, val] : section) {
            ...
            if (ret < 0) {
                ostringstream oss;
                oss << "error setting modifier for [" << name << "] type=" << key << " val=" << val;
                throw ceph::buffer::malformed_input(oss.str().c_str());
            }
        }
    }
...
{
    for (map<EntityName, EntityAuth>::iterator p = other.keys.begin(); p != other.keys.end(); ++p) {
