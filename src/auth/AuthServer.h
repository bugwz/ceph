...
public:
    ...
    virtual void get_supported_auth_methods(int peer_type, std::vector<uint32_t>* methods,
    ...
    virtual uint32_t pick_con_mode(int peer_type, uint32_t auth_method, const std::vector<uint32_t>& preferred_modes)
    {
    ...
    virtual int handle_auth_request(Connection* con, AuthConnectionMeta* auth_meta,
                                    bool more,   ///< true if this is not the first part of the handshake
                                    uint32_t auth_method, const ceph::buffer::list& bl, ceph::buffer::list* reply) = 0;
};
