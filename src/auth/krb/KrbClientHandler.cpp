...
AuthAuthorizer* KrbClientHandler::build_authorizer(uint32_t service_id) const
{
...
int KrbClientHandler::build_request(bufferlist& buff_list) const
{
    ...
    krb_request.m_request_type = static_cast<int>(GSSAuthenticationRequest::GSS_TOKEN);
...
int KrbClientHandler::handle_response(int ret, bufferlist::const_iterator& buff_list, CryptoKey* session_key,
                                      std::string* connection_secret)
{
    ...
    OM_uint32 gss_minor_status(0);
    OM_uint32 gss_wanted_flags(GSS_C_MUTUAL_FLAG | GSS_C_INTEG_FLAG);
    ...
    if (m_gss_credentials == GSS_C_NO_CREDENTIAL) {
        ...
        if (cct->_conf->name.get_type() == CEPH_ENTITY_TYPE_CLIENT) {
            gss_major_status = gss_import_name(&gss_minor_status, &gss_buffer_in, krb_client_type, &m_gss_client_name);
            if (gss_major_status != GSS_S_COMPLETE) {
                auto status_str(gss_auth_show_status(gss_major_status, gss_minor_status));
                ldout(cct, 0) << "ERROR: KrbClientHandler::handle_response() "
        ...
        if (gss_major_status != GSS_S_COMPLETE) {
            auto status_str(gss_auth_show_status(gss_major_status, gss_minor_status));
            ldout(cct, 20) << "ERROR: KrbClientHandler::handle_response() "
        ...
        std::string gss_target_name(cct->_conf.get_val<std::string>("gss_target_name"));
        krb_input_name_buff.length = gss_target_name.length();
        krb_input_name_buff.value = (const_cast<char*>(gss_target_name.c_str()));

        gss_major_status =
            gss_import_name(&gss_minor_status, &krb_input_name_buff, krb_input_type, &m_gss_service_name);
        if (gss_major_status != GSS_S_COMPLETE) {
            auto status_str(gss_auth_show_status(gss_major_status, gss_minor_status));
            ldout(cct, 0) << "ERROR: KrbClientHandler::handle_response() "
    ...
    case GSS_S_CONTINUE_NEEDED:
        ldout(cct, 20) << "KrbClientHandler::handle_response() : "
    ...
    case GSS_S_COMPLETE:
        ldout(cct, 20) << "KrbClientHandler::handle_response() : "
                          "[gss_init_sec_context(GSS_S_COMPLETE)] "
                       << dendl;
        result = 0;
        break;
