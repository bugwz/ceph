...
*/
static const gss_OID_desc GSS_API_KRB5_OID_PTR = {9, (void*)"\052\206\110\206\367\022\001\002\002"};
...
*/
static const gss_OID_desc GSS_API_SPNEGO_OID_PTR = {6, (void*)"\x2b\x06\x01\x05\x05\x02"};
...
public:
    KrbAuthorizer()
        : AuthAuthorizer(CEPH_AUTH_GSS)
    {}
    ~KrbAuthorizer() = default;
    bool build_authorizer(const EntityName& entity_name, const uint64_t guid)
    {
        uint8_t value = (1);
    ...
    bool verify_reply(bufferlist::const_iterator& buff_list, std::string* connection_secret) override { return true; }
    bool add_challenge(CephContext* ceph_ctx, const bufferlist& buff_list) override { return true; }
};
...
std::string gss_auth_show_status(const OM_uint32 gss_major_status, const OM_uint32 gss_minor_status);
