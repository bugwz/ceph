...
std::string gss_auth_show_status(const OM_uint32 gss_major_status, const OM_uint32 gss_minor_status)
{
    ...
    const auto gss_complete_status_str_format = [&](const uint32_t gss_status) {
        if (gss_status == GSS_S_COMPLETE) {
            std::string str_tmp("");
            str_tmp.append(reinterpret_cast<char*>(gss_str_status.value), gss_str_status.length);
            str_tmp += STR_DOT;
    ...
    while (gss_ctx_message != 0) {
        gss_maj_status = gss_display_status(
            &gss_min_status, gss_major_status, GSS_C_GSS_CODE, GSS_C_NO_OID, &gss_ctx_message, &gss_str_status);
