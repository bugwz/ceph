...
FS* FS::create(uint64_t f_type)
{
    switch (f_type) {
#ifdef HAVE_LIBXFS
    case XFS_SUPER_MAGIC: return new XFS;
#endif
    default: return new FS;
    }
}
...
int FS::copy_file_range(int to_fd, uint64_t to_offset, int from_fd, uint64_t from_offset, uint64_t from_len)
{
