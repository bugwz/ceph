...
public:
    ...
    struct Onode
    {
        ...
        void clear_pending_stripes() { pending_stripes.clear(); }
    };
    ...
    struct OnodeHashLRU
    {
        ...
        typedef boost::intrusive::list<
            Onode, boost::intrusive::member_hook<Onode, boost::intrusive::list_member_hook<>, &Onode::lru_item>>
            lru_list_t;
    ...
    struct Collection : public CollectionImpl
    {
        ...
        ceph::shared_mutex lock = ceph::make_shared_mutex("KStore::Collection::lock", true, false);
    ...
    {
        KStore* store;
        explicit KVSyncThread(KStore* s)
            : store(s)
        {}
        void* entry() override
        {
...
private:
    ...
    void _do_write_stripe(TransContext* txc, OnodeRef o, uint64_t offset, ceph::buffer::list& bl);
    void _do_remove_stripe(TransContext* txc, OnodeRef o, uint64_t offset);

    int _collection_list(Collection* c, const ghobject_t& start, const ghobject_t& end, int max,
...
public:
    ...
    std::string get_type() override { return "kstore"; }
    ...
    void get_db_statistics(ceph::Formatter* f) override { db->get_statistics(f); }
    ...
    int _do_read(OnodeRef o, uint64_t offset, size_t len, ceph::buffer::list& bl, bool do_cache, uint32_t op_flags = 0);
    ...
    int fiemap(CollectionHandle& c, const ghobject_t& oid, uint64_t offset, size_t len,
               ceph::buffer::list& outbl) override;
...
private:
    ...
    int _do_write(TransContext* txc, OnodeRef o, uint64_t offset, uint64_t length, ceph::buffer::list& bl,
                  uint32_t fadvise_flags);
    ...
    int _do_truncate(TransContext* txc, OnodeRef o, uint64_t offset);
    ...
    int _omap_rmkeys(TransContext* txc, CollectionRef& c, OnodeRef& o, const ceph::buffer::list& bl);
    int _omap_rmkey_range(TransContext* txc, CollectionRef& c, OnodeRef& o, const std::string& first,
                          const std::string& last);
    int _setallochint(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t expected_object_size,
    ...
    int _create_collection(TransContext* txc, coll_t cid, unsigned bits, CollectionRef* c);
