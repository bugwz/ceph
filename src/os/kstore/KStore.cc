...
static void get_coll_key_range(const coll_t& cid, int bits, string* temp_start, string* temp_end, string* start,
                               string* end)
{
    temp_start->clear();
    temp_end->clear();
    ...
    if (cid.is_pg(&pgid)) {
        ...
        *temp_end = *start;
        ...
        uint64_t end_hash = hobject_t::_reverse_bits(pgid.ps()) + (1ull << (32 - bits));
...
void KStore::Onode::flush()
{
    ...
    while (!flush_txns.empty()) flush_cond.wait(l);
...
void KStore::OnodeHashLRU::rename(const ghobject_t& old_oid, const ghobject_t& new_oid)
{
    std::lock_guard<std::mutex> l(lock);
...
bool KStore::OnodeHashLRU::get_next(const ghobject_t& after, pair<ghobject_t, OnodeRef>* next)
{
    std::lock_guard<std::mutex> l(lock);
...
int KStore::OnodeHashLRU::trim(int max)
{
    ...
    if (onode_map.size() == 0 || num <= 0) return 0;   // don't even try
    ...
    while (num > 0) {
        ...
        if (refs > 1) {
            dout(20) << __func__ << "  " << o->oid << " has " << refs << " refs; stopping with " << num
                     << " left to trim" << dendl;
            break;
        }
...
KStore::OnodeRef KStore::Collection::get_onode(const ghobject_t& oid, bool create)
{
...
{
    // XXX
    PerfCountersBuilder b(cct, "KStore", l_kstore_first, l_kstore_last);
...
int KStore::_open_db(bool create)
{
    ...
    string options;
    if (kv_backend == "rocksdb") options = cct->_conf->kstore_rocksdb_options;
    db->init(options);
    stringstream err;
    if (create)
...
void KStore::_sync()
{
    ...
    while (!kv_committing.empty() || !kv_queue.empty()) {
...
int KStore::pool_statfs(uint64_t pool_id, struct store_statfs_t* buf, bool* per_pool_omap)
{
    return -ENOTSUP;
}
        ...
        dout(10) << __func__ << " " << c->cid << dendl;
        {
            ...
            while (c->onode_map.get_next(next.first, &next)) {
                ...
                if (!next.second->flush_txns.empty()) {
                    dout(10) << __func__ << " " << c->cid << " " << next.second->oid << " flush_txns "
                             << next.second->flush_txns << dendl;
                    return;
                }
            }
        }
...
bool KStore::exists(CollectionHandle& ch, const ghobject_t& oid)
{
    ...
    if (!o || !o->exists) return false;
    return true;
}

int KStore::stat(CollectionHandle& ch, const ghobject_t& oid, struct stat* st, bool allow_eio)
{
    dout(10) << __func__ << " " << ch->cid << " " << oid << dendl;
    Collection* c = static_cast<Collection*>(ch.get());
    std::shared_lock l{c->lock};
...
int KStore::set_collection_opts(CollectionHandle& ch, const pool_opts_t& opts)
{
    return -EOPNOTSUPP;
}

int KStore::read(CollectionHandle& ch, const ghobject_t& oid, uint64_t offset, size_t length, bufferlist& bl,
                 uint32_t op_flags)
{
...
int KStore::_do_read(OnodeRef o, uint64_t offset, size_t length, bufferlist& bl, bool do_cache, uint32_t op_flags)
{
    int r = 0;
    uint64_t stripe_size = o->onode.stripe_size;
    uint64_t stripe_off;

    dout(20) << __func__ << " " << offset << "~" << length << " size " << o->onode.size << " nid " << o->onode.nid
             << dendl;
    bl.clear();
...
int KStore::fiemap(CollectionHandle& ch, const ghobject_t& oid, uint64_t offset, size_t len, bufferlist& bl)
{
...
int KStore::fiemap(CollectionHandle& ch, const ghobject_t& oid, uint64_t offset, size_t len,
...
int KStore::getattr(CollectionHandle& ch, const ghobject_t& oid, const char* name, bufferptr& value)
{
...
int KStore::getattrs(CollectionHandle& ch, const ghobject_t& oid, map<string, bufferptr, less<>>& aset)
{
...
int KStore::collection_list(CollectionHandle& c_, const ghobject_t& start, const ghobject_t& end, int max,
...
int KStore::_collection_list(Collection* c, const ghobject_t& start, const ghobject_t& end, int max,
                             vector<ghobject_t>* ls, ghobject_t* pnext)
{
    int r = 0;
    ...
    dout(20) << __func__ << " range " << pretty_binary_string(temp_start_key) << " to "
             << pretty_binary_string(temp_end_key) << " and " << pretty_binary_string(start_key) << " to "
...
int KStore::omap_get(CollectionHandle& ch,          ///< [in] Collection containing oid
...
int KStore::omap_get_header(CollectionHandle& ch,    ///< [in] Collection containing oid
                            const ghobject_t& oid,   ///< [in] Object containing omap
                            bufferlist* header,      ///< [out] omap header
...
int KStore::omap_get_keys(CollectionHandle& ch,    ///< [in] Collection containing oid
                          const ghobject_t& oid,   ///< [in] Object containing omap
...
int KStore::omap_get_values(CollectionHandle& ch,          ///< [in] Collection containing oid
                            const ghobject_t& oid,         ///< [in] Object containing omap
...
int KStore::omap_check_keys(CollectionHandle& ch,      ///< [in] Collection containing oid
                            const ghobject_t& oid,     ///< [in] Object containing omap
...
ObjectMap::ObjectMapIterator KStore::get_omap_iterator(CollectionHandle& ch,   ///< [in] collection
...
void KStore::_osr_reap_done(OpSequencer* osr)
{
    ...
    while (!osr->q.empty()) {
        ...
        if (osr->q.empty()) dout(20) << __func__ << " osr " << osr << " q now empty" << dendl;
    }
}
...
void KStore::_kv_sync_thread()
{
    ...
    while (true) {
        ...
        if (kv_queue.empty()) {
            ...
            if (!cct->_conf->kstore_sync_submit_transaction) {
                for (std::deque<TransContext*>::iterator it = kv_committing.begin(); it != kv_committing.end(); ++it) {
...
int KStore::queue_transactions(CollectionHandle& ch, vector<Transaction>& tls, TrackedOpRef op,
...
void KStore::_txc_add_transaction(TransContext* txc, Transaction* t)
{
    ...
    *_dout << dendl;
    ...
    for (vector<coll_t>::iterator p = i.colls.begin(); p != i.colls.end(); ++p, ++j) {
        ...
        if (!j && !txc->first_collection) txc->first_collection = cvec[j];
    }
    ...
    for (int pos = 0; i.have_op(); ++pos) {
        ...
        if (r < 0) {
            derr << " error " << cpp_strerror(r) << " not handled on operation " << op->op << " (op " << pos
        ...
        case Transaction::OP_CREATE: r = _touch(txc, c, o); break;
        ...
        case Transaction::OP_REMOVE: r = _remove(txc, c, o); break;
        ...
        case Transaction::OP_SETALLOCHINT:
        {
            ...
            r = _setallochint(txc, c, o, expected_object_size, expected_write_size, flags);
        } break;
    ...
    endop:
        if (r < 0) {
            ...
            if (!ok) {
                ...
                dout(0) << " error " << cpp_strerror(r) << " not handled on operation " << op->op << " (op " << pos
                        << ", counting from 0)" << dendl;
                dout(0) << msg << dendl;
                dout(0) << " transaction dump:\n";
                JSONFormatter f(true);
...
int KStore::_touch(TransContext* txc, CollectionRef& c, OnodeRef& o)
{
...
{
    dout(30) << __func__ << " " << o << " nid " << o->onode.nid << " size " << o->onode.size << " expected_object_size "
             << o->onode.expected_object_size << " expected_write_size " << o->onode.expected_write_size << dendl;
    for (map<string, bufferptr>::iterator p = o->onode.attrs.begin(); p != o->onode.attrs.end(); ++p) {
...
void KStore::_do_write_stripe(TransContext* txc, OnodeRef o, uint64_t offset, bufferlist& bl)
{
...
int KStore::_do_write(TransContext* txc, OnodeRef o, uint64_t offset, uint64_t length, bufferlist& orig_bl,
                      uint32_t fadvise_flags)
{
    int r = 0;

    dout(20) << __func__ << " " << o->oid << " " << offset << "~" << length << " - have " << o->onode.size
    ...
    while (length > 0) {
        ...
        if (use > length) use -= stripe_size - end_rem;
...
int KStore::_write(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, size_t length, bufferlist& bl,
                   uint32_t fadvise_flags)
{
...
int KStore::_zero(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, size_t length)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << " " << offset << "~" << length << dendl;
    int r = 0;
    o->exists = true;
    ...
    if (stripe_size) {
        ...
        while (pos < offset + length) {
            if (stripe_off || end - pos < stripe_size) {
                ...
                if (end >= pos - stripe_off + stripe_size || end >= o->onode.size) {
...
int KStore::_truncate(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset)
{
...
int KStore::_do_remove(TransContext* txc, OnodeRef o)
{
    string key;
...
int KStore::_remove(TransContext* txc, CollectionRef& c, OnodeRef& o)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
...
int KStore::_setattr(TransContext* txc, CollectionRef& c, OnodeRef& o, const string& name, bufferptr& val)
{
...
int KStore::_setattrs(TransContext* txc, CollectionRef& c, OnodeRef& o, const map<string, bufferptr>& aset)
{
...
int KStore::_rmattr(TransContext* txc, CollectionRef& c, OnodeRef& o, const string& name)
{
...
int KStore::_rmattrs(TransContext* txc, CollectionRef& c, OnodeRef& o)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
    int r = 0;
    o->onode.attrs.clear();
...
int KStore::_omap_clear(TransContext* txc, CollectionRef& c, OnodeRef& o)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
    int r = 0;
...
int KStore::_omap_setkeys(TransContext* txc, CollectionRef& c, OnodeRef& o, bufferlist& bl)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
    int r;
    auto p = bl.cbegin();
...
int KStore::_omap_setheader(TransContext* txc, CollectionRef& c, OnodeRef& o, bufferlist& bl)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
    int r;
    string key;
...
int KStore::_omap_rmkeys(TransContext* txc, CollectionRef& c, OnodeRef& o, const bufferlist& bl)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
    int r = 0;
    auto p = bl.cbegin();
...
int KStore::_omap_rmkey_range(TransContext* txc, CollectionRef& c, OnodeRef& o, const string& first, const string& last)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
    KeyValueDB::Iterator it;
...
int KStore::_setallochint(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t expected_object_size,
                          uint64_t expected_write_size, uint32_t flags)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << " object_size " << expected_object_size << " write_size "
             << expected_write_size << " flags " << flags << dendl;
    int r = 0;
...
int KStore::_clone(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo)
{
...
int KStore::_clone_range(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo, uint64_t srcoff,
                         uint64_t length, uint64_t dstoff)
{
    dout(15) << __func__ << " " << c->cid << " " << oldo->oid << " -> " << newo->oid << " from " << srcoff << "~"
             << length << " to offset " << dstoff << dendl;
    int r = 0;
...
out:
    dout(10) << __func__ << " " << c->cid << " " << oldo->oid << " -> " << newo->oid << " from " << srcoff << "~"
             << length << " to offset " << dstoff << " = " << r << dendl;
    return r;
}

int KStore::_rename(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo, const ghobject_t& new_oid)
{
...
int KStore::_create_collection(TransContext* txc, coll_t cid, unsigned bits, CollectionRef* c)
{
...
int KStore::_remove_collection(TransContext* txc, coll_t cid, CollectionRef* c)
{
...

    {
        ...
        r = _collection_list(c->get(), ghobject_t(), ghobject_t::get_max(), nonexistent_count + 1, &ls, &next);
        if (r >= 0) {
...
int KStore::_split_collection(TransContext* txc, CollectionRef& c, CollectionRef& d, unsigned bits, int rem)
{
...
int KStore::_merge_collection(TransContext* txc, CollectionRef* c, CollectionRef& d, unsigned bits)
{
    dout(15) << __func__ << " " << (*c)->cid << " to " << d->cid << " "
             << " bits " << bits << dendl;
    int r;
