...
int HashIndex::cleanup()
{
    ...
    if (in_progress.is_split())
        ...
        for (vector<string>::iterator i = in_progress.path.begin(); i != in_progress.path.end(); ++i) {
...
int HashIndex::col_split_level(HashIndex& from, HashIndex& to, const vector<string>& path, uint32_t inbits,
                               uint32_t match, unsigned* mkdirred)
{
    ...
    for (map<string, ghobject_t>::iterator i = objects.begin(); i != objects.end(); ++i) {
    ...
    for (map<string, ghobject_t>::iterator i = objs_to_move.begin(); i != objs_to_move.end(); ++i) {
        from_info.objs--;
        to_info.objs++;
...
int HashIndex::_merge_dirs(HashIndex& from, HashIndex& to, const vector<string>& path)
{
    ...
    if (r < 0) {
        lgeneric_subdout(g_ceph_context, filestore, 20) << __func__ << " r " << r << " from "
...
int HashIndex::_split(uint32_t match, uint32_t bits, CollectionIndex* dest)
{
    ...
    return col_split_level(*this, *static_cast<HashIndex*>(dest), vector<string>(), bits, match, &mkdirred);
}
...
int HashIndex::split_dirs(const vector<string>& path, int target_level)
{
    ...
    if (must_split(info, target_level)) {
        dout(1) << __func__ << " " << path << " has " << info.objs << " objects, " << info.hash_level
...
{
    vector<string> path;
    dout(10) << __func__ << " split multiple = " << split_multiplier << " merge threshold = " << merge_threshold
             << " split rand factor = " << cct->_conf->filestore_split_rand_factor << " target level = " << target_level
             << dendl;
...
int HashIndex::_created(const vector<string>& path, const ghobject_t& oid, const string& mangled_name)
{
    subdir_info_s info;
    int r;
    ...
    if (must_split(info)) {
        dout(1) << __func__ << " " << path << " has " << info.objs << " objects, starting split in pg " << coll() << "."
                << dendl;
        int r = initiate_split(path, info);
        if (r < 0) {
            derr << __func__ << " error starting split " << path << " in pg " << coll() << ": " << cpp_strerror(r)
                 << dendl;
            ...
            r = complete_split(path, info);
            if (r < 0) {
                derr << __func__ << " error completing split " << path << " in pg " << coll() << ": " << cpp_strerror(r)
                     << dendl;
                ceph_assert(!cct->_conf->filestore_fail_eio);
            }
            dout(1) << __func__ << " " << path << " split completed in pg " << coll() << "." << dendl;
...
int HashIndex::_remove(const vector<string>& path, const ghobject_t& oid, const string& mangled_name)
{
    int r;
    ...
    if (must_merge(info)) {
        dout(1) << __func__ << " " << path << " has " << info.objs << " objects, starting merge in pg " << coll() << "."
                << dendl;
        r = initiate_merge(path, info);
        if (r < 0) {
            derr << __func__ << " error starting merge " << path << " in pg " << coll() << ": " << cpp_strerror(r)
                 << dendl;
            ceph_assert(!cct->_conf->filestore_fail_eio);
        }
        else {
            ...
            if (r < 0) {
                derr << __func__ << " error completing merge " << path << " in pg " << coll() << ": " << cpp_strerror(r)
                     << dendl;
                ceph_assert(!cct->_conf->filestore_fail_eio);
            }
            dout(1) << __func__ << " " << path << " merge completed in pg " << coll() << "." << dendl;
        }
    }
...
int HashIndex::_lookup(const ghobject_t& oid, vector<string>* path, string* mangled_name, int* hardlink)
{
...
int HashIndex::_collection_list_partial(const ghobject_t& start, const ghobject_t& end, int max_count,
                                        vector<ghobject_t>* ls, ghobject_t* next)
{
    ...
    if (!next) next = &_next;
    *next = start;
    dout(20) << __func__ << " start:" << start << " end:" << end << "-" << max_count << " ls.size " << ls->size()
             << dendl;
...
int HashIndex::pre_split_folder(uint32_t pg_num, uint64_t expected_num_objs)
{
    ...
    const uint64_t objs_per_folder =
        ((uint64_t)(abs(merge_threshold)) * (uint64_t)split_multiplier + settings.split_rand_factor) * 16;
    uint64_t leavies = expected_num_objs / objs_per_folder;
    // No need to split
    if (leavies == 0 || expected_num_objs == objs_per_folder) return 0;

    spg_t spgid;
    if (!c.is_pg_prefix(&spgid)) return -EINVAL;
...
{
    return (info.hash_level > 0 && merge_threshold > 0 && info.objs < (unsigned)merge_threshold && info.subdirs == 0);
}
...
int HashIndex::complete_split(const vector<string>& path, subdir_info_s info)
{
    ...
    for (map<string, map<string, ghobject_t> >::iterator i = mapped.begin(); i != mapped.end();) {
        dst[level] = i->first;
        ...
        if (subdirs.count(i->first) && !get_info(dst, &temp)) {
            for (map<string, ghobject_t>::iterator j = i->second.begin(); j != i->second.end(); ++j) {
...
int HashIndex::get_path_contents_by_hash_bitwise(const vector<string>& path, const ghobject_t* next_object,
                                                 ...
                                                 set<pair<string, ghobject_t>, CmpPairBitwise>* objects)
{
    ...
    if (r < 0) return r;
    // bitwise sort
    for (map<string, ghobject_t>::iterator i = rev_objects.begin(); i != rev_objects.end(); ++i) {
    ...
    if (next_object) next_object_string = reverse_hexdigit_bits_string(get_path_str(*next_object));
    for (vector<string>::iterator i = subdirs.begin(); i != subdirs.end(); ++i) {
...
int HashIndex::list_by_hash(const vector<string>& path, const ghobject_t& end, int max_count, ghobject_t* next,
...
int HashIndex::list_by_hash_bitwise(const vector<string>& path, const ghobject_t& end, int max_count, ghobject_t* next,
                                    vector<ghobject_t>* out)
{
    ...
    set<string, CmpHexdigitStringBitwise> hash_prefixes;
    set<pair<string, ghobject_t>, CmpPairBitwise> objects;
    int r = get_path_contents_by_hash_bitwise(path, next, &hash_prefixes, &objects);
    if (r < 0) return r;
    for (set<string, CmpHexdigitStringBitwise>::iterator i = hash_prefixes.begin(); i != hash_prefixes.end(); ++i) {
        dout(20) << __func__ << " prefix " << *i << dendl;
        set<pair<string, ghobject_t>, CmpPairBitwise>::iterator j = objects.lower_bound(make_pair(*i, ghobject_t()));
    ...
    if (next) *next = ghobject_t::get_max();
    return 0;
}
