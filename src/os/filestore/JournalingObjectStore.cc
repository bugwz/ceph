...
int JournalingObjectStore::journal_replay(uint64_t fs_op_seq)
{
    ...
    if (cct->_conf->journal_replay_from) {
        dout(0) << "journal_replay forcing replay from " << cct->_conf->journal_replay_from << " instead of "
                << fs_op_seq << dendl;
...
void JournalingObjectStore::ApplyManager::op_apply_finish(uint64_t op)
{
    ...
    dout(10) << "op_apply_finish " << op << " open_ops " << open_ops << " -> " << (open_ops - 1) << ", max_applied_seq "
             << max_applied_seq << " -> " << std::max(op, max_applied_seq) << dendl;
    --open_ops;
...
void JournalingObjectStore::_op_journal_transactions(bufferlist& tbl, uint32_t orig_len, uint64_t op,
