 ...
 */
int ceph_os_setxattr(const char* path, const char* name, const void* value, size_t size)
{
    int error = -1;

#if defined(__FreeBSD__)
    error = extattr_set_file(path, EXTATTR_NAMESPACE_USER, name, value, size);
    if (error > 0) error = 0;
#elif defined(__linux__)
...
ssize_t ceph_os_getxattr(const char* path, const char* name, void* value, size_t size)
{
    ssize_t error = -1;

#if defined(__FreeBSD__)
    if (value == NULL || size == 0) {
        error = extattr_get_file(path, EXTATTR_NAMESPACE_USER, name, value, size);
    }
    else {
        error = extattr_get_file(path, EXTATTR_NAMESPACE_USER, name, NULL, 0);
        if (error > 0) {
            if (error > size) {
                errno = ERANGE;
                error = -1;
...
#if defined(__FreeBSD__)
    if (value == NULL || size == 0) {
        error = extattr_get_fd(fd, EXTATTR_NAMESPACE_USER, name, value, size);
    }
    else {
        error = extattr_get_fd(fd, EXTATTR_NAMESPACE_USER, name, NULL, 0);
        if (error > 0) {
            if (error > size) {
                errno = ERANGE;
                error = -1;
...
ssize_t ceph_os_listxattr(const char* path, char* list, size_t size)
{
    ...
    if (size != 0) {
        newlist = malloc(size);
        if (newlist != NULL) {
            error = extattr_list_file(path, EXTATTR_NAMESPACE_USER, newlist, size);
            if (error > 0) {
                p = newlist;
                p1 = list;
        ...
        error = extattr_list_file(path, EXTATTR_NAMESPACE_USER, list, size);
    }
#elif defined(__linux__)
...
ssize_t ceph_os_flistxattr(int fd, char* list, size_t size)
{
    ...
    if (size != 0) {
        newlist = malloc(size);
        if (newlist != NULL) {
            error = extattr_list_fd(fd, EXTATTR_NAMESPACE_USER, newlist, size);
            if (error > 0) {
                p = newlist;
                p1 = list;
                while ((p - newlist) < error) {
        ...
        error = extattr_list_fd(fd, EXTATTR_NAMESPACE_USER, list, size);
    }
#elif defined(__linux__)
