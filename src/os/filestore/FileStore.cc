...
int FileStore::get_block_device_fsid(CephContext* cct, const string& path, uuid_d* fsid)
{
...
{
    os_commit_latency_ns.consume_next(logger.get_tavg_ns(l_filestore_journal_latency));
    os_apply_latency_ns.consume_next(logger.get_tavg_ns(l_filestore_apply_latency));
}
...
int FileStore::lfn_open(const coll_t& cid, const ghobject_t& oid, bool create, FDRef* outfd, Index* index)
{
    ceph_assert(outfd);
    int r = 0;
    bool need_lock = true;
    ...
    if (r < 0) {
        r = -errno;
        dout(10) << "error opening file " << (*path)->path() << " with flags=" << flags << ": " << cpp_strerror(-r)
                 << dendl;
        goto fail;
    }
    fd = r;
    if (create && (!exist)) {
        ...
        r = chain_fsetxattr<true, true>(fd, XATTR_SPILL_OUT_NAME, XATTR_NO_SPILL_OUT, sizeof(XATTR_NO_SPILL_OUT));
        if (r < 0) {
            VOID_TEMP_FAILURE_RETRY(::close(fd));
...
int FileStore::lfn_unlink(const coll_t& cid, const ghobject_t& o, const SequencerPosition& spos, bool force_clear_omap)
{
    Index index;
    int r = get_index(cid, &index);
    if (r < 0) {
...

    {
        ...
        if (force_clear_omap) {
             ...
             * going away.
             */
            if (!backend->can_checkpoint()) object_map->sync(&o, &spos);
        }
        if (hardlink == 0) {
            if (!m_disable_wbthrottle) {
                wbthrottle.clear_object(o);   // should be only non-cache ref
...
FileStore::FileStore(CephContext* cct, const std::string& base, const std::string& jdev, osflagbits_t flags,
    ...
    , wbthrottle(cct)
    , next_osr_id(0)
    , m_disable_wbthrottle(cct->_conf->filestore_odsync_write || !cct->_conf->filestore_wbthrottle_enable)
    ...
    , op_wq(this, ceph::make_timespan(cct->_conf->filestore_op_thread_timeout),
            ceph::make_timespan(cct->_conf->filestore_op_thread_suicide_timeout), &op_tp)
    , logger(nullptr)
    ...
    , m_filestore_max_xattr_value_size(0)
{
    ...
    plb.add_time_avg(l_filestore_journal_latency,
                     "journal_latency",
                     "Average journal queue completing latency",
                     NULL,
...
void FileStore::collect_metadata(map<string, string>* pm)
{
    ...
    if (cct->_conf->filestore_collect_device_partition_information) {
        ...
        if (rc == 0 && vdo_fd >= 0) {
            (*pm)["vdo"] = "true";
            (*pm)["vdo_physical_size"] = stringify(4096 * get_vdo_stat(vdo_fd, "physical_blocks"));
        }
        if (journal) {
...
int FileStore::pool_statfs(uint64_t pool_id, struct store_statfs_t* buf, bool* per_pool_omap)
{
    return -ENOTSUP;
}
...
void FileStore::new_journal()
{
    if (journalpath.length()) {
        ...
        journal = new FileJournal(
            cct, fsid, &finisher, &sync_cond, journalpath.c_str(), m_journal_dio, m_journal_aio, m_journal_force_aio);
...
FileStoreBackend* FileStoreBackend::create(unsigned long f_type, FileStore* fs)
{
    switch (f_type) {
#if defined(__linux__)
    case BTRFS_SUPER_MAGIC: return new BtrfsFileStoreBackend(fs);
#ifdef HAVE_LIBXFS
    case XFS_SUPER_MAGIC: return new XfsFileStoreBackend(fs);
#endif
#endif
#ifdef HAVE_LIBZFS
    case ZFS_SUPER_MAGIC: return new ZFSFileStoreBackend(fs);
#endif
    default: return new GenericFileStoreBackend(fs);
    }
}
...
int FileStore::mkfs()
{
...
#if defined(__linux__)
    if (basefs.f_type == BTRFS_SUPER_MAGIC && !g_ceph_context->check_experimental_feature_enabled("btrfs")) {
    ...
    if (read_fsid(omap_fsid_fd, &old_omap_fsid) < 0 || old_omap_fsid.is_zero()) {
        ...
        if (fsid != old_omap_fsid) {
            derr << __FUNC__ << ": " << omap_fsid_fn << " has existed omap fsid " << old_omap_fsid
                 << " != expected osd fsid " << fsid << dendl;
            ret = -EINVAL;
            goto close_omap_fsid_fd;
        }
    ...
    // journal?
    ret = mkjournal();
    if (ret) goto close_omap_fsid_fd;
...
int FileStore::_detect_fs()
{
...
#if defined(__linux__)
    if (st.f_type == BTRFS_SUPER_MAGIC && !g_ceph_context->check_experimental_feature_enabled("btrfs")) {
        derr << __FUNC__ << ": deprecated btrfs support is not enabled" << dendl;
        return -EPERM;
    }
#endif
    ...
    if (ret >= 0) ret = chain_fgetxattr(tmpfd, "user.test", &y, sizeof(y));
    if ((ret < 0) || (x != y)) {
        derr << "Extended attributes don't appear to work. ";
        if (ret) *_dout << "Got error " + cpp_strerror(ret) + ". ";
...
int FileStore::_sanity_check_fs()
{
    ...
    if (((int)m_filestore_journal_writeahead + (int)m_filestore_journal_parallel + (int)m_filestore_journal_trailing) >
        1) {
        dout(0) << "mount ERROR: more than one of filestore journal {writeahead,parallel,trailing} enabled" << dendl;
        cerr << TEXT_RED << " ** WARNING: more than one of 'filestore journal {writeahead,parallel,trailing}'\n"
             << "             is enabled in ceph.conf.  You must choose a single journal mode." << TEXT_NORMAL
             << std::endl;
        return -EINVAL;
    }
    ...
    if (!backend->can_checkpoint()) {
        if (!journal || !m_filestore_journal_writeahead) {
            ...
            cerr << TEXT_RED << " ** WARNING: no btrfs AND (no journal OR journal not in writeahead mode)\n"
    ...
    if (!journal) {
        ...
        cerr << TEXT_YELLOW << " ** WARNING: No osd journal is configured: write latency may be high.\n"
...
int FileStore::write_superblock()
{
    ...
    return safe_write_file(basedir.c_str(), "superblock", bl.c_str(), bl.length(), 0600);
}

int FileStore::read_superblock()
{
    bufferptr bp(PATH_MAX);
    int ret = safe_read_file(basedir.c_str(), "superblock", bp.c_str(), bp.length());
    if (ret < 0) {
        if (ret == -ENOENT) {
...
{
    bufferptr bp(PATH_MAX);
    int ret = safe_read_file(basedir.c_str(), "store_version", bp.c_str(), bp.length());
    if (ret < 0) {
        return ret;
    }
    bufferlist bl;
...
int FileStore::write_version_stamp()
{
    ...
    return safe_write_file(basedir.c_str(), "store_version", bl.c_str(), bl.length(), 0600);
}
...
int FileStore::mount()
{
    ...
    if (ret < 0) {
        ...
        if (do_update || (int)version_stamp < cct->_conf->filestore_update_to) {
            derr << __FUNC__ << ": stale version stamp detected: " << version_stamp << ". Proceeding, do_update "
            ...
            derr << __FUNC__ << ": stale version stamp " << version_stamp
                 ...
                 << "OSD, or set filestore_update_to to " << target_version << " (currently "
                 << cct->_conf->filestore_update_to << ")" << dendl;
            goto close_fsid_fd;
        }
    }
    ...
    if (m_osd_rollback_to_cluster_snap.length() && cluster_snaps.count(m_osd_rollback_to_cluster_snap) == 0) {
    ...
    if (backend->can_checkpoint()) {
        if (snaps.empty()) {
            ...
            if (m_osd_rollback_to_cluster_snap.length()) {
                derr << TEXT_RED << " ** NOTE: rolling back to cluster snapshot " << m_osd_rollback_to_cluster_snap
                     << " **" << TEXT_NORMAL << dendl;
                ...
                if (::stat(nosnapfn, &st) == 0) {
                    ...
                    cerr << TEXT_YELLOW << " ** WARNING: forcing the use of stale snapshot data **" << TEXT_NORMAL
                         << std::endl;
                }
    ...
    if (::stat(omap_fsid_buf, &omap_fsid_stat) != 0) {
        ...
        if (ret < 0) {
            derr << __FUNC__ << ": error reading omap_fsid_fd"
                 << ", omap_fsid = " << omap_fsid << cpp_strerror(ret) << dendl;
            goto close_current_fd;
        }
        if (fsid != omap_fsid) {
            derr << __FUNC__ << ": " << omap_fsid_buf << " has existed omap fsid " << omap_fsid
                 << " != expected osd fsid " << fsid << dendl;
            ret = -EINVAL;
            goto close_current_fd;
        }
    ...
    // select journal mode?
    if (journal) {
        if (!m_filestore_journal_writeahead && !m_filestore_journal_parallel && !m_filestore_journal_trailing) {
            if (!backend->can_checkpoint()) {
...
void FileStore::init_temp_collections()
{
    ...
    for (vector<coll_t>::iterator p = ls.begin(); p != ls.end(); ++p)
        if (p->is_temp()) temps.insert(*p);
    dout(20) << " temps " << temps << dendl;

    for (vector<coll_t>::iterator p = ls.begin(); p != ls.end(); ++p) {
        if (p->is_temp()) continue;
...
FileStore::Op* FileStore::build_op(vector<Transaction>& tls, Context* onreadable, Context* onreadable_sync,
                                   TrackedOpRef osd_op)
{
...
void FileStore::queue_op(OpSequencer* osr, Op* o)
{
    ...
    dout(5) << __FUNC__ << ": " << o << " seq " << o->op << " " << *osr << " " << o->bytes << " bytes"
            << "   (queue has " << throttle_ops.get_current() << " ops and " << throttle_bytes.get_current()
            << " bytes)" << dendl;
    op_wq.queue(osr);
}
...
void FileStore::_do_op(OpSequencer* osr, ThreadPool::TPHandle& handle)
{
    ...
    dout(10) << __FUNC__ << ": " << o << " seq " << o->op << " r = " << r << ", finisher " << o->onreadable << " "
             << o->onreadable_sync << dendl;
}
...
struct C_JournaledAhead : public Context
{
    ...
    C_JournaledAhead(FileStore* f, FileStore::OpSequencer* os, FileStore::Op* o, Context* ondisk)
        ...
        , ondisk(ondisk)
    {}
    void finish(int r) override { fs->_journaled_ahead(osr, o, ondisk); }
};

int FileStore::queue_transactions(CollectionHandle& ch, vector<Transaction>& tls, TrackedOpRef osd_op,
                                  ThreadPool::TPHandle* handle)
{
    ...
    if (journal && journal->is_writeable() && !m_filestore_journal_trailing) {
        ...
        if (m_filestore_journal_parallel) {
            ...
            trace.event("journal started");
            _op_journal_transactions(tbl, orig_len, o->op, new C_JournaledAhead(this, osr, o, ondisk), osd_op);
        }
        else {
            ceph_abort();
        }
...
int FileStore::_do_transactions(vector<Transaction>& tls, uint64_t op_seq, ThreadPool::TPHandle* handle,
                                const char* osr_name)
{
    int trans_num = 0;
...
void FileStore::_set_global_replay_guard(const coll_t& cid, const SequencerPosition& spos)
{
    ...
    bufferlist v;
    encode(spos, v);
    int r = chain_fsetxattr<true, true>(fd, GLOBAL_REPLAY_GUARD_XATTR, v.c_str(), v.length());
    if (r < 0) {
...
int FileStore::_check_global_replay_guard(const coll_t& cid, const SequencerPosition& spos)
{
    char fn[PATH_MAX];
    get_cdir(cid, fn, sizeof(fn));
    int fd = ::open(fn, O_RDONLY | O_CLOEXEC);
...
void FileStore::_set_replay_guard(const coll_t& cid, const SequencerPosition& spos, bool in_progress = false)
{
    char fn[PATH_MAX];
    get_cdir(cid, fn, sizeof(fn));
    int fd = ::open(fn, O_RDONLY | O_CLOEXEC);
...
void FileStore::_set_replay_guard(int fd, const SequencerPosition& spos, const ghobject_t* hoid, bool in_progress)
{
    if (backend->can_checkpoint()) return;
    ...
    r = chain_fsetxattr<true, true>(fd, REPLAY_GUARD_XATTR, v.c_str(), v.length());
    if (r < 0) {
...
void FileStore::_close_replay_guard(const coll_t& cid, const SequencerPosition& spos)
{
    char fn[PATH_MAX];
    get_cdir(cid, fn, sizeof(fn));
    int fd = ::open(fn, O_RDONLY | O_CLOEXEC);
...
void FileStore::_close_replay_guard(int fd, const SequencerPosition& spos, const ghobject_t* hoid)
{
    if (backend->can_checkpoint()) return;
    ...
    encode(in_progress, v);
    int r = chain_fsetxattr<true, true>(fd, REPLAY_GUARD_XATTR, v.c_str(), v.length());
    if (r < 0) {
        derr << "fsetxattr " << REPLAY_GUARD_XATTR << " got " << cpp_strerror(r) << dendl;
        ceph_abort_msg("fsetxattr failed");
    }
...
int FileStore::_check_replay_guard(int fd, const SequencerPosition& spos)
{
    ...
    if (opos > spos) {
        ...
        dout(10) << __FUNC__ << ": object has " << opos << " < current pos " << spos << ", in past, will replay"
                 << dendl;
        return 1;
    }
}

void FileStore::_do_transaction(Transaction& t, uint64_t op_seq, int trans_num, ThreadPool::TPHandle* handle,
                                const char* osr_name)
{
    ...
    while (i.have_op()) {
        ...
        case Transaction::OP_WRITE:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _write(cid, oid, off, len, bl, fadvise_flags);
        ...
        case Transaction::OP_ZERO:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _zero(cid, oid, off, len);
        ...
        case Transaction::OP_TRUNCATE:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _truncate(cid, oid, off);
        ...
        case Transaction::OP_REMOVE:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _remove(cid, oid, spos);
        ...
        case Transaction::OP_SETATTR:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) {
                ...
                if (r == -ENOSPC)
                    dout(0) << " ENOSPC on setxattr on " << cid << "/" << oid << " name " << name << " size "
                            << bl.length() << dendl;
            }
        ...
        case Transaction::OP_SETATTRS:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _setattrs(cid, oid, aset, spos);
            tracepoint(objectstore, setattrs_exit, r);
            if (r == -ENOSPC) dout(0) << " ENOSPC on setxattrs on " << cid << "/" << oid << dendl;
        } break;
        ...
        case Transaction::OP_RMATTR:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _rmattr(cid, oid, name.c_str(), spos);
        ...
        case Transaction::OP_RMATTRS:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _rmattrs(cid, oid, spos);
        ...
        case Transaction::OP_CLONERANGE:
        {
            ...
            const ghobject_t& noid = i.get_oid(op->dest_oid);
            const coll_t& cid = !_need_temp_object_collection(_cid, oid) ? _cid : _cid.get_temp();
            const coll_t& ncid = !_need_temp_object_collection(_cid, noid) ? _cid : _cid.get_temp();
            uint64_t off = op->off;
            uint64_t len = op->len;
        ...
        case Transaction::OP_MKCOLL:
        {
            ...
            if (_check_replay_guard(cid, spos) > 0) r = _create_collection(cid, op->split_bits, spos);
        ...
        case Transaction::OP_COLL_ADD:
        {
            ...
            if (_check_replay_guard(ocid, oid, spos) > 0) r = _remove(ocid, oid, spos);
        ...
        case Transaction::OP_COLL_MOVE:
        {
            ...
            if (r == 0 && (_check_replay_guard(ocid, oid, spos) > 0)) r = _remove(ocid, oid, spos);
        ...
        case Transaction::OP_COLL_MOVE_RENAME:
        {
            ...
            const coll_t& oldcid = !_need_temp_object_collection(_oldcid, oldoid) ? _oldcid : _oldcid.get_temp();
            const coll_t& newcid = !_need_temp_object_collection(_newcid, newoid) ? _oldcid : _newcid.get_temp();
        ...
        case Transaction::OP_TRY_RENAME:
        {
            ...
            const ghobject_t& oldoid = i.get_oid(op->oid);
            const ghobject_t& newoid = i.get_oid(op->dest_oid);
            const coll_t& oldcid = !_need_temp_object_collection(_cid, oldoid) ? _cid : _cid.get_temp();
            const coll_t& newcid = !_need_temp_object_collection(_cid, newoid) ? _cid : _cid.get_temp();
        ...
        case Transaction::OP_COLL_RMATTR: ceph_abort_msg("collection attr methods no longer implemented"); break;
        ...
        case Transaction::OP_OMAP_CLEAR:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _omap_clear(cid, oid, spos);
        ...
        case Transaction::OP_OMAP_SETKEYS:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _omap_setkeys(cid, oid, aset, spos);
        ...
        case Transaction::OP_OMAP_RMKEYS:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _omap_rmkeys(cid, oid, keys, spos);
        ...
        case Transaction::OP_OMAP_RMKEYRANGE:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _omap_rmkeyrange(cid, oid, first, last, spos);
        ...
        case Transaction::OP_OMAP_SETHEADER:
        {
            ...
            if (_check_replay_guard(cid, oid, spos) > 0) r = _omap_setheader(cid, oid, bl, spos);
        ...
        default: derr << "bad op " << op->op << dendl; ceph_abort();
        }
        ...
        if (r < 0) {
            ...
            if (!ok) {
                ...
                derr << " error " << cpp_strerror(r) << " not handled on operation " << op << " (" << spos << ", or op "
                     << spos.op << ", counting from 0)" << dendl;
                dout(0) << msg << dendl;
...
int FileStore::stat(CollectionHandle& ch, const ghobject_t& oid, struct stat* st, bool allow_eio)
{
...
int FileStore::set_collection_opts(CollectionHandle& ch, const pool_opts_t& opts)
{
    return -EOPNOTSUPP;
}

int FileStore::read(CollectionHandle& ch, const ghobject_t& oid, uint64_t offset, size_t len, bufferlist& bl,
                    uint32_t op_flags)
{
    int got;
    ...
    if (op_flags & CEPH_OSD_OP_FLAG_FADVISE_RANDOM) posix_fadvise(**fd, offset, len, POSIX_FADV_RANDOM);
    if (op_flags & CEPH_OSD_OP_FLAG_FADVISE_SEQUENTIAL) posix_fadvise(**fd, offset, len, POSIX_FADV_SEQUENTIAL);
#endif
...
#ifdef HAVE_POSIX_FADVISE
    if (op_flags & CEPH_OSD_OP_FLAG_FADVISE_DONTNEED) posix_fadvise(**fd, offset, len, POSIX_FADV_DONTNEED);
...
int FileStore::_do_fiemap(int fd, uint64_t offset, size_t len, map<uint64_t, uint64_t>* m)
{
    uint64_t i;
...
more:
    ...
    while (i < fiemap->fm_mapped_extents) {
        ...
        while ((i < fiemap->fm_mapped_extents - 1) && (extent->fe_logical + extent->fe_length == next->fe_logical)) {
...
int FileStore::_do_seek_hole_data(int fd, uint64_t offset, size_t len, map<uint64_t, uint64_t>* m)
{
...
int FileStore::fiemap(CollectionHandle& ch, const ghobject_t& oid, uint64_t offset, size_t len, bufferlist& bl)
{
...
int FileStore::fiemap(CollectionHandle& ch, const ghobject_t& oid, uint64_t offset, size_t len,
                      map<uint64_t, uint64_t>& destmap)
{
    ...
    if ((!backend->has_seek_data_hole() && !backend->has_fiemap()) || len <= (size_t)m_filestore_fiemap_threshold) {
        destmap[offset] = len;
        return 0;
    }
    ...
    dout(10) << __FUNC__ << ": " << cid << "/" << oid << " " << offset << "~" << len << " = " << r
             << " num_extents=" << destmap.size() << " " << destmap << dendl;
    if (r == -EIO && m_filestore_fail_eio) handle_eio();
    tracepoint(objectstore, fiemap_exit, r);
    return r;
}

int FileStore::_remove(const coll_t& cid, const ghobject_t& oid, const SequencerPosition& spos)
{
...
int FileStore::_write(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len, const bufferlist& bl,
                      uint32_t fadvise_flags)
{
    ...
    if (replaying || m_disable_wbthrottle) {
        ...
        wbthrottle.queue_wb(fd, oid, offset, len, fadvise_flags & CEPH_OSD_OP_FLAG_FADVISE_DONTNEED);
    }
                      ...
                      const SequencerPosition& spos)
{
...

    {
        ...
        if (r >= 0 && !strncmp(buf, XATTR_NO_SPILL_OUT, sizeof(XATTR_NO_SPILL_OUT))) {
            r = chain_fsetxattr<true, true>(**n, XATTR_SPILL_OUT_NAME, XATTR_NO_SPILL_OUT, sizeof(XATTR_NO_SPILL_OUT));
        }
        else {
            r = chain_fsetxattr<true, true>(**n, XATTR_SPILL_OUT_NAME, XATTR_SPILL_OUT, sizeof(XATTR_SPILL_OUT));
        }
        if (r < 0) goto out3;
...
int FileStore::_do_sparse_copy_range(int from, int to, uint64_t srcoff, uint64_t len, uint64_t dstoff)
{
    ...
    for (map<uint64_t, uint64_t>::iterator miter = exomap.begin(); miter != exomap.end(); ++miter) {
        ...
        if (r < 0) {
            derr << __FUNC__ << ": copy error at " << miter->first << "~" << miter->second << " to " << it_off << ", "
                 << cpp_strerror(r) << dendl;
            break;
        }
...
int FileStore::_do_copy_range(int from, int to, uint64_t srcoff, uint64_t len, uint64_t dstoff, bool skip_sloppycrc)
{
    ...
    if (backend->has_splice()) {
        ...
        while (pos < end) {
            ...
            if (r < 0) {
                derr << __FUNC__ << ": safe_splice read error at " << pos << "~" << len << ", " << cpp_strerror(r)
                     << dendl;
                break;
            }
            if (r == 0) {
                ...
                r = -ERANGE;
                derr << __FUNC__ << ": got short read result at " << pos << " of fd " << from << " len " << len
                     << dendl;
                break;
            }
...
int FileStore::_clone_range(const coll_t& oldcid, const ghobject_t& oldoid, const coll_t& newcid,
                            const ghobject_t& newoid, uint64_t srcoff, uint64_t len, uint64_t dstoff,
                            const SequencerPosition& spos)
{
    dout(15) << __FUNC__ << ": " << oldcid << "/" << oldoid << " -> " << newcid << "/" << newoid << " " << srcoff << "~"
             << len << " to " << dstoff << dendl;
...
out:
    lfn_close(o);
out2:
    dout(10) << __FUNC__ << ": " << oldcid << "/" << oldoid << " -> " << newcid << "/" << newoid << " " << srcoff << "~"
             << len << " to " << dstoff << " = " << r << dendl;
    return r;
}
...
void FileStore::sync_entry()
{
    ...
    while (!stop) {
        ...
        if (force_sync) {
            ...
            if (woke < min_interval) {
                ...
                dout(20) << __FUNC__ << ": waiting for another " << t << " to reach min interval " << min_interval
                         << dendl;
    ...
    again:
        ...
        if (apply_manager.commit_start()) {
            ...
            SyncEntryTimeout* sync_entry_timeo = new SyncEntryTimeout(cct, m_filestore_commit_timeout);
            if (!timer.add_event_after(m_filestore_commit_timeout, sync_entry_timeo)) {
...
int FileStore::getattr(CollectionHandle& ch, const ghobject_t& oid, const char* name, bufferptr& bp)
{
    ...
    if (r == -ENODATA) {
        ...
        bp = bufferptr(got.begin()->second.c_str(), got.begin()->second.length());
        r = bp.length();
    }
out:
...
int FileStore::getattrs(CollectionHandle& ch, const ghobject_t& oid, map<string, bufferptr>& aset)
{
    ...
    if (r >= 0 && !strncmp(buf, XATTR_NO_SPILL_OUT, sizeof(XATTR_NO_SPILL_OUT))) spill_out = false;
    ...
    for (map<string, bufferlist>::iterator i = omap_aset.begin(); i != omap_aset.end(); ++i) {
        string key(i->first);
        aset.insert(make_pair(key, bufferptr(i->second.c_str(), i->second.length())));
    }
out:
    dout(10) << __FUNC__ << ": " << cid << "/" << oid << " = " << r << dendl;
    if (r == -EIO && m_filestore_fail_eio) handle_eio();
                         ...
                         const SequencerPosition& spos)
{
    ...
    if (r == -EIO && m_filestore_fail_eio) handle_eio();
    dout(15) << __FUNC__ << ": " << cid << "/" << oid << (incomplete_inline ? " (incomplete_inline, forcing omap)" : "")
             << dendl;
    ...
    for (map<string, bufferptr>::iterator p = aset.begin(); p != aset.end(); ++p) {
        ...
        if (p->second.length() > m_filestore_max_inline_xattr_size) {
            if (inline_set.count(p->first)) {
                ...
                if (r < 0) goto out_close;
            }
            omap_set[p->first].push_back(p->second);
            continue;
        }

        if (!inline_set.count(p->first) && inline_set.size() >= m_filestore_max_inline_xattrs) {
            omap_set[p->first].push_back(p->second);
            continue;
        }
    ...
    if (spill_out != 1 && !omap_set.empty()) {
        chain_fsetxattr(**fd, XATTR_SPILL_OUT_NAME, XATTR_SPILL_OUT, sizeof(XATTR_SPILL_OUT));
    }
...
int FileStore::_rmattr(const coll_t& cid, const ghobject_t& oid, const char* name, const SequencerPosition& spos)
{
    dout(15) << __FUNC__ << ": " << cid << "/" << oid << " '" << name << "'" << dendl;
    FDRef fd;
    bool spill_out = true;
...
int FileStore::_rmattrs(const coll_t& cid, const ghobject_t& oid, const SequencerPosition& spos)
{
    dout(15) << __FUNC__ << ": " << cid << "/" << oid << dendl;
...
int FileStore::_collection_remove_recursive(const coll_t& cid, const SequencerPosition& spos)
{
    struct stat st;
...
int FileStore::list_collections(vector<coll_t>& ls, bool include_temp)
{
    ...
    while (true) {
        ...
        if (!cid.is_temp() || include_temp) ls.push_back(cid);
    }
...
int FileStore::collection_empty(const coll_t& cid, bool* empty)
{
    ...
    r = index->collection_list_partial(ghobject_t(), ghobject_t::get_max(), 1, &ls, nullptr);
    if (r < 0) {
...
int FileStore::collection_list(const coll_t& c, const ghobject_t& orig_start, const ghobject_t& end, int max,
                               vector<ghobject_t>* ls, ghobject_t* next)
{
    ...
    if (!c.is_temp() && !c.is_meta()) {
        if (start < sep) {
            ...
            if (r < 0) return r;
            if (*next != ghobject_t::get_max()) return r;
            start = sep;
...
int FileStore::omap_get(CollectionHandle& ch, const ghobject_t& hoid, bufferlist* header, map<string, bufferlist>* out)
{
...
int FileStore::omap_get_header(CollectionHandle& ch, const ghobject_t& hoid, bufferlist* bl, bool allow_eio)
{
...
int FileStore::omap_get_values(CollectionHandle& ch, const ghobject_t& hoid, const set<string>& keys,
...
int FileStore::omap_check_keys(CollectionHandle& ch, const ghobject_t& hoid, const set<string>& keys, set<string>* out)
{
...
ObjectMap::ObjectMapIterator FileStore::get_omap_iterator(CollectionHandle& ch, const ghobject_t& oid)
{
    auto osr = static_cast<OpSequencer*>(ch.get());
    osr->wait_for_apply(oid);
...
ObjectMap::ObjectMapIterator FileStore::get_omap_iterator(const coll_t& _c, const ghobject_t& hoid)
{
...
int FileStore::_collection_hint_expected_num_objs(const coll_t& c, uint32_t pg_num, uint64_t expected_num_objs,
                                                  const SequencerPosition& spos)
{
    dout(15) << __FUNC__ << ": collection: " << c << " pg number: " << pg_num
    ...
    if (ret < 0) return ret;
    if (!empty && !replaying) {
        dout(0) << "Failed to give an expected number of objects hint to collection : " << c
...
int FileStore::_create_collection(const coll_t& c, int bits, const SequencerPosition& spos)
{
    char fn[PATH_MAX];
    get_cdir(c, fn, sizeof(fn));
    dout(15) << __FUNC__ << ": " << fn << dendl;
                               ...
                               const SequencerPosition& spos)
{
    ...
    if (r < 0) {
        ...
        dout(10) << __FUNC__ << ": " << c << "/" << o << " from " << oldcid << "/" << o << " (dne, continue replay) "
                 << dendl;
        return 0;
    }
    ...
    if (replaying && !backend->can_checkpoint() && r == -EEXIST)   // crashed between link() and set_replay_guard()
        r = 0;
...
int FileStore::_collection_move_rename(const coll_t& oldcid, const ghobject_t& oldoid, coll_t c, const ghobject_t& o,
                                       const SequencerPosition& spos, bool allow_enoent)
{
...

    {
        ...
        if (r == 0) r = lfn_unlink(oldcid, oldoid, spos, true);
...
int FileStore::_omap_clear(const coll_t& cid, const ghobject_t& hoid, const SequencerPosition& spos)
{
...
int FileStore::_omap_setkeys(const coll_t& cid, const ghobject_t& hoid, const map<string, bufferlist>& aset,
                             const SequencerPosition& spos)
{
    dout(15) << __FUNC__ << ": " << cid << "/" << hoid << dendl;
    Index index;
...
int FileStore::_omap_rmkeys(const coll_t& cid, const ghobject_t& hoid, const set<string>& keys,
                            const SequencerPosition& spos)
{
    dout(15) << __FUNC__ << ": " << cid << "/" << hoid << dendl;
    Index index;
...
int FileStore::_omap_rmkeyrange(const coll_t& cid, const ghobject_t& hoid, const string& first, const string& last,
                                const SequencerPosition& spos)
{
    ...
    set<string> keys;
    {
        ...
        for (iter->lower_bound(first); iter->valid() && iter->key() < last; iter->next()) {
...
int FileStore::_omap_setheader(const coll_t& cid, const ghobject_t& hoid, const bufferlist& bl,
                               const SequencerPosition& spos)
{
    dout(15) << __FUNC__ << ": " << cid << "/" << hoid << dendl;
    Index index;
...
int FileStore::_merge_collection(const coll_t& cid, uint32_t bits, coll_t dest, const SequencerPosition& spos)
{
    ...
    // temp too
    {
        ...
        Index to;
        if (!r) r = get_index(dest.get_temp(), &to);
    ...
    if (!r && cct->_conf->filestore_debug_verify_split) {
        ...
        ghobject_t next;
        while (1) {
            collection_list(dest, next, ghobject_t::get_max(), get_ideal_list_max(), &objects, &next);
...
int FileStore::_split_collection(const coll_t& cid, uint32_t bits, uint32_t rem, coll_t dest,
                                 const SequencerPosition& spos)
{
    int r;
    {
    ...
    if (!r && cct->_conf->filestore_debug_verify_split) {
        ...
        ghobject_t next;
        while (1) {
            collection_list(cid, next, ghobject_t::get_max(), get_ideal_list_max(), &objects, &next);
            if (objects.empty()) break;
            for (vector<ghobject_t>::iterator i = objects.begin(); i != objects.end(); ++i) {
...
int FileStore::_set_alloc_hint(const coll_t& cid, const ghobject_t& oid, uint64_t expected_object_size,
                               uint64_t expected_write_size)
{
    dout(15) << __FUNC__ << ": " << cid << "/" << oid << " object_size " << expected_object_size << " write_size "
    ...
    lfn_close(fd);
out:
    dout(10) << __FUNC__ << ": " << cid << "/" << oid << " object_size " << expected_object_size << " write_size "
             << expected_write_size << " = " << ret << dendl;
    ceph_assert(!m_filestore_fail_eio || ret != -EIO);
    return ret;
}
...
void FileStore::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    if (changed.count("filestore_max_inline_xattr_size") || changed.count("filestore_max_inline_xattr_size_xfs") ||
        changed.count("filestore_max_inline_xattr_size_btrfs") ||
        changed.count("filestore_max_inline_xattr_size_other") || changed.count("filestore_max_inline_xattrs") ||
        changed.count("filestore_max_inline_xattrs_xfs") || changed.count("filestore_max_inline_xattrs_btrfs") ||
        changed.count("filestore_max_inline_xattrs_other") || changed.count("filestore_max_xattr_value_size") ||
        changed.count("filestore_max_xattr_value_size_xfs") || changed.count("filestore_max_xattr_value_size_btrfs") ||
    ...
    if (changed.count("filestore_queue_max_bytes") || changed.count("filestore_queue_max_ops") ||
        changed.count("filestore_expected_throughput_bytes") || changed.count("filestore_expected_throughput_ops") ||
        changed.count("filestore_queue_low_threshhold") || changed.count("filestore_queue_high_threshhold") ||
        changed.count("filestore_queue_high_delay_multiple") || changed.count("filestore_queue_max_delay_multiple")) {
        std::lock_guard l(lock);
        set_throttle_params();
    }

    if (changed.count("filestore_min_sync_interval") || changed.count("filestore_max_sync_interval") ||
        changed.count("filestore_kill_at") || changed.count("filestore_fail_eio") ||
        changed.count("filestore_sloppy_crc") || changed.count("filestore_sloppy_crc_block_size") ||
        changed.count("filestore_max_alloc_hint_size") || changed.count("filestore_fadvise")) {
        std::lock_guard l(lock);
    ...
    if (changed.count("filestore_dump_file")) {
        if (conf->filestore_dump_file.length() && conf->filestore_dump_file != "-") {
...
int FileStore::set_throttle_params()
{
    ...
    bool valid = throttle_bytes.set_params(
        ...
        cct->_conf->filestore_queue_high_delay_multiple ? cct->_conf->filestore_queue_high_delay_multiple
                                                        : cct->_conf->filestore_queue_high_delay_multiple_bytes,
        cct->_conf->filestore_queue_max_delay_multiple ? cct->_conf->filestore_queue_max_delay_multiple
...
void FileStore::set_xattr_limits_via_conf()
{
    ...
    if (m_filestore_max_xattr_value_size < cct->_conf->osd_max_object_name_len) {
        derr << "WARNING: max attr value size (" << m_filestore_max_xattr_value_size
             << ") is smaller than osd_max_object_name_len (" << cct->_conf->osd_max_object_name_len
...
void FSSuperblock::generate_test_instances(list<FSSuperblock*>& o)
{
    ...
    z.compat_features = CompatSet(feature_compat, feature_ro_compat, feature_incompat);
    o.push_back(new FSSuperblock(z));
