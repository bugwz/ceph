    ...
    , m_filestore_splice(cct()->_conf->filestore_splice)
{
    ...
    // journal rotational?
    {
        ...
        dout(20) << __func__ << " journal filename " << fn.c_str() << " journal rotational "
                 << (int)m_journal_rotational << dendl;
        ::close(fd);
    }
}
...
int GenericFileStoreBackend::detect_features()
{
    ...
    if (!m_filestore_seek_data_hole) {
        dout(0) << "detect_features: SEEK_DATA/SEEK_HOLE is disabled via 'filestore seek data hole' config option"
                << dendl;
        seek_data_hole = false;
    }
    else {
    ...
    if (!have_syncfs) {
        ...
        if (m_filestore_fsync_flushes_journal_data) {
            dout(0) << "detect_features: no syncfs(2), but 'filestore fsync flushes journal data = true', so fsync "
                       "will suffice."
                    << dendl;
        }
        else {
...
int GenericFileStoreBackend::_crc_update_clone_range(int srcfd, int destfd, loff_t srcoff, size_t len, loff_t dstoff)
{
...
int GenericFileStoreBackend::_crc_verify_read(int fd, loff_t off, size_t len, const bufferlist& bl, ostream* out)
{
    SloppyCRCMap scm(get_crc_block_size());
    int r = _crc_load_or_init(fd, &scm);
    if (r < 0) return r;
