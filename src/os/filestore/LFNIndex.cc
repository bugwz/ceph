...
const int LFNIndex::FILENAME_PREFIX_LEN =
    FILENAME_SHORT_LEN - FILENAME_HASH_LEN - FILENAME_COOKIE.size() - FILENAME_EXTRA;
void LFNIndex::maybe_inject_failure()
{
    if (error_injection_enabled) {
        if (current_failure > last_failure &&
            (((double)(rand() % 10000)) / ((double)(10000)) < error_injection_probability)) {
...
{
    WRAP_RETRY(vector<string> path_comp; string short_name; r = decompose_full_path(path, &path_comp, 0, &short_name);
               if (r < 0) goto out;
...
{
    WRAP_RETRY(
        vector<string> path; string short_name; r = _lookup(oid, &path, &short_name, NULL);
        if (r < 0) { goto out; } r = _remove(path, oid, short_name);
        if (r < 0) { goto out; });
}

int LFNIndex::lookup(const ghobject_t& oid, IndexedPath* out_path, int* hardlink)
{
    WRAP_RETRY(vector<string> path; string short_name; r = _lookup(oid, &path, &short_name, hardlink);
               if (r < 0) goto out;
...
int LFNIndex::collection_list_partial(const ghobject_t& start, const ghobject_t& end, int max_count,
...
int LFNIndex::link_object(const vector<string>& from, const vector<string>& to, const ghobject_t& oid,
...
int LFNIndex::remove_objects(const vector<string>& dir, const map<string, ghobject_t>& to_remove,
                             map<string, ghobject_t>* remaining)
{
    set<string> clean_chains;
    for (map<string, ghobject_t>::const_iterator to_clean = to_remove.begin(); to_clean != to_remove.end();
         ++to_clean) {
        ...
        for (set<int>::iterator i = holes.begin(); i != holes.end(); ++i) {
            if (candidate == chain.rend() || *i > candidate->first) {
                string remove_path_name = get_full_path(dir, lfn_get_short_name(to_clean->second, *i));
                maybe_inject_failure();
            ...
            remaining->insert(
                pair<string, ghobject_t>(lfn_get_short_name(candidate->second.second, *i), candidate->second.second));
            ++candidate;
        }
...
int LFNIndex::move_objects(const vector<string>& from, const vector<string>& to)
{
    map<string, ghobject_t> to_move;
    int r;
    r = list_objects(from, 0, NULL, &to_move);
    if (r < 0) return r;
    for (map<string, ghobject_t>::iterator i = to_move.begin(); i != to_move.end(); ++i) {
...
int LFNIndex::get_mangled_name(const vector<string>& from, const ghobject_t& oid, string* mangled_name, int* hardlink)
{
    return lfn_get_name(from, oid, mangled_name, 0, hardlink);
}

int LFNIndex::move_subdir(LFNIndex& from, LFNIndex& dest, const vector<string>& path, string dir)
{
...
int LFNIndex::move_object(LFNIndex& from, LFNIndex& dest, const vector<string>& path,
...
static int get_hobject_from_oinfo(const char* dir, const char* file, ghobject_t* o)
{
    char path[PATH_MAX];
...
int LFNIndex::list_objects(const vector<string>& to_list, int max_objs, long* handle, map<string, ghobject_t>* out)
{
    ...
    while (true) {
        ...
        if (de == nullptr) {
            if (errno != 0) {
                r = -errno;
                dout(0) << "readdir failed " << to_list_path << ": " << cpp_strerror(-r) << dendl;
                goto cleanup;
            }
            break;
        }
...
int LFNIndex::add_attr_path(const vector<string>& path, const string& attr_name, bufferlist& attr_value)
{
    string full_path = get_full_path_subdir(path);
    maybe_inject_failure();
    return chain_setxattr<false, true>(full_path.c_str(),
                                       mangle_attr_name(attr_name).c_str(),
                                       reinterpret_cast<void*>(attr_value.c_str()),
                                       attr_value.length());
}

int LFNIndex::get_attr_path(const vector<string>& path, const string& attr_name, bufferlist& attr_value)
{
    string full_path = get_full_path_subdir(path);
    bufferptr bp;
    int r = chain_getxattr_buf(full_path.c_str(), mangle_attr_name(attr_name).c_str(), &bp);
    if (r > 0) attr_value.push_back(bp);
    return r;
}

int LFNIndex::remove_attr_path(const vector<string>& path, const string& attr_name)
{
    string full_path = get_full_path_subdir(path);
...
static void append_escaped(string::const_iterator begin, string::const_iterator end, string* out)
{
...
{
    if (index_version == HASH_INDEX_TAG) return lfn_generate_object_name_keyless(oid);
...
int LFNIndex::lfn_get_name(const vector<string>& path, const ghobject_t& oid, string* mangled_name, string* out_path,
                           int* hardlink)
{
    ...
    for (;; ++i) {
        ...
        bp = bufferptr();
        r = chain_getxattr_buf(candidate_path.c_str(), get_alt_lfn_attr().c_str(), &bp);
        if (r > 0) {
            ...
            if (st.st_nlink <= 1) {
                ...
                maybe_inject_failure();
                dout(20) << __func__ << " found extra alt attr for " << candidate_path << ", long name "
                         << string(bp.c_str(), bp.length()) << dendl;
                rc = chain_removexattr(candidate_path.c_str(), get_alt_lfn_attr().c_str());
                maybe_inject_failure();
                if (rc < 0) return rc;
                continue;
            }
...
int LFNIndex::lfn_created(const vector<string>& path, const ghobject_t& oid, const string& mangled_name)
{
    ...
    if (r > 0) {
        string lfn(bp.c_str(), bp.length());
        if (lfn != full_name) {
            dout(20) << __func__ << " " << mangled_name << " moving old name to alt attr " << lfn << ", new name is "
                     << full_name << dendl;
            r = chain_setxattr<false, true>(full_path.c_str(), get_alt_lfn_attr().c_str(), bp.c_str(), bp.length());
            if (r < 0) return r;
        }
    }

    return chain_setxattr<false, true>(full_path.c_str(), get_lfn_attr().c_str(), full_name.c_str(), full_name.size());
}

int LFNIndex::lfn_unlink(const vector<string>& path, const ghobject_t& oid, const string& mangled_name)
{
...
int LFNIndex::lfn_translate(const vector<string>& path, const string& short_name, ghobject_t* out)
{
    ...
    bufferptr bp;
    int r = chain_getxattr_buf(full_path.c_str(), get_alt_lfn_attr().c_str(), &bp);
    if (r > 0) {
...
int LFNIndex::lfn_parse_object_name_keyless(const string& long_name, ghobject_t* out)
{
    ...
    int64_t pool = -1;
    spg_t pg;
    if (coll().is_pg_prefix(&pg)) pool = (int64_t)pg.pgid.pool();
    out->hobj.pool = pool;
    if (!r) return -EINVAL;
...
static bool append_unescaped(string::const_iterator begin, string::const_iterator end, string* out)
{
    for (string::const_iterator i = begin; i != end; ++i) {
        if (*i == '\\') {
            ++i;
...
int LFNIndex::lfn_parse_object_name_poolless(const string& long_name, ghobject_t* out)
{
    string name;
    string key;
    uint32_t hash;
...
int LFNIndex::lfn_parse_object_name(const string& long_name, ghobject_t* out)
{
    ...
    if (index_version == HASH_INDEX_TAG) return lfn_parse_object_name_keyless(long_name, out);
    if (index_version == HASH_INDEX_TAG_2) return lfn_parse_object_name_poolless(long_name, out);
...
bool LFNIndex::lfn_is_hashed_filename(const string& name)
{
    ...
    if (name.substr(name.size() - FILENAME_COOKIE.size(), FILENAME_COOKIE.size()) == FILENAME_COOKIE) {
        return 1;
    }
    else {
        return 0;
...
int LFNIndex::decompose_full_path(const char* in, vector<string>* out, ghobject_t* oid, string* shortname)
{
