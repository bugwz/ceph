...
int chain_getxattr(const char* fn, const char* name, void* val, size_t size)
{
    ...
    if (!size) return getxattr_len(fn, name);
    ...
    do {
        ...
        i++;
    } while (size && (r == CHAIN_XATTR_MAX_BLOCK_LEN || r == CHAIN_XATTR_SHORT_BLOCK_LEN));

    if (r >= 0) {
        ret = pos;
        /* is there another chunk? that can happen if the last read size span over
           exactly one block */
        if (chunk_size == CHAIN_XATTR_MAX_BLOCK_LEN || chunk_size == CHAIN_XATTR_SHORT_BLOCK_LEN) {
            get_raw_xattr_name(name, i, raw_name, sizeof(raw_name));
            r = sys_getxattr(fn, raw_name, 0, 0);
...
int chain_fgetxattr(int fd, const char* name, void* val, size_t size)
{
    ...
    if (!size) return chain_fgetxattr_len(fd, name);
...
int chain_listxattr(const char* fn, char* names, size_t len)
{
    ...
    if (!len) return sys_listxattr(fn, names, len) * 2;
...
int chain_flistxattr(int fd, char* names, size_t len)
{
    ...
    if (!len) return sys_flistxattr(fd, names, len) * 2;
