...
int BtrfsFileStoreBackend::detect_features()
{
    ...
    if (m_filestore_btrfs_clone_range) {
        ...
        if (fd >= 0) {
            if (::unlinkat(get_basedir_fd(), "clone_range_test", 0) < 0) {
                r = -errno;
                dout(0) << "detect_feature: failed to unlink test file for CLONE_RANGE ioctl: " << cpp_strerror(r)
                        << dendl;
            }
    ...
    if (r != 0) {
        r = -errno;
        dout(0) << "detect_feature: failed to create simple subvolume " << vol_args.name << ": " << cpp_strerror(r)
                << dendl;
    }
    ...
    if (r == 0 || errno == EEXIST) {
        ...
        if (r == 0) {
            ...
            if (err == -EPERM && getuid() != 0) {
                dout(0) << "detect_feature: failed with EPERM as non-root; remount with -o user_subvol_rm_allowed"
                        << dendl;
                cerr << TEXT_YELLOW << "btrfs SNAP_DESTROY failed as non-root; remount with -o user_subvol_rm_allowed"
    ...
    if (m_filestore_btrfs_snap && !has_snap_create_v2) {
        ...
        cerr << TEXT_YELLOW << " ** WARNING: 'filestore btrfs snap' is enabled (for safe transactions,\n"
...
int BtrfsFileStoreBackend::create_current()
{
    ...
    if (::chmod(get_current_path().c_str(), 0755) < 0) {
        ret = -errno;
        dout(0) << "create_current: failed to chmod " << get_current_path() << " to 0755: " << cpp_strerror(ret)
                << dendl;
        return ret;
    }
...
int BtrfsFileStoreBackend::list_checkpoints(list<string>& ls)
{
    ...
    while (true) {
        ...
        if (de == nullptr) {
            if (errno != 0) {
                err = -errno;
                dout(0) << "list_checkpoints: readdir '" << get_basedir_path() << "' failed: " << cpp_strerror(err)
                        << dendl;
            }
            break;
        }
        ...
        if (fs.f_type == BTRFS_SUPER_MAGIC && basest.st_dev != st.st_dev) snaps.push_back(string(de->d_name));
    }
...
int BtrfsFileStoreBackend::clone_range(int from, int to, uint64_t srcoff, uint64_t len, uint64_t dstoff)
{
    ...
    if (!ALIGNED(lenclone, blk_size)) {
        ...
        if (srcoff + len != (uint64_t)from_stat.st_size || dstoff + len < (uint64_t)to_stat.st_size) {
    ...
    dout(20) << "clone_range: cloning " << srcoffclone << "~" << lenclone << " to " << dstoffclone << " = " << r
             << dendl;
    ...
    if (srcoffclone + lenclone != srcoff + len) {
        err = _copy_range(
            from, to, srcoffclone + lenclone, (srcoff + len) - (srcoffclone + lenclone), dstoffclone + lenclone);
        if (err >= 0) {
            r += err;
        }
        else {
