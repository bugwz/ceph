...
int DBObjectMap::check(std::ostream& out, bool repair, bool force)
{
    ...
    for (iter->seek_to_first(); iter->valid(); iter->next()) {
        ...
        while (true) {
            ...
            if (header.seq != 0) parent_to_actual_num_children[header.seq] = header.num_children;
            ...
            if (state.v == 2 || force) {
                ...
                KeyValueDB::Iterator complete_iter =
                    db->get_iterator(USER_PREFIX + header_key(header.seq) + COMPLETE_PREFIX);
                for (complete_iter->seek_to_first(); complete_iter->valid(); complete_iter->next()) {
                ...
                if (complete_error) {
                    ...
                    for (complete_iter->seek_to_first(); complete_iter->valid(); complete_iter->next()) {
                        out << complete_iter->key() << " -> "
                            << string(complete_iter->value().c_str(), complete_iter->value().length() - 1) << std::endl;
                    }
                    if (repair) {
                        repaired = true;
            ...
            if (header.parent == 0) break;

            if (!parent_to_num_children.count(header.parent)) parent_to_num_children[header.parent] = 0;
            parent_to_num_children[header.parent]++;
            if (parent_to_actual_num_children.count(header.parent)) break;
    ...
    for (map<uint64_t, uint64_t>::iterator i = parent_to_num_children.begin(); i != parent_to_num_children.end();
         parent_to_num_children.erase(i++)) {
        if (!parent_to_actual_num_children.count(i->first)) continue;
        if (parent_to_actual_num_children[i->first] != i->second) {
            out << "Invalid: seq " << i->first << " recorded children: " << parent_to_actual_num_children[i->first]
                << " found: " << i->second << std::endl;
            errors++;
        }
...
string DBObjectMap::ghobject_key(const ghobject_t& oid)
{
    ...
    if (oid.generation != ghobject_t::NO_GEN || oid.shard_id != shard_id_t::NO_SHARD) {
...
int DBObjectMap::is_buggy_ghobject_key_v1(CephContext* cct, const string& in)
{
    int dots = 5;   // skip 5 .'s
...
ObjectMap::ObjectMapIterator DBObjectMap::get_iterator(const ghobject_t& oid)
{
    MapHeaderLock hl(this, oid);
    Header header = lookup_map_header(hl, oid);
    if (!header) return ObjectMapIterator(new EmptyIteratorImpl());
...
int DBObjectMap::DBObjectMapIteratorImpl::seek_to_last()
{
    ...
    if (parent_iter) {
        ...
        if (r < 0) return r;
        if (parent_iter->valid()) r = parent_iter->next();
        if (r < 0) return r;
    }
    r = key_iter->seek_to_last();
    if (r < 0) return r;
    if (key_iter->valid()) r = key_iter->next();
    if (r < 0) return r;
    return adjust();
}
...
{
    if (parent_iter && parent_iter->valid() && (!key_iter->valid() || key_iter->key() > parent_iter->key()))
        return true;
    return false;
}
...
int DBObjectMap::DBObjectMapIteratorImpl::in_complete_region(const string& to_test, string* begin, string* end)
{
    ...
    if (complete_iter->valid()) {
        ...
        if (!complete_iter->valid()) return false;
    }

    ceph_assert(complete_iter->key() <= to_test);
    ceph_assert(complete_iter->value().length() >= 1);
    string _end(complete_iter->value().c_str(), complete_iter->value().length() - 1);
    if (_end.empty() || _end > to_test) {
        if (begin) *begin = complete_iter->key();
        if (end) *end = _end;
        return true;
    }
    else {
    ...
    while (parent_iter && parent_iter->valid()) {
        if (in_complete_region(parent_iter->key(), &begin, &end)) {
            if (end.size() == 0) {
                ...
                if (parent_iter->valid()) parent_iter->next();
            }
            else
...
int DBObjectMap::set_keys(const ghobject_t& oid, const map<string, bufferlist>& set, const SequencerPosition* spos)
{
    KeyValueDB::Transaction t = db->get_transaction();
    MapHeaderLock hl(this, oid);
    Header header = lookup_create_map_header(hl, oid, t);
    if (!header) return -EINVAL;
    if (check_spos(oid, header, spos)) return 0;
...
int DBObjectMap::set_header(const ghobject_t& oid, const bufferlist& bl, const SequencerPosition* spos)
{
    KeyValueDB::Transaction t = db->get_transaction();
    MapHeaderLock hl(this, oid);
    Header header = lookup_create_map_header(hl, oid, t);
...
void DBObjectMap::_set_header(Header header, const bufferlist& bl, KeyValueDB::Transaction t)
{
...
int DBObjectMap::get_header(const ghobject_t& oid, bufferlist* bl)
{
    MapHeaderLock hl(this, oid);
    Header header = lookup_map_header(hl, oid);
    if (!header) {
...
int DBObjectMap::_get_header(Header header, bufferlist* bl)
{
    ...
    if (!out.empty()) bl->swap(out.begin()->second);
    return 0;
}

int DBObjectMap::clear(const ghobject_t& oid, const SequencerPosition* spos)
{
    KeyValueDB::Transaction t = db->get_transaction();
    MapHeaderLock hl(this, oid);
    Header header = lookup_map_header(hl, oid);
...
int DBObjectMap::_clear(Header header, KeyValueDB::Transaction t)
{
    while (1) {
...
int DBObjectMap::copy_up_header(Header header, KeyValueDB::Transaction t)
{
    bufferlist bl;
...
int DBObjectMap::rm_keys(const ghobject_t& oid, const set<string>& to_clear, const SequencerPosition* spos)
{
    MapHeaderLock hl(this, oid);
    Header header = lookup_map_header(hl, oid);
    if (!header) return -ENOENT;
...

    {
        ...
        for (iter->seek_to_first(); iter->valid(); iter->next()) {
            if (iter->status()) return iter->status();
            if (!to_clear.count(iter->key())) to_write[iter->key()] = iter->value();
        }
...
int DBObjectMap::clear_keys_header(const ghobject_t& oid, const SequencerPosition* spos)
{
    KeyValueDB::Transaction t = db->get_transaction();
    MapHeaderLock hl(this, oid);
    Header header = lookup_map_header(hl, oid);
    ...
    if (!attrs.empty()) t->set(xattr_prefix(newheader), attrs);
    return db->submit_transaction(t);
}

int DBObjectMap::get(const ghobject_t& oid, bufferlist* _header, map<string, bufferlist>* out)
{
    MapHeaderLock hl(this, oid);
    Header header = lookup_map_header(hl, oid);
    if (!header) return -ENOENT;
...
int DBObjectMap::get_keys(const ghobject_t& oid, set<string>* keys)
{
    MapHeaderLock hl(this, oid);
    Header header = lookup_map_header(hl, oid);
    if (!header) return -ENOENT;
...
int DBObjectMap::scan(Header header, const set<string>& in_keys, set<string>* out_keys,
                      map<string, bufferlist>* out_values)
{
    ObjectMapIterator db_iter = _get_iterator(header);
    for (set<string>::const_iterator key_iter = in_keys.begin(); key_iter != in_keys.end(); ++key_iter) {
        db_iter->lower_bound(*key_iter);
        if (db_iter->status()) return db_iter->status();
        if (db_iter->valid() && db_iter->key() == *key_iter) {
            if (out_keys) out_keys->insert(*key_iter);
            if (out_values) out_values->insert(make_pair(db_iter->key(), db_iter->value()));
        }
    }
    return 0;
}

int DBObjectMap::get_values(const ghobject_t& oid, const set<string>& keys, map<string, bufferlist>* out)
{
    MapHeaderLock hl(this, oid);
    Header header = lookup_map_header(hl, oid);
    if (!header) return -ENOENT;
...
int DBObjectMap::check_keys(const ghobject_t& oid, const set<string>& keys, set<string>* out)
{
    MapHeaderLock hl(this, oid);
    Header header = lookup_map_header(hl, oid);
    if (!header) return -ENOENT;
...
int DBObjectMap::get_all_xattrs(const ghobject_t& oid, set<string>* out)
{
    MapHeaderLock hl(this, oid);
    Header header = lookup_map_header(hl, oid);
    if (!header) return -ENOENT;
    KeyValueDB::Iterator iter = db->get_iterator(xattr_prefix(header));
    if (!iter) return -EINVAL;
    for (iter->seek_to_first(); !iter->status() && iter->valid(); iter->next()) out->insert(iter->key());
    return iter->status();
}
...
int DBObjectMap::remove_xattrs(const ghobject_t& oid, const set<string>& to_remove, const SequencerPosition* spos)
{
    KeyValueDB::Transaction t = db->get_transaction();
    MapHeaderLock hl(this, oid);
    Header header = lookup_map_header(hl, oid);
...
int DBObjectMap::legacy_clone(const ghobject_t& oid, const ghobject_t& target, const SequencerPosition* spos)
{
    state.legacy = true;

    if (oid == target) return 0;
    ...
    KeyValueDB::Transaction t = db->get_transaction();
    {
        ...
        if (destination) {
            if (check_spos(target, destination, spos)) return 0;
    ...
    for (xattr_iter->seek_to_first(); xattr_iter->valid(); xattr_iter->next())
...
int DBObjectMap::clone(const ghobject_t& oid, const ghobject_t& target, const SequencerPosition* spos)
{
    if (oid == target) return 0;
    ...
    if (!source) return db->submit_transaction(t);
...
int DBObjectMap::upgrade_to_v2()
{
    ...
    while (iter->valid()) {
        ...
        for (; iter->valid() && count < 300; iter->next()) {
...
int DBObjectMap::init(bool do_upgrade)
{
    ...
    if (ret < 0) return ret;
    if (state.v < 1) {
        dout(1) << "DBObjectMap is *very* old; upgrade to an older version first" << dendl;
        return -ENOTSUP;
    }
    if (state.v < 2) {   // Needs upgrade
        if (!do_upgrade) {
            dout(1) << "DOBjbectMap requires an upgrade,"
                    << " set filestore_update_to" << dendl;
            return -ENOTSUP;
        }
        else {
            int r = upgrade_to_v2();
...
int DBObjectMap::sync(const ghobject_t* oid, const SequencerPosition* spos)
{
    KeyValueDB::Transaction t = db->get_transaction();
    if (oid) {
        ceph_assert(spos);
...
DBObjectMap::Header DBObjectMap::_lookup_map_header(const MapHeaderLock& l, const ghobject_t& oid)
{
...
DBObjectMap::Header DBObjectMap::lookup_create_map_header(const MapHeaderLock& hl, const ghobject_t& oid,
...
void DBObjectMap::clear_header(Header header, KeyValueDB::Transaction t)
{
    ...
    if (state.legacy) t->rmkeys_by_prefix(complete_prefix(header));   // Needed when header.parent != 0
...
void DBObjectMap::remove_map_header(const MapHeaderLock& l, const ghobject_t& oid, Header header,
                                    KeyValueDB::Transaction t)
{
    ceph_assert(l.get_locked() == oid);
...
void DBObjectMap::set_map_header(const MapHeaderLock& l, const ghobject_t& oid, _Header header,
                                 KeyValueDB::Transaction t)
{
    ceph_assert(l.get_locked() == oid);
...
bool DBObjectMap::check_spos(const ghobject_t& oid, Header header, const SequencerPosition* spos)
{
...
{
    out << "seq=" << h.seq << " parent=" << h.parent << " num_children=" << h.num_children << " ghobject=" << h.oid;
    return out;
}

int DBObjectMap::rename(const ghobject_t& from, const ghobject_t& to, const SequencerPosition* spos)
{
    if (from == to) return 0;
    ...
    KeyValueDB::Transaction t = db->get_transaction();
    {
        ...
        if (destination) {
            if (check_spos(to, destination, spos)) return 0;
            destination->num_children--;
    ...
    if (!hdr) return db->submit_transaction(t);
