...
int FileJournal::_open(bool forwrite, bool create)
{
    ...
    if (aio) {
        ...
        if (ret < 0) {
            ...
            case -EAGAIN:
                ...
                break;
            default: derr << "FileJournal::_open: unable to setup io_context " << cpp_strerror(-ret) << dendl; break;
            }
            goto out_fd;
        }
    }
    ...
    dout(1) << "_open " << fn << " fd " << fd << ": " << max_size << " bytes, block size " << block_size
...
int FileJournal::_open_file(int64_t oldsize, blksize_t blksize, bool create)
{
    ...
    if (create && (oldsize < conf_journal_sz)) {
        ...
        if (ret < 0) {
            int err = errno;
            derr << "FileJournal::_open_file : unable to extend journal to " << newsize
                 << " bytes: " << cpp_strerror(err) << dendl;
            return -err;
        }
        ret = ceph_posix_fallocate(fd, 0, newsize);
        if (ret) {
            derr << "FileJournal::_open_file : unable to preallocation journal to " << newsize
...
int FileJournal::create()
{
    ...
    if (ret) {
        ret = -ret;
        derr << "FileJournal::create: failed to allocate " << block_size << " bytes of memory: " << cpp_strerror(ret)
             << dendl;
        goto close_fd;
    }
...
int FileJournal::open(uint64_t fs_op_seq)
{
    ...
    if (header.max_size % header.block_size) {
        dout(2) << "open journal max size " << header.max_size << " not a multiple of block size " << header.block_size
                << dendl;
        err = -EINVAL;
        goto out;
    }
    if (header.alignment != block_size && directio) {
        dout(0) << "open journal alignment " << header.alignment << " does not match block size " << block_size
                << " (required for direct_io journal mode)" << dendl;
        err = -EINVAL;
        goto out;
    }
    if ((header.alignment % CEPH_DIRECTIO_ALIGNMENT) && directio) {
        dout(0) << "open journal alignment " << header.alignment << " is not multiple of minimum directio alignment "
    ...
    while (1) {
        ...
        if (seq > next_seq) {
            dout(10) << "open entry " << seq << " len " << bl.length() << " > next_seq " << next_seq
                     << ", ignoring journal contents" << dendl;
            read_pos = -1;
        ...
        if (result != SUCCESS) {
            if (seq < header.committed_up_to) {
                dout(2) << "Unable to read past sequence " << seq
                        << " but header indicates the journal has committed up through " << header.committed_up_to
...
{
    dout(10) << "header: block_size " << header.block_size << " alignment " << header.alignment << " max_size "
             << header.max_size << dendl;
...
int FileJournal::check_for_full(uint64_t seq, off64_t pos, off64_t size)
{
    ...
    if (do_sync_cond) {
        if (room >= (header.max_size >> 1) && room - size < (header.max_size >> 1)) {
    ...
    // full
    dout(1) << "check_for_full at " << pos << " : JOURNAL FULL " << pos << " >= " << room << " (max_size "
            << header.max_size << " start " << header.start << ")" << dendl;

    off64_t max = header.max_size - get_top();
    if (size > max)
        dout(0) << "JOURNAL TOO SMALL: continuing, but slow: item " << size << " > journal " << max << " (usable)"
                << dendl;
...
int FileJournal::prepare_multi_write(bufferlist& bl, uint64_t& orig_ops, uint64_t& orig_bytes)
{
    ...
    while (!writeq_empty()) {
        ...
        while (it != items.end()) {
            ...
            if (eleft) {
                if (--eleft == 0) {
                    dout(20) << "prepare_multi_write hit max events per write " << cct->_conf->journal_max_write_entries
                             << dendl;
                    batch_unpop_write(items);
                    goto out;
                }
            ...
            if (bmax) {
                if (bl.length() >= bmax) {
                    dout(20) << "prepare_multi_write hit max write size " << cct->_conf->journal_max_write_bytes
                             << dendl;
                    batch_unpop_write(items);
                    goto out;
                }
...
void FileJournal::queue_completions_thru(uint64_t seq)
{
    ...
    while (it != items.end()) {
        ...
        utime_t lat = now;
        lat -= next.start;
        dout(10) << "queue_completions_thru seq " << seq << " queueing seq " << next.seq << " " << next.finish
                 << " lat " << lat << dendl;
        if (logger) {
...
int FileJournal::prepare_single_write(write_item& next_write, bufferlist& bl, off64_t& queue_pos, uint64_t& orig_ops,
                                      uint64_t& orig_bytes)
{
    ...
    // add to write buffer
    dout(15) << "prepare_single_write " << orig_ops << " will write " << queue_pos << " : seq " << seq << " len "
             << orig_len << " -> " << size << dendl;
    ...
    queue_pos += size;
    if (queue_pos >= header.max_size) queue_pos = queue_pos + get_top() - header.max_size;
...
void FileJournal::do_write(bufferlist& bl)
{
    ...
    if (cct->_conf->journal_write_header_frequency &&
        (((++journaled_since_start) % cct->_conf->journal_write_header_frequency) == 0)) {
        must_write_header = true;
    }
    ...
    // Adjust write_pos
    write_pos += bl.length();
    if (write_pos >= header.max_size) write_pos = write_pos - header.max_size + get_top();
    ...
    if (pos + bl.length() > header.max_size) {
        ...
        dout(10) << "do_write wrapping, first bit at " << pos << " len " << first.length() << " second bit len "
                 << second.length() << " (orig len " << bl.length() << ")" << dendl;
        ...
        if (hbp.length()) {
            if (TEMP_FAILURE_RETRY(::pwrite(fd, hbp.c_str(), hbp.length(), 0)) < 0) {
                int err = errno;
                derr << "FileJournal::do_write: pwrite(fd=" << fd << ", hbp.length=" << hbp.length()
    ...
    if (!directio) {
        ...
        if (cct->_conf->filestore_fadvise) posix_fadvise(fd, 0, 0, POSIX_FADV_DONTNEED);
#endif
    }
...

    {
        ...
        if (full_state != FULL_NOTFULL) {
            dout(10) << "do_write NOT queueing finisher seq " << journaled_seq << ", full_commit_seq|full_restart_seq"
                     << dendl;
        }
        else {
            if (plug_journal_completions) {
                dout(20) << "do_write NOT queueing finishers through seq " << journaled_seq << " due to completion plug"
                         << dendl;
            }
            else {
        ...
        if (aio) {
            ...
            while (aio_num > 0) {
                ...
                dout(20) << "write_thread_entry aio throttle: aio num " << aio_num << " bytes " << aio_bytes
                         << " ... exp " << exp << " min_new " << min_new << " ... pending " << cur << dendl;
                if (cur >= min_new) break;
                dout(20) << "write_thread_entry deferring until more aios complete: " << aio_num << " aios with "
                         << aio_bytes << " bytes needs " << min_new << " bytes to start a new aio (currently " << cur
                         << " pending)" << dendl;
...
int FileJournal::prepare_entry(vector<ObjectStore::Transaction>& tls, bufferlist* tbl)
{
    ...
    memset(&h, 0, sizeof(h));
    if (data_align >= 0) h.pre_pad = ((unsigned int)data_align - (unsigned int)head_size) & ~CEPH_PAGE_MASK;
    ...
    dout(10) << " len " << bl.length() << " -> " << size << " (head " << head_size << " pre_pad " << h.pre_pad << " bl "
             << bl.length() << " post_pad " << post_pad << " tail " << head_size << ")"
...
void FileJournal::submit_entry(uint64_t seq, bufferlist& e, uint32_t orig_len, Context* oncommit, TrackedOpRef osd_op)
{
    ...
    }
    {
        ...
        completions.push_back(completion_item(seq, oncommit, ceph_clock_now(), osd_op));
...
void FileJournal::commit_start(uint64_t seq)
{
    ...
    // was full?
    switch (full_state) {
    case FULL_NOTFULL: break;   // all good

    case FULL_FULL:
        if (seq >= journaled_seq) {
            dout(1) << " FULL_FULL -> FULL_WAIT.  commit_start on seq " << seq << " > journaled_seq " << journaled_seq
                    << ", moving to FULL_WAIT." << dendl;
            full_state = FULL_WAIT;
        }
        else {
            dout(1) << "FULL_FULL commit_start on seq " << seq << " < journaled_seq " << journaled_seq
...
void FileJournal::committed_thru(uint64_t seq)
{
    ...
    while (!writeq_empty() && peek_write().seq <= seq) {
        dout(15) << " dropping committed but unwritten seq " << peek_write().seq << " len " << peek_write().bl.length()
                 << dendl;
        complete_write(1, peek_write().orig_len);
        pop_write();
    }
...
{
    stringstream ss;
    bool valid = throttle.set_params(cct->_conf->journal_throttle_low_threshhold,
...
{
    static const char* KEYS[] = {"journal_throttle_low_threshhold",
...
void FileJournal::wrap_read_bl(off64_t pos, int64_t olen, bufferlist* bl, off64_t* out_pos) const
{
    while (olen > 0) {
...
bool FileJournal::read_entry(bufferlist& bl, uint64_t& next_seq, bool* corrupt)
{
    ...
    if (result == SUCCESS) {
        ...
        uint64_t amount_to_take = next_pos > pos ? next_pos - pos : (header.max_size - pos) + (next_pos - get_top());
    ...
    if (seq && seq < header.committed_up_to) {
        derr << "Unable to read past sequence " << seq << " but header indicates the journal has committed up through "
...
FileJournal::read_entry_result FileJournal::do_read_entry(off64_t init_pos, off64_t* next_pos, bufferlist* bl,
                                                          uint64_t* seq, ostream* ss, entry_header_t* _h) const
{
    ...
    if ((header.flags & header_t::FLAG_CRC) ||   // if explicitly enabled (new journal)
        ...
        if (actual_crc != h->crc32c) {
            if (ss) *ss << "header crc (" << h->crc32c << ") doesn't match body crc (" << actual_crc << ")";
            if (next_pos) *next_pos = cur_pos;
            return MAYBE_CORRUPT;
        }
    }
...
void FileJournal::get_header(uint64_t wanted_seq, off64_t* _pos, entry_header_t* h)
{
...
{
    dout(2) << __func__ << dendl;
    if (corrupt_at >= header.max_size) corrupt_at = corrupt_at + get_top() - header.max_size;
...
void FileJournal::corrupt_footer_magic(int wfd, uint64_t seq)
{
    ...
    entry_header_t h;
    get_header(seq, &pos, &h);
    off64_t corrupt_at = pos + sizeof(entry_header_t) + h.pre_pad + h.len + h.post_pad +
...
void FileJournal::corrupt_header_magic(int wfd, uint64_t seq)
{
    ...
    entry_header_t h;
    get_header(seq, &pos, &h);
    off64_t corrupt_at = pos + (reinterpret_cast<char*>(&h.magic2) - reinterpret_cast<char*>(&h));
    corrupt(wfd, corrupt_at);
}
