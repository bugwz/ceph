    ...
    , fs(XFS)
{
    ...
    ceph_assert(cct);
    PerfCountersBuilder b(cct, string("WBThrottle"), l_wbthrottle_first, l_wbthrottle_last);
    ...
    for (unsigned i = l_wbthrottle_first + 1; i != l_wbthrottle_last; ++i) logger->set(i, 0);
...
{
    static const char* KEYS[] = {"filestore_wbthrottle_btrfs_bytes_start_flusher",
...
void WBThrottle::set_from_conf()
{
    ...
    if (fs == BTRFS) {
        size_limits.first = cct->_conf->filestore_wbthrottle_btrfs_bytes_start_flusher;
        size_limits.second = cct->_conf->filestore_wbthrottle_btrfs_bytes_hard_limit;
        ...
        size_limits.first = cct->_conf->filestore_wbthrottle_xfs_bytes_start_flusher;
...
void WBThrottle::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    std::lock_guard l{lock};
...
bool WBThrottle::get_next_should_flush(std::unique_lock<ceph::mutex>& locker,
                                       boost::tuple<ghobject_t, FDRef, PendingWB>* next)
{
    ceph_assert(ceph_mutex_is_locked(lock));
    ceph_assert(next);
    {
        cond.wait(locker, [this] { return stopping || (beyond_limit() && !pending_wbs.empty()); });
    }
    if (stopping) return false;
    ceph_assert(!pending_wbs.empty());
    ghobject_t obj(pop_object());

    ceph::unordered_map<ghobject_t, pair<PendingWB, FDRef> >::iterator i = pending_wbs.find(obj);
...
void WBThrottle::queue_wb(FDRef fd, const ghobject_t& hoid, uint64_t offset, uint64_t len, bool nocache)
{
    std::lock_guard l{lock};
    ceph::unordered_map<ghobject_t, pair<PendingWB, FDRef> >::iterator wbiter = pending_wbs.find(hoid);
    if (wbiter == pending_wbs.end()) {
        wbiter = pending_wbs.insert(make_pair(hoid, make_pair(PendingWB(), fd))).first;
...
{
    std::lock_guard l{lock};
    for (ceph::unordered_map<ghobject_t, pair<PendingWB, FDRef> >::iterator i = pending_wbs.begin();
         i != pending_wbs.end();
         ++i) {
#ifdef HAVE_POSIX_FADVISE
...
void WBThrottle::clear_object(const ghobject_t& hoid)
{
    ...
    ceph::unordered_map<ghobject_t, pair<PendingWB, FDRef> >::iterator i = pending_wbs.find(hoid);
