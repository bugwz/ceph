...
static int set_version(const char* path, uint32_t version)
{
    bufferlist bl;
    encode(version, bl);
    return chain_setxattr<true, true>(path, "user.cephos.collection_version", bl.c_str(), bl.length());
}

static int get_version(const char* path, uint32_t* version)
{
    bufferptr bp(PATH_MAX);
    int r = chain_getxattr(path, "user.cephos.collection_version", bp.c_str(), bp.length());
    if (r < 0) {
        if (r != -ENOENT) {
            *version = 0;
            return 0;
...
IndexManager::~IndexManager()
{
    ...
    for (ceph::unordered_map<coll_t, CollectionIndex*>::iterator it = col_indices.begin(); it != col_indices.end();
         ++it) {
...
int IndexManager::build_index(coll_t c, const char* path, CollectionIndex** index)
{
    if (upgrade) {
        ...
        case CollectionIndex::HOBJECT_WITH_POOL:
        {
            // Must be a HashIndex
            *index = new HashIndex(
                cct, c, path, cct->_conf->filestore_merge_threshold, cct->_conf->filestore_split_multiple, version);
            return (*index)->read_settings();
        }
        default: ceph_abort();
        }
