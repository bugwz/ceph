...
int ZFSFileStoreBackend::update_current_zh()
{
    ...
    if (zh) {
        ...
        if (zfs.is_mounted(zh, &mnt)) {
            ...
            if (ret < 0) {
                ret = -errno;
                dout(0) << "update_current_zh: zfs_mount '" << zfs.get_name(zh) << "' got " << cpp_strerror(ret)
                        << dendl;
                return ret;
            }
        }
...
{
    if (!current_zh) dout(0) << "detect_features: null zfs handle for current/" << dendl;
    return 0;
}
...
int ZFSFileStoreBackend::rollback_to(const string& name)
{
    ...
    if (!zfs.is_mounted(current_zh, NULL)) {
        ...
        if (ret < 0) {
            ret = -errno;
            dout(0) << "update_current_zh: zfs_mount '" << zfs.get_name(current_zh) << "' got " << cpp_strerror(ret)
                    << dendl;
            return ret;
        }
    }
