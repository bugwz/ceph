...
public:
    ...
    struct TransactionData
    {
        ...
        void encode(ceph::buffer::list& bl) const { bl.append((char*)this, sizeof(TransactionData)); }
        void decode(ceph::buffer::list::const_iterator& bl) { bl.copy(sizeof(TransactionData), (char*)this); }
    } __attribute__((packed));
...
public:
    ...
    explicit Transaction(ceph::buffer::list::const_iterator& dp) { decode(dp); }
    ...
    bool has_contexts() const { return !on_commit.empty() || !on_applied.empty() || !on_applied_sync.empty(); }

    static void collect_contexts(std::vector<Transaction>& t, Context** out_on_applied, Context** out_on_commit,
    ...
    static void collect_contexts(std::vector<Transaction>& t, std::list<Context*>* out_on_applied,
                                 std::list<Context*>* out_on_commit, std::list<Context*>* out_on_applied_sync)
    {
        ceph_assert(out_on_applied);
        ceph_assert(out_on_commit);
        ceph_assert(out_on_applied_sync);
    ...
    Context* get_on_applied() { return C_Contexts::list_to_context(on_applied); }
    Context* get_on_commit() { return C_Contexts::list_to_context(on_commit); }
    Context* get_on_applied_sync() { return C_Contexts::list_to_context(on_applied_sync); }

    void set_fadvise_flags(uint32_t flags) { data.fadvise_flags = flags; }
    void set_fadvise_flag(uint32_t flag) { data.fadvise_flags = data.fadvise_flags | flag; }
    ...
    void _update_op(Op* op, std::vector<uint32_t>& cm, std::vector<uint32_t>& om)
    {
        ...
        default: ceph_abort_msg("Unknown OP");
        }
    }
    void _update_op_bl(ceph::buffer::list& bl, std::vector<uint32_t>& cm, std::vector<uint32_t>& om)
    {
    ...
    /// Append the operations of the parameter to this Transaction. Those operations are removed from the parameter
    /// Transaction
    void append(Transaction& other)
    {
        ...
        for (coll_index_p = other.coll_index.begin(); coll_index_p != other.coll_index.end(); ++coll_index_p) {
        ...
        for (object_index_p = other.object_index.begin(); object_index_p != other.object_index.end();
             ++object_index_p) {
    ...
    uint64_t get_num_bytes() { return get_encoded_bytes(); }
    /// Size of largest data buffer to the "write" operation encountered so far
    uint32_t get_data_length() { return data.largest_data_len; }
    ...
    uint32_t get_data_offset()
    {
        if (data.largest_data_off_in_data_bl) {
            return data.largest_data_off_in_data_bl + sizeof(__u8) +   // encode struct_v
    ...
    bool empty() { return !data.ops; }
    /// Number of operations in the transaction
    int get_num_ops() { return data.ops; }
    ...
    private:
            ...
            , objects(t->object_index.size())
        {
            ...
            std::map<coll_t, uint32_t>::iterator coll_index_p;
            for (coll_index_p = t->coll_index.begin(); coll_index_p != t->coll_index.end(); ++coll_index_p) {
                colls[coll_index_p->second] = coll_index_p->first;
            }

            std::map<ghobject_t, uint32_t>::iterator object_index_p;
            for (object_index_p = t->object_index.begin(); object_index_p != t->object_index.end(); ++object_index_p) {
    ...
    public:
        ...
        void decode_attrset_bl(ceph::buffer::list* pbl) { decode_str_str_map_to_bl(data_bl_p, pbl); }
        void decode_keyset(std::set<std::string>& keys)
        {
            using ceph::decode;
            decode(keys, data_bl_p);
        }
        void decode_keyset_bl(ceph::buffer::list* pbl) { decode_str_set_to_bl(data_bl_p, pbl); }
        ...
        uint32_t get_fadvise_flags() const { return t->get_fadvise_flags(); }

        const std::vector<ghobject_t>& get_objects() const { return objects; }
    };
...
public:
     ...
     */
    void clone(const coll_t& cid, const ghobject_t& oid, const ghobject_t& noid)
    {
        Op* _op = _get_next_op();
        _op->op = OP_CLONE;
        _op->cid = _get_coll_id(cid);
     ...
     */
    void clone_range(const coll_t& cid, const ghobject_t& oid, const ghobject_t& noid, uint64_t srcoff, uint64_t srclen,
                     uint64_t dstoff)
    {
        Op* _op = _get_next_op();
    ...
    void collection_move(const coll_t& cid, const coll_t& oldcid, const ghobject_t& oid) __attribute__((deprecated))
    {
    ...
    void collection_move_rename(const coll_t& oldcid, const ghobject_t& oldoid, const coll_t& cid,
    ...
    void try_rename(const coll_t& cid, const ghobject_t& oldoid, const ghobject_t& oid)
    {
        Op* _op = _get_next_op();
    ...
    void omap_clear(const coll_t& cid,      ///< [in] Collection containing oid
    ...
    void omap_setkeys(const coll_t& cid,                                         ///< [in] Collection containing oid
    ...
    void omap_rmkeys(const coll_t& cid,                  ///< [in] Collection containing oid
    ...
    void omap_rmkey(const coll_t& cid,       ///< [in] Collection containing oid
                    const ghobject_t& oid,   ///< [in] Object from which to remove the omap
    ...
    void omap_rmkeyrange(const coll_t& cid,          ///< [in] Collection containing oid
    ...
    /// Set omap header
    void omap_setheader(const coll_t& cid,             ///< [in] Collection containing oid
    ...
    void split_collection(const coll_t& cid, uint32_t bits, uint32_t rem, const coll_t& destination)
    {
        Op* _op = _get_next_op();
    ...
    void merge_collection(coll_t cid, coll_t destination, uint32_t bits)
    {
        Op* _op = _get_next_op();
    ...
    void collection_set_bits(const coll_t& cid, int bits)
    {
        Op* _op = _get_next_op();
    ...
    void set_alloc_hint(const coll_t& cid, const ghobject_t& oid, uint64_t expected_object_size,
