...
int FuseStore::open_file(string p, struct fuse_file_info* fi, std::function<int(bufferlist* bl)> f)
{
...
static int parse_fn(CephContext* cct, const char* path, coll_t* cid, ghobject_t* oid, string* key, uint32_t* hash,
                    uint32_t* hash_bits)
{
    list<string> v;
    ...
    if (v.empty()) return FN_ROOT;

    if (v.front() == "type") return FN_TYPE;

    if (!cid->parse(v.front())) {
        return -ENOENT;
    }
    if (v.size() == 1) return FN_COLLECTION;
    v.pop_front();

    if (v.front() == "bitwise_hash_start") return FN_HASH_START;
    if (v.front() == "bitwise_hash_end") return FN_HASH_END;
    if (v.front() == "bitwise_hash_bits") return FN_HASH_BITS;
    if (v.front() == "pgmeta") {
        spg_t pgid;
        if (cid->is_pg(&pgid)) {
            *oid = pgid.make_pgmeta_oid();
            v.pop_front();
            if (v.empty()) return FN_OBJECT;
            goto do_object;
        }
        return -ENOENT;
    }
    if (v.front() == "all") {
        v.pop_front();
        if (v.empty()) return FN_ALL;
        goto do_dir;
    }
    if (v.front() == "by_bitwise_hash") {
        v.pop_front();
        if (v.empty()) return FN_HASH_DIR;
        unsigned long hv, hm;
        ...
        if (r != 1) return -ENOENT;
        if (hm < 1 || hm > 32) return -ENOENT;
        v.pop_front();
        *hash = hv << shift;   // hobject_t::_reverse_bits(hv << shift);
        *hash_bits = hm;
        if (v.empty()) return FN_HASH_VAL;
        goto do_dir;
    }
    return -ENOENT;
...
do_object:
    if (v.front() == "data") return FN_OBJECT_DATA;
    if (v.front() == "omap_header") return FN_OBJECT_OMAP_HEADER;
    if (v.front() == "omap") {
        v.pop_front();
        if (v.empty()) return FN_OBJECT_OMAP;
        *key = v.front();
        v.pop_front();
        if (v.empty()) return FN_OBJECT_OMAP_VAL;
        return -ENOENT;
    }
    if (v.front() == "attr") {
        v.pop_front();
        if (v.empty()) return FN_OBJECT_ATTR;
        *key = v.front();
        v.pop_front();
        if (v.empty()) return FN_OBJECT_ATTR_VAL;
        return -ENOENT;
    }
    if (v.front() == "bitwise_hash") return FN_OBJECT_HASH;
    return -ENOENT;
}
...
)
{
    ...
    int t = parse_fn(fs->store->cct, path, &cid, &oid, &key, &hash_value, &hash_bits);
    if (t < 0) return t;
    ...
    case FN_OBJECT_ATTR:
    case FN_OBJECT:
        if (!fs->store->exists(ch, oid)) return -ENOENT;
        // fall-thru
    case FN_ALL:
    case FN_HASH_DIR:
    case FN_HASH_VAL:
    case FN_COLLECTION:
        if (!fs->store->collection_exists(cid)) return -ENOENT;
        // fall-thru
    case FN_ROOT: stbuf->st_mode = S_IFDIR | 0700; return 0;

    case FN_TYPE: stbuf->st_size = fs->store->get_type().length() + 1; break;
    ...
    case FN_HASH_END:
        if (!ch) return -ENOENT;
        if (fs->store->collection_bits(ch) < 0) return -ENOENT;
        // fall-thru
    case FN_HASH_START: stbuf->st_size = 9; return 0;

    case FN_HASH_BITS:
    {
        if (!ch) return -ENOENT;
        int bits = fs->store->collection_bits(ch);
        if (bits < 0) return -ENOENT;
        char buf[12];
...
static int os_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
                      ,
...
)
{
    ...
    case FN_COLLECTION:
    {
        ...
        spg_t pgid;
        if (cid.is_pg(&pgid) && fs->store->exists(ch, pgid.make_pgmeta_oid())) {
    ...
    case FN_ALL:
    {
        ...
        ldout(fs->store->cct, 10) << __func__ << std::hex << " offset " << offset << " hash "
                                  << hobject_t::_reverse_bits(hash_value) << std::dec << "/" << hash_bits << " first "
                                  << next << " last " << last << dendl;
        while (true) {
            vector<ghobject_t> ls;
            int r = fs->store->collection_list(ch, next, last, 1000, &ls, &next);
            if (r < 0) return r;
            for (auto p : ls) {
                if (skip) {
                    --skip;
                ...
                ++hashoff;
                uint64_t cur_off = ((uint64_t)bitwise_hash << hash_shift) | (uint64_t)hashoff;
                string s = stringify(p);
                r = filler_compat(filler, buf, s.c_str(), NULL, cur_off);
                if (r) break;
            }
            if (r) break;
            if (next == ghobject_t::get_max() || next == last) break;
        }
    } break;
...
static int os_open(const char* path, struct fuse_file_info* fi)
{
    ...
    case FN_OBJECT_DATA:
    {
        int r = fs->open_file(path, fi, [&](bufferlist* pbl) { return fs->store->read(ch, oid, 0, 0, *pbl); });
        if (r < 0) {
            return r;
        }
    } break;
    ...
    case FN_OBJECT_ATTR_VAL:
    {
        int r = fs->open_file(path, fi, [&](bufferlist* pbl) {
            bufferptr bp;
    ...
    case FN_OBJECT_OMAP_HEADER:
    {
        int r = fs->open_file(path, fi, [&](bufferlist* pbl) { return fs->store->omap_get_header(ch, oid, pbl); });
        if (r < 0) return r;
    } break;
    }
...
static int os_mkdir(const char* path, mode_t mode)
{
    ...
    string key;
    uint32_t hash_value, hash_bits;
    int f = parse_fn(fs->store->cct, path, &cid, &oid, &key, &hash_value, &hash_bits);
    if (f < 0) return f;
...
static int os_read(const char* path, char* buf, size_t size, off_t offset, struct fuse_file_info* fi)
{
    ...
    if (offset + size > o->bl.length()) size = o->bl.length() - offset;
    bufferlist r;
...
int os_flush(const char* path, struct fuse_file_info* fi)
{
    ...
    switch (f) {
    case FN_OBJECT_DATA: t.write(cid, oid, 0, o->bl.length(), o->bl); break;
    ...
    case FN_OBJECT_OMAP_HEADER: t.omap_setheader(cid, oid, o->bl); break;
...
static int os_unlink(const char* path)
{
    ...
    case FN_OBJECT_ATTR_VAL: t.rmattr(cid, oid, key.c_str()); break;
    ...
    case FN_OBJECT: t.remove(cid, oid); break;
    ...
    case FN_OBJECT_DATA: t.truncate(cid, oid, 0); break;
...
)
{
    ...
    if (f == FN_OBJECT_OMAP_VAL || f == FN_OBJECT_ATTR_VAL || f == FN_OBJECT_OMAP_HEADER) {
        if (size) return -EPERM;
        return 0;
    }
    if (f != FN_OBJECT_DATA) return -EPERM;
...
int FuseStore::start()
{
    ...
    info->f = fuse_new(info->ch, &info->args, &fs_oper, sizeof(fs_oper), (void*)this);
    if (!info->f) {
...
int FuseStore::loop()
{
    ...
    if (r) derr << __func__ << " got " << cpp_strerror(r) << dendl;
