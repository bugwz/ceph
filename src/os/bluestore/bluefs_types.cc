...
{
    return out << (int)e.bdev << ":0x" << std::hex << e.offset << "~" << e.length << std::dec;
}
...
{
    return out << "super(uuid " << s.uuid << " osd " << s.osd_uuid << " v " << s.version << " block_size 0x" << std::hex
               << s.block_size << " log_fnode 0x" << s.log_fnode << std::dec << ")";
}

// bluefs_fnode_t

mempool::bluefs::vector<bluefs_extent_t>::iterator bluefs_fnode_t::seek(uint64_t offset, uint64_t* x_off)
{
    auto p = extents.begin();

    if (extents_index.size() > 4) {
        auto it = std::upper_bound(extents_index.begin(), extents_index.end(), offset);
...
{
    return out << "file(ino " << file.ino << " size 0x" << std::hex << file.size << std::dec << " mtime " << file.mtime
               << " allocated " << std::hex << file.allocated << std::dec << " alloc_commit " << std::hex
               << file.allocated_commited << std::dec << " extents " << file.extents << ")";
}
...
{
    return out << "delta(ino " << delta.ino << " size 0x" << std::hex << delta.size << std::dec << " mtime "
               << delta.mtime << " offset " << std::hex << delta.offset << std::dec << " extents " << delta.extents
               << ")";
}
...
void bluefs_transaction_t::decode(bufferlist::const_iterator& p)
{
    ...
    if (actual != crc)
        throw ceph::buffer::malformed_input("bad crc " + stringify(actual) + " expected " + stringify(crc));
}
...
void bluefs_transaction_t::generate_test_instances(list<bluefs_transaction_t*>& ls)
{
...
{
    return out << "txn(seq " << t.seq << " len 0x" << std::hex << t.op_bl.length() << " crc 0x" << t.op_bl.crc32c(-1)
               << std::dec << ")";
}
