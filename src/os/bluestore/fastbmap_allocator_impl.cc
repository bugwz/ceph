...
{
    return len >= min_length ? interval_t(offset, p2align<uint64_t>(len, min_length)) : interval_t();
}

interval_t AllocatorLevel01Loose::_get_longest_from_l0(uint64_t pos0, uint64_t pos1, uint64_t min_length,
        ...
        if ((pos % d) == 0) {
            ...
            if (pos1 - pos >= d) {
                ...
                case all_slot_set:
                    ...
                    if (end_loop) {
                        *tail = res_candidate;
                        res_candidate = _align2units(res_candidate.offset, res_candidate.length, min_granules);
...
void AllocatorLevel01Loose::_analyze_partials(uint64_t pos_start, uint64_t pos_end, uint64_t length,
    ...
    for (auto pos = pos_start / d; pos < pos_end / d; ++pos) {
        ...
        for (auto c = 0; c < d; c++) {
            ...
            case L1_ENTRY_FREE:
                ...
                break;
            case L1_ENTRY_FULL: prev_tail = empty_tail; break;
            case L1_ENTRY_PARTIAL:
                ...
                if (longest.length >= length) {
                    if ((ctx->affordable_len == 0) ||
                        ((ctx->affordable_len != 0) && (longest.length < ctx->affordable_len))) {
                        ctx->affordable_len = longest.length;
                        ctx->affordable_offs = longest.offset;
                    }
                }
                if (longest.length >= min_length &&
                    (ctx->min_affordable_len == 0 || (longest.length < ctx->min_affordable_len))) {
...
void AllocatorLevel01Loose::_mark_l1_on_l0(int64_t l0_pos, int64_t l0_pos_end)
{
    ...
    while (idx < idx_end) {
        ...
        if ((idx % slots_per_slotset) == 0) {
            ...
            switch (old_mask) {
            case L1_ENTRY_FREE: unalloc_l1_count--; break;
            case L1_ENTRY_PARTIAL: partial_l1_count--; break;
            }
            slot_val &= ~mask;
...
void AllocatorLevel01Loose::_mark_alloc_l0(int64_t l0_pos_start, int64_t l0_pos_end)
{
...
interval_t AllocatorLevel01Loose::_allocate_l1_contiguous(uint64_t length, uint64_t min_length, uint64_t max_length,
                                                          uint64_t pos_start, uint64_t pos_end)
{
    ...
    if (unlikely(length <= l0_granularity)) {
        search_ctx_t ctx;
        _analyze_partials(pos_start, pos_end, l0_granularity, l0_granularity, STOP_ON_PARTIAL, &ctx);
        ...
        if (ctx.free_count) {
            ...
            if (aligned_extent.length > 0) {
                aligned_extent.length = std::min(length, uint64_t(aligned_extent.length));
...
bool AllocatorLevel01Loose::_allocate_l1(uint64_t length, uint64_t min_length, uint64_t max_length,
                                         uint64_t l1_pos_start, uint64_t l1_pos_end, uint64_t* allocated,
                                         interval_vector_t* res)
{
    ...
    if (min_length != l0_granularity) {
        ...
        while (length > *allocated && has_space) {
            interval_t i =
                _allocate_l1_contiguous(length - *allocated, min_length, max_length, l1_pos_start, l1_pos_end);
            if (i.length == 0) {
                has_space = false;
            }
            else {
        ...
        for (auto idx = l1_pos_start / d1; idx < l1_pos_end / d1 && length > *allocated; ++idx) {
            ...
            if (slot_val == all_slot_clear) {
                ...
                *allocated += to_alloc;
                ...
                _fragment_and_emplace(max_length, idx * d1 * l1_granularity, to_alloc, res);
...
void AllocatorLevel01Loose::collect_stats(std::map<size_t, size_t>& bins_overall)
{
...
inline ssize_t AllocatorLevel01Loose::count_0s(slot_t slot_val, size_t start_pos)
{
    ...
    if (pos == 0) return sizeof(slot_t) * 8 - start_pos;
    return pos - 1;
#else
...
void AllocatorLevel01Loose::foreach_internal(std::function<void(uint64_t offset, uint64_t length)> notify)
{
    size_t len = 0;
    size_t off = 0;
    for (size_t i = 0; i < l1.size(); i++) {
        for (size_t j = 0; j < L1_ENTRIES_PER_SLOT * L1_ENTRY_WIDTH; j += L1_ENTRY_WIDTH) {
            ...
            case L1_ENTRY_FULL:
                ...
                break;
            case L1_ENTRY_FREE:
                if (len == 0) off = ((bits_per_slot * i + j) / L1_ENTRY_WIDTH) * slots_per_slotset * bits_per_slot;
