...
void bluestore_bdev_label_t::generate_test_instances(list<bluestore_bdev_label_t*>& o)
{
...
{
    return out << "bdev(osd_uuid " << l.osd_uuid << ", size 0x" << std::hex << l.size << std::dec << ", btime "
               << l.btime << ", desc " << l.description << ", " << l.meta.size() << " meta"
               << ")";
}
...
void bluestore_extent_ref_map_t::_maybe_merge_left(map<uint64_t, record_t>::iterator& p)
{
    ...
    auto q = p;
    --q;
    if (q->second.refs == p->second.refs && q->first + q->second.length == p->first) {
...
void bluestore_extent_ref_map_t::get(uint64_t offset, uint32_t length)
{
    ...
    while (length > 0) {
        if (p == ref_map.end()) {
            ...
            p = ref_map.insert(map<uint64_t, record_t>::value_type(offset, record_t(length, 1))).first;
            break;
        }
        if (p->first > offset) {
            // gap
            uint64_t newlen = std::min<uint64_t>(p->first - offset, length);
            p = ref_map.insert(map<uint64_t, record_t>::value_type(offset, record_t(newlen, 1))).first;
            offset += newlen;
            length -= newlen;
            _maybe_merge_left(p);
            ++p;
        ...
        if (p->first < offset) {
            ...
            p = ref_map.insert(map<uint64_t, record_t>::value_type(offset, record_t(left, p->second.refs))).first;
            // continue below
        }
        ceph_assert(p->first == offset);
        if (length < p->second.length) {
            ref_map.insert(make_pair(offset + length, record_t(p->second.length - length, p->second.refs)));
...
void bluestore_extent_ref_map_t::put(uint64_t offset, uint32_t length, PExtentVector* release, bool* maybe_unshared)
{
    ...
    while (length > 0) {
        ...
        if (p->second.refs > 1) {
            ...
            if (release) release->push_back(bluestore_pextent_t(p->first, p->second.length));
            ref_map.erase(p++);
        }
    }
    if (p != ref_map.end()) _maybe_merge_left(p);
...
bool bluestore_extent_ref_map_t::intersects(uint64_t offset, uint32_t length) const
{
    auto p = ref_map.lower_bound(offset);
    if (p != ref_map.begin()) {
        --p;
...
ostream& operator<<(ostream& out, const bluestore_extent_ref_map_t& m)
{
    ...
    for (auto p = m.ref_map.begin(); p != m.ref_map.end(); ++p) {
        ...
        out << std::hex << "0x" << p->first << "~" << p->second.length << std::dec << "=" << p->second.refs;
    }
    out << ")";
    return out;
}
...
bluestore_blob_use_tracker_t::bluestore_blob_use_tracker_t(const bluestore_blob_use_tracker_t& tracker)
...
bluestore_blob_use_tracker_t& bluestore_blob_use_tracker_t::operator=(const bluestore_blob_use_tracker_t& rhs)
{
    if (this == &rhs) {
        return *this;
    }
...
void bluestore_blob_use_tracker_t::allocate(uint32_t au_count)
{
    ...
    mempool::get_pool(mempool::pool_index_t(mempool::mempool_bluestore_cache_other))
...
void bluestore_blob_use_tracker_t::init(uint32_t full_length, uint32_t _au_size)
{
...
bool bluestore_blob_use_tracker_t::put(uint32_t offset, uint32_t length, PExtentVector* release_units)
{
    ceph_assert(au_size);
    if (release_units) {
        release_units->clear();
...
void bluestore_blob_use_tracker_t::split(uint32_t blob_offset, bluestore_blob_use_tracker_t* r)
{
    ceph_assert(au_size);
...
bool bluestore_blob_use_tracker_t::equal(const bluestore_blob_use_tracker_t& other) const
{
    ...
    uint32_t referenced = num_au ? other.get_referenced_bytes() : get_referenced_bytes();
...
void bluestore_blob_use_tracker_t::generate_test_instances(list<bluestore_blob_use_tracker_t*>& o)
{
...
void bluestore_blob_t::generate_test_instances(list<bluestore_blob_t*>& ls)
{
    ...
    ls.back()->allocated_test(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, 0x1000));
...
ostream& operator<<(ostream& out, const bluestore_blob_t& o)
{
    ...
    if (o.is_compressed()) {
        out << " clen 0x" << std::hex << o.get_logical_length() << " -> 0x" << o.get_compressed_payload_length()
            << std::dec;
    }
    if (o.flags) {
    ...
    if (o.has_csum()) {
        out << " " << Checksummer::get_csum_type_string(o.csum_type) << "/0x" << std::hex
            << (1ull << o.csum_chunk_order) << std::dec;
    }
...
void bluestore_blob_t::calc_csum(uint64_t b_off, const bufferlist& bl)
{
    ...
    case Checksummer::CSUM_XXHASH32:
        Checksummer::calculate<Checksummer::xxhash32>(get_csum_chunk_size(), b_off, bl.length(), bl, &csum_data);
        break;
    case Checksummer::CSUM_XXHASH64:
        Checksummer::calculate<Checksummer::xxhash64>(get_csum_chunk_size(), b_off, bl.length(), bl, &csum_data);
        break;
        ;
    case Checksummer::CSUM_CRC32C:
        Checksummer::calculate<Checksummer::crc32c>(get_csum_chunk_size(), b_off, bl.length(), bl, &csum_data);
        break;
    case Checksummer::CSUM_CRC32C_16:
        Checksummer::calculate<Checksummer::crc32c_16>(get_csum_chunk_size(), b_off, bl.length(), bl, &csum_data);
        break;
    case Checksummer::CSUM_CRC32C_8:
        Checksummer::calculate<Checksummer::crc32c_8>(get_csum_chunk_size(), b_off, bl.length(), bl, &csum_data);
        break;
    }
}

int bluestore_blob_t::verify_csum(uint64_t b_off, const bufferlist& bl, int* b_bad_off, uint64_t* bad_csum) const
{
    int r = 0;

    *b_bad_off = -1;
    switch (csum_type) {
    case Checksummer::CSUM_NONE: break;
    case Checksummer::CSUM_XXHASH32:
...
void bluestore_blob_t::allocated(uint32_t b_off, uint32_t length, const PExtentVector& allocs)
{
    if (extents.size() == 0) {
        ...
        ceph_assert((is_compressed() && logical_length != 0) || (!is_compressed() && logical_length == 0));

        extents.reserve(allocs.size() + (b_off ? 1 : 0));
        if (b_off) {
            extents.emplace_back(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, b_off));
        }
        ...
        count += head ? 1 : 0;
        count += tail ? 1 : 0;
        extents.insert(start_it, count, bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, 0));
...
struct vecbuilder
{
    ...
    {
        if (invalid) {
            v.emplace_back(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, invalid));
...
bool bluestore_blob_t::release_extents(bool all, const PExtentVector& logical, PExtentVector* r)
{
    ...
    while (pext_it != pext_end) {
        if (loffs_it == lend || pext_loffs_start + pext_it->length <= loffs_it->offset) {
            ...
            do {
                uint32_t to_release_part = std::min(pext_it->length - delta0 - delta, to_release);
...
void bluestore_blob_t::split(uint32_t blob_offset, bluestore_blob_t& rb)
{
    ...
    for (auto p = extents.begin(); p != extents.end(); ++p, ++i) {
        ...
        if (left) {
            if (p->is_valid()) {
                rb.extents.emplace_back(bluestore_pextent_t(p->offset + left, p->length - left));
            }
            else {
                rb.extents.emplace_back(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, p->length - left));
            }
...
MEMPOOL_DEFINE_OBJECT_FACTORY(bluestore_shared_blob_t, bluestore_shared_blob_t, bluestore_cache_other);
...
void bluestore_shared_blob_t::generate_test_instances(list<bluestore_shared_blob_t*>& ls)
{
...
{
    return out << std::hex << "0x" << si.offset << "(0x" << si.bytes << " bytes" << std::dec << ")";
}
...
void bluestore_compression_header_t::generate_test_instances(list<bluestore_compression_header_t*>& o)
{
...
shared_blob_2hash_tracker_t::hash_input_t shared_blob_2hash_tracker_t::build_hash_input(uint64_t sbid,
                                                                                        uint64_t offset) const
{
    hash_input_t res = {
        sbid, offset >> au_void_bits, ((sbid & 0xffffffff) << 32) + ~(uint32_t((offset >> au_void_bits) & 0xffffffff))};
    return res;
}
...
void shared_blob_2hash_tracker_t::inc(uint64_t sbid, uint64_t offset, int n)
{
    ...
    ref_counter_2hash_tracker_t::inc((char*)hash_input.data(), get_hash_input_size(), n);
}

void shared_blob_2hash_tracker_t::inc_range(uint64_t sbid, uint64_t offset, uint32_t len, int n)
{
...
bool shared_blob_2hash_tracker_t::test_hash_conflict(uint64_t sbid1, uint64_t offset1, uint64_t sbid2,
                                                     uint64_t offset2) const
{
    ...
    return ref_counter_2hash_tracker_t::test_hash_conflict(
        (char*)hash_input1.data(), (char*)hash_input2.data(), get_hash_input_size());
}

bool shared_blob_2hash_tracker_t::test_all_zero(uint64_t sbid, uint64_t offset) const
{
    auto hash_input = build_hash_input(sbid, offset);
    return ref_counter_2hash_tracker_t::test_all_zero((char*)hash_input.data(), get_hash_input_size());
}

bool shared_blob_2hash_tracker_t::test_all_zero_range(uint64_t sbid, uint64_t offset, uint32_t len) const
{
    uint32_t alloc_unit = 1 << au_void_bits;
    int64_t l = len;
    while (l > 0) {
