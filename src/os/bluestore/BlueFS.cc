...
int BlueFS::mkfs(uuid_d osd_uuid, const bluefs_layout_t& layout)
{
    ...
    int r = _allocate(
        vselector->select_prefer_bdev(log_file->vselector_hint), cct->_conf->bluefs_max_log_runway, &log_file->fnode);
                               ...
                               const char* op_name)
{
    ...
    for (auto e : fnode_extents) {
        ...
        apply_for_bitset_range(e.offset,
                               e.length,
                               alloc_size[id],
                               used_blocks[id],
...
int BlueFS::_verify_alloc_granularity(__u8 id, uint64_t offset, uint64_t length, const char* op)
{
    if ((offset & (alloc_size[id] - 1)) || (length & (alloc_size[id] - 1))) {
        derr << __func__ << " " << op << " of " << (int)id << ":0x" << std::hex << offset << "~" << length << std::dec
             << " does not align to alloc_size 0x" << std::hex << alloc_size[id] << std::dec << dendl;
        // be helpful
                                       ...
                                       const bluefs_layout_t& layout)
{
    ...
    for (auto& [ino, file_ref] : file_map) {
        ...
        bool rewrite = std::any_of(fnode_extents.begin(), fnode_extents.end(), [=](auto& ext) {
                      ...
                      char* out)           ///< [out] optional: or copy it here
{
    ...
    dout(10) << __func__ << " h " << h << " 0x" << std::hex << off << "~" << len << std::dec << " from "
             << h->file->fnode << (prefetch ? " prefetch" : "") << dendl;
...
bool BlueFS::_should_compact_log()
{
    ...
    dout(10) << __func__ << " current 0x" << std::hex << current << " expected " << expected << std::dec << " ratio "
             << ratio << (new_log ? " (async compaction in progress)" : "") << dendl;
...
void BlueFS::_rewrite_log_and_layout_sync(bool allocate_with_fallback, int super_dev, int log_dev, int log_dev_new,
                                          int flags, std::optional<bluefs_layout_t> layout)
{
    ...
    dout(20) << __func__ << " super_dev:" << super_dev << " log_dev:" << log_dev << " log_dev_new:" << log_dev_new
             << " flags:" << flags << dendl;
    bluefs_transaction_t t;
...
void BlueFS::_compact_log_async(std::unique_lock<ceph::mutex>& l)
{
    ...
    dout(10) << __func__ << " old_log_jump_to 0x" << std::hex << old_log_jump_to << " need 0x"
             << (old_log_jump_to + cct->_conf->bluefs_max_log_runway) << std::dec << dendl;
    int r = _allocate(
        vselector->select_prefer_bdev(log_file->vselector_hint), cct->_conf->bluefs_max_log_runway, &log_file->fnode);
    ceph_assert(r == 0);
    ...
    dout(10) << __func__ << " remove 0x" << std::hex << old_log_jump_to << std::dec << " of " << log_file->fnode.extents
             << dendl;
...
void BlueFS::_pad_bl(bufferlist& bl)
{
    ...
    if (partial) {
        dout(10) << __func__ << " padding with 0x" << std::hex << super.block_size - partial << " zeros" << std::dec
                 << dendl;
...
int BlueFS::_flush_and_sync_log(std::unique_lock<ceph::mutex>& l, uint64_t want_seq, uint64_t jump_to)
{
    ...
    if (want_seq && want_seq <= log_seq_stable) {
        dout(10) << __func__ << " want_seq " << want_seq << " <= log_seq_stable " << log_seq_stable << ", done"
                 << dendl;
        ceph_assert(!jump_to);
        return 0;
    }
    if (log_t.empty() && dirty_files.empty()) {
        dout(10) << __func__ << " want_seq " << want_seq << " " << log_t << " not dirty, dirty_files empty, no-op"
                 << dendl;
        ceph_assert(!jump_to);
        return 0;
    }
    ...
    if (jump_to) {
        dout(10) << __func__ << " jumping log offset from 0x" << std::hex << log_writer->pos << " -> 0x" << jump_to
                 << std::dec << dendl;
    ...
    if (seq > log_seq_stable) {
        ...
        dout(20) << __func__ << " log_seq_stable " << log_seq_stable << " already >= out seq " << seq
...
int BlueFS::_flush_range(FileWriter* h, uint64_t offset, uint64_t length)
{
    ...
    if (allocated < offset + length) {
        ...
        ceph_assert(h->file->fnode.ino != 1);
        int r = _allocate(
            vselector->select_prefer_bdev(h->file->vselector_hint), offset + length - allocated, &h->file->fnode);
        if (r < 0) {
...
int BlueFS::_allocate_without_fallback(uint8_t id, uint64_t len, PExtentVector* extents)
{
    ...
    if (alloc_len < 0 || alloc_len < need) {
        ...
        derr << __func__ << " unable to allocate 0x" << std::hex << need << " on bdev " << (int)id
             ...
             << ", free 0x" << alloc[id]->get_free() << ", fragmentation " << alloc[id]->get_fragmentation()
             << ", allocated 0x" << (alloc_len > 0 ? alloc_len : 0) << std::dec << dendl;
        alloc[id]->dump();
        return -ENOSPC;
    }
    if (is_shared_alloc(id)) {
 ...
 */
int BlueFS::do_replay_recovery_read(FileReader* log_reader, size_t replay_pos, size_t read_offset, size_t read_len,
                                    bufferlist* bl)
{
