...
void BlueFS::_init_alloc()
{
    ...
    for (unsigned id = 0; id < bdev.size(); ++id) {
        ...
        if (is_shared_alloc(id)) {
            ...
            alloc[id] =
                Allocator::create(cct, cct->_conf->bluefs_allocator, bdev[id]->get_size(), alloc_size[id], name);
...
int BlueFS::_flush_data(FileWriter* h, uint64_t offset, uint64_t length, bool buffered)
{
    ...
    case WRITER_WAL: logger->inc(l_bluefs_bytes_written_wal, length); break;
    case WRITER_SST: logger->inc(l_bluefs_bytes_written_sst, length); break;
    }
                      ...
                      bool permit_dev_fallback)
{
    ...
    if (alloc[id]) {
        ...
        if (shared && alloc_unit != shared_unit) {
            if (std::chrono::duration_cast<seconds>(real_clock::now().time_since_epoch()).count() < cooldown_deadline) {
    ...
    if (alloc_len < 0 || alloc_len < need) {
        if (alloc[id]) {
            ...
            if (!was_cooldown && shared) {
                ...
                cooldown_deadline =
                    delay_s + std::chrono::duration_cast<seconds>(real_clock::now().time_since_epoch()).count();
...
int BlueFS::open_for_write(std::string_view dirname, std::string_view filename, FileWriter** h,
                                     ...
                                     bufferlist* bl)
{
    ...
    int r =
        bdev[e->bdev]->read(e->offset + e_off, e->length - e_off, &fixed, ioc[e->bdev], cct->_conf->bluefs_buffered_io);
    ceph_assert(r == 0);
        ...
        for (size_t i = 0; i < disk_regions_count; reg++, i++) {
            ...
            while (len > last_32.length()) {
                ...
                for (char* sp = search_b;; sp += last_32.length()) {
                    ...
                    r = bdev[ne.bdev]->read(
                        ne.offset, ne.length, &candidate, ioc[ne.bdev], cct->_conf->bluefs_buffered_io);
                    ceph_assert(r == 0);
