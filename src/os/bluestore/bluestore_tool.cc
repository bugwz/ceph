...
void validate_path(CephContext* cct, const string& path, bool bluefs)
{
    ...
    if (kv_backend != "rocksdb") {
        cerr << "expect kv_backend to be rocksdb, but is " << kv_backend << std::endl;
        exit(EXIT_FAILURE);
    }
...
const char* find_device_path(int id, CephContext* cct, const vector<string>& devs)
{
    for (auto& i : devs) {
        ...
        if (r < 0) {
            cerr << "unable to read label for " << i << ": " << cpp_strerror(r) << std::endl;
            exit(EXIT_FAILURE);
        }
...
void parse_devices(CephContext* cct, const vector<string>& devs, map<string, int>* got, bool* has_db, bool* has_wal)
{
    ...
    for (auto& d : devs) {
        ...
        if (r < 0) {
            cerr << "unable to read label for " << d << ": " << cpp_strerror(r) << std::endl;
            exit(EXIT_FAILURE);
        }
        int id = -1;
    ...
    for (auto e : got) {
        ...
        if (!e.first.empty()) {
            if (realpath(e.first.c_str(), target_path) == nullptr) {
                cerr << "failed to retrieve absolute path for " << e.first << ": " << cpp_strerror(errno) << std::endl;
            }
        }
...
BlueFS* open_bluefs_readonly(CephContext* cct, const string& path, const vector<string>& devs)
{
...
void log_dump(CephContext* cct, const string& path, const vector<string>& devs)
{
    validate_path(cct, path, true);
    BlueFS* fs = new BlueFS(cct);
...
static void bluefs_import(const string& input_file, const string& dest_file, CephContext* cct, const string& path,
...
int main(int argc, char** argv)
{
    ...
    po_options.add_options()("help,h", "produce help message")(
        ...
        "path", po::value<string>(&path), "bluestore path")("out-dir", po::value<string>(&out_dir), "output directory")(
    ...
    po_positional.add_options()("command",
                                po::value<string>(&action),
                                "fsck, "
                                "qfsck, "
                                "allocmap, "
                                "restore_cfb, "
    ...
    auto cct = global_init(NULL,
                           args,
                           osd_instance.empty() ? CEPH_ENTITY_TYPE_CLIENT : CEPH_ENTITY_TYPE_OSD,
    ...
    if (action == "show-label") {
        ...
        if (devs.empty()) inferring_bluefs_devices(devs, path);
    }
    if (action == "bluefs-export" || action == "bluefs-import" || action == "bluefs-log-dump") {
        if (path.empty()) {
            cerr << "must specify bluestore path" << std::endl;
            exit(EXIT_FAILURE);
        }
    ...
    if (action == "free-score" || action == "free-dump" || action == "free-fragmentation") {
        ...
        for (auto name : allocs_name) {
            if (!name.empty() && name != "block" && name != "bluefs-db" && name != "bluefs-wal") {
                cerr << "unknown allocator '" << name << "'" << std::endl;
                exit(EXIT_FAILURE);
            }
        }
        if (allocs_name.empty()) allocs_name = vector<string>{"block", "bluefs-db", "bluefs-wal"};
    }
    if (action == "reshard") {
        if (path.empty()) {
            cerr << "must specify bluestore path" << std::endl;
    ...
    if (action == "restore_cfb") {
    ...
    else if (action == "fsck" || action == "repair" || action == "quick-fix") {
        validate_path(cct.get(), path, false);
        BlueStore bluestore(cct.get(), path);
        int r;
        if (action == "fsck") {
        ...
        if (r < 0) {
            cerr << "failed to read label for " << devs.front() << ": " << cpp_strerror(r) << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        for (auto kk : {"whoami", "osd_key", "ceph_fsid", "fsid", "type", "ready"}) {
            string k = kk;
            ...
            if (fd < 0) {
                cerr << "error writing " << p << ": " << cpp_strerror(errno) << std::endl;
                exit(EXIT_FAILURE);
            }
            int r = safe_write(fd, v.c_str(), v.size());
            if (r < 0) {
                cerr << "error writing to " << p << ": " << cpp_strerror(errno) << std::endl;
                exit(EXIT_FAILURE);
            }
            ::close(fd);
        }
        ...
        int r = BlueStore::_read_bdev_label(cct.get(), devs.front(), &label);
        if (r < 0) {
            cerr << "unable to read label for " << devs.front() << ": " << cpp_strerror(r) << std::endl;
            exit(EXIT_FAILURE);
        }
        if (key == "size") {
        ...
        if (r < 0) {
            cerr << "unable to write label for " << devs.front() << ": " << cpp_strerror(r) << std::endl;
            exit(EXIT_FAILURE);
        }
    }
        ...
        if (r < 0) {
            cerr << "failed to expand bluestore devices: " << cpp_strerror(r) << std::endl;
            exit(EXIT_FAILURE);
        }
    }
        ...
        for (auto& dir : dirs) {
            ...
            for (auto& file : ls) {
                ...
                if (size > 0) {
                    ...
                    if (r < 0) {
                        cerr << "open_for_read " << dir << "/" << file << " failed: " << cpp_strerror(r) << std::endl;
                        exit(EXIT_FAILURE);
                    }
                    int pos = 0;
                    int left = size;
                    while (left) {
                        ...
                        if (r <= 0) {
                            cerr << "read " << dir << "/" << file << " from " << pos << " failed: " << cpp_strerror(r)
                                 << std::endl;
                            exit(EXIT_FAILURE);
                        }
                        int rc = bl.write_fd(fd);
                        if (rc < 0) {
                            cerr << "write to " << path << " failed: " << cpp_strerror(r) << std::endl;
                            exit(EXIT_FAILURE);
                        }
                        pos += r;
                        left -= r;
        ...
        auto [target_path, has_size_spec] = [&dev_target]() -> std::pair<string, bool> {
            if (dev_target.empty()) {
                return {"", false};
            }
            std::error_code ec;
            ...
            if (ec) {
                cerr << "failed to retrieve absolute path for " << dev_target << ": " << ec.message() << std::endl;
                exit(EXIT_FAILURE);
            }
            return {target.native(),
                    fs::exists(target) &&
                        (fs::is_block_file(target) || (fs::is_regular_file(target) && fs::file_size(target) > 0))};
        }();
        ...
        BlueStore bluestore(cct.get(), path);
        int r = bluestore.add_new_bluefs_device(need_db ? BlueFS::BDEV_NEWDB : BlueFS::BDEV_NEWWAL, target_path);
        if (r == 0) {
            cout << (need_db ? "DB" : "WAL") << " device added " << target_path << std::endl;
        }
        else {
            cerr << "failed to add " << (need_db ? "DB" : "WAL") << " device:" << cpp_strerror(r) << std::endl;
        }
        return r;
    }
        ...
        for (auto& s : devs_source) {
            ...
            if (i != cur_devs_map.end()) {
                if (s == dev_target) {
                    cerr << "Device " << dev_target << " is present in both source and target lists, omitted."
                         << std::endl;
                }
                else {
                    src_devs.emplace(*i);
                ...
                cerr << "can't migrate " << s << ", not a valid bluefs volume " << std::endl;
                exit(EXIT_FAILURE);
            }
        }
        ...
        if (i != cur_devs_map.end()) {
            ...
            BlueStore bluestore(cct.get(), path);
            int r = bluestore.migrate_to_existing_bluefs_device(src_dev_ids, dev_target_id);
            if (r == 0) {
                for (auto src : src_devs) {
                    if (src.second != BlueFS::BDEV_SLOW) {
                        cout << " device removed:" << src.second << " " << src.first << std::endl;
                    }
                }
            }
            else {
                ...
                cerr << "failed to migrate to existing BlueFS device: "
                     << (need_db ? BlueFS::BDEV_DB : BlueFS::BDEV_WAL) << " " << dev_target << cpp_strerror(r)
                     << std::endl;
            }
            return r;
        }
            ...
            if (!dev_target.empty() && realpath(dev_target.c_str(), target_path) == nullptr) {
                cerr << "failed to retrieve absolute path for " << dev_target << ": " << cpp_strerror(errno)
                     << std::endl;
                exit(EXIT_FAILURE);
            }
            ...
            int r = bluestore.migrate_to_new_bluefs_device(src_dev_ids, dev_target_id, target_path);
            if (r == 0) {
                for (auto src : src_devs) {
                    if (src.second != BlueFS::BDEV_SLOW) {
                        cout << " device removed:" << src.second << " " << src.first << std::endl;
                ...
                cout << " device added: " << (need_db ? BlueFS::BDEV_DB : BlueFS::BDEV_DB) << " " << target_path
                     << std::endl;
            }
            else {
                cerr << "failed to migrate to new BlueFS device: " << (need_db ? BlueFS::BDEV_DB : BlueFS::BDEV_DB)
        ...
        std::string action_name = action == "free-dump" ? "dump" : action == "free-score" ? "score" : "fragmentation";
        validate_path(cct.get(), path, false);
        BlueStore bluestore(cct.get(), path);
        ...
        for (auto alloc_name : allocs_name) {
            ...
            int r = admin_socket->execute_command(
                {"{\"prefix\": \"bluestore allocator " + action_name + " " + alloc_name + "\"}"}, in, err, &out);
            if (r != 0) {
        ...
        g_conf().set_val_or_die("bluestore_volume_selection_policy", "use_some_extra_enforced");
        BlueStore bluestore(cct.get(), path);
        int r = bluestore.cold_open();
        if (r < 0) {
            cerr << "error from cold_open: " << cpp_strerror(r) << std::endl;
        ...
        ceph::bufferlist in, out;
        ostringstream err;
        r = admin_socket->execute_command({"{\"prefix\": \"bluefs stats\"}"}, in, err, &out);
        if (r != 0) {
