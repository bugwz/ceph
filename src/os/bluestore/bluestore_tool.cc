...
int main(int argc, char** argv)
{
    ...
    po_options.add_options()("help,h", "produce help message")("path", po::value<string>(&path), "bluestore path")(
        ...
        "allocator",
        po::value<vector<string>>(&allocs_name),
    ...
    if (action == "free-score" || action == "free-dump") {
        ...
        if (allocs_name.empty()) allocs_name = vector<string>{"block", "bluefs-db", "bluefs-wal", "bluefs-slow"};
    }
    if (action == "reshard") {
        if (path.empty()) {
            cerr << "must specify bluestore path" << std::endl;
    ...
    if (action == "fsck" || action == "repair" || action == "quick-fix") {
        ...
        else if (!dev_target.empty() && (rlpath = realpath(dev_target.c_str(), target_path)) == nullptr) {
            cerr << "failed to retrieve absolute path for " << dev_target << ": " << cpp_strerror(errno) << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        if (r == -1 || (r == 0 && st.st_size == 0 && !S_ISBLK(st.st_mode) && S_ISREG(st.st_mode))) {
            r = 0;
        ...
        if (r == 0) {
            BlueStore bluestore(cct.get(), path);
            r = bluestore.add_new_bluefs_device(need_db ? BlueFS::BDEV_NEWDB : BlueFS::BDEV_NEWWAL, target_path);
            if (r == 0) {
