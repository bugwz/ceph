...
public:
    ...
    void _set_max_defer_interval() { max_defer_interval = cct->_conf.get_val<double>("bluestore_max_defer_interval"); }
    ...
    struct Buffer
    {
        ...
        Buffer(BufferSpace* space, unsigned s, uint64_t q, uint32_t o, uint32_t l, unsigned f = 0)
            : space(space)
            , state(s)
            , flags(f)
            , seq(q)
            ...
            , length(l)
        {}
        Buffer(BufferSpace* space, unsigned s, uint64_t q, uint32_t o, ceph::buffer::list& b, unsigned f = 0)
            : space(space)
            , state(s)
            , flags(f)
            , seq(q)
        ...
        {
            if (data.length() &&
                (data.get_num_buffers() > 1 || data.front().wasted() > data.length() / MAX_BUFFER_SLOP_RATIO_DEN)) {
                data.rebuild();
            }
        }
    ...
    struct BufferSpace
    {
        ...
        typedef boost::intrusive::list<
            Buffer, boost::intrusive::member_hook<Buffer, boost::intrusive::list_member_hook<>, &Buffer::state_item>>
            state_list_t;
        ...
        void _rm_buffer(BufferCacheShard* cache, Buffer* b) { _rm_buffer(cache, buffer_map.find(b->offset)); }
        void _rm_buffer(BufferCacheShard* cache, std::map<uint32_t, std::unique_ptr<Buffer>>::iterator p)
        {
        ...
        std::map<uint32_t, std::unique_ptr<Buffer>>::iterator _data_lower_bound(uint32_t offset)
        {
            auto i = buffer_map.lower_bound(offset);
            if (i != buffer_map.begin()) {
                --i;
                if (i->first + i->second->length <= offset) ++i;
            }
            return i;
        }
        ...
        void write(BufferCacheShard* cache, uint64_t seq, uint32_t offset, ceph::buffer::list& bl, unsigned flags)
        {
            std::lock_guard l(cache->lock);
        ...
        void read(BufferCacheShard* cache, uint32_t offset, uint32_t length, BlueStore::ready_regions_t& res,
                  interval_set<uint32_t>& res_intervals, int flags = 0);

        void truncate(BufferCacheShard* cache, uint32_t offset) { discard(cache, offset, (uint32_t)-1 - offset); }
    ...
    struct SharedBlob
    {
        ...
        uint64_t get_sbid() const { return loaded ? persistent->sbid : sbid_unloaded; }
        ...
        friend bool operator==(const SharedBlob& l, const SharedBlob& r) { return l.get_sbid() == r.get_sbid(); }
        inline BufferCacheShard* get_cache() { return coll ? coll->cache : nullptr; }
        inline SharedBlobSet* get_parent() { return coll ? &(coll->shared_blob_set) : nullptr; }
    ...
    public:
        ...
        const bluestore_blob_use_tracker_t& get_blob_use_tracker() const { return used_in_blob; }
        bool is_referenced() const { return used_in_blob.is_not_empty(); }
        uint32_t get_referenced_bytes() const { return used_in_blob.get_referenced_bytes(); }
        ...
        bool can_split() const
        {
            ...
            return shared_blob->bc.writing.empty() && used_in_blob.can_split() && get_blob().can_split();
        }

        bool can_split_at(uint32_t blob_offset) const
        {
            return used_in_blob.can_split_at(blob_offset) && get_blob().can_split_at(blob_offset);
        }

        bool can_reuse_blob(uint32_t min_alloc_size, uint32_t target_blob_size, uint32_t b_offset, uint32_t* length0);
        ...
        /// put logical references, and get back any released extents
        bool put_ref(Collection* coll, uint32_t offset, uint32_t length, PExtentVector* r);
        ...
        void encode(ceph::buffer::list::contiguous_appender& p, bool include_ref_map) const
        {
            _encode();
            p.append(blob_bl);
            if (include_ref_map) {
        ...
        void decode(ceph::buffer::ptr::const_iterator& p, bool include_ref_map, Collection* /*coll*/)
        {
        ...
        void encode(ceph::buffer::list::contiguous_appender& p, uint64_t struct_v, uint64_t sbid,
        ...
        void decode(ceph::buffer::ptr::const_iterator& p, uint64_t struct_v, uint64_t* sbid, bool include_ref_map,
                    Collection* coll);
#endif
    };
    ...
    struct Extent : public ExtentBase
    {
        ...
        bool blob_escapes_range(uint32_t o, uint32_t l) const { return blob_start() < o || blob_end() > o + l; }
    };
    ...
    struct OldExtent
    {
        ...
        OldExtent(uint32_t lo, uint32_t o, uint32_t l, BlobRef& b)
            ...
            , blob_empty(false)
        {}
        static OldExtent* create(CollectionRef c, uint32_t lo, uint32_t o, uint32_t l, BlobRef& b);
    };
    typedef boost::intrusive::list<
        OldExtent,
        boost::intrusive::member_hook<OldExtent, boost::intrusive::list_member_hook<>, &OldExtent::old_extent_item>>
        old_extent_map_t;
    ...
    struct ExtentMap
    {
        ...
        void dup(BlueStore* b, TransContext*, CollectionRef&, OnodeRef&, OnodeRef&, uint64_t&, uint64_t&, uint64_t&);

        bool needs_reshard() const { return needs_reshard_end > needs_reshard_begin; }
        ...
        bool encode_some(uint32_t offset, uint32_t length, ceph::buffer::list& bl, unsigned* pn);
        ...
        protected:
            virtual void consume_blobid(Extent* le, bool spanning, uint64_t blobid) = 0;
            virtual void consume_blob(Extent* le, uint64_t extent_no, uint64_t sbid, BlobRef b) = 0;
            ...
            void decode_extent(Extent* le, __u8 struct_v, bptr_c_it_t& p, Collection* c);
        ...
        public:
            ExtentDecoderFull(ExtentMap& _extent_map)
                : extent_map(_extent_map)
            {}
        };
        ...
        int seek_shard(uint32_t offset)
        {
            ...
            while (left < right) {
                ...
                if (offset >= shards[mid].shard_info->offset) {
                    size_t next = mid + 1;
                    if (next >= end || offset < shards[next].shard_info->offset) return mid;
        ...
        /// add a new Extent
        void add(uint32_t lo, uint32_t o, uint32_t l, BlobRef& b) { extent_map.insert(*new Extent(lo, o, l, b)); }

        /// remove (and delete) an Extent
        void rm(extent_map_t::iterator p) { extent_map.erase_and_dispose(p, DeleteDisposer()); }
        ...
        void punch_hole(CollectionRef& c, uint64_t offset, uint64_t length, old_extent_map_t* old_extents);

        /// put new lextent into lextent_map overwriting existing ones if
        /// any and update references accordingly
        Extent* set_lextent(CollectionRef& c, uint64_t logical_offset, uint64_t offset, uint64_t length, BlobRef b,
    ...
    */
    ...
    public:
        ...
        int64_t estimate(uint64_t offset, uint64_t length, const ExtentMap& extent_map,
                         const old_extent_map_t& old_extents, uint64_t min_alloc_size);

        /// return a collection of extents to perform GC on
        const interval_set<uint64_t>& get_extents_to_collect() const { return extents_to_collect; }
        GarbageCollector(CephContext* _cct)
            : cct(_cct)
        {}
    ...
    protected:
        void process_protrusive_extents(const BlueStore::ExtentMap& extent_map, uint64_t start_offset,
    ...
    struct Onode
    {
        ...
        Onode(Collection* c, const ghobject_t& o, const mempool::bluestore_cache_meta::string& k)
            : c(c)
            , oid(o)
            , key(k)
            , exists(false)
            , cached(false)
            , extent_map(this, c->store->cct->_conf->bluestore_extent_map_inline_shard_prealloc_size)
        {}
        Onode(Collection* c, const ghobject_t& o, const std::string& k)
            : c(c)
            , oid(o)
            , key(k)
            , exists(false)
        ...
        Onode(CephContext* cct)
            ...
            , exists(false)
            , cached(false)
            , extent_map(this, cct->_conf->bluestore_extent_map_inline_shard_prealloc_size)
        {}
        static void decode_raw(BlueStore::Onode* on, const bufferlist& v, ExtentMap::ExtentDecoder& dencoder);

        static Onode* create_decode(CollectionRef c, const ghobject_t& oid, const std::string& key,
                                    const ceph::buffer::list& v, bool allow_empty = false);
        ...
        static void calc_omap_header(uint8_t flags, const Onode* o, std::string* out);
        static void calc_omap_key(uint8_t flags, const Onode* o, const std::string& key, std::string* out);
        static void calc_omap_tail(uint8_t flags, const Onode* o, std::string* out);

        const std::string& get_omap_prefix() { return calc_omap_prefix(onode.flags); }
        void get_omap_header(std::string* out) { calc_omap_header(onode.flags, this, out); }
        void get_omap_key(const std::string& key, std::string* out) { calc_omap_key(onode.flags, this, key, out); }
        void get_omap_tail(std::string* out) { calc_omap_tail(onode.flags, this, out); }
        ...
        {
            return extent_map.extent_map.begin()->blob->get_blob().calc_offset(0, nullptr);
        }
#endif
    private:
    ...
    public:
        OnodeCacheShard(CephContext* cct)
            : CacheShard(cct)
        {}
        static OnodeCacheShard* create(CephContext* cct, std::string type, PerfCounters* logger);
    ...
    public:
        BufferCacheShard(CephContext* cct)
            : CacheShard(cct)
        {}
        static BufferCacheShard* create(CephContext* cct, std::string type, PerfCounters* logger);
        ...
        virtual void add_stats(uint64_t* extents, uint64_t* blobs, uint64_t* buffers, uint64_t* bytes) = 0;
    ...
    struct Collection : public CollectionImpl
    {
        ...
        ceph::shared_mutex lock = ceph::make_shared_mutex("BlueStore::Collection::lock", true, false);
        ...
        OnodeCacheShard* get_onode_cache() const { return onode_space.cache; }
        ...
        {
            if (cid.is_meta()) return oid.hobj.pool == -1;
            spg_t spgid;
            if (cid.is_pg(&spgid)) return spgid.pgid.contains(cnode.bits, oid) && oid.shard_id == spgid.shard;
            return false;
        }
    ...
    public:
        ...
        std::string tail_key() override { return tail; }
    ...
    struct volatile_statfs
    {
        ...
        int64_t& compressed_original() { return values[STATFS_COMPRESSED_ORIGINAL]; }
        int64_t& compressed() { return values[STATFS_COMPRESSED]; }
        int64_t& compressed_allocated() { return values[STATFS_COMPRESSED_ALLOCATED]; }
        int64_t allocated() const { return values[STATFS_ALLOCATED]; }
        int64_t stored() const { return values[STATFS_STORED]; }
        int64_t compressed_original() const { return values[STATFS_COMPRESSED_ORIGINAL]; }
        int64_t compressed() const { return values[STATFS_COMPRESSED]; }
        int64_t compressed_allocated() const { return values[STATFS_COMPRESSED_ALLOCATED]; }
        ...
        bool is_empty()
        {
            return values[STATFS_ALLOCATED] == 0 && values[STATFS_STORED] == 0 && values[STATFS_COMPRESSED] == 0 &&
                   values[STATFS_COMPRESSED_ORIGINAL] == 0 && values[STATFS_COMPRESSED_ALLOCATED] == 0;
        }
    ...
    struct TransContext final : public AioContext
    {
        ...
        explicit TransContext(CephContext* cct, Collection* c, OpSequencer* o, std::list<Context*>* on_commits)
            : ch(c)
            , osr(o)
            , ioc(cct, this)
    ...
    class BlueStoreThrottle
    {
        ...
        {
            uint64_t min_period_mcs = trace_period_mcs.load(std::memory_order_relaxed);
            ...
            if (min_period_mcs == 0) {
                ...
                *completed = ios_completed_since_last_traced.exchange(0);
                ...
                auto now_mcs = ceph::to_microseconds<uint64_t>(ceph::mono_clock::now() - time_base);
                ...
                if (period_mcs > min_period_mcs) {
                    if (previous_emitted_tp_time_mono_mcs.compare_exchange_strong(previous_mcs, now_mcs)) {
    ...
    public:
        ...
        ceph::mono_clock::duration log_state_latency(TransContext& txc, PerfCounters* logger, int state);
        bool try_start_transaction(KeyValueDB& db, TransContext& txc, ceph::mono_clock::time_point);
        void finish_start_transaction(KeyValueDB& db, TransContext& txc, ceph::mono_clock::time_point);
        void release_kv_throttle(uint64_t cost) { throttle_bytes.put(cost); }
        void release_deferred_throttle(uint64_t cost) { throttle_deferred_bytes.put(cost); }
        bool should_submit_deferred() { return throttle_deferred_bytes.past_midpoint(); }
        void reset_throttle(const ConfigProxy& conf)
        {
            throttle_bytes.reset_max(conf->bluestore_throttle_bytes);
            throttle_deferred_bytes.reset_max(conf->bluestore_throttle_bytes + conf->bluestore_throttle_deferred_bytes);
#if defined(WITH_LTTNG)
    ...
    typedef boost::intrusive::list<TransContext,
                                   boost::intrusive::member_hook<TransContext, boost::intrusive::list_member_hook<>,
    ...
    struct DeferredBatch final : public AioContext
    {
        ...
        /// prepare a write
        void prepare_write(CephContext* cct, uint64_t seq, uint64_t offset, uint64_t length,
                           ceph::buffer::list::const_iterator& p);

        void aio_finish(BlueStore* store) override { store->_deferred_aio_finish(osr); }
    };
    ...
    public:
        ...
        void drain()
        {
            ...
            while (!q.empty()) qcond.wait(l);
        }

        void drain_preceding(TransContext* txc)
        {
            std::unique_lock l(qlock);
            while (&q.front() != txc) qcond.wait(l);
        }
    ...
    typedef boost::intrusive::list<OpSequencer,
                                   boost::intrusive::member_hook<OpSequencer, boost::intrusive::list_member_hook<>,
    ...
    {
        BlueStore* store;
        explicit KVSyncThread(BlueStore* s)
            : store(s)
        {}
        void* entry() override
        {
    ...
    {
        BlueStore* store;
        explicit KVFinalizeThread(BlueStore* s)
            : store(s)
        {}
        void* entry() override
        {
    ...
    {
        BlueStore* store;
        explicit ZonedCleanerThread(BlueStore* s)
            : store(s)
        {}
        void* entry() override
        {
    ...
    struct BigDeferredWriteContext
    {
        ...
        inline uint64_t blob_aligned_len() const { return used + head_read + tail_read; }

        bool can_defer(BlueStore::extent_map_t::iterator ep, uint64_t prefer_deferred_size, uint64_t block_size,
...
private:
    ...
    ceph::mutex atomic_alloc_and_submit_lock = ceph::make_mutex("BlueStore::atomic_alloc_and_submit_lock");
    ...
    static_assert(std::numeric_limits<uint8_t>::max() > std::numeric_limits<decltype(min_alloc_size)>::digits,
    ...
    public:
        ...
        struct MempoolCache : public PriorityCache::PriCache
        {
            ...
            {
                if (pri > PriorityCache::Priority::PRI0 && pri < PriorityCache::Priority::LAST) {
                    return bins[pri];
                }
                return 0;
            }
        ...
        {
            MetaCache(BlueStore* s)
                : MempoolCache(s){};

            virtual uint32_t get_bin_count() const { return store->onode_cache_shards[0]->get_bin_count(); }
            virtual void set_bin_count(uint32_t count)
            {
            ...
            {
                return mempool::bluestore_blob::allocated_bytes() + mempool::bluestore_extent::allocated_bytes() +
            ...
            virtual std::string get_cache_name() const { return "BlueStore Meta Cache"; }
            ...
            double get_bytes_per_onode() const { return (double)_get_used_bytes() / (double)_get_num_onodes(); }
        };
        std::shared_ptr<MetaCache> meta_cache;

        struct DataCache : public MempoolCache
        {
            DataCache(BlueStore* s)
                : MempoolCache(s){};

            virtual uint32_t get_bin_count() const { return store->buffer_cache_shards[0]->get_bin_count(); }
            virtual void set_bin_count(uint32_t count)
            {
            ...
            virtual std::string get_cache_name() const { return "BlueStore Data Cache"; }
        };
    ...
    int _prepare_db_environment(bool create, bool read_only, std::string* kv_dir, std::string* kv_backend);
    ...
    void _close_db();
    int _open_fm(KeyValueDB::Transaction t, bool read_only, bool db_avail, bool fm_restore = false);
    void _close_fm();
    ...
    int _setup_block_symlink_or_file(std::string name, std::string path, uint64_t size, bool create);
...
public:
    ...
    static int _write_bdev_label(CephContext* cct, const std::string& path, bluestore_bdev_label_t label);
    static int _read_bdev_label(CephContext* cct, const std::string& path, bluestore_bdev_label_t* label);

private:
    int _check_or_set_bdev_label(std::string path, uint64_t size, std::string desc, bool create);
    ...
    template<int LogLevelV> friend void _dump_onode(CephContext* cct, const Onode& o);
    template<int LogLevelV> friend void _dump_extent_map(CephContext* cct, const ExtentMap& em);
    template<int LogLevelV> friend void _dump_transaction(CephContext* cct, Transaction* t);

    TransContext* _txc_create(Collection* c, OpSequencer* osr, std::list<Context*>* on_commits,
...
private:
    ...
    void _zoned_clean_zone(uint64_t zone_num, class ZonedAllocator* a, class ZonedFreelistManager* f);
...
public:
    using mempool_dynamic_bitset = boost::dynamic_bitset<uint64_t, mempool::bluestore_fsck::pool_allocator<uint64_t>>;
    using per_pool_statfs = mempool::bluestore_fsck::map<uint64_t, store_statfs_t>;
...
private:
    int _fsck_check_extents(std::string_view ctx_descr, const PExtentVector& extents, bool compressed,
                            mempool_dynamic_bitset& used_blocks, uint64_t granularity, BlueStoreRepairer* repairer,
                            store_statfs_t& expected_statfs, FSCKDepth depth);

    void _fsck_check_statfs(const store_statfs_t& expected_store_statfs, const per_pool_statfs& expected_pool_statfs,
                            int64_t& errors, int64_t& warnings, BlueStoreRepairer* repairer);
    void _fsck_repair_shared_blobs(BlueStoreRepairer& repairer, shared_blob_2hash_tracker_t& sb_ref_counts,
    ...
    void _buffer_cache_write(TransContext* txc, BlobRef b, uint64_t offset, ceph::buffer::list& bl, unsigned flags)
    {
        b->shared_blob->bc.write(b->shared_blob->get_cache(), txc->seq, offset, bl, flags);
        txc->shared_blobs_written.insert(b->shared_blob);
    }

    int _collection_list(Collection* c, const ghobject_t& start, const ghobject_t& end, int max, bool legacy,
                         std::vector<ghobject_t>* ls, ghobject_t* next);

    template<typename T, typename F> T select_option(const std::string& opt_name, T val1, F f)
    {
    ...
    void _apply_padding(uint64_t head_pad, uint64_t tail_pad, ceph::buffer::list& padded);
...
public:
    ...
    std::string get_type() override { return "bluestore"; }
    ...
    int get_numa_node(int* numa_node, std::set<int>* nodes, std::set<std::string>* failed) override;

    static int get_block_device_fsid(CephContext* cct, const std::string& path, uuid_d* fsid);
...
public:
    ...
    int fsck(bool deep) override { return _fsck(deep ? FSCK_DEEP : FSCK_REGULAR, false); }
    int repair(bool deep) override { return _fsck(deep ? FSCK_DEEP : FSCK_REGULAR, true); }
    ...
    int migrate_to_existing_bluefs_device(const std::set<int>& devs_source, int id);
    int migrate_to_new_bluefs_device(const std::set<int>& devs_source, int id, const std::string& path);
...
public:
    ...
    int read(CollectionHandle& c, const ghobject_t& oid, uint64_t offset, size_t len, ceph::buffer::list& bl,
...
private:
    ...
    struct region_t
    {
        ...
        {
            return out << "0x" << std::hex << r.logical_offset << ":" << r.blob_xoffset << "~" << r.length << std::dec;
        }
    };
    ...
    struct read_req_t
    {
        ...
        read_req_t(uint64_t off, uint64_t len)
            : r_off(off)
            , r_len(len)
        {}
        ...
        friend std::ostream& operator<<(std::ostream& out, const read_req_t& r)
        {
            ...
            for (const auto& reg : r.regs) out << reg;
            return out << "]}" << std::dec;
        }
    };
    ...
    void _read_cache(OnodeRef& o, uint64_t offset, size_t length, int read_cache_policy, ready_regions_t& ready_regions,
                     blobs2read_t& blobs2read);


    int _prepare_read_ioc(blobs2read_t& blobs2read, std::vector<ceph::buffer::list>* compressed_blob_bls,
                          IOContext* ioc);

    int _generate_read_result_bl(OnodeRef& o, uint64_t offset, size_t length, ready_regions_t& ready_regions,
                                 std::vector<ceph::buffer::list>& compressed_blob_bls, blobs2read_t& blobs2read,
                                 bool buffered, bool* csum_error, ceph::buffer::list& bl);

    int _do_read(Collection* c, OnodeRef& o, uint64_t offset, size_t len, ceph::buffer::list& bl, uint32_t op_flags = 0,
                 uint64_t retry_count = 0);

    int _do_readv(Collection* c, OnodeRef& o, const interval_set<uint64_t>& m, ceph::buffer::list& bl,
                  uint32_t op_flags = 0, uint64_t retry_count = 0);

    int _fiemap(CollectionHandle& c_, const ghobject_t& oid, uint64_t offset, size_t len,
                interval_set<uint64_t>& destset);

public:
    int fiemap(CollectionHandle& c, const ghobject_t& oid, uint64_t offset, size_t len,
               ceph::buffer::list& bl) override;
    int fiemap(CollectionHandle& c, const ghobject_t& oid, uint64_t offset, size_t len,
               std::map<uint64_t, uint64_t>& destmap) override;

    int readv(CollectionHandle& c_, const ghobject_t& oid, interval_set<uint64_t>& m, ceph::buffer::list& bl,
              uint32_t op_flags) override;

    int dump_onode(CollectionHandle& c, const ghobject_t& oid, const std::string& section_name,
    ...
    int collection_list(CollectionHandle& c, const ghobject_t& start, const ghobject_t& end, int max,
                        std::vector<ghobject_t>* ls, ghobject_t* next) override;

    int collection_list_legacy(CollectionHandle& c, const ghobject_t& start, const ghobject_t& end, int max,
                               std::vector<ghobject_t>* ls, ghobject_t* next) override;
    ...
    ObjectMap::ObjectMapIterator get_omap_iterator(CollectionHandle& c,    ///< [in] collection
    ...
    const PerfCounters* get_bluefs_perf_counters() const { return bluefs->get_perf_counters(); }
    KeyValueDB* get_kv() { return db; }

    int queue_transactions(CollectionHandle& ch, std::vector<Transaction>& tls, TrackedOpRef op = TrackedOpRef(),
    ...
    void inject_broken_shared_blob_key(const std::string& key, const ceph::buffer::list& bl);
    ...
    void inject_misreference(coll_t cid1, ghobject_t oid1, coll_t cid2, ghobject_t oid2, uint64_t offset);
    ...
    void inject_bluefs_file(std::string_view dir, std::string_view name, size_t new_size);
    ...
    inline void log_latency(const char* name, int idx, const ceph::timespan& lat, double lat_threshold,
                            const char* info = "") const;

    inline void log_latency_fn(const char* name, int idx, const ceph::timespan& lat, double lat_threshold,
...
private:
    ...
    int _verify_csum(OnodeRef& o, const bluestore_blob_t* blob, uint64_t blob_xoffset, const ceph::buffer::list& bl,
    ...
    struct WriteContext
    {
        ...
        struct write_item
        {
            ...
            write_item(uint64_t logical_offs, BlobRef b, uint64_t blob_len, uint64_t o, ceph::buffer::list& bl,
                       uint64_t o0, uint64_t l0, bool _mark_unused, bool _new_blob)
        ...
        void write(uint64_t loffs, BlobRef b, uint64_t blob_len, uint64_t o, ceph::buffer::list& bl, uint64_t o0,
                   uint64_t len0, bool _mark_unused, bool _new_blob)
        {
        ...
        bool has_conflict(BlobRef b, uint64_t loffs, uint64_t loffs_end, uint64_t min_alloc_size);
    };
    void _do_write_small(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length,
                         ceph::buffer::list::iterator& blp, WriteContext* wctx);
    void _do_write_big_apply_deferred(TransContext* txc, CollectionRef& c, OnodeRef& o, BigDeferredWriteContext& dctx,
                                      bufferlist::iterator& blp, WriteContext* wctx);
    void _do_write_big(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length,
                       ceph::buffer::list::iterator& blp, WriteContext* wctx);
    int _do_alloc_write(TransContext* txc, CollectionRef c, OnodeRef& o, WriteContext* wctx);
    void _wctx_finish(TransContext* txc, CollectionRef& c, OnodeRef& o, WriteContext* wctx,
                      std::set<SharedBlob*>* maybe_unshared_blobs = 0);

    int _write(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, size_t len, ceph::buffer::list& bl,
               uint32_t fadvise_flags);
    void _pad_zeros(ceph::buffer::list* bl, uint64_t* offset, uint64_t chunk_size);

    void _choose_write_options(CollectionRef& c, OnodeRef& o, uint32_t fadvise_flags, WriteContext* wctx);

    int _do_gc(TransContext* txc, CollectionRef& c, OnodeRef& o, const WriteContext& wctx, uint64_t* dirty_start,
               uint64_t* dirty_end);

    int _do_write(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length,
                  ceph::buffer::list& bl, uint32_t fadvise_flags);
    void _do_write_data(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length,
                        ceph::buffer::list& bl, WriteContext* wctx);

    int _touch(TransContext* txc, CollectionRef& c, OnodeRef& o);
    int _do_zero(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, size_t len);
    int _zero(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, size_t len);
    ...
    int _setattr(TransContext* txc, CollectionRef& c, OnodeRef& o, const std::string& name, ceph::buffer::ptr& val);
    int _setattrs(TransContext* txc, CollectionRef& c, OnodeRef& o,
                  const std::map<std::string, ceph::buffer::ptr>& aset);
    int _rmattr(TransContext* txc, CollectionRef& c, OnodeRef& o, const std::string& name);
    ...
    int _omap_setkeys(TransContext* txc, CollectionRef& c, OnodeRef& o, ceph::buffer::list& bl);
    int _omap_setheader(TransContext* txc, CollectionRef& c, OnodeRef& o, ceph::buffer::list& header);
    int _omap_rmkeys(TransContext* txc, CollectionRef& c, OnodeRef& o, ceph::buffer::list& bl);
    int _omap_rmkey_range(TransContext* txc, CollectionRef& c, OnodeRef& o, const std::string& first,
                          const std::string& last);
    int _set_alloc_hint(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t expected_object_size,
                        uint64_t expected_write_size, uint32_t flags);
    int _do_clone_range(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo, uint64_t srcoff,
                        uint64_t length, uint64_t dstoff);
    int _clone(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo);
    int _clone_range(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo, uint64_t srcoff,
                     uint64_t length, uint64_t dstoff);
    ...
    void _collect_allocation_stats(uint64_t need, uint32_t alloc_size, const PExtentVector&);
...
private:
    ...
    //
    std::array<std::tuple<uint64_t, uint64_t, uint64_t>, 5> alloc_stats_history = {std::make_tuple(0ul, 0ul, 0ul)};

    inline bool _use_rotational_settings();

public:
    typedef btree::btree_set<uint64_t, std::less<uint64_t>, mempool::bluestore_fsck::pool_allocator<uint64_t>>
        uint64_t_btree_t;
    ...
    struct FSCK_ObjectCtx
    {
        ...
        FSCK_ObjectCtx(int64_t& e, int64_t& w, uint64_t& _num_objects, uint64_t& _num_extents, uint64_t& _num_blobs,
                       uint64_t& _num_sharded_objects, uint64_t& _num_spanning_blobs, mempool_dynamic_bitset* _ub,
                       uint64_t_btree_t* _used_omap_head,
                       std::vector<std::unordered_map<ghobject_t, uint64_t>>* _zone_refs,

                       ceph::mutex* _sb_info_lock, sb_info_space_efficient_map_t& _sb_info,
                       shared_blob_2hash_tracker_t& _sb_ref_counts, store_statfs_t& _store_statfs,
                       per_pool_statfs& _pool_statfs, BlueStoreRepairer* _repairer)
            : errors(e)
            , warnings(w)
    ...
    OnodeRef fsck_check_objects_shallow(FSCKDepth depth, int64_t pool_id, CollectionRef c, const ghobject_t& oid,
                                        const std::string& key, const ceph::buffer::list& value,
...
private:
    ...
    public:
        ExtentDecoderPartial(BlueStore& _store, read_alloc_stats_t& _stats, SimpleBitmap& _sbmap,
                             sb_info_space_efficient_map_t& _sb_info, uint8_t _min_alloc_size_order)
            : store(_store)
            , stats(_stats)
            , sbmap(_sbmap)
            , sb_info(_sb_info)
    ...
    friend std::ostream& operator<<(std::ostream& out, const read_alloc_stats_t& stats)
    {
        ...
        out << "NCB::onode_count             = ";
        out.width(10);
        out << stats.onode_count << std::endl << "NCB::shard_count             = ";
        out.width(10);
        out << stats.shard_count << std::endl << "NCB::shared_blob_count      = ";
        out.width(10);
        out << stats.shared_blob_count << std::endl << "NCB::compressed_blob_count   = ";
        out.width(10);
        out << stats.compressed_blob_count << std::endl << "NCB::spanning_blob_count     = ";
        out.width(10);
        out << stats.spanning_blob_count << std::endl << "NCB::skipped_illegal_extent  = ";
        out.width(10);
        out << stats.skipped_illegal_extent << std::endl << "NCB::extent_count            = ";
        out.width(10);
        out << stats.extent_count << std::endl << "NCB::insert_count            = ";
        out.width(10);
        out << stats.insert_count << std::endl;
    ...
    int allocator_add_restored_entries(Allocator* allocator, const void* buff, unsigned extent_count,
                                       uint64_t* p_read_alloc_size, uint64_t* p_extent_count, const void* v_header,
                                       BlueFS::FileReader* p_handle, uint64_t offset);
    ...
    void _fsck_check_object_omap(FSCKDepth depth, OnodeRef& o, const BlueStore::FSCK_ObjectCtx& ctx);
...
{
    return out << " allocated:" << s.values[BlueStore::volatile_statfs::STATFS_ALLOCATED]
               << " stored:" << s.values[BlueStore::volatile_statfs::STATFS_STORED]
               << " compressed:" << s.values[BlueStore::volatile_statfs::STATFS_COMPRESSED]
               << " compressed_orig:" << s.values[BlueStore::volatile_statfs::STATFS_COMPRESSED_ORIGINAL]
               << " compressed_alloc:" << s.values[BlueStore::volatile_statfs::STATFS_COMPRESSED_ALLOCATED];
}
...
public:
    ...
    struct StoreSpaceTracker
    {
        ...
        void init(uint64_t total, uint64_t min_alloc_size, uint64_t mem_cap = DEF_MEM_CAP)
        {
            ...
            collections_bfs.resize(
                entries,
                bloom_filter(BLOOM_FILTER_SALT_COUNT, BLOOM_FILTER_TABLE_SIZE, 0, BLOOM_FILTER_EXPECTED_COUNT));
            objects_bfs.resize(
                entries,
                bloom_filter(BLOOM_FILTER_SALT_COUNT, BLOOM_FILTER_TABLE_SIZE, 0, BLOOM_FILTER_EXPECTED_COUNT));
        }
        inline uint32_t get_hash(const coll_t& cid) const { return cid.hash_to_shard(1); }
        inline void set_used(uint64_t offset, uint64_t len, const coll_t& cid, const ghobject_t& oid)
        {
...
public:
    ...
    bool fix_shared_blob(KeyValueDB::Transaction txn, uint64_t sbid, bluestore_extent_ref_map_t* ref_map,
                         size_t repaired = 1);
    bool fix_statfs(KeyValueDB* db, const std::string& key, const store_statfs_t& new_statfs);

    bool fix_leaked(KeyValueDB* db, FreelistManager* fm, uint64_t offset, uint64_t len);
    bool fix_false_free(KeyValueDB* db, FreelistManager* fm, uint64_t offset, uint64_t len);
    bool fix_spanning_blobs(KeyValueDB* db, std::function<void(KeyValueDB::Transaction)> f);
    ...
    void inc_repaired(size_t n = 1) { to_repair_cnt += n; }
    void request_compaction() { need_compact = true; }
    //////////////////////

    void init_space_usage_tracker(uint64_t total_space, uint64_t lres_tracking_unit_size)
    {
        // NB: not for use in multithreading mode!!!
        space_usage_tracker.init(total_space, lres_tracking_unit_size);
    }
    void set_space_used(uint64_t offset, uint64_t len, const coll_t& cid, const ghobject_t& oid)
    {
        std::lock_guard l(lock);
...
class RocksDBBlueFSVolumeSelector : public BlueFSVolumeSelector
{
    ...
    typedef matrix_2d<std::atomic<uint64_t>, BlueFS::MAX_BDEV + 1, LEVEL_MAX - LEVEL_FIRST + 1>
...
public:
    RocksDBBlueFSVolumeSelector(uint64_t _wal_total, uint64_t _db_total, uint64_t _slow_total, uint64_t _level0_size,
                                uint64_t _level_base, uint64_t _level_multiplier, double reserved_factor,
                                uint64_t reserved, bool new_pol)
    {
    ...
    void* get_hint_for_log() const override { return reinterpret_cast<void*>(LEVEL_LOG); }
    ...
    void get_paths(const std::string& base, BlueFSVolumeSelector::paths& res) const override;
