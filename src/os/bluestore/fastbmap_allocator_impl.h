...
struct interval_t
{
    ...
    interval_t() {}
    interval_t(uint64_t o, uint64_t l)
        : offset(o)
        , length(l)
    {}
...
protected:
    ...
    inline bool _is_slot_fully_allocated(uint64_t idx) const { return l1[idx] == all_slot_clear; }
...
class AllocatorLevel01Loose : public AllocatorLevel01
{
    ...
    interval_t _get_longest_from_l0(uint64_t pos0, uint64_t pos1, uint64_t min_length, interval_t* tail) const;

    inline void _fragment_and_emplace(uint64_t max_length, uint64_t offset, uint64_t len, interval_vector_t* res)
    {
        auto it = res->rbegin();
        if (max_length) {
    ...
    bool _allocate_l0(uint64_t length, uint64_t max_length, uint64_t l0_pos0, uint64_t l0_pos1, uint64_t* allocated,
                      interval_vector_t* res)
    {
        ...
        for (auto idx = l0_pos0 / d0; (idx < l0_pos1 / d0) && (length > *allocated); ++idx) {
            ++l0_iterations;
            ...
            if (slot_val == all_slot_clear) {
                ...
                *allocated += to_alloc * l0_granularity;
                ...
                _fragment_and_emplace(max_length, base * l0_granularity, to_alloc * l0_granularity, res);
            ...
            while (next_pos < bits_per_slot && (next_pos - free_pos) < need_entries) {
                ...
                if (0 == (slot_val & (slot_t(1) << next_pos))) {
                    ...
                    *allocated += to_alloc * l0_granularity;
                    ++alloc_fragments;
                    need_entries -= to_alloc;
                    _fragment_and_emplace(
                        max_length, (base + free_pos) * l0_granularity, to_alloc * l0_granularity, res);
            ...
            if (need_entries && free_pos < bits_per_slot) {
                ...
                *allocated += to_alloc * l0_granularity;
                ++alloc_fragments;
                need_entries -= to_alloc;
                _fragment_and_emplace(max_length, (base + free_pos) * l0_granularity, to_alloc * l0_granularity, res);
...
protected:
    ...
    void _init(uint64_t capacity, uint64_t _alloc_unit, bool mark_as_free = true)
    {
        ...
        auto aligned_capacity =
            p2roundup((int64_t)capacity, int64_t(l1_granularity * slots_per_slotset * _children_per_slot()));
        size_t slot_count = aligned_capacity / l1_granularity / _children_per_slot();
    ...
    void _analyze_partials(uint64_t pos_start, uint64_t pos_end, uint64_t length, uint64_t min_length, int mode,
                           search_ctx_t* ctx);
    ...
    interval_t _allocate_l1_contiguous(uint64_t length, uint64_t min_length, uint64_t max_length, uint64_t pos_start,
                                       uint64_t pos_end);

    bool _allocate_l1(uint64_t length, uint64_t min_length, uint64_t max_length, uint64_t l1_pos_start,
                      uint64_t l1_pos_end, uint64_t* allocated, interval_vector_t* res);
    ...
    uint64_t claim_free_to_left_l1(uint64_t offs)
    {
        ...
        if (l0_pos_start < l0_pos_end) {
            _mark_l1_on_l0(p2align(l0_pos_start, uint64_t(bits_per_slotset)),
...
public:
    ...
    uint64_t claim_free_to_left(uint64_t offset)
    {
        ...
        uint64_t l2_pos_end = p2roundup(int64_t(offset), int64_t(l2_granularity)) / l2_granularity;
...
protected:
    ...
    void _init(uint64_t capacity, uint64_t _alloc_unit, bool mark_as_free = true)
    {
        ...
        l2_granularity = l1._level_granularity() * l1._children_per_slot() * slots_per_slotset;

        // capacity to have slot alignment at l2
        auto aligned_capacity = p2roundup((int64_t)capacity, (int64_t)l2_granularity * L1_ENTRIES_PER_SLOT);
        ...
        if (mark_as_free) {
            ...
            auto l2_pos_no_use = p2roundup((int64_t)capacity, (int64_t)l2_granularity) / l2_granularity;
    ...
    void _allocate_l2(uint64_t length, uint64_t min_length, uint64_t max_length, uint64_t hint,
