...
StupidAllocator::StupidAllocator(CephContext* cct, const std::string& name, int64_t _size, int64_t _block_size)
    : Allocator(name, _size, _block_size)
    , cct(cct)
    , num_free(0)
    , free(10)
{
...
void StupidAllocator::_insert_free(uint64_t off, uint64_t len)
{
    ...
    while (true) {
        ...
        if (newbin == bin) break;
        ldout(cct, 30) << __func__ << " promoting 0x" << std::hex << off << "~" << len << std::dec << " to bin "
                       << newbin << dendl;
...
uint64_t StupidAllocator::_aligned_len(StupidAllocator::interval_set_t::iterator p, uint64_t alloc_unit)
{
    uint64_t skew = p.get_start() % alloc_unit;
    if (skew) skew = alloc_unit - skew;
    if (skew > p.get_len())
        return 0;
    else
...
int64_t StupidAllocator::allocate_int(uint64_t want_size, uint64_t alloc_unit, int64_t hint, uint64_t* offset,
                                      uint32_t* length)
{
    std::lock_guard l(lock);
    ldout(cct, 10) << __func__ << " want_size 0x" << std::hex << want_size << " alloc_unit 0x" << alloc_unit
...
found:
    ...
    *length = std::min(std::max(alloc_unit, want_size), p2align((p.get_len() - skew), alloc_unit));
    if (cct->_conf->bluestore_debug_small_allocations) {
        uint64_t max = alloc_unit * (rand() % cct->_conf->bluestore_debug_small_allocations);
        if (max && *length > max) {
            ldout(cct, 10) << __func__ << " shortening allocation of 0x" << std::hex << *length << " -> 0x" << max
                           << " due to debug_small_allocations" << std::dec << dendl;
            *length = max;
        }
    }
    ldout(cct, 30) << __func__ << " got 0x" << std::hex << *offset << "~" << *length << " from bin " << std::dec << bin
                   << dendl;
    ...
    if (*offset && free[bin].contains(*offset - skew - 1, &off, &len)) {
        ...
        if (newbin != bin) {
            ldout(cct, 30) << __func__ << " demoting 0x" << std::hex << off << "~" << len << std::dec << " to bin "
                           << newbin << dendl;
            free[bin].erase(off, len);
...
int64_t StupidAllocator::allocate(uint64_t want_size, uint64_t alloc_unit, uint64_t max_alloc_size, int64_t hint,
                                  PExtentVector* extents)
{
    ...
    while (allocated_size < want_size) {
        res = allocate_int(std::min(max_alloc_size, (want_size - allocated_size)), alloc_unit, hint, &offset, &length);
        if (res != 0) {
            /*
             * Allocation failed.
             */
...
void StupidAllocator::release(const interval_set<uint64_t>& release_set)
{
    std::lock_guard l(lock);
    for (interval_set<uint64_t>::const_iterator p = release_set.begin(); p != release_set.end(); ++p) {
...
double StupidAllocator::get_fragmentation()
{
    ...
    {
        std::lock_guard l(lock);
        max_intervals = p2roundup<uint64_t>(num_free, get_block_size()) / get_block_size();
    ...
    for (unsigned i = 0; i < free.size() && !rm.empty(); ++i) {
        ...
        if (!overlap.empty()) {
            ...
            while (it != it_end) {
                ...
                free[i].erase(o, l, [&](uint64_t off, uint64_t len) {
                    unsigned newbin = _choose_bin(len);
                    if (newbin != i) {
                        ldout(cct, 30) << __func__ << " demoting1 0x" << std::hex << off << "~" << len << std::dec
                                       << " to bin " << newbin << dendl;
                        _insert_free(off, len);
                        return true;
                    }
