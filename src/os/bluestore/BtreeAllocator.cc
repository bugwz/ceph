 ...
 */
uint64_t BtreeAllocator::_pick_block_after(uint64_t* cursor, uint64_t size, uint64_t align)
{
...
uint64_t BtreeAllocator::_pick_block_fits(uint64_t size, uint64_t align)
{
...
void BtreeAllocator::_process_range_removal(uint64_t start, uint64_t end, BtreeAllocator::range_tree_t::iterator& rs)
{
...
int64_t BtreeAllocator::_allocate(uint64_t want, uint64_t unit, uint64_t max_alloc_size,
...
int BtreeAllocator::_allocate(uint64_t size, uint64_t unit, uint64_t* offset, uint64_t* length)
{
    uint64_t max_size = 0;
...
BtreeAllocator::BtreeAllocator(CephContext* cct, int64_t device_size, int64_t block_size, uint64_t max_mem,
...
BtreeAllocator::BtreeAllocator(CephContext* cct, int64_t device_size, int64_t block_size, std::string_view name)
...
int64_t BtreeAllocator::allocate(uint64_t want, uint64_t unit, uint64_t max_alloc_size,
                                 int64_t hint,   // unused, for now!
                                 PExtentVector* extents)
{
...
void BtreeAllocator::_dump() const
{
    ...
    for (auto& rs : range_tree) {
        ldout(cct, 0) << std::hex << "0x" << rs.first << "~" << rs.second << std::dec << dendl;
    }

    ldout(cct, 0) << __func__ << " range_size_tree: " << dendl;
    for (auto& rs : range_size_tree) {
        ldout(cct, 0) << std::hex << "0x" << rs.size << "@" << rs.start << std::dec << dendl;
    }
}
