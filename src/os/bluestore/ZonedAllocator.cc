...
ZonedAllocator::ZonedAllocator(CephContext* cct, int64_t size, int64_t blk_size, int64_t _zone_size,
                               int64_t _first_sequential_zone, std::string_view name)
...
{
    ldout(cct, 10) << " size 0x" << std::hex << size << ", zone size 0x" << zone_size << std::dec
                   << ", number of zones 0x" << num_zones << ", first sequential zone 0x" << starting_zone_num
                   << ", sequential size 0x" << sequential_size << std::dec << dendl;
...
int64_t ZonedAllocator::allocate(uint64_t want_size, uint64_t alloc_unit, uint64_t max_alloc_size, int64_t hint,
                                 PExtentVector* extents)
{
    ...
    ldout(cct, 10) << " trying to allocate 0x" << std::hex << want_size << std::dec << dendl;
    ...
    for (; left > 0; ++zone_num, --left) {
        ...
        if (zone_num == cleaning_zone) {
            ldout(cct, 10) << " skipping zone 0x" << std::hex << zone_num << " because we are cleaning it" << std::dec
                           << dendl;
            continue;
        }
        if (!fits(want_size, zone_num)) {
            ldout(cct, 10) << " skipping zone 0x" << std::hex << zone_num << " because there is not enough space: "
                           << " want_size = 0x" << want_size << " available = 0x" << get_remaining_space(zone_num)
                           << std::dec << dendl;
            continue;
        }
        break;
    ...
    ldout(cct, 10) << " moving zone 0x" << std::hex << zone_num << " write pointer from 0x" << offset << " -> 0x"
                   << offset + want_size << std::dec << dendl;
    ...
    ldout(cct, 10) << " allocated 0x" << std::hex << offset << "~" << want_size << " from zone 0x" << zone_num
                   << " and zone offset 0x" << (offset % zone_size) << std::dec << dendl;
...
void ZonedAllocator::release(const interval_set<uint64_t>& release_set)
{
    ...
    for (auto p = cbegin(release_set); p != cend(release_set); ++p) {
        ...
        ldout(cct, 10) << " 0x" << std::hex << offset << "~" << length << " from zone 0x" << zone_num << std::dec
                       << dendl;
...
void ZonedAllocator::foreach (std::function<void(uint64_t offset, uint64_t length)> notify)
{
    std::lock_guard l(lock);
}

void ZonedAllocator::init_from_zone_pointers(std::vector<zone_state_t>&& _zone_states)
{
    ...
    ldout(cct, 10) << "free 0x" << std::hex << num_sequential_free << " / 0x" << sequential_size << std::dec << dendl;
}
...
int64_t ZonedAllocator::pick_zone_to_clean(float min_score, uint64_t min_saved)
{
    ...
    for (size_t i = first_seq_zone_num; i < num_zones; ++i) {
        ...
        float score = (float)zone_states[i].num_dead_bytes / (float)(zone_states[i].get_num_live_bytes() + 1);
        if (score > 0) {
            ldout(cct, 20) << " zone 0x" << std::hex << i << " dead 0x" << zone_states[i].num_dead_bytes << " score "
                           << score << dendl;
        }
    ...
    if (best_score >= min_score) {
        ldout(cct, 10) << " zone 0x" << std::hex << best << " with score " << best_score << ": 0x"
                       << zone_states[best].num_dead_bytes << " dead and 0x"
                       << zone_states[best].write_pointer - zone_states[best].num_dead_bytes << " live bytes"
                       << std::dec << dendl;
    }
    else if (best > 0) {
        ldout(cct, 10) << " zone 0x" << std::hex << best << " with score " << best_score << ": 0x"
                       << zone_states[best].num_dead_bytes << " dead and 0x"
                       << zone_states[best].write_pointer - zone_states[best].num_dead_bytes << " live bytes"
                       << std::dec << " but below min_score " << min_score << dendl;
        best = -1;
    }
    else {
...
bool ZonedAllocator::low_on_space(void)
{
    ...
    ldout(cct, 10) << " free 0x" << std::hex << num_sequential_free << "/ 0x" << sequential_size << std::dec
