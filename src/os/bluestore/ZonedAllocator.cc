...
ZonedAllocator::ZonedAllocator(CephContext* cct, int64_t size, int64_t block_size, const std::string& name)
...
{
    ldout(cct, 10) << __func__ << " size 0x" << std::hex << size << " zone size 0x" << zone_size << std::dec
                   << " number of zones " << num_zones << " first sequential zone " << starting_zone_num << dendl;
                                 ...
                                 PExtentVector* extents)
{
    ...
    for (; zone_num < num_zones; ++zone_num) {
        ...
        ldout(cct, 10) << __func__ << " skipping zone " << zone_num << " because there is not enough space: "
                       << " want_size = " << want_size << " available = " << get_remaining_space(zone_num) << dendl;
    }
    ...
    ldout(cct, 10) << __func__ << " advancing zone " << std::hex << zone_num << " write pointer from " << offset
    ...
    ldout(cct, 10) << __func__ << std::hex << " zone " << zone_num << " offset is now " << get_write_pointer(zone_num)
                   << dendl;

    ldout(cct, 10) << __func__ << " allocated " << std::hex << want_size << " bytes at offset " << offset
                   << " located at zone " << zone_num << " and zone offset " << offset % zone_size << dendl;
...
void ZonedAllocator::init_rm_free(uint64_t offset, uint64_t length)
{
    ...
    ldout(cct, 40) << __func__ << " set zone 0x" << std::hex << zone_num << " write pointer to 0x" << zone_size
                   << dendl;
