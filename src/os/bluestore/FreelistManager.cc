...
FreelistManager* FreelistManager::create(CephContext* cct, std::string type, std::string prefix)
{
    ...
    if (type == "bitmap") return new BitmapFreelistManager(cct, "B", "b");
    ...
    // of BlueStore.cc.
    if (type == "zoned") return new ZonedFreelistManager(cct, "Z", "z");
#endif

    return NULL;
}

void FreelistManager::setup_merge_operators(KeyValueDB* db, const std::string& type)
{
#ifdef HAVE_LIBZBD
    if (type == "zoned")
