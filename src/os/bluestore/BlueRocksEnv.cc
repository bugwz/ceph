...
{
    switch (r) {
    case 0: return rocksdb::Status::OK();
    case -ENOENT: return rocksdb::Status::NotFound(rocksdb::Status::kNone);
    case -EINVAL: return rocksdb::Status::InvalidArgument(rocksdb::Status::kNone);
    case -EIO:
    case -EEXIST: return rocksdb::Status::IOError(rocksdb::Status::kNone);
    case -ENOLCK: return rocksdb::Status::IOError(strerror(r));
    default:
        // FIXME :(
...
std::pair<std::string_view, std::string_view> split(const std::string& fn)
{
    ...
    return {string_view(fn.data(), slash), string_view(fn.data() + file_begin, fn.size() - file_begin)};
}
...
public:
    ...
    {
        return snprintf(id, max_size, "%016llx", (unsigned long long)h->file->fnode.ino);
    };
    ...
    bool use_direct_io() const override { return !fs->cct->_conf->bluefs_buffered_io; }
...
public:
    ...
    void SetWriteLifeTimeHint(rocksdb::Env::WriteLifeTimeHint hint) override { h->write_hint = (const int)hint; }
    ...
    rocksdb::Status RangeSync(uint64_t offset, uint64_t nbytes) override
    {
        ...
        nbytes += partial;
        nbytes &= ~4095;
        if (nbytes) fs->flush_range(h, offset, nbytes);
        return rocksdb::Status::OK();
    }
...
public:
    explicit BlueRocksDirectory(BlueFS* f)
        : fs(f)
    {}
...
BlueRocksEnv::BlueRocksEnv(BlueFS* f)
    ...
    ,   // forward most of it to POSIX
    fs(f)
{}

rocksdb::Status BlueRocksEnv::NewSequentialFile(const std::string& fname,
                                                std::unique_ptr<rocksdb::SequentialFile>* result,
                                                const rocksdb::EnvOptions& options)
{
    if (fname[0] == '/') return target()->NewSequentialFile(fname, result, options);
...
rocksdb::Status BlueRocksEnv::NewRandomAccessFile(const std::string& fname,
...
rocksdb::Status BlueRocksEnv::NewWritableFile(const std::string& fname, std::unique_ptr<rocksdb::WritableFile>* result,
                                              const rocksdb::EnvOptions& options)
{
    auto [dir, file] = split(fname);
    BlueFS::FileWriter* h;
...
rocksdb::Status BlueRocksEnv::ReuseWritableFile(const std::string& new_fname, const std::string& old_fname,
...
rocksdb::Status BlueRocksEnv::NewDirectory(const std::string& name, std::unique_ptr<rocksdb::Directory>* result)
{
    if (!fs->dir_exists(name)) return rocksdb::Status::NotFound(name, strerror(ENOENT));
...
{
    if (fname[0] == '/') return target()->FileExists(fname);
    auto [dir, file] = split(fname);
    if (fs->stat(dir, file, NULL, NULL) == 0) return rocksdb::Status::OK();
    return err_to_status(-ENOENT);
}

rocksdb::Status BlueRocksEnv::GetChildren(const std::string& dir, std::vector<std::string>* result)
{
    result->clear();
    int r = fs->readdir(dir, result);
    if (r < 0) return rocksdb::Status::NotFound(dir, strerror(ENOENT));   //    return err_to_status(r);
    return rocksdb::Status::OK();
}
...
{
    int r = fs->mkdir(dirname);
    if (r < 0 && r != -EEXIST) return err_to_status(r);
    return rocksdb::Status::OK();
}
...
rocksdb::Status BlueRocksEnv::GetFileSize(const std::string& fname, uint64_t* file_size)
{
    auto [dir, file] = split(fname);
...
rocksdb::Status BlueRocksEnv::GetFileModificationTime(const std::string& fname, uint64_t* file_mtime)
{
    auto [dir, file] = split(fname);
    utime_t mtime;
...
rocksdb::Status BlueRocksEnv::RenameFile(const std::string& src, const std::string& target)
{
...
rocksdb::Status BlueRocksEnv::LinkFile(const std::string& src, const std::string& target)
{
    ceph_abort();
}

rocksdb::Status BlueRocksEnv::AreFilesSame(const std::string& first, const std::string& second, bool* res)
{
...
rocksdb::Status BlueRocksEnv::LockFile(const std::string& fname, rocksdb::FileLock** lock)
{
    auto [dir, file] = split(fname);
...
rocksdb::Status BlueRocksEnv::GetAbsolutePath(const std::string& db_path, std::string* output_path)
{
    // this is a lie...
    *output_path = "/" + db_path;
    return rocksdb::Status::OK();
...
rocksdb::Status BlueRocksEnv::NewLogger(const std::string& fname, std::shared_ptr<rocksdb::Logger>* result)
{
    // ignore the filename :)
