        ...
        , name(_name)
    {
        ...
        if (admin_socket) {
            int r = admin_socket->register_command(
                ("bluestore allocator dump " + name).c_str(), this, "dump allocator free regions");
            if (r != 0) alloc = nullptr;   // some collision, disable
            if (alloc) {
...
Allocator::Allocator(std::string_view name, int64_t _capacity, int64_t _block_size)
...
Allocator* Allocator::create(CephContext* cct, std::string_view type, int64_t size, int64_t block_size,
                             int64_t zone_size, int64_t first_sequential_zone, std::string_view name)
{
    ...
    if (type == "stupid") {
        ...
        return new HybridAllocator(
            cct, size, block_size, cct->_conf.get_val<uint64_t>("bluestore_hybrid_alloc_mem_cap"), name);
#ifdef HAVE_LIBZBD
    }
    else if (type == "zoned") {
        return new ZonedAllocator(cct, size, block_size, zone_size, first_sequential_zone, name);
#endif
    }
    if (alloc == nullptr) {
        lderr(cct) << "Allocator::" << __func__ << " unknown alloc type " << type << dendl;
    }
    return alloc;
}
...
double Allocator::get_fragmentation_score()
{
    ...
    auto get_score = [&](size_t v) -> double {
        ...
        double score = (sc_shifted)*scales[sc] * (1 - x) + (sc_shifted * 2) * scales[sc + 1] * x;
        return score;
    };
