...
void ZonedFreelistManager::write_zone_state_delta_to_db(uint64_t zone_num, const zone_state_t& zone_state,
...
void ZonedFreelistManager::write_zone_state_reset_to_db(uint64_t zone_num, const zone_state_t& zone_state,
                                                        KeyValueDB::Transaction txn)
{
    string key;
    _key_encode_u64(zone_num, &key);
...
void ZonedFreelistManager::load_zone_state_from_db(uint64_t zone_num, zone_state_t& zone_state,
...
ZonedFreelistManager::ZonedFreelistManager(CephContext* cct, string meta_prefix, string info_prefix)
    : FreelistManager(cct)
...
int ZonedFreelistManager::create(uint64_t new_size, uint64_t granularity, uint64_t new_zone_size,
                                 uint64_t first_sequential_zone, KeyValueDB::Transaction txn)
{
    size = new_size;
    ...
    dout(1) << __func__ << std::hex << " size 0x" << size << " bytes_per_block 0x" << bytes_per_block
            << " zone size 0x " << zone_size << " num_zones 0x" << num_zones << " starting_zone 0x" << starting_zone_num
            << dendl;
    {
        bufferlist bl;
        encode(size, bl);
...
int ZonedFreelistManager::init(KeyValueDB* kvdb, bool db_in_read_only, cfg_reader_t cfg_reader)
{
    ...
    dout(10) << __func__ << std::hex << " size 0x" << size << " bytes_per_block 0x" << bytes_per_block
             << " zone size 0x" << zone_size << " num_zones 0x" << num_zones << " starting_zone 0x" << starting_zone_num
             << std::dec << dendl;
    return 0;
}
...
// this call.
bool ZonedFreelistManager::enumerate_next(KeyValueDB* kvdb, uint64_t* offset, uint64_t* length)
{
    std::lock_guard l(lock);
...
void ZonedFreelistManager::allocate(uint64_t offset, uint64_t length, KeyValueDB::Transaction txn)
{
    while (length > 0) {
        uint64_t zone_num = offset / zone_size;
        uint64_t this_len = std::min(length, zone_size - offset % zone_size);
        dout(10) << __func__ << " 0x" << std::hex << offset << "~" << this_len << " zone 0x" << zone_num << std::dec
                 << dendl;
        zone_state_t zone_state;
...
void ZonedFreelistManager::release(uint64_t offset, uint64_t length, KeyValueDB::Transaction txn)
{
    while (length > 0) {
        uint64_t zone_num = offset / zone_size;
        uint64_t this_len = std::min(length, zone_size - offset % zone_size);
...
void ZonedFreelistManager::get_meta(uint64_t target_size, std::vector<std::pair<string, string>>* res) const
{
...
std::vector<zone_state_t> ZonedFreelistManager::get_zone_states(KeyValueDB* kvdb) const
{
...
int ZonedFreelistManager::_read_cfg(cfg_reader_t cfg_reader)
{
    ...
    string keys[key_count] = {
        "zfm_size", "zfm_bytes_per_block", "zfm_zone_size", "zfm_num_zones", "zfm_starting_zone_num"};
    uint64_t* vals[key_count] = {&size, &bytes_per_block, &zone_size, &num_zones, &starting_zone_num};
...
void ZonedFreelistManager::mark_zone_to_clean_free(uint64_t zone, KeyValueDB* kvdb)
{
