...
struct bluestore_cnode_t
{
    ...
    explicit bluestore_cnode_t(int b = 0)
        : bits(b)
    {}
...
template<typename OFFS_TYPE, typename LEN_TYPE> struct bluestore_interval_t
{
    ...
    bluestore_interval_t(uint64_t o, uint64_t l)
        : offset(o)
        , length(l)
    {}

    bool is_valid() const { return offset != INVALID_OFFSET; }
    uint64_t end() const { return offset != INVALID_OFFSET ? offset + length : INVALID_OFFSET; }
...
template<> struct denc_traits<PExtentVector>
{
    ...
    static void encode(const PExtentVector& v, ceph::buffer::list::contiguous_appender& p)
    {
...
struct bluestore_extent_ref_map_t
{
    struct record_t
    {
        uint32_t length;
        uint32_t refs;
        record_t(uint32_t l = 0, uint32_t r = 0)
            : length(l)
            , refs(r)
        {}
    ...
    void put(uint64_t offset, uint32_t len, PExtentVector* release, bool* maybe_unshared);
...
static inline bool operator==(const bluestore_extent_ref_map_t& l, const bluestore_extent_ref_map_t& r)
{
...
public:
    ...
    void encode(ceph::buffer::list::contiguous_appender& p, uint64_t struct_v) const
    {
        ...
        if (has_csum()) {
            ...
            denc(csum_chunk_order, p);
            denc_varint(csum_data.length(), p);
            memcpy(p.get_pos_add(csum_data.length()), csum_data.c_str(), csum_data.length());
        }
        if (has_unused()) {
            denc(unused, p);
        }
    ...
    bool can_split_at(uint32_t blob_offset) const { return !has_csum() || blob_offset % get_csum_chunk_size() == 0; }
    ...
    {
        return has_csum() ? std::max<uint64_t>(dev_block_size, get_csum_chunk_size()) : dev_block_size;
    }
    uint32_t get_csum_chunk_size() const { return 1 << csum_chunk_order; }
    uint32_t get_compressed_payload_length() const { return is_compressed() ? compressed_length : 0; }
    ...
    bool _validate_range(uint64_t b_off, uint64_t b_len, bool require_allocated) const
    {
        auto p = extents.begin();
        ceph_assert(p != extents.end());
    ...
    bool is_allocated(uint64_t b_off, uint64_t b_len) const { return _validate_range(b_off, b_len, true); }

    /// return true if the entire range is unallocated
    /// (not mapped to extents on disk)
    bool is_unallocated(uint64_t b_off, uint64_t b_len) const { return _validate_range(b_off, b_len, false); }
    ...
    template<class F, typename std::enable_if<std::is_invocable_r_v<int, F, uint64_t, uint64_t>>::type* = nullptr>
    int map_f_invoke(uint64_t lo, const bluestore_pextent_t& p, uint64_t o, uint64_t l, F&& f) const
    {
        return f(o, l);
    }

    template<class F,
             typename std::enable_if<std::is_invocable_r_v<int, F, uint64_t, uint64_t, uint64_t>>::type* = nullptr>
    int map_f_invoke(uint64_t lo, const bluestore_pextent_t& p, uint64_t o, uint64_t l, F&& f) const
    {
        return f(lo, o, l);
    }

    template<class F, typename std::enable_if<std::is_invocable_r_v<int, F, const bluestore_pextent_t&, uint64_t,
    ...
    template<class F> void map_bl(uint64_t x_off, ceph::buffer::list& bl, F&& f) const
    {
    ...
    uint64_t get_csum_item(unsigned i) const
    {
        ...
        switch (cs) {
        case 0: ceph_abort_msg("no csum data, bad index");
        case 1: return reinterpret_cast<const uint8_t*>(p)[i];
        case 2: return reinterpret_cast<const ceph_le16*>(p)[i];
        case 4: return reinterpret_cast<const ceph_le32*>(p)[i];
        case 8: return reinterpret_cast<const ceph_le64*>(p)[i];
        default: ceph_abort_msg("unrecognized csum word size");
        }
    }
    ...
    int verify_csum(uint64_t b_off, const ceph::buffer::list& bl, int* b_bad_off, uint64_t* bad_csum) const;
    ...
    void prune_tail()
    {
        ...
        if (has_csum()) {
            ...
            t.swap(csum_data);
            csum_data =
                ceph::buffer::ptr(t.c_str(), get_logical_length() / get_csum_chunk_size() * get_csum_value_size());
        }
    }
    void add_tail(uint32_t new_len)
    {
        ...
        extents.emplace_back(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, new_len - logical_length));
        logical_length = new_len;
        if (has_csum()) {
            ceph::buffer::ptr t;
            t.swap(csum_data);
            csum_data = ceph::buffer::create(get_csum_value_size() * logical_length / get_csum_chunk_size());
...
struct bluestore_onode_t
{
    ...
    static bool is_perpool_omap(uint8_t flags) { return flags & FLAG_PERPOOL_OMAP; }
    ...
    void set_omap_flags(bool legacy) { set_flag(FLAG_OMAP | (legacy ? 0 : (FLAG_PERPOOL_OMAP | FLAG_PERPG_OMAP))); }
    void set_omap_flags_pgmeta() { set_flag(FLAG_OMAP | FLAG_PGMETA_OMAP); }

    void clear_omap_flag() { clear_flag(FLAG_OMAP | FLAG_PGMETA_OMAP | FLAG_PERPOOL_OMAP | FLAG_PERPG_OMAP); }
...
template<template<typename> typename V, class COUNTER_TYPE = int32_t> class ref_counter_2hash_tracker_t
{
...
public:
    ...
    {
        auto h = ceph_str_hash_rjenkins((const char*)hash_val, hash_val_len) % num_buckets;
        if (buckets1[h] == 0 && n) {
            ++num_non_zero;
        }
    ...
    bool test_hash_conflict(const char* hash_val1, const char* hash_val2, size_t hash_val_len) const
    {
        ...
        return ((h1 % num_buckets) == (h2 % num_buckets)) && ((h3 % num_buckets) == (h4 % num_buckets));
    }
...
class shared_blob_2hash_tracker_t : public ref_counter_2hash_tracker_t<mempool::bluestore_fsck::vector>
{
    ...
    static size_t get_hash_input_size() { return hash_input_len * sizeof(hash_input_t::value_type); }
...
public:
    ...
    bool test_hash_conflict(uint64_t sbid, uint64_t offset, uint64_t sbid2, uint64_t offset2) const;
...
public:
    ...
    sb_info_t(int64_t _sbid = 0)
        : sbid(_sbid)
    {}
    bool operator<(const sb_info_t& other) const { return std::abs(sbid) < std::abs(other.sbid); }
    bool operator<(const uint64_t& other_sbid) const { return uint64_t(std::abs(sbid)) < other_sbid; }
    ...
    auto find(uint64_t id)
    {
        if (items.size() != 0) {
            auto it = std::lower_bound(
                items.begin(), items.end() - 1, id, [](const sb_info_t& a, const uint64_t& b) { return a < b; });
            if (it->get_sbid() == id) {
                return it;
            }
            if (aux_items.size() != 0) {
                auto it =
                    std::lower_bound(aux_items.begin(), aux_items.end(), id, [](const sb_info_t& a, const uint64_t& b) {
                        return a < b;
                    });
                if (it->get_sbid() == id) {
                    return it;
...
private:
    sb_info_t& _add(int64_t id)
    {
        ...
        it = std::upper_bound(aux_items.begin(), aux_items.end(), n_id, [](const uint64_t& a, const sb_info_t& b) {
            return a < b.get_sbid();
        });
