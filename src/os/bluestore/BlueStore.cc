...
MEMPOOL_DEFINE_OBJECT_FACTORY(BlueStore::Onode, bluestore_onode, bluestore_cache_onode);

MEMPOOL_DEFINE_OBJECT_FACTORY(BlueStore::Buffer, bluestore_buffer, bluestore_cache_buffer);
MEMPOOL_DEFINE_OBJECT_FACTORY(BlueStore::Extent, bluestore_extent, bluestore_extent);
MEMPOOL_DEFINE_OBJECT_FACTORY(BlueStore::Blob, bluestore_blob, bluestore_blob);
MEMPOOL_DEFINE_OBJECT_FACTORY(BlueStore::SharedBlob, bluestore_shared_blob, bluestore_shared_blob);

// bluestore_txc
MEMPOOL_DEFINE_OBJECT_FACTORY(BlueStore::TransContext, bluestore_transcontext, bluestore_txc);
using std::byte;
using std::deque;
using std::min;
using std::make_pair;
...
static void get_coll_range(const coll_t& cid, int bits, ghobject_t* temp_start, ghobject_t* temp_end, ghobject_t* start,
...
template<typename S> static void _key_encode_prefix(const ghobject_t& oid, S* key)
{
...
template<typename S> static void _get_object_key(const ghobject_t& oid, S* key)
{
    size_t max_len = ENCODED_KEY_PREFIX_LEN + (oid.hobj.nspace.length() * 3 + 1) +
                     (oid.hobj.get_key().length() * 3 + 1) + 1 +   // for '<', '=', or '>'
...
template<typename S> static void get_object_key(CephContext* cct, const ghobject_t& oid, S* key)
{
    key->clear();
...
// of the prefix bytes.
template<typename S> static void get_extent_shard_key(const S& onode_key, uint32_t offset, string* key)
{
    key->clear();
...
template<typename S>
static void generate_extent_shard_key_and_apply(const S& onode_key, uint32_t offset, string* key,
...
#ifdef HAVE_LIBZBD
static void get_zone_offset_object_key(uint32_t zone, uint64_t offset, ghobject_t oid, std::string* key)
{
    key->clear();
...
static int get_key_zone_offset_object(const string& key, uint32_t* zone, uint64_t* offset, ghobject_t* oid)
{
    const char* p = key.c_str();
    if (key.length() < sizeof(uint64_t) + sizeof(uint32_t) + ENCODED_KEY_PREFIX_LEN + 1) return -1;
...
template<int LogLevelV> void _dump_extent_map(CephContext* cct, const BlueStore::ExtentMap& em)
{
    uint64_t pos = 0;
    for (auto& s : em.shards) {
        dout(LogLevelV) << __func__ << "  shard " << *s.shard_info << (s.loaded ? " (loaded)" : "")
                        << (s.dirty ? " (dirty)" : "") << dendl;
    }
    for (auto& e : em.extent_map) {
        ...
        for (auto& i : e.blob->shared_blob->bc.buffer_map) {
            dout(LogLevelV) << __func__ << "       0x" << std::hex << i.first << "~" << i.second->length << std::dec
                            << " " << *i.second << dendl;
        }
    }
}

template<int LogLevelV> void _dump_onode(CephContext* cct, const BlueStore::Onode& o)
{
    if (!cct->_conf->subsys.should_gather<ceph_subsys_bluestore, LogLevelV>()) return;
    dout(LogLevelV) << __func__ << " " << &o << " " << o.oid << " nid " << o.onode.nid << " size 0x" << std::hex
                    << o.onode.size << " (" << std::dec << o.onode.size << ")"
                    << " expected_object_size " << o.onode.expected_object_size << " expected_write_size "
                    << o.onode.expected_write_size << " in " << o.onode.extent_map_shards.size() << " shards"
                    << ", " << o.extent_map.spanning_blob_map.size() << " spanning blobs" << dendl;
    for (auto& [zone, offset] : o.onode.zone_offset_refs) {
        dout(LogLevelV) << __func__ << " zone ref 0x" << std::hex << zone << " offset 0x" << offset << std::dec
                        << dendl;
    }
...
template<int LogLevelV> void _dump_transaction(CephContext* cct, ObjectStore::Transaction* t)
{
...
{
    out << "buffer(" << &b << " space " << b.space << " 0x" << std::hex << b.offset << "~" << b.length << std::dec
...
public:
    ...
    void lower_bound(const ghobject_t& oid) override
    {
        ...
        if (this->oid().shard_id != oid.shard_id || this->oid().hobj.pool != oid.hobj.pool ||
...
void BlueStore::GarbageCollector::process_protrusive_extents(const BlueStore::ExtentMap& extent_map,
                                                             uint64_t start_offset, uint64_t end_offset,
                                                             uint64_t start_touch_offset, uint64_t end_touch_offset,
                                                             uint64_t min_alloc_size)
{
    ...
    dout(30) << __func__ << " (hex): [" << std::hex << lookup_start_offset << ", " << lookup_end_offset << ")"
             << std::dec << dendl;

    for (auto it = extent_map.seek_lextent(lookup_start_offset);
         it != extent_map.extent_map.end() && it->logical_offset < lookup_end_offset;
         ++it) {
        ...
        if (it->logical_offset >= start_touch_offset && it->logical_end() <= end_touch_offset) {
            ...
            BlobInfo& bi = affected_blobs.emplace(b, BlobInfo(b->get_referenced_bytes())).first->second;

            int adjust = (used_alloc_unit && used_alloc_unit == alloc_unit_start) ? 0 : 1;
            bi.expected_allocations += alloc_unit_end - alloc_unit_start + adjust;
            dout(30) << __func__ << " expected_allocations=" << bi.expected_allocations << " end_au:" << alloc_unit_end
                     << dendl;
            ...
            dout(30) << __func__ << " affected_blob:" << *b << " unref 0x" << std::hex << it->length
                     << " referenced = 0x" << bi.referenced_bytes << std::dec << dendl;
    ...
    for (auto b_it = affected_blobs.begin(); b_it != affected_blobs.end(); ++b_it) {
        Blob* b = b_it->first;
        BlobInfo& bi = b_it->second;
        if (bi.referenced_bytes == 0) {
            uint64_t len_on_disk = b_it->first->get_blob().get_ondisk_length();
            int64_t blob_expected_for_release = round_up_to(len_on_disk, min_alloc_size) / min_alloc_size;

            dout(30) << __func__ << " " << *(b_it->first) << " expected4release=" << blob_expected_for_release
...
int64_t BlueStore::GarbageCollector::estimate(uint64_t start_offset, uint64_t length,
                                              ...
                                              const BlueStore::old_extent_map_t& old_extents, uint64_t min_alloc_size)
{
    ...
    for (auto it = old_extents.begin(); it != old_extents.end(); ++it) {
        ...
        if (b->get_blob().is_compressed()) {
            ...
            if (ref_bytes != 0) {
                dout(30) << __func__ << " affected_blob:" << *b << " unref 0x" << std::hex << o << "~" << l << std::dec
                         << dendl;
                affected_blobs.emplace(b, BlobInfo(ref_bytes));
            }
        }
    }
    dout(30) << __func__ << " gc range(hex): [" << std::hex << gc_start_offset << ", " << gc_end_offset << ")"
             << std::dec << dendl;
    ...
    if (gc_start_offset < start_offset || gc_end_offset > end_offset) {
        process_protrusive_extents(
            extent_map, gc_start_offset, gc_end_offset, start_offset, end_offset, min_alloc_size);
    }
...
{
    typedef boost::intrusive::list<BlueStore::Onode,
                                   boost::intrusive::member_hook<BlueStore::Onode, boost::intrusive::list_member_hook<>,
            ...
            if (!o->lru_item.is_linked()) {
                if (o->exists) {
                    ...
                    *(o->cache_age_bin) += 1;
                    ...
                    --num;
                    o->clear_cached();
                    dout(20) << __func__ << " " << this << " " << o->oid << " removed" << dendl;
                ...
                dout(20) << __func__ << " " << this << " " << o->oid << " touched" << dendl;
            }
        }
        ocs->lock.unlock();
    }
    ...
    void _trim_to(uint64_t new_size) override
    {
        ...
        while (n-- > 0 && lru.size() > 0) {
            ...
            dout(20) << __func__ << "  rm " << o->oid << " " << o->nref << " " << o->cached << dendl;
...
// OnodeCacheShard
BlueStore::OnodeCacheShard* BlueStore::OnodeCacheShard::create(CephContext* cct, string type, PerfCounters* logger)
{
...
struct LruBufferCacheShard : public BlueStore::BufferCacheShard
{
    typedef boost::intrusive::list<
        BlueStore::Buffer, boost::intrusive::member_hook<BlueStore::Buffer, boost::intrusive::list_member_hook<>,
    ...
    void add_stats(uint64_t* extents, uint64_t* blobs, uint64_t* buffers, uint64_t* bytes) override
    {
        *extents += num_extents;
        *blobs += num_blobs;
        *buffers += num;
...
public:
    explicit TwoQBufferCacheShard(CephContext* cct)
        : BufferCacheShard(cct)
    {}

    void _add(BlueStore::Buffer* b, int level, BlueStore::Buffer* near) override
    {
        dout(20) << __func__ << " level " << level << " near " << near << " on " << *b << " which has cache_private "
                 << b->cache_private << dendl;
        if (near) {
            b->cache_private = near->cache_private;
            switch (b->cache_private) {
            case BUFFER_WARM_IN: warm_in.insert(warm_in.iterator_to(*near), *b); break;
            ...
            default: ceph_abort_msg("bad cache_private");
            }
        }
    ...
    void _rm(BlueStore::Buffer* b) override
    {
        ...
        switch (b->cache_private) {
        case BUFFER_WARM_IN: warm_in.erase(warm_in.iterator_to(*b)); break;
        ...
        if (buffer_bytes > max) {
            ...
            if (evicted > 0) {
                dout(20) << __func__ << " evicted " << byte_u_t(evicted)
                         << " from warm_in list, done evicting warm_in buffers" << dendl;
            }
            ...
            if (evicted > 0) {
                dout(20) << __func__ << " evicted " << byte_u_t(evicted) << " from hot list, done evicting hot buffers"
                         << dendl;
            }
    ...
    void _audit(const char* s) override
    {
        ...
        if (warm_in_bytes != list_bytes[BUFFER_WARM_IN]) {
            derr << __func__ << " warm_in_list_bytes " << list_bytes[BUFFER_WARM_IN] << " != actual " << warm_in_bytes
                 << dendl;
...
BlueStore::BufferCacheShard* BlueStore::BufferCacheShard::create(CephContext* cct, string type, PerfCounters* logger)
{
...
int BlueStore::BufferSpace::_discard(BufferCacheShard* cache, uint32_t offset, uint32_t length)
{
    ...
    while (i != buffer_map.end()) {
        ...
        if (b->offset < offset) {
            ...
            if (b->end() > end) {
                ...
                if (b->data.length()) {
                    ...
                    _add_buffer(cache, new Buffer(this, b->state, b->seq, end, tail, b->flags), 0, b);
                }
                if (!b->is_writing()) {
        ...
        if (b->data.length()) {
            ...
            _add_buffer(cache, new Buffer(this, b->state, b->seq, end, keep, b->flags), 0, b);
        }
        _rm_buffer(cache, i);
...
void BlueStore::BufferSpace::read(BufferCacheShard* cache, uint32_t offset, uint32_t length,
                                  BlueStore::ready_regions_t& res, interval_set<uint32_t>& res_intervals, int flags)
{
    res.clear();
    res_intervals.clear();
...

    {
        ...
        for (auto i = _data_lower_bound(offset); i != buffer_map.end() && offset < end && i->first < end; ++i) {
            Buffer* b = i->second.get();
    ...
    while (true) {
        ...
        if (p->second->offset < pos) {
            ...
            if (p->second->data.length()) {
                ...
                r._add_buffer(cache,
                              new Buffer(&r, p->second->state, p->second->seq, 0, bl, p->second->flags),
                              0,
                              p->second.get());
            }
            else {
                r._add_buffer(cache,
                              new Buffer(&r, p->second->state, p->second->seq, 0, right, p->second->flags),
                              0,
                              p->second.get());
            }
        ...
        if (p->second->data.length()) {
            r._add_buffer(
                cache,
                new Buffer(
                    &r, p->second->state, p->second->seq, p->second->offset - pos, p->second->data, p->second->flags),
                0,
                p->second.get());
        }
        else {
            r._add_buffer(
                cache,
                new Buffer(
                    &r, p->second->state, p->second->seq, p->second->offset - pos, p->second->length, p->second->flags),
                0,
                p->second.get());
        }
...
BlueStore::OnodeRef BlueStore::OnodeSpace::add_onode(const ghobject_t& oid, OnodeRef& o)
{
    std::lock_guard l(cache->lock);
    // add entry or return existing one
    auto p = onode_map.emplace(oid, o);
    if (!p.second) {
        ldout(cache->cct, 30) << __func__ << " " << oid << " " << o << " raced, returning existing " << p.first->second
                              << dendl;
...
BlueStore::OnodeRef BlueStore::OnodeSpace::lookup(const ghobject_t& oid)
{
...

    {
        ...
        if (p == onode_map.end()) {
            ...
            ldout(cache->cct, 30) << __func__ << " " << oid << " hit " << p->second << " " << p->second->nref << " "
                                  << p->second->cached << dendl;
...
void BlueStore::OnodeSpace::rename(OnodeRef& oldo, const ghobject_t& old_oid, const ghobject_t& new_oid,
...
template<int LogLevelV = 30> void BlueStore::OnodeSpace::dump(CephContext* cct)
{
    for (auto& i : onode_map) {
        ldout(cct, LogLevelV) << i.first << " : " << i.second << " " << i.second->nref << " " << i.second->cached
                              << dendl;
    }
}
...
void BlueStore::SharedBlob::put_ref(uint64_t offset, uint32_t length, PExtentVector* r, bool* unshare)
{
    ceph_assert(persistent);
    persistent->ref_map.put(offset, length, r, unshare && !*unshare ? unshare : nullptr);
}
...
void BlueStore::SharedBlob::finish_write(uint64_t seq)
{
    while (true) {
        ...
        std::lock_guard l(cache->lock);
        if (coll->cache != cache) {
            dout(20) << __func__ << " raced with sb cache update, was " << cache << ", now " << coll->cache
                     << ", retrying" << dendl;
            continue;
        }
...
template<int LogLevelV = 30> void BlueStore::SharedBlobSet::dump(CephContext* cct)
{
    std::lock_guard l(lock);
    for (auto& i : sb_map) {
...
void BlueStore::Blob::discard_unallocated(Collection* coll)
{
    ...
    if (get_blob().is_compressed()) {
        ...
        if (discard) {
            shared_blob->bc.discard(shared_blob->get_cache(), 0, get_blob().get_logical_length());
        }
    }
    else {
        size_t pos = 0;
...
void BlueStore::Blob::get_ref(Collection* coll, uint32_t offset, uint32_t length)
{
    ...
    if (used_in_blob.is_empty()) {
        uint32_t min_release_size = get_blob().get_release_size(coll->store->min_alloc_size);
...
bool BlueStore::Blob::put_ref(Collection* coll, uint32_t offset, uint32_t length, PExtentVector* r)
{
...
bool BlueStore::Blob::can_reuse_blob(uint32_t min_alloc_size, uint32_t target_blob_size, uint32_t b_offset,
                                     uint32_t* length0)
{
    ...
    if (get_blob().has_csum() &&
        ((b_offset % get_blob().get_csum_chunk_size()) != 0 || (end % get_blob().get_csum_chunk_size()) != 0)) {
        return false;
    }
    ...
    if (new_blen > blen) {
        ...
        if (new_blen > blen) {
            ...
            used_in_blob.add_tail(new_blen, get_blob().get_release_size(min_alloc_size));
        }
    }
    return true;
}
...
void BlueStore::Blob::decode(bufferptr::const_iterator& p, uint64_t struct_v, uint64_t* sbid, bool include_ref_map,
                             Collection* coll)
{
...
{
    return out << std::hex << "0x" << e.logical_offset << "~" << e.length << ": 0x" << e.blob_offset << "~" << e.length
               << std::dec << " " << *e.blob;
}

// OldExtent
BlueStore::OldExtent* BlueStore::OldExtent::create(CollectionRef c, uint32_t lo, uint32_t o, uint32_t l, BlobRef& b)
{
...
void BlueStore::ExtentMap::dup(BlueStore* b, TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo,
                               uint64_t& srcoff, uint64_t& length, uint64_t& dstoff)
{
    ...
    for (auto ep = oldo->extent_map.seek_lextent(srcoff); ep != oldo->extent_map.extent_map.end(); ++ep) {
        auto& e = *ep;
        ...
        if (e.blob->last_encoded_id >= 0) {
            ...
            if (!blob.is_shared()) {
                ...
                else if (inject_21040 && dirty_range_begin == 0 && dirty_range_end == 0) {
                    dirty_range_begin = e.logical_offset;
                }
    ...
    if ((!inject_21040 && src_dirty) || (inject_21040 && dirty_range_end > dirty_range_begin)) {
        oldo->extent_map.dirty_range(dirty_range_begin, dirty_range_end - dirty_range_begin);
    ...
    if (onode->onode.extent_map_shards.empty()) {
        ...
        while (p != shards.end()) {
            ...
            if (p->dirty) {
                ...
                if (encode_some(p->shard_info->offset, endoff - p->shard_info->offset, bl, &p->extents)) {
                    if (force) {
                ...
                dout(20) << __func__ << "  shard 0x" << std::hex << p->shard_info->offset << std::dec << " is " << len
                         << " bytes (was " << p->shard_info->bytes << ") from " << p->extents << " extents" << dendl;
                ...
                if (!force) {
                    ...
                    else if (n != shards.end() && len < g_conf()->bluestore_extent_map_shard_min_size) {
        ...
        for (auto& it : encoded_shards) {
            dout(20) << __func__ << "  encoding key for shard 0x" << std::hex << it.shard->shard_info->offset
                     << std::dec << dendl;
            it.shard->dirty = false;
            it.shard->shard_info->bytes = it.bl.length();
            generate_extent_shard_key_and_apply(
                onode->key, it.shard->shard_info->offset, &key, [&](const string& final_key) {
...
void BlueStore::ExtentMap::reshard(KeyValueDB* db, KeyValueDB::Transaction t)
{
    ...
    dout(10) << __func__ << " 0x[" << std::hex << needs_reshard_begin << "," << needs_reshard_end << ")" << std::dec
             << " of " << onode->onode.extent_map_shards.size() << " shards on " << onode->oid << dendl;
    ...
    if (!shards.empty()) {
        while (si_begin + 1 < shards.size() && shards[si_begin + 1].shard_info->offset <= needs_reshard_begin) {
            ++si_begin;
        }
        ...
        dout(20) << __func__ << "   shards [" << si_begin << "," << si_end << ")"
                 << " over 0x[" << std::hex << needs_reshard_begin << "," << needs_reshard_end << ")" << std::dec
                 << dendl;
    }
    ...
    for (unsigned i = si_begin; i < si_end; ++i) {
        generate_extent_shard_key_and_apply(
            onode->key, shards[i].shard_info->offset, &key, [&](const string& final_key) {
    ...
    if (new_shard_info.empty() && (si_begin > 0 || si_end < shards.size())) {
        ...
        dout(20) << __func__ << "  new shard 0x" << std::hex << needs_reshard_begin << std::dec
    ...
    if (sv.empty()) {
        ...
        sv.insert(sv.begin() + si_begin, new_shard_info.begin(), new_shard_info.end());
    ...
    if (sv.empty()) {
        ...
        dout(20) << __func__ << " checking spanning blobs 0x[" << std::hex << spanning_scan_begin << ","
                 << spanning_scan_end << ")" << dendl;
        if (spanning_scan_begin < needs_reshard_begin) {
            fault_range(db, spanning_scan_begin, needs_reshard_begin - spanning_scan_begin);
        }
        ...
        auto too_many_blobs_threshold = g_conf()->bluestore_debug_too_many_blobs_threshold;
        ...
        for (auto e = extent_map.lower_bound(dummy); e != extent_map.end(); ++e) {
            ...
            while (e->logical_offset >= shard_end) {
                ...
                dout(30) << __func__ << "  shard 0x" << std::hex << shard_start << " to 0x" << shard_end << std::dec
                         << dendl;
            }
                        ...
                        for (const auto& sh : shards) {
                            if (bstart < sh.shard_info->offset && bend > sh.shard_info->offset) {
                                ...
                                if (b->can_split_at(blob_offset)) {
                                    dout(20) << __func__ << "    splitting blob, bstart 0x" << std::hex << bstart
                                             << " blob_offset 0x" << blob_offset << std::dec << " " << *b << dendl;
                    ...
                    if (must_span) {
                        ...
                        if (!was_too_many_blobs_check && too_many_blobs_threshold &&
                            ...
                            for (size_t i = 0; i < dumped_onodes.size(); ++i) {
                                ...
                                if (!oldest_slot || (oldest_slot && dumped_onodes[i].second < oldest_slot->second)) {
        ...
        bool do_dump = (!oid_slot && was_too_many_blobs_check) ||
                       (oid_slot && (mono_clock::now() - oid_slot->second >= make_timespan(5 * 60)));
        if (do_dump) {
            dout(0) << __func__ << " spanning blob count exceeds threshold, " << spanning_blob_map.size()
...
bool BlueStore::ExtentMap::encode_some(uint32_t offset, uint32_t length, bufferlist& bl, unsigned* pn)
{
    Extent dummy(offset);
    ...
    for (auto p = start; p != extent_map.end() && p->logical_offset < end; ++p, ++n) {
        ceph_assert(p->logical_offset >= offset);
        p->blob->last_encoded_id = -1;
        if (!p->blob->is_spanning() && p->blob_escapes_range(offset, length)) {
            dout(30) << __func__ << " 0x" << std::hex << offset << "~" << length << std::dec
                     << " hit new spanning blob " << *p << dendl;
        ...
        if (!must_reshard) {
            ...
            p->blob->bound_encode(bound, struct_v, p->blob->shared_blob->get_sbid(), false);
        }
    }
    if (must_reshard) {
        return true;
...
void BlueStore::ExtentMap::ExtentDecoder::decode_extent(Extent* le, __u8 struct_v, bptr_c_it_t& p, Collection* c)
{
    uint64_t blobid;
...
unsigned BlueStore::ExtentMap::ExtentDecoder::decode_some(const bufferlist& bl, Collection* c)
{
    __u8 struct_v;
    uint32_t num;
...
void BlueStore::ExtentMap::ExtentDecoder::decode_spanning_blobs(bptr_c_it_t& p, Collection* c)
{
    __u8 struct_v;
    denc(struct_v, p);
    // Version 2 differs from v1 in blob's ref_map
...
void BlueStore::ExtentMap::ExtentDecoderFull::consume_blobid(BlueStore::Extent* le, bool spanning, uint64_t blobid)
{
    ceph_assert(le);
    if (spanning) {
...
void BlueStore::ExtentMap::ExtentDecoderFull::consume_blob(BlueStore::Extent* le, uint64_t extent_no, uint64_t sbid,
                                                           BlobRef b)
{
    ceph_assert(le);
...
void BlueStore::ExtentMap::ExtentDecoderFull::consume_spanning_blob(uint64_t sbid, BlueStore::BlobRef b)
{
...
void BlueStore::ExtentMap::encode_spanning_blobs(bufferlist::contiguous_appender& p)
{
    // Version 2 differs from v1 in blob's ref_map
    // serialization only. Hence there is no specific
    // handling at ExtentMap level.
...
void BlueStore::ExtentMap::fault_range(KeyValueDB* db, uint32_t offset, uint32_t length)
{
    ...
    while (start <= last) {
        ...
        if (!p->loaded) {
            ...
            bufferlist v;
            generate_extent_shard_key_and_apply(onode->key, p->shard_info->offset, &key, [&](const string& final_key) {
                int r = db->get(PREFIX_OBJ, final_key, &v);
                if (r < 0) {
                    derr << __func__ << " missing shard 0x" << std::hex << p->shard_info->offset << std::dec << " for "
                         << onode->oid << dendl;
                    ceph_assert(r >= 0);
                }
            });
            ...
            p->loaded = true;
            dout(20) << __func__ << " open shard 0x" << std::hex << p->shard_info->offset << " for range 0x" << offset
                     << "~" << length << std::dec << " (" << v.length() << " bytes)" << dendl;
...
void BlueStore::ExtentMap::dirty_range(uint32_t offset, uint32_t length)
{
    ...
    while (start <= last) {
        ...
        auto p = &shards[start];
        if (!p->loaded) {
            derr << __func__ << "on write 0x" << std::hex << offset << "~" << length << " shard 0x"
                 << p->shard_info->offset << std::dec << " is not loaded, can't mark dirty" << dendl;
            ceph_abort_msg("can't mark unloaded shard dirty");
        }
        if (!p->dirty) {
            dout(20) << __func__ << " mark shard 0x" << std::hex << p->shard_info->offset << std::dec << " dirty"
                     << dendl;
            p->dirty = true;
        }
        ++start;
...
BlueStore::extent_map_t::iterator BlueStore::ExtentMap::seek_lextent(uint64_t offset)
{
    Extent dummy(offset);
...
BlueStore::extent_map_t::const_iterator BlueStore::ExtentMap::seek_lextent(uint64_t offset) const
{
    Extent dummy(offset);
    auto fp = extent_map.lower_bound(dummy);
    if (fp != extent_map.begin()) {
...
int BlueStore::ExtentMap::compress_extent_map(uint64_t offset, uint64_t length)
{
    ...
    while (pshard != shards.end() && p->logical_offset >= pshard->shard_info->offset) {
        ++pshard;
    }
    uint64_t shard_end;
    ...
    for (++n; n != extent_map.end(); p = n++) {
        ...
        while (n != extent_map.end() && p->logical_end() == n->logical_offset && p->blob == n->blob &&
               p->blob_offset + p->length == n->blob_offset && n->logical_offset < shard_end) {
            dout(20) << __func__ << " 0x" << std::hex << offset << "~" << length << " next shard 0x" << shard_end
                     << std::dec << " merging " << *p << " and " << *n << dendl;
            p->length += n->length;
            rm(n++);
            ++removed;
        }
...
void BlueStore::ExtentMap::punch_hole(CollectionRef& c, uint64_t offset, uint64_t length, old_extent_map_t* old_extents)
{
    auto p = seek_lextent(offset);
    ...
    while (p != extent_map.end()) {
        ...
        if (p->logical_offset < offset) {
            if (p->logical_end() > end) {
                ...
                OldExtent* oe = OldExtent::create(c, offset, p->blob_offset + front, length, p->blob);
                old_extents->push_back(*oe);
                add(end, p->blob_offset + front + length, p->length - front - length, p->blob);
                p->length = front;
                break;
            }
            else {
                ...
                OldExtent* oe = OldExtent::create(c, offset, p->blob_offset + keep, p->length - keep, p->blob);
                old_extents->push_back(*oe);
                p->length = keep;
                ++p;
                continue;
        ...
        if (p->logical_offset + p->length <= end) {
            // deref whole lextent
            OldExtent* oe = OldExtent::create(c, p->logical_offset, p->blob_offset, p->length, p->blob);
            old_extents->push_back(*oe);
            rm(p++);
            continue;
        }
        ...
        OldExtent* oe = OldExtent::create(c, p->logical_offset, p->blob_offset, p->length - keep, p->blob);
        old_extents->push_back(*oe);
...
BlueStore::Extent* BlueStore::ExtentMap::set_lextent(CollectionRef& c, uint64_t logical_offset, uint64_t blob_offset,
...
BlueStore::BlobRef BlueStore::ExtentMap::split_blob(BlobRef lb, uint32_t blob_offset, uint32_t pos)
{
    uint32_t end_pos = pos + lb->get_blob().get_logical_length() - blob_offset;
    dout(20) << __func__ << " 0x" << std::hex << pos << " end 0x" << end_pos << " blob_offset 0x" << blob_offset
             << std::dec << " " << *lb << dendl;
    BlobRef rb = onode->c->new_blob();
    lb->split(onode->c, blob_offset, rb.get());

    for (auto ep = seek_lextent(pos); ep != extent_map.end() && ep->logical_offset < end_pos; ++ep) {
        if (ep->blob != lb) {
            continue;
        }
...
// '-' < '.' < '~'
void BlueStore::Onode::calc_omap_header(uint8_t flags, const Onode* o, std::string* out)
{
    if (!bluestore_onode_t::is_pgmeta_omap(flags)) {
        if (bluestore_onode_t::is_perpg_omap(flags)) {
...
void BlueStore::Onode::calc_omap_key(uint8_t flags, const Onode* o, const std::string& key, std::string* out)
{
    if (!bluestore_onode_t::is_pgmeta_omap(flags)) {
        if (bluestore_onode_t::is_perpg_omap(flags)) {
            _key_encode_u64(o->c->pool(), out);
...
void BlueStore::Onode::calc_omap_tail(uint8_t flags, const Onode* o, std::string* out)
{
    if (!bluestore_onode_t::is_pgmeta_omap(flags)) {
        if (bluestore_onode_t::is_perpg_omap(flags)) {
            _key_encode_u64(o->c->pool(), out);
...
void BlueStore::Onode::decode_raw(BlueStore::Onode* on, const bufferlist& v,
...
BlueStore::Onode* BlueStore::Onode::create_decode(CollectionRef c, const ghobject_t& oid, const string& key,
                                                  const bufferlist& v, bool allow_empty)
{
    ...
    if (v.length()) {
        ...
        // initialize extent_map
        if (on->onode.extent_map_shards.empty()) {
            on->extent_map.inline_bl.reassign_to_mempool(mempool::mempool_bluestore_cache_data);
        }
        else {
...
bool BlueStore::WriteContext::has_conflict(BlobRef b, uint64_t loffs, uint64_t loffs_end, uint64_t min_alloc_size)
{
    ...
    for (auto w : writes) {
        if (b == w.b) {
            ...
            if ((loffs <= loffs2 && loffs_end > loffs2) || (loffs >= loffs2 && loffs < loffs2_end)) {
                return true;
            }
        }
    }
...
void BlueStore::DeferredBatch::prepare_write(CephContext* cct, uint64_t seq, uint64_t offset, uint64_t length,
                                             bufferlist::const_iterator& blp)
{
    ...
    i.first->second.bl.reassign_to_mempool(mempool::mempool_bluestore_writing_deferred);
    dout(20) << __func__ << " seq " << seq << " 0x" << std::hex << offset << "~" << length << " crc "
             << i.first->second.bl.crc32c(-1) << std::dec << dendl;
...
void BlueStore::DeferredBatch::_discard(CephContext* cct, uint64_t offset, uint64_t length)
{
    ...
    if (p != iomap.begin()) {
        ...
        if (end > offset) {
            ...
            dout(20) << __func__ << "  keep head " << p->second.seq << " 0x" << std::hex << p->first << "~"
                     << p->second.bl.length() << " -> 0x" << head.length() << std::dec << dendl;
            auto i = seq_bytes.find(p->second.seq);
            ceph_assert(i != seq_bytes.end());
            if (end > offset + length) {
                bufferlist tail;
                tail.substr_of(p->second.bl, offset + length - p->first, end - (offset + length));
                dout(20) << __func__ << "  keep tail " << p->second.seq << " 0x" << std::hex << p->first << "~"
                         << p->second.bl.length() << " -> 0x" << tail.length() << std::dec << dendl;
    ...
    while (p != iomap.end()) {
        ...
        if (end > offset + length) {
            ...
            dout(20) << __func__ << "  truncate front " << p->second.seq << " 0x" << std::hex << p->first << "~"
                     << p->second.bl.length() << " drop_front 0x" << drop_front << " keep_tail 0x" << keep_tail
                     << " to 0x" << (offset + length) << "~" << keep_tail << std::dec << dendl;
            ...
            dout(20) << __func__ << "  drop " << p->second.seq << " 0x" << std::hex << p->first << "~"
                     << p->second.bl.length() << std::dec << dendl;
...
void BlueStore::Collection::open_shared_blob(uint64_t sbid, BlobRef b)
{
    ...
    if (b->shared_blob) {
        ldout(store->cct, 10) << __func__ << " sbid 0x" << std::hex << sbid << std::dec << " had " << *b->shared_blob
                              << dendl;
    }
    else {
        b->shared_blob = new SharedBlob(sbid, this);
        shared_blob_set.add(this, b->shared_blob.get());
        ldout(store->cct, 10) << __func__ << " sbid 0x" << std::hex << sbid << std::dec << " opened " << *b->shared_blob
                              << dendl;
    }
}
...
void BlueStore::Collection::load_shared_blob(SharedBlobRef sb)
{
    if (!sb->is_loaded()) {
        ...
        if (r < 0) {
            lderr(store->cct) << __func__ << " sbid 0x" << std::hex << sbid << std::dec << " not found at key "
        ...
        ldout(store->cct, 10) << __func__ << " sbid 0x" << std::hex << sbid << std::dec << " loaded shared_blob " << *sb
                              << dendl;
    }
}
...
BlueStore::OnodeRef BlueStore::Collection::get_onode(const ghobject_t& oid, bool create, bool is_createop)
{
    ...
    if (cid.is_pg(&pgid)) {
        if (!oid.match(cnode.bits, pgid.ps())) {
            lderr(store->cct) << __func__ << " oid " << oid << " not part of " << pgid << " bits " << cnode.bits
                              << dendl;
            ceph_abort();
        }
    }
...
void* BlueStore::MempoolThread::entry()
{
    ...
    if (store->cache_autotune && binned_kv_cache != nullptr) {
        pcm = std::make_shared<PriorityCache::Manager>(store->cct, min, max, target, true, "bluestore-pricache");
    ...
    while (!stop) {
        ...
        // alloc stats dump
        if (alloc_stats_dump_interval > 0 && alloc_stats_dump_clock + alloc_stats_dump_interval < ceph_clock_now()) {
...
void BlueStore::MempoolThread::_resize_shards(bool interval_stats)
{
    ...
    int64_t kv_onode_alloc = static_cast<int64_t>(store->cache_kv_onode_ratio * cache_size);
    int64_t meta_alloc = static_cast<int64_t>(store->cache_meta_ratio * cache_size);
    int64_t data_alloc = static_cast<int64_t>(store->cache_data_ratio * cache_size);
    ...
    if (interval_stats) {
        dout(5) << __func__ << " cache_size: " << cache_size << " kv_alloc: " << kv_alloc << " kv_used: " << kv_used
                << " kv_onode_alloc: " << kv_onode_alloc << " kv_onode_used: " << kv_onode_used
                << " meta_alloc: " << meta_alloc << " meta_used: " << meta_used << " data_alloc: " << data_alloc
                << " data_used: " << data_used << dendl;
    }
    else {
        dout(20) << __func__ << " cache_size: " << cache_size << " kv_alloc: " << kv_alloc << " kv_used: " << kv_used
                 << " kv_onode_alloc: " << kv_onode_alloc << " kv_onode_used: " << kv_onode_used
                 << " meta_alloc: " << meta_alloc << " meta_used: " << meta_used << " data_alloc: " << data_alloc
                 << " data_used: " << data_used << dendl;
    }

    uint64_t max_shard_onodes =
        static_cast<uint64_t>((meta_alloc / (double)onode_shards) / meta_cache->get_bytes_per_onode());
    uint64_t max_shard_buffer = static_cast<uint64_t>(data_alloc / buffer_shards);

    dout(30) << __func__ << " max_shard_onodes: " << max_shard_onodes << " max_shard_buffer: " << max_shard_buffer
             << dendl;
...
BlueStore::OmapIteratorImpl::OmapIteratorImpl(PerfCounters* _logger, CollectionRef c, OnodeRef& o,
...
int BlueStore::OmapIteratorImpl::upper_bound(const string& after)
{
    ...
    c->store->log_latency_fn(__func__,
                             l_bluestore_omap_upper_bound_lat,
                             mono_clock::now() - start1,
                             c->store->cct->_conf->bluestore_log_omap_iterator_age,
                             [&](const ceph::timespan& lat) { return ", after = " + after + _stringify(); });
    return 0;
}
...
{
    std::shared_lock l(c->lock);
    bool r = o->onode.has_omap() && it && it->valid() && it->raw_key().second < tail;
    if (it && it->valid()) {
...
BlueStore::BlueStore(CephContext* cct, const string& path, uint64_t _min_alloc_size)
...
void BlueStore::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    if (changed.count("bluestore_warn_on_legacy_statfs")) {
        _check_legacy_statfs_alert();
    }
    if (changed.count("bluestore_warn_on_no_per_pool_omap") || changed.count("bluestore_warn_on_no_per_pg_omap")) {
    ...
    if (changed.count("bluestore_compression_mode") || changed.count("bluestore_compression_algorithm") ||
        changed.count("bluestore_compression_min_blob_size") || changed.count("bluestore_compression_max_blob_size")) {
        if (bdev) {
            _set_compression();
        }
    }
    if (changed.count("bluestore_max_blob_size") || changed.count("bluestore_max_blob_size_ssd") ||
    ...
    if (changed.count("bluestore_prefer_deferred_size") || changed.count("bluestore_prefer_deferred_size_hdd") ||
        changed.count("bluestore_prefer_deferred_size_ssd") || changed.count("bluestore_max_alloc_size") ||
        changed.count("bluestore_deferred_batch_ops") || changed.count("bluestore_deferred_batch_ops_hdd") ||
    ...
    if (changed.count("bluestore_throttle_cost_per_io") || changed.count("bluestore_throttle_cost_per_io_hdd") ||
        changed.count("bluestore_throttle_cost_per_io_ssd")) {
        if (bdev) {
            _set_throttle_params();
        }
    }
    if (changed.count("bluestore_throttle_bytes") || changed.count("bluestore_throttle_deferred_bytes") ||
    ...
    if (changed.count("osd_memory_target") || changed.count("osd_memory_base") ||
        changed.count("osd_memory_cache_min") || changed.count("osd_memory_expected_fragmentation")) {
...
void BlueStore::_set_compression()
{
    ...
    if (m) {
        ...
        derr << __func__ << " unrecognized value '" << cct->_conf->bluestore_compression_mode
    ...
    dout(10) << __func__ << " mode " << Compressor::get_comp_mode_name(comp_mode) << " alg "
             << (compressor ? compressor->get_type_name() : "(none)") << " min_blob " << comp_min_blob_size
             << " max_blob " << comp_max_blob_size << dendl;
}
...
void BlueStore::_update_osd_memory_options()
{
    ...
    config_changed++;
    dout(10) << __func__ << " osd_memory_target " << osd_memory_target << " osd_memory_base " << osd_memory_base
             << " osd_memory_expected_fragmentation " << osd_memory_expected_fragmentation << " osd_memory_cache_min "
...
int BlueStore::_set_cache_sizes()
{
    ...
    cache_autotune_interval = cct->_conf.get_val<double>("bluestore_cache_autotune_interval");
    cache_age_bin_interval = cct->_conf.get_val<double>("bluestore_cache_age_bin_interval");
    ...
    osd_memory_expected_fragmentation = cct->_conf.get_val<double>("osd_memory_expected_fragmentation");
    osd_memory_cache_min = cct->_conf.get_val<Option::size_t>("osd_memory_cache_min");
    osd_memory_cache_resize_interval = cct->_conf.get_val<double>("osd_memory_cache_resize_interval");
    ...
    if (cache_meta_ratio < 0 || cache_meta_ratio > 1.0) {
        derr << __func__ << " bluestore_cache_meta_ratio (" << cache_meta_ratio << ") must be in range [0,1.0]"
             << dendl;
        return -EINVAL;
    }
    ...
    if (cache_kv_onode_ratio < 0 || cache_kv_onode_ratio > 1.0) {
        derr << __func__ << " bluestore_cache_kv_onode_ratio (" << cache_kv_onode_ratio << ") must be in range [0,1.0]"
             << dendl;
        return -EINVAL;
    }

    if (cache_meta_ratio + cache_kv_ratio > 1.0) {
        derr << __func__ << " bluestore_cache_meta_ratio (" << cache_meta_ratio << ") + bluestore_cache_kv_ratio ("
             << cache_kv_ratio << ") = " << cache_meta_ratio + cache_kv_ratio << "; must be <= 1.0" << dendl;
        return -EINVAL;
    }

    cache_data_ratio = (double)1.0 - (double)cache_meta_ratio - (double)cache_kv_ratio - (double)cache_kv_onode_ratio;
    if (cache_data_ratio < 0) {
        // deal with floating point imprecision
        cache_data_ratio = 0;
    }

    dout(1) << __func__ << " cache_size " << cache_size << " meta " << cache_meta_ratio << " kv " << cache_kv_ratio
            << " data " << cache_data_ratio << dendl;
    return 0;
}
...
void BlueStore::_init_logger()
{
    ...
    b.add_time_avg(l_bluestore_state_aio_wait_lat,
                   "state_aio_wait_lat",
                   "Average aio_wait state latency",
                   "sawl",
                   PerfCountersBuilder::PRIO_INTERESTING);
    b.add_time_avg(l_bluestore_state_io_done_lat,
                   "state_io_done_lat",
                   "Average io_done state latency",
                   "sidl",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_state_kv_queued_lat,
                   "state_kv_queued_lat",
    ...
    b.add_time_avg(l_bluestore_state_kv_committing_lat,
                   ...
                   "skcl",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_state_kv_done_lat,
                   "state_kv_done_lat",
                   "Average kv_done state latency",
                   "skdl",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_state_finishing_lat,
                   "state_finishing_lat",
                   "Average finishing state latency",
                   "sfnl",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_state_done_lat,
                   "state_done_lat",
    ...
    //****************************************
    b.add_time_avg(l_bluestore_throttle_lat,
                   "txc_throttle_lat",
                   "Average submit throttle latency",
                   "th_l",
                   PerfCountersBuilder::PRIO_CRITICAL);
    b.add_time_avg(
        l_bluestore_submit_lat, "txc_submit_lat", "Average submit latency", "s_l", PerfCountersBuilder::PRIO_CRITICAL);
    b.add_time_avg(
        l_bluestore_commit_lat, "txc_commit_lat", "Average commit latency", "c_l", PerfCountersBuilder::PRIO_CRITICAL);
    b.add_u64_counter(l_bluestore_txc, "txc_count", "Transactions committed");
    //****************************************

    // Read op stats
    //****************************************
    b.add_time_avg(l_bluestore_read_onode_meta_lat,
                   "read_onode_meta_lat",
                   "Average read onode metadata latency",
                   "roml",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_read_wait_aio_lat,
                   "read_wait_aio_lat",
                   "Average read I/O waiting latency",
                   "rwal",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(
        l_bluestore_csum_lat, "csum_lat", "Average checksum latency", "csml", PerfCountersBuilder::PRIO_USEFUL);
    b.add_u64_counter(l_bluestore_read_eio, "read_eio", "Read EIO errors propagated to high level callers");
    b.add_u64_counter(l_bluestore_reads_with_retries,
                      ...
                      "rd_r",
                      PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_read_lat, "read_lat", "Average read latency", "r_l", PerfCountersBuilder::PRIO_CRITICAL);
    //****************************************

    // kv_thread latencies
    //****************************************
    b.add_time_avg(l_bluestore_kv_flush_lat,
                   "kv_flush_lat",
                   "Average kv_thread flush latency",
                   "kfsl",
                   PerfCountersBuilder::PRIO_INTERESTING);
    b.add_time_avg(l_bluestore_kv_commit_lat,
                   "kv_commit_lat",
                   "Average kv_thread commit latency",
                   "kcol",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_kv_sync_lat,
                   "kv_sync_lat",
                   "Average kv_sync thread latency",
                   "kscl",
                   PerfCountersBuilder::PRIO_INTERESTING);
    b.add_time_avg(l_bluestore_kv_final_lat,
                   "kv_final_lat",
    ...
    // write op stats
    //****************************************
    b.add_u64_counter(l_bluestore_write_big, "write_big", "Large aligned writes into fresh blobs");
    b.add_u64_counter(l_bluestore_write_big_bytes,
                      "write_big_bytes",
                      "Large aligned writes into fresh blobs (bytes)",
                      NULL,
                      PerfCountersBuilder::PRIO_DEBUGONLY,
                      unit_t(UNIT_BYTES));
    b.add_u64_counter(l_bluestore_write_big_blobs, "write_big_blobs", "Large aligned writes into fresh blobs (blobs)");
    b.add_u64_counter(l_bluestore_write_big_deferred, "write_big_deferred", "Big overwrites using deferred");

    b.add_u64_counter(l_bluestore_write_small, "write_small", "Small writes into existing or sparse small blobs");
    b.add_u64_counter(l_bluestore_write_small_bytes,
                      ...
                      unit_t(UNIT_BYTES));
    b.add_u64_counter(
        l_bluestore_write_small_unused, "write_small_unused", "Small writes into unused portion of existing blob");
    ...
    b.add_u64_counter(l_bluestore_write_pad_bytes,
                      "write_pad_bytes",
                      "Sum for write-op padded bytes",
                      NULL,
                      PerfCountersBuilder::PRIO_DEBUGONLY,
                      unit_t(UNIT_BYTES));
    b.add_u64_counter(l_bluestore_write_penalty_read_ops, "write_penalty_read_ops", "Sum for write penalty read ops");
    b.add_u64_counter(l_bluestore_write_new, "write_new", "Write into new blob");

    b.add_u64_counter(l_bluestore_issued_deferred_writes, "issued_deferred_writes", "Total deferred writes issued");
    b.add_u64_counter(l_bluestore_issued_deferred_write_bytes,
                      ...
                      unit_t(UNIT_BYTES));
    b.add_u64_counter(
        l_bluestore_submitted_deferred_writes, "submitted_deferred_writes", "Total deferred writes submitted to disk");
    ...
    // compressions stats
    //****************************************
    b.add_u64(l_bluestore_compressed,
              "compressed",
              "Sum for stored compressed bytes",
              "c",
              PerfCountersBuilder::PRIO_USEFUL,
              unit_t(UNIT_BYTES));
    b.add_u64(l_bluestore_compressed_allocated,
              "compressed_allocated",
              "Sum for bytes allocated for compressed data",
              "c_a",
              PerfCountersBuilder::PRIO_USEFUL,
              unit_t(UNIT_BYTES));
    b.add_u64(l_bluestore_compressed_original,
              "compressed_original",
              ...
              unit_t(UNIT_BYTES));
    b.add_time_avg(
        l_bluestore_compress_lat, "compress_lat", "Average compress latency", "_cpl", PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_decompress_lat,
                   "decompress_lat",
                   "Average decompress latency",
                   "dcpl",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_u64_counter(l_bluestore_compress_success_count, "compress_success_count", "Sum for beneficial compress ops");
    b.add_u64_counter(l_bluestore_compress_rejected_count,
    ...
    b.add_u64(l_bluestore_pinned_onodes, "onodes_pinned", "Number of pinned onodes in cache");
    b.add_u64_counter(l_bluestore_onode_hits,
                      "onode_hits",
                      "Count of onode cache lookup hits",
                      "o_ht",
                      PerfCountersBuilder::PRIO_USEFUL);
    b.add_u64_counter(l_bluestore_onode_misses,
                      "onode_misses",
                      "Count of onode cache lookup misses",
                      "o_ms",
                      PerfCountersBuilder::PRIO_USEFUL);
    b.add_u64_counter(l_bluestore_onode_shard_hits, "onode_shard_hits", "Count of onode shard cache lookups hits");
    b.add_u64_counter(
        l_bluestore_onode_shard_misses, "onode_shard_misses", "Count of onode shard cache lookups misses");
    ...
    b.add_u64(l_bluestore_buffer_bytes,
              "buffer_bytes",
              "Number of buffer bytes in cache",
              NULL,
              PerfCountersBuilder::PRIO_DEBUGONLY,
              unit_t(UNIT_BYTES));
    b.add_u64_counter(l_bluestore_buffer_hit_bytes,
                      "buffer_hit_bytes",
                      "Sum for bytes of read hit in the cache",
                      NULL,
                      PerfCountersBuilder::PRIO_DEBUGONLY,
                      unit_t(UNIT_BYTES));
    b.add_u64_counter(l_bluestore_buffer_miss_bytes,
                      "buffer_miss_bytes",
    ...
    // internal stats
    //****************************************
    b.add_u64_counter(l_bluestore_onode_reshard, "onode_reshard", "Onode extent map reshard events");
    b.add_u64_counter(l_bluestore_blob_split, "blob_split", "Sum for blob splitting due to resharding");
    b.add_u64_counter(
        l_bluestore_extent_compress, "extent_compress", "Sum for extents that have been removed due to compression");
    b.add_u64_counter(l_bluestore_gc_merged,
                      "gc_merged",
    ...
    // misc
    //****************************************
    b.add_u64_counter(l_bluestore_omap_iterator_count, "omap_iterator_count", "Open omap iterators count");
    b.add_u64_counter(l_bluestore_omap_rmkeys_count, "omap_rmkeys_count", "amount of omap keys removed via rmkeys");
    b.add_u64_counter(
        l_bluestore_omap_rmkey_ranges_count, "omap_rmkey_range_count", "amount of omap key ranges removed via rmkeys");
    //****************************************
    // other client ops latencies
    //****************************************
    b.add_time_avg(l_bluestore_omap_seek_to_first_lat,
                   "omap_seek_to_first_lat",
                   "Average omap iterator seek_to_first call latency",
                   "osfl",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_omap_upper_bound_lat,
                   "omap_upper_bound_lat",
                   "Average omap iterator upper_bound call latency",
                   "oubl",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_omap_lower_bound_lat,
                   "omap_lower_bound_lat",
                   "Average omap iterator lower_bound call latency",
                   "olbl",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_omap_next_lat,
                   "omap_next_lat",
                   "Average omap iterator next call latency",
                   "onxl",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_omap_get_keys_lat,
                   "omap_get_keys_lat",
                   "Average omap get_keys call latency",
                   "ogkl",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_omap_get_values_lat,
                   "omap_get_values_lat",
                   "Average omap get_values call latency",
                   "ogvl",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(l_bluestore_omap_clear_lat, "omap_clear_lat", "Average omap clear call latency");
    b.add_time_avg(l_bluestore_clist_lat,
                   "clist_lat",
                   "Average collection listing latency",
                   "cl_l",
                   PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(
        l_bluestore_remove_lat, "remove_lat", "Average removal latency", "rm_l", PerfCountersBuilder::PRIO_USEFUL);
    b.add_time_avg(
        l_bluestore_truncate_lat, "truncate_lat", "Average truncate latency", "tr_l", PerfCountersBuilder::PRIO_USEFUL);
    //****************************************
    ...
    b.add_u64_counter_histogram(l_bluestore_allocate_hist,
                                "allocate_histogram",
                                alloc_hist_x_axis_config,
...
int BlueStore::get_block_device_fsid(CephContext* cct, const string& path, uuid_d* fsid)
{
    bluestore_bdev_label_t label;
...
int BlueStore::_write_bdev_label(CephContext* cct, const string& path, bluestore_bdev_label_t label)
{
...
int BlueStore::_read_bdev_label(CephContext* cct, const string& path, bluestore_bdev_label_t* label)
{
    dout(10) << __func__ << dendl;
    ...
    try {
        ...
        derr << __func__ << " unable to decode label at offset " << p.get_off() << ": " << e.what() << dendl;
        return -ENOENT;
    }
    if (crc != expected_crc) {
...
int BlueStore::_check_or_set_bdev_label(string path, uint64_t size, string desc, bool create)
{
    bluestore_bdev_label_t label;
    if (create) {
        label.osd_uuid = fsid;
        ...
        if (cct->_conf->bluestore_debug_permit_any_bdev_label) {
            dout(20) << __func__ << " bdev " << path << " fsid " << label.osd_uuid << " and fsid " << fsid
                     << " check bypassed" << dendl;
        }
        else if (label.osd_uuid != fsid) {
            derr << __func__ << " bdev " << path << " fsid " << label.osd_uuid << " does not match our fsid " << fsid
                 << dendl;
            return -EIO;
        }
    }
...
void BlueStore::_set_alloc_sizes(void)
{
    ...
    dout(10) << __func__ << " min_alloc_size 0x" << std::hex << min_alloc_size << std::dec << " order "
             << (int)min_alloc_size_order << " max_alloc_size 0x" << std::hex << max_alloc_size
             << " prefer_deferred_size 0x" << prefer_deferred_size << std::dec << " deferred_batch_ops "
             << deferred_batch_ops << dendl;
}
...
int BlueStore::_open_fm(KeyValueDB::Transaction t, bool read_only, bool db_avail, bool fm_restore)
{
    int r;
    ...
    bool can_have_null_fm =
        !is_db_rotational() && !read_only && db_avail && cct->_conf->bluestore_allocation_from_file && !bdev->is_smr();
    ...
    if (t) {
        ...
        if (bdev->is_smr() && freelist_type != "zoned") {
            derr << "SMR device but freelist_type = " << freelist_type << " (not zoned)" << dendl;
            return -EINVAL;
        }
        if (!bdev->is_smr() && freelist_type == "zoned") {
            derr << "non-SMR device (or SMR support not built-in) but freelist_type = zoned" << dendl;
            return -EINVAL;
        }

        fm->create(bdev->get_size(), alloc_size, zone_size, first_sequential_zone, t);
        ...
        if (cct->_conf->bluestore_debug_prefill > 0) {
            ...
            dout(1) << __func__ << " pre-fragmenting freespace, using " << cct->_conf->bluestore_debug_prefill
                    << " with max free extent " << cct->_conf->bluestore_debug_prefragment_max << dendl;
            ...
            while (!stop && start < end) {
                ...
                dout(20) << __func__ << " free 0x" << std::hex << start << "~" << l << " use 0x" << u << std::dec
                         << dendl;
        ...
        r = fm->init(
            db, read_only, [&](const std::string& key, std::string* result) { return read_meta(key, result); });
        if (r < 0) {
...
int BlueStore::_create_alloc()
{
    ...
    alloc =
        Allocator::create(cct, allocator_type, bdev->get_size(), alloc_size, zone_size, first_sequential_zone, "block");
    if (!alloc) {
        lderr(cct) << __func__ << " failed to create " << allocator_type << " allocator" << dendl;
        return -EINVAL;
    }

#ifdef HAVE_LIBZBD
    if (freelist_type == "zoned") {
        Allocator* a = Allocator::create(cct,
                                         ...
                                         "zoned_block");
        if (!a) {
            lderr(cct) << __func__ << " failed to create " << cct->_conf->bluestore_allocator << " allocator" << dendl;
            delete alloc;
            return -EINVAL;
        }
...
int BlueStore::_init_alloc(std::map<uint64_t, uint64_t>* zone_adjustments)
{
    ...
    if (bdev->is_smr()) {
        ...
        for (unsigned i = first_sequential_zone; i < num_zones; ++i) {
            ...
            if (zones[i].write_pointer > p) {
                derr << __func__ << " zone 0x" << std::hex << i << " bluestore write pointer 0x"
                     << zones[i].write_pointer << " > device write pointer 0x" << p << std::dec
                ...
                dout(1) << __func__ << " zone 0x" << std::hex << i << " device write pointer 0x" << p
                        << " > bluestore pointer 0x" << zones[i].write_pointer << ", advancing 0x" << delta << std::dec
                        << dendl;
        ...
        dout(1) << __func__ << " loaded zone pointers: " << std::hex << ", allocator type " << alloc->get_type()
                << ", capacity 0x" << alloc->get_capacity() << ", block size 0x" << alloc->get_block_size()
                << ", free 0x" << alloc->get_free() << ", fragmentation " << alloc->get_fragmentation() << std::dec
                << dendl;
    ...
    if (!fm->is_null_manager()) {
        ...
        dout(5) << __func__ << "::num_entries=" << num << " free_size=" << bytes
                << " alloc_size=" << alloc->get_capacity() - bytes << " time=" << duration << " seconds" << dendl;
    }
    else {
        ...
        if (!cct->_conf->bluestore_allocation_from_file) {
            ...
            derr << __func__
                 << "::NCB::Please change the value of bluestore_allocation_from_file to TRUE in your ceph.conf file"
                 << dendl;
        ...
        if (restore_allocator(alloc, &num, &bytes) == 0) {
            ...
            dout(0) << __func__
                    << "::NCB::restore_allocator() failed! Run Full Recovery from ONodes (might take a while) ..."
                    << dendl;
            ...
            if (read_allocation_from_drive_on_startup() != 0) {
                ...
                derr << __func__ << "::NCB::Ceph-OSD won't start, make sure your drives are connected and readable"
                     << dendl;
                derr << __func__ << "::NCB::If no HW fault is found, please report failure and consider redeploying OSD"
                     << dendl;
                return -ENOTRECOVERABLE;
            }
        }
    }
    dout(1) << __func__ << " loaded " << byte_u_t(bytes) << " in " << num << " extents" << std::hex
            << ", allocator type " << alloc->get_type() << ", capacity 0x" << alloc->get_capacity() << ", block size 0x"
            << alloc->get_block_size() << ", free 0x" << alloc->get_free() << ", fragmentation "
            << alloc->get_fragmentation() << std::dec << dendl;
...
int BlueStore::_lock_fsid()
{
    ...
    if (r < 0) {
        ...
        derr << __func__ << " failed to lock " << path << "/fsid"
             << " (is another ceph-osd still running?)" << cpp_strerror(err) << dendl;
        return -err;
    }
    return 0;
}
...
int BlueStore::_minimal_open_bluefs(bool create)
{
    ...
    if (::stat(bfn.c_str(), &st) == 0) {
        r = bluefs->add_block_device(BlueFS::BDEV_DB, bfn, create && cct->_conf->bdev_enable_discard, SUPER_RESERVED);
        if (r < 0) {
        ...
        if (bluefs->bdev_support_label(BlueFS::BDEV_DB)) {
            r = _check_or_set_bdev_label(bfn, bluefs->get_block_device_size(BlueFS::BDEV_DB), "bluefs db", create);
            if (r < 0) {
    ...
    bfn = path + "/block";
    // never trim here
    r = bluefs->add_block_device(bluefs_layout.shared_bdev,
                                 bfn,
                                 false,
    ...
    if (::stat(bfn.c_str(), &st) == 0) {
        r = bluefs->add_block_device(
            BlueFS::BDEV_WAL, bfn, create && cct->_conf->bdev_enable_discard, BDEV_LABEL_BLOCK_SIZE);
        if (r < 0) {
            derr << __func__ << " add block device(" << bfn << ") returned: " << cpp_strerror(r) << dendl;
            goto free_bluefs;
        }
        ...
        if (bluefs->bdev_support_label(BlueFS::BDEV_WAL)) {
            r = _check_or_set_bdev_label(bfn, bluefs->get_block_device_size(BlueFS::BDEV_WAL), "bluefs wal", create);
            if (r < 0) {
                derr << __func__ << " check block device(" << bfn << ") label returned: " << cpp_strerror(r) << dendl;
                goto free_bluefs;
            }
...
int BlueStore::_open_bluefs(bool create, bool read_only)
{
    ...
    if (bluefs_layout.shared_bdev == BlueFS::BDEV_SLOW ||
        ...
        r = RocksDBStore::ParseOptionsFromStringStatic(cct, options, rocks_opts, nullptr);
        if (r < 0) {
            return r;
        }
        if (cct->_conf->bluestore_volume_selection_policy == "fit_to_fast") {
            vselector = new FitToFastVolumeSelector(bluefs->get_block_device_size(BlueFS::BDEV_WAL) * 95 / 100,
...
int BlueStore::_prepare_db_environment(bool create, bool read_only, std::string* _fn, std::string* _kv_backend)
{
    int r;
    ceph_assert(!db);
    std::string& fn = *_fn;
    ...
    if (do_bluefs) {
        ...
        if (cct->_conf->bluestore_bluefs_env_mirror) {
            ...
            if (create) {
                string cmd = "rm -rf " + path + "/db " + path + "/db.slow " + path + "/db.wal";
...
{
    dout(10) << __func__ << ":read_only=" << db_was_opened_read_only << " fm=" << fm
             << " destage_alloc_file=" << need_to_destage_allocation_file << " per_pool=" << per_pool_stat_collection
        ...
        if (per_pool_stat_collection) {
            ...
            for (auto& p : osd_pools) {
                ...
                if (!p.second.is_empty()) {
                    ...
                    dout(10) << __func__ << " persisting: " << p.first << "->" << s << dendl;
                }
            }
        }
        else {
...
void BlueStore::_dump_alloc_on_failure()
{
    ...
    if (dump_interval > 0 && next_dump_on_bluefs_alloc_failure <= ceph_clock_now()) {
        shared_alloc.a->dump();
...
void BlueStore::_open_statfs()
{
    ...
    if (r >= 0) {
        ...
        for (it->upper_bound(string()); it->valid(); it->next()) {
            ...
            try {
                ...
                dout(10) << __func__ << " pool " << std::hex << pool_id << " statfs(hex) " << st << std::dec << dendl;
            }
            catch (ceph::buffer::error& e) {
    ...
    dout(10) << __func__ << " statfs " << std::hex << vstatfs << std::dec << dendl;
}

int BlueStore::_setup_block_symlink_or_file(string name, string epath, uint64_t size, bool create)
{
    dout(20) << __func__ << " name " << name << " path " << epath << " size " << size << " create=" << (int)create
             << dendl;
    int r = 0;
    int flags = O_RDWR | O_CLOEXEC;
    if (create) flags |= O_CREAT;
    if (epath.length()) {
        ...
        if (r < 0) {
            r = -errno;
            derr << __func__ << " failed to create " << name << " symlink to " << epath << ": " << cpp_strerror(r)
                 << dendl;
            return r;
        }
    ...
    if (size) {
        ...
        if (fd >= 0) {
            ...
            if (r == 0 && S_ISREG(st.st_mode) &&   // if it is a regular file
                ...
                if (r < 0) {
                    r = -errno;
                    derr << __func__ << " failed to resize " << name << " file to " << size << ": " << cpp_strerror(r)
                         << dendl;
                    VOID_TEMP_FAILURE_RETRY(::close(fd));
                    return r;
                }
                ...
                if (cct->_conf->bluestore_block_preallocate_file) {
                    ...
                    if (r > 0) {
                        derr << __func__ << " failed to prefallocate " << name << " file to " << size << ": "
...
int BlueStore::mkfs()
{
    ...
    if (cct->_conf->osd_max_object_size > OBJECT_MAX_SIZE) {
        derr << __func__ << " osd_max_object_size " << cct->_conf->osd_max_object_size << " > bluestore max "
    ...
    dout(5) << __func__ << " optimal_io_size 0x" << std::hex << optimal_io_size << " block_size: 0x" << block_size
            << std::dec << dendl;
    if ((cct->_conf->bluestore_use_optimal_io_size_for_min_alloc_size) && (optimal_io_size != 0)) {
        dout(5) << __func__ << " optimal_io_size 0x" << std::hex << optimal_io_size << " for min_alloc_size 0x"
    ...
    if (min_alloc_size % block_size != 0) {
        derr << __func__ << " min_alloc_size 0x" << std::hex << min_alloc_size
             << " is less or not aligned with block_size: 0x" << block_size << std::dec << dendl;
        r = -EINVAL;
        goto out_close_bdev;
    }
    ...
    alloc->init_add_free(reserved, p2align(bdev->get_size(), min_alloc_size) - reserved);
#ifdef HAVE_LIBZBD
...
int BlueStore::add_new_bluefs_device(int id, const string& dev_path)
{
    ...
    if (id == BlueFS::BDEV_NEWWAL) {
        ...
        r = _setup_block_symlink_or_file("block.wal", dev_path, cct->_conf->bluestore_block_wal_size, true);
        ceph_assert(r == 0);

        r = bluefs->add_block_device(BlueFS::BDEV_NEWWAL, p, cct->_conf->bdev_enable_discard, BDEV_LABEL_BLOCK_SIZE);
        ceph_assert(r == 0);

        if (bluefs->bdev_support_label(BlueFS::BDEV_NEWWAL)) {
            r = _check_or_set_bdev_label(p, bluefs->get_block_device_size(BlueFS::BDEV_NEWWAL), "bluefs wal", true);
            ceph_assert(r == 0);
        }
        ...
        r = _setup_block_symlink_or_file("block.db", dev_path, cct->_conf->bluestore_block_db_size, true);
        ceph_assert(r == 0);

        r = bluefs->add_block_device(BlueFS::BDEV_NEWDB, p, cct->_conf->bdev_enable_discard, SUPER_RESERVED);
        ceph_assert(r == 0);

        if (bluefs->bdev_support_label(BlueFS::BDEV_NEWDB)) {
            r = _check_or_set_bdev_label(p, bluefs->get_block_device_size(BlueFS::BDEV_NEWDB), "bluefs db", true);
            ceph_assert(r == 0);
        }
        bluefs_layout.shared_bdev = BlueFS::BDEV_SLOW;
        bluefs_layout.dedicated_db = true;
...
int BlueStore::migrate_to_existing_bluefs_device(const set<int>& devs_source, int id)
{
...
int BlueStore::migrate_to_new_bluefs_device(const set<int>& devs_source, int id, const string& dev_path)
{
    dout(10) << __func__ << " path " << dev_path << " id:" << id << dendl;
    ceph_assert(path_fd < 0);
    ...
    string link_db;
    string link_wal;
    if (devs_source.count(BlueFS::BDEV_DB) && bluefs_layout.shared_bdev != BlueFS::BDEV_DB) {
    ...
    if (id == BlueFS::BDEV_NEWWAL) {
        ...
        r = bluefs->add_block_device(
            BlueFS::BDEV_NEWWAL, dev_path, cct->_conf->bdev_enable_discard, BDEV_LABEL_BLOCK_SIZE);
        ceph_assert(r == 0);

        if (bluefs->bdev_support_label(BlueFS::BDEV_NEWWAL)) {
            r = _check_or_set_bdev_label(
                dev_path, bluefs->get_block_device_size(BlueFS::BDEV_NEWWAL), "bluefs wal", true);
            ceph_assert(r == 0);
        }
    }
    else if (id == BlueFS::BDEV_NEWDB) {
        ...
        r = bluefs->add_block_device(BlueFS::BDEV_NEWDB, dev_path, cct->_conf->bdev_enable_discard, SUPER_RESERVED);
        ceph_assert(r == 0);

        if (bluefs->bdev_support_label(BlueFS::BDEV_NEWDB)) {
            r = _check_or_set_bdev_label(
                dev_path, bluefs->get_block_device_size(BlueFS::BDEV_NEWDB), "bluefs db", true);
            ceph_assert(r == 0);
        }
    }
...
string BlueStore::get_device_path(unsigned id)
{
    ...
    if (id < BlueFS::MAX_BDEV) {
        switch (id) {
        case BlueFS::BDEV_WAL: res = path + "/block.wal"; break;
        case BlueFS::BDEV_DB:
            ...
            break;
        case BlueFS::BDEV_SLOW: res = path + "/block"; break;
        }
    }
    return res;
}
...
int BlueStore::expand_devices(ostream& out)
{
    ...
    if (size0 < size) {
        out << bluefs_layout.shared_bdev << " : expanding "
            << " from 0x" << std::hex << size0 << " to 0x" << size << std::dec << std::endl;
        _write_out_fm_meta(size);
        if (bdev->supported_bdev_label()) {
            if (_set_bdev_label_size(path, size) >= 0) {
                out << bluefs_layout.shared_bdev << " : size label updated to " << size << std::endl;
            }
        }
...
void BlueStore::set_cache_shards(unsigned num)
{
    ...
    for (unsigned i = oold; i < num; ++i) {
        onode_cache_shards[i] = OnodeCacheShard::create(cct, cct->_conf->bluestore_cache_type, logger);
    }
    for (unsigned i = bold; i < num; ++i) {
        buffer_cache_shards[i] = BufferCacheShard::create(cct, cct->_conf->bluestore_cache_type, logger);
    }
}
...
int _fsck_sum_extents(const PExtentVector& extents, bool compressed, store_statfs_t& expected_statfs)
{
    for (auto e : extents) {
...
int BlueStore::_fsck_check_extents(std::string_view ctx_descr, const PExtentVector& extents, bool compressed,
                                   mempool_dynamic_bitset& used_blocks, uint64_t granularity,
                                   BlueStoreRepairer* repairer, store_statfs_t& expected_statfs, FSCKDepth depth)
{
    ...
    for (auto e : extents) {
        ...
        if (depth != FSCK_SHALLOW) {
            ...
            apply_for_bitset_range(
                e.offset, e.length, granularity, used_blocks, [&](uint64_t pos, mempool_dynamic_bitset& bs) {
                    if (bs.test(pos)) {
                        if (repairer) {
                            repairer->note_misreference(pos * min_alloc_size, min_alloc_size, !already);
                        }
                        if (!already) {
            ...
            if (e.end() > bdev->get_size()) {
                derr << "fsck error:  " << ctx_descr << ", extent " << e << " past end of block device" << dendl;
                ++errors;
            }
        }
    }
...
void BlueStore::_fsck_check_statfs(const store_statfs_t& expected_statfs, const per_pool_statfs& expected_pool_statfs,
                                   int64_t& errors, int64_t& warnings, BlueStoreRepairer* repairer)
{
    string key;
    ...
    store_statfs_t s;
    {
        ...
        while (op != osd_pools.end()) {
            ...
            if (it_expected == my_expected_pool_statfs.end()) {
                ...
                derr << __func__ << "::fsck error: " << std::hex << "pool " << op0->first
                     << " has got no statfs to match against: " << s << std::dec << dendl;
                ++errors;
                if (repairer) {
                    osd_pools.erase(op0);
                ...
                if (!(s == it_expected->second)) {
                    derr << "fsck error: actual " << s << " != expected " << it_expected->second << " for pool "
    ...
    if (repairer) {
        ...
        if (!(s == expected_statfs)) {
            derr << "fsck error: actual " << s << " != expected " << expected_statfs << dendl;
            ++errors;
        }
    }
}

void BlueStore::_fsck_repair_shared_blobs(BlueStoreRepairer& repairer, shared_blob_2hash_tracker_t& sb_ref_counts,
                                          sb_info_space_efficient_map_t& sb_info)
{
    auto sb_ref_mismatches = sb_ref_counts.count_non_zero();
    dout(1) << __func__ << " repairing shared_blobs, ref mismatch estimate: " << sb_ref_mismatches << dendl;
    if (!sb_ref_mismatches)   // not expected to succeed, just in case
        return;


    auto foreach_shared_blob = [&](std::function<void(coll_t, ghobject_t, uint64_t, const bluestore_blob_t&)> cb) {
        ...
        if (it) {
            ...
            for (it->lower_bound(string()); it->valid(); it->next()) {
                ...
                if (!c || oid.shard_id != pgid.shard || oid.hobj.get_logical_pool() != (int64_t)pgid.pool() ||
                    !c->contains(oid)) {
                    c = nullptr;
                    for (auto& p : coll_map) {
                ...
                dout(20) << __func__ << " inspecting shared blob refs for col:" << c->cid << " obj:" << oid << dendl;
    ...
    foreach_shared_blob([&](coll_t cid, ghobject_t oid, uint64_t sbid, const bluestore_blob_t& b) {
        auto it = refs_map.lower_bound(sbid);
        if (it != refs_map.end() && it->first == sbid) {
            return;
        }
        for (auto& p : b.get_extents()) {
            if (p.is_valid() && !sb_ref_counts.test_all_zero_range(sbid, p.offset, p.length)) {
                refs_map.emplace_hint(it, sbid, bluestore_extent_ref_map_t());
                dout(20) << __func__ << " broken shared blob found for col:" << cid << " obj:" << oid << " sbid 0x "
    ...
    while (ref_it != refs_map.end()) {
        ...
        for (cnt = 0; cnt < max_transactions && ref_it != refs_map.end(); ref_it++) {
            auto sbid = ref_it->first;
            dout(20) << __func__ << " repaired shared_blob 0x" << std::hex << sbid << std::dec << ref_it->second
                     << dendl;
    ...
    sb_info.foreach_stray([&](const sb_info_t& sbi) {
        ...
        dout(20) << __func__ << " removing stray shared_blob 0x" << std::hex << sbid << std::dec << dendl;
...
BlueStore::OnodeRef BlueStore::fsck_check_objects_shallow(BlueStore::FSCKDepth depth, int64_t pool_id,
                                                          BlueStore::CollectionRef c, const ghobject_t& oid,
                                                          ...
                                                          const BlueStore::FSCK_ObjectCtx& ctx)
{
    ...
    store_statfs_t* res_statfs =
        (per_pool_stat_collection || repairer) ? &ctx.expected_pool_statfs[pool_id] : &ctx.expected_store_statfs;
    ...
    if (!o->extent_map.shards.empty()) {
        ...
        if (depth != FSCK_SHALLOW) {
            ...
            for (auto& s : o->extent_map.shards) {
                ...
                get_extent_shard_key(o->key, s.shard_info->offset, &expecting_shards->back());
                if (s.shard_info->offset >= o->onode.size) {
                    derr << "fsck error: " << oid << " shard 0x" << std::hex << s.shard_info->offset
                         << " past EOF at 0x" << o->onode.size << std::dec << dendl;
                    ++errors;
                }
            }
        }
    ...
    for (auto& l : o->extent_map.extent_map) {
        ...
        if (l.logical_offset < pos) {
            derr << "fsck error: " << oid << " lextent at 0x" << std::hex << l.logical_offset
                 << " overlaps with the previous, which ends at 0x" << pos << std::dec << dendl;
            ++errors;
        }
        if (depth != FSCK_SHALLOW && o->extent_map.spans_shard(l.logical_offset, l.length)) {
            derr << "fsck error: " << oid << " lextent at 0x" << std::hex << l.logical_offset << "~" << l.length
                 << " spans a shard boundary" << std::dec << dendl;
            ++errors;
        }
        ...
        if (depth != FSCK_SHALLOW && blob.has_unused()) {
            ...
            uint64_t end = round_up_to(l.blob_offset + l.length, chunk_size) / chunk_size;
    ...
    for (auto& i : ref_map) {
        ...
        bool equal = depth == FSCK_SHALLOW ? true : i.first->get_blob_use_tracker().equal(i.second);
        if (!equal) {
            derr << "fsck error: " << oid << " blob " << *i.first << " doesn't match expected ref_map " << i.second
                 << dendl;
            ++errors;
        }
        ...
        if (blob.is_shared()) {
            if (i.first->shared_blob->get_sbid() > blobid_max) {
                derr << "fsck error: " << oid << " blob " << blob << " sbid " << i.first->shared_blob->get_sbid()
                     << " > blobid_max " << blobid_max << dendl;
                ++errors;
            }
            else if (i.first->shared_blob->get_sbid() == 0) {
                derr << "fsck error: " << oid << " blob " << blob << " marked as shared but has uninitialized sbid"
                     << dendl;
                ++errors;
            }
            ...
            ceph_assert(sbi.pool_id == sb_info_t::INVALID_POOL_ID || sbi.pool_id == oid.hobj.get_logical_pool());
            ...
            errors += _fsck_sum_extents(blob.get_extents(), blob.is_compressed(), *res_statfs);
        }
    }   // for (auto& i : ref_map)

    {
            ...
            for (auto& [zone, first_offset] : zone_first_offsets) {
                ...
                if (p != (*ctx.zone_refs)[zone].end()) {
                    if (first_offset < p->second) {
                        dout(20) << " slightly wonky zone ref 0x" << std::hex << zone << " offset 0x" << p->second
                                 << " but first offset is 0x" << first_offset << "; this can happen due to clone_range"
                                 << dendl;
                    }
                    else {
                        dout(20) << " good zone ref 0x" << std::hex << zone << " offset 0x" << p->second
                                 << " <= first offset 0x" << first_offset << std::dec << dendl;
                    }
        ...
        if (broken) {
            derr << "fsck error: " << oid << " - " << broken
                 << " zombie spanning blob(s) found, the first one: " << *first_broken << dendl;
            if (repairer) {
                repairer->fix_spanning_blobs(db, [&](KeyValueDB::Transaction txn) { _record_onode(o, txn); });
            }
        }
    }
...
public:
    ...
    template<size_t BatchLen> struct FSCKWorkQueue : public ThreadPool::WorkQueue_
    {
        struct Entry
        {
            int64_t pool_id;
        ...
        FSCKWorkQueue(std::string n, size_t _batchCount, BlueStore* _store, ceph::mutex* _sb_info_lock,
                      sb_info_space_efficient_map_t& _sb_info, shared_blob_2hash_tracker_t& _sb_ref_counts,
        ...
        void _void_process(void* item, TPHandle& handle) override
        {
            ...
            BlueStore::FSCK_ObjectCtx ctx(batch->errors,
                                          batch->warnings,
                                          batch->num_objects,
                                          batch->num_extents,
                                          batch->num_blobs,
            ...
            for (size_t i = 0; i < batch->entry_count; i++) {
                ...
                store->fsck_check_objects_shallow(BlueStore::FSCK_SHALLOW,
                                                  entry.pool_id,
                                                  entry.c,
                                                  entry.oid,
                                                  entry.key,
        ...
        bool queue(int64_t pool_id, BlueStore::CollectionRef c, const ghobject_t& oid, const string& key,
        ...
        void finalize(ThreadPool& tp, BlueStore::FSCK_ObjectCtx& ctx)
        {
            ...
            for (size_t i = 0; i < batchCount; i++) {
                ...
                if (batch.entry_count) {
                    TPHandle tp_handle(store->cct, nullptr, timeout_interval, suicide_interval);
                ...
                for (auto it = batch.expected_pool_statfs.begin(); it != batch.expected_pool_statfs.end(); it++) {
...
void BlueStore::_fsck_check_object_omap(FSCKDepth depth, OnodeRef& o, const BlueStore::FSCK_ObjectCtx& ctx)
{
    auto& errors = ctx.errors;
    ...
    if (!o->onode.is_perpool_omap() && !o->onode.is_pgmeta_omap()) {
        if (per_pool_omap == OMAP_PER_POOL) {
            fsck_derr(errors, MAX_FSCK_ERROR_LINES)
                << "fsck error: " << o->oid << " has omap that is not per-pool or pgmeta" << fsck_dendl;
            ++errors;
        }
        else {
            const char* w;
            ...
            fsck_derr(num, MAX_FSCK_ERROR_LINES)
                << "fsck " << w << ": " << o->oid << " has omap that is not per-pool or pgmeta" << fsck_dendl;
        }
    }
    else if (!o->onode.is_perpg_omap() && !o->onode.is_pgmeta_omap()) {
        if (per_pool_omap == OMAP_PER_PG) {
            fsck_derr(errors, MAX_FSCK_ERROR_LINES)
                << "fsck error: " << o->oid << " has omap that is not per-pg or pgmeta" << fsck_dendl;
            ++errors;
        }
        else {
            const char* w;
            ...
            fsck_derr(num, MAX_FSCK_ERROR_LINES)
                << "fsck " << w << ": " << o->oid << " has omap that is not per-pg or pgmeta" << fsck_dendl;
        }
    }
    if (repairer && !o->onode.is_perpg_omap() && !o->onode.is_pgmeta_omap()) {
        ...
        map<string, bufferlist> kv;
        {
            ...
            uint8_t new_flags =
                o->onode.flags | bluestore_onode_t::FLAG_PERPOOL_OMAP | bluestore_onode_t::FLAG_PERPG_OMAP;
...
void BlueStore::_fsck_check_objects(FSCKDepth depth, BlueStore::FSCK_ObjectCtx& ctx)
{
    auto& errors = ctx.errors;
    auto sb_info_lock = ctx.sb_info_lock;
    auto& sb_info = ctx.sb_info;
    ...
    if (it) {
        ...
        std::unique_ptr<WQ> wq(
            new WQ("FSCKWorkQueue", (thread_count ?: 1) * 32, this, sb_info_lock, sb_info, sb_ref_counts, repairer));
        ...
        for (it->lower_bound(string()); it->valid(); it->next()) {
            ...
            if (is_extent_shard_key(it->key())) {
                ...
                while (!expecting_shards.empty() && expecting_shards.front() < it->key()) {
                    derr << "fsck error: missing shard key " << pretty_binary_string(expecting_shards.front()) << dendl;
                    ++errors;
                    expecting_shards.pop_front();
                }
                if (!expecting_shards.empty() && expecting_shards.front() == it->key()) {
                    // all good
                    expecting_shards.pop_front();
                    continue;
                }
            ...
            if (!queued) {
                ...
                o = fsck_check_objects_shallow(
                    depth, pool_id, c, oid, it->key(), it->value(), &expecting_shards, &referenced, ctx);
            }
            ...
            if (depth != FSCK_SHALLOW) {
                ...
                for (auto& i : referenced) {
                    dout(20) << __func__ << "  referenced 0x" << std::hex << i.second << std::dec << " for " << *i.first
                             << dendl;
                    const bluestore_blob_t& blob = i.first->get_blob();
                    if (i.second & blob.unused) {
                        derr << "fsck error: " << oid << " blob claims unused 0x" << std::hex << blob.unused
                             << " but extents reference 0x" << i.second << std::dec << " on blob " << *i.first << dendl;
                        ++errors;
                    }
                    if (blob.has_csum()) {
                        uint64_t blob_len = blob.get_logical_length();
                        ...
                        for (unsigned p = 0; p < csum_count; ++p) {
                            ...
                            if ((blob.unused & mask) == mask) {
                                ...
                                if (blob.get_csum_item(p) != 0) {
                                    derr << "fsck error: " << oid << " blob claims csum chunk 0x" << std::hex << pos
                                         << "~" << csum_chunk_size << " is unused (mask 0x" << mask << " of unused 0x"
                                         << blob.unused << ") but csum is non-zero 0x" << blob.get_csum_item(p)
                                         << std::dec << " on blob " << *i.first << dendl;
                                    ++errors;
                                }
                            }
                        }
                ...
                if (depth == FSCK_DEEP) {
                    ...
                    do {
                        ...
                        int r = _do_read(c.get(), o, offset, l, bl, CEPH_OSD_OP_FLAG_FADVISE_NOCACHE);
                        if (r < 0) {
                            ++errors;
                            derr << "fsck error: " << oid << std::hex << " error during read: "
                                 << " " << offset << "~" << l << " " << cpp_strerror(r) << std::dec << dendl;
                            break;
                        }
                        offset += l;
        ...
        if (depth == FSCK_SHALLOW && thread_count > 0) {
            ...
            if (processed_myself) {
                ...
                dout(0) << __func__ << " partial offload"
                        << ", done myself " << processed_myself << " of " << ctx.num_objects << "objects, threads "
                        << thread_count << dendl;
            }
        }
    }   // if (it)
...
{
    uint64_t sb_hash_size = uint64_t(cct->_conf.get_val<Option::size_t>("osd_memory_target") *
    ...
    if (bluefs) {
        ...
        bluefs->foreach_block_extents(bluefs_layout.shared_bdev, [&](uint64_t start, uint32_t len) {
            apply_for_bitset_range(start, len, alloc_size, used_blocks, [&](uint64_t pos, mempool_dynamic_bitset& bs) {
    ...
    apply_for_bitset_range(0,
                           std::max<uint64_t>(min_alloc_size, SUPER_RESERVED),
                           alloc_size,
                           used_blocks,
    ...
    if (bdev->is_smr()) {
        ...
        for (unsigned i = first_sequential_zone; i < num_zones; ++i) {
            ...
            if (zones[i].write_pointer > p && zones[i].num_dead_bytes < zones[i].write_pointer) {
                derr << "fsck error: zone 0x" << std::hex << i << " bluestore write pointer 0x"
                     << zones[i].write_pointer << " > device write pointer 0x" << p << " (with only 0x"
                     << zones[i].num_dead_bytes << " dead bytes)" << std::dec << dendl;
                ++errors;
            }
        }
        ...
        if (depth != FSCK_SHALLOW) {
            ...
            if (it) {
                for (it->lower_bound(string()); it->valid(); it->next()) {
                    ...
                    if (r < 0) {
                        derr << "fsck error: invalid zone ref key " << pretty_binary_string(key) << dendl;
                        if (repair) {
                    ...
                    dout(30) << " zone ref 0x" << std::hex << zone << " offset 0x" << offset << " -> " << std::dec
                             << oid << dendl;
                    if (zone_refs[zone].count(oid)) {
                        derr << "fsck error: second zone ref in zone 0x" << std::hex << zone << " offset 0x" << offset
                             << std::dec << " for " << oid << dendl;
                        if (repair) {
                            repairer.remove_key(db, PREFIX_ZONED_CL_INFO, key);
                        }
                        ++errors;
    ...
    // walk PREFIX_OBJ
    {
        ...
        BlueStore::FSCK_ObjectCtx ctx(errors,
                                      warnings,
                                      num_objects,
                                      num_extents,
                                      num_blobs,
    ...
    if (bdev->is_smr() && depth != FSCK_SHALLOW) {
        ...
        for (uint32_t zone = 0; zone < zone_refs.size(); ++zone) {
            for (auto& [oid, offset] : zone_refs[zone]) {
                derr << "fsck error: stray zone ref 0x" << std::hex << zone << " offset 0x" << offset << " -> "
                     << std::dec << oid << dendl;
                // FIXME: add repair
                ++errors;
            }
    ...
    if (sb_ref_mismatches != 0) {
        derr << "fsck error:"
             << "*" << sb_ref_mismatches << " shared blob references aren't matching, at least " << sb_ref_mismatches
             << " found" << dendl;
        ...
        for (it->lower_bound(string()); it->valid(); it->next()) {
            ...
            if (p == sb_info.end()) {
                if (sb_ref_mismatches > 0) {
                    ...
                    dout(5) << __func__ << " found duplicate(?) stray shared blob data for sbid 0x" << std::hex << sbid
                            << std::dec << dendl;
                }
                else {
                    derr << "fsck error: found stray shared blob data for sbid 0x" << std::hex << sbid << std::dec
                         << dendl;
                    ++errors;
                    if (repair) {
                        repairer.remove_key(db, PREFIX_SHARED_BLOB, key);
                ...
                try {
                    ...
                    derr << "fsck error: failed to decode Shared Blob" << pretty_binary_string(key) << dendl;
                    if (repair) {
                        dout(20) << __func__ << " undecodable Shared Blob, key:'" << pretty_binary_string(key)
                                 << "', removing" << dendl;
                        repairer.remove_key(db, PREFIX_SHARED_BLOB, key);
                    }
                    continue;
                ...
                if (sbi.pool_id != sb_info_t::INVALID_POOL_ID && (per_pool_stat_collection || repair)) {
    ...
    if (repair && repairer.preprocess_misreference(db)) {
        ...
        if (it) {
            ...
            for (it->lower_bound(string()); it->valid() && !bypass_rest; it->next()) {
                ...
                for (auto b : blobs) {
                    ...
                    for (auto e = pextents.begin(); e != pextents.end(); e++) {
                        ...
                        int64_t alloc_len = alloc->allocate(e->length, min_alloc_size, 0, 0, &exts);
                        if (alloc_len < 0 || alloc_len < (int64_t)e->length) {
                            derr << __func__ << " failed to allocate 0x" << std::hex << e->length << " allocated 0x "
                                 << (alloc_len < 0 ? 0 : alloc_len) << " min_alloc_size 0x" << min_alloc_size
                        ...
                        if (r < 0) {
                            derr << __func__ << " failed to read from 0x" << std::hex << e->offset << "~" << e->length
                                 << std::dec << dendl;
                        ...
                        b->get_blob().map_bl(b_off_cur, bl, [&](uint64_t offset, bufferlist& t) {
                    ...
                    if (b->get_blob().is_shared()) {
                        ...
                        if (sbi.allocated_chunks < 0) {
                            ...
                            expected_statfs->data_compressed_allocated -= uint64_t(-sbi.allocated_chunks)
            ...
            for (auto it = to_release.begin(); it != to_release.end(); ++it) {
                dout(10) << __func__ << " release 0x" << std::hex << it.get_start() << "~" << it.get_len() << std::dec
                         << dendl;
    ...
    _fsck_check_statfs(expected_store_statfs, expected_pool_statfs, errors, warnings, repair ? &repairer : nullptr);
    if (depth != FSCK_SHALLOW) {
        ...
        if (it) {
            ...
            for (it->lower_bound(string()); it->valid(); it->next()) {
                ...
                if (used_omap_head.count(omap_head) == 0 && omap_head != last_omap_head) {
                    pair<string, string> rk = it->raw_key();
                    fsck_derr(errors, MAX_FSCK_ERROR_LINES)
                        << "fsck error: found stray omap data on omap_head " << omap_head << " " << last_omap_head
                        << " prefix/key: " << url_escape(rk.first) << " " << url_escape(rk.second) << fsck_dendl;
                    ++errors;
        ...
        if (it) {
            ...
            for (it->lower_bound(string()); it->valid(); it->next()) {
                ...
                if (used_omap_head.count(omap_head) == 0 && omap_head != last_omap_head) {
                    pair<string, string> rk = it->raw_key();
                    fsck_derr(errors, MAX_FSCK_ERROR_LINES)
                        << "fsck error: found stray (pgmeta) omap data on omap_head " << omap_head << " "
                        << last_omap_head << " prefix/key: " << url_escape(rk.first) << " " << url_escape(rk.second)
                        << fsck_dendl;
                    last_omap_head = omap_head;
                    ++errors;
                }
        ...
        if (it) {
            ...
            for (it->lower_bound(string()); it->valid(); it->next()) {
                ...
                if (used_omap_head.count(omap_head) == 0 && omap_head != last_omap_head) {
                    pair<string, string> rk = it->raw_key();
                    fsck_derr(errors, MAX_FSCK_ERROR_LINES)
                        << "fsck error: found stray (per-pool) omap data on omap_head " << omap_head << " "
                        << last_omap_head << " prefix/key: " << url_escape(rk.first) << " " << url_escape(rk.second)
                        << fsck_dendl;
                    ++errors;
                    last_omap_head = omap_head;
            ...
            for (it->lower_bound(string()); it->valid(); it->next()) {
                ...
                if (used_omap_head.count(omap_head) == 0 && omap_head != last_omap_head) {
                    fsck_derr(errors, MAX_FSCK_ERROR_LINES)
                        ...
                        << " key " << pretty_binary_string(it->key()) << omap_head << " " << last_omap_head << " "
                ...
                try {
                    ...
                    if (repair) {
                        dout(20) << __func__ << " undecodable deferred TXN record, key: '"
                                 << pretty_binary_string(it->key()) << "', removing" << dendl;
                        repairer.remove_key(db, PREFIX_DEFERRED, it->key());
                    }
                    continue;
                }
                dout(20) << __func__ << "  deferred " << wt.seq << " ops " << wt.ops.size() << " released 0x"
                         << std::hex << wt.released << std::dec << dendl;
                for (auto e = wt.released.begin(); e != wt.released.end(); ++e) {
                    apply_for_bitset_range(e.get_start(),
                                           e.get_len(),
                                           alloc_size,
                                           used_blocks,
                                           [&](uint64_t pos, mempool_dynamic_bitset& bs) { bs.set(pos); });
        ...
        if (!fm->is_null_manager()) {
            ...
            if (freelist_type == "zoned") {
                ...
                for (uint32_t zone = first_sequential_zone; zone < num_zones; ++zone) {
                    ...
                    dout(10) << "  marking zone 0x" << std::hex << zone << " region after wp 0x" << offset << "~"
                             << length << std::dec << dendl;
                    apply_for_bitset_range(
                        offset, length, alloc_size, used_blocks, [&](uint64_t pos, mempool_dynamic_bitset& bs) {
                            if (bs.test(pos)) {
                                derr << "fsck error: zone 0x" << std::hex << zone << " has used space at 0x"
                                     << pos * alloc_size << " beyond write pointer 0x" << wp << std::dec << dendl;
                                intersects = true;
                            }
                            else {
                                bs.set(pos);
                ...
                for (uint32_t zone = first_sequential_zone; zone < num_zones; ++zone, zone_dead = 0) {
                    while (pos != decltype(used_blocks)::npos && (pos * min_alloc_size) / zone_size == zone) {
                        dout(40) << " zone 0x" << std::hex << zone << " dead 0x" << (pos * min_alloc_size) << "~"
                                 << min_alloc_size << std::dec << dendl;
                        zone_dead += min_alloc_size;
                        pos = used_blocks.find_next(pos);
                    }
                    dout(20) << " zone 0x" << std::hex << zone << " dead is 0x" << zone_dead << std::dec << dendl;
                    ...
                    if (a->get_dead_bytes(zone) != zone_dead) {
                        derr << "fsck error: zone 0x" << std::hex << zone << " has 0x" << zone_dead
                             << " dead bytes but freelist says 0x" << a->get_dead_bytes(zone) << dendl;
                        ++errors;
                        // TODO: repair
                    }
                }
...
#endif
            {
                ...
                while (fm->enumerate_next(db, &offset, &length)) {
                    ...
                    apply_for_bitset_range(
                        offset, length, alloc_size, used_blocks, [&](uint64_t pos, mempool_dynamic_bitset& bs) {
                            ...
                            if (bs.test(pos) && !bluefs_used_blocks.test(pos)) {
                                if (offset == SUPER_RESERVED && length == min_alloc_size - SUPER_RESERVED) {
                                    ...
                                    dout(10) << __func__ << " ignoring free extent between SUPER_RESERVED"
                                             << " and min_alloc_size, 0x" << std::hex << offset << "~" << length
                                             << std::dec << dendl;
                                }
                                else {
                                    intersects = true;
                                    if (repair) {
                                        repairer.fix_false_free(db, fm, pos * min_alloc_size, min_alloc_size);
                                    }
                                }
                            }
                            else {
                    ...
                    if (intersects) {
                        derr << "fsck error: free extent 0x" << std::hex << offset << "~" << length << std::dec
                        ...
                        while (true) {
                            ...
                            if (next != cur + 1) {
                                ...
                                derr << "fsck error: leaked extent 0x" << std::hex
                                     ...
                                     << ((cur + 1 - start) * fm->get_alloc_size()) << std::dec << dendl;
                                if (repair) {
                                    repairer.fix_leaked(
                                        db, fm, start * min_alloc_size, (cur + 1 - start) * min_alloc_size);
                                }
                                start = next;
                                break;
                            }
...
out_scan:
    ...
    dout(2) << __func__ << " " << num_extents << " extents to " << num_blobs << " blobs, " << num_spanning_blobs
            << " spanning, " << num_shared_blobs << " shared." << dendl;

    utime_t duration = ceph_clock_now() - start;
    dout(1) << __func__ << " <<<FINISH>>> with " << errors << " errors, " << warnings << " warnings, " << repaired
            << " repaired, " << (errors + warnings - (int)repaired) << " remaining in " << duration << " seconds"
            << dendl;
...
void BlueStore::inject_broken_shared_blob_key(const string& key, const bufferlist& bl)
{
...
void BlueStore::inject_stray_shared_blob_key(uint64_t sbid)
{
    ...
    dout(20) << __func__ << " sbid " << sbid << " takes " << bl.length() << " bytes, updating" << dendl;
...
{
    PExtentVector exts;
    int64_t alloc_len = alloc->allocate(len, min_alloc_size, min_alloc_size * 256, 0, &exts);
...
void BlueStore::inject_false_free(coll_t cid, ghobject_t oid)
{
    ...
    for (auto pext : v) {
        if (pext->size()) {
            ...
            while (p != pext->end()) {
                if (p->is_valid()) {
                    dout(20) << __func__ << " release 0x" << std::hex << p->offset << "~" << p->length << std::dec
                             << dendl;
...
void BlueStore::inject_legacy_omap(coll_t cid, ghobject_t oid)
{
    ...
    o->onode.clear_flag(bluestore_onode_t::FLAG_PERPG_OMAP | bluestore_onode_t::FLAG_PERPOOL_OMAP |
...
void BlueStore::inject_misreference(coll_t cid1, ghobject_t oid1, coll_t cid2, ghobject_t oid2, uint64_t offset)
{
    OnodeRef o1;
...
void BlueStore::inject_zombie_spanning_blob(coll_t cid, ghobject_t oid, int16_t blob_id)
{
    OnodeRef o;
    CollectionRef c = _get_collection(cid);
    ceph_assert(c);
...
void BlueStore::collect_metadata(map<string, string>* pm)
{
    ...
    if (bluefs) {
        ...
        (*pm)["bluefs_single_shared_device"] = stringify((int)bluefs_layout.single_shared_device());
...
int BlueStore::get_numa_node(int* final_node, set<int>* out_nodes, set<string>* out_failed)
{
    int node = -1;
    set<string> devices;
    get_devices(&devices);
...
int BlueStore::pool_statfs(uint64_t pool_id, struct store_statfs_t* buf, bool* out_per_pool_omap)
{
...
{
    string s;
    if (!per_pool_stat_collection && cct->_conf->bluestore_warn_on_legacy_statfs) {
    ...
    while (p != removed_colls.end()) {
        ...
        if (c->onode_space.map_any([&](Onode* o) {
                ...
                if (o->flushing_count.load()) {
                    dout(10) << __func__ << " " << c << " " << c->cid << " " << o->oid << " flush_txns "
                             << o->flushing_count << dendl;
                    return true;
                }
                return false;
            })) {
...
ObjectStore::CollectionHandle BlueStore::create_new_collection(const coll_t& cid)
{
...
void BlueStore::set_collection_commit_queue(const coll_t& cid, ContextQueue* commit_queue)
{
    if (commit_queue) {
        std::shared_lock l(coll_lock);
...
int BlueStore::stat(CollectionHandle& c_, const ghobject_t& oid, struct stat* st, bool allow_eio)
{
    Collection* c = static_cast<Collection*>(c_.get());
...
int BlueStore::set_collection_opts(CollectionHandle& ch, const pool_opts_t& opts)
{
...
int BlueStore::read(CollectionHandle& c_, const ghobject_t& oid, uint64_t offset, size_t length, bufferlist& bl,
                    uint32_t op_flags)
{
    auto start = mono_clock::now();
    Collection* c = static_cast<Collection*>(c_.get());
    const coll_t& cid = c->get_cid();
    dout(15) << __func__ << " " << cid << " " << oid << " 0x" << std::hex << offset << "~" << length << std::dec
             << dendl;
    if (!c->exists) return -ENOENT;
...
out:
    ...
    dout(10) << __func__ << " " << cid << " " << oid << " 0x" << std::hex << offset << "~" << length << std::dec
             << " = " << r << dendl;
    log_latency(__func__, l_bluestore_read_lat, mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
    return r;
}

void BlueStore::_read_cache(OnodeRef& o, uint64_t offset, size_t length, int read_cache_policy,
                            ready_regions_t& ready_regions, blobs2read_t& blobs2read)
{
    ...
    while (left > 0 && lp != o->extent_map.extent_map.end()) {
        ...
        bptr->shared_blob->bc.read(
            bptr->shared_blob->get_cache(), b_off, b_len, cache_res, cache_interval, read_cache_policy);
        dout(20) << __func__ << "  blob " << *bptr << std::hex << " need 0x" << b_off << "~" << b_len << " cache has 0x"
                 << cache_interval << std::dec << dendl;
        ...
        while (b_len > 0) {
            ...
            if (pc != cache_res.end() && pc->first == b_off) {
                ...
                dout(30) << __func__ << "    use cache 0x" << std::hex << pos << ": 0x" << b_off << "~" << l << std::dec
                         << dendl;
                ++pc;
            }
            else {
                ...
                dout(30) << __func__ << "    will read 0x" << std::hex << pos << ": 0x" << b_off << "~" << l << std::dec
                         << dendl;
                // merge regions
                {
...
int BlueStore::_prepare_read_ioc(blobs2read_t& blobs2read, vector<bufferlist>* compressed_blob_bls, IOContext* ioc)
{
    for (auto& p : blobs2read) {
        ...
        if (bptr->get_blob().is_compressed()) {
            ...
            auto r =
                bptr->get_blob().map(0, bptr->get_blob().get_ondisk_length(), [&](uint64_t offset, uint64_t length) {
            ...
            // read the pieces
            for (auto& req : r2r) {
                dout(20) << __func__ << "    region 0x" << std::hex << req.regs.front().logical_offset << ": 0x"
                         << req.regs.front().blob_xoffset << " reading 0x" << req.r_off << "~" << req.r_len << std::dec
                         << dendl;

                // read it
                auto r = bptr->get_blob().map(req.r_off, req.r_len, [&](uint64_t offset, uint64_t length) {
...
int BlueStore::_generate_read_result_bl(OnodeRef& o, uint64_t offset, size_t length, ready_regions_t& ready_regions,
                                        vector<bufferlist>& compressed_blob_bls, blobs2read_t& blobs2read,
                                        bool buffered, bool* csum_error, bufferlist& bl)
{
    ...
    while (b2r_it != blobs2read.end()) {
        ...
        if (bptr->get_blob().is_compressed()) {
            ...
            if (_verify_csum(o, &bptr->get_blob(), 0, compressed_bl, r2r.front().regs.front().logical_offset) < 0) {
                *csum_error = true;
                return -EIO;
            }
            bufferlist raw_bl;
            ...
            for (auto& req : r2r) {
                for (auto& r : req.regs) {
                    ready_regions[r.logical_offset].substr_of(raw_bl, r.blob_xoffset, r.length);
                }
            }
        }
        else {
            for (auto& req : r2r) {
                if (_verify_csum(o, &bptr->get_blob(), req.r_off, req.bl, req.regs.front().logical_offset) < 0) {
                    *csum_error = true;
                    return -EIO;
                }
                if (buffered) {
                    bptr->shared_blob->bc.did_read(bptr->shared_blob->get_cache(), req.r_off, req.bl);
                }
    ...
    while (pos < length) {
        if (pr != pr_end && pr->first == pos + offset) {
            dout(30) << __func__ << " assemble 0x" << std::hex << pos << ": data from 0x" << pr->first << "~"
                     << pr->second.length() << std::dec << dendl;
            ...
            dout(30) << __func__ << " assemble 0x" << std::hex << pos << ": zeros for 0x" << (pos + offset) << "~" << l
                     << std::dec << dendl;
            bl.append_zero(l);
            pos += l;
        }
...
int BlueStore::_do_read(Collection* c, OnodeRef& o, uint64_t offset, size_t length, bufferlist& bl, uint32_t op_flags,
                        uint64_t retry_count)
{
    ...
    dout(20) << __func__ << " 0x" << std::hex << offset << "~" << length << " size 0x" << o->onode.size << " ("
             << std::dec << o->onode.size << ")" << dendl;
    bl.clear();
    ...
    log_latency(__func__, l_bluestore_read_onode_meta_lat, mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
    _dump_onode<30>(cct, *o);
    ...
    if (retry_count) {
        ...
        dout(5) << __func__ << " read at 0x" << std::hex << offset << "~" << length << " failed " << std::dec
                << retry_count << " times before succeeding" << dendl;
        stringstream s;
...
int BlueStore::_verify_csum(OnodeRef& o, const bluestore_blob_t* blob, uint64_t blob_xoffset, const bufferlist& bl,
                            uint64_t logical_offset) const
{
    ...
    if (r < 0) {
        if (r == -1) {
            PExtentVector pex;
            blob->map(bad, blob->get_csum_chunk_size(), [&](uint64_t offset, uint64_t length) {
                pex.emplace_back(bluestore_pextent_t(offset, length));
                return 0;
            });
            derr << __func__ << " bad " << Checksummer::get_csum_type_string(blob->csum_type) << "/0x" << std::hex
                 << blob->get_csum_chunk_size() << " checksum at blob offset 0x" << bad << ", got 0x" << bad_csum
                 << ", expected 0x" << blob->get_csum_item(bad / blob->get_csum_chunk_size()) << std::dec
                 << ", device location " << pex << ", logical extent 0x" << std::hex
                 << (logical_offset + bad - blob_xoffset) << "~" << blob->get_csum_chunk_size() << std::dec
                 << ", object " << o->oid << dendl;
        }
        else {
    ...
    log_latency(__func__, l_bluestore_csum_lat, mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
...
int BlueStore::_decompress(bufferlist& source, bufferlist* result)
{
    ...
    log_latency(__func__, l_bluestore_decompress_lat, mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
    return r;
}

// this stores fiemap into interval_set, other variations
// use it internally
int BlueStore::_fiemap(CollectionHandle& c_, const ghobject_t& oid, uint64_t offset, size_t length,
                       interval_set<uint64_t>& destset)
{
    ...
    if (!c->exists) return -ENOENT;
    {
        ...
        dout(20) << __func__ << " 0x" << std::hex << offset << "~" << length << " size 0x" << o->onode.size << std::dec
                 << dendl;
        ...
        while (length > 0) {
            ...
            if (ep != eend && ep->logical_offset <= offset) {
                ...
                dout(30) << __func__ << " lextent 0x" << std::hex << offset << "~" << x_len << std::dec << " blob "
                         << ep->blob << dendl;
            ...
            if (ep != eend && ep->logical_offset > offset && ep->logical_offset - offset < x_len) {
...
out:
    dout(20) << __func__ << " 0x" << std::hex << offset << "~" << length << " size = 0x(" << destset << ")" << std::dec
             << dendl;
    return 0;
}

int BlueStore::fiemap(CollectionHandle& c_, const ghobject_t& oid, uint64_t offset, size_t length, bufferlist& bl)
{
...
int BlueStore::fiemap(CollectionHandle& c_, const ghobject_t& oid, uint64_t offset, size_t length,
...
int BlueStore::readv(CollectionHandle& c_, const ghobject_t& oid, interval_set<uint64_t>& m, bufferlist& bl,
                     uint32_t op_flags)
{
    auto start = mono_clock::now();
    Collection* c = static_cast<Collection*>(c_.get());
...
int BlueStore::_do_readv(Collection* c, OnodeRef& o, const interval_set<uint64_t>& m, bufferlist& bl, uint32_t op_flags,
                         uint64_t retry_count)
{
    FUNCTRACE(cct);
    int r = 0;
    ...
    dout(20) << __func__ << " fiemap " << m << std::hex << " size 0x" << o->onode.size << " (" << std::dec
             << o->onode.size << ")" << dendl;
    ...
    for (auto p = m.begin(); p != m.end(); p++, i++) {
        ...
        _read_cache(
            o, p.get_start(), p.get_len(), read_cache_policy, std::get<0>(raw_results[i]), std::get<2>(raw_results[i]));
...
int BlueStore::dump_onode(CollectionHandle& c_, const ghobject_t& oid, const string& section_name, Formatter* f)
{
    Collection* c = static_cast<Collection*>(c_.get());
...
int BlueStore::getattr(CollectionHandle& c_, const ghobject_t& oid, const char* name, bufferptr& value)
{
    Collection* c = static_cast<Collection*>(c_.get());
...
int BlueStore::getattrs(CollectionHandle& c_, const ghobject_t& oid, map<string, bufferptr, less<>>& aset)
{
    Collection* c = static_cast<Collection*>(c_.get());
    dout(15) << __func__ << " " << c->cid << " " << oid << dendl;
    if (!c->exists) return -ENOENT;
...
int BlueStore::list_collections(vector<coll_t>& ls)
{
    ...
    for (ceph::unordered_map<coll_t, CollectionRef>::iterator p = coll_map.begin(); p != coll_map.end(); ++p)
...
int BlueStore::collection_empty(CollectionHandle& ch, bool* empty)
{
    ...
    ghobject_t next;
    int r = collection_list(ch, ghobject_t(), ghobject_t::get_max(), 1, &ls, &next);
    if (r < 0) {
...
int BlueStore::collection_list(CollectionHandle& c_, const ghobject_t& start, const ghobject_t& end, int max,
                               vector<ghobject_t>* ls, ghobject_t* pnext)
{
    ...
    dout(10) << __func__ << " " << c->cid << " start " << start << " end " << end << " max " << max << " = " << r
             << ", ls.size() = " << ls->size() << ", next = " << (pnext ? *pnext : ghobject_t()) << dendl;
    return r;
}

int BlueStore::collection_list_legacy(CollectionHandle& c_, const ghobject_t& start, const ghobject_t& end, int max,
                                      vector<ghobject_t>* ls, ghobject_t* pnext)
{
    Collection* c = static_cast<Collection*>(c_.get());
    c->flush();
...
int BlueStore::_collection_list(Collection* c, const ghobject_t& start, const ghobject_t& end, int max, bool legacy,
                                vector<ghobject_t>* ls, ghobject_t* pnext)
{
    ...
    auto log_latency = make_scope_guard([&, start_time = mono_clock::now(), func_name = __func__] {
        log_latency_fn(func_name,
                       l_bluestore_clist_lat,
                       mono_clock::now() - start_time,
                       cct->_conf->bluestore_log_collection_list_age,
                       [&](const ceph::timespan& lat) {
                           ostringstream ostr;
                           ostr << ", lat = " << timespan_str(lat) << " cid =" << c->cid << " start " << start
                                << " end " << end << " max " << max;
                           return ostr.str();
                       });
    });
    ...
    get_coll_range(c->cid,
                   ...
                   &coll_range_end,
                   legacy);
    dout(20) << __func__ << " range " << coll_range_temp_start << " to " << coll_range_temp_end << " and "
             << coll_range_start << " to " << coll_range_end << " start " << start << dendl;
    if (legacy) {
        it = std::make_unique<SimpleCollectionListIterator>(cct, db->get_iterator(PREFIX_OBJ));
    }
    else {
        it = std::make_unique<SortedCollectionListIterator>(db->get_iterator(PREFIX_OBJ));
    }
...
int BlueStore::omap_get(CollectionHandle& c_,          ///< [in] Collection containing oid
...
int BlueStore::_omap_get(Collection* c,                 ///< [in] Collection containing oid
                         const ghobject_t& oid,         ///< [in] Object containing omap
                         bufferlist* header,            ///< [out] omap header
                         map<string, bufferlist>* out   /// < [out] Key to value map
)
...
int BlueStore::_onode_omap_get(const OnodeRef& o,             ///< [in] Object containing omap
                               bufferlist* header,            ///< [out] omap header
                               map<string, bufferlist>* out   /// < [out] Key to value map
)
{
...
int BlueStore::omap_get_header(CollectionHandle& c_,    ///< [in] Collection containing oid
                               const ghobject_t& oid,   ///< [in] Object containing omap
                               bufferlist* header,      ///< [out] omap header
...
int BlueStore::omap_get_keys(CollectionHandle& c_,    ///< [in] Collection containing oid
                             const ghobject_t& oid,   ///< [in] Object containing omap
...
int BlueStore::omap_get_values(CollectionHandle& c_,          ///< [in] Collection containing oid
                               const ghobject_t& oid,         ///< [in] Object containing omap
...
int BlueStore::omap_check_keys(CollectionHandle& c_,      ///< [in] Collection containing oid
                               const ghobject_t& oid,     ///< [in] Object containing omap
...
ObjectMap::ObjectMapIterator BlueStore::get_omap_iterator(CollectionHandle& c_,   ///< [in] collection
...
{
    ceph_assert(min_alloc_size);
    return round_up_to(std::max<uint64_t>(SUPER_RESERVED, min_alloc_size), min_alloc_size);
}

void BlueStore::_prepare_ondisk_format_super(KeyValueDB::Transaction& t)
{
    dout(10) << __func__ << " ondisk_format " << ondisk_format << " min_compat_ondisk_format "
             << min_compat_ondisk_format << dendl;
...
int BlueStore::_open_super_meta()
{
    ...
    int32_t compat_ondisk_format = 0;
    {
        ...
        if (r < 0) {
            ...
            dout(20) << __func__ << " missing ondisk_format; assuming kraken" << dendl;
            ondisk_format = 1;
        ...
        dout(1) << __func__ << " ondisk_format " << ondisk_format << " compat_ondisk_format " << compat_ondisk_format
                << dendl;
    }

    if (latest_ondisk_format < compat_ondisk_format) {
        derr << __func__ << " compat_ondisk_format is " << compat_ondisk_format << " but we only understand version "
    ...
    }
    {
        ...
        if (r >= 0) {
            ...
            dout(1) << __func__ << " first_sequential_zone 0x" << std::hex << first_sequential_zone << std::dec
                    << dendl;
            ceph_assert(bdev->is_smr());
        }
        else {
...
BlueStore::TransContext* BlueStore::_txc_create(Collection* c, OpSequencer* osr, list<Context*>* on_commits,
                                                TrackedOpRef osd_op)
{
...
void BlueStore::_txc_state_proc(TransContext* txc)
{
    while (true) {
        ...
        {
            mono_clock::duration lat = throttle.log_state_latency(*txc, logger, l_bluestore_state_aio_wait_lat);
        ...
        case TransContext::STATE_IO_DONE:
            ...
            if (cct->_conf->bluestore_sync_submit_transaction) {
                if (txc->last_nid >= nid_max || txc->last_blobid >= blobid_max) {
                    ...
                    dout(20) << __func__ << " prior txc(s) with unstable ios " << txc->osr->txc_with_unstable_io.load()
                             << dendl;
                }
...
{
    dout(20) << __func__ << " txc " << txc << " onodes " << txc->onodes << " shared_blobs " << txc->shared_blobs
             << dendl;
    ...
    for (auto sb : txc->shared_blobs) {
        ...
        if (sb->persistent->empty()) {
            ...
            dout(20) << __func__ << " shared_blob 0x" << std::hex << sbid << std::dec << " is " << bl.length() << " "
                     << *sb << dendl;
...
{
    dout(20) << __func__ << " txc " << txc << std::hex << " allocated 0x" << txc->allocated << " released 0x"
    ...
    if (!fm->is_null_manager()) {
        ...
        if (!txc->allocated.empty() && !txc->released.empty()) {
            ...
            if (!overlap.empty()) {
                ...
                dout(20) << __func__ << "  overlap 0x" << std::hex << overlap << ", new allocated 0x" << tmp_allocated
                         << " released 0x" << tmp_released << std::dec << dendl;
        ...
        for (interval_set<uint64_t>::iterator p = preleased->begin(); p != preleased->end(); ++p) {
            dout(20) << __func__ << " release 0x" << std::hex << p.get_start() << "~" << p.get_len() << std::dec
                     << dendl;
    ...
    if (bdev->is_smr()) {
        for (auto& i : txc->old_zone_offset_refs) {
            dout(20) << __func__ << " rm ref zone 0x" << std::hex << i.first.second << " offset 0x" << i.second
                     << std::dec << " -> " << i.first.first->oid << dendl;
            string key;
        ...
        for (auto& i : txc->new_zone_offset_refs) {
            // (zone, offset) -> oid
            dout(20) << __func__ << " add ref zone 0x" << std::hex << i.first.second << " offset 0x" << i.second
                     << std::dec << " -> " << i.first.first->oid << dendl;
            string key;
            get_zone_offset_object_key(i.first.second, i.second, i.first.first->oid, &key);
            bufferlist v;
    ...
    OpSequencer::q_list_t releasing_txc;
    {
        ...
        while (!osr->q.empty()) {
            ...
            if (txc->get_state() != TransContext::STATE_DONE) {
                if (txc->get_state() == TransContext::STATE_PREPARE && deferred_aggressive) {
...
void BlueStore::_osr_attach(Collection* c)
{
    ...
    if (q != coll_map.end()) {
        c->osr = q->second->osr;
        ldout(cct, 10) << __func__ << " " << c->cid << " reusing osr " << c->osr << " from existing coll " << q->second
                       << dendl;
    }
    else {
        std::lock_guard l(zombie_osr_lock);
...
void BlueStore::_kv_sync_thread()
{
    ...
    while (true) {
        ...
        if (kv_queue.empty() &&
            ((deferred_done_queue.empty() && deferred_stable_queue.empty()) || !deferred_aggressive)) {
            if (kv_stop) break;
            ...
            dout(20) << __func__ << " committing " << kv_queue.size() << " submitting " << kv_queue_unsubmitted.size()
                     << " deferred done " << deferred_done_queue.size() << " stable " << deferred_stable_queue.size()
                     << dendl;
...

            {
                ...
                if (kv_committing_to_finalize.empty()) {
                    ...
                    kv_committing_to_finalize.insert(
                        kv_committing_to_finalize.end(), kv_committing.begin(), kv_committing.end());
                    kv_committing.clear();
                }
                if (deferred_stable_to_finalize.empty()) {
                    ...
                    deferred_stable_to_finalize.insert(
                        deferred_stable_to_finalize.end(), deferred_stable.begin(), deferred_stable.end());
...

            {
                ...
                dout(20) << __func__ << " committed " << committing_size << " cleaned " << deferred_size << " in "
                         << dur << " (" << dur_flush << " flush + " << dur_kv << " kv commit)" << dendl;
    ...
    while (true) {
        ...
        if (kv_committing_to_finalize.empty() && deferred_stable_to_finalize.empty()) {
            ...
            if (!deferred_aggressive) {
                if (deferred_queue_size >= deferred_batch_ops.load() || throttle.should_submit_deferred()) {
                    deferred_try_submit();
                }
            }
            ...
            logger->set(l_bluestore_fragmentation, (uint64_t)(alloc->get_fragmentation() * 1000));

            log_latency(
                "kv_final", l_bluestore_kv_final_lat, mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
...
void BlueStore::_zoned_clean_zone(uint64_t zone, ZonedAllocator* a, ZonedFreelistManager* f)
{
    ...
    for (it->lower_bound(zone_start); it->valid(); it->next()) {
        ...
        if (r < 0) {
            derr << __func__ << " failed to decode zone ref " << pretty_binary_string(k) << dendl;
            continue;
        }
        if (zone != z) {
        ...
        dout(10) << __func__ << " zone 0x" << std::hex << zone << " offset 0x" << offset << std::dec << " " << oid
                 << dendl;
        _clean_some(oid, zone);
    }

    if (a->get_live_bytes(zone) > 0) {
        derr << "zone 0x" << std::hex << zone << " still has 0x" << a->get_live_bytes(zone) << " live bytes" << std::dec
             << dendl;
    ...
    dout(10) << __func__ << " done cleaning zone 0x" << std::hex << zone << std::dec << dendl;
    a->reset_zone(zone);
}

void BlueStore::_clean_some(ghobject_t oid, uint32_t zone)
{
    dout(10) << __func__ << " " << oid << " from zone 0x" << std::hex << zone << std::dec << dendl;
    ...
    if (to_move.empty()) {
        dout(10) << __func__ << " no references to zone 0x" << std::hex << zone << std::dec << " from " << oid << dendl;
        return;
    }

    dout(10) << __func__ << " rewriting object extents 0x" << std::hex << to_move << std::dec << dendl;
...
bluestore_deferred_op_t* BlueStore::_get_deferred_op(TransContext* txc, uint64_t len)
{
    if (!txc->deferred_txn) {
...
void BlueStore::_deferred_submit_unlock(OpSequencer* osr)
{
    ...
    while (true) {
        if (i == b->iomap.end() || i->first != pos) {
            if (bl.length()) {
                dout(20) << __func__ << " write 0x" << std::hex << start << "~" << bl.length() << " crc "
                         << bl.crc32c(-1) << std::dec << dendl;
        ...
        dout(20) << __func__ << "   seq " << i->second.seq << " 0x" << std::hex << pos << "~" << i->second.bl.length()
                 << std::dec << dendl;
        if (!bl.length()) {
            start = pos;
        }
...
{
    BlueStore* store;
    C_DeferredTrySubmit(BlueStore* s)
        : store(s)
    {}
...
int BlueStore::queue_transactions(CollectionHandle& ch, vector<Transaction>& tls, TrackedOpRef op,
                                  ThreadPool::TPHandle* handle)
{
    ...
    log_latency("submit_transact", l_bluestore_submit_lat, mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
    log_latency("throttle_transact", l_bluestore_throttle_lat, tend - tstart, cct->_conf->bluestore_log_op_age);
    return 0;
}
...
void BlueStore::_txc_add_transaction(TransContext* txc, Transaction* t)
{
    ...
    for (int pos = 0; i.have_op(); ++pos) {
        ...
        case Transaction::OP_SPLIT_COLLECTION: ceph_abort_msg("deprecated"); break;
        ...
        case Transaction::OP_COLL_SETATTR: r = -EOPNOTSUPP; break;

        case Transaction::OP_COLL_RMATTR: r = -EOPNOTSUPP; break;

        case Transaction::OP_COLL_RENAME: ceph_abort_msg("not implemented"); break;
        }
        if (r < 0) {
            derr << __func__ << " error " << cpp_strerror(r) << " not handled on operation " << op->op << " (op " << pos
        ...
        bool create = false;
        if (op->op == Transaction::OP_TOUCH || op->op == Transaction::OP_CREATE || op->op == Transaction::OP_WRITE ||
        ...
        case Transaction::OP_TOUCH: r = _touch(txc, c, o); break;
        ...
        case Transaction::OP_CLONERANGE: ceph_abort_msg("deprecated"); break;
        ...
        case Transaction::OP_COLL_ADD: ceph_abort_msg("not implemented"); break;

        case Transaction::OP_COLL_REMOVE: ceph_abort_msg("not implemented"); break;

        case Transaction::OP_COLL_MOVE: ceph_abort_msg("deprecated"); break;
        ...
        {
            r = _set_alloc_hint(txc, c, o, op->expected_object_size, op->expected_write_size, op->hint);
        } break;

        default: derr << __func__ << " bad op " << op->op << dendl; ceph_abort();
        }
    ...
    endop:
        if (r < 0) {
            ...
            if (r == -ENOENT && !(op->op == Transaction::OP_CLONERANGE || op->op == Transaction::OP_CLONE ||
                                  ...
                                  op->op == Transaction::OP_OMAP_RMKEYS || op->op == Transaction::OP_OMAP_RMKEYRANGE ||
            ...
            if (!ok) {
                ...
                derr << __func__ << " error " << cpp_strerror(r) << " not handled on operation " << op->op << " (op "
                     << pos << ", counting from 0)" << dendl;
                derr << msg << dendl;
                _dump_transaction<0>(cct, t);
                ceph_abort_msg("unexpected error");
            }
...
void BlueStore::_pad_zeros(bufferlist* bl, uint64_t* offset, uint64_t chunk_size)
{
    auto length = bl->length();
    dout(30) << __func__ << " 0x" << std::hex << *offset << "~" << length << " chunk_size 0x" << chunk_size << std::dec
             << dendl;
    dout(40) << "before:\n";
    bl->hexdump(*_dout);
    *_dout << dendl;
    ...
    dout(20) << __func__ << " pad 0x" << std::hex << front_pad << " + 0x" << back_pad << " on front/back, now 0x"
...
void BlueStore::_do_write_small(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length,
                                bufferlist::iterator& blp, WriteContext* wctx)
{
    ...
    if (bdev->is_smr()) {
        ...
        if (!cct->_conf->bluestore_zero_block_detection || !bl.is_zero()) {
            ...
            dout(20) << __func__ << " skip small zero block " << std::hex << " (0x" << b_off0 << "~" << bl.length()
                     << ")"
        ...
        if (ep != end && ep->logical_offset < offset + max_bsize) {
            ...
            if (bstart >= end_offs) {
                ...
                if (head_pad || tail_pad) {
                    o->extent_map.fault_range(db, offset - head_pad, end_offs - offset + head_pad + tail_pad);
                }
                ...
                if ((b_off % chunk_size == 0 && b_len % chunk_size == 0) &&
                    b->get_blob().get_ondisk_length() >= b_off + b_len && b->get_blob().is_unused(b_off, b_len) &&
                    b->get_blob().is_allocated(b_off, b_len)) {
                    _apply_padding(head_pad, tail_pad, bl);

                    dout(20) << __func__ << "  write to unused 0x" << std::hex << b_off << "~" << b_len << " pad 0x"
                             << head_pad << " + 0x" << tail_pad << std::dec << " of mutable " << *b << dendl;
                    _buffer_cache_write(txc, b, b_off, bl, wctx->buffered ? 0 : Buffer::FLAG_NOCACHE);

                    if (!g_conf()->bluestore_debug_omit_block_device_write) {
                        if (b_len < prefer_deferred_size) {
                            dout(20) << __func__ << " deferring small 0x" << std::hex << b_len << std::dec
                ...
                if ((head_read || tail_read) && (b->get_blob().get_ondisk_length() >= b_off + b_len + tail_read) &&
                ...
                if (b->get_blob().get_ondisk_length() >= b_off + b_len && b_off % chunk_size == 0 &&
                    ...
                    dout(20) << __func__ << "  reading head 0x" << std::hex << head_read << " and tail 0x" << tail_read
                             << std::dec << dendl;
                    if (head_read) {
                        bufferlist head_bl;
                    ...
                    if (!g_conf()->bluestore_debug_omit_block_device_write) {
                        bluestore_deferred_op_t* op = _get_deferred_op(txc, bl.length());
                        op->op = bluestore_deferred_op_t::OP_WRITE;
                        int r = b->get_blob().map(b_off, b_len, [&](uint64_t offset, uint64_t length) {
                            op->extents.emplace_back(bluestore_pextent_t(offset, length));
                            return 0;
                        });
                        ceph_assert(r == 0);
                        op->data = std::move(bl);
                        dout(20) << __func__ << "  deferred write 0x" << std::hex << b_off << "~" << b_len << std::dec
                                 << " of mutable " << *b << " at " << op->extents << dendl;
                    }
                ...
                if (b->can_reuse_blob(min_alloc_size, max_bsize, offset0 - bstart, &alloc_len)) {
                    ...
                    if (!wctx->has_conflict(b, offset0, offset0 + alloc_len, min_alloc_size)) {
                        ...
                        if (!cct->_conf->bluestore_zero_block_detection || !bl.is_zero()) {
                            ...
                            dout(20) << __func__ << " skip small zero block " << std::hex << " (0x" << b_off0 << "~"
                                     << bl.length() << ")"
                                     << " (0x" << b_off << "~" << length << ")" << std::dec << dendl;
                            logger->inc(l_bluestore_write_small_skipped);
                            logger->inc(l_bluestore_write_small_skipped_bytes, length);
    ...
    if (above_blob_threshold) {
        dout(10) << __func__ << " request GC, blobs >= " << inspected_blobs.size() << " " << std::hex << min_off << "~"
                 << max_off << std::dec << dendl;
        ceph_assert(start_ep != end_ep);
        for (auto ep = start_ep; ep != end_ep; ++ep) {
            dout(20) << __func__ << " inserting for GC " << std::hex << ep->logical_offset << "~" << ep->length
                     << std::dec << dendl;
...
bool BlueStore::BigDeferredWriteContext::can_defer(BlueStore::extent_map_t::iterator ep, uint64_t prefer_deferred_size,
                                                   uint64_t block_size, uint64_t offset, uint64_t l)
{
    ...
    if (offset >= ep->blob_start() && blob.is_mutable()) {
        ...
        res = blob_aligned_len() < prefer_deferred_size && blob_aligned_len() <= ondisk &&
...
bool BlueStore::BigDeferredWriteContext::apply_defer()
{
    int r = blob_ref->get_blob().map(
        b_off, blob_aligned_len(), [&](const bluestore_pextent_t& pext, uint64_t offset, uint64_t length) {
...
void BlueStore::_do_write_big_apply_deferred(TransContext* txc, CollectionRef& c, OnodeRef& o,
                                             BlueStore::BigDeferredWriteContext& dctx, bufferlist::iterator& blp,
                                             WriteContext* wctx)
{
    bufferlist bl;
    dout(20) << __func__ << "  reading head 0x" << std::hex << dctx.head_read << " and tail 0x" << dctx.tail_read
             << std::dec << dendl;
    if (dctx.head_read) {
        int r = _do_read(c.get(), o, dctx.off - dctx.head_read, dctx.head_read, bl, 0);
...
void BlueStore::_do_write_big(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length,
                              bufferlist::iterator& blp, WriteContext* wctx)
{
    ...
    while (length > 0) {
        ...
        if (!wctx->compress) {
            ...
            dout(20) << __func__ << " may be defer: 0x" << std::hex << offset << "~" << l << std::dec << dendl;
            ...
            if (prefer_deferred_size_snapshot && l <= prefer_deferred_size_snapshot * 2) {
                ...
                bool will_defer =
                    ep != end ? head_info.can_defer(ep, prefer_deferred_size_snapshot, block_size, offset, l) : false;
                    ...
                    if (remaining <= prefer_deferred_size_snapshot) {
                        ...
                        will_defer =
                            ep_next == end
                                ? false
                                : tail_info.can_defer(
                                      ep_next, prefer_deferred_size_snapshot, block_size, offset_next, remaining);
                        will_defer = will_defer && remaining == tail_info.used;
                    }
                }
                if (will_defer) {
                    dout(20) << __func__ << " " << *(head_info.blob_ref) << " deferring big " << std::hex << " (0x"
                             << head_info.b_off << "~" << head_info.blob_aligned_len() << ")" << std::dec
                             << " write via deferred" << dendl;
                    if (remaining) {
                        dout(20) << __func__ << " " << *(tail_info.blob_ref) << " deferring big " << std::hex << " (0x"
                                 << tail_info.b_off << "~" << tail_info.blob_aligned_len() << ")" << std::dec
                                 << " write via deferred" << dendl;
                    }
                ...
                if (will_defer) {
                    ...
                    dout(20) << __func__ << " defer big: 0x" << std::hex << offset << "~" << l << std::dec << dendl;
                    offset += l;
                    length -= l;
            ...
            do {
                any_change = false;
                if (ep != end && ep->logical_offset < offset + max_bsize) {
                    dout(20) << __func__ << " considering " << *ep << " bstart 0x" << std::hex << ep->blob_start()
                             << std::dec << dendl;

                    if (offset >= ep->blob_start() &&
                        ep->blob->can_reuse_blob(min_alloc_size, max_bsize, offset - ep->blob_start(), &l)) {
                        b = ep->blob;
                        b_off = offset - ep->blob_start();
                        prev_ep = end;   // to avoid check below
                        dout(20) << __func__ << " reuse blob " << *b << std::hex << " (0x" << b_off << "~" << l << ")"
                                 << std::dec << dendl;
                    }
                    else {
                        ++ep;
                ...
                if (prev_ep != end && prev_ep->logical_offset >= min_off) {
                    dout(20) << __func__ << " considering rev " << *prev_ep << " bstart 0x" << std::hex
                             << prev_ep->blob_start() << std::dec << dendl;
        ...
        if (!cct->_conf->bluestore_zero_block_detection || !t.is_zero()) {
            ...
            dout(20) << __func__ << " schedule write big: 0x" << std::hex << offset << "~" << l << std::dec
                     << (new_blob ? " new " : " reuse ") << *b << dendl;

            logger->inc(l_bluestore_write_big_blobs);
        }
        else {   // if (!t.is_zero())
            dout(20) << __func__ << " skip big zero block " << std::hex << " (0x" << b_off << "~" << t.length() << ")"
...
int BlueStore::_do_alloc_write(TransContext* txc, CollectionRef coll, OnodeRef& o, WriteContext* wctx)
{
    ...
    if (wctx->compress) {
        ...
        crr = select_option("compression_required_ratio", cct->_conf->bluestore_compression_required_ratio, [&]() {
            double val;
        ...
        if (c && wi.blob_length > min_alloc_size) {
            ...
            if (r == 0 && result_len <= want_len && result_len < wi.blob_length) {
                ...
                if (result_len <= want_len && result_len < wi.blob_length) {
                    ...
                    dout(20) << __func__ << std::hex << "  compressed 0x" << wi.blob_length << " -> 0x"
                             << compressed_len << " => 0x" << result_len << " with " << c->get_type() << std::dec
                             << dendl;
                ...
                dout(5) << __func__ << std::hex << "  0x" << wi.blob_length << " bytes compressed using "
                        << c->get_type_name() << std::dec << " failed with errcode = " << r << ", leaving uncompressed"
                        << dendl;
            ...
            if (rejected) {
                dout(20) << __func__ << std::hex << "  0x" << wi.blob_length << " compressed to 0x" << compressed_len
                         << " -> 0x" << result_len << " with " << c->get_type() << ", which is more than required 0x"
                         << want_len_raw << " -> 0x" << want_len << ", leaving uncompressed" << std::dec << dendl;
                logger->inc(l_bluestore_compress_rejected_count);
                need += wi.blob_length;
                data_size += wi.bl.length();
            }
    ...
    if (prealloc_left < 0 || prealloc_left < (int64_t)need) {
        derr << __func__ << " failed to allocate 0x" << std::hex << need << " allocated 0x "
             << (prealloc_left < 0 ? 0 : prealloc_left) << " min_alloc_size 0x" << min_alloc_size << " available 0x "
    ...
    dout(20) << __func__ << std::hex << " need=0x" << need << " data=0x" << data_size << " prealloc " << prealloc
             << dendl;
    ...
    for (auto& wi : wctx->writes) {
        ...
        if (wi.compressed) {
            ...
            if (csum != Checksummer::CSUM_NONE) {
                dout(20) << __func__ << " initialize csum setting for compressed blob " << *wi.b << " csum_type "
                         << Checksummer::get_csum_type_string(csum) << " csum_order " << csum_order << " csum_length 0x"
                         << std::hex << csum_length << " blob_length 0x" << wi.blob_length << " compressed_length 0x"
                         << wi.compressed_len << std::dec << dendl;
            ...
            if (csum != Checksummer::CSUM_NONE) {
                dout(20) << __func__ << " initialize csum setting for new blob " << *wi.b << " csum_type "
                         << Checksummer::get_csum_type_string(csum) << " csum_order " << csum_order << " csum_length 0x"
        ...
        Extent* le = o->extent_map.set_lextent(
            coll, wi.logical_offset, b_off + (wi.b_off0 - wi.b_off), wi.length0, wi.b, nullptr);
        ...
        if (!g_conf()->bluestore_debug_omit_block_device_write) {
            if (data_size < prefer_deferred_size_snapshot) {
                dout(20) << __func__ << " deferring 0x" << std::hex << l->length() << std::dec << " write via deferred"
                         << dendl;
                bluestore_deferred_op_t* op = _get_deferred_op(txc, l->length());
                op->op = bluestore_deferred_op_t::OP_WRITE;
                int r = wi.b->get_blob().map(b_off, l->length(), [&](uint64_t offset, uint64_t length) {
                    op->extents.emplace_back(bluestore_pextent_t(offset, length));
                    return 0;
                });
                ceph_assert(r == 0);
                ...
                wi.b->get_blob().map_bl(
                    b_off, *l, [&](uint64_t offset, bufferlist& t) { bdev->aio_write(offset, t, &txc->ioc, false); });
...
void BlueStore::_wctx_finish(TransContext* txc, CollectionRef& c, OnodeRef& o, WriteContext* wctx,
        ...
        for (auto& w : wctx->writes) {
            for (auto& e : w.b->get_blob().get_extents()) {
                ...
                if (!o->onode.zone_offset_refs.count(zone)) {
                    ...
                    dout(20) << __func__ << " add ref zone 0x" << std::hex << zone << " offset 0x" << zoff << std::dec
                             << dendl;
    ...
    if (!zones_with_releases.empty()) {
        ...
        for (auto zone : zones_with_releases) {
            ...
            if (p != o->onode.zone_offset_refs.end()) {
                dout(20) << __func__ << " rm ref zone 0x" << std::hex << zone << " offset 0x" << p->second << std::dec
                         << dendl;
...
void BlueStore::_do_write_data(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length,
...
void BlueStore::_choose_write_options(CollectionRef& c, OnodeRef& o, uint32_t fadvise_flags, WriteContext* wctx)
{
    if (fadvise_flags & CEPH_OSD_OP_FLAG_FADVISE_WILLNEED) {
        ...
        wctx->buffered = true;
             ...
             (fadvise_flags & (CEPH_OSD_OP_FLAG_FADVISE_DONTNEED | CEPH_OSD_OP_FLAG_FADVISE_NOCACHE)) == 0) {
    ...
    auto cm = select_option("compression_mode", comp_mode.load(), [&]() {
        ...
        if (c->pool_opts.get(pool_opts_t::COMPRESSION_MODE, &val)) {
            return std::optional<Compressor::CompressionMode>(Compressor::get_comp_mode_type(val));
        }
    ...
    wctx->compress =
        ...
        ((cm == Compressor::COMP_FORCE) ||
         (cm == Compressor::COMP_AGGRESSIVE && (alloc_hints & CEPH_OSD_ALLOC_HINT_FLAG_INCOMPRESSIBLE) == 0) ||
         (cm == Compressor::COMP_PASSIVE && (alloc_hints & CEPH_OSD_ALLOC_HINT_FLAG_COMPRESSIBLE)));
    ...
    if ((alloc_hints & CEPH_OSD_ALLOC_HINT_FLAG_SEQUENTIAL_READ) &&
        ...
        if (wctx->compress) {
            wctx->target_blob_size = select_option("compression_max_blob_size", comp_max_blob_size.load(), [&]() {
                int64_t val;
        ...
        if (wctx->compress) {
            wctx->target_blob_size = select_option("compression_min_blob_size", comp_min_blob_size.load(), [&]() {
                int64_t val;
    ...
    dout(20) << __func__ << " prefer csum_order " << wctx->csum_order << " target_blob_size 0x" << std::hex
             << wctx->target_blob_size << " compress=" << (int)wctx->compress << " buffered=" << (int)wctx->buffered
             << std::dec << dendl;
}

int BlueStore::_do_gc(TransContext* txc, CollectionRef& c, OnodeRef& o, const WriteContext& wctx, uint64_t* dirty_start,
                      uint64_t* dirty_end)
{
    ...
    for (auto it = extents_to_collect.begin(); it != extents_to_collect.end(); ++it) {
        bufferlist bl;
...
int BlueStore::_do_write(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length,
                         bufferlist& bl, uint32_t fadvise_flags)
{
    int r = 0;

    dout(20) << __func__ << " " << o->oid << " 0x" << std::hex << offset << "~" << length << " - have 0x"
             << o->onode.size << " (" << std::dec << o->onode.size << ")"
             << " bytes" << std::hex << " fadvise_flags 0x" << fadvise_flags << " alloc_hint 0x"
             << o->onode.alloc_hint_flags << " expected_object_size " << o->onode.expected_object_size
             << " expected_write_size " << o->onode.expected_write_size << std::dec << dendl;
    _dump_onode<30>(cct, *o);
    ...
    if (benefit >= g_conf()->bluestore_gc_enable_total_threshold) {
        ...
        dout(20) << __func__ << " perform garbage collection for compressed extents, "
    ...
    if (!wctx.extents_to_gc.empty()) {
        ...
        dout(20) << __func__ << " gc range is " << std::hex << dirty_start << "~" << dirty_end - dirty_start << std::dec
                 << dendl;
    }
...
int BlueStore::_write(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, size_t length, bufferlist& bl,
                      uint32_t fadvise_flags)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex << offset << "~" << length << std::dec
             << dendl;
    int r = 0;
    ...
    dout(10) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex << offset << "~" << length << std::dec
             << " = " << r << dendl;
    return r;
}

int BlueStore::_zero(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, size_t length)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex << offset << "~" << length << std::dec
             << dendl;
    int r = 0;
...
int BlueStore::_do_zero(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, size_t length)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex << offset << "~" << length << std::dec
             << dendl;
    int r = 0;
...
void BlueStore::_do_truncate(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset,
                             set<SharedBlob*>* maybe_unshared_blobs)
{
    ...
    if (offset < o->onode.size) {
        ...
        if (!o->onode.extent_map_shards.empty() && o->onode.extent_map_shards.back().offset >= offset) {
...
int BlueStore::_truncate(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex << offset << std::dec << dendl;
    ...
    log_latency_fn(__func__,
                   ...
                   [&](const ceph::timespan& lat) {
                       ostringstream ostr;
                       ostr << ", lat = " << timespan_str(lat) << " cid =" << c->cid << " oid =" << o->oid;
                       return ostr.str();
                   });
    dout(10) << __func__ << " " << c->cid << " " << o->oid << " 0x" << std::hex << offset << std::dec << " = " << r
             << dendl;
    return r;
}
...
int BlueStore::_do_remove(TransContext* txc, CollectionRef& c, OnodeRef& o)
{
    ...
    for (auto& s : o->extent_map.shards) {
        ...
        generate_extent_shard_key_and_apply(
            o->key, s.shard_info->offset, &key, [&](const string& final_key) { txc->t->rmkey(PREFIX_OBJ, final_key); });
    }
    ...
    for (auto& e : h->extent_map.extent_map) {
        const bluestore_blob_t& b = e.blob->get_blob();
        SharedBlob* sb = e.blob->shared_blob.get();
        if (b.is_shared() && std::find(unshared_blobs.begin(), unshared_blobs.end(), sb) != unshared_blobs.end()) {
...
int BlueStore::_setattr(TransContext* txc, CollectionRef& c, OnodeRef& o, const string& name, bufferptr& val)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << " " << name << " (" << val.length() << " bytes)" << dendl;
    int r = 0;
    if (val.is_partial()) {
        auto& b = o->onode.attrs[name.c_str()] = bufferptr(val.c_str(), val.length());
    ...
    txc->write_onode(o);
    dout(10) << __func__ << " " << c->cid << " " << o->oid << " " << name << " (" << val.length() << " bytes)"
             << " = " << r << dendl;
    return r;
}

int BlueStore::_setattrs(TransContext* txc, CollectionRef& c, OnodeRef& o, const map<string, bufferptr>& aset)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << " " << aset.size() << " keys" << dendl;
    int r = 0;
    for (map<string, bufferptr>::const_iterator p = aset.begin(); p != aset.end(); ++p) {
        if (p->second.is_partial()) {
            auto& b = o->onode.attrs[p->first.c_str()] = bufferptr(p->second.c_str(), p->second.length());
            b.reassign_to_mempool(mempool::mempool_bluestore_cache_meta);
        }
        else {
    ...
    txc->write_onode(o);
    dout(10) << __func__ << " " << c->cid << " " << o->oid << " " << aset.size() << " keys"
             << " = " << r << dendl;
    return r;
}


int BlueStore::_rmattr(TransContext* txc, CollectionRef& c, OnodeRef& o, const string& name)
{
...
void BlueStore::_do_omap_clear(TransContext* txc, OnodeRef& o)
{
    ...
    dout(20) << __func__ << " remove range start: " << pretty_binary_string(prefix)
...
int BlueStore::_omap_setkeys(TransContext* txc, CollectionRef& c, OnodeRef& o, bufferlist& bl)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
    int r;
    auto p = bl.cbegin();
...
int BlueStore::_omap_setheader(TransContext* txc, CollectionRef& c, OnodeRef& o, bufferlist& bl)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
    int r;
    string key;
...
int BlueStore::_omap_rmkeys(TransContext* txc, CollectionRef& c, OnodeRef& o, bufferlist& bl)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
    int r = 0;
    auto p = bl.cbegin();
...
int BlueStore::_omap_rmkey_range(TransContext* txc, CollectionRef& c, OnodeRef& o, const string& first,
                                 const string& last)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
    ...
    }
    {
        ...
        dout(20) << __func__ << " remove range start: " << pretty_binary_string(key_first)
...
int BlueStore::_set_alloc_hint(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t expected_object_size,
                               uint64_t expected_write_size, uint32_t flags)
{
    dout(15) << __func__ << " " << c->cid << " " << o->oid << " object_size " << expected_object_size << " write_size "
             << expected_write_size << " flags " << ceph_osd_alloc_hint_flag_string(flags) << dendl;
    int r = 0;
    o->onode.expected_object_size = expected_object_size;
    o->onode.expected_write_size = expected_write_size;
    o->onode.alloc_hint_flags = flags;
    txc->write_onode(o);
    dout(10) << __func__ << " " << c->cid << " " << o->oid << " object_size " << expected_object_size << " write_size "
             << expected_write_size << " flags " << ceph_osd_alloc_hint_flag_string(flags) << " = " << r << dendl;
    return r;
}

int BlueStore::_clone(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo)
{
...
int BlueStore::_do_clone_range(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo, uint64_t srcoff,
                               uint64_t length, uint64_t dstoff)
{
    dout(15) << __func__ << " " << c->cid << " " << oldo->oid << " -> " << newo->oid << " 0x" << std::hex << srcoff
             << "~" << length << " -> "
    ...
    if (bdev->is_smr()) {
        ...
        Extent dummy(dstoff);
        for (auto e = newo->extent_map.extent_map.lower_bound(dummy); e != newo->extent_map.extent_map.end(); ++e) {
...
int BlueStore::_clone_range(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo, uint64_t srcoff,
                            uint64_t length, uint64_t dstoff)
{
    dout(15) << __func__ << " " << c->cid << " " << oldo->oid << " -> " << newo->oid << " from 0x" << std::hex << srcoff
             << "~" << length << " to offset 0x" << dstoff << std::dec << dendl;
    int r = 0;
...
out:
    dout(10) << __func__ << " " << c->cid << " " << oldo->oid << " -> " << newo->oid << " from 0x" << std::hex << srcoff
             << "~" << length << " to offset 0x" << dstoff << std::dec << " = " << r << dendl;
    return r;
}

int BlueStore::_rename(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo, const ghobject_t& new_oid)
{
    ...
    // rewrite shards
    {
        ...
        for (auto& s : oldo->extent_map.shards) {
            generate_extent_shard_key_and_apply(oldo->key, s.shard_info->offset, &key, [&](const string& final_key) {
    ...
    if (bdev->is_smr()) {
        ...
        for (auto& [zone, offset] : newo->onode.zone_offset_refs) {
            dout(20) << __func__ << " rm ref zone 0x" << std::hex << zone << " offset 0x" << offset << std::dec
                     << " -> " << oldo->oid << dendl;
            string key;
            get_zone_offset_object_key(zone, offset, oldo->oid, &key);
            txc->t->rmkey(PREFIX_ZONED_CL_INFO, key);

            dout(20) << __func__ << " add ref zone 0x" << std::hex << zone << " offset 0x" << offset << std::dec
                     << " -> " << newo->oid << dendl;
...
int BlueStore::_remove_collection(TransContext* txc, const coll_t& cid, CollectionRef* c)
{
    ...
    (*c)->flush_all_but_last();
    {
        ...
        r = _collection_list(c->get(), ghobject_t(), ghobject_t::get_max(), nonexistent_count + 1, false, &ls, &next);
        if (r >= 0) {
            ...
            for (auto it = ls.begin(); !exists && it < ls.end(); ++it) {
                ...
                if (exists) {
                    dout(1) << __func__ << " " << *it << " exists in db, "
                            << (!onode ? "not present in ram" : "present in ram") << dendl;
                }
            }
            if (!exists) {
...
int BlueStore::_split_collection(TransContext* txc, CollectionRef& c, CollectionRef& d, unsigned bits, int rem)
{
...
int BlueStore::_merge_collection(TransContext* txc, CollectionRef* c, CollectionRef& d, unsigned bits)
{
...
void BlueStore::log_latency(const char* name, int idx, const ceph::timespan& l, double lat_threshold,
...
void BlueStore::log_latency_fn(const char* name, int idx, const ceph::timespan& l, double lat_threshold,
...
#if defined(WITH_LTTNG)
void BlueStore::BlueStoreThrottle::emit_initial_tracepoint(KeyValueDB& db, TransContext& txc,
                                                           mono_clock::time_point start_throttle_acquire)
{
    ...
    if (should_trace(&started, &completed)) {
        txc.tracing = true;
        uint64_t rocksdb_base_level, rocksdb_estimate_pending_compaction_bytes, rocksdb_cur_size_all_mem_tables,
            rocksdb_compaction_pending, rocksdb_mem_table_flush_pending, rocksdb_num_running_compactions,
            rocksdb_num_running_flushes, rocksdb_actual_delayed_write_rate;
        db.get_property("rocksdb.base-level", &rocksdb_base_level);
        db.get_property("rocksdb.estimate-pending-compaction-bytes", &rocksdb_estimate_pending_compaction_bytes);
        db.get_property("rocksdb.cur-size-all-mem-tables", &rocksdb_cur_size_all_mem_tables);
        db.get_property("rocksdb.compaction-pending", &rocksdb_compaction_pending);
        db.get_property("rocksdb.mem-table-flush-pending", &rocksdb_mem_table_flush_pending);
        db.get_property("rocksdb.num-running-compactions", &rocksdb_num_running_compactions);
        db.get_property("rocksdb.num-running-flushes", &rocksdb_num_running_flushes);
        db.get_property("rocksdb.actual-delayed-write-rate", &rocksdb_actual_delayed_write_rate);
...
mono_clock::duration BlueStore::BlueStoreThrottle::log_state_latency(TransContext& txc, PerfCounters* logger, int state)
{
    mono_clock::time_point now = mono_clock::now();
    mono_clock::duration lat = now - txc.last_stamp;
    logger->tinc(state, lat);
#if defined(WITH_LTTNG)
    if (txc.tracing && state >= l_bluestore_state_prepare_lat && state <= l_bluestore_state_done_lat) {
...
bool BlueStore::BlueStoreThrottle::try_start_transaction(KeyValueDB& db, TransContext& txc,
                                                         mono_clock::time_point start_throttle_acquire)
{
...
void BlueStore::BlueStoreThrottle::finish_start_transaction(KeyValueDB& db, TransContext& txc,
                                                            mono_clock::time_point start_throttle_acquire)
{
...
void BlueStore::BlueStoreThrottle::complete(TransContext& txc)
{
    ...
    if (txc.tracing) {
        ...
        tracepoint(
            bluestore, transaction_total_duration, txc.osr->get_sequencer_id(), txc.seq, ceph::to_seconds<double>(lat));
    }
}
#endif
...
void BlueStore::_apply_padding(uint64_t head_pad, uint64_t tail_pad, bufferlist& padded)
{
    if (head_pad) {
    ...
    if (head_pad || tail_pad) {
        dout(20) << __func__ << "  can pad head 0x" << std::hex << head_pad << " tail 0x" << tail_pad << std::dec
                 << dendl;
...
void BlueStore::_record_onode(OnodeRef& o, KeyValueDB::Transaction& txn)
{
    ...
    dout(20) << __func__ << " onode " << o->oid << " is " << bl.length() << " (" << onode_part << " bytes onode + "
             << blob_part << " bytes spanning blobs + " << extent_part << " bytes inline extents)" << dendl;
...
{
    std::lock_guard l(qlock);
    size_t used = bluefs && bluefs_layout.shared_bdev == BlueFS::BDEV_SLOW ? bluefs->get_used(BlueFS::BDEV_SLOW) : 0;
    if (used > 0) {
        auto db_used = bluefs->get_used(BlueFS::BDEV_DB);
        auto db_total = bluefs->get_total(BlueFS::BDEV_DB);
        ostringstream ss;
        ss << "spilled over " << byte_u_t(used) << " metadata from 'db' device (" << byte_u_t(db_used) << " used of "
           << byte_u_t(db_total) << ") to slow device";
    ...
    if (!spurious_read_errors_alert.empty() && cct->_conf->bluestore_warn_on_spurious_read_errors) {
        alerts.emplace("BLUESTORE_SPURIOUS_READ_ERRORS", spurious_read_errors_alert);
    }
    ...
    if (!spillover_alert.empty() && cct->_conf->bluestore_warn_on_bluefs_spillover) {
...
void BlueStore::_collect_allocation_stats(uint64_t need, uint32_t alloc_size, const PExtentVector& extents)
{
    alloc_stats_count++;
...
void BlueStore::_record_allocation_stats()
{
    ...
    auto t0 =
        std::make_tuple(alloc_stats_count.exchange(0), alloc_stats_fragments.exchange(0), alloc_stats_size.exchange(0));

    dout(0) << " allocation stats probe " << probe_count << ":"
            << " cnt: " << std::get<0>(t0) << " frags: " << std::get<1>(t0) << " size: " << std::get<2>(t0) << dendl;
    ...
    for (auto& t : alloc_stats_history) {
        dout(0) << " probe -" << base + (probe_count % base) << ": " << std::get<0>(t) << ",  " << std::get<1>(t)
                << ", " << std::get<2>(t) << dendl;
        base <<= 1;
    }
    dout(0) << "------------" << dendl;
...
size_t BlueStoreRepairer::StoreSpaceTracker::filter_out(const interval_set<uint64_t>& extents)
{
    ...
    for (auto e : extents) {
        ...
        while (pos != npos && pos < end_pos) {
            ceph_assert(collections_bfs[pos].element_count() == objects_bfs[pos].element_count());
...
bool BlueStoreRepairer::remove_key(KeyValueDB* db, const string& prefix, const string& key)
{
    std::lock_guard l(lock);
    if (!remove_key_txn) {
...
bool BlueStoreRepairer::fix_shared_blob(KeyValueDB::Transaction txn, uint64_t sbid, bluestore_extent_ref_map_t* ref_map,
                                        size_t repaired)
{
    string key;
    get_shared_blob_key(sbid, &key);
...
bool BlueStoreRepairer::fix_statfs(KeyValueDB* db, const string& key, const store_statfs_t& new_statfs)
{
    std::lock_guard l(lock);
    if (!fix_statfs_txn) {
...
bool BlueStoreRepairer::fix_leaked(KeyValueDB* db, FreelistManager* fm, uint64_t offset, uint64_t len)
{
    std::lock_guard l(lock);
    ceph_assert(!fm->is_null_manager());
...
bool BlueStoreRepairer::fix_false_free(KeyValueDB* db, FreelistManager* fm, uint64_t offset, uint64_t len)
{
    std::lock_guard l(lock);
    ceph_assert(!fm->is_null_manager());
...
bool BlueStoreRepairer::fix_spanning_blobs(KeyValueDB* db, std::function<void(KeyValueDB::Transaction)> f)
{
    std::lock_guard l(lock);
    if (!fix_onode_txn) {
...
uint8_t RocksDBBlueFSVolumeSelector::select_prefer_bdev(void* h)
{
    ...
    case LEVEL_SLOW:
        ...
        if (db_avail4slow > 0) {
            ...
            uint64_t avail = min(db_avail4slow, max_db_use < db_total ? db_total - max_db_use : 0);
        ...
        break;
    case LEVEL_LOG:
    case LEVEL_WAL: res = BlueFS::BDEV_WAL; break;
    case LEVEL_DB:
    default: res = BlueFS::BDEV_DB; break;
    }
    return res;
}
...
void RocksDBBlueFSVolumeSelector::dump(ostream& sout)
{
    ...
    for (size_t l = 0; l < max_y; l++) {
        ...
        case LEVEL_LOG: sout << "LOG"; break;
        case LEVEL_WAL: sout << "WAL"; break;
        case LEVEL_DB: sout << "DB"; break;
        case LEVEL_SLOW: sout << "SLOW"; break;
        case LEVEL_MAX: sout << "TOTAL"; break;
        }
...
{
    RocksDBBlueFSVolumeSelector* ns = new RocksDBBlueFSVolumeSelector(0, 0, 0, 0, 0, 0, 0, 0, false);
    return ns;
}
...
//
// We changed the code skiping RocksDB updates on allocation time and instead performing a full desatge of the allocator
// object with all the OSD allocation state in a single step during umount(). This change leads to a 25% increase in
// IOPS and reduced latency in small random-write workload, but exposes the system to losing allocation info in failure
// cases where we don't call umount. We add code to perform a full allocation-map rebuild from information stored inside
// the ONode which is used in failure cases. When we perform a graceful shutdown there is no need for recovery and we
// simply read the allocation-map from a flat file where we store the allocation-map during umount().
//================================================================================================================
...
struct allocator_image_header
{
    ...
    allocator_image_header() { memset((char*)this, 0, sizeof(allocator_image_header)); }
    ...
    int verify(CephContext* cct, const std::string& path)
    {
        if (valid_signature == ALLOCATOR_IMAGE_VALID_SIGNATURE) {
            ...
            derr << "Illegal Header - signature=" << valid_signature << "(" << ALLOCATOR_IMAGE_VALID_SIGNATURE << ")"
                 << dendl;
            return -1;
        }
    }
...
struct allocator_image_trailer
{
    ...
    allocator_image_trailer(utime_t timestamp, uint32_t format_version, uint32_t serial, uint64_t entries_count,
    ...
    allocator_image_trailer() { memset((char*)this, 0, sizeof(allocator_image_trailer)); }
    ...
    int verify(CephContext* cct, const std::string& path, const allocator_image_header* p_header,
               uint64_t entries_count, uint64_t allocation_size)
    {
        if (valid_signature == ALLOCATOR_IMAGE_VALID_SIGNATURE) {
            ...
            if (null_extent.offset || null_extent.length) {
                derr << "illegal trailer - null_extent = [" << null_extent.offset << "," << null_extent.length << "]"
                     << dendl;
                return -1;
            }

            if (serial != p_header->serial) {
                derr << "Illegal trailer: header->serial(" << p_header->serial << ") != trailer->serial(" << serial
                     << ")" << dendl;
                return -1;
            }
            ...
            if (timestamp != p_header->timestamp) {
                derr << "Illegal trailer: header->timestamp(" << p_header->timestamp << ") != trailer->timestamp("
                     << timestamp << ")" << dendl;
                return -1;
            }
            ...
            derr << "Illegal Trailer - signature=" << valid_signature << "(" << ALLOCATOR_IMAGE_VALID_SIGNATURE << ")"
                 << dendl;
            return -1;
        }
    }
...
int BlueStore::invalidate_allocation_file_on_bluefs()
{
    ...
    if (ret != 0) {
        derr << __func__ << "::NCB:: Failed truncaste with error-code " << ret << dendl;
        bluefs->close_writer(p_handle);
        return -1;
    }
...
{
    *p_num_entries = 0;
    auto count_entries = [&](uint64_t extent_offset, uint64_t extent_length) { (*p_num_entries)++; };
...
//-----------------------------------------------------------------------------------
static uint32_t flush_extent_buffer_with_crc(BlueFS::FileWriter* p_handle, const char* buffer, const char* p_curr,
                                             uint32_t crc)
{
...
int BlueStore::store_allocator(Allocator* src_allocator)
{
    ...
    utime_t duration = ceph_clock_now() - start_time;
    dout(5) << "WRITE-extent_count=" << extent_count << ", allocation_size=" << allocation_size
            << ", serial=" << s_serial << dendl;
...
Allocator* BlueStore::create_bitmap_allocator(uint64_t bdev_size)
{
    ...
    uint64_t alloc_size = min_alloc_size;
    Allocator* alloc =
        Allocator::create(cct, "bitmap", bdev_size, alloc_size, zone_size, first_sequential_zone, "recovery");
    if (alloc) {
        return alloc;
    }
    else {
...
int BlueStore::__restore_allocator(Allocator* allocator, uint64_t* num, uint64_t* bytes)
{
    ...
    int header_size = calc_allocator_image_header_size();
    {
        ...
        if (read_bytes != header_size) {
            derr << "Failed bluefs->read() for header::read_bytes=" << read_bytes << ", req_bytes=" << header_size
                 << dendl;
            return -1;
        }
        ...
        if (read_bytes == sizeof(crc)) {
            ...
            if (crc != calc_crc) {
                ...
                derr << "extents_bytes_left=" << extents_bytes_left << ", offset=" << offset
                     << ", extent_count=" << extent_count << dendl;
                return -1;
            }
            ...
            derr << "Failed bluefs->read() for crc::read_bytes=" << read_bytes << ", req_bytes=" << sizeof(crc)
                 << dendl;
            return -1;
        }
    }
    ...
    // finally, read the trailer and verify it is in good shape and that we got all the extents
    {
        ...
        if (read_bytes != trailer_size) {
            derr << "Failed bluefs->read() for trailer::read_bytes=" << read_bytes << ", req_bytes=" << trailer_size
                 << dendl;
            return -1;
        }
        offset += read_bytes;
    ...
    utime_t duration = ceph_clock_now() - start_time;
    dout(5) << "READ--extent_count=" << extent_count << ", read_alloc_size=  " << read_alloc_size
...
{
    dout(30) << __func__ << " 0x" << std::hex << offset << "~" << length << " " << min_alloc_size_mask << dendl;
...
void BlueStore::ExtentDecoderPartial::_consume_new_blob(bool spanning, uint64_t extent_no, uint64_t sbid, BlobRef b)
{
...
void BlueStore::ExtentDecoderPartial::consume_blobid(Extent* le, bool spanning, uint64_t blobid)
{
    [[maybe_unused]] auto cct = store.cct;
...
void BlueStore::ExtentDecoderPartial::consume_blob(Extent* le, uint64_t extent_no, uint64_t sbid, BlobRef b)
{
...
void BlueStore::ExtentDecoderPartial::consume_spanning_blob(uint64_t sbid, BlobRef b)
{
    _consume_new_blob(true, 0 /*doesn't matter*/, sbid, b);
}

void BlueStore::ExtentDecoderPartial::reset(const ghobject_t _oid, volatile_statfs* _per_pool_statfs)
{
    oid = _oid;
...
int BlueStore::read_allocation_from_onodes(SimpleBitmap* sbmap, read_alloc_stats_t& stats)
{
    ...
    if (it) {
        for (it->lower_bound(string()); it->valid(); it->next()) {
            ...
            uint64_t sbid = 0;
            if (get_key_shared_blob(key, &sbid) != 0) {
                derr << __func__ << " bad shared blob key '" << pretty_binary_string(key) << "'" << dendl;
            }
            bluestore_shared_blob_t shared_blob(sbid);
            bufferlist bl = it->value();
            auto blp = bl.cbegin();
            try {
                ...
                derr << __func__ << " failed to decode Shared Blob" << pretty_binary_string(key) << dendl;
                continue;
            }
            dout(20) << __func__ << "  " << shared_blob << dendl;
    ...
    ExtentDecoderPartial edecoder(*this, stats, *sbmap, sb_info, min_alloc_size_order);
    ...
    for (it->lower_bound(string()); it->valid(); it->next(), kv_count++) {
        ...
        if (!is_extent_shard_key(it->key())) {
            ...
            if (r != 0) {
                derr << __func__ << " failed to decode onode key = " << pretty_binary_string(okey) << dendl;
                return -EIO;
            }
            edecoder.reset(oid, &stats.actual_pool_vstatfs[oid.hobj.get_logical_pool()]);
            Onode dummy_on(cct);
            ...
            if (r != 0) {
                derr << __func__ << " failed to decode onode extent key = " << pretty_binary_string(key) << dendl;
                return -EIO;
            }
            r = get_key_object(okey, &oid);
            if (r != 0) {
                derr << __func__ << " failed to decode onode key= " << pretty_binary_string(okey)
    ...
    for (auto& p : stats.actual_pool_vstatfs) {
        ...
        p.second.publish(&s);
        dout(5) << __func__ << " recovered pool " << std::hex << p.first << "->" << s << std::dec
                << " per-pool:" << per_pool_stat_collection << dendl;
    }
...
//---------------------------------------------------------
int BlueStore::compare_allocators(Allocator* alloc1, Allocator* alloc2, uint64_t req_extent_count,
...
int BlueStore::read_allocation_from_drive_for_bluestore_tool()
{
    ...
    auto shutdown_cache = make_scope_guard([&] {
        dout(1) << "Allocation Recovery was completed in " << duration
                << " seconds; insert_count=" << stats.insert_count << "; extent_count=" << stats.extent_count << dendl;
        _shutdown_cache();
        _close_db_and_around();
    });

    {
        ...
        auto count_entries = [&](uint64_t extent_offset, uint64_t extent_length) { stats.insert_count++; };
...
Allocator* BlueStore::clone_allocator_without_bluefs(Allocator* src_allocator)
{
    ...
    {
        bluefs->foreach_block_extents(bluefs_layout.shared_bdev,
                                      [&](uint64_t start, uint32_t len) { allocator->init_add_free(start, len); });
    }
...
int BlueStore::verify_rocksdb_allocations(Allocator* allocator)
{
    ...
    auto count_entries = [&](uint64_t extent_offset, uint64_t extent_length) { insert_count++; };
