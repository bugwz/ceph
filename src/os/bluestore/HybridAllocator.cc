...
int64_t HybridAllocator::allocate(uint64_t want, uint64_t unit, uint64_t max_alloc_size, int64_t hint,
                                  PExtentVector* extents)
{
    ...
    if (bmap_alloc && bmap_alloc->get_free() && want < _lowest_size_available()) {
        ...
        if ((uint64_t)res < want) {
            auto res2 = bmap_alloc ? bmap_alloc->allocate(want - res, unit, max_alloc_size, hint, extents) : 0;
            if (res2 < 0) {
                res = res2;   // caller to do the release
            }
            else {
...
void HybridAllocator::dump()
{
    ...
    ldout(cct, 0) << __func__ << " avl_free: " << _get_free()
                  << " bmap_free: " << (bmap_alloc ? bmap_alloc->get_free() : 0) << dendl;
}

void HybridAllocator::foreach (std::function<void(uint64_t offset, uint64_t length)> notify)
{
    std::lock_guard l(lock);
...
void HybridAllocator::init_rm_free(uint64_t offset, uint64_t length)
{
    ...
    _try_remove_from_tree(offset, length, [&](uint64_t o, uint64_t l, bool found) {
        if (!found) {
            if (bmap_alloc) {
                bmap_alloc->init_rm_free(o, l);
            }
            else {
                lderr(cct) << "init_rm_free lambda " << std::hex << "Uexpected extent: "
                           << " 0x" << o << "~" << l << std::dec << dendl;
                ceph_assert(false);
            }
        }
    });
...
void HybridAllocator::_spillover_range(uint64_t start, uint64_t end)
{
    ...
    if (!bmap_alloc) {
        ...
        bmap_alloc = new BitmapAllocator(cct, get_capacity(), get_block_size(), get_name() + ".fallback");
    }
