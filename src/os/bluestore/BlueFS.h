...
public:
    ...
    typedef boost::intrusive::list<
        File, boost::intrusive::member_hook<File, boost::intrusive::list_member_hook<>, &File::dirty_item>>
        dirty_file_list_t;
    ...
    public:
        ...
        ceph::bufferlist flush_buffer(CephContext* cct, const bool partial, const unsigned length,
    ...
    public:
        ...
        FileWriter(FileRef f)
            : file(std::move(f))
            , buffer_appender(buffer.get_page_aligned_appender(g_conf()->bluefs_alloc_size / CEPH_PAGE_SIZE))
        {
            ++file->num_writers;
            iocv.fill(nullptr);
        ...
        uint64_t get_effective_write_pos() { return pos + buffer.length(); }
    };
    ...
    struct FileReaderBuffer
    {
        ...
        {
            if (p >= bl_off && p < bl_off + bl.length()) return bl_off + bl.length() - p;
            return 0;
        }
    ...
    struct FileReader
    {
        ...
        ceph::shared_mutex lock{ceph::make_shared_mutex(std::string(), false, false, false)};
...
private:
    ...
    inline bool is_shared_alloc(unsigned id) const { return id == shared_alloc_id; }
    ...
    unsigned _get_slow_device_id() { return bdev[BDEV_SLOW] ? BDEV_SLOW : BDEV_DB; }
    const char* get_device_name(unsigned id);
    int _allocate(uint8_t bdev, uint64_t len, uint64_t alloc_unit, bluefs_fnode_t* node, size_t alloc_attempts = 0,
    ...
    int _flush_and_sync_log_jump_D(uint64_t jump_to, int64_t available_runway);
    int _flush_and_sync_log_LD(uint64_t want_seq = 0);

    uint64_t _estimate_transaction_size(bluefs_transaction_t* t);
    uint64_t _make_initial_transaction(uint64_t start_seq, bluefs_fnode_t& fnode, uint64_t expected_final_size,
                                       bufferlist* out);
    ...
    void _compact_log_dump_metadata_NF(uint64_t start_seq, bluefs_transaction_t* t, int flags,
                                       uint64_t capture_before_seq);

    void _compact_log_sync_LNF_LD();
    void _compact_log_async_LD_LNF_D();

    void _rewrite_log_and_layout_sync_LNF_LD(bool permit_dev_fallback, int super_dev, int log_dev, int new_log_dev,
    ...
    int64_t _read(FileReader* h,               ///< [in] read from here
                  uint64_t offset,             ///< [in] offset
                  size_t len,                  ///< [in] this many bytes
                  ceph::buffer::list* outbl,   ///< [out] optional: reference the result here
                  char* out);                  ///< [out] optional: or copy it here
    int64_t _read_random(FileReader* h,        ///< [in] read from here
                         uint64_t offset,      ///< [in] offset
    ...
    int _check_allocations(const bluefs_fnode_t& fnode, boost::dynamic_bitset<uint64_t>* used_blocks,
                           bool is_alloc,   // true when allocating, false when deallocating
                           const char* op_name);
    int _verify_alloc_granularity(__u8 id, uint64_t offset, uint64_t length, uint64_t alloc_unit, const char* op);
...
public:
    ...
    int device_migrate_to_new(CephContext* cct, const std::set<int>& devs_source, int dev_target,
                              const bluefs_layout_t& layout);
    int device_migrate_to_existing(CephContext* cct, const std::set<int>& devs_source, int dev_target,
                                   const bluefs_layout_t& layout);
    ...
    void foreach_block_extents(unsigned id, std::function<void(uint64_t, uint32_t)> cb);

    int open_for_write(std::string_view dir, std::string_view file, FileWriter** h, bool overwrite);

    int open_for_read(std::string_view dir, std::string_view file, FileReader** h, bool random = false);

    // data added after last fsync() is lost
    void close_writer(FileWriter* h);

    int rename(std::string_view old_dir, std::string_view old_file, std::string_view new_dir,
    ...
    int stat(std::string_view dirname, std::string_view filename, uint64_t* size, utime_t* mtime);
    ...
    void dump_volume_selector(std::ostream& sout) { vselector->dump(sout); }
    void get_vselector_paths(const std::string& base, BlueFSVolumeSelector::paths& res) const
    {
        return vselector->get_paths(base, res);
    }

    int add_block_device(unsigned bdev, const std::string& path, bool trim, uint64_t reserved,
    ...
    int64_t read(FileReader* h, uint64_t offset, size_t len, ceph::buffer::list* outbl, char* out)
    {
...
private:
    ...
    int _read_and_check(uint8_t ndev, uint64_t off, uint64_t len, ceph::buffer::list* pbl, IOContext* ioc,
                        bool buffered);
    int _read_random_and_check(uint8_t ndev, uint64_t off, uint64_t len, char* buf, bool buffered);

    int _bdev_read(uint8_t ndev, uint64_t off, uint64_t len, ceph::buffer::list* pbl, IOContext* ioc, bool buffered);
    ...
    int _do_replay_recovery_read(FileReader* log, size_t log_pos, size_t read_offset, size_t read_len, bufferlist* bl);
...
public:
    OriginalVolumeSelector(uint64_t _wal_total, uint64_t _db_total, uint64_t _slow_total)
        : wal_total(_wal_total)
        , db_total(_db_total)
...
{
public:
    FitToFastVolumeSelector(uint64_t _wal_total, uint64_t _db_total, uint64_t _slow_total)
