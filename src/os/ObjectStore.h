...
public:
     ...
     */
#ifndef WITH_SEASTAR
    static std::unique_ptr<ObjectStore> create(CephContext* cct, const std::string& type, const std::string& data,
                                               const std::string& journal, osflagbits_t flags = 0);
#endif
    static std::unique_ptr<ObjectStore> create(CephContext* cct, const std::string& type, const std::string& data);
     ...
     */
    static int probe_block_device_fsid(CephContext* cct, const std::string& path, uuid_d* fsid);
    ...
    int queue_transaction(CollectionHandle& ch, Transaction&& t, TrackedOpRef op = TrackedOpRef(),
    ...
    virtual int queue_transactions(CollectionHandle& ch, std::vector<Transaction>& tls,
                                   TrackedOpRef op = TrackedOpRef(), ThreadPool::TPHandle* handle = NULL) = 0;
...
public:
    ...
    virtual int fsck(bool deep) { return -EOPNOTSUPP; }
    virtual int repair(bool deep) { return -EOPNOTSUPP; }
    virtual int quick_fix() { return -EOPNOTSUPP; }
    ...
    virtual uint64_t get_min_alloc_size() const { return 0; }

    /// enumerate hardware devices (by 'devname', e.g., 'sda' as in /sys/block/sda)
    virtual int get_devices(std::set<std::string>* devls) { return -EOPNOTSUPP; }

    /// true if a txn is readable immediately after it is queued.
    virtual bool is_sync_onreadable() const { return true; }
     ...
     */
    virtual bool is_rotational() { return true; }
     ...
     * @return true for HDD, false for SSD
     */
    virtual bool is_journal_rotational() { return true; }

    virtual std::string get_default_device_class() { return is_rotational() ? "hdd" : "ssd"; }

    virtual int get_numa_node(int* numa_node, std::set<int>* nodes, std::set<std::string>* failed)
    {
        return -EOPNOTSUPP;
    }
    ...
    virtual int statfs(struct store_statfs_t* buf, osd_alert_list_t* alerts = nullptr) = 0;
     ...
     */
    virtual int set_collection_opts(CollectionHandle& c, const pool_opts_t& opts) = 0;
     ...
     * @returns 0 on success, negative error code on failure.
     */
    virtual int stat(CollectionHandle& c, const ghobject_t& oid, struct stat* st, bool allow_eio = false) = 0;
    /**
     ...
     */
    virtual int read(CollectionHandle& c, const ghobject_t& oid, uint64_t offset, size_t len, ceph::buffer::list& bl,
     ...
     * @returns 0 on success, negative error code on failure.
     */
    virtual int fiemap(CollectionHandle& c, const ghobject_t& oid, uint64_t offset, size_t len,
     ...
     * @returns number of bytes read on success, or negative error code on failure.
     */
    virtual int readv(CollectionHandle& c, const ghobject_t& oid, interval_set<uint64_t>& m, ceph::buffer::list& bl,
                      uint32_t op_flags = 0)
    {
        int total = 0;
     ...
     * @returns 0 on success, negative error code on failure.
     */
    virtual int dump_onode(CollectionHandle& c, const ghobject_t& oid, const std::string& section_name,
                           ceph::Formatter* f)
    {
        return -ENOTSUP;
    }
     ...
     * @returns 0 on success, negative error code on failure.
     */
    virtual int getattr(CollectionHandle& c, const ghobject_t& oid, const char* name, ceph::buffer::ptr& value) = 0;
     ...
     * @returns 0 on success, negative error code on failure.
     */
    int getattr(CollectionHandle& c, const ghobject_t& oid, const std::string& name, ceph::buffer::list& value)
    {
        ceph::buffer::ptr bp;
     ...
     */
    virtual int collection_list(CollectionHandle& c, const ghobject_t& start, const ghobject_t& end, int max,
                                std::vector<ghobject_t>* ls, ghobject_t* next) = 0;

    virtual int collection_list_legacy(CollectionHandle& c, const ghobject_t& start, const ghobject_t& end, int max,
    ...
    /// OMAP
    /// Get omap contents
    virtual int omap_get(CollectionHandle& c,                             ///< [in] Collection containing oid
    ...
    /// Get omap header
    virtual int omap_get_header(CollectionHandle& c,          ///< [in] Collection containing oid
                                const ghobject_t& oid,        ///< [in] Object containing omap
                                ceph::buffer::list* header,   ///< [out] omap header
    ...
    virtual int omap_get_keys(CollectionHandle& c,          ///< [in] Collection containing oid
                              const ghobject_t& oid,        ///< [in] Object containing omap
                              std::set<std::string>* keys   ///< [out] Keys defined on oid
                              ) = 0;

    /// Get key values
    virtual int omap_get_values(CollectionHandle& c,                             ///< [in] Collection containing oid
                                const ghobject_t& oid,                           ///< [in] Object containing omap
    ...
    virtual int omap_check_keys(CollectionHandle& c,                 ///< [in] Collection containing oid
                                const ghobject_t& oid,               ///< [in] Object containing omap
     ...
     */
    virtual ObjectMap::ObjectMapIterator get_omap_iterator(CollectionHandle& c,    ///< [in] collection
    ...
    /**
     * Estimates additional disk space used by the specified amount of objects and caused by file allocation granularity
     * and metadata store
    ...
    virtual bool has_builtin_csum() const { return false; }
};
