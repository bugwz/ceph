...
bool operator>(const MemStore::CollectionRef& l, const MemStore::CollectionRef& r)
{
...
void MemStore::dump(ceph::Formatter* f)
{
    ...
    for (auto p = coll_map.begin(); p != coll_map.end(); ++p) {
        ...
        for (auto q = p->second->xattr.begin(); q != p->second->xattr.end(); ++q) {
        ...
        for (auto q = p->second->object_map.begin(); q != p->second->object_map.end(); ++q) {
            f->open_object_section("object");
            f->dump_string("name", stringify(q->first));
            if (q->second) q->second->dump(f);
            f->close_section();
        }
        f->close_section();
...
int MemStore::_load()
{
    ...
    for (auto q = collections.begin(); q != collections.end(); ++q) {
...
int MemStore::pool_statfs(uint64_t pool_id, struct store_statfs_t* buf, bool* per_pool_omap)
{
    return -ENOTSUP;
}
...
int MemStore::stat(CollectionHandle& c_, const ghobject_t& oid, struct stat* st, bool allow_eio)
{
    Collection* c = static_cast<Collection*>(c_.get());
...
int MemStore::read(CollectionHandle& c_, const ghobject_t& oid, uint64_t offset, size_t len, ceph::buffer::list& bl,
                   uint32_t op_flags)
{
    Collection* c = static_cast<Collection*>(c_.get());
...
int MemStore::fiemap(CollectionHandle& ch, const ghobject_t& oid, uint64_t offset, size_t len, ceph::buffer::list& bl)
{
    std::map<uint64_t, uint64_t> destmap;
    int r = fiemap(ch, oid, offset, len, destmap);
    if (r >= 0) encode(destmap, bl);
    return r;
}

int MemStore::fiemap(CollectionHandle& ch, const ghobject_t& oid, uint64_t offset, size_t len,
                     std::map<uint64_t, uint64_t>& destmap)
{
    ...
    if (!o) return -ENOENT;
    size_t l = len;
    if (offset + l > o->get_size()) l = o->get_size() - offset;
    if (offset >= o->get_size()) goto out;
    destmap[offset] = l;
out:
    return 0;
}

int MemStore::getattr(CollectionHandle& c_, const ghobject_t& oid, const char* name, ceph::buffer::ptr& value)
{
    Collection* c = static_cast<Collection*>(c_.get());
...
int MemStore::getattrs(CollectionHandle& c_, const ghobject_t& oid, std::map<std::string, ceph::buffer::ptr>& aset)
{
    Collection* c = static_cast<Collection*>(c_.get());
    dout(10) << __func__ << " " << c->cid << " " << oid << dendl;
    if (!c->exists) return -ENOENT;
...
int MemStore::collection_list(CollectionHandle& ch, const ghobject_t& start, const ghobject_t& end, int max,
                              std::vector<ghobject_t>* ls, ghobject_t* next)
{
    ...
    while (p != c->object_map.end() && ls->size() < (unsigned)max && p->first < end) {
        ls->push_back(p->first);
        ++p;
    }
    if (next != NULL) {
...
)
{
    ...
    if (!o) return -ENOENT;
    std::lock_guard lock{o->omap_mutex};
    for (auto p = o->omap.begin(); p != o->omap.end(); ++p) keys->insert(p->first);
    return 0;
}
...
ObjectMap::ObjectMapIterator MemStore::get_omap_iterator(CollectionHandle& ch, const ghobject_t& oid)
{
    dout(10) << __func__ << " " << ch->cid << " " << oid << dendl;
    Collection* c = static_cast<Collection*>(ch.get());
    ObjectRef o = c->get_object(oid);
    if (!o) return ObjectMap::ObjectMapIterator();
...
int MemStore::queue_transactions(CollectionHandle& ch, std::vector<Transaction>& tls, TrackedOpRef op,
                                 ThreadPool::TPHandle* handle)
{
    ...
    if (on_apply_sync) on_apply_sync->complete(0);
    if (on_apply) finisher.queue(on_apply);
    if (on_commit) finisher.queue(on_commit);
    return 0;
}
...
int MemStore::_write(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len,
                     const ceph::buffer::list& bl, uint32_t fadvise_flags)
{
...
int MemStore::_zero(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len)
{
    dout(10) << __func__ << " " << cid << " " << oid << " " << offset << "~" << len << dendl;
    ceph::buffer::list bl;
    bl.append_zero(len);
...
int MemStore::_truncate(const coll_t& cid, const ghobject_t& oid, uint64_t size)
{
    ...
    ObjectRef o = c->get_object(oid);
    if (!o) return -ENOENT;
    if (cct->_conf->memstore_debug_omit_block_device_write) return 0;
    const ssize_t old_size = o->get_size();
...
int MemStore::_setattrs(const coll_t& cid, const ghobject_t& oid, std::map<std::string, ceph::buffer::ptr>& aset)
{
    dout(10) << __func__ << " " << cid << " " << oid << dendl;
    CollectionRef c = get_collection(cid);
    if (!c) return -ENOENT;
    ...
    if (!o) return -ENOENT;
    std::lock_guard lock{o->xattr_mutex};
    for (auto p = aset.begin(); p != aset.end(); ++p) o->xattr[p->first] = p->second;
    return 0;
}
...
int MemStore::_rmattr(const coll_t& cid, const ghobject_t& oid, const char* name)
{
    ...
    if (i == o->xattr.end()) return -ENODATA;
    o->xattr.erase(i);
    return 0;
}
...
int MemStore::_clone(const coll_t& cid, const ghobject_t& oldoid, const ghobject_t& newoid)
{
    ...
    std::scoped_lock l{oo->xattr_mutex, no->xattr_mutex, oo->omap_mutex, no->omap_mutex};
...
int MemStore::_clone_range(const coll_t& cid, const ghobject_t& oldoid, const ghobject_t& newoid, uint64_t srcoff,
                           uint64_t len, uint64_t dstoff)
{
    dout(10) << __func__ << " " << cid << " " << oldoid << " " << srcoff << "~" << len << " -> " << newoid << " "
             << dstoff << "~" << len << dendl;
    CollectionRef c = get_collection(cid);
    if (!c) return -ENOENT;
    ...
    if (!oo) return -ENOENT;
    ObjectRef no = c->get_or_create_object(newoid);
    if (srcoff >= oo->get_size()) return 0;
    if (srcoff + len >= oo->get_size()) len = oo->get_size() - srcoff;
...
int MemStore::_omap_setkeys(const coll_t& cid, const ghobject_t& oid, ceph::buffer::list& aset_bl)
{
    dout(10) << __func__ << " " << cid << " " << oid << dendl;
    CollectionRef c = get_collection(cid);
    if (!c) return -ENOENT;
...
int MemStore::_omap_rmkeys(const coll_t& cid, const ghobject_t& oid, ceph::buffer::list& keys_bl)
{
    dout(10) << __func__ << " " << cid << " " << oid << dendl;
    CollectionRef c = get_collection(cid);
    if (!c) return -ENOENT;
...
int MemStore::_omap_rmkeyrange(const coll_t& cid, const ghobject_t& oid, const std::string& first,
...
int MemStore::_omap_setheader(const coll_t& cid, const ghobject_t& oid, const ceph::buffer::list& bl)
{
    dout(10) << __func__ << " " << cid << " " << oid << dendl;
    CollectionRef c = get_collection(cid);
    if (!c) return -ENOENT;
...
int MemStore::_collection_add(const coll_t& cid, const coll_t& ocid, const ghobject_t& oid)
{
    ...
    std::scoped_lock l{std::min(&(*c), &(*oc))->lock, std::max(&(*c), &(*oc))->lock};
...
int MemStore::_collection_move_rename(const coll_t& oldcid, const ghobject_t& oldoid, coll_t cid, const ghobject_t& oid)
{
    ...
    std::lock_guard l{c->lock};
    if (c->object_hash.count(oid)) return -EEXIST;
    if (oc->object_hash.count(oldoid) == 0) return -ENOENT;
    {
...
int MemStore::_split_collection(const coll_t& cid, uint32_t bits, uint32_t match, coll_t dest)
{
    ...
    std::scoped_lock l{std::min(&(*sc), &(*dc))->lock, std::max(&(*sc), &(*dc))->lock};
...
struct BufferlistObject : public MemStore::Object
{
    ...
    int clone(Object* src, uint64_t srcoff, uint64_t len, uint64_t dstoff) override;
...
// BufferlistObject
int BufferlistObject::read(uint64_t offset, uint64_t len, ceph::buffer::list& bl)
{
...
int BufferlistObject::clone(Object* src, uint64_t srcoff, uint64_t len, uint64_t dstoff)
{
    auto srcbl = dynamic_cast<BufferlistObject*>(src);
    if (srcbl == nullptr) return -ENOTSUP;
...
private:
    ...
    explicit PageSetObject(size_t page_size)
        : data(page_size)
        , data_len(0)
    {}
};
...
int MemStore::PageSetObject::write(uint64_t offset, const ceph::buffer::list& src)
{
    ...
    while (len > 0) {
        ...
        offset += count;
        len -= count;
        if (count == pageoff) ++page;
    }
    if (data_len < offset) data_len = offset;
    tls_pages.clear();   // drop page refs
    return 0;
}

int MemStore::PageSetObject::clone(Object* src, uint64_t srcoff, uint64_t len, uint64_t dstoff)
{
        ...
        for (auto& src_page : tls_pages) {
            ...
            if (srcoff < sbegin) {
                while (dst_iter != dst_pages.end()) {
                    ...
                    std::fill(dst_page->data + dbegin - dst_page->offset, dst_page->data + dend - dst_page->offset, 0);
                    if (dend < dst_page->offset + dst_page_size) break;
                    ++dst_iter;
                }
    ...
    // update object size
    if (data_len < dstoff) data_len = dstoff;
    return 0;
}
...
{
    if (use_page_set) return ceph::make_ref<PageSetObject>(cct->_conf->memstore_page_size);
