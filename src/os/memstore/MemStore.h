...
public:
    struct Object : public RefCountedObject
    {
        ...
        virtual int clone(Object* src, uint64_t srcoff, uint64_t len, uint64_t dstoff) = 0;
    ...
    struct Collection : public CollectionImpl
    {
        ...
        ceph::shared_mutex lock{ceph::make_shared_mutex("MemStore::Collection::lock", true, false)};

        bool exists = true;
        ceph::mutex sequencer_mutex{ceph::make_mutex("MemStore::Collection::sequencer_mutex")};
        ...
        ObjectRef get_object(ghobject_t oid)
        {
            ...
            if (o == object_hash.end()) return ObjectRef();
            return o->second;
        }
        ...
        ObjectRef get_or_create_object(ghobject_t oid)
        {
            ...
            if (result.second) object_map[oid] = result.first->second = create_object();
        ...
        void flush() override {}
        bool flush_commit(Context* c) override { return true; }
...
private:
    ...
    ceph::shared_mutex coll_lock{ceph::make_shared_mutex("MemStore::coll_lock")};
    ...
    int _write(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len, const ceph::buffer::list& bl,
    ...
    int _clone_range(const coll_t& cid, const ghobject_t& oldoid, const ghobject_t& newoid, uint64_t srcoff,
    ...
    int _omap_rmkeyrange(const coll_t& cid, const ghobject_t& oid, const std::string& first, const std::string& last);
    int _omap_setheader(const coll_t& cid, const ghobject_t& oid, const ceph::buffer::list& bl);

    int _collection_hint_expected_num_objs(const coll_t& cid, uint32_t pg_num, uint64_t num_objs) const { return 0; }
...
public:
    ...
    std::string get_type() override { return "memstore"; }
    ...
    CollectionHandle open_collection(const coll_t& c) override { return get_collection(c); }
    ...
    int collection_list(CollectionHandle& cid, const ghobject_t& start, const ghobject_t& end, int max,
