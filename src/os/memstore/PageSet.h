...
struct Page
{
    ...
    struct Less
    {
        bool operator()(uint64_t offset, const Page& page) const { return offset < page.offset; }
        bool operator()(const Page& page, uint64_t offset) const { return page.offset < offset; }
        bool operator()(const Page& lhs, const Page& rhs) const { return lhs.offset < rhs.offset; }
    };
...
private:   // private constructor, use create() instead
    ...
    static void operator delete(void* p) { delete[] reinterpret_cast<Page*>(p)->data; }
};
...
private:
    ...
    typedef boost::intrusive::member_hook<Page, boost::intrusive::avl_set_member_hook<>, &Page::hook> member_option;
    typedef boost::intrusive::avl_set<Page, boost::intrusive::compare<page_cmp>, member_option> page_set;
...
public:
        ...
        while (length) {
            ...
            if (insert.second) {
                ...
                if (offset + length < page->offset + page_size)
                    std::fill(page->data + offset + length - page->offset, page->data + page_size, 0);
                // zero front of page between page_offset and offset
                if (offset > page->offset) std::fill(page->data, page->data + offset - page->offset, 0);
            }
            else {   // exists
                cur = insert.first;
            }
    ...
    void get_range(uint64_t offset, uint64_t length, page_vector& range)
    {
        ...
        while (cur != pages.end() && cur->offset < offset + length) range.push_back(&*cur++);
    }
    ...
    void encode(ceph::buffer::list& bl) const
    {
        ...
        encode(count, bl);
        for (auto p = pages.rbegin(); p != pages.rend(); ++p) p->encode(bl, page_size);
    }
