...
struct key_data
{
    ...
    bool operator==(key_data k) const { return ((raw_key == k.raw_key) && (prefix == k.prefix)); }

    bool operator!=(key_data k) const { return ((raw_key != k.raw_key) || (prefix != k.prefix)); }

    bool operator<(key_data k) const { return this->encoded() < k.encoded(); }
     ...
     */
    std::string encoded() const { return prefix + raw_key; }
...
struct object_data
{
    ...
    object_data(key_data min, key_data kdat, std::string the_name, std::map<std::string, bufferlist> the_omap)
        : min_kdata(min)
        , max_kdata(kdat)
        , name(the_name)
        , omap(the_omap)
...
struct index_data
{
    ...
    bool operator<(const index_data& other) const { return (kdata.encoded() < other.kdata.encoded()); }
    ...
    std::string str() const
    {
        ...
        strm << '(' << min_kdata.encoded() << "/" << kdata.encoded() << ',' << prefix;
        if (prefix == "1") {
            strm << ts.sec() << '.' << ts.usec();
            for (std::vector<create_data>::const_iterator it = to_create.begin(); it != to_create.end(); ++it) {
                strm << '(' << it->min.encoded() << '/' << it->max.encoded() << '|' << it->obj << ')';
            }
            strm << ';';
            for (std::vector<delete_data>::const_iterator it = to_delete.begin(); it != to_delete.end(); ++it) {
                strm << '(' << it->min.encoded() << '/' << it->max.encoded() << '|' << it->obj << '|' << it->version
                     << ')';
            }
            strm << ':';
        }
...
protected:
     ...
     * stored
     */
    int read_index(const std::string& key, index_data* idata, index_data* next_idata, bool force_update);
     ...
     */
    void set_up_prefix_index(const std::vector<object_data>& to_create, const std::vector<object_data>& to_delete,
     ...
     */
    void set_up_ops(const std::vector<object_data>& create_vector, const std::vector<object_data>& delete_vector,
     ...
     * unwritable to "0"
     */
    void set_up_make_object(const std::map<std::string, bufferlist>& to_set, librados::ObjectWriteOperation* owo);
     ...
     * returns it.
     */
    int perform_ops(const std::string& debug_prefix, const index_data& idata,
                    std::vector<std::pair<std::pair<int, std::string>, librados::ObjectWriteOperation*> >* ops);
     ...
     * until success.
     */
    int set_op(const std::string& key, const bufferlist& val, bool update_on_existing, index_data& idata);
     ...
     * until success.
     */
    int handle_set_rm_errors(int& err, std::string key, std::string obj, index_data* idata, index_data* next_idata);
...
public:
    ...
    KvFlatBtreeAsync(int k_val, std::string name, int cache, double cache_r, bool verb)
        : k(k_val)
    ...
    int set(const std::string& key, const bufferlist& val, bool update_on_existing) override;
    ...
    void aio_get(const std::string& key, bufferlist* val, callback cb, void* cb_args, int* err) override;
    void aio_set(const std::string& key, const bufferlist& val, bool exclusive, callback cb, void* cb_args,
                 int* err) override;
