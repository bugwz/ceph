 ...
 * stored
 */
static int get_idata_from_key(cls_method_context_t hctx, const string& key, index_data& idata, index_data& next_idata)
{
    bufferlist raw_val;
    int r = 0;
...
static int get_idata_from_key_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
static int get_next_idata(cls_method_context_t hctx, const index_data& idata, index_data& out_data)
{
    int r = 0;
...
static int get_next_idata_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
static int get_prev_idata(cls_method_context_t hctx, const index_data& idata, index_data& out_data)
{
    int r = 0;
    std::map<std::string, bufferlist> kvs;
    bool more;
    ...
    std::map<std::string, bufferlist>::iterator it = kvs.lower_bound(idata.kdata.encoded());
    if (it->first != idata.kdata.encoded()) {
        CLS_LOG(20,
                "object %s not found in the index (expected %s, found %s)",
                idata.str().c_str(),
                idata.kdata.encoded().c_str(),
                it->first.c_str());
        return -ENODATA;
    }
    if (it == kvs.begin()) {
...
static int get_prev_idata_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    CLS_LOG(20, "get_next_idata_op");
    idata_from_idata_args op;
    auto it = in->cbegin();
 ...
 */
static int read_many(cls_method_context_t hctx, const set<string>& keys, map<string, bufferlist>* out)
{
    int r = 0;
    bool more;
    CLS_ERR("reading from a map of size %d, first key encoded is %s",
            (int)keys.size(),
            key_data(*keys.begin()).encoded().c_str());
    r = cls_cxx_map_get_vals(hctx, key_data(*keys.begin()).encoded().c_str(), "", LONG_MAX, out, &more);
    if (r < 0) {
    ...
    if (out->size() > 1) {
        out->erase(out->upper_bound(key_data(*keys.rbegin()).encoded().c_str()), out->end());
    }
    CLS_ERR("returning map of size %d", (int)out->size());
    return r;
}

static int read_many_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
...
static int check_writable_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
static int assert_size_in_bound(cls_method_context_t hctx, int bound, int comparator)
{
    // determine size
    bufferlist size_bl;
    ...
    case CEPH_OSD_CMPXATTR_OP_GT:
        ...
        break;
    default: CLS_LOG(20, "invalid argument passed to assert_size_in_bound: %d", comparator); return -EINVAL;
    }
    return 0;
}

static int assert_size_in_bound_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
static int omap_insert(cls_method_context_t hctx, const map<string, bufferlist>& omap, int bound, bool exclusive)
{
    ...
    for (map<string, bufferlist>::const_iterator it = omap.begin(); it != omap.end(); ++it) {
        bufferlist bl;
        r = cls_cxx_map_get_val(hctx, it->first, &bl);
        if (r == 0 && string(bl.c_str(), bl.length()) != "") {
            if (exclusive) {
                CLS_LOG(20, "error: this is an exclusive insert and %s exists.", it->first.c_str());
                return -EEXIST;
            }
            assert_bound++;
...
static int omap_insert_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
...
static int create_with_omap(cls_method_context_t hctx, const map<string, bufferlist>& omap)
{
...
static int create_with_omap_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    CLS_LOG(20, "omap_insert");
 ...
 */
static int omap_remove(cls_method_context_t hctx, const std::set<string>& omap, int bound)
{
    int r;
    uint64_t size;
    time_t time;
    ...
    for (set<string>::const_iterator it = omap.begin(); it != omap.end(); ++it) {
        bufferlist bl;
        r = cls_cxx_map_get_val(hctx, *it, &bl);
        if (r == -ENOENT || r == -ENODATA || string(bl.c_str(), bl.length()) == "") {
            return -ENODATA;
        }
        else if (r < 0) {
    ...
    for (std::set<string>::const_iterator it = omap.begin(); it != omap.end(); ++it) {
...
static int omap_remove_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 */
static int maybe_read_for_balance(cls_method_context_t hctx, object_data& odata, int bound, int comparator)
{
    ...
    CLS_LOG(20,
            "rebalance read: size xattr is %llu, omap size is %llu",
            (unsigned long long)odata.size,
            (unsigned long long)odata.omap.size());
    return 0;
}

static int maybe_read_for_balance_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
...
CLS_INIT(kvs)
{
    ...
    cls_register_cxx_method(h_class, "get_idata_from_key", CLS_METHOD_RD, get_idata_from_key_op, &h_get_idata_from_key);
    cls_register_cxx_method(h_class, "get_next_idata", CLS_METHOD_RD, get_next_idata_op, &h_get_next_idata);
    cls_register_cxx_method(h_class, "get_prev_idata", CLS_METHOD_RD, get_prev_idata_op, &h_get_prev_idata);
    cls_register_cxx_method(h_class, "read_many", CLS_METHOD_RD, read_many_op, &h_read_many);
    cls_register_cxx_method(
        h_class, "check_writable", CLS_METHOD_RD | CLS_METHOD_WR, check_writable_op, &h_check_writable);
    cls_register_cxx_method(
        h_class, "assert_size_in_bound", CLS_METHOD_WR, assert_size_in_bound_op, &h_assert_size_in_bound);
    cls_register_cxx_method(h_class, "omap_insert", CLS_METHOD_WR, omap_insert_op, &h_omap_insert);
    cls_register_cxx_method(h_class, "create_with_omap", CLS_METHOD_WR, create_with_omap_op, &h_create_with_omap);
    cls_register_cxx_method(h_class, "omap_remove", CLS_METHOD_WR, omap_remove_op, &h_omap_remove);
    cls_register_cxx_method(
        h_class, "maybe_read_for_balance", CLS_METHOD_RD, maybe_read_for_balance_op, &h_maybe_read_for_balance);
