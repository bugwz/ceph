...
void IndexCache::push(const string& key, const index_data& idata)
{
    ...
    map<key_data, pair<index_data, utime_t> >::iterator old_it = k2itmap.lower_bound(key_data(key));
    if (old_it != k2itmap.end()) {
        t2kmap.erase(old_it->second.second);
        k2itmap.erase(old_it);
    }
    map<key_data, pair<index_data, utime_t> >::iterator new_it = k2itmap.find(idata.kdata);
...
int IndexCache::get(const string& key, index_data* idata) const
{
    ...
    map<key_data, pair<index_data, utime_t> >::const_iterator it = k2itmap.lower_bound(key_data(key));
...
int KvFlatBtreeAsync::split(const index_data& idata)
{
    ...
    ops.push_back(make_pair(pair<int, string>(ADD_PREFIX, index_name), &owos[0]));
    for (int i = 1; i < 6; i++) {
...
int KvFlatBtreeAsync::rebalance(const index_data& idata1, const index_data& next_idata)
{
    ...
    vector<pair<pair<int, string>, librados::ObjectWriteOperation*> > ops;
    ops.push_back(make_pair(pair<int, string>(ADD_PREFIX, index_name), &other_ops[0]));
    ...
    if ((int)args1.odata.size + (int)args2.odata.size <= 2 * k) {
        ...
        ops.push_back(make_pair(pair<int, string>(MAKE_OBJECT, o2w), &create[0]));
        ...
        ops.push_back(make_pair(pair<int, string>(MAKE_OBJECT, o1w), &create[0]));
        ops.push_back(make_pair(pair<int, string>(MAKE_OBJECT, o2w), &create[1]));
    }
                                           ...
                                           librados::ObjectWriteOperation* owo, index_data* idata, int* err)
{
    ...
    for (vector<object_data>::const_iterator it = to_delete.begin(); it != to_delete.end(); ++it) {
        ...
        assertions[it->max_kdata.encoded()] = pair<bufferlist, int>(to_bl(this_entry), CEPH_OSD_CMPXATTR_OP_EQ);
...
{
    vector<pair<pair<int, string>, librados::ObjectWriteOperation*> >::iterator it;
    ...
    for (int i = 0; i < (int)idata.to_delete.size(); ++i) {
        ...
        assertions[idata.to_delete[i].max.encoded()] =
            pair<bufferlist, int>(to_bl(this_entry), CEPH_OSD_CMPXATTR_OP_EQ);
...
void KvFlatBtreeAsync::set_up_make_object(const map<std::string, bufferlist>& to_set,
                                  ...
                                  vector<pair<pair<int, string>, librados::ObjectWriteOperation*> >* ops)
{
    ...
    int count = 0;
    for (vector<pair<pair<int, string>, librados::ObjectWriteOperation*> >::iterator it = ops->begin();
         it != ops->end();
         ++it) {
        ...
        for (vector<delete_data>::const_iterator it = idata.to_delete.begin(); it != idata.to_delete.end(); ++it) {
            ...
            if (verbose)
                cout << "\t\t\t" << client_name << "-cleanup: will assert index contains " << this_entry.str()
                     << std::endl;
            assertions[it->max.encoded()] = pair<bufferlist, int>(to_bl(this_entry), CEPH_OSD_CMPXATTR_OP_EQ);
        }
...
int KvFlatBtreeAsync::get_all_keys_and_values(map<std::string, bufferlist>* kv_map)
{
