...
static ostream& _prefix(std::ostream* _dout, const Monitor& mon, const HealthMonitor* hmon)
{
    return *_dout << "mon." << mon.name << "@" << mon.rank << "(" << mon.get_state_name() << ").health ";
}
...
void HealthMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
    ...
    for (auto& p : pending_health.checks) {
        p.second.summary =
            std::regex_replace(p.second.summary, std::regex("%hasorhave%"), names[p.first].size() > 1 ? "have" : "has");
        p.second.summary = std::regex_replace(p.second.summary, std::regex("%names%"), stringify(names[p.first]));
        p.second.summary =
            std::regex_replace(p.second.summary, std::regex("%plurals%"), names[p.first].size() > 1 ? "s" : "");
        p.second.summary =
            std::regex_replace(p.second.summary, std::regex("%isorare%"), names[p.first].size() > 1 ? "are" : "is");
    }
...
bool HealthMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    switch (m->get_type()) {
    case MSG_MON_COMMAND: return preprocess_command(op);
    case MSG_MON_HEALTH_CHECKS: return false;
    default:
        mon.no_reply(op);
...
bool HealthMonitor::prepare_update(MonOpRequestRef op)
{
    ...
    switch (m->get_type()) {
    case MSG_MON_HEALTH_CHECKS: return prepare_health_checks(op);
    case MSG_MON_COMMAND: return prepare_command(op);
...
bool HealthMonitor::check_mutes()
{
    ...
    while (p != pending_mutes.end()) {
        if (p->second.ttl != utime_t() && p->second.ttl <= now) {
            mon.clog->info() << "Health alert mute " << p->first << " cleared (passed TTL " << p->second.ttl << ")";
        ...
        if (!p->second.sticky) {
            ...
            if (q == all.checks.end()) {
                mon.clog->info() << "Health alert mute " << p->first << " cleared (health alert cleared)";
                p = pending_mutes.erase(p);
                changed = true;
                continue;
            }
            if (p->second.count) {
                ...
                if (q->second.count > p->second.count) {
                    mon.clog->info() << "Health alert mute " << p->first << " cleared (count increased from "
                ...
                if (q->second.count < p->second.count) {
                    ...
                    dout(10) << __func__ << " mute " << p->first << " count " << p->second.count << " -> "
                             << q->second.count << dendl;
                ...
                if (p->second.summary != q->second.summary) {
                    mon.clog->info() << "Health alert mute " << p->first << " cleared (summary changed)";
                    p = pending_mutes.erase(p);
                    changed = true;
                    continue;
                }
...
health_status_t HealthMonitor::get_health_status(bool want_detail, Formatter* f, std::string* plain, const char* sep1,
                                                 const char* sep2)
{
    health_check_map_t all;
    gather_all_health_checks(&all);
        ...
        *plain += "\n";
        ...
        if (want_detail) {
            for (auto& p : all.checks) {
                ...
                *plain +=
                    "["s + short_health_string(p.second.severity) + "] " + p.first + ": " + p.second.summary + "\n";
...
bool HealthMonitor::check_member_health()
{
    ...
    dout(10) << __func__ << " avail " << stats.fs_stats.avail_percent << "%"
             << " total " << byte_u_t(stats.fs_stats.byte_total) << ", used " << byte_u_t(stats.fs_stats.byte_used)
    ...
    if (stats.store_stats.bytes_total >= g_conf()->mon_data_size_warn) {
        ...
        ss2 << "mon." << mon.name << " is " << byte_u_t(stats.store_stats.bytes_total) << " >= mon_data_size_warn ("
    ...
    // OSD_NO_DOWN_OUT_INTERVAL
    {
        ...
        if (g_conf()->mon_warn_on_osd_down_out_interval_zero && g_conf()->mon_osd_down_out_interval == 0) {
            ostringstream ss, ds;
    ...
    if (g_conf().get_val<bool>("mon_warn_on_insecure_global_id_reclaim") &&
        ...
        for (auto p = mon.session_map.sessions.begin(); p != mon.session_map.sessions.end(); ++p) {
            if ((*p)->global_id_status == global_id_status_t::RECLAIM_INSECURE) {
                ostringstream ds;
                ds << (*p)->entity_name << " at " << (*p)->addrs << " is using insecure global_id reclaim";
        ...
        if (!detail.empty()) {
            ...
            auto& d = next.add("AUTH_INSECURE_GLOBAL_ID_RECLAIM", HEALTH_WARN, ss.str(), detail.size());
...
{
    static ceph::coarse_mono_time old_version_first_time = ceph::coarse_mono_clock::zero();
    ...
    if (now - old_version_first_time > warn_delay) {
        ...
        if (all_versions.size() > 1) {
            ...
            ss << "There " << (daemon_count == 1 ? "is a daemon" : "are daemons") << " running "
               << (ver_count > 1 ? "multiple old versions" : "an older version") << " of ceph";
            ...
            for (auto& g : all_versions) {
                ...
                ds << (g.second.size() == 1 ? "is" : "are") << " running an older version of ceph: " << g.first;
                d.detail.push_back(ds.str());
            }
        }
        else {
...
void HealthMonitor::check_for_mon_down(health_check_map_t* checks)
{
    ...
    if (actual < max &&
        ...
        for (int i = 0; i < max; i++) {
            if (q.count(i) == 0) {
                ostringstream ss;
                ss << "mon." << mon.monmap->get_name(i) << " (rank " << i << ") addr " << mon.monmap->get_addrs(i)
    ...
    if (!mon.timecheck_skews.empty()) {
        ...
        for (auto& i : mon.timecheck_skews) {
            ...
            if (tcstatus != HEALTH_OK) {
                ...
                tmp_ss << "mon." << name << " " << tcss.str() << " (latency " << latency << "s)";
...
void HealthMonitor::check_if_msgr2_enabled(health_check_map_t* checks)
{
    if (g_conf().get_val<bool>("ms_bind_msgr2") &&
        mon.monmap->get_required_features().contains_all(ceph::features::mon::FEATURE_NAUTILUS)) {
        list<string> details;
        for (auto& i : mon.monmap->mon_info) {
            if (!i.second.public_addrs.has_msgr2()) {
                ostringstream ds;
                ds << "mon." << i.first << " is not bound to a msgr2 port, only " << i.second.public_addrs;
        ...
        if (!details.empty()) {
            ...
            auto& d = checks->add("MON_MSGR2_NOT_ENABLED", HEALTH_WARN, ss.str(), details.size());
            d.detail.swap(details);
        }
    }
}
