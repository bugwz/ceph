...
public:
    const health_check_map_t& get_health_checks() const { return health_checks; }
     ...
     */
    virtual void get_store_prefixes(std::set<std::string>& s) const { s.insert(service_name); }
...
public:
     ...
     */
    void force_immediate_propose() { need_immediate_propose = true; }
...
public:
     ...
     */
    bool is_proposing() const { return proposing; }
    ...
    {
        return !is_proposing() && (paxos.is_active() || paxos.is_updating() || paxos.is_writing());
    }
    ...
    {
        if (ver > get_last_committed() || !paxos.is_readable(0) || get_last_committed() == 0) return false;
        return true;
    }
     ...
     */
    bool is_writeable() const { return is_active() && have_pending; }
    ...
    {
        if (op) op->mark_event(service_name + ":wait_for_finished_proposal");
    ...
    {
        if (op) op->mark_event(service_name + ":wait_for_active");
    ...
    void wait_for_readable(MonOpRequestRef op, Context* c, version_t ver = 0)
    {
         ...
         */
        if (op) op->mark_event(service_name + ":wait_for_readable");

        if (is_proposing() || ver > get_last_committed() || get_last_committed() == 0)
            wait_for_finished_proposal(op, c);
        else {
            if (op) op->mark_event(service_name + ":wait_for_readable/paxos");
    ...
    {
        if (op) op->mark_event(service_name + ":wait_for_writeable");
     ...
     */
    virtual version_t get_trim_to() const { return 0; }
     ...
     */
    void cancel_events() { paxos.cancel_events(); }
    ...
    void put_last_committed(MonitorDBStore::TransactionRef t, version_t ver)
    {
         ...
         */
        if (!get_first_committed()) put_first_committed(t, ver);
    }
    /**
     ...
     */
    void put_version(MonitorDBStore::TransactionRef t, version_t ver, ceph::buffer::list& bl)
    {
     ...
     * @param bl A ceph::buffer::list containing the version's value
     */
    void put_version_full(MonitorDBStore::TransactionRef t, version_t ver, ceph::buffer::list& bl)
    {
     ...
     */
    void put_value(MonitorDBStore::TransactionRef t, const std::string& key, ceph::buffer::list& bl)
    {
        t->put(get_service_name(), key, bl);
    }
     ...
     * @param v An integer
     */
    void put_value(MonitorDBStore::TransactionRef t, const std::string& key, version_t v)
    {
     ...
     */
    version_t get_first_committed() const { return cached_first_committed; }
    /**
     * Get the last committed version
     *
     * @returns Our last committed version
     */
    version_t get_last_committed() const { return cached_last_committed; }
     ...
     * @param[in] key The key
     */
    version_t get_value(const std::string& key) { return mon.store->get(get_service_name(), key); }
