...
public:
    ...
    const std::string get_name() const { return paxos_name; }

    void dispatch(MonOpRequestRef op);

    void read_and_prepare_transactions(MonitorDBStore::TransactionRef tx, version_t from, version_t last);
     ...
     */
    void share_state(MMonPaxos* m, version_t peer_first_committed, version_t peer_last_committed);
    /**
     ...
     */
    static void decode_append_transaction(MonitorDBStore::TransactionRef t, ceph::buffer::list& bl)
    {
     ...
     *	   state dir. -JL
     */
    void add_extra_state_dir(std::string s) { extra_state_dirs.push_back(s); }
    ...
    {
        if (op) op->mark_event("paxos:wait_for_active");
    ...
    bool should_trim()
    {
        ...
        if (trimming || (available_versions <= maximum_versions)) return false;

        return true;
    }

    bool is_plugged() const { return plugged; }
    void plug()
    {
    ...
    void wait_for_readable(MonOpRequestRef op, Context* onreadable)
    {
        ...
        if (op) op->mark_event("paxos:wait_for_readable");
    ...
    void wait_for_writeable(MonOpRequestRef op, Context* c)
    {
        ...
        if (op) op->mark_event("paxos:wait_for_writeable");
...
inline std::ostream& operator<<(std::ostream& out, Paxos::C_Proposal& p)
{
    ...
    out << " " << proposed << " queued " << (ceph_clock_now() - p.proposal_time) << " tx dump:\n";
