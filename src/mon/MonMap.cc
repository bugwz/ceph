...
{
    out << "mon." << name << " addrs " << public_addrs << " priority " << priority << " weight " << weight
...
struct rank_cmp
{
    ...
    {
        if (a.public_addrs.legacy_or_front_addr() == b.public_addrs.legacy_or_front_addr()) return a.name < b.name;
...
void MonMap::encode(ceph::buffer::list& blist, uint64_t con_features) const
{
    ...
    if (!HAVE_FEATURE(con_features, MONENC) || !HAVE_FEATURE(con_features, SERVER_NAUTILUS)) {
...
{
    out << "e" << epoch << ": " << mon_info.size() << " mons at {";
...
void MonMap::print(ostream& out) const
{
    ...
    if (stretch_mode_enabled || !disallowed_leaders.empty()) {
        out << "disallowed_leaders " << disallowed_leaders << "\n";
    }
    unsigned i = 0;
...
void MonMap::_add_ambiguous_addr(const string& name, entity_addr_t addr, int priority, int weight, bool for_mkfs)
{
...
int MonMap::init_with_ips(const std::string& ips, bool for_mkfs, std::string_view prefix)
{
    vector<entity_addrvec_t> addrs;
    if (!parse_ip_port_vec(ips.c_str(), addrs, entity_addr_t::TYPE_ANY)) {
        return -EINVAL;
    }
    if (addrs.empty()) return -ENOENT;
    init_with_addrs(addrs, for_mkfs, prefix);
    return 0;
}

int MonMap::init_with_hosts(const std::string& hostlist, bool for_mkfs, std::string_view prefix)
{
    // maybe they passed us a DNS-resolvable name
    char* hosts = resolve_addrs(hostlist.c_str());
    if (!hosts) return -EINVAL;
...
void MonMap::set_initial_members(CephContext* cct, list<std::string>& initial_members, string my_name,
                                 const entity_addrvec_t& my_addrs, set<entity_addrvec_t>* removed)
{
...
int MonMap::init_with_config_file(const ConfigProxy& conf, std::ostream& errout)
{
    std::vector<std::string> sections;
    int ret = conf.get_all_sections(sections);
    if (ret) {
        errout << "Unable to find any monitors in the configuration "
               << "file, because there was an error listing the sections. error " << ret << std::endl;
        return -ENOENT;
    }
    ...
    for (const auto& mon_name : mon_names) {
        ...
        if (res) {
            errout << "failed to get an address for mon." << mon_name << ": error " << res << std::endl;
            continue;
        }
        ...
        if (!addr.parse(val, entity_addr_t::TYPE_LEGACY)) {
            errout << "unable to parse address for mon." << mon_name << ": addr='" << val << "'" << std::endl;
            continue;
        }
        if (addr.get_port() == 0) {
            addr.set_port(CEPH_MON_PORT_LEGACY);
        ...
        if (!conf.get_val_from_conf_file(sections, "mon priority", val, false)) {
            try {
                ...
                errout << "unable to parse priority for mon." << mon_name << ": priority='" << val << "'" << std::endl;
                continue;
            }
        }
        uint16_t weight = 0;
        if (!conf.get_val_from_conf_file(sections, "mon weight", val, false)) {
            try {
                ...
                errout << "unable to parse weight for mon." << mon_name << ": weight='" << val << "'" << std::endl;
                continue;
            }
        }

        // make sure this mon isn't already in the map
        if (contains(addr)) remove(get_name(addr));
        if (contains(mon_name)) remove(mon_name);
...
void MonMap::check_health(health_check_map_t* checks) const
{
    if (stretch_mode_enabled) {
        ...
        if (!detail.empty()) {
            ...
            auto& d = checks->add("MON_LOCATION_NOT_SET", HEALTH_WARN, ss.str(), detail.size());
        ...
        return f.size().then([this, f = std::move(f)](size_t s) {
            return do_with(make_file_input_stream(f), [this, s](input_stream<char>& in) {
                return in.read_exactly(s).then([this](temporary_buffer<char> buf) {
                    ceph::buffer::list bl;
                    bl.push_back(ceph::buffer::ptr_node::create(ceph::buffer::create(std::move(buf))));
                    decode(bl);
                });
            });
        });
...
seastar::future<> MonMap::init_with_dns_srv(bool for_mkfs, const std::string& name)
{
    ...
    return seastar::net::dns::get_srv_records(seastar::net::dns_resolver::srv_proto::tcp, service, domain)
        .then([this](seastar::net::dns_resolver::srv_records records) {
            return seastar::parallel_for_each(records, [this](auto record) {
                return seastar::net::dns::resolve_name(record.target)
                    .then([record, this](seastar::net::inet_address a) {
                        ...
                        switch (a.in_family()) {
                        case seastar::net::inet_address::family::INET: addr.in4_addr().sin_addr = a; break;
                        case seastar::net::inet_address::family::INET6: addr.in6_addr().sin6_addr = a; break;
                        }
                        _add_ambiguous_addr(record.target, addr, record.priority, record.weight, false);
                    })
                    .handle_exception_type([t = record.target](const std::system_error& e) {
                        logger().debug("{}: unable to resolve name for {}: {}", "init_with_dns_srv", t, e);
                    });
            });
        })
        .handle_exception_type([name](const std::system_error& e) {
            logger().debug("{}: unable to get monitor info from DNS SRV with {}: {}", "init_with_dns_srv", name, e);
            // ignore DNS failures
            return seastar::make_ready_future<>();
        });
}

bool MonMap::maybe_init_with_mon_host(const std::string& mon_host, const bool for_mkfs)
{
    if (!mon_host.empty()) {
...
seastar::future<> MonMap::build_monmap(const crimson::common::ConfigProxy& conf, bool for_mkfs)
{
...
{
    // mon_host_override?
    if (maybe_init_with_mon_host(conf.get_val<std::string>("mon_host_override"), for_mkfs)) {
        return seastar::make_ready_future<>();
    }

    // file?
    if (const auto monmap = conf.get_val<std::string>("monmap"); !monmap.empty()) {
        return read_monmap(monmap);
    }
    else {
        // fsid from conf?
        if (const auto new_fsid = conf.get_val<uuid_d>("fsid"); !new_fsid.is_zero()) {
            fsid = new_fsid;
        }
...
int MonMap::init_with_monmap(const std::string& monmap, std::ostream& errout)
{
    ...
    if (r >= 0) return 0;
    errout << "unable to read/decode monmap from " << monmap << ": " << cpp_strerror(-r) << std::endl;
    return r;
}

int MonMap::init_with_dns_srv(CephContext* cct, std::string srv_name, bool for_mkfs, std::ostream& errout)
{
    ...
    if (DNSResolver::get_instance()->resolve_srv_hosts(
            cct, srv_name, DNSResolver::SRV_Protocol::TCP, domain, &records) != 0) {
        ...
        for (auto& record : records) {
            ...
            _add_ambiguous_addr(record.first, record.second.addr, record.second.priority, record.second.weight, false);
        }
        return 0;
    }
}
...
int MonMap::build_initial(CephContext* cct, bool for_mkfs, ostream& errout)
{
    ...
    if (!mon_host_override.empty()) {
        ...
        if (ret < 0) {
            errout << "unable to parse addrs in '" << mon_host_override << "'" << std::endl;
        }
        return ret;
    }
    ...
    // -m foo?
    if (const auto mon_host = conf.get_val<std::string>("mon_host"); !mon_host.empty()) {
        auto ret = init_with_ips(mon_host, for_mkfs, "noname-");
        if (ret == -EINVAL) {
            ret = init_with_hosts(mon_host, for_mkfs, "noname-");
        }
        if (ret < 0) {
            errout << "unable to parse addrs in '" << mon_host << "'" << std::endl;
            return ret;
        }
    }
    if (size() == 0) {
