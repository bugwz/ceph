...
static ostream& _prefix(std::ostream* _dout, const Monitor& mon, const ConfigMonitor* hmon)
{
    return *_dout << "mon." << mon.name << "@" << mon.rank << "(" << mon.get_state_name() << ").config ";
}
...
bool ConfigMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "config help") {
        ...
        list<pair<string, Section*>> sections = {make_pair("global", &config_map.global)};
        ...
        auto config = config_map.generate_entity_map(
            entity, crush_location, mon.osdmon()->osdmap.crush.get(), device_class, &src);
        ...
        if (cmd_getval(cmdmap, "key", name)) {
            ...
            for (; p != config.end(); ++p, ++q) {
                ...
                if (!f) {
                    ...
                    f->dump_bool("can_update_at_runtime", q->second.second->opt->can_update_at_runtime());
                    f->close_section();
                }
            }
            if (!f) {
        ...
        conf << "\tmon_host = ";
        for (auto i = mon.monmap->mon_info.begin(); i != mon.monmap->mon_info.end(); ++i) {
            if (i != mon.monmap->mon_info.begin()) {
                conf << " ";
            }
            if (i->second.public_addrs.size() == 1 && i->second.public_addrs.front().is_legacy() &&
...
void ConfigMonitor::handle_get_config(MonOpRequestRef op)
{
    ...
    auto out = config_map.generate_entity_map(m->name, crush_location, osdmap.crush.get(), m->device_class);
    ...
    if (prefix == "config set" || prefix == "config rm") {
        ...
        for (auto& [section, s] : cf) {
            ...
            for (auto& [key, val] : s) {
                ...
                if (!o || (o->flags & Option::FLAG_NO_MON_UPDATE) || (o->flags & Option::FLAG_CLUSTER_CREATE)) {
                    goto skip;
                }
                // normalize
                    ...
                    if (s) {
                        ...
                        if (k != s->options.end()) {
                            if (value != k->second.raw_value) {
                                dout(20) << __func__ << " have " << key << " = " << k->second.raw_value << " (not "
                                         << value << ")" << dendl;
                                goto skip;
                            }
...
update:
    ...
    wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, 0, ss.str(), odata, get_last_committed() + 1));
    return true;
}
...
void ConfigMonitor::load_config()
{
    ...
    current.clear();
    pending_cleanup.clear();
    while (it->valid() && it->key().compare(0, KEY_PREFIX.size(), KEY_PREFIX) == 0) {
    ...
    // refresh our own config
    {
        ...
        auto out = config_map.generate_entity_map(
            g_conf()->name, crush_location, osdmap.crush.get(), string{});   // no device class
...
bool ConfigMonitor::refresh_config(MonSession* s)
{
    ...
    if (s->remote_host.size()) {
        ...
        dout(10) << __func__ << " crush_location for remote_host " << s->remote_host << " is " << crush_location
                 << dendl;
    }
    ...
    dout(20) << __func__ << " " << s->entity_name << " crush " << crush_location << " device_class " << device_class
             << dendl;
    auto out = config_map.generate_entity_map(s->entity_name, crush_location, osdmap.crush.get(), device_class);
