...
public:
    ...
    bool definitely_converted_snapsets() const { return digest.definitely_converted_snapsets(); }
    ...
    uint64_t get_last_osd_stat_seq(int osd) { return digest.get_last_osd_stat_seq(osd); }

    void update_logger();

    const ServiceMap& get_service_map() const { return service_map; }

    const std::map<std::string, ProgressEvent>& get_progress_events() { return progress_events; }
    ...
    const PGMapDigest& get_digest() { return digest; }
    ...
    void print_summary(ceph::Formatter* f, std::ostream* out) const { digest.print_summary(f, out); }
    ...
    void dump_pool_stats(const OSDMap& osdm, std::stringstream* ss, ceph::Formatter* f, bool verbose) const
    {
