...
void LogMonitor::update_from_paxos(bool* need_bootstrap)
{
    ...
    while (version > summary.version) {
        ...
        while (!p.end()) {
            ...
            if (channels.do_log_to_graylog(channel)) {
                ...
                dout(7) << "graylog: " << channel << " " << graylog << " host:" << channels.log_to_graylog_host
                        << dendl;
            }

            if (g_conf()->mon_cluster_log_to_file) {
                string log_file = channels.get_log_file(channel);
                dout(20) << __func__ << " logging for channel '" << channel << "' to file '" << log_file << "'"
                         << dendl;
    ...
    for (map<string, bufferlist>::iterator p = channel_blog.begin(); p != channel_blog.end(); ++p) {
        if (!p->second.length()) {
        ...
        if (fd < 0) {
            int err = -errno;
            dout(1) << "unable to write to '" << log_file << "' for channel '" << p->first << "': " << cpp_strerror(err)
                    << dendl;
        }
        else {
            int err = p->second.write_fd(fd);
            if (err < 0) {
                dout(1) << "error writing to '" << log_file << "' for channel '" << p->first << ": "
...
void LogMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
    ...
    __u8 v = 1;
    encode(v, bl);
    for (auto p = pending_log.begin(); p != pending_log.end(); ++p) p->second.encode(bl, mon.get_quorum_con_features());
...
bool LogMonitor::preprocess_log(MonOpRequestRef op)
{
    ...
    for (auto p = m->entries.begin(); p != m->entries.end(); ++p) {
        if (!pending_summary.contains(p->key())) num_new++;
    }
    if (!num_new) {
