...
string LogMonitor::log_channel_info::get_log_file(const string& channel)
{
    ...
    if (expanded_log_file.count(channel) == 0) {
        string fname = expand_channel_meta(get_str_map_key(log_file, channel, &CLOG_CONFIG_DEFAULT_KEY), channel);
...
string LogMonitor::log_channel_info::expand_channel_meta(const string& input, const string& change_to)
{
    size_t pos = string::npos;
    string s(input);
    while ((pos = s.find(LOG_META_CHANNEL)) != string::npos) {
        string tmp = s.substr(0, pos) + change_to;
        if (pos + LOG_META_CHANNEL.length() < s.length()) tmp += s.substr(pos + LOG_META_CHANNEL.length());
        s = tmp;
    }
...
ceph::logging::Graylog::Ref LogMonitor::log_channel_info::get_graylog(const string& channel)
{
    dout(25) << __func__ << " for channel '" << channel << "'" << dendl;
    ...
    if (graylogs.count(channel) == 0) {
        ...
        graylog->set_destination(get_str_map_key(log_to_graylog_host, channel, &CLOG_CONFIG_DEFAULT_KEY),
                                 atoi(get_str_map_key(log_to_graylog_port, channel, &CLOG_CONFIG_DEFAULT_KEY).c_str()));

        graylogs[channel] = graylog;
        dout(20) << __func__ << " for channel '" << channel << "' to graylog host '" << log_to_graylog_host[channel]
...
{
    return *_dout << "mon." << mon.name << "@" << mon.rank << "(" << mon.get_state_name() << ").log v" << v << " ";
}
...
void LogMonitor::update_from_paxos(bool* need_bootstrap)
{
    ...
    if (version == summary.version) return;
    ...
    while (version > summary.version) {
        ...
        if (struct_v == 1) {
            ...
            for (auto& [channel, prune_to] : prune_channels_to) {
                dout(20) << __func__ << " channel " << channel << " pruned to " << prune_to << dendl;
...
void LogMonitor::log_external(const LogEntry& le)
{
    ...
    if (channels.do_log_to_syslog(channel)) {
        ...
        if (level.empty() || facility.empty()) {
            derr << __func__ << " unable to log to syslog -- level or facility"
                 << " not defined (level: " << level << ", facility: " << facility << ")" << dendl;
        }
        else {
            le.log_to_syslog(channels.get_level(channel), channels.get_facility(channel));
        }
    }
    ...
    if (g_conf()->mon_cluster_log_to_file) {
        ...
        if (p == channel_fds.end()) {
            ...
            if (!log_file.empty()) {
                ...
                if (fd < 0) {
                    int err = -errno;
                    dout(1) << "unable to write to '" << log_file << "' for channel '" << channel
    ...
    if (do_stderr || fd >= 0) {
        ...
        if (fd >= 0) {
            ...
            if (err < 0) {
                dout(1) << "error writing to '" << channels.get_log_file(channel) << "' for channel '" << channel
                        << ": " << cpp_strerror(err) << dendl;
                ::close(fd);
...
void LogMonitor::log_external_backlog()
{
    if (!external_log_to) {
        ...
        if (r == 0) {
            ...
            dout(10) << __func__ << " initialized external_log_to = " << external_log_to << " (summary v "
    ...
    if (external_log_to > get_last_committed()) {
        derr << __func__ << " rewinding external_log_to from " << external_log_to << " -> " << get_last_committed()
             << " (sync_force? mon rebuild?)" << dendl;
    ...
    if (auto first = get_first_committed(); external_log_to < first) {
        derr << __func__ << " local logs at " << external_log_to << ", skipping to " << first << dendl;
...
void LogMonitor::generate_logentry_key(const std::string& channel, version_t v, std::string* out)
{
    out->append(channel);
    out->append("/");
    char vs[10];
...
void LogMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
    ...
    for (auto& [channel, info] : summary.channel_info) {
        if (info.second - info.first > g_conf()->mon_log_max) {
            ...
            dout(10) << __func__ << " pruning channel " << channel << " " << from << " -> " << to << dendl;
...
bool LogMonitor::should_stash_full()
{
    ...
    auto period = std::min<uint64_t>(g_conf()->mon_log_full_interval, g_conf()->mon_max_log_epochs);
...
{
    if (!mon.is_leader()) return 0;

    unsigned max = g_conf()->mon_max_log_epochs;
    version_t version = get_last_committed();
    if (version > max) return version - max;
    return 0;
}
...
bool LogMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    case MSG_LOG: return preprocess_log(op);
...
bool LogMonitor::prepare_update(MonOpRequestRef op)
{
    ...
    case MSG_LOG: return prepare_log(op);
...
bool LogMonitor::preprocess_log(MonOpRequestRef op)
{
    ...
    for (auto p = m->entries.begin(); p != m->entries.end(); ++p) {
        if (!summary.contains(p->key())) num_new++;
    }
    if (!num_new) {
        ...
        if (!summary.tail_by_channel.empty()) {
            ...
            if (channel == "*") {
                ...
                for (auto& p : summary.channel_info) {
                    ...
                    dout(10) << __func__ << " channel " << p.first << " from " << from << " to " << to << dendl;
...
bool LogMonitor::prepare_command(MonOpRequestRef op)
{
    ...
    if (prefix == "log") {
        ...
        wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, 0, string(), get_last_committed() + 1));
        return true;
    }
...
{
    dout(10) << __func__ << dendl;
    for (map<string, xlist<Subscription*>*>::iterator i = mon.session_map.subs.begin(); i != mon.session_map.subs.end();
         ++i) {
        for (xlist<Subscription*>::iterator j = i->second->begin(); !j.end(); ++j) {
            if (sub_name_to_id((*j)->type) >= 0) check_sub(*j);
        }
    }
}
...
void LogMonitor::check_sub(Subscription* s)
{
    ...
    if (s->next > summary_version) {
        dout(10) << __func__ << " client " << s->session->name << " requested version (" << s->next
                 << ") is greater than ours (" << summary_version << "), which means we already sent him"
    ...
    dout(10) << __func__ << " sending message to " << s->session->name << " with " << mlog->entries.size() << " entries"
...
void LogMonitor::_create_sub_incremental(MLog* mlog, int level, version_t sv)
{
    ...
    while (sv && sv <= summary_ver) {
        ...
        while ((num == -2 && !p.end()) || (num >= 0 && num--)) {
            ...
            le.decode(p);
            if (le.prio < level) {
                dout(20) << __func__ << " requested " << level << ", skipping " << le << dendl;
                continue;
            }
            mlog->entries.push_back(le);
        }
...
void LogMonitor::update_log_channels()
{
    ...
    int r = get_conf_str_map_helper(
        g_conf().get_val<string>("mon_cluster_log_to_syslog"), oss, &channels.log_to_syslog, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
        derr << __func__ << " error parsing 'mon_cluster_log_to_syslog'" << dendl;
        return;
    }

    r = get_conf_str_map_helper(g_conf().get_val<string>("mon_cluster_log_to_syslog_level"),
                                oss,
                                &channels.syslog_level,
    ...
    r = get_conf_str_map_helper(g_conf().get_val<string>("mon_cluster_log_to_syslog_facility"),
                                oss,
                                &channels.syslog_facility,
                                CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
    ...
    r = get_conf_str_map_helper(
        g_conf().get_val<string>("mon_cluster_log_file"), oss, &channels.log_file, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
    ...
    r = get_conf_str_map_helper(
        g_conf().get_val<string>("mon_cluster_log_file_level"), oss, &channels.log_file_level, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
    ...
    r = get_conf_str_map_helper(
        g_conf().get_val<string>("mon_cluster_log_to_graylog"), oss, &channels.log_to_graylog, CLOG_CONFIG_DEFAULT_KEY);
    if (r < 0) {
        derr << __func__ << " error parsing 'mon_cluster_log_to_graylog'" << dendl;
        return;
    }

    r = get_conf_str_map_helper(g_conf().get_val<string>("mon_cluster_log_to_graylog_host"),
                                oss,
    ...
    r = get_conf_str_map_helper(g_conf().get_val<string>("mon_cluster_log_to_graylog_port"),
                                oss,
    ...
    r = get_conf_str_map_helper(g_conf().get_val<string>("mon_cluster_log_to_journald"),
                                oss,
...
void LogMonitor::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    if (changed.count("mon_cluster_log_to_syslog") || changed.count("mon_cluster_log_to_syslog_level") ||
        changed.count("mon_cluster_log_to_syslog_facility") || changed.count("mon_cluster_log_file") ||
        changed.count("mon_cluster_log_file_level") || changed.count("mon_cluster_log_to_graylog") ||
        changed.count("mon_cluster_log_to_graylog_host") || changed.count("mon_cluster_log_to_graylog_port") ||
        changed.count("mon_cluster_log_to_journald") || changed.count("mon_cluster_log_to_file")) {
        update_log_channels();
    }
}
