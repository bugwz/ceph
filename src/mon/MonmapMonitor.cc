...
{
    return *_dout << "mon." << mon.name << "@" << mon.rank << "(" << mon.get_state_name() << ").monmap v"
                  << mon.monmap->epoch << " ";
}
...
void MonmapMonitor::update_from_paxos(bool* need_bootstrap)
{
    ...
    if (version <= mon.monmap->get_epoch()) return;
    ...
    string val;
    if (mon.store->read_meta("min_mon_release", &val) < 0 || val.size() == 0 ||
        atoi(val.c_str()) != (int)ceph_release()) {
        dout(10) << __func__ << " updating min_mon_release meta" << dendl;
        mon.store->write_meta("min_mon_release", stringify(ceph_release()));
    }
...
void MonmapMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
    ...
    ceph_assert(mon.monmap->epoch + 1 == pending_map.epoch || pending_map.epoch == 1);   // special case mkfs!
    bufferlist bl;
...
void MonmapMonitor::apply_mon_features(const mon_feature_t& features, ceph_release_t min_mon_release)
{
    if (!is_writeable()) {
    ...
    mon_feature_t new_features = (pending_map.persistent_features ^ (features & ceph::features::mon::get_persistent()));

    if (new_features.empty() && pending_map.min_mon_release == min_mon_release) {
        dout(10) << __func__ << " min_mon_release (" << (int)min_mon_release << ") and features (" << features
                 << ") match" << dendl;
        return;
    }

    if (!new_features.empty()) {
        dout(1) << __func__ << " applying new features " << new_features << ", had " << pending_map.persistent_features
                << ", will have " << (new_features | pending_map.persistent_features) << dendl;
        pending_map.persistent_features |= new_features;
    }
    if (min_mon_release > pending_map.min_mon_release) {
        dout(1) << __func__ << " increasing min_mon_release to " << to_integer<int>(min_mon_release) << " ("
                << min_mon_release << ")" << dendl;
...
void MonmapMonitor::on_active()
{
    ...
    apply_mon_features(mon.get_quorum_mon_features(), mon.quorum_min_mon_release);
...
bool MonmapMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    case MSG_MON_JOIN: return preprocess_join(op);
...
void MonmapMonitor::dump_info(Formatter* f)
{
    ...
    for (set<int>::iterator q = mon.get_quorum().begin(); q != mon.get_quorum().end(); ++q) f->dump_int("mon", *q);
    f->close_section();
}
...
bool MonmapMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "mon stat") {
        if (f) {
            ...
            ss << ", election epoch " << mon.get_epoch() << ", leader " << mon.get_leader() << " "
               << mon.get_leader_name() << ", quorum " << mon.get_quorum() << " " << mon.get_quorum_names();
        }
    ...
    else if (prefix == "mon getmap" || prefix == "mon dump") {
        ...
        string with_value;
        if (cmd_getval(cmdmap, "with_value", with_value) && with_value == "--with-value") {
        ...
        if (f) {
            ...
            ds << "all features" << std::endl << "\tsupported: ";
            print_feature(supported, nullptr);
            ds << std::endl << "\tpersistent: ";
            print_feature(persistent, nullptr);
            ds << std::endl << std::endl;

            ds << "on current monmap (epoch " << p->get_epoch() << ")" << std::endl << "\tpersistent: ";
...
bool MonmapMonitor::prepare_update(MonOpRequestRef op)
{
    ...
    case MSG_MON_JOIN: return prepare_join(op);
    default: ceph_abort();
    }
...
bool MonmapMonitor::prepare_command(MonOpRequestRef op)
{
    ...
    if (prefix == "mon add") {
        ...
        if (locationvec.size() && !mon.get_quorum_mon_features().contains_all(ceph::features::mon::FEATURE_PINGING)) {
            err = -ENOTSUP;
        ...
        if (monmap.persistent_features.contains_all(ceph::features::mon::FEATURE_NAUTILUS)) {
        ...
        do {
            if (monmap.contains(name)) {
                if (monmap.get_addrs(name) == addrs) {
                    ...
                    ss << "mon." << name << " already exists at address " << monmap.get_addrs(name);
                }
            }
            else if (monmap.contains(addrs)) {
                // we established on the previous branch that name is different
                ss << "mon." << monmap.get_name(addrs) << " already exists at address " << addr;
            }
            else {
                // go ahead and add
                break;
        ...
        if (pending_map.stretch_mode_enabled && name == pending_map.tiebreaker_mon) {
            err = -EINVAL;
        ...
        if (!sure) {
            ss << "please specify '--yes-i-really-mean-it' if you "
               << "really, **really** want to set feature '" << feature << "' in the monmap.";
            err = -EPERM;
            goto reply;
        }
        ...
        if (!mon.get_quorum_mon_features().contains_all(feature)) {
            ss << "current quorum does not support feature '" << feature
               << "'; supported features: " << mon.get_quorum_mon_features();
            err = -EINVAL;
            goto reply;
        }
        ...
        if (monmap.persistent_features.contains_all(feature)) {
            dout(10) << __func__ << " feature '" << feature << "' already set on monmap; no-op." << dendl;
            goto reply;
        }
        ...
        string name;
        int64_t rank;
        if (!cmd_getval(cmdmap, "name", name) || !cmd_getval(cmdmap, "rank", rank)) {
            err = -EINVAL;
            goto reply;
        }
        ...
        string name;
        string addrs;
        if (!cmd_getval(cmdmap, "name", name) || !cmd_getval(cmdmap, "addrs", addrs)) {
            err = -EINVAL;
            goto reply;
        }
        ...
        string name;
        int64_t weight;
        if (!cmd_getval(cmdmap, "name", name) || !cmd_getval(cmdmap, "weight", weight)) {
            err = -EINVAL;
            goto reply;
        }
        if (!pending_map.contains(name)) {
        ...
        for (auto& i : pending_map.mon_info) {
            if (i.second.public_addrs.v.size() == 1 && i.second.public_addrs.front().is_legacy() &&
        ...
        if (!mon.get_quorum_mon_features().contains_all(ceph::features::mon::FEATURE_PINGING)) {
            err = -ENOTSUP;
        ...
        if (pending_map.strategy != MonMap::DISALLOW && pending_map.strategy != MonMap::CONNECTIVITY) {
        ...
        // TODO: validate location in crush map
        if (!loc.size()) {
            ss << "We could not parse your input location to anything real; " << argvec << " turned into an empty map!";
            err = -EINVAL;
            goto reply;
        }
        // TODO: validate location against any existing stretch config
        ...
        if (!mon.osdmon()->is_readable()) {
            dout(10) << __func__ << ": waiting for osdmon readable to inspect crush barrier" << dendl;
        ...
        string stretch_bucket_divider = mon.osdmon()->osdmap.crush->get_type_name(stretch_divider_id);

        const auto& new_loc_i = new_info.crush_loc.find(stretch_bucket_divider);
        if (new_loc_i == new_info.crush_loc.end()) {
            ss << "mon." << name << " has a specificed location, but not a " << stretch_bucket_divider
               << ", which is the stretch divider";
            err = -EINVAL;
            goto reply;
        }
        ...
        for (const auto& mii : pending_map.mon_info) {
            ...
            if (other_loc == new_loc && mii.first != existing_tiebreaker_info_i->first) {
                matching_mons.insert(mii.first);
            }
        }
        if (!matching_mons.empty()) {
            ss << "mon." << name << " has location " << new_loc_i->second << ", which matches mons " << matching_mons
               << " on the " << stretch_bucket_divider
        ...
        if (!mon.osdmon()->is_writeable()) {
            dout(10) << __func__ << ":  waiting for osdmon writeable for stretch mode" << dendl;
        ...
        }
        {
            ...
            mon.osdmon()->try_enable_stretch_mode_pools(ss, &okay, &errcode, &pools, new_crush_rule);
            if (!okay) {
                err = errcode;
                goto reply;
            }
            try_enable_stretch_mode(ss, &okay, &errcode, false, tiebreaker_mon, dividing_bucket);
            if (!okay) {
                err = errcode;
                goto reply;
            }
            mon.osdmon()->try_enable_stretch_mode(
                ss, &okay, &errcode, false, dividing_bucket, 2, pools, new_crush_rule);
            if (!okay) {
                err = errcode;
                goto reply;
            }
            // everything looks good, actually commit the changes!
            try_enable_stretch_mode(ss, &okay, &errcode, true, tiebreaker_mon, dividing_bucket);
            mon.osdmon()->try_enable_stretch_mode(ss,
                                                  &okay,
                                                  &errcode,
                                                  true,
                                                  dividing_bucket,
...
void MonmapMonitor::try_enable_stretch_mode(stringstream& ss, bool* okay, int* errcode, bool commit,
                                            const string& tiebreaker_mon, const string& dividing_bucket)
{
    ...
    *okay = false;
    ...
    for (const auto& mii : mon.monmap->mon_info) {
        ...
        if (bi == mi.crush_loc.end()) {
            ss << "Could not find location entry for " << dividing_bucket << " on monitor " << mi.name;
            *errcode = -EINVAL;
            ceph_assert(!commit);
            return;
        }
    ...
    for (auto& i : buckets) {
        ...
        if (bucket1 != i.second && bucket2.empty()) {
            bucket2 = i.second;
        }
        if (bucket1 != i.second && bucket2 != i.second) {
            ss << "There are too many monitor buckets for stretch mode, found " << bucket1 << "," << bucket2 << ","
               << i.second;
            *errcode = -EINVAL;
            ceph_assert(!commit);
            return;
    ...
    if (bucket1.empty() || bucket2.empty()) {
        ss << "There are not enough monitor buckets for stretch mode;"
           << " must have at least 2 plus the tiebreaker but only found " << (bucket1.empty() ? bucket1 : bucket2);
        *errcode = -EINVAL;
        ceph_assert(!commit);
        return;
    }
    if (tiebreaker_bucket == bucket1 || tiebreaker_bucket == bucket2) {
        ss << "The named tiebreaker monitor " << tiebreaker_mon << " is in the same CRUSH bucket " << tiebreaker_bucket
           << " as other monitors";
        *errcode = -EINVAL;
        ceph_assert(!commit);
        return;
...
bool MonmapMonitor::preprocess_join(MonOpRequestRef op)
{
    ...
    MonSession* session = op->get_session();
    if (!session || !session->is_capable("mon", MON_CAP_W | MON_CAP_X)) {
        dout(10) << " insufficient caps" << dendl;
        return true;
    }

    const auto name_info_i = pending_map.mon_info.find(join->name);
    if (name_info_i != pending_map.mon_info.end() && !name_info_i->second.public_addrs.front().is_blank_ip() &&
    ...
    if (!addr_name.empty() && addr_name == join->name &&
        (!join->force_loc || join->crush_loc.empty() || pending_map.mon_info[addr_name].crush_loc == join->crush_loc)) {
        dout(10) << " already have " << join->addrs << dendl;
        return true;
    }
    if (pending_map.stretch_mode_enabled && join->crush_loc.empty() &&
        (addr_name.empty() || pending_map.mon_info[addr_name].crush_loc.empty())) {
        dout(10) << "stretch mode engaged but no source of crush_loc" << dendl;
        mon.clog->info() << join->name << " attempted to join from " << join->name << ' ' << join->addrs
...
bool MonmapMonitor::prepare_join(MonOpRequestRef op)
{
    ...
    if (pending_map.contains(join->name)) pending_map.remove(join->name);
    pending_map.add(join->name, join->addrs);
    pending_map.mon_info[join->name].crush_loc =
        ((join->force_loc || existing_loc.empty()) ? join->crush_loc : existing_loc);
    pending_map.last_changed = ceph_clock_now();
    return true;
}
...
int MonmapMonitor::get_monmap(bufferlist& bl)
{
    ...
    if (!mon.store->exists(get_service_name(), stringify(latest_ver))) return -ENOENT;
...
void MonmapMonitor::check_subs()
{
    ...
    mon.with_session_map([this, &type](const MonSessionMap& session_map) {
        ...
        if (subs == session_map.subs.end()) return;
...
void MonmapMonitor::check_sub(Subscription* sub)
{
    ...
    if (sub->next <= epoch) {
        ...
        if (sub->onetime) {
            mon.with_session_map([sub](MonSessionMap& session_map) { session_map.remove_sub(sub); });
        }
        else {
            sub->next = epoch + 1;
        }
    ...
    if (mon.monmap->created.is_zero()) {
        ...
        for (version_t v = 1; v <= get_last_committed(); v++) {
            ...
            if (!m.last_changed.is_zero()) {
                dout(10) << __func__ << " first monmap with last_changed is " << v << " with " << m.last_changed
                         << dendl;
