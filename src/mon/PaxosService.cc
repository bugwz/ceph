...
static ostream& _prefix(std::ostream* _dout, Monitor& mon, Paxos& paxos, string service_name, version_t fc,
                        version_t lc)
{
    return *_dout << "mon." << mon.name << "@" << mon.rank << "(" << mon.get_state_name() << ").paxosservice("
                  << service_name << " " << fc << ".." << lc << ") ";
}
...
bool PaxosService::dispatch(MonOpRequestRef op)
{
    ...
    dout(10) << __func__ << " " << m << " " << *m << " from " << m->get_orig_source_inst() << " con "
    ...
    if (m->rx_election_epoch && m->rx_election_epoch < mon.get_epoch()) {
        dout(10) << " discarding forwarded message from previous election epoch " << m->rx_election_epoch << " < "
                 << mon.get_epoch() << dendl;
        return true;
    }
    ...
    if (m->get_connection() && !m->get_connection()->is_connected() && m->get_connection() != mon.con_self &&
    ...
    // preprocess
    if (preprocess_query(op)) return true;   // easy!
...
void PaxosService::propose_pending()
{
    ...
    if (should_stash_full()) encode_full(t);
    ...
    public:
        explicit C_Committed(PaxosService* p)
            : ps(p)
        {}
...
void PaxosService::_active()
{
    ...
    if (!is_active()) {
        ...
        public:
            explicit C_Active(PaxosService* s)
                : svc(s)
            {}
            void finish(int r) override
            {
    ...
    if (mon.is_leader()) upgrade_format();
    ...
    to_remove = [to_remove, trim_to, this] {
        ...
        if (to_remove < trim_max * 1.5) {
            dout(10) << __func__ << " trim to " << trim_to << " would only trim " << to_remove
                     << " > paxos_service_trim_max, limiting to " << trim_max << dendl;
            return trim_max;
        }
...
void PaxosService::trim(MonitorDBStore::TransactionRef t, version_t from, version_t to)
{
