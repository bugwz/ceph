...
{
    o << "rank=" << c.rank << ",epoch=" << c.epoch << ",version=" << c.epoch_version << ", current links: " << c.current
      << ", history: " << c.history;
    return o;
}
...
{
    o << "rank=" << c.rank << ", epoch=" << c.epoch << ", version=" << c.version << ", half_life=" << c.half_life
      << ", reports: " << c.peer_reports;
    return o;
}
...
void ConnectionTracker::report_live_connection(int peer_rank, double units_alive)
{
    ...
    if (i == my_reports.history.end()) {
        ldout(cct, 30) << "couldn't find: " << peer_rank << " in my_reports.history"
                       << "... inserting: "
                       << "(" << peer_rank << ", 1" << dendl;
    ...
    pscore = pscore * (1 - units_alive / (2 * half_life)) + (units_alive / (2 * half_life));
...
void ConnectionTracker::report_dead_connection(int peer_rank, double units_dead)
{
    ...
    double& pscore = i->second;
    ldout(cct, 30) << "adding new pscore to my_reports" << dendl;
    pscore = pscore * (1 - units_dead / (2 * half_life)) - (units_dead / (2 * half_life));
...
void ConnectionTracker::get_total_connection_score(int peer_rank, double* rating, int* live_count) const
{
    ldout(cct, 30) << __func__ << dendl;
    *rating = 0;
    *live_count = 0;
...
{
    ldout(cct, 20) << __func__ << " " << rank_removed << " new_rank: " << new_rank << dendl;
    ...
    ceph_assert((peer_reports.size() == starting_size) || (peer_reports.size() + 1 == starting_size));
...
bool ConnectionTracker::is_clean(int mon_rank, int monmap_size)
{
    ...
    if (rank != mon_rank || my_reports.rank != mon_rank) {
        return false;
    }
...
void ConnectionTracker::decode(bufferlist::const_iterator& bl)
{
    ...
    DECODE_FINISH(bl);
    if (rank >= 0) my_reports = peer_reports[rank];
}
