...
struct MonSession : public RefCountedObject
{
    ...
    std::vector<std::string> get_allowed_fs_names() const { return caps.allowed_fs_names(); }

    bool fs_name_capable(std::string_view fsname, __u8 mask) { return caps.fs_name_capable(entity_name, fsname, mask); }

    const entity_addr_t& get_peer_socket_addr() { return socket_addr; }
    ...
    void dump(ceph::Formatter* f) const
    {
        ...
        f->dump_string("con_features_release", ceph_release_name(ceph_release_from_features(con_features)));
...
struct MonSessionMap
{
    ...
    unsigned get_size() const { return sessions.size(); }
    ...
    MonSession* new_session(const entity_name_t& n, const entity_addrvec_t& av, Connection* c)
    {
    ...
    MonSession* get_random_osd_session(OSDMap* osdmap)
    {
        ...
        while (backward || forward) {
            if (backward) {
                if (osdmap->is_up(b->first) && osdmap->get_addrs(b->first) == b->second->con->get_peer_addrs()) {
                    s = b->second;
                    break;
                }
                if (b != by_osd.begin())
    ...
    void add_update_sub(MonSession* s, const std::string& what, version_t start, bool onetime, bool incremental_onetime)
    {
        ...
        if (s->sub_map.count(what)) {
            ...
            if (!subs.count(what)) subs[what] = new xlist<Subscription*>;
...
{
    out << "MonSession(" << s.name << " " << s.addrs << " is " << (s.closed ? "closed" : "open") << " " << s.caps
