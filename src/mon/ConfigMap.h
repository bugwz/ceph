...
struct OptionMask
{
    ...
    bool empty() const { return location_type.size() == 0 && location_value.size() == 0 && device_class.size() == 0; }
...
struct ConfigMap
{
    ...
    std::map<std::string, std::string, std::less<>> generate_entity_map(
        const EntityName& name, const std::map<std::string, std::string>& crush_location, const CrushWrapper* crush,
        const std::string& device_class, std::map<std::string, std::pair<std::string, const MaskedOption*>>* src = 0);

    void parse_key(const std::string& key, std::string* name, std::string* who);
    static bool parse_mask(const std::string& in, std::string* section, OptionMask* mask);
};
