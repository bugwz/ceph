...
void PGMapDigest::dump(ceph::Formatter* f) const
{
    ...
    for (auto& p : pg_pool_sum) {
        ...
        if (q != num_pg_by_pool.end()) f->dump_unsigned("num_pg", q->second);
        p.second.dump(f);
        f->close_section();
    }
    f->close_section();
...
{
    if (f) f->open_array_section("pgs_by_state");

    // list is descending numeric order (by count)
    std::multimap<int, uint64_t> state_by_count;   // count -> state
    for (auto p = num_pg_by_state.begin(); p != num_pg_by_state.end(); ++p) {
    ...
    if (f) {
        ...
        *out << "    pools:   " << pg_pool_sum.size() << " pools, " << num_pg << " pgs\n";
        *out << "    objects: " << si_u_t(pg_sum.stats.sum.num_objects) << " objects, "
             << byte_u_t(pg_sum.stats.sum.num_bytes) << "\n";
        *out << "    usage:   " << byte_u_t(osd_sum.statfs.get_used_raw()) << " used, "
             << byte_u_t(osd_sum.statfs.available) << " / " << byte_u_t(osd_sum.statfs.total) << " avail\n";
        *out << "    pgs:     ";
    }
    ...
    if (!f) {
        ...
        for (auto p = state_by_count.rbegin(); p != state_by_count.rend(); ++p) {
            ...
            *out << std::setw(max_width) << p->first << " " << pg_state_string(p->second) << "\n";
    ...
    if (!f && (ss_client_io.str().length() || ss_rec_io.str().length() || ss_cache_io.str().length())) {
        *out << "\n \n";
        *out << "  io:\n";
    }
...
void PGMapDigest::print_oneline_summary(ceph::Formatter* f, ostream* out) const
{
    ...
    if (f) f->open_array_section("num_pg_by_state");
    for (auto p = num_pg_by_state.begin(); p != num_pg_by_state.end(); ++p) {
        if (f) {
    ...
    if (out)
        *out << num_pg << " pgs: " << states << "; " << byte_u_t(pg_sum.stats.sum.num_bytes) << " data, "
             << byte_u_t(osd_sum.statfs.get_used()) << " used, " << byte_u_t(osd_sum.statfs.available) << " / "
    ...
    pos_delta.floor(0);
    if (pos_delta.stats.sum.num_rd || pos_delta.stats.sum.num_wr) {
        if (out) *out << "; ";
        if (pos_delta.stats.sum.num_rd) {
            int64_t rd = (pos_delta.stats.sum.num_rd_kb << 10) / (double)stamp_delta;
            if (out) *out << byte_u_t(rd) << "/s rd, ";
            if (f) f->dump_unsigned("read_bytes_sec", rd);
        }
        if (pos_delta.stats.sum.num_wr) {
            int64_t wr = (pos_delta.stats.sum.num_wr_kb << 10) / (double)stamp_delta;
            if (out) *out << byte_u_t(wr) << "/s wr, ";
            if (f) f->dump_unsigned("write_bytes_sec", wr);
        }
        int64_t iops = (pos_delta.stats.sum.num_rd + pos_delta.stats.sum.num_wr) / (double)stamp_delta;
        if (out) *out << si_u_t(iops) << " op/s";
        if (f) f->dump_unsigned("io_sec", iops);
    }

    list<string> sl;
    overall_recovery_summary(f, &sl);
    if (out)
        for (auto p = sl.begin(); p != sl.end(); ++p) *out << "; " << *p;
    std::stringstream ssr;
    overall_recovery_rate_summary(f, &ssr);
    if (out && ssr.str().length()) *out << "; " << ssr.str() << " recovering";
}

void PGMapDigest::get_recovery_stats(double* misplaced_ratio, double* degraded_ratio, double* inactive_pgs_ratio,
                                     double* unknown_pgs_ratio) const
{
    if (pg_sum.stats.sum.num_objects_degraded && pg_sum.stats.sum.num_object_copies > 0) {
        *degraded_ratio = (double)pg_sum.stats.sum.num_objects_degraded / (double)pg_sum.stats.sum.num_object_copies;
    }
    else {
        *degraded_ratio = 0;
    }
    if (pg_sum.stats.sum.num_objects_misplaced && pg_sum.stats.sum.num_object_copies > 0) {
        *misplaced_ratio = (double)pg_sum.stats.sum.num_objects_misplaced / (double)pg_sum.stats.sum.num_object_copies;
    }
    else {
        *misplaced_ratio = 0;
    }
...
void PGMapDigest::recovery_summary(ceph::Formatter* f, list<string>* psl, const pool_stat_t& pool_sum) const
{
    if (pool_sum.stats.sum.num_objects_degraded && pool_sum.stats.sum.num_object_copies > 0) {
        double pc = (double)pool_sum.stats.sum.num_objects_degraded / (double)pool_sum.stats.sum.num_object_copies *
                    (double)100.0;
        char b[20];
        ...
        if (f) {
            ...
            ostringstream ss;
            ss << pool_sum.stats.sum.num_objects_degraded << "/" << pool_sum.stats.sum.num_object_copies
               << " objects degraded (" << b << "%)";
            psl->push_back(ss.str());
        }
    }
    if (pool_sum.stats.sum.num_objects_misplaced && pool_sum.stats.sum.num_object_copies > 0) {
        double pc = (double)pool_sum.stats.sum.num_objects_misplaced / (double)pool_sum.stats.sum.num_object_copies *
                    (double)100.0;
        char b[20];
        snprintf(b, sizeof(b), "%.3lf", pc);
        if (f) {
            ...
            ostringstream ss;
            ss << pool_sum.stats.sum.num_objects_misplaced << "/" << pool_sum.stats.sum.num_object_copies
               << " objects misplaced (" << b << "%)";
            psl->push_back(ss.str());
        }
    }
    if (pool_sum.stats.sum.num_objects_unfound && pool_sum.stats.sum.num_objects) {
        double pc =
            (double)pool_sum.stats.sum.num_objects_unfound / (double)pool_sum.stats.sum.num_objects * (double)100.0;
        char b[20];
        snprintf(b, sizeof(b), "%.3lf", pc);
        if (f) {
            ...
            ostringstream ss;
            ss << pool_sum.stats.sum.num_objects_unfound << "/" << pool_sum.stats.sum.num_objects
               << " objects unfound (" << b << "%)";
            psl->push_back(ss.str());
        }
    }
...
void PGMapDigest::recovery_rate_summary(ceph::Formatter* f, ostream* out, const pool_stat_t& delta_sum,
                                        utime_t delta_stamp) const
{
    ...
    pos_delta.floor(0);
    if (pos_delta.stats.sum.num_objects_recovered || pos_delta.stats.sum.num_bytes_recovered ||
        ...
        if (f) {
            ...
            *out << byte_u_t(bps) << "/s";
            if (pos_delta.stats.sum.num_keys_recovered) *out << ", " << si_u_t(kps) << " keys/s";
...
void PGMapDigest::pool_recovery_rate_summary(ceph::Formatter* f, ostream* out, uint64_t poolid) const
{
    auto p = per_pool_sum_delta.find(poolid);
    if (p == per_pool_sum_delta.end()) return;
...
void PGMapDigest::pool_recovery_summary(ceph::Formatter* f, list<string>* psl, uint64_t poolid) const
{
    auto p = pg_pool_sum.find(poolid);
    if (p == pg_pool_sum.end()) return;

    recovery_summary(f, psl, p->second);
}

void PGMapDigest::client_io_rate_summary(ceph::Formatter* f, ostream* out, const pool_stat_t& delta_sum,
                                         utime_t delta_stamp) const
{
    pool_stat_t pos_delta = delta_sum;
    pos_delta.floor(0);
...
void PGMapDigest::pool_client_io_rate_summary(ceph::Formatter* f, ostream* out, uint64_t poolid) const
{
    auto p = per_pool_sum_delta.find(poolid);
    if (p == per_pool_sum_delta.end()) return;
...
void PGMapDigest::cache_io_rate_summary(ceph::Formatter* f, ostream* out, const pool_stat_t& delta_sum,
                                        utime_t delta_stamp) const
{
    pool_stat_t pos_delta = delta_sum;
    pos_delta.floor(0);
...
void PGMapDigest::pool_cache_io_rate_summary(ceph::Formatter* f, ostream* out, uint64_t poolid) const
{
    auto p = per_pool_sum_delta.find(poolid);
    if (p == per_pool_sum_delta.end()) return;
...
ceph_statfs PGMapDigest::get_statfs(OSDMap& osdmap, boost::optional<int64_t> data_pool) const
{
    ceph_statfs statfs;
    bool filter = false;
...
void PGMapDigest::dump_pool_stats_full(const OSDMap& osd_map, stringstream* ss, ceph::Formatter* f, bool verbose) const
{
    TextTable tbl;
    ...
    for (auto p = osd_map.get_pools().begin(); p != osd_map.get_pools().end(); ++p) {
        int64_t pool_id = p->first;
        if ((pool_id < 0) || (pg_pool_sum.count(pool_id) == 0)) continue;
        ...
        int ruleno = osd_map.crush->find_rule(pool->get_crush_rule(), pool->get_type(), pool->get_size());
        int64_t avail;
        ...
        if (f) {
            ...
            tbl << pool_name << pool_id << pool_pg_num;
        }
        ...
        dump_object_stat_sum(tbl, f, stat, avail, raw_used_rate, verbose, per_pool, per_pool_omap, pool);
        if (f) {
...
void PGMapDigest::dump_cluster_stats(stringstream* ss, ceph::Formatter* f, bool verbose) const
{
    if (f) {
        f->open_object_section("stats");
        f->dump_int("total_bytes", osd_sum.statfs.total);
        ...
        for (auto& i : osd_sum_by_class) {
            ...
            f->dump_float("total_used_raw_ratio", i.second.statfs.get_used_raw_ratio());
            f->close_section();
        }
        f->close_section();
    }
        ...
        for (auto& i : osd_sum_by_class) {
            tbl << i.first;
            tbl << stringify(byte_u_t(i.second.statfs.total)) << stringify(byte_u_t(i.second.statfs.available))
                << stringify(byte_u_t(i.second.statfs.get_used()))
                << stringify(byte_u_t(i.second.statfs.get_used_raw()))
                << percentify(i.second.statfs.get_used_raw_ratio() * 100.0) << TextTable::endrow;
        }
        tbl << "TOTAL";
        tbl << stringify(byte_u_t(osd_sum.statfs.total)) << stringify(byte_u_t(osd_sum.statfs.available))
            << stringify(byte_u_t(osd_sum.statfs.get_used())) << stringify(byte_u_t(osd_sum.statfs.get_used_raw()))
            << percentify(osd_sum.statfs.get_used_raw_ratio() * 100.0) << TextTable::endrow;
...
int64_t PGMapDigest::get_pool_free_space(const OSDMap& osd_map, int64_t poolid) const
{
...
void PGMap::get_rules_avail(const OSDMap& osdmap, std::map<int, int64_t>* avail_map) const
{
    avail_map->clear();
    for (auto p : osdmap.get_pools()) {
        ...
        if (avail_map->count(ruleno) == 0) (*avail_map)[ruleno] = get_rule_avail(osdmap, ruleno);
    }
}
...
void PGMap::Incremental::dump(ceph::Formatter* f) const
{
    ...
    for (auto p = osd_stat_rm.begin(); p != osd_stat_rm.end(); ++p) f->dump_int("osd", *p);
    f->close_section();

    f->open_array_section("pg_removals");
    for (auto p = pg_remove.begin(); p != pg_remove.end(); ++p) f->dump_stream("pgid") << *p;
    f->close_section();
}
...
void PGMap::apply_incremental(CephContext* cct, const Incremental& inc)
{
    ...
    for (auto p = inc.pg_stat_updates.begin(); p != inc.pg_stat_updates.end(); ++p) {
    ...
    for (auto p = inc.pool_statfs_updates.begin(); p != inc.pool_statfs_updates.end(); ++p) {
        auto update_pool = p->first.first;
        auto update_osd = p->first.second;
        auto& statfs_inc = p->second;

        auto pool_statfs_iter = pool_statfs.find(std::make_pair(update_pool, update_osd));
    ...
    for (auto p = inc.get_osd_stat_updates().begin(); p != inc.get_osd_stat_updates().end(); ++p) {
        int osd = p->first;
    ...
    for (auto p = inc.pg_remove.begin(); p != inc.pg_remove.end(); ++p) {
    ...
    for (auto p = inc.get_osd_stat_rm().begin(); p != inc.get_osd_stat_rm().end(); ++p) {
    ...
    if (!stamp.is_zero() && !pg_sum_old.stats.sum.is_zero()) {
        ...
        auto smooth_intervals = cct ? cct->_conf.get_val<uint64_t>("mon_stat_smooth_intervals") : 1;
    ...
    for (auto p : deleted_pools) {
        if (cct) dout(20) << " deleted pool " << p << dendl;
        deleted_pool(p);
    }

    if (inc.osdmap_epoch) last_osdmap_epoch = inc.osdmap_epoch;
    if (inc.pg_scan) last_pg_scan = inc.pg_scan;
}
...
void PGMap::calc_stats()
{
    ...
    for (auto p = pool_statfs.begin(); p != pool_statfs.end(); ++p) {
        auto pool = p->first.first;
        pg_pool_sum[pool].add(p->second);
    }
    for (auto p = osd_stat.begin(); p != osd_stat.end(); ++p) stat_osd_add(p->first, p->second);
}

void PGMap::stat_pg_add(const pg_t& pgid, const pg_stat_t& s, bool sameosds)
{
    auto pool = pgid.pool();
    pg_sum.add(s);
    ...
    if ((s.state & PG_STATE_CREATING) && s.parent_split_bits == 0) {
    ...
    for (auto p = s.blocked_by.begin(); p != s.blocked_by.end(); ++p) {
        ++blocked_by_sum[*p];
    }
...
bool PGMap::stat_pg_sub(const pg_t& pgid, const pg_stat_t& s, bool sameosds)
{
    bool pool_erased = false;
    pg_sum.sub(s);
    ...
    ceph_assert(end >= 0);
    if (end == 0) num_pg_by_state.erase(s.state);
    ...
    if ((s.state & PG_STATE_CREATING) && s.parent_split_bits == 0) {
        ...
        if (s.acting_primary >= 0) {
            ...
            if (r[s.mapping_epoch].empty()) r.erase(s.mapping_epoch);
            if (r.empty()) creating_pgs_by_osd_epoch.erase(s.acting_primary);
        }
    }
    ...
    if (sameosds) return pool_erased;

    for (auto p = s.blocked_by.begin(); p != s.blocked_by.end(); ++p) {
        auto q = blocked_by_sum.find(*p);
        ceph_assert(q != blocked_by_sum.end());
        --q->second;
        if (q->second == 0) blocked_by_sum.erase(q);
    }
    ...
    for (auto p = s.acting.begin(); p != s.acting.end(); ++p) {
        ...
        oset.erase(pgid);
        if (oset.empty()) pg_by_osd.erase(*p);
        auto it = num_pg_by_osd.find(*p);
        if (it != num_pg_by_osd.end() && it->second.acting > 0) it->second.acting--;
    }
    for (auto p = s.up.begin(); p != s.up.end(); ++p) {
        auto& oset = pg_by_osd[*p];
        oset.erase(pgid);
        if (oset.empty()) pg_by_osd.erase(*p);
        if (actingset.count(*p)) continue;
        auto it = num_pg_by_osd.find(*p);
        if (it != num_pg_by_osd.end() && it->second.up_not_acting > 0) it->second.up_not_acting--;
    }

    if (s.up_primary >= 0) {
        auto it = num_pg_by_osd.find(s.up_primary);
        if (it != num_pg_by_osd.end() && it->second.primary > 0) it->second.primary--;
    }
    return pool_erased;
}
...
void PGMap::encode_digest(const OSDMap& osdmap, bufferlist& bl, uint64_t features)
{
...
{
    f->open_array_section("pool_stats");
    for (auto p = pg_pool_sum.begin(); p != pg_pool_sum.end(); ++p) {
        f->open_object_section("pool_stat");
        f->dump_int("poolid", p->first);
        auto q = num_pg_by_pool.find(p->first);
        if (q != num_pg_by_pool.end()) f->dump_unsigned("num_pg", q->second);
...
{
    f->open_array_section("osd_stats");
    for (auto q = osd_stat.begin(); q != osd_stat.end(); ++q) {
        f->open_object_section("osd_stat");
...
void PGMap::dump_pg_stats_plain(ostream& ss, const mempool::pgmap::unordered_map<pg_t, pg_stat_t>& pg_stats,
                                bool brief) const
{
    TextTable tab;
    ...
    for (auto i = pg_stats.begin(); i != pg_stats.end(); ++i) {
        const pg_stat_t& st(i->second);
        if (brief) {
            tab << i->first << pg_state_string(st.state) << st.up << st.up_primary << st.acting << st.acting_primary
                << TextTable::endrow;
        }
        else {
            ...
            tab << i->first << st.stats.sum.num_objects << st.stats.sum.num_objects_missing_on_primary
                << st.stats.sum.num_objects_degraded << st.stats.sum.num_objects_misplaced
                << st.stats.sum.num_objects_unfound << st.stats.sum.num_bytes << st.stats.sum.num_omap_bytes
                << st.stats.sum.num_omap_keys << st.log_size << st.ondisk_log_size << pg_state_string(st.state)
                << st.last_change << st.version << reported.str() << pg_vector_string(st.up) << st.up_primary
                << pg_vector_string(st.acting) << st.acting_primary << st.last_scrub << st.last_scrub_stamp
                << st.last_deep_scrub << st.last_deep_scrub_stamp << st.snaptrimq_len << TextTable::endrow;
        }
    }
...
void PGMap::dump_pool_stats(ostream& ss, bool header) const
{
    ...
    for (auto p = pg_pool_sum.begin(); p != pg_pool_sum.end(); ++p) {
        tab << p->first << p->second.stats.sum.num_objects << p->second.stats.sum.num_objects_missing_on_primary
            << p->second.stats.sum.num_objects_degraded << p->second.stats.sum.num_objects_misplaced
            << p->second.stats.sum.num_objects_unfound << p->second.stats.sum.num_bytes
            << p->second.stats.sum.num_omap_bytes << p->second.stats.sum.num_omap_keys << p->second.log_size
            << p->second.ondisk_log_size << TextTable::endrow;
    }
...
void PGMap::dump_pg_sum_stats(ostream& ss, bool header) const
{
    ...
    tab << "sum" << pg_sum.stats.sum.num_objects << pg_sum.stats.sum.num_objects_missing_on_primary
        << pg_sum.stats.sum.num_objects_degraded << pg_sum.stats.sum.num_objects_misplaced
        << pg_sum.stats.sum.num_objects_unfound << pg_sum.stats.sum.num_bytes << pg_sum.stats.sum.num_omap_bytes
        << pg_sum.stats.sum.num_omap_keys << pg_sum.log_size << pg_sum.ondisk_log_size << TextTable::endrow;
...
void PGMap::dump_osd_stats(ostream& ss) const
{
    ...
    for (auto p = osd_stat.begin(); p != osd_stat.end(); ++p) {
        tab << p->first << byte_u_t(p->second.statfs.get_used()) << byte_u_t(p->second.statfs.available)
            << byte_u_t(p->second.statfs.get_used_raw()) << byte_u_t(p->second.statfs.total) << p->second.hb_peers
            << get_num_pg_by_osd(p->first) << get_num_primary_pg_by_osd(p->first) << TextTable::endrow;
    }

    tab << "sum" << byte_u_t(osd_sum.statfs.get_used()) << byte_u_t(osd_sum.statfs.available)
        << byte_u_t(osd_sum.statfs.get_used_raw()) << byte_u_t(osd_sum.statfs.total) << TextTable::endrow;
                            ...
                            mempool::pgmap::unordered_map<pg_t, pg_stat_t>& stuck_pgs) const
{
    ...
    for (auto i = pg_stat.begin(); i != pg_stat.end(); ++i) {
        ...
        if ((types & STUCK_INACTIVE) && !(i->second.state & PG_STATE_ACTIVE)) {
            if (i->second.last_active < val) val = i->second.last_active;
        }

        if ((types & STUCK_UNCLEAN) && !(i->second.state & PG_STATE_CLEAN)) {
            if (i->second.last_clean < val) val = i->second.last_clean;
        }

        if ((types & STUCK_DEGRADED) && (i->second.state & PG_STATE_DEGRADED)) {
            if (i->second.last_undegraded < val) val = i->second.last_undegraded;
        }

        if ((types & STUCK_UNDERSIZED) && (i->second.state & PG_STATE_UNDERSIZED)) {
            if (i->second.last_fullsized < val) val = i->second.last_fullsized;
        }

        if ((types & STUCK_STALE) && (i->second.state & PG_STATE_STALE)) {
            if (i->second.last_unstale < val) val = i->second.last_unstale;
        }
...
bool PGMap::get_stuck_counts(const utime_t cutoff, map<string, int>& note) const
{
    ...
    for (auto i = pg_stat.begin(); i != pg_stat.end(); ++i) {
        if (!(i->second.state & PG_STATE_ACTIVE)) {
            if (i->second.last_active < cutoff) ++inactive;
        }
        if (!(i->second.state & PG_STATE_CLEAN)) {
            if (i->second.last_clean < cutoff) ++unclean;
        }
        if (i->second.state & PG_STATE_DEGRADED) {
            if (i->second.last_undegraded < cutoff) ++degraded;
        }
        if (i->second.state & PG_STATE_UNDERSIZED) {
            if (i->second.last_fullsized < cutoff) ++undersized;
        }
        if (i->second.state & PG_STATE_STALE) {
            if (i->second.last_unstale < cutoff) ++stale;
        }
    }

    if (inactive) note["stuck inactive"] = inactive;

    if (unclean) note["stuck unclean"] = unclean;

    if (undersized) note["stuck undersized"] = undersized;

    if (degraded) note["stuck degraded"] = degraded;

    if (stale) note["stuck stale"] = stale;
...
void PGMap::dump_stuck(ceph::Formatter* f, int types, utime_t cutoff) const
{
    ...
    for (auto i = stuck_pg_stats.begin(); i != stuck_pg_stats.end(); ++i) {
        f->open_object_section("pg_stat");
        f->dump_stream("pgid") << i->first;
        i->second.dump(f);
        f->close_section();
...
void PGMap::dump_stuck_plain(ostream& ss, int types, utime_t cutoff) const
{
    mempool::pgmap::unordered_map<pg_t, pg_stat_t> stuck_pg_stats;
    get_stuck_stats(types, cutoff, stuck_pg_stats);
    if (!stuck_pg_stats.empty()) dump_pg_stats_plain(ss, stuck_pg_stats, true);
}

int PGMap::dump_stuck_pg_stats(stringstream& ds, ceph::Formatter* f, int threshold, vector<string>& args) const
{
    int stuck_types = 0;
...
void PGMap::dump_osd_blocked_by_stats(ceph::Formatter* f) const
{
    ...
    for (auto i = blocked_by_sum.begin(); i != blocked_by_sum.end(); ++i) {
        f->open_object_section("osd");
        f->dump_int("id", i->first);
 ...
 */
void PGMap::update_delta(CephContext* cct, const utime_t ts, const pool_stat_t& old_pool_sum, utime_t* last_ts,
                         const pool_stat_t& current_pool_sum, pool_stat_t* result_pool_delta, utime_t* result_ts_delta,
                         mempool::pgmap::list<pair<pool_stat_t, utime_t>>* delta_avg_list)
{
    ...
    *last_ts = ts;         // @p ts becomes the last timestamp we saw
    ...
    delta_t = std::min(delta_t, utime_t(2 * (cct ? cct->_conf->mon_delta_reset_interval : 10), 0));
 ...
 * @param old_pool_sum  Previous stats sum
 */
void PGMap::update_one_pool_delta(CephContext* cct, const utime_t ts, const int64_t pool,
                                  const pool_stat_t& old_pool_sum)
{
    ...
    update_delta(cct,
                 ...
                 pg_pool_sum[pool],
                 &sum_delta.first,
                 &per_pool_sum_deltas_stamps[pool],
...
{
    for (auto it = pg_pool_sum_old.begin(); it != pg_pool_sum_old.end(); ++it) {
...
void PGMap::get_filtered_pg_stats(uint64_t state, int64_t poolid, int64_t osdid, bool primary, set<pg_t>& pgs) const
{
    for (auto i = pg_stat.begin(); i != pg_stat.end(); ++i) {
        if ((poolid >= 0) && (poolid != i->first.pool())) continue;
        if ((osdid >= 0) && !(i->second.is_acting_osd(osdid, primary))) continue;
...
void PGMap::dump_filtered_pg_stats(ostream& ss, set<pg_t>& pgs) const
{
    ...
    for (auto i = pgs.begin(); i != pgs.end(); ++i) {
        ...
        tab << *i << st.stats.sum.num_objects << st.stats.sum.num_objects_degraded << st.stats.sum.num_objects_misplaced
            << st.stats.sum.num_objects_unfound << st.stats.sum.num_bytes << st.stats.sum.num_omap_bytes
            << st.stats.sum.num_omap_keys << st.log_size << pg_state_string(st.state)
            << utimespan_str(now - st.last_change) << st.version << reported.str() << upstr.str() << actingstr.str()
            << st.last_scrub_stamp << st.last_deep_scrub_stamp << TextTable::endrow;
    }

    ss << tab;
}

void PGMap::dump_pool_stats_and_io_rate(int64_t poolid, const OSDMap& osd_map, ceph::Formatter* f,
                                        stringstream* rs) const
{
    ...
    if (!f && !rss.str().empty()) tss << "  recovery io " << rss.str() << "\n";
    if (f) {
    ...
    if (!f && !rss.str().empty()) tss << "  client io " << rss.str() << "\n";
    ...
    if (pool->is_tier()) {
        ...
        if (!f && !rss.str().empty()) tss << "  cache tier io " << rss.str() << "\n";
    }
    if (f) {
...
set<std::string> PGMap::osd_parentage(const OSDMap& osdmap, int id) const
{
    ...
    for (auto& [parent_bucket_type, parent_id] : loc) {
        ...
        if (parent_bucket_type != "root" && parent_bucket_type != reporter_subtree_level) {
...
void PGMap::get_health_checks(CephContext* cct, const OSDMap& osdmap, health_check_map_t* checks) const
{
    utime_t now = ceph_clock_now();
        ...
        for (const auto& i : pg_stat) {
            ...
            for (const auto& j : state_to_response) {
                ...
                if (pg_response.stuck_since) {
                    ...
                    if (last_whatever.is_zero() && pg_info.last_change >= cutoff) {
                        // still moving, ignore
                        continue;
                    }
                ...
                if (pg_response.stuck_since) {
                    ...
                    ss << ", current state " << pg_state_string(pg_info.state) << ", last acting " << pg_info.acting;
                }
                else {
                    ss << "pg " << pg_id << " is " << pg_state_string(pg_info.state);
                    ss << ", acting " << pg_info.acting;
                    if (pg_info.stats.sum.num_objects_unfound) {
                        ss << ", " << pg_info.stats.sum.num_objects_unfound << " unfound";
                    }
                }
                ...
                if (pg_info.state & PG_STATE_INCOMPLETE) {
                    ...
                    if (pi && pi->min_size > 1) {
                        ss << " (reducing pool " << osdmap.get_pool_name(pg_id.pool()) << " min_size from "
                           << (int)pi->min_size << " may help; search ceph.com/docs for 'incomplete')";
                    }
                }
    ...
    for (const auto& i : detected) {
        ...
        if (i.first == DEGRADED) {
            if (pg_sum.stats.sum.num_objects_degraded && pg_sum.stats.sum.num_object_copies > 0) {
                double pc = (double)pg_sum.stats.sum.num_objects_degraded / (double)pg_sum.stats.sum.num_object_copies *
                            (double)100.0;
                char b[20];
                snprintf(b, sizeof(b), "%.3lf", pc);
                ostringstream ss;
                ss << pg_sum.stats.sum.num_objects_degraded << "/" << pg_sum.stats.sum.num_object_copies
                   << " objects degraded (" << b << "%)";
        ...
        health_check_t* check = &checks->add(health_code, sev, summary, count);
    ...
    if (pg_sum.stats.sum.num_scrub_errors) {
        ...
        checks->add("OSD_SCRUB_ERRORS", HEALTH_ERR, ss.str(), pg_sum.stats.sum.num_scrub_errors);
    }
    ...
    if (pg_sum.stats.sum.num_large_omap_objects) {
        ...
        if (!detail.empty()) {
            ...
            auto& d = checks->add("LARGE_OMAP_OBJECTS", HEALTH_WARN, ss.str(), pg_sum.stats.sum.num_large_omap_objects);
            stringstream tip;
    ...
    // CACHE_POOL_NEAR_FULL
    {
        ...
        for (auto& p : pools) {
            if ((!p.second.target_max_objects && !p.second.target_max_bytes) || !pg_pool_sum.count(p.first)) {
                continue;
            }
            bool nearfull = false;
            ...
            uint64_t ratio =
                p.second.cache_target_full_ratio_micro +
                ((1000000 - p.second.cache_target_full_ratio_micro) * cct->_conf->mon_cache_target_full_warn_ratio);
            if (p.second.target_max_objects &&
                (uint64_t)(st.stats.sum.num_objects - st.stats.sum.num_objects_hit_set_archive) >
                    p.second.target_max_objects * (ratio / 1000000.0)) {
                ostringstream ss;
                ss << "cache pool '" << name << "' with " << si_u_t(st.stats.sum.num_objects)
                   << " objects at/near target max " << si_u_t(p.second.target_max_objects) << " objects";
                detail.push_back(ss.str());
                nearfull = true;
            }
            if (p.second.target_max_bytes &&
                (uint64_t)(st.stats.sum.num_bytes - st.stats.sum.num_bytes_hit_set_archive) >
                    p.second.target_max_bytes * (ratio / 1000000.0)) {
                ostringstream ss;
                ss << "cache pool '" << name << "' with " << byte_u_t(st.stats.sum.num_bytes) << " at/near target max "
        ...
        if (!detail.empty()) {
            ...
            auto& d = checks->add("CACHE_POOL_NEAR_FULL", HEALTH_WARN, ss.str(), num_pools);
            d.detail.swap(detail);
        }
    }
    ...
    if (num_in && min_pg_per_osd > 0 && osdmap.get_pools().size() > 0) {
        ...
        if (per < min_pg_per_osd && per) {
            ...
            checks->add("TOO_FEW_PGS", HEALTH_WARN, ss.str(), min_pg_per_osd - per);
        }
    }
    ...
    if (num_in && max_pg_per_osd > 0) {
        ...
        if (per > max_pg_per_osd) {
            ostringstream ss;
            ss << "too many PGs per OSD (" << per << " > max " << max_pg_per_osd << ")";
            checks->add("TOO_MANY_PGS", HEALTH_WARN, ss.str(), per - max_pg_per_osd);
        }
    }
    ...
    if (warn_too_few_osds && osdmap.get_num_osds() < osd_pool_default_size) {
        ostringstream ss;
        ss << "OSD count " << osdmap.get_num_osds() << " < osd_pool_default_size " << osd_pool_default_size;
        checks->add("TOO_FEW_OSDS", HEALTH_WARN, ss.str(), osd_pool_default_size - osdmap.get_num_osds());
    }
    ...
    if (warn_slow_ping_time > 0) {
        ...
        for (auto i : osd_stat) {
            for (auto j : i.second.hb_pingtime) {
                ...
                if (now.sec() - j.second.last_update > grace * 60) continue;
            ...
            if (i.second.num_shards_repaired > cct->_conf.get_val<uint64_t>("mon_osd_warn_num_repaired")) {
                ostringstream ss;
        ...
        if (!detail.empty()) {
            ...
            auto& d = checks->add("OSD_TOO_MANY_REPAIRS", HEALTH_WARN, ss.str(), detail.size());
            d.detail.swap(detail);
        }
        int max_detail = 10;
        for (auto& sback : boost::adaptors::reverse(back_sorted)) {
            ...
            max_detail--;
            ss << "Slow OSD heartbeats on back from osd." << sback.from << " [" << osd_parentage(osdmap, sback.from)
               << "]" << (osdmap.is_down(sback.from) ? " (down)" : "") << " to osd." << sback.to << " ["
               << osd_parentage(osdmap, sback.to) << "]" << (osdmap.is_down(sback.to) ? " (down)" : "") << " "
               << fixed_u_to_string(sback.pingtime, 3) << " msec" << (sback.improving ? " possibly improving" : "");
            detail_back.push_back(ss.str());
        }
        max_detail = 10;
        for (auto& sfront : boost::adaptors::reverse(front_sorted)) {
            ...
            ss << "Slow OSD heartbeats on front from osd." << sfront.from << " [" << osd_parentage(osdmap, sfront.from)
               << "]" << (osdmap.is_down(sfront.from) ? " (down)" : "") << " to osd." << sfront.to << " ["
               << osd_parentage(osdmap, sfront.to) << "]" << (osdmap.is_down(sfront.to) ? " (down)" : "") << " "
               << fixed_u_to_string(sfront.pingtime, 3) << " msec" << (sfront.improving ? " possibly improving" : "");
            detail_front.push_back(ss.str());
        }
        if (detail_back.size() != 0) {
            ostringstream ss;
            ss << "Slow OSD heartbeats on back (longest " << fixed_u_to_string(back_sorted.rbegin()->pingtime, 3)
               << "ms)";
            auto& d = checks->add("OSD_SLOW_PING_TIME_BACK", HEALTH_WARN, ss.str(), back_sorted.size());
            d.detail.swap(detail_back);
        }
        if (detail_front.size() != 0) {
            ostringstream ss;
            ss << "Slow OSD heartbeats on front (longest " << fixed_u_to_string(front_sorted.rbegin()->pingtime, 3)
               << "ms)";
            auto& d = checks->add("OSD_SLOW_PING_TIME_FRONT", HEALTH_WARN, ss.str(), front_sorted.size());
    ...
    if (!pg_stat.empty()) {
        ...
        const auto mon_pg_warn_min_objects = cct->_conf.get_val<int64_t>("mon_pg_warn_min_objects");
        const auto mon_pg_warn_min_pool_objects = cct->_conf.get_val<int64_t>("mon_pg_warn_min_pool_objects");
        const auto mon_pg_warn_max_object_skew = cct->_conf.get_val<double>("mon_pg_warn_max_object_skew");
        for (auto p = pg_pool_sum.begin(); p != pg_pool_sum.end(); ++p) {
            const pg_pool_t* pi = osdmap.get_pg_pool(p->first);
            if (!pi) continue;   // in case osdmap changes haven't propagated to PGMap yet
            const string& name = osdmap.get_pool_name(p->first);
            ...
            if (pi->get_pg_num_target() > pi->get_pgp_num_target() &&
                !(name.find(".DELETED") != string::npos && cct->_conf->mon_fake_pool_delete)) {
                ostringstream ss;
                ss << "pool " << name << " pg_num " << pi->get_pg_num_target() << " > pgp_num "
                   << pi->get_pgp_num_target();
                pgp_detail.push_back(ss.str());
            }
            int average_objects_per_pg = pg_sum.stats.sum.num_objects / pg_stat.size();
            if (average_objects_per_pg > 0 && pg_sum.stats.sum.num_objects >= mon_pg_warn_min_objects &&
                p->second.stats.sum.num_objects >= mon_pg_warn_min_pool_objects) {
                int objects_per_pg = p->second.stats.sum.num_objects / pi->get_pg_num_target();
                float ratio = (float)objects_per_pg / (float)average_objects_per_pg;
                if (mon_pg_warn_max_object_skew > 0 && ratio > mon_pg_warn_max_object_skew) {
                    ostringstream ss;
                    if (pi->pg_autoscale_mode != pg_pool_t::pg_autoscale_mode_t::ON) {
                        ss << "pool " << name << " objects per pg (" << objects_per_pg << ") is more than " << ratio
                           << " times cluster average (" << average_objects_per_pg << ")";
        ...
        if (!pgp_detail.empty()) {
            ...
            auto& d = checks->add("SMALLER_PGP_NUM", HEALTH_WARN, ss.str(), pgp_detail.size());
        ...
        if (!many_detail.empty()) {
            ...
            ss << many_detail.size() << " pools have many more objects per pg than"
               << " average";
            auto& d = checks->add("MANY_OBJECTS_PER_PG", HEALTH_WARN, ss.str(), many_detail.size());
    ...
    // POOL_NEAR_FULL
    {
        ...
        for (auto it : pools) {
            ...
            if (pool.quota_max_objects > 0) {
                ...
                else if (crit_threshold > 0 && sum.num_objects >= pool.quota_max_objects * crit_threshold) {
                    ss << "pool '" << pool_name << "' has " << sum.num_objects << " objects"
                       << " (max " << pool.quota_max_objects << ")";
                    full_detail.push_back(ss.str());
                    full = true;
                }
                else if (warn_threshold > 0 && sum.num_objects >= pool.quota_max_objects * warn_threshold) {
                    ss << "pool '" << pool_name << "' has " << sum.num_objects << " objects"
                       << " (max " << pool.quota_max_objects << ")";
            ...
            if (pool.quota_max_bytes > 0) {
                ...
                else if (crit_threshold > 0 && sum.num_bytes >= pool.quota_max_bytes * crit_threshold) {
                    ss << "pool '" << pool_name << "' has " << byte_u_t(sum.num_bytes) << " (max "
                       << byte_u_t(pool.quota_max_bytes) << ")";
                    full_detail.push_back(ss.str());
                    full = true;
                }
                else if (warn_threshold > 0 && sum.num_bytes >= pool.quota_max_bytes * warn_threshold) {
                    ss << "pool '" << pool_name << "' has " << byte_u_t(sum.num_bytes) << " (max "
                       << byte_u_t(pool.quota_max_bytes) << ")";
                    nearfull_detail.push_back(ss.str());
                    nearfull = true;
    ...
    if (pg_sum.stats.sum.num_objects_misplaced && pg_sum.stats.sum.num_object_copies > 0 &&
        ...
        double pc =
            (double)pg_sum.stats.sum.num_objects_misplaced / (double)pg_sum.stats.sum.num_object_copies * (double)100.0;
        char b[20];
        snprintf(b, sizeof(b), "%.3lf", pc);
        ostringstream ss;
        ss << pg_sum.stats.sum.num_objects_misplaced << "/" << pg_sum.stats.sum.num_object_copies
           << " objects misplaced (" << b << "%)";
        checks->add("OBJECT_MISPLACED", HEALTH_WARN, ss.str(), pg_sum.stats.sum.num_objects_misplaced);
    }

    // OBJECT_UNFOUND
    if (pg_sum.stats.sum.num_objects_unfound && pg_sum.stats.sum.num_objects) {
        double pc = (double)pg_sum.stats.sum.num_objects_unfound / (double)pg_sum.stats.sum.num_objects * (double)100.0;
        char b[20];
        snprintf(b, sizeof(b), "%.3lf", pc);
        ostringstream ss;
        ss << pg_sum.stats.sum.num_objects_unfound << "/" << pg_sum.stats.sum.num_objects << " objects unfound (" << b
           << "%)";
        auto& d = checks->add("OBJECT_UNFOUND", HEALTH_WARN, ss.str(), pg_sum.stats.sum.num_objects_unfound);

        for (auto& p : pg_stat) {
            if (p.second.stats.sum.num_objects_unfound) {
                ostringstream ss;
                ss << "pg " << p.first << " has " << p.second.stats.sum.num_objects_unfound << " unfound objects";
    ...
    if (osdmap.require_osd_release < ceph_release_t::mimic && cct->_conf->mon_osd_warn_op_age > 0 &&
        !osd_sum.op_queue_age_hist.h.empty() &&
        osd_sum.op_queue_age_hist.upper_bound() / 1000.0 > cct->_conf->mon_osd_warn_op_age) {
        list<string> warn_detail, error_detail;
        unsigned warn = 0, error = 0;
        float err_age = cct->_conf->mon_osd_warn_op_age * cct->_conf->mon_osd_err_op_age_ratio;
        const pow2_hist_t& h = osd_sum.op_queue_age_hist;
        for (unsigned i = h.h.size() - 1; i > 0; --i) {
            float ub = (float)(1 << i) / 1000.0;
            if (ub < cct->_conf->mon_osd_warn_op_age) break;
            if (h.h[i]) {
                ostringstream ss;
        ...
        if (!warn_detail.empty()) {
            ostringstream ss;
            ss << warn << " slow requests are blocked > " << cct->_conf->mon_osd_warn_op_age << " sec";
            ...
            for (auto& p : warn_osd_by_max) {
                ostringstream ss;
                if (p.second.size() > 1) {
                    ss << "osds " << p.second << " have blocked requests > " << p.first << " sec";
                }
                else {
                    ss << "osd." << *p.second.begin() << " has blocked requests > " << p.first << " sec";
                }
                d.detail.push_back(ss.str());
                if (--left == 0) {
                    break;
        ...
        if (!error_detail.empty()) {
            ostringstream ss;
            ss << error << " stuck requests are blocked > " << err_age << " sec";
            ...
            for (auto& p : error_osd_by_max) {
                ostringstream ss;
                if (p.second.size() > 1) {
                    ss << "osds " << p.second << " have stuck requests > " << p.first << " sec";
                }
                else {
                    ss << "osd." << *p.second.begin() << " has stuck requests > " << p.first << " sec";
                }
                d.detail.push_back(ss.str());
                if (--left == 0) {
                    break;
    ...
    if (cct->_conf->mon_warn_pg_not_scrubbed_ratio || cct->_conf->mon_warn_pg_not_deep_scrubbed_ratio) {
        ...
        for (auto& p : pg_stat) {
            ...
            if (cct->_conf->mon_warn_pg_not_scrubbed_ratio) {
                ...
                const double age =
                    (cct->_conf->mon_warn_pg_not_scrubbed_ratio * scrub_max_interval) + scrub_max_interval;
                utime_t cutoff = now;
                cutoff -= age;
                if (p.second.last_scrub_stamp < cutoff) {
                    if (detail_max > 0) {
                        ostringstream ss;
                        ss << "pg " << p.first << " not scrubbed since " << p.second.last_scrub_stamp;
                        detail.push_back(ss.str());
                        --detail_max;
                    }
                    else {
            ...
            if (cct->_conf->mon_warn_pg_not_deep_scrubbed_ratio) {
                ...
                double deep_age =
                    (cct->_conf->mon_warn_pg_not_deep_scrubbed_ratio * deep_scrub_interval) + deep_scrub_interval;
                utime_t deep_cutoff = now;
                deep_cutoff -= deep_age;
                if (p.second.last_deep_scrub_stamp < deep_cutoff) {
                    if (deep_detail_max > 0) {
                        ostringstream ss;
                        ss << "pg " << p.first << " not deep-scrubbed since " << p.second.last_deep_scrub_stamp;
        ...
        if (deep_detail_total) {
            ...
            auto& d = checks->add("PG_NOT_DEEP_SCRUBBED", HEALTH_WARN, ss.str(), deep_detail_total);
    ...
    if (g_conf().get_val<bool>("mon_warn_on_pool_no_app")) {
        ...
        for (auto& it : pools) {
            ...
            // required release
            if (sum.num_objects > 0 && pool.application_metadata.empty() && !pool.is_tier()) {
                stringstream ss;
        ...
        if (!detail.empty()) {
            ...
            auto& d = checks->add("POOL_APP_NOT_ENABLED", HEALTH_WARN, ss.str(), detail.size());
            stringstream tip;
    ...
    if (!pg_stat.empty() && cct->_conf->mon_osd_snap_trim_queue_warn_on > 0) {
        ...
        if (snaptrimq_exceeded) {
            ...
            stringstream ss;
            ss << "snap trim queue for " << snaptrimq_exceeded << " pg(s) >= " << snapthreshold
               << " (mon_osd_snap_trim_queue_warn_on)";
            auto& d = checks->add("PG_SLOW_SNAP_TRIMMING", HEALTH_WARN, ss.str(), snaptrimq_exceeded);
            detail.push_back(
...
int process_pg_map_command(const string& orig_prefix, const cmdmap_t& orig_cmdmap, const PGMap& pg_map,
                           const OSDMap& osdmap, ceph::Formatter* f, stringstream* ss, bufferlist* odata)
{
    ...
    if (prefix == "pg dump") {
        ...
        if (cmd_getval(cmdmap, "dumpcontents", dumpcontents)) {
            copy(dumpcontents.begin(), dumpcontents.end(), inserter(what, what.end()));
        }
        if (what.empty()) what.insert("all");
        if (f) {
            if (what.count("all")) {
    ...
    if (prefix == "pg ls") {
        ...
        if (states.empty()) states.push_back("all");
    ...
    if (prefix == "pg dump_stuck") {
        ...
        if (stuckop_vec.empty()) stuckop_vec.push_back("unclean");
        int64_t threshold;
        cmd_getval(cmdmap, "threshold", threshold, g_conf().get_val<int64_t>("mon_pg_stuck_threshold"));
    ...
    if (prefix == "pg debug") {
        string debugop;
        cmd_getval(cmdmap, "debugop", debugop, string("unfound_objects_exist"));
...
void PGMapUpdater::check_osd_map(CephContext* cct, const OSDMap& osdmap, const PGMap& pgmap,
                                 PGMap::Incremental* pending_inc)
{
    ...
    for (auto& p : osdmap.get_pools()) {
        ...
        if (q != pgmap.num_pg_by_pool.end()) my_pg_num = q->second;
        unsigned pg_num = pi.get_pg_num();
        new_pg_num[poolid] = pg_num;
        if (my_pg_num < pg_num) {
            ldout(cct, 10) << __func__ << " pool " << poolid << " pg_num " << pg_num << " > my pg_num " << my_pg_num
                           << dendl;
    ...
    while (i != pending_inc->pg_stat_updates.end()) {
        ...
        if (j == new_pg_num.end() || i->first.ps() >= j->second) {
...
static void _try_mark_pg_stale(const OSDMap& osdmap, pg_t pgid, const pg_stat_t& cur, PGMap::Incremental* pending_inc)
{
    if ((cur.state & PG_STATE_STALE) == 0 && cur.acting_primary != -1 && osdmap.is_down(cur.acting_primary)) {
        pg_stat_t* newstat;
        ...
        if (q != pending_inc->pg_stat_updates.end()) {
            if ((q->second.acting_primary == cur.acting_primary) ||
                ((q->second.state & PG_STATE_STALE) == 0 && q->second.acting_primary != -1 &&
        ...
        dout(10) << __func__ << " marking pg " << pgid << " stale (acting_primary " << newstat->acting_primary << ")"
                 << dendl;
...
void PGMapUpdater::check_down_pgs(const OSDMap& osdmap, const PGMap& pg_map, bool check_all,
                                  const set<int>& need_check_down_pg_osds, PGMap::Incremental* pending_inc)
{
    // if a large number of osds changed state, just iterate over the whole
    // pg map.
    if (need_check_down_pg_osds.size() >
        (unsigned)osdmap.get_num_osds() * g_conf().get_val<double>("mon_pg_check_down_all_threshold")) {
        check_all = true;
    }
...
int reweight::by_utilization(const OSDMap& osdmap, const PGMap& pgm, int oload, double max_changef, int max_osds,
                             bool by_pg, const set<int64_t>* pools, bool no_increasing,
                             mempool::osdmap::map<int32_t, uint32_t>* new_weights, std::stringstream* ss,
                             std::string* out_str, ceph::Formatter* f)
{
    if (oload <= 100) {
    ...
    if (by_pg) {
        ...
        for (const auto& pg : pgm.pg_stat) {
            if (pools && pools->count(pg.first.pool()) == 0) continue;
            for (const auto acting : pg.second.acting) {
                if (!osdmap.exists(acting)) {
                    continue;
                }
                if (acting >= (int)pgs_by_osd.size()) pgs_by_osd.resize(acting);
        ...
        if (!num_osds || (num_pg_copies / num_osds < g_conf()->mon_reweight_min_pgs_per_osd)) {
            *ss << "Refusing to reweight: we only have " << num_pg_copies << " PGs across " << num_osds << " osds!\n";
            return -EDOM;
        }
        ...
        if ((uint64_t)pgm.osd_sum.statfs.total / num_osd < g_conf()->mon_reweight_min_bytes_per_osd) {
            *ss << "Refusing to reweight: we only have " << pgm.osd_sum.statfs.kb() << " kb across all osds!\n";
            return -EDOM;
        }
        if ((uint64_t)pgm.osd_sum.statfs.get_used_raw() / num_osd < g_conf()->mon_reweight_min_bytes_per_osd) {
            *ss << "Refusing to reweight: we only have " << pgm.osd_sum.statfs.kb_used_raw()
                << " kb used across all osds!\n";
            return -EDOM;
        }

        average_util = (double)pgm.osd_sum.statfs.get_used_raw() / (double)pgm.osd_sum.statfs.total;
    }
    ...
    for (const auto& p : pgm.osd_stat) {
        ...
        if (by_pg) {
            if (p.first >= (int)pgs_by_osd.size() || pgs_by_osd[p.first] == 0) {
            ...
            osd_util.second = (double)p.second.statfs.get_used_raw() / (double)p.second.statfs.total;
        }
    ...
    // in descending order.
    std::sort(util_by_osd.begin(), util_by_osd.end(), [average_util](std::pair<int, float> l, std::pair<int, float> r) {
        return abs(l.second - average_util) > abs(r.second - average_util);
    });

    if (f) f->open_array_section("reweights");
    ...
    for (const auto& p : util_by_osd) {
        ...
        if (util >= overload_util) {
            ...
            if (weight > max_change) new_weight = std::max(new_weight, weight - max_change);
            ...
            if (f) {
                ...
                oss << "osd." << p.first << " weight " << (float)weight / (float)0x10000 << " -> "
        ...
        if (!no_increasing && util <= underload_util) {
            ...
            if (new_weight > 0x10000) new_weight = 0x10000;
