...
ceph_statfs PGMapDigest::get_statfs(OSDMap& osdmap, std::optional<int64_t> data_pool) const
{
    ceph_statfs statfs;
    bool filter = false;
...
void PGMap::dump_pg_stats_plain(ostream& ss, const mempool::pgmap::unordered_map<pg_t, pg_stat_t>& pg_stats, bool brief)
{
    TextTable tab;
    ...
    for (const auto& [pg, st] : pg_stats) {
        if (brief) {
            tab << pg << pg_state_string(st.state) << st.up << st.up_primary << st.acting << st.acting_primary
                << TextTable::endrow;
        }
        else {
            ...
            tab << pg << st.stats.sum.num_objects << st.stats.sum.num_objects_missing_on_primary
                ...
                << st.stats.sum.num_omap_keys << st.log_size << st.log_dups_size << st.ondisk_log_size
                << pg_state_string(st.state) << st.last_change << st.version << reported.str()
                << pg_vector_string(st.up) << st.up_primary << pg_vector_string(st.acting) << st.acting_primary
                << st.last_scrub << st.last_scrub_stamp << st.last_deep_scrub << st.last_deep_scrub_stamp
                << st.snaptrimq_len << st.last_scrub_duration << st.dump_scrub_schedule() << st.objects_scrubbed
                << st.objects_trimmed << TextTable::endrow;
        }
    }
...
void PGMap::dump_filtered_pg_stats(ostream& ss, set<pg_t>& pgs) const
{
    ...
    for (auto i = pgs.begin(); i != pgs.end(); ++i) {
        ...
        tab << *i << st.stats.sum.num_objects << st.stats.sum.num_objects_degraded << st.stats.sum.num_objects_misplaced
            << st.stats.sum.num_objects_unfound << st.stats.sum.num_bytes << st.stats.sum.num_omap_bytes
            << st.stats.sum.num_omap_keys << st.log_size << st.log_dups_size << pg_state_string(st.state)
            << utimespan_str(now - st.last_change) << st.version << reported.str() << upstr.str() << actingstr.str()
            << st.last_scrub_stamp << st.last_deep_scrub_stamp << st.last_scrub_duration << st.dump_scrub_schedule()
            << TextTable::endrow;
    }
        ...
        for (const auto& i : pg_stat) {
            ...
            for (const auto& j : state_to_response) {
                ...
                if (pg_response.stuck_since) {
                    ...
                    ss << ", current state " << pg_state_string(pg_info.state) << ", last acting "
                       << pg_vector_string(pg_info.acting);
                }
                else {
                           ...
                           const OSDMap& osdmap, ceph::Formatter* f, stringstream* ss, bufferlist* odata)
{
    ...
    if (prefix == "pg dump_stuck") {
        ...
        const int64_t threshold =
            cmd_getval_or<int64_t>(cmdmap, "threshold", g_conf().get_val<int64_t>("mon_pg_stuck_threshold"));
    ...
    if (prefix == "pg debug") {
        const string debugop = cmd_getval_or<string>(cmdmap, "debugop", "unfound_objects_exist");
    ...
    for (const auto& p : util_by_osd) {
        ...
        if (util >= overload_util) {
            ...
            if (f) {
                ...
                oss << "osd." << p.first << " weight " << (float)weight / (float)CEPH_OSD_IN << " -> "
        ...
        if (!no_increasing && util <= underload_util) {
            ...
            if (new_weight > CEPH_OSD_IN) new_weight = CEPH_OSD_IN;
