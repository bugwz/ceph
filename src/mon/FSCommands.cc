...
public:
    ...
    int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) override
    {
        string flag_name;
...
public:
    ...
    int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) override
    {
        ...
        auto f = [](auto fs) { fs->mds_map.set_flag(CEPH_MDSMAP_NOT_JOINABLE); };
...
public:
    ...
    bool batched_propose() override { return true; }
    ...
    int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) override
    {
        ...
        if (fsmap.get_filesystem(fs_name)) {
            auto fs = fsmap.get_filesystem(fs_name);
            if (*(fs->mds_map.get_data_pools().begin()) == data && fs->mds_map.get_metadata_pool() == metadata) {
        ...
        if (stat) {
            ...
            if (!force && metadata_num_objects > 0) {
                ss << "pool '" << metadata_name << "' already contains some objects. Use an empty pool instead.";
                return -EINVAL;
            }
        }
        ...
        for (auto& fs : fsmap.get_filesystems()) {
            ...
            if ((std::find(data_pools.begin(), data_pools.end(), data) != data_pools.end() ||
                ...
                ss << "Filesystem '" << fs_name
                   << "' is already using one of the specified RADOS pools. This should ONLY be done in emergencies "
                      "and after careful reading of the documentation. Pass --allow-dangerous-metadata-overlay to "
                      "permit this.";
                return -EINVAL;
            }
        }
        ...
        mon->osdmon()->do_application_enable(data, pg_pool_t::APPLICATION_NAME_CEPHFS, "data", fs_name, true);
        mon->osdmon()->do_application_enable(metadata, pg_pool_t::APPLICATION_NAME_CEPHFS, "metadata", fs_name, true);
        mon->osdmon()->do_set_pool_opt(metadata, pool_opts_t::RECOVERY_PRIORITY, static_cast<int64_t>(5));
        mon->osdmon()->do_set_pool_opt(metadata, pool_opts_t::PG_NUM_MIN, static_cast<int64_t>(16));
        mon->osdmon()->do_set_pool_opt(metadata, pool_opts_t::PG_AUTOSCALE_BIAS, static_cast<double>(4.0));
        mon->osdmon()->propose_pending();
        ...
        auto&& fs = fsmap.create_filesystem(fs_name, metadata, data, mon->get_quorum_con_features(), fscid, recover);
        ...
        if (info) {
            mon->clog->info() << info->human_name() << " assigned to filesystem " << fs_name << " as rank 0";
    ...
    int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) override
    {
        ...
        if (var == "max_mds") {
            ...
            if (n > 1 && n > fs->mds_map.get_max_mds()) {
                if (fs->mds_map.was_snaps_ever_allowed() && !fs->mds_map.allows_multimds_snaps()) {
            ...
            fsmap.modify_filesystem(fs->fscid, [n](std::shared_ptr<Filesystem> fs) {
            ...
            if (enable_inline) {
                ...
                fsmap.modify_filesystem(
                    fs->fscid, [](std::shared_ptr<Filesystem> fs) { fs->mds_map.set_inline_data_enabled(true); });
            }
            else {
                ss << "inline data disabled";
                fsmap.modify_filesystem(
                    fs->fscid, [](std::shared_ptr<Filesystem> fs) { fs->mds_map.set_inline_data_enabled(false); });
            }
        }
            ...
            fsmap.modify_filesystem(fs->fscid,
                                    [val](std::shared_ptr<Filesystem> fs) { fs->mds_map.set_balancer(val); });
            return true;
        }
            ...
            fsmap.modify_filesystem(fs->fscid,
                                    [val](std::shared_ptr<Filesystem> fs) { fs->mds_map.set_bal_rank_mask(val); });
            return true;
        }
            ...
            fsmap.modify_filesystem(fs->fscid,
                                    [n](std::shared_ptr<Filesystem> fs) { fs->mds_map.set_max_filesize(n); });
        }
            ...
            if (!enable_snaps) {
                fsmap.modify_filesystem(fs->fscid,
                                        [](std::shared_ptr<Filesystem> fs) { fs->mds_map.clear_snaps_allowed(); });
                ss << "disabled new snapshots";
            }
            else {
                fsmap.modify_filesystem(fs->fscid,
                                        [](std::shared_ptr<Filesystem> fs) { fs->mds_map.set_snaps_allowed(); });
            ...
            string confirm;
            if (!cmd_getval(cmdmap, "confirm", confirm) || confirm != "--yes-i-am-really-a-mds") {
            ...
            if (enable) {
                ...
                fsmap.modify_filesystem(
                    fs->fscid, [](std::shared_ptr<Filesystem> fs) { fs->mds_map.set_multimds_snaps_allowed(); });
            }
            else {
                ss << "disabled multimds with snapshot";
                fsmap.modify_filesystem(
                    fs->fscid, [](std::shared_ptr<Filesystem> fs) { fs->mds_map.clear_multimds_snaps_allowed(); });
            }
        }
            ...
            fsmap.modify_filesystem(fs->fscid, [is_down](std::shared_ptr<Filesystem> fs) {
                if (is_down) {
            ...
            fsmap.modify_filesystem(fs->fscid, [joinable](std::shared_ptr<Filesystem> fs) {
                if (joinable) {
                    fs->mds_map.clear_flag(CEPH_MDSMAP_NOT_JOINABLE);
                }
                else {
            ...
            fsmap.modify_filesystem(fs->fscid,
                                    [n](std::shared_ptr<Filesystem> fs) { fs->mds_map.set_standby_count_wanted(n); });
        }
            ...
            fsmap.modify_filesystem(
                fs->fscid, [n](std::shared_ptr<Filesystem> fs) { fs->mds_map.set_session_timeout((uint32_t)n); });
        }
            ...
            fsmap.modify_filesystem(
                fs->fscid, [n](std::shared_ptr<Filesystem> fs) { fs->mds_map.set_session_autoclose((uint32_t)n); });
        }
            ...
            auto f = [vno](auto&& fs) { fs->mds_map.set_min_compat_client(vno); };
            fsmap.modify_filesystem(fs->fscid, std::move(f));
        }
            ...
            if (refuse_session) {
                if (!(fs->mds_map.test_flag(CEPH_MDSMAP_REFUSE_CLIENT_SESSION))) {
                    fsmap.modify_filesystem(fs->fscid, [](std::shared_ptr<Filesystem> fs) {
...
public:
    ...
    int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) override
    {
        ...
        auto modifyf = [cs = std::move(cs)](auto&& fs) { fs->mds_map.compat = cs; };
...
public:
    ...
    int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) override
    {
        ...
        string subop;
        if (!cmd_getval(cmdmap, "subop", subop) || (subop != "add" && subop != "rm")) {
        ...
        if (subop == "add") {
            bool ret = false;
            fsmap.modify_filesystem(fs->fscid, [feature, &ret](auto&& fs) {
                if (fs->mds_map.get_required_client_features().test(feature)) return;
...
public:
    ...
    int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) override
    {
        ...
        mon->osdmon()->do_application_enable(poolid, pg_pool_t::APPLICATION_NAME_CEPHFS, "data", fs_name, true);
        mon->osdmon()->propose_pending();

        fsmap.modify_filesystem(fs->fscid,
                                [poolid](std::shared_ptr<Filesystem> fs) { fs->mds_map.add_data_pool(poolid); });
    ...
    int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) override
    {
        ...
        if (fs == nullptr) {
            if (new_fs) {
                ...
                ss << "File system may already have been renamed. Desired file system '" << new_fs_name << "' exists.";
                return 0;
            }
            else {
        ...
        for (const auto p : fs->mds_map.get_data_pools()) {
            mon->osdmon()->do_application_enable(p, pg_pool_t::APPLICATION_NAME_CEPHFS, "data", new_fs_name, true);
        }

        mon->osdmon()->do_application_enable(
            fs->mds_map.get_metadata_pool(), pg_pool_t::APPLICATION_NAME_CEPHFS, "metadata", new_fs_name, true);
        mon->osdmon()->propose_pending();

        auto f = [new_fs_name](auto fs) { fs->mds_map.set_fs_name(new_fs_name); };
        fsmap.modify_filesystem(fs->fscid, std::move(f));
...
public:
    ...
    int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) override
    {
        ...
        int r = 0;
        fsmap.modify_filesystem(
            fs->fscid, [&r, poolid](std::shared_ptr<Filesystem> fs) { r = fs->mds_map.remove_data_pool(poolid); });
        if (r == -ENOENT) {
 ...
 */
template<typename T> class AliasHandler : public T
{
...
public:
    ...
    int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) override
    {
        ...
        auto f = [](auto&& fs) { fs->mirror_info.enable_mirroring(); };
        fsmap.modify_filesystem(fs->fscid, std::move(f));
...
public:
    ...
    int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) override
    {
        ...
        auto f = [](auto&& fs) { fs->mirror_info.disable_mirroring(); };
        fsmap.modify_filesystem(fs->fscid, std::move(f));
...
public:
    ...
    boost::optional<std::pair<string, string>> extract_remote_cluster_conf(const std::string& spec)
    {
        auto pos = spec.find("@");
    ...
    bool peer_add(FSMap& fsmap, Filesystem::const_ref&& fs, const cmdmap_t& cmdmap, std::ostream& ss)
    {
        string peer_uuid;
        string remote_spec;
        ...
        if (fs->mirror_info.has_peer((*remote_conf).first, (*remote_conf).second, remote_fs_name)) {
            ss << "peer already exists";
            return true;
        }

        auto f = [peer_uuid, remote_conf, remote_fs_name](auto&& fs) {
            fs->mirror_info.peer_add(peer_uuid, (*remote_conf).first, (*remote_conf).second, remote_fs_name);
        };
        fsmap.modify_filesystem(fs->fscid, std::move(f));
        return true;
    }
...
public:
    ...
    bool peer_remove(FSMap& fsmap, Filesystem::const_ref&& fs, const cmdmap_t& cmdmap, std::ostream& ss)
    {
        string peer_uuid;
        cmd_getval(cmdmap, "uuid", peer_uuid);
        ...
        auto f = [peer_uuid](auto&& fs) { fs->mirror_info.peer_remove(peer_uuid); };
        fsmap.modify_filesystem(fs->fscid, std::move(f));
        return true;
    }
...
std::list<std::shared_ptr<FileSystemCommandHandler>> FileSystemCommandHandler::load(Paxos* paxos)
{
    ...
    handlers.push_back(std::make_shared<AliasHandler<SetDefaultHandler>>("fs set_default"));
...
int FileSystemCommandHandler::_check_pool(OSDMap& osd_map, const int64_t pool_id, int type, bool force,
                                          std::ostream* ss, bool allow_overlay) const
{
    ceph_assert(ss != NULL);
    ...
    if (!allow_overlay && !force && !app_map.empty()) {
        ...
        if (app != app_map.end()) {
            ...
            if (itr != app_metadata.end()) {
                ...
                *ss << "RADOS pool '" << pool_name << "' is already used by filesystem '" << filesystem << "' as a '"
                    << type << "' pool for application '" << app_name << "'";
                return -EINVAL;
            }
        }
        else {
            *ss << "RADOS pool '" << pool_name << "' has another non-CephFS application enabled.";
            return -EINVAL;
        }
    }
        ...
        if (type == POOL_METADATA) {
            ...
            *ss << "pool '" << pool_name << "' (id '" << pool_id
            ...
            if (write_tier->cache_mode == pg_pool_t::CACHEMODE_FORWARD ||
                ...
                *ss << "EC pool '" << pool_name << "' has a write tier (" << osd_map.get_pool_name(pool->write_tier)
                    << ") that is configured "
    ...
    if (pool->is_tier()) {
        *ss << " pool '" << pool_name << "' (id '" << pool_id << "') is already in use as a cache tier.";
        return -EINVAL;
    }

    if (!force && !pool->application_metadata.empty() &&
        pool->application_metadata.count(pg_pool_t::APPLICATION_NAME_CEPHFS) == 0) {
        *ss << " pool '" << pool_name << "' (id '" << pool_id << "') has a non-CephFS application enabled.";
        return -EINVAL;
    }

    if (type != POOL_METADATA && pool->pg_autoscale_mode == pg_pool_t::pg_autoscale_mode_t::ON &&
...
int FileSystemCommandHandler::is_op_allowed(const MonOpRequestRef& op, const FSMap& fsmap, const cmdmap_t& cmdmap,
