...
static ostream& _prefix(std::ostream* _dout, Monitor& mon, const MgrMap& mgrmap)
{
    return *_dout << "mon." << mon.name << "@" << mon.rank << "(" << mon.get_state_name() << ").mgr e"
                  << mgrmap.get_epoch() << " ";
}

// the system treats always_on_modules as if they provide built-in functionality
// by ensuring that they are always enabled.
const static std::map<uint32_t, std::set<std::string>> always_on_modules = {{CEPH_RELEASE_NAUTILUS,
                                                                             {
                                                                                 "crash",
                                                                                 "status",
                                                                                 "progress",
...
void MgrMonitor::create_initial()
{
    ...
    dout(10) << __func__ << " initial modules " << pending_map.modules << ", always on modules "
             << pending_map.get_always_on_modules() << ", " << pending_command_descs.size() << " commands" << dendl;
}
...
void MgrMonitor::update_from_paxos(bool* need_bootstrap)
{
    ...
    if (version != map.epoch) {
        ...
        if (version == 1 || command_descs.empty() ||
            (map.get_available() && (!old_available || old_gid != map.get_active_gid()))) {
    ...
    for (auto& i : map.available_modules) {
        for (auto& j : i.module_options) {
            ...
            auto p = mgr_module_options.emplace(
                name,
                Option(name, static_cast<Option::type_t>(j.second.type), static_cast<Option::level_t>(j.second.level)));
            ...
            Option::value_t v, v2;
            std::string err;
            if (j.second.default_value.size() && !opt.parse_value(j.second.default_value, &v, &err)) {
                opt.set_default(v);
            }
            if (j.second.min.size() && j.second.max.size() && !opt.parse_value(j.second.min, &v, &err) &&
    ...
    if (digest_event) {
        ...
        if (send) {
            if (is_active()) {
                ...
                cancel_timer();
                wait_for_active_ctx(new C_MonContext{&mon, [this](int) { send_digests(); }});
            }
        }
    }
}
...
bool MgrMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    switch (m->get_type()) {
    case MSG_MGR_BEACON: return preprocess_beacon(op);
...
bool MgrMonitor::prepare_update(MonOpRequestRef op)
{
    auto m = op->get_req<PaxosServiceMessage>();
    switch (m->get_type()) {
    case MSG_MGR_BEACON: return prepare_beacon(op);
...
bool MgrMonitor::prepare_beacon(MonOpRequestRef op)
{
    ...
    if (m->get_name() == pending_map.active_name && m->get_gid() != pending_map.active_gid) {
        dout(4) << "Active daemon restart (mgr." << m->get_name() << ")" << dendl;
        mon.clog->info() << "Active manager daemon " << m->get_name() << " restarted";
        if (!mon.osdmon()->is_writeable()) {
            dout(1) << __func__
                    << ":  waiting for osdmon writeable to"
                       " blocklist old instance."
                    << dendl;
    ...
    for (const auto& i : pending_map.standbys) {
        ...
        if (s.name == m->get_name() && s.gid != m->get_gid()) {
            ...
            mon.clog->debug() << "Standby manager daemon " << m->get_name() << " restarted";
    ...
    if (pending_map.active_gid == m->get_gid()) {
        ...
        if (pending_map.active_addrs != m->get_server_addrs()) {
            dout(4) << "learned address " << m->get_server_addrs() << " (was " << pending_map.active_addrs << ")"
                    << dendl;
        ...
        if (pending_map.get_available() != m->get_available()) {
            ...
            mon.clog->info() << "Manager daemon " << pending_map.active_name << " is now available";
            ...
            if (pending_command_descs.empty()) {
                ...
                dout(4) << "First available beacon from " << pending_map.active_name << "(" << m->get_gid()
                        << ") does not include command descs" << dendl;
            }
            else {
                dout(4) << "First available beacon from " << pending_map.active_name << "(" << m->get_gid()
                        << ") includes " << pending_command_descs.size() << " command descs" << dendl;
            }
        ...
        if (pending_map.available_modules != m->get_available_modules()) {
            dout(4) << "available_modules " << m->get_available_modules() << " (was " << pending_map.available_modules
                    << ")" << dendl;
        ...
        dout(4) << "selecting new active " << m->get_gid() << " " << m->get_name() << " (was " << pending_map.active_gid
        ...
        mon.clog->info() << "Activating manager daemon " << pending_map.active_name;
        ...
        if (pending_map.standbys.count(m->get_gid()) > 0) {
            ...
            if (pending_map.standbys[m->get_gid()].available_modules != m->get_available_modules()) {
                dout(10) << "existing standby " << m->get_gid() << " available_modules " << m->get_available_modules()
                         << " (was " << pending_map.standbys[m->get_gid()].available_modules << ")" << dendl;
                pending_map.standbys[m->get_gid()].available_modules = m->get_available_modules();
                updated = true;
            }
        }
        else {
            dout(10) << "new standby " << m->get_gid() << dendl;
            mon.clog->debug() << "Standby manager daemon " << m->get_name() << " started";
            pending_map.standbys[m->get_gid()] = {
                m->get_gid(), m->get_name(), m->get_available_modules(), m->get_mgr_features()};
            encode(m->get_metadata(), pending_metadata[m->get_name()]);
            pending_metadata_rm.erase(m->get_name());
            updated = true;
        }
...
void MgrMonitor::check_sub(Subscription* sub)
{
    if (sub->type == "mgrmap") {
        if (sub->next <= map.get_epoch()) {
            dout(20) << "Sending map to subscriber " << sub->session->con << " " << sub->session->con->get_peer_addr()
                     << dendl;
...
timer:
    digest_event = mon.timer.add_event_after(g_conf().get_val<int64_t>("mon_mgr_digest_period"),
                                             new C_MonContext{&mon, [this](int) { send_digests(); }});
}
...
void MgrMonitor::on_active()
{
    ...
    dout(4) << "always on modules changed, pending " << pending_map.always_on_modules << " != wanted "
...
{
    if (!is_active() || !mon.is_leader()) return;

    const auto now = ceph::coarse_mono_clock::now();

    const auto mgr_beacon_grace = g_conf().get_val<std::chrono::seconds>("mon_mgr_beacon_grace");
    ...
    if (pending_map.active_gid != 0 && last_beacon.count(pending_map.active_gid) == 0) {
    ...
    if (pending_map.active_gid != 0 && last_beacon.at(pending_map.active_gid) < cutoff &&
        ...
        if (promote_standby()) {
            ...
            mon.clog->info() << "Manager daemon " << old_active_name << " is unresponsive, replacing it with standby"
    ...
    if (!pending_map.available && !ever_had_active_mgr && should_warn_about_mgr_down() != HEALTH_OK) {
        dout(10) << " exceeded mon_mgr_mkfs_grace " << g_conf().get_val<int64_t>("mon_mgr_mkfs_grace") << " seconds"
                 << dendl;
        propose = true;
    }

    // obsolete modules?
    if (mon.monmap->min_mon_release >= ceph_release_t::octopus && pending_map.module_enabled("orchestrator_cli")) {
...
bool MgrMonitor::promote_standby()
{
    ...
    if (pending_map.standbys.size()) {
        ...
        pending_map.available_modules = pending_map.standbys.at(replacement_gid).available_modules;
        pending_map.active_mgr_features = pending_map.standbys.at(replacement_gid).mgr_features;
...
void MgrMonitor::drop_active()
{
    ...
    dout(5) << "blocklisting previous mgr." << pending_map.active_name << "." << pending_map.active_gid << " ("
    ...
    if (prefix == "mgr stat") {
        ...
        f->open_object_section("modules");
        {
            ...
            for (auto& p : map.modules) {
                if (map.get_always_on_modules().count(p) > 0) continue;
            ...
            for (auto& p : map.available_modules) {
                if (map.modules.count(p.name) == 0 && map.get_always_on_modules().count(p.name) == 0) {
...
bool MgrMonitor::prepare_command(MonOpRequestRef op)
{
    ...
    if (prefix == "mgr fail") {
        ...
        if (!pending_map.all_support_module(module) && force != "--force") {
...
int MgrMonitor::load_metadata(const string& name, std::map<string, string>& m, ostream* err) const
{
    bufferlist bl;
    ...
    try {
        ...
        if (err) *err << "mgr." << name << " metadata is corrupt";
        return -EIO;
    }
    return 0;
}
...
int MgrMonitor::dump_metadata(const string& name, Formatter* f, ostream* err)
{
    ...
    if (int r = load_metadata(name, m, err)) return r;
    for (auto& p : m) {
