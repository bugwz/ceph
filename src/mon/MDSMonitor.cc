...
{
    return *_dout << "mon." << mon.name << "@" << mon.rank << "(" << mon.get_state_name() << ").mds e"
                  << fsmap.get_epoch() << " ";
}
...
template<> bool cmd_getval(const cmdmap_t& cmdmap, const std::string& k, mds_gid_t& val)
{
    return cmd_getval(cmdmap, k, (int64_t&)val);
}

template<> bool cmd_getval(const cmdmap_t& cmdmap, const std::string& k, mds_rank_t& val)
{
    return cmd_getval(cmdmap, k, (int64_t&)val);
}

template<> bool cmd_getval(const cmdmap_t& cmdmap, const std::string& k, MDSMap::DaemonState& val)
{
    return cmd_getval(cmdmap, k, (int64_t&)val);
}
}   // namespace TOPNSPC::common
...
template<int dblV> void MDSMonitor::print_map(const FSMap& m)
{
    dout(dblV) << "print_map\n";
    m.print(*_dout);
    *_dout << dendl;
...
void MDSMonitor::update_from_paxos(bool* need_bootstrap)
{
    ...
    if (version == get_fsmap().epoch) return;
...
void MDSMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
    ...
    for (std::map<uint64_t, MDSHealth>::iterator i = pending_daemon_health.begin(); i != pending_daemon_health.end();
         ++i) {
        bufferlist bl;
        i->second.encode(bl);
    ...
    for (std::set<uint64_t>::iterator i = pending_daemon_health_rm.begin(); i != pending_daemon_health_rm.end(); ++i) {
    ...
    for (const auto& i : info_map) {
        ...
        for (const auto& metric : health.metrics) {
            ...
            health_check_t* check =
                &new_checks.get_or_add(mds_metric_name(metric.type), metric.sev, mds_metric_summary(metric.type), 1);
            ostringstream ss;
    ...
    for (auto& p : new_checks.checks) {
        p.second.summary = std::regex_replace(p.second.summary, std::regex("%num%"), stringify(p.second.detail.size()));
        p.second.summary =
            std::regex_replace(p.second.summary, std::regex("%plurals%"), p.second.detail.size() > 1 ? "s" : "");
        p.second.summary =
            std::regex_replace(p.second.summary, std::regex("%isorare%"), p.second.detail.size() > 1 ? "are" : "is");
        p.second.summary = std::regex_replace(
            p.second.summary, std::regex("%hasorhave%"), p.second.detail.size() > 1 ? "have" : "has");
    }
...
{
    version_t floor = 0;
    if (g_conf()->mon_mds_force_trim_to > 0 && g_conf()->mon_mds_force_trim_to <= (int)get_last_committed()) {
...
bool MDSMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    dout(10) << "preprocess_query " << *m << " from " << m->get_orig_source() << " " << m->get_orig_source_addrs()
             << dendl;

    switch (m->get_type()) {

    case MSG_MDS_BEACON: return preprocess_beacon(op);
    ...
    case MSG_MDS_OFFLOAD_TARGETS: return preprocess_offload_targets(op);
...
bool MDSMonitor::preprocess_beacon(MonOpRequestRef op)
{
    ...
    if (!session->is_capable("mds", MON_CAP_X)) {
        dout(0) << "preprocess_beacon got MMDSBeacon from entity with insufficient privileges " << session->caps
                << dendl;
        goto ignore;
    }
    ...
    dout(5) << "preprocess_beacon " << *m << " from " << m->get_orig_source() << " " << m->get_orig_source_addrs()
            << " " << m->get_compat() << dendl;
...
bool MDSMonitor::preprocess_offload_targets(MonOpRequestRef op)
{
    ...
    if (!session) goto ignore;
    if (!session->is_capable("mds", MON_CAP_X)) {
        dout(0) << "preprocess_offload_targets got MMDSLoadTargets from entity with insufficient caps " << session->caps
                << dendl;
        goto ignore;
    }

    if (fsmap.gid_exists(m->global_id) && m->targets == fsmap.get_info_gid(m->global_id).export_targets) goto ignore;
...
bool MDSMonitor::prepare_update(MonOpRequestRef op)
{
    ...
    case MSG_MDS_BEACON: return prepare_beacon(op);
    ...
    case MSG_MDS_OFFLOAD_TARGETS: return prepare_offload_targets(op);
...
bool MDSMonitor::prepare_beacon(MonOpRequestRef op)
{
    ...
    auto m = op->get_req<MMDSBeacon>();
    // -- this is an update --
    dout(12) << "prepare_beacon " << *m << " from " << m->get_orig_source() << " " << m->get_orig_source_addrs()
             << dendl;
    ...
    dout(15) << __func__ << " got health from gid " << gid << " with " << m->get_health().metrics.size() << " metrics."
             << dendl;
    ...
    for (const auto& new_metric : new_health) {
        ...
        if (old_types.count(new_metric.type) == 0) {
            dout(10) << "MDS health message (" << m->get_orig_source() << "): " << new_metric.sev << " "
                     << new_metric.message << dendl;
        }
    }
    ...
    for (const auto& old_metric : old_health) {
        if (new_types.count(old_metric.type) == 0) {
            mon.clog->info() << "MDS health message cleared (" << m->get_orig_source() << "): " << old_metric.message;
        }
    }
    ...
    if (state == MDSMap::STATE_BOOT) {
        ...
        if ((info.state == MDSMap::STATE_STANDBY && state != info.state) ||
            ...
            derr << "standby " << gid << " attempted to change state to " << ceph_mds_state_name(state) << ", rejecting"
                 << dendl;
            goto evict;
        }
            ...
            derr << "daemon " << gid << " (rank " << info.rank << ") "
                 << "reported invalid state transition " << ceph_mds_state_name(info.state) << " -> "
        ...
        if (info.laggy()) {
            ...
            pending.modify_daemon(info.global_id, [](auto& info) { info.clear_laggy(); });
        }

        dout(5) << "prepare_beacon mds." << info.rank << " " << ceph_mds_state_name(info.state) << " -> "
                << ceph_mds_state_name(state) << dendl;
        ...
        pending.modify_daemon(gid, [fscid](auto& info) { info.join_fscid = fscid; });
        ...
        if (state == MDSMap::STATE_STOPPED) {
            ...
            mon.clog->info() << info.human_name() << " finished "
                             << "stopping rank " << info.rank << " in filesystem " << fs->mds_map.fs_name
                             << " (now has " << fs->mds_map.get_num_in_mds() - 1 << " ranks)";
            ...
            if (info.state != MDSMap::STATE_ACTIVE && state == MDSMap::STATE_ACTIVE) {
                ...
                mon.clog->info() << info.human_name() << " is now active in "
                                 << "filesystem " << fs->mds_map.fs_name << " as rank " << info.rank;
            }
...
void MDSMonitor::_updated(MonOpRequestRef op)
{
    ...
    mon.clog->debug() << m->get_orig_source() << " " << m->get_orig_source_addrs() << " "
...
bool MDSMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "mds stat") {
        ...
        for (auto gid : stopping) {
            if (fsmap.gid_has_rank(gid)) {
                ...
                if (!fsmap.is_standby_replay(gid)) {
                    ...
                    if (standby == MDS_GID_NONE || stopping.count(standby)) {
        ...
        if (fsmap.get_num_standby() - standby.size() < active.size()) {
            r = -EBUSY;
            ss << "insufficent standby MDS daemons to stop active gids " << stringify(active) << " and/or standby gids "
               << stringify(standby);
            ;
            goto out;
        }
        ...
        if (f) {
            ...
            for (const auto& p : fsmap.filesystems) {
                ...
                f->open_object_section("filesystem");
                {
                    ...
                    f->dump_string("metadata_pool", mon.osdmon()->osdmap.get_pool_name(mds_map.metadata_pool));
            ...
            for (const auto& p : fsmap.filesystems) {
                const auto& fs = p.second;
                const MDSMap& mds_map = fs->mds_map;
                const string& md_pool_name = mon.osdmon()->osdmap.get_pool_name(mds_map.metadata_pool);

                ds << "name: " << mds_map.fs_name << ", metadata pool: " << md_pool_name << ", data pools: [";
                for (const auto& id : mds_map.data_pools) {
...
mds_gid_t MDSMonitor::gid_from_arg(const FSMap& fsmap, const std::string& arg, std::ostream& ss)
{
    ...
    if (!err.empty()) {
        ...
        if (!mds_info) {
            ss << "MDS named '" << arg << "' does not exist, or is not up";
            return MDS_GID_NONE;
        }
...
int MDSMonitor::fail_mds(FSMap& fsmap, std::ostream& ss, const std::string& arg, MDSMap::mds_info_t* failed_info)
{
...
out:
    ...
    if (r >= 0) {
        // success.. delay reply
        wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, r, rs, get_last_committed() + 1));
        if (batched_propose) {
...
int MDSMonitor::filesystem_command(FSMap& fsmap, MonOpRequestRef op, std::string const& prefix, const cmdmap_t& cmdmap,
                                   std::stringstream& ss)
{
    ...
    if (prefix == "mds set_state") {
        ...
        if (!cmd_getval(cmdmap, "gid", gid)) {
            ss << "error parsing 'gid' value '" << cmd_vartype_stringify(cmdmap.at("gid")) << "'";
            return -EINVAL;
        }
        MDSMap::DaemonState state;
        if (!cmd_getval(cmdmap, "state", state)) {
            ss << "error parsing 'state' string value '" << cmd_vartype_stringify(cmdmap.at("state")) << "'";
            return -EINVAL;
        }
        if (fsmap.gid_exists(gid, op->get_session()->get_allowed_fs_names())) {
            fsmap.modify_daemon(gid, [state](auto& info) { info.state = state; });
        ...
        if (r < 0 && r == -EAGAIN) {
            ...
            if (failed_info.global_id != MDS_GID_NONE) {
                mon.clog->info() << failed_info.human_name() << " marked failed by " << op->get_session()->entity_name;
            }
        }
    }
        ...
        if (state > 0) {
            ss << "cannot remove active mds." << info.name << " rank " << info.rank;
            return -EBUSY;
        }
        else {
            fsmap.erase(gid, {});
        ...
        fsmap.modify_filesystem(role.fscid,
                                [role](std::shared_ptr<Filesystem> fs) { fs->mds_map.failed.erase(role.rank); });
        ...
        if (!cmd_getval(cmdmap, "feature", f)) {
            ss << "error parsing feature value '" << cmd_vartype_stringify(cmdmap.at("feature")) << "'";
            return -EINVAL;
        }
...
void MDSMonitor::check_sub(Subscription* sub)
{
    ...
    if (sub->type == "fsmap") {
        ...
        dout(10) << __func__ << " selected MDS map epoch " << mds_map->epoch << " for namespace " << fscid
                 << " for subscriber " << sub->session->name << " who wants epoch " << sub->next << dendl;

        if (sub->next > mds_map->epoch) {
            return;
        }
        auto msg = make_message<MMDSMap>(mon.monmap->fsid, *mds_map, mds_map->fs_name);
...
void MDSMonitor::update_metadata(mds_gid_t gid, const map<string, string>& metadata)
{
...
int MDSMonitor::dump_metadata(const FSMap& fsmap, const std::string& who, Formatter* f, ostream& err)
{
    ceph_assert(f);
...
bool MDSMonitor::maybe_resize_cluster(FSMap& fsmap, fs_cluster_id_t fscid)
{
     ...
     */
    if (!get_fsmap().filesystem_exists(fscid) || !get_fsmap().get_filesystem(fscid)->mds_map.is_resizeable() ||
    ...
    if (in < max && !mds_map.test_flag(CEPH_MDSMAP_NOT_JOINABLE)) {
        ...
        mon.clog->info() << info->human_name()
                         << " assigned to "
                            "filesystem "
                         << mds_map.fs_name << " as rank " << mds << " (now has " << mds_map.get_num_in_mds() + 1
                         << " ranks)";
        ...
        if (mds_map.is_active(target)) {
            ...
            auto f = [](auto& info) { info.state = MDSMap::STATE_STOPPING; };
...
bool MDSMonitor::drop_mds(FSMap& fsmap, mds_gid_t gid, const mds_info_t* rep_info, bool* osd_propose)
{
    ...
    if (info.is_frozen()) {
    ...
    else if (state == MDSMap::STATE_STANDBY_REPLAY || state == MDSMap::STATE_STANDBY) {
        dout(1) << " failing and removing standby " << gid << " " << info.addrs << " mds." << rank << "." << info.inc
                << " " << ceph_mds_state_name(state) << dendl;
        *osd_propose |= fail_mds_gid(fsmap, gid);
        ...
        dout(1) << " replacing " << gid << " " << info.addrs << " mds." << rank << "." << info.inc << " "
                << ceph_mds_state_name(state) << " with " << rep_info->global_id << "/" << rep_info->name << " "
                << rep_info->addrs << dendl;

        mon.clog->warn() << "Replacing " << info.human_name() << " as rank " << rank << " with standby "
                         << rep_info->human_name();
...
bool MDSMonitor::check_health(FSMap& fsmap, bool* propose_osdmap)
{
        ...
        if (since_last.count() > (mds_beacon_grace - mds_beacon_interval)) {
            ...
            dout(1) << __func__
                    << ": resetting beacon timeouts due to mon delay "
                       "(slow election?) of "
                    << since_last.count() << " seconds" << dendl;
    ...
    for (auto& p : fsmap.mds_roles) {
        auto& gid = p.first;
        last_beacon.emplace(std::piecewise_construct, std::forward_as_tuple(gid), std::forward_as_tuple(now, 0));
    }
    ...
    const bool may_replace = since.count() < std::max(g_conf()->mds_beacon_interval, g_conf()->mds_beacon_grace * 0.5);

    // check beacon timestamps
    std::vector<mds_gid_t> to_remove;
    const bool mon_down = mon.is_mon_down();
    const auto mds_beacon_mon_down_grace = g_conf().get_val<std::chrono::seconds>("mds_beacon_mon_down_grace");
    ...
    for (auto it = last_beacon.begin(); it != last_beacon.end();) {
        ...
        if (since_last.count() >= g_conf()->mds_beacon_grace) {
            ...
            dout(1) << "no beacon from mds." << info.rank << "." << info.inc << " (gid: " << gid
                    << " addr: " << info.addrs << " state: " << ceph_mds_state_name(info.state) << ")"
            ...
            if (!info.laggy()) {
                dout(1) << " marking " << gid << " " << info.addrs << " mds." << info.rank << "." << info.inc << " "
                        << ceph_mds_state_name(info.state) << " laggy" << dendl;
                fsmap.modify_daemon(info.global_id, [](auto& info) { info.laggy_since = ceph_clock_now(); });
                do_propose = true;
            }
    ...
    if (osdmap_writeable) {
        for (auto& [fscid, fs] : fsmap.filesystems) {
            if (!fs->mds_map.test_flag(CEPH_MDSMAP_NOT_JOINABLE) && fs->mds_map.is_resizeable()) {
                ...
                for (const auto& [gid, info] : fs->mds_map.get_mds_info()) {
                    ...
                    if (join_fscid == FS_CLUSTER_ID_NONE) {
                        ...
                        better_affinity =
                            (rep_info->join_fscid == fscid) || (rep_info->join_fscid == FS_CLUSTER_ID_NONE);
                    }
                    if (better_affinity) {
                        if (state == MDSMap::STATE_STANDBY_REPLAY) {
                            mon.clog->info() << "Dropping low affinity standby-replay " << info.human_name()
                                             << " in favor of higher affinity standby.";
                            *propose_osdmap |= fail_mds_gid(fsmap, gid);
                            /* Now let maybe_promote_standby do the promotion. */
                        }
                        else {
                            mon.clog->info() << "Dropping low affinity active " << info.human_name()
                                             << " in favor of higher affinity standby.";
...
bool MDSMonitor::maybe_promote_standby(FSMap& fsmap, Filesystem& fs)
{
    ...
    for (const auto& rank : failed) {
        ...
        if (info) {
            dout(1) << " taking over failed mds." << rank << " with " << info->global_id << "/" << info->name << " "
                    << info->addrs << dendl;
            mon.clog->info() << "Standby " << info->human_name() << " assigned to filesystem " << fs.mds_map.fs_name
                             << " as rank " << rank;
