...
private:
    ...
    struct log_channel_info
    {
        ...
        std::string expand_channel_meta(const std::string& input, const std::string& change_to);

        bool do_log_to_syslog(const std::string& channel);

        std::string get_facility(const std::string& channel)
        {
            return get_str_map_key(syslog_facility, channel, &CLOG_CONFIG_DEFAULT_KEY);
        }

        std::string get_level(const std::string& channel)
        {
            return get_str_map_key(syslog_level, channel, &CLOG_CONFIG_DEFAULT_KEY);
        }
        ...
        {
            return get_str_map_key(log_file_level, channel, &CLOG_CONFIG_DEFAULT_KEY);
        }

        bool do_log_to_graylog(const std::string& channel)
        {
            return (get_str_map_key(log_to_graylog, channel, &CLOG_CONFIG_DEFAULT_KEY) == "true");
        }
        ...
        {
            return (get_str_map_key(log_to_journald, channel, &CLOG_CONFIG_DEFAULT_KEY) == "true");
        }
...
public:
    ...
    void reopen_logs() { this->log_rotated.store(true); }
    ...
    void on_shutdown() override { g_conf().remove_observer(this); }

    const char** get_tracked_conf_keys() const override
    {
        static const char* KEYS[] = {"mon_cluster_log_to_syslog",
