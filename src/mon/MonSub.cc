...
bool MonSub::want(const std::string& what, version_t start, unsigned flags)
{
    if (auto sub = sub_new.find(what);
        sub != sub_new.end() && sub->second.start == start && sub->second.flags == flags) {
        return false;
    }
    else if (auto sub = sub_sent.find(what);
             sub != sub_sent.end() && sub->second.start == start && sub->second.flags == flags) {
        return false;
    }
    else {
...
bool MonSub::inc_want(const std::string& what, version_t start, unsigned flags)
{
    ...
    else if (auto sub = sub_sent.find(what); sub == sub_sent.end() || sub->second.start < start) {
