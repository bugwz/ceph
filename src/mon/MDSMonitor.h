...
protected:
    ...
    // my helpers
    template<int dblV = 7> void print_map(const FSMap& m);
    ...
    int fail_mds(FSMap& fsmap, std::ostream& ss, const std::string& arg, mds_info_t* failed_info);

    bool preprocess_command(MonOpRequestRef op);
    bool prepare_command(MonOpRequestRef op);

    int filesystem_command(FSMap& fsmap, MonOpRequestRef op, std::string const& prefix, const cmdmap_t& cmdmap,
                           std::stringstream& ss);
    ...
    struct beacon_info_t
    {
        ...
        uint64_t seq = 0;
        beacon_info_t() {}
        beacon_info_t(ceph::mono_time stamp, uint64_t seq)
            : stamp(stamp)
            , seq(seq)
        {}
    };
    ...
    int dump_metadata(const FSMap& fsmap, const std::string& who, ceph::Formatter* f, std::ostream& err);
...
public:
    void print_fs_summary(std::ostream& out) { get_fsmap().print_fs_summary(out); }
