...
{
public:
    MonConnection(CephContext* cct, ConnectionRef conn, uint64_t global_id, AuthRegistry* auth_registry);
    ~MonConnection();
    ...
    void start(epoch_t epoch, const EntityName& entity_name);
    bool have_session() const;
    uint64_t get_global_id() const { return global_id; }
    ConnectionRef get_con() { return con; }
    std::unique_ptr<AuthClientHandler>& get_auth() { return auth; }

    int get_auth_request(uint32_t* method, std::vector<uint32_t>* preferred_modes, ceph::buffer::list* out,
                         const EntityName& entity_name, uint32_t want_keys, RotatingKeyRing* keyring);
    int handle_auth_reply_more(AuthConnectionMeta* auth_meta, const ceph::buffer::list& bl, ceph::buffer::list* reply);
    int handle_auth_done(AuthConnectionMeta* auth_meta, uint64_t global_id, const ceph::buffer::list& bl,
    ...
    bool is_con(Connection* c) const { return con.get() == c; }
    void queue_command(Message* m) { pending_tell_command = m; }

private:
    int _negotiate(MAuthReply* m, const EntityName& entity_name, uint32_t want_keys, RotatingKeyRing* keyring);
    int _init_auth(uint32_t method, const EntityName& entity_name, uint32_t want_keys, RotatingKeyRing* keyring,
                   bool msgr2);
...
struct MonClientPinger : public Dispatcher, public AuthClient
{
    ...
    MonClientPinger(CephContext* cct_, RotatingKeyRing* keyring, std::string* res_)
        : Dispatcher(cct_)
        , result(res_)
        , done(false)
        , keyring(keyring)
    ...
    int wait_for_reply(double timeout = 0.0)
    {
        ...
        if (ping_recvd_cond.wait_for(locker, ceph::make_timespan(timeout), [this] { return done; })) {
            return 0;
        }
        else {
            return ETIMEDOUT;
    ...
    bool ms_dispatch(Message* m) override
    {
        ...
        if (m->get_type() != CEPH_MSG_PING) return false;
    ...
    // AuthClient
    int get_auth_request(Connection* con, AuthConnectionMeta* auth_meta, uint32_t* auth_method,
                         std::vector<uint32_t>* preferred_modes, ceph::buffer::list* bl) override
    {
        return mc->get_auth_request(auth_method, preferred_modes, bl, cct->_conf->name, 0, keyring);
    }
    int handle_auth_reply_more(Connection* con, AuthConnectionMeta* auth_meta, const ceph::buffer::list& bl,
                               ceph::buffer::list* reply) override
    {
        return mc->handle_auth_reply_more(auth_meta, bl, reply);
    }
    int handle_auth_done(Connection* con, AuthConnectionMeta* auth_meta, uint64_t global_id, uint32_t con_mode,
                         const ceph::buffer::list& bl, CryptoKey* session_key, std::string* connection_secret) override
    {
        return mc->handle_auth_done(auth_meta, global_id, bl, session_key, connection_secret);
    }
    int handle_auth_bad_method(Connection* con, AuthConnectionMeta* auth_meta, uint32_t old_auth_method, int result,
                               const std::vector<uint32_t>& allowed_methods,
                               const std::vector<uint32_t>& allowed_modes) override
    {
        return mc->handle_auth_bad_method(old_auth_method, result, allowed_methods, allowed_modes);
    }
};
...
template<> struct is_error_code_enum<::monc_errc>
{
...
private:
    ...
    int call(std::string_view command, const cmdmap_t& cmdmap, const ceph::buffer::list& inbl, ceph::Formatter* f,
             std::ostream& errss, ceph::buffer::list& out) override;
    ...
    std::map<entity_addrvec_t, MonConnection>::iterator _find_pending_con(const ConnectionRef& con)
    {
...
public:
    // AuthClient
    int get_auth_request(Connection* con, AuthConnectionMeta* auth_meta, uint32_t* method,
                         std::vector<uint32_t>* preferred_modes, ceph::buffer::list* bl) override;
    int handle_auth_reply_more(Connection* con, AuthConnectionMeta* auth_meta, const ceph::buffer::list& bl,
                               ceph::buffer::list* reply) override;
    int handle_auth_done(Connection* con, AuthConnectionMeta* auth_meta, uint64_t global_id, uint32_t con_mode,
    ...
    int handle_auth_bad_method(Connection* con, AuthConnectionMeta* auth_meta, uint32_t old_auth_method, int result,
                               ...
                               const std::vector<uint32_t>& allowed_modes) override;
    // AuthServer
    int handle_auth_request(Connection* con, AuthConnectionMeta* auth_meta, bool more, uint32_t auth_method,
                            const ceph::buffer::list& bl, ceph::buffer::list* reply) override;

    void set_entity_name(EntityName name) { entity_name = name; }
    void set_handle_authentication_dispatcher(Dispatcher* d) { handle_authentication_dispatcher = d; }
...
public:
    ...
    void set_log_client(LogClient* clog) { log_client = clog; }
    LogClient* get_log_client() { return log_client; }
    ...
    void send_mon_message(Message* m) { send_mon_message(MessageRef{m, false}); }
    ...
    const uuid_d& get_fsid() const { return monmap.fsid; }

    entity_addrvec_t get_mon_addrs(unsigned i) const
    {
        std::lock_guard l(monc_lock);
        if (i < monmap.size()) return monmap.get_addrs(i);
    ...
    void set_want_keys(uint32_t want) { want_keys = want; }
...
private:
    ...
    struct MonCommand
    {
        ...
        {
            auto timeout = monc.cct->_conf.get_val<std::chrono::seconds>("rados_mon_op_timeout");
            if (timeout.count() > 0) {
                cancel_timer.emplace(monc.service, timeout);
                cancel_timer->async_wait([this, &monc](boost::system::error_code ec) {
                    if (ec) return;
        ...
        bool is_tell() const { return target_name.size() || target_rank >= 0; }
    };
    friend MonCommand;
    ...
    void _finish_command(MonCommand* r, boost::system::error_code ret, std::string_view rs, bufferlist&& bl);
    void _finish_auth();
...
public:
    ...
    auto start_mon_command(const std::vector<std::string>& cmd, const ceph::buffer::list& inbl, CompletionToken&& token)
    {
    ...
    template<typename CompletionToken>
    auto start_mon_command(int mon_rank, const std::vector<std::string>& cmd, const ceph::buffer::list& inbl,
    ...
    template<typename CompletionToken>
    auto start_mon_command(const std::string& mon_name, const std::vector<std::string>& cmd,
        ...
        boost::asio::async_completion<CompletionToken, CommandSig> init(token);
        {
            ...
            if (!initialized || stopping) {
                ...
                if (err.size() == 0 && rank >= 0) {
                    ldout(cct, 10) << __func__ << " interpreting name '" << mon_name << "' as rank " << rank << dendl;
                    r->target_rank = rank;
                }
                else {
    ...
    public:
        ...
        void operator()(boost::system::error_code e, std::string s, ceph::bufferlist bl)
        {
            if (outs) *outs = std::move(s);
            if (outbl) *outbl = std::move(bl);
            if (onfinish) onfinish->complete(ceph::from_error_code(e));
        }
    };

    void start_mon_command(const std::vector<std::string>& cmd, const bufferlist& inbl, bufferlist* outbl,
                           std::string* outs, Context* onfinish)
    {
        start_mon_command(cmd, inbl, ContextVerter(outs, outbl, onfinish));
    }
    void start_mon_command(int mon_rank, const std::vector<std::string>& cmd, const bufferlist& inbl, bufferlist* outbl,
                           std::string* outs, Context* onfinish)
    {
...
public:
     ...
     */
    template<typename CompletionToken> auto get_version(std::string&& map, CompletionToken&& token)
    {
        boost::asio::async_completion<CompletionToken, VersionSig> init(token);
        {
            ...
            version_requests.emplace(
                m->handle, VersionCompletion::create(service.get_executor(), std::move(init.completion_handler)));
            _send_mon_message(m);
        }
        return init.result.get();
    }
    ...
    auto with_monmap(Callback&& cb, Args&&... args) const -> decltype(cb(monmap, std::forward<Args>(args)...))
    {
        std::lock_guard l(monc_lock);
    ...
    void register_config_notify_callback(std::function<void(void)> f) { config_notify_cb = f; }
