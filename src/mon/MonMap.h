...
struct mon_info_t
{
    ...
    mon_info_t(const std::string& n, const entity_addrvec_t& p_addrs, uint16_t p, uint16_t w)
        : name(n)
...
public:
     ...
     * @returns the set of features required by this monmap
     */
    mon_feature_t get_required_features() const { return (persistent_features | optional_features); }

    // upgrade gate
    ceph_release_t min_mon_release{ceph_release_t::unknown};

    void _add_ambiguous_addr(const std::string& name, entity_addr_t addr, int priority, int weight, bool for_mkfs);
...
public:
    ...
    unsigned size() const { return mon_info.size(); }
     ...
     */
    void add(const std::string& name, const entity_addrvec_t& addrv, uint16_t priority = 0, uint16_t weight = 0)
    {
    ...
    bool contains(const std::string& name) const { return mon_info.count(name); }
    ...
    std::string get_name(const entity_addrvec_t& av) const
    {
        for (auto& i : av.v) {
            ...
            if (p != addr_mons.end()) return p->second;
        }
        return std::string();
    }
    ...
    {
        if (auto found = std::find(ranks.begin(), ranks.end(), n); found != ranks.end()) {
    ...
    {
        if (addr_mons.count(a) == 0) return false;
        name = addr_mons[a];
        return true;
    }
    ...
    void generate_fsid() { fsid.generate_random(); }
     ...
     */
    void set_initial_members(CephContext* cct, std::list<std::string>& initial_members, std::string my_name,
                             const entity_addrvec_t& my_addrs, std::set<entity_addrvec_t>* removed);
...
protected:
     ...
     */
    void init_with_addrs(const std::vector<entity_addrvec_t>& addrs, bool for_mkfs, std::string_view prefix);
    /**
     ...
     */
    int init_with_ips(const std::string& ips, bool for_mkfs, std::string_view prefix);
    /**
     ...
     * @return 0 for success, -errno on error
     */
    int init_with_hosts(const std::string& hostlist, bool for_mkfs, std::string_view prefix);
    ...
    int init_with_dns_srv(CephContext* cct, std::string srv_name, bool for_mkfs, std::ostream& errout);
#endif
};
