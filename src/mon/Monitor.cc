...
{
    return *_dout << "mon." << mon->name << "@" << mon->rank << "(" << mon->get_state_name() << ") e"
...
#define COMMAND(parsesig, helptext, modulename, req_perms) {parsesig, helptext, modulename, req_perms, FLAG(NONE)},
...
Monitor::Monitor(CephContext* cct_, string nm, MonitorDBStore* s, Messenger* m, Messenger* mgr_m, MonMap* map)
    : Dispatcher(cct_)
    , AuthServer(cct_)
    , name(nm)
    , rank(-1)
    ...
    , auth_cluster_required(
          cct, cct->_conf->auth_supported.empty() ? cct->_conf->auth_cluster_required : cct->_conf->auth_supported)
    , auth_service_required(
          cct, cct->_conf->auth_supported.empty() ? cct->_conf->auth_service_required : cct->_conf->auth_supported)
    , mgr_messenger(mgr_m)
    ...
    , op_tracker(cct, g_conf().get_val<bool>("mon_enable_op_tracker"), 1)
{
    ...
    op_tracker.set_complaint_and_threshold(g_conf().get_val<std::chrono::seconds>("mon_op_complaint_time").count(),
...
int Monitor::do_admin_command(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, std::ostream& err,
                              std::ostream& out)
{
    ...
    for (auto p = cmdmap.begin(); p != cmdmap.end(); ++p) {
        if (p->first == "prefix") continue;
        if (!args.empty()) args += ", ";
        args += cmd_vartype_stringify(p->second);
    }
    args = "[" + args + "]";

    bool read_only = (command == "mon_status" || command == "mon metadata" || command == "quorum_status" ||
                      command == "ops" || command == "sessions");

    (read_only ? audit_clog->debug() : audit_clog->info()) << "from='admin socket' entity='admin socket' "
    ...
    if (command == "mon_status") {
        ...
        if (!cmd_getval(cmdmap, "yes_i_really_mean_it", validate)) {
            std::string v;
            if (cmd_getval(cmdmap, "validate", v) && v == "--yes-i-really-mean-it") {
                validate = true;
            }
        }
        if (!validate) {
        ...
        if (!_add_bootstrap_peer_hint(command, cmdmap, out)) goto abort;
    }
        ...
        if (!get_quorum_mon_features().contains_all(ceph::features::mon::FEATURE_PINGING)) {
        ...
        uint64_t smart_timeout = cct->_conf.get_val<uint64_t>("mon_smart_report_timeout");
        for (auto& devname : devnames) {
            ...
            if (block_device_get_metrics(devname, smart_timeout, &smart_json)) {
        ...
        out << "compacted " << g_conf().get_val<std::string>("mon_keyvaluedb") << " in " << duration << " seconds";
    }
    else {
        ceph_abort_msg("bad AdminSocket command binding");
    }
    (read_only ? audit_clog->debug() : audit_clog->info()) << "from='admin socket' "
                                                           << "entity='admin socket' "
                                                           << "cmd=" << command << " "
                                                           << "args=" << args << ": finished";
    return r;
...
CompatSet Monitor::get_initial_supported_features()
{
    ...
    return CompatSet(ceph_mon_feature_compat, ceph_mon_feature_ro_compat, ceph_mon_feature_incompat);
}
...
{
    static const char* KEYS[] = {"crushtool",   // helpful for testing
...
void Monitor::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    sanitize_options();
    ...
    if (changed.count("mon_health_to_clog") || changed.count("mon_health_to_clog_interval") ||
    ...
    if (changed.count("mon_scrub_interval")) {
        auto scrub_interval = conf.get_val<std::chrono::seconds>("mon_scrub_interval");
...
int Monitor::sanitize_options()
{
    ...
    if (g_conf()->mon_lease_renew_interval_factor >= 1.0) {
        clog->error() << "mon_lease_renew_interval_factor (" << g_conf()->mon_lease_renew_interval_factor
    ...
    if (g_conf()->mon_lease_ack_timeout_factor <= 1.0) {
        clog->error() << "mon_lease_ack_timeout_factor (" << g_conf()->mon_lease_ack_timeout_factor
...
int Monitor::preinit()
{
    ...
    ceph_assert(!logger);
    {
        ...
        pcb.add_u64(l_mon_num_sessions, "num_sessions", "Open sessions", "sess", PerfCountersBuilder::PRIO_USEFUL);
        pcb.add_u64_counter(
            l_mon_session_add, "session_add", "Created sessions", "sadd", PerfCountersBuilder::PRIO_INTERESTING);
        pcb.add_u64_counter(
            l_mon_session_rm, "session_rm", "Removed sessions", "srm", PerfCountersBuilder::PRIO_INTERESTING);
        pcb.add_u64_counter(
            l_mon_session_trim, "session_trim", "Trimmed sessions", "strm", PerfCountersBuilder::PRIO_USEFUL);
        pcb.add_u64_counter(l_mon_num_elections,
                            "num_elections",
                            "Elections participated in",
                            "ecnt",
                            PerfCountersBuilder::PRIO_USEFUL);
        pcb.add_u64_counter(
            l_mon_election_call, "election_call", "Elections started", "estt", PerfCountersBuilder::PRIO_INTERESTING);
        pcb.add_u64_counter(
            l_mon_election_win, "election_win", "Elections won", "ewon", PerfCountersBuilder::PRIO_INTERESTING);
        pcb.add_u64_counter(
            l_mon_election_lose, "election_lose", "Elections lost", "elst", PerfCountersBuilder::PRIO_INTERESTING);
    ...
    ceph_assert(!cluster_logger);
    {
        ...
        pcb.add_u64(l_cluster_num_object_misplaced,
    ...
    if (!has_ever_joined) {
        ...
        if (!initial_members.empty()) {
            ...
            monmap->set_initial_members(
                g_ceph_context, initial_members, name, messenger->get_myaddrs(), &extra_probe_peers);
    ...
    for (const auto& command : local_mon_commands) {
        ...
        if (prefix == "injectargs" || prefix == "version" || prefix == "tell") {
            // not registerd by me
            continue;
        }
        r = admin_socket->register_command(command.cmdstring, admin_hook, command.helpstring);
        ceph_assert(r == 0);
    }
    l.lock();
...
void Monitor::shutdown()
{
    ...
    if (cluster_logger) {
        if (cluster_logger_registered) cct->get_perfcounters_collection()->remove(cluster_logger);
...
void Monitor::bootstrap()
{
    ...
    if (newrank >= 0 && monmap->get_addrs(newrank) != messenger->get_myaddrs()) {
        dout(0) << " monmap addrs for rank " << newrank << " changed, i am " << messenger->get_myaddrs()
                << ", monmap is " << monmap->get_addrs(newrank) << ", respawning" << dendl;
    ...
    if (monmap->contains(name)) outside_quorum.insert(name);

    // probe monitors
    dout(10) << "probing other monitors" << dendl;
    for (unsigned i = 0; i < monmap->size(); i++) {
        if ((int)i != rank)
            send_mon_message(new MMonProbe(monmap->fsid, MMonProbe::OP_PROBE, name, has_ever_joined, ceph_release()),
                             i);
    }
    for (auto& av : extra_probe_peers) {
        if (av != messenger->get_myaddrs()) {
            messenger->send_to_mon(
                new MMonProbe(monmap->fsid, MMonProbe::OP_PROBE, name, has_ever_joined, ceph_release()), av);
        }
    }
}

bool Monitor::_add_bootstrap_peer_hint(std::string_view cmd, const cmdmap_t& cmdmap, ostream& ss)
{
    ...
    if (cmd_getval(cmdmap, "addr", addrstr)) {
        ...
        if (!addr.parse(addrstr, entity_addr_t::TYPE_ANY)) {
            ss << "failed to parse addrs '" << addrstr << "'; syntax is 'add_bootstrap_peer_hint ip[:port]'";
            return false;
        }
...
void Monitor::sync_obtain_latest_monmap(bufferlist& bl)
{
    ...
    if (store->exists("mon_sync", "latest_monmap")) {
        ...
        if (backup_monmap.epoch > latest_monmap.epoch) latest_monmap = backup_monmap;
    }

    // Check if our current monmap's epoch is greater than the one we've
    // got so far.
    if (monmap->epoch > latest_monmap.epoch) latest_monmap = *monmap;
...
void Monitor::sync_start(entity_addrvec_t& addrs, bool full)
{
    ...
    ceph_assert(state == STATE_PROBING || state == STATE_SYNCHRONIZING);
    ...
    if (!sync_full) m->last_committed = paxos->get_version();
...
{
    dout(10) << __func__ << dendl;
    if (sync_timeout_event) timer.cancel_event(sync_timeout_event);
    sync_timeout_event =
        timer.add_event_after(g_conf()->mon_sync_timeout, new C_MonContext{this, [this](int) { sync_timeout(); }});
}
...
void Monitor::handle_sync(MonOpRequestRef op)
{
    ...
    case MMonSync::OP_GET_COOKIE_RECENT: handle_sync_get_cookie(op); break;
    ...
    case MMonSync::OP_COOKIE: handle_sync_cookie(op); break;

    case MMonSync::OP_CHUNK:
    case MMonSync::OP_LAST_CHUNK: handle_sync_chunk(op); break;
    case MMonSync::OP_NO_COOKIE: handle_sync_no_cookie(op); break;

    default: dout(0) << __func__ << " unknown op " << m->op << dendl; ceph_abort_msg("unknown op");
    }
}
...
void Monitor::handle_sync_get_cookie(MonOpRequestRef op)
{
    ...
    if ((required_features ^ m->get_connection()->get_features()) & required_features) {
        dout(5) << " ignoring peer mon." << m->get_source().num() << " has features " << std::hex
                << m->get_connection()->get_features() << " but we require " << required_features << std::dec << dendl;
        return;
    }
...
void Monitor::handle_sync_get_chunk(MonOpRequestRef op)
{
    ...
    if (sp.last_committed < paxos->get_first_committed() && paxos->get_first_committed() > 1) {
        dout(10) << __func__ << " sync requester fell behind paxos, their lc " << sp.last_committed << " < our fc "
                 << paxos->get_first_committed() << dendl;
    ...
    while (sp.last_committed < paxos->get_version() && bytes_left > 0 && keys_left > 0) {
        bufferlist bl;
        sp.last_committed++;
    ...
    if ((sp.full && sp.synchronizer->has_next_chunk()) || sp.last_committed < paxos->get_version()) {
...
{
    cancel_probe_timeout();
    probe_timeout_event = new C_MonContext{this, [this](int r) { probe_timeout(r); }};
...
void Monitor::handle_probe(MonOpRequestRef op)
{
    ...
    switch (m->op) {
    case MMonProbe::OP_PROBE: handle_probe_probe(op); break;

    case MMonProbe::OP_REPLY: handle_probe_reply(op); break;

    case MMonProbe::OP_MISSING_FEATURES:
        derr << __func__ << " require release " << (int)m->mon_release << " > " << (int)ceph_release()
             << ", or missing features (have " << CEPH_FEATURES_ALL << ", required " << m->required_features
             << ", missing " << (m->required_features & ~CEPH_FEATURES_ALL) << ")" << dendl;
        break;
    }
}
...
void Monitor::handle_probe_probe(MonOpRequestRef op)
{
    ...
    dout(10) << "handle_probe_probe " << m->get_source_inst() << " " << *m << " features "
             << m->get_connection()->get_features() << dendl;
    uint64_t missing = required_features & ~m->get_connection()->get_features();
    if ((m->mon_release != ceph_release_t::unknown && m->mon_release < monmap->min_mon_release) || missing) {
        dout(1) << " peer " << m->get_source_addr() << " release " << m->mon_release << " < min_mon_release "
                << monmap->min_mon_release << ", or missing features " << missing << dendl;
        MMonProbe* r =
            new MMonProbe(monmap->fsid, MMonProbe::OP_MISSING_FEATURES, name, has_ever_joined, monmap->min_mon_release);
    ...
    MMonProbe* r;
    r = new MMonProbe(monmap->fsid, MMonProbe::OP_REPLY, name, has_ever_joined, ceph_release());
    r->name = name;
    r->quorum = quorum;
    r->leader = leader;
...
void Monitor::handle_probe_reply(MonOpRequestRef op)
{
    ...
    if (!mybl.contents_equal(m->monmap_bl)) {
        ...
        if (m->has_ever_joined && (newmap->get_epoch() > monmap->get_epoch() || !has_ever_joined)) {
            dout(10) << " got newer/committed monmap epoch " << newmap->get_epoch() << ", mine was "
                     << monmap->get_epoch() << dendl;
    ...
    if (monmap->get_epoch() == 0 && peer_name.compare(0, 7, "noname-") == 0) {
        dout(10) << " renaming peer " << m->get_source_addr() << " " << peer_name << " -> " << m->name
                 << " in my monmap" << dendl;
    ...
    // new initial peer?
    if (monmap->get_epoch() == 0 && monmap->contains(m->name) && monmap->get_addrs(m->name).front().is_blank_ip()) {
    ...
    if (m->paxos_last_version < sync_last_committed_floor) {
        dout(10) << " peer paxos versions [" << m->paxos_first_version << "," << m->paxos_last_version
                 << "] < my sync_last_committed_floor " << sync_last_committed_floor << ", ignoring" << dendl;
    }
    else {
        if (paxos->get_version() < m->paxos_first_version &&
            m->paxos_first_version > 1) {   // no need to sync if we're 0 and they start at 1.
            dout(10) << " peer paxos first versions [" << m->paxos_first_version << "," << m->paxos_last_version << "]"
        ...
        if (paxos->get_version() + g_conf()->paxos_max_join_drift < m->paxos_last_version) {
            dout(10) << " peer paxos last version " << m->paxos_last_version << " vs my version "
                     << paxos->get_version() << " (too far ahead)" << dendl;
            cancel_probe_timeout();
    ...
    if (m->quorum.size()) {
        ...
        dout(10) << " peer paxos version " << m->paxos_last_version << " vs my version " << paxos->get_version()
                 << " (ok)" << dendl;
        bool in_map = false;
        ...
        if (in_map && !monmap->get_addrs(name).front().is_blank_ip() &&
            ...
            dout(10) << " ready to join, but i'm not in the monmap/"
                        "my addr is blank/location is wrong, trying to join"
                     << dendl;
            send_mon_message(new MMonJoin(monmap->fsid, name, messenger->get_myaddrs(), crush_loc, need_set_crush_loc),
                             m->leader);
        }
    }
    else {
...
void Monitor::win_standalone_election()
{
    ...
    win_election(
        elector.get_epoch(), q, CEPH_FEATURES_ALL, ceph::features::mon::get_supported(), ceph_release(), metadata);
}
...
void Monitor::_finish_svc_election()
{
    ...
    for (auto& svc : paxos_service) {
        ...
        if (state == STATE_LEADER && svc.get() == monmon()) continue;
        svc->election_finished();
    }
}

void Monitor::win_election(epoch_t epoch, const set<int>& active, uint64_t features, const mon_feature_t& mon_features,
                           ceph_release_t min_mon_release, const map<int, Metadata>& metadata)
{
    dout(10) << __func__ << " epoch " << epoch << " quorum " << active << " features " << features << " mon_features "
             << mon_features << " min_mon_release " << min_mon_release << dendl;
    ...
    clog->info() << "mon." << name << " is new leader, mons " << get_quorum_names() << " in quorum (ranks " << quorum
                 << ")";
    ...
    finish_election();
    if (monmap->size() > 1 && monmap->get_epoch() > 0) {
        timecheck_start();
        health_tick_start();
        ...
        healthmon()->wait_for_active_ctx(new LambdaContext([this](int r) {
            ...
            if (healthmon()->is_proposing()) {
                ...
                healthmon()->wait_for_finished_proposal(
                    nullptr, new C_MonContext{this, [this](int r) {
...
void Monitor::lose_election(epoch_t epoch, set<int>& q, int l, uint64_t features, const mon_feature_t& mon_features,
                            ceph_release_t min_mon_release)
{
    ...
    quorum_mon_features = mon_features;
    quorum_min_mon_release = min_mon_release;
    dout(10) << "lose_election, epoch " << epoch << " leader is mon" << leader << " quorum is " << quorum
             << " features are " << quorum_con_features << " mon_features are " << quorum_mon_features
             << " min_mon_release " << min_mon_release << dendl;
...
void Monitor::finish_election()
{
    ...
    if (cur_name != name || (need_set_crush_loc && map_crush_loc != crush_loc)) {
        dout(10) << " renaming/moving myself from " << cur_name << "/" << map_crush_loc << " -> " << name << "/"
                 << crush_loc << dendl;
        send_mon_message(new MMonJoin(monmap->fsid, name, messenger->get_myaddrs(), crush_loc, need_set_crush_loc),
                         leader);
        return;
...
void Monitor::apply_monmap_to_compatset_features()
{
    ...
    if (monmap_features.contains_all(ceph::features::mon::FEATURE_KRAKEN)) {
        ceph_assert(ceph::features::mon::get_persistent().contains_all(ceph::features::mon::FEATURE_KRAKEN));
    ...
    if (monmap_features.contains_all(ceph::features::mon::FEATURE_LUMINOUS)) {
        ceph_assert(ceph::features::mon::get_persistent().contains_all(ceph::features::mon::FEATURE_LUMINOUS));
        // this feature should only ever be set if the quorum supports it.
    ...
    if (monmap_features.contains_all(ceph::features::mon::FEATURE_MIMIC)) {
        ceph_assert(ceph::features::mon::get_persistent().contains_all(ceph::features::mon::FEATURE_MIMIC));
        // this feature should only ever be set if the quorum supports it.
    ...
    if (monmap_features.contains_all(ceph::features::mon::FEATURE_NAUTILUS)) {
        ceph_assert(ceph::features::mon::get_persistent().contains_all(ceph::features::mon::FEATURE_NAUTILUS));
        // this feature should only ever be set if the quorum supports it.
    ...
    if (monmap_features.contains_all(ceph::features::mon::FEATURE_PACIFIC)) {
        ceph_assert(ceph::features::mon::get_persistent().contains_all(ceph::features::mon::FEATURE_PACIFIC));
        // this feature should only ever be set if the quorum supports it.
    ...
    if (monmap_features.contains_all(ceph::features::mon::FEATURE_REEF)) {
        ceph_assert(ceph::features::mon::get_persistent().contains_all(ceph::features::mon::FEATURE_REEF));
        // this feature should only ever be set if the quorum supports it.
...
void Monitor::calc_quorum_requirements()
{
    ...
    if (features.incompat.contains(CEPH_MON_FEATURE_INCOMPAT_NAUTILUS)) {
        required_features |= CEPH_FEATUREMASK_SERVER_NAUTILUS | CEPH_FEATUREMASK_CEPHX_V2;
    }
    ...
    // monmap
    if (monmap->get_required_features().contains_all(ceph::features::mon::FEATURE_KRAKEN)) {
        required_features |= CEPH_FEATUREMASK_SERVER_KRAKEN;
    }
    if (monmap->get_required_features().contains_all(ceph::features::mon::FEATURE_LUMINOUS)) {
        required_features |= CEPH_FEATUREMASK_SERVER_LUMINOUS;
    }
    if (monmap->get_required_features().contains_all(ceph::features::mon::FEATURE_MIMIC)) {
        required_features |= CEPH_FEATUREMASK_SERVER_MIMIC;
    }
    if (monmap->get_required_features().contains_all(ceph::features::mon::FEATURE_NAUTILUS)) {
        required_features |= CEPH_FEATUREMASK_SERVER_NAUTILUS | CEPH_FEATUREMASK_CEPHX_V2;
    }
...
void Monitor::get_mon_status(Formatter* f)
{
    ...
    for (set<string>::iterator p = outside_quorum.begin(); p != outside_quorum.end(); ++p) f->dump_string("mon", *p);
    f->close_section();   // outside_quorum

    f->open_array_section("extra_probe_peers");
    for (set<entity_addrvec_t>::iterator p = extra_probe_peers.begin(); p != extra_probe_peers.end(); ++p) {
        f->dump_object("peer", *p);
    }
    f->close_section();   // extra_probe_peers

    f->open_array_section("sync_provider");
    for (map<uint64_t, SyncProvider>::const_iterator p = sync_providers.begin(); p != sync_providers.end(); ++p) {
    ...
    if (g_conf()->mon_sync_provider_kill_at > 0) f->dump_int("provider_kill_at", g_conf()->mon_sync_provider_kill_at);
...
{
    if (!cct->_conf->mon_health_to_clog || cct->_conf->mon_health_to_clog_tick_interval <= 0) return;

    dout(15) << __func__ << dendl;

    health_tick_stop();
    health_tick_event =
        timer.add_event_after(cct->_conf->mon_health_to_clog_tick_interval, new C_MonContext{this, [this](int r) {
                                                                                                 if (r < 0) return;
                                                                                                 health_tick_start();
                                                                                             }});
}
...
ceph::real_clock::time_point Monitor::health_interval_calc_next_update()
{
    ...
    dout(20) << __func__ << " now: " << now << ","
             << " next: " << next << ","
             << " interval: " << cct->_conf->mon_health_to_clog_interval << dendl;
...
void Monitor::health_interval_start()
{
    ...
    if (!cct->_conf->mon_health_to_clog || cct->_conf->mon_health_to_clog_interval <= 0) {
        return;
    }
...
void Monitor::do_health_to_clog_interval()
{
    ...
    if (!cct->_conf->mon_health_to_clog || cct->_conf->mon_health_to_clog_interval <= 0) return;
...
void Monitor::do_health_to_clog(bool force)
{
    ...
    if (!force && summary == health_status_cache.summary && level == health_status_cache.overall) return;

    if (g_conf()->mon_health_detail_to_clog && summary != health_status_cache.summary && level != HEALTH_OK) {
        string details;
...
void Monitor::log_health(const health_check_map_t& updated, const health_check_map_t& previous,
                         MonitorDBStore::TransactionRef t)
{
    ...
    for (auto& p : updated.checks) {
        ...
        if (q == previous.checks.end()) {
            // new
            ostringstream ss;
            ss << "Health check failed: " << p.second.summary << " (" << p.first << ")";
            clog->health(p.second.severity) << ss.str();

            logged = true;
        }
        else {
            if (p.second.summary != q->second.summary || p.second.severity != q->second.severity) {
        ...
        if (logged) {
            ...
            if (iter == health_check_log_times.end()) {
                health_check_log_times.emplace(p.first, HealthCheckLogStatus(p.second.severity, p.second.summary, now));
            }
            else {
                iter->second = HealthCheckLogStatus(p.second.severity, p.second.summary, now);
            }
        }
    }
...
void Monitor::get_cluster_status(stringstream& ss, Formatter* f, MonSession* session)
{
    if (f) f->open_object_section("status");
    ...
    if (f) {
        ...
        {
            f->open_array_section("quorum");
            for (set<int>::iterator p = quorum.begin(); p != quorum.end(); ++p) f->dump_int("rank", *p);
            f->close_section();
            f->open_array_section("quorum_names");
            for (set<int>::iterator p = quorum.begin(); p != quorum.end(); ++p)
        ...
        ss << "\n \n  services:\n";
        {
            ...
            ss << "    mon: " << spacing << mon_count << " daemons, quorum " << quorum_names << " (age "
               << timespan_str(mnow - quorum_since) << ")";
            if (quorum_names.size() != mon_count) {
                ...
                ss << ", out of quorum: " << joinify(out_of_q.begin(), out_of_q.end(), std::string(", "));
            }
            ss << "\n";
            if (mgrmon()->in_use()) {
                ss << "    mgr: " << spacing;
            ...
            for (auto& p : service_map.services) {
                ...
                ss << "    " << p.first << ": " << string(maxlen - p.first.size(), ' ') << p.second.get_summary()
                   << "\n";
            }
        }

        if (auto& service_map = mgrstatmon()->get_service_map();
            std::any_of(service_map.services.begin(), service_map.services.end(), [](auto& service) {
...
void Monitor::_generate_command_map(cmdmap_t& cmdmap, map<string, string>& param_str_map)
{
    for (auto p = cmdmap.begin(); p != cmdmap.end(); ++p) {
        if (p->first == "prefix") continue;
        if (p->first == "caps") {
            vector<string> cv;
            if (cmd_getval(cmdmap, "caps", cv) && cv.size() % 2 == 0) {
...
const MonCommand* Monitor::_get_moncommand(const string& cmd_prefix, const vector<MonCommand>& cmds)
{
    for (auto& c : cmds) {
...
bool Monitor::_allowed_command(MonSession* s, const string& module, const string& prefix, const cmdmap_t& cmdmap,
                               const map<string, string>& param_str_map, const MonCommand* this_cmd)
{
    ...
    bool capable = s->caps.is_capable(
        g_ceph_context, s->entity_name, module, prefix, param_str_map, cmd_r, cmd_w, cmd_x, s->get_peer_socket_addr());

    dout(10) << __func__ << " " << (capable ? "" : "not ") << "capable" << dendl;
    return capable;
}

void Monitor::format_command_descriptions(const std::vector<MonCommand>& commands, Formatter* f, uint64_t features,
                                          bufferlist* rdata)
{
    int cmdnum = 0;
    ...
    for (const auto& cmd : commands) {
        ...
        dump_cmddesc_to_json(
            f, features, secname.str(), cmd.cmdstring, cmd.helpstring, cmd.module, cmd.req_perms, flags);
        cmdnum++;
    }
...
void Monitor::handle_tell_command(MonOpRequestRef op)
{
    ...
    if (auto cmd = _get_moncommand(prefix, get_local_commands(quorum_mon_features)); cmd) {
        if (cmd->is_obsolete() || (cct->_conf->mon_debug_deprecated_as_obsolete && cmd->is_deprecated())) {
            return reply_tell_command(op,
                                      -ENOTSUP,
                                      "command is obsolete; "
...
void Monitor::handle_command(MonOpRequestRef op)
{
    ...
    if (prefix == "get_command_descriptions") {
        ...
        std::vector<MonCommand> commands =
            static_cast<MgrMonitor*>(paxos_service[PAXOS_MGR].get())->get_command_descs();
    ...
    const MonCommand* mon_cmd = _get_moncommand(prefix, get_local_commands(quorum_mon_features));
    if (!mon_cmd) {
        mon_cmd = mgr_cmd;
    }
    if (!is_leader()) {
        if (!mon_cmd) {
            if (leader_cmd->is_noforward()) {
                reply_command(op, -EINVAL, "command not locally supported and not allowed to forward", 0);
                return;
            }
            ...
            if (mon_cmd->is_noforward()) {
                reply_command(op, -EINVAL, "command not compatible with leader and not allowed to forward", 0);
                return;
            }
    ...
    if (mon_cmd->is_obsolete() || (cct->_conf->mon_debug_deprecated_as_obsolete && mon_cmd->is_deprecated())) {
        reply_command(op, -ENOTSUP, "command is obsolete; please check usage and/or man page", 0);
        return;
    }
    ...
    bool cmd_is_rw = (mon_cmd->requires_perm('w') || mon_cmd->requires_perm('x'));
    ...
    if (!_allowed_command(session, service, prefix, cmdmap, param_str_map, mon_cmd)) {
    ...
    if ((!HAVE_FEATURE(m->get_connection()->get_features(), SERVER_OCTOPUS) ||
        ...
        (prefix == "injectargs" || prefix == "smart" || prefix == "mon_status" || prefix == "heap")) {
        if (m->get_connection()->get_messenger() == 0) {
            ...
            reply_command(op, -EBUSY, "failing forwarded tell command in mixed-version mon cluster", 0);
            return;
        }
    ...
    if (mon_cmd->is_mgr()) {
        ...
        if (mgr_proxy_bytes + size > max) {
            dout(10) << __func__ << " current mgr proxy bytes " << mgr_proxy_bytes << " + " << size << " > max " << max
                     << dendl;
        ...
        mgr_client.start_command(m->cmd, m->get_data(), &fin->outbl, &fin->outs, new C_OnFinisher(fin, &finisher));
        return;
    }

    if ((module == "mds" || module == "fs") && prefix != "fs authorize") {
        mdsmon()->dispatch(op);
        return;
    }
    if ((module == "osd" || prefix == "pg map" || prefix == "pg repeer") && prefix != "osd last-stat-seq") {
        osdmon()->dispatch(op);
        return;
    }
    if (module == "config") {
    ...
    if (module == "mon" &&
         ...
         *  'mon scrub'
         */
        prefix != "mon scrub" && prefix != "mon metadata" && prefix != "mon versions" &&
        prefix != "mon count-metadata" && prefix != "mon ok-to-stop" && prefix != "mon ok-to-add-offline" &&
        prefix != "mon ok-to-rm") {
        monmon()->dispatch(op);
        return;
    }
    ...
    if (prefix == "time-sync-status") {
        ...
        f->dump_stream("round_status") << ((timecheck_round % 2) ? "on-going" : "finished");
        f->close_section();
        f->close_section();
        f->flush(rdata);
        r = 0;
    ...
    else if (prefix == "status" || prefix == "health" || prefix == "df") {
        string detail;
        ...
        if (prefix == "status") {
            ...
            if (f) f->open_object_section("stats");
        ...
        if (!tagstr.empty()) tagstr = tagstr.substr(0, tagstr.find_last_of(' '));
        ...
        if (!all) {
            ...
            for (unsigned int rank = 0; rank < monmap->size(); ++rank) {
                ...
                if (r == -ENOENT || r == -EINVAL) {
                    ...
                    derr << "Unexpected error from get_mon_metadata: " << cpp_strerror(r) << dendl;
                    ds << get_err.str();
                    break;
                }
            }
        ...
        if (wouldbe.size() < monmap->min_quorum_size()) {
            r = -EBUSY;
            rs = "not enough monitors would be available (" + stringify(wouldbe) + ") after stopping mons " +
                 stringify(ids);
            goto out;
        }
        r = 0;
        rs = "quorum should be preserved (" + stringify(wouldbe) + ") after stopping " + stringify(ids);
    }
        ...
        if (quorum.count(rank) && quorum.size() - 1 < monmap->min_quorum_size(monmap->size() - 1)) {
            r = -EBUSY;
...
void Monitor::reply_command(MonOpRequestRef op, int rc, const string& rs, bufferlist& rdata, version_t version)
{
    auto m = op->get_req<MMonCommand>();
...
void Monitor::reply_tell_command(MonOpRequestRef op, int rc, const string& rs)
{
    MCommand* m = static_cast<MCommand*>(op->get_req());
...
void Monitor::forward_request_leader(MonOpRequestRef op)
{
    ...
    if (req->get_source().is_mon() && req->get_source_addrs() != messenger->get_myaddrs()) {
        ...
        MForward* forward = new MForward(rr->tid, req, rr->con_features, rr->session->caps);
...
struct AnonConnection : public Connection
{
    ...
    int send_message(Message* m) override { ceph_assert(!"send_message on anonymous connection"); }
    void send_keepalive() override { ceph_assert(!"send_keepalive on anonymous connection"); }
    ...
    entity_addr_t get_peer_socket_addr() const override { return socket_addr; }
...
void Monitor::handle_forward(MonOpRequestRef op)
{
    ...
    dout(10) << "received forwarded message from " << ceph_entity_type_name(m->client_type) << " " << m->client_addrs
    ...
    if (!session->is_capable("mon", MON_CAP_X)) {
        ...
        s->_ident(req->get_source(), req->get_source_addrs());
...
void Monitor::send_reply(MonOpRequestRef op, Message* reply)
{
    ...
    if (session->proxy_con) {
        dout(15) << "send_reply routing reply to " << con->get_peer_addr() << " via "
                 << session->proxy_con->get_peer_addr() << " for request " << *req << dendl;
...
void Monitor::no_reply(MonOpRequestRef op)
{
    ...
    if (session->proxy_con) {
        dout(10) << "no_reply to " << req->get_source_inst() << " via " << session->proxy_con->get_peer_addr()
...
void Monitor::handle_route(MonOpRequestRef op)
{
    ...
    if (m->send_osdmap_first) {
        ...
        osdmon()->send_incremental(m->send_osdmap_first, rr->session, true, MonOpRequestRef());
    }
...
void Monitor::resend_routed_requests()
{
    ...
    int mon = get_leader();
    list<Context*> retry;
    for (map<uint64_t, RoutedRequest*>::iterator p = routed_requests.begin(); p != routed_requests.end(); ++p) {
...
{
    dout(10) << "remove_session " << s << " " << s->name << " " << s->addrs << " features 0x" << std::hex
             << s->con_features << std::dec << dendl;
    ceph_assert(s->con);
    ceph_assert(!s->closed);
    for (set<uint64_t>::iterator p = s->routed_request_tids.begin(); p != s->routed_request_tids.end(); ++p) {
...
void Monitor::waitlist_or_zap_client(MonOpRequestRef op)
{
    ...
    if (m->get_recv_stamp() > too_old && con->is_connected()) {
...
void Monitor::_ms_dispatch(Message* m)
{
    ...
    if (src_is_mon && s) {
        ...
        if (con->get_messenger() && con->get_features() != s->con_features) {
            ...
            dout(10) << __func__ << " feature change for " << m->get_source_inst() << " (was " << s->con_features
    ...
    if (!s) {
        ...
        if (!src_is_mon && (m->get_type() != CEPH_MSG_AUTH && m->get_type() != CEPH_MSG_MON_GET_MAP &&
        ...
        {
            std::lock_guard l(session_map_lock);
            s = session_map.new_session(m->get_source(), m->get_source_addrs(), con.get());
        }
        ceph_assert(s);
        con->set_priv(RefCountedPtr{s, false});
        dout(10) << __func__ << " new session " << s << " " << *s << " features 0x" << std::hex << s->con_features
                 << std::dec << dendl;
        op->set_session(s);
    ...
    dout(20) << " entity_name " << s->entity_name << " global_id " << s->global_id << " (" << s->global_id_status
             << ") caps " << s->caps.get_str() << dendl;

    if (!session_stretch_allowed(s, op)) {
        return;
    }
    if ((is_synchronizing() || (!s->authenticated && !exited_quorum.is_zero())) && !src_is_mon &&
        m->get_type() != CEPH_MSG_PING) {
...
void Monitor::dispatch_op(MonOpRequestRef op)
{
    ...
    if (!op->get_session()->authenticated) {
        dout(5) << __func__ << " " << op->get_req()->get_source_inst() << " is not authenticated, dropping "
                << *(op->get_req()) << dendl;
        return;
    }
    ...
    ceph_assert(s->global_id_status == global_id_status_t::NONE || s->global_id_status == global_id_status_t::NEW_OK ||
    ...
    if (cct->_conf->auth_expose_insecure_global_id_reclaim &&
        ...
        dout(5) << __func__ << " " << op->get_req()->get_source_inst()
                << " may omit old_ticket on reconnects, discarding " << *op->get_req() << " and forcing reconnect"
                << dendl;
    ...
    switch (op->get_req()->get_type()) {
    case CEPH_MSG_MON_GET_MAP: handle_mon_get_map(op); return;

    case MSG_GET_CONFIG: configmon()->handle_get_config(op); return;
    ...
    case MSG_OSD_PG_READY_TO_MERGE: paxos_service[PAXOS_OSDMAP]->dispatch(op); return;

    // MDSs
    case MSG_MDS_BEACON:
    case MSG_MDS_OFFLOAD_TARGETS: paxos_service[PAXOS_MDSMAP]->dispatch(op); return;

    // Mgrs
    case MSG_MGR_BEACON: paxos_service[PAXOS_MGR]->dispatch(op); return;
    ...
    case MSG_GETPOOLSTATS:
        ...
        // log
    case MSG_LOG: paxos_service[PAXOS_LOG]->dispatch(op); return;
    ...
    if (!op->get_session()->is_capable("mon", MON_CAP_R)) {
        dout(5) << __func__ << " " << op->get_req()->get_source_inst() << " not enough caps for " << *(op->get_req())
                << " -- dropping" << dendl;
        return;
    }
    ...
    switch (op->get_req()->get_type()) {
    // misc
    case CEPH_MSG_MON_GET_VERSION: handle_get_version(op); return;
    }

    if (!op->is_src_mon()) {
        dout(1) << __func__ << " unexpected monitor message from"
                << " non-monitor entity " << op->get_req()->get_source_inst() << " " << *(op->get_req())
                << " -- dropping" << dendl;
        return;
    }
    ...
    case MSG_ROUTE: handle_route(op); return;

    case MSG_MON_PROBE: handle_probe(op); return;

    // Sync (i.e., the new slurp, but on steroids)
    case MSG_MON_SYNC: handle_sync(op); return;
    case MSG_MON_SCRUB: handle_scrub(op); return;

    /* log acks are sent from a monitor we sent the MLog to, and are
       never sent by clients to us. */
    case MSG_LOGACK: log_client.handle_log_ack((MLogAck*)op->get_req()); return;
    ...
    case MSG_FORWARD: handle_forward(op); return;

    case MSG_TIMECHECK: dout(5) << __func__ << " ignoring " << op << dendl; return;
    case MSG_TIMECHECK2: handle_timecheck(op); return;

    case MSG_MON_HEALTH: dout(5) << __func__ << " dropping deprecated message: " << *op->get_req() << dendl; break;
...
void Monitor::timecheck_start()
{
    dout(10) << __func__ << dendl;
    timecheck_cleanup();
    if (get_quorum_mon_features().contains_all(ceph::features::mon::FEATURE_NAUTILUS)) {
...
void Monitor::timecheck_reset_event()
{
    ...
    double delay = cct->_conf->mon_timecheck_skew_interval * timecheck_rounds_since_clean;
    ...
    timecheck_event = timer.add_event_after(delay, new C_MonContext{this, [this](int) { timecheck_start_round(); }});
}
...
void Monitor::timecheck_report()
{
    ...
    for (set<int>::iterator q = quorum.begin(); q != quorum.end(); ++q) {
        if (monmap->get_name(*q) == name) continue;
...
void Monitor::timecheck()
{
    ...
    for (set<int>::iterator it = quorum.begin(); it != quorum.end(); ++it) {
        if (monmap->get_name(*it) == name) continue;
...
health_status_t Monitor::timecheck_status(ostringstream& ss, const double skew_bound, const double latency)
{
...
void Monitor::handle_timecheck_leader(MonOpRequestRef op)
{
    ...
    if (m->epoch < get_epoch()) {
        dout(1) << __func__ << " got old timecheck epoch " << m->epoch << " from " << other << " curr " << get_epoch()
    ...
    if (m->round < timecheck_round) {
        dout(1) << __func__ << " got old round " << m->round << " from " << other << " curr " << timecheck_round
                << " -- discard" << dendl;
        return;
    }
    ...
    if (curr_time < timecheck_sent) {
        ...
        dout(1) << __func__ << " our clock was readjusted --"
                << " bump round and drop current check" << dendl;
        timecheck_cancel_round();
        return;
    }
    ...
    dout(10) << __func__ << " from " << other << " ts " << m->timestamp << " delta " << delta << " skew_bound "
             << skew_bound << " latency " << latency << dendl;
...
void Monitor::handle_timecheck_peon(MonOpRequestRef op)
{
    ...
    if (m->epoch != get_epoch()) {
        dout(1) << __func__ << " got wrong epoch "
                << "(ours " << get_epoch() << " theirs: " << m->epoch << ") -- discarding" << dendl;
        return;
    }

    if (m->round < timecheck_round) {
        dout(1) << __func__ << " got old round " << m->round << " current " << timecheck_round << " (epoch "
...
void Monitor::handle_subscribe(MonOpRequestRef op)
{
    ...
    for (map<string, ceph_mon_subscribe_item>::iterator p = m->what.begin(); p != m->what.end(); ++p) {
        if (p->first == "monmap" || p->first == "config") {
            ...
            dout(5) << __func__ << " " << op->get_req()->get_source_inst() << " not enough caps for "
                    << *(op->get_req()) << " -- dropping" << dendl;
            continue;
        }

        // if there are any non-onetime subscriptions, we need to reply to start the resubscribe timer
        if ((p->second.flags & CEPH_SUBSCRIBE_ONETIME) == 0) reply = true;

        // remove conflicting subscribes
        if (logmon()->sub_name_to_id(p->first) >= 0) {
            for (map<string, Subscription*>::iterator it = s->sub_map.begin(); it != s->sub_map.end();) {
    ...
    if (reply) {
        ...
        if (!con->has_feature(CEPH_FEATURE_MON_STATEFUL_SUB))
            m->get_connection()->send_message(
                new MMonSubscribeAck(monmap->get_fsid(), (int)g_conf()->mon_subscribe_interval));
    }
}
...
bool Monitor::ms_handle_reset(Connection* con)
{
    ...
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_MON) return false;
...
int Monitor::print_nodes(Formatter* f, ostream& err)
{
    ...
    for (map<int, Metadata>::iterator it = mon_metadata.begin(); it != mon_metadata.end(); ++it) {
...
int Monitor::scrub()
{
    ...
    int32_t num_keys = (quorum.size() == 1 ? -1 : cct->_conf->mon_scrub_max_keys);

    for (set<int>::iterator p = quorum.begin(); p != quorum.end(); ++p) {
        if (*p == rank) continue;
        MMonScrub* r = new MMonScrub(MMonScrub::OP_SCRUB, scrub_version, num_keys);
        r->key = scrub_state->last_key;
        send_mon_message(r, *p);
    }

    // scrub my keys
    bool r = _scrub(&scrub_result[rank], &scrub_state->last_key, &num_keys);
...
void Monitor::handle_scrub(MonOpRequestRef op)
{
    ...
    case MMonScrub::OP_SCRUB:
    {
        ...
        if (m->version != paxos->get_version()) break;

        MMonScrub* reply = new MMonScrub(MMonScrub::OP_RESULT, m->version, m->num_keys);
    ...
    {
        if (!is_leader()) break;
        if (m->version != scrub_version) break;
...
bool Monitor::_scrub(ScrubResult* r, pair<string, string>* start, int* num_keys)
{
    ceph_assert(r != NULL);
    ...
    while (it->has_next_chunk()) {
        ...
        if (*num_keys > 0 && scrubbed_keys == *num_keys) break;

        pair<string, string> k = it->get_next_key();
        if (prefixes.count(k.first) == 0) continue;
    ...
    dout(20) << __func__ << " last_key (" << last_key << ")"
             << " scrubbed_keys " << scrubbed_keys << " has_next " << it->has_next_chunk() << dendl;
...
void Monitor::scrub_check_results()
{
    ...
    for (map<int, ScrubResult>::iterator p = scrub_result.begin(); p != scrub_result.end(); ++p) {
        if (p->first == rank) continue;
        if (p->second != mine) {
            ++errors;
    ...
    if (!errors) clog->debug() << "scrub ok on " << quorum << ": " << mine;
}
...
void Monitor::scrub_event_start()
{
    ...
    if (scrub_event) scrub_event_cancel();

    auto scrub_interval = cct->_conf.get_val<std::chrono::seconds>("mon_scrub_interval");
    if (scrub_interval == std::chrono::seconds::zero()) {
        dout(1) << __func__ << " scrub event is disabled"
                << " (mon_scrub_interval = " << scrub_interval << ")" << dendl;
        return;
    }

    scrub_event = timer.add_event_after(scrub_interval, new C_MonContext{this, [this](int) { scrub_start(); }});
}
...
void Monitor::scrub_reset_timeout()
{
    ...
    scrub_cancel_timeout();
    scrub_timeout_event =
        timer.add_event_after(g_conf()->mon_scrub_timeout, new C_MonContext{this, [this](int) { scrub_timeout(); }});
}

/************ TICK ***************/
void Monitor::new_tick()
{
    timer.add_event_after(g_conf()->mon_tick_interval, new C_MonContext{this, [this](int) { tick(); }});
}
...
void Monitor::tick()
{
    ...
    if (is_leader()) {
        const auto min_period = g_conf().get_val<int64_t>("mon_health_log_update_period");
        for (auto& svc : paxos_service) {
            ...
            for (const auto& i : health.checks) {
                ...
                bool const changed = log_status.last_message != summary || log_status.severity != severity;
    ...
    // trim sessions
    {
        ...
        bool out_for_too_long = (!exited_quorum.is_zero() && now > (exited_quorum + 2 * g_conf()->mon_lease));
        ...
        while (!p.end()) {
            ...
            if (s->session_timeout < now) {
                dout(10) << " trimming session " << s->con << " " << s->name << " " << s->addrs << " (timeout "
                         << s->session_timeout << " < now " << now << ")" << dendl;
            }
...
int Monitor::check_fsid()
{
    ...
    if (pos != string::npos) es.resize(pos);
    ...
    if (monmap->get_fsid() != ondisk) {
        derr << "error: cluster_uuid file exists with value " << ondisk << ", != our uuid " << monmap->get_fsid()
             << dendl;
        return -EEXIST;
    }
    ...
    if (is_keyring_required()) {
        ...
        if (r) {
            if (g_conf()->key != "") {
                ...
                derr << "unable to find a keyring on " << g_conf()->keyring << ": " << cpp_strerror(r) << dendl;
                return r;
            }
        }
        else {
...
int Monitor::get_auth_request(Connection* con, AuthConnectionMeta* auth_meta, uint32_t* method,
                              vector<uint32_t>* preferred_modes, bufferlist* out)
{
    ...
    auth_registry.get_supported_modes(con->get_peer_type(), auth->protocol, preferred_modes);
    *method = auth->protocol;
    *out = auth->bl;
    return 0;
}

int Monitor::handle_auth_reply_more(Connection* con, AuthConnectionMeta* auth_meta, const bufferlist& bl,
                                    bufferlist* reply)
{
    std::scoped_lock l(auth_lock);
...
int Monitor::handle_auth_done(Connection* con, AuthConnectionMeta* auth_meta, uint64_t global_id, uint32_t con_mode,
                              const bufferlist& bl, CryptoKey* session_key, std::string* connection_secret)
{
    std::scoped_lock l(auth_lock);
...
int Monitor::handle_auth_bad_method(Connection* con, AuthConnectionMeta* auth_meta, uint32_t old_auth_method,
                                    int result, const std::vector<uint32_t>& allowed_methods,
...
bool Monitor::get_authorizer(int service_id, AuthAuthorizer** authorizer)
{
    ...
    if (service_id != CEPH_ENTITY_TYPE_MON && service_id != CEPH_ENTITY_TYPE_MGR) return false;
    ...
    if (service_id == CEPH_ENTITY_TYPE_MON) {
        ...
        // rotating key
        CryptoKey secret;
        if (!keyring.get_secret(name, secret) && !key_server.get_secret(name, secret)) {
        ...
        ret = key_server.build_session_auth_info(service_id, auth_ticket_info.ticket, secret, (uint64_t)-1, info);
        if (ret < 0) {
...
int Monitor::handle_auth_request(Connection* con, AuthConnectionMeta* auth_meta, bool more, uint32_t auth_method,
                                 const bufferlist& payload, bufferlist* reply)
{
    std::scoped_lock l(auth_lock);
    ...
    dout(10) << __func__ << " con " << con << (more ? " (more)" : " (start)") << " method " << auth_method
             << " payload " << payload.length() << dendl;
    if (!payload.length()) {
    ...
    if (auth_meta->auth_mode >= AUTH_MODE_AUTHORIZER && auth_meta->auth_mode <= AUTH_MODE_AUTHORIZER_MAX) {
        AuthAuthorizeHandler* ah = get_auth_authorize_handler(con->get_peer_type(), auth_method);
        if (!ah) {
            lderr(cct) << __func__ << " no AuthAuthorizeHandler found for auth method " << auth_method << dendl;
            return -EOPNOTSUPP;
        }
        bool was_challenge = (bool)auth_meta->authorizer_challenge;
        bool isvalid = ah->verify_authorizer(cct,
                                             keyring,
                                             payload,
    ...
    else if (auth_meta->auth_mode < AUTH_MODE_MON || auth_meta->auth_mode > AUTH_MODE_MON_MAX) {
    ...
    if (!more) {
        ...
        // handler?
        unique_ptr<AuthServiceHandler> auth_handler{get_auth_service_handler(auth_method, g_ceph_context, &key_server)};
        if (!auth_handler) {
        ...
        try {
            decode(mode, p);
            if (mode < AUTH_MODE_MON || mode > AUTH_MODE_MON_MAX) {
        ...
        // supported method?
        if (entity_name.get_type() == CEPH_ENTITY_TYPE_MON || entity_name.get_type() == CEPH_ENTITY_TYPE_OSD ||
            entity_name.get_type() == CEPH_ENTITY_TYPE_MDS || entity_name.get_type() == CEPH_ENTITY_TYPE_MGR) {
            if (!auth_cluster_required.is_supported_auth(auth_method)) {
                dout(10) << __func__ << " entity " << entity_name << " method " << auth_method
                         << " not among supported " << auth_cluster_required.get_supported_set() << dendl;
                return -EOPNOTSUPP;
            }
        }
        else {
        ...
        r = s->auth_handler->start_session(
            entity_name, con->peer_global_id, is_new_global_id, reply, &con->peer_caps_info);
    }
    else {
        priv = con->get_priv();
        if (!priv) {
        ...
        r = s->auth_handler->handle_request(p,
                                            auth_meta->get_connection_secret_length(),
                                            reply,
                                            &con->peer_caps_info,
                                            &auth_meta->session_key,
...
void Monitor::ms_handle_accept(Connection* con)
{
    ...
    if (s->item.is_on_list()) {
        ...
        dout(10) << __func__ << " con " << con << " session " << s << " registering session for "
                 << con->get_peer_addrs() << dendl;
        s->_ident(entity_name_t(con->get_peer_type(), con->get_peer_id()), con->get_peer_addrs());
...
int Monitor::ms_handle_fast_authentication(Connection* con)
{
    ...
    if (!s) {
        ...
        s = session_map.new_session(entity_name_t(con->get_peer_type(), -1),   // we don't know yet
                                    con->get_peer_addrs(),
                                    con);
        assert(s);
    ...
    dout(10) << __func__ << " session " << s << " con " << con << " addr " << s->con->get_peer_addr() << " " << *s
             << dendl;
...
void Monitor::notify_new_monmap(bool can_change_external_state, bool remove_rank_elector)
{
    if (need_set_crush_loc) {
        ...
        if (my_info_i != monmap->mon_info.end() && my_info_i->second.crush_loc == crush_loc) {
    ...
    if (remove_rank_elector) {
        ...
        for (auto i = monmap->removed_ranks.rbegin(); i != monmap->removed_ranks.rend(); ++i) {
            int remove_rank = *i;
...
void Monitor::set_elector_disallowed_leaders(bool allow_election)
{
    ...
    if (!monmap->tiebreaker_mon.empty() && monmap->contains(monmap->tiebreaker_mon)) {
...
{
    Monitor* m;
    CMonEnableStretchMode(Monitor* mon)
        : m(mon)
    {}
    void finish(int r) { m->try_engage_stretch_mode(); }
};
void Monitor::try_engage_stretch_mode()
{
    ...
    if (osdmon()->osdmap.stretch_mode_enabled && monmap->stretch_mode_enabled) {
        ...
        stretch_bucket_divider = osdmon()->osdmap.crush->get_type_name(stretch_divider_id);
...
{
    dout(20) << __func__ << dendl;
    if (!is_stretch_mode() || !is_leader()) return;
    ...
    dout(20) << "prior dead_mon_buckets: " << old_dead_buckets << "; down_mon_buckets: " << down_mon_buckets
    ...
    if (dead_mon_buckets != old_dead_buckets && dead_mon_buckets.size() >= old_dead_buckets.size()) {
...
{
    Monitor* m;
    CMonGoDegraded(Monitor* mon)
        : m(mon)
    {}
    void finish(int r) { m->maybe_go_degraded_stretch_mode(); }
};

struct CMonGoRecovery : public Context
{
    Monitor* m;
    CMonGoRecovery(Monitor* mon)
        : m(mon)
    {}
    void finish(int r) { m->go_recovery_stretch_mode(); }
};
...
void Monitor::maybe_go_degraded_stretch_mode()
{
    ...
    bool dead = osdmon()->check_for_dead_crush_zones(filtered_dead_buckets, &matched_down_buckets, &matched_down_mons);
    if (dead) {
        if (!osdmon()->is_writeable()) {
            dout(20) << "osdmon is not writeable" << dendl;
...
void Monitor::trigger_degraded_stretch_mode(const set<string>& dead_mons, const set<int>& dead_buckets)
{
    dout(20) << __func__ << dendl;
...
{
    Monitor* m;
    CMonGoHealthy(Monitor* mon)
        : m(mon)
    {}
    void finish(int r) { m->trigger_healthy_stretch_mode(); }
};
...
bool Monitor::session_stretch_allowed(MonSession* s, MonOpRequestRef& op)
{
    ...
    if (s->con->peer_is_osd()) {
        ...
        int barrier_id = [&] {
            auto type_id = osdmon()->osdmap.crush->get_validated_type_id(stretch_bucket_divider);
            ceph_assert(type_id.has_value());
            return *type_id;
        }();
        int osd_bucket_id = osdmon()->osdmap.crush->get_parent_of_type(s->con->peer_id, barrier_id);
        ...
        if (osd_bucket_id != mon_bucket_id) {
            dout(5) << "discarding session " << *s << " and sending OSD to matched zone" << dendl;
            s->con->mark_down();
            std::lock_guard l(session_map_lock);
            remove_session(s);
            if (op) {
