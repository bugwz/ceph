...
COMMAND("pg map name=pgid,type=CephPgid", "show mapping of pg to osds", "pg", "r")
COMMAND("pg repeer name=pgid,type=CephPgid", "force a PG to repeer", "osd", "rw")
COMMAND("osd last-stat-seq name=id,type=CephOsdName", "get the last pg stats sequence number reported for this osd",
        "osd", "r")
...
COMMAND("auth export name=entity,type=CephString,req=false",
        "write keyring for requested entity, or master keyring if none given", "auth", "rx")
COMMAND("auth get name=entity,type=CephString", "write keyring file with requested key", "auth", "rx")
COMMAND("auth get-key name=entity,type=CephString", "display requested key", "auth", "rx")
COMMAND("auth print-key name=entity,type=CephString", "display requested key", "auth", "rx")
COMMAND("auth print_key name=entity,type=CephString", "display requested key", "auth", "rx")
COMMAND_WITH_FLAG("auth list", "list authentication state", "auth", "rx", FLAG(DEPRECATED))
COMMAND("auth ls", "list authentication state", "auth", "rx")
COMMAND("auth import", "auth import: read keyring file from -i <file>", "auth", "rwx")
COMMAND("auth add "
        "name=entity,type=CephString "
        "name=caps,type=CephString,n=N,req=false",
...
COMMAND("auth get-or-create-key "
        "name=entity,type=CephString "
        "name=caps,type=CephString,n=N,req=false",
        "get, or add, key for <name> from system/caps pairs specified in the command.  If key already exists, any "
        "given caps must match the existing caps for that key.",
        "auth", "rwx")
COMMAND("auth get-or-create "
        "name=entity,type=CephString "
        "name=caps,type=CephString,n=N,req=false",
        "add auth info for <entity> from input file, or random key if no input given, and/or any caps specified in the "
        "command",
        "auth", "rwx")
COMMAND("auth get-or-create-pending "
        "name=entity,type=CephString",
        "generate and/or retrieve existing pending key (rotated into place on first use)", "auth", "rwx")
COMMAND("auth clear-pending "
        "name=entity,type=CephString",
        "clear pending key", "auth", "rwx")
COMMAND("auth commit-pending "
        "name=entity,type=CephString",
        "rotate pending key into active position", "auth", "rwx")
...
COMMAND("auth caps "
        "name=entity,type=CephString "
        "name=caps,type=CephString,n=N",
        "update caps for <name> from caps specified in the command", "auth", "rwx")
COMMAND_WITH_FLAG("auth del "
                  "name=entity,type=CephString",
                  "delete all caps for <name>", "auth", "rwx", FLAG(DEPRECATED))
COMMAND("auth rm "
        "name=entity,type=CephString",
        "remove all caps for <name>", "auth", "rwx")

/*
 * Monitor commands (Monitor.cc)
 */
COMMAND_WITH_FLAG("compact", "cause compaction of monitor's leveldb/rocksdb storage", "mon", "rw", FLAG(TELL))
COMMAND("fsid", "show cluster FSID/UUID", "mon", "r")
COMMAND("log name=logtext,type=CephString,n=N", "log supplied text to the monitor log", "mon", "rw")
COMMAND("log last "
        ...
        "print last few lines of the cluster log", "mon", "r")

COMMAND("status", "show cluster status", "mon", "r")
COMMAND("health name=detail,type=CephChoices,strings=detail,req=false", "show cluster health", "mon", "r")
COMMAND("health mute "
...
COMMAND("report name=tags,type=CephString,n=N,req=false", "report full status of cluster, optional title tag strings",
        "mon", "r")
COMMAND("features", "report of connected features", "mon", "r")
COMMAND("quorum_status", "report status of monitor quorum", "mon", "r")
COMMAND("mon ok-to-stop "
        "name=ids,type=CephString,n=N",
        "check whether mon(s) can be safely stopped without reducing immediate "
        "availability",
        "mon", "r")
COMMAND("mon ok-to-add-offline", "check whether adding a mon and not starting it would break quorum", "mon", "r")
COMMAND("mon ok-to-rm "
        "name=id,type=CephString",
        "check whether removing the specified mon would break quorum", "mon", "r")

COMMAND("tell "
        "name=target,type=CephName "
        "name=args,type=CephString,n=N",
        "send a command to a specific daemon", "mon", "rw")
COMMAND_WITH_FLAG("version", "show mon daemon version", "mon", "r", FLAG(TELL))
 ...
 */
COMMAND_WITH_FLAG("mon scrub", "scrub the monitor stores", "mon", "rw", FLAG(NONE))
COMMAND("mon metadata name=id,type=CephString,req=false", "fetch metadata for mon <id>", "mon", "r")
COMMAND("mon count-metadata name=property,type=CephString", "count mons by metadata field property", "mon", "r")
COMMAND("mon versions", "check running versions of monitors", "mon", "r")
COMMAND("versions", "check running versions of ceph daemons", "mon", "r")
...
COMMAND("mds metadata name=who,type=CephString,req=false", "fetch metadata for mds <role>", "mds", "r")
COMMAND("mds count-metadata name=property,type=CephString", "count MDSs by metadata field property", "mds", "r")
COMMAND("mds versions", "check running versions of MDSs", "mds", "r")
COMMAND("mds ok-to-stop name=ids,type=CephString,n=N",
        "check whether stopping the specified MDS would reduce immediate availability", "mds", "r")
...
COMMAND("mds fail name=role_or_gid,type=CephString", "Mark MDS failed: trigger a failover if a standby is available",
        "mds", "rw")
COMMAND("mds repaired name=role,type=CephString", "mark a damaged MDS rank as no longer damaged", "mds", "rw")
COMMAND("mds rm "
        "name=gid,type=CephInt,range=0",
...
COMMAND_WITH_FLAG("mds compat show", "show mds compatibility settings", "mds", "r", FLAG(DEPRECATED))
COMMAND("fs compat show "
        "name=fs_name,type=CephString ",
        "show fs compatibility settings", "mds", "r")
...
COMMAND("fs new "
        "name=fs_name,type=CephString,goodchars=" FS_NAME_GOODCHARS " name=metadata,type=CephString "
        ...
        "make new filesystem using named pools <metadata> and <data>", "fs", "rw")
COMMAND("fs fail "
        "name=fs_name,type=CephString ",
        "bring the file system down and all of its ranks", "fs", "rw")
COMMAND("fs rm "
        "name=fs_name,type=CephString "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "disable the named filesystem", "fs", "rw")
COMMAND("fs reset "
        "name=fs_name,type=CephString "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "disaster recovery only: reset to a single-MDS map", "fs", "rw")
COMMAND("fs ls ", "list filesystems", "fs", "r")
COMMAND("fs get name=fs_name,type=CephString", "get info about one filesystem", "fs", "r")
COMMAND("fs set "
        "name=fs_name,type=CephString "
...
COMMAND("fs flag set name=flag_name,type=CephChoices,strings=enable_multiple "
        "name=val,type=CephString "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "Set a global CephFS flag", "fs", "rw")

COMMAND("fs feature ls", "list available cephfs features to be set/unset", "mds", "r")

COMMAND("fs lsflags name=fs_name,type=CephString", "list the flags set on a ceph filesystem", "fs", "r")
...
COMMAND_WITH_FLAG("fs set_default name=fs_name,type=CephString", "set the default to the named filesystem", "fs", "rw",
                  FLAG(DEPRECATED))
COMMAND("fs set-default name=fs_name,type=CephString", "set the default to the named filesystem", "fs", "rw")
...
COMMAND("mon dump "
        ...
        "dump formatted monmap (optionally from epoch)", "mon", "r")
...
COMMAND_WITH_FLAG("mon remove "
                  "name=name,type=CephString",
                  "remove monitor named <name>", "mon", "rw", FLAG(DEPRECATED))
COMMAND("mon feature ls "
        "name=with_value,type=CephBool,req=false",
        "list available mon map features to be set/unset", "mon", "r")
COMMAND("mon feature set "
        "name=feature_name,type=CephString "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "set provided feature on mon map", "mon", "rw")
COMMAND("mon set-rank "
        "name=name,type=CephString "
        "name=rank,type=CephInt",
        "set the rank for the specified mon", "mon", "rw")
COMMAND("mon set-addrs "
        "name=name,type=CephString "
        "name=addrs,type=CephString",
        "set the addrs (IPs and ports) a specific monitor binds to", "mon", "rw")
COMMAND("mon set-weight "
        "name=name,type=CephString "
        "name=weight,type=CephInt,range=0|65535",
        "set the weight for the specified mon", "mon", "rw")
COMMAND("mon enable-msgr2", "enable the msgr2 protocol on port 3300", "mon", "rw")
COMMAND("mon set election_strategy "
        "name=strategy,type=CephString",
        "set the election strategy to use; choices classic, disallow, connectivity", "mon", "rw")
COMMAND("mon add disallowed_leader "
        "name=name,type=CephString",
        "prevent the named mon from being a leader", "mon", "rw")
COMMAND("mon rm disallowed_leader "
        "name=name,type=CephString",
        "allow the named mon to be a leader again", "mon", "rw")
COMMAND("mon set_location "
        "name=name,type=CephString "
        "name=args,type=CephString,n=N,goodchars=[A-Za-z0-9-_.=]",
        "specify location <args> for the monitor <name>, using CRUSH bucket names", "mon", "rw")
...
COMMAND("mon set_new_tiebreaker "
        "name=name,type=CephString "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "switch the stretch tiebreaker to be the named mon", "mon", "rw")
...
COMMAND("osd info "
        ...
        "print osd's {id} information (instead of all osds from map)", "osd", "r")
COMMAND("osd tree "
        "name=epoch,type=CephInt,range=0,req=false "
...
COMMAND("osd ls-tree "
        ...
        "show OSD ids under bucket <name> in the CRUSH map", "osd", "r")
COMMAND("osd find "
        "name=id,type=CephOsdName",
        "find osd <id> in the CRUSH map and show its location", "osd", "r")
COMMAND("osd metadata "
        "name=id,type=CephOsdName,req=false",
        "fetch metadata for osd {id} (default all)", "osd", "r")
COMMAND("osd count-metadata name=property,type=CephString", "count OSDs by metadata field property", "osd", "r")
COMMAND("osd versions", "check running versions of OSDs", "osd", "r")
COMMAND("osd numa-status", "show NUMA status of OSDs", "osd", "r")
COMMAND("osd map "
        "name=pool,type=CephPoolname "
        "name=object,type=CephObjectname "
        "name=nspace,type=CephString,req=false",
        "find pg for <object> in <pool> with [namespace]", "osd", "r")
COMMAND_WITH_FLAG("osd lspools", "list pools", "osd", "r", FLAG(DEPRECATED))
COMMAND_WITH_FLAG("osd crush rule list", "list crush rules", "osd", "r", FLAG(DEPRECATED))
...
COMMAND("osd crush rule ls-by-class "
        ...
        "list all crush rules that reference the same <class>", "osd", "r")
COMMAND("osd crush rule dump "
        "name=name,type=CephString,goodchars=[A-Za-z0-9-_.],req=false",
        "dump crush rule <name> (default all)", "osd", "r")
COMMAND("osd crush dump", "dump crush map", "osd", "r")
COMMAND("osd setcrushmap name=prior_version,type=CephInt,req=false", "set crush map from input file", "osd", "rw")
COMMAND("osd crush set name=prior_version,type=CephInt,req=false", "set crush map from input file", "osd", "rw")
...
COMMAND("osd crush rename-bucket "
        ...
        "rename bucket <srcname> to <dstname>", "osd", "rw")
COMMAND("osd crush set "
        "name=id,type=CephOsdName "
        "name=weight,type=CephFloat,range=0.0 "
        "name=args,type=CephString,n=N,goodchars=[A-Za-z0-9-_.=]",
        "update crushmap position and weight for <name> to <weight> with location <args>", "osd", "rw")
COMMAND("osd crush add "
        "name=id,type=CephOsdName "
        "name=weight,type=CephFloat,range=0.0 "
        "name=args,type=CephString,n=N,goodchars=[A-Za-z0-9-_.=]",
        "add or update crushmap position and weight for <name> with <weight> and location <args>", "osd", "rw")
COMMAND("osd crush set-all-straw-buckets-to-straw2",
        "convert all CRUSH current straw buckets to use the straw2 algorithm", "osd", "rw")
COMMAND("osd crush class create "
        "name=class,type=CephString,goodchars=[A-Za-z0-9-_]",
        "create crush device class <class>", "osd", "rw")
COMMAND("osd crush class rm "
        "name=class,type=CephString,goodchars=[A-Za-z0-9-_]",
        "remove crush device class <class>", "osd", "rw")
...
COMMAND("osd crush class rename "
        "name=srcname,type=CephString,goodchars=[A-Za-z0-9-_] "
        "name=dstname,type=CephString,goodchars=[A-Za-z0-9-_]",
        "rename crush device class <srcname> to <dstname>", "osd", "rw")
COMMAND("osd crush create-or-move "
        "name=id,type=CephOsdName "
        "name=weight,type=CephFloat,range=0.0 "
        "name=args,type=CephString,n=N,goodchars=[A-Za-z0-9-_.=]",
        "create entry or move existing entry for <name> <weight> at/to location <args>", "osd", "rw")
COMMAND("osd crush move "
        "name=name,type=CephString,goodchars=[A-Za-z0-9-_.] "
        "name=args,type=CephString,n=N,goodchars=[A-Za-z0-9-_.=]",
        "move existing entry for <name> to location <args>", "osd", "rw")
COMMAND("osd crush swap-bucket "
        ...
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "swap existing bucket contents from (orphan) bucket <source> and <target>", "osd", "rw")
COMMAND("osd crush link "
        "name=name,type=CephString "
        "name=args,type=CephString,n=N,goodchars=[A-Za-z0-9-_.=]",
        "link existing entry for <name> under location <args>", "osd", "rw")
COMMAND("osd crush rm "
        "name=name,type=CephString,goodchars=[A-Za-z0-9-_.] "
        "name=ancestor,type=CephString,req=false,goodchars=[A-Za-z0-9-_.]",
        "remove <name> from crush map (everywhere, or just at <ancestor>)", "osd", "rw")
COMMAND_WITH_FLAG("osd crush remove "
                  "name=name,type=CephString,goodchars=[A-Za-z0-9-_.] "
                  "name=ancestor,type=CephString,req=false,goodchars=[A-Za-z0-9-_.]",
                  "remove <name> from crush map (everywhere, or just at <ancestor>)", "osd", "rw", FLAG(DEPRECATED))
COMMAND("osd crush unlink "
        "name=name,type=CephString,goodchars=[A-Za-z0-9-_.] "
        "name=ancestor,type=CephString,req=false,goodchars=[A-Za-z0-9-_.]",
        "unlink <name> from crush map (everywhere, or just at <ancestor>)", "osd", "rw")
COMMAND("osd crush reweight-all", "recalculate the weights for the tree to ensure they sum correctly", "osd", "rw")
COMMAND("osd crush reweight "
        "name=name,type=CephString,goodchars=[A-Za-z0-9-_.] "
        "name=weight,type=CephFloat,range=0.0",
        "change <name>'s weight to <weight> in crush map", "osd", "rw")
COMMAND("osd crush reweight-subtree "
        "name=name,type=CephString,goodchars=[A-Za-z0-9-_.] "
        "name=weight,type=CephFloat,range=0.0",
        "change all leaf items beneath <name> to <weight> in crush map", "osd", "rw")
...
COMMAND("osd crush set-tunable "
        ...
        "set crush tunable <tunable> to <value>", "osd", "rw")
COMMAND("osd crush get-tunable "
        "name=tunable,type=CephChoices,strings=straw_calc_version",
        "get crush tunable <tunable>", "osd", "r")
COMMAND("osd crush show-tunables", "show current crush tunables", "osd", "r")
COMMAND("osd crush rule create-simple "
        ...
        "create crush rule <name> to start from <root>, replicate across buckets of type <type>, using a choose mode "
...
COMMAND("osd crush rule create-replicated "
        ...
        "create crush rule <name> for replicated pool to start from <root>, replicate across buckets of type <type>, "
        "use devices of type <class> (ssd or hdd)",
        "osd", "rw")
COMMAND("osd crush rule create-erasure "
        "name=name,type=CephString,goodchars=[A-Za-z0-9-_.] "
        "name=profile,type=CephString,req=false,goodchars=[A-Za-z0-9-_.=]",
        "create crush rule <name> for erasure coded pool created with <profile> (default default)", "osd", "rw")
...
COMMAND("osd crush rule rename "
        "name=srcname,type=CephString,goodchars=[A-Za-z0-9-_.] "
        "name=dstname,type=CephString,goodchars=[A-Za-z0-9-_.]",
        "rename crush rule <srcname> to <dstname>", "osd", "rw")
COMMAND("osd crush tree "
        "name=show_shadow,type=CephBool,req=false",
        "dump crush buckets and items in a tree view", "osd", "r")
COMMAND("osd crush ls name=node,type=CephString,goodchars=[A-Za-z0-9-_.]",
        "list items beneath a node in the CRUSH tree", "osd", "r")
COMMAND("osd crush class ls", "list all crush device classes", "osd", "r")
COMMAND("osd crush class ls-osd "
        "name=class,type=CephString,goodchars=[A-Za-z0-9-_]",
        "list all osds belonging to the specific <class>", "osd", "r")
COMMAND("osd crush get-device-class "
        "name=ids,type=CephString,n=N",
        "get classes of specified osd(s) <id> [<id>...]", "osd", "r")
COMMAND("osd crush weight-set ls", "list crush weight sets", "osd", "r")
COMMAND("osd crush weight-set dump", "dump crush weight sets", "osd", "r")
COMMAND("osd crush weight-set create-compat", "create a default backward-compatible weight-set", "osd", "rw")
COMMAND("osd crush weight-set create "
        "name=pool,type=CephPoolname "
        "name=mode,type=CephChoices,strings=flat|positional",
        "create a weight-set for a given pool", "osd", "rw")
COMMAND("osd crush weight-set rm name=pool,type=CephPoolname", "remove the weight-set for a given pool", "osd", "rw")
COMMAND("osd crush weight-set rm-compat", "remove the backward-compatible weight-set", "osd", "rw")
COMMAND("osd crush weight-set reweight "
        "name=pool,type=CephPoolname "
        "name=item,type=CephString "
        "name=weight,type=CephFloat,range=0.0,n=N",
        "set weight for an item (bucket or osd) in a pool's weight-set", "osd", "rw")
COMMAND("osd crush weight-set reweight-compat "
        "name=item,type=CephString "
        "name=weight,type=CephFloat,range=0.0,n=N",
        "set weight for an item (bucket or osd) in the backward-compatible weight-set", "osd", "rw")
...
COMMAND("osd set-full-ratio "
        ...
        "set usage ratio at which OSDs are marked full", "osd", "rw")
COMMAND("osd set-backfillfull-ratio "
        "name=ratio,type=CephFloat,range=0.0|1.0",
        "set usage ratio at which OSDs are marked too full to backfill", "osd", "rw")
COMMAND("osd set-nearfull-ratio "
        "name=ratio,type=CephFloat,range=0.0|1.0",
        "set usage ratio at which OSDs are marked near-full", "osd", "rw")
COMMAND("osd get-require-min-compat-client", "get the minimum client version we will maintain compatibility with",
        "osd", "r")
COMMAND("osd set-require-min-compat-client "
        "name=version,type=CephString "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "set the minimum client version we will maintain compatibility with", "osd", "rw")
...
COMMAND("osd erasure-code-profile set "
        ...
        "name=force,type=CephBool,req=false",
        "create erasure code profile <name> with [<key[=value]> ...] pairs. Add a --force at the end to override an "
        "existing profile (VERY DANGEROUS)",
        "osd", "rw")
COMMAND("osd erasure-code-profile get "
        "name=name,type=CephString,goodchars=[A-Za-z0-9-_.]",
        "get erasure code profile <name>", "osd", "r")
COMMAND("osd erasure-code-profile rm "
        "name=name,type=CephString,goodchars=[A-Za-z0-9-_.]",
        "remove erasure code profile <name>", "osd", "rw")
COMMAND("osd erasure-code-profile ls", "list all erasure code profiles", "osd", "r")
COMMAND("osd set "
...
COMMAND("osd require-osd-release "
        ...
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "set the minimum allowed OSD release to participate in the cluster", "osd", "rw")
COMMAND("osd down "
        "name=ids,type=CephString,n=N "
...
COMMAND("osd stop "
        ...
        "stop the corresponding osd daemons and mark them as down", "osd", "rw")
COMMAND("osd out "
        "name=ids,type=CephString,n=N",
        "set osd(s) <id> [<id>...] out, "
...
COMMAND("osd reweightn "
        ...
        "reweight osds with {<id>: <weight>,...}", "osd", "rw")
COMMAND("osd force-create-pg "
        "name=pgid,type=CephPgid "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "force creation of pg <pgid>", "osd", "rw")
COMMAND("osd pg-temp "
        "name=pgid,type=CephPgid "
        "name=id,type=CephOsdName,n=N,req=false",
        "set pg_temp mapping <pgid>:[<id> [<id>...]] (developers only)", "osd", "rw")
COMMAND("osd pg-upmap "
        "name=pgid,type=CephPgid "
        "name=id,type=CephOsdName,n=N",
        "set pg_upmap mapping <pgid>:[<id> [<id>...]] (developers only)", "osd", "rw")
COMMAND("osd rm-pg-upmap "
        "name=pgid,type=CephPgid",
        "clear pg_upmap mapping for <pgid> (developers only)", "osd", "rw")
COMMAND("osd pg-upmap-items "
        "name=pgid,type=CephPgid "
        "name=id,type=CephOsdName,n=N",
        "set pg_upmap_items mapping <pgid>:{<id> to <id>, [...]} (developers only)", "osd", "rw")
COMMAND("osd rm-pg-upmap-items "
        "name=pgid,type=CephPgid",
        "clear pg_upmap_items mapping for <pgid> (developers only)", "osd", "rw")
COMMAND("osd pg-upmap-primary "
        "name=pgid,type=CephPgid "
        "name=id,type=CephOsdName ",
        "set pg primary osd <pgid>:<id> (id (osd) must be part of pgid)", "osd", "rw")
COMMAND("osd rm-pg-upmap-primary "
        "name=pgid,type=CephPgid ",
        "clear pg primary setting for <pgid>", "osd", "rw")
COMMAND("osd primary-temp "
        "name=pgid,type=CephPgid "
        "name=id,type=CephOsdName",
        "set primary_temp mapping pgid:<id> (developers only)", "osd", "rw")
COMMAND("osd rm-primary-temp "
        "name=pgid,type=CephPgid ",
        "clear primary_temp mapping pgid (developers only)", "osd", "rw")
COMMAND("osd primary-affinity "
        "name=id,type=CephOsdName "
        "type=CephFloat,name=weight,range=0.0|1.0",
        "adjust osd primary-affinity from 0.0 <= <weight> <= 1.0", "osd", "rw")
...
COMMAND("osd lost "
        "name=id,type=CephOsdName "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "mark osd as permanently lost. THIS DESTROYS DATA IF NO MORE REPLICAS EXIST, BE CAREFUL", "osd", "rw")
COMMAND_WITH_FLAG("osd create "
                  "name=uuid,type=CephUUID,req=false "
                  "name=id,type=CephOsdName,req=false",
                  "create new osd (with optional UUID and ID)", "osd", "rw", FLAG(DEPRECATED))
COMMAND("osd new "
...
COMMAND("osd blocklist "
        ...
        "add (optionally until <expire> seconds from now) or remove <addr> from blocklist", "osd", "rw")
...
COMMAND_WITH_FLAG("osd blacklist "
                  ...
                  "name=addr,type=CephEntityAddr "
                  "name=expire,type=CephFloat,range=0.0,req=false",
                  "add (optionally until <expire> seconds from now) or remove <addr> from blacklist", "osd", "rw",
                  FLAG(DEPRECATED))
COMMAND_WITH_FLAG("osd blacklist ls", "show blacklisted clients", "osd", "r", FLAG(DEPRECATED))
COMMAND_WITH_FLAG("osd blacklist clear", "clear all blacklisted clients", "osd", "rw", FLAG(DEPRECATED))
...
COMMAND_WITH_FLAG("osd pool delete "
                  ...
                  "delete pool", "osd", "rw", FLAG(DEPRECATED))
COMMAND("osd pool rm "
        "name=pool,type=CephPoolname "
        "name=pool2,type=CephPoolname,req=false "
        "name=yes_i_really_really_mean_it,type=CephBool,req=false "
...
COMMAND("osd pool get "
        "name=pool,type=CephPoolname "
        "name=var,type=CephChoices,strings=size|min_size|pg_num|pgp_num|crush_rule|hashpspool|nodelete|nopgchange|"
        "nosizechange|write_fadvise_dontneed|noscrub|nodeep-scrub|hit_set_type|hit_set_period|hit_set_count|hit_set_"
        "fpp|use_gmt_hitset|target_max_objects|target_max_bytes|cache_target_dirty_ratio|cache_target_dirty_high_ratio|"
        "cache_target_full_ratio|cache_min_flush_age|cache_min_evict_age|erasure_code_profile|min_read_recency_for_"
        "promote|all|min_write_recency_for_promote|fast_read|hit_set_grade_decay_rate|hit_set_search_last_n|scrub_min_"
        "interval|scrub_max_interval|deep_scrub_interval|recovery_priority|recovery_op_priority|scrub_priority|"
        "compression_mode|compression_algorithm|compression_required_ratio|compression_max_blob_size|compression_min_"
        "blob_size|csum_type|csum_min_block|csum_max_block|allow_ec_overwrites|fingerprint_algorithm|pg_autoscale_mode|"
        "pg_autoscale_bias|pg_num_min|pg_num_max|target_size_bytes|target_size_ratio|dedup_tier|dedup_chunk_algorithm|"
        "dedup_cdc_chunk_size|eio|bulk",
        "get pool parameter <var>", "osd", "r")
COMMAND(
    "osd pool set "
    "name=pool,type=CephPoolname "
    "name=var,type=CephChoices,strings=size|min_size|pg_num|pgp_num|pgp_num_actual|crush_rule|hashpspool|nodelete|"
    "nopgchange|nosizechange|write_fadvise_dontneed|noscrub|nodeep-scrub|hit_set_type|hit_set_period|hit_set_count|hit_"
    "set_fpp|use_gmt_hitset|target_max_bytes|target_max_objects|cache_target_dirty_ratio|cache_target_dirty_high_ratio|"
    "cache_target_full_ratio|cache_min_flush_age|cache_min_evict_age|min_read_recency_for_promote|min_write_recency_"
    "for_promote|fast_read|hit_set_grade_decay_rate|hit_set_search_last_n|scrub_min_interval|scrub_max_interval|deep_"
    "scrub_interval|recovery_priority|recovery_op_priority|scrub_priority|compression_mode|compression_algorithm|"
    "compression_required_ratio|compression_max_blob_size|compression_min_blob_size|csum_type|csum_min_block|csum_max_"
    "block|allow_ec_overwrites|fingerprint_algorithm|pg_autoscale_mode|pg_autoscale_bias|pg_num_min|pg_num_max|target_"
    "size_bytes|target_size_ratio|dedup_tier|dedup_chunk_algorithm|dedup_cdc_chunk_size|eio|bulk "
    "name=val,type=CephString "
    "name=yes_i_really_mean_it,type=CephBool,req=false",
...
COMMAND("osd pool get-quota "
        "name=pool,type=CephPoolname ",
        "obtain object or byte limits for pool", "osd", "r")
COMMAND("osd pool application enable "
        "name=pool,type=CephPoolname "
        "name=app,type=CephString,goodchars=[A-Za-z0-9-_.] "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "enable use of an application <app> [cephfs,rbd,rgw] on pool <poolname>", "osd", "rw")
COMMAND("osd pool application disable "
        "name=pool,type=CephPoolname "
        "name=app,type=CephString "
        "name=yes_i_really_mean_it,type=CephBool,req=false",
        "disables use of an application <app> on pool <poolname>", "osd", "rw")
COMMAND("osd pool application set "
        ...
        "sets application <app> metadata key <key> to <value> on pool <poolname>", "osd", "rw")
COMMAND("osd pool application rm "
        "name=pool,type=CephPoolname "
        "name=app,type=CephString "
        "name=key,type=CephString",
        "removes application <app> metadata key <key> on pool <poolname>", "osd", "rw")
COMMAND("osd pool application get "
        ...
        "get value of key <key> of application <app> on pool <poolname>", "osd", "r")
COMMAND("osd utilization", "get basic pg distribution stats", "osd", "r")
...
COMMAND("osd tier add "
        ...
        "add the tier <tierpool> (the second one) to base pool <pool> (the first one)", "osd", "rw")
COMMAND("osd tier rm "
        "name=pool,type=CephPoolname "
        "name=tierpool,type=CephPoolname",
        "remove the tier <tierpool> (the second one) from base pool <pool> (the first one)", "osd", "rw")
...
COMMAND_WITH_FLAG("osd tier remove-overlay "
                  "name=pool,type=CephPoolname ",
                  "remove the overlay pool for base pool <pool>", "osd", "rw", FLAG(DEPRECATED))

COMMAND("osd tier add-cache "
        "name=pool,type=CephPoolname "
        "name=tierpool,type=CephPoolname "
        "name=size,type=CephInt,range=0",
        "add a cache <tierpool> (the second one) of size <size> to existing pool <pool> (the first one)", "osd", "rw")
...
COMMAND_WITH_FLAG("config-key put "
                  "name=key,type=CephString "
                  "name=val,type=CephString,req=false",
                  "put <key>, value <val>", "config-key", "rw", FLAG(DEPRECATED))
COMMAND_WITH_FLAG("config-key del "
                  "name=key,type=CephString",
                  "delete <key>", "config-key", "rw", FLAG(DEPRECATED))
...
COMMAND_WITH_FLAG("config-key list ", "list keys", "config-key", "r", FLAG(DEPRECATED))
COMMAND("config-key ls ", "list keys", "config-key", "r")
COMMAND("config-key dump "
        "name=key,type=CephString,req=false",
        "dump keys and values (with optional prefix)", "config-key", "r")


/*
 * mon/MgrMonitor.cc
 */
COMMAND("mgr stat", "dump basic info about the mgr cluster state", "mgr", "r")
COMMAND("mgr dump "
        "name=epoch,type=CephInt,range=0,req=false",
        "dump the latest MgrMap", "mgr", "r")
COMMAND("mgr fail name=who,type=CephString,req=false", "treat the named manager daemon as failed", "mgr", "rw")
COMMAND("mgr module ls", "list active mgr modules", "mgr", "r")
COMMAND("mgr services", "list service endpoints provided by mgr modules", "mgr", "r")
...
COMMAND("mgr metadata name=who,type=CephString,req=false", "dump metadata for all daemons or a specific daemon", "mgr",
        "r")
COMMAND("mgr count-metadata name=property,type=CephString", "count ceph-mgr daemons by metadata field property", "mgr",
        "r")
COMMAND("mgr versions", "check running versions of ceph-mgr daemons", "mgr", "r")
...
COMMAND("config set"
        ...
        " name=force,type=CephBool,req=false",
        "Set a configuration option for one or more entities", "config", "rw")
COMMAND("config rm"
        " name=who,type=CephString"
        " name=name,type=CephString",
        "Clear a configuration option for one or more entities", "config", "rw")
COMMAND("config get "
        "name=who,type=CephString "
        "name=key,type=CephString,req=false",
        "Show configuration option(s) for an entity", "config", "r")
COMMAND("config dump", "Show all configuration option(s)", "mon", "r")
COMMAND("config help "
        "name=key,type=CephString",
        "Describe a configuration option", "config", "r")
COMMAND("config ls", "List available configuration options", "config", "r")
COMMAND("config assimilate-conf", "Assimilate options from a conf, and return a new, minimal conf file", "config", "rw")
COMMAND("config log name=num,type=CephInt,req=false", "Show recent history of config changes", "config", "r")
COMMAND("config reset "
        "name=num,type=CephInt,range=0",
        "Revert configuration to a historical version specified by <num>", "config", "rw")
COMMAND("config generate-minimal-conf", "Generate a minimal ceph.conf file", "config", "r")
...
COMMAND_WITH_FLAG("injectargs "
                  ...
                  "inject config arguments into monitor", "mon", "rw", FLAG(TELL))
COMMAND_WITH_FLAG("smart name=devid,type=CephString,req=false", "Query health metrics for underlying device", "mon",
                  "rw", FLAG(TELL))
COMMAND_WITH_FLAG("mon_status", "report status of monitors", "mon", "r", FLAG(TELL))
COMMAND_WITH_FLAG("heap "
                  "name=heapcmd,type=CephChoices,strings=dump|start_profiler|stop_profiler|release|stats "
                  "name=value,type=CephString,req=false",
                  "show heap usage info (available only if compiled with tcmalloc)", "mon", "rw", FLAG(TELL))
COMMAND_WITH_FLAG("connection scores dump", "show the scores used in connectivity-based elections", "mon", "rwx",
                  FLAG(TELL))
COMMAND_WITH_FLAG("connection scores reset", "reset the scores used in connectivity-based elections", "mon", "rwx",
                  FLAG(TELL))
COMMAND_WITH_FLAG("sync_force "
                  "name=yes_i_really_mean_it,type=CephBool,req=false",
                  "force sync of and clear monitor store", "mon", "rw", FLAG(TELL))
...
COMMAND_WITH_FLAG("add_bootstrap_peer_hintv "
                  ...
                  "peer for cluster bringup",
                  "mon", "rw", FLAG(TELL))
COMMAND_WITH_FLAG("quorum enter ", "force monitor back into quorum", "mon", "rw", FLAG(TELL))
COMMAND_WITH_FLAG("quorum exit", "force monitor out of the quorum", "mon", "rw", FLAG(TELL))
COMMAND_WITH_FLAG("ops", "show the ops currently in flight", "mon", "r", FLAG(TELL))
COMMAND_WITH_FLAG("sessions", "list existing sessions", "mon", "r", FLAG(TELL))
COMMAND_WITH_FLAG("dump_historic_ops", "show recent ops", "mon", "r", FLAG(TELL))
COMMAND_WITH_FLAG("dump_historic_slow_ops", "show recent slow ops", "mon", "r", FLAG(TELL))
