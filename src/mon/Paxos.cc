...
static std::ostream& _prefix(std::ostream* _dout, Monitor& mon, const string& name, int rank, const string& paxos_name,
                             int state, version_t first_committed, version_t last_committed)
{
    return *_dout << "mon." << name << "@" << rank << "(" << mon.get_state_name() << ")"
                  << ".paxos(" << paxos_name << " " << Paxos::get_statename(state) << " c " << first_committed << ".."
                  << last_committed << ") ";
}
...
public:
    explicit C_Trimmed(Paxos* p)
        : paxos(p)
    {}
    void finish(int r) override { paxos->trimming = false; }
};
...
void Paxos::read_and_prepare_transactions(MonitorDBStore::TransactionRef tx, version_t first, version_t last)
{
...
void Paxos::init()
{
    ...
    dout(10) << __func__ << " last_pn: " << last_pn << " accepted_pn: " << accepted_pn
             << " last_committed: " << last_committed << " first_committed: " << first_committed << dendl;
...
void Paxos::init_logger()
{
    ...
    pcb.add_u64_counter(l_paxos_commit, "commit", "Commits", "cmt");
    ...
    pcb.add_time_avg(l_paxos_commit_latency, "commit_latency", "Commit latency", "clat");
    ...
    pcb.add_u64_avg(
        l_paxos_collect_bytes, "collect_bytes", "Data in transaction on peon collect", NULL, 0, unit_t(UNIT_BYTES));
    pcb.add_time_avg(l_paxos_collect_latency, "collect_latency", "Peon collect latency");
    pcb.add_u64_counter(
        l_paxos_collect_uncommitted, "collect_uncommitted", "Uncommitted values in started and handled collects");
    ...
    pcb.add_u64_avg(l_paxos_store_state_bytes,
                    "store_state_bytes",
    ...
    pcb.add_u64_avg(
        l_paxos_share_state_bytes, "share_state_bytes", "Data in shared state", NULL, 0, unit_t(UNIT_BYTES));
...
void Paxos::collect(version_t oldpn)
{
    ...
    if (get_store()->exists(get_name(), last_committed + 1)) {
        ...
        dout(10) << "learned uncommitted " << (last_committed + 1) << " pn " << uncommitted_pn << " ("
                 << uncommitted_value.length() << " bytes) from myself" << dendl;
    ...
    // set timeout event
    collect_timeout_event = mon.timer.add_event_after(g_conf()->mon_accept_timeout_factor * g_conf()->mon_lease,
...
void Paxos::handle_collect(MonOpRequestRef op)
{
    ...
    if (collect->first_committed > last_committed + 1) {
        dout(2) << __func__ << " leader's lowest version is too high for our last committed"
                << " (theirs: " << collect->first_committed << "; ours: " << last_committed << ") -- bootstrap!"
                << dendl;
    ...
    if (collect->pn > accepted_pn) {
        ...
        *_dout << dendl;
        ...
        // don't accept!
        dout(10) << "NOT accepting pn " << collect->pn << " from " << collect->pn_from << ", we already accepted "
                 << accepted_pn << " from " << accepted_pn_from << dendl;
    }
    last->pn = accepted_pn;
    last->pn_from = accepted_pn_from;

    // share whatever committed values we have
    if (collect->last_committed < last_committed) share_state(last, collect->first_committed, collect->last_committed);
    ...
    if (collect->last_committed <= last_committed && get_store()->exists(get_name(), last_committed + 1)) {
        ...
        dout(10) << " sharing our accepted but uncommitted value for " << last_committed + 1 << " (" << bl.length()
                 << " bytes)" << dendl;
...
void Paxos::handle_last(MonOpRequestRef op)
{
    ...
    if (last->first_committed > last_committed + 1) {
        dout(5) << __func__ << " mon." << from << " lowest version is too high for our last committed"
                << " (theirs: " << last->first_committed << "; ours: " << last_committed << ") -- bootstrap!" << dendl;
        op->mark_paxos_event("need to bootstrap");
        mon.bootstrap();
        return;
    }
    ...
    for (auto p = peer_last_committed.begin(); p != peer_last_committed.end(); ++p) {
        if (p->second + 1 < first_committed && first_committed > 1) {
            dout(5) << __func__ << " peon " << p->first << " last_committed (" << p->second
                    << ") is too low for our first_committed (" << first_committed << ") -- bootstrap!" << dendl;
            op->mark_paxos_event("need to bootstrap");
            mon.bootstrap();
            return;
        }
    ...
    if (last->pn > accepted_pn) {
        ...
        if (last->uncommitted_pn) {
            if (last->uncommitted_pn >= uncommitted_pn && last->last_committed >= last_committed &&
                last->last_committed + 1 >= uncommitted_v) {
                uncommitted_v = last->last_committed + 1;
                uncommitted_pn = last->uncommitted_pn;
                uncommitted_value = last->values[uncommitted_v];
                dout(10) << "we learned an uncommitted value for " << uncommitted_v << " pn " << uncommitted_pn << " "
                         << uncommitted_value.length() << " bytes" << dendl;
            }
            else {
                dout(10) << "ignoring uncommitted value for " << (last->last_committed + 1) << " pn "
                         << last->uncommitted_pn << " " << last->values[last->last_committed + 1].length() << " bytes"
                         << dendl;
            }
        }
    ...
    if (need_refresh) (void)do_refresh();
}
...
void Paxos::begin(bufferlist& v)
{
    ...
    ceph_assert(mon.get_quorum().size() == 1 || num_last > (unsigned)mon.monmap->size() / 2);
    ...
    *_dout << dendl;
    ...
    // set timeout event
    accept_timeout_event = mon.timer.add_event_after(g_conf()->mon_accept_timeout_factor * g_conf()->mon_lease,
                                                     new C_MonContext{&mon, [this](int r) {
                                                                          if (r == -ECANCELED) return;
                                                                          accept_timeout();
                                                                      }});
...
void Paxos::accept_timeout()
{
    ...
    ceph_assert(mon.is_leader());
    ceph_assert(is_updating() || is_updating_previous() || is_writing() || is_writing_previous());
...
void Paxos::abort_commit()
{
    ...
    --commits_started;
    if (commits_started == 0) shutdown_cond.notify_all();
}
...
void Paxos::extend_lease()
{
    ...
    if (!lease_ack_timeout_event) {
        lease_ack_timeout_event = mon.timer.add_event_after(
            g_conf()->mon_lease_ack_timeout_factor * g_conf()->mon_lease, new C_MonContext{&mon, [this](int r) {
                                                                                               if (r == -ECANCELED)
                                                                                                   return;
                                                                                               lease_ack_timeout();
                                                                                           }});
    ...
    at += ceph::make_timespan(g_conf()->mon_lease_renew_interval_factor * g_conf()->mon_lease);
    lease_renew_event = mon.timer.add_event_at(at, new C_MonContext{&mon, [this](int r) {
                                                                        if (r == -ECANCELED) return;
                                                                        lease_renew_timeout();
                                                                    }});
}
...
void Paxos::warn_on_future_time(utime_t t, entity_name_t from)
{
    ...
    if (t > now) {
        ...
        if (diff > g_conf()->mon_clock_drift_allowed) {
            ...
            if (warn_diff > pow(g_conf()->mon_clock_drift_warn_backoff, clock_drift_warned)) {
...
void Paxos::handle_lease(MonOpRequestRef op)
{
    ...
    // extend lease
    if (auto new_expire = lease->lease_timestamp.to_real_time(); lease_expire < new_expire) {
...
void Paxos::handle_lease_ack(MonOpRequestRef op)
{
    ...
    if (!lease_ack_timeout_event) {
        ...
        if (acked_lease == mon.get_quorum()) {
            ...
            dout(10) << "handle_lease_ack from " << ack->get_source() << " -- still need "
                     << mon.get_quorum().size() - acked_lease.size() << " more" << dendl;
        }
    }
    else {
...
void Paxos::reset_lease_timeout()
{
    ...
    if (lease_timeout_event) mon.timer.cancel_event(lease_timeout_event);
    lease_timeout_event = mon.timer.add_event_after(g_conf()->mon_lease_ack_timeout_factor * g_conf()->mon_lease,
                                                    new C_MonContext{&mon, [this](int r) {
                                                                         if (r == -ECANCELED) return;
                                                                         lease_timeout();
                                                                     }});
...
void Paxos::trim()
{
    ...
    version_t end = std::min(get_version() - g_conf()->paxos_min, get_first_committed() + g_conf()->paxos_trim_max);

    if (first_committed >= end) return;
...
void Paxos::shutdown()
{
    ...
    if (logger) g_ceph_context->get_perfcounters_collection()->remove(logger);
}
...
void Paxos::dispatch(MonOpRequestRef op)
{
    ...
    if (op->get_req()->get_type() != MSG_MON_PAXOS) {
        dout(0) << "Got unexpected message type " << op->get_req()->get_type() << " in Paxos::dispatch, aborting!"
                << dendl;
        ceph_abort();
    }
    ...
    // check sanity
    ceph_assert(mon.is_leader() || (mon.is_peon() && req->get_source().num() == mon.get_leader()));

    // NOTE: these ops are defined in messages/MMonPaxos.h
    switch (req->op) {
        // learner
    case MMonPaxos::OP_COLLECT: handle_collect(op); break;
    case MMonPaxos::OP_LAST: handle_last(op); break;
    case MMonPaxos::OP_BEGIN: handle_begin(op); break;
    case MMonPaxos::OP_ACCEPT: handle_accept(op); break;
    case MMonPaxos::OP_COMMIT: handle_commit(op); break;
    case MMonPaxos::OP_LEASE: handle_lease(op); break;
    case MMonPaxos::OP_LEASE_ACK: handle_lease_ack(op); break;
    default: ceph_abort();
    }
}
...
bool Paxos::is_readable(version_t v)
{
    ...
    if (v > last_committed)
        ...
        ret = (mon.is_peon() || mon.is_leader()) && (is_active() || is_updating() || is_writing()) &&
              last_committed > 0 && is_lease_valid();   // must have a value alone, or have lease
    dout(5) << __func__ << " = " << (int)ret << " - now=" << ceph_clock_now() << " lease_expire=" << lease_expire
...
{
    if (!get_store()->get(get_name(), v, bl)) return false;
    return true;
}

version_t Paxos::read_current(bufferlist& bl)
{
    if (read(last_committed, bl)) return last_committed;
    return 0;
}
...
{
    return ((mon.get_quorum().size() == 1) || (ceph::real_clock::now() < lease_expire));
}
...
{
    return mon.is_leader() && is_active() && is_lease_valid();
}
