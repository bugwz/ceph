...
{
    return *_dout << "mon." << mon.name << "@" << mon.rank << "(" << mon.get_state_name() << ").auth v" << v << " ";
}
...
void AuthMonitor::process_used_pending_keys(const std::map<EntityName, CryptoKey>& used_pending_keys)
{
    for (auto& [name, used_key] : used_pending_keys) {
        ...
        for (auto& p : pending_auth) {
            if (p.inc_type == AUTH_DATA) {
                ...
                decode(auth_inc, q);
                if (auth_inc.op == KeyServerData::AUTH_INC_ADD && auth_inc.name == name) {
        ...
        if (stringify(inc.auth.pending_key) == stringify(used_key)) {
            dout(10) << __func__ << " committing pending_key -> key for " << name << dendl;
...
void AuthMonitor::tick()
{
    ...
    if (mon.monmap->min_mon_release >= ceph_release_t::quincy) {
        ...
        if (!used_pending_keys.empty()) {
            dout(10) << __func__ << " " << used_pending_keys.size() << " used pending_keys" << dendl;
            if (mon.is_leader()) {
...
void _generate_bootstrap_keys(list<pair<EntityName, EntityAuth>>* auth_lst)
{
    ...
    map<string, map<string, bufferlist>> bootstrap = {
        ...
        {"bootstrap-osd", {{"mon", _encode_cap("allow profile bootstrap-osd")}}},
        {"bootstrap-rgw", {{"mon", _encode_cap("allow profile bootstrap-rgw")}}},
        {"bootstrap-mds", {{"mon", _encode_cap("allow profile bootstrap-mds")}}},
        {"bootstrap-mgr", {{"mon", _encode_cap("allow profile bootstrap-mgr")}}},
        {"bootstrap-rbd", {{"mon", _encode_cap("allow profile bootstrap-rbd")}}},
        {"bootstrap-rbd-mirror", {{"mon", _encode_cap("allow profile bootstrap-rbd-mirror")}}}};
...
void AuthMonitor::update_from_paxos(bool* need_bootstrap)
{
    ...
    while (version > keys_ver) {
        ...
        // clear out.
        if (keys_ver == 0) mon.key_server.clear_secrets();
        ...
        while (!p.end()) {
            ...
            decode(inc, p);
            switch (inc.inc_type) {
            case GLOBAL_ID: max_global_id = inc.max_global_id; break;
...
void AuthMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
    ...
    for (p = pending_auth.begin(); p != pending_auth.end(); ++p) p->encode(bl, mon.get_quorum_con_features());
    ...
    for (auto i = mon.key_server.secrets_begin(); i != mon.key_server.secrets_end(); ++i) {
    ...
    if (bad_detail.size()) {
        ...
        health_check_t* check = &next.add("AUTH_BAD_CAPS", HEALTH_ERR, ss.str(), bad_detail.size());
...
version_t AuthMonitor::get_trim_to() const
{
    ...
    version_t version = get_last_committed();
    if (mon.is_leader() && (version > max)) return version - max;
    return 0;
}
...
bool AuthMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    case CEPH_MSG_AUTH: return prep_auth(op, false);

    case MSG_MON_GLOBAL_ID: return false;

    case MSG_MON_USED_PENDING_KEYS: return false;
...
bool AuthMonitor::prepare_update(MonOpRequestRef op)
{
    ...
    case MSG_MON_GLOBAL_ID: return prepare_global_id(op);
    case MSG_MON_USED_PENDING_KEYS: return prepare_used_pending_keys(op);
    case CEPH_MSG_AUTH: return prep_auth(op, true);
...
bool AuthMonitor::prep_auth(MonOpRequestRef op, bool paxos_writable)
{
    ...
    if (m->protocol == 0 && !s->auth_handler) {
        ...
        if (!m->get_connection()->has_feature(CEPH_FEATURE_MSG_AUTH)) {
            if (entity_name.get_type() == CEPH_ENTITY_TYPE_MON || entity_name.get_type() == CEPH_ENTITY_TYPE_OSD ||
                ...
                if (g_conf()->cephx_cluster_require_signatures || g_conf()->cephx_require_signatures) {
                    dout(1) << m->get_source_inst() << " supports cephx but not signatures and"
                ...
                if (g_conf()->cephx_service_require_signatures || g_conf()->cephx_require_signatures) {
                    dout(1) << m->get_source_inst() << " supports cephx but not signatures and"
            ...
            if (entity_name.get_type() == CEPH_ENTITY_TYPE_MON || entity_name.get_type() == CEPH_ENTITY_TYPE_OSD ||
                entity_name.get_type() == CEPH_ENTITY_TYPE_MDS || entity_name.get_type() == CEPH_ENTITY_TYPE_MGR) {
                if (g_conf()->cephx_cluster_require_version >= 2 || g_conf()->cephx_require_version >= 2) {
                    dout(1) << m->get_source_inst() << " supports cephx but not v2 and"
                ...
                if (g_conf()->cephx_service_require_version >= 2 || g_conf()->cephx_require_version >= 2) {
                    dout(1) << m->get_source_inst() << " supports cephx but not v2 and"
    ...
    try {
        if (start) {
            // new session
            ret = s->auth_handler->start_session(
                entity_name, s->con->peer_global_id, is_new_global_id, &response_bl, &s->con->peer_caps_info);
        }
        else {
            // request
            ret = s->auth_handler->handle_request(indata,
        ...
        if (ret > 0) {
            if (!s->authenticated && mon.ms_handle_fast_authentication(s->con.get()) > 0) {
                finished = true;
            }
            ret = 0;
        }
...
reply:
    ...
    if (finished) {
        ...
        if (m->monmap_epoch < mon.monmap->get_epoch()) mon.send_latest_monmap(m->get_connection().get());
...
bool AuthMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "auth add" || prefix == "auth del" || prefix == "auth rm" || prefix == "auth get-or-create" ||
        prefix == "auth get-or-create-key" || prefix == "auth get-or-create-pending" ||
        prefix == "auth clear-pending" || prefix == "auth commit-pending" || prefix == "fs authorize" ||
        prefix == "auth import" || prefix == "auth caps") {
        return false;
    }
    ...
    else if (prefix == "auth print-key" || prefix == "auth print_key" || prefix == "auth get-key") {
        EntityAuth auth;
    ...
    else if (prefix == "auth list" || prefix == "auth ls") {
        if (f) {
...
int AuthMonitor::import_keyring(KeyRing& keyring)
{
    ...
    for (map<EntityName, EntityAuth>::iterator p = keyring.get_keys().begin(); p != keyring.get_keys().end(); ++p) {
...
int AuthMonitor::remove_entity(const EntityName& entity)
{
    ...
    if (!mon.key_server.contains(entity)) return -ENOENT;
...
bool AuthMonitor::entity_is_pending(EntityName& entity)
{
    ...
    for (auto& p : pending_auth) {
        if (p.inc_type == AUTH_DATA) {
            ...
            auto q = p.auth_data.cbegin();
            decode(inc, q);
            if (inc.op == KeyServerData::AUTH_INC_ADD && inc.name == entity) {
                return true;
            }
        }
    }
...
int AuthMonitor::exists_and_matches_entity(const auth_entity_t& entity, bool has_secret, stringstream& ss)
{
    return exists_and_matches_entity(entity.name, entity.auth, entity.auth.caps, has_secret, ss);
}

int AuthMonitor::exists_and_matches_entity(const EntityName& name, const EntityAuth& auth,
                                           const map<string, bufferlist>& caps, bool has_secret, stringstream& ss)
{

    dout(20) << __func__ << " entity " << name << " auth " << auth << " caps " << caps << " has_secret " << has_secret
             << dendl;
    ...
    if (mon.key_server.get_auth(name, existing_auth)) {
        ...
        for (auto& it : caps) {
            if (existing_auth.caps.count(it.first) == 0 || !existing_auth.caps[it.first].contents_equal(it.second)) {
                ss << "entity " << name << " exists but cap " << it.first << " does not match";
                return -EINVAL;
            }
        }
...
int AuthMonitor::add_entity(const EntityName& name, const EntityAuth& auth)
{
...
int AuthMonitor::validate_osd_destroy(int32_t id, const uuid_d& uuid, EntityName& cephx_entity,
                                      EntityName& lockbox_entity, stringstream& ss)
{
    ...
    if (!mon.key_server.contains(cephx_entity) && !mon.key_server.contains(lockbox_entity)) {
        return -ENOENT;
    }
...
int AuthMonitor::do_osd_destroy(const EntityName& cephx_entity, const EntityName& lockbox_entity)
{
    ceph_assert(paxos.is_plugged());
...
int _create_auth(EntityAuth& auth, const string& key, const map<string, bufferlist>& caps)
{
...
int AuthMonitor::validate_osd_new(int32_t id, const uuid_d& uuid, const string& cephx_secret,
                                  const string& lockbox_secret, auth_entity_t& cephx_entity,
                                  auth_entity_t& lockbox_entity, stringstream& ss)
{

    dout(10) << __func__ << " osd." << id << " uuid " << uuid << dendl;

    map<string, bufferlist> cephx_caps = {{"osd", _encode_cap("allow *")},
                                          {"mon", _encode_cap("allow profile osd")},
                                          {"mgr", _encode_cap("allow profile osd")}};
    map<string, bufferlist> lockbox_caps = {{"mon",
                                             _encode_cap("allow command \"config-key get\" "
    ...
    if (entity_is_pending(cephx_entity.name) || (has_lockbox && entity_is_pending(lockbox_entity.name))) {
...
int AuthMonitor::do_osd_new(const auth_entity_t& cephx_entity, const auth_entity_t& lockbox_entity, bool has_lockbox)
{
    ceph_assert(paxos.is_plugged());
    ...
    *_dout << dendl;
    ...
    if (has_lockbox && !mon.key_server.contains(lockbox_entity.name)) {
...
bool AuthMonitor::valid_caps(const string& type, const string& caps, ostream* out)
{
    if (type == "mon") {
        MonCap moncap;
...
{
    for (vector<string>::const_iterator p = caps.begin(); p != caps.end(); p += 2) {
        if ((p + 1) == caps.end()) {
...
bool AuthMonitor::prepare_command(MonOpRequestRef op)
{
    ...
    if (prefix == "auth import") {
        ...
        for (vector<string>::iterator it = caps_vec.begin(); it != caps_vec.end() && (it + 1) != caps_vec.end();
             it += 2) {
            string sys = *it;
            bufferlist cap;
            encode(*(it + 1), cap);
        ...
        err = exists_and_matches_entity(auth_inc.name, new_inc, new_caps, has_keyring, ss);
    ...
    else if ((prefix == "auth get-or-create-pending" || prefix == "auth clear-pending" ||
        ...
        for (auto& p : pending_auth) {
            if (p.inc_type == AUTH_DATA) {
                ...
                auto q = p.auth_data.cbegin();
                decode(auth_inc, q);
                if (auth_inc.op == KeyServerData::AUTH_INC_ADD && auth_inc.name == entity) {
                    wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, 0, rs, get_last_committed() + 1));
                    return true;
                }
            }
    ...
    else if ((prefix == "auth get-or-create-key" || prefix == "auth get-or-create") && !entity_name.empty()) {
        ...
        for (vector<string>::const_iterator it = caps_vec.begin(); it != caps_vec.end() && (it + 1) != caps_vec.end();
             it += 2) {
        ...
        if (mon.key_server.get_auth(entity, entity_auth)) {
            for (const auto& sys_cap : wanted_caps) {
                if (entity_auth.caps.count(sys_cap.first) == 0 ||
                    ...
                    ss << "key for " << entity << " exists but cap " << sys_cap.first << " does not match";
                    err = -EINVAL;
                    goto done;
                }
            }
        ...
        // ...or are we about to?
        for (vector<Incremental>::iterator p = pending_auth.begin(); p != pending_auth.end(); ++p) {
            if (p->inc_type == AUTH_DATA) {
                KeyServerData::Incremental auth_inc;
                auto q = p->auth_data.cbegin();
                decode(auth_inc, q);
        ...
        for (auto it = caps_vec.begin(); it != caps_vec.end() && (it + 1) != caps_vec.end(); it += 2) {
            ...
            if (cap.compare(0, 2, "rw") == 0) osd_cap_wanted = "rw";
        ...
        osd_cap_string +=
            "allow " + osd_cap_wanted + " tag " + pg_pool_t::APPLICATION_NAME_CEPHFS + " data=" + filesystem;

        std::map<string, bufferlist> wanted_caps = {{"mon", _encode_cap(mon_cap_string)},
                                                    {"osd", _encode_cap(osd_cap_string)},
                                                    {"mds", _encode_cap(mds_cap_string)}};

        if (!valid_caps("mon", mon_cap_string, &ss) || !valid_caps("osd", osd_cap_string, &ss) ||
            ...
            for (const auto& sys_cap : wanted_caps) {
                if (entity_auth.caps.count(sys_cap.first) == 0 ||
                    ...
                    ss << entity << " already has fs capabilities that differ from "
                       << "those supplied. To generate a new auth key for " << entity << ", first remove " << entity
        ...
        for (vector<string>::iterator it = caps_vec.begin(); it != caps_vec.end(); it += 2)
    ...
    else if ((prefix == "auth del" || prefix == "auth rm") && !entity_name.empty()) {
        KeyServerData::Incremental auth_inc;
        auth_inc.name = entity;
...
bool AuthMonitor::_upgrade_format_to_dumpling()
{
    ...
    for (p = mon.key_server.secrets_begin(); p != mon.key_server.secrets_end(); ++p) {
        // grab mon caps, if any
        string mon_caps;
        if (p->second.caps.count("mon") == 0) continue;
        try {
        ...
        // set daemon profiles
        if ((p->first.is_osd() || p->first.is_mds()) && mon_caps == "allow rwx") {
        ...
        if (new_caps.length() > 0) {
            dout(5) << __func__ << " updating " << p->first << " mon cap from " << mon_caps << " to " << new_caps
                    << dendl;
...
bool AuthMonitor::_upgrade_format_to_luminous()
{
    ...
    for (p = mon.key_server.secrets_begin(); p != mon.key_server.secrets_end(); ++p) {
        ...
        else if (n.find("osd.") == 0 || n.find("mds.") == 0 || n.find("mon.") == 0) {
        ...
        if (n.find("mgr.") == 0 && p->second.caps.count("mon")) {
...
void AuthMonitor::upgrade_format()
{
    ...
    if (!mon.get_quorum_mon_features().contains_all(ceph::features::mon::FEATURE_LUMINOUS)) {
        // pre-luminous quorum
        current = FORMAT_DUMPLING;
    }
    else if (!mon.get_quorum_mon_features().contains_all(ceph::features::mon::FEATURE_MIMIC)) {
        // pre-mimic quorum
