...
struct FeatureMap
{
    ...
    void add_mon(uint64_t features) { m[CEPH_ENTITY_TYPE_MON][features]++; }
    ...
    void dump(ceph::Formatter* f) const
    {
        for (auto& p : m) {
            ...
            for (auto& q : p.second) {
                ...
                f->dump_string("release", ceph_release_name(ceph_release_from_features(q.first)));
...
struct DataStats
{
    ...
    void decode(ceph::buffer::list::const_iterator& p)
    {
        ...
        decode(last_update, p);
        if (struct_v > 1) decode(store_stats, p);
...
struct ScrubResult
{
    ...
    void dump(ceph::Formatter* f) const
    {
        ...
        for (auto p = prefix_crc.begin(); p != prefix_crc.end(); ++p) f->dump_unsigned(p->first.c_str(), p->second);
        f->close_section();
        f->open_object_section("keys");
        for (auto p = prefix_keys.begin(); p != prefix_keys.end(); ++p) f->dump_unsigned(p->first.c_str(), p->second);
        f->close_section();
    }
...
public:
     ...
     *    accordingly. So should consumers of this interface.
     */
    uint64_t get_raw() const { return features; }

    constexpr friend mon_feature_t operator&(const mon_feature_t a, const mon_feature_t b)
    {
    ...
    bool operator==(const mon_feature_t other) const { return (features == other.features); }

    bool operator!=(const mon_feature_t other) const { return (features != other.features); }

    bool empty() const { return features == 0; }
     ...
     */
    mon_feature_t intersection(const mon_feature_t other) const { return mon_feature_t((features & other.features)); }
    ...
    void set_feature(const mon_feature_t f) { features |= f.features; }

    void unset_feature(const mon_feature_t f) { features &= ~(f.features); }
...
{
    return (FEATURE_KRAKEN | FEATURE_LUMINOUS | FEATURE_MIMIC | FEATURE_OSDMAP_PRUNE | FEATURE_NAUTILUS |
            FEATURE_OCTOPUS | FEATURE_PACIFIC | FEATURE_PINGING | FEATURE_QUINCY | FEATURE_REEF | FEATURE_NONE);
}
/**
...
{
    return (FEATURE_KRAKEN | FEATURE_LUMINOUS | FEATURE_MIMIC | FEATURE_NAUTILUS | FEATURE_OSDMAP_PRUNE |
            FEATURE_OCTOPUS | FEATURE_PACIFIC | FEATURE_PINGING | FEATURE_QUINCY | FEATURE_REEF | FEATURE_NONE);
}
