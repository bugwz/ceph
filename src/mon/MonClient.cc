...
MonClient::MonClient(CephContext* cct_, boost::asio::io_context& service)
    ...
    , reopen_interval_multiplier(cct_->_conf.get_val<double>("mon_client_hunt_interval_min_multiple"))
...
int MonClient::get_monmap_and_config()
{
    ...
    cct->init_crypto();
    auto shutdown_crypto = make_scope_guard([this] { cct->shutdown_crypto(); });
    ...
    while (tries-- > 0) {
        ...
        r = authenticate(cct->_conf.get_val<std::chrono::seconds>("client_mount_timeout").count());
        if (r < 0) {
            break;
        }
        {
            std::unique_lock l(monc_lock);
            if (monmap.get_epoch() && !monmap.persistent_features.contains_all(ceph::features::mon::FEATURE_MIMIC)) {
            ...
            while ((!bootstrap_config || monmap.get_epoch() == 0) && r == 0) {
                ...
                auto status = map_cond.wait_for(l, ceph::make_timespan(cct->_conf->mon_client_hunt_interval));
...
int MonClient::ping_monitor(const string& mon_id, string* result_reply)
{
    ...
    MonClientPinger* pinger = new MonClientPinger(cct, &rkeyring, result_reply);
...
bool MonClient::ms_dispatch(Message* m)
{
    ...
    case MSG_LOGACK:
    case MSG_CONFIG: break;
    case CEPH_MSG_PING: m->put(); return true;
    default: return false;
    }

    std::lock_guard lock(monc_lock);

    if (!m->get_connection()->is_anon() && m->get_source().type() == CEPH_ENTITY_TYPE_MON) {
        if (_hunting()) {
    ...
    case CEPH_MSG_MON_MAP:
        ...
        break;
    case CEPH_MSG_AUTH_REPLY: handle_auth(static_cast<MAuthReply*>(m)); break;
    case CEPH_MSG_MON_SUBSCRIBE_ACK: handle_subscribe_ack(static_cast<MMonSubscribeAck*>(m)); break;
    case CEPH_MSG_MON_GET_VERSION_REPLY: handle_get_version_reply(static_cast<MMonGetVersionReply*>(m)); break;
    case MSG_MON_COMMAND_ACK: handle_mon_command_ack(static_cast<MMonCommandAck*>(m)); break;
    case MSG_COMMAND_REPLY:
        if (m->get_connection()->is_anon() && m->get_source().type() == CEPH_ENTITY_TYPE_MON) {
    ...
    case MSG_LOGACK:
        ...
        break;
    case MSG_CONFIG: handle_config(static_cast<MConfig*>(m)); break;
    }
    return true;
}
...
void MonClient::send_log(bool flush)
{
    if (log_client) {
        ...
        if (lm) _send_mon_message(std::move(lm));
...
void MonClient::handle_monmap(MMonMap* m)
{
    ...
    ldout(cct, 10) << " got monmap " << monmap.epoch << " from mon." << old_name << " (according to old e"
    ...
    *_dout << dendl;
    ...
    if (old_name.size() == 0) {
        ...
        else if (messenger->should_use_msgr2() && monmap.get_addrs(new_name).has_msgr2() && !con_addrs.has_msgr2()) {
            ldout(cct, 1) << " mon." << new_name << " has (v2) addrs " << monmap.get_addrs(new_name)
                          << " but i'm connected to " << con_addrs << ", reconnecting" << dendl;
            _reopen_session();
        }
    }
...
void MonClient::handle_config(MConfig* m)
{
    ...
    boost::asio::post(finish_strand,
                      [m,
                       cct = boost::intrusive_ptr<CephContext>(cct),
...
int MonClient::init()
{
    ...
    if (auth_registry.is_supported_method(messenger->get_mytype(), CEPH_AUTH_CEPHX)) {
    ...
    rotating_secrets.reset(new RotatingKeyRing(cct, cct->get_module_type(), keyring.get()));
    ...
    cct->get_admin_socket()->register_command("rotate-key", this, "rotate live authentication key");
...
void MonClient::shutdown()
{
    ...
    while (!version_requests.empty()) {
        ceph::async::post(std::move(version_requests.begin()->second), monc_errc::shutting_down, 0, 0);
        ldout(cct, 20) << __func__ << " canceling and discarding version request " << version_requests.begin()->first
                       << dendl;
...
int MonClient::authenticate(double timeout)
{
    ...
    if (timeout > 0.0) ldout(cct, 10) << "authenticate will time out at " << until << dendl;
...
int MonClient::call(std::string_view command, const cmdmap_t& cmdmap, const ceph::buffer::list& inbl,
...
void MonClient::handle_auth(MAuthReply* m)
{
    ...
    if (m->get_connection()->is_anon()) {
        ...
        for (auto& p : mon_commands) {
            if (p.second->target_con == m->get_connection()) {
                ...
                int ret = mc->handle_auth(m, entity_name, CEPH_ENTITY_TYPE_MON, rotating_secrets.get());
    ...
    if (!_hunting()) {
        ...
        if (global_id != active_con->get_global_id()) {
            lderr(cct) << __func__ << " peer assigned me a different global_id: " << active_con->get_global_id()
                       << dendl;
        }
        if (ret != -EAGAIN) {
            _finish_auth(ret);
    ...
    int auth_err = found->second.handle_auth(m, entity_name, want_keys, rotating_secrets.get());
    m->put();
    if (auth_err == -EAGAIN) {
        return;
    }
...
void MonClient::_finish_auth(int auth_err)
{
    ...
    if (!auth_err && active_con) {
        ...
        ldout(cct, 10) << __func__ << " auth returned EAGAIN, reopening the session to try again" << dendl;
        _reopen_session();
    }
...
void MonClient::_send_mon_message(MessageRef m)
{
    ...
    if (active_con) {
        ...
        ldout(cct, 10) << "_send_mon_message to mon." << monmap.get_name(cur_con->get_peer_addr()) << " at "
...
void MonClient::_reopen_session(int rank)
{
    ...
    while (!version_requests.empty()) {
        ceph::async::post(std::move(version_requests.begin()->second), monc_errc::session_reset, 0, 0);
        version_requests.erase(version_requests.begin());
    }
...
void MonClient::_add_conns()
{
    ...
    auto get_next_batch = [this]() -> std::vector<unsigned> {
        ...
        auto cands = boost::make_iterator_range(ranks_by_priority.equal_range(ranks_by_priority.begin()->first));
        std::vector<unsigned> ranks;
        boost::range::copy(cands | boost::adaptors::map_values, std::back_inserter(ranks));
        return ranks;
    };
    ...
    if (ranks.size() > 1) {
        ...
        if (std::accumulate(begin(weights), end(weights), 0u) == 0) {
            ...
            weighted_shuffle(begin(ranks), end(ranks), begin(weights), end(weights), std::mt19937{rd()});
        }
    }
...
void MonClient::_start_hunting()
{
    ...
    if (!had_a_connection) return;
    reopen_interval_multiplier *= cct->_conf->mon_client_hunt_interval_backoff;
    if (reopen_interval_multiplier > cct->_conf->mon_client_hunt_interval_max_multiple) {
        reopen_interval_multiplier = cct->_conf->mon_client_hunt_interval_max_multiple;
    }
}
    ...
    if (!auth_err) {
        ...
        if (active_con) {
            ...
            if (global_id && global_id != active_con->get_global_id()) {
                lderr(cct) << __func__ << " global_id changed from " << global_id << " to "
...
void MonClient::tick()
{
    ...
    auto reschedule_tick = make_scope_guard([this] { schedule_tick(); });
    ...
    if (_hunting()) {
        ...
        if (now > last_keepalive + cct->_conf->mon_client_ping_interval) {
            ...
            if (cct->_conf->mon_client_ping_timeout > 0 && cur_con->has_feature(CEPH_FEATURE_MSGR_KEEPALIVE2)) {
                utime_t lk = cur_con->get_last_keepalive_ack();
                utime_t interval = now - lk;
                if (interval > cct->_conf->mon_client_ping_timeout) {
                    ldout(cct, 1) << "no keepalive since " << lk << " (" << interval << " seconds), reconnecting"
                                  << dendl;
                    return _reopen_session();
                }
            }
...
void MonClient::_un_backoff()
{
    ...
    reopen_interval_multiplier =
        std::max(cct->_conf.get_val<double>("mon_client_hunt_interval_min_multiple"),
...
{
    ceph_assert(ceph_mutex_is_locked(monc_lock));
    if (!rotating_secrets || !auth_principal_needs_rotating_keys(entity_name)) {
    ...
    if (!rotating_secrets->need_new_secrets() && rotating_secrets->need_new_secrets(issued_at_lower_bound)) {
    ...
    if ((now > last_rotating_renew_sent) && double(now - last_rotating_renew_sent) < 1) {
        ldout(cct, 10) << __func__ << " called too often (last: " << last_rotating_renew_sent << "), skipping refresh"
                       << dendl;
        return 0;
    }
    auto m = ceph::make_message<MAuth>();
...
int MonClient::wait_auth_rotating(double timeout)
{
    ...
    if (auth->get_protocol() == CEPH_AUTH_NONE) return 0;
    ...
    if (auth_cond.wait_for(l, ceph::make_timespan(timeout), [this, cutoff] {
            return (!auth_principal_needs_rotating_keys(entity_name) || !rotating_secrets->need_new_secrets(cutoff));
        })) {
        ldout(cct, 10) << __func__ << " done" << dendl;
        return 0;
    }
...
void MonClient::_send_command(MonCommand* r)
{
    if (r->is_tell()) {
        ...
        if (r->target_rank >= 0 && r->target_rank != monmap.get_rank(peer)) {
            ldout(cct, 10) << __func__ << " " << r->tid << " " << r->cmd << " wants rank " << r->target_rank
        ...
        if (r->target_name.length() && r->target_name != monmap.get_name(peer)) {
            ldout(cct, 10) << __func__ << " " << r->tid << " " << r->cmd << " wants mon " << r->target_name
                           << ", reopening session" << dendl;
            if (!monmap.contains(r->target_name)) {
                ldout(cct, 10) << " target " << r->target_name << " not present in monmap" << dendl;
                _finish_command(r, monc_errc::mon_dne, "mon dne"sv, {});
...
void MonClient::_check_tell_commands()
{
    ...
    while (p != mon_commands.end()) {
        auto cmd = p->second;
        ++p;
        if (cmd->is_tell() && cmd->last_send_attempt != utime_t() &&
...
void MonClient::handle_mon_command_ack(MMonCommandAck* ack)
{
    ...
    ldout(cct, 10) << __func__ << " " << r->tid << " " << r->cmd << dendl;
    auto ec = ack->r < 0 ? bs::error_code(-ack->r, mon_category()) : bs::error_code();
    _finish_command(r, ec, ack->rs, std::move(ack->get_data()));
    ack->put();
}
...
void MonClient::handle_command_reply(MCommandReply* reply)
{
    ...
    ldout(cct, 10) << __func__ << " " << r->tid << " " << r->cmd << dendl;
    auto ec = reply->r < 0 ? bs::error_code(-reply->r, mon_category()) : bs::error_code();
...
void MonClient::_finish_command(MonCommand* r, bs::error_code ret, std::string_view rs, ceph::buffer::list&& bl)
{
    ldout(cct, 10) << __func__ << " " << r->tid << " = " << ret << " " << rs << dendl;
    ceph::async::post(std::move(r->onfinish), ret, std::string(rs), std::move(bl));
    if (r->target_con) {
        r->target_con->mark_down();
    }
...
void MonClient::handle_get_version_reply(MMonGetVersionReply* m)
{
    ...
    if (iter == version_requests.end()) {
        ...
        ceph::async::post(std::move(req), bs::error_code(), m->version, m->oldest_version);
    }
    m->put();
}

int MonClient::get_auth_request(Connection* con, AuthConnectionMeta* auth_meta, uint32_t* auth_method,
                                std::vector<uint32_t>* preferred_modes, ceph::buffer::list* bl)
{
    std::lock_guard l(monc_lock);
            ...
            for (auto& i : mon_commands) {
                if (i.second->target_con == con) {
                    return i.second->target_session->get_auth_request(
                        auth_method, preferred_modes, bl, entity_name, want_keys, rotating_secrets.get());
                }
            }
        }
    ...
    if (!auth_meta->authorizer) {
        lderr(cct) << __func__ << " failed to build_authorizer for type " << ceph_entity_type_name(con->get_peer_type())
                   << dendl;
        return -EACCES;
    }
    auth_meta->auth_method = auth_meta->authorizer->protocol;
    auth_registry.get_supported_modes(con->get_peer_type(), auth_meta->auth_method, preferred_modes);
    *bl = auth_meta->authorizer->bl;
    return 0;
}

int MonClient::handle_auth_reply_more(Connection* con, AuthConnectionMeta* auth_meta, const ceph::buffer::list& bl,
                                      ceph::buffer::list* reply)
{
    ...
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_MON) {
        if (con->is_anon()) {
            for (auto& i : mon_commands) {
                if (i.second->target_con == con) {
                    return i.second->target_session->handle_auth_reply_more(auth_meta, bl, reply);
                }
            }
        }
        for (auto& i : pending_cons) {
...
int MonClient::handle_auth_done(Connection* con, AuthConnectionMeta* auth_meta, uint64_t global_id, uint32_t con_mode,
                                const ceph::buffer::list& bl, CryptoKey* session_key, std::string* connection_secret)
{
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_MON) {
        std::lock_guard l(monc_lock);
        if (con->is_anon()) {
            for (auto& i : mon_commands) {
                if (i.second->target_con == con) {
                    return i.second->target_session->handle_auth_done(
                        auth_meta, global_id, bl, session_key, connection_secret);
                }
            }
        }
        for (auto& i : pending_cons) {
            if (i.second.is_con(con)) {
                int r = i.second.handle_auth_done(auth_meta, global_id, bl, session_key, connection_secret);
                if (r) {
...
int MonClient::handle_auth_bad_method(Connection* con, AuthConnectionMeta* auth_meta, uint32_t old_auth_method,
                                      ...
                                      const std::vector<uint32_t>& allowed_modes)
{
    ...
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_MON) {
            ...
            for (auto& i : mon_commands) {
                if (i.second->target_con == con) {
                    int r = i.second->target_session->handle_auth_bad_method(
                        old_auth_method, result, allowed_methods, allowed_modes);
                    if (r < 0) {
        ...
        for (auto& i : pending_cons) {
            if (i.second.is_con(con)) {
                int r = i.second.handle_auth_bad_method(old_auth_method, result, allowed_methods, allowed_modes);
                if (r == 0) {
        ...
        // huh...
        ldout(cct, 10) << __func__ << " hmm, they didn't like " << old_auth_method << " result " << cpp_strerror(result)
                       << " and auth is " << (auth ? auth->get_protocol() : 0) << dendl;
        return -EACCES;
    }
}

int MonClient::handle_auth_request(Connection* con, AuthConnectionMeta* auth_meta, bool more, uint32_t auth_method,
                                   const ceph::buffer::list& payload, ceph::buffer::list* reply)
{
    if (payload.length() == 0) {
...
AuthAuthorizer* MonClient::build_authorizer(int service_id) const
{
    ...
    if (auth) {
        ...
        ldout(cct, 0) << __func__ << " for " << ceph_entity_type_name(service_id) << ", but no auth is available now"
                      << dendl;
        return nullptr;
    }
}
...
MonConnection::MonConnection(CephContext* cct, ConnectionRef con, uint64_t global_id, AuthRegistry* ar)
    : cct(cct)
    , con(con)
    , global_id(global_id)
    , auth_registry(ar)
...
void MonConnection::start(epoch_t epoch, const EntityName& entity_name)
{
    using ceph::encode;
...
int MonConnection::get_auth_request(uint32_t* method, std::vector<uint32_t>* preferred_modes, ceph::buffer::list* bl,
                                    const EntityName& entity_name, uint32_t want_keys, RotatingKeyRing* keyring)
{
    using ceph::encode;
    // choose method
    if (auth_method < 0) {
    ...
    *method = auth_method;
    auth_registry->get_supported_modes(con->get_peer_type(), auth_method, preferred_modes);
...
int MonConnection::handle_auth_reply_more(AuthConnectionMeta* auth_meta, const ceph::buffer::list& bl,
                                          ceph::buffer::list* reply)
{
    ...
    *_dout << dendl;
    ...
    int r = auth->handle_response(0, p, &auth_meta->session_key, &auth_meta->connection_secret);
    if (r == -EAGAIN) {
        auth->prepare_build_request();
...
int MonConnection::handle_auth_done(AuthConnectionMeta* auth_meta, uint64_t new_global_id, const ceph::buffer::list& bl,
                                    CryptoKey* session_key, std::string* connection_secret)
{
    ldout(cct, 10) << __func__ << " global_id " << new_global_id << " payload " << bl.length() << dendl;
    global_id = new_global_id;
    auth->set_global_id(global_id);
    auto p = bl.begin();
    int auth_err = auth->handle_response(0, p, &auth_meta->session_key, &auth_meta->connection_secret);
    if (auth_err >= 0) {
...
int MonConnection::handle_auth_bad_method(uint32_t old_auth_method, int result,
                                          const std::vector<uint32_t>& allowed_methods,
                                          const std::vector<uint32_t>& allowed_modes)
{
    ldout(cct, 10) << __func__ << " old_auth_method " << old_auth_method << " result " << cpp_strerror(result)
                   << " allowed_methods " << allowed_methods << dendl;
    std::vector<uint32_t> auth_supported;
    auth_registry->get_supported_methods(con->get_peer_type(), &auth_supported);
    auto p = std::find(auth_supported.begin(), auth_supported.end(), old_auth_method);
    assert(p != auth_supported.end());
    p = std::find_first_of(std::next(p), auth_supported.end(), allowed_methods.begin(), allowed_methods.end());
    if (p == auth_supported.end()) {
        lderr(cct) << __func__ << " server allowed_methods " << allowed_methods << " but i only support "
                   << auth_supported << dendl;
        return -EACCES;
    }
    auth_method = *p;
...
int MonConnection::_negotiate(MAuthReply* m, const EntityName& entity_name, uint32_t want_keys,
                              RotatingKeyRing* keyring)
{
    ldout(cct, 10) << __func__ << dendl;
...
int MonConnection::_init_auth(uint32_t method, const EntityName& entity_name, uint32_t want_keys,
                              RotatingKeyRing* keyring, bool msgr2)
{
    ...
    if (!msgr2 && (want_keys & CEPH_ENTITY_TYPE_MGR) && !(con->has_features(CEPH_FEATUREMASK_SERVER_KRAKEN))) {
...
const char* monc_error_category::message(int ev, char*, std::size_t) const noexcept
{
    ...
    case monc_errc::session_reset: return "Monitor session was reset";
    case monc_errc::rank_dne: return "Requested monitor rank does not exist";
    case monc_errc::mon_dne: return "Requested monitor does not exist";
    case monc_errc::timed_out: return "Monitor operation timed out";
    case monc_errc::mon_unavailable: return "Monitor unavailable";
    }
...
{
    switch (static_cast<monc_errc>(ev)) {
    case monc_errc::shutting_down: return bs::errc::operation_canceled;
    case monc_errc::session_reset: return bs::errc::resource_unavailable_try_again;
    case monc_errc::rank_dne: [[fallthrough]];
    case monc_errc::mon_dne: return ceph::errc::not_in_map;
    case monc_errc::timed_out: return bs::errc::timed_out;
    case monc_errc::mon_unavailable: return bs::errc::no_such_device;
    }
    return {ev, *this};
}
...
bool monc_error_category::equivalent(int ev, const bs::error_condition& c) const noexcept
{
    switch (static_cast<monc_errc>(ev)) {
    case monc_errc::rank_dne: [[fallthrough]];
    case monc_errc::mon_dne: return c == bs::errc::no_such_file_or_directory;
    default: return default_error_condition(ev) == c;
    }
}
...
int monc_error_category::from_code(int ev) const noexcept
{
    ...
    switch (static_cast<monc_errc>(ev)) {
    case monc_errc::shutting_down: return -ECANCELED;
    case monc_errc::session_reset: return -EAGAIN;
    case monc_errc::rank_dne: [[fallthrough]];
    case monc_errc::mon_dne: return -ENOENT;
    case monc_errc::timed_out: return -ETIMEDOUT;
    case monc_errc::mon_unavailable: return -ENXIO;
    }
    return -EDOM;
}
