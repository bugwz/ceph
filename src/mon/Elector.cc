...
{
    return *_dout << "mon." << mon->name << "@" << mon->rank << "(" << mon->get_state_name() << ").elector(" << epoch
                  << ") ";
}

Elector::Elector(Monitor* m, int strategy)
    : logic(this, static_cast<ElectionLogic::election_strategy>(strategy), &peer_tracker,
            m->cct->_conf.get_val<double>("mon_elector_ignore_propose_margin"), m->cct)
    , peer_tracker(this, m->rank, m->cct->_conf.get_val<uint64_t>("mon_con_tracker_score_halflife"),
...
void Elector::propose_to_peers(epoch_t e, bufferlist& logic_bl)
{
    ...
    for (unsigned i = 0; i < mon->monmap->size(); ++i) {
        ...
        MMonElection* m =
            new MMonElection(MMonElection::OP_PROPOSE, e, peer_tracker.get_encoded_bl(), logic.strategy, mon->monmap);
...
{
    MMonElection* m =
        new MMonElection(MMonElection::OP_ACK, get_epoch(), peer_tracker.get_encoded_bl(), logic.strategy, mon->monmap);
    m->mon_features = ceph::features::mon::get_supported();
    m->mon_release = ceph_release();
...
void Elector::reset_timer(double plus)
{
     ...
     * Leader.
     */
    expire_event = mon->timer.add_event_after(g_conf()->mon_election_timeout + plus,
                                              new C_MonContext{mon, [this](int) { logic.end_election_period(); }});
}
...
void Elector::message_victory(const std::set<int>& quorum)
{
    ...
    for (auto id : quorum) {
        ...
        if (min_mon_release == ceph_release_t::unknown || info.mon_release < min_mon_release) {
    ...
    for (set<int>::iterator p = quorum.begin(); p != quorum.end(); ++p) {
        ...
        MMonElection* m = new MMonElection(
            MMonElection::OP_VICTORY, get_epoch(), peer_tracker.get_encoded_bl(), logic.strategy, mon->monmap);
        m->quorum = quorum;
    ...
    // tell monitor
    mon->win_election(get_epoch(), quorum, cluster_features, mon_features, min_mon_release, metadata);
}
...
void Elector::handle_propose(MonOpRequestRef op)
{
    ...
    dout(10) << __func__ << " required features " << required_features << " " << required_mon_features
             << ", peer features " << m->get_connection()->get_features() << " " << m->mon_features << dendl;
    ...
    if ((required_features ^ m->get_connection()->get_features()) & required_features) {
        ...
        dout(5) << " ignoring propose from mon" << from << " release " << (int)m->mon_release << " < min_mon_release "
                << (int)mon->monmap->min_mon_release << dendl;
        nak_old_peer(op);
        return;
    }
...
void Elector::handle_ack(MonOpRequestRef op)
{
    ...
    if (logic.electing_me) {
        ...
        for (auto q = logic.acked_me.begin(); q != logic.acked_me.end(); ++q) {
            auto p = peer_info.find(*q);
            ceph_assert(p != peer_info.end());
            if (q != logic.acked_me.begin()) *_dout << ",";
            *_dout << " mon." << p->first << ":"
                   << " features " << p->second.cluster_features << " " << p->second.mon_features;
        }
        *_dout << " }" << dendl;
    }
...
void Elector::handle_victory(MonOpRequestRef op)
{
    ...
    mon->lose_election(get_epoch(), m->quorum, from, m->quorum_features, m->mon_features, m->mon_release);
...
void Elector::nak_old_peer(MonOpRequestRef op)
{
    ...
    uint64_t required_features = mon->get_required_features();
    mon_feature_t required_mon_features = mon->get_required_mon_features();
    dout(10) << "sending nak to peer " << m->get_source() << " supports " << supported_features << " "
             << m->mon_features << ", required " << required_features << " " << required_mon_features << ", release "
             << (int)m->mon_release << " vs required " << (int)mon->monmap->min_mon_release << dendl;
    MMonElection* reply =
        new MMonElection(MMonElection::OP_NAK, m->epoch, peer_tracker.get_encoded_bl(), logic.strategy, mon->monmap);
...
void Elector::handle_nak(MonOpRequestRef op)
{
    ...
    auto m = op->get_req<MMonElection>();
    dout(1) << "handle_nak from " << m->get_source() << " quorum_features " << m->quorum_features << " "
            << m->mon_features << " min_mon_release " << (int)m->mon_release << dendl;

    if (m->mon_release > ceph_release()) {
        derr << "Shutting down because I am release " << (int)ceph_release() << " < min_mon_release "
             << (int)m->mon_release << dendl;
    }
    else {
        CompatSet other;
        auto bi = m->sharing_bl.cbegin();
...
void Elector::begin_peer_ping(int peer)
{
    ...
    mon->timer.add_event_after(ping_timeout / PING_DIVISOR,
                               new C_MonContext{mon, [this, peer](int) { ping_check(peer); }});
}
...
bool Elector::send_peer_ping(int peer, const utime_t* n)
{
    ...
    if (peer >= ssize(mon->monmap->ranks)) {
        ...
        dout(5) << "peer: " << peer << " >= ranks_size: " << ssize(mon->monmap->ranks) << " ... dropping to prevent "
...
void Elector::ping_check(int peer)
{
    ...
    if (!live_pinging.count(peer) && !dead_pinging.count(peer)) {
...
void Elector::begin_dead_ping(int peer)
{
    ...
    mon->timer.add_event_after(ping_timeout, new C_MonContext{mon, [this, peer](int) { dead_ping(peer); }});
}
...
void Elector::handle_ping(MonOpRequestRef op)
{
    ...
    case MMonPing::PING_REPLY:
        ...
        utime_t now = ceph_clock_now();
        dout(30) << "now: " << now << " m->stamp: " << m->stamp << " ping_timeout: " << ping_timeout
                 << " PING_DIVISOR: " << PING_DIVISOR << dendl;
...
void Elector::dispatch(MonOpRequestRef op)
{
    ...
    case MSG_MON_ELECTION:
    {
        ...
        if (!mon->monmap->contains(em->get_source_addr())) {
            dout(1) << "discarding election message: " << em->get_source_addr() << " not in my monmap " << *mon->monmap
                    << dendl;
            return;
        }
        ...
        if (peermap.epoch > mon->monmap->epoch) {
            dout(0) << em->get_source_inst() << " has newer monmap epoch " << peermap.epoch << " > my epoch "
                    << mon->monmap->epoch << ", taking it" << dendl;
        ...
        if (peermap.epoch < mon->monmap->epoch) {
            dout(0) << em->get_source_inst() << " has older monmap epoch " << peermap.epoch << " < my epoch "
                    << mon->monmap->epoch << dendl;
        }

        if (em->strategy != logic.strategy) {
            dout(5) << __func__ << " somehow got an Election message with different strategy " << em->strategy
                    << " from local " << logic.strategy << "; dropping for now to let race resolve" << dendl;
            return;
        }
        ...
        switch (em->op) {
        case MMonElection::OP_PROPOSE: handle_propose(op); return;
        }
        ...
        switch (em->op) {
        case MMonElection::OP_ACK: handle_ack(op); return;
        case MMonElection::OP_VICTORY: handle_victory(op); return;
        case MMonElection::OP_NAK: handle_nak(op); return;
        default: ceph_abort();
        }
    } break;

    case MSG_MON_PING: handle_ping(op); break;
