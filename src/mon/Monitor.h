...
class Monitor : public Dispatcher, public AuthClient, public AuthServer, public md_config_obs_t
{
public:
    int orig_argc = 0;
...
public:
    ...
    {
        auto age = std::chrono::duration_cast<std::chrono::seconds>(ceph::mono_clock::now() - quorum_since);
        return age.count();
    }
...
public:
    ...
    void trigger_degraded_stretch_mode(const std::set<std::string>& dead_mons, const std::set<int>& dead_buckets);
...
private:
    ...
    bool _scrub(ScrubResult* r, std::pair<std::string, std::string>* start, int* num_keys);
...
private:
    ...
    void timecheck();
    health_status_t timecheck_status(std::ostringstream& ss, const double skew_bound, const double latency);
...
public:
    ...
    std::string get_leader_name() { return quorum.empty() ? std::string() : monmap->get_name(leader); }
    const std::set<int>& get_quorum() const { return quorum; }
    std::list<std::string> get_quorum_names()
    {
        std::list<std::string> q;
        for (auto p = quorum.begin(); p != quorum.end(); ++p) q.push_back(monmap->get_name(*p));
        return q;
    }
    uint64_t get_quorum_con_features() const { return quorum_con_features; }
    mon_feature_t get_quorum_mon_features() const { return quorum_mon_features; }
    uint64_t get_required_features() const { return required_features; }
    mon_feature_t get_required_mon_features() const { return monmap->get_required_features(); }
...
public:
    ...
    void win_election(epoch_t epoch, const std::set<int>& q, uint64_t features, const mon_feature_t& mon_features,
                      ceph_release_t min_mon_release, const std::map<int, Metadata>& metadata);
    void lose_election(epoch_t epoch, std::set<int>& q, int l, uint64_t features, const mon_feature_t& mon_features,
    ...
    class MonmapMonitor* monmon() { return (class MonmapMonitor*)paxos_service[PAXOS_MONMAP].get(); }

    class OSDMonitor* osdmon() { return (class OSDMonitor*)paxos_service[PAXOS_OSDMAP].get(); }

    class AuthMonitor* authmon() { return (class AuthMonitor*)paxos_service[PAXOS_AUTH].get(); }

    class LogMonitor* logmon() { return (class LogMonitor*)paxos_service[PAXOS_LOG].get(); }

    class MgrMonitor* mgrmon() { return (class MgrMonitor*)paxos_service[PAXOS_MGR].get(); }

    class MgrStatMonitor* mgrstatmon() { return (class MgrStatMonitor*)paxos_service[PAXOS_MGRSTAT].get(); }

    class HealthMonitor* healthmon() { return (class HealthMonitor*)paxos_service[PAXOS_HEALTH].get(); }

    class ConfigMonitor* configmon() { return (class ConfigMonitor*)paxos_service[PAXOS_CONFIG].get(); }

    class KVMonitor* kvmon() { return (class KVMonitor*)paxos_service[PAXOS_KV].get(); }
    ...
    template<typename Func, typename... Args> void with_session_map(Func&& func)
    {
    ...
    static void _generate_command_map(cmdmap_t& cmdmap, std::map<std::string, std::string>& param_str_map);
    static const MonCommand* _get_moncommand(const std::string& cmd_prefix, const std::vector<MonCommand>& cmds);
    bool _allowed_command(MonSession* s, const std::string& module, const std::string& prefix, const cmdmap_t& cmdmap,
                          const std::map<std::string, std::string>& param_str_map, const MonCommand* this_cmd);
    void get_mon_status(ceph::Formatter* f);
    void _quorum_status(ceph::Formatter* f, std::ostream& ss);
    bool _add_bootstrap_peer_hint(std::string_view cmd, const cmdmap_t& cmdmap, std::ostream& ss);
    ...
    void log_health(const health_check_map_t& updated, const health_check_map_t& previous,
...
protected:
    ...
    public:
        ...
        HealthCheckLogStatus(health_status_t severity_, const std::string& last_message_, utime_t updated_at_)
            : severity(severity_)
...
public:
    void get_cluster_status(std::stringstream& ss, ceph::Formatter* f, MonSession* session);
    ...
    struct C_Command : public C_MonOp
    {
        ...
        void _finish(int r) override
        {
            ...
            if (r >= 0) {
                ...
                if (prefix != "config set" && prefix != "config-key set") ss << "cmd='" << m->cmd << "': finished";
...
private:
    ...
    int get_auth_request(Connection* con, AuthConnectionMeta* auth_meta, uint32_t* method,
                         std::vector<uint32_t>* preferred_modes, ceph::buffer::list* out) override;
...
public:
    Monitor(CephContext* cct_, std::string nm, MonitorDBStore* s, Messenger* m, Messenger* mgr_m, MonMap* map);
    ~Monitor() override;
    ...
    int do_admin_command(std::string_view command, const cmdmap_t& cmdmap, ceph::Formatter* f, std::ostream& err,
                         std::ostream& out);
...
public:
    static void format_command_descriptions(const std::vector<MonCommand>& commands, ceph::Formatter* f,
                                            uint64_t features, ceph::buffer::list* rdata);
    ...
    void set_leader_commands(const std::vector<MonCommand>& cmds) { leader_mon_commands = cmds; }
 ...
 */
template<typename T> class C_MonContext : public LambdaContext<T>
{
public:
