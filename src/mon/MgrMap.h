...
public:
    struct ModuleOption
    {
        ...
        void dump(ceph::Formatter* f) const
        {
            ...
            f->dump_string("level", Option::level_to_str(static_cast<Option::level_t>(level)));
    ...
    public:
        ...
        bool operator==(const ModuleInfo& rhs) const { return (name == rhs.name) && (can_run == rhs.can_run); }
    ...
    public:
        ...
        StandbyInfo(uint64_t gid_, const std::string& name_, const std::vector<ModuleInfo>& am, uint64_t feat)
            : gid(gid_)
            , name(name_)
        ...
        bool have_module(const std::string& module_name) const
        {
            auto it = std::find_if(available_modules.begin(),
                                   ...
                                   [module_name](const ModuleInfo& m) -> bool { return m.name == module_name; });
    ...
    bool module_enabled(const std::string& module_name) const { return modules.find(module_name) != modules.end(); }
    ...
    static void generate_test_instances(std::list<MgrMap*>& l) { l.push_back(new MgrMap); }
