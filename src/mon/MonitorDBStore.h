...
public:
    std::string get_devname()
    {
        ...
        get_device_by_path(path.c_str(), partition, devname, sizeof(devname));
        return devname;
    }

    std::string get_path() { return path; }

    std::shared_ptr<PriorityCache::PriCache> get_priority_cache() const { return db->get_priority_cache(); }
    ...
    struct Op
    {
        ...
        void decode(ceph::buffer::list::const_iterator& decode_bl)
        {
            ...
            decode(bl, decode_bl);
            if (struct_v >= 2) decode(endkey, decode_bl);
        ...
        {
            return 6 + 1 + 4 + prefix.size() + 4 + key.size() + 4 + endkey.size() + 4 + bl.length();
        }
    ...
    struct Transaction
    {
        ...
        void erase_range(const std::string& prefix, const std::string& begin, const std::string& end)
        {
        ...
        void compact_prefix(const std::string& prefix) { ops.push_back(Op(OP_COMPACT, prefix, {})); }

        void compact_range(const std::string& prefix, const std::string& start, const std::string& end)
        {
        ...
        uint64_t get_keys() const { return keys; }
    ...
    int apply_transaction(MonitorDBStore::TransactionRef t)
    {
        ...
        for (auto it = t->ops.begin(); it != t->ops.end(); ++it) {
            const Op& op = *it;
            switch (op.type) {
            case Transaction::OP_PUT: dbt->set(op.prefix, op.key, op.bl); break;
            case Transaction::OP_ERASE: dbt->rmkey(op.prefix, op.key); break;
            case Transaction::OP_ERASE_RANGE: dbt->rm_range_keys(op.prefix, op.key, op.endkey); break;
            case Transaction::OP_COMPACT: compact.push_back(make_pair(op.prefix, make_pair(op.key, op.endkey))); break;
            default:
        ...
        if (r >= 0) {
            while (!compact.empty()) {
                if (compact.front().second.first == std::string() && compact.front().second.second == std::string())
                    db->compact_prefix_async(compact.front().first);
                else
                    db->compact_range_async(
                        compact.front().first, compact.front().second.first, compact.front().second.second);
                compact.pop_front();
            }
        }
        else {
    ...
    struct C_DoTransaction : public Context
    {
        ...
        Context* oncommit;
        C_DoTransaction(MonitorDBStore* s, MonitorDBStore::TransactionRef t, Context* f)
            : store(s)
            , t(t)
            , oncommit(f)
        {}
        void finish(int r) override
        {
            ...
            if (delay_prob && (rand() % 10000 < delay_prob * 10000.0)) {
                ...
                lsubdout(g_ceph_context, mon, 1)
                    << "apply_transaction will be delayed for " << delay << " seconds" << dendl;
                delay.sleep();
            }
     ...
     */
    void queue_transaction(MonitorDBStore::TransactionRef t, Context* oncommit)
    {
     ...
     */
    void flush() { io_work.wait_for_empty(); }
    ...
    public:
        __u32 crc()
        {
            if (g_conf()->mon_sync_debug) return crc_bl.crc32c(0);
            return 0;
        }
        std::pair<std::string, std::string> get_last_key() { return last_key; }
        virtual bool has_next_chunk() { return !done && _is_valid(); }
        virtual void get_chunk_tx(TransactionRef tx, uint64_t max_bytes, uint64_t max_keys) = 0;
    ...
    public:
        WholeStoreIteratorImpl(KeyValueDB::WholeSpaceIterator iter, std::set<std::string>& prefixes)
            : StoreIteratorImpl()
            , iter(iter)
         ...
         */
        void get_chunk_tx(TransactionRef tx, uint64_t max_bytes, uint64_t max_keys) override
        {
            using ceph::encode;
            ...
            while (iter->valid()) {
                ...
                if (sync_prefixes.count(prefix)) {
                    ...
                    if (tx->empty() || (tx->get_bytes() + value.length() + key.size() + prefix.size() < max_bytes &&
        ...
        bool _is_valid() override { return iter->valid(); }
    };

    Synchronizer get_synchronizer(std::pair<std::string, std::string>& key, std::set<std::string>& prefixes)
    {
        KeyValueDB::WholeSpaceIterator iter;
        ...
        return std::shared_ptr<StoreIteratorImpl>(new WholeStoreIteratorImpl(iter, prefixes));
    }
    ...
    version_t get(const std::string& prefix, const std::string& key)
    {
        ...
        if (err < 0) {
            ...
            generic_dout(0) << "MonitorDBStore::get() error obtaining"
                            << " (" << prefix << ":" << key << "): " << cpp_strerror(err) << dendl;
    ...
    void _open(const std::string& kv_type)
    {
        ...
        if (!db_ptr) {
            derr << __func__ << " error initializing " << kv_type << " db back storage in " << full_path << dendl;
        ...
        if (g_conf()->mon_debug_dump_transactions) {
            if (!g_conf()->mon_debug_dump_json) {
                dump_fd_binary =
                    ::open(g_conf()->mon_debug_dump_location.c_str(), O_CREAT | O_APPEND | O_WRONLY | O_CLOEXEC, 0644);
                if (dump_fd_binary < 0) {
    ...
    int open(std::ostream& out)
    {
        ...
        if (db->get_perf_counters()) {
            db->get_perf_counters()->set_prio_adjust(PerfCountersBuilder::PRIO_USEFUL -
    ...
    void compact_async() { db->compact_async(); }

    void compact_prefix(const std::string& prefix) { db->compact_prefix(prefix); }

    uint64_t get_estimated_size(std::map<std::string, uint64_t>& extras) { return db->get_estimated_size(extras); }
     ...
     */
    int write_meta(const std::string& key, const std::string& value) const
    {
        std::string v = value;
        v += "\n";
        int r = safe_write_file(path.c_str(), key.c_str(), v.c_str(), v.length(), 0600);
        if (r < 0) return r;
        return 0;
    }
     ...
     * @returns 0 for success, or an error code
     */
    int read_meta(const std::string& key, std::string* value) const
    {
        char buf[4096];
        int r = safe_read_file(path.c_str(), key.c_str(), buf, sizeof(buf));
        if (r <= 0) return r;
