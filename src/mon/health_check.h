...
struct health_check_t
{
    ...
    {
        return l.severity == r.severity && l.summary == r.summary && l.detail == r.detail && l.count == r.count;
    }
    friend bool operator!=(const health_check_t& l, const health_check_t& r) { return !(l == r); }
...
struct health_check_map_t
{
    ...
    void clear() { checks.clear(); }
    bool empty() const { return checks.empty(); }
    void swap(health_check_map_t& other) { checks.swap(other.checks); }

    health_check_t& add(const std::string& code, health_status_t severity, const std::string& summary, int64_t count)
    {
    ...
    health_check_t& get_or_add(const std::string& code, health_status_t severity, const std::string& summary,
                               int64_t count)
    {
        health_check_t& r = checks[code];
        r.severity = severity;
    ...
    void merge(const health_check_map_t& o)
    {
        for (auto& [code, check] : o.checks) {
            ...
            if (!new_check) {
                ...
                it->second.detail.insert(it->second.detail.end(), check.detail.begin(), check.detail.end());
    ...
    friend bool operator!=(const health_check_map_t& l, const health_check_map_t& r) { return !(l == r); }
};
