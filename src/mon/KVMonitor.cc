...
static ostream& _prefix(std::ostream* _dout, const Monitor& mon, const KVMonitor* hmon)
{
    return *_dout << "mon." << mon.name << "@" << mon.rank << "(" << mon.get_state_name() << ").kv ";
}
...
bool KVMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "config-key get") {
        ...
        while (iter->valid()) {
            if (key.size() && iter->key().find(key) != 0) {
                break;
            }
...
bool KVMonitor::prepare_command(MonOpRequestRef op)
{
    ...
    if (prefix == "config-key set" || prefix == "config-key put") {
        bufferlist data;
        string val;
        ...
        if (data.length() > (size_t)g_conf()->mon_config_key_max_entry_size) {
            ...
            ss << "error: entry size limited to " << g_conf()->mon_config_key_max_entry_size << " bytes. "
...
int KVMonitor::validate_osd_destroy(const int32_t id, const uuid_d& uuid)
{
    ...
    if (!_have_prefix(dmcrypt_prefix) && !_have_prefix(daemon_prefix)) {
        return -ENOENT;
    }
    return 0;
}
...
int KVMonitor::validate_osd_new(const uuid_d& uuid, const string& dmcrypt_key, stringstream& ss)
{
...
bool KVMonitor::maybe_send_update(Subscription* sub)
{
    ...
    if (sub->next && sub->next > get_first_committed()) {
        ...
        dout(10) << __func__ << " incremental keys for " << m->prefix << ", v " << sub->next << ".." << version << ", "
        ...
        while (iter->valid() && iter->key().find(m->prefix) == 0) {
