...
protected:
     ...
     */
    int _check_pool(OSDMap& osd_map, const int64_t pool_id, int type, bool force, std::ostream* ss,
...
public:
    ...
    int is_op_allowed(const MonOpRequestRef& op, const FSMap& fsmap, const cmdmap_t& cmdmap, std::ostream& ss) const;

    int can_handle(std::string const& prefix_, MonOpRequestRef& op, FSMap& fsmap, const cmdmap_t& cmdmap,
    ...
    virtual bool batched_propose() { return false; }

    virtual int handle(Monitor* mon, FSMap& fsmap, MonOpRequestRef op, const cmdmap_t& cmdmap, std::ostream& ss) = 0;
};
