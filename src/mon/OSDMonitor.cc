...
struct OSDMemCache : public PriorityCache::PriCache
{
    ...
    virtual int64_t get_cache_bytes(PriorityCache::Priority pri) const { return cache_bytes[pri]; }
    ...
    virtual void set_cache_bytes(PriorityCache::Priority pri, int64_t bytes) { cache_bytes[pri] = bytes; }
    virtual void add_cache_bytes(PriorityCache::Priority pri, int64_t bytes) { cache_bytes[pri] += bytes; }
...
{
    IncCache(OSDMonitor* m)
        : OSDMemCache(m){};

    virtual uint64_t _get_used_bytes() const { return osdmon->inc_osd_cache.get_bytes(); }

    virtual string get_cache_name() const { return "OSDMap Inc Cache"; }

    uint64_t _get_num_osdmaps() const { return osdmon->inc_osd_cache.get_size(); }
};

struct FullCache : public OSDMemCache
{
    FullCache(OSDMonitor* m)
        : OSDMemCache(m){};

    virtual uint64_t _get_used_bytes() const { return osdmon->full_osd_cache.get_bytes(); }

    virtual string get_cache_name() const { return "OSDMap Full Cache"; }

    uint64_t _get_num_osdmaps() const { return osdmon->full_osd_cache.get_size(); }
};
...
bool is_osd_writable(const OSDCapGrant& grant, const std::string* pool_name)
{
    ...
    if ((grant.spec.allow & OSD_CAP_W) != 0) {
        ...
        else if (pool_name != nullptr && !match.pool_namespace.pool_name.empty() &&
...
bool is_unmanaged_snap_op_permitted(CephContext* cct, const KeyServer& key_server, const EntityName& entity_name,
                                    const MonCap& mon_caps, const entity_addr_t& peer_socket_addr,
                                    const std::string* pool_name)
{
    typedef std::map<std::string, std::string> CommandArgs;

    if (mon_caps.is_capable(cct,
                            entity_name,
                            "osd",
                            "osd pool op unmanaged-snap",
                            (pool_name == nullptr ? CommandArgs{} /* pool DNE, require unrestricted cap */
    ...
    if (!key_server.get_service_caps(entity_name, CEPH_ENTITY_TYPE_OSD, caps_info)) {
...
void LastEpochClean::Lec::report(unsigned pg_num, ps_t ps, epoch_t last_epoch_clean)
{
    if (ps >= pg_num) {
        // removed PG
        return;
    ...
    if (last_epoch_clean < floor) {
        ...
        if (old_lec == floor) {
            ...
            auto new_floor = std::min_element(std::begin(epoch_by_pg), std::end(epoch_by_pg));
            floor = *new_floor;
        }
    }
    if (ps != next_missing) {
...
void LastEpochClean::report(unsigned pg_num, const pg_t& pg, epoch_t last_epoch_clean)
{
...
{
    return *_dout << "mon." << mon.name << "@" << mon.rank << "(" << mon.get_state_name() << ").osd e"
                  << osdmap.get_epoch() << " ";
}
    ...
    , mapper(mn.cct, &mn.cpu_tp)
{
    ...
    if (r < 0) {
        derr << __func__ << " using default osd cache size - mon_osd_cache_size (" << g_conf()->mon_osd_cache_size
             << ") without priority cache management" << dendl;
    }
}

const char** OSDMonitor::get_tracked_conf_keys() const
{
    static const char* KEYS[] = {"mon_memory_target", "mon_memory_autotune", "rocksdb_cache_size", NULL};
    return KEYS;
}

void OSDMonitor::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    ...
    if (changed.count("mon_memory_target") || changed.count("rocksdb_cache_size")) {
        int r = _update_mon_cache_settings();
        if (r < 0) {
            derr << __func__ << " mon_memory_target:" << g_conf()->mon_memory_target
                 << " rocksdb_cache_size:" << g_conf()->rocksdb_cache_size << ". Unable to update cache size." << dendl;
        }
    }
}
...
void OSDMonitor::_set_cache_autotuning()
{
    ...
    if (g_conf()->mon_memory_autotune && pcm == nullptr) {
        ...
        if (r < 0) {
            dout(10) << __func__ << " Error while registering osdmon caches with pcm."
                     << " Cache auto tuning not enabled." << dendl;
...
{
    if (g_conf()->mon_memory_target <= 0 || g_conf()->mon_memory_target < mon_memory_min ||
    ...
    if (r < 0) {
        derr << __func__ << " Cache ratios for pcm could not be set."
             << " Review the kv (rocksdb) and mon_memory_target sizes." << dendl;
    ...
    if (mon_memory_autotune && pcm != nullptr) {
        ...
        dout(1) << __func__ << " Updated mon cache setting."
                << " target: " << target << " min: " << min << " max: " << max << dendl;
    }
    return 0;
}
...
int OSDMonitor::_set_cache_sizes()
{
    if (g_conf()->mon_memory_autotune) {
        ...
        if (mon_memory_target <= 0 || mon_memory_min <= 0) {
            derr << __func__ << " mon_memory_target:" << mon_memory_target << " mon_memory_min:" << mon_memory_min
                 << ". Invalid size option(s) provided." << dendl;
            return -EINVAL;
        }
...
void OSDMonitor::create_initial()
{
    ...
    newmap.flags |= CEPH_OSDMAP_RECOVERY_DELETES | CEPH_OSDMAP_PURGED_SNAPDIRS | CEPH_OSDMAP_PGLOG_HARDLIMIT;
    ...
    newmap.encode(pending_inc.fullmap, features | CEPH_FEATURE_RESERVED);
...
void OSDMonitor::update_from_paxos(bool* need_bootstrap)
{
    ...
    if (version == osdmap.epoch) return;
    ...
    if (mapping_job) {
        if (!mapping_job->is_done()) {
            dout(1) << __func__ << " mapping job " << mapping_job.get() << " did not complete, " << mapping_job->shards
    ...
    if (latest_full == 0 && get_first_committed() > 1) latest_full = get_first_committed();

    if (get_first_committed() > 1 && latest_full < get_first_committed()) {
    ...
    if (!mon.store->get(OSD_PG_CREATING_PREFIX, "creating", bl)) {
        ...
        dout(7) << __func__ << " loading creating_pgs last_scan_epoch " << creating_pgs.last_scan_epoch << " with "
                << creating_pgs.pgs.size() << " pgs" << dendl;
    }
    else {
        dout(1) << __func__ << " missing creating pgs; upgrade from post-kraken?" << dendl;
    }
    ...
    while (version > osdmap.epoch) {
        ...
        if (mon_memory_autotune && pcm == nullptr) {
            ...
            if (r < 0) {
                dout(10) << __func__ << " Error while registering osdmon caches with pcm."
                         << " Proceeding without cache auto tuning." << dendl;
            }
        }
        ...
        if (!t) t.reset(new MonitorDBStore::Transaction);
        ...
        if (!f) f = mon.get_quorum_con_features();
        if (!f) f = -1;
        bufferlist full_bl;
    ...
    if (osdmap.stretch_mode_enabled) {
        ...
        if (osdmap.degraded_stretch_mode) {
            ...
            if (!osdmap.recovering_stretch_mode) {
                ...
                dout(20) << "mon_stretch_cluster_recovery_ratio: "
                         << cct->_conf.get_val<double>("mon_stretch_cluster_recovery_ratio") << dendl;
        ...
        if (marked_osd_down && (!osdmap.degraded_stretch_mode || osdmap.recovering_stretch_mode)) {
...
int OSDMonitor::register_cache_with_pcm()
{
    if (mon_memory_target <= 0 || mon_memory_min <= 0) {
        derr << __func__ << " Invalid memory size specified for mon caches."
             << " Caches will not be auto-tuned." << dendl;
        return -EINVAL;
    }
    uint64_t base = mon_memory_base;
    double fragmentation = mon_memory_fragmentation;
    ...
    pcm = std::make_shared<PriorityCache::Manager>(cct, min, max, target, true);
    pcm->insert("kv", rocksdb_binned_kv_cache, true);
    pcm->insert("inc", inc_cache, true);
    pcm->insert("full", full_cache, true);
    dout(1) << __func__ << " pcm target: " << target << " pcm max: " << max << " pcm min: " << min
            << " inc_osd_cache size: " << inc_osd_cache.get_size() << dendl;
    return 0;
}
...
int OSDMonitor::_set_cache_ratios()
{
    ...
    dout(1) << __func__ << " kv ratio " << cache_kv_ratio << " inc ratio " << cache_inc_ratio << " full ratio "
            << cache_full_ratio << dendl;
    return 0;
}
...
void OSDMonitor::start_mapping()
{
    ...
    if (!osdmap.get_pools().empty()) {
        ...
        mapping_job = mapping.start_update(osdmap, mapper, g_conf()->mon_osd_mapping_pgs_per_chunk);
...
{
    const int types[] = {
        entity_name_t::TYPE_OSD, entity_name_t::TYPE_CLIENT, entity_name_t::TYPE_MDS, entity_name_t::TYPE_MON};
    for (int type : types) {
        uint64_t mask;
...
void OSDMonitor::create_pending()
{
    ...
    // safety checks (this shouldn't really happen)
    {
        if (osdmap.backfillfull_ratio <= 0) {
            ...
            if (pending_inc.new_backfillfull_ratio > 1.0) pending_inc.new_backfillfull_ratio /= 100;
        ...
        if (osdmap.full_ratio <= 0) {
            ...
            if (pending_inc.new_full_ratio > 1.0) pending_inc.new_full_ratio /= 100;
        ...
        if (osdmap.nearfull_ratio <= 0) {
            ...
            if (pending_inc.new_nearfull_ratio > 1.0) pending_inc.new_nearfull_ratio /= 100;
            dout(1) << __func__ << " setting nearfull_ratio = " << pending_inc.new_nearfull_ratio << dendl;
        }
    }
}

creating_pgs_t OSDMonitor::update_pending_pgs(const OSDMap::Incremental& inc, const OSDMap& nextmap)
{
    dout(10) << __func__ << dendl;
    ...
    if (pending_creatings.last_scan_epoch < inc.epoch) {
        unsigned queued = 0;
        queued += scan_for_creating_pgs(osdmap.get_pools(), inc.old_pools, inc.modified, &pending_creatings);
        queued += scan_for_creating_pgs(inc.new_pools, inc.old_pools, inc.modified, &pending_creatings);
        ...
        for (auto deleted_pool : inc.old_pools) {
            ...
            dout(10) << __func__ << " " << removed << " pg removed because containing pool deleted: " << deleted_pool
                     << dendl;
    ...
    while (pending_creatings.pgs.size() < max && !pending_creatings.queue.empty()) {
        auto p = pending_creatings.queue.begin();
        int64_t poolid = p->first;
        dout(10) << __func__ << " pool " << poolid << " created " << p->second.created << " modified "
                 << p->second.modified << " [" << p->second.start << "-" << p->second.end << ")" << dendl;
        int64_t n = std::min<int64_t>(max - pending_creatings.pgs.size(), p->second.end - p->second.start);
        ...
        for (ps_t ps = first; ps < end; ++ps) {
            ...
            pending_creatings.pgs.emplace(pgid, creating_pgs_t::pg_create_info(inc.epoch, p->second.modified));
        ...
        if (p->second.done()) {
            ...
            dout(10) << __func__ << " pool " << poolid << " now [" << p->second.start << "-" << p->second.end << ")"
                     << dendl;
        }
    }
    ...
    if (mon.monmap->min_mon_release >= ceph_release_t::octopus) {
        ...
        for (auto& i : pending_creatings.pgs) {
            ...
            {
                const pg_pool_t* pi;
                bool operator()(const set<pg_shard_t>& have) const { return have.size() >= pi->min_size; }
                explicit min_size_predicate_t(const pg_pool_t* i)
                    : pi(i)
                {}
            ...
            nextmap.pg_to_up_acting_osds(pgid, &up, &up_primary, &acting, &acting_primary);
            if (i.second.history.epoch_created == 0) {
                ...
                i.second.history = pg_history_t(i.second.create_epoch, i.second.create_stamp);
                dout(10) << __func__ << "  pg " << pgid << " just added, "
                         << " up " << i.second.up << " p " << i.second.up_primary << " acting " << i.second.acting
                         << " p " << i.second.acting_primary << " history " << i.second.history << " past_intervals "
                         << i.second.past_intervals << dendl;
            }
            else {
                std::stringstream debug;
                if (PastIntervals::check_new_interval(i.second.acting_primary,
                                                      acting_primary,
                                                      i.second.acting,
                                                      acting,
                                                      i.second.up_primary,
                    ...
                    if (pgid.is_split(osdmap.get_pg_num(pgid.pool()), nextmap.get_pg_num(pgid.pool()), nullptr)) {
                        i.second.history.last_epoch_split = e;
                    }
                    dout(10) << __func__ << "  pg " << pgid << " new interval,"
                             << " up " << i.second.up << " -> " << up << " p " << i.second.up_primary << " -> "
                             << up_primary << " acting " << i.second.acting << " -> " << acting << " p "
                             << i.second.acting_primary << " -> " << acting_primary << " history " << i.second.history
                             << " past_intervals " << i.second.past_intervals << dendl;
    ...
    dout(10) << __func__ << " " << (pending_creatings.pgs.size() - total) << "/" << pending_creatings.pgs.size()
...
void OSDMonitor::maybe_prime_pg_temp()
{
    ...
    set<int> osds;
    for (auto p = pending_inc.new_state.begin(); !all && p != pending_inc.new_state.end(); ++p) {
        if ((p->second & CEPH_OSD_UP) && osdmap.is_up(p->first)) {
            osds.insert(p->first);
        }
    }
    for (auto p = pending_inc.new_weight.begin(); !all && p != pending_inc.new_weight.end(); ++p) {
    ...
    if (!all && osds.empty()) return;

    if (!all) {
        unsigned estimate = mapping.get_osd_acting_pgs(*osds.begin()).size() * osds.size();
        if (estimate > mapping.get_num_pgs() * g_conf()->mon_osd_prime_pg_temp_max_estimate) {
            dout(10) << __func__ << " estimate " << estimate << " pgs on " << osds.size()
                     << " osds >= " << g_conf()->mon_osd_prime_pg_temp_max_estimate << " of total "
                     << mapping.get_num_pgs() << " pgs, all" << dendl;
            all = true;
        }
        else {
    ...
    if (next.get_pools().empty()) {
        ...
        if (job.wait_for(g_conf()->mon_osd_prime_pg_temp_max_time)) {
            ...
            dout(10) << __func__ << " did not finish in " << g_conf()->mon_osd_prime_pg_temp_max_time << ", stopping"
                     << dendl;
            job.abort();
        }
    }
...
void OSDMonitor::prime_pg_temp(const OSDMap& next, pg_t pgid)
{
    ...
    next.pg_to_up_acting_osds(pgid, &next_up, &next_up_primary, &next_acting, &next_acting_primary);
    if (acting == next_acting && !(up != acting && next_up == next_acting)) return;   // no change since last epoch

    if (acting.empty()) return;   // if previously empty now we can be no worse off
    const pg_pool_t* pool = next.get_pg_pool(pgid.pool());
    if (pool && acting.size() < pool->min_size) return;   // can be no worse off than before
    ...
    dout(20) << __func__ << " " << pgid << " " << up << "/" << acting << " -> " << next_up << "/" << next_acting
             << ", priming " << acting << dendl;
    {
        std::lock_guard l(prime_pg_temp_lock);
        // do not touch a mapping if a change is pending
        pending_inc.new_pg_temp.emplace(pgid, mempool::osdmap::vector<int>(acting.begin(), acting.end()));
    }
}
...
void OSDMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
    ...
    if (mapping_job) {
        if (!mapping_job->is_done()) {
            dout(1) << __func__ << " skipping prime_pg_temp; mapping job " << mapping_job.get() << " did not complete, "
                    << mapping_job->shards << " left" << dendl;
            mapping_job->abort();
        }
        else if (mapping.get_epoch() < osdmap.get_epoch()) {
            dout(1) << __func__ << " skipping prime_pg_temp; mapping job " << mapping_job.get() << " is prior epoch "
        ...
        dout(1) << __func__ << " skipping prime_pg_temp; mapping job did not start" << dendl;
    }
    mapping_job.reset();
...

    {
        ...
        // clean inappropriate pg_upmap/pg_upmap_items (if any)
        {
            ...
            if (pgs_to_check.size() < static_cast<uint64_t>(g_conf()->mon_clean_pg_upmaps_per_chunk * 2)) {
        ...
        for (auto& i : tmp.get_pools()) {
            ...
            if (i.second.has_flag(pg_pool_t::FLAG_CREATING) && !pending_creatings.still_creating_pool(i.first)) {
        ...
        tmp.get_full_pools(cct, &full_pool_ids, &backfillfull_pool_ids, &nearfull_pool_ids);
        if (full_pool_ids.empty() || backfillfull_pool_ids.empty() || nearfull_pool_ids.empty()) {
            ...
            for (auto& pool : tmp.get_pools()) {
                auto p = pool.first;
                if (tmp.get_pg_pool(p)->has_flag(pg_pool_t::FLAG_NEARFULL) && nearfull_pool_ids.empty()) {
                ...
                if (tmp.get_pg_pool(p)->has_flag(pg_pool_t::FLAG_BACKFILLFULL) && backfillfull_pool_ids.empty()) {
                ...
                if (tmp.get_pg_pool(p)->has_flag(pg_pool_t::FLAG_FULL) && full_pool_ids.empty()) {
        ...
        if (!nearfull_pool_ids.empty()) {
            ...
            for (auto& pool : tmp.get_pools()) {
                auto p = pool.first;
                if (full_pool_ids.count(p) || backfillfull_pool_ids.count(p) || nearfull_pool_ids.count(p)) {
        ...
        if (osdmap.require_osd_release < ceph_release_t::octopus &&
            ...
            for (auto& [poolid, pi] : tmp.pools) {
                ...
                if (pi.cache_mode == pg_pool_t::CACHEMODE_READFORWARD) {
                    ...
                    dout(10) << __func__ << " switching pool " << poolid << " cachemode from readforward -> readproxy"
                             << dendl;
                    pending_inc.new_pools[poolid].cache_mode = pg_pool_t::CACHEMODE_READPROXY;
                }
            }
            ...
            if (!combined.empty()) {
                ...
                dout(10) << __func__ << " recording pre-octopus purged_snaps in epoch " << (pending_inc.epoch - 1)
    ...
    // tell me about it
    for (auto i = pending_inc.new_state.begin(); i != pending_inc.new_state.end(); ++i) {
        ...
        if (s & CEPH_OSD_EXISTS) dout(2) << " osd." << i->first << " DNE" << dendl;
    }
    for (auto i = pending_inc.new_up_client.begin(); i != pending_inc.new_up_client.end(); ++i) {
        // FIXME: insert cluster addresses too
        dout(2) << " osd." << i->first << " UP " << i->second << dendl;
    }
    for (map<int32_t, uint32_t>::iterator i = pending_inc.new_weight.begin(); i != pending_inc.new_weight.end(); ++i) {
    ...
    OSDMap tmp;
    {
        ...
        dout(10) << __func__ << " encoding full map with " << tmp.require_osd_release << " features " << features
                 << dendl;
    ...
    // metadata, too!
    for (map<int, bufferlist>::iterator p = pending_metadata.begin(); p != pending_metadata.end(); ++p) {
        Metadata m;
    ...
    for (set<int>::iterator p = pending_metadata_rm.begin(); p != pending_metadata_rm.end(); ++p) {
    ...
    // purged_snaps
    if (tmp.require_osd_release >= ceph_release_t::octopus && !pending_inc.new_purged_snaps.empty()) {
    ...
    for (auto& i : pending_inc.new_purged_snaps) {
        for (auto q = i.second.begin(); q != i.second.end(); ++q) {
            insert_purged_snap_update(i.first, q.get_start(), q.get_end(), pending_inc.epoch, t);
        }
    }
    for (auto& [pool, snaps] : pending_pseudo_purged_snaps) {
        for (auto snap : snaps) {
            insert_purged_snap_update(pool, snap, snap + 1, pending_inc.epoch, t);
        }
    }
...
int OSDMonitor::load_metadata(int osd, map<string, string>& m, ostream* err)
{
    ...
    try {
        ...
        if (err) *err << "osd." << osd << " metadata is corrupt";
        return -EIO;
    }
    return 0;
}
...
int OSDMonitor::get_osd_objectstore_type(int osd, string* type)
{
    ...
    if (it == metadata.end()) return -ENOENT;
    *type = it->second;
    return 0;
}
...
{
    if (g_conf()->mon_warn_on_filestore_osds && filestore_osds.size() > 0) {
        ostringstream ss, deprecated_tip;
        list<string> detail;
        ss << filestore_osds.size() << " osd(s) " << (filestore_osds.size() == 1 ? "is" : "are")
           << " running Filestore";
        deprecated_tip << ss.str();
        ss << " [Deprecated]";
        auto& d = checks->add("OSD_FILESTORE", HEALTH_WARN, ss.str(), filestore_osds.size());
...
bool OSDMonitor::is_pool_currently_all_bluestore(int64_t pool_id, const pg_pool_t& pool, ostream* err)
{
    ...
    for (unsigned ps = 0; ps < std::min(8u, pool.get_pg_num()); ++ps) {
        ...
        for (int osd : up) {
            if (checked_osds.find(osd) != checked_osds.end()) continue;
            string objectstore_type;
            int r = get_osd_objectstore_type(osd, &objectstore_type);
...
{
    map<string, string> m;
    if (int r = load_metadata(osd, m, err)) return r;
    for (map<string, string>::iterator p = m.begin(); p != m.end(); ++p) f->dump_string(p->first.c_str(), p->second);
    return 0;
}
...
version_t OSDMonitor::get_trim_to() const
{
    ...
    if (g_conf().get_val<bool>("mon_debug_block_osdmap_trim")) {
        dout(0) << __func__ << " blocking osdmap trim"
...

    {
        ...
        if (g_conf()->mon_osd_force_trim_to > 0 && g_conf()->mon_osd_force_trim_to < (int)get_last_committed()) {
...
void OSDMonitor::encode_trim_extra(MonitorDBStore::TransactionRef tx, version_t first)
{
    ...
    if (has_osdmap_manifest && first > osdmap_manifest.get_first_pinned()) {
...
{
    bool store_has_manifest = mon.store->exists(get_service_name(), "osdmap_manifest");
    ...
    dout(20) << __func__ << " osdmap manifest detected in store; reload." << dendl;
    ...
    dout(10) << __func__ << " store osdmap manifest pinned (" << osdmap_manifest.get_first_pinned() << " .. "
             << osdmap_manifest.get_last_pinned() << ")" << dendl;
}
...
bool OSDMonitor::should_prune() const
{
    ...
    version_t min_osdmap_epochs = g_conf().get_val<int64_t>("mon_min_osdmap_epochs");
    version_t prune_min = g_conf().get_val<uint64_t>("mon_osdmap_full_prune_min");
    version_t prune_interval = g_conf().get_val<uint64_t>("mon_osdmap_full_prune_interval");
    ...
    if ((last - first) <= min_osdmap_epochs) {
        ...
        dout(10) << __func__ << " currently holding only " << (last - first)
                 << " epochs (min osdmap epochs: " << min_osdmap_epochs << "); do not prune." << dendl;
        return false;
    }
        ...
        dout(10) << __func__ << " could only prune " << (last_to_pin - first) << " epochs (" << first << ".."
                 << last_to_pin
                 << "), which"
        ...
        dout(10) << __func__ << " we have pruned as far as we can; do not prune." << dendl;
        return false;
    }
    else if (last_pinned + prune_interval > last_to_pin) {
        dout(10) << __func__ << " not enough epochs to form an interval (last pinned: " << last_pinned
                 << ", last to pin: " << last_to_pin << ", interval: " << prune_interval << ")" << dendl;
        return false;
    }

    dout(15) << __func__ << " should prune (" << last_pinned << ".." << last_to_pin << ")"
             << " lc (" << first << ".." << last << ")" << dendl;
    return true;
}

void OSDMonitor::_prune_update_trimmed(MonitorDBStore::TransactionRef tx, version_t first)
{
    ...
    if (manifest.get_last_pinned() == first + 1 || manifest.pinned.size() == 1) {
...
void OSDMonitor::prune_init(osdmap_manifest_t& manifest)
{
    ...
    if (!has_osdmap_manifest) {
        ...
        dout(10) << __func__ << " first_pinned " << osdmap_manifest.get_first_pinned() << " last_pinned "
                 << osdmap_manifest.get_last_pinned() << dendl;
...
{
    uint64_t prune_interval = g_conf().get_val<uint64_t>("mon_osdmap_full_prune_interval");
    uint64_t prune_min = g_conf().get_val<uint64_t>("mon_osdmap_full_prune_min");
    uint64_t txsize = g_conf().get_val<uint64_t>("mon_osdmap_full_prune_txsize");

    bool r = true;

    if (prune_interval == 0) {
        derr << __func__ << " prune is enabled BUT prune interval is zero; abort." << dendl;
        r = false;
    }
    ...
    if (prune_min == 0) {
        derr << __func__ << " prune is enabled BUT prune min is zero; abort." << dendl;
        r = false;
    }
    if (prune_interval > prune_min) {
        derr << __func__ << " impossible to ascertain proper prune interval because"
             << " it is greater than the minimum prune epochs"
             << " (min: " << prune_min << ", interval: " << prune_interval << ")" << dendl;
        r = false;
    }

    if (txsize < prune_interval - 1) {
        derr << __func__ << " 'mon_osdmap_full_prune_txsize' (" << txsize << ") < 'mon_osdmap_full_prune_interval-1' ("
             << prune_interval - 1 << "); abort." << dendl;
        r = false;
    }
    return r;
}
...
{
    return mon.get_required_mon_features().contains_any(ceph::features::mon::FEATURE_OSDMAP_PRUNE);
}
...
bool OSDMonitor::do_prune(MonitorDBStore::TransactionRef tx)
{
    ...
    dout(5) << __func__ << " lc (" << first << " .. " << last << ")"
            << " last_pinned " << last_pinned << " interval " << prune_interval << " last_to_pin " << last_to_pin
            << dendl;
    ...
    while (num_pruned + removal_interval <= txsize) {
        ...
        dout(20) << __func__ << " last_pinned " << last_pinned << " next_pinned " << next_pinned << " num_pruned "
                 << num_pruned << " removal interval (" << (last_pinned + 1) << ".." << (next_pinned - 1) << ")"
                 << " txsize " << txsize << dendl;
...
bool OSDMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    switch (m->get_type()) {
        // READs
    case MSG_MON_COMMAND: try { return preprocess_command(op);
        }
    ...
    case CEPH_MSG_MON_GET_OSDMAP:
        ...
        // damp updates
    case MSG_OSD_MARK_ME_DOWN: return preprocess_mark_me_down(op);
    case MSG_OSD_MARK_ME_DEAD: return preprocess_mark_me_dead(op);
    case MSG_OSD_FULL: return preprocess_full(op);
    case MSG_OSD_FAILURE: return preprocess_failure(op);
    case MSG_OSD_BOOT: return preprocess_boot(op);
    case MSG_OSD_ALIVE: return preprocess_alive(op);
    case MSG_OSD_PG_CREATED: return preprocess_pg_created(op);
    case MSG_OSD_PG_READY_TO_MERGE: return preprocess_pg_ready_to_merge(op);
    case MSG_OSD_PGTEMP: return preprocess_pgtemp(op);
    case MSG_OSD_BEACON: return preprocess_beacon(op);

    case CEPH_MSG_POOLOP: return preprocess_pool_op(op);

    case MSG_REMOVE_SNAPS: return preprocess_remove_snaps(op);

    case MSG_MON_GET_PURGED_SNAPS: return preprocess_get_purged_snaps(op);

    default: ceph_abort(); return false;
    }
}
...
bool OSDMonitor::prepare_update(MonOpRequestRef op)
{
    ...
    switch (m->get_type()) {
        // damp updates
    case MSG_OSD_MARK_ME_DOWN: return prepare_mark_me_down(op);
    case MSG_OSD_MARK_ME_DEAD: return prepare_mark_me_dead(op);
    case MSG_OSD_FULL: return prepare_full(op);
    case MSG_OSD_FAILURE: return prepare_failure(op);
    case MSG_OSD_BOOT: return prepare_boot(op);
    case MSG_OSD_ALIVE: return prepare_alive(op);
    case MSG_OSD_PG_CREATED: return prepare_pg_created(op);
    case MSG_OSD_PGTEMP: return prepare_pgtemp(op);
    case MSG_OSD_PG_READY_TO_MERGE: return prepare_pg_ready_to_merge(op);
    case MSG_OSD_BEACON: return prepare_beacon(op);

    case MSG_MON_COMMAND: try { return prepare_command(op);
        }
        catch (const bad_cmd_get& e) {
            bufferlist bl;
            mon.reply_command(op, -EINVAL, e.what(), bl, get_last_committed());
    ...
    case CEPH_MSG_POOLOP: return prepare_pool_op(op);

    case MSG_REMOVE_SNAPS: return prepare_remove_snaps(op);
...
bool OSDMonitor::should_propose(double& delay)
{
    ...
    if (pending_inc.fullmap.length()) return true;

    // adjust osd weights?
    if (!osd_weight.empty() && osd_weight.size() == (unsigned)osdmap.get_max_osd()) {
...
bool OSDMonitor::preprocess_get_osdmap(MonOpRequestRef op)
{
    ...
    if (op->get_session() && op->get_session()->con_features) features = op->get_session()->con_features;
...
bool OSDMonitor::preprocess_failure(MonOpRequestRef op)
{
    ...
    // check permissions
    if (check_source(op, m->fsid)) goto didit;

    // first, verify the reporting host is valid
    if (m->get_orig_source().is_osd()) {
        int from = m->get_orig_source().num();
        if (!osdmap.exists(from) || !osdmap.get_addrs(from).legacy_equals(m->get_orig_source_addrs()) ||
    ...
    if (osdmap.is_down(badboy)) {
        dout(5) << "preprocess_failure dne(/dup?): osd." << m->get_target_osd() << " " << m->get_target_addrs()
                << ", from " << m->get_orig_source() << dendl;
        if (m->get_epoch() < osdmap.get_epoch()) send_incremental(op, m->get_epoch() + 1);
        goto didit;
    }
    if (osdmap.get_addrs(badboy) != m->get_target_addrs()) {
        dout(5) << "preprocess_failure wrong osd: report osd." << m->get_target_osd() << " " << m->get_target_addrs()
                << " != map's " << osdmap.get_addrs(badboy) << ", from " << m->get_orig_source() << dendl;
        if (m->get_epoch() < osdmap.get_epoch()) send_incremental(op, m->get_epoch() + 1);
        goto didit;
    }

    // already reported?
    if (osdmap.is_down(badboy) || osdmap.get_up_from(badboy) > m->get_epoch()) {
        dout(5) << "preprocess_failure dup/old: osd." << m->get_target_osd() << " " << m->get_target_addrs()
                << ", from " << m->get_orig_source() << dendl;
        if (m->get_epoch() < osdmap.get_epoch()) send_incremental(op, m->get_epoch() + 1);
        goto didit;
    }
    ...
    if (!can_mark_down(badboy)) {
        dout(5) << "preprocess_failure ignoring report of osd." << m->get_target_osd() << " " << m->get_target_addrs()
                << " from " << m->get_orig_source() << dendl;
        goto didit;
    }

    dout(10) << "preprocess_failure new: osd." << m->get_target_osd() << " " << m->get_target_addrs() << ", from "
             << m->get_orig_source() << dendl;
    return false;
...
public:
    C_AckMarkedDown(OSDMonitor* osdmon, MonOpRequestRef op)
        : C_MonOp(op)
        , osdmon(osdmon)
    {}
    ...
    void _finish(int r) override
    {
        if (r == 0) {
            ...
            osdmon->mon.send_reply(op,
                                   new MOSDMarkMeDown(m->fsid,
                                                      m->target_osd,
                                                      m->target_addrs,
                                                      m->get_epoch(),
...
bool OSDMonitor::preprocess_mark_me_down(MonOpRequestRef op)
{
    ...
    // check permissions
    if (check_source(op, m->fsid)) goto reply;

    // first, verify the reporting host is valid
    if (!m->get_orig_source().is_osd()) goto reply;

    if (!osdmap.exists(from) || osdmap.is_down(from) || osdmap.get_addrs(from) != m->target_addrs) {
    ...
    if (!can_mark_down(from)) goto reply;
...
bool OSDMonitor::prepare_mark_me_down(MonOpRequestRef op)
{
    ...
    if (m->request_ack) wait_for_finished_proposal(op, new C_AckMarkedDown(this, op));
    return true;
}
...
bool OSDMonitor::preprocess_mark_me_dead(MonOpRequestRef op)
{
    ...
    if (!osdmap.exists(from) || !osdmap.is_down(from)) {
...
bool OSDMonitor::prepare_mark_me_dead(MonOpRequestRef op)
{
    ...
    mon.clog->info() << "osd." << target_osd << " marked itself dead as of e" << m->get_epoch();
    if (!pending_inc.new_xinfo.count(target_osd)) {
        pending_inc.new_xinfo[target_osd] = osdmap.osd_xinfo[target_osd];
    }
    pending_inc.new_xinfo[target_osd].dead_epoch = m->get_epoch();
...
bool OSDMonitor::can_mark_down(int i)
{
    ...
    if (up_ratio < g_conf()->mon_osd_min_up_ratio) {
        dout(2) << __func__ << " current up_ratio " << up_ratio << " < min " << g_conf()->mon_osd_min_up_ratio
...
bool OSDMonitor::can_mark_out(int i)
{
    ...
    if (in_ratio < g_conf()->mon_osd_min_in_ratio) {
        if (i >= 0)
            dout(5) << __func__ << " current in_ratio " << in_ratio << " < min " << g_conf()->mon_osd_min_in_ratio
...
bool OSDMonitor::check_failures(utime_t now)
{
    ...
    while (p != failure_info.end()) {
        ...
        if (can_mark_down(target_osd) && check_failure(now, target_osd, fi)) {
            found_failure = true;
            ++p;
        }
        else if (is_failure_stale(now, fi)) {
            dout(10) << " dropping stale failure_info for osd." << target_osd << " from " << fi.reporters.size()
                     << " reporters" << dendl;
...
utime_t OSDMonitor::get_grace_time(utime_t now, int target_osd, failure_info_t& fi) const
{
    ...
    dout(20) << " halflife " << halflife << " decay_k " << decay_k << " failed_for " << failed_for << " decay " << decay
             << dendl;
    ...
    grace += peer_grace;
    dout(10) << " osd." << target_osd << " has " << fi.reporters.size() << " reporters, " << grace << " grace ("
             << orig_grace << " + " << my_grace << " + " << peer_grace << "), max_failed_since "
             << fi.get_failed_since() << dendl;
...
bool OSDMonitor::check_failure(utime_t now, int target_osd, failure_info_t& fi)
{
    ...
    if (pending_inc.new_state.count(target_osd) && pending_inc.new_state[target_osd] & CEPH_OSD_UP) {
    ...
    for (auto p = fi.reporters.begin(); p != fi.reporters.end();) {
        ...
        if (osdmap.exists(p->first)) {
            ...
            if (auto iter = reporter_loc.find(reporter_subtree_level); iter == reporter_loc.end()) {
    ...
    if (failed_for >= grace) {
        ...
        mon.clog->info() << "osd." << target_osd << " failed ("
                         ...
                         << (int)reporters_by_subtree.size() << " reporters from different " << reporter_subtree_level
                         << " after " << failed_for << " >= grace " << grace << ")";
        return true;
    }
    return false;
}
...
bool OSDMonitor::prepare_failure(MonOpRequestRef op)
{
    op->mark_osdmon_event(__func__);
    auto m = op->get_req<MOSDFailure>();
    dout(1) << "prepare_failure osd." << m->get_target_osd() << " " << m->get_target_addrs() << " from "
            << m->get_orig_source() << " is reporting failure:" << m->if_osd_failed() << dendl;
    ...
    if (m->if_osd_failed()) {
        ...
        utime_t failed_since = m->get_recv_stamp() - utime_t(m->failed_for, 0);

        // add a report
        if (m->is_immediate()) {
            mon.clog->debug() << "osd." << m->get_target_osd() << " reported immediately failed by "
                              << m->get_orig_source();
            force_failure(target_osd, reporter);
            return true;
        }
        mon.clog->debug() << "osd." << m->get_target_osd() << " reported failed by " << m->get_orig_source();
        ...
        // remove the report
        mon.clog->debug() << "osd." << m->get_target_osd() << " failure report canceled by " << m->get_orig_source();
        if (failure_info.count(target_osd)) {
            ...
            if (fi.reporters.empty()) {
                ...
                dout(10) << " failure_info for osd." << target_osd << " now " << fi.reporters.size() << " reporters"
                         << dendl;
            }
        }
        else {
...
void OSDMonitor::take_all_failures(list<MonOpRequestRef>& ls)
{
    ...
    for (map<int, failure_info_t>::iterator p = failure_info.begin(); p != failure_info.end(); ++p) {
        p->second.take_report_messages(ls);
    }
    failure_info.clear();
}
...
bool OSDMonitor::grace_interval_threshold_exceeded(int last_failed_interval)
{
    ...
    if (last_failed_interval > grace_interval_threshold_secs) {
        dout(1) << " last_failed_interval " << last_failed_interval << " > grace_interval_threshold_secs "
                << grace_interval_threshold_secs << dendl;
        return true;
    }
    return false;
}
...
bool OSDMonitor::preprocess_boot(MonOpRequestRef op)
{
    ...
    if (HAVE_FEATURE(m->osd_features, SERVER_QUINCY) && osdmap.require_osd_release < ceph_release_t::octopus) {
        mon.clog->info() << "disallowing boot of quincy+ OSD " << m->get_orig_source_inst()
                         << " because require_osd_release < octopus";
        goto ignore;
    }
    if (HAVE_FEATURE(m->osd_features, SERVER_REEF) && osdmap.require_osd_release < ceph_release_t::pacific) {
        mon.clog->info() << "disallowing boot of reef+ OSD " << m->get_orig_source_inst()
                         << " because require_osd_release < pacific";
        goto ignore;
    }

    // See crimson/osd/osd.cc: OSD::_send_boot
    if (auto type_iter = m->metadata.find("osd_type"); type_iter != m->metadata.end()) {
        ...
        if (otype == "crimson") {
            if (!osdmap.get_allow_crimson()) {
                mon.clog->info() << "Disallowing boot of crimson-osd without allow_crimson "
            ...
            derr << __func__ << ": osd " << m->get_orig_source_inst()
                 << " sent non-crimson osd_type field in MOSDBoot: " << otype << " -- booting anyway" << dendl;
        }
    }

    if (osdmap.stretch_mode_enabled && !(m->osd_features & CEPH_FEATUREMASK_STRETCH_MODE)) {
        mon.clog->info() << "disallowing boot of OSD " << m->get_orig_source_inst()
    ...
    // already booted?
    if (osdmap.is_up(from) && osdmap.get_addrs(from).legacy_equals(m->get_orig_source_addrs()) &&
        osdmap.get_cluster_addrs(from).legacy_equals(m->cluster_addrs)) {
        // yup.
        dout(7) << "preprocess_boot dup from " << m->get_orig_source() << " " << m->get_orig_source_addrs() << " =~ "
                << osdmap.get_addrs(from) << dendl;
        _booted(op, false);
        return true;
    }

    if (osdmap.exists(from) && !osdmap.get_uuid(from).is_zero() && osdmap.get_uuid(from) != m->sb.osd_fsid) {
        dout(7) << __func__ << " from " << m->get_orig_source_inst() << " clashes with existing osd: different fsid"
                << " (ours: " << osdmap.get_uuid(from) << " ; theirs: " << m->sb.osd_fsid << ")" << dendl;
        goto ignore;
    }

    if (osdmap.exists(from) && osdmap.get_info(from).up_from > m->version &&
        osdmap.get_most_recent_addrs(from).legacy_equals(m->get_orig_source_addrs())) {
...
bool OSDMonitor::prepare_boot(MonOpRequestRef op)
{
    op->mark_osdmon_event(__func__);
    auto m = op->get_req<MOSDBoot>();
    dout(7) << __func__ << " from " << m->get_source() << " sb " << m->sb << " client_addrs"
            << m->get_connection()->get_peer_addrs() << " cluster_addrs " << m->cluster_addrs << " hb_back_addrs "
            << m->hb_back_addrs << " hb_front_addrs " << m->hb_front_addrs << dendl;
    ...
    if (pending_inc.new_state.count(from)) oldstate ^= pending_inc.new_state[from];
    ...
    if (osdmap.is_up(from)) {
        ...
        ceph_assert(!osdmap.get_addrs(from).legacy_equals(m->get_orig_source_addrs()) ||
                    !osdmap.get_cluster_addrs(from).legacy_equals(m->cluster_addrs));
        ceph_assert(osdmap.get_uuid(from) == m->sb.osd_fsid);

        if (pending_inc.new_state.count(from) == 0 || (pending_inc.new_state[from] & CEPH_OSD_UP) == 0) {
        ...
        if (m->sb.weight) osd_weight[from] = m->sb.weight;
        ...
        if (m->sb.mounted > info.last_clean_begin ||
            (m->sb.mounted == info.last_clean_begin && m->sb.clean_thru > info.last_clean_end)) {
            epoch_t begin = m->sb.mounted;
            epoch_t end = m->sb.clean_thru;

            dout(10) << __func__ << " osd." << from << " last_clean_interval "
                     << "[" << info.last_clean_begin << "," << info.last_clean_end << ") -> [" << begin << "-" << end
                     << ")" << dendl;
            pending_inc.new_last_clean_interval[from] = pair<epoch_t, epoch_t>(begin, end);
        }

        if (pending_inc.new_xinfo.count(from) == 0) pending_inc.new_xinfo[from] = osdmap.osd_xinfo[from];
        ...
        if (m->boot_epoch == 0) {
            ...
            if (xi.down_stamp.sec()) {
                int interval = ceph_clock_now().sec() - xi.down_stamp.sec();
                ...
                xi.laggy_interval = interval * g_conf()->mon_osd_laggy_weight +
                                    xi.laggy_interval * (1.0 - g_conf()->mon_osd_laggy_weight);
            }
            xi.laggy_probability =
                g_conf()->mon_osd_laggy_weight + xi.laggy_probability * (1.0 - g_conf()->mon_osd_laggy_weight);
        ...
        if ((g_conf()->mon_osd_auto_mark_auto_out_in && (oldstate & CEPH_OSD_AUTOOUT)) ||
            (g_conf()->mon_osd_auto_mark_new_in && (oldstate & CEPH_OSD_NEW)) || (g_conf()->mon_osd_auto_mark_in)) {
            if (can_mark_in(from)) {
                if (xi.old_weight > 0) {
...
void OSDMonitor::_booted(MonOpRequestRef op, bool logit)
{
    op->mark_osdmon_event(__func__);
    auto m = op->get_req<MOSDBoot>();
    dout(7) << "_booted " << m->get_orig_source_inst() << " w " << m->sb.weight << " from " << m->sb.current_epoch
            << dendl;

    if (logit) {
        mon.clog->info() << m->get_source() << " " << m->get_orig_source_addrs() << " boot";
    }
...
bool OSDMonitor::preprocess_full(MonOpRequestRef op)
{
    ...
    if ((!osdmap.is_up(from) && osdmap.get_most_recent_addrs(from).legacy_equals(m->get_orig_source_addrs())) ||
        (osdmap.is_up(from) && !osdmap.get_addrs(from).legacy_equals(m->get_orig_source_addrs()))) {
    ...
    if ((osdmap.get_state(from) & mask) == m->state) {
        dout(7) << __func__ << " state already " << state << " for osd." << from << " " << m->get_orig_source_inst()
                << dendl;
...
bool OSDMonitor::prepare_full(MonOpRequestRef op)
{
    ...
    if (cur_state != want_state) {
        ...
        dout(7) << __func__ << " osd." << from << " " << cur_state_set << " = wanted " << want_state_set
                << ", just waiting" << dendl;
    }
...
bool OSDMonitor::preprocess_alive(MonOpRequestRef op)
{
    ...
    if (!osdmap.is_up(from) || !osdmap.get_addrs(from).legacy_equals(m->get_orig_source_addrs())) {
        dout(7) << "preprocess_alive ignoring alive message from down " << m->get_orig_source() << " "
...
bool OSDMonitor::prepare_alive(MonOpRequestRef op)
{
    ...
    dout(7) << "prepare_alive want up_thru " << m->want << " have " << m->version << " from "
...
bool OSDMonitor::prepare_pg_created(MonOpRequestRef op)
{
    ...
    auto from = src.num();
    if (!src.is_osd() || !mon.osdmon()->osdmap.is_up(from) ||
        !mon.osdmon()->osdmap.get_addrs(from).legacy_equals(m->get_orig_source_addrs())) {
...
bool OSDMonitor::prepare_pg_ready_to_merge(MonOpRequestRef op)
{
    ...
    if (p.get_pg_num() != m->pgid.ps() + 1 || p.get_pg_num_pending() > m->pgid.ps()) {
    ...
    if (m->ready && prob > 0 && prob > (double)(rand() % 1000) / 1000.0) {
        ...
        n->cmd = {"{\"prefix\":\"osd pool set\", \"pool\": \"" + osdmap.get_pool_name(m->pgid.pool()) +
                  "\", \"var\": \"pg_num_actual\", \"val\": \"" + stringify(m->pgid.ps() + 1) + "\"}"};
...
bool OSDMonitor::preprocess_pgtemp(MonOpRequestRef op)
{
    ...
    if (!osdmap.is_up(from) || !osdmap.get_addrs(from).legacy_equals(m->get_orig_source_addrs())) {
        dout(7) << "ignoring pgtemp message from down " << m->get_orig_source() << " " << m->get_orig_source_addrs()
                << dendl;
        goto ignore;
    }
    ...
    for (auto p = m->pg_temp.begin(); p != m->pg_temp.end(); ++p) {
        dout(20) << " " << p->first << (osdmap.pg_temp->count(p->first) ? osdmap.pg_temp->get(p->first) : empty)
                 << " -> " << p->second << dendl;
        ...
        osdmap.pg_to_up_acting_osds(p->first, nullptr, nullptr, nullptr, &acting_primary);
        ...
        // removal?
        if (p->second.empty() && (osdmap.pg_temp->count(p->first) || osdmap.primary_temp->count(p->first)))
            return false;
        // change?
        //  NOTE: we assume that this will clear pg_primary, so consider
        //        an existing pg_primary field to imply a change
        if (p->second.size() && (osdmap.pg_temp->count(p->first) == 0 || osdmap.pg_temp->get(p->first) != p->second ||
                                 osdmap.primary_temp->count(p->first)))
            return false;
    }

    // should we ignore all the pgs?
    if (ignore_cnt == m->pg_temp.size()) goto ignore;
...
bool OSDMonitor::prepare_pgtemp(MonOpRequestRef op)
{
    ...
    for (map<pg_t, vector<int32_t>>::iterator p = m->pg_temp.begin(); p != m->pg_temp.end(); ++p) {
        ...
        pending_inc.new_pg_temp[p->first] = mempool::osdmap::vector<int>(p->second.begin(), p->second.end());

        // unconditionally clear pg_primary (until this message can encode
        // a change for that, too.. at which point we need to also fix
        // preprocess_pg_temp)
        if (osdmap.primary_temp->count(p->first) || pending_inc.new_primary_temp.count(p->first))
...
bool OSDMonitor::preprocess_remove_snaps(MonOpRequestRef op)
{
    ...
    mon.no_reply(op);
    if (!session) goto ignore;
    if (!session->caps.is_capable(cct,
                                  session->entity_name,
                                  "osd",
                                  "osd pool rmsnap",
                                  {},
    ...
    for (map<int, vector<snapid_t>>::iterator q = m->snaps.begin(); q != m->snaps.end(); ++q) {
        ...
        for (vector<snapid_t>::iterator p = q->second.begin(); p != q->second.end(); ++p) {
            if (*p > pi->get_snap_seq() || !_is_removed_snap(q->first, *p)) {
                return false;
            }
        }
    }
    ...
    for (auto& [pool, snaps] : m->snaps) {
        ...
        for (auto s : snaps) {
            if (!_is_removed_snap(pool, s) &&
                (!pending_inc.new_pools.count(pool) || !pending_inc.new_pools[pool].removed_snaps.contains(s)) &&
                (!pending_inc.new_removed_snaps.count(pool) || !pending_inc.new_removed_snaps[pool].contains(s))) {
                pg_pool_t* newpi = pending_inc.get_new_pool(pool, &pi);
                if (osdmap.require_osd_release < ceph_release_t::octopus) {
                    newpi->removed_snaps.insert(s);
                    dout(10) << " pool " << pool << " removed_snaps added " << s << " (now " << newpi->removed_snaps
                             << ")" << dendl;
                }
...
bool OSDMonitor::prepare_beacon(MonOpRequestRef op)
{
    ...
    if (osdmap.osd_xinfo[from].last_purged_snaps_scrub < beacon->last_purged_snaps_scrub) {
        if (pending_inc.new_xinfo.count(from) == 0) {
            pending_inc.new_xinfo[from] = osdmap.osd_xinfo[from];
        }
        pending_inc.new_xinfo[from].last_purged_snaps_scrub = beacon->last_purged_snaps_scrub;
        return true;
    }
    else {
        return false; /* nothing to propose */
...
{
    dout(10) << "build_incremental [" << from << ".." << to << "] with features " << std::hex << features << std::dec
             << dendl;
...
void OSDMonitor::send_incremental(epoch_t first, MonSession* session, bool onetime, MonOpRequestRef req)
{
    ...
    while (first <= osdmap.get_epoch()) {
        epoch_t last = std::min<epoch_t>(first + g_conf()->osd_map_message_max - 1, osdmap.get_epoch());
...
int OSDMonitor::get_version(version_t ver, uint64_t features, bufferlist& bl)
{
    ...
    // feature masks.
    if (significant_features != OSDMap::get_significant_features(mon.get_quorum_con_features())) {
...
int OSDMonitor::get_inc(version_t ver, OSDMap::Incremental& inc)
{
    ...
    inc.decode(p);
    dout(10) << __func__ << "     "
             << " epoch " << inc.epoch << " inc_crc " << inc.inc_crc << " full_crc " << inc.full_crc
...
int OSDMonitor::get_full_from_pinned_map(version_t ver, bufferlist& bl)
{
    ...
    for (version_t v = ver - 1; v >= closest_pinned; --v) {
        if (full_osd_cache.lookup({v, mon.get_quorum_con_features()}, &osdm_bl)) {
    ...
    dout(10) << __func__ << " loaded osdmap epoch " << closest_pinned << " e" << osdm.epoch << " crc " << osdm.get_crc()
    ...
    for (version_t v = closest_pinned + 1; v <= ver; ++v) {
        ...
        if (g_conf().get_val<bool>("mon_debug_extra_checks") && inc.full_crc != 0) {
            ...
            if (tosdm.get_crc() != inc.full_crc) {
                derr << __func__ << "    osdmap crc mismatch! (osdmap crc " << tosdm.get_crc() << ", expected "
                     << inc.full_crc << ")" << dendl;
    ...
    if (!encode_features) {
        ...
        encode_features = (mon.quorum_con_features ? mon.quorum_con_features : -1);
    }
...
{
    dout(10) << __func__ << " " << sub << " next " << sub->next << (sub->onetime ? " (onetime)" : " (ongoing)")
             << dendl;
...
void OSDMonitor::check_pg_creates_sub(Subscription* sub)
{
    ...
    if (sub->session->name.is_osd() && mon.osdmon()->osdmap.is_up(sub->session->name.num())) {
        sub->next = send_pg_creates(sub->session->name.num(), sub->session->con.get(), sub->next);
    }
}

void OSDMonitor::do_application_enable(int64_t pool_id, const std::string& app_name, const std::string& app_key,
                                       const std::string& app_value, bool force)
{
...
void OSDMonitor::do_set_pool_opt(int64_t pool_id, pool_opts_t::key_t opt, pool_opts_t::value_t val)
{
    dout(10) << __func__ << " pool: " << pool_id << " option: " << opt << " val: " << val << dendl;
    auto p = pending_inc.new_pools.try_emplace(pool_id, *osdmap.get_pg_pool(pool_id));
    p.first->second.opts.set(opt, val);
}

unsigned OSDMonitor::scan_for_creating_pgs(const mempool::osdmap::map<int64_t, pg_pool_t>& pools,
                                           const mempool::osdmap::set<int64_t>& removed_pools, utime_t modified,
                                           creating_pgs_t* creating_pgs) const
{
    ...
    for (auto& p : pools) {
        ...
        if (ruleno < 0 || !osdmap.crush->rule_exists(ruleno)) continue;
...
{
    dout(10) << __func__ << " " << creating_pgs.pgs.size() << " pgs creating, " << creating_pgs.queue.size()
             << " pools in queue" << dendl;
    ...
    for (const auto& pg : creating_pgs.pgs) {
                ...
                if (pgs.second.count(spgid)) {
                    if (last_acting_primary == acting_primary) {
                        ...
                        dout(20) << __func__ << " " << pgid << " "
                                 << " acting_primary:" << last_acting_primary << " -> " << acting_primary << dendl;
        ...
        dout(10) << __func__ << " will instruct osd." << acting_primary << " to create " << pgid << "@" << mapped
                 << dendl;
...
epoch_t OSDMonitor::send_pg_creates(int osd, Connection* con, epoch_t next) const
{
    ...
    if (creating_pgs_by_epoch == creating_pgs_by_osd_epoch.end()) return next;
    ...
    for (auto epoch_pgs = creating_pgs_by_epoch->second.lower_bound(next);
         epoch_pgs != creating_pgs_by_epoch->second.end();
         ++epoch_pgs) {
        auto epoch = epoch_pgs->first;
        auto& pgs = epoch_pgs->second;
        dout(20) << __func__ << " osd." << osd << " from " << next << " : epoch " << epoch << " " << pgs.size()
                 << " pgs" << dendl;
        last = epoch;
        for (auto& pg : pgs) {
            ...
            m->pgs.emplace(pg, make_pair(create->second.create_epoch, create->second.create_stamp));
            if (create->second.history.epoch_created) {
                dout(20) << __func__ << "   " << pg << " " << create->second.history << " "
                         << create->second.past_intervals << dendl;
                m->pg_extra.emplace(pg, make_pair(create->second.history, create->second.past_intervals));
            }
...
void OSDMonitor::tick()
{
    ...
    if (ceph_using_tcmalloc() && mon_memory_autotune) {
        ...
        if (pcm != nullptr) {
            ...
            dout(10) << "tick balancer "
                     ...
                     << " inc num_osdmaps: " << inc_cache->_get_num_osdmaps() << dendl;
            dout(10) << "tick balancer "
                     ...
                     << " full num_osdmaps: " << full_cache->_get_num_osdmaps() << dendl;
        }
    }
    ...
    if (can_mark_out(-1)) {
        ...
        while (i != down_pending_out.end()) {
            ...
            if (osdmap.is_down(o) && osdmap.is_in(o) && can_mark_out(o)) {
                ...
                if (g_conf()->mon_osd_adjust_down_out_interval) {
                    ...
                    dout(20) << "osd." << o << " laggy halflife " << halflife << " decay_k " << decay_k << " down for "
                ...
                if (down_out_subtree_limit.length()) {
                    ...
                    if (type > 0) {
                        if (osdmap.containing_subtree_is_down(cct, o, type, &down_cache)) {
                            dout(10) << "tick entire containing " << down_out_subtree_limit << " subtree for osd." << o
                ...
                bool down_out =
                    !osdmap.is_destroyed(o) && g_conf()->mon_osd_down_out_interval > 0 && down.sec() >= grace;
                bool destroyed_out = osdmap.is_destroyed(o) && g_conf()->mon_osd_destroyed_out_interval > 0 &&
                ...
                if (down_out || destroyed_out) {
                    dout(10) << "tick marking osd." << o << " OUT after " << down << " sec (target " << grace << " = "
                             << orig_grace << " + " << my_grace << ")" << dendl;
                    pending_inc.new_weight[o] = CEPH_OSD_OUT;

                    // set the AUTOOUT bit.
                    if (pending_inc.new_state.count(o) == 0) pending_inc.new_state[o] = 0;
                    pending_inc.new_state[o] |= CEPH_OSD_AUTOOUT;

                    // remember previous weight
                    if (pending_inc.new_xinfo.count(o) == 0) pending_inc.new_xinfo[o] = osdmap.osd_xinfo[o];
                    pending_inc.new_xinfo[o].old_weight = osdmap.osd_weight[o];

                    do_propose = true;

                    mon.clog->info() << "Marking osd." << o << " out (has been down for " << int(down.sec())
                                     << " seconds)";
                }
                else
                    continue;
    ...
    for (auto p = osdmap.range_blocklist.begin(); p != osdmap.range_blocklist.end(); ++p) {
        if (p->second < now) {
            dout(10) << "expiring range_blocklist item " << p->first << " expired " << p->second << " < now " << now
                     << dendl;
    ...
    if (update_pools_status()) do_propose = true;
...
void OSDMonitor::_set_new_cache_sizes()
{
    ...
    dout(1) << __func__ << " cache_size:" << cache_size << " inc_alloc: " << inc_alloc << " full_alloc: " << full_alloc
            << " kv_alloc: " << kv_alloc << dendl;
}

bool OSDMonitor::handle_osd_timeouts(const utime_t& now, std::map<int, std::pair<utime_t, int>>& last_osd_report)
{
    ...
    for (int i = 0; i < max_osd; ++i) {
        ...
        if (t == last_osd_report.end()) {
            ...
            if (diff > max_timeout) {
                mon.clog->info() << "osd." << i << " marked down after no beacon for " << diff << " seconds";
                derr << "no beacon from osd." << i << " since " << t->second.first << ", " << diff
...
static void dump_cpu_list(Formatter* f, const char* name, const string& strlist)
{
    cpu_set_t cpu_set;
    size_t cpu_set_size;
...
std::set<osd_pool_get_choices> subtract_second_from_first(const std::set<osd_pool_get_choices>& first,
...
bool OSDMonitor::preprocess_command(MonOpRequestRef op)
{
    ...
    if (prefix == "osd stat") {
    ...
    else if (prefix == "osd dump" || prefix == "osd tree" || prefix == "osd tree-from" || prefix == "osd ls" ||
             prefix == "osd getmap" || prefix == "osd getcrushmap" || prefix == "osd ls-tree" || prefix == "osd info") {
        ...
        if (prefix == "osd dump") {
            ...
            if (((filter & (OSDMap::DUMP_UP | OSDMap::DUMP_DOWN)) == (OSDMap::DUMP_UP | OSDMap::DUMP_DOWN)) ||
                ((filter & (OSDMap::DUMP_UP | OSDMap::DUMP_DESTROYED)) == (OSDMap::DUMP_UP | OSDMap::DUMP_DESTROYED)) ||
        ...
        if (!cmd_getval(cmdmap, "id", osd)) {
            ss << "unable to parse osd id value '" << cmd_vartype_stringify(cmdmap["id"]) << "'";
            r = -EINVAL;
            goto reply;
        }
        ...
        int64_t osd = -1;
        if (cmd_vartype_stringify(cmdmap["id"]).size() && !cmd_getval(cmdmap, "id", osd)) {
            ss << "unable to parse osd id value '" << cmd_vartype_stringify(cmdmap["id"]) << "'";
            r = -EINVAL;
            goto reply;
        }
        ...
        if (!f) f.reset(Formatter::create("json-pretty"));
        ...
        for (int i = 0; i < osdmap.get_max_osd(); ++i) {
            if (osdmap.exists(i)) {
                ...
                if (f) {
                    ...
                    for (auto n : {"network_numa_node", "objectstore_numa_node", "numa_node"}) {
                        p = m.find(n);
                        if (p != m.end()) {
        ...
        if (f) {
            ...
            for (vector<int>::iterator p = up.begin(); p != up.end(); ++p) f->dump_int("osd", *p);
            f->close_section();
            f->dump_int("up_primary", up_p);
            f->open_array_section("acting");
            for (vector<int>::iterator p = acting.begin(); p != acting.end(); ++p) f->dump_int("osd", *p);
            f->close_section();
            ...
            ds << "osdmap e" << osdmap.get_epoch() << " pool '" << poolstr << "' (" << pool << ")"
               << " object '" << fullobjname << "' ->"
               << " pg " << pgid << " (" << mpgid << ")"
               << " -> up (" << pg_vector_string(up) << ", p" << up_p << ") acting (" << pg_vector_string(acting)
               << ", p" << acting_p << ")";
            rdata.append(ds);
        }
    }
        ...
        if (f) {
            ...
            ds << "osdmap e" << osdmap.get_epoch() << " pg " << pgid << " (" << mpgid << ")"
               << " -> up " << up << " acting " << acting;
            rdata.append(ds);
        }
        goto reply;
        ...
        if (f) f->open_array_section("pools");
    ...
    else if (prefix == "osd blocklist ls" || prefix == "osd blacklist ls") {
        if (f) f->open_array_section("blocklist");
        ...
        if (f) f->open_array_section("range_blocklist");
        ...
        ostringstream rss;
        if (f) f->open_object_section("tunable");
        ...
        const choices_map_t ALL_CHOICES = {{"size", SIZE},
                                           {"min_size", MIN_SIZE},
                                           {"pg_num", PG_NUM},
                                           {"pgp_num", PGP_NUM},
        ...
        const choices_set_t ONLY_TIER_CHOICES = {HIT_SET_TYPE,
                                                 HIT_SET_PERIOD,
                                                 HIT_SET_COUNT,
                                                 HIT_SET_FPP,
                                                 TARGET_MAX_OBJECTS,
                                                 ...
                                                 HIT_SET_GRADE_DECAY_RATE,
                                                 HIT_SET_SEARCH_LAST_N};
        const choices_set_t ONLY_ERASURE_CHOICES = {EC_OVERWRITES, ERASURE_CODE_PROFILE};
        ...
        if (var == "all") {
            ...
            if (!p->is_tier()) {
                selected_choices = subtract_second_from_first(selected_choices, ONLY_TIER_CHOICES);
            }

            if (!p->is_erasure()) {
                selected_choices = subtract_second_from_first(selected_choices, ONLY_ERASURE_CHOICES);
            }
        }
        else /* var != "all" */ {
            choices_map_t::const_iterator found = ALL_CHOICES.find(var);
            if (found == ALL_CHOICES.end()) {
                ss << "pool '" << poolstr << "': invalid variable: '" << var << "'";
                r = -EINVAL;
                goto reply;
            }
            ...
            if (!p->is_erasure() && ONLY_ERASURE_CHOICES.find(selected) != ONLY_ERASURE_CHOICES.end()) {
                ss << "pool '" << poolstr << "' is not a erasure pool: variable not applicable";
                r = -EACCES;
                goto reply;
            }
        ...
        if (f) {
            ...
            for (choices_set_t::const_iterator it = selected_choices.begin(); it != selected_choices.end(); ++it) {
                ...
                switch (*it) {
                case PG_NUM: f->dump_int("pg_num", p->get_pg_num()); break;
                case PGP_NUM: f->dump_int("pgp_num", p->get_pgp_num()); break;
                case SIZE: f->dump_int("size", p->get_size()); break;
                case MIN_SIZE: f->dump_int("min_size", p->get_min_size()); break;
                case CRUSH_RULE:
                ...
                case NOSCRUB:
                case NODEEP_SCRUB:
                    f->dump_bool(i->first.c_str(), p->has_flag(pg_pool_t::get_flag_by_name(i->first)));
                    break;
                case HIT_SET_PERIOD: f->dump_int("hit_set_period", p->hit_set_period); break;
                case HIT_SET_COUNT: f->dump_int("hit_set_count", p->hit_set_count); break;
                case HIT_SET_TYPE:
                ...
                case USE_GMT_HITSET: f->dump_bool("use_gmt_hitset", p->use_gmt_hitset); break;
                case TARGET_MAX_OBJECTS: f->dump_unsigned("target_max_objects", p->target_max_objects); break;
                case TARGET_MAX_BYTES: f->dump_unsigned("target_max_bytes", p->target_max_bytes); break;
                case CACHE_TARGET_DIRTY_RATIO:
                    f->dump_unsigned("cache_target_dirty_ratio_micro", p->cache_target_dirty_ratio_micro);
                ...
                case CACHE_TARGET_FULL_RATIO:
                    ...
                    break;
                case CACHE_MIN_FLUSH_AGE: f->dump_unsigned("cache_min_flush_age", p->cache_min_flush_age); break;
                case CACHE_MIN_EVICT_AGE: f->dump_unsigned("cache_min_evict_age", p->cache_min_evict_age); break;
                case ERASURE_CODE_PROFILE: f->dump_string("erasure_code_profile", p->erasure_code_profile); break;
                ...
                case MIN_WRITE_RECENCY_FOR_PROMOTE:
                    ...
                    break;
                case FAST_READ: f->dump_int("fast_read", p->fast_read); break;
                case HIT_SET_GRADE_DECAY_RATE:
                    f->dump_int("hit_set_grade_decay_rate", p->hit_set_grade_decay_rate);
                    break;
                case HIT_SET_SEARCH_LAST_N: f->dump_int("hit_set_search_last_n", p->hit_set_search_last_n); break;
            ...
            for (choices_set_t::const_iterator it = selected_choices.begin(); it != selected_choices.end(); ++it) {
                choices_map_t::const_iterator i;
                switch (*it) {
                case PG_NUM: ss << "pg_num: " << p->get_pg_num() << "\n"; break;
                case PGP_NUM: ss << "pgp_num: " << p->get_pgp_num() << "\n"; break;
                case SIZE: ss << "size: " << p->get_size() << "\n"; break;
                case MIN_SIZE: ss << "min_size: " << p->get_min_size() << "\n"; break;
                case CRUSH_RULE:
                    if (osdmap.crush->rule_exists(p->get_crush_rule())) {
                    ...
                    break;
                case PG_AUTOSCALE_MODE:
                    ss << "pg_autoscale_mode: " << pg_pool_t::get_pg_autoscale_mode_name(p->pg_autoscale_mode) << "\n";
                    break;
                case HIT_SET_PERIOD: ss << "hit_set_period: " << p->hit_set_period << "\n"; break;
                case HIT_SET_COUNT: ss << "hit_set_count: " << p->hit_set_count << "\n"; break;
                case HIT_SET_TYPE:
                ...
                case USE_GMT_HITSET: ss << "use_gmt_hitset: " << p->use_gmt_hitset << "\n"; break;
                case TARGET_MAX_OBJECTS: ss << "target_max_objects: " << p->target_max_objects << "\n"; break;
                case TARGET_MAX_BYTES: ss << "target_max_bytes: " << p->target_max_bytes << "\n"; break;
                case CACHE_TARGET_DIRTY_RATIO:
                    ss << "cache_target_dirty_ratio: " << ((float)p->cache_target_dirty_ratio_micro / 1000000) << "\n";
                    break;
                case CACHE_TARGET_DIRTY_HIGH_RATIO:
                    ss << "cache_target_dirty_high_ratio: " << ((float)p->cache_target_dirty_high_ratio_micro / 1000000)
                       << "\n";
                    break;
                case CACHE_TARGET_FULL_RATIO:
                    ss << "cache_target_full_ratio: " << ((float)p->cache_target_full_ratio_micro / 1000000) << "\n";
                    break;
                case CACHE_MIN_FLUSH_AGE: ss << "cache_min_flush_age: " << p->cache_min_flush_age << "\n"; break;
                case CACHE_MIN_EVICT_AGE: ss << "cache_min_evict_age: " << p->cache_min_evict_age << "\n"; break;
                case ERASURE_CODE_PROFILE: ss << "erasure_code_profile: " << p->erasure_code_profile << "\n"; break;
                case MIN_READ_RECENCY_FOR_PROMOTE:
                    ss << "min_read_recency_for_promote: " << p->min_read_recency_for_promote << "\n";
                    break;
                case HIT_SET_GRADE_DECAY_RATE:
                    ss << "hit_set_grade_decay_rate: " << p->hit_set_grade_decay_rate << "\n";
                    break;
                case HIT_SET_SEARCH_LAST_N: ss << "hit_set_search_last_n: " << p->hit_set_search_last_n << "\n"; break;
                case EC_OVERWRITES:
                    ss << "allow_ec_overwrites: " << (p->has_flag(pg_pool_t::FLAG_EC_OVERWRITES) ? "true" : "false")
                       << "\n";
                    break;
                case HASHPSPOOL:
                case POOL_EIO:
                ...
                case MIN_WRITE_RECENCY_FOR_PROMOTE:
                    ...
                    break;
                case FAST_READ: ss << "fast_read: " << p->fast_read << "\n"; break;
                case SCRUB_MIN_INTERVAL:
                case SCRUB_MAX_INTERVAL:
                case DEEP_SCRUB_INTERVAL:
                case RECOVERY_PRIORITY:
    ...
    else if (prefix == "osd crush rule list" || prefix == "osd crush rule ls") {
        if (f) {
        ...
        if (!cmd_getval_compat_cephbool(cmdmap, "show_shadow", show_shadow)) {
            std::string shadow;
            if (cmd_getval(cmdmap, "shadow", shadow) && shadow == "--show-shadow") {
                show_shadow = true;
            }
        }
        boost::scoped_ptr<Formatter> f(Formatter::create(format));
        if (f) {
            ...
            osdmap.crush->dump_tree(nullptr, f.get(), osdmap.get_pool_names(), show_shadow);
            f->close_section();
            f->flush(rdata);
        }
        else {
            ostringstream ss;
            osdmap.crush->dump_tree(&ss, nullptr, osdmap.get_pool_names(), show_shadow);
            rdata.append(ss.str());
        }
    }
        ...
        for (auto i : osdmap.crush->class_name) f->dump_string("class", i.second);
        f->close_section();
        f->flush(rdata);
    }
        ...
        if (f) {
            f->open_array_section("osds");
            for (auto& osd : osds) f->dump_int("osd", osd);
            f->close_section();
            f->flush(rdata);
        }
        else {
        ...
        if (f) {
            ...
            if (class_by_osd.size() == 1) {
                ...
                for (auto it = class_by_osd.begin(); it != class_by_osd.end(); it++) {
                    ...
                    if (next(it) != class_by_osd.end()) ds << '\n';
                }
            }
            rdata.append(ds);
        }
        ...
        if (f) f->open_array_section("erasure-code-profiles");
        ...
        if (f) f->open_object_section("profile");
        ...
        if (pool_name.empty()) {
            ...
            for (const auto& pool : osdmap.pools) {
                ...
                if (pni != osdmap.pool_name.end()) name = pni->second;
            ...
            if (key_it == app_it->second.end()) {
                ss << "application '" << app << "' on pool '" << pool_name << "' does not have key '" << key << "'";
                r = -ENOENT;
                goto reply;
            }
            ss << key_it->second << "\n";
    ...
    else if (prefix == "osd pool application enable" || prefix == "osd pool application disable" ||
             prefix == "osd pool application set" || prefix == "osd pool application rm") {
        bool changed = false;
...
{
    pg_pool_t* pool = pending_inc.get_new_pool(pool_id, osdmap.get_pg_pool(pool_id));
    ceph_assert(pool);
    pool->set_flag(flags);
}
...
{
    char k[80];
    snprintf(k, sizeof(k), "purged_snap_%llu_%016llx", (unsigned long long)pool, (unsigned long long)snap);
    return k;
}

string OSDMonitor::make_purged_snap_key_value(int64_t pool, snapid_t snap, snapid_t num, epoch_t epoch, bufferlist* v)
{
...
int OSDMonitor::lookup_purged_snap(int64_t pool, snapid_t snap, snapid_t* begin, snapid_t* end)
{
    ...
    if (it->key().find("purged_snap_") != 0) {
        dout(20) << __func__ << " pool " << pool << " snap " << snap << " - key '" << k << "' got '" << it->key()
                 << "', wrong prefix" << dendl;
        return -ENOENT;
    }
    string gotk = it->key();
    ...
    if (pool != keypool) {
        dout(20) << __func__ << " pool " << pool << " snap " << snap << " - key '" << k << "' got '" << gotk
                 << "', wrong pool " << keypool << dendl;
        return -ENOENT;
    }
    bufferlist v = it->value();
    auto p = v.cbegin();
    ...
    if (snap < *begin || snap >= *end) {
        dout(20) << __func__ << " pool " << pool << " snap " << snap << " - found [" << *begin << "," << *end
                 << "), no overlap" << dendl;
        return -ENOENT;
    }
    return 0;
...
void OSDMonitor::insert_purged_snap_update(int64_t pool, snapid_t start, snapid_t end, epoch_t epoch,
                                           MonitorDBStore::TransactionRef t)
{
    snapid_t before_begin, before_end;
    snapid_t after_begin, after_end;
    int b = lookup_purged_snap(pool, start - 1, &before_begin, &before_end);
    int a = lookup_purged_snap(pool, end, &after_begin, &after_end);
    if (!b && !a) {
        dout(10) << __func__ << " [" << start << "," << end << ") - joins [" << before_begin << "," << before_end
                 << ") and [" << after_begin << "," << after_end << ")" << dendl;
        // erase only the begin record; we'll overwrite the end one.
        t->erase(OSD_SNAP_PREFIX, make_purged_snap_key(pool, before_end - 1));
        bufferlist v;
        string k = make_purged_snap_key_value(pool, before_begin, after_end - before_begin, pending_inc.epoch, &v);
        t->put(OSD_SNAP_PREFIX, k, v);
    }
    else if (!b) {
        dout(10) << __func__ << " [" << start << "," << end << ") - join with earlier [" << before_begin << ","
                 << before_end << ")" << dendl;
        t->erase(OSD_SNAP_PREFIX, make_purged_snap_key(pool, before_end - 1));
        bufferlist v;
        string k = make_purged_snap_key_value(pool, before_begin, end - before_begin, pending_inc.epoch, &v);
        t->put(OSD_SNAP_PREFIX, k, v);
    }
    else if (!a) {
        dout(10) << __func__ << " [" << start << "," << end << ") - join with later [" << after_begin << ","
                 << after_end << ")" << dendl;
        // overwrite after record
        bufferlist v;
        string k = make_purged_snap_key_value(pool, start, after_end - start, pending_inc.epoch, &v);
        t->put(OSD_SNAP_PREFIX, k, v);
    }
    else {
        dout(10) << __func__ << " [" << start << "," << end << ") - new" << dendl;
        bufferlist v;
        string k = make_purged_snap_key_value(pool, start, end - start, pending_inc.epoch, &v);
        t->put(OSD_SNAP_PREFIX, k, v);
    }
}
...
bool OSDMonitor::try_prune_purged_snaps()
{
    ...
    unsigned max_prune = cct->_conf.get_val<uint64_t>("mon_max_snap_prune_per_epoch");
    if (!max_prune) {
        max_prune = 100000;
    }
    ...
    for (auto& p : osdmap.get_pools()) {
        ...
        if (!to_prune.empty()) {
            ...
            dout(10) << __func__ << " pool " << p.first << " reports pruned " << to_prune << ", actual pruned "
                     << actual << dendl;
            if (!actual.empty()) {
...
{
    if (!mon.mgrstatmon()->is_readable()) return false;
    ...
    for (auto it = pools.begin(); it != pools.end(); ++it) {
        ...
        bool pool_is_full = (pool.quota_max_bytes > 0 && (uint64_t)sum.num_bytes >= pool.quota_max_bytes) ||
                            (pool.quota_max_objects > 0 && (uint64_t)sum.num_objects >= pool.quota_max_objects);

        if (pool.has_flag(pg_pool_t::FLAG_FULL_QUOTA)) {
            if (pool_is_full) continue;

            mon.clog->info() << "pool '" << pool_name << "' no longer out of quota; removing NO_QUOTA flag";
            // below we cancel FLAG_FULL too, we'll set it again in
            // OSDMonitor::encode_pending if it still fails the osd-full checking.
            clear_pool_flags(it->first, pg_pool_t::FLAG_FULL_QUOTA | pg_pool_t::FLAG_FULL);
            ret = true;
        }
        else {
            if (!pool_is_full) continue;

            if (pool.quota_max_bytes > 0 && (uint64_t)sum.num_bytes >= pool.quota_max_bytes) {
                mon.clog->warn() << "pool '" << pool_name << "' is full"
                                 << " (reached quota's max_bytes: " << byte_u_t(pool.quota_max_bytes) << ")";
            }
            if (pool.quota_max_objects > 0 && (uint64_t)sum.num_objects >= pool.quota_max_objects) {
                mon.clog->warn() << "pool '" << pool_name << "' is full"
                                 << " (reached quota's max_objects: " << pool.quota_max_objects << ")";
            }
            ...
            set_pool_flags(it->first, pg_pool_t::FLAG_FULL_QUOTA | pg_pool_t::FLAG_FULL);
            clear_pool_flags(it->first, pg_pool_t::FLAG_NEARFULL | pg_pool_t::FLAG_BACKFILLFULL);
            ret = true;
        }
    }
    return ret;
...
int OSDMonitor::prepare_new_pool(MonOpRequestRef op)
{
    ...
    MonSession* session = op->get_session();
    if (!session) return -EPERM;
    string erasure_code_profile;
    stringstream ss;
    string rule_name;
    bool bulk = false;
    int ret = 0;
    ret = prepare_new_pool(m->name,
                           m->crush_rule,
                           rule_name,
                           0,
                           0,
...
int OSDMonitor::crush_rename_bucket(const string& srcname, const string& dstname, ostream* ss)
{
    int ret;
    //
    ...
    if (!_have_pending_crush()) {
        ret = _get_stable_crush().can_rename_bucket(srcname, dstname, ss);
        if (ret) return ret;
    }

    CrushWrapper newcrush = _get_pending_crush();

    ret = newcrush.rename_bucket(srcname, dstname, ss);
    if (ret) return ret;
...
void OSDMonitor::check_legacy_ec_plugin(const string& plugin, const string& profile) const
{
    ...
    if (plugin == "jerasure_generic" || plugin == "jerasure_sse3" || plugin == "jerasure_sse4" ||
        plugin == "jerasure_neon") {
        replacement = "jerasure";
    }
    else if (plugin == "shec_generic" || plugin == "shec_sse3" || plugin == "shec_sse4" || plugin == "shec_neon") {
        replacement = "shec";
    }

    if (replacement != "") {
        dout(0) << "WARNING: erasure coding profile " << profile << " uses plugin " << plugin
                << " that has been deprecated. Please use " << replacement << " instead." << dendl;
    }
}

int OSDMonitor::normalize_profile(const string& profilename, ErasureCodeProfile& profile, bool force, ostream* ss)
{
    ...
    int err =
        instance.factory(plugin->second, g_conf().get_val<std::string>("erasure_code_dir"), profile, &erasure_code, ss);
    if (err) {
        return err;
    }
    ...
    if (it != profile.end()) {
        ...
        if (!err_str.empty()) {
            *ss << "could not parse stripe_unit '" << it->second << "': " << err_str << std::endl;
            return -EINVAL;
        }
        ...
        if (chunk_size != stripe_unit) {
            *ss << "stripe_unit " << stripe_unit << " does not match ec profile "
                << "alignment. Would be padded to " << chunk_size << std::endl;
            return -EINVAL;
        }
...
int OSDMonitor::crush_rule_create_erasure(const string& name, const string& profile, int* rule, ostream* ss)
{
...
int OSDMonitor::get_erasure_code(const string& erasure_code_profile, ErasureCodeInterfaceRef* erasure_code,
                                 ostream* ss) const
{
    if (pending_inc.has_erasure_code_profile(erasure_code_profile)) return -EAGAIN;
    ...
    if (plugin == profile.end()) {
        *ss << "cannot determine the erasure code plugin"
            << " because there is no 'plugin' entry in the erasure_code_profile " << profile << std::endl;
        return -EINVAL;
    }
    check_legacy_ec_plugin(plugin->second, erasure_code_profile);
    auto& instance = ErasureCodePluginRegistry::instance();
    return instance.factory(
        plugin->second, g_conf().get_val<std::string>("erasure_code_dir"), profile, erasure_code, ss);
}

int OSDMonitor::check_cluster_features(uint64_t features, stringstream& ss)
{
    ...
    for (set<int32_t>::iterator it = up_osds.begin(); it != up_osds.end(); ++it) {
        const osd_xinfo_t& xi = osdmap.get_xinfo(*it);
        if ((xi.features & features) != features) {
            if (unsupported_count > 0) unsupported_ss << ", ";
    ...
    if (unsupported_count > 0) {
        ss << "features " << features << " unsupported by: " << unsupported_ss.str();
        return -ENOTSUP;
    }

    // check pending osd state, too!
    for (map<int32_t, osd_xinfo_t>::const_iterator p = pending_inc.new_xinfo.begin(); p != pending_inc.new_xinfo.end();
         ++p) {
        const osd_xinfo_t& xi = p->second;
        if ((xi.features & features) != features) {
            dout(10) << __func__ << " pending osd." << p->first << " features are insufficient; retry" << dendl;
            return -EAGAIN;
        }
    }
...
bool OSDMonitor::validate_crush_against_features(const CrushWrapper* newcrush, stringstream& ss)
{
    ...
    if (newmap.require_min_compat_client != ceph_release_t::unknown) {
        ...
        if (mv > newmap.require_min_compat_client) {
            ss << "new crush map requires client version " << mv << " but require_min_compat_client is "
    ...
    // osd compat
    uint64_t features =
        newmap.get_features(CEPH_ENTITY_TYPE_MON, NULL) | newmap.get_features(CEPH_ENTITY_TYPE_OSD, NULL);
...
bool OSDMonitor::erasure_code_profile_in_use(const mempool::osdmap::map<int64_t, pg_pool_t>& pools,
                                             const string& profile, ostream* ss)
{
    bool found = false;
...
int OSDMonitor::parse_erasure_code_profile(const vector<string>& erasure_code_profile,
                                           map<string, string>* erasure_code_profile_map, ostream* ss)
{
    int r = g_conf().with_val<string>(
        "osd_pool_default_erasure_code_profile", get_json_str_map, *ss, erasure_code_profile_map, true);
    if (r) return r;
    ceph_assert((*erasure_code_profile_map).count("plugin"));
    string default_plugin = (*erasure_code_profile_map)["plugin"];
    map<string, string> user_map;
    for (vector<string>::const_iterator i = erasure_code_profile.begin(); i != erasure_code_profile.end(); ++i) {
        size_t equal = i->find('=');
    ...
    if (user_map.count("plugin") && user_map["plugin"] != default_plugin) (*erasure_code_profile_map) = user_map;

    return 0;
}

int OSDMonitor::prepare_pool_size(const unsigned pool_type, const string& erasure_code_profile, uint8_t repl_size,
                                  unsigned* size, unsigned* min_size, ostream* ss)
{
    int err = 0;
    ...
    case pg_pool_t::TYPE_REPLICATED:
        if (osdmap.stretch_mode_enabled) {
            if (repl_size == 0) repl_size = g_conf().get_val<uint64_t>("mon_stretch_pool_size");
            if (repl_size != g_conf().get_val<uint64_t>("mon_stretch_pool_size")) {
                *ss << "prepare_pool_size: we are in stretch mode but size " << repl_size << " does not match!";
                return -EINVAL;
            }
        ...
        *size = repl_size;
        if (!set_min_size) *min_size = g_conf().get_osd_pool_default_min_size(repl_size);
        break;
    case pg_pool_t::TYPE_ERASURE:
    {
        ...
        if (err == 0) {
            ...
            *min_size =
                erasure_code->get_data_chunk_count() + std::min<int>(1, erasure_code->get_coding_chunk_count() - 1);
...
int OSDMonitor::prepare_pool_stripe_width(const unsigned pool_type, const string& erasure_code_profile,
                                          uint32_t* stripe_width, ostream* ss)
{
    int err = 0;
    switch (pool_type) {
    case pg_pool_t::TYPE_REPLICATED:
    ...
    case pg_pool_t::TYPE_ERASURE:
    {
        ...
        *stripe_width = data_chunks * erasure_code->get_chunk_size(stripe_unit * data_chunks);
    } break;
    default:
        *ss << "prepare_pool_stripe_width: " << pool_type << " is not a known pool type";
        err = -EINVAL;
        break;
    }
    return err;
...
int OSDMonitor::prepare_pool_crush_rule(const unsigned pool_type, const string& erasure_code_profile,
                                        const string& rule_name, int* crush_rule, ostream* ss)
{
    ...
    if (*crush_rule < 0) {
        ...
        case pg_pool_t::TYPE_ERASURE:
        {
            int err = crush_rule_create_erasure(rule_name, erasure_code_profile, crush_rule, ss);
            switch (err) {
            case -EALREADY:
            ...
            case 0:
                ...
                err = -EAGAIN;
                break;
            case -EEXIST: err = 0; break;
            }
            return err;
        } break;
        default: *ss << "prepare_pool_crush_rule: " << pool_type << " is not a known pool type"; return -EINVAL;
        }
    }
    else {
...
int OSDMonitor::get_crush_rule(const string& rule_name, int* crush_rule, ostream* ss)
{
    int ret;
...
uint32_t OSDMonitor::get_osd_num_by_crush(int crush_rule)
{
    ...
    for (auto root : roots) {
        ...
        std::set_difference(crush_all_osds.begin(),
                            crush_all_osds.end(),
                            out_osds.begin(),
                            out_osds.end(),
...
int OSDMonitor::check_pg_num(int64_t pool, int pg_num, int size, int crush_rule, ostream* ss)
{
    ...
    if (projected_pgs_per_osd > max_pgs_per_osd) {
        ...
        *ss << " pg_num " << pg_num << " size " << size << " for this pool would result in " << projected_pgs_per_osd
            << " cumulative PGs per OSD (" << projected << " total PG replicas on " << osd_num_by_crush
 ...
 */
int OSDMonitor::prepare_new_pool(string& name, int crush_rule, const string& crush_rule_name, unsigned pg_num,
                                 unsigned pgp_num, unsigned pg_num_min, unsigned pg_num_max, const uint64_t repl_size,
                                 const uint64_t target_size_bytes, const float target_size_ratio,
                                 const string& erasure_code_profile, const unsigned pool_type,
                                 const uint64_t expected_num_objects, FastReadType fast_read, string pg_autoscale_mode,
                                 bool bulk, bool crimson, ostream* ss)
{
    ...
    if (name.length() == 0) return -EINVAL;

    if (pg_num == 0) {
        auto pg_num_from_mode = [pg_num = g_conf().get_val<uint64_t>("osd_pool_default_pg_num")](const string& mode) {
            return mode == "on" ? 1 : pg_num;
        };
        pg_num =
            pg_num_from_mode(pg_autoscale_mode.empty() ? g_conf().get_val<string>("osd_pool_default_pg_autoscale_mode")
                                                       : pg_autoscale_mode);
    }
    if (pgp_num == 0) pgp_num = g_conf().get_val<uint64_t>("osd_pool_default_pgp_num");
    if (!pgp_num) pgp_num = pg_num;
    ...
    int r;
    r = prepare_pool_crush_rule(pool_type, erasure_code_profile, crush_rule_name, &crush_rule, ss);
    if (r) {
        dout(10) << "prepare_pool_crush_rule returns " << r << dendl;
        return r;
    }
    unsigned size, min_size;
    r = prepare_pool_size(pool_type, erasure_code_profile, repl_size, &size, &min_size, ss);
    if (r) {
    ...
    if (pool_type == pg_pool_t::TYPE_ERASURE) {
        switch (fast_read) {
        case FAST_READ_OFF: fread = false; break;
        case FAST_READ_ON: fread = true; break;
        case FAST_READ_DEFAULT: fread = g_conf()->osd_pool_default_ec_fast_read; break;
        default: *ss << "invalid fast_read setting: " << fast_read; return -EINVAL;
        }
    }

    for (map<int64_t, string>::iterator p = pending_inc.new_pool_names.begin(); p != pending_inc.new_pool_names.end();
         ++p) {
        if (p->second == name) return 0;
    }

    if (-1 == pending_inc.new_pool_max) pending_inc.new_pool_max = osdmap.pool_max;
    ...
    if (g_conf()->osd_pool_default_flag_hashpspool) pi->set_flag(pg_pool_t::FLAG_HASHPSPOOL);
    if (g_conf()->osd_pool_default_flag_nodelete) pi->set_flag(pg_pool_t::FLAG_NODELETE);
    if (g_conf()->osd_pool_default_flag_nopgchange) pi->set_flag(pg_pool_t::FLAG_NOPGCHANGE);
    if (g_conf()->osd_pool_default_flag_nosizechange) pi->set_flag(pg_pool_t::FLAG_NOSIZECHANGE);
    ...
    if (auto m =
            pg_pool_t::get_pg_autoscale_mode_by_name(g_conf().get_val<string>("osd_pool_default_pg_autoscale_mode"));
    ...
    pi->set_pg_num(max > 0 ? std::min<uint64_t>(pg_num, std::max<int64_t>(1, max)) : pg_num);
    pi->set_pg_num_pending(pi->get_pg_num());
    pi->set_pg_num_target(pg_num);
    pi->set_pgp_num(pi->get_pg_num());
    pi->set_pgp_num_target(pgp_num);
    if (osdmap.require_osd_release >= ceph_release_t::nautilus && pg_num_min) {
        pi->opts.set(pool_opts_t::PG_NUM_MIN, static_cast<int64_t>(pg_num_min));
    }
    if (osdmap.require_osd_release >= ceph_release_t::quincy && pg_num_max) {
        pi->opts.set(pool_opts_t::PG_NUM_MAX, static_cast<int64_t>(pg_num_max));
    }
    if (auto m = pg_pool_t::get_pg_autoscale_mode_by_name(pg_autoscale_mode);
        m != pg_pool_t::pg_autoscale_mode_t::UNKNOWN) {
        pi->pg_autoscale_mode = m;
    }
    ...
    if (osdmap.require_osd_release >= ceph_release_t::nautilus && target_size_bytes) {
    ...
    if (target_size_ratio > 0.0 && osdmap.require_osd_release >= ceph_release_t::nautilus) {
        // only store for nautilus+, just to be consistent and tidy.
        pi->opts.set(pool_opts_t::TARGET_SIZE_RATIO, target_size_ratio);
    }

    pi->cache_target_dirty_ratio_micro = g_conf()->osd_pool_default_cache_target_dirty_ratio * 1000000;
    pi->cache_target_dirty_high_ratio_micro = g_conf()->osd_pool_default_cache_target_dirty_high_ratio * 1000000;
    pi->cache_target_full_ratio_micro = g_conf()->osd_pool_default_cache_target_full_ratio * 1000000;
...
bool OSDMonitor::prepare_set_flag(MonOpRequestRef op, int flag)
{
    op->mark_osdmon_event(__func__);
    ostringstream ss;
    if (pending_inc.new_flags < 0) pending_inc.new_flags = osdmap.get_flags();
...
int OSDMonitor::prepare_command_pool_set(const cmdmap_t& cmdmap, stringstream& ss)
{
    string poolstr;
    cmd_getval(cmdmap, "pool", poolstr);
    int64_t pool = osdmap.lookup_pg_pool_name(poolstr.c_str());
    ...
    if (pending_inc.new_pools.count(pool)) p = pending_inc.new_pools[pool];
    ...
    if (!p.is_tier() &&
        (var == "hit_set_type" || var == "hit_set_period" || var == "hit_set_count" || var == "hit_set_fpp" ||
         var == "target_max_objects" || var == "target_max_bytes" || var == "cache_target_full_ratio" ||
         var == "cache_target_dirty_ratio" || var == "cache_target_dirty_high_ratio" || var == "use_gmt_hitset" ||
         var == "cache_min_flush_age" || var == "cache_min_evict_age" || var == "hit_set_grade_decay_rate" ||
         var == "hit_set_search_last_n" || var == "min_read_recency_for_promote" ||
    ...
    if (var == "size") {
        ...
        if (n <= 0 || static_cast<uint64_t>(n) > g_conf().get_val<uint64_t>("mon_max_pool_pg_num")) {
            ss << "'pg_num' must be greater than 0 and less than or equal to "
               << g_conf().get_val<uint64_t>("mon_max_pool_pg_num")
               << " (you may adjust 'mon max pool pg num' for higher values)";
            return -ERANGE;
        ...
        if (n > (int)p.get_pg_num_target()) {
            ...
            if (p.cache_mode != pg_pool_t::CACHEMODE_NONE && !force) {
                ss << "splits in cache pools must be followed by scrubs and leave sufficient free space to avoid "
                      "overfilling.  use --yes-i-really-mean-it to force.";
                return -EPERM;
            }
        }
        else {
        ...
        if (pg_min && n < pg_min) {
            ss << "specified pg_num " << n << " < pg_num_min " << pg_min;
            return -EINVAL;
        }
        if (pg_max && n > pg_max) {
            ss << "specified pg_num " << n << " < pg_num_max " << pg_max;
            return -EINVAL;
        }
        if (osdmap.require_osd_release < ceph_release_t::nautilus) {
        ...
        if (n > (int)p.get_pg_num_pending()) {
            ss << "specified pgp_num " << n << " > pg_num_pending " << p.get_pg_num_pending();
            return -EINVAL;
        }
        p.set_pgp_num(n);
    }
    ...
    else if (var == "nodelete" || var == "nopgchange" || var == "nosizechange" || var == "write_fadvise_dontneed" ||
        ...
        if (var == "compression_mode") {
        ...
        else if (var == "compression_max_blob_size" || var == "compression_min_blob_size" || var == "csum_max_block" ||
                 var == "csum_min_block") {
            if (interr.length()) {
            ...
            if (n > (int)p.get_pg_num_target()) {
                ss << "specified pg_num_min " << n << " > pg_num " << p.get_pg_num_target();
                return -EINVAL;
            }
        }
            ...
            if (n && n < (int)p.get_pg_num_target()) {
                ss << "specified pg_num_max " << n << " < pg_num " << p.get_pg_num_target();
                return -EINVAL;
            }
        }
            ...
            if (!g_conf()->debug_allow_any_pool_priority) {
                if (n > OSD_POOL_PRIORITY_MAX || n < OSD_POOL_PRIORITY_MIN) {
                    ss << "pool recovery_priority must be between " << OSD_POOL_PRIORITY_MIN << " and "
            ...
            if (p.is_erasure()) {
                ss << "pool '" << poolstr << "' is an ec pool, which cannot be a base tier";
                return -ENOTSUP;
            }
        ...
        case pool_opts_t::DOUBLE:
            ...
            break;
        default: ceph_assert(!"unknown type");
        }
    }
    else {
...
int OSDMonitor::prepare_command_pool_application(const string& prefix, const cmdmap_t& cmdmap, stringstream& ss)
{
    return _command_pool_application(prefix, cmdmap, ss, nullptr, true);
}

int OSDMonitor::preprocess_command_pool_application(const string& prefix, const cmdmap_t& cmdmap, stringstream& ss,
                                                    bool* modified)
{
 ...
 */
int OSDMonitor::_command_pool_application(const string& prefix, const cmdmap_t& cmdmap, stringstream& ss,
                                          bool* modified, bool preparing)
{
    string pool_name;
    cmd_getval(cmdmap, "pool", pool_name);
    int64_t pool = osdmap.lookup_pg_pool_name(pool_name.c_str());
    ...
    if (boost::algorithm::ends_with(prefix, "enable")) {
        ...
        if (app.length() > MAX_POOL_APPLICATION_LENGTH) {
            ss << "application name '" << app << "' too long; max length " << MAX_POOL_APPLICATION_LENGTH;
            return -EINVAL;
        }
        ...
        if (app_keys.count(key) == 0 && app_keys.size() >= MAX_POOL_APPLICATION_KEYS) {
            ss << "too many keys set for application '" << app << "' on pool '" << pool_name << "'; max "
               << MAX_POOL_APPLICATION_KEYS;
            return -EINVAL;
        }

        if (key.length() > MAX_POOL_APPLICATION_LENGTH) {
            ss << "key '" << app << "' too long; max length " << MAX_POOL_APPLICATION_LENGTH;
            return -EINVAL;
        }
        ...
        if (value.length() > MAX_POOL_APPLICATION_LENGTH) {
            ss << "value '" << value << "' too long; max length " << MAX_POOL_APPLICATION_LENGTH;
            return -EINVAL;
        }

        p.application_metadata[app][key] = value;
        ss << "set application '" << app << "' key '" << key << "' to '" << value << "' on pool '" << pool_name << "'";
    }
        ...
        ss << "removed application '" << app << "' key '" << key << "' on pool '" << pool_name << "'";
    }
    else {
        ceph_abort();
    }
...
int OSDMonitor::_prepare_command_osd_crush_remove(CrushWrapper& newcrush, int32_t id, int32_t ancestor,
                                                  bool has_ancestor, bool unlink_only)
{
    int err = 0;

    if (has_ancestor) {
        err = newcrush.remove_item_under(cct, id, ancestor, unlink_only);
    }
    else {
...
int OSDMonitor::prepare_command_osd_crush_remove(CrushWrapper& newcrush, int32_t id, int32_t ancestor,
                                                 bool has_ancestor, bool unlink_only)
{
    int err = _prepare_command_osd_crush_remove(newcrush, id, ancestor, has_ancestor, unlink_only);
...
int32_t OSDMonitor::_allocate_osd_id(int32_t* existing_id)
{
    ...
    *existing_id = -1;
    ...
    for (int32_t i = 0; i < osdmap.get_max_osd(); ++i) {
        if (!osdmap.exists(i) && pending_inc.new_up_client.count(i) == 0 &&
            (pending_inc.new_state.count(i) == 0 || (pending_inc.new_state[i] & CEPH_OSD_EXISTS) == 0)) {
            *existing_id = i;
            return -1;
        }
    }
...
void OSDMonitor::do_osd_create(const int32_t id, const uuid_d& uuid, const string& device_class, int32_t* new_id)
{
...
out:
    if (device_class.size()) {
        ...
        if (r < 0) {
            derr << __func__ << " failed to set " << name << " device_class " << device_class << ": " << cpp_strerror(r)
                 << " - " << ss.str() << dendl;
    ...
    if (!uuid.is_zero()) pending_inc.new_uuid[*new_id] = uuid;
}

int OSDMonitor::validate_osd_create(const int32_t id, const uuid_d& uuid, const bool check_osd_exists,
                                    int32_t* existing_id, stringstream& ss)
{
    ...
    if (id >= 0) {
        ...
        if (check_osd_exists && osdmap.exists(id)) {
            ss << "id " << id << " already in use and does not match uuid " << uuid;
            return -EINVAL;
        }
    }
    return 0;
...
int OSDMonitor::prepare_command_osd_create(const int32_t id, const uuid_d& uuid, int32_t* existing_id, stringstream& ss)
{
...
int OSDMonitor::prepare_command_osd_new(MonOpRequestRef op, const cmdmap_t& cmdmap, const map<string, string>& params,
                                        stringstream& ss, Formatter* f)
{
    uuid_d uuid;
    string uuidstr;
    int64_t id = -1;
    ...
    if (cmd_getval(cmdmap, "id", id) && (id < 0)) {
    ...
    int32_t existing_id = -1;
    int err = validate_osd_create(id, uuid, !is_recreate_destroyed, &existing_id, ss);
    ...
    bool has_secrets =
        params.count("cephx_secret") || params.count("cephx_lockbox_secret") || params.count("dmcrypt_key");
    ...
    if (has_secrets) {
        ...
        err = mon.authmon()->validate_osd_new(id, uuid, cephx_secret, lockbox_secret, cephx_entity, lockbox_entity, ss);
        if (err < 0) {
            return err;
        }
    ...
    if (has_secrets) {
        ...
        err = mon.authmon()->do_osd_new(cephx_entity, lockbox_entity, has_lockbox);
        ceph_assert(0 == err);
...
static int parse_reweights(CephContext* cct, const cmdmap_t& cmdmap, const OSDMap& osdmap,
...
int OSDMonitor::prepare_command_osd_destroy(int32_t id, stringstream& ss)
{
    ceph_assert(paxos.is_plugged());
    ...
    int err = mon.authmon()->validate_osd_destroy(id, uuid, cephx_entity, lockbox_entity, ss);
    if (err < 0) {
        if (err == -ENOENT) {
...
int OSDMonitor::prepare_command_osd_purge(int32_t id, stringstream& ss)
{
    ceph_assert(paxos.is_plugged());
                           ...
                           /* out */ pg_t& pgid, std::optional<string> pgids)
{
    ...
    if (!cmd_getval(cmdmap, "pgid", pgidstr)) {
        ss << "unable to parse 'pgid' value '" << cmd_vartype_stringify(cmdmap.at("pgid")) << "'";
        return -EINVAL;
    }
    ...
    if (pgids.has_value()) pgids.value() = pgidstr;
    return 0;
}
...
bool OSDMonitor::prepare_command_impl(MonOpRequestRef op, const cmdmap_t& cmdmap)
{
    ...
    if (prefix != "osd pg-temp" && prefix != "osd pg-upmap" &&
    ...
    if (prefix == "osd setcrushmap" || (prefix == "osd crush set" && !osdid_present)) {
        if (pending_inc.crush.length()) {
        ...
        if (cmd_getval(cmdmap, "prior_version", prior_version)) {
            ...
            if (prior_version != osdmap.get_crush_version()) {
                err = -EPERM;
                ss << "prior_version " << prior_version << " != crush version " << osdmap.get_crush_version();
                goto reply_no_propose;
            }
        }
        ...
        for (int b = 0; b < newcrush.get_max_buckets(); ++b) {
            int bid = -1 - b;
            if (newcrush.bucket_exists(bid) && newcrush.get_bucket_alg(bid) == CRUSH_BUCKET_STRAW) {
        ...
        for (unsigned j = 0; j < idvec.size() && !stop; j++) {
            set<int> osds;
            // wildcard?
            if (j == 0 && (idvec[0] == "any" || idvec[0] == "all" || idvec[0] == "*")) {
            ...
            for (auto& osd : osds) {
                ...
                dout(5) << action << " crush item id " << osd << " name '" << name << "' device_class '" << device_class
                        << "'" << dendl;
                ...
                if (err == 0 && !_have_pending_crush()) {
                    if (!stop) {
                        ...
                        ss << "set-device-class item id " << osd << " name '" << name << "' device_class '"
        ...
        if (!_have_pending_crush() && _get_stable_crush().class_exists(device_class)) {
        ...
        pending_inc.crush.clear();
        newcrush.encode(pending_inc.crush, mon.get_quorum_con_features());
        ss << "created class " << device_class << " with id " << class_id << " to crush map";
        goto update;
    }
        ...
        mempool::osdmap::map<string, map<string, string>> old_ec_profiles = osdmap.get_erasure_code_profiles();
        auto ec_profiles = pending_inc.get_erasure_code_profiles();
#ifdef HAVE_STDLIB_MAP_SPLICING
        ec_profiles.merge(old_ec_profiles);
#else
        ec_profiles.insert(make_move_iterator(begin(old_ec_profiles)), make_move_iterator(end(old_ec_profiles)));
#endif
        ...
        if (!referenced_by.empty()) {
            err = -EBUSY;
            ss << "class '" << device_class << "' is still referenced by erasure-code-profile(s): " << referenced_by;
            goto reply_no_propose;
        }
        ...
        if (osds.empty()) {
            ...
            if (err < 0) {
                ss << "class '" << device_class << "' cannot be removed '" << cpp_strerror(err) << "'";
                goto reply_no_propose;
            }
        }
        ...
        pending_inc.crush.clear();
        newcrush.encode(pending_inc.crush, mon.get_quorum_con_features());
        ss << "removed class " << device_class << " with id " << class_id << " from crush map";
        goto update;
    }
        ...
        if (err < 0) {
            ss << "fail to rename '" << srcname << "' to '" << dstname << "' : " << cpp_strerror(err);
            goto reply_no_propose;
        }
        ...
        if (!_have_pending_crush() && _get_stable_crush().name_exists(name)) {
        ...
        int bucketno;
        err = newcrush.add_bucket(0, 0, CRUSH_HASH_DEFAULT, type, 0, NULL, NULL, &bucketno);
        if (err < 0) {
        ...
        if (!loc.empty()) {
            if (!newcrush.check_item_loc(cct, bucketno, loc, (int*)NULL)) {
                ...
                ss << "no need to move item id " << bucketno << " name '" << name << "' to location " << loc
                   << " in crush map";
            }
        }
        ...
        newcrush.encode(pending_inc.crush, mon.get_quorum_con_features());
        if (loc.empty()) {
            ss << "added bucket " << name << " type " << typestr << " to crush map";
        }
        else {
            ss << "added bucket " << name << " type " << typestr << " to location " << loc;
        }
        goto update;
    }
    ...
    else if (prefix == "osd crush weight-set create" || prefix == "osd crush weight-set create-compat") {
        if (_have_pending_crush()) {
        ...
        if (!newcrush.create_choose_args(pool, positions)) {
            if (pool == CrushWrapper::DEFAULT_CHOOSE_ARGS) {
                ...
                ss << "weight-set for pool '" << osdmap.get_pool_name(pool) << "' already created";
            }
            goto reply_no_propose;
        }
        pending_inc.crush.clear();
    ...
    else if (prefix == "osd crush weight-set rm" || prefix == "osd crush weight-set rm-compat") {
        CrushWrapper newcrush = _get_pending_crush();
        int64_t pool;
    ...
    else if (prefix == "osd crush weight-set reweight" || prefix == "osd crush weight-set reweight-compat") {
        ...
        err = newcrush.choose_args_adjust_item_weightf(
            cct, newcrush.choose_args_get(pool), newcrush.get_item_id(item), weight, &ss);
        if (err < 0) {
            goto reply_no_propose;
        }
        err = 0;
    ...
    else if (osdid_present && (prefix == "osd crush set" || prefix == "osd crush add")) {
        ...
        if (!osdmap.exists(osdid)) {
            err = -ENOENT;
            ss << osd_name << " does not exist. Create it before updating the crush map";
            goto reply_no_propose;
        }

        double weight;
        if (!cmd_getval(cmdmap, "weight", weight)) {
            ss << "unable to parse weight value '" << cmd_vartype_stringify(cmdmap.at("weight")) << "'";
            err = -EINVAL;
            goto reply_no_propose;
        }
        ...
        if (prefix == "osd crush set" && !_get_stable_crush().item_exists(osdid)) {
            err = -ENOENT;
            ss << "unable to set item id " << osdid << " name '" << osd_name << "' weight " << weight << " at location "
               << loc << ": does not exist";
            goto reply_no_propose;
        }

        dout(5) << "adding/updating crush item id " << osdid << " name '" << osd_name << "' weight " << weight
                << " at location " << loc << dendl;
        CrushWrapper newcrush = _get_pending_crush();

        string action;
        if (prefix == "osd crush set" || newcrush.check_item_loc(cct, osdid, loc, (int*)NULL)) {
            action = "set";
        ...
        if (err < 0) goto reply_no_propose;

        if (err == 0 && !_have_pending_crush()) {
            ss << action << " item id " << osdid << " name '" << osd_name << "' weight " << weight << " at location "
               << loc << ": no change";
            goto reply_no_propose;
        }
        ...
        pending_inc.crush.clear();
        newcrush.encode(pending_inc.crush, mon.get_quorum_con_features());
        ss << action << " item id " << osdid << " name '" << osd_name << "' weight " << weight << " at location " << loc
           << " to crush map";
        getline(ss, rs);
        wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, 0, rs, get_last_committed() + 1));
        return true;
        ...
        do {
            ...
            dout(0) << "create-or-move crush item name '" << osd_name << "' initial_weight " << weight
                    << " at location " << loc << dendl;

            CrushWrapper newcrush = _get_pending_crush();

            err =
                newcrush.create_or_move_item(cct, osdid, weight, osd_name, loc, g_conf()->osd_crush_update_weight_set);
            if (err == 0) {
                ss << "create-or-move updated item name '" << osd_name << "' weight " << weight << " at location "
                   << loc << " to crush map";
                break;
            }
            if (err > 0) {
                pending_inc.crush.clear();
                newcrush.encode(pending_inc.crush, mon.get_quorum_con_features());
                ss << "create-or-move updating item name '" << osd_name << "' weight " << weight << " at location "
                   << loc << " to crush map";
                getline(ss, rs);
                wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, 0, rs, get_last_committed() + 1));
                return true;
        ...
        do {
            ...
            if (!newcrush.check_item_loc(cct, id, loc, (int*)NULL)) {
                if (id >= 0) {
                    err = newcrush.create_or_move_item(cct, id, 0, name, loc, g_conf()->osd_crush_update_weight_set);
                }
                else {
        ...
        if (newcrush.get_bucket_alg(sid) != newcrush.get_bucket_alg(did) && !force) {
        ...
        pending_inc.crush.clear();
        newcrush.encode(pending_inc.crush, mon.get_quorum_con_features());
        wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, err, ss.str(), get_last_committed() + 1));
        return true;
    }
        ...
        if (!newcrush.name_exists(name)) {
            ...
            if (!newcrush.check_item_loc(cct, id, loc, (int*)NULL)) {
                ...
                if (err >= 0) {
                    ...
                    ss << "cannot link item id " << id << " name '" << name << "' to location " << loc;
                    goto reply_no_propose;
                }
            }
            else {
    ...
    else if (prefix == "osd crush rm" || prefix == "osd crush remove" || prefix == "osd crush unlink") {
        do {
            ...
            err = prepare_command_osd_crush_remove(newcrush, id, ancestor, (ancestor < 0), unlink_only);
            ...
            if (err == 0) {
                if (!unlink_only) pending_inc.new_crush_node_flags[id] = 0;
        ...
        err = newcrush.adjust_item_weightf(cct, id, w, g_conf()->osd_crush_update_weight_set);
        if (err < 0) goto reply_no_propose;
        pending_inc.crush.clear();
        newcrush.encode(pending_inc.crush, mon.get_quorum_con_features());
        ss << "reweighted item id " << id << " name '" << name << "' to " << w << " in crush map";
        getline(ss, rs);
        wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, 0, rs, get_last_committed() + 1));
        return true;
    }
        ...
        err = newcrush.adjust_subtree_weightf(cct, id, w, g_conf()->osd_crush_update_weight_set);
        if (err < 0) goto reply_no_propose;
        pending_inc.crush.clear();
        newcrush.encode(pending_inc.crush, mon.get_quorum_con_features());
        ss << "reweighted subtree id " << id << " name '" << name << "' to " << w << " in crush map";
        getline(ss, rs);
        wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, 0, rs, get_last_committed() + 1));
        return true;
    }
        ...
        if (!cmd_getval(cmdmap, "value", value)) {
            err = -EINVAL;
            ss << "failed to parse integer value " << cmd_vartype_stringify(cmdmap.at("value"));
            goto reply_no_propose;
        }
        ...
        if (newcrush.rule_exists(name)) {
            ...
            int ruleno = newcrush.add_simple_rule(name, root, type, "", mode, pg_pool_t::TYPE_REPLICATED, &ss);
            if (ruleno < 0) {
                err = ruleno;
                goto reply_no_propose;
            }
        ...
        if (newcrush.rule_exists(name)) {
            ...
            int ruleno =
                newcrush.add_simple_rule(name, root, type, device_class, "firstn", pg_pool_t::TYPE_REPLICATED, &ss);
            if (ruleno < 0) {
                err = ruleno;
                goto reply_no_propose;
            }
        ...
        if (erasure_code_profile_in_use(pending_inc.new_pools, name, &ss)) goto wait;
        ...
        if (osdmap.has_erasure_code_profile(name) || pending_inc.new_erasure_code_profiles.count(name)) {
            if (osdmap.has_erasure_code_profile(name)) {
        ...
        if (auto found = profile_map.find("crush-failure-domain"); found != profile_map.end()) {
            ...
            if (failure_domain_type < 0) {
                ss << "erasure-code-profile " << profile_map << " contains an invalid failure-domain "
                   << std::quoted(failure_domain);
                err = -EINVAL;
                goto reply_no_propose;
            }
        }
        ...
        if (profile_map.find("plugin") == profile_map.end()) {
            ss << "erasure-code-profile " << profile_map << " must contain a plugin entry" << std::endl;
            err = -EINVAL;
            goto reply_no_propose;
        }
        ...
        if (pending_inc.has_erasure_code_profile(name)) {
            ...
            if (osdmap.has_erasure_code_profile(name)) {
                ErasureCodeProfile existing_profile_map = osdmap.get_erasure_code_profile(name);
                ...
                if (!force) {
                    err = -EPERM;
                    ss << "will not override erasure code profile " << name << " because the existing profile "
                       << existing_profile_map << " is different from the proposed profile " << profile_map;
                    goto reply_no_propose;
                }
            }
        ...
        if (!cmd_getval(cmdmap, "newmax", newmax)) {
            ss << "unable to parse 'newmax' value '" << cmd_vartype_stringify(cmdmap.at("newmax")) << "'";
            err = -EINVAL;
            goto reply_no_propose;
        }
        ...
        if (newmax > g_conf()->mon_max_osd) {
            err = -ERANGE;
            ss << "cannot set max_osd to " << newmax << " which is > conf.mon_max_osd (" << g_conf()->mon_max_osd
               << ")";
            goto reply_no_propose;
        }
        ...
        if (newmax < osdmap.get_max_osd()) {
            ...
            for (int i = newmax; i < osdmap.get_max_osd(); i++) {
                if (osdmap.exists(i)) {
                    err = -EBUSY;
                    ss << "cannot shrink max_osd to " << newmax << " because osd." << i
    ...
    else if (prefix == "osd set-full-ratio" || prefix == "osd set-backfillfull-ratio" ||
             prefix == "osd set-nearfull-ratio") {
        double n;
        if (!cmd_getval(cmdmap, "ratio", n)) {
            ss << "unable to parse 'ratio' value '" << cmd_vartype_stringify(cmdmap.at("ratio")) << "'";
            err = -EINVAL;
            goto reply_no_propose;
        }
        ...
        if (!sure) {
            ...
            bool first = true;
            bool ok = true;
            for (int type : {CEPH_ENTITY_TYPE_CLIENT, CEPH_ENTITY_TYPE_MDS, CEPH_ENTITY_TYPE_MGR}) {
                ...
                for (auto& q : p->second) {
                    ...
                    if (missing) {
                        ...
                        first = false;
                        ss << q.second << " connected " << ceph_entity_type_name(type) << "(s) look like "
                           << ceph_release_name(ceph_release_from_features(q.first)) << " (missing 0x" << std::hex
                           << missing << std::dec << ")";
                        ok = false;
                    }
                }
        ...
        if (rel == ceph_release_t::pacific) {
            if (!mon.monmap->get_required_features().contains_all(ceph::features::mon::FEATURE_PACIFIC)) {
                ss << "not all mons are pacific";
                err = -EPERM;
                goto reply_no_propose;
            }
            if ((!HAVE_FEATURE(osdmap.get_up_osd_features(), SERVER_PACIFIC)) && !sure) {
            ...
            if (!mon.monmap->get_required_features().contains_all(ceph::features::mon::FEATURE_QUINCY)) {
                ss << "not all mons are quincy";
                err = -EPERM;
                goto reply_no_propose;
            }
            if ((!HAVE_FEATURE(osdmap.get_up_osd_features(), SERVER_QUINCY)) && !sure) {
            ...
            if (!mon.monmap->get_required_features().contains_all(ceph::features::mon::FEATURE_REEF)) {
                ss << "not all mons are reef";
                err = -EPERM;
                goto reply_no_propose;
            }
            if ((!HAVE_FEATURE(osdmap.get_up_osd_features(), SERVER_REEF)) && !sure) {
    ...
    else if (prefix == "osd down" || prefix == "osd out" || prefix == "osd in" || prefix == "osd rm" ||
             prefix == "osd stop") {
            ...
            for (auto& osd : osds) {
                if (prefix == "osd down") {
                    if (osdmap.is_down(osd)) {
                        if (verbose) ss << "osd." << osd << " is already down. ";
                    }
                    else {
                    ...
                    if (osdmap.is_out(osd)) {
                        if (verbose) ss << "osd." << osd << " is already out. ";
                    }
                    else {
                        ...
                        msg << "Client " << op->get_session()->entity_name << " marked osd." << osd << " out";
                        if (osdmap.is_up(osd)) {
                            msg << ", while it was still marked up";
                        }
                        else {
                            auto period = ceph_clock_now() - down_pending_out[osd];
                            msg << ", after it was down for " << int(period.sec()) << " seconds";
                        }
                    ...
                    if (osdmap.is_in(osd)) {
                        if (verbose) ss << "osd." << osd << " is already in. ";
                    }
                    else {
                    ...
                    if (osdmap.is_stop(osd)) {
                        if (verbose) ss << "osd." << osd << " is already stopped. ";
                    }
        ...
        if (any) {
            getline(ss, rs);
            wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, err, rs, get_last_committed() + 1));
            return true;
        }
    }
    else if (prefix == "osd set-group" || prefix == "osd unset-group" || prefix == "osd add-noup" ||
             prefix == "osd add-nodown" || prefix == "osd add-noin" || prefix == "osd add-noout" ||
             prefix == "osd rm-noup" || prefix == "osd rm-nodown" || prefix == "osd rm-noin" ||
             prefix == "osd rm-noout") {
        bool do_set = prefix == "osd set-group" || prefix.find("add") != string::npos;
        string flag_str;
        unsigned flags = 0;
        vector<string> who;
        ...
        for (auto osd : osds) {
            ...
            if (do_set) {
                if (flags & CEPH_OSD_NOUP) {
                    any |= osdmap.is_noup_by_osd(osd) ? pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NOUP)
                                                      : pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOUP);
                }
                if (flags & CEPH_OSD_NODOWN) {
                    any |= osdmap.is_nodown_by_osd(osd) ? pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NODOWN)
                                                        : pending_inc.pending_osd_state_set(osd, CEPH_OSD_NODOWN);
                }
                if (flags & CEPH_OSD_NOIN) {
                    any |= osdmap.is_noin_by_osd(osd) ? pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NOIN)
                                                      : pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOIN);
                }
                if (flags & CEPH_OSD_NOOUT) {
                    any |= osdmap.is_noout_by_osd(osd) ? pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NOOUT)
                                                       : pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOOUT);
                }
            }
            else {
                if (flags & CEPH_OSD_NOUP) {
                    any |= osdmap.is_noup_by_osd(osd) ? pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOUP)
                                                      : pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NOUP);
                }
                if (flags & CEPH_OSD_NODOWN) {
                    any |= osdmap.is_nodown_by_osd(osd) ? pending_inc.pending_osd_state_set(osd, CEPH_OSD_NODOWN)
                                                        : pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NODOWN);
                }
                if (flags & CEPH_OSD_NOIN) {
                    any |= osdmap.is_noin_by_osd(osd) ? pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOIN)
                                                      : pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NOIN);
                }
                if (flags & CEPH_OSD_NOOUT) {
                    any |= osdmap.is_noout_by_osd(osd) ? pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOOUT)
        ...
        if ((int)new_pg_temp.size() < pool_min_size) {
            ss << "num of osds (" << new_pg_temp.size() << ") < pool min size (" << pool_min_size << ")";
            err = -EINVAL;
            goto reply_no_propose;
        }
        ...
        if ((int)new_pg_temp.size() > pool_size) {
            ss << "num of osds (" << new_pg_temp.size() << ") > pool size (" << pool_size << ")";
            err = -EINVAL;
            goto reply_no_propose;
        }

        pending_inc.new_pg_temp[pgid] = mempool::osdmap::vector<int>(new_pg_temp.begin(), new_pg_temp.end());
        ss << "set " << pgid << " pg_temp mapping to " << new_pg_temp;
        goto update;
    }
    else if (prefix == "osd primary-temp" || prefix == "osd rm-primary-temp") {
        pg_t pgid;
        err = parse_pgid(cmdmap, ss, pgid);
        if (err < 0) goto reply_no_propose;
        ...
        if (prefix == "osd primary-temp") {
            if (!cmd_getval(cmdmap, "id", osd)) {
                ss << "unable to parse 'id' value '" << cmd_vartype_stringify(cmdmap.at("id")) << "'";
                err = -EINVAL;
                goto reply_no_propose;
            }
            if (!osdmap.exists(osd)) {
    ...
    else if (prefix == "osd pg-upmap" || prefix == "osd rm-pg-upmap" || prefix == "osd pg-upmap-items" ||
             prefix == "osd rm-pg-upmap-items" || prefix == "osd pg-upmap-primary" ||
        ...
        default: ceph_abort_msg("invalid upmap option");
        }
        ...
        if (osdmap.require_min_compat_client < min_release) {
            ss << "min_compat_client " << osdmap.require_min_compat_client << " < " << min_release_name
        ...
        case OP_PG_UPMAP:   // fall through
        case OP_RM_PG_UPMAP:
            if (pending_inc.new_pg_upmap.count(pgid) || pending_inc.old_pg_upmap.count(pgid)) {
                dout(10) << __func__ << " waiting for pending update on " << pgid << dendl;
                goto wait;
            }
            break;
        ...
        case OP_RM_PG_UPMAP_ITEMS:   // fall through
            if (pending_inc.new_pg_upmap_items.count(pgid) || pending_inc.old_pg_upmap_items.count(pgid)) {
                dout(10) << __func__ << " waiting for pending update on " << pgid << dendl;
                goto wait;
            }
            break;
        ...
        case OP_PG_UPMAP:
        {
            ...
            if (!cmd_getval(cmdmap, "id", id_vec)) {
                ss << "unable to parse 'id' value(s) '" << cmd_vartype_stringify(cmdmap.at("id")) << "'";
                err = -EINVAL;
                goto reply_no_propose;
            }
            ...
            if ((int)id_vec.size() < pool_min_size) {
                ss << "num of osds (" << id_vec.size() << ") < pool min size (" << pool_min_size << ")";
                err = -EINVAL;
                goto reply_no_propose;
            }
            ...
            if ((int)id_vec.size() > pool_size) {
                ss << "num of osds (" << id_vec.size() << ") > pool size (" << pool_size << ")";
                err = -EINVAL;
                goto reply_no_propose;
            }
            ...
            pending_inc.new_pg_upmap[pgid] = mempool::osdmap::vector<int32_t>(new_pg_upmap.begin(), new_pg_upmap.end());
        ...
        case OP_PG_UPMAP_ITEMS:
        {
            ...
            if ((int)(id_vec.size() / 2) > pool_size) {
                ss << "num of osd pairs (" << id_vec.size() / 2 << ") > pool size (" << pool_size << ")";
                err = -EINVAL;
                goto reply_no_propose;
            }
            ...
            for (auto p = id_vec.begin(); p != id_vec.end(); ++p) {
                ...
                auto it = std::find(new_pg_upmap_items.begin(), new_pg_upmap_items.end(), entry);
            ...
            pending_inc.new_pg_upmap_items[pgid] =
                mempool::osdmap::vector<pair<int32_t, int32_t>>(new_pg_upmap_items.begin(), new_pg_upmap_items.end());
        ...
        case OP_PG_UPMAP_PRIMARY:
        {
            ...
            if (!cmd_getval(cmdmap, "id", id)) {
                ss << "invalid osd id value '" << cmd_vartype_stringify(cmdmap.at("id")) << "'";
                err = -EINVAL;
                goto reply_no_propose;
            }
            ...
            new_acting[0] = id;
            int pool_size = osdmap.get_pg_pool_size(pgid);
            if (osdmap.crush->verify_upmap(cct, osdmap.get_pg_pool_crush_rule(pgid), pool_size, new_acting) >= 0) {
                ss << "change primary for pg " << pgid << " to osd." << id;
            }
            else {
                ss << "can't change primary for pg " << pgid << " to osd." << id << " - illegal pg after the change";
                err = -EINVAL;
                goto reply_no_propose;
            }
        ...
        int64_t id;
        if (!cmd_getval(cmdmap, "id", id)) {
            ss << "unable to parse osd id value '" << cmd_vartype_stringify(cmdmap.at("id")) << "'";
            err = -EINVAL;
            goto reply_no_propose;
        }
        double w;
        ...
        if (err) {
            ss << "unable to parse 'weights' value '" << cmd_vartype_stringify(cmdmap.at("weights")) << "'";
            goto reply_no_propose;
        }
        pending_inc.new_weight.insert(weights.begin(), weights.end());
        wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, 0, rs, rdata, get_last_committed() + 1));
        return true;
    }
    ...
    else if (prefix == "osd destroy-actual" || prefix == "osd purge-actual" || prefix == "osd purge-new") {
        ...
        if (!is_destroy) {
            ceph_assert("osd purge-actual" == prefix || "osd purge-new" == prefix);
        }
        ...
        if (prefix == "osd purge-new" && (osdmap.get_state(id) & CEPH_OSD_NEW) == 0) {
    ...
    else if (prefix == "osd blocklist clear" || prefix == "osd blacklist clear") {
    ...
    else if (prefix == "osd blocklist" || prefix == "osd blacklist") {
        ...
        if (!addr.parse(addrstr)) {
            ...
            if (range) {
                if (!addr.maybe_cidr()) {
                    ss << "You specified a range command, but " << addr << " does not parse as a CIDR range";
                    err = -EINVAL;
                    goto reply_no_propose;
                }
                ...
                if ((addr.is_ipv4() && addr.get_nonce() > 32) || (addr.is_ipv6() && addr.get_nonce() > 128)) {
            ...
            if (blocklistop == "add") {
                ...
                // default one hour
                double d = cmd_getval_or<double>(cmdmap, "expire", g_conf()->mon_osd_blocklist_default_expire);
                expires += d;

                auto add_to_pending_blocklists = [](auto& nb, auto& ob, const auto& addr, const auto& expires) {
                    nb[addr] = expires;
                    // cancel any pending un-blocklisting request too
                    auto it = std::find(ob.begin(), ob.end(), addr);
                    if (it != ob.end()) {
                        ob.erase(it);
                    }
                };
                if (range) {
                    add_to_pending_blocklists(
                        pending_inc.new_range_blocklist, pending_inc.old_range_blocklist, addr, expires);
                }
                else {
                    add_to_pending_blocklists(pending_inc.new_blocklist, pending_inc.old_blocklist, addr, expires);
                }
                ...
                auto rm_from_pending_blocklists = [](const auto& addr, auto& blocklist, auto& ob, auto& pb) {
                ...
                if ((!range && rm_from_pending_blocklists(
                                   addr, osdmap.blocklist, pending_inc.old_blocklist, pending_inc.new_blocklist)) ||
                    (range && rm_from_pending_blocklists(addr,
                                                         osdmap.range_blocklist,
        ...
        if (pool_type_str.empty()) pool_type_str = g_conf().get_val<string>("osd_pool_default_type");
        ...
        if (pool_type == pg_pool_t::TYPE_ERASURE) {
            if (erasure_code_profile == "") erasure_code_profile = "default";
            ...
            expected_num_objects = cmd_getval_or<int64_t>(cmdmap, "expected_num_objects", 0);
        }
        else {
        ...
        if (has_filestore_osd && expected_num_objects > 0 && cct->_conf->filestore_merge_threshold > 0) {
        ...
        bool crimson =
            cmd_getval_or<bool>(cmdmap, "crimson", false) || cct->_conf.get_val<bool>("osd_pool_default_crimson");
        ...
        if (err < 0) {
            ...
            case -EEXIST:
                ...
                err = 0;
                goto reply_no_propose;
            case -EAGAIN: goto wait;
            case -ERANGE: goto reply_no_propose;
            default: goto reply_no_propose;
            }
        }
        else {
    ...
    else if (prefix == "osd pool delete" || prefix == "osd pool rm") {
        ...
        if (poolstr2 != poolstr || (!force && !force_no_fake)) {
        ...
        if (pool_src < 0) {
            if (pool_dst >= 0) {
                ...
                ss << "pool '" << srcpoolstr << "' does not exist; pool '" << destpoolstr
        ...
        if (ret == 0) {
            ...
            ss << "failed to rename pool '" << srcpoolstr << "' to '" << destpoolstr << "': " << cpp_strerror(ret);
        }
        getline(ss, rs);
        wait_for_finished_proposal(op, new Monitor::C_Command(mon, op, ret, rs, get_last_committed() + 1));
        return true;
    }
        ...
        if (pstats && pstats->stats.sum.num_objects != 0 && !force_nonempty) {
            ss << "tier pool '" << tierpoolstr << "' is not empty; --force-nonempty to force";
            err = -ENOTEMPTY;
            goto reply_no_propose;
        }
        if (tp->is_erasure()) {
            ss << "tier pool '" << tierpoolstr << "' is an ec pool, which cannot be a tier";
            err = -ENOTSUP;
            goto reply_no_propose;
        }
        if ((!tp->removed_snaps.empty() || !tp->snaps.empty()) &&
            (!force_nonempty || !g_conf()->mon_debug_unsafe_allow_tier_with_nonempty_snaps)) {
    ...
    else if (prefix == "osd tier remove" || prefix == "osd tier rm") {
        string poolstr;
        cmd_getval(cmdmap, "pool", poolstr);
        int64_t pool_id = osdmap.lookup_pg_pool_name(poolstr);
        if (pool_id < 0) {
        ...
        if (tp->tier_of != pool_id) {
            ss << "tier pool '" << tierpoolstr << "' is a tier of '" << osdmap.get_pool_name(tp->tier_of)
               << "': "
        ...
        pg_pool_t* np = pending_inc.get_new_pool(pool_id, p);
        pg_pool_t* ntp = pending_inc.get_new_pool(tierpool_id, tp);
        if (np->tiers.count(tierpool_id) == 0 || ntp->tier_of != pool_id || np->read_tier == tierpool_id) {
            goto wait;
        }
        ...
        if (p->has_read_tier()) {
            ss << "pool '" << poolstr << "' has overlay '" << osdmap.get_pool_name(p->read_tier)
               << "'; please remove-overlay first";
            err = -EINVAL;
            goto reply_no_propose;
        }
    ...
    else if (prefix == "osd tier remove-overlay" || prefix == "osd tier rm-overlay") {
        string poolstr;
        cmd_getval(cmdmap, "pool", poolstr);
        int64_t pool_id = osdmap.lookup_pg_pool_name(poolstr);
        if (pool_id < 0) {
        ...
        if (mode == pg_pool_t::CACHEMODE_FORWARD || mode == pg_pool_t::CACHEMODE_READFORWARD) {
            ss << "'" << modestr << "' is no longer a supported cache mode";
            err = -EPERM;
            goto reply_no_propose;
        }
        if ((mode != pg_pool_t::CACHEMODE_WRITEBACK && mode != pg_pool_t::CACHEMODE_NONE &&
             mode != pg_pool_t::CACHEMODE_PROXY && mode != pg_pool_t::CACHEMODE_READPROXY) &&
            !sure) {
        ...
        if (p->cache_mode == mode &&
            (pending_inc.new_pools.count(pool_id) == 0 || pending_inc.new_pools[pool_id].cache_mode == p->cache_mode)) {
        ...
        if (p->cache_mode == pg_pool_t::CACHEMODE_WRITEBACK &&
            (mode != pg_pool_t::CACHEMODE_PROXY && mode != pg_pool_t::CACHEMODE_READPROXY)) {
            ss << "unable to set cache-mode '" << pg_pool_t::get_cache_mode_name(mode) << "' on a '"
               << pg_pool_t::get_cache_mode_name(p->cache_mode) << "' pool; only '"
               << pg_pool_t::get_cache_mode_name(pg_pool_t::CACHEMODE_PROXY) << "','"
               << pg_pool_t::get_cache_mode_name(pg_pool_t::CACHEMODE_READPROXY) << "' allowed.";
            err = -EINVAL;
            goto reply_no_propose;
        }
        if ((p->cache_mode == pg_pool_t::CACHEMODE_READFORWARD &&
             (mode != pg_pool_t::CACHEMODE_WRITEBACK && mode != pg_pool_t::CACHEMODE_PROXY &&
            ...
            (p->cache_mode == pg_pool_t::CACHEMODE_PROXY &&
             (mode != pg_pool_t::CACHEMODE_WRITEBACK && mode != pg_pool_t::CACHEMODE_READPROXY)) ||

            (p->cache_mode == pg_pool_t::CACHEMODE_FORWARD &&
             (mode != pg_pool_t::CACHEMODE_WRITEBACK && mode != pg_pool_t::CACHEMODE_PROXY &&
              mode != pg_pool_t::CACHEMODE_READPROXY))) {

            const pool_stat_t* pstats = mon.mgrstatmon()->get_pool_stat(pool_id);

            if (pstats && pstats->stats.sum.num_objects_dirty > 0) {
                ss << "unable to set cache-mode '" << pg_pool_t::get_cache_mode_name(mode) << "' on pool '" << poolstr
                   << "': dirty objects found";
                err = -EBUSY;
                goto reply_no_propose;
            }
        ...
        ss << "set cache-mode for pool '" << poolstr << "' to " << pg_pool_t::get_cache_mode_name(mode);
        if (mode == pg_pool_t::CACHEMODE_NONE) {
            const pg_pool_t* base_pool = osdmap.get_pg_pool(np->tier_of);
            ceph_assert(base_pool);
            if (base_pool->read_tier == pool_id || base_pool->write_tier == pool_id)
        ...
        if (!cmd_getval(cmdmap, "size", size)) {
            ss << "unable to parse 'size' value '" << cmd_vartype_stringify(cmdmap.at("size")) << "'";
            err = -EINVAL;
            goto reply_no_propose;
        }
        // make sure new tier is empty
        ...
        HitSet::Params hsp;
        auto& cache_hit_set_type = g_conf().get_val<string>("osd_tier_default_cache_hit_set_type");
        if (cache_hit_set_type == "bloom") {
            ...
            ss << "osd tier cache default hit set type '" << cache_hit_set_type << "' is not a known type";
            err = -EINVAL;
            goto reply_no_propose;
        }
        // go
        ...
        ntp->min_read_recency_for_promote =
            g_conf().get_val<uint64_t>("osd_tier_default_cache_min_read_recency_for_promote");
        ntp->min_write_recency_for_promote =
            g_conf().get_val<uint64_t>("osd_tier_default_cache_min_write_recency_for_promote");
        ...
        bool creating_now;
        {
            std::lock_guard<std::mutex> l(creating_pgs_lock);
            auto emplaced =
                creating_pgs.pgs.emplace(pgid, creating_pgs_t::pg_create_info(osdmap.get_epoch(), ceph_clock_now()));
        ...
        bool experimental_enabled = g_ceph_context->check_experimental_feature_enabled("crimson");
...
reply_no_propose:
    getline(ss, rs);
    if (err < 0 && rs.length() == 0) rs = cpp_strerror(err);
...
bool OSDMonitor::enforce_pool_op_caps(MonOpRequestRef op)
{
    ...
    case POOL_OP_DELETE_UNMANAGED_SNAP:
    {
        ...
        if (!is_unmanaged_snap_op_permitted(
                cct, mon.key_server, session->entity_name, session->caps, session->get_peer_socket_addr(), pool_name)) {
...
bool OSDMonitor::preprocess_pool_op(MonOpRequestRef op)
{
    ...
    if (m->fsid != mon.monmap->fsid) {
        dout(0) << __func__ << " drop message on fsid " << m->fsid << " != " << mon.monmap->fsid << " for " << *m
                << dendl;
        _pool_op_reply(op, -EINVAL, osdmap.get_epoch());
        return true;
    }

    if (m->op == POOL_OP_CREATE) return preprocess_pool_op_create(op);
    ...
    if (p->snap_exists(m->name.c_str())) snap_exists = true;
    ...
    case POOL_OP_DELETE:
        ...
        return false;
    case POOL_OP_AUID_CHANGE: return false;
...
bool OSDMonitor::_is_removed_snap(int64_t pool, snapid_t snap)
{
    ...
    if (r == 0) {
        dout(10) << __func__ << " pool " << pool << " snap " << snap << " - purged, [" << begin << "," << end << ")"
                 << dendl;
        return true;
    }
    return false;
...
bool OSDMonitor::prepare_pool_op(MonOpRequestRef op)
{
    ...
    if (m->op == POOL_OP_CREATE_SNAP || m->op == POOL_OP_CREATE_UNMANAGED_SNAP) {
    ...
    case POOL_OP_DELETE_SNAP:
        if (!pool->is_unmanaged_snaps_mode()) {
            ...
            if ((m->op == POOL_OP_CREATE_SNAP && snap_exists) || (m->op == POOL_OP_DELETE_SNAP && !snap_exists)) {
                ret = 0;
            }
            else {
                break;
    ...
    case POOL_OP_CREATE_UNMANAGED_SNAP:
    {
        uint64_t snapid = pp.add_unmanaged_snap(osdmap.require_osd_release < ceph_release_t::octopus);
        encode(snapid, reply_data);
        changed = true;
    } break;

    case POOL_OP_DELETE_UNMANAGED_SNAP:
        if (!_is_removed_snap(m->pool, m->snapid) && !_is_pending_removed_snap(m->pool, m->snapid)) {
...
int OSDMonitor::_check_remove_pool(int64_t pool_id, const pg_pool_t& pool, ostream* ss)
{
    ...
    if (pool.tier_of >= 0) {
        *ss << "pool '" << poolstr << "' is a tier of '" << osdmap.get_pool_name(pool.tier_of) << "'";
        return -EBUSY;
    }
    if (!pool.tiers.empty()) {
    ...
    if (!g_conf()->mon_allow_pool_delete) {
        *ss << "pool deletion is disabled; you must first set the mon_allow_pool_delete config option to true before "
 ...
 */
bool OSDMonitor::_check_become_tier(const int64_t tier_pool_id, const pg_pool_t* tier_pool, const int64_t base_pool_id,
                                    const pg_pool_t* base_pool, int* err, ostream* ss) const
{
    ...
    if (base_pool->tiers.count(tier_pool_id)) {
        ...
        *err = 0;
        *ss << "pool '" << tier_pool_name << "' is now (or already was) a tier of '" << base_pool_name << "'";
        return false;
    }

    if (base_pool->is_tier()) {
        *ss << "pool '" << base_pool_name << "' is already a tier of '" << osdmap.get_pool_name(base_pool->tier_of)
            << "', "
    ...
    if (tier_pool->has_tiers()) {
        *ss << "pool '" << tier_pool_name << "' has following tier(s) already:";
        for (set<uint64_t>::iterator it = tier_pool->tiers.begin(); it != tier_pool->tiers.end(); ++it)
    ...
    if (tier_pool->is_tier()) {
        *ss << "tier pool '" << tier_pool_name << "' is already a tier of '" << osdmap.get_pool_name(tier_pool->tier_of)
            << "'";
        *err = -EINVAL;
        return false;
    }
 ...
 * (abort doesn't necessarily mean error, could be idempotency)
 */
bool OSDMonitor::_check_remove_tier(const int64_t base_pool_id, const pg_pool_t* base_pool, const pg_pool_t* tier_pool,
...
int OSDMonitor::_prepare_remove_pool(int64_t pool, ostream* ss, bool no_fake)
{
    ...
    if (g_conf()->mon_fake_pool_delete && !no_fake) {
        ...
        dout(1) << __func__ << " faking pool deletion: renaming " << pool << " " << old_name << " -> " << new_name
                << dendl;
    ...
    for (auto p = osdmap.pg_temp->begin(); p != osdmap.pg_temp->end(); ++p) {
    ...
    for (auto p = osdmap.primary_temp->begin(); p != osdmap.primary_temp->end(); ++p) {
        if (p->first.pool() == pool) {
...
bool OSDMonitor::prepare_pool_op_delete(MonOpRequestRef op)
{
    ...
    if (ret < 0) dout(10) << __func__ << " got " << ret << " " << ss.str() << dendl;
    wait_for_finished_proposal(op, new OSDMonitor::C_PoolOp(this, op, ret, pending_inc.epoch));
    return true;
}

void OSDMonitor::_pool_op_reply(MonOpRequestRef op, int ret, epoch_t epoch, bufferlist* blp)
{
    op->mark_osdmon_event(__func__);
    auto m = op->get_req<MPoolOp>();
    dout(20) << "_pool_op_reply " << ret << dendl;
    MPoolOpReply* reply = new MPoolOpReply(m->fsid, m->get_tid(), ret, epoch, get_last_committed(), blp);
    mon.send_reply(op, reply);
}
...
void OSDMonitor::try_enable_stretch_mode_pools(stringstream& ss, bool* okay, int* errcode, set<pg_pool_t*>* pools,
    ...
    for (const auto& pooli : osdmap.pools) {
        ...
        if ((p->get_size() != default_size ||
            ...
            ss << "we currently require stretch mode pools start out with the"
                  " default size/min_size, which '"
               << osdmap.pool_name[poolid] << "' does not";
            *errcode = -EINVAL;
            return;
        }
...
void OSDMonitor::try_enable_stretch_mode(stringstream& ss, bool* okay, int* errcode, bool commit,
                                         const string& dividing_bucket, uint32_t bucket_count,
                                         const set<pg_pool_t*>& pools, const string& new_crush_rule)
{
    dout(20) << __func__ << dendl;
    *okay = false;
    CrushWrapper crush = _get_pending_crush();
    int dividing_id = -1;
    if (auto type_id = crush.get_validated_type_id(dividing_bucket); !type_id.has_value()) {
    ...
    if (weight1 != weight2) {
        ...
        ss << "the 2 " << dividing_bucket << "instances in the cluster have differing weights " << weight1 << " and "
           << weight2 << " but stretch mode currently requires they be the same!";
        *errcode = -EINVAL;
...
bool OSDMonitor::check_for_dead_crush_zones(const map<string, set<string>>& dead_buckets, set<int>* really_down_buckets,
                                            set<string>* really_down_mons)
{
    ...
    dout(10) << "We determined CRUSH buckets " << *really_down_buckets << " and mons " << *really_down_mons
             << " are really down" << dendl;
    return really_down;
}

void OSDMonitor::trigger_degraded_stretch_mode(const set<int>& dead_buckets, const set<string>& live_zones)
{
    dout(20) << __func__ << dendl;
...
struct CMonExitRecovery : public Context
{
    OSDMonitor* m;
    bool force;
    CMonExitRecovery(OSDMonitor* mon, bool f)
        : m(mon)
        , force(f)
    {}
    void finish(int r) { m->try_end_recovery_stretch_mode(force); }
};
...
void OSDMonitor::try_end_recovery_stretch_mode(bool force)
{
    ...
    if (osdmap.recovering_stretch_mode &&
        ((!stretch_recovery_triggered.is_zero() &&
          ceph_clock_now() - g_conf().get_val<double>("mon_stretch_recovery_min_wait") > stretch_recovery_triggered) ||
         force)) {
        if (!mon.mgrstatmon()->is_readable()) {
