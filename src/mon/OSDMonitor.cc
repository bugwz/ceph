...
void OSDMonitor::create_pending()
{
    ...
    if (osdmap.crush->has_legacy_rule_ids()) {
        ...
        for (const auto& i : osdmap.get_pools()) {
            ...
            int new_rule_id = newcrush.find_rule(pool.crush_rule, pool.type, pool.size);

            dout(1) << __func__ << " rewriting pool " << osdmap.get_pool_name(pool_id) << " crush ruleset "
...
bool OSDMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    default: ceph_abort(); return true;
    }
}
...
bool OSDMonitor::preprocess_boot(MonOpRequestRef op)
{
    ...
    // force all osds to have gone through luminous prior to upgrade to nautilus
    {
        ...
        if (!missing.empty()) {
            ...
            mon.clog->info() << "disallowing boot of OSD " << m->get_orig_source_inst() << " because the osd lacks "
                             << ss.str();
            goto ignore;
        }
    }
    ...
    if (HAVE_FEATURE(m->osd_features, SERVER_OCTOPUS) && osdmap.require_osd_release < ceph_release_t::mimic) {
        mon.clog->info() << "disallowing boot of octopus+ OSD " << m->get_orig_source_inst()
                         << " because require_osd_release < mimic";
        goto ignore;
    }
    if (HAVE_FEATURE(m->osd_features, SERVER_PACIFIC) && osdmap.require_osd_release < ceph_release_t::nautilus) {
        mon.clog->info() << "disallowing boot of pacific+ OSD " << m->get_orig_source_inst()
    ...
    if (osdmap.require_osd_release >= ceph_release_t::luminous && osdmap.test_flag(CEPH_OSDMAP_PGLOG_HARDLIMIT) &&
        !(m->osd_features & CEPH_FEATURE_OSD_PGLOG_HARDLIMIT)) {
        mon.clog->info()
            << "disallowing boot of OSD " << m->get_orig_source_inst()
                                           ...
                                           creating_pgs_t* creating_pgs) const
{
    ...
    for (auto& p : pools) {
        ...
        int ruleno = osdmap.crush->find_rule(pool.get_crush_rule(), pool.get_type(), pool.get_size());
    ...
    for (auto epoch_pgs = creating_pgs_by_epoch->second.lower_bound(next);
        ...
        for (auto& pg : pgs) {
            ...
            if (old) {
                ...
                oldm->mkpg.emplace(pg.pgid, pg_create_t{create->second.create_epoch, pg.pgid, 0});
...
int OSDMonitor::check_pg_num(int64_t pool, int pg_num, int size, ostream* ss)
{
    ...
    if (projected > max_pgs) {
        ...
        *ss << " pg_num " << pg_num << " size " << size << " would mean " << projected
            << " total pgs, which exceeds max " << max_pgs << " (mon_max_pg_per_osd " << max_pgs_per_osd
...
int OSDMonitor::prepare_new_pool(string& name, int crush_rule, const string& crush_rule_name, unsigned pg_num,
                                 ...
                                 const uint64_t expected_num_objects, FastReadType fast_read,
                                 const string& pg_autoscale_mode, bool bulk, ostream* ss)
{
    if (name.length() == 0) return -EINVAL;
    if (pg_num == 0) pg_num = g_conf().get_val<uint64_t>("osd_pool_default_pg_num");
    ...
    if (osdmap.require_osd_release >= ceph_release_t::pacific && pg_num_max) {
...
bool OSDMonitor::prepare_command_impl(MonOpRequestRef op, const cmdmap_t& cmdmap)
{
    ...
    if (prefix == "osd setcrushmap" || (prefix == "osd crush set" && !osdid_present)) {
        ...
        if (rel == ceph_release_t::mimic) {
            if (!mon.monmap->get_required_features().contains_all(ceph::features::mon::FEATURE_MIMIC)) {
                ss << "not all mons are mimic";
                err = -EPERM;
                goto reply;
            }
            if ((!HAVE_FEATURE(osdmap.get_up_osd_features(), SERVER_MIMIC)) && !sure) {
            ...
            if (!mon.monmap->get_required_features().contains_all(ceph::features::mon::FEATURE_NAUTILUS)) {
                ss << "not all mons are nautilus";
                err = -EPERM;
                goto reply;
            }
            if ((!HAVE_FEATURE(osdmap.get_up_osd_features(), SERVER_NAUTILUS)) && !sure) {
            ...
            if (!mon.monmap->get_required_features().contains_all(ceph::features::mon::FEATURE_OCTOPUS)) {
                ss << "not all mons are octopus";
                err = -EPERM;
                goto reply;
            }
            if ((!HAVE_FEATURE(osdmap.get_up_osd_features(), SERVER_OCTOPUS)) && !sure) {
        ...
        default: ceph_abort_msg("invalid option");
        }
        ...
        if (!addr.parse(addrstr.c_str(), 0)) {
            ...
            if (blocklistop == "add") {
                ...
                double d;
                // default one hour
                cmd_getval(cmdmap, "expire", d, g_conf()->mon_osd_blocklist_default_expire);
                expires += d;
        ...
        if (pool_type == pg_pool_t::TYPE_ERASURE) {
            ...
            cmd_getval(cmdmap, "expected_num_objects", expected_num_objects, int64_t(0));
        }
        else {
        ...
        if (err < 0) {
            switch (err) {
            case -EEXIST: ss << "pool '" << poolstr << "' already exists"; break;
            case -EAGAIN: wait_for_finished_proposal(op, new C_RetryMessage(this, op)); return true;
            case -ERANGE: goto reply;
            default: goto reply; break;
            }
        }
        else {
        ...
        if (pstats && pstats->stats.sum.num_objects != 0 && force_nonempty != "--force-nonempty") {
        ...
        if ((!tp->removed_snaps.empty() || !tp->snaps.empty()) &&
            ((force_nonempty != "--force-nonempty") || (!g_conf()->mon_debug_unsafe_allow_tier_with_nonempty_snaps))) {
