...
{
    return *_dout << "mon." << mon.name << "@" << mon.rank << "(" << mon.get_state_name() << ").mgrstat ";
}
...
void MgrStatMonitor::update_from_paxos(bool* need_bootstrap)
{
    ...
    if (version) {
        ...
        try {
            ...
            dout(10) << __func__ << " v" << version << " service_map e" << service_map.epoch << " "
                     << progress_events.size() << " progress events" << dendl;
        }
        catch (ceph::buffer::error& e) {
            derr << "failed to decode mgrstat state; luminous dev version? " << e.what() << dendl;
        }
    }
    check_subs();
    update_logger();
...
void MgrStatMonitor::update_logger()
{
    ...
    mon.cluster_logger->set(l_cluster_osd_bytes_used, digest.osd_sum.statfs.get_used_raw());
    mon.cluster_logger->set(l_cluster_osd_bytes_avail, digest.osd_sum.statfs.available);
    ...
    for (auto p = digest.num_pg_by_state.begin(); p != digest.num_pg_by_state.end(); ++p) {
        if (p->first & PG_STATE_ACTIVE) {
            active += p->second;
            if (p->first & PG_STATE_CLEAN) active_clean += p->second;
        }
        if (p->first & PG_STATE_PEERING) peering += p->second;
    }
...
bool MgrStatMonitor::preprocess_query(MonOpRequestRef op)
{
    ...
    switch (m->get_type()) {
    case CEPH_MSG_STATFS: return preprocess_statfs(op);
    case MSG_MON_MGR_REPORT: return preprocess_report(op);
    case MSG_GETPOOLSTATS: return preprocess_getpoolstats(op);
    default:
        mon.no_reply(op);
...
bool MgrStatMonitor::prepare_update(MonOpRequestRef op)
{
    auto m = op->get_req<PaxosServiceMessage>();
    switch (m->get_type()) {
    case MSG_MON_MGR_REPORT: return prepare_report(op);
    default:
        mon.no_reply(op);
        derr << "Unhandled message type " << m->get_type() << dendl;
        return true;
...
bool MgrStatMonitor::prepare_report(MonOpRequestRef op)
{
    ...
    dout(10) << __func__ << " " << pending_digest << ", " << pending_health_checks.checks.size() << " health checks, "
             << progress_events.size() << " progress events" << dendl;
...
bool MgrStatMonitor::preprocess_getpoolstats(MonOpRequestRef op)
{
    ...
    for (const auto& pool_name : m->pools) {
        ...
        if (pool_id == -ENOENT) continue;
        auto pool_stat = get_pool_stat(pool_id);
        if (!pool_stat) continue;
...
{
    dout(10) << __func__ << " next " << sub->next << " vs service_map.epoch " << service_map.epoch << dendl;
