...
void usage()
{
    cout << "usage: ceph-fuse [-n client.username] [-m mon-ip-addr:mon-port] <mount point> [OPTIONS]\n"
...
int main(int argc, const char** argv, const char* envp[])
{
    ...
    std::map<std::string, std::string> defaults = {
        {"pid_file", ""}, {"chdir", "/"}   // FUSE will chdir("/"); be ready.
    };
    ...
    for (auto i = args.begin(); i != args.end();) {
        if (ceph_argparse_double_dash(args, i)) {
            ...
            const char* tmpargv[] = {"ceph-fuse", "-V"};
...

    {
        ...
        public:
            ...
            {
#if defined(__linux__)
                bool can_invalidate_dentries = g_conf().get_val<bool>("client_try_dentry_invalidate");
                uint64_t max_retries = g_conf().get_val<uint64_t>("client_max_retries_on_remount_failure");
                ...
                bool client_die_on_failed_dentry_invalidate =
                    g_conf().get_val<bool>("client_die_on_failed_dentry_invalidate");
                if (tr != 0 && client_die_on_failed_dentry_invalidate) {
                    cerr << "ceph-fuse[" << getpid() << "]: fuse failed dentry invalidate/remount test with error "
                    ...
                    if (umount_r) {
                        if (umount_r != -1) {
                            if (WIFEXITED(umount_r)) {
                                ...
                                cerr << "got error " << umount_r << " when unmounting Ceph on failed remount test!"
                                     << std::endl;
                            }
                            else {
        ...
        if (r < 0) goto out_mc_start_failed;
        ...
        messenger->set_policy(entity_name_t::TYPE_MDS, Messenger::Policy::lossless_client(0));
        ...
        perms = client->pick_my_perms();
        {
            ...
            auto client_mountpoint = g_conf().get_val<std::string>("client_mountpoint");
            auto mountpoint = client_mountpoint.c_str();
            auto fuse_require_active_mds = g_conf().get_val<bool>("fuse_require_active_mds");
        ...
        cerr << "ceph-fuse[" << getpid() << "]: fuse finished with error " << r << " and tester_r " << tester_r
             << std::endl;
