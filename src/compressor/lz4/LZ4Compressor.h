...
public:
                 ...
                 std::optional<int32_t>& compressor_message) override
    {
...
#endif
        ceph::buffer::ptr outptr = ceph::buffer::create_small_page_aligned(LZ4_compressBound(src.length()));
        ...
        while (left) {
            ...
            int compressed_len = LZ4_compress_fast_continue(
                &lz4_stream, data, outptr.c_str() + pos, origin_len, outptr.length() - pos, 1);
    ...
    int decompress(ceph::buffer::list::const_iterator& p, size_t compressed_len, ceph::buffer::list& dst,
                   std::optional<int32_t> compressor_message) override
    {
#ifdef HAVE_QATZIP
