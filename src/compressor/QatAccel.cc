...
bool QatAccel::init(const std::string& alg)
{
    ...
    if (rc != QZ_OK && rc != QZ_DUPLICATE && rc != QZ_NO_HW) return false;
...
int QatAccel::decompress(bufferlist::const_iterator& p, size_t compressed_len, bufferlist& dst,
                         boost::optional<int32_t> compressor_message)
{
    ...
    while (remaining) {
        ...
        if (joint) {
            if (read_more) tmp.append(cur_ptr.c_str(), len);
            len = tmp.length();
        }
        ...
        if (joint)
            ...
            rc = qzDecompress(
                &session, (const unsigned char*)cur_ptr.c_str(), &len, (unsigned char*)ptr.c_str(), &out_len);
        if (rc == QZ_DATA_ERROR) {
            if (!joint) {
            ...
            if (ratio_idx == std::size(expansion_ratio)) return -1;
            if (joint) read_more = false;
            ratio_idx++;
            continue;
        }
        else if (rc != QZ_OK) {
