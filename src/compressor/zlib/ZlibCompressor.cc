...
int ZlibCompressor::zlib_compress(const bufferlist& in, bufferlist& out, boost::optional<int32_t>& compressor_message)
{
    ...
    ret = deflateInit2(&strm,
                       cct->_conf->compressor_zlib_level,
                       Z_DEFLATED,
                       cct->_conf->compressor_zlib_winsize,
                       ZLIB_MEMORY_LEVEL,
                       Z_DEFAULT_STRATEGY);
    if (ret != Z_OK) {
...
{
#ifdef HAVE_QATZIP
    if (qat_enabled) return qat_accel.compress(in, out, compressor_message);
#endif
#if __x86_64__ && defined(HAVE_NASM_X64_AVX2)
    if (isal_enabled)
...
{
#ifdef HAVE_QATZIP
    if (qat_enabled) return qat_accel.decompress(p, compressed_size, out, compressor_message);
#endif
    ...
    if (!compressor_message) compressor_message = ZLIB_DEFAULT_WIN_SIZE;
...
{
#ifdef HAVE_QATZIP
    if (qat_enabled) return qat_accel.decompress(in, out, compressor_message);
#endif
