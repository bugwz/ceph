...
protected:
    CompletionData(libradosstriper::RadosStriperImpl* striper, const std::string& soid, const std::string& lockCookie,
                   librados::AioCompletionImpl* userCompletion = 0);
    ~CompletionData() override;
};

CompletionData::CompletionData(libradosstriper::RadosStriperImpl* striper, const std::string& soid,
                               const std::string& lockCookie, librados::AioCompletionImpl* userCompletion)
...
private:
    ...
    ReadCompletionData(libradosstriper::RadosStriperImpl* striper, const std::string& soid,
                       const std::string& lockCookie, librados::AioCompletionImpl* userCompletion, bufferlist* bl,
                       std::vector<ObjectExtent>* extents, std::vector<bufferlist>* resultbl);
    ~ReadCompletionData() override;
};

ReadCompletionData::ReadCompletionData(libradosstriper::RadosStriperImpl* striper, const std::string& soid,
                                       const std::string& lockCookie, librados::AioCompletionImpl* userCompletion,
                                       bufferlist* bl, std::vector<ObjectExtent>* extents,
...
void ReadCompletionData::complete_read(int r)
{
    ...
    for (vector<ObjectExtent>::iterator eit = m_extents->begin(); eit != m_extents->end(); ++eit, ++bit) {
...
WriteCompletionData::WriteCompletionData(libradosstriper::RadosStriperImpl* striper, const std::string& soid,
                                         const std::string& lockCookie, librados::AioCompletionImpl* userCompletion)
    : CompletionData(striper, soid, lockCookie, userCompletion)
    , m_safe(0)
    , m_unlockCompletion(0)
...
private:
     ...
     */
    RemoveCompletionData(libradosstriper::RadosStriperImpl* striper, const std::string& soid,
                         const std::string& lockCookie, librados::AioCompletionImpl* userCompletion, int flags = 0)
        : CompletionData(striper, soid, lockCookie, userCompletion)
        , flags(flags)
    {}
};
...
private:
    ...
    /// constructor
    TruncateCompletionData(libradosstriper::RadosStriperImpl* striper, const std::string& soid, uint64_t size)
        : RefCountedObject(striper->cct())
        , m_striper(striper)
        , m_soid(soid)
        , m_size(size)
    ...
    /// destructor
    ~TruncateCompletionData() override { m_striper->put(); }
};
...
private:
    ...
    /// constructor
    RadosReadCompletionData(MultiAioCompletionImplPtr multiAioCompl, uint64_t expectedBytes, bufferlist* bl,
                            CephContext* context)
...
protected:
    /// constructor
    BasicStatCompletionData(libradosstriper::RadosStriperImpl* striper, const std::string& soid,
                            librados::AioCompletionImpl* userCompletion,
                            libradosstriper::MultiAioCompletionImpl* multiCompletion, uint64_t* psize)
        : CompletionData(striper, soid, "", userCompletion)
        , m_multiCompletion(multiCompletion)
        , m_psize(psize)
        , m_statRC(0)
        , m_getxattrRC(0){};
};
 ...
 */
template<class TimeType> struct StatCompletionData : BasicStatCompletionData
{
...
private:
    ...
    /// constructor
    StatCompletionData(libradosstriper::RadosStriperImpl* striper, const std::string& soid,
                       librados::AioCompletionImpl* userCompletion,
                       libradosstriper::MultiAioCompletionImpl* multiCompletion, uint64_t* psize, TimeType* pmtime)
...
private:
    ...
    /// constructor
    RadosRemoveCompletionData(MultiAioCompletionImplPtr multiAioCompl, CephContext* context)
        : RefCountedObject(context)
        , m_multiAioCompl(multiAioCompl){};
};
...
int libradosstriper::RadosStriperImpl::setObjectLayoutStripeUnit(unsigned int stripe_unit)
{
    /* stripe unit must be non-zero, 64k increment */
    if (!stripe_unit || (stripe_unit & (CEPH_MIN_STRIPE_UNIT - 1))) return -EINVAL;
    m_layout.fl_stripe_unit = stripe_unit;
    return 0;
}

int libradosstriper::RadosStriperImpl::setObjectLayoutStripeCount(unsigned int stripe_count)
{
    /* stripe count must be non-zero */
    if (!stripe_count) return -EINVAL;
    m_layout.fl_stripe_count = stripe_count;
    return 0;
}

int libradosstriper::RadosStriperImpl::setObjectLayoutObjectSize(unsigned int object_size)
{
    /* object size must be non-zero, 64k increment */
    if (!object_size || (object_size & (CEPH_MIN_STRIPE_UNIT - 1))) return -EINVAL;
    /* object size must be a multiple of stripe unit */
    if (object_size < m_layout.fl_stripe_unit || object_size % m_layout.fl_stripe_unit) return -EINVAL;
...
int libradosstriper::RadosStriperImpl::getxattr(const object_t& soid, const char* name, bufferlist& bl)
{
    std::string firstObjOid = getObjectId(soid, 0);
    return m_ioCtx.getxattr(firstObjOid, name, bl);
}

int libradosstriper::RadosStriperImpl::setxattr(const object_t& soid, const char* name, bufferlist& bl)
{
    std::string firstObjOid = getObjectId(soid, 0);
    return m_ioCtx.setxattr(firstObjOid, name, bl);
}

int libradosstriper::RadosStriperImpl::getxattrs(const object_t& soid, map<string, bufferlist>& attrset)
{
    std::string firstObjOid = getObjectId(soid, 0);
...
int libradosstriper::RadosStriperImpl::rmxattr(const object_t& soid, const char* name)
{
    std::string firstObjOid = getObjectId(soid, 0);
...
int libradosstriper::RadosStriperImpl::write(const std::string& soid, const bufferlist& bl, size_t len, uint64_t off)
{
...
int libradosstriper::RadosStriperImpl::append(const std::string& soid, const bufferlist& bl, size_t len)
{
    // open the object. This will create it if needed, retrieve its layout
    // and size and take a shared lock on it
    ceph_file_layout layout;
...
int libradosstriper::RadosStriperImpl::write_full(const std::string& soid, const bufferlist& bl)
{
    int rc = trunc(soid, 0);
...
int libradosstriper::RadosStriperImpl::read(const std::string& soid, bufferlist* bl, size_t len, uint64_t off)
{
...
int libradosstriper::RadosStriperImpl::aio_write(const std::string& soid, librados::AioCompletionImpl* c,
...
int libradosstriper::RadosStriperImpl::aio_append(const std::string& soid, librados::AioCompletionImpl* c,
...
int libradosstriper::RadosStriperImpl::aio_write_full(const std::string& soid, librados::AioCompletionImpl* c,
                                                      const bufferlist& bl)
{
    int rc = trunc(soid, 0);
    if (rc) return rc;
...
static void rados_read_aio_unlock_complete(rados_striper_multi_completion_t c, void* arg)
{
    ...
    libradosstriper::MultiAioCompletionImpl* comp = reinterpret_cast<libradosstriper::MultiAioCompletionImpl*>(c);
...
int libradosstriper::RadosStriperImpl::aio_read(const std::string& soid, librados::AioCompletionImpl* c, bufferlist* bl,
                                                size_t len, uint64_t off)
{
    ...
    if (read_len > 0) {
        ...
        Striper::file_to_extents(cct(), format.c_str(), &l, off, read_len, 0, *extents);
    }
    ...
    MultiAioCompletionImplPtr nc{new libradosstriper::MultiAioCompletionImpl, false};
    ...
    for (vector<ObjectExtent>::iterator p = extents->begin(); p != extents->end(); ++p) {
        ...
        for (vector<pair<uint64_t, uint64_t>>::iterator q = p->buffer_extents.begin(); q != p->buffer_extents.end();
             ++q) {
...
int libradosstriper::RadosStriperImpl::aio_read(const std::string& soid, librados::AioCompletionImpl* c, char* buf,
                                                size_t len, uint64_t off)
{
...
static void striper_stat_aio_req_complete(rados_striper_multi_completion_t c, void* arg)
{
    auto data = ceph::ref_t<BasicStatCompletionData>(static_cast<BasicStatCompletionData*>(arg), false);
    if (data->m_statRC) {
...
int libradosstriper::RadosStriperImpl::aio_generic_stat(
    const std::string& soid, librados::AioCompletionImpl* c, uint64_t* psize, TimeType* pmtime,
    typename libradosstriper::RadosStriperImpl::StatFunction<TimeType>::Type statFunction)
{
    ...
    MultiAioCompletionImplPtr multi_completion{new libradosstriper::MultiAioCompletionImpl, false};
    // Data object used for passing context to asynchronous calls
    std::string firstObjOid = getObjectId(soid, 0);
    auto cdata =
        ceph::make_ref<StatCompletionData<TimeType>>(this, firstObjOid, c, multi_completion.get(), psize, pmtime);
    ...
    int rc = (m_ioCtxImpl->*statFunction)(obj, stat_completion->pc, &cdata->m_objectSize, cdata->m_pmtime);
    ...
    rc = m_ioCtxImpl->aio_getxattr(obj, getxattr_completion->pc, XATTR_SIZE, cdata->m_bl);
...
int libradosstriper::RadosStriperImpl::aio_stat(const std::string& soid, librados::AioCompletionImpl* c,
...
int libradosstriper::RadosStriperImpl::aio_stat2(const std::string& soid, librados::AioCompletionImpl* c,
...
static void striper_remove_aio_req_complete(rados_striper_multi_completion_t c, void* arg)
{
    ...
    libradosstriper::MultiAioCompletionImpl* comp = reinterpret_cast<libradosstriper::MultiAioCompletionImpl*>(c);
    ldout(cdata->m_striper->cct(), 10) << "RadosStriperImpl : striper_remove_aio_req_complete called for "
                                       << cdata->m_soid << dendl;
    int rc = comp->rval;
    if (rc == 0) {
        // All went fine, synchronously remove first object
        rc = cdata->m_striper->m_ioCtx.remove(cdata->m_striper->getObjectId(cdata->m_soid, 0), cdata->flags);
    }
    else {
        lderr(cdata->m_striper->cct()) << "RadosStriperImpl : deletion/truncation incomplete for " << cdata->m_soid
                                       << ", as errors were encountered. The file is left present but it's content "
                                       << " has been partially removed" << dendl;
    }
    cdata->complete(rc);
}
...
int libradosstriper::RadosStriperImpl::aio_remove(const std::string& soid, librados::AioCompletionImpl* c, int flags)
{
...
int libradosstriper::RadosStriperImpl::internal_aio_remove(const std::string& soid,
                                                           MultiAioCompletionImplPtr multi_completion, int flags)
{
    std::string firstObjOid = getObjectId(soid, 0);
    try {
        ...
        for (int i = nb_objects - 1; i >= 1; i--) {
            ...
            librados::AioCompletion* rados_completion =
                librados::Rados::aio_create_completion(data->get() /* create ref! */, rados_req_remove_complete);
            if (flags == 0) {
            ...
            if (rcr < 0 and -ENOENT != rcr) {
                lderr(cct()) << "RadosStriperImpl::remove : deletion incomplete for " << soid << ", as "
                             << getObjectId(soid, i) << " could not be deleted (rc=" << rc << ")" << dendl;
                break;
            }
        }
...
std::string libradosstriper::RadosStriperImpl::getObjectId(const object_t& soid, long long unsigned objectno)
{
    std::ostringstream s;
...
void libradosstriper::RadosStriperImpl::aio_unlockObject(const std::string& soid, const std::string& lockCookie,
...
int libradosstriper::RadosStriperImpl::write_in_open_object(const std::string& soid, const ceph_file_layout& layout,
                                                            const std::string& lockCookie, const bufferlist& bl,
                                                            size_t len, uint64_t off)
{
    ...
    MultiAioCompletionImplPtr c{new libradosstriper::MultiAioCompletionImpl, false};
...
int libradosstriper::RadosStriperImpl::aio_write_in_open_object(const std::string& soid, librados::AioCompletionImpl* c,
                                                                ...
                                                                size_t len, uint64_t off)
{
    ...
    cdata->m_unlockCompletion = unlock_completion;
    // create the multicompletion that will handle the write completion
    libradosstriper::MultiAioCompletionImplPtr nc{new libradosstriper::MultiAioCompletionImpl, false};
...
int libradosstriper::RadosStriperImpl::internal_aio_write(const std::string& soid,
                                                          ...
                                                          const ceph_file_layout& layout)
{
    ...
    if (len > 0) {
        ...
        for (vector<ObjectExtent>::iterator p = extents.begin(); p != extents.end(); ++p) {
            ...
            c->add_request();
            librados::AioCompletion* rados_completion =
                librados::Rados::aio_create_completion(c.get(), rados_req_write_complete);
...
int libradosstriper::RadosStriperImpl::extract_uint32_attr(std::map<std::string, bufferlist>& attrs,
                                                           const std::string& key, ceph_le32* value)
{
...
int libradosstriper::RadosStriperImpl::extract_sizet_attr(std::map<std::string, bufferlist>& attrs,
...
int libradosstriper::RadosStriperImpl::internal_get_layout_and_size(const std::string& oid, ceph_file_layout* layout,
                                                                    uint64_t* size)
{
...
int libradosstriper::RadosStriperImpl::openStripedObjectForRead(const std::string& soid, ceph_file_layout* layout,
                                                                uint64_t* size, std::string* lockCookie)
{
    ...
    *lockCookie = getUUID();
    ...
    if (rc) {
        ...
        lderr(cct()) << "RadosStriperImpl::openStripedObjectForRead : "
                     << "could not load layout and size for " << soid << " : rc = " << rc << dendl;
    }
    return rc;
}

int libradosstriper::RadosStriperImpl::openStripedObjectForWrite(const std::string& soid, ceph_file_layout* layout,
                                                                 uint64_t* size, std::string* lockCookie,
                                                                 bool isFileSizeAbsolute)
{
    ...
    *lockCookie = getUUID();
    ...
    if (!isFileSizeAbsolute) *size += curSize;
    ...
    *size = curSize;
    ...
    if (-ECANCELED == rc) rc = 0;
    if (rc) {
        unlockObject(soid, *lockCookie);
        lderr(cct()) << "RadosStriperImpl::openStripedObjectForWrite : "
                     << "could not set new size for " << soid << " : rc = " << rc << dendl;
    }
    return rc;
}

int libradosstriper::RadosStriperImpl::createAndOpenStripedObject(const std::string& soid, ceph_file_layout* layout,
                                                                  uint64_t size, std::string* lockCookie,
                                                                  bool isFileSizeAbsolute)
{
...
int libradosstriper::RadosStriperImpl::truncate(const std::string& soid, uint64_t original_size, uint64_t size,
                                                ceph_file_layout& layout)
{
    auto cdata = ceph::make_ref<TruncateCompletionData>(this, soid, size);
    libradosstriper::MultiAioCompletionImplPtr multi_completion{new libradosstriper::MultiAioCompletionImpl, false};
...
int libradosstriper::RadosStriperImpl::aio_truncate(const std::string& soid,
                                                    libradosstriper::MultiAioCompletionImplPtr multi_completion,
                                                    uint64_t original_size, uint64_t size, ceph_file_layout& layout)
{
...
int libradosstriper::RadosStriperImpl::grow(const std::string& soid, uint64_t original_size, uint64_t size,
                                            ceph_file_layout& layout)
{
