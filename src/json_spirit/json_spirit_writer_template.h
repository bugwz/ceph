...
template<class String_type> String_type non_printable_to_string(unsigned int c)
{
...
template<typename Char_type, class String_type> bool add_esc_char(Char_type c, String_type& s)
{
    switch (c) {
...
template<class String_type> String_type add_esc_chars(const String_type& s, bool raw_utf8)
{
...
template<class Ostream> void append_double(Ostream& os, const double d, const int precision)
{
    os << std::showpoint << std::setprecision(precision) << d;
}

template<class String_type> void erase_and_extract_exponent(String_type& str, String_type& exp)
{
...
template<class String_type> typename String_type::size_type find_first_non_zero(const String_type& str)
{
...
template<class String_type> void remove_trailing(String_type& str)
{
    String_type exp;
    ...
    if (first_non_zero != 0) {
        const int offset =
            str[first_non_zero] == '.' ? 2 : 1;   // note zero digits following a decimal point is non standard
...
private:
    ...
    void output(const String_type& s) { os_ << '"' << add_esc_chars(s, raw_utf8_) << '"'; }
    ...
    template<class Iter> void output_composite_item(Iter i, Iter last)
    {
        output(*i);
    ...
    template<class T> void output_array_or_obj(const T& t, Char_type start_char, Char_type end_char)
    {
        os_ << start_char;
        new_line();
