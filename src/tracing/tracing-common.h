...
#define ceph_ctf_integerp(type, field, val) \
    ctf_integer(type, field, (val) == NULL ? 0 : (val)) ctf_integer(uint8_t, field##_isnull, (val) == NULL)

// val should have type char*
#define ceph_ctf_string(field, val) \
    ctf_string(field, (val) == NULL ? "" : (val)) ctf_integer(uint8_t, field##_isnull, (val) == NULL)
...
#define ceph_ctf_sequence(type, field, val, lentype, lenval)                                                          \
    ctf_integer_hex(void*, field, val) ctf_sequence(type,                                                             \
                                                    field##_data,                                                     \
                                                    (val) == NULL ? "" : (val),                                       \
                                                    lentype,                                                          \
                                                    (val) == NULL ? 0 : CEPH_MIN((lenval), CEPH_TRACE_BUF_TRUNC_LEN)) \
        ctf_integer(uint8_t, field##_isnull, (val) == NULL) ctf_integer(lentype, field##_len, lenval)
#else
#define ceph_ctf_sequence(type, field, val, lentype, lenval)                               \
    ctf_integer_hex(void*, field, val) ctf_integer(uint8_t, field##_isnull, (val) == NULL) \
...
#define ceph_ctf_sequencep(type, field, val, lentype, lenval)                                                      \
    ctf_integer_hex(void*, field, val) ctf_sequence(                                                               \
        type,                                                                                                      \
        field##_data,                                                                                              \
...
#define ceph_ctf_timevalp(field, p)                                                \
    ctf_integer(long int, field##_sec, (p) == NULL ? 0 : (p)->tv_sec) ctf_integer( \
        long int, field##_usec, (p) == NULL ? 0 : (p)->tv_usec) ctf_integer(uint8_t, field##_isnull, (p) == NULL)

// p should be of type struct timespec*
#define ceph_ctf_timespecp(field, p)                                               \
    ctf_integer(long int, field##_sec, (p) == NULL ? 0 : (p)->tv_sec) ctf_integer( \
        long int, field##_nsec, (p) == NULL ? 0 : (p)->tv_nsec) ctf_integer(uint8_t, field##_isnull, (p) == NULL)
...
#define ceph_ctf_time_t(field, val) ctf_integer(uint64_t, field, (uint64_t)(val))
