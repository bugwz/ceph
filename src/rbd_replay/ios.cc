...
bool compare_dependencies_by_start_time(const action::Dependency& lhs, const action::Dependency& rhs)
{
    return lhs.time_delta < rhs.time_delta;
}

action::Dependencies convert_dependencies(uint64_t start_time, const io_set_t& deps)
{
    ...
    std::sort(action_deps.begin(), action_deps.end(), compare_dependencies_by_start_time);
    return action_deps;
}
...
void StartThreadIO::encode(bufferlist& bl) const
{
    ...
    action::Action action(
        (action::StartThreadAction(ionum(), thread_id(), convert_dependencies(start_time(), dependencies()))));
    encode(action, bl);
}
...
void StopThreadIO::encode(bufferlist& bl) const
{
    using ceph::encode;
    action::Action action(
        (action::StopThreadAction(ionum(), thread_id(), convert_dependencies(start_time(), dependencies()))));
    encode(action, bl);
}
...
void ReadIO::encode(bufferlist& bl) const
{
    ...
    action::Action action((action::ReadAction(
        ionum(), thread_id(), convert_dependencies(start_time(), dependencies()), m_imagectx, m_offset, m_length)));
    encode(action, bl);
}
...
{
    using ceph::encode;
    action::Action action((action::OpenImageAction(ionum(),
                                                   thread_id(),
...
void OpenImageIO::write_debug(std::ostream& out) const
{
    ...
    out << ", imagectx=" << m_imagectx << ", name='" << m_name << "', snap_name='" << m_snap_name
        << "', readonly=" << m_readonly;
}
...
void CloseImageIO::encode(bufferlist& bl) const
{
    ...
    action::Action action((action::CloseImageAction(
        ionum(), thread_id(), convert_dependencies(start_time(), dependencies()), m_imagectx)));
    encode(action, bl);
}
...
{
    using ceph::encode;
    action::Action action((action::AioOpenImageAction(ionum(),
                                                      thread_id(),
                                                      convert_dependencies(start_time(), dependencies()),
                                                      m_imagectx,
                                                      m_name,
