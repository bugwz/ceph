...
struct ConstructVisitor : public boost::static_visitor<Action::ptr>
{
    ...
    inline Action::ptr operator()(const action::UnknownAction& action) const { return Action::ptr(); }
};
...
void OpenImageAction::perform(ActionCtx& worker)
{
    ...
    if (r) {
        cerr << "Unable to open image '" << m_action.name << "' with snap '" << m_action.snap_name << "' (mapped to '"
             << name.str() << "') and readonly " << m_action.read_only << ": (" << -r << ") " << strerror(-r)
             << std::endl;
        exit(1);
    }
