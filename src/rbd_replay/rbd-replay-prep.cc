...
public:
    ...
    uint64_t max_ts() const { return m_max_ts; }
    ...
    thread_id_t id() const { return m_id; }

    IO::ptr latest_io() { return m_latest_io; }
...
public:
    ...
    string image_name() const { return m_image_name; }
...
public:
    Processor()
        : m_window(1000000000ULL)
        ,   // 1 billion nanoseconds, i.e., one second
        m_io_count(0)
        , m_anonymize(false)
        , m_verbose(false)
    {}
    ...
    void run(vector<string> args)
    {
        ...
        ASSERT_EXIT(start_time_ns != -1ULL, "Error extracting creation time from trace");
        ...
        ASSERT_EXIT(fd >= 0, "Error opening output file " << output_file_name << ": " << cpp_strerror(errno));
        BOOST_SCOPE_EXIT((fd))
        {
            close(fd);
        }
        ...
        while (true) {
            ...
            ts -= trace_start;
            ts += 4;   // This is so we have room to insert two events (thread start and open image) at unique
                       // timestamps before whatever the first event is.
...
private:
    ...
    {
        IO::ptrs ios;
        for (map<thread_id_t, Thread::ptr>::const_iterator itr = m_threads.begin(), end = m_threads.end(); itr != end;
             ++itr) {
            Thread::ptr thread(itr->second);
            ios.push_back(IO::ptr(new StopThreadIO(next_id(), thread->max_ts(), thread->id(), m_recent_completions)));
        }
    ...
    void process_event(uint64_t ts, struct bt_ctf_event* evt, IO::ptrs* ios)
    {
        ...
        const struct bt_definition* scope_context = bt_ctf_get_top_level_scope(evt, BT_STREAM_EVENT_CONTEXT);
        ASSERT_EXIT(scope_context != NULL, "Error retrieving event context");

        const struct bt_definition* scope_fields = bt_ctf_get_top_level_scope(evt, BT_EVENT_FIELDS);
        ...
        {
        public:
            FieldLookup(struct bt_ctf_event* evt, const struct bt_definition* scope)
                : m_evt(evt)
                , m_scope(scope)
            {}
            ...
            const char* string(const char* name)
            {
                ...
                ASSERT_EXIT(c && err == 0, "Error retrieving field value '" << name << "': error=" << err);
                return c;
            }
            ...
            int64_t int64(const char* name)
            {
                ...
                int err = bt_ctf_field_get_error();
                ASSERT_EXIT(err == 0, "Error retrieving field value '" << name << "': error=" << err);
                return val;
            }
        ...
        if (strcmp(event_name, "librbd:open_image_enter") == 0) {
            ...
            IO::ptr io(new OpenImageIO(
                ionum, ts, threadID, m_recent_completions, imagectx, aname.first, aname.second, readonly));
            ...
            imagectx_id_t imagectx = fields.uint64("imagectx");
            action_id_t ionum = next_id();
            IO::ptr io(new CloseImageIO(ionum, ts, threadID, m_recent_completions, imagectx));
            thread->issued_io(io, &m_latest_ios);
            ...
            uint64_t completion = fields.uint64("completion");
            action_id_t ionum = next_id();
            IO::ptr io(new AioCloseImageIO(ionum, ts, threadID, m_recent_completions, imagectx));
            thread->issued_io(io, &m_latest_ios);
            ios->push_back(thread->latest_io());
            m_pending_ios[completion] = io;
        }
        else if (strcmp(event_name, "librbd:read_enter") == 0 || strcmp(event_name, "librbd:read2_enter") == 0) {
            string name(fields.string("name"));
            string snap_name(fields.string("snap_name"));
            ...
            action_id_t ionum = next_id();
            IO::ptr io(new ReadIO(ionum, ts, threadID, m_recent_completions, imagectx, offset, length));
            thread->issued_io(io, &m_latest_ios);
            ios->push_back(io);
        }
        ...
        else if (strcmp(event_name, "librbd:write_enter") == 0 || strcmp(event_name, "librbd:write2_enter") == 0) {
            string name(fields.string("name"));
            string snap_name(fields.string("snap_name"));
            bool readonly = fields.int64("read_only");
            ...
            require_image(ts, thread, imagectx, name, snap_name, readonly, ios);
            action_id_t ionum = next_id();
            IO::ptr io(new WriteIO(ionum, ts, threadID, m_recent_completions, imagectx, offset, length));
            thread->issued_io(io, &m_latest_ios);
            ios->push_back(io);
        }
            ...
            require_image(ts, thread, imagectx, name, snap_name, readonly, ios);
            action_id_t ionum = next_id();
            IO::ptr io(new DiscardIO(ionum, ts, threadID, m_recent_completions, imagectx, offset, length));
            thread->issued_io(io, &m_latest_ios);
            ios->push_back(io);
        }
            ...
            require_image(ts, thread, imagectx, name, snap_name, readonly, ios);
            action_id_t ionum = next_id();
            IO::ptr io(new AioReadIO(ionum, ts, threadID, m_recent_completions, imagectx, offset, length));
            ios->push_back(io);
            thread->issued_io(io, &m_latest_ios);
            m_pending_ios[completion] = io;
        }
            ...
            require_image(ts, thread, imagectx, name, snap_name, readonly, ios);
            action_id_t ionum = next_id();
            IO::ptr io(new AioWriteIO(ionum, ts, threadID, m_recent_completions, imagectx, offset, length));
            thread->issued_io(io, &m_latest_ios);
            ios->push_back(io);
            m_pending_ios[completion] = io;
        }
            ...
            require_image(ts, thread, imagectx, name, snap_name, readonly, ios);
            action_id_t ionum = next_id();
            IO::ptr io(new AioDiscardIO(ionum, ts, threadID, m_recent_completions, imagectx, offset, length));
            thread->issued_io(io, &m_latest_ios);
            ios->push_back(io);
            m_pending_ios[completion] = io;
        }
    ...
    void completed(IO::ptr io)
    {
        ...
        for (io_set_t::iterator itr = m_recent_completions.begin(); itr != m_recent_completions.end();) {
            ...
            if ((recent_comp->start_time() < limit || io->dependencies().count(recent_comp) != 0) &&
    ...
    void require_image(uint64_t ts, Thread::ptr thread, imagectx_id_t imagectx, const string& name,
                       const string& snap_name, bool readonly, IO::ptrs* ios)
    {
        ceph_assert(thread);
        ...
        pair<string, string> aname(map_image_snap(name, snap_name));
        IO::ptr io(new OpenImageIO(
            ionum, ts - 2, thread->id(), m_recent_completions, imagectx, aname.first, aname.second, readonly));
        thread->issued_io(io, &m_latest_ios);
        ios->push_back(io);
        completed(io);
