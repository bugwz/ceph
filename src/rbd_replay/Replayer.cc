...
void Worker::run()
{
    ...
    }
    {
        ...
        while (!m_pending_ios.empty()) {
            if (!first_time) {
                dout(THREAD_LEVEL) << "Worker thread trying to stop, still waiting for " << m_pending_ios.size()
                                   << " pending IOs to complete:" << dendl;
...
{
    {
        ...
        if (m_pool_name.empty()) {
            ...
            if (r < 0) {
                cerr << "Failed to retrieve default pool: " << cpp_strerror(r) << std::endl;
                goto out;
            }
        }
        ...
        m_ioctx = new librados::IoCtx();
        {
            ...
            if (r) {
                cerr << "Failed to open pool " << m_pool_name << ": " << cpp_strerror(r) << std::endl;
                goto out2;
            }
            ...
            if (fd < 0) {
                std::cerr << "Failed to open " << replay_file << ": " << cpp_strerror(errno) << std::endl;
                exit(1);
            }
            ...
            while (true) {
                ...
                try {
                    ...
                    int r = buffer_reader.fetch(&it);
                    if (r < 0) {
                        std::cerr << "Failed to read from trace file: " << cpp_strerror(r) << std::endl;
                        exit(-r);
                    }
...
void Replayer::wait_for_actions(const action::Dependencies& deps)
{
    ...
    for (auto& dep : deps) {
        ...
        auto sub_release_time{action_completed_time + std::chrono::microseconds{static_cast<long long>(
                                                          dep.time_delta * m_latency_multiplier / 1000)}};
    ...
    if (release_time > std::chrono::system_clock::now()) {
        ...
        dout(SLEEP_LEVEL) << "Sleeping for " << std::chrono::duration_cast<std::chrono::microseconds>(sleep_for).count()
                          << " microseconds" << dendl;
