...
public:
    ...
    struct dn_weak
    {
        ...
        inodeno_t ino = 0;
        dn_weak() = default;
        dn_weak(snapid_t f, inodeno_t pi)
            : first(f)
            , ino(pi)
        {}
        void encode(ceph::buffer::list& bl) const
    ...
    void print(std::ostream& out) const override { out << "cache_rejoin " << get_opname(op); }

    // -- builders --
    // inodes
    void add_weak_inode(vinodeno_t i) { weak_inodes.insert(i); }
    ...
    {
        if (inode_scatterlocks.count(in->ino())) return;   // already added this one
    ...
    // dirfrags
    void add_strong_dirfrag(dirfrag_t df, int n, int dr) { strong_dirfrags[df] = dirfrag_strong(n, dr); }
    ...
    // dentries
    void add_weak_dirfrag(dirfrag_t df) { weak_dirfrags.insert(df); }
    ...
    void add_strong_dentry(dirfrag_t df, std::string_view dname, std::string_view altn, snapid_t first, snapid_t last,
                           inodeno_t pi, inodeno_t ri, unsigned char rdt, int n, int ls)
    {
        auto& m = strong_dentries[df];
        m.insert_or_assign(string_snap_t(dname, last), dn_strong(first, altn, pi, ri, rdt, n, ls));
    }
    void add_dentry_authpin(dirfrag_t df, std::string_view dname, snapid_t last, const metareqid_t& ri, __u32 attempt)
    {
        authpinned_dentries[df][string_snap_t(dname, last)].push_back(peer_reqid(ri, attempt));
    }
    void add_dentry_xlock(dirfrag_t df, std::string_view dname, snapid_t last, const metareqid_t& ri, __u32 attempt)
    {
    ...
    void decode_payload() override
    {
        ...
        if (header.version >= 2) decode(client_metadata_map, p);
    }
...
private:
    ...
    MMDSCacheRejoin()
        : MMDSOp{MSG_MDS_CACHEREJOIN, HEAD_VERSION, COMPAT_VERSION}
    {}
