...
public:
    ...
    void print(std::ostream& out) const override { out << "dir_update(" << get_dirfrag() << ")"; }
...
protected:
    ...
    {}
    MDirUpdate(mds_rank_t f, dirfrag_t dirfrag, int dir_rep, const std::set<int32_t>& dir_rep_by, filepath& path,
               bool discover = false)
        : MMDSOp(MSG_MDS_DIRUPDATE, HEAD_VERSION, COMPAT_VERSION)
        , from_mds(f)
        , dirfrag(dirfrag)
