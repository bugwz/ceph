...
public:
    ...
    {}
    MOSDBackoff(spg_t pgid_, epoch_t ep, uint8_t op_, uint64_t id_, hobject_t begin_, hobject_t end_)
        : MOSDFastDispatchOp{CEPH_MSG_OSD_BACKOFF, HEAD_VERSION, COMPAT_VERSION}
        , pgid(pgid_)
        , map_epoch(ep)
        , op(op_)
    ...
    {
        out << "osd_backoff(" << pgid << " " << ceph_osd_backoff_op_name(op) << " id " << id << " [" << begin << ","
            << end << ")"
            << " e" << map_epoch << ")";
    }
