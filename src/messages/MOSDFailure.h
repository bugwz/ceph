...
public:
    ...
    MOSDFailure()
        : PaxosServiceMessage(MSG_OSD_FAILURE, 0, HEAD_VERSION)
    {}
    MOSDFailure(const uuid_d& fs, int osd, const entity_addrvec_t& av, int duration, epoch_t e)
        ...
        , failed_for(duration)
    {}
    MOSDFailure(const uuid_d& fs, int osd, const entity_addrvec_t& av, int duration, epoch_t e, __u8 extra_flags)
        : PaxosServiceMessage(MSG_OSD_FAILURE, e, HEAD_VERSION, COMPAT_VERSION)
        , fsid(fs)
        , target_osd(osd)
        , target_addrs(av)
...
public:
    ...
    bool if_osd_failed() const { return flags & FLAG_FAILED; }
    bool is_immediate() const { return flags & FLAG_IMMEDIATE; }
    ...
    void print(std::ostream& out) const override
    {
        out << "osd_failure(" << (if_osd_failed() ? "failed " : "recovered ")
            << (is_immediate() ? "immediate " : "timeout ") << "osd." << target_osd << " " << target_addrs << " for "
            << failed_for << "sec e" << epoch << " v" << version << ")";
    }
