...
public:
    ...
    epoch_t get_first() const
    {
        ...
        if (i != incremental_maps.end() && (e == 0 || i->first < e)) e = i->first;
        return e;
    }
    epoch_t get_last() const
    {
        ...
        if (i != incremental_maps.rend() && (e == 0 || i->first > e)) e = i->first;
        return e;
    }
...
public:
    ...
    void encode_payload(uint64_t features) override
    {
        ...
        encode(fsid, payload);
        if (OSDMap::get_significant_features(encode_features) != OSDMap::get_significant_features(features)) {
            if ((features & CEPH_FEATURE_PGID64) == 0 || (features & CEPH_FEATURE_PGPOOL3) == 0) {
    ...
    void print(std::ostream& out) const override
    {
        ...
        if (cluster_osdmap_trim_lower_bound || newest_map)
            out << " src has " << cluster_osdmap_trim_lower_bound << ".." << newest_map;
        out << ")";
    }
