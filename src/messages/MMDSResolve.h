...
public:
    ...
    public:
        ...
        peer_inode_cap() {}
        peer_inode_cap(inodeno_t a, std::map<client_t, Capability::Export> b)
            : ino(a)
            , cap_exports(b)
        {}
...
protected:
    MMDSResolve()
        : MMDSOp{MSG_MDS_RESOLVE, HEAD_VERSION, COMPAT_VERSION}
    {}
    ~MMDSResolve() final {}
...
public:
    ...
    {
        out << "mds_resolve(" << subtrees.size() << "+" << ambiguous_imports.size() << " subtrees +"
            << peer_requests.size() << " peer requests)";
    }

    void add_subtree(dirfrag_t im) { subtrees[im].clear(); }
    void add_subtree_bound(dirfrag_t im, dirfrag_t ex) { subtrees[im].push_back(ex); }

    void add_ambiguous_import(dirfrag_t im, const std::vector<dirfrag_t>& m) { ambiguous_imports[im] = m; }

    void add_peer_request(metareqid_t reqid, bool committing) { peer_requests[reqid].committing = committing; }
