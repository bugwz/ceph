...
public:
    ...
    MPoolOpReply(uuid_d& f, ceph_tid_t t, int rc, int e, version_t v, ceph::buffer::list* blp)
        : PaxosServiceMessage{CEPH_MSG_POOLOP_REPLY, v}
        , fsid(f)
        , replyCode(rc)
        , epoch(e)
    {
        set_tid(t);
        if (blp) response_data = std::move(*blp);
    }
    ...
    {
        out << "pool_op_reply(tid " << get_tid() << " " << cpp_strerror(-replyCode) << " v" << version << ")";
    }
