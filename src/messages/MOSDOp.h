...
namespace _mosdop {
template<typename V> class MOSDOp final : public MOSDFastDispatchOp
{
private:
...
public:
    ...
    void set_snapid(const snapid_t& s) { hobj.snap = s; }
    void set_snaps(const std::vector<snapid_t>& i) { snaps = i; }
    void set_snap_seq(const snapid_t& s) { snap_seq = s; }
    void set_reqid(const osd_reqid_t rid) { reqid = rid; }
    void set_spg(spg_t p) { pgid = p; }
    ...
    osd_reqid_t get_reqid() const
    {
        ...
        if (reqid.name != entity_name_t() || reqid.tid != 0) {
            ...
            if (!final_decode_needed) ceph_assert(reqid.inc == (int32_t)client_inc);   // decode() should have done this
            return osd_reqid_t(get_orig_source(), reqid.inc, header.tid);
        }
    }
    ...
    const hobject_t& get_hobj() const { return hobj; }
     ...
     */
    int get_retry_attempt() const { return retry_attempt; }
    ...
    MOSDOp()
        ...
        , bdata_encode(false)
    {}
    MOSDOp(int inc, long tid, const hobject_t& ho, spg_t& _pgid, epoch_t _osdmap_epoch, int _flags, uint64_t feat)
        : MOSDFastDispatchOp(CEPH_MSG_OSD_OP, HEAD_VERSION, COMPAT_VERSION)
        , client_inc(inc)
...
public:
    ...
    void set_mtime(ceph::real_time mt) { mtime = ceph::real_clock::to_timespec(mt); }
    ...
    void zero(uint64_t off, uint64_t len) { add_simple_op(CEPH_OSD_OP_ZERO, off, len); }
    void truncate(uint64_t off) { add_simple_op(CEPH_OSD_OP_TRUNCATE, off, 0); }
    void remove() { add_simple_op(CEPH_OSD_OP_DELETE, 0, 0); }

    void read(uint64_t off, uint64_t len) { add_simple_op(CEPH_OSD_OP_READ, off, len); }
    void stat() { add_simple_op(CEPH_OSD_OP_STAT, 0, 0); }
...
struct ceph_osd_request_head {
	...
	struct ceph_osd_op ops[];  /* followed by ops[], obj, ticket, snaps */
            ...
            for (unsigned i = 0; i < ops.size(); i++) encode(ops[i].op, payload);
    ...
    void decode_payload() override
    {
        ...
        if (header.version == HEAD_VERSION) {
            ...
            decode(num_ops, p);
            ops.resize(num_ops);
            for (unsigned i = 0; i < num_ops; i++) decode(ops[i].op, p);

            ceph::decode_nohead(oid_len, hobj.oid.name, p);
            ceph::decode_nohead(num_snaps, snaps, p);

            // recalculate pgid hash value
            pgid.pgid.set_ps(ceph_str_hash(CEPH_STR_HASH_RJENKINS, hobj.oid.name.c_str(), hobj.oid.name.length()));
            ...
            // put client_inc in reqid.inc for get_reqid()'s benefit
            if (reqid.name == entity_name_t() && reqid.tid == 0) reqid.inc = client_inc;
        }
    ...
    bool finish_decode()
    {
        ...
        if (!final_decode_needed) return false;   // Message is already final decoded
    ...
    void print(std::ostream& out) const override
    {
        ...
        if (!partial_decode_needed) {
            ...
            if (!final_decode_needed) {
                out << ' ';
                out << hobj << " " << ops << " snapc " << get_snap_seq() << "=" << snaps;
                if (is_retry_attempt()) out << " RETRY=" << get_retry_attempt();
            }
            else {
