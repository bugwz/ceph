...
public:
    ...
    void decode_payload() override
    {
        ...
        if (!begin.is_max() && begin.pool == -1) begin.pool = pgid.pool();
        if (!end.is_max() && end.pool == -1) end.pool = pgid.pool();
    ...
    {}
    MOSDPGScan(__u32 o, pg_shard_t from, epoch_t e, epoch_t qe, spg_t p, hobject_t be, hobject_t en)
        : MOSDFastDispatchOp{MSG_OSD_PG_SCAN, HEAD_VERSION, COMPAT_VERSION}
        , op(o)
        , map_epoch(e)
        , query_epoch(qe)
...
public:
    ...
    {
        out << "pg_scan(" << get_op_name(op) << " " << pgid << " " << begin << "-" << end << " e " << map_epoch << "/"
            << query_epoch << ")";
    }
