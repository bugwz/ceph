...
public:
    ...
    void print(std::ostream& out) const override { out << "discover_reply(" << header.tid << " " << base_ino << ")"; }

    // builders
    bool is_empty() const
    {
        return trace.length() == 0 && !flag_error_dn && !flag_error_dir && dir_auth_hint == CDIR_AUTH_UNKNOWN;
    }
    ...
    void set_dir_auth_hint(int a) { dir_auth_hint = a; }
    void set_error_dentry(std::string_view dn) { error_dentry = dn; }
