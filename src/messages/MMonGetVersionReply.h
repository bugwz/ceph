...
public:
    MMonGetVersionReply()
        : Message{CEPH_MSG_MON_GET_VERSION_REPLY, HEAD_VERSION}
    {}

    std::string_view get_type_name() const override { return "mon_get_version_reply"; }
    ...
    void decode_payload() override
    {
        ...
        decode(handle, p);
        decode(version, p);
        if (header.version >= 2) decode(oldest_version, p);
    }
