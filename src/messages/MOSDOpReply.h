...
public:
        ...
        , bdata_encode(false)
    {
        ...
        set_tid(req->get_tid());
        result = r;
        flags = (req->flags & ~(CEPH_OSD_FLAG_ONDISK | CEPH_OSD_FLAG_ONNVRAM | CEPH_OSD_FLAG_ACK)) | acktype;
        osdmap_epoch = e;
        user_version = 0;
...
public:
    void encode_payload(uint64_t features) override
    {
        ...
        if ((features & CEPH_FEATURE_PGID64) == 0) {
            ...
            for (unsigned i = 0; i < num_ops; i++) encode(ops[i].op, payload);

            encode(retry_attempt, payload);

            for (unsigned i = 0; i < num_ops; i++) encode(ops[i].rval, payload);
    ...
    void decode_payload() override
    {
        ...
        if (header.version == HEAD_VERSION) {
            ...
            for (unsigned i = 0; i < num_ops; ++i) decode(ops[i].rval, p);
            ...
            if (do_redirect) decode(redirect, p);
            decode_trace(p);
        }
    ...
    {
        out << "osd_op_reply(" << get_tid() << " " << oid << " " << ops << " v" << get_replay_version() << " uv"
            << get_user_version();
        if (is_ondisk())
            out << " ondisk";
        else if (is_onnvram())
