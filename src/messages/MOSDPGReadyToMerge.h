...
public:
    ...
    {}
    MOSDPGReadyToMerge(pg_t p, eversion_t sv, eversion_t tv, epoch_t les, epoch_t lec, bool r, epoch_t v)
    ...
    {
        out << get_type_name() << "(" << pgid << " sv " << source_version << " tv " << target_version << " les/c "
            << last_epoch_started << "/" << last_epoch_clean << (ready ? " ready" : " NOT READY") << " v" << version
            << ")";
    }
