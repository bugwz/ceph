...
public:
    ...
    {
        out << "client_reconnect(" << caps.size() << " caps " << realms.size() << " realms )";
    }
    ...
    {
        header.version = v;
        if (v <= 3) header.compat_version = 0;
    }
    size_t get_approx_size() { return approx_size; }
    void mark_more() { more = true; }
    bool has_more() const { return more; }

    void add_cap(inodeno_t ino, uint64_t cap_id, inodeno_t pathbase, const std::string& path, int wanted, int issued,
                 inodeno_t sr, snapid_t sf, ceph::buffer::list& lb)
    {
        caps[ino] = cap_reconnect_t(cap_id, pathbase, path, wanted, issued, sr, sf, lb);
        if (!cap_size) calc_item_size();
    ...
    void encode_payload(uint64_t features) override
    {
        ...
        if (header.version >= 4) {
            ...
            if (header.version == 3) {
                ...
                for (auto& r : realms) r.encode_old(data);
            }
        }
    }
    void decode_payload() override
    {
        ...
        if (header.version >= 4) {
            decode(caps, p);
            decode(realms, p);
            if (header.version >= 5) decode(more, p);
        }
        else {
            // compat crap
            if (header.version == 3) {
                ...
                std::map<inodeno_t, old_cap_reconnect_t> ocaps;
                decode(ocaps, p);
                for (auto& q : ocaps) caps[q.first] = q.second;
            }
            while (!p.end()) {
