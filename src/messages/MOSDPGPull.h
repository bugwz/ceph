...
public:
    ...
    std::vector<PullOp> take_pulls() { return std::move(pulls); }
    void set_pulls(std::vector<PullOp>&& pull_ops) { pulls = std::move(pull_ops); }
    ...
    {
        out << "MOSDPGPull(" << pgid << " e" << map_epoch << "/" << min_epoch << " cost " << cost << ")";
    }
