...
{
    ClassHandler::ClassData* cls = ClassHandler::get_instance().register_class(name);
...
{
    if (!(flags & (CLS_METHOD_RD | CLS_METHOD_WR))) return -EINVAL;
...
int cls_cxx_read(cls_method_context_t hctx, int ofs, int len, ceph::buffer::list* outbl)
{
    return cls_cxx_read2(hctx, ofs, len, outbl, 0);
}

int cls_cxx_write(cls_method_context_t hctx, int ofs, int len, ceph::buffer::list* inbl)
{
