...
public:
    explicit CephLevelDBLogger(CephContext* c)
        : cct(c)
    {
        cct->get();
    }
    ~CephLevelDBLogger() override { cct->put(); }
...
{
    if (options.write_buffer_size) ldoptions.write_buffer_size = options.write_buffer_size;
    if (options.max_open_files) ldoptions.max_open_files = options.max_open_files;
    ...
    if (options.block_size) ldoptions.block_size = options.block_size;
    if (options.bloom_size) {
#ifdef HAVE_LEVELDB_FILTER_POLICY
        const leveldb::FilterPolicy* _filterpolicy = leveldb::NewBloomFilterPolicy(options.bloom_size);
    ...
    if (options.block_restart_interval) ldoptions.block_restart_interval = options.block_restart_interval;
...
int LevelDBStore::do_open(ostream& out, bool create_if_missing)
{
    ...
    plb.add_u64_counter(
        l_leveldb_compact_queue_merge, "leveldb_compact_queue_merge", "Mergings of ranges in compaction queue");
...
void LevelDBStore::LevelDBTransactionImpl::set(const string& prefix, const string& k, const bufferlist& to_set_bl)
{
...
void LevelDBStore::LevelDBTransactionImpl::rmkey(const string& prefix, const string& k)
{
    string key = combine_strings(prefix, k);
...
int LevelDBStore::get(const string& prefix, const std::set<string>& keys, std::map<string, bufferlist>* out)
{
    utime_t start = ceph_clock_now();
    for (std::set<string>::const_iterator i = keys.begin(); i != keys.end(); ++i) {
        std::string value;
        std::string bound = combine_strings(prefix, *i);
        auto status = db->Get(leveldb::ReadOptions(), leveldb::Slice(bound), &value);
        if (status.ok()) (*out)[*i].append(value);
    }
    utime_t lat = ceph_clock_now() - start;
...
int LevelDBStore::get(const string& prefix, const string& key, bufferlist* out)
{
