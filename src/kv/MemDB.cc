...
void MemDB::_save()
{
    ...
    int mode = 0644;
    int fd = TEMP_FAILURE_RETRY(::open(_get_data_fn().c_str(), O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, mode));
    if (fd < 0) {
        int err = errno;
        cerr << "write_file(" << _get_data_fn().c_str() << "): failed to open file: " << cpp_strerror(err) << std::endl;
        return;
    }
    bufferlist bl;
...
int MemDB::_load()
{
    ...
    if (fd < 0) {
        int err = errno;
        cerr << "can't open " << _get_data_fn().c_str() << ": " << cpp_strerror(err) << std::endl;
        return -err;
    }
    ...
    if (::fstat(fd, &st) < 0) {
        int err = errno;
        cerr << "can't stat file " << _get_data_fn().c_str() << ": " << cpp_strerror(err) << std::endl;
        VOID_TEMP_FAILURE_RETRY(::close(fd));
        return -err;
    }
...
int MemDB::set_merge_operator(const string& prefix, std::shared_ptr<KeyValueDB::MergeOperator> mop)
{
...
void MemDB::close()
{
     ...
     */
    _save();
    if (logger) m_cct->get_perfcounters_collection()->remove(logger);
}
...
void MemDB::MDBTransactionImpl::set(const string& prefix, const string& k, const bufferlist& to_set_bl)
{
    dtrace << __func__ << " " << prefix << " " << k << dendl;
    ops.push_back(make_pair(WRITE, std::make_pair(std::make_pair(prefix, k), to_set_bl)));
}

void MemDB::MDBTransactionImpl::rmkey(const string& prefix, const string& k)
{
    dtrace << __func__ << " " << prefix << " " << k << dendl;
...
void MemDB::MDBTransactionImpl::merge(const std::string& prefix, const std::string& key, const bufferlist& value)
{
...
int MemDB::get(const string& prefix, const std::string& key, bufferlist* out)
{
    utime_t start = ceph_clock_now();
    int ret;
...
int MemDB::get(const string& prefix, const std::set<string>& keys, std::map<string, bufferlist>* out)
{
    utime_t start = ceph_clock_now();

    for (const auto& i : keys) {
        bufferlist bl;
        if (_get_locked(prefix, i, &bl)) out->insert(make_pair(i, bl));
    }
...
void MemDB::MDBWholeSpaceIteratorImpl::free_last()
{
...
bool MemDB::MDBWholeSpaceIteratorImpl::raw_key_is_prefixed(const string& prefix)
{
    string p, k;
...
int MemDB::MDBWholeSpaceIteratorImpl::upper_bound(const std::string& prefix, const std::string& after)
{
...
int MemDB::MDBWholeSpaceIteratorImpl::lower_bound(const std::string& prefix, const std::string& to)
{
    std::lock_guard<std::mutex> l(*m_map_lock_p);
