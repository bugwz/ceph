...
rocksdb::Status ShardedCache::Insert(const rocksdb::Slice& key, void* value, size_t charge, DeleterFn deleter,
...
void ShardedCache::ApplyToAllCacheEntries(void (*callback)(void*, size_t), bool thread_safe)
{
    ...
    for (int s = 0; s < num_shards; s++) {
        GetShard(s)->ApplyToAllCacheEntries(
            [callback](const rocksdb::Slice&, void* value, size_t charge, DeleterFn) { callback(value, charge); },
            thread_safe);
    }
}
#endif
