...
std::shared_ptr<rocksdb::Cache> NewBinnedLRUCache(CephContext* c, size_t capacity, int num_shard_bits = -1,
...
public:
    ...
    template<typename T> void ApplyToAllCacheEntries(T func)
    {
...
{
public:
    BinnedLRUCacheShard(CephContext* c, size_t capacity, bool strict_capacity_limit, double high_pri_pool_ratio);
...
public:
    ...
    virtual int64_t get_committed_size() const { return GetCapacity(); }
    virtual void shift_bins();
    uint64_t sum_bins(uint32_t start, uint32_t end) const;
    uint32_t get_bin_count() const;
    void set_bin_count(uint32_t count);

    virtual std::string get_cache_name() const { return "RocksDB Binned LRU Cache"; }
