...
void BinnedLRUCacheShard::ApplyToAllCacheEntries(
    const std::function<void(const rocksdb::Slice& key, void* value, size_t charge, DeleterFn)>& callback,
    bool thread_safe)
{
    if (thread_safe) {
        mutex_.lock();
    ...
    table_.ApplyToAllCacheEntries(
        [callback](BinnedLRUHandle* h) { callback(h->key(), h->value, h->charge, h->deleter); });
    if (thread_safe) {
        mutex_.unlock();
    }
}
...
void BinnedLRUCacheShard::EvictFromLRU(size_t charge, ceph::autovector<BinnedLRUHandle*>* deleted)
{
...
rocksdb::Status BinnedLRUCacheShard::Insert(const rocksdb::Slice& key, uint32_t hash, void* value, size_t charge,
                                            DeleterFn deleter, rocksdb::Cache::Handle** handle,
                                            rocksdb::Cache::Priority priority)
{
    ...
    e->flags = 0;
    e->hash = hash;
    e->refs = (handle == nullptr ? 1 : 2);   // One from BinnedLRUCache, one for the returned handle
...

    {
        ...
        if (usage_ - lru_usage_ + charge > capacity_ && (strict_capacity_limit_ || handle == nullptr)) {
            if (handle == nullptr) {
...
BinnedLRUCache::BinnedLRUCache(CephContext* c, size_t capacity, int num_shard_bits, bool strict_capacity_limit,
                               double high_pri_pool_ratio)
    ...
    , cct(c)
{
    ...
    int rc = posix_memalign((void**)&shards_, CACHE_LINE_SIZE, sizeof(BinnedLRUCacheShard) * num_shards_);
    if (rc != 0) {
        throw std::bad_alloc();
    }
    size_t per_shard = (capacity + (num_shards_ - 1)) / num_shards_;
    for (int i = 0; i < num_shards_; i++) {
        new (&shards_[i]) BinnedLRUCacheShard(c, per_shard, strict_capacity_limit, high_pri_pool_ratio);
    }
}
...
int64_t BinnedLRUCache::commit_cache_size(uint64_t total_bytes)
{
    ...
    int64_t new_bytes = PriorityCache::get_chunk(get_cache_bytes(), total_bytes);
...
std::shared_ptr<rocksdb::Cache> NewBinnedLRUCache(CephContext* c, size_t capacity, int num_shard_bits,
                                                  bool strict_capacity_limit, double high_pri_pool_ratio)
{
    ...
    return std::make_shared<BinnedLRUCache>(c, capacity, num_shard_bits, strict_capacity_limit, high_pri_pool_ratio);
}
