...
void BinnedLRUCacheShard::ApplyToAllCacheEntries(void (*callback)(void*, size_t), bool thread_safe)
{
    if (thread_safe) {
        mutex_.lock();
    }
    table_.ApplyToAllCacheEntries([callback](BinnedLRUHandle* h) { callback(h->value, h->charge); });
    if (thread_safe) {
        mutex_.unlock();
    }
}
