...
static rocksdb::SliceParts prepare_sliceparts(const bufferlist& bl, vector<rocksdb::Slice>* slices)
{
    unsigned n = 0;
...
public:
    ...
    explicit MergeOperatorRouter(RocksDBStore& _store)
        : store(_store)
    {}
               ...
               std::string* new_value, rocksdb::Logger* logger) const override
    {
        ...
        for (auto& p : store.merge_ops) {
            if (p.first.compare(0, p.first.length(), key.data(), p.first.length()) == 0 &&
                ...
                if (existing_value) {
                    p.second->merge(
                        existing_value->data(), existing_value->size(), value.data(), value.size(), new_value);
                }
                else {
...
//
class RocksDBStore::MergeOperatorLinker : public rocksdb::AssociativeMergeOperator
{
private:
    std::shared_ptr<KeyValueDB::MergeOperator> mop;

public:
    explicit MergeOperatorLinker(const std::shared_ptr<KeyValueDB::MergeOperator>& o)
        : mop(o)
    {}

    const char* Name() const override { return mop->name(); }

    bool Merge(const rocksdb::Slice& key, const rocksdb::Slice* existing_value, const rocksdb::Slice& value,
               std::string* new_value, rocksdb::Logger* logger) const override
    {
        if (existing_value) {
            mop->merge(existing_value->data(), existing_value->size(), value.data(), value.size(), new_value);
        }
        else {
...
public:
    explicit CephRocksdbLogger(CephContext* c)
        : cct(c)
    {
        cct->get();
    }
    ~CephRocksdbLogger() override { cct->put(); }
...
rocksdb::Status StringToMap(const std::string& opts_str, std::unordered_map<std::string, std::string>* opts_map)
{
    ...
    while (pos < opts.size()) {
        ...
        if (opts[pos] == '{') {
            ...
            if (count == 0) {
                ...
                return Status::InvalidArgument("Mismatched curly braces for nested options");
            }
        }
        else {
...
{
    return ParseOptionsFromStringStatic(cct, opt_str, opt, [&](const string& k, const string& v, rocksdb::Options& o) {
        return tryInterpret(k, v, o);
    });
}

int RocksDBStore::ParseOptionsFromStringStatic(CephContext* cct, const string& opt_str, rocksdb::Options& opt,
                                               function<int(const string&, const string&, rocksdb::Options&)> interp)
{
    ...
    if (!status.ok()) {
        dout(5) << __func__ << " error '" << status.getState() << "' while parsing options '" << opt_str << "'"
                << dendl;
        return -EINVAL;
    }
...
int RocksDBStore::install_cf_mergeop(const string& key_prefix, rocksdb::ColumnFamilyOptions* cf_opt)
{
...
int RocksDBStore::create_and_open(ostream& out, const std::string& cfs)
{
    int r = create_db_dir();
    if (r < 0) return r;
...
std::shared_ptr<rocksdb::Cache> RocksDBStore::create_block_cache(const std::string& cache_type, size_t cache_size,
...
int RocksDBStore::load_rocksdb_options(bool create_if_missing, rocksdb::Options& opt)
{
    ...
    dout(10) << __func__ << " block size " << cct->_conf->rocksdb_block_size << ", block_cache size "
             << byte_u_t(block_cache_size) << ", row_cache size " << byte_u_t(row_cache_size) << "; shards "
             << (1 << cct->_conf->rocksdb_cache_shard_bits) << ", type " << cct->_conf->rocksdb_cache_type << dendl;
...
void RocksDBStore::add_column_family(const std::string& cf_name, uint32_t hash_l, uint32_t hash_h, size_t shard_idx,
                                     rocksdb::ColumnFamilyHandle* handle)
{
    dout(10) << __func__ << " column_name=" << cf_name << " shard_idx=" << shard_idx << " hash_l=" << hash_l
             << " hash_h=" << hash_h << " handle=" << (void*)handle << dendl;
    ...
    if (column.handles.size() <= shard_idx) column.handles.resize(shard_idx + 1);
 ...
 * to a single CF.
 */
rocksdb::ColumnFamilyHandle* RocksDBStore::check_cf_handle_bounds(const cf_handles_iterator& iter,
                                                                  const IteratorBounds& bounds)
{
 ...
 * I=write_buffer_size=1048576 O(6) m(7,10-)
 * prefix(4,0-10)=disable_auto_compactions=true,max_bytes_for_level_base=1048576
 */
bool RocksDBStore::parse_sharding_def(const std::string_view text_def_in, std::vector<ColumnFamily>& sharding_def,
                                      char const** error_position, std::string* error_msg)
{
...
int RocksDBStore::create_shards(const rocksdb::Options& opt, const std::vector<ColumnFamily>& sharding_def)
{
...
int RocksDBStore::apply_sharding(const rocksdb::Options& opt, const std::string& sharding_text)
{
    ...
    if (!sharding_text.empty()) {
        ...
        status = rocksdb::WriteStringToFile(opt.env, sharding_text, sharding_def_file, true);
        if (!status.ok()) {
                                              ...
                                              std::string* block_cache_opt)
{
    ...
    if (!status.ok()) {
        dout(5) << __func__ << " error '" << status.getState() << "' while parsing options '" << options << "'"
                << dendl;
        return -EINVAL;
    }
...
int RocksDBStore::update_column_family_options(const std::string& base_name, const std::string& more_options,
                                               rocksdb::ColumnFamilyOptions* cf_opt)
{
    ...
    if (r != 0) {
        dout(5) << __func__ << " failed to parse options; column family=" << base_name << " options=" << more_options
                << dendl;
        return r;
    }
    status = rocksdb::GetColumnFamilyOptionsFromMap(*cf_opt, options_map, cf_opt);
    if (!status.ok()) {
        dout(5) << __func__ << " invalid column family optionsp; column family=" << base_name
...
int RocksDBStore::apply_block_cache_options(const std::string& column_name, const std::string& block_cache_opt,
                                            rocksdb::ColumnFamilyOptions* cf_opt)
{
    rocksdb::Status status;
    ...
    if (!status.ok()) {
        dout(5) << __func__ << " invalid block cache options; column=" << column_name << " options=" << block_cache_opt
                << dendl;
        dout(5) << __func__ << " RocksDB error='" << status.getState() << "'" << dendl;
        return -EINVAL;
    }
                                  ...
                                  std::vector<std::pair<size_t, RocksDBStore::ColumnFamily> >& missing_cfs_shard)
{
    ...
    if (status.ok()) {
        status = rocksdb::ReadFileToString(opt.env, sharding_def_file, &stored_sharding_text);
        if (!status.ok()) {
    ...
    std::sort(stored_sharding_def.begin(), stored_sharding_def.end(), [](ColumnFamily& a, ColumnFamily& b) {
        return a.name < b.name;
    });

    std::vector<string> rocksdb_cfs;
    status = rocksdb::DB::ListColumnFamilies(rocksdb::DBOptions(opt), path, &rocksdb_cfs);
    if (!status.ok()) {
...
int RocksDBStore::do_open(ostream& out, bool create_if_missing, bool open_readonly, const std::string& sharding_text)
{
    ...
    if (create_if_missing) {
        ...
        r = verify_sharding(opt, existing_cfs, existing_cfs_shard, missing_cfs, missing_cfs_shard);
        if (r < 0) {
            return r;
        }
        std::string sharding_recreate_text;
        status = rocksdb::ReadFileToString(opt.env, sharding_recreate, &sharding_recreate_text);
        ...
        if (recreate_mode == false && missing_cfs.size() != 0) {
            ...
            if (open_readonly == false &&
                std::find_if(missing_cfs.begin(), missing_cfs.end(), [](const rocksdb::ColumnFamilyDescriptor& c) {
        ...
        if (existing_cfs.empty()) {
            ...
            if (open_readonly) {
                status = rocksdb::DB::OpenForReadOnly(rocksdb::DBOptions(opt), path, existing_cfs, &handles, &db);
            }
            else {
                status = rocksdb::DB::Open(rocksdb::DBOptions(opt), path, existing_cfs, &handles, &db);
            }
            if (!status.ok()) {
                derr << status.ToString() << dendl;
                return -EINVAL;
    ...
    plb.add_time_avg(l_rocksdb_write_pre_and_post_process_time,
                     ...
                     "total time spent on writing a record, excluding write process");
...
int RocksDBStore::repair(std::ostream& out)
{
    ...
    if (!stored_sharding_text.empty()) {
        ...
        opt.env->CreateDir(sharding_def_dir);
        status = rocksdb::WriteStringToFile(opt.env, stored_sharding_text, sharding_def_file, true);
        if (!status.ok()) {
            derr << __func__ << " cannot write to " << sharding_def_file << dendl;
            return -1;
        }
        status = rocksdb::WriteStringToFile(opt.env, "1", sharding_recreate, true);
        if (!status.ok()) {
...
bool RocksDBStore::get_property(const std::string& property, uint64_t* out)
{
    return db->GetIntProperty(property, out);
}

int64_t RocksDBStore::estimate_prefix_size(const string& prefix, const string& key_prefix)
{
    uint64_t size = 0;
    uint8_t flags =
...
{
    RocksWBHandler(const RocksDBStore& db)
        : db(db)
    {}
    ...
    void dump(const char* op_name, uint32_t column_family_id, const rocksdb::Slice& key_in,
              const rocksdb::Slice* value = nullptr)
    {
        ...
        if (size != -1) seen << " value size = " << std::to_string(size);
        seen << ")";
        num_seen++;
    }
    void Put(const rocksdb::Slice& key, const rocksdb::Slice& value) override { dump("Put", 0, key, &value); }
    rocksdb::Status PutCF(uint32_t column_family_id, const rocksdb::Slice& key, const rocksdb::Slice& value) override
    {
        dump("PutCF", column_family_id, key, &value);
        return rocksdb::Status::OK();
    }
    void SingleDelete(const rocksdb::Slice& key) override { dump("SingleDelete", 0, key); }
    rocksdb::Status SingleDeleteCF(uint32_t column_family_id, const rocksdb::Slice& key) override
    {
        dump("SingleDeleteCF", column_family_id, key);
        return rocksdb::Status::OK();
    }
    void Delete(const rocksdb::Slice& key) override { dump("Delete", 0, key); }
    rocksdb::Status DeleteCF(uint32_t column_family_id, const rocksdb::Slice& key) override
    {
        dump("DeleteCF", column_family_id, key);
        return rocksdb::Status::OK();
    }
    void Merge(const rocksdb::Slice& key, const rocksdb::Slice& value) override { dump("Merge", 0, key, &value); }
...
int RocksDBStore::submit_common(rocksdb::WriteOptions& woptions, KeyValueDB::Transaction t)
{
    ...
    RocksDBTransactionImpl* _t = static_cast<RocksDBTransactionImpl*>(t.get());
    ...
    *_dout << " Rocksdb transaction: " << bat_txc.seen.str() << dendl;
    ...
    if (cct->_conf->rocksdb_perf) {
        ...
        write_wal_time.set_from_double(static_cast<double>(rocksdb::get_perf_context()->write_wal_time) / 1000000000);
        write_memtable_time.set_from_double(static_cast<double>(rocksdb::get_perf_context()->write_memtable_time) /
                                            1000000000);
        write_delay_time.set_from_double(static_cast<double>(rocksdb::get_perf_context()->write_delay_time) /
                                         1000000000);
...
void RocksDBStore::RocksDBTransactionImpl::put_bat(rocksdb::WriteBatch& bat, rocksdb::ColumnFamilyHandle* cf,
                                                   const string& key, const bufferlist& to_set_bl)
{
    // bufferlist::c_str() is non-constant, so we can't call c_str()
    if (to_set_bl.is_contiguous() && to_set_bl.length() > 0) {
        bat.Put(cf, rocksdb::Slice(key), rocksdb::Slice(to_set_bl.buffers().front().c_str(), to_set_bl.length()));
    }
    else {
        rocksdb::Slice key_slice(key);
        vector<rocksdb::Slice> value_slices(to_set_bl.get_num_buffers());
        bat.Put(cf, rocksdb::SliceParts(&key_slice, 1), prepare_sliceparts(to_set_bl, &value_slices));
    }
}

void RocksDBStore::RocksDBTransactionImpl::set(const string& prefix, const string& k, const bufferlist& to_set_bl)
{
...
void RocksDBStore::RocksDBTransactionImpl::set(const string& prefix, const char* k, size_t keylen,
...
void RocksDBStore::RocksDBTransactionImpl::rmkey(const string& prefix, const char* k, size_t keylen)
{
    auto cf = db->get_cf_handle(prefix, k, keylen);
    if (cf) {
...
void RocksDBStore::RocksDBTransactionImpl::rm_single_key(const string& prefix, const string& k)
{
    auto cf = db->get_cf_handle(prefix, k);
    if (cf) {
...
void RocksDBStore::RocksDBTransactionImpl::rmkeys_by_prefix(const string& prefix)
{
    ...
    if (p_iter == db->cf_handles.end()) {
        ...
        if (cnt == 0) {
            ...
            bat.DeleteRange(db->default_cf, combine_strings(prefix, string()), combine_strings(endprefix, string()));
        }
        else {
            bat.PopSavePoint();
        }
...
void RocksDBStore::RocksDBTransactionImpl::rm_range_keys(const string& prefix, const string& start, const string& end)
{
    ldout(db->cct, 10) << __func__ << " enter prefix=" << prefix << " start=" << pretty_binary_string(start)
    ...
    if (p_iter == db->cf_handles.end()) {
        ...
        bat.SetSavePoint();
        auto it = db->get_iterator(prefix);
        for (it->lower_bound(start); it->valid() && db->comparator->Compare(it->key(), end) < 0 && (--cnt) != 0;
             it->next()) {
            bat.Delete(db->default_cf, combine_strings(prefix, it->key()));
        }
        ldout(db->cct, 15) << __func__ << " count = " << cnt0 - cnt << dendl;
        if (cnt == 0) {
            ldout(db->cct, 10) << __func__ << " p_iter == end(), resorting to DeleteRange" << dendl;
            bat.RollbackToSavePoint();
        ...
        for (auto cf : p_iter->second.handles) {
            ...
            for (it->lower_bound(start); it->valid() && (--cnt) != 0; it->next()) {
                bat.Delete(cf, it->key());
            }
            ldout(db->cct, 10) << __func__ << " count = " << cnt0 - cnt << dendl;
            if (cnt == 0) {
                ldout(db->cct, 10) << __func__ << " p_iter != end(), resorting to DeleteRange" << dendl;
                bat.RollbackToSavePoint();
                bat.DeleteRange(cf, rocksdb::Slice(start), rocksdb::Slice(end));
...
void RocksDBStore::RocksDBTransactionImpl::merge(const string& prefix, const string& k, const bufferlist& to_set_bl)
{
    auto cf = db->get_cf_handle(prefix, k);
    if (cf) {
        // bufferlist::c_str() is non-constant, so we can't call c_str()
        if (to_set_bl.is_contiguous() && to_set_bl.length() > 0) {
            bat.Merge(cf, rocksdb::Slice(k), rocksdb::Slice(to_set_bl.buffers().front().c_str(), to_set_bl.length()));
        }
        else {
            // make a copy
            rocksdb::Slice key_slice(k);
            vector<rocksdb::Slice> value_slices(to_set_bl.get_num_buffers());
            bat.Merge(cf, rocksdb::SliceParts(&key_slice, 1), prepare_sliceparts(to_set_bl, &value_slices));
        }
    }
    else {
        string key = combine_strings(prefix, k);
        ...
        if (to_set_bl.is_contiguous() && to_set_bl.length() > 0) {
            ...
            rocksdb::Slice key_slice(key);
            vector<rocksdb::Slice> value_slices(to_set_bl.get_num_buffers());
            bat.Merge(db->default_cf, rocksdb::SliceParts(&key_slice, 1), prepare_sliceparts(to_set_bl, &value_slices));
        }
    }
}

int RocksDBStore::get(const string& prefix, const std::set<string>& keys, std::map<string, bufferlist>* out)
{
    ...
    if (cf_handles.count(prefix) > 0) {
        for (auto& key : keys) {
            ...
            auto status = db->Get(rocksdb::ReadOptions(), cf_handle, rocksdb::Slice(key), &value);
            if (status.ok()) {
        ...
        for (auto& key : keys) {
            ...
            auto status = db->Get(rocksdb::ReadOptions(), default_cf, rocksdb::Slice(k), &value);
            if (status.ok()) {
                (*out)[key].append(value.data(), value.size());
            }
            else if (status.IsIOError()) {
...
int RocksDBStore::get(const string& prefix, const char* key, size_t keylen, bufferlist* out)
{
    ceph_assert(out && (out->length() == 0));
    utime_t start = ceph_clock_now();
    int r = 0;
    ...
    if (cf) {
        s = db->Get(rocksdb::ReadOptions(), cf, rocksdb::Slice(key, keylen), &value);
    }
    else {
        string k;
...
int RocksDBStore::split_key(rocksdb::Slice in, string* prefix, string* key)
{
    ...
    if (separator == NULL) return -EINVAL;
    prefix_len = size_t(separator - in.data());
    if (prefix_len >= in.size()) return -EINVAL;

    // Fetch prefix and/or key directly from Slice
    if (prefix) *prefix = string(in.data(), prefix_len);
    if (key) *key = string(separator + 1, in.size() - prefix_len - 1);
    return 0;
}
...
void RocksDBStore::compact()
{
    ...
    for (auto cf : cf_handles) {
        for (auto shard_cf : cf.second.handles) {
            db->CompactRange(options, shard_cf, nullptr, nullptr);
        }
    }
}
...
void RocksDBStore::compact_range(const string& start, const string& end)
{
    ...
    auto compact_range =
        [&](const decltype(cf_handles)::iterator column_it, const std::string& start, const std::string& end) {
            rocksdb::Slice cstart(start);
            rocksdb::Slice cend(end);
...
int RocksDBStore::RocksDBWholeSpaceIteratorImpl::upper_bound(const string& prefix, const string& after)
{
    ...
    if (valid()) {
        ...
        if (key.first == prefix && key.second == after) next();
    }
    return dbiter->status().ok() ? 0 : -1;
}
...
public:
    explicit CFIteratorImpl(const RocksDBStore* db, const std::string& p, rocksdb::ColumnFamilyHandle* cf,
    ...
    bool valid() override { return dbiter->Valid(); }
    string key() override { return dbiter->key().ToString(); }
    std::pair<std::string, std::string> raw_key() override { return make_pair(prefix, key()); }
    bufferlist value() override { return to_bufferlist(dbiter->value()); }
    bufferptr value_as_ptr() override
    {
        rocksdb::Slice val = dbiter->value();
        return bufferptr(val.data(), val.size());
    }
    int status() override { return dbiter->status().ok() ? 0 : -1; }
};
...
public:
    ...
    bool is_main_smaller()
    {
        if (main->valid()) {
            if (current_shard != shards.end()) {
                ...
                if (main_rk.first < shards_rk.first) return true;
                if (main_rk.first > shards_rk.first) return false;
        ...
        if (current_shard != shards.end()) {
            ...
            if (!located) {
                while (current_shard != shards.end()) {
                    r1 = current_shard->second->seek_to_first();
                    if (r1 != 0) return r1;
                    if (current_shard->second->valid()) break;
                    ++current_shard;
                }
            }
        }
    ...
    bool valid() override
    {
        if (smaller == on_main) {
            ...
            if (current_shard == shards.end()) return false;
    ...
    int shards_next()
    {
        ...
        if (r != 0) return r;
        if (current_shard->second->valid()) return 0;
...
private:
    ...
    public:
        KeyLess(const rocksdb::Comparator* comparator)
...
public:
    explicit ShardMergeIteratorImpl(const RocksDBStore* db, const std::string& prefix,
                                    const std::vector<rocksdb::ColumnFamilyHandle*>& shards,
                                    KeyValueDB::IteratorBounds bounds_)
        : db(db)
        , keyless(db->comparator)
        , prefix(prefix)
        , bounds(std::move(bounds_))
        , iterate_lower_bound(make_slice(bounds.lower_bound))
        , iterate_upper_bound(make_slice(bounds.upper_bound))
    ...
    bool valid() override { return iters[0]->Valid(); }
    string key() override { return iters[0]->key().ToString(); }
    std::pair<std::string, std::string> raw_key() override { return make_pair(prefix, key()); }
    bufferlist value() override { return to_bufferlist(iters[0]->value()); }
    bufferptr value_as_ptr() override
    {
        rocksdb::Slice val = iters[0]->value();
        return bufferptr(val.data(), val.size());
    }
    int status() override { return iters[0]->status().ok() ? 0 : -1; }
};
...
KeyValueDB::Iterator RocksDBStore::get_iterator(const std::string& prefix, IteratorOpts opts, IteratorBounds bounds)
{
    ...
    if (cf_it != cf_handles.end()) {
        ...
        if (cf) {
            return std::make_shared<CFIteratorImpl>(this, prefix, cf, std::move(bounds));
        }
        else {
            return std::make_shared<ShardMergeIteratorImpl>(this, prefix, cf_it->second.handles, std::move(bounds));
        }
    }
    else {
        ...
        auto w_it =
            cf_handles.size() == 0 || prefix.empty() ? get_wholespace_iterator(opts) : get_default_cf_iterator();
...
{
    return std::make_shared<RocksDBWholeSpaceIteratorImpl>(this, cf, 0);
}

KeyValueDB::Iterator RocksDBStore::new_shard_iterator(rocksdb::ColumnFamilyHandle* cf, const std::string& prefix,
...
int RocksDBStore::prepare_for_reshard(const std::string& new_sharding, RocksDBStore::columns_t& to_process_columns)
{
    ...
    // 0. lock db from opening
    std::string stored_sharding_text;
    rocksdb::ReadFileToString(env, sharding_def_file, &stored_sharding_text);
    if (stored_sharding_text.find(resharding_column_lock) == string::npos) {
        rocksdb::Status status;
        if (stored_sharding_text.size() != 0) stored_sharding_text += " ";
        stored_sharding_text += resharding_column_lock;
        env->CreateDir(sharding_def_dir);
        status = rocksdb::WriteStringToFile(env, stored_sharding_text, sharding_def_file, true);
        if (!status.ok()) {
            derr << __func__ << " cannot write to " << sharding_def_file << dendl;
            return -EIO;
        }
    ...
    // 4. open db, acquire existing column handles
    std::vector<rocksdb::ColumnFamilyHandle*> handles;
    status = rocksdb::DB::Open(rocksdb::DBOptions(opt), path, cfs_to_open, &handles, &db);
    if (!status.ok()) {
        derr << status.ToString() << dendl;
        return -EINVAL;
    }
    ...
    sharding_def_to_columns(new_sharding_def, new_sharding_columns);
    ...
    for (size_t i = 0; i < existing_columns.size(); i++) {
        ...
        if (rocksdb::kDefaultColumnFamilyName == base_name) {
            ...
            must_close_default_cf = true;
            std::unique_ptr<rocksdb::ColumnFamilyHandle, cf_deleter_t> ptr{cf, [](rocksdb::ColumnFamilyHandle*) {}};
            ...
            std::unique_ptr<rocksdb::ColumnFamilyHandle, cf_deleter_t> ptr{
                cf, [this](rocksdb::ColumnFamilyHandle* handle) { db->DestroyColumnFamilyHandle(handle); }};
            to_process_columns.emplace(full_name, std::move(ptr));
        }
    }
...
int RocksDBStore::reshard_cleanup(const RocksDBStore::columns_t& current_columns)
{
    ...
    for (auto& [name, handle] : current_columns) {
        auto found =
            std::find(new_sharding_columns.begin(), new_sharding_columns.end(), name) != new_sharding_columns.end();
        ...
        std::unique_ptr<rocksdb::Iterator> it{db->NewIterator(rocksdb::ReadOptions(), handle.get())};
        ceph_assert(it);
        it->SeekToFirst();
...
int RocksDBStore::reshard(const std::string& new_sharding, const RocksDBStore::resharding_ctrl* ctrl_in)
{
    ...
    auto process_column = [&](rocksdb::ColumnFamilyHandle* handle, const std::string& fixed_prefix) {
        ...
        for (it->SeekToFirst(); it->Valid(); it->Next()) {
            ...
            if (bytes_per_iterator >= ctrl.bytes_per_iterator || keys_per_iterator >= ctrl.keys_per_iterator) {
            ...
            dout(25) << "moving " << (void*)handle << "/" << pretty_binary_string(raw_key.ToString()) << " to "
                     << (void*)new_handle << "/" << pretty_binary_string(new_raw_key) << " size " << value.size()
                     << dendl;
            keys_moved++;
            ...
            if (bytes_in_batch >= ctrl.bytes_per_batch || keys_in_batch >= ctrl.keys_per_batch) {
                flush_batch(&bat);
    ...
    env->CreateDir(sharding_def_dir);
    if (auto status = rocksdb::WriteStringToFile(env, new_sharding, sharding_def_file, true); !status.ok()) {
        derr << __func__ << " cannot write to " << sharding_def_file << dendl;
        return -EIO;
    }
...
bool RocksDBStore::get_sharding(std::string& sharding)
{
    ...
    if (status.ok()) {
        status = rocksdb::ReadFileToString(env, sharding_def_file, &stored_sharding_text);
        if (status.ok()) {
            result = true;
