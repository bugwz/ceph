...
KeyValueDB* KeyValueDB::create(CephContext* cct, const string& type, const string& dir, map<string, string> options,
                               void* p)
{
#ifdef WITH_LEVELDB
    if (type == "leveldb") {
    ...
    if ((type == "memdb") && cct->check_experimental_feature_enabled("memdb")) {
