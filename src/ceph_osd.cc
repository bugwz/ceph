...
int main(int argc, const char** argv)
{
    ...
    ObjectStore* store = ObjectStore::create(g_ceph_context, store_type, data_path, journal_path, flags);
    if (!store) {
    ...
    if (mkfs) {
        ...
        int err = OSD::mkfs(g_ceph_context, store, g_conf().get_val<uuid_d>("fsid"), whoami, osdspec_affinity);
        if (err < 0) {
    ...
    int w;
    int r = OSD::peek_meta(store, &magic, &cluster_fsid, &osd_fsid, &w, &require_osd_release);
    if (r < 0) {
