...
TracepointProvider::Traits osd_tracepoint_traits("libosd_tp.so", "osd_tracing");
TracepointProvider::Traits os_tracepoint_traits("libos_tp.so", "osd_objectstore_tracing");
...
TracepointProvider::Traits cyg_profile_traits("libcyg_profile_tp.so", "osd_function_tracing");
#endif
...
{
    if (osdptr) osdptr->handle_signal(signum);
}
...
int main(int argc, const char** argv)
{
    ...
    map<string, string> defaults = {// We want to enable leveldb's log, while allowing users to override this
                                    // option, therefore we will pass it as a default argument to global_init().
                                    {"leveldb_log", ""}};
    auto cct = global_init(&defaults, args, CEPH_ENTITY_TYPE_OSD, CODE_ENVIRONMENT_DAEMON, 0);
    ...
    for (std::vector<const char*>::iterator i = args.begin(); i != args.end();) {
        ...
        else if (ceph_argparse_witharg(args, i, &device_path, "--get-device-fsid", (char*)NULL)) {
    ...
    if (get_device_fsid) {
        uuid_d uuid;
        int r = ObjectStore::probe_block_device_fsid(g_ceph_context, device_path, &uuid);
        if (r < 0) {
            cerr << "failed to get device fsid for " << device_path << ": " << cpp_strerror(r) << std::endl;
            forker.exit(1);
        }
        cout << uuid << std::endl;
        forker.exit(0);
    ...
    std::unique_ptr<ObjectStore> store =
        ObjectStore::create(g_ceph_context, store_type, data_path, journal_path, flags);
    if (!store) {
    ...
    if (mkkey) {
        ...
        if (ret == 0 && keyring.get_auth(ename, eauth)) {
            ...
            if (r)
                derr << TEXT_RED << " ** ERROR: writing new keyring to " << keyring_path << ": " << cpp_strerror(r)
                     << TEXT_NORMAL << dendl;
            else
    ...
    if (mkfs) {
        ...
        int err =
            OSD::mkfs(g_ceph_context, std::move(store), g_conf().get_val<uuid_d>("fsid"), whoami, osdspec_affinity);
        if (err < 0) {
            derr << TEXT_RED << " ** ERROR: error creating empty object store in " << data_path << ": "
                 << cpp_strerror(-err) << TEXT_NORMAL << dendl;
            forker.exit(1);
        }
        dout(0) << "created object store " << data_path << " for osd." << whoami << " fsid "
                << g_conf().get_val<uuid_d>("fsid") << dendl;
    }
    if (mkfs || mkkey) {
        forker.exit(0);
    }
    if (mkjournal) {
        ...
        if (err < 0) {
            derr << TEXT_RED << " ** ERROR: error creating fresh journal " << journal_path << " for object store "
    ...
    if (flushjournal) {
        ...
        int err = store->mount();
        if (err < 0) {
            derr << TEXT_RED << " ** ERROR: error flushing journal " << journal_path << " for object store "
                 << data_path << ": " << cpp_strerror(-err) << TEXT_NORMAL << dendl;
    ...
    if (dump_journal) {
        ...
        if (err < 0) {
            derr << TEXT_RED << " ** ERROR: error dumping journal " << journal_path << " for object store " << data_path
                 << ": " << cpp_strerror(-err) << TEXT_NORMAL << dendl;
            forker.exit(1);
        }
    ...
    if (convertfilestore) {
        int err = store->mount();
        if (err < 0) {
            derr << TEXT_RED << " ** ERROR: error mounting store " << data_path << ": " << cpp_strerror(-err)
                 << TEXT_NORMAL << dendl;
            forker.exit(1);
        }
        err = store->upgrade();
        store->umount();
        if (err < 0) {
            derr << TEXT_RED << " ** ERROR: error converting store " << data_path << ": " << cpp_strerror(-err)
                 << TEXT_NORMAL << dendl;
            forker.exit(1);
        }
        forker.exit(0);
    ...
    int w;
    int r = OSD::peek_meta(store.get(), &magic, &cluster_fsid, &osd_fsid, &w, &require_osd_release);
    if (r < 0) {
        derr << TEXT_RED << " ** ERROR: unable to open OSD superblock on " << data_path << ": " << cpp_strerror(-r)
             << TEXT_NORMAL << dendl;
        if (r == -ENOTSUP) {
    ...
    std::string public_msg_type = g_conf().get_val<std::string>("ms_public_type");
    std::string cluster_msg_type = g_conf().get_val<std::string>("ms_cluster_type");
    ...
    dout(0) << "starting osd." << whoami << " osd_data " << data_path << " "
    ...
    ms_public->set_policy_throttlers(
        entity_name_t::TYPE_CLIENT, client_byte_throttler.get(), client_msg_throttler.get());
    ms_public->set_policy(entity_name_t::TYPE_MON, Messenger::Policy::lossy_client(osd_required));
    ms_public->set_policy(entity_name_t::TYPE_MGR, Messenger::Policy::lossy_client(osd_required));
    ...
    r = pick_addresses(g_ceph_context, CEPH_PICK_ADDRESS_PUBLIC, &public_addrs, iface_preferred_numa_node);
    if (r < 0) {
    ...
    r = pick_addresses(g_ceph_context, CEPH_PICK_ADDRESS_PUBLIC_BIND, &public_bind_addrs, iface_preferred_numa_node);
    if (r == -ENOENT) {
    ...
    r = pick_addresses(g_ceph_context, CEPH_PICK_ADDRESS_CLUSTER, &cluster_addrs, iface_preferred_numa_node);
    if (r < 0) {
    ...
    if (ms_cluster->bindv(cluster_addrs) < 0) forker.exit(1);
    ...
    if (ms_hb_front_server->bindv(hb_front_addrs) < 0) forker.exit(1);
    if (ms_hb_front_client->client_bind(hb_front_addrs.front()) < 0) forker.exit(1);

    entity_addrvec_t hb_back_addrs = cluster_addrs;
    for (auto& a : hb_back_addrs.v) {
        a.set_port(0);
    }
    if (ms_hb_back_server->bindv(hb_back_addrs) < 0) forker.exit(1);
    if (ms_hb_back_client->client_bind(hb_back_addrs.front()) < 0) forker.exit(1);
    ...
    ceph::async::io_context_pool poolctx(cct->_conf.get_val<std::uint64_t>("osd_asio_thread_count"));

    MonClient mc(g_ceph_context, poolctx);
    if (mc.build_initial_monmap() < 0) return -1;
    global_init_chdir(g_ceph_context);
    ...
    if (g_conf().get_val<bool>("inject_early_sigterm")) kill(getpid(), SIGTERM);
