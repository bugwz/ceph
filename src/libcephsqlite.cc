...
struct cephsqlite_appdata
{
    ...
    int setup_perf()
    {
        ...
        plb.add_time_avg(
            P_OPF_CHECKRESERVEDLOCK, "opf_checkreservedlock", "Time average of CheckReservedLock file operations");
        ...
        plb.add_time_avg(P_OPF_DEVICECHARACTERISTICS,
                         "opf_devicecharacteristics",
    ...
    int _open(CephContext* _cct)
    {
        if (!_cct) {
            ...
            CephInitParameters iparams =
                ceph_argparse_early_args(env_args, CEPH_ENTITY_TYPE_CLIENT, &cluster, &conf_file_list);
...
{
    return out << "[" << fileloc.pool << ":" << fileloc.radosns << "/" << fileloc.name << "]";
}
...
static int makestriper(sqlite3_vfs* vfs, cctptr cct, rsptr cluster, const cephsqlite_fileloc& loc,
...
enoent_retry:
    ...
    if (!loc.radosns.empty()) io->ioctx.set_namespace(loc.radosns);
...
static int DeviceCharacteristics(sqlite3_file* sf)
{
    ...
    auto start = ceph::coarse_mono_clock::now();
    df(5) << dendl;
    static const int c = 0 | SQLITE_IOCAP_ATOMIC | SQLITE_IOCAP_POWERSAFE_OVERWRITE |
...
static int Open(sqlite3_vfs* vfs, const char* name, sqlite3_file* file, int flags, int* oflags)
{
