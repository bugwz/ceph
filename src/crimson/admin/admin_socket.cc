...
seastar::future<> AdminSocket::finalize_response(seastar::output_stream<char>& out, ceph::bufferlist&& msgs)
{
    ...
    return out.write(reinterpret_cast<char*>(&response_length), sizeof(response_length)).then([&out, outbuf_cont] {
...
seastar::future<> AdminSocket::start(const std::string& path)
{
    ...
    try {
        ...
        if (e.code() == std::errc::address_in_use) {
            logger().debug("{}: Admin Socket socket path={} already exists, retrying", __func__, path);
...
public:
    ConfigHelpHook()
        : AdminSocketHook{"config help", "", "get config setting schema and descriptions"}
    {}
    seastar::future<tell_result_t> call(const cmdmap_t&, std::string_view format, ceph::bufferlist&& input) const final
    {
        unique_ptr<Formatter> f{Formatter::create(format, "json-pretty", "json-pretty")};
