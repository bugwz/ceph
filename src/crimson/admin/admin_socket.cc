...
seastar::future<> AdminSocket::register_command(std::unique_ptr<AdminSocketHook>&& hook)
{
    return seastar::with_lock(servers_tbl_rwlock, [this, hook = std::move(hook)]() mutable {
 ...
 */
auto AdminSocket::parse_cmd(const std::vector<std::string>& cmd) -> std::variant<parsed_command_t, tell_result_t>
{
    // preliminaries:
    ...
    if (auto found = hooks.find(prefix); found != hooks.end()) {
        ...
        return tell_result_t{-EINVAL, fmt::format("unknown command '{}'", prefix), std::move(out)};
    }
}

seastar::future<> AdminSocket::finalize_response(seastar::output_stream<char>& out, ceph::bufferlist&& msgs)
{
...
seastar::future<> AdminSocket::handle_command(crimson::net::ConnectionRef conn, boost::intrusive_ptr<MCommand> m)
{
    return execute_command(m->cmd, std::move(m->get_data())).then([conn, tid = m->get_tid()](auto result) {
...
auto AdminSocket::execute_command(const std::vector<std::string>& cmd, ceph::bufferlist&& buf)
    -> seastar::future<tell_result_t>
{
    return seastar::with_shared(servers_tbl_rwlock, [cmd, buf = std::move(buf), this]() mutable {
...
seastar::future<> AdminSocket::handle_client(seastar::input_stream<char>& in, seastar::output_stream<char>& out)
{
    auto consumer = seastar::make_shared<line_consumer>();
    return in.consume(*consumer)
        .then([consumer, &out, this] {
            logger().debug("AdminSocket::handle_client: incoming asok string: {}", consumer->line);
            return execute_line(consumer->line, out);
        })
        .then([&out] { return out.flush(); })
        .finally([&out] { return out.close(); })
        .then([&in] { return in.close(); })
        .handle_exception([](auto ep) { logger().debug("exception on {}: {}", __func__, ep); });
}

seastar::future<> AdminSocket::start(const std::string& path)
{
    if (path.empty()) {
        logger().error("{}: Admin Socket socket path missing from the configuration", __func__);
        return seastar::now();
    }
    ...
    task = seastar::do_until([this] { return stop_gate.is_closed(); },
               ...
               .finally([path] { return seastar::remove_file(path); });
    return seastar::make_ready_future<>();
}
...
public:
    ...
    {}
    seastar::future<tell_result_t> call(const cmdmap_t&, std::string_view format, ceph::bufferlist&&) const final
    {
...
public:
    explicit GetdescsHook(const AdminSocket& as)
        : AdminSocketHook{"get_command_descriptions", "", "list available commands"}
        , m_as{as}
    {}

    seastar::future<tell_result_t> call(const cmdmap_t& cmdmap, std::string_view format, ceph::bufferlist&&) const final
    {
        return seastar::with_shared(m_as.servers_tbl_rwlock, [format, this] {
            unique_ptr<Formatter> f{Formatter::create(format, "json-pretty", "json-pretty")};
            int cmdnum = 0;
            ...
            for (const auto& [prefix, hook] : m_as) {
                ...
                dump_cmd_and_help_to_json(f.get(), CEPH_FEATURES_ALL, secname, cmd, std::string{hook->help});
                cmdnum++;
            }
            f->close_section();
            return seastar::make_ready_future<tell_result_t>(std::move(f));
...
public:
    ...
    seastar::future<tell_result_t> call(const cmdmap_t& cmdmap, std::string_view format, ceph::bufferlist&&) const final
    {
        ...
        return local_conf()
            .inject_args(args)
            .then([] { return seastar::make_ready_future<tell_result_t>(); })
            .handle_exception_type([](const std::invalid_argument& e) {
                return seastar::make_ready_future<tell_result_t>(tell_result_t{-EINVAL, e.what()});
            });
    }
};
...
public:
    ConfigShowHook()
        : AdminSocketHook{"config show", "", "dump current config settings"}
    {}
    seastar::future<tell_result_t> call(const cmdmap_t&, std::string_view format, ceph::bufferlist&& input) const final
    {
        unique_ptr<Formatter> f{Formatter::create(format, "json-pretty", "json-pretty")};
...
public:
    ConfigGetHook()
        : AdminSocketHook("config get", "name=var,type=CephString", "config get <field>: get the config value")
    {}
    seastar::future<tell_result_t> call(const cmdmap_t& cmdmap, std::string_view format,
                                        ceph::bufferlist&& input) const final
    {
        ...
        if (int r = local_conf().get_val(var, &conf_val); r < 0) {
            return seastar::make_ready_future<tell_result_t>(
                tell_result_t{r, fmt::format("error getting {}: {}", var, cpp_strerror(r))});
        }
        unique_ptr<Formatter> f{Formatter::create(format, "json-pretty", "json-pretty")};
...
{
    return seastar::when_all_succeed(register_command(std::make_unique<VersionHook>()),
