...
{
public:
    AdminSocketHook(std::string_view prefix, std::string_view desc, std::string_view help)
        : prefix{prefix}
        , desc{desc}
        , help{help}
    {}
     ...
     */
    virtual seastar::future<tell_result_t> call(const cmdmap_t& cmdmap, std::string_view format,
...
public:
     ...
     */
    seastar::future<> handle_command(crimson::net::ConnectionRef conn, boost::intrusive_ptr<MCommand> m);
...
private:
    ...
    seastar::future<> finalize_response(seastar::output_stream<char>& out, ceph::bufferlist&& msgs);

    seastar::future<tell_result_t> execute_command(const std::vector<std::string>& cmd, ceph::bufferlist&& buf);
     ...
     */
    std::variant<parsed_command_t, tell_result_t> parse_cmd(const std::vector<std::string>& cmd);
...
public:
     ...
     * iterator support
     */
    hooks_t::const_iterator begin() const { return hooks.cbegin(); }
    hooks_t::const_iterator end() const { return hooks.cend(); }
};
