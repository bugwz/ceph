...
public:
    ...
    PGCommand(crimson::osd::OSD& osd, std::string_view prefix, std::string_view desc, std::string_view help)
                                        ...
                                        ceph::bufferlist&& input) const final
    {
        ...
        if (!cmd_getval(cmdmap, "pgid", pgid_str)) {
            return seastar::make_ready_future<tell_result_t>(tell_result_t{-EINVAL, "no pgid specified"});
        }
        ...
        if (!osdmap->get_primary_shard(pgid, &spg_id)) {
            return seastar::make_ready_future<tell_result_t>(
                tell_result_t{-ENOENT, fmt::format("pgid '{}' does not exist", pgid_str)});
        }
        return osd.get_pg_shard_manager().with_pg(
            spg_id,
            [this, spg_id, cmdmap = std::move(cmdmap), format = std::move(format), input = std::move(input)](
                auto&& pg) mutable {
                if (!pg) {
                    return seastar::make_ready_future<tell_result_t>(
                        tell_result_t{-ENOENT, fmt::format("i don't have pgid '{}'", spg_id)});
                }
                if (!pg->is_primary()) {
                    return seastar::make_ready_future<tell_result_t>(
                        tell_result_t{-EAGAIN, fmt::format("not primary for pgid '{}'", spg_id)});
                }
...
private:
    virtual seastar::future<tell_result_t> do_command(Ref<PG> pg, const cmdmap_t& cmdmap, std::string_view format,
...
public:
    ...
    explicit QueryCommand(crimson::osd::OSD& osd)
        : PGCommand{osd, "query", "", "show details of a specific pg"}
    {}

private:
    seastar::future<tell_result_t> do_command(Ref<PG> pg, const cmdmap_t&, std::string_view format,
                                              ceph::bufferlist&& input) const final
    {
...
public:
    explicit MarkUnfoundLostCommand(crimson::osd::OSD& osd)
        : PGCommand{osd,
                    ...
                    " available"}
    {}
    seastar::future<tell_result_t> do_command(Ref<PG> pg, const cmdmap_t& cmdmap, std::string_view,
                                              ceph::bufferlist&&) const final
    {
        ...
        if (cmd == "revert") {
            ...
            return seastar::make_ready_future<tell_result_t>(
                tell_result_t{-EINVAL, "mode must be 'revert' or 'delete'; mark not yet implemented"});
        }
...
template std::unique_ptr<AdminSocketHook> make_asok_hook<crimson::admin::pg::QueryCommand>(crimson::osd::OSD& osd);

template std::unique_ptr<AdminSocketHook> make_asok_hook<crimson::admin::pg::MarkUnfoundLostCommand>(
    crimson::osd::OSD& osd);
