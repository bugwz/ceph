...
template std::unique_ptr<AdminSocketHook> make_asok_hook<OsdStatusHook>(const crimson::osd::OSD& osd);
...
public:
    ...
    seastar::future<tell_result_t> call(const cmdmap_t&, std::string_view format, ceph::bufferlist&& input) const final
    {
        return osd.send_beacon().then([] { return seastar::make_ready_future<tell_result_t>(); });
    }

private:
    crimson::osd::OSD& osd;
};
template std::unique_ptr<AdminSocketHook> make_asok_hook<SendBeaconHook>(crimson::osd::OSD& osd);
...
public:
    explicit FlushPgStatsHook(crimson::osd::OSD& osd)
        : AdminSocketHook("flush_pg_stats", "", "flush pg stats")
        , osd{osd}
    {}
    seastar::future<tell_result_t> call(const cmdmap_t&, std::string_view format, ceph::bufferlist&& input) const final
    {
...
public:
    explicit DumpPGStateHistory(const crimson::osd::PGShardManager& pg_shard_manager)
        : AdminSocketHook{"dump_pgstate_history", "", "dump history of PGs' peering state"}
    ...
    seastar::future<tell_result_t> call(const cmdmap_t&, std::string_view format, ceph::bufferlist&& input) const final
    {
        ...
        return pg_shard_manager
            .for_each_pg([f](auto& pgid, auto& pg) {
...
template std::unique_ptr<AdminSocketHook> make_asok_hook<DumpPGStateHistory>(const crimson::osd::PGShardManager&);
...
public:
    DumpMetricsHook()
        : AdminSocketHook("dump_metrics", "name=group,type=CephString,req=false",
                                        ...
                                        ceph::bufferlist&& input) const final
    {
        ...
        return seastar::do_with(std::move(prefix),
                                [f](auto& prefix) {
...
private:
    ...
    static void dump_metric_value(Formatter* f, string_view full_name, const registered_metric& metric,
                                  const seastar::metrics::impl::labels_type& labels)
    {
        ...
        case data_type::GAUGE: f->dump_float(value_name, v.d()); break;
        case data_type::REAL_COUNTER: f->dump_float(value_name, v.d()); break;
...
static ghobject_t test_ops_get_object_name(const OSDMap& osdmap, const cmdmap_t& cmdmap)
{
    auto pool = [&] {
    ...
    auto [objname, nspace, raw_pg] = [&] {
        ...
        if (std::size_t sep_pos = obj_arg->find_first_of('/'); sep_pos != obj_arg->npos) {
        ...
        pg_t raw_pg;
        if (object_locator_t oloc(pool, nspace); osdmap.object_locator_to_pg(object_t(objname), oloc, raw_pg) < 0) {
            throw std::invalid_argument{"Invalid namespace/objname"};
        }
        return std::make_tuple(std::move(objname), std::move(nspace), std::move(raw_pg));
    }();

    auto shard_id = cmd_getval_or<int64_t>(cmdmap, "shardid", shard_id_t::NO_SHARD);

    return ghobject_t{hobject_t{object_t{objname}, std::string{}, CEPH_NOSNAP, raw_pg.ps(), pool, nspace},
                      ghobject_t::NO_GEN,
...
public:
                                        ...
                                        ceph::bufferlist&& input) const final
    {
        ...
        try {
            ...
            return seastar::make_ready_future<tell_result_t>(-EINVAL, e.what());
        }
...
template std::unique_ptr<AdminSocketHook> make_asok_hook<InjectDataErrorHook>(crimson::osd::ShardServices&);
...
template std::unique_ptr<AdminSocketHook> make_asok_hook<InjectMDataErrorHook>(crimson::osd::ShardServices&);
...
public:
    ...
    seastar::future<tell_result_t> call(const cmdmap_t&, std::string_view format, ceph::bufferlist&& input) const final
    {
        unique_ptr<Formatter> fref{Formatter::create(format, "json-pretty", "json-pretty")};
        auto* f = fref.get();
        f->open_object_section("ops_in_flight");
        f->open_array_section("ops_in_flight");
        return pg_shard_manager
            .invoke_on_each_shard_seq([f](const auto& shard_services) { return shard_services.dump_ops_in_flight(f); })
            .then([fref = std::move(fref)]() mutable {
                fref->close_section();
                fref->close_section();
                return seastar::make_ready_future<tell_result_t>(std::move(fref));
...
template std::unique_ptr<AdminSocketHook> make_asok_hook<DumpInFlightOpsHook>(const crimson::osd::PGShardManager&);
...
template std::unique_ptr<AdminSocketHook> make_asok_hook<DumpSlowestHistoricOpsHook>(
    const crimson::osd::OSDOperationRegistry& op_registry);
...
template std::unique_ptr<AdminSocketHook> make_asok_hook<DumpRecoveryReservationsHook>(
    crimson::osd::ShardServices& shard_services);
