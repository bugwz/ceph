...
public:
    ...
    // v1
    seastar::future<auth_result_t> authenticate_v1(epoch_t epoch, const EntityName& name, uint32_t want_keys);
    // v2
...
private:
    seastar::future<> setup_session(epoch_t epoch, const EntityName& name);
...
seastar::future<> Connection::setup_session(epoch_t epoch, const EntityName& name)
{
...
seastar::future<std::optional<Connection::auth_result_t>> Connection::do_auth_single(Connection::request_t what)
{
    ...
    return conn->send(m)
        ...
        .then([this](Ref<MAuthReply> m) {
            ...
            logger().info("do_auth: mon {} => {} returns {}: {}",
            ...
            if (ret != 0 && ret != -EAGAIN) {
                logger().error("do_auth: got error {} on mon {}", ret, conn->get_peer_addr());
            }
            return seastar::make_ready_future<std::optional<Connection::auth_result_t>>(
                ret == -EAGAIN ? std::nullopt
                               : std::make_optional(ret == 0 ? auth_result_t::success : auth_result_t::failure));
        });
}

seastar::future<Connection::auth_result_t> Connection::do_auth(Connection::request_t what)
{
    return seastar::repeat_until_value([this, what]() { return do_auth_single(what); });
}

seastar::future<Connection::auth_result_t> Connection::authenticate_v1(epoch_t epoch, const EntityName& name,
                                                                       uint32_t want_keys)
{
    return conn->keepalive()
        .then([epoch, name, this] { return setup_session(epoch, name); })
        .then([this] { return reply.get_shared_future(); })
        .then([name, want_keys, this](Ref<MAuthReply> m) {
            ...
            switch (auto p = m->result_bl.cbegin(); auth->handle_response(m->result, p, nullptr, nullptr)) {
            case 0:
                // none
...
int Client::handle_auth_request(crimson::net::ConnectionRef con, AuthConnectionMetaRef auth_meta, bool more,
                                uint32_t auth_method, const ceph::bufferlist& payload, ceph::bufferlist* reply)
{
...
auth::AuthClient::auth_request_t Client::get_auth_request(crimson::net::ConnectionRef con,
                                                          AuthConnectionMetaRef auth_meta)
{
    logger().info("get_auth_request(con={}, auth_method={})", con, auth_meta->auth_method);
    // connection to mon?
    if (con->get_peer_type() == CEPH_ENTITY_TYPE_MON) {
        auto found = std::find_if(pending_conns.begin(),
                                  pending_conns.end(),
                                  [peer_addr = con->get_peer_addr()](auto& mc) { return mc->is_my_peer(peer_addr); });
        if (found == pending_conns.end()) {
        ...
        if (!authorizer) {
            logger().error("failed to build_authorizer for type {}", ceph_entity_type_name(con->get_peer_type()));
            throw crimson::auth::error("unable to build auth");
        }
        auth_meta->authorizer.reset(authorizer);
        auth_meta->auth_method = authorizer->protocol;
        vector<uint32_t> modes;
        auth_registry.get_supported_modes(con->get_peer_type(), auth_meta->auth_method, &modes);
        return {authorizer->protocol, modes, authorizer->bl};
    }
}

ceph::bufferlist Client::handle_auth_reply_more(crimson::net::ConnectionRef conn, AuthConnectionMetaRef auth_meta,
                                                const bufferlist& bl)
{
...
int Client::handle_auth_done(crimson::net::ConnectionRef conn, AuthConnectionMetaRef auth_meta, uint64_t global_id,
...
int Client::handle_auth_bad_method(crimson::net::ConnectionRef conn, AuthConnectionMetaRef auth_meta,
...
seastar::future<> Client::handle_monmap(crimson::net::ConnectionRef conn, Ref<MMonMap> m)
{
...
seastar::future<> Client::handle_auth_reply(crimson::net::ConnectionRef conn, Ref<MAuthReply> m)
{
    logger().info("handle_auth_reply mon {} => {} returns {}: {}",
                  conn->get_messenger()->get_myaddr(),
                  conn->get_peer_addr(),
                  *m,
                  m->result);
...
Client::get_version_t Client::get_version(const std::string& map)
{
    ...
    return send_message(m).then([&req] { return req.get_future(); });
}
...
seastar::future<> Client::handle_mon_command_ack(Ref<MMonCommandAck> m)
{
    ...
    if (auto found = mon_commands.find(tid); found != mon_commands.end()) {
        auto& result = found->second;
...
seastar::future<> Client::reopen_session(int rank)
{
    ...
    return seastar::parallel_for_each(
               ...
               [this](auto rank) {
                   ...
                   return seastar::futurize_invoke([peer, this]() -> seastar::future<Connection::auth_result_t> {
...
Client::command_result_t Client::run_command(const std::vector<std::string>& cmd, const bufferlist& bl)
{
...
seastar::future<> Client::renew_subs()
{
    ...
    return send_message(m).then([this] { sub.renewed(); });
}
