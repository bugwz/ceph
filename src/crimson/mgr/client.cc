...
Client::Client(crimson::net::Messenger& msgr, WithStats& with_stats)
    : msgr{msgr}
...
std::optional<seastar::future<>> Client::ms_dispatch(crimson::net::ConnectionRef conn, MessageRef m)
{
    ...
    gate.dispatch_in_background(__func__, *this, [this, conn, &m, &dispatched] {
        switch (m->get_type()) {
        case MSG_MGR_MAP: return handle_mgr_map(conn, boost::static_pointer_cast<MMgrMap>(m));
        case MSG_MGR_CONFIGURE: return handle_mgr_conf(conn, boost::static_pointer_cast<MMgrConfigure>(m));
...
void Client::ms_handle_connect(crimson::net::ConnectionRef c, seastar::shard_id prv_shard)
{
...
seastar::future<> Client::reconnect()
{
    ...
    auto retry_interval = std::chrono::duration<double>(local_conf().get_val<double>("mgr_connect_retry_interval"));
    auto a_while = std::chrono::duration_cast<seastar::steady_clock_type::duration>(retry_interval);
    return seastar::sleep(a_while).then([this] {
        ...
        if (peer == entity_addr_t{}) {
            ...
            logger().error("mgr.{} does not have an addr compatible with me", mgrmap.get_active_name());
            return;
        }
...
seastar::future<> Client::handle_mgr_map(crimson::net::ConnectionRef, Ref<MMgrMap> m)
{
    mgrmap = m->get_map();
    if (!conn) {
        return reconnect();
    }
    else if (conn->get_peer_addr() != mgrmap.get_active_addrs().legacy_addr()) {
        return reconnect();
    }
    else {
        return seastar::now();
...
seastar::future<> Client::handle_mgr_conf(crimson::net::ConnectionRef, Ref<MMgrConfigure> m)
{
...
void Client::report()
{
    gate.dispatch_in_background(__func__, *this, [this] {
        ...
        return with_stats.get_stats().then([this](auto&& pg_stats) { return conn->send(std::move(pg_stats)); });
    });
}
