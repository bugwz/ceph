...
template<class T> void assert_moveable(T& t)
{
    // It's fine
}
template<class T> void assert_moveable(const T& t)
{
...
static auto _apply_method_to_tuple(Obj& obj, Method method, ArgTuple&& tuple, std::index_sequence<I...>)
{
...
auto apply_method_to_tuple(Obj& obj, Method method, ArgTuple&& tuple)
{
    ...
    return internal::_apply_method_to_tuple(
        obj, method, std::forward<ArgTuple>(tuple), std::make_index_sequence<tuple_size>());
}
