...
{
    install_oneshot_signals_handler<SIGSEGV, SIGABRT, SIGBUS, SIGILL, SIGFPE, SIGXCPU, SIGXFSZ, SIGSYS>();
}

template<int... SigNums> void FatalSignal::install_oneshot_signals_handler()
{
...
static void reraise_fatal(const int signum)
{
    ...
    if (::raise(signum)) {
        std::cerr << "reraise_fatal: failed to re-raise signal " << signum << std::endl;
    }
    else {
        std::cerr << "reraise_fatal: default handler for signal " << signum << " didn't terminate the process?"
                  << std::endl;
    }
    std::cerr << std::flush;
    ::_exit(1);
...
[[gnu::noinline]] void FatalSignal::signal_entry(const int signum, siginfo_t* const info, void*)
{
...
template<int SigNum> void FatalSignal::install_oneshot_signal_handler()
{
    struct sigaction sa;
...
[[gnu::noinline]] static void print_backtrace(std::string_view cause)
{
    ...
    std::cerr << ".\nBacktrace:\n";
    std::cerr << boost::stacktrace::stacktrace(FRAMES_TO_SKIP,
...
[[gnu::noinline]] void FatalSignal::signaled(const int signum, const siginfo_t& siginfo)
{
    switch (signum) {
    case SIGSEGV:
        ...
        break;
    case SIGABRT: print_backtrace("Aborting"); break;
    default: print_backtrace(fmt::format("Signal {}", signum)); break;
    }
    print_proc_maps();
}
