...
using registry_hook_t = boost::intrusive::list_member_hook<boost::intrusive::link_mode<boost::intrusive::auto_unlink>>;
...
namespace detail {
void dump_time_event(const char* name, const utime_t& timestamp, ceph::Formatter* f);
void dump_blocking_event(const char* name, const utime_t& timestamp, const Blocker* blocker, ceph::Formatter* f);
}   // namespace detail
...
template<class T> struct EventBackendRegistry
{
...
template<class T> struct Event
{
    ...
    template<class OpT, class... Args> void trigger(OpT&& op, Args&&... args)
    {
        that()->internal_backend.handle(*that(), std::forward<OpT>(op), std::forward<Args>(args)...);
        ...
        std::apply(
            [&,   // args=std::forward_as_tuple(std::forward<Args>(args)...),
             this](auto... backend) {
                (..., backend.handle(*that(), std::forward<OpT>(op), std::forward<Args>(args)...));
            },
...
template<class T> struct TimeEvent : Event<T>
{
    struct Backend
    {
    ...
    {
        void handle(T&, const Operation&) override { timestamp = ceph_clock_now(); }
        utime_t timestamp;
    } internal_backend;
    ...
    void dump(ceph::Formatter* f) const
    {
        ...
        detail::dump_time_event(demangled_name.c_str(), internal_backend.timestamp, f);
    }

    auto get_timestamp() const { return internal_backend.timestamp; }
};


template<typename T> class BlockerT : public Blocker
{
public:
    struct BlockingEvent : Event<typename T::BlockingEvent>
    {
        ...
        {
            void handle(typename T::BlockingEvent&, const Operation&, const T& blocker) override
            {
        ...
        struct TriggerI
        {
            TriggerI(BlockingEvent& event)
                : event(event)
            {}

            template<class FutureT> auto maybe_record_blocking(FutureT&& fut, const T& blocker)
            {
                if (!fut.available()) {
                    ...
                    return std::forward<FutureT>(fut).finally([&event = this->event, &blocker]() mutable {
        ...
        template<class OpT> struct Trigger : TriggerI
        {
            Trigger(BlockingEvent& event, const OpT& op)
                : TriggerI(event)
                , op(op)
            {}
            ...
            template<class FutureT> auto maybe_record_blocking(FutureT&& fut, const T& blocker)
            {
                if (!fut.available()) {
                    ...
                    return std::forward<FutureT>(fut).finally(
                        [&event = this->event, &blocker]() mutable { Trigger::record_unblocking(event, blocker); });
                }
                return std::forward<FutureT>(fut);
            }
        ...
        protected:
            void record_blocking(const T& blocker) override { this->event.trigger(op, blocker); }
        ...
        void dump(ceph::Formatter* f) const
        {
            ...
            detail::dump_blocking_event(
                demangled_name.c_str(), internal_backend.timestamp, internal_backend.blocker, f);
        }
    };

    virtual ~BlockerT() = default;
    template<class TriggerT, class... Args> decltype(auto) track_blocking(TriggerT&& trigger, Args&&... args)
    {
        return std::forward<TriggerT>(trigger).maybe_record_blocking(std::forward<Args>(args)...,
                                                                     static_cast<const T&>(*this));
    }

private:
    const char* get_type_name() const final { return static_cast<const T*>(this)->type_name; }
};

template<class T> struct AggregateBlockingEvent
{
    struct TriggerI
    {
    protected:
    ...
    public:
        template<class FutureT> auto maybe_record_blocking(FutureT&& fut, const typename T::Blocker& blocker)
        {
            ...
            return tcont->get_trigger()
                .maybe_record_blocking(std::move(fut), blocker)
    ...
    template<class OpT> struct Trigger final : TriggerI
    {
        ...
        class TriggerContainer final : public TriggerI::TriggerContainerI
        {
            ...
            typename T::TriggerI& get_trigger() final { return trigger; }
        ...
        public:
            ...
            ~TriggerContainer() final { event.events.erase(iter); }
        };
...
private:
    std::list<T> events;
    template<class OpT> friend class Trigger;
};
 ...
 */
class Operation : public boost::intrusive_ref_counter<Operation, boost::thread_unsafe_counter>
{
public:
    using id_t = uint64_t;
...
private:
    ...
    id_t id = 0;
    void set_id(id_t in_id) { id = in_id; }

    friend class OperationRegistryI;
    template<size_t> friend class OperationRegistryT;
};
...
{
    using op_list_member_option = boost::intrusive::member_hook<Operation, registry_hook_t, &Operation::registry_hook>;
...
public:
    using op_list =
        boost::intrusive::list<Operation, op_list_member_option, boost::intrusive::constant_time_size<false>>;

    template<typename T, typename... Args> auto create_operation(Args&&... args)
    {
...
template<size_t NUM_REGISTRIES> class OperationRegistryT : public OperationRegistryI
{
    Operation::id_t next_id = 0;
    std::array<op_list, NUM_REGISTRIES> registries;
...
protected:
    ...
    {
        return std::all_of(registries.begin(), registries.end(), [](auto& opl) { return opl.empty(); });
    }
...
protected:
    OperationRegistryT(core_id_t core)
        ...
        : next_id(static_cast<id_t>(core) << (std::numeric_limits<id_t>::digits - 8))
    {}

    template<size_t REGISTRY_INDEX> const op_list& get_registry() const
    {
        static_assert(REGISTRY_INDEX < std::tuple_size<decltype(registries)>::value);
        return registries[REGISTRY_INDEX];
    }

    template<size_t REGISTRY_INDEX> op_list& get_registry()
    {
        static_assert(REGISTRY_INDEX < std::tuple_size<decltype(registries)>::value);
        return registries[REGISTRY_INDEX];
    }
...
public:
    ...
    template<typename F> void for_each_op(F&& f) const
    {
...
template<class T> class PipelineStageIT : public BlockerT<T>
{
...
public:
    ...
    template<class... Args> decltype(auto) enter(Args&&... args)
    {
...
class PipelineHandle
{
    ...
    std::optional<seastar::future<>> wait_barrier() { return barrier ? barrier->wait() : std::nullopt; }
...
public:
    ...
    seastar::future<> enter(T& stage, typename T::BlockingEvent::template Trigger<OpT>&& t)
    {
        ...
        if (wait_fut.has_value()) {
            return wait_fut.value().then([this, &stage, t = std::move(t)]() mutable {
                ...
                exit();
                return std::move(fut).then([this, t = std::move(t)](auto&& barrier_ref) mutable {
 ...
 */
template<class T> class OrderedExclusivePhaseT : public PipelineStageIT<T>
{
    ...
    public:
        ...
        std::optional<seastar::future<>> wait() final { return std::nullopt; }
        ...
        void exit() final
        {
            if (phase) {
                ...
                auto id = op_id;
                phase = nullptr;
                std::ignore = seastar::smp::submit_to(p->get_core(), [p, id] { p->exit(id); });
            }
        }

        void cancel() final { exit(); }

        ~ExitBarrier() final { cancel(); }
    };
...
public:
    template<class TriggerT> seastar::future<PipelineExitBarrierI::Ref> enter(TriggerT& t)
    {
        waiting++;
 ...
 * enter.
 */
template<class T> class OrderedConcurrentPhaseT : public PipelineStageIT<T>
{
...
public:
    struct BlockingEvent : base_t::BlockingEvent
    {
        ...
        template<class OpT> struct Trigger : base_t::BlockingEvent::template Trigger<OpT>
        {
            using base_t::BlockingEvent::template Trigger<OpT>::Trigger;

            template<class FutureT> decltype(auto) maybe_record_exit_barrier(FutureT&& fut)
            {
                if (!fut.available()) {
...
private:
    ...
    template<class TriggerT> class ExitBarrier final : public PipelineExitBarrierI
    {
    ...
    public:
        ExitBarrier(OrderedConcurrentPhaseT* phase, seastar::future<>&& barrier, TriggerT& trigger)
            : phase(phase)
        ...
        {
            if (barrier) {
                static_cast<void>(std::move(*barrier).then([phase = this->phase] { phase->mutex.unlock(); }));
                barrier = std::nullopt;
                phase = nullptr;
            }
            if (phase) {
                std::ignore = seastar::smp::submit_to(phase->get_core(), [this] {
                    phase->mutex.unlock();
                    phase = nullptr;
                });
            }
...
public:
    template<class TriggerT> seastar::future<PipelineExitBarrierI::Ref> enter(TriggerT& t)
    {
        return seastar::make_ready_future<PipelineExitBarrierI::Ref>(new ExitBarrier<TriggerT>{this, mutex.lock(), t});
    }
 ...
 */
template<class T> class UnorderedStageT : public PipelineStageIT<T>
{
    void dump_detail(ceph::Formatter* f) const final {}
...
public:
    template<class... IgnoreArgs> seastar::future<PipelineExitBarrierI::Ref> enter(IgnoreArgs&&...)
    {
        return seastar::make_ready_future<PipelineExitBarrierI::Ref>(new ExitBarrier);
    }
};
