...
template<typename T, bool is_const> struct maybe_const_t
{};
template<typename T> struct maybe_const_t<T, true>
{
    using type = const T*;
};
template<typename T> struct maybe_const_t<T, false>
{
    using type = T*;
};
 ...
 */
template<size_t CAPACITY, typename Meta, typename MetaInt, typename K, typename KINT, typename V, typename VINT,
...
public:
    template<bool is_const> struct iter_t
    {
        ...
        iter_t() = default;
        iter_t(parent_t parent, uint16_t offset)
            : node(parent)
            , offset(offset)
        {}
        ...
        iter_t operator+(uint16_t off) const { return iter_t(node, offset + off); }
        iter_t operator-(uint16_t off) const { return iter_t(node, offset - off); }
        ...
        friend bool operator!=(const iter_t& lhs, const iter_t& rhs) { return !(lhs == rhs); }
        ...
        friend bool operator!=(const iter_t<is_const>& lhs, const iter_t<!is_const>& rhs) { return !(lhs == rhs); }
        K get_key() const { return K(node->get_key_ptr()[offset]); }
        ...
        V get_val() const { return V(node->get_val_ptr()[offset]); };

        bool contains(K addr) const { return (get_key() <= addr) && (get_next_key_or_max() > addr); }
    ...
    private:
        ...
        {
            return reinterpret_cast<typename maybe_const_t<char, is_const>::type>(node->get_key_ptr() + offset);
        }

        typename maybe_const_t<char, is_const>::type get_val_ptr() const
        {
            return reinterpret_cast<typename maybe_const_t<char, is_const>::type>(node->get_val_ptr() + offset);
        }
    };
    ...
    struct delta_t
    {
        ...
        bool operator==(const delta_t& rhs) const { return op == rhs.op && key == rhs.key && val == rhs.val; }
    };
...
public:
    ...
    public:
        ...
        void insert(const K& key, const V& val)
        {
            KINT k;
            k = key;
            buffer.push_back(delta_t{delta_t::op_t::INSERT, k, VINT(val)});
        }
        void update(const K& key, const V& val)
        {
            KINT k;
            k = key;
            buffer.push_back(delta_t{delta_t::op_t::UPDATE, k, VINT(val)});
        }
        void remove(const K& key)
        {
            KINT k;
            k = key;
            buffer.push_back(delta_t{delta_t::op_t::REMOVE, k, VINT()});
        }
        ...
        size_t get_bytes() const { return buffer.size() * sizeof(delta_t); }
        ...
        void copy_in(const char* out, size_t len)
        {
            ...
            buffer = std::vector(reinterpret_cast<const delta_t*>(out), reinterpret_cast<const delta_t*>(out + len));
        }
        bool operator==(const delta_buffer_t& rhs) const { return buffer == rhs.buffer; }
    };

    void journal_insert(const_iterator _iter, const K& key, const V& val, delta_buffer_t* recorder)
    {
    ...
    void journal_update(const_iterator _iter, const V& val, delta_buffer_t* recorder)
    {
        auto iter = iterator(this, _iter.offset);
        if (recorder) {
    ...
    void journal_replace(const_iterator _iter, const K& key, const V& val, delta_buffer_t* recorder)
    {
        auto iter = iterator(this, _iter.offset);
        if (recorder) {
    ...
    void journal_remove(const_iterator _iter, delta_buffer_t* recorder)
    {
        auto iter = iterator(this, _iter.offset);
        if (recorder) {
            recorder->remove(iter->get_key());
    ...
    const_iterator end() const { return const_iterator(this, get_size()); }
    ...
    const_iterator get_split_pivot() const { return iter_idx(get_size() / 2); }

    uint16_t get_size() const { return *layout.template Pointer<0>(buf); }
     ...
     */
    void set_size(uint16_t size) { *layout.template Pointer<0>(buf) = size; }
    ...
    void set_meta(const Meta& meta) { *layout.template Pointer<1>(buf) = MetaInt(meta); }

    constexpr static size_t get_capacity() { return CAPACITY; }
     ...
     */
    K split_into(FixedKVNodeLayout& left, FixedKVNodeLayout& right) const
    {
     ...
     */
    void merge_from(const FixedKVNodeLayout& left, const FixedKVNodeLayout& right)
    {
        copy_from_foreign(end(), left.begin(), left.end());
        set_size(left.get_size());
        copy_from_foreign(end(), right.begin(), right.end());
     ...
     */
    static K balance_into_new_nodes(const FixedKVNodeLayout& left, const FixedKVNodeLayout& right, bool prefer_left,
                                    FixedKVNodeLayout& replacement_left, FixedKVNodeLayout& replacement_right)
    {
        ...
        if (pivot_idx < left.get_size()) {
            replacement_left.copy_from_foreign(replacement_left.end(), left.begin(), left.iter_idx(pivot_idx));
            replacement_left.set_size(pivot_idx);

            replacement_right.copy_from_foreign(replacement_right.end(), left.iter_idx(pivot_idx), left.end());

            replacement_right.set_size(left.get_size() - pivot_idx);
            replacement_right.copy_from_foreign(replacement_right.end(), right.begin(), right.end());
            replacement_right.set_size(total - pivot_idx);
        }
        else {
            replacement_left.copy_from_foreign(replacement_left.end(), left.begin(), left.end());
            replacement_left.set_size(left.get_size());

            replacement_left.copy_from_foreign(
                replacement_left.end(), right.begin(), right.iter_idx(pivot_idx - left.get_size()));
            replacement_left.set_size(pivot_idx);

            replacement_right.copy_from_foreign(
                replacement_right.end(), right.iter_idx(pivot_idx - left.get_size()), right.end());
            replacement_right.set_size(total - pivot_idx);
        }

        auto [lmeta, rmeta] = Meta::rebalance(left.get_meta(), right.get_meta(), replacement_pivot);
...
private:
    void insert(iterator iter, const K& key, const V& val)
    {
    ...
    void update(iterator iter, V val)
    {
        assert(iter != end());
        iter->set_val(val);
    }

    void replace(iterator iter, const K& key, const V& val)
    {
        assert(iter != end());
        if (VALIDATE_INVARIANTS) {
            if (iter != begin()) {
     ...
     */
    KINT* get_key_ptr() { return layout.template Pointer<2>(buf); }
    const KINT* get_key_ptr() const { return layout.template Pointer<2>(buf); }
     ...
     */
    VINT* get_val_ptr() { return layout.template Pointer<3>(buf); }
    const VINT* get_val_ptr() const { return layout.template Pointer<3>(buf); }
    ...
    static void copy_from_foreign(iterator tgt, const_iterator from_src, const_iterator to_src)
    {
        ...
        memcpy(tgt->get_val_ptr(), from_src->get_val_ptr(), to_src->get_val_ptr() - from_src->get_val_ptr());
        memcpy(tgt->get_key_ptr(), from_src->get_key_ptr(), to_src->get_key_ptr() - from_src->get_key_ptr());
     ...
     */
    static void copy_from_local(iterator tgt, iterator from_src, iterator to_src)
    {
        assert(tgt->node == from_src->node);
        assert(to_src->node == from_src->node);
        memmove(tgt->get_val_ptr(), from_src->get_val_ptr(), to_src->get_val_ptr() - from_src->get_val_ptr());
        memmove(tgt->get_key_ptr(), from_src->get_key_ptr(), to_src->get_key_ptr() - from_src->get_key_ptr());
    }
};
