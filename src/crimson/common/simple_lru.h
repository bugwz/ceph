...
template<class Key, class Value, bool Ordered> class SimpleLRU
{
    static_assert(std::is_default_constructible_v<Value>);
    using list_type = std::list<Key>;
    template<class K, class V> using map_t = std::conditional_t<Ordered, std::map<K, V>, std::unordered_map<K, V>>;
...
typename SimpleLRU<Key, Value, Ordered>::insert_return_type SimpleLRU<Key, Value, Ordered>::insert(const Key& key,
                                                                                                   Value value)
{
...
template<class Key, class Value, bool Ordered> std::optional<Value> SimpleLRU<Key, Value, Ordered>::find(const Key& key)
{
...
template<class Key, class Value, bool Ordered>
std::optional<std::enable_if<Ordered, Value>> SimpleLRU<Key, Value, Ordered>::lower_bound(const Key& key)
{
...
template<class Key, class Value, bool Ordered> void SimpleLRU<Key, Value, Ordered>::clear()
{
    lru.clear();
    cache.clear();
}

template<class Key, class Value, bool Ordered> void SimpleLRU<Key, Value, Ordered>::erase(const Key& key)
{
    if (auto found = cache.find(key); found != cache.end()) {
...
template<class Key, class Value, bool Ordered>
Value SimpleLRU<Key, Value, Ordered>::_lru_add(typename SimpleLRU<Key, Value, Ordered>::map_type::iterator found)
{
...
template<class Key, class Value, bool Ordered> void SimpleLRU<Key, Value, Ordered>::_evict()
{
    // evict the last element of most recently used list
    auto last = --lru.end();
    cache.erase(*last);
