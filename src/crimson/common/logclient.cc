...
LogChannel::LogChannel(LogClient* lc, const string& channel, const string& facility, const string& prio)
    : parent(lc)
    , log_channel(channel)
    , log_prio(prio)
...
LogClient::LogClient(crimson::net::Messenger* m, logclient_flag_t flags)
    : messenger(m)
...
void LogChannel::update_config(map<string, string>& log_to_monitors, map<string, string>& log_to_syslog,
                               ...
                               map<string, string>& log_to_graylog_port, uuid_d& fsid, string& host)
{
    logger().debug("{} log_to_monitors {} log_to_syslog {} log_channels {} log_prios {}",
                   __func__,
                   log_to_monitors,
                   log_to_syslog,
                   log_channels,
    ...
    logger().debug("{} to_monitors: {} to_syslog: {}"
                   ...
                   "graylog_port: {}",
                   __func__,
                   (to_monitors ? "true" : "false"),
                   (to_syslog ? "true" : "false"),
                   syslog_facility,
                   prio,
                   (to_graylog ? "true" : "false"),
                   graylog_host,
                   graylog_port);
}
...
MessageURef LogClient::_get_mon_log_message()
{
    ...
    if (local_conf()->mon_client_max_log_entries_per_message > 0) {
        num_to_send = std::min(num_unsent, local_conf()->mon_client_max_log_entries_per_message);
    }
    else {
    ...
    return crimson::make_message<MLog>(m_fsid, std::move(out_log_queue));
}
...
LogChannelRef LogClient::create_channel(const std::string& name)
{
    ...
    if (it == channels.end()) {
        it = channels.insert(it, {name, seastar::make_lw_shared<LogChannel>(this, name)});
    }
    return it->second;
}
