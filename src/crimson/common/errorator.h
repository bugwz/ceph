...
template<typename, typename> class parallel_for_each_state;

template<typename, typename> class interruptible_future_detail;
...
public:
    do_for_each_state(Iterator begin, Iterator end, AsyncAction action, FutureT&& first_unavailable)
    ...
    task* waiting_task() noexcept override { return _pr.waiting_task(); }
    FutureT get_future() { return _pr.get_future(); }
};
...
template<typename Container, typename AsyncAction> inline auto do_for_each(Container& c, AsyncAction action)
{
    return ::crimson::do_for_each(std::begin(c), std::end(c), std::move(action));
}

template<typename AsyncAction> inline auto repeat(AsyncAction action)
{
    using errorator_t = typename ::seastar::futurize_t<std::invoke_result_t<AsyncAction>>::errorator_type;

    while (true) {
        auto f = ::seastar::futurize_invoke(action);
        if (f.failed()) {
            return errorator_t::template make_exception_future2<>(f.get_exception());
        }
        else if (f.available()) {
            if (auto done = f.get0()) {
            ...
            return std::move(f)._then([action = std::move(action)](auto stop) mutable {
                if (stop == seastar::stop_iteration::yes) {
                    return errorator_t::template make_ready_future<>();
                }
                return ::crimson::repeat(std::move(action));
            });
        }
    }
}
...
template<class ConcreteErrorT> class error_t
{
    ...
    template<class... AllowedErrorsT> friend struct errorator;

    template<class ErrorVisitorT, class FuturatorT> friend class maybe_handle_error_t;
...
public:
    template<class Func> static decltype(auto) handle(Func&& func)
    {
...
template<class ErrorT, ErrorT ErrorV> struct unthrowable_wrapper : error_t<unthrowable_wrapper<ErrorT, ErrorV>>
{
    ...
    static auto exception_ptr() { return make().to_exception_ptr(); }

    template<class Func> static auto handle(Func&& func)
    {
        return [func = std::forward<Func>(func)](const unthrowable_wrapper& raw_error) mutable -> decltype(auto) {
            if constexpr (std::is_invocable_v<Func, ErrorT, decltype(raw_error)>) {
                ...
                return std::invoke(std::forward<Func>(func), ErrorV, std::move(raw_error));
            }
    ...
    struct pass_further
    {
        decltype(auto) operator()(const unthrowable_wrapper& e) { return e; }
    };
...
private:
    ...
    static constexpr const std::type_info& exception_ptr_type_info() { return typeid(throwable_carrier); }
    ...
    static const auto& from_exception_ptr(std::exception_ptr) { return make(); }
...
std::exception_ptr unthrowable_wrapper<ErrorT, ErrorV>::carrier_instance =
    std::make_exception_ptr<unthrowable_wrapper<ErrorT, ErrorV>::throwable_carrier>({});
...
template<class ErrorT> struct stateful_error_t : error_t<stateful_error_t<ErrorT>>
{
    ...
    template<class Func> static auto handle(Func&& func)
    {
        return [func = std::forward<Func>(func)](stateful_error_t<ErrorT>&& e) mutable -> decltype(auto) {
...
private:
    ...
    explicit stateful_error_t(std::exception_ptr ep)
        : ep(std::move(ep))
    {}

    static constexpr const std::type_info& exception_ptr_type_info() { return typeid(ErrorT); }
    auto to_exception_ptr() const { return ep; }
...
public:
    ...
    template<class ErrorT> void handle()
    {
        static_assert(std::is_invocable<ErrorVisitorT, ErrorT>::value, "provided Error Visitor is not exhaustive");
        ...
        if (type_info == ErrorT::error_t::get_exception_ptr_type_info()) {
            ...
            if constexpr (std::is_assignable_v<decltype(result), return_t>) {
                ...
                std::invoke(std::forward<ErrorVisitorT>(errfunc), ErrorT::error_t::from_exception_ptr(std::move(ep)));
            }
                ...
                result = seastar::make_ready_future<return_t>(std::invoke(
                    std::forward<ErrorVisitorT>(errfunc), ErrorT::error_t::from_exception_ptr(std::move(ep))));
            }
            else {
                result = FuturatorT::type::errorator_type::template make_ready_future<return_t>(std::invoke(
                    std::forward<ErrorVisitorT>(errfunc), ErrorT::error_t::from_exception_ptr(std::move(ep))));
            }
        }
    }

    auto get_result() && { return std::move(result); }
};

template<class FuncHead, class... FuncTail> static constexpr auto composer(FuncHead&& head, FuncTail&&... tail)
{
    return [head = std::forward<FuncHead>(head),
            ...
            tail = std::make_tuple(std::forward<FuncTail>(tail)...)](auto&&... args) mutable -> decltype(auto) {
        if constexpr (std::is_invocable_v<FuncHead, decltype(args)...>) {
            return std::invoke(std::forward<FuncHead>(head), std::forward<decltype(args)>(args)...);
        }
        else if constexpr (sizeof...(FuncTail) > 0) {
            using next_composer_t = decltype(composer<FuncTail...>);
            auto&& next = std::apply<next_composer_t>(composer<FuncTail...>, std::move(tail));
            return std::invoke(std::move(next), std::forward<decltype(args)>(args)...);
        }
        else {
            static_assert(std::is_invocable_v<FuncHead, decltype(args)...> || (sizeof...(FuncTail) > 0),
                          "composition is not exhaustive");
        }
    };
}

template<class ValueT> struct errorated_future_marker
{};

template<class... AllowedErrors> class parallel_for_each_state;

template<class T> static inline constexpr bool is_error_v = std::is_base_of_v<error_t<T>, T>;

template<typename... AllowedErrors> struct errorator;

template<typename Iterator, typename Func, typename... AllowedErrors>
static inline typename errorator<AllowedErrors...>::template future<> parallel_for_each(Iterator first, Iterator last,
                                                                                        Func&& func) noexcept;

template<class... AllowedErrors> struct errorator
{

    static_assert((... && is_error_v<AllowedErrors>), "errorator expects presence of ::is_error in all error types");

    template<class ErrorT> struct contains_once
    {
        static constexpr bool value = (0 + ... + std::is_same_v<ErrorT, AllowedErrors>) == 1;
    };
    template<class... Errors> struct contains_once<errorator<Errors...>>
    {
        static constexpr bool value = (... && contains_once<Errors>::value);
    };
    template<class T> static constexpr bool contains_once_v = contains_once<T>::value;

    static_assert((... && contains_once_v<AllowedErrors>), "no error type in errorator can be duplicated");
...
private:
    ...
    template<class> class [[nodiscard]] _future
    {};
    template<class ValueT>
    class [[nodiscard]] _future<::crimson::errorated_future_marker<ValueT>> : private seastar::future<ValueT>
    {
        ...
        template<class ErrorVisitor, class Futurator> friend class maybe_handle_error_t;

        // any `seastar::futurize` specialization must be able to access the base.
        // see : `satisfy_with_result_of()` far below.
        template<typename> friend struct seastar::futurize;

        template<typename T1, typename T2, typename... More>
        friend auto seastar::internal::do_with_impl(T1&& rv1, T2&& rv2, More&&... more);

        template<class, class = std::void_t<>> struct get_errorator
        {
            // generic template for non-errorated things (plain types and
            // vanilla seastar::future as well).
            using type = errorator<>;
        };
        template<class FutureT> struct get_errorator<FutureT, std::void_t<typename FutureT::errorator_type>>
        {
            using type = typename FutureT::errorator_type;
        };
        template<class T> using get_errorator_t = typename get_errorator<T>::type;

        template<class ValueFuncErroratorT, class... ErrorVisitorRetsT> struct make_errorator
        {
            // NOP. The generic template.
        };
        template<class... ValueFuncAllowedErrors, class ErrorVisitorRetsHeadT, class... ErrorVisitorRetsTailT>
        struct make_errorator<errorator<ValueFuncAllowedErrors...>, ErrorVisitorRetsHeadT, ErrorVisitorRetsTailT...>
        {
        private:
            ...
            using _next_errorator =
                std::conditional_t<is_error_v<ErrorVisitorRetsHeadT> &&
                                       !step_errorator::template contains_once_v<ErrorVisitorRetsHeadT>,
                                   typename step_errorator::template extend<ErrorVisitorRetsHeadT>, step_errorator>;
            using maybe_head_ertr = get_errorator_t<ErrorVisitorRetsHeadT>;
            using next_errorator = typename _next_errorator::template extend_ertr<maybe_head_ertr>;

        public:
            using type = typename make_errorator<next_errorator, ErrorVisitorRetsTailT...>::type;
        };
        // finish the recursion
        template<class... ValueFuncAllowedErrors> struct make_errorator<errorator<ValueFuncAllowedErrors...>>
        {
            using type = ::crimson::errorator<ValueFuncAllowedErrors...>;
        };
        template<class... Args> using make_errorator_t = typename make_errorator<Args...>::type;

        using base_t::base_t;

        template<class Futurator, class Future, class ErrorVisitor>
        [[gnu::noinline]] static auto _safe_then_handle_errors(Future&& future, ErrorVisitor&& errfunc)
        {
            maybe_handle_error_t<ErrorVisitor, Futurator> maybe_handle_error(std::forward<ErrorVisitor>(errfunc),
    ...
    public:
        ...
        [[gnu::always_inline]] _future(base_t&& base)
            : base_t(std::move(base))
        {}

        base_t to_base() && { return std::move(*this); }

        template<class... A>
        [[gnu::always_inline]] _future(ready_future_marker, A&&... a)
            : base_t(::seastar::make_ready_future<ValueT>(std::forward<A>(a)...))
        {}
        [[gnu::always_inline]] _future(exception_future_marker, ::seastar::future_state_base&& state) noexcept
            : base_t(::seastar::futurize<base_t>::make_exception_future(std::move(state)))
        {}
        [[gnu::always_inline]] _future(exception_future_marker, std::exception_ptr&& ep) noexcept
            : base_t(::seastar::futurize<base_t>::make_exception_future(std::move(ep)))
        {}

        template<
            template<class...> class ErroratedFuture,
            class = std::void_t<typename ErroratedFuture<::crimson::errorated_future_marker<ValueT>>::errorator_type>>
        operator ErroratedFuture<errorated_future_marker<ValueT>>() &&
        {
            using dest_errorator_t =
                typename ErroratedFuture<::crimson::errorated_future_marker<ValueT>>::errorator_type;
        ...
        template<class ErrorT, class DecayedT = std::decay_t<ErrorT>, bool IsError = is_error_v<DecayedT>,
                 class = std::enable_if_t<IsError>>
        _future(ErrorT&& e)
            : base_t(seastar::make_exception_future<ValueT>(errorator_type::make_exception_ptr(e)))
        {
            static_assert(errorator_type::contains_once_v<DecayedT>, "ErrorT is not enlisted in errorator");
        }

        template<class ValueFuncT, class ErrorVisitorT> auto safe_then(ValueFuncT&& valfunc, ErrorVisitorT&& errfunc)
        {
            static_assert((... && std::is_invocable_v<ErrorVisitorT, AllowedErrors>),
                          "provided Error Visitor is not exhaustive");
            static_assert(
                std::is_void_v<ValueT> ? std::is_invocable_v<ValueFuncT> : std::is_invocable_v<ValueFuncT, ValueT>,
                "Value Func is not invocable with future's value");
            using value_func_result_t = typename std::conditional_t<std::is_void_v<ValueT>,
            ...
            using return_errorator_t =
                make_errorator_t<value_func_errorator_t,
            ...
            using futurator_t = typename return_errorator_t::template futurize<value_func_result_t>;
            ...
            return this->then_wrapped([valfunc = std::forward<ValueFuncT>(valfunc),
                                       errfunc = std::forward<ErrorVisitorT>(errfunc)](auto&& future) mutable noexcept {
                if (__builtin_expect(future.failed(), false)) {
                    return _safe_then_handle_errors<futurator_t>(std::move(future),
                    ...
                    // `::then()` already does.
                    return futurator_t::invoke(std::forward<ValueFuncT>(valfunc), std::move(future).get());
                }
            });
        }
         ...
         */
        auto&& unsafe_get() { return seastar::future<ValueT>::get(); }
        auto unsafe_get0() { return seastar::future<ValueT>::get0(); }

        template<class FuncT> _future finally(FuncT&& func)
        {
            return this->then_wrapped([func = std::forward<FuncT>(func)](auto&& result) mutable noexcept {
                if constexpr (seastar::InvokeReturnsAnyFuture<FuncT>) {
                    return ::seastar::futurize_invoke(std::forward<FuncT>(func))
                        .then_wrapped([result = std::move(result)](auto&& f_res) mutable {
                            // TODO: f_res.failed()
        ...
        _future<::crimson::errorated_future_marker<void>> discard_result() noexcept
        {
        ...
        // to avoid SFINAE
        template<class ValueFunc, class ErrorFuncHead, class... ErrorFuncTail>
        auto safe_then(ValueFunc&& value_func, ErrorFuncHead&& error_func_head, ErrorFuncTail&&... error_func_tail)
        {
        ...
        template<class ValueFunc> auto safe_then(ValueFunc&& value_func)
        {
            return safe_then(std::forward<ValueFunc>(value_func), errorator_type::pass_further{});
        }

        template<class ValueFunc, class... ErrorFuncs>
        auto safe_then_unpack(ValueFunc&& value_func, ErrorFuncs&&... error_funcs)
        {
            return safe_then(
        ...
        template<class Func> void then(Func&&) = delete;

        template<class ErrorVisitorT> auto handle_error(ErrorVisitorT&& errfunc)
        {
            static_assert((... && std::is_invocable_v<ErrorVisitorT, AllowedErrors>),
                          "provided Error Visitor is not exhaustive");
            using return_errorator_t =
                make_errorator_t<errorator<>, std::decay_t<std::invoke_result_t<ErrorVisitorT, AllowedErrors>>...>;
            using futurator_t = typename return_errorator_t::template futurize<::seastar::future<ValueT>>;
            return this->then_wrapped([errfunc = std::forward<ErrorVisitorT>(errfunc)](auto&& future) mutable noexcept {
                if (__builtin_expect(future.failed(), false)) {
                    return _safe_then_handle_errors<futurator_t>(std::move(future),
                                                                 std::forward<ErrorVisitorT>(errfunc));
                }
        ...
        template<class ErrorFuncHead, class... ErrorFuncTail>
        auto handle_error(ErrorFuncHead&& error_func_head, ErrorFuncTail&&... error_func_tail)
        {
            static_assert(sizeof...(ErrorFuncTail) > 0);
            return this->handle_error(composer(std::forward<ErrorFuncHead>(error_func_head),
                                               std::forward<ErrorFuncTail>(error_func_tail)...));
        }

    private:
        // for ::crimson::do_for_each
        template<class Func> auto _then(Func&& func) { return base_t::then(std::forward<Func>(func)); }
        template<class T> auto _forward_to(T&& pr) { return base_t::forward_to(std::forward<T>(pr)); }
        template<typename Iterator, typename AsyncAction>
        friend inline auto ::crimson::do_for_each(Iterator begin, Iterator end, AsyncAction action);

        template<typename Iterator, typename AsyncAction, typename FutureT> friend class ::crimson::do_for_each_state;

        template<typename AsyncAction> friend inline auto ::crimson::repeat(AsyncAction action);

        template<typename Result> friend class ::seastar::future;

        // let seastar::do_with_impl to up-cast us to seastar::future.
        template<typename T, typename F> friend inline auto ::seastar::internal::do_with_impl(T&& rvalue, F&& f);
        template<typename T1, typename T2, typename T3_or_F, typename... More>
        friend inline auto ::seastar::internal::do_with_impl(T1&& rv1, T2&& rv2, T3_or_F&& rv3, More&&... more);
        template<typename, typename> friend class ::crimson::interruptible::interruptible_future_detail;
        friend class ::crimson::parallel_for_each_state<AllowedErrors...>;
        template<typename IC, typename FT> friend class ::crimson::interruptible::parallel_for_each_state;
    };

    class Enabler
    {};

    template<typename T> using EnableIf = typename std::enable_if<contains_once_v<std::decay_t<T>>, Enabler>::type;

    template<typename ErrorFunc> struct all_same_way_t
    {
        ErrorFunc func;
        ...
        template<typename ErrorT, EnableIf<ErrorT>...> decltype(auto) operator()(ErrorT&& e)
        {
            using decayed_t = std::decay_t<decltype(e)>;
            auto&& handler = decayed_t::error_t::handle(std::forward<ErrorFunc>(func));
...
public:
    ...
    template<class ValueT = void> using future = _future<::crimson::errorated_future_marker<ValueT>>;

    // the visitor that forwards handling of all errors to next continuation
    struct pass_further
    {
        template<class ErrorT, EnableIf<ErrorT>...> decltype(auto) operator()(ErrorT&& e)
        {
            static_assert(contains_once_v<std::decay_t<ErrorT>>, "passing further disallowed ErrorT");
    ...
    struct discard_all
    {
        template<class ErrorT, EnableIf<ErrorT>...> void operator()(ErrorT&&)
        {
            static_assert(contains_once_v<std::decay_t<ErrorT>>, "discarding disallowed ErrorT");
        }
    };

    template<typename T> static future<T> make_errorator_future(seastar::future<T>&& fut) { return std::move(fut); }
    ...
    template<class ErrorFunc> static decltype(auto) all_same_way(ErrorFunc&& error_func)
    {
    ...
    template<class... NewAllowedErrorsT> using extend = errorator<AllowedErrors..., NewAllowedErrorsT...>;
    ...
    template<class OtherErroratorT> struct unify
    {
        // 1st: generic NOP template
    };
    template<class OtherAllowedErrorsHead, class... OtherAllowedErrorsTail>
    struct unify<errorator<OtherAllowedErrorsHead, OtherAllowedErrorsTail...>>
    {
    private:
        ...
        using step_errorator =
            std::conditional_t<contains_once_v<OtherAllowedErrorsHead> == false,
                               errorator<AllowedErrors..., OtherAllowedErrorsHead>, errorator<AllowedErrors...>>;
    ...
    template<class... EmptyPack> struct unify<errorator<EmptyPack...>>
    {
    ...
    template<class E> using extend_ertr = typename unify<E>::type;

    template<typename T = void, typename... A> static future<T> make_ready_future(A&&... value)
    {
        return future<T>(ready_future_marker(), std::forward<A>(value)...);
    }

    template<typename T = void> static future<T> make_exception_future2(std::exception_ptr&& ex) noexcept
    {
        return future<T>(exception_future_marker(), std::move(ex));
    }
    template<typename T = void> static future<T> make_exception_future2(seastar::future_state_base&& state) noexcept
    {
        return future<T>(exception_future_marker(), std::move(state));
    }
    template<typename T = void, typename Exception> static future<T> make_exception_future2(Exception&& ex) noexcept
    {
        return make_exception_future2<T>(std::make_exception_ptr(std::forward<Exception>(ex)));
    }

    static auto now() { return make_ready_future<>(); }
    ...
    static inline errorator<AllowedErrors...>::future<> parallel_for_each(Iterator first, Iterator last,
                                                                          Func&& func) noexcept
    {
        return crimson::parallel_for_each<Iterator, Func, AllowedErrors...>(first, last, std::forward<Func>(func));
    }

private:
    template<class T> class futurize
    {
        ...
        template<class Stored, int Dummy = 0> struct stored_to_future
        {
            using type = future<Stored>;
        };
        template<int Dummy> struct stored_to_future<seastar::internal::monostate, Dummy>
        {
            using type = future<>;
        };

    public:
        using type = typename stored_to_future<typename vanilla_futurize::value_type>::type;

        template<class Func, class... Args> static type invoke(Func&& func, Args&&... args)
        {
            try {
                return vanilla_futurize::invoke(std::forward<Func>(func), std::forward<Args>(args)...);
            }
            catch (...) {
        ...
        template<class Func> static type invoke(Func&& func, seastar::internal::monostate)
        {
            try {
    ...
    template<template<class...> class ErroratedFutureT, class ValueT>
    ...
    class futurize<::crimson::interruptible::interruptible_future_detail<InterruptCond, FutureType>>
    {
    public:
        using type =
            ::crimson::interruptible::interruptible_future_detail<InterruptCond, typename futurize<FutureType>::type>;

        template<typename Func, typename... Args> static type invoke(Func&& func, Args&&... args)
        {
            try {
                return ::seastar::futurize_invoke(std::forward<Func>(func), std::forward<Args>(args)...);
            }
        ...
        template<typename Func> static type invoke(Func&& func, seastar::internal::monostate)
        {
            try {
                return ::seastar::futurize_invoke(std::forward<Func>(func));
            }
        ...
        template<typename Arg> static type make_exception_future(Arg&& arg)
        {
            return ::seastar::futurize<
                ::crimson::interruptible::interruptible_future_detail<InterruptCond, FutureType>>::
                make_exception_future(std::forward<Arg>(arg));
        }
    };

    template<class ErrorT> static std::exception_ptr make_exception_ptr(ErrorT&& e)
    {
    ...
    template<class...> friend class errorator;
    template<typename, typename> friend class ::crimson::interruptible::interruptible_future_detail;
};   // class errorator, generic template

// no errors? errorator<>::future is plain seastar::future then!
template<> class errorator<>
{
public:
    template<class ValueT = void> using future = ::seastar::futurize_t<ValueT>;

    template<class T> using futurize = ::seastar::futurize<T>;

    // get a new errorator by extending current one with errors
    template<class... NewAllowedErrors> using extend = errorator<NewAllowedErrors...>;

    // get a new errorator by extending current one with another errorator
    template<class E> using extend_ertr = E;

    // errorator with empty error set never contains any error
    template<class T> static constexpr bool contains_once_v = false;
};   // class errorator, <> specialization


template<class ErroratorOne, class ErroratorTwo, class... FurtherErrators> struct compound_errorator
{
private:
    // generic template. Empty `FurtherErrators` are handled by
    // the specialization below.
    static_assert(sizeof...(FurtherErrators) > 0);
    using step = typename compound_errorator<ErroratorOne, ErroratorTwo>::type;

public:
    using type = typename compound_errorator<step, FurtherErrators...>::type;
};
template<class ErroratorOne, class ErroratorTwo> struct compound_errorator<ErroratorOne, ErroratorTwo>
{
    // specialization for empty `FurtherErrators` arg pack
    using type = typename ErroratorOne::template unify<ErroratorTwo>::type;
};
template<class... Args> using compound_errorator_t = typename compound_errorator<Args...>::type;
...
template<int ErrorV> inline std::error_code ec = std::error_code(ErrorV, std::generic_category());

template<int ErrorV> using ct_error_code = unthrowable_wrapper<const std::error_code&, ec<ErrorV>>;
...
using operation_not_supported = ct_error_code<static_cast<int>(std::errc::operation_not_supported)>;
...
using erange = ct_error_code<static_cast<int>(std::errc::result_out_of_range)>;
using ebadf = ct_error_code<static_cast<int>(std::errc::bad_file_descriptor)>;
using enospc = ct_error_code<static_cast<int>(std::errc::no_space_on_device)>;
using value_too_large = ct_error_code<static_cast<int>(std::errc::value_too_large)>;
using eagain = ct_error_code<static_cast<int>(std::errc::resource_unavailable_try_again)>;
using file_too_large = ct_error_code<static_cast<int>(std::errc::file_too_large)>;
...
{
    template<class ErrorT> decltype(auto) operator()(ErrorT&& e) { return std::forward<ErrorT>(e); }
};

struct discard_all
{
    template<class ErrorT> void operator()(ErrorT&&) {}
};
...
template<class ErrorFunc> static decltype(auto) all_same_way(ErrorFunc&& error_func)
{
    return [error_func = std::forward<ErrorFunc>(error_func)](auto&& e) mutable -> decltype(auto) {
        using decayed_t = std::decay_t<decltype(e)>;
        auto&& handler = decayed_t::error_t::handle(std::forward<ErrorFunc>(error_func));
...
template<typename F> struct is_errorated_future
{
    static constexpr bool value = false;
};
template<template<class...> class ErroratedFutureT, class ValueT>
struct is_errorated_future<ErroratedFutureT<::crimson::errorated_future_marker<ValueT>>>
{
    static constexpr bool value = true;
};
template<typename T> constexpr bool is_errorated_future_v = is_errorated_future<T>::value;
...
template<template<class> class Container, class Value>
struct futurize<Container<::crimson::errorated_future_marker<Value>>>
{
    using errorator_type = typename Container<::crimson::errorated_future_marker<Value>>::errorator_type;
    ...
    [[gnu::always_inline]] static type apply(Func&& func, std::tuple<FuncArgs...>&& args) noexcept
    {
        try {
            return std::apply(std::forward<Func>(func), std::forward<std::tuple<FuncArgs...>>(args));
        }
        catch (...) {
            return make_exception_future(std::current_exception());
        }
    ...
    template<class Func> [[gnu::always_inline]] static type invoke(Func&& func, seastar::internal::monostate) noexcept
    {
        try {
            return func();
        }
    ...
    template<typename Arg> [[gnu::always_inline]] static type make_exception_future(Arg&& arg)
    {
