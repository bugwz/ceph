...
template<class K, class V> class SharedLRU
{
    ...
    void _erase_weak(const K& key) { weak_refs.erase(key); }
...
public:
     ...
     * in the cache.
     */
    bool empty() const { return weak_refs.empty(); }
...
typename SharedLRU<K, V>::shared_ptr_t SharedLRU<K, V>::insert(const K& key, std::unique_ptr<V> value)
{
    shared_ptr_t val;
...
template<class K, class V> typename SharedLRU<K, V>::shared_ptr_t SharedLRU<K, V>::operator[](const K& key)
{
...
template<class K, class V> typename SharedLRU<K, V>::shared_ptr_t SharedLRU<K, V>::find(const K& key)
{
    if (auto found = cache.find(key); found) {
        return *found;
    }
...
template<class K, class V> typename SharedLRU<K, V>::shared_ptr_t SharedLRU<K, V>::lower_bound(const K& key)
{
    if (weak_refs.empty()) {
        return {};
    }
...
template<class K, class V>
std::optional<typename SharedLRU<K, V>::value_type> SharedLRU<K, V>::upper_bound(const K& key)
{
    for (auto found = weak_refs.upper_bound(key); found != weak_refs.end(); ++found) {
        if (auto val = found->second.first.lock(); val) {
