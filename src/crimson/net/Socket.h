...
public:
    ...
    side_t get_side() const { return side; }

    uint16_t get_ephemeral_port() const { return ephemeral_port; }

    seastar::socket_address get_local_address() const { return socket.local_address(); }
    ...
    {
        assert(side == side_t::connector && (ephemeral_port == 0 || ephemeral_port == port));
        ephemeral_port = port;
    }
...
using listen_ertr = crimson::errorator<
    ...
    crimson::ct_error::
        address_not_available   // https://techoverflow.net/2021/08/06/how-i-fixed-python-oserror-errno-99-cannot-assign-requested-address/
    >;

class ShardedServerSocket : public seastar::peering_sharded_service<ShardedServerSocket>
{
    struct construct_tag
    {};

public:
    ShardedServerSocket(seastar::shard_id sid, bool dispatch_only_on_primary_sid, construct_tag);
    ...
    bool is_fixed_shard_dispatching() const { return dispatch_only_on_primary_sid; }

    listen_ertr::future<> listen(entity_addr_t addr);

    using accept_func_t = std::function<seastar::future<>(SocketRef, entity_addr_t)>;
    seastar::future<> accept(accept_func_t&& _fn_accept);

    seastar::future<> shutdown_destroy();

    static seastar::future<ShardedServerSocket*> create(bool dispatch_only_on_this_shard);
