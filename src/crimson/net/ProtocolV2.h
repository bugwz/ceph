...
{
public:
    ProtocolV2(ChainedDispatchers& dispatchers, SocketConnection& conn, SocketMessenger& messenger);
    ~ProtocolV2() override;
...
private:
    ...
    void start_connect(const entity_addr_t& peer_addr, const entity_name_t& peer_name) override;

    void start_accept(SocketRef&& socket, const entity_addr_t& peer_addr) override;

    void trigger_close() override;

    ceph::bufferlist do_sweep_messages(const std::deque<MessageRef>& msgs, size_t num_msgs, bool require_keepalive,
                                       std::optional<utime_t> keepalive_ack, bool require_ack) override;
...
private:
    ...
    template<typename Func> void gated_execute(const char* what, Func&& func)
    {
...
private:
    ...
    template<class F> seastar::future<> write_frame(F& frame, bool flush = true);
...
private:
    ...
    // REPLACING (server)
    void trigger_replacing(bool reconnect, bool do_reset, SocketRef&& new_socket, AuthConnectionMetaRef&& new_auth_meta,
                           ceph::crypto::onwire::rxtx_t new_rxtx, uint64_t new_peer_global_seq,
                           // !reconnect
                           uint64_t new_client_cookie, entity_name_t new_peer_name, uint64_t new_conn_features,
                           bool tx_is_rev1, bool rx_is_rev1,
                           // reconnect
