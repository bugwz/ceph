...
public:
    SocketMessenger(const entity_name_t& myname, const std::string& logic_name, uint32_t nonce,
                    bool dispatch_only_on_this_shard);

    ~SocketMessenger() override;

    const entity_name_t& get_myname() const override { return my_name; }

    const entity_addrvec_t& get_myaddrs() const override { return my_addrs; }
    ...
    ConnectionRef connect(const entity_addr_t& peer_addr, const entity_name_t& peer_name) override;
    ...
    void print(std::ostream& out) const override { out << get_myname() << "(" << logic_name << ") " << get_myaddr(); }
...
public:
    ...
    void learned_addr(const entity_addr_t& peer_addr_for_me, const SocketConnection& conn);
    ...
    void set_interceptor(Interceptor* i) override { interceptor = i; }
...
private:
    ...
    bind_ertr::future<> try_bind(const entity_addrvec_t& addr, uint32_t min_port, uint32_t max_port);
