...
public:
    ...
    bind_ertr::future<> try_bind(const entity_addrvec_t& addr, uint32_t min_port, uint32_t max_port) override;
    ...
    void stop() override { dispatchers.clear(); }

    bool is_started() const override { return !dispatchers.empty(); }

    seastar::future<> shutdown() override;

    void print(ostream& out) const override { out << get_myname() << "(" << logic_name << ") " << get_myaddr(); }
...
public:
    ...
    seastar::future<> learned_addr(const entity_addr_t& peer_addr_for_me, const SocketConnection& conn);
