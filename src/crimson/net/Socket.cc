...
seastar::future<seastar::temporary_buffer<char>> Socket::read_exactly(size_t bytes)
{
#ifdef UNIT_TESTS_BUILT
    return try_trap_pre(next_trap_read)
        .then([bytes, this] {
...
seastar::future<> Socket::close()
{
    ...
    closed = true;
#endif
    return seastar::when_all_succeed(in.close(), close_and_handle_errors(out))
...
FixedCPUServerSocket::listen_ertr::future<> FixedCPUServerSocket::listen(entity_addr_t addr)
{
...
seastar::future<FixedCPUServerSocket*> FixedCPUServerSocket::create()
{
    ...
    return seastar::smp::submit_to(
               ...
               [cpu] {
                   ...
                   return service->start(cpu, construct_tag{}).then([service = std::move(service)]() mutable {
