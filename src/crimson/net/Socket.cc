    ...
    seastar::future<consumption_result_type> operator()(tmp_buf&& data)
    {
        if (remaining >= data.size()) {
            ...
            if (remaining > 0) {
                ...
                return seastar::make_ready_future<consumption_result_type>(seastar::continue_consuming{});
            }
            else {
...
{
    if (float delay_period = local_conf()->ms_inject_internal_delays; delay_period) {
        logger().debug("Socket::inject_delay: sleep for {}", delay_period);
        return seastar::sleep(std::chrono::milliseconds((int)(delay_period * 1000.0)));
    }
    return seastar::now();
}
...
void inject_failure()
{
    if (local_conf()->ms_inject_socket_failures) {
        uint64_t rand = ceph::util::generate_random_number<uint64_t>(1, RAND_MAX);
...
Socket::Socket(seastar::connected_socket&& _socket, side_t _side, uint16_t e_port, construct_tag)
...
seastar::future<bufferlist> Socket::read(size_t bytes)
{
    ...
    return try_trap_pre(next_trap_read)
        .then([bytes, this] {
#endif
            if (bytes == 0) {
            ...
            return in.consume(bufferlist_consumer{r.buffer, r.remaining}).then([this] {
                ...
                return inject_delay().then(
                    [this] { return seastar::make_ready_future<bufferlist>(std::move(r.buffer)); });
            });
#ifdef UNIT_TESTS_BUILT
        })
        .then([this](auto buf) {
            return try_trap_post(next_trap_read).then([buf = std::move(buf)]() mutable { return std::move(buf); });
        });
#endif
}

seastar::future<bufferptr> Socket::read_exactly(size_t bytes)
{
    assert(seastar::this_shard_id() == sid);
#ifdef UNIT_TESTS_BUILT
    return try_trap_pre(next_trap_read)
        .then([bytes, this] {
            ...
            return in.read_exactly(bytes).then([bytes](auto buf) {
                ...
                inject_failure();
                return inject_delay().then(
                    [ptr = std::move(ptr)]() mutable { return seastar::make_ready_future<bufferptr>(std::move(ptr)); });
            });
#ifdef UNIT_TESTS_BUILT
        })
        .then([this](auto ptr) {
            return try_trap_post(next_trap_read).then([ptr = std::move(ptr)]() mutable { return std::move(ptr); });
        });
#endif
}

seastar::future<> Socket::write(bufferlist buf)
{
    assert(seastar::this_shard_id() == sid);
#ifdef UNIT_TESTS_BUILT
...
#endif
            inject_failure();
            return inject_delay().then([buf = std::move(buf), this]() mutable {
                packet p(std::move(buf));
                return out.write(std::move(p));
            });
#ifdef UNIT_TESTS_BUILT
        })
        .then([this] { return try_trap_post(next_trap_write); });
#endif
}

seastar::future<> Socket::flush()
{
    assert(seastar::this_shard_id() == sid);
    inject_failure();
    return inject_delay().then([this] { return out.flush(); });
}

seastar::future<> Socket::write_flush(bufferlist buf)
{
    assert(seastar::this_shard_id() == sid);
#ifdef UNIT_TESTS_BUILT
    return try_trap_pre(next_trap_write)
        .then([buf = std::move(buf), this]() mutable {
            ...
            return inject_delay().then([buf = std::move(buf), this]() mutable {
                packet p(std::move(buf));
                return out.write(std::move(p)).then([this] { return out.flush(); });
            });
#ifdef UNIT_TESTS_BUILT
        })
        .then([this] { return try_trap_post(next_trap_write); });
...
seastar::future<> Socket::close()
{
    ...
    closed = true;
#endif
    return seastar::when_all_succeed(inject_delay(), in.close(), close_and_handle_errors(out))
        .then_unpack([] { return seastar::make_ready_future<>(); })
...
seastar::future<SocketRef> Socket::connect(const entity_addr_t& peer_addr)
{
    inject_failure();
    return inject_delay()
        .then([peer_addr] { return seastar::connect(peer_addr.in4_addr()); })
        .then([peer_addr](seastar::connected_socket socket) {
            auto ret = std::make_unique<Socket>(std::move(socket), side_t::connector, 0, construct_tag{});
            logger().debug("Socket::connect(): connected to {}, socket {}", peer_addr, fmt::ptr(ret));
            return ret;
        });
}
...
seastar::future<> Socket::try_trap_pre(bp_action_t& trap)
{
    ...
    switch (action) {
    case bp_action_t::CONTINUE: break;
    case bp_action_t::FAULT:
        logger().info("[Test] got FAULT");
        throw std::system_error(make_error_code(error::negotiation_failure));
    case bp_action_t::BLOCK: logger().info("[Test] got BLOCK"); return blocker->block();
    case bp_action_t::STALL: trap = action; break;
    default: ceph_abort("unexpected action from trap");
    }
...
ShardedServerSocket::ShardedServerSocket(seastar::shard_id sid, bool dispatch_only_on_primary_sid, construct_tag)
    : primary_sid{sid}
...
listen_ertr::future<> ShardedServerSocket::listen(entity_addr_t addr)
{
    ...
    return this->container()
        ...
        .then([] { return listen_ertr::now(); })
        .handle_exception_type([addr](const std::system_error& e) -> listen_ertr::future<> {
            if (e.code() == std::errc::address_in_use) {
                ...
                logger().debug("ShardedServerSocket({})::listen(): address not available", addr);
...
seastar::future<> ShardedServerSocket::accept(accept_func_t&& _fn_accept)
{
    ceph_assert_always(seastar::this_shard_id() == primary_sid);
    ...
    return this->container().invoke_on_all([_fn_accept](auto& ss) {
        ...
        std::ignore = seastar::with_gate(ss.shutdown_gate, [&ss] {
            return seastar::keep_doing([&ss] {
                       return ss.listener->accept().then([&ss](seastar::accept_result accept_result) {
#ifndef NDEBUG
                           if (ss.dispatch_only_on_primary_sid) {
                           ...
                           SocketRef _socket = std::make_unique<Socket>(std::move(socket),
                           ...
                           std::ignore = seastar::with_gate(
                               ss.shutdown_gate, [socket = std::move(_socket), peer_addr, &ss]() mutable {
                ...
                .handle_exception_type([&ss](const std::system_error& e) {
                    if (e.code() == std::errc::connection_aborted || e.code() == std::errc::invalid_argument) {
                        logger().debug("ShardedServerSocket({})::accept(): stopped ({})", ss.listen_addr, e.what());
                    }
                    else {
                        throw;
                    }
...
seastar::future<ShardedServerSocket*> ShardedServerSocket::create(bool dispatch_only_on_this_shard)
{
    ...
    return seastar::smp::submit_to(0,
                                   [primary_sid, dispatch_only_on_this_shard] {
                                       auto service = std::make_unique<sharded_service_t>();
                                       return service->start(primary_sid, dispatch_only_on_this_shard, construct_tag{})
        ...
        .then([](auto p_shard) { return &p_shard->local(); });
}
