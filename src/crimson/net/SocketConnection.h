...
public:
    SocketConnection(SocketMessenger& messenger, ChainedDispatchers& dispatchers, bool is_msgr2);
    ...
    /// only call when SocketConnection first construct
    void start_accept(SocketRef&& socket, const entity_addr_t& peer_addr);

    seastar::future<> close_clean(bool dispatch_reset);

    bool is_server_side() const { return policy.server; }

    bool is_lossy() const { return policy.lossy; }
