...
#ifdef UNIT_TESTS_BUILT
void intercept(Breakpoint bp, bp_type_t type, SocketConnection& conn, SocketRef& socket)
{
    if (conn.interceptor) {
...
#define INTERCEPT_CUSTOM(bp, type) intercept({bp}, type, conn, socket)

#define INTERCEPT_FRAME(tag, type) intercept({static_cast<Tag>(tag), type}, type, conn, socket)
...
ProtocolV2::ProtocolV2(ChainedDispatchers& dispatchers, SocketConnection& conn, SocketMessenger& messenger)
...
{
    return state == state_t::READY || state == state_t::ESTABLISHING || state == state_t::REPLACING;
}
...
void ProtocolV2::start_accept(SocketRef&& sock, const entity_addr_t& _peer_addr)
{
    ceph_assert(state == state_t::NONE);
    ceph_assert(!socket);
...
seastar::future<Socket::tmp_buf> ProtocolV2::read_exactly(size_t bytes)
{
    if (unlikely(record_io)) {
        return socket->read_exactly(bytes).then([this](auto bl) {
...
{
    if (unlikely(record_io)) {
        return socket->read(bytes).then([this](auto buf) {
            rxbuf.append(buf);
            return buf;
        });
    }
...
{
    rx_preamble.clear();
    return read_exactly(rx_frame_asm.get_preamble_onwire_len()).then([this](auto bl) {
...
seastar::future<> ProtocolV2::read_frame_payload()
{
    ...
    return seastar::do_until([this] { return rx_frame_asm.get_num_segments() == rx_segments_data.size(); },
                             [this] {
                                 ...
                                 return read_exactly(onwire_len).then([this](auto tmp_bl) {
                                     logger().trace(
                                         "{} RECV({}) frame segment[{}]", conn, tmp_bl.size(), rx_segments_data.size());
                                     bufferlist segment;
                                     segment.append(buffer::create(std::move(tmp_bl)));
                                     rx_segments_data.emplace_back(std::move(segment));
                                 });
                             })
        .then([this] { return read_exactly(rx_frame_asm.get_epilogue_onwire_len()); })
        .then([this](auto bl) {
...
template<class F> seastar::future<> ProtocolV2::write_frame(F& frame, bool flush)
{
    ...
    logger().trace("{} SEND({}) frame: tag={}, num_segments={}, crc={}",
                   conn,
                   bl.length(),
                   (int)main_preamble->tag,
                   (int)main_preamble->num_segments,
                   main_preamble->crc);
...
void ProtocolV2::trigger_state(state_t _state, write_state_t _write_state, bool reentrant)
{
    ...
    logger().debug("{} TRIGGER {}, was {}", conn, get_state_name(_state), get_state_name(state));
    state = _state;
...
void ProtocolV2::fault(bool backoff, const char* func_name, std::exception_ptr eptr)
{
    if (conn.policy.lossy) {
    ...
    else if (conn.policy.server || (conn.policy.standby && (!is_queued() && conn.sent.empty()))) {
        ...
        logger().info("{} {}: fault at {}, going to WAIT -- {}", conn, func_name, get_state_name(state), eptr);
        execute_wait(false);
    }
    else {
        logger().info("{} {}: fault at {}, going to CONNECTING -- {}", conn, func_name, get_state_name(state), eptr);
        execute_connecting();
    }
}
...
void ProtocolV2::reset_session(bool full)
{
    ...
    if (full) {
        ...
        peer_global_seq = 0;
        reset_write();
        dispatchers.ms_handle_remote_reset(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()));
    }
}
...
seastar::future<std::tuple<entity_type_t, entity_addr_t>> ProtocolV2::banner_exchange(bool is_connect)
{
    ...
    return write_flush(std::move(bl))
        ...
        .then([this](auto bl) {
            ...
            logger().debug(
                "{} RECV({}) banner: \"{}\"", conn, bl.size(), std::string((const char*)bl.get(), banner_prefix_len));
    ...
    return read_main_preamble().then([this](Tag tag) {
        ...
        case Tag::AUTH_BAD_METHOD:
            return read_frame_payload().then([this] {
                ...
                int r = messenger.get_auth_client()->handle_auth_bad_method(conn.shared_from_this(),
                                                                            auth_meta,
                                                                            bad_method.method(),
                                                                            bad_method.result(),
                                                                            bad_method.allowed_methods(),
        ...
        case Tag::AUTH_DONE:
            return read_frame_payload().then([this] {
                ...
                ceph_assert(messenger.get_auth_client());
                int r = messenger.get_auth_client()->handle_auth_done(conn.shared_from_this(),
                                                                      auth_meta,
                                                                      auth_done.global_id(),
                                                                      auth_done.con_mode(),
...
seastar::future<> ProtocolV2::client_auth(std::vector<uint32_t>& allowed_methods)
{
    ...
    try {
        ...
        logger().debug("{} WRITE AuthRequestFrame: method={},"
                       ...
                       preferred_modes,
                       bl.length());
        return write_frame(frame).then([this] { return handle_auth_reply(); });
    }
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::client_connect()
{
    ...
    logger().debug("{} WRITE ClientIdentFrame: addrs={}, target={}, gid={},"
                   ...
                   flags,
                   client_cookie);
    return write_frame(client_ident).then([this] { return read_main_preamble(); }).then([this](Tag tag) {
        switch (tag) {
        ...
        case Tag::SERVER_IDENT:
            return read_frame_payload().then([this] {
                ...
                if (!server_ident.addrs().contains(conn.target_addr)) {
                    logger().warn(
                        "{} peer identifies as {}, does not include {}", conn, server_ident.addrs(), conn.target_addr);
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::client_reconnect()
{
    ...
    auto reconnect = ReconnectFrame::Encode(
        messenger.get_myaddrs(), client_cookie, server_cookie, global_seq, connect_seq, conn.in_seq);
    logger().debug("{} WRITE ReconnectFrame: addrs={}, client_cookie={},"
                   ...
                   connect_seq,
                   conn.in_seq);
    return write_frame(reconnect).then([this] { return read_main_preamble(); }).then([this](Tag tag) {
        switch (tag) {
        ...
        case Tag::SESSION_RECONNECT_OK:
            return read_frame_payload().then([this] {
                ...
                logger().debug("{} GOT ReconnectOkFrame: msg_seq={}", conn, reconnect_ok.msg_seq());
        ...
        return messenger.get_global_seq()
            ...
            .then([this] {
                ...
                if (socket) {
                    gate.dispatch_in_background(
                        "close_sockect_connecting", *this, [sock = std::move(socket)]() mutable {
            ...
            .then([this](SocketRef sock) {
                ...
                if (unlikely(state != state_t::CONNECTING)) {
                    ...
                    return sock->close().then([sock = std::move(sock)] { abort_protocol(); });
                }
                socket = std::move(sock);
                return seastar::now();
            })
            ...
            .handle_exception([this](std::exception_ptr eptr) {
                if (state != state_t::CONNECTING) {
                    logger().info(
                        "{} execute_connecting(): protocol aborted at {} -- {}", conn, get_state_name(state), eptr);
                    assert(state == state_t::CLOSING || state == state_t::REPLACING);
                    return;
                }

                if (conn.policy.server || (conn.policy.standby && (!is_queued() && conn.sent.empty()))) {
                    ...
                    logger().info(
                        "{} execute_connecting(): fault at {}, going to WAIT -- {}", conn, get_state_name(state), eptr);
                    execute_wait(false);
                }
            });
    });
...
seastar::future<> ProtocolV2::_auth_bad_method(int r)
{
    ...
    logger().warn("{} WRITE AuthBadMethodFrame: method={}, result={}, "
                  ...
                  allowed_methods,
                  allowed_modes);
    return write_frame(bad_method).then([this] { return server_auth(); });
}
...
seastar::future<> ProtocolV2::_handle_auth_request(bufferlist& auth_payload, bool more)
{
    ...
    int r = messenger.get_auth_server()->handle_auth_request(
        conn.shared_from_this(), auth_meta, more, auth_meta->auth_method, auth_payload, &reply);
    switch (r) {
    // successful
    case 1:
    {
    ...
    case 0:
    {
        ...
        return write_frame(more)
            .then([this] { return read_main_preamble(); })
            .then([this](Tag tag) {
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::send_wait()
{
    ...
    return write_frame(wait).then([] { return next_step_t::wait; });
}
        ...
        if (existing_proto->client_cookie != client_cookie) {
            if (existing_conn->peer_wins()) {
                ...
                logger().warn("{} server_connect: connection race detected (cs={}, e_cs={}, ss=0)"
                              ...
                              existing_proto->client_cookie,
                              *existing_conn);
                return existing_conn->keepalive().then([this] { return send_wait(); });
            }
        }
        else {
            logger().warn("{} server_connect: found client session with existing {}"
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::server_connect()
{
    return read_frame_payload().then([this] {
        ...
        if ((paddr.is_msgr2() || paddr.is_any()) && paddr.is_same_host(conn.target_addr)) {
            // good
        }
        else {
            logger().warn("{} peer's address {} is not v2 or not the same host with {}", conn, paddr, conn.target_addr);
            throw std::system_error(make_error_code(crimson::net::error::bad_peer_address));
        }
        ...
        if (feat_missing) {
            ...
            return write_frame(ident_missing_features).then([] { return next_step_t::wait; });
        }
        connection_features = client_ident.supported_features() & conn.policy.features_supported;
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::send_retry(uint64_t connect_seq)
{
    ...
    return write_frame(retry).then([this] { return read_reconnect(); });
}
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::send_reset(bool full)
{
    ...
    return write_frame(reset).then([this] { return read_main_preamble(); }).then([this](Tag tag) {
...
void ProtocolV2::execute_accepting()
{
    ...
    gate.dispatch_in_background("execute_accepting", *this, [this] {
        return seastar::futurize_invoke([this] {
            ...
            .handle_exception([this](std::exception_ptr eptr) {
                logger().info(
                    "{} execute_accepting(): fault at {}, going to CLOSING -- {}", conn, get_state_name(state), eptr);
                close(false);
            });
    });
}
...
seastar::future<> ProtocolV2::finish_auth()
{
    ...
    const auto sig =
        auth_meta->session_key.empty() ? sha256_digest_t() : auth_meta->session_key.hmac_sha256(nullptr, rxbuf);
...
void ProtocolV2::execute_establishing(SocketConnectionRef existing_conn, bool dispatch_reset)
{
    if (unlikely(state != state_t::ACCEPTING)) {
    ...
    dispatchers.ms_handle_accept(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()));

    gated_execute("execute_establishing", [this] {
        return seastar::futurize_invoke([this] { return send_server_ident(); })
            .then([this] {
                if (unlikely(state != state_t::ESTABLISHING)) {
            ...
            .handle_exception([this](std::exception_ptr eptr) {
                if (state != state_t::ESTABLISHING) {
                    logger().info(
                        "{} execute_establishing() protocol aborted at {} -- {}", conn, get_state_name(state), eptr);
                    assert(state == state_t::CLOSING || state == state_t::REPLACING);
                    return;
                }
...
void ProtocolV2::trigger_replacing(bool reconnect, bool do_reset, SocketRef&& new_socket,
                                   AuthConnectionMetaRef&& new_auth_meta, ceph::crypto::onwire::rxtx_t new_rxtx,
                                   uint64_t new_peer_global_seq, uint64_t new_client_cookie,
                                   entity_name_t new_peer_name, uint64_t new_conn_features, bool tx_is_rev1,
                                   bool rx_is_rev1, uint64_t new_connect_seq, uint64_t new_msg_seq)
{
            ...
            return wait_write_exit()
                ...
                .then([this,
                    ...
                    if (unlikely(state != state_t::REPLACING)) {
                        return new_socket->close().then([sock = std::move(new_socket)] { abort_protocol(); });
                    }

                    if (socket) {
                        gate.dispatch_in_background(
                            "close_socket_replacing", *this, [sock = std::move(socket)]() mutable {
                                return sock->close().then([sock = std::move(sock)] {});
                            });
                    }
                ...
                .then([this, reconnect] {
                    if (unlikely(state != state_t::REPLACING)) {
                        logger().debug(
                            "{} triggered {} at the end of trigger_replacing()", conn, get_state_name(state));
                        abort_protocol();
                    }
                ...
                .handle_exception([this](std::exception_ptr eptr) {
                    if (state != state_t::REPLACING) {
                        logger().info(
                            "{} trigger_replacing(): protocol aborted at {} -- {}", conn, get_state_name(state), eptr);
...
ceph::bufferlist ProtocolV2::do_sweep_messages(const std::deque<MessageRef>& msgs, size_t num_msgs,
                                               bool require_keepalive, std::optional<utime_t> _keepalive_ack,
                                               bool require_ack)
{
    ceph::bufferlist bl;
    ...
    std::for_each(msgs.begin(), msgs.begin() + num_msgs, [this, &bl](const MessageRef& msg) {
        ...
        auto message = MessageFrame::Encode(header2, msg->get_payload(), msg->get_middle(), msg->get_data());
        logger().debug("{} --> #{} === {} ({})", conn, msg->get_seq(), *msg, msg->get_type());
...
{
    return read_frame_payload().then([this, throttle_stamp] {
        ...
        auto conn_ref = seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this());
        Message* message = decode_message(
            nullptr, 0, header, footer, msg_frame.front(), msg_frame.middle(), msg_frame.data(), conn_ref);
        if (!message) {
        ...
        if (message->get_seq() <= cur_seq) {
            logger().error("{} got old message {} <= {} {}, discarding", conn, message->get_seq(), cur_seq, *message);
            if (HAVE_FEATURE(conn.features, RECONNECT_SEQ) && local_conf()->ms_die_on_old_message) {
                ceph_assert(0 == "old msgs despite reconnect_seq feature");
            }
            return seastar::now();
        }
        else if (message->get_seq() > cur_seq + 1) {
            logger().error("{} missed message? skipped from seq {} to {}", conn, cur_seq, message->get_seq());
        ...
        logger().debug("{} <== #{} === {} ({})", conn, message->get_seq(), *message, message->get_type());
        notify_ack();
...
void ProtocolV2::execute_ready(bool dispatch_connect)
{
    ...
    if (dispatch_connect) {
        dispatchers.ms_handle_connect(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()));
    }
#ifdef UNIT_TESTS_BUILT
    if (conn.interceptor) {
        ...
        return seastar::keep_doing([this] {
                   return read_main_preamble().then([this](Tag tag) {
                       ...
                       case Tag::KEEPALIVE2:
                           return read_frame_payload().then([this] {
                               ...
                               logger().debug("{} GOT KeepAliveFrame: timestamp={}", conn, keepalive_frame.timestamp());
                       ...
                       case Tag::KEEPALIVE2_ACK:
                           return read_frame_payload().then([this] {
                               ...
                               logger().debug("{} GOT KeepAliveFrameAck: timestamp={}", conn, conn.last_keepalive_ack);
                           });
                       default:
                       {
            ...
            .handle_exception([this](std::exception_ptr eptr) {
                if (state != state_t::READY) {
                    logger().info(
                        "{} execute_ready(): protocol aborted at {} -- {}", conn, get_state_name(state), eptr);
...
void ProtocolV2::notify_write()
{
    if (unlikely(state == state_t::STANDBY && !conn.policy.server)) {
        logger().info("{} notify_write(): at {}, going to CONNECTING", conn, get_state_name(state));
        execute_connecting();
    }
}
...
void ProtocolV2::execute_wait(bool max_backoff)
{
    ...
    gated_execute("execute_wait", [this, max_backoff] {
        ...
        return protocol_timer.backoff(backoff)
            ...
            .handle_exception([this](std::exception_ptr eptr) {
                logger().info("{} execute_wait(): protocol aborted at {} -- {}", conn, get_state_name(state), eptr);
                assert(state == state_t::REPLACING || state == state_t::CLOSING);
            });
    });
}
...
void ProtocolV2::execute_server_wait()
{
    ...
    gated_execute("execute_server_wait", [this] {
        return read_exactly(1)
            ...
            .handle_exception([this](std::exception_ptr eptr) {
                logger().info(
                    "{} execute_server_wait(): fault at {}, going to CLOSING -- {}", conn, get_state_name(state), eptr);
                close(false);
            });
    });
}
