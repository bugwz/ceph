...
const uint64_t CRIMSON_MSGR2_SUPPORTED_FEATURES = (CEPH_MSGR2_FEATURE_REVISION_1 |
...
inline void expect_tag(const Tag& expected, const Tag& actual, crimson::net::SocketConnection& conn, const char* where)
{
    if (actual != expected) {
...
inline void unexpected_tag(const Tag& unexpected, crimson::net::SocketConnection& conn, const char* where)
{
    logger().warn("{} {} received unexpected tag: {}", conn, where, static_cast<uint32_t>(unexpected));
    abort_in_fault();
}

inline uint64_t generate_client_cookie()
{
    return ceph::util::generate_random_number<uint64_t>(1, std::numeric_limits<uint64_t>::max());
}
...
seastar::future<> ProtocolV2::Timer::backoff(double seconds)
{
    ...
    auto dur = std::chrono::duration_cast<seastar::lowres_clock::duration>(std::chrono::duration<double>(seconds));
    return seastar::sleep_abortable(dur, *as).handle_exception_type([this](const seastar::sleep_aborted& e) {
        logger().debug("{} wait aborted", conn);
        abort_protocol();
    });
}

ProtocolV2::ProtocolV2(SocketConnection& conn, IOHandler& io_handler)
    : conn{conn}
...
void ProtocolV2::start_connect(const entity_addr_t& _peer_addr, const entity_name_t& _peer_name)
{
    ...
    logger().info("{} ProtocolV2::start_connect(): peer_addr={}, peer_name={}, cc={}"
                  ...
                  conn.policy.server,
                  conn.policy.standby,
                  conn.policy.resetcheck);
    messenger.register_conn(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()));
    execute_connecting();
}

void ProtocolV2::start_accept(SocketFRef&& new_socket, const entity_addr_t& _peer_addr)
{
    assert(seastar::this_shard_id() == conn.get_messenger_shard_id());
    ceph_assert(state == state_t::NONE);
    ...
    messenger.accept_conn(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()));
    ...
    gate.dispatch_in_background("set_accepted_sid", conn, [this, cc_seq] {
        return io_handler.set_accepted_sid(
            cc_seq, frame_assembler->get_socket_shard_id(), seastar::make_foreign(conn.shared_from_this()));
    });
...
void ProtocolV2::trigger_state_phase1(state_t new_state)
{
    ...
    if (new_state == state) {
        logger().error("{} is not allowed to re-trigger state {}", conn, get_state_name(state));
        ceph_abort();
    }
    if (state == state_t::CLOSING) {
        logger().error("{} CLOSING is not allowed to trigger state {}", conn, get_state_name(new_state));
        ceph_abort();
    }
    logger().debug("{} TRIGGER {}, was {}", conn, get_state_name(new_state), get_state_name(state));
...
void ProtocolV2::trigger_state_phase2(state_t new_state, io_state_t new_io_state)
{
    ...
    if (new_state == state_t::READY) {
        ...
        assert(io_handler.get_shard_id() == frame_assembler->get_socket_shard_id());
    ...
    logger().debug("{} send {} IOHandler::set_io_state(): new_state={}, new_io_state={}, "
                   ...
                   need_notify_out);
    gate.dispatch_in_background("set_io_state", conn, [this, cc_seq, new_io_state, fa = std::move(fa)]() mutable {
        return seastar::smp::submit_to(
            io_handler.get_shard_id(),
            [this, cc_seq, new_io_state, fa = std::move(fa), set_notify_out = need_notify_out]() mutable {
                return io_handler.set_io_state(cc_seq, new_io_state, std::move(fa), set_notify_out);
            });
    });
    ...
    if (need_exit_io) {
        // from READY
        auto cc_seq = crosscore.prepare_submit();
        logger().debug("{} send {} IOHandler::wait_io_exit_dispatching() ...", conn, cc_seq);
        assert(pr_exit_io.has_value());
        assert(new_io_state != io_state_t::open);
        need_exit_io = false;
        gate.dispatch_in_background("exit_io", conn, [this, cc_seq] {
            return seastar::smp::submit_to(io_handler.get_shard_id(),
                                           [this, cc_seq] { return io_handler.wait_io_exit_dispatching(cc_seq); })
                .then([this, cc_seq](auto ret) {
                    logger().debug(
                        "{} finish {} IOHandler::wait_io_exit_dispatching(), {}", conn, cc_seq, ret.io_states);
                    frame_assembler = std::move(ret.frame_assembler);
                    assert(seastar::this_shard_id() == conn.get_messenger_shard_id());
                    ceph_assert_always(seastar::this_shard_id() == frame_assembler->get_shard_id());
...
void ProtocolV2::fault(state_t expected_state, const char* where, std::exception_ptr eptr)
{
    assert(expected_state == state_t::CONNECTING || expected_state == state_t::ESTABLISHING ||
           expected_state == state_t::REPLACING || expected_state == state_t::READY);
    const char* e_what;
    try {
    ...
    if (state != expected_state) {
        ...
        if (expected_state == state_t::REPLACING) {
            ...
            assert(state == state_t::CLOSING || state == state_t::REPLACING || state == state_t::CONNECTING ||
                   state == state_t::STANDBY);
        }
        else {
            assert(state == state_t::CLOSING || state == state_t::REPLACING);
        }
#endif
        return;
    }
    ...
    if (conn.policy.server || (conn.policy.standby && !io_states.is_out_queued_or_sent())) {
    ...
    else if (state == state_t::CONNECTING || state == state_t::REPLACING) {
        logger().info(
            "{} protocol {} {} fault, going to WAIT {} -- {}", conn, get_state_name(state), where, io_states, e_what);
        execute_wait(false);
    }
    else {
        assert(state == state_t::READY || state == state_t::ESTABLISHING);
...
void ProtocolV2::reset_session(bool full)
{
    ...
    auto cc_seq = crosscore.prepare_submit();
    logger().debug("{} send {} IOHandler::reset_session({})", conn, cc_seq, full);
    io_states.reset_session(full);
    gate.dispatch_in_background("reset_session", conn, [this, cc_seq, full] {
        return seastar::smp::submit_to(io_handler.get_shard_id(),
                                       [this, cc_seq, full] { return io_handler.reset_session(cc_seq, full); });
    });
    // user can make changes
}

seastar::future<std::tuple<entity_type_t, entity_addr_t>> ProtocolV2::banner_exchange(bool is_connect)
{
    ...
    return frame_assembler->intercept_frame(custom_bp_t::BANNER_WRITE, true)
        .then([this, bl = std::move(bl)]() mutable { return frame_assembler->write_flush(std::move(bl)); }
#else
    return frame_assembler
...
#ifdef UNIT_TESTS_BUILT
            return frame_assembler->intercept_frame(custom_bp_t::BANNER_READ, false).then([this, banner_len] {
        ...
        .then([this](auto bptr) {
            ...
            logger().debug(
                "{} RECV({}) banner: \"{}\"", conn, bptr.length(), std::string(bptr.c_str(), banner_prefix_len));
...
#ifdef UNIT_TESTS_BUILT
            return frame_assembler->intercept_frame(custom_bp_t::BANNER_PAYLOAD_READ, false).then([this, payload_len] {
        ...
        .then([this, is_connect](bufferlist bl) {
            ...
            logger().debug("{} WRITE HelloFrame: my_type={}, peer_addr={}",
                           conn,
                           ceph_entity_type_name(messenger.get_mytype()),
                           conn.target_addr);
        ...
        .then([this](auto payload) {
            ...
            logger().debug("{} GOT HelloFrame: my_type={} peer_addr={}",
                           conn,
                           ceph_entity_type_name(hello.entity_type()),
                           hello.peer_addr());
...
{
    return frame_assembler->read_main_preamble().then([this](auto ret) {
        switch (ret.tag) {
        case Tag::AUTH_BAD_METHOD:
            return frame_assembler->read_frame_payload().then([this](auto payload) {
                ...
                logger().warn("{} GOT AuthBadMethodFrame: method={} result={}, "
                              ...
                              conn,
                              bad_method.method(),
                              cpp_strerror(bad_method.result()),
                              bad_method.allowed_methods(),
                              bad_method.allowed_modes());
                ceph_assert(messenger.get_auth_client());
                int r = messenger.get_auth_client()->handle_auth_bad_method(conn,
                                                                            *auth_meta,
                                                                            bad_method.method(),
                                                                            bad_method.result(),
                                                                            bad_method.allowed_methods(),
                                                                            bad_method.allowed_modes());
                if (r < 0) {
                    logger().warn("{} auth_client handle_auth_bad_method returned {}", conn, r);
                    abort_in_fault();
                }
        ...
        case Tag::AUTH_REPLY_MORE:
            return frame_assembler->read_frame_payload()
                .then([this](auto payload) {
                    ...
                    logger().debug(
                        "{} GOT AuthReplyMoreFrame: payload_len={}", conn, auth_more.auth_payload().length());
                    ceph_assert(messenger.get_auth_client());
                    // let execute_connecting() take care of the thrown exception
                    auto reply =
                        messenger.get_auth_client()->handle_auth_reply_more(conn, *auth_meta, auth_more.auth_payload());
                    auto more_reply = AuthRequestMoreFrame::Encode(reply);
                    logger().debug("{} WRITE AuthRequestMoreFrame: payload_len={}", conn, reply.length());
                    return frame_assembler->write_flush_frame(more_reply);
                })
                .then([this] { return handle_auth_reply(); });
        case Tag::AUTH_DONE:
            return frame_assembler->read_frame_payload().then([this](auto payload) {
                ...
                int r = messenger.get_auth_client()->handle_auth_done(
                    conn, *auth_meta, auth_done.global_id(), auth_done.con_mode(), auth_done.auth_payload());
                if (r < 0) {
...
seastar::future<> ProtocolV2::client_auth(std::vector<uint32_t>& allowed_methods)
{
    ...
    try {
        auto [auth_method, preferred_modes, bl] = messenger.get_auth_client()->get_auth_request(conn, *auth_meta);
        ...
        logger().debug("{} WRITE AuthRequestFrame: method={},"
                       ...
                       preferred_modes,
                       bl.length());
        return frame_assembler->write_flush_frame(frame).then([this] { return handle_auth_reply(); });
    }
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::process_wait()
{
    return frame_assembler->read_frame_payload().then([this](auto payload) {
        // handle_wait() logic
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::client_connect()
{
    ...
    return frame_assembler->write_flush_frame(client_ident)
        .then([this] { return frame_assembler->read_main_preamble(); })
        .then([this](auto ret) {
            switch (ret.tag) {
            case Tag::IDENT_MISSING_FEATURES:
                return frame_assembler->read_frame_payload().then([this](auto payload) {
                    ...
                    logger().warn("{} GOT IdentMissingFeaturesFrame: features={}"
                                  ...
                                  conn,
                                  ident_missing.features());
                    abort_in_fault();
                    return next_step_t::none;
                });
            case Tag::WAIT: return process_wait();
            case Tag::SERVER_IDENT:
                return frame_assembler->read_frame_payload().then([this](auto payload) {
                    if (unlikely(state != state_t::CONNECTING)) {
                        logger().debug("{} triggered {} at receiving SERVER_IDENT", conn, get_state_name(state));
                        abort_protocol();
                    }

                    // handle_server_ident() logic
                    auto cc_seq = crosscore.prepare_submit();
                    logger().debug("{} send {} IOHandler::requeue_out_sent()", conn, cc_seq);
                    io_states.requeue_out_sent();
                    gate.dispatch_in_background("requeue_out_sent", conn, [this, cc_seq] {
                        return seastar::smp::submit_to(io_handler.get_shard_id(),
                                                       [this, cc_seq] { return io_handler.requeue_out_sent(cc_seq); });
                    });
                    ...
                    if (!server_ident.addrs().contains(conn.target_addr)) {
                        logger().warn("{} peer identifies as {}, does not include {}",
                                      ...
                                      server_ident.addrs(),
                                      conn.target_addr);
                        throw std::system_error(make_error_code(crimson::net::error::bad_peer_address));
                    }
                    ...
                    if (server_ident.addrs().front() != conn.peer_addr) {
                        logger().warn(
                            "{} peer advertises as {}, does not match {}", conn, server_ident.addrs(), conn.peer_addr);
                        throw std::system_error(make_error_code(crimson::net::error::bad_peer_address));
                    }
                    if (conn.get_peer_id() != entity_name_t::NEW && conn.get_peer_id() != server_ident.gid()) {
                    ...
                    conn.set_features(server_ident.supported_features() & conn.policy.features_supported);
                    ...
                    if (lossy && (connect_seq != 0 || server_cookie != 0)) {
                        logger().warn("{} UPDATE cs=0({}) sc=0({}) for lossy policy", conn, connect_seq, server_cookie);
                        connect_seq = 0;
                        server_cookie = 0;
                    }
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::client_reconnect()
{
    // send_reconnect() logic
    auto reconnect = ReconnectFrame::Encode(
        messenger.get_myaddrs(), client_cookie, server_cookie, global_seq, connect_seq, io_states.in_seq);
    logger().debug("{} WRITE ReconnectFrame: addrs={}, client_cookie={},"
                   ...
                   connect_seq,
                   io_states.in_seq);
    return frame_assembler->write_flush_frame(reconnect)
        .then([this] { return frame_assembler->read_main_preamble(); })
        .then([this](auto ret) {
            switch (ret.tag) {
            case Tag::SESSION_RETRY_GLOBAL:
                return frame_assembler->read_frame_payload().then([this](auto payload) {
                    ...
                    logger().warn("{} GOT RetryGlobalFrame: gs={}", conn, retry.global_seq());
            ...
            case Tag::SESSION_RETRY:
                return frame_assembler->read_frame_payload().then([this](auto payload) {
                    ...
                    logger().warn("{} GOT RetryFrame: cs={}", conn, retry.connect_seq());
            ...
            case Tag::SESSION_RESET:
                return frame_assembler->read_frame_payload().then([this](auto payload) {
                    if (unlikely(state != state_t::CONNECTING)) {
                        logger().debug("{} triggered {} before reset_session()", conn, get_state_name(state));
                        abort_protocol();
                    }
            ...
            case Tag::SESSION_RECONNECT_OK:
                return frame_assembler->read_frame_payload().then([this](auto payload) {
                    if (unlikely(state != state_t::CONNECTING)) {
                        logger().debug("{} triggered {} at receiving RECONNECT_OK", conn, get_state_name(state));
                        abort_protocol();
                    }
                    ...
                    logger().debug("{} GOT ReconnectOkFrame: msg_seq={}, "
                                   ...
                                   conn,
                                   reconnect_ok.msg_seq(),
                                   cc_seq);

                    io_states.requeue_out_sent_up_to();
                    auto msg_seq = reconnect_ok.msg_seq();
                    gate.dispatch_in_background("requeue_out_reconnecting", conn, [this, cc_seq, msg_seq] {
                        return seastar::smp::submit_to(io_handler.get_shard_id(), [this, cc_seq, msg_seq] {
...
void ProtocolV2::execute_connecting()
{
    ...
    gated_execute("execute_connecting", conn, [this] {
        ...
        if (!conn.policy.lossy && server_cookie != 0) {
            ++connect_seq;
            logger().debug("{} UPDATE: gs={}, cs={} for reconnect", conn, global_seq, connect_seq);
        }
        ...
        return wait_exit_io()
            .then([this] {
                ...
                return conn.interceptor->intercept(conn, {Breakpoint{custom_bp_t::SOCKET_CONNECTING}})
                    .then([this](bp_action_t action) {
                        switch (action) {
                        case bp_action_t::CONTINUE: return seastar::now();
                        case bp_action_t::FAULT: logger().info("[Test] got FAULT"); abort_in_fault();
                        case bp_action_t::BLOCK:
                            logger().info("[Test] got BLOCK");
                            return conn.interceptor->blocker.block();
                        default: ceph_abort("unexpected action from trap"); return seastar::now();
                        }
                    });
                ;
            })
            .then([this] {
                ...
                if (unlikely(state != state_t::CONNECTING)) {
                    logger().debug("{} triggered {} before Socket::connect()", conn, get_state_name(state));
                    abort_protocol();
                }
            ...
            .then([this](SocketRef _new_socket) {
                ...
                if (unlikely(state != state_t::CONNECTING)) {
                    logger().debug("{} triggered {} during Socket::connect()", conn, get_state_name(state));
                    return _new_socket->close().then([sock = std::move(_new_socket)] { abort_protocol(); });
                }
                ...
                if (!has_socket) {
                    ...
                    gate.dispatch_in_background(
                        "replace_socket_connecting", conn, [this, new_socket = std::move(new_socket)]() mutable {
            ...
            .then([this](auto&& ret) {
                ...
                if (unlikely(state != state_t::CONNECTING)) {
                    logger().debug("{} triggered {} during banner_exchange(), abort", conn, get_state_name(state));
                    abort_protocol();
                }
                ...
                if (unlikely(_my_addr_from_peer.is_legacy())) {
                    logger().warn("{} peer sent a legacy address for me: {}", conn, _my_addr_from_peer);
                    throw std::system_error(make_error_code(crimson::net::error::bad_peer_address));
                }
            ...
            .then([this](next_step_t next) {
                if (unlikely(state != state_t::CONNECTING)) {
                    logger().debug("{} triggered {} at the end of execute_connecting()", conn, get_state_name(state));
                    abort_protocol();
                }
                switch (next) {
                ...
                {
                    if (unlikely(state != state_t::CONNECTING)) {
                        logger().debug("{} triggered {} before dispatch_connect(), abort", conn, get_state_name(state));
                        abort_protocol();
                    }
                    ...
                    ConnectionFRef conn_fref = seastar::make_foreign(conn.shared_from_this());
                    ceph_assert_always(!pr_switch_io_shard.has_value());
                    pr_switch_io_shard = seastar::shared_promise<>();
                    return seastar::smp::submit_to(
                               io_handler.get_shard_id(),
                               [this, cc_seq, new_io_shard, conn_fref = std::move(conn_fref)]() mutable {
                                   return io_handler.dispatch_connect(cc_seq, new_io_shard, std::move(conn_fref));
                               })
                        .then([this, new_io_shard] {
                            ...
                            if (unlikely(state != state_t::CONNECTING)) {
                                logger().debug(
                                    "{} triggered {} after dispatch_connect(), abort", conn, get_state_name(state));
                                abort_protocol();
                            }
                            execute_ready();
                        });
            ...
            .handle_exception(
                [this](std::exception_ptr eptr) { fault(state_t::CONNECTING, "execute_connecting", eptr); });
    });
}
...
seastar::future<> ProtocolV2::_auth_bad_method(int r)
{
    ...
    logger().warn("{} WRITE AuthBadMethodFrame: method={}, result={}, "
                  ...
                  allowed_methods,
                  allowed_modes);
    return frame_assembler->write_flush_frame(bad_method).then([this] { return server_auth(); });
}
...
seastar::future<> ProtocolV2::_handle_auth_request(bufferlist& auth_payload, bool more)
{
    ...
    int r = messenger.get_auth_server()->handle_auth_request(
        conn, *auth_meta, more, auth_meta->auth_method, auth_payload, &conn.peer_global_id, &reply);
    switch (r) {
    // successful
    case 1:
    {
        auto auth_done = AuthDoneFrame::Encode(conn.peer_global_id, auth_meta->con_mode, reply);
    ...
    case 0:
    {
        ...
        logger().debug("{} WRITE AuthReplyMoreFrame: payload_len={}", conn, reply.length());
        return frame_assembler->write_flush_frame(more)
            ...
            .then([this](auto payload) {
                ...
                logger().debug("{} GOT AuthRequestMoreFrame: payload_len={}", conn, auth_more.auth_payload().length());
    ...
    return frame_assembler->read_main_preamble()
        ...
        .then([this](auto payload) {
            ...
            auth_meta->con_mode = messenger.get_auth_server()->pick_con_mode(
                conn.get_peer_type(), auth_meta->auth_method, request.preferred_modes());
...
bool ProtocolV2::validate_peer_name(const entity_name_t& peer_name) const
{
    ...
    if (my_peer_name.num() != entity_name_t::NEW && peer_name.num() != entity_name_t::NEW &&
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::send_wait()
{
    auto wait = WaitFrame::Encode();
    logger().debug("{} WRITE WaitFrame", conn);
    return frame_assembler->write_flush_frame(wait).then([] { return next_step_t::wait; });
}

seastar::future<ProtocolV2::next_step_t> ProtocolV2::reuse_connection(ProtocolV2* existing_proto, bool do_reset,
                                                                      bool reconnect, uint64_t conn_seq,
                                                                      uint64_t msg_seq)
{
    if (unlikely(state != state_t::ACCEPTING)) {
        logger().debug("{} triggered {} before trigger_replacing()", conn, get_state_name(state));
        abort_protocol();
    }
...
#ifdef UNIT_TESTS_BUILT
    if (conn.interceptor) {
        conn.interceptor->register_conn_replaced(conn.get_local_shared_foreign_from_this());
    }
#endif
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::handle_existing_connection(SocketConnectionRef existing_conn)
{
    // handle_existing_connection() logic
    ProtocolV2* existing_proto = dynamic_cast<ProtocolV2*>(existing_conn->protocol.get());
    ...
    if (existing_conn->policy.lossy) {
        ...
        logger().warn("{} server_connect:"
                      ...
                      *existing_conn);
        if (unlikely(state != state_t::ACCEPTING)) {
            logger().debug("{} triggered {} before execute_establishing()", conn, get_state_name(state));
            abort_protocol();
        }
        ...
        if (existing_proto->client_cookie != client_cookie) {
            if (existing_conn->peer_wins()) {
                ...
                logger().warn("{} server_connect: connection race detected (cs={}, e_cs={}, ss=0)"
                              ...
                              existing_proto->client_cookie,
                              *existing_conn);
                return existing_conn->send_keepalive().then([this] { return send_wait(); });
            }
        }
        else {
            logger().warn("{} server_connect: found client session with existing {} {}"
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::server_connect()
{
    return frame_assembler->read_frame_payload().then([this](auto payload) {
        ...
        if (!conn.policy.lossy && !conn.policy.server && conn.target_addr.get_port() <= 0) {
            logger().warn("{} we don't know how to reconnect to peer {}", conn, conn.target_addr);
            throw std::system_error(make_error_code(crimson::net::error::bad_peer_address));
        }

        if (conn.get_peer_id() != entity_name_t::NEW && conn.get_peer_id() != client_ident.gid()) {
        ...
        uint64_t feat_missing =
            (conn.policy.features_required | msgr2_required) & ~(uint64_t)client_ident.supported_features();
        if (feat_missing) {
            auto ident_missing_features = IdentMissingFeaturesFrame::Encode(feat_missing);
            logger().warn("{} WRITE IdentMissingFeaturesFrame: features={} (peer missing)", conn, feat_missing);
            return frame_assembler->write_flush_frame(ident_missing_features).then([] { return next_step_t::wait; });
        }
        conn.set_features(client_ident.supported_features() & conn.policy.features_supported);
        logger().debug("{} UPDATE: features={}", conn, conn.get_features());
        ...
        if (lossy != conn.policy.lossy) {
            logger().warn("{} my lossy policy {} doesn't match client {}, ignore", conn, conn.policy.lossy, lossy);
        }
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::read_reconnect()
{
    return frame_assembler->read_main_preamble().then([this](auto ret) {
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::send_retry(uint64_t connect_seq)
{
    auto retry = RetryFrame::Encode(connect_seq);
    logger().warn("{} WRITE RetryFrame: cs={}", conn, connect_seq);
    return frame_assembler->write_flush_frame(retry).then([this] { return read_reconnect(); });
}

seastar::future<ProtocolV2::next_step_t> ProtocolV2::send_retry_global(uint64_t global_seq)
{
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::send_reset(bool full)
{
...
seastar::future<ProtocolV2::next_step_t> ProtocolV2::server_reconnect()
{
    return frame_assembler->read_frame_payload().then([this](auto payload) {
        ...
        logger().debug("{} GOT ReconnectFrame: addrs={}, client_cookie={},"
                       ...
                       conn,
                       reconnect.addrs(),
                       reconnect.client_cookie(),
                       reconnect.server_cookie(),
        ...
        if (existing_proto->connect_seq > reconnect.connect_seq()) {
            ...
            if (existing_conn->peer_wins()) {
                ...
                logger().warn("{} server_reconnect: reconnect race detected (cs={})"
                              ...
                              get_state_name(existing_proto->state),
                              *existing_conn);
                return reuse_connection(existing_proto, false, true, reconnect.connect_seq(), reconnect.msg_seq());
            }
            else {
                // acceptor (this connection, the peer) loses
                logger().warn("{} server_reconnect: reconnect race detected (cs={})"
    ...
    gate.dispatch_in_background("execute_accepting", conn, [this] {
        return seastar::futurize_invoke([this] {
                   ...
                   if (conn.interceptor) {
                       ...
                       gate.dispatch_in_background("test_intercept_socket_accepted", conn, [this] {
                           return conn.interceptor->intercept(conn, {Breakpoint{custom_bp_t::SOCKET_ACCEPTED}})
                               .then([](bp_action_t action) { ceph_assert(action == bp_action_t::CONTINUE); });
                       });
                   }
#endif
                   auth_meta = seastar::make_lw_shared<AuthConnectionMeta>();
            ...
            .then([this](auto ret) {
                switch (ret.tag) {
                case Tag::CLIENT_IDENT: return server_connect();
                case Tag::SESSION_RECONNECT: return server_reconnect();
                default:
                {
            ...
            .then([this](next_step_t next) {
                switch (next) {
                case next_step_t::ready: assert(state != state_t::ACCEPTING); break;
                case next_step_t::wait:
                    if (unlikely(state != state_t::ACCEPTING)) {
                        logger().debug(
                            "{} triggered {} at the end of execute_accepting()", conn, get_state_name(state));
                        abort_protocol();
                    }
                    logger().info("{} execute_accepting(): going to SERVER_WAIT", conn);
                    execute_server_wait();
                    break;
                default: ceph_abort("impossible next step");
                }
            })
            .handle_exception([this](std::exception_ptr eptr) {
                ...
                logger().info(
                    "{} execute_accepting(): fault at {}, going to CLOSING -- {}", conn, get_state_name(state), e_what);
                do_close(false);
            });
    });
}
...
seastar::future<> ProtocolV2::finish_auth()
{
    ...
    const auto sig =
        auth_meta->session_key.empty() ? sha256_digest_t() : auth_meta->session_key.hmac_sha256(nullptr, records.rxbuf);
    ...
    return frame_assembler->write_flush_frame(sig_frame)
        ...
        .then([this, txbuf = std::move(records.txbuf)](auto payload) {
            ...
            const auto actual_tx_sig =
                auth_meta->session_key.empty() ? sha256_digest_t() : auth_meta->session_key.hmac_sha256(nullptr, txbuf);
...
void ProtocolV2::execute_establishing(SocketConnectionRef existing_conn)
{
    auto accept_me = [this] {
        messenger.register_conn(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()));
        messenger.unaccept_conn(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()));
    };
    ...
    if (existing_conn) {
        ...
        is_replace = true;
        ProtocolV2* existing_proto = dynamic_cast<ProtocolV2*>(existing_conn->protocol.get());
        existing_proto->do_close(true,   // is_dispatch_reset
                                 std::move(accept_me));
    ...
    gated_execute("execute_establishing", conn, [this, is_replace] {
        ...
        crosscore.prepare_submit();
        auto new_io_shard = frame_assembler->get_socket_shard_id();
        logger().debug("{} send {} IOHandler::dispatch_accept({})", conn, cc_seq, new_io_shard);
        ConnectionFRef conn_fref = seastar::make_foreign(conn.shared_from_this());
        ceph_assert_always(!pr_switch_io_shard.has_value());
        pr_switch_io_shard = seastar::shared_promise<>();
        return seastar::smp::submit_to(
                   io_handler.get_shard_id(),
                   [this, cc_seq, new_io_shard, is_replace, conn_fref = std::move(conn_fref)]() mutable {
                       return io_handler.dispatch_accept(cc_seq, new_io_shard, std::move(conn_fref), is_replace);
                   })
            .then([this, new_io_shard] {
                ceph_assert_always(io_handler.get_shard_id() == new_io_shard);
                pr_switch_io_shard->set_value();
            ...
            .then([this] {
                if (unlikely(state != state_t::ESTABLISHING)) {
                    logger().debug("{} triggered {} at the end of execute_establishing()", conn, get_state_name(state));
                    abort_protocol();
                }
            ...
            .handle_exception(
                [this](std::exception_ptr eptr) { fault(state_t::ESTABLISHING, "execute_establishing", eptr); });
    });
}
...
{
    ceph_assert_always(state == state_t::ESTABLISHING || state == state_t::REPLACING);
    ...
    gate.dispatch_in_background("reset_peer_state", conn, [this, cc_seq] {
        return seastar::smp::submit_to(io_handler.get_shard_id(),
                                       [this, cc_seq] { return io_handler.reset_peer_state(cc_seq); });
    });
...
void ProtocolV2::trigger_replacing(bool reconnect, bool do_reset, FrameAssemblerV2::mover_t&& mover,
                                   AuthConnectionMetaRef&& new_auth_meta, uint64_t new_peer_global_seq,
                                   uint64_t new_client_cookie, entity_name_t new_peer_name, uint64_t new_conn_features,
                                   uint64_t new_peer_supported_features, uint64_t new_connect_seq, uint64_t new_msg_seq)
{
    ceph_assert_always(state >= state_t::ESTABLISHING);
            ...
            return wait_switch_io_shard()
                ...
                .then([this, new_io_shard] {
                    ...
                    return seastar::smp::submit_to(
                               io_handler.get_shard_id(),
                               [this, cc_seq, new_io_shard, conn_fref = std::move(conn_fref)]() mutable {
                                   return io_handler.dispatch_accept(cc_seq, new_io_shard, std::move(conn_fref), false);
                               })
                        .then([this, new_io_shard] {
                            ceph_assert_always(io_handler.get_shard_id() == new_io_shard);
                            pr_switch_io_shard->set_value();
                ...
                .then([this,
                    ...
                    if (unlikely(state != state_t::REPLACING)) {
                        logger().debug(
                            "{} triggered {} in the middle of trigger_replacing(), abort", conn, get_state_name(state));
                        ceph_assert_always(state == state_t::CLOSING);
                        return mover.socket->close().then([sock = std::move(mover.socket)] { abort_protocol(); });
                    }

                    auth_meta = std::move(new_auth_meta);
                    peer_global_seq = new_peer_global_seq;
                    gate.dispatch_in_background(
                        "replace_frame_assembler", conn, [this, mover = std::move(mover)]() mutable {
                    ...
                    if (reconnect) {
                        ...
                        auto cc_seq = crosscore.prepare_submit();
                        logger().debug("{} send {} IOHandler::requeue_out_sent_up_to({})", conn, cc_seq, new_msg_seq);
                        io_states.requeue_out_sent_up_to();
                        gate.dispatch_in_background("requeue_out_replacing", conn, [this, cc_seq, new_msg_seq] {
                            return seastar::smp::submit_to(io_handler.get_shard_id(), [this, cc_seq, new_msg_seq] {
                ...
                .then([this, reconnect] {
                    if (unlikely(state != state_t::REPLACING)) {
                        logger().debug(
                            "{} triggered {} at the end of trigger_replacing(), abort", conn, get_state_name(state));
                        ceph_assert_always(state == state_t::CLOSING);
                        abort_protocol();
                    }
                ...
                .handle_exception(
                    [this](std::exception_ptr eptr) { fault(state_t::REPLACING, "trigger_replacing", eptr); });
        });
}

// READY state

seastar::future<> ProtocolV2::notify_out_fault(crosscore_t::seq_t cc_seq, const char* where, std::exception_ptr eptr,
                                               io_handler_state _io_states)
{
    assert(seastar::this_shard_id() == conn.get_messenger_shard_id());
    if (!crosscore.proceed_or_wait(cc_seq)) {
        logger().debug("{} got {} notify_out_fault(), wait at {}", conn, cc_seq, crosscore.get_in_seq());
        return crosscore.wait(cc_seq).then(
            [this, cc_seq, where, eptr, _io_states] { return notify_out_fault(cc_seq, where, eptr, _io_states); });
    }

    io_states = _io_states;
    logger().debug("{} got {} notify_out_fault(): io_states={}", conn, cc_seq, io_states);
...
void ProtocolV2::execute_ready()
{
    ...
    if (conn.interceptor) {
        ...
        conn.interceptor->register_conn_ready(conn.get_local_shared_foreign_from_this());
    }
#endif
}
...
seastar::future<> ProtocolV2::notify_out(crosscore_t::seq_t cc_seq)
{
    assert(seastar::this_shard_id() == conn.get_messenger_shard_id());
    if (!crosscore.proceed_or_wait(cc_seq)) {
        logger().debug("{} got {} notify_out(), wait at {}", conn, cc_seq, crosscore.get_in_seq());
        return crosscore.wait(cc_seq).then([this, cc_seq] { return notify_out(cc_seq); });
    }

    logger().debug("{} got {} notify_out(): at {}", conn, cc_seq, get_state_name(state));
    io_states.is_out_queued = true;
    if (unlikely(state == state_t::STANDBY && !conn.policy.server)) {
        logger().info("{} notify_out(): at {}, going to CONNECTING", conn, get_state_name(state));
        execute_connecting();
    }
    return seastar::now();
}
...
void ProtocolV2::execute_wait(bool max_backoff)
{
    ...
    gated_execute("execute_wait", conn, [this, max_backoff] {
        ...
        return protocol_timer.backoff(backoff)
            .then([this] {
                if (unlikely(state != state_t::WAIT)) {
                    logger().debug("{} triggered {} at the end of execute_wait()", conn, get_state_name(state));
                    abort_protocol();
                }
            ...
            .handle_exception([this](std::exception_ptr eptr) {
                ...
                logger().info("{} execute_wait(): protocol aborted at {} -- {}", conn, get_state_name(state), e_what);
                assert(state == state_t::REPLACING || state == state_t::CLOSING);
            });
    });
}
...
seastar::future<> ProtocolV2::notify_mark_down(crosscore_t::seq_t cc_seq)
{
    assert(seastar::this_shard_id() == conn.get_messenger_shard_id());
    if (!crosscore.proceed_or_wait(cc_seq)) {
        logger().debug("{} got {} notify_mark_down(), wait at {}", conn, cc_seq, crosscore.get_in_seq());
        return crosscore.wait(cc_seq).then([this, cc_seq] { return notify_mark_down(cc_seq); });
    }

    logger().debug("{} got {} notify_mark_down()", conn, cc_seq);
    do_close(false);
    return seastar::now();
}
...
void ProtocolV2::do_close(bool is_dispatch_reset, std::optional<std::function<void()>> f_accept_new)
{
    if (state == state_t::CLOSING) {
        // already closing
        return;
    ...
    logger().info(
        "{} closing: reset {}, replace {}", conn, is_dispatch_reset ? "yes" : "no", is_replace ? "yes" : "no");
    ...
    messenger.closing_conn(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()));
    if (state == state_t::ACCEPTING || state == state_t::SERVER_WAIT) {
        messenger.unaccept_conn(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()));
    }
    else if (state >= state_t::ESTABLISHING && state < state_t::CLOSING) {
        messenger.unregister_conn(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()));
    }
    else {
        // cannot happen
        ceph_assert(false);
    ...
    gate.dispatch_in_background("close_io", conn, [this, is_dispatch_reset, is_replace] {
        // this is preemptive
        return wait_switch_io_shard().then([this, is_dispatch_reset, is_replace] {
            trigger_state_phase2(state_t::CLOSING, io_state_t::drop);
            auto cc_seq = crosscore.prepare_submit();
            logger().debug(
                "{} send {} IOHandler::close_io(reset={}, replace={})", conn, cc_seq, is_dispatch_reset, is_replace);
            ...
            std::ignore =
                gate.close()
                    ...
                    .then([this] {
                        ...
                        messenger.closed_conn(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()));
                        pr_closed_clean.set_value();
#ifdef UNIT_TESTS_BUILT
                        closed_clean = true;
                        if (conn.interceptor) {
                            conn.interceptor->register_conn_closed(conn.get_local_shared_foreign_from_this());
                        }
#endif
                    ...
                    .handle_exception([conn_ref = conn.shared_from_this(), this](auto eptr) {
                        logger().error("{} closing got unexpected exception {}", conn, eptr);
                        ceph_abort();
                    });

            return seastar::smp::submit_to(io_handler.get_shard_id(), [this, cc_seq, is_dispatch_reset, is_replace] {
