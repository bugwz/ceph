...
SocketConnection::SocketConnection(SocketMessenger& messenger, ChainedDispatchers& dispatchers, bool is_msgr2)
    : messenger(messenger)
{
    if (is_msgr2) {
...
crimson::net::Messenger* SocketConnection::get_messenger() const
{
    return &messenger;
}
...
{
    if (seq <= in_seq) {
        if (HAVE_FEATURE(features, RECONNECT_SEQ) && local_conf()->ms_die_on_old_message) {
...
void SocketConnection::start_accept(SocketRef&& sock, const entity_addr_t& _peer_addr)
{
    protocol->start_accept(std::move(sock), _peer_addr);
}

seastar::future<> SocketConnection::close_clean(bool dispatch_reset)
{
...
void SocketConnection::print(ostream& out) const
{
    ...
    if (!protocol->socket) {
        ...
        out << " >> " << get_peer_name() << " " << peer_addr << "@" << protocol->socket->get_ephemeral_port();
    }
    else {   // protocol->socket->get_side() == Socket::side_t::connector
        out << "@" << protocol->socket->get_ephemeral_port() << " >> " << get_peer_name() << " " << peer_addr;
    }
}
