...
Protocol::Protocol(proto_t type, ChainedDispatchers& dispatchers, SocketConnection& conn)
    : proto_type(type)
...
void Protocol::close(bool dispatch_reset, std::optional<std::function<void()>> f_accept_new)
{
    if (closed) {
        // already closing
        return;
    ...
    logger().info("{} closing: reset {}, replace {}", conn, dispatch_reset ? "yes" : "no", is_replace ? "yes" : "no");
    ...
    if (dispatch_reset) {
        dispatchers.ms_handle_reset(seastar::static_pointer_cast<SocketConnection>(conn.shared_from_this()),
                                    is_replace);
    }

    // asynchronous operations
    assert(!close_ready.valid());
    close_ready = std::move(gate_closed)
                      .then([this] {
                          if (socket) {
                              return socket->close();
                          }
...
void Protocol::requeue_sent()
{
    ...
    logger().debug("{} requeue {} items, revert out_seq to {}", conn, conn.sent.size(), conn.out_seq);
    for (MessageRef& msg : conn.sent) {
        msg->clear_payload();
        msg->set_seq(0);
    }
    conn.out_q.insert(
        conn.out_q.begin(), std::make_move_iterator(conn.sent.begin()), std::make_move_iterator(conn.sent.end()));
    conn.sent.clear();
    write_event();
}
...
void Protocol::requeue_up_to(seq_num_t seq)
{
    ...
    if (conn.sent.empty() && conn.out_q.empty()) {
        logger().debug("{} nothing to requeue, reset out_seq from {} to seq {}", conn, conn.out_seq, seq);
        conn.out_seq = seq;
        return;
    }
    logger().debug(
        "{} discarding sent items by seq {} (sent_len={}, out_seq={})", conn, seq, conn.sent.size(), conn.out_seq);
...
void Protocol::ack_writes(seq_num_t seq)
{
    ...
    while (!conn.sent.empty() && conn.sent.front()->get_seq() <= seq) {
        logger().trace("{} got ack seq {} >= {}, pop {}", conn, seq, conn.sent.front()->get_seq(), conn.sent.front());
        conn.sent.pop_front();
    }
}
...
seastar::future<> Protocol::do_write_dispatch_sweep()
{
    return seastar::repeat([this] {
               ...
               case write_state_t::open:
               {
                   ...
                   if (!conn.policy.lossy) {
                       conn.sent.insert(conn.sent.end(), conn.pending_q.begin(), conn.pending_q.end());
                   }
                   auto acked = ack_left;
                   assert(acked == 0 || conn.in_seq > 0);
                   // sweep all pending writes with the concrete Protocol
                   return socket
                       ->write(do_sweep_messages(conn.pending_q, num_msgs, need_keepalive, keepalive_ack, acked > 0))
               ...
               case write_state_t::delay:
                   ...
                   return state_changed.get_shared_future().then([] { return stop_t::no; });
        ...
        .handle_exception_type([this](const std::system_error& e) {
            if (e.code() != std::errc::broken_pipe && e.code() != std::errc::connection_reset &&
                ...
                logger().error("{} write_event(): unexpected error at {} -- {}", conn, get_state_name(write_state), e);
                ceph_abort();
            }
            socket->shutdown();
            if (write_state == write_state_t::open) {
                logger().info(
                    "{} write_event(): fault at {}, going to delay -- {}", conn, get_state_name(write_state), e);
                write_state = write_state_t::delay;
            }
            else {
                logger().info("{} write_event(): fault at {} -- {}", conn, get_state_name(write_state), e);
            }
...
void Protocol::write_event()
{
    ...
    switch (write_state) {
    case write_state_t::open: [[fallthrough]];
    case write_state_t::delay:
        assert(!gate.is_closed());
        gate.dispatch_in_background("do_write_dispatch_sweep", *this, [this] { return do_write_dispatch_sweep(); });
        return;
    case write_state_t::drop: write_dispatching = false; return;
    default: ceph_assert(false);
    }
}
