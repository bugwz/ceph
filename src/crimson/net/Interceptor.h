...
inline const char* get_bp_name(custom_bp_t bp)
{
    ...
    static const char* const bp_names[] = {
        "BANNER_WRITE", "BANNER_READ", "BANNER_PAYLOAD_READ", "SOCKET_CONNECTING", "SOCKET_ACCEPTED"};
...
public:
    ...
    seastar::future<> wait_blocked()
    {
        ...
        if (p_unblocked) {
            ...
            return seastar::sleep_abortable(std::chrono::seconds(10), *p_blocked)
                .then([] { throw std::runtime_error("Timeout (10s) in socket_blocker::wait_blocked()"); })
    ...
    seastar::future<> block()
    {
        return seastar::smp::submit_to(primary_sid, [this] {
            ...
            return seastar::sleep_abortable(std::chrono::seconds(10), *p_unblocked)
                .then([] { ceph_abort("Timeout (10s) in socket_blocker::block()"); })
                .handle_exception_type([](const seastar::sleep_aborted& e) {
                    // wait done!
                });
        });
...
struct tag_bp_t
{
    ...
    bp_type_t type;
    bool operator==(const tag_bp_t& x) const { return tag == x.tag && type == x.type; }
    bool operator!=(const tag_bp_t& x) const { return !operator==(x); }
    bool operator<(const tag_bp_t& x) const { return std::tie(tag, type) < std::tie(x.tag, x.type); }
};
...
struct Interceptor
{
    ...
    virtual seastar::future<bp_action_t> intercept(Connection&, std::vector<Breakpoint> bp) = 0;
};

}   // namespace crimson::net

template<> struct fmt::formatter<crimson::net::bp_action_t> : fmt::formatter<std::string_view>
{
    template<typename FormatContext> auto format(const crimson::net::bp_action_t& action, FormatContext& ctx) const
    {
        static const char* const action_names[] = {"CONTINUE", "FAULT", "BLOCK", "STALL"};
...
template<> struct fmt::formatter<crimson::net::Breakpoint> : fmt::formatter<std::string_view>
{
    template<typename FormatContext> auto format(const crimson::net::Breakpoint& bp, FormatContext& ctx) const
    {
