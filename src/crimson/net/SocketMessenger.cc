...
SocketMessenger::bind_ertr::future<> SocketMessenger::do_bind(const entity_addrvec_t& addrs)
{
    ...
    return set_myaddrs(addrs)
        .then([this] {
            if (!listener) {
                return FixedCPUServerSocket::create().then([this](auto _listener) { listener = _listener; });
            }
            else {
                return seastar::now();
            }
...
{
    return do_bind(addrs).safe_then([this] { logger().info("{} bind: done", *this); });
}
                                                               ...
                                                               uint32_t max_port)
{
    ...
    if (addr.get_port() != 0) {
        return do_bind(addrs).safe_then([this] { logger().info("{} try_bind: done", *this); });
    }
                   ...
                   return do_bind(entity_addrvec_t{to_bind})
                       .safe_then(
                           [this]() -> seastar::future<std::optional<bool>> {
                               ...
                               return seastar::make_ready_future<std::optional<bool>>(std::make_optional<bool>(true));
                           },
                           bind_ertr::all_same_way([this, max_port, &port](const std::error_code& e) mutable
    ...
    if (listener) {
        ...
        return listener->accept([this](SocketRef socket, entity_addr_t peer_addr) {
            ...
            SocketConnectionRef conn =
                seastar::make_shared<SocketConnection>(*this, dispatchers, get_myaddr().is_msgr2());
...
crimson::net::ConnectionRef SocketMessenger::connect(const entity_addr_t& peer_addr, const entity_name_t& peer_name)
{
    ...
    SocketConnectionRef conn = seastar::make_shared<SocketConnection>(*this, dispatchers, peer_addr.is_msgr2());
...
seastar::future<> SocketMessenger::shutdown()
{
    ...
    return seastar::futurize_invoke([this] {
        ...
        .then([this] {
            return seastar::parallel_for_each(accepting_conns, [](auto conn) { return conn->close_clean(false); });
        })
        .then([this] {
            ceph_assert(accepting_conns.empty());
            return seastar::parallel_for_each(connections, [](auto conn) { return conn.second->close_clean(false); });
        })
        .then([this] {
            return seastar::parallel_for_each(closing_conns, [](auto conn) { return conn->close_clean(false); });
        })
        .then([this] {
...
seastar::future<> SocketMessenger::learned_addr(const entity_addr_t& peer_addr_for_me, const SocketConnection& conn)
{
    ...
    if (get_myaddr().get_type() == entity_addr_t::TYPE_NONE) {
        ...
        addr.set_port(0);
        need_addr = false;
        return set_myaddrs(entity_addrvec_t{addr}).then([this, &conn, peer_addr_for_me] {
            logger().info("{} learned myaddr={} (unbound) from {}", conn, get_myaddr(), peer_addr_for_me);
        });
    }
    else {
        // Already bound
        ...
        if (get_myaddr().is_blank_ip()) {
            ...
            need_addr = false;
            return set_myaddrs(entity_addrvec_t{addr}).then([this, &conn, peer_addr_for_me] {
                logger().info("{} learned myaddr={} (blank IP) from {}", conn, get_myaddr(), peer_addr_for_me);
            });
        }
...
seastar::future<uint32_t> SocketMessenger::get_global_seq(uint32_t old)
{
    if (old > global_seq) {
        global_seq = old;
    }
