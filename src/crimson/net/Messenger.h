...
public:
    ...
    using bind_ertr = crimson::errorator<crimson::ct_error::address_in_use,   // The address (range) is already bound
    ...
    virtual ConnectionRef connect(const entity_addr_t& peer_addr, const entity_name_t& peer_name) = 0;

    ConnectionRef connect(const entity_addr_t& peer_addr, const entity_type_t& peer_type)
    {
    ...
    static MessengerRef create(const entity_name_t& name, const std::string& lname, uint64_t nonce,
