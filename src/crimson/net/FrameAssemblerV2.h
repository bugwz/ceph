...
public:
    ...
    void create_session_stream_handlers(const AuthConnectionMeta& auth_meta, bool crossed);
    ...
    template<bool may_cross_core> void shutdown_socket(crimson::common::Gated* gate);
    ...
    template<bool may_cross_core = true> seastar::future<ceph::bufferptr> read_exactly(std::size_t bytes);

    template<bool may_cross_core = true> seastar::future<ceph::bufferlist> read(std::size_t bytes);

    template<bool may_cross_core = true> seastar::future<> write(ceph::bufferlist);

    template<bool may_cross_core = true> seastar::future<> flush();

    template<bool may_cross_core = true> seastar::future<> write_flush(ceph::bufferlist);
    ...
    template<bool may_cross_core = true> seastar::future<read_main_t> read_main_preamble();

    /// may throw negotiation_failure as fault
    using read_payload_t = ceph::msgr::v2::segment_bls_t;
    // FIXME: read_payload_t cannot be no-throw move constructible
    template<bool may_cross_core = true> seastar::future<read_payload_t*> read_frame_payload();

    template<class F> ceph::bufferlist get_buffer(F& tx_frame)
    {
        assert(seastar::this_shard_id() == sid);
    ...
    template<class F, bool may_cross_core = true> seastar::future<> write_flush_frame(F& tx_frame)
    {
        assert(seastar::this_shard_id() == sid);
        auto bl = get_buffer(tx_frame);
#ifdef UNIT_TESTS_BUILT
        return intercept_frame(F::tag, true).then([this, bl = std::move(bl)]() mutable {
...
#ifdef UNIT_TESTS_BUILT
    seastar::future<> intercept_frames(std::vector<ceph::msgr::v2::Tag> tags, bool is_write)
    {
    ...
    seastar::future<> intercept_frame(ceph::msgr::v2::Tag tag, bool is_write)
    {
        auto type = is_write ? bp_type_t::WRITE : bp_type_t::READ;
        std::vector<Breakpoint> bps;
        bps.emplace_back(Breakpoint{tag, type});
    ...
    seastar::future<> intercept_frame(custom_bp_t bp, bool is_write)
    {
        auto type = is_write ? bp_type_t::WRITE : bp_type_t::READ;
        std::vector<Breakpoint> bps;
...
private:
    ...
    ceph::msgr::v2::FrameAssembler tx_frame_asm{
        &session_stream_handlers, is_rev1, common::local_conf()->ms_crc_data, &session_comp_handlers};
