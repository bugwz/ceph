...
seastar::future<> FrameAssemblerV2::intercept_frames(std::vector<Breakpoint> bps, bp_type_t type)
{
    assert(seastar::this_shard_id() == sid);
    assert(has_socket());
    if (!conn.interceptor) {
    ...
    return conn.interceptor->intercept(conn, bps).then([this, type](bp_action_t action) {
...
void FrameAssemblerV2::create_session_stream_handlers(const AuthConnectionMeta& auth_meta, bool crossed)
{
    assert(seastar::this_shard_id() == sid);
    session_stream_handlers = ceph::crypto::onwire::rxtx_t::create_handler_pair(nullptr, auth_meta, is_rev1, crossed);
}
...
FrameAssemblerV2::mover_t FrameAssemblerV2::to_replace()
{
    assert(seastar::this_shard_id() == sid);
    ...
    return mover_t{move_socket(), std::move(session_stream_handlers), std::move(session_comp_handlers)};
}
...
FrameAssemblerV2::record_bufs_t FrameAssemblerV2::stop_recording()
{
    assert(seastar::this_shard_id() == sid);
...
seastar::shard_id FrameAssemblerV2::get_socket_shard_id() const
{
    assert(seastar::this_shard_id() == sid);
    assert(is_socket_valid());
...
template<bool may_cross_core> void FrameAssemblerV2::shutdown_socket(crimson::common::Gated* gate)
{
    assert(seastar::this_shard_id() == sid);
    assert(is_socket_valid());
    ...
    if constexpr (may_cross_core) {
        ...
        gate->dispatch_in_background("shutdown_socket", conn, [this] {
            return seastar::smp::submit_to(socket->get_shard_id(), [this] { socket->shutdown(); });
        });
    }
    else {
...
seastar::future<> FrameAssemblerV2::replace_shutdown_socket(SocketFRef&& new_socket)
{
    ...
    return seastar::smp::submit_to(old_socket_shard_id, [old_socket = std::move(old_socket)]() mutable {
...
seastar::future<> FrameAssemblerV2::close_shutdown_socket()
{
    ...
    assert(has_socket());
    assert(!is_socket_valid());
    return seastar::smp::submit_to(socket->get_shard_id(), [this] { return socket->close(); });
}

template<bool may_cross_core> seastar::future<ceph::bufferptr> FrameAssemblerV2::read_exactly(std::size_t bytes)
{
    assert(seastar::this_shard_id() == sid);
    assert(has_socket());
    if constexpr (may_cross_core) {
        assert(conn.get_messenger_shard_id() == sid);
        return seastar::smp::submit_to(socket->get_shard_id(), [this, bytes] { return socket->read_exactly(bytes); })
            .then([this](auto bptr) {
                if (record_io) {
                    rxbuf.append(bptr);
                }
...
template<bool may_cross_core> seastar::future<ceph::bufferlist> FrameAssemblerV2::read(std::size_t bytes)
{
    assert(seastar::this_shard_id() == sid);
    assert(has_socket());
    if constexpr (may_cross_core) {
        assert(conn.get_messenger_shard_id() == sid);
        return seastar::smp::submit_to(socket->get_shard_id(), [this, bytes] { return socket->read(bytes); })
            .then([this](auto buf) {
                if (record_io) {
                    rxbuf.append(buf);
                }
...
template<bool may_cross_core> seastar::future<> FrameAssemblerV2::write(ceph::bufferlist buf)
{
    assert(seastar::this_shard_id() == sid);
    assert(has_socket());
    if constexpr (may_cross_core) {
        ...
        return seastar::smp::submit_to(
            socket->get_shard_id(), [this, buf = std::move(buf)]() mutable { return socket->write(std::move(buf)); });
    }
    else {
        assert(socket->get_shard_id() == sid);
...
template<bool may_cross_core> seastar::future<> FrameAssemblerV2::flush()
{
    assert(seastar::this_shard_id() == sid);
    assert(has_socket());
    if constexpr (may_cross_core) {
        assert(conn.get_messenger_shard_id() == sid);
        return seastar::smp::submit_to(socket->get_shard_id(), [this] { return socket->flush(); });
    }
    else {
        assert(socket->get_shard_id() == sid);
        return socket->flush();
...
template<bool may_cross_core> seastar::future<> FrameAssemblerV2::write_flush(ceph::bufferlist buf)
{
    assert(seastar::this_shard_id() == sid);
    assert(has_socket());
    if constexpr (may_cross_core) {
        ...
        return seastar::smp::submit_to(socket->get_shard_id(), [this, buf = std::move(buf)]() mutable {
...
template<bool may_cross_core> seastar::future<FrameAssemblerV2::read_main_t> FrameAssemblerV2::read_main_preamble()
{
    assert(seastar::this_shard_id() == sid);
    rx_preamble.clear();
    return read_exactly<may_cross_core>(rx_frame_asm.get_preamble_onwire_len()).then([this](auto bptr) {
...
#ifdef UNIT_TESTS_BUILT
        return intercept_frame(tag, false).then([this, tag] { return read_main_t{tag, &rx_frame_asm}; });
#else
...
template<bool may_cross_core> seastar::future<FrameAssemblerV2::read_payload_t*> FrameAssemblerV2::read_frame_payload()
{
    assert(seastar::this_shard_id() == sid);
    ...
    return seastar::do_until([this] { return rx_frame_asm.get_num_segments() == rx_segments_data.size(); },
                             [this] {
                                 ...
                                 // TODO: create aligned and contiguous buffer from socket
                                 return read_exactly<may_cross_core>(onwire_len).then([this](auto bptr) {
                                     logger().trace(
                                         "{} RECV({}) frame segment[{}]", conn, bptr.length(), rx_segments_data.size());
                                     bufferlist segment;
                                     segment.append(std::move(bptr));
                                     rx_segments_data.emplace_back(std::move(segment));
                                 });
                             })
        .then([this] { return read_exactly<may_cross_core>(rx_frame_asm.get_epilogue_onwire_len()); })
        .then([this](auto bptr) {
