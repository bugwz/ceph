...
public:
    ...
    seq_t get_in_seq() const { return in_seq; }
...
struct io_handler_state
{
    ...
    bool is_out_queued_or_sent() const { return is_out_queued || has_out_sent; }
...
public:
    ...
    virtual seastar::future<> notify_out(crosscore_t::seq_t cc_seq) = 0;

    virtual seastar::future<> notify_out_fault(crosscore_t::seq_t cc_seq, const char* where, std::exception_ptr,
                                               io_handler_state) = 0;

    virtual seastar::future<> notify_mark_down(crosscore_t::seq_t cc_seq) = 0;
...
{
public:
    IOHandler(ChainedDispatchers&, SocketConnection&);
     ...
     */
public:
    seastar::shard_id get_shard_id() const final { return shard_states->get_shard_id(); }
...
public:
    ...
    seastar::future<> set_accepted_sid(crosscore_t::seq_t cc_seq, seastar::shard_id sid, ConnectionFRef conn_fref);

    /*
     * may be called cross-core
     */

    seastar::future<> close_io(crosscore_t::seq_t cc_seq, bool is_dispatch_reset, bool is_replace);
    ...
    seastar::future<> set_io_state(crosscore_t::seq_t cc_seq, io_state_t new_state, FrameAssemblerV2Ref fa,
                                   bool set_notify_out);
    ...
    seastar::future<exit_dispatching_ret> wait_io_exit_dispatching(crosscore_t::seq_t cc_seq);

    seastar::future<> reset_session(crosscore_t::seq_t cc_seq, bool full);

    seastar::future<> reset_peer_state(crosscore_t::seq_t cc_seq);

    seastar::future<> requeue_out_sent_up_to(crosscore_t::seq_t cc_seq, seq_num_t msg_seq);

    seastar::future<> requeue_out_sent(crosscore_t::seq_t cc_seq);

    seastar::future<> dispatch_accept(crosscore_t::seq_t cc_seq, seastar::shard_id new_sid, ConnectionFRef,
                                      bool is_replace);

    seastar::future<> dispatch_connect(crosscore_t::seq_t cc_seq, seastar::shard_id new_sid, ConnectionFRef);
...
private:
    ...
    public:
        ...
        seastar::shard_id get_shard_id() const { return sid; }
        ...
        template<typename Func> void dispatch_in_background(const char* what, SocketConnection& who, Func&& func)
        {
            assert(seastar::this_shard_id() == sid);
        ...
        bool try_enter_out_dispatching()
        {
            ...
            switch (io_state) {
            case io_state_t::open: [[fallthrough]];
            case io_state_t::delay: out_dispatching = true; return true;
            case io_state_t::drop: [[fallthrough]];
        ...
        void notify_out_dispatching_stopped(const char* what, SocketConnection& conn);

        void exit_out_dispatching(const char* what, SocketConnection& conn)
        {
            assert(seastar::this_shard_id() == sid);
        ...
        bool assert_closed_and_exit() const
        {
            assert(seastar::this_shard_id() == sid);
            if (gate.is_closed()) {
                ceph_assert_always(io_state == io_state_t::drop || io_state == io_state_t::switched);
                ceph_assert_always(!out_dispatching);
        ...
        static shard_states_ref_t create(seastar::shard_id sid, io_state_t state)
        {
            return std::make_unique<shard_states_t>(sid, state);
        }

        static shard_states_ref_t create_from_previous(shard_states_t& prv_states, seastar::shard_id new_sid);
    ...
    void do_set_io_state(io_state_t new_state, std::optional<crosscore_t::seq_t> cc_seq = std::nullopt,
                         FrameAssemblerV2Ref fa = nullptr, bool set_notify_out = false);

    io_state_t get_io_state() const { return shard_states->get_io_state(); }
    ...
    seastar::future<> to_new_sid(crosscore_t::seq_t cc_seq, seastar::shard_id new_sid, ConnectionFRef,
    ...
    {
        return (!out_pending_msgs.empty() || ack_left > 0 || need_keepalive || next_keepalive_ack.has_value());
    }

    bool has_out_sent() const { return !out_sent_msgs.empty(); }
    ...
    ceph::bufferlist
#endif
    sweep_out_pending_msgs_to_sent(bool require_keepalive, std::optional<utime_t> maybe_keepalive_ack,
                                   bool require_ack);
    ...
    seastar::future<> read_message(shard_states_t& ctx, utime_t throttle_stamp, std::size_t msg_size);
...
inline std::ostream& operator<<(std::ostream& out, IOHandler::io_stat_printer stat)
{
...
template<> struct fmt::formatter<crimson::net::io_handler_state>
{
    constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

    template<typename FormatContext> auto format(crimson::net::io_handler_state state, FormatContext& ctx)
    {
...
template<> struct fmt::formatter<crimson::net::IOHandler::io_state_t> : fmt::formatter<std::string_view>
{
    template<typename FormatContext> auto format(crimson::net::IOHandler::io_state_t state, FormatContext& ctx)
    {
        using enum crimson::net::IOHandler::io_state_t;
        std::string_view name;
        switch (state) {
        case none: name = "none"; break;
        case delay: name = "delay"; break;
        case open: name = "open"; break;
        case drop: name = "drop"; break;
        case switched: name = "switched"; break;
        }
