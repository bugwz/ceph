...
public:
    ...
    virtual void start_connect(const entity_addr_t& peer_addr, const entity_name_t& peer_name) = 0;

    virtual void start_accept(SocketRef&& socket, const entity_addr_t& peer_addr) = 0;

    virtual void print(std::ostream&) const = 0;

protected:
    Protocol(proto_t type, ChainedDispatchers& dispatchers, SocketConnection& conn);

    virtual void trigger_close() = 0;

    virtual ceph::bufferlist do_sweep_messages(const std::deque<MessageRef>& msgs, size_t num_msgs,
                                               bool require_keepalive, std::optional<utime_t> keepalive_ack,
                                               bool require_ack) = 0;
...
protected:
    ...
    static const char* get_state_name(write_state_t state)
    {
        ...
        static const char* const state_names[] = {"none", "delay", "open", "drop"};
    ...
    {
        if (write_state == write_state_t::open && state != write_state_t::open && write_dispatching) {
    ...
    {
        return (!conn.out_q.empty() || ack_left > 0 || need_keepalive || keepalive_ack.has_value());
    }
