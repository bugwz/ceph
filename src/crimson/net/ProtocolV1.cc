...
{
    return out << "connect{features=" << std::hex << c.features << std::dec << " host_type=" << c.host_type
               << " global_seq=" << c.global_seq << " connect_seq=" << c.connect_seq
               << " protocol_version=" << c.protocol_version << " authorizer_protocol=" << c.authorizer_protocol
               << " authorizer_len=" << c.authorizer_len << " flags=" << std::hex << static_cast<uint16_t>(c.flags)
               << std::dec << '}';
}

std::ostream& operator<<(std::ostream& out, const ceph_msg_connect_reply& r)
{
    return out << "connect_reply{tag=" << static_cast<uint16_t>(r.tag) << " features=" << std::hex << r.features
               << std::dec << " global_seq=" << r.global_seq << " connect_seq=" << r.connect_seq
               << " protocol_version=" << r.protocol_version << " authorizer_len=" << r.authorizer_len
...
template<typename T> seastar::net::packet make_static_packet(const T& value)
{
...
void validate_banner(bufferlist::const_iterator& p)
{
    ...
    while (b != end) {
        ...
        if (!std::equal(buf, buf + len, b)) {
            throw std::system_error(make_error_code(crimson::net::error::bad_connect_banner));
        }
        b += len;
    }
}
...
template<typename T> bufferptr create_static(T& obj)
{
...
void discard_up_to(std::deque<MessageRef>* queue, crimson::net::seq_num_t seq)
{
    while (!queue->empty() && queue->front()->get_seq() < seq) {
        queue->pop_front();
    }
}
...
ProtocolV1::ProtocolV1(ChainedDispatchers& dispatchers, SocketConnection& conn, SocketMessenger& messenger)
    : Protocol(proto_t::v1, dispatchers, conn)
    , messenger{messenger}
{}
...
seastar::future<stop_t> ProtocolV1::handle_connect_reply(msgr_tag_t tag)
{
    if (h.auth_payload.length() && !conn.peer_is_mon()) {
        if (tag == CEPH_MSGR_TAG_CHALLENGE_AUTHORIZER) {   // more
            h.auth_more =
                messenger.get_auth_client()->handle_auth_reply_more(conn.shared_from_this(), auth_meta, h.auth_payload);
            return seastar::make_ready_future<stop_t>(stop_t::no);
        }
        else {
            int ret =
                messenger.get_auth_client()->handle_auth_done(conn.shared_from_this(), auth_meta, 0, 0, h.auth_payload);
            if (ret < 0) {
                // fault
    ...
    case CEPH_MSGR_TAG_BADAUTHORIZER:
        ...
        throw std::system_error(make_error_code(error::negotiation_failure));
    case CEPH_MSGR_TAG_RESETSESSION: reset_session(); return seastar::make_ready_future<stop_t>(stop_t::no);
    ...
    case CEPH_MSGR_TAG_READY:
        if (auto missing = (conn.policy.features_required & ~(uint64_t)h.reply.features); missing) {
            logger().error("{} missing required features", __func__);
            throw std::system_error(make_error_code(error::negotiation_failure));
        }
        return seastar::futurize_invoke([this, tag] {
                   if (tag == CEPH_MSGR_TAG_SEQ) {
                       return socket->read_exactly(sizeof(seq_num_t)).then([this](auto buf) {
            ...
            .then([this] {
                ...
                if (auth_meta->authorizer) {
                    session_security.reset(get_auth_session_handler(
                        nullptr, auth_meta->authorizer->protocol, auth_meta->session_key, conn.features));
                }
                else {
...
ceph::bufferlist ProtocolV1::get_auth_payload()
{
    ...
    if (conn.peer_is_mon() && messenger.get_mytype() != CEPH_ENTITY_TYPE_MON) {
        return {};
    }
    else {
...
seastar::future<stop_t> ProtocolV1::repeat_connect()
{
...
void ProtocolV1::start_connect(const entity_addr_t& _peer_addr, const entity_name_t& _peer_name)
{
    ...
    gate.dispatch_in_background("start_connect", *this, [this] {
        return Socket::connect(conn.peer_addr)
            ...
            .then([this] { return messenger.get_global_seq(); })
            .then([this](auto gs) {
                h.global_seq = gs;
            ...
            .then([this](bufferlist headerbl) {
                ...
                if (saddr != conn.peer_addr) {
                    logger().error(
                        "{} my peer_addr {} doesn't match what peer advertized {}", conn, conn.peer_addr, saddr);
                ...
                if (unlikely(caddr.is_msgr2())) {
                    logger().warn("{} peer sent a v2 address for me: {}", conn, caddr);
                    throw std::system_error(make_error_code(crimson::net::error::bad_peer_address));
                }
            ...
            .then([=] { return seastar::repeat([this] { return repeat_connect(); }); })
            .then([this] {
                if (state != state_t::connecting) {
                    assert(state == state_t::closing);
                    throw std::system_error(make_error_code(error::protocol_aborted));
...
seastar::future<stop_t> ProtocolV1::send_connect_reply(msgr_tag_t tag, bufferlist&& authorizer_reply)
{
    h.reply.tag = tag;
    h.reply.features =
        static_cast<uint64_t>((h.connect.features & conn.policy.features_supported) | conn.policy.features_required);
    h.reply.authorizer_len = authorizer_reply.length();
    return socket->write(make_static_packet(h.reply))
        .then([this, reply = std::move(authorizer_reply)]() mutable { return socket->write_flush(std::move(reply)); })
        .then([] { return stop_t::no; });
}

seastar::future<stop_t> ProtocolV1::send_connect_reply_ready(msgr_tag_t tag, bufferlist&& authorizer_reply)
{
    return messenger.get_global_seq()
        .then([this, tag, auth_len = authorizer_reply.length()](auto gs) {
            ...
            session_security.reset(
                get_auth_session_handler(nullptr, auth_meta->auth_method, auth_meta->session_key, conn.features));
        ...
        .then([this] {
            if (h.reply.tag == CEPH_MSGR_TAG_SEQ) {
                return socket->write_flush(make_static_packet(conn.in_seq))
                    .then([this] { return socket->read_exactly(sizeof(seq_num_t)); })
                    .then([this](auto buf) {
                        auto acked_seq = reinterpret_cast<const seq_num_t*>(buf.get());
                        discard_up_to(&conn.out_q, *acked_seq);
                    });
        ...
        .then([] { return stop_t::yes; });
}

seastar::future<stop_t> ProtocolV1::replace_existing(SocketConnectionRef existing, bufferlist&& authorizer_reply,
                                                     bool is_reset_from_peer)
{
    msgr_tag_t reply_tag;
    if (HAVE_FEATURE(h.connect.features, RECONNECT_SEQ) && !is_reset_from_peer) {
...
seastar::future<stop_t> ProtocolV1::handle_connect_with_existing(SocketConnectionRef existing,
                                                                 bufferlist&& authorizer_reply)
{
    ...
    if (h.connect.global_seq < exproto->peer_global_seq()) {
        ...
        if (exproto->get_state() == state_t::open || exproto->get_state() == state_t::standby) {
    ...
    else if (conn.policy.resetcheck && exproto->connect_seq() == 0) {
...
bool ProtocolV1::require_auth_feature() const
{
    ...
    if (h.connect.host_type == CEPH_ENTITY_TYPE_OSD || h.connect.host_type == CEPH_ENTITY_TYPE_MDS ||
...
seastar::future<stop_t> ProtocolV1::repeat_handle_connect()
{
    return socket->read(sizeof(h.connect))
        .then([this](bufferlist bl) {
            auto p = bl.cbegin();
            ::decode(h.connect, p);
            if (conn.get_peer_type() != 0 && conn.get_peer_type() != h.connect.host_type) {
            ...
            if (!conn.policy.lossy && !conn.policy.server && conn.target_addr.get_port() <= 0) {
                logger().error("{} we don't know how to reconnect to peer {}", conn, conn.target_addr);
                throw std::system_error(make_error_code(crimson::net::error::bad_peer_address));
            }
        ...
        .then([this](bufferlist authorizer) {
            ...
            if (h.connect.protocol_version != get_proto_version(h.connect.host_type, false)) {
                return send_connect_reply(CEPH_MSGR_TAG_BADPROTOVER, bufferlist{});
            }
            ...
            if (auto feat_missing = conn.policy.features_required & ~(uint64_t)h.connect.features; feat_missing != 0) {
                return send_connect_reply(CEPH_MSGR_TAG_FEATURES, bufferlist{});
            }
            ...
            if (r < 0) {
                session_security.reset();
                return send_connect_reply(CEPH_MSGR_TAG_BADAUTHORIZER, std::move(authorizer_reply));
            }
            else if (r == 0) {
                ceph_assert(authorizer_reply.length());
                return send_connect_reply(CEPH_MSGR_TAG_CHALLENGE_AUTHORIZER, std::move(authorizer_reply));
            }
            ...
            if (h.connect.connect_seq > 0) {
                return send_connect_reply(CEPH_MSGR_TAG_RESETSESSION, std::move(authorizer_reply));
            }
...
void ProtocolV1::start_accept(SocketRef&& sock, const entity_addr_t& _peer_addr)
{
    ceph_assert(state == state_t::none);
    logger().trace("{} trigger accepting, was {}", conn, static_cast<int>(state));
    ...
    gate.dispatch_in_background("start_accept", *this, [this] {
        ...
        return messenger.learned_addr(messenger.get_myaddr(), conn)
            .then([this] {
            ...
            .then([this](bufferlist bl) {
                ...
                ::decode(addr, p);
                ceph_assert(p.end());
                if ((addr.is_legacy() || addr.is_any()) && addr.is_same_host(conn.target_addr)) {
                    // good
                }
                else {
...
ceph::bufferlist ProtocolV1::do_sweep_messages(const std::deque<MessageRef>& msgs, size_t num_msgs,
                                               bool require_keepalive, std::optional<utime_t> _keepalive_ack,
                                               bool require_ack)
{
    static const size_t RESERVE_MSG_SIZE =
        sizeof(CEPH_MSGR_TAG_MSG) + sizeof(ceph_msg_header) + sizeof(ceph_msg_footer);
    static const size_t RESERVE_MSG_SIZE_OLD =
        sizeof(CEPH_MSGR_TAG_MSG) + sizeof(ceph_msg_header) + sizeof(ceph_msg_footer_old);
    ...
    if (unlikely(require_keepalive)) {
        k.req.stamp = ceph::coarse_real_clock::to_ceph_timespec(ceph::coarse_real_clock::now());
...
{
    return socket->read_exactly(sizeof(ceph_timespec)).then([this](auto buf) {
...
{
    return socket->read_exactly(sizeof(ceph_le64)).then([this](auto buf) {
...
seastar::future<> ProtocolV1::maybe_throttle()
{
    ...
    const auto to_read = (m.header.front_len + m.header.middle_len + m.header.data_len);
...
seastar::future<> ProtocolV1::read_message()
{
    return socket->read(sizeof(m.header))
        ...
        .then([this](bufferlist bl) {
            ...
            auto msg = ::decode_message(nullptr, 0, m.header, m.footer, m.front, m.middle, m.data, conn_ref);
            if (unlikely(!msg)) {
            ...
            logger().debug("{} <== #{} === {} ({})", conn, msg_ref->get_seq(), *msg_ref, msg_ref->get_type());
...
seastar::future<> ProtocolV1::handle_tags()
{
    return seastar::keep_doing([this] {
        // read the next tag
        return socket->read_exactly(1).then([this](auto buf) {
            switch (buf[0]) {
            case CEPH_MSGR_TAG_MSG: return read_message();
            case CEPH_MSGR_TAG_ACK: return handle_ack();
            case CEPH_MSGR_TAG_KEEPALIVE: return seastar::now();
            case CEPH_MSGR_TAG_KEEPALIVE2: return handle_keepalive2();
            case CEPH_MSGR_TAG_KEEPALIVE2_ACK: return handle_keepalive2_ack();
            case CEPH_MSGR_TAG_CLOSE:
                logger().info("{} got tag close", conn);
                throw std::system_error(make_error_code(error::protocol_aborted));
            default:
                logger().error("{} got unknown msgr tag {}", conn, static_cast<int>(buf[0]));
...
{
    logger().trace("{} trigger closing, was {}", conn, static_cast<int>(state));
