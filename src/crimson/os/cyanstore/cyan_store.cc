...
template<const char* MsgV> struct singleton_ec : std::error_code
{
    singleton_ec()
...
seastar::future<store_statfs_t> CyanStore::stat() const
{
    ...
    return shard_stores
        .map_reduce0([](const CyanStore::Shard& local_store) { return local_store.get_used_bytes(); },
                     (uint64_t)0,
                     std::plus<uint64_t>())
...
CyanStore::mkfs_ertr::future<> CyanStore::mkfs(uuid_d new_osd_fsid)
{
    ...
    return read_meta("fsid")
        .then([=, this](auto&& ret) -> mkfs_ertr::future<> {
            auto& [r, fsid_str] = ret;
            if (r == -ENOENT) {
        ...
        .safe_then([this] { return write_meta("type", "memstore"); })
        .safe_then([this] { return shard_stores.invoke_on_all([](auto& local_store) { return local_store.mkfs(); }); });
}

seastar::future<> CyanStore::Shard::mkfs()
{
    std::string fn = path + "/collections" + std::to_string(seastar::this_shard_id());
    ceph::bufferlist bl;
...
seastar::future<std::vector<coll_core_t>> CyanStore::list_collections()
{
    ceph_assert(seastar::this_shard_id() == primary_core);
...
CyanStore::mount_ertr::future<> CyanStore::Shard::mount()
{
    ...
    for (auto& coll : collections) {
        std::string fn = fmt::format("{}/{}{}", path, coll, std::to_string(seastar::this_shard_id()));
        ceph::bufferlist cbl;
...
seastar::future<> CyanStore::Shard::umount()
{
    return seastar::do_with(std::set<coll_t>{}, [this](auto& collections) {
        return seastar::do_for_each(coll_map,
                                    [&collections, this](auto& coll) {
                                        auto& [col, ch] = coll;
                                        ...
                                        ch->encode(bl);
                                        std::string fn =
                                            fmt::format("{}/{}{}", path, col, std::to_string(seastar::this_shard_id()));
                                        return crimson::write_file(std::move(bl), fn);
                                    })
            .then([&collections, this] {
                ceph::bufferlist bl;
                ceph::encode(collections, bl);
                std::string fn = fmt::format("{}/collections{}", path, std::to_string(seastar::this_shard_id()));
                return crimson::write_file(std::move(bl), fn);
            });
    });
}

seastar::future<std::tuple<std::vector<ghobject_t>, ghobject_t>> CyanStore::Shard::list_objects(CollectionRef ch,
                                                                                                const ghobject_t& start,
                                                                                                const ghobject_t& end,
                                                                                                uint64_t limit) const
{
    auto c = static_cast<Collection*>(ch.get());
    logger().debug("{} {} {} {} {}", __func__, c->get_cid(), start, end, limit);
    std::vector<ghobject_t> objects;
    objects.reserve(limit);
    ghobject_t next = ghobject_t::get_max();
    for (const auto& [oid, obj] : boost::make_iterator_range(c->object_map.lower_bound(start), c->object_map.end())) {
        std::ignore = obj;
...
seastar::future<CollectionRef> CyanStore::Shard::create_new_collection(const coll_t& cid)
{
...
seastar::future<CollectionRef> CyanStore::Shard::open_collection(const coll_t& cid)
{
    return seastar::make_ready_future<CollectionRef>(_get_collection(cid));
}

seastar::future<std::vector<coll_core_t>> CyanStore::Shard::list_collections()
{
...
CyanStore::Shard::read_errorator::future<ceph::bufferlist> CyanStore::Shard::read(CollectionRef ch,
                                                                                  const ghobject_t& oid,
                                                                                  uint64_t offset, size_t len,
                                                                                  uint32_t op_flags)
{
    auto c = static_cast<Collection*>(ch.get());
    logger().debug("{} {} {} {}~{}", __func__, c->get_cid(), oid, offset, len);
    if (!c->exists) {
    ...
    if (offset >= o->get_size()) return read_errorator::make_ready_future<ceph::bufferlist>();
    size_t l = len;
...
CyanStore::Shard::read_errorator::future<ceph::bufferlist> CyanStore::Shard::readv(CollectionRef ch,
                                                                                   const ghobject_t& oid,
                                                                                   interval_set<uint64_t>& m,
                                                                                   uint32_t op_flags)
{
    return seastar::do_with(ceph::bufferlist{}, [this, ch, oid, &m, op_flags](auto& bl) {
        return crimson::do_for_each(m,
                                    [this, ch, oid, op_flags, &bl](auto& p) {
                                        return read(ch, oid, p.first, p.second, op_flags).safe_then([&bl](auto ret) {
                                            bl.claim_append(ret);
                                        });
                                    })
            .safe_then([&bl] { return read_errorator::make_ready_future<ceph::bufferlist>(std::move(bl)); });
    });
}

CyanStore::Shard::get_attr_errorator::future<ceph::bufferlist> CyanStore::Shard::get_attr(CollectionRef ch,
                                                                                          const ghobject_t& oid,
...
CyanStore::Shard::get_attrs_ertr::future<CyanStore::Shard::attrs_t> CyanStore::Shard::get_attrs(CollectionRef ch,
                                                                                                const ghobject_t& oid)
{
    auto c = static_cast<Collection*>(ch.get());
    logger().debug("{} {} {}", __func__, c->get_cid(), oid);
...
auto CyanStore::Shard::omap_get_values(CollectionRef ch, const ghobject_t& oid, const omap_keys_t& keys)
...
auto CyanStore::Shard::omap_get_values(CollectionRef ch, const ghobject_t& oid, const std::optional<string>& start)
    -> CyanStore::Shard::read_errorator::future<std::tuple<bool, omap_values_t>>
{
    ...
    omap_values_t values;
    for (auto i = start ? o->omap.upper_bound(*start) : o->omap.begin(); i != o->omap.end(); ++i) {
        values.insert(*i);
    }
    return seastar::make_ready_future<std::tuple<bool, omap_values_t>>(std::make_tuple(true, std::move(values)));
}

auto CyanStore::Shard::omap_get_header(CollectionRef ch, const ghobject_t& oid)
    -> CyanStore::Shard::get_attr_errorator::future<ceph::bufferlist>
{
    ...
    return get_attr_errorator::make_ready_future<ceph::bufferlist>(o->omap_header);
}

seastar::future<> CyanStore::Shard::do_transaction_no_callbacks(CollectionRef ch, ceph::os::Transaction&& t)
{
    ...
    try {
        ...
        while (i.have_op()) {
            ...
            default: logger().error("bad op {}", static_cast<unsigned>(op->op)); abort();
            }
            if (r < 0) {
                break;
            }
...
int CyanStore::Shard::_remove(const coll_t& cid, const ghobject_t& oid)
{
    ...
    if (i == c->object_hash.end()) return -ENOENT;
...
int CyanStore::Shard::_write(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len,
                             const ceph::bufferlist& bl, uint32_t fadvise_flags)
{
    logger().debug("{} {} {} {} ~ {}", __func__, cid, oid, offset, len);
...
int CyanStore::Shard::_zero(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len)
{
    logger().debug("{} {} {} {} ~ {}", __func__, cid, oid, offset, len);
...
int CyanStore::Shard::_omap_clear(const coll_t& cid, const ghobject_t& oid)
{
...
int CyanStore::Shard::_omap_set_values(const coll_t& cid, const ghobject_t& oid,
                                       std::map<std::string, ceph::bufferlist>&& aset)
{
    logger().debug("{} {} {} {} keys", __func__, cid, oid, aset.size());
...
int CyanStore::Shard::_omap_set_header(const coll_t& cid, const ghobject_t& oid, const ceph::bufferlist& header)
{
    logger().debug("{} {} {} {} bytes", __func__, cid, oid, header.length());
...
int CyanStore::Shard::_omap_rmkeys(const coll_t& cid, const ghobject_t& oid, const omap_keys_t& aset)
{
    logger().debug("{} {} {} {} keys", __func__, cid, oid, aset.size());
...
int CyanStore::Shard::_omap_rmkeyrange(const coll_t& cid, const ghobject_t& oid, const std::string& first,
                                       const std::string& last)
{
    logger().debug("{} {} {} first={} last={}", __func__, cid, oid, first, last);

    auto c = _get_collection(cid);
    if (!c) return -ENOENT;

    ObjectRef o = c->get_or_create_object(oid);
    for (auto i = o->omap.lower_bound(first); i != o->omap.end() && i->first < last; o->omap.erase(i++))
        ;
    return 0;
}

int CyanStore::Shard::_truncate(const coll_t& cid, const ghobject_t& oid, uint64_t size)
{
    logger().debug("{} cid={} oid={} size={}", __func__, cid, oid, size);
    auto c = _get_collection(cid);
    if (!c) return -ENOENT;

    ObjectRef o = c->get_object(oid);
    if (!o) return -ENOENT;
    if (local_conf()->memstore_debug_omit_block_device_write) return 0;
    const ssize_t old_size = o->get_size();
...
int CyanStore::Shard::_clone(const coll_t& cid, const ghobject_t& oid, const ghobject_t& noid)
{
    logger().debug("{} cid={} oid={} noid={}", __func__, cid, oid, noid);
    auto c = _get_collection(cid);
    if (!c) return -ENOENT;
...
int CyanStore::Shard::_setattrs(const coll_t& cid, const ghobject_t& oid, std::map<std::string, bufferlist>&& aset)
{
    logger().debug("{} cid={} oid={}", __func__, cid, oid);
    auto c = _get_collection(cid);
    if (!c) return -ENOENT;
...
int CyanStore::Shard::_rm_attr(const coll_t& cid, const ghobject_t& oid, std::string_view name)
{
...
int CyanStore::Shard::_rm_attrs(const coll_t& cid, const ghobject_t& oid)
{
    logger().debug("{} cid={} oid={}", __func__, cid, oid);
    auto c = _get_collection(cid);
    if (!c) {
...
int CyanStore::Shard::_create_collection(const coll_t& cid, int bits)
{
    ...
    if (!result.second) return -EEXIST;
...
boost::intrusive_ptr<Collection> CyanStore::Shard::_get_collection(const coll_t& cid)
{
    auto cp = coll_map.find(cid);
    if (cp == coll_map.end()) return {};
    return cp->second;
}

seastar::future<> CyanStore::write_meta(const std::string& key, const std::string& value)
{
    ceph_assert(seastar::this_shard_id() == primary_core);
    std::string v = value;
    v += "\n";
    if (int r = safe_write_file(path.c_str(), key.c_str(), v.c_str(), v.length(), 0600); r < 0) {
        throw std::runtime_error{fmt::format("unable to write_meta({})", key)};
    }
    return seastar::make_ready_future<>();
}

seastar::future<std::tuple<int, std::string>> CyanStore::read_meta(const std::string& key)
{
    ceph_assert(seastar::this_shard_id() == primary_core);
    ...
    if (r > 0) {
        ...
        boost::algorithm::trim_right_if(fsid, [](unsigned char c) { return isspace(c); });
    }
    else {
        fsid.clear();
    }
    return seastar::make_ready_future<std::tuple<int, std::string>>(std::make_tuple(r, fsid));
}
...
CyanStore::Shard::read_errorator::future<std::map<uint64_t, uint64_t>> CyanStore::Shard::fiemap(CollectionRef ch,
                                                                                                const ghobject_t& oid,
                                                                                                uint64_t off,
                                                                                                uint64_t len)
{
...
seastar::future<struct stat> CyanStore::Shard::stat(CollectionRef ch, const ghobject_t& oid)
{
    auto c = static_cast<Collection*>(ch.get());
    auto o = c->get_object(oid);
    if (!o) {
