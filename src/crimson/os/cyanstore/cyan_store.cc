...
seastar::future<> CyanStore::mkfs(uuid_d new_osd_fsid)
{
    return read_meta("fsid")
        ...
        .then([this] { return write_meta("type", "memstore"); });
}
...
seastar::future<std::tuple<std::vector<ghobject_t>, ghobject_t>> CyanStore::list_objects(CollectionRef ch,
...
CyanStore::read_errorator::future<ceph::bufferlist> CyanStore::read(CollectionRef ch, const ghobject_t& oid,
                                                                    uint64_t offset, size_t len, uint32_t op_flags)
{
    auto c = static_cast<Collection*>(ch.get());
...
CyanStore::read_errorator::future<ceph::bufferlist> CyanStore::readv(CollectionRef ch, const ghobject_t& oid,
                                                                     interval_set<uint64_t>& m, uint32_t op_flags)
{
...
CyanStore::get_attr_errorator::future<ceph::bufferptr> CyanStore::get_attr(CollectionRef ch, const ghobject_t& oid,
...
CyanStore::get_attrs_ertr::future<CyanStore::attrs_t> CyanStore::get_attrs(CollectionRef ch, const ghobject_t& oid)
{
    auto c = static_cast<Collection*>(ch.get());
    logger().debug("{} {} {}", __func__, c->get_cid(), oid);
    auto o = c->get_object(oid);
...
auto CyanStore::omap_get_values(CollectionRef ch, const ghobject_t& oid, const omap_keys_t& keys)
...
auto CyanStore::omap_get_values(CollectionRef ch, const ghobject_t& oid, const std::optional<string>& start)
...
auto CyanStore::omap_get_header(CollectionRef ch, const ghobject_t& oid) -> read_errorator::future<ceph::bufferlist>
{
    auto c = static_cast<Collection*>(ch.get());
    auto o = c->get_object(oid);
    if (!o) {
    ...
    return read_errorator::make_ready_future<ceph::bufferlist>(o->omap_header);
}

seastar::future<> CyanStore::do_transaction(CollectionRef ch, ceph::os::Transaction&& t)
{
    ...
    for (auto i : {t.get_on_applied(), t.get_on_commit(), t.get_on_applied_sync()}) {
        if (i) {
            i->complete(0);
        }
    }
...
int CyanStore::_write(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len, const ceph::bufferlist& bl,
...
int CyanStore::_zero(const coll_t& cid, const ghobject_t& oid, uint64_t offset, size_t len)
{
    logger().debug("{} {} {} {} ~ {}", __func__, cid, oid, offset, len);
...
int CyanStore::_omap_clear(const coll_t& cid, const ghobject_t& oid)
{
...
int CyanStore::_omap_set_values(const coll_t& cid, const ghobject_t& oid,
                                std::map<std::string, ceph::bufferlist>&& aset)
{
...
int CyanStore::_omap_set_header(const coll_t& cid, const ghobject_t& oid, const ceph::bufferlist& header)
{
...
int CyanStore::_omap_rmkeys(const coll_t& cid, const ghobject_t& oid, const omap_keys_t& aset)
{
    logger().debug("{} {} {} {} keys", __func__, cid, oid, aset.size());
...
int CyanStore::_omap_rmkeyrange(const coll_t& cid, const ghobject_t& oid, const std::string& first,
...
int CyanStore::_setattrs(const coll_t& cid, const ghobject_t& oid, std::map<std::string, bufferptr>& aset)
{
    logger().debug("{} cid={} oid={}", __func__, cid, oid);
    auto c = _get_collection(cid);
    if (!c) return -ENOENT;
    ...
    ObjectRef o = c->get_object(oid);
    if (!o) return -ENOENT;
    for (std::map<std::string, bufferptr>::const_iterator p = aset.begin(); p != aset.end(); ++p)
        o->xattr[p->first] = p->second;
    return 0;
}

int CyanStore::_rm_attr(const coll_t& cid, const ghobject_t& oid, std::string_view name)
{
...
seastar::future<FuturizedStore::OmapIteratorRef> CyanStore::get_omap_iterator(CollectionRef ch, const ghobject_t& oid)
{
    auto c = static_cast<Collection*>(ch.get());
    auto o = c->get_object(oid);
    if (!o) {
    ...
    return seastar::make_ready_future<FuturizedStore::OmapIteratorRef>(new CyanStore::CyanOmapIterator(o));
}

seastar::future<std::map<uint64_t, uint64_t>> CyanStore::fiemap(CollectionRef ch, const ghobject_t& oid, uint64_t off,
                                                                uint64_t len)
{
    auto c = static_cast<Collection*>(ch.get());
...
seastar::future<struct stat> CyanStore::stat(CollectionRef ch, const ghobject_t& oid)
{
    auto c = static_cast<Collection*>(ch.get());
    auto o = c->get_object(oid);
    if (!o) {
