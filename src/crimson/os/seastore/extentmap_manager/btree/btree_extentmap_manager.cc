...
BtreeExtentMapManager::BtreeExtentMapManager(TransactionManager& tm)
    : tm(tm)
{}

BtreeExtentMapManager::initialize_extmap_ret BtreeExtentMapManager::initialize_extmap(Transaction& t)
{

    logger().debug("{}", __func__);
    return tm.alloc_extent<ExtMapLeafNode>(t, L_ADDR_MIN, EXTMAP_BLOCK_SIZE).safe_then([](auto&& root_extent) {
...
BtreeExtentMapManager::get_root_ret BtreeExtentMapManager::get_extmap_root(const extmap_root_t& extmap_root,
                                                                           Transaction& t)
{
...
BtreeExtentMapManager::find_lextent_ret BtreeExtentMapManager::find_lextent(const extmap_root_t& extmap_root,
                                                                            Transaction& t, objaddr_t lo,
                                                                            extent_len_t len)
{
    logger().debug("{}: {}, {}", __func__, lo, len);
    return get_extmap_root(extmap_root, t)
        .safe_then([this, &t, lo, len](auto&& extent) { return extent->find_lextent(get_ext_context(t), lo, len); })
        .safe_then([](auto&& e) {
            logger().debug("{}: found_lextent {}", __func__, e);
            return find_lextent_ret(find_lextent_ertr::ready_future_marker{}, std::move(e));
        });
}

BtreeExtentMapManager::add_lextent_ret BtreeExtentMapManager::add_lextent(extmap_root_t& extmap_root, Transaction& t,
                                                                          objaddr_t lo, lext_map_val_t val)
{
    logger().debug("{}: {}, {}, {}", __func__, lo, val.laddr, val.length);
    return get_extmap_root(extmap_root, t)
        .safe_then(
            [this, &extmap_root, &t, lo, val](auto&& root) { return insert_lextent(extmap_root, t, root, lo, val); })
        .safe_then([](auto ret) {
            logger().debug("{}:  {}", __func__, ret);
            return add_lextent_ret(add_lextent_ertr::ready_future_marker{}, std::move(ret));
        });
}

BtreeExtentMapManager::insert_lextent_ret BtreeExtentMapManager::insert_lextent(extmap_root_t& extmap_root,
                                                                                Transaction& t, ExtMapNodeRef root,
    ...
    if (root->at_max_capacity()) {
        ...
        split = root->extmap_alloc_extent<ExtMapInnerNode>(get_ext_context(t), EXTMAP_BLOCK_SIZE)
                    .safe_then([this, &extmap_root, root, &t, logical_offset](auto&& nroot) {
                        ...
                        nroot->set_meta(meta);
                        nroot->journal_insert(nroot->begin(), OBJ_ADDR_MIN, root->get_laddr(), nullptr);
...
BtreeExtentMapManager::rm_lextent_ret BtreeExtentMapManager::rm_lextent(extmap_root_t& extmap_root, Transaction& t,
                                                                        objaddr_t lo, lext_map_val_t val)
{
    logger().debug("{}: {}, {}, {}", __func__, lo, val.laddr, val.length);
    return get_extmap_root(extmap_root, t)
        .safe_then([this, &t, lo, val](auto extent) { return extent->rm_lextent(get_ext_context(t), lo, val); })
        .safe_then([](auto removed) {
            logger().debug("{}: {}", __func__, removed);
            return rm_lextent_ret(rm_lextent_ertr::ready_future_marker{}, removed);
        });
}
