...
btree_range_pin_t* btree_pin_set_t::maybe_get_parent(const lba_node_meta_t& meta)
{
    auto cmeta = meta;
    cmeta.depth++;
...
const btree_range_pin_t* btree_pin_set_t::maybe_get_first_child(const lba_node_meta_t& meta) const
{
    if (meta.depth == 0) {
        return nullptr;
    }
...
void btree_pin_set_t::add_pin(btree_range_pin_t& pin)
{
    ...
    if (!pin.is_root()) {
        ...
        assert(parent);
        if (!parent->has_ref()) {
            logger().debug("{}: acquiring parent {}", __func__, static_cast<void*>(parent));
            parent->acquire_ref();
        }
        else {
            logger().debug("{}: parent has ref {}", __func__, static_cast<void*>(parent));
        }
    }
    if (maybe_get_first_child(pin.range) != nullptr) {
