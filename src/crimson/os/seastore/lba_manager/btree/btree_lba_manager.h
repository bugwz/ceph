...
public:
    ...
    {}
    BtreeLBAMapping(op_context_t<laddr_t> c, CachedExtentRef parent, uint16_t pos, lba_map_val_t& val,
                    lba_node_meta_t meta)
        : BtreeNodeMapping(c, parent, pos, val.pladdr.is_paddr() ? val.pladdr.get_paddr() : P_ADDR_NULL, val.len, meta)
        , key(meta.begin)
    ...
    lba_map_val_t get_map_val() const { return map_val; }

    bool is_indirect() const final { return indirect; }

    void set_key_for_indirect(laddr_t new_key, extent_len_t length, laddr_t interkey = L_ADDR_NULL)
    {
    ...
    laddr_t get_key() const final { return key; }

    pladdr_t get_raw_val() const { return raw_val; }

    void set_paddr(paddr_t addr) { value = addr; }
    ...
    extent_len_t get_intermediate_offset() const final
    {
        ...
        assert((intermediate_key == L_ADDR_NULL) == (intermediate_base == L_ADDR_NULL));
    ...
    void set_intermediate_base(laddr_t base) { intermediate_base = base; }

protected:
    std::unique_ptr<BtreeNodeMapping<laddr_t, paddr_t>> _duplicate(op_context_t<laddr_t> ctx) const final
    {
...
using LBABtree =
    FixedKVBtree<laddr_t, lba_map_val_t, LBAInternalNode, LBALeafNode, BtreeLBAMapping, LBA_BLOCK_SIZE, true>;
...
public:
    ...
    get_mappings_ret get_mappings(Transaction& t, laddr_t offset, extent_len_t length) final;

    get_mappings_ret get_mappings(Transaction& t, laddr_list_t&& list) final;

    get_mapping_ret get_mapping(Transaction& t, laddr_t offset) final;

    alloc_extent_ret reserve_region(Transaction& t, laddr_t hint, extent_len_t len)
    {
        return _alloc_extent(t, hint, len, P_ADDR_ZERO, P_ADDR_NULL, L_ADDR_NULL, nullptr);
    }

    alloc_extent_ret clone_extent(Transaction& t, laddr_t hint, extent_len_t len, laddr_t intermediate_key,
                                  paddr_t actual_addr, laddr_t intermediate_base)
    {
        return _alloc_extent(t, hint, len, intermediate_key, actual_addr, intermediate_base, nullptr);
    }

    alloc_extent_ret alloc_extent(Transaction& t, laddr_t hint, extent_len_t len, paddr_t addr,
                                  LogicalCachedExtent& ext) final
    {
        return _alloc_extent(t, hint, len, addr, P_ADDR_NULL, L_ADDR_NULL, &ext);
    }

    ref_ret decref_extent(Transaction& t, laddr_t addr, bool cascade_remove) final
    {
        return update_refcount(t, addr, -1, cascade_remove);
    }

    ref_ret incref_extent(Transaction& t, laddr_t addr) final { return update_refcount(t, addr, 1, false); }

    ref_ret incref_extent(Transaction& t, laddr_t addr, int delta) final
    {
        ceph_assert(delta > 0);
     ...
     */
    init_cached_extent_ret init_cached_extent(Transaction& t, CachedExtentRef e) final;

    check_child_trackers_ret check_child_trackers(Transaction& t) final;

    scan_mappings_ret scan_mappings(Transaction& t, laddr_t begin, laddr_t end, scan_mappings_func_t&& f) final;

    rewrite_extent_ret rewrite_extent(Transaction& t, CachedExtentRef extent) final;

    update_mapping_ret update_mapping(Transaction& t, laddr_t laddr, paddr_t prev_addr, paddr_t paddr,
                                      LogicalCachedExtent*) final;

    get_physical_extent_if_live_ret get_physical_extent_if_live(Transaction& t, extent_types_t type, paddr_t addr,
                                                                laddr_t laddr, extent_len_t len) final;
...
private:
    ...
    op_context_t<laddr_t> get_context(Transaction& t) { return op_context_t<laddr_t>{cache, t}; }
    ...
    update_refcount_ret update_refcount(Transaction& t, laddr_t addr, int delta, bool cascade_remove);
    ...
    using update_func_t = std::function<lba_map_val_t(const lba_map_val_t& v)>;
    _update_mapping_ret _update_mapping(Transaction& t, laddr_t addr, update_func_t&& f, LogicalCachedExtent*);

    alloc_extent_ret _alloc_extent(Transaction& t, laddr_t hint, extent_len_t len, pladdr_t addr, paddr_t actual_addr,
                                   laddr_t intermediate_base, LogicalCachedExtent*);

    using _get_mapping_ret = get_mapping_iertr::future<BtreeLBAMappingRef>;
    _get_mapping_ret _get_mapping(Transaction& t, laddr_t offset);

    using _get_original_mappings_ret = get_mappings_ret;
    _get_original_mappings_ret _get_original_mappings(op_context_t<laddr_t> c, std::list<BtreeLBAMappingRef>& pin_list);

    ref_iertr::future<std::optional<std::pair<paddr_t, extent_len_t>>> _decref_intermediate(Transaction& t,
                                                                                            laddr_t addr,
                                                                                            extent_len_t len);
};
