...
struct lba_map_val_t
{
    ...
    lba_map_val_t(extent_len_t len, pladdr_t pladdr, uint32_t refcount, uint32_t checksum)
        : len(len)
        , pladdr(pladdr)
        , refcount(refcount)
        , checksum(checksum)
...
struct LBAInternalNode
    : FixedKVInternalNode<INTERNAL_NODE_CAPACITY, laddr_t, laddr_le_t, LBA_BLOCK_SIZE, LBAInternalNode>
{
...
struct lba_map_val_le_t
{
    ...
    operator lba_map_val_t() const { return lba_map_val_t{len, pladdr, refcount, checksum}; }
};

struct LBALeafNode : FixedKVLeafNode<LEAF_NODE_CAPACITY, laddr_t, laddr_le_t, lba_map_val_t, lba_map_val_le_t,
                                     LBA_BLOCK_SIZE, LBALeafNode, true>
{
    using Ref = TCachedExtentRef<LBALeafNode>;
    using parent_type_t = FixedKVLeafNode<LEAF_NODE_CAPACITY, laddr_t, laddr_le_t, lba_map_val_t, lba_map_val_le_t,
                                          LBA_BLOCK_SIZE, LBALeafNode, true>;
    using internal_const_iterator_t = typename parent_type_t::node_layout_t::const_iterator;
    ...
    bool validate_stable_children() final
    {
        ...
        for (auto i : *this) {
            ...
            if (is_valid_child_ptr(child) && child->get_laddr() != i.get_key()) {
                SUBERROR(seastore_fixedkv_tree, "stable child not valid: child {}, key {}", *child, i.get_key());
                ceph_abort();
                return false;
            }
        }
    ...
    void update(internal_const_iterator_t iter, lba_map_val_t val, LogicalCachedExtent* nextent) final
    {
        ...
        return this->journal_update(iter, val, this->maybe_get_delta_buffer());
    }

    internal_const_iterator_t insert(internal_const_iterator_t iter, laddr_t addr, lba_map_val_t val,
                                     LogicalCachedExtent* nextent) final
    {
        LOG_PREFIX(LBALeafNode::insert);
        SUBTRACE(seastore_fixedkv_tree,
                 "trans.{}, pos {}, key {}, extent {}",
                 this->pending_for_transaction,
                 iter.get_offset(),
                 addr,
                 (void*)nextent);
        ...
        this->journal_insert(iter, addr, val, this->maybe_get_delta_buffer());
        return iter;
    }
    ...
    void node_resolve_vals(internal_iterator_t from, internal_iterator_t to) const final
    {
    ...
    void node_unresolve_vals(internal_iterator_t from, internal_iterator_t to) const final
    {
        if (this->is_initial_pending()) {
            for (auto i = from; i != to; ++i) {
                auto val = i->get_val();
