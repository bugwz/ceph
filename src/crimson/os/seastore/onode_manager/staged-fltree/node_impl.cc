...
eagain_ifuture<InternalNodeImpl::fresh_impl_t> InternalNodeImpl::allocate(context_t c, laddr_t hint, field_type_t type,
...
eagain_ifuture<LeafNodeImpl::fresh_impl_t> LeafNodeImpl::allocate(context_t c, laddr_t hint, field_type_t type,
                                                                  bool is_level_tail)
{
    if (type == field_type_t::N0) {
...
InternalNodeImplURef InternalNodeImpl::load(NodeExtentRef extent, field_type_t type)
{
    if (type == field_type_t::N0) {
...
LeafNodeImplURef LeafNodeImpl::load(NodeExtentRef extent, field_type_t type)
{
    if (type == field_type_t::N0) {
