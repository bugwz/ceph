...
{
public:
    SeastoreSuper(Transaction& t, RootNodeTracker& tracker, laddr_t root_addr, TransactionManager& tm)
        : Super(t, tracker)
        , root_addr{root_addr}
        , tm{tm}
    {}
...
protected:
    laddr_t get_root_laddr() const override { return root_addr; }
...
public:
    ...
    extent_types_t get_type() const override { return TYPE; }

protected:
    NodeExtentRef mutate(context_t, DeltaRecorderURef&&) override;

    DeltaRecorder* get_recorder() const override { return recorder.get(); }
...
{
public:
    TransactionManagerHandle(TransactionManager& tm)
        : tm{tm}
    {}
    TransactionManager& tm;
};

template<bool INJECT_EAGAIN = false> class SeastoreNodeExtentManager final : public TransactionManagerHandle
{
public:
    SeastoreNodeExtentManager(TransactionManager& tm, laddr_t min, double p_eagain)
        : TransactionManagerHandle(tm)
        , addr_min{min}
        , p_eagain{p_eagain}
    {
    ...
    void set_generate_eagain(bool enable) { generate_eagain = enable; }
...
protected:
    ...
    read_iertr::future<NodeExtentRef> read_extent(Transaction& t, laddr_t addr) override
    {
        ...
        return tm.read_extent<SeastoreNodeExtent>(t, addr).si_then(
            [addr, &t](auto&& e) -> read_iertr::future<NodeExtentRef> {
                SUBTRACET(seastore_onode, "read {}B at {:#x} -- {}", t, e->get_length(), e->get_laddr(), *e);
    ...
    alloc_iertr::future<NodeExtentRef> alloc_extent(Transaction& t, laddr_t hint, extent_len_t len) override
    {
        ...
        return tm.alloc_extent<SeastoreNodeExtent>(t, hint, len).si_then([len, &t](auto extent) {
            SUBDEBUGT(
                seastore_onode, "allocated {}B at {:#x} -- {}", t, extent->get_length(), extent->get_laddr(), *extent);
            if (!extent->is_initial_pending()) {
                SUBERRORT(seastore_onode, "allocated {}B but got invalid extent: {}", t, len, *extent);
    ...
    retire_iertr::future<> retire_extent(Transaction& t, NodeExtentRef _extent) override
    {
        LogicalCachedExtentRef extent = _extent;
        auto addr = extent->get_laddr();
        auto len = extent->get_length();
        SUBDEBUGT(seastore_onode, "retiring {}B at {:#x} -- {} ...", t, len, addr, *extent);
        if constexpr (INJECT_EAGAIN) {
            if (trigger_eagain()) {
                SUBDEBUGT(seastore_onode, "retiring {}B at {:#x} -- {} : trigger eagain", t, len, addr, *extent);
                t.test_set_conflict();
