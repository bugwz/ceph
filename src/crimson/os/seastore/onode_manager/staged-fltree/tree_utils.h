...
void initialize_cursor_from_item(Transaction& t, const ghobject_t& key,
                                 const typename decltype(std::declval<CursorType>().value())::item_t& item,
                                 CursorType& cursor, bool insert_success)
{
...
template<typename CursorType>
void validate_cursor_from_item(const ghobject_t& key,
                               const typename decltype(std::declval<CursorType>().value())::item_t& item,
                               CursorType& cursor)
{
    ceph_assert(!cursor.is_end());
...
template<typename ValueItem> class Values
{
public:
    Values(size_t n)
    {
    ...
    ValueItem create(size_t size) { return ValueItem::create(size, id++); }
...
template<typename ValueItem> class KVPool
{
public:
    struct kv_t
    {
    ...
    iterator_t begin() { return serial_p_kvs.begin(); }
    iterator_t end() { return serial_p_kvs.end(); }
    iterator_t random_begin() { return random_p_kvs.begin(); }
    iterator_t random_end() { return random_p_kvs.end(); }

    void shuffle() { std::shuffle(random_p_kvs.begin(), random_p_kvs.end(), std::default_random_engine{}); }
    ...
    void erase_from_random(iterator_t begin, iterator_t end)
    {
        ...
        std::sort(new_kvs.begin(), new_kvs.end(), [](auto& l, auto& r) { return l.key < r.key; });
    ...
    static KVPool create_raw_range(const std::vector<size_t>& ns_sizes, const std::vector<size_t>& oid_sizes,
                                   const std::vector<size_t>& value_sizes, const std::pair<index_t, index_t>& range2,
                                   const std::pair<index_t, index_t>& range1, const std::pair<index_t, index_t>& range0)
    {
        ...
        for (index_t i = range2.first; i < range2.second; ++i) {
            for (index_t j = range1.first; j < range1.second; ++j) {
                ...
                for (index_t k = range0.first; k < range0.second; ++k) {
                    kvs.emplace_back(kv_t{make_raw_oid(i, j, k, ns_size, oid_size), values.pick()});
                }
            }
        }
    ...
    static KVPool create_range(const std::pair<index_t, index_t>& range_i, const std::vector<size_t>& value_sizes,
                               const uint64_t block_size)
    {
        ...
        for (index_t i = range_i.first; i < range_i.second; ++i) {
            ...
            kvs.emplace_back(kv_t{make_oid(i), ValueItem::create(value_size, i, block_size)});
        }
        return KVPool(std::move(kvs));
    }
...
private:
    KVPool(kv_vector_t&& _kvs)
        : kvs(std::move(_kvs))
        , serial_p_kvs(kvs.size())
        , random_p_kvs(kvs.size())
    {
        init();
    }
    ...
    void init()
    {
        std::transform(kvs.begin(), kvs.end(), serial_p_kvs.begin(), [](kv_t& item) { return &item; });
        std::transform(kvs.begin(), kvs.end(), random_p_kvs.begin(), [](kv_t& item) { return &item; });
        shuffle();
    }

    static ghobject_t make_raw_oid(index_t index2, index_t index1, index_t index0, size_t ns_size, size_t oid_size)
    {
        assert(index1 < 10);
        ...
        return ghobject_t(shard_id_t(index2), index2, index2, os_ns.str(), os_oid.str(), index0, index0);
    }
...
template<bool TRACK, typename ValueImpl> class TreeBuilder
{
public:
    ...
    eagain_ifuture<BtreeCursor> insert_one(Transaction& t, const iterator_t& iter_rd)
    {
        auto p_kv = *iter_rd;
        logger().debug("[{}] insert {} -> {}", iter_rd - kvs.random_begin(), key_hobj_t{p_kv->key}, p_kv->value);
        return tree->insert(t, p_kv->key, {p_kv->value.get_payload_size()})
            .si_then([&t, this, p_kv](auto ret) {
                ...
                return tree->find(t, p_kv->key).si_then([cursor, p_kv](auto cursor_) mutable {
            ...
            .handle_error_interruptible(
                [](const crimson::ct_error::value_too_large& e) { ceph_abort("impossible path"); },
    ...
    eagain_ifuture<> insert(Transaction& t)
    {
        ...
        *ref_kv_iter = kvs.random_begin();
        ...
        return trans_intr::repeat(
                   [&t, this, cursors, ref_kv_iter, start_time]() -> eagain_ifuture<seastar::stop_iteration> {
                       if (*ref_kv_iter == kvs.random_end()) {
                           ...
                           return insert_one(t, *ref_kv_iter).si_then([cursors, ref_kv_iter](auto cursor) {
                               if constexpr (TRACK) {
            ...
            .si_then([&t, this, cursors, ref_kv_iter] {
                if (!cursors->empty()) {
                    ...
                    *ref_kv_iter = kvs.random_begin();
                    return seastar::do_with(cursors->begin(), [&t, this, cursors, ref_kv_iter](auto& c_iter) {
                        return trans_intr::repeat(
                            [&t, this, &c_iter, cursors, ref_kv_iter]() -> eagain_ifuture<seastar::stop_iteration> {
                                if (*ref_kv_iter == kvs.random_end()) {
    ...
    eagain_ifuture<> erase_one(Transaction& t, const iterator_t& iter_rd)
    {
        auto p_kv = *iter_rd;
        logger().debug("[{}] erase {} -> {}", iter_rd - kvs.random_begin(), key_hobj_t{p_kv->key}, p_kv->value);
        return tree->erase(t, p_kv->key).si_then([&t, this, p_kv](auto size) {
            ...
            ceph_assert(size == 1);
#ifndef NDEBUG
            return tree->contains(t, p_kv->key).si_then([](bool ret) { ceph_assert(ret == false); });
#else
            return eagain_iertr::now();
#endif
        });
    ...
    eagain_ifuture<> erase(Transaction& t, std::size_t erase_size)
    {
        ...
        return eagain_iertr::now()
            ...
            .si_then([&t, this, ref_kv_iter, erase_end] {
                *ref_kv_iter = kvs.random_begin();
                logger().warn("start erasing {}/{} kvs ...", erase_end - kvs.random_begin(), kvs.size());
                auto start_time = mono_clock::now();
                return trans_intr::repeat(
                    [&t, this, ref_kv_iter, start_time, erase_end]() -> eagain_ifuture<seastar::stop_iteration> {
    ...
    {
        return tree->get_stats_slow(t).si_then([](auto stats) { logger().warn("{}", stats); });
    }

    eagain_ifuture<std::size_t> height(Transaction& t) { return tree->height(t); }

    void reload(NodeExtentManagerURef&& nm) { tree.emplace(std::move(nm)); }

    eagain_ifuture<> validate_one(Transaction& t, const iterator_t& iter_seq)
    {
    ...
    eagain_ifuture<> validate(Transaction& t)
    {
        ...
        return seastar::do_with(kvs.begin(), [this, &t](auto& iter) {
            return trans_intr::repeat([this, &t, &iter]() -> eagain_iertr::future<seastar::stop_iteration> {
                if (iter == kvs.end()) {
                    return seastar::make_ready_future<seastar::stop_iteration>(seastar::stop_iteration::yes);
                }
...
private:
    static seastar::logger& logger() { return crimson::get_logger(ceph_subsys_test); }
