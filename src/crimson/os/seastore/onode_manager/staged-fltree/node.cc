...
tree_cursor_t::tree_cursor_t(Ref<LeafNode> node, const search_position_t& pos, const key_view_t& key,
...
void tree_cursor_t::update_kv(const key_view_t& key, const onode_t* _p_value, layout_version_t v) const
{
    assert(!is_end());
    assert(_p_value);
...
node_future<Node::search_result_t> Node::lower_bound(context_t c, const key_hobj_t& key)
{
    return seastar::do_with(MatchHistory(),
                            [this, c, &key](auto& history) { return lower_bound_tracked(c, key, history); });
}

node_future<std::pair<Ref<tree_cursor_t>, bool>> Node::insert(context_t c, const key_hobj_t& key, const onode_t& value)
{
    return seastar::do_with(MatchHistory(), [this, c, &key, &value](auto& history) {
        return lower_bound_tracked(c, key, history).safe_then([c, &key, &value, &history](auto result) {
            if (result.match() == MatchKindBS::EQ) {
                ...
                return leaf_node->insert_value(c, key, value, result.p_cursor->get_position(), history, result.mstat)
...
node_future<tree_stats_t> Node::get_tree_stats(context_t c)
{
    return seastar::do_with(tree_stats_t(), [this, c](auto& stats) {
        return do_get_tree_stats(c, stats).safe_then([&stats] { return stats; });
    });
}
...
{
    return LeafNode::allocate_root(c, root_tracker).safe_then([](auto ret) { /* FIXME: discard_result(); */ });
}

node_future<Ref<Node>> Node::load_root(context_t c, RootNodeTracker& root_tracker)
{
    return c.nm.get_super(c.t, root_tracker).safe_then([c, &root_tracker](auto&& _super) {
        auto root_addr = _super->get_root_laddr();
        assert(root_addr != L_ADDR_NULL);
        return Node::load(c, root_addr, true)
            .safe_then([c, _super = std::move(_super), &root_tracker](auto root) mutable {
...
node_future<> Node::upgrade_root(context_t c)
{
    ...
    return InternalNode::allocate_root(c, impl->level(), impl->laddr(), std::move(super))
        .safe_then([this](auto new_root) { as_child(search_position_t::end(), new_root); });
}
...
node_future<> Node::insert_parent(context_t c, Ref<Node> right_node)
{
    ...
    return parent_info().ptr->apply_child_split(c, parent_info().position, this, right_node);
}

node_future<Ref<Node>> Node::load(context_t c, laddr_t addr, bool expect_is_level_tail)
{
    // NOTE:
    ...
    return c.nm.read_extent(c.t, addr, NODE_BLOCK_SIZE).safe_then([expect_is_level_tail](auto extent) {
...
node_future<> InternalNode::apply_child_split(context_t c, const search_position_t& pos, Ref<Node> left_child,
                                              Ref<Node> right_child)
{
    ...
    auto [insert_stage, insert_size] = impl->evaluate_insert(left_key, left_child_addr, insert_pos);
    auto free_size = impl->free_size();
    if (free_size >= insert_size) {
        // insert
        [[maybe_unused]] auto p_value =
            impl->insert(left_key, left_child_addr_packed, insert_pos, insert_stage, insert_size);
    ...
    return (is_root() ? upgrade_root(c) : node_ertr::now())
        .safe_then(
            [this, c] { return InternalNode::allocate(c, impl->field_type(), impl->is_level_tail(), impl->level()); })
        .safe_then([this_ref,
                    this,
                    c,
                    left_key,
            ...
            auto left_child_addr = left_child->impl->laddr();
            auto left_child_addr_packed = laddr_packed_t{left_child_addr};
            auto [split_pos, is_insert_left, p_value] = impl->split_insert(fresh_right.mut,
                                                                           *right_node->impl,
                                                                           left_key,
                                                                           left_child_addr_packed,
                                                                           insert_pos,
                                                                           insert_stage,
                                                                           insert_size);
            assert(p_value->value == left_child_addr);
...
node_future<Ref<InternalNode>> InternalNode::allocate_root(context_t c, level_t old_root_level, laddr_t old_root_addr,
                                                           Super::URef&& super)
{
    return InternalNode::allocate(c, field_type_t::N0, true, old_root_level + 1)
        .safe_then([c, old_root_addr, super = std::move(super)](auto fresh_node) mutable {
...
node_future<Ref<tree_cursor_t>> InternalNode::lookup_smallest(context_t c)
{
    auto position = search_position_t::begin();
    laddr_t child_addr = impl->get_p_value(position)->value;
    return get_or_track_child(c, position, child_addr).safe_then([c](auto child) { return child->lookup_smallest(c); });
}

node_future<Ref<tree_cursor_t>> InternalNode::lookup_largest(context_t c)
{
    ...
    laddr_t child_addr = impl->get_p_value(position)->value;
    return get_or_track_child(c, position, child_addr).safe_then([c](auto child) { return child->lookup_largest(c); });
}

node_future<Node::search_result_t> InternalNode::lower_bound_tracked(context_t c, const key_hobj_t& key,
                                                                     MatchHistory& history)
{
    auto result = impl->lower_bound(key, history);
    return get_or_track_child(c, result.position, result.p_value->value).safe_then([c, &key, &history](auto child) {
        // XXX(multi-type): pass result.mstat to child
        return child->lower_bound_tracked(c, key, history);
    });
}

node_future<> InternalNode::do_get_tree_stats(context_t c, tree_stats_t& stats)
{
    ...
    return seastar::do_with(search_position_t(), [this, this_ref, c, &stats](auto& pos) {
        pos = search_position_t::begin();
        return crimson::do_until([this, this_ref, c, &stats, &pos]() -> node_future<bool> {
            auto child_addr = impl->get_p_value(pos)->value;
            return get_or_track_child(c, pos, child_addr)
                .safe_then([c, &stats](auto child) { return child->do_get_tree_stats(c, stats); })
...
node_future<> InternalNode::test_clone_root(context_t c_other, RootNodeTracker& tracker_other) const
{
    assert(is_root());
    assert(impl->is_level_tail());
    assert(impl->field_type() == field_type_t::N0);
    ...
    return InternalNode::allocate(c_other, field_type_t::N0, true, impl->level())
        .safe_then([this, c_other, &tracker_other](auto fresh_other) {
            ...
            return c_other.nm.get_super(c_other.t, tracker_other).safe_then([c_other, cloned_root](auto&& super_other) {
...
node_future<Ref<Node>> InternalNode::get_or_track_child(context_t c, const search_position_t& position,
                                                        laddr_t child_addr)
{
    ...
    return (found == tracked_child_nodes.end()
                ? (logger().trace("OTree::Internal: load child untracked at {:#x}, pos({}), level={}",
                                  ...
                                  position,
                                  level() - 1),
                   Node::load(c, child_addr, level_tail).safe_then([this, position](auto child) {
...
void InternalNode::replace_track(const search_position_t& position, Ref<Node> new_child, Ref<Node> old_child)
{
...
node_future<InternalNode::fresh_node_t> InternalNode::allocate(context_t c, field_type_t field_type, bool is_level_tail,
                                                               level_t level)
{
    return InternalNodeImpl::allocate(c, field_type, is_level_tail, level).safe_then([](auto&& fresh_impl) {
        auto node = Ref<InternalNode>(new InternalNode(fresh_impl.impl.get(), std::move(fresh_impl.impl)));
...
std::tuple<key_view_t, const onode_t*, layout_version_t> LeafNode::get_kv(const search_position_t& pos) const
{
    key_view_t key_view;
...
node_future<Ref<tree_cursor_t>> LeafNode::lookup_smallest(context_t)
{
    if (unlikely(impl->is_empty())) {
        assert(is_root());
        return node_ertr::make_ready_future<Ref<tree_cursor_t>>(new tree_cursor_t(this));
    }
    auto pos = search_position_t::begin();
    key_view_t index_key;
    auto p_value = impl->get_p_value(pos, &index_key);
    return node_ertr::make_ready_future<Ref<tree_cursor_t>>(get_or_track_cursor(pos, index_key, p_value));
}

node_future<Ref<tree_cursor_t>> LeafNode::lookup_largest(context_t)
{
    if (unlikely(impl->is_empty())) {
        assert(is_root());
        return node_ertr::make_ready_future<Ref<tree_cursor_t>>(new tree_cursor_t(this));
...
node_future<Node::search_result_t> LeafNode::lower_bound_tracked(context_t c, const key_hobj_t& key,
                                                                 MatchHistory& history)
{
    key_view_t index_key;
    ...
    return node_ertr::make_ready_future<search_result_t>(search_result_t{cursor, result.mstat});
}
...
node_future<> LeafNode::test_clone_root(context_t c_other, RootNodeTracker& tracker_other) const
{
    assert(is_root());
    assert(impl->is_level_tail());
    assert(impl->field_type() == field_type_t::N0);
...
node_future<Ref<tree_cursor_t>> LeafNode::insert_value(context_t c, const key_hobj_t& key, const onode_t& value,
                                                       ...
                                                       match_stat_t mstat)
{
    ...
    logger().debug("OTree::Leaf::Insert: "
                   ...
                   mstat);
    search_position_t insert_pos = pos;
    auto [insert_stage, insert_size] = impl->evaluate_insert(key, value, history, mstat, insert_pos);
    auto free_size = impl->free_size();
    if (free_size >= insert_size) {
        // insert
        on_layout_change();
    ...
    return (is_root() ? upgrade_root(c) : node_ertr::now())
        .safe_then([this, c] { return LeafNode::allocate(c, impl->field_type(), impl->is_level_tail()); })
        .safe_then(
            [this_ref, this, c, &key, &value, insert_pos, insert_stage = insert_stage, insert_size = insert_size](
                auto fresh_right) mutable {
                auto right_node = fresh_right.node;
                // no need to bump version for right node, as it is fresh
                on_layout_change();
                impl->prepare_mutate(c);
                auto [split_pos, is_insert_left, p_value] = impl->split_insert(
                    fresh_right.mut, *right_node->impl, key, value, insert_pos, insert_stage, insert_size);
                assert(p_value->size == value.size);
                track_split(split_pos, right_node);
                ...
                return insert_parent(c, right_node).safe_then([ret] { return ret; });
                // TODO (optimize)
                // try to acquire space from siblings before split... see btrfs
            });
}

node_future<Ref<LeafNode>> LeafNode::allocate_root(context_t c, RootNodeTracker& root_tracker)
{
    return LeafNode::allocate(c, field_type_t::N0, true).safe_then([c, &root_tracker](auto fresh_node) {
        auto root = fresh_node.node;
        return c.nm.get_super(c.t, root_tracker).safe_then([c, root](auto&& super) {
...
node_future<LeafNode::fresh_node_t> LeafNode::allocate(context_t c, field_type_t field_type, bool is_level_tail)
{
    return LeafNodeImpl::allocate(c, field_type, is_level_tail).safe_then([](auto&& fresh_impl) {
        auto node = Ref<LeafNode>(new LeafNode(fresh_impl.impl.get(), std::move(fresh_impl.impl)));
        return fresh_node_t{node, fresh_impl.mut};
    });
}
