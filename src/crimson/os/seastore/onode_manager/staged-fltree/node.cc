...
namespace fmt {
template<typename T> const void* ptr(const ::boost::intrusive_ptr<T>& p)
{
    return p.get();
}
}   // namespace fmt
...
// create from lookup
tree_cursor_t::tree_cursor_t(Ref<LeafNode> node, const search_position_t& pos, const key_view_t& key_view,
...
tree_cursor_t::tree_cursor_t(Ref<LeafNode> node)
    : ref_leaf_node{node}
    , position{search_position_t::end()}
    , cache{ref_leaf_node}
{
    assert(is_end());
...
eagain_ifuture<Ref<tree_cursor_t>> tree_cursor_t::get_next(context_t c)
{
    assert(is_tracked());
    return ref_leaf_node->get_next_cursor(c, position);
}

void tree_cursor_t::assert_next_to(const tree_cursor_t& prv, value_magic_t magic) const
{
#ifndef NDEBUG
    assert(!prv.is_end());
    if (is_end()) {
...
template<bool FORCE_MERGE> eagain_ifuture<Ref<tree_cursor_t>> tree_cursor_t::erase(context_t c, bool get_next)
{
    assert(is_tracked());
    return ref_leaf_node->erase<FORCE_MERGE>(c, position, get_next);
}
template eagain_ifuture<Ref<tree_cursor_t>> tree_cursor_t::erase<true>(context_t, bool);
template eagain_ifuture<Ref<tree_cursor_t>> tree_cursor_t::erase<false>(context_t, bool);

std::strong_ordering tree_cursor_t::compare_to(const tree_cursor_t& o, value_magic_t magic) const
{
...
eagain_ifuture<> tree_cursor_t::extend_value(context_t c, value_size_t extend_size)
{
    assert(is_tracked());
    return ref_leaf_node->extend_value(c, position, extend_size);
}

eagain_ifuture<> tree_cursor_t::trim_value(context_t c, value_size_t trim_size)
{
    assert(is_tracked());
    return ref_leaf_node->trim_value(c, position, trim_size);
}

template<bool VALIDATE> void tree_cursor_t::update_track(Ref<LeafNode> node, const search_position_t& pos)
{
...
void tree_cursor_t::update_cache_same_node(const key_view_t& key_view, const value_header_t* p_value_header) const
{
    assert(is_tracked());
...
void tree_cursor_t::Cache::update_all(const node_version_t& current_version, const key_view_t& _key_view,
                                      const value_header_t* _p_value_header)
{
    ...
    assert((const char*)p_value_header - p_node_base < (int)ref_leaf_node->get_node_size());
...
void tree_cursor_t::Cache::make_latest(value_magic_t magic, const search_position_t& pos)
{
...
std::pair<NodeExtentMutable&, ValueDeltaRecorder*> tree_cursor_t::Cache::prepare_mutate_value_payload(
...
eagain_ifuture<Node::search_result_t> Node::lower_bound(context_t c, const key_hobj_t& key)
{
    return seastar::do_with(MatchHistory(),
                            [this, c, &key](auto& history) { return lower_bound_tracked(c, key, history); });
}

eagain_ifuture<std::pair<Ref<tree_cursor_t>, bool>> Node::insert(context_t c, const key_hobj_t& key,
                                                                 value_config_t vconf, Ref<Node>&& this_ref)
{
    return seastar::do_with(
        MatchHistory(), [this, c, &key, vconf, this_ref = std::move(this_ref)](auto& history) mutable {
            return lower_bound_tracked(c, key, history)
                .si_then([c, &key, vconf, &history, this_ref = std::move(this_ref)](auto result) mutable {
                    ...
                    if (result.match() == MatchKindBS::EQ) {
                        ...
                        return leaf_node
                            ->insert_value(c, key, vconf, result.p_cursor->get_position(), history, result.mstat)
...
eagain_ifuture<std::size_t> Node::erase(context_t c, const key_hobj_t& key, Ref<Node>&& this_ref)
{
    return lower_bound(c, key).si_then([c, this_ref = std::move(this_ref)](auto result) mutable {
        // the cursor in the result should already hold the root node upwards
        this_ref.reset();
        if (result.match() != MatchKindBS::EQ) {
        ...
        return ref_cursor->erase(c, false).si_then([ref_cursor](auto next_cursor) {
...
{
    return seastar::do_with(tree_stats_t(), [this, c](auto& stats) {
        return do_get_tree_stats(c, stats).si_then([&stats] { return stats; });
    });
}
...
void Node::test_make_destructable(context_t c, NodeExtentMutable& mut, Super::URef&& _super)
{
...
eagain_ifuture<> Node::mkfs(context_t c, RootNodeTracker& root_tracker)
{
    ...
    return LeafNode::allocate_root(c, root_tracker).si_then([c, FNAME](auto ret) {
...
eagain_ifuture<Ref<Node>> Node::load_root(context_t c, RootNodeTracker& root_tracker)
{
    ...
    return c.nm.get_super(c.t, root_tracker)
        .handle_error_interruptible(eagain_iertr::pass_further{},
        ...
        .si_then([c, &root_tracker, FNAME](auto&& _super) {
            ...
            return Node::load(c, root_addr, true)
                .si_then([c, _super = std::move(_super), &root_tracker, FNAME](auto root) mutable {
...
eagain_ifuture<> Node::upgrade_root(context_t c, laddr_t hint)
{
    ...
    return InternalNode::allocate_root(c, hint, impl->level(), impl->laddr(), std::move(super_to_move))
        .si_then([this, c, FNAME](auto new_root) {
            as_child(search_position_t::end(), new_root);
            INFOT("upgraded from {} to {}", c.t, get_name(), new_root->get_name());
        });
}

template<bool VALIDATE> void Node::as_child(const search_position_t& pos, Ref<InternalNode> parent_node)
{
...
eagain_ifuture<> Node::apply_split_to_parent(context_t c, Ref<Node>&& this_ref, Ref<Node>&& split_right,
                                             bool update_right_index)
{
    assert(!is_root());
    assert(this == this_ref.get());
    // TODO(cross-node string dedup)
    return parent_info().ptr->apply_child_split(c, std::move(this_ref), std::move(split_right), update_right_index);
}

eagain_ifuture<Ref<tree_cursor_t>> Node::get_next_cursor_from_parent(context_t c)
{
    assert(!impl->is_level_tail());
    assert(!is_root());
...
eagain_ifuture<> Node::try_merge_adjacent(context_t c, bool update_parent_index, Ref<Node>&& this_ref)
{
    ...
    if constexpr (!FORCE_MERGE) {
        if (!impl->is_size_underflow() && !impl->has_single_value()) {
            // skip merge
    ...
    return parent_info()
        .ptr->get_child_peers(c, parent_info().position)
        .si_then([c, this_ref = std::move(this_ref), this, FNAME, update_parent_index](
                     auto lr_nodes) mutable -> eagain_ifuture<> {
            ...
            if (left_for_merge) {
                ...
                auto [merge_stage, merge_size] = left_for_merge->impl->evaluate_merge(*right_for_merge->impl);
                if (merge_size <= left_for_merge->impl->total_size()) {
                    ...
                    DEBUGT("merge {} and {} at merge_stage={}, merge_size={}B, "
                           ...
                           c.t,
                           left_for_merge->get_name(),
                           right_for_merge->get_name(),
                           merge_stage,
                           merge_size,
                           update_index_after_merge,
                           is_left);
                    ...
                    return left_for_merge->rebuild_extent(c).si_then(
                        [c,
                         update_index_after_merge,
                         left_addr,
                         ...
                         right_for_merge = std::move(right_for_merge)](auto left_mut) mutable {
                            ...
                            search_position_t left_last_pos =
                                left_for_merge->impl->merge(left_mut, *right_for_merge->impl, merge_stage, merge_size);
                            left_for_merge->track_merge(right_for_merge, merge_stage, left_last_pos);
                            --(c.t.get_onode_tree_stats().extents_num_delta);
                            return left_for_merge->parent_info().ptr->apply_children_merge(c,
                                                                                           std::move(left_for_merge),
                                                                                           left_addr,
                                                                                           std::move(right_for_merge),
                                                                                           update_index_after_merge);
                        });
                }
                else {
...
template<bool FORCE_MERGE>
eagain_ifuture<> Node::fix_parent_index(context_t c, Ref<Node>&& this_ref, bool check_downgrade)
{
    assert(!is_root());
    assert(this == this_ref.get());
    return parent_info().ptr->fix_index<FORCE_MERGE>(c, std::move(this_ref), check_downgrade);
}
...
eagain_ifuture<Ref<Node>> Node::load(context_t c, laddr_t addr, bool expect_is_level_tail)
{
    LOG_PREFIX(OTree::Node::load);
    return c.nm.read_extent(c.t, addr)
        .handle_error_interruptible(
            eagain_iertr::pass_further{},
            crimson::ct_error::input_output_error::handle([FNAME, c, addr, expect_is_level_tail] {
                ERRORT("EIO -- addr={:x}, is_level_tail={}", c.t, addr, expect_is_level_tail);
                ceph_abort("fatal error");
            }),
            crimson::ct_error::invarg::handle([FNAME, c, addr, expect_is_level_tail] {
                ERRORT("EINVAL -- addr={:x}, is_level_tail={}", c.t, addr, expect_is_level_tail);
                ceph_abort("fatal error");
            }),
            crimson::ct_error::enoent::handle([FNAME, c, addr, expect_is_level_tail] {
                ERRORT("ENOENT -- addr={:x}, is_level_tail={}", c.t, addr, expect_is_level_tail);
                ceph_abort("fatal error");
            }),
            crimson::ct_error::erange::handle([FNAME, c, addr, expect_is_level_tail] {
                ERRORT("ERANGE -- addr={:x}, is_level_tail={}", c.t, addr, expect_is_level_tail);
                ceph_abort("fatal error");
            }))
        .si_then([FNAME, c, addr, expect_is_level_tail](auto extent) -> eagain_ifuture<Ref<Node>> {
            assert(extent);
            ...
            if (node_type == node_type_t::LEAF) {
                ...
                return eagain_iertr::make_ready_future<Ref<Node>>(new LeafNode(derived_ptr, std::move(impl)));
            }
                ...
                auto* derived_ptr = impl.get();
                return eagain_iertr::make_ready_future<Ref<Node>>(new InternalNode(derived_ptr, std::move(impl)));
            }
            else {
...
eagain_ifuture<> Node::retire(context_t c, Ref<Node>&& this_ref)
{
    ...
    return impl->retire_extent(c).si_then([this_ref = std::move(this_ref)] { /* deallocate node */ });
}
...
InternalNode::InternalNode(InternalNodeImpl* impl, NodeImplURef&& impl_ref)
    : Node(std::move(impl_ref))
    , impl{impl}
{}

eagain_ifuture<Ref<tree_cursor_t>> InternalNode::get_next_cursor(context_t c, const search_position_t& pos)
{
    impl->validate_non_empty();
    if (pos.is_end()) {
        assert(impl->is_level_tail());
    ...
    if (next_pos.is_end() && !impl->is_level_tail()) {
        ...
        assert(p_child_addr);
        return get_or_track_child(c, next_pos, p_child_addr->value).si_then([c](auto child) {
            return child->lookup_smallest(c);
        });
    }
}

eagain_ifuture<> InternalNode::apply_child_split(context_t c, Ref<Node>&& left_child, Ref<Node>&& right_child,
                                                 bool update_right_index)
{
    ...
    return insert_or_split(c, left_pos, left_key, left_child, (update_right_index ? right_child : nullptr))
        .si_then([this, c, this_ref = std::move(this_ref)](auto split_right) mutable {
            if (split_right) {
                ...
                return apply_split_to_parent(c, std::move(this_ref), std::move(split_right), false);
            }
            else {
                return eagain_iertr::now();
            }
        })
        .si_then([c, update_right_index, right_child = std::move(right_child)]() mutable {
...
eagain_ifuture<> InternalNode::erase_child(context_t c, Ref<Node>&& child_ref)
{
    ...
    bool fix_tail = (child_ref->parent_info().position.is_end() && !impl->is_keys_empty());
    return eagain_iertr::now()
        .si_then([c, this, fix_tail] {
            if (fix_tail) {
        ...
        .si_then([c, this, child_ref = std::move(child_ref), FNAME](auto&& new_tail_child) mutable {
            ...
            if (new_tail_child) {
                ...
                DEBUGT("erase {}'s child {} at pos({}) ...", c.t, get_name(), child_ref->get_name(), child_pos);
            }

            Ref<Node> this_ref = child_ref->deref_parent();
            assert(this_ref == this);
            return child_ref->retire(c, std::move(child_ref))
                .si_then([c, this, child_pos, FNAME, this_ref = std::move(this_ref)]() mutable {
                    ...
                    if (is_root()) {
                        ...
                        if (impl->is_level_tail()) {
                            ...
                            // next_or_last_pos as next_pos
                            next_or_last_pos.is_end() ? update_parent_index = true : update_parent_index = false;
                        }
                ...
                .si_then([c, new_tail_child = std::move(new_tail_child)]() mutable {
                    ...
                    if (new_tail_child && !new_tail_child->is_root()) {
                        assert(new_tail_child->impl->is_level_tail());
                        return new_tail_child->try_merge_adjacent(c, false, std::move(new_tail_child));
                    }
                    else {
                        return eagain_iertr::now();
                    }
...
template<bool FORCE_MERGE>
eagain_ifuture<> InternalNode::fix_index(context_t c, Ref<Node>&& child, bool check_downgrade)
{
    ...
    if (impl->is_level_tail()) {
        ...
        next_pos.is_end() ? update_parent_index = true : update_parent_index = false;
    }

    return insert_or_split(c, next_pos, new_key, child)
        .si_then(
            [this, c, update_parent_index, check_downgrade, this_ref = std::move(this_ref)](auto split_right) mutable {
                if (split_right) {
                    // after split, the parent index to the split_right will be incorrect
                    // if update_parent_index is true.
                    return apply_split_to_parent(c, std::move(this_ref), std::move(split_right), update_parent_index);
                }
                else {
                    // no split path
                    if (is_root()) {
                        ...
                        return try_merge_adjacent<FORCE_MERGE>(c, update_parent_index, std::move(this_ref));
                    }
                }
            });
}
...
template<bool FORCE_MERGE>
eagain_ifuture<> InternalNode::apply_children_merge(context_t c, Ref<Node>&& left_child, laddr_t origin_left_addr,
                                                    Ref<Node>&& right_child, bool update_index)
{
    ...
    return right_child->retire(c, std::move(right_child))
        .si_then([c, this, update_index, left_child = std::move(left_child)]() mutable {
            if (update_index) {
                // I'm all good but:
                ...
                if (is_root()) {
                    ...
                    return try_merge_adjacent<FORCE_MERGE>(c, false, std::move(this_ref));
                }
            }
        });
}
template eagain_ifuture<> InternalNode::apply_children_merge<true>(context_t, Ref<Node>&&, laddr_t, Ref<Node>&&, bool);
template eagain_ifuture<> InternalNode::apply_children_merge<false>(context_t, Ref<Node>&&, laddr_t, Ref<Node>&&, bool);

eagain_ifuture<std::pair<Ref<Node>, Ref<Node>>> InternalNode::get_child_peers(context_t c, const search_position_t& pos)
{
    // assume I'm already ref counted by caller
    ...
    return eagain_iertr::now()
        .si_then([this, c, prev_pos, prev_p_child_addr] {
        ...
        .si_then([this, c, next_pos, next_p_child_addr](Ref<Node> lnode) {
            if (next_p_child_addr != nullptr) {
                return get_or_track_child(c, next_pos, next_p_child_addr->value).si_then([lnode](Ref<Node> rnode) {
                    return seastar::make_ready_future<std::pair<Ref<Node>, Ref<Node>>>(lnode, rnode);
                });
            }
            else {
                return eagain_iertr::make_ready_future<std::pair<Ref<Node>, Ref<Node>>>(lnode, nullptr);
            }
        });
}

eagain_ifuture<Ref<InternalNode>> InternalNode::allocate_root(context_t c, laddr_t hint, level_t old_root_level,
                                                              laddr_t old_root_addr, Super::URef&& super)
{
    ...
    return InternalNode::allocate(c, hint, field_type_t::N0, true, old_root_level + 1)
        .si_then([c, old_root_addr, super = std::move(super)](auto fresh_node) mutable {
            auto root = fresh_node.node;
            assert(root->impl->is_keys_empty());
            auto p_value = root->impl->get_tail_value();
            fresh_node.mut.copy_in_absolute(const_cast<laddr_packed_t*>(p_value), old_root_addr);
...
eagain_ifuture<Ref<tree_cursor_t>> InternalNode::lookup_smallest(context_t c)
{
    impl->validate_non_empty();
    auto position = search_position_t::begin();
    const laddr_packed_t* p_child_addr;
    impl->get_slot(position, nullptr, &p_child_addr);
    return get_or_track_child(c, position, p_child_addr->value).si_then([c](auto child) {
        return child->lookup_smallest(c);
    });
}

eagain_ifuture<Ref<tree_cursor_t>> InternalNode::lookup_largest(context_t c)
{
    ...
    return get_or_track_child(c, search_position_t::end(), p_child_addr->value).si_then([c](auto child) {
        return child->lookup_largest(c);
    });
}

eagain_ifuture<Node::search_result_t> InternalNode::lower_bound_tracked(context_t c, const key_hobj_t& key,
                                                                        MatchHistory& history)
{
    auto result = impl->lower_bound(key, history);
    return get_or_track_child(c, result.position, result.p_value->value).si_then([c, &key, &history](auto child) {
        // XXX(multi-type): pass result.mstat to child
        return child->lower_bound_tracked(c, key, history);
    });
}

eagain_ifuture<> InternalNode::do_get_tree_stats(context_t c, tree_stats_t& stats)
{
    impl->validate_non_empty();
    ...
    return seastar::do_with(
        search_position_t(),
        (const laddr_packed_t*)(nullptr),
        [this, this_ref, c, &stats](auto& pos, auto& p_child_addr) {
            pos = search_position_t::begin();
            impl->get_slot(pos, nullptr, &p_child_addr);
            return trans_intr::repeat(
                [this, this_ref, c, &stats, &pos, &p_child_addr]() -> eagain_ifuture<seastar::stop_iteration> {
                    return get_or_track_child(c, pos, p_child_addr->value)
                        .si_then([c, &stats](auto child) { return child->do_get_tree_stats(c, stats); })
...
void InternalNode::track_merge(Ref<Node> _right_node, match_stage_t stage, search_position_t& left_last_pos)
{
...
eagain_ifuture<> InternalNode::test_clone_root(context_t c_other, RootNodeTracker& tracker_other) const
{
    assert(is_root());
    assert(impl->is_level_tail());
    assert(impl->field_type() == field_type_t::N0);
    ...
    return InternalNode::allocate(c_other, L_ADDR_MIN, field_type_t::N0, true, impl->level())
        ...
        .si_then([this_ref, this, c_other](auto cloned_root) {
            ...
            return trans_intr::do_for_each(
                tracked_child_nodes.begin(), tracked_child_nodes.end(), [this_ref, c_other, cloned_root](auto& kv) {
...
eagain_ifuture<> InternalNode::try_downgrade_root(context_t c, Ref<Node>&& this_ref)
{
    ...
    return get_or_track_child(c, search_position_t::end(), child_addr)
        .si_then([c, this, FNAME, this_ref = std::move(this_ref)](auto child) mutable {
            INFOT("downgrade {} to new root {}", c.t, get_name(), child->get_name());
...
eagain_ifuture<Ref<InternalNode>> InternalNode::insert_or_split(context_t c, const search_position_t& pos,
                                                                const key_view_t& insert_key, Ref<Node> insert_child,
                                                                Ref<Node> outdated_child)
{
    ...
    DEBUGT("insert {} with insert_key={}, insert_child={}, insert_pos({}), "
           ...
           insert_pos,
           (outdated_child ? "True" : "False"));
    auto [insert_stage, insert_size] = impl->evaluate_insert(insert_key, insert_value, insert_pos);
    auto free_size = impl->free_size();
    if (free_size >= insert_size) {
        // proceed to insert
        [[maybe_unused]] auto p_value = impl->insert(insert_key, insert_value, insert_pos, insert_stage, insert_size);
    ...
    return (is_root() ? upgrade_root(c, left_hint) : eagain_iertr::now())
        .si_then([this, c, right_hint] {
            return InternalNode::allocate(c, right_hint, impl->field_type(), impl->is_level_tail(), impl->level());
        })
        .si_then([this,
                  insert_key,
                  insert_child,
                  ...
                  outdated_child,
                  c,
                  FNAME](auto fresh_right) mutable {
            ...
            auto [split_pos, is_insert_left, p_value] = impl->split_insert(
                fresh_right.mut, *right_node->impl, insert_key, insert_value, insert_pos, insert_stage, insert_size);
            assert(p_value->value == insert_value);
...
eagain_ifuture<Ref<Node>> InternalNode::get_or_track_child(context_t c, const search_position_t& position,
                                                           laddr_t child_addr)
{
    ...
    return [this, position, child_addr, c, FNAME] {
        ...
        if (found != tracked_child_nodes.end()) {
            TRACET(
                "loaded child tracked {} at pos({}) addr={:x}", c.t, found->second->get_name(), position, child_addr);
            return eagain_iertr::make_ready_future<Ref<Node>>(found->second);
        }
        // the child is not loaded yet
        TRACET("loading child at pos({}) addr={:x} ...", c.t, position, child_addr);
        bool level_tail = position.is_end();
        return Node::load(c, child_addr, level_tail).si_then([this, position, c, FNAME](auto child) {
            TRACET("loaded child untracked {}", c.t, child->get_name());
...
void InternalNode::track_insert(const search_position_t& insert_pos, match_stage_t insert_stage, Ref<Node> insert_child,
                                Ref<Node> nxt_child)
{
    // update tracks
    ...
    std::for_each(first, last, [&nodes](auto& kv) { nodes.push_back(kv.second); });
...
void InternalNode::replace_track(Ref<Node> new_child, Ref<Node> old_child, bool is_new_child_outdated)
{
...
void InternalNode::track_split(const search_position_t& split_pos, Ref<InternalNode> right_node)
{
...
template<bool VALIDATE> void InternalNode::track_erase(const search_position_t& erase_pos, match_stage_t erase_stage)
{
    auto first = tracked_child_nodes.lower_bound(erase_pos);
    assert(first == tracked_child_nodes.end() || first->first != erase_pos);
    auto pos_upper_bound = erase_pos;
    pos_upper_bound.index_by_stage(erase_stage) = INDEX_UPPER_BOUND;
    auto last = tracked_child_nodes.lower_bound(pos_upper_bound);
    std::vector<Node*> p_nodes;
    std::for_each(first, last, [&p_nodes](auto& kv) { p_nodes.push_back(kv.second); });
    tracked_child_nodes.erase(first, last);
...
void InternalNode::track_make_tail(const search_position_t& last_pos)
{
    ...
    assert(tracked_child_nodes.find(search_position_t::end()) == tracked_child_nodes.end());
    ...
    if (last_it != tracked_child_nodes.end()) {
        ...
        assert(tracked_child_nodes.lower_bound(last_pos) == tracked_child_nodes.end());
    }
}
...
eagain_ifuture<InternalNode::fresh_node_t> InternalNode::allocate(context_t c, laddr_t hint, field_type_t field_type,
                                                                  bool is_level_tail, level_t level)
{
    return InternalNodeImpl::allocate(c, hint, field_type, is_level_tail, level).si_then([](auto&& fresh_impl) {
        auto* derived_ptr = fresh_impl.impl.get();
        auto node = Ref<InternalNode>(new InternalNode(derived_ptr, std::move(fresh_impl.impl)));
...
std::tuple<key_view_t, const value_header_t*> LeafNode::get_kv(const search_position_t& pos) const
{
    key_view_t key_view;
    const value_header_t* p_value_header;
...
eagain_ifuture<Ref<tree_cursor_t>> LeafNode::get_next_cursor(context_t c, const search_position_t& pos)
{
    impl->validate_non_empty();
    search_position_t next_pos = pos;
    key_view_t index_key;
    ...
    if (next_pos.is_end()) {
        if (unlikely(is_level_tail())) {
            return eagain_iertr::make_ready_future<Ref<tree_cursor_t>>(tree_cursor_t::create_end(this));
        }
        else {
            return get_next_cursor_from_parent(c);
        }
...
template<bool FORCE_MERGE>
eagain_ifuture<Ref<tree_cursor_t>> LeafNode::erase(context_t c, const search_position_t& pos, bool get_next)
{
    LOG_PREFIX(OTree::LeafNode::erase);
    assert(!pos.is_end());
    assert(!impl->is_keys_empty());
    Ref<Node> this_ref = this;
    DEBUGT("erase {}'s pos({}), get_next={} ...", c.t, get_name(), pos, get_next);
    ++(c.t.get_onode_tree_stats().num_erases);

    // get the next cursor
    return eagain_iertr::now()
        .si_then([c, &pos, get_next, this] {
            if (get_next) {
        ...
        .si_then([c, &pos, this_ref = std::move(this_ref), this, FNAME](Ref<tree_cursor_t> next_cursor) mutable {
            if (next_cursor && next_cursor->is_end()) {
                // reset the node reference from the end cursor
                next_cursor.reset();
            }
            return eagain_iertr::now()
                .si_then([c, &pos, this_ref = std::move(this_ref), this, FNAME]() mutable {
                ...
                .si_then([next_cursor] { return next_cursor; });
        });
}
template eagain_ifuture<Ref<tree_cursor_t>> LeafNode::erase<true>(context_t, const search_position_t&, bool);
template eagain_ifuture<Ref<tree_cursor_t>> LeafNode::erase<false>(context_t, const search_position_t&, bool);

eagain_ifuture<> LeafNode::extend_value(context_t c, const search_position_t& pos, value_size_t extend_size)
{
    ceph_abort("not implemented");
    return eagain_iertr::now();
}

eagain_ifuture<> LeafNode::trim_value(context_t c, const search_position_t& pos, value_size_t trim_size)
{
    ceph_abort("not implemented");
    return eagain_iertr::now();
}

std::pair<NodeExtentMutable&, ValueDeltaRecorder*> LeafNode::prepare_mutate_value_payload(context_t c)
{
    return impl->prepare_mutate_value_payload(c);
}

eagain_ifuture<Ref<tree_cursor_t>> LeafNode::lookup_smallest(context_t)
{
    if (unlikely(impl->is_keys_empty())) {
        assert(is_root());
        return seastar::make_ready_future<Ref<tree_cursor_t>>(tree_cursor_t::create_end(this));
    }
    auto pos = search_position_t::begin();
    key_view_t index_key;
    const value_header_t* p_value_header;
    impl->get_slot(pos, &index_key, &p_value_header);
    return seastar::make_ready_future<Ref<tree_cursor_t>>(get_or_track_cursor(pos, index_key, p_value_header));
}

eagain_ifuture<Ref<tree_cursor_t>> LeafNode::lookup_largest(context_t)
{
    if (unlikely(impl->is_keys_empty())) {
        assert(is_root());
        return seastar::make_ready_future<Ref<tree_cursor_t>>(tree_cursor_t::create_end(this));
...
eagain_ifuture<Node::search_result_t> LeafNode::lower_bound_tracked(context_t c, const key_hobj_t& key,
                                                                    MatchHistory& history)
{
    key_view_t index_key;
...
void LeafNode::track_merge(Ref<Node> _right_node, match_stage_t stage, search_position_t& left_last_pos)
{
    assert(level() == _right_node->level());
    // assert(impl->node_type() == _right_node->impl->node_type());
...
eagain_ifuture<> LeafNode::test_clone_root(context_t c_other, RootNodeTracker& tracker_other) const
{
    assert(is_root());
    assert(impl->is_level_tail());
    assert(impl->field_type() == field_type_t::N0);
...
eagain_ifuture<Ref<tree_cursor_t>> LeafNode::insert_value(context_t c, const key_hobj_t& key, value_config_t vconf,
                                                          ...
                                                          match_stat_t mstat)
{
    ...
    auto [insert_stage, insert_size] = impl->evaluate_insert(key, vconf, history, mstat, insert_pos);
    auto free_size = impl->free_size();
    if (free_size >= insert_size) {
        // proceed to insert
        on_layout_change();
    ...
    return (is_root() ? upgrade_root(c, left_hint) : eagain_iertr::now())
        ...
        .si_then([this_ref = std::move(this_ref),
                  this,
                  c,
                  &key,
                  vconf,
                  ...
                  insert_pos,
                  insert_stage = insert_stage,
                  insert_size = insert_size](auto fresh_right) mutable {
            auto right_node = fresh_right.node;
            DEBUGT("proceed split {} to fresh {} ...", c.t, get_name(), right_node->get_name());
            // no need to bump version for right node, as it is fresh
            on_layout_change();
            impl->prepare_mutate(c);
            auto [split_pos, is_insert_left, p_value_header] = impl->split_insert(
                fresh_right.mut, *right_node->impl, key, vconf, insert_pos, insert_stage, insert_size);
            assert(p_value_header->payload_size == vconf.payload_size);
            track_split(split_pos, right_node);
            ...
            ++(c.t.get_onode_tree_stats().extents_num_delta);
            return apply_split_to_parent(c, std::move(this_ref), std::move(right_node), false).si_then([ret] {
                return ret;
            });
            // TODO (optimize)
...
eagain_ifuture<Ref<LeafNode>> LeafNode::allocate_root(context_t c, RootNodeTracker& root_tracker)
{
...
Ref<tree_cursor_t> LeafNode::get_or_track_cursor(const search_position_t& position, const key_view_t& key,
                                                 const value_header_t* p_value_header)
{
    ...
    if (found == tracked_cursors.end()) {
        p_cursor = tree_cursor_t::create_tracked(this, position, key, p_value_header);
    }
    else {
...
Ref<tree_cursor_t> LeafNode::track_insert(const search_position_t& insert_pos, match_stage_t insert_stage,
                                          const value_header_t* p_value_header)
{
    ...
    std::for_each(first, last, [&p_cursors](auto& kv) { p_cursors.push_back(kv.second); });
    ...
    return tree_cursor_t::create_inserted(this, insert_pos);
}

void LeafNode::track_split(const search_position_t& split_pos, Ref<LeafNode> right_node)
{
...
void LeafNode::track_erase(const search_position_t& erase_pos, match_stage_t erase_stage)
{
...
eagain_ifuture<LeafNode::fresh_node_t> LeafNode::allocate(context_t c, laddr_t hint, field_type_t field_type,
                                                          bool is_level_tail)
{
    return LeafNodeImpl::allocate(c, hint, field_type, is_level_tail).si_then([](auto&& fresh_impl) {
        auto* derived_ptr = fresh_impl.impl.get();
        auto node = Ref<LeafNode>(new LeafNode(derived_ptr, std::move(fresh_impl.impl)));
        return fresh_node_t{node, fresh_impl.mut};
    });
}
