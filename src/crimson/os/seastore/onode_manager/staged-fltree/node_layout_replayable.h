 ...
 */
template<typename FieldType, node_type_t NODE_TYPE> struct NodeLayoutReplayableT
{
    ...
    template<KeyT KT>
    static const value_t* insert(NodeExtentMutable& mut, const node_stage_t& node_stage, const full_key_t<KT>& key,
                                 const value_input_t& value, position_t& insert_pos, match_stage_t& insert_stage,
    ...
    static void split(NodeExtentMutable& mut, const node_stage_t& node_stage, StagedIterator& split_at)
    {
    ...
    template<KeyT KT>
    static const value_t* split_insert(NodeExtentMutable& mut, const node_stage_t& node_stage, StagedIterator& split_at,
                                       const full_key_t<KT>& key, const value_input_t& value, position_t& insert_pos,
    ...
    static void update_child_addr(NodeExtentMutable& mut, const laddr_t new_addr, laddr_packed_t* p_addr)
    {
        assert(NODE_TYPE == node_type_t::INTERNAL);
        mut.copy_in_absolute(p_addr, new_addr);
    }

    static std::tuple<match_stage_t, position_t> erase(NodeExtentMutable& mut, const node_stage_t& node_stage,
    ...
    static position_t make_tail(NodeExtentMutable& mut, const node_stage_t& node_stage)
    {
        assert(!node_stage.is_level_tail());
...
private:
    static std::tuple<match_stage_t, position_t> update_last_to_tail(NodeExtentMutable& mut,
                                                                     const node_stage_t& node_stage)
    {
        if constexpr (NODE_TYPE == node_type_t::INTERNAL) {
            ...
            laddr_t last_value;
            {
                ...
                stage_t::template get_largest_slot<true, false, true>(node_stage, &last_pos, nullptr, &p_last_value);
            ...
            node_stage_t::update_is_level_tail(mut, node_stage, true);
            auto p_last_value = const_cast<laddr_packed_t*>(node_stage.get_end_p_laddr());
