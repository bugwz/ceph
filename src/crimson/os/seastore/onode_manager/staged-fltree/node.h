...
struct node_version_t
{
    ...
    bool operator==(const node_version_t& rhs) const { return (layout == rhs.layout && state == rhs.state); }
    bool operator!=(const node_version_t& rhs) const { return !(*this == rhs); }
};
 ...
 */
class tree_cursor_t final : public boost::intrusive_ref_counter<tree_cursor_t, boost::thread_unsafe_counter>
{
public:
    ~tree_cursor_t();
    ...
    template<bool FORCE_MERGE = false> eagain_ifuture<Ref<tree_cursor_t>> erase(context_t, bool get_next);
...
private:
    ...
    // create from lookup
    tree_cursor_t(Ref<LeafNode>, const search_position_t&, const key_view_t&, const value_header_t*);
    ...
    template<bool VALIDATE> void update_track(Ref<LeafNode>, const search_position_t&);
    void update_cache_same_node(const key_view_t&, const value_header_t*) const;
    void invalidate();

    static Ref<tree_cursor_t> create_inserted(Ref<LeafNode> node, const search_position_t& pos)
    {
        return new tree_cursor_t(node, pos);
    }

    static Ref<tree_cursor_t> create_tracked(Ref<LeafNode> node, const search_position_t& pos, const key_view_t& key,
                                             const value_header_t* p_header)
    {
        return new tree_cursor_t(node, pos, key, p_header);
    }

    static Ref<tree_cursor_t> create_end(Ref<LeafNode> node) { return new tree_cursor_t(node); }
    ...
    public:
        ...
        const key_view_t& get_key_view(value_magic_t magic, const search_position_t& pos)
        {
            make_latest(magic, pos);
            return *key_view;
        }
        const value_header_t* get_p_value_header(value_magic_t magic, const search_position_t& pos)
        {
            make_latest(magic, pos);
            return p_value_header;
        }
        std::pair<NodeExtentMutable&, ValueDeltaRecorder*> prepare_mutate_value_payload(context_t,
 ...
 */
class Node : public boost::intrusive_ref_counter<Node, boost::thread_unsafe_counter>
{
public:
    // public to Btree
     ...
     */
    eagain_ifuture<std::pair<Ref<tree_cursor_t>, bool>> insert(context_t, const key_hobj_t&, value_config_t,
                                                               Ref<Node>&&);
...
protected:
    virtual eagain_ifuture<> test_clone_non_root(context_t, Ref<InternalNode>) const { ceph_abort("impossible path"); }
    virtual eagain_ifuture<search_result_t> lower_bound_tracked(context_t, const key_hobj_t&, MatchHistory&) = 0;
...
protected:
    ...
    // as child/non-root
    template<bool VALIDATE = true> void as_child(const search_position_t&, Ref<InternalNode>);
    ...
    template<bool FORCE_MERGE = false> eagain_ifuture<> try_merge_adjacent(context_t, bool, Ref<Node>&&);
    eagain_ifuture<> erase_node(context_t, Ref<Node>&&);
    template<bool FORCE_MERGE = false> eagain_ifuture<> fix_parent_index(context_t, Ref<Node>&&, bool);
...
public:
    ...
    template<bool VALIDATE> void do_track_child(Node& child)
    {
    ...
    eagain_ifuture<std::pair<Ref<Node>, Ref<Node>>> get_child_peers(context_t, const search_position_t&);

    eagain_ifuture<> erase_child(context_t, Ref<Node>&&);

    template<bool FORCE_MERGE = false> eagain_ifuture<> fix_index(context_t, Ref<Node>&&, bool);

    template<bool FORCE_MERGE = false>
    eagain_ifuture<> apply_children_merge(context_t, Ref<Node>&& left, laddr_t, Ref<Node>&& right, bool update_index);

    void validate_child_tracked(const Node& child) const
    {
        validate_child(child);
        assert(tracked_child_nodes.find(child.parent_info().position) != tracked_child_nodes.end());
    ...
    static eagain_ifuture<Ref<InternalNode>> allocate_root(context_t, laddr_t, level_t, laddr_t, Super::URef&&);
...
protected:
    ...
    bool is_tracking() const override { return !tracked_child_nodes.empty(); }
...
private:
    ...
    eagain_ifuture<Ref<InternalNode>> insert_or_split(context_t, const search_position_t&, const key_view_t&, Ref<Node>,
    ...
    void track_insert(const search_position_t&, match_stage_t, Ref<Node>, Ref<Node> nxt_child = nullptr);
    void replace_track(Ref<Node> new_child, Ref<Node> old_child, bool);
    void track_split(const search_position_t&, Ref<InternalNode>);
    template<bool VALIDATE = true> void track_erase(const search_position_t&, match_stage_t);
    ...
    struct fresh_node_t
    {
        ...
        std::pair<Ref<Node>, NodeExtentMutable> make_pair() { return std::make_pair(Ref<Node>(node), mut); }
    };
...
public:
    ...
    eagain_ifuture<Ref<tree_cursor_t>> erase(context_t, const search_position_t&, bool get_next);

    template<bool VALIDATE> void do_track_cursor(tree_cursor_t& cursor)
    {
        if constexpr (VALIDATE) {
...
protected:
    ...
    eagain_ifuture<search_result_t> lower_bound_tracked(context_t, const key_hobj_t&, MatchHistory&) override;
    eagain_ifuture<> do_get_tree_stats(context_t, tree_stats_t&) override;
    bool is_tracking() const override { return !tracked_cursors.empty(); }
    void track_merge(Ref<Node>, match_stage_t, search_position_t&) override;
...
private:
    ...
    eagain_ifuture<Ref<tree_cursor_t>> insert_value(context_t, const key_hobj_t&, value_config_t,
                                                    const search_position_t&, const MatchHistory&, match_stat_t mstat);
...
private:
    // XXX: extract a common tracker for InternalNode to track Node,
    // and LeafNode to track tree_cursor_t.
    Ref<tree_cursor_t> get_or_track_cursor(const search_position_t&, const key_view_t&, const value_header_t*);
