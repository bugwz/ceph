...
public:
    ...
    template<typename T> void copy_in_absolute(void* dst, const T& src) { copy_in_absolute(dst, &src, sizeof(T)); }

    const void* copy_in_relative(extent_len_t dst_offset, const void* src, extent_len_t len)
    {
        auto dst = get_write() + dst_offset;
        copy_in_absolute(dst, src, len);
        return dst;
    }
    template<typename T> const T* copy_in_relative(extent_len_t dst_offset, const T& src)
    {
    ...
    template<typename T> void validate_inplace_update(const T& updated) { assert(is_safe(&updated, sizeof(T))); }
    ...
    NodeExtentMutable get_mutable_relative(node_offset_t offset, node_offset_t len) const
    {
...
private:
    ...
    {
        return ((const char*)src >= p_start) && ((const char*)src + len <= p_start + length);
    }
