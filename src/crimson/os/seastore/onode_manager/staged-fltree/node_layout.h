...
public:
    ...
    static eagain_ifuture<typename parent_t::fresh_impl_t> allocate(context_t c, laddr_t hint, bool is_level_tail,
                                                                    level_t level)
    {
        ...
        return c.nm.alloc_extent(c.t, hint, extent_size)
            .handle_error_interruptible(
                ...
                crimson::ct_error::input_output_error::handle([FNAME, c, extent_size, is_level_tail, level] {
            ...
            .si_then([is_level_tail, level](auto extent) {
                ...
                node_stage_t::bootstrap_extent(mut, FIELD_TYPE, NODE_TYPE, is_level_tail, level);
                return typename parent_t::fresh_impl_t{std::unique_ptr<parent_t>(new NodeLayoutT(extent)), mut};
            });
    }
...
protected:
    ...
    bool has_single_value() const override
    {
        validate_non_empty();
        if constexpr (NODE_TYPE == node_type_t::INTERNAL) {
            return ((is_level_tail() && is_keys_empty()) || (!is_level_tail() && stage_t::is_keys_one(extent.read())));
        }
        else {
    ...
    std::optional<key_view_t> get_pivot_index() const override
    {
        ...
        stage_t::template get_largest_slot<false, true, false>(extent.read(), nullptr, &pivot_index, nullptr);
        return {pivot_index};
    }
    ...
    std::tuple<match_stage_t, search_position_t> erase(const search_position_t& pos) override
    {
    ...
    std::tuple<match_stage_t, std::size_t> evaluate_merge(NodeImpl& _right_node) override
    {
        ...
        if (right_node.is_keys_empty()) {
            ...
            std::tie(merge_stage, size_comp) = stage_t::evaluate_merge(left_pivot_index, right_node_stage);
        }
    ...
    search_position_t merge(NodeExtentMutable& mut, NodeImpl& _right_node, match_stage_t merge_stage,
                            extent_len_t merge_size) override
    {
        ...
        stage_t::template get_largest_slot<true, false, false>(left_node_stage, &left_last_pos, nullptr, nullptr);
        ...
        if (right_node.is_keys_empty()) {
            ...
            stage_t::template append_until<KeyT::VIEW>(right_append_at, left_appender, pos_end, STAGE);
    ...
    eagain_ifuture<NodeExtentMutable> rebuild_extent(context_t c) override
    {
        assert(!is_keys_empty());
        key_view_t first_index;
        stage_t::template get_slot<true, false>(extent.read(), position_t::begin(), &first_index, nullptr);
    ...
    eagain_ifuture<> retire_extent(context_t c) override { return extent.retire(c); }
    ...
    std::ostream& dump(std::ostream& os) const override
    {
        ...
        os << " num_kvs=" << stats.num_kvs << ", logical=" << stats.size_logical
           << "B, overhead=" << stats.size_overhead << "B, value=" << stats.size_value << "B";
        ...
        if constexpr (NODE_TYPE == node_type_t::INTERNAL) {
            if (node_stage.is_level_tail()) {
                ...
                os << "\n  tail value: 0x" << std::hex << value_ptr->value << std::dec << " " << size << "B"
                   << "  @" << offset << "B";
            }
        }
    ...
    void test_copy_to(NodeExtentMutable& to) const override { extent.test_copy_to(to); }
     ...
     * Common
     */
    void get_slot(const search_position_t& pos, key_view_t* p_index_key = nullptr,
                  const value_t** pp_value = nullptr) const override
    {
        assert(!is_keys_empty());
        assert(!pos.is_end());
        if (p_index_key && pp_value) {
            stage_t::template get_slot<true, true>(extent.read(), cast_down<STAGE>(pos), p_index_key, pp_value);
        }
        else if (!p_index_key && pp_value) {
            stage_t::template get_slot<false, true>(extent.read(), cast_down<STAGE>(pos), nullptr, pp_value);
        }
        else if (p_index_key && !pp_value) {
            stage_t::template get_slot<true, false>(extent.read(), cast_down<STAGE>(pos), p_index_key, nullptr);
        }
        else {
            ceph_abort("impossible path");
        }
#ifndef NDEBUG
        if (pp_value) {
            assert((const char*)(*pp_value) - extent.read().p_start() < extent.get_length());
        }
#endif
    }

    void get_prev_slot(search_position_t& pos, key_view_t* p_index_key = nullptr,
                       const value_t** pp_value = nullptr) const override
    {
        assert(!is_keys_empty());
        assert(!pos.is_end());
...
#endif
        if (!p_index_key && pp_value) {
            stage_t::template get_prev_slot<false, true>(extent.read(), _pos, nullptr, pp_value);
        }
        else {
...
#ifndef NDEBUG
        auto _nxt_pos = _pos;
        stage_t::template get_next_slot<false, false>(extent.read(), _nxt_pos, nullptr, nullptr);
        assert(nxt_pos == _nxt_pos);
#endif
    }

    void get_next_slot(search_position_t& pos, key_view_t* p_index_key = nullptr,
                       const value_t** pp_value = nullptr) const override
    {
        assert(!is_keys_empty());
        assert(!pos.is_end());
        ...
        if (p_index_key && pp_value) {
            ...
            find_next =
                stage_t::template get_next_slot<false, true>(extent.read(), cast_down<STAGE>(pos), nullptr, pp_value);
        }
        else {
            ceph_abort("not implemented");
        }
    ...
    void get_largest_slot(search_position_t* p_pos = nullptr, key_view_t* p_index_key = nullptr,
                          const value_t** pp_value = nullptr) const override
    {
        assert(!is_keys_empty());
        if (p_pos && p_index_key && pp_value) {
            ...
            stage_t::template get_largest_slot<false, true, false>(extent.read(), nullptr, p_index_key, nullptr);
        }
        else if (p_pos && !p_index_key && pp_value) {
    ...
    lookup_result_t<NODE_TYPE> lower_bound(const key_hobj_t& key, MatchHistory& history,
                                           key_view_t* index_key = nullptr, marker_t = {}) const override
    {
        ...
        if (index_key) {
            result_raw = stage_t::template lower_bound<true>(node_stage, key, history, index_key);
#ifndef NDEBUG
            if (!result_raw.is_end()) {
                key_view_t index;
                stage_t::template get_slot<true, false>(node_stage, result_raw.position, &index, nullptr);
        ...
        if constexpr (FIELD_TYPE == field_type_t::N0) {
            ...
            if constexpr (NODE_TYPE == node_type_t::INTERNAL) {
                if (result_raw.mstat == MSTAT_LT2) {
                    auto cmp = key <=> node_stage[result_raw.position.index].shard_pool;
    ...
    const value_t* insert(const full_key_t<KEY_TYPE>& key, const value_input_t& value, search_position_t& insert_pos,
                          match_stage_t& insert_stage, node_offset_t& insert_size) override
    {
    ...
    std::tuple<search_position_t, bool, const value_t*> split_insert(
        NodeExtentMutable& right_mut, NodeImpl& _right_impl, const full_key_t<KEY_TYPE>& key,
        const value_input_t& value, search_position_t& _insert_pos, match_stage_t& insert_stage,
        node_offset_t& insert_size) override
    {
        ...
        size_t target_split_size;
        {
            ...
            bool locate_nxt = stage_t::recursively_locate_split_inserted(
                split_size, 0, target_split_size, insert_pos, insert_stage, insert_size, _is_insert_left, split_at);
        ...
        if (!is_insert_left) {
            ...
            stage_t::template append_until<KEY_TYPE>(append_at, right_appender, insert_pos, insert_stage);
            SUBDEBUG(seastore_onode,
            ...
            [[maybe_unused]] bool is_end = stage_t::template append_insert<KEY_TYPE>(
                key, value, append_at, right_appender, is_front_insert, insert_stage, p_value);
        ...
        auto pos_end = position_t::end();
        stage_t::template append_until<KEY_TYPE>(append_at, right_appender, pos_end, STAGE);
        ...
        if (is_insert_left) {
            SUBDEBUG(
                seastore_onode, "-- left trim/insert at insert_pos({}), insert_stage={} ...", insert_pos, insert_stage);
        ...
        if (is_insert_left) {
            ...
            node_stage_t right_stage{reinterpret_cast<FieldType*>(right_mut.get_write()), right_mut.get_length()};
            stage_t::template get_largest_slot<true, false, false>(
    ...
    void replace_child_addr(const search_position_t& pos, laddr_t dst, laddr_t src) override
    {
        if constexpr (NODE_TYPE == node_type_t::INTERNAL) {
    ...
    std::tuple<match_stage_t, node_offset_t> evaluate_insert(const key_view_t& key, const laddr_t& value,
        ...
        if constexpr (NODE_TYPE == node_type_t::INTERNAL) {
            ...
            if (unlikely(is_keys_empty())) {
                ...
                std::tie(insert_stage, insert_size) =
                    stage_t::evaluate_insert(node_stage, key, value, cast_down<STAGE>(insert_pos), false);
            }
     ...
     * LeafNodeImpl
     */
    std::tuple<match_stage_t, node_offset_t> evaluate_insert(const key_hobj_t& key, const value_config_t& value,
                                                             ...
                                                             search_position_t& insert_pos) const override
    {
        if constexpr (NODE_TYPE == node_type_t::LEAF) {
            if (unlikely(is_keys_empty())) {
                ...
                return stage_t::evaluate_insert(key, value, history, mstat, cast_down<STAGE>(insert_pos));
            }
        }
        else {
            ceph_abort("impossible path");
    ...
    std::pair<NodeExtentMutable&, ValueDeltaRecorder*> prepare_mutate_value_payload(context_t c)
    {
        return extent.prepare_mutate_value_payload(c);
    }

private:
    NodeLayoutT(NodeExtentRef extent)
        : extent{extent}
    {
        build_name();
    }
    ...
    void build_name()
    {
        ...
        std::ostringstream sos;
        sos << "Node" << NODE_TYPE << FIELD_TYPE << "@0x" << std::hex << extent.get_laddr() << "+"
            << extent.get_length() << std::dec << "Lv" << (unsigned)level() << (is_level_tail() ? "$" : "");
        name = sos.str();
    }
