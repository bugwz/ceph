...
ceph::bufferlist& ValueDeltaRecorder::get_encoded(NodeExtentMutable& payload_mut)
{
...
Value::Value(NodeExtentManager& nm, const ValueBuilder& vb, Ref<tree_cursor_t>& p_cursor)
    : nm{nm}
    , vb{vb}
    , p_cursor{p_cursor}
{}
...
#ifndef NDEBUG
        .si_then([this, target_size] { assert(target_size == get_payload_size()); })
#endif
        ;
}
...
std::pair<NodeExtentMutable&, ValueDeltaRecorder*> Value::do_prepare_mutate_payload(Transaction& t)
{
...
std::unique_ptr<ValueDeltaRecorder> build_value_recorder_by_type(ceph::bufferlist& encoded, const value_magic_t& magic)
{
    ...
    switch (magic) {
    case value_magic_t::ONODE: ret = std::make_unique<FLTreeOnode::Recorder>(encoded); break;
    case value_magic_t::TEST_UNBOUND: ret = std::make_unique<UnboundedValue::Recorder>(encoded); break;
    case value_magic_t::TEST_BOUNDED: ret = std::make_unique<BoundedValue::Recorder>(encoded); break;
    case value_magic_t::TEST_EXTENDED: ret = std::make_unique<ExtendedValue::Recorder>(encoded); break;
...
#define NXT_T(StageType)   staged<typename StageType::next_param_t>
        ...
        unsigned internal_size_bound =
            sizeof(node_header_t) + (insert_size_2 + max_str_size) * 2 + (insert_size_2 - insert_size_0 + max_str_size);
