...
template<class ValueT = void> using btree_future = Btree::btree_future<ValueT>;
...
{
    return get_root(t)
        .safe_then([this, &t](auto root) { return root->lookup_smallest(get_context(t)); })
...
{
    return get_root(t)
        .safe_then([this, &t](auto root) { return root->lookup_largest(get_context(t)); })
        .safe_then([this](auto cursor) { return Cursor(this, cursor); });
}
...
btree_future<bool> Btree::contains(Transaction& t, const ghobject_t& obj)
{
    return seastar::do_with(full_key_t<KeyT::HOBJ>(obj), [this, &t](auto& key) -> btree_future<bool> {
        return get_root(t)
            .safe_then([this, &t, &key](auto root) {
                // TODO: improve lower_bound()
                return root->lower_bound(get_context(t), key);
            })
            .safe_then([](auto result) { return MatchKindBS::EQ == result.match(); });
    });
}

btree_future<Cursor> Btree::find(Transaction& t, const ghobject_t& obj)
{
    return seastar::do_with(full_key_t<KeyT::HOBJ>(obj), [this, &t](auto& key) -> btree_future<Cursor> {
        return get_root(t)
            .safe_then([this, &t, &key](auto root) {
                // TODO: improve lower_bound()
                return root->lower_bound(get_context(t), key);
...
btree_future<Cursor> Btree::lower_bound(Transaction& t, const ghobject_t& obj)
{
    return seastar::do_with(full_key_t<KeyT::HOBJ>(obj), [this, &t](auto& key) -> btree_future<Cursor> {
        return get_root(t)
            .safe_then([this, &t, &key](auto root) { return root->lower_bound(get_context(t), key); })
            .safe_then([this](auto result) { return Cursor(this, result.p_cursor); });
    });
}

btree_future<std::pair<Cursor, bool>> Btree::insert(Transaction& t, const ghobject_t& obj, const onode_t& value)
{
    return seastar::do_with(
        full_key_t<KeyT::HOBJ>(obj), [this, &t, &value](auto& key) -> btree_future<std::pair<Cursor, bool>> {
            return get_root(t)
                .safe_then([this, &t, &key, &value](auto root) { return root->insert(get_context(t), key, value); })
...
{
    // TODO
    return btree_ertr::make_ready_future<Cursor>(Cursor::make_end(this));
}

btree_future<Cursor> Btree::erase(Cursor& first, Cursor& last)
{
    // TODO
    return btree_ertr::make_ready_future<Cursor>(Cursor::make_end(this));
}
...
{
    return get_root(t).safe_then([](auto root) { return size_t(root->level() + 1); });
}
...
btree_future<tree_stats_t> Btree::get_stats_slow(Transaction& t)
{
    return get_root(t).safe_then([this, &t](auto root) {
        ...
        return root->get_tree_stats(get_context(t)).safe_then([height](auto stats) {
            stats.height = height;
...
btree_future<> Btree::test_clone_from(Transaction& t, Transaction& t_from, Btree& from)
{
    ...
    return from.get_root(t_from).safe_then(
