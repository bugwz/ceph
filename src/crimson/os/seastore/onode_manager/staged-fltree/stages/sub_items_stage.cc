...
const laddr_packed_t* internal_sub_items_t::insert_at(NodeExtentMutable& mut, const internal_sub_items_t& sub_items,
                                                      const Key& key, const laddr_t& value, index_t index,
                                                      node_offset_t size, const char* p_left_bound)
{
    ...
    const char* p_shift_end = reinterpret_cast<const char*>(sub_items.p_first_item + 1 - index);
    mut.shift_absolute(p_shift_start, p_shift_end - p_shift_start, -(int)size);

    auto p_insert = const_cast<char*>(p_shift_end) - size;
    auto item = internal_sub_item_t{snap_gen_t::from_key(key), laddr_packed_t{value}};
    mut.copy_in_absolute(p_insert, item);
    return &reinterpret_cast<internal_sub_item_t*>(p_insert)->value;
}
#define IA_TEMPLATE(Key)                                                                             \
    template const laddr_packed_t* internal_sub_items_t::insert_at<Key>(NodeExtentMutable&,          \
...
node_offset_t internal_sub_items_t::trim_until(NodeExtentMutable& mut, internal_sub_items_t& items, index_t index)
{
    assert(index != 0);
...
node_offset_t internal_sub_items_t::erase_at(NodeExtentMutable& mut, const internal_sub_items_t& sub_items,
                                             index_t index, const char* p_left_bound)
{
    assert(index < sub_items.keys());
    node_offset_t erase_size = sizeof(internal_sub_item_t);
    const char* p_shift_start = p_left_bound;
    const char* p_shift_end = reinterpret_cast<const char*>(sub_items.p_first_item - index);
    mut.shift_absolute(p_shift_start, p_shift_end - p_shift_start, erase_size);
    return erase_size;
}

template<KeyT KT>
void internal_sub_items_t::Appender<KT>::append(const internal_sub_items_t& src, index_t from, index_t items)
{
...
template<KeyT KT>
void internal_sub_items_t::Appender<KT>::append(const full_key_t<KT>& key, const laddr_t& value,
...
template<IsFullKey Key>
const value_header_t* leaf_sub_items_t::insert_at(NodeExtentMutable& mut, const leaf_sub_items_t& sub_items,
                                                  const Key& key, const value_config_t& value, index_t index,
                                                  node_offset_t size, const char* p_left_bound)
{
    assert(index <= sub_items.keys());
    assert(size == estimate_insert(key, value));
    ...
    const char* p_offset = (index == 0 ? (const char*)&sub_items.get_offset(0) + sizeof(node_offset_t)
    ...
    mut.copy_in_absolute(const_cast<char*>(p_shift_end) - sizeof(node_offset_t), offset_to_item_start);
...
template const value_header_t* leaf_sub_items_t::insert_at<key_hobj_t>(NodeExtentMutable&, const leaf_sub_items_t&,
                                                                       const key_hobj_t&, const value_config_t&,
                                                                       index_t, node_offset_t, const char*);

node_offset_t leaf_sub_items_t::trim_until(NodeExtentMutable& mut, leaf_sub_items_t& items, index_t index)
{
    assert(index != 0);
    auto keys = items.keys();
    assert(index <= keys);
    ...
    mut.shift_absolute(p_shift_start, p_shift_end - p_shift_start, size_trim_offsets);
...
node_offset_t leaf_sub_items_t::erase_at(NodeExtentMutable& mut, const leaf_sub_items_t& sub_items, index_t index,
...
template<KeyT KT> void leaf_sub_items_t::Appender<KT>::append(const leaf_sub_items_t& src, index_t from, index_t items)
{
    if (p_append) {
        // append from empty
        ...
        p_mut->shift_absolute(p_items_start, p_items_end - p_items_start, -(int)src_offsets_size);
...
template<KeyT KT> char* leaf_sub_items_t::Appender<KT>::wrap()
{
    if (op_dst.has_value()) {
        // append from existing
        assert(p_appended);
    ...
    for (auto i = 0u; i < cnt; ++i) {
        auto& a = appends[i];
        std::visit(overloaded{[&](const range_items_t& arg) { num_keys += arg.items; },
    ...
    for (auto i = 0u; i < cnt; ++i) {
        auto& a = appends[i];
        std::visit(overloaded{[&](const range_items_t& arg) {
