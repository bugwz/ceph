...
template<typename FieldType, node_type_t NODE_TYPE> memory_range_t NODE_T::get_nxt_container(index_t index) const
{
    if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
...
template<typename FieldType, node_type_t NODE_TYPE>
template<KeyT KT>
memory_range_t NODE_T::insert_prefix_at(NodeExtentMutable& mut, const node_extent_t& node, const full_key_t<KT>& key,
...
#define IPA_TEMPLATE(FT, NT, KT)                                     \
    template memory_range_t NODE_INST(FT, NT)::insert_prefix_at<KT>( \
        NodeExtentMutable&, const node_extent_t&, const full_key_t<KT>&, index_t, node_offset_t, const char*)
...
void APPEND_T::append(const node_extent_t& src, index_t from, index_t items)
{
    ...
    if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
        ...
        p_mut->copy_in_absolute(p_append_right, src.p_start() + offset_right_start, right_size);
    }
}

template<typename FieldType, node_type_t NODE_TYPE>
template<KeyT KT>
void APPEND_T::append(const full_key_t<KT>& key, const value_t& value, const value_t*& p_value)
{
