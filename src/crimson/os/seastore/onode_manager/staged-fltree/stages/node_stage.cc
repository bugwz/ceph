...
template<typename FieldType, node_type_t NODE_TYPE> const char* NODE_T::p_left_bound() const
{
    if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
        ...
        auto ret = p_start() + fields().get_item_end_offset(keys(), node_size);
...
template<typename FieldType, node_type_t NODE_TYPE> node_offset_t NODE_T::size_to_nxt_at(index_t index) const
{
    assert(index < keys());
    if constexpr (FIELD_TYPE == field_type_t::N0 || FIELD_TYPE == field_type_t::N1) {
        return FieldType::estimate_insert_one();
    }
    else if constexpr (FIELD_TYPE == field_type_t::N2) {
        auto p_end = p_start() + p_fields->get_item_end_offset(index, node_size);
...
template<typename FieldType, node_type_t NODE_TYPE> container_range_t NODE_T::get_nxt_container(index_t index) const
{
    if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
        ceph_abort("N3 internal node doesn't have the right part");
    }
    else {
        auto item_start_offset = p_fields->get_item_start_offset(index, node_size);
        auto item_end_offset = p_fields->get_item_end_offset(index, node_size);
...
void NODE_T::bootstrap_extent(NodeExtentMutable& mut, field_type_t field_type, node_type_t node_type,
                              bool is_level_tail, level_t level)
{
    node_header_t::bootstrap_extent(mut, field_type, node_type, is_level_tail, level);
    mut.copy_in_relative(sizeof(node_header_t), typename FieldType::num_keys_t(0u));
}

template<typename FieldType, node_type_t NODE_TYPE>
void NODE_T::update_is_level_tail(NodeExtentMutable& mut, const node_extent_t& extent, bool value)
{
...
memory_range_t NODE_T::insert_prefix_at(NodeExtentMutable& mut, const node_extent_t& node, const Key& key,
                                        index_t index, node_offset_t size, const char* p_left_bound)
{
    ...
    if constexpr (FIELD_TYPE == field_type_t::N0 || FIELD_TYPE == field_type_t::N1) {
        ...
        const char* p_insert = node.p_start() + node.fields().get_item_end_offset(index, mut.get_length());
        const char* p_insert_front = p_insert - size_right;
        FieldType::insert_at(mut, key, node.fields(), index, size_right);
        mut.shift_absolute(p_left_bound, p_insert - p_left_bound, -(int)size_right);
...
#define IPA_TEMPLATE(FT, NT, Key)                                     \
    template memory_range_t NODE_INST(FT, NT)::insert_prefix_at<Key>( \
        NodeExtentMutable&, const node_extent_t&, const Key&, index_t, node_offset_t, const char*)
...
template<typename FieldType, node_type_t NODE_TYPE>
void NODE_T::update_size_at(NodeExtentMutable& mut, const node_extent_t& node, index_t index, int change)
{
    assert(mut.get_length() == node.node_size);
    assert(mut.get_read() == node.p_start());
    assert(index < node.keys());
...
template<typename FieldType, node_type_t NODE_TYPE>
node_offset_t NODE_T::trim_until(NodeExtentMutable& mut, const node_extent_t& node, index_t index)
{
    assert(mut.get_length() == node.node_size);
    assert(mut.get_read() == node.p_start());
    ...
    if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
        ...
        mut.copy_in_absolute((void*)&node.p_fields->num_keys, num_keys_t(index));
    }
...
template<typename FieldType, node_type_t NODE_TYPE>
node_offset_t NODE_T::trim_at(NodeExtentMutable& mut, const node_extent_t& node, index_t index, node_offset_t trimmed)
{
    assert(mut.get_length() == node.node_size);
    assert(mut.get_read() == node.p_start());
    assert(!node.is_level_tail());
    ...
    if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
        ...
        node_offset_t offset = node.p_fields->get_item_start_offset(index, node_size);
        size_t new_offset = offset + trimmed;
        assert(new_offset < node.p_fields->get_item_end_offset(index, node_size));
        mut.copy_in_absolute(const_cast<void*>(node.p_fields->p_offset(index)), node_offset_t(new_offset));
        mut.copy_in_absolute((void*)&node.p_fields->num_keys, num_keys_t(index + 1));
    }
    // no need to calculate trim size for node
    return 0;
}
...
template<typename FieldType, node_type_t NODE_TYPE>
node_offset_t NODE_T::erase_at(NodeExtentMutable& mut, const node_extent_t& node, index_t index,
    ...
    , p_start{p_mut->get_write()}
{
    ...
    if (open) {
        // seek as open_nxt()
        if constexpr (FIELD_TYPE == field_type_t::N0 || FIELD_TYPE == field_type_t::N1) {
            p_append_left = p_start + node.fields().get_key_start_offset(node.keys() - 1, node_size);
            p_append_left += sizeof(typename FieldType::key_t);
            p_append_right = p_start + node.fields().get_item_end_offset(node.keys() - 1, node_size);
        }
        else if constexpr (FIELD_TYPE == field_type_t::N2) {
            ceph_abort("not implemented");
        }
        ...
        if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
            ...
            p_append_left = p_start + node.fields().get_key_start_offset(node.keys(), node_size);
            p_append_right = p_start + node.fields().get_item_end_offset(node.keys(), node_size);
        }
        num_keys = node.keys();
    }
}
...
void APPEND_T::append(const node_extent_t& src, index_t from, index_t items)
{
    ...
    if constexpr (std::is_same_v<FieldType, internal_fields_3_t>) {
        ...
        node_offset_t offset_left_start = src.fields().get_key_start_offset(from, node_size);
        node_offset_t offset_left_end = src.fields().get_key_start_offset(from + items, node_size);
        node_offset_t left_size = offset_left_end - offset_left_start;
        if (num_keys == 0) {
            // no need to adjust offset
            assert(from == 0);
            assert(p_start + offset_left_start == p_append_left);
            p_mut->copy_in_absolute(p_append_left, src.p_start() + offset_left_start, left_size);
        }
        else {
            ...
            for (auto i = from; i < from + items; ++i) {
                int new_offset = src.fields().get_item_start_offset(i, node_size) + offset_change;
                assert(new_offset > 0);
                assert(new_offset < (int)node_size);
                p_mut->copy_in_absolute(p_offset_dst, node_offset_t(new_offset));
                p_offset_dst += step_size;
            }
            assert(p_append_left + left_size + sizeof(typename FieldType::key_t) == p_offset_dst);
        }
        p_append_left += left_size;

        // append right part backwards
        auto offset_right_start = src.fields().get_item_end_offset(from + items, node_size);
        auto offset_right_end = src.fields().get_item_end_offset(from, node_size);
        int right_size = offset_right_end - offset_right_start;
        assert(right_size > 0);
        assert(right_size < (int)node_size);
        p_append_right -= right_size;
        p_mut->copy_in_absolute(p_append_right, src.p_start() + offset_right_start, node_offset_t(right_size));
    }
}

template<typename FieldType, node_type_t NODE_TYPE>
template<KeyT KT>
void APPEND_T::append(const full_key_t<KT>& key, const value_input_t& value, const value_t*& p_value)
{
...
template<typename FieldType, node_type_t NODE_TYPE>
template<KeyT KT>
std::tuple<NodeExtentMutable*, char*> APPEND_T::open_nxt(const key_get_type& partial_key)
{
    if constexpr (FIELD_TYPE == field_type_t::N0 || FIELD_TYPE == field_type_t::N1) {
...
template<typename FieldType, node_type_t NODE_TYPE>
template<KeyT KT>
std::tuple<NodeExtentMutable*, char*> APPEND_T::open_nxt(const full_key_t<KT>& key)
{
    if constexpr (FIELD_TYPE == field_type_t::N0 || FIELD_TYPE == field_type_t::N1) {
...
template<typename FieldType, node_type_t NODE_TYPE> template<KeyT KT> char* APPEND_T::wrap()
{
