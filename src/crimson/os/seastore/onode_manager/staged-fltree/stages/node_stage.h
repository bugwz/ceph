 ...
 */
template<typename FieldType, node_type_t _NODE_TYPE> class node_extent_t
{
public:
    ...
    node_offset_t free_size() const { return p_fields->template free_size_before<NODE_TYPE>(keys(), node_size); }
    extent_len_t total_size() const { return p_fields->total_size(node_size); }
    const char* p_left_bound() const;
    template<node_type_t T = NODE_TYPE>
    std::enable_if_t<T == node_type_t::INTERNAL, const laddr_packed_t*> get_end_p_laddr() const
    {
        ...
        if constexpr (FIELD_TYPE == field_type_t::N3) {
            ...
            auto offset_start = p_fields->get_item_end_offset(keys(), node_size);
    ...
    key_get_type operator[](index_t index) const { return p_fields->get_key(index, node_size); }
    extent_len_t size_before(index_t index) const
    {
        auto free_size = p_fields->template free_size_before<NODE_TYPE>(index, node_size);
        assert(total_size() >= free_size);
        return total_size() - free_size;
    }
    node_offset_t size_to_nxt_at(index_t index) const;
    node_offset_t size_overhead_at(index_t index) const { return FieldType::ITEM_OVERHEAD; }
    container_range_t get_nxt_container(index_t index) const;

    template<typename T = FieldType>
    std::enable_if_t<T::FIELD_TYPE == field_type_t::N3, const value_t*> get_p_value(index_t index) const
    {
        assert(index < keys());
    ...
    static node_extent_t decode(const char* p_node_start, extent_len_t node_size,
                                ceph::bufferlist::const_iterator& delta)
    {
        // nothing to decode
        return node_extent_t(reinterpret_cast<const FieldType*>(p_node_start), node_size);
    }
    ...
    template<IsFullKey Key> static node_offset_t estimate_insert(const Key& key, const value_input_t& value)
    {
        ...
        else if constexpr (FIELD_TYPE == field_type_t::N3 && NODE_TYPE == node_type_t::LEAF) {
    ...
    static const value_t* insert_at(NodeExtentMutable& mut, const node_extent_t&, const Key& key,
                                    const value_input_t& value, index_t index, node_offset_t size,
    ...
    template<IsFullKey Key>
    static memory_range_t insert_prefix_at(NodeExtentMutable&, const node_extent_t&, const Key& key, index_t index,
                                           node_offset_t size, const char* p_left_bound);

    static void update_size_at(NodeExtentMutable&, const node_extent_t&, index_t index, int change);

    static node_offset_t trim_until(NodeExtentMutable&, const node_extent_t&, index_t index);
    static node_offset_t trim_at(NodeExtentMutable&, const node_extent_t&, index_t index, node_offset_t trimmed);

    static node_offset_t erase_at(NodeExtentMutable&, const node_extent_t&, index_t index, const char* p_left_bound);
