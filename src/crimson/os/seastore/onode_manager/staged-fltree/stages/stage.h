...
search_result_bs_t binary_search(const key_hobj_t& key, index_t begin, index_t end, FGetKey&& f_get_key)
{
    assert(begin <= end);
    while (begin < end) {
...
search_result_bs_t binary_search_r(index_t rend, index_t rbegin, FGet&& f_get, const PivotType& key)
{
    assert(rend <= rbegin);
    while (rend < rbegin) {
...
inline void assert_mstat(const key_hobj_t& key, const key_view_t& index, match_stat_t mstat)
{
    assert(mstat >= MSTAT_MIN && mstat <= MSTAT_LT2);
    // key < index ...
    switch (mstat) {
    case MSTAT_EQ: break;
    case MSTAT_LT0: assert(key < index.snap_gen_packed()); break;
    case MSTAT_LT1: assert(key < index.ns_oid_view()); break;
    case MSTAT_LT2:
        if (index.has_shard_pool()) {
            assert((key < shard_pool_crush_t{index.shard_pool_packed(), index.crush_packed()}));
        }
        else {
            assert(key < index.crush_packed());
        }
        break;
    default: ceph_abort("impossible path");
    }
    // key == index ...
    switch (mstat) {
    case MSTAT_EQ: assert(key == index.snap_gen_packed());
    case MSTAT_LT0:
        if (!index.has_ns_oid()) break;
        assert(index.ns_oid_view().type() == ns_oid_view_t::Type::MAX || key == index.ns_oid_view());
    case MSTAT_LT1:
        if (!index.has_crush()) break;
        assert(key == index.crush_packed());
        if (!index.has_shard_pool()) break;
 ...
 */
template<typename Params> struct staged
{
    ...
    static void _left_or_right(index_t& split_index, index_t insert_index, std::optional<bool>& is_insert_left)
    {
    ...
    template<ContainerType CTYPE> class _iterator_t<CTYPE, std::enable_if_t<CTYPE == ContainerType::INDEXABLE>>
    {
        /*
    ...
    public:
        ...
        _iterator_t(const container_t& container)
            : container{container}
        {
        ...
        template<typename T = value_t> std::enable_if_t<IS_BOTTOM, const T*> get_p_value() const
        {
            assert(!is_end());
            return container.get_p_value(_index);
        }
        bool is_last() const { return _index + 1 == container.keys(); }
        ...
        node_offset_t size() const
        {
            ...
            return container.size_before(_index + 1) - container.size_before(_index);
        }
        ...
        MatchKindBS seek(const key_hobj_t& key, bool exclude_last)
        {
            ...
            auto ret = binary_search(key, _index, end_index, [this](index_t index) { return container[index]; });
            _index = ret.index;
            return ret.match;
        }

        template<IsFullKey Key, typename T = value_t>
        std::enable_if_t<IS_BOTTOM, const T*> insert(NodeExtentMutable& mut, const Key& key, const value_input_t& value,
                                                     node_offset_t insert_size, const char* p_left_bound)
        {
            return container_t::insert_at(mut, container, key, value, _index, insert_size, p_left_bound);
        }

        template<IsFullKey Key, typename T = memory_range_t>
        std::enable_if_t<!IS_BOTTOM, T> insert_prefix(NodeExtentMutable& mut, const Key& key, node_offset_t size,
                                                      const char* p_left_bound)
        {
            return container_t::insert_prefix_at(mut, container, key, _index, size, p_left_bound);
        }

        template<typename T = void> std::enable_if_t<!IS_BOTTOM, T> update_size(NodeExtentMutable& mut, int insert_size)
        {
            assert(!is_end());
        ...
        template<bool is_exclusive>
        size_t seek_split_inserted(size_t start_size, size_t extra_size, size_t target_size, index_t& insert_index,
                                   size_t insert_size, std::optional<bool>& is_insert_left)
        {
            assert(!is_end());
            assert(index() == 0);
            ...
            auto f_get_used_size = [this, start_size, start_size_1, insert_index, insert_size](index_t index) {
                size_t current_size;
                if (unlikely(index == 0)) {
        ...
        size_t seek_split(size_t start_size, size_t extra_size, size_t target_size)
        {
            ...
            _index = binary_search_r(0, container.keys() - 1, f_get_used_size, target_size).index;
            size_t current_size = f_get_used_size(_index);
            assert(current_size <= target_size);
            return current_size;
        }
        ...
        template<KeyT KT> void copy_out_until(typename container_t::template Appender<KT>& appender, index_t& to_index)
        {
        ...
        node_offset_t trim_until(NodeExtentMutable& mut) { return container_t::trim_until(mut, container, _index); }

        template<typename T = node_offset_t>
        std::enable_if_t<!IS_BOTTOM, T> trim_at(NodeExtentMutable& mut, node_offset_t trimmed)
        {
        ...
        template<KeyT KT> typename container_t::template Appender<KT> get_appender(NodeExtentMutable* p_mut)
        {
            assert(_index + 1 == container.keys());
            return typename container_t::template Appender<KT>(p_mut, container);
        }

        template<KeyT KT> typename container_t::template Appender<KT> get_appender_opened(NodeExtentMutable* p_mut)
        {
        ...
        static me_t decode(const char* p_node_start, extent_len_t node_size, ceph::bufferlist::const_iterator& delta)
        {
            auto container = container_t::decode(p_node_start, node_size, delta);
        ...
        static node_offset_t header_size() { return container_t::header_size(); }

        template<IsFullKey Key> static node_offset_t estimate_insert(const Key& key, const value_input_t& value)
        {
    ...
    template<ContainerType CTYPE> class _iterator_t<CTYPE, std::enable_if_t<CTYPE == ContainerType::ITERATIVE>>
    {
        /*
    ...
    public:
        ...
        memory_range_t insert_prefix(NodeExtentMutable& mut, const Key& key, node_offset_t size,
                                     const char* p_left_bound)
        {
            return container_t::insert_prefix(mut, container, key, is_end(), size, p_left_bound);
        }
    ...
    static bool is_keys_one(const container_t& container)
    {   // IN
    ...
    template<bool GET_KEY> static result_t smallest_result(const iterator_t& iter, key_view_t* p_index_key)
    {
        ...
        NXT_STAGE_T::template get_slot<GET_KEY, true>(nxt_container, pos_smallest, p_index_key, &p_value);
    ...
    static result_t nxt_lower_bound(const key_hobj_t& key, iterator_t& iter, MatchHistory& history,
                                    key_view_t* index_key)
    {
        static_assert(!IS_BOTTOM);
        assert(!iter.is_end());
        auto nxt_container = iter.get_nxt_container();
        auto nxt_result = NXT_STAGE_T::template lower_bound<GET_KEY>(nxt_container, key, history, index_key);
    ...
    static void get_largest_slot(const container_t& container,   // IN
                                 position_t* p_position,         // OUT
                                 key_view_t* p_index_key,        // OUT
                                 const value_t** pp_value)
    {   // OUT
        auto iter = iterator_t(container);
        iter.seek_last();
        if constexpr (GET_KEY) {
    ...
    static void get_slot(const container_t& container,   // IN
    ...
    {   // OUT
        ...
        if constexpr (!IS_BOTTOM) {
            auto nxt_container = iter.get_nxt_container();
            NXT_STAGE_T::template get_slot<GET_KEY, GET_VAL>(nxt_container, pos.nxt, p_index_key, pp_value);
        }
        else {
            if constexpr (GET_VAL) {
                assert(pp_value);
    ...
    static result_t lower_bound(const container_t& container, const key_hobj_t& key, MatchHistory& history,
                                key_view_t* index_key = nullptr)
    {
        ...
        if (history.get<STAGE>().has_value()) {
            if (*history.get<STAGE>() == MatchKindCMP::EQ) {
                ...
                if constexpr (IS_BOTTOM) {
                    ...
                    auto nxt_container = iter.get_nxt_container();
                    auto nxt_result =
                        NXT_STAGE_T::template lower_bound<GET_KEY>(nxt_container, key, history, index_key);
        ...
        history.set<STAGE>(bs_match == MatchKindBS::EQ ? MatchKindCMP::EQ : MatchKindCMP::LT);
        if constexpr (IS_BOTTOM) {
            if constexpr (GET_KEY) {
                index_key->set(iter.get_key());
            }
            auto value_ptr = iter.get_p_value();
            return result_t{{iter.index()}, value_ptr, (bs_match == MatchKindBS::EQ ? MSTAT_EQ : MSTAT_LT0)};
        }
        else {
    ...
    template<IsFullKey Key> static node_offset_t insert_size(const Key& key, const value_input_t& value)
    {
        if constexpr (IS_BOTTOM) {
            return iterator_t::estimate_insert(key, value);
        }
        else {
            return iterator_t::estimate_insert(key, value) + NXT_STAGE_T::iterator_t::header_size() +
                   NXT_STAGE_T::insert_size(key, value);
        }
    }

    template<IsFullKey Key>
    static node_offset_t insert_size_at(match_stage_t stage, const Key& key, const value_input_t& value)
    {
        if (stage == STAGE) {
    ...
    static std::enable_if_t<NODE_TYPE == node_type_t::INTERNAL, T> evaluate_insert(const container_t& container,
                                                                                   const key_view_t& key,
    ...
    static std::enable_if_t<NODE_TYPE == node_type_t::LEAF, T> compensate_insert_position_at(match_stage_t stage,
                                                                                             position_t& position)
    {
        auto& index = position.index;
        if (stage == STAGE) {
            ...
            if constexpr (IS_BOTTOM) {
                ...
                assert(stage < STAGE);
                bool compensate = NXT_STAGE_T::compensate_insert_position_at(stage, position.nxt);
                if (compensate) {
                    assert(is_valid_index(index));
                    if (index == 0) {
    ...
    template<typename T = std::tuple<match_stage_t, node_offset_t>>
    static std::enable_if_t<NODE_TYPE == node_type_t::LEAF, T> evaluate_insert(const key_hobj_t& key,
                                                                               const value_config_t& value,
                                                                               const MatchHistory& history,
                                                                               match_stat_t mstat, position_t& position)
    {
    ...
    template<KeyT KT>
    static const value_t* insert_new(NodeExtentMutable& mut, const memory_range_t& range, const full_key_t<KT>& key,
                                     const value_input_t& value)
    {
    ...
    static const value_t* proceed_insert_recursively(NodeExtentMutable& mut, const container_t& container,
                                                     ...
                                                     node_offset_t& _insert_size, const char* p_left_bound)
    {
        ...
        if (do_insert) {
            ...
            if constexpr (IS_BOTTOM) {
                return iter.insert(mut, key, value, _insert_size, p_left_bound);
            }
            else {
                auto range = iter.insert_prefix(mut, key, _insert_size, p_left_bound);
            ...
            if constexpr (!IS_BOTTOM) {
                ...
                auto p_value = NXT_STAGE_T::template proceed_insert_recursively<KT, SPLIT>(
                    mut, nxt_container, key, value, position.nxt, stage, _insert_size, p_left_bound);
    ...
    template<KeyT KT, bool SPLIT>
    static const value_t* proceed_insert(NodeExtentMutable& mut, const container_t& container,
                                         const full_key_t<KT>& key, const value_input_t& value, position_t& position,
                                         match_stage_t& stage, node_offset_t& _insert_size)
    {
        ...
        if (unlikely(!container.keys())) {
            ...
            if constexpr (IS_BOTTOM) {
                return container_t::insert_at(mut, container, key, value, 0, _insert_size, p_left_bound);
            }
            else {
                auto range = container_t::template insert_prefix_at(mut, container, key, 0, _insert_size, p_left_bound);
                return NXT_STAGE_T::template insert_new<KT>(mut, range, key, value);
            }
        }
        else {
            return proceed_insert_recursively<KT, SPLIT>(
                mut, container, key, value, position, stage, _insert_size, p_left_bound);
        }
    }

    static std::ostream& dump(const container_t& container, std::ostream& os, const std::string& prefix, size_t& size,
                              const char* p_start)
    {
        auto iter = iterator_t(container);
        assert(!iter.is_end());
    ...
    static void get_stats(const container_t& container, node_stats_t& stats, key_view_t& index_key)
    {
        auto iter = iterator_t(container);
        assert(!iter.is_end());
    ...
    template<bool GET_KEY, bool GET_VAL>
    static bool get_next_slot(const container_t& container,   // IN
    ...
    {   // OUT
        ...
        if constexpr (!IS_BOTTOM) {
            auto nxt_container = iter.get_nxt_container();
            find_next =
                NXT_STAGE_T::template get_next_slot<GET_KEY, GET_VAL>(nxt_container, pos.nxt, p_index_key, pp_value);
        }
        else {
            find_next = true;
        }
        ...
        if (find_next) {
            if (iter.is_last()) {
                ...
                get_slot<GET_KEY, GET_VAL>(container, pos, p_index_key, pp_value);
                return false;
            }
        }
    ...
    template<bool GET_KEY, bool GET_VAL>
    static void get_prev_slot(const container_t& container,   // IN
                              position_t& pos,                // IN&OUT
                              key_view_t* p_index_key,        // OUT
                              const value_t** pp_value)
    {   // OUT
        ...
        if constexpr (!IS_BOTTOM) {
            ...
            if (nxt_pos == NXT_STAGE_T::position_t::begin()) {
                ...
                iter.seek_at(index);
                auto nxt_container = iter.get_nxt_container();
                NXT_STAGE_T::template get_prev_slot<GET_KEY, GET_VAL>(nxt_container, nxt_pos, p_index_key, pp_value);
            }
        }
        else {
            assert(index);
    ...
    class StagedIterator : std::conditional_t<IS_BOTTOM, _BaseEmpty, _BaseWithNxtIterator>
    {
    public:
        StagedIterator() = default;
        bool valid() const { return iter.has_value(); }
        index_t index() const { return iter->index(); }
        ...
        template<typename OutputIt> auto do_format_to(OutputIt out, bool is_top) const
        {
            if (valid()) {
                if (iter->is_end()) {
        ...
        static StagedIterator decode(const char* p_node_start, extent_len_t node_size,
                                     ceph::bufferlist::const_iterator& delta)
        {
            ...
            if (present) {
                ret.iter = iterator_t::decode(p_node_start, node_size, delta);
                if constexpr (!IS_BOTTOM) {
                    ret._nxt = NXT_STAGE_T::StagedIterator::decode(p_node_start, node_size, delta);
                }
            }
            return ret;
        }
    ...
    static bool recursively_locate_split(size_t& current_size, size_t extra_size, size_t target_size,
                                         StagedIterator& split_at)
    {
        ...
        if constexpr (!IS_BOTTOM) {
            locate_nxt = NXT_STAGE_T::recursively_locate_split(
                current_size, extra_size + split_iter.size_to_nxt(), target_size, split_at.nxt());
        }
        else {   // IS_BOTTOM
    ...
    static bool recursively_locate_split_inserted(size_t& current_size, size_t extra_size, size_t target_size,
                                                  position_t& insert_pos, match_stage_t insert_stage,
                                                  size_t insert_size, std::optional<bool>& is_insert_left,
                                                  StagedIterator& split_at)
    {
        assert(current_size <= target_size);
        ...
        if (insert_stage == STAGE) {
            current_size = split_iter.template seek_split_inserted<true>(
                current_size, extra_size, target_size, insert_index, insert_size, is_insert_left);
            assert(is_insert_left.has_value());
            assert(current_size <= target_size);
            if (split_iter.index() == 0) {
                if (insert_index == 0) {
            ...
            if constexpr (!IS_BOTTOM) {
                ...
                if (!is_insert_left.has_value()) {
                    ...
                    locate_nxt = NXT_STAGE_T::recursively_locate_split_inserted(current_size,
                                                                                extra_size + split_iter.size_to_nxt(),
                                                                                target_size,
                                                                                insert_pos.nxt,
                                                                                insert_stage,
                                                                                insert_size,
     ...
     */
    template<KeyT KT> struct _BaseWithNxtAppender
    {
        typename NXT_STAGE_T::template StagedAppender<KT> _nxt;
    };
    template<KeyT KT> class StagedAppender : std::conditional_t<IS_BOTTOM, _BaseEmpty, _BaseWithNxtAppender<KT>>
    {
    public:
        ...
        void init_tail(NodeExtentMutable* p_mut, const container_t& container, match_stage_t stage)
        {
            assert(!valid());
            auto iter = iterator_t(container);
            iter.seek_last();
        ...
        void append(const full_key_t<KT>& key, const value_input_t& value, const value_t*& p_value)
        {
            assert(!require_wrap_nxt);
            if constexpr (!IS_BOTTOM) {
        ...
        typename NXT_STAGE_T::template StagedAppender<KT>& open_nxt(key_get_type paritial_key)
        {
            assert(!require_wrap_nxt);
            if constexpr (!IS_BOTTOM) {
                require_wrap_nxt = true;
        ...
        typename NXT_STAGE_T::template StagedAppender<KT>& open_nxt(const full_key_t<KT>& key)
        {
            assert(!require_wrap_nxt);
            if constexpr (!IS_BOTTOM) {
                require_wrap_nxt = true;
    ...
    template<KeyT KT>
    static void _append_range(StagedIterator& src_iter, StagedAppender<KT>& appender, index_t& to_index)
    {
        if (src_iter.is_end()) {
            // append done
            ...
            if (appender.in_progress()) {
                ...
                NXT_STAGE_T::template _append_range<KT>(src_iter.nxt(), appender.get_nxt(), to_index_nxt);
                ++src_iter;
                appender.wrap_nxt();
            }
    ...
    template<KeyT KT>
    static void _append_into(StagedIterator& src_iter, StagedAppender<KT>& appender, position_t& position,
                             match_stage_t stage)
    {
        ...
        if (stage == STAGE) {
            ...
            NXT_STAGE_T::template append_until<KT>(
                src_iter.nxt(), appender.open_nxt(src_iter.get_key()), position.nxt, stage);
        }
    }

    template<KeyT KT>
    static void append_until(StagedIterator& src_iter, StagedAppender<KT>& appender, position_t& position,
                             match_stage_t stage)
    {
    ...
    template<KeyT KT>
    static bool append_insert(const full_key_t<KT>& key, const value_input_t& value, StagedIterator& src_iter,
                              StagedAppender<KT>& appender, bool is_front_insert, match_stage_t& stage,
                              const value_t*& p_value)
    {
        ...
        if (stage == STAGE) {
            ...
            if constexpr (!IS_BOTTOM) {
                auto nxt_is_end = NXT_STAGE_T::template append_insert<KT>(
                    key, value, src_iter.get_nxt(), appender.get_nxt(), is_front_insert, stage, p_value);
                if (nxt_is_end) {
                    appender.wrap_nxt();
                    ++src_iter;
                    if (is_front_insert) {
     ...
     */
    static std::tuple<TrimType, node_offset_t> recursively_trim(NodeExtentMutable& mut, StagedIterator& trim_at)
    {
        if (!trim_at.valid()) {
        ...
        if constexpr (!IS_BOTTOM) {
            auto [type, trimmed] = NXT_STAGE_T::recursively_trim(mut, trim_at.get_nxt());
    ...
    static std::optional<std::tuple<match_stage_t, node_offset_t, bool>> proceed_erase_recursively(
    ...
    {   // IN&OUT
        ...
        if constexpr (!IS_BOTTOM) {
            auto nxt_container = iter.get_nxt_container();
            auto ret = NXT_STAGE_T::proceed_erase_recursively(mut, nxt_container, p_left_bound, pos.nxt);
            if (ret.has_value()) {
    ...
    static match_stage_t erase(NodeExtentMutable& mut,
                               const container_t& node_stage,   // IN
                               position_t& erase_pos)
    {   // IN&OUT
        auto p_left_bound = node_stage.p_left_bound();
        auto ret = proceed_erase_recursively(mut, node_stage, p_left_bound, erase_pos);
        if (ret.has_value()) {
            auto [r_stage, r_erase_size, r_done] = *ret;
    ...
    static std::tuple<match_stage_t, node_offset_t> evaluate_merge(const key_view_t& left_pivot_index,
...
template<node_type_t _NODE_TYPE> struct staged_params_subitems
{
...
template<node_type_t _NODE_TYPE> struct staged_params_item_iterator
{
...
template<typename NodeType> struct staged_params_node_01
{
...
template<typename NodeType> struct staged_params_node_2
{
    using container_t = NodeType;
    static constexpr auto NODE_TYPE = NodeType::NODE_TYPE;
    static constexpr auto STAGE = STAGE_STRING;
...
template<typename NodeType> struct staged_params_node_3
{
    using container_t = NodeType;
    static constexpr auto NODE_TYPE = NodeType::NODE_TYPE;
    static constexpr auto STAGE = STAGE_RIGHT;
...
struct _node_to_stage_t<
    NodeType, std::enable_if_t<NodeType::FIELD_TYPE == field_type_t::N0 || NodeType::FIELD_TYPE == field_type_t::N1>>
{
    using type = staged<staged_params_node_01<NodeType>>;
};
template<typename NodeType>
struct _node_to_stage_t<NodeType, std::enable_if_t<NodeType::FIELD_TYPE == field_type_t::N2>>
{
    using type = staged<staged_params_node_2<NodeType>>;
};
template<typename NodeType>
struct _node_to_stage_t<NodeType, std::enable_if_t<NodeType::FIELD_TYPE == field_type_t::N3>>
{
    using type = staged<staged_params_node_3<NodeType>>;
};
template<typename NodeType> using node_to_stage_t = typename _node_to_stage_t<NodeType>::type;
...
concept HasDoFormatTo = requires(T x, std::back_insert_iterator<fmt::memory_buffer> out) {
    {
        x.do_format_to(out, true)
    } -> std::same_as<decltype(out)>;
};
template<HasDoFormatTo T> struct fmt::formatter<T> : fmt::formatter<std::string_view>
{
    template<typename FormatContext> auto format(const T& staged_iterator, FormatContext& ctx)
    {
