...
memory_range_t ITER_T::insert_prefix(NodeExtentMutable& mut, const ITER_T& iter, const Key& key, bool is_end,
...
#define IP_TEMPLATE(NT, Key)                                   \
    template memory_range_t ITER_INST(NT)::insert_prefix<Key>( \
        NodeExtentMutable&, const ITER_INST(NT)&, const Key&, bool, node_offset_t, const char*)
...
template<node_type_t NODE_TYPE> void ITER_T::update_size(NodeExtentMutable& mut, const ITER_T& iter, int change)
{
    node_offset_t offset = iter.get_back_offset();
    int new_size = change + offset;
    assert(new_size > 0 && new_size < (int)mut.get_length());
    mut.copy_in_absolute((void*)iter.get_item_range().p_end, node_offset_t(new_size));
}

template<node_type_t NODE_TYPE> node_offset_t ITER_T::trim_until(NodeExtentMutable& mut, const ITER_T& iter)
{
    assert(iter.index() != 0);
...
template<node_type_t NODE_TYPE>
node_offset_t ITER_T::trim_at(NodeExtentMutable& mut, const ITER_T& iter, node_offset_t trimmed)
{
...
template<node_type_t NODE_TYPE>
node_offset_t ITER_T::erase(NodeExtentMutable& mut, const ITER_T& iter, const char* p_left_bound)
{
...
template<node_type_t NODE_TYPE>
template<KeyT KT>
APPEND_T::Appender(NodeExtentMutable* p_mut, const item_iterator_t& iter, bool open)
    : p_mut{p_mut}
{
    assert(!iter.has_next());
    if (open) {
...
template<node_type_t NODE_TYPE> template<KeyT KT> bool APPEND_T::append(const ITER_T& src, index_t& items)
{
...
template<node_type_t NODE_TYPE> template<KeyT KT> void APPEND_T::wrap_nxt(char* _p_append)
{
    assert(_p_append < p_append);
    p_mut->copy_in_absolute(p_offset_while_open, node_offset_t(p_offset_while_open - _p_append));
    p_append = _p_append;
}
