...
struct node_header_t
{
    ...
    {
        if (field_type >= FIELD_TYPE_MAGIC && field_type < static_cast<uint8_t>(field_type_t::_MAX)) {
    ...
    node_type_t get_node_type() const { return static_cast<node_type_t>(node_type); }
    bool get_is_level_tail() const { return is_level_tail; }

    static void bootstrap_extent(NodeExtentMutable&, field_type_t, node_type_t, bool, level_t);
    ...
    static_assert(sizeof(bits_t) * 8 == FIELD_TYPE_BITS + NODE_TYPE_BITS + B_LEVEL_TAIL_BITS);
    level_t level;

private:
    void set_field_type(field_type_t type) { field_type = static_cast<uint8_t>(type); }
    void set_node_type(node_type_t type) { node_type = static_cast<uint8_t>(type); }
    void set_is_level_tail(bool value) { is_level_tail = static_cast<uint8_t>(value); }
} __attribute__((packed));
inline std::ostream& operator<<(std::ostream& os, const node_header_t& header)
{
    auto field_type = header.get_field_type();
    if (field_type.has_value()) {
        os << "header" << header.get_node_type() << *field_type << "(is_level_tail=" << header.get_is_level_tail()
...
template<typename FixedKeyType, field_type_t _FIELD_TYPE> struct _slot_t
{
...
template<typename FieldType> const char* fields_start(const FieldType& node)
{
    return reinterpret_cast<const char*>(&node);
}

template<node_type_t NODE_TYPE, typename FieldType>
node_range_t fields_free_range_before(const FieldType& node, index_t index, extent_len_t node_size)
{
 ...
 *                           +--------------------------------------------+
 */
template<typename SlotType> struct _node_fields_013_t
{
    ...
    static constexpr node_offset_t HEADER_SIZE = sizeof(node_header_t) + sizeof(num_keys_t);
    static constexpr node_offset_t ITEM_OVERHEAD = SlotType::OVERHEAD;

    bool is_level_tail() const { return header.get_is_level_tail(); }
    extent_len_t total_size(extent_len_t node_size) const { return node_size; }
    key_get_type get_key(index_t index, extent_len_t node_size) const
    {
        assert(index < num_keys);
        return slots[index].key;
    }
    node_offset_t get_key_start_offset(index_t index, extent_len_t node_size) const
    {
        assert(index <= num_keys);
    ...
    node_offset_t get_item_start_offset(index_t index, extent_len_t node_size) const
    {
        assert(index < num_keys);
    ...
    extent_len_t get_item_end_offset(index_t index, extent_len_t node_size) const
    {
        return index == 0 ? node_size : get_item_start_offset(index - 1, node_size);
    }
    template<node_type_t NODE_TYPE> node_offset_t free_size_before(index_t index, extent_len_t node_size) const
    {
    ...
    static void insert_at(NodeExtentMutable&, const Key& key, const me_t& node, index_t index,
                          node_offset_t size_right);
    static node_offset_t erase_at(NodeExtentMutable&, const me_t&, index_t, const char*);
    static void update_size_at(NodeExtentMutable&, const me_t& node, index_t index, int change);
    static void append_key(NodeExtentMutable&, const key_t& key, char*& p_append);
    template<IsFullKey Key> static void append_key(NodeExtentMutable& mut, const Key& key, char*& p_append)
    {
        append_key(mut, key_t::from_key(key), p_append);
    }
    static void append_offset(NodeExtentMutable& mut, node_offset_t offset_to_right, char*& p_append);
...
struct node_fields_2_t
{
    ...
    template<IsFullKey Key>
    static void insert_at(NodeExtentMutable& mut, const Key& key, const node_fields_2_t& node, index_t index,
                          node_offset_t size_right)
    {
        ceph_abort("not implemented");
    }
    static void update_size_at(NodeExtentMutable& mut, const node_fields_2_t& node, index_t index, int change)
    {
        ceph_abort("not implemented");
    }
    static void append_key(NodeExtentMutable& mut, const key_t& key, char*& p_append)
    {
...
struct internal_fields_3_t
{
    ...
    static constexpr node_offset_t HEADER_SIZE = sizeof(node_header_t) + sizeof(num_keys_t);
    static constexpr node_offset_t ITEM_SIZE = sizeof(snap_gen_t) + sizeof(laddr_t);
    ...
    std::enable_if_t<NODE_TYPE == node_type_t::INTERNAL, node_offset_t> free_size_before(index_t index,
                                                                                         extent_len_t node_size) const
    {
        assert(index <= num_keys);
        assert(num_keys <= get_max_num_keys(node_size));
        extent_len_t free = total_size(node_size) - HEADER_SIZE - index * ITEM_SIZE;
    ...
    const laddr_packed_t* get_p_child_addr(index_t index, extent_len_t node_size) const
    {
#ifndef NDEBUG
        if (is_level_tail()) {
            assert(index <= num_keys);
...
#endif
        auto p_addrs = reinterpret_cast<const laddr_packed_t*>(&keys[get_num_keys_limit(node_size)]);
    ...
    template<IsFullKey Key>
    static void insert_at(NodeExtentMutable& mut, const Key& key, const internal_fields_3_t& node, index_t index,
                          node_offset_t size_right)
    {
        ceph_abort("not implemented");
    }
    static void update_size_at(NodeExtentMutable& mut, const internal_fields_3_t& node, index_t index, int change)
    {
        ceph_abort("not implemented");
    }
...
private:
    ...
    static num_keys_t get_num_keys_limit(extent_len_t node_size) { return (node_size - HEADER_SIZE) / ITEM_SIZE; }
} __attribute__((packed));
