...
public:
    ...
    {
        auto p_end = reinterpret_cast<const char*>(p_first_item) + sizeof(internal_sub_item_t);
    ...
    static internal_sub_items_t decode(const char* p_node_start, extent_len_t node_size,
                                       ceph::bufferlist::const_iterator& delta)
    {
        ...
        return internal_sub_items_t(
            {{p_node_start + start_offset, p_node_start + start_offset + stage_size}, node_size});
    }

    static node_offset_t header_size() { return 0u; }

    template<IsFullKey Key> static node_offset_t estimate_insert(const Key&, const laddr_t&)
    {
        return sizeof(internal_sub_item_t);
    }

    template<IsFullKey Key>
    static const laddr_packed_t* insert_at(NodeExtentMutable&, const internal_sub_items_t&, const Key&, const laddr_t&,
                                           index_t index, node_offset_t size, const char* p_left_bound);

    static node_offset_t trim_until(NodeExtentMutable&, internal_sub_items_t&, index_t);

    static node_offset_t erase_at(NodeExtentMutable&, const internal_sub_items_t&, index_t, const char*);

    template<KeyT KT> class Appender;
...
template<KeyT KT> class internal_sub_items_t::Appender
{
public:
...
public:
    ...
    {
        return (p_num_keys == x.p_num_keys && p_offsets == x.p_offsets && p_items_end == x.p_items_end);
    }
    ...
    const char* get_item_start(index_t index) const { return p_items_end - get_offset(index).value; }

    const char* get_item_end(index_t index) const { return p_items_end - get_offset_to_end(index); }
    ...
    node_offset_t size_before(index_t index) const
    {
        ...
        if (index == 0) {
            ...
            --index;
            ret = sizeof(num_keys_t) + (index + 1) * sizeof(node_offset_t) + get_offset(index).value;
        }
        assert(ret < node_size);
        return ret;
    }
    ...
    const value_header_t* get_p_value(index_t index) const
    {
        ...
        assert(pointer + value->allocation_size() + sizeof(snap_gen_t) == get_item_end(index));
        return value;
    }
    void encode(const char* p_node_start, ceph::bufferlist& encoded) const
    {
    ...
    static leaf_sub_items_t decode(const char* p_node_start, extent_len_t node_size,
                                   ceph::bufferlist::const_iterator& delta)
    {
        node_offset_t start_offset;
        ceph::decode(start_offset, delta);
        ...
        assert(stage_size > 0);
        assert((unsigned)start_offset + stage_size < node_size);
        return leaf_sub_items_t({{p_node_start + start_offset, p_node_start + start_offset + stage_size}, node_size});
    }

    static node_offset_t header_size() { return sizeof(num_keys_t); }

    template<IsFullKey Key> static node_offset_t estimate_insert(const Key&, const value_config_t& value)
    {
        return value.allocation_size() + sizeof(snap_gen_t) + sizeof(node_offset_t);
    }

    template<IsFullKey Key>
    static const value_header_t* insert_at(NodeExtentMutable&, const leaf_sub_items_t&, const Key&,
                                           const value_config_t&, index_t index, node_offset_t size,
                                           const char* p_left_bound);

    static node_offset_t trim_until(NodeExtentMutable&, leaf_sub_items_t&, index_t index);

    static node_offset_t erase_at(NodeExtentMutable&, const leaf_sub_items_t&, index_t, const char*);
...
template<KeyT KT> class leaf_sub_items_t::Appender
{
...
public:
    ...
    void append(const full_key_t<KT>& key, const value_config_t& value, const value_header_t*& p_value)
    {
        // append from empty
        assert(p_append);
...
template<> struct _sub_items_t<node_type_t::INTERNAL>
{
    using type = internal_sub_items_t;
};
template<> struct _sub_items_t<node_type_t::LEAF>
{
    using type = leaf_sub_items_t;
};
template<node_type_t NODE_TYPE> using sub_items_t = typename _sub_items_t<NODE_TYPE>::type;
