...
template<> struct _full_key_type<KeyT::VIEW>
{
    using type = key_view_t;
};
template<> struct _full_key_type<KeyT::HOBJ>
{
    using type = key_hobj_t;
};
template<KeyT type> using full_key_t = typename _full_key_type<type>::type;
...
static laddr_t get_lba_hint(shard_t shard, pool_t pool, crush_hash_t crush)
{
    ...
    // conflicts.
    return ((uint64_t)(shard & 0XFF) << 56 | (uint64_t)(pool & 0xFF) << 48 | (uint64_t)(crush) << 16);
}
...
struct string_key_view_t
{
    ...
    static bool is_valid_size(size_t size) { return size <= VALID_UPPER_BOUND; }
    ...
    {
        if (type() == x.type() && type() != Type::STR) return true;
    ...
    void reset_to(const char* origin_base, const char* new_base, extent_len_t node_size)
    {
    ...
    static void append_str(NodeExtentMutable&, std::string_view, char*& p_append);
    ...
    static void append_dedup(NodeExtentMutable&, const Type& dedup_type, char*& p_append);
...
public:
    ...
    {
        if (get_type() == x.get_type() && get_type() != Type::STR) return true;
        if (get_type() != x.get_type()) return false;
    ...
    static string_view_masked_t decode(std::string& str_storage, ceph::bufferlist::const_iterator& delta)
    {
        string_size_t size;
    ...
    if (type == Type::MIN) {
        ...
        if (view.length() <= 12) {
            ...
            os << "\"" << std::string_view(view.data(), 4) << ".."
               << std::string_view(view.data() + view.length() - 2, 2) << "/" << view.length() << "B\"";
        }
        return os;
    }
}
...
struct ns_oid_view_t
{
    ...
    ns_oid_view_t(const char* p_end)
        : nspace(p_end)
        , oid(nspace.p_next_end())
    {}
    ...
    template<typename Key> static node_offset_t estimate_size(const Key& key);

    template<typename Key> static void append(NodeExtentMutable&, const Key& key, char*& p_append);

    static void append(NodeExtentMutable& mut, const ns_oid_view_t& view, char*& p_append)
    {
    ...
    template<typename Key> static void test_append(const Key& key, char*& p_append);
...
{
    return os << string_view_masked_t{ns_oid.nspace} << "," << string_view_masked_t{ns_oid.oid};
}
...
{
    auto ret = ghobject_t(shard_id_t(MAX_SHARD), MAX_POOL, MAX_CRUSH, "MAX", "MAX", MAX_SNAP, MAX_GEN);
...
template<typename Key> bool is_valid_key(const Key& key);
...
public:
     ...
     */
    shard_t shard() const { return ghobj.shard_id; }
    pool_t pool() const { return ghobj.hobj.pool; }
    crush_hash_t crush() const
    {
        // Note: this is the reversed version of the object hash
        return ghobj.hobj.get_bitwise_key_u32();
    }
    laddr_t get_hint() const { return get_lba_hint(shard(), pool(), crush()); }
    ...
    ns_oid_view_t::Type dedup_type() const { return _dedup_type; }
    snap_t snap() const { return ghobj.hobj.snap; }
    gen_t gen() const { return ghobj.generation; }

    std::ostream& dump(std::ostream& os) const
    {
        os << "key_hobj(" << (int)shard() << "," << pool() << ",0x" << std::hex << crush() << std::dec << "; "
           << string_view_masked_t{nspace()} << "," << string_view_masked_t{oid()} << "; " << snap() << "," << gen()
           << ")";
        return os;
    }

    bool is_valid() const { return is_valid_key(*this); }
    ...
    static key_hobj_t decode(ceph::bufferlist::const_iterator& delta)
    {
        ...
        return key_hobj_t(ghobject_t(shard_id_t(shard), pool, crush, nspace, oid, snap, gen));
    }
...
public:
    ...
    ns_oid_view_t::Type dedup_type() const { return ns_oid_view().type(); }
     ...
     */
    bool has_shard_pool() const { return p_shard_pool != nullptr; }
    bool has_crush() const { return p_crush != nullptr; }
    bool has_ns_oid() const { return p_ns_oid.has_value(); }
    bool has_snap_gen() const { return p_snap_gen != nullptr; }
    ...
    {
        return sizeof(shard_t) + sizeof(pool_t) + sizeof(crush_hash_t) + sizeof(snap_t) + sizeof(gen_t) +
               ns_oid_view().size_logical();
    }
    ...
    ghobject_t to_ghobj() const
    {
        ...
        return ghobject_t(
            shard_id_t(shard()), pool(), crush(), std::string(nspace()), std::string(oid()), snap(), gen());
    }
...
struct shard_pool_t
{
    ...
    template<IsFullKey Key> static shard_pool_t from_key(const Key& key)
    {
...
struct crush_t
{
    ...
    template<IsFullKey Key> static crush_t from_key(const Key& key)
    {
        if constexpr (std::same_as<Key, key_view_t>) {
...
struct shard_pool_crush_t
{
    ...
    template<IsFullKey Key> static shard_pool_crush_t from_key(const Key& key)
    {
...
struct snap_gen_t
{
    ...
    template<IsFullKey Key> static snap_gen_t from_key(const Key& key)
    {
        if constexpr (std::same_as<Key, key_view_t>) {
...
template<IsFullKey Key> void encode_key(const Key& key, ceph::bufferlist& bl)
{
...
template<IsFullKey LHS, IsFullKey RHS> std::strong_ordering operator<=>(const LHS& lhs, const RHS& rhs) noexcept
{
...
template<typename Key> bool is_valid_key(const Key& key)
{
    ...
    return (key > key_hobj_t(ghobject_t()) && key < key_hobj_t(ghobject_t::get_max()));
}
...
template<IsFullKey T> auto operator<=>(const T& key, const shard_pool_t& target)
{
...
template<IsFullKey T> auto operator<=>(const T& key, const crush_t& target)
{
    return key.crush() <=> target.crush;
}

template<IsFullKey T> auto operator<=>(const T& key, const shard_pool_crush_t& target)
{
...
template<IsFullKey T> auto operator<=>(const T& key, const ns_oid_view_t& target)
{
...
template<IsFullKey T> auto operator<=>(const T& key, const snap_gen_t& target)
{
...
template<IsFullKey LHS, typename RHS> bool operator==(LHS lhs, RHS rhs)
{
    return lhs <=> rhs == 0;
}

template<typename Key> node_offset_t ns_oid_view_t::estimate_size(const Key& key)
{
    static_assert(IsFullKey<Key>);
    if constexpr (std::same_as<Key, key_view_t>) {
...
template<typename Key> void ns_oid_view_t::append(NodeExtentMutable& mut, const Key& key, char*& p_append)
{
    static_assert(IsFullKey<Key>);
    if (key.dedup_type() == Type::STR) {
...
template<typename Key> void ns_oid_view_t::test_append(const Key& key, char*& p_append)
{
    static_assert(IsFullKey<Key>);
    if (key.dedup_type() == Type::STR) {
