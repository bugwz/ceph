...
void node_header_t::bootstrap_extent(NodeExtentMutable& mut, field_type_t field_type, node_type_t node_type,
...
void node_header_t::update_is_level_tail(NodeExtentMutable& mut, const node_header_t& header, bool value)
{
...
void F013_T::update_size_at(NodeExtentMutable& mut, const me_t& node, index_t index, int change)
{
    ...
    if (change < 0 && index != node.num_keys) {
        assert(node.get_item_start_offset(index, node_size) < node.get_item_end_offset(index, node_size));
    }
#endif
    for (const auto* p_slot = &node.slots[index]; p_slot < &node.slots[node.num_keys]; ++p_slot) {
        node_offset_t offset = p_slot->right_offset;
        int new_offset = offset - change;
        assert(new_offset > 0);
        assert(new_offset < (int)node_size);
        mut.copy_in_absolute((void*)&(p_slot->right_offset), node_offset_t(new_offset));
    }
#ifndef NDEBUG
    // check overflow
    if (change > 0 && index != node.num_keys) {
        assert(node.num_keys > 0);
        assert(node.get_key_start_offset(node.num_keys, node_size) <= node.slots[node.num_keys - 1].right_offset);
    }
#endif
}

template<typename SlotType> void F013_T::append_key(NodeExtentMutable& mut, const key_t& key, char*& p_append)
{
...
template<typename SlotType>
void F013_T::append_offset(NodeExtentMutable& mut, node_offset_t offset_to_right, char*& p_append)
{
...
void F013_T::insert_at(NodeExtentMutable& mut, const Key& key, const me_t& node, index_t index,
                       node_offset_t size_right)
{
    assert(index <= node.num_keys);
    extent_len_t node_size = mut.get_length();
    update_size_at(mut, node, index, size_right);
    auto p_insert = const_cast<char*>(fields_start(node)) + node.get_key_start_offset(index, node_size);
    auto p_shift_end = fields_start(node) + node.get_key_start_offset(node.num_keys, node_size);
...
#define IA_TEMPLATE(ST, KT)                     \
    template void F013_INST(ST)::insert_at<KT>( \
        NodeExtentMutable&, const KT&, const F013_INST(ST)&, index_t, node_offset_t)
...
template<typename SlotType>
node_offset_t F013_T::erase_at(NodeExtentMutable& mut, const me_t& node, index_t index, const char* p_left_bound)
{
    extent_len_t node_size = mut.get_length();
    ...
    const char* p_shift_start = fields_start(node) + node.get_key_start_offset(index + 1, node_size);
...
void node_fields_2_t::append_offset(NodeExtentMutable& mut, node_offset_t offset_to_right, char*& p_append)
{
    mut.copy_in_absolute(p_append, offset_to_right);
    p_append += sizeof(node_offset_t);
}
