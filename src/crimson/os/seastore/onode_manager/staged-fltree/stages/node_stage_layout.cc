...
void F013_T::update_size_at(NodeExtentMutable& mut, const me_t& node, index_t index, int change)
{
    ...
    for (const auto* p_slot = &node.slots[index]; p_slot < &node.slots[node.num_keys]; ++p_slot) {
        ...
        mut.copy_in_absolute((void*)&(p_slot->right_offset), node_offset_t(offset - change));
    }
}
...
template<typename SlotType>
template<KeyT KT>
void F013_T::insert_at(NodeExtentMutable& mut, const full_key_t<KT>& key, const me_t& node, index_t index,
                       node_offset_t size_right)
{
    assert(index <= node.num_keys);
    update_size_at(mut, node, index, size_right);
    auto p_insert = const_cast<char*>(fields_start(node)) + node.get_key_start_offset(index);
...
#define IA_TEMPLATE(ST, KT)                     \
    template void F013_INST(ST)::insert_at<KT>( \
        NodeExtentMutable&, const full_key_t<KT>&, const F013_INST(ST)&, index_t, node_offset_t)
