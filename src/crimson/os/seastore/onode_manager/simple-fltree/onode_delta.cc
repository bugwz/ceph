...
delta_t delta_t::insert_child(unsigned slot, const ghobject_t& oid, crimson::os::seastore::laddr_t addr)
{
...
delta_t delta_t::insert_front(ceph::buffer::ptr keys, ceph::buffer::ptr cells)
{
...
delta_t delta_t::insert_back(ceph::buffer::ptr keys, ceph::buffer::ptr cells)
{
...
void delta_t::encode(ceph::bufferlist& bl)
{
    using ceph::encode;
    switch (op) {
    case op_t::insert_onode: [[fallthrough]];
    case op_t::update_onode:
        ...
        encode(*onode, bl);
        break;
    case op_t::insert_child: encode(oid, bl); encode(addr, bl);
    case op_t::update_key:
        encode(n, bl);
        encode(oid, bl);
        break;
    case op_t::shift_left: encode(n, bl); break;
    case op_t::trim_right: encode(n, bl); break;
    case op_t::insert_front: [[fallthrough]];
    ...
    default: assert(0 == "unknown onode op");
    }
}
...
void delta_t::decode(ceph::bufferlist::const_iterator& p)
{
    ...
    case op_t::update_onode:
        ...
        decode(*onode, p);
        break;
    case op_t::insert_child: [[fallthrough]];
    case op_t::update_key:
        decode(n, p);
        decode(oid, p);
        break;
    case op_t::shift_left: decode(n, p); break;
    case op_t::trim_right: decode(n, p); break;
    case op_t::insert_front: [[fallthrough]];
    case op_t::insert_back:
        decode(n, p);
        decode(keys, p);
        decode(cells, p);
        break;
    case op_t::remove_from: decode(n, p); break;
    default: assert(0 == "unknown onode op");
    }
}
