...
auto node_t<BlockSize, N, NodeType>::key_at(unsigned slot) const -> std::pair<const key_prefix_t&, const key_suffix_t&>
{
    auto& key = keys[slot];
...
template<size_t BlockSize, int N, ntype_t NodeType>
ghobject_t node_t<BlockSize, N, NodeType>::get_oid_at(unsigned slot, const ghobject_t& oid) const
{
...
template<size_t BlockSize, int N, ntype_t NodeType>
auto node_t<BlockSize, N, NodeType>::item_at(const key_prefix_t& key) const -> const_item_t
{
    if constexpr (item_in_key) {
...
template<size_t BlockSize, int N, ntype_t NodeType> void node_t<BlockSize, N, NodeType>::dump(std::ostream& os) const
{
...
template<size_t BlockSize, int N, ntype_t NodeType> char* node_t<BlockSize, N, NodeType>::from_end(uint16_t offset)
{
...
template<size_t BlockSize, int N, ntype_t NodeType> uint16_t node_t<BlockSize, N, NodeType>::used_space() const
{
    if constexpr (item_in_key) {
...
template<size_t BlockSize, int N, ntype_t NodeType> uint16_t node_t<BlockSize, N, NodeType>::capacity()
{
    auto p = reinterpret_cast<node_t*>(0);
    return BlockSize - (reinterpret_cast<char*>(p->keys) - reinterpret_cast<char*>(p));
}

// TODO: if it's allowed to update 2 siblings at the same time, we can have
//       B* tree
template<size_t BlockSize, int N, ntype_t NodeType> constexpr uint16_t node_t<BlockSize, N, NodeType>::min_size()
{
    return capacity() / 2;
}

template<size_t BlockSize, int N, ntype_t NodeType>
constexpr std::pair<int16_t, int16_t> node_t<BlockSize, N, NodeType>::bytes_to_add(uint16_t size)
{
...
template<size_t BlockSize, int N, ntype_t NodeType>
constexpr std::pair<int16_t, int16_t> node_t<BlockSize, N, NodeType>::bytes_to_remove(uint16_t size)
{
...
bool node_t<BlockSize, N, NodeType>::is_underflow(uint16_t size) const
{
    ...
    case size_state_t::underflow: return true;
    case size_state_t::okay: return false;
    default: assert(0); return false;
    }
}

template<size_t BlockSize, int N, ntype_t NodeType>
int16_t node_t<BlockSize, N, NodeType>::size_with_key(unsigned slot, const ghobject_t& oid) const
{
    if constexpr (item_in_key) {
        return capacity();
    }
...
template<size_t BlockSize, int N, ntype_t NodeType>
ordering_t node_t<BlockSize, N, NodeType>::compare_with_slot(unsigned slot, const ghobject_t& oid) const
{
...
std::pair<unsigned, bool> node_t<BlockSize, N, NodeType>::lower_bound(const ghobject_t& oid) const
{
    ...
    while (s != e) {
        ...
        case ordering_t::less: s = ++mid; break;
        case ordering_t::greater: e = mid; break;
        case ordering_t::equivalent: assert(mid == 0 || mid < count); return {mid, true};
        }
    }
    return {s, false};
}
...
template<size_t BlockSize, int N, ntype_t NodeType>
uint16_t node_t<BlockSize, N, NodeType>::size_of_item(const ghobject_t& oid, const item_t& item)
{
    if constexpr (item_in_key) {
        return sizeof(key_prefix_t);
    }
    else {
        return (sizeof(key_prefix_t) + key_suffix_t::size_from(oid) + size_of(item));
    }
}

template<size_t BlockSize, int N, ntype_t NodeType>
bool node_t<BlockSize, N, NodeType>::is_overflow(const ghobject_t& oid, const item_t& item) const
{
    return free_space() < size_of_item(oid, item);
}

template<size_t BlockSize, int N, ntype_t NodeType>
bool node_t<BlockSize, N, NodeType>::is_overflow(const ghobject_t& oid, const OnodeRef& item) const
{
...
template<size_t BlockSize, int N, ntype_t NodeType>
void node_t<BlockSize, N, NodeType>::insert_at(unsigned slot, const ghobject_t& oid, const item_t& item)
{
...
template<size_t BlockSize, int N, ntype_t NodeType>
std::pair<unsigned, uint16_t> node_t<BlockSize, N, NodeType>::calc_grab_front(uint16_t min_grab,
...
template<size_t BlockSize, int N, ntype_t NodeType>
std::pair<unsigned, uint16_t> node_t<BlockSize, N, NodeType>::calc_grab_back(uint16_t min_grab, uint16_t max_grab) const
{
    // TODO: split by likeness
    uint16_t grabbed = 0;
    uint16_t used = used_space();
...
void node_t<BlockSize, N, NodeType>::grab_from_left(node_t<BlockSize, LeftN, NodeType>& left, unsigned n,
                                                    uint16_t bytes, Mover& mover)
{
...
delta_t node_t<BlockSize, N, NodeType>::acquire_right(node_t<BlockSize, RightN, NodeType>& right, unsigned whoami,
                                                      Mover& mover)
{
...
template<size_t BlockSize, int N, ntype_t NodeType>
template<int RightN, class Mover>
void node_t<BlockSize, N, NodeType>::grab_from_right(node_t<BlockSize, RightN, NodeType>& right, unsigned n,
                                                     uint16_t bytes, Mover& mover)
{
...
template<size_t BlockSize, int N, ntype_t NodeType>
template<int LeftN, class Mover>
void node_t<BlockSize, N, NodeType>::push_to_left(node_t<BlockSize, LeftN, NodeType>& left, unsigned n, uint16_t bytes,
                                                  Mover& mover)
{
...
template<size_t BlockSize, int N, ntype_t NodeType>
template<int RightN, class Mover>
void node_t<BlockSize, N, NodeType>::push_to_right(node_t<BlockSize, RightN, NodeType>& right, unsigned n,
                                                   uint16_t bytes, Mover& mover)
{
...
void node_t<BlockSize, N, NodeType>::shift_left(unsigned from, unsigned to)
{
    ...
    if constexpr (item_in_key) {
        ...
        for (auto src_key = keys + from, dst_key = keys + to; src_key != keys + count; ++src_key, ++dst_key) {
            // shift the keys left
            *dst_key = *src_key;
            // update the pointers
...
template<size_t BlockSize, int N, ntype_t NodeType>
void node_t<BlockSize, N, NodeType>::insert_front(const ceph::bufferptr& keys_buf, const ceph::bufferptr& cells_buf)
{
...
template<size_t BlockSize, int N, ntype_t NodeType>
void node_t<BlockSize, N, NodeType>::insert_back(const ceph::bufferptr& keys_buf, const ceph::bufferptr& cells_buf)
{
...
template<size_t BlockSize, int N, ntype_t NodeType> void node_t<BlockSize, N, NodeType>::remove_from(unsigned slot)
{
    assert(slot < count);
...
template<size_t BlockSize, int N, ntype_t NodeType> void node_t<BlockSize, N, NodeType>::trim_right(unsigned n)
{
    count = n;
}
...
void node_t<BlockSize, N, NodeType>::play_delta(const delta_t& delta)
{
    ...
    case delta_t::op_t::shift_left: return shift_left(delta.n, 0);
    case delta_t::op_t::trim_right: return trim_right(delta.n);
    case delta_t::op_t::insert_front: return insert_front(delta.keys, delta.cells);
    case delta_t::op_t::insert_back: return insert_back(delta.keys, delta.cells);
    case delta_t::op_t::remove_from: return remove_from(delta.n);
    default: assert(0 == "unknown onode delta");
    }
}
