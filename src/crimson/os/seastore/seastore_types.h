...
constexpr device_id_t DEVICE_ID_RANDOM_BLOCK_MIN = 1 << (std::numeric_limits<device_id_t>::digits - 1);
...
{
    return id == DEVICE_ID_RECORD_RELATIVE || id == DEVICE_ID_BLOCK_RELATIVE || id == DEVICE_ID_DELAYED ||
           id == DEVICE_ID_FAKE || id == DEVICE_ID_ROOT;
}
...
public:
    ...
    [[gnu::always_inline]] constexpr device_id_t device_id() const
    {
        return static_cast<device_id_t>(segment >> DEVICE_SEGMENT_ID_BITS);
    }

    [[gnu::always_inline]] constexpr device_segment_id_t device_segment_id() const
    {
        constexpr internal_segment_id_t _SEGMENT_ID_MASK = (1u << DEVICE_SEGMENT_ID_BITS) - 1;
        return segment & _SEGMENT_ID_MASK;
    }

    bool operator==(const segment_id_t& other) const { return segment == other.segment; }
    bool operator!=(const segment_id_t& other) const { return segment != other.segment; }
    bool operator<(const segment_id_t& other) const { return segment < other.segment; }
    bool operator<=(const segment_id_t& other) const { return segment <= other.segment; }
    bool operator>(const segment_id_t& other) const { return segment > other.segment; }
    ...
    DENC(segment_id_t, v, p) { denc(v.segment, p); }

    static constexpr segment_id_t create_const(device_id_t id, device_segment_id_t segment)
    {
...
private:
    ...
    constexpr static inline internal_segment_id_t make_internal(device_id_t d_id, device_segment_id_t s_id)
    {
...
struct __attribute((packed)) segment_id_le_t
{
    ...
    operator segment_id_t() const { return segment_id_t(segment); }
};

constexpr segment_id_t MIN_SEG_ID = segment_id_t::create_const(0, 0);
// segment_id_t() == MAX_SEG_ID == NULL_SEG_ID
constexpr segment_id_t MAX_SEG_ID = segment_id_t::create_const(DEVICE_ID_MAX_VALID_SEGMENT, DEVICE_SEGMENT_ID_MAX);
...
static constexpr segment_seq_t MAX_SEG_SEQ = std::numeric_limits<segment_seq_t>::max();
 ...
 */
template<typename T> class segment_map_t
{
public:
    segment_map_t()
    {
    ...
    auto device_end(device_id_t id) { return iterator<false>::lower_bound(*this, id + 1, 0); }

    size_t size() const { return total_segments; }

private:
    template<bool is_const = false> class iterator
    {
        /// points at set being iterated over
        std::conditional_t<is_const, const segment_map_t&, segment_map_t&> parent;
        ...
        std::optional<std::pair<const segment_id_t, std::conditional_t<is_const, const T&, T&>>> current;
        ...
        void find_valid()
        {
            ...
            if (device_vec.size() == 0 || device_segment_id == device_vec.size()) {
                while (++device_id < DEVICE_ID_MAX_VALID && parent.device_to_segments[device_id].size() == 0)
                    ;
                device_segment_id = 0;
            }
            if (is_end()) {
                ...
                current.emplace(segment_id_t{device_id, device_segment_id},
                                parent.device_to_segments[device_id][device_segment_id]);
            }
        }

        iterator(decltype(parent)& parent, device_id_t device_id, device_segment_id_t device_segment_id)
            : parent(parent)
            , device_id(device_id)
    ...
    public:
        static iterator lower_bound(decltype(parent)& parent, device_id_t device_id,
                                    device_segment_id_t device_segment_id)
        {
        ...
        {
            return (device_id == rit.device_id && device_segment_id == rit.device_segment_id);
        }
        ...
        template<bool c = is_const, std::enable_if_t<!c, int> = 0> std::pair<const segment_id_t, T&>* operator->()
        {
            assert(!is_end());
            return &*current;
        }

        using reference = std::conditional_t<is_const, const std::pair<const segment_id_t, const T&>&,
...
constexpr auto DEVICE_OFF_MAX = std::numeric_limits<device_off_t>::max() >> DEVICE_ID_BITS;
...
constexpr auto DEVICE_ID_MASK = ((internal_paddr_t(1) << DEVICE_ID_BITS) - 1) << DEVICE_OFF_BITS;
constexpr auto DEVICE_OFF_MASK = std::numeric_limits<u_device_off_t>::max() >> DEVICE_ID_BITS;
constexpr auto SEGMENT_ID_MASK = ((internal_paddr_t(1) << SEGMENT_ID_BITS) - 1) << SEGMENT_OFF_BITS;
constexpr auto SEGMENT_OFF_MASK = (internal_paddr_t(1) << SEGMENT_OFF_BITS) - 1;
...
public:
    ...
    paddr_t()
        : paddr_t(DEVICE_ID_MAX, device_off_t(0))
    {}

    static paddr_t make_seg_paddr(segment_id_t seg, segment_off_t offset) { return paddr_t(seg, offset); }

    static paddr_t make_seg_paddr(device_id_t device, device_segment_id_t seg, segment_off_t offset)
    {
        return paddr_t(segment_id_t(device, seg), offset);
    }

    static paddr_t make_blk_paddr(device_id_t device, device_off_t offset)
    {
        assert(device_id_to_paddr_type(device) == paddr_types_t::RANDOM_BLOCK);
        return paddr_t(device, offset);
    }

    static paddr_t make_res_paddr(device_id_t device, device_off_t offset)
    {
        assert(device_id_to_paddr_type(device) == paddr_types_t::RESERVED);
        return paddr_t(device, offset);
    }

    void swap(paddr_t& other) { std::swap(internal_paddr, other.internal_paddr); }

    device_id_t get_device_id() const { return static_cast<device_id_t>(internal_paddr >> DEVICE_OFF_BITS); }
    ...
    paddr_t operator+(device_off_t o) const { return add_offset(o); }
    ...
    bool is_delayed() const { return get_device_id() == DEVICE_ID_DELAYED; }
    bool is_block_relative() const { return get_device_id() == DEVICE_ID_BLOCK_RELATIVE; }
    bool is_record_relative() const { return get_device_id() == DEVICE_ID_RECORD_RELATIVE; }
    bool is_relative() const { return is_block_relative() || is_record_relative(); }
    /// Denotes special null addr
    bool is_null() const { return get_device_id() == DEVICE_ID_NULL; }
    /// Denotes special zero addr
    bool is_zero() const { return get_device_id() == DEVICE_ID_ZERO; }
    /// Denotes the root addr
    bool is_root() const { return get_device_id() == DEVICE_ID_ROOT; }
     ...
     */
    bool is_real() const { return !is_zero() && !is_null() && !is_root(); }

    bool is_absolute() const { return get_addr_type() != paddr_types_t::RESERVED; }

    bool is_fake() const { return get_device_id() == DEVICE_ID_FAKE; }
    ...
    constexpr static paddr_t create_const(device_id_t d_id, device_off_t offset)
    {
...
private:
    ...
    paddr_t(device_id_t d_id, device_off_t offset)
        : paddr_t((static_cast<internal_paddr_t>(d_id) << DEVICE_OFF_BITS) | encode_device_off(offset))
    {
    ...
    constexpr paddr_t(device_id_t d_id, device_off_t offset, const_construct_t)
        : internal_paddr((static_cast<internal_paddr_t>(d_id) << DEVICE_OFF_BITS) | static_cast<u_device_off_t>(offset))
    {}
...
struct seg_paddr_t : public paddr_t
{
    ...
    {
        return segment_id_t(static_cast<internal_segment_id_t>(internal_paddr >> SEGMENT_OFF_BITS));
    }

    segment_off_t get_segment_off() const { return segment_off_t(internal_paddr & SEGMENT_OFF_MASK); }
    ...
    paddr_t add_offset(device_off_t o) const
    {
        ...
        return paddr_t::make_seg_paddr(get_segment_id(), static_cast<segment_off_t>(off));
    }
};
...
struct blk_paddr_t : public paddr_t
{
    ...
    device_off_t get_device_off() const { return decode_device_off(internal_paddr); }
...
inline paddr_t::paddr_t(internal_paddr_t val)
    : internal_paddr(val)
{
#ifndef NDEBUG
...
{
    ceph_le64 internal_paddr = ceph_le64(P_ADDR_NULL.internal_paddr);

    using orig_type = paddr_t;

    paddr_le_t() = default;
    paddr_le_t(const paddr_t& addr)
        : internal_paddr(ceph_le64(addr.internal_paddr))
    {}

    operator paddr_t() const { return paddr_t{internal_paddr}; }
};
...
{
    assert(dtype != device_type_t::NONE && dtype != device_type_t::NUM_TYPES);
    if (dtype >= device_type_t::HDD && dtype <= device_type_t::EPHEMERAL_MAIN) {
...
struct journal_seq_t
{
    ...
    journal_seq_t add_offset(journal_type_t type, device_off_t off, device_off_t roll_start,
                             device_off_t roll_size) const;

    device_off_t relative_to(journal_type_t type, const journal_seq_t& r, device_off_t roll_start,
                             device_off_t roll_size) const;
...
constexpr journal_seq_t JOURNAL_SEQ_MIN{0, P_ADDR_MIN};
constexpr journal_seq_t JOURNAL_SEQ_MAX{MAX_SEG_SEQ, P_ADDR_MAX};
// JOURNAL_SEQ_NULL == JOURNAL_SEQ_MAX == journal_seq_t{}
...
struct __attribute((packed)) laddr_le_t
{
    ...
    operator laddr_t() const { return laddr_t(laddr); }
...
struct pladdr_t
{
    ...
    bool is_laddr() const { return pladdr.index() == 0; }

    bool is_paddr() const { return pladdr.index() == 1; }
...
struct __attribute((packed)) pladdr_le_t
{
    ...
    explicit pladdr_le_t(const pladdr_t& addr)
        : pladdr(ceph_le64(addr.is_laddr() ? std::get<0>(addr.pladdr) : std::get<1>(addr.pladdr).internal_paddr))
        , addr_type(addr.is_laddr() ? addr_type_t::LADDR : addr_type_t::PADDR)
    {}
...
template<typename T> struct min_max_t
{};

template<> struct min_max_t<laddr_t>
{
...
template<> struct min_max_t<paddr_t>
{
...
constexpr extent_len_t EXTENT_LEN_MAX = std::numeric_limits<extent_len_t>::max();
...
constexpr bool is_logical_type(extent_types_t type)
{
    ...
    case extent_types_t::BACKREF_LEAF: return false;
    default: return true;
    }
}
...
{
    return type == extent_types_t::LADDR_INTERNAL || type == extent_types_t::LADDR_LEAF ||
           type == extent_types_t::DINK_LADDR_LEAF;
}

constexpr bool is_backref_node(extent_types_t type)
{
    return type == extent_types_t::BACKREF_INTERNAL || type == extent_types_t::BACKREF_LEAF;
}
...
constexpr rewrite_gen_t NULL_GENERATION = std::numeric_limits<rewrite_gen_t>::max();
...
{
    return gen == INIT_GENERATION || (gen >= MIN_REWRITE_GENERATION && gen <= REWRITE_GENERATIONS);
}

// after EPM decision
constexpr bool is_rewrite_generation(rewrite_gen_t gen)
{
    return gen >= INLINE_GENERATION && gen < REWRITE_GENERATIONS;
}
...
{
    if (type == extent_types_t::OBJECT_DATA_BLOCK || type == extent_types_t::TEST_BLOCK) {
...
constexpr mod_time_point_t timepoint_to_mod(const sea_time_point& t)
{
    return std::chrono::duration_cast<std::chrono::milliseconds>(t.time_since_epoch()).count();
}

constexpr sea_time_point mod_to_timepoint(mod_time_point_t t)
{
    return sea_time_point(std::chrono::duration_cast<sea_duration>(std::chrono::milliseconds(t)));
}
...
constexpr sea_time_point get_average_time(const sea_time_point& t1, std::size_t n1, const sea_time_point& t2,
                                          std::size_t n2)
{
    assert(t1 != NULL_TIME);
    assert(t2 != NULL_TIME);
...
struct delta_info_t
{
    ...
    {
        return (type == rhs.type && paddr == rhs.paddr && laddr == rhs.laddr && prev_crc == rhs.prev_crc &&
                final_crc == rhs.final_crc && length == rhs.length && pversion == rhs.pversion &&
                ext_seq == rhs.ext_seq && bl == rhs.bl);
    }
};
...
public:
    object_data_t(laddr_t reserved_data_base, extent_len_t reserved_data_len)
        : reserved_data_base(reserved_data_base)
        , reserved_data_len(reserved_data_len)
    {}

    laddr_t get_reserved_data_base() const { return reserved_data_base; }

    extent_len_t get_reserved_data_len() const { return reserved_data_len; }

    bool is_null() const { return reserved_data_base == L_ADDR_NULL; }

    bool must_update() const { return dirty; }

    void update_reserved(laddr_t base, extent_len_t len)
    {
        dirty = true;
    ...
    void update_len(extent_len_t len)
    {
        dirty = true;
        reserved_data_len = len;
    }
...
struct __attribute__((packed)) object_data_le_t
{
    ...
    object_data_t get() const { return object_data_t(reserved_data_base, reserved_data_len); }
};
...
struct omap_root_t
{
    ...
    bool is_null() const { return addr == L_ADDR_NULL; }

    bool must_update() const { return mutated; }
    ...
    laddr_t get_location() const { return addr; }

    depth_t get_depth() const { return depth; }

    laddr_t get_hint() const { return hint; }
};
...
public:
    ...
    omap_root_t get(laddr_t hint) const { return omap_root_t(addr, depth, hint); }
};
...
public:
    ...
    paddr_t get_location() const { return root_addr; }

    void set_location(paddr_t location) { root_addr = location; }

    depth_t get_depth() const { return depth; }

    void set_depth(depth_t ndepth) { depth = ndepth; }
...
public:
    coll_root_t() = default;
    coll_root_t(laddr_t addr, extent_len_t size)
        : addr(addr)
        , size(size)
    {}
    ...
    extent_len_t get_size() const { return size; }
};
...
public:
    ...
    coll_root_t get() const { return coll_root_t(addr, size); }
};
...
struct alloc_blk_t
{
    alloc_blk_t(paddr_t paddr, laddr_t laddr, extent_len_t len, extent_types_t type)
        : paddr(paddr)
        , laddr(laddr)
        , len(len)
        , type(type)
...
struct segment_header_t
{
    ...
    segment_type_t get_type() const { return type; }
...
static constexpr auto TRANSACTION_TYPE_MAX = static_cast<std::size_t>(transaction_type_t::MAX);
...
{
    return (type >= transaction_type_t::TRIM_DIRTY && type < transaction_type_t::MAX);
}

constexpr bool is_trim_transaction(transaction_type_t type)
{
    return (type == transaction_type_t::TRIM_DIRTY || type == transaction_type_t::TRIM_ALLOC);
}
...
struct record_size_t
{
    ...
    bool is_empty() const { return plain_mdlength == 0 && dlength == 0; }

    void account_extent(extent_len_t extent_len);

    void account(const extent_t& extent) { account_extent(extent.bl.length()); }
...
struct record_t
{
    ...
    record_t(transaction_type_t type)
        : type{type}
    {}

    // unit test only
    record_t() { type = transaction_type_t::MUTATE; }

    // unit test only
    record_t(std::vector<extent_t>&& _extents, std::vector<delta_info_t>&& _deltas)
    {
    ...
    bool is_empty() const { return extents.size() == 0 && deltas.size() == 0; }

    std::size_t get_delta_size() const
    {
        auto delta_size = std::accumulate(
            deltas.begin(), deltas.end(), 0, [](uint64_t sum, auto& delta) { return sum + delta.bl.length(); });
        return delta_size;
    }
...
struct record_group_size_t
{
    ...
    record_group_size_t(const record_size_t& rsize, extent_len_t block_size) { account(rsize, block_size); }
    ...
    record_group_size_t get_encoded_length_after(const record_size_t& rsize, extent_len_t block_size) const
    {
    ...
    {
        assert(block_size > 0);
        return ((double)(get_raw_mdlength() + dlength) / get_encoded_length());
    }

    void account(const record_size_t& rsize, extent_len_t block_size);
...
struct record_group_t
{
    ...
    record_group_t(record_t&& record, extent_len_t block_size) { push_back(std::move(record), block_size); }

    std::size_t get_size() const { return records.size(); }

    void push_back(record_t&& record, extent_len_t block_size)
    {
    ...
    void reserve(std::size_t limit) { records.reserve(limit); }
...
ceph::bufferlist encode_record(record_t&& record, extent_len_t block_size, const journal_seq_t& committed_to,
                               segment_nonce_t current_segment_nonce);

ceph::bufferlist encode_records(record_group_t& record_group, const journal_seq_t& committed_to,
                                segment_nonce_t current_segment_nonce);

std::optional<record_group_header_t> try_decode_records_header(const ceph::bufferlist& header_bl,
                                                               segment_nonce_t expected_nonce);

bool validate_records_metadata(const ceph::bufferlist& md_bl);

bool validate_records_data(const record_group_header_t& header, const ceph::bufferlist& data_bl);
...
std::optional<std::vector<record_extent_infos_t>> try_decode_extent_infos(const record_group_header_t& header,
                                                                          const ceph::bufferlist& md_bl);
std::optional<std::vector<record_header_t>> try_decode_record_headers(const record_group_header_t& header,
                                                                      const ceph::bufferlist& md_bl);
...
std::optional<std::vector<record_deltas_t>> try_decode_deltas(const record_group_header_t& header,
...
struct write_result_t
{
    ...
    {
        return journal_seq_t{start_seq.segment_seq, start_seq.offset.add_offset(length)};
    }
};
...
struct scan_valid_records_cursor
{
    ...
    struct found_record_group_t
    {
        ...
        found_record_group_t(paddr_t offset, const record_group_header_t& header, const bufferlist& mdbuffer)
            : offset(offset)
            , header(header)
            , mdbuffer(mdbuffer)
        {}
    ...
    bool is_complete() const { return last_valid_header_found && pending_record_groups.empty(); }

    segment_id_t get_segment_id() const { return seq.offset.as_seg_paddr().get_segment_id(); }

    segment_off_t get_segment_offset() const { return seq.offset.as_seg_paddr().get_segment_off(); }

    extent_len_t get_block_size() const { return block_size; }

    void increment_seq(segment_off_t off) { seq.offset = seq.offset.add_offset(off); }
    ...
    scan_valid_records_cursor(journal_seq_t seq)
        : seq(seq)
    {}
};
