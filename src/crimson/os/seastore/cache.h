...
{
    backref_entry_t(const paddr_t paddr, const laddr_t laddr, const extent_len_t len, const extent_types_t type,
    ...
    journal_seq_t seq;
    friend bool operator<(const backref_entry_t& l, const backref_entry_t& r) { return l.paddr < r.paddr; }
    friend bool operator>(const backref_entry_t& l, const backref_entry_t& r) { return l.paddr > r.paddr; }
    friend bool operator==(const backref_entry_t& l, const backref_entry_t& r) { return l.paddr == r.paddr; }
    ...
    using backref_set_member_options =
        boost::intrusive::member_hook<backref_entry_t, set_hook_t, &backref_entry_t::backref_set_hook>;
    using multiset_t = boost::intrusive::multiset<backref_entry_t, backref_set_member_options,
                                                  boost::intrusive::constant_time_size<false>>;

    struct cmp_t
    {
        using is_transparent = paddr_t;
        bool operator()(const backref_entry_t& l, const backref_entry_t& r) const { return l.paddr < r.paddr; }
        bool operator()(const paddr_t l, const backref_entry_t& r) const { return l < r.paddr; }
        bool operator()(const backref_entry_t& l, const paddr_t r) const { return l.paddr < r; }
    };
};
...
public:
    ...
    TransactionRef create_transaction(Transaction::src_t src, const char* name, bool is_weak)
    {
        ...
        auto ret = std::make_unique<Transaction>(
            ...
            src,
            last_commit,
            [this](Transaction& t) { return on_transaction_destruct(t); },
            ++next_id);
        SUBDEBUGT(seastore_t, "created name={}, source={}, is_weak={}", *ret, name, src, is_weak);
    ...
    retire_extent_ret retire_extent_addr(Transaction& t, paddr_t addr, extent_len_t length);
    ...
    template<typename T> using get_extent_ret = get_extent_ertr::future<TCachedExtentRef<T>>;
    template<typename T, typename Func, typename OnCache>
    get_extent_ret<T> get_extent(paddr_t offset,               ///< [in] starting addr
                                 ...
                                 OnCache&& on_cache)
    {
        ...
        if (!cached) {
            auto ret = CachedExtent::make_cached_extent_ref<T>(alloc_cache_buf(length));
            ret->init(CachedExtent::extent_state_t::CLEAN_PENDING,
                      ...
                      TRANS_ID_NULL);
            SUBDEBUG(
                seastore_cache, "{} {}~{} is absent, add extent and reading ... -- {}", T::TYPE, offset, length, *ret);
        ...
        if (cached->get_type() == extent_types_t::RETIRED_PLACEHOLDER) {
            ...
            ret->init(CachedExtent::extent_state_t::CLEAN_PENDING,
                      ...
                      TRANS_ID_NULL);
            SUBDEBUG(
                seastore_cache, "{} {}~{} is absent(placeholder), reading ... -- {}", T::TYPE, offset, length, *ret);
            ...
            SUBTRACE(seastore_cache, "{} {}~{} is present in cache -- {}", T::TYPE, offset, length, *cached);
            auto ret = TCachedExtentRef<T>(static_cast<T*>(cached.get()));
            on_cache(*ret);
            return ret->wait_io().then([ret = std::move(ret)]() mutable -> get_extent_ret<T> {
                // ret may be invalid, caller must check
                return get_extent_ret<T>(get_extent_ertr::ready_future_marker{}, std::move(ret));
            });
        }
    }
    template<typename T>
    ...
    {
        return get_extent<T>(
            offset, length, p_metric_key, [](T&) {}, [](T&) {});
    }
    ...
    using get_extent_if_cached_ret = get_extent_if_cached_iertr::future<CachedExtentRef>;
    get_extent_if_cached_ret get_extent_if_cached(Transaction& t, paddr_t offset, extent_types_t type)
    {
        CachedExtentRef ret;
        LOG_PREFIX(Cache::get_extent_if_cached);
        auto result = t.get_extent(offset, &ret);
        if (result == Transaction::get_extent_ret::RETIRED) {
            SUBDEBUGT(seastore_cache, "{} {} is retired on t -- {}", t, type, offset, *ret);
            ...
            if (ret->is_fully_loaded()) {
                SUBTRACET(seastore_cache, "{} {} is present on t -- {}", t, type, offset, *ret);
                return ret->wait_io().then(
                    [ret] { return get_extent_if_cached_iertr::make_ready_future<CachedExtentRef>(ret); });
            }
            else {
        ...
        if (!ret) {
            ...
            SUBDEBUGT(seastore_cache, "{} {} is absent(placeholder)", t, type, offset);
            return get_extent_if_cached_iertr::make_ready_future<CachedExtentRef>();
        }
        ...
        SUBDEBUGT(seastore_cache, "{} {} is present in cache -- {}", t, type, offset, *ret);
    ...
    get_extent_iertr::future<TCachedExtentRef<T>> get_extent(Transaction& t, paddr_t offset, extent_len_t length,
                                                             Func&& extent_init_func)
    {
        ...
        auto result = t.get_extent(offset, &ret);
        if (result == Transaction::get_extent_ret::RETIRED) {
            SUBERRORT(seastore_cache, "{} {}~{} is retired on t -- {}", t, T::TYPE, offset, length, *ret);
            ceph_abort("impossible");
        }
        else if (result == Transaction::get_extent_ret::PRESENT) {
            if (ret->is_fully_loaded()) {
                SUBTRACET(seastore_cache, "{} {}~{} is present on t -- {}", t, T::TYPE, offset, length, *ret);
                return ret->wait_io().then(
                    [ret] { return seastar::make_ready_future<TCachedExtentRef<T>>(ret->cast<T>()); });
            }
            else {
                assert(!ret->is_mutable());
                touch_extent(*ret);
                SUBDEBUGT(seastore_cache,
                          "{} {}~{} is present on t without been \
          fully loaded, reading ... {}",
                          t,
                          T::TYPE,
                          offset,
                          length,
                ...
                ret->set_bptr(std::move(bp));
                return read_extent<T>(ret->cast<T>());
            }
        }
        else {
            SUBTRACET(seastore_cache, "{} {}~{} is absent on t, query cache ...", t, T::TYPE, offset, length);
            ...
            return trans_intr::make_interruptible(
                get_extent<T>(offset, length, &metric_key, std::forward<Func>(extent_init_func), std::move(f)));
        }
    }
     ...
     */
    template<typename T, typename Func>
    get_extent_iertr::future<TCachedExtentRef<T>> get_absent_extent(Transaction& t, paddr_t offset, extent_len_t length,
                                                                    Func&& extent_init_func)
    {
        CachedExtentRef ret;
    ...
    get_extent_ertr::future<CachedExtentRef> get_extent_viewable_by_trans(Transaction& t, CachedExtentRef extent)
    {
        ...
        return p_extent->wait_io().then(
            [p_extent] { return get_extent_ertr::make_ready_future<CachedExtentRef>(CachedExtentRef(p_extent)); });
    }

    template<typename T>
    get_extent_ertr::future<TCachedExtentRef<T>> get_extent_viewable_by_trans(Transaction& t,
                                                                              TCachedExtentRef<T> extent)
    {
        return get_extent_viewable_by_trans(t, CachedExtentRef(extent.get())).safe_then([](auto p_extent) {
            return p_extent->template cast<T>();
        });
    }

    extent_len_t get_block_size() const { return epm.get_block_size(); }
...
private:
    ...
    class extent_init_func_t
    {
        ...
        template<typename Func> struct callable_wrapper final : callable_i
        {
            Func func;
            callable_wrapper(Func&& func)
                : func(std::forward<Func>(func))
            {}
            void operator()(CachedExtent& extent) final { return func(extent); }
    ...
    public:
        ...
        extent_init_func_t(Func&& func)
            : wrapped(std::make_unique<callable_wrapper<Func>>(std::forward<Func>(func)))
        {}
        void operator()(CachedExtent& extent) { return (*wrapped)(extent); }
    };
    get_extent_ertr::future<CachedExtentRef> _get_extent_by_type(extent_types_t type, paddr_t offset, laddr_t laddr,
                                                                 extent_len_t length, const Transaction::src_t* p_src,
                                                                 extent_init_func_t&& extent_init_func,
                                                                 extent_init_func_t&& on_cache);

    using get_extent_by_type_iertr = get_extent_iertr;
    using get_extent_by_type_ret = get_extent_by_type_iertr::future<CachedExtentRef>;
    get_extent_by_type_ret _get_extent_by_type(Transaction& t, extent_types_t type, paddr_t offset, laddr_t laddr,
                                               extent_len_t length, extent_init_func_t&& extent_init_func)
    {
        LOG_PREFIX(Cache::get_extent_by_type);
        CachedExtentRef ret;
        auto status = t.get_extent(offset, &ret);
        if (status == Transaction::get_extent_ret::RETIRED) {
            SUBERRORT(seastore_cache, "{} {}~{} {} is retired on t -- {}", t, type, offset, length, laddr, *ret);
            ceph_abort("impossible");
        }
        else if (status == Transaction::get_extent_ret::PRESENT) {
            if (ret->is_fully_loaded()) {
                SUBTRACET(seastore_cache, "{} {}~{} {} is present on t -- {}", t, type, offset, length, laddr, *ret);
                return ret->wait_io().then([ret] { return seastar::make_ready_future<CachedExtentRef>(ret); });
            }
            else {
                assert(!ret->is_mutable());
                touch_extent(*ret);
                ...
                auto bp = alloc_cache_buf(ret->get_length());
                ret->set_bptr(std::move(bp));
                return read_extent<CachedExtent>(std::move(ret));
            }
        }
        else {
            SUBTRACET(seastore_cache, "{} {}~{} {} is absent on t, query cache ...", t, type, offset, length, laddr);
            auto f = [&t, this](CachedExtent& ext) {
                t.add_to_read_set(CachedExtentRef(&ext));
                touch_extent(ext);
            };
            auto src = t.get_src();
            return trans_intr::make_interruptible(
                _get_extent_by_type(type, offset, laddr, length, &src, std::move(extent_init_func), std::move(f)));
        }
    }

    get_extent_by_type_ret _get_absent_extent_by_type(Transaction& t, extent_types_t type, paddr_t offset,
    ...
    using backref_entry_query_mset_t = std::multiset<backref_entry_t, backref_entry_t::cmp_t>;
    backref_entry_query_mset_t get_backref_entries_in_range(paddr_t start, paddr_t end)
    {
        auto start_iter = backref_entry_mset.lower_bound(start, backref_entry_t::cmp_t());
        auto end_iter = backref_entry_mset.lower_bound(end, backref_entry_t::cmp_t());
    ...
    const backref_entry_mset_t& get_backref_entry_mset() { return backref_entry_mset; }
...
public:
     ...
     */
    template<typename Func>
    get_extent_by_type_ret get_extent_by_type(Transaction& t,           ///< [in] transaction
    ...
    )
    {
        return _get_extent_by_type(
            t, type, offset, laddr, length, extent_init_func_t(std::forward<Func>(extent_init_func)));
    }
     ...
     */
    template<typename Func>
    get_extent_by_type_ret get_absent_extent_by_type(Transaction& t,           ///< [in] transaction
                                                     extent_types_t type,      ///< [in] type tag
                                                     paddr_t offset,           ///< [in] starting addr
                                                     laddr_t laddr,            ///< [in] logical address if logical
                                                     extent_len_t length,      ///< [in] length
    ...
    get_extent_by_type_ret get_extent_by_type(Transaction& t, extent_types_t type, paddr_t offset, laddr_t laddr,
                                              extent_len_t length)
    {
        return get_extent_by_type(t, type, offset, laddr, length, [](CachedExtent&) {});
    }
     ...
     * the specific CachedExtent
     */
    get_extent_by_type_ret get_absent_extent_by_type(Transaction& t, extent_types_t type, paddr_t offset, laddr_t laddr,
                                                     extent_len_t length)
    {
        return get_absent_extent_by_type(t, type, offset, laddr, length, [](CachedExtent&) {});
    }
     ...
     */
    template<typename T>
    TCachedExtentRef<T> alloc_new_extent(Transaction& t,          ///< [in, out] current transaction
                                         extent_len_t length,     ///< [in] length
    ...
    )
    {
        ...
        SUBTRACET(
            seastore_cache, "allocate {} {}B, hint={}, gen={}", t, T::TYPE, length, hint, rewrite_gen_printer_t{gen});
#ifdef UNIT_TESTS_BUILT
        ...
        ret->init(
            CachedExtent::extent_state_t::INITIAL_WRITE_PENDING, result.paddr, hint, result.gen, t.get_trans_id());
        ...
        SUBDEBUGT(seastore_cache,
                  ...
                  result.paddr,
                  hint,
                  rewrite_gen_printer_t{result.gen},
                  *ret);
        return ret;
    }
     ...
     */
    template<typename T>
    TCachedExtentRef<T> alloc_remapped_extent(Transaction& t, laddr_t remap_laddr, paddr_t remap_paddr,
                                              extent_len_t remap_length, laddr_t original_laddr,
                                              std::optional<ceph::bufferptr>&& original_bptr)
    {
        ...
        if (original_bptr.has_value()) {
            ...
            auto nbp = ceph::bufferptr(*original_bptr, remap_laddr - original_laddr, remap_length);
        ...
        SUBTRACET(seastore_cache,
                  "allocated {} {}B, hint={}, has ptr? {} -- {}",
                  t,
                  T::TYPE,
                  remap_length,
                  remap_laddr,
                  original_bptr.has_value(),
                  *ext);
        return ext;
    }
     ...
     */
    CachedExtentRef alloc_new_extent_by_type(Transaction& t,          ///< [in, out] current transaction
                                             extent_types_t type,     ///< [in] type tag
                                             extent_len_t length,     ///< [in] length
                                             placement_hint_t hint,   ///< [in] user hint
                                             rewrite_gen_t gen        ///< [in] rewrite generation
     ...
     */
    CachedExtentRef duplicate_for_write(Transaction& t,     ///< [in, out] current transaction
     ...
     */
    record_t prepare_record(Transaction& t,   ///< [in, out] current transaction
                            const journal_seq_t& journal_head, const journal_seq_t& journal_dirty_tail);
     ...
     * and mutated exents.
     */
    void complete_commit(Transaction& t,              ///< [in, out] current transaction
     ...
     */
    using replay_delta_ertr = crimson::errorator<crimson::ct_error::input_output_error>;
    using replay_delta_ret = replay_delta_ertr::future<bool>;
    replay_delta_ret replay_delta(journal_seq_t seq, paddr_t record_block_base, const delta_info_t& delta,
    ...
    template<typename F> init_cached_extents_ret init_cached_extents(Transaction& t, F&& f)
    {
        ...
        return seastar::do_with(std::forward<F>(f),
                                std::move(_dirty),
                                [this, FNAME, &t](auto& f, auto& refs) mutable {
                                    return trans_intr::do_for_each(refs, [this, FNAME, &t, &f](auto& e) {
                                        SUBTRACET(seastore_cache, "inspecting extent ... -- {}", t, *e);
                                        return f(t, e).si_then([this, FNAME, &t, e](bool is_alive) {
                                            if (!is_alive) {
                                                SUBDEBUGT(
                                                    seastore_cache, "extent is not alive, remove extent -- {}", t, *e);
                                                remove_extent(e);
                                                e->set_invalid(t);
                                            }
                                            else {
            ...
            .handle_error_interruptible(init_cached_extents_iertr::pass_further{},
     ...
     */
    CachedExtentRef update_extent_from_transaction(Transaction& t, CachedExtentRef extent)
    {
    ...
    get_next_dirty_extents_ret get_next_dirty_extents(Transaction& t, journal_seq_t seq, size_t max_bytes);
    ...
    {
        backref_extent_entry_t(paddr_t paddr, paddr_t key, extent_types_t type)
            : paddr(paddr)
            , key(key)
            , type(type)
        {}
        ...
        {
            using is_transparent = paddr_t;
            bool operator()(const backref_extent_entry_t& l, const backref_extent_entry_t& r) const
            {
                return l.paddr < r.paddr;
            }
            bool operator()(const paddr_t& l, const backref_extent_entry_t& r) const { return l < r.paddr; }
            bool operator()(const backref_extent_entry_t& l, const paddr_t& r) const { return l.paddr < r; }
        };
    };
    ...
    {
        switch (type) {
        case extent_types_t::LADDR_INTERNAL: [[fallthrough]];
        case extent_types_t::DINK_LADDR_LEAF: [[fallthrough]];
        case extent_types_t::LADDR_LEAF:
            stats.lba_tree_extents_num += delta;
            ceph_assert(stats.lba_tree_extents_num >= 0);
            return;
        case extent_types_t::OMAP_INNER: [[fallthrough]];
        ...
        case extent_types_t::ONODE_BLOCK_STAGED:
            ...
            return;
        case extent_types_t::BACKREF_INTERNAL: [[fallthrough]];
    ...
    uint64_t get_omap_tree_depth() { return stats.omap_tree_depth; }

    /// Update lru for access to ref
    void touch_extent(CachedExtent& ext, const Transaction::src_t* p_src = nullptr)
    {
        if (p_src && is_background_transaction(*p_src)) return;
...
private:
    ...
    using backref_extent_entry_query_set_t = std::set<backref_extent_entry_t, backref_extent_entry_t::cmp_t>;
    backref_extent_entry_query_set_t backref_extents;

    void add_backref_extent(paddr_t paddr, paddr_t key, extent_types_t type)
    {
    ...
    void remove_backref_extent(paddr_t paddr)
    {
        ...
        if (iter != backref_extents.end()) backref_extents.erase(iter);
    }

    backref_extent_entry_query_set_t get_backref_extents_in_range(paddr_t start, paddr_t end)
    {
    ...
    public:
        ...
        size_t get_current_contents_bytes() const { return contents; }

        size_t get_current_contents_extents() const { return lru.size(); }
    ...
    template<typename CounterT> using counter_by_extent_t = std::array<CounterT, EXTENT_TYPES_MAX>;
    ...
    template<typename CounterT> using counter_by_src_t = std::array<CounterT, TRANSACTION_TYPE_MAX>;

    static constexpr std::size_t NUM_SRC_COMB = TRANSACTION_TYPE_MAX * (TRANSACTION_TYPE_MAX + 1) / 2;
    ...
    CounterT& get_by_src(counter_by_src_t<CounterT>& counters_by_src, Transaction::src_t src)
    {
        assert(static_cast<std::size_t>(src) < counters_by_src.size());
        return counters_by_src[static_cast<std::size_t>(src)];
    }

    template<typename CounterT> CounterT& get_by_ext(counter_by_extent_t<CounterT>& counters_by_ext, extent_types_t ext)
    {
    ...
    void account_conflict(Transaction::src_t src1, Transaction::src_t src2)
    {
        ...
        assert(!(src1 == Transaction::src_t::READ && src2 == Transaction::src_t::READ));
        assert(!(src1 == Transaction::src_t::TRIM_DIRTY && src2 == Transaction::src_t::TRIM_DIRTY));
        assert(!(src1 == Transaction::src_t::CLEANER_MAIN && src2 == Transaction::src_t::CLEANER_MAIN));
        assert(!(src1 == Transaction::src_t::CLEANER_COLD && src2 == Transaction::src_t::CLEANER_COLD));
        assert(!(src1 == Transaction::src_t::TRIM_ALLOC && src2 == Transaction::src_t::TRIM_ALLOC));
        ...
        auto conflict_index = num_srcs * src1_value + src2_value - src1_value * (src1_value + 1) / 2;
    ...
    bufferptr alloc_cache_buf(size_t size)
    {
        ...
        auto bp = ceph::bufferptr(buffer::create_page_aligned(size));
        bp.zero();
        return bp;
    }

    void backref_batch_update(std::vector<backref_entry_ref>&&, const journal_seq_t&);
    ...
    void mark_transaction_conflicted(Transaction& t, CachedExtent& conflicting_extent);

    /// Introspect transaction when it is being destructed
    void on_transaction_destruct(Transaction& t);

    template<typename T> get_extent_ret<T> read_extent(TCachedExtentRef<T>&& extent)
    {
        ...
        extent->set_io_wait();
        return epm.read(extent->get_paddr(), extent->get_length(), extent->get_bptr())
            .safe_then(
                [extent = std::move(extent)]() mutable {
                    ...
                    return get_extent_ertr::make_ready_future<TCachedExtentRef<T>>(std::move(extent));
                },
                get_extent_ertr::pass_further{},
                crimson::ct_error::assert_all{"Cache::get_extent: invalid error"});
    }

    // Extents in cache may contain placeholders
    CachedExtentRef query_cache(paddr_t offset, const src_ext_t* p_metric_key)
    {
        ...
        if (auto iter = extents.find_offset(offset); iter != extents.end()) {
            if (p_metric_key &&
                // retired_placeholder is not really cached yet
