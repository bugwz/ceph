...
Cache::retire_extent_ret Cache::retire_extent_if_cached(Transaction& t, paddr_t addr)
{
    ...
    else if (auto iter = extents.find_offset(addr); iter != extents.end()) {
...
{
    switch (type) {
    case extent_types_t::ROOT: assert(0 == "ROOT is never directly alloc'd"); return CachedExtentRef();
    case extent_types_t::LADDR_INTERNAL: return alloc_new_extent<lba_manager::btree::LBAInternalNode>(t, length);
    case extent_types_t::LADDR_LEAF: return alloc_new_extent<lba_manager::btree::LBALeafNode>(t, length);
    case extent_types_t::ONODE_BLOCK: return alloc_new_extent<OnodeBlock>(t, length);
    case extent_types_t::EXTMAP_INNER: return alloc_new_extent<extentmap_manager::ExtMapInnerNode>(t, length);
    case extent_types_t::EXTMAP_LEAF: return alloc_new_extent<extentmap_manager::ExtMapLeafNode>(t, length);
    case extent_types_t::TEST_BLOCK: return alloc_new_extent<TestBlock>(t, length);
    case extent_types_t::TEST_BLOCK_PHYSICAL: return alloc_new_extent<TestBlockPhysical>(t, length);
...
std::optional<record_t> Cache::try_construct_record(Transaction& t)
{
    ...
    for (auto& i : t.read_set) {
        if (i->state == CachedExtent::extent_state_t::INVALID) return std::nullopt;
    }
    ...
    if (t.root) {
        logger().debug("{}: writing out root delta for {}", __func__, *t.root);
        record.deltas.push_back(delta_info_t{
            extent_types_t::ROOT, paddr_t{}, L_ADDR_NULL, 0, 0, 0, t.root->get_version() - 1, t.root->get_delta()});
    }
    ...
    for (auto& i : t.fresh_block_list) {
        ...
        record.extents.push_back(extent_t{
            i->get_type(), i->is_logical() ? i->cast<LogicalCachedExtent>()->get_laddr() : L_ADDR_NULL, std::move(bl)});
    }

    return std::make_optional<record_t>(std::move(record));
}

void Cache::complete_commit(Transaction& t, paddr_t final_block_start, journal_seq_t seq, SegmentCleaner* cleaner)
{
    if (t.root) {
        remove_extent(root);
        root = t.root;
    ...
    for (auto& i : t.fresh_block_list) {
        ...
        add_extent(i);
        if (cleaner) {
            cleaner->mark_space_used(i->get_paddr(), i->get_length());
        }
    }
    ...
    if (cleaner) {
        for (auto& i : t.retired_set) {
            cleaner->mark_space_free(i->get_paddr(), i->get_length());
        }
    }
...
Cache::replay_delta_ret Cache::replay_delta(journal_seq_t journal_seq, paddr_t record_base, const delta_info_t& delta)
{
    if (delta.type == extent_types_t::ROOT) {
        ...
        auto get_extent_if_cached = [this](paddr_t addr) -> replay_delta_ertr::future<CachedExtentRef> {
        ...
        auto extent_fut = delta.pversion == 0 ? get_extent_by_type(delta.type, delta.paddr, delta.laddr, delta.length)
                                              : get_extent_if_cached(delta.paddr);
        return extent_fut.safe_then([=, &delta](auto extent) {
            if (!extent) {
                assert(delta.pversion > 0);
                logger().debug("replay_delta: replaying {}, extent not present so delta is obsolete", delta);
                return;
            }

            logger().debug("replay_delta: replaying {} on {}", *extent, delta);
...
Cache::get_next_dirty_extents_ret Cache::get_next_dirty_extents(journal_seq_t seq)
{
    ...
    return seastar::do_with(std::move(ret), [](auto& ret) {
        return seastar::do_for_each(ret,
                                    [](auto& ext) {
                                        logger().debug("get_next_dirty_extents: waiting on {}", *ext);
                                        return ext->wait_io();
                                    })
            .then(
                [&ret]() mutable { return seastar::make_ready_future<std::vector<CachedExtentRef>>(std::move(ret)); });
    });
}
...
{
    if (t.root) {
        return get_root_ret(get_root_ertr::ready_future_marker{}, t.root);
    }
    else {
        auto ret = root;
        return ret->wait_io().then([ret, &t] {
            t.root = ret;
            return get_root_ret(get_root_ertr::ready_future_marker{}, ret);
        });
    }
}
...
Cache::get_extent_ertr::future<CachedExtentRef> Cache::get_extent_by_type(extent_types_t type, paddr_t offset,
                                                                          laddr_t laddr, segment_off_t length)
{
    return [=] {
        switch (type) {
        ...
        case extent_types_t::LADDR_INTERNAL:
            return get_extent<lba_manager::btree::LBAInternalNode>(offset, length).safe_then([](auto extent) {
                return CachedExtentRef(extent.detach(), false /* add_ref */);
            });
        case extent_types_t::LADDR_LEAF:
            return get_extent<lba_manager::btree::LBALeafNode>(offset, length).safe_then([](auto extent) {
                return CachedExtentRef(extent.detach(), false /* add_ref */);
            });
        case extent_types_t::EXTMAP_INNER:
            return get_extent<extentmap_manager::ExtMapInnerNode>(offset, length).safe_then([](auto extent) {
                return CachedExtentRef(extent.detach(), false /* add_ref */);
            });
        case extent_types_t::EXTMAP_LEAF:
            return get_extent<extentmap_manager::ExtMapLeafNode>(offset, length).safe_then([](auto extent) {
                return CachedExtentRef(extent.detach(), false /* add_ref */);
            });
        case extent_types_t::ONODE_BLOCK:
            return get_extent<OnodeBlock>(offset, length).safe_then([](auto extent) {
                return CachedExtentRef(extent.detach(), false /* add_ref */);
            });
        case extent_types_t::ONODE_BLOCK_STAGED:
            return get_extent<StagedOnodeBlock>(offset, length).safe_then([](auto extent) {
                return CachedExtentRef(extent.detach(), false /* add_ref */);
            });
        case extent_types_t::TEST_BLOCK:
            return get_extent<TestBlock>(offset, length).safe_then([](auto extent) {
                return CachedExtentRef(extent.detach(), false /* add_ref */);
            });
        case extent_types_t::TEST_BLOCK_PHYSICAL:
            return get_extent<TestBlockPhysical>(offset, length).safe_then([](auto extent) {
                return CachedExtentRef(extent.detach(), false /* add_ref */);
            });
        case extent_types_t::NONE:
        {
