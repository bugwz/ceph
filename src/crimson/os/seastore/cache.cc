...
{
    return out << "backref_entry_t{" << ent.paddr << "~" << ent.len << ", "
               << "laddr: " << ent.laddr << ", "
               << "type: " << ent.type << ", "
               << "seq: " << ent.seq << ", "
               << "}";
...
Cache::Cache(ExtentPlacementManager& epm)
    : epm(epm)
    , lru(crimson::common::get_conf<Option::size_t>("seastore_cache_lru_size"))
{
...
Cache::retire_extent_ret Cache::retire_extent_addr(Transaction& t, paddr_t addr, extent_len_t length)
{
    ...
    if (ext) {
        ...
        ext = CachedExtent::make_cached_extent_ref<RetiredExtentPlaceholder>(length);
        ext->init(CachedExtent::extent_state_t::CLEAN, addr, PLACEMENT_HINT_NULL, NULL_GENERATION, TRANS_ID_NULL);
        DEBUGT("retire {}~{} as placeholder, add extent -- {}", t, addr, length, *ext);
...
void Cache::register_metrics()
{
...

    {
        ...
        for (auto& [src, src_label] : labels_by_src) {
            ...
            for (auto& [ext, ext_label] : labels_by_ext) {
                ...
                metrics.add_group(
                    "cache",
                    {
                        sm::make_counter("trans_invalidated_by_extent",
                                         counter,
            ...
            if (src == src_t::READ) {
                ...
                metrics.add_group("cache",
                                  {
                                      sm::make_counter("invalidated_extents",
                                                       efforts.read.num,
        ...
        for (auto& [src, src_label] : labels_by_src) {
            ...
            for (auto& effort_name : committed_effort_names) {
                auto& effort_by_ext = [&efforts, &effort_name]() -> counter_by_extent_t<io_stat_t>& {
                    if (effort_name == "READ") {
            ...
            for (auto& [ext, ext_label] : labels_by_ext) {
                ...
                metrics.add_group("cache",
                                  {
                                      sm::make_counter("committed_delta_bytes",
                                                       value,
    ...
    metrics.add_group(
        ...
        {
            sm::make_counter(
                "cached_extents", [this] { return extents.size(); }, sm::description("total number of cached extents")),
            sm::make_counter(
                "cached_extent_bytes",
                [this] { return extents.get_bytes(); },
                sm::description("total bytes of cached extents")),
            sm::make_counter(
                "dirty_extents", [this] { return dirty.size(); }, sm::description("total number of dirty extents")),
            sm::make_counter("dirty_extent_bytes", stats.dirty_bytes, sm::description("total bytes of dirty extents")),
            sm::make_counter(
                "cache_lru_size_bytes",
                [this] { return lru.get_current_contents_bytes(); },
                sm::description("total bytes pinned by the lru")),
            sm::make_counter(
                "cache_lru_size_extents",
                [this] { return lru.get_current_contents_extents(); },
    ...
    auto register_tree_metrics = [&labels_by_src, &onode_label, &omap_label, this](
        ...
        metrics.add_group(
            "cache",
            {sm::make_counter("tree_depth", tree_depth, sm::description("the depth of tree"), {tree_label}),
             sm::make_counter(
                 "tree_extents_num", tree_extents_num, sm::description("num of extents of the tree"), {tree_label})});
        for (auto& [src, src_label] : labels_by_src) {
            if (src == src_t::READ) {
                // READ transaction won't contain any tree inserts and erases
                continue;
            }
            if (is_background_transaction(src) && (tree_label == onode_label || tree_label == omap_label)) {
    ...
    for (uint8_t src2_int = 0; src2_int < num_srcs; ++src2_int) {
        ...
        for (uint8_t src1_int = src2_int; src1_int < num_srcs; ++src1_int) {
            ...
            if ((src1 == Transaction::src_t::READ && src2 == Transaction::src_t::READ) ||
                (src1 == Transaction::src_t::TRIM_DIRTY && src2 == Transaction::src_t::TRIM_DIRTY) ||
                (src1 == Transaction::src_t::CLEANER_MAIN && src2 == Transaction::src_t::CLEANER_MAIN) ||
                (src1 == Transaction::src_t::CLEANER_COLD && src2 == Transaction::src_t::CLEANER_COLD) ||
                (src1 == Transaction::src_t::TRIM_ALLOC && src2 == Transaction::src_t::TRIM_ALLOC)) {
                continue;
            }
            ...
            metrics.add_group("cache",
                              {
                                  sm::make_counter("trans_srcs_invalidated",
...
void Cache::commit_retire_extent(Transaction& t, CachedExtentRef ref)
{
    remove_extent(ref);
...
void Cache::commit_replace_extent(Transaction& t, CachedExtentRef next, CachedExtentRef prev)
{
    ...
    if (prev->get_type() == extent_types_t::ROOT) {
        assert(prev->is_stable_clean() || prev->primary_ref_list_hook.is_linked());
        if (prev->is_dirty()) {
...
void Cache::invalidate_extent(Transaction& t, CachedExtent& extent)
{
...
void Cache::mark_transaction_conflicted(Transaction& t, CachedExtent& conflicting_extent)
{
    ...
    auto& efforts = get_by_src(stats.invalidated_efforts_by_src, t.get_src());
    ++efforts.total_trans_invalidated;

    auto& counter = get_by_ext(efforts.num_trans_invalidated, conflicting_extent.get_type());
    ++counter;
    ...
    if (t.get_src() != Transaction::src_t::READ) {
        ...
        if (is_background_transaction(t.get_src())) {
            ...
            get_by_src(stats.invalidated_onode_tree_efforts, t.get_src()).increment(t.onode_tree_stats);
        ...
        get_by_src(stats.invalidated_backref_tree_efforts, t.get_src()).increment(t.backref_tree_stats);
...
void Cache::on_transaction_destruct(Transaction& t)
{
    ...
    SUBTRACET(seastore_t, "", t);
    if (t.get_src() == Transaction::src_t::READ && t.conflicted == false) {
        io_stat_t read_stat;
        for (auto& i : t.read_set) {
            read_stat.increment(i.ref->get_length());
        }
...
CachedExtentRef Cache::alloc_new_extent_by_type(Transaction& t,          ///< [in, out] current transaction
...
)
{
    ...
    SUBDEBUGT(seastore_cache, "allocate {} {}B, hint={}, gen={}", t, type, length, hint, rewrite_gen_printer_t{gen});
    switch (type) {
    case extent_types_t::ROOT: ceph_assert(0 == "ROOT is never directly alloc'd"); return CachedExtentRef();
    case extent_types_t::LADDR_INTERNAL:
        return alloc_new_extent<lba_manager::btree::LBAInternalNode>(t, length, hint, gen);
    case extent_types_t::LADDR_LEAF: return alloc_new_extent<lba_manager::btree::LBALeafNode>(t, length, hint, gen);
    case extent_types_t::ONODE_BLOCK_STAGED: return alloc_new_extent<onode::SeastoreNodeExtent>(t, length, hint, gen);
    case extent_types_t::OMAP_INNER: return alloc_new_extent<omap_manager::OMapInnerNode>(t, length, hint, gen);
    case extent_types_t::OMAP_LEAF: return alloc_new_extent<omap_manager::OMapLeafNode>(t, length, hint, gen);
    case extent_types_t::COLL_BLOCK: return alloc_new_extent<collection_manager::CollectionNode>(t, length, hint, gen);
    case extent_types_t::OBJECT_DATA_BLOCK: return alloc_new_extent<ObjectDataBlock>(t, length, hint, gen);
    case extent_types_t::RETIRED_PLACEHOLDER: ceph_assert(0 == "impossible"); return CachedExtentRef();
    case extent_types_t::TEST_BLOCK: return alloc_new_extent<TestBlock>(t, length, hint, gen);
    case extent_types_t::TEST_BLOCK_PHYSICAL: return alloc_new_extent<TestBlockPhysical>(t, length, hint, gen);
    case extent_types_t::NONE:
    {
        ceph_assert(0 == "NONE is an invalid extent type");
        return CachedExtentRef();
    }
    default: ceph_assert(0 == "impossible"); return CachedExtentRef();
    }
}

CachedExtentRef Cache::duplicate_for_write(Transaction& t, CachedExtentRef i)
{
    LOG_PREFIX(Cache::duplicate_for_write);
    assert(i->is_fully_loaded());

    if (i->is_mutable()) return i;
...
record_t Cache::prepare_record(Transaction& t, const journal_seq_t& journal_head,
                               const journal_seq_t& journal_dirty_tail)
{
    ...
    auto& efforts = get_by_src(stats.committed_efforts_by_src, trans_src);

    // Should be valid due to interruptible future
    io_stat_t read_stat;
    for (auto& i : t.read_set) {
        if (!i.ref->is_valid()) {
            SUBERRORT(seastore_t, "read_set got invalid extent, aborting -- {}", t, *i.ref);
            ceph_abort("no invalid extent allowed in transactions' read_set");
        }
        get_by_ext(efforts.read_by_ext, i.ref->get_type()).increment(i.ref->get_length());
        read_stat.increment(i.ref->get_length());
    }
    t.read_set.clear();
    t.write_set.clear();
    ...
    for (auto& i : t.mutated_block_list) {
        ...
        get_by_ext(efforts.mutate_by_ext, i->get_type()).increment(i->get_length());

        auto delta_bl = i->get_delta();
        auto delta_length = delta_bl.length();
        i->set_modify_time(commit_time);
        DEBUGT("mutated extent with {}B delta -- {}", t, delta_length, *i);
        ...
        if (i->get_type() == extent_types_t::ROOT) {
            SUBTRACET(seastore_t, "writing out root delta {}B -- {}", t, delta_length, *i);
            assert(t.root == i);
            root = t.root;
            record.push_back(delta_info_t{extent_types_t::ROOT,
                                          P_ADDR_NULL,
                                          L_ADDR_NULL,
                                          0,
                                          0,
            ...
            record.push_back(delta_info_t{i->get_type(),
                                          i->get_paddr(),
                                          (i->is_logical() ? i->cast<LogicalCachedExtent>()->get_laddr() : L_ADDR_NULL),
    ...
    for (auto& i : t.retired_set) {
        get_by_ext(efforts.retire_by_ext, i->get_type()).increment(i->get_length());
        retire_stat.increment(i->get_length());
        DEBUGT("retired and remove extent -- {}", t, *i);
        commit_retire_extent(t, i);
        if (is_backref_mapped_extent_node(i) || is_retired_placeholder(i->get_type())) {
            rel_delta.alloc_blk_ranges.emplace_back(i->get_paddr(), L_ADDR_NULL, i->get_length(), i->get_type());
        }
    }
    ...
    for (auto& i : t.inline_block_list) {
        if (!i->is_valid()) {
            ...
            get_by_ext(efforts.fresh_invalid_by_ext, i->get_type()).increment(i->get_length());
        }
        else {
            TRACET("fresh inline extent -- {}", t, *i);
        }
        fresh_stat.increment(i->get_length());
        get_by_ext(efforts.fresh_inline_by_ext, i->get_type()).increment(i->get_length());
        ...
        record.push_back(extent_t{i->get_type(),
                                  i->is_logical() ? i->cast<LogicalCachedExtent>()->get_laddr()
                                                  : (is_lba_node(i->get_type())
                                                         ? i->cast<lba_manager::btree::LBANode>()->get_node_meta().begin
                                                         : L_ADDR_NULL),
                                  std::move(bl)},
                         modify_time);
        if (i->is_valid() && is_backref_mapped_extent_node(i)) {
            alloc_delta.alloc_blk_ranges.emplace_back(
                i->get_paddr(),
                i->is_logical()
                    ? i->cast<LogicalCachedExtent>()->get_laddr()
                    : (is_lba_node(i->get_type()) ? i->cast<lba_manager::btree::LBANode>()->get_node_meta().begin
                                                  : L_ADDR_NULL),
                i->get_length(),
                i->get_type());
        }
    ...
    for (auto& i : t.written_ool_block_list) {
        ...
        assert(!i->is_inline());
        get_by_ext(efforts.fresh_ool_by_ext, i->get_type()).increment(i->get_length());
        i->prepare_commit();
    ...
    for (auto& i : t.existing_block_list) {
        if (i->is_valid()) {
            alloc_delta.alloc_blk_ranges.emplace_back(
                i->get_paddr(), i->cast<LogicalCachedExtent>()->get_laddr(), i->get_length(), i->get_type());
        }
    }
    ...
    if (is_background_transaction(trans_src)) {
        ...
        if (!maybe_dirty_tail.has_value()) {
            ...
            SUBINFOT(seastore_t, "dirty_tail all trimmed, set to head {}, src={}", t, dirty_tail, trans_src);
        }
        else if (*maybe_dirty_tail == JOURNAL_SEQ_NULL) {
            dirty_tail = journal_dirty_tail;
            SUBINFOT(seastore_t, "dirty_tail is pending, set to {}, src={}", t, dirty_tail, trans_src);
        }
        else {
        ...
        if (!maybe_alloc_tail.has_value()) {
            ...
            SUBINFOT(seastore_t, "alloc_tail all trimmed, set to head {}, src={}", t, alloc_tail, trans_src);
        }
    ...
    ceph_assert(t.get_fresh_block_stats().num == t.inline_block_list.size() + t.written_ool_block_list.size() +
                                                     t.num_delayed_invalid_extents + t.num_allocated_invalid_extents);

    auto& ool_stats = t.get_ool_write_stats();
    ceph_assert(ool_stats.extents.num == t.written_ool_block_list.size());

    if (record.is_empty()) {
        SUBINFOT(seastore_t, "record to submit is empty, src={}", t, trans_src);
        assert(t.onode_tree_stats.is_clear());
        assert(t.omap_tree_stats.is_clear());
        assert(t.lba_tree_stats.is_clear());
        assert(t.backref_tree_stats.is_clear());
    ...
    if (is_background_transaction(trans_src)) {
        ...
        get_by_src(stats.committed_onode_tree_efforts, trans_src).increment(t.onode_tree_stats);
        stats.omap_tree_extents_num += t.omap_tree_stats.extents_num_delta;
        ceph_assert(stats.omap_tree_extents_num >= 0);
        get_by_src(stats.committed_omap_tree_efforts, trans_src).increment(t.omap_tree_stats);
    }
    ...
    get_by_src(stats.committed_lba_tree_efforts, trans_src).increment(t.lba_tree_stats);
    ...
    get_by_src(stats.committed_backref_tree_efforts, trans_src).increment(t.backref_tree_stats);

    ++(efforts.num_trans);
    efforts.num_ool_records += ool_stats.num_records;
    efforts.ool_record_metadata_bytes += ool_stats.md_bytes;
    efforts.ool_record_data_bytes += ool_stats.get_data_bytes();
    efforts.inline_record_metadata_bytes += (record.size.get_raw_mdlength() - record.get_delta_size());
    ...
    else if (trans_src == Transaction::src_t::CLEANER_MAIN || trans_src == Transaction::src_t::CLEANER_COLD) {
...
void Cache::backref_batch_update(std::vector<backref_entry_ref>&& list, const journal_seq_t& seq)
{
    ...
    if (iter == backref_entryrefs_by_seq.end()) {
        ...
        iter->second.insert(
            iter->second.end(), std::make_move_iterator(list.begin()), std::make_move_iterator(list.end()));
    }
}

void Cache::complete_commit(Transaction& t, paddr_t final_block_start, journal_seq_t start_seq)
{
    LOG_PREFIX(Cache::complete_commit);
    SUBTRACET(seastore_t, "final_block_start={}, start_seq={}", t, final_block_start, start_seq);
    ...
    t.for_each_fresh_block([&](const CachedExtentRef& i) {
        ...
        DEBUGT("add extent as fresh, inline={} -- {}", t, is_inline, *i);
        const auto t_src = t.get_src();
        i->invalidate_hints();
        add_extent(i, &t_src);
        epm.commit_space_used(i->get_paddr(), i->get_length());
        if (is_backref_mapped_extent_node(i)) {
            DEBUGT("backref_list new {} len {}", t, i->get_paddr(), i->get_length());
            backref_list.emplace_back(std::make_unique<backref_entry_t>(
                i->get_paddr(),
                i->is_logical()
                    ? i->cast<LogicalCachedExtent>()->get_laddr()
                    : (is_lba_node(i->get_type()) ? i->cast<lba_manager::btree::LBANode>()->get_node_meta().begin
            ...
            add_backref_extent(i->get_paddr(), i->cast<backref::BackrefNode>()->get_node_meta().begin, i->get_type());
        }
        else {
            ERRORT("{}", t, *i);
    ...
    for (auto& i : t.retired_set) {
        i->dirty_from_or_retired_at = start_seq;
        if (is_backref_mapped_extent_node(i) || is_retired_placeholder(i->get_type())) {
            DEBUGT("backref_list free {} len {}", t, i->get_paddr(), i->get_length());
            backref_list.emplace_back(std::make_unique<backref_entry_t>(
                i->get_paddr(), L_ADDR_NULL, i->get_length(), i->get_type(), start_seq));
        }
        else if (is_backref_node(i->get_type())) {
    ...
    for (auto& i : t.existing_block_list) {
        if (i->is_valid()) {
            ...
            DEBUGT("backref_list new existing {} len {}", t, i->get_paddr(), i->get_length());
            backref_list.emplace_back(std::make_unique<backref_entry_t>(i->get_paddr(),
                                                                        i->cast<LogicalCachedExtent>()->get_laddr(),
                                                                        i->get_length(),
                                                                        i->get_type(),
                                                                        start_seq));
...
void Cache::init()
{
    ...
    root = new RootBlock();
    root->init(CachedExtent::extent_state_t::CLEAN, P_ADDR_ROOT, PLACEMENT_HINT_NULL, NULL_GENERATION, TRANS_ID_NULL);
    INFO("init root -- {}", *root);
    extents.insert(*root);
}
...
Cache::mkfs_iertr::future<> Cache::mkfs(Transaction& t)
{
    ...
    INFOT("create root", t);
    return get_root(t)
        .si_then([this, &t](auto croot) {
            duplicate_for_write(t, croot);
            return mkfs_iertr::now();
        })
        .handle_error_interruptible(mkfs_iertr::pass_further{},
                                    crimson::ct_error::assert_all{"Invalid error in Cache::mkfs"});
}
...
Cache::replay_delta_ret Cache::replay_delta(journal_seq_t journal_seq, paddr_t record_base, const delta_info_t& delta,
                                            const journal_seq_t& dirty_tail, const journal_seq_t& alloc_tail,
                                            sea_time_point modify_time)
{
     ...
     * have been rewritten.
     */
    if (delta.paddr != P_ADDR_NULL && delta.paddr.get_addr_type() == paddr_types_t::SEGMENT) {
        ...
        if (seg_info) {
            ...
            if (delta_paddr_segment_seq != delta.ext_seq || delta_paddr_segment_type != delta.seg_type) {
    ...
    if (delta.type == extent_types_t::ALLOC_INFO) {
        if (journal_seq < alloc_tail) {
            DEBUG("journal_seq {} < alloc_tail {}, don't replay {}", journal_seq, alloc_tail, delta);
            return replay_delta_ertr::make_ready_future<bool>(false);
        }
        ...
        for (auto& alloc_blk : alloc_delta.alloc_blk_ranges) {
            ...
            DEBUG("replay alloc_blk {}~{} {}, journal_seq: {}",
                  ...
                  journal_seq);
            backref_list.emplace_back(std::make_unique<backref_entry_t>(
                alloc_blk.paddr, alloc_blk.laddr, alloc_blk.len, alloc_blk.type, journal_seq));
        }
        if (!backref_list.empty()) {
    ...
    if (journal_seq < dirty_tail) {
        DEBUG("journal_seq {} < dirty_tail {}, don't replay {}", journal_seq, dirty_tail, delta);
        return replay_delta_ertr::make_ready_future<bool>(false);
    }

    if (delta.type == extent_types_t::ROOT) {
        TRACE(
            "replay root delta at {} {}, remove extent ... -- {}, prv_root={}", journal_seq, record_base, delta, *root);
        remove_extent(root);
        root->apply_delta_and_adjust_crc(record_base, delta.bl);
        root->dirty_from_or_retired_at = journal_seq;
        root->state = CachedExtent::extent_state_t::DIRTY;
        DEBUG("replayed root delta at {} {}, add extent -- {}, root={}", journal_seq, record_base, delta, *root);
        root->set_modify_time(modify_time);
        add_extent(root);
        return replay_delta_ertr::make_ready_future<bool>(true);
    }
    else {
        auto _get_extent_if_cached = [this](paddr_t addr) -> get_extent_ertr::future<CachedExtentRef> {
            ...
            if (ret) {
                ...
                return ret->wait_io().then([ret] { return ret; });
            }
            else {
        ...
        auto extent_fut = (delta.pversion == 0 ?
                               ...
                               _get_extent_by_type(
                                   ...
                                   [](CachedExtent&) {},
                                   [](CachedExtent&) {})
                                               : _get_extent_if_cached(delta.paddr))
                              .handle_error(replay_delta_ertr::pass_further{},
                                            crimson::ct_error::assert_all{"Invalid error in Cache::replay_delta"});
        return extent_fut.safe_then([=, this, &delta](auto extent) {
            ...
            DEBUG("replay extent delta at {} {} ... -- {}, prv_extent={}", journal_seq, record_base, delta, *extent);
            ...
            if (extent->version == 1) {
                ...
                DEBUG("replayed extent delta at {} {} -- {}, extent={}", journal_seq, record_base, delta, *extent);
            }
            mark_dirty(extent);
            return replay_delta_ertr::make_ready_future<bool>(true);
        });
...
Cache::get_next_dirty_extents_ret Cache::get_next_dirty_extents(Transaction& t, journal_seq_t seq, size_t max_bytes)
{
    LOG_PREFIX(Cache::get_next_dirty_extents);
    if (dirty.empty()) {
        DEBUGT("max_bytes={}B, seq={}, dirty is empty", t, max_bytes, seq);
    }
    else {
        DEBUGT("max_bytes={}B, seq={}, dirty_from={}", t, max_bytes, seq, dirty.begin()->get_dirty_from());
    }
    std::vector<CachedExtentRef> cand;
    size_t bytes_so_far = 0;
    for (auto i = dirty.begin(); i != dirty.end() && bytes_so_far < max_bytes; ++i) {
        ...
        if (dirty_from < seq) {
            ...
            if (!cand.empty() && cand.back()->get_dirty_from() > dirty_from) {
                ERRORT("dirty extents are not ordered by dirty_from -- last={}, next={}", t, *cand.back(), *i);
                ceph_abort();
            }
    ...
    return seastar::do_with(std::move(cand), decltype(cand)(), [FNAME, this, &t](auto& cand, auto& ret) {
        return trans_intr::do_for_each(
                   cand,
                   [FNAME, this, &t, &ret](auto& ext) {
                       TRACET("waiting on extent -- {}", t, *ext);
                       return trans_intr::make_interruptible(ext->wait_io())
            ...
            .then_interruptible([&ret] { return std::move(ret); });
    });
}
...
Cache::get_root_ret Cache::get_root(Transaction& t)
{
    ...
    if (t.root) {
        ...
        return t.root->wait_io().then([&t] { return get_root_iertr::make_ready_future<RootBlockRef>(t.root); });
    }
    else {
        DEBUGT("root not on t -- {}", t, *root);
        t.root = root;
        t.add_to_read_set(root);
        return root->wait_io().then([root = root] { return get_root_iertr::make_ready_future<RootBlockRef>(root); });
    }
}

Cache::get_extent_ertr::future<CachedExtentRef> Cache::_get_extent_by_type(extent_types_t type, paddr_t offset,
    ...
    return [=, this, extent_init_func = std::move(extent_init_func)]() mutable {
        ...
        case extent_types_t::BACKREF_INTERNAL:
            return get_extent<backref::BackrefInternalNode>(
                ...
                .safe_then([](auto extent) { return CachedExtentRef(extent.detach(), false /* add_ref */); });
        ...
        case extent_types_t::TEST_BLOCK:
            return get_extent<TestBlock>(offset, length, p_metric_key, std::move(extent_init_func), std::move(on_cache))
                .safe_then([](auto extent) { return CachedExtentRef(extent.detach(), false /* add_ref */); });
        ...
        default: ceph_assert(0 == "impossible"); return get_extent_ertr::make_ready_future<CachedExtentRef>();
        }
    }()
