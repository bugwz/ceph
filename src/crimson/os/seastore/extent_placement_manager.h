...
public:
    ...
    virtual alloc_write_iertr::future<> alloc_write_ool_extents(Transaction& t,
...
{
public:
    SegmentedOolWriter(data_category_t category, rewrite_gen_t gen, SegmentProvider& sp, SegmentSeqAllocator& ssa);

    open_ertr::future<> open() final { return record_submitter.open(false).discard_result(); }

    alloc_write_iertr::future<> alloc_write_ool_extents(Transaction& t,
                                                        std::list<LogicalCachedExtentRef>& extents) final;

    close_ertr::future<> close() final
    {
        return write_guard.close().then([this] { return record_submitter.close(); }).safe_then([this] {
            write_guard = seastar::gate();
        });
    }

    paddr_t alloc_paddr(extent_len_t length) final { return make_delayed_temp_paddr(0); }

private:
    alloc_write_iertr::future<> do_write(Transaction& t, std::list<LogicalCachedExtentRef>& extent);

    alloc_write_ertr::future<> write_record(Transaction& t, record_t&& record,
...
public:
    ...
    open_ertr::future<> open() final { return open_ertr::now(); }
...
struct reserve_cleaner_result_t
{
    ...
    bool is_successful() const { return reserve_main_success && reserve_cold_success; }
};
...
struct io_usage_t
{
    ...
    friend std::ostream& operator<<(std::ostream& out, const io_usage_t& usage)
    {
        return out << "io_usage_t("
                   << "inline_usage=" << usage.inline_usage << ", main_cleaner_usage=" << usage.cleaner_usage.main_usage
...
struct reserve_io_result_t
{
    ...
    bool is_successful() const { return reserve_inline_success && cleaner_result.is_successful(); }
};
...
public:
    ExtentPlacementManager()
        : ool_segment_seq_allocator(std::make_unique<SegmentSeqAllocator>(segment_type_t::OOL))
    {
    ...
    void set_extent_callback(ExtentCallbackInterface* cb) { background_process.set_extent_callback(cb); }

    journal_type_t get_journal_type() const { return background_process.get_journal_type(); }
    ...
    store_statfs_t get_stat() const { return background_process.get_stat(); }

    using mount_ertr = crimson::errorator<crimson::ct_error::input_output_error>;
    using mount_ret = mount_ertr::future<>;
    mount_ret mount() { return background_process.mount(); }

    using open_ertr = ExtentOolWriter::open_ertr;
    open_ertr::future<> open_for_write();

    void start_scan_space() { return background_process.start_scan_space(); }

    void start_background() { return background_process.start_background(); }
    ...
    alloc_result_t alloc_new_extent(Transaction& t, extent_types_t type, extent_len_t length, placement_hint_t hint,
#ifdef UNIT_TESTS_BUILT
                                    rewrite_gen_t gen, std::optional<paddr_t> external_paddr = std::nullopt
#else
                                    rewrite_gen_t gen
#endif
    )
    {
        ...
        if (gen == INLINE_GENERATION) {
            ...
            addr = data_writers_by_gen[generation_to_writer(gen)]->alloc_paddr(length);
        }
        else {
            assert(category == data_category_t::METADATA);
            assert(md_writers_by_gen[generation_to_writer(gen)]);
            addr = md_writers_by_gen[generation_to_writer(gen)]->alloc_paddr(length);
        }
     ...
     */
    using extents_by_writer_t = std::map<ExtentOolWriter*, std::list<LogicalCachedExtentRef>>;
    ...
    alloc_paddr_iertr::future<> write_delayed_ool_extents(Transaction& t, extents_by_writer_t& alloc_map);
     ...
     */
    alloc_paddr_iertr::future<> write_preallocated_ool_extents(Transaction& t,
                                                               std::list<LogicalCachedExtentRef> extents);

    seastar::future<> stop_background() { return background_process.stop_background(); }

    using close_ertr = ExtentOolWriter::close_ertr;
    close_ertr::future<> close();

    using read_ertr = Device::read_ertr;
    read_ertr::future<> read(paddr_t addr, size_t len, ceph::bufferptr& out)
    {
        assert(devices_by_id[addr.get_device_id()] != nullptr);
        return devices_by_id[addr.get_device_id()]->read(addr, len, out);
    }

    void mark_space_used(paddr_t addr, extent_len_t len) { background_process.mark_space_used(addr, len); }

    void mark_space_free(paddr_t addr, extent_len_t len) { background_process.mark_space_free(addr, len); }

    void commit_space_used(paddr_t addr, extent_len_t len) { return background_process.commit_space_used(addr, len); }
    ...
    void release_projected_usage(const io_usage_t& usage) { background_process.release_projected_usage(usage); }
    ...
    bool check_usage() { return background_process.check_usage(); }

    seastar::future<> run_background_work_until_halt() { return background_process.run_until_halt(); }

private:
    rewrite_gen_t adjust_generation(data_category_t category, extent_types_t type, placement_hint_t hint,
                                    rewrite_gen_t gen)
    {
        if (type == extent_types_t::ROOT) {
            gen = INLINE_GENERATION;
        }
        else if (get_main_backend_type() == backend_type_t::SEGMENTED && is_lba_backref_node(type)) {
            gen = INLINE_GENERATION;
        }
    ...
    ExtentOolWriter* get_writer(placement_hint_t hint, data_category_t category, rewrite_gen_t gen)
    {
        assert(hint < placement_hint_t::NUM_HINTS);
    ...
    public:
        ...
        void init(JournalTrimmerImplRef&& _trimmer, AsyncCleanerRef&& _cleaner, AsyncCleanerRef&& _cold_cleaner)
        {
            ...
            if (_cold_cleaner) {
                ...
                eviction_state.init(crimson::common::get_conf<double>("seastore_multiple_tiers_stop_evict_ratio"),
                                    crimson::common::get_conf<double>("seastore_multiple_tiers_default_evict_ratio"),
                                    crimson::common::get_conf<double>("seastore_multiple_tiers_fast_evict_ratio"));
            }
        }

        journal_type_t get_journal_type() const { return trimmer->get_journal_type(); }

        bool has_cold_tier() const { return cold_cleaner.get() != nullptr; }
        ...
        mount_ret mount()
        {
            ...
            stats = {};
            register_metrics();
            return main_cleaner->mount().safe_then(
                [this] { return has_cold_tier() ? cold_cleaner->mount() : mount_ertr::now(); });
        }
        ...
        backend_type_t get_main_backend_type() const { return get_journal_type(); }

        // Testing interfaces

        bool check_usage() { return main_cleaner->check_usage() && (!has_cold_tier() || cold_cleaner->check_usage()); }

        seastar::future<> run_until_halt();

        bool is_no_background() const { return !trimmer || !main_cleaner; }

    protected:
        state_t get_state() const final { return state; }
    ...
    private:
        ...
        void abort_cleaner_usage(const cleaner_usage_t& usage, const reserve_cleaner_result_t& result);

        reserve_io_result_t try_reserve_io(const io_usage_t& usage);
        void abort_io_usage(const io_usage_t& usage, const reserve_io_result_t& result);
        ...
        bool background_should_run()
        {
            ...
            return main_cleaner_should_run() || cold_cleaner_should_run() || trimmer->should_trim();
        }
        ...
        bool main_cleaner_should_run() const
        {
            ...
            return main_cleaner->should_clean_space() ||
                   (has_cold_tier() && main_cleaner->can_clean_space() && eviction_state.is_fast_mode());
        }
        ...
        {
            assert(is_ready());
            return trimmer->should_block_io_on_trim() || main_cleaner->should_block_io_on_clean() ||
        ...
        struct eviction_state_t
        {
            ...
            void init(double stop_ratio, double default_ratio, double fast_ratio)
            {
            ...
            bool is_stop_mode() const { return eviction_mode == eviction_mode_t::STOP; }

            bool is_default_mode() const { return eviction_mode == eviction_mode_t::DEFAULT; }

            bool is_fast_mode() const { return eviction_mode == eviction_mode_t::FAST; }
            ...
            rewrite_gen_t adjust_generation_with_eviction(rewrite_gen_t gen)
            {
                ...
                case eviction_mode_t::STOP:
                    ...
                    break;
                case eviction_mode_t::DEFAULT: break;
