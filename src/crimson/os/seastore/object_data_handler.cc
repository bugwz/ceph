...
struct extent_to_write_t
{
    ...
    bool is_data() const { return type == type_t::DATA; }

    bool is_zero() const { return type == type_t::ZERO; }

    bool is_existing() const { return type == type_t::EXISTING; }

    laddr_t get_end_addr() const { return addr + len; }

    static extent_to_write_t create_data(laddr_t addr, bufferlist to_write)
    {
        return extent_to_write_t(addr, to_write);
    }

    static extent_to_write_t create_zero(laddr_t addr, extent_len_t len) { return extent_to_write_t(addr, len); }

    static extent_to_write_t create_existing(LBAMappingRef&& pin, laddr_t addr, extent_len_t len)
    {
        assert(pin);
...
struct extent_to_remap_t
{
    ...
    bool is_remap() const { return type == type_t::REMAP; }
    ...
    {
        assert(is_remap());
        return remap_entry(new_offset, new_len);
    }

    remap_entry create_left_remap_entry()
    {
        assert(is_overwrite());
        return remap_entry(0, new_offset);
    }

    remap_entry create_right_remap_entry()
    {
        assert(is_overwrite());
        return remap_entry(new_offset + new_len, pin->get_length() - new_offset - new_len);
    }

    static extent_to_remap_t create_remap(LBAMappingRef&& pin, extent_len_t new_offset, extent_len_t new_len)
    {
        return extent_to_remap_t(type_t::REMAP, std::move(pin), new_offset, new_len);
    }

    static extent_to_remap_t create_overwrite(LBAMappingRef&& pin, extent_len_t new_offset, extent_len_t new_len)
    {
        return extent_to_remap_t(type_t::OVERWRITE, std::move(pin), new_offset, new_len);
    }

private:
    extent_to_remap_t(type_t type, LBAMappingRef&& pin, extent_len_t new_offset, extent_len_t new_len)
        : type(type)
        , pin(std::move(pin))
...
struct extent_to_insert_t
{
    ...
    static extent_to_insert_t create_data(laddr_t addr, extent_len_t len, std::optional<bufferlist> bl)
    {
        return extent_to_insert_t(addr, len, bl);
    }

    static extent_to_insert_t create_zero(laddr_t addr, extent_len_t len) { return extent_to_insert_t(addr, len); }

private:
    extent_to_insert_t(laddr_t addr, extent_len_t len, std::optional<bufferlist> bl)
        : type(type_t::DATA)
        , addr(addr)
        , len(len)
        , bl(bl)
...
overwrite_ops_t prepare_ops_list(lba_pin_list_t& pins_to_remove, extent_to_write_list_t& to_write)
{
    ...
    if (ops.to_remove.size() == 1 && front.is_existing() && back.is_existing()) {
        visitted += 2;
        assert(to_write.size() > 2);
        assert(front.addr == front.pin->get_key());
        assert(back.addr > back.pin->get_key());
        ops.to_remap.push_back(
            extent_to_remap_t::create_overwrite(std::move(front.pin), front.len, back.addr - front.addr - front.len));
        ...
        if (front.is_existing()) {
            ...
            assert(front.addr == front.pin->get_key());
            ops.to_remap.push_back(extent_to_remap_t::create_remap(std::move(front.pin), 0, front.len));
            ops.to_remove.pop_front();
        }
        if (back.is_existing()) {
            visitted++;
            assert(to_write.size() > 1);
            assert(back.addr + back.len == back.pin->get_key() + back.pin->get_length());
            ops.to_remap.push_back(
                extent_to_remap_t::create_remap(std::move(back.pin), back.addr - back.pin->get_key(), back.len));
    ...
    for (auto& region : to_write) {
        if (region.is_data()) {
            ...
            ops.to_insert.push_back(extent_to_insert_t::create_data(region.addr, region.len, region.to_write));
        }
        else if (region.is_zero()) {
            visitted++;
            assert(!(region.to_write.has_value()));
            ops.to_insert.push_back(extent_to_insert_t::create_zero(region.addr, region.len));
        }
    }

    logger().debug("to_remap list size: {}"
                   " to_insert list size: {}"
                   " to_remove list size: {}",
                   ops.to_remap.size(),
                   ops.to_insert.size(),
 ...
 * combining them.
 */
void append_extent_to_write(extent_to_write_list_t& to_write, extent_to_write_t&& to_append)
{
    assert(to_write.empty() || to_write.back().get_end_addr() == to_append.addr);
    if (to_write.empty() || to_write.back().is_data() || to_append.is_data() ||
 ...
 * combining them.
 */
void splice_extent_to_write(extent_to_write_list_t& to_write, extent_to_write_list_t&& to_splice)
{
    if (!to_splice.empty()) {
...
ObjectDataHandler::write_ret do_remappings(context_t ctx, extent_to_remap_list_t& to_remap)
{
    return trans_intr::do_for_each(to_remap, [ctx](auto& region) {
        if (region.is_remap()) {
            return ctx.tm
                .remap_pin<ObjectDataBlock, 1>(ctx.t, std::move(region.pin), std::array{region.create_remap_entry()})
            ...
            return ctx.tm
                .remap_pin<ObjectDataBlock, 2>(
                    ctx.t,
                    std::move(region.pin),
                    std::array{region.create_left_remap_entry(), region.create_right_remap_entry()})
                .si_then([&region](auto pins) {
                    ceph_assert(pins.size() == 2);
                    ceph_assert(region.pin->get_key() == pins[0]->get_key());
                    ceph_assert(region.pin->get_key() + pins[0]->get_length() + region.new_len == pins[1]->get_key());
                    return ObjectDataHandler::write_iertr::now();
                });
        }
        else {
...
ObjectDataHandler::write_ret do_removals(context_t ctx, lba_pin_list_t& to_remove)
{
    return trans_intr::do_for_each(to_remove, [ctx](auto& pin) {
        LOG_PREFIX(object_data_handler.cc::do_removals);
        DEBUGT("decreasing ref: {}", ctx.t, pin->get_key());
        return ctx.tm.dec_ref(ctx.t, pin->get_key())
            .si_then([](auto) {},
                     ObjectDataHandler::write_iertr::pass_further{},
                     crimson::ct_error::assert_all{"object_data_handler::do_removals invalid error"});
    });
}

/// Creates zero/data extents in to_insert
ObjectDataHandler::write_ret do_insertions(context_t ctx, extent_to_insert_list_t& to_insert)
{
    return trans_intr::do_for_each(to_insert, [ctx](auto& region) {
        ...
        if (region.is_data()) {
            ...
            DEBUGT("allocating extent: {}~{}", ctx.t, region.addr, region.len);
            return ctx.tm.alloc_extent<ObjectDataBlock>(ctx.t, region.addr, region.len).si_then([&region](auto extent) {
                if (extent->get_laddr() != region.addr) {
                    logger().debug("object_data_handler::do_insertions alloc got addr {},"
                                   " should have been {}",
                                   extent->get_laddr(),
                                   region.addr);
                }
            ...
            DEBUGT("reserving: {}~{}", ctx.t, region.addr, region.len);
            return ctx.tm.reserve_region(ctx.t, region.addr, region.len).si_then([FNAME, ctx, &region](auto pin) {
                ceph_assert(pin->get_length() == region.len);
                if (pin->get_key() != region.addr) {
                    ERRORT("inconsistent laddr: pin: {} region {}", ctx.t, pin->get_key(), region.addr);
                }
...
std::ostream& operator<<(std::ostream& out, const overwrite_operation_t& operation)
{
    switch (operation) {
    case overwrite_operation_t::UNKNOWN: return out << "UNKNOWN";
    case overwrite_operation_t::OVERWRITE_ZERO: return out << "OVERWRITE_ZERO";
    case overwrite_operation_t::MERGE_EXISTING: return out << "MERGE_EXISTING";
    case overwrite_operation_t::SPLIT_EXISTING: return out << "SPLIT_EXISTING";
    default: return out << "!IMPOSSIBLE_OPERATION";
    }
}
...
public:
    extent_len_t get_left_size() const { return data_begin - pin_begin; }

    extent_len_t get_left_extent_size() const { return aligned_data_begin - pin_begin; }

    extent_len_t get_left_alignment_size() const { return data_begin - aligned_data_begin; }

    extent_len_t get_right_size() const { return pin_end - data_end; }

    extent_len_t get_right_extent_size() const { return pin_end - aligned_data_end; }

    extent_len_t get_right_alignment_size() const { return aligned_data_end - data_end; }

    extent_len_t get_aligned_data_size() const { return aligned_data_end - aligned_data_begin; }

    extent_len_t get_pins_size() const { return pin_end - pin_begin; }

    friend std::ostream& operator<<(std::ostream& out, const overwrite_plan_t& overwrite_plan)
    {
        return out << "overwrite_plan_t("
                   << "pin_begin=" << overwrite_plan.pin_begin << ", pin_end=" << overwrite_plan.pin_end
                   << ", left_paddr=" << overwrite_plan.left_paddr << ", right_paddr=" << overwrite_plan.right_paddr
                   << ", data_begin=" << overwrite_plan.data_begin << ", data_end=" << overwrite_plan.data_end
    ...
    overwrite_plan_t(laddr_t offset, extent_len_t len, const lba_pin_list_t& pins, extent_len_t block_size,
                     Transaction& t)
...
private:
    ...
    void evaluate_operations(Transaction& t)
    {
        ...
        auto can_merge = [](Transaction& t, paddr_t paddr) {
            ...
            else if (t.get_extent(paddr, &ext) == Transaction::get_extent_ret::PRESENT) {
        ...
        while (left_operation == overwrite_operation_t::UNKNOWN || right_operation == overwrite_operation_t::UNKNOWN) {
 ...
 */
using operate_ret_bare = std::pair<std::optional<extent_to_write_t>, std::optional<bufferptr>>;
...
operate_ret operate_left(context_t ctx, LBAMappingRef& pin, const overwrite_plan_t& overwrite_plan)
{
    if (overwrite_plan.get_left_size() == 0) {
        return get_iertr::make_ready_future<operate_ret_bare>(std::nullopt, std::nullopt);
    }
    ...
    if (overwrite_plan.left_operation == overwrite_operation_t::OVERWRITE_ZERO) {
        ...
        return get_iertr::make_ready_future<operate_ret_bare>(
            (zero_extent_len == 0
                 ? std::nullopt
                 : std::make_optional(extent_to_write_t::create_zero(overwrite_plan.pin_begin, zero_extent_len))),
            (zero_prepend_len == 0 ? std::nullopt
                                   : std::make_optional(bufferptr(ceph::buffer::create(zero_prepend_len, 0)))));
    }
        ...
        if (prepend_len == 0) {
            ...
            return ctx.tm.read_pin<ObjectDataBlock>(ctx.t, pin->duplicate())
                .si_then([prepend_len, off](auto left_extent) {
                    return get_iertr::make_ready_future<operate_ret_bare>(
                        std::nullopt, std::make_optional(bufferptr(left_extent->get_bptr(), off, prepend_len)));
                });
        }
    }
    else {
        ...
        std::optional<extent_to_write_t> left_to_write_extent =
            std::make_optional(extent_to_write_t::create_existing(pin->duplicate(), pin->get_key(), extent_len));

        auto prepend_len = overwrite_plan.get_left_alignment_size();
        if (prepend_len == 0) {
            return get_iertr::make_ready_future<operate_ret_bare>(std::move(left_to_write_extent), std::nullopt);
        }
        else {
            extent_len_t off = pin->get_intermediate_offset();
            return ctx.tm.read_pin<ObjectDataBlock>(ctx.t, pin->duplicate())
                .si_then([prepend_offset = extent_len + off,
                          prepend_len,
                          left_to_write_extent = std::move(left_to_write_extent)](auto left_extent) mutable {
                    return get_iertr::make_ready_future<operate_ret_bare>(
                        std::move(left_to_write_extent),
                        std::make_optional(bufferptr(left_extent->get_bptr(), prepend_offset, prepend_len)));
                });
        }
    }
};
...
operate_ret operate_right(context_t ctx, LBAMappingRef& pin, const overwrite_plan_t& overwrite_plan)
{
    ...
    if (overwrite_plan.right_operation == overwrite_operation_t::OVERWRITE_ZERO) {
        ...
        assert_aligned(zero_extent_len);
        return get_iertr::make_ready_future<operate_ret_bare>(
            (zero_extent_len == 0 ? std::nullopt
                                  : std::make_optional(extent_to_write_t::create_zero(overwrite_plan.aligned_data_end,
                                                                                      zero_extent_len))),
            (zero_suffix_len == 0 ? std::nullopt
                                  : std::make_optional(bufferptr(ceph::buffer::create(zero_suffix_len, 0)))));
    }
        ...
        if (append_len == 0) {
            ...
            auto append_offset = overwrite_plan.data_end - right_pin_begin + pin->get_intermediate_offset();
            return ctx.tm.read_pin<ObjectDataBlock>(ctx.t, pin->duplicate())
                .si_then([append_offset, append_len](auto right_extent) {
                    return get_iertr::make_ready_future<operate_ret_bare>(
                        std::nullopt,
                        std::make_optional(bufferptr(right_extent->get_bptr(), append_offset, append_len)));
                });
        }
    }
    else {
        ...
        assert(extent_len);
        std::optional<extent_to_write_t> right_to_write_extent = std::make_optional(
            extent_to_write_t::create_existing(pin->duplicate(), overwrite_plan.aligned_data_end, extent_len));

        auto append_len = overwrite_plan.get_right_alignment_size();
        if (append_len == 0) {
            return get_iertr::make_ready_future<operate_ret_bare>(std::move(right_to_write_extent), std::nullopt);
        }
        else {
            auto append_offset = overwrite_plan.data_end - right_pin_begin + pin->get_intermediate_offset();
            return ctx.tm.read_pin<ObjectDataBlock>(ctx.t, pin->duplicate())
                .si_then([append_offset, append_len, right_to_write_extent = std::move(right_to_write_extent)](
...
template<typename F> auto with_object_data(ObjectDataHandler::context_t ctx, F&& f)
{
    return seastar::do_with(
        ctx.onode.get_layout().object_data.get(), std::forward<F>(f), [ctx](auto& object_data, auto& f) {
            return std::invoke(f, object_data).si_then([ctx, &object_data] {
...
template<typename F> auto with_objects_data(ObjectDataHandler::context_t ctx, F&& f)
{
    ...
    return seastar::do_with(
        ...
        [ctx](auto& object_data, auto& d_object_data, auto& f) {
            return std::invoke(f, object_data, d_object_data).si_then([ctx, &object_data, &d_object_data] {
                if (object_data.must_update()) {
                    ctx.onode.get_mutable_layout(ctx.t).object_data.update(object_data);
                }
                if (d_object_data.must_update()) {
                    ctx.d_onode->get_mutable_layout(ctx.t).object_data.update(d_object_data);
                }
                return seastar::now();
            });
        });
...
ObjectDataHandler::write_ret ObjectDataHandler::prepare_data_reservation(context_t ctx, object_data_t& object_data,
                                                                         extent_len_t size)
{
    ...
    if (!object_data.is_null()) {
        ...
        DEBUGT("reserving: {}~{}", ctx.t, ctx.onode.get_data_hint(), max_object_size);
        return ctx.tm.reserve_region(ctx.t, ctx.onode.get_data_hint(), max_object_size)
            .si_then([max_object_size = max_object_size, &object_data](auto pin) {
                ceph_assert(pin->get_length() == max_object_size);
                object_data.update_reserved(pin->get_key(), pin->get_length());
                return write_iertr::now();
            });
    }
}

ObjectDataHandler::clear_ret ObjectDataHandler::trim_data_reservation(context_t ctx, object_data_t& object_data,
                                                                      extent_len_t size)
{
    ceph_assert(!object_data.is_null());
    ceph_assert(size <= object_data.get_reserved_data_len());
    return seastar::do_with(
        lba_pin_list_t(), extent_to_write_list_t(), [ctx, size, &object_data](auto& pins, auto& to_write) {
            LOG_PREFIX(ObjectDataHandler::trim_data_reservation);
            DEBUGT(
                "object_data: {}~{}", ctx.t, object_data.get_reserved_data_base(), object_data.get_reserved_data_len());
            return ctx.tm
                .get_pins(
                    ctx.t, object_data.get_reserved_data_base() + size, object_data.get_reserved_data_len() - size)
                .si_then([ctx, size, &pins, &object_data, &to_write](auto _pins) {
                    ...
                    ceph_assert(pin.get_key() <= object_data.get_reserved_data_base() + size);
                    auto pin_offset = pin.get_key() - object_data.get_reserved_data_base();
                    if ((pin.get_key() == (object_data.get_reserved_data_base() + size)) || (pin.get_val().is_zero())) {
                        ...
                        to_write.push_back(extent_to_write_t::create_zero(
                            pin.get_key(), object_data.get_reserved_data_len() - pin_offset));
                        return clear_iertr::now();
                    }
                    else {
                        ...
                        if (append_len == 0) {
                            ...
                            TRACET("First pin overlaps the boundary and has aligned data"
                                   ...
                                   size - pin_offset);
                            to_write.push_back(
                                extent_to_write_t::create_existing(pin.duplicate(), pin.get_key(), size - pin_offset));
                            to_write.push_back(
                                extent_to_write_t::create_zero(object_data.get_reserved_data_base() + roundup_size,
                            ...
                            return ctx.tm.read_pin<ObjectDataBlock>(ctx.t, pin.duplicate())
                                .si_then(
                                    [ctx, size, pin_offset, append_len, roundup_size, &pin, &object_data, &to_write](
                                        auto extent) {
                                        bufferlist bl;
                                        bl.append(bufferptr(
                                            extent->get_bptr(), pin.get_intermediate_offset(), size - pin_offset));
                                        ...
                                        TRACET("First pin overlaps the boundary and has unaligned data"
                                               ...
                                               pin.get_key(),
                                               bl.length());
                                        to_write.push_back(extent_to_write_t::create_data(pin.get_key(), bl));
                                        to_write.push_back(extent_to_write_t::create_zero(
                ...
                .si_then([ctx, size, &to_write, &object_data, &pins] {
                    return seastar::do_with(prepare_ops_list(pins, to_write), [ctx, size, &object_data](auto& ops) {
                        return do_remappings(ctx, ops.to_remap)
                            .si_then([ctx, &ops] { return do_removals(ctx, ops.to_remove); })
                            .si_then([ctx, &ops] { return do_insertions(ctx, ops.to_insert); })
 ...
 */
extent_to_write_list_t get_to_writes_with_zero_buffer(const extent_len_t block_size, laddr_t offset, extent_len_t len,
                                                      std::optional<bufferptr>&& headptr,
                                                      std::optional<bufferptr>&& tailptr)
{
    ...
    auto right = tailptr ? (offset + len + tailptr->length()) : (offset + len);

    assert((headptr && ((zero_left - left) == p2roundup(headptr->length(), block_size))) ^
           (!headptr && (zero_left == left)));
    assert((tailptr && ((right - zero_right) == p2roundup(tailptr->length(), block_size))) ^
    ...
    if (zero_right <= zero_left) {
        ...
        bl.append_zero(right - left - bl.length() - (tailptr ? tailptr->length() : 0));
        if (tailptr) {
            bl.append(*tailptr);
        }
...
ObjectDataHandler::write_ret ObjectDataHandler::overwrite(context_t ctx, laddr_t offset, extent_len_t len,
                                                          std::optional<bufferlist>&& bl, lba_pin_list_t&& _pins)
{
    if (bl.has_value()) {
    ...
    return seastar::do_with(
        ...
        [ctx, len, offset, overwrite_plan, bl = std::move(bl)](auto& pins, auto& to_write) mutable {
            LOG_PREFIX(ObjectDataHandler::overwrite);
            DEBUGT("overwrite: {}~{}", ctx.t, offset, len);
            ceph_assert(pins.size() >= 1);
            DEBUGT("overwrite: split overwrite_plan {}", ctx.t, overwrite_plan);

            return operate_left(ctx, pins.front(), overwrite_plan)
                .si_then([ctx, len, offset, overwrite_plan, bl = std::move(bl), &to_write, &pins](auto p) mutable {
                    ...
                    return operate_right(ctx, pins.back(), overwrite_plan)
                        .si_then([ctx,
                                  len,
                                  offset,
                            ...
                            if (bl.has_value()) {
                                ...
                                splice_extent_to_write(to_write, get_to_writes(write_offset, write_bl));
                            }
                            else {
                                splice_extent_to_write(
                                    to_write,
                                    get_to_writes_with_zero_buffer(
                                        ctx.tm.get_block_size(), offset, len, std::move(headptr), std::move(tailptr)));
                            }
                            if (right_extent) {
                            ...
                            return seastar::do_with(prepare_ops_list(pins, to_write), [ctx](auto& ops) {
                                return do_remappings(ctx, ops.to_remap)
                                    .si_then([ctx, &ops] { return do_removals(ctx, ops.to_remove); })
                                    .si_then([ctx, &ops] { return do_insertions(ctx, ops.to_insert); });
                            });
...
ObjectDataHandler::zero_ret ObjectDataHandler::zero(context_t ctx, objaddr_t offset, extent_len_t len)
{
    return with_object_data(ctx, [this, ctx, offset, len](auto& object_data) {
        ...
        DEBUGT("zero to {}~{}, object_data: {}~{}, is_null {}",
               ...
               object_data.get_reserved_data_len(),
               object_data.is_null());
        return prepare_data_reservation(ctx, object_data, p2roundup(offset + len, ctx.tm.get_block_size()))
            .si_then([this, ctx, offset, len, &object_data] {
                auto logical_offset = object_data.get_reserved_data_base() + offset;
                return ctx.tm.get_pins(ctx.t, logical_offset, len).si_then([this, ctx, logical_offset, len](auto pins) {
                    return overwrite(ctx, logical_offset, len, std::nullopt, std::move(pins));
                });
            });
    });
}

ObjectDataHandler::write_ret ObjectDataHandler::write(context_t ctx, objaddr_t offset, const bufferlist& bl)
{
    return with_object_data(ctx, [this, ctx, offset, &bl](auto& object_data) {
        ...
        DEBUGT("writing to {}~{}, object_data: {}~{}, is_null {}",
               ...
               object_data.get_reserved_data_len(),
               object_data.is_null());
        return prepare_data_reservation(ctx, object_data, p2roundup(offset + bl.length(), ctx.tm.get_block_size()))
            .si_then([this, ctx, offset, &object_data, &bl] {
                auto logical_offset = object_data.get_reserved_data_base() + offset;
                return ctx.tm.get_pins(ctx.t, logical_offset, bl.length())
                    .si_then([this, ctx, logical_offset, &bl](auto pins) {
                        return overwrite(ctx, logical_offset, bl.length(), bufferlist(bl), std::move(pins));
                    });
            });
    });
}

ObjectDataHandler::read_ret ObjectDataHandler::read(context_t ctx, objaddr_t obj_offset, extent_len_t len)
{
    return seastar::do_with(bufferlist(), [ctx, obj_offset, len](auto& ret) {
        return with_object_data(
                   ...
                   [ctx, obj_offset, len, &ret](const auto& object_data) {
                       ...
                       return ctx.tm.get_pins(ctx.t, loffset, len).si_then([ctx, loffset, len, &ret](auto _pins) {
                           // offset~len falls within reserved region and len > 0
                           ceph_assert(_pins.size() >= 1);
                           ceph_assert((*_pins.begin())->get_key() <= loffset);
                           return seastar::do_with(
                               std::move(_pins), loffset, [ctx, loffset, len, &ret](auto& pins, auto& current) {
                                   return trans_intr::do_for_each(
                                       pins, [ctx, loffset, len, &current, &ret](auto& pin) -> read_iertr::future<> {
                                           ceph_assert(current <= (loffset + len));
                                           ceph_assert((loffset + len) > pin->get_key());
                                           laddr_t end = std::min(pin->get_key() + pin->get_length(), loffset + len);
                                           if (pin->get_val().is_zero()) {
                                               ...
                                               return ctx.tm.read_pin<ObjectDataBlock>(ctx.t, std::move(pin))
...
ObjectDataHandler::fiemap_ret ObjectDataHandler::fiemap(context_t ctx, objaddr_t obj_offset, extent_len_t len)
{
    return seastar::do_with(std::map<uint64_t, uint64_t>(), [ctx, obj_offset, len](auto& ret) {
        return with_object_data(
                   ctx,
                   [ctx, obj_offset, len, &ret](const auto& object_data) {
                       ...
                       ceph_assert(len > 0);
                       laddr_t loffset = object_data.get_reserved_data_base() + obj_offset;
                       return ctx.tm.get_pins(ctx.t, loffset, len)
                           .si_then([loffset, len, &object_data, &ret](auto&& pins) {
                               ...
                               for (auto&& i : pins) {
                                   if (!(i->get_val().is_zero())) {
                                       ...
                                       auto ret_right = std::min(i->get_key() + i->get_length(), loffset + len);
                                       assert(ret_right > ret_left);
                                       ret.emplace(std::make_pair(ret_left - object_data.get_reserved_data_base(),
                                                                  ret_right - ret_left));
                                   }
                               }
                           });
...
ObjectDataHandler::truncate_ret ObjectDataHandler::truncate(context_t ctx, objaddr_t offset)
{
    return with_object_data(ctx, [this, ctx, offset](auto& object_data) {
        ...
        if (offset < object_data.get_reserved_data_len()) {
            ...
            return prepare_data_reservation(ctx, object_data, p2roundup(offset, ctx.tm.get_block_size()));
        }
        else {
...
ObjectDataHandler::clear_ret ObjectDataHandler::clear(context_t ctx)
{
    return with_object_data(ctx, [this, ctx](auto& object_data) {
        LOG_PREFIX(ObjectDataHandler::clear);
        DEBUGT("clearing: {}~{}", ctx.t, object_data.get_reserved_data_base(), object_data.get_reserved_data_len());
        if (object_data.is_null()) {
            return clear_iertr::now();
        }
...
ObjectDataHandler::clone_ret ObjectDataHandler::clone_extents(context_t ctx, object_data_t& object_data,
                                                              lba_pin_list_t& pins, laddr_t data_base)
{
    ...
    TRACET(" object_data: {}~{}, data_base: {}",
           ...
           object_data.get_reserved_data_len(),
           data_base);
    return ctx.tm.dec_ref(ctx.t, object_data.get_reserved_data_base())
        .si_then(
            [&pins, &object_data, ctx, data_base](auto) mutable {
                return seastar::do_with((extent_len_t)0, [&object_data, ctx, data_base, &pins](auto& last_pos) {
                    return trans_intr::do_for_each(
                               pins,
                               [&last_pos, &object_data, ctx, data_base](auto& pin) {
                                   ...
                                   auto fut =
                                       TransactionManager::alloc_extent_iertr ::make_ready_future<LBAMappingRef>();
                                   ...
                                   return fut
                                       .si_then([&pin, &last_pos, offset](auto) {
            ...
            ObjectDataHandler::write_iertr::pass_further{},
            crimson::ct_error::assert_all{"object_data_handler::clone invalid error"});
}
...
ObjectDataHandler::clone_ret ObjectDataHandler::clone(context_t ctx)
{
    ...
    // 	   length.
    return with_objects_data(ctx, [ctx, this](auto& object_data, auto& d_object_data) {
        ceph_assert(d_object_data.is_null());
        if (object_data.is_null()) {
            return clone_iertr::now();
        }
        return prepare_data_reservation(ctx, d_object_data, object_data.get_reserved_data_len())
            .si_then([&object_data, &d_object_data, ctx, this] {
                ...
                return prepare_data_reservation(ctx, object_data, d_object_data.get_reserved_data_len())
                    .si_then([&d_object_data, ctx, &object_data, base, len, this] {
                        ...
                        return ctx.tm.get_pins(ctx.t, base, len)
                            .si_then([ctx, &object_data, &d_object_data, base, this](auto pins) {
                                return seastar::do_with(
                                    std::move(pins), [ctx, &object_data, &d_object_data, base, this](auto& pins) {
                                        return clone_extents(ctx, object_data, pins, base)
                                            ...
                                            .si_then([&pins, ctx] { return do_removals(ctx, pins); });
                                    });
                            });
                    });
            });
