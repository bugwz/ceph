...
public:
    ...
    virtual get_mappings_ret get_mappings(Transaction& t, laddr_t offset, extent_len_t length) = 0;
     ...
     * lba value.
     */
    virtual get_mappings_ret get_mappings(Transaction& t, laddr_list_t&& extent_lisk) = 0;
     ...
     * lba value.
     */
    using get_mapping_iertr = base_iertr::extend<crimson::ct_error::enoent>;
    using get_mapping_ret = get_mapping_iertr::future<LBAMappingRef>;
    virtual get_mapping_ret get_mapping(Transaction& t, laddr_t offset) = 0;
    ...
    virtual alloc_extent_ret alloc_extent(Transaction& t, laddr_t hint, extent_len_t len, paddr_t addr,
                                          LogicalCachedExtent& nextent) = 0;

    virtual alloc_extent_ret clone_extent(Transaction& t, laddr_t hint, extent_len_t len, laddr_t intermediate_key,
                                          paddr_t actual_addr, laddr_t intermediate_base) = 0;

    virtual alloc_extent_ret reserve_region(Transaction& t, laddr_t hint, extent_len_t len) = 0;
    ...
    using ref_iertr = base_iertr::extend<crimson::ct_error::enoent>;
     ...
     */
    virtual ref_ret decref_extent(Transaction& t, laddr_t addr, bool cascade_remove) = 0;
     ...
     * @return returns resulting refcount
     */
    virtual ref_ret incref_extent(Transaction& t, laddr_t addr) = 0;
     ...
     * @return returns resulting refcount
     */
    virtual ref_ret incref_extent(Transaction& t, laddr_t addr, int delta) = 0;
    ...
    virtual init_cached_extent_ret init_cached_extent(Transaction& t, CachedExtentRef e) = 0;
    ...
    using scan_mappings_func_t = std::function<void(laddr_t, paddr_t, extent_len_t)>;
    virtual scan_mappings_ret scan_mappings(Transaction& t, laddr_t begin, laddr_t end, scan_mappings_func_t&& f) = 0;
    ...
    virtual rewrite_extent_ret rewrite_extent(Transaction& t, CachedExtentRef extent) = 0;
    ...
    virtual update_mapping_ret update_mapping(Transaction& t, laddr_t laddr, paddr_t prev_addr, paddr_t paddr,
                                              LogicalCachedExtent* nextent) = 0;
    ...
    update_mappings_ret update_mappings(Transaction& t, const std::list<LogicalCachedExtentRef>& extents);
    ...
    using get_physical_extent_if_live_ret = get_physical_extent_if_live_iertr::future<CachedExtentRef>;
    virtual get_physical_extent_if_live_ret get_physical_extent_if_live(Transaction& t, extent_types_t type,
