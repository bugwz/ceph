...
RecordScanner::scan_valid_records_ret RecordScanner::scan_valid_records(scan_valid_records_cursor& cursor,
                                                                        segment_nonce_t nonce, size_t budget,
                                                                        found_record_handler_t& handler)
{
    ...
    return crimson::repeat([=, &cursor, &budget_used, &handler, this]() mutable
                       ...
                       return read_validate_record_metadata(cursor, nonce)
                           .safe_then([=, &cursor](auto md) {
                               if (!md) {
                                   ...
                                   if (cursor.is_complete()) {
                                       INFO("complete at {}, invalid record group metadata", cursor);
                                   }
                                   else {
                           ...
                           .safe_then([=, &cursor, &budget_used, &handler, this] {
                               DEBUG("processing committed record groups until {}, {} pending",
                                     ...
                                     cursor.pending_record_groups.size());
                               return crimson::repeat([=, &budget_used, &cursor, &handler, this] {
                                   if (cursor.pending_record_groups.empty()) {
                                       ...
                                       return scan_valid_records_ertr::make_ready_future<seastar::stop_iteration>(
                                           seastar::stop_iteration::yes);
                                   }
                                   auto& next = cursor.pending_record_groups.front();
                                   journal_seq_t next_seq = {cursor.seq.segment_seq, next.offset};
                                   if (cursor.last_committed == JOURNAL_SEQ_NULL || next_seq > cursor.last_committed) {
                                       return scan_valid_records_ertr::make_ready_future<seastar::stop_iteration>(
                                           seastar::stop_iteration::yes);
                                   }
                                   return consume_next_records(cursor, handler, budget_used).safe_then([] {
                                       return scan_valid_records_ertr::make_ready_future<seastar::stop_iteration>(
                       ...
                       return read_validate_data(next.offset, next.header)
                          ...
                          .safe_then([=, &budget_used, &cursor] {
                              if (cursor.is_complete() || budget_used >= budget) {
                                  DEBUG("finish at {}, budget_used={}, budget={}", cursor, budget_used, budget);
...
RecordScanner::read_validate_record_metadata_ret RecordScanner::read_validate_record_metadata(
    scan_valid_records_cursor& cursor, segment_nonce_t nonce)
{
    ...
    return read(start, block_size)
        .safe_then([=](bufferptr bptr) mutable -> read_validate_record_metadata_ret {
            ...
            if (header.mdlength < block_size || header.mdlength % block_size != 0 || header.dlength % block_size != 0 ||
                (header.committed_to != JOURNAL_SEQ_NULL &&
                 get_segment_off(header.committed_to.offset) % cursor.get_block_size() != 0) ||
...
RecordScanner::read_validate_data_ret RecordScanner::read_validate_data(paddr_t record_base,
                                                                        const record_group_header_t& header)
{
    ...
    return read(data_addr, header.dlength).safe_then([=, &header](auto bptr) {
        bufferlist bl;
        bl.append(bptr);
...
RecordScanner::consume_record_group_ertr::future<> RecordScanner::consume_next_records(
    scan_valid_records_cursor& cursor, found_record_handler_t& handler, std::size_t& budget_used)
{
    ...
    auto locator = record_locator_t{next.offset.add_offset(next.header.mdlength),
                                    write_result_t{journal_seq_t{cursor.seq.segment_seq, next.offset}, total_length}};
    DEBUG("processing {} at {}, budget_used={}", next.header, locator, budget_used);
    return handler(locator, next.header, next.mdbuffer).safe_then([FNAME, &cursor] {
