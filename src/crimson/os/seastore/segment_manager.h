...
struct block_sm_superblock_t
{
    ...
    void validate() const
    {
        ...
        for (unsigned int i = 0; i < seastar::smp::count; i++) {
            ceph_assert(shard_infos[i].size > segment_size && shard_infos[i].size % block_size == 0);
            ...
            ceph_assert(shard_infos[i].tracker_offset > 0 && shard_infos[i].tracker_offset % block_size == 0);
        ...
        ceph_assert(get_default_backend_of_device(config.spec.dtype) == backend_type_t::SEGMENTED);
...
class Segment : public boost::intrusive_ref_counter<Segment, boost::thread_unsafe_counter>
{
public:
     ...
     */
    using write_ertr = crimson::errorator<crimson::ct_error::input_output_error,   // media error or corruption
     ...
     */
    virtual write_ertr::future<> advance_wp(segment_off_t offset) = 0;
...
{
public:
    backend_type_t get_backend_type() const final { return backend_type_t::SEGMENTED; }
    ...
    static seastar::future<SegmentManagerRef> get_segment_manager(const std::string& device, device_type_t dtype);
};

}   // namespace crimson::os::seastore

WRITE_CLASS_DENC(crimson::os::seastore::block_shard_info_t)
WRITE_CLASS_DENC(crimson::os::seastore::block_sm_superblock_t)
