...
static void copy_from_foreign(iterator tgt, const_iterator from_src, const_iterator to_src)
{
    assert(tgt->node != from_src->node);
    assert(to_src->node == from_src->node);
    if (from_src == to_src) return;

    auto to_copy = from_src->get_right_ptr_end() - to_src->get_right_ptr_end();
    assert(to_copy > 0);
    memcpy(tgt->get_right_ptr_end() - to_copy, to_src->get_right_ptr_end(), to_copy);
    memcpy(tgt->get_node_key_ptr(),
 ...
 */
template<typename iterator> static void copy_from_local(unsigned len, iterator tgt, iterator from_src, iterator to_src)
{
    assert(tgt->node == from_src->node);
    assert(to_src->node == from_src->node);
    ...
    memmove(to_src->get_right_ptr_end() + adjust_offset, to_src->get_right_ptr_end(), to_copy);

    for (auto ite = from_src; ite < to_src; ite++) {
        ite->update_offset(-adjust_offset);
    }
    memmove(tgt->get_node_key_ptr(),
            from_src->get_node_key_ptr(),
            to_src->get_node_key_ptr() - from_src->get_node_key_ptr());
}
...
struct delta_inner_t
{
    ...
    bool operator==(const delta_inner_t& rhs) const { return op == rhs.op && key == rhs.key && addr == rhs.addr; }
};
}   // namespace crimson::os::seastore::omap_manager
...
struct delta_leaf_t
{
    ...
    bool operator==(const delta_leaf_t& rhs) const { return op == rhs.op && key == rhs.key && val == rhs.val; }
};
}   // namespace crimson::os::seastore::omap_manager
...
public:
    ...
    void insert(const std::string& key, laddr_t addr)
    {
        buffer.push_back(delta_inner_t{delta_inner_t::op_t::INSERT, key, addr});
    }
    void update(const std::string& key, laddr_t addr)
    {
        buffer.push_back(delta_inner_t{delta_inner_t::op_t::UPDATE, key, addr});
    }
    void remove(const std::string& key)
    {
        buffer.push_back(delta_inner_t{delta_inner_t::op_t::REMOVE, key, L_ADDR_NULL});
    }
    ...
    bool operator==(const delta_inner_buffer_t& rhs) const { return buffer == rhs.buffer; }
};
}   // namespace crimson::os::seastore::omap_manager
...
public:
    bool empty() const { return buffer.empty(); }
    void insert(const std::string& key, const ceph::bufferlist& val)
    {
        buffer.push_back(delta_leaf_t{delta_leaf_t::op_t::INSERT, key, val});
    }
    void update(const std::string& key, const ceph::bufferlist& val)
    {
        buffer.push_back(delta_leaf_t{delta_leaf_t::op_t::UPDATE, key, val});
    }
    void remove(const std::string& key)
    {
        buffer.push_back(delta_leaf_t{delta_leaf_t::op_t::REMOVE, key, bufferlist()});
    }
    ...
    bool operator==(const delta_leaf_buffer_t& rhs) const { return buffer == rhs.buffer; }
};
}   // namespace crimson::os::seastore::omap_manager
...
public:
    template<bool is_const> class iter_t
    {
        friend class StringKVInnerNodeLayout;

        template<typename iterator, typename const_iterator>
        friend void copy_from_foreign(iterator, const_iterator, const_iterator);
        template<typename iterator> friend void copy_from_local(unsigned, iterator, iterator, iterator);
        ...
        iter_t(parent_t parent, uint16_t index)
            : node(parent)
            , index(index)
        {}
    ...
    public:
        ...
        iter_t operator+(uint16_t off) const { return iter_t(node, index + off); }
        iter_t operator-(uint16_t off) const { return iter_t(node, index - off); }
    ...
    private:
        ...
        {
            return reinterpret_cast<typename crimson::common::maybe_const_t<char, is_const>::type>(
                node->get_node_key_ptr() + index);
        }

        uint32_t get_node_val_offset() const { return get_node_key().key_off; }
        ...
        auto get_right_ptr_end() const { return node->buf + OMAP_INNER_BLOCK_SIZE - get_right_offset_end(); }
    ...
    public:
        ...
        std::string get_key() const { return std::string(get_node_val_ptr(), get_node_key().key_len); }

        laddr_t get_val() const { return get_node_key().laddr; }
...
public:
    void journal_inner_insert(const_iterator _iter, const laddr_t laddr, const std::string& key,
                              delta_inner_buffer_t* recorder)
    {
        auto iter = iterator(this, _iter.index);
        if (recorder) {
            recorder->insert(key, laddr);
        }
        inner_insert(iter, key, laddr);
    }

    void journal_inner_update(const_iterator _iter, const laddr_t laddr, delta_inner_buffer_t* recorder)
    {
        auto iter = iterator(this, _iter.index);
    ...
    void journal_inner_remove(const_iterator _iter, delta_inner_buffer_t* recorder)
    {
        auto iter = iterator(this, _iter.index);
        if (recorder) {
    ...
    const_iterator iter_cbegin() const { return const_iterator(this, 0); }
    const_iterator iter_begin() const { return iter_cbegin(); }

    const_iterator iter_cend() const { return const_iterator(this, get_size()); }
    const_iterator iter_end() const { return iter_cend(); }

    iterator iter_begin() { return iterator(this, 0); }

    iterator iter_end() { return iterator(this, get_size()); }

    const_iterator iter_idx(uint16_t off) const { return const_iterator(this, off); }
    ...
    void set_meta(const omap_node_meta_t& meta) { *layout.template Pointer<1>(buf) = omap_node_meta_le_t(meta); }
    ...
    uint32_t free_space() const { return capacity() - used_space(); }

    uint16_t capacity() const
    {
        return OMAP_INNER_BLOCK_SIZE - (reinterpret_cast<char*>(layout.template Pointer<2>(buf)) -
                                        reinterpret_cast<char*>(layout.template Pointer<0>(buf)));
    }

    bool is_overflow(size_t ksize) const { return free_space() < (sizeof(omap_inner_key_le_t) + ksize); }

    bool is_overflow(const StringKVInnerNodeLayout& rhs) const { return free_space() < rhs.used_space(); }

    bool below_min() const { return free_space() > (capacity() / 2); }
    ...
    bool operator==(const StringKVInnerNodeLayout& rhs) const
    {
        ...
        while (iter != iter_end()) {
            if (iter->get_key() != iter2->get_key() || iter->get_val() != iter2->get_val()) {
                return false;
            }
            iter++;
            iter2++;
     ...
     */
    std::string split_into(StringKVInnerNodeLayout& left, StringKVInnerNodeLayout& right) const
    {
     ...
     */
    void merge_from(const StringKVInnerNodeLayout& left, const StringKVInnerNodeLayout& right)
    {
        copy_from_foreign(iter_end(), left.iter_begin(), left.iter_end());
        set_size(left.get_size());

        copy_from_foreign(iter_end(), right.iter_begin(), right.iter_end());
     ...
     */
    static std::string balance_into_new_nodes(const StringKVInnerNodeLayout& left, const StringKVInnerNodeLayout& right,
                                              ...
                                              StringKVInnerNodeLayout& replacement_right)
    {
        ...
        auto replacement_pivot = pivot_idx >= left.get_size() ? right.iter_idx(pivot_idx - left.get_size())->get_key()
                                                              : left.iter_idx(pivot_idx)->get_key();

        if (pivot_size < left_size) {
            copy_from_foreign(replacement_left.iter_end(), left.iter_begin(), left.iter_idx(pivot_idx));
            replacement_left.set_size(pivot_idx);

            copy_from_foreign(replacement_right.iter_end(), left.iter_idx(pivot_idx), left.iter_end());
            replacement_right.set_size(left.get_size() - pivot_idx);

            copy_from_foreign(replacement_right.iter_end(), right.iter_begin(), right.iter_end());
            replacement_right.set_size(right.get_size() + left.get_size() - pivot_idx);
        }
        else {
            copy_from_foreign(replacement_left.iter_end(), left.iter_begin(), left.iter_end());
            replacement_left.set_size(left.get_size());

            copy_from_foreign(
                replacement_left.iter_end(), right.iter_begin(), right.iter_idx(pivot_idx - left.get_size()));
            replacement_left.set_size(pivot_idx);

            copy_from_foreign(
                replacement_right.iter_end(), right.iter_idx(pivot_idx - left.get_size()), right.iter_end());
            replacement_right.set_size(right.get_size() + left.get_size() - pivot_idx);
        }

        auto [lmeta, rmeta] = omap_node_meta_t::rebalance(left.get_meta(), right.get_meta());
...
private:
    void inner_insert(iterator iter, const std::string& key, laddr_t val)
    {
        if (iter != iter_begin()) {
    ...
    void inner_update(iterator iter, laddr_t addr)
    {
    ...
    {
        assert(iter != iter_end());
        if ((iter + 1) != iter_end()) copy_from_local(iter->get_node_key().key_len, iter, iter + 1, iter_end());
        set_size(get_size() - 1);
    }
     ...
     */
    omap_inner_key_le_t* get_node_key_ptr() { return L::Partial(1, 1, get_size()).template Pointer<2>(buf); }
...
public:
    ...
    private:
        ...
        auto get_right_ptr_end() const { return node->buf + OMAP_LEAF_BLOCK_SIZE - get_right_offset_end(); }
    ...
    public:
        ...
        {
            auto node_key = get_node_key();
            return std::string(get_node_val_ptr() + node_key.key_len, get_node_key().val_len);
        }
        ...
        ceph::bufferlist get_val() const
        {
            auto node_key = get_node_key();
            ceph::bufferlist bl;
            ceph::bufferptr bptr(get_node_val_ptr() + node_key.key_len, get_node_key().val_len);
            bl.append(bptr);
            return bl;
        }
    };
...
public:
    void journal_leaf_insert(const_iterator _iter, const std::string& key, const ceph::bufferlist& val,
    ...
    void journal_leaf_update(const_iterator _iter, const std::string& key, const ceph::bufferlist& val,
                             delta_leaf_buffer_t* recorder)
    {
        auto iter = iterator(this, _iter.index);
        if (recorder) {
    ...
    void journal_leaf_remove(const_iterator _iter, delta_leaf_buffer_t* recorder)
    {
        auto iter = iterator(this, _iter.index);
        if (recorder) {
            recorder->remove(iter->get_key());
    ...
    const_iterator iter_begin() const { return const_iterator(this, 0); }

    const_iterator iter_end() const { return const_iterator(this, get_size()); }
    ...
    {
        return OMAP_LEAF_BLOCK_SIZE - (reinterpret_cast<char*>(layout.template Pointer<2>(buf)) -
                                       reinterpret_cast<char*>(layout.template Pointer<0>(buf)));
    }
    ...
    bool is_overflow(const StringKVLeafNodeLayout& rhs) const { return free_space() < rhs.used_space(); }
     ...
     * Takes *this and splits its contents into left and right.
     */
    std::string split_into(StringKVLeafNodeLayout& left, StringKVLeafNodeLayout& right) const
    {
        auto piviter = get_split_pivot();
        assert(piviter != iter_end());
     ...
     * precondition: left.size() + right.size() < CAPACITY
     */
    void merge_from(const StringKVLeafNodeLayout& left, const StringKVLeafNodeLayout& right)
    {
        copy_from_foreign(iter_end(), left.iter_begin(), left.iter_end());
        set_size(left.get_size());
        copy_from_foreign(iter_end(), right.iter_begin(), right.iter_end());
     ...
     */
    static std::string balance_into_new_nodes(const StringKVLeafNodeLayout& left, const StringKVLeafNodeLayout& right,
...
private:
    void leaf_insert(iterator iter, const std::string& key, const bufferlist& val)
    {
        if (iter != iter_begin()) {
            assert((iter - 1)->get_key() < key);
        }
        ...
        if (iter == iter_begin()) {
            ...
            node_key.key_off = (iter - 1)->get_node_key().key_off + (key.size() + val.length());
            node_key.key_len = key.size();
            node_key.val_len = val.length();
        }
    ...
    void leaf_update(iterator iter, const std::string& key, const ceph::bufferlist& val)
    {
        assert(iter != iter_end());
        leaf_remove(iter);
        assert(!is_overflow(key.size(), val.length()));
     ...
     * Get pointer to start of key array
     */
    omap_leaf_key_le_t* get_node_key_ptr() { return L::Partial(1, 1, get_size()).template Pointer<2>(buf); }
    const omap_leaf_key_le_t* get_node_key_ptr() const { return L::Partial(1, 1, get_size()).template Pointer<2>(buf); }
};
