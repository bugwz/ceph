...
struct omap_node_meta_t
{
    ...
    {
        return std::make_pair(omap_node_meta_t{depth}, omap_node_meta_t{depth});
    }

    static omap_node_meta_t merge_from(const omap_node_meta_t& lhs, const omap_node_meta_t& rhs)
    {
        assert(lhs.depth == rhs.depth);
        return omap_node_meta_t{lhs.depth};
    }

    static std::pair<omap_node_meta_t, omap_node_meta_t> rebalance(const omap_node_meta_t& lhs,
                                                                   const omap_node_meta_t& rhs)
    {
        assert(lhs.depth == rhs.depth);
        return std::make_pair(omap_node_meta_t{lhs.depth}, omap_node_meta_t{lhs.depth});
    }
};
...
struct omap_node_meta_le_t
{
    ...
    operator omap_node_meta_t() const { return omap_node_meta_t{depth}; }
};
...
struct omap_inner_key_le_t
{
    ...
    operator omap_inner_key_t() const { return omap_inner_key_t{uint16_t(key_off), uint16_t(key_len), laddr_t(laddr)}; }
...
struct omap_leaf_key_t
{
    ...
    {
        return key_off == b.key_off && key_len == b.key_len && val_len == b.val_len;
    }
    inline bool operator!=(const omap_leaf_key_t b) const
    {
        return key_off != b.key_off || key_len != b.key_len || val_len != b.val_len;
...
struct omap_leaf_key_le_t
{
    ...
    {
        return omap_leaf_key_t{uint16_t(key_off), uint16_t(key_len), uint16_t(val_len)};
    }
