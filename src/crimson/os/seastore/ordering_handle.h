...
struct WritePipeline
{
    ...
    using BlockingEvents = std::tuple<ReserveProjectedUsage::BlockingEvent, OolWrites::BlockingEvent,
                                      Prepare::BlockingEvent, DeviceSubmission::BlockingEvent, Finalize::BlockingEvent>;
};
...
public:
    ...
    constexpr static auto type_name = "crimson::os::seastore::PlaceholderOperation";

    static PlaceholderOperation::IRef create() { return IRef{new PlaceholderOperation()}; }
...
{
    OperationRef op;
    OperationProxy(OperationRef op)
        : op(std::move(op))
    {}
...
{
    OperationProxyT(typename OpT::IRef op)
        : OperationProxy(op)
    {}

    OpT* that() { return static_cast<OpT*>(op.get()); }
    const OpT* that() const { return static_cast<const OpT*>(op.get()); }

    seastar::future<> enter(WritePipeline::ReserveProjectedUsage& s) final { return that()->enter_stage(s); }
    seastar::future<> enter(WritePipeline::OolWrites& s) final { return that()->enter_stage(s); }
    seastar::future<> enter(WritePipeline::Prepare& s) final { return that()->enter_stage(s); }
    seastar::future<> enter(WritePipeline::DeviceSubmission& s) final { return that()->enter_stage(s); }
    seastar::future<> enter(WritePipeline::Finalize& s) final { return that()->enter_stage(s); }

    void exit() final { return that()->handle.exit(); }
    seastar::future<> complete() final { return that()->handle.complete(); }
};
...
struct OrderingHandle
{
    ...
    OrderingHandle(std::unique_ptr<OperationProxy> op)
        : op(std::move(op))
    {}
    ...
    template<typename T> seastar::future<> enter(T& t) { return op->enter(t); }

    void exit() { op->exit(); }

    seastar::future<> complete() { return op->complete(); }

    ~OrderingHandle() { maybe_release_collection_lock(); }
};
...
namespace crimson {
template<> struct EventBackendRegistry<os::seastore::PlaceholderOperation>
{
