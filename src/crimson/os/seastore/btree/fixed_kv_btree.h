...
template<typename T> phy_tree_root_t& get_phy_tree_root(root_t& r);

using get_child_iertr =
    ::crimson::interruptible::interruptible_errorator<typename trans_intr::condition, get_child_ertr>;
using get_phy_tree_root_node_ret = std::pair<bool, get_child_iertr::future<CachedExtentRef>>;

template<typename T, typename key_t>
const get_phy_tree_root_node_ret get_phy_tree_root_node(const RootBlockRef& root_block, op_context_t<key_t> c);

template<typename ROOT_T> void link_phy_tree_root_node(RootBlockRef& root_block, ROOT_T* root_node);

template<typename T> void unlink_phy_tree_root_node(RootBlockRef& root_block);

template<typename T> Transaction::tree_stats_t& get_tree_stats(Transaction& t);

template<typename node_key_t, typename node_val_t, typename internal_node_t, typename leaf_node_t, typename pin_t,
         size_t node_size, bool leaf_has_children>
class FixedKVBtree
{
    static constexpr size_t MAX_DEPTH = 16;
    using self_type =
        FixedKVBtree<node_key_t, node_val_t, internal_node_t, leaf_node_t, pin_t, node_size, leaf_has_children>;

public:
    using InternalNodeRef = TCachedExtentRef<internal_node_t>;
    using LeafNodeRef = TCachedExtentRef<leaf_node_t>;

    using base_ertr = crimson::errorator<crimson::ct_error::input_output_error>;
    using base_iertr = trans_iertr<base_ertr>;

    class iterator;
    using iterator_fut = base_iertr::future<iterator>;

    using mapped_space_visitor_t =
        std::function<void(paddr_t, node_key_t, extent_len_t, depth_t, extent_types_t, iterator&)>;
    ...
    public:
        ...
        iterator_fut next(op_context_t<node_key_t> c, mapped_space_visitor_t* visitor = nullptr) const
        {
            assert_valid();
            assert(!is_end());
            ...
            ret.leaf.pos++;
            if (ret.at_boundary()) {
                return seastar::do_with(ret, [c, visitor](auto& ret) mutable {
                    return ret.handle_boundary(c, visitor).si_then([&ret] { return std::move(ret); });
                });
            }
            else {
                return iterator_fut(interruptible::ready_future_marker{}, ret);
            }
        }
        ...
        iterator_fut prev(op_context_t<node_key_t> c) const
        {
            ...
            return seastar::do_with(
                ...
                [c, depth_with_space](auto& ret, auto& li, auto& ll) {
                    ...
                    return lookup_depth_range(c, ret, depth_with_space - 1, 0, li, ll, nullptr).si_then([&ret] {
        ...
        depth_t get_depth() const { return internal.size() + 1; }
        ...
        node_val_t get_val() const
        {
            ...
            if constexpr (std::is_same_v<crimson::os::seastore::lba_manager::btree::lba_map_val_t, node_val_t>) {
                if (ret.pladdr.is_paddr()) {
                    ret.pladdr = ret.pladdr.get_paddr().maybe_relative_to(leaf.node->get_paddr());
                }
            }
            return ret;
        }
        ...
        std::unique_ptr<pin_t> get_pin(op_context_t<node_key_t> ctx) const
        {
            ...
            return std::make_unique<pin_t>(
                ctx, leaf.node, leaf.pos, val, fixed_kv_node_meta_t<node_key_t>{key, key + val.len, 0});
        }

        typename leaf_node_t::Ref get_leaf_node() { return leaf.node; }

        uint16_t get_leaf_pos() { return leaf.pos; }
    ...
    private:
        ...
        template<typename NodeType> struct node_position_t
        {
            typename NodeType::Ref node;
            uint16_t pos = INVALID;

            node_position_t() = default;
            node_position_t(typename NodeType::Ref node, uint16_t pos)
                : node(node)
                , pos(pos)
            {}

            void reset() { *this = node_position_t{}; }
        ...
        boost::container::static_vector<node_position_t<internal_node_t>, MAX_DEPTH> internal;
        ...
        handle_boundary_ret handle_boundary(op_context_t<node_key_t> c, mapped_space_visitor_t* visitor)
        {
            assert(at_boundary());
            depth_t depth_with_space = 2;
            for (; depth_with_space <= get_depth(); ++depth_with_space) {
                if ((get_internal(depth_with_space).pos + 1) < get_internal(depth_with_space).node->get_size()) {
                    break;
                }
            }

            if (depth_with_space <= get_depth()) {
                return seastar::do_with([](const internal_node_t& internal) { return internal.begin(); },
        ...
        depth_t check_split() const
        {
            ...
            for (depth_t split_from = 1; split_from < get_depth(); ++split_from) {
                if (!get_internal(split_from + 1).node->at_max_capacity()) return split_from;
            }
            return get_depth();
        }
        ...
        depth_t check_merge() const
        {
            ...
            for (depth_t merge_from = 1; merge_from < get_depth(); ++merge_from) {
                if (!get_internal(merge_from + 1).node->below_min_capacity()) return merge_from;
            }
            return get_depth();
        }
    };

    FixedKVBtree(RootBlockRef& root_block)
        : root_block(root_block)
    {}

    auto& get_root() { return get_phy_tree_root<self_type>(root_block->get_root()); }

    auto& get_root() const { return get_phy_tree_root<self_type>(root_block->get_root()); }

    template<typename T> void set_root_node(const TCachedExtentRef<T>& root_node)
    {
        static_assert(std::is_base_of_v<typename internal_node_t::base_t, T>);
        link_phy_tree_root_node(root_block, root_node.get());
    }

    auto get_root_node(op_context_t<node_key_t> c) const { return get_phy_tree_root_node<self_type>(root_block, c); }
    ...
    static mkfs_ret mkfs(RootBlockRef& root_block, op_context_t<node_key_t> c)
    {
        ...
        auto root_leaf =
            c.cache.template alloc_new_extent<leaf_node_t>(c.trans, node_size, placement_hint_t::HOT, INIT_GENERATION);
     ...
     */
    iterator_fut lower_bound(op_context_t<node_key_t> c, node_key_t addr, mapped_space_visitor_t* visitor = nullptr,
                             depth_t min_depth = 1) const
    {
        ...
        return lookup(
            ...
            .si_then([FNAME, c, min_depth](auto&& ret) {
                SUBTRACET(seastore_fixedkv_tree, "ret.leaf.pos {}", c.trans, ret.leaf.pos);
                if (min_depth == 1) {
                    ret.assert_valid();
                }
                return std::move(ret);
     ...
     * @return least iterator > key
     */
    iterator_fut upper_bound(op_context_t<node_key_t> c, node_key_t addr) const
    {
        return lower_bound(c, addr).si_then([c, addr](auto iter) {
            if (!iter.is_end() && iter.get_key() == addr) {
                return iter.next(c);
            }
            else {
                return iterator_fut(interruptible::ready_future_marker{}, iter);
            }
        });
    }
     ...
     */
    iterator_fut upper_bound_right(op_context_t<node_key_t> c, node_key_t addr) const
    {
        return lower_bound(c, addr).si_then([c, addr](auto iter) {
            if (iter.is_begin()) {
                return iterator_fut(interruptible::ready_future_marker{}, iter);
                ...
                return iter.prev(c).si_then([iter, addr](auto prev) {
                    if ((prev.get_key() + prev.get_val().len) > addr) {
                        return iterator_fut(interruptible::ready_future_marker{}, prev);
                    }
                    else {
                        return iterator_fut(interruptible::ready_future_marker{}, iter);
                    }
    ...
    iterator_fut begin(op_context_t<node_key_t> c) const { return lower_bound(c, 0); }
    iterator_fut end(op_context_t<node_key_t> c) const { return upper_bound(c, min_max_t<node_key_t>::max); }

    template<typename child_node_t, typename node_t, bool lhc = leaf_has_children,
             typename std::enable_if<lhc, int>::type = 0>
    void check_node(op_context_t<node_key_t> c, TCachedExtentRef<node_t> node)
    {
        ...
        for (auto i : *node) {
            ...
            if constexpr (std::is_base_of_v<typename internal_node_t::base_t, child_node_t>) {
                ret = c.trans.get_extent(i->get_val().maybe_relative_to(node->get_paddr()), &child_node);
            }
            else {
                assert(i->get_val().pladdr.is_paddr());
                ret = c.trans.get_extent(i->get_val().pladdr.get_paddr().maybe_relative_to(node->get_paddr()),
                                         &child_node);
            }
                ...
                if (!is_valid_child_ptr(child)) {
                    if constexpr (std::is_base_of_v<typename internal_node_t::base_t, child_node_t>) {
                        ...
                        if constexpr (leaf_has_children) {
                            assert(i->get_val().pladdr.is_paddr()
                                       ? (bool)!c.cache.query_cache(i->get_val().pladdr.get_paddr(), nullptr)
                                       : true);
                        }
                    }
                }
    ...
    template<bool lhc = leaf_has_children, typename std::enable_if<lhc, int>::type = 0>
    check_child_trackers_ret check_child_trackers(op_context_t<node_key_t> c)
    {
        mapped_space_visitor_t checker =
            [c, this](paddr_t, node_key_t, extent_len_t, depth_t depth, extent_types_t, iterator& iter) {
                if constexpr (!leaf_has_children) {
                    if (depth == 1) {
                        return seastar::now();
                    }
        ...
        return seastar::do_with(std::move(checker), [this, c](auto& checker) {
            return iterate_repeat(
                c,
                lower_bound(c, min_max_t<node_key_t>::min, &checker),
                [](auto& pos) {
                    if (pos.is_end()) {
                        return base_iertr::make_ready_future<seastar::stop_iteration>(seastar::stop_iteration::yes);
                    }
                    return base_iertr::make_ready_future<seastar::stop_iteration>(seastar::stop_iteration::no);
                },
                &checker);
        });
    }

    using iterate_repeat_ret_inner = base_iertr::future<seastar::stop_iteration>;
    template<typename F>
    static base_iertr::future<> iterate_repeat(op_context_t<node_key_t> c, iterator_fut&& iter_fut, F&& f,
                                               mapped_space_visitor_t* visitor = nullptr)
    {
        return std::move(iter_fut).si_then([c, visitor, f = std::forward<F>(f)](auto iter) {
            return seastar::do_with(iter, std::move(f), [c, visitor](auto& pos, auto& f) {
                return trans_intr::repeat([c, visitor, &f, &pos] {
                    return f(pos).si_then([c, visitor, &pos](auto done) {
                        if (done == seastar::stop_iteration::yes) {
                            return iterate_repeat_ret_inner(interruptible::ready_future_marker{},
                                                            seastar::stop_iteration::yes);
                        }
                        else {
                            ceph_assert(!pos.is_end());
                            return pos.next(c, visitor).si_then([&pos](auto next) {
                                pos = next;
                                return iterate_repeat_ret_inner(interruptible::ready_future_marker{},
    ...
    insert_ret insert(op_context_t<node_key_t> c, iterator iter, node_key_t laddr, node_val_t val,
                      LogicalCachedExtent* nextent)
    {
        ...
        return seastar::do_with(iter, [this, c, laddr, val, nextent](auto& ret) {
            return find_insertion(c, laddr, ret).si_then([this, c, laddr, val, &ret, nextent] {
                if (!ret.at_boundary() && ret.get_key() == laddr) {
                    return insert_ret(interruptible::ready_future_marker{}, std::make_pair(ret, false));
                }
                else {
                    ++(get_tree_stats<self_type>(c.trans).num_inserts);
                    return handle_split(c, ret).si_then([c, laddr, val, &ret, nextent] {
                        if (!ret.leaf.node->is_mutable()) {
                            CachedExtentRef mut = c.cache.duplicate_for_write(c.trans, ret.leaf.node);
                            ret.leaf.node = mut->cast<leaf_node_t>();
                        }
                        auto iter = typename leaf_node_t::const_iterator(ret.leaf.node.get(), ret.leaf.pos);
                        assert(iter == ret.leaf.node->lower_bound(laddr));
                        assert(iter == ret.leaf.node->end() || iter->get_key() > laddr);
                        assert(laddr >= ret.leaf.node->get_meta().begin && laddr < ret.leaf.node->get_meta().end);
                        ret.leaf.node->insert(iter, laddr, val, nextent);
                        return insert_ret(interruptible::ready_future_marker{}, std::make_pair(ret, true));
                    });
                }
            });
        });
    ...
    insert_ret insert(op_context_t<node_key_t> c, node_key_t laddr, node_val_t val, LogicalCachedExtent* nextent)
    {
        return lower_bound(c, laddr).si_then(
            [this, c, laddr, val, nextent](auto iter) { return this->insert(c, iter, laddr, val, nextent); });
    }
    ...
    update_ret update(op_context_t<node_key_t> c, iterator iter, node_val_t val, LogicalCachedExtent* nextent)
    {
        ...
        if (!iter.leaf.node->is_mutable()) {
            CachedExtentRef mut = c.cache.duplicate_for_write(c.trans, iter.leaf.node);
            iter.leaf.node = mut->cast<leaf_node_t>();
        }
        ++(get_tree_stats<self_type>(c.trans).num_updates);
        iter.leaf.node->update(iter.leaf.node->iter_idx(iter.leaf.pos), val, nextent);
        return update_ret(interruptible::ready_future_marker{}, iter);
    }
    ...
    remove_ret remove(op_context_t<node_key_t> c, iterator iter)
    {
        ...
        return seastar::do_with(iter, [this, c](auto& ret) {
            if (!ret.leaf.node->is_mutable()) {
                CachedExtentRef mut = c.cache.duplicate_for_write(c.trans, ret.leaf.node);
                ret.leaf.node = mut->cast<leaf_node_t>();
            }
            ret.leaf.node->remove(ret.leaf.node->iter_idx(ret.leaf.pos));

            return handle_merge(c, ret);
        });
    }
    ...
    init_cached_extent_ret init_cached_extent(op_context_t<node_key_t> c, CachedExtentRef e)
    {
        assert(!e->is_logical());
        ...
        if (e->get_type() == internal_node_t::TYPE) {
            ...
            return lower_bound(c, eint->get_node_meta().begin).si_then([e, c, eint](auto iter) {
                // Note, this check is valid even if iter.is_end()
                LOG_PREFIX(FixedKVTree::init_cached_extent);
                depth_t cand_depth = eint->get_node_meta().depth;
                if (cand_depth <= iter.get_depth() && &*iter.get_internal(cand_depth).node == &*eint) {
                    SUBTRACET(seastore_fixedkv_tree, "extent {} is live", c.trans, *eint);
                    return true;
                }
                else {
                    SUBTRACET(seastore_fixedkv_tree, "extent {} is not live", c.trans, *eint);
                    return false;
                }
            });
        }
            ...
            return lower_bound(c, eleaf->get_node_meta().begin).si_then([c, e, eleaf](auto iter) {
                // Note, this check is valid even if iter.is_end()
                LOG_PREFIX(FixedKVTree::init_cached_extent);
                if (iter.leaf.node == &*eleaf) {
                    SUBTRACET(seastore_fixedkv_tree, "extent {} is live", c.trans, *eleaf);
                    return true;
                }
                else {
                    SUBTRACET(seastore_fixedkv_tree, "extent {} is not live", c.trans, *eleaf);
                    return false;
                }
            });
        }
        else {
            SUBTRACET(seastore_fixedkv_tree, "found other extent {} type {}", c.trans, *e, e->get_type());
            return init_cached_extent_ret(interruptible::ready_future_marker{}, true);
        }
    }
    ...
    get_leaf_if_live_ret get_leaf_if_live(op_context_t<node_key_t> c, paddr_t addr, node_key_t laddr, extent_len_t len)
    {
        LOG_PREFIX(FixedKVBtree::get_leaf_if_live);
        return lower_bound(c, laddr).si_then([FNAME, c, addr, laddr, len](auto iter) {
    ...
    get_internal_if_live_ret get_internal_if_live(op_context_t<node_key_t> c, paddr_t addr, node_key_t laddr,
                                                  extent_len_t len)
    {
    ...
    rewrite_extent_ret rewrite_extent(op_context_t<node_key_t> c, CachedExtentRef e)
    {
        LOG_PREFIX(FixedKVBtree::rewrite_extent);
        assert(is_lba_backref_node(e->get_type()));

        auto do_rewrite = [&](auto& fixed_kv_extent) {
            auto n_fixed_kv_extent =
                c.cache.template alloc_new_extent<std::remove_reference_t<decltype(fixed_kv_extent)>>(
                    c.trans,
                    fixed_kv_extent.get_length(),
                    fixed_kv_extent.get_user_hint(),
                    // get target rewrite generation
                    fixed_kv_extent.get_rewrite_generation());
            fixed_kv_extent.get_bptr().copy_out(0, fixed_kv_extent.get_length(), n_fixed_kv_extent->get_bptr().c_str());
            n_fixed_kv_extent->set_modify_time(fixed_kv_extent.get_modify_time());
            n_fixed_kv_extent->range = n_fixed_kv_extent->get_node_meta();

            if (fixed_kv_extent.get_type() == internal_node_t::TYPE || leaf_node_t::do_has_children) {
                if (!fixed_kv_extent.is_pending()) {
                    ...
                    n_fixed_kv_extent->copy_sources.emplace(
                        (typename internal_node_t::base_t*)fixed_kv_extent.get_prior_instance().get());
            ...
            if (!n_fixed_kv_extent->get_paddr().is_absolute()) {
                ...
                n_fixed_kv_extent->resolve_relative_addrs(
                    make_record_relative_paddr(0).block_relative_to(n_fixed_kv_extent->get_paddr()));
            }   // else: backend_type_t::RANDOM_BLOCK

            SUBTRACET(seastore_fixedkv_tree, "rewriting {} into {}", c.trans, fixed_kv_extent, *n_fixed_kv_extent);

            return update_internal_mapping(c,
                                           n_fixed_kv_extent->get_node_meta().depth,
                                           n_fixed_kv_extent->get_node_meta().begin,
                                           e->get_paddr(),
                                           n_fixed_kv_extent->get_paddr(),
                                           n_fixed_kv_extent)
                .si_then([c, e] { c.cache.retire_extent(c.trans, e); });
        };
    ...
    update_internal_mapping_ret update_internal_mapping(op_context_t<node_key_t> c, depth_t depth, node_key_t laddr,
                                                        paddr_t old_addr, paddr_t new_addr,
                                                        typename internal_node_t::base_ref nextent)
    {
        ...
        return lower_bound(c, laddr, nullptr, depth + 1).si_then([=, this](auto iter) {
            ...
            if (depth == iter.get_depth()) {
                ...
                if (laddr != min_max_t<node_key_t>::min) {
                    SUBERRORT(seastore_fixedkv_tree,
                ...
                root_block = c.cache.duplicate_for_write(c.trans, root_block)->template cast<RootBlock>();
                ...
                CachedExtentRef mut = c.cache.duplicate_for_write(c.trans, parent.node);
...
private:
    ...
    template<typename T> using node_position_t = typename iterator::template node_position_t<T>;

    using get_internal_node_iertr = base_iertr;
    using get_internal_node_ret = get_internal_node_iertr::future<InternalNodeRef>;
    static get_internal_node_ret get_internal_node(op_context_t<node_key_t> c, depth_t depth, paddr_t offset,
                                                   node_key_t begin, node_key_t end,
                                                   typename std::optional<node_position_t<internal_node_t>> parent_pos)
    {
        ...
        assert(depth > 1);
        auto init_internal = [c, depth, begin, end, parent_pos = std::move(parent_pos)](internal_node_t& node) {
        ...
        return c.cache.template get_absent_extent<internal_node_t>(c.trans, offset, node_size, init_internal)
            .si_then([FNAME, c, offset, init_internal, depth, begin, end](typename internal_node_t::Ref ret) {
                SUBTRACET(seastore_fixedkv_tree, "read internal at offset {} {}", c.trans, offset, *ret);
                ...
                if (!ret->is_pending() && !ret->is_linked()) {
                    assert(ret->is_dirty() || (is_backref_node(ret->get_type()) && ret->is_clean()));
                    init_internal(*ret);
                }
                ...
                return get_internal_node_ret(interruptible::ready_future_marker{}, ret);
            });
    }
    ...
    static get_leaf_node_ret get_leaf_node(op_context_t<node_key_t> c, paddr_t offset, node_key_t begin, node_key_t end,
                                           typename std::optional<node_position_t<leaf_node_t>> parent_pos)
    {
        LOG_PREFIX(FixedKVBtree::get_leaf_node);
        SUBTRACET(seastore_fixedkv_tree, "reading leaf at offset {}, begin {}, end {}", c.trans, offset, begin, end);
        auto init_leaf = [c, begin, end, parent_pos = std::move(parent_pos)](leaf_node_t& node) {
            assert(!node.is_pending());
            assert(!node.is_linked());
        ...
        return c.cache.template get_absent_extent<leaf_node_t>(c.trans, offset, node_size, init_leaf)
            .si_then([FNAME, c, offset, init_leaf, begin, end](typename leaf_node_t::Ref ret) {
                SUBTRACET(seastore_fixedkv_tree, "read leaf at offset {} {}", c.trans, offset, *ret);
                // This can only happen during init_cached_extent
                // or when backref extent being rewritten by gc space reclaiming
                if (!ret->is_pending() && !ret->is_linked()) {
                    assert(ret->is_dirty() || (is_backref_node(ret->get_type()) && ret->is_clean()));
                ...
                ceph_assert(begin == meta.begin);
                ceph_assert(end == meta.end);
                return get_leaf_node_ret(interruptible::ready_future_marker{}, ret);
            });
    }

    using lookup_root_iertr = base_iertr;
    using lookup_root_ret = lookup_root_iertr::future<>;
    lookup_root_ret lookup_root(op_context_t<node_key_t> c, iterator& iter, mapped_space_visitor_t* visitor) const
    {
        LOG_PREFIX(FixedKVBtree::lookup_root);
        SUBTRACET(seastore_fixedkv_tree, "looking up root on {}", c.trans, *root_block);
        auto [found, fut] = get_root_node(c);

        auto on_found_internal = [this, visitor, &iter](InternalNodeRef& root_node) {
            iter.get_internal(get_root().get_depth()).node = root_node;
            if (visitor)
                (*visitor)(root_node->get_paddr(),
        ...
        auto on_found_leaf = [visitor, &iter, this](LeafNodeRef root_node) {
        ...
        if (found) {
            return fut.si_then(
                [this, c, on_found_internal = std::move(on_found_internal), on_found_leaf = std::move(on_found_leaf)](
                    auto root) {
                    LOG_PREFIX(FixedKVBtree::lookup_root);
                    ceph_assert(root);
                    SUBTRACET(seastore_fixedkv_tree, "got root node on {}, res: {}", c.trans, *root_block, *root);
            ...
            if (get_root().get_depth() > 1) {
                ...
                return get_leaf_node(c,
                                     get_root().get_location(),
                                     min_max_t<node_key_t>::min,
                                     min_max_t<node_key_t>::max,
                                     std::nullopt)
                    .si_then(
                        [on_found = std::move(on_found_leaf)](LeafNodeRef root_node) { return on_found(root_node); });
            }
        }
    }
    ...
    template<typename F>
    static lookup_internal_level_ret lookup_internal_level(op_context_t<node_key_t> c, depth_t depth, iterator& iter,
                                                           F& f, mapped_space_visitor_t* visitor)
    {
        assert(depth > 1);
        ...
        auto on_found = [depth, visitor, &iter, &f](InternalNodeRef node) {
            ...
            if (visitor)
                (*visitor)(
                    node->get_paddr(), node->get_node_meta().begin, node->get_length(), depth, node->get_type(), iter);
            return seastar::now();
        };
        ...
        if (v.has_child()) {
            return v.get_child_fut().safe_then(
                [on_found = std::move(on_found), node_iter, c, parent_entry](auto child) mutable {
        ...
        auto end = next_iter == parent->end() ? parent->get_node_meta().end : next_iter->get_key();
        return get_internal_node(c,
                                 depth,
                                 ...
                                 std::make_optional<node_position_t<internal_node_t>>(
                                     child_pos.template get_parent<internal_node_t>(), child_pos.get_pos()))
            .si_then([on_found = std::move(on_found)](InternalNodeRef node) { return on_found(node); });
    }

    using lookup_leaf_iertr = base_iertr;
    using lookup_leaf_ret = lookup_leaf_iertr::future<>;
    template<typename F>
    static lookup_internal_level_ret lookup_leaf(op_context_t<node_key_t> c, iterator& iter, F& f,
                                                 mapped_space_visitor_t* visitor)
    {
        ...
        auto on_found = [visitor, &iter, &f](LeafNodeRef node) {
            ...
            if (visitor)
                (*visitor)(
                    node->get_paddr(), node->get_node_meta().begin, node->get_length(), 1, node->get_type(), iter);
            return seastar::now();
        };
        ...
        return get_leaf_node(c,
                             ...
                             std::make_optional<node_position_t<leaf_node_t>>(
                                 child_pos.template get_parent<leaf_node_t>(), child_pos.get_pos()))
            .si_then([on_found = std::move(on_found)](LeafNodeRef node) { return on_found(node); });
    }
    ...
    static lookup_depth_range_ret
    lookup_depth_range(op_context_t<node_key_t> c,       ///< [in] context
    ...
    )
    {
        ...
        return seastar::do_with(from, [c, to, visitor, &iter, &li, &ll](auto& d) {
            return trans_intr::repeat([c, to, visitor, &iter, &li, &ll, &d] {
                if (d > to) {
                    return [&] {
                        if (d > 1) {
                            return lookup_internal_level(c, d, iter, li, visitor);
                        }
                        else {
                            assert(d == 1);
                            return lookup_leaf(c, iter, ll, visitor);
                        }
                    }()
                               .si_then([&d] {
                                   --d;
                                   return lookup_depth_range_iertr::make_ready_future<seastar::stop_iteration>(
                                       seastar::stop_iteration::no);
                               });
                }
                else {
    ...
    template<typename LI, typename LL>
    lookup_ret lookup(op_context_t<node_key_t> c, LI&& lookup_internal, LL&& lookup_leaf, depth_t min_depth,
                      mapped_space_visitor_t* visitor) const
    {
        ...
        return seastar::do_with(
            ...
            [FNAME, this, visitor, c, min_depth](auto& iter, auto& li, auto& ll) {
                return lookup_root(c, iter, visitor)
                    .si_then([FNAME, this, visitor, c, &iter, &li, &ll, min_depth] {
                        ...
                        SUBTRACET(seastore_fixedkv_tree, "got root, depth {}", c.trans, get_root().get_depth());
                        return lookup_depth_range(c, iter, get_root().get_depth() - 1, min_depth - 1, li, ll, visitor)
                    ...
                    .si_then([&iter] { return std::move(iter); });
            });
    }
    ...
    static find_insertion_ret find_insertion(op_context_t<node_key_t> c, node_key_t laddr, iterator& iter)
    {
        ...
        if (!iter.is_end() && iter.get_key() == laddr) {
            ...
            return iter.prev(c).si_then([laddr, &iter](auto p) {
    ...
    handle_split_ret handle_split(op_context_t<node_key_t> c, iterator& iter)
    {
        ...
        if (split_from == iter.get_depth()) {
            ...
            nroot->set_meta(meta);
            nroot->range = meta;
            nroot->journal_insert(nroot->begin(), min_max_t<node_key_t>::min, get_root().get_location(), nullptr);
        ...
        auto split_level = [&](auto& parent_pos, auto& pos) {
            ...
            parent_node->update(parent_iter, left->get_paddr(), left.get());
            parent_node->insert(parent_iter + 1, pivot, right->get_paddr(), right.get());

            SUBTRACET(seastore_fixedkv_tree, "splitted {} into left: {}, right: {}", c.trans, *pos.node, *left, *right);
        ...
        for (; split_from > 0; --split_from) {
            ...
            if (!parent_pos.node->is_mutable()) {
                parent_pos.node =
                    c.cache.duplicate_for_write(c.trans, parent_pos.node)->template cast<internal_node_t>();
            }
    ...
    handle_merge_ret handle_merge(op_context_t<node_key_t> c, iterator& iter)
    {
        LOG_PREFIX(FixedKVBtree::handle_merge);
        if (iter.get_depth() == 1 || !iter.leaf.node->below_min_capacity()) {
            SUBTRACET(seastore_fixedkv_tree,
        ...
        return seastar::do_with(depth_t{1}, [FNAME, this, c, &iter](auto& to_merge) {
            return trans_intr::repeat([FNAME, this, c, &iter, &to_merge] {
                SUBTRACET(seastore_fixedkv_tree, "merging depth {}", c.trans, to_merge);
                ...
                return merge_fut.si_then([FNAME, this, c, &iter, &to_merge] {
                    ...
                    if (to_merge == iter.get_depth()) {
                        if (pos.node->get_size() == 1) {
                            ...
                            root_block = c.cache.duplicate_for_write(c.trans, root_block)->template cast<RootBlock>();
                            get_root().set_location(node_iter->get_val().maybe_relative_to(pos.node->get_paddr()));
                            get_root().set_depth(iter.get_depth());
                            if (iter.get_depth() > 1) {
    ...
    template<typename NodeType, std::enable_if_t<std::is_same_v<NodeType, leaf_node_t>, int> = 0>
    base_iertr::future<typename NodeType::Ref> get_node(op_context_t<node_key_t> c, depth_t depth, paddr_t addr,
                                                        node_key_t begin, node_key_t end,
                                                        typename std::optional<node_position_t<leaf_node_t>> parent_pos)
    {
        assert(depth == 1);
    ...
    template<typename NodeType, std::enable_if_t<std::is_same_v<NodeType, internal_node_t>, int> = 0>
    base_iertr::future<typename NodeType::Ref>
    get_node(op_context_t<node_key_t> c, depth_t depth, paddr_t addr, node_key_t begin, node_key_t end,
             typename std::optional<node_position_t<internal_node_t>> parent_pos)
    {
    ...
    handle_merge_ret merge_level(op_context_t<node_key_t> c, depth_t depth,
                                 node_position_t<internal_node_t>& parent_pos, node_position_t<NodeType>& pos)
    {
        LOG_PREFIX(FixedKVBtree::merge_level);
        if (!parent_pos.node->is_mutable()) {
            parent_pos.node = c.cache.duplicate_for_write(c.trans, parent_pos.node)->template cast<internal_node_t>();
        }
        ...
        auto end = next_iter == parent_pos.node->end() ? parent_pos.node->get_node_meta().end : next_iter->get_key();

        SUBTRACET(seastore_fixedkv_tree, "parent: {}, node: {}", c.trans, *parent_pos.node, *pos.node);
        auto do_merge = [c, iter, donor_iter, donor_is_left, &parent_pos, &pos](typename NodeType::Ref donor) {
            LOG_PREFIX(FixedKVBtree::merge_level);
            auto [l, r] = donor_is_left ? std::make_pair(donor, pos.node) : std::make_pair(pos.node, donor);

            auto [liter, riter] = donor_is_left ? std::make_pair(donor_iter, iter) : std::make_pair(iter, donor_iter);

            if (donor->at_min_capacity()) {
                auto replacement = l->make_full_merge(c, r);

                parent_pos.node->update(liter, replacement->get_paddr(), replacement.get());
                ...
                LOG_PREFIX(FixedKVBtree::merge_level);
                auto [replacement_l, replacement_r, pivot] = l->make_balanced(c, r, !donor_is_left);

                parent_pos.node->update(liter, replacement_l->get_paddr(), replacement_l.get());
                parent_pos.node->replace(riter, pivot, replacement_r->get_paddr(), replacement_r.get());
                ...
                auto orig_position = donor_is_left ? l->get_size() + pos.pos : pos.pos;
        ...
        if (v.has_child()) {
            return v.get_child_fut().safe_then(
                [do_merge = std::move(do_merge), &pos, donor_iter, donor_is_left, c, parent_pos](auto child) mutable {
                    LOG_PREFIX(FixedKVBtree::merge_level);
                    SUBTRACET(seastore_fixedkv_tree,
                              "got child on {}, pos: {}, res: {}",
                              c.trans,
                    ...
                    assert(donor_is_left ? node.get_node_meta().end == pos.node->get_node_meta().begin
        ...
        auto child_pos = v.get_child_pos();
        return get_node<NodeType>(c,
                                  depth,
                                  donor_iter.get_val().maybe_relative_to(parent_pos.node->get_paddr()),
                                  begin,
                                  end,
                                  std::make_optional<node_position_t<NodeType>>(
                                      child_pos.template get_parent<NodeType>(), child_pos.get_pos()))
            .si_then([do_merge = std::move(do_merge)](typename NodeType::Ref donor) { return do_merge(donor); });
    }
};

template<typename T> struct is_fixed_kv_tree : std::false_type
{};
...
template<typename tree_type_t, typename node_key_t, typename F,
         std::enable_if_t<is_fixed_kv_tree<tree_type_t>::value, int> = 0>
auto with_btree(Cache& cache, op_context_t<node_key_t> c, F&& f)
{
    return cache.get_root(c.trans).si_then([f = std::forward<F>(f)](RootBlockRef croot) mutable {
        return seastar::do_with(tree_type_t(croot), [f = std::move(f)](auto& btree) mutable { return f(btree); });
    });
}

template<typename tree_type_t, typename State, typename node_key_t, typename F,
         std::enable_if_t<is_fixed_kv_tree<tree_type_t>::value, int> = 0>
auto with_btree_state(Cache& cache, op_context_t<node_key_t> c, State&& init, F&& f)
{
    return seastar::do_with(std::forward<State>(init), [&cache, c, f = std::forward<F>(f)](auto& state) mutable {
        return with_btree<tree_type_t>(
                   cache, c, [&state, f = std::move(f)](auto& btree) mutable { return f(btree, state); })
            .si_then([&state] { return seastar::make_ready_future<State>(std::move(state)); });
    });
}

template<typename tree_type_t, typename State, typename node_key_t, typename F,
         std::enable_if_t<is_fixed_kv_tree<tree_type_t>::value, int> = 0>
auto with_btree_state(Cache& cache, op_context_t<node_key_t> c, F&& f)
{
    return crimson::os::seastore::with_btree_state<tree_type_t, State>(cache, c, State{}, std::forward<F>(f));
}

template<typename tree_type_t, typename Ret, typename node_key_t, typename F>
auto with_btree_ret(Cache& cache, op_context_t<node_key_t> c, F&& f)
{
    return with_btree_state<tree_type_t, Ret>(cache, c, [f = std::forward<F>(f)](auto& btree, auto& ret) mutable {
        return f(btree).si_then([&ret](auto&& _ret) { ret = std::move(_ret); });
    });
}
