 ...
 */
template<typename node_key_t> struct FixedKVNode : ChildableCachedExtent
{
    ...
    struct copy_source_cmp_t
    {
        ...
        {
            assert(l->range.end <= r->range.begin || r->range.end <= l->range.begin ||
                   (l->range.begin == r->range.begin && l->range.end == r->range.end));
            return l->range.begin < r->range.begin;
        }
        bool operator()(const node_key_t& l, const FixedKVNodeRef& r) const { return l < r->range.begin; }
        bool operator()(const FixedKVNodeRef& l, const node_key_t& r) const { return l->range.begin < r; }
    };
    ...
    template<typename iter_t> void update_child_ptr(iter_t iter, ChildableCachedExtent* child)
    {
    ...
    template<typename iter_t> void insert_child_ptr(iter_t iter, ChildableCachedExtent* child)
    {
        auto raw_children = children.data();
        auto offset = iter.get_offset();
        std::memmove(&raw_children[offset + 1],
                     &raw_children[offset],
    ...
    template<typename iter_t> void remove_child_ptr(iter_t iter)
    {
        ...
        // reset it here
        std::memmove(&raw_children[offset],
    ...
    static void push_copy_sources(FixedKVNode& dest, FixedKVNode& src)
    {
        ...
        if (!src.is_pending()) {
            ...
            dest.copy_sources.emplace(src.get_prior_instance()->template cast<FixedKVNode>());
        }
        else {
            ceph_assert(src.is_initial_pending());
            dest.copy_sources.insert(src.copy_sources.begin(), src.copy_sources.end());
        }
    }

    virtual uint16_t get_node_split_pivot() = 0;

    static void move_child_ptrs(FixedKVNode& dest, FixedKVNode& src, size_t dest_start, size_t src_start,
                                size_t src_end)
    {
        std::memmove(dest.children.data() + dest_start,
        ...
        for (auto it = src.children.begin() + src_start; it != src.children.begin() + src_end; it++) {
            auto child = *it;
    ...
    virtual get_child_ret_t<LogicalCachedExtent> get_logical_child(op_context_t<node_key_t> c, uint16_t pos) = 0;

    template<typename T, typename iter_t> get_child_ret_t<T> get_child(op_context_t<node_key_t> c, iter_t iter)
    {
    ...
    void split_child_ptrs(FixedKVNode& left, FixedKVNode& right)
    {
    ...
    void merge_child_ptrs(FixedKVNode& left, FixedKVNode& right)
    {
    ...
    static void balance_child_ptrs(FixedKVNode& left, FixedKVNode& right, bool prefer_left,
                                   FixedKVNode& replacement_left, FixedKVNode& replacement_right)
    {
    ...
    {
        for (auto it = children.begin(); it != children.begin() + get_node_size(); it++) {
            if (is_valid_child_ptr(*it) && (*it)->is_valid()) {
                return false;
            }
        }
        return true;
    ...
    uint16_t copy_children_from_stable_source(FixedKVNode& source, iter_t foreign_start_it, iter_t foreign_end_it,
                                              iter_t local_start_it)
    {
        auto foreign_it = foreign_start_it, local_it = local_start_it;
        while (foreign_it != foreign_end_it && local_it.get_offset() < get_node_size()) {
    ...
    template<typename Func> void copy_children_from_stable_sources(Func&& get_iter)
    {
        if (!copy_sources.empty()) {
            auto it = --copy_sources.upper_bound(get_node_meta().begin);
            auto& cs = *it;
            uint16_t start_pos = cs->lower_bound_offset(get_node_meta().begin);
    ...
    void on_invalidated(Transaction& t) final { reset_parent_tracker(); }

    bool is_rewrite() { return is_initial_pending() && get_prior_instance(); }
 ...
 * FixedKVBTree.
 */
template<size_t CAPACITY, typename NODE_KEY, typename NODE_KEY_LE, size_t node_size, typename node_type_t>
struct FixedKVInternalNode
    : FixedKVNode<NODE_KEY>,
      common::FixedKVNodeLayout<CAPACITY, fixed_kv_node_meta_t<NODE_KEY>, fixed_kv_node_meta_le_t<NODE_KEY_LE>,
                                NODE_KEY, NODE_KEY_LE, paddr_t, paddr_le_t>
{
    ...
    using this_type_t = FixedKVInternalNode<CAPACITY, NODE_KEY, NODE_KEY_LE, node_size, node_type_t>;
    ...
    bool is_leaf_and_has_children() const final { return false; }

    uint16_t get_node_split_pivot() final { return this->get_split_pivot().get_offset(); }
    ...
    void prepare_commit() final
    {
        if (this->is_initial_pending()) {
            ...
            this->copy_children_from_stable_sources([this](base_t& node, uint16_t pos) {
    ...
    get_child_ret_t<LogicalCachedExtent> get_logical_child(op_context_t<NODE_KEY>, uint16_t pos) final
    {
    ...
    uint16_t lower_bound_offset(NODE_KEY key) const final { return this->lower_bound(key).get_offset(); }

    uint16_t upper_bound_offset(NODE_KEY key) const final { return this->upper_bound(key).get_offset(); }
    ...
    NODE_KEY get_key_from_idx(uint16_t idx) const final { return this->iter_idx(idx).get_key(); }

    fixed_kv_node_meta_t<NODE_KEY> get_node_meta() const { return this->get_meta(); }

    uint16_t get_node_size() const final { return this->get_size(); }
    ...
    void on_replace_prior(Transaction&) final
    {
        ...
        auto copied = this->copy_children_from_stable_source(prior, prior.begin(), prior.end(), this->begin());
    ...
    void update(internal_const_iterator_t iter, paddr_t addr, FixedKVNode<NODE_KEY>* nextent)
    {
        ...
        SUBTRACE(
            seastore_fixedkv_tree, "trans.{}, pos {}, {}", this->pending_for_transaction, iter.get_offset(), *nextent);
    ...
    void insert(internal_const_iterator_t iter, NODE_KEY pivot, paddr_t addr, FixedKVNode<NODE_KEY>* nextent)
    {
        ...
        return this->journal_insert(iter, pivot, this->maybe_generate_relative(addr), maybe_get_delta_buffer());
    }
    ...
    void remove(internal_const_iterator_t iter)
    {
        ...
        return this->journal_remove(iter, maybe_get_delta_buffer());
    }

    void replace(internal_const_iterator_t iter, NODE_KEY pivot, paddr_t addr, FixedKVNode<NODE_KEY>* nextent)
    {
        ...
        SUBTRACE(seastore_fixedkv_tree,
                 ...
                 *nextent);
        this->update_child_ptr(iter, nextent);
        return this->journal_replace(iter, pivot, this->maybe_generate_relative(addr), maybe_get_delta_buffer());
    }

    std::tuple<Ref, Ref, NODE_KEY> make_split_children(op_context_t<NODE_KEY> c)
    {
        auto left =
            c.cache.template alloc_new_extent<node_type_t>(c.trans, node_size, placement_hint_t::HOT, INIT_GENERATION);
        auto right =
            c.cache.template alloc_new_extent<node_type_t>(c.trans, node_size, placement_hint_t::HOT, INIT_GENERATION);
        ...
        return std::make_tuple(left, right, pivot);
    }

    Ref make_full_merge(op_context_t<NODE_KEY> c, Ref& right)
    {
        auto replacement =
            c.cache.template alloc_new_extent<node_type_t>(c.trans, node_size, placement_hint_t::HOT, INIT_GENERATION);
    ...
    std::tuple<Ref, Ref, NODE_KEY> make_balanced(op_context_t<NODE_KEY> c, Ref& _right, bool prefer_left)
    {
        ceph_assert(_right->get_type() == this->get_type());
        auto& right = *_right->template cast<node_type_t>();
        auto replacement_left =
            c.cache.template alloc_new_extent<node_type_t>(c.trans, node_size, placement_hint_t::HOT, INIT_GENERATION);
        auto replacement_right =
            c.cache.template alloc_new_extent<node_type_t>(c.trans, node_size, placement_hint_t::HOT, INIT_GENERATION);

        auto pivot = this->balance_into_new_nodes(*this, right, prefer_left, *replacement_left, *replacement_right);
        ...
        return std::make_tuple(replacement_left, replacement_right, pivot);
    }
    ...
    void node_resolve_vals(internal_iterator_t from, internal_iterator_t to) const
    {
        if (this->is_initial_pending()) {
    ...
    void node_unresolve_vals(internal_iterator_t from, internal_iterator_t to) const
    {
        if (this->is_initial_pending()) {
            for (auto i = from; i != to; ++i) {
                if (i->get_val().is_relative()) {
    ...
    constexpr static size_t get_min_capacity() { return (node_layout_t::get_capacity() - 1) / 2; }
...
template<size_t CAPACITY, typename NODE_KEY, typename NODE_KEY_LE, typename VAL, typename VAL_LE, size_t node_size,
         typename node_type_t, bool has_children>
struct FixedKVLeafNode
    : FixedKVNode<NODE_KEY>,
      common::FixedKVNodeLayout<CAPACITY, fixed_kv_node_meta_t<NODE_KEY>, fixed_kv_node_meta_le_t<NODE_KEY_LE>,
                                NODE_KEY, NODE_KEY_LE, VAL, VAL_LE>
{
    using Ref = TCachedExtentRef<node_type_t>;
    using node_layout_t =
        common::FixedKVNodeLayout<CAPACITY, fixed_kv_node_meta_t<NODE_KEY>, fixed_kv_node_meta_le_t<NODE_KEY_LE>,
    ...
    using internal_const_iterator_t = typename node_layout_t::const_iterator;
    using this_type_t =
        FixedKVLeafNode<CAPACITY, NODE_KEY, NODE_KEY_LE, VAL, VAL_LE, node_size, node_type_t, has_children>;
    using base_t = FixedKVNode<NODE_KEY>;
    ...
    bool is_leaf_and_has_children() const final { return has_children; }

    uint16_t get_node_split_pivot() final { return this->get_split_pivot().get_offset(); }

    get_child_ret_t<LogicalCachedExtent> get_logical_child(op_context_t<NODE_KEY> c, uint16_t pos) final
    {
        auto child = this->children[pos];
        if (is_valid_child_ptr(child)) {
            ceph_assert(child->is_logical());
            return c.cache.template get_extent_viewable_by_trans<LogicalCachedExtent>(c.trans,
                                                                                      (LogicalCachedExtent*)child);
        }
    ...
    bool validate_stable_children() override { return true; }
    ...
    void prepare_commit() final
    {
        ...
        assert(this->is_initial_pending() ? this->copy_sources.empty() : true);
    }
    ...
    uint16_t child_pos_for_key(NODE_KEY key) const final { return lower_bound_offset(key); }
    ...
    virtual void update(internal_const_iterator_t iter, VAL val, LogicalCachedExtent* nextent) = 0;
    virtual internal_const_iterator_t insert(internal_const_iterator_t iter, NODE_KEY addr, VAL val,
    ...
    std::ostream& _print_detail(std::ostream& out) const
    {
        return out << ", size=" << this->get_size() << ", meta=" << this->get_meta();
    }
...
template<>
struct fmt::formatter<crimson::os::seastore::FixedKVNode<crimson::os::seastore::laddr_t>> : fmt::ostream_formatter
{};
template<>
struct fmt::formatter<crimson::os::seastore::FixedKVNode<crimson::os::seastore::paddr_t>> : fmt::ostream_formatter
{};
#endif
