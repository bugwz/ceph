...
struct ObjectDataBlock : crimson::os::seastore::LogicalCachedExtent
{
    ...
    CachedExtentRef duplicate_for_write(Transaction&) final { return CachedExtentRef(new ObjectDataBlock(*this)); };
...
public:
    ...
    ObjectDataHandler(uint32_t mos)
        : max_object_size(mos)
    {}
...
private:
    ...
    write_ret prepare_data_reservation(context_t ctx, object_data_t& object_data, extent_len_t size);

    /// Trims data past size
    clear_ret trim_data_reservation(context_t ctx, object_data_t& object_data, extent_len_t size);

    clone_ret clone_extents(context_t ctx, object_data_t& object_data, lba_pin_list_t& pins, laddr_t data_base);
