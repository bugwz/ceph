...
RBMDevice::mkfs_ret RBMDevice::do_primary_mkfs(device_config_t config, int shard_num, size_t journal_size)
{
    LOG_PREFIX(RBMDevice::do_primary_mkfs);
    return stat_device()
        .handle_error(mkfs_ertr::pass_further{},
                      crimson::ct_error::assert_all{"Invalid error stat_device in RBMDevice::do_primary_mkfs"})
        .safe_then([this, FNAME, config = std::move(config), shard_num, journal_size](auto st) {
            ...
            for (int i = 0; i < shard_num; i++) {
                uint64_t aligned_size = (super.size / shard_num) - ((super.size / shard_num) % super.block_size);
            ...
            // write super block
            return open(get_device_path(), seastar::open_flags::rw | seastar::open_flags::dsync)
                .handle_error(mkfs_ertr::pass_further{},
                              crimson::ct_error::assert_all{"Invalid error open in RBMDevice::do_primary_mkfs"})
                .safe_then([this] {
...
read_ertr::future<rbm_metadata_header_t> RBMDevice::read_rbm_header(rbm_abs_addr addr)
{
    LOG_PREFIX(RBMDevice::read_rbm_header);
    assert(super.block_size > 0);
    return seastar::do_with(
        bufferptr(ceph::buffer::create_page_aligned(super.block_size)), [this, addr, FNAME](auto& bptr) {
            return read(addr, bptr)
                .safe_then([length = bptr.length(), this, bptr, FNAME]() -> read_ertr::future<rbm_metadata_header_t> {
                    bufferlist bl;
                    bl.append(bptr);
                    auto p = bl.cbegin();
                    ...
                    try {
                        ...
                        DEBUG("read_rbm_header: unable to decode rbm super block {}", e.what());
                    ...
                    assert(ceph::encoded_sizeof<rbm_metadata_header_t>(super_block) < super_block.block_size);
                    ...
                    if (is_data_protection_enabled() == false) {
                        if (meta_b_header.crc32c(-1) != crc) {
                            DEBUG("bad crc on super block, expected {} != actual {} ", meta_b_header.crc32c(-1), crc);
                    ...
                    super = super_block;
                    DEBUG("got {} ", super);
                    return read_ertr::future<rbm_metadata_header_t>(read_ertr::ready_future_marker{}, super_block);
                });
        });
}
...
RBMDevice::mount_ret RBMDevice::do_shard_mount()
{
    return open(get_device_path(), seastar::open_flags::rw | seastar::open_flags::dsync)
        .safe_then([this] {
            return stat_device()
                .handle_error(mount_ertr::pass_further{},
                              crimson::ct_error::assert_all{"Invalid error stat_device in RBMDevice::do_shard_mount"})
                .safe_then([this](auto st) {
                    assert(st.block_size > 0);
                    super.block_size = st.block_size;
                    return read_rbm_header(RBM_START_ADDRESS).safe_then([this](auto s) {
        ...
        .handle_error(mount_ertr::pass_further{},
                      crimson::ct_error::assert_all{"Invalid error mount in RBMDevice::do_shard_mount"});
}

EphemeralRBMDeviceRef create_test_ephemeral(uint64_t journal_size, uint64_t data_size)
{
    return EphemeralRBMDeviceRef(
        new EphemeralRBMDevice(journal_size + data_size + random_block_device::RBMDevice::get_shard_reserved_size(),
                               EphemeralRBMDevice::TEST_BLOCK_SIZE));
}

open_ertr::future<> EphemeralRBMDevice::open(const std::string& in_path, seastar::open_flags mode)
{
    ...
    DEBUG("Initializing test memory device {}", size);
...
write_ertr::future<> EphemeralRBMDevice::write(uint64_t offset, bufferptr&& bptr, uint16_t stream)
{
    LOG_PREFIX(EphemeralRBMDevice::write);
    ceph_assert(buf);
    DEBUG("EphemeralRBMDevice: write offset {} len {}", offset, bptr.length());
...
read_ertr::future<> EphemeralRBMDevice::read(uint64_t offset, bufferptr& bptr)
{
    LOG_PREFIX(EphemeralRBMDevice::read);
    ceph_assert(buf);
    DEBUG("EphemeralRBMDevice: read offset {} len {}", offset, bptr.length());
...
write_ertr::future<> EphemeralRBMDevice::writev(uint64_t offset, ceph::bufferlist bl, uint16_t stream)
{
    LOG_PREFIX(EphemeralRBMDevice::writev);
    ceph_assert(buf);
    DEBUG("EphemeralRBMDevice: write offset {} len {}", offset, bl.length());
