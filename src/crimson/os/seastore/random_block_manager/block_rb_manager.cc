...
device_config_t get_rbm_ephemeral_device_config(std::size_t index, std::size_t num_devices)
{
    ...
    if (index == 0) {
        ...
        for (std::size_t secondary_index = index + 1; secondary_index < num_devices; ++secondary_index) {
            ...
            secondary_devices.insert({secondary_index, device_spec_t{magic, type, secondary_id}});
        }
    }
    else {   // index > 0
    ...
    return {is_major_device, device_spec_t{magic, type, id}, meta, secondary_devices};
}
...
paddr_t BlockRBManager::alloc_extent(size_t size)
{
    ...
    paddr_t paddr = convert_abs_addr_to_paddr(extent.get_start(), device->get_device_id());
    DEBUG("allocated addr: {}, size: {}, requested size: {}", paddr, extent.get_len(), size);
    return paddr;
}

void BlockRBManager::complete_allocation(paddr_t paddr, size_t size)
{
    assert(allocator);
...
BlockRBManager::open_ertr::future<> BlockRBManager::open()
{
    ...
    allocator->init(ool_start, device->get_shard_end() - ool_start, device->get_block_size());
    return open_ertr::now();
}

BlockRBManager::write_ertr::future<> BlockRBManager::write(paddr_t paddr, bufferptr& bptr)
{
    ...
    if (addr < start || addr + bptr.length() > end) {
        ERROR("out of range: start {}, end {}, addr {}, length {}", start, end, addr, bptr.length());
        return crimson::ct_error::erange::make();
    }
    bufferptr bp = bufferptr(ceph::buffer::create_page_aligned(bptr.length()));
    bp.copy_in(0, bptr.length(), bptr.c_str());
    return device->write(addr, std::move(bp));
}

BlockRBManager::read_ertr::future<> BlockRBManager::read(paddr_t paddr, bufferptr& bptr)
{
...
BlockRBManager::write_ertr::future<> BlockRBManager::write(rbm_abs_addr addr, bufferlist& bl)
{
    LOG_PREFIX(BlockRBManager::write);
    ceph_assert(device);
    bufferptr bptr;
    ...
    return device->write(addr, std::move(bptr));
}

std::ostream& operator<<(std::ostream& out, const rbm_metadata_header_t& header)
{
    out << " rbm_metadata_header_t(size=" << header.size << ", block_size=" << header.block_size
        << ", feature=" << header.feature << ", journal_size=" << header.journal_size << ", crc=" << header.crc
        << ", config=" << header.config << ", shard_num=" << header.shard_num;
...
{
    out << " rbm_shard_info_t(size=" << shard.size << ", start_offset=" << shard.start_offset;
    return out << ")";
}
