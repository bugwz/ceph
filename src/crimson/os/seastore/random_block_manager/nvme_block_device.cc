...
NVMeBlockDevice::mkfs_ret NVMeBlockDevice::mkfs(device_config_t config)
{
    ...
    return shard_devices.local().do_primary_mkfs(
        config, seastar::smp::count, get_conf<Option::size_t>("seastore_cbjournal_size"));
}

open_ertr::future<> NVMeBlockDevice::open(const std::string& in_path, seastar::open_flags mode)
{
    return seastar::do_with(in_path, [this, mode](auto& in_path) {
        return seastar::file_stat(in_path).then([this, mode, in_path](auto stat) {
            return seastar::open_file_dma(in_path, mode).then([=, this](auto file) {
                ...
                return identify_controller(device)
                    .safe_then([this, in_path, mode](auto id_controller_data) {
                        ...
                        return identify_namespace(device).safe_then([this, in_path, mode](auto id_namespace_data) {
...
open_ertr::future<> NVMeBlockDevice::open_for_io(const std::string& in_path, seastar::open_flags mode)
{
    ...
    return seastar::do_for_each(io_device, [=, this](auto& target_device) {
        return seastar::open_file_dma(in_path, mode).then([this](auto file) {
            ...
            return io_device[stream_index_to_open]
                .fcntl(F_SET_FILE_RW_HINT, (uintptr_t)&stream_index_to_open)
                .then([this](auto ret) {
...
write_ertr::future<> NVMeBlockDevice::write(uint64_t offset, bufferptr&& bptr, uint16_t stream)
{
    ...
    return seastar::do_with(std::move(bptr), [this, offset, length, supported_stream](auto& bptr) {
        return io_device[supported_stream]
            .dma_write(offset, bptr.c_str(), length)
...
read_ertr::future<> NVMeBlockDevice::read(uint64_t offset, bufferptr& bptr)
{
...
write_ertr::future<> NVMeBlockDevice::writev(uint64_t offset, ceph::bufferlist bl, uint16_t stream)
{
    logger().debug("block: write offset {} len {}", offset, bl.length());
    ...
    return seastar::do_with(bl.prepare_iovs(), std::move(bl), [this, supported_stream, offset](auto& iovs, auto& bl) {
        return write_ertr::parallel_for_each(iovs, [this, supported_stream, offset](auto& p) mutable {
            auto off = offset + p.offset;
            auto len = p.length;
            auto& iov = p.iov;
            return io_device[supported_stream]
                .dma_write(off, std::move(iov))
...
Device::close_ertr::future<> NVMeBlockDevice::close()
{
    ...
    return device.close().then(
        [this]() { return seastar::do_for_each(io_device, [](auto target_device) { return target_device.close(); }); });
}

nvme_command_ertr::future<nvme_identify_controller_data_t> NVMeBlockDevice::identify_controller(seastar::file f)
{
    return seastar::do_with(
        nvme_admin_command_t(), nvme_identify_controller_data_t(), [this, f](auto& admin_command, auto& data) {
            ...
            return pass_admin(admin_command, f).safe_then([&data](auto status) {
                return seastar::make_ready_future<nvme_identify_controller_data_t>(std::move(data));
            });
        });
}
...
nvme_command_ertr::future<nvme_identify_namespace_data_t> NVMeBlockDevice::identify_namespace(seastar::file f)
{
    return get_nsid(f).safe_then([this, f](auto nsid) {
        return seastar::do_with(
            nvme_admin_command_t(), nvme_identify_namespace_data_t(), [this, nsid, f](auto& admin_command, auto& data) {
                admin_command.common.opcode = nvme_admin_command_t::OPCODE_IDENTIFY;
                admin_command.common.addr = (uint64_t)&data;
                admin_command.common.data_len = sizeof(data);
                ...
                return pass_admin(admin_command, f).safe_then([&data](auto status) {
                    return seastar::make_ready_future<nvme_identify_namespace_data_t>(std::move(data));
                });
            });
    });
}
...
{
    return f.ioctl(NVME_IOCTL_ID, nullptr).handle_exception([](auto e) -> nvme_command_ertr::future<int> {
        logger().error("pass_admin: ioctl failed");
        return crimson::ct_error::input_output_error::make();
    });
}

nvme_command_ertr::future<int> NVMeBlockDevice::pass_admin(nvme_admin_command_t& admin_cmd, seastar::file f)
{
    return f.ioctl(NVME_IOCTL_ADMIN_CMD, &admin_cmd).handle_exception([](auto e) -> nvme_command_ertr::future<int> {
        logger().error("pass_admin: ioctl failed");
        return crimson::ct_error::input_output_error::make();
    });
}

nvme_command_ertr::future<int> NVMeBlockDevice::pass_through_io(nvme_io_command_t& io_cmd)
{
