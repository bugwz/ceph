...
rbm_abs_addr AvlAllocator::find_block(size_t size)
{
    ...
    auto iter = extent_size_tree.lower_bound(extent_range_t{base_addr, base_addr + size}, comp);
...
void AvlAllocator::_add_to_tree(rbm_abs_addr start, rbm_abs_addr size)
{
    ...
    auto rs_after = extent_tree.upper_bound(extent_range_t{start, end}, extent_tree.key_comp());
...
std::optional<interval_set<rbm_abs_addr>> AvlAllocator::alloc_extent(size_t size)
{
    ...
    auto try_to_alloc_block = [this, &result, FNAME](uint64_t alloc_size) -> uint64_t {
        ...
        if (start != base_addr + total_size) {
            ...
            DEBUG("allocate addr: {}, allocate size: {}, available size: {}", start, alloc_size, available_size);
