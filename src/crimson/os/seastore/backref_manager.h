...
public:
    ...
    virtual get_mappings_ret get_mappings(Transaction& t, paddr_t offset, paddr_t end) = 0;
    ...
    virtual get_mapping_ret get_mapping(Transaction& t, paddr_t offset) = 0;
    ...
    virtual new_mapping_ret new_mapping(Transaction& t, paddr_t key, extent_len_t len, laddr_t val,
    ...
    virtual Cache::backref_entry_query_mset_t get_cached_backref_entries_in_range(paddr_t start, paddr_t end) = 0;
    ...
    virtual retrieve_backref_extents_in_range_ret retrieve_backref_extents_in_range(Transaction& t, paddr_t start,
                                                                                    paddr_t end) = 0;

    virtual void cache_new_backref_extent(paddr_t paddr, paddr_t key, extent_types_t type) = 0;
    ...
    virtual merge_cached_backrefs_ret merge_cached_backrefs(Transaction& t, const journal_seq_t& limit,
     ...
     */
    using remove_mapping_iertr = base_iertr::extend<crimson::ct_error::enoent>;
    using remove_mapping_ret = remove_mapping_iertr::future<remove_mapping_result_t>;
    virtual remove_mapping_ret remove_mapping(Transaction& t, paddr_t offset) = 0;
    ...
    using scan_mapped_space_func_t = std::function<void(paddr_t, paddr_t, extent_len_t, extent_types_t, laddr_t)>;
    virtual scan_mapped_space_ret scan_mapped_space(Transaction& t, scan_mapped_space_func_t&& f) = 0;

    virtual ~BackrefManager() {}
};

using BackrefManagerRef = std::unique_ptr<BackrefManager>;
