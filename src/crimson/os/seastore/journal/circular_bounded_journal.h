...
{
public:
    CircularBoundedJournal(JournalTrimmer& trimmer, RBMDevice* device, const std::string& path);
    ~CircularBoundedJournal() {}

    JournalTrimmer& get_trimmer() final { return trimmer; }
    ...
    journal_type_t get_type() final { return journal_type_t::RANDOM_BLOCK; }

    submit_record_ret submit_record(record_t&& record, OrderingHandle& handle) final;
    ...
    rbm_abs_addr get_rbm_addr(journal_seq_t seq) const { return convert_paddr_to_abs_addr(seq.offset); }
    ...
    seastar::future<> update_journal_tail(journal_seq_t dirty, journal_seq_t alloc)
    {
        return cjs.update_journal_tail(dirty, alloc);
    }
    journal_seq_t get_dirty_tail() const { return cjs.get_dirty_tail(); }
    journal_seq_t get_alloc_tail() const { return cjs.get_alloc_tail(); }

    void set_write_pipeline(WritePipeline* _write_pipeline) final { write_pipeline = _write_pipeline; }

    device_id_t get_device_id() const { return cjs.get_device_id(); }
    extent_len_t get_block_size() const { return cjs.get_block_size(); }

    rbm_abs_addr get_journal_end() const { return cjs.get_journal_end(); }

    void set_written_to(journal_seq_t seq) { cjs.set_written_to(seq); }

    journal_seq_t get_written_to() { return cjs.get_written_to(); }

    rbm_abs_addr get_records_start() const { return cjs.get_records_start(); }

    seastar::future<> finish_commit(transaction_type_t type) final;

    using cbj_delta_handler_t = std::function<replay_ertr::future<bool>(const record_locator_t&, const delta_info_t&,
                                                                        sea_time_point modify_time)>;

    Journal::replay_ret scan_valid_record_delta(cbj_delta_handler_t&& delta_handler, journal_seq_t tail);
    ...
    {
        paddr_t addr = convert_abs_addr_to_paddr(get_records_start(), get_device_id());
        cursor.seq.offset = addr;
        cursor.seq.segment_seq += 1;
    }

    void initialize_cursor(scan_valid_records_cursor& cursor) final { cursor.block_size = get_block_size(); };

    Journal::replay_ret replay_segment(cbj_delta_handler_t& handler, scan_valid_records_cursor& cursor);

    read_ret read(paddr_t start, size_t len) final;

    bool is_record_segment_seq_invalid(scan_valid_records_cursor& cursor, record_group_header_t& h) final;

    int64_t get_segment_end_offset(paddr_t addr) final { return get_journal_end(); }

    // Test interfaces

    CircularJournalSpace& get_cjs() { return cjs; }

    read_validate_record_metadata_ret test_read_validate_record_metadata(scan_valid_records_cursor& cursor,
                                                                         segment_nonce_t nonce)
    {
        return read_validate_record_metadata(cursor, nonce);
    }

    void test_initialize_cursor(scan_valid_records_cursor& cursor) { initialize_cursor(cursor); }
