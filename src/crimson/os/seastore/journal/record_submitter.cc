...
RecordBatch::add_pending_ret RecordBatch::add_pending(const std::string& name, record_t&& record,
                                                      extent_len_t block_size)
{
    ...
    return io_promise->get_shared_future().then([dlength_offset, FNAME, &name](
        ...
        auto submit_result = record_locator_t{
            write_result.start_seq.offset.add_offset(maybe_promise_result->mdlength + dlength_offset), write_result};
        TRACE("{} write finish with {}", name, submit_result);
        return add_pending_ret(add_pending_ertr::ready_future_marker{}, submit_result);
    });
}

std::pair<ceph::bufferlist, record_group_size_t> RecordBatch::encode_batch(const journal_seq_t& committed_to,
...
void RecordBatch::set_result(maybe_result_t maybe_write_result)
{
    maybe_promise_result_t result;
    if (maybe_write_result.has_value()) {
        assert(maybe_write_result->length == submitting_length);
        result = promise_result_t{*maybe_write_result, submitting_mdlength};
    }
    assert(state == state_t::SUBMITTING);
    assert(io_promise.has_value());
...
std::pair<ceph::bufferlist, record_group_size_t> RecordBatch::submit_pending_fast(record_t&& record,
                                                                                  extent_len_t block_size,
...
RecordSubmitter::RecordSubmitter(std::size_t io_depth, std::size_t batch_capacity, std::size_t batch_flush_size,
                                 double preferred_fullness, JournalAllocator& ja)
...
bool RecordSubmitter::is_available() const
{
    ...
    if (ret) {
        ...
        if (!p_current_batch->is_empty()) {
            auto submit_length = p_current_batch->get_submit_size().get_encoded_length();
...
RecordSubmitter::wa_ertr::future<> RecordSubmitter::wait_available()
{
    ...
    return wait_available_promise->get_shared_future().then([FNAME, this]() -> wa_ertr::future<> {
        if (has_io_error) {
...
RecordSubmitter::action_t RecordSubmitter::check_action(const record_size_t& rsize) const
{
    assert(is_available());
    auto eval = p_current_batch->evaluate_submit(rsize, journal_allocator.get_block_size());
...
RecordSubmitter::roll_segment_ertr::future<> RecordSubmitter::roll_segment()
{
               ...
               .then_wrapped([FNAME, this](auto fut) {
                   if (fut.failed()) {
                       ...
                       std::ignore = journal_allocator.roll()
                                         ...
                                         .handle_error(crimson::ct_error::all_same_way([FNAME, this](auto e) {
...
RecordSubmitter::submit_ret RecordSubmitter::submit(record_t&& record, bool with_atomic_roll_segment)
{
    ...
    auto eval = p_current_batch->evaluate_submit(record.size, journal_allocator.get_block_size());
    bool needs_flush =
        (state == state_t::IDLE || eval.submit_size.get_fullness() > preferred_fullness ||
         // RecordBatch::needs_flush()
         eval.is_full || p_current_batch->get_num_records() + 1 >= p_current_batch->get_batch_capacity());
    if (p_current_batch->is_empty() && needs_flush && state != state_t::FULL) {
        ...
        auto [to_write, sizes] = p_current_batch->submit_pending_fast(
            std::move(record), journal_allocator.get_block_size(), get_committed_to(), journal_allocator.get_nonce());
        ...
        return journal_allocator.write(std::move(to_write))
            .safe_then([mdlength = sizes.get_mdlength()](auto write_result) {
                return record_locator_t{write_result.start_seq.offset.add_offset(mdlength), write_result};
            })
            .finally([this] { decrement_io_with_flush(); });
    }
    // indirect batched write
    auto write_fut = p_current_batch->add_pending(get_name(), std::move(record), journal_allocator.get_block_size());
    if (needs_flush) {
        if (state == state_t::FULL) {
            ...
            if (with_atomic_roll_segment) {
                ...
                std::ignore = wait_unfull_flush_promise->get_future().finally([FNAME, this] {
...
RecordSubmitter::open_ret RecordSubmitter::open(bool is_mkfs)
{
    return journal_allocator.open(is_mkfs).safe_then([this](journal_seq_t ret) {
...
RecordSubmitter::close_ertr::future<> RecordSubmitter::close()
{
...
void RecordSubmitter::decrement_io_with_flush()
{
    ...
    auto needs_flush =
        (!p_current_batch->is_empty() &&
         (state == state_t::IDLE || p_current_batch->get_submit_size().get_fullness() > preferred_fullness ||
...
void RecordSubmitter::account_submission(std::size_t num, const record_group_size_t& size)
{
    stats.record_group_padding_bytes += (size.get_mdlength() - size.get_raw_mdlength());
...
void RecordSubmitter::finish_submit_batch(RecordBatch* p_batch, maybe_result_t maybe_result)
{
...
void RecordSubmitter::flush_current_batch()
{
    ...
    auto [to_write, sizes] = p_batch->encode_batch(get_committed_to(), journal_allocator.get_nonce());
    ...
    std::ignore = journal_allocator.write(std::move(to_write))
                      .safe_then([this, p_batch, FNAME, num, sizes = sizes](auto write_result) {
                          TRACE("{} {} records, {}, write done with {}", get_name(), num, sizes, write_result);
                          finish_submit_batch(p_batch, write_result);
                      })
                      .handle_error(crimson::ct_error::all_same_way([this, p_batch, FNAME, num, sizes = sizes](auto e) {
                      ...
                      .handle_exception([this, p_batch, FNAME, num, sizes = sizes](auto e) {
                          ERROR("{} {} records, {}, got exception {}", get_name(), num, sizes, e);
                          finish_submit_batch(p_batch, std::nullopt);
                      });
}
