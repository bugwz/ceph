...
struct segment_info_t
{
    ...
    {
        return type == segment_type_t::JOURNAL && tail_committed.segment_seq <= seq;
    }

    bool is_empty() const { return state == Segment::segment_state_t::EMPTY; }

    bool is_closed() const { return state == Segment::segment_state_t::CLOSED; }

    bool is_open() const { return state == Segment::segment_state_t::OPEN; }

    void init_closed(segment_seq_t, segment_type_t, data_category_t, rewrite_gen_t, segment_off_t);

    void set_open(segment_seq_t, segment_type_t, data_category_t, rewrite_gen_t);
    ...
    void update_modify_time(sea_time_point _modify_time, std::size_t _num_extents)
    {
        ...
        if (modify_time == NULL_TIME) {
            ...
            modify_time = get_average_time(modify_time, num_extents, _modify_time, _num_extents);
            num_extents += _num_extents;
        }
    }
};
...
{
public:
    segments_info_t() { reset(); }

    const segment_info_t& operator[](segment_id_t id) const { return segments[id]; }
    ...
    std::size_t get_num_in_journal_open() const { return num_in_journal_open; }
    std::size_t get_num_type_journal() const { return num_type_journal; }
    std::size_t get_num_type_ool() const { return num_type_ool; }
    std::size_t get_num_open() const { return num_open; }
    std::size_t get_num_empty() const { return num_empty; }
    std::size_t get_num_closed() const { return num_closed; }
    std::size_t get_count_open_journal() const { return count_open_journal; }
    std::size_t get_count_open_ool() const { return count_open_ool; }
    std::size_t get_count_release_journal() const { return count_release_journal; }
    std::size_t get_count_release_ool() const { return count_release_ool; }
    std::size_t get_count_close_journal() const { return count_close_journal; }
    std::size_t get_count_close_ool() const { return count_close_ool; }

    std::size_t get_total_bytes() const { return total_bytes; }
    /// the available space that is writable, including in open segments
    std::size_t get_available_bytes() const { return num_empty * get_segment_size() + avail_bytes_in_open; }
    ...
    std::size_t get_available_bytes_in_open() const { return avail_bytes_in_open; }
    double get_available_ratio() const { return (double)get_available_bytes() / (double)total_bytes; }
    ...
    journal_seq_t get_submitted_journal_head() const
    {
        ...
        return journal_seq_t{segment_info.seq, paddr_t::make_seg_paddr(journal_segment_id, segment_info.written_to)};
    }
    ...
    void init_closed(segment_id_t, segment_seq_t, segment_type_t, data_category_t, rewrite_gen_t);

    void mark_open(segment_id_t, segment_seq_t, segment_type_t, data_category_t, rewrite_gen_t);
    ...
    void update_modify_time(segment_id_t id, sea_time_point tp, std::size_t num)
    {
        if (num == 0) {
            return;
        }
...
public:
    ...
    virtual TransactionRef create_transaction(Transaction::src_t, const char* name, bool is_weak = false) = 0;

    /// Creates empty transaction with interruptible context
    template<typename Func> auto with_transaction_intr(Transaction::src_t src, const char* name, Func&& f)
    {
        return do_with_transaction_intr<Func, false>(src, name, std::forward<Func>(f));
    }

    template<typename Func> auto with_transaction_weak(const char* name, Func&& f)
    {
        return do_with_transaction_intr<Func, true>(Transaction::src_t::READ, name, std::forward<Func>(f))
            .handle_error(crimson::ct_error::eagain::handle([] { ceph_assert(0 == "eagain impossible"); }),
                          crimson::ct_error::pass_further_all{});
    }

    /// See Cache::get_next_dirty_extents
    using get_next_dirty_extents_iertr = base_iertr;
    using get_next_dirty_extents_ret = get_next_dirty_extents_iertr::future<std::vector<CachedExtentRef>>;
    ...
    virtual rewrite_extent_ret rewrite_extent(Transaction& t, CachedExtentRef extent, rewrite_gen_t target_generation,
    ...
    using get_extents_if_live_ret = get_extents_if_live_iertr::future<std::list<CachedExtentRef>>;
    virtual get_extents_if_live_ret get_extents_if_live(Transaction& t, extent_types_t type, paddr_t addr,
                                                        laddr_t laddr, extent_len_t len) = 0;
    ...
    using submit_transaction_direct_ret = submit_transaction_direct_iertr::future<>;
    virtual submit_transaction_direct_ret submit_transaction_direct(
        Transaction& t, std::optional<journal_seq_t> seq_to_trim = std::nullopt) = 0;

private:
    template<typename Func, bool IsWeak>
    auto do_with_transaction_intr(Transaction::src_t src, const char* name, Func&& f)
    {
        return seastar::do_with(
            create_transaction(src, name, IsWeak), [f = std::forward<Func>(f)](auto& ref_t) mutable {
                return with_trans_intr(*ref_t, [f = std::forward<Func>(f)](auto& t) mutable { return f(t); });
            });
    }
};
...
struct BackgroundListener
{
    ...
    bool is_ready() const { return get_state() >= state_t::RUNNING; }
};
...
public:
    ...
    virtual void update_journal_tails(journal_seq_t dirty_tail, journal_seq_t alloc_tail) = 0;
    ...
    journal_seq_t get_journal_tail() const { return std::min(get_alloc_tail(), get_dirty_tail()); }

    virtual std::size_t get_trim_size_per_cycle() const = 0;

    bool check_is_ready() const
    {
        return (get_journal_head() != JOURNAL_SEQ_NULL && get_dirty_tail() != JOURNAL_SEQ_NULL &&
    ...
    std::size_t get_num_rolls() const
    {
        ...
        assert(get_journal_head().segment_seq >= get_journal_tail().segment_seq);
        return get_journal_head().segment_seq + 1 - get_journal_tail().segment_seq;
    }
};
...
public:
    struct config_t
    {
        ...
        static config_t get_default(std::size_t roll_size, journal_type_t type);

        static config_t get_test(std::size_t roll_size, journal_type_t type);
    };

    JournalTrimmerImpl(BackrefManager& backref_manager, config_t config, journal_type_t type, device_off_t roll_start,
    ...
    journal_seq_t get_journal_head() const final { return journal_head; }

    void set_journal_head(journal_seq_t) final;

    journal_seq_t get_dirty_tail() const final { return journal_dirty_tail; }

    journal_seq_t get_alloc_tail() const final { return journal_alloc_tail; }

    void update_journal_tails(journal_seq_t dirty_tail, journal_seq_t alloc_tail) final;

    std::size_t get_trim_size_per_cycle() const final
    {
        return config.rewrite_backref_bytes_per_cycle + config.rewrite_dirty_bytes_per_cycle;
    }

    journal_type_t get_journal_type() const { return journal_type; }

    void set_extent_callback(ExtentCallbackInterface* cb) { extent_callback = cb; }

    void set_background_callback(BackgroundListener* cb) { background_callback = cb; }
    ...
    bool should_trim() const { return should_trim_alloc() || should_trim_dirty(); }

    bool should_block_io_on_trim() const
    {
        return get_tail_limit() > get_journal_tail().add_offset(journal_type, reserved_usage, roll_start, roll_size);
    }
    ...
    static JournalTrimmerImplRef create(BackrefManager& backref_manager, config_t config, journal_type_t type,
                                        device_off_t roll_start, device_off_t roll_size)
    {
        return std::make_unique<JournalTrimmerImpl>(backref_manager, config, type, roll_start, roll_size);
    }
...
private:
    ...
    bool should_trim_alloc() const { return get_alloc_tail_target() > journal_alloc_tail; }

    using trim_ertr = crimson::errorator<crimson::ct_error::input_output_error>;
...
std::ostream& operator<<(std::ostream&, const JournalTrimmerImpl::stat_printer_t&);
...
public:
    ...
    virtual segment_id_t allocate_segment(segment_seq_t, segment_type_t, data_category_t, rewrite_gen_t) = 0;
    ...
    virtual void update_modify_time(segment_id_t, sea_time_point, std::size_t) = 0;
...
{
public:
    virtual int64_t allocate(segment_id_t segment, segment_off_t offset, extent_len_t len) = 0;

    virtual int64_t release(segment_id_t segment, segment_off_t offset, extent_len_t len) = 0;

    virtual int64_t get_usage(segment_id_t segment) const = 0;
...
public:
    ...
    {
        for (auto sm : sms) {
            live_bytes_by_segment.add_device(sm->get_device_id(), sm->get_num_segments(), {0, sm->get_segment_size()});
        }
    }

    int64_t allocate(segment_id_t segment, segment_off_t offset, extent_len_t len) final
    {
        return update_usage(segment, len);
    }

    int64_t release(segment_id_t segment, segment_off_t offset, extent_len_t len) final
    {
        return update_usage(segment, -(int64_t)len);
    }

    int64_t get_usage(segment_id_t segment) const final { return live_bytes_by_segment[segment].live_bytes; }
...
class SpaceTrackerDetailed : public SpaceTrackerI
{
    ...
    public:
        SegmentMap(size_t blocks, segment_off_t total_bytes)
        ...
        int64_t allocate(device_segment_id_t segment, segment_off_t offset, extent_len_t len,
                         const extent_len_t block_size);

        int64_t release(device_segment_id_t segment, segment_off_t offset, extent_len_t len,
                        const extent_len_t block_size);

        int64_t get_usage() const { return used; }

        void dump_usage(extent_len_t block_size) const;

        double calc_utilization() const { return (double)used / (double)total_bytes; }
...
public:
    ...
    SpaceTrackerDetailed(const std::vector<SegmentManager*>& sms)
    {
        ...
        for (auto sm : sms) {
            segment_usage.add_device(sm->get_device_id(),
                                     sm->get_num_segments(),
                                     SegmentMap(sm->get_segment_size() / sm->get_block_size(), sm->get_segment_size()));
    ...
    int64_t allocate(segment_id_t segment, segment_off_t offset, extent_len_t len) final
    {
        return segment_usage[segment].allocate(
            segment.device_segment_id(), offset, len, block_size_by_segment_manager[segment.device_id()]);
    }
    ...
    int64_t get_usage(segment_id_t segment) const final { return segment_usage[segment].get_usage(); }

    double calc_utilization(segment_id_t segment) const final { return segment_usage[segment].calc_utilization(); }
...
template<typename T> class block_map_t
{
public:
    block_map_t()
    {
    ...
    void add_device(device_id_t device, std::size_t blocks, const T& init, size_t block_size)
    {
    ...
    auto begin() { return iterator<false>::lower_bound(*this, 0, 0); }
    auto begin() const { return iterator<true>::lower_bound(*this, 0, 0); }

    auto end() { return iterator<false>::end_iterator(*this); }
    auto end() const { return iterator<true>::end_iterator(*this); }

    size_t size() const { return total_blocks; }

    uint64_t get_block_size(device_id_t device_id) { return device_block_size[device_id]; }
    ...
    template<bool is_const = false> class iterator
    {
        /// points at set being iterated over
        std::conditional_t<is_const, const block_map_t&, block_map_t&> parent;
        ...
        std::optional<std::pair<const device_off_t, std::conditional_t<is_const, const T&, T&>>> current;

        bool is_end() const { return device_id == DEVICE_ID_MAX_VALID; }

        uint32_t get_block_id() { return parent.get_block_id(device_id, blk_off); }

        void find_valid()
        {
            assert(!is_end());
            auto& device_vec = parent.device_to_blocks[device_id];
            if (device_vec.size() == 0 || get_block_id() == device_vec.size()) {
                while (++device_id < DEVICE_ID_MAX_VALID && parent.device_to_blocks[device_id].size() == 0)
                    ;
                blk_off = 0;
            }
            if (is_end()) {
                ...
                current.emplace(blk_off, parent.device_to_blocks[device_id][get_block_id()]);
            }
        }

        iterator(decltype(parent)& parent, device_id_t device_id, device_off_t device_block_off)
            : parent(parent)
            , device_id(device_id)
    ...
    public:
        static iterator lower_bound(decltype(parent)& parent, device_id_t device_id, device_off_t block_off)
        {
        ...
        static iterator end_iterator(decltype(parent)& parent) { return iterator{parent, DEVICE_ID_MAX_VALID, 0}; }
        ...
        bool operator==(iterator<is_const> rit) { return (device_id == rit.device_id && blk_off == rit.blk_off); }

        bool operator!=(iterator<is_const> rit) { return !(*this == rit); }
        ...
        template<bool c = is_const, std::enable_if_t<!c, int> = 0> std::pair<const device_off_t, T&>* operator->()
        {
            assert(!is_end());
            return &*current;
        }
...
public:
    ...
    {
        for (auto rbm : rbms) {
            block_usage.add_device(rbm->get_device_id(),
    ...
    void allocate(paddr_t addr, extent_len_t len)
    {
        paddr_t cursor = addr;
        paddr_t end = addr.add_offset(len);
        do {
            block_usage[cursor].allocate();
            cursor = cursor.add_offset(block_usage.get_block_size(addr.get_device_id()));
        } while (cursor < end);
    }

    void release(paddr_t addr, extent_len_t len)
    {
        paddr_t cursor = addr;
        paddr_t end = addr.add_offset(len);
        do {
...
std::ostream& operator<<(std::ostream&, const AsyncCleaner::stat_printer_t&);
...
public:
    ...
    SegmentCleaner(config_t config, SegmentManagerGroupRef&& sm_group, BackrefManager& backref_manager,
                   SegmentSeqAllocator& segment_seq_allocator, bool detailed, bool is_cold);

    void set_journal_trimmer(JournalTrimmer& _trimmer) { trimmer = &_trimmer; }

    static SegmentCleanerRef create(config_t config, SegmentManagerGroupRef&& sm_group, BackrefManager& backref_manager,
                                    SegmentSeqAllocator& ool_seq_allocator, bool detailed, bool is_cold = false)
    {
        return std::make_unique<SegmentCleaner>(
            config, std::move(sm_group), backref_manager, ool_seq_allocator, detailed, is_cold);
    }
    ...
    const segment_info_t& get_seg_info(segment_id_t id) const final { return segments[id]; }

    segment_id_t allocate_segment(segment_seq_t, segment_type_t, data_category_t, rewrite_gen_t) final;

    void close_segment(segment_id_t segment) final;

    void update_segment_avail_bytes(segment_type_t type, paddr_t offset) final
    {
        assert(type == segment_type_t::OOL || trimmer != nullptr);   // segment_type_t::JOURNAL
        segments.update_written_to(type, offset);
        background_callback->maybe_wake_background();
    }

    void update_modify_time(segment_id_t id, sea_time_point tp, std::size_t num_extents) final
    {
        ceph_assert(num_extents == 0 || tp != NULL_TIME);
        segments.update_modify_time(id, tp, num_extents);
    }

    SegmentManagerGroup* get_segment_manager_group() final { return sm_group.get(); }

    /*
     * AsyncCleaner interfaces
     */

    void set_background_callback(BackgroundListener* cb) final { background_callback = cb; }

    void set_extent_callback(ExtentCallbackInterface* cb) final { extent_callback = cb; }
    ...
    bool check_usage_is_empty() const final { return space_tracker->equals(*space_tracker->make_empty()); }
    ...
    void commit_space_used(paddr_t addr, extent_len_t len) final { mark_space_used(addr, len); }
    ...
    bool should_clean_space() const final
    {
        ...
        return ((aratio < config.available_ratio_hard_limit) ||
                ((aratio < config.available_ratio_gc_max) && (rratio > config.reclaim_ratio_gc_threshold)));
    }

    clean_space_ret clean_space() final;

    const std::set<device_id_t>& get_device_ids() const final { return sm_group->get_device_ids(); }

    std::size_t get_reclaim_size_per_cycle() const final { return config.reclaim_bytes_per_cycle; }
...
private:
    ...
    double calc_gc_benefit_cost(segment_id_t id, const sea_time_point& now_time,
    ...
    struct reclaim_state_t
    {
        ...
        static reclaim_state_t create(segment_id_t segment_id, rewrite_gen_t generation, segment_off_t segment_size)
        {
            ...
            return {generation, target_gen, segment_size, P_ADDR_NULL, paddr_t::make_seg_paddr(segment_id, 0)};
        }

        segment_id_t get_segment_id() const { return end_pos.as_seg_paddr().get_segment_id(); }

        bool is_complete() const { return end_pos.as_seg_paddr().get_segment_off() >= segment_size; }
    ...
    do_reclaim_space_ret do_reclaim_space(const std::vector<CachedExtentRef>& backref_extents,
                                          const backref_pin_list_t& pin_list, std::size_t& reclaimed,
                                          std::size_t& runs);
    ...
    double get_alive_ratio() const { return stats.used_bytes / (double)segments.get_total_bytes(); }
    ...
    {
        return (double)get_projected_available_bytes() / (double)segments.get_total_bytes();
    }

    using scan_extents_ertr = SegmentManagerGroup::scan_valid_records_ertr;
    using scan_extents_ret = scan_extents_ertr::future<>;
    scan_extents_ret scan_no_tail_segment(const segment_header_t& header, segment_id_t segment_id);
    ...
    void init_mark_segment_closed(segment_id_t segment, segment_seq_t seq, segment_type_t s_type,
                                  data_category_t category, rewrite_gen_t generation)
    {
        assert(background_callback->get_state() == state_t::MOUNT);
        ceph_assert(s_type == segment_type_t::OOL || trimmer != nullptr);   // segment_type_t::JOURNAL
...
{
public:
    RBMCleaner(RBMDeviceGroupRef&& rb_group, BackrefManager& backref_manager, bool detailed);

    static RBMCleanerRef create(RBMDeviceGroupRef&& rb_group, BackrefManager& backref_manager, bool detailed)
    {
        return std::make_unique<RBMCleaner>(std::move(rb_group), backref_manager, detailed);
    }

    RBMDeviceGroup* get_rb_group() { return rb_group.get(); }
    ...
    bool should_block_io_on_clean() const final { return false; }

    bool can_clean_space() const final { return false; }

    bool should_clean_space() const final { return false; }

    clean_space_ret clean_space() final;

    const std::set<device_id_t>& get_device_ids() const final { return rb_group->get_device_ids(); }

    std::size_t get_reclaim_size_per_cycle() const final { return 0; }
