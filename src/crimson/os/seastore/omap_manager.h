...
public:
    ...
    using omap_get_value_ret = omap_get_value_iertr::future<std::optional<bufferlist>>;
    virtual omap_get_value_ret omap_get_value(const omap_root_t& omap_root, Transaction& t, const std::string& key) = 0;
    ...
    virtual omap_set_key_ret omap_set_key(omap_root_t& omap_root, Transaction& t, const std::string& key,
                                          const ceph::bufferlist& value) = 0;

    using omap_set_keys_iertr = base_iertr;
    using omap_set_keys_ret = omap_set_keys_iertr::future<>;
    virtual omap_set_keys_ret omap_set_keys(omap_root_t& omap_root, Transaction& t,
    ...
    virtual omap_rm_key_ret omap_rm_key(omap_root_t& omap_root, Transaction& t, const std::string& key) = 0;
    ...
    struct omap_list_config_t
    {
        ...
        omap_list_config_t(size_t max_result_size, bool first_inclusive, bool last_inclusive)
        ...
        auto with_inclusive(bool first_inclusive, bool last_inclusive)
        {
        ...
        auto with_reduced_max(size_t reduced_by) const
        {
            ...
            return omap_list_config_t(max_result_size - reduced_by, first_inclusive, last_inclusive);
        }
    };
    using omap_list_iertr = base_iertr;
    using omap_list_bare_ret = std::tuple<bool, std::map<std::string, bufferlist, std::less<>>>;
    ...
    virtual omap_list_ret omap_list(const omap_root_t& omap_root, Transaction& t,
                                    const std::optional<std::string>& first, const std::optional<std::string>& last,
    ...
    virtual omap_rm_key_range_ret omap_rm_key_range(omap_root_t& omap_root, Transaction& t, const std::string& first,
                                                    const std::string& last, omap_list_config_t config) = 0;
