...
bool SpaceTrackerSimple::equals(const SpaceTrackerI& _other) const
{
    ...
    for (segment_id_t i = 0; i < live_bytes_by_segment.size(); ++i) {
        if (other.live_bytes_by_segment[i] != live_bytes_by_segment[i]) {
            all_match = false;
            logger().debug("{}: segment_id {} live bytes mismatch *this: {}, other: {}",
                           __func__,
                           i,
...
int64_t SpaceTrackerDetailed::SegmentMap::allocate(segment_id_t segment, segment_off_t offset, extent_len_t len,
                                                   const extent_len_t block_size)
{
    ...
    for (auto i = b; i < e; ++i) {
        if (bitmap[i]) {
            if (!error) {
                logger().error("SegmentMap::allocate found allocated in {}, {} ~ {}", segment, offset, len);
                error = true;
            }
            logger().debug("SegmentMap::allocate block {} allocated", i * block_size);
        }
        bitmap[i] = true;
    }
...
int64_t SpaceTrackerDetailed::SegmentMap::release(segment_id_t segment, segment_off_t offset, extent_len_t len,
                                                  const extent_len_t block_size)
{
    assert(offset % block_size == 0);
    assert(len % block_size == 0);
    ...
    for (auto i = b; i < e; ++i) {
        if (!bitmap[i]) {
            if (!error) {
                logger().error("SegmentMap::release found unallocated in {}, {} ~ {}", segment, offset, len);
                error = true;
            }
            logger().debug("SegmentMap::release block {} unallocated", i * block_size);
        }
        bitmap[i] = false;
    }
...
bool SpaceTrackerDetailed::equals(const SpaceTrackerI& _other) const
{
    ...
    for (segment_id_t i = 0; i < segment_usage.size(); ++i) {
        if (other.segment_usage[i].get_usage() != segment_usage[i].get_usage()) {
            all_match = false;
            logger().error("{}: segment_id {} live bytes mismatch *this: {}, other: {}",
                           __func__,
                           i,
...
SegmentCleaner::get_segment_ret SegmentCleaner::get_segment()
{
    for (size_t i = 0; i < segments.size(); ++i) {
        if (segments[i].is_empty()) {
            ...
            return get_segment_ret(get_segment_ertr::ready_future_marker{}, i);
        }
    }
    assert(0 == "out of space handling todo");
    return get_segment_ret(get_segment_ertr::ready_future_marker{}, 0);
}

void SegmentCleaner::update_journal_tail_target(journal_seq_t target)
{
    logger().debug("{}: {}", __func__, target);
...
{
    if (journal_tail_committed == journal_seq_t() || committed > journal_tail_committed) {
        logger().debug("{}: update journal_tail_committed {}", __func__, committed);
        journal_tail_committed = committed;
    }
    if (journal_tail_target == journal_seq_t() || committed > journal_tail_target) {
        logger().debug("{}: update journal_tail_target {}", __func__, committed);
...
SegmentCleaner::do_immediate_work_ret SegmentCleaner::do_immediate_work(Transaction& t)
{
    ...
    logger().debug("SegmentCleaner::do_immediate_work gc total {}, available {}, unavailable {}, used {}  "
                   "available_ratio {}, reclaim_ratio {}, bytes_to_gc_for_available {}, bytes_to_gc_for_reclaim {}",
                   get_total_bytes(),
                   get_available_bytes(),
    ...
    return dirty_fut.safe_then([=, &t] { return do_gc(t, get_immediate_bytes_to_gc()); })
        .handle_error(do_immediate_work_ertr::pass_further{}, crimson::ct_error::assert_all{});
}

SegmentCleaner::do_deferred_work_ret SegmentCleaner::do_deferred_work(Transaction& t)
{
    return do_deferred_work_ret(do_deferred_work_ertr::ready_future_marker{}, ceph::timespan());
}

SegmentCleaner::rewrite_dirty_ret SegmentCleaner::rewrite_dirty(Transaction& t, journal_seq_t limit)
{
    return ecb->get_next_dirty_extents(limit).then([=, &t](auto dirty_list) {
        if (dirty_list.empty()) {
        ...
        return seastar::do_with(std::move(dirty_list), [this, &t](auto& dirty_list) {
            return crimson::do_for_each(dirty_list, [this, &t](auto& e) {
                logger().debug("SegmentCleaner::do_immediate_work cleaning {}", *e);
...
SegmentCleaner::do_gc_ret SegmentCleaner::do_gc(Transaction& t, size_t bytes)
{
    if (bytes == 0) {
    ...
    if (!scan_cursor) {
        ...
        if (next == P_ADDR_NULL) {
            logger().debug("SegmentCleaner::do_gc: no segments to gc");
            return do_gc_ertr::now();
        }
        next.offset = 0;
        scan_cursor = std::make_unique<ExtentCallbackInterface::scan_extents_cursor>(next);
        logger().debug("SegmentCleaner::do_gc: starting gc on segment {}", scan_cursor->get_offset().segment);
    }

    return ecb->scan_extents(*scan_cursor, bytes).safe_then([=, &t](auto addrs) {
        return seastar::do_with(std::move(addrs), [=, &t](auto& addr_list) {
            return crimson::do_for_each(
                       addr_list,
                       [=, &t](auto& addr_pair) {
                           auto& [addr, info] = addr_pair;
                           logger().debug("SegmentCleaner::do_gc: checking addr {}", addr);
                           return ecb->get_extent_if_live(t, info.type, addr, info.addr, info.len)
                               .safe_then([addr = addr, &t, this](CachedExtentRef ext) {
                                   if (!ext) {
                                       logger().debug("SegmentCleaner::do_gc: addr {} dead, skipping", addr);
                                       return ExtentCallbackInterface::rewrite_extent_ertr::now();
                                   }
                                   else {
                                       logger().debug("SegmentCleaner::do_gc: addr {} alive, gc'ing {}", addr, *ext);
                                   }
