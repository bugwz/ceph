...
{
    auto insert(coll_t coll, unsigned bits) { return emplace(std::make_pair(denc_coll_t{coll}, bits)); }

    void update(coll_t coll, unsigned bits) { (*this)[denc_coll_t{coll}] = bits; }

    void remove(coll_t coll) { erase(denc_coll_t{coll}); }
};
...
public:
    ...
    void remove(coll_t coll) { buffer.push_back(delta_t{delta_t::op_t::REMOVE, denc_coll_t(coll), 0}); }
...
struct CollectionNode : LogicalCachedExtent
{
    ...
    delta_buffer_t* maybe_get_delta_buffer() { return is_mutation_pending() ? &delta_buffer : nullptr; }
