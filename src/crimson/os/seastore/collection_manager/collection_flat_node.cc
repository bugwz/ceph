...
CollectionNode::list_ret CollectionNode::list()
{
    ...
    return list_ret(interruptible::ready_future_marker{}, std::move(list_result));
}

CollectionNode::create_ret CollectionNode::create(coll_context_t cc, coll_t coll, unsigned bits)
{
    ...
    if (encoded_sizeof((base_coll_map_t&)decoded) > get_bptr().length()) {
        decoded.erase(iter);
        return create_ret(interruptible::ready_future_marker{}, create_result_t::OVERFLOW);
    }
    else {
        ...
        copy_to_node();
        return create_ret(interruptible::ready_future_marker{}, create_result_t::SUCCESS);
    }
}

CollectionNode::update_ret CollectionNode::update(coll_context_t cc, coll_t coll, unsigned bits)
{
    logger().debug("trans.{} CollectionNode:{} {} {}", cc.t.get_trans_id(), __func__, coll, bits);
    if (!is_mutable()) {
        auto mut = cc.tm.get_mutable_extent(cc.t, this)->cast<CollectionNode>();
...
CollectionNode::remove_ret CollectionNode::remove(coll_context_t cc, coll_t coll)
{
    logger().debug("trans.{} CollectionNode:{} {}", cc.t.get_trans_id(), __func__, coll);
    if (!is_mutable()) {
        auto mut = cc.tm.get_mutable_extent(cc.t, this)->cast<CollectionNode>();
