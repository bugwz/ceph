...
{
    template<typename FormatContext> auto format(z_op s, FormatContext& ctx)
    {
        std::string_view name = "Unknown";
        switch (s) {
            using enum z_op;
        case OPEN: name = "BLKOPENZONE"; break;
        case FINISH: name = "BLKFINISHZONE"; break;
        case CLOSE: name = "BLKCLOSEZONE"; break;
        case RESET: name = "BLKRESETZONE"; break;
        }
...
using open_device_ret = ZBDSegmentManager::access_ertr::future<std::pair<seastar::file, seastar::stat_data>>;
static open_device_ret open_device(const std::string& path, seastar::open_flags mode)
{
    ...
    return seastar::file_stat(path, seastar::follow_symlink::yes)
        .then([FNAME, mode, &path](auto stat) mutable {
            return seastar::open_file_dma(path, mode).then([=](auto file) {
                DEBUG("open of device {} successful, size {}", path, stat.size);
...
static zbd_sm_metadata_t make_metadata(uint64_t total_size, seastore_meta_t meta, const seastar::stat_data& data,
                                       size_t zone_size_sectors, size_t zone_capacity_sectors, size_t nr_cnv_zones,
                                       size_t num_zones)
{
    ...
    INFO("device size: {}, available size: {}, block size: {}, allocated size: {},"
    ...
    for (unsigned int i = 0; i < seastar::smp::count; i++) {
        ...
        shard_infos[i].first_segment_offset = zone_size * skipped_zones + i * segment_size * per_shard_segments;
        INFO("First segment offset for shard {} is: {}", i, shard_infos[i].first_segment_offset);
    }

    zbd_sm_metadata_t ret = zbd_sm_metadata_t{seastar::smp::count,
                                              segment_size,
                                              zone_capacity * zones_per_segment,
                                              zones_per_segment,
                                              zone_capacity,
...
struct ZoneReport
{
    ...
    ZoneReport(int nr_zones)
        : hdr((blk_zone_report*)malloc(sizeof(struct blk_zone_report) + nr_zones * sizeof(struct blk_zone)))
    {
        ;
    }
    ~ZoneReport() { free(hdr); }
    ZoneReport(const ZoneReport&) = delete;
    ZoneReport(ZoneReport&& rhs)
        : hdr(rhs.hdr)
    {
        rhs.hdr = nullptr;
    }
...
static seastar::future<size_t> get_blk_dev_size(seastar::file& device)
{
    return seastar::do_with((uint64_t)0, [&](auto& size_sects) {
        return device.ioctl(BLKGETSIZE, (void*)&size_sects).then([&](int ret) {
...
static seastar::future<> reset_device(seastar::file& device, uint64_t zone_size_sects, uint64_t nr_zones)
{
    return seastar::do_with(blk_zone_range{}, [&, nr_zones, zone_size_sects](auto& range) {
        range.sector = 0;
        range.nr_sectors = zone_size_sects * nr_zones;
        return device.ioctl(BLKRESETZONE, &range).then([&](int ret) { return seastar::now(); });
    });
}

static seastar::future<size_t> get_zone_capacity(seastar::file& device, uint32_t nr_zones)
{
    return seastar::do_with(ZoneReport(nr_zones), [&](auto& zr) {
        zr.hdr->sector = 0;
...
static seastar::future<size_t> get_nr_cnv_zones(seastar::file& device, uint32_t nr_zones)
{
    return seastar::do_with(ZoneReport(nr_zones), [&](auto& zr) {
        zr.hdr->sector = 0;
        zr.hdr->nr_zones = nr_zones;
        return device.ioctl(BLKREPORTZONE, zr.hdr).then([&, nr_zones](int ret) {
            size_t cnv_zones = 0;
            for (uint32_t i = 0; i < nr_zones; i++) {
                if (zr.hdr->zones[i].type == BLK_ZONE_TYPE_CONVENTIONAL) cnv_zones++;
            }
...
static write_ertr::future<> do_write(seastar::file& device, uint64_t offset, bufferptr& bptr)
{
...
static write_ertr::future<> do_writev(device_id_t device_id, seastar::file& device, uint64_t offset, bufferlist&& bl,
                                      size_t block_size)
{
    LOG_PREFIX(ZBDSegmentManager::do_writev);
    DEBUG("{} offset {} len {}", device_id_printer_t{device_id}, offset, bl.length());
    ...
    return seastar::do_with(
        bl.prepare_iovs(), std::move(bl), [&device, device_id, offset, FNAME](auto& iovs, auto& bl) {
            return write_ertr::parallel_for_each(iovs, [&device, device_id, offset, FNAME](auto& p) {
                auto off = offset + p.offset;
                auto len = p.length;
                auto& iov = p.iov;
                DEBUG("{} poffset={}~{} dma_write ...", device_id_printer_t{device_id}, off, len);
                return device.dma_write(off, std::move(iov))
                    .handle_exception([FNAME, device_id, off, len](auto e) -> write_ertr::future<size_t> {
                        ERROR(
                            "{} poffset={}~{} dma_write got error -- {}", device_id_printer_t{device_id}, off, len, e);
                        return crimson::ct_error::input_output_error::make();
                    })
                    .then([FNAME, device_id, off, len](size_t written) -> write_ertr::future<> {
                        ...
                        DEBUG("{} poffset={}~{} dma_write done", device_id_printer_t{device_id}, off, len);
                        return write_ertr::now();
                    });
            });
        });
...
static ZBDSegmentManager::access_ertr::future<> write_metadata(seastar::file& device, zbd_sm_metadata_t sb)
{
    assert(ceph::encoded_sizeof_bounded<zbd_sm_metadata_t>() < sb.block_size);
    return seastar::do_with(bufferptr(ceph::buffer::create_page_aligned(sb.block_size)), [=, &device](auto& bp) {
...
static read_ertr::future<> do_read(seastar::file& device, uint64_t offset, size_t len, bufferptr& bptr)
{
    ...
    return device.dma_read(offset, bptr.c_str(), len)
        .handle_exception([FNAME](auto e) -> read_ertr::future<size_t> {
...
static ZBDSegmentManager::access_ertr::future<zbd_sm_metadata_t> read_metadata(seastar::file& device,
                                                                               seastar::stat_data sd)
{
    assert(ceph::encoded_sizeof_bounded<zbd_sm_metadata_t>() < sd.block_size);
    return seastar::do_with(bufferptr(ceph::buffer::create_page_aligned(sd.block_size)), [=, &device](auto& bp) {
        return do_read(device, 0, bp.length(), bp).safe_then([=, &bp] {
            bufferlist bl;
            bl.push_back(bp);
            ...
            return ZBDSegmentManager::access_ertr::future<zbd_sm_metadata_t>(
                ZBDSegmentManager::access_ertr::ready_future_marker{}, ret);
        });
    });
}
...
ZBDSegmentManager::mount_ret ZBDSegmentManager::mount()
{
    return shard_devices.invoke_on_all([](auto& local_device) {
        return local_device.shard_mount().handle_error(
            crimson::ct_error::assert_all{"Invalid error in ZBDSegmentManager::mount"});
    });
}
...
ZBDSegmentManager::mkfs_ret ZBDSegmentManager::mkfs(device_config_t config)
{
    return shard_devices.local().primary_mkfs(config).safe_then([this] {
        return shard_devices.invoke_on_all([](auto& local_device) {
            return local_device.shard_mkfs().handle_error(
                crimson::ct_error::assert_all{"Invalid error in ZBDSegmentManager::mkfs"});
        });
    });
}

ZBDSegmentManager::mkfs_ret ZBDSegmentManager::primary_mkfs(device_config_t config)
{
    ...
    return seastar::do_with(
        ...
        size_t(),
        [=, this](
            auto& device, auto& stat, auto& sb, auto& zone_size_sects, auto& nr_zones, auto& size, auto& nr_cnv_zones) {
            return open_device(device_path, seastar::open_flags::rw)
                .safe_then([=, this, &device, &stat, &sb, &zone_size_sects, &nr_zones, &size, &nr_cnv_zones](auto p) {
                    ...
                    return device.ioctl(BLKGETNRZONES, (void*)&nr_zones)
                        ...
                        .then([&] { return get_blk_dev_size(device); })
                        .then([&](auto devsize) {
                            size = devsize;
                        ...
                        .then([&, FNAME, config](auto zone_capacity_sects) {
                            ...
                            DEBUG("zone_size in sectors {}, zone_capacity in sectors {}",
                                  ...
                                  zone_capacity_sects);
                            sb = make_metadata(
                                size, config.meta, stat, zone_size_sects, zone_capacity_sects, nr_cnv_zones, nr_zones);
                            metadata = sb;
                            stats.metadata_write.increment(ceph::encoded_sizeof_bounded<zbd_sm_metadata_t>());
...
struct blk_zone_range make_range(segment_id_t id, size_t segment_size, size_t first_segment_offset)
{
    return blk_zone_range{
        (id.device_segment_id() * (segment_size >> SECT_SHIFT) + (first_segment_offset >> SECT_SHIFT)),
        (segment_size >> SECT_SHIFT)};
}

using blk_zone_op_ertr = crimson::errorator<crimson::ct_error::input_output_error>;
using blk_zone_op_ret = blk_zone_op_ertr::future<>;
blk_zone_op_ret blk_zone_op(seastar::file& device, blk_zone_range& range, zone_op op)
{
    ...
    switch (op) {
        using enum zone_op;
    case OPEN: ioctl_op = BLKOPENZONE; break;
    case FINISH: ioctl_op = BLKFINISHZONE; break;
    case RESET: ioctl_op = BLKRESETZONE; break;
    case CLOSE: ioctl_op = BLKCLOSEZONE; break;
    default: ERROR("Invalid zone operation {}", op); ceph_assert(ioctl_op);
    }

    return device.ioctl(ioctl_op, &range).then_wrapped([=](auto f) -> blk_zone_op_ret {
        if (f.failed()) {
...
ZBDSegmentManager::open_ertr::future<SegmentRef> ZBDSegmentManager::open(segment_id_t id)
{
    LOG_PREFIX(ZBDSegmentManager::open);
    return seastar::do_with(blk_zone_range{},
                            [=, this](auto& range) {
                                range = make_range(id, metadata.segment_size, shard_info.first_segment_offset);
                                return blk_zone_op(device, range, zone_op::OPEN);
                            })
        .safe_then([=, this] {
            DEBUG("segment {}, open successful", id);
            return open_ertr::future<SegmentRef>(open_ertr::ready_future_marker{},
                                                 SegmentRef(new ZBDSegment(*this, id)));
        });
}

ZBDSegmentManager::release_ertr::future<> ZBDSegmentManager::release(segment_id_t id)
{
    ...
    return seastar::do_with(blk_zone_range{},
                            [=, this](auto& range) {
                                range = make_range(id, metadata.segment_size, shard_info.first_segment_offset);
                                return blk_zone_op(device, range, zone_op::RESET);
                            })
        .safe_then([=] {
...
SegmentManager::read_ertr::future<> ZBDSegmentManager::read(paddr_t addr, size_t len, ceph::bufferptr& out)
{
    LOG_PREFIX(ZBDSegmentManager::read);
    auto& seg_addr = addr.as_seg_paddr();
    if (seg_addr.get_segment_id().device_segment_id() >= get_num_segments()) {
        ERROR("invalid segment {}", seg_addr.get_segment_id().device_segment_id());
        return crimson::ct_error::invarg::make();
    }

    if (seg_addr.get_segment_off() + len > metadata.segment_capacity) {
        ERROR("invalid read offset {}, len {}", addr, len);
        return crimson::ct_error::invarg::make();
    }
    return do_read(device, get_offset(addr), len, out);
}

Segment::close_ertr::future<> ZBDSegmentManager::segment_close(segment_id_t id, segment_off_t write_pointer)
{
    LOG_PREFIX(ZBDSegmentManager::segment_close);
    return seastar::do_with(blk_zone_range{},
                            [=, this](auto& range) {
                                range = make_range(id, metadata.segment_size, shard_info.first_segment_offset);
                                return blk_zone_op(device, range, zone_op::FINISH);
                            })
        .safe_then([=] {
...
Segment::write_ertr::future<> ZBDSegmentManager::segment_write(paddr_t addr, ceph::bufferlist bl, bool ignore_check)
{
    ...
    return do_writev(get_device_id(), device, get_offset(addr), std::move(bl), metadata.block_size);
}
...
Segment::write_ertr::future<> ZBDSegment::write(segment_off_t offset, ceph::bufferlist bl)
{
...
Segment::write_ertr::future<> ZBDSegment::write_padding_bytes(size_t padding_bytes)
{
    LOG_PREFIX(ZBDSegment::write_padding_bytes);
    DEBUG("Writing {} padding bytes to segment {} at wp {}", padding_bytes, id, write_pointer);
...
Segment::write_ertr::future<> ZBDSegment::advance_wp(segment_off_t offset)
{
