...
template<> struct fmt::formatter<segment_state_t> : fmt::formatter<std::string_view>
{
    ...
    template<typename FormatContext> auto format(segment_state_t s, FormatContext& ctx)
    {
        std::string_view name = "unknown";
        switch (s) {
        case segment_state_t::EMPTY: name = "empty"; break;
        case segment_state_t::OPEN: name = "open"; break;
        case segment_state_t::CLOSED: name = "closed"; break;
        }
...
static write_ertr::future<> do_write(device_id_t device_id, seastar::file& device, uint64_t offset, bufferptr& bptr)
{
    ...
    return device.dma_write(offset, bptr.c_str(), len)
        .handle_exception([FNAME, device_id, offset, len](auto e) -> write_ertr::future<size_t> {
            ERROR("{} poffset={}~{} got error -- {}", device_id_printer_t{device_id}, offset, len, e);
            return crimson::ct_error::input_output_error::make();
        })
        .then([FNAME, device_id, offset, len](auto result) -> write_ertr::future<> {
            if (result != len) {
                ERROR(
                    "{} poffset={}~{} write len={} inconsistent", device_id_printer_t{device_id}, offset, len, result);
                return crimson::ct_error::input_output_error::make();
            }
                                      ...
                                      size_t block_size)
{
    ...
    TRACE("{} poffset={}~{}, {} buffers", device_id_printer_t{device_id}, offset, bl.length(), bl.get_num_buffers());
    ...
    return seastar::do_with(
        bl.prepare_iovs(), std::move(bl), [&device, device_id, offset, FNAME](auto& iovs, auto& bl) {
            return write_ertr::parallel_for_each(iovs, [&device, device_id, offset, FNAME](auto& p) mutable {
                auto off = offset + p.offset;
                auto len = p.length;
                auto& iov = p.iov;
                TRACE("{} poffset={}~{} dma_write ...", device_id_printer_t{device_id}, off, len);
                return device.dma_write(off, std::move(iov))
                    ...
                    .then([FNAME, device_id, off, len](size_t written) -> write_ertr::future<> {
                        ...
                        TRACE("{} poffset={}~{} dma_write done", device_id_printer_t{device_id}, off, len);
                        return write_ertr::now();
                    });
            });
        });
...
static read_ertr::future<> do_read(device_id_t device_id, seastar::file& device, uint64_t offset, size_t len,
                                   bufferptr& bptr)
{
    ...
    return device.dma_read(offset, bptr.c_str(), len)
        ...
        .then([FNAME, device_id, offset, len](auto result) -> read_ertr::future<> {
            if (result != len) {
                ERROR("{} poffset={}~{} read len={} inconsistent", device_id_printer_t{device_id}, offset, len, result);
                return crimson::ct_error::input_output_error::make();
            }
            TRACE("{} poffset={}~{} done", device_id_printer_t{device_id}, offset, len);
...
write_ertr::future<> SegmentStateTracker::write_out(device_id_t device_id, seastar::file& device, uint64_t offset)
{
    LOG_PREFIX(SegmentStateTracker::write_out);
    DEBUG("{} poffset={}~{}", device_id_printer_t{device_id}, offset, bptr.length());
    return do_write(device_id, device, offset, bptr);
}

write_ertr::future<> SegmentStateTracker::read_in(device_id_t device_id, seastar::file& device, uint64_t offset)
{
...
using std::vector;
static block_sm_superblock_t make_superblock(device_id_t device_id, device_config_t sm_config,
                                             const seastar::stat_data& data)
{
    ...
    for (unsigned int i = 0; i < seastar::smp::count; i++) {
        ...
        shard_infos[i].first_segment_offset =
            tracker_off + total_tracker_size + i * segments_per_shard * config_segment_size;
    }
    ...
    return block_sm_superblock_t{
        seastar::smp::count, config_segment_size, data.block_size, shard_infos, std::move(sm_config)};
}
...
static check_create_device_ret check_create_device(const std::string& path, size_t size)
{
    LOG_PREFIX(block_check_create_device);
    INFO("path={}, size={}", path, size);
    return seastar::open_file_dma(
               path, seastar::open_flags::exclusive | seastar::open_flags::rw | seastar::open_flags::create)
        .then([size, FNAME, &path](auto file) {
            return seastar::do_with(file, [size, FNAME, &path](auto& f) -> seastar::future<> {
                DEBUG("path={} created, truncating to {}", path, size);
                ceph_assert(f);
                return f.truncate(size).then([&f, size] { return f.allocate(0, size); }).finally([&f] {
                    return f.close();
                });
            });
        })
...
using open_device_ret = BlockSegmentManager::access_ertr::future<std::pair<seastar::file, seastar::stat_data> >;
static open_device_ret open_device(const std::string& path)
{
    ...
    return seastar::file_stat(path, seastar::follow_symlink::yes)
        .then([&path, FNAME](auto stat) mutable {
            return seastar::open_file_dma(path, seastar::open_flags::rw | seastar::open_flags::dsync)
                .then([stat, &path, FNAME](auto file) mutable {
                    return file.size().then([stat, file, &path, FNAME](auto size) mutable {
                        stat.size = size;
                        INFO("path={} successful, size={}, block_size={}", path, stat.size, stat.block_size);
...
static BlockSegmentManager::access_ertr::future<> write_superblock(device_id_t device_id, seastar::file& device,
...
static BlockSegmentManager::access_ertr::future<block_sm_superblock_t> read_superblock(seastar::file& device,
                                                                                       seastar::stat_data sd)
{
    ...
    return seastar::do_with(bufferptr(ceph::buffer::create_page_aligned(sd.block_size)), [=, &device](auto& bp) {
        return do_read(DEVICE_ID_NULL,   // unknown
                       device,
                       0,
                       bp.length(),
                       bp)
            .safe_then([=, &bp] {
                ...
                assert(ceph::encoded_sizeof<block_sm_superblock_t>(ret) < sd.block_size);
...
Segment::write_ertr::future<> BlockSegment::write(segment_off_t offset, ceph::bufferlist bl)
{
    LOG_PREFIX(BlockSegment::write);
    auto paddr = paddr_t::make_seg_paddr(id, offset);
    DEBUG("{} offset={}~{} poffset={} ...", id, offset, bl.length(), manager.get_offset(paddr));

    if (offset < write_pointer || offset % manager.superblock.block_size != 0 ||
        bl.length() % manager.superblock.block_size != 0) {
        ERROR("{} offset={}~{} poffset={} invalid write", id, offset, bl.length(), manager.get_offset(paddr));
...
Segment::write_ertr::future<> BlockSegment::advance_wp(segment_off_t offset)
{
    return write_ertr::now();
}

Segment::close_ertr::future<> BlockSegmentManager::segment_close(segment_id_t id, segment_off_t write_pointer)
{
...
Segment::write_ertr::future<> BlockSegmentManager::segment_write(paddr_t addr, ceph::bufferlist bl, bool ignore_check)
{
    assert(addr.get_device_id() == get_device_id());
    assert((bl.length() % superblock.block_size) == 0);
    stats.data_write.increment(bl.length());
    return do_writev(get_device_id(), device, get_offset(addr), std::move(bl), superblock.block_size);
}
...
BlockSegmentManager::mount_ret BlockSegmentManager::mount()
{
    return shard_devices.invoke_on_all([](auto& local_device) {
        return local_device.shard_mount().handle_error(
            crimson::ct_error::assert_all{"Invalid error in BlockSegmentManager::mount"});
    });
}
...
BlockSegmentManager::mount_ret BlockSegmentManager::shard_mount()
{
    ...
    return open_device(device_path)
        ...
        .safe_then([=, this](auto sb) {
            ...
            sb.validate();
            superblock = sb;
            stats.data_read.increment(ceph::encoded_sizeof<block_sm_superblock_t>(superblock));
            tracker = std::make_unique<SegmentStateTracker>(shard_info.segments, superblock.block_size);
            stats.data_read.increment(tracker->get_size());
            return tracker->read_in(get_device_id(), device, shard_info.tracker_offset).safe_then([this] {
...
BlockSegmentManager::mkfs_ret BlockSegmentManager::mkfs(device_config_t sm_config)
{
    return shard_devices.local().primary_mkfs(sm_config).safe_then([this] {
        return shard_devices.invoke_on_all([](auto& local_device) {
            return local_device.shard_mkfs().handle_error(
                crimson::ct_error::assert_all{"Invalid error in BlockSegmentManager::mkfs"});
        });
    });
}

BlockSegmentManager::mkfs_ret BlockSegmentManager::primary_mkfs(device_config_t sm_config)
{
    ...
    INFO("{} path={}, {}", device_id_printer_t{get_device_id()}, device_path, sm_config);
    return seastar::do_with(seastar::file{},
                            seastar::stat_data{},
                            ...
                            [=, this](auto& device, auto& stat, auto& sb, auto& tracker) {
                                ...
                                return maybe_create.safe_then([this] { return open_device(device_path); })
...
SegmentManager::open_ertr::future<SegmentRef> BlockSegmentManager::open(segment_id_t id)
{
    ...
    stats.metadata_write.increment(tracker->get_size());
    return tracker->write_out(get_device_id(), device, shard_info.tracker_offset).safe_then([this, id, FNAME] {
        ++stats.opened_segments;
        DEBUG("{} done", id);
        return open_ertr::future<SegmentRef>(open_ertr::ready_future_marker{}, SegmentRef(new BlockSegment(*this, id)));
    });
}

SegmentManager::release_ertr::future<> BlockSegmentManager::release(segment_id_t id)
{
...
SegmentManager::read_ertr::future<> BlockSegmentManager::read(paddr_t addr, size_t len, ceph::bufferptr& out)
{
    ...
    if (s_id >= get_num_segments()) {
        ERROR("{} offset={}~{} poffset={} segment-id out of range {}", id, s_off, len, p_off, get_num_segments());
        return crimson::ct_error::invarg::make();
    }

    if (s_off + len > superblock.segment_size) {
        ERROR("{} offset={}~{} poffset={} read out of range {}", id, s_off, len, p_off, superblock.segment_size);
        return crimson::ct_error::invarg::make();
    }
    ...
    if (tracker->get(s_id) == segment_state_t::EMPTY) {
        ...
        DEBUG("{} offset={}~{} poffset={} invalid state {}", id, s_off, len, p_off, tracker->get(s_id));
...
void BlockSegmentManager::register_metrics()
{
    ...
    metrics.add_group(
        ...
        {
            sm::make_counter(
                "data_read_num", stats.data_read.num, sm::description("total number of data read"), label_instances),
            sm::make_counter(
                "data_read_bytes", stats.data_read.bytes, sm::description("total bytes of data read"), label_instances),
            sm::make_counter(
                "data_write_num", stats.data_write.num, sm::description("total number of data write"), label_instances),
            sm::make_counter("data_write_bytes",
                             stats.data_write.bytes,
                             sm::description("total bytes of data write"),
                             label_instances),
            sm::make_counter("metadata_write_num",
                             stats.metadata_write.num,
                             sm::description("total number of metadata write"),
                             label_instances),
            sm::make_counter("metadata_write_bytes",
                             stats.metadata_write.bytes,
                             sm::description("total bytes of metadata write"),
                             label_instances),
            sm::make_counter(
                "opened_segments", stats.opened_segments, sm::description("total segments opened"), label_instances),
            sm::make_counter(
                "closed_segments", stats.closed_segments, sm::description("total segments closed"), label_instances),
            sm::make_counter("closed_segments_unused_bytes",
                             stats.closed_segments_unused_bytes,
                             sm::description("total unused bytes of closed segments"),
                             label_instances),
            sm::make_counter("released_segments",
                             stats.released_segments,
