...
class EphemeralSegmentManager final : public SegmentManager
{
    ...
    size_t get_offset(paddr_t addr)
    {
        ...
        return (seg_addr.get_segment_id().device_segment_id() * config.segment_size) + seg_addr.get_segment_off();
    }
...
public:
    ...
    close_ertr::future<> close() final { return close_ertr::now(); }
    ...
    mount_ret mount() final { return mount_ertr::now(); }
    ...
    size_t get_available_size() const final { return config.size; }
    extent_len_t get_block_size() const final { return config.block_size; }
    segment_off_t get_segment_size() const final { return config.segment_size; }
    ...
    magic_t get_magic() const final { return device_config->spec.magic; }

    using init_ertr = crimson::errorator<crimson::ct_error::enospc, crimson::ct_error::invarg>;
