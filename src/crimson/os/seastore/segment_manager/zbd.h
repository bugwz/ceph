...
{
public:
    ZBDSegment(ZBDSegmentManager& man, segment_id_t i)
        : manager(man)
        , id(i){};
...
public:
    seastar::future<> start() { return shard_devices.start(device_path); }
    ...
    ZBDSegmentManager(const std::string& path)
        : device_path(path)
    {}
...
public:
    ...
    device_type_t get_device_type() const final { return device_type_t::ZBD; }

    size_t get_available_size() const final { return shard_info.size; };

    extent_len_t get_block_size() const final { return metadata.block_size; };

    segment_off_t get_segment_size() const final { return metadata.segment_capacity; };

    const seastore_meta_t& get_meta() const { return metadata.meta; };
...
private:
    ...
    uint64_t get_offset(paddr_t addr)
    {
        ...
        return (shard_info.first_segment_offset +
                (seg_addr.get_segment_id().device_segment_id() * metadata.segment_size)) +
...
WRITE_CLASS_DENC_BOUNDED(crimson::os::seastore::segment_manager::zbd::zbd_shard_info_t)
WRITE_CLASS_DENC_BOUNDED(crimson::os::seastore::segment_manager::zbd::zbd_sm_metadata_t)
