...
public:
    SegmentSeqAllocator(segment_type_t type)
        : type(type)
    {}
    segment_seq_t get_and_inc_next_segment_seq() { return next_segment_seq++; }
...
private:
    void set_next_segment_seq(segment_seq_t seq)
    {
        ...
        assert(type == segment_type_t::JOURNAL ? seq >= next_segment_seq : true);
        if (seq > next_segment_seq) next_segment_seq = seq;
    }
...
using SegmentSeqAllocatorRef = std::unique_ptr<SegmentSeqAllocator>;
