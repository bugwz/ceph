...
template<unsigned LeastMaxValue = 64> class counting_semaphore
{
    using clock_t = std::chrono::system_clock;

public:
    explicit counting_semaphore(unsigned count) noexcept { sem_init(&sem, 0, count); }
    ...
    bool try_acquire_until(const std::chrono::time_point<Clock, Duration>& abs_time) noexcept
    {
        ...
        struct timespec ts = {static_cast<std::time_t>(s.time_since_epoch().count()), static_cast<long>(ns.count())};
        for (;;) {
    ...
    template<typename Rep, typename Period> bool try_acquire_for(const std::chrono::duration<Rep, Period>& rel_time)
    {
