...
public:
    OnCommit(int id, seastar::promise<>& done, seastar::alien::instance& alien, ceph::os::Transaction& txn)
        : cpuid(id)
        , alien(alien)
        , alien_done(done)
    {}
    ...
    {
        return seastar::alien::submit_to(alien,
                                         cpuid,
                                         [this] {
...
AlienStore::AlienStore(const std::string& type, const std::string& path, const ConfigValues& values)
    : type(type)
    , path{path}
    , values(values)
{}
...
seastar::future<> AlienStore::start()
{
    cct = std::make_unique<CephContext>(
        ...
        CephContext::create_options{CODE_ENVIRONMENT_UTILITY, 0, [](const ceph::logging::SubsystemMap* subsys_map) {
    ...
    auto cpu_cores = seastar::resource::parse_cpuset(get_conf<std::string>("crimson_alien_thread_cpu_cores"));
    ...
    const auto num_threads = get_conf<uint64_t>("crimson_alien_op_num_threads");
...
AlienStore::mount_ertr::future<> AlienStore::mount()
{
    ...
    assert(tp);
    return tp->submit([this] { return store->mount(); }).then([](const int r) -> mount_ertr::future<> {
        if (r != 0) {
            return crimson::stateful_ec{std::error_code(-r, std::generic_category())};
        }
        else {
...
seastar::future<> AlienStore::umount()
{
    ...
    return op_gate.close().then([this] { return tp->submit([this] { return store->umount(); }); }).then([](int r) {
        assert(r == 0);
        return seastar::now();
    });
}
...
AlienStore::mkfs_ertr::future<> AlienStore::mkfs(uuid_d osd_fsid)
{
    ...
    assert(tp);
    return tp->submit([this] { return store->mkfs(); }).then([](int r) -> mkfs_ertr::future<> {
        if (r != 0) {
            return crimson::stateful_ec{std::error_code(-r, std::generic_category())};
        }
        else {
...
seastar::future<std::tuple<std::vector<ghobject_t>, ghobject_t>> AlienStore::list_objects(CollectionRef ch,
                                                                                          ...
                                                                                          uint64_t limit) const
{
    logger().debug("{}", __func__);
    assert(tp);
    return do_with_op_gate(std::vector<ghobject_t>(), ghobject_t(), [=, this](auto& objects, auto& next) {
        objects.reserve(limit);
        return tp
            ->submit(ch->get_cid().hash_to_shard(tp->size()),
                     [=, this, &objects, &next] {
                         auto c = static_cast<AlienCollection*>(ch.get());
                         return store->collection_list(
                             c->collection, start, end, store->get_ideal_list_max(), &objects, &next);
                     })
            .then([&objects, &next](int r) {
                assert(r == 0);
                return seastar::make_ready_future<std::tuple<std::vector<ghobject_t>, ghobject_t>>(std::move(objects),
                                                                                                   std::move(next));
            });
    });
}
...
seastar::future<CollectionRef> AlienStore::create_new_collection(const coll_t& cid)
{
    logger().debug("{}", __func__);
    assert(tp);
    return tp->submit([this, cid] { return store->create_new_collection(cid); })
...
seastar::future<CollectionRef> AlienStore::open_collection(const coll_t& cid)
{
    logger().debug("{}", __func__);
    assert(tp);
    return tp->submit([this, cid] { return store->open_collection(cid); })
    ...
    return do_with_op_gate(std::vector<coll_t>{}, [this](auto& ls) {
        return tp->submit([this, &ls] { return store->list_collections(ls); })
            .then([&ls](int r) -> seastar::future<std::vector<coll_core_t>> {
                ...
                ret.resize(ls.size());
                std::transform(ls.begin(), ls.end(), ret.begin(), [](auto p) { return std::make_pair(p, NULL_CORE); });
                return seastar::make_ready_future<std::vector<coll_core_t>>(std::move(ret));
            });
    });
}

AlienStore::read_errorator::future<ceph::bufferlist> AlienStore::read(CollectionRef ch, const ghobject_t& oid,
...
AlienStore::read_errorator::future<ceph::bufferlist> AlienStore::readv(CollectionRef ch, const ghobject_t& oid,
                                                                       interval_set<uint64_t>& m, uint32_t op_flags)
{
    logger().debug("{}", __func__);
    assert(tp);
    return do_with_op_gate(ceph::bufferlist{}, [this, ch, oid, &m, op_flags](auto& bl) {
        return tp
            ->submit(ch->get_cid().hash_to_shard(tp->size()),
...
AlienStore::get_attr_errorator::future<ceph::bufferlist> AlienStore::get_attr(CollectionRef ch, const ghobject_t& oid,
                                                                              std::string_view name) const
{
    logger().debug("{}", __func__);
    assert(tp);
    return do_with_op_gate(ceph::bufferlist{}, std::string{name}, [=, this](auto& value, const auto& name) {
        return tp
            ->submit(ch->get_cid().hash_to_shard(tp->size()),
                     [=, this, &value, &name] {
            ...
            .then([oid, &value](int r) -> get_attr_errorator::future<ceph::bufferlist> {
                if (r == -ENOENT) {
                    ...
                    return get_attr_errorator::make_ready_future<ceph::bufferlist>(std::move(value));
                }
            });
    });
}

AlienStore::get_attrs_ertr::future<AlienStore::attrs_t> AlienStore::get_attrs(CollectionRef ch, const ghobject_t& oid)
{
    logger().debug("{}", __func__);
    assert(tp);
...
auto AlienStore::omap_get_values(CollectionRef ch, const ghobject_t& oid, const set<string>& keys)
            ...
            ->submit(ch->get_cid().hash_to_shard(tp->size()),
                     [=, this, &values] {
                         ...
                         return store->omap_get_values(
                             c->collection, oid, keys, reinterpret_cast<map<string, bufferlist>*>(&values));
                     })
...
auto AlienStore::omap_get_values(CollectionRef ch, const ghobject_t& oid, const std::optional<string>& start)
        ...
        return tp
            ->submit(ch->get_cid().hash_to_shard(tp->size()),
                     [=, this, &values] {
                         auto c = static_cast<AlienCollection*>(ch.get());
                         return store->omap_get_values(
                             c->collection, oid, start, reinterpret_cast<map<string, bufferlist>*>(&values));
                     })
            .then([&values](int r) -> read_errorator::future<std::tuple<bool, omap_values_t>> {
                if (r == -ENOENT) {
                    return crimson::ct_error::enoent::make();
                }
                else if (r < 0) {
                    ...
                    return read_errorator::make_ready_future<std::tuple<bool, omap_values_t>>(true, std::move(values));
                }
            });
    });
}

seastar::future<> AlienStore::do_transaction_no_callbacks(CollectionRef ch, ceph::os::Transaction&& txn)
{
    logger().debug("{}", __func__);
    auto id = seastar::this_shard_id();
    auto done = seastar::promise<>();
    return do_with_op_gate(std::move(txn), std::move(done), [this, ch, id](auto& txn, auto& done) {
...
seastar::future<> AlienStore::inject_data_error(const ghobject_t& o)
{
    ...
    return seastar::with_gate(op_gate,
                              [=, this] { return tp->submit([o, this] { return store->inject_data_error(o); }); });
}
...
seastar::future<> AlienStore::inject_mdata_error(const ghobject_t& o)
{
    ...
    assert(tp);
    return seastar::with_gate(op_gate,
                              [=, this] { return tp->submit([=, this] { return store->inject_mdata_error(o); }); });
}

seastar::future<> AlienStore::write_meta(const std::string& key, const std::string& value)
{
    logger().debug("{}", __func__);
    assert(tp);
    return seastar::with_gate(op_gate, [=, this] {
        return tp->submit([=, this] { return store->write_meta(key, value); }).then([](int r) {
            assert(r == 0);
            return seastar::make_ready_future<>();
        });
    });
...
seastar::future<std::tuple<int, std::string>> AlienStore::read_meta(const std::string& key)
{
    logger().debug("{}", __func__);
    assert(tp);
    return seastar::with_gate(op_gate, [this, key] {
        return tp
            ->submit([this, key] {
                ...
                if (r > 0) {
                    value.resize(r);
                    boost::algorithm::trim_right_if(value, [](unsigned char c) { return isspace(c); });
                }
                else {
                    value.clear();
                }
            ...
            .then(
                [](auto entry) { return seastar::make_ready_future<std::tuple<int, std::string>>(std::move(entry)); });
    });
}
...
seastar::future<store_statfs_t> AlienStore::stat() const
{
    ...
    return do_with_op_gate(store_statfs_t{}, [this](store_statfs_t& st) {
        return tp->submit([this, &st] { return store->statfs(&st, nullptr); }).then([&st](int r) {
            assert(r == 0);
...
seastar::future<struct stat> AlienStore::stat(CollectionRef ch, const ghobject_t& oid)
{
    assert(tp);
...
auto AlienStore::omap_get_header(CollectionRef ch, const ghobject_t& oid)
        ...
        return tp
            ...
            .then([&bl](int r) -> get_attr_errorator::future<ceph::bufferlist> {
                if (r == -ENOENT) {
                    ...
                    return get_attr_errorator::make_ready_future<ceph::bufferlist>(std::move(bl));
                }
            });
    });
}

AlienStore::read_errorator::future<std::map<uint64_t, uint64_t>> AlienStore::fiemap(CollectionRef ch,
                                                                                    ...
                                                                                    uint64_t len)
{
    ...
    return do_with_op_gate(std::map<uint64_t, uint64_t>(), [=, this](auto& destmap) {
        return tp
            ...
            .then([&destmap](int r) -> read_errorator::future<std::map<uint64_t, uint64_t>> {
                if (r == -ENOENT) {
                    return crimson::ct_error::enoent::make();
                }
                else {
                    return read_errorator::make_ready_future<std::map<uint64_t, uint64_t>>(std::move(destmap));
                }
            });
    });
}
