...
public:
    OnCommit(int id, seastar::promise<>& done, Context* oncommit, ceph::os::Transaction& txn)
        : cpuid(id)
        , oncommit(oncommit)
        , alien_done(done)
    {}
    ...
    {
        return seastar::alien::submit_to(cpuid,
                                         [this] {
...
seastar::future<> AlienStore::stop()
{
    return tp
        ->submit([this] {
            for (auto [cid, ch] : coll_map) static_cast<AlienCollection*>(ch.get())->collection.reset();
            store.reset();
        })
...
seastar::future<> AlienStore::mount()
{
    ...
    return tp->submit([this] { return store->mount(); }).then([](int r) {
        assert(r == 0);
        return seastar::now();
    });
}
...
seastar::future<> AlienStore::umount()
{
    ...
    return transaction_gate.close()
        .then([this] { return tp->submit([this] { return store->umount(); }); })
        .then([](int r) {
            assert(r == 0);
            return seastar::now();
        });
...
seastar::future<> AlienStore::mkfs(uuid_d osd_fsid)
{
    ...
    return tp->submit([this] { return store->mkfs(); }).then([](int r) {
        assert(r == 0);
        return seastar::now();
    });
}
...
{
    logger().debug("{}", __func__);
    return seastar::do_with(std::vector<ghobject_t>(), ghobject_t(), [=](auto& objects, auto& next) {
        objects.reserve(limit);
        return tp
            ->submit([=, &objects, &next] {
                auto c = static_cast<AlienCollection*>(ch.get());
                return store->collection_list(c->collection, start, end, store->get_ideal_list_max(), &objects, &next);
            })
...
seastar::future<std::vector<coll_t>> AlienStore::list_collections()
{
    ...
    return seastar::do_with(std::vector<coll_t>{}, [=](auto& ls) {
        return tp->submit([this, &ls] { return store->list_collections(ls); }).then([&ls](int r) {
            assert(r == 0);
                                                                       ...
                                                                       interval_set<uint64_t>& m, uint32_t op_flags)
{
    ...
    return seastar::do_with(ceph::bufferlist{}, [this, ch, oid, &m, op_flags](auto& bl) {
        return tp
            ->submit([this, ch, oid, &m, op_flags, &bl] {
                auto c = static_cast<AlienCollection*>(ch.get());
...
AlienStore::get_attr_errorator::future<ceph::bufferptr> AlienStore::get_attr(CollectionRef ch, const ghobject_t& oid,
                                                                             std::string_view name) const
{
    ...
    return seastar::do_with(ceph::bufferptr{}, [=](auto& value) {
        return tp
            ->submit([=, &value] {
                auto c = static_cast<AlienCollection*>(ch.get());
                return store->getattr(c->collection, oid, static_cast<std::string>(name).c_str(), value);
            })
...
AlienStore::get_attrs_ertr::future<AlienStore::attrs_t> AlienStore::get_attrs(CollectionRef ch, const ghobject_t& oid)
{
    ...
    return seastar::do_with(attrs_t{}, [=](auto& aset) {
        return tp
            ->submit([=, &aset] {
                auto c = static_cast<AlienCollection*>(ch.get());
                return store->getattrs(c->collection, oid, reinterpret_cast<map<string, bufferptr>&>(aset));
            })
...
seastar::future<> AlienStore::do_transaction(CollectionRef ch, ceph::os::Transaction&& txn)
{
    logger().debug("{}", __func__);
    auto id = seastar::this_shard_id();
    auto done = seastar::promise<>();
    return seastar::do_with(std::move(txn), std::move(done), [this, ch, id](auto& txn, auto& done) {
        return seastar::with_gate(transaction_gate, [this, ch, id, &txn, &done] {
            return tp_mutex.lock()
                .then([this, ch, id, &txn, &done] {
...
{
    logger().debug("{}", __func__);
    return tp->submit([=] { return store->write_meta(key, value); }).then([](int r) {
        assert(r == 0);
        return seastar::make_ready_future<>();
    });
}
...
seastar::future<std::tuple<int, std::string>> AlienStore::read_meta(const std::string& key)
{
    ...
    return tp
        ...
        .then([](auto entry) { return seastar::make_ready_future<std::tuple<int, std::string>>(std::move(entry)); });
}
...
auto AlienStore::omap_get_header(CollectionRef ch, const ghobject_t& oid) -> read_errorator::future<ceph::bufferlist>
{
    return seastar::do_with(ceph::bufferlist(), [=](auto& bl) {
        return tp
            ->submit([=, &bl] {
...
seastar::future<std::map<uint64_t, uint64_t>> AlienStore::fiemap(CollectionRef ch, const ghobject_t& oid, uint64_t off,
                                                                 uint64_t len)
{
...
seastar::future<FuturizedStore::OmapIteratorRef> AlienStore::get_omap_iterator(CollectionRef ch, const ghobject_t& oid)
{
    return tp->submit([=] {
        auto c = static_cast<AlienCollection*>(ch.get());
        auto iter = store->get_omap_iterator(c->collection, oid);
        return FuturizedStore::OmapIteratorRef(new AlienStore::AlienOmapIterator(iter, this));
    });
}
...
{
    return store->tp->submit([=] { return iter->seek_to_first(); }).then([](int r) {
        assert(r == 0);
        return seastar::now();
    });
}

seastar::future<> AlienStore::AlienOmapIterator::upper_bound(const std::string& after)
{
    return store->tp->submit([this, after] { return iter->upper_bound(after); }).then([](int r) {
        assert(r == 0);
        return seastar::now();
    });
}

seastar::future<> AlienStore::AlienOmapIterator::lower_bound(const std::string& to)
{
    return store->tp->submit([this, to] { return iter->lower_bound(to); }).then([](int r) {
        assert(r == 0);
        return seastar::now();
    });
}
...
{
    return store->tp->submit([this] { return iter->next(); }).then([](int r) {
        assert(r == 0);
        return seastar::now();
    });
}
...
{
    return store->tp->submit([this] { return iter->tail_key(); });
}
