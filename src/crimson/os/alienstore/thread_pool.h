...
template<typename Func> struct Task final : WorkItem
{
    using T = std::invoke_result_t<Func>;
    using future_stored_type_t = std::conditional_t<std::is_void_v<T>, seastar::internal::future_stored_type_t<>,
...
struct SubmitQueue
{
    ...
    {}
    seastar::future<> stop() { return pending_tasks.close(); }
};
...
public:
    ...
    seastar::future<> stop();
    size_t size() { return n_threads; }
    template<typename Func, typename... Args> auto submit(int shard, Func&& func, Args&&... args)
    {
        auto packaged = [func = std::move(func), args = std::forward_as_tuple(args...)] {
            return std::apply(std::move(func), std::move(args));
        };
        return seastar::with_gate(submit_queue.local().pending_tasks, [packaged = std::move(packaged), shard, this] {
            return local_free_slots().wait().then([packaged = std::move(packaged), shard, this] {
    ...
    template<typename Func> auto submit(Func&& func) { return submit(::rand() % n_threads, std::forward<Func>(func)); }
