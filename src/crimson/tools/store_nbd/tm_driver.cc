...
seastar::future<> TMDriver::write(off_t offset, bufferptr ptr)
{
    ...
    return seastar::do_with(ptr,
        ...
        .handle_error(crimson::ct_error::assert_all{"store-nbd write"});
}

TMDriver::read_extents_ret TMDriver::read_extents(Transaction& t, laddr_t offset, extent_len_t length)
{
    return seastar::do_with(
        lba_pin_list_t(), lextent_list_t<TestBlock>(), [this, &t, offset, length](auto& pins, auto& ret) {
            return tm->get_pins(t, offset, length).si_then([this, &t, &pins, &ret](auto _pins) {
                _pins.swap(pins);
                ...
                return trans_intr::do_for_each(
                           ...
                           pins.end(),
                           [this, &t, &ret](auto&& pin) {
                               logger().debug("read_extents: get_extent {}~{}", pin->get_val(), pin->get_length());
                               return tm->read_pin<TestBlock>(t, std::move(pin)).si_then([&ret](auto ref) mutable {
                                   ret.push_back(std::make_pair(ref->get_laddr(), ref));
                                   logger().debug("read_extents: got extent {}", *ref);
                                   return seastar::now();
                               });
                           })
...
seastar::future<bufferlist> TMDriver::read(off_t offset, size_t size)
{
    ...
    return repeat_eagain([=, &blret, this] {
               return tm->with_transaction_intr(Transaction::src_t::READ, "read", [=, &blret, this](auto& t) {
                   return read_extents(t, offset, size).si_then([=, &blret](auto ext_list) {
                       size_t cur = offset;
                       for (auto& i : ext_list) {
                           if (cur != i.first) {
                               assert(cur < i.first);
        ...
        .handle_error(crimson::ct_error::assert_all{"store-nbd read"})
...
seastar::future<> TMDriver::mkfs()
{
    ...
    return Device::make_device(*config.path, device_type_t::SSD)
        .then([this](DeviceRef dev) {
            ...
            return device->mkfs(
                device_config_t{true, (magic_t)std::rand(), device_type_t::SSD, 0, meta, secondary_device_set_t()});
        })
        .safe_then([this] {
        ...
        .handle_error(crimson::ct_error::assert_all{"Invalid errror during TMDriver::mkfs"});
}

seastar::future<> TMDriver::mount()
{
    return (config.mkfs ? mkfs() : seastar::now())
        .then([this] { return Device::make_device(*config.path, device_type_t::SSD); })
        .then([this](DeviceRef dev) {
            device = std::move(dev);
            return device->mount();
        })
        ...
        .handle_error(crimson::ct_error::assert_all{"Invalid errror during TMDriver::mount"});
};
...
seastar::future<> TMDriver::close()
{
    return tm->close()
        ...
        .handle_error(crimson::ct_error::assert_all{"Invalid errror during TMDriver::close"});
}
