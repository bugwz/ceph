...
struct request_context_t
{
    ...
    static ref make_ref() { return std::make_unique<request_context_t>(); }

    bool check_magic() const
    {
        auto ret = magic == NBD_REQUEST_MAGIC;
        if (!ret) {
            logger().error("Invalid magic {} should be {}", magic, NBD_REQUEST_MAGIC);
        }
        return ret;
    }
...
struct RequestWriter
{
    ...
    RequestWriter(seastar::output_stream<char>&& stream)
    ...
    seastar::future<> complete(request_context_t::ref&& req)
    {
        ...
        return lock.write_lock()
            .then([&request, this] { return request.write_reply(stream); })
    ...
    {
        return gate.close().then([this] { return stream.close(); });
    }
};
...
int main(int argc, char** argv)
{
    ...
    return app.run(av.size(), av.data(), [&] {
        ...
        return seastar::async([&] {
            ...
            crimson::common::sharded_conf().start(EntityName{}, std::string_view{"ceph"}).get();
            auto stop_conf = seastar::defer([] { crimson::common::sharded_conf().stop().get(); });

            auto backend = get_backend(backend_config);
            NBDHandler nbd(*backend, nbd_config);
            backend->mount().get();
            auto close_backend = seastar::defer([&] { backend->close().get(); });

            logger().debug("Running nbd server...");
            nbd.run();
            auto stop_nbd = seastar::defer([&] { nbd.stop().get(); });
...
seastar::future<> handle_command(BlockDriver& backend, request_context_t::ref request_ref, RequestWriter& out)
{
    auto& request = *request_ref;
    logger().debug("got command {}", request.get_command());
    return ([&] {
               switch (request.get_command()) {
               case NBD_CMD_WRITE: return backend.write(request.from, *request.in_buffer);
               case NBD_CMD_READ:
                   return backend.read(request.from, request.len).then([&](auto buffer) {
...
seastar::future<> handle_commands(BlockDriver& backend, seastar::input_stream<char>& in, RequestWriter& out)
{
    ...
    return seastar::keep_doing([&] {
               ...
               auto& request = *request_ref;
               return request.read_request(in).then([&, request_ref = std::move(request_ref)]() mutable {
...
void NBDHandler::run()
{
    ...
    server_socket = seastar::engine().listen(seastar::socket_address{seastar::unix_domain_addr{uds_path}});
