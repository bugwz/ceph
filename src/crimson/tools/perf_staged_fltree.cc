...
template<bool TRACK> class PerfTree : public TMTestState
{
public:
            ...
            .then([this, &kvs, erase_ratio] {
                return seastar::async([this, &kvs, erase_ratio] {
                    auto tree = std::make_unique<TreeBuilder<TRACK, ExtendedValue>>(
                        kvs,
                        (is_dummy ? NodeExtentManager::create_dummy(true) : NodeExtentManager::create_seastore(*tm)));
                    {
                        auto t = create_mutate_transaction();
                        with_trans_intr(*t, [&](auto& tr) { return tree->bootstrap(tr); }).unsafe_get();
                        submit_transaction(std::move(t));
                    }
                    {
                        auto t = create_mutate_transaction();
                        with_trans_intr(*t, [&](auto& tr) { return tree->insert(tr); }).unsafe_get();
                    ...
                    }
                    {
                        ...
                        with_trans_intr(*t, [&](auto& tr) { return tree->get_stats(tr); }).unsafe_get();

                        with_trans_intr(*t, [&](auto& tr) { return tree->validate(tr); }).unsafe_get();
                    }
                    {
                        auto t = create_mutate_transaction();
...
template<bool TRACK> seastar::future<> run(const bpo::variables_map& config)
{
    return seastar::async([&config] {
        ...
        seastar::engine().at_exit([] { return sharded_conf().stop(); });

        using crimson::common::sharded_perf_coll;
        sharded_perf_coll().start().get();
        seastar::engine().at_exit([] { return sharded_perf_coll().stop(); });

        auto kvs = KVPool<test_item_t>::create_raw_range(
            ns_sizes, oid_sizes, onode_sizes, {range2[0], range2[1]}, {range1[0], range1[1]}, {range0[0], range0[1]});
...
int main(int argc, char** argv)
{
    ...
    app.add_options()("backend", bpo::value<std::string>()->default_value("dummy"), "tree backend: dummy, seastore")(
        "tracked", bpo::value<bool>()->default_value(false), "track inserted cursors")(
        "ns-sizes",
        bpo::value<std::vector<size_t>>()->default_value({8, 11, 64, 128, 255, 256}),
        "sizes of ns strings")("oid-sizes",
                               bpo::value<std::vector<size_t>>()->default_value({8, 13, 64, 512, 2035, 2048}),
                               "sizes of oid strings")(
        "onode-sizes",
        bpo::value<std::vector<size_t>>()->default_value({8, 16, 128, 576, 992, 1200}),
        "sizes of onode")(
        "range2", bpo::value<std::vector<int>>()->default_value({0, 128}), "range of shard-pool-crush [a, b)")(
        "range1", bpo::value<std::vector<unsigned>>()->default_value({0, 10}), "range of ns-oid strings [a, b)")(
        "range0", bpo::value<std::vector<unsigned>>()->default_value({0, 4}), "range of snap-gen [a, b)")(
        "erase-ratio", bpo::value<double>()->default_value(0.8), "erase-ratio of all the inserted onodes");
