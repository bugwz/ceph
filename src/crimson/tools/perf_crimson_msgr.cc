                                   ...
                                   [=] {
                                       ...
                                       return sharded_obj->start(args...).then([sharded_obj]() {
                                           seastar::engine().at_exit(
                                               [sharded_obj]() { return sharded_obj->stop().then([sharded_obj] {}); });
...
struct client_config
{
    ...
    std::string str() const
    {
        ...
        out << "client[>> " << server_addr << "](bs=" << block_size << ", ramptime=" << ramptime
            << ", msgtime=" << msgtime << ", num_clients=" << num_clients << ", num_conns=" << num_conns
            << ", depth=" << depth << ", skip_core_0=" << skip_core_0 << ")";
        return out.str();
    }
...
struct server_config
{
    ...
    {
        std::ostringstream out;
        out << "server[" << addr << "](bs=" << block_size << ", is_fixed_cpu=" << is_fixed_cpu << ", core=" << core
            << ")";
        return out.str();
    }
...
static seastar::future<> run(perf_mode_t mode, const client_config& client_conf, const server_config& server_conf,
                             bool crc_enabled)
{
    struct test_state
    {
        struct Server final : public crimson::net::Dispatcher, public seastar::peering_sharded_service<Server>
        {
                ...
                , msg_len{msg_len}
            {
                ...
                if (seastar::this_shard_id() == msgr_sid && needs_report) {
                    start_report();
                }
            }

            void ms_handle_connect(crimson::net::ConnectionRef, seastar::shard_id) override
            {
                ceph_abort("impossible, server won't connect");
            }

            void ms_handle_accept(crimson::net::ConnectionRef, seastar::shard_id new_shard, bool is_replace) override
            {
            ...
            void ms_handle_reset(crimson::net::ConnectionRef, bool) override
            {
                auto& server = container().local();
                --server.conn_count;
            }
            ...
            std::optional<seastar::future<>> ms_dispatch(crimson::net::ConnectionRef c, MessageRef m) override
            {
                ...
                const static hobject_t hobj(object_t(), oloc.key, CEPH_NOSNAP, pgid.ps(), pgid.pool(), oloc.nspace);
            ...
            {
                return container().invoke_on(msgr_sid, [addr, is_fixed_cpu](auto& server) {
                    // server msgr is always with nonce 0
                    server.msgr = crimson::net::Messenger::create(
                        entity_name_t::OSD(server.msgr_sid), server.lname, 0, is_fixed_cpu);
                    ...
                    return server.msgr->bind(entity_addrvec_t{addr})
                        .safe_then([&server] { return server.msgr->start({&server}); },
            ...
            seastar::future<> shutdown()
            {
                ...
                return container().invoke_on(msgr_sid, [](auto& server) {
                    ...
                    server.msgr->stop();
                    return server.msgr->shutdown().then([&server] {
        ...
        private:
            ...
            void start_report()
            {
                ...
                seastar::do_with(TimerReport(seastar::smp::count), [this](auto& report) {
                    return seastar::do_until(
                        [this] { return is_stopped; },
                        [&report, this] {
                            return seastar::sleep(2s).then([&report, this] {
                                report.elapsed += 2;
                                if (is_fixed_cpu) {
                                    return seastar::smp::submit_to(
                                        ...
                                        .then([&report, this] {
                                            ...
                                            sout << setfill(' ') << report.elapsed << "(" << std::setw(5) << duration
                                                 << ") " << std::setw(9) << iops << "IOPS " << std::setw(8)
                                                 << throughput_MB << "MiB/s " << ireport.reactor_utilization << "("
                                                 << ireport.conn_count << ")";
                                    ...
                                    return seastar::smp::invoke_on_all([&report, this] {
                                        ...
                                        .then([&report, this] {
                                            ...
                                            sout << setfill(' ') << report.elapsed << "(" << std::setw(5) << duration
                                                 ...
                                                 << throughput_MB << "MiB/s ";
                                            for (auto& i : report.reports) {
                                                sout << i.reactor_utilization << "(" << i.conn_count << ") ";
                                            }
                                            std::cout << sout.str() << std::endl;
                                        });
                                }
        ...
        struct Client final : public crimson::net::Dispatcher, public seastar::peering_sharded_service<Client>
        {
            ...
            struct ConnStats
            {
                ...
                void start_connecting() { connecting_time = mono_clock::now(); }
            ...
            struct PeriodStats
            {
                ...
                void reset_period(unsigned received_count, unsigned _depth, PeriodStats& snapshot)
                {
            ...
            Client(unsigned num_clients, unsigned num_conns, unsigned msg_len, unsigned _depth, unsigned nonce_base,
            ...
            std::string get_name(unsigned i) { return fmt::format("client{}Conn{}@{}", id, i, sid); }
            ...
            std::optional<seastar::future<>> ms_dispatch(crimson::net::ConnectionRef conn, MessageRef m) override
            {
                assert(is_active());
                // server replies with MOSDOp to generate server-side write workload
                ceph_assert(m->get_type() == CEPH_MSG_OSD_OP);
                ...
                if (msg_id % SAMPLE_RATE == 0) {
                    ...
                    ceph_assert(conn_state.time_msgs_sent[msg_index] != lowres_clock_t::time_point::min());
                        ...
                        return seastar::do_for_each(
                            ...
                            [&client](auto i) {
                                ...
                                conn_state.msgr = crimson::net::Messenger::create(
                                    ...
                                    name,
                                    client.nonce_base + client.id * client.num_conns + i,
                                    true);
            ...
            seastar::future<> shutdown()
            {
                return seastar::do_with(std::vector<JobReport>(num_clients * num_conns), [this](auto& all_stats) {
                    return container()
                        .invoke_on_all([&all_stats](auto& client) {
                            if (!client.is_active()) {
                                return seastar::now();
                            }
                            ...
                            return seastar::parallel_for_each(boost::make_counting_iterator(0u),
                                                              ...
                                                              [&all_stats, &client](auto i) {
                                                                  ...
                                                                  auto& conn_state = client.conn_states[i];
                                                                  return conn_state.stop_dispatch_messages().then(
                                                                      [&all_stats, &client, i](auto stats) {
                                                                          all_stats[client.id * client.num_conns + i] =
                                                                              stats;
                                                                      });
                                                              })
                                .then([&client] {
                                    return seastar::do_for_each(boost::make_counting_iterator(0u),
                                                                boost::make_counting_iterator(client.num_conns),
                                                                [&client](auto i) {
                                                                    auto& conn_state = client.conn_states[i];
                ...
                return container().invoke_on_all([peer_addr](auto& client) {
                    ...
                    if (client.is_active()) {
                        for (unsigned i = 0; i < client.num_conns; ++i) {
                            ...
                            conn_state.active_conn->set_user_private(std::make_unique<ConnectionPriv>(i));
                        }
        ...
        private:
            ...
            public:
                ...
                PeriodStats& get_snap(unsigned client_id, unsigned i) { return snaps[client_id * num_conns + i]; }

                ConnStats& get_summary(unsigned client_id, unsigned i) { return summaries[client_id * num_conns + i]; }
                ...
                void set_server_reactor_utilization(double ru) { server_reactor_utilization = ru; }

                bool should_stop() const { return elapsed >= msgtime; }

                seastar::future<> ticktock()
                {
                    return seastar::sleep(1s).then([this] { ++elapsed; });
                }

                void report_header() const
                {
                    std::ostringstream sout;
                    sout << std::setfill(' ') << std::setw(6) << "sec" << std::setw(7) << "depth" << std::setw(10)
                         << "IOPS" << std::setw(9) << "MB/s" << std::setw(9) << "lat(ms)";
                    std::cout << sout.str() << std::endl;
                }
                ...
                void report_period()
                {
                    ...
                    std::ostringstream sout;
                    sout << setfill(' ') << std::setw(5) << elapsed_s << " " << std::setw(6) << depth << " "
                         << std::setw(9) << iops << " " << std::setw(8) << iops * bytes_of_block / 1048576 << " "
                ...
                void report_summary() const
                {
                    ...
                    sout << "--------------"
                         << " summary "
                         << "--------------\n"
                         << setfill(' ') << std::setw(7) << elapsed_s << std::setw(6) << "-" << std::setw(8) << iops
                         << std::setw(8) << iops * bytes_of_block / 1048576 << std::setw(8)
                         << (sampled_total_lat_s / sampled_count * 1000) << "\n";
                    std::cout << sout.str() << std::endl;
                }
            };
            ...
            seastar::future<> report_summary(TimerReport& report)
            {
                return container()
                    ...
                    .then([&report] { report.report_summary(); });
            }
        ...
        public:
            ...
            {
                logger().info(
                    "[all clients]: start sending MOSDOps from {} clients * {} conns", num_clients, num_conns);
                return container()
                    .invoke_on_all([](auto& client) {
                        if (client.is_active()) {
                            for (unsigned i = 0; i < client.num_conns; ++i) {
                    ...
                    .then([this, msgtime] {
                        ...
                        return seastar::do_with(
                            TimerReport(num_clients, num_conns, msgtime, msg_len), [this](auto& report) {
                                report.report_header();
                                return seastar::do_until([&report] { return report.should_stop(); },
                                                         [&report, this] {
        ...
        private:
            seastar::future<> send_msg(ConnState& conn_state)
            {
                ...
                return conn_state.depth.wait(1).then([this, &conn_state] {
                    const static pg_t pgid;
                    const static object_locator_t oloc;
                    const static hobject_t hobj(object_t(), oloc.key, CEPH_NOSNAP, pgid.ps(), pgid.pool(), oloc.nspace);
                    static spg_t spgid(pgid);
                    ...
                    if (unlikely(conn_state.sent_count % SAMPLE_RATE == 0)) {
                        ...
                        ceph_assert(conn_state.time_msgs_sent[index] == lowres_clock_t::time_point::min());
            ...
            void do_dispatch_messages(unsigned i)
            {
                ...
                (void)seastar::do_until([&conn_state] { return conn_state.stop_send; },
                    ...
                    .then([this, &conn_state, i] {
                        ...
                        logger().info("{} {}: stopped sending OSDOPs", name, *conn_state.active_conn);

                        std::chrono::duration<double> dur_conn =
                            conn_state.conn_stats.connected_time - conn_state.conn_stats.connecting_time;
                        std::chrono::duration<double> dur_msg = mono_clock::now() - conn_state.conn_stats.start_time;
                        unsigned ops = conn_state.conn_stats.received_count - conn_state.conn_stats.start_count;
                        ...
                        stats.latency_ms =
                            conn_state.conn_stats.sampled_total_lat_s / conn_state.conn_stats.sampled_count * 1000;
    ...
    return seastar::when_all(seastar::futurize_invoke([mode, server_conf, server_needs_report] {
                                 if (mode == perf_mode_t::client) {
                                     ...
                                     return create_sharded<test_state::Server>(
                                         server_conf.core, server_conf.block_size, server_needs_report);
                                 }
                             }),
                             seastar::futurize_invoke([mode, client_conf, server_sid] {
                                 if (mode == perf_mode_t::server) {
                                     ...
                                     return create_sharded<test_state::Client>(client_conf.num_clients,
                             ...
                             crimson::common::sharded_conf()
                                 ...
                                 .then([] { return crimson::common::local_conf().start(); })
                                 .then([crc_enabled] {
                                     return crimson::common::local_conf().set_val("ms_crc_data",
                                                                                  crc_enabled ? "true" : "false");
                                 }))
        .then([=](auto&& ret) {
            ...
            if (mode == perf_mode_t::both) {
                ...
                return seastar::when_all_succeed(
                           ...
                           client->init())
                    .then_unpack([client, addr = client_conf.server_addr] { return client->connect_wait_verify(addr); })
                    .then([client, ramptime = client_conf.ramptime, msgtime = client_conf.msgtime] {
                        return client->dispatch_with_timer(ramptime, msgtime);
                    })
                    .then([client] { return client->shutdown(); })
                    .then([server] { return server->shutdown(); });
            }
                ...
                ceph_assert(client_conf.num_clients > 0);
                return client->init()
                    .then([client, addr = client_conf.server_addr] { return client->connect_wait_verify(addr); })
                    .then([client, ramptime = client_conf.ramptime, msgtime = client_conf.msgtime] {
                        return client->dispatch_with_timer(ramptime, msgtime);
                    })
                    .then([client] { return client->shutdown(); });
        ...
        .finally([] { return crimson::common::sharded_conf().stop(); });
}
...
int main(int argc, char** argv)
{
    ...
    app.add_options()("mode", bpo::value<unsigned>()->default_value(0), "0: both, 1:client, 2:server")(
        "server-addr",
        bpo::value<std::string>()->default_value("v2:127.0.0.1:9010"),
        "server address(only support msgr v2 protocol)")(
        "ramptime", bpo::value<unsigned>()->default_value(5), "seconds of client ramp-up time")(
        "msgtime", bpo::value<unsigned>()->default_value(15), "seconds of client messaging time")(
        "clients", bpo::value<unsigned>()->default_value(1), "number of client messengers")(
        "conns-per-client", bpo::value<unsigned>()->default_value(1), "number of connections per client")(
        "client-bs", bpo::value<unsigned>()->default_value(4096), "client block size")(
        "depth", bpo::value<unsigned>()->default_value(512), "client io depth per job")(
        "client-skip-core-0", bpo::value<bool>()->default_value(true), "client skip core 0")(
        "server-fixed-cpu",
        bpo::value<bool>()->default_value(true),
        "server is in the fixed cpu mode, non-fixed doesn't support the mode both")(
        "server-core", bpo::value<unsigned>()->default_value(1), "server messenger running core")(
        "server-bs", bpo::value<unsigned>()->default_value(0), "server block size")(
        "crc-enabled", bpo::value<bool>()->default_value(false), "enable CRC checks");
