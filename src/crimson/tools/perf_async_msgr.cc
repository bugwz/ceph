...
struct Server
{
    ...
    struct ServerDispatcher : Dispatcher
    {
        ...
        bool ms_can_fast_dispatch(const Message* m) const override { return m->get_type() == CEPH_MSG_OSD_OP; }
...
int main(int argc, char** argv)
{
    ...
    desc.add_options()("help,h", "show help message")("addr",
                                                      po::value<std::string>()->default_value("v2:127.0.0.1:9010"),
                                                      "server address(crimson only supports msgr v2 protocol)")(
        "bs", po::value<unsigned>()->default_value(0), "server block size")(
        "crc-enabled", po::value<bool>()->default_value(false), "enable CRC checks")(
        "threads", po::value<unsigned>()->default_value(3), "async messenger worker threads");
    po::variables_map vm;
    ...
    std::cout << "server[" << addr << "](bs=" << bs << ", crc_enabled=" << crc_enabled
              << ", worker_threads=" << worker_threads << std::endl;
