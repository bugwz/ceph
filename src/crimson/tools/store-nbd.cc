 ...
 * Example usage:
 *
 *  $ ./bin/crimson-store-nbd --device-path /dev/nvme1n1 -c 1 --total-device-size=107374182400 --mkfs true --uds-path
...
public:
    struct config_t
    {
        ...
        void populate_options(po::options_description& desc)
        {
            desc.add_options()(
                "type",
                po::value<std::string>()->default_value("transaction_manager")->notifier([this](auto s) { type = s; }),
                "Backend to use, options are transaction_manager")(
                "segment-size",
                po::value<size_t>()->default_value(16ul << 20 /* 16MB */)->notifier([this](auto s) {
                    segment_size = s;
                }),
                "Total working set size")(
                "total-device-size",
                po::value<size_t>()->default_value(10ul << 30 /* 10G */)->notifier([this](auto s) {
                    total_device_size = s;
                }),
                "Size of writes")("device-path",
                                  po::value<std::string>()->required()->notifier([this](auto s) { path = s; }),
                                  "Number of writes outstanding")(
                "mkfs",
                po::value<bool>()->default_value(false)->notifier([this](auto s) { mkfs = s; }),
                "Do mkfs first");
        }
    };
    ...
    virtual seastar::future<> write(off_t offset, bufferptr ptr) = 0;

    virtual seastar::future<bufferlist> read(off_t offset, size_t size) = 0;
...
struct request_context_t
{
    ...
    uint32_t get_command() const { return type & 0xff; }

    bool has_input_buffer() const { return get_command() == NBD_CMD_WRITE; }

    seastar::future<> read_request(seastar::input_stream<char>& in)
    {
        return in.read_exactly(sizeof(struct nbd_request)).then([this, &in](auto buf) {
            auto p = buf.get();
            ...
            logger().debug("Got request, magic {}, type {}, from {}, len {}", magic, type, from, len);
    ...
    seastar::future<> write_reply(seastar::output_stream<char>& out)
    {
        ...
        return out.write(std::move(buffer))
            .then([this, &out] {
                if (out_buffer) {
                    return seastar::do_for_each(out_buffer->mut_buffers(), [&out](bufferptr& ptr) {
                        return out.write(seastar::temporary_buffer<char>(
                            ptr.c_str(), ptr.length(), seastar::make_deleter([ptr]() {})));
                    });
                }
                else {
                    return seastar::now();
...
public:
    struct config_t
    {
        ...
        void populate_options(po::options_description& desc)
        {
            desc.add_options()(
                "uds-path",
                po::value<std::string>()->default_value("/tmp/store_nbd_socket.sock")->notifier([this](auto s) {
                    uds_path = s;
                }),
    ...
    NBDHandler(BlockDriver& backend, config_t config)
        : backend(backend)
...
int main(int argc, char** argv)
{
    ...
    desc.add_options()("help,h",
                       "show help message")("debug", po::value<bool>(&debug)->default_value(false), "enable debugging");
    ...
    std::transform(begin(unrecognized_options), end(unrecognized_options), std::back_inserter(av), [](auto& s) {
    ...
    sc.run([=] {
        ...
        return seastar::do_with(NBDHandler(*backend, nbd_config), std::move(backend), [](auto& nbd, auto& backend) {
            return backend->mount()
                .then([&] {
                    logger().debug("Running nbd server...");
                    return nbd.run();
                })
                .then([&] { return backend->close(); });
        });
    });
    sc.stop();
}
...
public:
    nbd_oldstyle_negotiation_t(uint64_t size, uint32_t flags)
        : size(seastar::cpu_to_be(size))
        , flags(seastar::cpu_to_be(flags))
    {}
} __attribute__((packed));

seastar::future<> send_negotiation(size_t size, seastar::output_stream<char>& out)
{
...
seastar::future<> handle_command(BlockDriver& backend, request_context_t& context, seastar::output_stream<char>& out)
{
    logger().debug("got command {}", context.get_command());
    return ([&] {
               switch (context.get_command()) {
               case NBD_CMD_WRITE: return backend.write(context.from, *context.in_buffer);
               case NBD_CMD_READ:
                   return backend.read(context.from, context.len).then([&context](auto buffer) {
                       context.out_buffer = buffer;
                   });
               case NBD_CMD_DISC: throw std::system_error(std::make_error_code(std::errc::bad_message));
               case NBD_CMD_TRIM: throw std::system_error(std::make_error_code(std::errc::bad_message));
               default: throw std::system_error(std::make_error_code(std::errc::bad_message));
               }
           })()
        .then([&] {
...
seastar::future<> handle_commands(BlockDriver& backend, seastar::input_stream<char>& in,
                                  seastar::output_stream<char>& out)
{
    ...
    return seastar::keep_doing([&] {
        ...
        return request.read_request(in)
            .then([&] { return handle_command(backend, request, out); })
            .then([req = std::move(request_ref)] { logger().debug("complete"); });
    });
}
...
seastar::future<> NBDHandler::run()
{
    ...
    return seastar::do_with(
        seastar::engine().listen(seastar::socket_address{seastar::unix_domain_addr{uds_path}}), [=](auto& socket) {
            return seastar::keep_doing([this, &socket] {
                return socket.accept().then([this](auto acc) {
                    logger().debug("Accepted");
                    return seastar::do_with(std::move(acc.connection), [this](auto& conn) {
                        return seastar::do_with(conn.input(), conn.output(), [&, this](auto& input, auto& output) {
                            return send_negotiation(backend.get_size(), output)
                                .then([&, this] { return handle_commands(backend, input, output); })
                                .finally([&] { return input.close(); })
                                .finally([&] { return output.close(); })
                                .handle_exception([](auto e) { return seastar::now(); });
                        });
                    });
                });
            });
...
public:
    ...
    bufferptr get_buffer(size_t size) final { return ceph::buffer::create_page_aligned(size); }
    ...
    seastar::future<> write(off_t offset, bufferptr ptr) final
    {
        ...
        return seastar::do_with(tm->create_transaction(),
                                std::move(ptr),
    ...
    seastar::future<bufferlist> read(off_t offset, size_t size) final
    {
        ...
        return seastar::do_with(
                   ...
                   [this, offset, size](auto& t) {
                       return tm->read_extents<TestBlock>(*t, offset, size).safe_then([=](auto ext_list) mutable {
                           size_t cur = offset;
                           bufferlist bl;
                           for (auto& i : ext_list) {
                               if (cur != i.first) {
    ...
    void init()
    {
        segment_cleaner = std::make_unique<SegmentCleaner>(
            SegmentCleaner::config_t::default_from_segment_manager(*segment_manager), true);
        ...
        tm = std::make_unique<TransactionManager>(*segment_manager, *segment_cleaner, *journal, *cache, *lba_manager);
    ...
    size_t get_size() const final { return segment_manager->get_size() * .5; }

    seastar::future<> mkfs()
    {
        assert(config.path);
        segment_manager = std::make_unique<segment_manager::block::BlockSegmentManager>();
        logger().debug("mkfs");
        return segment_manager->mkfs({*config.path, config.segment_size, config.total_device_size})
            .safe_then([this] {
                logger().debug("");
    ...
    {
        return segment_manager->close()
            .safe_then([this] { return tm->close(); })
            .safe_then([this] {
                clear();
                return seastar::now();
            })
