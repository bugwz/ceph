...
seastar::future<> ReplicatedRecoveryBackend::recover_object(const hobject_t& soid, eversion_t need)
{
    ...
    return maybe_pull_missing_obj(soid, need).then([this, soid, need] {
        ...
        return pg
            .with_head_obc<RWState::RWREAD>(soid,
            ...
            .handle_error(crimson::osd::PG::load_obc_ertr::all_same_way([soid](auto& code) {
...
seastar::future<> ReplicatedRecoveryBackend::maybe_push_shards(const hobject_t& soid, eversion_t need)
{
    return seastar::parallel_for_each(
               ...
               [this, need, soid](auto shard) {
                   return prep_push(soid, need, shard).then([this, soid, shard](auto push) {
                       ...
                       return shard_services.send_to_osd(shard.osd, std::move(msg), pg.get_osdmap_epoch())
                           .then([this, soid, shard] { return recovering.at(soid).wait_for_pushes(shard); });
                   });
               })
        .then([this, soid] {
...
seastar::future<> ReplicatedRecoveryBackend::maybe_pull_missing_obj(const hobject_t& soid, eversion_t need)
{
    ...
    return shard_services.send_to_osd(pi.from.osd, std::move(msg), pg.get_osdmap_epoch()).then([&recovery_waiter] {
        return recovery_waiter.wait_for_pull();
    });
}

seastar::future<> ReplicatedRecoveryBackend::push_delete(const hobject_t& soid, eversion_t need)
{
    logger().debug("{}: {}, {}", __func__, soid, need);
    recovering[soid];
    ...
    return seastar::parallel_for_each(
        pg.get_acting_recovery_backfill(), [this, soid, need, min_epoch](pg_shard_t shard) {
            ...
            if (iter->second.is_missing(soid)) {
                ...
                auto msg =
                    make_message<MOSDPGRecoveryDelete>(pg.get_pg_whoami(), target_pg, pg.get_osdmap_epoch(), min_epoch);
                msg->set_priority(pg.get_recovery_op_priority());
...
seastar::future<> ReplicatedRecoveryBackend::handle_recovery_delete(Ref<MOSDPGRecoveryDelete> m)
{
    logger().debug("{}: {}", __func__, *m);

    auto& p = m->objects.front();   // TODO: only one delete per message for now.
    return local_recover_delete(p.first, p.second, pg.get_osdmap_epoch()).then([this, m] {
...
seastar::future<> ReplicatedRecoveryBackend::on_local_recover_persist(const hobject_t& soid,
                                                                      const ObjectRecoveryInfo& _recovery_info,
                                                                      bool is_delete, epoch_t epoch_frozen)
{
    ...
    return shard_services.get_store()
        ...
        .then([this, epoch_frozen, last_complete = pg.get_info().last_complete] {
            pg.get_recovery_handler()->_committed_pushed_object(epoch_frozen, last_complete);
            return seastar::make_ready_future<>();
        });
}

seastar::future<> ReplicatedRecoveryBackend::local_recover_delete(const hobject_t& soid, eversion_t need,
                                                                  epoch_t epoch_to_freeze)
{
    logger().debug("{}: {}, {}", __func__, soid, need);
    return backend->load_metadata(soid)
        .safe_then([this](auto lomt) {
            if (lomt->os.exists) {
...
seastar::future<> ReplicatedRecoveryBackend::recover_delete(const hobject_t& soid, eversion_t need)
{
    logger().debug("{}: {}, {}", __func__, soid, need);
    ...
    return seastar::do_with(object_stat_sum_t(), [this, soid, need, cur_epoch](auto& stat_diff) {
        return local_recover_delete(soid, need, cur_epoch)
            .then([this, &stat_diff, cur_epoch, soid, need] {
...
seastar::future<PushOp> ReplicatedRecoveryBackend::prep_push(const hobject_t& soid, eversion_t need,
                                                             pg_shard_t pg_shard)
{
    logger().debug("{}: {}, {}", __func__, soid, need);
    ...
    pi.recovery_progress.omap_complete =
        (!missing_iter->second.clean_regions.omap_is_dirty() && HAVE_FEATURE(pg.min_peer_features(), SERVER_OCTOPUS));

    return build_push_op(pi.recovery_info, pi.recovery_progress, &pi.stat).then([this, soid, pg_shard](auto pop) {
        auto& recovery_waiter = recovering.at(soid);
        auto& pi = recovery_waiter.pushing[pg_shard];
...
void ReplicatedRecoveryBackend::prepare_pull(PullOp& po, PullInfo& pi, const hobject_t& soid, eversion_t need)
{
    logger().debug("{}: {}, {}", __func__, soid, need);
    ...
    if (HAVE_FEATURE(pg.min_peer_features(), SERVER_OCTOPUS))
        po.recovery_info.copy_subset.intersection_of(missing_iter->second.clean_regions.get_dirty_regions());
    po.recovery_info.size = ((uint64_t)-1);
    po.recovery_info.object_exist = missing_iter->second.clean_regions.object_is_exist();
...
seastar::future<PushOp> ReplicatedRecoveryBackend::build_push_op(const ObjectRecoveryInfo& recovery_info,
                                                                 ...
                                                                 object_stat_sum_t* stat)
{
    ...
    return seastar::do_with(
        ...
        PushOp(),
        [this, &recovery_info, &progress, stat](auto new_progress, auto available, auto v, auto pop) {
            return read_metadata_for_push_op(recovery_info.soid, progress, new_progress, v, &pop)
                .then([&](eversion_t local_ver) mutable {
                    ...
                    return read_omap_for_push_op(recovery_info.soid, progress, new_progress, &available, &pop);
                })
                .then([this, &recovery_info, &progress, &available, &pop]() mutable {
                    ...
                    return read_object_for_push_op(
                        recovery_info.soid, recovery_info.copy_subset, progress.data_recovered_to, available, &pop);
                })
                .then([&recovery_info, &v, &progress, &new_progress, stat, &pop](uint64_t recovered_to) mutable {
                    ...
                    logger().debug(
                        "build_push_op: pop version: {}, pop data length: {}", pop.version, pop.data.length());
                    return seastar::make_ready_future<PushOp>(std::move(pop));
                });
        });
}

seastar::future<eversion_t> ReplicatedRecoveryBackend::read_metadata_for_push_op(const hobject_t& oid,
                                                                                 const ObjectRecoveryProgress& progress,
                                                                                 ObjectRecoveryProgress& new_progress,
                                                                                 eversion_t ver, PushOp* push_op)
{
    if (!progress.first) {
    ...
    return seastar::when_all_succeed(
               backend->omap_get_header(coll, ghobject_t(oid))
                   .handle_error(crimson::os::FuturizedStore::read_errorator::all_same_way(
                       [](const std::error_code& e) { return seastar::make_ready_future<bufferlist>(); })),
               store->get_attrs(coll, ghobject_t(oid))
                   .handle_error(
                       crimson::os::FuturizedStore::get_attrs_ertr::all_same_way([](const std::error_code& e) {
...
seastar::future<uint64_t> ReplicatedRecoveryBackend::read_object_for_push_op(const hobject_t& oid,
                                                                             ...
                                                                             PushOp* push_op)
{
    ...
    return backend->fiemap(coll, ghobject_t{oid}, 0, copy_subset.range_end())
        .then_wrapped([=](auto&& fiemap_included) mutable {
...
seastar::future<> ReplicatedRecoveryBackend::read_omap_for_push_op(const hobject_t& oid,
                                                                   const ObjectRecoveryProgress& progress,
                                                                   ObjectRecoveryProgress& new_progress,
                                                                   uint64_t* max_len, PushOp* push_op)
{
    ...
    return shard_services.get_store()
        .get_omap_iterator(coll, ghobject_t{oid})
        .then([&progress, &new_progress, max_len, push_op](auto omap_iter) {
            return omap_iter->lower_bound(progress.omap_recovered_to)
                .then([omap_iter, &new_progress, max_len, push_op] {
                    return seastar::do_until(
                        [omap_iter, &new_progress, max_len, push_op] {
                            ...
                            if (const uint64_t entries_per_chunk =
                                ...
                                entries_per_chunk > 0 && push_op->omap_entries.size() >= entries_per_chunk) {
                        ...
                        [omap_iter, max_len, push_op] {
                            ...
                            if (const uint64_t entry_size = omap_iter->key().size() + omap_iter->value().length();
                                entry_size > *max_len) {
                                *max_len -= entry_size;
                            }
                            else {
...
seastar::future<bool> ReplicatedRecoveryBackend::_handle_pull_response(pg_shard_t from, const PushOp& pop,
                                                                       PullOp* response, ceph::os::Transaction* t)
{
    ...
    if (pi.recovery_info.version == eversion_t()) pi.recovery_info.version = pop.version;

    auto prepare_waiter = seastar::make_ready_future<>();
    if (pi.recovery_progress.first) {
        prepare_waiter =
            pg.with_head_obc<RWState::RWNONE>(pi.recovery_info.soid, [&pi, &recovery_waiter, &pop](auto obc) {
                  pi.obc = obc;
                  recovery_waiter.obc = obc;
    ...
    return prepare_waiter.then([this, &pi, &pop, t, response]() mutable {
        ...
        pi.recovery_progress = pop.after_progress;
        logger().debug("new recovery_info {}, new progress {}", pi.recovery_info, pi.recovery_progress);
        ...
        auto [usable_intervals, data] = trim_pushed_data(pi.recovery_info.copy_subset, pop.data_included, pop.data);
        bool complete = pi.is_complete();
        bool clear_omap = !pop.before_progress.omap_complete;
        return submit_push_data(pi.recovery_info,
                                first,
                                complete,
                                clear_omap,
                                std::move(data_zeros),
                                ...
                                pop.omap_entries,
                                t)
            .then([this, response, &pi, &pop, complete, t, bytes_recovered = data.length()] {
                ...
                if (complete) {
                    ...
                    pg.get_recovery_handler()->on_local_recover(
                        pop.soid, recovering.at(pop.soid).pi->recovery_info, false, *t);
                    return true;
                }
                else {
...
seastar::future<> ReplicatedRecoveryBackend::handle_pull_response(Ref<MOSDPGPush> m)
{
    const PushOp& pop = m->pushes[0];   // TODO: only one push per message for now.
    if (pop.version == eversion_t()) {
        // replica doesn't have it!
        pg.get_recovery_handler()->on_failed_recover(
            {m->from}, pop.soid, get_recovering(pop.soid).pi->recovery_info.version);
        return seastar::make_exception_future<>(
            std::runtime_error(fmt::format("Error on pushing side {} when pulling obj {}", m->from, pop.soid)));
    }
    ...
    return seastar::do_with(PullOp(), [this, m](auto& response) {
        return seastar::do_with(
                   ...
                   [this, &response](auto& t, auto& m) {
                       ...
                       return _handle_pull_response(from, pop, &response, &t).then([this, &t](bool complete) {
...
seastar::future<> ReplicatedRecoveryBackend::_handle_push(pg_shard_t from, const PushOp& pop, PushReplyOp* response,
                                                          ceph::os::Transaction* t)
{
    logger().debug("{}", __func__);
    ...
    return submit_push_data(pop.recovery_info,
                            first,
                            complete,
                            clear_omap,
                            std::move(data_zeros),
                            ...
                            pop.omap_entries,
                            t)
        .then([this, complete, &pop, t] {
            if (complete) {
                pg.get_recovery_handler()->on_local_recover(pop.recovery_info.soid, pop.recovery_info, false, *t);
            }
        });
}

seastar::future<> ReplicatedRecoveryBackend::handle_push(Ref<MOSDPGPush> m)
{
    if (pg.is_primary()) {
    ...
    return seastar::do_with(PushReplyOp(), [this, m](auto& response) {
        ...
        return seastar::do_with(ceph::os::Transaction(),
                                [this, m, &pop, &response](auto& t) {
                                    return _handle_push(m->from, pop, &response, &t).then([this, &t] {
                                        epoch_t epoch_frozen = pg.get_osdmap_epoch();
                                        return shard_services.get_store()
                                            .do_transaction(coll, std::move(t))
                                            .then([this, epoch_frozen, last_complete = pg.get_info().last_complete] {
                                                // TODO: this should be grouped with pg.on_local_recover somehow.
                                                pg.get_recovery_handler()->_committed_pushed_object(epoch_frozen,
                                                                                                    last_complete);
                                            });
                                    });
                                })
...
seastar::future<std::optional<PushOp>> ReplicatedRecoveryBackend::_handle_push_reply(pg_shard_t peer,
                                                                                     const PushReplyOp& op)
{
    ...
    if (recovering_iter == recovering.end() || !recovering_iter->second.pushing.count(peer)) {
        ...
        if (!pi.recovery_progress.data_complete && !error) {
            return build_push_op(pi.recovery_info, pi.recovery_progress, &pi.stat)
                .then([&pi](auto pop) {
                    pi.recovery_progress = pop.after_progress;
...
seastar::future<> ReplicatedRecoveryBackend::handle_push_reply(Ref<MOSDPGPushReply> m)
{
    logger().debug("{}: {}", __func__, *m);
    auto from = m->from;
    auto& push_reply = m->replies[0];   // TODO: only one reply per message

    return _handle_push_reply(from, push_reply).then([this, from](std::optional<PushOp> push_op) {
        if (push_op) {
            auto msg = make_message<MOSDPGPush>();
            msg->from = pg.get_pg_whoami();
            msg->pgid = pg.get_pgid();
...
seastar::future<> ReplicatedRecoveryBackend::submit_push_data(
    ...
    interval_set<uint64_t> data_zeros, const interval_set<uint64_t>& intervals_included, bufferlist data_included,
    bufferlist omap_header, const map<string, bufferlist>& attrs, const map<string, bufferlist>& omap_entries,
    ObjectStore::Transaction* t)
{
    ...
    if (first && complete) {
        ...
        target_oid = get_temp_recovery_object(recovery_info.soid, recovery_info.version);
        if (first) {
    ...
    return [this, &recovery_info, first, complete, t, &omap_header, &attrs, target_oid, clear_omap] {
        if (first) {
            if (!complete) {
                ...
                if (clear_omap) t->omap_clear(coll->get_cid(), ghobject_t(target_oid));
            }

            t->truncate(coll->get_cid(), ghobject_t(target_oid), recovery_info.size);
            if (omap_header.length()) t->omap_setheader(coll->get_cid(), ghobject_t(target_oid), omap_header);

            return store->stat(coll, ghobject_t(recovery_info.soid))
                .then([this, &recovery_info, complete, t, target_oid, omap_header = std::move(omap_header)](auto st) {
                    ...
                    if (!complete) {
                        ...
                        if (recovery_info.object_exist) {
                            ...
                            if (local_size) {
                                ...
                                local_intervals_excluded.intersection_of(local_intervals_included,
               ...
               .then([this,
                   ...
                   if (complete) {
                       if (!first) {
                           ...
                           t->collection_move_rename(coll->get_cid(),
                                                     ghobject_t(target_oid),
                                                     coll->get_cid(),
                                                     ghobject_t(recovery_info.soid));
                       }
...
seastar::future<> ReplicatedRecoveryBackend::handle_recovery_delete_reply(Ref<MOSDPGRecoveryDeleteReply> m)
{
