...
{
    switch (pool.type) {
    case pg_pool_t::TYPE_REPLICATED: return std::make_unique<ReplicatedBackend>(pgid, pg_shard, coll, shard_services);
    case pg_pool_t::TYPE_ERASURE:
        return std::make_unique<ECBackend>(
            pg_shard.shard, coll, shard_services, std::move(ec_profile), pool.stripe_width);
    default: throw runtime_error(seastar::format("unsupported pool type '{}'", pool.type));
    }
}

PGBackend::PGBackend(shard_id_t shard, CollectionRef coll, crimson::os::FuturizedStore* store)
    : shard{shard}
    , coll{coll}
    , store{store}
{}

PGBackend::load_metadata_ertr::future<PGBackend::loaded_object_md_t::ref> PGBackend::load_metadata(const hobject_t& oid)
{
    if (__builtin_expect(stopping, false)) {
        throw crimson::common::system_shutdown_exception();
    }

    return store->get_attrs(coll, ghobject_t{oid, ghobject_t::NO_GEN, shard})
        .safe_then(
...
seastar::future<crimson::osd::acked_peers_t> PGBackend::mutate_object(
    std::set<pg_shard_t> pg_shards, crimson::osd::ObjectContextRef&& obc, ceph::os::Transaction&& txn,
    const osd_op_params_t& osd_op_p, epoch_t min_epoch, epoch_t map_epoch, std::vector<pg_log_entry_t>&& log_entries)
{
...
PGBackend::read_errorator::future<> PGBackend::read(const ObjectState& os, OSDOp& osd_op)
{
    const auto& oi = os.oi;
    ...
    return _read(oi.soid, offset, length, op.flags).safe_then([&oi, &osd_op](auto&& bl) -> read_errorator::future<> {
...
PGBackend::read_errorator::future<> PGBackend::sparse_read(const ObjectState& os, OSDOp& osd_op)
{
    const auto& op = osd_op.op;
    logger().trace("sparse_read: {} {}~{}", os.oi.soid, op.extent.offset, op.extent.length);
    return store->fiemap(coll, ghobject_t{os.oi.soid}, op.extent.offset, op.extent.length)
        .then([&os, &osd_op, this](auto&& m) {
            return seastar::do_with(interval_set<uint64_t>{std::move(m)}, [&os, &osd_op, this](auto&& extents) {
                return store->readv(coll, ghobject_t{os.oi.soid}, extents, osd_op.op.flags)
                    .safe_then([&os, &osd_op, &extents](auto&& bl) -> read_errorator::future<> {
...
PGBackend::checksum_errorator::future<> PGBackend::checksum(const ObjectState& os, OSDOp& osd_op)
{
    ...
    return _read(os.oi.soid, checksum.offset, checksum.length, osd_op.op.flags)
        .safe_then([&osd_op](auto&& read_bl) mutable -> checksum_errorator::future<> {
            auto& checksum = osd_op.op.checksum;
...
PGBackend::cmp_ext_errorator::future<> PGBackend::cmp_ext(const ObjectState& os, OSDOp& osd_op)
{
    const ceph_osd_op& op = osd_op.op;
...
PGBackend::stat_errorator::future<> PGBackend::stat(const ObjectState& os, OSDOp& osd_op)
{
...
bool PGBackend::maybe_create_new_object(ObjectState& os, ceph::os::Transaction& txn)
{
    if (!os.exists) {
...
seastar::future<> PGBackend::write(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
...
seastar::future<> PGBackend::write_same(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                        osd_op_params_t& osd_op_params)
{
    const ceph_osd_op& op = osd_op.op;
...
seastar::future<> PGBackend::writefull(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                       osd_op_params_t& osd_op_params)
{
    const ceph_osd_op& op = osd_op.op;
    ...
    if (existing && op.extent.length < os.oi.size) {
        ...
        osd_op_params.clean_regions.mark_data_region_dirty(op.extent.length, os.oi.size - op.extent.length);
    }
    if (op.extent.length) {
        txn.write(coll->get_cid(), ghobject_t{os.oi.soid}, 0, op.extent.length, osd_op.indata, op.flags);
        os.oi.size = op.extent.length;
        osd_op_params.clean_regions.mark_data_region_dirty(0, std::max((uint64_t)op.extent.length, os.oi.size));
    }
    return seastar::now();
}

PGBackend::append_errorator::future<> PGBackend::append(ObjectState& os, OSDOp& osd_op, ceph::os::Transaction& txn,
                                                        osd_op_params_t& osd_op_params)
{
    const ceph_osd_op& op = osd_op.op;
    if (op.extent.length != osd_op.indata.length()) {
...
PGBackend::write_ertr::future<> PGBackend::truncate(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                                    osd_op_params_t& osd_op_params)
{
    if (!os.exists || os.oi.is_whiteout()) {
    ...
    if (op.extent.truncate_seq) {
        ...
        if (op.extent.truncate_seq <= os.oi.truncate_seq) {
            logger().debug(
                "{} truncate seq {} <= current {}, no-op", __func__, op.extent.truncate_seq, os.oi.truncate_seq);
            return write_ertr::make_ready_future<>();
        }
        else {
            logger().debug(
                "{} truncate seq {} > current {}, truncating", __func__, op.extent.truncate_seq, os.oi.truncate_seq);
            os.oi.truncate_seq = op.extent.truncate_seq;
            os.oi.truncate_size = op.extent.truncate_size;
        }
    }
    ...
    if (os.oi.size != op.extent.offset) {
        ...
        if (os.oi.size > op.extent.offset) {
            ...
            osd_op_params.clean_regions.mark_data_region_dirty(op.extent.offset, os.oi.size - op.extent.offset);
        }
        else {
            // os.oi.size < op.extent.offset
            osd_op_params.clean_regions.mark_data_region_dirty(os.oi.size, op.extent.offset - os.oi.size);
        }
        os.oi.size = op.extent.offset;
...
PGBackend::write_ertr::future<> PGBackend::zero(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                                osd_op_params_t& osd_op_params)
{
    if (!os.exists || os.oi.is_whiteout()) {
...
seastar::future<> PGBackend::create(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn)
{
...
seastar::future<> PGBackend::remove(ObjectState& os, ceph::os::Transaction& txn)
{
    // todo: snapset
...
seastar::future<std::tuple<std::vector<hobject_t>, hobject_t>> PGBackend::list_objects(const hobject_t& start,
                                                                                       uint64_t limit) const
{
    ...
    return store->list_objects(coll, gstart, ghobject_t::get_max(), limit).then([](auto ret) {
...
seastar::future<> PGBackend::setxattr(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn)
{
...
PGBackend::get_attr_errorator::future<> PGBackend::getxattr(const ObjectState& os, OSDOp& osd_op) const
{
    std::string name;
    ceph::bufferlist val;
    {
...
PGBackend::get_attr_errorator::future<ceph::bufferptr> PGBackend::getxattr(const hobject_t& soid,
...
PGBackend::get_attr_errorator::future<> PGBackend::get_xattrs(const ObjectState& os, OSDOp& osd_op) const
{
    if (__builtin_expect(stopping, false)) {
        throw crimson::common::system_shutdown_exception();
    }
    return store->get_attrs(coll, ghobject_t{os.oi.soid}).safe_then([&osd_op](auto&& attrs) {
...
PGBackend::rm_xattr_ertr::future<> PGBackend::rm_xattr(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn)
{
    if (__builtin_expect(stopping, false)) {
        throw crimson::common::system_shutdown_exception();
    }
...
using get_omap_ertr = crimson::os::FuturizedStore::read_errorator::extend<crimson::ct_error::enodata>;
static get_omap_ertr::future<crimson::os::FuturizedStore::omap_values_t> maybe_get_omap_vals_by_keys(
    crimson::os::FuturizedStore* store, const crimson::os::CollectionRef& coll, const object_info_t& oi,
...
static get_omap_ertr::future<std::tuple<bool, crimson::os::FuturizedStore::omap_values_t>> maybe_get_omap_vals(
    crimson::os::FuturizedStore* store, const crimson::os::CollectionRef& coll, const object_info_t& oi,
...
PGBackend::ll_read_errorator::future<ceph::bufferlist> PGBackend::omap_get_header(const crimson::os::CollectionRef& c,
                                                                                  const ghobject_t& oid) const
{
    return store->omap_get_header(c, oid);
}

PGBackend::ll_read_errorator::future<> PGBackend::omap_get_header(const ObjectState& os, OSDOp& osd_op) const
{
    return omap_get_header(coll, ghobject_t{os.oi.soid}).safe_then([&osd_op](ceph::bufferlist&& header) {
        osd_op.outdata = std::move(header);
        return seastar::now();
    });
}

PGBackend::ll_read_errorator::future<> PGBackend::omap_get_keys(const ObjectState& os, OSDOp& osd_op) const
{
    if (__builtin_expect(stopping, false)) {
        throw crimson::common::system_shutdown_exception();
    }
    ...
    return maybe_get_omap_vals(store, coll, os.oi, start_after)
        ...
        .handle_error(crimson::ct_error::enodata::handle([&osd_op] {
                          uint32_t num = 0;
                          bool truncated = false;
                          encode(num, osd_op.outdata);
                          encode(truncated, osd_op.outdata);
...
PGBackend::ll_read_errorator::future<> PGBackend::omap_get_vals(const ObjectState& os, OSDOp& osd_op) const
{
    if (__builtin_expect(stopping, false)) {
        throw crimson::common::system_shutdown_exception();
    }
...
PGBackend::ll_read_errorator::future<> PGBackend::omap_get_vals_by_keys(const ObjectState& os, OSDOp& osd_op) const
{
    if (__builtin_expect(stopping, false)) {
        throw crimson::common::system_shutdown_exception();
    }
    ...
    return maybe_get_omap_vals_by_keys(store, coll, os.oi, keys_to_get)
        .safe_then([&osd_op](crimson::os::FuturizedStore::omap_values_t&& vals) {
...
seastar::future<> PGBackend::omap_set_vals(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                           osd_op_params_t& osd_op_params)
{
    maybe_create_new_object(os, txn);
...
seastar::future<> PGBackend::omap_set_header(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn)
{
    maybe_create_new_object(os, txn);
...
seastar::future<> PGBackend::omap_remove_range(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn)
{
...
PGBackend::omap_clear_ertr::future<> PGBackend::omap_clear(ObjectState& os, OSDOp& osd_op, ceph::os::Transaction& txn,
                                                           osd_op_params_t& osd_op_params)
{
    if (__builtin_expect(stopping, false)) {
        throw crimson::common::system_shutdown_exception();
...
seastar::future<struct stat> PGBackend::stat(CollectionRef c, const ghobject_t& oid) const
{
    return store->stat(c, oid);
}

seastar::future<std::map<uint64_t, uint64_t>> PGBackend::fiemap(CollectionRef c, const ghobject_t& oid, uint64_t off,
                                                                uint64_t len)
{
