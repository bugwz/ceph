...
std::unique_ptr<PGBackend> PGBackend::create(pg_t pgid, const pg_shard_t pg_shard, const pg_pool_t& pool,
                                             crimson::os::CollectionRef coll,
                                             crimson::osd::ShardServices& shard_services,
                                             const ec_profile_t& ec_profile, DoutPrefixProvider& dpp)
{
    switch (pool.type) {
    case pg_pool_t::TYPE_REPLICATED:
        return std::make_unique<ReplicatedBackend>(pgid, pg_shard, coll, shard_services, dpp);
    case pg_pool_t::TYPE_ERASURE:
        return std::make_unique<ECBackend>(
            pg_shard.shard, coll, shard_services, std::move(ec_profile), pool.stripe_width, dpp);
    default: throw runtime_error(seastar::format("unsupported pool type '{}'", pool.type));
    }
}

PGBackend::PGBackend(shard_id_t shard, CollectionRef coll, crimson::osd::ShardServices& shard_services,
...
PGBackend::load_metadata_iertr::future<PGBackend::loaded_object_md_t::ref> PGBackend::load_metadata(
    const hobject_t& oid)
{
    return interruptor::make_interruptible(store->get_attrs(coll, ghobject_t{oid, ghobject_t::NO_GEN, shard}))
        .safe_then_interruptible(
            [oid](auto&& attrs) -> load_metadata_ertr::future<loaded_object_md_t::ref> {
                ...
                if (auto oiiter = attrs.find(OI_ATTR); oiiter != attrs.end()) {
                    ...
                    try {
                        ret->os = ObjectState(object_info_t(bl, oid), true);
                    }
                    ...
                    logger().error("load_metadata: object {} present but missing object info", oid);
                ...
                if (oid.is_head()) {
                    ...
                    if (auto ssiter = attrs.find(SS_ATTR); ssiter != attrs.end()) {
                        ...
                        if (bl.length()) {
                            ...
                            try {
                                ...
                                ret->ssc->exists = true;
                                logger().debug(
                                    "load_metadata: object {} and snapset {} present", oid, ret->ssc->snapset);
                            }
                            catch (const buffer::error&) {
                    ...
                    if (object_corrupted) {
                        logger().error("load_metadata: object {} present but missing snapset", oid);
                        return crimson::ct_error::object_corrupted::make();
                    }
                }

                return load_metadata_ertr::make_ready_future<loaded_object_md_t::ref>(std::move(ret));
            },
            crimson::ct_error::enoent::handle([oid] {
                logger().debug("load_metadata: object {} doesn't exist, returning empty metadata", oid);
                return load_metadata_ertr::make_ready_future<loaded_object_md_t::ref>(
                    new loaded_object_md_t{ObjectState(object_info_t(oid), false),
                                           oid.is_head() ? (new crimson::osd::SnapSetContext(oid)) : nullptr});
            }));
}

PGBackend::rep_op_fut_t PGBackend::mutate_object(std::set<pg_shard_t> pg_shards, crimson::osd::ObjectContextRef&& obc,
                                                 ceph::os::Transaction&& txn, osd_op_params_t&& osd_op_p,
                                                 ...
                                                 std::vector<pg_log_entry_t>&& log_entries)
{
    ...
    obc->obs.oi.prior_version = ctx->obs->oi.version;
        ...
        if (osd_op_p.user_at_version > obc->obs.oi.user_version) obc->obs.oi.user_version = osd_op_p.user_at_version;
        ...
        if (obc->obs.oi.soid.snap == CEPH_NOSNAP) {
            logger().debug("final snapset {} in {}", obc->ssc->snapset, obc->obs.oi.soid);
            ceph::bufferlist bss;
    ...
    return _submit_transaction(std::move(pg_shards),
                               obc->obs.oi.soid,
                               std::move(txn),
                               std::move(osd_op_p),
                               min_epoch,
...
static inline bool _read_verify_data(const object_info_t& oi, const ceph::bufferlist& data)
{
    if (oi.is_data_digest() && oi.size == data.length()) {
        // whole object?  can we verify the checksum?
        if (auto crc = data.crc32c(-1); crc != oi.data_digest) {
            logger().error("full-object read crc {} != expected {} on {}", crc, oi.data_digest, oi.soid);
...
PGBackend::read_ierrorator::future<> PGBackend::read(const ObjectState& os, OSDOp& osd_op,
                                                     object_stat_sum_t& delta_stats)
{
    ...
    size_t size = oi.size;
    if ((op.extent.truncate_seq > oi.truncate_seq) && (op.extent.truncate_size < offset + length) &&
    ...
    return _read(oi.soid, offset, length, op.flags)
...
PGBackend::read_ierrorator::future<> PGBackend::sparse_read(const ObjectState& os, OSDOp& osd_op,
                                                            object_stat_sum_t& delta_stats)
{
    if (!os.exists || os.oi.is_whiteout()) {
        logger().debug("{}: {} DNE", __func__, os.oi.soid);
    ...
    if ((os.oi.truncate_seq < op.extent.truncate_seq) &&
        (op.extent.offset + op.extent.length > op.extent.truncate_size) && (adjusted_size > op.extent.truncate_size)) {
    ...
    logger().trace("sparse_read: {} {}~{}", os.oi.soid, op.extent.offset, op.extent.length);
    return interruptor::make_interruptible(store->fiemap(coll, ghobject_t{os.oi.soid}, offset, adjusted_length))
        .safe_then_interruptible([&delta_stats, &os, &osd_op, this](auto&& m) {
            return seastar::do_with(
                interval_set<uint64_t>{std::move(m)}, [&delta_stats, &os, &osd_op, this](auto&& extents) {
                    return interruptor::make_interruptible(
                               store->readv(coll, ghobject_t{os.oi.soid}, extents, osd_op.op.flags))
                        .safe_then_interruptible_tuple(
                            [&delta_stats, &os, &osd_op, &extents](auto&& bl) -> read_errorator::future<> {
                                if (_read_verify_data(os.oi, bl)) {
                                    ...
                                    logger().trace(
                                        "sparse_read got {} bytes from object {}", osd_op.op.extent.length, os.oi.soid);
                                    delta_stats.num_rd++;
...
template<class CSum>
PGBackend::checksum_errorator::future<> do_checksum(ceph::bufferlist& init_value_bl, size_t chunk_size,
                                                    const ceph::bufferlist& buf, ceph::bufferlist& result)
{
    ...
    ceph::bufferptr csum_data{ceph::buffer::create(sizeof(typename CSum::value_t) * chunk_count)};
    Checksummer::calculate<CSum>(init_value, chunk_size, 0, buf.length(), buf, &csum_data);
...
PGBackend::checksum_ierrorator::future<> PGBackend::checksum(const ObjectState& os, OSDOp& osd_op)
{
    ...
    return _read(os.oi.soid, checksum.offset, checksum.length, osd_op.op.flags)
        .safe_then_interruptible([&osd_op](auto&& read_bl) mutable -> checksum_errorator::future<> {
            auto& checksum = osd_op.op.checksum;
            if (read_bl.length() != checksum.length) {
                logger().warn("checksum: bytes read {} != {}", read_bl.length(), checksum.length);
                return crimson::ct_error::invarg::make();
            }
            // calculate its checksum and put the result in outdata
            switch (checksum.type) {
            case CEPH_OSD_CHECKSUM_OP_TYPE_XXHASH32:
                return do_checksum<Checksummer::xxhash32>(osd_op.indata, checksum.chunk_size, read_bl, osd_op.outdata);
            case CEPH_OSD_CHECKSUM_OP_TYPE_XXHASH64:
                return do_checksum<Checksummer::xxhash64>(osd_op.indata, checksum.chunk_size, read_bl, osd_op.outdata);
            case CEPH_OSD_CHECKSUM_OP_TYPE_CRC32C:
                return do_checksum<Checksummer::crc32c>(osd_op.indata, checksum.chunk_size, read_bl, osd_op.outdata);
            default:
                logger().warn("checksum: unknown crc type ({})", static_cast<uint32_t>(checksum.type));
                return crimson::ct_error::invarg::make();
            }
        });
}

PGBackend::cmp_ext_ierrorator::future<> PGBackend::cmp_ext(const ObjectState& os, OSDOp& osd_op)
{
    const ceph_osd_op& op = osd_op.op;
    uint64_t obj_size = os.oi.size;
    if (os.oi.truncate_seq < op.extent.truncate_seq && op.extent.offset + op.extent.length > op.extent.truncate_size) {
    ...
    return read_ext.safe_then_interruptible([&osd_op](auto&& read_bl) -> cmp_ext_errorator::future<> {
...
PGBackend::stat_ierrorator::future<> PGBackend::stat(const ObjectState& os, OSDOp& osd_op,
                                                     object_stat_sum_t& delta_stats)
{
...
PGBackend::write_iertr::future<> PGBackend::_writefull(ObjectState& os, off_t truncate_size, const bufferlist& bl,
                                                       ceph::os::Transaction& txn, osd_op_params_t& osd_op_params,
                                                       object_stat_sum_t& delta_stats, unsigned flags)
{
    ...
    if (existing && bl.length() < os.oi.size) {
        ...
        osd_op_params.clean_regions.mark_data_region_dirty(bl.length(), os.oi.size - bl.length());
    }
    if (bl.length()) {
        txn.write(coll->get_cid(), ghobject_t{os.oi.soid}, 0, bl.length(), bl, flags);
        update_size_and_usage(delta_stats, os.oi, 0, bl.length(), true);
        osd_op_params.clean_regions.mark_data_region_dirty(0, std::max((uint64_t)bl.length(), os.oi.size));
    }
    return seastar::now();
}

PGBackend::write_iertr::future<> PGBackend::_truncate(ObjectState& os, ceph::os::Transaction& txn,
                                                      osd_op_params_t& osd_op_params, object_stat_sum_t& delta_stats,
                                                      size_t offset, size_t truncate_size, uint32_t truncate_seq)
{
    if (truncate_seq) {
        assert(offset == truncate_size);
        if (truncate_seq <= os.oi.truncate_seq) {
            logger().debug("{} truncate seq {} <= current {}, no-op", __func__, truncate_seq, os.oi.truncate_seq);
            return write_ertr::make_ready_future<>();
        }
        else {
            logger().debug("{} truncate seq {} > current {}, truncating", __func__, truncate_seq, os.oi.truncate_seq);
    ...
    if (os.oi.size != offset) {
        txn.truncate(coll->get_cid(), ghobject_t{os.oi.soid}, offset);
        if (os.oi.size > offset) {
            // TODO: modified_ranges.union_of(trim);
            osd_op_params.clean_regions.mark_data_region_dirty(offset, os.oi.size - offset);
        }
        else {
            // os.oi.size < offset
            osd_op_params.clean_regions.mark_data_region_dirty(os.oi.size, offset - os.oi.size);
        }
...
bool PGBackend::maybe_create_new_object(ObjectState& os, ceph::os::Transaction& txn, object_stat_sum_t& delta_stats)
{
    if (!os.exists) {
...
void PGBackend::update_size_and_usage(object_stat_sum_t& delta_stats, object_info_t& oi, uint64_t offset,
...
void PGBackend::truncate_update_size_and_usage(object_stat_sum_t& delta_stats, object_info_t& oi,
...
static bool is_offset_and_length_valid(const std::uint64_t offset, const std::uint64_t length)
{
...
PGBackend::interruptible_future<> PGBackend::set_allochint(ObjectState& os, const OSDOp& osd_op,
                                                           ceph::os::Transaction& txn, object_stat_sum_t& delta_stats)
{
    maybe_create_new_object(os, txn, delta_stats);
...
PGBackend::write_iertr::future<> PGBackend::write(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                                  osd_op_params_t& osd_op_params, object_stat_sum_t& delta_stats)
{
    const ceph_osd_op& op = osd_op.op;
    ...
    if (auto seq = os.oi.truncate_seq; seq != 0 && op.extent.truncate_seq < seq) {
    ...
    if (length == 0) {
        ...
        txn.write(coll->get_cid(), ghobject_t{os.oi.soid}, offset, length, std::move(buf), op.flags);
        update_size_and_usage(delta_stats, os.oi, offset, length);
    }
    osd_op_params.clean_regions.mark_data_region_dirty(op.extent.offset, op.extent.length);

    return seastar::now();
}

PGBackend::interruptible_future<> PGBackend::write_same(ObjectState& os, const OSDOp& osd_op,
                                                        ceph::os::Transaction& txn, osd_op_params_t& osd_op_params,
                                                        object_stat_sum_t& delta_stats)
{
    const ceph_osd_op& op = osd_op.op;
    ...
    if (op.writesame.data_length == 0 || len % op.writesame.data_length != 0 ||
    ...
    maybe_create_new_object(os, txn, delta_stats);
    txn.write(coll->get_cid(), ghobject_t{os.oi.soid}, op.writesame.offset, len, std::move(repeated_indata), op.flags);
...
PGBackend::write_iertr::future<> PGBackend::writefull(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                                      osd_op_params_t& osd_op_params, object_stat_sum_t& delta_stats)
{
    const ceph_osd_op& op = osd_op.op;
    if (op.extent.length != osd_op.indata.length()) {
    ...
    return _writefull(os, op.extent.truncate_size, osd_op.indata, txn, osd_op_params, delta_stats, op.flags);
}

PGBackend::rollback_iertr::future<> PGBackend::rollback(ObjectState& os, const OSDOp& osd_op,
                                                        ceph::os::Transaction& txn, osd_op_params_t& osd_op_params,
                                                        object_stat_sum_t& delta_stats,
                                                        ...
                                                        crimson::osd::ObjectContextLoader& obc_loader)
{
    ...
    logger().debug("{} deleting {} and rolling back to old snap {}", __func__, os.oi.soid, snapid);
    hobject_t target_coid = os.oi.soid;
    target_coid.snap = snapid;
    return obc_loader
        .with_clone_obc_only<RWState::RWWRITE>(
            head,
            target_coid,
            [this, &os, &txn, &delta_stats, &osd_op_params](auto resolved_obc) {
                 ...
                 */
                logger().debug("PGBackend::rollback: loaded clone_obc: {}", resolved_obc->obs.oi.soid);
                // 1) Delete current head
                if (os.exists) {
                    txn.remove(coll->get_cid(), ghobject_t{os.oi.soid, ghobject_t::NO_GEN, shard});
                }
                // 2) Clone correct snapshot into head
                txn.clone(coll->get_cid(), ghobject_t{resolved_obc->obs.oi.soid}, ghobject_t{os.oi.soid});
                ...
                osd_op_params.clean_regions.mark_data_region_dirty(0, std::max(os.oi.size, resolved_obc->obs.oi.size));
        ...
        .safe_then_interruptible(
            ...
            crimson::ct_error::enoent::handle([this, &os, &snapid, &txn, &delta_stats] {
...
PGBackend::append_ierrorator::future<> PGBackend::append(ObjectState& os, OSDOp& osd_op, ceph::os::Transaction& txn,
                                                         osd_op_params_t& osd_op_params, object_stat_sum_t& delta_stats)
{
    const ceph_osd_op& op = osd_op.op;
    if (op.extent.length != osd_op.indata.length()) {
    ...
    maybe_create_new_object(os, txn, delta_stats);
    if (op.extent.length) {
        txn.write(coll->get_cid(),
                  ghobject_t{os.oi.soid},
                  os.oi.size /* offset */,
                  op.extent.length,
                  std::move(osd_op.indata),
                  op.flags);
        update_size_and_usage(delta_stats, os.oi, os.oi.size, op.extent.length);
        osd_op_params.clean_regions.mark_data_region_dirty(os.oi.size, op.extent.length);
    }
    return seastar::now();
}

PGBackend::write_iertr::future<> PGBackend::truncate(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                                     osd_op_params_t& osd_op_params, object_stat_sum_t& delta_stats)
{
    if (!os.exists || os.oi.is_whiteout()) {
    ...
    return _truncate(
        os, txn, osd_op_params, delta_stats, op.extent.offset, op.extent.truncate_size, op.extent.truncate_seq);
}

PGBackend::write_iertr::future<> PGBackend::zero(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                                 osd_op_params_t& osd_op_params, object_stat_sum_t& delta_stats)
{
    if (!os.exists || os.oi.is_whiteout()) {
    ...
    txn.zero(coll->get_cid(), ghobject_t{os.oi.soid}, op.extent.offset, op.extent.length);
...
PGBackend::create_iertr::future<> PGBackend::create(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                                    object_stat_sum_t& delta_stats)
{
    if (os.exists && !os.oi.is_whiteout() && (osd_op.op.flags & CEPH_OSD_OP_FLAG_EXCL)) {
    ...
    maybe_create_new_object(os, txn, delta_stats);
    txn.create(coll->get_cid(), ghobject_t{os.oi.soid, ghobject_t::NO_GEN, shard});
    return seastar::now();
}

PGBackend::interruptible_future<> PGBackend::remove(ObjectState& os, ceph::os::Transaction& txn)
{
    // todo: snapset
    txn.remove(coll->get_cid(), ghobject_t{os.oi.soid, ghobject_t::NO_GEN, shard});
    os.oi.size = 0;
...
PGBackend::remove_iertr::future<> PGBackend::remove(ObjectState& os, ceph::os::Transaction& txn,
...
PGBackend::interruptible_future<std::tuple<std::vector<hobject_t>, hobject_t>> PGBackend::list_objects(
    const hobject_t& start, uint64_t limit) const
{
    auto gstart = start.is_min() ? ghobject_t{} : ghobject_t{start, 0, shard};
    return interruptor::make_interruptible(store->list_objects(coll, gstart, ghobject_t::get_max(), limit))
...
PGBackend::setxattr_ierrorator::future<> PGBackend::setxattr(ObjectState& os, const OSDOp& osd_op,
                                                             ceph::os::Transaction& txn, object_stat_sum_t& delta_stats)
{
    if (local_conf()->osd_max_attr_size > 0 && osd_op.op.xattr.value_len > local_conf()->osd_max_attr_size) {
        return crimson::ct_error::file_too_large::make();
    }

    const auto max_name_len =
        std::min<uint64_t>(store->get_max_attr_name_length(), local_conf()->osd_max_attr_name_len);
...
PGBackend::get_attr_ierrorator::future<> PGBackend::getxattr(const ObjectState& os, OSDOp& osd_op,
                                                             object_stat_sum_t& delta_stats) const
{
    ...
    return getxattr(os.oi.soid, std::move(name))
        .safe_then_interruptible([&delta_stats, &osd_op](ceph::bufferlist&& val) {
...
PGBackend::get_attr_ierrorator::future<ceph::bufferlist> PGBackend::getxattr(const hobject_t& soid,
...
{
    return seastar::do_with(key, [this, &soid](auto& key) { return store->get_attr(coll, ghobject_t{soid}, key); });
}

PGBackend::get_attr_ierrorator::future<> PGBackend::get_xattrs(const ObjectState& os, OSDOp& osd_op,
                                                               object_stat_sum_t& delta_stats) const
{
    return store->get_attrs(coll, ghobject_t{os.oi.soid}).safe_then([&delta_stats, &osd_op](auto&& attrs) {
...
static int do_xattr_cmp_u64(int op, uint64_t lhs, bufferlist& rhs_xattr)
{
    ...
    if (rhs_xattr.length() > 0) {
        ...
        if (auto [p, ec] = std::from_chars(first, first + rhs_xattr.length(), rhs); ec != std::errc()) {
            return -EINVAL;
        }
    }
    else {
...
PGBackend::cmp_xattr_ierrorator::future<> PGBackend::cmp_xattr(const ObjectState& os, OSDOp& osd_op,
                                                               object_stat_sum_t& delta_stats) const
{
    std::string name{"_"};
    auto bp = osd_op.indata.cbegin();
    ...
    return getxattr(os.oi.soid, std::move(name))
        .safe_then_interruptible([&delta_stats, &osd_op](auto&& xattr) -> cmp_xattr_ierrorator::future<> {
            delta_stats.num_rd++;
            ...
            default: logger().info("bad cmp mode {}", osd_op.op.xattr.cmp_mode); result = -EINVAL;
            }
            if (result == 0) {
        ...
        .handle_error_interruptible(
            crimson::ct_error::enodata::handle([&delta_stats, &osd_op]() -> cmp_xattr_errorator::future<> {
                delta_stats.num_rd++;
                delta_stats.num_rd_kb += shift_round_up(osd_op.op.xattr.value_len, 10);
                return crimson::ct_error::ecanceled::make();
            }),
...
PGBackend::rm_xattr_iertr::future<> PGBackend::rm_xattr(ObjectState& os, const OSDOp& osd_op,
...
void PGBackend::clone(
    /* const */ object_info_t& snap_oi, const ObjectState& os, const ObjectState& d_os, ceph::os::Transaction& txn)
{
...
using get_omap_ertr = crimson::os::FuturizedStore::Shard::read_errorator::extend<crimson::ct_error::enodata>;
using get_omap_iertr =
    ::crimson::interruptible::interruptible_errorator<::crimson::osd::IOInterruptCondition, get_omap_ertr>;
static get_omap_iertr::future<crimson::os::FuturizedStore::Shard::omap_values_t> maybe_get_omap_vals_by_keys(
    crimson::os::FuturizedStore::Shard* store, const crimson::os::CollectionRef& coll, const object_info_t& oi,
...
static get_omap_iertr::future<std::tuple<bool, crimson::os::FuturizedStore::Shard::omap_values_t>> maybe_get_omap_vals(
    crimson::os::FuturizedStore::Shard* store, const crimson::os::CollectionRef& coll, const object_info_t& oi,
...
PGBackend::ll_read_ierrorator::future<ceph::bufferlist> PGBackend::omap_get_header(const crimson::os::CollectionRef& c,
                                                                                   const ghobject_t& oid) const
{
    return store->omap_get_header(c, oid).handle_error(
        crimson::ct_error::enodata::handle([] { return seastar::make_ready_future<bufferlist>(); }),
        ll_read_errorator::pass_further{});
}

PGBackend::ll_read_ierrorator::future<> PGBackend::omap_get_header(const ObjectState& os, OSDOp& osd_op,
                                                                   object_stat_sum_t& delta_stats) const
{
    if (os.oi.is_omap()) {
        return omap_get_header(coll, ghobject_t{os.oi.soid})
            .safe_then_interruptible([&delta_stats, &osd_op](ceph::bufferlist&& header) {
...
PGBackend::ll_read_ierrorator::future<> PGBackend::omap_get_keys(const ObjectState& os, OSDOp& osd_op,
                                                                 object_stat_sum_t& delta_stats) const
{
    if (!os.exists || os.oi.is_whiteout()) {
    ...
    return maybe_get_omap_vals(store, coll, os.oi, start_after)
        .safe_then_interruptible([=, &delta_stats, &osd_op](auto ret) {
            ceph::bufferlist result;
            bool truncated = false;
            uint32_t num = 0;
            for (auto& [key, val] : std::get<1>(ret)) {
                if (num >= max_return || result.length() >= local_conf()->osd_max_omap_bytes_per_request) {
                    truncated = true;
                    break;
                }
                encode(key, result);
        ...
        .handle_error_interruptible(crimson::ct_error::enodata::handle([&osd_op] {
                                        uint32_t num = 0;
                                        bool truncated = false;
                                        encode(num, osd_op.outdata);
                                        encode(truncated, osd_op.outdata);
...
static PGBackend::omap_cmp_ertr::future<> do_omap_val_cmp(std::map<std::string, bufferlist, std::less<>> out,
                                                          std::map<std::string, std::pair<bufferlist, int>> assertions)
{
    ...
    for (const auto& [akey, avalue] : assertions) {
        ...
        case CEPH_OSD_CMPXATTR_OP_GT:
            ...
            break;
        default: return crimson::ct_error::invarg::make();
        }
    }
    return PGBackend::omap_cmp_ertr::now();
}
PGBackend::omap_cmp_iertr::future<> PGBackend::omap_cmp(const ObjectState& os, OSDOp& osd_op,
                                                        object_stat_sum_t& delta_stats) const
{
    if (!os.exists || os.oi.is_whiteout()) {
        logger().debug("{}: object does not exist: {}", os.oi.soid);
...
PGBackend::ll_read_ierrorator::future<> PGBackend::omap_get_vals(const ObjectState& os, OSDOp& osd_op,
                                                                 object_stat_sum_t& delta_stats) const
{
    if (!os.exists || os.oi.is_whiteout()) {
        logger().debug("{}: object does not exist: {}", os.oi.soid);
    ...
    // TODO: truly chunk the reading
    return maybe_get_omap_vals(store, coll, os.oi, start_after)
        .safe_then_interruptible([=, &osd_op](auto&& ret) {
            auto [done, vals] = std::move(ret);
            assert(done);
            ceph::bufferlist result;
            bool truncated = false;
            uint32_t num = 0;
            auto iter = filter_prefix > start_after ? vals.lower_bound(filter_prefix) : std::begin(vals);
            for (; iter != std::end(vals); ++iter) {
                const auto& [key, value] = *iter;
                if (key.substr(0, filter_prefix.size()) != filter_prefix) {
                    break;
                }
                else if (num >= max_return || result.length() >= local_conf()->osd_max_omap_bytes_per_request) {
                    truncated = true;
                    break;
                }
                encode(key, result);
...
PGBackend::ll_read_ierrorator::future<> PGBackend::omap_get_vals_by_keys(const ObjectState& os, OSDOp& osd_op,
                                                                         object_stat_sum_t& delta_stats) const
{
    if (!os.exists || os.oi.is_whiteout()) {
    ...
    return maybe_get_omap_vals_by_keys(store, coll, os.oi, keys_to_get)
        .safe_then_interruptible([&osd_op](crimson::os::FuturizedStore::Shard::omap_values_t&& vals) {
...
PGBackend::interruptible_future<> PGBackend::omap_set_vals(ObjectState& os, const OSDOp& osd_op,
                                                           ceph::os::Transaction& txn, osd_op_params_t& osd_op_params,
                                                           object_stat_sum_t& delta_stats)
{
    maybe_create_new_object(os, txn, delta_stats);
...
PGBackend::interruptible_future<> PGBackend::omap_set_header(ObjectState& os, const OSDOp& osd_op,
                                                             ceph::os::Transaction& txn, osd_op_params_t& osd_op_params,
                                                             object_stat_sum_t& delta_stats)
{
    maybe_create_new_object(os, txn, delta_stats);
...
PGBackend::interruptible_future<> PGBackend::omap_remove_range(ObjectState& os, const OSDOp& osd_op,
                                                               ceph::os::Transaction& txn,
                                                               object_stat_sum_t& delta_stats)
{
...
PGBackend::interruptible_future<> PGBackend::omap_remove_key(ObjectState& os, const OSDOp& osd_op,
                                                             ceph::os::Transaction& txn)
{
...
PGBackend::omap_clear_iertr::future<> PGBackend::omap_clear(ObjectState& os, OSDOp& osd_op, ceph::os::Transaction& txn,
...
PGBackend::interruptible_future<struct stat> PGBackend::stat(CollectionRef c, const ghobject_t& oid) const
{
    return store->stat(c, oid);
}

PGBackend::read_errorator::future<std::map<uint64_t, uint64_t>> PGBackend::fiemap(CollectionRef c,
                                                                                  const ghobject_t& oid, uint64_t off,
                                                                                  uint64_t len)
{
    return store->fiemap(c, oid, off, len);
}

PGBackend::write_iertr::future<> PGBackend::tmapput(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                                    object_stat_sum_t& delta_stats, osd_op_params_t& osd_op_params)
{
    ...
    if (!ret.has_value()) {
        ...
        return _writefull(os, bl.length(), std::move(bl), txn, osd_op_params, delta_stats, 0);
    }
}

PGBackend::tmapup_iertr::future<> PGBackend::tmapup(ObjectState& os, const OSDOp& osd_op, ceph::os::Transaction& txn,
                                                    object_stat_sum_t& delta_stats, osd_op_params_t& osd_op_params)
{
    logger().debug("PGBackend::tmapup: {}", os.oi.soid);
    return PGBackend::write_iertr::now()
        .si_then([this, &os] { return _read(os.oi.soid, 0, os.oi.size, 0); })
        .handle_error_interruptible(
            crimson::ct_error::enoent::handle([](auto&) { return seastar::make_ready_future<bufferlist>(); }),
            PGBackend::write_iertr::pass_further{},
            crimson::ct_error::assert_all{"read error in mutate_object_contents"})
        .si_then([this, &os, &osd_op, &txn, &delta_stats, &osd_op_params](
                     auto&& bl) mutable -> PGBackend::tmapup_iertr::future<> {
            auto result = crimson::common::do_tmap_up(osd_op.indata.cbegin(), std::move(bl));
            if (!result.has_value()) {
                int ret = result.error();
                logger().debug("PGBackend::tmapup: {}, ret={}", os.oi.soid, ret);
                switch (ret) {
                case -EEXIST: return crimson::ct_error::eexist::make();
                case -ENOENT: return crimson::ct_error::enoent::make();
                case -EINVAL: return crimson::ct_error::invarg::make();
                default: ceph_assert(0 == "impossible error"); return crimson::ct_error::invarg::make();
                }
            }

            logger().debug(
                "PGBackend::tmapup: {}, result.value.length()={}, ret=0", os.oi.soid, result.value().length());
            return _writefull(os, result.value().length(), result.value(), txn, osd_op_params, delta_stats, 0);
        });
}

PGBackend::read_ierrorator::future<> PGBackend::tmapget(const ObjectState& os, OSDOp& osd_op,
                                                        object_stat_sum_t& delta_stats)
{
    ...
    return _read(oi.soid, 0, oi.size, 0)
        .safe_then_interruptible_tuple(
