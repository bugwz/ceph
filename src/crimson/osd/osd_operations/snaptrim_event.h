...
public:
    ...
    using remove_or_update_iertr =
        crimson::interruptible::interruptible_errorator<IOInterruptCondition, remove_or_update_ertr>;
    using snap_trim_ertr = remove_or_update_ertr::extend<crimson::ct_error::eagain>;
    using snap_trim_iertr = remove_or_update_iertr::extend<crimson::ct_error::eagain>;

    static constexpr OperationTypeCode type = OperationTypeCode::snaptrim_event;

    SnapTrimEvent(Ref<PG> pg, SnapMapper& snap_mapper, const snapid_t snapid, const bool needs_pause)
        : pg(std::move(pg))
    ...
    snap_trim_ertr::future<seastar::stop_iteration> with_pg(ShardServices& shard_services, Ref<PG> pg);
...
private:
    ...
    struct SubOpBlocker : crimson::BlockerT<SubOpBlocker>
    {
        ...
        using id_done_t = std::pair<crimson::Operation::id_t, remove_or_update_iertr::future<>>;
...
public:
    ...
    std::tuple<StartEvent, CommonPGPipeline::WaitForActive::BlockingEvent, PGActivationBlocker::BlockingEvent,
               CommonPGPipeline::RecoverMissing::BlockingEvent, CommonPGPipeline::GetOBC::BlockingEvent,
               CommonPGPipeline::Process::BlockingEvent, WaitSubop::BlockingEvent,
               PG::SnapTrimMutex::WaitPG::BlockingEvent, WaitTrimTimer::BlockingEvent, CompletionEvent>
        tracking_events;
...
public:
    ...
    static constexpr OperationTypeCode type = OperationTypeCode::snaptrimobj_subevent;
    ...
    remove_or_update_iertr::future<> with_pg(ShardServices& shard_services, Ref<PG> pg);
...
private:
    ...
    remove_or_update_iertr::future<> remove_clone(ObjectContextRef obc, ObjectContextRef head_obc,
                                                  ceph::os::Transaction& txn, std::vector<pg_log_entry_t>& log_entries);
    void remove_head_whiteout(ObjectContextRef obc, ObjectContextRef head_obc, ceph::os::Transaction& txn,
                              std::vector<pg_log_entry_t>& log_entries);
    interruptible_future<> adjust_snaps(ObjectContextRef obc, ObjectContextRef head_obc,
                                        const std::set<snapid_t>& new_snaps, ceph::os::Transaction& txn,
                                        std::vector<pg_log_entry_t>& log_entries);
    void update_head(ObjectContextRef obc, ObjectContextRef head_obc, ceph::os::Transaction& txn,
                     std::vector<pg_log_entry_t>& log_entries);

    using remove_or_update_ret_t = std::pair<ceph::os::Transaction, std::vector<pg_log_entry_t>>;
    remove_or_update_iertr::future<remove_or_update_ret_t> remove_or_update(ObjectContextRef obc,
...
public:
    ...
    std::tuple<StartEvent, CommonPGPipeline::WaitForActive::BlockingEvent, PGActivationBlocker::BlockingEvent,
               CommonPGPipeline::RecoverMissing::BlockingEvent, CommonPGPipeline::GetOBC::BlockingEvent,
               CommonPGPipeline::Process::BlockingEvent, WaitRepop::BlockingEvent, CompletionEvent>
        tracking_events;
};
