...
template<class T> class PeeringEvent : public PhasedOperationT<T>
{
...
protected:
    ...
    const PGPeeringEvent& get_event() const { return evt; }
    ...
    virtual seastar::future<> complete_rctx_no_pg(ShardServices& shard_services) { return seastar::now(); }

public:
    template<typename... Args>
    PeeringEvent(const pg_shard_t& from, const spg_t& pgid, Args&&... args)
        : from(from)
        , pgid(pgid)
        , evt(std::forward<Args>(args)...)
    {}
    template<typename... Args>
    PeeringEvent(const pg_shard_t& from, const spg_t& pgid, float delay, Args&&... args)
        : from(from)
        , pgid(pgid)
        , delay(delay)
        , evt(std::forward<Args>(args)...)
...
protected:
    ...
    PeeringEvent::interruptible_future<> complete_rctx(ShardServices& shard_services, Ref<PG> pg) override;
    seastar::future<> complete_rctx_no_pg(ShardServices& shard_services) override;
...
public:
    class OSDPipeline
    {
        ...
        {
            static constexpr auto type_name = "PeeringRequest::OSDPipeline::await_active";
        } await_active;
        friend class RemotePeeringEvent;
    };
    ...
    std::tuple<StartEvent, ConnectionPipeline::AwaitActive::BlockingEvent, ConnectionPipeline::AwaitMap::BlockingEvent,
               ...
               PGMap::PGCreationBlockingEvent, PGPeeringPipeline::AwaitMap::BlockingEvent,
               PG_OSDMapGate::OSDMapBlocker::BlockingEvent, PGPeeringPipeline::Process::BlockingEvent,
               OSDPipeline::AwaitActive::BlockingEvent, CompletionEvent>
        tracking_events;
...
public:
    ...
    std::tuple<StartEvent, PGPeeringPipeline::AwaitMap::BlockingEvent, PG_OSDMapGate::OSDMapBlocker::BlockingEvent,
               PGPeeringPipeline::Process::BlockingEvent, CompletionEvent>
        tracking_events;
};
