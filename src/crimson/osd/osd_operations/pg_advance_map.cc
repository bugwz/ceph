...
PGAdvanceMap::PGAdvanceMap(OSD& osd, Ref<PG> pg, epoch_t from, epoch_t to, PeeringCtx&& rctx, bool do_init)
    : osd(osd)
    , pg(pg)
    , from(from)
    , to(to)
...
{
    lhs << "PGAdvanceMap("
        << "pg=" << pg->get_pgid() << " from=" << from << " to=" << to;
    if (do_init) {
        lhs << " do_init";
    }
    lhs << ")";
...
seastar::future<> PGAdvanceMap::start()
{
    ...
    IRef ref = this;
    return with_blocking_future(handle.enter(pg->peering_request_pg_pipeline.process))
        .then([this] {
            if (do_init) {
            ...
            return seastar::do_for_each(boost::make_counting_iterator(from + 1),
                                        ...
                                        [this](epoch_t next_epoch) {
                                            return osd.get_map(next_epoch).then([this](cached_map_t&& next_map) {
                ...
                .then([this] {
                    ...
                    return seastar::when_all_succeed(
                        pg->get_need_up_thru() ? osd.shard_services.send_alive(pg->get_same_interval_since())
                                               : seastar::now(),
                        osd.shard_services.dispatch_context(pg->get_collection_ref(), std::move(rctx)));
                })
                .then_unpack([this] { return osd.shard_services.send_pg_temp(); });
        })
