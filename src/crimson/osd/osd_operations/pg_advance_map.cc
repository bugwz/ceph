...
PGAdvanceMap::PGAdvanceMap(ShardServices& shard_services, Ref<PG> pg, epoch_t to, PeeringCtx&& rctx, bool do_init)
...
{
    lhs << "PGAdvanceMap("
        << "pg=" << pg->get_pgid() << " from=" << (from ? *from : -1) << " to=" << to;
    if (do_init) {
        lhs << " do_init";
    }
    lhs << ")";
...
seastar::future<> PGAdvanceMap::start()
{
    ...
    IRef ref = this;
    return enter_stage<>(peering_pp(*pg).process)
        .then([this] {
            /*
            ...
            if (do_init) {
                fut = pg->handle_initialize(rctx).then([this] { return pg->handle_activate_map(rctx); });
            }
            return fut.then([this] {
                ...
                return seastar::do_for_each(
                           ...
                           [this](epoch_t next_epoch) {
                               logger().debug("{}: start: getting map {}", *this, next_epoch);
                               return shard_services.get_map(next_epoch).then([this](cached_map_t&& next_map) {
                                   logger().debug("{}: advancing map to {}", *this, next_map->get_epoch());
                    ...
                    .then([this] {
                        return pg->handle_activate_map(rctx).then([this] {
                            ...
                            return seastar::when_all_succeed(
                                pg->get_need_up_thru() ? shard_services.send_alive(pg->get_same_interval_since())
                                                       : seastar::now(),
                                shard_services.dispatch_context(pg->get_collection_ref(), std::move(rctx)));
                        });
                    })
                    .then_unpack([this] {
