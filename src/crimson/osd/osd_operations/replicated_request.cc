...
RepRequest::RepRequest(OSD& osd, crimson::net::ConnectionRef&& conn, Ref<MOSDRepOp>&& req)
    : osd{osd}
    , conn{std::move(conn)}
    , req{req}
{}
...
seastar::future<> RepRequest::start()
{
    ...
    return with_blocking_future(handle.enter(cp().await_map))
        .then([this]() { return with_blocking_future(osd.osdmap_gate.wait_for_map(req->get_min_epoch())); })
        .then([this](epoch_t epoch) { return with_blocking_future(handle.enter(cp().get_pg)); })
        .then([this] { return with_blocking_future(osd.wait_for_pg(req->get_spg())); })
        .then([this, ref = std::move(ref)](Ref<PG> pg) { return pg->handle_rep_op(std::move(req)); });
}
}   // namespace crimson::osd
