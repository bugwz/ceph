...
RepRequest::RepRequest(crimson::net::ConnectionRef&& conn, Ref<MOSDRepOp>&& req)
    : conn{std::move(conn)}
    , req{std::move(req)}
{}
...
seastar::future<> RepRequest::with_pg(ShardServices& shard_services, Ref<PG> pg)
{
    ...
    return interruptor::with_interruption(
        [this, pg] {
            ...
            return this->template enter_stage<interruptor>(client_pp(*pg).await_map)
                ...
                .then_interruptible([this, pg](auto) { return pg->handle_rep_op(req); });
        },
