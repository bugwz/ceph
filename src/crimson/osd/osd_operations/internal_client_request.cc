...
namespace crimson {
template<> struct EventBackendRegistry<osd::InternalClientRequest>
{
...
seastar::future<> InternalClientRequest::start()
{
    ...
    return crimson::common::handle_system_shutdown([this] {
        return seastar::repeat([this] {
                   ...
                   return interruptor::with_interruption(
                       [this]() mutable {
                           return enter_stage<interruptor>(client_pp().wait_for_active)
                               ...
                               .then_interruptible(
                                   [this] { return enter_stage<interruptor>(client_pp().recover_missing); })
                               .then_interruptible([this] { return do_recover_missing(pg, get_target_oid()); })
                               ...
                               .then_interruptible([this]() -> PG::load_obc_iertr::future<> {
                                   ...
                                   return seastar::do_with(create_osd_ops(), [this](auto& osd_ops) mutable {
                                       ...
                                       return pg->with_locked_obc(
                                           get_target_oid(), op_info, [&osd_ops, this](auto obc) {
                                               return enter_stage<interruptor>(client_pp().process)
                                                   .then_interruptible([obc = std::move(obc), &osd_ops, this] {
                                                       return pg
                                                           ->do_osd_ops(
                                                               std::move(obc),
                                                               osd_ops,
                               ...
                               .handle_error_interruptible(
                                   PG::load_obc_ertr::all_same_way([] { return seastar::now(); }))
                               .then_interruptible([] { return seastar::stop_iteration::yes; });
                       },
            ...
            .then([this] { track_event<CompletionEvent>(); });
    });
}
