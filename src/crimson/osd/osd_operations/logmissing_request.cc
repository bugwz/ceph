...
LogMissingRequest::LogMissingRequest(crimson::net::ConnectionRef&& conn, Ref<MOSDPGUpdateLogMissing>&& req)
    : conn{std::move(conn)}
    , req{std::move(req)}
{}
...
seastar::future<> LogMissingRequest::with_pg(ShardServices& shard_services, Ref<PG> pg)
{
                ...
                .then_interruptible([this, pg] {
                    return this->template with_blocking_event<PG_OSDMapGate::OSDMapBlocker::BlockingEvent>(
                        [this, pg](auto&& trigger) {
                            return pg->osdmap_gate.wait_for_map(std::move(trigger), req->min_epoch);
                        });
                })
                .then_interruptible([this, pg](auto) { return pg->do_update_log_missing(req, conn); });
        },
        [ref](std::exception_ptr) { return seastar::now(); },
        pg);
}
