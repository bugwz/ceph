...
LogMissingRequestReply::LogMissingRequestReply(crimson::net::ConnectionRef&& conn,
...
seastar::future<> LogMissingRequestReply::with_pg(ShardServices& shard_services, Ref<PG> pg)
{
    logger().debug("{}: LogMissingRequestReply::with_pg", *this);

    IRef ref = this;
    return interruptor::with_interruption([this, pg] { return pg->do_update_log_missing_reply(std::move(req)); },
