...
template<class T> class BackgroundRecoveryT : public PhasedOperationT<T>
{
public:
    static constexpr OperationTypeCode type = OperationTypeCode::background_recovery;

    BackgroundRecoveryT(Ref<PG> pg, ShardServices& ss, epoch_t epoch_started,
...
{
public:
    UrgentRecovery(const hobject_t& soid, const eversion_t& need, Ref<PG> pg, ShardServices& ss, epoch_t epoch_started);
    void print(std::ostream&) const final;

    std::tuple<OperationThrottler::BlockingEvent, RecoveryBackend::RecoveryBlockingEvent> tracking_events;
...
{
public:
    PglogBasedRecovery(Ref<PG> pg, ShardServices& ss, epoch_t epoch_started, float delay = 0);
...
{
public:
    template<class EventT> BackfillRecovery(Ref<PG> pg, ShardServices& ss, epoch_t epoch_started, const EventT& evt);

    PipelineHandle& get_handle() { return handle; }

    std::tuple<OperationThrottler::BlockingEvent, PGPeeringPipeline::Process::BlockingEvent> tracking_events;
...
BackfillRecovery::BackfillRecovery(Ref<PG> pg, ShardServices& ss, const epoch_t epoch_started, const EventT& evt)
