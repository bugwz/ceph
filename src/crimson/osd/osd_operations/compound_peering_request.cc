...
struct compound_state
{
    ...
    ~compound_state() { promise.set_value(std::move(ctx)); }
};
...
public:
    ...
    PeeringSubEvent(compound_state_ref state, Args&&... args)
        : RemotePeeringEvent(std::forward<Args>(args)...)
        , state(state)
    {}
    ...
    seastar::future<> complete_rctx(Ref<crimson::osd::PG> pg) final
    {
        ...
        if (!pg) {
            ...
            return osd.get_shard_services().dispatch_context_transaction(pg->get_collection_ref(), ctx);
        }
    }
};

std::vector<OperationRef> handle_pg_create(OSD& osd, crimson::net::ConnectionRef conn, compound_state_ref state,
                                           Ref<MOSDPGCreate2> m)
{
    ...
    for (auto& [pgid, when] : m->pgs) {
        ...
        if (!pi.empty() && m->epoch < pi.get_bounds().second) {
            ...
            auto op = osd.get_shard_services()
                          .start_operation<PeeringSubEvent>(state,
                                                            osd,
                                                            conn,
...
CompoundPeeringRequest::CompoundPeeringRequest(OSD& osd, crimson::net::ConnectionRef conn, Ref<Message> m)
    : osd(osd)
    , conn(conn)
    , m(m)
{}
...
seastar::future<> CompoundPeeringRequest::start()
{
    ...
    auto blocker = std::make_unique<SubOpBlocker>([&] {
        ...
        return handle_pg_create(osd, conn, state, boost::static_pointer_cast<MOSDPGCreate2>(m));
    }());

    IRef ref = this;
    logger().info("{}: about to fork future", *this);
    return crimson::common::handle_system_shutdown([this, ref, blocker = std::move(blocker), state]() mutable {
        return with_blocking_future(blocker->make_blocking_future(state->promise.get_future()))
            ...
            .then([this, ref = std::move(ref)] { logger().info("{}: complete", *this); });
    });
}
