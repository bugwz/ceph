               ...
               .then([this](Ref<PG> pg) {
                   if (!pg) {
                       ...
                       return with_blocking_future(handle.enter(pp(*pg).await_map))
                           .then([this, pg] {
                               return with_blocking_future(pg->osdmap_gate.wait_for_map(evt.get_epoch_sent()));
                           })
                           .then([this, pg](auto) { return with_blocking_future(handle.enter(pp(*pg).process)); })
                           .then([this, pg] {
                               // TODO: likely we should synchronize also with the pg log-based
                               // recovery.
                               return with_blocking_future(handle.enter(BackfillRecovery::bp(*pg).process));
                           })
                           .then([this, pg] {
...
void RemotePeeringEvent::on_pg_absent()
{
    if (auto& e = get_event().get_event(); e.dynamic_type() == MQuery::static_type()) {
        ...
        if (q.query.type == q.query.LOG || q.query.type == q.query.FULLLOG) {
            auto m = ceph::make_message<MOSDPGLog>(q.query.from, q.query.to, map_epoch, empty, q.query.epoch_sent);
...
seastar::future<Ref<PG>> RemotePeeringEvent::get_pg()
{
    return with_blocking_future(handle.enter(cp().await_map))
        .then([this] { return with_blocking_future(osd.osdmap_gate.wait_for_map(evt.get_epoch_sent())); })
        .then([this](auto epoch) {
            logger().debug("{}: got map {}", *this, epoch);
            return with_blocking_future(handle.enter(cp().get_pg));
        })
        .then([this] {
            return with_blocking_future(osd.get_or_create_pg(pgid, evt.get_epoch_sent(), std::move(evt.create_info)));
        });
}
