...
BackgroundRecovery::BackgroundRecovery(Ref<PG> pg, ShardServices& ss, epoch_t epoch_started,
...
seastar::future<> BackgroundRecovery::start()
{
    ...
    IRef ref = this;
    return ss.throttler.with_throttle_while(this, get_scheduler_params(), [this] { return do_recovery(); })
...
seastar::future<bool> UrgentRecovery::do_recovery()
{
    if (!pg->has_reset_since(epoch_started)) {
        return with_blocking_future(pg->get_recovery_handler()->recover_missing(soid, need)).then([] {
...
PglogBasedRecovery::PglogBasedRecovery(Ref<PG> pg, ShardServices& ss, const epoch_t epoch_started)
    : BackgroundRecovery(std::move(pg), ss, epoch_started,
                         crimson::osd::scheduler::scheduler_class_t::background_recovery)
{}

seastar::future<bool> PglogBasedRecovery::do_recovery()
{
    if (pg->has_reset_since(epoch_started)) return seastar::make_ready_future<bool>(false);
    return with_blocking_future(
        pg->get_recovery_handler()->start_recovery_ops(crimson::common::local_conf()->osd_recovery_max_single_start));
}
