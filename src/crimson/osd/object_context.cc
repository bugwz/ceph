...
{
    logger().debug("{} oid.snap={},head snapset.seq={}", __func__, oid.snap, ss.seq);
    if (oid.snap > ss.seq) {
        ...
        if (std::find(citer->second.begin(), citer->second.end(), oid.snap) == citer->second.end()) {
            logger().debug("{} {} does not contain {} -- DNE", __func__, ss.clone_snaps, oid.snap);
            return std::nullopt;
        }
        else {
            auto soid = oid;
