...
seastar::future<> populate_config_from_mon()
{
    ...
    {
    public:
        void handle_authentication(const EntityName& name, const AuthCapsInfo& caps) {}
    };
    return seastar::async([] {
        auto auth_handler = std::make_unique<DummyAuthHandler>();
        auto msgr = crimson::net::Messenger::create(entity_name_t::CLIENT(), "temp_mon_client", get_nonce(), true);
...
static tl::expected<early_config_t, int> _get_early_config(int argc, const char* argv[])
{
    early_config_t ret;

    // pull off ceph configs the stuff from early_args
    std::vector<const char*> early_args;
    early_args.insert(std::end(early_args), argv, argv + argc);

    ret.init_params =
        ceph_argparse_early_args(early_args, CEPH_ENTITY_TYPE_OSD, &ret.cluster_name, &ret.conf_file_list);
    ...
    int r = app.run(
        ...
        [argc, argv, &ret, &early_args] {
            return seastar::async([argc, argv, &ret, &early_args] {
                ...
                sharded_conf().start(ret.init_params.name, ret.cluster_name).get();
                ...
                // get ceph configs
                std::set_difference(
                    argv, argv + argc, std::begin(early_args), std::end(early_args), std::back_inserter(ret.ceph_args));

                ret.early_args.insert(std::end(ret.early_args), std::begin(early_args), std::end(early_args));

                if (auto found = std::find_if(
                        std::begin(early_args), std::end(early_args), [](auto* arg) { return "--smp"sv == arg; });
                    found == std::end(early_args)) {

                    // Set --smp based on crimson_seastar_smp config option
                    ret.early_args.emplace_back("--smp");

                    auto smp_config = local_conf().get_val<uint64_t>("crimson_seastar_smp");
 ...
 */
tl::expected<early_config_t, int> get_early_config(int argc, const char* argv[])
{
    int pipes[2];
    int r = pipe2(pipes, 0);
    if (r < 0) {
        std::cerr << "get_early_config: failed to create pipes: " << -errno << std::endl;
    ...
    if (worker < 0) {
        close(pipes[0]);
        close(pipes[1]);
        std::cerr << "get_early_config: failed to fork: " << -errno << std::endl;
        return tl::unexpected(-errno);
    }
        ...
        if (ret.has_value()) {
            ...
            close(pipes[1]);
            if (r < 0) {
                std::cerr << "get_early_config: child failed to write_fd: " << r << std::endl;
                exit(-r);
            }
            else {
                exit(0);
            ...
            std::cerr << "get_early_config: child failed: " << -ret.error() << std::endl;
            exit(-ret.error());
        }
        ...
        if (r < 0) {
            std::cerr << "get_early_config: parent failed to read from pipe: " << r << std::endl;
            return tl::unexpected(r);
        }
        try {
