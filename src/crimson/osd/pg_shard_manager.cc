...
seastar::future<> PGShardManager::load_pgs(crimson::os::FuturizedStore& store)
{
    ...
    return store.list_collections().then([this](auto colls_cores) {
        return seastar::parallel_for_each(colls_cores, [this](auto coll_core) {
            auto [coll, shard_core] = coll_core;
            spg_t pgid;
            if (coll.is_pg(&pgid)) {
                return get_pg_to_shard_mapping().maybe_create_pg(pgid, shard_core).then([this, pgid](auto core) {
                    return this->template with_remote_shard_state(
                        core, [pgid](PerShardState& per_shard_state, ShardServices& shard_services) {
                            return shard_services.load_pg(pgid).then([pgid, &per_shard_state](auto&& pg) {
                ...
                logger().warn("found temp collection on crimson osd, should be impossible: {}", coll);
...
{
    ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
    return shard_services.invoke_on_all([](auto& local_service) { return local_service.local_state.stop_pgs(); });
}

seastar::future<std::map<pg_t, pg_stat_t>> PGShardManager::get_pg_stats() const
{
    ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
    return shard_services.map_reduce0([](auto& local) { return local.local_state.get_pg_stats(); },
...
seastar::future<> PGShardManager::broadcast_map_to_pgs(epoch_t epoch)
{
    ...
    return shard_services
        .invoke_on_all([epoch](auto& local_service) {
            return local_service.local_state.broadcast_map_to_pgs(local_service, epoch);
        })
        .then([this, epoch] {
...
{
    ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
    return shard_services.invoke_on_all(seastar::smp_submit_to_options{}, [e](auto& local_service) {
