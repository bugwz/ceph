...
struct PeeringFacade final : BackfillState::PeeringFacade
{
    ...
    hobject_t earliest_backfill() const override { return peering_state.earliest_backfill(); }

    const std::set<pg_shard_t>& get_backfill_targets() const override { return peering_state.get_backfill_targets(); }
    ...
    const eversion_t& get_last_update() const override { return peering_state.get_info().last_update; }

    const eversion_t& get_log_tail() const override { return peering_state.get_info().log_tail; }
    ...
    bool is_backfill_target(pg_shard_t peer) const override { return peering_state.is_backfill_target(peer); }
    ...
    bool is_backfilling() const override { return peering_state.is_backfilling(); }
...
struct PGFacade final : BackfillState::PGFacade
{
    ...
    const eversion_t& get_projected_last_update() const override { return pg.projected_last_update; }
