...
template<class OpT> class OperationProxyT;
}
...
struct ConnectionPipeline
{
    ...
    {
        static constexpr auto type_name = "ConnectionPipeline::get_pg";
    } get_pg;
};
...
static_assert((sizeof(OP_NAMES) / sizeof(OP_NAMES[0])) == static_cast<int>(OperationTypeCode::last_op));
...
template<typename T> struct OperationT : InterruptibleOperation
{
    ...
    unsigned get_type() const final { return static_cast<unsigned>(T::type); }

    const char* get_type_name() const final { return T::type_name; }
...
template<class T> class TrackableOperationT : public OperationT<T>
{
    T* that() { return static_cast<T*>(this); }
    const T* that() const { return static_cast<const T*>(this); }

protected:
    template<class EventT> decltype(auto) get_event()
    {
    ...
    template<class EventT> decltype(auto) get_event() const { return std::get<EventT>(that()->tracking_events); }
    ...
    template<class EventT, class... Args> void track_event(Args&&... args)
    {
    ...
    template<class BlockingEventT, class InterruptorT = void, class F> auto with_blocking_event(F&& f)
    {
        auto ret =
            std::forward<F>(f)(typename BlockingEventT::template Trigger<T>{get_event<BlockingEventT>(), *that()});
...
template<class T> class PhasedOperationT : public TrackableOperationT<T>
{
...
protected:
    ...
    template<class InterruptorT = void, class StageT> auto enter_stage(StageT& stage)
    {
    ...
    template<class OpT> friend class crimson::os::seastore::OperationProxyT;
 ...
 */
struct OSDOperationRegistry : OperationRegistryT<static_cast<size_t>(OperationTypeCode::last_op)>
{
 ...
 * concurrently active.
 */
class OperationThrottler : public BlockerT<OperationThrottler>, private md_config_obs_t
{
    ...
    auto with_throttle(OperationT* op, crimson::osd::scheduler::params_t params, F&& f)
    {
        if (!max_in_progress) return f();
        return acquire_throttle(params).then(std::forward<F>(f)).then([this](auto x) {
            release_throttle();
            return x;
        });
    }
    ...
    template<typename OperationT, typename F>
    seastar::future<> with_throttle_while(OperationT* op, crimson::osd::scheduler::params_t params, F&& f)
    {
...
public:
    ...
    seastar::future<> with_throttle_while(BlockingEvent::Trigger<OpT>&& trigger, Args&&... args)
    {
        return trigger.maybe_record_blocking(with_throttle_while(std::forward<Args>(args)...), *this);
    }
