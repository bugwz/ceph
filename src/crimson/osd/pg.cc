...
{
    template<typename FormatContext> auto format(const std::optional<T>& v, FormatContext& ctx) const
    {
        if (v.has_value()) {
...
{
public:
    bool operator()(const set<pg_shard_t>& have) const override { return !have.empty(); }
};
...
public:
    explicit ReadablePredicate(pg_shard_t whoami)
        : whoami(whoami)
    {}
    bool operator()(const set<pg_shard_t>& have) const override { return have.count(whoami); }
};

PG::PG(spg_t pgid, pg_shard_t pg_shard, crimson::os::CollectionRef coll_ref, pg_pool_t&& pool, std::string&& name,
       cached_map_t osdmap, ShardServices& shard_services, ec_profile_t profile)
    : pgid{pgid}
    , pg_whoami{pg_shard}
    , coll_ref{coll_ref}
    ...
    , backend(PGBackend::create(pgid.pgid, pg_shard, pool, coll_ref, shard_services, profile, *this))
    , recovery_backend(std::make_unique<ReplicatedRecoveryBackend>(*this, shard_services, coll_ref, backend.get()))
    , recovery_handler(std::make_unique<PGRecovery>(this))
    , peering_state(shard_services.get_cct(), pg_shard, pgid, PGPool(osdmap, pgid.pool(), pool, name), osdmap, this,
                    this)
    , obc_registry{local_conf()}
    , obc_loader{obc_registry, *backend.get(), *this}
    , osdriver(&shard_services.get_store(), coll_ref, pgid.make_pgmeta_oid())
    , snap_mapper(this->shard_services.get_cct(), &osdriver, pgid.ps(), pgid.get_split_bits(pool.get_pg_num()),
                  pgid.pool(), pgid.shard)
    , wait_for_active_blocker(this)
{
    peering_state.set_backend_predicates(new ReadablePredicate(pg_whoami), new RecoverablePredicate());
...
void PG::check_blocklisted_watchers()
{
    ...
    obc_registry.for_each([this](ObjectContextRef obc) {
        ...
        for (const auto& [key, watch] : obc->watchers) {
            ...
            logger().debug(
                "watch: Found {} cookie {}. Checking entity_add_t {}", watch->get_entity(), watch->get_cookie(), ea);
...
bool PG::try_flush_or_schedule_async()
{
    ...
    (void)shard_services.get_store().flush(coll_ref).then([this, epoch = get_osdmap_epoch()]() {
        return shard_services.start_operation<LocalPeeringEvent>(
            this, pg_whoami, pgid, epoch, epoch, PeeringState::IntervalFlush());
    });
    return false;
}
...
void PG::publish_stats_to_osd()
{
    ...
    if (auto new_pg_stats = peering_state.prepare_stats_for_publish(pg_stats, object_stat_collection_t());
...
void PG::queue_check_readable(epoch_t last_peering_reset, ceph::timespan delay)
{
    ...
    logger().debug("{}: PG::queue_check_readable lpr: {}, delay: {}", *this, last_peering_reset, delay);
    check_readable_timer.cancel();
    check_readable_timer.set_callback([last_peering_reset, this] {
        logger().debug("{}: PG::queue_check_readable callback lpr: {}", *this, last_peering_reset);
        (void)shard_services.start_operation<LocalPeeringEvent>(
            this, pg_whoami, pgid, last_peering_reset, last_peering_reset, PeeringState::CheckReadable{});
    });
    check_readable_timer.arm(std::chrono::duration_cast<seastar::lowres_clock::duration>(delay));
}
...
void PG::recheck_readable()
{
    ...
    if (peering_state.state_test(PG_STATE_WAIT)) {
        ...
        if (mnow < prior_readable_until_ub) {
            ...
            queue_check_readable(peering_state.get_last_peering_reset(), prior_readable_until_ub - mnow);
        }
        else {
    ...
    if (peering_state.state_test(PG_STATE_LAGGY)) {
        ...
        if (readable_until == readable_until.zero()) {
            logger().info("{}:{} still laggy (mnow {}, readable_until zero)", *this, __func__, mnow);
        }
        else if (mnow >= readable_until) {
            logger().info("{}:{} still laggy (mnow {} >= readable_until {})", *this, __func__, mnow, readable_until);
        }
        else {
            logger().info("{}:{} no longer laggy (mnow {} < readable_until {})", *this, __func__, mnow, readable_until);
    ...
    if (changed) {
        ...
        if (!peering_state.state_test(PG_STATE_WAIT) && !peering_state.state_test(PG_STATE_LAGGY)) {
...
unsigned PG::get_target_pg_log_entries() const
{
    ...
    const unsigned local_target =
        local_conf().get_val<uint64_t>("osd_target_pg_log_entries_per_osd") / seastar::smp::count;
    const unsigned min_pg_log_entries = local_conf().get_val<uint64_t>("osd_min_pg_log_entries");
    if (local_num_pgs > 0 && local_target > 0) {
        ...
        // will work out.
        const unsigned max_pg_log_entries = local_conf().get_val<uint64_t>("osd_max_pg_log_entries");
        return std::clamp(local_target / local_num_pgs, min_pg_log_entries, max_pg_log_entries);
    }
    else {
...
{
    t.register_on_commit(new LambdaContext([this](int r) {
        ceph_assert(r == 0);
        (void)shard_services.start_operation<LocalPeeringEvent>(
            this, pg_whoami, pgid, float(0.001), get_osdmap_epoch(), get_osdmap_epoch(), PeeringState::DeleteSome());
    }));
}
...
void PG::on_activate_complete()
{
    ...
    if (peering_state.needs_recovery()) {
        logger().info("{}: requesting recovery", __func__);
        (void)shard_services.start_operation<LocalPeeringEvent>(
            this, pg_whoami, pgid, float(0.001), get_osdmap_epoch(), get_osdmap_epoch(), PeeringState::DoRecovery{});
    }
    else if (peering_state.needs_backfill()) {
        logger().info("{}: requesting backfill", __func__);
        (void)shard_services.start_operation<LocalPeeringEvent>(this,
                                                                pg_whoami,
                                                                pgid,
                                                                float(0.001),
                                                                get_osdmap_epoch(),
        ...
        logger().debug("{}: no need to recover or backfill, AllReplicasRecovered", " for pg: {}", __func__, pgid);
        (void)shard_services.start_operation<LocalPeeringEvent>(this,
                                                                pg_whoami,
                                                                pgid,
                                                                float(0.001),
...
void PG::prepare_write(pg_info_t& info, pg_info_t& last_written_info, PastIntervals& past_intervals, PGLog& pglog,
                       bool dirty_info, bool dirty_big_info, bool need_write_epoch, ceph::os::Transaction& t)
{
    std::map<string, bufferlist> km;
    std::string key_to_remove;
    if (dirty_big_info || dirty_info) {
        int ret = prepare_info_keymap(shard_services.get_cct(),
                                      &km,
                                      &key_to_remove,
                                      get_osdmap_epoch(),
                                      info,
    ...
    pglog.write_log_and_missing(
        t, &km, coll_ref->get_cid(), pgmeta_oid, peering_state.get_pgpool().info.require_rollback());
    if (!km.empty()) {
...
std::pair<ghobject_t, bool> PG::do_delete_work(ceph::os::Transaction& t, ghobject_t _next)
{
    logger().info("removing pg {}", pgid);
    auto fut = interruptor::make_interruptible(shard_services.get_store().list_objects(
        coll_ref, _next, ghobject_t::get_max(), local_conf()->osd_target_transaction_size));
    ...
    if (objs_to_rm.empty()) {
        ...
        (void)shard_services.get_store().do_transaction(coll_ref, std::move(t)).then([this] {
...
void PG::on_active_actmap()
{
    ...
    std::ignore =
        seastar::do_until(
            [this] { return snap_trimq.empty() || peering_state.state_test(PG_STATE_SNAPTRIM_ERROR); },
            [this] {
                ...
                return seastar::repeat([to_trim, needs_pause, this] {
                           logger().debug("{}: going to start SnapTrimEvent, to_trim={}", *this, to_trim);
                           return shard_services.start_operation<SnapTrimEvent>(this, snap_mapper, to_trim, needs_pause)
                               .second.handle_error(crimson::ct_error::enoent::handle([this] {
                    ...
                    .then([this, trimmed = to_trim] { logger().debug("{}: trimmed snap={}", *this, trimmed); });
            })
            .finally([this] {
                logger().debug("{}: PG::on_active_actmap() finished trimming", *this);
...
void PG::on_active_advmap(const OSDMapRef& osdmap)
{
    ...
    if (auto it = new_removed_snaps.find(get_pgid().pool()); it != new_removed_snaps.end()) {
        bool bad = false;
        for (auto j : it->second) {
            if (snap_trimq.intersects(j.first, j.second)) {
                ...
                logger().error("{}: {} removed_snaps already contains {}", *this, __func__, overlap);
                bad = true;
        ...
        logger().info("{}: {} new removed snaps {}, snap_trimq now{}", *this, __func__, it->second, snap_trimq);
...
{
    logger().info("Exiting state: {}, entered at {}, {} spent on {} events", state_name, enter_time, event_dur, events);
}
...
void PG::schedule_renew_lease(epoch_t last_peering_reset, ceph::timespan delay)
{
    ...
    renew_lease_timer.set_callback([last_peering_reset, this] {
        (void)shard_services.start_operation<LocalPeeringEvent>(
            this, pg_whoami, pgid, last_peering_reset, last_peering_reset, RenewLease{});
    });
    renew_lease_timer.arm(std::chrono::duration_cast<seastar::lowres_clock::duration>(delay));
}
...
{
    peering_state.init(role, newup, new_up_primary, newacting, new_acting_primary, history, pi, t);
}

seastar::future<> PG::read_state(crimson::os::FuturizedStore::Shard* store)
{
    if (__builtin_expect(stopping, false)) {
        return seastar::make_exception_future<>(crimson::common::system_shutdown_exception());
    }

    return seastar::do_with(PGMeta(*store, pgid), [](auto& pg_meta) { return pg_meta.load(); })
        .then([this, store](auto&& ret) {
            auto [pg_info, past_intervals] = std::move(ret);
            return peering_state.init_from_disk_state(
                std::move(pg_info), std::move(past_intervals), [this, store](PGLog& pglog) {
                    return pglog.read_log_and_missing_crimson(*store, coll_ref, peering_state.get_info(), pgmeta_oid);
                });
        })
        .then([this]() {
            int primary, up_primary;
            vector<int> acting, up;
            peering_state.get_osdmap()->pg_to_up_acting_osds(pgid.pgid, &up, &up_primary, &acting, &primary);
            peering_state.init_primary_up_acting(up, acting, up_primary, primary);
            int rr = OSDMap::calc_pg_role(pg_whoami, acting);
            peering_state.set_role(rr);

            epoch_t epoch = get_osdmap_epoch();
            (void)shard_services.start_operation<LocalPeeringEvent>(
                this, pg_whoami, pgid, epoch, epoch, PeeringState::Initialize());

            return seastar::now();
        });
}

PG::interruptible_future<> PG::do_peering_event(PGPeeringEvent& evt, PeeringCtx& rctx)
{
    if (peering_state.pg_has_reset_since(evt.get_epoch_requested()) ||
        ...
        return interruptor::async([this, &evt, &rctx] {
            peering_state.handle_event(evt.get_event(), &rctx);
            peering_state.write_if_dirty(rctx.transaction);
        });
    }
}

seastar::future<> PG::handle_advance_map(cached_map_t next_map, PeeringCtx& rctx)
{
    return seastar::async([this, next_map = std::move(next_map), &rctx] {
        vector<int> newup, newacting;
        int up_primary, acting_primary;
        next_map->pg_to_up_acting_osds(pgid.pgid, &newup, &up_primary, &newacting, &acting_primary);
        peering_state.advance_map(
            next_map, peering_state.get_osdmap(), newup, up_primary, newacting, acting_primary, rctx);
...
{
    return seastar::async([this, &rctx] { peering_state.activate_map(rctx); });
}

seastar::future<> PG::handle_initialize(PeeringCtx& rctx)
{
    return seastar::async([this, &rctx] { peering_state.handle_event(PeeringState::Initialize{}, &rctx); });
}
...
std::tuple<PG::interruptible_future<>, PG::interruptible_future<>> PG::submit_transaction(
    ObjectContextRef&& obc, ceph::os::Transaction&& txn, osd_op_params_t&& osd_op_p,
    std::vector<pg_log_entry_t>&& log_entries)
{
    if (__builtin_expect(stopping, false)) {
        return {seastar::make_exception_future<>(crimson::common::system_shutdown_exception()), seastar::now()};
    }
    ...
    peering_state.append_log_with_trim_to_updated(std::move(log_entries), osd_op_p.at_version, txn, true, false);

    auto [submitted, all_completed] = backend->mutate_object(peering_state.get_acting_recovery_backfill(),
                                                             std::move(obc),
                                                             std::move(txn),
                                                             std::move(osd_op_p),
    ...
    return std::make_tuple(std::move(submitted),
                           all_completed.then_interruptible([this,
                                                             last_complete = peering_state.get_info().last_complete,
                                                             at_version = osd_op_p.at_version](auto acked) {
                               for (const auto& peer : acked) {
                                   peering_state.update_peer_last_complete_ondisk(peer.shard,
                                                                                  peer.last_complete_ondisk);
                               }
...
PG::interruptible_future<> PG::repair_object(const hobject_t& oid, eversion_t& v)
{
    ...
    auto [op, fut] =
        get_shard_services().start_operation<UrgentRecovery>(oid, v, this, get_shard_services(), get_osdmap_epoch());
    return std::move(fut);
}

template<class Ret, class SuccessFunc, class FailureFunc>
PG::do_osd_ops_iertr::future<PG::pg_rep_op_fut_t<Ret>> PG::do_osd_ops_execute(seastar::lw_shared_ptr<OpsExecuter> ox,
                                                                              ...
                                                                              FailureFunc&& failure_func)
{
    ...
    return interruptor::do_for_each(ops,
                                    [ox](OSDOp& osd_op) {
                                        logger().debug("do_osd_ops_execute: object {} - handling op {}",
                                                       ox->get_target(),
                                                       ceph_osd_op_name(osd_op.op.op));
                                        return ox->execute_op(osd_op);
                                    })
        .safe_then_interruptible([this, ox, &ops] {
            logger().debug("do_osd_ops_execute: object {} all operations successful", ox->get_target());
            // check for full
            if ((ox->delta_stats.num_bytes > 0 || ox->delta_stats.num_objects > 0) &&
                ...
                if (m.get_reqid().name.is_mds() ||   // FIXME: ignore MDS for now
                    ...
                    if (get_pgpool().info.has_flag(pg_pool_t::FLAG_FULL_QUOTA))
                        return interruptor::make_ready_future<OpsExecuter::rep_op_fut_tuple>(
                            seastar::now(),
                            OpsExecuter::osd_op_ierrorator::future<>(crimson::ct_error::edquot::make()));
                    else
                        return interruptor::make_ready_future<OpsExecuter::rep_op_fut_tuple>(
                            seastar::now(),
                            OpsExecuter::osd_op_ierrorator::future<>(crimson::ct_error::enospc::make()));
                }
                else {
                    // drop request
                    logger().info(" full, dropping request (bad client)");
                    return interruptor::make_ready_future<OpsExecuter::rep_op_fut_tuple>(
                        seastar::now(), OpsExecuter::osd_op_ierrorator::future<>(crimson::ct_error::eagain::make()));
                }
            }
            return std::move(*ox).flush_changes_n_do_ops_effects(
                ops, snap_mapper, osdriver, [this](auto&& txn, auto&& obc, auto&& osd_op_p, auto&& log_entries) {
                    logger().debug("do_osd_ops_execute: object {} submitting txn", obc->get_oid());
                    return submit_transaction(
                        std::move(obc), std::move(txn), std::move(osd_op_p), std::move(log_entries));
                });
        })
            ...
            [success_func = std::move(success_func), rollbacker, this, failure_func_ptr](
                ...
                return PG::do_osd_ops_iertr::make_ready_future<pg_rep_op_fut_t<Ret>>(
                    ...
                    all_completed_fut.safe_then_interruptible_tuple(
                        ...
                        crimson::ct_error::object_corrupted::handle([rollbacker,
                                                                     this](const std::error_code& e) mutable {
                            ...
                            return rollbacker.rollback_obc_if_modified(e).then_interruptible(
                                [obc = rollbacker.get_obc(), this] {
                                    return repair_object(obc->obs.oi.soid, obc->obs.oi.version).then_interruptible([] {
                                        return do_osd_ops_iertr::future<Ret>{crimson::ct_error::eagain::make()};
                                    });
                                });
                        }),
                        OpsExecuter::osd_op_errorator::all_same_way(
                            [rollbacker, failure_func_ptr](const std::error_code& e) mutable {
                                return rollbacker.rollback_obc_if_modified(e).then_interruptible(
                                    [e, failure_func_ptr] { return (*failure_func_ptr)(e); });
                            })));
            },
            OpsExecuter::osd_op_errorator::all_same_way(
                [rollbacker, failure_func_ptr](const std::error_code& e) mutable {
...
seastar::future<> PG::submit_error_log(Ref<MOSDOp> m, const OpInfo& op_info, ObjectContextRef obc,
                                       const std::error_code e, ceph_tid_t rep_tid, eversion_t& version)
{
    const osd_reqid_t& reqid = m->get_reqid();
    mempool::osd_pglog::list<pg_log_entry_t> log_entries;
    log_entries.push_back(pg_log_entry_t(
        pg_log_entry_t::ERROR, obc->obs.oi.soid, next_version(), eversion_t(), 0, reqid, utime_t(), -e.value()));
    ...
    peering_state.merge_new_log_entries(
        log_entries, t, peering_state.get_pg_trim_to(), peering_state.get_min_last_complete_ondisk());
    ...
    for (auto& i : get_acting_recovery_backfill()) {
        ...
        auto log_m = crimson::make_message<MOSDPGUpdateLogMissing>(log_entries,
    ...
    log_entry_update_waiting_on.insert(std::make_pair(rep_tid, log_update_t{std::move(waiting_on)}));
    return shard_services.get_store().do_transaction(get_collection_ref(), std::move(t)).then([this] {
        peering_state.update_trim_to();
        return seastar::now();
    });
}

PG::do_osd_ops_iertr::future<PG::pg_rep_op_fut_t<MURef<MOSDOpReply>>> PG::do_osd_ops(Ref<MOSDOp> m,
                                                                                     ...
                                                                                     const SnapContext& snapc)
{
    ...
    return do_osd_ops_execute<MURef<MOSDOpReply>>(
        seastar::make_lw_shared<OpsExecuter>(Ref<PG>{this}, obc, op_info, *m, conn, snapc),
        m->ops,
        [this,
         m,
         obc,
         may_write = op_info.may_write(),
         may_read = op_info.may_read(),
         rvec = op_info.allows_returnvec()] {
            ...
            else if (result < 0 && (m->ops.empty() ? 0 : m->ops.back().op.flags & CEPH_OSD_OP_FLAG_FAILOK)) {
                result = 0;
            }
            auto reply = crimson::make_message<MOSDOpReply>(m.get(), result, get_osdmap_epoch(), 0, false);
            reply->add_flags(CEPH_OSD_FLAG_ACK | CEPH_OSD_FLAG_ONDISK);
            logger().debug("do_osd_ops: {} - object {} sending reply", *m, m->get_hobj());
            if (obc->obs.exists) {
                reply->set_reply_versions(peering_state.get_info().last_update, obc->obs.oi.user_version);
            }
            else {
            ...
            return seastar::do_with(eversion_t(), [m, &op_info, obc, e, this](auto& version) {
                ...
                return fut.then([m, e, epoch, &op_info, rep_tid, &version, last_complete, this] {
                    auto log_reply = [m, e, this] {
                        auto reply =
                            crimson::make_message<MOSDOpReply>(m.get(), -e.value(), get_osdmap_epoch(), 0, false);
                        if (m->ops.empty() ? 0 : m->ops.back().op.flags & CEPH_OSD_OP_FLAG_FAILOK) {
                            reply->set_result(0);
                        }
                    ...
                    if (!peering_state.pg_has_reset_since(epoch) && op_info.may_write()) {
                        ...
                        if (it->second.waiting_on.empty()) {
                            ...
                            return it->second.all_committed.get_shared_future().then(
                                [this, &version, last_complete, log_reply = std::move(log_reply)] {
                                    if (version != eversion_t()) {
                                        peering_state.complete_write(version, last_complete);
                                    }
                                    return log_reply();
...
PG::do_osd_ops_iertr::future<PG::pg_rep_op_fut_t<>> PG::do_osd_ops(ObjectContextRef obc, std::vector<OSDOp>& ops,
                                                                   const OpInfo& op_info,
                                                                   ...
                                                                   do_osd_ops_failure_func_t failure_func)
{
    ...
    return seastar::do_with(std::move(msg_params), [=, this, &ops, &op_info](auto& msg_params) {
        return do_osd_ops_execute<void>(
            seastar::make_lw_shared<OpsExecuter>(
                Ref<PG>{this}, std::move(obc), op_info, msg_params, msg_params.get_connection(), SnapContext{}),
            ops,
            std::move(success_func),
...
PG::interruptible_future<MURef<MOSDOpReply>> PG::do_pg_ops(Ref<MOSDOp> m)
{
    ...
    auto ox = std::make_unique<PgOpsExecuter>(std::as_const(*this), std::as_const(*m));
    return interruptor::do_for_each(m->ops,
                                    [ox = ox.get()](OSDOp& osd_op) {
                                        logger().debug("will be handling pg op {}", ceph_osd_op_name(osd_op.op.op));
                                        return ox->execute_op(osd_op);
                                    })
        .then_interruptible([m, this, ox = std::move(ox)] {
            auto reply = crimson::make_message<MOSDOpReply>(
                m.get(), 0, get_osdmap_epoch(), CEPH_OSD_FLAG_ACK | CEPH_OSD_FLAG_ONDISK, false);
        ...
        .handle_exception_type_interruptible([=, this](const crimson::osd::error& e) {
            auto reply = crimson::make_message<MOSDOpReply>(m.get(), -e.code().value(), get_osdmap_epoch(), 0, false);
            reply->set_enoent_reply_versions(peering_state.get_info().last_update,
                                             peering_state.get_info().last_user_version);
            return seastar::make_ready_future<MURef<MOSDOpReply>>(std::move(reply));
        });
...
void PG::check_blocklisted_obc_watchers(ObjectContextRef& obc)
{
    if (obc->watchers.empty()) {
        for (auto& [src, winfo] : obc->obs.oi.watchers) {
            auto watch = crimson::osd::Watch::create(obc, winfo, src.second, this);
            watch->disconnect();
            auto [it, emplaced] = obc->watchers.emplace(src, std::move(watch));
            assert(emplaced);
            logger().debug("added watch for obj {}, client {}", obc->get_oid(), src.second);
        }
    }
}

PG::load_obc_iertr::future<> PG::with_locked_obc(const hobject_t& hobj, const OpInfo& op_info, with_obc_func_t&& f)
{
    if (__builtin_expect(stopping, false)) {
        throw crimson::common::system_shutdown_exception();
    }
    ...
    case RWState::RWREAD: return obc_loader.with_obc<RWState::RWREAD>(oid, std::move(wrapper));
    case RWState::RWWRITE: return obc_loader.with_obc<RWState::RWWRITE>(oid, std::move(wrapper));
    case RWState::RWEXCL: return obc_loader.with_obc<RWState::RWEXCL>(oid, std::move(wrapper));
    default: ceph_abort();
    };
}
...
PG::interruptible_future<> PG::handle_rep_op(Ref<MOSDRepOp> req)
{
    ...
    log_operation(
        std::move(log_entries), req->pg_trim_to, req->version, req->min_last_complete_ondisk, !txn.empty(), txn, false);
    logger().debug("PG::handle_rep_op: do_transaction...");
    return interruptor::make_interruptible(shard_services.get_store().do_transaction(coll_ref, std::move(txn)))
        .then_interruptible([req, lcod = peering_state.get_info().last_complete, this] {
            peering_state.update_last_complete_ondisk(lcod);
            const auto map_epoch = get_osdmap_epoch();
            auto reply = crimson::make_message<MOSDRepOpReply>(
                req.get(), pg_whoami, 0, map_epoch, req->get_min_epoch(), CEPH_OSD_FLAG_ONDISK);
            reply->set_last_complete_ondisk(lcod);
            return shard_services.send_to_osd(req->from.osd, std::move(reply), map_epoch);
        });
}

void PG::log_operation(std::vector<pg_log_entry_t>&& logv, const eversion_t& trim_to, const eversion_t& roll_forward_to,
                       const eversion_t& min_last_complete_ondisk, bool transaction_applied,
                       ObjectStore::Transaction& txn, bool async)
{
    logger().debug("{}", __func__);
    if (is_primary()) {
  ...
  if (is_primary() && last != logv.rend()) {
    ...
    peering_state.append_log(
        std::move(logv), trim_to, roll_forward_to, min_last_complete_ondisk, txn, !txn.empty(), false);
}

void PG::replica_clear_repop_obc(const std::vector<pg_log_entry_t>& logv)
{
    ...
    for (auto&& e : logv) {
        ...
        obc_registry.clear_range(e.soid.get_object_boundary(), e.soid.get_head());
    }
}
...
PG::interruptible_future<> PG::do_update_log_missing(Ref<MOSDPGUpdateLogMissing> m, crimson::net::ConnectionRef conn)
{
    if (__builtin_expect(stopping, false)) {
        return seastar::make_exception_future<>(crimson::common::system_shutdown_exception());
    }
    ...
    logger().debug("op_trim_to = {}, op_roll_forward_to = {}", op_trim_to, op_roll_forward_to);

    peering_state.append_log_entries_update_missing(m->entries, t, op_trim_to, op_roll_forward_to);

    return interruptor::make_interruptible(shard_services.get_store().do_transaction(coll_ref, std::move(t)))
        .then_interruptible([m, conn, lcod = peering_state.get_info().last_complete, this] {
            if (!peering_state.pg_has_reset_since(m->get_epoch())) {
                peering_state.update_last_complete_ondisk(lcod);
                auto reply = crimson::make_message<MOSDPGUpdateLogMissingReply>(
...
PG::interruptible_future<> PG::do_update_log_missing_reply(Ref<MOSDPGUpdateLogMissingReply> m)
{
    ...
    if (it != log_entry_update_waiting_on.end()) {
        if (it->second.waiting_on.count(m->get_from())) {
            ...
            if (m->last_complete_ondisk != eversion_t()) {
                peering_state.update_peer_last_complete_ondisk(m->get_from(), m->last_complete_ondisk);
            }
        }
        else {
            logger().error("{} : {} got reply {} from shard we are not waiting for ",
                           __func__,
                           peering_state.get_info().pgid,
                           *m,
                           m->get_from());
        }
        ...
        logger().error(
            "{} : {} got reply {} on unknown tid {}", __func__, peering_state.get_info().pgid, *m, m->get_tid());
    }
    return seastar::now();
}

bool PG::old_peering_msg(const epoch_t reply_epoch, const epoch_t query_epoch) const
{
    if (const epoch_t lpr = peering_state.get_last_peering_reset(); lpr > reply_epoch || lpr > query_epoch) {
...
seastar::future<> PG::stop()
{
    ...
    check_readable_timer.cancel();
    renew_lease_timer.cancel();
    return osdmap_gate.stop()
        .then([this] { return wait_for_active_blocker.stop(); })
        .then([this] { return recovery_handler->stop(); })
        .then([this] { return recovery_backend->stop(); })
        .then([this] { return backend->stop(); });
}
...
void PG::context_registry_on_change()
{
    obc_registry.for_each([](ObjectContextRef obc) {
        assert(obc);
        for (auto j = obc->watchers.begin(); j != obc->watchers.end(); j = obc->watchers.erase(j)) {
...
{
    if (m.get_map_epoch() < peering_state.get_info().history.same_primary_since) {
        logger().debug("{} changed after {} dropping {} ", __func__, m.get_map_epoch(), m);
        return true;
    }

    if ((m.get_flags() & (CEPH_OSD_FLAG_BALANCE_READS | CEPH_OSD_FLAG_LOCALIZE_READS)) && !is_primary() &&
        (m.get_map_epoch() < peering_state.get_info().history.same_interval_since)) {
    ...
    return __builtin_expect(m.get_map_epoch() < peering_state.get_info().history.same_primary_since, false);
}
...
bool PG::is_degraded_or_backfilling_object(const hobject_t& soid) const
{
     ...
     */
    if (peering_state.get_pg_log().get_missing().get_items().count(soid)) return true;
    ...
    for (auto& peer : get_acting_recovery_backfill()) {
        ...
        if (is_backfill_target(peer) && peering_state.get_peer_info(peer).last_backfill <= soid &&
            recovery_handler->backfill_state && recovery_handler->backfill_state->get_last_backfill_started() >= soid &&
...
PG::interruptible_future<std::optional<PG::complete_op_t>> PG::already_complete(const osd_reqid_t& reqid)
{
    eversion_t version;
    ...
    if (peering_state.get_pg_log().get_log().get_request(reqid, &version, &user_version, &ret, &op_returns)) {
        complete_op_t dupinfo{user_version, version, ret};
