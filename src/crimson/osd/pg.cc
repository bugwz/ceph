...
PG::PG(spg_t pgid, pg_shard_t pg_shard, crimson::os::CollectionRef coll_ref, pg_pool_t&& pool, std::string&& name,
    ...
    , osdmap{osdmap}
    , backend(PGBackend::create(pgid.pgid, pg_shard, pool, coll_ref, shard_services, profile))
...
{
    (void)shard_services.get_store()
        .do_transaction(coll_ref, ObjectStore::Transaction())
        .then([this, epoch = get_osdmap_epoch()]() {
            return shard_services.start_operation<LocalPeeringEvent>(
                this, shard_services, pg_whoami, pgid, epoch, epoch, PeeringState::IntervalFlush());
        });
    return false;
}
...
void PG::recheck_readable()
{
    ...
    if (peering_state.state_test(PG_STATE_WAIT)) {
        ...
        if (mnow < prior_readable_until_ub) {
            logger().info(
                "{} will wait (mnow {} < prior_readable_until_ub {})", __func__, mnow, prior_readable_until_ub);
        }
        else {
            logger().info(
                "{} no longer wait (mnow {} >= prior_readable_until_ub {})", __func__, mnow, prior_readable_until_ub);
    ...
    if (peering_state.state_test(PG_STATE_LAGGY)) {
        ...
        if (readable_until == readable_until.zero()) {
            logger().info("{} still laggy (mnow {}, readable_until zero)", __func__, mnow);
        }
        else if (mnow >= readable_until) {
            logger().info("{} still laggy (mnow {} >= readable_until {})", __func__, mnow, readable_until);
        }
        else {
            logger().info("{} no longer laggy (mnow {} < readable_until {})", __func__, mnow, readable_until);
...
unsigned PG::get_target_pg_log_entries() const
{
    ...
    const unsigned target = local_conf().get_val<uint64_t>("osd_target_pg_log_entries_per_osd");
    ...
    if (num_pgs > 0 && target > 0) {
        ...
        return std::clamp(target / num_pgs, min_pg_log_entries, max_pg_log_entries);
    }
    else {
...
void PG::on_activate_complete()
{
    ...
    if (peering_state.needs_recovery()) {
        ...
        (void)shard_services.start_operation<LocalPeeringEvent>(
            this, shard_services, pg_whoami, pgid, get_osdmap_epoch(), get_osdmap_epoch(), PeeringState::DoRecovery{});
    }
                       ...
                       bool dirty_info, bool dirty_big_info, bool need_write_epoch, ceph::os::Transaction& t)
{
    ...
    pglog.write_log_and_missing(
        t, &km, coll_ref->get_cid(), pgmeta_oid, peering_state.get_pool().info.require_rollback());
    if (!km.empty()) {
...
void PG::schedule_renew_lease(epoch_t last_peering_reset, ceph::timespan delay)
{
    ...
    renew_lease_timer.set_callback([last_peering_reset, this] {
        (void)shard_services.start_operation<LocalPeeringEvent>(
            this, shard_services, pg_whoami, pgid, last_peering_reset, last_peering_reset, RenewLease{});
    });
...
{
    peering_state.init(role, newup, new_up_primary, newacting, new_acting_primary, history, pi, backfill, t);
}
...
seastar::future<> PG::read_state(crimson::os::FuturizedStore* store)
{
    ...
    return seastar::do_with(PGMeta(store, pgid), [](auto& pg_meta) { return pg_meta.load(); })
        ...
        .then([this]() {
            ...
            (void)shard_services.start_operation<LocalPeeringEvent>(
                this, shard_services, pg_whoami, pgid, epoch, epoch, PeeringState::Initialize());

            return seastar::now();
        });
}

void PG::do_peering_event(const boost::statechart::event_base& evt, PeeringCtx& rctx)
{
    peering_state.handle_event(evt, &rctx);
    peering_state.write_if_dirty(rctx.transaction);
}

void PG::do_peering_event(PGPeeringEvent& evt, PeeringCtx& rctx)
{
...
void PG::handle_advance_map(cached_map_t next_map, PeeringCtx& rctx)
{
    vector<int> newup, newacting;
    int up_primary, acting_primary;
    next_map->pg_to_up_acting_osds(pgid.pgid, &newup, &up_primary, &newacting, &acting_primary);
    peering_state.advance_map(next_map, peering_state.get_osdmap(), newup, up_primary, newacting, acting_primary, rctx);
...
seastar::future<> PG::submit_transaction(const OpInfo& op_info, const std::vector<OSDOp>& ops, ObjectContextRef&& obc,
                                         ceph::os::Transaction&& txn, const osd_op_params_t& osd_op_p)
{
    if (__builtin_expect(stopping, false)) {
        return seastar::make_exception_future<>(crimson::common::system_shutdown_exception());
    }
    ...
    log_entries.emplace_back(obc->obs.exists ? pg_log_entry_t::MODIFY : pg_log_entry_t::DELETE,
                             ...
                             osd_op_p.req->get_reqid(),
                             osd_op_p.req->get_mtime(),
    ...
    return backend
        ->mutate_object(peering_state.get_acting_recovery_backfill(),
                        std::move(obc),
                        std::move(txn),
                        std::move(osd_op_p),
        ...
        .then([this, last_complete = peering_state.get_info().last_complete, at_version = osd_op_p.at_version](
                  auto acked) {
...
osd_op_params_t&& PG::fill_op_params_bump_pg_version(osd_op_params_t&& osd_op_p, Ref<MOSDOp> m, const bool user_modify)
{
...
seastar::future<Ref<MOSDOpReply>> PG::handle_failed_op(const std::error_code& e, ObjectContextRef obc,
                                                       const OpsExecuter& ox, const MOSDOp& m) const
{
    ...
    logger().debug("{}: {} - object {} got error code {}, {}; need_reload_obc {}",
                   __func__,
                   m,
                   obc->obs.oi.soid,
                   e.value(),
                   e.message(),
                   need_reload_obc);
    return (need_reload_obc ? reload_obc(*obc) : load_obc_ertr::now())
        .safe_then(
            [&e, &m, obc = std::move(obc), this] {
                auto reply = make_message<MOSDOpReply>(&m, -e.value(), get_osdmap_epoch(), 0, false);
...
seastar::future<Ref<MOSDOpReply>> PG::do_osd_ops(Ref<MOSDOp> m, ObjectContextRef obc, const OpInfo& op_info)
{
    if (__builtin_expect(stopping, false)) {
    ...
    const auto oid = m->get_snapid() == CEPH_SNAPDIR ? m->get_hobj().get_head() : m->get_hobj();
    auto ox = std::make_unique<OpsExecuter>(obc, op_info, get_pool().info, get_backend(), *m);
    return crimson::do_for_each(m->ops,
                                [obc, m, ox = ox.get()](OSDOp& osd_op) {
                                    logger().debug("do_osd_ops: {} - object {} - handling op {}",
                                                   *m,
                                                   obc->obs.oi.soid,
        ...
        .safe_then([this, obc, m, ox = ox.get(), &op_info] {
            logger().debug("do_osd_ops: {} - object {} all operations successful", *m, obc->obs.oi.soid);
            return std::move(*ox).flush_changes(
                [m](auto&& obc) -> osd_op_errorator::future<> {
                    logger().debug("do_osd_ops: {} - object {} txn is empty, bypassing mutate", *m, obc->obs.oi.soid);
                    return osd_op_errorator::now();
                },
                [this, m, &op_info](
                    auto&& txn, auto&& obc, auto&& osd_op_p, bool user_modify) -> osd_op_errorator::future<> {
                    logger().debug("do_osd_ops: {} - object {} submitting txn", *m, obc->obs.oi.soid);
                    auto filled_osd_op_p =
                        fill_op_params_bump_pg_version(std::move(osd_op_p), std::move(m), user_modify);
                    return submit_transaction(
                        op_info, filled_osd_op_p.req->ops, std::move(obc), std::move(txn), std::move(filled_osd_op_p));
                });
        })
        .safe_then(
            [this, m, obc, rvec = op_info.allows_returnvec()] {
                ...
                auto reply = make_message<MOSDOpReply>(m.get(), result, get_osdmap_epoch(), 0, false);
                reply->add_flags(CEPH_OSD_FLAG_ACK | CEPH_OSD_FLAG_ONDISK);
                logger().debug("do_osd_ops: {} - object {} sending reply", *m, obc->obs.oi.soid);
                return seastar::make_ready_future<Ref<MOSDOpReply>>(std::move(reply));
            },
            osd_op_errorator::all_same_way([ox = ox.get(), m, obc, this](const std::error_code& e) {
                return handle_failed_op(e, std::move(obc), *ox, *m);
            }))
        .handle_exception_type([ox_deleter = std::move(ox), m, obc, this](const crimson::osd::error& e) {
...
seastar::future<Ref<MOSDOpReply>> PG::do_pg_ops(Ref<MOSDOp> m)
{
    ...
    return seastar::do_for_each(m->ops,
        ...
        .handle_exception_type([=](const crimson::osd::error& e) {
            auto reply = make_message<MOSDOpReply>(m.get(), -e.code().value(), get_osdmap_epoch(), 0, false);
            reply->set_enoent_reply_versions(peering_state.get_info().last_update,
                                             peering_state.get_info().last_user_version);
            return seastar::make_ready_future<Ref<MOSDOpReply>>(std::move(reply));
        });
...
{
    return (m.get_snapid() == CEPH_SNAPDIR ? m.get_hobj().get_head() : m.get_hobj());
}
...
std::optional<hobject_t> PG::resolve_oid(const SnapSet& ss, const hobject_t& oid)
{
    if (oid.snap > ss.seq) {
        return oid.get_head();
    }
        ...
        auto clone = std::upper_bound(begin(ss.clones), end(ss.clones), oid.snap);
        ...
        if (std::find(citer->second.begin(), citer->second.end(), *clone) == citer->second.end()) {
            return std::nullopt;
        }
        else {
            auto soid = oid;
...
template<RWState::State State> PG::load_obc_ertr::future<> PG::with_head_obc(hobject_t oid, with_obc_func_t&& func)
{
    assert(oid.is_head());
    auto [obc, existed] = shard_services.obc_registry.get_cached_obc(oid);
    return obc->with_lock<State>(
        [oid = std::move(oid), existed = existed, obc = std::move(obc), func = std::move(func), this] {
            ...
            if (existed) {
                ...
                loaded = obc->with_promoted_lock<State>([this, obc] { return load_head_obc(obc); });
            }
            return loaded.safe_then([func = std::move(func)](auto obc) { return func(std::move(obc)); });
        });
}

template<RWState::State State> PG::load_obc_ertr::future<> PG::with_clone_obc(hobject_t oid, with_obc_func_t&& func)
{
    assert(!oid.is_head());
    return with_head_obc<RWState::RWREAD>(
        oid.get_head(), [oid, func = std::move(func), this](auto head) -> load_obc_ertr::future<> {
            ...
            return clone->template with_lock<State>([coid = *coid,
                                                     existed = existed,
                                                     head = std::move(head),
                ...
                if (existed) {
                    ...
                    loaded = clone->template with_promoted_lock<State>([coid, clone, head, this] {
                ...
                return loaded.safe_then([func = std::move(func)](auto clone) { return func(std::move(clone)); });
            });
        });
}

// explicitly instantiate the used instantiations
template PG::load_obc_ertr::future<> PG::with_head_obc<RWState::RWNONE>(hobject_t, with_obc_func_t&&);

PG::load_obc_ertr::future<crimson::osd::ObjectContextRef> PG::load_head_obc(ObjectContextRef obc)
{
    hobject_t oid = obc->get_oid();
    return backend->load_metadata(oid).safe_then(
        [obc = std::move(obc)](auto md) -> load_obc_ertr::future<crimson::osd::ObjectContextRef> {
            const hobject_t& oid = md->os.oi.soid;
            logger().debug("load_head_obc: loaded obs {} for {}", md->os.oi, oid);
            if (!md->ss) {
                logger().error("load_head_obc: oid {} missing snapset", oid);
                return crimson::ct_error::object_corrupted::make();
            }
            obc->set_head_state(std::move(md->os), std::move(*(md->ss)));
            logger().debug("load_head_obc: returning obc {} for {}", obc->obs.oi, obc->obs.oi.soid);
            return load_obc_ertr::make_ready_future<crimson::osd::ObjectContextRef>(obc);
        });
}

PG::load_obc_ertr::future<> PG::reload_obc(crimson::osd::ObjectContext& obc) const
{
    assert(obc.is_head());
    return backend->load_metadata(obc.get_oid()).safe_then([&obc](auto md) -> load_obc_ertr::future<> {
        logger().debug("{}: reloaded obs {} for {}", __func__, md->os.oi, obc.get_oid());
        if (!md->ss) {
            logger().error("{}: oid {} missing snapset", __func__, obc.get_oid());
            return crimson::ct_error::object_corrupted::make();
        }
        obc.set_head_state(std::move(md->os), std::move(*(md->ss)));
...
PG::load_obc_ertr::future<> PG::with_locked_obc(Ref<MOSDOp>& m, const OpInfo& op_info, Operation* op,
                                                PG::with_obc_func_t&& f)
{
    if (__builtin_expect(stopping, false)) {
        throw crimson::common::system_shutdown_exception();
    }
...
seastar::future<> PG::handle_rep_op(Ref<MOSDRepOp> req)
{
    ...
    peering_state.append_log(
        std::move(log_entries), req->pg_trim_to, req->version, req->min_last_complete_ondisk, txn, !txn.empty(), false);
    return shard_services.get_store()
        .do_transaction(coll_ref, std::move(txn))
...
void PG::handle_rep_op_reply(crimson::net::ConnectionRef conn, const MOSDRepOpReply& m)
{
...
template<typename MsgType> bool PG::can_discard_replica_op(const MsgType& m) const
{
    ...
    if (epoch_t lpr = peering_state.get_last_peering_reset(); lpr > m.map_epoch) {
