...
void PGRecovery::start_pglogbased_recovery()
{
    ...
    (void)pg->get_shard_services().start_operation<PglogBasedRecovery>(
        static_cast<crimson::osd::PG*>(pg), pg->get_shard_services(), pg->get_osdmap_epoch());
}

crimson::osd::blocking_future<bool> PGRecovery::start_recovery_ops(size_t max_to_start)
{
    ...
    return crimson::osd::join_blocking_futures(std::move(started)).then([this] {
        ...
        if (done) {
            ...
            if (!pg->get_peering_state().needs_backfill()) {
                logger().debug("start_recovery_ops: AllReplicasRecovered for pg: {}", pg->get_pgid());
                (void)pg->get_shard_services().start_operation<LocalPeeringEvent>(static_cast<crimson::osd::PG*>(pg),
...
size_t PGRecovery::start_primary_recovery_ops(size_t max_to_start, std::vector<crimson::osd::blocking_future<>>* out)
{
    if (!pg->is_recovering()) {
        return 0;
    }
...
size_t PGRecovery::start_replica_recovery_ops(size_t max_to_start, std::vector<crimson::osd::blocking_future<>>* out)
{
    if (!pg->is_recovering()) {
        return 0;
    }
    ...
    for (auto& peer : recovery_order) {
        ...
        for (auto p = pm.get_rmissing().begin(); p != pm.get_rmissing().end() && started < max_to_start; ++p) {
            ...
            if (pg->get_peering_state().get_missing_loc().is_deleted(soid)) {
                ...
                started += prep_object_replica_deletes(soid, r->second.need, out);
                continue;
            }
            ...
            map<hobject_t, pg_missing_item>::const_iterator r = pm.get_items().find(soid);
            started += prep_object_replica_pushes(soid, r->second.need, out);
        }
    }
...
crimson::osd::blocking_future<> PGRecovery::recover_missing(const hobject_t& soid, eversion_t need)
{
    if (pg->get_peering_state().get_missing_loc().is_deleted(soid)) {
        ...
        return pg->get_recovery_backend()->add_recovering(soid).make_blocking_future(
            pg->get_recovery_backend()
                ...
                .handle_exception([=, soid = std::move(soid)](auto e) {
...
size_t PGRecovery::prep_object_replica_deletes(const hobject_t& soid, eversion_t need,
...
size_t PGRecovery::prep_object_replica_pushes(const hobject_t& soid, eversion_t need,
                                              std::vector<crimson::osd::blocking_future<>>* in_progress)
{
    in_progress->push_back(pg->get_recovery_backend()->add_recovering(soid).make_blocking_future(
        pg->get_recovery_backend()->recover_object(soid, need).handle_exception([=, soid = std::move(soid)](auto e) {
            on_failed_recover({pg->get_pg_whoami()}, soid, need);
            return seastar::make_ready_future<>();
        })));
    return 1;
...
void PGRecovery::request_replica_scan(const pg_shard_t& target, const hobject_t& begin, const hobject_t& end)
{
    ...
    auto msg = make_message<MOSDPGScan>(MOSDPGScan::OP_SCAN_GET_DIGEST,
                                        pg->get_pg_whoami(),
                                        pg->get_osdmap_epoch(),
...
void PGRecovery::enqueue_drop(const pg_shard_t& target, const hobject_t& obj, const eversion_t& v)
{
    ...
    if (!req) {
        req =
            ceph::make_message<MOSDPGBackfillRemove>(spg_t(pg->get_pgid().pgid, target.shard), pg->get_osdmap_epoch());
    }
...
void PGRecovery::update_peers_last_backfill(const hobject_t& new_last_backfill)
{
    ...
    for (const auto& bt : pg->get_peering_state().get_backfill_targets()) {
        if (const pg_info_t& pinfo = pg->get_peering_state().get_peer_info(bt);
            ...
            auto m = make_message<MOSDPGBackfill>(pinfo.last_backfill.is_max() ? MOSDPGBackfill::OP_BACKFILL_FINISH
