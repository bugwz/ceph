...
seastar::future<epoch_t> OSDMapGate<OSDMapGateTypeV>::wait_for_map(
    typename OSDMapBlocker::BlockingEvent::TriggerI&& trigger, epoch_t epoch, ShardServices* shard_services)
{
    if (__builtin_expect(stopping, false)) {
        return seastar::make_exception_future<epoch_t>(crimson::common::system_shutdown_exception());
    }
    if (current >= epoch) {
        ...
        auto& blocker = waiting_peering.emplace(epoch, std::make_pair(blocker_type, epoch)).first->second;
        auto fut = blocker.promise.get_shared_future();
        if (shard_services) {
            return trigger.maybe_record_blocking(
                shard_services->osdmap_subscribe(current, true).then([fut = std::move(fut)]() mutable {
                    return std::move(fut);
                }),
                blocker);
        }
...
template<OSDMapGateType OSDMapGateTypeV> void OSDMapGate<OSDMapGateTypeV>::got_map(epoch_t epoch)
{
    if (epoch == 0) {
        return;
    }
    ...
    std::for_each(first, last, [epoch](auto& blocked_requests) { blocked_requests.second.promise.set_value(epoch); });
    waiting_peering.erase(first, last);
}

template<OSDMapGateType OSDMapGateTypeV> seastar::future<> OSDMapGate<OSDMapGateTypeV>::stop()
{
    ...
    std::for_each(first, last, [](auto& blocked_requests) {
        blocked_requests.second.promise.set_exception(crimson::common::system_shutdown_exception());
    });
    return seastar::now();
}
