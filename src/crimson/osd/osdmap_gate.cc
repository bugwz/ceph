...
blocking_future<epoch_t> OSDMapGate::wait_for_map(epoch_t epoch)
{
    if (__builtin_expect(stopping, false)) {
        return make_exception_blocking_future<epoch_t>(crimson::common::system_shutdown_exception());
    }
    if (current >= epoch) {
        ...
        auto& blocker = waiting_peering.emplace(epoch, make_pair(blocker_type, epoch)).first->second;
        auto fut = blocker.promise.get_shared_future();
        if (shard_services) {
            return blocker.make_blocking_future(
                (*shard_services).get().osdmap_subscribe(current, true).then([fut = std::move(fut)]() mutable {
                    return std::move(fut);
                }));
        }
        else {
