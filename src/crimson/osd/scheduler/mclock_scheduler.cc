...
mClockScheduler::mClockScheduler(ConfigProxy& conf)
    : scheduler(std::bind(&mClockScheduler::ClientRegistry::get_info, &client_registry, _1), dmc::AtLimit::Allow,
...
void mClockScheduler::ClientRegistry::update_from_config(const ConfigProxy& conf)
{
    ...
    internal_client_infos[static_cast<size_t>(scheduler_class_t::background_recovery)].update(
    ...
    internal_client_infos[static_cast<size_t>(scheduler_class_t::background_best_effort)].update(
...
{
    switch (id.class_id) {
    case scheduler_class_t::immediate: ceph_assert(0 == "Cannot schedule immediate"); return (dmc::ClientInfo*)nullptr;
    case scheduler_class_t::repop:
    case scheduler_class_t::client: return get_external_client(id.client_profile_id);
    default:
...
item_t mClockScheduler::dequeue()
{
    if (!immediate.empty()) {
        ...
        if (result.is_future()) {
            ceph_assert(0 == "Not implemented, user would have to be able to be woken up");
