...
public:
    ...
    {
        return is_active() ? seastar::now() : wait_for_active.get_shared_future();
    };
    ...
    bool is_stopping() const { return state == State::STOPPING; }
    ...
    {
        ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
        return container().invoke_on_all([](auto& osd_state) { osd_state._set_active(); });
    }
    void set_prestop()
    {
        ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
    ...
    {
        ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
        return container().invoke_on_all([](auto& osd_state) { osd_state._set_stopping(); });
    }
...
inline std::ostream& operator<<(std::ostream& os, const OSDState& s)
{
