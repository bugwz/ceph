...
PerShardState::PerShardState(int whoami, ceph::mono_time startup_time, PerfCounters* perf,
                             PerfCounters* recoverystate_perf, crimson::os::FuturizedStore& store, OSDState& osd_state)
    : whoami(whoami)
    ...
    , next_tid(static_cast<ceph_tid_t>(seastar::this_shard_id()) << (std::numeric_limits<ceph_tid_t>::digits - 8))
    , startup_time(startup_time)
{}

seastar::future<> PerShardState::dump_ops_in_flight(Formatter* f) const
{
    registry.for_each_op([f](const auto& op) { op.dump(f); });
    return seastar::now();
}
...
seastar::future<> PerShardState::broadcast_map_to_pgs(ShardServices& shard_services, epoch_t epoch)
{
    assert_core();
    auto& pgs = pg_map.get_pgs();
    return seastar::parallel_for_each(pgs.begin(), pgs.end(), [=, &shard_services](auto& pg) {
        return shard_services.start_operation<PGAdvanceMap>(shard_services, pg.second, epoch, PeeringCtx{}, false)
            .second;
    });
}
...
OSDSingletonState::OSDSingletonState(int whoami, crimson::net::Messenger& cluster_msgr,
                                     crimson::net::Messenger& public_msgr, crimson::mon::Client& monc,
    ...
    , monc(monc)
    , mgrc(mgrc)
    , local_reserver(&cct, &finisher, crimson::common::local_conf()->osd_max_backfills,
                     crimson::common::local_conf()->osd_min_recovery_priority)
    , remote_reserver(&cct, &finisher, crimson::common::local_conf()->osd_max_backfills,
                      crimson::common::local_conf()->osd_min_recovery_priority)
    , snap_reserver(&cct, &finisher, crimson::common::local_conf()->osd_max_trimming_pgs)
{
...
seastar::future<> OSDSingletonState::send_to_osd(int peer, MessageURef m, epoch_t from_epoch)
{
    if (osdmap->is_down(peer)) {
        ...
        logger().info("{}: osd.{} {} > {}", __func__, peer, osdmap->get_info(peer).up_from, from_epoch);
        return seastar::now();
    }
    else {
        auto conn = cluster_msgr.connect(osdmap->get_cluster_addrs(peer).front(), CEPH_ENTITY_TYPE_OSD);
        return conn->send(std::move(m));
    }
}

seastar::future<> OSDSingletonState::osdmap_subscribe(version_t epoch, bool force_request)
{
...
void OSDSingletonState::queue_want_pg_temp(pg_t pgid, const vector<int>& want, bool forced)
{
...
void OSDSingletonState::requeue_pg_temp()
{
    ...
    logger().debug("{}: {} + {} -> {}", __func__, old_wanted, old_pending, pg_temp_wanted.size());
}

seastar::future<> OSDSingletonState::send_pg_temp()
{
    if (pg_temp_wanted.empty()) return seastar::now();
    ...
    return seastar::parallel_for_each(std::begin(ms), std::end(ms), [this](auto& m) {
        if (m) {
...
std::ostream& operator<<(std::ostream& out, const OSDSingletonState::pg_temp_t& pg_temp)
{
    out << pg_temp.acting;
    if (pg_temp.forced) {
        out << " (forced)";
...
seastar::future<> OSDSingletonState::send_pg_created()
{
    ...
    return seastar::parallel_for_each(
        pg_created, [this](auto& pgid) { return monc.send_message(crimson::make_message<MOSDPGCreated>(pgid)); });
}
...
{
    logger().info("{} want={} up_thru_wanted={}", __func__, want, up_thru_wanted);

    if (want > up_thru_wanted) {
        up_thru_wanted = want;
    }
    else {
        logger().debug("{} want={} <= up_thru_wanted={}; skipping", __func__, want, up_thru_wanted);
        return seastar::now();
    }
    ...
    if (const epoch_t up_thru = osdmap->get_up_thru(whoami); up_thru_wanted > up_thru) {
        logger().debug("{} up_thru_wanted={} up_thru={}", __func__, want, up_thru);
        return monc.send_message(crimson::make_message<MOSDAlive>(osdmap->get_epoch(), want));
    }
    else {
...
{
    static const char* KEYS[] = {"osd_max_backfills", "osd_min_recovery_priority", "osd_max_trimming_pgs", nullptr};
    return KEYS;
}

void OSDSingletonState::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
...
seastar::future<OSDSingletonState::local_cached_map_t> OSDSingletonState::get_local_map(epoch_t e)
{
    ...
    if (auto found = osdmaps.find(e); found) {
        ...
        return load_map(e).then([e, this](std::unique_ptr<OSDMap> osdmap) {
            return seastar::make_ready_future<local_cached_map_t>(osdmaps.insert(e, std::move(osdmap)));
        });
    }
}

void OSDSingletonState::store_map_bl(ceph::os::Transaction& t, epoch_t e, bufferlist&& bl)
{
    meta_coll->store_map(t, e, bl);
    map_bl_cache.insert(e, std::move(bl));
}

seastar::future<bufferlist> OSDSingletonState::load_map_bl(epoch_t e)
{
...
seastar::future<std::map<epoch_t, bufferlist>> OSDSingletonState::load_map_bls(epoch_t first, epoch_t last)
{
    logger().debug("{} loading maps [{},{}]", __func__, first, last);
    ...
    return seastar::map_reduce(
        ...
        [this](epoch_t e) {
            return load_map_bl(e).then([e](auto&& bl) {
                return seastar::make_ready_future<std::pair<epoch_t, bufferlist>>(std::make_pair(e, std::move(bl)));
            });
        },
...
seastar::future<> OSDSingletonState::store_maps(ceph::os::Transaction& t, epoch_t start, Ref<MOSDMap> m)
{
...
seastar::future<Ref<PG>> ShardServices::make_pg(OSDMapService::cached_map_t create_map, spg_t pgid, bool do_create)
{
    ...
    auto get_pool_info_for_pg = [create_map, pgid, this] {
        if (create_map->have_pg_pool(pgid.pool())) {
            ...
            return seastar::make_ready_future<std::tuple<pg_pool_t, std::string, ec_profile_t>>(
    ...
    return seastar::when_all(std::move(get_pool_info_for_pg), std::move(get_collection))
...
seastar::future<Ref<PG>> ShardServices::handle_pg_create_info(std::unique_ptr<PGCreateInfo> info)
{
    return seastar::do_with(std::move(info), [this](auto& info) -> seastar::future<Ref<PG>> {
        return get_map(info->epoch)
            .then([&info, this](cached_map_t startmap) -> seastar::future<std::tuple<Ref<PG>, cached_map_t>> {
                ...
                if (info->by_mon) {
                    ...
                    if (!pool) {
                        ...
                        logger().debug("{} ignoring pgid {}, pool lacks crimson flag", __func__, pgid);
                        local_state.pg_map.pg_creation_canceled(pgid);
                        return seastar::make_ready_future<std::tuple<Ref<PG>, OSDMapService::cached_map_t>>(
                            std::make_tuple(Ref<PG>(), startmap));
                    }
                    ceph_assert(get_map()->require_osd_release >= ceph_release_t::octopus);
            ...
            .then([this, &info](auto&& ret) -> seastar::future<Ref<PG>> {
                ...
                int role = startmap->calc_pg_role(pg_shard_t(local_state.whoami, info->pgid.shard), acting);
                ...
                return start_operation<PGAdvanceMap>(*this, pg, get_map()->get_epoch(), std::move(rctx), true)
...
ShardServices::get_or_create_pg_ret ShardServices::get_or_create_pg(PGMap::PGCreationBlockingEvent::TriggerI&& trigger,
                                                                    spg_t pgid, std::unique_ptr<PGCreateInfo> info)
{
    if (info) {
        auto [fut, creating] = local_state.pg_map.wait_for_pg(std::move(trigger), pgid);
        if (!creating) {
        ...
        return get_or_create_pg_ret(get_or_create_pg_ertr::ready_future_marker{}, local_state.pg_map.get_pg(pgid));
    }
}

ShardServices::wait_for_pg_ret ShardServices::wait_for_pg(PGMap::PGCreationBlockingEvent::TriggerI&& trigger,
                                                          spg_t pgid)
{
...

{
    ...
    return seastar::do_with(PGMeta(get_store(), pgid), [](auto& pg_meta) { return pg_meta.get_epoch(); })
...
seastar::future<> ShardServices::dispatch_context_transaction(crimson::os::CollectionRef col, PeeringCtx& ctx)
{
    ...
    auto ret = get_store().do_transaction(col, std::move(ctx.transaction));
    ctx.reset_transaction();
    return ret;
}

seastar::future<> ShardServices::dispatch_context_messages(BufferedRecoveryMessages&& ctx)
{
    auto ret = seastar::parallel_for_each(std::move(ctx.message_map), [this](auto& osd_messages) {
        auto& [peer, messages] = osd_messages;
        logger().debug("dispatch_context_messages sending messages to {}", peer);
        return seastar::parallel_for_each(std::move(messages), [=, peer = peer, this](auto& m) {
...
seastar::future<> ShardServices::dispatch_context(crimson::os::CollectionRef col, PeeringCtx&& ctx)
{
    ceph_assert(col || ctx.transaction.empty());
    return seastar::when_all_succeed(dispatch_context_messages(BufferedRecoveryMessages{ctx}),
                                     col ? dispatch_context_transaction(col, ctx) : seastar::now())
        .then_unpack([] { return seastar::now(); });
}

seastar::future<> OSDSingletonState::send_incremental_map(crimson::net::Connection& conn, epoch_t first)
{
    ...
    if (first >= superblock.oldest_map) {
        return load_map_bls(first, superblock.newest_map).then([this, &conn, first](auto&& bls) {
            auto m = crimson::make_message<MOSDMap>(monc.get_fsid(), osdmap->get_encoding_features());
...
seastar::future<> OSDSingletonState::send_incremental_map_to_osd(int osd, epoch_t first)
{
    if (osdmap->is_down(osd)) {
        ...
        auto conn = cluster_msgr.connect(osdmap->get_cluster_addrs(osd).front(), CEPH_ENTITY_TYPE_OSD);
