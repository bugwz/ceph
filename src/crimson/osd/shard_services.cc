...
ShardServices::ShardServices(OSDMapService& osdmap_service, const int whoami, crimson::net::Messenger& cluster_msgr,
                             crimson::net::Messenger& public_msgr, crimson::mon::Client& monc,
                             crimson::mgr::Client& mgrc, crimson::os::FuturizedStore& store)
...
{
    static const char* KEYS[] = {"osd_max_backfills", "osd_min_recovery_priority", nullptr};
    return KEYS;
}

void ShardServices::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
...
seastar::future<> ShardServices::send_to_osd(int peer, Ref<Message> m, epoch_t from_epoch)
{
...
{
    auto ret = store.do_transaction(col, std::move(ctx.transaction));
...
seastar::future<> ShardServices::dispatch_context_messages(BufferedRecoveryMessages&& ctx)
{
    auto ret = seastar::parallel_for_each(std::move(ctx.message_map), [this](auto& osd_messages) {
        ...
        return seastar::parallel_for_each(
            std::move(messages), [=, peer = peer](auto& m) { return send_to_osd(peer, m, osdmap->get_epoch()); });
    });
...
seastar::future<> ShardServices::dispatch_context(crimson::os::CollectionRef col, PeeringCtx&& ctx)
{
    ...
    return seastar::when_all_succeed(dispatch_context_messages(BufferedRecoveryMessages{ceph_release_t::octopus, ctx}),
                                     col ? dispatch_context_transaction(col, ctx) : seastar::now())
        .then_unpack([] { return seastar::now(); });
}

void ShardServices::queue_want_pg_temp(pg_t pgid, const vector<int>& want, bool forced)
{
...
std::ostream& operator<<(std::ostream& out, const ShardServices::pg_temp_t& pg_temp)
{
    out << pg_temp.acting;
    if (pg_temp.forced) {
        out << " (forced)";
...
seastar::future<> ShardServices::send_pg_created()
{
    ...
    ceph_assert(o->require_osd_release >= ceph_release_t::luminous);
    return seastar::parallel_for_each(
        pg_created, [this](auto& pgid) { return monc.send_message(make_message<MOSDPGCreated>(pgid)); });
}
...
seastar::future<> ShardServices::send_alive(const epoch_t want)
{
    ...
    if (const epoch_t up_thru = osdmap->get_up_thru(whoami); up_thru_wanted > up_thru) {
        ...
        return monc.send_message(make_message<MOSDAlive>(osdmap->get_epoch(), want));
    }
    else {
