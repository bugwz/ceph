...
#define FORWARD_TO_OSD_SINGLETON(METHOD) FORWARD(METHOD, METHOD, get_osd_singleton_state())

public:
    using cached_map_t = OSDMapService::cached_map_t;
    using local_cached_map_t = OSDMapService::local_cached_map_t;

    PGShardManager(seastar::sharded<OSDSingletonState>& osd_singleton_state,
                   seastar::sharded<ShardServices>& shard_services,
                   seastar::sharded<PGShardMapping>& pg_to_shard_mapping)
    ...
    auto& get_shard_services() { return shard_services.local(); }
    auto& get_shard_services() const { return shard_services.local(); }
    ...
    {
        get_osd_singleton_state().update_map(make_local_shared_foreign(local_cached_map_t(map)));
         ...
         * up front. */
        return seastar::do_with(std::vector<seastar::foreign_ptr<local_cached_map_t>>(), [this, map](auto& fmaps) {
            fmaps.resize(seastar::smp::count);
            for (auto& i : fmaps) {
                i = seastar::foreign_ptr(map);
            }
            return shard_services.invoke_on_all([&fmaps](auto& local) mutable {
                local.local_state.update_map(make_local_shared_foreign(std::move(fmaps[seastar::this_shard_id()])));
            });
        });
    }
    ...
    {
        return shard_services.invoke_on_all([](auto& local) { return local.local_state.stop_registry(); });
    }
    ...
    template<typename F> auto with_remote_shard_state(core_id_t core, F&& f)
    {
        return shard_services.invoke_on(core, [f = std::move(f)](auto& target_shard_services) mutable {
            return std::invoke(std::move(f), target_shard_services.local_state, target_shard_services);
        });
    }
    ...
    template<typename T, typename F> auto with_remote_shard_state_and_op(core_id_t core, typename T::IRef&& op, F&& f)
    {
        if (seastar::this_shard_id() == core) {
            ...
            return std::invoke(std::move(f), target_shard_services.local_state, target_shard_services, std::move(op));
        }
        return op->prepare_remote_submission().then(
            [op = std::move(op), f = std::move(f), this, core](auto f_conn) mutable {
                return shard_services.invoke_on(
                    ...
                    [f = std::move(f), op = std::move(op), f_conn = std::move(f_conn)](
                        ...
                        return std::invoke(
                            std::move(f), target_shard_services.local_state, target_shard_services, std::move(op));
                    });
            });
    }

    /// Runs opref on the appropriate core, creating the pg as necessary.
    template<typename T> seastar::future<> run_with_pg_maybe_create(typename T::IRef op)
    {
        ...
        return get_pg_to_shard_mapping()
            .maybe_create_pg(op->get_pgid())
            .then([this, op = std::move(op)](auto core) mutable {
                return this->template with_remote_shard_state_and_op<T>(
                    core,
                    std::move(op),
                    [](PerShardState& per_shard_state, ShardServices& shard_services, typename T::IRef op) {
                        per_shard_state.registry.add_to_registry(*op);
                        auto& logger = crimson::get_logger(ceph_subsys_osd);
                        auto& opref = *op;
                        return opref
                            .template with_blocking_event<PGMap::PGCreationBlockingEvent>(
                                [&shard_services, &opref](auto&& trigger) {
                                    return shard_services.get_or_create_pg(
                                        std::move(trigger), opref.get_pgid(), std::move(opref.get_create_info()));
                                })
                            .safe_then([&logger, &shard_services, &opref](Ref<PG> pgref) {
                                logger.debug("{}: have_pg", opref);
                                return opref.with_pg(shard_services, pgref);
                            })
                            .handle_error(crimson::ct_error::ecanceled::handle([&logger, &opref](auto) {
    ...
    template<typename T> seastar::future<> run_with_pg_maybe_wait(typename T::IRef op)
    {
        ceph_assert(op->use_count() == 1);
        auto& logger = crimson::get_logger(ceph_subsys_osd);
        static_assert(!T::can_create());
                ...
                return this->template with_remote_shard_state_and_op<T>(
                    ...
                    [](PerShardState& per_shard_state, ShardServices& shard_services, typename T::IRef op) {
                        ...
                        return opref
                            .template with_blocking_event<PGMap::PGCreationBlockingEvent>(
                                [&shard_services, &opref](auto&& trigger) {
                                    return shard_services.wait_for_pg(std::move(trigger), opref.get_pgid());
                                })
                            .safe_then([&logger, &shard_services, &opref](Ref<PG> pgref) {
                                logger.debug("{}: have_pg", opref);
                                return opref.with_pg(shard_services, pgref);
     ...
     */
    template<typename F, typename... Args> seastar::future<> invoke_on_each_shard_seq(F&& f) const
    {
        return sharded_map_seq(shard_services, [f = std::forward<F>(f)](const ShardServices& shard_services) mutable {
     ...
     */
    template<typename F> seastar::future<> for_each_pg(F&& f) const
    {
        return invoke_on_each_shard_seq([f = std::move(f)](const auto& local_service) mutable {
     ...
     */
    template<typename F> void for_each_pgid(F&& f) const
    {
        return get_pg_to_shard_mapping().for_each_pgid(std::forward<F>(f));
    }

    auto get_num_pgs() const { return get_pg_to_shard_mapping().get_num_pgs(); }

    seastar::future<> broadcast_map_to_pgs(epoch_t epoch);

    template<typename F> auto with_pg(spg_t pgid, F&& f)
    {
        core_id_t core = get_pg_to_shard_mapping().get_pg_mapping(pgid);
        return with_remote_shard_state(core, [pgid, f = std::move(f)](auto& local_state, auto& local_service) mutable {
            return std::invoke(std::move(f), local_state.pg_map.get_pg(pgid));
        });
    }

    template<typename T, typename... Args> auto start_pg_operation(Args&&... args)
    {
        auto op = get_local_state().registry.create_operation<T>(std::forward<Args>(args)...);
        auto& logger = crimson::get_logger(ceph_subsys_osd);
        ...
        auto fut =
            opref.template enter_stage<>(opref.get_connection_pipeline().await_active)
                ...
                .then([&logger, &opref] {
                    ...
                    return opref.template enter_stage<>(opref.get_connection_pipeline().await_map);
                })
                .then([this, &logger, &opref] {
                    logger.debug("{}: start_pg_operation await_map stage", opref);
                    using OSDMapBlockingEvent = OSD_OSDMapGate::OSDMapBlocker::BlockingEvent;
                    return opref.template with_blocking_event<OSDMapBlockingEvent>([this, &opref](auto&& trigger) {
                        std::ignore = this;
                        return get_shard_services().local_state.osdmap_gate.wait_for_map(
                            std::move(trigger), opref.get_epoch(), &get_shard_services());
                    });
                })
                .then([&logger, &opref](auto epoch) {
                    logger.debug("{}: got map {}, entering get_pg", opref, epoch);
                    return opref.template enter_stage<>(opref.get_connection_pipeline().get_pg);
                })
