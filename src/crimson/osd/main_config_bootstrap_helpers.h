...
struct early_config_t
{
    ...
    auto to_ptr_vector(const std::vector<std::string>& in)
    {
        ...
        ret.reserve(in.size());
        std::transform(
            std::begin(in), std::end(in), std::back_inserter(ret), [](const auto& str) { return str.c_str(); });
        return ret;
    }

    std::vector<const char*> get_early_args() { return to_ptr_vector(early_args); }

    std::vector<const char*> get_ceph_args() { return to_ptr_vector(ceph_args); }
