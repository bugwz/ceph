...
static inline int execute_osd_op(cls_method_context_t hctx, OSDOp& op)
{
    ...
    reinterpret_cast<crimson::osd::OpsExecuter*>(hctx)
        ->execute_op(op)
        .handle_error_interruptible(osd_op_errorator::all_same_way([&ret](const std::error_code& err) {
            assert(err.value() > 0);
            ret = -err.value();
            return seastar::now();
        }))
...
int cls_get_request_origin(cls_method_context_t hctx, entity_inst_t* origin)
{
    ...
    try {
        const auto& message = reinterpret_cast<crimson::osd::OpsExecuter*>(hctx)->get_message();
...
int cls_cxx_map_get_keys(cls_method_context_t hctx, const std::string& start_obj, const uint64_t max_to_get,
                         std::set<std::string>* const keys, bool* const more)
{
...
int cls_cxx_map_get_vals(cls_method_context_t hctx, const std::string& start_obj, const std::string& filter_prefix,
                         const uint64_t max_to_get, std::map<std::string, ceph::bufferlist>* vals, bool* const more)
{
...
int cls_cxx_map_get_vals_by_keys(cls_method_context_t hctx, const std::set<std::string>& keys,
...
int cls_cxx_map_set_vals(cls_method_context_t hctx, const std::map<string, ceph::bufferlist>* map)
{
    OSDOp op{CEPH_OSD_OP_OMAPSETVALS};
...
int cls_cxx_map_remove_range(cls_method_context_t hctx, const std::string& key_begin, const std::string& key_end)
{
...
int cls_get_snapset_seq(cls_method_context_t hctx, uint64_t* snap_seq)
{
    auto* ox = reinterpret_cast<crimson::osd::OpsExecuter*>(hctx);
    auto obc = ox->get_obc();
    if (!obc->obs.exists || (obc->obs.oi.is_whiteout() && obc->ssc->snapset.clones.empty())) {
        return -ENOENT;
    }
