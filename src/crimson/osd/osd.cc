...
OSD::OSD(int id, uint32_t nonce, crimson::net::MessengerRef cluster_msgr, crimson::net::MessengerRef public_msgr,
    ...
    , store{crimson::os::FuturizedStore::create(local_conf().get_val<std::string>("osd_objectstore"),
...
{
    return store->start()
        .then([this, osd_uuid] { return store->mkfs(osd_uuid); })
        .then([this] { return store->mount(); })
        .then([cluster_fsid, this] {
            ...
            logger().info(
                "{} writing superblock cluster_fsid {} osd_fsid {}", __func__, cluster_fsid, superblock.osd_fsid);
        ...
        .then([cluster_fsid, this] {
            return when_all_succeed(store->write_meta("ceph_fsid", cluster_fsid.to_string()),
...
seastar::future<> OSD::start()
{
    ...
    return store->start()
        .then([this] { return store->mount(); })
        .then([this] { return store->open_collection(coll_t::meta()); })
        .then([this](auto ch) {
            meta_coll = make_unique<OSDMeta>(ch, store.get());
        ...
        .then([this] { return heartbeat->start(public_msgr->get_myaddrs(), cluster_msgr->get_myaddrs()); })
        .then([this] {
...
seastar::future<> OSD::stop()
{
    ...
    return prepare_to_stop().then([this] {
        ...
        return asok->stop()
            ...
            .then([this] {
                return seastar::parallel_for_each(pg_map.get_pgs(), [](auto& p) { return p.second->stop(); });
            })
            .then([this] { return monc->stop(); })
            .then([this] { return mgrc->stop(); })
            .then([fut = std::move(gate_close_fut)]() mutable { return std::move(fut); })
            .then([this] { return when_all_succeed(public_msgr->shutdown(), cluster_msgr->shutdown()); })
            .then_unpack([] { return seastar::now(); })
...
void OSD::print(std::ostream& out) const
{
    out << "{osd." << superblock.whoami << " " << superblock.osd_fsid << " [" << superblock.oldest_map << ","
        ...
        << "tlb:" << superblock.cluster_osdmap_trim_lower_bound << pg_map.get_pgs().size() << " pgs}";
}
...
seastar::future<Ref<PG>> OSD::make_pg(cached_map_t create_map, spg_t pgid, bool do_create)
{
    ...
    auto get_pool_info = [create_map, pgid, this] {
        if (create_map->have_pg_pool(pgid.pool())) {
            ...
            return seastar::make_ready_future<std::tuple<pg_pool_t, string, ec_profile_t>>(
                std::make_tuple(std::move(pi), std::move(name), std::move(ec_profile)));
        }
        else {
    ...
    return seastar::when_all(std::move(get_pool_info), std::move(get_collection))
...
seastar::future<Ref<PG>> OSD::load_pg(spg_t pgid)
{
    ...
    return seastar::do_with(PGMeta(store.get(), pgid), [](auto& pg_meta) { return pg_meta.get_epoch(); })
        .then([this](epoch_t e) { return get_map(e); })
        .then([pgid, this](auto&& create_map) { return make_pg(std::move(create_map), pgid, false); })
        .then([this](Ref<PG> pg) {
...
std::optional<seastar::future<>> OSD::ms_dispatch(crimson::net::ConnectionRef conn, MessageRef m)
{
    ...
    gate.dispatch_in_background(__func__, *this, [this, conn, &m, &dispatched] {
        switch (m->get_type()) {
        case CEPH_MSG_OSD_MAP: return handle_osd_map(conn, boost::static_pointer_cast<MOSDMap>(m));
        case CEPH_MSG_OSD_OP: return handle_osd_op(conn, boost::static_pointer_cast<MOSDOp>(m));
        case MSG_OSD_PG_CREATE2:
            shard_services.start_operation<CompoundPeeringRequest>(*this, conn, m);
            return seastar::now();
        ...
        default: dispatched = false; return seastar::now();
        }
    });
...
seastar::future<OSD::cached_map_t> OSD::get_map(epoch_t e)
{
    ...
    if (auto found = osdmaps.find(e); found) {
        ...
        return load_map(e).then([e, this](unique_ptr<OSDMap> osdmap) {
            return seastar::make_ready_future<cached_map_t>(osdmaps.insert(e, std::move(osdmap)));
        });
    }
}

void OSD::store_map_bl(ceph::os::Transaction& t, epoch_t e, bufferlist&& bl)
{
...
seastar::future<std::map<epoch_t, bufferlist>> OSD::load_map_bls(epoch_t first, epoch_t last)
{
    return seastar::map_reduce(
        boost::make_counting_iterator<epoch_t>(first),
        boost::make_counting_iterator<epoch_t>(last + 1),
        [this](epoch_t e) {
            return load_map_bl(e).then([e](auto&& bl) {
                return seastar::make_ready_future<pair<epoch_t, bufferlist>>(std::make_pair(e, std::move(bl)));
            });
        },
...
seastar::future<> OSD::store_maps(ceph::os::Transaction& t, epoch_t start, Ref<MOSDMap> m)
{
    return seastar::do_for_each(boost::make_counting_iterator(start),
                                ...
                                [&t, m, this](epoch_t e) {
                                    ...
                                    else if (auto p = m->incremental_maps.find(e); p != m->incremental_maps.end()) {
...
{
    if (!conn->peer_is_mon()) {
        logger().info("{} received from non-mon {}, {}", __func__, conn->get_peer_addr(), *m);
        return false;
    }
    return true;
}

seastar::future<Ref<PG>> OSD::handle_pg_create_info(std::unique_ptr<PGCreateInfo> info)
{
    return seastar::do_with(std::move(info), [this](auto& info) -> seastar::future<Ref<PG>> {
        return get_map(info->epoch)
            .then([&info, this](cached_map_t startmap) -> seastar::future<std::tuple<Ref<PG>, cached_map_t>> {
                const spg_t& pgid = info->pgid;
                if (info->by_mon) {
                    int64_t pool_id = pgid.pgid.pool();
                    const pg_pool_t* pool = osdmap->get_pg_pool(pool_id);
                    if (!pool) {
                        logger().debug("{} ignoring pgid {}, pool dne", __func__, pgid);
                    ...
                    if (!pool->has_flag(pg_pool_t::FLAG_CREATING)) {
                        ...
                        logger().debug("{} dropping {} create, pool does not have CREATING flag set", __func__, pgid);
                        return seastar::make_ready_future<std::tuple<Ref<PG>, cached_map_t>>(
                            std::make_tuple(Ref<PG>(), startmap));
                    }
                }
                return make_pg(startmap, pgid, true).then([startmap = std::move(startmap)](auto pg) mutable {
                    return seastar::make_ready_future<std::tuple<Ref<PG>, cached_map_t>>(
                        std::make_tuple(std::move(pg), std::move(startmap)));
                });
            })
            .then([this, &info](auto&& ret) -> seastar::future<Ref<PG>> {
                auto [pg, startmap] = std::move(ret);
                if (!pg) return seastar::make_ready_future<Ref<PG>>(Ref<PG>());
                PeeringCtx rctx{ceph_release_t::octopus};
                const pg_pool_t* pp = startmap->get_pg_pool(info->pgid.pool());

                int up_primary, acting_primary;
                vector<int> up, acting;
                startmap->pg_to_up_acting_osds(info->pgid.pgid, &up, &up_primary, &acting, &acting_primary);

                int role = startmap->calc_pg_role(pg_shard_t(whoami, info->pgid.shard), acting);

                create_pg_collection(rctx.transaction, info->pgid, info->pgid.get_split_bits(pp->get_pg_num()));
                init_pg_ondisk(rctx.transaction, info->pgid, pp);
                ...
                return shard_services
                    .start_operation<PGAdvanceMap>(
                        *this, pg, pg->get_osdmap_epoch(), osdmap->get_epoch(), std::move(rctx), true)
                    .second.then([pg = pg] { return seastar::make_ready_future<Ref<PG>>(pg); });
            });
    });
}

seastar::future<> OSD::handle_osd_map(crimson::net::ConnectionRef conn, Ref<MOSDMap> m)
{
    ...
    if (first > start) {
        ...
        if (m->cluster_osdmap_trim_lower_bound < first) {
            return shard_services.osdmap_subscribe(m->cluster_osdmap_trim_lower_bound - 1, true);
        }
        skip_maps = true;
        start = first;
    }
    ...
    return seastar::do_for_each(boost::make_counting_iterator(first),
                                ...
                                [this](epoch_t cur) {
                                    return get_map(cur).then([this](cached_map_t&& o) {
                                        ...
                                        if (up_epoch == 0 && osdmap->is_up(whoami) &&
        ...
        .then([m, this] {
            if (osdmap->is_up(whoami) && osdmap->get_addrs(whoami) == public_msgr->get_myaddrs() &&
                ...
                if (state.is_booting()) {
                    ...
                    tick_timer.arm_periodic(std::chrono::seconds(TICK_INTERVAL));
                }
            }
...
{
    (void)shard_services.start_operation<ClientRequest>(*this, conn, std::move(m));
    return seastar::now();
}

seastar::future<> OSD::send_incremental_map(crimson::net::ConnectionRef conn, epoch_t first)
{
    if (first >= superblock.oldest_map) {
        return load_map_bls(first, superblock.newest_map).then([this, conn, first](auto&& bls) {
            auto m = make_message<MOSDMap>(monc->get_fsid(), osdmap->get_encoding_features());
        ...
        return load_map_bl(osdmap->get_epoch()).then([this, conn](auto&& bl) mutable {
            auto m = make_message<MOSDMap>(monc->get_fsid(), osdmap->get_encoding_features());
...
{
    m->finish_decode();
    (void)shard_services.start_operation<RepRequest>(*this, std::move(conn), std::move(m));
    return seastar::now();
}
...
seastar::future<> OSD::handle_scrub(crimson::net::ConnectionRef conn, Ref<MOSDScrub2> m)
{
    ...
    return seastar::parallel_for_each(std::move(m->scrub_pgs), [m, conn, this](spg_t pgid) {
        ...
        return shard_services
            .start_operation<RemotePeeringEvent>(
                *this, conn, shard_services, from_shard, pgid, PGPeeringEvent{m->epoch, m->epoch, scrub_request})
            .second;
    });
}
...
{
    (void)shard_services.start_operation<RecoverySubRequest>(*this, conn, std::move(m));
    return seastar::now();
}
...
void OSD::update_heartbeat_peers()
{
    ...
    for (auto& pg : pg_map.get_pgs()) {
        vector<int> up, acting;
        osdmap->pg_to_up_acting_osds(pg.first.pgid, &up, nullptr, &acting, nullptr);
...
seastar::future<> OSD::handle_peering_op(crimson::net::ConnectionRef conn, Ref<MOSDPeeringOp> m)
{
    ...
    (void)shard_services.start_operation<RemotePeeringEvent>(
        *this, conn, shard_services, pg_shard_t{from, m->get_spg().shard}, m->get_spg(), std::move(*evt));
    return seastar::now();
}
...
seastar::future<> OSD::consume_map(epoch_t epoch)
{
    ...
    return seastar::parallel_for_each(pgs.begin(),
                                      pgs.end(),
                                      [=](auto& pg) {
                                          return shard_services
                                              .start_operation<PGAdvanceMap>(*this,
                                                                             pg.second,
                                                                             pg.second->get_osdmap_epoch(),
                                                                             epoch,
                                                                             PeeringCtx{ceph_release_t::octopus},
                                                                             false)
                                              .second;
                                      })
        .then([epoch, this] {
...
blocking_future<Ref<PG>> OSD::get_or_create_pg(spg_t pgid, epoch_t epoch, std::unique_ptr<PGCreateInfo> info)
{
    if (info) {
...
blocking_future<Ref<PG>> OSD::wait_for_pg(spg_t pgid)
{
...
seastar::future<> OSD::prepare_to_stop()
{
    if (osdmap && osdmap->is_up(whoami)) {
        ...
        return seastar::with_timeout(
                   ...
                   monc
                       ->send_message(make_message<MOSDMarkMeDown>(
