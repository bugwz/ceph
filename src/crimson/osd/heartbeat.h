...
public:
    ...
    Heartbeat(osd_id_t whoami, crimson::osd::ShardServices& service, crimson::mon::Client& monc,
...
private:
    seastar::future<> handle_osd_ping(crimson::net::ConnectionRef conn, Ref<MOSDPing> m);
    seastar::future<> handle_ping(crimson::net::ConnectionRef conn, Ref<MOSDPing> m);
    seastar::future<> handle_reply(crimson::net::ConnectionRef conn, Ref<MOSDPing> m);
    ...
    seastar::future<> start_messenger(crimson::net::Messenger& msgr, const entity_addrvec_t& addrs);
    seastar::future<> maybe_share_osdmap(crimson::net::ConnectionRef, Ref<MOSDPing> m);
...
private:
    ...
    public:
        FailingPeers(Heartbeat& heartbeat)
            : heartbeat(heartbeat)
        {}
        bool add_pending(osd_id_t peer, clock::time_point failed_since, clock::time_point now,
...
public:
    ...
    Connection(osd_id_t peer, bool is_winner_side, type_t type, crimson::net::Messenger& msgr,
...
public:
    ...
    void on_ping(const utime_t& sent_stamp, const clock::time_point& deadline)
    {
        assert(connected);
        [[maybe_unused]] auto [reply, added] = ping_history.emplace(sent_stamp, reply_t{deadline, 2});
    }

    bool on_pong(const utime_t& ping_stamp, Connection::type_t type, clock::time_point now)
    {
        assert(connected);
...
public:
    ...
    clock::time_point failed_since(clock::time_point now) const { return session.failed_since(now); }
    void send_heartbeat(clock::time_point, ceph::signedspan, std::vector<seastar::future<>>&);
...
private:
    ...
    void do_send_heartbeat(clock::time_point, ceph::signedspan, std::vector<seastar::future<>>*);

    template<typename Func> void for_each_conn(Func&& f)
    {
        f(con_front);
        f(con_back);
    }
    ...
    friend std::ostream& operator<<(std::ostream& os, const Peer& p) { return os << "peer(osd." << p.peer << ")"; }
};
