...
void OSDMeta::store_map(ceph::os::Transaction& t, epoch_t e, const bufferlist& m)
{
...
{
    return store->read(coll, osdmap_oid(e), 0, 0, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED)
        .handle_error(read_errorator::all_same_way(
            [e] { throw std::runtime_error(fmt::format("read gave enoent on {}", osdmap_oid(e))); }));
}

void OSDMeta::store_superblock(ceph::os::Transaction& t, const OSDSuperblock& superblock)
{
    bufferlist bl;
    ...
    return store->read(coll, superblock_oid(), 0, 0)
        .safe_then(
            ...
            read_errorator::all_same_way(
                [] { throw std::runtime_error(fmt::format("read gave enoent on {}", superblock_oid())); }));
}

seastar::future<std::tuple<pg_pool_t, std::string, OSDMeta::ec_profile_t>> OSDMeta::load_final_pool_info(int64_t pool)
{
    return store->read(coll, final_pool_info_oid(pool), 0, 0)
        .safe_then(
            [](bufferlist&& bl) {
                auto p = bl.cbegin();
                pg_pool_t pi;
            ...
            read_errorator::all_same_way([pool] {
                throw std::runtime_error(fmt::format("read gave enoent on {}", final_pool_info_oid(pool)));
            }));
}
