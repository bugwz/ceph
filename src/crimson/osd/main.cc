...
seastar::future<> make_keyring()
{
    ...
    return seastar::file_exists(path)
        .then([path](bool exists) {
            KeyRing keyring;
            EntityName name{local_conf()->name};
            EntityAuth auth;
            if (exists && keyring.load(nullptr, path) == 0 && keyring.get_auth(name, auth)) {
                ...
                const auto permissions = (seastar::file_permissions::user_read | seastar::file_permissions::user_write);
...
int main(int argc, const char* argv[])
{
    ...
    app.add_options()("mkkey",
                      "generate a new secret key. "
                      "This is normally used in combination with --mkfs")("mkfs", "create a [new] data directory")(
        "debug", "enable debug output on all loggers")("trace", "enable trace output on all loggers")(
        "osdspec-affinity", bpo::value<std::string>()->default_value(std::string{}), "set affinity to an osdspec")(
        "prometheus_port", bpo::value<uint16_t>()->default_value(0), "Prometheus port. Set to zero to disable")(
        "prometheus_address", bpo::value<std::string>()->default_value("0.0.0.0"), "Prometheus listening address")(
        "prometheus_prefix", bpo::value<std::string>()->default_value("osd"), "Prometheus metrics prefix");
            ...
            return seastar::async([&] {
                try {
                    ...
                    if (config.count("trace")) {
                        seastar::global_logger_registry().set_all_loggers_level(seastar::log_level::trace);
                    }
                    sharded_conf().start(early_config.init_params.name, early_config.cluster_name).get();
                    ...
                    auto reset_logger = seastar::defer([] { logger().set_ostream(std::cerr); });
                    if (const auto ret = pidfile_write(local_conf()->pid_file); ret == -EACCES || ret == -EAGAIN) {
                        ceph_abort_msg("likely there is another crimson-osd instance with the same id");
                    }
                    else if (ret < 0) {
                        ceph_abort_msg(fmt::format("pidfile_write failed with {} {}", ret, cpp_strerror(-ret)));
                    }
                    ...
                    if (uint16_t prom_port = config["prometheus_port"].as<uint16_t>(); prom_port != 0) {
                    ...
                    for (auto [msgr, name] : {make_pair(std::ref(cluster_msgr), "cluster"s),
                        ...
                        msgr = crimson::net::Messenger::create(entity_name_t::OSD(whoami), name, nonce, true);
                    }
                    auto store =
                        crimson::os::FuturizedStore::create(local_conf().get_val<std::string>("osd_objectstore"),
                                                            local_conf().get_val<std::string>("osd_data"),
                                                            local_conf().get_config_values());

                    crimson::osd::OSD osd(whoami,
                                          nonce,
                                          std::ref(should_stop.abort_source()),
                                          std::ref(*store),
                                          cluster_msgr,
                                          client_msgr,
                                          hb_front_msgr,
                    ...
                    if (config.count("mkfs")) {
                        ...
                        osd.mkfs(*store,
                                 ...
                                 config["osdspec-affinity"].as<std::string>())
                            .get();
                    }
