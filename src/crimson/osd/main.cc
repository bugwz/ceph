...
auto partition_args(seastar::app_template& app, char** argv_begin, char** argv_end)
{
    ...
    auto parsed = bpo::command_line_parser(std::distance(argv_begin, argv_end), argv_begin)
                      .options(app.get_options_description())
                      .allow_unregistered()
                      .run();
    auto unknown_args = bpo::collect_unrecognized(parsed.options, bpo::include_positional);
    ...
    auto consume_unknown_arg = [&](char** argv) {
        for (; unknown != unknown_args.end() && argv != argv_end && *unknown == *argv; ++argv, ++unknown) {
...
int main(int argc, char* argv[])
{
    ...
    auto init_params = ceph_argparse_early_args(ceph_args, CEPH_ENTITY_TYPE_OSD, &cluster_name, &conf_file_list);
    seastar::sharded<crimson::osd::OSD> osd;
    using crimson::common::sharded_conf;
    using crimson::common::sharded_perf_coll;
    try {
        return app.run_deprecated(app_args.size(), const_cast<char**>(app_args.data()), [&, &ceph_args = ceph_args] {
            ...
            return seastar::async([&] {
                ...
                osd.start_single(whoami, nonce, cluster_msgr, client_msgr, hb_front_msgr, hb_back_msgr).get();
                if (config.count("mkkey")) {
                    make_keyring().handle_exception([](std::exception_ptr) { seastar::engine().exit(1); }).get();
                }
                ...
                seastar::engine().at_exit([&] { return osd.stop(); });
