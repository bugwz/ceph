...
seastar::future<> Watch::send_notify_msg(NotifyRef notify)
{
    ...
    return conn->send(make_message<MWatchNotify>(winfo.cookie,
                                                 notify->user_version,
...
seastar::future<> Watch::send_disconnect_msg()
{
    ...
    return conn->send(make_message<MWatchNotify>(winfo.cookie, 0, 0, CEPH_WATCH_EVENT_DISCONNECT, empty));
}
...
{
    logger().info("{}", __func__);
    auto disconnected = send_disconnect ? send_disconnect_msg() : seastar::now();
    return std::move(disconnected).then([this] {
        return seastar::do_for_each(
                   in_progress_notifies,
                   [this_shared = shared_from_this()](auto notify) { return notify->remove_watcher(this_shared); })
            .then([this] {
                discard_state();
                return seastar::now();
            });
