...
namespace {
template<typename T> std::optional<T> find_value(const FuturizedStore::omap_values_t& values, string_view key)
{
    ...
    return store->open_collection(coll_t{pgid}).then([this](auto ch) {
        return store->omap_get_values(ch, pgid.make_pgmeta_oid(), {string{infover_key}, string{epoch_key}})
            .safe_then(
                ...
                FuturizedStore::read_errorator::assert_all{"PGMeta::get_epoch: unable to read pgmeta"});
    });
}
...
seastar::future<std::tuple<pg_info_t, PastIntervals>> PGMeta::load()
{
    return store->open_collection(coll_t{pgid})
        ...
        .safe_then(
            ...
            FuturizedStore::read_errorator::assert_all{"PGMeta::load: unable to read pgmeta"});
}
