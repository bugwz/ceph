...
BackfillState::BackfillState(BackfillState::BackfillListener& backfill_listener,
                             std::unique_ptr<BackfillState::PeeringFacade> peering_state,
                             std::unique_ptr<BackfillState::PGFacade> pg)
    : backfill_machine(*this, backfill_listener, std::move(peering_state), std::move(pg))
    , progress_tracker(std::make_unique<BackfillState::ProgressTracker>(backfill_machine))
{
    logger().debug("{}:{}", __func__, __LINE__);
    backfill_machine.initiate();
}

template<class S> BackfillState::StateHelper<S>::StateHelper()
{
    logger().debug("enter {}", boost::typeindex::type_id<S>().pretty_name());
}

template<class S> BackfillState::StateHelper<S>::~StateHelper()
{
    logger().debug("exit {}", boost::typeindex::type_id<S>().pretty_name());
}

BackfillState::~BackfillState() = default;

BackfillState::BackfillMachine::BackfillMachine(BackfillState& backfill_state,
    ...
    : my_base(ctx)
{
    ...
    logger().debug(
        "{}: bft={} from {}", __func__, peering_state().get_backfill_targets(), backfill_state().last_backfill_started);
    for (const auto& bt : peering_state().get_backfill_targets()) {
        logger().debug("{}: target shard {} from {}", __func__, bt, peering_state().get_peer_last_backfill(bt));
    }
    ceph_assert(peering_state().get_backfill_targets().size());
    ceph_assert(!backfill_state().last_backfill_started.is_max());
}

boost::statechart::result BackfillState::Initial::react(const BackfillState::Triggered& evt)
{
    logger().debug("{}: backfill triggered", __func__);
    ceph_assert(backfill_state().last_backfill_started == peering_state().earliest_backfill());
    ceph_assert(peering_state().is_backfilling());
    // initialize BackfillIntervals
    for (const auto& bt : peering_state().get_backfill_targets()) {
        backfill_state().peer_backfill_info[bt].reset(peering_state().get_peer_last_backfill(bt));
    }
    backfill_state().backfill_info.reset(backfill_state().last_backfill_started);
    if (Enqueuing::all_enqueued(peering_state(), backfill_state().backfill_info, backfill_state().peer_backfill_info)) {
...
{
    if (auto& primary_bi = backfill_state().backfill_info; primary_bi.version >= pg().get_projected_last_update()) {
    ...
    if (peering_state().get_pg_log().get_log().empty() &&
      ...
      return;
        ...
        peering_state().scan_log_after(primary_bi.version, [&](const pg_log_entry_t& e) {
            logger().debug("maybe_update_range(lambda): updating from version {}", e.version);
            if (e.soid >= primary_bi.begin && e.soid < primary_bi.end) {
                if (e.is_update()) {
                    logger().debug("maybe_update_range(lambda): {} updated to ver {}", e.soid, e.version);
                    primary_bi.objects.erase(e.soid);
                    primary_bi.objects.insert(std::make_pair(e.soid, e.version));
                }
                else if (e.is_delete()) {
                    logger().debug("maybe_update_range(lambda): {} removed", e.soid);
                    primary_bi.objects.erase(e.soid);
                }
            }
        });
        ...
        ceph_abort_msg("scan_range should have raised primary_bi.version past log_tail");
    }
}
...
void BackfillState::Enqueuing::trim_backfill_infos()
{
    for (const auto& bt : peering_state().get_backfill_targets()) {
        backfill_state().peer_backfill_info[bt].trim_to(
            std::max(peering_state().get_peer_last_backfill(bt), backfill_state().last_backfill_started));
    }
    backfill_state().backfill_info.trim_to(backfill_state().last_backfill_started);
}

/* static */ bool BackfillState::Enqueuing::all_enqueued(
    const PeeringFacade& peering_state, const BackfillInterval& backfill_info,
    const std::map<pg_shard_t, BackfillInterval>& peer_backfill_info)
{
    const bool all_local_enqueued = backfill_info.extends_to_end() && backfill_info.empty();
    const bool all_peer_enqueued =
        std::all_of(std::begin(peer_backfill_info), std::end(peer_backfill_info), [](const auto& kv) {
...
bool BackfillState::Enqueuing::should_rescan_replicas(const std::map<pg_shard_t, BackfillInterval>& peer_backfill_info,
                                                      const BackfillInterval& backfill_info) const
{
    const auto& targets = peering_state().get_backfill_targets();
    return std::any_of(std::begin(targets), std::end(targets), [&](const auto& bt) {
        return ReplicasScanning::replica_needs_scan(peer_backfill_info.at(bt), backfill_info);
    });
}

bool BackfillState::Enqueuing::should_rescan_primary(const std::map<pg_shard_t, BackfillInterval>& peer_backfill_info,
                                                     const BackfillInterval& backfill_info) const
{
    return backfill_info.begin <= earliest_peer_backfill(peer_backfill_info) && !backfill_info.extends_to_end();
}

void BackfillState::Enqueuing::trim_backfilled_object_from_intervals(
    BackfillState::Enqueuing::result_t&& result, hobject_t& last_backfill_started,
    std::map<pg_shard_t, BackfillInterval>& peer_backfill_info)
{
    std::for_each(std::begin(result.pbi_targets), std::end(result.pbi_targets), [&peer_backfill_info](const auto& bt) {
        peer_backfill_info.at(bt).pop_front();
    });
    last_backfill_started = std::move(result.new_last_backfill_started);
}

BackfillState::Enqueuing::result_t BackfillState::Enqueuing::remove_on_peers(const hobject_t& check)
{
    ...
    logger().debug("{}: BACKFILL removing {} from peers {}", __func__, check, result.pbi_targets);
    ceph_assert(!result.pbi_targets.empty());
    return result;
}

BackfillState::Enqueuing::result_t BackfillState::Enqueuing::update_on_peers(const hobject_t& check)
{
...
bool BackfillState::Enqueuing::Enqueuing::all_emptied(
    const BackfillInterval& local_backfill_info, const std::map<pg_shard_t, BackfillInterval>& peer_backfill_info) const
{
    const auto& targets = peering_state().get_backfill_targets();
    const auto replicas_emptied = std::all_of(
        std::begin(targets), std::end(targets), [&](const auto& bt) { return peer_backfill_info.at(bt).empty(); });
    ...
    : my_base(ctx)
{
    ...
    if (primary_bi.version < peering_state().get_log_tail()) {
        ...
        logger().warn("{}: bi is old, rescanning of local backfill_info", __func__);
    ...
    while (!all_emptied(primary_bi, backfill_state().peer_backfill_info)) {
        ...
        else if (should_rescan_replicas(backfill_state().peer_backfill_info, primary_bi)) {
        ...
        if (const hobject_t check = earliest_peer_backfill(backfill_state().peer_backfill_info);
            ...
            trim_backfilled_object_from_intervals(
                std::move(result), backfill_state().last_backfill_started, backfill_state().peer_backfill_info);
        }
        else {
    ...
    if (should_rescan_primary(backfill_state().peer_backfill_info, primary_bi)) {
        ...
        logger().debug("{}: reached end for both local and all peers "
                       "but still has in-flight operations",
                       __func__);
        post_event(RequestWaiting{});
    }
}
    ...
    : my_base(ctx)
{
    ...
    backfill_listener().request_primary_scan(backfill_state().backfill_info.begin);
}

boost::statechart::result BackfillState::PrimaryScanning::react(PrimaryScanned evt)
{
...
boost::statechart::result BackfillState::PrimaryScanning::react(ObjectPushed evt)
{
    logger().debug("PrimaryScanning::react() on ObjectPushed; evt.object={}", evt.object);
    backfill_state().progress_tracker->complete_to(evt.object, evt.stat);
    return discard_event();
}

// -- ReplicasScanning
bool BackfillState::ReplicasScanning::replica_needs_scan(const BackfillInterval& replica_backfill_info,
                                                         const BackfillInterval& local_backfill_info)
{
    return replica_backfill_info.empty() && replica_backfill_info.begin <= local_backfill_info.begin &&
    ...
    : my_base(ctx)
{
    for (const auto& bt : peering_state().get_backfill_targets()) {
        if (const auto& pbi = backfill_state().peer_backfill_info.at(bt);
            ...
            logger().debug("{}: scanning peer osd.{} from {}", __func__, bt, pbi.end);
...
boost::statechart::result BackfillState::ReplicasScanning::react(ReplicaScanned evt)
{
    logger().debug("{}: got scan result from osd={}, result={}", __func__, evt.from, evt.result);
    ...
    if (waiting_on_backfill.erase(evt.from)) {
        ...
        if (waiting_on_backfill.empty()) {
            ceph_assert(backfill_state().peer_backfill_info.size() == peering_state().get_backfill_targets().size());
            return transit<Enqueuing>();
        }
    }
    else {
...
boost::statechart::result BackfillState::ReplicasScanning::react(ObjectPushed evt)
{
    logger().debug("ReplicasScanning::react() on ObjectPushed; evt.object={}", evt.object);
    backfill_state().progress_tracker->complete_to(evt.object, evt.stat);
    return discard_event();
}
...
boost::statechart::result BackfillState::Waiting::react(ObjectPushed evt)
{
    logger().debug("Waiting::react() on ObjectPushed; evt.object={}", evt.object);
    backfill_state().progress_tracker->complete_to(evt.object, evt.stat);
    if (!Enqueuing::all_enqueued(
            peering_state(), backfill_state().backfill_info, backfill_state().peer_backfill_info)) {
        return transit<Enqueuing>();
    }
    else if (backfill_state().progress_tracker->tracked_objects_completed()) {
        return transit<Done>();
...
{
    [[maybe_unused]] const auto [it, first_seen] =
        registry.try_emplace(obj, registry_item_t{op_stage_t::enqueued_push, std::nullopt});
    return first_seen;
}
...
void BackfillState::ProgressTracker::complete_to(const hobject_t& obj, const pg_stat_t& stats)
{
    logger().debug("{}: obj={}", __func__, obj);
    if (auto completion_iter = registry.find(obj); completion_iter != std::end(registry)) {
        completion_iter->second = registry_item_t{op_stage_t::completed_push, stats};
    }
    else {
        ceph_abort_msg("completing untracked object shall not happen");
    }
    for (auto it = std::begin(registry); it != std::end(registry) && it->second.stage != op_stage_t::enqueued_push;
         it = registry.erase(it)) {
        auto& [soid, item] = *it;
        assert(item.stats);
        peering_state().update_complete_backfill_object_stats(soid, *item.stats);
    }
    if (Enqueuing::all_enqueued(peering_state(), backfill_state().backfill_info, backfill_state().peer_backfill_info) &&
