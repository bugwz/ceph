...
OpsExecuter::call_errorator::future<> OpsExecuter::do_op_call(OSDOp& osd_op)
{
  ...
  if (flags & CLS_METHOD_WR) {
    ...
    return seastar::async([this, method, indata = std::move(indata)]() mutable {
        ...
        .then([this, prev_rd, prev_wr, &osd_op, flags](auto outcome) -> call_errorator::future<> {
...
static watch_info_t create_watch_info(const OSDOp& osd_op, const MOSDOp& msg)
{
    using crimson::common::local_conf;
    const uint32_t timeout =
        osd_op.op.watch.timeout == 0 ? local_conf()->osd_client_watch_timeout : osd_op.op.watch.timeout;
    return {osd_op.op.watch.cookie, timeout, msg.get_connection()->get_peer_addr()};
}

OpsExecuter::watch_errorator::future<> OpsExecuter::do_op_watch_subop_watch(OSDOp& osd_op, ObjectState& os,
                                                                            ceph::os::Transaction& txn)
{
    ...
    return with_effect_on_obc(
        connect_ctx_t{osd_op, get_message()},
        [&](auto& ctx) {
...
OpsExecuter::watch_errorator::future<> OpsExecuter::do_op_watch_subop_reconnect(OSDOp& osd_op, ObjectState& os,
                                                                                ceph::os::Transaction& txn)
{
...
OpsExecuter::watch_errorator::future<> OpsExecuter::do_op_watch_subop_unwatch(OSDOp& osd_op, ObjectState& os,
                                                                              ceph::os::Transaction& txn)
{
...
OpsExecuter::watch_errorator::future<> OpsExecuter::do_op_watch_subop_ping(OSDOp& osd_op, ObjectState& os,
                                                                           ceph::os::Transaction& txn)
{
    const entity_name_t& entity = get_message().get_reqid().name;
    const auto& cookie = osd_op.op.watch.cookie;
...
OpsExecuter::watch_errorator::future<> OpsExecuter::do_op_watch(OSDOp& osd_op, ObjectState& os,
                                                                ceph::os::Transaction& txn)
{
...
OpsExecuter::watch_errorator::future<> OpsExecuter::do_op_notify(OSDOp& osd_op, const ObjectState& os)
{
...
OpsExecuter::watch_errorator::future<> OpsExecuter::do_op_notify_ack(OSDOp& osd_op, const ObjectState& os)
{
    logger().debug("{}", __func__);
    ...
    struct notifyack_ctx_t
    {
        ...
        notifyack_ctx_t(const MOSDOp& msg)
...
OpsExecuter::osd_op_errorator::future<> OpsExecuter::execute_op(OSDOp& osd_op)
{
    ...
    case CEPH_OSD_OP_READ:
        return do_read_op([&osd_op](auto& backend, const auto& os) { return backend.read(os, osd_op); });
    case CEPH_OSD_OP_SPARSE_READ:
        return do_read_op([&osd_op](auto& backend, const auto& os) { return backend.sparse_read(os, osd_op); });
    ...
    case CEPH_OSD_OP_GETXATTR:
        return do_read_op([&osd_op](auto& backend, const auto& os) { return backend.getxattr(os, osd_op); });
    case CEPH_OSD_OP_GETXATTRS:
        return do_read_op([&osd_op](auto& backend, const auto& os) { return backend.get_xattrs(os, osd_op); });
    case CEPH_OSD_OP_RMXATTR:
        return do_write_op([&osd_op](auto& backend, auto& os, auto& txn) { return backend.rm_xattr(os, osd_op, txn); },
                           true);
    case CEPH_OSD_OP_CREATE:
        return do_write_op([&osd_op](auto& backend, auto& os, auto& txn) { return backend.create(os, osd_op, txn); },
                           true);
    case CEPH_OSD_OP_WRITE:
        return do_write_op(
            [this, &osd_op](auto& backend, auto& os, auto& txn) {
    ...
    case CEPH_OSD_OP_ZERO:
        return do_write_op(
            ...
            true);
    case CEPH_OSD_OP_SETALLOCHINT: return osd_op_errorator::now();
    case CEPH_OSD_OP_SETXATTR:
        return do_write_op([&osd_op](auto& backend, auto& os, auto& txn) { return backend.setxattr(os, osd_op, txn); },
                           true);
    case CEPH_OSD_OP_DELETE:
        return do_write_op([](auto& backend, auto& os, auto& txn) { return backend.remove(os, txn); }, true);
    case CEPH_OSD_OP_CALL: return this->do_op_call(osd_op);
    case CEPH_OSD_OP_STAT:
        // note: stat does not require RD
        return do_const_op([&osd_op](/* const */ auto& backend, const auto& os) { return backend.stat(os, osd_op); });
    ...
    case CEPH_OSD_OP_OMAPGETKEYS:
        return do_read_op([&osd_op](auto& backend, const auto& os) { return backend.omap_get_keys(os, osd_op); });
    case CEPH_OSD_OP_OMAPGETVALS:
        return do_read_op([&osd_op](auto& backend, const auto& os) { return backend.omap_get_vals(os, osd_op); });
    case CEPH_OSD_OP_OMAPGETHEADER:
        return do_read_op([&osd_op](auto& backend, const auto& os) { return backend.omap_get_header(os, osd_op); });
    case CEPH_OSD_OP_OMAPGETVALSBYKEYS:
        return do_read_op(
            [&osd_op](auto& backend, const auto& os) { return backend.omap_get_vals_by_keys(os, osd_op); });
...
#endif
        return do_write_op(
            [&osd_op](auto& backend, auto& os, auto& txn) { return backend.omap_set_header(os, osd_op, txn); }, true);
...
#endif
        return do_write_op(
            [&osd_op](auto& backend, auto& os, auto& txn) { return backend.omap_remove_range(os, osd_op, txn); }, true);
...
static seastar::future<hobject_t> pgls_filter(const PGLSFilter& filter, const PGBackend& backend, const hobject_t& sobj)
{
    if (const auto xattr = filter.get_xattr(); !xattr.empty()) {
        ...
        return backend.getxattr(sobj, xattr)
            .safe_then(
...
static seastar::future<ceph::bufferlist> do_pgnls_common(const hobject_t& pg_start, const hobject_t& pg_end,
                                                         ...
                                                         const PGLSFilter* const filter)
{
    ...
    return backend.list_objects(lower_bound, limit)
        .then([&backend, filter, nspace](auto&& ret) {
...
static seastar::future<> do_pgnls(const PG& pg, const std::string& nspace, OSDOp& osd_op)
{
...
static seastar::future<> do_pgnls_filtered(const PG& pg, const std::string& nspace, OSDOp& osd_op)
{
...
static seastar::future<ceph::bufferlist> do_pgls_common(const hobject_t& pg_start, const hobject_t& pg_end,
                                                        const PGBackend& backend, const hobject_t& lower_bound,
                                                        const std::string& nspace, const uint64_t limit,
                                                        const PGLSFilter* const filter)
{
    ...
    return backend.list_objects(lower_bound, limit)
        .then([&backend, filter, nspace](auto&& ret) {
            auto& [objects, next] = ret;
            return seastar::when_all(seastar::map_reduce(
                                         std::move(objects),
...
static seastar::future<> do_pgls(const PG& pg, const std::string& nspace, OSDOp& osd_op)
{
    hobject_t lower_bound;
    auto bp = osd_op.indata.cbegin();
    try {
...
static seastar::future<> do_pgls_filtered(const PG& pg, const std::string& nspace, OSDOp& osd_op)
{
    std::string cname, mname, type;
    auto bp = osd_op.indata.cbegin();
    try {
...
seastar::future<> PgOpsExecuter::execute_op(OSDOp& osd_op)
{
