...
Heartbeat::Heartbeat(osd_id_t whoami, crimson::osd::ShardServices& service, crimson::mon::Client& monc,
                     crimson::net::Messenger& front_msgr, crimson::net::Messenger& back_msgr)
    : whoami{whoami}
    , service{service}
    , monc{monc}
...
seastar::future<> Heartbeat::start(entity_addrvec_t front_addrs, entity_addrvec_t back_addrs)
{
    logger().info("heartbeat: start front_addrs={}, back_addrs={}", front_addrs, back_addrs);
    ...
    front_msgr.set_policy(entity_name_t::TYPE_OSD, SocketPolicy::lossy_client(0));
    back_msgr.set_policy(entity_name_t::TYPE_OSD, SocketPolicy::lossy_client(0));
    return seastar::when_all_succeed(start_messenger(front_msgr, front_addrs), start_messenger(back_msgr, back_addrs))
        .then_unpack([this] { timer.arm_periodic(std::chrono::seconds(local_conf()->osd_heartbeat_interval)); });
}

seastar::future<> Heartbeat::start_messenger(crimson::net::Messenger& msgr, const entity_addrvec_t& addrs)
{
    return msgr.bind(addrs).safe_then(
        [this, &msgr]() mutable { return msgr.start({this}); },
        crimson::net::Messenger::bind_ertr::all_same_way([addrs](const std::error_code& e) {
...
seastar::future<> Heartbeat::stop()
{
    ...
    back_msgr.stop();
    return gate.close()
        .then([this] { return seastar::when_all_succeed(front_msgr.shutdown(), back_msgr.shutdown()); })
...
void Heartbeat::add_reporter_peers(int whoami)
{
    ...
    osdmap->get_random_up_osds_by_subtree(whoami, subtree, min_down, want, &want);
...
void Heartbeat::update_peers(int whoami)
{
    ...
    auto osdmap = service.get_map();
    auto epoch = osdmap->get_epoch();
    for (auto next = osdmap->get_next_up_osd_after(whoami); peers.size() < min_peers && next >= 0 && next != whoami;
...
std::optional<seastar::future<>> Heartbeat::ms_dispatch(crimson::net::ConnectionRef conn, MessageRef m)
{
    ...
    gate.dispatch_in_background(__func__, *this, [this, conn, &m, &dispatched] {
        switch (m->get_type()) {
        case MSG_OSD_PING: return handle_osd_ping(conn, boost::static_pointer_cast<MOSDPing>(m));
...
void Heartbeat::ms_handle_reset(crimson::net::ConnectionRef conn, bool is_replace)
{
    ...
    if (conn->get_peer_type() != entity_name_t::TYPE_OSD || peer == entity_name_t::NEW) {
        return;
    }
    if (auto found = peers.find(peer); found != peers.end()) {
        found->second.handle_reset(conn, is_replace);
    }
}

void Heartbeat::ms_handle_connect(crimson::net::ConnectionRef conn, seastar::shard_id prv_shard)
{
...
void Heartbeat::ms_handle_accept(crimson::net::ConnectionRef conn, seastar::shard_id prv_shard, bool is_replace)
{
    ceph_assert_always(seastar::this_shard_id() == prv_shard);
    auto peer = conn->get_peer_id();
    if (conn->get_peer_type() != entity_name_t::TYPE_OSD || peer == entity_name_t::NEW) {
...
seastar::future<> Heartbeat::handle_osd_ping(crimson::net::ConnectionRef conn, Ref<MOSDPing> m)
{
    switch (m->op) {
    case MOSDPing::PING: return handle_ping(conn, m);
    case MOSDPing::PING_REPLY: return handle_reply(conn, m);
    case MOSDPing::YOU_DIED: return handle_you_died();
    default: return seastar::now();
    }
}

seastar::future<> Heartbeat::handle_ping(crimson::net::ConnectionRef conn, Ref<MOSDPing> m)
{
    ...
    auto reply = crimson::make_message<MOSDPing>(m->fsid,
                                                 ...
                                                 min_message);
    return conn->send(std::move(reply)).then([this, m, conn] { return maybe_share_osdmap(conn, m); });
}

seastar::future<> Heartbeat::maybe_share_osdmap(crimson::net::ConnectionRef conn, Ref<MOSDPing> m)
{
...
seastar::future<> Heartbeat::handle_reply(crimson::net::ConnectionRef conn, Ref<MOSDPing> m)
{
    const osd_id_t from = m->get_source().num();
    auto found = peers.find(from);
    if (found == peers.end()) {
    ...
    auto& peer = found->second;
    return peer.handle_reply(conn, m).then([this, conn, m] { return maybe_share_osdmap(conn, m); });
}
...
bool Heartbeat::Connection::accepted(crimson::net::ConnectionRef accepted_conn, bool is_replace)
{
...
Heartbeat::clock::time_point Heartbeat::Session::failed_since(Heartbeat::clock::time_point now) const
{
...
void Heartbeat::Session::set_inactive_history(clock::time_point now)
{
    ...
    if (ping_history.empty()) {
        ...
        const auto deadline = now + std::chrono::seconds(local_conf()->osd_heartbeat_grace);
...
Heartbeat::Peer::Peer(Heartbeat& heartbeat, osd_id_t peer)
    ...
    , peer{peer}
    , session{peer}
    , con_front(peer, heartbeat.whoami > peer, Connection::type_t::front, heartbeat.front_msgr, *this)
    , con_back(peer, heartbeat.whoami > peer, Connection::type_t::back, heartbeat.back_msgr, *this)
{
...
void Heartbeat::Peer::send_heartbeat(clock::time_point now, ceph::signedspan mnow,
                                     std::vector<seastar::future<>>& futures)
{
    session.set_tx(now);
    if (session.is_started()) {
        do_send_heartbeat(now, mnow, &futures);
        for_each_conn([](auto& conn) { conn.validate(); });
    }
    else {
        ...
        if (pending_send) {
            ...
            logger().warn("Heartbeat::Peer::send_heartbeat(): "
                          "heartbeat to osd.{} is still pending...",
                          peer);
            for_each_conn([](auto& conn) { conn.retry(); });
        }
        else {
            logger().info("Heartbeat::Peer::send_heartbeat(): "
                          "heartbeat to osd.{} is pending send...",
                          peer);
...
void Heartbeat::Peer::handle_reset(crimson::net::ConnectionRef conn, bool is_replace)
{
    ...
    if (cnt == 0) {
        logger().info(
            "Heartbeat::Peer::handle_reset(): {} ignores conn, is_replace={} -- {}", *this, is_replace, *conn);
    }
    else if (cnt > 1) {
        logger().error("Heartbeat::Peer::handle_reset(): {} handles conn {} times -- {}", *this, cnt, *conn);
    }
}
...
void Heartbeat::Peer::handle_connect(crimson::net::ConnectionRef conn)
{
    ...
    if (cnt == 0) {
        logger().error("Heartbeat::Peer::handle_connect(): {} ignores conn -- {}", *this, *conn);
        conn->mark_down();
    }
    else if (cnt > 1) {
        logger().error("Heartbeat::Peer::handle_connect(): {} handles conn {} times -- {}", *this, cnt, *conn);
    }
}
...
void Heartbeat::Peer::handle_accept(crimson::net::ConnectionRef conn, bool is_replace)
{
    ...
    if (cnt == 0) {
        logger().warn("Heartbeat::Peer::handle_accept(): {} ignores conn -- {}", *this, *conn);
    }
    else if (cnt > 1) {
        logger().error("Heartbeat::Peer::handle_accept(): {} handles conn {} times -- {}", *this, cnt, *conn);
    }
}

seastar::future<> Heartbeat::Peer::handle_reply(crimson::net::ConnectionRef conn, Ref<MOSDPing> m)
{
    if (!session.is_started()) {
...
{
    logger().info("Heartbeat::Peer: osd.{} connected (send={})", peer, pending_send);
...
void Heartbeat::Peer::do_send_heartbeat(Heartbeat::clock::time_point now, ceph::signedspan mnow,
                                        std::vector<seastar::future<>>* futures)
{
    const utime_t sent_stamp{now};
    const auto deadline = now + std::chrono::seconds(local_conf()->osd_heartbeat_grace);
    ...
    for_each_conn([&, this](auto& conn) {
        auto min_message = static_cast<uint32_t>(local_conf()->osd_heartbeat_min_size);
        auto ping = crimson::make_message<MOSDPing>(heartbeat.monc.get_fsid(),
...
bool Heartbeat::FailingPeers::add_pending(osd_id_t peer, clock::time_point failed_since, clock::time_point now,
                                          std::vector<seastar::future<>>& futures)
{
    if (failure_pending.count(peer)) {
        return false;
    }
    auto failed_for = std::chrono::duration_cast<std::chrono::seconds>(now - failed_since).count();
    auto osdmap = heartbeat.service.get_map();
    auto failure_report = crimson::make_message<MOSDFailure>(
        heartbeat.monc.get_fsid(), peer, osdmap->get_addrs(peer), static_cast<int>(failed_for), osdmap->get_epoch());
    failure_pending.emplace(peer, failure_info_t{failed_since, osdmap->get_addrs(peer)});
...
{
    if (auto pending = failure_pending.find(peer); pending != failure_pending.end()) {
...
seastar::future<> Heartbeat::FailingPeers::send_still_alive(osd_id_t osd, const entity_addrvec_t& addrs)
{
    auto still_alive = crimson::make_message<MOSDFailure>(
        heartbeat.monc.get_fsid(), osd, addrs, 0, heartbeat.service.get_map()->get_epoch(), MOSDFailure::FLAG_ALIVE);
