...
Heartbeat::Heartbeat(osd_id_t whoami, const crimson::osd::ShardServices& service, crimson::mon::Client& monc,
                     crimson::net::MessengerRef front_msgr, crimson::net::MessengerRef back_msgr)
    : whoami{whoami}
    , service{service}
    , monc{monc}
...
{
    return msgr.try_bind(addrs, local_conf()->ms_bind_port_min, local_conf()->ms_bind_port_max)
        .safe_then([this, &msgr]() mutable { return msgr.start({this}); },
...
seastar::future<> Heartbeat::handle_ping(crimson::net::ConnectionRef conn, Ref<MOSDPing> m)
{
    ...
    auto reply = make_message<MOSDPing>(m->fsid,
...
void Heartbeat::Connection::replaced()
{
    ...
    conn = msgr.connect(replaced_conn->get_peer_addr(), replaced_conn->get_peer_name());
                                        ...
                                        std::vector<seastar::future<>>* futures)
{
    ...
    for_each_conn([&, this](auto& conn) {
        auto min_message = static_cast<uint32_t>(local_conf()->osd_heartbeat_min_size);
        auto ping = make_message<MOSDPing>(heartbeat.monc.get_fsid(),
                                          ...
                                          std::vector<seastar::future<>>& futures)
{
    ...
    auto failed_for = chrono::duration_cast<chrono::seconds>(now - failed_since).count();
    auto osdmap = heartbeat.service.get_osdmap_service().get_map();
    auto failure_report = make_message<MOSDFailure>(
...
{
    auto still_alive = make_message<MOSDFailure>(heartbeat.monc.get_fsid(),
                                                 osd,
                                                 addrs,
                                                 0,
                                                 heartbeat.service.get_osdmap_service().get_map()->get_epoch(),
