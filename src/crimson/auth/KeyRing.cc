...
seastar::future<KeyRing*> load_from_keyring(KeyRing* keyring)
{
    ...
    boost::split(paths, crimson::common::local_conf()->keyring, boost::is_any_of(",;"));
    std::pair<bool, std::string> found;
    return seastar::map_reduce(
               paths,
               [](auto path) {
                   return seastar::engine().file_exists(path).then(
                       [path](bool file_exists) { return std::make_pair(file_exists, path); });
               },
               std::move(found),
               [](auto found, auto file_exists_and_path) {
...
seastar::future<KeyRing*> load_from_keyfile(KeyRing* keyring)
{
    ...
    if (!path.empty()) {
        return read_file(path).then([keyring](auto buf) {
            EntityAuth ea;
            ea.key.decode_base64(std::string(buf.begin(), buf.end()));
