...
int CrushTester::get_maximum_affected_by_rule(int ruleno)
{
    ...
    for (vector<int>::iterator it = affected_types.begin(); it != affected_types.end(); ++it) {
        ...
        for (map<int, string>::iterator p = crush.name_map.begin(); p != crush.name_map.end(); ++p) {
            ...
            if (bucket_type == *it) max_devices_of_type[*it]++;
        }
    }
...
void CrushTester::adjust_weights(vector<__u32>& weight)
{
    ...
    if (mark_down_device_ratio > 0) {
        ...
        for (unsigned i = 0; i < bucket_ids.size(); i++) {
            ...
            int id = bucket_ids[i];
            if (crush.get_bucket_size(id) == 0) continue;
        ...
        for (int i = 0; i < num_buckets_to_visit; i++) {
            ...
            vector<int> items;
            for (int o = 0; o < size; o++) items.push_back(crush.get_bucket_item(id, o));
...
bool CrushTester::check_valid_placement(int ruleno, vector<int> in, const vector<__u32>& weight)
{
    ...
    if (!only_osd_affected) {
        ...
        for (vector<int>::iterator it = included_devices.begin(); it != included_devices.end(); ++it) {
            if (valid_placement == false) break;
...
int CrushTester::random_placement(int ruleno, vector<int>& out, int maxout, vector<__u32>& weight)
{
    ...
    int total_weight = 0;
    for (unsigned i = 0; i < weight.size(); i++) total_weight += weight[i];

    if (total_weight == 0 || crush.get_max_devices() == 0) return -EINVAL;
    ...
    if (accept_placement) out.assign(trial_placement.begin(), trial_placement.end());
...
{
    ostringstream sink;
    int r = fork_function(timeout, sink, [&]() { return test(); });
    if (r == -ETIMEDOUT) {
...
public:
    CrushWalker(const CrushWrapper* crush, unsigned max_id)
        : Parent(crush, CrushTreeDumper::name_map_t())
        , max_id(max_id)
    {}
...
void CrushTester::check_overlapped_rules() const
{
    ...
    for (int rule = 0; rule < crush.get_max_rules(); rule++) {
        ...
        Rules& rules = rulesets[{crush.get_rule_mask_ruleset(rule), crush.get_rule_mask_type(rule)}];
        rules += make_pair(interval::closed(crush.get_rule_mask_min_size(rule), crush.get_rule_mask_max_size(rule)),
    ...
    for (auto i : rulesets) {
        ...
        for (auto r : rules) {
            ...
            // print them out.
            if (names.size() > 1) {
                err << "overlapped rules in ruleset " << ruleset_type.first << ": " << boost::join(names, ", ") << "\n";
            }
        }
    }
}
...
int CrushTester::test()
{
    ...
    if (output_utilization_all) cerr << "devices weights (hex): " << std::hex << weight << std::dec << std::endl;
    ...
    for (vector<__u32>::iterator p = weight.begin(); p != weight.end(); ++p)
        if (*p > 0) num_devices_active++;

    if (output_choose_tries) crush.start_choose_profile();

    for (int r = min_rule; r < crush.get_max_rules() && r <= max_rule; r++) {
        if (!crush.rule_exists(r)) {
            if (output_statistics) err << "rule " << r << " dne" << std::endl;
            continue;
        }
        if (ruleset >= 0 && crush.get_rule_mask_ruleset(r) != ruleset) {
            continue;
        }
        ...
        if (output_statistics)
            err << "rule " << r << " (" << crush.get_rule_name(r) << "), x = " << min_x << ".." << max_x
                << ", numrep = " << minr << ".." << maxr << std::endl;
        ...
        for (int nr = minr; nr <= maxr; nr++) {
            ...
            // get the total weight of the system
            int total_weight = 0;
            for (unsigned i = 0; i < per.size(); i++) total_weight += weight[i];
            ...
            for (unsigned i = 0; i < per.size(); i++) proportional_weights[i] = (float)weight[i] / (float)total_weight;
            ...
            if (output_data_file) {
                ...
                for (unsigned i = 0; i < per.size(); i++) {
                    if (proportional_weights[i] > 0) tester_data.proportional_weights[i] = proportional_weights[i];
            ...
            for (int current_batch = 0; current_batch < num_batches; current_batch++) {
                ...
                for (int x = batch_min; x <= batch_max; x++) {
                    ...
                    if (use_crush) {
                        if (output_mappings) err << "CRUSH";   // prepend CRUSH to placement output
                        uint32_t real_x = x;
                        if (pool_id != -1) {
                        ...
                        if (output_mappings) err << "RNG";   // prepend RNG to placement output to denote simulation
                        // test our new monte carlo placement generator
                        random_placement(r, out, nr, weight);
                    }

                    if (output_mappings) err << " rule " << r << " x " << x << " " << out << std::endl;
                    ...
                    sizes[out.size()]++;
                    if (output_bad_mappings && (out.size() != (unsigned)nr || has_item_none)) {
                        err << "bad mapping rule " << r << " x " << x << " num_rep " << nr << " result " << out
                            << std::endl;
                    }
                }
            ...
            for (unsigned i = 0; i < per.size(); i++)
                if (output_utilization && !output_statistics) err << "  device " << i << ":\t" << per[i] << std::endl;

            for (map<int, int>::iterator p = sizes.begin(); p != sizes.end(); ++p)
                if (output_statistics)
                    err << "rule " << r << " (" << crush.get_rule_name(r) << ") num_rep " << nr
                        << " result size == " << p->first << ":\t" << p->second << "/" << (max_x - min_x + 1)
                        << std::endl;
                    ...
                    if (output_utilization) {
                        if (num_objects_expected[i] > 0 && per[i] > 0) {
                            err << "  device " << i << ":\t"
                                ...
                                << ": " << per[i] << "\t"
                                << " expected "
                                << ": " << num_objects_expected[i] << std::endl;
                        }
                    }
            ...
            if (output_data_file)
                for (unsigned i = 0; i < per.size(); i++) {
                    ...
                    write_integer_indexed_vector_data_string(
            ...
            if (output_data_file && num_batches > 1) {
                ...
                for (int i = 0; i < num_batches; i++) {
                    ...
                    write_integer_indexed_vector_data_string(
                        tester_data.batch_device_expected_utilization_all, i, batch_per[i]);
                }
            }

            string rule_tag = crush.get_rule_name(r);

            if (output_csv) write_data_set_to_csv(output_data_file_name + rule_tag, tester_data);
        }
    }
    ...
    if (output_choose_tries) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            cout << std::setw(2) << i << ": " << std::setw(9) << v[i];
...
int CrushTester::compare(CrushWrapper& crush2)
{
    ...
    for (int r = min_rule; r < crush.get_max_rules() && r <= max_rule; r++) {
        ...
        cout << "rule " << r << " had " << bad << "/" << max << " mismatched mappings (" << ratio << ")" << std::endl;
    }
    if (ret) {
