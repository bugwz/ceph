...
int crush_find_rule(const struct crush_map* map, int ruleset, int type, int size)
{
    ...
    for (i = 0; i < map->max_rules; i++) {
        if (map->rules[i] && map->rules[i]->mask.ruleset == ruleset && map->rules[i]->mask.type == type &&
...
/* uniform */
static int bucket_uniform_choose(const struct crush_bucket_uniform* bucket, struct crush_work_bucket* work, int x,
                                 int r)
{
...
{
    if ((arg == NULL) || (arg->weight_set == NULL)) return bucket->item_weights;
    if (position >= arg->weight_set_positions) position = arg->weight_set_positions - 1;
    return arg->weight_set[position].weights;
}

static inline __s32* get_choose_arg_ids(const struct crush_bucket_straw2* bucket, const struct crush_choose_arg* arg)
{
    if ((arg == NULL) || (arg->ids == NULL)) return bucket->h.items;
    return arg->ids;
}
...
static int bucket_straw2_choose(const struct crush_bucket_straw2* bucket, int x, int r,
                                const struct crush_choose_arg* arg, int position)
{
...
static int crush_bucket_choose(const struct crush_bucket* in, struct crush_work_bucket* work, int x, int r,
                               const struct crush_choose_arg* arg, int position)
{
...
static int is_out(const struct crush_map* map, const __u32* weight, int weight_max, int item, int x)
{
    ...
    if ((crush_hash32_2(CRUSH_HASH_RJENKINS1, x, item) & 0xffff) < weight[item]) return 0;
    return 1;
}
 ...
 */
static int crush_choose_firstn(const struct crush_map* map, struct crush_work* work, const struct crush_bucket* bucket,
        ...
        do {
            ...
            reject:
                if (reject || collide) {
                    ...
                    if (collide && flocal <= local_retries) /* retry locally a few times */
                        retry_bucket = 1;
 ...
 *
 */
static void crush_choose_indep(const struct crush_map* map, struct crush_work* work, const struct crush_bucket* bucket,
                               const __u32* weight, int weight_max, int x, int left, int numrep, int type, int* out,
                               int outpos, unsigned int tries, unsigned int recurse_tries, int recurse_to_leaf,
    ...
    for (ftotal = 0; left > 0 && ftotal < tries; ftotal++) {
        ...
        for (rep = outpos; rep < endpos; rep++) {
            ...
            for (;;) {
                ...
                /* be careful */
                if (in->alg == CRUSH_BUCKET_UNIFORM && in->size % numrep == 0) /* r'=r+(n+1)*f_total */
                    r += (numrep + 1) * ftotal;
                else
                    /* r' = r + n*f_total */
                    r += numrep * ftotal;
...
void crush_init_workspace(const struct crush_map* m, void* v)
{
    ...
    for (b = 0; b < m->max_buckets; ++b) {
        ...
        default: point += sizeof(struct crush_work_bucket); break;
        }
        w->work[b]->perm_x = 0;
        w->work[b]->perm_n = 0;
                  ...
                  int weight_max, void* cwin, const struct crush_choose_arg* choose_args)
{
    ...
    for (step = 0; step < rule->len; step++) {
        ...
        case CRUSH_RULE_SET_CHOOSE_TRIES:
            if (curstep->arg1 > 0) choose_tries = curstep->arg1;
            break;

        case CRUSH_RULE_SET_CHOOSELEAF_TRIES:
            if (curstep->arg1 > 0) choose_leaf_tries = curstep->arg1;
            break;

        case CRUSH_RULE_SET_CHOOSE_LOCAL_TRIES:
            if (curstep->arg1 >= 0) choose_local_retries = curstep->arg1;
            break;

        case CRUSH_RULE_SET_CHOOSE_LOCAL_FALLBACK_TRIES:
            if (curstep->arg1 >= 0) choose_local_fallback_retries = curstep->arg1;
            break;

        case CRUSH_RULE_SET_CHOOSELEAF_VARY_R:
            if (curstep->arg1 >= 0) vary_r = curstep->arg1;
            break;

        case CRUSH_RULE_SET_CHOOSELEAF_STABLE:
            if (curstep->arg1 >= 0) stable = curstep->arg1;
            break;
        ...
        case CRUSH_RULE_CHOOSE_INDEP:
            ...
            recurse_to_leaf = curstep->op == CRUSH_RULE_CHOOSELEAF_FIRSTN || curstep->op == CRUSH_RULE_CHOOSELEAF_INDEP;
        ...
        default: dprintk(" unknown op %d at step %d\n", curstep->op, step); break;
        }
    }
