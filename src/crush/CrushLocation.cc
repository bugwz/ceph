...
{
    if (cct->_conf->crush_location.length()) return _parse(cct->_conf->crush_location);
    return 0;
}
...
int CrushLocation::_parse(const std::string& s)
{
    ...
    if (r < 0) {
        lderr(cct) << "warning: crush_location '" << cct->_conf->crush_location
                   << "' does not parse, keeping original crush_location " << loc << dendl;
        return -EINVAL;
    }
...
{
    if (cct->_conf->crush_location_hook.length() == 0) return 0;
    ...
    SubProcessTimed hook(cct->_conf->crush_location_hook.c_str(),
                         SubProcess::CLOSE,
                         SubProcess::PIPE,
                         SubProcess::PIPE,
                         cct->_conf->crush_location_hook_timeout);
    hook.add_cmd_args("--cluster",
                      cct->_conf->cluster.c_str(),
                      "--id",
                      cct->_conf->name.get_id().c_str(),
                      "--type",
                      cct->_conf->name.get_type_str(),
                      NULL);
    int ret = hook.spawn();
    if (ret != 0) {
        lderr(cct) << "error: failed run " << cct->_conf->crush_location_hook << ": " << hook.err() << dendl;
        return ret;
    }
    ...
    if (ret < 0) {
        lderr(cct) << "error: failed read stdout from " << cct->_conf->crush_location_hook << ": " << cpp_strerror(-ret)
                   << dendl;
