...
{
    if (crush.class_bucket.count(t) == 0) return;
...
int CrushCompiler::decompile_bucket_impl(int i, ostream& out)
{
    ...
    if (name && !crush.is_valid_crush_name(name)) return 0;
    ...
    case CRUSH_BUCKET_UNIFORM:
        ...
        dopos = true;
        break;
    case CRUSH_BUCKET_LIST: out << "\t# add new items at the end; do not change order unnecessarily"; break;
 ...
 */
int CrushCompiler::decompile_bucket(int cur, std::map<int, dcb_state_t>& dcb_states, ostream& out)
{
    if ((cur == 0) || (!crush.bucket_exists(cur))) return 0;
    ...
    if (c == dcb_states.end()) {
        ...
        std::pair<std::map<int, dcb_state_t>::iterator, bool> rval(dcb_states.insert(val));
        ...
        err << "decompile_crush_bucket: logic error: tried to decompile "
               "a bucket that is already being decompiled"
            << std::endl;
        return -EBADE;
    }
    else {
        err << "decompile_crush_bucket: logic error: illegal bucket state! " << c->second << std::endl;
        return -EBADE;
    }
    ...
    for (int i = 0; i < bsize; ++i) {
        ...
        if (d == dcb_states.end()) {
            ...
            err << "decompile_crush_bucket: error: while trying to output bucket " << cur
            ...
            err << "decompile_crush_bucket: logic error: illegal bucket state " << d->second << std::endl;
            return -EBADE;
        }
    }
...
int CrushCompiler::decompile_weight_set_weights(crush_weight_set weight_set, ostream& out)
{
    out << "      [ ";
...
int CrushCompiler::decompile_weight_set(crush_weight_set* weight_set, __u32 size, ostream& out)
{
    out << "    weight_set [\n";
...
{
    out << "    ids [ ";
    for (__u32 i = 0; i < size; i++) out << ids[i] << " ";
    out << "]\n";
    return 0;
}

int CrushCompiler::decompile_choose_arg(crush_choose_arg* arg, int bucket_id, ostream& out)
{
    int r;
    out << "  {\n";
...
int CrushCompiler::decompile_choose_arg_map(crush_choose_arg_map arg_map, ostream& out)
{
    for (__u32 i = 0; i < arg_map.size; i++) {
        if ((arg_map.args[i].ids_size == 0) && (arg_map.args[i].weight_set_positions == 0)) continue;
...
int CrushCompiler::decompile(ostream& out)
{
    ...
    for (int i = 0; i < crush.get_max_rules(); i++) {
        ...
        out << "rule ";
        if (crush.get_rule_name(i)) print_rule_name(out, i, crush);
        out << " {\n";
        out << "\tid " << i << "\n";
        if (i != crush.get_rule_mask_ruleset(i)) {
            out << "\t# WARNING: ruleset " << crush.get_rule_mask_ruleset(i) << " != id " << i
                << "; this will not recompile to the same map\n";
        }

        switch (crush.get_rule_mask_type(i)) {
        case CEPH_PG_TYPE_REPLICATED: out << "\ttype replicated\n"; break;
        case CEPH_PG_TYPE_ERASURE: out << "\ttype erasure\n"; break;
        ...
        for (int j = 0; j < crush.get_rule_len(i); j++) {
            ...
            case CRUSH_RULE_NOOP: out << "\tstep noop\n"; break;
            case CRUSH_RULE_TAKE:
                out << "\tstep take ";
                {
                    ...
                    if (res < 0) return res;
                    if (c >= 0) step_item = original_item;
                    print_item_name(out, step_item, crush);
                    if (c >= 0) print_class(out, c, crush);
                }
                out << "\n";
                break;
            case CRUSH_RULE_EMIT: out << "\tstep emit\n"; break;
            ...
            case CRUSH_RULE_CHOOSE_FIRSTN:
                out << "\tstep choose firstn " << crush.get_rule_arg1(i, j) << " type ";
                print_type_name(out, crush.get_rule_arg2(i, j), crush);
                out << "\n";
                break;
            case CRUSH_RULE_CHOOSE_INDEP:
                out << "\tstep choose indep " << crush.get_rule_arg1(i, j) << " type ";
                print_type_name(out, crush.get_rule_arg2(i, j), crush);
                out << "\n";
                break;
            case CRUSH_RULE_CHOOSELEAF_FIRSTN:
                out << "\tstep chooseleaf firstn " << crush.get_rule_arg1(i, j) << " type ";
                print_type_name(out, crush.get_rule_arg2(i, j), crush);
                out << "\n";
                break;
            case CRUSH_RULE_CHOOSELEAF_INDEP:
                out << "\tstep chooseleaf indep " << crush.get_rule_arg1(i, j) << " type ";
                print_type_name(out, crush.get_rule_arg2(i, j), crush);
                out << "\n";
                break;
            }
...
int CrushCompiler::parse_tunable(iter_t const& i)
{
    ...
    if (!unsafe_tunables) {
      err << "tunables are NOT FULLY IMPLEMENTED; enable with --enable-unsafe-tunables to enable this feature" <<
    std::endl; return -1;
    }
    */
...
int CrushCompiler::parse_bucket(iter_t const& i)
{
            ...
            for (unsigned q = 2; q < sub->children.size(); q++) {
                ...
                if (tag == "pos") {
                    ...
                    if (used_items.count(pos)) {
                        err << "item '" << string_node(sub->children[1]) << "' in bucket '" << name
                            << "' has explicit pos " << pos << ", which is occupied" << std::endl;
                        return -1;
                    }
    ...
    // now do the items.
    if (!used_items.empty()) size = std::max(size, *used_items.rbegin());
    ...
    for (unsigned p = 3; p < i->children.size() - 1; p++) {
        ...
        if (tag == "item") {
            ...
            if (item_weight.count(itemid)) weight = item_weight[itemid];
            ...
            if (pos >= size) {
                err << "item '" << iname << "' in bucket '" << name << "' has pos " << pos << " >= size " << size
                    << std::endl;
                return -1;
            }
            if (pos < 0) {
            ...
            if (crush_addition_is_unsafe(bucketweight, weight)) {
                err << "oh no! our bucket weights are overflowing all over the place, better lower the item weights"
                    << std::endl;
                return -ERANGE;
            }
    ...
    for (auto& i : class_id) class_bucket[id][i.first] = i.second;

    if (verbose)
        err << "bucket " << name << " (" << id << ") " << size << " items and weight "
    ...
    ceph_assert(id != 0);
    int idout;
    int r = crush.add_bucket(id, alg, hash, type, size, items.data(), weights.data(), &idout);
    if (r < 0) {
        if (r == -EEXIST)
...
int CrushCompiler::parse_rule(iter_t const& i)
{
    ...
    if (r != ruleno) {
        err << "unable to add rule id " << ruleno << " for rule '" << rname << "'" << std::endl;
        return -1;
    }
    if (rname.length()) {
    ...
    for (iter_t p = i->children.begin() + start + 7; step < steps; p++) {
        ...
        case crush_grammar::_step_take:
        {
            ...
            if (s->children.size() > 2) {
                ...
                if (crush.class_bucket.count(id) == 0) {
                    err << "in rule '" << rname << "' step take " << item << " has no class information" << std::endl;
                    return -EINVAL;
                }
                if (crush.class_bucket[id].count(c) == 0) {
                    err << "in rule '" << rname << "' step take " << item << " no matching bucket for class "
                        << class_name << std::endl;
                    return -EINVAL;
                }
        ...
        case crush_grammar::_step_emit: crush.set_rule_step_emit(ruleno, step++); break;

        default: err << "bad crush step " << stepid << std::endl; return -1;
        }
    }
...
int CrushCompiler::parse_weight_set_weights(iter_t const& i, int bucket_id, crush_weight_set* weight_set)
{
    ...
    if (size != bucket_size) {
        err << bucket_id << " needs exactly " << bucket_size << " weights but got " << size << std::endl;
        return -1;
    }
    ...
    for (iter_t p = i->children.begin() + 1; p != i->children.end(); p++, pos++)
        if (pos < size) weight_set->weights[pos] = float_node(*p) * (float)0x10000;
    return 0;
}
...
int CrushCompiler::parse_choose_arg_ids(iter_t const& i, int bucket_id, crush_choose_arg* arg)
{
    ...
    __u32 bucket_size = crush.get_bucket_size(bucket_id);
    if (size != bucket_size) {
        err << bucket_id << " needs exactly " << bucket_size << " ids but got " << size << std::endl;
        return -1;
    }
    arg->ids_size = size;
    arg->ids = (__s32*)calloc(arg->ids_size, sizeof(__s32));
    __u32 pos = 0;
    for (iter_t p = i->children.begin() + 2; pos < size; p++, pos++) arg->ids[pos] = int_node(*p);
    return 0;
}
...
int CrushCompiler::parse_choose_arg(iter_t const& i, crush_choose_arg* args)
{
    ...
    for (iter_t p = i->children.begin(); p != i->children.end(); p++) {
        int r = 0;
        switch ((int)p->value.id().to_long()) {
        case crush_grammar::_weight_set: r = parse_weight_set(p, bucket_id, arg); break;
        case crush_grammar::_choose_arg_ids: r = parse_choose_arg_ids(p, bucket_id, arg); break;
        }
        if (r < 0) return r;
    }
    return 0;
...
int CrushCompiler::parse_choose_args(iter_t const& i)
{
    ...
    for (iter_t p = i->children.begin() + 2; p != i->children.end(); p++) {
        int r = 0;
        switch ((int)p->value.id().to_long()) {
        case crush_grammar::_choose_arg: r = parse_choose_arg(p, arg_map.args); break;
        }
        if (r < 0) {
...
void CrushCompiler::find_used_bucket_ids(iter_t const& i)
{
    for (iter_t p = i->children.begin(); p != i->children.end(); p++) {
        if ((int)p->value.id().to_long() == crush_grammar::_bucket) {
            for (iter_t firstline = p->children.begin() + 3; firstline != p->children.end(); ++firstline) {
...
int CrushCompiler::parse_crush(iter_t const& i)
{
    ...
    for (iter_t p = i->children.begin(); p != i->children.end(); p++) {
        ...
        case crush_grammar::_tunable: r = parse_tunable(p); break;
        case crush_grammar::_device: r = parse_device(p); break;
        case crush_grammar::_bucket_type: r = parse_bucket_type(p); break;
        ...
        case crush_grammar::_crushrule:
            ...
            r = parse_rule(p);
            break;
        case crush_grammar::_choose_args: r = parse_choose_args(p); break;
        default: ceph_abort();
        }
        if (r < 0) {
            return r;
...
string CrushCompiler::consolidate_whitespace(string in)
{
    ...
    if (verbose > 3) err << " \"" << in << "\" -> \"" << out << "\"" << std::endl;
    return out;
}
...
void CrushCompiler::dump(iter_t const& i, int ind)
{
    ...
    err << id << "\t";
    err << "'" << string(i->value.begin(), i->value.end()) << "' " << i->children.size() << " children" << std::endl;
    for (unsigned int j = 0; j < i->children.size(); j++) dump(i->children.begin() + j, ind + 1);
}
    ...
    for (iter_t p = i->children.begin(); p != i->children.end(); ++p) {
        if ((int)p->value.id().to_long() == crush_grammar::_bucket) {
            ...
            for (unsigned q = 3; q < p->children.size() - 1; ++q) {
                ...
                if ((int)sub->value.id().to_long() == crush_grammar::_bucket_item) {
                    string iname = string_node(sub->children[1]);
        ...
        for (unsigned j = i + 1; j < buckets.size(); ++j) {
            if (bucket_items[buckets[i]].count(buckets[j])) {
                if (bucket_items[buckets[j]].count(buckets[i])) {
                    err << "bucket  '" << buckets[i] << "' and bucket '" << buckets[j] << "' are included each other"
                        << std::endl;
                    return -1;
                }
                else {
...
int CrushCompiler::compile(istream& in, const char* infn)
{
    ...
    while (getline(in, str)) {
        ...
        // strip comment
        int n = str.find("#");
        if (n >= 0) str.erase(n, str.length() - n);
