...
private:
    ...
    {
        r.clear();
        for (auto p = f.begin(); p != f.end(); ++p) r[p->second] = p->first;
    }
...
public:
    ...
    void set_tunables_hammer()
    {
        ...
        crush->chooseleaf_vary_r = 1;
        crush->chooseleaf_stable = 0;
        crush->allowed_bucket_algs = (1 << CRUSH_BUCKET_UNIFORM) | (1 << CRUSH_BUCKET_LIST) |
    ...
    int get_choose_local_tries() const { return crush->choose_local_tries; }
    void set_choose_local_tries(int n) { crush->choose_local_tries = n; }

    int get_choose_local_fallback_tries() const { return crush->choose_local_fallback_tries; }
    void set_choose_local_fallback_tries(int n) { crush->choose_local_fallback_tries = n; }

    int get_choose_total_tries() const { return crush->choose_total_tries; }
    void set_choose_total_tries(int n) { crush->choose_total_tries = n; }

    int get_chooseleaf_descend_once() const { return crush->chooseleaf_descend_once; }
    void set_chooseleaf_descend_once(int n) { crush->chooseleaf_descend_once = !!n; }

    int get_chooseleaf_vary_r() const { return crush->chooseleaf_vary_r; }
    void set_chooseleaf_vary_r(int n) { crush->chooseleaf_vary_r = n; }

    int get_chooseleaf_stable() const { return crush->chooseleaf_stable; }
    void set_chooseleaf_stable(int n) { crush->chooseleaf_stable = n; }

    int get_straw_calc_version() const { return crush->straw_calc_version; }
    void set_straw_calc_version(int n) { crush->straw_calc_version = n; }

    unsigned get_allowed_bucket_algs() const { return crush->allowed_bucket_algs; }
    void set_allowed_bucket_algs(unsigned n) { crush->allowed_bucket_algs = n; }

    bool has_argonaut_tunables() const
    {
        return crush->choose_local_tries == 2 && crush->choose_local_fallback_tries == 5 &&
               crush->choose_total_tries == 19 && crush->chooseleaf_descend_once == 0 &&
               crush->chooseleaf_vary_r == 0 && crush->chooseleaf_stable == 0 &&
               crush->allowed_bucket_algs == CRUSH_LEGACY_ALLOWED_BUCKET_ALGS;
    }
    bool has_bobtail_tunables() const
    {
        return crush->choose_local_tries == 0 && crush->choose_local_fallback_tries == 0 &&
               crush->choose_total_tries == 50 && crush->chooseleaf_descend_once == 1 &&
               crush->chooseleaf_vary_r == 0 && crush->chooseleaf_stable == 0 &&
               crush->allowed_bucket_algs == CRUSH_LEGACY_ALLOWED_BUCKET_ALGS;
    }
    bool has_firefly_tunables() const
    {
        return crush->choose_local_tries == 0 && crush->choose_local_fallback_tries == 0 &&
               crush->choose_total_tries == 50 && crush->chooseleaf_descend_once == 1 &&
               crush->chooseleaf_vary_r == 1 && crush->chooseleaf_stable == 0 &&
               crush->allowed_bucket_algs == CRUSH_LEGACY_ALLOWED_BUCKET_ALGS;
    }
    ...
    bool has_jewel_tunables() const
    {
        return crush->choose_local_tries == 0 && crush->choose_local_fallback_tries == 0 &&
               crush->choose_total_tries == 50 && crush->chooseleaf_descend_once == 1 &&
               crush->chooseleaf_vary_r == 1 && crush->chooseleaf_stable == 1 &&
               crush->allowed_bucket_algs == ((1 << CRUSH_BUCKET_UNIFORM) | (1 << CRUSH_BUCKET_LIST) |
                                              (1 << CRUSH_BUCKET_STRAW) | (1 << CRUSH_BUCKET_STRAW2));
    }

    bool has_optimal_tunables() const { return has_jewel_tunables(); }
    bool has_legacy_tunables() const { return has_argonaut_tunables(); }
    ...
    bool has_nondefault_tunables2() const { return crush->chooseleaf_descend_once != 0; }
    bool has_nondefault_tunables3() const { return crush->chooseleaf_vary_r != 0; }
    bool has_nondefault_tunables5() const { return crush->chooseleaf_stable != 0; }
    ...
    unsigned get_default_bucket_alg() const
    {
        ...
        if (crush->allowed_bucket_algs & (1 << CRUSH_BUCKET_STRAW2)) return CRUSH_BUCKET_STRAW2;
        if (crush->allowed_bucket_algs & (1 << CRUSH_BUCKET_STRAW)) return CRUSH_BUCKET_STRAW;
        if (crush->allowed_bucket_algs & (1 << CRUSH_BUCKET_TREE)) return CRUSH_BUCKET_TREE;
        if (crush->allowed_bucket_algs & (1 << CRUSH_BUCKET_LIST)) return CRUSH_BUCKET_LIST;
        if (crush->allowed_bucket_algs & (1 << CRUSH_BUCKET_UNIFORM)) return CRUSH_BUCKET_UNIFORM;
        return 0;
    }

    // bucket types
    int get_num_type_names() const { return type_map.size(); }
    int get_max_type_id() const
    {
        if (type_map.empty()) return 0;
    ...
    const char* get_type_name(int t) const
    {
        ...
        if (p != type_map.end()) return p->second.c_str();
        return 0;
    }
    void set_type_name(int i, const std::string& name)
    {
        type_map[i] = name;
        if (have_rmaps) type_rmap[name] = i;
    }
    ...
    bool item_exists(int i) const { return name_map.count(i); }
    int get_item_id(const std::string& name) const
    {
        build_rmaps();
        if (name_rmap.count(name)) return name_rmap[name];
        return 0; /* hrm */
    }
    const char* get_item_name(int t) const
    {
        std::map<int, std::string>::const_iterator p = name_map.find(t);
        if (p != name_map.end()) return p->second.c_str();
        return 0;
    }
    int set_item_name(int i, const std::string& name)
    {
        ...
        name_map[i] = name;
        if (have_rmaps) name_rmap[name] = i;
        return 0;
    }
    ...
    bool class_exists(const std::string& name) const { return class_rname.count(name); }
    const char* get_class_name(int i) const
    {
        auto p = class_name.find(i);
        if (p != class_name.end()) return p->second.c_str();
        return 0;
    }
    ...
    {
        auto p = class_rname.find(name);
        if (p == class_rname.end()) return -ENOENT;
        int class_id = p->second;
        auto q = class_name.find(class_id);
        if (q == class_name.end()) return -ENOENT;
    ...
    void get_devices_by_class(const std::string& name, std::set<int>* devices) const
    {
        ceph_assert(devices);
        devices->clear();
    ...
    int can_rename_item(const std::string& srcname, const std::string& dstname, std::ostream* ss) const;
    int rename_item(const std::string& srcname, const std::string& dstname, std::ostream* ss);
    int can_rename_bucket(const std::string& srcname, const std::string& dstname, std::ostream* ss) const;
    int rename_bucket(const std::string& srcname, const std::string& dstname, std::ostream* ss);

    // rule names
    int rename_rule(const std::string& srcname, const std::string& dstname, std::ostream* ss);
    ...
    {
        build_rmaps();
        if (rule_name_rmap.count(name)) return rule_name_rmap[name];
        return -ENOENT;
    }
    const char* get_rule_name(int t) const
    {
        auto p = rule_name_map.find(t);
        if (p != rule_name_map.end()) return p->second.c_str();
        return 0;
    }
    void set_rule_name(int i, const std::string& name)
    {
        rule_name_map[i] = name;
        if (have_rmaps) rule_name_rmap[name] = i;
    }
...
public:
     ...
     */
    bool check_item_loc(CephContext* cct, int item, const std::map<std::string, std::string>& loc, int* iweight);
    bool check_item_loc(CephContext* cct, int item, const std::map<std::string, std::string>& loc, float* weight)
    {
        int iweight;
        bool ret = check_item_loc(cct, item, loc, &iweight);
        if (weight) *weight = (float)iweight / (float)0x10000;
        return ret;
    }
     ...
     */
    int get_full_location(const std::string& name, std::map<std::string, std::string>* ploc);
    ...
    void get_children_of_type(int id, int type, std::vector<int>* children, bool exclude_shadow = true) const;
    /**
     ...
     */
    int verify_upmap(CephContext* cct, int rule_id, int pool_size, const std::vector<int>& up);
...
public:
    ...
    int insert_item(CephContext* cct, int id, float weight, std::string name,
                    const std::map<std::string, std::string>& loc, bool init_weight_sets = true);
     ...
     * @return 0 for success, negative on error
     */
    int link_bucket(CephContext* cct, int id, const std::map<std::string, std::string>& loc);
     ...
     * @return 0 for no change, 1 for successful change, negative on error
     */
    int create_or_move_item(CephContext* cct, int item, float weight, std::string name,
                            const std::map<std::string, std::string>& loc, bool init_weight_sets = true);
...
public:
     ...
     */
    static int parse_loc_map(const std::vector<std::string>& args, std::map<std::string, std::string>* ploc);
    static int parse_loc_multimap(const std::vector<std::string>& args, std::multimap<std::string, std::string>* ploc);
    ...
    float get_item_weightf(int id) const { return (float)get_item_weight(id) / (float)0x10000; }
    ...
    int adjust_item_weight(CephContext* cct, int id, int weight, bool update_weight_sets = true);
    int adjust_item_weightf(CephContext* cct, int id, float weight, bool update_weight_sets = true)
    {
        int r = validate_weightf(weight);
        if (r < 0) {
            return r;
        }
        return adjust_item_weight(cct, id, (int)(weight * (float)0x10000), update_weight_sets);
    }
    int adjust_item_weight_in_bucket(CephContext* cct, int id, int weight, int bucket_id, bool update_weight_sets);
    int adjust_item_weight_in_loc(CephContext* cct, int id, int weight, const std::map<std::string, std::string>& loc,
                                  bool update_weight_sets = true);
    int adjust_item_weightf_in_loc(CephContext* cct, int id, float weight,
                                   const std::map<std::string, std::string>& loc, bool update_weight_sets = true)
    {
        int r = validate_weightf(weight);
        if (r < 0) {
            return r;
        }
        return adjust_item_weight_in_loc(cct, id, (int)(weight * (float)0x10000), loc, update_weight_sets);
    }
    void reweight(CephContext* cct);
    void reweight_bucket(crush_bucket* b, crush_choose_arg_map& arg_map, std::vector<uint32_t>* weightv);

    int adjust_subtree_weight(CephContext* cct, int id, int weight, bool update_weight_sets = true);
    int adjust_subtree_weightf(CephContext* cct, int id, float weight, bool update_weight_sets = true)
    {
        int r = validate_weightf(weight);
        if (r < 0) {
            return r;
        }
        return adjust_subtree_weight(cct, id, (int)(weight * (float)0x10000), update_weight_sets);
    }
...
private:
    crush_rule* get_rule(unsigned ruleno) const
    {
        ...
        if (ruleno >= crush->max_rules) return 0;
...
public:
    ...
    bool rule_exists(unsigned ruleno) const
    {
        ...
        if (ruleno < crush->max_rules && crush->rules[ruleno] != NULL) return true;
        return false;
    }
    bool rule_has_take(unsigned ruleno, int take) const
    {
        ...
        for (unsigned i = 0; i < rule->len; ++i) {
            if (rule->steps[i].op == CRUSH_RULE_TAKE && rule->steps[i].arg1 == take) {
                return true;
            }
        }
        return false;
...
private:
    ...
    void _normalize_weight_map(float sum, const std::map<int, float>& m, std::map<int, float>* pmap) const;
...
public:
    ...
    int add_simple_rule(std::string name, std::string root_name, std::string failure_domain_type,
                        std::string device_class, std::string mode, int rule_type, std::ostream* err = 0);

    /**
     * @param rno rule[set] id to use, -1 to pick the lowest available
     */
    int add_simple_rule_at(std::string name, std::string root_name, std::string failure_domain_type,
                           std::string device_class, std::string mode, int rule_type, int rno, std::ostream* err = 0);
    ...
    {
        if (!crush) return (crush_bucket*)(-EINVAL);
        unsigned int pos = (unsigned int)(-1 - id);
        unsigned int max_buckets = crush->max_buckets;
        if (pos >= max_buckets) return (crush_bucket*)(-ENOENT);
        crush_bucket* ret = crush->buckets[pos];
        if (ret == NULL) return (crush_bucket*)(-ENOENT);
        return ret;
    }
...
public:
    ...
    int get_bucket_item(int id, int pos) const
    {
        const crush_bucket* b = get_bucket(id);
        if (IS_ERR(b)) return PTR_ERR(b);
        if ((__u32)pos >= b->size) return PTR_ERR(b);
        return b->items[pos];
    }
    ...
    /* modifiers */
    int add_bucket(int bucketno, int alg, int hash, int type, int size, int* items, int* weights, int* idout);
    ...
    void finalize()
    {
        ...
        if (!name_map.empty() && name_map.rbegin()->first >= crush->max_devices) {
    ...
    int update_device_class(int id, const std::string& class_name, const std::string& name, std::ostream* ss);
    int remove_device_class(CephContext* cct, int id, std::ostream* ss);
    int device_class_clone(int original, int device_class,
    ...
    int populate_classes(const std::map<int32_t, std::map<int32_t, int32_t>>& old_class_bucket);
    ...
    int reclassify(CephContext* cct, std::ostream& out, const std::map<std::string, std::string>& classify_root,
    ...
    void start_choose_profile()
    {
         ...
         */
        crush->choose_tries = (__u32*)calloc(sizeof(*crush->choose_tries), (crush->choose_total_tries + 1));
        memset(crush->choose_tries, 0, sizeof(*crush->choose_tries) * (crush->choose_total_tries + 1));
    }
    ...
    void set_max_devices(int m) { crush->max_devices = m; }
    ...
    int find_first_rule(int type) const
    {
        for (size_t i = 0; i < crush->max_rules; ++i) {
            if (crush->rules[i] && crush->rules[i]->type == type) {
                return i;
            }
        }
        return -1;
    ...
    bool have_choose_args(int64_t choose_args_index) const { return choose_args.count(choose_args_index); }

    crush_choose_arg_map choose_args_get_with_fallback(int64_t choose_args_index) const
    {
    ...
    void destroy_choose_args(crush_choose_arg_map arg_map)
    {
        for (__u32 i = 0; i < arg_map.size; i++) {
            ...
            if (arg->weight_set) free(arg->weight_set);
            if (arg->ids) free(arg->ids);
        }
        free(arg_map.args);
    }
    ...
    {
        if (choose_args.count(id)) return false;
        ceph_assert(positions);
        auto& cmap = choose_args[id];
        cmap.args = static_cast<crush_choose_arg*>(calloc(sizeof(crush_choose_arg), crush->max_buckets));
        ...
        for (int bidx = 0; bidx < crush->max_buckets; ++bidx) {
            ...
            if (b && b->alg == CRUSH_BUCKET_STRAW2) {
                ...
                carg.weight_set =
                    static_cast<crush_weight_set*>(calloc(sizeof(crush_weight_set), carg.weight_set_positions));
    ...
    {
        for (auto w : choose_args) destroy_choose_args(w.second);
        choose_args.clear();
    }
    ...
    int _choose_args_adjust_item_weight_in_bucket(CephContext* cct, crush_choose_arg_map cmap, int bucketid, int id,
                                                  const std::vector<int>& weight, std::ostream* ss);
    int choose_args_adjust_item_weight(CephContext* cct, crush_choose_arg_map cmap, int id,
                                       const std::vector<int>& weight, std::ostream* ss);
    int choose_args_adjust_item_weightf(CephContext* cct, crush_choose_arg_map cmap, int id,
                                        const std::vector<double>& weightf, std::ostream* ss)
    {
    ...
    void do_rule(int rule, int x, std::vector<int>& out, int maxout, const WeightVector& weight,
                 uint64_t choose_args_index) const
    {
        ...
        int numrep =
            crush_do_rule(crush, rule, x, rawout, maxout, std::data(weight), std::size(weight), work, arg_map.args);
        if (numrep < 0) numrep = 0;
        out.resize(numrep);
        for (int i = 0; i < numrep; i++) out[i] = rawout[i];
    }

    int _choose_type_stack(CephContext* cct, const std::vector<std::pair<int, int>>& stack,
                           const std::set<int>& overfull, const std::vector<int>& underfull,
                           const std::vector<int>& more_underfull, const std::vector<int>& orig,
                           std::vector<int>::const_iterator& i, std::set<int>& used, std::vector<int>* pw,
                           int root_bucket, int rule) const;

    int try_remap_rule(CephContext* cct, int rule, int maxout, const std::set<int>& overfull,
                       const std::vector<int>& underfull, const std::vector<int>& more_underfull,
                       const std::vector<int>& orig, std::vector<int>* out) const;

    void encode(ceph::buffer::list& bl, uint64_t features) const;
    void decode(ceph::buffer::list::const_iterator& blp);
    void decode_crush_bucket(crush_bucket** bptr, ceph::buffer::list::const_iterator& blp);
    ...
    void dump_tree(std::ostream* out, ceph::Formatter* f, const CrushTreeDumper::name_map_t& ws,
                   bool show_shadow = false) const;
    void dump_tree(std::ostream* out, ceph::Formatter* f) { dump_tree(out, f, CrushTreeDumper::name_map_t()); }
    void dump_tree(ceph::Formatter* f, const CrushTreeDumper::name_map_t& ws) const;
    ...
    static bool is_valid_crush_loc(CephContext* cct, const std::map<std::string, std::string>& loc);
};
