...
bool CrushWrapper::has_non_straw2_buckets() const
{
    for (int i = 0; i < crush->max_buckets; ++i) {
        ...
        if (!b) continue;
        if (b->alg != CRUSH_BUCKET_STRAW2) return true;
    }
    return false;
}
...
bool CrushWrapper::is_v2_rule(unsigned ruleid) const
{
    ...
    if (ruleid >= crush->max_rules) return false;
    crush_rule* r = crush->rules[ruleid];
    if (!r) return false;
    for (unsigned j = 0; j < r->len; j++) {
        if (r->steps[j].op == CRUSH_RULE_CHOOSE_INDEP || r->steps[j].op == CRUSH_RULE_CHOOSELEAF_INDEP ||
            r->steps[j].op == CRUSH_RULE_SET_CHOOSE_TRIES || r->steps[j].op == CRUSH_RULE_SET_CHOOSELEAF_TRIES) {
            return true;
        }
    }
    return false;
...
{
    if (choose_args.empty()) return false;
    if (choose_args.size() > 1) return true;
    if (choose_args.begin()->first != DEFAULT_CHOOSE_ARGS) return true;
    crush_choose_arg_map arg_map = choose_args.begin()->second;
    for (__u32 i = 0; i < arg_map.size; i++) {
        crush_choose_arg* arg = &arg_map.args[i];
        if (arg->weight_set_positions == 0 && arg->ids_size == 0) continue;
        if (arg->weight_set_positions != 1) return true;
        if (arg->ids_size != 0) return true;
    }
    return false;
}
...
{
    if (!item_exists(i)) return -EINVAL;
    ...
    if (!name_exists(name_no_class)) return -ENOENT;
    string class_name = name.substr(pos + 1);
    if (!class_exists(class_name)) return -ENOENT;
    *idout = get_item_id(name_no_class);
    *classout = get_class_id(class_name);
    return 0;
}

int CrushWrapper::can_rename_item(const string& srcname, const string& dstname, ostream* ss) const
{
...
int CrushWrapper::rename_item(const string& srcname, const string& dstname, ostream* ss)
{
...
int CrushWrapper::can_rename_bucket(const string& srcname, const string& dstname, ostream* ss) const
{
    int ret = can_rename_item(srcname, dstname, ss);
    if (ret) return ret;
...
int CrushWrapper::rename_bucket(const string& srcname, const string& dstname, ostream* ss)
{
...
int CrushWrapper::rename_rule(const string& srcname, const string& dstname, ostream* ss)
{
...
void CrushWrapper::find_takes(set<int>* roots) const
{
    for (unsigned i = 0; i < crush->max_rules; i++) {
        ...
        if (!r) continue;
        for (unsigned j = 0; j < r->len; j++) {
            if (r->steps[j].op == CRUSH_RULE_TAKE) roots->insert(r->steps[j].arg1);
        }
    }
}
...
{
    if (rule < 0 || rule >= (int)crush->max_rules) return;
    crush_rule* r = crush->rules[rule];
    if (!r) return;
    for (unsigned i = 0; i < r->len; i++) {
        if (r->steps[i].op == CRUSH_RULE_TAKE) roots->insert(r->steps[i].arg1);
    }
}
...
{
    for (int i = 0; i < crush->max_buckets; i++) {
        if (!crush->buckets[i]) continue;
        crush_bucket* b = crush->buckets[i];
        if (!_search_item_exists(b->id)) roots->insert(b->id);
    }
}
...
bool CrushWrapper::subtree_contains(int root, int item) const
{
    ...
    if (root >= 0) return false;   // root is a leaf

    const crush_bucket* b = get_bucket(root);
    if (IS_ERR(b)) return false;

    for (unsigned j = 0; j < b->size; j++) {
        if (subtree_contains(b->items[j], item)) return true;
    }
    return false;
}
...
bool CrushWrapper::_maybe_remove_last_instance(CephContext* cct, int item, bool unlink_only)
{
    ...
    if (item < 0 && !unlink_only) {
        ...
        if (class_bucket.count(item) != 0) class_bucket.erase(item);
    ...
    for (auto& i : choose_args) {
        ...
        for (int j = 0; j < crush->max_buckets; ++j) {
            ...
            if (!b || b->alg != CRUSH_BUCKET_STRAW2) {
                if (carg.ids) {
                    if (cct)
                        ldout(cct, 10) << __func__ << " removing " << i.first << " bucket " << (-1 - j) << " ids"
                                       << dendl;
                    free(carg.ids);
                    carg.ids = 0;
                    carg.ids_size = 0;
                ...
                if (carg.weight_set) {
                    if (cct)
                        ldout(cct, 10) << __func__ << " removing " << i.first << " bucket " << (-1 - j)
                                       << " weight_sets" << dendl;
            ...
            if (carg.weight_set_positions != positions) {
                if (cct)
                    lderr(cct) << __func__ << " " << i.first << " bucket " << (-1 - j) << " positions "
                               << carg.weight_set_positions << " -> " << positions << dendl;
                continue;   // wth... skip!
            }
            ...
            for (unsigned p = 0; p < positions; ++p) {
                if (carg.weight_set[p].size != b->size) {
                    if (cct)
                        lderr(cct) << __func__ << " fixing " << i.first << " bucket " << (-1 - j) << " position " << p
                                   << " size " << carg.weight_set[p].size << " -> " << b->size << dendl;
...
int CrushWrapper::remove_item(CephContext* cct, int item, bool unlink_only)
{
    ...
    if (_maybe_remove_last_instance(cct, item, unlink_only)) ret = 0;
    ...
    for (unsigned i = 0; i < crush->max_rules; ++i) {
        ...
        for (unsigned j = 0; j < r->len; ++j) {
            if (r->steps[j].op == CRUSH_RULE_TAKE) {
                ...
                if (step_item == item || original_item == item) return true;
            }
        }
    }
    return false;
...
{
    ldout(cct, 5) << "_remove_item_under " << item << " under " << ancestor << (unlink_only ? " unlink_only" : "")
                  << dendl;
    ...
    if (!bucket_exists(ancestor)) return -EINVAL;
...
{
    ldout(cct, 5) << "remove_item_under " << item << " under " << ancestor << (unlink_only ? " unlink_only" : "")
                  << dendl;
...
int CrushWrapper::get_common_ancestor_distance(CephContext* cct, int id, const std::multimap<string, string>& loc) const
{
    ldout(cct, 5) << __func__ << " " << id << " " << loc << dendl;
    if (!item_exists(id)) return -ENOENT;
    ...
    for (map<int, string>::const_iterator p = type_map.begin(); p != type_map.end(); ++p) {
        ...
        if (ip == id_loc.end()) continue;
        for (std::multimap<string, string>::const_iterator q = loc.find(p->second); q != loc.end(); ++q) {
            if (q->first != p->second) break;
            if (q->second == ip->second) return p->first;
        }
    }
    return -ERANGE;
}

int CrushWrapper::parse_loc_map(const std::vector<string>& args, std::map<string, string>* ploc)
{
    ploc->clear();
...
int CrushWrapper::parse_loc_multimap(const std::vector<string>& args, std::multimap<string, string>* ploc)
{
    ploc->clear();
    for (unsigned i = 0; i < args.size(); ++i) {
        const char* s = args[i].c_str();
...
bool CrushWrapper::check_item_loc(CephContext* cct, int item, const map<string, string>& loc, int* weight)
{
    ...
    for (map<int, string>::const_iterator p = type_map.begin(); p != type_map.end(); ++p) {
        ...
        for (unsigned j = 0; j < b->size; j++) {
            if (b->items[j] == item) {
                ...
                if (weight) *weight = crush_get_bucket_item_weight(b, j);
                return true;
            }
        }
        return false;
...
int CrushWrapper::get_full_location(const string& name, map<string, string>* ploc)
{
    build_rmaps();
...
map<int, string> CrushWrapper::get_parent_hierarchy(int id) const
{
    ...
    if (type_counter < 0) type_counter = 0;

    // read the type map and get the name of the type with the largest ID
    int high_type = 0;
    if (!type_map.empty()) high_type = type_map.rbegin()->first;
...
void CrushWrapper::get_children_of_type(int id, int type, vector<int>* children, bool exclude_shadow) const
{
    if (id >= 0) {
        if (type == 0) {
            // want leaf?
...
int CrushWrapper::verify_upmap(CephContext* cct, int rule_id, int pool_size, const vector<int>& up)
{
    auto rule = get_rule(rule_id);
    if (IS_ERR(rule) || !rule) {
        lderr(cct) << __func__ << " rule " << rule_id << " does not exist" << dendl;
        return -ENOENT;
    }
    int root_bucket = 0;
    int cursor = 0;
    ...
    for (unsigned step = 0; step < rule->len; ++step) {
        ...
        case CRUSH_RULE_CHOOSELEAF_INDEP:
        {
            ...
            for (auto osd : up) {
                ...
                if (parent < 0) {
                    ...
                    ldout(cct, 1) << __func__ << " unable to get parent of osd." << osd << ", skipping for now"
                                  << dendl;
                }
            }
            for (auto i : osds_by_parent) {
                if (i.second.size() > 1) {
                    lderr(cct) << __func__ << " multiple osds " << i.second << " come from same failure domain "
                               << i.first << dendl;
                    return -EINVAL;
                }
            }
        ...
        case CRUSH_RULE_CHOOSE_INDEP:
        {
            ...
            if ((int)parents_of_type.size() > numrep) {
                lderr(cct) << __func__ << " number of buckets " << parents_of_type.size() << " exceeds desired "
                           << numrep << dendl;
                return -EINVAL;
            }
        } break;
                              ...
                              bool init_weight_sets)
{
    ...
    if (!is_valid_crush_name(name)) return -EINVAL;

    if (!is_valid_crush_loc(cct, loc)) return -EINVAL;
    ...
    for (auto p = type_map.begin(); p != type_map.end(); ++p) {
        ...
        if (!name_exists(q->second)) {
            ...
            int r = add_bucket(0, 0, CRUSH_HASH_DEFAULT, p->first, 1, &cur, &zero_weight, &new_bucket_id);
            if (r < 0) {
        ...
        if (subtree_contains(cur, b->id)) {
            ldout(cct, 1) << "insert_item " << cur << " already contains " << b->id << "; cannot form loop" << dendl;
            return -ELOOP;
        }
    ...
    if (adjust_item_weightf_in_loc(cct, item, weight, loc, item >= 0 && init_weight_sets) > 0) {
...
int CrushWrapper::move_bucket(CephContext* cct, int id, const map<string, string>& loc)
{
    ...
    return insert_item(cct, id, bucket_weight / (float)0x10000, id_name, loc, false);
}
...
{
    if (src >= 0 || dst >= 0) return -EINVAL;
    if (!item_exists(src) || !item_exists(dst)) return -EINVAL;
                                      ...
                                      bool init_weight_sets)
{
    ...
    if (check_item_loc(cct, item, loc, &old_iweight)) {
        ...
        ret = insert_item(cct, item, weight, name, loc, item >= 0 && init_weight_sets);
        if (ret == 0) ret = 1;   // changed
    }
    return ret;
}
                              ...
                              const map<string, string>& loc)   // typename -> bucketname
{
    ...
    if (check_item_loc(cct, item, loc, &old_iweight)) {
        ...
        if (old_iweight != iweight) {
            ldout(cct, 5) << "update_item " << item << " adjusting weight " << ((float)old_iweight / (float)0x10000)
                          << " -> " << weight << dendl;
...
int CrushWrapper::get_item_weight(int id) const
{
    for (int bidx = 0; bidx < crush->max_buckets; bidx++) {
        ...
        if (b == NULL) continue;
        if (b->id == id) return b->weight;
        for (unsigned i = 0; i < b->size; i++)
            if (b->items[i] == id) return crush_get_bucket_item_weight(b, i);
    }
    return -ENOENT;
}
...
int CrushWrapper::adjust_item_weight(CephContext* cct, int id, int weight, bool update_weight_sets)
{
    ldout(cct, 5) << __func__ << " " << id << " weight " << weight << " update_weight_sets=" << (int)update_weight_sets
                  << dendl;
    int changed = 0;
    for (int bidx = 0; bidx < crush->max_buckets; bidx++) {
        ...
        int r = adjust_item_weight_in_bucket(cct, id, weight, -1 - bidx, update_weight_sets);
        if (r > 0) {
            ++changed;
        }
    }
...
int CrushWrapper::adjust_item_weight_in_bucket(CephContext* cct, int id, int weight, int bucket_id,
                                               bool update_weight_sets)
{
    ldout(cct, 5) << __func__ << " " << id << " weight " << weight << " in bucket " << bucket_id
                  << " update_weight_sets=" << (int)update_weight_sets << dendl;
    int changed = 0;
    ...
    for (unsigned int i = 0; i < b->size; i++) {
        if (b->items[i] == id) {
            int diff = bucket_adjust_item_weight(cct, b, id, weight, update_weight_sets);
    ...
    for (auto& p : choose_args) {
        ...
        ldout(cct, 5) << __func__ << "  adjusting bucket " << bucket_id << " cmap " << p.first << " weights to " << w
                      << dendl;
        ostringstream ss;
...
int CrushWrapper::adjust_item_weight_in_loc(CephContext* cct, int id, int weight, const map<string, string>& loc,
                                            bool update_weight_sets)
{
    ldout(cct, 5) << "adjust_item_weight_in_loc " << id << " weight " << weight << " in " << loc
                  << " update_weight_sets=" << (int)update_weight_sets << dendl;
    int changed = 0;
    for (auto l = loc.begin(); l != loc.end(); ++l) {
        int bid = get_item_id(l->second);
        if (!bucket_exists(bid)) continue;
        int r = adjust_item_weight_in_bucket(cct, id, weight, bid, update_weight_sets);
        if (r > 0) {
            ++changed;
        }
    }
...
int CrushWrapper::adjust_subtree_weight(CephContext* cct, int id, int weight, bool update_weight_sets)
{
...
pair<string, string> CrushWrapper::get_immediate_parent(int id, int* _ret) const
{
    ...
    for (int bidx = 0; bidx < crush->max_buckets; bidx++) {
        crush_bucket* b = crush->buckets[bidx];
        if (b == 0) continue;
        if (is_shadow_item(b->id)) continue;
        for (unsigned i = 0; i < b->size; i++)
            if (b->items[i] == id) {
...
int CrushWrapper::rename_class(const string& srcname, const string& dstname)
{
    ...
    if (i == class_rname.end()) return -ENOENT;
    auto j = class_rname.find(dstname);
    if (j != class_rname.end()) return -EEXIST;
...
int CrushWrapper::populate_classes(const std::map<int32_t, map<int32_t, int32_t>>& old_class_bucket)
{
    ...
    for (auto& r : roots) {
        ...
        for (auto& c : class_name) {
            int clone;
            int res = device_class_clone(r, c.first, old_class_bucket, used_ids, &clone, &cmap_item_weight);
            if (res < 0) return res;
        }
    }
    return 0;
...
int CrushWrapper::set_subtree_class(const string& subtree, const string& new_class)
{
...
int CrushWrapper::reclassify(CephContext* cct, ostream& out, const map<string, string>& classify_root,
                             const map<string, pair<string, string>>& classify_bucket)
{
    ...
    for (auto& i : classify_root) {
        ...
        int new_class_id = get_or_create_class_id(new_class);
        out << "classify_root " << root << " (" << root_id << ") as " << new_class << std::endl;
                    ...
                    if (rule->steps[k].op == CRUSH_RULE_TAKE) {
                        ...
                        if (c >= 0) {
                            if (original_item == root_id) {
                                out << "  rule " << j << " includes take on root " << root << " class " << c
                                    << std::endl;
                                return -EINVAL;
                            }
                        }
        ...
        while (!q.empty()) {
            ...
            if (IS_ERR(bucket)) {
                out << "cannot find bucket " << id << ": " << cpp_strerror(PTR_ERR(bucket)) << std::endl;
            ...
            bucket->id = new_id;
            crush->buckets[-1 - id] = crush_make_bucket(crush, bucket->alg, bucket->hash, bucket->type, 0, NULL, NULL);
        ...
        if (r < 0) {
            out << "failed to rebuild_roots_with_classes: " << cpp_strerror(r) << std::endl;
            return r;
        }
    ...
    for (auto& i : classify_bucket) {
        ...
        if (!name_exists(default_parent)) {
            out << "default parent " << default_parent << " does not exist" << std::endl;
            return -EINVAL;
        }
        ...
        out << "classify_bucket " << match << " as " << new_class << " default bucket " << default_parent << " ("
        ...
        for (int j = 0; j < crush->max_buckets; ++j) {
            ...
            if (match[0] == '%') {
                ...
                if (match.substr(0, match.size() - 1) != name.substr(0, match.size() - 1)) {
                    continue;
                }
            ...
            cout << "match " << match << " to " << name << " basename " << basename << std::endl;
            ...
            if (name_exists(basename)) {
                ...
                crush->buckets[-1 - base_id] = crush_make_bucket(crush, b->alg, b->hash, b->type, 0, NULL, NULL);
    ...
    for (auto& i : send_to) {
        ...
        cout << "moving items from " << from->id << " (" << get_item_name(from->id) << ") to " << to->id << " ("
             << get_item_name(to->id) << ")" << std::endl;
        for (unsigned j = 0; j < from->size; ++j) {
            ...
            if (item >= 0) {
                ...
                r = insert_item(cct, item, w, get_item_name(item), to_loc);
            }
            else {
                if (!send_to.count(item)) {
                    lderr(cct) << "item " << item << " in bucket " << from->id << " is not also a reclassified bucket"
                               << dendl;
                    return -EINVAL;
                }
            ...
            if (r != 0) {
                cout << __func__ << " err from insert_item: " << cpp_strerror(r) << std::endl;
                return r;
            }
        }
    }
    ...
    for (auto& i : new_buckets) {
        ...
        if (get_immediate_parent_id(i.first, &parent) < 0) {
            cout << "new bucket " << i.first << " missing parent, adding at " << i.second << std::endl;
...
{
    int id = -1;
    while (crush->buckets[-1 - id] && -1 - id < crush->max_buckets) {
        id--;
    }
    if (-1 - id == crush->max_buckets) {
        ++crush->max_buckets;
        crush->buckets = (struct crush_bucket**)realloc(crush->buckets, sizeof(crush->buckets[0]) * crush->max_buckets);
        for (auto& i : choose_args) {
            assert(i.second.size == (__u32)crush->max_buckets - 1);
            ++i.second.size;
            i.second.args = (struct crush_choose_arg*)realloc(i.second.args, sizeof(i.second.args[0]) * i.second.size);
        }
    }
    return id;
}
...
void CrushWrapper::reweight_bucket(crush_bucket* b, crush_choose_arg_map& arg_map, vector<uint32_t>* weightv)
{
    int idx = -1 - b->id;
...
int CrushWrapper::add_simple_rule_at(string name, string root_name, string failure_domain_name, string device_class,
                                     string mode, int rule_type, int rno, ostream* err)
{
    if (rule_exists(name)) {
        if (err) *err << "rule " << name << " exists";
        return -EEXIST;
    }
    if (rno >= 0) {
        if (rule_exists(rno)) {
            if (err) *err << "rule with ruleno " << rno << " exists";
            return -EEXIST;
        }
        if (ruleset_exists(rno)) {
            if (err) *err << "ruleset " << rno << " exists";
            return -EEXIST;
        }
    }
    else {
        for (rno = 0; rno < get_max_rules(); rno++) {
            if (!rule_exists(rno) && !ruleset_exists(rno)) break;
        }
    }
    if (!name_exists(root_name)) {
        if (err) *err << "root item " << root_name << " does not exist";
        return -ENOENT;
    }
    ...
    if (failure_domain_name.length()) {
        ...
        if (type < 0) {
            if (err) *err << "unknown type " << failure_domain_name;
            return -EINVAL;
        }
    }
    if (device_class.size()) {
        if (!class_exists(device_class)) {
            if (err) *err << "device class " << device_class << " does not exist";
            return -EINVAL;
        }
        int c = get_class_id(device_class);
        if (class_bucket.count(root) == 0 || class_bucket[root].count(c) == 0) {
            if (err) *err << "root " << root_name << " has no devices with class " << device_class;
            return -EINVAL;
        }
    ...
    if (type)
        ...
        crush_rule_set_step(
            rule, step++, mode == "firstn" ? CRUSH_RULE_CHOOSE_FIRSTN : CRUSH_RULE_CHOOSE_INDEP, CRUSH_CHOOSE_N, 0);
...
int CrushWrapper::add_simple_rule(string name, string root_name, string failure_domain_name, string device_class,
                                  string mode, int rule_type, ostream* err)
{
    return add_simple_rule_at(name, root_name, failure_domain_name, device_class, mode, rule_type, -1, err);
}

float CrushWrapper::_get_take_weight_osd_map(int root, map<int, float>* pmap) const
{
    float sum = 0.0;
    list<int> q;
    q.push_back(root);
...
void CrushWrapper::_normalize_weight_map(float sum, const map<int, float>& m, map<int, float>* pmap) const
{
    for (auto& p : m) {
...
int CrushWrapper::get_rule_weight_osd_map(unsigned ruleno, map<int, float>* pmap) const
{
    if (ruleno >= crush->max_rules) return -ENOENT;
    if (crush->rules[ruleno] == NULL) return -ENOENT;
...
{
    if (ruleno >= (int)crush->max_rules) return -ENOENT;
    if (crush->rules[ruleno] == NULL) return -ENOENT;
                                            ...
                                            bool adjust_weight_sets)
{
    if (adjust_weight_sets) {
        ...
        for (position = 0; position < bucket->size; position++)
            if (bucket->items[position] == item) break;
...
int CrushWrapper::add_bucket(int bucketno, int alg, int hash, int type, int size, int* items, int* weights, int* idout)
{
    if (alg == 0) {
        alg = get_default_bucket_alg();
        if (alg == 0) return -EINVAL;
    }
    ...
    for (auto& p : choose_args) {
        ...
        if (cmap.args) {
            if ((int)cmap.size < crush->max_buckets) {
                cmap.args = static_cast<crush_choose_arg*>(realloc(cmap.args, sizeof(crush_choose_arg) * new_size));
                ceph_assert(cmap.args);
                memset(&cmap.args[cmap.size], 0, sizeof(crush_choose_arg) * (new_size - cmap.size));
                cmap.size = new_size;
            }
        }
        else {
            cmap.args = static_cast<crush_choose_arg*>(calloc(sizeof(crush_choose_arg), new_size));
            ceph_assert(cmap.args);
            cmap.size = new_size;
        }
        if (size > 0) {
            ...
            carg.weight_set = static_cast<crush_weight_set*>(calloc(sizeof(crush_weight_set), size));
...
int CrushWrapper::bucket_remove_item(crush_bucket* bucket, int item)
{
    ...
    for (auto& w : choose_args) {
        ...
        for (__u32 j = 0; j < arg->weight_set_positions; j++) {
            crush_weight_set* weight_set = &arg->weight_set[j];
            ceph_assert(weight_set->size - 1 == new_size);
            for (__u32 k = position; k < new_size; k++) weight_set->weights[k] = weight_set->weights[k + 1];
            if (new_size) {
                weight_set->weights = (__u32*)realloc(weight_set->weights, new_size * sizeof(__u32));
            }
            else {
        ...
        if (arg->ids_size) {
            ceph_assert(arg->ids_size - 1 == new_size);
            for (__u32 k = position; k < new_size; k++) arg->ids[k] = arg->ids[k + 1];
            if (new_size) {
                arg->ids = (__s32*)realloc(arg->ids, new_size * sizeof(__s32));
            }
            else {
...
int CrushWrapper::update_device_class(int id, const string& class_name, const string& name, ostream* ss)
{
    ceph_assert(item_exists(id));
    auto old_class_name = get_item_class(id);
    if (old_class_name && old_class_name != class_name) {
        *ss << "osd." << id << " has already bound to class '" << old_class_name << "', can not reset class to '"
            << class_name << "'; "
                                     ...
                                     map<int, map<int, vector<int>>>* cmap_item_weight)
{
    ...
    crush_bucket* copy = crush_make_bucket(crush, original->alg, original->hash, original->type, 0, NULL, NULL);
    ceph_assert(copy);
    ...
    for (unsigned i = 0; i < original->size; i++) {
        ...
        if (item >= 0) {
            ...
            int child_copy_id;
            int res =
                device_class_clone(item, device_class, old_class_bucket, used_ids, &child_copy_id, cmap_item_weight);
            if (res < 0) return res;
    ...
    int bno = 0;
    if (old_class_bucket.count(original_id) && old_class_bucket.at(original_id).count(device_class)) {
        ...
        bno = -1;
        while (((-1 - bno) < crush->max_buckets && crush->buckets[-1 - bno]) || used_ids.count(bno)) {
            --bno;
        }
    }
    ...
    if (have_rmaps) name_rmap[copy_name] = *clone;
    ...
    for (auto& w : choose_args) {
        ...
        if (crush->max_buckets > (int)cmap.size) {
            unsigned new_size = crush->max_buckets;
            cmap.args = static_cast<crush_choose_arg*>(realloc(cmap.args, new_size * sizeof(cmap.args[0])));
            ceph_assert(cmap.args);
            memset(cmap.args + cmap.size, 0, (new_size - cmap.size) * sizeof(cmap.args[0]));
            cmap.size = new_size;
        }
        ...
        n.weight_set = static_cast<crush_weight_set*>(calloc(n.weight_set_positions, sizeof(crush_weight_set)));
...
void CrushWrapper::encode(bufferlist& bl, uint64_t features) const
{
    ...
    if (has_choose_args() && !HAVE_FEATURE(features, CRUSH_CHOOSE_ARGS)) {
    ...
    for (int i = 0; i < crush->max_buckets; i++) {
        ...
        for (unsigned j = 0; j < crush->buckets[i]->size; j++) encode(crush->buckets[i]->items[j], bl);
        ...
        {
            __u32* weights;
            if (encode_compat_choose_args && arg_map.args[i].weight_set_positions > 0) {
    ...
    for (unsigned i = 0; i < crush->max_rules; i++) {
        ...
        for (unsigned j = 0; j < crush->rules[i]->len; j++) encode(crush->rules[i]->steps[j], bl);
    }
...
void CrushWrapper::decode(bufferlist::const_iterator& blp)
{
    ...
    __u32 magic;
    decode(magic, blp);
    if (magic != CRUSH_MAGIC) throw ceph::buffer::malformed_input("bad magic number");
    ...
    try {
        ...
        for (unsigned i = 0; i < crush->max_rules; ++i) {
            ...
            for (unsigned j = 0; j < crush->rules[i]->len; j++) decode(crush->rules[i]->steps[j], blp);
        }
        ...
        if (!blp.end()) {
            ...
            for (auto& c : class_name) class_rname[c.second] = c.first;
        ...
        if (!blp.end()) {
            ...
            for (__u32 i = 0; i < choose_args_size; i++) {
                ...
                arg_map.args = static_cast<crush_choose_arg*>(calloc(arg_map.size, sizeof(crush_choose_arg)));
                __u32 size;
                decode(size, blp);
                for (__u32 j = 0; j < size; j++) {
                    ...
                    if (arg->weight_set_positions) {
                        arg->weight_set =
                            static_cast<crush_weight_set*>(calloc(arg->weight_set_positions, sizeof(crush_weight_set)));
                        for (__u32 k = 0; k < arg->weight_set_positions; k++) {
                            ...
                            for (__u32 l = 0; l < weight_set->size; l++) decode(weight_set->weights[l], blp);
                        }
                    }
                    ...
                    if (arg->ids_size) {
                        ...
                        for (__u32 k = 0; k < arg->ids_size; k++) decode(arg->ids[k], blp);
                    }
                }
...
void CrushWrapper::decode_crush_bucket(crush_bucket** bptr, bufferlist::const_iterator& blp)
{
    ...
    int size = 0;
    switch (alg) {
    case CRUSH_BUCKET_UNIFORM: size = sizeof(crush_bucket_uniform); break;
    case CRUSH_BUCKET_LIST: size = sizeof(crush_bucket_list); break;
    case CRUSH_BUCKET_TREE: size = sizeof(crush_bucket_tree); break;
    case CRUSH_BUCKET_STRAW: size = sizeof(crush_bucket_straw); break;
    case CRUSH_BUCKET_STRAW2: size = sizeof(crush_bucket_straw2); break;
    default:
    {
        char str[128];
    ...
    *bptr = bucket;
    ...
    switch (bucket->alg) {
    case CRUSH_BUCKET_UNIFORM: decode((reinterpret_cast<crush_bucket_uniform*>(bucket))->item_weight, blp); break;
...
void CrushWrapper::dump(Formatter* f) const
{
    ...
    for (int i = 0; i < get_max_devices(); i++) {
        ...
        if (device_class != NULL) f->dump_string("class", device_class);
        f->close_section();
    }
    f->close_section();
    ...
    for (int bucket = -1; bucket > -1 - get_max_buckets(); --bucket) {
        if (!bucket_exists(bucket)) continue;
        f->open_object_section("bucket");
        f->dump_int("id", bucket);
        if (get_item_name(bucket)) f->dump_string("name", get_item_name(bucket));
        f->dump_int("type_id", get_bucket_type(bucket));
        if (get_type_name(get_bucket_type(bucket))) f->dump_string("type_name", get_type_name(get_bucket_type(bucket)));
...
public:
    explicit TreeDumper(const CrushWrapper* crush, const CrushTreeDumper::name_map_t& wsnames)
        : crush(crush)
...
void CrushWrapper::dump_tree(Formatter* f, const CrushTreeDumper::name_map_t& weight_set_names) const
{
    ceph_assert(f);
    ...
    for (auto c : choose_args) {
        ...
        for (__u32 i = 0; i < arg_map.size; i++) {
            ...
            if (arg->ids_size > 0) {
                ...
                for (__u32 j = 0; j < arg->ids_size; j++) f->dump_int("id", arg->ids[j]);
                f->close_section();
            }
            f->close_section();
        }
...
void CrushWrapper::dump_rule(int ruleset, Formatter* f) const
{
    ...
    if (get_rule_name(ruleset)) f->dump_string("rule_name", get_rule_name(ruleset));
    ...
    for (int j = 0; j < get_rule_len(ruleset); j++) {
        ...
        case CRUSH_RULE_NOOP: f->dump_string("op", "noop"); break;
        case CRUSH_RULE_TAKE:
            ...
            break;
        case CRUSH_RULE_EMIT: f->dump_string("op", "emit"); break;
        case CRUSH_RULE_CHOOSE_FIRSTN:
...
public:
    ...
    explicit CrushTreePlainDumper(const CrushWrapper* crush, const CrushTreeDumper::name_map_t& wsnames)
    ...
    void dump(TextTable* tbl)
    {
        ...
        for (auto& p : crush->choose_args) {
            if (p.first == CrushWrapper::DEFAULT_CHOOSE_ARGS) {
                ...
                name = q != weight_set_names.end() ? q->second : stringify(p.first);
        ...
        *tbl << qi.id << c << weightf_t(qi.weight);
        for (auto& p : crush->choose_args) {
            if (qi.parent < 0) {
                ...
                if (b && bidx < (int)cmap.size && cmap.args[bidx].weight_set &&
                    cmap.args[bidx].weight_set_positions >= 1) {
                    int pos;
                    for (pos = 0; pos < (int)cmap.args[bidx].weight_set[0].size && b->items[pos] != qi.id; ++pos)
                        ;
                    *tbl << weightf_t((float)cmap.args[bidx].weight_set[0].weights[pos] / (float)0x10000);
                    continue;
                }
            }
            *tbl << "";
        ...
        if (qi.is_bucket()) {
            ss << crush->get_type_name(crush->get_bucket_type(qi.id)) << " " << crush->get_item_name(qi.id);
        }
        else {
            ss << "osd." << qi.id;
        }
...
public:
    ...
    explicit CrushTreeFormattingDumper(const CrushWrapper* crush, const CrushTreeDumper::name_map_t& wsnames)
        : Parent(crush, wsnames)
    {}
...
void CrushWrapper::dump_tree(ostream* out, Formatter* f, const CrushTreeDumper::name_map_t& weight_set_names,
...
bool CrushWrapper::is_valid_crush_loc(CephContext* cct, const map<string, string>& loc)
{
    for (map<string, string>::const_iterator l = loc.begin(); l != loc.end(); ++l) {
        if (!is_valid_crush_name(l->first) || !is_valid_crush_name(l->second)) {
            ldout(cct, 1) << "loc[" << l->first << "] = '" << l->second << "' not a valid crush name ([A-Za-z0-9_-.]+)"
                          << dendl;
            return false;
        }
    }
...
int CrushWrapper::_choose_type_stack(CephContext* cct, const vector<pair<int, int>>& stack, const set<int>& overfull,
                                     const vector<int>& underfull, const vector<int>& more_underfull,
                                     const vector<int>& orig, vector<int>::const_iterator& i, set<int>& used,
                                     vector<int>* pw, int root_bucket, int rule) const
{
    vector<int> w = *pw;
    vector<int> o;
    ...
    for (unsigned j = 0; j < stack.size(); ++j) {
        ...
        ldout(cct, 10) << " level " << j << ": type " << type << " fanout " << fanout << " cumulative " << cum_fanout
                       << " w " << w << dendl;
        vector<int> o;
        auto tmpi = i;
        if (i == orig.end()) {
        ...
        for (auto from : w) {
            ...
            for (int pos = 0; pos < fanout; ++pos) {
                if (type > 0) {
                    ...
                    ldout(cct, 10) << __func__ << "   from " << *tmpi << " got " << item << " of type " << type
                                   << " over leaves " << leaves[pos] << dendl;
                }
                else {
                    // leaf
                    bool replaced = false;
                    if (overfull.count(*i)) {
                        for (auto item : underfull) {
                            ldout(cct, 10)
                                << __func__ << " pos " << pos << " was " << *i << " considering " << item << dendl;
                            if (used.count(item)) {
                            ...
                            used.insert(item);
                            ldout(cct, 10)
                                << __func__ << " pos " << pos << " replace " << *i << " -> " << item << dendl;
                            replaced = true;
                        ...
                        if (!replaced) {
                            for (auto item : more_underfull) {
                                ldout(cct, 10) << __func__ << " more underfull pos " << pos << " was " << *i
                                               << " considering " << item << dendl;
                                if (used.count(item)) {
                                    ldout(cct, 20) << __func__ << "   in used " << used << dendl;
                                    continue;
                                }
                    ...
                    if (underfull_buckets[j].count(o[pos]) == 0) {
                        ...
                        if (any_overfull) {
                            ldout(cct, 10) << " bucket " << o[pos] << " has no underfull targets and "
                                           ...
                                           << underfull_buckets[j] << dendl;
                            for (auto alt : underfull_buckets[j]) {
                                if (std::find(o.begin(), o.end(), alt) == o.end()) {
                                    // see if alt has the same parent
                                    if (j == 0 || get_parent_of_type(o[pos], stack[j - 1].first, rule) ==
                                                      get_parent_of_type(alt, stack[j - 1].first, rule)) {
                                        if (j)
                                            ldout(cct, 10)
                                                << "  replacing " << o[pos] << " (which has no underfull leaves) with "
                                                << alt << " (same parent "
                                                << get_parent_of_type(alt, stack[j - 1].first, rule) << " type " << type
                                                << ")" << dendl;
                                        else
                                            ldout(cct, 10)
                                                << "  replacing " << o[pos] << " (which has no underfull leaves) with "
                                                << alt << " (first level)" << dendl;
                                        o[pos] = alt;
                                        break;
                                    }
                                    else {
...
int CrushWrapper::try_remap_rule(CephContext* cct, int ruleno, int maxout, const set<int>& overfull,
                                 const vector<int>& underfull, const vector<int>& more_underfull,
                                 const vector<int>& orig, vector<int>* out) const
{
    const crush_map* map = crush;
    const crush_rule* rule = get_rule(ruleno);
    ceph_assert(rule);

    ldout(cct, 10) << __func__ << " ruleno " << ruleno << " numrep " << maxout << " overfull " << overfull
                   << " underfull " << underfull << " more_underfull " << more_underfull << " orig " << orig << dendl;
    ...
    for (unsigned step = 0; step < rule->len; ++step) {
        ...
        case CRUSH_RULE_CHOOSELEAF_INDEP:
        {
            int numrep = curstep->arg1;
            int type = curstep->arg2;
            if (numrep <= 0) numrep += maxout;
            type_stack.push_back(make_pair(type, numrep));
            if (type > 0) type_stack.push_back(make_pair(0, 1));
            int r = _choose_type_stack(
                cct, type_stack, overfull, underfull, more_underfull, orig, i, used, &w, root_bucket, ruleno);
            if (r < 0) return r;
            type_stack.clear();
        } break;
...
int CrushWrapper::_choose_args_adjust_item_weight_in_bucket(CephContext* cct, crush_choose_arg_map cmap, int bucketid,
                                                            int id, const vector<int>& weight, ostream* ss)
{
    int changed = 0;
    int bidx = -1 - bucketid;
    crush_bucket* b = crush->buckets[bidx];
    if (bidx >= (int)cmap.size) {
        if (ss) *ss << "no weight-set for bucket " << b->id;
    ...
    if (carg->weight_set == NULL) {
        ...
        carg->weight_set_positions = positions;
        carg->weight_set = static_cast<crush_weight_set*>(calloc(sizeof(crush_weight_set), positions));
        for (unsigned p = 0; p < positions; ++p) {
            carg->weight_set[p].size = b->size;
            carg->weight_set[p].weights = (__u32*)calloc(b->size, sizeof(__u32));
            for (unsigned i = 0; i < b->size; ++i) {
    ...
    if (carg->weight_set_positions != weight.size()) {
        if (ss) *ss << "weight_set_positions != " << weight.size() << " for bucket " << b->id;
...
int CrushWrapper::choose_args_adjust_item_weight(CephContext* cct, crush_choose_arg_map cmap, int id,
                                                 const vector<int>& weight, ostream* ss)
{
    ldout(cct, 5) << __func__ << " " << id << " weight " << weight << dendl;
    int changed = 0;
    for (int bidx = 0; bidx < crush->max_buckets; bidx++) {
        ...
        changed += _choose_args_adjust_item_weight_in_bucket(cct, cmap, b->id, id, weight, ss);
    }
    if (!changed) {
        if (ss) *ss << "item " << id << " not found in crush map";
        return -ENOENT;
    }
    return changed;
}
