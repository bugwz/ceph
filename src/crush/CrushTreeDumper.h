...
struct Item
{
    ...
    Item()
        ...
        , weight(0)
    {}
    Item(int i, int p, int d, float w)
        : id(i)
        , parent(p)
        , depth(d)
        , weight(w)
...
template<typename F> class Dumper : public std::list<Item>
{
public:
    explicit Dumper(const CrushWrapper* crush_, const name_map_t& weight_set_names_)
        : crush(crush_)
    ...
    explicit Dumper(const CrushWrapper* crush_, const name_map_t& weight_set_names_, bool show_shadow)
        : crush(crush_)
        , weight_set_names(weight_set_names_)
    {
        if (show_shadow) {
    ...
    virtual bool should_dump_leaf(int i) const { return true; }
    virtual bool should_dump_empty_bucket() const { return true; }

    bool should_dump(int id)
    {
        if (id >= 0) return should_dump_leaf(id);
        if (should_dump_empty_bucket()) return true;
        int s = crush->get_bucket_size(id);
        for (int k = s - 1; k >= 0; k--) {
            int c = crush->get_bucket_item(id, k);
            if (should_dump(c)) return true;
        }
        return false;
    }
    ...
    {
        if (empty()) {
            while (root != roots.end() && !should_dump(*root)) ++root;
            if (root == roots.end()) return false;
        ...
        if (qi.is_bucket()) {
            ...
            for (int k = s - 1; k >= 0; k--) {
                ...
                if (should_dump(id)) {
                    ...
                    sorted[sort_by] = std::make_pair(id, crush->get_bucket_item_weightf(qi.id, k));
                }
            }
            for (auto p = sorted.rbegin(); p != sorted.rend(); ++p) {
                qi.children.push_back(p->second.first);
                push_front(Item(p->second.first, qi.id, qi.depth + 1, p->second.second));
            }
        }
        return true;
    }
...
inline void dump_item_fields(const CrushWrapper* crush, const name_map_t& weight_set_names, const Item& qi,
                             ceph::Formatter* f)
{
    f->dump_int("id", qi.id);
    const char* c = crush->get_item_class(qi.id);
    if (c) f->dump_string("device_class", c);
    if (qi.is_bucket()) {
    ...
    if (qi.parent < 0) {
        ...
        for (auto& p : crush->choose_args) {
            ...
            if (b && bidx < (int)cmap.size && cmap.args[bidx].weight_set && cmap.args[bidx].weight_set_positions >= 1) {
                int bpos;
                for (bpos = 0; bpos < (int)cmap.args[bidx].weight_set[0].size && b->items[bpos] != qi.id; ++bpos)
                    ;
                std::string name;
                ...
                for (unsigned opos = 0; opos < cmap.args[bidx].weight_set_positions; ++opos) {
                    float w = (float)cmap.args[bidx].weight_set[opos].weights[bpos] / (float)0x10000;
...
inline void dump_bucket_children(const CrushWrapper* crush, const Item& qi, ceph::Formatter* f)
{
    if (!qi.is_bucket()) return;

    f->open_array_section("children");
    for (std::list<int>::const_iterator i = qi.children.begin(); i != qi.children.end(); ++i) {
        f->dump_int("child", *i);
    }
    f->close_section();
}
...
{
public:
    explicit FormattingDumper(const CrushWrapper* crush, const name_map_t& weight_set_names)
        : Dumper<ceph::Formatter>(crush, weight_set_names)
    {}
    explicit FormattingDumper(const CrushWrapper* crush, const name_map_t& weight_set_names, bool show_shadow)
