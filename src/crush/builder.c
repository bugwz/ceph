...
void crush_finalize(struct crush_map* map)
{
    ...
    map->working_size += map->max_buckets * sizeof(struct crush_work_bucket*);
    ...
    for (b = 0; b < map->max_buckets; b++) {
        ...
        for (i = 0; i < map->buckets[b]->size; i++)
            if (map->buckets[b]->items[i] >= map->max_devices) map->max_devices = map->buckets[b]->items[i] + 1;
...
int crush_add_rule(struct crush_map* map, struct crush_rule* rule, int ruleno)
{
    ...
    if (r >= map->max_rules) {
        ...
        int oldsize;
        void* _realloc = NULL;
        if (map->max_rules + 1 > CRUSH_MAX_RULES) return -ENOSPC;
...
int crush_get_next_bucket_id(struct crush_map* map)
{
    ...
    for (pos = 0; pos < map->max_buckets; pos++)
        if (map->buckets[pos] == 0) break;
    return -1 - pos;
}


int crush_add_bucket(struct crush_map* map, int id, struct crush_bucket* bucket, int* idout)
{
    int pos;

    /* find a bucket id */
    if (id == 0) id = crush_get_next_bucket_id(map);
    pos = -1 - id;
...
struct crush_bucket_uniform* crush_make_uniform_bucket(int hash, int type, int size, int* items, int item_weight)
{
    ...
    if (crush_multiplication_is_unsafe(size, item_weight)) goto err;
    ...
    if (!bucket->h.items) goto err;

    for (i = 0; i < size; i++) bucket->h.items[i] = items[i];
...
struct crush_bucket_list* crush_make_list_bucket(int hash, int type, int size, int* items, int* weights)
{
    ...
    if (!bucket->item_weights) goto err;
    bucket->sum_weights = malloc(sizeof(__u32) * size);
    if (!bucket->sum_weights) goto err;
    w = 0;
    for (i = 0; i < size; i++) {
        bucket->h.items[i] = items[i];
        bucket->item_weights[i] = weights[i];

        if (crush_addition_is_unsafe(w, weights[i])) goto err;
...
struct crush_bucket_tree* crush_make_tree_bucket(int hash, int type, int size, int* items, /* in leaf order */
                                                 int* weights)
{
    ...
    if (!bucket->node_weights) goto err;
    ...
    for (i = 0; i < size; i++) {
        ...
        if (crush_addition_is_unsafe(bucket->h.weight, weights[i])) goto err;

        bucket->h.weight += weights[i];
        for (j = 1; j < depth; j++) {
            node = parent(node);

            if (crush_addition_is_unsafe(bucket->node_weights[node], weights[i])) goto err;
...
int crush_calc_straw(struct crush_map* map, struct crush_bucket_straw* bucket)
{
    ...
    if (!reverse) return -ENOMEM;
    if (size) reverse[0] = 0;
    for (i = 1; i < size; i++) {
        for (j = 0; j < i; j++) {
            if (weights[i] < weights[reverse[j]]) {
                /* insert here */
                for (k = i; k > j; k--) reverse[k] = reverse[k - 1];
                reverse[j] = i;
                break;
            }
        }
...
struct crush_bucket_straw* crush_make_straw_bucket(struct crush_map* map, int hash, int type, int size, int* items,
                                                   int* weights)
{
    ...
    if (crush_calc_straw(map, bucket) < 0) goto err;
...
struct crush_bucket_straw2* crush_make_straw2_bucket(struct crush_map* map, int hash, int type, int size, int* items,
                                                     int* weights)
{
                                       ...
                                       int* weights)
{
    ...
    case CRUSH_BUCKET_LIST: return (struct crush_bucket*)crush_make_list_bucket(hash, type, size, items, weights);

    case CRUSH_BUCKET_TREE: return (struct crush_bucket*)crush_make_tree_bucket(hash, type, size, items, weights);
...
int crush_add_uniform_bucket_item(struct crush_bucket_uniform* bucket, int item, int weight)
{
    ...
    if (crush_addition_is_unsafe(bucket->h.weight, weight)) return -ERANGE;
...
int crush_add_list_bucket_item(struct crush_bucket_list* bucket, int item, int weight)
{
    ...
    if (newsize > 1) {
        ...
        if (crush_addition_is_unsafe(bucket->sum_weights[newsize - 2], weight)) return -ERANGE;
...
int crush_add_tree_bucket_item(struct crush_bucket_tree* bucket, int item, int weight)
{
    ...
    for (j = 1; j < depth; j++) {
        ...
        if (crush_addition_is_unsafe(bucket->node_weights[node], weight)) return -ERANGE;
...
int crush_bucket_add_item(struct crush_map* map, struct crush_bucket* b, int item, int weight)
{
    switch (b->alg) {
    case CRUSH_BUCKET_UNIFORM: return crush_add_uniform_bucket_item((struct crush_bucket_uniform*)b, item, weight);
    case CRUSH_BUCKET_LIST: return crush_add_list_bucket_item((struct crush_bucket_list*)b, item, weight);
    case CRUSH_BUCKET_TREE: return crush_add_tree_bucket_item((struct crush_bucket_tree*)b, item, weight);
    case CRUSH_BUCKET_STRAW: return crush_add_straw_bucket_item(map, (struct crush_bucket_straw*)b, item, weight);
    case CRUSH_BUCKET_STRAW2: return crush_add_straw2_bucket_item(map, (struct crush_bucket_straw2*)b, item, weight);
    default: return -1;
    }
}
...
int crush_remove_uniform_bucket_item(struct crush_bucket_uniform* bucket, int item)
{
    ...
    for (i = 0; i < bucket->h.size; i++)
        if (bucket->h.items[i] == item) break;
    if (i == bucket->h.size) return -ENOENT;

    for (j = i; j < bucket->h.size; j++) bucket->h.items[j] = bucket->h.items[j + 1];
...
int crush_remove_tree_bucket_item(struct crush_bucket_tree* bucket, int item)
{
    ...
    for (i = 0; i < bucket->h.size; i++) {
        ...
        if (bucket->h.items[i] != item) continue;
    ...
    while (newsize > 0) {
        ...
        if (bucket->node_weights[node]) break;
        --newsize;
    }
...
{
    switch (b->alg) {
    case CRUSH_BUCKET_UNIFORM: return crush_remove_uniform_bucket_item((struct crush_bucket_uniform*)b, item);
    case CRUSH_BUCKET_LIST: return crush_remove_list_bucket_item((struct crush_bucket_list*)b, item);
    case CRUSH_BUCKET_TREE: return crush_remove_tree_bucket_item((struct crush_bucket_tree*)b, item);
    case CRUSH_BUCKET_STRAW: return crush_remove_straw_bucket_item(map, (struct crush_bucket_straw*)b, item);
    case CRUSH_BUCKET_STRAW2: return crush_remove_straw2_bucket_item(map, (struct crush_bucket_straw2*)b, item);
    default: return -1;
    }
}
...
int crush_adjust_straw_bucket_item_weight(struct crush_map* map, struct crush_bucket_straw* bucket, int item,
                                          int weight)
{
    unsigned idx;
    int diff;
    ...
    for (idx = 0; idx < bucket->h.size; idx++)
        if (bucket->h.items[idx] == item) break;
    if (idx == bucket->h.size) return 0;
...
int crush_adjust_straw2_bucket_item_weight(struct crush_map* map, struct crush_bucket_straw2* bucket, int item,
                                           int weight)
{
    unsigned idx;
    int diff;
...
int crush_bucket_adjust_item_weight(struct crush_map* map, struct crush_bucket* b, int item, int weight)
{
    switch (b->alg) {
    case CRUSH_BUCKET_UNIFORM:
        return crush_adjust_uniform_bucket_item_weight((struct crush_bucket_uniform*)b, item, weight);
    case CRUSH_BUCKET_LIST: return crush_adjust_list_bucket_item_weight((struct crush_bucket_list*)b, item, weight);
    case CRUSH_BUCKET_TREE: return crush_adjust_tree_bucket_item_weight((struct crush_bucket_tree*)b, item, weight);
    case CRUSH_BUCKET_STRAW:
        return crush_adjust_straw_bucket_item_weight(map, (struct crush_bucket_straw*)b, item, weight);
    case CRUSH_BUCKET_STRAW2:
        return crush_adjust_straw2_bucket_item_weight(map, (struct crush_bucket_straw2*)b, item, weight);
    default: return -1;
    }
}
...
static int crush_reweight_uniform_bucket(struct crush_map* map, struct crush_bucket_uniform* bucket)
{
    ...
    for (i = 0; i < bucket->h.size; i++) {
        ...
        if (id < 0) {
            ...
            if (crush_addition_is_unsafe(sum, c->weight)) return -ERANGE;
    ...
    if (n > leaves) bucket->item_weight = sum / n;   // more bucket children than leaves, average!
    bucket->h.weight = bucket->item_weight * bucket->h.size;
...
static int crush_reweight_list_bucket(struct crush_map* map, struct crush_bucket_list* bucket)
{
    ...
    for (i = 0; i < bucket->h.size; i++) {
        ...
        if (crush_addition_is_unsafe(bucket->h.weight, bucket->item_weights[i])) return -ERANGE;
...
static int crush_reweight_tree_bucket(struct crush_map* map, struct crush_bucket_tree* bucket)
{
    ...
    for (i = 0; i < bucket->h.size; i++) {
        ...
        if (crush_addition_is_unsafe(bucket->h.weight, bucket->node_weights[node])) return -ERANGE;
...
{
    switch (b->alg) {
    case CRUSH_BUCKET_UNIFORM: return crush_reweight_uniform_bucket(map, (struct crush_bucket_uniform*)b);
    case CRUSH_BUCKET_LIST: return crush_reweight_list_bucket(map, (struct crush_bucket_list*)b);
    case CRUSH_BUCKET_TREE: return crush_reweight_tree_bucket(map, (struct crush_bucket_tree*)b);
    case CRUSH_BUCKET_STRAW: return crush_reweight_straw_bucket(map, (struct crush_bucket_straw*)b);
    case CRUSH_BUCKET_STRAW2: return crush_reweight_straw2_bucket(map, (struct crush_bucket_straw2*)b);
    default: return -1;
    }
}
...
void set_optimal_crush_map(struct crush_map* map)
{
    ...
    map->allowed_bucket_algs = ((1 << CRUSH_BUCKET_UNIFORM) | (1 << CRUSH_BUCKET_LIST) | (1 << CRUSH_BUCKET_STRAW) |
