...
const char* ceph_msgr_types[] = {"undef", "async+posix", "async+dpdk", "async+rdma"};
...
static void create_or_get_ceph_context(struct ceph_msgr_options* o)
{
    ...
    if (o->conffile) args = {"--conf", o->conffile};
...
static void ceph_msgr_receiver_on_request(struct ceph_msgr_data* data, MOSDOp* req)
{
    MOSDOpReply* rep;
    ...
    if (data->io_inflight_nr) {
        ...
        data->io_inflight_nr--;
        io = flist_first_entry(&data->io_inflight_list, struct ceph_msgr_io, list);
        flist_del(&io->list);
...
public:
    ...
    {
        switch (m->get_type()) {
        case CEPH_MSG_OSD_OP: return m_data->o->is_receiver;
...
{
    entity_name_t ename = o->is_receiver ? entity_name_t::OSD(0) : entity_name_t::CLIENT(0);
    std::string lname = o->is_receiver ? "receiver" : "sender";

    std::string ms_type = o->ms_type != CEPH_MSGR_TYPE_UNDEF ? ceph_msgr_types[o->ms_type]
    ...
    Messenger* msgr = Messenger::create(g_ceph_context, ms_type.c_str(), ename, lname, nonce);
    if (o->is_receiver) {
...
static void put_messenger(struct ceph_msgr_data* data)
{
    ...
    if (o->is_single) {
        if (--single_msgr_ref == 0) {
             ...
             */
            for (auto disp : single_msgr_disps) delete disp;
            single_msgr = NULL;
        }
    }
    else {
...
static void fio_ceph_msgr_cleanup(struct thread_data* td)
{
    ...
    if (nr) fprintf(stderr, "fio: io_completed_nr==%d, but should be zero\n", nr);
    if (data->io_inflight_nr) fprintf(stderr, "fio: io_inflight_nr==%d, but should be zero\n", data->io_inflight_nr);
    if (data->io_pending_nr) fprintf(stderr, "fio: io_pending_nr==%d, but should be zero\n", data->io_pending_nr);
    if (!flist_empty(&data->io_inflight_list)) fprintf(stderr, "fio: io_inflight_list is not empty\n");
...
static int fio_ceph_msgr_io_u_init(struct thread_data* td, struct io_u* io_u)
{
    ...
    if (!o->is_receiver) {
        ...
        ConnectionRef con = msgr->connect_to(dest.name.type(), entity_addrvec_t(dest.addr));
...
static void fio_ceph_msgr_io_u_free(struct thread_data* td, struct io_u* io_u)
{
    ...
    if (io) {
        ...
        if (io->req_msg) io->req_msg->put();
        free(io);
    }
}

static enum fio_q_status ceph_msgr_sender_queue(struct thread_data* td, struct io_u* io_u)
{
    struct ceph_msgr_data* data;
    struct ceph_msgr_io* io;

    bufferlist buflist = bufferlist::static_from_mem((char*)io_u->buf, io_u->buflen);
...
static int fio_ceph_msgr_getevents(struct thread_data* td, unsigned int min, unsigned int max,
                                   const struct timespec* ts)
{
    ...
    nr = ring_buffer_used_size(&data->io_completed_q);
    if (nr >= min) /* We got something */
        return min(nr, max);

    /* Here we are only if min != 0 and ts == NULL */
    assert(min && !ts);

    while ((nr = ring_buffer_used_size(&data->io_completed_q)) < min && !td->terminate) {
...
static enum fio_q_status ceph_msgr_receiver_queue(struct thread_data* td, struct io_u* io_u)
{
    struct ceph_msgr_data* data;
    struct ceph_msgr_io* io;
    ...
    if (data->io_pending_nr) {
        ...
        data->io_pending_nr--;
        rep_io = flist_first_entry(&data->io_pending_list, struct ceph_msgr_reply_io, list);
...
static enum fio_q_status fio_ceph_msgr_queue(struct thread_data* td, struct io_u* io_u)
{
    struct ceph_msgr_options* o = (decltype(o))td->eo;
...
template<class Func> fio_option make_option(Func&& func)
{
    auto o = fio_option{};
