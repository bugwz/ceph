...
struct librgw_data
{
    ...
    void save_handle(rgw_file_handle* fh) { fh_vec.push_back(fh); }
...
template<class F> fio_option make_option(F&& func)
{
...
struct save_args
{
    ...
    void push_arg(const std::string sarg) { argv[argc++] = strdup(sarg.c_str()); }
...
static int fio_librgw_setup(struct thread_data* td)
{
    ...
    if (!td->io_ops_data) {
        ...
        r = rgw_mount2(data->rgw_h, o.userid, o.access_key, o.secret_key, "/", &data->fs, RGW_MOUNT_FLAG_NONE);
        if (!!r) {
        ...
        r = rgw_lookup(data->fs, data->fs->root_fh, o.bucket_name, &data->bucket_fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
        if (!data->bucket_fh) {
            ...
            r = rgw_mkdir(
                data->fs, data->fs->root_fh, o.bucket_name, &st, create_mask, &data->bucket_fh, RGW_MKDIR_FLAG_NONE);
            if (!data->bucket_fh) {
 ...
 * numbers. Required.
 */
static int fio_librgw_getevents(struct thread_data* td, unsigned int min, unsigned int max, const struct timespec* t)
{
    return 0;
}
 ...
 */
static enum fio_q_status fio_librgw_queue(struct thread_data* td, struct io_u* io_u)
{
    librgw_data* data = static_cast<librgw_data*>(td->io_ops_data);
    ...
    if (io_u->ddir == DDIR_WRITE) {
        /* Do full write cycle */
        r = rgw_lookup(data->fs, data->bucket_fh, object, &object_fh, nullptr, 0, RGW_LOOKUP_FLAG_CREATE);
        if (!!r) {
        ...
        r = rgw_write(data->fs, object_fh, 0, io_u->xfer_buflen, &nbytes, (void*)io_u->xfer_buf, RGW_WRITE_FLAG_NONE);
        if (!!r) {
        ...
        r = rgw_lookup(data->fs, data->bucket_fh, object, &object_fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
        if (!!r) {
            dprint(FD_IO, "rgw_lookup failed to create filehandle for %s\n", object);
            goto out;
        }
        ...
        r = rgw_read(data->fs, object_fh, io_u->offset, io_u->xfer_buflen, &nbytes, io_u->xfer_buf, RGW_READ_FLAG_NONE);
        if (!!r) {
