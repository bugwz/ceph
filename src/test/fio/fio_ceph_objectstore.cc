...
struct Options
{
    ...
    unsigned long long cycle_throttle_period, oi_attr_len_low, oi_attr_len_high, snapset_attr_len_low,
        snapset_attr_len_high, pglog_omap_len_low, pglog_omap_len_high, pglog_dup_omap_len_low, pglog_dup_omap_len_high,
        _fastinfo_omap_len_low, _fastinfo_omap_len_high;
...
static std::vector<fio_option> ceph_options{
    ...
    make_option([](fio_option& o) {
        ...
        o.type = FIO_OPT_STR_STORE;
        o.help = "comma delimited list of throttle values", o.off1 = offsetof(Options, throttle_values);
        o.def = 0;
    }),
    make_option([](fio_option& o) {
        ...
        o.type = FIO_OPT_STR_STORE;
        o.help = "comma delimited list of throttle values", o.off1 = offsetof(Options, deferred_throttle_values);
        o.def = 0;
    }),
    make_option([](fio_option& o) {
...
int destroy_collections(std::unique_ptr<ObjectStore>& os, std::vector<Collection>& collections)
{
...
int init_collections(std::unique_ptr<ObjectStore>& os, uint64_t pool, std::vector<Collection>& collections,
                     uint64_t count)
{
    ceph_assert(count > 0);
        ...
        if (!ref_count) {
            ...
            if (perf_output_file) {
                try {
                    ...
                    std::cerr << "Unable to write formatted output to " << *perf_output_file
...
TracepointProvider::Traits bluestore_tracepoint_traits("libbluestore_tp.so", "bluestore_tracing");

Engine::Engine(thread_data* td)
    : ref_count(0)
    , unlink(td->o.unlink)
    , perf_output_file(static_cast<Options*>(td->eo)->perf_output_file
                           ...
                           : std::nullopt)
{
    ...
    std::vector<const char*> args{
        "-i",
        "0",   // identify as osd.0 for osd_data and osd_journal
        "--conf",
        o->conf,   // use the requested conf file
    };
    ...
    if (!os) throw std::runtime_error("bad objectstore type " + g_conf()->osd_objectstore);
    ...
    o->snapset_attr_len_high = max(o->snapset_attr_len_low, o->snapset_attr_len_high);
    o->pglog_omap_len_high = max(o->pglog_omap_len_low, o->pglog_omap_len_high);
    o->pglog_dup_omap_len_high = max(o->pglog_dup_omap_len_low, o->pglog_dup_omap_len_high);
    o->_fastinfo_omap_len_high = max(o->_fastinfo_omap_len_low, o->_fastinfo_omap_len_high);

    int r = os->mkfs();
    if (r < 0) throw std::system_error(-r, std::system_category(), "mkfs failed");

    r = os->mount();
    if (r < 0) throw std::system_error(-r, std::system_category(), "mount failed");
    ...
    if (o->single_pool_mode) {
        ...
        if (count > td->o.nr_files) count = td->o.nr_files;
...
Job::Job(Engine* engine, const thread_data* td)
    ...
    , events(td->o.iodepth)
    , unlink(td->o.unlink)
    , throttle_values(parse_throttle_str(static_cast<Options*>(td->eo)->throttle_values))
    , deferred_throttle_values(parse_throttle_str(static_cast<Options*>(td->eo)->deferred_throttle_values))
    , cycle_throttle_period(static_cast<Options*>(td->eo)->cycle_throttle_period)
{
    engine->ref();
    auto o = static_cast<Options*>(td->eo);
    unsigned long long max_data = max(o->oi_attr_len_high, o->snapset_attr_len_high);
    ...
    for (uint32_t i = 0; i < td->o.nr_files; i++) {
        ...
        if (o->check_files) {
            ...
            if (r || ((unsigned)st.st_size) != file_size) {
                derr << "Problem checking " << oid << ", r=" << r << ", st.st_size=" << st.st_size
                     << ", file_size=" << file_size << ", nr_files=" << td->o.nr_files << dendl;
                engine->deref();
                throw std::system_error(r, std::system_category(), "job init -- cannot check file");
            }
        }
...
{
    if (subjob_number != 0) return;

    std::lock_guard<std::mutex> l(throttle_lock);
    if (throttle_values.empty() && deferred_throttle_values.empty()) return;

    if (ceph::mono_clock::is_zero(last) || ((cycle_throttle_period != cycle_throttle_period.zero()) &&
        ...
        if (!throttle_values.empty()) {
            ...
            int r = engine->cct->_conf.set_val("bluestore_throttle_bytes", val, nullptr);
            ceph_assert(r == 0);
        }
        if (!deferred_throttle_values.empty()) {
            ...
            int r = engine->cct->_conf.set_val("bluestore_throttle_deferred_bytes", val, nullptr);
            ceph_assert(r == 0);
        }
...
int fio_ceph_os_getevents(thread_data* td, unsigned int min, unsigned int max, const timespec* t)
{
    auto job = static_cast<Job*>(td->io_ops_data);
    ...
    do {
        ...
        {
            if (!(u->flags & IO_U_F_FLIGHT)) continue;
...
enum fio_q_status fio_ceph_os_queue(thread_data* td, io_u* u)
{
    ...
    if (u->ddir == DDIR_WRITE) {
        ...
        bufferlist bl;
        bl.push_back(buffer::copy(reinterpret_cast<char*>(u->xfer_buf), u->xfer_buflen));
        ...
        if (o->oi_attr_len_high) {
            ...
            job->one_for_all_data.set_length(
                ceph::util::generate_random_number(o->oi_attr_len_low, o->oi_attr_len_high));
            attrset["_"] = job->one_for_all_data;
        }
        if (o->snapset_attr_len_high) {
            ceph_assert(o->snapset_attr_len_high >= o->snapset_attr_len_low);
            job->one_for_all_data.set_length(
                ceph::util::generate_random_number(o->snapset_attr_len_low, o->snapset_attr_len_high));
        ...
        if (o->_fastinfo_omap_len_high) {
            ...
            // fill with the garbage as we do not care of the actual content...
            job->one_for_all_data.set_length(
                ceph::util::generate_random_number(o->_fastinfo_omap_len_low, o->_fastinfo_omap_len_high));
        ...
        if (o->simulate_pglog) {
            ...
            uint64_t pglog_ver_cnt = 0;
            {
                ...
                if (o->pglog_omap_len_high && pglog_ver_cnt >= coll.pglog_ver_tail + g_conf()->osd_min_pg_log_entries +
                                                                   g_conf()->osd_pg_log_trim_min) {
                    pglog_trim_tail = coll.pglog_ver_tail;
                    coll.pglog_ver_tail = pglog_trim_head = pglog_trim_tail + g_conf()->osd_pg_log_trim_min;

                    if (o->pglog_dup_omap_len_high && pglog_ver_cnt >= coll.pglog_dup_ver_tail +
                                                                           g_conf()->osd_pg_log_dups_tracked +
                                                                           g_conf()->osd_pg_log_trim_min) {
            ...
            if (o->pglog_omap_len_high) {
                ...
                snprintf(ver_key, sizeof(ver_key), "0000000011.%020llu", (unsigned long long)pglog_ver_cnt);
                // fill with the garbage as we do not care of the actual content...
                job->one_for_all_data.set_length(
                    ceph::util::generate_random_number(o->pglog_omap_len_low, o->pglog_omap_len_high));
            ...
            if (o->pglog_dup_omap_len_high) {
                ...
                for (auto i = pglog_trim_tail; i < pglog_trim_head; ++i) {
                    snprintf(ver_key, sizeof(ver_key), "dup_0000000011.%020llu", (unsigned long long)i);
                    // fill with the garbage as we do not care of the actual content...
                    job->one_for_all_data.set_length(
                        ceph::util::generate_random_number(o->pglog_dup_omap_len_low, o->pglog_dup_omap_len_high));
                    omaps[ver_key].append(job->one_for_all_data);
                }
            }
        }
        ...
        set<string> rmkeys;
        for (auto i = pglog_trim_tail; i < pglog_trim_head; ++i) {
            snprintf(ver_key, sizeof(ver_key), "0000000011.%020llu", (unsigned long long)i);
        ...
        os->queue_transaction(coll.ch, std::move(t));
        return FIO_Q_QUEUED;
    }
...
int fio_ceph_os_open(thread_data* td, fio_file* f)
{
    return 0;
}
int fio_ceph_os_close(thread_data* td, fio_file* f)
{
    return 0;
}
