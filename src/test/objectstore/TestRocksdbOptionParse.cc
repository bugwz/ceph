...
TEST(RocksDBOption, interpret)
{
    ...
    for (vector<rocksdb::ThreadStatus>::iterator it = thread_list.begin(); it != thread_list.end(); ++it) {
        if (it->thread_type == rocksdb::ThreadStatus::HIGH_PRIORITY) num_high_pri_threads++;
        if (it->thread_type == rocksdb::ThreadStatus::LOW_PRIORITY) num_low_pri_threads++;
    }
