...
bool FileStoreDiff::diff_attrs(std::map<std::string, bufferptr>& b, std::map<std::string, bufferptr>& a)
{
    bool ret = false;
    ...
    for (; b_it != b.end(); ++b_it, ++a_it) {
        if (b_it->first != a_it->first) {
            cout << "diff_attrs name mismatch (verify: " << b_it->first << ", store: " << a_it->first << ")"
                 << std::endl;
            ret = true;
            continue;
        }
...
static bool diff_omap(std::map<std::string, bufferlist>& b, std::map<std::string, bufferlist>& a)
{
    bool ret = false;
    ...
    for (; b_it != b.end(); ++b_it, ++a_it) {
        if (a_it == a.end()) {
            cout << __func__ << " a reached end before b, a missing " << b_it->first << std::endl;
            ret = true;
            break;
        }
        if (b_it->first != a_it->first) {
...
bool FileStoreDiff::diff_objects_stat(struct stat& a, struct stat& b)
{
    ...
    if (a.st_uid != b.st_uid) {
        cout << "diff_objects_stat uid mismatch (A: " << a.st_uid << " != B: " << b.st_uid << ")" << std::endl;
        ret = true;
    }

    if (a.st_gid != b.st_gid) {
        cout << "diff_objects_stat gid mismatch (A: " << a.st_gid << " != B: " << b.st_gid << ")" << std::endl;
        ret = true;
    }

    if (a.st_mode != b.st_mode) {
        cout << "diff_objects_stat mode mismatch (A: " << a.st_mode << " != B: " << b.st_mode << ")" << std::endl;
        ret = true;
    }

    if (a.st_nlink != b.st_nlink) {
        cout << "diff_objects_stat nlink mismatch (A: " << a.st_nlink << " != B: " << b.st_nlink << ")" << std::endl;
        ret = true;
    }

    if (a.st_size != b.st_size) {
        cout << "diff_objects_stat size mismatch (A: " << a.st_size << " != B: " << b.st_size << ")" << std::endl;
        ret = true;
    }
    return ret;
}
...
bool FileStoreDiff::diff_objects(FileStore* a_store, FileStore* b_store, coll_t coll)
{
    ...
    if (err < 0) {
        cout << "diff_objects list on verify coll " << coll.to_str() << " returns " << err << std::endl;
        return true;
    }
    err = a_store->collection_list(coll, ghobject_t(), ghobject_t::get_max(), INT_MAX, &a_objects, NULL);
    if (err < 0) {
        cout << "diff_objects list on store coll " << coll.to_str() << " returns " << err << std::endl;
        return true;
    }

    if (b_objects.size() != a_objects.size()) {
        cout << "diff_objects " << coll << " num objs mismatch (A: " << a_objects.size() << ", B: " << b_objects.size()
             << ")" << std::endl;
        ret = true;
    ...
    for (; b_it != b_objects.end(); ++b_it, ++a_it) {
        ...
        if (b_obj.hobj.oid.name != a_obj.hobj.oid.name) {
            cout << "diff_objects name mismatch on A object " << coll << "/" << a_obj << " and B object " << coll << "/"
                 << b_obj << std::endl;
            ret = true;
            continue;
        }
        ...
        if (err < 0) {
            cout << "diff_objects error stating B object " << coll.to_str() << "/" << b_obj.hobj.oid.name << std::endl;
            ret = true;
        }
        err = a_store->stat(a_ch, a_obj, &a_stat);
        if (err < 0) {
            cout << "diff_objects error stating A object " << coll << "/" << a_obj << std::endl;
            ret = true;
        }

        if (diff_objects_stat(a_stat, b_stat)) {
            cout << "diff_objects stat mismatch on " << coll << "/" << b_obj << std::endl;
            ret = true;
        }
        ...
        if (!a_obj_bl.contents_equal(b_obj_bl)) {
            cout << "diff_objects content mismatch on " << coll << "/" << b_obj << std::endl;
            ret = true;
        }
        ...
        if (err < 0) {
            cout << "diff_objects getattrs on A object " << coll << "/" << a_obj << " returns " << err << std::endl;
            ret = true;
        }
        err = b_store->getattrs(b_ch, b_obj, b_obj_attrs_map);
        if (err < 0) {
            cout << "diff_objects getattrs on B object " << coll << "/" << b_obj << "returns " << err << std::endl;
            ret = true;
        }

        if (diff_attrs(b_obj_attrs_map, a_obj_attrs_map)) {
            cout << "diff_objects attrs mismatch on A object " << coll << "/" << a_obj << " and B object " << coll
                 << "/" << b_obj << std::endl;
            ret = true;
        }
        ...
        if (err < 0) {
            cout << "diff_objects getomap on A object " << coll << "/" << a_obj << " returns " << err << std::endl;
            ret = true;
        }
        ...
        if (err < 0) {
            cout << "diff_objects getomap on A object " << coll << "/" << b_obj << " returns " << err << std::endl;
            ret = true;
        }
        ...
        if (diff_omap(a_obj_omap, b_obj_omap)) {
            cout << "diff_objects omap mismatch on A object " << coll << "/" << a_obj << " and B object " << coll << "/"
                 << b_obj << std::endl;
...
bool FileStoreDiff::diff()
{
    ...
    for (; it != b_coll_list.end(); ++it) {
        ...
        for (std::vector<coll_t>::iterator j = a_coll_list.begin(); j != a_coll_list.end(); ++j) {
            if (*j == *it) {
                a_coll_list.erase(j);
                break;
            }
        ...
        if (diff_objects(a_store, b_store, b_coll)) ret = true;
    }
    for (std::vector<coll_t>::iterator it = a_coll_list.begin(); it != a_coll_list.end(); ++it) {
