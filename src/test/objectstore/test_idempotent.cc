...
{
    std::cerr << "Usage: " << name << " [new|continue] store_path store_journal db_path" << std::endl;
}
    ...
    while (1) {
        ...
        for (unsigned j = 0; j < 100; ++j) {
            ...
            if (val < 30) {
                ...
                t.clone(coll, *rand_choose(objects), *rand_choose(objects));
            }
            else if (val < 70) {
                t.remove(coll, *rand_choose(objects));
            }
            else {
                t.clone_range(coll, *rand_choose(objects), *rand_choose(objects));
            }
        }
