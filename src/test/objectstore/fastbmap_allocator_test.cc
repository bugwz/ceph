...
{
public:
    void init(uint64_t capacity, uint64_t alloc_unit) { _init(capacity, alloc_unit); }
    interval_t allocate_l1_cont(uint64_t length, uint64_t min_length, uint64_t pos_start, uint64_t pos_end)
    {
        return _allocate_l1_contiguous(length, min_length, 0, pos_start, pos_end);
    }
    void free_l1(const interval_t& r) { _free_l1(r.offset, r.length); }
};
...
public:
    void init(uint64_t capacity, uint64_t alloc_unit) { _init(capacity, alloc_unit); }
    void allocate_l2(uint64_t length, uint64_t min_length, uint64_t* allocated0, interval_vector_t* res)
    {
    ...
    void free_l2(const interval_vector_t& r) { _free_l2(r); }
    void mark_free(uint64_t o, uint64_t len) { _mark_free(o, len); }
    void mark_allocated(uint64_t o, uint64_t len) { _mark_allocated(o, len); }
};
...
TEST(TestAllocatorLevel01, test_l2)
{
    ...
    for (uint64_t i = 0; i < capacity; i += 0x1000) {
        ...
        if (0 == (i % (1 * 1024 * _1m))) {
            std::cout << "alloc1 " << i / 1024 / 1024 << " mb of " << capacity / 1024 / 1024 << std::endl;
        }
    }
#else
    for (uint64_t i = 0; i < capacity; i += _2m) {
    ...
    for (uint64_t i = 0; i < capacity; i += _1m) {
        ...
        al2.free_l2(r);
        if (0 == (i % (1 * 1024 * _1m))) {
            std::cout << "free1 " << i / 1024 / 1024 << " mb of " << capacity / 1024 / 1024 << std::endl;
        }
    }
    ...
    for (uint64_t i = 0; i < capacity; i += _1m) {
        ...
        if (0 == (i % (1 * 1024 * _1m))) {
            std::cout << "alloc2 " << i / 1024 / 1024 << " mb of " << capacity / 1024 / 1024 << std::endl;
        }
    }
    ASSERT_EQ(0u, al2.debug_get_free());
    uint64_t allocated4 = 0;
    ...
    for (uint64_t i = 0; i < capacity; i += 0x2000) {
        ...
        al2.free_l2(r);
        if (0 == (i % (1 * 1024 * _1m))) {
            std::cout << "free2 " << i / 1024 / 1024 << " mb of " << capacity / 1024 / 1024 << std::endl;
        }
    }
    ...
    for (uint64_t i = 0; i < capacity; i += 2 * _1m) {
        ...
        ASSERT_EQ(a4[0].length, 0x1000u);
        if (0 == (i % (1 * 1024 * _1m))) {
            std::cout << "alloc3 " << i / 1024 / 1024 << " mb of " << capacity / 1024 / 1024 << std::endl;
        }
    }
    ASSERT_EQ(0u, al2.debug_get_free());
...
TEST(TestAllocatorLevel01, test_l2_huge)
{
    ...
    for (uint64_t i = 0; i < capacity; i += _1m) {
        ...
        if (0 == (i % (1 * 1024 * _1m))) {
            std::cout << "allocH " << i / 1024 / 1024 << " mb of " << capacity / 1024 / 1024 << std::endl;
        }
    }
    for (uint64_t i = 0; i < capacity; i += _1m) {
        interval_vector_t a4;
        ...
        al2.free_l2(a4);
        if (0 == (i % (1 * 1024 * _1m))) {
            std::cout << "freeH1 " << i / 1024 / 1024 << " mb of " << capacity / 1024 / 1024 << std::endl;
        }
    }
    {
...
{
    {
        ...
        for (uint64_t i = 0; i < capacity; i += _1m / 2) {
            ...
            if (0 == (i % (1 * 1024 * _1m))) {
                std::cout << "allocU " << i / 1024 / 1024 << " mb of " << capacity / 1024 / 1024 << std::endl;
            }
        }
        ASSERT_EQ(0u, al2.debug_get_free());
        {
    ...
    }
    {
        ...
        for (uint64_t i = 0; i < capacity; i += _1m / 2) {
            ...
            ASSERT_EQ(a4[0].length, _1m / 2);
            if (0 == (i % (1 * 1024 * _1m))) {
                std::cout << "allocU2 " << i / 1024 / 1024 << " mb of " << capacity / 1024 / 1024 << std::endl;
            }
        }
        ASSERT_EQ(0u, al2.debug_get_free());
        {
...
TEST(TestAllocatorLevel01, test_claim_free_l2)
{
    ...
    ASSERT_EQ(max_available2 - max_available - 0x3e2000 - 0x19000, al2.get_available());
    ASSERT_EQ(max_available2 - max_available - 0x3e2000 - 0x19000, al2.debug_get_free());
    ...
    ASSERT_EQ(0x15000, al2.get_available());
    ASSERT_EQ(0x15000, al2.debug_get_free());
}
