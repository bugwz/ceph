...
int run_diff(std::string& a_path, std::string& a_journal, std::string& b_path, std::string& b_journal)
{
...
int run_sequence_to(int val, std::string& filestore_path, std::string& journal_path)
{
    num_txs = val;

    if (!is_seed_set) seed = (int)time(NULL);
...
int main(int argc, const char* argv[])
{
    ...
    for (std::vector<const char*>::iterator i = args.begin(); i != args.end();) {
        ...
        else if (ceph_argparse_witharg(args, i, &val, "--test-seed", (char*)NULL)) {
            seed = strtoll(val.c_str(), NULL, 10);
            is_seed_set = true;
        }
        else if (ceph_argparse_witharg(args, i, &val, "--test-num-colls", (char*)NULL)) {
            num_colls = strtoll(val.c_str(), NULL, 10);
        }
        else if (ceph_argparse_witharg(args, i, &val, "--test-num-objs", (char*)NULL)) {
            num_objs = strtoll(val.c_str(), NULL, 10);
        }
        else if (ceph_argparse_witharg(args, i, &val, "--test-status-file", (char*)NULL)) {
            status_file = val;
        }
