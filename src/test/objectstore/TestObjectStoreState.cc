...
TestObjectStoreState::coll_entry_t* TestObjectStoreState::coll_create(spg_t pgid, ObjectStore::CollectionHandle ch)
{
    char meta_buf[100];
...
TestObjectStoreState::coll_entry_t* TestObjectStoreState::get_coll_at(int pos, bool erase)
{
    ...
    if (m_collections.empty()) return NULL;
...
{
    if (m_objects.count(id)) return true;
    return false;
}
...
hobject_t* TestObjectStoreState::coll_entry_t::get_obj(int id, bool remove)
{
    ...
    hobject_t* obj = it->second;
    if (remove) m_objects.erase(it);

    dout(5) << "get_obj coll " << m_cid << " id " << id << ": " << obj->oid.name << "(removed: " << remove << ")"
            << dendl;
...
hobject_t* TestObjectStoreState::coll_entry_t::get_obj_at(int pos, bool remove, int* key)
{
    if (m_objects.empty()) {
    ...
    if (key != NULL) *key = it->first;

    if (remove) m_objects.erase(it);

    dout(5) << "get_obj_at coll id " << m_cid << " pos " << pos << ": " << ret->oid.name << "(removed: " << remove
            << ")" << dendl;

    return ret;
}

hobject_t* TestObjectStoreState::coll_entry_t::replace_obj(int id, hobject_t* obj)
{
