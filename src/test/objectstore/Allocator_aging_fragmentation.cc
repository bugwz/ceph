...
std::vector<Scenario> scenarios{
    Scenario{512, 65536, 0.8, 0.6, 0.1, 3},           Scenario{512, 65536, 0.9, 0.7, 0.0, 3},
    Scenario{512, 65536, 0.9, 0.7, 0.1, 3},           Scenario{512, 65536, 0.8, 0.6, 0.5, 3},
    Scenario{512, 65536, 0.9, 0.7, 0.5, 3},           Scenario{1024, 65536, 0.8, 0.6, 0.1, 3},
    Scenario{1024, 65536, 0.9, 0.7, 0.0, 3},          Scenario{1024, 65536, 0.9, 0.7, 0.1, 3},
    Scenario{1024 * 2, 65536, 0.8, 0.6, 0.3, 3},      Scenario{1024 * 2, 65536, 0.9, 0.7, 0.0, 3},
    Scenario{1024 * 2, 65536, 0.9, 0.7, 0.3, 3},      Scenario{512, 65536 / 16, 0.8, 0.6, 0.1, 3},
    Scenario{512, 65536 / 16, 0.9, 0.7, 0.0, 3},      Scenario{512, 65536 / 16, 0.9, 0.7, 0.1, 3},
    Scenario{512, 65536 / 16, 0.8, 0.6, 0.5, 3},      Scenario{512, 65536 / 16, 0.9, 0.7, 0.5, 3},
    Scenario{1024, 65536 / 16, 0.8, 0.6, 0.1, 3},     Scenario{1024, 65536 / 16, 0.9, 0.7, 0.0, 3},
    Scenario{1024, 65536 / 16, 0.9, 0.7, 0.1, 3},     Scenario{1024 * 2, 65536 / 16, 0.8, 0.6, 0.3, 3},
    Scenario{1024 * 2, 65536 / 16, 0.9, 0.7, 0.0, 3}, Scenario{1024 * 2, 65536 / 16, 0.9, 0.7, 0.3, 3}};
...
public:
    ...
    void init_close();
    void doAgingTest(std::function<uint32_t()> size_generator, const std::string& alloc_name, uint64_t capacity,
                     uint32_t alloc_unit, uint64_t high_mark, uint64_t low_mark, uint32_t iterations,
...
public:
    ...
    {
        assert(len != 0);
        if (size + 1 > allocations.size()) allocations.resize(size + 100);
        allocations[size++] = bluestore_pextent_t(offs, len);
        return true;
    }

    bool pop_random(gen_type& rng, uint64_t* offs, uint32_t* len, uint32_t max_len = 0)
    {
...
void AllocTest::init_alloc(const std::string& allocator_name, int64_t size, uint64_t min_alloc_size)
{
    ...
    rng.seed(0);
    alloc.reset(Allocator::create(cct.get(), allocator_name, size, min_alloc_size));
...
void AllocTest::do_free(uint64_t low_mark)
{
    while (level > low_mark) {
        if (free_random() == 0) break;
    }
}

void AllocTest::doAgingTest(std::function<uint32_t()> size_generator, const std::string& allocator_name,
                            uint64_t capacity, uint32_t alloc_unit, uint64_t high_mark, uint64_t low_mark,
                            uint32_t iterations, double leak_factor)
{
    ...
    for (uint32_t i = 0; i < iterations; i++) {
        ...
        if (verbose)
            std::cout << "    level change: " << double(level_previous) / capacity * 100 << "% -> "
                      << double(level) / capacity * 100 << "% time=" << (ceph_clock_now() - start) * 1000 << "ms"
                      << std::endl;
    ...
    if (verbose) std::cout << "free chunks sum=" << sum << " free chunks count=" << cnt << std::endl;
...
void AllocTest::SetUpTestSuite()
{
    ...
    cct = global_init(NULL, args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_UTILITY, CINIT_FLAG_NO_DEFAULT_CONFIG_FILE);
...
void AllocTest::TearDownTestSuite()
{
    ...
    for (auto& r : results_per_allocator) {
        std::cout << r.first << "    fragmented allocs=" << r.second.fragmented_percent / r.second.tests_cnt << "%"
                  << " #frags=" << r.second.fragments_count / r.second.tests_cnt
                  << " free_score=" << r.second.frag_score / r.second.tests_cnt << " time=" << r.second.time * 1000
                  << "ms" << std::endl;
    }
}
...
TEST_P(AllocTest, test_alloc_triangle_0_8M_16M)
{
    ...
    for (auto& s : scenarios) {
        ...
        auto size_generator = [&]() -> uint32_t { return (uint32_t(D(rng)) + s.alloc_unit) & ~(s.alloc_unit - 1); };

        doAgingTest(size_generator,
                    allocator_name,
                    s.capacity * _1G,
                    s.alloc_unit,
...
TEST_P(AllocTest, test_alloc_fragmentation_max_chunk_8M)
{
    ...
    for (auto& s : scenarios) {
        ...
        auto size_generator = [&]() -> uint32_t {
            uint32_t c;
            if (object_size == 0) object_size = (uint32_t(D(rng)) * s.alloc_unit);
...
INSTANTIATE_TEST_SUITE_P(Allocator, AllocTest, ::testing::Values("stupid", "bitmap", "avl", "btree"));
