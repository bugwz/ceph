...
void AllocTest::init_alloc(const std::string& allocator_name, int64_t size, uint64_t min_alloc_size)
{
    ...
    rng.seed(0);
    alloc.reset(Allocator::create(g_ceph_context, allocator_name, size, min_alloc_size));
...
INSTANTIATE_TEST_CASE_P(Allocator, AllocTest, ::testing::Values("stupid", "bitmap", "avl"));
