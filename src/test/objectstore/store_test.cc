...
{
    if (expected.contents_equal(actual)) return true;

    unsigned first = 0;
    if (expected.length() != actual.length()) {
        cout << "--- buffer lengths mismatch " << std::hex << "expected 0x" << expected.length() << " != actual 0x"
             << actual.length() << std::dec << std::endl;
        derr << "--- buffer lengths mismatch " << std::hex << "expected 0x" << expected.length() << " != actual 0x"
             << actual.length() << std::dec << dendl;
    }
    auto len = std::min(expected.length(), actual.length());
    while (first < len && expected[first] == actual[first]) ++first;
    unsigned last = len;
    while (last > 0 && expected[last - 1] == actual[last - 1]) --last;
    if (len > 0) {
        cout << "--- buffer mismatch between offset 0x" << std::hex << first << " and 0x" << last << ", total 0x" << len
             << std::dec << std::endl;
        derr << "--- buffer mismatch between offset 0x" << std::hex << first << " and 0x" << last << ", total 0x" << len
             << std::dec << dendl;
        cout << "--- expected:\n";
...
template<typename T> int queue_transaction(T& store, ObjectStore::CollectionHandle ch, ObjectStore::Transaction&& t)
{
    if (rand() % 2) {
...
template<typename T>
int collection_list(T& store, ObjectStore::CollectionHandle& c, const ghobject_t& start, const ghobject_t& end, int max,
                    vector<ghobject_t>* ls, ghobject_t* pnext, bool disable_legacy = false)
{
...
{
    ghobject_t start;
    for (vector<ghobject_t>::const_iterator i = in.begin(); i != in.end(); ++i) {
        if (start > *i) {
...
public:
    ...
    void doSyntheticTest(int num_ops, uint64_t max_obj, uint64_t max_wr, uint64_t align);
};
...
protected:
    void DeferredSetup() { StoreTest::SetUp(); }
...
public:
    typedef std::function<void(uint64_t num_ops, uint64_t max_obj, uint64_t max_wr, uint64_t align)> MatrixTest;
...
protected:
    ...
    void do_matrix_choose(const char* matrix[][10], int i, int pos, int num, MatrixTest fn)
    {
        if (matrix[i][0]) {
            int count;
            ...
            for (int j = 1; matrix[i][j]; ++j) {
                ...
                do_matrix_choose(matrix, i + 1, pos * count + j - 1, num * count, fn);
            }
        }
        else {
            cout << "---------------------- " << (pos + 1) << " / " << num << " ----------------------" << std::endl;
            for (unsigned k = 0; matrix[k][0]; ++k) {
                cout << "  " << matrix[k][0] << " = " << matrix_get(matrix[k][0]) << std::endl;
            }
...
{
protected:
    void StartDeferred() { DeferredSetup(); }
...
{
    if (string(GetParam()) != "bluestore") return;
    int r = store->umount();
    ASSERT_EQ(0, r);
    r = store->fsck(false);
    ASSERT_EQ(0, r);
...
TEST_P(StoreTest, FiemapHoles)
{
    ...
    auto ch = store->create_new_collection(cid);
    {
        ...
        t.create_collection(cid, 0);
        t.touch(cid, oid);
        for (uint64_t i = 0; i < MAX_EXTENTS; i++) t.write(cid, oid, SKIP_STEP * i, 3, bl);
        r = queue_transaction(store, ch, std::move(t));
        ASSERT_EQ(r, 0);
    }
    {
...
void StoreTest::doCompressionTest()
{
    ...
    for (size_t i = 0; i < data2.size(); i++) data2[i] = (i + 1) / 256;
    {
        ObjectStore::Transaction t;
        bufferlist bl, newdata;
        bl.append(data2);
    ...
    for (size_t i = 0; i < data2.size(); i++) data2[i] = (i + 2) / 256;
    {
        ObjectStore::Transaction t;
        bufferlist bl, newdata;
        bl.append(data2);
    ...
    data.resize(0x1001);
    for (size_t i = 0; i < data.size(); i++) data[i] = (i + 3) / 256;
    {
        ObjectStore::Transaction t;
        bufferlist bl, newdata;
        bl.append(data);
...
TEST_P(StoreTestSpecificAUSize, BluestoreStatFSTest)
{
    ...
    ghobject_t hoid(hobject_t(sobject_t("Object 1", CEPH_NOSNAP), string(), 0, poolid, string()));
    ...
    }
    {
        ...
        ASSERT_GT(statfs2_pool.data_compressed_original, statfs_pool.data_compressed_original);
        ASSERT_EQ(statfs2_pool.data_compressed_allocated, statfs_pool.data_compressed_allocated);
    }

    {
        ...
        ghobject_t hoid(hobject_t(sobject_t("Object 2", CEPH_NOSNAP), string(), 0, poolid2, string()));
...

    {
        ...
        ghobject_t hoid3(hobject_t(sobject_t("Object 3", CEPH_NOSNAP), string(), 0, poolid3, string()));
        ...
        auto ch3 = store->create_new_collection(cid3);
        {
            ...
            t = ObjectStore::Transaction();
            t.collection_move_rename(cid3, hoid3_temp, cid3, hoid3);
            r = queue_transaction(store, ch3, std::move(t));
            ASSERT_EQ(r, 0);
...
TEST_P(StoreTestSpecificAUSize, BluestoreFragmentedBlobTest)
{
    ...
    {
        ObjectStore::Transaction t;
        for (size_t i = 0; i < data.size(); i++) data[i] = i / 256 + 1;
        bufferlist bl, newdata;
        bl.append(data);
        t.write(cid, hoid, 0, bl.length(), bl);
...
TEST_P(StoreTest, SmallSkipFront)
{
    ...
    }
    {
        ...
        for (unsigned i = 0; i < 4096; ++i) ASSERT_EQ(0, bl[i]);
        for (unsigned i = 4096; i < 8192; ++i) ASSERT_EQ(1, bl[i]);
    }
    {
        ObjectStore::Transaction t;
        t.remove(cid, a);
...
TEST_P(StoreTest, AppendDeferredVsTailCache)
{
    ...
    bufferlist actual;
    {
        ASSERT_EQ((int) final.length(), store->read(ch, a, 0, final.length(), actual));
...
TEST_P(StoreTest, SimpleListTest)
{
    ...
    set<ghobject_t> all;
    {
        ...
        for (int i = 0; i < 200; ++i) {
            string name("object_");
            name += stringify(i);
            ghobject_t hoid(hobject_t(sobject_t(name, CEPH_NOSNAP)), ghobject_t::NO_GEN, shard_id_t(1));
            hoid.hobj.pool = 1;
            all.insert(hoid);
            t.touch(cid, hoid);
            cerr << "Creating object " << hoid << std::endl;
    ...
    }
    {
        ...
        while (!next.is_max()) {
            int r = collection_list(store, ch, current, ghobject_t::get_max(), 50, &objects, &next);
            ASSERT_EQ(r, 0);
            ASSERT_TRUE(sorted(objects));
            cout << " got " << objects.size() << " next " << next << std::endl;
            for (vector<ghobject_t>::iterator p = objects.begin(); p != objects.end(); ++p) {
                if (saw.count(*p)) {
    ...
    {
        ObjectStore::Transaction t;
        for (set<ghobject_t>::iterator p = all.begin(); p != all.end(); ++p) t.remove(cid, *p);
        t.remove_collection(cid);
        cerr << "Cleaning" << std::endl;
        r = queue_transaction(store, ch, std::move(t));
        ASSERT_EQ(r, 0);
...
TEST_P(StoreTest, ListEndTest)
{
    ...
    {
        ghobject_t end(hobject_t(sobject_t("object_100", CEPH_NOSNAP)), ghobject_t::NO_GEN, shard_id_t(1));
        end.hobj.pool = 1;
        vector<ghobject_t> objects;
        ghobject_t next;
...
TEST_P(StoreTest, List_0xfffffff_Hash_Test_in_meta)
{
    ...
    {
        ObjectStore::Transaction t;
        ghobject_t hoid(hobject_t(sobject_t("obj", CEPH_NOSNAP), "", UINT32_C(0xffffffff), -1, "nspace"));
        t.touch(cid, hoid);
        r = queue_transaction(store, ch, std::move(t));
        ASSERT_EQ(r, 0);
    }
    {
        vector<ghobject_t> objects;
        r = collection_list(store, ch, ghobject_t(), ghobject_t::get_max(), INT_MAX, &objects, nullptr, true);
        ASSERT_EQ(r, 0);
...
TEST_P(StoreTest, List_0xfffffff_Hash_Test_in_PG)
{
    ...
    {
        ObjectStore::Transaction t;
        ghobject_t hoid(hobject_t(sobject_t("obj", CEPH_NOSNAP), "", UINT32_C(0xffffffff), poolid, "nspace"));
        t.touch(cid, hoid);
        r = queue_transaction(store, ch, std::move(t));
        ASSERT_EQ(r, 0);
    }
...
TEST_P(StoreTest, SimpleCloneTest)
{
    ...
    }
    {
        ...
        ASSERT_EQ(r, 0);
        bufferlist rl;
        ASSERT_EQ((int) final.length(), store->read(ch, hoid, 0, final.length(), rl));
...
TEST_P(StoreTest, OmapSimple)
{
    ...
    ghobject_t hoid(hobject_t(sobject_t("omap_obj", CEPH_NOSNAP), "key", 123, -1, ""));
    bufferlist small;
    small.append("small");
...
TEST_P(StoreTest, SimpleObjectLongnameTest)
{
    ...
    ghobject_t hoid(hobject_t(sobject_t(
        "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaObjectaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
...
TEST_P(StoreTest, LongnameSplitTest)
{
    ...
    test_obj_2.generation = 0;
    {
        ...
        t.collection_move_rename(cid, test_obj, cid, test_obj_2);
        r = queue_transaction(store, ch, std::move(t));
        ASSERT_EQ(0, r);
    }
...
TEST_P(StoreTest, ManyObjectTest)
{
    ...
    for (set<ghobject_t>::iterator i = created.begin(); i != created.end(); ++i) {
        struct stat buf;
    ...
    vector<ghobject_t> objects;
    r = collection_list(store, ch, ghobject_t(), ghobject_t::get_max(), INT_MAX, &objects, 0);
    ASSERT_EQ(r, 0);
    ...
    objects.clear();
    r = collection_list(store, ch, ghobject_t::get_max(), ghobject_t::get_max(), 50, &objects, &next);
    ASSERT_EQ(r, 0);
    ...
    for (set<ghobject_t>::iterator i = listed.begin(); i != listed.end(); ++i) {
        ASSERT_TRUE(created.count(*i));
    }
...
public:
    unsigned seq;
    int64_t poolid;
    explicit MixedGenerator(int64_t p)
        : seq(0)
        , poolid(p)
    {}
    ghobject_t create_object(gen_type* gen) override
    {
        ...
        ++seq;
        return ghobject_t(hobject_t(name,
                                    string(),
                                    rand() & 2 ? CEPH_NOSNAP : rand(),
                                    (((seq / 1024) % 2) * 0xF00) + (seq & 0xFF),
                                    poolid,
                                    ""));
    }
};
...
public:
    ...
    {
        const char* msg;
        explicit EnterExit(const char* m)
            : msg(m)
        {
    ...
    public:
        ...
        C_SyntheticOnStash(SyntheticWorkloadState* state, ghobject_t oid, ghobject_t noid)
            : state(state)
            , oid(oid)
            , noid(noid)
        {}
        ...
        void finish(int r) override
        {
            ...
            if (state->contents.count(noid)) state->available_objects.insert(noid);
            --(state->in_flight);
            bufferlist r2;
    ...
    public:
        ...
        C_SyntheticOnClone(SyntheticWorkloadState* state, ghobject_t oid, ghobject_t noid)
            : state(state)
            , oid(oid)
            , noid(noid)
        {}
        ...
        void finish(int r) override
        {
            ...
            ASSERT_EQ(r, 0);
            state->in_flight_objects.erase(oid);
            if (state->contents.count(oid)) state->available_objects.insert(oid);
            if (state->contents.count(noid)) state->available_objects.insert(noid);
            --(state->in_flight);
            bufferlist r2;
            r = state->store->read(state->ch, noid, 0, state->contents[noid].data.length(), r2);
    ...
    SyntheticWorkloadState(ObjectStore* store, ObjectGenerator* gen, gen_type* rng, coll_t cid, unsigned max_size,
                           unsigned max_write, unsigned alignment)
        : cid(cid)
        , write_alignment(alignment)
        , max_object_len(max_size)
        , max_write_len(max_write)
        , in_flight(0)
    ...
    void statfs(store_statfs_t& stat) { store->statfs(&stat); }

    ghobject_t get_uniform_random_object(std::unique_lock<ceph::mutex>& locker)
    {
        cond.wait(locker, [this] { return in_flight < max_in_flight && !available_objects.empty(); });
    ...
    bool can_create() { return (available_objects.size() + in_flight_objects.size()) < max_objects; }

    bool can_unlink() { return (available_objects.size() + in_flight_objects.size()) > 0; }
    ...
    unsigned get_random_alloc_hints()
    {
        unsigned f = 0;
        {
            ...
            switch (u(*rng)) {
            case 1: f |= CEPH_OSD_ALLOC_HINT_FLAG_SEQUENTIAL_WRITE; break;
            case 2: f |= CEPH_OSD_ALLOC_HINT_FLAG_RANDOM_WRITE; break;
            }
        }
        {
            boost::uniform_int<> u(0, 3);
            switch (u(*rng)) {
            case 1: f |= CEPH_OSD_ALLOC_HINT_FLAG_SEQUENTIAL_READ; break;
            case 2: f |= CEPH_OSD_ALLOC_HINT_FLAG_RANDOM_READ; break;
            }
        }
        {
        ...
        }
        {
            boost::uniform_int<> u(0, 3);
            switch (u(*rng)) {
            case 1: f |= CEPH_OSD_ALLOC_HINT_FLAG_SHORTLIVED; break;
            case 2: f |= CEPH_OSD_ALLOC_HINT_FLAG_LONGLIVED; break;
            }
        }
        {
            boost::uniform_int<> u(0, 3);
            switch (u(*rng)) {
            case 1: f |= CEPH_OSD_ALLOC_HINT_FLAG_COMPRESSIBLE; break;
            case 2: f |= CEPH_OSD_ALLOC_HINT_FLAG_INCOMPRESSIBLE; break;
            }
        }
        return f;
    }
    ...
    int touch()
    {
        ...
        t.set_alloc_hint(cid, new_obj, 1ull << u(*rng), 1ull << v(*rng), get_random_alloc_hints());
        ++in_flight;
        in_flight_objects.insert(new_obj);
        if (!contents.count(new_obj)) contents[new_obj] = Object();
    ...
    int clone_range()
    {
        ...
        if (0)
            cout << __func__ << " from " << srcoff << "~" << len << " (size " << srcdata.length() << ") to " << dstoff
                 << "~" << len << std::endl;
        ...
        if (dstdata.length() <= dstoff) {
            ...
            if (value.length() < dstdata.length())
                dstdata.cbegin(value.length()).copy(dstdata.length() - value.length(), value);
            value.swap(dstdata);
        }
    ...
    int write()
    {
        ...
        if (data.length() <= offset) {
            ...
            value.append(bl);
            if (value.length() < data.length()) data.cbegin(value.length()).copy(data.length() - value.length(), value);
            value.swap(data);
        }
    ...
    int zero()
    {
        ...
        if (len > 0) {
            ...
            n.append_zero(len);
            if (data.length() > offset + len) data.cbegin(offset + len).copy(data.length() - offset - len, n);
            data.swap(n);
        }
    ...
    void read()
    {
        ...
        if (0)
            cout << " obj " << obj << " size " << expected.length() << " offset " << offset << " len " << len
                 << std::endl;
    ...
    int setattrs()
    {
        ...
        set<string> keys;
        for (map<string, bufferlist>::iterator it = contents[obj].attrs.begin(); it != contents[obj].attrs.end(); ++it)
    ...
    void getattrs()
    {
        ...
        for (map<string, bufferlist>::iterator it = expected.begin(); it != expected.end(); ++it) {
    ...
    void scan()
    {
        ...
        while (1) {
            ...
            int r = collection_list(store, ch, current, ghobject_t::get_max(), 100, &objects, &next);
            ASSERT_EQ(r, 0);
            ASSERT_TRUE(sorted(objects));
        ...
        if (objects_set.size() != available_objects.size()) {
            for (set<ghobject_t>::iterator p = objects_set.begin(); p != objects_set.end(); ++p)
                if (available_objects.count(*p) == 0) {
                    cerr << "+ " << *p << std::endl;
                    ceph_abort();
                }
            for (set<ghobject_t>::iterator p = available_objects.begin(); p != available_objects.end(); ++p)
                if (objects_set.count(*p) == 0) cerr << "- " << *p << std::endl;
        ...
        for (set<ghobject_t>::iterator i = objects_set.begin(); i != objects_set.end(); ++i) {
            ASSERT_GT(available_objects.count(*i), (unsigned)0);
        }

        int r = collection_list(store, ch, ghobject_t(), ghobject_t::get_max(), INT_MAX, &objects, 0);
        ASSERT_EQ(r, 0);
        objects_set2.insert(objects.begin(), objects.end());
        ASSERT_EQ(objects_set2.size(), available_objects.size());
        for (set<ghobject_t>::iterator i = objects_set2.begin(); i != objects_set2.end(); ++i) {
            ASSERT_GT(available_objects.count(*i), (unsigned)0);
    ...
    {
        std::lock_guard locker{lock};
        cerr << "available_objects: " << available_objects.size() << " in_flight_objects: " << in_flight_objects.size()
             << " total objects: " << in_flight_objects.size() + available_objects.size() << " in_flight " << in_flight
             << std::endl;
    }
};


void StoreTest::doSyntheticTest(int num_ops, uint64_t max_obj, uint64_t max_wr, uint64_t align)
{
    ...
    SyntheticWorkloadState test_obj(store.get(), &gen, &rng, cid, max_obj, max_wr, align);
    test_obj.init();
...
TEST_P(StoreTestSpecificAUSize, SyntheticMatrixCsumAlgorithm)
{
    ...
    const char* m[][10] = {
        ...
        {"max_size", "1048576", 0},
        {"alignment", "16", 0},
        {"bluestore_csum_type", "crc32c", "crc32c_16", "crc32c_8", "xxhash32", "xxhash64", "none", 0},
...
TEST_P(StoreTest, HashCollisionTest)
{
    ...
    while (1) {
        r = collection_list(store, ch, current, ghobject_t::get_max(), 60, &objects, &next);
        ASSERT_EQ(r, 0);
        ASSERT_TRUE(sorted(objects));
        for (vector<ghobject_t>::iterator i = objects.begin(); i != objects.end(); ++i) {
            if (listed.count(*i)) cerr << *i << " repeated" << std::endl;
            listed.insert(*i);
        }
        if (objects.size() < 50) {
            ASSERT_TRUE(next.is_max());
...
TEST_P(StoreTest, HashCollisionSorting)
{
    ...
    std::map<uint32_t, std::vector<std::string>> object_names = {
        {121664318, {{buf121664318_1}, {buf121664318_2}, {buf121664318_3}, {buf121664318_4}, {buf121664318_5}}},
        {121666222, {{buf121666222_1}, {buf121666222_2}, {buf121666222_3}, {buf121666222_4}, {buf121666222_5}}}};
    ...
    for (auto& [hash, names] : object_names) {
        for (auto& name : names) {
            ghobject_t hoid(hobject_t(sobject_t(name, CEPH_NOSNAP), string(), hash, poolid, string()));
    ...
    vector<ghobject_t> objects;
    int r = collection_list(store, ch, ghobject_t(), ghobject_t::get_max(), INT_MAX, &objects, 0, disable_legacy);
    ASSERT_EQ(r, 0);
    ...
    for (auto i = created.begin(); i != created.end(); i++) {
        ...
        for (j++; j != created.end(); j++) {
            ...
            ghobject_t next;
            r = collection_list(
                store, ch, *i, ghobject_t::get_max(), created_sub.size(), &objects, &next, disable_legacy);
            ASSERT_EQ(r, 0);
    ...
    for (auto i = created.begin(); i != created.end(); i++) {
        ...
        for (j++; j != created.end(); j++) {
            ...
            objects.clear();
            ghobject_t next;
            r = collection_list(store, ch, *i, *j, INT_MAX, &objects, &next, disable_legacy);
            ASSERT_EQ(r, 0);
            ASSERT_EQ(created_sub.size(), objects.size());
            it = objects.begin();
            for (auto& hoid : created_sub) {
...
TEST_P(StoreTest, ScrubTest)
{
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        ghobject_t hoid(
            hobject_t(string(buf) + base, string(), CEPH_NOSNAP, i, poolid, ""), ghobject_t::NO_GEN, shard_id_t(1));
        {
            ObjectStore::Transaction t;
            t.touch(cid, hoid);
            r = queue_transaction(store, ch, std::move(t));
    ...
    {
        ghobject_t hoid1(
            hobject_t("same-object", string(), CEPH_NOSNAP, 0, poolid, ""), ghobject_t::NO_GEN, shard_id_t(1));
...
TEST_P(StoreTest, OMapTest)
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        ASSERT_EQ(r, 0);
        for (map<string, bufferlist>::iterator j = attrs.begin(); j != attrs.end(); ++j) {
            bool correct =
                cur_attrs.count(j->first) && string(cur_attrs[j->first].c_str()) == string(j->second.c_str());
            if (!correct) {
        ...
        for (iter->seek_to_first(), count = 0; iter->valid(); iter->next(), count++) {
            ...
            if (!correct) {
                if (attrs.count(key) > 0) {
                    std::cout << "key " << key << "in omap , " << value.c_str() << " : " << attrs[key].c_str()
                              << std::endl;
                }
                else
...
void colsplittest(ObjectStore* store, unsigned num_objects, unsigned common_suffix_size, bool clones)
{
    ...
    small.append("small");
    {
        ...
        for (uint32_t i = 0; i < (2 - (int)clones) * num_objects; ++i) {
            ...
            objname << "obj" << i;
            ghobject_t a(hobject_t(objname.str(), "", CEPH_NOSNAP, i << common_suffix_size, 52, ""));
            t.write(cid, a, 0, small.length(), small, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
            if (clones) {
                objname << "-clone";
                ghobject_t b(hobject_t(objname.str(), "", CEPH_NOSNAP, i << common_suffix_size, 52, ""));
                t.clone(cid, a, b);
            }
            if (i % 100) {
                r = queue_transaction(store, ch, std::move(t));
    ...
    objects.clear();
    r = collection_list(store, tch, ghobject_t(), ghobject_t::get_max(), INT_MAX, &objects, 0);
    ASSERT_EQ(r, 0);
    ASSERT_EQ(objects.size(), num_objects);
    for (vector<ghobject_t>::iterator i = objects.begin(); i != objects.end(); ++i) {
        ASSERT_EQ(!(i->hobj.get_hash() & (1 << common_suffix_size)), 0u);
...
void test_merge_skewed(ObjectStore* store, unsigned base, unsigned bits, unsigned anum, unsigned bnum)
{
    cout << __func__ << " 0x" << std::hex << base << std::dec << " bits " << bits << " anum " << anum << " bnum "
         << bnum << std::endl;
    /*
    ...
    string suffix =
        ...
        "ooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooaaaaaaaaaa";
    ...
    // verify
    {
        ...
        collection_list(store, cha, ghobject_t(), ghobject_t::get_max(), INT_MAX, &got, 0);
...
{
    if (string(GetParam()) != "filestore") return;
...
INSTANTIATE_TEST_SUITE_P(ObjectStore, StoreTest,
...
INSTANTIATE_TEST_SUITE_P(ObjectStore, StoreTestSpecificAUSize,
                         ::testing::Values("memstore",
#if defined(WITH_BLUESTORE)
                                           "bluestore",
#endif
...
// Note: instantiate all stores to preserve store numbering order only
INSTANTIATE_TEST_SUITE_P(ObjectStore, StoreTestOmapUpgrade,
                         ::testing::Values("memstore",
#if defined(WITH_BLUESTORE)
                                           "bluestore",
#endif
...
#if defined(WITH_BLUESTORE)
INSTANTIATE_TEST_SUITE_P(ObjectStore, StoreTestDeferredSetup, ::testing::Values("bluestore"));
#endif
...
{
    *os << t.bdev_block_size << "/" << t.min_alloc_size << "/" << t.max_blob_size << "/" << t.prefer_deferred_size;
}

class DeferredWriteTest : public StoreTestFixture, public ::testing::WithParamInterface<deferred_test_t>
{
public:
    DeferredWriteTest()
...
protected:
    void DeferredSetup() { StoreTestFixture::SetUp(); }
    ...
    }
    {
        for (auto offset : offsets) {
            for (auto length : lengths) {
                ...
                if (print) std::cout << hname << std::endl;
                ...
                auto i_deferred_w = logger->get(l_bluestore_issued_deferred_writes);
                {
                    ...
                    t.write(cid, hoid, offset, bl.length(), bl, CEPH_OSD_OP_FLAG_FADVISE_NOCACHE);
                    r = queue_transaction(store, ch, std::move(t));
                    ASSERT_EQ(r, 0);
                }
...
#if defined(WITH_BLUESTORE)
INSTANTIATE_TEST_SUITE_P(BlueStore, DeferredWriteTest,
                         ::testing::Values(
...
void doMany4KWritesTest(ObjectStore* store, unsigned max_objects, unsigned max_ops, unsigned max_object_size,
                        unsigned max_write_size, unsigned write_alignment)
{
    MixedGenerator gen(555);
    gen_type rng(time(NULL));
    coll_t cid(spg_t(pg_t(0, 555), shard_id_t::NO_SHARD));
    ...
    SyntheticWorkloadState test_obj(store, &gen, &rng, cid, max_object_size, max_write_size, write_alignment);
    test_obj.init();
    ...
    if (!(res_stat.data_stored <= max_object_size) || !(res_stat.allocated <= max_object_size)) {
...
TEST_P(StoreTestSpecificAUSize, Many4KWritesTest)
{
    if (string(GetParam()) != "bluestore") return;
    if (smr) {
        cout << "SKIP: no deferred; assertions around res_stat.allocated don't apply" << std::endl;
        return;
    }
...
void get_mempool_stats(uint64_t* total_bytes, uint64_t* total_items)
{
    ...
    cout << "meta(" << meta_allocated << "/" << meta_items << ") onode(" << onode_allocated << "/" << onode_items
         << ") other(" << other_allocated << "/" << other_items << ")" << std::endl;
...
TEST_P(StoreTestSpecificAUSize, DeferredOnBigOverwrite)
{
    ...
    }
    {
        ...
        t.set_alloc_hint(cid, hoid2, block_size * 4, block_size * 4, CEPH_OSD_ALLOC_HINT_FLAG_SEQUENTIAL_READ);
...
TEST_P(StoreTestSpecificAUSize, DeferredDifferentChunks)
{
    ...
    SetVal(g_conf(), "bluestore_max_blob_size", "131072");
    SetVal(g_conf(), "bluestore_prefer_deferred_size", stringify(prefer_deferred_size).c_str());
    g_conf().apply_changes(nullptr);
    ...
    for (size_t expected_write_size = 1024; expected_write_size <= prefer_deferred_size; expected_write_size *= 2) {
        ...
        // check whether write will properly use deferred
        {
            ...
            t.write(
                cid, hoid, large_object_size - 2 * alloc_size - 1, bl.length(), bl, CEPH_OSD_OP_FLAG_FADVISE_NOCACHE);
            r = queue_transaction(store, ch, std::move(t));
            ++exp_bluestore_write_big;
            if (expected_write_size < prefer_deferred_size) ++exp_bluestore_write_big_deferred;
            ASSERT_EQ(r, 0);
        }
        ASSERT_EQ(logger->get(l_bluestore_write_big), exp_bluestore_write_big);
        ASSERT_EQ(logger->get(l_bluestore_write_big_deferred), exp_bluestore_write_big_deferred);
...
TEST_P(StoreTestSpecificAUSize, BlobReuseOnOverwriteReverse)
{
    ...
    }
    {
        ...
        bl.append(std::string(block_size * 2, 'a'));
        t.write(cid, hoid, block_size * 10, bl.length(), bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
        r = queue_transaction(store, ch, std::move(t));
        ASSERT_EQ(r, 0);
    }
    {
        ...
        bl.append(std::string(block_size, 'b'));
        t.write(cid, hoid, block_size * 9, bl.length(), bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
        r = queue_transaction(store, ch, std::move(t));
        ASSERT_EQ(r, 0);
    }
    {
...

    {
        ...
        t.write(cid, hoid, block_size * 7, bl.length(), bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
        r = queue_transaction(store, ch, std::move(t));
        ASSERT_EQ(r, 0);
    }
    {
...

    {
        ...
        t.write(cid, hoid, block_size * 13, bl.length(), bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
        r = queue_transaction(store, ch, std::move(t));
        ASSERT_EQ(r, 0);
    }
    {
...

    {
        ...
        bl.append(std::string(block_size, 'e'));
        t.write(cid, hoid, block_size * 17, bl.length(), bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
    ...
    }
    {
        ...
        bl.append(std::string(block_size, 'f'));
        t.write(cid, hoid, block_size * 16, bl.length(), bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
...
TEST_P(StoreTestSpecificAUSize, BlobReuseOnSmallOverwrite)
{
    ...
    }
    {
        ...
        t.write(cid, hoid, block_size + 1, bl.length(), bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
        r = queue_transaction(store, ch, std::move(t));
        ASSERT_EQ(r, 0);
    }
    {
...
TEST_P(StoreTestSpecificAUSize, ReproBug56488Test)
{
    ...
    }
    {
        ...
        ASSERT_EQ(logger->get(l_bluestore_issued_deferred_write_bytes), issued_dw_bytes + write_size);
    }
    {
        ...
        auto issued_dw_bytes = logger->get(l_bluestore_issued_deferred_write_bytes);
        {
            ...
            t.write(cid, hoid, alloc_size - write_size, bl.length(), bl, CEPH_OSD_OP_FLAG_FADVISE_NOCACHE);
            r = queue_transaction(store, ch, std::move(t));
            ASSERT_EQ(r, 0);
        }
        ASSERT_EQ(logger->get(l_bluestore_issued_deferred_writes), issued_dw + 2);
        ASSERT_EQ(logger->get(l_bluestore_issued_deferred_write_bytes), issued_dw_bytes + write_size * 2);
    }
    {
        ObjectStore::Transaction t;
        ghobject_t hoid(hobject_t("test", "", CEPH_NOSNAP, 0, -1, ""));
...
TEST_P(StoreTestSpecificAUSize, BluestoreRepairTest)
{
    ...
    SetVal(g_conf(), "bluestore_fsck_on_mount", "false");
    SetVal(g_conf(), "bluestore_fsck_on_umount", "false");
    SetVal(g_conf(), "bluestore_max_blob_size", stringify(2 * offs_base).c_str());
    ...
    bstore->inject_broken_shared_blob_key("undec1", bufferlist());
    bstore->inject_broken_shared_blob_key("undecodable key 2", bufferlist());
    bstore->inject_broken_shared_blob_key("undecodable key 3", bufferlist());
    bstore->umount();
...
TEST_P(StoreTestSpecificAUSize, BluestoreRepairSharedBlobTest)
{
    ...
    // o1 -> 0x2000~1K
    // o2 -> 0x4000~1k
    cerr << "introduce 2 non-overlapped extents in a shared blob" << std::endl;
    {
        ObjectStore::Transaction t;
        t.write(cid, hoid, 0, bl.length(), bl);
...
TEST_P(StoreTestSpecificAUSize, BluestoreBrokenNoSharedBlobRepairTest)
{
...

    {
        ...
        size_t expected_error_count =
            has_null_manager ? 4 :   // 4 sb ref mismatch errors [+ 1 optional statfs, hence ASSERT_GE]
...
{
    const auto& ibp = static_cast<const ceph::buffer_instrumentation::instrumented_bptr&>(bp);
...
TEST_P(StoreTestDeferredSetup, DISABLED_BluestoreHugeReads)
{
    ...
    {
        SetVal(g_conf(), "bdev_read_preallocated_huge_buffers", fmt::format("{}=2", HUGE_BUFFER_SIZE).c_str());
        SetVal(g_conf(), "bluestore_max_blob_size", std::to_string(HUGE_BUFFER_SIZE).c_str());
...
void doManySetAttr(ObjectStore* store, std::function<void(ObjectStore*)> do_check_fn)
{
    MixedGenerator gen(447);
    gen_type rng(time(NULL));
    coll_t cid(spg_t(pg_t(0, 447), shard_id_t::NO_SHARD));
    ...
    auto r = admin_socket->execute_command({"{\"prefix\": \"bluefs stats\"}"}, in, err, &out);
    if (r != 0) {
...
TEST_P(StoreTestSpecificAUSize, SpilloverTest)
{
    ...
    doManySetAttr(store.get(), [&](ObjectStore* _store) {
        ...
        std::cout << "spillover_alert:" << alerts.find("BLUEFS_SPILLOVER")->second << std::endl;
    });
}
...
TEST_P(StoreTestSpecificAUSize, SpilloverFixed2Test)
{
    ...
    doManySetAttr(store.get(), [&](ObjectStore* _store) {
        ...
        bstore->compact();
        const PerfCounters* logger = bstore->get_bluefs_perf_counters();
        ASSERT_LE(logger->get(l_bluefs_slow_used_bytes),
...
int main(int argc, char** argv)
{
    ...
    auto cct =
        global_init(NULL, args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_UTILITY, CINIT_FLAG_NO_DEFAULT_CONFIG_FILE);
    ...
    g_ceph_context->_conf.set_val_or_die("bluestore_debug_inject_allocation_from_file_failure", "0.66");
    ...
    g_ceph_context->_conf.set_val_or_die("bluestore_debug_randomize_serial_transaction", "10");

    g_ceph_context->_conf.set_val_or_die("bdev_debug_aio", "true");

    // specify device size
    g_ceph_context->_conf.set_val_or_die("bluestore_block_size", stringify(DEF_STORE_TEST_BLOCKDEV_SIZE));

    g_ceph_context->_conf.set_val_or_die("enable_experimental_unrecoverable_data_corrupting_features", "*");
    g_ceph_context->_conf.apply_changes(nullptr);
