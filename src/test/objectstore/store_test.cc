...
INSTANTIATE_TEST_SUITE_P(ObjectStore, StoreTest,
                         ::testing::Values("memstore", "filestore",
#if defined(WITH_BLUESTORE)
                                           "bluestore",
#endif
                                           "kstore"));
...
void doMany4KWritesTest(boost::scoped_ptr<ObjectStore>& store, unsigned max_objects, unsigned max_ops,
                        unsigned max_object_size, unsigned max_write_size, unsigned write_alignment)
{
    MixedGenerator gen(555);
    gen_type rng(time(NULL));
    coll_t cid(spg_t(pg_t(0, 555), shard_id_t::NO_SHARD));
    ...
    SyntheticWorkloadState test_obj(store.get(), &gen, &rng, cid, max_object_size, max_write_size, write_alignment);
    test_obj.init();
...
TEST_P(StoreTestSpecificAUSize, ReproBug56488Test)
{
    ...
    }
    {
        ...
        ASSERT_EQ(logger->get(l_bluestore_write_deferred_bytes), issued_dw_bytes + write_size);
    }
    {
        ...
        ASSERT_EQ(logger->get(l_bluestore_write_deferred_bytes), issued_dw_bytes + write_size * 2);
    }
    {
        ObjectStore::Transaction t;
        ghobject_t hoid(hobject_t("test", "", CEPH_NOSNAP, 0, -1, ""));
...
TEST_P(StoreTestSpecificAUSize, BluestoreBrokenNoSharedBlobRepairTest)
{
...

    {
        ...
        // value
        size_t expected_error_count = 7;   // 4 sb ref mismatch errors + 1 statfs + 1 block leak + 1 non-free
