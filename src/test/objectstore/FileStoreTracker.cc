...
public:
    OnApplied(FileStoreTracker* tracker, list<pair<pair<coll_t, string>, uint64_t> > in_flight)
        : tracker(tracker)
        , in_flight(in_flight)
    {}
    ...
    {
        for (list<pair<pair<coll_t, string>, uint64_t> >::iterator i = in_flight.begin(); i != in_flight.end(); ++i) {
...
public:
    OnCommitted(FileStoreTracker* tracker, list<pair<pair<coll_t, string>, uint64_t> > in_flight)
        : tracker(tracker)
        , in_flight(in_flight)
    {}
...
void FileStoreTracker::submit_transaction(Transaction& t)
{
    ...
    for (list<Transaction::Op*>::iterator i = t.ops.begin(); i != t.ops.end(); ++i) {
        (**i)(this, &out);
    }
...
void FileStoreTracker::write(const pair<coll_t, string>& obj, OutTransaction* out)
{
    ...
    iter.seek_to(offset);
    for (uint64_t i = offset; i < offset + len; ++i, ++iter) {
        ceph_assert(iter.valid());
        to_write.append(*iter);
    }
    out->t->write(coll_t(obj.first), ghobject_t(hobject_t(sobject_t(obj.second, CEPH_NOSNAP))), offset, len, to_write);
    out->in_flight->push_back(make_pair(obj, set_content(obj, contents)));
}

void FileStoreTracker::remove(const pair<coll_t, string>& obj, OutTransaction* out)
{
    std::cerr << "Deleting " << obj << std::endl;
    std::lock_guard l{lock};
    ObjectContents old_contents = get_current_content(obj);
    if (!old_contents.exists()) return;
    out->t->remove(coll_t(obj.first), ghobject_t(hobject_t(sobject_t(obj.second, CEPH_NOSNAP))));
    ObjectContents contents;
    out->in_flight->push_back(make_pair(obj, set_content(obj, contents)));
}

void FileStoreTracker::clone_range(const pair<coll_t, string>& from, const pair<coll_t, string>& to,
                                   OutTransaction* out)
{
    std::lock_guard l{lock};
...
void FileStoreTracker::clone(const pair<coll_t, string>& from, const pair<coll_t, string>& to, OutTransaction* out)
{
    std::lock_guard l{lock};
    ...
    if (to_contents.exists())
        out->t->remove(coll_t(to.first), ghobject_t(hobject_t(sobject_t(to.second, CEPH_NOSNAP))));
...
struct ObjStatus
{
    ...
    uint64_t trim_to() const { return last_applied < last_committed ? last_applied : last_committed; }
};
...
ObjStatus get_obj_status(const pair<coll_t, string>& obj, KeyValueDB* db)
{
    set<string> to_get;
    to_get.insert("META");
    map<string, bufferlist> got;
...
void set_obj_status(const pair<coll_t, string>& obj, const ObjStatus& status, KeyValueDB::Transaction t)
{
...
void _clean_forward(const pair<coll_t, string>& obj, uint64_t last_valid, KeyValueDB* db)
{
    KeyValueDB::Transaction t = db->get_transaction();
...
void FileStoreTracker::verify(const coll_t& coll, const string& obj, bool on_start)
{
    std::lock_guard l{lock};
    ...
    int r = store->read(ch, ghobject_t(hobject_t(sobject_t(obj, CEPH_NOSNAP))), 0, 2 * SIZE, contents);
    std::cerr << "exists: " << r << std::endl;


    for (uint64_t i = valid_reads.first; i < valid_reads.second; ++i) {
        ObjectContents old_contents = get_content(make_pair(coll, obj), i);

        std::cerr << "old_contents exists " << old_contents.exists() << std::endl;
        if (!old_contents.exists() && (r == -ENOENT)) return;

        if (old_contents.exists() && (r == -ENOENT)) continue;

        if (!old_contents.exists() && (r != -ENOENT)) continue;

        if (contents.length() != old_contents.size()) {
            std::cerr << "old_contents.size() is " << old_contents.size() << std::endl;
            continue;
        }
        ...
        bool matches = true;
        uint64_t pos = 0;
        for (; !bp.end() && iter.valid(); ++iter, ++bp, ++pos) {
            if (*iter != *bp) {
        ...
        if (matches) {
            if (on_start) _clean_forward(make_pair(coll, obj), i, db);
            return;
        }
    }
...
ObjectContents FileStoreTracker::get_current_content(const pair<coll_t, string>& obj)
{
...
ObjectContents FileStoreTracker::get_content(const pair<coll_t, string>& obj, uint64_t version)
{
    set<string> to_get;
    map<string, bufferlist> got;
    to_get.insert(seq_to_key(version));
    db->get(obj_to_prefix(obj), to_get, &got);
    if (got.empty()) return ObjectContents();
    pair<uint64_t, bufferlist> val;
    auto bp = got.begin()->second.cbegin();
    decode(val, bp);
    bp = val.second.begin();
...
pair<uint64_t, uint64_t> FileStoreTracker::get_valid_reads(const pair<coll_t, string>& obj)
{
...
void clear_obsolete(const pair<coll_t, string>& obj, const ObjStatus& status, KeyValueDB* db, KeyValueDB::Transaction t)
{
    KeyValueDB::Iterator iter = db->get_iterator(obj_to_prefix(obj));
    set<string> to_remove;
    iter->seek_to_first();
    for (; iter->valid() && iter->key() < seq_to_key(status.trim_to()); iter->next()) to_remove.insert(iter->key());
    t->rmkeys(obj_to_prefix(obj), to_remove);
}

void FileStoreTracker::committed(const pair<coll_t, string>& obj, uint64_t seq)
{
    std::lock_guard l{lock};
...
void FileStoreTracker::applied(const pair<coll_t, string>& obj, uint64_t seq)
{
    std::lock_guard l{lock};
...
uint64_t FileStoreTracker::set_content(const pair<coll_t, string>& obj, ObjectContents& content)
{
    KeyValueDB::Transaction t = db->get_transaction();
    KeyValueDB::Iterator iter = db->get_iterator(obj_to_prefix(obj));
    iter->seek_to_last();
    ...
    map<string, bufferlist> to_set;
    encode(make_pair(most_recent + 1, buf_content), to_set[seq_to_key(most_recent + 1)]);
