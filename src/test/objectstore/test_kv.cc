...
public:
    ...
    void rm_r(string path)
    {
        ...
        if (r) {
            cerr << "failed with exit code " << r << ", continuing anyway" << std::endl;
        }
    }
    ...
    void init()
    {
        ...
        db.reset(KeyValueDB::create(g_ceph_context, string(GetParam()), "kv_test_temp_dir"));
    }
    void fini() { db.reset(NULL); }
    ...
    void SetUp() override
    {
        ...
        if (r < 0 && errno != EEXIST) {
            r = -errno;
            cerr << __func__ << ": unable to create kv_test_temp_dir: " << cpp_strerror(r) << std::endl;
            return;
        }
        init();
    }
...
TEST_P(KVTest, BenchCommit)
{
    ...
    cout << n << " commits in " << dur << ", avg latency " << (dur / (double)n) << std::endl;
    fini();
}
...
struct AppendMOP : public KeyValueDB::MergeOperator
{
    ...
    void merge(const char* ldata, size_t llen, const char* rdata, size_t rlen, std::string* new_value) override
    {
...
TEST_P(KVTest, Merge)
{
    ...
    if (r < 0) return;   // No merge operators for this database type
    ASSERT_EQ(0, db->create_and_open(cout));
    {
        KeyValueDB::Transaction t = db->get_transaction();
        bufferlist v1, v2, v3;
...
{
    if (string(GetParam()) != "rocksdb") return;
    std::string cfs("O(7)=");
...
{
    if (string(GetParam()) != "rocksdb") GTEST_SKIP();
    ...
    for (int test = 0; test < 20; test++) {
        ...
        for (int i = 0; i < 100; i++) t->set("A", to_string(rand() % 100000), v1);
        db->submit_transaction_sync(t);
        db->compact();
...
TEST_P(KVTest, RocksDB_estimate_size_column_family)
{
    ...
    for (int test = 0; test < 20; test++) {
        ...
        bufferlist v1;
        v1.append(string(1000, '1'));
        for (int i = 0; i < 100; i++) t->set("cf1", to_string(rand() % 100000), v1);
        db->submit_transaction_sync(t);
        db->compact();
...
TEST_P(KVTest, RocksDB_parse_sharding_def)
{
    ...
    result = RocksDBStore::parse_sharding_def(text_def, sharding_def, &error_position, &error_msg);
...
public:
    ...
    void SetUp() override
    {
        ...
        db.reset(KeyValueDB::create(g_ceph_context, "rocksdb", "kv_test_temp_dir"));
        ASSERT_EQ(0, db->init(g_conf()->bluestore_rocksdb_options));
        if (verbose) cout << "Creating database with sharding: " << GetParam() << std::endl;
    ...
    */
    ...
    std::vector<std::string> sharding_defs = {
        "Betelgeuse D", "Betelgeuse(3) D", "Betelgeuse D(3)", "Betelgeuse(3) D(3)"};
    std::vector<std::string> prefixes = {"Ad", "Betelgeuse", "C", "D", "Evade"};
    std::vector<std::string> randoms = {
        "0",          "1",       "2",       "3",         "4",      "5",           "found",     "brain",
        "fully",      "pen",     "worth",   "race",      "stand",  "nodded",      "whenever",  "surrounded",
        "industrial", "skin",    "this",    "direction", "family", "beginning",   "whenever",  "held",
        "metal",      "year",    "like",    "valuable",  "softly", "whistle",     "perfectly", "broken",
        "idea",       "also",    "coffee",  "branch",    "tongue", "immediately", "bent",      "partly",
        "burn",       "include", "certain", "burst",     "final",  "smoke",       "positive",  "perfectly"};
    int R = randoms.size();
    ...
    bool end(const test_id& x) { return x[5] != 0; }
    void next(test_id& x)
    {
        x[0]++;
        for (int i = 0; i < 5; i++) {
    ...
    void generate_data(const test_id& x)
    {
        data.clear();
        for (int i = 0; i < 5; i++) {
            if (verbose) std::cout << x[i] << "-";
            switch (x[i]) {
            case 0: break;
            case 1: data[RocksDBStore::combine_strings(prefixes[i], randoms[k++ % R])] = randoms[v++ % R]; break;
            case 2:
    ...
    void data_to_db()
    {
        ...
        for (auto& d : data) {
            ...
            t->set(prefix, key, v1);
            if (verbose) std::cout << "SET " << prefix << " " << key << std::endl;
        }
...
TEST_P(RocksDBShardingTest, wholespace_next)
{
    ...
    do {
        ...
        while (dit != data.end()) {
            ...
            if (verbose) std::cout << "next " << prefix << " " << key << std::endl;
            ASSERT_EQ(it->next(), 0);
            ++dit;
        }
        ASSERT_EQ(it->valid(), false);
...
TEST_P(RocksDBShardingTest, wholespace_prev)
{
    ...
    do {
        ...
        while (dit != data.rend()) {
            ...
            ASSERT_EQ(raw_key.second, key);
            ASSERT_EQ(it->value().to_str(), dit->second);
            if (verbose) std::cout << "prev " << prefix << " " << key << std::endl;
            ASSERT_EQ(it->prev(), 0);
            ++dit;
        }
        ASSERT_EQ(it->valid(), false);
...
TEST_P(RocksDBShardingTest, wholespace_lower_bound)
{
    ...
    do {
        ...
        while (dit != data.end()) {
            ...
            ASSERT_EQ(raw_key.first, prefix);
            ASSERT_EQ(raw_key.second, key);
            if (verbose) std::cout << "lower_bound " << prefix << " " << key << std::endl;
            ASSERT_EQ(it->next(), 0);
            ++dit;
        }
        ASSERT_EQ(it->valid(), false);
...
TEST_P(RocksDBShardingTest, wholespace_upper_bound)
{
    ...
    do {
        ...
        while (dit != data.end()) {
            ...
            ASSERT_EQ(raw_key.first, prefix);
            ASSERT_EQ(raw_key.second, key);
            if (verbose) std::cout << "upper_bound " << prefix << " " << key_minus_1 << std::endl;
            ASSERT_EQ(it->next(), 0);
            ++dit;
        }
        ASSERT_EQ(it->valid(), false);
    ...
    void data_to_db()
    {
        ...
        for (auto& d : data) {
            ...
            if ((i % 1000) == 0) {
                ASSERT_EQ(db->submit_transaction_sync(t), 0);
                t.reset();
                if (verbose) std::cout << "writing key to DB" << std::endl;
                t = db->get_transaction();
            }
        }
        if (verbose) std::cout << "writing keys to DB" << std::endl;
        ASSERT_EQ(db->submit_transaction_sync(t), 0);
    }
...
INSTANTIATE_TEST_SUITE_P(KeyValueDB, KVTest, ::testing::Values("leveldb", "rocksdb", "memdb"));

INSTANTIATE_TEST_SUITE_P(KeyValueDB, RocksDBShardingTest,
                         ::testing::Values("Betelgeuse D", "Betelgeuse(3) D", "Betelgeuse D(3)", "Betelgeuse(3) D(3)"));
...
int main(int argc, char** argv)
{
    ...
    g_ceph_context->_conf.set_val("enable_experimental_unrecoverable_data_corrupting_features", "rocksdb, memdb");
