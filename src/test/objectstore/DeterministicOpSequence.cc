...
DeterministicOpSequence::DeterministicOpSequence(ObjectStore* store, std::string status)
    : TestObjectStoreState(store)
    , txn(0)
{
    if (!status.empty()) m_status.open(status.c_str());
}
...
bool DeterministicOpSequence::run_one_op(int op, rngen_t& gen)
{
    bool ok = false;
    switch (op) {
    case DSOP_TOUCH: ok = do_touch(gen); break;
    case DSOP_WRITE: ok = do_write(gen); break;
    case DSOP_CLONE: ok = do_clone(gen); break;
    case DSOP_CLONE_RANGE: ok = do_clone_range(gen); break;
    case DSOP_OBJ_REMOVE: ok = do_remove(gen); break;
    case DSOP_COLL_MOVE: ok = do_coll_move(gen); break;
    case DSOP_SET_ATTRS: ok = do_set_attrs(gen); break;
    case DSOP_COLL_CREATE: ok = do_coll_create(gen); break;

    default: cout << "bad op " << op << std::endl; ceph_abort_msg("bad op");
    }
    return ok;
}
...
void DeterministicOpSequence::note_txn(coll_entry_t* entry, ObjectStore::Transaction* t)
{
    bufferlist bl;
    encode(txn, bl);
...
static void _gen_random(rngen_t& gen, size_t size, bufferlist& bl)
{
...
static void gen_attrs(rngen_t& gen, map<string, bufferlist>* out)
{
...
bool DeterministicOpSequence::_prepare_clone(rngen_t& gen, coll_entry_t** entry_ret, int* orig_obj_id,
                                             hobject_t* orig_obj_ret, int* new_obj_id, hobject_t* new_obj_ret)
{
    int coll_id = _gen_coll_id(gen);
...
bool DeterministicOpSequence::do_clone_range(rngen_t& gen)
{
    ...
    dout(0) << "do_clone_range " << entry->m_cid << "/" << orig_obj << " (0~" << size << ")"
            << " => " << entry->m_cid << "/" << new_obj << " (0)" << dendl;
...
void DeterministicOpSequence::_do_coll_create(coll_entry_t* entry, uint32_t pg_num, uint64_t num_objs)
{
    ...
    dout(0) << "Give collection: " << entry->m_cid << " a hint, pg_num is: " << pg_num << ", num_objs is: " << num_objs
            << dendl;
...
void DeterministicOpSequence::_do_set_attrs(coll_entry_t* entry, hobject_t& obj, const map<string, bufferlist>& attrs)
{
    ObjectStore::Transaction t;
    note_txn(entry, &t);
...
void DeterministicOpSequence::_do_write(coll_entry_t* entry, hobject_t& obj, uint64_t off, uint64_t len,
...
void DeterministicOpSequence::_do_clone(coll_entry_t* entry, hobject_t& orig_obj, hobject_t& new_obj)
{
    ObjectStore::Transaction t;
    note_txn(entry, &t);
...
void DeterministicOpSequence::_do_clone_range(coll_entry_t* entry, hobject_t& orig_obj, hobject_t& new_obj,
                                              uint64_t srcoff, uint64_t srclen, uint64_t dstoff)
{
    ObjectStore::Transaction t;
    note_txn(entry, &t);
    t.clone_range(entry->m_cid, ghobject_t(orig_obj), ghobject_t(new_obj), srcoff, srclen, dstoff);
    m_store->queue_transaction(entry->m_ch, std::move(t));
}

void DeterministicOpSequence::_do_write_and_clone_range(coll_entry_t* entry, hobject_t& orig_obj, hobject_t& new_obj,
                                                        uint64_t srcoff, uint64_t srclen, uint64_t dstoff,
                                                        bufferlist& bl)
{
    ObjectStore::Transaction t;
...
void DeterministicOpSequence::_do_coll_move(coll_entry_t* entry, hobject_t& orig_obj, hobject_t& new_obj)
{
    ObjectStore::Transaction t;
    note_txn(entry, &t);
    t.remove(entry->m_cid, ghobject_t(new_obj));
    t.collection_move_rename(entry->m_cid, ghobject_t(orig_obj), entry->m_cid, ghobject_t(new_obj));
    m_store->queue_transaction(entry->m_ch, std::move(t));
}
