...
TEST(sb_info_space_efficient_map_t, size)
{
    ...
    BlueStore::OnodeCacheShard* oc = BlueStore::OnodeCacheShard::create(g_ceph_context, "lru", NULL);
    BlueStore::BufferCacheShard* bc = BlueStore::BufferCacheShard::create(g_ceph_context, "lru", NULL);
...
TEST(bluestore_blob_t, calc_csum)
{
    ...
    for (unsigned csum_type = Checksummer::CSUM_NONE + 1; csum_type < Checksummer::CSUM_MAX; ++csum_type) {
        cout << "csum_type " << Checksummer::get_csum_type_string(csum_type) << std::endl;
...
TEST(bluestore_blob_t, csum_bench)
{
    ...
    for (char* a = bp.c_str(); a < bp.c_str() + bp.length(); ++a) *a = (unsigned long)a & 0xff;
    bl.append(bp);
    int count = 256;
    for (unsigned csum_type = 1; csum_type < Checksummer::CSUM_MAX; ++csum_type) {
        ...
        cout << "csum_type " << Checksummer::get_csum_type_string(csum_type) << ", " << dur << " seconds, " << mbsec
             << " MB/sec" << std::endl;
    }
}
...
{
    {
        ...
        b.dirty_blob().allocated_test(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, 0x8000));
    ...
    }
    {
        ...
        b.allocated_test(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, 0x13000));
...
TEST(bluestore_blob_t, prune_tail)
{
    ...
    a.allocated_test(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, 0x2000));
    ...
    bluestore_blob_t b;
    b.allocated_test(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, 0x2000));
    ASSERT_FALSE(a.can_prune_tail());
}
...
TEST(Blob, legacy_decode)
{
    ...
    bufferlist bl, bl2;
    {
        ...
        B.bound_encode(bound,
                       1, /*struct_v*/
                       0, /*sbid*/
                       false);
        fake_ref_map.bound_encode(bound);

        B.bound_encode(bound2,
                       2, /*struct_v*/
                       0, /*sbid*/
                       true);
...
TEST(GarbageCollector, BasicTest)
{
     ...
     */
    {
        ...
        em.extent_map.insert(*new BlueStore::Extent(0x8000, 0x8000, 0x8000, b2));   // new extent
        b2->get_ref(coll.get(), 0x8000, 0x8000);
        em.extent_map.insert(*new BlueStore::Extent(0x10000, 0, 0x20000, b3));   // new extent
        b3->get_ref(coll.get(), 0, 0x20000);
        em.extent_map.insert(*new BlueStore::Extent(0x30000, 0, 0xf000, b4));   // new extent
        ...
        old_extents.push_back(*new BlueStore::OldExtent(0x10000, 0x10000, 0x20000, b1));
     ...
     */
    {
        ...
        em.extent_map.insert(*new BlueStore::Extent(0x3000, 0, 0x4000, b2));   // new extent
     ...
     */
    {
        ...
        old_extents.push_back(*new BlueStore::OldExtent(0x10000, 0x10000, 0x10000, b0));
        old_extents.push_back(*new BlueStore::OldExtent(0x20000, 0x00000, 0x1f000, b1));
...
{
    {
        ...
        b.allocated_test(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET, 1 * min_alloc_size));
...
static int test_extents(uint64_t index, extent_t* ext_arr, uint64_t ext_arr_size, SimpleBitmap& sbmap, bool set)
{
    ...
    if (ext.length == 0) {
        ...
        std::cerr << "sbmap.get_next_" << (set ? "set" : "clr") << "_extent(" << offset
                  << ") return length = " << ext.length << std::endl;
        return -1;
    }
}
...
static int test_intersections(unsigned test_idx, SimpleBitmap& sbmap, uint8_t map[], uint64_t map_size)
{
    ...
    std::cout << " [" << test_idx << "] set_bit_count = 0x" << std::setfill('0') << std::setw(8) << set_bit_count
              << ", clr_bit_count = 0x" << std::setfill('0') << std::setw(8) << clr_bit_count << ", sum = 0x"
              << set_bit_count + clr_bit_count << std::endl;
    std::cout << std::dec;
    uint64_t offset = 0;
    for (uint64_t i = 0; i < (set_op_count + clr_op_count); i++) {
        ...
        for (uint64_t idx = ext.offset; idx < ext.offset + ext.length; idx++) {
            if (map[idx] != 0xFF) {
                std::cerr << "map[" << idx << "] is clear, but extent [" << ext.offset << ", " << ext.length
                          << "] is set" << std::endl;
                return -1;
            }
        }
    ...
    for (uint64_t i = 0; i < (set_op_count + clr_op_count); i++) {
        ...
        for (uint64_t idx = ext.offset; idx < ext.offset + ext.length; idx++) {
            if (map[idx]) {
                std::cerr << "map[" << idx << "] is set, but extent [" << ext.offset << ", " << ext.length
                          << "] is free" << std::endl;
                return -1;
            }
        }
...
//---------------------------------------------------------------------------------
static int test_extents_boundaries(uint64_t index, extent_t* ext_arr, uint64_t ext_arr_size, SimpleBitmap& sbmap,
                                   bool set)
{
    uint64_t n = sbmap.get_size();
