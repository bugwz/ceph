...
public:
    struct coll_entry_t
    {
        ...
        coll_entry_t(spg_t pgid, ObjectStore::CollectionHandle& ch, char* meta_obj_buf)
            : m_pgid(pgid)
            , m_cid(m_pgid)
...
protected:
    ...
    void wait_for_ready()
    {
        ...
        m_finished_cond.wait(locker, [this] { return m_max_in_flight <= 0 || m_in_flight < m_max_in_flight; });
    }
    ...
    void set_max_in_flight(int max) { m_max_in_flight = max; }
    void set_num_objs_per_coll(int val) { m_num_objs_per_coll = val; }
...
public:
    ...
    void init() { init(m_default_num_colls, 0); }

    int inc_in_flight() { return ++m_in_flight; }

    int dec_in_flight() { return --m_in_flight; }
    ...
    public:
        explicit C_OnFinished(TestObjectStoreState* state)
            : m_state(state)
        {}
