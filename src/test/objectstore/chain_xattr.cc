...
TEST(chain_xattr, get_and_set)
{
    ...
    //
    {
...

        {
            ...
            ASSERT_EQ(LARGE_BLOCK_LEN, chain_setxattr(file, name.c_str(), x.c_str(), LARGE_BLOCK_LEN));
            ASSERT_EQ(CHAIN_XATTR_MAX_BLOCK_LEN,
                      chain_setxattr(file, name.c_str(), x.c_str(), CHAIN_XATTR_MAX_BLOCK_LEN));
...

        {
            ...
            ASSERT_EQ(LARGE_BLOCK_LEN, chain_fsetxattr(fd, name.c_str(), x.c_str(), LARGE_BLOCK_LEN));
            ASSERT_EQ(CHAIN_XATTR_MAX_BLOCK_LEN,
                      chain_fsetxattr(fd, name.c_str(), x.c_str(), CHAIN_XATTR_MAX_BLOCK_LEN));
...
TEST(chain_xattr, chunk_aligned)
{
    ...
    for (int len = CHAIN_XATTR_MAX_BLOCK_LEN - 10; len < CHAIN_XATTR_MAX_BLOCK_LEN + 10; ++len) {
        cout << len << std::endl;
    ...
    for (int len = CHAIN_XATTR_SHORT_BLOCK_LEN - 10; len < CHAIN_XATTR_SHORT_BLOCK_LEN + 10; ++len) {
        cout << len << std::endl;
        const string x(len, 'x');
        char buf[len * 2];
        ASSERT_EQ(len, chain_setxattr(file, name.c_str(), x.c_str(), len));
...

    {
        ...
        ASSERT_EQ(-ERANGE, chain_getxattr(file, aname, buf, CHAIN_XATTR_SHORT_BLOCK_LEN * 2));
    }
    {
        ...
        ASSERT_EQ(-ERANGE, chain_fgetxattr(fd, aname, buf, CHAIN_XATTR_SHORT_BLOCK_LEN * 2));
    }
...
bool listxattr_cmp(char* xattr1, char* xattr2, int size)
{
    ...
    if (xattrs1.size() != xattrs2.size()) return false;

    std::sort(xattrs1.begin(), xattrs1.end());
    std::sort(xattrs2.begin(), xattrs2.end());
    std::vector<string> diff;
    std::set_difference(xattrs1.begin(), xattrs1.end(), xattrs2.begin(), xattrs2.end(), diff.begin());
...
TEST(chain_xattr, listxattr)
{
    ...
    int buffer_size = 0;
    if (orig_size) buffer_size += orig_size + sizeof(char);
...
int main(int argc, char** argv)
{
    ...
    if (ret >= 0) ret = ::ceph_os_fgetxattr(tmpfd, "user.test", &y, sizeof(y));
    ::close(tmpfd);
    ::unlink(file);
    if ((ret < 0) || (x != y)) {
        cerr << "SKIP all tests because extended attributes don't appear to work in the file system in which the tests "
                "are run: "
...
// Local Variables:
// compile-command: "cd ../.. ; make unittest_chain_xattr ; valgrind --tool=memcheck ./unittest_chain_xattr #
// --gtest_filter=chain_xattr.get_and_set" End:
