...
public:
    ...
    void init_alloc(int64_t size, uint64_t min_alloc_size)
    {
        ...
        alloc.reset(
            Allocator::create(g_ceph_context, GetParam(), size, min_alloc_size, 256 * 1048576, 100 * 256 * 1048576ull));
    }

    void init_close() { alloc.reset(0); }
};
...
TEST_P(AllocTest, test_alloc_min_alloc)
{
...

    {
        ...
        EXPECT_EQ(block_size, alloc->allocate(block_size, block_size, 0, (int64_t)0, &extents));
    }
     ...
     */
    {
        ...
        EXPECT_EQ(4 * block_size,
                  alloc->allocate(4 * (uint64_t)block_size, (uint64_t)block_size, 0, (int64_t)0, &extents));
...
TEST_P(AllocTest, test_alloc_min_max_alloc)
{
     ...
     */
    {
        ...
        PExtentVector extents;
        EXPECT_EQ(4 * block_size,
                  alloc->allocate(4 * (uint64_t)block_size, (uint64_t)block_size, block_size, (int64_t)0, &extents));
        for (auto e : extents) {
     ...
     */
    {
        ...
        EXPECT_EQ(
            4 * block_size,
            alloc->allocate(4 * (uint64_t)block_size, (uint64_t)block_size, 2 * block_size, (int64_t)0, &extents));
        EXPECT_EQ(2u, extents.size());
        for (auto& e : extents) {
     ...
     */
    {
        ...
        EXPECT_EQ(1024 * block_size,
                  alloc->allocate(
                      1024 * (uint64_t)block_size, (uint64_t)block_size * 4, block_size * 4, (int64_t)0, &extents));
        for (auto& e : extents) {
     ...
     */
    {
        ...
        EXPECT_EQ(
            16 * block_size,
            alloc->allocate(16 * (uint64_t)block_size, (uint64_t)block_size, 2 * block_size, (int64_t)0, &extents));
...
TEST_P(AllocTest, test_alloc_failure)
{
...

    {
        ...
        EXPECT_EQ(512 * block_size,
                  alloc->allocate(
                      512 * (uint64_t)block_size, (uint64_t)block_size * 256, block_size * 256, (int64_t)0, &extents));
        alloc->init_add_free(0, block_size * 256);
        alloc->init_add_free(block_size * 512, block_size * 256);
        extents.clear();
        EXPECT_EQ(-ENOSPC,
                  alloc->allocate(
                      512 * (uint64_t)block_size, (uint64_t)block_size * 512, block_size * 512, (int64_t)0, &extents));
    }
}
...
TEST_P(AllocTest, test_alloc_big)
{
    ...
    for (int64_t big = mas; big < 1048576 * 128; big *= 2) {
        cout << big << std::endl;
        PExtentVector extents;
        EXPECT_EQ(big, alloc->allocate(big, mas, 0, &extents));
    }
}
...
TEST_P(AllocTest, test_alloc_fragmentation)
{
    ...
    for (size_t i = 0; i < capacity / alloc_unit; ++i) {
        tmp.clear();
        EXPECT_EQ(static_cast<int64_t>(want_size), alloc->allocate(want_size, alloc_unit, 0, 0, &tmp));
    ...
    for (size_t round = 0; round < rounds; round++) {
        for (size_t j = 0; j < actions_per_round; j++) {
            ...
            if (rng() % capacity >= allocated_cnt) {
                ...
                if (r > 0) {
                    for (auto& t : tmp) {
                        if (t.length > 0) allocated.push_back(t);
                    }
                    allocated_cnt += r;
                }
            }
...
{
    if (string(GetParam()) != "bitmap") return;
    ...
    EXPECT_EQ(static_cast<int64_t>(want_size), alloc->allocate(want_size, 0x100000, 0, 0, &tmp));
...
TEST_P(AllocTest, test_alloc_big2)
{
    ...
    EXPECT_EQ(need, alloc->allocate(need, mas, 0, &extents));
...
TEST_P(AllocTest, test_alloc_contiguous)
{
...

    {
        ...
        EXPECT_EQ(need, alloc->allocate(need, need, 0, (int64_t)0, &extents));
        EXPECT_EQ(1u, extents.size());
...
INSTANTIATE_TEST_SUITE_P(Allocator, AllocTest, ::testing::Values("stupid", "bitmap", "avl", "hybrid"));
