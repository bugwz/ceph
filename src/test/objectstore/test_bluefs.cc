...
private:
    ...
    {
        static int n = 0;
        string fn = "ceph_test_bluefs.tmp.block." + stringify(getpid()) + "." + stringify(++n);
    ...
    static void rm_temp_bdev(string f) { ::unlink(f.c_str()); }
};
...
TEST(BlueFS, very_large_write)
{
    ...
    }
    {
        ...
        for (unsigned i = 0; i < 3 * 1024 * 1048576ull / sizeof(buf); ++i) {
            ...
            if (r) {
                cerr << "read got mismatch at offset " << i * sizeof(buf) << " r " << r << std::endl;
            }
            ASSERT_EQ(0, r);
        }
        for (unsigned i = 0; i < 2 * 1024 * 1048576ull / sizeof(buf); ++i) {
...
TEST(BlueFS, test_tracker_50965)
{
    ...
    ASSERT_FALSE(h_slow_dirty_seq_1 != 0 && h_slow_dirty_seq_2 == 0 && h_slow_dev_dirty == true);
...
TEST(BlueFS, test_shared_alloc)
{
    ...
    shared_alloc.set(Allocator::create(g_ceph_context,
                                       g_ceph_context->_conf->bluefs_allocator,
                                       size,
                                       shared_alloc_unit,
                                       0,
                                       0,
                                       "test shared allocator"),
                     shared_alloc_unit);
    ...
    ASSERT_EQ(0, fs.add_block_device(BlueFS::BDEV_SLOW, bdev_slow.path, false, 0, &shared_alloc));
    uuid_d fsid;
    ASSERT_EQ(0, fs.mkfs(fsid, {BlueFS::BDEV_DB, false, false}));
    ASSERT_EQ(0, fs.mount());
    ASSERT_EQ(0, fs.maybe_verify_layout({BlueFS::BDEV_DB, false, false}));
...
TEST(BlueFS, test_shared_alloc_sparse)
{
    ...
    ConfSaver conf(g_ceph_context->_conf);
    conf.SetVal("bluefs_shared_alloc_size", stringify(bluefs_alloc_unit).c_str());

    bluefs_shared_alloc_context_t shared_alloc;
    shared_alloc.set(
        Allocator::create(
            g_ceph_context, g_ceph_context->_conf->bluefs_allocator, size, main_unit, 0, 0, "test shared allocator"),
        main_unit);
    ...
    BlueFS fs(g_ceph_context);
    ASSERT_EQ(0, fs.add_block_device(BlueFS::BDEV_DB, bdev_slow.path, false, 0, &shared_alloc));
    uuid_d fsid;
    ASSERT_EQ(0, fs.mkfs(fsid, {BlueFS::BDEV_DB, false, false}));
    ASSERT_EQ(0, fs.mount());
    ASSERT_EQ(0, fs.maybe_verify_layout({BlueFS::BDEV_DB, false, false}));
...
void create_files(BlueFS& fs, atomic_bool& stop_creating, atomic_bool& started_creating)
{
    uint32_t i = 0;
    stringstream ss;
    string dir = "dir.";
...
TEST(BlueFS, test_concurrent_dir_link_and_compact_log_56210)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        ASSERT_EQ(0, fs.maybe_verify_layout({BlueFS::BDEV_DB, false, false}));
        {
            ...
            create_thread =
                std::thread(create_files, std::ref(fs), std::ref(stop_creating), std::ref(started_creating));
...
int main(int argc, char** argv)
{
    ...
    map<string, string> defaults = {{"debug_bluefs", "1/20"}, {"debug_bdev", "1/20"}};
