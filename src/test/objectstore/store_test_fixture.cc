...
static void rm_r(const string& path)
{
    ...
    if (r) {
        if (r == -1) {
            r = errno;
            cerr << "system() failed to fork() " << cpp_strerror(r) << ", continuing anyway" << std::endl;
        }
        else {
...
void StoreTestFixture::SetUp()
{
    ...
    store.reset(ObjectStore::create(g_ceph_context, type, data_dir, string("store_test_temp_journal")));
    if (!store) {
