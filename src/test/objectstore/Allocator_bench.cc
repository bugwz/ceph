...
public:
    ...
    void doOverwriteTest(uint64_t capacity, uint64_t prefill, uint64_t overwrite);
};
...
public:
    ...
    inline uint64_t get_head() const { return head; }

    inline uint64_t get_tail() const { return tail; }
...
TEST_P(AllocTest, test_alloc_bench_seq)
{
    ...
    for (uint64_t i = 0; i < capacity; i += want_size) {
        ...
        if (0 == (i % (1 * 1024 * _1m))) {
            std::cout << "alloc " << i / 1024 / 1024 << " mb of " << capacity / 1024 / 1024 << std::endl;
        }
    }
    ...
    for (size_t i = 0; i < capacity; i += want_size) {
        ...
        if (0 == (i % (1 * 1024 * _1m))) {
            std::cout << "release " << i / 1024 / 1024 << " mb of " << capacity / 1024 / 1024 << std::endl;
        }
    }
...
void AllocTest::doOverwriteTest(uint64_t capacity, uint64_t prefill, uint64_t overwrite)
{
    uint64_t alloc_unit = 4096;
    PExtentVector allocated, tmp;
    AllocTracker at(capacity, alloc_unit);
    ...
    for (uint64_t i = 0; i < cap;) {
        ...
        if (0 == (i % (1 * 1024 * _1m))) {
            std::cout << "alloc " << i / 1024 / 1024 << " mb of " << cap / 1024 / 1024 << std::endl;
        }
    }
    ...
    for (uint64_t i = 0; i < cap;) {
        ...
        if (0 == (i % (1 * 1024 * _1m))) {
            std::cout << "reuse " << i / 1024 / 1024 << " mb of " << cap / 1024 / 1024 << std::endl;
        }
    }
    std::cout << "Executed in " << ceph_clock_now() - start << std::endl;
    std::cout << "Avail " << alloc->get_free() / _1m << " MB" << std::endl;
...
TEST_P(AllocTest, mempoolAccounting)
{
    ...
    Allocator* alloc = Allocator::create(g_ceph_context, string(GetParam()), capacity, alloc_size);
