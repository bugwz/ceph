...
{
    cerr << "Usage: " << name
         << " <log_to_replay> <raw_duplicates|duplicates|free_dump|try_alloc count want alloc_unit|replay_alloc "
            "alloc_list_file|export_binary out_file>"
         << std::endl;
}

void usage_replay_alloc(const string& name)
{
    cerr << "Detailed replay_alloc usage: " << name
         << " <allocator_dump_JSON> replay_alloc <alloc_list_file> [number of replays]" << std::endl;
    ...
    cerr
        << "Allocation request format (space separated, optional parameters are 0 if not given): want unit [max] [hint]"
        << std::endl;
}
    ...
    while (true) {
        ...
        if (sp) {
            ...
            if (!owned_by_app.contains(offs, len)) {
                std::cerr << "error: unexpected return to allocator, not owned by app: " << s << std::endl;
                return -1;
            }
        ...
        if (sp) {
            ...
            if (owned_by_app.intersects(offs, len)) {
                std::cerr << "error: unexpected takeover from allocator, already owned by app: " << s << std::endl;
                return -1;
            }
            else {
        ...
        if (sp) {
            ...
            //  and need to bypass
            //  2019-05-30 03:23:48.780 7f889a5edf00 10 fbmap_alloc 0x5642ed370600 allocate
            //  0x69d400000~200000/100000,0,0
            ...
            for (auto& ee : tmp) {
                std::cerr << "dump extent: " << std::hex << ee.offset << "~" << ee.length << std::dec << std::endl;
            }
            std::cerr << "dump completed." << std::endl;
            for (auto& e : tmp) {
                if (owned_by_app.intersects(e.offset, e.length)) {
                    std::cerr << "error: unexpected allocated extent: " << std::hex << e.offset << "~" << e.length
                              << " dumping all allocations:" << std::dec << std::endl;
                    for (auto& ee : tmp) {
                        std::cerr << "dump extent: " << std::hex << ee.offset << "~" << ee.length << std::dec
                                  << std::endl;
                    }
                    std::cerr << "dump completed." << std::endl;
                    return -1;
        ...
        if (sp) {
            ...
            alloc.reset(Allocator::create(g_ceph_context, alloc_type, total, alloc_unit));
...
int replay_free_dump_and_apply_raw(char* fname,
                                   std::function<void(std::string_view, int64_t, int64_t, std::string_view)> create,
                                   std::function<void(uint64_t, uint64_t)> add_ext)
{
    ...
    if (o) {
        ...
        if (fd < 0) {
            std::cerr << "error: unable to open extents file: " << filename << ", " << cpp_strerror(-errno)
                      << std::endl;
            return -1;
        }
    }
    ...
    if (fd < 0) {
        ...
        if (r < 0) {
            std::cerr << "error: error reading from extents file: " << cpp_strerror(-errno) << std::endl;
        }
        else {
            auto p = bl.cbegin();
...
*/
int replay_free_dump_and_apply(char* fname, std::function<int(Allocator*, const string& aname)> fn)
{
    unique_ptr<Allocator> alloc;
    auto create_fn =
        [&](std::string_view alloc_type, int64_t capacity, int64_t alloc_unit, std::string_view alloc_name) {
            alloc.reset(Allocator::create(g_ceph_context, alloc_type, capacity, alloc_unit, 0, 0, alloc_name));
        };
    auto add_fn = [&](uint64_t offset, uint64_t len) { alloc->init_add_free(offset, len); };
    int r = replay_free_dump_and_apply_raw(fname, create_fn, add_fn);
    if (r == 0) {
...
void dump_alloc(Allocator* alloc, const string& aname)
{
    ...
    auto r = admin_socket->execute_command({cmd}, in, err, &out);
    if (r != 0) {
...
int export_as_binary(char* fname, char* target_fname)
{
    ...
    if (fd < 0) {
        std::cerr << "error: unable to open target file: " << target_fname << ", " << cpp_strerror(-errno) << std::endl;
        return -1;
    }
    ...
    auto dummy_create_fn =
        [&](std::string_view alloc_type, int64_t capacity, int64_t alloc_unit, std::string_view alloc_name) {};
    auto add_fn = [&](uint64_t offset, uint64_t len) { amap.free_extents.emplace_back(offset, len); };
    int r = replay_free_dump_and_apply_raw(fname, dummy_create_fn, add_fn);
    if (r == 0) {
        bufferlist out;
        ceph::encode(amap, out);
        auto w = write(fd, out.c_str(), out.length());
        if (w < 1) {
            std::cerr << "error: unable to open target file: " << target_fname << ", " << cpp_strerror(-errno)
                      << std::endl;
        }
    }
    close(fd);
...
int check_duplicates(char* fname)
{
    ...
    auto add_fn = [&](uint64_t offset, uint64_t len) {
        ++pos;
    ...
    pos = 0;
    auto add_fn2 = [&](uint64_t offset, uint64_t len) {
        ++pos;
    ...
    r = replay_free_dump_and_apply_raw(fname, dummy_create_fn, add_fn2);
    ceph_assert(r >= 0);
    auto itA = invalid_extentsA.begin();
    auto itB = invalid_extentsB.begin();
    while (itA != invalid_extentsA.end()) {
        std::cerr << "error: overlapping extents: " << std::hex << itA.get_start() << "~"
                  << itA.get_end() - itA.get_start() << " vs.";
        while (itB != invalid_extentsB.end() && itB.get_start() >= itA.get_start() && itB.get_end() <= itA.get_end()) {
...
int main(int argc, char** argv)
{
    ...
    if (strcmp(argv[2], "raw_duplicates") == 0) {
        ...
        return replay_free_dump_and_apply(argv[1], [&](Allocator* a, const string& aname) {
            ceph_assert(a);
            std::cout << "Fragmentation:" << a->get_fragmentation() << std::endl;
            std::cout << "Fragmentation score:" << a->get_fragmentation_score() << std::endl;
            std::cout << "Free:" << std::hex << a->get_free() << std::dec << std::endl;
            {
        ...
        if (argc < 6) {
            std::cerr << "Error: insufficient arguments for \"try_alloc\" operation." << std::endl;
            usage(argv[0]);
            return 1;
        }
        ...
        return replay_free_dump_and_apply(argv[1], [&](Allocator* a, const string& aname) {
            ...
            std::cout << "Free:" << std::hex << a->get_free() << std::dec << std::endl;
            {
                ...
                for (size_t i = 0; i < count; i++) {
                    ...
                    if (r < 0) {
                        std::cerr << "Error: allocation failure at step:" << i + 1 << ", ret = " << r << std::endl;
                        return -1;
                    }
                }
            }
            std::cout << "Successfully allocated: " << count << " * " << want << ", unit:" << alloc_unit << std::endl;
            return 0;
        });
    }
    else if (strcmp(argv[2], "replay_alloc") == 0) {
        if (argc < 4) {
            std::cerr << "Error: insufficient arguments for \"replay_alloc\" option." << std::endl;
                    ...
                    while (fgets(s, sizeof(s), f_alloc_list) != nullptr) {
                        ...
                        if (r < 0) {
                            ...
                            std::cerr << "Error: allocation failure code: " << r
                                      << " requested want/unit/max/hint (hex): " << std::hex << want << "/" << unit
                                      << "/" << max << "/" << hint << std::dec << std::endl;
                            std::cerr << "Fragmentation:" << a->get_fragmentation() << std::endl;
                            std::cerr << "Fragmentation score:" << a->get_fragmentation_score() << std::endl;
                            std::cerr << "Free:" << std::hex << a->get_free() << std::dec << std::endl;
                        ...
                        std::cout << "Duration (ns): " << (ceph::mono_clock::now() - t0).count()
                                  << " want/unit/max/hint (hex): " << std::hex << want << "/" << unit << "/" << max
                                  << "/" << hint << std::dec << std::endl;
