...
std::pair<bufferptr, std::vector<snapid_t>> create_object_snapset(const ScrubGenerator::RealObj& robj,
...
RealObjsConfList ScrubGenerator::make_real_objs_conf(int64_t pool_id, const RealObjsConf& blueprint,
...
ScrubGenerator::SmapEntry ScrubGenerator::make_smobject(const ScrubGenerator::RealObj& blueprint, int osd_num)
{
    ...
    if (blueprint.snapset_mock_data) {
        auto [bp, snaps] = create_object_snapset(blueprint, blueprint.snapset_mock_data);
        ret.smobj.attrs[SS_ATTR] = bp;
        std::cout << fmt::format("{}: ({}) osd:{} snaps:{}", __func__, ret.ghobj.hobj, osd_num, snaps) << std::endl;
    }
    ...
    for (const auto& [at_k, at_v] : blueprint.data.attrs) {
        ret.smobj.attrs[at_k] = ceph::buffer::copy(at_v.c_str(), at_v.size());
        {
            ...
            std::cout << fmt::format("{}: verification: {}", __func__, bkstr) << std::endl;
        }
    }
...
all_clones_snaps_t ScrubGenerator::all_clones(const ScrubGenerator::RealObj& head_obj)
{
    std::cout << fmt::format("{}: head_obj.ghobj.hobj:{}", __func__, head_obj.ghobj.hobj) << std::endl;
    ...
    for (const auto& clone : head_obj.snapset_mock_data->clones) {
        ...
        std::cout << fmt::format("{}: clone:{} clone_set:{}", __func__, clone_hobj, clone_set) << std::endl;
    }

    return ret;
}

void ScrubGenerator::add_object(ScrubMap& map, const ScrubGenerator::RealObj& real_obj, int osd_num)
{
    ...
    std::cout << fmt::format(
                     "{}: osd:{} smap entry: {} {}", __func__, osd_num, entry.smobj.size, entry.smobj.attrs.size())
              << std::endl;
