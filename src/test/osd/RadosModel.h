...
public:
    ...
    uint64_t& get_handle() { return handle; }
};
...
public:
    ...
    TestOpStat* stat;
    bool done = false;
    TestOp(int n, RadosTestContext* context, TestOpStat* stat = 0)
        : num(n)
        , context(context)
        , stat(stat)
    {}
    ...
    {
        uint64_t id;
        explicit CallbackInfo(uint64_t id)
            : id(id)
        {}
     ...
     */
    virtual void _finish(CallbackInfo* info) { return; }
...
public:
    ...
    RadosTestContext(const std::string& pool_name, int max_in_flight, uint64_t max_size, uint64_t min_stride_size,
                     uint64_t max_stride_size, bool no_omap, bool no_sparse, bool pool_snaps,
                     bool write_fadvise_dontneed, const std::string& low_tier_pool_name, bool enable_dedup,
                     std::string chunk_algo, std::string chunk_size, const char* id = 0)
        : pool_obj_cont()
        , current_snap(0)
        , pool_name(pool_name)
        , next_oid(0)
    ...
    int init()
    {
        ...
        bufferlist inbl;
        r = rados.mon_command("{\"prefix\": \"osd pool set\", \"pool\": \"" + pool_name +
                                  "\", \"var\": \"write_fadvise_dontneed\", \"val\": \"" +
                                  (write_fadvise_dontneed ? "true" : "false") + "\"}",
                              inbl,
                              NULL,
                              NULL);
        if (r < 0) {
    ...
    void kick() { wait_cond.notify_all(); }

    TestWatchContext* get_watch_context(const std::string& oid) { return watches.count(oid) ? watches[oid] : 0; }
    ...
    {
        ObjectDesc new_obj;
        for (auto i = pool_obj_cont.rbegin(); i != pool_obj_cont.rend(); ++i) {
    ...
    void rm_object_attrs(const std::string& oid, const std::set<std::string>& attrs)
    {
        ...
        for (std::set<std::string>::const_iterator i = attrs.begin(); i != attrs.end(); ++i) {
    ...
    void update_object(ContentsGenerator* cont_gen, const std::string& oid, const ContDesc& contents)
    {
        ObjectDesc new_obj = get_most_recent(oid);
        new_obj.exists = true;
        new_obj.dirty = true;
        new_obj.flushed = false;
        new_obj.update(cont_gen, contents);
        pool_obj_cont[current_snap].insert_or_assign(oid, new_obj);
    }
    ...
    void update_object_version(const std::string& oid, uint64_t version, int snap = -1)
    {
        for (auto i = pool_obj_cont.rbegin(); i != pool_obj_cont.rend(); ++i) {
            if (snap != -1 && snap < i->first) continue;
            std::map<std::string, ObjectDesc>::iterator j = i->second.find(oid);
            if (j != i->second.end()) {
                if (version) j->second.version = version;
                std::cout << __func__ << " oid " << oid << " v " << version << " " << j->second.most_recent() << " "
                          << (j->second.dirty ? "dirty" : "clean") << " " << (j->second.exists ? "exists" : "dne")
                          << std::endl;
                break;
            }
        }
    ...
    {
        for (auto i = pool_obj_cont.crbegin(); i != pool_obj_cont.crend(); ++i) {
            if (snap != -1 && snap < i->first) continue;
    ...
    bool check_oldest_snap_flushed(const std::string& oid, int snap)
    {
        for (auto i = pool_obj_cont.rbegin(); i != pool_obj_cont.rend(); ++i) {
            ...
            if (j != i->second.end() && !j->second.flushed) {
                std::cout << __func__ << " oid " << oid << " v " << j->second.version << " " << j->second.most_recent()
                          << " " << (j->second.flushed ? "flushed" : "unflushed") << " " << i->first << std::endl;
                return false;
            }
        }
        return true;
    ...
    bool check_chunks_refcount(librados::IoCtx& chunk_pool_ctx, librados::IoCtx& manifest_pool_ctx)
    {
        ...
        chunk_pool_ctx.object_list_slice(begin, end, 1, 1, &shard_start, &shard_end);
        ...
        while (c < shard_end) {
            ...
            for (const auto& i : result) {
                ...
                chunk_refs_by_object_t* byo = static_cast<chunk_refs_by_object_t*>(refs.r.get());
                ...
                for (auto& pp : byo->by_object) {
                    ...
                    if (src_refcount > dst_refcount) {
                        std::cerr << " src_object " << pp << ": src_refcount " << src_refcount << ", dst_object " << oid
                                  << ": dst_refcount " << dst_refcount << std::endl;
                        return false;
                    }
                }
            }
...
public:
    ...
    RemoveAttrsOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat)
    ...
    void _begin() override
    {
        ...
        std::set<std::string> to_remove;
        {
            ...
            cont = ContDesc(context->seq_num, context->current_snap, context->seq_num, "");
            context->oid_in_use.insert(oid);
            context->oid_not_in_use.erase(oid);

            if (rand() % 30) {
                ContentsGenerator::iterator iter = context->attr_gen.get_iterator(cont);
                for (auto i = obj.attrs.begin(); i != obj.attrs.end(); ++i, ++iter) {
                    if (!(*iter % 3)) {
                ...
                for (auto i = obj.attrs.begin(); i != obj.attrs.end(); ++i) {
                    op.rmxattr(i->first.c_str());
                    to_remove.insert(i->first);
                }
        ...
        std::pair<TestOp*, TestOp::CallbackInfo*>* cb_arg =
            new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(0));
    ...
    std::string getType() override { return "RemoveAttrsOp"; }
};
...
public:
    ...
    librados::ObjectWriteOperation op;
    librados::AioCompletion* comp;
    SetAttrsOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat)
        : TestOp(n, context, stat)
        , oid(oid)
        , comp(NULL)
    {}
    ...
    void _begin() override
    {
        ...
        while (!*keygen) ++keygen;
        while (*keygen) {
            if (*keygen != '_') header.append(*keygen);
            ++keygen;
        }
        for (int i = 0; i < 20; ++i) {
            std::string key;
    ...
    std::string getType() override { return "SetAttrsOp"; }
};
...
public:
    ...
    WriteOp(int n, RadosTestContext* context, const std::string& oid, bool do_append, bool do_excl,
            TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , oid(oid)
        , do_append(do_append)
    ...
    void _begin() override
    {
        ...
        if (do_append) {
            ...
            cont_gen =
                new AppendGenerator(prev_length, alignment, context->min_stride_size, context->max_stride_size, 3);
        }
        else {
            cont_gen = new VarLenGenerator(context->max_size, context->min_stride_size, context->max_stride_size);
        }
        ...
        for (auto [offset, len] : ranges) {
            ...
            std::cout << num << ":  writing " << context->prefix + oid << " from " << offset << " to " << len + offset
                      << " tid " << tid << std::endl;
            auto cb_arg = new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(tid++));
            librados::AioCompletion* completion = context->rados.aio_create_completion((void*)cb_arg, &write_callback);
            ...
            if (do_excl && cb_arg->second->id == last_acked_tid + 1) op.assert_exists();
            context->io_ctx.aio_operate(context->prefix + oid, completion, &op);
        }

        bufferlist contbl;
        encode(cont, contbl);
        std::pair<TestOp*, TestOp::CallbackInfo*>* cb_arg =
            new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(tid++));
        librados::AioCompletion* completion = context->rados.aio_create_completion((void*)cb_arg, &write_callback);
        waiting.insert(completion);
        waiting_on++;
        ...
        context->io_ctx.aio_operate(context->prefix + oid, completion, &write_op);

        cb_arg = new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(tid++));
        rcompletion = context->rados.aio_create_completion((void*)cb_arg, &write_callback);
        waiting_on++;
        read_op.read(0, 1, &rbuffer, 0);
        context->io_ctx.aio_operate(context->prefix + oid,
                                    rcompletion,
                                    &read_op,
    ...
    void _finish(CallbackInfo* info) override
    {
        ...
        if (tid <= last_acked_tid) {
            std::cerr << "Error: finished tid " << tid << " when last_acked_tid was " << last_acked_tid << std::endl;
            ceph_abort();
        }
        last_acked_tid = tid;
        ...
        if (waiting_on == 0) {
            ...
            for (auto i = waiting.begin(); i != waiting.end();) {
                ...
                if (int err = (*i)->get_return_value()) {
                    std::cerr << "Error: oid " << oid << " write returned error code " << err << std::endl;
                    ceph_abort();
                }
                if ((*i)->get_version64() > version) {
                    std::cout << num << ":  oid " << oid << " updating version " << version << " to "
                              << (*i)->get_version64() << std::endl;
                    version = (*i)->get_version64();
                }
                else {
                    std::cout << num << ":  oid " << oid << " version " << version << " is already newer than "
                              << (*i)->get_version64() << std::endl;
                }
                (*i)->release();
                waiting.erase(i++);
            ...
            if (rcompletion->get_version64() != version) {
                std::cerr << "Error: racing read on " << oid << " returned version " << rcompletion->get_version64()
                          << " rather than version " << version << std::endl;
...

            {
                ...
                if (old_value.deleted())
                    ...
                    std::cout << num << ":  left oid " << oid << " " << old_value.most_recent() << std::endl;
            }
    ...
    std::string getType() override { return "WriteOp"; }
};
...
public:
    ...
    WriteSameOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , oid(oid)
        , rcompletion(NULL)
        , waiting_on(0)
    ...
    void _begin() override
    {
        ...
        for (auto [offset, len] : ranges) {
            ...
            ceph_assert(to_write.length() == len);
            ceph_assert(to_write.length() > 0);
            std::cout << num << ":  writing " << context->prefix + oid << " from " << offset << " to " << offset + len
                      << " tid " << tid << std::endl;
            auto cb_arg = new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(tid++));
            librados::AioCompletion* completion = context->rados.aio_create_completion((void*)cb_arg, &write_callback);
            waiting.insert(completion);
    ...
    void _finish(CallbackInfo* info) override
    {
        ...
        if (waiting_on == 0) {
            ...
            for (auto i = waiting.begin(); i != waiting.end();) {
                ceph_assert((*i)->is_complete());
                if (int err = (*i)->get_return_value()) {
                    std::cerr << "Error: oid " << oid << " writesame returned error code " << err << std::endl;
                    ceph_abort();
                }
                if ((*i)->get_version64() > version) {
                    std::cout << "oid " << oid << "updating version " << version << "to " << (*i)->get_version64()
                              << std::endl;
                    version = (*i)->get_version64();
                }
                else {
                    std::cout << "oid " << oid << "version " << version << "is already newer than "
                              << (*i)->get_version64() << std::endl;
                }
                (*i)->release();
                waiting.erase(i++);
    ...
    std::string getType() override { return "WriteSameOp"; }
};
...
public:
    ...
    DeleteOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , oid(oid)
    {}
    ...
    std::string getType() override { return "DeleteOp"; }
};
...
public:
    ...
    ReadOp(int n, RadosTestContext* context, const std::string& oid, bool balance_reads, bool localize_reads,
           TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , completions(3)
        , oid(oid)
    ...
    {
        uint64_t len = 0;
        if (old_value.has_contents()) len = old_value.most_recent_gen()->get_length(old_value.most_recent());
        if (context->no_sparse || rand() % 2) {
            is_sparse_read[index] = false;
            read_op.read(0, len, &results[index], &retvals[index]);
            bufferlist init_value_bl;
            encode(static_cast<uint32_t>(-1), init_value_bl);
            read_op.checksum(
                LIBRADOS_CHECKSUM_TYPE_CRC32C, init_value_bl, 0, len, 0, &checksums[index], &checksum_retvals[index]);
        }
        else {
            is_sparse_read[index] = true;
            read_op.sparse_read(0, len, &extent_results[index], &results[index], &retvals[index]);
        }
    }
    ...
    void _begin() override
    {
        ...
        for (auto i = old_value.attrs.begin(); i != old_value.attrs.end(); ++i) {
            if (rand() % 2) {
                ...
                if (rand() % 2) key.push_back((rand() % 26) + 'a');
        ...
        unsigned flags = 0;
        if (balance_reads) flags |= librados::OPERATION_BALANCE_READS;
        if (localize_reads) flags |= librados::OPERATION_LOCALIZE_READS;

        ceph_assert(!context->io_ctx.aio_operate(context->prefix + oid, completions[0], &op, flags, NULL));
        waiting_on++;
    ...
    void _finish(CallbackInfo* info) override
    {
        ...
        for (auto it = completions.begin(); it != completions.end(); ++it) {
            ...
            if (err != retval) {
                std::cerr << num << ": Error: oid " << oid << " read returned different error codes: " << retval
                          << " and " << err << std::endl;
                ceph_abort();
            }
            if (err) {
                if (!(err == -ENOENT && old_value.deleted())) {
                    std::cerr << num << ": Error: oid " << oid << " read returned error code " << err << std::endl;
                    ceph_abort();
                }
            }
            else if (version != old_value.version) {
                std::cerr << num << ": oid " << oid << " version is " << version << " and expected "
                          << old_value.version << std::endl;
        ...
        if (!retval) {
            ...
            if (iter == xattrs.end()) {
                if (old_value.has_contents()) {
                    std::cerr << num << ": Error: did not find header attr, has_contents: " << old_value.has_contents()
                              << std::endl;
                    ...
                    if (is_sparse_read[i]) {
                        ...
                        if (checksum_retvals[i] != 0 || checksum != results[i].crc32c(-1)) {
                            std::cerr << num << ": oid " << oid << " checksum " << checksums[i]
                                      << " incorrect, expecting " << results[i].crc32c(-1) << std::endl;
                            context->errors++;
                        }
                    }
                }
            ...
            if (!context->no_omap) {
                if (!(old_value.header == header)) {
                    std::cerr << num << ": oid " << oid
                              << " header does not match, old size: " << old_value.header.length() << " new size "
                              << header.length() << std::endl;
                    ceph_assert(old_value.header == header);
                }
                if (omap.size() != old_value.attrs.size()) {
                    std::cerr << num << ": oid " << oid << " omap.size() is " << omap.size() << " and old is "
                              << old_value.attrs.size() << std::endl;
                    ceph_assert(omap.size() == old_value.attrs.size());
                }
                if (omap_keys.size() != old_value.attrs.size()) {
                    std::cerr << num << ": oid " << oid << " omap.size() is " << omap_keys.size() << " and old is "
                              << old_value.attrs.size() << std::endl;
                    ceph_assert(omap_keys.size() == old_value.attrs.size());
                }
            }
            if (xattrs.size() != old_value.attrs.size()) {
                std::cerr << num << ": oid " << oid << " xattrs.size() is " << xattrs.size() << " and old is "
                          << old_value.attrs.size() << std::endl;
                ceph_assert(xattrs.size() == old_value.attrs.size());
            }
            for (auto iter = old_value.attrs.begin(); iter != old_value.attrs.end(); ++iter) {
                bufferlist bl = context->attr_gen.gen_bl(iter->second);
                if (!context->no_omap) {
                    ...
                    for (bufferlist::iterator l = omap_iter->second.begin(); !k.end() && !l.end(); ++k, ++l) {
                        ceph_assert(*l == *k);
                    }
                }
                ...
                bufferlist::iterator k = bl.begin();
                for (bufferlist::iterator j = xattr_iter->second.begin(); !k.end() && !j.end(); ++j, ++k) {
                    ceph_assert(*j == *k);
                }
            }
            if (!context->no_omap) {
                for (std::set<std::string>::iterator i = omap_requested_keys.begin(); i != omap_requested_keys.end();
                     ++i) {
                    if (!omap_returned_values.count(*i)) ceph_assert(!old_value.attrs.count(*i));
                    if (!old_value.attrs.count(*i)) ceph_assert(!omap_returned_values.count(*i));
                }
                for (auto i = omap_returned_values.begin(); i != omap_returned_values.end(); ++i) {
    ...
    std::string getType() override { return "ReadOp"; }
};

class SnapCreateOp : public TestOp
{
public:
    SnapCreateOp(int n, RadosTestContext* context, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
    {}
    ...
    void _begin() override
    {
        ...
        if (!context->pool_snaps) {
            ...
            int j = 0;
            for (auto i = context->snaps.rbegin(); i != context->snaps.rend(); ++i, ++j) {
                snapset[j] = i->second;
            }
    ...
    std::string getType() override { return "SnapCreateOp"; }
...
public:
    int to_remove;
    SnapRemoveOp(int n, RadosTestContext* context, int snap, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , to_remove(snap)
    {}
    ...
    std::string getType() override { return "SnapRemoveOp"; }
};
...
public:
    WatchOp(int n, RadosTestContext* context, const std::string& _oid, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , oid(_oid)
    {}
    ...
    void _begin() override
    {
        ...
        if (!ctx) {
            ...
            r = context->io_ctx.watch2(context->prefix + oid, &ctx->get_handle(), ctx);
        }
        else {
    ...
    std::string getType() override { return "WatchOp"; }
};
...
public:
    ...
    RollbackOp(int n, RadosTestContext* context, const std::string& _oid, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , oid(_oid)
        , roll_back_to(-1)
        , comps(3, NULL)
    ...
    void _begin() override
    {
        ...
        in_use = context->snaps_in_use.lookup_or_create(roll_back_to, roll_back_to);
        ...
        if (existed_before) {
            std::pair<TestOp*, TestOp::CallbackInfo*>* cb_arg =
                new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(0));
            comps[0] = context->rados.aio_create_completion((void*)cb_arg, &write_callback);
            context->io_ctx.aio_operate(context->prefix + oid, comps[0], &zero_write_op1);
        }
        {
            std::pair<TestOp*, TestOp::CallbackInfo*>* cb_arg =
                new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(1));
            comps[1] = context->rados.aio_create_completion((void*)cb_arg, &write_callback);
            context->io_ctx.aio_operate(context->prefix + oid, comps[1], &op);
        }
        if (existed_after) {
            std::pair<TestOp*, TestOp::CallbackInfo*>* cb_arg =
                new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(2));
            comps[2] = context->rados.aio_create_completion((void*)cb_arg, &write_callback);
            context->io_ctx.aio_operate(context->prefix + oid, comps[2], &zero_write_op2);
        }
    }
    ...
    void _finish(CallbackInfo* info) override
    {
        std::lock_guard l{context->state_lock};
        uint64_t tid = info->id;
        std::cout << num << ":  finishing rollback tid " << tid << " to " << context->prefix + oid << std::endl;
    ...
    std::string getType() override { return "RollBackOp"; }
};
...
public:
    ...
    uint64_t version;
    int r;
    CopyFromOp(int n, RadosTestContext* context, const std::string& oid, const std::string& oid_src, TestOpStat* stat)
        : TestOp(n, context, stat)
        , oid(oid)
        , oid_src(oid_src)
        , comp(NULL)
    ...
    void _begin() override
    {
        ContDesc cont;
        {
            ...
            if (!src_value.deleted()) context->update_object_full(oid, src_value);
        }
        ...
        if (info->id == 0) {
            ...
            if ((r = comp->get_return_value())) {
                if (r == -ENOENT && src_value.deleted()) {
                    ...
                    std::cerr << "Error: oid " << oid << " copy_from " << oid_src << " returned error code " << r
                              << std::endl;
                    ceph_abort();
                }
            }
    ...
    bool finished() override { return done == 2; }

    std::string getType() override { return "CopyFromOp"; }
};
...
public:
    ...
    ChunkReadOp(int n, RadosTestContext* context, const std::string& oid, const std::string& tgt_pool_name,
                bool balance_reads, bool localize_reads, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , completions(2)
        , oid(oid)
        , snap(0)
    ...
    {
        read_op.read(offset, length, &results[index], &retvals[index]);
        if (index != 0) {
            bufferlist init_value_bl;
            encode(static_cast<uint32_t>(-1), init_value_bl);
            read_op.checksum(LIBRADOS_CHECKSUM_TYPE_CRC32C,
                             init_value_bl,
                             offset,
                             length,
                             0,
                             &checksums[index],
                             &checksum_retvals[index]);
        }
    }
    ...
    void _begin() override
    {
        ...
        std::cout << num << ": ori offset " << iter->first << " req offset " << offset << " ori length "
                  << iter->second.length << " req length " << length << " ori tgt_offset " << iter->second.offset
                  << " req tgt_offset " << tgt_offset << " tgt_oid " << tgt_oid << std::endl;
        ...
        ceph_assert(!context->io_ctx.aio_operate(context->prefix + tgt_oid, completions[1], &op, flags, NULL));
    ...
    void _finish(CallbackInfo* info) override
    {
        ...
        if (!retval) {
            ...
            if (tgt_value.has_contents()) {
                ...
                if (checksum_retvals[1] != 0) {
                    std::cerr << num << ": oid " << oid << " checksum retvals " << checksums[0] << " error "
                              << std::endl;
                    context->errors++;
                }
                ...
                if (checksum[0] != checksum[1]) {
                    std::cerr << num << ": oid " << oid << " checksum src " << checksum[0] << " chunksum tgt "
                              << checksum[1] << " incorrect, expecting " << results[0].crc32c(-1) << std::endl;
                    context->errors++;
                }
                if (context->errors) ceph_abort();
            }
    ...
    std::string getType() override { return "ChunkReadOp"; }
};
...
public:
    ...
    int done;
    int r;
    CopyOp(int n, RadosTestContext* context, const std::string& oid_src, const std::string& oid,
           const std::string& tgt_pool_name, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , oid(oid)
        , oid_src(oid_src)
        , tgt_pool_name(tgt_pool_name)
    ...
    void _finish(CallbackInfo* info) override
    {
        ...
        if (info->id == 0) {
            ...
            if ((r = comp->get_return_value())) {
                std::cerr << "Error: oid " << oid << " write returned error code " << r << std::endl;
                ceph_abort();
            }
        }
    ...
    bool finished() override { return done == 1; }

    std::string getType() override { return "CopyOp"; }
};
...
public:
    ...
    int snap;
    std::shared_ptr<int> in_use;
    SetChunkOp(int n, RadosTestContext* context, const std::string& oid, const std::string& oid_tgt,
               TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , oid(oid)
        , oid_tgt(oid_tgt)
    ...
    void _begin() override
    {
        ...
        std::cout << num << ": "
                  << "set_chunk oid " << oid << " offset: " << offset << " length: " << length << " target oid "
                  << target_oid << " offset: " << tgt_offset << " snap " << snap << std::endl;

        op.set_chunk(offset, length, context->low_tier_io_ctx, target_oid, tgt_offset, CEPH_OSD_OP_FLAG_WITH_REFERENCE);

        std::pair<TestOp*, TestOp::CallbackInfo*>* cb_arg =
            new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(0));
        comp = context->rados.aio_create_completion((void*)cb_arg, &write_callback);
        context->io_ctx.aio_operate(context->prefix + oid, comp, &op, librados::OPERATION_ORDER_READS_WRITES, NULL);
        if (snap >= 0) {
            context->io_ctx.snap_set_read(0);
        }
    }
            ...
            if ((r = comp->get_return_value())) {
                if (r == -ENOENT && src_value.deleted()) {
                    std::cout << num << ":  got expected ENOENT (src dne)" << std::endl;
                         ...
                         context->oid_set_chunk_tgt_pool.find(oid_tgt) != context->oid_set_chunk_tgt_pool.end()) {
                    ...
                    std::cout << "Range is overlapped: oid " << oid << " set_chunk " << oid_tgt
                              << " returned error code " << r << " offset: " << offset << " length: " << length
                              << std::endl;
                    context->update_object_version(oid, comp->get_version64());
                }
                else {
                    std::cerr << "Error: oid " << oid << " set_chunk " << oid_tgt << " returned error code " << r
                              << std::endl;
                    ceph_abort();
                }
            }
    ...
    std::string getType() override { return "SetChunkOp"; }
};
...
public:
    ...
    int done;
    int r;
    SetRedirectOp(int n, RadosTestContext* context, const std::string& oid, const std::string& oid_tgt,
                  const std::string& tgt_pool_name, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , oid(oid)
        , oid_tgt(oid_tgt)
    ...
    void _begin() override
    {
        ...
        if (!context->redirect_objs[oid].empty()) {
            ...
            op.copy_from(src.c_str(), context->io_ctx, src_value.version, 0);
            context->low_tier_io_ctx.aio_operate(
                context->prefix + oid_tgt, comp, &op, librados::OPERATION_ORDER_READS_WRITES);
            comp->wait_for_complete();
            if ((r = comp->get_return_value())) {
                std::cerr << "Error: oid " << oid << " copy_from " << oid_tgt << " returned error code " << r
                          << std::endl;
                ceph_abort();
            }
            comp->release();
            ...
            context->io_ctx.aio_operate(context->prefix + oid,
                                        comp,
                                        &op,
                                        librados::OPERATION_ORDER_READS_WRITES | librados::OPERATION_IGNORE_REDIRECT);
            comp->wait_for_complete();
            if ((r = comp->get_return_value())) {
                if (!(r == -ENOENT && !present) && r != -EOPNOTSUPP) {
                    std::cerr << "r is " << r << " while deleting " << oid << " and present is " << present
                              << std::endl;
                    ceph_abort();
                }
            }
        ...
        if ((r = comp->get_return_value()) && !src_value.deleted()) {
            std::cerr << "Error: oid " << oid << " stat returned error code " << r << std::endl;
            ceph_abort();
        }
        context->update_object_version(oid, comp->get_version64());
        comp->release();
        ...
        comp->wait_for_complete();
        if ((r = comp->get_return_value())) {
            std::cerr << "Error: oid " << oid_tgt << " stat returned error code " << r << std::endl;
            ceph_abort();
        }
        ...
        if (src_value.version != 0 && !src_value.deleted()) op.assert_version(src_value.version);
        op.set_redirect(context->prefix + oid_tgt, context->low_tier_io_ctx, tgt_version);

        std::pair<TestOp*, TestOp::CallbackInfo*>* cb_arg =
            new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(0));
        comp = context->rados.aio_create_completion((void*)cb_arg, &write_callback);
        context->io_ctx.aio_operate(context->prefix + oid, comp, &op, librados::OPERATION_ORDER_READS_WRITES);
    }
        ...
        if (info->id == 0) {
            ...
            if ((r = comp->get_return_value())) {
                if (r == -ENOENT && src_value.deleted()) {
                    ...
                    std::cerr << "Error: oid " << oid << " set_redirect " << oid_tgt << " returned error code " << r
                              << std::endl;
                    ceph_abort();
                }
            }
    ...
    std::string getType() override { return "SetRedirectOp"; }
};
...
public:
    ...
    UnsetRedirectOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , oid(oid)
    {}
    ...
    std::string getType() override { return "UnsetRedirectOp"; }
};
...
public:
    ...
    TierPromoteOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat)
        : TestOp(n, context, stat)
        , completion(NULL)
        , oid(oid)
    {}
    ...
    void _begin() override
    {
        ...
        std::pair<TestOp*, TestOp::CallbackInfo*>* cb_arg =
            new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(0));
        completion = context->rados.aio_create_completion((void*)cb_arg, &write_callback);
        context->state_lock.unlock();

        op.tier_promote();
        int r = context->io_ctx.aio_operate(context->prefix + oid, completion, &op);
        ceph_assert(!r);
    }
    ...
    std::string getType() override { return "TierPromoteOp"; }
};
...
public:
    ...
    TierFlushOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat)
        : TestOp(n, context, stat)
        , completion(NULL)
        , oid(oid)
        , snap(-1)
    ...
    void _begin() override
    {
        ...
        int r = context->io_ctx.aio_operate(context->prefix + oid, completion, &op, flags, NULL);
        ceph_assert(!r);
    ...
    std::string getType() override { return "TierFlushOp"; }
};
...
public:
    ...
    TierEvictOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat)
        : TestOp(n, context, stat)
        , completion(NULL)
        , oid(oid)
        , snap(-1)
    ...
    void _begin() override
    {
        ...
        op.cache_evict();
        int r =
            context->io_ctx.aio_operate(context->prefix + oid, completion, &op, librados::OPERATION_IGNORE_CACHE, NULL);
        ceph_assert(!r);
    ...
    std::string getType() override { return "TierEvictOp"; }
};
...
public:
    HitSetListOp(int n, RadosTestContext* context, uint32_t hash, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , comp1(NULL)
        , comp2(NULL)
        , hash(hash)
    ...
    void _finish(CallbackInfo* info) override
    {
        ...
        if (!comp2) {
            if (ls.empty()) {
                ...
                auto p = ls.begin();
                while (r--) ++p;
                auto cb_arg = new std::pair<TestOp*, TestOp::CallbackInfo*>(this, new TestOp::CallbackInfo(0));
            ...
            if (r == 0) {
                ...
                auto p = bl.cbegin();
                decode(hitset, p);
                std::cout << num << ": got hitset of type " << hitset.get_type_name() << " size " << bl.length()
                          << std::endl;
            }
            else {
    ...
    std::string getType() override { return "HitSetListOp"; }
};
...
public:
    ...
    UndirtyOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , completion(NULL)
        , oid(oid)
    {}
    ...
    void _begin() override
    {
        ...
        op.undirty();
        int r = context->io_ctx.aio_operate(context->prefix + oid, completion, &op, 0);
        ceph_assert(!r);
    }
    ...
    std::string getType() override { return "UndirtyOp"; }
};
...
public:
    ...
    IsDirtyOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat = 0)
        : TestOp(n, context, stat)
        , completion(NULL)
        , oid(oid)
        , dirty(false)
    ...
    void _begin() override
    {
        ...
        std::cout << num << ": is_dirty oid " << oid << " snap " << snap << std::endl;
    ...
    std::string getType() override { return "IsDirtyOp"; }
};
...
public:
    ...
    CacheFlushOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat, bool b)
        : TestOp(n, context, stat)
        , completion(NULL)
        , oid(oid)
        , blocking(b)
    ...
    void _begin() override
    {
        ...
        can_fail = true;
        std::cout << num << ": " << (blocking ? "cache_flush" : "cache_try_flush") << " oid " << oid << " snap " << snap
                  << std::endl;
    ...
    std::string getType() override { return "CacheFlushOp"; }
};
...
public:
    ...
    CacheEvictOp(int n, RadosTestContext* context, const std::string& oid, TestOpStat* stat)
        : TestOp(n, context, stat)
        , completion(NULL)
        , oid(oid)
    {}
    ...
    std::string getType() override { return "CacheEvictOp"; }
};
