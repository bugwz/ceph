...
TEST(pgtransaction, simple)
{
    ...
    unsigned num = 0;
    t.safe_create_traverse([&](const pair<const hobject_t, PGTransaction::ObjectOperation>& p) {
        ASSERT_EQ(p.first, h);
...
TEST(pgtransaction, clone_safe_create_traverse)
{
    ...
    t.safe_create_traverse([&](const pair<const hobject_t, PGTransaction::ObjectOperation>& p) {
        ...
        if (num == 0) {
            ...
            ASSERT_EQ(boost::get<T::Clone>(&p.second.init_type)->source, h2);
        }
        else if (num == 1) {
...
TEST(pgtransaction, clone_safe_create_traverse2)
{
    ...
    t.safe_create_traverse([&](const pair<const hobject_t, PGTransaction::ObjectOperation>& p) {
        ...
        if (num == 0) {
            ...
            ASSERT_EQ(p.first, h2);
            ASSERT_TRUE(boost::get<T::Clone>(&p.second.init_type));
            ASSERT_EQ(boost::get<T::Clone>(&p.second.init_type)->source, h3);
        }
        else if (num == 2) {
