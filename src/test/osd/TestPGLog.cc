...
struct PGLogTestBase
{
    ...
    static eversion_t mk_evt(unsigned ep, unsigned v) { return eversion_t(ep, v); }
    static pg_log_entry_t mk_ple_mod(const hobject_t& hoid, eversion_t v, eversion_t pv, osd_reqid_t reqid)
    {
        pg_log_entry_t e;
    ...
    static pg_log_entry_t mk_ple_dt(const hobject_t& hoid, eversion_t v, eversion_t pv, osd_reqid_t reqid)
    {
        pg_log_entry_t e;
        e.mark_unrollbackable();
    ...
    static pg_log_entry_t mk_ple_ldt(const hobject_t& hoid, eversion_t v, eversion_t pv)
    {
        pg_log_entry_t e;
        e.mark_unrollbackable();
    ...
    static pg_log_entry_t mk_ple_mod_rb(const hobject_t& hoid, eversion_t v, eversion_t pv, osd_reqid_t reqid)
    {
        pg_log_entry_t e;
        e.op = pg_log_entry_t::MODIFY;
        e.soid = hoid;
    ...
    static pg_log_entry_t mk_ple_dt_rb(const hobject_t& hoid, eversion_t v, eversion_t pv, osd_reqid_t reqid)
    {
        pg_log_entry_t e;
        e.op = pg_log_entry_t::DELETE;
        e.soid = hoid;
    ...
    static pg_log_entry_t mk_ple_err(const hobject_t& hoid, eversion_t v, osd_reqid_t reqid)
    {
        pg_log_entry_t e;
    ...
    static pg_log_entry_t mk_ple_mod(const hobject_t& hoid, eversion_t v, eversion_t pv)
    {
        return mk_ple_mod(hoid, v, pv, osd_reqid_t());
    }
    static pg_log_entry_t mk_ple_dt(const hobject_t& hoid, eversion_t v, eversion_t pv)
    {
        return mk_ple_dt(hoid, v, pv, osd_reqid_t());
    }
    static pg_log_entry_t mk_ple_mod_rb(const hobject_t& hoid, eversion_t v, eversion_t pv)
    {
        return mk_ple_mod_rb(hoid, v, pv, osd_reqid_t());
    }
    static pg_log_entry_t mk_ple_dt_rb(const hobject_t& hoid, eversion_t v, eversion_t pv)
    {
        return mk_ple_dt_rb(hoid, v, pv, osd_reqid_t());
    }
    static pg_log_entry_t mk_ple_err(const hobject_t& hoid, eversion_t v) { return mk_ple_err(hoid, v, osd_reqid_t()); }
};   // PGLogTestBase
...
public:
    PGLogTest()
        : PGLog(g_ceph_context)
    {}
    void SetUp() override { missing.may_include_deletes = true; }
...
#include "common/config.h"
    ...
    public:
        ...
        void setup()
        {
            ...
            if (init.get_items().empty()) {
                ...
                for (list<pg_log_entry_t>::const_iterator i = fulldiv.log.begin(); i != fulldiv.log.end(); ++i) {
    ...
    struct LogHandler : public PGLog::LogEntryHandler
    {
        ...
        void rollback(const pg_log_entry_t& entry) override { rolledback.push_back(entry); }
        void rollforward(const pg_log_entry_t& entry) override {}
        void remove(const hobject_t& hoid) override { removed.insert(hoid); }
    ...
    template<typename missing_t> void verify_missing(const TestCase& tcase, const missing_t& missing)
    {
    ...
    void test_merge_log(const TestCase& tcase)
    {
        ...
        merge_log(oinfo, std::move(olog), pg_shard_t(1, shard_id_t(0)), info, &h, dirty_info, dirty_big_info);
    ...
    void test_proc_replica_log(const TestCase& tcase)
    {
        ...
        for (list<pg_log_entry_t>::const_iterator i = tcase.auth.begin(); i != tcase.auth.end(); ++i) {
...
struct TestHandler : public PGLog::LogEntryHandler
{
    ...
    explicit TestHandler(list<hobject_t>& removed)
        : removed(removed)
    {}

    void rollback(const pg_log_entry_t& entry) override {}
    void rollforward(const pg_log_entry_t& entry) override {}
    void remove(const hobject_t& hoid) override { removed.push_back(hoid); }
...
TEST_F(PGLogTest, rewind_divergent_log)
{
    ...
    */
    {
        ...
        rewind_divergent_log(newhead, info, &h, dirty_info, dirty_big_info);
    ...
    */
    ...
    // Test for 13965
    {
        ...
        rewind_divergent_log(eversion_t(1, 5), info, &h, dirty_info, dirty_big_info);
        pg_log_t log;
...
TEST_F(PGLogTest, merge_log)
{
    ...
    // noop
    {
        ...
        TestHandler h(remove_snap);
        merge_log(oinfo, std::move(olog), fromosd, info, &h, dirty_info, dirty_big_info);
...
TEST_F(PGLogTest, merge_log_2)
{
    ...
    t.torollback.insert(t.torollback.begin(), t.div.rbegin(), t.div.rend());
...
{
    {
        ...
        // Some should be removed
        {
            ...
            pg_log_t::filter_log(pgid, *osdmap, hit_set_namespace, log, filtered, reject);
        ...
        int count = 0;
        for (list<pg_log_entry_t>::iterator i = log.log.begin(); i != log.log.end(); ++i) {
...
TEST_F(PGLogTest, get_request)
{
    ...
    entries.push_back(pg_log_entry_t(pg_log_entry_t::ERROR,
                                     oid,
                                     eversion_t(6, 2),
                                     eversion_t(3, 4),
                                     1,
                                     osd_reqid_t(entity_name_t::CLIENT(777), 8, 1),
                                     utime_t(0, 1),
                                     -ENOENT));
    entries.push_back(pg_log_entry_t(pg_log_entry_t::MODIFY,
                                     oid,
                                     eversion_t(6, 3),
                                     eversion_t(3, 4),
                                     2,
                                     ...
                                     utime_t(1, 2),
                                     0));
    entries.push_back(pg_log_entry_t(pg_log_entry_t::DELETE,
                                     oid,
                                     eversion_t(7, 4),
                                     eversion_t(7, 4),
                                     3,
    ...
    entries.push_back(pg_log_entry_t(pg_log_entry_t::ERROR,
                                     ...
                                     eversion_t(7, 4),
                                     3,
                                     osd_reqid_t(entity_name_t::CLIENT(777), 8, 4),
                                     utime_t(20, 1),
                                     -ENOENT));
    ...
    for (auto& entry : entries) {
        ...
        bool got = log.get_request(entry.reqid, &replay_version, &user_version, &return_code, &op_returns);
        EXPECT_TRUE(got);
...
TEST_F(PGLogTest, ErrorNotIndexedByObject)
{
    ...
    // make sure writes, deletes, and errors are found
    hobject_t oid(object_t("objname"), "key", 123, 456, 0, "");
    log.add(pg_log_entry_t(pg_log_entry_t::ERROR,
                           oid,
                           eversion_t(6, 2),
                           eversion_t(3, 4),
                           1,
    ...
    pg_log_entry_t modify(pg_log_entry_t::MODIFY,
                          oid,
                          eversion_t(6, 3),
                          eversion_t(3, 4),
                          2,
    ...
    pg_log_entry_t del(pg_log_entry_t::DELETE,
                       oid,
                       eversion_t(7, 4),
                       eversion_t(7, 4),
                       3,
...
public:
    ...
    static pg_log_dup_t create_dup_entry(uint a, uint b)
    {
        ...
        return pg_log_dup_t(eversion_t(a, b), a, osd_reqid_t(entity_name_t::CLIENT(client_id++), 8, 1), 0);
    }

    static std::vector<pg_log_dup_t> example_dups_1()
    {
        std::vector<pg_log_dup_t> result = {create_dup_entry(10, 11),
    ...
    {
        std::vector<pg_log_dup_t> result = {create_dup_entry(12, 3),
                                            create_dup_entry(13, 99),
    ...
    void test_disk_roundtrip()
    {
        ...
        clear();
        ostringstream err;
        read_log_and_missing(store.get(), ch, log_oid, pg_info_t(), err, false);
...
struct PGLogTrimTest : public ::testing::Test, public PGLogTestBase, public PGLog::IndexedLog
{
...
TEST_F(PGLogTrimTest, TestGetRequest)
{
    ...
    log.add(mk_ple_mod(mk_obj(1), mk_evt(10, 100), mk_evt(8, 70), osd_reqid_t(client, 8, 1)));
    log.add(mk_ple_dt(mk_obj(2), mk_evt(15, 150), mk_evt(10, 100), osd_reqid_t(client, 8, 2)));
    log.add(mk_ple_mod_rb(mk_obj(3), mk_evt(15, 155), mk_evt(15, 150), osd_reqid_t(client, 8, 3)));
    log.add(mk_ple_mod(mk_obj(1), mk_evt(20, 160), mk_evt(25, 152), osd_reqid_t(client, 8, 4)));
    log.add(mk_ple_mod(mk_obj(4), mk_evt(21, 165), mk_evt(26, 160), osd_reqid_t(client, 8, 5)));
    log.add(mk_ple_dt_rb(mk_obj(5), mk_evt(21, 167), mk_evt(31, 166), osd_reqid_t(client, 8, 6)));
    ...
    result = log.get_request(log_reqid, &version, &user_version, &return_code, &op_returns);
    EXPECT_EQ(true, result);
    EXPECT_EQ(mk_evt(21, 165), version);

    result = log.get_request(dup_reqid, &version, &user_version, &return_code, &op_returns);
    EXPECT_EQ(true, result);
    EXPECT_EQ(mk_evt(15, 155), version);

    result = log.get_request(bad_reqid, &version, &user_version, &return_code, &op_returns);
    EXPECT_FALSE(result);
}
...
{
    {
        ...
        _merge_object_divergent_entries(
            log, hoid, orig_entries, oinfo, log.get_can_rollback_to(), missing, &rollbacker, this);
        // No core dump
    }
    {
...
{
    pg_log_dup_t a(eversion_t(1234, 5678), 13, osd_reqid_t(entity_name_t::CLIENT(777), 8, 999), 15);
    std::string a_key_name = a.get_key_name();
...
TEST_F(PGLogTrimTest, TestTrimDups)
{
    ...
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(9, 99), mk_evt(8, 98), osd_reqid_t(client, 8, 1))));

    log.add(mk_ple_mod(mk_obj(1), mk_evt(10, 100), mk_evt(9, 99), osd_reqid_t(client, 8, 1)));
    log.add(mk_ple_dt(mk_obj(2), mk_evt(15, 101), mk_evt(10, 100), osd_reqid_t(client, 8, 2)));
    log.add(mk_ple_mod_rb(mk_obj(3), mk_evt(15, 102), mk_evt(15, 101), osd_reqid_t(client, 8, 3)));
    log.add(mk_ple_mod(mk_obj(1), mk_evt(20, 103), mk_evt(15, 102), osd_reqid_t(client, 8, 4)));
    log.add(mk_ple_mod(mk_obj(4), mk_evt(21, 104), mk_evt(20, 103), osd_reqid_t(client, 8, 5)));
    log.add(mk_ple_dt_rb(mk_obj(5), mk_evt(21, 105), mk_evt(21, 104), osd_reqid_t(client, 8, 6)));
    log.add(mk_ple_dt_rb(mk_obj(5), mk_evt(21, 106), mk_evt(21, 105), osd_reqid_t(client, 8, 6)));
    log.add(mk_ple_dt_rb(mk_obj(5), mk_evt(21, 107), mk_evt(21, 106), osd_reqid_t(client, 8, 6)));
...
TEST_F(PGLogTrimTest, TestTrimDups2)
{
    ...
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(9, 98), mk_evt(8, 97), osd_reqid_t(client, 8, 1))));
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(9, 99), mk_evt(8, 98), osd_reqid_t(client, 8, 1))));
...
TEST_F(PGLogTrimTest, TestCopyUpTo2)
{
    ...
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(8, 98), mk_evt(8, 97), osd_reqid_t(client, 8, 1))));
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(9, 99), mk_evt(8, 98), osd_reqid_t(client, 8, 1))));
...
TEST_F(PGLogTrimTest, TestCopyAfter2)
{
    ...
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(8, 93), mk_evt(8, 92), osd_reqid_t(client, 8, 1))));
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(8, 94), mk_evt(8, 93), osd_reqid_t(client, 8, 1))));
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(8, 95), mk_evt(8, 94), osd_reqid_t(client, 8, 1))));
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(8, 96), mk_evt(8, 95), osd_reqid_t(client, 8, 1))));
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(8, 97), mk_evt(8, 96), osd_reqid_t(client, 8, 1))));
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(8, 98), mk_evt(8, 97), osd_reqid_t(client, 8, 1))));
    log.dups.push_back(pg_log_dup_t(mk_ple_mod(mk_obj(1), mk_evt(9, 99), mk_evt(8, 98), osd_reqid_t(client, 8, 1))));
...
// Local Variables:
// compile-command: "cd ../.. ; make unittest_pglog ; ./unittest_pglog --log-to-stderr=true  --debug-osd=20 #
// --gtest_filter=*.* " End:
