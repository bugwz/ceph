...
{
public:
    TestScrubBackend(ScrubBeListener& scrubber, PgScrubBeListener& pg, pg_shard_t i_am, bool repair,
...
public:
    ...
    uint64_t logical_to_ondisk_size(uint64_t logical_size) const final { return logical_size; }
...
public:
    ...
    {
        std::cout << fmt::format("{} submit_digest_fixes({})", __func__, fmt::join(fixes, ",")) << std::endl;
    }

    int get_snaps(const hobject_t& hoid, std::set<snapid_t>* snaps_set) const;

    tl::expected<std::set<snapid_t>, result_t> get_snaps(const hobject_t& oid) const final;

    tl::expected<std::set<snapid_t>, result_t> get_snaps_check_consistency(const hobject_t& oid) const final
    {
        /// \todo for now
    ...
    {
        std::cout << fmt::format("{}: ({}) -> #{} {}", __func__, hoid, snaps.size(), snaps) << std::endl;
    ...
    void set_snaps(const ScrubGenerator::all_clones_snaps_t& clones_snaps)
    {
        for (const auto& [clone, snaps] : clones_snaps) {
            std::cout << fmt::format("{}: ({}) -> #{} {}", __func__, clone, snaps.size(), snaps) << std::endl;
            std::set<snapid_t> snaps_set(snaps.begin(), snaps.end());
...
int TestScrubber::get_snaps(const hobject_t& hoid, std::set<snapid_t>* snaps_set) const
{
    ...
    *snaps_set = it->second;
    std::cout << fmt::format("{}: ({}) -> #{} {}", __func__, hoid, snaps_set->size(), *snaps_set) << std::endl;
    return 0;
}

tl::expected<std::set<snapid_t>, Scrub::SnapMapReaderI::result_t> TestScrubber::get_snaps(const hobject_t& oid) const
{
    ...
    return tl::make_unexpected(Scrub::SnapMapReaderI::result_t{Scrub::SnapMapReaderI::result_t::code_t::not_found, r});
}
...
void TestTScrubberBe::SetUp()
{
    ...
    pool = std::make_shared<PGPool>(osdmap, pool_id, *ext_pool_info, pool_conf.name);
    ...
    real_objs_list = ScrubGenerator::make_real_objs_conf(pool_id, real_objs, acting_osds);
    ...
    std::cout << fmt::format("{}: acting: {}", __func__, acting_shards) << std::endl;
...
OSDMapRef TestTScrubberBe::setup_map(int num_osds, const ScrubGenerator::pool_conf_t& pconf)
{
...
pg_info_t TestTScrubberBe::setup_pg_in_map()
{
    ...
    std::cout << fmt::format("{}: pg: {} up_osds: {} up_primary: {} acting_osds: {} "
                             "acting_primary: "
                             "{}",
                             __func__,
                             pgid,
    ...
    std::cout << fmt::format("{}: spg: {} and I am {}", __func__, spg, i_am) << std::endl;

    // the 'acting shards' set - the one actually used by the scrubber
    std::for_each(acting_osds.begin(), acting_osds.end(), [&](int osd) { acting_shards.insert(pg_shard_t{osd}); });
    std::cout << fmt::format("{}: acting_shards: {}", __func__, acting_shards) << std::endl;
...
void TestTScrubberBe::fake_a_scrub_set(ScrubGenerator::RealObjsConfList& all_sets)
{
    for (int osd_num = 0; osd_num < pool_conf.size; ++osd_num) {
        ...
        for (auto& obj : all_sets[osd_num]->objs) {
            std::cout << fmt::format("{}: object: {}", __func__, obj.ghobj.hobj) << std::endl;
            ScrubGenerator::add_object(smap, obj, osd_num);
        }

        std::cout << fmt::format("{}: {} inserting smap {:D}", __func__, osd_num, smap) << std::endl;
    ...
    for (const auto& robj : all_sets[i_am.osd]->objs) {
        ...
        std::cout << fmt::format("{}: object: {}", __func__, robj.ghobj.hobj) << std::endl;
...
void TestScrubBackend::insert_faked_smap(pg_shard_t shard, const ScrubMap& smap)
{
    ...
    std::cout << fmt::format("{}: inserting faked smap for osd {}", __func__, shard.osd) << std::endl;
...
public:
    ...
    {
        std::cout << fmt::format("{}: injecting params (minimal snaps conf.)", __func__) << std::endl;
...
public:
    ...
    TestTScrubberBeParams inject_params() override
    {
        std::cout << fmt::format("{}: injecting params (minimal-snaps + size change)", __func__) << std::endl;
        TestTScrubberBeParams params{/* pool_conf */ pl,
                                     /* real_objs_conf */ ScrubDatasets::minimal_snaps_configuration,
                                     /*num_osds */ 3};
