...
TEST(pg_missing_t, add_next_event)
{
    ...
    pg_log_entry_t sample_e(pg_log_entry_t::DELETE,
                            oid,
                            version,
                            prior_version,
                            0,
...
TEST(pg_pool_t_test, get_pg_num_divisor)
{
    ...
    for (int i = 0; i < 16; ++i) ASSERT_EQ(16u, p.get_pg_num_divisor(pg_t(i, 1)));
...
TEST(pg_pool_t_test, get_random_pg_position)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        cout << p.get_pg_num() << " " << pgid << ": " << h << " -> " << pg_t(ps, 1) << std::endl;
...
TEST(coll_t, parse)
{
    const char* ok[] = {"meta", "1.2_head", "1.2_TEMP", "1.2s3_head", "1.3s2_TEMP", "1.2s0_head", 0};
    const char* bad[] = {"foo",
                         "1.2_food",
                         "1.2_head ",
...
TEST(pool_opts_t, scrub_min_interval)
{
    ...
    EXPECT_EQ(pool_opts_t::get_opt_desc("scrub_min_interval"),
              pool_opts_t::opt_desc_t(pool_opts_t::SCRUB_MIN_INTERVAL, pool_opts_t::DOUBLE));
...
TEST(pool_opts_t, scrub_max_interval)
{
    ...
    EXPECT_EQ(pool_opts_t::get_opt_desc("scrub_max_interval"),
              pool_opts_t::opt_desc_t(pool_opts_t::SCRUB_MAX_INTERVAL, pool_opts_t::DOUBLE));
...
TEST(pool_opts_t, deep_scrub_interval)
{
    ...
    EXPECT_EQ(pool_opts_t::get_opt_desc("deep_scrub_interval"),
              pool_opts_t::opt_desc_t(pool_opts_t::DEEP_SCRUB_INTERVAL, pool_opts_t::DOUBLE));
...
struct RequiredPredicate : IsPGRecoverablePredicate
{
    ...
    {}
    bool operator()(const set<pg_shard_t>& have) const override { return have.size() >= required_size; }
};
...
struct MapPredicate
{
    ...
    explicit MapPredicate(const vector<pair<int, pair<PastIntervals::osd_state_t, epoch_t>>>& _states)
...
{
    PITest() {}
    void run(bool ec_pool, ivallst intervals, epoch_t last_epoch_started, unsigned min_to_peer,
             vector<pair<int, pair<PastIntervals::osd_state_t, epoch_t>>> osd_states, vector<int> up,
             vector<int> acting, set<pg_shard_t> probe, set<int> down, map<int, epoch_t> blocked_by, bool pg_down)
    {
        RequiredPredicate rec_pred(min_to_peer);
        MapPredicate map_pred(osd_states);

        PI::PriorSet correct(ec_pool, probe, down, blocked_by, pg_down, new RequiredPredicate(rec_pred));
        ...
        PI::PriorSet compact_ps = compact.get_prior_set(
            ec_pool, last_epoch_started, new RequiredPredicate(rec_pred), map_pred, up, acting, nullptr);
...
TEST_F(PITest, past_intervals_rep)
{
    run(
        ...
        /* min_peer   */ 1,
        /* osd states at end */
        {make_pair(0, make_pair(PI::UP, 0)), make_pair(1, make_pair(PI::UP, 0)), make_pair(2, make_pair(PI::DOWN, 0))},
        /* acting     */ {0, 1},
        /* up         */ {0, 1},
        /* probe      */ {pst(0), pst(1)},
        /* down       */ {2},
...
TEST_F(PITest, past_intervals_ec)
{
    run(
        /* ec_pool    */ true,
        /* intervals  */
        {ival{{0, 1, 2}, {0, 1, 2}, 10, 20, true, 0, 0}, ival{{N, 1, 2}, {N, 1, 2}, 21, 30, true, 1, 1}},
        /* les        */ 5,
        /* min_peer   */ 2,
        /* osd states at end */
        {make_pair(0, make_pair(PI::DOWN, 0)), make_pair(1, make_pair(PI::UP, 0)), make_pair(2, make_pair(PI::UP, 0))},
        /* acting     */ {N, 1, 2},
        /* up         */ {N, 1, 2},
...
TEST_F(PITest, past_intervals_ec_down)
{
    run(
        ...
        /* min_peer   */ 2,
        /* osd states at end */
        {make_pair(0, make_pair(PI::UP, 0)), make_pair(1, make_pair(PI::DOWN, 0)), make_pair(2, make_pair(PI::UP, 0))},
        /* acting     */ {0, N, 2},
        /* up         */ {0, N, 2},
...
TEST_F(PITest, past_intervals_rep_lost)
{
    run(
        ...
        /* min_peer   */ 1,
        /* osd states at end */
        {make_pair(0, make_pair(PI::UP, 0)), make_pair(1, make_pair(PI::UP, 0)), make_pair(2, make_pair(PI::LOST, 55))},
        /* acting     */ {0, 1},
        /* up         */ {0, 1},
        /* probe      */ {pst(0), pst(1)},
        /* down       */ {2},
...
TEST_F(PITest, past_intervals_ec_lost)
{
    run(
        ...
        /* min_peer   */ 2,
        /* osd states at end */
        {make_pair(0, make_pair(PI::UP, 0)), make_pair(1, make_pair(PI::LOST, 36)), make_pair(2, make_pair(PI::UP, 0))},
        /* acting     */ {0, N, 2},
        /* up         */ {0, N, 2},
        /* probe      */ {pst(0, sit(0)), pst(2, sit(2))},
        /* down       */ {1},
...
void ci_ref_test(object_manifest_t l, object_manifest_t to_remove, object_manifest_t g,
                 object_ref_delta_t expected_delta)
{
    ...
    if (l.chunk_map.empty() || g.chunk_map.empty()) {
        ...
        to_remove.calc_refs_to_drop_on_removal(
            l.chunk_map.empty() ? nullptr : &l, g.chunk_map.empty() ? nullptr : &g, delta);
        ASSERT_EQ(expected_delta, delta);
    }
}

void ci_ref_test_on_modify(object_manifest_t l, object_manifest_t to_remove, ObjectCleanRegions clean_regions,
                           object_ref_delta_t expected_delta)
{
    {
        object_ref_delta_t delta;
...
void ci_ref_test_inc_on_set(object_manifest_t l, object_manifest_t added_set, object_manifest_t g,
                            object_ref_delta_t expected_delta)
{
    {
        object_ref_delta_t delta;
...
object_manifest_t mk_manifest(std::map<uint64_t, std::tuple<uint64_t, uint64_t, string>> m)
{
...
{
    ci_ref_test(mk_manifest({}), mk_manifest({{0, {0, 1024, "foo"}}}), mk_manifest({}), mk_delta({{"foo", -1}}));
}


TEST(chunk_info_test, calc_refs_to_drop_match)
{
    ci_ref_test(mk_manifest({{0, {0, 1024, "foo"}}}),
...
{
    ci_ref_test(
        mk_manifest({}), mk_manifest({{0, {0, 1024, "foo"}}}), mk_manifest({{0, {0, 1024, "foo"}}}), mk_delta({}));
}

TEST(chunk_info_test, calc_refs_to_drop_tail_match)
{
    ci_ref_test(
        mk_manifest({{0, {0, 1024, "foo"}}}), mk_manifest({{0, {0, 1024, "foo"}}}), mk_manifest({}), mk_delta({}));
}
...
{
    ci_ref_test(mk_manifest({{4096, {0, 1024, "foo"}}}),
...
TEST(chunk_info_test, calc_refs_modify_mismatch)
{
    ...
    ci_ref_test_on_modify(mk_manifest({{512, {2048, 1024, "foo"}}, {4096, {0, 1024, "foo"}}}),
...
TEST(chunk_info_test, calc_refs_modify_match)
{
    ...
    ci_ref_test_on_modify(mk_manifest({{512, {2048, 1024, "foo"}}, {4096, {0, 1024, "ttt"}}}),
...
{
    ci_ref_test_inc_on_set(mk_manifest({{256, {0, 256, "aaa"}}, {4096, {0, 1024, "foo"}}}),
...
{
    ci_ref_test_inc_on_set(mk_manifest({{512, {0, 1024, "aaa"}}, {4096, {0, 1024, "foo"}}}),
...
{
    ci_ref_test_inc_on_set(mk_manifest({{256, {0, 256, "bbb"}}, {4096, {0, 1024, "foo"}}}),
                           mk_manifest({{256, {0, 256, "aaa"}}, {4096, {0, 1024, "foo"}}}),
