...
public:
    ...
    {
        return (rhs.objnum == objnum && rhs.cursnap == cursnap && rhs.seqnum == seqnum && rhs.prefix == prefix &&
                rhs.oid == oid);
    }

    bool operator<(const ContDesc& rhs) const { return seqnum < rhs.seqnum; }

    bool operator!=(const ContDesc& rhs) { return !((*this) == rhs); }
...
public:
    ...
    public:
        ...
        {
            uint64_t _off = 0;
            for (bufferlist::iterator i = bl.begin(); !i.end(); ++i, ++_off, ++(*this)) {
                if (*i != **this) {
                    if (off) *off = _off;
                    return false;
                }
    ...
    public:
        ...
        bufferlist gen_bl_advance(uint64_t s) { return impl->gen_bl_advance(s); }
        bool check_bl_advance(bufferlist& bl, uint64_t* off = nullptr) { return impl->check_bl_advance(bl, off); }
    ...
    virtual void get_ranges_map(const ContDesc& cont, std::map<uint64_t, uint64_t>& out) = 0;
    void get_ranges(const ContDesc& cont, interval_set<uint64_t>& out)
    {
        std::map<uint64_t, uint64_t> ranges;
        get_ranges_map(cont, ranges);
        for (std::map<uint64_t, uint64_t>::iterator i = ranges.begin(); i != ranges.end(); ++i) {
    ...
    iterator get_iterator(const ContDesc& in) { return iterator(this, get_iterator_impl(in)); }
};
...
public:
    ...
    public:
        ...
        char operator*() override { return current; }
        ...
        bool end() override { return pos >= cont_gen->get_length(cont); }
    };
    ...
    void put_iterator_impl(ContentsGenerator::iterator_impl* in) override { delete in; }

    ContentsGenerator::iterator_impl* dup_iterator_impl(const ContentsGenerator::iterator_impl* in) override
    {
...
public:
    ...
    {}
    void get_ranges_map(const ContDesc& cont, std::map<uint64_t, uint64_t>& out) override;
...
public:
    AppendGenerator(uint64_t off, uint64_t alignment, uint64_t min_append_size, uint64_t _max_append_size,
    ...
    {
        if (_max_append_size == min_append_size) max_append_size += alignment;
    ...
    uint64_t get_length(const ContDesc& in) override { return off + get_append_size(in); }
    void get_ranges_map(const ContDesc& cont, std::map<uint64_t, uint64_t>& out) override;
};
...
public:
    ...
    public:
        ...
        public:
            ...
            ContState(const ContDesc& _cont, std::shared_ptr<ContentsGenerator> _gen, ContentsGenerator::iterator _iter)
            ...
            const interval_set<uint64_t>& get_ranges() { return ranges; }

            uint64_t get_size() { return gen->get_length(cont); }

            bool covers(uint64_t pos) { return ranges.contains(pos) || (!ranges.starts_after(pos) && pos >= size); }
            ...
            {
                ceph_assert(covers(pos));
                return ranges.contains(pos) ? ranges.end_after(pos) : std::numeric_limits<uint64_t>::max();
            }
        };
    ...
    bool deleted() { return !exists; }

    bool has_contents() { return layers.size(); }

    // takes ownership of gen
    void update(ContentsGenerator* gen, const ContDesc& next);
    bool check(bufferlist& to_check);
    bool check_sparse(const std::map<uint64_t, uint64_t>& extends, bufferlist& to_check);
    const ContDesc& most_recent();
    ContentsGenerator* most_recent_gen() { return layers.begin()->first.get(); }
