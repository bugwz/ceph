...
TEST(extentcache, simple_write)
{
    ...
    auto to_read = iset_from_vector({{0, 2}, {8, 2}, {20, 2}});
    auto to_write = iset_from_vector({{0, 10}, {20, 4}});
    ...
    auto pending = c.get_remaining_extents_for_rmw(oid, pin, pending_read);
    ASSERT_TRUE(pending.empty());

    auto write_map = imap_from_iset(to_write);
    c.present_rmw_update(oid, pin, write_map);
...
TEST(extentcache, write_write_overlap)
{
    ...
    auto to_read2 = iset_from_vector({{2, 4}, {10, 4}, {18, 4}});
    auto to_write2 = iset_from_vector({{2, 12}, {18, 12}});
    auto must_read2 = c.reserve_extents_for_rmw(oid, pin2, to_write2, to_read2);
    ASSERT_EQ(must_read2, iset_from_vector({{10, 4}, {18, 2}}));
    ...
    auto pending2 = c.get_remaining_extents_for_rmw(oid, pin2, pending_read2);
    ASSERT_EQ(pending2, imap_from_iset(pending_read2));

    auto write_map2 = imap_from_iset(to_write2);
    c.present_rmw_update(oid, pin2, write_map2);
...
TEST(extentcache, write_write_overlap2)
{
    ...
    auto pending3 = c.get_remaining_extents_for_rmw(oid, pin3, pending_read3);
    ASSERT_EQ(pending3, imap_from_iset(pending_read3));

    auto write_map3 = imap_from_iset(to_write3);
    c.present_rmw_update(oid, pin3, write_map3);
