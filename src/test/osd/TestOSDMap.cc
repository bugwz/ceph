...
int main(int argc, char** argv)
{
    ...
    auto cct = global_init(
        &defaults, args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_UTILITY, CINIT_FLAG_NO_DEFAULT_CONFIG_FILE);
    common_init_finish(g_ceph_context);
    ::testing::InitGoogleTest(&argc, argv);
    return RUN_ALL_TESTS();
}

class OSDMapTest : public testing::Test, public ::testing::WithParamInterface<std::pair<int, int>>
{
    int num_osds = 6;
...
public:
    ...
    int get_ec_crush_rule()
    {
        ...
        if (r < 0) {
            r = osdmap.crush->add_simple_rule(
                EC_RULE_NAME, "default", "osd", "", "indep", pg_pool_t::TYPE_ERASURE, &cerr);
        }
        return r;
    }
    uint64_t set_ec_pool(const string& name, OSDMap::Incremental& new_pool_inc, bool assert_pool_id = true)
    {
        pg_pool_t empty;
        uint64_t pool_id = ++new_pool_inc.new_pool_max;
        if (assert_pool_id) ceph_assert(pool_id == my_ec_pool);
    ...
    uint64_t set_rep_pool(const string name, OSDMap::Incremental& new_pool_inc, bool assert_pool_id = true)
    {
        pg_pool_t empty;
        uint64_t pool_id = ++new_pool_inc.new_pool_max;
        if (assert_pool_id) ceph_assert(pool_id == my_rep_pool);
        pg_pool_t* p = new_pool_inc.get_new_pool(pool_id, &empty);
        p->size = 3;
        p->set_pg_num(64);
        p->set_pgp_num(64);
    ...
    int crush_rule_create_replicated(const string& name, const string& root, const string& type)
    {
    ...
    void test_mappings(int pool, int num, vector<int>* any, vector<int>* first, vector<int>* primary)
    {
        ...
        for (int i = 0; i < num; ++i) {
            ...
            for (unsigned j = 0; j < acting.size(); ++j) (*any)[acting[j]]++;
            if (!acting.empty()) (*first)[acting[0]]++;
            if (acting_primary >= 0) (*primary)[acting_primary]++;
    ...
    void clean_pg_upmaps(CephContext* cct, const OSDMap& om, OSDMap::Incremental& pending_inc)
    {
        int cpu_num = 8;
...
TEST_F(OSDMapTest, PGTempRespected)
{
    ...
    *new_acting_osds.rbegin() = first;
    ...
    pgtemp_map.new_pg_temp[pgid] = mempool::osdmap::vector<int>(new_acting_osds.begin(), new_acting_osds.end());
...
TEST_F(OSDMapTest, CleanTemps)
{
    ...
    pg_t pga = osdmap.raw_pg_to_pg(pg_t(0, my_rep_pool));
    {
        vector<int> up_osds, acting_osds;
        int up_primary, acting_primary;
        osdmap.pg_to_up_acting_osds(pga, &up_osds, &up_primary, &acting_osds, &acting_primary);
        pgtemp_map.new_pg_temp[pga] = mempool::osdmap::vector<int>(up_osds.begin(), up_osds.end());
    ...
    pg_t pgb = osdmap.raw_pg_to_pg(pg_t(1, my_rep_pool));
    {
        vector<int> up_osds, acting_osds;
        int up_primary, acting_primary;
        osdmap.pg_to_up_acting_osds(pgb, &up_osds, &up_primary, &acting_osds, &acting_primary);
        pending_inc.new_pg_temp[pgb] = mempool::osdmap::vector<int>(up_osds.begin(), up_osds.end());
    ...
    EXPECT_TRUE(pending_inc.new_pg_temp.count(pga) && pending_inc.new_pg_temp[pga].size() == 0);
    EXPECT_EQ(-1, pending_inc.new_primary_temp[pga]);

    EXPECT_TRUE(!pending_inc.new_pg_temp.count(pgb) && !pending_inc.new_primary_temp.count(pgb));
}
...
TEST_F(OSDMapTest, KeepsNecessaryTemps)
{
    ...
    for (; i != (int)get_num_osds(); ++i) {
        bool in_use = false;
        for (vector<int>::iterator osd_it = up_osds.begin(); osd_it != up_osds.end(); ++osd_it) {
            if (i == *osd_it) {
                in_use = true;
                break;
            }
    ...
    if (i == (int)get_num_osds()) FAIL() << "did not find unused OSD for temp mapping";

    pgtemp_map.new_pg_temp[pgid] = mempool::osdmap::vector<int>(up_osds.begin(), up_osds.end());
...
TEST_F(OSDMapTest, PrimaryAffinity)
{
    ...
    int n = get_num_osds();
    for (map<int64_t, pg_pool_t>::const_iterator p = osdmap.get_pools().begin(); p != osdmap.get_pools().end(); ++p) {
        int pool = p->first;
        int expect_primary = 10000 / n;
        cout << "pool " << pool << " size " << (int)p->second.size << " expect_primary " << expect_primary << std::endl;
        {
            vector<int> any(n, 0);
...
TEST_F(OSDMapTest, CleanPGUpmaps)
{
    ...
    int upmap_rule_no = crush_rule_create_replicated(upmap_rule, "default", "host");
    ...
    ASSERT_LT(1U, up.size());
    {
        ...
        int parent_0 = osdmap.crush->get_parent_of_type(up[0], osdmap.crush->get_type_id("host"));
        int parent_1 = osdmap.crush->get_parent_of_type(up[1], osdmap.crush->get_type_id("host"));
    ...
    { // TEST pg_upmap
     {// STEP-1: enumerate all children of up[0]'s parent,
      ...
      int parent = osdmap.crush->get_parent_of_type(up[0], osdmap.crush->get_type_id("host"));
    set<int> candidates;
    ...
    osdmap.apply_incremental(pending_inc);
    {
        ...
        int parent_0 = osdmap.crush->get_parent_of_type(new_up[0], osdmap.crush->get_type_id("host"));
        int parent_1 = osdmap.crush->get_parent_of_type(new_up[1], osdmap.crush->get_type_id("host"));
...

{
    ...
    set<int> parents;
    for (auto u : up) {
        int parent = osdmap.crush->get_parent_of_type(u, osdmap.crush->get_type_id("host"));
        ASSERT_GT(0, parent);
    ...
    vector<int> up_after_out;
    {
        ...
        int parent = osdmap.crush->get_parent_of_type(up[1], osdmap.crush->get_type_id("host"));
        set<int> children;
        osdmap.crush->get_leaves(osdmap.crush->get_item_name(parent), &children);
        OSDMap::Incremental pending_inc(osdmap.get_epoch() + 1);
        for (auto c : children) {
        ...
        int will_choose = -1;
        for (auto o : new_up) {
            int parent = tmpmap.crush->get_parent_of_type(o, osdmap.crush->get_type_id("host"));
        ...
        osdmap.crush->get_leaves(osdmap.crush->get_item_name(candidate_parent), &candidate_children);
    ...
    }
    {
        ...
        osdmap.apply_incremental(pending_inc);
        {
            ...
            osdmap.pg_to_raw_up(pgid, &new_up, &new_up_primary);
            ASSERT_EQ(new_up.size(), up.size());
            ASSERT_TRUE(std::find(new_up.begin(), new_up.end(), replaced_by) != new_up.end());
            // and up[1] too
            ASSERT_TRUE(std::find(new_up.begin(), new_up.end(), up[1]) != new_up.end());
        }
    }
    {
...

    {
        ...
        ASSERT_TRUE(osdmap.get_pool_name(pool_id) == pool_1);
        {
            for (unsigned i = 0; i < 3; i++) {
                ...
                {
                    OSDMap::Incremental pending_inc(osdmap.get_epoch() + 1);
                    pending_inc.new_pg_upmap[pgid] = mempool::osdmap::vector<int32_t>(new_up.begin(), new_up.end());
                    osdmap.apply_incremental(pending_inc);
                }
                osdmap.pg_to_raw_up(pgid, &up, &up_primary);
...
TEST_F(OSDMapTest, BUG_40104)
{
    ...
    }
    {
        ...
        std::cout << "clean_pg_upmaps (~" << big_pg_num << " pg_upmap_items) latency:" << timespan_str(latency)
                  << std::endl;
    }
}
        ...
        pg_t rep_pgid = tmp.raw_pg_to_pg(rep_pg);
        {
            ...
            for (int i = 0; i < (int)get_num_osds(); i++) {
                if (std::find(rep_up.begin(), rep_up.end(), i) == rep_up.end()) {
                    ...
                    if (dc_p == dc_parent && rack_parents.find(rack_p) == rack_parents.end()) {
                        to = i;
                        break;
                    }
                }
...
const string OSDMapTest::range_addrs[] = {"198.51.100.0/22",
                                          "10.2.5.102/32",
                                          "2001:db8::/48",
                                          "3001:db8::/72",
                                          "4001:db8::/30",
                                          ...
                                          "6001:db8::/128",
                                          "7001:db8::/127"};
const string OSDMapTest::ip_addrs[] = {"198.51.100.14",
                                       "198.51.100.0",
                                       "198.51.103.255",
                                       "10.2.5.102",
                                       "2001:db8:0:0:0:0:0:0",
                                       "2001:db8:0:0:0:0001:ffff:ffff",
                                       "2001:db8:0:ffff:ffff:ffff:ffff:ffff",
                                       "3001:db8:0:0:0:0:0:0",
                                       "3001:db8:0:0:0:0001:ffff:ffff",
                                       "3001:db8:0:0:00ff:ffff:ffff:ffff",
                                       "4001:db8::",
                                       "4001:db8:0:0:0:0001:ffff:ffff",
                                       "4001:dbb:ffff:ffff:ffff:ffff:ffff:ffff",
                                       "5001:db8:0:0:0:0:0:0",
                                       "5001:db8:0:0:0:0:ffff:ffff",
                                       "5001:db8:0:0:ffff:ffff:ffff:ffff",
                                       "6001:db8:0:0:0:0:0:0",
                                       "7001:db8:0:0:0:0:0:0",
                                       "7001:db8:0:0:0:0:0:0001"};
const string OSDMapTest::unblocked_ip_addrs[] = {"0.0.0.0",
                                                 "1.1.1.1",
                                                 "192.168.1.1",
                                                 "198.51.99.255",
                                                 "198.51.104.0",
                                                 ...
                                                 "2001:db8:0001::",
                                                 "3001:db7:ffff:ffff:ffff:ffff:ffff:ffff",
                                                 "3001:db8:0:0:0100::",
                                                 "4001:db7:ffff:ffff:ffff:ffff:ffff:ffff",
                                                 "4001:dbc::",
                                                 "5001:db7:ffff:ffff:ffff:ffff:ffff:ffff",
                                                 "5001:db8:0:0001:0:0:0:0",
                                                 "6001:db8:0:0:0:0:0:0001",
                                                 "7001:db7:ffff:ffff:ffff:ffff:ffff:ffff",
                                                 "7001:db8:0:0:0:0:0:0002"};
...
TEST_F(OSDMapTest, read_balance_small_map)
{
    ...
    for (const auto& test : test_cases) {
        ...
        OSDMap::Incremental pending_inc(osdmap.get_epoch() + 1);
        osdmap.calc_pg_upmaps(g_ceph_context, 0, 100, only_pools, &pending_inc);
        osdmap.apply_incremental(pending_inc);
        ...
        rc = osdmap.calc_desired_primary_distribution(g_ceph_context, my_rep_pool, osds_to_check, desired_prim_dist);
        ASSERT_TRUE(rc >= 0);
...
INSTANTIATE_TEST_SUITE_P(OSDMap, OSDMapTest,
                         ::testing::Values(std::make_pair<int, int>(0, 1),   // chooseleaf firstn 0 host
