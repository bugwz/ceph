...
{
public:
    WeightedTestGenerator(int ops, int objects, map<TestOpType, unsigned int> op_weights, TestOpStat* stats,
                          int max_seconds, bool ec_pool, bool balance_reads, bool localize_reads, bool set_redirect,
                          bool set_chunk, bool enable_dedup)
        : m_nextop(NULL)
        , m_op(0)
        , m_ops(ops)
    ...
    {
        m_start = time(0);
        for (map<TestOpType, unsigned int>::const_iterator it = op_weights.begin(); it != op_weights.end(); ++it) {
            m_total_weight += it->second;
            m_weight_sums.insert(pair<TestOpType, unsigned int>(it->first, m_total_weight));
        }
    ...
    TestOp* next(RadosTestContext& context) override
    {
        ...
        while (retval == NULL) {
            ...
            time_t now = time(0);
            if (m_seconds && now - m_start > m_seconds) break;

            for (map<TestOpType, unsigned int>::const_iterator it = m_weight_sums.begin(); it != m_weight_sums.end();
                 ++it) {
    ...
    bool init_extensible_tier(RadosTestContext& context, TestOp*& op)
    {
        ...
        if (m_op <= m_objects) {
          ...
          oid << " " << string(300, 'o');
            ...
            cout << m_op << ": "
                 << "copy oid " << oid.str() << " target oid " << oid2.str() << std::endl;
            ...
            if (m_set_redirect) {
                ...
                cout << m_op << ": "
                     << "set_redirect oid " << oid.str() << " target oid " << oid2.str() << std::endl;
                ...
                cout << m_op << ": "
                     << "set_chunk oid " << oid.str() << " target oid " << oid2.str() << std::endl;
            ...
            cout << m_op << " oid_not_in_use " << set_size << " oid_redirect_not_in_use " << set_manifest_size
                 << std::endl;
...
private:
    TestOp* gen_op(RadosTestContext& context, TestOpType type)
    {
        ...
        case TEST_OP_READ:
            ...
            return new ReadOp(m_op, &context, oid, m_balance_reads, m_localize_reads, m_stats);

        case TEST_OP_WRITE:
            oid = *(rand_choose(context.oid_not_in_use));
            cout << m_op << ": "
                 << "write oid " << oid << " current snap is " << context.current_snap << std::endl;
            return new WriteOp(m_op, &context, oid, false, false, m_stats);

        case TEST_OP_WRITE_EXCL:
            oid = *(rand_choose(context.oid_not_in_use));
            cout << m_op << ": "
                 << "write (excl) oid " << oid << " current snap is " << context.current_snap << std::endl;
            return new WriteOp(m_op, &context, oid, false, true, m_stats);

        case TEST_OP_WRITESAME:
            oid = *(rand_choose(context.oid_not_in_use));
            cout << m_op << ": "
                 << "writesame oid " << oid << " current snap is " << context.current_snap << std::endl;
            return new WriteSameOp(m_op, &context, oid, m_stats);

        case TEST_OP_DELETE:
            oid = *(rand_choose(context.oid_not_in_use));
            cout << m_op << ": "
                 << "delete oid " << oid << " current snap is " << context.current_snap << std::endl;
        ...
        case TEST_OP_SNAP_REMOVE:
            ...
            while (true) {
                ...
                if (context.snaps_in_use.lookup(snap)) continue;   // in use; try again!
                cout << m_op << ": "
                     << "snap_remove snap " << snap << std::endl;
        ...
        case TEST_OP_ROLLBACK:
        {
            ...
            cout << m_op << ": "
                 << "rollback oid " << oid << " current snap is " << context.current_snap << std::endl;
        ...
        case TEST_OP_SETATTR:
            oid = *(rand_choose(context.oid_not_in_use));
            cout << m_op << ": "
                 << "setattr oid " << oid << " current snap is " << context.current_snap << std::endl;
            return new SetAttrsOp(m_op, &context, oid, m_stats);

        case TEST_OP_RMATTR:
            oid = *(rand_choose(context.oid_not_in_use));
            cout << m_op << ": "
                 << "rmattr oid " << oid << " current snap is " << context.current_snap << std::endl;
            return new RemoveAttrsOp(m_op, &context, oid, m_stats);

        case TEST_OP_WATCH:
            oid = *(rand_choose(context.oid_not_in_use));
            cout << m_op << ": "
                 << "watch oid " << oid << " current snap is " << context.current_snap << std::endl;
        ...
        case TEST_OP_COPY_FROM:
            ...
            cout << m_op << ": "
                 << "copy_from oid " << oid << " from oid " << oid2 << " current snap is " << context.current_snap
                 << std::endl;
        ...
        case TEST_OP_SET_CHUNK:
        {
            ...
            oid = *(rand_choose(context.oid_not_in_use));
            cout << m_op << ": "
                 << "set_chunk oid " << oid << " target oid " << std::endl;
...
int main(int argc, char** argv)
{
        ...
        if (strcmp(argv[i], "--max-ops") == 0)
            ...
            if (weight < 0) {
                ...
                if (ec_pool && !op_types[j].ec_pool_valid) {
                    cerr << "Error: cannot use op type " << op_types[j].name << " with --ec-pool" << std::endl;
                    exit(1);
                }
    ...
    if (enable_dedup) {
        if (chunk_algo == "" || chunk_size == "") {
            cerr << "Missing chunking algorithm: " << chunk_algo << " or chunking size: " << chunk_size << std::endl;
            exit(1);
        }
    }
    ...
    if (min_stride_size < 0) min_stride_size = size / 10;
    if (max_stride_size < 0) max_stride_size = size / 5;
    ...
    if (min_stride_size >= max_stride_size) {
        cerr << "Error: max_stride_size must be more than min_stride_size" << std::endl;
        return 1;
    }
    ...
    if (max_in_flight * 2 > objects) {
        cerr << "Error: max_in_flight must be <= than the number of objects / 2" << std::endl;
        return 1;
    }

    char* id = getenv("CEPH_CLIENT_ID");
    RadosTestContext context(pool_name,
                             max_in_flight,
                             size,
                             min_stride_size,
                             max_stride_size,
    ...
    TestOpStat stats;
    WeightedTestGenerator gen = WeightedTestGenerator(ops,
                                                      objects,
                                                      op_weights,
                                                      &stats,
                                                      max_seconds,
    ...
    int r = context.init();
    if (r < 0) {
        cerr << "Error initializing rados test context: " << cpp_strerror(r) << std::endl;
        exit(1);
    }
    context.loop(&gen);
    if (enable_dedup) {
