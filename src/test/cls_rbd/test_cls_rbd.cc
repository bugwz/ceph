...
TEST_F(TestClsRbd, mirror_snapshot)
{
    ...
    cls::rbd::SnapshotInfo snap;
    ASSERT_EQ(0, snapshot_get(&ioctx, oid, 1, &snap));
    auto sn = boost::get<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
    ...
    ASSERT_EQ(0, snapshot_get(&ioctx, oid, 1, &snap));
    sn = boost::get<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
    ASSERT_NE(nullptr, sn);
    ASSERT_EQ(1U, sn->mirror_peer_uuids.size());
    ASSERT_EQ(1U, sn->mirror_peer_uuids.count("peer2"));

    ASSERT_EQ(-ERESTART, mirror_image_snapshot_unlink_peer(&ioctx, oid, 1, "peer2"));
    ASSERT_EQ(0, snapshot_get(&ioctx, oid, 1, &snap));
    sn = boost::get<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
    ASSERT_NE(nullptr, sn);
    ASSERT_EQ(1U, sn->mirror_peer_uuids.size());
    ...
    auto nsn = boost::get<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
    ...
    ASSERT_EQ(0, snapshot_get(&ioctx, oid, 2, &snap));
    nsn = boost::get<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
    ASSERT_NE(nullptr, nsn);
