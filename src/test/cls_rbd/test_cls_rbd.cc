...
static int snapshot_add(librados::IoCtx* ioctx, const std::string& oid, uint64_t snap_id, const std::string& snap_name)
{
...
static int snapshot_remove(librados::IoCtx* ioctx, const std::string& oid, uint64_t snap_id)
{
    librados::ObjectWriteOperation op;
...
static int snapshot_rename(librados::IoCtx* ioctx, const std::string& oid, uint64_t snap_id,
...
static int old_snapshot_add(librados::IoCtx* ioctx, const std::string& oid, uint64_t snap_id,
                            const std::string& snap_name)
{
    librados::ObjectWriteOperation op;
...
{
    char* b = new char[len];
    for (size_t i = 0; i < len; i++) b[i] = (rand() % (128 - 32)) + 32;
    return b;
}

static bool is_sparse_read_supported(librados::IoCtx& ioctx, const std::string& oid)
{
    ...
    return (r == expected_r && m == expected_m && outbl.contents_equal(expected_outbl));
}
...
public:
    ...
    static void TearDownTestCase() { ASSERT_EQ(0, destroy_one_pool_pp(_pool_name, _rados)); }
...
TEST_F(TestClsRbd, get_all_features)
{
    ...
    ASSERT_EQ(static_cast<uint64_t>(RBD_FEATURES_ALL), static_cast<uint64_t>(all_features & RBD_FEATURES_ALL));
...
TEST_F(TestClsRbd, sparse_copyup)
{
    ...
    ASSERT_EQ((int)expected_outbl.length(), ioctx.read(oid, outbl, expected_outbl.length() + 1, 0));
...
TEST_F(TestClsRbd, add_remove_child)
{
    ...
    ASSERT_EQ(0, create_image(&ioctx, parent_image, 2 << 20, 0, RBD_FEATURE_LAYERING, parent_image, -1));
...
TEST_F(TestClsRbd, directory_methods)
{
    ...
    ASSERT_EQ(-ENOENT, dir_state_assert(&ioctx, oid, cls::rbd::DIRECTORY_STATE_READY));
    ASSERT_EQ(-ENOENT, dir_state_set(&ioctx, oid, cls::rbd::DIRECTORY_STATE_ADD_DISABLED));
    ...
    ASSERT_EQ(-EBUSY, dir_state_set(&ioctx, oid, cls::rbd::DIRECTORY_STATE_ADD_DISABLED));
...
TEST_F(TestClsRbd, create)
{
    ...
    ASSERT_EQ(0, create_image(&ioctx, oid, size, order, features, object_prefix, -1));
    ASSERT_EQ(-EEXIST, create_image(&ioctx, oid, size, order, features, object_prefix, -1));
    ASSERT_EQ(0, ioctx.remove(oid));

    ASSERT_EQ(-EINVAL, create_image(&ioctx, oid, size, order, features, "", -1));
    ASSERT_EQ(-ENOENT, ioctx.remove(oid));

    ASSERT_EQ(0, create_image(&ioctx, oid, 0, order, features, object_prefix, -1));
    ASSERT_EQ(0, ioctx.remove(oid));

    ASSERT_EQ(-ENOSYS, create_image(&ioctx, oid, size, order, -1, object_prefix, -1));
    ASSERT_EQ(-ENOENT, ioctx.remove(oid));

    ASSERT_EQ(0, create_image(&ioctx, oid, size, order, RBD_FEATURE_DATA_POOL, object_prefix, 123));
    ASSERT_EQ(0, ioctx.remove(oid));
    ASSERT_EQ(-EINVAL, create_image(&ioctx, oid, size, order, RBD_FEATURE_OPERATIONS, object_prefix, -1));
    ASSERT_EQ(-EINVAL, create_image(&ioctx, oid, size, order, RBD_FEATURE_DATA_POOL, object_prefix, -1));
    ASSERT_EQ(-EINVAL, create_image(&ioctx, oid, size, order, 0, object_prefix, 123));
...
TEST_F(TestClsRbd, get_features)
{
    ...
    ASSERT_EQ(-ENOENT, get_features(&ioctx, oid, false, &features, &incompatible_features));

    ASSERT_EQ(0, create_image(&ioctx, oid, 0, 22, 0, oid, -1));
    ASSERT_EQ(0, get_features(&ioctx, oid, false, &features, &incompatible_features));
...
TEST_F(TestClsRbd, get_data_pool)
{
    ...
    ASSERT_EQ(0, create_image(&ioctx, oid, 0, 22, RBD_FEATURE_DATA_POOL, oid, 12));
    ASSERT_EQ(0, get_data_pool(&ioctx, oid, &data_pool_id));
...
TEST_F(TestClsRbd, protection_status)
{
    ...
    ASSERT_EQ(-ENOENT, get_protection_status(&ioctx, oid, CEPH_NOSNAP, &status));
    ...
    ASSERT_EQ(-EINVAL, get_protection_status(&ioctx, oid2, CEPH_NOSNAP, &status));
    ASSERT_EQ(-ENOEXEC, set_protection_status(&ioctx, oid2, CEPH_NOSNAP, status));
    ASSERT_EQ(-EINVAL, get_protection_status(&ioctx, oid, CEPH_NOSNAP, &status));
    ...
    ASSERT_EQ(0, set_protection_status(&ioctx, oid, 10, RBD_PROTECTION_STATUS_PROTECTED));
    ASSERT_EQ(0, get_protection_status(&ioctx, oid, 10, &status));
    ASSERT_EQ(+RBD_PROTECTION_STATUS_PROTECTED, status);
    ASSERT_EQ(-EBUSY, snapshot_remove(&ioctx, oid, 10));

    ASSERT_EQ(0, set_protection_status(&ioctx, oid, 10, RBD_PROTECTION_STATUS_UNPROTECTING));
    ASSERT_EQ(0, get_protection_status(&ioctx, oid, 10, &status));
    ASSERT_EQ(+RBD_PROTECTION_STATUS_UNPROTECTING, status);
    ASSERT_EQ(-EBUSY, snapshot_remove(&ioctx, oid, 10));

    ASSERT_EQ(-EINVAL, set_protection_status(&ioctx, oid, 10, RBD_PROTECTION_STATUS_LAST));
    ASSERT_EQ(0, get_protection_status(&ioctx, oid, 10, &status));
    ASSERT_EQ(+RBD_PROTECTION_STATUS_UNPROTECTING, status);

    ASSERT_EQ(0, snapshot_add(&ioctx, oid, 20, "snap2"));
    ASSERT_EQ(0, get_protection_status(&ioctx, oid, 20, &status));
    ASSERT_EQ(+RBD_PROTECTION_STATUS_UNPROTECTED, status);
    ASSERT_EQ(0, set_protection_status(&ioctx, oid, 10, RBD_PROTECTION_STATUS_UNPROTECTED));
    ASSERT_EQ(0, get_protection_status(&ioctx, oid, 10, &status));
    ASSERT_EQ(+RBD_PROTECTION_STATUS_UNPROTECTED, status);
...
TEST_F(TestClsRbd, parents_v1)
{
    ...
    ASSERT_EQ(0, create_image(&ioctx, oid, 33 << 20, 22, RBD_FEATURE_LAYERING, "foo.", -1));
    ...
    ASSERT_EQ(0, ioctx.remove(oid));
    ASSERT_EQ(0, create_image(&ioctx, oid, 33 << 20, 22, RBD_FEATURE_LAYERING | RBD_FEATURE_DEEP_FLATTEN, "foo.", -1));
    ASSERT_EQ(0, set_parent(&ioctx, oid, {1, "", "parent", 3}, 100 << 20));
...
TEST_F(TestClsRbd, parents_v2)
{
    ...
    ASSERT_EQ(-ENOENT, parent_overlap_get(&ioctx, oid, CEPH_NOSNAP, &parent_overlap));
    ASSERT_EQ(-ENOENT, parent_attach(&ioctx, oid, parent_image_spec, 0ULL, false));
    ...
    ASSERT_EQ(0, parent_attach(&ioctx, oid, parent_image_spec, *parent_overlap, false));
    ASSERT_EQ(-EEXIST, parent_attach(&ioctx, oid, parent_image_spec, *parent_overlap, false));
    ASSERT_EQ(0, parent_attach(&ioctx, oid, parent_image_spec, *parent_overlap, true));
    --(*parent_overlap);
    ...
    ASSERT_EQ(-EXDEV, get_parent(&ioctx, oid, CEPH_NOSNAP, &on_disk_parent_spec, &legacy_parent_overlap));
    ASSERT_EQ(-EXDEV, get_parent(&ioctx, oid, 10, &on_disk_parent_spec, &legacy_parent_overlap));
...
TEST_F(TestClsRbd, stripingv2)
{
    ...
    ASSERT_EQ(0, create_image(&ioctx, oid2, 10, 22, RBD_FEATURE_STRIPINGV2, oid2, -1));
...
TEST_F(TestClsRbd, metadata)
{
    ...
    do {
        ...
        size += cur.size();
        for (map<string, bufferlist>::iterator it = cur.begin(); it != cur.end(); ++it) data[it->first] = it->second;
        last_read = cur.rbegin()->first;
        r = cur.size();
    } while (r == max_read);
    ASSERT_EQ(size, 1024U);
    for (map<string, bufferlist>::iterator it = data.begin(); it != data.end(); ++it) {
...
TEST_F(TestClsRbd, set_features)
{
    ...
    uint64_t incompatible_features;
    ASSERT_EQ(0, get_features(&ioctx, oid, true, &actual_features, &incompatible_features));
    ...
    expected_features = (RBD_FEATURES_MUTABLE | base_features) & ~RBD_FEATURE_OBJECT_MAP;
    ASSERT_EQ(expected_features, actual_features);

    ASSERT_EQ(0, set_features(&ioctx, oid, 0, RBD_FEATURE_DEEP_FLATTEN));
    ASSERT_EQ(-EINVAL, set_features(&ioctx, oid, RBD_FEATURE_DEEP_FLATTEN, RBD_FEATURE_DEEP_FLATTEN));
...
TEST_F(TestClsRbd, mirror)
{
    ...
    ASSERT_EQ(-EINVAL, mirror_peer_add(&ioctx, {"uuid1", MIRROR_PEER_DIRECTION_RX, "siteA", "client", "mirror uuid"}));
    ...
    ASSERT_EQ(-EINVAL, mirror_peer_add(&ioctx, {"mirror-uuid", MIRROR_PEER_DIRECTION_RX, "siteA", "client", ""}));
    ASSERT_EQ(-EINVAL, mirror_peer_add(&ioctx, {"uuid1", MIRROR_PEER_DIRECTION_TX, "siteA", "client", "mirror uuid"}));
    ASSERT_EQ(0, mirror_peer_add(&ioctx, {"uuid1", MIRROR_PEER_DIRECTION_RX, "siteA", "client", "fsidA"}));
    ASSERT_EQ(0, mirror_peer_add(&ioctx, {"uuid2", MIRROR_PEER_DIRECTION_RX, "siteB", "admin", ""}));
    ASSERT_EQ(-ESTALE, mirror_peer_add(&ioctx, {"uuid2", MIRROR_PEER_DIRECTION_RX, "siteC", "foo", ""}));
    ASSERT_EQ(-EEXIST, mirror_peer_add(&ioctx, {"uuid3", MIRROR_PEER_DIRECTION_RX, "siteA", "foo", ""}));
    ASSERT_EQ(-EEXIST, mirror_peer_add(&ioctx, {"uuid3", MIRROR_PEER_DIRECTION_RX, "siteC", "client", "fsidA"}));
    ASSERT_EQ(0, mirror_peer_add(&ioctx, {"uuid3", MIRROR_PEER_DIRECTION_RX, "siteC", "admin", ""}));
    ASSERT_EQ(0, mirror_peer_add(&ioctx, {"uuid4", MIRROR_PEER_DIRECTION_RX, "siteD", "admin", ""}));

    ASSERT_EQ(0, mirror_peer_list(&ioctx, &peers));
    std::vector<cls::rbd::MirrorPeer> expected_peers = {{"uuid1", MIRROR_PEER_DIRECTION_RX, "siteA", "client", "fsidA"},
    ...
    ASSERT_EQ(0, mirror_peer_list(&ioctx, &peers));
    expected_peers = {{"uuid1", MIRROR_PEER_DIRECTION_RX, "siteA", "client", "fsidA"},
                      {"uuid3", MIRROR_PEER_DIRECTION_RX, "siteC", "admin", ""}};
    ASSERT_EQ(expected_peers, peers);
    ...
    ASSERT_EQ(0, mirror_peer_list(&ioctx, &peers));
    expected_peers = {{"uuid1", MIRROR_PEER_DIRECTION_RX, "siteA", "new client", "fsidA"},
    ...
    ASSERT_EQ(0, mirror_peer_list(&ioctx, &peers));
    expected_peers = {{"uuid3", MIRROR_PEER_DIRECTION_RX, "new site", "admin", ""}};
    ASSERT_EQ(expected_peers, peers);
    ...
    expected_peers = {{"uuid3", MIRROR_PEER_DIRECTION_RX_TX, "new site", "admin", "mirror uuid"}};
    ...
    ASSERT_LT(utime_t{}, peers[0].last_seen);
    expected_peers = {{peers[0].uuid, MIRROR_PEER_DIRECTION_TX, "siteA", "", "mirror uuid"}};
    expected_peers[0].last_seen = peers[0].last_seen;
    ASSERT_EQ(expected_peers, peers);
...
TEST_F(TestClsRbd, mirror_image)
{
    ...
    cls::rbd::MirrorImage image1(cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "uuid1", cls::rbd::MIRROR_IMAGE_STATE_ENABLED);
    cls::rbd::MirrorImage image2(cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "uuid2", cls::rbd::MIRROR_IMAGE_STATE_DISABLING);
    cls::rbd::MirrorImage image3(cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "uuid3", cls::rbd::MIRROR_IMAGE_STATE_ENABLED);
    ...
    std::map<std::string, std::string> expected_mirror_image_ids = {{"image_id1", "uuid1"}};
...
TEST_F(TestClsRbd, mirror_image_status)
{
    struct WatchCtx : public librados::WatchCtx2
    {
        ...
        explicit WatchCtx(librados::IoCtx* ioctx)
            : m_ioctx(ioctx)
        {}
        void handle_notify(uint64_t notify_id, uint64_t cookie, uint64_t notifier_id, bufferlist& bl_) override
        {
            bufferlist bl;
    ...
    ASSERT_EQ(-ENOENT, mirror_image_status_list(&ioctx, "", 1024, &images, &statuses));

    // Test status set

    cls::rbd::MirrorImage image1(cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "uuid1", cls::rbd::MIRROR_IMAGE_STATE_ENABLED);
    cls::rbd::MirrorImage image2(cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "uuid2", cls::rbd::MIRROR_IMAGE_STATE_ENABLED);
    cls::rbd::MirrorImage image3(cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "uuid3", cls::rbd::MIRROR_IMAGE_STATE_ENABLED);
    ...
    cls::rbd::MirrorImageSiteStatus status1("", cls::rbd::MIRROR_IMAGE_STATUS_STATE_UNKNOWN, "");
    cls::rbd::MirrorImageSiteStatus status2("", cls::rbd::MIRROR_IMAGE_STATUS_STATE_REPLAYING, "");
    cls::rbd::MirrorImageSiteStatus status3("", cls::rbd::MIRROR_IMAGE_STATUS_STATE_ERROR, "");
    ...
    states.clear();
    cls::rbd::MirrorPeer mirror_peer{"uuid", cls::rbd::MIRROR_PEER_DIRECTION_RX, "siteA", "client", "fsidA"};
    ...
    cls::rbd::MirrorImageSiteStatus remote_status1("fsidA", cls::rbd::MIRROR_IMAGE_STATUS_STATE_REPLAYING, "");
    ...
    cls::rbd::MirrorImageSiteStatus remote_status2("fsidA", cls::rbd::MIRROR_IMAGE_STATUS_STATE_REPLAYING, "");
    remote_status2.up = true;
    cls::rbd::MirrorImageSiteStatus remote_status3("fsidA", cls::rbd::MIRROR_IMAGE_STATUS_STATE_UNKNOWN, "");
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        cls::rbd::MirrorImage image(cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, uuid, cls::rbd::MIRROR_IMAGE_STATE_ENABLED);
        cls::rbd::MirrorImageSiteStatus status("", cls::rbd::MIRROR_IMAGE_STATUS_STATE_UNKNOWN, "");
    ...
    images.clear();
    statuses.clear();
    ASSERT_EQ(0, mirror_image_status_list(&ioctx, last_read, N * 2, &images, &statuses));
...
TEST_F(TestClsRbd, mirror_image_map)
{
    ...
    while (expected_image_mapping.size() < 1024) {
        ...
        for (uint32_t i = 0; i < 32; ++i) {
            ...
            cls::rbd::MirrorImageMap mirror_image_map{stringify(i), expected_time, expected_data};
    ...
    ASSERT_EQ(0, mirror_image_map_list(&ioctx, image_mapping.rbegin()->first, 1000, &image_mapping));
    ASSERT_EQ(24U, image_mapping.size());

    const auto& image_map = *image_mapping.begin();
    ASSERT_EQ("978", image_map.first);

    cls::rbd::MirrorImageMap expected_mirror_image_map{stringify(18), expected_time, expected_data};
...
TEST_F(TestClsRbd, mirror_snapshot)
{
    ...
    cls::rbd::MirrorSnapshotNamespace primary = {
        cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {"peer1", "peer2"}, "", CEPH_NOSNAP};
    ...
    cls::rbd::SnapshotInfo snap;
    ASSERT_EQ(0, snapshot_get(&ioctx, oid, 1, &snap));
    auto sn = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
    ...
    ASSERT_EQ(-ENOENT, mirror_image_snapshot_unlink_peer(&ioctx, oid, 1, "peer1"));
    ASSERT_EQ(0, snapshot_get(&ioctx, oid, 1, &snap));
    sn = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
    ASSERT_NE(nullptr, sn);
    ASSERT_EQ(1U, sn->mirror_peer_uuids.size());
    ASSERT_EQ(1U, sn->mirror_peer_uuids.count("peer2"));

    ASSERT_EQ(0, mirror_image_snapshot_unlink_peer(&ioctx, oid, 1, "peer2"));
    ASSERT_EQ(-ENOENT, mirror_image_snapshot_unlink_peer(&ioctx, oid, 1, "peer2"));
    ASSERT_EQ(0, snapshot_get(&ioctx, oid, 1, &snap));
    sn = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
    ASSERT_NE(nullptr, sn);
    ...
    auto nsn = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
    ...
    ASSERT_EQ(0, mirror_image_snapshot_set_copy_progress(&ioctx, oid, 2, true, 10));
    ASSERT_EQ(0, snapshot_get(&ioctx, oid, 2, &snap));
    nsn = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
    ASSERT_NE(nullptr, nsn);
...
TEST_F(TestClsRbd, group_dir_rename)
{
    ...
    ASSERT_EQ(0, group_dir_rename(&ioctx, RBD_GROUP_DIRECTORY, src_name, dest_name, group_id));
    map<string, string> cgs;
    ASSERT_EQ(0, group_dir_list(&ioctx, RBD_GROUP_DIRECTORY, "", 10, &cgs));
    ...
    ASSERT_EQ(-EEXIST, group_dir_rename(&ioctx, RBD_GROUP_DIRECTORY, dest_name, dest_name, group_id));
    ASSERT_EQ(0, group_dir_remove(&ioctx, RBD_GROUP_DIRECTORY, dest_name, group_id));
    // source group name missing
    ASSERT_EQ(-ENOENT, group_dir_rename(&ioctx, RBD_GROUP_DIRECTORY, dest_name, src_name, group_id));
}
...
void test_image_add(librados::IoCtx& ioctx, const string& group_id, const string& image_id, int64_t pool_id)
{

    cls::rbd::GroupImageStatus st(image_id, pool_id, cls::rbd::GROUP_IMAGE_LINK_STATE_INCOMPLETE);
...
TEST_F(TestClsRbd, group_image_clean)
{
    ...
    cls::rbd::GroupImageStatus incomplete_st(image_id, pool_id, cls::rbd::GROUP_IMAGE_LINK_STATE_INCOMPLETE);
    ...
    cls::rbd::GroupImageStatus attached_st(image_id, pool_id, cls::rbd::GROUP_IMAGE_LINK_STATE_ATTACHED);
...
TEST_F(TestClsRbd, image_group_add)
{
    ...
    ASSERT_EQ(0, create_image(&ioctx, image_id, 2 << 20, 0, RBD_FEATURE_LAYERING, image_id, -1));
...
TEST_F(TestClsRbd, group_snap_list_max_return)
{
    ...
    for (int i = 0; i < 15; ++i) {
        ...
        cls::rbd::GroupSnapshot snap = {
            snap_id, "test_snapshot" + hexify(i), cls::rbd::GROUP_SNAPSHOT_STATE_INCOMPLETE};
        ASSERT_EQ(0, group_snap_set(&ioctx, group_id, snap));
    }
...
TEST_F(TestClsRbd, trash_methods)
{
    ...
    now1_delay += 380;
    cls::rbd::TrashImageSpec trash_spec(cls::rbd::TRASH_IMAGE_SOURCE_USER, "name", now1, now1_delay);
    ASSERT_EQ(0, trash_add(&ioctx, id, trash_spec));

    utime_t now2 = ceph_clock_now();
    utime_t now2_delay = now2;
    now2_delay += 480;
    cls::rbd::TrashImageSpec trash_spec2(cls::rbd::TRASH_IMAGE_SOURCE_MIRRORING, "name2", now2, now2_delay);
...
TEST_F(TestClsRbd, op_features)
{
    ...
    uint64_t features;
    uint64_t incompatible_features;
    ASSERT_EQ(0, get_features(&ioctx, oid, true, &features, &incompatible_features));
    ASSERT_EQ(0u, features);
    ...
    uint64_t expected_op_features = RBD_OPERATION_FEATURES_ALL & ~RBD_OPERATION_FEATURE_CLONE_PARENT;
...
TEST_F(TestClsRbd, clone_parent)
{
    ...
    librados::ObjectWriteOperation op1;
    ::librbd::cls_client::snapshot_add(&op1, 234, "trash_snap", cls::rbd::UserSnapshotNamespace());
    ASSERT_EQ(0, ioctx.operate(oid, &op1));
    librados::ObjectWriteOperation op2;
    ...
    cls::rbd::ChildImageSpecs expected_child_images = {{1, "", "image1"}, {1, "", "image2"}, {2, "", "image2"}};
    ...
    ASSERT_EQ(0, ioctx.operate(oid, &op3));
    ASSERT_EQ(0, snapshot_get(&ioctx, oid, 123, &snap));
    ASSERT_EQ(cls::rbd::SNAPSHOT_NAMESPACE_TYPE_TRASH, cls::rbd::get_snap_namespace_type(snap.snapshot_namespace));
    ...
    ASSERT_EQ(0, op_features_get(&ioctx, oid, &op_features));
    ASSERT_TRUE((op_features & expected_op_features) == RBD_OPERATION_FEATURE_SNAP_TRASH);
...
TEST_F(TestClsRbd, clone_parent_ns)
{
    ...
    cls::rbd::ChildImageSpecs expected_child_images = {{1, "ns1", "image1"}, {1, "ns2", "image1"}};
    ASSERT_EQ(expected_child_images, child_images);
...
TEST_F(TestClsRbd, clone_child)
{
    ...
    string oid = get_temp_image_name();
    ASSERT_EQ(0, create_image(&ioctx, oid, 0, 22, RBD_FEATURE_LAYERING | RBD_FEATURE_DEEP_FLATTEN, oid, -1));
    ASSERT_EQ(0, set_parent(&ioctx, oid, {1, "", "parent", 2}, 1));
    ASSERT_EQ(0, snapshot_add(&ioctx, oid, 123, "user_snap1"));
    ASSERT_EQ(0, op_features_set(&ioctx, oid, RBD_OPERATION_FEATURE_CLONE_CHILD, RBD_OPERATION_FEATURE_CLONE_CHILD));
    ...
    ASSERT_EQ(0, remove_parent(&ioctx, oid));
    ASSERT_EQ(0, op_features_get(&ioctx, oid, &op_features));
    ASSERT_TRUE((op_features & RBD_OPERATION_FEATURE_CLONE_CHILD) == RBD_OPERATION_FEATURE_CLONE_CHILD);
...
TEST_F(TestClsRbd, migration)
{
    ...
    uint64_t features;
    uint64_t incompatible_features;
    ASSERT_EQ(0, get_features(&ioctx, oid, CEPH_NOSNAP, &features, &incompatible_features));
    ASSERT_EQ(0U, features);
    ...
    ASSERT_EQ(0, migration_set_state(&ioctx, oid, migration_spec.state, migration_spec.state_description));
    ASSERT_EQ(0, migration_get(&ioctx, oid, &read_migration_spec));
    ASSERT_EQ(migration_spec, read_migration_spec);
    ...
    ASSERT_EQ(-EINVAL, migration_get(&ioctx, oid, &read_migration_spec));
    ASSERT_EQ(-EINVAL, migration_set_state(&ioctx, oid, migration_spec.state, migration_spec.state_description));
...
TEST_F(TestClsRbd, migration_v1)
{
    ...
    ASSERT_EQ(-EINVAL, migration_set_state(&ioctx, oid, migration_spec.state, migration_spec.state_description));
    header.clear();
    ASSERT_EQ(static_cast<int>(sizeof(RBD_HEADER_TEXT)), ioctx.read(oid, header, sizeof(RBD_HEADER_TEXT), 0));
...
TEST_F(TestClsRbd, assert_snapc_seq)
{
    ...
    ASSERT_EQ(0, assert_snapc_seq(&ioctx, oid, 0, cls::rbd::ASSERT_SNAPC_SEQ_GT_SNAPSET_SEQ));
    ASSERT_EQ(-ERANGE, assert_snapc_seq(&ioctx, oid, 0, cls::rbd::ASSERT_SNAPC_SEQ_LE_SNAPSET_SEQ));

    ASSERT_EQ(0, ioctx.create(oid, true));

    uint64_t snapc_seq = 0;

    ASSERT_EQ(-ERANGE, assert_snapc_seq(&ioctx, oid, snapc_seq, cls::rbd::ASSERT_SNAPC_SEQ_GT_SNAPSET_SEQ));
    ASSERT_EQ(0, assert_snapc_seq(&ioctx, oid, snapc_seq, cls::rbd::ASSERT_SNAPC_SEQ_LE_SNAPSET_SEQ));
    ...
    ASSERT_EQ(0, assert_snapc_seq(&ioctx, oid, snapc_seq, cls::rbd::ASSERT_SNAPC_SEQ_GT_SNAPSET_SEQ));
    ASSERT_EQ(-ERANGE, assert_snapc_seq(&ioctx, oid, snapc_seq, cls::rbd::ASSERT_SNAPC_SEQ_LE_SNAPSET_SEQ));
    ...
    ASSERT_EQ(0, assert_snapc_seq(&ioctx, oid, snapc_seq + 1, cls::rbd::ASSERT_SNAPC_SEQ_GT_SNAPSET_SEQ));
    ASSERT_EQ(-ERANGE, assert_snapc_seq(&ioctx, oid, snapc_seq + 1, cls::rbd::ASSERT_SNAPC_SEQ_LE_SNAPSET_SEQ));
...
TEST_F(TestClsRbd, sparsify)
{
    ...
    expected_outbl = inbl;
    ASSERT_EQ((int)expected_m.size(), ioctx.sparse_read(oid, m, outbl, inbl.length(), 0));
    ASSERT_EQ(m, expected_m);
    ASSERT_TRUE(outbl.contents_equal(expected_outbl));
    ...
    ASSERT_EQ(0, sparsify(&ioctx, oid, 16, true));
    outbl.clear();
    ASSERT_EQ((int)(inbl.length() - 4096), ioctx.read(oid, outbl, inbl.length(), 0));
