...
public:
    void SetUp() final
    {
        ...
        cct = common_preinit(params, CODE_ENVIRONMENT_UTILITY, CINIT_FLAG_NO_DEFAULT_CONFIG_FILE);
...
TEST_F(CrushWrapperTest, get_immediate_parent)
{
    ...
    int rootno;
    c->add_bucket(0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, ROOT_TYPE, 0, NULL, NULL, &rootno);
...

    {
        ...
        EXPECT_EQ(0, c->insert_item(cct, item, 1.0, "osd.0", loc));
    }
...
TEST_F(CrushWrapperTest, move_bucket)
{
    ...
    int root0;
    EXPECT_EQ(0, c->add_bucket(0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, ROOT_TYPE, 0, NULL, NULL, &root0));
    ...
    int root1;
    EXPECT_EQ(0, c->add_bucket(0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, ROOT_TYPE, 0, NULL, NULL, &root1));
...
TEST_F(CrushWrapperTest, swap_bucket)
{
    ...
    int root;
    EXPECT_EQ(0, c->add_bucket(0, CRUSH_BUCKET_STRAW2, CRUSH_HASH_RJENKINS1, ROOT_TYPE, 0, NULL, NULL, &root));
    EXPECT_EQ(0, c->set_item_name(root, "root"));

    int a, b;
    EXPECT_EQ(0, c->add_bucket(0, CRUSH_BUCKET_STRAW2, CRUSH_HASH_RJENKINS1, HOST_TYPE, 0, NULL, NULL, &a));
    EXPECT_EQ(0, c->set_item_name(a, "a"));
    EXPECT_EQ(0, c->add_bucket(0, CRUSH_BUCKET_STRAW2, CRUSH_HASH_RJENKINS1, HOST_TYPE, 0, NULL, NULL, &b));
...
TEST_F(CrushWrapperTest, rename_bucket_or_item)
{
    ...
    item++;
    {
        ...
        EXPECT_EQ(0, c->insert_item(cct, item, 1.0, "osd.1", loc));
    }
...
TEST_F(CrushWrapperTest, check_item_loc)
{
    ...
    const string OSD("osd.0");
    {
        map<string, string> loc;
        loc["root"] = "default";
        EXPECT_EQ(0, c->insert_item(cct, item, expected_weight, OSD, loc));
    }
...
TEST_F(CrushWrapperTest, update_item)
{
    ...
    int host0;
    c->add_bucket(0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, HOST_TYPE, 0, NULL, NULL, &host0);
    c->set_item_name(host0, HOST0);

    const string HOST1("host1");
    int host1;
    c->add_bucket(0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, HOST_TYPE, 0, NULL, NULL, &host1);
    ...
    // fail if invalid names anywhere in loc
    {
        map<string, string> loc;
        loc["rack"] = "\001";
        EXPECT_EQ(-EINVAL, c->update_item(cct, item, 1.0, "osd." + stringify(item), loc));
    }
    // fail if invalid item name
    {
        map<string, string> loc;
        EXPECT_EQ(-EINVAL, c->update_item(cct, item, 1.0, "\005", loc));
    }
    ...
    EXPECT_EQ(0, c->insert_item(cct, item, original_weight, OSD0, loc));

    // updating nothing changes nothing
    EXPECT_EQ(OSD0, c->get_item_name(item));
    EXPECT_EQ(original_weight, c->get_item_weightf(item));
    EXPECT_TRUE(c->check_item_loc(cct, item, loc, &weight));
    EXPECT_EQ(0, c->update_item(cct, item, original_weight, OSD0, loc));
    EXPECT_EQ(OSD0, c->get_item_name(item));
    EXPECT_EQ(original_weight, c->get_item_weightf(item));
    EXPECT_TRUE(c->check_item_loc(cct, item, loc, &weight));
    ...
    EXPECT_EQ(original_weight, c->get_item_weightf(item));
    EXPECT_TRUE(c->check_item_loc(cct, item, loc, &weight));
    EXPECT_EQ(1, c->update_item(cct, item, modified_weight, OSD1, loc));
    ...
    EXPECT_EQ(1, c->update_item(cct, item, modified_weight, OSD1, other_loc));
    EXPECT_EQ(OSD1, c->get_item_name(item));
    EXPECT_EQ(modified_weight, c->get_item_weightf(item));
    EXPECT_FALSE(c->check_item_loc(cct, item, loc, &weight));
...
TEST_F(CrushWrapperTest, adjust_item_weight)
{
    ...
    int hostfake;
    c->add_bucket(0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, HOST_TYPE, 0, NULL, NULL, &hostfake);
...

    {
        ...
        item = 0;
        EXPECT_EQ(0, c->insert_item(cct, item, 1.0, "osd." + stringify(item), loc));
        item = 1;
        EXPECT_EQ(0, c->insert_item(cct, item, 1.0, "osd." + stringify(item), loc));
        ...
        bloc["root"] = "default";
        EXPECT_EQ(0, c->insert_item(cct, host0, host_weight, HOST0, bloc));
    }

    {
        ...
        map<string, string> bloc;
        bloc["root"] = "default";
        EXPECT_EQ(0, c->insert_item(cct, hostfake, host_weight, FAKE, bloc));
    }
...
TEST_F(CrushWrapperTest, insert_item)
{
    ...
    // invalid names anywhere in loc trigger an error
    {
        map<string, string> loc;
        loc["host"] = "\001";
        EXPECT_EQ(-EINVAL, c->insert_item(cct, item, 1.0, "osd." + stringify(item), loc));
    }
    ...
    // insert an item in an existing bucket
    {
        ...
        EXPECT_EQ(-EEXIST, c->insert_item(cct, another_item, 1.0, "osd." + stringify(item), loc));
    }
    ...
    //
    {
        ...
        EXPECT_EQ(-ELOOP, c->insert_item(cct, rootno, 1.0, "default", loc));
    }
    // fail when mapping a bucket to the wrong type
    {
        // create an OSD bucket
        int osdno;
        int r = c->add_bucket(0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, 10, 0, NULL, NULL, &osdno);
        ASSERT_EQ(0, r);
...
TEST_F(CrushWrapperTest, remove_item)
{
    ...
    {
        int root;
        ASSERT_EQ(0, c->add_bucket(0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, ROOT_TYPE, 0, NULL, NULL, &root));
        c->set_item_name(root, "root0");
    }

    {
        int host;
        c->add_bucket(0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, HOST_TYPE, 0, NULL, NULL, &host);
        c->set_item_name(host, "host0");
    }

    const int num_osd = 12;
    {
        map<string, string> loc = {{"root", "root0"}, {"host", "host0"}};
        string name{"osd."};
        for (int item = 0; item < num_osd; item++) {
            ASSERT_EQ(0, c->insert_item(cct, item, 1.0, name + to_string(item), loc));
        }
    }
...
TEST_F(CrushWrapperTest, dump_rules)
{
    ...
    string name("NAME");
    int rule = c->add_simple_rule(name, root_name, failure_domain_type, "", "firstn", pg_pool_t::TYPE_ERASURE);
    EXPECT_EQ(0, rule);
...

    {
        ...
        f->flush(ss);
        EXPECT_EQ((unsigned)0, ss.str().find("<rule><rule_id>0</rule_id><rule_name>NAME</rule_name>"));
        EXPECT_NE(string::npos, ss.str().find("<item_name>default</item_name></step>"));
    }
...
TEST_F(CrushWrapperTest, distance)
{
    ...
    int bno;
    int r = c.add_bucket(0, CRUSH_BUCKET_STRAW, CRUSH_HASH_DEFAULT, 3, 0, NULL, NULL, &bno);
    ASSERT_EQ(0, r);
    ASSERT_EQ(-1, bno);
...
TEST_F(CrushWrapperTest, choose_args_compat)
{
    ...
    ceph_assert(c.add_simple_rule("rule1", "r11", "host", "", "firstn", pg_pool_t::TYPE_ERASURE) >= 0);
...
TEST_F(CrushWrapperTest, trim_roots_with_class)
{
    ...
    ASSERT_EQ(c.device_class_clone(root_id, cl, old_class_bucket, used_ids, &clone_id, &cmap_item_weight), 0);
...
TEST_F(CrushWrapperTest, device_class_clone)
{
    ...
    int other_clone_id;
    ASSERT_EQ(c.device_class_clone(root_id, cl, old_class_bucket, used_ids, &other_clone_id, &cmap_item_weight), 0);
    ASSERT_EQ(clone_id, other_clone_id);
    // invalid arguments
    ASSERT_EQ(c.device_class_clone(12345, cl, old_class_bucket, used_ids, &other_clone_id, &cmap_item_weight), -ECHILD);
    ASSERT_EQ(c.device_class_clone(root_id, 12345, old_class_bucket, used_ids, &other_clone_id, &cmap_item_weight),
              -EBADF);
}
...
TEST_F(CrushWrapperTest, split_id_class)
{
    ...
    int clone_id;
    ASSERT_EQ(c.device_class_clone(item_id, class_id, old_class_bucket, used_ids, &clone_id, &cmap_item_weight), 0);
...
TEST_F(CrushWrapperTest, try_remap_rule)
{
    ...
    c.set_type_name(3, "root");
    int bno;
    int r = c.add_bucket(0, CRUSH_BUCKET_STRAW2, CRUSH_HASH_DEFAULT, 3, 0, NULL, NULL, &bno);
    ASSERT_EQ(0, r);
    ASSERT_EQ(-1, bno);
    c.set_item_name(bno, "default");
    ...
    // take + choose device + emit
    {
        ...
        vector<int> out;
        int r = c.try_remap_rule(cct, rule, 3, overfull, underfull, more_underfull, orig, &out);
        ...
        r = c.try_remap_rule(cct, rule, 3, overfull, underfull, more_underfull, orig, &out);
        cout << orig << " -> r = " << (int)r << " out " << out << std::endl;
        ASSERT_EQ(r, 0);
        ASSERT_EQ(3u, out.size());
        ASSERT_EQ(1, out[0]);
...
// Local Variables:
// compile-command: "cd ../../../build ; make -j4 unittest_crush_wrapper && valgrind --tool=memcheck
// bin/unittest_crush_wrapper" End:
