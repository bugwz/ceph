    ...
    if (temp_dir.empty()) {
        ...
        if (!fs::exists(oss.str())) {
            ...
            if (!fs::create_directory(oss.str(), ec)) {
                cerr << "failed to create temp directory '" << temp_dir << "' " << ec.message() << std::endl;
                return "";
            }
...
static int create_tempfile(const std::string& fname, const char* text)
{
    ...
    if (!fp) {
        int err = errno;
        cerr << "Failed to write file '" << fname << "' to temp directory '" << get_temp_dir() << "'. "
             << cpp_strerror(err) << std::endl;
        return err;
    }
    std::shared_ptr<FILE> fpp(fp, fclose);
    if (unlink_idx >= MAX_FILES_TO_DELETE) return -ENOBUFS;
    if (unlink_idx == 0) {
    ...
    if (res != strlen_text) {
        int err = errno;
        cerr << "fwrite error while writing to " << fname << ": " << cpp_strerror(err) << std::endl;
        return err;
    }
    return 0;
}
