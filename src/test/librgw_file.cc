...
TEST(LibRGW, LIST_BUCKETS)
{
    ...
    bool eof = false;
    uint64_t offset = 0;
    int ret = rgw_readdir(fs, fs->root_fh, &offset, r1_cb, &fids1, &eof, RGW_READDIR_FLAG_NONE);
    for (auto& fid : fids1) {
...
TEST(LibRGW, LOOKUP_BUCKETS)
{
    ...
    for (auto& fid_row : bucket_matrix) {
        ...
        ASSERT_EQ(
            0,
            rgw_lookup(fs, fs->root_fh, get<0>(fid).c_str(), &rgw_fh, nullptr /* stat */, 0 /* mask */, 0 /* flags */));
        get<2>(fid) = rgw_fh;
...
TEST(LibRGW, LIST_OBJECTS)
{
    ...
    for (auto& fid_row : bucket_matrix) {
        ...
        bool eof = false;
        uint64_t offset = 0;
        int ret = rgw_readdir(fs, bucket_fh, &offset, r2_cb, &obj_vector, &eof, RGW_READDIR_FLAG_NONE);
        for (auto& obj : obj_vector) {
            std::cout << "fname: " << get<0>(obj) << " fid: " << get<1>(obj) << std::endl;
        }
        ASSERT_EQ(ret, 0);
    }
}
    ...
    if (do_getattr) {
        ...
        for (auto& fid_row : bucket_matrix) {
            ...
            for (auto& obj : obj_vector) {
                ...
                ret = rgw_lookup(
                    fs, bucket_fh, get<0>(obj).c_str(), &obj_fh, nullptr /* stat */, 0 /* mask */, 0 /* flags */);
                ASSERT_EQ(ret, 0);
...
int main(int argc, char* argv[])
{
    ...
    for (auto arg_iter = args.begin(); arg_iter != args.end();) {
        ...
        else if (ceph_argparse_flag(args, arg_iter, "--getattr", (char*)nullptr)) {
            do_getattr = true;
        }
        else {
            ++arg_iter;
