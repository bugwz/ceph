...
TEST(WeightedShuffle, Basic)
{
    ...
    for (auto i = 0; i < samples; i++) {
        weighted_shuffle(begin(choices), end(choices), begin(weights), end(weights), std::mt19937{rd()});
        for (size_t j = 0; j < choices.size(); ++j) ++frequency[choices[j]][j];
    }
...
TEST(WeightedShuffle, ZeroedWeights)
{
    ...
    for (char ch : choices) {
        ...
        ASSERT_EQ(std::accumulate(begin(frequency[ch]), end(frequency[ch]), 0), samples);
