...
public:
    void tick()
    {
        ...
        if (do_shutdown || (tick_seconds <= 0)) {
            std::cout << __func__ << " " << (do_shutdown ? "shutdown" : "stop ticking") << std::endl;
            return;
        }
        _tick();
        timer.add_event_after(tick_seconds, new C_Tick(this));
    ...
    virtual void print(ostream& out) { out << "stub(" << get_name() << ")"; }

    void wait()
    {
        if (messenger != NULL) messenger->wait();
    }
...
protected:
    ...
    const string get_name() override { return "client"; }
...
public:
        ...
        , mon_osd_rng(STUB_MON_OSD_FIRST, STUB_MON_OSD_LAST)
    {
        ...
        std::string public_msgr_type = cct->_conf->ms_public_type.empty() ? cct->_conf.get_val<std::string>("ms_type")
                                                                          : cct->_conf->ms_public_type;
        messenger.reset(
            Messenger::create(cct, public_msgr_type, entity_name_t::OSD(whoami), ss.str().c_str(), getpid()));

        Throttle throttler(g_ceph_context, "osd_client_bytes", g_conf()->osd_client_message_size_cap);

        messenger->set_default_policy(Messenger::Policy::stateless_server(0));
        messenger->set_policy_throttlers(entity_name_t::TYPE_CLIENT, &throttler, NULL);
        messenger->set_policy(
            entity_name_t::TYPE_MON,
            Messenger::Policy::lossy_client(CEPH_FEATURE_UID | CEPH_FEATURE_PGID64 | CEPH_FEATURE_OSDENC));
        messenger->set_policy(entity_name_t::TYPE_OSD, Messenger::Policy::stateless_server(0));

        dout(10) << __func__ << " public addr " << g_conf()->public_addr << dendl;
        int err = messenger->bind(g_conf()->public_addr);
        if (err < 0) exit(1);

        if (monc.build_initial_monmap() < 0) exit(1);
    ...
    void boot()
    {
        ...
        if ((last_boot_attempt > 0.0) && ((now - last_boot_attempt)) <= STUB_BOOT_INTERVAL) {
    ...
    void auto_create_pgs()
    {
        ...
        dout(10) << __func__ << ": " << (has_pgs ? "has pgs; ignore" : "create pgs") << dendl;
        if (has_pgs) return;

        if (!osdmap.get_epoch()) {
            dout(1) << __func__ << " still don't have osdmap; reschedule pg creation" << dendl;
    ...
    void modify_pg(pg_t pgid)
    {
        ...
        if (s.state & PG_STATE_CLEAN) s.last_clean = now;
        s.last_change = now;
        s.reported_seq++;
    ...
    void modify_pgs()
    {
        ...
        while ((int)pgs_pos.size() < num_pgs) pgs_pos.insert(pg_rng(gen));
    ...
    void op_failure() { dout(10) << __func__ << dendl; }
    ...
    void op_pgstats()
    {
        ...
        modify_pgs();
        if (!pgs_changes.empty()) send_pg_stats();
    ...
    void _tick() override
    {
        ...
        int op = op_rng(gen);
        switch (op) {
        case STUB_MON_OSD_ALIVE: op_alive(); break;
        case STUB_MON_OSD_PGTEMP: op_pgtemp(); break;
        case STUB_MON_OSD_FAILURE: op_failure(); break;
        case STUB_MON_OSD_PGSTATS: op_pgstats(); break;
        case STUB_MON_LOG: op_log(); break;
        }
    }
    ...
    void handle_pg_create(MOSDPGCreate* m)
    {
        ...
        if (m->epoch < osdmap.get_epoch()) {
            std::cout << __func__ << " epoch " << m->epoch << " < " << osdmap.get_epoch() << "; dropping" << std::endl;
            m->put();
            return;
        }

        for (map<pg_t, pg_create_t>::iterator it = m->mkpg.begin(); it != m->mkpg.end(); ++it) {
            pg_create_t& c = it->second;
            std::cout << __func__ << " pg " << it->first << " created " << c.created << " parent " << c.parent
                      << std::endl;
            if (pgs.count(it->first)) {
                std::cout << __func__ << " pg " << it->first << " exists; skipping" << std::endl;
                continue;
            }
    ...
    void handle_osd_map(MOSDMap* m)
    {
        ...
        if (first > osdmap.get_epoch() + 1) {
            ...
            if ((m->oldest_map < first && osdmap.get_epoch() == 0) || m->oldest_map <= osdmap.get_epoch()) {
        ...
        for (epoch_t e = start_full; e <= last; e++) {
            ...
            if (it == m->incremental_maps.end()) continue;
            ...
            if (err < 0) {
                derr << "osd." << whoami << "::" << __func__ << "** ERROR: applying incremental: " << cpp_strerror(err)
                     << dendl;
        ...
        *_dout << dendl;
        ...
        if (osdmap.is_up(whoami) && osdmap.get_addrs(whoami) == messenger->get_myaddrs()) {
    ...
    bool ms_dispatch(Message* m) override
    {
        ...
        switch (m->get_type()) {
        case MSG_OSD_PG_CREATE: handle_pg_create((MOSDPGCreate*)m); break;
        case CEPH_MSG_OSD_MAP: handle_osd_map((MOSDMap*)m); break;
        default: m->put(); break;
        }
        return true;
    }
...
void usage()
{
    ...
    std::cout << "usage: " << our_name << " <--stub-id ID> [--stub-id ID...]" << std::endl;
    std::cout << "\n\
Global Options:\n\
...
int main(int argc, const char* argv[])
{
    ...
    auto cct =
        global_init(NULL, args, CEPH_ENTITY_TYPE_OSD, CODE_ENVIRONMENT_UTILITY, CINIT_FLAG_NO_DEFAULT_CONFIG_FILE);
    ...
    for (std::vector<const char*>::iterator i = args.begin(); i != args.end();) {
        ...
        if (ceph_argparse_double_dash(args, i)) {
            ...
            for (; first <= last; ++first) stub_ids.insert(first);
        }
            ...
            if ((duration == 0) && (!err.empty())) {
                std::cerr << "** error parsing '--duration " << val << "': '" << err << std::endl;
                exit(1);
            }
        }
    ...
    if (stub_ids.empty()) {
        std::cerr << "** error: must specify at least one '--stub-id <ID>'" << std::endl;
        usage();
        return 1;
    }
...

    {
        ...
        shutdown_timer->init();
        if (duration != 0) {
            std::cout << __func__ << " run test for " << duration << " seconds" << std::endl;
