...
struct Election
{
    ...
    void queue_stable_or_timeout(int from, int to, function<void()> m, function<void()> t);
...
struct Owner : public ElectionOwner, RankProvider
{
    ...
    string prefix_str;
    Owner(int r, ElectionLogic::election_strategy es, double tracker_halflife, Election* p)
        : parent(p)
        , rank(r)
        , persisted_epoch(0)
        , ever_joined(false)
    ...
    void cancel_timer() { timer_steps = -1; }
    ...
    {
        ldout(g_ceph_context, 2) << "election epoch " << logic.get_epoch() << " timed out for " << rank
                                 << ", electing me:" << logic.electing_me << ", acked_me:" << logic.acked_me << dendl;
        ceph_assert(!rank_deleted);
        logic.end_election_period();
    }
    void victory_timeout()
    {
        ldout(g_ceph_context, 2) << "victory epoch " << logic.get_epoch() << " timed out for " << rank
                                 << ", electing me:" << logic.electing_me << ", acked_me:" << logic.acked_me << dendl;
        ceph_assert(!rank_deleted);
        reset_election();
    }
    void encode_scores(bufferlist& bl) { encode(peer_tracker, bl); }
    void send_pings()
    {
        ceph_assert(!rank_deleted);
        if (!parent->ping_interval || parent->timesteps_run % parent->ping_interval != 0) {
            return;
        }
        ...
        for (int i = 0; i < parent->get_paxos_size(); ++i) {
            ...
            parent->queue_stable_or_timeout(
                rank, i, [o, r = rank, bl] { o->receive_ping(r, bl); }, [o, r = rank] { o->receive_ping_timeout(r); });
        }
    }
    ...
    const char* prefix_name() const { return prefix_str.c_str(); }
    int timestep_count() const { return parent->timesteps_run; }
};

Election::Election(int c, ElectionLogic::election_strategy es, int pingi, double tracker_halflife)
    : count(c)
    , election_strategy(es)
    , ping_interval(pingi)
    , pending_election_messages(0)
    , timesteps_run(0)
    , last_quorum_change(0)
    , last_quorum_formed(-1)
{
...
void Election::queue_stable_or_timeout(int from, int to, function<void()> m, function<void()> t)
{
    if (blocked_messages[from].count(to)) {
...
{
    Owner* o = electors[to];
    queue_election_message(from, to, [o, from, e] { o->receive_ack(from, e); });
}
...
void Election::propose_to(int from, int to, epoch_t e, bufferlist& cbl)
{
    ...
    queue_election_message(from, to, [o, from, e, oct] { o->receive_propose(from, e, oct); });
}

void Election::claim_victory(int from, int to, epoch_t e, const set<int>& members)
{
    Owner* o = electors[to];
    queue_election_message(from, to, [o, from, e, members] { o->receive_victory_claim(from, e, members); });
}

void Election::accept_victory(int from, int to, epoch_t e)
{
    Owner* o = electors[to];
    queue_election_message(from, to, [o, from, e] { o->receive_victory_ack(from, e); });
}
...
{
    ldout(g_ceph_context, 1) << "quorum_stable? last formed:" << last_quorum_formed << ", last changed "
                             << last_quorum_change << ", last reported members " << last_quorum_reported << dendl;
    ...
    if (timesteps_run - timesteps_stable > last_quorum_change) return true;
...
void Election::remove_elector(int rank)
{
    ...
    for (auto bi = blocked_messages.begin(); bi != blocked_messages.end();) {
        ...
        for (auto i = bi->second.upper_bound(rank); i != bi->second.end();) {
...
void netsplit_with_disallowed_tiebreaker_converges(ElectionLogic::election_strategy strategy)
{
    ...
    for (int i = 0; i < 5; ++i) {
        netsplit();
        call_elections();
        election.run_timesteps(
            15);   // tests fail when I run 10 because 0 and 1 time out on same timestamp for some reason, why?
...
#define test_classic(utest)            \
    TEST(classic, utest)               \
    {                                  \
        utest(ElectionLogic::CLASSIC); \
    }

#define test_disallowed(utest)          \
    TEST(disallowed, utest)             \
    {                                   \
        utest(ElectionLogic::DISALLOW); \
    }

#define test_connectivity(utest)            \
    TEST(connectivity, utest)               \
    {                                       \
        utest(ElectionLogic::CONNECTIVITY); \
    }


// TODO: test for expected failures; gtest probably supports that?
test_classic(single_startup_election_completes) test_classic(everybody_starts_completes)
    test_classic(blocked_connection_continues_election) test_classic(converges_after_flapping)

        test_disallowed(single_startup_election_completes) test_disallowed(everybody_starts_completes)
            test_disallowed(blocked_connection_continues_election) test_disallowed(disallowed_doesnt_win)
    ...
    test_connectivity(everybody_starts_completes) test_connectivity(blocked_connection_converges_election)
        test_connectivity(disallowed_doesnt_win) test_connectivity(converges_after_flapping)
            test_connectivity(converges_while_flapping) test_connectivity(netsplit_with_disallowed_tiebreaker_converges)
                test_connectivity(handles_singly_connected_peon) test_connectivity(handles_disagreeing_connectivity)
                    test_connectivity(handles_outdated_scoring) test_connectivity(handles_removing_ranks)
