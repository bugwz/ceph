...
{
protected:
    virtual void SetUp() { g_ceph_context->_conf->subsys.set_log_level(dout_subsys, TEST_DEBUG); }
...
#ifdef HAVE_RES_NQUERY
    EXPECT_CALL(*resolvH, res_nsearch(_, StrEq("_ceph-mon._tcp"), C_IN, T_SRV, _, _)).WillOnce(Return(0));
#else
    EXPECT_CALL(*resolvH, res_search(StrEq("_ceph-mon._tcp"), C_IN, T_SRV, _, _)).WillOnce(Return(0));
#endif
