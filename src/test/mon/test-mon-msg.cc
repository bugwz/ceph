...
public:
    ...
    int init_messenger()
    {
        ...
        msg = Messenger::create(cct, public_msgr_type, entity_name_t::CLIENT(-1), "test-mon-msg", 0);
    ...
    MonMap* get_monmap() { return &monc.monmap; }
    ...
    int init()
    {
        ...
        if (err < 0) goto fail;
        err = init_monc();
        if (err < 0) goto fail_msgr;
        err = post_init();
        if (err < 0) goto fail_monc;
        return 0;
    fail_monc:
        shutdown_monc();
    fail_msgr:
    ...
    bool ms_dispatch(Message* m) override { return handle_message(m); }
...
{
    Message* m = new MMonProbe(get_monmap()->fsid, MMonProbe::OP_PROBE, "b", false, ceph_release());
...
{
    Message* m = new MMonJoin(get_monmap()->fsid, string("client"), msg->get_myaddrs());
