...
TEST(DirectMessenger, SyncDispatch)
{
    ...
    DirectMessenger client(cct, entity_name_t::CLIENT(1), "client", 0, new FastStrategy());
    DirectMessenger server(cct, entity_name_t::CLIENT(2), "server", 0, new FastStrategy());
...
TEST(DirectMessenger, AsyncDispatch)
{
    ...
    DirectMessenger client(cct, entity_name_t::CLIENT(1), "client", 0, new QueueStrategy(1));
    DirectMessenger server(cct, entity_name_t::CLIENT(2), "server", 0, new FastStrategy());
...
TEST(DirectMessenger, SendShutdown)
{
    ...
    std::unique_ptr<DirectMessenger> client{
        new DirectMessenger(cct, entity_name_t::CLIENT(1), "client", 0, new FastStrategy())};
    DirectMessenger server(cct, entity_name_t::CLIENT(2), "server", 0, new FastStrategy());
...
int main(int argc, char** argv)
{
    ...
    auto cct =
        global_init(nullptr, args, CEPH_ENTITY_TYPE_ANY, CODE_ENVIRONMENT_DAEMON, CINIT_FLAG_NO_DEFAULT_CONFIG_FILE);
