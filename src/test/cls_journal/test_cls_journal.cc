...
TEST_F(TestClsJournal, Create)
{
    ...
    C_SaferCond cond;
    client::get_immutable_metadata(ioctx, oid, &read_order, &read_splay_width, &read_pool_id, &cond);
...
TEST_F(TestClsJournal, MinimumSet)
{
    ...
    client::get_mutable_metadata(ioctx, oid, &read_minimum_set, &read_active_set, &read_clients, &cond);
    ASSERT_EQ(0, cond.wait());
...
TEST_F(TestClsJournal, ClientUpdateData)
{
    ...
    ASSERT_EQ(-ENOENT, client::client_update_data(ioctx, oid, "id1", bufferlist()));
...
TEST_F(TestClsJournal, ClientUpdateState)
{
    ...
    ASSERT_EQ(-ENOENT, client::client_update_state(ioctx, oid, "id1", CLIENT_STATE_DISCONNECTED));

    ASSERT_EQ(0, client::client_register(ioctx, oid, "id1", bufferlist()));

    bufferlist data;
    data.append(std::string(128, '1'));
    ASSERT_EQ(0, client::client_update_state(ioctx, oid, "id1", CLIENT_STATE_DISCONNECTED));
...
TEST_F(TestClsJournal, ClientUnregisterPruneTags)
{
    ...
    ASSERT_EQ(0, client::tag_create(ioctx, oid, 0, Tag::TAG_CLASS_NEW, bufferlist()));
    ASSERT_EQ(0, client::tag_create(ioctx, oid, 1, Tag::TAG_CLASS_NEW, bufferlist()));
    ...
    std::set<Tag> tags;
    ASSERT_EQ(0, client::tag_list(ioctx, oid, "id1", boost::optional<uint64_t>(), &tags));
...
TEST_F(TestClsJournal, ClientCommit)
{
    ...
    object_positions = {cls::journal::ObjectPosition(0, 234, 120), cls::journal::ObjectPosition(3, 235, 121)};
    ...
    std::set<Client> expected_clients = {Client("id1", bufferlist(), object_set_position)};
    ASSERT_EQ(expected_clients, clients);
}
...
TEST_F(TestClsJournal, ClientCommitInvalid)
{
    ...
    cls::journal::ObjectPositions object_positions;
    object_positions = {cls::journal::ObjectPosition(0, 234, 120),
...
TEST_F(TestClsJournal, TagCreate)
{
    ...
    ASSERT_EQ(-ENOENT, client::tag_create(ioctx, oid, 0, Tag::TAG_CLASS_NEW, bufferlist()));

    ASSERT_EQ(0, client::create(ioctx, oid, 2, 2, ioctx.get_id()));
    ASSERT_EQ(0, client::client_register(ioctx, oid, "id1", bufferlist()));

    ASSERT_EQ(-ESTALE, client::tag_create(ioctx, oid, 1, Tag::TAG_CLASS_NEW, bufferlist()));
    ASSERT_EQ(-EINVAL, client::tag_create(ioctx, oid, 0, 1, bufferlist()));

    ASSERT_EQ(0, client::tag_create(ioctx, oid, 0, Tag::TAG_CLASS_NEW, bufferlist()));
    ASSERT_EQ(-EEXIST, client::tag_create(ioctx, oid, 0, Tag::TAG_CLASS_NEW, bufferlist()));
    ASSERT_EQ(0, client::tag_create(ioctx, oid, 1, Tag::TAG_CLASS_NEW, bufferlist()));
...
TEST_F(TestClsJournal, TagList)
{
    ...
    ASSERT_EQ(0, client::tag_list(ioctx, oid, "id1", boost::optional<uint64_t>(0), &tags));
...
TEST_F(TestClsJournal, Append)
{
    ...
    outbl.clear();
    std::map<uint64_t, uint64_t> expected_m = {{0, bl.length()}, {pad_len, bl.length()}};
