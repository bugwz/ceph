...
TEST(CommonIPAddr, TestV4_SkipLoopback)
{
    ...
    result =
        find_ip_in_subnet_list(nullptr, (struct ifaddrs*)&one, CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_IPV6, "", "");
    ...
    result = find_ip_in_subnet_list(
        nullptr, (struct ifaddrs*)&one, CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_IPV6, "127.0.0.0/8", "");
...
TEST(CommonIPAddr, TestV6_SkipLoopback)
{
    ...
    // the subnet criteria leaves us no choice but the UP loopback address
    result = find_ip_in_subnet_list(
        nullptr, (struct ifaddrs*)&one, CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_IPV6, "::1/128", "");
    ASSERT_EQ((struct sockaddr*)&a_two, result);
}
...
TEST(pick_address, find_ip_in_subnet_list)
{
    ...
    // match by network
    result = find_ip_in_subnet_list(cct.get(), &one, CEPH_PICK_ADDRESS_IPV4, "10.1.0.0/16", "eth0");
    ASSERT_EQ((struct sockaddr*)&a_one, result);

    result = find_ip_in_subnet_list(cct.get(), &one, CEPH_PICK_ADDRESS_IPV4, "10.2.0.0/16", "eth1");
    ASSERT_EQ((struct sockaddr*)&a_two, result);

    // match by eth name
    result = find_ip_in_subnet_list(cct.get(), &one, CEPH_PICK_ADDRESS_IPV4, "10.0.0.0/8", "eth0");
    ASSERT_EQ((struct sockaddr*)&a_one, result);

    result = find_ip_in_subnet_list(cct.get(), &one, CEPH_PICK_ADDRESS_IPV4, "10.0.0.0/8", "eth1");
    ASSERT_EQ((struct sockaddr*)&a_two, result);

    result = find_ip_in_subnet_list(cct.get(), &one, CEPH_PICK_ADDRESS_IPV6, "2001::/16", "eth1");
    ASSERT_EQ((struct sockaddr*)&a_three, result);
}
...
TEST(pick_address, filtering)
{
    ...
    entity_addrvec_t av;
    {
        int r = pick_addresses(
            cct.get(), CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_MSGR1, &one, &av);
        cout << av << std::endl;
        ASSERT_EQ(0, r);
    ...
    {
        int r = pick_addresses(
            cct.get(), CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_IPV6 | CEPH_PICK_ADDRESS_MSGR1, &one, &av);
        cout << av << std::endl;
        ASSERT_EQ(0, r);
        ASSERT_EQ(1u, av.v.size());
    ...
    }
    {
        ...
        int r = pick_addresses(
            cct.get(), CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_MSGR2, &one, &av);
        cout << av << std::endl;
        ASSERT_EQ(0, r);
        ASSERT_EQ(1u, av.v.size());
    ...
    }
    {
        ...
        cct->_conf.set_val("cluster_network", "10.1.0.0/16");
        int r = pick_addresses(
            cct.get(), CEPH_PICK_ADDRESS_CLUSTER | CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_MSGR1, &one, &av);
        cout << av << std::endl;
        ASSERT_EQ(0, r);
        ASSERT_EQ(1u, av.v.size());
...

    {
        ...
        int r = pick_addresses(
            cct.get(), CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_IPV6 | CEPH_PICK_ADDRESS_MSGR2, &one, &av);
        cout << av << std::endl;
        ASSERT_EQ(0, r);
        ASSERT_EQ(1u, av.v.size());
    ...
    }
    {
        ...
        int r = pick_addresses(
            cct.get(),
            CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_IPV6 | CEPH_PICK_ADDRESS_MSGR2,
            &one,
            &av);
        cout << av << std::endl;
        ASSERT_EQ(0, r);
    ...
    }
    {
        ...
        int r = pick_addresses(cct.get(),
                               CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_IPV6 |
                                   CEPH_PICK_ADDRESS_MSGR1 | CEPH_PICK_ADDRESS_PREFER_IPV4,
                               &one,
                               &av);
        cout << av << std::endl;
        ASSERT_EQ(0, r);
...

    {
        ...
        int r = pick_addresses(
            cct.get(),
            CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_IPV6 | CEPH_PICK_ADDRESS_MSGR1 | CEPH_PICK_ADDRESS_MSGR2,
            &one,
            &av);
        cout << av << std::endl;
        ASSERT_EQ(0, r);
...

    {
        int r = pick_addresses(
            cct.get(),
            CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_IPV4 | CEPH_PICK_ADDRESS_MSGR1 | CEPH_PICK_ADDRESS_MSGR2,
            &one,
            &av);
        cout << av << std::endl;
        ASSERT_EQ(0, r);
...
TEST(pick_address, ipv4_ipv6_enabled)
{
    ...
    entity_addrvec_t av;
    {
        int r = pick_addresses(cct.get(), CEPH_PICK_ADDRESS_PUBLIC | CEPH_PICK_ADDRESS_MSGR1, &one, &av);
        cout << av << std::endl;
        ASSERT_EQ(-1, r);
    }
}
