...
{
public:
    void notify(uint8_t opcode, uint64_t ver, bufferlist& bl) override { sem_post(&sem); }
};
...
struct WatcherUnwatcher : public Thread
{
    ...
    void* entry() override
    {
        ...
        while (!stop_flag) {
            ...
            if (r == 0) ioctx.unwatch("foo", handle);
            ioctx.close();
        }
        return NULL;
    }
...
INSTANTIATE_TEST_SUITE_P(WatchStressTests, WatchStress, ::testing::Values("", "cache"));
