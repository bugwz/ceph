...
TEST(ErasureCodeTest, create_rule)
{
    ...
    vector<__u32> weight;
    for (int o = 0; o < c->get_max_devices(); o++) weight.push_back(0x10000);
...
TEST(ErasureCodeLrc, layers_description)
{
    ...
    EXPECT_EQ(ERROR_LRC_DESCRIPTION, lrc.layers_description(profile, &description, &cerr));

    {
        const char* description_string = "\"not an array\"";
        profile["layers"] = description_string;
        EXPECT_EQ(ERROR_LRC_ARRAY, lrc.layers_description(profile, &description, &cerr));
    }
    {
        const char* description_string = "invalid json";
        profile["layers"] = description_string;
        EXPECT_EQ(ERROR_LRC_PARSE_JSON, lrc.layers_description(profile, &description, &cerr));
    }
    {
...
{
    {
        ...
        json_spirit::mArray description;
        EXPECT_EQ(0, lrc.layers_description(profile, &description, &cerr));
        EXPECT_EQ(ERROR_LRC_ARRAY, lrc.layers_parse(description_string, description, &cerr));
    }

    {
        ...
        json_spirit::mArray description;
        EXPECT_EQ(0, lrc.layers_description(profile, &description, &cerr));
        EXPECT_EQ(ERROR_LRC_STR, lrc.layers_parse(description_string, description, &cerr));
    }

    {
        ...
        json_spirit::mArray description;
        EXPECT_EQ(0, lrc.layers_description(profile, &description, &cerr));
        EXPECT_EQ(ERROR_LRC_CONFIG_OPTIONS, lrc.layers_parse(description_string, description, &cerr));
    }
...
TEST(ErasureCodeLrc, minimum_to_decode)
{
    ...
    // locally repairable erasure
    {
        ...
        profile["layers"] = description_string;
        EXPECT_EQ(0, lrc.init(profile, &cerr));
        EXPECT_EQ(profile["mapping"].length(), lrc.get_chunk_count());
        {
            ...
            set<int> available_chunks;
            for (int i = 0; i < (int)lrc.get_chunk_count() - 1; i++) available_chunks.insert(i);
        ...
        }
        {
            ...
            set<int> available_chunks;
            for (int i = 1; i < (int)lrc.get_chunk_count(); i++) available_chunks.insert(i);
            set<int> minimum;
            EXPECT_EQ(0, lrc._minimum_to_decode(want_to_read, available_chunks, &minimum));
            set<int> expected_minimum;
...
TEST(ErasureCodeLrc, encode_decode_2)
{
    ...
    {
        set<int> want_to_read;
        for (unsigned int i = 0; i < lrc.get_chunk_count(); i++) want_to_read.insert(i);
        map<int, bufferlist> chunks;
        chunks[1] = encoded[1];
        chunks[3] = encoded[3];
        chunks[5] = encoded[5];
