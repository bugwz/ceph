...
void IsaErasureCodeTest::compare_chunks(bufferlist& in, map<int, bufferlist>& encoded)
{
    ...
    for (unsigned i = 0; i < encoded.size(); i++) {
        if (i * chunk_size >= object_size) break;
...
void IsaErasureCodeTest::encode_decode(unsigned object_size)
{
    ...
    EXPECT_EQ(0, Isa.encode(set<int>(want_to_encode, want_to_encode + 4), in, &encoded));
    ...
    // all chunks are available
    {
        ...
        EXPECT_EQ(0, Isa._decode(set<int>(want_to_decode, want_to_decode + 2), encoded, &decoded));
    ...
    // one data chunk is missing
    {
        ...
        map<int, bufferlist> decoded;
        EXPECT_EQ(0, Isa._decode(set<int>(want_to_decode, want_to_decode + 1), degraded, &decoded));
    ...
    // one data and one coding chunk is missing
    {
        ...
        map<int, bufferlist> decoded;
        EXPECT_EQ(0, Isa._decode(set<int>(want_to_decode, want_to_decode + 2), degraded, &decoded));
        // always decode all, regardless of want_to_decode
        EXPECT_EQ(4u, decoded.size());
        EXPECT_EQ(chunk_size, decoded[1].length());
        EXPECT_EQ(0, memcmp(decoded[3].c_str(), enc3.c_str(), chunk_size));
...
TEST_F(IsaErasureCodeTest, minimum_to_decode)
{
    ...
    //
    {
        ...
        EXPECT_EQ(0, Isa._minimum_to_decode(want_to_read, available_chunks, &minimum));
    ...
    //
    {
        ...
        EXPECT_EQ(-EIO, Isa._minimum_to_decode(want_to_read, available_chunks, &minimum));
    }
    //
...
bool DecodeAndVerify(ErasureCodeIsaDefault& Isa, map<int, bufferlist>& degraded, set<int> want_to_decode,
                     buffer::ptr* enc, int length)
{
    map<int, bufferlist> decoded;
    bool ok;
