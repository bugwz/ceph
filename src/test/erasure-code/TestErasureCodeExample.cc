...
TEST(ErasureCodeExample, minimum_to_decode)
{
    ...
    {
        set<int> minimum;
        EXPECT_EQ(-EIO, example._minimum_to_decode(want_to_read, available_chunks, &minimum));
    }
    available_chunks.insert(0);
    available_chunks.insert(2);
    {
        set<int> minimum;
        EXPECT_EQ(0, example._minimum_to_decode(want_to_read, available_chunks, &minimum));
...
TEST(ErasureCodeExample, minimum_to_decode_with_cost)
{
    ...
    {
        set<int> minimum;
        EXPECT_EQ(-EIO, example.minimum_to_decode_with_cost(want_to_read, available, &minimum));
    }
    available[0] = 1;
    available[2] = 1;
    {
        set<int> minimum;
        EXPECT_EQ(0, example.minimum_to_decode_with_cost(want_to_read, available, &minimum));
        EXPECT_EQ(2u, minimum.size());
        EXPECT_EQ(1u, minimum.count(0));
        EXPECT_EQ(1u, minimum.count(2));
    }
...
TEST(ErasureCodeExample, encode_decode)
{
    ...
    for (unsigned int i = 0; i < example.get_chunk_count(); i++) want_to_encode.insert(i);
    ...
    // all chunks are available
    {
        ...
        EXPECT_EQ(0, example._decode(set<int>(want_to_decode, want_to_decode + 2), encoded, &decoded));
    ...
    // one chunk is missing
    {
        ...
        int want_to_decode[] = {0, 1};
        map<int, bufferlist> decoded;
        EXPECT_EQ(0, example._decode(set<int>(want_to_decode, want_to_decode + 2), degraded, &decoded));
        EXPECT_EQ(2u, decoded.size());
        EXPECT_EQ(3u, decoded[0].length());
        EXPECT_EQ('A', decoded[0][0]);
        EXPECT_EQ('B', decoded[0][1]);
...
#define LARGE_ENOUGH 2048
    ...
    map<int, bufferlist> encoded;
    EXPECT_EQ(0, example.encode(set<int>(want_to_encode, want_to_encode + 3), in, &encoded));
