...
#define LARGE_ENOUGH 2048
    ...
    EXPECT_EQ(0, clay.encode(set<int>(want_to_encode, want_to_encode + 4), in, &encoded));
    ...
    // all chunks are available
    {
        ...
        EXPECT_EQ(0, clay._decode(set<int>(want_to_decode, want_to_decode + 2), encoded, &decoded));
    ...
    for (int i = 1; i < 4; i++) {
        for (int j = 0; j < i; j++) {
            ...
            map<int, bufferlist> decoded;
            EXPECT_EQ(0, clay._decode(set<int>(want_to_decode, want_to_decode + 2), degraded, &decoded));
...
#define LARGE_ENOUGH 2048
    ...
    map<int, bufferlist> encoded;
    EXPECT_EQ(0, clay.encode(set<int>(want_to_encode, want_to_encode + 6), in, &encoded));
    ...
    // all chunks are available
    {
        ...
        map<int, bufferlist> decoded;
        EXPECT_EQ(0, clay._decode(set<int>(want_to_decode, want_to_decode + 3), encoded, &decoded));
    ...
    for (int i = 2; i < 6; i++) {
        for (int j = 1; j < i; j++) {
            for (int k = 0; k < j; k++) {
                ...
                map<int, bufferlist> decoded;
                EXPECT_EQ(0, clay._decode(set<int>(want_to_decode, want_to_decode + 3), degraded, &decoded));
...
TEST(ErasureCodeClay, encode_decode_shortening_case)
{
    ...
    map<int, bufferlist> encoded;
    EXPECT_EQ(0, clay.encode(set<int>(want_to_encode, want_to_encode + 7), in, &encoded));
    ...
    // all chunks are available
    {
        ...
        map<int, bufferlist> decoded;
        EXPECT_EQ(0, clay._decode(set<int>(want_to_decode, want_to_decode + 4), encoded, &decoded));
        EXPECT_EQ(4u, decoded.size());
        EXPECT_EQ(length, decoded[0].length());
        EXPECT_EQ(0, memcmp(decoded[0].c_str(), encoded[0].c_str(), length));
        EXPECT_EQ(0, memcmp(decoded[1].c_str(), encoded[1].c_str(), length));
...
TEST(ErasureCodeClay, minimum_to_decode)
{
    ...
    //
    {
        ...
        EXPECT_EQ(0, clay._minimum_to_decode(want_to_read, available_chunks, &minimum));
    ...
    //
    {
        ...
        EXPECT_EQ(-EIO, clay._minimum_to_decode(want_to_read, available_chunks, &minimum));
    }
    //
