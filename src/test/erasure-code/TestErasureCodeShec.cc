...
TEST(ErasureCodeShec, minimum_to_decode_8)
{
    ...
    int r = shec->_minimum_to_decode(want_to_decode, available_chunks, &minimum_chunks);
    EXPECT_EQ(-EINVAL, r);
...
TEST(ErasureCodeShec, encode_1)
{
    ...
    decoded.clear();
    r = shec->_decode(set<int>(want_to_decode, want_to_decode + 2), encoded, &decoded);
...
TEST(ErasureCodeShec, encode_2)
{
    ...
    bufferlist out1, out2, usable;
    // out1 is "encoded"
    for (unsigned int i = 0; i < encoded.size(); ++i) out1.append(encoded[i]);
    // out2 is "decoded"
...
TEST(ErasureCodeShec, decode_1)
{
    ...
    r = shec->_decode(set<int>(want_to_decode, want_to_decode + 7), encoded, &decoded);
    EXPECT_TRUE(shec->matrix != NULL);
    EXPECT_EQ(0, r);
...
TEST(ErasureCodeShec, decode_8)
{
    ...
    r = shec->_decode(set<int>(want_to_decode, want_to_decode + 8), encoded, &decoded);
    EXPECT_EQ(0, r);
    EXPECT_EQ(7u, decoded.size());
    EXPECT_EQ(shec->get_chunk_size(in.length()), encoded[0].length());
...
TEST(ErasureCodeShec, decode_9)
{
    ...
    r = shec->_decode(set<int>(want_to_decode, want_to_decode + 10), encoded, &decoded);
    EXPECT_TRUE(shec->matrix != NULL);
    EXPECT_EQ(0, r);
    EXPECT_EQ(7u, decoded.size());
    EXPECT_EQ(shec->get_chunk_size(in.length()), decoded[0].length());
...
TEST(ErasureCodeShec, decode_10)
{
    ...
    r = shec->_decode(set<int>(want_to_decode, want_to_decode + 7), inchunks, &decoded);
    EXPECT_EQ(-1, r);
...
TEST(ErasureCodeShec, decode_11)
{
    ...
    r = shec->_decode(set<int>(want_to_decode, want_to_decode + 5), inchunks, &decoded);
    EXPECT_EQ(-1, r);
...
TEST(ErasureCodeShec, decode_12)
{
    ...
    // decoded = NULL
    r = shec->_decode(set<int>(want_to_decode, want_to_decode + 7), encoded, NULL);
    EXPECT_NE(0, r);
...
TEST(ErasureCodeShec, decode2_4)
{
    ...
    r = shec->_decode(set<int>(want_to_decode, want_to_decode + 2), degraded, &decoded);
    EXPECT_EQ(-1, r);
...
void* thread5(void* pParam)
{
    ...
    g_flag = 1;
    while (g_flag == 1) {
        shec->_decode(set<int>(want_to_decode, want_to_decode + 2), encoded, &decoded);
        decoded.clear();
    }
    printf("*** thread loop end ***\n");
