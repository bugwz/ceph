...
typedef ::testing::Types<ErasureCodeJerasureReedSolomonVandermonde, ErasureCodeJerasureReedSolomonRAID6,
                         ErasureCodeJerasureCauchyOrig, ErasureCodeJerasureCauchyGood, ErasureCodeJerasureLiberation,
                         ErasureCodeJerasureBlaumRoth, ErasureCodeJerasureLiber8tion>
    JerasureTypes;
...
TYPED_TEST(ErasureCodeTest, encode_decode)
{
    ...
    for (int per_chunk_alignment = 0; per_chunk_alignment < 2; per_chunk_alignment++) {
        TypeParam jerasure;
        ErasureCodeProfile profile;
        profile["k"] = "2";
        profile["m"] = "2";
        profile["packetsize"] = "8";
        profile["jerasure-per-chunk-alignment"] = per_chunk_alignments[per_chunk_alignment];
...
#define LARGE_ENOUGH 2048
        ...
        EXPECT_EQ(0, jerasure.encode(set<int>(want_to_encode, want_to_encode + 4), in, &encoded));
        ...
        // all chunks are available
        {
            ...
            EXPECT_EQ(0, jerasure._decode(set<int>(want_to_decode, want_to_decode + 2), encoded, &decoded));
            EXPECT_EQ(2u, decoded.size());
            EXPECT_EQ(length, decoded[0].length());
            EXPECT_EQ(0, memcmp(decoded[0].c_str(), in.c_str(), length));
            EXPECT_EQ(0, memcmp(decoded[1].c_str(), in.c_str() + length, in.length() - length));
        }
        ...
        // two chunks are missing
        {
            ...
            int want_to_decode[] = {0, 1};
            map<int, bufferlist> decoded;
            EXPECT_EQ(0, jerasure._decode(set<int>(want_to_decode, want_to_decode + 2), degraded, &decoded));
...
TYPED_TEST(ErasureCodeTest, minimum_to_decode)
{
    ...
    //
    {
        ...
        EXPECT_EQ(0, jerasure._minimum_to_decode(want_to_read, available_chunks, &minimum));
    ...
    //
    {
        ...
        EXPECT_EQ(-EIO, jerasure._minimum_to_decode(want_to_read, available_chunks, &minimum));
    }
    //
...
TEST(ErasureCodeTest, create_rule)
{
...

    {
        ...
        for (unsigned i = 0; i < out.size(); ++i) ASSERT_NE(CRUSH_ITEM_NONE, out[i]);
    }
    {
        stringstream ss;
        ErasureCodeJerasureReedSolomonVandermonde jerasure;
