...
public:
    ...
    int run_create();
    int run_check();
    int decode_erasures(ErasureCodeInterfaceRef erasure_code, set<int> erasures, map<int, bufferlist> chunks);
    string content_path();
...
int ErasureCodeNonRegression::setup(int argc, char** argv)
{
    ...
    desc.add_options()("help,h", "produce help message")("stripe-width,s",
                                                         po::value<int>()->default_value(4 * 1024),
        ...
        "base", po::value<string>()->default_value("."), "prefix all paths with base")(
    ...
    cct = global_init(NULL, ceph_options, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_UTILITY, CINIT_FLAG_NO_MON_CONFIG);
...

    {
        stringstream path;
        path << base << "/"
             << "plugin=" << plugin << " stripe-width=" << stripe_width;
        directory = path.str();
    }
...
{
    int ret = 0;
    if (create && (ret = run_create())) return ret;
    if (check && (ret = run_check())) return ret;
    return ret;
}
...
int ErasureCodeNonRegression::run_create()
{
    ...
    string payload;
    for (unsigned j = 0; j < payload_chunk_size; ++j) payload.push_back('a' + (rand() % 26));
    bufferlist in;
    for (unsigned j = 0; j < stripe_width; j += payload_chunk_size) in.append(payload);
    if (stripe_width < in.length()) in.splice(stripe_width, in.length() - stripe_width);
    if (in.write_file(content_path().c_str())) return 1;
    ...
    if (code) return code;
    for (map<int, bufferlist>::iterator chunk = encoded.begin(); chunk != encoded.end(); ++chunk) {
        if (chunk->second.write_file(chunk_path(chunk->first).c_str())) return 1;
    }
    return 0;
}

int ErasureCodeNonRegression::decode_erasures(ErasureCodeInterfaceRef erasure_code, set<int> erasures,
                                              map<int, bufferlist> chunks)
{
    map<int, bufferlist> available;
    for (map<int, bufferlist>::iterator chunk = chunks.begin(); chunk != chunks.end(); ++chunk) {
        if (erasures.count(chunk->first) == 0) available[chunk->first] = chunk->second;
    }
    map<int, bufferlist> decoded;
    int code = erasure_code->decode(erasures, available, &decoded, available.begin()->second.length());
    if (code) return code;
    for (set<int>::iterator erasure = erasures.begin(); erasure != erasures.end(); ++erasure) {
...
int ErasureCodeNonRegression::run_check()
{
    ...
    for (map<int, bufferlist>::iterator chunk = encoded.begin(); chunk != encoded.end(); ++chunk) {
        ...
        if (existing.length() != old.length() || memcmp(existing.c_str(), old.c_str(), old.length())) {
