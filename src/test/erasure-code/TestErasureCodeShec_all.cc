...
TEST_P(ParameterTest, parameter_all)
{
    ...
    for (int w = 1; w <= i_c; w++) {
        ...
        do {
            ...
            result = shec->_minimum_to_decode(want_to_decode, available_chunks, &minimum_chunks);
        ...
        } while (std::prev_permutation(array_want_to_decode, array_want_to_decode + shec->get_chunk_count()));
    }
    ...
    result = shec->minimum_to_decode_with_cost(
        want_to_decode_with_cost, available_chunks_with_cost, &minimum_chunks_with_cost);
    EXPECT_EQ(0, result);
    ...
    result = shec->_decode(set<int>(want_to_decode2, want_to_decode2 + 2), encoded, &decoded);
    EXPECT_EQ(0, result);
    ...
    int rootno;
    crush->add_bucket(0, CRUSH_BUCKET_STRAW, CRUSH_HASH_RJENKINS1, 2, 0, NULL, NULL, &rootno);
...
int main(int argc, char** argv)
{
    ...
    std::cout << "minimum_to_decode:cannot_recover_num = " << g_cannot_recover << std::endl;
    recovery_percentage = 100.0 - (float)(100.0 * g_cannot_recover / (g_recover + g_cannot_recover));
    printf("recovery_percentage:%f\n", recovery_percentage);
    if (recovery_percentage > 99.0) {
        std::cout << "[       OK ] Recovery percentage is more than 99.0%" << std::endl;
    }
    else {
        std::cout << "[       NG ] Recovery percentage is less than 99.0%" << std::endl;
    }
    std::cout << "cannot recovery patterns:" << std::endl;
    for (std::vector<Recover_d>::const_iterator i = cannot_recover.begin(); i != cannot_recover.end(); ++i) {
        std::cout << "---" << std::endl;
        std::cout << "k = " << i->k << ", m = " << i->m << ", c = " << i->c << std::endl;
