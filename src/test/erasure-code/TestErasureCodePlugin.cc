...
TEST_F(ErasureCodePluginRegistryTest, factory_mutex)
{
    ...
    std::thread sleep_for_10_secs([] {
        ...
        instance.factory("hangs", g_conf().get_val<std::string>("erasure_code_dir"), profile, &erasure_code, &cerr);
    });
...
TEST_F(ErasureCodePluginRegistryTest, all)
{
    ...
    EXPECT_EQ(
        -EIO,
        instance.factory("invalid", g_conf().get_val<std::string>("erasure_code_dir"), profile, &erasure_code, &cerr));
    EXPECT_FALSE(erasure_code);
    EXPECT_EQ(-EXDEV,
              instance.factory(
                  "missing_version", g_conf().get_val<std::string>("erasure_code_dir"), profile, &erasure_code, &cerr));
    EXPECT_FALSE(erasure_code);
    EXPECT_EQ(
        -ENOENT,
        instance.factory(
            "missing_entry_point", g_conf().get_val<std::string>("erasure_code_dir"), profile, &erasure_code, &cerr));
    EXPECT_FALSE(erasure_code);
    EXPECT_EQ(
        -ESRCH,
        instance.factory(
            "fail_to_initialize", g_conf().get_val<std::string>("erasure_code_dir"), profile, &erasure_code, &cerr));
    EXPECT_FALSE(erasure_code);
    EXPECT_EQ(
        -EBADF,
        instance.factory(
            "fail_to_register", g_conf().get_val<std::string>("erasure_code_dir"), profile, &erasure_code, &cerr));
    EXPECT_FALSE(erasure_code);
    EXPECT_EQ(
        0,
        instance.factory("example", g_conf().get_val<std::string>("erasure_code_dir"), profile, &erasure_code, &cerr));
