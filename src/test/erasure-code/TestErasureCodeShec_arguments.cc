    ...
    for (int want_count = 0; want_count < 7; ++want_count) {
        for (int want = 1; want < (1 << 7); ++want) {
            ...
            for (int avails = 0; avails < (1 << 7); ++avails) {
                ...
                if (__builtin_popcount(avails) == 2 && __builtin_popcount(want) == 1) {
                    if ((want | avails) == getint(0, 1, 5) || (want | avails) == getint(2, 3, 6)) {
            ...
            for (int avails = 0; avails < (1 << 7); ++avails) {
                ...
                if (__builtin_popcount(avails) == 4) {
                    if ((avails) == getint(0, 1, 2, 3) || (avails) == getint(0, 1, 2, 4) ||
                        (avails) == getint(0, 1, 2, 6) || (avails) == getint(0, 1, 3, 4) ||
                        (avails) == getint(0, 1, 3, 6) || (avails) == getint(0, 1, 4, 6) ||
                        (avails) == getint(0, 2, 3, 4) || (avails) == getint(0, 2, 3, 5) ||
                        (avails) == getint(0, 2, 4, 5) || (avails) == getint(0, 2, 4, 6) ||
                        (avails) == getint(0, 2, 5, 6) || (avails) == getint(0, 3, 4, 5) ||
                        (avails) == getint(0, 3, 4, 6) || (avails) == getint(0, 3, 5, 6) ||
                        (avails) == getint(0, 4, 5, 6) || (avails) == getint(1, 2, 3, 4) ||
                        (avails) == getint(1, 2, 3, 5) || (avails) == getint(1, 2, 4, 5) ||
                        (avails) == getint(1, 2, 4, 6) || (avails) == getint(1, 2, 5, 6) ||
                        (avails) == getint(1, 3, 4, 5) || (avails) == getint(1, 3, 4, 6) ||
                        (avails) == getint(1, 3, 5, 6) || (avails) == getint(1, 4, 5, 6) ||
                        (avails) == getint(2, 3, 4, 5) || (avails) == getint(2, 4, 5, 6) ||
...
TEST(ParameterTest, combination_all)
{
    ...
    map<std::string, std::string>* profile = new map<std::string, std::string>();
    (*profile)["plugin"] = "shec";
    (*profile)["technique"] = "";
        ...
        for (unsigned w2 = 0; w2 <= shec->get_chunk_count(); ++w2) {
            ...
            do {
                do {
                    ...
                    result = shec->minimum_to_decode(want_to_read, available_chunks, &minimum_chunks);
                    int dresult = shec->decode(want_to_read, inchunks, &decoded, shec->get_chunk_size(kObjectSize));
                    ++count_num;
                ...
                } while (std::prev_permutation(array_want_to_read, array_want_to_read + shec->get_chunk_count()));

            } while (std::prev_permutation(array_available_chunks, array_available_chunks + shec->get_chunk_count()));
        }
    }
...
int main(int argc, char** argv)
{
    ...
    std::cout << "minimum_to_decode:unexpected_num = " << unexpected_count << std::endl;
