...
public:
    ...
    {
        std::unique_lock locker{lock};
        return cond.wait_for(locker, std::chrono::seconds(1200), [this] { return notified; });
    }
...
void test_replicated(Rados& cluster, std::string pool_name, const std::string& obj_name)
{
    // May already exist
...
void test_erasure(Rados& cluster, const std::string& pool_name, const std::string& obj_name)
{
    string outs;
    bufferlist inbl;
    int ret;
    ret = cluster.mon_command("{\"prefix\": \"osd erasure-code-profile set\", \"name\": \"testprofile\", \"profile\": "
