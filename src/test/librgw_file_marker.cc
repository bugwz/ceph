...
TEST(LibRGW, MARKER1_SETUP_BUCKET)
{
    ...
    if (!bucket_fh) {
        if (do_create) {
            ...
            ret = rgw_mkdir(fs, fs->root_fh, bucket_name.c_str(), &st, create_mask, &bucket_fh, RGW_MKDIR_FLAG_NONE);
            ASSERT_EQ(ret, 0);
        }
    }
    ...
    (void)rgw_lookup(fs, bucket_fh, marker_dir.c_str(), &marker_fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
    if (!marker_fh) {
        if (do_create) {
...
int main(int argc, char* argv[])
{
    ...
    for (auto arg_iter = args.begin(); arg_iter != args.end();) {
        ...
        else if (ceph_argparse_witharg(args, arg_iter, &val, "--nobjs", (char*)nullptr)) {
