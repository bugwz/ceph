...
void AdminSocketOutput::add_command(const std::string& target, const std::string& command)
{
...
void AdminSocketOutput::add_test(const std::string& target, const std::string& command, bool (*test)(std::string&))
{
...
void AdminSocketOutput::postpone(const std::string& target, const std::string& command)
{
        ...
        if (x.path().extension() == ".asok") {
            for (auto target = targets.cbegin(); target != targets.cend();) {
                ...
                if (std::regex_search(x.path().filename().string(), reg)) {
                    std::cout << "Found " << *target << " socket " << x.path() << std::endl;
...
std::pair<std::string, std::string> AdminSocketOutput::run_command(AdminSocketClient& client,
                                                                   const std::string& raw_command, bool send_untouched)
{
    ...
    if (!err.empty()) {
        std::cerr << __func__ << " AdminSocketClient::do_request errored with: " << err << std::endl;
        ceph_abort();
    }
...
bool AdminSocketOutput::gather_socket_output()
{
    ...
    for (const auto& socket : sockets) {
        ...
        std::cout << std::endl << "Sending request to " << socket << std::endl << std::endl;
        ...
        std::cout << "Sending commands to " << socket.first << " socket" << std::endl;
        for (; !iter.end(); ++iter) {
            if (std::find(postponed.begin(), postponed.end(), (*iter)->get_name()) != std::end(postponed)) {
                std::cout << "Command \"" << (*iter)->get_name() << "\" postponed" << std::endl;
                continue;
            }
        ...
        // Postponed commands
        if (!postponed.empty()) std::cout << std::endl << "Sending postponed commands" << std::endl;
        for (const auto& command : postponed) {
            sresults.insert(run_command(client, command));
        }

        results.insert(std::pair<std::string, socket_results>(socket.first, sresults));
    }

    return true;
}

std::string AdminSocketOutput::get_result(const std::string& target, const std::string& command) const
{
