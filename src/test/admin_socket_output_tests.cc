...
bool test_dump_pgstate_history(std::string& output)
{
    ...
    if (iterone.end()) {   // Empty
        std::cerr << "test_dump_pgstate_history: command output empty, failing" << std::endl;
        return false;
    }
    ...
    if ((*iterone)->get_name() == "pgs") {
        ...
        for (; !iter.end(); ++iter) {
            if ((*iter)->get_name() == "pg") {
                ...
                if (ret == false) {
                    std::cerr << "test_dump_pgstate_history: pg value empty, failing" << std::endl;
                ...
                ret = !(*iter)->get_data().empty();
                if (ret == false) {
                    std::cerr << "test_dump_pgstate_history: currently value empty, failing" << std::endl;
                    std::cerr << "Dumping full output: " << std::endl;
                    std::cerr << output << std::endl;
                    break;
                }
                ...
                std::cerr << "test_dump_pgstate_history: unrecognised field " << (*iter)->get_name() << ", failing"
                          << std::endl;
                std::cerr << "Dumping full output: " << std::endl;
                std::cerr << output << std::endl;
                break;
        ...
        std::cerr << "test_dump_pgstate_history: unrecognised format, failing" << std::endl;
        std::cerr << "Dumping full output: " << std::endl;
        std::cerr << output << std::endl;
        return false;
    }
