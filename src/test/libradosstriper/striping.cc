...
protected:
    ...
    void checkObjectFromRados(const std::string& soid, bufferlist& bl, uint64_t exp_stripe_unit,
                              uint64_t exp_stripe_count, uint64_t exp_object_size, size_t size)
    {
                              ...
                              size_t actual_size_if_sparse)
    {
        ...
        for (uint64_t stripe_nb = 0; stripe_nb < nb_stripes_in_object; stripe_nb++) {
            ...
            if (actual_size_if_sparse < size and (actual_size_if_sparse + stripe_unit - 1) / stripe_unit <= stripe_nb) {
...
TEST_P(StriperTestRT, StripedRoundtrip)
{
    ...
    oss << "StripedRoundtrip_" << testData.stripe_unit << "_" << testData.stripe_count << "_" << testData.object_size
        << "_" << testData.size;
    ...
    bufferlist bl1;
    {
        ...
        ASSERT_NO_FATAL_FAILURE(checkObjectFromRados(
            soid, bl1, testData.stripe_unit, testData.stripe_count, testData.object_size, testData.size));
    }
    ...
    bufferlist bl2;
    {
        ...
        ASSERT_NO_FATAL_FAILURE(checkObjectFromRados(
            soid, bl1, testData.stripe_unit, testData.stripe_count, testData.object_size, testData.size * 2));
    }
    ...
    // truncating back to original size and checking again (especially for 0s)
    {
        ...
        ASSERT_NO_FATAL_FAILURE(checkObjectFromRados(soid,
                                                     bl3,
                                                     testData.stripe_unit,
                                                     testData.stripe_count,
                                                     testData.object_size,
                                                     testData.size,
                                                     testData.size / 2));
    }
    {
        ...
        checkObjectFromRados(
            soid, bl2, testData.stripe_unit, testData.stripe_count, testData.object_size, testData.size);
    }
    {
        ...
        uint64_t size;
        time_t mtime;
        for (uint64_t object_nb = 0; object_nb < testData.size * 2 / testData.object_size + testData.stripe_count;
             object_nb++) {
            char* oid = getObjName(soid, object_nb);
...
INSTANTIATE_TEST_SUITE_P(SimpleStriping, StriperTestRT, ::testing::ValuesIn(simple_stripe_schemes));
