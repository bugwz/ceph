...
class PausyAsyncMap : public MapCacher::StoreDriver<string, bufferlist>
{
    ...
    struct Remove : public _Op
    {
        ...
        explicit Remove(const set<string>& to_remove)
            : to_remove(to_remove)
        {}
    ...
    struct Insert : public _Op
    {
        ...
        explicit Insert(const map<string, bufferlist>& to_insert)
            : to_insert(to_insert)
        {}
        void operate(map<string, bufferlist>* store) override
        {
            for (map<string, bufferlist>::iterator i = to_insert.begin(); i != to_insert.end(); ++i) {
                store->erase(i->first);
                store->insert(*i);
            }
        }
    ...
    {
        Context* context;
        explicit Callback(Context* c)
            : context(c)
        {}
        void operate(map<string, bufferlist>* store) override { context->complete(0); }
    };
...
public:
    ...
    public:
        void set_keys(const map<string, bufferlist>& i) override { ops.push_back(Op(new Insert(i))); }
        void remove_keys(const set<string>& r) override { ops.push_back(Op(new Remove(r))); }
        void add_callback(Context* c) override { callbacks.push_back(Op(new Callback(c))); }
    };
        ...
        void* entry() override
        {
            while (1) {
                list<Op> ops;
                {
                    ...
                    cond.wait(l, [this] { return stopping || (!queue.empty() && !paused); });
                ...
                for (list<Op>::iterator i = ops.begin(); i != ops.end(); ops.erase(i++)) {
                    if (!(rand() % 3)) usleep(1 + (rand() % 5000));
...
public:
    ...
    ~PausyAsyncMap() override { doer.join(); }
    int get_keys(const set<string>& keys, map<string, bufferlist>* out) override
    {
        std::lock_guard l{lock};
        for (set<string>::const_iterator i = keys.begin(); i != keys.end(); ++i) {
            map<string, bufferlist>::iterator j = store.find(*i);
            if (j != store.end()) out->insert(*j);
        }
        return 0;
    }
    ...
    int get_next_or_current(const string& key, pair<string, bufferlist>* next_or_current) override
    {
        std::lock_guard l{lock};
        map<string, bufferlist>::iterator j = store.lower_bound(key);
        if (j != store.end()) {
            if (next_or_current) *next_or_current = *j;
            return 0;
        }
        else {
            return -ENOENT;
    ...
    void pause() { doer.pause(); }
    void resume() { doer.resume(); }
    void stop() { doer.stop(); }
};
...
public:
    ...
    void assert_bl_map_eq(map<string, bufferlist>& m1, map<string, bufferlist>& m2)
    {
        ASSERT_EQ(m1.size(), m2.size());
        map<string, bufferlist>::iterator j = m2.begin();
        for (map<string, bufferlist>::iterator i = m1.begin(); i != m1.end(); ++i, ++j) {
            ASSERT_TRUE(j != m2.end());
            ASSERT_EQ(i->first, j->first);
            assert_bl_eq(i->second, j->second);
        }
    }
    size_t random_num() { return random() % 10; }
    size_t random_size() { return random() % 1000; }
    ...
    void get()
    {
        ...
        for (set<string>::iterator i = to_get.begin(); i != to_get.end(); ++i) {
            map<string, bufferlist>::iterator j = truth.find(*i);
            if (j != truth.end()) got_truth.insert(*j);
        }
...
public:
    ...
    {
        std::lock_guard l{lock};
        if (snap_to_hobject.empty()) return;
        hobject_t obj;
        do {
            obj = random_hobject();
    ...
    std::pair<std::string, ceph::buffer::list> to_raw(const std::pair<snapid_t, hobject_t>& to_map)
    {
        return mapper->to_raw(to_map);
    }

    std::string to_legacy_raw_key(const std::pair<snapid_t, hobject_t>& to_map)
    {
    ...
    std::string to_raw_key(const std::pair<snapid_t, hobject_t>& to_map) { return mapper->to_raw_key(to_map); }

    template<typename... Args> std::string make_purged_snap_key(Args&&... args)
    {
    ...
    void trim_snap()
    {
        ...
        while (mapper->get_next_objects_to_trim(snap->first, rand() % 5 + 1, &hoids) == 0) {
            for (auto&& hoid : hoids) {
                ...
                {
                    PausyAsyncMap::Transaction t;
                    mapper->update_snaps(hoid, j->second, &old_snaps, &t);
                    driver->submit(&t);
                }
                if (j->second.empty()) {
    ...
    {
        std::lock_guard l{lock};
        if (hobject_to_snap.empty()) return;
        map<hobject_t, set<snapid_t>>::iterator obj = rand_choose(hobject_to_snap);
        for (set<snapid_t>::iterator i = obj->second.begin(); i != obj->second.end(); ++i) {
            map<snapid_t, set<hobject_t>>::iterator j = snap_to_hobject.find(*i);
...
protected:
    ...
    void init(uint32_t to_set)
    {
        ...
        for (uint32_t i = 0; i < pgnum; ++i) {
            pg_t pgid(i, 0);
            mappers[pgid].reset(new MapperVerifier(driver.get(), i, pgid.get_split_bits(pgnum)));
        }
    }
    ...
    void run()
    {
        for (int i = 0; i < 5000; ++i) {
            if (!(i % 50)) std::cout << i << std::endl;
            switch (rand() % 5) {
            case 0: get_tester().create_snap(); break;
            case 1: get_tester().create_object(); break;
            case 2: get_tester().trim_snap(); break;
            case 3: get_tester().check_oid(); break;
            case 4: get_tester().remove_oid(); break;
            }
        }
    }
};
...
TEST_F(SnapMapperTest, CheckObjectKeyFormat)
{
    ...
    std::vector<std::tuple<hobject_t, std::string>> object_to_object_key(
        {{hobject_t{"test_object", "", 20, 0x01234567, 20, ""}, "OBJ_.1_0000000000000014.76543210.14.test%uobject.."},
    ...
    for (auto& [object, test_object_key] : object_to_object_key) {
        ...
        if (object_key != test_object_key) {
            std::cout << object << " should be " << test_object_key << " is " << get_tester().to_object_key(object)
                      << std::endl;
        }
...
TEST_F(SnapMapperTest, CheckRawKeyFormat)
{
    ...
    std::vector<std::tuple<hobject_t, snapid_t, std::string>> object_to_raw_key(
        {{hobject_t{"test_object", "", 20, 0x01234567, 20, ""},
         ...
         {hobject_t{"test_object",
                    ...
                    "namespace"},
          std::numeric_limits<snapid_t>::max() - 20,
          "SNA_9223372036854775787_FFFFFFFFFFFFFFEC_.1_7FFFFFFFFFFFFFEB.76543210.ffffffffffffffec.test%uobject.."
          "namespace"}});
    ...
    for (auto& [object, snap, test_raw_key] : object_to_raw_key) {
        ...
        if (raw_key != test_raw_key) {
            std::cout << object << " " << snap << " should be " << test_raw_key << " is "
                      << get_tester().to_raw_key(std::make_pair(snap, object)) << std::endl;
        }
...
TEST_F(SnapMapperTest, CheckMakePurgedSnapKeyFormat)
{
    ...
    for (auto& [pool, snap, test_key] : purged_snap_to_key) {
        ...
        if (raw_purged_snap_key != test_key) {
            std::cout << "<" << pool << ", " << snap << "> should be " << test_key << " is " << raw_purged_snap_key
                      << std::endl;
        }
...
TEST_F(SnapMapperTest, LegacyKeyConvertion)
{
    ...
    std::string converted_key = SnapMapper::convert_legacy_key(old_key, raw.second);
