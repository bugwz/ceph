...
{
    map<string, string> defaults = {{"admin_socket", get_rand_socket_path()}};
    std::vector<const char*> args;
    auto cct = global_init(&defaults,
                           args,
                           CEPH_ENTITY_TYPE_CLIENT,
                           CODE_ENVIRONMENT_UTILITY,
                           CINIT_FLAG_NO_DEFAULT_CONFIG_FILE | CINIT_FLAG_NO_CCT_PERF_COUNTERS);
...
{
    PerfCountersBuilder bld(
        cct, "test_perfcounter_1", TEST_PERFCOUNTERS1_ELEMENT_FIRST, TEST_PERFCOUNTERS1_ELEMENT_LAST);
...
TEST(PerfCounters, SinglePerfCounters)
{
    ...
    ASSERT_EQ(
        sd("{\"test_perfcounter_1\":{\"element1\":0,"
    ...
    ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
    ASSERT_EQ(
        sd("{\"test_perfcounter_1\":{\"element1\":1,"
...
{
    PerfCountersBuilder bld(
        cct, "test_perfcounter_2", TEST_PERFCOUNTERS2_ELEMENT_FIRST, TEST_PERFCOUNTERS2_ELEMENT_LAST);
...
TEST(PerfCounters, MultiplePerfCounters)
{
    ...
    ASSERT_EQ(sd("{\"test_perfcounter_1\":{\"element1\":{\"type\":2,\"metric_type\":\"gauge\",\"value_type\":"
                 "\"integer\",\"description\":\"\",\"nick\":\"\",\"priority\":0,\"units\":\"none\"},\"element2\":{"
                 "\"type\":1,\"metric_type\":\"gauge\",\"value_type\":\"real\",\"description\":\"\",\"nick\":\"\","
                 "\"priority\":0,\"units\":\"none\"},\"element3\":{\"type\":5,\"metric_type\":\"gauge\",\"value_type\":"
                 "\"real-integer-pair\",\"description\":\"\",\"nick\":\"\",\"priority\":0,\"units\":\"none\"}}}"),
              msg);
    coll->clear();
    ASSERT_EQ("", client.do_request("{ \"prefix\": \"perf dump\", \"format\": \"json\" }", &msg));
    ASSERT_EQ("{}", msg);
...
TEST(PerfCounters, ResetPerfCounters)
{
    ...
    ASSERT_EQ("",
              client.do_request("{ \"prefix\": \"perf reset\", \"var\": \"test_perfcounter_1\", \"format\": \"json\" }",
                                &msg));
...
{
    PerfCountersBuilder bld(
        cct, "test_percounter_3", TEST_PERFCOUNTERS3_ELEMENT_FIRST, TEST_PERFCOUNTERS3_ELEMENT_LAST);
...
{
    PerfCountersBuilder bld(cct, name, TEST_PERFCOUNTERS2_ELEMENT_FIRST, TEST_PERFCOUNTERS2_ELEMENT_LAST);
    bld.add_u64(TEST_PERFCOUNTERS2_ELEMENT_FOO, "foo");
    bld.add_time(TEST_PERFCOUNTERS2_ELEMENT_BAR, "bar");
