...
public:
    ...
    ~test_helper() { curl_global_cleanup(); }
    int send_request(string method, string uri, size_t (*function)(void*, size_t, size_t, void*) = 0, void* ud = 0,
                     size_t length = 0);
    int extract_input(int argc, char* argv[]);
    string& get_response(string hdr) { return response[hdr]; }
    void set_extra_header(string hdr) { extra_hdrs.push_back(hdr); }
    ...
    string& get_rgw_admin_path() { return rgw_admin_path; }
    string& get_ceph_conf_path() { return conf_path; }
    void set_creds(string& c) { creds = c; }
...
#define ERR_CHECK_NEXT_PARAM(o)  \
    ...
    if (host.empty() || rgw_admin_path.empty()) return -1;
    return 0;
}
...
static void calc_hmac_sha1(const char* key, int key_len, const char* msg, int msg_len, char* dest)
...
static int get_s3_auth(const string& method, string creds, const string& date, string res, string& out)
{
    ...
    if (off != string::npos) {
        ...
        admin_meta::calc_hmac_sha1(secret.c_str(), secret.length(), auth_hdr.c_str(), auth_hdr.length(), hmac_sha1);
        int ret = ceph_armor(b64, b64 + 64, hmac_sha1, hmac_sha1 + CEPH_CRYPTO_HMACSHA1_DIGESTSIZE);
        if (ret < 0) {
...
void get_date(string& d)
{
    ...
    char date[64];
    struct tm tm;
    char* days[] = {(char*)"Sun", (char*)"Mon", (char*)"Tue", (char*)"Wed", (char*)"Thu", (char*)"Fri", (char*)"Sat"};
    char* months[] = {(char*)"Jan",
                      (char*)"Feb",
                      (char*)"Mar",
                      (char*)"Apr",
                      (char*)"May",
...
int test_helper::send_request(string method, string res, size_t (*read_function)(void*, size_t, size_t, void*),
                              void* ud, size_t length)
{
    ...
    if (curl_inst) {
        ...
        string s3auth;
        if (admin_meta::get_s3_auth(method, creds, date, res, s3auth) < 0) return -1;
        ...
        for (list<string>::iterator it = extra_hdrs.begin(); it != extra_hdrs.end(); ++it) {
            slist = curl_slist_append(slist, (*it).c_str());
        }
        if (read_function) curl_slist_append(slist, "Expect:");
...
int get_creds(string& json, string& creds)
{
    ...
    creds = "";
    for (map<string, RGWAccessKey>::iterator it = info.access_keys.begin(); it != info.access_keys.end(); ++it) {
...
int user_create(string& uid, string& display_name, bool set_creds = true)
{
    stringstream ss;
    string creds;
    ss << "-c " << g_test->get_ceph_conf_path() << " user create --uid=" << uid << " --display-name=" << display_name;
    ...
    get_creds(out, creds);
    if (set_creds) g_test->set_creds(creds);
    return 0;
}
...
{
    stringstream ss;
    ss << "-c " << g_test->get_ceph_conf_path() << " user info --uid=" << uid << " --display-name=" << display_name;
...
{
    stringstream ss;
    ss << "-c " << g_test->get_ceph_conf_path() << " user rm --uid=" << uid << " --display-name=" << display_name;
...
int meta_caps_add(const char* perm)
{
    ...
    ss << "-c " << g_test->get_ceph_conf_path() << " caps add --caps=" << meta_caps << "=" << perm << " --uid=" << uid;
    string out;
    string cmd = ss.str();
    if (run_rgw_admin(cmd, out) != 0) {
        cout << "Error creating user" << std::endl;
...
int meta_caps_rm(const char* perm)
{
    ...
    ss << "-c " << g_test->get_ceph_conf_path() << " caps rm --caps=" << meta_caps << "=" << perm << " --uid=" << uid;
    string out;
    string cmd = ss.str();
    if (run_rgw_admin(cmd, out) != 0) {
        cout << "Error creating user" << std::endl;
...
{
    if (k1.id.compare(k2.id) != 0) return -1;
    if (k1.key.compare(k2.key) != 0) return -1;
    if (k1.subuser.compare(k2.subuser) != 0) return -1;
...
int compare_user_info(RGWUserInfo& i1, RGWUserInfo& i2)
{
    ...
    if ((rv = i1.user_id.compare(i2.user_id)) != 0) return rv;
    if ((rv = i1.display_name.compare(i2.display_name)) != 0) return rv;
    if ((rv = i1.user_email.compare(i2.user_email)) != 0) return rv;
    if (i1.access_keys.size() != i2.access_keys.size()) return -1;
    for (map<string, RGWAccessKey>::iterator it = i1.access_keys.begin(); it != i1.access_keys.end(); ++it) {
        RGWAccessKey k1, k2;
        k1 = it->second;
        if (i2.access_keys.count(it->first) == 0) return -1;
        k2 = i2.access_keys[it->first];
        if (compare_access_keys(k1, k2) != 0) return -1;
    }
    if (i1.swift_keys.size() != i2.swift_keys.size()) return -1;
    for (map<string, RGWAccessKey>::iterator it = i1.swift_keys.begin(); it != i1.swift_keys.end(); ++it) {
        RGWAccessKey k1, k2;
        k1 = it->second;
        if (i2.swift_keys.count(it->first) == 0) return -1;
        k2 = i2.swift_keys[it->first];
        if (compare_access_keys(k1, k2) != 0) return -1;
    }
    if (i1.subusers.size() != i2.subusers.size()) return -1;
    for (map<string, RGWSubUser>::iterator it = i1.subusers.begin(); it != i1.subusers.end(); ++it) {
        RGWSubUser k1, k2;
        k1 = it->second;
        if (!i2.subusers.count(it->first)) return -1;
        k2 = i2.subusers[it->first];
        if (k1.name.compare(k2.name) != 0) return -1;
        if (k1.perm_mask != k2.perm_mask) return -1;
    }
    if (i1.suspended != i2.suspended) return -1;
    if (i1.max_buckets != i2.max_buckets) return -1;
    uint32_t p1, p2;
    p1 = p2 = RGW_CAP_ALL;
    if (i1.caps.check_cap(meta_caps, p1) != 0) return -1;
    if (i2.caps.check_cap(meta_caps, p2) != 0) return -1;
    return 0;
}
...
TEST(TestRGWAdmin, meta_put)
{
    ...
    g_test->send_request(
        string("PUT"), (string("/admin/metadata/user?key=") + uid), meta_read_json, (void*)&ss, ss.str().length());
    EXPECT_EQ(200U, g_test->get_resp_code());
