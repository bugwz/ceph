...
void bench_buffer_alloc(int size, int num)
{
    ...
    cout << num << " alloc of size " << size << " in " << (end - start) << std::endl;
}
...
TEST(BufferPtr, constructors)
{
    ...
    //
    {
        ...
        EXPECT_TRUE(ptr.have_raw());
        EXPECT_EQ(static_cast<instrumented_bptr&>(original).get_raw(), static_cast<instrumented_bptr&>(ptr).get_raw());
...
#pragma clang diagnostic pop
    ...
    //
    {
        ...
        ptr = original;
        ASSERT_EQ(2, original.raw_nref());
        ASSERT_EQ(static_cast<instrumented_bptr&>(ptr).get_raw(), static_cast<instrumented_bptr&>(original).get_raw());
...
TEST(BufferPtr, copy_out_bench)
{
    for (int s = 1; s <= 8; s *= 2) {
        ...
        utime_t end = ceph_clock_now();
        cout << count << " fills of buffer len " << buflen << " with " << s << " byte copy_out in " << (end - start)
             << std::endl;
    }
}
...
TEST(BufferPtr, copy_in_bench)
{
    for (int s = 1; s <= 8; s *= 2) {
        ...
        utime_t end = ceph_clock_now();
        cout << count << " fills of buffer len " << buflen << " with " << s << " byte copy_in in " << (end - start)
             << std::endl;
    }
}
...
TEST(BufferPtr, append_bench)
{
    ...
    for (int s = 4; s <= 16384; s *= 4) {
        ...
        utime_t end = ceph_clock_now();
        cout << count << " fills of buffer len " << buflen << " with " << s << " byte appends in " << (end - start)
             << std::endl;
    }
}
...
static void bench_bufferlistiter_deref(const size_t step, const size_t bufsize, const size_t bufnum)
{
    ...
    utime_t end = ceph_clock_now();
    cout << bufsize * bufnum << " derefs over bl with " << bufnum << " buffers, each " << bufsize << " bytes long"
...
void bench_bufferlist_alloc(int size, int num, int per)
{
    ...
    for (int i = 0; i < num; ++i) {
        bufferlist bl;
        for (int j = 0; j < per; ++j) bl.push_back(buffer::ptr_node::create(buffer::create(size)));
    }
    utime_t end = ceph_clock_now();
    cout << num << " alloc of size " << size << " in " << (end - start) << std::endl;
}
...
TEST(BufferList, append_bench_with_size_hint)
{
    ...
    for (size_t step = 4; step <= 16384; step *= 4) {
        ...
        for (size_t r = 0; r < rounds; ++r) {
            ...
            for (auto iter = std::begin(src); iter != std::end(src); iter = std::next(iter, step)) {
                bl.append(&*iter, step);
            }
        }
        cout << rounds << " fills of buffer len " << src.size() << " with " << step << " byte appends in "
...
TEST(BufferList, append_hole_bench)
{
    ...
    for (size_t step = 512; step <= 65536; step *= 2) {
        ...
        cout << rounds << " fills of buffer len " << targeted_bl_size << " with " << step
             << " byte long append_hole in " << (ceph_clock_now() - start) << std::endl;
    }
}
