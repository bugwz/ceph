...
protected:
    ...
    void test_enqueue(const std::string& queue_name, int number_of_ops, int number_of_elements, int expected_rc)
    {
...
TEST_F(TestClsQueue, List)
{
    ...
    do {
        ...
        const auto ret =
            cls_queue_list_entries(ioctx, queue_name, marker, max_elements, entries, &truncated, next_marker);
        ASSERT_EQ(0, ret);
        marker = next_marker;
...
TEST_F(TestClsQueue, Dequeue)
{
    ...
    std::vector<cls_queue_entry> entries;
    const auto ret =
        cls_queue_list_entries(ioctx, queue_name, marker, remove_elements, entries, &truncated, end_marker);
    ASSERT_EQ(0, ret);
...
TEST_F(TestClsQueue, ListAll)
{
    ...
    std::vector<cls_queue_entry> entries;
    const auto ret =
        cls_queue_list_entries(ioctx, queue_name, marker, total_elements, entries, &truncated, next_marker);
    ASSERT_EQ(0, ret);
...
TEST_F(TestClsQueue, EnqueueDequeue)
{
    ...
    std::thread consumer([this, &queue_name, &consume_count, &done] {
        ...
        while (!done || truncated) {
            const auto ret =
                cls_queue_list_entries(ioctx, queue_name, marker, max_elements, entries, &truncated, end_marker);
            ASSERT_EQ(0, ret);
