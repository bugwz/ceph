...
} subtests[3] = {{false, false, false, "DIRECTIO OFF  AIO OFF"},
...
public:
    ...
    C_Sync() { c = new C_SafeCond(lock, cond, &done); }
    ~C_Sync()
    {
...
TEST(TestFileJournal, Create)
{
    ...
    for (unsigned i = 0; i < 3; ++i) {
        ...
        FileJournal fj(
            g_ceph_context, fsid, finisher, &sync_cond, path, subtests[i].directio, subtests[i].aio, subtests[i].faio);
...
TEST(TestFileJournal, WriteTrimSmall)
{
    ...
    for (unsigned i = 0; i < 3; ++i) {
        ...
        for (unsigned i = 0; i < size_mb * 2; i++) {
            bl.clear();
            for (int k = 0; k < 128; k++) bl.push_back(buffer::copy(foo, sizeof(foo) / 128));
            bl.zero();
            ls.push_back(new C_Sync);
            int orig_len = fj.prepare_entry(tls, &bl);
            fj.reserve_throttle_and_backoff(bl.length());
