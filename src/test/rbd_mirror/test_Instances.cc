...
public:
    struct Listener : public rbd::mirror::instances::Listener
    {
        ...
        void handle_added(const InstanceIds& instance_ids) override { handle(instance_ids, &add); }

        void handle_removed(const InstanceIds& instance_ids) override { handle(instance_ids, &remove); }
    };
...
TEST_F(TestInstances, InitShutdown)
{
    ...
    ASSERT_EQ(0, librbd::cls_client::mirror_instances_add(&m_local_io_ctx, instance_id));
...
TEST_F(TestInstances, NotifyRemove)
{
    ...
    EXPECT_EQ(0, _rados->conf_set("rbd_mirror_leader_max_acquire_attempts_before_break", "0"));
    ...
    ASSERT_EQ(0, librbd::cls_client::mirror_instances_add(&m_local_io_ctx, instance_id1));
    ...
    ASSERT_EQ(0, m_listener.add.ctx.wait());
    ASSERT_EQ(std::set<std::string>({instance_id1, instance_id2}), m_listener.add.ids);
