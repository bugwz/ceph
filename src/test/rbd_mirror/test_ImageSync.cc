...
{
    C_SaferCond ctx;
    auto aio_comp = librbd::io::AioCompletion::create_and_start(&ctx, image_ctx, librbd::io::AIO_TYPE_FLUSH);
    auto req = librbd::io::ImageDispatchSpec::create_flush(
        *image_ctx, librbd::io::IMAGE_DISPATCH_LAYER_INTERNAL_START, aio_comp, librbd::io::FLUSH_SOURCE_INTERNAL, {});
    req->send();
    return ctx.wait();
}
...
void scribble(librbd::ImageCtx* image_ctx, int num_ops, uint64_t max_size)
{
    ...
    for (int i = 0; i < num_ops; i++) {
        ...
        if (rand() % 4 == 0) {
            ASSERT_EQ((int)len, librbd::api::Io<>::discard(*image_ctx, off, len, image_ctx->discard_granularity_bytes));
        }
        else {
            bufferlist bl;
            bl.append(std::string(len, '1'));
            ASSERT_EQ((int)len, librbd::api::Io<>::write(*image_ctx, off, len, std::move(bl), 0));
        }
    }
...
public:
    void SetUp() override
    {
        ...
        m_image_sync_throttler = rbd::mirror::Throttler<>::create(cct, "rbd_mirror_concurrent_image_syncs");
        ...
        m_remote_journaler = new ::journal::Journaler(context_wq,
                                                      m_threads->timer,
                                                      &m_threads->timer_lock,
                                                      m_remote_io_ctx,
                                                      m_remote_image_ctx->id,
                                                      "mirror-uuid",
                                                      {},
                                                      nullptr);
        ...
        m_state_builder =
            rbd::mirror::image_replayer::journal::StateBuilder<librbd::ImageCtx>::create("global image id");
    ...
    {
        return new ImageSync<>(m_threads,
                               m_local_image_ctx,
                               m_remote_image_ctx,
                               "mirror-uuid",
...
TEST_F(TestImageSync, Simple)
{
    ...
    int64_t object_size = std::min<int64_t>(m_remote_image_ctx->size, 1 << m_remote_image_ctx->order);
    ...
    for (uint64_t offset = 0; offset < m_remote_image_ctx->size; offset += object_size) {
        ASSERT_LE(0,
                  librbd::api::Io<>::read(
                      *m_remote_image_ctx, offset, object_size, librbd::io::ReadResult{&read_remote_bl}, 0));
        ASSERT_LE(0,
                  librbd::api::Io<>::read(
                      *m_local_image_ctx, offset, object_size, librbd::io::ReadResult{&read_local_bl}, 0));
...
TEST_F(TestImageSync, Resize)
{
    ...
    bufferlist bl;
    bl.append(std::string(len, '1'));
    ASSERT_EQ((int)len, librbd::api::Io<>::write(*m_remote_image_ctx, off, len, std::move(bl), 0));
    {
    ...
    ASSERT_EQ(0, m_remote_image_ctx->operations->resize(size, true, no_op_progress_ctx));
    ...
    ASSERT_LE(0, librbd::api::Io<>::read(*m_remote_image_ctx, off, len, librbd::io::ReadResult{&read_remote_bl}, 0));
    ASSERT_LE(0, librbd::api::Io<>::read(*m_local_image_ctx, off, len, librbd::io::ReadResult{&read_local_bl}, 0));
...
TEST_F(TestImageSync, Discard)
{
    ...
    ASSERT_EQ((int)len - 2,
              librbd::api::Io<>::discard(
                  *m_remote_image_ctx, off + 1, len - 2, m_remote_image_ctx->discard_granularity_bytes));
    {
        std::shared_lock owner_locker{m_remote_image_ctx->owner_lock};
        ASSERT_EQ(0, flush(m_remote_image_ctx));
    }
...
TEST_F(TestImageSync, SnapshotStress)
{
    ...
    for (int idx = 0; idx <= num_snaps; ++idx) {
        ...
        if (idx < num_snaps) {
            ...
            ASSERT_EQ(0, create_snap(m_remote_image_ctx, snap_names.back().c_str(), nullptr));
        }
        else {
    ...
    for (auto& snap_name : snap_names) {
        uint64_t remote_snap_id;
        {
            ...
            remote_snap_id = m_remote_image_ctx->get_snap_id(cls::rbd::UserSnapshotNamespace{}, snap_name);
        }
        ...
        uint64_t remote_size;
        {
            ...
            std::shared_lock remote_image_locker{m_remote_image_ctx->image_lock};
            remote_size = m_remote_image_ctx->get_image_size(m_remote_image_ctx->snap_id);
        }

        uint64_t local_snap_id;
        {
            std::shared_lock image_locker{m_local_image_ctx->image_lock};
            local_snap_id = m_local_image_ctx->get_snap_id(cls::rbd::UserSnapshotNamespace{}, snap_name);
        }
        ...
        uint64_t local_size;
        {
            ...
            std::shared_lock image_locker{m_local_image_ctx->image_lock};
            local_size = m_local_image_ctx->get_image_size(m_local_image_ctx->snap_id);
            bool flags_set;
            ASSERT_EQ(0,
                      m_local_image_ctx->test_flags(m_local_image_ctx->snap_id,
