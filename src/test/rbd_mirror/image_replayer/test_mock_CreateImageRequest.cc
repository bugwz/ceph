...
template<> struct CreateRequest<librbd::MockTestImageCtx>
{
    ...
    static CreateRequest* create(const ConfigProxy& config, IoCtx& ioctx, const std::string& imgname,
                                 const std::string& imageid, uint64_t size, const librbd::ImageOptions& image_options,
                                 bool skip_mirror_enable, cls::rbd::MirrorImageMode mode,
    ...
    ~CreateRequest() { s_instance = nullptr; }

    MOCK_METHOD0(send, void());
    MOCK_METHOD1(construct, void(librados::IoCtx& ioctx));
};

CreateRequest<librbd::MockTestImageCtx>* CreateRequest<librbd::MockTestImageCtx>::s_instance = nullptr;

template<> struct CloneRequest<librbd::MockTestImageCtx>
{
    static CloneRequest* s_instance;
    Context* on_finish = nullptr;

    static CloneRequest* create(ConfigProxy& config, IoCtx& p_ioctx, const std::string& p_id,
                                const std::string& p_snap_name, const cls::rbd::SnapshotNamespace& snap_ns,
                                uint64_t p_snap_id, IoCtx& c_ioctx, const std::string& c_name, const std::string& c_id,
                                ImageOptions c_options, cls::rbd::MirrorImageMode mode,
                                const std::string& non_primary_global_image_id, const std::string& primary_mirror_uuid,
                                MockContextWQ* op_work_queue, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
    ...
    CloneRequest() { s_instance = this; }

    ~CloneRequest() { s_instance = nullptr; }

    MOCK_METHOD0(send, void());
    MOCK_METHOD0(construct, void());
};

CloneRequest<librbd::MockTestImageCtx>* CloneRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
...
template<> struct CloseImageRequest<librbd::MockTestImageCtx>
{
    ...
    ~CloseImageRequest() { s_instance = nullptr; }
...
public:
    ...
    void snap_create(librbd::ImageCtx* image_ctx, const std::string& snap_name)
    {
        ...
        ASSERT_EQ(0, image_ctx->operations->snap_create(cls::rbd::UserSnapshotNamespace(), snap_name, 0, prog_ctx));
        ASSERT_EQ(0, image_ctx->operations->snap_protect(cls::rbd::UserSnapshotNamespace(), snap_name));
        ASSERT_EQ(0, image_ctx->state->refresh());
    }

    int clone_image(librbd::ImageCtx* parent_image_ctx, const std::string& snap_name, const std::string& clone_name)
    {
        snap_create(parent_image_ctx, snap_name);

        int order = 0;
        return librbd::clone(m_remote_io_ctx,
                             parent_image_ctx->name.c_str(),
                             snap_name.c_str(),
                             m_remote_io_ctx,
                             clone_name.c_str(),
                             parent_image_ctx->features,
                             &order,
                             0,
                             0);
    }

    void expect_create_image(MockCreateRequest& mock_create_request, librados::IoCtx& ioctx, int r)
    {
        EXPECT_CALL(mock_create_request, construct(IsSameIoCtx(&ioctx)));
        EXPECT_CALL(mock_create_request, send()).WillOnce(Invoke([this, &mock_create_request, r]() {
    ...
    void expect_ioctx_create(librados::IoCtx& io_ctx)
    {
        ...
        EXPECT_CALL(*get_mock_io_ctx(io_ctx).get_mock_rados_client(), create_ioctx(_, _))
            .WillOnce(DoAll(GetReference(&io_ctx_impl), Return(&get_mock_io_ctx(io_ctx))));
    }

    void expect_get_parent_global_image_id(librados::IoCtx& io_ctx, const std::string& global_id, int r)
    {
        ...
        EXPECT_CALL(get_mock_io_ctx(io_ctx),
                    exec(RBD_MIRRORING, _, StrEq("rbd"), StrEq("mirror_image_get"), _, _, _, _))
            .WillOnce(DoAll(WithArg<5>(Invoke([bl](bufferlist* out_bl) { *out_bl = bl; })), Return(r)));
    }

    void expect_mirror_image_get_image_id(librados::IoCtx& io_ctx, const std::string& image_id, int r)
    {
        bufferlist bl;
        encode(image_id, bl);

        EXPECT_CALL(get_mock_io_ctx(io_ctx),
                    exec(RBD_MIRRORING, _, StrEq("rbd"), StrEq("mirror_image_get_image_id"), _, _, _, _))
            .WillOnce(DoAll(WithArg<5>(Invoke([bl](bufferlist* out_bl) { *out_bl = bl; })), Return(r)));
    }
    ...
    void expect_test_op_features(librbd::MockTestImageCtx& mock_image_ctx, bool enabled)
    {
        EXPECT_CALL(mock_image_ctx, test_op_features(RBD_OPERATION_FEATURE_CLONE_CHILD)).WillOnce(Return(enabled));
    }

    void expect_clone_image(MockCloneRequest& mock_clone_request, int r)
    {
        EXPECT_CALL(mock_clone_request, construct());
        EXPECT_CALL(mock_clone_request, send()).WillOnce(Invoke([this, &mock_clone_request, r]() {
            m_threads->work_queue->queue(mock_clone_request.on_finish, r);
        }));
    }

    void expect_close_image(MockCloseImageRequest& mock_close_image_request, librbd::MockTestImageCtx& mock_image_ctx,
                            int r)
    {
    ...
    MockCreateImageRequest* create_request(MockThreads* mock_threads, const std::string& global_image_id,
                                           const std::string& remote_mirror_uuid, const std::string& local_image_name,
                                           const std::string& local_image_id,
                                           librbd::MockTestImageCtx& mock_remote_image_ctx, Context* on_finish)
    {
        return new MockCreateImageRequest(mock_threads,
                                          m_local_io_ctx,
                                          global_image_id,
                                          remote_mirror_uuid,
                                          local_image_name,
...
TEST_F(TestMockImageReplayerCreateImageRequest, Create)
{
    ...
    MockCreateImageRequest* request = create_request(
        &mock_threads, "global uuid", "remote uuid", "image name", "101241a7c4c9", mock_remote_image_ctx, &ctx);
    request->send();
...
TEST_F(TestMockImageReplayerCreateImageRequest, CloneGetGlobalImageIdError)
{
    ...
    ASSERT_EQ(0, open_image(m_remote_io_ctx, clone_image_name, &remote_clone_image_ctx));
    ...
    MockThreads mock_threads(m_threads);
    MockCreateImageRequest* request = create_request(
        &mock_threads, "global uuid", "remote uuid", "image name", "101241a7c4c9", mock_remote_clone_image_ctx, &ctx);
    request->send();
...
TEST_F(TestMockImageReplayerCreateImageRequest, CloneOpenRemoteParentError)
{
    ...
    expect_open_image(
        mock_open_image_request, m_remote_io_ctx, m_remote_image_ctx->id, mock_remote_parent_image_ctx, -ENOENT);
...
TEST_F(TestMockImageReplayerCreateImageRequest, CloneParentImageSyncing)
{
    ...
    m_pool_meta_cache.set_local_pool_meta(m_local_io_ctx.get_id(), {"local parent uuid"});
    ...
    expect_open_image(
        mock_open_image_request, m_remote_io_ctx, m_remote_image_ctx->id, mock_remote_parent_image_ctx, 0);
