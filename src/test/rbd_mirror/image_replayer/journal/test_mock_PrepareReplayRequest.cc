...
public:
    ...
    void expect_journaler_get_tags(::journal::MockJournaler& mock_journaler, uint64_t tag_class, const Tags& tags,
                                   int r)
    {
        EXPECT_CALL(mock_journaler, get_tags(tag_class, _, _))
            .WillOnce(DoAll(
                WithArg<1>(Invoke([tags](Tags* out_tags) { *out_tags = tags; })),
                WithArg<2>(Invoke([this, r](Context* on_finish) { m_threads->work_queue->queue(on_finish, r); }))));
    }

    void expect_journal_get_tag_tid(librbd::MockJournal& mock_journal, uint64_t tag_tid)
    {
        EXPECT_CALL(mock_journal, get_tag_tid()).WillOnce(Return(tag_tid));
    }

    void expect_journal_get_tag_data(librbd::MockJournal& mock_journal, const librbd::journal::TagData& tag_data)
    {
        EXPECT_CALL(mock_journal, get_tag_data()).WillOnce(Return(tag_data));
    }

    void expect_is_resync_requested(librbd::MockJournal& mock_journal, bool do_resync, int r)
    {
        EXPECT_CALL(mock_journal, is_resync_requested(_)).WillOnce(DoAll(SetArgPointee<0>(do_resync), Return(r)));
    }
    ...
    MockPrepareReplayRequest* create_request(MockStateBuilder& mock_state_builder, const std::string& local_mirror_uuid,
...
TEST_F(TestMockImageReplayerJournalPrepareReplayRequest, Success)
{
    ...
    librbd::journal::ClientData client_data{librbd::journal::ImageClientMeta{123}};
    cls::journal::Client client;
    encode(client_data, client.data);
    ::journal::MockJournaler mock_remote_journaler;
    expect_journaler_get_client(mock_remote_journaler, librbd::Journal<>::IMAGE_CLIENT_ID, client, 0);

    // single promotion event
    Tags tags = {
        {2,
         123,
         encode_tag_data({librbd::Journal<>::LOCAL_MIRROR_UUID, librbd::Journal<>::LOCAL_MIRROR_UUID, true, 344, 99})},
    };
    ...
    MockStateBuilder mock_state_builder(mock_local_image_ctx, mock_remote_journaler, mirror_peer_client_meta);
    bool resync_requested;
    bool syncing;
    auto request = create_request(mock_state_builder, "local mirror uuid", &resync_requested, &syncing, &ctx);
    request->send();
...
TEST_F(TestMockImageReplayerJournalPrepareReplayRequest, GetRemoteTagClassError)
{
    ...
    encode(client_data, client.data);
    ::journal::MockJournaler mock_remote_journaler;
    expect_journaler_get_client(mock_remote_journaler, librbd::Journal<>::IMAGE_CLIENT_ID, client, -EINVAL);
...
TEST_F(TestMockImageReplayerJournalPrepareReplayRequest, LocalDemotedRemoteSyncingState)
{
    ...
    expect_journal_get_tag_data(mock_journal,
                                {librbd::Journal<>::ORPHAN_MIRROR_UUID, "remote mirror uuid", true, 4, 1});

    // update client state
    librbd::journal::MirrorPeerClientMeta mirror_peer_client_meta{mock_local_image_ctx.id};
    mirror_peer_client_meta.state = librbd::journal::MIRROR_PEER_STATE_REPLAYING;
    ...
    Tags tags = {
        {2,
         123,
         encode_tag_data({librbd::Journal<>::LOCAL_MIRROR_UUID, librbd::Journal<>::LOCAL_MIRROR_UUID, true, 1, 99})},
        {3,
         123,
         encode_tag_data({librbd::Journal<>::ORPHAN_MIRROR_UUID, librbd::Journal<>::LOCAL_MIRROR_UUID, true, 2, 1})},
        {4,
         123,
         encode_tag_data({librbd::Journal<>::LOCAL_MIRROR_UUID, librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 3, 1})},
        {5,
         123,
         encode_tag_data({librbd::Journal<>::ORPHAN_MIRROR_UUID, librbd::Journal<>::LOCAL_MIRROR_UUID, true, 4, 1})},
        {6,
         123,
         encode_tag_data({librbd::Journal<>::LOCAL_MIRROR_UUID, librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 5, 1})},
        {7,
         123,
         encode_tag_data({librbd::Journal<>::ORPHAN_MIRROR_UUID, librbd::Journal<>::LOCAL_MIRROR_UUID, true, 6, 1})},
        {8,
         123,
         encode_tag_data({librbd::Journal<>::LOCAL_MIRROR_UUID, librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 7, 1})}};
    expect_journaler_get_tags(mock_remote_journaler, 123, tags, 0);
...
TEST_F(TestMockImageReplayerJournalPrepareReplayRequest, RemoteDemotePromote)
{
    ...
    Tags tags = {
        ...
        {4,
         123,
         encode_tag_data({librbd::Journal<>::LOCAL_MIRROR_UUID, librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 2, 1})},
        {5,
         123,
         encode_tag_data({librbd::Journal<>::LOCAL_MIRROR_UUID, librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 4, 369})}};
    expect_journaler_get_tags(mock_remote_journaler, 123, tags, 0);
...
TEST_F(TestMockImageReplayerJournalPrepareReplayRequest, LocalDemoteRemotePromote)
{
    ...
    expect_journal_get_tag_data(
        mock_journal, {librbd::Journal<>::ORPHAN_MIRROR_UUID, librbd::Journal<>::LOCAL_MIRROR_UUID, true, 345, 1});
    ...
    // remote demotion / promotion event
    Tags tags = {
        {2, 123, encode_tag_data({"local mirror uuid", "local mirror uuid", true, 344, 99})},
        {3, 123, encode_tag_data({librbd::Journal<>::ORPHAN_MIRROR_UUID, "local mirror uuid", true, 345, 1})},
        {4,
         123,
         encode_tag_data({librbd::Journal<>::LOCAL_MIRROR_UUID, librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 3, 1})}};
    expect_journaler_get_tags(mock_remote_journaler, 123, tags, 0);
...
TEST_F(TestMockImageReplayerJournalPrepareReplayRequest, SplitBrainForcePromote)
{
    ...
    expect_journal_get_tag_tid(mock_journal, 345);
    expect_journal_get_tag_data(
        mock_journal, {librbd::Journal<>::LOCAL_MIRROR_UUID, librbd::Journal<>::ORPHAN_MIRROR_UUID, true, 344, 0});
