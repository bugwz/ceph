...
public:
    ...
    {
        EXPECT_CALL(mock_journaler, update_client(_, _)).WillOnce(WithArg<1>(CompleteContext(r)));
    }
...
TEST_F(TestMockImageReplayerJournalEventPreprocessor, IsNotRequired)
{
    ...
    MockEventPreprocessor event_preprocessor(
        mock_local_image_ctx, mock_remote_journaler, "local mirror uuid", &m_client_meta, m_threads->work_queue);
...
TEST_F(TestMockImageReplayerJournalEventPreprocessor, PreprocessSnapRename)
{
    ...
    librbd::journal::EventEntry event_entry{librbd::journal::SnapRenameEvent{0, 5, "snap", "new_snap"}};
    C_SaferCond ctx;
    event_preprocessor.preprocess(&event_entry, &ctx);
    ASSERT_EQ(0, ctx.wait());
    ...
    librbd::journal::SnapRenameEvent* event = boost::get<librbd::journal::SnapRenameEvent>(&event_entry.event);
