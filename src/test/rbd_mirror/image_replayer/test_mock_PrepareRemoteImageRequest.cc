...
template<> struct GetMirrorImageIdRequest<librbd::MockTestImageCtx>
{
    static GetMirrorImageIdRequest* s_instance;
    std::string* image_id = nullptr;
    Context* on_finish = nullptr;

    static GetMirrorImageIdRequest* create(librados::IoCtx& io_ctx, const std::string& global_image_id,
                                           std::string* image_id, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
    ...
    GetMirrorImageIdRequest() { s_instance = this; }
...
GetMirrorImageIdRequest<librbd::MockTestImageCtx>* GetMirrorImageIdRequest<librbd::MockTestImageCtx>::s_instance =
    nullptr;

namespace journal {

template<> struct StateBuilder<librbd::MockTestImageCtx> : public image_replayer::StateBuilder<librbd::MockTestImageCtx>
{
    static StateBuilder* s_instance;

    cls::rbd::MirrorImageMode mirror_image_mode = cls::rbd::MIRROR_IMAGE_MODE_JOURNAL;
...
template<> struct StateBuilder<librbd::MockTestImageCtx> : public image_replayer::StateBuilder<librbd::MockTestImageCtx>
{
    ...
    cls::rbd::MirrorImageMode mirror_image_mode = cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT;
...
public:
    ...
    void expect_get_mirror_image_mode(MockStateBuilder& mock_state_builder, cls::rbd::MirrorImageMode mirror_image_mode)
    {
        EXPECT_CALL(mock_state_builder, get_mirror_image_mode()).WillOnce(Return(mirror_image_mode));
    }
    ...
    void expect_journaler_get_client(::journal::MockJournaler& mock_journaler, const std::string& client_id,
                                     cls::journal::Client& client, int r)
    {
        EXPECT_CALL(mock_journaler, get_client(StrEq(client_id), _, _))
            .WillOnce(DoAll(
                WithArg<1>(Invoke([client](cls::journal::Client* out_client) { *out_client = client; })),
                WithArg<2>(Invoke([this, r](Context* on_finish) { m_threads->work_queue->queue(on_finish, r); }))));
    }
                                          ...
                                          const librbd::journal::ClientData& client_data, int r)
    {
        ...
        EXPECT_CALL(mock_journaler, register_client(ContentsEqual(bl), _))
            .WillOnce(
                WithArg<1>(Invoke([this, r](Context* on_finish) { m_threads->work_queue->queue(on_finish, r); })));
    }
};
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, SuccessJournal)
{
    ...
    expect_get_mirror_image_id(mock_get_mirror_image_id_request, "remote image id", 0);
    ...
    expect_journaler_get_client(mock_remote_journaler, "local mirror uuid", client, 0);
    ...
    ASSERT_EQ(cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, mock_journal_state_builder.mirror_image_mode);
    ASSERT_EQ(std::string("remote mirror uuid"), mock_journal_state_builder.remote_mirror_uuid);
    ASSERT_EQ(std::string("remote image id"), mock_journal_state_builder.remote_image_id);
    ASSERT_EQ(librbd::mirror::PROMOTION_STATE_PRIMARY, mock_journal_state_builder.remote_promotion_state);
    ASSERT_TRUE(mock_journal_state_builder.remote_journaler != nullptr);
    ASSERT_EQ(cls::journal::CLIENT_STATE_DISCONNECTED, mock_journal_state_builder.remote_client_state);
}
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, SuccessSnapshot)
{
    ...
    ASSERT_EQ(0, ctx.wait());
    ASSERT_TRUE(mock_state_builder != nullptr);
    ASSERT_EQ(cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, mock_snapshot_state_builder.mirror_image_mode);
    ASSERT_EQ(std::string("remote mirror uuid"), mock_snapshot_state_builder.remote_mirror_uuid);
    ASSERT_EQ(std::string("remote mirror peer uuid"), mock_snapshot_state_builder.remote_mirror_peer_uuid);
    ASSERT_EQ(std::string("remote image id"), mock_snapshot_state_builder.remote_image_id);
    ASSERT_EQ(librbd::mirror::PROMOTION_STATE_PRIMARY, mock_snapshot_state_builder.remote_promotion_state);
}
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, SuccessNotRegistered)
{
    ...
    MockJournalStateBuilder mock_journal_state_builder;
    expect_get_mirror_image_mode(mock_journal_state_builder, cls::rbd::MIRROR_IMAGE_MODE_JOURNAL);

    EXPECT_CALL(mock_remote_journaler, construct());

    cls::journal::Client client;
    expect_journaler_get_client(mock_remote_journaler, "local mirror uuid", client, -ENOENT);
    ...
    ASSERT_EQ(librbd::mirror::PROMOTION_STATE_PRIMARY, mock_journal_state_builder.remote_promotion_state);
    ASSERT_TRUE(mock_journal_state_builder.remote_journaler != nullptr);
    ASSERT_EQ(cls::journal::CLIENT_STATE_CONNECTED, mock_journal_state_builder.remote_client_state);
}
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, GetClientError)
{
    ...
    cls::journal::Client client;
    expect_journaler_get_client(mock_remote_journaler, "local mirror uuid", client, -EINVAL);
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, MirrorImageIdDNEJournal)
{
    ...
    ASSERT_EQ(cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, mock_journal_state_builder.mirror_image_mode);
    ASSERT_EQ("remote mirror uuid", mock_journal_state_builder.remote_mirror_uuid);
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, MirrorImageIdDNESnapshot)
{
    ...
    ASSERT_EQ(-ENOENT, ctx.wait());
    ASSERT_EQ(cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, mock_snapshot_state_builder.mirror_image_mode);
    ASSERT_EQ("remote mirror uuid", mock_snapshot_state_builder.remote_mirror_uuid);
    ASSERT_EQ("remote mirror peer uuid", mock_snapshot_state_builder.remote_mirror_peer_uuid);
...
TEST_F(TestMockImageReplayerPrepareRemoteImageRequest, MirrorInfoDisablingSnapshot)
{
    ...
    MockSnapshotStateBuilder mock_snapshot_state_builder;
    expect_get_mirror_image_mode(mock_snapshot_state_builder, cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT);
    MockStateBuilder* mock_state_builder = &mock_snapshot_state_builder;
    C_SaferCond ctx;
    auto req = MockPrepareRemoteImageRequest::create(&mock_threads,
                                                     m_local_io_ctx,
