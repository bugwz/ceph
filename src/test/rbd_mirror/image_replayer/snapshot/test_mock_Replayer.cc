...
template<> struct ImageCopyRequest<MockTestImageCtx>
{
    ...
    static ImageCopyRequest* create(MockTestImageCtx* src_image_ctx, MockTestImageCtx* dst_image_ctx,
                                    librados::snap_t src_snap_id_start, librados::snap_t src_snap_id_end,
                                    librados::snap_t dst_snap_id_start, bool flatten, const ObjectNumber& object_number,
                                    const SnapSeqs& snap_seqs, Handler* handler, Context* on_finish)
    {
    ...
    ImageCopyRequest() { s_instance = this; }

    MOCK_METHOD0(send, void());
};

template<> struct SnapshotCopyRequest<MockTestImageCtx>
{
    ...
    static SnapshotCopyRequest* create(MockTestImageCtx* src_image_ctx, MockTestImageCtx* dst_image_ctx,
                                       librados::snap_t src_snap_id_start, librados::snap_t src_snap_id_end,
                                       librados::snap_t dst_snap_id_start, bool flatten, ::MockContextWQ* work_queue,
    ...
    SnapshotCopyRequest() { s_instance = this; }
...
template<> struct ImageStateUpdateRequest<MockTestImageCtx>
{
    static ImageStateUpdateRequest* s_instance;
    static ImageStateUpdateRequest* create(librados::IoCtx& io_ctx, const std::string& image_id,
                                           cls::rbd::MirrorImageState mirror_image_state,
                                           const cls::rbd::MirrorImage& mirror_image, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        EXPECT_EQ(cls::rbd::MIRROR_IMAGE_STATE_ENABLED, mirror_image_state);
    ...
    Context* on_finish = nullptr;
    ImageStateUpdateRequest() { s_instance = this; }
...
template<> struct CreateNonPrimaryRequest<MockTestImageCtx>
{
    bool demoted = false;
    ...
    static CreateNonPrimaryRequest* create(MockTestImageCtx* image_ctx, bool demoted,
                                           const std::string& primary_mirror_uuid, uint64_t primary_snap_id,
                                           const SnapSeqs& snap_seqs, const ImageState& image_state, uint64_t* snap_id,
                                           Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
    ...
    CreateNonPrimaryRequest() { s_instance = this; }

    MOCK_METHOD0(send, void());
};

template<> struct GetImageStateRequest<MockTestImageCtx>
{
    uint64_t snap_id = CEPH_NOSNAP;

    static GetImageStateRequest* s_instance;
    static GetImageStateRequest* create(MockTestImageCtx* image_ctx, uint64_t snap_id, ImageState* image_state,
                                        Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->snap_id = snap_id;
    ...
    GetImageStateRequest() { s_instance = this; }

    MOCK_METHOD0(send, void());
};

template<> struct ImageMeta<MockTestImageCtx>
{
...
template<> struct UnlinkPeerRequest<MockTestImageCtx>
{
    uint64_t snap_id;
    ...
    static UnlinkPeerRequest* create(MockTestImageCtx* image_ctx, uint64_t snap_id, const std::string& mirror_peer_uuid,
    ...
    UnlinkPeerRequest() { s_instance = this; }
...
template<> struct InstanceWatcher<librbd::MockTestImageCtx>
{
...
template<> struct Threads<librbd::MockTestImageCtx>
{
    MockSafeTimer* timer;
...
template<> struct CloseImageRequest<librbd::MockTestImageCtx>
{
    static CloseImageRequest* s_instance;
    librbd::MockTestImageCtx** image_ctx = nullptr;
    Context* on_finish = nullptr;

    static CloseImageRequest* create(librbd::MockTestImageCtx** image_ctx, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
...
template<> struct ApplyImageStateRequest<librbd::MockTestImageCtx>
{
    Context* on_finish = nullptr;

    static ApplyImageStateRequest* s_instance;
    static ApplyImageStateRequest* create(const std::string& local_mirror_uuid, const std::string& remote_mirror_uuid,
                                          librbd::MockTestImageCtx* local_image_ctx,
                                          librbd::MockTestImageCtx* remote_image_ctx,
                                          const librbd::mirror::snapshot::ImageState& image_state, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->on_finish = on_finish;
        return s_instance;
    ...
    ApplyImageStateRequest() { s_instance = this; }

    MOCK_METHOD0(send, void());
};

template<> struct StateBuilder<librbd::MockTestImageCtx>
{
    StateBuilder(librbd::MockTestImageCtx& local_image_ctx, librbd::MockTestImageCtx& remote_image_ctx,
                 librbd::mirror::snapshot::ImageMeta<librbd::MockTestImageCtx>& local_image_meta)
    ...
    librbd::mirror::snapshot::ImageMeta<librbd::MockTestImageCtx>* local_image_meta = nullptr;
};

ApplyImageStateRequest<librbd::MockTestImageCtx>* ApplyImageStateRequest<librbd::MockTestImageCtx>::s_instance =
    nullptr;
...
public:
    ...
    {
        EXPECT_CALL(*mock_threads.work_queue, queue(_, _)).WillRepeatedly(Invoke([this](Context* ctx, int r) {
    ...
    void expect_add_event_after_repeatedly(MockThreads& mock_threads)
    {
        EXPECT_CALL(*mock_threads.timer, add_event_after(_, _))
            .WillRepeatedly(
                DoAll(Invoke([this](double seconds, Context* ctx) { m_threads->timer->add_event_after(seconds, ctx); }),
                      ReturnArg<1>()));
        EXPECT_CALL(*mock_threads.timer, cancel_event(_)).WillRepeatedly(Invoke([this](Context* ctx) {
            return m_threads->timer->cancel_event(ctx);
        }));
    }

    void expect_register_update_watcher(librbd::MockTestImageCtx& mock_image_ctx,
                                        librbd::UpdateWatchCtx** update_watch_ctx, uint64_t watch_handle, int r)
    {
        EXPECT_CALL(*mock_image_ctx.state, register_update_watcher(_, _))
            .WillOnce(Invoke([update_watch_ctx, watch_handle, r](librbd::UpdateWatchCtx* ctx, uint64_t* handle) {
                if (r >= 0) {
    ...
    void expect_unregister_update_watcher(librbd::MockTestImageCtx& mock_image_ctx, uint64_t watch_handle, int r)
    {
        EXPECT_CALL(*mock_image_ctx.state, unregister_update_watcher(watch_handle, _))
            .WillOnce(WithArg<1>(Invoke([this, r](Context* ctx) { m_threads->work_queue->queue(ctx, r); })));
    }

    void expect_load_image_meta(MockImageMeta& mock_image_meta, bool resync_requested, int r)
    {
    ...
    void expect_is_refresh_required(librbd::MockTestImageCtx& mock_image_ctx, bool is_required)
    {
        EXPECT_CALL(*mock_image_ctx.state, is_refresh_required()).WillOnce(Return(is_required));
    }

    void expect_refresh(librbd::MockTestImageCtx& mock_image_ctx, const std::map<uint64_t, librbd::SnapInfo>& snaps,
                        int r)
    {
        EXPECT_CALL(*mock_image_ctx.state, refresh(_)).WillOnce(Invoke([this, &mock_image_ctx, snaps, r](Context* ctx) {
    ...
    {
        EXPECT_CALL(mock_image_ctx, notify_update(_)).WillOnce(Invoke([this](Context* ctx) {
            m_threads->work_queue->queue(ctx, 0);
        }));
    }

    void expect_prune_non_primary_snapshot(librbd::MockTestImageCtx& mock_image_ctx, uint64_t snap_id, int r)
    {
        ...
        EXPECT_CALL(*mock_image_ctx.operations, snap_remove(_, _, _))
            .WillOnce(WithArg<2>(Invoke([this, r](Context* ctx) { m_threads->work_queue->queue(ctx, r); })));
    }

    void expect_snapshot_copy(MockSnapshotCopyRequest& mock_snapshot_copy_request, uint64_t src_snap_id_start,
                              uint64_t src_snap_id_end, uint64_t dst_snap_id_start, const librbd::SnapSeqs& snap_seqs,
                              int r)
    {
        EXPECT_CALL(mock_snapshot_copy_request, send())
            .WillOnce(Invoke([this,
                              &req = mock_snapshot_copy_request,
                              src_snap_id_start,
                              src_snap_id_end,
                              dst_snap_id_start,
                              snap_seqs,
    ...
    void expect_get_image_state(MockGetImageStateRequest& mock_get_image_state_request, uint64_t snap_id, int r)
    {
    ...
    void expect_create_non_primary_request(MockCreateNonPrimaryRequest& mock_create_non_primary_request, bool demoted,
                                           const std::string& primary_mirror_uuid, uint64_t primary_snap_id,
                                           const librbd::SnapSeqs& snap_seqs, uint64_t snap_id, int r)
    {
        EXPECT_CALL(mock_create_non_primary_request, send())
            .WillOnce(Invoke([this,
                              &req = mock_create_non_primary_request,
                              demoted,
                              primary_mirror_uuid,
                              primary_snap_id,
                              snap_seqs,
    ...
    void expect_update_mirror_image_state(MockImageStateUpdateRequest& mock_image_state_update_request, int r)
    {
    ...
    void expect_notify_sync_request(MockInstanceWatcher& mock_instance_watcher, const std::string& image_id, int r)
    {
        EXPECT_CALL(mock_instance_watcher, notify_sync_request(image_id, _))
            .WillOnce(WithArg<1>(Invoke([this, r](Context* ctx) { m_threads->work_queue->queue(ctx, r); })));
    }

    void expect_notify_sync_complete(MockInstanceWatcher& mock_instance_watcher, const std::string& image_id)
    {
        EXPECT_CALL(mock_instance_watcher, notify_sync_complete(image_id));
    }

    void expect_cancel_sync_request(MockInstanceWatcher& mock_instance_watcher, const std::string& image_id)
    {
        EXPECT_CALL(mock_instance_watcher, cancel_sync_request(image_id));
    }

    void expect_image_copy(MockImageCopyRequest& mock_image_copy_request, uint64_t src_snap_id_start,
                           uint64_t src_snap_id_end, uint64_t dst_snap_id_start,
                           const librbd::deep_copy::ObjectNumber& object_number, const librbd::SnapSeqs& snap_seqs,
                           int r)
    {
        EXPECT_CALL(mock_image_copy_request, send())
            .WillOnce(Invoke([this,
                              &req = mock_image_copy_request,
                              src_snap_id_start,
                              src_snap_id_end,
                              dst_snap_id_start,
                              object_number,
    ...
    void expect_unlink_peer(MockUnlinkPeerRequest& mock_unlink_peer_request, uint64_t snap_id,
                            const std::string& mirror_peer_uuid, bool allow_remove, int r)
    {
        EXPECT_CALL(mock_unlink_peer_request, send())
            .WillOnce(Invoke([this, &req = mock_unlink_peer_request, snap_id, mirror_peer_uuid, allow_remove, r]() {
                ASSERT_EQ(snap_id, req.snap_id);
    ...
    void expect_apply_image_state(MockApplyImageStateRequest& mock_request, int r)
    {
        EXPECT_CALL(mock_request, send()).WillOnce(Invoke([this, &req = mock_request, r]() {
            m_threads->work_queue->queue(req.on_finish, r);
        }));
    }

    void expect_mirror_image_snapshot_set_copy_progress(librbd::MockTestImageCtx& mock_test_image_ctx, uint64_t snap_id,
                                                        bool completed, uint64_t last_copied_object, int r)
    {
        ...
        EXPECT_CALL(get_mock_io_ctx(mock_test_image_ctx.md_ctx),
                    exec(mock_test_image_ctx.header_oid,
                         _,
                         StrEq("rbd"),
    ...
    {
        EXPECT_CALL(mock_close_image_request, send()).WillOnce(Invoke([this, &mock_close_image_request, r]() {
            *mock_close_image_request.image_ctx = nullptr;
            m_threads->work_queue->queue(mock_close_image_request.on_finish, r);
        }));
    }

    void expect_notification(MockThreads& mock_threads, MockReplayerListener& mock_replayer_listener)
    {
        EXPECT_CALL(mock_replayer_listener, handle_notification()).WillRepeatedly(Invoke([this]() {
    ...
    int init_entry_replayer(MockReplayer& mock_replayer, MockThreads& mock_threads,
                            ...
                            MockReplayerListener& mock_replayer_listener, MockImageMeta& mock_image_meta,
                            librbd::UpdateWatchCtx** update_watch_ctx)
    {
        expect_register_update_watcher(mock_local_image_ctx, update_watch_ctx, 123, 0);
        expect_register_update_watcher(mock_remote_image_ctx, update_watch_ctx, 234, 0);
    ...
    int shut_down_entry_replayer(MockReplayer& mock_replayer, MockThreads& mock_threads,
                                 librbd::MockTestImageCtx& mock_local_image_ctx,
                                 librbd::MockTestImageCtx& mock_remote_image_ctx)
    {
    ...
    ceph::mutex m_lock = ceph::make_mutex("TestMockImageReplayerSnapshotReplayer");
...
TEST_F(TestMockImageReplayerSnapshotReplayer, InitShutDown)
{
    ...
    MockStateBuilder mock_state_builder(mock_local_image_ctx, mock_remote_image_ctx, mock_image_meta);
    MockReplayer mock_replayer{&mock_threads,
                               &mock_instance_watcher,
                               "local mirror uuid",
                               &m_pool_meta_cache,
                               &mock_state_builder,
                               &mock_replayer_listener};
    m_pool_meta_cache.set_remote_pool_meta(m_remote_io_ctx.get_id(), {"remote mirror uuid", "remote mirror peer uuid"});

    librbd::UpdateWatchCtx* update_watch_ctx = nullptr;
    ASSERT_EQ(0,
              init_entry_replayer(mock_replayer,
                                  mock_threads,
                                  ...
                                  mock_image_meta,
                                  &update_watch_ctx));
    ASSERT_EQ(0, shut_down_entry_replayer(mock_replayer, mock_threads, mock_local_image_ctx, mock_remote_image_ctx));
}
...
TEST_F(TestMockImageReplayerSnapshotReplayer, SyncSnapshot)
{
        ...
        {1U,
         librbd::SnapInfo{
             ...
             cls::rbd::MirrorSnapshotNamespace{
                 cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {"remote mirror peer uuid"}, "", CEPH_NOSNAP, true, 0, {}},
             0,
             {},
             0,
             0,
             {}}},
        {2U, librbd::SnapInfo{"snap2", cls::rbd::UserSnapshotNamespace{}, 0, {}, 0, 0, {}}},
        {3U,
         librbd::SnapInfo{"snap3",
                          cls::rbd::MirrorSnapshotNamespace{
    ...
    expect_snapshot_copy(mock_snapshot_copy_request, 0, 1, 0, {{1, CEPH_NOSNAP}}, 0);
    ...
    expect_create_non_primary_request(
    ...
    expect_image_copy(mock_image_copy_request, 0, 1, 0, {}, {{1, CEPH_NOSNAP}}, 0);
    MockApplyImageStateRequest mock_apply_state_request;
    expect_apply_image_state(mock_apply_state_request, 0);
    expect_mirror_image_snapshot_set_copy_progress(mock_local_image_ctx, 11, true, 0, 0);
    ...
    expect_refresh(mock_local_image_ctx,
                   {
                       {11U,
                        librbd::SnapInfo{"snap1",
                                         cls::rbd::MirrorSnapshotNamespace{cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY,
                                                                           {},
                                                                           "remote mirror uuid",
                                                                           1,
                                                                           true,
        ...
        {{1U,
         ...
         {5U,
          librbd::SnapInfo{
              ...
              {}}}},
        0);
    expect_snapshot_copy(mock_snapshot_copy_request, 1, 4, 11, {{1, 11}, {2, 12}, {4, CEPH_NOSNAP}}, 0);
    expect_get_image_state(mock_get_image_state_request, 4, 0);
    expect_create_non_primary_request(
        mock_create_non_primary_request, false, "remote mirror uuid", 4, {{1, 11}, {2, 12}, {4, CEPH_NOSNAP}}, 14, 0);
    expect_notify_sync_request(mock_instance_watcher, mock_local_image_ctx.id, 0);
    expect_image_copy(mock_image_copy_request, 1, 4, 11, {}, {{1, 11}, {2, 12}, {4, CEPH_NOSNAP}}, 0);
    expect_apply_image_state(mock_apply_state_request, 0);
    expect_mirror_image_snapshot_set_copy_progress(mock_local_image_ctx, 14, true, 0, 0);
    expect_notify_update(mock_local_image_ctx);
    MockUnlinkPeerRequest mock_unlink_peer_request;
    expect_unlink_peer(mock_unlink_peer_request, 1, "remote mirror peer uuid", false, 0);
    expect_notify_sync_complete(mock_instance_watcher, mock_local_image_ctx.id);
    ...
    expect_refresh(mock_local_image_ctx,
                   {
                       {11U,
                        librbd::SnapInfo{
                            "snap1",
                            cls::rbd::MirrorSnapshotNamespace{
                                cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY, {}, "remote mirror uuid", 1, true, 0, {}},
                            0,
                            {},
                            0,
                            0,
                            {}}},
                       {12U, librbd::SnapInfo{"snap2", cls::rbd::UserSnapshotNamespace{}, 0, {}, 0, 0, {}}},
                       {14U,
                        librbd::SnapInfo{
                            "snap4",
                            cls::rbd::MirrorSnapshotNamespace{
                                cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY, {}, "remote mirror uuid", 4, true, 0, {}},
                            0,
                            {},
                            0,
                            0,
...
TEST_F(TestMockImageReplayerSnapshotReplayer, InterruptedSyncInitial)
{
    ...
    expect_notify_sync_request(mock_instance_watcher, mock_local_image_ctx.id, 0);
    MockImageCopyRequest mock_image_copy_request;
    expect_image_copy(mock_image_copy_request, 0, 1, 0, librbd::deep_copy::ObjectNumber{123U}, {{1, CEPH_NOSNAP}}, 0);
    MockApplyImageStateRequest mock_apply_state_request;
    expect_apply_image_state(mock_apply_state_request, 0);
    expect_mirror_image_snapshot_set_copy_progress(mock_local_image_ctx, 11, true, 123, 0);
    expect_notify_update(mock_local_image_ctx);
    expect_notify_sync_complete(mock_instance_watcher, mock_local_image_ctx.id);
...
TEST_F(TestMockImageReplayerSnapshotReplayer, InterruptedSyncDelta)
{
    ...
    mock_local_image_ctx.snap_info = {
        {11U,
         librbd::SnapInfo{
             "snap1",
             cls::rbd::MirrorSnapshotNamespace{
                 cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY, {}, "remote mirror uuid", 1, true, 0, {{1, CEPH_NOSNAP}}},
             0,
             {},
             0,
             0,
    ...
    expect_notify_sync_request(mock_instance_watcher, mock_local_image_ctx.id, 0);
    MockImageCopyRequest mock_image_copy_request;
    expect_image_copy(mock_image_copy_request, 1, 2, 11, librbd::deep_copy::ObjectNumber{123U}, {{2, CEPH_NOSNAP}}, 0);
    MockApplyImageStateRequest mock_apply_state_request;
    expect_apply_image_state(mock_apply_state_request, 0);
    expect_mirror_image_snapshot_set_copy_progress(mock_local_image_ctx, 12, true, 123, 0);
    expect_notify_update(mock_local_image_ctx);
    MockUnlinkPeerRequest mock_unlink_peer_request;
    expect_unlink_peer(mock_unlink_peer_request, 1, "remote mirror peer uuid", false, 0);
    expect_notify_sync_complete(mock_instance_watcher, mock_local_image_ctx.id);
    ...
    expect_refresh(mock_local_image_ctx,
                   {
                       ...
                       {12U,
                        librbd::SnapInfo{
                            "snap2",
                            cls::rbd::MirrorSnapshotNamespace{
                                cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY, {}, "remote mirror uuid", 2, true, 0, {}},
                            0,
                            {},
                            0,
                            0,
                   ...
                   0);
    expect_is_refresh_required(mock_remote_image_ctx, true);
    expect_refresh(mock_remote_image_ctx,
                   {
                       {2U,
                        librbd::SnapInfo{"snap2",
                                         cls::rbd::MirrorSnapshotNamespace{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY,
...
TEST_F(TestMockImageReplayerSnapshotReplayer, InterruptedSyncDeltaDemote)
{
    ...
    mock_remote_image_ctx.snap_info = {
        {1U,
         librbd::SnapInfo{"snap1",
                          cls::rbd::MirrorSnapshotNamespace{cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY_DEMOTED,
                                                            {"remote mirror peer uuid"},
                                                            "local mirror uuid",
                                                            11,
                                                            true,
    ...
    mock_local_image_ctx.snap_info = {
        {11U,
         librbd::SnapInfo{"snap1",
                          cls::rbd::MirrorSnapshotNamespace{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED,
                                                            {"remote mirror peer uuid"},
                                                            "",
                                                            CEPH_NOSNAP,
                                                            true,
...
TEST_F(TestMockImageReplayerSnapshotReplayer, InterruptedPendingSyncDelta)
{
    ...
    expect_is_refresh_required(mock_remote_image_ctx, false);
    MockSnapshotCopyRequest mock_snapshot_copy_request;
    expect_snapshot_copy(mock_snapshot_copy_request, 1, 2, 11, {{2, CEPH_NOSNAP}}, 0);
    MockGetImageStateRequest mock_get_image_state_request;
    expect_get_image_state(mock_get_image_state_request, 2, 0);
    MockCreateNonPrimaryRequest mock_create_non_primary_request;
    expect_create_non_primary_request(
        mock_create_non_primary_request, false, "remote mirror uuid", 2, {{2, CEPH_NOSNAP}}, 13, 0);
    expect_notify_sync_request(mock_instance_watcher, mock_local_image_ctx.id, 0);
    MockImageCopyRequest mock_image_copy_request;
    expect_image_copy(mock_image_copy_request, 1, 2, 11, {}, {{2, CEPH_NOSNAP}}, 0);
    MockApplyImageStateRequest mock_apply_state_request;
    expect_apply_image_state(mock_apply_state_request, 0);
    expect_mirror_image_snapshot_set_copy_progress(mock_local_image_ctx, 13, true, 0, 0);
    expect_notify_update(mock_local_image_ctx);
    MockUnlinkPeerRequest mock_unlink_peer_request;
    expect_unlink_peer(mock_unlink_peer_request, 1, "remote mirror peer uuid", false, 0);
    expect_notify_sync_complete(mock_instance_watcher, mock_local_image_ctx.id);
...
TEST_F(TestMockImageReplayerSnapshotReplayer, RegisterLocalUpdateWatcherError)
{
    ...
    // init
    librbd::UpdateWatchCtx* update_watch_ctx = nullptr;
    expect_register_update_watcher(mock_local_image_ctx, &update_watch_ctx, 123, -EINVAL);
...
TEST_F(TestMockImageReplayerSnapshotReplayer, RegisterRemoteUpdateWatcherError)
{
    ...
    librbd::UpdateWatchCtx* update_watch_ctx = nullptr;
    expect_register_update_watcher(mock_local_image_ctx, &update_watch_ctx, 123, 0);
    expect_register_update_watcher(mock_remote_image_ctx, &update_watch_ctx, 234, -EINVAL);
...
TEST_F(TestMockImageReplayerSnapshotReplayer, CopySnapshotsError)
{
    ...
    expect_is_refresh_required(mock_remote_image_ctx, false);
    MockSnapshotCopyRequest mock_snapshot_copy_request;
    expect_snapshot_copy(mock_snapshot_copy_request, 0, 1, 0, {{1, CEPH_NOSNAP}}, -EINVAL);
...
TEST_F(TestMockImageReplayerSnapshotReplayer, CreateNonPrimarySnapshotError)
{
    ...
    MockCreateNonPrimaryRequest mock_create_non_primary_request;
    expect_create_non_primary_request(
        mock_create_non_primary_request, false, "remote mirror uuid", 1, {{1, CEPH_NOSNAP}}, 11, -EINVAL);
...
TEST_F(TestMockImageReplayerSnapshotReplayer, RequestSyncError)
{
    ...
    MockImageStateUpdateRequest mock_image_state_update_request;
    expect_update_mirror_image_state(mock_image_state_update_request, 0);
    expect_notify_sync_request(mock_instance_watcher, mock_local_image_ctx.id, -ECANCELED);
...
TEST_F(TestMockImageReplayerSnapshotReplayer, CopyImageError)
{
    ...
    expect_notify_sync_request(mock_instance_watcher, mock_local_image_ctx.id, 0);
    MockImageCopyRequest mock_image_copy_request;
    expect_image_copy(mock_image_copy_request, 0, 1, 0, {}, {{1, CEPH_NOSNAP}}, -EINVAL);
    expect_notify_sync_complete(mock_instance_watcher, mock_local_image_ctx.id);
...
TEST_F(TestMockImageReplayerSnapshotReplayer, UpdateNonPrimarySnapshotError)
{
    ...
    MockApplyImageStateRequest mock_apply_state_request;
    expect_apply_image_state(mock_apply_state_request, 0);
    expect_mirror_image_snapshot_set_copy_progress(mock_local_image_ctx, 11, true, 0, -EINVAL);
    expect_notify_sync_complete(mock_instance_watcher, mock_local_image_ctx.id);
...
TEST_F(TestMockImageReplayerSnapshotReplayer, UnlinkPeerError)
{
    ...
    MockCreateNonPrimaryRequest mock_create_non_primary_request;
    expect_create_non_primary_request(
        mock_create_non_primary_request, false, "remote mirror uuid", 2, {{2, CEPH_NOSNAP}}, 12, 0);
    expect_notify_sync_request(mock_instance_watcher, mock_local_image_ctx.id, 0);
    MockImageCopyRequest mock_image_copy_request;
    expect_image_copy(mock_image_copy_request, 1, 2, 11, {}, {{2, CEPH_NOSNAP}}, 0);
    MockApplyImageStateRequest mock_apply_state_request;
    expect_apply_image_state(mock_apply_state_request, 0);
    expect_mirror_image_snapshot_set_copy_progress(mock_local_image_ctx, 12, true, 0, 0);
    expect_notify_update(mock_local_image_ctx);
    MockUnlinkPeerRequest mock_unlink_peer_request;
    expect_unlink_peer(mock_unlink_peer_request, 1, "remote mirror peer uuid", false, -EINVAL);
    expect_notify_sync_complete(mock_instance_watcher, mock_local_image_ctx.id);
...
TEST_F(TestMockImageReplayerSnapshotReplayer, SplitBrain)
{
    ...
    mock_local_image_ctx.snap_info = {
        {1U,
         librbd::SnapInfo{"snap1",
                          cls::rbd::MirrorSnapshotNamespace{
                              cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED, {}, "", CEPH_NOSNAP, true, 0, {}},
                          0,
                          {},
                          0,
                          0,
...
TEST_F(TestMockImageReplayerSnapshotReplayer, RemoteFailover)
{
    ...
    // inject a primary demote to local image
    mock_remote_image_ctx.snap_info = {
        {1U, librbd::SnapInfo{"snap1", cls::rbd::UserSnapshotNamespace{}, 0, {}, 0, 0, {}}},
        {2U,
         librbd::SnapInfo{"snap2",
                          cls::rbd::MirrorSnapshotNamespace{cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY_DEMOTED,
                                                            {"remote mirror peer uuid"},
        ...
        {3U,
         librbd::SnapInfo{
             ...
             0,
             {}}}};
    mock_local_image_ctx.snap_ids = {{{cls::rbd::UserSnapshotNamespace{}, "snap1"}, 11},
                                     {{cls::rbd::MirrorSnapshotNamespace{}, "snap2"}, 12}};
    mock_local_image_ctx.snap_info = {
        {11U, librbd::SnapInfo{"snap1", cls::rbd::UserSnapshotNamespace{}, 0, {}, 0, 0, {}}},
        {12U,
         librbd::SnapInfo{"snap2",
                          cls::rbd::MirrorSnapshotNamespace{
                              cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED, {}, "", CEPH_NOSNAP, true, 0, {}},
    ...
    expect_is_refresh_required(mock_remote_image_ctx, false);
    MockSnapshotCopyRequest mock_snapshot_copy_request;
    expect_snapshot_copy(mock_snapshot_copy_request, 2, 3, 12, {{2, 12}, {3, CEPH_NOSNAP}}, 0);
    MockGetImageStateRequest mock_get_image_state_request;
    expect_get_image_state(mock_get_image_state_request, 3, 0);
    MockCreateNonPrimaryRequest mock_create_non_primary_request;
    expect_create_non_primary_request(
        mock_create_non_primary_request, false, "remote mirror uuid", 3, {{1, 11}, {2, 12}, {3, CEPH_NOSNAP}}, 13, 0);
    expect_notify_sync_request(mock_instance_watcher, mock_local_image_ctx.id, 0);
    MockImageCopyRequest mock_image_copy_request;
    expect_image_copy(mock_image_copy_request, 2, 3, 12, {}, {{1, 11}, {2, 12}, {3, CEPH_NOSNAP}}, 0);
    MockApplyImageStateRequest mock_apply_state_request;
    expect_apply_image_state(mock_apply_state_request, 0);
    expect_mirror_image_snapshot_set_copy_progress(mock_local_image_ctx, 13, true, 0, 0);
    expect_notify_update(mock_local_image_ctx);
    MockUnlinkPeerRequest mock_unlink_peer_request;
    expect_unlink_peer(mock_unlink_peer_request, 2, "remote mirror peer uuid", false, 0);
    expect_notify_sync_complete(mock_instance_watcher, mock_local_image_ctx.id);
    ...
    mock_remote_image_ctx.snap_info = {
        {1U,
         librbd::SnapInfo{
             "snap1",
             cls::rbd::MirrorSnapshotNamespace{
                 cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {"remote mirror peer uuid"}, "", 0U, true, 0, {}},
             0,
             {},
             0,
             0,
...
TEST_F(TestMockImageReplayerSnapshotReplayer, ImageNameUpdated)
{
    ...
    // wait for sync to complete and expect replay complete
    ASSERT_EQ(0, wait_for_notification(2));
    auto image_spec = image_replayer::util::compute_image_spec(m_local_io_ctx, "NEW NAME");
...
TEST_F(TestMockImageReplayerSnapshotReplayer, ApplyImageStatePendingShutdown)
{
    ...
    EXPECT_CALL(mock_apply_state_request, send())
        .WillOnce(Invoke([this, &req = mock_apply_state_request, &replayer = mock_replayer, &ctx = shutdown_ctx]() {
