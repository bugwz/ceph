...
template<> struct GetMetadataRequest<MockTestImageCtx>
{
    std::map<std::string, bufferlist>* pairs = nullptr;
    Context* on_finish = nullptr;

    static GetMetadataRequest* s_instance;
    static GetMetadataRequest* create(librados::IoCtx& io_ctx, const std::string& oid, bool filter_internal,
                                      const std::string& filter_key_prefix, const std::string& last_key,
                                      size_t max_results, std::map<std::string, bufferlist>* pairs, Context* on_finish)
    {
...
public:
    ...
    {
        EXPECT_CALL(*m_mock_local_image_ctx->operations, execute_update_features(features, enable, _, 0U))
    ...
    void expect_get_metadata(MockGetMetadataRequest& mock_get_metadata_request,
                             const std::map<std::string, bufferlist>& pairs, int r)
    {
        EXPECT_CALL(mock_get_metadata_request, send()).WillOnce(Invoke([this, &mock_get_metadata_request, pairs, r]() {
            *mock_get_metadata_request.pairs = pairs;
            m_threads->work_queue->queue(mock_get_metadata_request.on_finish, r);
        }));
    }

    void expect_update_metadata(const std::vector<std::string>& remove, const std::map<std::string, bufferlist>& pairs,
                                int r)
    {
        for (auto& key : remove) {
            bufferlist bl;
            ...
            EXPECT_CALL(get_mock_io_ctx(m_mock_local_image_ctx->md_ctx),
                        exec(m_mock_local_image_ctx->header_oid,
                             _,
                             StrEq("rbd"),
                             StrEq("metadata_remove"),
                             ContentsEqual(bl),
                             _,
                             _,
                             _))
    ...
    void expect_unprotect_snapshot(const std::string& name, int r)
    {
        EXPECT_CALL(*m_mock_local_image_ctx->operations,
                    execute_snap_unprotect({cls::rbd::UserSnapshotNamespace{}}, name, _))
            .WillOnce(WithArg<2>(Invoke([this, r](Context* ctx) { m_threads->work_queue->queue(ctx, r); })));
    }
    ...
    {
        EXPECT_CALL(*m_mock_local_image_ctx->operations,
                    execute_snap_remove({cls::rbd::UserSnapshotNamespace{}}, name, _))
            .WillOnce(WithArg<2>(Invoke([this, r](Context* ctx) { m_threads->work_queue->queue(ctx, r); })));
    }
    ...
    {
        EXPECT_CALL(*m_mock_local_image_ctx->operations,
                    execute_snap_protect({cls::rbd::UserSnapshotNamespace{}}, name, _))
            .WillOnce(WithArg<2>(Invoke([this, r](Context* ctx) { m_threads->work_queue->queue(ctx, r); })));
    }

    void expect_rename_snapshot(uint64_t snap_id, const std::string& name, int r)
    {
        EXPECT_CALL(*m_mock_local_image_ctx->operations, execute_snap_rename(snap_id, name, _))
            .WillOnce(WithArg<2>(Invoke([this, r](Context* ctx) { m_threads->work_queue->queue(ctx, r); })));
    }
    ...
    {
        EXPECT_CALL(*m_mock_local_image_ctx->operations, execute_snap_set_limit(limit, _))
            .WillOnce(WithArg<1>(Invoke([this, r](Context* ctx) { m_threads->work_queue->queue(ctx, r); })));
    }
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest, NoChanges)
{
    ...
    auto req = MockApplyImageStateRequest::create(
        "local mirror uuid", "remote mirror uuid", m_mock_local_image_ctx, m_mock_remote_image_ctx, image_state, &ctx);
    req->send();
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest, UpdateFeatures)
{
    ...
    librbd::mirror::snapshot::ImageState image_state;
    image_state.name = m_image_name;
    image_state.features = RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_OBJECT_MAP;
    m_mock_local_image_ctx->features = RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_DEEP_FLATTEN;
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest, UpdateImageMeta)
{
    ...
    MockGetMetadataRequest mock_get_metadata_request;
    expect_get_metadata(mock_get_metadata_request, {{"key1", {}}, {"key2", {}}}, 0);
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest, GetImageMetaError)
{
    ...
    ceph::encode("data", data_bl);
    MockGetMetadataRequest mock_get_metadata_request;
    expect_get_metadata(mock_get_metadata_request, {{"key1", {}}, {"key2", {}}}, -EINVAL);
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest, UnprotectSnapshot)
{
    ...
    image_state.name = m_image_name;
    image_state.features = m_remote_image_ctx->features;
    image_state.snapshots = {{1U, {cls::rbd::UserSnapshotNamespace{}, "snap1", RBD_PROTECTION_STATUS_UNPROTECTED}}};
    m_mock_local_image_ctx->snap_info = {
        {11U,
         librbd::SnapInfo{"snap1", cls::rbd::UserSnapshotNamespace{}, 0U, {}, RBD_PROTECTION_STATUS_PROTECTED, 0, {}}},
        {12U,
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest, RemoveSnapshot)
{
    ...
    m_mock_local_image_ctx->snap_info = {
        {11U,
         librbd::SnapInfo{
             "snap1", cls::rbd::UserSnapshotNamespace{}, 0U, {}, RBD_PROTECTION_STATUS_UNPROTECTED, 0, {}}},
        {12U,
         librbd::SnapInfo{"snap2",
                          cls::rbd::MirrorSnapshotNamespace{cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY,
                                                            {},
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest, ProtectSnapshot)
{
    ...
    image_state.name = m_image_name;
    image_state.features = m_remote_image_ctx->features;
    image_state.snapshots = {{1U, {cls::rbd::UserSnapshotNamespace{}, "snap1", RBD_PROTECTION_STATUS_PROTECTED}}};
    m_mock_local_image_ctx->snap_info = {
        {11U,
         librbd::SnapInfo{
             "snap1", cls::rbd::UserSnapshotNamespace{}, 0U, {}, RBD_PROTECTION_STATUS_UNPROTECTED, 0, {}}},
...
TEST_F(TestMockImageReplayerSnapshotApplyImageStateRequest, RenameSnapshot)
{
    ...
    image_state.snapshots = {
        {1U, {cls::rbd::UserSnapshotNamespace{}, "snap1-renamed", RBD_PROTECTION_STATUS_PROTECTED}}};
    m_mock_local_image_ctx->snap_info = {
        {11U,
         librbd::SnapInfo{"snap1", cls::rbd::UserSnapshotNamespace{}, 0U, {}, RBD_PROTECTION_STATUS_PROTECTED, 0, {}}},
        {12U,
