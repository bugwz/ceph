...
template<> std::string generate_image_id<MockTestImageCtx>(librados::IoCtx&)
{
...
template<> struct CreateImageRequest<librbd::MockTestImageCtx>
{
    static CreateImageRequest* s_instance;
    Context* on_finish = nullptr;

    static CreateImageRequest* create(Threads<librbd::MockTestImageCtx>* threads, librados::IoCtx& local_io_ctx,
                                      ...
                                      librbd::MockTestImageCtx* remote_image_ctx, PoolMetaCache* pool_meta_cache,
...
CreateImageRequest<librbd::MockTestImageCtx>* CreateImageRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
...
public:
    ...
    void expect_mirror_image_remove(const std::string& image_id, int r)
    {
        ...
        EXPECT_CALL(
            ...
            exec(StrEq("rbd_mirroring"), _, StrEq("rbd"), StrEq("mirror_image_remove"), ContentsEqual(bl), _, _, _))
            .WillOnce(Return(r));
    }

    void expect_create_image(MockCreateImageRequest& mock_create_image_request, const std::string& image_id, int r)
    {
        EXPECT_CALL(mock_create_image_request, construct(image_id));
        EXPECT_CALL(mock_create_image_request, send()).WillOnce(Invoke([this, &mock_create_image_request, r]() {
            m_threads->work_queue->queue(mock_create_image_request.on_finish, r);
        }));
    }

    MockCreateLocalImageRequest* create_request(MockThreads& mock_threads, MockStateBuilder& mock_state_builder,
                                                const std::string& global_image_id, Context* on_finish)
    {
        return new MockCreateLocalImageRequest(&mock_threads,
                                               m_local_io_ctx,
...
TEST_F(TestMockImageReplayerSnapshotCreateLocalImageRequest, Success)
{
    ...
    auto request = create_request(mock_threads, mock_state_builder, "global image id", &ctx);
    request->send();
