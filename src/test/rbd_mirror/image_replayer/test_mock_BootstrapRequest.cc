...
template<> struct ImageSync<librbd::MockTestImageCtx>
{
    static ImageSync* s_instance;
    Context* on_finish = nullptr;

    static ImageSync* create(Threads<librbd::MockTestImageCtx>* threads, librbd::MockTestImageCtx* local_image_ctx,
                             librbd::MockTestImageCtx* remote_image_ctx, const std::string& local_mirror_uuid,
                             image_sync::SyncPointHandler* sync_point_handler,
                             InstanceWatcher<librbd::MockTestImageCtx>* instance_watcher, ProgressContext* progress_ctx,
                             Context* on_finish)
    {
...
template<> struct OpenImageRequest<librbd::MockTestImageCtx>
{
    static OpenImageRequest* s_instance;
    librbd::MockTestImageCtx** image_ctx = nullptr;
    Context* on_finish = nullptr;

    static OpenImageRequest* create(librados::IoCtx& io_ctx, librbd::MockTestImageCtx** image_ctx,
                                    const std::string& image_id, bool read_only, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
    ...
    ~OpenImageRequest() { s_instance = nullptr; }

    MOCK_METHOD2(construct, void(librados::IoCtx& io_ctx, const std::string& image_id));
    MOCK_METHOD0(send, void());
};

template<> struct OpenLocalImageRequest<librbd::MockTestImageCtx>
{
    static OpenLocalImageRequest* s_instance;
    librbd::MockTestImageCtx** image_ctx = nullptr;
    Context* on_finish = nullptr;

    static OpenLocalImageRequest* create(librados::IoCtx& local_io_ctx, librbd::MockTestImageCtx** local_image_ctx,
                                         const std::string& local_image_id, librbd::asio::ContextWQ* work_queue,
                                         Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
    ...
    ~OpenLocalImageRequest() { s_instance = nullptr; }

    MOCK_METHOD2(construct, void(librados::IoCtx& io_ctx, const std::string& image_id));
    MOCK_METHOD0(send, void());
};

template<> struct PrepareLocalImageRequest<librbd::MockTestImageCtx>
{
    ...
    static PrepareLocalImageRequest* create(librados::IoCtx&, const std::string& global_image_id,
                                            std::string* local_image_name,
                                            StateBuilder<librbd::MockTestImageCtx>** state_builder,
                                            librbd::asio::ContextWQ* work_queue, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
    ...
    PrepareLocalImageRequest() { s_instance = this; }

    MOCK_METHOD0(send, void());
};

template<> struct PrepareRemoteImageRequest<librbd::MockTestImageCtx>
{
    static PrepareRemoteImageRequest* s_instance;
    StateBuilder<librbd::MockTestImageCtx>** state_builder = nullptr;
    Context* on_finish = nullptr;

    static PrepareRemoteImageRequest* create(Threads<librbd::MockTestImageCtx>* threads, librados::IoCtx&,
                                             librados::IoCtx&, const std::string& global_image_id,
                                             const std::string& local_mirror_uuid,
                                             const RemotePoolMeta& remote_pool_meta,
                                             ::journal::CacheManagerHandler* cache_manager_handler,
                                             StateBuilder<librbd::MockTestImageCtx>** state_builder, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->state_builder = state_builder;
        s_instance->on_finish = on_finish;
    ...
    PrepareRemoteImageRequest() { s_instance = this; }
...
template<> struct StateBuilder<librbd::MockTestImageCtx>
{
    ...
    image_sync::MockSyncPointHandler* create_sync_point_handler() { return &mock_sync_point_handler; }

    StateBuilder() { s_instance = this; }
    ...
    MOCK_METHOD6(create_local_image_request,
                 BaseRequest*(Threads<librbd::MockTestImageCtx>*, librados::IoCtx&, const std::string&, PoolMetaCache*,
                              ProgressContext*, Context*));
    MOCK_METHOD5(create_prepare_replay_request,
                 BaseRequest*(const std::string&, ProgressContext*, bool*, bool*, Context*));

    void destroy_sync_point_handler() {}
    void destroy() {}
};

OpenImageRequest<librbd::MockTestImageCtx>* OpenImageRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
OpenLocalImageRequest<librbd::MockTestImageCtx>* OpenLocalImageRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
PrepareLocalImageRequest<librbd::MockTestImageCtx>* PrepareLocalImageRequest<librbd::MockTestImageCtx>::s_instance =
    nullptr;
PrepareRemoteImageRequest<librbd::MockTestImageCtx>* PrepareRemoteImageRequest<librbd::MockTestImageCtx>::s_instance =
    nullptr;
...
public:
    ...
    void expect_send(MockPrepareLocalImageRequest& mock_request, MockStateBuilder& mock_state_builder,
                     const std::string& local_image_id, const std::string& local_image_name, int r)
    {
        EXPECT_CALL(mock_request, send())
            .WillOnce(Invoke([&mock_request, &mock_state_builder, local_image_id, local_image_name, r]() {
                if (r == 0) {
    ...
    void expect_send(MockPrepareRemoteImageRequest& mock_request, MockStateBuilder& mock_state_builder,
                     const std::string& remote_mirror_uuid, const std::string& remote_image_id, int r)
    {
        EXPECT_CALL(mock_request, send())
            .WillOnce(Invoke([&mock_request, &mock_state_builder, remote_mirror_uuid, remote_image_id, r]() {
                if (r >= 0) {
                    *mock_request.state_builder = &mock_state_builder;
    ...
    void expect_is_local_primary(MockStateBuilder& mock_state_builder, bool is_primary)
    {
        EXPECT_CALL(mock_state_builder, is_local_primary()).WillOnce(Return(is_primary));
    }

    void expect_is_remote_primary(MockStateBuilder& mock_state_builder, bool is_primary)
    {
        EXPECT_CALL(mock_state_builder, is_remote_primary()).WillOnce(Return(is_primary));
    }

    void expect_is_linked(MockStateBuilder& mock_state_builder, bool is_linked)
    {
        EXPECT_CALL(mock_state_builder, is_linked()).WillOnce(Return(is_linked));
    }

    void expect_is_disconnected(MockStateBuilder& mock_state_builder, bool is_disconnected)
    {
        EXPECT_CALL(mock_state_builder, is_disconnected()).WillOnce(Return(is_disconnected));
    }

    void expect_replay_requires_remote_image(MockStateBuilder& mock_state_builder, bool requires_image)
    {
        EXPECT_CALL(mock_state_builder, replay_requires_remote_image()).WillOnce(Return(requires_image));
    }

    void expect_open_image(MockOpenImageRequest& mock_open_image_request, librados::IoCtx& io_ctx,
                           const std::string& image_id, librbd::MockTestImageCtx& mock_image_ctx, int r)
    {
    ...
    void expect_open_local_image(MockOpenLocalImageRequest& mock_open_local_image_request, librados::IoCtx& io_ctx,
                                 const std::string& image_id, librbd::MockTestImageCtx* mock_image_ctx, int r)
    {
        EXPECT_CALL(mock_open_local_image_request, construct(IsSameIoCtx(&io_ctx), image_id));
        EXPECT_CALL(mock_open_local_image_request, send())
            .WillOnce(Invoke([this, &mock_open_local_image_request, mock_image_ctx, r]() {
                ...
                m_threads->work_queue->queue(mock_open_local_image_request.on_finish, r);
            }));
    }

    void expect_close_remote_image(MockStateBuilder& mock_state_builder, int r)
    {
        EXPECT_CALL(mock_state_builder, close_remote_image(_))
            .WillOnce(Invoke([&mock_state_builder, r](Context* on_finish) {
                mock_state_builder.remote_image_ctx = nullptr;
                on_finish->complete(r);
            }));
    }

    void expect_create_local_image(MockStateBuilder& mock_state_builder, const std::string& local_image_id, int r)
    {
        EXPECT_CALL(mock_state_builder, create_local_image_request(_, _, _, _, _, _))
            .WillOnce(WithArg<5>(Invoke([&mock_state_builder, local_image_id, r](Context* ctx) {
                if (r >= 0) {
                    mock_state_builder.local_image_id = local_image_id;
                }
        ...
        EXPECT_CALL(mock_state_builder.mock_base_request, send()).WillOnce(Invoke([this, &mock_state_builder, r]() {
            m_threads->work_queue->queue(mock_state_builder.mock_base_request.on_finish, r);
        }));
    }

    void expect_prepare_replay(MockStateBuilder& mock_state_builder, bool resync_requested, bool syncing, int r)
    {
        EXPECT_CALL(mock_state_builder, create_prepare_replay_request(_, _, _, _, _))
            .WillOnce(WithArgs<2, 3, 4>(
                Invoke([&mock_state_builder, resync_requested, syncing, r](bool* resync, bool* sync, Context* ctx) {
                    if (r >= 0) {
    ...
    void expect_image_sync(MockImageSync& mock_image_sync, int r)
    {
        ...
        EXPECT_CALL(mock_image_sync, send()).WillOnce(Invoke([this, &mock_image_sync, r]() {
            m_threads->work_queue->queue(mock_image_sync.on_finish, r);
        }));
        EXPECT_CALL(mock_image_sync, put());
    }

    MockBootstrapRequest* create_request(MockThreads* mock_threads, MockInstanceWatcher* mock_instance_watcher,
                                         const std::string& global_image_id, const std::string& local_mirror_uuid,
                                         Context* on_finish)
    {
...
TEST_F(TestMockImageReplayerBootstrapRequest, Success)
{
    ...
    expect_send(
        mock_prepare_local_image_request, mock_state_builder, m_local_image_ctx->id, m_local_image_ctx->name, 0);

    // prepare remote image
    MockPrepareRemoteImageRequest mock_prepare_remote_image_request;
    expect_send(mock_prepare_remote_image_request, mock_state_builder, "remote mirror uuid", m_remote_image_ctx->id, 0);
    ...
    expect_open_image(mock_open_image_request, m_remote_io_ctx, mock_remote_image_ctx.id, mock_remote_image_ctx, 0);

    // open the local image
    librbd::MockTestImageCtx mock_local_image_ctx(*m_local_image_ctx);
    MockOpenLocalImageRequest mock_open_local_image_request;
    expect_open_local_image(
        mock_open_local_image_request, m_local_io_ctx, mock_local_image_ctx.id, &mock_local_image_ctx, 0);
    ...
    MockBootstrapRequest* request =
        create_request(&mock_threads, &mock_instance_watcher, "global image id", "local mirror uuid", &ctx);
    request->send();
...
TEST_F(TestMockImageReplayerBootstrapRequest, PrepareRemoteImageNotPrimaryLocalDNE)
{
    ...
    MockPrepareLocalImageRequest mock_prepare_local_image_request;
    expect_send(
        mock_prepare_local_image_request, mock_state_builder, m_local_image_ctx->id, m_local_image_ctx->name, -ENOENT);
...
TEST_F(TestMockImageReplayerBootstrapRequest, PrepareRemoteImageDNELocalLinked)
{
    ...
    MockPrepareRemoteImageRequest mock_prepare_remote_image_request;
    expect_send(
        mock_prepare_remote_image_request, mock_state_builder, "remote mirror uuid", m_remote_image_ctx->id, -ENOENT);
    expect_is_local_primary(mock_state_builder, false);
    expect_is_linked(mock_state_builder, true);
...
TEST_F(TestMockImageReplayerBootstrapRequest, OpenLocalImageError)
{
    ...
    MockOpenLocalImageRequest mock_open_local_image_request;
    expect_open_local_image(
        mock_open_local_image_request, m_local_io_ctx, mock_local_image_ctx.id, &mock_local_image_ctx, -EINVAL);
...
TEST_F(TestMockImageReplayerBootstrapRequest, OpenLocalImageDNE)
{
    ...
    MockOpenLocalImageRequest mock_open_local_image_request;
    expect_open_local_image(
        mock_open_local_image_request, m_local_io_ctx, mock_local_image_ctx.id, &mock_local_image_ctx, -ENOENT);

    // create local image
    expect_create_local_image(mock_state_builder, "local image id", 0);

    // re-open the local image
    expect_open_local_image(mock_open_local_image_request, m_local_io_ctx, "local image id", &mock_local_image_ctx, 0);
...
TEST_F(TestMockImageReplayerBootstrapRequest, OpenLocalImagePrimary)
{
    ...
    MockOpenLocalImageRequest mock_open_local_image_request;
    expect_open_local_image(
        mock_open_local_image_request, m_local_io_ctx, mock_local_image_ctx.id, &mock_local_image_ctx, -EREMOTEIO);
...
TEST_F(TestMockImageReplayerBootstrapRequest, CreateLocalImageError)
{
    ...
    MockPrepareLocalImageRequest mock_prepare_local_image_request;
    MockStateBuilder mock_state_builder;
    expect_send(mock_prepare_local_image_request, mock_state_builder, "", "", -ENOENT);
