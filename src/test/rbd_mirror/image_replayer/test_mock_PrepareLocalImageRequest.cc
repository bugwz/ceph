...
public:
    ...
    void expect_dir_get_name(librados::IoCtx& io_ctx, const std::string& image_name, int r)
    {
        bufferlist bl;
        encode(image_name, bl);

        EXPECT_CALL(get_mock_io_ctx(io_ctx), exec(RBD_DIRECTORY, _, StrEq("rbd"), StrEq("dir_get_name"), _, _, _, _))
...
TEST_F(TestMockImageReplayerPrepareLocalImageRequest, SuccessJournal)
{
    ...
    expect_get_mirror_image_id(mock_get_mirror_image_id_request, "local image id", 0);
    ...
    C_SaferCond ctx;
    auto req = MockPrepareLocalImageRequest::create(
        m_local_io_ctx, "global image id", &local_image_name, &mock_state_builder, m_threads->work_queue, &ctx);
    req->send();

    ASSERT_EQ(0, ctx.wait());
    ASSERT_TRUE(mock_state_builder != nullptr);
    ASSERT_EQ(std::string("local image name"), local_image_name);
    ASSERT_EQ(std::string("local image id"), mock_journal_state_builder.local_image_id);
    ASSERT_EQ(cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, mock_journal_state_builder.mirror_image_mode);
    ASSERT_EQ(librbd::mirror::PROMOTION_STATE_NON_PRIMARY, mock_journal_state_builder.local_promotion_state);
    ASSERT_EQ(std::string("remote mirror uuid"), mock_journal_state_builder.local_primary_mirror_uuid);
}
...
TEST_F(TestMockImageReplayerPrepareLocalImageRequest, SuccessSnapshot)
{
    ...
    ASSERT_EQ(std::string("local image name"), local_image_name);
    ASSERT_EQ(std::string("local image id"), mock_journal_state_builder.local_image_id);
    ASSERT_EQ(cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, mock_journal_state_builder.mirror_image_mode);
    ASSERT_EQ(librbd::mirror::PROMOTION_STATE_NON_PRIMARY, mock_journal_state_builder.local_promotion_state);
}
...
TEST_F(TestMockImageReplayerPrepareLocalImageRequest, ImageCreating)
{
    ...
    expect_get_mirror_info(
        mock_get_mirror_info_request,
        {cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, "global image id", cls::rbd::MIRROR_IMAGE_STATE_CREATING},
        librbd::mirror::PROMOTION_STATE_NON_PRIMARY,
        "remote mirror uuid",
        0);
