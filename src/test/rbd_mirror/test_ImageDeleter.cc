...
public:
    ...
    void SetUp() override
    {
        ...
        m_image_deletion_throttler.reset(
            new rbd::mirror::Throttler<>(g_ceph_context, "rbd_mirror_concurrent_image_deletions"));

        m_service_daemon.reset(new rbd::mirror::ServiceDaemon<>(g_ceph_context, _rados, m_threads));

        librbd::api::Mirror<>::mode_set(m_local_io_ctx, RBD_MIRROR_MODE_IMAGE);

        m_deleter = new rbd::mirror::ImageDeleter<>(
            m_local_io_ctx, m_threads, m_image_deletion_throttler.get(), m_service_daemon.get());
        ...
        cls::rbd::MirrorImage mirror_image(
            MirrorImageMode::MIRROR_IMAGE_MODE_JOURNAL, GLOBAL_IMAGE_ID, MirrorImageState::MIRROR_IMAGE_STATE_ENABLED);
        EXPECT_EQ(0, cls_client::mirror_image_set(&m_local_io_ctx, m_local_image_id, mirror_image));
    }
    ...
    {
        cls::rbd::MirrorImage mirror_image;
        int r = cls_client::mirror_image_get(&m_local_io_ctx, m_local_image_id, &mirror_image);
    ...
    void promote_image(ImageCtx* ictx = nullptr)
    {
        ...
        int r = 0;
        if (!ictx) {
            ictx = new ImageCtx("", m_local_image_id, "", m_local_io_ctx, false);
            r = ictx->state->open(0);
            close = (r == 0);
        }
    ...
    {
        ImageCtx* ictx = new ImageCtx("", m_local_image_id, "", m_local_io_ctx, false);
        EXPECT_EQ(0, ictx->state->open(0));
        {
        ...
        EXPECT_EQ(0, ictx->operations->snap_create(cls::rbd::UserSnapshotNamespace(), snap_name, 0, prog_ctx));

        if (protect) {
            EXPECT_EQ(0, ictx->operations->snap_protect(cls::rbd::UserSnapshotNamespace(), snap_name));
        }
    ...
    std::string create_clone()
    {
        ...
        librbd::NoOpProgressContext prog_ctx;
        EXPECT_EQ(0, ictx->operations->snap_create(cls::rbd::UserSnapshotNamespace(), "snap1", 0, prog_ctx));
        EXPECT_EQ(0, ictx->operations->snap_protect(cls::rbd::UserSnapshotNamespace(), "snap1"));
        EXPECT_EQ(0, librbd::api::Image<>::snap_set(ictx, cls::rbd::UserSnapshotNamespace(), "snap1"));
        ...
        EXPECT_EQ(0,
                  librbd::clone(m_local_io_ctx,
                                m_local_image_id.c_str(),
                                nullptr,
                                "snap1",
                                m_local_io_ctx,
                                clone_id.c_str(),
        ...
        EXPECT_EQ(0, cls_client::mirror_image_set(&m_local_io_ctx, clone_id, mirror_image));
        EXPECT_EQ(0, ictx->state->close());
        return clone_id;
    }
    ...
    void check_image_deleted()
    {
        ...
        cls::rbd::MirrorImage mirror_image;
        EXPECT_EQ(-ENOENT, cls_client::mirror_image_get(&m_local_io_ctx, m_local_image_id, &mirror_image));
    }

    int trash_move(const std::string& global_image_id)
    {
        C_SaferCond ctx;
        rbd::mirror::ImageDeleter<>::trash_move(m_local_io_ctx, global_image_id, true, m_threads->work_queue, &ctx);
        return ctx.wait();
    }
