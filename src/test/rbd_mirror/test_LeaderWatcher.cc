...
public:
    ...
    {
        TestFixture::SetUp();
        EXPECT_EQ(0, librbd::api::Mirror<>::mode_set(m_local_io_ctx, RBD_MIRROR_MODE_POOL));
    ...
    librados::IoCtx& create_connection(bool no_heartbeats = false)
    {
        ...
        if (no_heartbeats) {
            EXPECT_EQ(0, c->cluster.conf_set("rbd_mirror_leader_heartbeat_interval", "3600"));
        }
        else if (is_librados_test_stub(*_rados)) {
            EXPECT_EQ(0, c->cluster.conf_set("rbd_mirror_leader_heartbeat_interval", "1"));
        }
...
TEST_F(TestLeaderWatcher, Break)
{
    ...
    LeaderWatcher<> leader_watcher1(m_threads, create_connection(true /* no heartbeats */), &listener1);
    LeaderWatcher<> leader_watcher2(m_threads, create_connection(), &listener2);
...
TEST_F(TestLeaderWatcher, Stress)
{
    ...
    for (int i = 0; i < WATCHERS_COUNT; i++) {
        auto leader_watcher = new LeaderWatcher<>(m_threads, create_connection(), &listener);
