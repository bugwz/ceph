...
template<typename T> class TestImageReplayer : public TestFixture
{
public:
    static const cls::rbd::MirrorImageMode MIRROR_IMAGE_MODE = T::MIRROR_IMAGE_MODE;
    ...
    struct C_WatchCtx : public librados::WatchCtx2
    {
        ...
        void handle_error(uint64_t cookie, int err) override { ASSERT_EQ(0, err); }
    };

    TestImageReplayer()
        : m_local_cluster(new librados::Rados())
        , m_watch_handle(0)
    {
        ...
        EXPECT_EQ(0, m_local_cluster->conf_set("rbd_mirror_journal_commit_age", "0.1"));
        m_local_pool_name = get_temp_pool_name();
        EXPECT_EQ(0, m_local_cluster->pool_create(m_local_pool_name.c_str()));
        EXPECT_EQ(0, m_local_cluster->ioctx_create(m_local_pool_name.c_str(), m_local_ioctx));
        ...
        EXPECT_EQ(0, m_remote_cluster.ioctx_create(m_remote_pool_name.c_str(), m_remote_ioctx));
        ...
        if (MIRROR_IMAGE_MODE == cls::rbd::MIRROR_IMAGE_MODE_JOURNAL) {
            EXPECT_EQ(0, librbd::api::Mirror<>::mode_set(m_remote_ioctx, RBD_MIRROR_MODE_POOL));
            EXPECT_EQ(0, librbd::api::Mirror<>::mode_set(m_local_ioctx, RBD_MIRROR_MODE_POOL));
        }
        else {
            EXPECT_EQ(0, librbd::api::Mirror<>::mode_set(m_remote_ioctx, RBD_MIRROR_MODE_IMAGE));
            EXPECT_EQ(0, librbd::api::Mirror<>::mode_set(m_local_ioctx, RBD_MIRROR_MODE_IMAGE));
            ...
            EXPECT_EQ(
                0,
                librbd::cls_client::mirror_peer_add(
                    &m_remote_ioctx,
                    {remote_peer_uuid, cls::rbd::MIRROR_PEER_DIRECTION_RX_TX, "siteA", "client", m_local_mirror_uuid}));

            m_pool_meta_cache.set_remote_pool_meta(m_remote_ioctx.get_id(), {m_remote_mirror_uuid, remote_peer_uuid});
        }

        EXPECT_EQ(0, librbd::api::Mirror<>::uuid_get(m_remote_ioctx, &m_remote_mirror_uuid));
        EXPECT_EQ(0, librbd::api::Mirror<>::uuid_get(m_local_ioctx, &m_local_mirror_uuid));

        m_image_name = get_temp_image_name();
        int order = 0;
        EXPECT_EQ(0, librbd::create(m_remote_ioctx, m_image_name.c_str(), 1 << 22, false, features, &order, 0, 0));
        ...
        if (MIRROR_IMAGE_MODE != cls::rbd::MIRROR_IMAGE_MODE_JOURNAL) {
            ...
            EXPECT_EQ(0,
                      librbd::api::Mirror<>::image_enable(
                          remote_image_ctx, static_cast<rbd_mirror_image_mode_t>(MIRROR_IMAGE_MODE), false));
        ...
        m_image_sync_throttler.reset(new Throttler<>(cct, "rbd_mirror_concurrent_image_syncs"));

        m_instance_watcher =
            InstanceWatcher<>::create(m_local_ioctx, *m_threads->asio_engine, nullptr, m_image_sync_throttler.get());
        m_instance_watcher->handle_acquire_leader();

        EXPECT_EQ(0, m_local_ioctx.create(RBD_MIRRORING, false));

        m_local_status_updater = MirrorStatusUpdater<>::create(m_local_ioctx, m_threads.get(), "");
    ...
    {
        m_replayer = new ImageReplayer<>(m_local_ioctx,
                                         m_local_mirror_uuid,
                                         m_global_image_id,
                                         m_threads.get(),
                                         m_instance_watcher,
                                         ...
                                         nullptr,
                                         &m_pool_meta_cache);
        m_replayer->add_peer({"peer uuid", m_remote_ioctx, {m_remote_mirror_uuid, "remote mirror peer uuid"}, nullptr});
    }
    ...
    std::string get_temp_image_name() { return "image" + stringify(++_image_number); }
    ...
    std::string get_global_image_id(librados::IoCtx& io_ctx, const std::string& image_id)
    {
        cls::rbd::MirrorImage mirror_image;
        EXPECT_EQ(0, librbd::cls_client::mirror_image_get(&io_ctx, image_id, &mirror_image));
        return mirror_image.global_image_id;
    }

    void open_image(librados::IoCtx& ioctx, const std::string& image_name, bool readonly, librbd::ImageCtx** ictxp)
    {
        librbd::ImageCtx* ictx = new librbd::ImageCtx(image_name.c_str(), "", "", ioctx, readonly);
        EXPECT_EQ(0, ictx->state->open(0));
        *ictxp = ictx;
    }

    void open_local_image(librbd::ImageCtx** ictxp) { open_image(m_local_ioctx, m_image_name, true, ictxp); }

    void open_remote_image(librbd::ImageCtx** ictxp) { open_image(m_remote_ioctx, m_image_name, false, ictxp); }

    void close_image(librbd::ImageCtx* ictx) { ictx->state->close(); }
                              ...
                              cls::journal::ObjectPosition* mirror_position)
    {
        ...
        cls::journal::client::get_mutable_metadata(
            m_remote_ioctx, oid, &minimum_set, &active_set, &registered_clients, &cond);
        ASSERT_EQ(0, cond.wait());
        ...
        *mirror_position = cls::journal::ObjectPosition();
        ...
        for (c = registered_clients.begin(); c != registered_clients.end(); ++c) {
            ...
            cls::journal::ObjectPositions object_positions = c->commit_position.object_positions;
            cls::journal::ObjectPositions::const_iterator p = object_positions.begin();
    ...
    bool wait_for_watcher_notify(int seconds)
    {
        ...
        while (!m_watch_ctx->notified) {
            if (m_watch_ctx->cond.wait_for(locker, std::chrono::seconds(seconds)) == std::cv_status::timeout) {
                return false;
            }
        }
    ...
    int get_last_mirror_snapshot(librados::IoCtx& io_ctx, const std::string& image_id, uint64_t* mirror_snap_id,
                                 cls::rbd::MirrorSnapshotNamespace* mirror_ns)
    {
        ...
        for (auto snap_id : snapc.snaps) {
            ...
            r = librbd::cls_client::snapshot_get(&io_ctx, header_oid, snap_id, &snap_info);
            if (r < 0) {
                return r;
            }

            auto ns = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap_info.snapshot_namespace);
            if (ns != nullptr) {
    ...
    void wait_for_snapshot_synced()
    {
        ...
        ASSERT_EQ(0, get_last_mirror_snapshot(m_remote_ioctx, m_remote_image_id, &remote_snap_id, &remote_mirror_ns));

        std::cout << "remote_snap_id=" << remote_snap_id << std::endl;

        std::string local_image_id;
        ASSERT_EQ(0, librbd::cls_client::mirror_image_get_image_id(&m_local_ioctx, m_global_image_id, &local_image_id));

        uint64_t local_snap_id = CEPH_NOSNAP;
        cls::rbd::MirrorSnapshotNamespace local_mirror_ns;
        for (int i = 0; i < 100; i++) {
            int r = get_last_mirror_snapshot(m_local_ioctx, local_image_id, &local_snap_id, &local_mirror_ns);
            if (r == 0 &&
                ((remote_mirror_ns.state == cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY &&
                  local_mirror_ns.state == cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY) ||
                 (remote_mirror_ns.state == cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED &&
                  local_mirror_ns.state == cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY_DEMOTED)) &&
                local_mirror_ns.primary_mirror_uuid == m_remote_mirror_uuid &&
                local_mirror_ns.primary_snap_id == remote_snap_id && local_mirror_ns.complete) {
    ...
    void write_test_data(librbd::ImageCtx* ictx, const char* test_data, off_t off, size_t len)
    {
        size_t written;
        bufferlist bl;
    ...
    void read_test_data(librbd::ImageCtx* ictx, const char* expected, off_t off, size_t len)
    {
        ssize_t read;
        char* result = (char*)malloc(len + 1);

        ASSERT_NE(static_cast<char*>(NULL), result);
        read = librbd::api::Io<>::read(*ictx, off, len, librbd::io::ReadResult{result, len}, 0);
        printf("read: %d\n", (int)read);
    ...
    void flush(librbd::ImageCtx* ictx)
    {
        ...
        if (MIRROR_IMAGE_MODE == cls::rbd::MIRROR_IMAGE_MODE_JOURNAL) {
            ...
            ASSERT_EQ(0, librbd::api::Mirror<>::image_snapshot_create(ictx, 0, &snap_id));
        }
...
template<typename T> int TestImageReplayer<T>::_image_number;

template<cls::rbd::MirrorImageMode _mirror_image_mode, uint64_t _features> class TestImageReplayerParams
{
public:
...
typedef ::testing::Types<TestImageReplayerParams<cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, 125>,
                         TestImageReplayerParams<cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, 1>,
                         TestImageReplayerParams<cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, 5>,
                         TestImageReplayerParams<cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, 61>,
                         TestImageReplayerParams<cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, 125>>
...
typedef TestImageReplayer<TestImageReplayerParams<cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, 125>> TestImageReplayerJournal;

TYPED_TEST(TestImageReplayer, BootstrapErrorLocalImageExists)
{
    int order = 0;
    EXPECT_EQ(0, librbd::create(this->m_local_ioctx, this->m_image_name.c_str(), 1 << 22, false, 0, &order, 0, 0));
...
{
    ASSERT_EQ(0, librbd::Journal<>::remove(this->m_remote_ioctx, this->m_remote_image_id));
...
TYPED_TEST(TestImageReplayer, BootstrapErrorMirrorDisabled)
{
    ...
    ASSERT_EQ(0, librbd::api::Mirror<>::mode_set(this->m_remote_ioctx, RBD_MIRROR_MODE_IMAGE));
...
TYPED_TEST(TestImageReplayer, BootstrapMirrorDisabling)
{
    ...
    if (gtest_TypeParam_::MIRROR_IMAGE_MODE == cls::rbd::MIRROR_IMAGE_MODE_JOURNAL) {
        ...
        librbd::ImageCtx* ictx;
        this->open_remote_image(&ictx);
        ASSERT_EQ(0, librbd::api::Mirror<>::image_enable(ictx, RBD_MIRROR_IMAGE_MODE_JOURNAL, false));
        this->close_image(ictx);
    }

    cls::rbd::MirrorImage mirror_image;
    ASSERT_EQ(0, librbd::cls_client::mirror_image_get(&this->m_remote_ioctx, this->m_remote_image_id, &mirror_image));
    mirror_image.state = cls::rbd::MirrorImageState::MIRROR_IMAGE_STATE_DISABLING;
    ASSERT_EQ(0, librbd::cls_client::mirror_image_set(&this->m_remote_ioctx, this->m_remote_image_id, mirror_image));
...
TEST_F(TestImageReplayerJournal, JournalReset)
{
    ...
    ASSERT_EQ(0, librbd::Journal<>::reset(this->m_remote_ioctx, this->m_remote_image_id));
...
TEST_F(TestImageReplayerJournal, MultipleReplayFailures_SingleEpoch)
{
    ...
    ASSERT_EQ(0, ictx->operations->snap_create(cls::rbd::UserSnapshotNamespace(), "foo", 0, prog_ctx));
    ASSERT_EQ(0, ictx->operations->snap_protect(cls::rbd::UserSnapshotNamespace(), "foo"));
    ASSERT_EQ(0,
              librbd::cls_client::add_child(
                  &ictx->md_ctx,
                  RBD_CHILDREN,
                  {ictx->md_ctx.get_id(), "", ictx->id, ictx->snap_ids[{cls::rbd::UserSnapshotNamespace(), "foo"}]},
                  "dummy child id"));
    this->close_image(ictx);
    ...
    for (uint64_t i = 0; i < 10; ++i) {
        ...
        ictx->journal->append_op_event(i,
                                       librbd::journal::EventEntry{librbd::journal::SnapUnprotectEvent{
                                           i, cls::rbd::UserSnapshotNamespace(), "foo"}},
                                       &append_ctx);
...
TEST_F(TestImageReplayerJournal, MultipleReplayFailures_MultiEpoch)
{
    ...
    this->open_remote_image(&ictx);
    {
        ...
        C_SaferCond append_ctx;
        ictx->journal->append_op_event(1U,
                                       librbd::journal::EventEntry{librbd::journal::SnapUnprotectEvent{
                                           1U, cls::rbd::UserSnapshotNamespace(), "foo"}},
                                       &append_ctx);
        ASSERT_EQ(0, append_ctx.wait());
...
TEST_F(TestImageReplayerJournal, Disconnect)
{
    ...
    ASSERT_EQ(0,
              cls::journal::client::client_update_state(
                  this->m_remote_ioctx, oid, this->m_local_mirror_uuid, cls::journal::CLIENT_STATE_DISCONNECTED));
...
TEST_F(TestImageReplayerJournal, UpdateFeatures)
{
    ...
    const uint64_t FEATURES_TO_UPDATE = RBD_FEATURE_OBJECT_MAP | RBD_FEATURE_FAST_DIFF;
    ...
    if (features) {
        ASSERT_EQ(0, ictx->operations->update_features(FEATURES_TO_UPDATE, false));
    }
    ASSERT_EQ(0, librbd::get_features(ictx, &features));
    ...
    this->open_remote_image(&ictx);
    ASSERT_EQ(0, ictx->operations->update_features(FEATURES_TO_UPDATE, true));
    ASSERT_EQ(0, librbd::get_features(ictx, &features));
    ...
    ASSERT_EQ(-EINVAL, ictx->operations->update_features(RBD_FEATURE_EXCLUSIVE_LOCK, false));
    this->generate_test_data();
    for (int i = 0; i < TEST_IO_COUNT; ++i) {
        this->write_test_data(ictx, this->m_test_data, TEST_IO_SIZE * i, TEST_IO_SIZE);
    }
...
TEST_F(TestImageReplayerJournal, MirroringDelay)
{
    ...
    ASSERT_EQ(0, this->m_local_cluster->conf_set("rbd_mirroring_replay_delay", stringify(DELAY).c_str()));
    this->open_local_image(&ictx);
...
{
    const uint64_t FEATURES_TO_UPDATE = RBD_FEATURE_OBJECT_MAP | RBD_FEATURE_FAST_DIFF | RBD_FEATURE_DEEP_FLATTEN;
    ...
    ASSERT_EQ(0,
              remote_image_ctx->operations->update_features((RBD_FEATURE_OBJECT_MAP | RBD_FEATURE_FAST_DIFF), false));
    this->flush(remote_image_ctx);
    ...
    ASSERT_EQ(0U, local_image_ctx->features & (RBD_FEATURE_OBJECT_MAP | RBD_FEATURE_FAST_DIFF));

    // enable object-map/fast-diff
    ASSERT_EQ(0, remote_image_ctx->operations->update_features((RBD_FEATURE_OBJECT_MAP | RBD_FEATURE_FAST_DIFF), true));
    this->flush(remote_image_ctx);
    this->wait_for_replay_complete();

    ASSERT_EQ(0, local_image_ctx->state->refresh());
    ASSERT_EQ(RBD_FEATURE_OBJECT_MAP | RBD_FEATURE_FAST_DIFF,
              local_image_ctx->features & (RBD_FEATURE_OBJECT_MAP | RBD_FEATURE_FAST_DIFF));

    // disable deep-flatten
    ASSERT_EQ(0, remote_image_ctx->operations->update_features(RBD_FEATURE_DEEP_FLATTEN, false));
    this->flush(remote_image_ctx);
    this->wait_for_replay_complete();
...
TYPED_TEST(TestImageReplayer, SnapshotUnprotect)
{
    ...
    librbd::NoOpProgressContext prog_ctx;
    ASSERT_EQ(0, remote_image_ctx->operations->snap_create(cls::rbd::UserSnapshotNamespace{}, "snap1", 0, prog_ctx));
    ASSERT_EQ(0, remote_image_ctx->operations->snap_protect(cls::rbd::UserSnapshotNamespace{}, "snap1"));
    this->flush(remote_image_ctx);
    ...
    librbd::ImageCtx* local_image_ctx = nullptr;
    this->open_local_image(&local_image_ctx);
    auto local_snap_id_it = local_image_ctx->snap_ids.find({{cls::rbd::UserSnapshotNamespace{}}, "snap1"});
    ...
    ASSERT_EQ(RBD_PROTECTION_STATUS_PROTECTED, local_snap_info_it->second.protection_status);

    // unprotect the snapshot
    ASSERT_EQ(0, remote_image_ctx->operations->snap_unprotect(cls::rbd::UserSnapshotNamespace{}, "snap1"));
    this->flush(remote_image_ctx);
    this->wait_for_replay_complete();
    ...
    ASSERT_NE(local_image_ctx->snap_info.end(), local_snap_info_it);
    ASSERT_EQ(RBD_PROTECTION_STATUS_UNPROTECTED, local_snap_info_it->second.protection_status);
...
TYPED_TEST(TestImageReplayer, SnapshotRemove)
{
    ...
    // remove the snapshot
    ASSERT_EQ(0, remote_image_ctx->operations->snap_remove(cls::rbd::UserSnapshotNamespace{}, "snap1"));
    this->flush(remote_image_ctx);
    this->wait_for_replay_complete();

    ASSERT_EQ(0, local_image_ctx->state->refresh());
    local_snap_id_it = local_image_ctx->snap_ids.find({{cls::rbd::UserSnapshotNamespace{}}, "snap1"});
...
TYPED_TEST(TestImageReplayer, SnapshotRename)
{
    ...
    // rename the snapshot
    ASSERT_EQ(0, remote_image_ctx->operations->snap_rename("snap1", "snap1-renamed"));
    this->flush(remote_image_ctx);
    this->wait_for_replay_complete();
...
TYPED_TEST(TestImageReplayer, SnapshotLimit)
{
    ...
    ASSERT_EQ(0, librbd::api::Snapshot<>::get_limit(local_image_ctx, &local_snap_limit));
    ASSERT_EQ(123U, local_snap_limit);

    // update the limit again
    ASSERT_EQ(0, librbd::api::Snapshot<>::set_limit(remote_image_ctx, std::numeric_limits<uint64_t>::max()));
    this->flush(remote_image_ctx);
    this->wait_for_replay_complete();
