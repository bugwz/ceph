...
template<> struct LoadRequest<librbd::MockTestImageCtx>
{
    std::map<std::string, cls::rbd::MirrorImageMap>* image_map;
    Context* on_finish = nullptr;

    static LoadRequest* s_instance;
    static LoadRequest* create(librados::IoCtx& ioctx, std::map<std::string, cls::rbd::MirrorImageMap>* image_map,
                               Context* on_finish)
    {
    ...
    LoadRequest() { s_instance = this; }
};

template<> struct UpdateRequest<librbd::MockTestImageCtx>
{
    Context* on_finish = nullptr;
    ...
    UpdateRequest() { s_instance = this; }
};

LoadRequest<librbd::MockTestImageCtx>* LoadRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
UpdateRequest<librbd::MockTestImageCtx>* UpdateRequest<librbd::MockTestImageCtx>::s_instance = nullptr;
...
public:
    ...
    struct MockListener : Listener
    {
        ...
        MOCK_METHOD3(mock_remove_image, void(const std::string&, const std::string&, Context*));

        void acquire_image(const std::string& global_image_id, const std::string& instance_id, Context* on_finish)
        {
            mock_acquire_image(global_image_id, on_finish);
        }

        void release_image(const std::string& global_image_id, const std::string& instance_id, Context* on_finish)
        {
            mock_release_image(global_image_id, on_finish);
        }

        void remove_image(const std::string& mirror_uuid, const std::string& global_image_id,
    ...
    void SetUp() override
    {
        ...
        EXPECT_EQ(0, _rados->conf_set("rbd_mirror_image_policy_migration_throttle", "0"));
    ...
    {
        EXPECT_CALL(request, send()).WillOnce(Invoke([&request, r]() { request.on_finish->complete(r); }));
    }

    void expect_update_request(MockUpdateRequest& request, int r)
    {
        EXPECT_CALL(request, send()).WillOnce(Invoke([this, &request, r]() {
    ...
    void expect_listener_acquire_image(MockListener& mock_listener, const std::string& global_image_id,
    ...
    void expect_listener_release_image(MockListener& mock_listener, const std::string& global_image_id,
                                       std::map<std::string, Context*>* peer_ack_ctxs)
    {
    ...
    void expect_listener_remove_image(MockListener& mock_listener, const std::string& mirror_uuid,
                                      const std::string& global_image_id,
                                      std::map<std::string, Context*>* peer_ack_ctxs)
    {
        EXPECT_CALL(mock_listener, mock_remove_image(mirror_uuid, global_image_id, _))
    ...
    void remote_peer_ack_nowait(MockImageMap* image_map, const std::set<std::string>& global_image_ids, int ret,
                                std::map<std::string, Context*>* peer_ack_ctxs)
    {
    ...
    void remote_peer_ack_wait(MockImageMap* image_map, const std::set<std::string>& global_image_ids, int ret,
                              std::map<std::string, Context*>* peer_ack_ctxs)
    {
        for (auto& global_image_id : global_image_ids) {
            auto it = peer_ack_ctxs->find(global_image_id);
    ...
    void remote_peer_ack_listener_wait(MockImageMap* image_map, const std::set<std::string>& global_image_ids, int ret,
                                       std::map<std::string, Context*>* peer_ack_ctxs)
    {
        for (auto& global_image_id : global_image_ids) {
            auto it = peer_ack_ctxs->find(global_image_id);
    ...
    void update_map_and_acquire(MockThreads& mock_threads, MockUpdateRequest& mock_update_request,
                                MockListener& mock_listener, const std::set<std::string>& global_image_ids, int ret,
                                std::map<std::string, Context*>* peer_ack_ctxs)
    {
        for (auto const& global_image_id : global_image_ids) {
            ...
            expect_add_event(mock_threads);
            expect_listener_acquire_image(mock_listener, global_image_id, peer_ack_ctxs);
        }
    }

    void update_map_request(MockThreads& mock_threads, MockUpdateRequest& mock_update_request,
    ...
    void wait_for_scheduled_task() { m_threads->work_queue->drain(); }
    ...
    void listener_acquire_images(MockListener& mock_listener, const std::set<std::string>& global_image_ids,
                                 std::map<std::string, Context*>* peer_ack_ctxs)
    {
        for (auto const& global_image_id : global_image_ids) {
            expect_listener_acquire_image(mock_listener, global_image_id, peer_ack_ctxs);
    ...
    {
        for (auto const& global_image_id : global_image_ids) {
            expect_listener_release_image(mock_listener, global_image_id, peer_ack_ctxs);
        }
    }

    void listener_remove_images(MockListener& mock_listener, const std::string& mirror_uuid,
                                std::set<std::string>& global_image_ids, std::map<std::string, Context*>* peer_ack_ctxs)
    {
        for (auto const& global_image_id : global_image_ids) {
            expect_listener_remove_image(mock_listener, mirror_uuid, global_image_id, peer_ack_ctxs);
        }
    }
...
TEST_F(TestMockImageMap, SetLocalImages)
{
    ...
    std::unique_ptr<MockImageMap> mock_image_map{
        MockImageMap::create(m_local_io_ctx, &mock_threads, m_local_instance_id, mock_listener)};

    C_SaferCond cond;
    mock_image_map->init(&cond);
    ASSERT_EQ(0, cond.wait());

    std::set<std::string> global_image_ids{"global id 1", "global id 2"};
...
TEST_F(TestMockImageMap, AddRemoveLocalImage)
{
    ...
    std::set<std::string> initial_global_image_ids{"global id 1", "global id 2"};
    std::set<std::string> initial_global_image_ids_ack(initial_global_image_ids);

    std::set<std::string> remove_global_image_ids{"global id 1", "global id 2"};
    ...
    expect_add_event(mock_threads);
    std::map<std::string, Context*> peer_ack_ctxs;
    listener_acquire_images(mock_listener, initial_global_image_ids, &peer_ack_ctxs);
    ...
    // remote peer ACKs image acquire request
    remote_peer_ack_nowait(mock_image_map.get(), initial_global_image_ids_ack, 0, &peer_ack_ctxs);

    // RELEASE+REMOVE_MAPPING
    expect_add_event(mock_threads);
    listener_release_images(mock_listener, remove_global_image_ids, &peer_ack_ctxs);
    ...
    remote_peer_ack_wait(mock_image_map.get(), remove_global_image_ids_ack, 0, &peer_ack_ctxs);
...
TEST_F(TestMockImageMap, AddRemoveRemoteImage)
{
    ...
    listener_remove_images(mock_listener, "uuid1", remove_global_image_ids, &peer_remove_ack_ctxs);
    expect_add_event(mock_threads);
    listener_release_images(mock_listener, remove_global_image_ids, &peer_ack_ctxs);
    update_map_request(mock_threads, mock_update_request, remove_global_image_ids, 0);
    ...
    remote_peer_ack_nowait(mock_image_map.get(), remove_global_image_ids_ack, 0, &peer_remove_ack_ctxs);
    remote_peer_ack_wait(mock_image_map.get(), remove_global_image_ids_ack, 0, &peer_ack_ctxs);
...
TEST_F(TestMockImageMap, RemoveRemoteAndLocalImage)
{
    ...
    // remote image set
    std::set<std::string> initial_remote_global_image_ids{"global id 1"};
    std::set<std::string> initial_remote_global_image_ids_ack(initial_remote_global_image_ids);

    // local image set
    std::set<std::string> initial_local_global_image_ids{"global id 1"};

    // remote/local images to remove
    std::set<std::string> remote_remove_global_image_ids{"global id 1"};
    std::set<std::string> remote_remove_global_image_ids_ack(remote_remove_global_image_ids);

    std::set<std::string> local_remove_global_image_ids{"global id 1"};
    ...
    expect_add_event(mock_threads);
    std::map<std::string, Context*> peer_ack_ctxs;
    listener_acquire_images(mock_listener, initial_remote_global_image_ids, &peer_ack_ctxs);
    ...
    listener_remove_images(mock_listener, "uuid1", remote_remove_global_image_ids, &peer_ack_remove_ctxs);
    ...
    // RELEASE+REMOVE_MAPPING
    expect_add_event(mock_threads);
    listener_release_images(mock_listener, local_remove_global_image_ids, &peer_ack_ctxs);
...
TEST_F(TestMockImageMap, AddInstance)
{
    ...
    std::set<std::string> global_image_ids{"global id 1", "global id 2", "global id 3", "global id 4", "global id 5"};
    std::set<std::string> global_image_ids_ack(global_image_ids);
    ...
    expect_add_event(mock_threads);
    expect_listener_images_unmapped(mock_listener, 3, &shuffled_global_image_ids, &peer_ack_ctxs);
    ...
    update_map_and_acquire(
        mock_threads, mock_update_request, mock_listener, shuffled_global_image_ids, 0, &peer_ack_ctxs);
    ...
    remote_peer_ack_nowait(mock_image_map.get(), shuffled_global_image_ids, 0, &peer_ack_ctxs);
...
TEST_F(TestMockImageMap, RemoveInstance)
{
    ...
    expect_add_event(mock_threads);
    expect_listener_images_unmapped(mock_listener, 2, &shuffled_global_image_ids, &peer_ack_ctxs);
...
TEST_F(TestMockImageMap, AddInstancePingPongImageTest)
{
    ...
    // ACQUIRE
    MockLoadRequest mock_load_request;
    EXPECT_CALL(mock_load_request, send()).WillOnce(Invoke([&mock_load_request, &image_mapping]() {
    ...
    // RELEASE+UPDATE_MAPPING+ACQUIRE
    expect_add_event(mock_threads);
    expect_listener_images_unmapped(mock_listener, 7, &shuffled_global_image_ids, &peer_ack_ctxs);
    ...
    std::set_intersection(migrated_global_image_ids.begin(),
                          migrated_global_image_ids.end(),
                          shuffled_global_image_ids.begin(),
                          shuffled_global_image_ids.end(),
...
TEST_F(TestMockImageMap, RemoveInstanceWithRemoveImage)
{
    ...
    std::set<std::string> remove_global_image_ids{"global id 1"};
    std::set<std::string> remove_global_image_ids_ack(remove_global_image_ids);
    ...
    std::map<std::string, Context*> peer_ack_remove_ctxs;
    listener_remove_images(mock_listener, "uuid1", shuffled_global_image_ids, &peer_ack_remove_ctxs);
    expect_add_event(mock_threads);
    listener_release_images(mock_listener, shuffled_global_image_ids, &peer_ack_ctxs);
    expect_add_event(mock_threads);
    expect_update_request(mock_update_request, 0);
    expect_add_event(mock_threads);
    expect_update_request(mock_update_request, 0);
    ...
    remote_peer_ack_nowait(mock_image_map.get(), shuffled_global_image_ids, -ENOENT, &peer_ack_remove_ctxs);
    remote_peer_ack_wait(mock_image_map.get(), shuffled_global_image_ids, -EBLOCKLISTED, &peer_ack_ctxs);
...
TEST_F(TestMockImageMap, AddErrorAndRemoveImage)
{
    ...
    expect_add_event(mock_threads);
    expect_listener_images_unmapped(mock_listener, 1, &released_global_image_ids, &release_peer_ack_ctxs);
    expect_add_event(mock_threads);
    expect_listener_images_unmapped(mock_listener, 1, &released_global_image_ids, &release_peer_ack_ctxs);

    // instance blocklisted -- ACQUIRE request fails
    remote_peer_ack_nowait(mock_image_map.get(), shuffled_global_image_ids, -EBLOCKLISTED, &peer_ack_ctxs);
    ASSERT_TRUE(wait_for_listener_notify(shuffled_global_image_ids.size()));

    std::map<std::string, Context*> remap_peer_ack_ctxs;
    update_map_and_acquire(
        mock_threads, mock_update_request, mock_listener, shuffled_global_image_ids, 0, &remap_peer_ack_ctxs);

    // instance blocklisted -- RELEASE request fails
    remote_peer_ack_listener_wait(mock_image_map.get(), shuffled_global_image_ids, -ENOENT, &release_peer_ack_ctxs);
    wait_for_scheduled_task();

    // new peer acks acquire request
    remote_peer_ack_nowait(mock_image_map.get(), shuffled_global_image_ids, 0, &remap_peer_ack_ctxs);
    wait_for_scheduled_task();
    ...
    remote_peer_ack_nowait(mock_image_map.get(), shuffled_global_image_ids_ack, 0, &peer_ack_remove_ctxs);
...
TEST_F(TestMockImageMap, MirrorUUIDUpdated)
{
    ...
    // remote image set
    std::set<std::string> initial_remote_global_image_ids{"global id 1", "global id 2", "global id 3"};
    std::set<std::string> initial_remote_global_image_ids_ack(initial_remote_global_image_ids);

    // remote/local images to remove
    std::set<std::string> remote_removed_global_image_ids{"global id 1", "global id 2", "global id 3"};
    std::set<std::string> remote_removed_global_image_ids_ack(remote_removed_global_image_ids);

    std::set<std::string> remote_added_global_image_ids{"global id 1", "global id 2", "global id 3"};
    ...
    // RELEASE+REMOVE_MAPPING
    std::map<std::string, Context*> peer_remove_ack_ctxs;
    listener_remove_images(mock_listener, "uuid1", remote_removed_global_image_ids, &peer_remove_ack_ctxs);
    expect_add_event(mock_threads);
    listener_release_images(mock_listener, remote_removed_global_image_ids, &peer_ack_ctxs);
    ...
    remote_peer_ack_nowait(mock_image_map.get(), remote_removed_global_image_ids_ack, 0, &peer_remove_ack_ctxs);
    ...
    expect_update_request(mock_update_request, 0);
    expect_add_event(mock_threads);
    listener_acquire_images(mock_listener, remote_added_global_image_ids, &peer_ack_ctxs);
    ...
    // remote peer ACKs image acquire request
    remote_peer_ack_nowait(mock_image_map.get(), remote_added_global_image_ids_ack, 0, &peer_ack_ctxs);
...
TEST_F(TestMockImageMap, RebalanceImageMap)
{
    ...
    // RELEASE+UPDATE_MAPPING+ACQUIRE
    expect_add_event(mock_threads);
    expect_listener_images_unmapped(mock_listener, 5, &shuffled_global_image_ids, &peer_ack_ctxs);
    ...
    std::set<std::string> new_global_image_ids = {"global id 11"};
    ...
    remote_peer_ack_nowait(mock_image_map.get(), new_global_image_ids_ack, 0, &peer_ack_ctxs);
