...
struct rbd_bencher
{
    ...
    bool start_write(int max, uint64_t off, uint64_t len, bufferlist& bl, int op_flags)
    {
        {
            std::lock_guard l{lock};
            if (in_flight >= max) return false;
            in_flight++;
        }
...
void write_image(librbd::Image& image)
{
    ...
    while (true) {
        ...
        for (uint32_t i = 0; i < NUM_THREADS; ++i) {
            ...
            if (!b.start_write(NUM_THREADS, thread_offset[i], io_size, bl, LIBRADOS_OP_FLAG_FADVISE_RANDOM)) {
                break;
            }
            ++i;
            ...
            total_bytes += io_size;
            if (total_ios % 100 == 0) {
                std::cout << total_ios << " IOs, " << total_bytes << " bytes" << std::endl;
            }
        }
    }
    b.wait_for(0);
