...
public:
    ...
    void expect_update_sync_points(MockSyncPointHandler& mock_sync_point_handler, bool complete, int r)
    {
    ...
    void expect_get_snap_id(librbd::MockTestImageCtx& mock_remote_image_ctx, const std::string& snap_name,
                            uint64_t snap_id)
    {
        EXPECT_CALL(mock_remote_image_ctx, get_snap_id(_, StrEq(snap_name))).WillOnce(Return(snap_id));
    }

    void expect_image_refresh(librbd::MockTestImageCtx& mock_remote_image_ctx, int r)
    {
        EXPECT_CALL(*mock_remote_image_ctx.state, refresh(_)).WillOnce(CompleteContext(r));
    }

    void expect_snap_remove(librbd::MockTestImageCtx& mock_remote_image_ctx, const std::string& snap_name, int r)
    {
    ...
    MockSyncPointPruneRequest* create_request(librbd::MockTestImageCtx& mock_remote_image_ctx,
                                              MockSyncPointHandler& mock_sync_point_handler, bool sync_complete,
                                              Context* ctx)
    {
        return new MockSyncPointPruneRequest(&mock_remote_image_ctx, sync_complete, &mock_sync_point_handler, ctx);
    }
...
{
    m_sync_points.emplace_front(cls::rbd::UserSnapshotNamespace(), "snap1", "", boost::none);
    ...
    C_SaferCond ctx;
    MockSyncPointPruneRequest* req = create_request(mock_remote_image_ctx, mock_sync_point_handler, false, &ctx);
    req->send();
...
{
    m_sync_points.emplace_front(cls::rbd::UserSnapshotNamespace(), "snap2", "snap1", boost::none);
    m_sync_points.emplace_front(cls::rbd::UserSnapshotNamespace(), "snap1", "", boost::none);
    auto sync_points = m_sync_points;
...
TEST_F(TestMockImageSyncSyncPointPruneRequest, SyncCompleteSuccess)
{
    ...
    C_SaferCond ctx;
    MockSyncPointPruneRequest* req = create_request(mock_remote_image_ctx, mock_sync_point_handler, true, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
...
{
    m_sync_points.emplace_front(cls::rbd::UserSnapshotNamespace(), "snap3", "snap2", boost::none);
    m_sync_points.emplace_front(cls::rbd::UserSnapshotNamespace(), "snap2", "snap1", boost::none);
    auto sync_points = m_sync_points;
