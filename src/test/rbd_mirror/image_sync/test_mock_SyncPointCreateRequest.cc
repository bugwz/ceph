...
public:
    ...
    void expect_update_sync_points(MockSyncPointHandler& mock_sync_point_handler, int r)
    {
    ...
    MockSyncPointCreateRequest* create_request(librbd::MockTestImageCtx& mock_remote_image_ctx,
                                               MockSyncPointHandler& mock_sync_point_handler, Context* ctx)
    {
        return new MockSyncPointCreateRequest(&mock_remote_image_ctx, "uuid", &mock_sync_point_handler, ctx);
    }
...
TEST_F(TestMockImageSyncSyncPointCreateRequest, Success)
{
    ...
    C_SaferCond ctx;
    MockSyncPointCreateRequest* req = create_request(mock_remote_image_ctx, mock_sync_point_handler, &ctx);
    req->send();
...
{
    m_sync_points.emplace_front(cls::rbd::UserSnapshotNamespace(), "start snap", "", boost::none);
