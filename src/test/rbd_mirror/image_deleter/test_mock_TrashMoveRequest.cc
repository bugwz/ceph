...
struct MockTestImageCtx : public librbd::MockImageCtx
{
    ...
    static MockTestImageCtx* create(const std::string& image_name, const std::string& image_id, const char* snap,
                                    librados::IoCtx& p, bool read_only)
    {
...
template<> struct TrashWatcher<MockTestImageCtx>
{
    static TrashWatcher* s_instance;
    static void notify_image_added(librados::IoCtx&, const std::string& image_id, const cls::rbd::TrashImageSpec& spec,
                                   Context* ctx)
    {
        ceph_assert(s_instance != nullptr);
    ...
    MOCK_METHOD3(notify_image_added, void(const std::string&, const cls::rbd::TrashImageSpec&, Context*));

    TrashWatcher() { s_instance = this; }
};
...
template<> struct ResetRequest<MockTestImageCtx>
{
    static ResetRequest* s_instance;
    Context* on_finish = nullptr;

    static ResetRequest* create(librados::IoCtx& io_ctx, const std::string& image_id, const std::string& client_id,
                                const std::string& mirror_uuid, ContextWQ* op_work_queue, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
...
template<> struct GetInfoRequest<librbd::MockTestImageCtx>
{
    ...
    static GetInfoRequest* create(librados::IoCtx& io_ctx, librbd::asio::ContextWQ* context_wq,
                                  const std::string& image_id, cls::rbd::MirrorImage* mirror_image,
                                  PromotionState* promotion_state, std::string* primary_mirror_uuid, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
    ...
    ~GetInfoRequest() { s_instance = nullptr; }
...
template<> struct ImageRemoveRequest<librbd::MockTestImageCtx>
{
    ...
    static ImageRemoveRequest* create(librados::IoCtx& io_ctx, const std::string& global_image_id,
    ...
    ~ImageRemoveRequest() { s_instance = nullptr; }
...
template<> struct MoveRequest<MockTestImageCtx>
{
    ...
    static MoveRequest* create(librados::IoCtx& io_ctx, const std::string& image_id,
    ...
    MOCK_METHOD2(construct, void(const std::string&, const cls::rbd::TrashImageSpec&));
    MOCK_METHOD0(send, void());

    MoveRequest() { s_instance = this; }
};
...
public:
    ...
    void expect_get_mirror_info(MockGetMirrorInfoRequest& mock_get_mirror_info_request,
                                const cls::rbd::MirrorImage& mirror_image,
                                librbd::mirror::PromotionState promotion_state, const std::string& primary_mirror_uuid,
                                int r)
    {
        EXPECT_CALL(mock_get_mirror_info_request, send())
            .WillOnce(
                Invoke([this, &mock_get_mirror_info_request, mirror_image, promotion_state, primary_mirror_uuid, r]() {
                    *mock_get_mirror_info_request.mirror_image = mirror_image;
                    *mock_get_mirror_info_request.promotion_state = promotion_state;
                    *mock_get_mirror_info_request.primary_mirror_uuid = primary_mirror_uuid;
    ...
    {
        EXPECT_CALL(mock_image_ctx, set_journal_policy(_)).WillOnce(Invoke([](librbd::journal::Policy* policy) {
            delete policy;
        }));
    }
    ...
    void expect_open(librbd::MockTestImageCtx& mock_image_ctx, int r)
    {
        EXPECT_CALL(*mock_image_ctx.state, open(true, _))
            .WillOnce(WithArg<1>(Invoke([this, &mock_image_ctx, r](Context* ctx) {
                EXPECT_EQ(0U, mock_image_ctx.read_only_mask & librbd::IMAGE_READ_ONLY_FLAG_NON_PRIMARY);
    ...
    {
        EXPECT_CALL(*mock_image_ctx.state, close(_)).WillOnce(Invoke([this, r](Context* ctx) {
            m_threads->work_queue->queue(ctx, r);
        }));
    }
    ...
    {
        EXPECT_CALL(*mock_image_ctx.exclusive_lock, acquire_lock(_)).WillOnce(Invoke([this, r](Context* ctx) {
            m_threads->work_queue->queue(ctx, r);
        }));
    }

    void expect_mirror_image_set(const std::string& image_id, const cls::rbd::MirrorImage& mirror_image, int r)
    {
        bufferlist bl;
        encode(image_id, bl);
        ...
        EXPECT_CALL(get_mock_io_ctx(m_local_io_ctx),
                    exec(RBD_MIRRORING, _, StrEq("rbd"), StrEq("mirror_image_set"), ContentsEqual(bl), _, _, _))
            .WillOnce(Return(r));
    }

    void expect_mirror_image_remove_request(MockImageRemoveRequest& mock_image_remove_request, int r)
    {
        EXPECT_CALL(mock_image_remove_request, send()).WillOnce(Invoke([this, &mock_image_remove_request, r]() {
            m_threads->work_queue->queue(mock_image_remove_request.on_finish, r);
        }));
    }

    void expect_journal_reset(MockJournalResetRequest& mock_journal_reset_request, int r)
    {
        EXPECT_CALL(mock_journal_reset_request, send()).WillOnce(Invoke([this, &mock_journal_reset_request, r]() {
            m_threads->work_queue->queue(mock_journal_reset_request.on_finish, r);
        }));
    }

    void expect_trash_move(MockLibrbdTrashMoveRequest& mock_trash_move_request, const std::string& image_name,
                           const std::string& image_id, const boost::optional<uint32_t>& delay, int r)
    {
        EXPECT_CALL(mock_trash_move_request, construct(image_id, _))
            .WillOnce(WithArg<1>(Invoke([image_name, delay](const cls::rbd::TrashImageSpec& spec) {
                ASSERT_EQ(cls::rbd::TRASH_IMAGE_SOURCE_MIRRORING, spec.source);
        ...
        EXPECT_CALL(mock_trash_move_request, send()).WillOnce(Invoke([this, &mock_trash_move_request, r]() {
            m_threads->work_queue->queue(mock_trash_move_request.on_finish, r);
        }));
    }

    void expect_notify_image_added(MockTrashWatcher& mock_trash_watcher, const std::string& image_id)
    {
        EXPECT_CALL(mock_trash_watcher, notify_image_added(image_id, _, _))
            .WillOnce(WithArg<2>(Invoke([this](Context* ctx) { m_threads->work_queue->queue(ctx, 0); })));
    }
...
TEST_F(TestMockImageDeleterTrashMoveRequest, SuccessJournal)
{
    ...
    expect_get_mirror_info(
        ...
        {cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "global image id", cls::rbd::MIRROR_IMAGE_STATE_ENABLED},
        librbd::mirror::PROMOTION_STATE_ORPHAN,
        "remote mirror uuid",
        0);

    expect_mirror_image_set(
        "image id",
        {cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "global image id", cls::rbd::MIRROR_IMAGE_STATE_DISABLING},
        0);
    ...
    expect_trash_move(mock_librbd_trash_move_request, m_image_name, "image id", {}, 0);
    ...
    C_SaferCond ctx;
    auto req =
        MockTrashMoveRequest::create(m_local_io_ctx, "global image id", true, m_local_image_ctx->op_work_queue, &ctx);
    req->send();
...
TEST_F(TestMockImageDeleterTrashMoveRequest, SuccessSnapshot)
{
    ...
    expect_get_mirror_info(
        mock_get_mirror_info_request,
        {cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, "global image id", cls::rbd::MIRROR_IMAGE_STATE_ENABLED},
    ...
    expect_mirror_image_set(
        "image id",
        {cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, "global image id", cls::rbd::MIRROR_IMAGE_STATE_DISABLING},
        0);
    ...
    C_SaferCond ctx;
    auto req =
        MockTrashMoveRequest::create(m_local_io_ctx, "global image id", false, m_local_image_ctx->op_work_queue, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(TestMockImageDeleterTrashMoveRequest, TrashMoveError)
{
    ...
    MockLibrbdTrashMoveRequest mock_librbd_trash_move_request;
    expect_trash_move(mock_librbd_trash_move_request, m_image_name, "image id", {}, -EINVAL);
...
TEST_F(TestMockImageDeleterTrashMoveRequest, DelayedDelation)
{
    ...
    MockLibrbdTrashMoveRequest mock_librbd_trash_move_request;
    expect_trash_move(mock_librbd_trash_move_request, m_image_name, "image id", 600, 0);
