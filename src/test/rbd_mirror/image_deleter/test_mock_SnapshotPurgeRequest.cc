...
public:
    ...
    void expect_get_snap_namespace(librbd::MockTestImageCtx& mock_image_ctx, uint64_t snap_id,
                                   const cls::rbd::SnapshotNamespace& snap_namespace, int r)
    {
    ...
    void expect_get_snap_name(librbd::MockTestImageCtx& mock_image_ctx, uint64_t snap_id, const std::string& name,
                              int r)
    {
        EXPECT_CALL(mock_image_ctx, get_snap_name(snap_id, _)).WillOnce(WithArg<1>(Invoke([name, r](std::string* n) {
            *n = name;
            return r;
        })));
    }

    void expect_is_snap_protected(librbd::MockTestImageCtx& mock_image_ctx, uint64_t snap_id, bool is_protected, int r)
    {
    ...
    void expect_snap_unprotect(librbd::MockTestImageCtx& mock_image_ctx, const cls::rbd::SnapshotNamespace& ns,
    ...
    void expect_snap_remove(librbd::MockTestImageCtx& mock_image_ctx, const cls::rbd::SnapshotNamespace& ns,
                            const std::string& name, int r)
    {
    ...
    {
        EXPECT_CALL(*mock_image_ctx.exclusive_lock, start_op(_)).WillOnce(Invoke([success](int* r) {
            auto f = [](int r) {};
            if (!success) {
                *r = -EROFS;
...
{
    {
        ...
        m_local_image_ctx->add_snap(
            cls::rbd::UserSnapshotNamespace{}, "snap1", 1, 0, {}, RBD_PROTECTION_STATUS_PROTECTED, 0, {});
        m_local_image_ctx->add_snap(
            cls::rbd::UserSnapshotNamespace{}, "snap2", 2, 0, {}, RBD_PROTECTION_STATUS_UNPROTECTED, 0, {});
    }
    ...
    expect_get_snap_namespace(mock_image_ctx, 2, cls::rbd::UserSnapshotNamespace{}, 0);
    ...
    expect_snap_remove(mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap2", 0);

    expect_get_snap_namespace(mock_image_ctx, 1, cls::rbd::UserSnapshotNamespace{}, 0);
    expect_get_snap_name(mock_image_ctx, 1, "snap1", 0);
    expect_is_snap_protected(mock_image_ctx, 1, true, 0);
    expect_start_op(mock_image_ctx, true);
    expect_snap_unprotect(mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap1", 0);
    expect_start_op(mock_image_ctx, true);
    expect_snap_remove(mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap1", 0);

    expect_close(mock_image_ctx, 0);

    C_SaferCond ctx;
    auto req = MockSnapshotPurgeRequest::create(m_local_io_ctx, mock_image_ctx.id, &ctx);
    req->send();
...
{
    {
        std::unique_lock image_locker{m_local_image_ctx->image_lock};
        m_local_image_ctx->add_snap(
            cls::rbd::UserSnapshotNamespace{}, "snap1", 1, 0, {}, RBD_PROTECTION_STATUS_UNPROTECTED, 0, {});
    }
...
TEST_F(TestMockImageDeleterSnapshotPurgeRequest, SnapUnprotectBusy)
{
    ...
    expect_is_snap_protected(mock_image_ctx, 1, true, 0);
    expect_start_op(mock_image_ctx, true);
    expect_snap_unprotect(mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap1", -EBUSY);
...
TEST_F(TestMockImageDeleterSnapshotPurgeRequest, SnapUnprotectError)
{
    ...
    expect_is_snap_protected(mock_image_ctx, 1, true, 0);
    expect_start_op(mock_image_ctx, true);
    expect_snap_unprotect(mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap1", -EPERM);
...
TEST_F(TestMockImageDeleterSnapshotPurgeRequest, SnapRemoveError)
{
    ...
    expect_start_op(mock_image_ctx, true);
    expect_snap_remove(mock_image_ctx, cls::rbd::UserSnapshotNamespace{}, "snap1", -EINVAL);
