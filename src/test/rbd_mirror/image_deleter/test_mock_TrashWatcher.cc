...
struct MockTrashWatcher
{
    ...
    MockTrashWatcher() { s_instance = this; }
...
template<> struct TrashWatcher<MockTestImageCtx>
{
    ...
    TrashWatcher(librados::IoCtx& io_ctx, ::MockContextWQ* work_queue) { s_instance = this; }
    ...
    virtual void handle_image_added(const std::string& image_id, const cls::rbd::TrashImageSpec& spec) = 0;
    virtual void handle_image_removed(const std::string& image_id) = 0;

    bool is_unregistered() const { return MockTrashWatcher::get_instance().is_unregistered(); }
    void register_watch(Context* ctx) { MockTrashWatcher::get_instance().register_watch(ctx); }
    void unregister_watch(Context* ctx) { MockTrashWatcher::get_instance().unregister_watch(ctx); }
};
...
public:
    ...
    {
        MOCK_METHOD2(handle_trash_image, void(const std::string&, const ceph::real_clock::time_point&));
    };
    ...
    void expect_trash_watcher_is_unregistered(MockLibrbdTrashWatcher& mock_trash_watcher, bool unregistered)
    {
        EXPECT_CALL(mock_trash_watcher, is_unregistered()).WillOnce(Return(unregistered));
    }

    void expect_trash_watcher_register(MockLibrbdTrashWatcher& mock_trash_watcher, int r)
    {
        EXPECT_CALL(mock_trash_watcher, register_watch(_)).WillOnce(CompleteContext(r));
    }

    void expect_trash_watcher_unregister(MockLibrbdTrashWatcher& mock_trash_watcher, int r)
    {
        EXPECT_CALL(mock_trash_watcher, unregister_watch(_)).WillOnce(CompleteContext(r));
    }

    void expect_create_trash(librados::IoCtx& io_ctx, int r)
    {
        EXPECT_CALL(get_mock_io_ctx(io_ctx), create(RBD_TRASH, false, _)).WillOnce(Return(r));
    }

    void expect_trash_list(librados::IoCtx& io_ctx, const std::string& last_image_id,
                           std::map<std::string, cls::rbd::TrashImageSpec>&& images, int r)
    {
        bufferlist bl;
        ...
        EXPECT_CALL(get_mock_io_ctx(io_ctx),
                    exec(RBD_TRASH, _, StrEq("rbd"), StrEq("trash_list"), ContentsEqual(bl), _, _, _))
    ...
    void expect_handle_trash_image(MockListener& mock_listener, const std::string& global_image_id)
    {
...
TEST_F(TestMockImageDeleterTrashWatcher, EmptyPool)
{
    ...
    MockTrashWatcher mock_trash_watcher(m_local_io_ctx, &mock_threads, mock_listener);
    C_SaferCond ctx;
...
TEST_F(TestMockImageDeleterTrashWatcher, Notify)
{
    ...
    LibrbdTrashWatcher::get_instance().handle_image_added("image1",
                                                          {cls::rbd::TRASH_IMAGE_SOURCE_MIRRORING, "name", {}, {}});
    m_threads->work_queue->drain();
