...
public:
    ...
    {
        TestFixture::SetUp();
        m_service_daemon.reset(new rbd::mirror::ServiceDaemon<>(g_ceph_context, m_cluster, m_threads));
        m_cluster_watcher.reset(new ClusterWatcher(m_cluster, m_lock, m_service_daemon.get()));
    }
    ...
    void create_pool(bool enable_mirroring, const PeerSpec& peer, string* uuid = nullptr, string* name = nullptr)
    {
    ...
    void set_peer_config_key(const std::string& pool_name, const PeerSpec& peer)
    {
        int64_t pool_id = m_cluster->pool_lookup(pool_name.c_str());
        ASSERT_GE(pool_id, 0);
    ...
    void create_cache_pool(const string& base_pool, string* cache_pool_name)
    {
        ...
        *cache_pool_name = get_temp_pool_name("test-rbd-mirror-");
        ...
        ASSERT_EQ(
            0,
            m_cluster->mon_command("{\"prefix\": \"osd tier add\", \"pool\": \"" + base_pool + "\", \"tierpool\": \"" +
                                       *cache_pool_name + "\", \"force_nonempty\": \"--force-nonempty\" }",
                                   inbl,
                                   NULL,
                                   NULL));
        ASSERT_EQ(0,
                  m_cluster->mon_command("{\"prefix\": \"osd tier set-overlay\", \"pool\": \"" + base_pool +
                                             "\", \"overlaypool\": \"" + *cache_pool_name + "\"}",
                                         inbl,
                                         NULL,
                                         NULL));
        ASSERT_EQ(0,
                  m_cluster->mon_command("{\"prefix\": \"osd tier cache-mode\", \"pool\": \"" + *cache_pool_name +
                                             "\", \"mode\": \"writeback\"}",
                                         inbl,
                                         NULL,
                                         NULL));
    ...
    void remove_cache_pool(const string& base_pool, const string& cache_pool)
    {
        ...
        ASSERT_EQ(0,
                  m_cluster->mon_command(
                      "{\"prefix\": \"osd tier remove-overlay\", \"pool\": \"" + base_pool + "\"}", inbl, NULL, NULL));
        ASSERT_EQ(0,
                  m_cluster->mon_command("{\"prefix\": \"osd tier remove\", \"pool\": \"" + base_pool +
                                             "\", \"tierpool\": \"" + cache_pool + "\"}",
                                         inbl,
                                         NULL,
                                         NULL));
