...
public:
    ...
    struct PoolWatcherListener : public rbd::mirror::pool_watcher::Listener
    {
        ...
        explicit PoolWatcherListener(TestPoolWatcher* test)
            : test(test)
        {}
    ...
    void create_pool(bool enable_mirroring, const PeerSpec& peer, string* name = nullptr)
    {
        ...
        m_pool_watcher.reset(new PoolWatcher<>(m_threads, ioctx, "mirror uuid", m_pool_watcher_listener));

        if (enable_mirroring) {
            ASSERT_EQ(0, librbd::api::Mirror<>::mode_set(ioctx, RBD_MIRROR_MODE_POOL));
            std::string uuid;
            ASSERT_EQ(0,
                      librbd::api::Mirror<>::peer_site_add(
                          ioctx, &uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, peer.cluster_name, peer.client_name));
        }
        if (name != nullptr) {
            *name = pool_name;
        }
    ...
    void create_image(const string& pool_name, bool mirrored = true, string* image_name = nullptr)
    {
        ...
        int order = 0;
        ASSERT_EQ(0, librbd::create(ioctx, name.c_str(), 1 << 22, false, features, &order, 0, 0));
        if (mirrored) {
            librbd::Image image;
            librbd::RBD rbd;
            ...
            ASSERT_EQ(0, image.mirror_image_get_info(&mirror_image_info, sizeof(mirror_image_info)));
            image.close();

            m_mirrored_images.insert(ImageId(mirror_image_info.global_id, get_image_id(&ioctx, name)));
        }
        if (image_name != nullptr) *image_name = name;
    }

    void clone_image(const string& parent_pool_name, const string& parent_image_name, const string& clone_pool_name,
                     bool mirrored = true, string* image_name = nullptr)
    {
        ...
        {
            librbd::ImageCtx* ictx = new librbd::ImageCtx(parent_image_name.c_str(), "", "", pioctx, false);
            ictx->state->open(0);
        ...
        int order = 0;
        librbd::clone(
            pioctx, parent_image_name.c_str(), snap_name.c_str(), cioctx, name.c_str(), features, &order, 0, 0);
        if (mirrored) {
            librbd::Image image;
            librbd::RBD rbd;
            ...
            m_mirrored_images.insert(ImageId(mirror_image_info.global_id, get_image_id(&cioctx, name)));
        }
        if (image_name != nullptr) *image_name = name;
    }
