...
template<> struct ImageDeleter<librbd::MockTestImageCtx>
{
    ...
    static void trash_move(librados::IoCtx& local_io_ctx, const std::string& global_image_id, bool resync,
...
template<> struct MirrorStatusUpdater<librbd::MockTestImageCtx>
{
    ...
    MOCK_METHOD3(set_mirror_image_status, void(const std::string&, const cls::rbd::MirrorImageSiteStatus&, bool));
    MOCK_METHOD2(remove_refresh_mirror_image_status, void(const std::string&, Context*));
    MOCK_METHOD3(remove_mirror_image_status, void(const std::string&, bool, Context*));
};
...
template<> class InstanceWatcher<librbd::MockTestImageCtx>
{};

namespace image_replayer {

template<> struct BootstrapRequest<librbd::MockTestImageCtx>
{
    ...
    static BootstrapRequest* create(Threads<librbd::MockTestImageCtx>* threads, librados::IoCtx& local_io_ctx,
                                    ...
                                    PoolMetaCache* pool_meta_cache, rbd::mirror::ProgressContext* progress_ctx,
                                    StateBuilder<librbd::MockTestImageCtx>** state_builder, bool* do_resync,
                                    Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
    ...
    std::string get_local_image_name() const { return "local image name"; }

    inline bool is_syncing() const { return false; }
...
template<> struct StateBuilder<librbd::MockTestImageCtx>
{
    ...
    MOCK_METHOD5(create_replayer,
                 Replayer*(Threads<librbd::MockTestImageCtx>*, InstanceWatcher<librbd::MockTestImageCtx>*,
                           const std::string&, PoolMetaCache*, ReplayerListener*));
...
public:
    ...
    void expect_trash_move(MockImageDeleter& mock_image_deleter, const std::string& global_image_id, bool ignore_orphan,
                           int r)
    {
        EXPECT_CALL(mock_image_deleter, trash_move(global_image_id, ignore_orphan, _))
    ...
    void expect_send(MockBootstrapRequest& mock_bootstrap_request, MockStateBuilder& mock_state_builder,
                     librbd::MockTestImageCtx& mock_local_image_ctx, bool do_resync, bool set_local_image, int r)
    {
    ...
    void expect_create_replayer(MockStateBuilder& mock_state_builder, MockReplayer& mock_replayer)
    {
        EXPECT_CALL(mock_state_builder, create_replayer(_, _, _, _, _))
            .WillOnce(WithArg<4>(Invoke([&mock_replayer](image_replayer::ReplayerListener* replayer_listener) {
    ...
    {
        EXPECT_CALL(mock_state_builder, close(_)).WillOnce(Invoke([this, r](Context* ctx) {
            m_threads->work_queue->queue(ctx, r);
        }));
    }
    ...
    {
        EXPECT_CALL(mock_replayer, init(_)).WillOnce(Invoke([this, r](Context* ctx) {
            m_threads->work_queue->queue(ctx, r);
        }));
    }
    ...
    {
        EXPECT_CALL(mock_replayer, shut_down(_)).WillOnce(Invoke([this, r](Context* ctx) {
            m_threads->work_queue->queue(ctx, r);
        }));
    ...
    void expect_get_replay_status(MockReplayer& mock_replayer)
    {
        EXPECT_CALL(mock_replayer, get_replay_status(_, _))
            .WillRepeatedly(DoAll(WithArg<1>(CompleteContext(-EEXIST)), Return(true)));
    }
    ...
    {
        EXPECT_CALL(m_local_status_updater, exists(_)).WillOnce(Return(exists));
        EXPECT_CALL(m_remote_status_updater, exists(_)).WillOnce(Return(exists));
    }

    void create_image_replayer(MockThreads& mock_threads)
    {
        m_image_replayer = new MockImageReplayer(m_local_io_ctx,
                                                 "local_mirror_uuid",
                                                 "global image id",
                                                 &mock_threads,
                                                 &m_instance_watcher,
                                                 ...
                                                 nullptr,
                                                 nullptr);
        m_image_replayer->add_peer({"peer_uuid",
                                    m_remote_io_ctx,
...
TEST_F(TestMockImageReplayer, StartStop)
{
    ...
    expect_send(mock_bootstrap_request, mock_state_builder, mock_local_image_ctx, false, false, 0);
    ...
    ASSERT_EQ(image_replayer::HEALTH_STATE_OK, m_image_replayer->get_health_state());
...
TEST_F(TestMockImageReplayer, LocalImagePrimary)
{
    ...
    MockStateBuilder mock_state_builder;
    expect_send(mock_bootstrap_request, mock_state_builder, mock_local_image_ctx, false, false, -ENOMSG);
...
TEST_F(TestMockImageReplayer, MetadataCleanup)
{
    ...
    MockStateBuilder mock_state_builder;
    expect_send(mock_bootstrap_request, mock_state_builder, mock_local_image_ctx, false, true, -ENOLINK);
...
TEST_F(TestMockImageReplayer, BootstrapRemoteDeleted)
{
    ...
    MockBootstrapRequest mock_bootstrap_request;
    MockStateBuilder mock_state_builder;
    expect_send(mock_bootstrap_request, mock_state_builder, mock_local_image_ctx, false, false, -ENOLINK);
...
TEST_F(TestMockImageReplayer, BootstrapResyncRequested)
{
    ...
    MockBootstrapRequest mock_bootstrap_request;
    MockStateBuilder mock_state_builder;
    expect_send(mock_bootstrap_request, mock_state_builder, mock_local_image_ctx, true, false, 0);
...
TEST_F(TestMockImageReplayer, BootstrapError)
{
    ...
    InSequence seq;
    MockStateBuilder mock_state_builder;
    expect_send(mock_bootstrap_request, mock_state_builder, mock_local_image_ctx, false, false, -EINVAL);
...
TEST_F(TestMockImageReplayer, BootstrapCancel)
{
    ...
    MockBootstrapRequest mock_bootstrap_request;
    MockStateBuilder mock_state_builder;
    EXPECT_CALL(mock_bootstrap_request, send()).WillOnce(Invoke([this, &mock_bootstrap_request]() {
...
TEST_F(TestMockImageReplayer, BootstrapRemoteDeletedCancel)
{
    ...
    MockStateBuilder mock_state_builder;
    EXPECT_CALL(mock_bootstrap_request, send())
        .WillOnce(Invoke([this, &mock_bootstrap_request, &mock_state_builder, &mock_local_image_ctx]() {
            mock_state_builder.local_image_id = mock_local_image_ctx.id;
            mock_state_builder.remote_image_id = "";
            *mock_bootstrap_request.state_builder = &mock_state_builder;
            m_image_replayer->stop(nullptr);
...
TEST_F(TestMockImageReplayer, ReplayerError)
{
    ...
    EXPECT_CALL(mock_replayer, get_error_description()).WillOnce(Return("FAIL"));
...
TEST_F(TestMockImageReplayer, ReplayerResync)
{
    ...
    // NOTIFY
    EXPECT_CALL(mock_replayer, is_resync_requested()).WillOnce(Return(true));
    expect_shut_down(mock_replayer, 0);
    expect_close(mock_state_builder, 0);
    expect_trash_move(mock_image_deleter, "global image id", true, 0);
    expect_mirror_image_status_exists(false);
...
TEST_F(TestMockImageReplayer, ReplayerInterrupted)
{
    ...
    // NOTIFY
    EXPECT_CALL(mock_replayer, is_resync_requested()).WillOnce(Return(false));
    EXPECT_CALL(mock_replayer, is_replaying()).WillOnce(Return(false));
    EXPECT_CALL(mock_replayer, get_error_code()).WillOnce(Return(-EINVAL));
    EXPECT_CALL(mock_replayer, get_error_description()).WillOnce(Return("INVALID"));
    expect_shut_down(mock_replayer, 0);
    expect_close(mock_state_builder, 0);
    expect_mirror_image_status_exists(false);
    mock_replayer.replayer_listener->handle_notification();
...
TEST_F(TestMockImageReplayer, ReplayerRenamed)
{
    ...
    // NOTIFY
    EXPECT_CALL(mock_replayer, is_resync_requested()).WillOnce(Return(false));
    EXPECT_CALL(mock_replayer, is_replaying()).WillOnce(Return(true));
...
TEST_F(TestMockImageReplayer, StopJoinInterruptedReplayer)
{
    ...
    EXPECT_CALL(mock_replayer, shut_down(_)).WillOnce(Invoke([this, DELAY](Context* ctx) {
