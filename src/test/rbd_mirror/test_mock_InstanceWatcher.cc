...
template<> struct ManagedLock<MockTestImageCtx>
{
    static ManagedLock* s_instance;

    static ManagedLock* create(librados::IoCtx& ioctx, librbd::AsioEngine& asio_engine, const std::string& oid,
                               librbd::Watcher* watcher, managed_lock::Mode mode, bool blocklist_on_break_lock,
...
{
    MOCK_METHOD3(acquire_image, void(InstanceWatcher<librbd::MockTestImageCtx>*, const std::string&, Context*));
    MOCK_METHOD2(release_image, void(const std::string&, Context*));
    MOCK_METHOD3(remove_peer_image, void(const std::string&, const std::string&, Context*));
};
...
template<> struct Throttler<librbd::MockTestImageCtx>
{
    ...
    MOCK_METHOD3(start_op, void(const std::string&, const std::string&, Context*));
...
public:
    ...
    void expect_register_watch(librados::MockTestMemIoCtxImpl& mock_io_ctx, const std::string& instance_id)
    {
    ...
    void expect_register_instance(librados::MockTestMemIoCtxImpl& mock_io_ctx, int r)
    {
        EXPECT_CALL(mock_io_ctx, exec(RBD_MIRROR_LEADER, _, StrEq("rbd"), StrEq("mirror_instances_add"), _, _, _, _))
            .WillOnce(Return(r));
    }

    void expect_unregister_instance(librados::MockTestMemIoCtxImpl& mock_io_ctx, int r)
    {
        EXPECT_CALL(mock_io_ctx, exec(RBD_MIRROR_LEADER, _, StrEq("rbd"), StrEq("mirror_instances_remove"), _, _, _, _))
            .WillOnce(Return(r));
    }

    void expect_acquire_lock(MockManagedLock& mock_managed_lock, int r)
    {
        EXPECT_CALL(mock_managed_lock, acquire_lock(_)).WillOnce(CompleteContext(r));
    }
    ...
    void expect_destroy_lock(MockManagedLock& mock_managed_lock, Context* ctx = nullptr)
    {
        EXPECT_CALL(mock_managed_lock, destroy()).WillOnce(Invoke([ctx]() {
            if (ctx != nullptr) {
                ctx->complete(0);
            }
        }));
    ...
    void expect_get_locker(MockManagedLock& mock_managed_lock, const librbd::managed_lock::Locker& locker, int r)
    {
        EXPECT_CALL(mock_managed_lock, get_locker(_, _))
            .WillOnce(Invoke([r, locker](librbd::managed_lock::Locker* out, Context* ctx) {
                if (r == 0) {
                    *out = locker;
                }
                ctx->complete(r);
    ...
    void expect_break_lock(MockManagedLock& mock_managed_lock, const librbd::managed_lock::Locker& locker, int r)
    {
        EXPECT_CALL(mock_managed_lock, break_lock(locker, true, _)).WillOnce(WithArg<2>(CompleteContext(r)));
    }
};
...
TEST_F(TestMockInstanceWatcher, InitShutdown)
{
    ...
    auto instance_watcher =
        new MockInstanceWatcher(m_local_io_ctx, *m_mock_threads->asio_engine, nullptr, nullptr, m_instance_id);
    InSequence seq;
...
TEST_F(TestMockInstanceWatcher, Remove)
{
    MockManagedLock mock_managed_lock;
    librados::MockTestMemIoCtxImpl& mock_io_ctx(get_mock_io_ctx(m_local_io_ctx));
    librbd::managed_lock::Locker locker{entity_name_t::CLIENT(1), "auto 123", "1.2.3.4:0/0", 123};
    ...
    MockInstanceWatcher::remove_instance(m_local_io_ctx, *m_mock_threads->asio_engine, "instance_id", &on_remove);
...
TEST_F(TestMockInstanceWatcher, ImageAcquireRelease)
{
    ...
    auto instance_watcher1 =
        MockInstanceWatcher::create(io_ctx1, *m_mock_threads->asio_engine, &mock_instance_replayer1, nullptr);
    ...
    auto instance_watcher2 =
        MockInstanceWatcher::create(io_ctx2, *m_mock_threads->asio_engine, &mock_instance_replayer2, nullptr);
    ...
    EXPECT_CALL(mock_instance_replayer1, acquire_image(instance_watcher1, "gid", _))
    ...
    EXPECT_CALL(mock_instance_replayer2, acquire_image(instance_watcher2, "gid", _))
        .WillOnce(WithArg<2>(CompleteContext(0)));
    ...
    EXPECT_CALL(mock_instance_replayer1, release_image("gid", _)).WillOnce(WithArg<1>(CompleteContext(0)));
    ...
    EXPECT_CALL(mock_instance_replayer2, release_image("gid", _)).WillOnce(WithArg<1>(CompleteContext(0)));
...
TEST_F(TestMockInstanceWatcher, PeerImageRemoved)
{
    ...
    EXPECT_CALL(mock_instance_replayer1, remove_peer_image("gid", "uuid", _)).WillOnce(WithArg<2>(CompleteContext(0)));
    C_SaferCond on_removed1;
    instance_watcher1->notify_peer_image_removed(instance_id1, "gid", "uuid", &on_removed1);
    ASSERT_EQ(0, on_removed1.wait());

    // Peer Image Removed on the other instance
    EXPECT_CALL(mock_instance_replayer2, remove_peer_image("gid", "uuid", _)).WillOnce(WithArg<2>(CompleteContext(0)));
    C_SaferCond on_removed2;
    instance_watcher1->notify_peer_image_removed(instance_id2, "gid", "uuid", &on_removed2);
...
TEST_F(TestMockInstanceWatcher, ImageAcquireReleaseCancel)
{
    ...
    EXPECT_CALL(mock_io_ctx, aio_notify(_, _, _, _, _))
        .WillOnce(Invoke([this, instance_watcher, &mock_io_ctx](const std::string& o,
                                                                librados::AioCompletionImpl* c,
                                                                bufferlist& bl,
                                                                uint64_t timeout_ms,
                                                                bufferlist* pbl) {
            c->get();
            auto ctx = new LambdaContext([instance_watcher, &mock_io_ctx, c, pbl](int r) {
                instance_watcher->cancel_notify_requests("other");
                encode(librbd::watcher::NotifyResponse(), *pbl);
                mock_io_ctx.get_mock_rados_client()->finish_aio_completion(c, -ETIMEDOUT);
            });
...
TEST_F(TestMockInstanceWatcher, PeerImageAcquireWatchDNE)
{
    ...
    auto instance_watcher = new MockInstanceWatcher(
        m_local_io_ctx, *m_mock_threads->asio_engine, &mock_instance_replayer, nullptr, m_instance_id);
    InSequence seq;
    ...
    C_SaferCond on_acquire;
    instance_watcher->notify_image_acquire("dead instance", "global image id", &on_acquire);
...
TEST_F(TestMockInstanceWatcher, PeerImageReleaseWatchDNE)
{
    ...
    C_SaferCond on_acquire;
    instance_watcher->notify_image_release("dead instance", "global image id", &on_acquire);
    ASSERT_EQ(-ENOENT, on_acquire.wait());
...
TEST_F(TestMockInstanceWatcher, PeerImageRemovedCancel)
{
    ...
    C_SaferCond on_acquire;
    instance_watcher->notify_peer_image_removed("other", "gid", "uuid", &on_acquire);
    ASSERT_EQ(-ECANCELED, on_acquire.wait());
...
public:
    ...
    void SetUp() override
    {
        ...
        librados::MockTestMemIoCtxImpl& mock_io_ctx1(get_mock_io_ctx(io_ctx1));
        instance_watcher1 =
            MockInstanceWatcher::create(io_ctx1, *m_mock_threads->asio_engine, nullptr, &mock_image_sync_throttler);
        EXPECT_EQ("", connect_cluster_pp(cluster));
        EXPECT_EQ(0, cluster.ioctx_create(_local_pool_name.c_str(), io_ctx2));
        instance_id2 = stringify(io_ctx2.get_instance_id());
        librados::MockTestMemIoCtxImpl& mock_io_ctx2(get_mock_io_ctx(io_ctx2));
        instance_watcher2 =
            MockInstanceWatcher::create(io_ctx2, *m_mock_threads->asio_engine, nullptr, &mock_image_sync_throttler);
        InSequence seq;
    ...
    void expect_throttler_start_op(const std::string& sync_id, Context* on_call = nullptr,
                                   Context** on_start_ctx = nullptr)
    {
        EXPECT_CALL(mock_image_sync_throttler, start_op("", sync_id, _))
            .WillOnce(Invoke([on_call, on_start_ctx](const std::string&, const std::string&, Context* ctx) {
    ...
    void expect_throttler_finish_op(const std::string& sync_id, Context* on_finish)
    {
        EXPECT_CALL(mock_image_sync_throttler, finish_op("", "sync_id"))
            .WillOnce(Invoke([on_finish](const std::string&, const std::string&) { on_finish->complete(0); }));
    }

    void expect_throttler_drain() { EXPECT_CALL(mock_image_sync_throttler, drain("", -ESTALE)); }
};
...
TEST_F(TestMockInstanceWatcher_NotifySync, InFlightPrevNotification)
{
    ...
    EXPECT_CALL(mock_image_sync_throttler, finish_op("", "sync_id"))
        .WillOnce(Invoke([this, &on_start2](const std::string&, const std::string&) {
...
TEST_F(TestMockInstanceWatcher_NotifySync, WaitingOnNonLeaderAcquireLeader)
{
    ...
    EXPECT_CALL(mock_image_sync_throttler, start_op("", "sync_id", _)).WillOnce(WithArg<2>(CompleteContext(0)));
    instance_watcher2->handle_acquire_leader();
    instance_watcher1->handle_update_leader(instance_id2);
