...
struct MockMirroringWatcher
{
    ...
    MockMirroringWatcher() { s_instance = this; }
...
template<> struct MirroringWatcher<MockTestImageCtx>
{
    static MirroringWatcher* s_instance;

    MirroringWatcher(librados::IoCtx& io_ctx, ::MockContextWQ* work_queue) { s_instance = this; }
    ...
    virtual void handle_image_updated(cls::rbd::MirrorImageState state, const std::string& remote_image_id,
                                      const std::string& global_image_id) = 0;

    bool is_unregistered() const { return MockMirroringWatcher::get_instance().is_unregistered(); }
    void register_watch(Context* ctx) { MockMirroringWatcher::get_instance().register_watch(ctx); }
    void unregister_watch(Context* ctx) { MockMirroringWatcher::get_instance().unregister_watch(ctx); }
    std::string get_oid() const { return MockMirroringWatcher::get_instance().get_oid(); }
};
...
template<> struct RefreshImagesRequest<librbd::MockTestImageCtx>
{
    ImageIds* image_ids = nullptr;
    Context* on_finish = nullptr;
    static RefreshImagesRequest* s_instance;
    static RefreshImagesRequest* create(librados::IoCtx& io_ctx, ImageIds* image_ids, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
    ...
    RefreshImagesRequest() { s_instance = this; }
};
...
public:
    ...
    struct MockListener : pool_watcher::Listener
    {
        ...
        MockListener(TestMockPoolWatcher* test)
            : test(test)
        {}

        MOCK_METHOD3(mock_handle_update, void(const std::string&, const ImageIds&, const ImageIds&));
    ...
    void expect_mirroring_watcher_is_unregistered(MockMirroringWatcher& mock_mirroring_watcher, bool unregistered)
    {
        EXPECT_CALL(mock_mirroring_watcher, is_unregistered()).WillOnce(Return(unregistered));
    }

    void expect_mirroring_watcher_register(MockMirroringWatcher& mock_mirroring_watcher, int r)
    {
        EXPECT_CALL(mock_mirroring_watcher, register_watch(_)).WillOnce(CompleteContext(r));
    }

    void expect_mirroring_watcher_unregister(MockMirroringWatcher& mock_mirroring_watcher, int r)
    {
        EXPECT_CALL(mock_mirroring_watcher, unregister_watch(_)).WillOnce(CompleteContext(r));
    }

    void expect_refresh_images(MockRefreshImagesRequest& request, const ImageIds& image_ids, int r)
    {
        EXPECT_CALL(request, send()).WillOnce(Invoke([&request, image_ids, r]() {
            *request.image_ids = image_ids;
            request.on_finish->complete(r);
        }));
    }

    void expect_listener_handle_update(MockListener& mock_listener, const std::string& mirror_uuid,
                                       const ImageIds& added_image_ids, const ImageIds& removed_image_ids)
    {
        EXPECT_CALL(mock_listener, mock_handle_update(mirror_uuid, added_image_ids, removed_image_ids))
    ...
    bool wait_for_update(uint32_t count)
    {
        ...
        if (m_cond.wait_for(locker, 10s, [count, this] { return m_update_count >= count; })) {
...
TEST_F(TestMockPoolWatcher, EmptyPool)
{
    ...
    MockPoolWatcher mock_pool_watcher(&mock_threads, m_remote_io_ctx, "remote uuid", mock_listener);
    C_SaferCond ctx;
...
TEST_F(TestMockPoolWatcher, NonEmptyPool)
{
    ...
    ImageIds image_ids{{"global id 1", "remote id 1"}, {"global id 2", "remote id 2"}};
    MockRefreshImagesRequest mock_refresh_images_request;
...
TEST_F(TestMockPoolWatcher, NotifyDuringRefresh)
{
    ...
    EXPECT_CALL(mock_refresh_images_request, send())
        .WillOnce(Invoke([this, &mock_refresh_images_request, &image_ids, &refresh_sent]() {
    ...
    MockListener mock_listener(this);
    image_ids = {{"global id 1", "remote id 1a"}, {"global id 3", "remote id 3"}};
    expect_listener_handle_update(mock_listener, "remote uuid", image_ids, {});
...
TEST_F(TestMockPoolWatcher, Notify)
{
    ...
    EXPECT_CALL(*mock_threads.work_queue, queue(_, _)).WillOnce(Invoke([this](Context* ctx, int r) {
        m_threads->work_queue->queue(ctx, r);
    }));
    ...
    EXPECT_CALL(*mock_threads.work_queue, queue(_, _)).WillOnce(Invoke([this, &notify_ctx](Context* ctx, int r) {
        std::lock_guard locker{m_lock};
...
TEST_F(TestMockPoolWatcher, Rewatch)
{
    ...
    expect_listener_handle_update(mock_listener, "remote uuid", {{"global id", "image id"}}, {});
...
TEST_F(TestMockPoolWatcher, DeferredRefresh)
{
    ...
    EXPECT_CALL(mock_refresh_images_request, send()).WillOnce(Invoke([&mock_refresh_images_request]() {
