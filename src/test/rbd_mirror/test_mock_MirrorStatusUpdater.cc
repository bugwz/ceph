...
template<> struct MirrorStatusWatcher<librbd::MockTestImageCtx>
{
    static MirrorStatusWatcher* s_instance;
    static MirrorStatusWatcher* create(librados::IoCtx& io_ctx, MockContextWQ* mock_context_wq)
    {
    ...
    MirrorStatusWatcher() { s_instance = this; }
};
...
public:
    ...
    typedef std::map<std::string, cls::rbd::MirrorImageSiteStatus> MirrorImageSiteStatuses;
    ...
    {
        EXPECT_CALL(*m_mock_threads->timer, cancel_event(_)).WillOnce(Invoke([](Context* ctx) {
            delete ctx;
            return false;
        }));
    }
    ...
    {
        EXPECT_CALL(*m_mock_threads->work_queue, queue(_, _)).WillOnce(Invoke([this, async](Context* ctx, int r) {
            if (async) {
    ...
    void expect_mirror_status_watcher_init(MockMirrorStatusWatcher& mock_mirror_status_watcher, int r)
    {
        EXPECT_CALL(*mock_mirror_status_watcher.s_instance, init(_)).WillOnce(Invoke([this, r](Context* ctx) {
            m_threads->work_queue->queue(ctx, r);
        }));
    }

    void expect_mirror_status_watcher_shut_down(MockMirrorStatusWatcher& mock_mirror_status_watcher, int r)
    {
        EXPECT_CALL(*mock_mirror_status_watcher.s_instance, shut_down(_)).WillOnce(Invoke([this, r](Context* ctx) {
            m_threads->work_queue->queue(ctx, r);
        }));
    }

    void expect_mirror_status_update(const std::string& global_image_id,
                                     const cls::rbd::MirrorImageSiteStatus& mirror_image_status, int r)
    {
        EXPECT_CALL(*m_mock_local_io_ctx,
                    exec(RBD_MIRRORING, _, StrEq("rbd"), StrEq("mirror_image_status_set"), _, _, _, _))
            .WillOnce(WithArg<4>(Invoke([r, global_image_id, mirror_image_status](bufferlist& in_bl) {
    ...
    void expect_mirror_status_update(const MirrorImageSiteStatuses& mirror_image_site_statuses,
                                     const std::string& mirror_uuid, int r)
    {
        EXPECT_CALL(*m_mock_local_io_ctx, aio_operate(_, _, _, _, _, _)).WillOnce(Invoke([this](auto&&... args) {
            int r = m_mock_local_io_ctx->do_aio_operate(decltype(args)(args)...);
            m_mock_local_io_ctx->aio_flush();
            return r;
        }));

        for (auto [global_image_id, mirror_image_status] : mirror_image_site_statuses) {
    ...
    {
        EXPECT_CALL(*m_mock_local_io_ctx,
                    exec(RBD_MIRRORING, _, StrEq("rbd"), StrEq("mirror_image_status_remove"), _, _, _, _))
            .WillOnce(WithArg<4>(Invoke([r, global_image_id](bufferlist& in_bl) {
                auto bl_it = in_bl.cbegin();
                std::string decode_global_image_id;
                decode(decode_global_image_id, bl_it);
                EXPECT_EQ(global_image_id, decode_global_image_id);
    ...
    void expect_mirror_status_removes(const std::set<std::string>& mirror_images, int r)
    {
        EXPECT_CALL(*m_mock_local_io_ctx, aio_operate(_, _, _, _, _, _)).WillOnce(Invoke([this](auto&&... args) {
            int r = m_mock_local_io_ctx->do_aio_operate(decltype(args)(args)...);
            m_mock_local_io_ctx->aio_flush();
    ...
    void fire_timer_event(Context** timer_event, Context** update_task)
    {
        expect_timer_add_event(timer_event);

        // timer queues the update task
        EXPECT_CALL(*m_mock_threads->work_queue, queue(_, _))
            .WillOnce(WithArg<0>(Invoke([update_task](Context* ctx) mutable { *update_task = ctx; })));
    ...
    void init_mirror_status_updater(MockMirrorStatusUpdater& mock_mirror_status_updater,
                                    MockMirrorStatusWatcher& mock_mirror_status_watcher, Context** timer_event)
    {
        expect_timer_add_event(timer_event);
    ...
    void shut_down_mirror_status_updater(MockMirrorStatusUpdater& mock_mirror_status_updater,
...
{
    MockMirrorStatusUpdater mock_mirror_status_updater(m_local_io_ctx, m_mock_threads, "");
    MockMirrorStatusWatcher* mock_mirror_status_watcher = new MockMirrorStatusWatcher();

    Context* timer_event = nullptr;
    init_mirror_status_updater(mock_mirror_status_updater, *mock_mirror_status_watcher, &timer_event);

    shut_down_mirror_status_updater(mock_mirror_status_updater, *mock_mirror_status_watcher);
}
...
TEST_F(TestMockMirrorStatusUpdater, SmallBatch)
{
    ...
    for (auto i = 0; i < 100; ++i) {
        auto pair = mirror_image_site_statuses.emplace(stringify(i), cls::rbd::MirrorImageSiteStatus{});
        mock_mirror_status_updater.set_mirror_image_status(pair.first->first, pair.first->second, false);
    }
...
TEST_F(TestMockMirrorStatusUpdater, OverwriteStatus)
{
    ...
    mock_mirror_status_updater.set_mirror_image_status(
        "1", {"", cls::rbd::MIRROR_IMAGE_STATUS_STATE_REPLAYING, "description"}, false);

    Context* update_task = nullptr;
    fire_timer_event(&timer_event, &update_task);

    expect_mirror_status_update(
        {{"1", cls::rbd::MirrorImageSiteStatus{"", cls::rbd::MIRROR_IMAGE_STATUS_STATE_REPLAYING, "description"}}},
        "",
        0);
    update_task->complete(0);
...
TEST_F(TestMockMirrorStatusUpdater, OverwriteStatusInFlight)
{
    ...
    EXPECT_CALL(*m_mock_local_io_ctx, aio_operate(_, _, _, _, _, _))
        .WillOnce(Invoke([this, &mock_mirror_status_updater](auto&&... args) {
            mock_mirror_status_updater.set_mirror_image_status(
                "1", {"", cls::rbd::MIRROR_IMAGE_STATUS_STATE_REPLAYING, "description"}, true);
...
TEST_F(TestMockMirrorStatusUpdater, RemoveRefreshInFlightStatus)
{
    ...
    EXPECT_CALL(*m_mock_local_io_ctx, aio_operate(_, _, _, _, _, _))
        .WillOnce(Invoke([this, &mock_mirror_status_updater, &on_removed](auto&&... args) {
            mock_mirror_status_updater.remove_refresh_mirror_image_status("1", &on_removed);
...
TEST_F(TestMockMirrorStatusUpdater, ShutDownWhileUpdating)
{
    ...
    C_SaferCond on_shutdown;
    EXPECT_CALL(*m_mock_local_io_ctx, aio_operate(_, _, _, _, _, _))
        .WillOnce(Invoke([this, &mock_mirror_status_updater, &on_shutdown](auto&&... args) {
            mock_mirror_status_updater.shut_down(&on_shutdown);
...
{
    MockMirrorStatusUpdater mock_mirror_status_updater(m_local_io_ctx, m_mock_threads, "mirror uuid");
    MockMirrorStatusWatcher* mock_mirror_status_watcher = new MockMirrorStatusWatcher();
