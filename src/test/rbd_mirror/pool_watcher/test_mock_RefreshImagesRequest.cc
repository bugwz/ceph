...
public:
    ...
    void expect_mirror_image_list(librados::IoCtx& io_ctx, const std::map<std::string, std::string>& ids, int r)
    {
        bufferlist bl;
        encode(ids, bl);

        EXPECT_CALL(get_mock_io_ctx(io_ctx),
                    exec(RBD_MIRRORING, _, StrEq("rbd"), StrEq("mirror_image_list"), _, _, _, _))
...
TEST_F(TestMockPoolWatcherRefreshImagesRequest, Success)
{
    ...
    C_SaferCond ctx;
    ImageIds image_ids;
    MockRefreshImagesRequest* req = new MockRefreshImagesRequest(m_remote_io_ctx, &image_ids, &ctx);
...
TEST_F(TestMockPoolWatcherRefreshImagesRequest, LargeDirectory)
{
    ...
    for (uint32_t idx = 1; idx <= 1024; ++idx) {
        mirror_list.insert(std::make_pair("local id " + stringify(idx), "global id " + stringify(idx)));
        expected_image_ids.insert({{"global id " + stringify(idx), "local id " + stringify(idx)}});
    }
