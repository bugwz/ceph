...
TEST(LibRadosServicePP, RegisterEarly)
{
    ...
    ASSERT_EQ(0, cluster.service_daemon_register("laundry", name, {{"foo", "bar"}, {"this", "that"}}));
    ASSERT_EQ(-EEXIST, cluster.service_daemon_register("laundry", name, {{"foo", "bar"}, {"this", "that"}}));
...
TEST(LibRadosServicePP, Status)
{
    ...
    cluster.conf_parse_env(NULL);
    string name = string("pid") + stringify(getpid());
    ASSERT_EQ(-ENOTCONN, cluster.service_daemon_update_status({{"testing", "starting"}}));
    ASSERT_EQ(0, cluster.connect());
    ASSERT_EQ(0, cluster.service_daemon_register("laundry", name, {{"foo", "bar"}, {"this", "that"}}));
    for (int i = 0; i < 20; ++i) {
        ASSERT_EQ(0, cluster.service_daemon_update_status({{"testing", "running"}, {"count", stringify(i)}}));
        sleep(1);
    }
    cluster.shutdown();
}
...
TEST(LibRadosServicePP, Close)
{
    ...
    for (i = 0; i < tries; ++i) {
        ...
        std::this_thread::sleep_for(g_conf().get_val<std::chrono::seconds>("mgr_tick_period"));
        // make sure we are deregistered
        {
            ...
            ASSERT_EQ(0, cluster.mon_command("{\"prefix\": \"service dump\"}", inbl, &outbl, NULL));
