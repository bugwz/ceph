...
TEST_F(LibRadosMiscPP, LongLocatorPP)
{
    ...
    bl.append("content");
    int maxlen = g_conf()->osd_max_object_name_len;
    ioctx.locator_set_key(string((maxlen / 2), 'a'));
    ASSERT_EQ(0, ioctx.write(string("a").c_str(), bl, bl.length(), 0));
    ioctx.locator_set_key(string(maxlen - 1, 'a'));
    ASSERT_EQ(0, ioctx.write(string("a").c_str(), bl, bl.length(), 0));
    ioctx.locator_set_key(string(maxlen, 'a'));
    ASSERT_EQ(0, ioctx.write(string("a").c_str(), bl, bl.length(), 0));
    ioctx.locator_set_key(string(maxlen + 1, 'a'));
    ASSERT_EQ(-ENAMETOOLONG, ioctx.write(string("a").c_str(), bl, bl.length(), 0));
    ioctx.locator_set_key(string((maxlen * 2), 'a'));
    ASSERT_EQ(-ENAMETOOLONG, ioctx.write(string("a").c_str(), bl, bl.length(), 0));
}
...
TEST_F(LibRadosMiscPP, LongNSpacePP)
{
    ...
    ioctx.set_namespace(string((maxlen / 2), 'a'));
    ASSERT_EQ(0, ioctx.write(string("a").c_str(), bl, bl.length(), 0));
    ioctx.set_namespace(string(maxlen - 1, 'a'));
    ASSERT_EQ(0, ioctx.write(string("a").c_str(), bl, bl.length(), 0));
    ioctx.set_namespace(string(maxlen, 'a'));
    ASSERT_EQ(0, ioctx.write(string("a").c_str(), bl, bl.length(), 0));
    ioctx.set_namespace(string(maxlen + 1, 'a'));
    ASSERT_EQ(-ENAMETOOLONG, ioctx.write(string("a").c_str(), bl, bl.length(), 0));
    ioctx.set_namespace(string((maxlen * 2), 'a'));
...
TEST_F(LibRadosMiscPP, AioOperatePP)
{
    ...
    AioCompletion* my_completion = cluster.aio_create_completion((void*)&my_aio_complete, set_completion_complete);
...
TEST_F(LibRadosMiscPP, BigAttrPP)
{
    ...
    for (int i = 0; i < 1000; i++) {
        bl.clear();
        got.clear();
        bl.append(buffer::create(std::min<uint64_t>(g_conf()->osd_max_attr_size, 1024)));
        char n[10];
...
TEST_F(LibRadosMiscPP, CopyScrubPP)
{
    ...
    for (int i = 0; i < 100; ++i) x.append("barrrrrrrrrrrrrrrrrrrrrrrrrr");
    bl.append(buffer::create(g_conf()->osd_copyfrom_max_chunk * 3));
    bl.zero();
    bl.append("tail");
    bufferlist cbl;
    ...
    for (int i = 0; i < 1000; ++i) to_set[string("foo") + stringify(i)] = x;
    ...
    // deep scrub to ensure digests are in place
    {
        for (int i = 0; i < 10; ++i) {
            ostringstream ss;
            ss << "{\"prefix\": \"pg deep-scrub\", \"pgid\": \"" << ioctx.get_id() << "." << i << "\"}";
...
TEST_F(LibRadosMiscPP, WriteSamePP)
{
    ...
    ASSERT_EQ(-EINVAL, ioctx.writesame("ws", bl, bl.length() / 2, 0));
...
template<rados_checksum_type_t _type, typename AlgT, typename ValueT> class LibRadosChecksumParams
{
public:
    typedef AlgT alg_t;
...
typedef ::testing::Types<LibRadosChecksumParams<LIBRADOS_CHECKSUM_TYPE_XXHASH32, Checksummer::xxhash32, ceph_le32>,
                         LibRadosChecksumParams<LIBRADOS_CHECKSUM_TYPE_XXHASH64, Checksummer::xxhash64, ceph_le64>,
                         LibRadosChecksumParams<LIBRADOS_CHECKSUM_TYPE_CRC32C, Checksummer::crc32c, ceph_le32> >
...
TYPED_TEST(LibRadosChecksum, Subset)
{
    ...
    for (uint32_t i = 0; i < csum_count; ++i) {
        op.checksum(
            TestFixture::type, init_value_bl, i * chunk_size, chunk_size, 0, &checksum_bls[i], &checksum_rvals[i]);
    }
    ...
    for (uint32_t i = 0; i < csum_count; ++i) {
        ...
        bufferptr expected_value_bp =
            buffer::create_static(sizeof(expected_value), reinterpret_cast<char*>(&expected_value));
        Checksummer::template calculate<typename TestFixture::alg_t>(
            init_value, chunk_size, 0, chunk_size, content_sub_bl, &expected_value_bp);
...
TYPED_TEST(LibRadosChecksum, Chunked)
{
    ...
    ObjectReadOperation op;
    op.checksum(
        TestFixture::type, init_value_bl, 0, this->content_bl.length(), chunk_size, &checksum_bl, &checksum_rval);
    ASSERT_EQ(0, this->ioctx.operate("foo", &op, NULL));
    ...
    bufferptr expected_values_bp = buffer::create_static(csum_count * sizeof(typename TestFixture::value_t),
                                                         reinterpret_cast<char*>(&expected_values[0]));

    Checksummer::template calculate<typename TestFixture::alg_t>(
        init_value, chunk_size, 0, this->content_bl.length(), this->content_bl, &expected_values_bp);
...
TEST_F(LibRadosMiscPP, Applications)
{
    ...
    string outs;
    ASSERT_EQ(0, cluster.mon_command("{\"prefix\": \"osd dump\"}", inbl, &outbl, &outs));
    ASSERT_LT(0u, outbl.length());
    ASSERT_LE(0u, outs.length());
    if (!std::regex_search(outbl.to_str(), std::regex("require_osd_release [l-z]"))) {
        std::cout << "SKIPPING";
        return;
    }
...
TEST_F(LibRadosMiscPP, MinCompatClient)
{
    ...
    ASSERT_EQ(0, cluster.get_min_compatible_client(&min_compat_client, &require_min_compat_client));
...
TEST_F(LibRadosMiscPP, Conf)
{
    ...
    auto restore_setting = make_scope_guard([&] { cluster.conf_set(option, original.c_str()); });
