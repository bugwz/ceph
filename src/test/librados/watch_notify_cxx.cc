...
public:
    ...
    {
        std::cout << __func__ << " cookie " << cookie << " notify_id " << notify_id << " notifier_gid " << notifier_gid
                  << std::endl;
        notify->notify_bl = bl;
...
TEST_P(LibRadosWatchNotifyPP, WatchNotify3)
{
    ...
    for (std::list<obj_watch_t>::iterator it = watches.begin(); it != watches.end(); ++it) {
...
INSTANTIATE_TEST_SUITE_P(LibRadosWatchNotifyPPTests, LibRadosWatchNotifyPP, ::testing::Values("", "cache"));
