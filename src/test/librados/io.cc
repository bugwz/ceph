...
TEST_F(LibRadosIo, ReadTimeout)
{
...

    {
        ...
        ASSERT_EQ(0,
                  rados_conf_set(
                      cluster, "rados_osd_op_timeout", "1"));   // use any small value that will result in a timeout
...
TEST_F(LibRadosIo, Checksum)
{
    ...
    uint32_t expected_crc = ceph_crc32c(-1, reinterpret_cast<const uint8_t*>(buf), sizeof(buf));
    ceph_le32 init_value(-1);
    ceph_le32 crc[2];
    ASSERT_EQ(0,
              rados_checksum(ioctx,
                             "foo",
...
TEST_F(LibRadosIo, XattrsRoundTrip)
{
    ...
    ASSERT_EQ((int)sizeof(attr1_buf), rados_getxattr(ioctx, "foo", attr1, buf, sizeof(buf)));
...
TEST_F(LibRadosIo, RmXattr)
{
    ...
    ASSERT_EQ(0, rados_setxattr(ioctx, "foo_rmxattr", attr2, attr2_buf, sizeof(attr2_buf)));
