...
void flush_evict_all(librados::Rados& cluster, librados::IoCtx& cache_ioctx)
{
    ...
    for (NObjectIterator it = cache_ioctx.nobjects_begin(); it != cache_ioctx.nobjects_end(); ++it) {
        ...
        cache_ioctx.set_namespace(it->get_nspace());
        {
            ...
            cache_ioctx.aio_operate(it->get_oid(), completion, &op, librados::OPERATION_IGNORE_OVERLAY, NULL);
...
void manifest_set_chunk(Rados& cluster, librados::IoCtx& src_ioctx, librados::IoCtx& tgt_ioctx, uint64_t src_offset,
                        uint64_t length, std::string src_oid, std::string tgt_oid)
{
    ObjectReadOperation op;
    op.set_chunk(src_offset, length, src_ioctx, src_oid, 0, CEPH_OSD_OP_FLAG_WITH_REFERENCE);
    librados::AioCompletion* completion = cluster.aio_create_completion();
    ASSERT_EQ(0, tgt_ioctx.aio_operate(tgt_oid, completion, &op, librados::OPERATION_IGNORE_CACHE, NULL));
    completion->wait_for_complete();
...
void check_fp_oid_refcount(librados::IoCtx& ioctx, std::string foid, uint64_t count, std::string fp_algo = NULL)
{
    bufferlist t;
...
void is_intended_refcount_state(librados::IoCtx& src_ioctx, std::string src_oid, librados::IoCtx& dst_ioctx,
                                std::string dst_oid, int expected_refcount)
{
...
protected:
    ...
    static void TearDownTestCase() { ASSERT_EQ(0, destroy_one_pool_pp(pool_name, s_cluster)); }
    ...
    void TearDown() override
    {
        ...
        ASSERT_EQ(0,
                  cluster.mon_command(
                      "{\"prefix\": \"osd tier remove-overlay\", \"pool\": \"" + pool_name + "\"}", inbl, NULL, NULL));
        ASSERT_EQ(0,
                  cluster.mon_command("{\"prefix\": \"osd tier remove\", \"pool\": \"" + pool_name +
...
TEST_F(LibRadosTwoPoolsPP, Overlay)
{
    ...
    bufferlist inbl;
    ASSERT_EQ(0,
              cluster.mon_command("{\"prefix\": \"osd tier add\", \"pool\": \"" + pool_name + "\", \"tierpool\": \"" +
                                      cache_pool_name + "\", \"force_nonempty\": \"--force-nonempty\" }",
                                  inbl,
                                  NULL,
                                  NULL));
    ASSERT_EQ(0,
              cluster.mon_command("{\"prefix\": \"osd tier set-overlay\", \"pool\": \"" + pool_name +
    ...
    // unless we say otherwise
    {
        ...
        op.read(0, 1, &bl, NULL);
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, ioctx.aio_operate("foo", completion, &op, librados::OPERATION_IGNORE_OVERLAY, NULL));
        completion->wait_for_complete();
        ASSERT_EQ(0, completion->get_return_value());
        completion->release();
        ASSERT_EQ('b', bl[0]);
...
TEST_F(LibRadosTwoPoolsPP, Promote)
{
    ...
    ASSERT_EQ(0,
              cluster.mon_command("{\"prefix\": \"osd tier set-overlay\", \"pool\": \"" + pool_name +
                                  ...
                                  NULL));
    ASSERT_EQ(0,
              cluster.mon_command("{\"prefix\": \"osd tier cache-mode\", \"pool\": \"" + cache_pool_name +
                                      "\", \"mode\": \"writeback\"}",
                                  inbl,
                                  NULL,
                                  NULL));
...
TEST_F(LibRadosTwoPoolsPP, PromoteSnapScrub)
{
    ...
    // missing clones in the cache tier).
    {
        ...
        for (int i = 0; i < 10; ++i) {
            do {
                ostringstream ss;
                ss << "{\"prefix\": \"pg scrub\", \"pgid\": \"" << cache_ioctx.get_id() << "." << i << "\"}";
...
TEST_F(LibRadosTwoPoolsPP, Whiteout)
{
    ...
    ASSERT_EQ(-ENOENT, ioctx.remove("foo"));
    {
        ...
        op.remove();
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, ioctx.aio_operate("bar", completion, &op, librados::OPERATION_IGNORE_CACHE));
        completion->wait_for_complete();
        ASSERT_EQ(0, completion->get_return_value());
        completion->release();
...
TEST_F(LibRadosTwoPoolsPP, Evict)
{
    ...
    // evict the pinned object with -EPERM
    {
        ...
        op.cache_evict();
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, cache_ioctx.aio_operate("foo", completion, &op, librados::OPERATION_IGNORE_CACHE, NULL));
        completion->wait_for_complete();
    ...
    // flush
    {
        ...
        op.cache_flush();
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, cache_ioctx.aio_operate("foo", completion, &op, librados::OPERATION_IGNORE_OVERLAY, NULL));
        completion->wait_for_complete();
        ASSERT_EQ(0, completion->get_return_value());
        completion->release();
    }
    ...
    }
    {
        ...
        op.cache_evict();
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, cache_ioctx.aio_operate("bar", completion, &op, librados::OPERATION_IGNORE_CACHE, NULL));
        completion->wait_for_complete();
...
TEST_F(LibRadosTwoPoolsPP, EvictSnap)
{
    ...
    // evict bam
    {
        ...
        op.cache_evict();
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, cache_ioctx.aio_operate("bam", completion, &op, librados::OPERATION_IGNORE_CACHE, NULL));
        completion->wait_for_complete();
        ASSERT_EQ(0, completion->get_return_value());
        completion->release();
    }
    ...
    // evict foo snap
    {
        ...
        op.cache_evict();
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, ioctx.aio_operate("foo", completion, &op, librados::OPERATION_IGNORE_CACHE, NULL));
        completion->wait_for_complete();
        ASSERT_EQ(0, completion->get_return_value());
        completion->release();
    }
    ...
    ioctx.snap_set_read(librados::SNAP_HEAD);
    {
        ...
        op.cache_evict();
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, ioctx.aio_operate("bar", completion, &op, librados::OPERATION_IGNORE_CACHE, NULL));
        completion->wait_for_complete();
        ASSERT_EQ(-EBUSY, completion->get_return_value());
        completion->release();
    }
...
TEST_F(LibRadosTwoPoolsPP, ListSnap)
{
    ...
    ioctx.snap_set_read(CEPH_SNAPDIR);
    {
        ...
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, ioctx.aio_operate("foo", completion, &op, 0, NULL));
        completion->wait_for_complete();
        ASSERT_EQ(0, snap_ret);
        ASSERT_LT(0u, snap_set.clones.size());
        for (vector<librados::clone_info_t>::const_iterator r = snap_set.clones.begin(); r != snap_set.clones.end();
             ++r) {
    ...
    // flush the pinned object with -EPERM
    {
        ...
        ASSERT_EQ(
            ...
            cache_ioctx.aio_operate(
                "foo", completion, &op, librados::OPERATION_IGNORE_OVERLAY | librados::OPERATION_SKIPRWLOCKS, NULL));
        completion->wait_for_complete();
        ASSERT_EQ(-EPERM, completion->get_return_value());
        completion->release();
    }
...
TEST_F(LibRadosTierPP, FlushWriteRaces)
{
    ...
    do {
        ...
        // try-flush + write
        {
            ...
            librados::AioCompletion* completion = cluster.aio_create_completion();
            ASSERT_EQ(0,
                      cache_ioctx.aio_operate("foo",
                                              completion,
                                              &op,
                                              librados::OPERATION_IGNORE_OVERLAY | librados::OPERATION_SKIPRWLOCKS,
                                              NULL));
...
TEST_F(LibRadosTwoPoolsPP, FlushTryFlushRaces)
{
    ...
    // flush + flush
    {
        ...
        op2.cache_flush();
        librados::AioCompletion* completion2 = cluster.aio_create_completion();
        ASSERT_EQ(0, cache_ioctx.aio_operate("foo", completion2, &op2, librados::OPERATION_IGNORE_OVERLAY, NULL));
    ...
    // flush + try-flush
    {
        ...
        ASSERT_EQ(
            0,
            cache_ioctx.aio_operate(
                "foo", completion2, &op2, librados::OPERATION_IGNORE_OVERLAY | librados::OPERATION_SKIPRWLOCKS, NULL));
...
{
    return string("{\"prefix\": \"osd pool set\",\"pool\":\"") + pool + string("\",\"var\": \"") + var +
...
{
    return string("{\"prefix\": \"osd pool set\",\"pool\":\"") + pool + string("\",\"var\": \"") + var +
           string("\",\"val\": \"") + stringify(val) + string("\"}");
}
...
TEST_F(LibRadosTwoPoolsPP, HitSetRead)
{
    ...
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_count", 2), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_period", 600), inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_type", "explicit_object"), inbl, NULL, NULL));
    ...
    while (true) {
        ...
        hobject_t oid(sobject_t(name, CEPH_NOSNAP), "", hash, cluster.pool_lookup(cache_pool_name.c_str()), "");
...
{
    bufferlist inbl;
    string cmd = string("{\"prefix\": \"osd pool get\",\"pool\":\"") + pool_name +
...
int make_hitset(Rados& cluster, librados::IoCtx& cache_ioctx, int num_pg, int num, std::map<int, HitSet>& hitsets,
                std::string& cache_pool_name)
{
    ...
    for (int i = 0; i < pg; ++i) {
        ...
        if (i == pg - 1) pg = _get_pg_num(cluster, cache_pool_name);
    }
    return pg;
}
...
TEST_F(LibRadosTwoPoolsPP, HitSetWrite)
{
    ...
    // enable hitset tracking for this pool
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_count", 8), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_period", 600), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_type", "explicit_hash"), inbl, NULL, NULL));
    ...
    for (int i = 0; i < num; ++i) {
        ...
        hobject_t oid(sobject_t(n, CEPH_NOSNAP), "", hash, cluster.pool_lookup(cache_pool_name.c_str()), "");
...
TEST_F(LibRadosTwoPoolsPP, HitSetTrim)
{
    ...
    // enable hitset tracking for this pool
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_count", count), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_period", period), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_type", "bloom"), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_fpp", ".01"), inbl, NULL, NULL));
...
TEST_F(LibRadosTwoPoolsPP, PromoteOn2ndRead)
{
    ...
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_type", "bloom"), inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.mon_command(set_pool_str(cache_pool_name, "min_read_recency_for_promote", 1), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_grade_decay_rate", 20), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_search_last_n", 1), inbl, NULL, NULL));
    ...
    while (true) {
        ...
        cout << "hrm, object is present in cache on attempt " << attempt << ", retrying" << std::endl;
    }
...
TEST_F(LibRadosTwoPoolsPP, CachePin)
{
    ...
    // verify the objects are present in the cache tier
    {
        ...
        for (uint32_t i = 0; i < 4; i++) {
            ASSERT_TRUE(it->get_oid() == string("foo") || it->get_oid() == string("bar") ||
                        it->get_oid() == string("baz") || it->get_oid() == string("bam"));
            ++it;
        }
        ASSERT_TRUE(it == cache_ioctx.nobjects_end());
    }
    ...
    ASSERT_EQ(0,
              cluster.mon_command(set_pool_str(cache_pool_name, "min_read_recency_for_promote", 1), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "target_max_objects", 1), inbl, NULL, NULL));
    ...
    while (true) {
        ...
        if (count == 2) {
            ASSERT_TRUE(it->get_oid() == string("foo") || it->get_oid() == string("baz"));
            break;
        }
...
TEST_F(LibRadosTwoPoolsPP, ManifestRefRead)
{
    ...
    // set-chunk
    {
        ...
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, ioctx.aio_operate("foo-chunk", completion, &op, librados::OPERATION_IGNORE_CACHE, NULL));
        completion->wait_for_complete();
        ASSERT_EQ(0, completion->get_return_value());
        completion->release();
    }
...
TEST_F(LibRadosTwoPoolsPP, ManifestDedupRefRead)
{
    ...
    bufferlist inbl;
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(pool_name, "fingerprint_algorithm", "sha1"), inbl, NULL, NULL));
    cluster.wait_for_latest_osdmap();
    string tgt_oid;
...
TEST_F(LibRadosTwoPoolsPP, ManifestCheckRefcountWhenModification)
{
    ...
    string er_fp_oid, hi_fp_oid, HI_fp_oid, ai_fp_oid, bi_fp_oid, Er_fp_oid, Hi_fp_oid, Si_fp_oid;
...
TEST_F(LibRadosTwoPoolsPP, ManifestSnapSizeMismatch)
{
    ...
    ASSERT_EQ(0, cache_ioctx.selfmanaged_snap_set_write_ctx(my_snaps[0], my_snaps));
    ...
    // scrub
    {
        for (int tries = 0; tries < 5; ++tries) {
            bufferlist inbl;
            ostringstream ss;
            ss << "{\"prefix\": \"pg deep-scrub\", \"pgid\": \"" << cache_ioctx.get_id() << "." << std::hex << hash
               << "\"}";
            int r = cluster.mon_command(ss.str(), inbl, NULL, NULL);
...
TEST_F(LibRadosTwoPoolsPP, DedupFlushRead)
{
    ...
    bufferlist inbl;
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "fingerprint_algorithm", "sha1"), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "dedup_tier", pool_name), inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.mon_command(set_pool_str(cache_pool_name, "dedup_chunk_algorithm", "fastcdc"), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "dedup_cdc_chunk_size", 1024), inbl, NULL, NULL));
    ...
    {
        ObjectReadOperation op;
        op.set_chunk(0, 2, ioctx, "bar-chunk", 0, CEPH_OSD_OP_FLAG_WITH_REFERENCE);
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, cache_ioctx.aio_operate("foo-chunk", completion, &op, librados::OPERATION_IGNORE_CACHE, NULL));
        completion->wait_for_complete();
        ASSERT_EQ(0, completion->get_return_value());
        completion->release();
    }
    ...
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "dedup_cdc_chunk_size", 512), inbl, NULL, NULL));
    cluster.wait_for_latest_osdmap();
    ...
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "dedup_cdc_chunk_size", 16384), inbl, NULL, NULL));
    cluster.wait_for_latest_osdmap();
...
TEST_F(LibRadosTwoPoolsPP, TierFlushDuringFlush)
{
    ...
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "fingerprint_algorithm", "sha1"), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "dedup_tier", temp_pool_name), inbl, NULL, NULL));
    ASSERT_EQ(0,
              cluster.mon_command(set_pool_str(cache_pool_name, "dedup_chunk_algorithm", "fastcdc"), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "dedup_cdc_chunk_size", 1024), inbl, NULL, NULL));
...
protected:
    ...
    static void TearDownTestCase() { ASSERT_EQ(0, destroy_one_ec_pool_pp(pool_name, s_cluster)); }
    static std::string cache_pool_name;
...
TEST_F(LibRadosTwoPoolsECPP, PromoteSnap)
{
    ...
    // This test requires cache tier and base tier to have the same pg_num/pgp_num
    {
        for (int tries = 0; tries < 5; ++tries) {
            ...
            ostringstream ss;
            ss << "{\"prefix\": \"pg scrub\", \"pgid\": \"" << cache_ioctx.get_id() << "." << hash << "\"}";
            int r = cluster.mon_command(ss.str(), inbl, NULL, NULL);
            if (r == -EAGAIN || r == -ENOENT) {   // in case mgr osdmap is a bit stale
                sleep(5);
                continue;
            }
            ASSERT_EQ(0, r);
...
TEST_F(LibRadosTwoPoolsECPP, FailedFlush)
{
    ...
    // get omap
    {
        ...
        if (!completed) {
            ...
            ASSERT_TRUE(
                completed);   // in fact we are locked forever at test case shutdown unless fix for
                              // http://tracker.ceph.com/issues/14511 is applied. Seems there is no workaround for that
        }
        completion->release();
    }
...
TEST_F(LibRadosTierECPP, CallForcesPromote)
{
    ...
    ASSERT_EQ(0,
              cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_type", "explicit_object"), inbl, NULL, NULL));
    ASSERT_EQ(
        0, cluster.mon_command(set_pool_str(cache_pool_name, "min_read_recency_for_promote", "4"), inbl, NULL, NULL));
...
TEST_F(LibRadosTwoPoolsECPP, TrySetDedupTier)
{
    ...
    ASSERT_EQ(-EOPNOTSUPP,
              cluster.mon_command(set_pool_str(pool_name, "dedup_tier", cache_pool_name), inbl, NULL, NULL));
}
...
TEST_F(LibRadosTwoPoolsPP, PropagateBaseTierError)
{
    ...
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_type", "bloom"), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_count", 1), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "hit_set_period", 600), inbl, NULL, NULL));
    ASSERT_EQ(0, cluster.mon_command(set_pool_str(cache_pool_name, "target_max_objects", 250), inbl, NULL, NULL));
...
TEST_F(LibRadosTwoPoolsPP, HelloWriteReturn)
{
    ...
    ASSERT_EQ(
        0,
        cluster.mon_command(set_pool_str(cache_pool_name, "min_read_recency_for_promote", "10000"), inbl, NULL, NULL));
    ...
    // this *will* return data due to the RETURNVEC flag
    {
        ...
        librados::AioCompletion* completion = cluster.aio_create_completion();
        ASSERT_EQ(0, ioctx.aio_operate("foo", completion, &o, librados::OPERATION_RETURNVEC));
        completion->wait_for_complete();
