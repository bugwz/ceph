...
TEST(LibRadosPools, PoolList)
{
    ...
    while (buf[0] != '\0') {
        ...
        if (!firstlen) firstlen = strlen(buf) + 1;
        buf += strlen(buf) + 1;
    }
...
TEST(LibRadosPools, PoolCreateWithCrushRule)
{
    ...
    ASSERT_EQ(0, rados_pool_create_with_crush_rule(cluster, pool2_name.c_str(), 0));
...
TEST(LibRadosPools, PoolGetBaseTier)
{
    ...
    std::string cmdstr = "{\"prefix\": \"osd tier add\", \"pool\": \"" + pool_name + "\", \"tierpool\":\"" +
                         tier_pool_name + "\", \"force_nonempty\":\"\"}";
    char* cmd[1];
    cmd[0] = (char*)cmdstr.c_str();
    ASSERT_EQ(0, rados_mon_command(cluster, (const char**)cmd, 1, "", 0, NULL, 0, NULL, 0));

    cmdstr = "{\"prefix\": \"osd tier cache-mode\", \"pool\": \"" + tier_pool_name + "\", \"mode\":\"readonly\"," +
    ...
    cmdstr =
        "{\"prefix\": \"osd tier remove\", \"pool\": \"" + pool_name + "\", \"tierpool\":\"" + tier_pool_name + "\"}";
    cmd[0] = (char*)cmdstr.c_str();
    ASSERT_EQ(0, rados_mon_command(cluster, (const char**)cmd, 1, "", 0, NULL, 0, NULL, 0));
