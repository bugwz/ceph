...
{
    if (cleanup) cleanup_all_objects(ioctx);
    ioctx.close();
}
...
void RadosTestPPNS::cleanup_all_objects(librados::IoCtx ioctx)
{
    ...
    for (NObjectIterator it = ioctx.nobjects_begin(); it != ioctx.nobjects_end(); ++it) {
...
void RadosTestParamPPNS::TearDownTestCase()
{
    if (cache_pool_name.length()) {
        ...
        bufferlist inbl;
        ASSERT_EQ(0,
                  s_cluster.mon_command(
                      "{\"prefix\": \"osd tier remove-overlay\", \"pool\": \"" + pool_name + "\"}", inbl, NULL, NULL));
        ASSERT_EQ(0,
                  s_cluster.mon_command("{\"prefix\": \"osd tier remove\", \"pool\": \"" + pool_name +
                                            "\", \"tierpool\": \"" + cache_pool_name + "\"}",
                                        inbl,
                                        NULL,
                                        NULL));
        ASSERT_EQ(0,
                  s_cluster.mon_command("{\"prefix\": \"osd pool delete\", \"pool\": \"" + cache_pool_name +
                                            "\", \"pool2\": \"" + cache_pool_name +
...
{
    if (!is_crimson_cluster() && strcmp(GetParam(), "cache") == 0 && cache_pool_name.empty()) {
        auto pool_prefix = fmt::format("{}_", ::testing::UnitTest::GetInstance()->current_test_case()->name());
        cache_pool_name = get_temp_pool_name();
        bufferlist inbl;
        ASSERT_EQ(0,
                  cluster.mon_command(
                      "{\"prefix\": \"osd pool create\", \"pool\": \"" + cache_pool_name + "\", \"pg_num\": 4}",
                      inbl,
                      NULL,
                      NULL));
        ASSERT_EQ(
...
void RadosTestPP::cleanup_namespace(librados::IoCtx ioctx, std::string ns)
{
    ...
    while (--tries) {
        ...
        for (NObjectIterator it = ioctx.nobjects_begin(); it != ioctx.nobjects_end(); ++it) {
            ...
            ASSERT_EQ(0, ioctx.aio_operate(it->get_oid(), completion, &op, librados::OPERATION_IGNORE_CACHE));
            completion->wait_for_complete();
            if (completion->get_return_value() == -ENOENT) {
                ++got_enoent;
                std::cout << " got ENOENT removing " << it->get_oid() << " in ns " << ns << std::endl;
            }
            else {
        ...
        std::cout << " got ENOENT on " << got_enoent << " objects, waiting a bit for snap"
                  << " trimming before retrying " << tries << " more times..." << std::endl;
        sleep(1);
    }
    if (tries == 0) {
        std::cout << "failed to clean up; probably need to scrub purged_snaps." << std::endl;
    }
}
