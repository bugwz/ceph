...
TEST_F(LibRadosList, ListObjectsZeroInName)
{
    ...
    size_t entry_size;
    bool foundit = false;
    while (rados_nobjects_list_next2(ctx, &entry, NULL, NULL, &entry_size, NULL, NULL) != -ENOENT) {
        foundit = true;
...
static void check_list(std::set<std::string>& myset, rados_list_ctx_t& ctx, const std::string& check_nspace)
{
...
TEST_F(LibRadosList, ListObjectsNS)
{
    ...
    ASSERT_EQ(static_cast<int>(strlen("ns2")), rados_ioctx_get_namespace(ioctx, nspace, sizeof(nspace)));
...
TEST_F(LibRadosListNP, ListObjectsError)
{
    ...
    // ASSERT_EQ(0, rados_pool_delete(cluster, pool_name.c_str()));
    {
        char *buf, *st;
        size_t buflen, stlen;
        string c = "{\"prefix\":\"osd pool rm\",\"pool\": \"" + pool_name + "\",\"pool2\":\"" + pool_name +
...
TEST_F(LibRadosList, EnumerateObjectsSplit)
{
    ...
    for (unsigned n = 0; n < m; ++n) {
        ...
        std::cout << "split " << n << "/" << m << " -> " << *(hobject_t*)shard_start << " " << *(hobject_t*)shard_end
                  << std::endl;
