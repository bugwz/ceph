...
void get_snaptrim_stats(json_spirit::Object& pg_dump, int* objs_trimmed, double* trim_duration)
{
    // pg_map
...
TEST_F(LibRadosSnapshotStatsSelfManaged, SnaptrimStats)
{
    ...
    for (int snap = 0; snap < 1; ++snap) {
        ...
        ASSERT_EQ(0, rados_ioctx_selfmanaged_snap_set_write_ctx(ioctx, my_snaps[0], &my_snaps[0], my_snaps.size()));
...
TEST_F(LibRadosSnapshotStatsSelfManagedEC, SnaptrimStats)
{
    ...
    do {
        ...
        string c = string("{\"prefix\": \"pg dump\",\"format\":\"json\"}");
        const char* cmd = c.c_str();
        ASSERT_EQ(0, rados_mon_command(cluster, (const char**)&cmd, 1, 0, 0, &buf, &buflen, &st, &stlen));
        string outstr(buf, buflen);
        json_spirit::Value v;
