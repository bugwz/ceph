...
protected:
    ...
    static void watch_notify2_test_cb(void* arg, uint64_t notify_id, uint64_t cookie, uint64_t notifier_gid, void* data,
                                      size_t data_len);
...
void LibRadosWatchNotify::watch_notify2_test_cb(void* arg, uint64_t notify_id, uint64_t cookie, uint64_t notifier_gid,
                                                void* data, size_t data_len)
{
    std::cout << __func__ << " from " << notifier_gid << " notify_id " << notify_id << " cookie " << cookie
              << std::endl;
    ...
    thiz->notify_err = 0;
    rados_notify_ack(thiz->notify_io, thiz->notify_oid, notify_id, cookie, "reply", 5);
}

void LibRadosWatchNotify::watch_notify2_test_errcb(void* arg, uint64_t cookie, int err)
{
...
void LibRadosWatchNotify::watch_notify2_test_errcb_reconnect(void* arg, uint64_t cookie, int err)
{
    std::cout << __func__ << " cookie " << cookie << " err " << err << std::endl;
    ceph_assert(cookie > 1000);
    auto thiz = reinterpret_cast<LibRadosWatchNotify*>(arg);
    ...
    thiz->notify_err = rados_watch2(
        thiz->ioctx, thiz->notify_oid, &cookie, watch_notify2_test_cb, watch_notify2_test_errcb_reconnect, thiz);
...
void LibRadosWatchNotify::watch_notify2_test_errcb_aio_reconnect(void* arg, uint64_t cookie, int err)
{
    std::cout << __func__ << " cookie " << cookie << " err " << err << std::endl;
    ceph_assert(cookie > 1000);
    auto thiz = reinterpret_cast<LibRadosWatchNotify*>(arg);
    ...
    thiz->notify_cookies.erase(cookie);   // delete old cookie
    thiz->notify_err = rados_aio_watch(thiz->ioctx,
                                       thiz->notify_oid,
                                       thiz->notify_comp,
                                       &cookie,
                                       watch_notify2_test_cb,
                                       watch_notify2_test_errcb_aio_reconnect,
                                       thiz);
...
TEST_F(LibRadosWatchNotify, WatchNotify)
{
    ...
    uint64_t handle;
    ASSERT_EQ(0, rados_watch(ioctx, "foo", 0, &handle, watch_notify_test_cb, NULL));
    ...
    // when dne ...
    ASSERT_EQ(-ENOENT, rados_watch(ioctx, "dne", 0, &handle, watch_notify_test_cb, NULL));
...
TEST_F(LibRadosWatchNotify, Watch2Delete)
{
    ...
    uint64_t handle;
    ASSERT_EQ(0, rados_watch2(ioctx, notify_oid, &handle, watch_notify2_test_cb, watch_notify2_test_errcb, this));
    ASSERT_EQ(0, rados_remove(ioctx, notify_oid));
    int left = 300;
    std::cout << "waiting up to " << left << " for disconnect notification ..." << std::endl;
...
TEST_F(LibRadosWatchNotify, AioWatchDelete)
{
    ...
    rados_aio_watch(ioctx, notify_oid, comp, &handle, watch_notify2_test_cb, watch_notify2_test_errcb, this);
...
TEST_F(LibRadosWatchNotify, WatchNotify2)
{
    ...
    uint64_t handle;
    ASSERT_EQ(
        0, rados_watch2(ioctx, notify_oid, &handle, watch_notify2_test_cb, watch_notify2_test_errcb_reconnect, this));
    ASSERT_GT(rados_watch_check(ioctx, handle), 0);
    char* reply_buf = 0;
    size_t reply_buf_len;
    ASSERT_EQ(0, rados_notify2(ioctx, notify_oid, "notify", 6, 300000, &reply_buf, &reply_buf_len));
    bufferlist reply;
    ...
    // should get zeroed.
    ASSERT_EQ(-ENOENT, rados_notify2(ioctx, "doesnotexist", "notify", 6, 300000, &reply_buf, &reply_buf_len));
...
TEST_F(LibRadosWatchNotify, AioWatchNotify2)
{
    ...
    rados_aio_watch(
        ioctx, notify_oid, notify_comp, &handle, watch_notify2_test_cb, watch_notify2_test_errcb_aio_reconnect, this);
...
TEST_F(LibRadosWatchNotify, AioNotify)
{
    ...
    rados_completion_t comp;
    ASSERT_EQ(0, rados_aio_create_completion2(nullptr, nullptr, &comp));
    ASSERT_EQ(0, rados_aio_notify(ioctx, "foo", comp, "notify", 6, 300000, &reply_buf, &reply_buf_len));
    ASSERT_EQ(0, rados_aio_wait_for_complete(comp));
    ASSERT_EQ(0, rados_aio_get_return_value(comp));
    rados_aio_release(comp);
    ...
    ASSERT_EQ(0, rados_decode_notify_response(reply_buf, reply_buf_len, &acks, &nr_acks, &timeouts, &nr_timeouts));
    ...
    // should get zeroed.
    ASSERT_EQ(0, rados_aio_create_completion2(nullptr, nullptr, &comp));
    ASSERT_EQ(0, rados_aio_notify(ioctx, "doesnotexist", comp, "notify", 6, 300000, &reply_buf, &reply_buf_len));
    ASSERT_EQ(0, rados_aio_wait_for_complete(comp));
    ASSERT_EQ(-ENOENT, rados_aio_get_return_value(comp));
    rados_aio_release(comp);
    ASSERT_EQ((char*)0, reply_buf);
...
TEST_F(LibRadosWatchNotify, WatchNotify2Multi)
{
    ...
    ASSERT_EQ(0, rados_watch2(ioctx, notify_oid, &handle1, watch_notify2_test_cb, watch_notify2_test_errcb, this));
    ASSERT_EQ(0, rados_watch2(ioctx, notify_oid, &handle2, watch_notify2_test_cb, watch_notify2_test_errcb, this));
...
TEST_F(LibRadosWatchNotify, Watch3Timeout)
{
    ...
    const uint32_t timeout = 4;
    {
        ...
        char conf[128];
        ASSERT_EQ(0, rados_conf_get(cluster, "ms_connection_idle_timeout", conf, sizeof(conf)));
        auto connection_idle_timeout = std::stoll(conf);
        ASSERT_LT(timeout, connection_idle_timeout);
    }
    ASSERT_EQ(0,
              rados_watch3(ioctx, notify_oid, &handle, watch_notify2_test_cb, watch_notify2_test_errcb, timeout, this));
    ...
    // watch.
    int left = 16 * timeout;
    std::cout << "waiting up to " << left << " for osd to time us out ..." << std::endl;
    while (notify_err == 0 && --left) {
        sleep(1);
    }
    ASSERT_GT(left, 0);
...
TEST_F(LibRadosWatchNotify, AioWatchDelete2)
{
    ...
    uint64_t handle;
    ASSERT_EQ(0, rados_aio_create_completion2(nullptr, nullptr, &comp));
    rados_aio_watch2(ioctx, notify_oid, comp, &handle, watch_notify2_test_cb, watch_notify2_test_errcb, timeout, this);
    ASSERT_EQ(0, rados_aio_wait_for_complete(comp));
    ASSERT_EQ(0, rados_aio_get_return_value(comp));
    rados_aio_release(comp);
    ASSERT_EQ(0, rados_remove(ioctx, notify_oid));
