...
TEST(LibRadosService, RegisterEarly)
{
    ...
    ASSERT_EQ(0, rados_service_register(cluster, "laundry", name.c_str(), "foo\0bar\0this\0that\0"));
    ASSERT_EQ(-EEXIST, rados_service_register(cluster, "laundry", name.c_str(), "foo\0bar\0this\0that\0"));
...
static void status_format_func(const int i, std::mutex& lock, std::condition_variable& cond, int& threads_started,
                               bool& stopped)
{
    rados_t cluster;
    ...
    ASSERT_EQ(0, rados_connect(cluster));
    if (i == 0) {
        ASSERT_NE(-1, asprintf(&metadata_buf, "%s%c%s%c", "foo", '\0', "bar", '\0'));
    }
    else if (i == 1) {
        ASSERT_NE(-1, asprintf(&metadata_buf, "%s%c%s%c", "daemon_type", '\0', "portal", '\0'));
    }
    else if (i == 2) {
        ASSERT_NE(-1, asprintf(&metadata_buf, "%s%c%s%c", "daemon_prefix", '\0', "gateway", '\0'));
    }
    else {
        string prefix = string("gw") + stringify(i % 4);
        string zone = string("z") + stringify(i % 3);
        ASSERT_NE(-1,
                  asprintf(&metadata_buf,
    ...
    ASSERT_EQ(0, rados_service_register(cluster, "foo", name.c_str(), metadata_buf));

    std::unique_lock<std::mutex> l(lock);
    threads_started++;
    cond.notify_all();
    cond.wait(l, [&stopped] { return stopped; });
...
TEST(LibRadosService, StatusFormat)
{
    ...
    for (int i = 0; i < nthreads; ++i)
        threads[i] = std::thread(
            status_format_func, i, std::ref(lock), std::ref(cond), std::ref(threads_started), std::ref(stopped));
    ...
    while (retry) {
        ...
        char* outbuf = NULL;
        size_t outlen = 0;
        ASSERT_EQ(0, rados_mon_command(cluster, (const char**)cmd, 1, "", 0, &outbuf, &outlen, NULL, NULL));
...
TEST(LibRadosService, Status)
{
    ...
    ASSERT_EQ(-ENOTCONN, rados_service_update_status(cluster, "testing\0testing\0"));
    ...
    for (int i = 0; i < 20; ++i) {
        char buffer[1024];
        snprintf(buffer, sizeof(buffer), "%s%c%s%c%s%c%d%c", "testing", '\0', "testing", '\0', "count", '\0', i, '\0');
