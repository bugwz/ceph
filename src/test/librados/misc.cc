...
static void test_rados_log_cb(void* arg, const char* line, const char* who, uint64_t sec, uint64_t nsec, uint64_t seq,
...
TEST(LibRadosMiscConnectFailure, ConnectFailure)
{
    ...
    ASSERT_EQ(-ENOTCONN, rados_monitor_log(cluster, "error", test_rados_log_cb, NULL));
...
TEST(LibRadosMiscConnectFailure, ConnectTimeout)
{
    ...
    ASSERT_EQ(0, rados_conf_set(cluster, "key", "AQAAAAAAAAAAABAAAAAAAAAAAAAAAAAAAAAAAA=="));
...
TEST(LibRadosMiscPool, PoolCreationRace)
{
    ...
    ASSERT_EQ(0, rados_conf_set(cluster_a, "objecter_debug_inject_relock_delay", "true"));
...
TEST_F(LibRadosMisc, ClusterFSID)
{
    ...
    ASSERT_EQ(sizeof(fsid) - 1, (size_t)rados_cluster_fsid(cluster, fsid, sizeof(fsid)));
}
...
TEST_F(LibRadosMisc, Exec)
{
    ...
    char buf2[512];
    int res = rados_exec(ioctx, "foo", "rbd", "get_all_features", NULL, 0, buf2, sizeof(buf2));
    ASSERT_GT(res, 0);
    bufferlist bl;
    bl.append(buf2, res);
    auto iter = bl.cbegin();
...
TEST_F(LibRadosMisc, WriteSame)
{
    ...
    ASSERT_EQ(-EINVAL, rados_writesame(ioctx, "ws", buf, sizeof(buf), (sizeof(buf) * 4) - 1, 0));
    ASSERT_EQ(-EINVAL, rados_writesame(ioctx, "ws", buf, sizeof(buf), sizeof(buf) / 2, 0));
...
TEST_F(LibRadosMisc, CmpExt)
{
    ...
    ASSERT_EQ(0, rados_write(ioctx, "cmpextpp", stored_str, sizeof(stored_str), 0));

    ASSERT_EQ(0, rados_cmpext(ioctx, "cmpextpp", stored_str, sizeof(stored_str), 0));

    ASSERT_EQ(-MAX_ERRNO - 5, rados_cmpext(ioctx, "cmpextpp", mismatch_str, sizeof(mismatch_str), 0));
}
...
TEST_F(LibRadosMisc, Applications)
{
    ...
    key_len = sizeof(keys);
    val_len = sizeof(vals);
    ASSERT_EQ(-ENOENT, rados_application_metadata_list(ioctx, "dne", keys, &key_len, vals, &val_len));
    ASSERT_EQ(0, rados_application_metadata_list(ioctx, "app1", keys, &key_len, vals, &val_len));
    ASSERT_EQ(0U, key_len);
    ASSERT_EQ(0U, val_len);

    ASSERT_EQ(-ENOENT, rados_application_metadata_set(ioctx, "dne", "key", "value"));
    ...
    ASSERT_EQ(-ERANGE, rados_application_metadata_list(ioctx, "app1", keys, &key_len, vals, &val_len));
...
TEST_F(LibRadosMisc, MinCompatClient)
{
    ...
    ASSERT_EQ(0, rados_get_min_compatible_client(cluster, &min_compat_client, &require_min_compat_client));
