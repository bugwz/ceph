...
TEST(LibRadosAio, PoolQuotaPP)
{
    ...
    bufferlist inbl;
    ASSERT_EQ(0,
              test_data.m_cluster.mon_command("{\"prefix\": \"osd pool set-quota\", \"pool\": \"" + p +
    ...
    for (n = 0; n < 1024; ++n) {
        ...
        ASSERT_EQ(
            0, ioctx.aio_operate(test_data.m_oid + stringify(n), completion.get(), &op, librados::OPERATION_FULL_TRY));
...
TEST(LibRadosAio, SimpleWritePP)
{
    ...
    bl1.append(buf, sizeof(buf));
    {
        ...
        ASSERT_EQ(0, test_data.m_ioctx.aio_write(test_data.m_oid, my_completion.get(), bl1, sizeof(buf), 0));
        {
            TestAlarm alarm;
...
TEST(LibRadosAio, RoundTripPP)
{
    ...
    ASSERT_EQ(0, test_data.m_ioctx.aio_read(test_data.m_oid, my_completion2.get(), &bl2, sizeof(buf), 0));
    {
        TestAlarm alarm;
...
TEST(LibRadosAio, RoundTripPP3)
{
    ...
    bufferlist csum_bl;
    op1.checksum(LIBRADOS_CHECKSUM_TYPE_CRC32C, init_value_bl, 0, 0, 0, &csum_bl, nullptr);
...
TEST(LibRadosAio, RoundTripSparseReadPP)
{
    ...
    ASSERT_TRUE(my_completion2);
    ASSERT_EQ(0,
              test_data.m_ioctx.aio_sparse_read(test_data.m_oid, my_completion2.get(), &extents, &bl2, sizeof(buf), 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion2->wait_for_complete());
    }
...
TEST(LibRadosAio, ReturnValuePP)
{
    ...
    ASSERT_TRUE(my_completion);
    bufferlist bl1;
    ASSERT_EQ(0, test_data.m_ioctx.aio_read("nonexistent", my_completion.get(), &bl1, 128, 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion->wait_for_complete());
    }
...
TEST(LibRadosAio, RoundTripWriteFullPP)
{
    ...
    ASSERT_EQ(0, test_data.m_ioctx.aio_read(test_data.m_oid, my_completion3.get(), &bl3, sizeof(buf), 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion3->wait_for_complete());
    }
...
TEST(LibRadosAio, RoundTripWriteSamePP)
{
    ...
    ASSERT_EQ(0, test_data.m_ioctx.aio_write(test_data.m_oid, my_completion.get(), bl1, sizeof(full), 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion->wait_for_complete());
    }
    ...
    auto my_completion2 = std::unique_ptr<AioCompletion>{Rados::aio_create_completion()};
    ASSERT_TRUE(my_completion2);
    ASSERT_EQ(0, test_data.m_ioctx.aio_writesame(test_data.m_oid, my_completion2.get(), bl2, ws_write_len, 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion2->wait_for_complete());
    }
    ...
    auto my_completion3 = std::unique_ptr<AioCompletion>{Rados::aio_create_completion()};
    ASSERT_TRUE(my_completion3);
    ASSERT_EQ(0, test_data.m_ioctx.aio_read(test_data.m_oid, my_completion3.get(), &bl3, sizeof(full), 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion3->wait_for_complete());
    }
    ...
    for (char* cmp = bl3.c_str(); cmp < bl3.c_str() + bl3.length(); cmp += sizeof(buf)) {
...
TEST(LibRadosAio, RoundTripWriteSamePP2)
{
    ...
    boost::scoped_ptr<AioCompletion> rd_cmpl(cluster.aio_create_completion(0, 0));
    char* cmp;
...
TEST(LibRadosAio, SimpleStatPPNS)
{
    ...
    auto my_completion2 = std::unique_ptr<AioCompletion>{Rados::aio_create_completion()};
    ASSERT_TRUE(my_completion2);
    ASSERT_EQ(0, test_data.m_ioctx.aio_stat(test_data.m_oid, my_completion2.get(), &psize, &pmtime));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion2->wait_for_complete());
    }
...
TEST(LibRadosAio, OperateMtime)
{
    ...
    time_t set_mtime = 1457129052;
    {
        ...
        op.mtime(&set_mtime);
        op.create(false);
        ASSERT_EQ(0, test_data.m_ioctx.aio_operate("foo", c.get(), &op));
        {
            TestAlarm alarm;
    ...
    }
    {
        uint64_t size;
        timespec mtime;
        ASSERT_EQ(0, test_data.m_ioctx.stat2("foo", &size, &mtime));
        EXPECT_EQ(0, size);
...
TEST(LibRadosAio, StatRemovePP)
{
    ...
    ASSERT_EQ(0, test_data.m_ioctx.aio_stat(test_data.m_oid, my_completion4.get(), &psize2, &pmtime2));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion4->wait_for_complete());
    }
...
TEST(LibRadosAio, ExecuteClassPP)
{
    ...
    ASSERT_TRUE(my_completion2);
    bufferlist in, out;
    ASSERT_EQ(0, test_data.m_ioctx.aio_exec(test_data.m_oid, my_completion2.get(), "hello", "say_hello", in, &out));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion2->wait_for_complete());
    }
...
TEST(LibRadosAio, MultiWritePP)
{
    ...
    auto my_completion2 = std::unique_ptr<AioCompletion>{Rados::aio_create_completion()};
    ASSERT_TRUE(my_completion2);
    ASSERT_EQ(0, test_data.m_ioctx.aio_write(test_data.m_oid, my_completion2.get(), bl2, sizeof(buf2), sizeof(buf)));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion2->wait_for_complete());
    }
    ...
    ASSERT_EQ(
        0,
        test_data.m_ioctx.aio_read(test_data.m_oid, my_completion3.get(), &bl3, (sizeof(buf) + sizeof(buf2) * 3), 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion3->wait_for_complete());
    }
...
TEST(LibRadosAio, RoundTripAppendPP)
{
    ...
    bufferlist bl1;
    bl1.append(buf, sizeof(buf));
    ASSERT_EQ(0, test_data.m_ioctx.aio_append(test_data.m_oid, my_completion.get(), bl1, sizeof(buf)));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion->wait_for_complete());
    }
    ...
    auto my_completion2 = std::unique_ptr<AioCompletion>{Rados::aio_create_completion()};
    ASSERT_TRUE(my_completion2);
    ASSERT_EQ(0, test_data.m_ioctx.aio_append(test_data.m_oid, my_completion2.get(), bl2, sizeof(buf2)));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion2->wait_for_complete());
    }
    ...
    auto my_completion3 = std::unique_ptr<AioCompletion>{Rados::aio_create_completion()};
    ASSERT_TRUE(my_completion3);
    ASSERT_EQ(0, test_data.m_ioctx.aio_read(test_data.m_oid, my_completion3.get(), &bl3, 2 * sizeof(buf), 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion3->wait_for_complete());
    }
...
TEST(LibRadosAioEC, RoundTripAppendPP)
{
    ...
    ASSERT_EQ(0, test_data.m_ioctx.aio_append(test_data.m_oid, my_completion.get(), bl1, bsize));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion->wait_for_complete());
    }
    ...
    auto my_completion2 = std::unique_ptr<AioCompletion>{Rados::aio_create_completion()};
    ASSERT_TRUE(my_completion2);
    ASSERT_EQ(0, test_data.m_ioctx.aio_append(test_data.m_oid, my_completion2.get(), bl2, hbsize));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion2->wait_for_complete());
    }
    ...
    auto my_completion3 = std::unique_ptr<AioCompletion>{Rados::aio_create_completion()};
    ASSERT_TRUE(my_completion3);
    ASSERT_EQ(0, test_data.m_ioctx.aio_append(test_data.m_oid, my_completion3.get(), bl2, hbsize));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion3->wait_for_complete());
    }
    ...
    auto my_completion4 = std::unique_ptr<AioCompletion>{Rados::aio_create_completion()};
    ASSERT_TRUE(my_completion4);
    ASSERT_EQ(0, test_data.m_ioctx.aio_read(test_data.m_oid, my_completion4.get(), &bl3, bsize * 3, 0));
    {
        TestAlarm alarm;
        ASSERT_EQ(0, my_completion4->wait_for_complete());
    }
...
TEST(LibRadosAio, RacingRemovePP)
{
    ...
    ASSERT_EQ(0, test_data.m_ioctx.aio_write(test_data.m_oid, my_completion.get(), bl, sizeof(buf), 0));
    {
        TestAlarm alarm;
...
TEST(LibRadosAio, MultiReads)
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        ASSERT_TRUE(aiocp);
        ASSERT_EQ(0, test_data.m_ioctx.aio_read("foo", aiocp.get(), &bl, sizeof(buf), 0));
    }
