...
public:
    ...
    static void TearDownTestCase() { rados.shutdown(); }
};
...
TEST_F(AsioRados, AsyncReadFuture)
{
    ...
    std::future<bufferlist> f1 = librados::async_read(service, io, "exist", 256, 0, boost::asio::use_future);
    std::future<bufferlist> f2 = librados::async_read(service, io, "noexist", 256, 0, boost::asio::use_future);
...
TEST_F(AsioRados, AsyncWriteCallback)
{
    ...
    auto success_cb = [&](boost::system::error_code ec) { EXPECT_FALSE(ec); };
    librados::async_write(service, io, "exist", bl, bl.length(), 0, success_cb);

    auto failure_cb = [&](boost::system::error_code ec) { EXPECT_EQ(boost::system::errc::read_only_file_system, ec); };
    librados::async_write(service, snapio, "exist", bl, bl.length(), 0, failure_cb);
...
TEST_F(AsioRados, AsyncWriteFuture)
{
    ...
    auto f1 = librados::async_write(service, io, "exist", bl, bl.length(), 0, boost::asio::use_future);
    auto f2 = librados::async_write(service, snapio, "exist", bl, bl.length(), 0, boost::asio::use_future);
...
TEST_F(AsioRados, AsyncWriteYield)
{
    ...
    auto success_cr = [&](boost::asio::yield_context yield) {
        boost::system::error_code ec;
        librados::async_write(service, io, "exist", bl, bl.length(), 0, yield[ec]);
        EXPECT_FALSE(ec);
        EXPECT_EQ("hello", bl.to_str());
    };
    boost::asio::spawn(service, success_cr);
    ...
    auto failure_cr = [&](boost::asio::yield_context yield) {
        boost::system::error_code ec;
        librados::async_write(service, snapio, "exist", bl, bl.length(), 0, yield[ec]);
...
TEST_F(AsioRados, AsyncReadOperationFuture)
{
    ...
    std::future<bufferlist> f1;
    {
        librados::ObjectReadOperation op;
        op.read(0, 0, nullptr, nullptr);
        f1 = librados::async_operate(service, io, "exist", &op, 0, boost::asio::use_future);
    }
    std::future<bufferlist> f2;
    {
        librados::ObjectReadOperation op;
        op.read(0, 0, nullptr, nullptr);
        f2 = librados::async_operate(service, io, "noexist", &op, 0, boost::asio::use_future);
    }
    service.run();
...
TEST_F(AsioRados, AsyncReadOperationYield)
{
    ...
    auto success_cr = [&](boost::asio::yield_context yield) {
        ...
        op.read(0, 0, nullptr, nullptr);
        boost::system::error_code ec;
        auto bl = librados::async_operate(service, io, "exist", &op, 0, yield[ec]);
        EXPECT_FALSE(ec);
        EXPECT_EQ("hello", bl.to_str());
    };
    boost::asio::spawn(service, success_cr);
    ...
    auto failure_cr = [&](boost::asio::yield_context yield) {
        ...
        op.read(0, 0, nullptr, nullptr);
        boost::system::error_code ec;
        auto bl = librados::async_operate(service, io, "noexist", &op, 0, yield[ec]);
        EXPECT_EQ(boost::system::errc::no_such_file_or_directory, ec);
    };
    boost::asio::spawn(service, failure_cr);
...
TEST_F(AsioRados, AsyncWriteOperationFuture)
{
    ...
    std::future<void> f2;
    {
        librados::ObjectWriteOperation op;
        op.write_full(bl);
        f2 = librados::async_operate(service, snapio, "exist", &op, 0, boost::asio::use_future);
    }
    service.run();
