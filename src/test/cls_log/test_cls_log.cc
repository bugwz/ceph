...
static int log_list(librados::IoCtx& ioctx, const std::string& oid, utime_t& from, utime_t& to, const string& in_marker,
                    int max_entries, list<cls_log_entry>& entries, string* out_marker, bool* truncated)
{
    librados::ObjectReadOperation rop;
    cls_log_list(rop, from, to, in_marker, max_entries, entries, out_marker, truncated);
    bufferlist obl;
    return ioctx.operate(oid, &rop, &obl);
}

static int log_list(librados::IoCtx& ioctx, const std::string& oid, utime_t& from, utime_t& to, int max_entries,
                    list<cls_log_entry>& entries, bool* truncated)
{
    std::string marker;
    return log_list(ioctx, oid, from, to, marker, max_entries, entries, &marker, truncated);
}

static int log_list(librados::IoCtx& ioctx, const std::string& oid, list<cls_log_entry>& entries)
{
    utime_t from, to;
...
TEST_F(cls_log, test_log_add_same_time)
{
    ...
    /* check list */
    {
        ASSERT_EQ(0, log_list(ioctx, oid, start_time, to_time, 0, entries, &truncated));
    ...
    {
        ASSERT_EQ(0, log_list(ioctx, oid, start_time, to_time, 1, entries, &truncated));
...
TEST_F(cls_log, test_log_add_different_time)
{
    ...
    /* check list again with shifted time */
    {
        ...
        ASSERT_EQ(0, log_list(ioctx, oid, next_time, to_time, 0, entries, &truncated));
    ...
    do {
        ...
        ASSERT_EQ(0, log_list(ioctx, oid, start_time, to_time, old_marker, 1, entries, &marker, &truncated));
...
int do_log_trim(librados::IoCtx& ioctx, const std::string& oid, const std::string& from_marker,
...
int do_log_trim(librados::IoCtx& ioctx, const std::string& oid, const utime_t& from_time, const utime_t& to_time)
{
    librados::ObjectWriteOperation op;
...
TEST_F(cls_log, trim_by_marker)
{
    ...
    std::vector<cls_log_entry> log1;
    {
        ...
        log1.assign(std::make_move_iterator(entries.begin()), std::make_move_iterator(entries.end()));
    }
    // trim front of log
    {
