...
{
public:
    using JournalRecorderPtr =
        std::unique_ptr<journal::JournalRecorder, std::function<void(journal::JournalRecorder*)>>;
    JournalRecorderPtr create_recorder(const std::string& oid, const ceph::ref_t<journal::JournalMetadata>& metadata)
    {
        JournalRecorderPtr recorder{new journal::JournalRecorder(m_ioctx, oid + ".", metadata, 0),
...
TEST_F(TestJournalRecorder, AppendKnownOverflow)
{
    ...
    recorder->append(123,
                     create_payload(std::string(metadata->get_object_size() - journal::Entry::get_fixed_size(), '1')));
...
TEST_F(TestJournalRecorder, AppendDelayedOverflow)
{
    ...
    recorder2->append(123,
                      create_payload(std::string(metadata->get_object_size() - journal::Entry::get_fixed_size(), '2')));
