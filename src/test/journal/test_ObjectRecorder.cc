...
public:
    ...
    public:
        ...
        {}
        ObjectRecorderFlusher(librados::IoCtx& ioctx, ContextWQ* work_queue, uint32_t flush_interval,
                              uint16_t flush_bytes, double flush_age, int max_in_flight)
            : m_ioctx{ioctx}
            , m_work_queue{work_queue}
            , m_flush_interval{flush_interval}
            , m_flush_bytes{flush_bytes}
            , m_flush_age{flush_age}
            , m_max_in_flight_appends{max_in_flight < 0 ? std::numeric_limits<uint64_t>::max()
        ...
        auto create_object(std::string_view oid, uint8_t order, ceph::mutex* lock)
        {
            auto object = ceph::make_ref<journal::ObjectRecorder>(
                m_ioctx, oid, 0, lock, m_work_queue, &m_handler, order, m_max_in_flight_appends);
            {
                std::lock_guard locker{*lock};
                object->set_append_batch_options(m_flush_interval, m_flush_bytes, m_flush_age);
            }
        ...
        bool wait_for_closed()
        {
            ...
            return m_handler.cond.wait_for(locker, 10s, [this] { return m_handler.is_closed; });
        }
        bool wait_for_overflow()
        {
            std::unique_lock locker{m_handler.lock};
            if (m_handler.cond.wait_for(locker, 10s, [this] { return m_handler.overflows > 0; })) {
    ...
    private:
        ...
        using ObjectRecorders = std::list<std::pair<ceph::ref_t<journal::ObjectRecorder>, ceph::mutex*>>;
        ObjectRecorders m_object_recorders;
        Handler m_handler;
    };

    journal::AppendBuffer create_append_buffer(uint64_t tag_tid, uint64_t entry_tid, const std::string& payload)
    {
...
TEST_F(TestObjectRecorder, Append)
{
    ...
    journal::AppendBuffer append_buffer1 = create_append_buffer(234, 123, "payload");
    journal::AppendBuffers append_buffers;
    ...
    journal::AppendBuffer append_buffer2 = create_append_buffer(234, 124, "payload");
...
TEST_F(TestObjectRecorder, AppendFlushByAge)
{
    ...
    while (!append_buffer1.first->is_flush_in_progress() && !append_buffer1.first->is_complete()) {
        usleep(1000);
...
TEST_F(TestObjectRecorder, FlushFuture)
{
    ...
    journal::AppendBuffer append_buffer = create_append_buffer(234, 123, "payload");
    journal::AppendBuffers append_buffers;
    ...
    ASSERT_TRUE(append_buffer.first->is_flush_in_progress() || append_buffer.first->is_complete());
    ASSERT_EQ(0, cond.wait());
}
...
TEST_F(TestObjectRecorder, Overflow)
{
    ...
    journal::AppendBuffer append_buffer3 = create_append_buffer(456, 123, payload);
