...
template<typename T> class TestJournalPlayer : public RadosTestFixture
{
public:
    ...
    {
        for (JournalPlayers::iterator it = m_players.begin(); it != m_players.end(); ++it) {
            delete *it;
        }
    ...
    {
        return RadosTestFixture::create_metadata(oid, "client", 0.1, max_fetch_bytes);
    }

    int client_commit(const std::string& oid, journal::JournalPlayer::ObjectSetPosition position)
    {
    ...
    journal::JournalPlayer* create_player(const std::string& oid, const ceph::ref_t<journal::JournalMetadata>& metadata)
    {
        journal::JournalPlayer* player(
            new journal::JournalPlayer(m_ioctx, oid + ".", metadata, &m_replay_hander, nullptr));
        m_players.push_back(player);
        return player;
    }

    bool wait_for_entries(journal::JournalPlayer* player, uint32_t count, Entries* entries)
    {
        entries->clear();
        while (entries->size() < count) {
            journal::Entry entry;
            uint64_t commit_tid;
            while (entries->size() < count && player->try_pop_front(&entry, &commit_tid)) {
            ...
            else if (m_replay_hander.cond.wait_for(locker, 10s) == std::cv_status::timeout) {
                break;
            }
        }
    ...
    bool wait_for_complete(journal::JournalPlayer* player)
    {
        ...
        while (!m_replay_hander.complete) {
            ...
            if (m_replay_hander.cond.wait_for(locker, 10s) == std::cv_status::timeout) {
                return false;
            }
        }
    ...
    int write_entry(const std::string& oid, uint64_t object_num, uint64_t tag_tid, uint64_t entry_tid)
    {
        bufferlist bl;
...
template<uint64_t _max_fetch_bytes> class TestJournalPlayerParams
{
public:
    static const uint64_t max_fetch_bytes = _max_fetch_bytes;
};

typedef ::testing::Types<TestJournalPlayerParams<0>, TestJournalPlayerParams<16> > TestJournalPlayerTypes;
...
TYPED_TEST(TestJournalPlayer, Prefetch)
{
    ...
    expected_entries = {this->create_entry(234, 123), this->create_entry(234, 124), this->create_entry(234, 125)};
...
TYPED_TEST(TestJournalPlayer, PrefetchSkip)
{
    ...
    journal::JournalPlayer::ObjectPositions positions;
    positions = {cls::journal::ObjectPosition(0, 234, 125), cls::journal::ObjectPosition(1, 234, 124)};
    cls::journal::ObjectSetPosition commit_position(positions);
...
TYPED_TEST(TestJournalPlayer, PrefetchWithoutCommit)
{
    ...
    Entries expected_entries;
    expected_entries = {this->create_entry(234, 122), this->create_entry(234, 123)};
    ASSERT_EQ(expected_entries, entries);
}
...
TYPED_TEST(TestJournalPlayer, PrefetchMultipleTags)
{
    ...
    journal::JournalPlayer::ObjectPositions positions;
    positions = {cls::journal::ObjectPosition(2, 234, 122),
...
TYPED_TEST(TestJournalPlayer, PrefetchMissingSequence)
{
    ...
    Entries expected_entries = {this->create_entry(2, 852),
...
TYPED_TEST(TestJournalPlayer, PrefetchLargeMissingSequence)
{
    ...
    Entries expected_entries = {this->create_entry(0, 0), this->create_entry(0, 1), this->create_entry(1, 0)};
    ASSERT_EQ(expected_entries, entries);
}
...
TYPED_TEST(TestJournalPlayer, PrefetchBlockedNewTag)
{
    ...
    Entries expected_entries = {
        this->create_entry(0, 0), this->create_entry(0, 1), this->create_entry(0, 2), this->create_entry(1, 0)};
    ASSERT_EQ(expected_entries, entries);
}
...
TYPED_TEST(TestJournalPlayer, PrefetchStaleEntries)
{
    ...
    journal::JournalPlayer::ObjectPositions positions = {cls::journal::ObjectPosition(0, 1, 0)};
    cls::journal::ObjectSetPosition commit_position(positions);
...
TYPED_TEST(TestJournalPlayer, PrefetchSkippedObject)
{
    ...
    Entries expected_entries;
    expected_entries = {this->create_entry(234, 122),
...
TYPED_TEST(TestJournalPlayer, ImbalancedJournal)
{
    ...
    journal::JournalPlayer::ObjectPositions positions = {cls::journal::ObjectPosition(9, 300, 1),
    ...
    Entries expected_entries;
    expected_entries = {
        this->create_entry(301, 0), this->create_entry(301, 1), this->create_entry(301, 2), this->create_entry(301, 3)};
    ASSERT_EQ(expected_entries, entries);
...
TYPED_TEST(TestJournalPlayer, LiveReplayLaggyAppend)
{
    ...
    Entries expected_entries = {this->create_entry(0, 0), this->create_entry(0, 1), this->create_entry(0, 2)};
    ASSERT_EQ(expected_entries, entries);
    ...
    expected_entries = {this->create_entry(0, 3), this->create_entry(0, 4), this->create_entry(0, 5)};
    ASSERT_EQ(expected_entries, entries);
}
...
TYPED_TEST(TestJournalPlayer, LiveReplayMissingSequence)
{
    ...
    Entries expected_entries = {this->create_entry(2, 852), this->create_entry(2, 853), this->create_entry(2, 854)};
    ASSERT_EQ(expected_entries, entries);
    ...
    expected_entries = {
        this->create_entry(3, 0), this->create_entry(3, 1), this->create_entry(3, 2), this->create_entry(3, 3)};
    ASSERT_EQ(expected_entries, entries);
}
...
TYPED_TEST(TestJournalPlayer, LiveReplayBlockedNewTag)
{
    ...
    Entries expected_entries = {
        this->create_entry(tag1.tid, 0), this->create_entry(tag1.tid, 1), this->create_entry(tag1.tid, 2)};
    ASSERT_EQ(expected_entries, entries);
...
TYPED_TEST(TestJournalPlayer, LiveReplayRefetchRemoveEmpty)
{
    ...
    journal::JournalPlayer::ObjectPositions positions = {cls::journal::ObjectPosition(1, 0, 1),
