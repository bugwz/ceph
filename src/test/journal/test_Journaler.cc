...
public:
    ...
    static std::string get_temp_journal_id() { return stringify(++_journal_id); }
    ...
    void SetUp() override
    {
        ...
        m_journaler =
            new journal::Journaler(m_work_queue, m_timer, &m_timer_lock, m_ioctx, m_journal_id, CLIENT_ID, {}, nullptr);
    }
    ...
    int register_client(const std::string& client_id, const std::string& desc)
    {
        journal::Journaler journaler(
            m_work_queue, m_timer, &m_timer_lock, m_ioctx, m_journal_id, client_id, {}, nullptr);
        bufferlist data;
        data.append(desc);
        C_SaferCond cond;
