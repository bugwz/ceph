...
public:
    auto create_object(const std::string& oid, uint8_t order)
    {
        auto object = ceph::make_ref<journal::ObjectPlayer>(
            m_ioctx, oid + ".", 0, *m_timer, m_timer_lock, order, T::max_fetch_bytes);
        return object;
    }
    ...
    int fetch(const ceph::ref_t<journal::ObjectPlayer>& object_player)
    {
        while (true) {
            C_SaferCond ctx;
            object_player->set_refetch_state(journal::ObjectPlayer::REFETCH_STATE_NONE);
    ...
    int watch_and_wait_for_entries(const ceph::ref_t<journal::ObjectPlayer>& object_player,
                                   journal::ObjectPlayer::Entries* entries, size_t count)
    {
    ...
    std::string get_object_name(const std::string& oid) { return oid + ".0"; }
};

template<uint32_t _max_fetch_bytes> struct TestObjectPlayerParams
{
    static inline const uint32_t max_fetch_bytes = _max_fetch_bytes;
};

typedef ::testing::Types<TestObjectPlayerParams<0>, TestObjectPlayerParams<10> > TestObjectPlayerTypes;
...
TYPED_TEST(TestObjectPlayer, FetchLarge)
{
    ...
    journal::Entry entry1(234, 123, this->create_payload(std::string(8192 - 32, '1')));
