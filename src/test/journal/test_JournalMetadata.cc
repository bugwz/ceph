...
public:
    ...
    {
        for (MetadataList::iterator it = m_metadata_list.begin(); it != m_metadata_list.end(); ++it) {
    ...
    auto create_metadata(const std::string& oid, const std::string& client_id, double commit_interval = 0.1,
                         int max_concurrent_object_sets = 0)
    {
        auto metadata = RadosTestFixture::create_metadata(oid, client_id, commit_interval, max_concurrent_object_sets);
...
TEST_F(TestJournalMetadata, DisconnectLaggyClient)
{
    ...
    auto metadata = create_metadata(oid, "client1", 0.1, max_concurrent_object_sets);
...
#define ASSERT_CLIENT_STATES(s1, s2)  \
    ...
    ASSERT_CLIENT_STATES(cls::journal::CLIENT_STATE_CONNECTED, cls::journal::CLIENT_STATE_CONNECTED);
    ...
    clients.clear();
    metadata->get_registered_clients(&clients);
    ASSERT_CLIENT_STATES(cls::journal::CLIENT_STATE_CONNECTED, cls::journal::CLIENT_STATE_DISCONNECTED);
}
