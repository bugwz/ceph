...
TEST_F(PolicyTest, Eval1)
{
    ...
    EXPECT_EQ(p.eval(e, none, s3ListBucket, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Allow);

    EXPECT_EQ(p.eval(e, none, s3PutBucketAcl, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Pass);

    EXPECT_EQ(p.eval(e, none, s3ListBucket, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "erroneous_bucket")),
              Effect::Pass);
}
...
TEST_F(PolicyTest, Eval2)
{
    ...
    for (auto i = 0ULL; i < s3Count; ++i) {
        EXPECT_EQ(p.eval(e, trueacct, i, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "mybucket")),
                  Effect::Allow);
        EXPECT_EQ(p.eval(e, trueacct, i, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "mybucket/myobject")),
                  Effect::Allow);

        EXPECT_EQ(p.eval(e, notacct, i, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "mybucket")),
                  Effect::Pass);
        EXPECT_EQ(p.eval(e, notacct, i, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "mybucket/myobject")),
                  Effect::Pass);

        EXPECT_EQ(p.eval(e, trueacct, i, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "notyourbucket")),
                  Effect::Pass);
        EXPECT_EQ(
            p.eval(
                e, trueacct, i, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "notyourbucket/notyourobject")),
            Effect::Pass);
    }
}
...
TEST_F(PolicyTest, Eval3)
{
    ...
    EXPECT_EQ(p.eval(em, none, s3PutBucketPolicy, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "mybucket")),
              Effect::Allow);
    ...
    for (auto op = 0ULL; op < s3Count; ++op) {
        ...
        EXPECT_EQ(p.eval(em, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "confidential-data")),
                  Effect::Pass);
        EXPECT_EQ(p.eval(tr, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "confidential-data")),
                  s3allow[op] ? Effect::Allow : Effect::Pass);
        EXPECT_EQ(p.eval(fa, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "confidential-data")),
                  Effect::Pass);

        EXPECT_EQ(p.eval(em, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "confidential-data/moo")),
                  Effect::Pass);
        EXPECT_EQ(p.eval(tr, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "confidential-data/moo")),
                  s3allow[op] ? Effect::Allow : Effect::Pass);
        EXPECT_EQ(p.eval(fa, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "confidential-data/moo")),
                  Effect::Pass);

        EXPECT_EQ(
            p.eval(em, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "really-confidential-data")),
            Effect::Pass);
        EXPECT_EQ(
            p.eval(tr, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "really-confidential-data")),
            Effect::Pass);
        EXPECT_EQ(
            p.eval(fa, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "really-confidential-data")),
            Effect::Pass);

        EXPECT_EQ(
            p.eval(
                em, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "really-confidential-data/moo")),
            Effect::Pass);
        EXPECT_EQ(
            p.eval(
                tr, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "really-confidential-data/moo")),
            Effect::Pass);
        EXPECT_EQ(
            p.eval(
                fa, none, op, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "really-confidential-data/moo")),
            Effect::Pass);
    }
}
...
TEST_F(PolicyTest, Eval4)
{
    ...
    EXPECT_EQ(
        p.eval(e, none, iamCreateRole, ARN(Partition::aws, Service::iam, "", arbitrary_tenant, "role/example_role")),
        Effect::Allow);

    EXPECT_EQ(
        p.eval(e, none, iamDeleteRole, ARN(Partition::aws, Service::iam, "", arbitrary_tenant, "role/example_role")),
        Effect::Pass);
}
...
TEST_F(PolicyTest, Eval5)
{
    ...
    EXPECT_EQ(
        p.eval(e, none, s3ListBucket, ARN(Partition::aws, Service::iam, "", arbitrary_tenant, "role/example_role")),
        Effect::Pass);

    EXPECT_EQ(p.eval(e, none, iamCreateRole, ARN(Partition::aws, Service::iam, "", "", "role/example_role")),
              Effect::Pass);
}
...
TEST_F(PolicyTest, Eval6)
{
    ...
    EXPECT_EQ(p.eval(e, none, iamCreateRole, ARN(Partition::aws, Service::iam, "", arbitrary_tenant, "user/A")),
              Effect::Allow);

    EXPECT_EQ(p.eval(e, none, s3ListBucket, ARN(Partition::aws, Service::iam, "", arbitrary_tenant, "user/A")),
              Effect::Allow);
}
...
TEST_F(PolicyTest, Eval7)
{
    ...
    EXPECT_EQ(p.eval(e, subacct, s3ListBucket, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "mybucket/*")),
              Effect::Allow);

    EXPECT_EQ(p.eval(e, parentacct, s3ListBucket, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "mybucket/*")),
              Effect::Pass);

    EXPECT_EQ(p.eval(e, sub2acct, s3ListBucket, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "mybucket/*")),
              Effect::Pass);
}
...
TEST_F(IPPolicyTest, EvalIPAddress)
{
    ...
    EXPECT_EQ(allowp.eval(
                  e, trueacct, s3ListBucket, ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Pass);
    EXPECT_EQ(fullp.eval(e,
                         trueacct,
                         s3ListBucket,
                         ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket/myobject")),
              Effect::Pass);

    EXPECT_EQ(allowp.eval(allowedIP,
                          trueacct,
                          s3ListBucket,
                          ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Allow);
    EXPECT_EQ(allowp.eval(blocklistedIPv6,
                          trueacct,
                          s3ListBucket,
                          ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Pass);
    ...
    EXPECT_EQ(denyp.eval(allowedIP,
                         trueacct,
                         s3ListBucket,
                         ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Deny);
    ...
    EXPECT_EQ(denyp.eval(blocklistedIP,
                         trueacct,
                         s3ListBucket,
                         ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Pass);
    ...
    EXPECT_EQ(denyp.eval(blocklistedIPv6,
                         trueacct,
                         s3ListBucket,
                         ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Pass);
    EXPECT_EQ(denyp.eval(blocklistedIPv6,
                         ...
                         ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket/myobject")),
              Effect::Pass);
    EXPECT_EQ(denyp.eval(allowedIPv6,
                         trueacct,
                         s3ListBucket,
                         ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Deny);
    ...
    EXPECT_EQ(fullp.eval(allowedIP,
                         trueacct,
                         s3ListBucket,
                         ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Allow);
    ...
    EXPECT_EQ(fullp.eval(blocklistedIP,
                         trueacct,
                         s3ListBucket,
                         ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Pass);
    ...
    EXPECT_EQ(fullp.eval(allowedIPv6,
                         trueacct,
                         s3ListBucket,
                         ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Allow);
    ...
    EXPECT_EQ(fullp.eval(blocklistedIPv6,
                         trueacct,
                         s3ListBucket,
                         ARN(Partition::aws, Service::s3, "", arbitrary_tenant, "example_bucket")),
              Effect::Pass);
