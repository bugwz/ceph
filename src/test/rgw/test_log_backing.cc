...
protected:
    ...
    {
        return (gen_id > 0 ? fmt::format("shard@G{}.{}", gen_id, i) : fmt::format("shard.{}", i));
    }
    ...
    void assert_empty()
    {
        ...
        auto r = ioctx.object_list(ioctx.object_list_begin(), ioctx.object_list_end(), 100, {}, &result, &next);
        ASSERT_GE(r, 0);
...
{
    make_omap();
    auto stat = log_backing_type(
        &dp, ioctx, log_type::fifo, SHARDS, [this](int shard) { return get_oid(0, shard); }, null_yield);
...
{
    auto stat = log_backing_type(
        &dp, ioctx, log_type::omap, SHARDS, [this](int shard) { return get_oid(0, shard); }, null_yield);
    ASSERT_EQ(log_type::omap, *stat);
}
