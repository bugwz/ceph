...
TEST(ReshardWait, stop_block)
{
    ...
    const auto start = Clock::now();
    std::thread thread([&long_waiter] { EXPECT_EQ(-ECANCELED, long_waiter.wait(null_yield)); });
...
TEST(ReshardWait, wait_yield)
{
    ...
    spawn::spawn(context, [&](yield_context yield) { EXPECT_EQ(0, waiter.wait(optional_yield{context, yield})); });
...
TEST(ReshardWait, stop_multiple)
{
    ...
    {
        auto sync_waiter([&long_waiter] { EXPECT_EQ(-ECANCELED, long_waiter.wait(null_yield)); });
