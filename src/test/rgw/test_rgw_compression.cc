...
public:
    ...
    bufferlist& get_sink() { return sink; }
};
...
public:
    ...
    int handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len) override
    {
        if (bl_len > (off_t)max_size) max_size = bl_len;
        return 0;
    }
    size_t get_size() { return max_size; }
};
...
{
    int handle_data(bufferlist& bl, off_t bl_ofs, off_t bl_len) override { return 0; }
} cb;
...
TEST(Compress, BillionZeros)
{
    ...
    for (int i = 0; i < 1000; i++) compressor.process(bufferlist{bl}, size * i);
