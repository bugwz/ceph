...
public:
    explicit StoreDestructor(rgw::sal::RadosStore* _s)
        : driver(_s)
    {}
    ~StoreDestructor() { DriverManager::close_storage(store); }
};

struct TempPool
{
    inline static uint64_t num = 0;
    std::string name = fmt::format("{}-{}-{}", ::time(nullptr), ::getpid(), num++);
    ...
    operator rgw_pool() { return {name}; }

    operator librados::IoCtx()
    {
        librados::IoCtx ioctx;
        auto r = store->getRados()->get_rados_handle()->ioctx_create(name.c_str(), ioctx);
        assert(r == 0);
        return ioctx;
    }
};
...
TEST(ReadAttrs, Unfiltered)
{
    ...
    encode(dummy, bl);
    const std::map<std::string, ceph::bufferlist> ref_attrs{{"foo"s, bl}, {"bar"s, bl}, {"baz"s, bl}};
    auto oid = "object"s;
    {
    ...
    auto r = run(new RGWSimpleRadosReadAttrsCR(dpp(), store, {pool, oid}, &attrs, true));
    ASSERT_EQ(0, r);
...
TEST(ReadAttrs, Filtered)
{
    ...
    const std::map<std::string, ceph::bufferlist> ref_attrs{
        {RGW_ATTR_PREFIX "foo"s, bl}, {RGW_ATTR_PREFIX "bar"s, bl}, {RGW_ATTR_PREFIX "baz"s, bl}};
    auto oid = "object"s;
    {
        librados::IoCtx ioctx(pool);
        librados::ObjectWriteOperation op;
    ...
    std::map<std::string, ceph::bufferlist> attrs;
    auto r = run(new RGWSimpleRadosReadAttrsCR(dpp(), store, {pool, oid}, &attrs, false));
    ASSERT_EQ(0, r);
    ASSERT_EQ(ref_attrs, attrs);
}
...
TEST(Read, Dne)
{
    TempPool pool;
    std::string result;
    auto r = run(new RGWSimpleRadosReadCR(dpp(), store, {pool, "doesnotexist"}, &result, false));
    ASSERT_EQ(-ENOENT, r);
}
...
TEST(Read, Read)
{
    ...
    std::string result;
    auto r = run(new RGWSimpleRadosReadCR(dpp(), store, {pool, oid}, &result, false));
    ASSERT_EQ(0, r);
...
TEST(Read, ReadVersion)
{
    ...
    std::string result;
    auto r = run(new RGWSimpleRadosReadCR(dpp(), store, {pool, oid}, &result, false, &robjv));
    ASSERT_EQ(0, r);
    ASSERT_EQ(result, data);
    ...
    result.clear();
    r = run(new RGWSimpleRadosReadCR(dpp(), store, {pool, oid}, &result, false, &robjv));
...
TEST(Write, Exclusive)
{
    ...
    auto r = run(new RGWSimpleRadosWriteCR(dpp(), store, {pool, oid}, "I am some DIFFERENT data!"s, nullptr, true));
    ASSERT_EQ(-EEXIST, r);
}
...
TEST(Write, Write)
{
    ...
    auto r = run(new RGWSimpleRadosWriteCR(dpp(), store, {pool, oid}, data, nullptr, true));
    ASSERT_EQ(0, r);
    bufferlist bl;
    librados::IoCtx ioctx(pool);
...
TEST(Write, ObjV)
{
    ...
    auto r = run(new RGWSimpleRadosWriteCR(dpp(), store, {pool, oid}, "I'm some data!"s, &objv, true));
    RGWObjVersionTracker interfering_objv(objv);
    r = run(new RGWSimpleRadosWriteCR(
        dpp(), store, {pool, oid}, "I'm some newer, better data!"s, &interfering_objv, false));
    ASSERT_EQ(0, r);
    r = run(
        new RGWSimpleRadosWriteCR(dpp(), store, {pool, oid}, "I'm some treacherous, obsolete data!"s, &objv, false));
    ASSERT_EQ(-ECANCELED, r);
}
...
TEST(WriteAttrs, Attrs)
{
    ...
    std::map<std::string, bufferlist> wrattrs{{"foo", bl}, {"bar", bl}, {"baz", bl}};
    auto r = run(new RGWSimpleRadosWriteAttrsCR(dpp(), store, {pool, oid}, wrattrs, nullptr, true));
    ASSERT_EQ(0, r);
...
int main(int argc, const char** argv)
{
    ...
    store = static_cast<rgw::sal::RadosStore*>(
        DriverManager::get_storage(dpp(), g_ceph_context, cfg, false, false, false, false, false, true, false));
    if (!store) {
