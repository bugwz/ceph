...
public:
    ...
    bool is_identity(const flat_set<Principal>& ids) const override { return false; }
};

class TestUser : public sal::StoreUser
{
public:
    virtual std::unique_ptr<User> clone() override { return std::unique_ptr<User>(new TestUser(*this)); }

    virtual int list_buckets(const DoutPrefixProvider* dpp, const string&, const string&, uint64_t, bool,
                             sal::BucketList&, optional_yield y) override
    {
        return 0;
    }
    ...
    virtual int create_bucket(const DoutPrefixProvider* dpp, const rgw_bucket& b, const std::string& zonegroup_id,
                              ...
                              const RGWQuotaInfo* pquota_info, const RGWAccessControlPolicy& policy, sal::Attrs& attrs,
                              RGWBucketInfo& info, obj_version& ep_objv, bool exclusive, bool obj_lock_enabled,
                              bool* existed, req_info& req_info, std::unique_ptr<sal::Bucket>* bucket,
...
TEST(TestRGWLua, OpsLog)
{
    ...
    {
        bool logged = false;
        int log(req_state*, rgw_log_entry&) override
        {
            logged = true;
            return 0;
        }
    ...
    s.auth.identity = std::unique_ptr<rgw::auth::Identity>(new FakeIdentity());
...
template<typename T> const T& get_table_value(const TestBackground& b, const std::string& index)
{
    try {
