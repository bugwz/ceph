...
public:
    ...
    const map<uint64_t, OldObjManifestPart>& get_objs() { return objs; }
...
static void gen_obj(test_rgw_env& env, uint64_t obj_size, uint64_t head_max_size, uint64_t stripe_size,
                    RGWObjManifest* manifest, const rgw_placement_rule& placement_rule, rgw_bucket* bucket,
                    rgw_obj* head, RGWObjManifest::generator* gen, list<rgw_obj>* test_objs)
{
...
TEST(TestRGWManifest, head_only_obj)
{
    ...
    for (iter = manifest.obj_begin(&dp), liter = objs.begin(); iter != manifest.obj_end(&dp) && liter != objs.end();
         ++iter, ++liter) {
...
TEST(TestRGWManifest, obj_with_head_and_tail)
{
    ...
    gen_obj(
        env, obj_size, head_size, stripe_size, &manifest, env.zonegroup.default_placement, &bucket, &head, &gen, &objs);
...
TEST(TestRGWManifest, old_obj_manifest)
{
    ...
    for (iter = manifest.obj_begin(&dp); iter != manifest.obj_end(&dp) && liter != old_objs.end(); ++iter, ++liter) {
