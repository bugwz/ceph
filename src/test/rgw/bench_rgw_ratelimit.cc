...
void simulate_transfer(client_info& it, const RGWRateLimitInfo* info, std::shared_ptr<RateLimiter> ratelimit,
                       const parameters& params, spawn::yield_context& yield, boost::asio::io_context& ioctx)
{
...
void simulate_client(client_info& it, const RGWRateLimitInfo& info, std::shared_ptr<RateLimiter> ratelimit,
                     const parameters& params, spawn::yield_context& ctx, bool& to_run, boost::asio::io_context& ioctx)
{
    for (;;) {
...
void simulate_clients(boost::asio::io_context& context, std::string tenant, const RGWRateLimitInfo& info,
                      std::shared_ptr<RateLimiter> ratelimit, const parameters& params, bool& to_run)
{
    for (int i = 0; i < params.num_clients; i++) {
        auto& it = ds->emplace_back(client_info());
        it.tenant = tenant;
        int x = ds->size() - 1;
        spawn::spawn(context, [&to_run, x, ratelimit, info, params, &context](spawn::yield_context ctx) {
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        desc.add_options()("help,h", "Help screen")(
            ...
            "what is the request size we are testing if 0, it will be randomized")(
            "backend_bandwidth",
            value<int64_t>()->default_value(1),
            "what is the backend bandwidth, so there will be wait between decrease_bytes")(
    ...
    for (int i = 0; i < thread_count; i++) {
        threads.emplace_back([&]() noexcept { context.run(); });
    }
    // client execution
    bool to_run = true;
