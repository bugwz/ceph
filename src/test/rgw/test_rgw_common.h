...
struct old_rgw_bucket
{
    ...
    old_rgw_bucket(const char* t, const char* n, const char* dp, const char* ip, const char* m, const char* id,
                   const char* h)
        : tenant(t)
        , name(n)
        , data_pool(dp)
    ...
    bool operator<(const old_rgw_bucket& b) const { return name.compare(b.name) < 0; }
};
...
public:
    ...
    {
        if (i[0] == '_') return -EINVAL;
        instance = i;
        set_obj(orig_obj);
        return 0;
    }

    int clear_instance() { return set_instance(std::string()); }

    void set_loc(const std::string& k) { loc = k; }
    ...
    bool have_null_instance() { return instance == "null"; }

    bool have_instance() { return !instance.empty(); }

    bool need_to_encode_instance() { return have_instance() && !have_null_instance(); }
    ...
    std::string& get_hash_object() { return index_hash_source.empty() ? orig_obj : index_hash_source; }
    /**
    ...
    static bool parse_raw_oid(const std::string& oid, std::string* obj_name, std::string* obj_instance,
                              std::string* obj_ns)
    {
        obj_instance->clear();
        obj_ns->clear();
    ...
    void set_in_extra_data(bool val) { in_extra_data = val; }

    bool is_in_extra_data() const { return in_extra_data; }
    ...
    void decode(bufferlist::const_iterator& bl)
    {
        ...
        decode(ns, bl);
        decode(object, bl);
        if (struct_v >= 2) decode(bucket, bl);
        if (struct_v >= 4) decode(instance, bl);
        if (ns.empty() && instance.empty()) {
            if (object[0] != '_') {
                orig_obj = object;
            }
    ...
    {
        return (object.compare(o.object) == 0) && (bucket.name.compare(o.bucket.name) == 0) &&
...
static inline void prepend_old_bucket_marker(const old_rgw_bucket& bucket, const std::string& orig_oid,
                                             std::string& oid)
{
...
struct test_rgw_env
{
    ...
    test_rgw_env()
    {
        ...
        default_placement.data_pool =
            rgw_pool(zone_params.placement_pools[zonegroup.default_placement.name].get_standard_data_pool());
        default_placement.data_extra_pool =
            rgw_pool(zone_params.placement_pools[zonegroup.default_placement.name].data_extra_pool);
    }
    ...
    rgw_raw_obj get_raw(const rgw_obj_select& os) { return os.get_raw_obj(zonegroup, zone_params); }
};

void test_rgw_add_placement(RGWZoneGroup* zonegroup, RGWZoneParams* zone_params, const std::string& name,
                            bool is_default);
void test_rgw_populate_explicit_placement_bucket(rgw_bucket* b, const char* t, const char* n, const char* dp,
                                                 const char* ip, const char* m, const char* id);
void test_rgw_populate_old_bucket(old_rgw_bucket* b, const char* t, const char* n, const char* dp, const char* ip,
...
rgw_obj test_rgw_create_obj(const rgw_bucket& bucket, const std::string& name, const std::string& instance,
                            const std::string& ns);
