...
static const char* good_input =
    ...
    "<Item><Value>4</Value><ExtraValue>42</ExtraValue><NameAndStatus><Name>bar</Name><Status>False</Status></"
    "NameAndStatus></Item>"
    "</Items>";
...
std::string to_string(const Items& items)
{
    ...
    for (const auto& item : items.item_list) {
        ss << "((" << item.name_and_status.name << "," << item.name_and_status.status << ")," << item.value << ","
           << item.extra_value << ")"
           << ",";
    }
    return ss.str();
...
std::string to_string_with_attributes(const Items& items)
{
    std::stringstream ss;
    for (const auto& item : items.item_list) {
        ss << "(" << item.date << "," << item.comment << ",(" << item.name_and_status.name << ","
           << item.name_and_status.status << ")," << item.value << "," << item.extra_value << ")"
           << ",";
    }
    return ss.str();
}
...
static const char* good_input2 = "</NameAndStatus><Value>2</Value></Item>"
                                 "<Item><Value>3</Value><NameAndStatus><Name>foo</Name></NameAndStatus></Item>"
                                 "<Item><Value>4</Value><ExtraValue>42</ExtraValue><NameAndStatus><Name>bar</"
...
TEST(TestParser, Attributes)
{
    ...
    ASSERT_EQ(parser.items->item_list.size(), 4U);
    ASSERT_STREQ(to_string_with_attributes(*parser.items).c_str(), expected_output_with_attributes);
}
...
TEST(TestDecoder, BasicParsing)
{
    ...
    ASSERT_TRUE(parser.parse(good_input, strlen(good_input), 1));
    Items result;
    ASSERT_NO_THROW({ ASSERT_TRUE(RGWXMLDecoder::decode_xml("Items", result, &parser, true)); });
...
TEST(TestDecoder, MissingMandatoryTag)
{
    ...
    Items result;
    ASSERT_ANY_THROW({ ASSERT_TRUE(RGWXMLDecoder::decode_xml("Items", result, &parser, true)); });
}
...
TEST(TestDecoder, Attributes)
{
    ...
    ASSERT_NO_THROW({ ASSERT_TRUE(RGWXMLDecoder::decode_xml("Items", result, &parser, true)); });
    ASSERT_EQ(result.item_list.size(), 4U);
    ASSERT_STREQ(to_string_with_attributes(result).c_str(), expected_output_with_attributes);
}
