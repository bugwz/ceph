...
public:
    ...
    std::string get_sink() { return sink.str(); }
};
...
public:
    BlockCryptNone(){};
    BlockCryptNone(size_t sz)
        : block_size(sz)
    {}
    virtual ~BlockCryptNone(){};
    size_t get_block_size() override { return block_size; }
    bool encrypt(bufferlist& input, off_t in_ofs, size_t size, bufferlist& output, off_t stream_offset) override
    {
        output.clear();
        output.append(input.c_str(), input.length());
        return true;
    }
    bool decrypt(bufferlist& input, off_t in_ofs, size_t size, bufferlist& output, off_t stream_offset) override
    {
        output.clear();
        output.append(input.c_str(), input.length());
        return true;
...
TEST(TestRGWCrypto, verify_AES_256_CBC_identity)
{
    ...
    char* p = buf.c_str();
    for (size_t i = 0; i < buf.length(); i++) p[i] = i + i * i + (i >> 2);
    ...
    for (unsigned int step : {1, 2, 3, 5, 7, 11, 13, 17}) {
        ...
        uint8_t key[32];
        for (size_t i = 0; i < sizeof(key); i++) key[i] = i * step;
...
TEST(TestRGWCrypto, verify_AES_256_CBC_identity_2)
{
    ...
    for (unsigned int step : {1, 2, 3, 5, 7, 11, 13, 17}) {
        ...
        for (off_t end = 1; end < 6096; end += 3) {
            ...
            ASSERT_EQ(std::string_view(input.c_str(), end), std::string_view(decrypted.c_str(), end));
        }
    }
}
    ...
    for (unsigned int step : {1, 2, 3, 5, 7, 11, 13, 17}) {
        ...
        for (size_t r = 97; r < 123; r++) {
            ...
            while (pos < end) {
                ...
                if (pos + chunk > end) chunk = end - pos;
                bufferlist tmp;
            ...
            ASSERT_EQ(std::string_view(encrypted1.c_str(), end), std::string_view(encrypted2.c_str(), end));
        }
    }
}
...
TEST(TestRGWCrypto, verify_RGWGetObj_BlockDecrypt_chunks)
{
    ...
    for (off_t r = 93; r < 150; r++) {
        ...
        do {
            ...
            if (pos + size > f_end + 1) size = f_end + 1 - pos;
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup)
{
    ...
    RGWGetObj_BlockDecrypt decrypt(&no_dpp, g_ceph_context, &get_sink, std::move(nonecrypt), {});
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_simple)
{
    ...
    RGWGetObj_BlockDecrypt decrypt(
        &no_dpp, g_ceph_context, &get_sink, std::move(nonecrypt), create_mp_parts(obj_size, part_size));
    ...
    ASSERT_EQ(fixup_range(&decrypt, part_size - 2, part_size + 2), range_t(part_size - 4096, part_size + 4095));

    // request last byte
    ASSERT_EQ(fixup_range(&decrypt, obj_size - 1, obj_size - 1), range_t(obj_size - 4096, obj_size - 1));
}
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_non_aligned_obj_size)
{
    ...
    auto nonecrypt = std::make_unique<BlockCryptNone>(4096);
    RGWGetObj_BlockDecrypt decrypt(
        &no_dpp, g_ceph_context, &get_sink, std::move(nonecrypt), create_mp_parts(na_obj_size, part_size));
    ...
    ASSERT_EQ(fixup_range(&decrypt, na_obj_size - 1, na_obj_size - 1), range_t(na_obj_size - 1, na_obj_size - 1));
}
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_non_aligned_part_size)
{
    ...
    auto nonecrypt = std::make_unique<BlockCryptNone>(4096);
    RGWGetObj_BlockDecrypt decrypt(
        &no_dpp, g_ceph_context, &get_sink, std::move(nonecrypt), create_mp_parts(obj_size, na_part_size));
    ...
    ASSERT_EQ(fixup_range(&decrypt, obj_size - 1, obj_size - 1), range_t(obj_size + 5 - 4096, obj_size - 1));
}
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_non_aligned)
{
    ...
    auto nonecrypt = std::make_unique<BlockCryptNone>(4096);
    RGWGetObj_BlockDecrypt decrypt(
        &no_dpp, g_ceph_context, &get_sink, std::move(nonecrypt), create_mp_parts(na_obj_size, na_part_size));
    ...
    ASSERT_EQ(fixup_range(&decrypt, na_obj_size - 2, na_obj_size - 1), range_t(na_obj_size - 2, na_obj_size - 1));
}
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_invalid_ranges)
{
    ...
    ASSERT_EQ(fixup_range(&decrypt, obj_size - 1, obj_size + 100), range_t(obj_size - 4096, obj_size - 1));
    ASSERT_EQ(fixup_range(&decrypt, obj_size, obj_size + 1), range_t(obj_size - 1, obj_size - 1));
    ASSERT_EQ(fixup_range(&decrypt, obj_size + 1, obj_size + 100), range_t(obj_size - 1, obj_size - 1));
}
...
TEST(TestRGWCrypto, verify_RGWPutObj_BlockEncrypt_chunks)
{
    ...
    for (off_t r = 93; r < 150; r++) {
        ...
        auto cbc = AES_256_CBC_create(&no_dpp, g_ceph_context, &key[0], 32);
        ASSERT_NE(cbc.get(), nullptr);
        RGWPutObj_BlockEncrypt encrypt(&no_dpp, g_ceph_context, &put_sink, std::move(cbc));
        ...
        do {
            off_t size = 2 << ((pos * 17 + pos / 113 + r) % 16);
            size = (pos + 1117) * (pos + 2229) % size + 1;
            if (pos + size > test_size) size = test_size - pos;
        ...
        ASSERT_EQ(std::string_view(decrypted.c_str(), test_size), std::string_view(input.c_str(), test_size));
    }
}
...
TEST(TestRGWCrypto, verify_Encrypt_Decrypt)
{
    ...
    do {
        ...
        ut_put_sink put_sink;
        RGWPutObj_BlockEncrypt encrypt(
            &no_dpp, g_ceph_context, &put_sink, AES_256_CBC_create(&no_dpp, g_ceph_context, &key[0], 32));
        bufferlist bl;
        ...
        ut_get_sink get_sink;
        RGWGetObj_BlockDecrypt decrypt(
            &no_dpp, g_ceph_context, &get_sink, AES_256_CBC_create(&no_dpp, g_ceph_context, &key[0], 32), {});
