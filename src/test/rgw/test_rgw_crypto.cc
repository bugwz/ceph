...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup)
{
    ...
    RGWGetObj_BlockDecrypt decrypt(g_ceph_context, &get_sink, std::move(nonecrypt));
...
public:
    void set_parts_len(parts_len_t&& other) { parts_len = std::move(other); }
};
...
TEST(TestRGWCrypto, check_RGWGetObj_BlockDecrypt_fixup_simple)
{
    ...
    TestRGWGetObj_BlockDecrypt decrypt(g_ceph_context, &get_sink, std::move(nonecrypt));
...
TEST(TestRGWCrypto, verify_RGWPutObj_BlockEncrypt_chunks)
{
    ...
    for (off_t r = 93; r < 150; r++) {
        ...
        auto cbc = AES_256_CBC_create(g_ceph_context, &key[0], 32);
        ASSERT_NE(cbc.get(), nullptr);
        RGWPutObj_BlockEncrypt encrypt(g_ceph_context, &put_sink, std::move(cbc));
...
TEST(TestRGWCrypto, verify_Encrypt_Decrypt)
{
    ...
    do {
        ...
        ut_put_sink put_sink;
        RGWPutObj_BlockEncrypt encrypt(g_ceph_context, &put_sink, AES_256_CBC_create(g_ceph_context, &key[0], 32));
        bufferlist bl;
        ...
        ut_get_sink get_sink;
        RGWGetObj_BlockDecrypt decrypt(g_ceph_context, &get_sink, AES_256_CBC_create(g_ceph_context, &key[0], 32));
