...
RGWPeriod make_period(const std::string& id, epoch_t realm_epoch, const std::string& predecessor)
{
    RGWPeriod period(id);
...
{
    return lhs.get_id() == rhs.get_id() && lhs.get_realm_epoch() == rhs.get_realm_epoch();
}
