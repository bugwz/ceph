...
public:
    MockTransitSecretEngine(CephContext* cct, EngineParmMap parms)
        : TransitSecretEngine(cct, parms)
    {}

    MOCK_METHOD(int, send_request,
                (const char* method, std::string_view infix, std::string_view key_id, const std::string& postdata,
                 bufferlist& bl),
                (override));
};
...
public:
    MockKvSecretEngine(CephContext* cct, EngineParmMap parms)
        : KvSecretEngine(cct, parms)
    {}
...
typedef int SendRequestMethod(const char*, std::string_view, std::string_view, const std::string&, bufferlist&);
...
public:
    ...
    int Perform(const ::std::tuple<const char*, std::string_view, std::string_view, const std::string&, bufferlist&>&
                    args) override
    {
...
TEST_F(TestSSEKMS, test_transit_key_version_extraction)
{
    ...
    EXPECT_CALL(*old_engine, send_request(StrEq("GET"), StrEq(""), StrEq("1/2/3/4/5/6"), StrEq(""), _))
...
TEST_F(TestSSEKMS, test_transit_backend)
{
    ...
    EXPECT_CALL(*old_engine, send_request(StrEq("GET"), StrEq(""), StrEq("my_key/1"), StrEq(""), _))
        .WillOnce(SetPointedValue(json));
...
TEST_F(TestSSEKMS, test_transit_backend_empty_response)
{
    ...
    EXPECT_CALL(*old_engine, send_request(StrEq("GET"), StrEq(""), StrEq("/key/nonexistent/1"), StrEq(""), _))
        .WillOnce(SetPointedValue(json));
