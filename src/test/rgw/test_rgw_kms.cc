...
public:
    MockTransitSecretEngine(CephContext* cct, SSEContext& kctx, EngineParmMap parms)
        : TransitSecretEngine(cct, kctx, parms)
    {}

    MOCK_METHOD(int, send_request,
                (const DoutPrefixProvider* dpp, const char* method, std::string_view infix, std::string_view key_id,
                 const std::string& postdata, bufferlist& bl),
                (override));
};
...
public:
    MockKvSecretEngine(CephContext* cct, SSEContext& kctx, EngineParmMap parms)
        : KvSecretEngine(cct, kctx, parms)
    {}
...
typedef int SendRequestMethod(const DoutPrefixProvider* dpp, const char*, std::string_view, std::string_view,
...
public:
    ...
    SetPointedValueAction(std::string json) { this->json = json; }

    int Perform(const ::std::tuple<const DoutPrefixProvider*, const char*, std::string_view, std::string_view,
                                   const std::string&, bufferlist&>& args) override
    {
...
TEST_F(TestSSEKMS, test_transit_key_version_extraction)
{
    ...
    EXPECT_CALL(*old_engine, send_request(&no_dpp, StrEq("GET"), StrEq(""), StrEq("1/2/3/4/5/6"), StrEq(""), _))
...
TEST_F(TestSSEKMS, test_transit_backend)
{
    ...
    const NoDoutPrefix no_dpp(cct, 1);
    EXPECT_CALL(*old_engine, send_request(&no_dpp, StrEq("GET"), StrEq(""), StrEq("my_key/1"), StrEq(""), _))
        .WillOnce(SetPointedValue(json));
...
TEST_F(TestSSEKMS, test_transit_makekey)
{
    ...
    // Mocks the expected return Value from Vault Server using custom Argument Action
    string post_json =
        R"({"data": {"ciphertext": "vault:v2:HbdxLnUztGVo+RseCIaYVn/4wEUiJNT6GQfw57KXQmhXVe7i1/kgLWegEPg1I6lexhIuXAM6Q2YvY0aZ","key_version": 1,"plaintext": "3xfTra/dsIf3TMa3mAT2IxPpM7YWm/NvUb4gDfSDX4g="}})";
    EXPECT_CALL(*transit_engine,
                send_request(&no_dpp, StrEq("POST"), StrEq("/datakey/plaintext/"), StrEq("my_key"), _, _))
...
TEST_F(TestSSEKMS, test_transit_backend_empty_response)
{
    ...
    EXPECT_CALL(*old_engine, send_request(&no_dpp, StrEq("GET"), StrEq(""), StrEq("/key/nonexistent/1"), StrEq(""), _))
        .WillOnce(SetPointedValue(json));
