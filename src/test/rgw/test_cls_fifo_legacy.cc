...
namespace {
int fifo_create(const DoutPrefixProvider* dpp, R::IoCtx& ioctx, const std::string& oid, std::string_view id,
                optional_yield y, std::optional<fifo::objv> objv = std::nullopt,
                std::optional<std::string_view> oid_prefix = std::nullopt, bool exclusive = false,
                ...
                std::uint64_t max_entry_size = RCf::default_max_entry_size)
{
    ...
    RCf::create_meta(&op, id, objv, oid_prefix, exclusive, max_part_size, max_entry_size);
...
protected:
    ...
    void TearDown() override { destroy_one_pool_pp(pool_name, rados); }
};
...
TEST_F(LegacyClsFIFO, TestCreate)
{
    ...
    EXPECT_EQ(-EINVAL, r);
    r = fifo_create(&dp, ioctx, fifo_id, fifo_id, null_yield, std::nullopt, std::nullopt, false, 0);
    EXPECT_EQ(-EINVAL, r);
    r = fifo_create(
        &dp, ioctx, fifo_id, {}, null_yield, std::nullopt, std::nullopt, false, RCf::default_max_part_size, 0);
    EXPECT_EQ(-EINVAL, r);
    ...
    r = fifo_create(&dp, ioctx, fifo_id, fifo_id, null_yield);
    EXPECT_EQ(0, r);
    r = fifo_create(&dp, ioctx, fifo_id, {}, null_yield, std::nullopt, std::nullopt, false);
    EXPECT_EQ(-EINVAL, r);
    r = fifo_create(&dp, ioctx, fifo_id, {}, null_yield, std::nullopt, "myprefix"sv, false);
    EXPECT_EQ(-EINVAL, r);
    r = fifo_create(&dp, ioctx, fifo_id, "foo"sv, null_yield, std::nullopt, std::nullopt, false);
    EXPECT_EQ(-EEXIST, r);
}
...
TEST_F(LegacyClsFIFO, TestGetInfo)
{
    ...
    r = RCf::get_meta(&dp, ioctx, fifo_id, std::nullopt, &info, &part_header_size, &part_entry_overhead, 0, null_yield);
    EXPECT_EQ(0, r);
    EXPECT_GT(part_header_size, 0);
    EXPECT_GT(part_entry_overhead, 0);
    EXPECT_FALSE(info.version.instance.empty());

    r = RCf::get_meta(&dp, ioctx, fifo_id, info.version, &info, &part_header_size, &part_entry_overhead, 0, null_yield);
    EXPECT_EQ(0, r);
    fifo::objv objv;
    objv.instance = "foo";
    objv.ver = 12;
    r = RCf::get_meta(&dp, ioctx, fifo_id, objv, &info, &part_header_size, &part_entry_overhead, 0, null_yield);
...
TEST_F(LegacyFIFO, TestOpenParams)
{
    ...
    auto r =
        RCf::FIFO::create(&dp, ioctx, fifo_id, &f, null_yield, objv, oid_prefix, false, max_part_size, max_entry_size);
    ASSERT_EQ(0, r);
...
namespace {
template<class T> std::pair<T, std::string> decode_entry(const RCf::list_entry& entry)
{
    T val;
...
TEST_F(LegacyFIFO, TestPushListTrim)
{
    ...
    for (auto i = min_entry; i < max_entries; ++i) {
        std::uint32_t val;
        std::tie(val, markers[i - min_entry]) = decode_entry<std::uint32_t>(result[i - min_entry]);
        EXPECT_EQ(i, val);
    }
}
...
TEST_F(LegacyFIFO, TestPushTooBig)
{
    ...
    std::unique_ptr<RCf::FIFO> f;
    auto r = RCf::FIFO::create(
        &dp, ioctx, fifo_id, &f, null_yield, std::nullopt, std::nullopt, false, max_part_size, max_entry_size);
    ASSERT_EQ(0, r);
...
TEST_F(LegacyFIFO, TestMultipleParts)
{
    ...
    memset(buf, 0, sizeof(buf));
    const auto [part_header_size, part_entry_overhead] = f->get_part_layout_info();
    const auto entries_per_part = ((max_part_size - part_header_size) / (max_entry_size + part_entry_overhead));
...
TEST_F(LegacyFIFO, TestTwoPushersTrim)
{
    ...
    auto r = RCf::FIFO::create(
        &dp, ioctx, fifo_id, &f1, null_yield, std::nullopt, std::nullopt, false, max_part_size, max_entry_size);
    ASSERT_EQ(0, r);
...
TEST_F(LegacyFIFO, TestPushBatch)
{
    ...
    memset(buf, 0, sizeof(buf));
    auto [part_header_size, part_entry_overhead] = f->get_part_layout_info();
    auto entries_per_part = ((max_part_size - part_header_size) / (max_entry_size + part_entry_overhead));
...
TEST_F(LegacyFIFO, TestAioTrim)
{
    ...
    for (auto i = 0u; i < max_entries; ++i) {
        ...
        marker = result.front().marker;
        std::unique_ptr<R::AioCompletion> c(rados.aio_create_completion(nullptr, nullptr));
...
TEST_F(AioLegacyFIFO, TestMultipleParts)
{
    ...
    c = R::Rados::aio_create_completion();
    f->get_head_info(
        &dp, [&](int r, RCf::part_info&& p) { ASSERT_EQ(next_ofs, p.next_ofs); }, c);
    c->wait_for_complete();
    r = c->get_return_value();
    c->release();
    ASSERT_EQ(0, r);
