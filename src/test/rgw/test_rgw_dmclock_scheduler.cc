...
auto capture(std::optional<error_code>& opt_ec, std::optional<PhaseType>& opt_phase)
{
...
TEST(Queue, SyncRequest)
{
    ...
    SyncScheduler queue(
        g_ceph_context, std::ref(counters), client_info_f, server_ready_f, std::ref(SyncScheduler::handle_request_cb));
...
TEST(Queue, Cancel)
{
    boost::asio::io_context context;
    ClientCounters counters(g_ceph_context);
    AsyncScheduler queue(g_ceph_context, context, std::ref(counters), nullptr, [](client_id client) -> ClientInfo* {
...
auto capture(const Executor& ex, std::optional<error_code>& opt_ec, std::optional<PhaseType>& opt_res)
{
...
TEST(Queue, CrossExecutorRequest)
{
    ...
    ClientCounters counters(g_ceph_context);
    AsyncScheduler queue(
        g_ceph_context, queue_context, std::ref(counters), nullptr, [](client_id client) -> ClientInfo* {
            static ClientInfo info{0, 1, 1};
            return &info;
        });
