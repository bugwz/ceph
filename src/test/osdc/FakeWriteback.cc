...
public:
    C_Delay(CephContext* cct, Context* c, ceph::mutex* lock, uint64_t off, bufferlist* pbl, uint64_t delay_ns = 0)
        : m_cct(cct)
        , m_con(c)
...
void FakeWriteback::read(const object_t& oid, uint64_t object_no, const object_locator_t& oloc, uint64_t off,
...
ceph_tid_t FakeWriteback::write(const object_t& oid, const object_locator_t& oloc, uint64_t off, uint64_t len,
