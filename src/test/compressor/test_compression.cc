...
class CompressorTest : public ::testing::Test, public ::testing::WithParamInterface<const char*>
{
public:
    std::string plugin;
    ...
    void TearDown() override { compressor.reset(); }
};
...
TEST_P(CompressorTest, small_round_trip)
{
    ...
    cout << "orig " << orig.length() << " compressed " << compressed.length() << " with " << GetParam() << std::endl;
}
...
#include "osdmaps/osdmap.2982809.h"
    ...
    for (int j = 0; j < 3; j++) {
        ...
        cout << "chunk " << chunk.length() << " compressed " << compressed.length() << " decompressed "
             << decompressed.length() << " with " << plugin << std::endl;
    }
    delete o;
}
...
INSTANTIATE_TEST_SUITE_P(Compressor, CompressorTest,
                         ::testing::Values(
#ifdef HAVE_LZ4
                             "lz4",
#endif
...
TEST(ZlibCompressor, zlib_isal_compatibility)
{
    ...
    char test[101];
    srand(time(0));
    for (int i = 0; i < 100; ++i) test[i] = 'a' + rand() % 26;
    test[100] = '\0';
    int len = strlen(test);
    bufferlist in, out;
    in.append(test, len);
...
TEST(ZlibCompressor, isal_compress_zlib_decompress_walk)
{
    ...
    for (int cnt = 0; cnt < 100; cnt++) {
        ...
        char test[size];
        test[0] = rand() % 256;
        for (int i = 1; i < size; ++i) test[i] = test[i - 1] + rand() % (range * 2 + 1) - range;
        bufferlist in, out;
        in.append(test, size);
