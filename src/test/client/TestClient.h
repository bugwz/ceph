...
{
public:
    ClientScaffold(Messenger* m, MonClient* mc, Objecter* objecter_)
        : Client(m, mc, objecter_)
    {}
...
public:
    ...
    static void TearDownTestSuite() { icp.stop(); }
    void SetUp() override
    {
    ...
    {
        if (client->is_mounted()) client->unmount();
        client->shutdown();
        objecter->shutdown();
        mc->shutdown();
        messenger->shutdown();
