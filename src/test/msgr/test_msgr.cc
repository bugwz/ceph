...
#define CHECK_AND_WAIT_TRUE(expr) \
    do {                          \
        int n = 1000;             \
        while (--n) {             \
            if (expr) break;      \
...
public:
    ...
    void SetUp() override
    {
        ...
        client_msgr =
            Messenger::create(g_ceph_context, string(GetParam()), entity_name_t::CLIENT(-1), "client", getpid());
...
public:
    struct Session : public RefCountedObject
    {
        ...
        explicit Session(ConnectionRef c)
    ...
    explicit FakeDispatcher(bool s)
        : Dispatcher(g_ceph_context)
        , is_server(s)
        , got_new(false)
        , got_remote_reset(false)
        , got_connect(false)
        , loopback(false)
    {}
    ...
    {
        switch (m->get_type()) {
        case CEPH_MSG_PING: return true;
    ...
    void ms_handle_fast_connect(Connection* con) override
    {
        ...
        if (!s) {
            ...
            lderr(g_ceph_context) << __func__ << " con: " << con << " count: " << session->count << dendl;
        }
        got_connect = true;
        cond.notify_all();
    }
    ...
    bool ms_dispatch(Message* m) override
    {
        ...
        s->count++;
        lderr(g_ceph_context) << __func__ << " conn: " << m->get_connection() << " session " << s
                              << " count: " << s->count << dendl;
        if (is_server) {
            reply_message(m);
        }
    ...
    int ms_handle_fast_authentication(Connection* con) override { return 1; }
...
struct TestInterceptor : public Interceptor
{
    ...
    void breakpoint(uint32_t step) { breakpoints.insert(step); }

    void remove_bp(uint32_t step) { breakpoints.erase(step); }
    ...
    {
        lderr(g_ceph_context) << __func__ << " conn(" << conn << ") intercept called on step=" << step << dendl;
        ...
        if (breakpoints.find(step) != breakpoints.end()) {
            lderr(g_ceph_context) << __func__ << " conn(" << conn << ") pausing on step=" << step << dendl;
        ...
        lderr(g_ceph_context) << __func__ << " conn(" << conn << ") resuming step=" << step
                              << " with decision=" << decision << dendl;
...
TEST_P(MessengerTest, ConnectionRaceTest)
{
    ...
    ConnectionRef s2c = server_msgr->connect_to(client_msgr->get_mytype(), client_msgr->get_myaddrs());
    MPing* m2 = new MPing();
...
TEST_P(MessengerTest, SimpleTest)
{
    ...
    // 1. simple round trip
    MPing* m = new MPing();
    ConnectionRef conn = client_msgr->connect_to(server_msgr->get_mytype(), server_msgr->get_myaddrs());
    {
    ...
    conn = client_msgr->connect_to(server_msgr->get_mytype(), server_msgr->get_myaddrs());
    {
        m = new MPing();
        ASSERT_EQ(conn->send_message(m), 0);
        std::unique_lock l{cli_dispatcher.lock};
...
TEST_P(MessengerTest, FeatureTest)
{
    ...
    for (int i = 0; i < 10; i++) feature_supported |= 1ULL << i;
...
TEST_P(MessengerTest, StatefulTest)
{
    ...
    conn->mark_down();
    ASSERT_FALSE(conn->is_connected());
    ConnectionRef server_conn = server_msgr->connect_to(client_msgr->get_mytype(), srv_dispatcher.last_accept);
    // don't lose state
    ...
    ASSERT_EQ(1U, static_cast<Session*>(conn->get_priv().get())->get_count());
    server_conn = server_msgr->connect_to(client_msgr->get_mytype(), srv_dispatcher.last_accept);
    {
        std::unique_lock l{srv_dispatcher.lock};
...
TEST_P(MessengerTest, AnonTest)
{
    ...
    ConnectionRef con_a = client_msgr->connect_to(server_msgr->get_mytype(), server_msgr->get_myaddrs(), true);
    {
        m = new MPing();
    ...
    ConnectionRef con_b = client_msgr->connect_to(server_msgr->get_mytype(), server_msgr->get_myaddrs(), true);
    {
        m = new MPing();
...
TEST_P(MessengerTest, MessageTest)
{
    ...
    ConnectionRef conn = client_msgr->connect_to(server_msgr->get_mytype(), server_msgr->get_myaddrs());
    {
        ...
        for (int i = 0; i < 1024 * 30; i++) cmds.push_back(s);
...
public:
    ...
    void ms_handle_fast_connect(Connection* con) override
    {
        ...
        for (list<uint64_t>::iterator it = c.begin(); it != c.end(); ++it) sent.erase(*it);
        conn_sent.erase(con);
        got_connect = true;
        cond.notify_all();
    }
    ...
    void reply_message(const Message* m, Payload& pl)
    {
        ...
        rm->set_data(bl);
        m->get_connection()->send_message(rm);
        lderr(g_ceph_context) << __func__ << " conn=" << m->get_connection() << " reply m=" << m << " i=" << pl.seq
                              << dendl;
    }
    ...
    void clear_pending(ConnectionRef con)
    {
        ...
        for (list<uint64_t>::iterator it = conn_sent[con].begin(); it != conn_sent[con].end(); ++it) sent.erase(*it);
        conn_sent.erase(con);
    }
...
public:
    ...
    SyntheticWorkload(int servers, int clients, string type, int random_num, Messenger::Policy srv_policy,
                      Messenger::Policy cli_policy, int _max_in_flight = 64, int _max_connections = 128)
        ...
        , max_connections(_max_connections)
    {
        ...
        for (int i = 0; i < servers; ++i) {
            msgr = Messenger::create(g_ceph_context, type, entity_name_t::OSD(0), "server", getpid() + i);
            snprintf(addr, sizeof(addr), "v2:127.0.0.1:%d", base_port + i);
        ...
        for (int i = 0; i < clients; ++i) {
            msgr = Messenger::create(g_ceph_context, type, entity_name_t::CLIENT(-1), "client", getpid() + i + servers);
            if (cli_policy.standby) {
                snprintf(addr, sizeof(addr), "v2:127.0.0.1:%d", base_port + i + servers);
                bind_addr.parse(addr);
                msgr->bind(bind_addr);
            }
            msgr->add_dispatcher_head(&dispatcher);
    ...
    {
        std::lock_guard l{lock};
        if (!can_create_connection()) return;
        ...
        pair<Messenger*, Messenger*> p;
        {
            ...
            if (server->get_default_policy().server) {
                ...
                ConnectionRef conn = client->connect_to(server->get_mytype(), server->get_myaddrs());
    ...
    {
        std::lock_guard l{lock};
        if (available_connections.size() < 10) return;
        ConnectionRef conn = _get_random_connection();
        dispatcher.clear_pending(conn);
        conn->mark_down();
        if (!client_policy.server && !client_policy.lossy && client_policy.standby) {
            ...
            if (!p.first->get_default_policy().server && !p.second->get_default_policy().server) {
                ...
                ConnectionRef peer = p.second->connect_to(p.first->get_mytype(), p.first->get_myaddrs());
                peer->mark_down();
    ...
    void wait_for_done()
    {
        ...
        while (dispatcher.get_pending()) {
            usleep(tick_us);
            timeout_us -= tick_us;
            if (i++ % 50 == 0) print_internal_state(true);
            if (timeout_us < 0) ceph_abort_msg(" loop time exceed 5 mins, it looks we stuck into some problems!");
        }
        for (set<Messenger*>::iterator it = available_servers.begin(); it != available_servers.end(); ++it) {
            (*it)->shutdown();
            (*it)->wait();
        ...
        for (set<Messenger*>::iterator it = available_clients.begin(); it != available_clients.end(); ++it) {
            (*it)->shutdown();
            (*it)->wait();
            ASSERT_EQ((*it)->get_dispatch_queue_len(), 0);
            delete (*it);
...
TEST_P(MessengerTest, SyntheticStressTest)
{
    SyntheticWorkload test_msg(
        8, 32, GetParam(), 100, Messenger::Policy::stateful_server(0), Messenger::Policy::lossless_client(0));
    for (int i = 0; i < 100; ++i) {
...
{
    SyntheticWorkload test_msg(
        16, 32, GetParam(), 100, Messenger::Policy::lossless_peer_reuse(0), Messenger::Policy::lossless_peer_reuse(0));
    for (int i = 0; i < 10; ++i) {
        if (!(i % 10)) lderr(g_ceph_context) << "seeding connection " << i << dendl;
        test_msg.generate_connection();
    }
...
TEST_P(MessengerTest, SyntheticInjectTest)
{
    ...
    g_ceph_context->_conf.set_val("ms_dispatch_throttle_bytes", std::to_string(dispatch_throttle_bytes));
}
...
TEST_P(MessengerTest, SyntheticInjectTest2)
{
    ...
    g_ceph_context->_conf.set_val("ms_inject_internal_delays", "0.1");
    SyntheticWorkload test_msg(
        8, 16, GetParam(), 100, Messenger::Policy::lossless_peer_reuse(0), Messenger::Policy::lossless_peer_reuse(0));
    for (int i = 0; i < 100; ++i) {
        if (!(i % 10)) lderr(g_ceph_context) << "seeding connection " << i << dendl;
        test_msg.generate_connection();
    }
...
TEST_P(MessengerTest, SyntheticInjectTest3)
{
    ...
    g_ceph_context->_conf.set_val("ms_inject_internal_delays", "0.1");
    SyntheticWorkload test_msg(
        8, 16, GetParam(), 100, Messenger::Policy::stateless_server(0), Messenger::Policy::lossy_client(0));
    for (int i = 0; i < 100; ++i) {
        if (!(i % 10)) lderr(g_ceph_context) << "seeding connection " << i << dendl;
        test_msg.generate_connection();
    }
...
TEST_P(MessengerTest, SyntheticInjectTest4)
{
    ...
    SyntheticWorkload test_msg(
        16, 32, GetParam(), 100, Messenger::Policy::lossless_peer(0), Messenger::Policy::lossless_peer(0));
    for (int i = 0; i < 100; ++i) {
        if (!(i % 10)) lderr(g_ceph_context) << "seeding connection " << i << dendl;
        test_msg.generate_connection();
    }
...
{
    SyntheticWorkload test_msg(
        1, 8, GetParam(), 100, Messenger::Policy::stateful_server(0), Messenger::Policy::lossless_client(0), 64, 2);
    bool simulate_network_congestion = true;
    for (int i = 0; i < 2; ++i) test_msg.generate_connection();
    for (int i = 0; i < 5000; ++i) {
        if (!(i % 10)) {
...
public:
    ...
    explicit MarkdownDispatcher(bool s)
        : Dispatcher(g_ceph_context)
        , last_mark(false)
    {}
    ...
    bool ms_dispatch(Message* m) override
    {
        ...
        if (conns.empty()) last_mark = false;
        m->put();
        return true;
    }
    bool ms_handle_reset(Connection* con) override
...
{
    Messenger* server_msgr2 =
        Messenger::create(g_ceph_context, string(GetParam()), entity_name_t::OSD(0), "server", getpid());
    ...
    while (i--) {
        ConnectionRef conn1 = client_msgr->connect_to(server_msgr->get_mytype(), server_msgr->get_myaddrs());
        ConnectionRef conn2 = client_msgr->connect_to(server_msgr2->get_mytype(), server_msgr2->get_myaddrs());
        MPing* m = new MPing();
        ...
        if (equal_count) usleep(1000 * 500);
...
INSTANTIATE_TEST_SUITE_P(Messenger, MessengerTest, ::testing::Values("async+posix"));
