...
public:
    NoopConfigObserver(std::list<std::string> l)
        : options(l)
    {
    ...
    ~NoopConfigObserver() { delete[] ptrs; }

    const char** get_tracked_conf_keys() const override { return ptrs; }
...
public:
    ...
    void TearDown() override { stack->stop(); }
    string get_addr() const { return addr; }
    string get_ip_different_port() const { return port_addr; }
    string get_different_ip() const { return "10.0.123.100:4323"; }
    EventCenter* get_center(unsigned i) { return &stack->get_worker(i)->center; }
    Worker* get_worker(unsigned i) { return stack->get_worker(i); }
    template<typename func> class C_dispatch : public EventCallback
    {
        Worker* worker;
        func f;
    ...
    template<typename func> void exec_events(func&& f)
    {
...
public:
    explicit C_poll(EventCenter* c)
        ...
        , woken(false)
    {}
    void do_request(uint64_t r) override { woken = true; }
    bool poll(int milliseconds)
    {
        ...
        while (!woken) {
            ...
            usleep(sleepus);
            auto r = std::chrono::duration_cast<std::chrono::milliseconds>(ceph::coarse_real_clock::now() - start);
            if (r >= std::chrono::milliseconds(milliseconds)) break;
        }
        return woken;
    }
    void reset() { woken = false; }
};
...
TEST_P(NetworkWorkerTest, AcceptAndCloseTest)
{
    ...
    exec_events([this, bind_addr, accepted_p, count_p](Worker* worker) mutable {
        ...
        int r = 0;
        {
            ...
            if (bind_socket) center->create_file_event(bind_socket.fd(), EVENT_READABLE, &cb);
            if (worker->id == 0) {
                *accepted_p = false;
                r = worker->connect(bind_addr, options, &cli_socket);
                ASSERT_EQ(0, r);
    ...
    exec_events([this, bind_addr, listen_p, accepted_p, done_p](Worker* worker) mutable {
        ...
        if (worker->id == 1) {
            while (!*listen_p || stack->support_local_listen_table()) {
                ...
                r = worker->connect(bind_addr, options, &cli_socket);
                ASSERT_EQ(0, r);
                if (stack->support_local_listen_table()) break;
            }
        }
        ...
        for (size_t i = 0; i < message_size; i += 100) message[i] = ',';
        size_t len = message_size * count;
        C_poll cb(center);
        if (worker->id == 1) center->create_file_event(cli_socket.fd(), EVENT_WRITABLE, &cb);
        if (srv_socket) center->create_file_event(srv_socket.fd(), EVENT_READABLE, &cb);
        size_t left = len;
        len *= 2;
        string read_string;
        int again_count = 0;
        int c = 2;
        bufferlist bl;
        for (size_t i = 0; i < count; ++i) bl.push_back(bufferptr((char*)message.data(), message_size));
        while (!*done_p) {
            again_count = 0;
            if (worker->id == 1) {
                if (c > 0) {
        ...
        if (worker->id == 1) center->delete_file_event(cli_socket.fd(), EVENT_WRITABLE);
        if (srv_socket) center->delete_file_event(srv_socket.fd(), EVENT_READABLE);

        if (bind_socket) bind_socket.abort_accept();
        if (srv_socket) srv_socket.close();
        if (worker->id == 1) cli_socket.close();
    });
}
...
class StressFactory
{
    ...
    struct ThreadData
    {
        ...
        ~ThreadData()
        {
            for (auto&& i : clients) delete i;
            for (auto&& i : servers) delete i;
        }
    };
    ...
    struct RandomString
    {
        ...
        void prepare(size_t n)
        {
            ...
            std::generate_n(std::back_inserter(strs), strs.capacity(), [&] {
                std::string str;
                str.reserve(slen);
                std::generate_n(std::back_inserter(str), slen, [&]() { return alphabet[dist(rng)]; });
                return str;
            });
        }
    ...
    struct Message
    {
            ...
            : idx(i)
        {
            ...
            std::generate_n(std::back_inserter(strs), strs.capacity(), [&] { return rs.get_random_string(); });
            len = slen * strs.size();
            content.reserve(len);
            for (auto&& s : strs) content.append(s);
        }
        bool verify(const char* b, size_t len = 0) const { return content.compare(0, len, b, 0, len) == 0; }
    };

    template<typename T> class C_delete : public EventCallback
    {
        T* ctxt;
    ...
    class Client
    {
        ...
        public:
            explicit Client_read_handle(Client* _c)
                : c(_c)
            {}
            void do_request(uint64_t id) override { c->do_read_request(); }
        } read_ctxt;
        ...
        public:
            explicit Client_write_handle(Client* _c)
                : c(_c)
            {}
            void do_request(uint64_t id) override { c->do_write_request(); }
        } write_ctxt;
    ...
    public:
        ...
        void do_read_request()
        {
            ...
            if (first) {
                ...
                if (acking.empty()) return;
            }
            StressFactory::Message* m = acking.front();
            int r = 0;
            if (buffer.empty()) buffer.resize(m->len);
            bool must_no = false;
            while (true) {
                r = socket.read((char*)buffer.data() + read_offset, m->len - read_offset);
        ...
        void do_write_request()
        {
            ...
            while (left > 0 && factory->queue_depth > writings.size() + acking.size()) {
                StressFactory::Message* m =
                    new StressFactory::Message(factory->rs, ++index, factory->rd() % factory->max_message_length);
            ...
            if (writings.empty() && write_enabled) {
                ...
                ASSERT_EQ(0, center->create_file_event(socket.fd(), EVENT_WRITABLE, &write_ctxt));
                write_enabled = true;
            }
        }

        bool finish() const { return left == 0 && acking.empty() && writings.empty(); }
    };
    friend class Client;
    ...
    class Server
    {
        ...
        public:
            explicit Server_read_handle(Server* _s)
                : s(_s)
            {}
            void do_request(uint64_t id) override { s->do_read_request(); }
        } read_ctxt;
        ...
        public:
            explicit Server_write_handle(Server* _s)
                : s(_s)
            {}
            void do_request(uint64_t id) override { s->do_write_request(); }
        } write_ctxt;
    ...
    public:
        ...
        void do_read_request()
        {
            ...
            if (!buffers.empty() && !write_enabled) center->dispatch_event_external(&write_ctxt);
        }
...
public:
    ...
    explicit StressFactory(const std::shared_ptr<NetworkStack>& s, const string& addr, size_t cli, size_t qd, size_t mc,
                           size_t l)
        : stack(s)
        , rs(128)
        , client_num(cli)
    ...
    void add_client(ThreadData* t_data)
    {
        ...
        std::uniform_int_distribution<> dist(min_client_send_messages, max_client_send_messages);
        ASSERT_EQ(0, r);
        int c = dist(rng);
        if (c > message_count.load()) c = message_count.load();
    ...
    void start(Worker* worker)
    {
        ...
        if (bind_fd) worker->center.delete_file_event(bind_fd, EVENT_READABLE);
...
INSTANTIATE_TEST_SUITE_P(NetworkStack, NetworkWorkerTest,
                         ::testing::Values(
#ifdef HAVE_DPDK
                             "dpdk",
#endif
