...
#ifdef HAVE_EPOLL
        if (strcmp(GetParam(), "epoll")) driver = new EpollDriver(g_ceph_context);
#endif
#ifdef HAVE_KQUEUE
        if (strcmp(GetParam(), "kqueue")) driver = new KqueueDriver(g_ceph_context);
#endif
        if (strcmp(GetParam(), "select")) driver = new SelectDriver(g_ceph_context);
...
TEST_P(EventDriverTest, NetworkSocketTest)
{
    ...
    do {
        ...
        driver->del_event(client_sd, EVENT_READABLE | EVENT_WRITABLE, EVENT_WRITABLE);
    } while (1);
...
TEST(EventCenterTest, FileEventExpansion)
{
    ...
    for (vector<int>::iterator it = sds.begin(); it != sds.end(); ++it) center.delete_file_event(*it, EVENT_READABLE);
}
...
public:
    ...
    {
        center.set_owner();
        while (!done) center.process_events(1000000);
        return 0;
    }
};
...
public:
    CountEvent(std::atomic<unsigned>* atomic, ceph::mutex* l, ceph::condition_variable* c)
        : count(atomic)
        , lock(l)
        , cond(c)
    {}
...
INSTANTIATE_TEST_SUITE_P(AsyncMessenger, EventDriverTest,
                         ::testing::Values(
#ifdef HAVE_EPOLL
                             "epoll",
#endif
