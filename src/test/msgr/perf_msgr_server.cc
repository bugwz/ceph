...
class ServerDispatcher : public Dispatcher
{
    ...
    public:
        ...
        void _dequeue(Message* m) override { ceph_abort(); }
        bool _empty() override { return messages.empty(); }
        ...
        void _clear() override { ceph_assert(messages.empty()); }
    } op_wq;
...
public:
    ...
    {
        switch (m->get_type()) {
        case CEPH_MSG_OSD_OP: return true;
...
void usage(const string& name)
{
    ...
    cerr << "       [server worker threads]: threads will process incoming messages and reply(matching pg threads)"
         << std::endl;
