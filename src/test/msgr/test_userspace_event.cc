...
public:
    ...
    virtual void SetUp() { manager = new UserspaceEventManager(g_ceph_context); }
    virtual void TearDown() { delete manager; }
};
...
TEST_F(UserspaceManagerTest, StressTest)
{
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        if (value > 55) {
            ...
            for (auto&& d : fds) expected.insert(d);
            ...
            ASSERT_TRUE(r > 0);
            if ((size_t)r >= mappings.size()) mappings.resize(r + 1);
