...
class MessengerClient
{
    ...
    public:
        ClientDispatcher(uint64_t delay, ClientThread* t)
        ...
        {
            switch (m->get_type()) {
            case CEPH_MSG_OSD_OPREPLY: return true;
...
public:
    ...
    ~MessengerClient()
    {
        for (uint64_t i = 0; i < clients.size(); ++i) delete clients[i];
    ...
    void start()
    {
        for (uint64_t i = 0; i < clients.size(); ++i) clients[i]->create("client");
        for (uint64_t i = 0; i < msgrs.size(); ++i) msgrs[i]->wait();
    }
};
...
{
    cout << "Usage: " << name << " [server ip:port] [numjobs] [concurrency] [ios] [thinktime us] [msg length]"
         << std::endl;
...
int main(int argc, char** argv)
{
    ...
    std::string public_msgr_type = g_ceph_context->_conf->ms_public_type.empty()
    ...
    cout << " Total op " << (ios * numjobs) << " run time " << Cycles::to_microseconds(stop - start) << "us."
         << std::endl;
