...
#define ERR_CHECK_NEXT_PARAM(o)  \
    ...
    if (!host.length() || !rgw_admin_path.length()) return -1;
    return 0;
}
...
static int get_s3_auth(const string& method, string creds, const string& date, string res, string& out)
{
    ...
    if (off != string::npos) {
        ...
        admin_log::calc_hmac_sha1(secret.c_str(), secret.length(), auth_hdr.c_str(), auth_hdr.length(), hmac_sha1);
                              ...
                              void* ud, size_t length)
{
    ...
    if (curl_inst) {
        ...
        string s3auth;
        if (admin_log::get_s3_auth(method, creds, date, res, s3auth) < 0) return -1;
...
{
    stringstream ss;
    ss << "-c " << g_test->get_ceph_conf_path() << " metadata rm --metadata-key=user:" << uid;
...
int caps_add(const char* name, const char* perm)
{
    ...
    ss << "-c " << g_test->get_ceph_conf_path() << " caps add --caps=" << name << "=" << perm << " --uid=" << uid;
    string out;
    string cmd = ss.str();
    if (run_rgw_admin(cmd, out) != 0) {
        cout << "Error creating user" << std::endl;
...
int caps_rm(const char* name, const char* perm)
{
    ...
    ss << "-c " << g_test->get_ceph_conf_path() << " caps rm --caps=" << name << "=" << perm << " --uid=" << uid;
    string out;
    string cmd = ss.str();
    if (run_rgw_admin(cmd, out) != 0) {
        cout << "Error creating user" << std::endl;
...
static int put_bucket_obj(const char* obj_name, char* data, unsigned len)
{
    ...
    req.append(obj_name);
    g_test->send_request(string("PUT"), req, read_bucket_object, (void*)data, (size_t)len);
    if (g_test->get_resp_code() != 200U) {
...
{
    JSONParser parser;
    if (parse_json_resp(parser) != 0) return -1;
    if (!parser.is_array()) return -1;

    vector<string> l;
    l = parser.get_array_elements();
    int loop = 0;
    for (vector<string>::iterator it = l.begin(); it != l.end(); ++it, loop++) {
        JSONParser jp;
...
static int decode_json(JSONObj* obj, rgw_data_change& ret)
{
    ...
    if (entity.compare("bucket") == 0) ret.entity_type = ENTITY_TYPE_BUCKET;
...
TEST(TestRGWAdmin, datalog_list)
{
    ...
    ss.str("");
    ss << "/admin/log?type=data&id=" << shard_id << "&start-time=" << start_time << "&max-entries=1";
    rest_req = ss.str();
    g_test->send_request(string("GET"), rest_req);
    EXPECT_EQ(200U, g_test->get_resp_code());
    entries.clear();
    ...
    ss.str("");
    ss << "/admin/log?type=data&id=" << shard_id << "&start-time=" << start_time << "&end-time=" << end_time;
    rest_req = ss.str();
    g_test->send_request(string("GET"), rest_req);
    EXPECT_EQ(200U, g_test->get_resp_code());
    entries.clear();
...
{
    string start_time, end_time, start_time_2;
    const char *cname = "mdlog", *perm = "*";
    string rest_req;
    ...
    ss.str("");
    ss << "/admin/log?type=metadata&id=" << shard_id << "&start-time=" << start_time << "&max-entries=" << 1;
    rest_req = ss.str();
    g_test->send_request(string("GET"), rest_req);
    EXPECT_EQ(200U, g_test->get_resp_code());
    entries.clear();
    ...
    ss.str("");
    ss << "/admin/log?type=metadata&id=" << shard_id << "&start-time=" << start_time << "&max-entries=" << 6;
    rest_req = ss.str();
    g_test->send_request(string("GET"), rest_req);
    EXPECT_EQ(200U, g_test->get_resp_code());
    entries.clear();
