...
std::string temp_pool_name(const std::string_view prefix)
{
    ...
    return fmt::format("{}-{}-{}-{}-{}",
                       ...
                       hostname(),
                       getpid(),
                       duration_cast<milliseconds>(ceph::coarse_real_clock::now().time_since_epoch()).count(),
                       num++);
}
...
bs::error_code noisy_list(R::RADOS& r, int64_t p)
{
    ...
    try {
        auto [v, next] = r.enumerate_objects(p, b, e, 1000, {}, ca::use_blocked, R::all_nspaces);
...
bs::error_code create_several(R::RADOS& r, const R::IOContext& i, std::initializer_list<std::string> l)
{
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        auto pd = make_scope_guard([&pool_name, &r]() { r.delete_pool(pool_name, ca::use_blocked); });
