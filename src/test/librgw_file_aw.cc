...
TEST(LibRGW, MOUNT)
{
    int ret = rgw_mount2(rgw, userid.c_str(), access_key.c_str(), secret_key.c_str(), "/", &fs, RGW_MOUNT_FLAG_NONE);
    ASSERT_EQ(ret, 0);
...
TEST(LibRGW, LARGE1)
{
    if (do_large) {
        ...
        for (int ix = 0; ix < iovcnt; ++ix) {
            struct iovec* iov = &iovs[ix];
            // write iov->iov_len
            ret = rgw_write(fs, object_fh, offset, iov->iov_len, &nbytes, iov->iov_base, RGW_WRITE_FLAG_NONE);
            offset += iov->iov_len;
            ASSERT_EQ(ret, 0);
            ASSERT_EQ(nbytes, iov->iov_len);
        }
    ...
    if (do_large) {
        ...
        if (do_verify) {
            ...
            for (int ix = 0; ix < iovcnt; ++ix) {
                ...
                ret = rgw_read(fs, object_fh, offset2, iov2->iov_len, &nread, iov2->iov_base, RGW_READ_FLAG_NONE);
                iov2->iov_len = nread;
