...
template<typename T> static int basic_timer_test(T& timer, ceph::mutex* lock)
{
    int ret = 0;
    ...
    for (int i = 0; i < MAX_TEST_CONTEXTS; ++i) {
        if (test_array[i] != i) {
            ret = 1;
            cout << "error: expected test_array[" << i << "] = " << i << "; got " << test_array[i] << " instead."
                 << std::endl;
        }
    }
...
static int test_out_of_order_insertion(SafeTimer& timer, ceph::mutex* lock)
{
    ...
    if (secs == 100) {
        ret = 1;
        cout << "error: expected test_array[" << 1 << "] = " << 1 << "; got " << test_array[1] << " instead."
             << std::endl;
    }
...
static int safe_timer_cancel_all_test(SafeTimer& safe_timer, ceph::mutex& safe_timer_lock)
{
    cout << __PRETTY_FUNCTION__ << std::endl;
...
static int safe_timer_cancellation_test(SafeTimer& safe_timer, ceph::mutex& safe_timer_lock)
{
    cout << __PRETTY_FUNCTION__ << std::endl;
