...
public:
    ...
    ~test_cors_helper() { curl_global_cleanup(); }
...
#define ERR_CHECK_NEXT_PARAM(o) \
    ...
    if (host.empty() || creds.empty()) return -1;
    return 0;
}
...
int test_cors_helper::send_request(string method, string res, size_t (*read_function)(void*, size_t, size_t, void*),
...
void send_cors(set<string> o, set<string> h, list<string> e, uint8_t flags, unsigned max_age)
{
    if (g_test->get_key_type() == KEY_TYPE_S3) {
        ...
        g_test->send_request(
            string("PUT"), string("/" S3_BUCKET_NAME "?cors"), cors_read_xml, (void*)&ss, ss.str().length());
    }
    else if (g_test->get_key_type() == KEY_TYPE_SWIFT) {
        ...
        stringstream ss;
        ss << "1";
        g_test->send_request(string("POST"), string("/swift/v1/" SWIFT_BUCKET_NAME), cors_read_xml, (void*)&ss, 1);
    }
}
...
TEST(TestCORS, putcors_firsttime)
{
    ...
    if (g_test->get_key_type() == KEY_TYPE_S3) {
        ...
        EXPECT_TRUE((r->get_allowed_methods() & (RGW_CORS_GET | RGW_CORS_PUT)) == (RGW_CORS_GET | RGW_CORS_PUT));
    }
    ASSERT_EQ(0, delete_bucket());
}
...
TEST(TestCORS, optionscors_test_options_7)
{
    ...
    if (g_test->get_resp_code() == 200) {
        ...
        s = g_test->get_response(string("Access-Control-Allow-Headers"));
        EXPECT_EQ(0,
                  s.compare("Header1,Header2,Header3,"
                            "Hdr--Length,Hdr-1-Length,Header-Length,Content-Length,foofoofoo"));
    }
    ASSERT_EQ(0, delete_bucket());
}
