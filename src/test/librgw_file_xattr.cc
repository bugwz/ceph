...
TEST(LibRGW, MOUNT)
{
    int ret = rgw_mount(rgw_h, uid.c_str(), access_key.c_str(), secret_key.c_str(), &fs, RGW_MOUNT_FLAG_NONE);
    ASSERT_EQ(ret, 0);
...
{
    int ret = rgw_lookup(fs, fs->root_fh, bucket_name.c_str(), &bucket_fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
    ASSERT_EQ(ret, 0);
}
...
TEST(LibRGW, CREATE_BUCKET)
{
    if ((!bucket_fh) && do_create) {
        ...
        int ret = rgw_mkdir(fs, fs->root_fh, bucket_name.c_str(), &st, create_mask, &bucket_fh, RGW_MKDIR_FLAG_NONE);
        ASSERT_EQ(ret, 0);
    }
}
...
TEST(LibRGW, CREATE_PATH)
{
    ...
    for (ix = 0; ix < sz; ++ix) {
        ...
        if (do_create) {
            ret = rgw_mkdir(fs, dir.parent_fh, dir.name.c_str(), &st, create_mask, &dir.fh, RGW_MKDIR_FLAG_NONE);
        }
        else {
            ret = rgw_lookup(fs, dir.parent_fh, dir.name.c_str(), &dir.fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
        }
        ASSERT_EQ(ret, 0);
        dir.sync();
        ovec.push_back(dir);
...
TEST(LibRGW, CHECK_PATH_REFS)
{
    ...
    for (ix = 0; ix < sz; ++ix) {
        auto& dir = ovec[ix];
        if (verbose) {
            std::cout << "name: " << dir.name << " refcnt: " << dir.rgw_fh->get_refcnt() << std::endl;
        }
        if (ix == 0) {
...
TEST(LibRGW, SETXATTR1)
{
    ...
    rgw_xattrstr xattr_k = {const_cast<char*>(key1.c_str()), uint32_t(key1.length())};
    rgw_xattrstr xattr_v = {const_cast<char*>(val1.c_str()), uint32_t(val1.length())};
...
{
    auto& attrmap = *(static_cast<std::map<std::string, std::string>*>(arg));
    for (uint32_t ix = 0; ix < attrs->xattr_cnt; ++ix) {
        ...
        if (verbose) {
            std::cout << __func__ << " attr k: " << k << " v: " << v << std::endl;
        }
...
TEST(LibRGW, GETXATTR1)
{
    ...
    rgw_xattrstr xattr_k1 = {const_cast<char*>(key1.c_str()), uint32_t(key1.length())};
    rgw_xattrstr xattr_v1 = {nullptr, 0};

    std::string key2 = "user.rgw.etag";
    rgw_xattrstr xattr_k2 = {const_cast<char*>(key2.c_str()), uint32_t(key2.length())};
    rgw_xattrstr xattr_v2 = {nullptr, 0};

    rgw_xattr xattrs[2] = {{xattr_k1, xattr_v1}, {xattr_k2, xattr_v2}};
    ...
    int ret = rgw_getxattrs(fs, dir.fh, &xattrlist, getattr_cb, &out_attrmap, RGW_GETXATTR_FLAG_NONE);
    ASSERT_EQ(ret, 0);
    /* check exposed attrs */
...
TEST(LibRGW, LSXATTR1)
{
    ...
    int ret = rgw_lsxattrs(fs, dir.fh, &filter_prefix, getattr_cb, &out_attrmap, RGW_LSXATTR_FLAG_NONE);
    ASSERT_EQ(ret, 0);
