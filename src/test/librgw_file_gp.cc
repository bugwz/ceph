...
struct ZPageSet
{
    ...
    bool operator==(const ZPageSet& rhs)
    {
        ...
        for (int page_ix = 0; page_ix < n; ++page_ix) {
            ...
            if (p1->cksum != p2->cksum) return false;
        }
        return true;
    }
    ...
    ~ZPageSet()
    {
        for (unsigned int ix = 0; ix < pages.size(); ++ix) delete pages[ix];
        free(iovs);
    }
}; /* ZPageSet */
...
TEST(LibRGW, MOUNT)
{
    int ret = rgw_mount2(rgw, uid.c_str(), access_key.c_str(), secret_key.c_str(), "/", &fs, RGW_MOUNT_FLAG_NONE);
    ASSERT_EQ(ret, 0);
...
TEST(LibRGW, CREATE_BUCKET)
{
    if (do_create) {
        ...
        int ret = rgw_mkdir(fs, fs->root_fh, bucket_name.c_str(), &st, create_mask, &fh, RGW_MKDIR_FLAG_NONE);
        ASSERT_EQ(ret, 0);
    }
}
...
TEST(LibRGW, LIST_OBJECTS)
{
    if (do_pre_list) {
        ...
        bool eof = false;
        uint64_t offset = 0;
        int ret = rgw_readdir(fs, bucket_fh, &offset, r2_cb, &fids, &eof, RGW_READDIR_FLAG_NONE);
        for (auto& fid : fids) {
            std::cout << "fname: " << get<0>(fid) << " fid: " << get<1>(fid) << std::endl;
        }
        ASSERT_EQ(ret, 0);
    }
}
...
TEST(LibRGW, LOOKUP_OBJECT)
{
    if (do_get || do_stat || do_put || do_bulk || do_readv || do_writev) {
        int ret = rgw_lookup(fs, bucket_fh, object_name.c_str(), &object_fh, nullptr, 0, RGW_LOOKUP_FLAG_CREATE);
        ASSERT_EQ(ret, 0);
    }
}
...
TEST(LibRGW, PUT_OBJECT)
{
    if (do_put) {
        ...
        int ret = rgw_write(fs, object_fh, 0, data.length(), &nbytes, (void*)data.c_str(), RGW_WRITE_FLAG_NONE);
        ASSERT_EQ(ret, 0);
...
TEST(LibRGW, GET_OBJECT)
{
    if (do_get) {
        ...
        memset(sbuf, 0, 512);
        size_t nread;
        int ret = rgw_read(fs, object_fh, 0 /* off */, 512 /* len */, &nread, sbuf, RGW_READ_FLAG_NONE);
        ASSERT_EQ(ret, 0);
        buffer::list bl;
...
TEST(LibRGW, WRITE_READ_VERIFY)
{
    if (do_bulk && do_put) {
        ...
        for (int ix = 0; ix < 16; ++ix, off += page_size) {
            ...
            int ret = rgw_write(fs, object_fh, off, page_size, &nbytes, iov->iov_base, RGW_WRITE_FLAG_NONE);
            ASSERT_EQ(ret, 0);
...
{
    if (do_delete) {
        int ret = rgw_unlink(fs, bucket_fh, object_name.c_str(), RGW_UNLINK_FLAG_NONE);
        ASSERT_EQ(ret, 0);
    }
}
...
{
    if (do_delete) {
        int ret = rgw_unlink(fs, fs->root_fh, bucket_name.c_str(), RGW_UNLINK_FLAG_NONE);
        ASSERT_EQ(ret, 0);
    }
}
