...
void LazyOmapStatsTest::scrub() const
{
    ...
    if (ec) {
        cout << "Deep scrub command failed! Error: " << ec.value() << " " << ec.message() << endl;
        exit(ec.value());
    }
    cout << is.rdbuf() << endl;
}
...
const string LazyOmapStatsTest::get_output(const string command, const bool silent)
{
...
const string LazyOmapStatsTest::get_pool_id(string& pool)
{
    ...
    if (ret < 0) {
        ret = -ret;
        cerr << "Failed to get pool id! Error: " << ret << " " << strerror(ret) << endl;
        exit(ret);
    }
