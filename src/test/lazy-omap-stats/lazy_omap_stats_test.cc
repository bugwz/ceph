...
void LazyOmapStatsTest::init(const int argc, const char** argv)
{
    ...
    if (ret < 0) {
        ret = -ret;
        cerr << "Failed to initialise rados! Error: " << ret << " " << strerror(ret) << endl;
        exit(ret);
    }
    ...
    if (ret < 0) {
        ret = -ret;
        cerr << "Failed to parse command line config options! Error: " << ret << " " << strerror(ret) << endl;
        exit(ret);
    }
    ...
    if (ret < 0) {
        ret = -ret;
        cerr << "Failed to parse environment! Error: " << ret << " " << strerror(ret) << endl;
        exit(ret);
    }
    ...
    if (ret < 0) {
        ret = -ret;
        cerr << "Failed to read config file! Error: " << ret << " " << strerror(ret) << endl;
        exit(ret);
    }
    ...
    if (ret < 0) {
        ret = -ret;
        cerr << "Failed to connect to running cluster! Error: " << ret << " " << strerror(ret) << endl;
        exit(ret);
    }
    ...
    if (ret < 0) {
        ret = -ret;
        cerr << "Failed to create pool! Error: " << ret << " " << strerror(ret) << endl;
        exit(ret);
    }
    ...
    if (ret < 0) {
        ret = -ret;
        cerr << "Failed to create ioctx! Error: " << ret << " " << strerror(ret) << endl;
        exit(ret);
    }
...
void LazyOmapStatsTest::write_omap(const string& object_name)
{
    ...
    if (ret < 0) {
        ret = -ret;
        cerr << "Failed to create object! Error: " << ret << " " << strerror(ret) << endl;
        exit(ret);
    }
    ret = io_ctx.omap_set(object_name, payload);
    if (ret < 0) {
        ret = -ret;
        cerr << "Failed to write omap payload! Error: " << ret << " " << strerror(ret) << endl;
        exit(ret);
    }
    cout << "Wrote " << conf.keys << " omap keys of " << conf.payload_size << " bytes to "
...
void LazyOmapStatsTest::create_payload()
{
    ...
    Lorem.append("Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do "
    ...
    cout << "Created payload with " << conf.keys << " keys of " << conf.payload_size
         << " bytes each. Total size in bytes = " << conf.keys * conf.payload_size << endl;
}
...
void LazyOmapStatsTest::scrub()
{
    ...
    if (!target_pool_found) {
        cout << "Error: Target pool " << conf.pool_name << ":" << conf.pool_id << " not found!" << endl;
        exit(2);   // ENOENT
    }
    cout << endl;
...
const string LazyOmapStatsTest::get_output(const string command, const bool silent, const CommandTarget target)
{
    ...
    if (ret < 0) {
        ret = -ret;
        cerr << "Failed to get " << command << "! Error: " << ret << " " << strerror(ret) << endl;
        exit(ret);
    }
...
map<string, string> LazyOmapStatsTest::get_scrub_stamps()
{
    ...
    for (auto pg = obj->find_first(); !pg.end(); ++pg) {
        stamps.insert({(*pg)->find_obj("pgid")->get_data(), (*pg)->find_obj("last_deep_scrub_stamp")->get_data()});
    }
    return stamps;
}
...
void LazyOmapStatsTest::check_one()
{
    ...
    total += result;
    if (total != 6) {
        cout << "Error: Found " << total << " matches, expected 6! Exiting..." << endl;
        exit(22);   // EINVAL
    }
    cout << "check_one successful. Found " << total << " matches as expected" << endl;
}

const int LazyOmapStatsTest::find_index(string& haystack, regex& needle, string label) const
{
    smatch match;
...
const uint LazyOmapStatsTest::tally_column(const uint omap_bytes_index, const string& table, bool header) const
{
...
void LazyOmapStatsTest::check_column(const int index, const string& table, const string& type, bool header) const
{
    uint expected;
    string errormsg;
