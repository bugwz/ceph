...
struct obj
{
    ...
    obj(int _a, int _b)
        ...
        , b(_b)
    {}
    friend inline bool operator<(const obj& l, const obj& r) { return l.a < r.a; }
};
...
TEST(mempool, dump)
{
    ...
    delete f;
    ASSERT_NE(ostr.str().find(mempool::get_pool_name((mempool::pool_index_t)0)), std::string::npos);
...
{
    typedef mempool::pool_allocator<mempool::mempool_osd, pair<const uint64_t, uint64_t>> allocator_t;
