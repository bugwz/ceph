...
public:
    ...
    obj_rec(string _name, struct rgw_file_handle* _fh, struct rgw_file_handle* _parent_fh, RGWFileHandle* _rgw_fh)
        : name(std::move(_name))
        , fh(_fh)
...
ostream& operator<<(ostream& os, const obj_rec& rec)
{
    ...
    if (rgw_fh) {
        ...
        os << rec.rgw_fh->full_object_name() << " (" << rec.rgw_fh->object_name() << "): " << type;
    }
    return os;
}
...
ostream& operator<<(ostream& os, const obj_rec_st& rec)
{
    ...
    if (rgw_fh) {
        const char* type = rgw_fh->is_dir() ? "DIR " : "FILE ";
        os << rgw_fh->full_object_name() << " (" << rgw_fh->object_name() << "): " << type;
...
TEST(LibRGW, MOUNT_NOROOT)
{
    ...
    struct rgw_fs* fs = nullptr;
    int ret = rgw_mount2(rgw_h, userid.c_str(), access_key.c_str(), secret_key.c_str(), "", &fs, RGW_MOUNT_FLAG_NONE);
    ASSERT_EQ(ret, 0);
...
{
    if (do_hier1) {
        (void)rgw_lookup(fs, fs->root_fh, bucket_name.c_str(), &bucket_fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
        if (!bucket_fh) {
            if (do_create) {
                struct stat st;
        ...
        if (do_create) {
            ...
            std::vector<std::string> obj_names = {"foo/bar/baz/quux",
                                                  "foo/f1",
                                                  "foo/f2",
                                                  "foo/bar/f1",
                                                  "foo/bar/d1/",
            ...
            for (const auto& obj_name : obj_names) {
                if (verbose) {
                    std::cout << "creating: " << bucket_name << ":" << obj_name << std::endl;
                }
                RGWPutObjRequest req(
                    cct, g_rgwlib->get_driver()->get_user(fs_private->get_user()->user_id), bucket_name, obj_name, bl);
...
TEST(LibRGW, SETUP_DIRS1)
{
    if (do_dirs1) {
        ...
        (void)rgw_lookup(
            fs, dirs1_b.parent_fh, dirs1_bucket_name.c_str(), &dirs1_b.fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);

        if (!dirs1_b.fh) {
            if (do_create) {
                rc = rgw_mkdir(
                    fs, dirs1_b.parent_fh, dirs1_b.name.c_str(), &st, create_mask, &dirs1_b.fh, RGW_MKDIR_FLAG_NONE);
                ASSERT_EQ(rc, 0);
            }
            else {
        ...
        for (d_ix = 0; d_ix < n_dirs1_dirs; ++d_ix) {
            ...
            (void)rgw_lookup(fs, dir.parent_fh, dir.name.c_str(), &dir.fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
            if (!dir.fh) {
                if (do_create) {
                    rc = rgw_mkdir(fs, dir.parent_fh, dir.name.c_str(), &st, create_mask, &dir.fh, RGW_MKDIR_FLAG_NONE);
                    ASSERT_EQ(rc, 0);
                }
            }
            ...
            for (f_ix = 0; f_ix < n_dirs1_objs; ++f_ix) {
                ...
                (void)rgw_lookup(fs, sdir.parent_fh, sdir.name.c_str(), &sdir.fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);

                if (!sdir.fh) {
                    if (do_create) {
                        rc = rgw_mkdir(
                            fs, sdir.parent_fh, sdir.name.c_str(), &st, create_mask, &sdir.fh, RGW_MKDIR_FLAG_NONE);
                        ASSERT_EQ(rc, 0);
                    }
                }
                ...
                (void)rgw_lookup(fs, sf.parent_fh, sf.name.c_str(), &sf.fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);

                if (!sf.fh) {
                    if (do_create) {
                        /* make a new file object (the hard way) */
                        rc = rgw_lookup(fs, sf.parent_fh, sf.name.c_str(), &sf.fh, nullptr, 0, RGW_LOOKUP_FLAG_CREATE);
                        ASSERT_EQ(rc, 0);
                        sf.sync();
                        ...
                        rc = rgw_write(fs, sf.fh, 0, data.length(), &nbytes, (void*)data.c_str(), RGW_WRITE_FLAG_NONE);
                        ASSERT_EQ(rc, 0);
...
TEST(LibRGW, RGW_SETUP_RENAME1)
{
    ...
    if (do_rename) {
        ...
        for (int b_ix : {0, 1}) {
            ...
            (void)rgw_lookup(fs, fs->root_fh, brec.name.c_str(), &brec.fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
            if (!brec.fh) {
                if (do_create) {
                    struct stat st;
                    int rc =
                        rgw_mkdir(fs, fs->root_fh, brec.name.c_str(), &st, create_mask, &brec.fh, RGW_MKDIR_FLAG_NONE);
                    ASSERT_EQ(rc, 0);
                }
            }
            ...
            for (int f_ix : {0, 1}) {
                ...
                (void)rgw_lookup(fs, rf.parent_fh, rf.name.c_str(), &rf.fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
                if (!rf.fh) {
                    rc = rgw_create(fs,
                                    rf.parent_fh,
                                    rf.name.c_str(),
...
TEST(LibRGW, RGW_INTRABUCKET_RENAME1)
{
    ...
    if (do_rename) {
        ...
        if (verbose) {
            std::cout << "rename file " << src_obj.name << " to " << rfname << " (bucket " << bdir0.name << ")"
                      << std::endl;
        }
        rc = rgw_rename(fs, bdir0.fh, src_obj.name.c_str(), bdir0.fh, rfname.c_str(), RGW_RENAME_FLAG_NONE);
        ASSERT_EQ(rc, 0);
    }
}
...
TEST(LibRGW, RGW_CROSSBUCKET_RENAME1)
{
    ...
    if (do_rename) {
        ...
        if (verbose) {
            std::cout << "rename file " << src_obj.name << " (bucket " << bdir0.name << ") to " << rfname << " (bucket "
                      << bdir1.name << ")" << std::endl;
        }
        rc = rgw_rename(fs, bdir0.fh, src_obj.name.c_str(), bdir1.fh, rfname.c_str(), RGW_RENAME_FLAG_NONE);
        ASSERT_EQ(rc, 0);
    }
}
...
TEST(LibRGW, GETATTR_DIRS1)
{
    if (do_dirs1) {
        ...
        for (auto& dirs_rec : dirs_vec) {
            obj_rec& dir = get<0>(dirs_rec);
            if (verbose) {
                std::cout << "scanning objects in " << dir.rgw_fh->full_object_name() << std::endl;
            }
...
TEST(LibRGW, READ_DIRS1)
{
    if (do_dirs1) {
        ...
        for (auto& dirs_rec : dirs_vec) {
            obj_rec& dir = get<0>(dirs_rec);
            if (verbose) {
                std::cout << "read back objects in " << dir.rgw_fh->full_object_name() << std::endl;
            }
            for (auto& sobj : get<1>(dirs_rec)) {
                /* only the first 2 file objects have data */
                if ((sobj.rgw_fh->object_name().find("sfile_0") != std::string::npos) ||
                    (sobj.rgw_fh->object_name().find("sfile_1") != std::string::npos)) {
                    ASSERT_TRUE(sobj.rgw_fh->is_file());
                    ASSERT_EQ(sobj.rgw_fh->get_size(), 16UL);
                    // do it
                    memset(buf, 0, 256);
                    if (verbose) {
                        std::cout << "reading 0,256 " << sobj.rgw_fh->relative_object_name() << std::endl;
                    }
                    rc = rgw_read(fs, sobj.fh, 0, 256, &nread, buf, RGW_READ_FLAG_NONE);
                    ASSERT_EQ(rc, 0);
                    if (verbose) {
                        std::cout << "\tread back from " << sobj.name << " : \"" << buf << "\"" << std::endl;
                    }
                }
            }
        }
...
TEST(LibRGW, READF_SETUP1)
{
    ...
    if (do_dirs1) {
        if (do_create) {
            if ((!stat(readf_out_name.c_str(), &st)) && (S_ISREG(st.st_mode)) && (st.st_size == 6291456)) return;
            ofstream of;
...
TEST(LibRGW, READF_DIRS1)
{
    if (do_dirs1) {
        if (do_readf) {
            ...
            int rc = rgw_lookup(fs, dirs1_b.fh, fobj.name.c_str(), &fobj.fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
            ASSERT_EQ(rc, 0);
            ...
            for (int ix = 0; ix < 6; ++ix) {
                ...
                rc = rgw_read(fs, fobj.fh, offset, length, &nread, buffer.get(), RGW_READ_FLAG_NONE);
                ASSERT_EQ(rc, 0);
...
TEST(LibRGW, WRITEF_DIRS1)
{
    if (do_dirs1) {
        if (do_writef) {
            ...
            (void)rgw_lookup(fs, fobj.parent_fh, fobj.name.c_str(), &fobj.fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
            if (!fobj.fh) {
                if (do_create) {
                    /* make a new file object (the hard way) */
                    rc =
                        rgw_lookup(fs, fobj.parent_fh, fobj.name.c_str(), &fobj.fh, nullptr, 0, RGW_LOOKUP_FLAG_CREATE);
                    ASSERT_EQ(rc, 0);
                }
            }
            ASSERT_NE(fobj.fh, nullptr);
            ...
            for (int ix = 0; ix < 6; ++ix) {
                ...
                if (verbose) {
                    std::cout << "read and writing " << length << " bytes"
                              << " from " << readf_out_name << " at offset " << offset << " (" << str
                              << "... [first 4 chars])" << std::endl;
                }
                char* leakbuf = (char*)malloc(bufsz);
                memcpy(leakbuf, buffer, length);
                rc = rgw_write(fs, fobj.fh, offset, length, &nwritten, leakbuf, RGW_WRITE_FLAG_NONE);
                ASSERT_EQ(rc, 0);
    ...
    if (do_dirs1) {
        ...
        for (auto& dirs_rec : dirs_vec) {
            for (auto& obj : get<1>(dirs_rec)) {
                if (verbose) {
                    std::cout << "release " << obj.name << " type: " << obj.rgw_fh->stype()
                              << " refs: " << obj.rgw_fh->get_refcnt() << std::endl;
                }
...
extern "C" {
static int r1_cb(const char* name, void* arg, uint64_t offset, struct stat* st, uint32_t st_mask, uint32_t flags)
{
    struct rgw_file_handle* parent_fh = static_cast<struct rgw_file_handle*>(arg);
    RGWFileHandle* rgw_fh = get_rgwfh(parent_fh);
    lsubdout(cct, rgw, 10) << __func__ << " bucket=" << rgw_fh->bucket_name() << " dir=" << rgw_fh->full_object_name()
                           << " called back name=" << name << " flags=" << flags << dendl;
    string name_str{name};
    if (!((name_str == ".") || (name_str == ".."))) {
        obj_stack.push(obj_rec{std::move(name_str), nullptr, parent_fh, nullptr});
    }
    return true; /* XXX */
}
}
...
TEST(LibRGW, HIER1)
{
    if (do_hier1) {
        ...
        while (!obj_stack.empty()) {
            ...
            if (!elt.fh) {
                struct rgw_file_handle* parent_fh = elt.parent_fh ? elt.parent_fh : fs->root_fh;
                RGWFileHandle* pfh = get_rgwfh(parent_fh);
                rgw::ignore(pfh);
                lsubdout(cct, rgw, 10) << "rgw_lookup:"
                                       << " parent object_name()=" << pfh->object_name()
                                       << " parent full_object_name()=" << pfh->full_object_name()
                                       << " elt.name=" << elt.name << dendl;
                rc = rgw_lookup(fs, parent_fh, elt.name.c_str(), &elt.fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
                ASSERT_EQ(rc, 0);
                // XXXX
                RGWFileHandle* efh = get_rgwfh(elt.fh);
                rgw::ignore(efh);
                lsubdout(cct, rgw, 10) << "rgw_lookup result:"
                ...
                case RGW_FS_TYPE_DIRECTORY:
                    if (!elt.state.readdir) {
                        ...
                        lsubdout(cct, rgw, 10)
                            << "readdir in"
                            << " bucket: " << elt.rgw_fh->bucket_name() << " object_name: " << elt.rgw_fh->object_name()
                            << " full_name: " << elt.rgw_fh->full_object_name() << dendl;
                        rc = rgw_readdir(fs, elt.fh, &offset, r1_cb, elt.fh, &eof, RGW_READDIR_FLAG_DOTDOT);
...
TEST(LibRGW, MARKER1_SETUP_BUCKET)
{
    ...
    if (do_marker1) {
        ...
        if (do_create) {
            ret = rgw_mkdir(fs, bucket_fh, marker_dir.c_str(), &st, create_mask, &marker_fh, RGW_MKDIR_FLAG_NONE);
        }
        else {
            ret = rgw_lookup(fs, bucket_fh, marker_dir.c_str(), &marker_fh, nullptr, 0, RGW_LOOKUP_FLAG_NONE);
        }
        ASSERT_EQ(ret, 0);
    }
}
...
TEST(LibRGW, MARKER1_SETUP_OBJECTS)
{
    ...
    if (do_marker1 && do_create) {
        ...
        for (int ix = 0; ix < marker_nobjs; ++ix) {
            ...
            ret = rgw_lookup(fs, marker_fh, obj.name.c_str(), &obj.fh, nullptr, 0, RGW_LOOKUP_FLAG_CREATE);
            ASSERT_EQ(ret, 0);
            ...
            data += object_name;
            int ret = rgw_write(fs, obj.fh, 0, data.length(), &nbytes, (void*)data.c_str(), RGW_WRITE_FLAG_NONE);
            ASSERT_EQ(ret, 0);
            ASSERT_EQ(nbytes, data.length());
...
extern "C" {
static int r2_cb(const char* name, void* arg, uint64_t offset, struct stat* st, uint32_t st_mask, uint32_t flags)
{
    dirent_vec& dvec = *(static_cast<dirent_vec*>(arg));
    lsubdout(cct, rgw, 10) << __func__ << " bucket=" << bucket_name << " dir=" << marker_dir
                           << " iv count=" << dvec.count << " called back name=" << name << " flags=" << flags << dendl;
    string name_str{name};
    if (!((name_str == ".") || (name_str == ".."))) {
...
TEST(LibRGW, MARKER1_READDIR)
{
    if (do_marker1) {
        ...
        do {
            ...
            int ret = rgw_readdir(fs, marker_fh, &offset, r2_cb, &dvec, &eof, RGW_READDIR_FLAG_DOTDOT);
            ASSERT_EQ(ret, 0);
            ASSERT_EQ(offset, get<1>(dvec.obj_names.back()));   // cookie check
            ++dvec.count;
        } while (!eof);
        std::cout << "Read " << dvec.obj_names.size() << " objects in " << marker_dir.c_str() << std::endl;
    }
}
...
TEST(LibRGW, MARKER1_OBJ_CLEANUP)
{
    ...
    for (auto& obj : marker_objs) {
        if (obj.fh) {
            if (do_delete) {
                if (verbose) {
                    std::cout << "unlinking: " << bucket_name << ":" << obj.name << std::endl;
                }
...
TEST(LibRGW, CLEANUP)
{
    ...
    if (do_marker1) {
        cleanup_queue.push_back(obj_rec{bucket_name, bucket_fh, fs->root_fh, get_rgwfh(fs->root_fh)});
    }
...
int main(int argc, char* argv[])
{
    ...
    for (auto arg_iter = args.begin(); arg_iter != args.end();) {
        ...
        else if (ceph_argparse_witharg(args, arg_iter, &val, "--secret", (char*)nullptr)) {
            secret_key = val;
        }
        else if (ceph_argparse_witharg(args, arg_iter, &val, "--userid", (char*)nullptr)) {
            userid = val;
        }
        else if (ceph_argparse_witharg(args, arg_iter, &val, "--bn", (char*)nullptr)) {
            bucket_name = val;
        }
        else if (ceph_argparse_witharg(args, arg_iter, &val, "--uid", (char*)nullptr)) {
            owner_uid = std::stoi(val);
        }
        else if (ceph_argparse_witharg(args, arg_iter, &val, "--gid", (char*)nullptr)) {
            owner_gid = std::stoi(val);
        }
        else if (ceph_argparse_flag(args, arg_iter, "--hier1", (char*)nullptr)) {
            do_hier1 = true;
        }
        else if (ceph_argparse_flag(args, arg_iter, "--dirs1", (char*)nullptr)) {
            do_dirs1 = true;
        }
        else if (ceph_argparse_flag(args, arg_iter, "--marker1", (char*)nullptr)) {
            do_marker1 = true;
        }
        else if (ceph_argparse_flag(args, arg_iter, "--setattr", (char*)nullptr)) {
            do_setattr = true;
        }
        ...
        else if (ceph_argparse_flag(args, arg_iter, "--rename", (char*)nullptr)) {
            do_rename = true;
        }
        else if (ceph_argparse_flag(args, arg_iter, "--readf", (char*)nullptr)) {
            do_readf = true;
        }
        else if (ceph_argparse_flag(args, arg_iter, "--writef", (char*)nullptr)) {
            do_writef = true;
        }
