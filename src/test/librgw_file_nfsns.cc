        ...
        if (do_create) {
            ...
            for (const auto& obj_name : obj_names) {
                ...
                RGWPutObjRequest req(
                    cct, rgwlib.get_store()->get_user(fs_private->get_user()->user_id), bucket_name, obj_name, bl);
...
extern "C" {
static bool r1_cb(const char* name, void* arg, uint64_t offset, struct stat* st, uint32_t st_mask, uint32_t flags)
{
...
extern "C" {
static bool r2_cb(const char* name, void* arg, uint64_t offset, struct stat* st, uint32_t st_mask, uint32_t flags)
{
