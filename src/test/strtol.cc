...
static void test_strict_iecstrtoll_units(const std::string& foo, std::string u, const int m)
{
    std::string s(foo);
    s.append(u);
...
TEST(IECStrToLL, WithUnits)
{
    ...
    for (std::map<std::string, int>::iterator p = units.begin(); p != units.end(); ++p) {
...
static void test_strict_sistrtoll_units(const std::string& foo, std::string u, const long long m)
{
    std::string s(foo);
    s.append(u);
    const char* str = s.c_str();
...
TEST(SIStrToLL, WithUnits)
{
    ...
    for (std::map<std::string, long long>::iterator p = units.begin(); p != units.end(); ++p) {
        // the upper bound of uint64_t is 2^64 = 4E
...
template<typename T> inline void test_parse()
{
...
template<typename T> inline void test_consume()
{
    auto pos = "23"sv;
    auto spacepos = "    23"sv;
    auto neg = "-5"sv;
