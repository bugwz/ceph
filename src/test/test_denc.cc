...
template<typename T> void test_encode_decode(T v)
{
    bufferlist bl;
    encode(v, bl);
    auto p = bl.cbegin();
...
template<typename T> void test_denc(T v)
{
    // estimate
    size_t s = 0;
    denc(v, s);
...
template<typename T> void test_encode_decode_featured(T v)
{
    bufferlist bl;
    encode(v, bl, 123);
    auto p = bl.cbegin();
...
template<typename T> void test_denc_featured(T v)
{
    // estimate
    size_t s = 0;
    denc(v, s, 0);
...
struct legacy_t
{
    ...
    explicit legacy_t(int32_t i)
        : a(i)
    {}
    friend bool operator<(const legacy_t& l, const legacy_t& r) { return l.a < r.a; }
    friend bool operator==(const legacy_t& l, const legacy_t& r) { return l.a == r.a; }
};
WRITE_CLASS_ENCODER(legacy_t)

template<template<class> class C> void test_common_veclist(const char* c)
{
    {
...
template<typename T> using default_vector = std::vector<T>;
...
template<typename T> using default_list = std::list<T>;
...
template<template<class> class C> void test_setlike(const char* c)
{
    {
        cout << c << "<std::string>" << std::endl;
        C<std::string> s;
...
template<typename T> using default_set = std::set<T>;
...
template<typename T> using default_flat_set = boost::container::flat_set<T>;
...
struct foo_t
{
    ...
    friend bool operator==(const foo_t& l, const foo_t& r) { return l.a == r.a && l.b == r.b; }
};
...
struct foo2_t
{
    ...
    friend bool operator==(const foo2_t& l, const foo2_t& r) { return l.c == r.c && l.d == r.d; }
};
...
struct bar_t
{
    ...
    friend bool operator==(const bar_t& l, const bar_t& r) { return l.a == r.a && l.b == r.b; }
};
...
template<template<class, class> class C> void test_common_maplike(const char* c)
{
    {
...
template<typename U, typename V> using default_map = std::map<U, V>;
...
template<typename U, typename V> using default_flat_map = boost::container::flat_map<U, V>;
...
TEST(denc, tuple)
{
    ...
    }
    {
        ...
        std::tuple<std::string, std::set<uint32_t>> s("bar", std::set<uint32_t>{uint32_t(1), uint32_t(2), uint32_t(3)});
        test_denc(s);
    }
}
...
struct Legacy
{
    ...
    static void reset() { n_denc = n_decode = 0; }
...
{
    static_assert(_denc::has_legacy_denc<Legacy>::value, "Legacy do have legacy denc");
    {
