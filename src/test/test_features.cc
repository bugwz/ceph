...
TEST(features, release_features)
{
    for (int r = 1; r < CEPH_RELEASE_MAX; ++r) {
        ...
        ASSERT_EQ(ceph_release_t{static_cast<uint8_t>(r)}, ceph_release_from_name(name));
        uint64_t features = ceph_release_features(r);
        int rr = ceph_release_from_features(features);
        cout << r << " " << name << " features 0x" << std::hex << features << std::dec << " looks like "
             << ceph_release_name(rr) << std::endl;
        EXPECT_LE(rr, r);
    }
}
...
TEST(features, release_from_features)
{
    ...
    ASSERT_EQ(CEPH_RELEASE_LUMINOUS, ceph_release_from_features(1152323339925389307));
}
