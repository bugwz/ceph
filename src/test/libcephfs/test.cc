...
TEST(LibCephFS, ClearSetuid)
{
    ...
    ASSERT_EQ(ceph_ll_create(
                  cmount, root, filename, before_mode, O_RDWR | O_CREAT | O_EXCL, &in, &fh, &stx, want, 0, usercred),
              0);
...
TEST(LibCephFS, SnapQuota)
{
    ...
    sprintf(
        c_temp, "/.snap/test_snap_dir_quota_xattr_snap_%d/test_snap_dir_quota_xattr_%d/subdir_noquota", mypid, mypid);
    ...
    int found = 0;
    while (len > 0) {
        if (strcmp(p, "ceph.quota.max_bytes") == 0) found++;
        len -= strlen(p) + 1;
        p += strlen(p) + 1;
    }
    ASSERT_EQ(found, 0);
