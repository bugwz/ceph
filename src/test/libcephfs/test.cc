...
TEST(LibCephFS, DirLs)
{
    ...
    while (true) {
        struct dirent rdent;
        struct ceph_statx stx;
        int len = ceph_readdirplus_r(cmount, ls_dir, &rdent, &stx, CEPH_STATX_SIZE, AT_STATX_DONT_SYNC, NULL);
        if (len == 0) break;
        ASSERT_EQ(len, 1);
...
TEST(LibCephFS, Xattrs_ll)
{
    ...
    res = ceph_ll_lookup(cmount, root, test_xattr_file, &existent_file_handle, &stx, 0, 0, perms);
    ASSERT_EQ(res, 0);
...
TEST(LibCephFS, LlreadvLlwritev)
{
    ...
    ASSERT_EQ(ceph_ll_create(cmount, root, filename, 0666, O_RDWR | O_CREAT | O_TRUNC, &file, &fh, &stx, 0, 0, perms),
              0);
...
TEST(LibCephFS, GetPoolReplication)
{
    ...
    ASSERT_EQ(0, ceph_get_path_layout(cmount, "/", &stripe_unit, &stripe_count, &object_size, &pool_id));
    ASSERT_GE(pool_id, 0);
...
TEST(LibCephFS, GetExtentOsds)
{
    ...
    int fd = ceph_open_layout(cmount, test_file, O_CREAT | O_RDWR, 0666, stripe_unit, 2, stripe_unit * 2, NULL);
    ASSERT_GT(fd, 0);
...
TEST(LibCephFS, Nlink)
{
    ...
    ASSERT_EQ(ceph_ll_create(
                  cmount, dir, filename, 0666, O_RDWR | O_CREAT | O_EXCL, &file, &fh, &stx, CEPH_STATX_NLINK, 0, perms),
              0);
    ...
    ASSERT_EQ(ceph_ll_lookup(cmount, dir, filename, &file, &stx, CEPH_STATX_NLINK, 0, perms), 0);
    ASSERT_EQ(stx.stx_nlink, (nlink_t)1);
...
static inline bool timespec_eq(timespec const& lhs, timespec const& rhs)
{
...
TEST(LibCephFS, LazyStatx)
{
    ...
    ASSERT_EQ(
        ceph_ll_create(cmount1, root1, filename, 0666, O_RDWR | O_CREAT | O_EXCL, &file1, &fh, &stx, 0, 0, perms1), 0);
    ...
    ASSERT_TRUE(stx.stx_ctime.tv_sec == old_ctime.tv_sec && stx.stx_ctime.tv_nsec == old_ctime.tv_nsec);
...
TEST(LibCephFS, ShutdownRace)
{
    ...
    cout << "Setting RLIMIT_NOFILE from " << rold.rlim_cur << " to " << rnew.rlim_cur << std::endl;

    ASSERT_EQ(setrlimit(RLIMIT_NOFILE, &rnew), 0);

    for (int i = 0; i < nthreads; ++i) threads[i] = std::thread(shutdown_racer_func);
...
TEST(LibCephFS, TestUtime)
{
    ...
    ASSERT_EQ(ceph_statx(cmount, test_file, &stx, CEPH_STATX_MTIME | CEPH_STATX_ATIME, 0), 0);
...
TEST(LibCephFS, TestUtimes)
{
    ...
    ASSERT_EQ(ceph_statx(cmount, test_symlink, &stx, CEPH_STATX_MTIME | CEPH_STATX_ATIME, 0), 0);
    ...
    ASSERT_EQ(ceph_statx(cmount, test_symlink, &stx, CEPH_STATX_MTIME | CEPH_STATX_ATIME, AT_SYMLINK_NOFOLLOW), 0);
    ASSERT_EQ(utime_t(stx.stx_atime), utime_t(times[0]));
    ASSERT_EQ(utime_t(stx.stx_mtime), utime_t(times[1]));
...
TEST(LibCephFS, OperationsOnDotDot)
{
    ...
    sprintf(c_non_existent_dirs, "/%s/../ood_nonexistent1_%d/oodd_nonexistent2_%d", c_dir, mypid, mypid);
...
TEST(LibCephFS, SnapXattrs)
{
    ...
    *s = '\0';
    ...
    int found = 0;
    while (len > 0) {
        if (strcmp(p, "ceph.snap.btime") == 0) found++;
        len -= strlen(p) + 1;
        p += strlen(p) + 1;
    }
    ASSERT_EQ(found, 0);
...
TEST(LibCephFS, Utimensat)
{
    ...
    struct ceph_statx stx;
    ASSERT_EQ(ceph_statx(cmount, file_path, &stx, CEPH_STATX_MTIME | CEPH_STATX_ATIME, 0), 0);
    ASSERT_EQ(utime_t(stx.stx_atime), utime_t(times[0]));
    ASSERT_EQ(utime_t(stx.stx_mtime), utime_t(times[1]));
...
TEST(LibCephFS, SnapdirAttrs)
{
    ...
    ASSERT_EQ(ceph_statx(cmount,
                         dir_path,
                         &stx_dir,
                         CEPH_STATX_MTIME | CEPH_STATX_ATIME | CEPH_STATX_MODE | CEPH_STATX_MODE | CEPH_STATX_GID |
                             CEPH_STATX_VERSION,
                         0),
              0);
    ASSERT_EQ(ceph_statx(cmount,
...
TEST(LibCephFS, SnapdirAttrsOnSnapCreate)
{
    ...
    ASSERT_EQ(
        ceph_statx(cmount, snap_dir_path, &stx_snap_dir, CEPH_STATX_MTIME | CEPH_STATX_CTIME | CEPH_STATX_VERSION, 0),
        0);
    ASSERT_EQ(
        ceph_statx(cmount, "/.snap", &stx_root_snap_dir, CEPH_STATX_MTIME | CEPH_STATX_CTIME | CEPH_STATX_VERSION, 0),
        0);
    ASSERT_EQ(utime_t(stx_root_snap_dir.stx_mtime), utime_t(stx_snap_dir.stx_mtime));
    ASSERT_EQ(utime_t(stx_root_snap_dir.stx_ctime), utime_t(stx_snap_dir.stx_ctime));
    ASSERT_EQ(stx_root_snap_dir.stx_version, stx_snap_dir.stx_version);
    ...
    ASSERT_EQ(
        ceph_statx(cmount, snap_dir_path, &stx_snap_dir_1, CEPH_STATX_MTIME | CEPH_STATX_CTIME | CEPH_STATX_VERSION, 0),
        0);
...
TEST(LibCephFS, SnapdirAttrsOnSnapDelete)
{
    ...
    ASSERT_EQ(
        ceph_statx(cmount, snap_dir_path, &stx_snap_dir_2, CEPH_STATX_MTIME | CEPH_STATX_CTIME | CEPH_STATX_VERSION, 0),
        0);
