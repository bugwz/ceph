...
TEST(LibCephFS, BasicRecordLocking)
{
    ...
    rc = ceph_ll_create(cmount, root, c_file, fileMode, O_RDWR | O_CREAT, &inode, &fh, &stx, 0, 0, perms);
    ASSERT_EQ(rc, 0);
...
#define WAIT_MAIN(n) ASSERT_EQ(0, sem_timedwait(&s.semReply[n % 2], abstime(ts, waitSlowMs)))
...
#define WAIT_WORKER(n) ASSERT_EQ(0, sem_timedwait(&s.sem[n % 2], abstime(ts, waitSlowMs)))
// Worker shall not wake us up (for (N) steps)
#define NOT_WAIT_WORKER(n) ASSERT_EQ(-1, sem_timedwait(&s.sem[n % 2], abstime(ts, waitMs)))
...
static void thread_ConcurrentRecordLocking(str_ConcurrentRecordLocking& s)
{
    ...
    // Get the inode and Fh corresponding to c_file
    rc = ceph_ll_create(
        cmount, root, s.file, fileMode, O_RDWR | O_CREAT, &inode, &fh, &stx, 0, 0, ceph_mount_perms(cmount));
    ASSERT_EQ(rc, 0);
...
{
    str_ConcurrentRecordLocking* const s = reinterpret_cast<str_ConcurrentRecordLocking*>(arg);
...
TEST(LibCephFS, DISABLED_InterProcessRecordLocking)
{
    ...
    str_ConcurrentRecordLocking* const shs = reinterpret_cast<str_ConcurrentRecordLocking*>(
        mmap(0, sizeof(*shs), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0));
