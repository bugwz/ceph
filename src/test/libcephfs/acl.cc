...
{
    if (size < sizeof(acl_ea_header)) return -1;
    size -= sizeof(acl_ea_header);
    if (size % sizeof(acl_ea_entry)) return -1;
...
static int check_acl_and_mode(const void* buf, size_t size, mode_t mode)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        switch (tag) {
        case ACL_USER_OBJ:
            if (perm != ((mode >> 6) & 7)) return -CEPHFS_EINVAL;
            break;
        case ACL_USER:
        case ACL_GROUP: break;
        case ACL_GROUP_OBJ: group_entry = entry; break;
        case ACL_OTHER:
            if (perm != (mode & 7)) return -CEPHFS_EINVAL;
            break;
    ...
    if (mask_entry) {
        ...
        if (perm != ((mode >> 3) & 7)) return -CEPHFS_EINVAL;
    }
    else {
...
{
    if (acl_ea_count(size) != 5) return -1;
...
static int generate_empty_acl(void* buf, size_t size, mode_t mode)
{
    ...
    if (acl_ea_count(size) != 3) return -1;
    acl_ea_header* header = reinterpret_cast<acl_ea_header*>(buf);
    header->a_version = (__u32)ACL_EA_VERSION;
    acl_ea_entry* entry = header->a_entries;
    entry->e_tag = ACL_USER_OBJ;
