...
static void thread_ConcurrentLocking(str_ConcurrentLocking& s)
{
    ...
    ASSERT_EQ(-CEPHFS_EWOULDBLOCK, ceph_flock(cmount, fd, LOCK_EX | LOCK_NB, ceph_pthread_self()));
    PING_MAIN(1);   // (1)
...
{
    str_ConcurrentLocking* const s = reinterpret_cast<str_ConcurrentLocking*>(arg);
...
TEST(LibCephFS, ConcurrentLocking)
{
    ...
    ASSERT_EQ(-CEPHFS_EWOULDBLOCK, ceph_flock(cmount, fd, LOCK_EX | LOCK_NB, ceph_pthread_self()));
    ASSERT_EQ(-CEPHFS_EWOULDBLOCK, ceph_flock(cmount, fd, LOCK_SH | LOCK_NB, ceph_pthread_self()));
...
TEST(LibCephFS, DISABLED_InterProcessLocking)
{
    ...
    str_ConcurrentLocking* const shs = reinterpret_cast<str_ConcurrentLocking*>(
