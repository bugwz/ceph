...
static void thread_ConcurrentLocking(str_ConcurrentLocking& s)
{
    ...
    ASSERT_EQ(-EWOULDBLOCK, ceph_flock(cmount, fd, LOCK_EX | LOCK_NB, ceph_pthread_self()));
    PING_MAIN(1);   // (1)
...
TEST(LibCephFS, ConcurrentLocking)
{
    ...
    ASSERT_EQ(-EWOULDBLOCK, ceph_flock(cmount, fd, LOCK_EX | LOCK_NB, ceph_pthread_self()));
    ASSERT_EQ(-EWOULDBLOCK, ceph_flock(cmount, fd, LOCK_SH | LOCK_NB, ceph_pthread_self()));
