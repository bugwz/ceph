...
static int ceph_ll_delegation_wait(struct ceph_mount_info* cmount, Fh* fh, unsigned cmd, ceph_deleg_cb_t cb, void* priv)
{
    int ret, retry = 0;
...
static void open_breaker_func(struct ceph_mount_info* cmount, const char* filename, int flags, std::atomic_bool* opened)
{
    ...
    if (do_shutdown) ceph_shutdown(cmount);
}
...
static void namespace_breaker_func(struct ceph_mount_info* cmount, int cmd, const char* oldname, const char* newname)
{
    ...
    for (;;) {
        switch (cmd) {
        case DelegTestRename: ret = ceph_ll_rename(cmount, root, oldname, root, newname, perms); break;
        case DelegTestLink:
            if (!file) {
            ...
            break;
        case DelegTestUnlink: ret = ceph_ll_unlink(cmount, root, oldname, perms); break;
        default:
            // Bad command
            ceph_abort();
        }
...
static void simple_deleg_test(struct ceph_mount_info* cmount, struct ceph_mount_info* tcmount)
{
    ...
    ASSERT_EQ(ceph_ll_create(cmount, root, filename, 0666, O_RDWR | O_CREAT | O_EXCL, &file, &fh, &stx, 0, 0, perms),
              0);
    ...
    ASSERT_EQ(ceph_ll_create(cmount, root, filename, 0666, O_RDONLY | O_CREAT | O_EXCL, &file, &fh, &stx, 0, 0, perms),
              0);
    recalled.store(false);
...
TEST(LibCephFS, RecalledGetattr)
{
    ...
    ASSERT_EQ(ceph_ll_create(cmount1, root, filename, 0666, O_RDWR | O_CREAT | O_EXCL, &file, &fh, &stx, 0, 0, perms),
              0);
    ASSERT_EQ(ceph_ll_write(cmount1, fh, 0, sizeof(filename), filename), static_cast<int>(sizeof(filename)));
    ...
    ASSERT_EQ(ceph_ll_write(cmount2, fh, 0, sizeof(filename), filename), static_cast<int>(sizeof(filename)));
    ...
    ASSERT_EQ(ceph_ll_read(cmount2, fh, 0, sizeof(filename), filename), static_cast<int>(sizeof(filename)));
