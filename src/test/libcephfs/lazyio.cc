...
TEST(LibCephFS, LazyIOOneWriterMulipleReaders)
{
    ...
    char in_buf[40];
    /* Calling ceph_lazyio_synchronize here will invalidate client b's cache and hence enable client a to fetch the
     * propagated write of client a in the subsequent read */
    ...
    /* Client a does not need to call ceph_lazyio_synchronize here because it is the latest writer and fda holds the
     * updated inode*/
...
TEST(LibCephFS, LazyIOMultipleWritersMulipleReaders)
{
    ...
    char in_buf[40];
    /* Calling ceph_lazyio_synchronize here will invalidate client a's cache and hence enable client a to fetch the
     * propagated writes of client b in the subsequent read */
    ASSERT_EQ(0, ceph_lazyio_synchronize(ca, fda, 0, 0));
    ASSERT_EQ(ceph_read(ca, fda, in_buf, sizeof(in_buf), 0), 2 * strlen(out_buf) + 1);
    ASSERT_STREQ(in_buf, "fooooooooofooooooooo");

    /* Client b does not need to call ceph_lazyio_synchronize here because it is the latest writer and the writes before
     * it have already been propagated*/
    ASSERT_EQ(ceph_read(cb, fdb, in_buf, sizeof(in_buf), 0), 2 * strlen(out_buf) + 1);
    ASSERT_STREQ(in_buf, "fooooooooofooooooooo");
    ...
    /* Client a does not need to call ceph_lazyio_synchronize here because it is the latest writer and the writes before
     * it have already been propagated*/
...
TEST(LibCephFS, LazyIOMultipleWritersOneReader)
{
    ...
    /* Client a reads the file again, this time with a lazyio_synchronize to check if the cache gets invalidated and
     * data is refetched i.e all the propagated writes are being read*/
    ASSERT_EQ(0, ceph_lazyio_synchronize(ca, fda, 0, 0));
    ASSERT_EQ(ceph_read(ca, fda, in_buf, sizeof(in_buf), 0), 2 * strlen(out_buf) + 1);
    ASSERT_STREQ(in_buf, "fooooooooofooooooooo");
...
TEST(LibCephFS, LazyIOSynchronizeFlush)
{
    ...
    char in_buf[40];
    /* Calling ceph_lazyio_synchronize here will first flush the possibly pending buffered write of client b and
     * invalidate client b's cache and hence enable client b to fetch all the propagated writes */
    ASSERT_EQ(0, ceph_lazyio_synchronize(cb, fdb, 0, 0));
    ASSERT_EQ(ceph_read(cb, fdb, in_buf, sizeof(in_buf), 0), 3 * strlen(out_buf) + 1);
    ASSERT_STREQ(in_buf, "fooooooooofooooooooofooooooooo");

    /* Required to call ceph_lazyio_synchronize here since client b is the latest writer and client a is out of sync
     * with updated file*/
    ASSERT_EQ(0, ceph_lazyio_synchronize(ca, fda, 0, 0));
