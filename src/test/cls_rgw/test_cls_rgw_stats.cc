...
int index_prepare(librados::IoCtx& ioctx, const std::string& oid, const cls_rgw_obj_key& key, const std::string& tag,
                  RGWModifyOp type)
{
    librados::ObjectWriteOperation op;
...
int index_complete(librados::IoCtx& ioctx, const std::string& oid, const cls_rgw_obj_key& key, const std::string& tag,
                   RGWModifyOp type, const rgw_bucket_entry_ver& ver, const rgw_bucket_dir_entry_meta& meta,
                   std::list<cls_rgw_obj_key>* remove_objs)
{
    ...
    cls_rgw_bucket_complete_op(op, type, tag, ver, key, meta, remove_objs, log_op, flags, zones);
    return ioctx.operate(oid, &op);
}

void read_stats(librados::IoCtx& ioctx, const std::string& oid, rgw_bucket_dir_stats& stats)
{
...
static void account_entry(rgw_bucket_dir_stats& stats, const rgw_bucket_dir_entry_meta& meta)
{
...
static void unaccount_entry(rgw_bucket_dir_stats& stats, const rgw_bucket_dir_entry_meta& meta)
{
    rgw_bucket_category_stats& c = stats[meta.category];
    c.num_entries--;
    c.total_size -= meta.accounted_size;
...
{
    explicit object(const cls_rgw_obj_key& key) { this->key = key; }
};
...
using object_map_base = boost::intrusive::set<object, boost::intrusive::key_of_value<object_key>>;

struct object_map : object_map_base
{
    ~object_map() { clear_and_dispose(std::default_delete<object>{}); }
};
...
int simulator::try_start(const cls_rgw_obj_key& key, const std::string& tag)
{
    ...
    const auto type =
        static_cast<RGWModifyOp>(ceph::util::generate_random_number<size_t, size_t>(CLS_RGW_OP_ADD, CLS_RGW_OP_DEL));
    auto op = operation{type, key, tag};

    op.meta.category = RGWObjCategory::Main;
    op.meta.size = op.meta.accounted_size = ceph::util::generate_random_number(1, max_object_size);
    ...
    if (type == CLS_RGW_OP_ADD && op.meta.size > max_part_size) {
        ...
        if (r != 0) {
            derr << "> failed to prepare multipart upload key=" << key << " upload=" << op.upload_id << " tag=" << tag
                 << " type=" << type << ": " << cpp_strerror(r) << dendl;
            return r;
        }

        dout(1) << "> prepared multipart upload key=" << key << " upload=" << op.upload_id << " tag=" << tag
        ...
        if (r != 0) {
            derr << "> failed to prepare operation key=" << key << " tag=" << tag << " type=" << type << ": "
                 << cpp_strerror(r) << dendl;
            return r;
        }

        dout(1) << "> prepared operation key=" << key << " tag=" << tag << " type=" << type << " size=" << op.meta.size
                << dendl;
    }
...
{
    int r = index_complete(ioctx, oid, op.key, op.tag, type, op.ver, op.meta, nullptr);
    if (r != 0) {
        derr << "< failed to complete operation key=" << op.key << " tag=" << op.tag << " type=" << op.type
             << " size=" << op.meta.size << ": " << cpp_strerror(r) << dendl;
        return;
    }

    if (type == CLS_RGW_OP_CANCEL) {
        dout(1) << "< canceled operation key=" << op.key << " tag=" << op.tag << " type=" << op.type
        ...
        dout(1) << "< completed write operation key=" << op.key << " tag=" << op.tag << " type=" << type
                << " size=" << op.meta.size << dendl;
    }
    else {
        ...
        dout(1) << "< completed delete operation key=" << op.key << " tag=" << op.tag << " type=" << type << dendl;
    }
}
...
void simulator::suggest(const operation& op, char suggestion)
{
    ...
    if (r != 0) {
        derr << "< no bi entry to suggest for operation key=" << op.key << " tag=" << op.tag << " type=" << op.type
             << " size=" << op.meta.size << ": " << cpp_strerror(r) << dendl;
        return;
    }
    ...
    if (r != 0) {
        derr << "< failed to suggest operation key=" << op.key << " tag=" << op.tag << " type=" << op.type
             << " size=" << op.meta.size << ": " << cpp_strerror(r) << dendl;
        return;
    }
    ...
    if (suggestion == CEPH_RGW_UPDATE) {
        ...
        obj->meta = op.meta;
        account_entry(stats, obj->meta);
        dout(1) << "< suggested update operation key=" << op.key << " tag=" << op.tag << " type=" << op.type
                << " size=" << op.meta.size << dendl;
    }
    else {
        ...
        dout(1) << "< suggested remove operation key=" << op.key << " tag=" << op.tag << " type=" << op.type << dendl;
    }
}
...
int simulator::init_multipart(const operation& op)
{
    ...
    const auto meta_key = cls_rgw_obj_key{fmt::format("_multipart_{}.2~{}.meta", op.key.name, op.upload_id)};
    ...
    int r = index_complete(ioctx, oid, meta_key, empty_tag, CLS_RGW_OP_ADD, empty_ver, meta_meta, nullptr);
    if (r != 0) {
        derr << "  < failed to create multipart meta key=" << meta_key << ": " << cpp_strerror(r) << dendl;
        return r;
    }
    else {
    ...
    while (remaining > max_part_size) {
        ...
        const auto part_key = cls_rgw_obj_key{fmt::format("_multipart_{}.2~{}.{}", op.key.name, op.upload_id, part_id)};
        part_id++;
        ...
        if (r != 0) {
            ...
            [[maybe_unused]] int ignored =
                index_complete(ioctx, oid, meta_key, empty_tag, CLS_RGW_OP_DEL, empty_ver, meta_meta, &remove_objs);
            derr << "  > failed to prepare part key=" << part_key << " size=" << part_size << dendl;
            return r;   // return the error from prepare
        }
        dout(1) << "  > prepared part key=" << part_key << " size=" << part_size << dendl;
...
void simulator::complete_multipart(const operation& op)
{
    ...
    while (remaining > max_part_size) {
        ...
        if (result < cancel_percent) {
            ...
            dout(1) << "  < canceled part key=" << part_key << " size=" << part_size << dendl;
        }
        else {
            rgw_bucket_dir_entry_meta meta;
            ...
            int r = index_complete(ioctx, oid, part_key, op.tag, op.type, empty_ver, meta, nullptr);
            if (r != 0) {
                derr << "  < failed to complete part key=" << part_key << " size=" << meta.size << ": "
                     << cpp_strerror(r) << dendl;
                type = CLS_RGW_OP_CANCEL;   // abort multipart
            }
            else {
                dout(1) << "  < completed part key=" << part_key << " size=" << meta.size << dendl;
    ...
    int r = index_complete(ioctx, oid, meta_key, empty_tag, CLS_RGW_OP_DEL, empty_ver, meta_meta, nullptr);
    if (r != 0) {
        derr << "  < failed to remove multipart meta key=" << meta_key << ": " << cpp_strerror(r) << dendl;
    }
    else {
    ...
    r = index_complete(ioctx, oid, op.key, empty_tag, type, empty_ver, op.meta, &remove_objs);
    if (r != 0) {
        derr << "< failed to complete multipart upload key=" << op.key << " upload=" << op.upload_id
             << " tag=" << op.tag << " type=" << type << " size=" << op.meta.size << ": " << cpp_strerror(r) << dendl;
        return;
    }

    if (type == CLS_RGW_OP_ADD) {
        dout(1) << "< completed multipart upload key=" << op.key << " upload=" << op.upload_id << " tag=" << op.tag
        ...
        dout(1) << "< canceled multipart upload key=" << op.key << " upload=" << op.upload_id << " tag=" << op.tag
                << " type=" << op.type << " size=" << op.meta.size << dendl;
    }
