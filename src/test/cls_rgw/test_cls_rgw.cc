...
void index_prepare(librados::IoCtx& ioctx, string& oid, RGWModifyOp index_op, string& tag, const cls_rgw_obj_key& key,
                   string& loc, uint16_t bi_flags = 0, bool log_op = true)
{
...
void index_complete(librados::IoCtx& ioctx, string& oid, RGWModifyOp index_op, string& tag, int epoch,
                    const cls_rgw_obj_key& key, rgw_bucket_dir_entry_meta& meta, uint16_t bi_flags = 0,
                    bool log_op = true)
{
    ObjectWriteOperation op;
    ...
    if (!key.instance.empty()) {
        ...
        ASSERT_EQ(0,
                  cls_rgw_bucket_link_olh(
                      ioctx, oid, key, olh_tag, false, tag, &meta, epoch, ceph::real_time{}, true, true, zone_set));
    }
}
...
static void list_entries(librados::IoCtx& ioctx, const std::string& oid, uint32_t num_entries,
                         std::map<int, rgw_cls_list_ret>& results)
{
    ...
    ASSERT_EQ(
        0,
        CLSRGWIssueBucketList(ioctx, start_key, empty_prefix, empty_delimiter, num_entries, true, oids, results, 1)());
}
...
TEST_F(cls_rgw, index_list)
{
    ...
    for (int i = 0; i < num_objs; i++) {
        ...
        index_prepare(ioctx, bucket_oid, CLS_RGW_OP_ADD, tag, obj, loc, 0 /* bi_flags */, false /* log_op */);

        rgw_bucket_dir_entry_meta meta;
        meta.category = RGWObjCategory::None;
        meta.size = obj_size;
        index_complete(ioctx, bucket_oid, CLS_RGW_OP_ADD, tag, epoch, obj, meta, 0 /* bi_flags */, false /* log_op */);
    }
    ...
    string empty_prefix;
    string empty_delimiter;
    int r = CLSRGWIssueBucketList(ioctx, start_key, empty_prefix, empty_delimiter, 1000, true, oids, list_results, 1)();
    ASSERT_EQ(r, 0);
...
TEST_F(cls_rgw, index_list_delimited)
{
    ...
    std::vector<std::string> file_prefixes = {"a", "c", "e", "g", "i", "k", "m", "o", "q", "s", "u"};
    std::vector<std::string> dir_prefixes = {"b/", "d/", "f/", "h/", "j/", "l/", "n/", "p/", "r/", "t/"};
    ...
    for (const auto& p : file_prefixes) {
        for (int i = 0; i < file_num_objs; i++) {
            ...
            index_complete(
                ioctx, bucket_oid, CLS_RGW_OP_ADD, tag, epoch, obj, meta, 0 /* bi_flags */, false /* log_op */);
        }
    }
    ...
    int r = CLSRGWIssueBucketList(ioctx, start_key, empty_prefix, delimiter, 1000, true, oids, list_results, 1)();
    ASSERT_EQ(r, 0);
    ASSERT_EQ(1u, list_results.size()) << "Because we only have one bucket index shard, we should "
    ...
    ASSERT_EQ(48u, id_entry_map.size()) << "We should get 40 top-level entries and the tops of 8 \"subdirectories\".";
    ...
    r = CLSRGWIssueBucketList(ioctx, start_key2, empty_prefix, delimiter, 1000, true, oids, list_results, 1)();
    ASSERT_EQ(r, 0);
    ...
    ASSERT_EQ(17u, id_entry_map.size()) << "We should get 15 top-level entries and the tops of 2 \"subdirectories\".";
...
TEST_F(cls_rgw, bi_list)
{
    ...
    int ret = cls_rgw_bi_list(ioctx, bucket_oid, empty_name_filter, marker, max, &entries, &is_truncated);
    ASSERT_EQ(ret, 0);
    ASSERT_EQ(entries.size(), 0u) << "The listing of an empty bucket as 0 entries.";
    ASSERT_EQ(is_truncated, false) << "The listing of an empty bucket is not truncated.";
    ...
    ret = cls_rgw_bi_list(ioctx, bucket_oid, empty_name_filter, marker, num_objs + 10, &entries, &is_truncated);
    ASSERT_EQ(ret, 0);
    if (is_truncated) {
    ...
    marker.clear();
    while (is_truncated) {
        ret = cls_rgw_bi_list(ioctx, bucket_oid, empty_name_filter, marker, max, &entries, &is_truncated);
        ASSERT_EQ(ret, 0);
        if (is_truncated) {
    ...
    const std::list<std::pair<const std::string, unsigned>> filters_results = {
        {str_int("obj", 9), 1}, {str_int("об'єкт", 8), 1}, {str_int("obj", 8), 0}};
    for (const auto& filter_result : filters_results) {
        is_truncated = true;
        entries.clear();
        marker.clear();

        ret = cls_rgw_bi_list(ioctx, bucket_oid, filter_result.first, marker, max, &entries, &is_truncated);
        ...
        ASSERT_EQ(is_truncated, false) << "bi list test with filters should return correct truncation indicator";
    }
    ...
    ret = cls_rgw_bi_list(ioctx, bucket_oid, empty_name_filter, marker, num_objs - 1, &entries, &is_truncated);
    ASSERT_EQ(ret, 0) << "combined segment count should succeed";
    ASSERT_EQ(entries.size(), num_objs - 1) << "combined segment count should return the correct number of results";
    ASSERT_EQ(is_truncated, true) << "combined segment count should return correct truncation indicator";


    marker = entries.back().idx;   // advance marker
    ret = cls_rgw_bi_list(ioctx, bucket_oid, empty_name_filter, marker, num_objs - 1, &entries, &is_truncated);
    ASSERT_EQ(ret, 0) << "combined segment count should succeed";
    ASSERT_EQ(entries.size(), 1) << "combined segment count should return the correct number of results";
    ASSERT_EQ(is_truncated, false) << "combined segment count should return correct truncation indicator";
}
...
{
    return (obj1.pool == obj2.pool) && (obj1.key == obj2.key) && (obj1.loc == obj2.loc);
}
...
TEST_F(cls_rgw, usage_basic)
{
    ...
    int ret =
        cls_rgw_usage_log_read(ioctx, oid, user, "", start_epoch, end_epoch, max_entries, read_iter, usage, &truncated);
    ...
    ret = cls_rgw_usage_log_read(
        ioctx, oid, user, "", start_epoch, end_epoch, max_entries, read_iter, usage2, &truncated);
    ASSERT_EQ(0, ret);
    ...
    ASSERT_EQ(0, ioctx.operate(oid, &op));
    ret = cls_rgw_usage_log_read(
        ioctx, oid, "", bucket1, start_epoch, end_epoch, max_entries, read_iter, usage2, &truncated);
    ASSERT_EQ(0, ret);
...
static int bilog_list(librados::IoCtx& ioctx, const std::string& oid, cls_rgw_bi_log_list_ret* result)
{
    int retcode = 0;
...
static int bilog_trim(librados::IoCtx& ioctx, const std::string& oid, const std::string& start_marker,
...
TEST_F(cls_rgw, bi_log_trim)
{
    ...
    // bi list
    {
        ...
        ASSERT_EQ(0, cls_rgw_bi_list(ioctx, bucket_oid, "", "", 128, &entries, &truncated));
    ...
    vector<rgw_bi_log_entry> bilog1;
    {
        ...
        bilog1.assign(std::make_move_iterator(bilog.entries.begin()), std::make_move_iterator(bilog.entries.end()));
    }
    // trim front of bilog
    {
