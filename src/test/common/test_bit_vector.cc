...
TYPED_TEST(BitVectorTest, resize)
{
    ...
    ASSERT_EQ(bit_vector.get_data().length(), static_cast<uint64_t>(std::ceil(size / elements_per_byte)));
}
...
TYPED_TEST(BitVectorTest, get_buffer_extents)
{
    ...
    bit_vector.get_data_extents(offset, length, &data_byte_offset, &object_byte_offset, &byte_length);
    ASSERT_EQ(bit_vector.BLOCK_SIZE, data_byte_offset);
    ASSERT_EQ(bit_vector.BLOCK_SIZE + (element_count % bit_vector.BLOCK_SIZE), byte_length);

    bit_vector.get_data_extents(1, 1, &data_byte_offset, &object_byte_offset, &byte_length);
...
TYPED_TEST(BitVectorTest, get_footer_offset)
{
    ...
    uint64_t object_byte_offset;
    uint64_t byte_length;
    bit_vector.get_data_extents(0, bit_vector.size(), &data_byte_offset, &object_byte_offset, &byte_length);

    ASSERT_EQ(bit_vector.get_header_length() + byte_length, bit_vector.get_footer_offset());
}
...
TYPED_TEST(BitVectorTest, partial_decode_encode)
{
    ...
    Extents extents = boost::assign::list_of(std::make_pair(0, 1))(
        std::make_pair((bit_vector.BLOCK_SIZE * elements_per_byte) - 2, 4))(
        std::make_pair((bit_vector.BLOCK_SIZE * elements_per_byte) + 2, 2))(
        std::make_pair((2 * bit_vector.BLOCK_SIZE * elements_per_byte) - 2, 4))(std::make_pair(
        (2 * bit_vector.BLOCK_SIZE * elements_per_byte) + 2, 2))(std::make_pair(2, 2 * bit_vector.BLOCK_SIZE));
    for (Extents::iterator it = extents.begin(); it != extents.end(); ++it) {
        ...
        bit_vector.get_data_crcs_extents(it->first, it->second, &footer_byte_offset, &footer_byte_length);
        ...
        uint64_t data_byte_offset;
        bit_vector.get_data_extents(
            element_offset, element_length, &data_byte_offset, &object_byte_offset, &byte_length);

        bufferlist data_bl;
        data_bl.substr_of(bl, bit_vector.get_header_length() + data_byte_offset, byte_length);
        ...
        updated_bl.substr_of(bl, 0, bit_vector.get_header_length() + data_byte_offset);
        updated_bl.append(data_bl);

        if (data_byte_offset + byte_length < bit_vector.get_footer_offset()) {
            uint64_t tail_data_offset = bit_vector.get_header_length() + data_byte_offset + byte_length;
            data_bl.substr_of(bl, tail_data_offset, bit_vector.get_footer_offset() - tail_data_offset);
            updated_bl.append(data_bl);
        }

        bufferlist full_footer;
        full_footer.substr_of(bl, bit_vector.get_footer_offset(), footer_byte_offset - bit_vector.get_footer_offset());
...
TYPED_TEST(BitVectorTest, data_crc)
{
    ...
    uint64_t object_byte_offset;
    uint64_t byte_length;
    bit_vector1.get_data_extents(0, bit_vector1.size(), &data_byte_offset, &object_byte_offset, &byte_length);
    ...
    data_it = data.begin();
    ASSERT_THROW(bit_vector2.decode_data(data_it, data_byte_offset), buffer::malformed_input);
}
