    ...
    virtual void SetUp()
    {
        ...
        *root += "/src/test/common/test_blkdev_sys_block/sys";
        ...
        EXPECT_CALL(sda, wholedisk(NotNull(), Ge(0ul)))
            .WillRepeatedly(DoAll(SetArrayArgument<0>(sda_name, sda_name + strlen(sda_name) + 1), Return(0)));

        EXPECT_CALL(sdb, sysfsdir()).WillRepeatedly(Return(root->c_str()));
        EXPECT_CALL(sdb, wholedisk(NotNull(), Ge(0ul)))
            .WillRepeatedly(DoAll(SetArrayArgument<0>(sdb_name, sdb_name + strlen(sdb_name) + 1), Return(0)));
    }

    virtual void TearDown() { delete root; }
...
TEST(blkdev, _decode_model_enc)
{
    ...
    const char* foo[][2] = {
        {"WDC\\x20WDS200T2B0A-"
         "00SM50\\x20\\x20\\x20\\x20\\x20\\x20\\x20\\x20\\x20\\x20\\x20\\x20\\x20\\x20\\x20\\x20\\x20\\x20",
...
TEST(blkdev, get_device_id)
{
    ...
    for (char c = 'a'; c < 'z'; ++c) {
        ...
        cout << "devname " << devname << " -> '" << i << "' (" << err << ")" << std::endl;
    }
}
