...
protected:
    ...
    void fill_queue(WQ& wq, LQ& strictq, LQ& normq, unsigned item_size, bool randomize = false)
    {
        ...
        for (unsigned i = 1; i <= item_size; ++i) {
            ...
            case 6:
                ...
                if (fob == 4) {
                    // Queue to the front.
                    strictq[p][k].push_front(std::make_pair(c, std::make_tuple(p, k, o)));
                    wq.enqueue_strict_front(Klass(k), p, std::make_tuple(p, k, o));
                }
                else {
                    // Queue to the back.
                    strictq[p][k].push_back(std::make_pair(c, std::make_tuple(p, k, o)));
            ...
            default:
                ...
                if (fob == 4) {
                    // Queue to the front.
                    normq[p][k].push_front(std::make_pair(c, std::make_tuple(p, k, o)));
                    wq.enqueue_front(Klass(k), p, c, std::make_tuple(p, k, o));
                }
                else {
                    // Queue to the back.
                    normq[p][k].push_back(std::make_pair(c, std::make_tuple(p, k, o)));
...
TEST_F(WeightedPriorityQueueTest, wpq_test_remove_by_class)
{
    ...
    for (LQ::iterator it = strictq.begin(); it != strictq.end(); ++it) {
        num_to_remove += it->second[k].size();
    }
    for (LQ::iterator it = normq.begin(); it != normq.end(); ++it) {
        num_to_remove += it->second[k].size();
    }
    Removed wq_removed;
    ...
    for (Removed::iterator it = wq_removed.begin(); it != wq_removed.end(); ++it) {
