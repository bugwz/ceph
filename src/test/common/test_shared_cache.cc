...
public:
    ...
    map<unsigned int, pair<std::weak_ptr<int>, int*> >& get_weak_refs() { return weak_refs; }
};
...
public:
    ...
    public:
        ...
        {
            switch (in_method) {
            case LOWER_BOUND: ptr = cache.lower_bound(key); break;
            case LOOKUP:
...
// Local Variables:
// compile-command: "cd ../.. ; make unittest_shared_cache && ./unittest_shared_cache # --gtest_filter=*.*
