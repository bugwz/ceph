...
TEST(AsyncCompletion, CompletionList)
{
    ...
    completions.clear_and_dispose([](Completion* c) { Completion::post(std::unique_ptr<Completion>{c}); });
...
struct throws_on_move
{
    ...
    throws_on_move& operator=(throws_on_move&&) { throw std::runtime_error("oops"); }
...
TEST(AsyncCompletion, ThrowOnCtor)
{
    ...
    auto ex1 = context.get_executor();
    {
        ...
        EXPECT_THROW(Completion::create(ex1,
    ...
    }
    {
        ...
        EXPECT_THROW(Completion::create(
...
TEST(AsyncCompletion, FreeFunctions)
{
    ...
    auto c2 = create_completion<void(int), int>(
        ex1, [](int) {}, 5);
