...
static_assert(!coarse_real_clock::is_steady, "ceph::coarse_real_clock must not be steady.");

static_assert(mono_clock::is_steady, "ceph::mono_clock must be steady.");
static_assert(coarse_mono_clock::is_steady, "ceph::coarse_mono_clock must be steady.");
...
template<typename Clock> static void system_clock_sanity()
{
...
template<typename Clock> static void system_clock_conversions()
{
    static typename Clock::time_point brt(seconds(bs) + nanoseconds(bns));
...
TEST(TimePoints, stringify)
{
    ...
    ceph::coarse_real_clock::time_point ctp(seconds(1556122013) + nanoseconds(399000000));
    s = stringify(ctp);
...
namespace {
template<typename Rep, typename Period> std::string to_string(const chrono::duration<Rep, Period>& t)
{
...
void float_format_eq(string_view lhs, string_view rhs, unsigned precision)
{
...
{
    float_format_eq("0.123456700s", to_string(std::chrono::duration_cast<ceph::timespan>(0.1234567s)), 9);
    float_format_eq("-0.123456700s", to_string(std::chrono::duration_cast<ceph::signedspan>(-0.1234567s)), 9);
