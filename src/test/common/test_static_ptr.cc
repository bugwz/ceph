...
TEST(StaticPtr, ConstCast)
{
    static constexpr auto sz = sizeof(constable);
    {
        ...
        static_assert(!std::is_const<decltype(p2)::element_type>{}, "Things are more const than they ought to be.");
        EXPECT_TRUE(p2);
        EXPECT_EQ(p2->foo(), 2);
    }
}
...
TEST(StaticPtr, ReinterpretCast)
{
    static constexpr auto sz = sizeof(grandchild);
    {
        ...
        static_assert(std::is_same<decltype(p2)::element_type, constable>{}, "Reinterpret is screwy.");
        auto p3 = ceph::reinterpret_pointer_cast<grandchild, sz>(std::move(p2));
        static_assert(std::is_same<decltype(p3)::element_type, grandchild>{}, "Reinterpret is screwy.");
        EXPECT_EQ(p3->func(), 9);
