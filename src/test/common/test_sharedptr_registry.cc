...
public:
    ...
    map<unsigned int, pair<std::weak_ptr<int>, int*> >& get_contents() { return contents; }
};
    ...
    bool wait_for(SharedPtrRegistryTest& registry, int waiting)
    {
        do {
            ...
            if (delay > 0) usleep(delay);
            {
                ...
                if (registry.waiting == waiting) break;
            }
            if (delay > 0) cout << "delay " << delay << "us, is not long enough, try again\n";
...
public:
    ...
    void SetUp() override { died = UNDEFINED; }
};
...
// Local Variables:
// compile-command: "cd ../.. ; make unittest_sharedptr_registry && ./unittest_sharedptr_registry # --gtest_filter=*.*
// --log-to-stderr=true" End:
