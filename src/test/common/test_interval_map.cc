...
template<typename _key> struct bufferlist_test_type
{
    using key = _key;
    ...
    struct make_splitter
    {
        template<typename merge_t> struct apply
        {
            ...
            bool can_merge(const bufferlist& left, const bufferlist& right) const { return merge_t::value; }
            ...
            uint64_t length(const bufferlist& r) const { return r.length(); }
        };
    };
...
#define USING(_can_merge)                                                                \
    ...
    using key = typename TT::key;                                                        \
    (void)key(0);                                                                        \
    using val = typename TT::value;                                                      \
    (void)val(0);                                                                        \
    using splitter = typename boost::mpl::apply<typename TT::make_splitter, _can_merge>; \
    using imap = interval_map<key, val, splitter>;                                       \
    (void)imap();                                                                        \
    typename TT::generate_random gen;                                                    \
    val v(gen(5));                                                                       \
    splitter split;                                                                      \
    (void)split.split(0, 0, v);
