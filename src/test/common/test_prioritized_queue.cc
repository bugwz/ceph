...
protected:
    ...
    void TearDown() override { items.clear(); }
};
...
TEST_F(PrioritizedQueueTest, fairness_by_class)
{
    ...
    unsigned total = std::accumulate(num_picked_in_class.begin(), num_picked_in_class.end(), 0);
...
TEST_F(PrioritizedQueueTest, remove_by_class)
{
    ...
    for (std::list<Item>::iterator it = removed.begin(); it != removed.end(); ++it) {
        const Item& item = *it;
        Klass k = ITEM_TO_CLASS(item);
