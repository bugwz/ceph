...
TEST(iso_8601, epoch)
{
    ...
    ASSERT_EQ("1970-01-01T00:00:00.000000000Z", to_iso_8601(epoch, iso_8601_format::YMDhmsn));
...
TEST(iso_8601, now)
{
    ...
    ASSERT_EQ(real_time(time_point_cast<minutes>(now)), *from_iso_8601(to_iso_8601(now, iso_8601_format::YMDhm)));
    ASSERT_EQ(real_time(time_point_cast<seconds>(now)), *from_iso_8601(to_iso_8601(now, iso_8601_format::YMDhms)));
