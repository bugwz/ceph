...
template<typename SharedMutex> static bool test_try_lock(SharedMutex* sm)
{
    if (!sm->try_lock()) return false;
    sm->unlock();
    return true;
}

template<typename SharedMutex> static bool test_try_lock_shared(SharedMutex* sm)
{
    if (!sm->try_lock_shared()) return false;
    sm->unlock_shared();
    return true;
}

template<typename SharedMutex, typename AcquireType> static void check_conflicts(SharedMutex sm, AcquireType) {}

template<typename SharedMutex> static void ensure_conflicts(SharedMutex& sm, ceph::acquire_unique_t)
{
...
template<typename SharedMutex> static void ensure_conflicts(SharedMutex& sm, ceph::acquire_shared_t)
{
    auto ttl = &test_try_lock<std::shared_timed_mutex>;
    auto ttls = &test_try_lock_shared<std::shared_timed_mutex>;
    ASSERT_FALSE(std::async(std::launch::async, ttl, &sm).get());
...
template<typename SharedMutex> static void ensure_free(SharedMutex& sm)
{
    auto ttl = &test_try_lock<std::shared_timed_mutex>;
    auto ttls = &test_try_lock_shared<std::shared_timed_mutex>;
...
static void check_owns_lock(const SharedMutex& sm, const ceph::shunique_lock<SharedMutex>& sul, AcquireType)
{}

template<typename SharedMutex>
static void check_owns_lock(const SharedMutex& sm, const ceph::shunique_lock<SharedMutex>& sul, ceph::acquire_unique_t)
{
...
template<typename SharedMutex>
static void check_owns_lock(const SharedMutex& sm, const ceph::shunique_lock<SharedMutex>& sul, ceph::acquire_shared_t)
{
...
template<typename SharedMutex>
static void check_abjures_lock(const SharedMutex& sm, const ceph::shunique_lock<SharedMutex>& sul)
{
...
template<typename SharedMutex> static void check_abjures_lock(const ceph::shunique_lock<SharedMutex>& sul)
{
...
template<typename AcquireType> void lock_unlock(AcquireType at)
{
...
template<typename AcquireType> void lock_destruct(AcquireType at)
{
    std::shared_timed_mutex sm;
    typedef ceph::shunique_lock<std::shared_timed_mutex> shunique_lock;
...
template<typename AcquireType> void move_construct(AcquireType at)
{
    std::shared_timed_mutex sm;
...
template<typename AcquireType> void move_assign(AcquireType at)
{
    std::shared_timed_mutex sm;
...
template<typename AcquireType> void construct_deferred(AcquireType at)
{
    std::shared_timed_mutex sm;
...
template<typename AcquireType> void construct_try(AcquireType at)
{
    std::shared_timed_mutex sm;
    typedef ceph::shunique_lock<std::shared_timed_mutex> shunique_lock;
...
template<typename AcquireType> void construct_adopt(AcquireType at)
{
    std::shared_timed_mutex sm;
...
template<typename AcquireType> void try_lock(AcquireType at)
{
    std::shared_timed_mutex sm;
