...
TEST_F(ThrottleTest, get)
{
    ...
    do {
        ...
        if (!(waited = t.waited && u.waited)) delay *= 2;
    } while (!waited);
}
...
std::pair<double, std::chrono::duration<double> > test_backoff(double low_threshhold, double high_threshhold,
                                                               double expected_throughput, double high_multiple,
                                                               double max_multiple, uint64_t max,
                                                               double put_delay_per_count, unsigned getters,
                                                               unsigned putters)
{
    std::mutex l;
    ...
    bool valid =
        throttle.set_params(low_threshhold, high_threshhold, expected_throughput, high_multiple, max_multiple, max, 0);
    ceph_assert(valid);
    ...
    auto putter = [&]() {
        ...
        while (!stop_putters || !in_queue.empty()) {
            ...
            g.unlock();
            std::this_thread::sleep_for(c * std::chrono::duration<double>(put_delay_per_count * putters));
            g.lock();
    ...
    return make_pair(((double)total_observed_total) / ((double)total_observations), wait_time / waits);
}

TEST(BackoffThrottle, undersaturated)
{
    auto results = test_backoff(0.4, 0.6, 1000, 2, 10, 100, 0.0001, 3, 6);
...
{
    auto results = test_backoff(0.4, 0.6, 1000, 2, 10, 100, 0.001, 7, 2);
...
{
    auto results = test_backoff(0.4, 0.6, 10000000, 2, 10, 100, 0.001, 1, 3);
