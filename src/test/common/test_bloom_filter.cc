...
TEST(BloomFilter, Sweep)
{
    ...
    for (int ex = 3; ex < 12; ex += 2) {
        for (float fpp = .001; fpp < .5; fpp *= 4.0) {
            ...
            for (int n = 0; n < max; n++) bf.insert("ok" + stringify(n));

            int test = max * 100;
            int hit = 0;
            for (int n = 0; n < test; n++)
                if (bf.contains("asdf" + stringify(n))) hit++;
...
TEST(BloomFilter, SweepInt)
{
    ...
    for (int ex = 3; ex < 12; ex += 2) {
        for (float fpp = .001; fpp < .5; fpp *= 4.0) {
            ...
            for (int n = 0; n < max; n++) bf.insert((uint32_t)rand());

            int test = max * 100;
            int hit = 0;
            for (int n = 0; n < test; n++)
                if (bf.contains((uint32_t)rand())) hit++;
...
TEST(BloomFilter, CompressibleSweep)
{
    ...
    for (int div = 1; div < 10; div++) {
        ...
        if (div > 1) bf.compress(1.0 / div);

        for (auto val : values) ASSERT_TRUE(bf.contains(val));
        ...
        std::cout << max << "\t" << t << "\t" << est << "\t" << est_after << "\t" << fpp << "\t" << actual << "\t"
...
TEST(BloomFilter, BinSweep)
{
    ...
    for (int bins = 1; bins < 16; ++bins) {
        ...
        double actual = (double)hit / (double)test;
        std::cout << "bins " << bins << " bin-max " << max << " bin-fpp " << fpp << " actual-fpp " << actual
