...
static void json_print(const std::vector<MonCommand>& mon_commands)
{
    ...
    Monitor::format_command_descriptions(mon_commands, f.get(), CEPH_FEATURES_ALL, &rdata);
...
#define COMMAND(parsesig, helptext, modulename, req_perms) {parsesig, helptext, modulename, req_perms, 0},
...
#define COMMAND(parsesig, helptext, modulename, req_perms) {parsesig, helptext, modulename, req_perms, FLAG(MGR)},
#define COMMAND_WITH_FLAG(parsesig, helptext, modulename, req_perms, flags) \
