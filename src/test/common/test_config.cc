...
public:
    ...
    void test_expand_meta()
    {
        ...
        // recursive variable expansion
        {
            ...
            early_expand_meta(val, &oss);
            EXPECT_EQ(public_network + " " + public_network + " " + lockdep + " " + "localhost", val);
            EXPECT_EQ("", oss.str());
        }
        // variable expansion loops are non fatal
        {
            ...
            const char* expected_oss = "variable expansion loop at mon_host=$cluster_network\n"
                                       "expansion stack:\n"
...
TEST(md_config_t, set_val)
{
    ...
    // set_val should support 1 days 2 hours 4 minutes
    {
        ...
        auto expected =
            (duration_cast<seconds>(days_t{1}) + duration_cast<seconds>(hours{2}) + duration_cast<seconds>(minutes{4}));
        EXPECT_EQ(0, conf.set_val("mgr_tick_period", "1 days 2 hours 4 minutes", nullptr));
    ...
    std::vector<testcase> good = {
        ...
        {"1d2h3m4s"s,
         duration_cast<seconds>(days_t{1}) + duration_cast<seconds>(hours{2}) + duration_cast<seconds>(minutes{3}) +
