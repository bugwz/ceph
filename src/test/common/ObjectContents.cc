...
void ObjectContents::clone_range(ObjectContents& other, interval_set<uint64_t>& intervals)
{
    ...
    for (interval_set<uint64_t>::iterator i = written_to_clone.begin(); i != written_to_clone.end(); ++i) {
        ...
        seeds.insert(other.seeds.upper_bound(start), other.seeds.lower_bound(start + len));
    }

    if (intervals.range_end() > _size) _size = intervals.range_end();
    _exists = true;
    return;
}

void ObjectContents::write(unsigned int seed, uint64_t start, uint64_t len)
{
    _exists = true;
    ...
    if (start + len > _size) _size = start + len;
    return;
}
