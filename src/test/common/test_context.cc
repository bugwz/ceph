...
TEST(CephContext, do_command)
{
...

    {
        ...
        f->flush(out);
        string s(out.c_str(), out.length());
        EXPECT_EQ("<config_diff_get><diff><key><default></default><override>" + value +
                      "</override><final>value</final></key><rbd_default_features><default>61</default><final>61</"
                      "final></rbd_default_features><rbd_qos_exclude_ops><default>0</default><final>0</final></"
                      "rbd_qos_exclude_ops></diff></config_diff_get>",
                  s);
    }
    cct->put();
}
...
TEST(CephContext, experimental_features)
{
    ...
    cct->_conf.set_val("enable_experimental_unrecoverable_data_corrupting_features", "foo,bar");
    ...
    cct->_conf.set_val("enable_experimental_unrecoverable_data_corrupting_features", "baz foo");
    cct->_conf.apply_changes(&cout);
    ASSERT_TRUE(cct->check_experimental_feature_enabled("foo"));
    ASSERT_FALSE(cct->check_experimental_feature_enabled("bar"));
