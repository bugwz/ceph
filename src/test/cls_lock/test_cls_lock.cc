               ...
               ClsLockType* assert_type, string* assert_tag)
{
    ...
    for (liter = lockers.begin(); liter != lockers.end(); ++liter) {
        ...
        cout << "    " << locker.locker << " expiration=" << liter->second.expiration << " addr=" << liter->second.addr
             << " cookie=" << locker.cookie << std::endl;
    }
}
...
bool lock_expired(IoCtx* ioctx, string& oid, string& name)
{
    ...
    if (0 == get_lock_info(ioctx, oid, name, &lockers, &lock_type, &tag)) return false;
    utime_t now = ceph_clock_now();
    map<locker_id_t, locker_info_t>::iterator liter;
    for (liter = lockers.begin(); liter != lockers.end(); ++liter) {
        if (liter->second.expiration > now) return false;
    }
...
TEST(ClsLock, TestRenew)
{
    ...
    ASSERT_EQ(0, l1.lock_exclusive(&ioctx, oid1));
    sleep(7);
    ASSERT_EQ(0, l1.lock_exclusive(&ioctx, oid1)) << "when a cls_lock is set to may_renew, a relock after expiration "
                                                     "should still work";
    ...
    ASSERT_EQ(-ENOENT, l3.lock_exclusive(&ioctx, oid3)) << "unable to create a lock with must_renew";
