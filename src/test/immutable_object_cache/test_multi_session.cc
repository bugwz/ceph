...
public:
    ...
    TestMultiSession()
        : m_local_path("/tmp/ceph_test_multisession_socket")
        , m_cache_server_thread(nullptr)
    ...
    void SetUp() override
    {
        ...
        m_cache_server =
            new CacheServer(g_ceph_context, m_local_path, [this](CacheSession* session_id, ObjectCacheRequest* req) {
    ...
    void server_handle_request(CacheSession* session_id, ObjectCacheRequest* req)
    {
        ...
        case RBDSC_READ:
        {
            ObjectCacheRequest* reply = new ObjectCacheReadReplyData(RBDSC_READ_REPLY, req->seq);
            session_id->send(reply);
            break;
        }
        }
    ...
    void test_register_client(uint64_t random_index)
    {
        ...
        auto ctx = new LambdaContext([](int ret) { ASSERT_TRUE(ret == 0); });
    ...
    void test_lookup_object(std::string pool_nspace, uint64_t index, uint64_t request_num, bool is_last)
    {

        for (uint64_t i = 0; i < request_num; i++) {
            auto ctx = make_gen_lambda_context<ObjectCacheRequest*, std::function<void(ObjectCacheRequest*)>>(
                [this](ObjectCacheRequest* ack) { m_recv_ack_index++; });
...
TEST_F(TestMultiSession, test_multi_session)
{
    ...
    for (uint64_t i = 0; i <= test_times; i++) {
        ...
        if (m_cache_client_vec[random_index] == nullptr) {
            ...
            test_lookup_object(
                string("test_nspace") + std::to_string(random_index), random_index, 4, i == test_times ? true : false);
        }
    }
