...
class FailoverSuitePeer : public Dispatcher
{
    ...
    void ms_fast_dispatch(Message* m) override
    {
        ...
        if (tracked_conn == nullptr) {
            ...
            lderr(cct) << "[TestPeer] got op from Test: conn(" << conn << ") != tracked_conn(" << tracked_conn << ")"
                       << dendl;
            ceph_abort();
        }
        else {
    ...
    void ms_handle_fast_connect(Connection* conn) override
    {
        if (tracked_conn == conn) {
            ...
            lderr(cct) << "[TestPeer] connected: conn(" << conn << ") != tracked_conn(" << tracked_conn << ")" << dendl;
            ceph_abort();
        }
    }
    void ms_handle_fast_accept(Connection* conn) override
    {
        if (tracked_conn == nullptr) {
            ...
            lderr(cct) << "[TestPeer] accepted: conn(" << conn << ") != tracked_conn(" << tracked_conn << ")" << dendl;
            ceph_abort();
        }
        else {
    ...
    bool ms_handle_reset(Connection* conn) override
    {
        if (tracked_conn == conn) {
            ...
            ldout(cct, 0) << "[!TestPeer] reset(invalid event): conn(" << conn << ") != tracked_conn(" << tracked_conn
                          << ")" << dendl;
        }
        return true;
    }
...
private:
    ...
    {
        peer_msgr.reset(
            Messenger::create(cct, "async", entity_name_t::OSD(TEST_PEER_OSD), "TestPeer", TEST_PEER_NONCE));
...
public:
    ...
    void connect_peer(entity_addr_t test_addr)
    {
        ...
        if (tracked_conn) {
            if (tracked_conn == conn.get()) {
                ...
                ldout(cct, 0) << "[TestPeer] this is a new session " << conn.get() << ", replacing old one "
                              << tracked_conn << dendl;
            }
        }
        else {
    ...
    static std::unique_ptr<FailoverSuitePeer> create(CephContext* cct, entity_addr_t test_peer_addr,
...
SocketPolicy to_socket_policy(CephContext* cct, policy_t policy)
{
    ...
    default: lderr(cct) << "[CmdSrv] unexpected policy type" << dendl; ceph_abort();
    }
}
...
class FailoverTestPeer : public Dispatcher
{
    ...
    bool ms_dispatch(Message* m) override
    {
        ...
        if (cmd_conn == nullptr) {
            ...
            lderr(cct) << "[CmdSrv] got msg from CmdCli: conn(" << conn << ") != cmd_conn(" << cmd_conn << ")" << dendl;
            ceph_abort();
        }
        else {
            // good!
        ...
        default:
            lderr(cct) << "[CmdSrv] " << __func__ << " " << cmd_conn << " got unexpected msg from CmdCli: " << m
                       << dendl;
            ceph_abort();
        }
        m->put();
    ...
    void ms_handle_fast_accept(Connection* conn) override
    {
        if (cmd_conn == nullptr) {
            ...
            lderr(cct) << "[CmdSrv] accepted: conn(" << conn << ") != cmd_conn(" << cmd_conn << ")" << dendl;
            ceph_abort();
        }
        else {
    ...
    bool ms_handle_reset(Connection* conn) override
    {
        if (cmd_conn == conn) {
            ...
            ldout(cct, 0) << "[!CmdSrv] reset(invalid event): conn(" << conn << ") != cmd_conn(" << cmd_conn << ")"
                          << dendl;
        }
        return true;
    }
...
private:
    ...
    void handle_cmd(cmd_t cmd, MRef<MCommand> m_cmd)
    {
        ...
        case cmd_t::suite_start:
        {
            ...
            auto suite = FailoverSuitePeer::create(cct, test_peer_addr, policy, [this] { notify_recv_op(); });
        ...
        case cmd_t::suite_markdown:
            ...
            test_suite->markdown();
            return;
        default: lderr(cct) << "[CmdSrv] got unexpected command " << m_cmd << " from CmdCli" << dendl; ceph_abort();
        }
    }

    void init(entity_addr_t cmd_peer_addr)
    {
        cmd_msgr.reset(Messenger::create(cct, "async", entity_name_t::OSD(CMD_SRV_OSD), "CmdSrv", CMD_SRV_NONCE));
        dummy_auth.auth_registry.refresh_config();
...
public:
    FailoverTestPeer(CephContext* cct, entity_addr_t test_peer_addr, bool nonstop)
        : Dispatcher(cct)
        , dummy_auth(cct)
    ...
    static std::unique_ptr<FailoverTestPeer> create(CephContext* cct, entity_addr_t cmd_peer_addr,
                                                    entity_addr_t test_peer_addr, bool nonstop)
    {
...
int main(int argc, char** argv)
{
    ...
    desc.add_options()("help,h", "show help message")("addr",
                                                      po::value<std::string>()->default_value("v2:127.0.0.1:9012"),
                                                      "This is CmdSrv address, and TestPeer address is at port+=1")(
        "nonstop", po::value<bool>()->default_value(false), "Do not shutdown TestPeer when all tests are successful");
    po::variables_map vm;
    ...
    ldout(cct, 0) << "test configuration: cmd_peer_addr=" << cmd_peer_addr << ", test_peer_addr=" << test_peer_addr
                  << ", nonstop=" << nonstop << dendl;

    auto test_peer = FailoverTestPeer::create(cct.get(), cmd_peer_addr, test_peer_addr, nonstop);
    test_peer->wait();
}
