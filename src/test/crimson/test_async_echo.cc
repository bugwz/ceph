...
struct Server
{
    ...
    struct ServerDispatcher : Dispatcher
    {
        ...
        bool ms_can_fast_dispatch(const Message* m) const override { return m->get_type() == CEPH_MSG_PING; }
        ...
        bool ms_dispatch(Message*) override { ceph_abort(); }
        bool ms_handle_reset(Connection*) override { return true; }
        void ms_handle_remote_reset(Connection*) override {}
        bool ms_handle_refused(Connection*) override { return true; }
        void echo()
        {
            replied = false;
...
struct Client
{
    ...
    struct ClientDispatcher : Dispatcher
    {
        ...
        {
            auto conn = msgr->connect_to(peer.name.type(), entity_addrvec_t{peer.addr});
            replied = false;
            conn->send_message(new MPing);
            std::unique_lock lock{mutex};
            return on_reply.wait_for(lock, 500ms, [&] { return replied; });
        }
    } dispatcher;
    void ping(const entity_inst_t& peer) { dispatcher.ping(msgr.get(), peer); }
};
}   // namespace native_pingpong

static void ceph_echo(CephContext* cct, entity_addr_t addr, echo_role role, unsigned count)
{
    std::cout << "ceph/";
    ...
    if (role == echo_role::as_server) {
        ...
        client.msgr->start();
        auto conn = client.msgr->connect_to(entity.name.type(), entity_addrvec_t{entity.addr});
        for (unsigned i = 0; i < count; i++) {
