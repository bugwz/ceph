...
future<> test_bind_same(bool is_fixed_cpu)
{
    ...
    return ShardedServerSocket::create(is_fixed_cpu).then([is_fixed_cpu](auto pss1) {
        auto saddr = get_server_addr();
        return pss1->listen(saddr)
            .safe_then(
                [saddr, is_fixed_cpu] {
                    // try to bind the same address
                    return ShardedServerSocket::create(is_fixed_cpu).then([saddr](auto pss2) {
                        return pss2->listen(saddr)
                            .safe_then(
                                [] {
                                    logger().error("test_bind_same() should raise address_in_use");
                                    ceph_abort();
                                },
                                listen_ertr::all_same_way([](const std::error_code& e) {
                            ...
                            .then([pss2] { return pss2->shutdown_destroy(); });
                    });
                },
                listen_ertr::all_same_way([saddr](const std::error_code& e) {
                    logger().error("test_bind_same(): there is another instance running at {}", saddr);
                    ceph_abort();
                }))
            .then([pss1] { return pss1->shutdown_destroy(); })
            .handle_exception([](auto eptr) {
...
future<> test_accept(bool is_fixed_cpu)
{
    ...
    return ShardedServerSocket::create(is_fixed_cpu).then([](auto pss) {
        auto saddr = get_server_addr();
        return pss->listen(saddr)
            .safe_then(
                ...
                listen_ertr::all_same_way([saddr](const std::error_code& e) {
                    logger().error("test_accept(): there is another instance running at {}", saddr);
                    ceph_abort();
                }))
            .then([saddr] {
                return seastar::when_all(socket_connect(saddr).then([](auto socket) {
                           return socket->close().finally([cleanup = std::move(socket)] {});
                       }),
...
public:
    ...
    static future<> dispatch_sockets(bool is_fixed_cpu, FuncC&& cb_client, FuncS&& cb_server)
    {
        ...
        return seastar::smp::submit_to(
                   SERVER_CPU,
                   [psf, saddr, is_fixed_cpu] {
                       return ShardedServerSocket::create(is_fixed_cpu).then([psf, saddr](auto pss) {
                           psf->pss = pss;
                           return pss->listen(saddr).safe_then(
                               [] {}, listen_ertr::all_same_way([saddr](const std::error_code& e) {
                                   logger().error("dispatch_sockets(): there is another instance running at {}", saddr);
                                   ceph_abort();
                               }));
                       });
                   })
                ...
                return seastar::when_all_succeed(
                    ...
                    seastar::smp::submit_to(SERVER_CPU, [psf] {
                        return psf->pss->accept([psf](auto _socket, auto paddr) {
                            logger().info("dispatch_sockets(): accepted at shard {}", seastar::this_shard_id());
                            ...
                            return seastar::smp::submit_to(CLIENT_CPU, [psf] { psf->server_connected.set_value(); });
                        });
                    }));
            })
            .then_unpack([] { return seastar::now(); })
            .then([psf] { return psf->server_connected.get_future(); })
            .then([psf] {
                if (psf->pss) {
                    return seastar::smp::submit_to(SERVER_CPU, [psf] { return psf->pss->shutdown_destroy(); });
                }
                return seastar::now();
            })
            .then([psf, cb_client = std::move(cb_client), cb_server = std::move(cb_server)]() mutable {
                logger().debug("dispatch_sockets(): client/server socket are ready");
                return seastar::when_all_succeed(
                    seastar::smp::submit_to(CLIENT_CPU,
                                            [socket = psf->client_socket.get(), cb_client = std::move(cb_client)] {
                                                return cb_client(socket)
                                                    .then([socket] {
                                                    ...
                                                    .handle_exception([](auto eptr) {
                                                        logger().error("dispatch_sockets():"
                                                                       " cb_client() got unexpeted exception {}",
                                                                       eptr);
                                                        ceph_abort();
                                                    });
                                            }),
                    seastar::smp::submit_to(psf->server_socket_CPU,
                                            [socket = psf->server_socket.get(), cb_server = std::move(cb_server)] {
                                                return cb_server(socket)
                                                    ...
                                                    .handle_exception([](auto eptr) {
                                                        logger().error("dispatch_sockets():"
                                                                       " cb_server() got unexpeted exception {}",
                                                                       eptr);
                                                        ceph_abort();
                                                    });
                                            }));
...
class Connection
{
        ...
        return seastar::repeat([this, round, force_shut] {
            if (round != 0 && round <= write_count) {
                return seastar::futurize_invoke([this, force_shut] {
                    ...
                    .then([] { return seastar::make_ready_future<stop_t>(stop_t::yes); });
            }
            else {
                data[0] = write_count;
                bufferlist bl;
                bl.append(buffer::copy(reinterpret_cast<const char*>(&data), sizeof(data)));
                return socket->write(bl).then([this] { return socket->flush(); }).then([this] {
                    write_count += 1;
    ...
    {
        return dispatch_write().then([] { ceph_abort(); }).handle_exception_type([this](const std::system_error& e) {
        ...
        return seastar::repeat([this, round, force_shut] {
            if (round != 0 && round <= read_count) {
                ...
                return seastar::futurize_invoke([this] {
                           ...
                           if (read_count % 2) {
                               return socket->read(DATA_SIZE * sizeof(uint64_t)).then([this](ceph::bufferlist bl) {
                               ...
                               return socket->read_exactly(DATA_SIZE * sizeof(uint64_t)).then([this](auto bptr) {
                                   uint64_t read_data[DATA_SIZE];
    ...
    {
        return dispatch_read().then([] { ceph_abort(); }).handle_exception_type([this](const std::system_error& e) {
            if (e.code() != error::read_eof && e.code() != std::errc::connection_reset) {
...
public:
    static future<> dispatch_rw_bounded(Socket* socket, unsigned round, bool force_shut = false)
    {
        logger().debug("dispatch_rw_bounded(round={}, force_shut={})...", round, force_shut);
        return seastar::do_with(Connection{socket}, [round, force_shut](auto& conn) {
            ceph_assert(round != 0);
            return seastar::when_all_succeed(conn.dispatch_write(round, force_shut),
...
future<> test_read_write(bool is_fixed_cpu)
{
    ...
    return SocketFactory::dispatch_sockets(
        ...
        .then([] { logger().info("test_read_write() ok\n"); })
        .handle_exception([](auto eptr) {
...
future<> test_unexpected_down(bool is_fixed_cpu)
{
    ...
    return SocketFactory::dispatch_sockets(
        ...
        .then([] { logger().info("test_unexpected_down() ok\n"); })
        .handle_exception([](auto eptr) {
...
future<> test_shutdown_propagated(bool is_fixed_cpu)
{
    ...
    return SocketFactory::dispatch_sockets(
               ...
               [](auto ss) { return Connection::dispatch_rw_unbounded(ss); })
        .then([] { logger().info("test_shutdown_propagated() ok\n"); })
        .handle_exception([](auto eptr) {
...
future<> test_preemptive_down(bool is_fixed_cpu)
{
    ...
    return SocketFactory::dispatch_sockets(
               ...
               [](auto ss) { return Connection::dispatch_rw_unbounded(ss); })
        .then([] { logger().info("test_preemptive_down() ok\n"); })
        .handle_exception([](auto eptr) {
...
future<> do_test_with_type(bool is_fixed_cpu)
{
    return test_bind_same(is_fixed_cpu)
        .then([is_fixed_cpu] { return test_accept(is_fixed_cpu); })
        .then([is_fixed_cpu] { return test_read_write(is_fixed_cpu); })
        .then([is_fixed_cpu] { return test_unexpected_down(is_fixed_cpu); })
        .then([is_fixed_cpu] { return test_shutdown_propagated(is_fixed_cpu); })
        .then([is_fixed_cpu] { return test_preemptive_down(is_fixed_cpu); });
}
...
seastar::future<int> do_test(seastar::app_template& app)
{
    ...
    return crimson::common::sharded_conf()
        ...
        .then([] { return local_conf().set_val("ms_inject_internal_delays", "0"); })
        .then([] { return test_refused(); })
        .then([] { return do_test_with_type(true); })
        .then([] { return do_test_with_type(false); })
        .then([] {
