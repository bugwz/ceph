                    ...
                    return FixedCPUServerSocket::create().then([saddr](auto pss2) {
                        return pss2->listen(saddr)
                            .safe_then(
                                ...
                                FixedCPUServerSocket::listen_ertr::all_same_way([](const std::error_code& e) {
                            ...
                            .then([pss2] { return pss2->destroy(); });
                    });
                },
                FixedCPUServerSocket::listen_ertr::all_same_way([saddr](const std::error_code& e) {
                    logger.error("test_bind_same(): there is another instance running at {}", saddr);
                    ceph_abort();
                }))
            .then([pss1] { return pss1->destroy(); })
            .handle_exception([](auto eptr) {
...
public:
    ...
    template<typename FuncC, typename FuncS> static future<> dispatch_sockets(FuncC&& cb_client, FuncS&& cb_server)
    {
        ...
        return seastar::smp::submit_to(
            ...
            .then([psf, saddr] {
                return seastar::when_all_succeed(
                    seastar::smp::submit_to(0u,
                                            [psf, saddr] {
                                                return socket_connect(saddr).then(
                                                    [psf](auto socket) { psf->client_socket = std::move(socket); });
                                            }),
                    seastar::smp::submit_to(1u, [psf] {
                        return psf->pss->accept([psf](auto socket, auto paddr) {
                            ...
                            return seastar::smp::submit_to(0u, [psf] { psf->server_connected.set_value(); });
                        });
                    }));
            })
            ...
            .then([psf] {
                if (psf->pss) {
                    return seastar::smp::submit_to(1u, [psf] { return psf->pss->destroy(); });
                }
                return seastar::now();
            })
            .then([psf, cb_client = std::move(cb_client), cb_server = std::move(cb_server)]() mutable {
                ...
                return seastar::when_all_succeed(
                    ...
                    seastar::smp::submit_to(1u, [socket = psf->server_socket.get(), cb_server = std::move(cb_server)] {
    ...
    future<> dispatch_write(unsigned round = 0, bool force_shut = false)
    {
        ...
        return seastar::repeat([this, round, force_shut] {
            if (round != 0 && round <= write_count) {
                ...
                data[0] = write_count;
                return socket->write(seastar::net::packet(reinterpret_cast<const char*>(&data), sizeof(data)))
                    .then([this] { return socket->flush(); })
                    .then([this] {
                        write_count += 1;
            ...
            if (round != 0 && round <= read_count) {
                ...
                return seastar::futurize_invoke([this] {
                           ...
                           if (read_count % 2) {
                               ...
                               return socket->read_exactly(DATA_SIZE * sizeof(uint64_t)).then([this](auto buf) {
...
future<> test_read_write()
{
    ...
    return SocketFactory::dispatch_sockets([](auto cs) { return Connection::dispatch_rw_bounded(cs, 128); },
...
future<> test_preemptive_down()
{
    ...
    return SocketFactory::dispatch_sockets([](auto cs) { return Connection::dispatch_rw_unbounded(cs, true); },
                                           [](auto ss) { return Connection::dispatch_rw_unbounded(ss); })
...
int main(int argc, char** argv)
{
    ...
    return app.run(argc, argv, [] {
        return seastar::futurize_invoke([] { return test_refused(); })
            .then([] { return test_bind_same(); })
            .then([] { return test_accept(); })
            .then([] { return test_read_write(); })
            .then([] { return test_unexpected_down(); })
            .then([] { return test_shutdown_propagated(); })
            .then([] { return test_preemptive_down(); })
            .then([] {
