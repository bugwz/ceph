...
struct test_extent_desc_t
{
    ...
    bool operator==(const test_extent_desc_t& rhs) const { return (len == rhs.len && checksum == rhs.checksum); }
    bool operator!=(const test_extent_desc_t& rhs) const { return !(*this == rhs); }
};
...
inline std::ostream& operator<<(std::ostream& lhs, const test_extent_desc_t& rhs)
{
    return lhs << "test_extent_desc_t(len=" << rhs.len << ", checksum=" << rhs.checksum << ")";
}
...
struct TestBlock : crimson::os::seastore::LogicalCachedExtent
{
    ...
    CachedExtentRef duplicate_for_write(Transaction&) final { return CachedExtentRef(new TestBlock(*this)); };
    ...
    void set_contents(char c) { set_contents(c, 0, get_length()); }

    test_extent_desc_t get_desc() { return {get_length(), get_crc32c()}; }
...
struct TestBlockPhysical : crimson::os::seastore::CachedExtent
{
    ...
    CachedExtentRef duplicate_for_write(Transaction&) final { return CachedExtentRef(new TestBlockPhysical(*this)); };
...
{
    std::uniform_int_distribution<int8_t> contents_distribution =
        std::uniform_int_distribution<int8_t>(std::numeric_limits<int8_t>::min(), std::numeric_limits<int8_t>::max());

    std::uniform_int_distribution<uint16_t> offset_distribution =
        std::uniform_int_distribution<uint16_t>(0, TestBlock::SIZE - 1);

    std::uniform_int_distribution<uint16_t> length_distribution(uint16_t offset)
    {
        return std::uniform_int_distribution<uint16_t>(0, TestBlock::SIZE - offset - 1);
    }


    template<typename generator_t> void mutate(TestBlock& block, generator_t& gen)
    {
        auto offset = offset_distribution(gen);
        block.set_contents(contents_distribution(gen), offset, length_distribution(offset)(gen));
    }
};
