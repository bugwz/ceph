...
public:
    TestOnode(uint32_t ddr, uint32_t dmr)
        : Onode(ddr, dmr)
    {}
    const onode_layout_t& get_layout() const final { return layout; }
...
struct object_data_handler_test_t : public seastar_test_suite_t, TMTestState
{
    OnodeRef onode;
    ...
    void write(Transaction& t, objaddr_t offset, extent_len_t len, char fill)
    {
        ...
        memset(known_contents.c_str() + offset, fill, len);
        bufferlist bl;
        bl.append(bufferptr(known_contents, offset, len));
    ...
    {
        if (size > offset) {
            memset(known_contents.c_str() + offset, 0, size - offset);
            with_trans_intr(t, [&](auto& t) {
                return ObjectDataHandler(MAX_OBJECT_SIZE)
                    .truncate(ObjectDataHandler::context_t{*tm, t, *onode}, offset);
            }).unsafe_get0();
        }
        size = offset;
    }
    ...
    void read(Transaction& t, objaddr_t offset, extent_len_t len)
    {
        bufferlist bl = with_trans_intr(t, [&](auto& t) {
                            return ObjectDataHandler(MAX_OBJECT_SIZE)
                                .read(ObjectDataHandler::context_t{*tm, t, *onode}, offset, len);
                        }).unsafe_get0();
        bufferlist known;
        known.append(bufferptr(known_contents, offset, len));
    ...
    {
        auto t = create_mutate_transaction();
        auto ret = with_trans_intr(*t, [&](auto& t) { return tm->get_pins(t, offset, length); }).unsafe_get0();
        return ret;
    }

    seastar::future<> set_up_fut() final
    {
        onode = new TestOnode(DEFAULT_OBJECT_DATA_RESERVATION, DEFAULT_OBJECT_METADATA_RESERVATION);
...
TEST_P(object_data_handler_test_t, multiple_split)
{
    run_async([this] {
        ...
        size_t res[10] = {0, 4 << 10, 12 << 10, 20 << 10, 32 << 10, 36 << 10, 60 << 10, 96 << 10, 120 << 10, 124 << 10};
        auto base = pins.front()->get_key();
        int i = 0;
        for (auto& pin : pins) {
            EXPECT_EQ(pin->get_key() - base, res[i]);
...
INSTANTIATE_TEST_SUITE_P(object_data_handler_test, object_data_handler_test_t,
