...
{
    return lhs << "test_extent_record_t(" << rhs.desc << ", refcount=" << rhs.refcount << ")";
}
...
struct transaction_manager_test_t : public seastar_test_suite_t, TMTestState
{
    ...
    public:
        void insert(TestBlock& extent)
        {
            ...
            emplace(std::make_pair(extent.get_laddr(), test_extent_record_t{extent.get_desc(), 1}));
        }
    ...
    test_transaction_t create_transaction() { return {tm->create_transaction(), test_mappings}; }

    test_transaction_t create_weak_transaction() { return {tm->create_weak_transaction(), test_mappings}; }

    TestBlockRef alloc_extent(test_transaction_t& t, laddr_t hint, extent_len_t len, char contents)
    {
        auto extent = tm->alloc_extent<TestBlock>(*(t.t), hint, len).unsafe_get0();
    ...
    bool check_usage()
    {
        ...
        lba_manager
            ->scan_mapped_space(
                *t.t, [&tracker](auto offset, auto len) { tracker->allocate(offset.segment, offset.offset, len); })
            .unsafe_get0();
    ...
    TestBlockRef get_extent(test_transaction_t& t, laddr_t addr, extent_len_t len)
    {
        ...
        auto ret_list = tm->read_extents<TestBlock>(*t.t, addr, len).unsafe_get0();
