...
struct test_extent_record_t
{
    ...
    test_extent_record_t(const test_extent_desc_t& desc, unsigned refcount)
        : desc(desc)
        , refcount(refcount)
    {}

    void update(const test_extent_desc_t& to) { desc = to; }

    bool operator==(const test_extent_desc_t& rhs) const { return desc == rhs; }
    bool operator!=(const test_extent_desc_t& rhs) const { return desc != rhs; }
};

template<> struct fmt::formatter<test_extent_record_t> : fmt::formatter<std::string_view>
{
    template<typename FormatContext> auto format(const test_extent_record_t& r, FormatContext& ctx) const
    {
        return fmt::format_to(ctx.out(), "test_extent_record_t({}, refcount={})", r.desc, r.refcount);
    }
};

struct transaction_manager_test_t : public seastar_test_suite_t, TMTestState
{
    ...
    transaction_manager_test_t(std::size_t num_main_devices, std::size_t num_cold_devices)
        : TMTestState(num_main_devices, num_cold_devices)
        , gen(rd())
    {}
    ...
    char get_random_contents() { return static_cast<char>(std::uniform_int_distribution<>(0, 255)(gen)); }

    seastar::future<> set_up_fut() final { return tm_setup(); }

    seastar::future<> tear_down_fut() final { return tm_teardown(); }
    ...
    struct test_extents_t : std::map<laddr_t, test_extent_record_t>
    {
        ...
        struct delta_overlay_t
        {
            ...
            delta_overlay_t(const test_extents_t& extents, const delta_t& delta)
                : extents(extents)
                , delta(delta)
            {}
            ...
            class iterator
            {
                ...
                iterator(const delta_overlay_t& parent, test_extents_t::const_iterator biter,
                         delta_t::const_iterator oiter)
                    : parent(parent)
                    , biter(biter)
                    , oiter(oiter)
                ...
                laddr_t get_bkey() { return biter == parent.extents.end() ? L_ADDR_MAX : biter->first; }

                laddr_t get_okey() { return oiter == parent.delta.end() ? L_ADDR_MAX : oiter->first; }

                bool is_end() { return oiter == parent.delta.end() && biter == parent.extents.end(); }

                bool is_valid()
                {
                    return is_end() || ((get_okey() < get_bkey()) && (oiter->second)) || (get_okey() > get_bkey());
                }
                ...
                auto get_pair()
                {
                    ...
                    auto okey = get_okey();
                    auto bkey = get_bkey();
                    return (bkey < okey ? std::pair<laddr_t, test_extent_record_t>(*biter)
            ...
            public:
                ...
                bool operator==(const iterator& o) const { return o.biter == biter && o.oiter == oiter; }
    ...
    private:
        void check_available(laddr_t addr, extent_len_t len, const delta_t& delta) const
        {
        ...
        void check_hint(laddr_t hint, laddr_t addr, extent_len_t len, delta_t& delta) const
        {
            delta_overlay_t overlay(*this, delta);
        ...
        std::optional<test_extent_record_t>& populate_delta(laddr_t addr, delta_t& delta,
                                                            const test_extent_desc_t* desc) const
        {
            auto diter = delta.find(addr);
            if (diter != delta.end()) return diter->second;

            auto iter = find(addr);
            if (iter == end()) {
                assert(desc);
                auto ret = delta.emplace(std::make_pair(addr, test_extent_record_t{*desc, 0}));
                assert(ret.second);
    ...
    public:
        delta_overlay_t get_overlay(const delta_t& delta) const { return delta_overlay_t{*this, delta}; }

        void insert(TestBlock& extent, delta_t& delta) const
        {
            check_available(extent.get_laddr(), extent.get_length(), delta);
            delta[extent.get_laddr()] = test_extent_record_t{extent.get_desc(), 1};
        }
        ...
        void update(laddr_t addr, const test_extent_desc_t& desc, delta_t& delta) const
        {
        ...
        int inc_ref(laddr_t addr, delta_t& delta) const
        {
        ...
        int dec_ref(laddr_t addr, delta_t& delta) const
        {
            auto& rec = populate_delta(addr, delta, nullptr);
            assert(rec);
    ...
    test_transaction_t create_transaction() { return {create_mutate_transaction(), {}}; }

    test_transaction_t create_read_test_transaction() { return {create_read_transaction(), {}}; }

    test_transaction_t create_weak_test_transaction() { return {create_weak_transaction(), {}}; }

    TestBlockRef alloc_extent(test_transaction_t& t, laddr_t hint, extent_len_t len, char contents)
    {
    ...
    TestBlockRef alloc_extent(test_transaction_t& t, laddr_t hint, extent_len_t len)
    {
        return alloc_extent(t, hint, len, get_random_contents());
    }

    bool check_usage() { return epm->check_usage(); }
    ...
    TestBlockRef get_extent(test_transaction_t& t, laddr_t addr, extent_len_t len)
    {
    ...
    TestBlockRef try_get_extent(test_transaction_t& t, laddr_t addr)
    {
        ceph_assert(test_mappings.contains(addr, t.mapping_delta));
        ...
        auto ext =
            with_trans_intr(*(t.t), [&](auto& trans) { return tm->read_extent<TestBlock>(trans, addr); })
                .safe_then([](auto ext) -> ret { return ertr::make_ready_future<TestBlockRef>(ext); })
                .handle_error(
                    [](const crimson::ct_error::eagain& e) { return seastar::make_ready_future<TestBlockRef>(); },
                    crimson::ct_error::assert_all{"get_extent got invalid error"})
                .get0();
        if (ext) {
            EXPECT_EQ(addr, ext->get_laddr());
        }
    ...
    TestBlockRef try_get_extent(test_transaction_t& t, laddr_t addr, extent_len_t len)
    {
        ceph_assert(test_mappings.contains(addr, t.mapping_delta));
        ceph_assert(test_mappings.get(addr, t.mapping_delta).desc.len == len);
        ...
        using ret = ertr::future<TestBlockRef>;
        auto ext =
            with_trans_intr(*(t.t), [&](auto& trans) { return tm->read_extent<TestBlock>(trans, addr, len); })
                .safe_then([](auto ext) -> ret { return ertr::make_ready_future<TestBlockRef>(ext); })
                .handle_error(
                    [](const crimson::ct_error::eagain& e) { return seastar::make_ready_future<TestBlockRef>(); },
                    crimson::ct_error::assert_all{"get_extent got invalid error"})
    ...
    TestBlockRef try_read_pin(test_transaction_t& t, LBAMappingRef&& pin)
    {
        using ertr = with_trans_ertr<TransactionManager::base_iertr>;
        using ret = ertr::future<TestBlockRef>;
        auto addr = pin->get_key();
        auto ext =
            with_trans_intr(*(t.t), [&](auto& trans) { return tm->read_pin<TestBlock>(trans, std::move(pin)); })
                .safe_then([](auto ext) -> ret { return ertr::make_ready_future<TestBlockRef>(ext); })
                .handle_error(
                    [](const crimson::ct_error::eagain& e) { return seastar::make_ready_future<TestBlockRef>(); },
                    crimson::ct_error::assert_all{"read_pin got invalid error"})
                .get0();
        if (ext) {
            EXPECT_EQ(addr, ext->get_laddr());
        }
    ...
    TestBlockRef mutate_extent(test_transaction_t& t, TestBlockRef ref)
    {
        ceph_assert(test_mappings.contains(ref->get_laddr(), t.mapping_delta));
        ceph_assert(test_mappings.get(ref->get_laddr(), t.mapping_delta).desc.len == ref->get_length());
    ...
    TestBlockRef mutate_addr(test_transaction_t& t, laddr_t offset, size_t length)
    {
    ...
    LBAMappingRef get_pin(test_transaction_t& t, laddr_t offset)
    {
        ceph_assert(test_mappings.contains(offset, t.mapping_delta));
        auto pin = with_trans_intr(*(t.t), [&](auto& trans) { return tm->get_pin(trans, offset); }).unsafe_get0();
        EXPECT_EQ(offset, pin->get_key());
        return pin;
    }

    LBAMappingRef try_get_pin(test_transaction_t& t, laddr_t offset)
    {
        ceph_assert(test_mappings.contains(offset, t.mapping_delta));
        using ertr = with_trans_ertr<TransactionManager::get_pin_iertr>;
        using ret = ertr::future<LBAMappingRef>;
        auto pin =
            with_trans_intr(*(t.t), [&](auto& trans) { return tm->get_pin(trans, offset); })
                .safe_then([](auto pin) -> ret { return ertr::make_ready_future<LBAMappingRef>(std::move(pin)); })
                .handle_error(
                    [](const crimson::ct_error::eagain& e) { return seastar::make_ready_future<LBAMappingRef>(); },
                    crimson::ct_error::assert_all{"get_extent got invalid error"})
                .get0();
        if (pin) {
            EXPECT_EQ(offset, pin->get_key());
    ...
    void inc_ref(test_transaction_t& t, laddr_t offset)
    {
        ...
        auto refcnt = with_trans_intr(*(t.t), [&](auto& trans) { return tm->inc_ref(trans, offset); }).unsafe_get0();
    ...
    void dec_ref(test_transaction_t& t, laddr_t offset)
    {
        ...
        auto refcnt = with_trans_intr(*(t.t), [&](auto& trans) { return tm->dec_ref(trans, offset); }).unsafe_get0();
        auto check_refcnt = test_mappings.dec_ref(offset, t.mapping_delta);
        EXPECT_EQ(refcnt, check_refcnt);
        if (refcnt == 0) logger().debug("dec_ref: {} at refcount 0", offset);
    }
    ...
    void check_mappings(test_transaction_t& t)
    {
        ...
        with_trans_intr(*t.t, [this, &overlay](auto& t) {
            return lba_manager->scan_mappings(
                t, 0, L_ADDR_MAX, [iter = overlay.begin(), &overlay](auto l, auto p, auto len) mutable {
                    EXPECT_NE(iter, overlay.end());
                    logger().debug("check_mappings: scan {}", l);
                    EXPECT_EQ(l, iter->first);
                    ++iter;
                });
        }).unsafe_get0();
        (void)with_trans_intr(*t.t, [=, this](auto& t) { return lba_manager->check_child_trackers(t); }).unsafe_get0();
    }
    ...
    bool try_submit_transaction(test_transaction_t t)
    {
        ...
        bool success =
            submit_transaction_fut_with_seq(*t.t)
                .safe_then([&write_seq](auto seq) -> ret {
                    write_seq = seq;
                    return ertr::make_ready_future<bool>(true);
                })
                .handle_error(
                    [](const crimson::ct_error::eagain& e) { return seastar::make_ready_future<bool>(false); },
                    crimson::ct_error::assert_all{"try_submit_transaction hit invalid error"})
                .then([this](auto ret) { return epm->run_background_work_until_halt().then([ret] { return ret; }); })
                .get0();
    ...
    auto allocate_sequentially(const size_t size, const int num, bool run_clean = true)
    {
        return repeat_eagain([this, size, num] {
                   return seastar::do_with(create_transaction(), [this, size, num](auto& t) {
                                  ...
                                  *t.t,
                                  [&t, this, size, num](auto&) {
                                      return trans_intr::do_for_each(
                                                 ...
                                                 [&t, this, size](auto) {
                                                     return tm->alloc_extent<TestBlock>(*(t.t), L_ADDR_MIN, size)
                                                         .si_then([&t, this, size](auto extent) {
                                                             extent->set_contents(get_random_contents());
                                                             EXPECT_FALSE(test_mappings.contains(extent->get_laddr(),
                                                                                                 t.mapping_delta));
                                          ...
                                          .si_then([&t, this] { return tm->submit_transaction(*t.t); });
                                  })
                           .safe_then([&t, this] { test_mappings.consume(t.mapping_delta); });
                   });
               })
    ...
    void test_parallel_extent_read()
    {
        ...
        run_async([this] {
            ...
            seastar::do_with(create_read_test_transaction(), [this](auto& t) {
                return with_trans_intr(*(t.t), [this](auto& t) {
                    return trans_intr::parallel_for_each(
                        boost::make_counting_iterator(0lu), boost::make_counting_iterator(BLOCKS), [this, &t](auto i) {
                            return tm->read_extent<TestBlock>(t, i * BSIZE, BSIZE).si_then([](auto) {
                                return seastar::now();
                            });
                        });
                });
    ...
    void test_random_writes_concurrent()
    {
        ...
        run_async([this] {
            std::for_each(
                boost::make_counting_iterator(0u), boost::make_counting_iterator(WRITE_STREAMS), [&](auto idx) {
                    for (unsigned i = idx; i < BLOCKS; i += WRITE_STREAMS) {
                        while (true) {
                            auto t = create_transaction();
                            auto extent = alloc_extent(t, i * BSIZE, BSIZE);
                            ASSERT_EQ(i * BSIZE, extent->get_laddr());
                            if (try_submit_transaction(std::move(t))) break;
                        }
                    }
                });
            ...
            int writes = 0;
            unsigned failures = 0;
            seastar::parallel_for_each(boost::make_counting_iterator(0u),
                                       boost::make_counting_iterator(WRITE_STREAMS),
                                       [&](auto) {
                                           return seastar::async([&] {
                                               while (writes < 300) {
                                                   auto t = create_transaction();
                                                   auto ext = try_get_extent(t, get_random_laddr(BSIZE, TOTAL), BSIZE);
                                                   if (!ext) {
                                                       failures++;
                                                       continue;
                                                   }
            ...
            check();
            logger().info("random_writes_concurrent: {} suceeded, {} failed", writes, failures);
        });
    }
    ...
    void test_evict()
    {
        ...
        constexpr size_t device_size = segment_manager::DEFAULT_TEST_EPHEMERAL.size;
        constexpr size_t block_size = segment_manager::DEFAULT_TEST_EPHEMERAL.block_size;
        constexpr size_t segment_size = segment_manager::DEFAULT_TEST_EPHEMERAL.segment_size;
        ...
        run_async([this] {
            ...
            epm->background_process.eviction_state.init(stop_ratio, default_ratio, fast_ratio);
            ...
            auto run_until = [this](size_t size) -> seastar::future<> {
                return seastar::repeat([this, size] {
                    size_t current_size = epm->background_process.main_cleaner->get_stat().data_stored;
                    if (current_size >= size) {
                        return seastar::futurize_invoke([] { return seastar::stop_iteration::yes; });
                    }
                    else {
                        int num = (size - current_size) / block_size;
                        return seastar::do_for_each(boost::make_counting_iterator(0),
                                                    boost::make_counting_iterator(num),
                                                    [this](auto) {
            ...
            std::vector<extent_types_t> all_extent_types{extent_types_t::ROOT,
            ...
            auto test_gen = [&](const char* caller) {
                for (auto t : all_extent_types) {
                    for (auto gen : all_generations) {
                        auto epm_gen = epm->adjust_generation(get_extent_category(t), t, placement_hint_t::HOT, gen);
    ...
    LBAMappingRef remap_pin(test_transaction_t& t, LBAMappingRef&& opin, extent_len_t new_offset, extent_len_t new_len)
    {
        if (t.t->is_conflicted()) {
            return nullptr;
        }
        auto o_laddr = opin->get_key();
        auto pin = with_trans_intr(*(t.t),
                                   [&](auto& trans) {
                                       return tm
                                           ->remap_pin<TestBlock>(
                                               trans, std::move(opin), std::array{remap_entry(new_offset, new_len)})
                                           .si_then([](auto ret) { return std::move(ret[0]); });
                                   })
    ...
    using _overwrite_pin_ret = _overwrite_pin_iertr::future<std::tuple<LBAMappingRef, TestBlockRef, LBAMappingRef>>;
    _overwrite_pin_ret _overwrite_pin(Transaction& t, LBAMappingRef&& opin, extent_len_t new_offset,
                                      extent_len_t new_len, ceph::bufferlist& bl)
    {
        auto o_laddr = opin->get_key();
        auto o_len = opin->get_length();
        if (new_offset != 0 && o_len != new_offset + new_len) {
            return tm
                ->remap_pin<TestBlock, 2>(t,
                                          std::move(opin),
                                          std::array{remap_entry(0, new_offset),
                                                     remap_entry(new_offset + new_len, o_len - new_offset - new_len)})
                .si_then([this, new_offset, new_len, o_laddr, &t, &bl](auto ret) {
                    return tm->alloc_extent<TestBlock>(t, o_laddr + new_offset, new_len)
                        .si_then([this, ret = std::move(ret), new_len, new_offset, o_laddr, &t, &bl](auto ext) mutable {
                            ...
                            return tm->get_pin(t, o_laddr)
                                .si_then([this, &t, ext = std::move(ext), r_laddr](auto lpin) mutable {
                                    return tm->get_pin(t, r_laddr)
                                        .si_then([lpin = std::move(lpin), ext = std::move(ext)](auto rpin) mutable {
                                            return _overwrite_pin_iertr::make_ready_future<
                                                std::tuple<LBAMappingRef, TestBlockRef, LBAMappingRef>>(
                                                std::make_tuple(std::move(lpin), std::move(ext), std::move(rpin)));
                                        });
                                });
                        });
                });
            ...
            return tm
                ->remap_pin<TestBlock, 1>(
                    t, std::move(opin), std::array{remap_entry(new_offset + new_len, o_len - new_offset - new_len)})
                .si_then([this, new_offset, new_len, o_laddr, &t, &bl](auto ret) {
                    return tm->alloc_extent<TestBlock>(t, o_laddr + new_offset, new_len)
                        .si_then([this, ret = std::move(ret), new_offset, new_len, o_laddr, &t, &bl](auto ext) mutable {
                            ceph_assert(ret.size() == 1);
                            auto iter = bl.cbegin();
                            iter.copy(new_len, ext->get_bptr().c_str());
                            auto r_laddr = o_laddr + new_offset + new_len;
                            return tm->get_pin(t, r_laddr).si_then([ext = std::move(ext)](auto rpin) mutable {
                                return _overwrite_pin_iertr::make_ready_future<
                                    std::tuple<LBAMappingRef, TestBlockRef, LBAMappingRef>>(
                                    std::make_tuple(nullptr, std::move(ext), std::move(rpin)));
                            });
                        });
                });
        }
        else if (new_offset != 0 && o_len == new_offset + new_len) {
            return tm->remap_pin<TestBlock, 1>(t, std::move(opin), std::array{remap_entry(0, new_offset)})
                .si_then([this, new_offset, new_len, o_laddr, &t, &bl](auto ret) {
                    return tm->alloc_extent<TestBlock>(t, o_laddr + new_offset, new_len)
                        .si_then([this, ret = std::move(ret), new_len, o_laddr, &t, &bl](auto ext) mutable {
                            ceph_assert(ret.size() == 1);
                            auto iter = bl.cbegin();
                            iter.copy(new_len, ext->get_bptr().c_str());
                            return tm->get_pin(t, o_laddr).si_then([ext = std::move(ext)](auto lpin) mutable {
                                return _overwrite_pin_iertr::make_ready_future<
                                    std::tuple<LBAMappingRef, TestBlockRef, LBAMappingRef>>(
                                    std::make_tuple(std::move(lpin), std::move(ext), nullptr));
                            });
                        });
                });
        }
            ...
            return _overwrite_pin_iertr::make_ready_future<std::tuple<LBAMappingRef, TestBlockRef, LBAMappingRef>>(
                std::make_tuple(nullptr, nullptr, nullptr));
        }
    }

    using overwrite_pin_ret = std::tuple<LBAMappingRef, TestBlockRef, LBAMappingRef>;
    overwrite_pin_ret overwrite_pin(test_transaction_t& t, LBAMappingRef&& opin, extent_len_t new_offset,
                                    extent_len_t new_len, ceph::bufferlist& bl)
    {
        if (t.t->is_conflicted()) {
            return std::make_tuple<LBAMappingRef, TestBlockRef, LBAMappingRef>(nullptr, nullptr, nullptr);
        }
        auto o_laddr = opin->get_key();
        ...
        auto res =
            with_trans_intr(
                *(t.t), [&](auto& trans) { return _overwrite_pin(trans, std::move(opin), new_offset, new_len, bl); })
                .handle_error(crimson::ct_error::eagain::handle([] {
        ...
        if (rpin) {
            ...
            EXPECT_EQ(rpin->get_val(), o_paddr.add_offset(new_offset).add_offset(new_len));
    ...
    void test_overwrite_pin()
    {
        run_async([this] {
            ...
            }
            {
                ...
                auto [mlp1, mext1, mrp1] = overwrite_pin(t, std::move(mpin), 8 << 10, 8 << 10, mbl1);
                auto [mlp2, mext2, mrp2] = overwrite_pin(t, std::move(mrp1), 4 << 10, 16 << 10, mbl2);
                auto [mlpin3, me3, mrpin3] = overwrite_pin(t, std::move(mrp2), 4 << 10, 12 << 10, mbl3);
                ...
                auto [llp1, lext1, lrp1] = overwrite_pin(t, std::move(lpin), 0, 32 << 10, lbl1);
                EXPECT_FALSE(llp1);
                EXPECT_TRUE(lrp1);
                EXPECT_TRUE(lext1);

                rbl1.append(ceph::bufferptr(ceph::buffer::create(32 << 10, 0)));
                auto [rlp1, rext1, rrp1] = overwrite_pin(t, std::move(rpin), 32 << 10, 32 << 10, rbl1);
                EXPECT_TRUE(rlp1);
                EXPECT_TRUE(rext1);
                EXPECT_FALSE(rrp1);
    ...
    void test_remap_pin_concurrent()
    {
        run_async([this] {
            ...
            seastar::parallel_for_each(boost::make_counting_iterator(0u),
                ...
                .handle_exception([](std::exception_ptr e) { logger().info("{}", e); })
                .get0();
                ...
                [&](auto) {
                    return seastar::async([&] {
                        ...
                        while (!split_points.empty()) {
                            ...
                            if (((end_off << 10) == pin0->get_key() + pin0->get_length()) || (start_off == end_off)) {
                            ...
                            auto [lpin, ext, rpin] = overwrite_pin(t, last_rpin->duplicate(), new_off, new_len, bl);
                            if (!ext) {
                                conflicted++;
                                return;
                            }
...
struct tm_single_device_test_t : public transaction_manager_test_t
{

    tm_single_device_test_t()
        : transaction_manager_test_t(1, 0)
    {}
};

struct tm_multi_device_test_t : public transaction_manager_test_t
{

    tm_multi_device_test_t()
        : transaction_manager_test_t(3, 0)
    {}
};

struct tm_multi_tier_device_test_t : public transaction_manager_test_t
{

    tm_multi_tier_device_test_t()
        : transaction_manager_test_t(1, 2)
    {}
};
...
TEST_P(tm_single_device_test_t, basic)
{
    ...
    run_async([this] {
        ...
        {
            auto t = create_transaction();
            auto extent = alloc_extent(t, ADDR, SIZE, 'a');
...
TEST_P(tm_single_device_test_t, mutate)
{
    ...
    run_async([this] {
        ...
        {
            auto t = create_transaction();
            auto ext = get_extent(t, ADDR, SIZE);
            auto mut = mutate_extent(t, ext);
            check_mappings(t);
            check();
            submit_transaction(std::move(t));
...
TEST_P(tm_single_device_test_t, allocate_lba_conflict)
{
    ...
    run_async([this] {
        ...
        auto extent2 = alloc_extent(t2, ADDR2, SIZE, 'a');
    ...
    run_async([this] {
        {
            ...
            for (unsigned i = 0; i < 300; ++i) {
                auto extent = alloc_extent(t, laddr_t(i * SIZE), SIZE);
            }
            check_mappings(t);
            submit_transaction(std::move(t));
            check();
...
TEST_P(tm_single_device_test_t, create_remove_same_transaction)
{
    ...
    run_async([this] {
        constexpr laddr_t ADDR = 0xFF * SIZE;
        {
            ...
            extent = alloc_extent(t, ADDR, SIZE, 'a');
...
TEST_P(tm_single_device_test_t, cause_lba_split)
{
    ...
    run_async([this] {
        for (unsigned i = 0; i < 200; ++i) {
            auto t = create_transaction();
            auto extent = alloc_extent(t, i * SIZE, SIZE, (char)(i & 0xFF));
        ...
        for (unsigned i = 0; i < 4; ++i) {
            for (unsigned j = 0; j < 65; ++j) {
                ...
                for (unsigned k = 0; k < 2; ++k) {
                    auto ext = get_extent(t, get_random_laddr(BSIZE, TOTAL), BSIZE);
                    auto mut = mutate_extent(t, ext);
                    // pad out transaction
                    auto padding = alloc_extent(t, TOTAL + (k * PADDING_SIZE), PADDING_SIZE);
...
TEST_P(tm_single_device_test_t, find_hole_assert_trigger)
{
    ...
    int num = 40;
    run([&, this] {
        return seastar::parallel_for_each(boost::make_counting_iterator(0u),
                                          boost::make_counting_iterator(max),
                                          [&, this](auto idx) { return allocate_sequentially(BSIZE, num); });
    });
}
...
TEST_P(tm_single_device_test_t, remap_lazy_read)
{
    ...
    run_async([this, offset] {
        {
            auto t = create_transaction();
            auto extent = alloc_extent(t, offset, length, 'a');
        ...
        replay();
        {
            ...
            auto [lpin, ext, rpin] = overwrite_pin(t, std::move(pin), 4 << 10, 64 << 10, bl);
            check_mappings(t);
            submit_transaction(std::move(t));
            check();
        }
...
INSTANTIATE_TEST_SUITE_P(transaction_manager_test, tm_single_device_test_t,
                         ::testing::Values("segmented", "circularbounded"));

INSTANTIATE_TEST_SUITE_P(transaction_manager_test, tm_multi_device_test_t, ::testing::Values("segmented"));

INSTANTIATE_TEST_SUITE_P(transaction_manager_test, tm_multi_tier_device_test_t, ::testing::Values("segmented"));
