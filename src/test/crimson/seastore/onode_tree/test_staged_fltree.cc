...
#define INTR(fun, t) with_trans_intr(t, [&](auto& tr) { return fun(tr); })

#define INTR_R(fun, t, args...) with_trans_intr(t, [&](auto& tr) { return fun(tr, args); })

#define INTR_WITH_PARAM(fun, c, b, v) with_trans_intr(c.t, [=](auto& t) { return fun(c, L_ADDR_MIN, b, v); })
...
ghobject_t make_ghobj(shard_t shard, pool_t pool, crush_hash_t crush, std::string ns, std::string oid, snap_t snap,
                      gen_t gen)
{
...
std::pair<key_view_t, void*> build_key_view(const ghobject_t& hobj)
{
    ...
    size_t key_size = sizeof(shard_pool_crush_t) + sizeof(snap_gen_t) + ns_oid_view_t::estimate_size(key_hobj);
...
TEST_F(a_basic_test_t, 1_basic_sizes)
{
    logger().info("\n"
                  ...
                  sizeof(slot_1_t),
                  sizeof(slot_3_t),
                  sizeof(node_fields_0_t),
                  sizeof(node_fields_1_t),
                  sizeof(node_fields_2_t),
                  sizeof(internal_fields_3_t),
...
struct b_dummy_tree_test_t : public seastar_test_suite_t
{
    ...
    seastar::future<> set_up_fut() override final
    {
        ...
        tree.reset(new UnboundedBtree(NodeExtentManager::create_dummy(IS_DUMMY_SYNC)));
        return INTR(tree->mkfs, *ref_t).handle_error(crimson::ct_error::all_same_way([] {
...
TEST_F(b_dummy_tree_test_t, 3_random_insert_erase_leaf_node)
{
    run_async([this] {
        ...
        std::map<ghobject_t, std::tuple<test_item_t, UnboundedBtree::Cursor>> insert_history;

        auto f_validate_insert_new = [this, &insert_history](const ghobject_t& key, const test_item_t& value) {
            auto conf = UnboundedBtree::tree_value_config_t{value.get_payload_size()};
            auto [cursor, success] = INTR_R(tree->insert, *ref_t, key, conf).unsafe_get0();
        ...
        auto f_insert_erase_insert = [&f_validate_insert_new, &f_validate_erase](const ghobject_t& key,
        ...
        // insert the same key1 with a different value
        {
            ...
            auto [cursor1_dup, ret1_dup] = INTR_R(tree->insert, *ref_t, key1, conf).unsafe_get0();
        ...
        std::vector<std::pair<ghobject_t, test_item_t>> kvs{{make_ghobj(2, 2, 2, "ns2", "oid2", 2, 2), values.pick()},
        ...
        std::for_each(
            kvs.begin(), kvs.end(), [&f_insert_erase_insert](auto& kv) { f_insert_erase_insert(kv.first, kv.second); });
        ...
        for (auto& [k, val] : insert_history) {
            ...
            auto cursor = with_trans_intr(*ref_t, [this, &k = k](auto& tr) { return tree->find(tr, k); }).unsafe_get0();
        ...
        // validate range query
        {
            ...
            std::sort(kvs.begin(), kvs.end(), [](auto& l, auto& r) { return l.first < r.first; });
            auto cursor = INTR(tree->begin, *ref_t).unsafe_get0();
            for (auto& [k, v] : kvs) {
        ...
        for (auto& [k, v] : kvs) {
            auto e_size =
                with_trans_intr(*ref_t, [this, &k = k](auto& tr) { return tree->erase(tr, k); }).unsafe_get0();
            ASSERT_EQ(e_size, 1);
        }
        auto cursor = INTR(tree->begin, *ref_t).unsafe_get0();
        ASSERT_TRUE(cursor.is_end());
...
static std::set<ghobject_t> build_key_set(std::pair<unsigned, unsigned> range_2, std::pair<unsigned, unsigned> range_1,
                                          std::pair<unsigned, unsigned> range_0, std::string padding = "",
...
public:
    ...
    seastar::future<> build_tree(std::pair<unsigned, unsigned> range_2, std::pair<unsigned, unsigned> range_1,
                                 std::pair<unsigned, unsigned> range_0, size_t value_size)
    {
    ...
    seastar::future<> build_tree(const std::vector<ghobject_t>& keys, const std::vector<test_item_t>& values)
    {
    ...
    seastar::future<> split_merge(const ghobject_t& key, const test_item_t& value, const split_expectation_t& expected,
                                  std::optional<ghobject_t> next_key)
    {
        return seastar::async([this, key, value, expected, next_key] {
            ...
            auto conf = UnboundedBtree::tree_value_config_t{value.get_payload_size()};
            auto [cursor, success] = INTR_R(tree_clone.insert, t_clone, key, conf).unsafe_get0();
            ...
            auto nxt_cursor =
                with_trans_intr(t_clone, [&cursor = cursor](auto& tr) { return cursor.erase<true>(tr); }).unsafe_get0();
            ...
            if (next_key.has_value()) {
                ...
                validate_cursor_from_item(*next_key, std::get<0>(found->second), nxt_cursor);
            }
            else {
    ...
    test_item_t create_value(size_t size) { return values.create(size); }
...
private:
    seastar::future<> insert_tree(const ghobject_t& key, const test_item_t& value)
    {
        return seastar::async([this, &key, &value] {
            auto conf = UnboundedBtree::tree_value_config_t{value.get_payload_size()};
            auto [cursor, success] = INTR_R(tree.insert, t, key, conf).unsafe_get0();
            initialize_cursor_from_item(t, key, value, cursor, success);
            insert_history.emplace(key, std::make_tuple(value, cursor));
        });
    }
    ...
    run_async([] {
        {
            ...
            test.split_merge(make_ghobj(1, 1, 1, "ns3", "oid3", 3, 3),
                             value,
                             {2u, 2u, true, InsertType::BEGIN},
                             {make_ghobj(2, 2, 2, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(2, 2, 2, "ns1", "oid1", 3, 3),
                             value,
                             {2u, 1u, true, InsertType::BEGIN},
                             {make_ghobj(2, 2, 2, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(2, 2, 2, "ns2", "oid2", 1, 1),
                             value,
            ...
            test.split_merge(make_ghobj(2, 2, 2, "ns4", "oid4", 5, 5),
                             value,
                             {2u, 0u, true, InsertType::LAST},
                             {make_ghobj(3, 3, 3, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(2, 2, 2, "ns5", "oid5", 3, 3),
                             value,
                             {2u, 1u, true, InsertType::LAST},
                             {make_ghobj(3, 3, 3, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(2, 3, 3, "ns3", "oid3", 3, 3),
                             value,
                             {2u, 2u, true, InsertType::LAST},
                             {make_ghobj(3, 3, 3, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(3, 3, 3, "ns1", "oid1", 3, 3),
                             value,
                             {2u, 1u, true, InsertType::LAST},
                             {make_ghobj(3, 3, 3, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(3, 3, 3, "ns2", "oid2", 1, 1),
                             value,
                             {2u, 0u, true, InsertType::LAST},
                             {make_ghobj(3, 3, 3, "ns2", "oid2", 2, 2)})
                .get0();
            ...
            test.split_merge(make_ghobj(3, 3, 3, "ns4", "oid4", 5, 5),
                             value0,
                             {2u, 0u, false, InsertType::BEGIN},
                             {make_ghobj(4, 4, 4, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(3, 3, 3, "ns5", "oid5", 3, 3),
                             value0,
                             {2u, 1u, false, InsertType::BEGIN},
                             {make_ghobj(4, 4, 4, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(3, 4, 4, "ns3", "oid3", 3, 3),
                             value0,
                             {2u, 2u, false, InsertType::BEGIN},
                             {make_ghobj(4, 4, 4, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(4, 4, 4, "ns1", "oid1", 3, 3),
                             value0,
                             {2u, 1u, false, InsertType::BEGIN},
                             {make_ghobj(4, 4, 4, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(4, 4, 4, "ns2", "oid2", 1, 1),
                             value0,
                             {2u, 0u, false, InsertType::BEGIN},
                             {make_ghobj(4, 4, 4, "ns2", "oid2", 2, 2)})
                .get0();
            ...
            test.split_merge(
                    make_ghobj(4, 4, 4, "ns4", "oid4", 5, 5), value0, {2u, 0u, false, InsertType::LAST}, std::nullopt)
                .get0();
            test.split_merge(
                    make_ghobj(4, 4, 4, "ns5", "oid5", 3, 3), value0, {2u, 1u, false, InsertType::LAST}, std::nullopt)
                .get0();
            test.split_merge(
                    make_ghobj(5, 5, 5, "ns3", "oid3", 3, 3), value0, {2u, 2u, false, InsertType::LAST}, std::nullopt)
                .get0();
            ...
            test.split_merge(make_ghobj(2, 2, 2, "ns5", "oid5", 3, 3),
                             ...
                             {make_ghobj(3, 3, 3, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(2, 2, 3, "ns3", "oid3", 3, 3),
                             value1,
            ...
            test.split_merge(make_ghobj(3, 3, 3, "ns2", "oid2", 5, 5),
                             value1,
                             {1u, 0u, true, InsertType::LAST},
                             {make_ghobj(3, 3, 3, "ns3", "oid3", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(3, 3, 3, "ns2", "oid3", 3, 3),
                             value1,
                             {1u, 1u, true, InsertType::LAST},
                             {make_ghobj(3, 3, 3, "ns3", "oid3", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(3, 3, 3, "ns3", "oid3", 1, 1),
                             value1,
                             {1u, 0u, true, InsertType::LAST},
                             {make_ghobj(3, 3, 3, "ns3", "oid3", 2, 2)})
                .get0();
            ...
            test.split_merge(make_ghobj(3, 3, 3, "ns3", "oid3", 5, 5),
                             value2,
                             {1u, 0u, false, InsertType::BEGIN},
                             {make_ghobj(3, 3, 3, "ns4", "oid4", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(3, 3, 3, "ns3", "oid4", 3, 3),
                             value2,
                             {1u, 1u, false, InsertType::BEGIN},
                             {make_ghobj(3, 3, 3, "ns4", "oid4", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(3, 3, 3, "ns4", "oid4", 1, 1),
                             value2,
                             {1u, 0u, false, InsertType::BEGIN},
                             {make_ghobj(3, 3, 3, "ns4", "oid4", 2, 2)})
                .get0();
            ...
            test.split_merge(make_ghobj(3, 3, 3, "ns5", "oid5", 3, 3),
                             ...
                             {make_ghobj(4, 4, 4, "ns2", "oid2", 2, 2)})
                .get0();
            test.split_merge(make_ghobj(3, 3, 4, "ns3", "oid3", 3, 3),
                             value2,
            ...
            test.split_merge(make_ghobj(3, 3, 3, "ns4", "oid4", 2, 3),
                             value3,
            ...
            test.split_merge(make_ghobj(3, 3, 3, "ns2", "oid2", 3, 4),
                             value4,
                             {0u, 0u, true, InsertType::LAST},
                             {make_ghobj(3, 3, 3, "ns2", "oid2", 4, 4)})
                .get0();
        }
        ...
        {
            TestTree test;
            std::vector<ghobject_t> keys = {make_ghobj(2, 2, 2, "ns3", "oid3", 3, 3),
                                            make_ghobj(3, 3, 3, "ns3", "oid3", 3, 3)};
            std::vector<test_item_t> values = {test.create_value(1360), test.create_value(1632)};
            test.build_tree(keys, values).get0();
            auto value = test.create_value(1640);
            logger().info("\n---------------------------------------------"
                          "\nsplit at [END, END, END]; insert to right at stage 0, 1, 2\n");
            test.split_merge(
                    make_ghobj(3, 3, 3, "ns3", "oid3", 4, 4), value, {0u, 0u, false, InsertType::BEGIN}, std::nullopt)
                .get0();
            EXPECT_TRUE(last_split.match_split_pos({1, {0, {1}}}));
            test.split_merge(
                    make_ghobj(3, 3, 3, "ns4", "oid4", 3, 3), value, {1u, 1u, false, InsertType::BEGIN}, std::nullopt)
                .get0();
            EXPECT_TRUE(last_split.match_split_pos({1, {1, {0}}}));
            test.split_merge(
                    make_ghobj(4, 4, 4, "ns3", "oid3", 3, 3), value, {2u, 2u, false, InsertType::BEGIN}, std::nullopt)
                .get0();
...
class DummyChildPool
{
    ...
    public:
        ...
        DummyChildImpl(const std::set<ghobject_t>& keys, bool is_level_tail, laddr_t laddr)
            : keys{keys}
            , _is_level_tail{is_level_tail}
            , _laddr{laddr}
        {
            std::tie(key_view, p_mem_key_view) = build_key_view(*keys.crbegin());
            build_name();
        }
        ~DummyChildImpl() override { std::free(p_mem_key_view); }
    ...
    protected:
        ...
        const char* read() const override { ceph_abort("impossible path"); }
        extent_len_t get_node_size() const override { ceph_abort("impossible path"); }
        nextent_state_t get_extent_state() const override { ceph_abort("impossible path"); }
        level_t level() const override { return 0u; }
        void prepare_mutate(context_t) override { ceph_abort("impossible path"); }
        void validate_non_empty() const override { ceph_abort("impossible path"); }
        bool is_keys_empty() const override { ceph_abort("impossible path"); }
        bool has_single_value() const override { ceph_abort("impossible path"); }
        node_offset_t free_size() const override { ceph_abort("impossible path"); }
        extent_len_t total_size() const override { ceph_abort("impossible path"); }
        bool is_size_underflow() const override { ceph_abort("impossible path"); }
        std::tuple<match_stage_t, search_position_t> erase(const search_position_t&) override
        {
            ceph_abort("impossible path");
        }
        std::tuple<match_stage_t, std::size_t> evaluate_merge(NodeImpl&) override { ceph_abort("impossible path"); }
        search_position_t merge(NodeExtentMutable&, NodeImpl&, match_stage_t, extent_len_t) override
        {
            ceph_abort("impossible path");
        }
        eagain_ifuture<NodeExtentMutable> rebuild_extent(context_t) override { ceph_abort("impossible path"); }
        node_stats_t get_stats() const override { ceph_abort("impossible path"); }
        std::ostream& dump(std::ostream&) const override { ceph_abort("impossible path"); }
        std::ostream& dump_brief(std::ostream&) const override { ceph_abort("impossible path"); }
        void validate_layout() const override { ceph_abort("impossible path"); }
        void test_copy_to(NodeExtentMutable&) const override { ceph_abort("impossible path"); }
        void test_set_tail(NodeExtentMutable&) override { ceph_abort("impossible path"); }
    ...
    private:
        void build_name()
        {
            ...
            sos << "DummyNode"
                << "@0x" << std::hex << laddr() << std::dec << "Lv" << (unsigned)level() << (is_level_tail() ? "$" : "")
                << "(" << key_view << ")";
            name = sos.str();
        }
    ...
    public:
        ...
        eagain_ifuture<> populate_split(context_t c, std::set<Ref<DummyChild>>& splitable_nodes)
        {
            ...
            return apply_split_to_parent(c, std::move(this_ref), std::move(right_child), false);
        }

        eagain_ifuture<> insert_and_split(context_t c, const ghobject_t& insert_key,
        ...
        static Ref<DummyChild> create(const std::set<ghobject_t>& keys, bool is_level_tail, laddr_t addr,
                                      DummyChildPool& pool)
        {
        ...
        static Ref<DummyChild> create_new(const std::set<ghobject_t>& keys, bool is_level_tail, DummyChildPool& pool)
        {
            static laddr_t seed = 0;
            return create(keys, is_level_tail, seed++, pool);
        }

        static eagain_ifuture<Ref<DummyChild>> create_initial(context_t c, const std::set<ghobject_t>& keys,
                                                              DummyChildPool& pool, RootNodeTracker& root_tracker)
        {
            ...
            return c.nm.get_super(c.t, root_tracker)
                ...
                .si_then([c, initial](auto super) {
                    ...
                    return initial->upgrade_root(c, L_ADDR_MIN).si_then([initial] { return initial; });
                });
        }

    protected:
        eagain_ifuture<> test_clone_non_root(context_t, Ref<InternalNode> new_parent) const override
        {
            ceph_assert(!is_root());
            auto p_pool_clone = pool.pool_clone_in_progress;
            ceph_assert(p_pool_clone != nullptr);
            auto clone = create(impl->get_keys(), impl->is_level_tail(), impl->laddr(), *p_pool_clone);
            clone->as_child(parent_info().position, new_parent);
            return eagain_iertr::now();
        }
        eagain_ifuture<Ref<tree_cursor_t>> lookup_smallest(context_t) override { ceph_abort("impossible path"); }
        eagain_ifuture<Ref<tree_cursor_t>> lookup_largest(context_t) override { ceph_abort("impossible path"); }
        eagain_ifuture<> test_clone_root(context_t, RootNodeTracker&) const override { ceph_abort("impossible path"); }
        eagain_ifuture<search_result_t> lower_bound_tracked(context_t, const key_hobj_t&, MatchHistory&) override
        {
            ceph_abort("impossible path");
        }
        eagain_ifuture<> do_get_tree_stats(context_t, tree_stats_t&) override { ceph_abort("impossible path"); }
        bool is_tracking() const override { return false; }
        void track_merge(Ref<Node>, match_stage_t, search_position_t&) override { ceph_abort("impossible path"); }
    ...
    private:
        ...
        static eagain_ifuture<> do_merge(context_t c, Ref<DummyChild>&& left, Ref<DummyChild>&& right, bool stole_key)
        {
...
public:
        ...
        return with_trans_intr(get_context().t, [this, &keys](auto& tr) {
            return DummyChild::create_initial(get_context(), keys, *this, *p_btree->root_tracker)
                .si_then([this](auto initial_child) {
                    ...
                    return trans_intr::repeat([this]() -> eagain_ifuture<seastar::stop_iteration> {
                        ...
                        return child->populate_split(get_context(), splitable_nodes).si_then([] {
                ...
                .si_then([](auto height) { ceph_assert(height == 2); });
        });
    }

    seastar::future<> split_merge(ghobject_t key, search_position_t pos, const split_expectation_t& expected)
    {
        return seastar::async([this, key, pos, expected] {
            ...
            with_trans_intr(pool_clone.get_context().t, [&](auto& t) {
                return node_to_split->insert_and_split(pool_clone.get_context(), key, pool_clone.splitable_nodes);
            }).unsafe_get0();
            {
                std::ostringstream oss;
            ...
            with_trans_intr(pool_clone.get_context().t, [&](auto& t) {
                return node_to_split->merge(pool_clone.get_context(), std::move(node_to_split));
            }).unsafe_get0();
    ...
    seastar::future<> fix_index(ghobject_t new_key, search_position_t pos, bool expect_split)
    {
        return seastar::async([this, new_key, pos, expect_split] {
            ...
            // fix back
            logger().info("\n\nFIX pos({}) from {} back to {}:", pos, node_to_fix->get_name(), key_hobj_t(old_key));
            with_trans_intr(pool_clone.get_context().t, [&](auto& t) {
...
private:
    void clone_to(DummyChildPool& pool_clone)
    {
        ...
        INTR_R(pool_clone.p_btree->test_clone_from, pt, tr, *p_btree).unsafe_get0();
    ...
    Ref<DummyChild> get_node_by_pos(const search_position_t& pos) const
    {
        auto iter = std::find_if(
            tracked_children.begin(), tracked_children.end(), [&pos](auto& child) { return child->match_pos(pos); });
...
TEST_F(c_dummy_test_t, 5_split_merge_internal_node)
{
    run_async([] {
        DummyChildPool pool;
        {
            ...
            logger().info("\n---------------------------------------------"
                          "\nsplit at stage 2; insert to right front at stage 0, 1, 2, 1, 0\n");
            pool.split_merge(make_ghobj(3, 3, 3, "ns4", "oid4" + padding, 5, 5),
                             {2, {0, {0}}},
                             {2u, 0u, false, InsertType::BEGIN})
                .get();
            pool.split_merge(
                    make_ghobj(3, 3, 3, "ns5", "oid5", 3, 3), {2, {0, {0}}}, {2u, 1u, false, InsertType::BEGIN})
                .get();
            pool.split_merge(
                    make_ghobj(3, 4, 4, "ns3", "oid3", 3, 3), {2, {0, {0}}}, {2u, 2u, false, InsertType::BEGIN})
                .get();
            pool.split_merge(
                    make_ghobj(4, 4, 4, "ns1", "oid1", 3, 3), {2, {0, {0}}}, {2u, 1u, false, InsertType::BEGIN})
                .get();
            pool.split_merge(make_ghobj(4, 4, 4, "ns2", "oid2" + padding, 1, 1),
                             {2, {0, {0}}},
                             {2u, 0u, false, InsertType::BEGIN})
                .get();
            ...
            pool.split_merge(
                    make_ghobj(4, 4, 4, "ns4", "oid4" + padding, 5, 5), {3, {0, {0}}}, {2u, 0u, false, InsertType::MID})
                .get();
            pool.split_merge(make_ghobj(4, 4, 4, "ns5", "oid5", 3, 3), {3, {0, {0}}}, {2u, 1u, false, InsertType::MID})
                .get();
            pool.split_merge(make_ghobj(4, 4, 5, "ns3", "oid3", 3, 3), {3, {0, {0}}}, {2u, 2u, false, InsertType::MID})
                .get();
            pool.split_merge(make_ghobj(5, 5, 5, "ns1", "oid1", 3, 3), {3, {0, {0}}}, {2u, 1u, false, InsertType::MID})
                .get();
            pool.split_merge(
                    make_ghobj(5, 5, 5, "ns2", "oid2" + padding, 1, 1), {3, {0, {0}}}, {2u, 0u, false, InsertType::MID})
                .get();

            logger().info("\n---------------------------------------------"
                          "\nsplit at stage 2; insert to right back at stage 0, 1, 2\n");
            pool.split_merge(make_ghobj(5, 5, 5, "ns4", "oid4" + padding_e, 5, 5),
                             search_position_t::end(),
                             {2u, 0u, false, InsertType::LAST})
                .get();
            pool.split_merge(make_ghobj(5, 5, 5, "ns5", "oid5", 3, 3),
                             search_position_t::end(),
                             {2u, 1u, false, InsertType::LAST})
                .get();
            pool.split_merge(make_ghobj(6, 6, 6, "ns3", "oid3", 3, 3),
                             search_position_t::end(),
            ...
            pool.split_merge(make_ghobj(1, 1, 1, "ns3", "oid3", 3, 3), {0, {0, {0}}}, {0u, 2u, true, InsertType::BEGIN})
                .get();
            pool.split_merge(make_ghobj(2, 2, 2, "ns1", "oid1", 3, 3), {0, {0, {0}}}, {0u, 1u, true, InsertType::BEGIN})
                .get();
            pool.split_merge(make_ghobj(2, 2, 2, "ns2", "oid2" + padding_s, 1, 1),
                             {0, {0, {0}}},
                             {0u, 0u, true, InsertType::BEGIN})
                .get();

            logger().info("\n---------------------------------------------"
                          "\nsplit at stage 0; insert to left middle at stage 0, 1, 2, 1, 0\n");
            pool.split_merge(
                    make_ghobj(2, 2, 2, "ns4", "oid4" + padding, 5, 5), {1, {0, {0}}}, {0u, 0u, true, InsertType::MID})
                .get();
            pool.split_merge(make_ghobj(2, 2, 2, "ns5", "oid5", 3, 3), {1, {0, {0}}}, {0u, 1u, true, InsertType::MID})
                .get();
            pool.split_merge(make_ghobj(2, 2, 3, "ns3", "oid3" + std::string(80, '_'), 3, 3),
                             {1, {0, {0}}},
                             {0u, 2u, true, InsertType::MID})
                .get();
            pool.split_merge(make_ghobj(3, 3, 3, "ns1", "oid1", 3, 3), {1, {0, {0}}}, {0u, 1u, true, InsertType::MID})
                .get();
            pool.split_merge(
                    make_ghobj(3, 3, 3, "ns2", "oid2" + padding, 1, 1), {1, {0, {0}}}, {0u, 0u, true, InsertType::MID})
                .get();

            logger().info("\n---------------------------------------------"
                          "\nsplit at stage 0; insert to left back at stage 0\n");
            pool.split_merge(
                    make_ghobj(3, 3, 3, "ns4", "oid4" + padding, 3, 4), {1, {2, {2}}}, {0u, 0u, true, InsertType::LAST})
                .get();
        }

        {
            ...
            pool.split_merge(
                    make_ghobj(3, 3, 3, "ns4", "oid4" + padding, 5, 5), {2, {0, {0}}}, {2u, 0u, true, InsertType::LAST})
                .get();
            pool.split_merge(make_ghobj(3, 3, 3, "ns5", "oid5", 3, 3), {2, {0, {0}}}, {2u, 1u, true, InsertType::LAST})
                .get();
            pool.split_merge(make_ghobj(3, 4, 4, "n", "o", 3, 3), {2, {0, {0}}}, {2u, 2u, true, InsertType::LAST})
                .get();
            pool.split_merge(make_ghobj(4, 4, 4, "n", "o", 3, 3), {2, {0, {0}}}, {2u, 1u, true, InsertType::LAST})
                .get();

            logger().info("\n---------------------------------------------"
                          "\nsplit at stage 2; insert to left middle at stage 2\n");
            pool.split_merge(make_ghobj(2, 3, 3, "n", "o", 3, 3), {1, {0, {0}}}, {2u, 2u, true, InsertType::MID}).get();
        }

        {
            ...
            logger().info("\n---------------------------------------------"
                          "\nsplit at stage 2; insert to left back at stage (0, 1, 2, 1,) 0\n");
            pool.split_merge(make_ghobj(4, 4, 4, "n", "o", 2, 2), {2, {0, {0}}}, {2u, 0u, true, InsertType::LAST})
                .get();
        }

        {
            ...
            logger().info("\n---------------------------------------------"
                          "\nsplit at stage 1; insert to right front at stage 0, 1, 0\n");
            pool.split_merge(make_ghobj(3, 3, 3, "ns2", "oid2" + padding, 5, 5),
                             {1, {1, {0}}},
                             {1u, 0u, false, InsertType::BEGIN})
                .get();
            pool.split_merge(
                    make_ghobj(3, 3, 3, "ns2", "oid3", 3, 3), {1, {1, {0}}}, {1u, 1u, false, InsertType::BEGIN})
                .get();
            pool.split_merge(make_ghobj(3, 3, 3, "ns3", "oid3" + padding, 1, 1),
                             {1, {1, {0}}},
                             {1u, 0u, false, InsertType::BEGIN})
                .get();
        }

        {
            ...
            pool.split_merge(
                    make_ghobj(3, 3, 3, "ns2", "oid2" + padding, 5, 5), {1, {1, {0}}}, {1u, 0u, true, InsertType::LAST})
                .get();
            pool.split_merge(make_ghobj(3, 3, 3, "ns2", "oid3", 3, 3), {1, {1, {0}}}, {1u, 1u, true, InsertType::LAST})
                .get();

            logger().info("\n---------------------------------------------"
                          "\nfix end index from stage 0 to 0, 1, 2\n");
            auto padding1 = std::string(400, '_');
            pool.fix_index(make_ghobj(4, 4, 4, "ns4", "oid4" + padding, 5, 5), {2, {2, {2}}}, false).get();
            pool.fix_index(make_ghobj(4, 4, 4, "ns5", "oid5" + padding1, 3, 3), {2, {2, {2}}}, true).get();
            pool.fix_index(make_ghobj(5, 5, 5, "ns3", "oid3" + padding1, 3, 3), {2, {2, {2}}}, true).get();
        }

        {
            ...
            logger().info("\n---------------------------------------------"
                          "\nsplit at stage 1; insert to left back at stage (0, 1,) 0\n");
            pool.split_merge(make_ghobj(3, 3, 3, "ns2", "oid3", 2, 2), {1, {1, {0}}}, {1u, 0u, true, InsertType::LAST})
                .get();
        }

        {
            ...
            logger().info("\n---------------------------------------------"
                          "\nsplit at stage 0; insert to right front at stage 0\n");
            pool.split_merge(make_ghobj(3, 3, 3, "ns3", "oid3" + padding, 2, 3),
                             {1, {1, {1}}},
                             {0u, 0u, false, InsertType::BEGIN})
                .get();
            ...
            auto padding1 = std::string(400, '_');
            pool.fix_index(make_ghobj(4, 4, 4, "ns4", "oid4" + padding, 5, 5), {3, {0, {0}}}, false).get();
            pool.fix_index(make_ghobj(4, 4, 4, "ns5", "oid5" + padding1, 3, 3), {3, {0, {0}}}, true).get();
            pool.fix_index(make_ghobj(5, 5, 5, "ns4", "oid4" + padding1, 3, 3), {3, {0, {0}}}, true).get();
        }

        {
            ...
            auto padding1 = std::string(400, '_');
            pool.fix_index(make_ghobj(4, 4, 4, "ns4", "oid4" + padding, 5, 5), {2, {3, {0}}}, false).get();
            pool.fix_index(make_ghobj(4, 4, 4, "ns6", "oid6" + padding1, 3, 3), {2, {3, {0}}}, true).get();
            pool.fix_index(make_ghobj(5, 5, 5, "ns3", "oid3" + padding1, 3, 3), {2, {3, {0}}}, true).get();
        }
...
struct d_seastore_tm_test_t : public seastar_test_suite_t, TMTestState
{
    seastar::future<> set_up_fut() override final { return tm_setup(); }
    seastar::future<> tear_down_fut() override final { return tm_teardown(); }
};
...
TEST_P(d_seastore_tm_test_t, 6_random_tree_insert_erase)
{
    run_async([this] {
        ...
        auto kvs = KVPool<test_item_t>::create_raw_range({8, 11, 64, 256, 301, 320},
                                                         {8, 11, 64, 256, 301, 320},
                                                         ...
                                                         {0, 4});
        auto moved_nm =
            (TEST_SEASTORE ? NodeExtentManager::create_seastore(*tm) : NodeExtentManager::create_dummy(IS_DUMMY_SYNC));
        auto p_nm = moved_nm.get();
        auto tree = std::make_unique<TreeBuilder<TRACK_CURSORS, BoundedValue>>(kvs, std::move(moved_nm));
        {
...
TEST_P(d_seastore_tm_test_t, 7_tree_insert_erase_eagain)
{
    run_async([this] {
        ...
        auto kvs = KVPool<test_item_t>::create_raw_range({8, 11, 64, 128, 255, 256},
                                                         {8, 13, 64, 512, 2035, 2048},
                                                         {8, 16, 128, 576, 992, 1200},
                                                         {0, 8},
                                                         {0, 10},
                                                         {0, 4});
        auto moved_nm = NodeExtentManager::create_seastore(*tm, L_ADDR_MIN, EAGAIN_PROBABILITY);
        auto p_nm = static_cast<SeastoreNodeExtentManager<true>*>(moved_nm.get());
        auto tree = std::make_unique<TreeBuilder<TRACK_CURSORS, ExtendedValue>>(kvs, std::move(moved_nm));
        unsigned num_ops = 0;
        ...
        repeat_eagain([this, &tree, &num_ops_eagain] {
            ++num_ops_eagain;
            return seastar::do_with(create_mutate_transaction(), [this, &tree](auto& t) {
                return INTR(tree->bootstrap, *t).safe_then([this, &t] { return submit_transaction_fut(*t); });
            });
        }).unsafe_get0();
        ...
        logger().warn("start inserting {} kvs ...", kvs.size());
        {
            ...
            while (iter != kvs.random_end()) {
                ...
                repeat_eagain([this, &tree, &num_ops_eagain, &iter] {
                    ++num_ops_eagain;
                    return seastar::do_with(create_mutate_transaction(), [this, &tree, &iter](auto& t) {
                        return INTR_R(tree->insert_one, *t, iter).safe_then([this, &t](auto cursor) {
                            cursor.invalidate();
        ...
        logger().warn("start lookup {} kvs ...", kvs.size());
        {
            ...
            while (iter != kvs.end()) {
                ...
                repeat_eagain([this, &tree, &num_ops_eagain, &iter] {
                    ++num_ops_eagain;
                    auto t = create_read_transaction();
                    return INTR_R(tree->validate_one, *t, iter).safe_then([t = std::move(t)] {});
                }).unsafe_get0();
                ++iter;
            }
        }
        ...
        logger().warn("start erase {} kvs ...", kvs.size());
        {
            ...
            while (iter != kvs.random_end()) {
                ...
                repeat_eagain([this, &tree, &num_ops_eagain, &iter] {
                    ++num_ops_eagain;
                    return seastar::do_with(create_mutate_transaction(), [this, &tree, &iter](auto& t) {
                        return INTR_R(tree->erase_one, *t, iter).safe_then([this, &t]() mutable {
                            return submit_transaction_fut(*t);
                        });
                    });
                }).unsafe_get0();
...
INSTANTIATE_TEST_SUITE_P(d_seastore_tm_test, d_seastore_tm_test_t, ::testing::Values("segmented", "circularbounded"));
