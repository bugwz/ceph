...
struct allocator_test_t : public seastar_test_suite_t, ::testing::WithParamInterface<const char*>
{
    ...
    auto allocate(size_t size) { return allocator->alloc_extent(size); }
    void free(uint64_t start, uint64_t length) { allocator->free_extent(start, length); }
    rbm_abs_addr get_random_addr(size_t block_size, size_t capacity)
    {
        return block_size * std::uniform_int_distribution<>(0, (capacity / block_size) - 1)(gen);
    }
};
...
TEST_P(allocator_test_t, test_init_alloc_free)
{
...

    {
        ...
        ASSERT_EQ(free_length - allocator->get_max_alloc_size(), allocator->get_available_size());
...
TEST_P(allocator_test_t, test_random_alloc_verify)
{
...

    {
        for (int i = 0; i < 256; i++) {
            ...
            if (addr + size > capacity || size == 0 || alloc_map.intersects(addr, size)) continue;
...
INSTANTIATE_TEST_SUITE_P(allocator_test, allocator_test_t, ::testing::Values("avl"));
