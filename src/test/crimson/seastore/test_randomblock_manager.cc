...
struct rbm_test_t : public seastar_test_suite_t
{
    ...
    {
        void add_rbm_allocated_blocks(alloc_delta_t& d) { allocated_blocks.push_back(d); }
        ...
        const auto& get_rbm_allocated_blocks() { return allocated_blocks; }
    ...
    {
        device = random_block_device::create_test_ephemeral(random_block_device::DEFAULT_TEST_CBJOURNAL_SIZE,
                                                            DEFAULT_TEST_SIZE);
        ...
        return device->mkfs(config).handle_error(crimson::ct_error::assert_all{}).then([this] {
            return device->mount().handle_error(crimson::ct_error::assert_all{}).then([this] {
    ...
    auto mkfs() { return device->mkfs(config).unsafe_get0(); }

    auto read_rbm_header() { return device->read_rbm_header(RBM_START_ADDRESS).unsafe_get0(); }
    ...
    {
        paddr_t paddr = convert_abs_addr_to_paddr(addr, rbm_manager->get_device_id());
...
TEST_F(rbm_test_t, mkfs_test)
{
    run_async([this] {
        ...
        ASSERT_TRUE(super.block_size == block_size && super.size == size);
        config.spec.id = DEVICE_ID_NULL;
        mkfs();
        super = read_rbm_header();
        ASSERT_TRUE(super.config.spec.id == DEVICE_ID_NULL && super.size == size);
    });
}
...
TEST_F(rbm_test_t, open_read_write_test)
{
    run_async([this] {
        auto content = generate_extent(1);
        {
            ...
            ASSERT_EQ(bl.begin().crc32c(bl.length(), 1), block.begin().crc32c(block.length(), 1));
        }
        close();
        open();
        {
