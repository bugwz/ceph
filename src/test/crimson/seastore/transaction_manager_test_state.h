...
public:
    EphemeralSegmentedDevices(std::size_t num_main_devices, std::size_t num_cold_devices)
    ...
    seastar::future<> setup() final
    {
        ...
        return segment_manager->init()
            .safe_then([this] {
                return crimson::do_for_each(secondary_segment_managers.begin(),
                                            secondary_segment_managers.end(),
                                            [](auto& sec_sm) { return sec_sm->init(); });
            })
            .safe_then([this] {
                return segment_manager->mkfs(segment_manager::get_ephemeral_device_config(
                ...
                return seastar::do_with(std::size_t(0), [this](auto& cnt) {
                    return crimson::do_for_each(secondary_segment_managers.begin(),
                                                ...
                                                [this, &cnt](auto& sec_sm) {
                                                    ++cnt;
                                                    return sec_sm->mkfs(segment_manager::get_ephemeral_device_config(
    ...
    std::size_t get_num_devices() const final { return secondary_segment_managers.size() + 1; }
    ...
    Device* get_primary_device() final { return segment_manager.get(); }
...
public:
    ...
    std::size_t get_num_devices() const final { return secondary_rb_devices.size() + 1; }
    ...
    Device* get_primary_device() final { return rb_device.get(); }
    DeviceRef get_primary_device_ref() final;
    void set_primary_device_ref(DeviceRef) final;
};
...
protected:
    ...
    EphemeralTestState(std::size_t num_main_device_managers, std::size_t num_cold_device_managers)
    ...
    {
        return _teardown().then([this] { return _destroy(); });
    }
    ...
    seastar::future<> restart_fut()
    {
        ...
        return teardown()
            .then([this] {
                devices->remount();
                return _init().then([this] { return _mount().handle_error(crimson::ct_error::assert_all{}); });
            })
            .then([FNAME] { SUBINFO(test, "finish"); });
    }

    void restart() { restart_fut().get0(); }
    ...
    seastar::future<> tm_setup()
    {
        ...
        if (j_type == "circularbounded") {
            ...
            devices.reset(new EphemeralSegmentedDevices(num_main_device_managers, num_cold_device_managers));
        }
        SUBINFO(test, "begin with {} devices ...", devices->get_num_devices());
        return devices->setup().then([this] { return _init(); }).then([this, FNAME] {
            return _mkfs()
                .safe_then([this] { return restart_fut(); })
                .handle_error(crimson::ct_error::assert_all{})
                .then([FNAME] { SUBINFO(test, "finish"); });
        });
    }
...
protected:
    ...
    {
        return tm->close().handle_error(crimson::ct_error::assert_all{"Error in teardown"});
    }

    virtual FuturizedStore::mount_ertr::future<> _mount()
    {
        return tm->mount()
            .handle_error(crimson::ct_error::assert_all{"Error in mount"})
            .then([this] { return epm->stop_background(); })
            .then([this] { return epm->run_background_work_until_halt(); });
    }

    virtual FuturizedStore::mkfs_ertr::future<> _mkfs()
    {
        return tm->mkfs().handle_error(crimson::ct_error::assert_all{"Error in mkfs"});
    }

    auto create_mutate_transaction() { return tm->create_transaction(Transaction::src_t::MUTATE, "test_mutate"); }

    auto create_read_transaction() { return tm->create_transaction(Transaction::src_t::READ, "test_read"); }

    auto create_weak_transaction() { return tm->create_transaction(Transaction::src_t::READ, "test_read_weak", true); }

    auto submit_transaction_fut2(Transaction& t) { return tm->submit_transaction(t); }

    auto submit_transaction_fut(Transaction& t)
    {
        return with_trans_intr(t, [this](auto& t) { return tm->submit_transaction(t); });
    }
    auto submit_transaction_fut_with_seq(Transaction& t)
    {
        using ertr = TransactionManager::base_iertr;
        return with_trans_intr(t, [this](auto& t) {
            return tm->submit_transaction(t).si_then([this] { return ertr::make_ready_future<uint64_t>(seq++); });
        });
    }
...
{
    segment_manager = segment_manager::EphemeralSegmentManagerRef(
...
{
    rb_device = random_block_device::RBMDeviceRef(static_cast<random_block_device::RBMDevice*>(dev.release()));
}
        ...
        public:
            ...
            read_meta_ret read_meta(const std::string& key) final
            {
                ...
                if (iter != parent.md.end()) {
                    return read_meta_ret(read_meta_ertr::ready_future_marker{}, iter->second);
                }
                else {
                    return read_meta_ret(read_meta_ertr::ready_future_marker{}, std::nullopt);
                }
            }
        };
        Store get_mdstore() { return Store(*this); }
    } mdstore_state;
...
protected:
    ...
    {
        seastore = make_test_seastore(std::make_unique<TestMDStoreState::Store>(mdstore_state.get_mdstore()));
        return seastore->test_start(devices->get_primary_device_ref()).then([this] {
    ...
    virtual seastar::future<> _destroy() final
    {
        ...
        return seastore->stop().then([this] { seastore.reset(); });
    }

    virtual seastar::future<> _teardown() final { return seastore->umount(); }

    virtual FuturizedStore::mount_ertr::future<> _mount() final { return seastore->test_mount(); }

    virtual FuturizedStore::mkfs_ertr::future<> _mkfs() final { return seastore->test_mkfs(uuid_d{}); }
};
