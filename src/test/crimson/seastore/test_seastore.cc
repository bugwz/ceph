...
struct seastore_test_t : public seastar_test_suite_t, SeaStoreTestState
{
    ...
    {
        return tm_setup()
            .then([this] { return sharded_seastore->create_new_collection(coll_name); })
            .then([this](auto coll_ref) {
                coll = coll_ref;
                CTransaction t;
                t.create_collection(coll_name, 0);
                return sharded_seastore->do_transaction(coll, std::move(t));
            });
    }
    ...
    void do_transaction(CTransaction&& t) { return sharded_seastore->do_transaction(coll, std::move(t)).get0(); }

    void set_meta(const std::string& key, const std::string& value) { return seastore->write_meta(key, value).get0(); }

    std::tuple<int, std::string> get_meta(const std::string& key) { return seastore->read_meta(key).get(); }
    ...
    struct object_state_t
    {
        ...
        void touch(CTransaction& t) { t.touch(cid, oid); }

        void touch(SeaStoreShard& sharded_seastore)
        {
            CTransaction t;
            touch(t);
        ...
        void truncate(CTransaction& t, uint64_t off) { t.truncate(cid, oid, off); }

        void truncate(SeaStoreShard& sharded_seastore, uint64_t off)
        {
            CTransaction t;
            truncate(t, off);
            sharded_seastore.do_transaction(coll, std::move(t)).get0();
        ...
        std::map<uint64_t, uint64_t> fiemap(SeaStoreShard& sharded_seastore, uint64_t off, uint64_t len)
        {
            return sharded_seastore.fiemap(coll, oid, off, len).unsafe_get0();
        }

        bufferlist readv(SeaStoreShard& sharded_seastore, interval_set<uint64_t>& m)
        {
            return sharded_seastore.readv(coll, oid, m).unsafe_get0();
        }

        void remove(CTransaction& t)
        {
            t.remove(cid, oid);
            t.remove_collection(cid);
        }

        void remove(SeaStoreShard& sharded_seastore)
        {
            CTransaction t;
            remove(t);
            sharded_seastore.do_transaction(coll, std::move(t)).get0();
        ...
        void set_omap(CTransaction& t, const string& key, const bufferlist& val)
        {
            omap[key] = val;
        ...
        void set_omap(SeaStoreShard& sharded_seastore, const string& key, const bufferlist& val)
        {
            CTransaction t;
            set_omap(t, key, val);
            sharded_seastore.do_transaction(coll, std::move(t)).get0();
        ...
        void write(SeaStoreShard& sharded_seastore, CTransaction& t, uint64_t offset, bufferlist bl)
        {
            bufferlist new_contents;
            if (offset > 0 && contents.length()) {
                new_contents.substr_of(contents, 0, std::min<size_t>(offset, contents.length()));
            }
            ...
            if (contents.length() > tail_offset) {
                bufferlist tail;
                tail.substr_of(contents, tail_offset, contents.length() - tail_offset);
                new_contents.append(tail);
            }
            contents.swap(new_contents);

            t.write(cid, oid, offset, bl.length(), bl);
        }

        void write(SeaStoreShard& sharded_seastore, uint64_t offset, bufferlist bl)
        {
            CTransaction t;
        ...
        void clone(SeaStoreShard& sharded_seastore, snapid_t snap)
        {
            ghobject_t coid = oid;
            coid.hobj.snap = snap;
            CTransaction t;
        ...
        void write(SeaStoreShard& sharded_seastore, uint64_t offset, size_t len, char fill)
        {
        ...
        void zero(SeaStoreShard& sharded_seastore, CTransaction& t, uint64_t offset, size_t len)
        {
            ceph::buffer::list bl;
            bl.append_zero(len);
            bufferlist new_contents;
            ...
            t.zero(cid, oid, offset, len);
        }

        void zero(SeaStoreShard& sharded_seastore, uint64_t offset, size_t len)
        {
            CTransaction t;
        ...
        void read(SeaStoreShard& sharded_seastore, uint64_t offset, uint64_t len)
        {
            bufferlist to_check;
            if (contents.length() >= offset) {
                to_check.substr_of(contents, offset, std::min(len, (uint64_t)contents.length()));
            }
            auto ret = sharded_seastore.read(coll, oid, offset, len).unsafe_get0();
        ...
        {
            auto st = sharded_seastore.stat(coll, oid).get0();
            EXPECT_EQ(contents.length(), st.st_size);
        }

        void set_attr(SeaStoreShard& sharded_seastore, std::string key, bufferlist& val)
        {
            CTransaction t;
        ...
        void rm_attr(SeaStoreShard& sharded_seastore, std::string key)
        {
            CTransaction t;
        ...
        void rm_attrs(SeaStoreShard& sharded_seastore)
        {
            CTransaction t;
            t.rmattrs(cid, oid);
            sharded_seastore.do_transaction(coll, std::move(t)).get0();
        ...
        SeaStoreShard::attrs_t get_attrs(SeaStoreShard& sharded_seastore)
        {
        ...
        ceph::bufferlist get_attr(SeaStoreShard& sharded_seastore, std::string_view name)
        {
            return sharded_seastore.get_attr(coll, oid, name)
                .handle_error(SeaStoreShard::get_attr_errorator::discard_all{})
                .get();
        }

        void check_omap_key(SeaStoreShard& sharded_seastore, const string& key)
        {
            std::set<string> to_check;
            to_check.insert(key);
            auto result = sharded_seastore.omap_get_values(coll, oid, to_check).unsafe_get0();
            if (result.empty()) {
        ...
        void check_omap(SeaStoreShard& sharded_seastore)
        {
            ...
            while (true) {
                auto [done, kvs] = sharded_seastore.omap_get_values(coll, oid, start).unsafe_get0();
                auto iter = kvs.begin();
                while (true) {
                    ...
                    if (iter == kvs.end() || refiter->first < iter->first) {
                        logger().debug("check_omap: missing omap key {}", refiter->first);
                        GTEST_FAIL() << "missing omap key " << refiter->first;
                        ++refiter;
                    }
                    else if (refiter == omap.end() || refiter->first > iter->first) {
                        logger().debug("check_omap: extra omap key {}", iter->first);
    ...
    object_state_t& get_object(const ghobject_t& oid)
    {
        return test_objects.emplace(std::make_pair(oid, object_state_t{coll_name, coll, oid})).first->second;
    }

    void remove_object(object_state_t& sobj)
    {
    ...
    void validate_objects() const
    {
        ...
        auto ret = sharded_seastore
                       ->list_objects(coll, ghobject_t(), ghobject_t::get_max(), std::numeric_limits<uint64_t>::max())
                       .get0();
    ...
    struct bound_t
    {
        ...
        static bound_t get_temp(unsigned index) { return bound_t{type_t::TEMP, index}; }
        static bound_t get_normal(unsigned index) { return bound_t{type_t::NORMAL, index}; }
        ...
        static bound_t get_normal_begin() { return bound_t{type_t::NORMAL_BEGIN}; }

        ghobject_t get_oid(SeaStore& seastore, CollectionRef& coll) const
        {
            switch (type) {
            case type_t::MIN: return ghobject_t();
            case type_t::MAX: return ghobject_t::get_max();
            case type_t::TEMP: return make_temp_oid(index);
            case type_t::TEMP_END: return seastore.get_objs_range(coll, 0).temp_end;
            case type_t::NORMAL_BEGIN: return seastore.get_objs_range(coll, 0).obj_begin;
            case type_t::NORMAL: return make_oid(index);
            default: assert(0 == "impossible"); return ghobject_t();
            }
        }
    };
    ...
    void test_list(unsigned temp_to_create,     /// create temp 0..temp_to_create-1
    ...
    )
    {
        ...
        for (auto [limit, in_left_bound, in_right_bound] : cases) {
            ...
            auto [listed, next] = sharded_seastore->list_objects(coll, left_bound, right_bound, limit).get0();

            // compute correct answer
            auto correct_begin =
                std::find_if(objs.begin(), objs.end(), [&left_bound](const auto& in) { return in >= left_bound; });
            unsigned count = 0;
            auto correct_end = correct_begin;
            for (; count < limit && correct_end != objs.end() && *correct_end < right_bound; ++correct_end, ++count)
                ;
        ...
        // teardown
        for (auto&& hoid : objs) {
            get_object(hoid).remove(*sharded_seastore);
        }
    }
};

template<typename T, typename V> auto contains(const T& t, const V& v)
{
...
TEST_P(seastore_test_t, collection_create_list_remove)
{
    run_async([this] {
        coll_t test_coll{spg_t{pg_t{1, 0}}};
        {
            ...
            std::transform(colls_cores.begin(), colls_cores.end(), colls.begin(), [](auto p) { return p.first; });
...
TEST_P(seastore_test_t, omap_test_simple)
{
    run_async([this] {
        auto& test_obj = get_object(make_oid(0));
        test_obj.touch(*sharded_seastore);
        test_obj.set_omap(*sharded_seastore, "asdf", make_bufferlist(128));
        test_obj.check_omap_key(*sharded_seastore, "asdf");
    });
}
...
TEST_P(seastore_test_t, omap_test_iterator)
{
    run_async([this] {
        ...
        for (unsigned i = 0; i < 20; ++i) {
            test_obj.set_omap(*sharded_seastore, make_key(i), make_bufferlist(128));
        }
...
TEST_P(seastore_test_t, object_data_omap_remove)
{
    run_async([this] {
        ...
        for (uint64_t i = 0; i < 16; i++) {
            test_obj.write(*sharded_seastore, 4096 * i, 4096, 'a');
        }
...
TEST_P(seastore_test_t, simple_extent_test)
{
    run_async([this] {
        auto& test_obj = get_object(make_oid(0));
        test_obj.write(*sharded_seastore, 1024, 1024, 'a');
        test_obj.read(*sharded_seastore, 1024, 1024);
        test_obj.check_size(*sharded_seastore);
    });
}
...
TEST_P(seastore_test_t, fiemap_holes)
{
    run_async([this] {
        ...
        {   // fiemap test from 0 to SKIP_STEP * (MAX_EXTENTS - 1) + 3
            auto m = test_obj.fiemap(*sharded_seastore, 0, SKIP_STEP * (MAX_EXTENTS - 1) + 3);
        ...
        {   // fiemap test from SKIP_STEP to SKIP_STEP * (MAX_EXTENTS - 2) + 3
            auto m = test_obj.fiemap(*sharded_seastore, SKIP_STEP, SKIP_STEP * (MAX_EXTENTS - 3) + 3);
        ...
        {   // fiemap test SKIP_STEP + 1 to 2 * SKIP_STEP + 1 (partial overlap)
            auto m = test_obj.fiemap(*sharded_seastore, SKIP_STEP + 1, SKIP_STEP + 1);
            ASSERT_EQ(m.size(), 2);
...
TEST_P(seastore_test_t, sparse_read)
{
    run_async([this] {
        ...
        m = interval_set<uint64_t>(test_obj.fiemap(*sharded_seastore, 0, SKIP_STEP * (MAX_EXTENTS - 1) + 3));
        ASSERT_TRUE(!m.empty());
        uint64_t off = 0;
...
TEST_P(seastore_test_t, zero)
{
    run_async([this] {
        auto test_zero = [this](
            ...
            test_obj.read(*sharded_seastore, 0, size);
            test_obj.check_size(*sharded_seastore);
        ...
        // Test zero within a block
        test_zero({{1 << 10, 1 << 10, 1}}, 1124, 200);

        // Multiple writes, partial on left, partial on right.
        test_zero({{BS, BS, 10}}, BS + 128, BS * 4);

        // Single large write, block boundary on right, partial on left.
        test_zero({{BS, BS * 10, 1}}, BS + 128, (BS * 4) - 128);

        // Multiple writes, block boundary on left, partial on right.
        test_zero({{BS, BS, 10}}, BS, (BS * 4) + 128);
    });
}
INSTANTIATE_TEST_SUITE_P(seastore_test, seastore_test_t, ::testing::Values("segmented", "circularbounded"));
