...
struct record_validator_t
{
    ...
    template<typename... T>
    record_validator_t(T&&... record)
        : record(std::forward<T>(record)...)
    {}
    ...
    void validate(SegmentManager& manager)
    {
        ...
        for (auto&& block : record.extents) {
            auto test = manager.read(record_final_offset.add_relative(addr), block.bl.length()).unsafe_get0();
            addr = addr.add_offset(block.bl.length());
            bufferlist bl;
            bl.push_back(test);
            ASSERT_EQ(bl.length(), block.bl.length());
            ASSERT_EQ(bl.begin().crc32c(bl.length(), 1), block.bl.begin().crc32c(block.bl.length(), 1));
        }
    }

    auto get_replay_handler()
    {
        auto checker = [this, iter = record.deltas.begin()](paddr_t base, const delta_info_t& di) mutable {
...
struct journal_test_t : seastar_test_suite_t, SegmentProvider, JournalTrimmer
{
    ...
    seastar::future<> set_up_fut() final
    {
        ...
        return segment_manager->init()
            ...
            .safe_then(
                ...
                crimson::ct_error::all_same_way([] { ASSERT_FALSE("Unable to mount"); }));
    }
    ...
    seastar::future<> tear_down_fut() final
    {
        return journal->close()
            ...
            .handle_error(crimson::ct_error::all_same_way([](auto e) { ASSERT_FALSE("Unable to close"); }));
    }

    template<typename T> auto replay(T&& f)
    {
        return journal->close()
    ...
    auto replay_and_check()
    {
        ...
        advance();
        replay([&advance, &delta_checker](
                   const auto& offsets, const auto& di, const journal_seq_t&, const journal_seq_t&, auto t) mutable {
            if (!delta_checker) {
    ...
    template<typename... T> auto submit_record(T&&... _record)
    {
        auto record{std::forward<T>(_record)...};
        records.push_back(record);
        OrderingHandle handle = get_dummy_ordering_handle();
        auto [addr, _] = journal->submit_record(std::move(record), handle).unsafe_get0();
...
{
    run_async([this] {
        submit_record(record_t{{generate_extent(1), generate_extent(2)}, {generate_delta(23), generate_delta(30)}});
        replay_and_check();
    });
}
...
TEST_F(journal_test_t, replay_two_records)
{
    run_async([this] {
        submit_record(record_t{{generate_extent(1), generate_extent(2)}, {generate_delta(23), generate_delta(30)}});
        submit_record(record_t{{generate_extent(4), generate_extent(1)}, {generate_delta(23), generate_delta(400)}});
        replay_and_check();
    });
}
...
TEST_F(journal_test_t, replay_twice)
{
    run_async([this] {
        ...
        submit_record(record_t{{generate_extent(4), generate_extent(1)}, {generate_delta(23), generate_delta(400)}});
        replay_and_check();
        submit_record(record_t{{generate_extent(2), generate_extent(5)}, {generate_delta(230), generate_delta(40)}});
        replay_and_check();
    });
}
...
TEST_F(journal_test_t, roll_journal_and_replay)
{
    run_async([this] {
        ...
        while (current.as_seg_paddr().get_segment_id() == starting_segment) {
            current = submit_record(
                record_t{{generate_extent(512), generate_extent(512)}, {generate_delta(23), generate_delta(400)}});
            ++so_far;
