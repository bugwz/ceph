...
struct btree_test_base : public seastar_test_suite_t, SegmentProvider, JournalTrimmer
{
    ...
    std::size_t get_trim_size_per_cycle() const final { return 0; }
    ...
    segment_id_t allocate_segment(segment_seq_t seq, segment_type_t type, data_category_t, rewrite_gen_t) final
    {
        auto ret = next;
        next = segment_id_t{segment_manager->get_device_id(), next.device_segment_id() + 1};
    ...
    seastar::future<> submit_transaction(TransactionRef t)
    {
        ...
        return journal->submit_record(std::move(record), t->get_handle())
            .safe_then([this, t = std::move(t)](auto submit_result) mutable {
                cache->complete_commit(*t, submit_result.record_block_base, submit_result.write_result.start_seq);
                complete_commit(*t);
            })
    ...
    seastar::future<> set_up_fut() final
    {
        ...
        return segment_manager->init()
            .safe_then([this] { return segment_manager->mkfs(segment_manager::get_ephemeral_device_config(0, 1, 0)); })
            .safe_then([this] {
            ...
            .safe_then([this] {
                dummy_tail =
                    journal_seq_t{0, paddr_t::make_seg_paddr(segment_id_t(segment_manager->get_device_id(), 0), 0)};
                return epm->open_for_write();
            })
            .safe_then([this] {
                return seastar::do_with(
                    cache->create_transaction(Transaction::src_t::MUTATE, "test_set_up_fut", false),
                    [this](auto& ref_t) {
                        return with_trans_intr(
                                   *ref_t,
                                   [&](auto& t) {
                                       cache->init();
                                       return cache->mkfs(t).si_then([this, &t] { return test_structure_setup(t); });
                                   })
                            .safe_then([this, &ref_t] { return submit_transaction(std::move(ref_t)); });
                    });
            })
            .handle_error(crimson::ct_error::all_same_way([] { ceph_assert(0 == "error"); }));
    }
    ...
    {
        return cache->close()
            .safe_then([this] { return journal->close(); })
            ...
            .handle_error(crimson::ct_error::all_same_way([] { ASSERT_FALSE("Unable to close"); }));
    }
};
...
struct lba_btree_test : btree_test_base
{
    ...
    auto get_op_context(Transaction& t) { return op_context_t<laddr_t>{*cache, t}; }

    LBAManager::mkfs_ret test_structure_setup(Transaction& t) final
    {
        return cache->get_root(t).si_then([this, &t](RootBlockRef croot) {
            auto mut_croot = cache->duplicate_for_write(t, croot)->cast<RootBlock>();
            mut_croot->root.lba_root = LBABtree::mkfs(mut_croot, get_op_context(t));
        });
    }

    template<typename F> auto lba_btree_update(F&& f)
    {
        auto tref = cache->create_transaction(Transaction::src_t::MUTATE, "test_btree_update", false);
        auto& t = *tref;
        with_trans_intr(t, [this, tref = std::move(tref), f = std::forward<F>(f)](auto& t) mutable {
            return cache->get_root(t)
                .si_then([f = std::move(f), &t](RootBlockRef croot) {
                    return seastar::do_with(LBABtree(croot), [f = std::move(f), &t](auto& btree) mutable {
                        return std::invoke(std::move(f), btree, t);
                    });
                })
                .si_then([this, tref = std::move(tref)]() mutable { return submit_transaction(std::move(tref)); });
        }).unsafe_get0();
    }

    template<typename F> auto lba_btree_read(F&& f)
    {
        auto t = cache->create_transaction(Transaction::src_t::READ, "test_btree_read", false);
        return with_trans_intr(
                   *t,
                   [this, f = std::forward<F>(f)](auto& t) mutable {
                       return cache->get_root(t).si_then([f = std::move(f), &t](RootBlockRef croot) mutable {
                           return seastar::do_with(LBABtree(croot), [f = std::move(f), &t](auto& btree) mutable {
                               return std::invoke(std::move(f), btree, t);
                           });
                       });
    ...
    static auto get_map_val(extent_len_t len) { return lba_map_val_t{0, (pladdr_t)P_ADDR_NULL, len, 0}; }
    ...
    void insert(laddr_t addr, extent_len_t len)
    {
        ...
        lba_btree_update([=, this](auto& btree, auto& t) {
            auto extent = cache->alloc_new_extent<TestBlock>(t, TestBlock::SIZE, placement_hint_t::HOT, 0, get_paddr());
            return btree.insert(get_op_context(t), addr, get_map_val(len), extent.get())
    ...
    void remove(laddr_t addr)
    {
        ...
        check.erase(iter++);
        lba_btree_update([=, this](auto& btree, auto& t) {
            return btree.lower_bound(get_op_context(t), addr).si_then([this, len, addr, &btree, &t](auto iter) {
                EXPECT_FALSE(iter.is_end());
                EXPECT_TRUE(iter.get_key() == addr);
                EXPECT_TRUE(iter.get_val().len == len);
                return btree.remove(get_op_context(t), iter);
            });
        });
    }
    ...
    void check_lower_bound(laddr_t addr)
    {
        ...
        auto result = lba_btree_read([=, this](auto& btree, auto& t) {
            return btree.lower_bound(get_op_context(t), addr)
                .si_then([](auto iter) -> std::optional<std::pair<const laddr_t, const lba_map_val_t>> {
                    if (iter.is_end()) {
                        return std::nullopt;
                    }
                    else {
                        return std::make_optional(std::make_pair(iter.get_key(), iter.get_val()));
                    }
                });
        });
        if (iter == check.end()) {
...
struct btree_lba_manager_test : btree_test_base
{
    ...
    void test_structure_reset() final { lba_manager.reset(); }
    ...
    {
        auto t = test_transaction_t{cache->create_transaction(Transaction::src_t::MUTATE, "test_mutate_lba", false),
                                    test_lba_mappings};
        if (create_fake_extent) {
            cache->alloc_new_extent<TestBlockPhysical>(*t.t, TestBlockPhysical::SIZE, placement_hint_t::HOT, 0);
        };
        return t;
    }
    ...
    {
        auto t = test_transaction_t{cache->create_transaction(Transaction::src_t::READ, "test_read_weak", true),
                                    test_lba_mappings};
        return t;
    }
    ...
    auto get_overlap(test_transaction_t& t, laddr_t addr, size_t len)
    {
        ...
        if (bottom != t.mappings.begin()) --bottom;
        if (bottom != t.mappings.end() && bottom->first + bottom->second.len <= addr) ++bottom;
    ...
    auto alloc_mapping(test_transaction_t& t, laddr_t hint, size_t len)
    {
        auto ret = with_trans_intr(*t.t, [=, this](auto& t) {
                       auto extent = cache->alloc_new_extent<TestBlock>(
                           t, TestBlock::SIZE, placement_hint_t::HOT, 0, get_paddr());
                       return lba_manager->alloc_extent(t, hint, len, extent->get_paddr(), *extent);
                   }).unsafe_get0();
        logger().debug("alloc'd: {}", *ret);
        EXPECT_EQ(len, ret->get_length());
        auto [b, e] = get_overlap(t, ret->get_key(), len);
        EXPECT_EQ(b, e);
        t.mappings.emplace(std::make_pair(ret->get_key(), test_extent_t{ret->get_val(), ret->get_length(), 1}));
        return ret;
    }

    auto decref_mapping(test_transaction_t& t, laddr_t addr) { return decref_mapping(t, t.mappings.find(addr)); }

    void decref_mapping(test_transaction_t& t, test_lba_mapping_t::iterator target)
    {
        ceph_assert(target != t.mappings.end());
        ceph_assert(target->second.refcount > 0);
        target->second.refcount--;

        (void)with_trans_intr(*t.t, [=, this](auto& t) {
            return lba_manager->decref_extent(t, target->first, true).si_then([this, &t, target](auto result) {
                EXPECT_EQ(result.refcount, target->second.refcount);
                if (result.refcount == 0) {
                    return cache->retire_extent_addr(t, result.addr.get_paddr(), result.length);
                }
    ...
    auto incref_mapping(test_transaction_t& t, laddr_t addr) { return incref_mapping(t, t.mappings.find(addr)); }

    void incref_mapping(test_transaction_t& t, test_lba_mapping_t::iterator target)
    {
        ceph_assert(target->second.refcount > 0);
        target->second.refcount++;
        auto refcnt = with_trans_intr(*t.t, [=, this](auto& t) { return lba_manager->incref_extent(t, target->first); })
                          .unsafe_get0()
                          .refcount;
    ...
    void check_mappings(test_transaction_t& t)
    {
        ...
        for (auto&& i : t.mappings) {
            ...
            auto ret_list = with_trans_intr(*t.t, [=, this](auto& t) {
            ...
            auto ret_pin =
                with_trans_intr(*t.t, [=, this](auto& t) { return lba_manager->get_mapping(t, laddr); }).unsafe_get0();
            EXPECT_EQ(i.second.addr, ret_pin->get_val());
            EXPECT_EQ(laddr, ret_pin->get_key());
            EXPECT_EQ(len, ret_pin->get_length());
        }
        with_trans_intr(*t.t, [=, &t, this](auto&) {
            return lba_manager->scan_mappings(
                *t.t, 0, L_ADDR_MAX, [iter = t.mappings.begin(), &t](auto l, auto p, auto len) mutable {
...
TEST_F(btree_lba_manager_test, split_merge_multi)
{
    run_async([this] {
        ...
        iterate([&](auto& t, auto idx) { alloc_mapping(t, idx * block_size, block_size); });
        check_mappings();
        ...
        check_mappings();
        iterate([&](auto& t, auto idx) { decref_mapping(t, idx * block_size); });
        check_mappings();
    });
}
