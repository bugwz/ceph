...
struct btree_lba_manager_test : public seastar_test_suite_t, JournalSegmentProvider
{
    ...
    segment_id_t next = 0;
    get_segment_ret get_segment() final { return get_segment_ret(get_segment_ertr::ready_future_marker{}, next++); }
    ...
    seastar::future<> set_up_fut() final
    {
        return segment_manager->init()
            ...
            .safe_then([this](auto addr) {
                return seastar::do_with(make_transaction(), [this](auto& transaction) {
                    cache.init();
                    return cache.mkfs(*transaction)
                        .safe_then([this, &transaction] { return lba_manager->mkfs(*transaction); })
                        .safe_then([this, &transaction] { return submit_transaction(std::move(transaction)); });
                });
            })
    ...
    {
        auto t = test_transaction_t{make_transaction(), test_lba_mappings};
    ...
    {
        auto t = test_transaction_t{make_weak_transaction(), test_lba_mappings};
        return t;
    }
    ...
    auto alloc_mapping(test_transaction_t& t, laddr_t hint, size_t len, paddr_t paddr)
    {
        ...
        EXPECT_EQ(b, e);
        t.mappings.emplace(std::make_pair(ret->get_laddr(), test_extent_t{ret->get_paddr(), ret->get_length(), 1}));
        return ret;
    }

    auto set_mapping(test_transaction_t& t, laddr_t addr, size_t len, paddr_t paddr)
    {
    ...
    void decref_mapping(test_transaction_t& t, test_lba_mapping_t::iterator target)
    {
        ...
        auto refcnt = lba_manager->decref_extent(*t.t, target->first).unsafe_get0().refcount;
    ...
    void incref_mapping(test_transaction_t& t, test_lba_mapping_t::iterator target)
    {
        ceph_assert(target->second.refcount > 0);
        target->second.refcount++;
        auto refcnt = lba_manager->incref_extent(*t.t, target->first).unsafe_get0().refcount;
        EXPECT_EQ(refcnt, target->second.refcount);
    }
    ...
    {
        for (auto&& i : t.mappings) {
            auto ret_list = lba_manager->get_mapping(*t.t, i.first, i.second.len).unsafe_get0();
