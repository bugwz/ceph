...
struct cache_test_t : public seastar_test_suite_t
{
    ...
    seastar::future<paddr_t> submit_transaction(TransactionRef t)
    {
        ...
        ceph_assert((segment_off_t)bl.length() < segment_manager->get_segment_size());
        if (current.as_seg_paddr().get_segment_off() + (segment_off_t)bl.length() > segment_manager->get_segment_size())
            current =
                paddr_t::make_seg_paddr(segment_id_t(current.as_seg_paddr().get_segment_id().device_id(),
                                                     current.as_seg_paddr().get_segment_id().device_segment_id() + 1),
                                        0);

        auto prev = current;
        current.as_seg_paddr().set_segment_off(current.as_seg_paddr().get_segment_off() + bl.length());
        return segment_manager->segment_write(prev, std::move(bl), true)
            .safe_then(
                [this, prev, t = std::move(t)]() mutable {
                    cache->complete_commit(*t, prev, seq /* TODO */);
                    return prev;
                },
                crimson::ct_error::all_same_way([](auto e) { ASSERT_FALSE("failed to submit"); }));
    }

    auto get_transaction() { return cache->create_transaction(Transaction::src_t::MUTATE, "test_cache", false); }

    template<typename T, typename... Args> auto get_extent(Transaction& t, Args&&... args)
    {
        return with_trans_intr(
            t, [this](auto&&... args) { return cache->get_extent<T>(args...); }, std::forward<Args>(args)...);
    }
    ...
    seastar::future<> set_up_fut() final
    {
        ...
        return segment_manager->init()
            ...
            .safe_then([this] {
                ...
                return seastar::do_with(get_transaction(), [this](auto& ref_t) {
                    cache->init();
                    return with_trans_intr(*ref_t, [&](auto& t) { return cache->mkfs(t); }).safe_then([this, &ref_t] {
                        return submit_transaction(std::move(ref_t)).then([](auto p) {});
                    });
                });
            })
            .handle_error(crimson::ct_error::all_same_way([](auto e) { ASSERT_FALSE("failed to submit"); }));
    }
    ...
    seastar::future<> tear_down_fut() final
    {
        return cache->close()
            ...
            .handle_error(Cache::close_ertr::assert_all{});
    }
};
...
TEST_F(cache_test_t, test_addr_fixup)
{
    run_async([this] {
        ...
        int csum = 0;
        {
            ...
            auto extent =
                cache->alloc_new_extent<TestBlockPhysical>(*t, TestBlockPhysical::SIZE, placement_hint_t::HOT, 0);
        ...
        {
            auto t = get_transaction();
            auto extent = get_extent<TestBlockPhysical>(*t, addr, TestBlockPhysical::SIZE).unsafe_get0();
    ...
    run_async([this] {
        ...
        int csum2 = 0;
        {
            ...
            ASSERT_TRUE(reladdr.is_relative());
            {
                ...
                // uncommitted
                auto extent = get_extent<TestBlockPhysical>(*t, reladdr, TestBlockPhysical::SIZE).unsafe_get0();
        ...
        }
        {
            ...
            auto extent2 = get_extent<TestBlockPhysical>(*t2, addr, TestBlockPhysical::SIZE).unsafe_get0();
        ...
        }
        {
            ...
            ASSERT_EQ(extent->get_paddr(), addr);
            {
                ...
                // block
                auto t2 = get_transaction();
                auto extent = get_extent<TestBlockPhysical>(*t2, addr, TestBlockPhysical::SIZE).unsafe_get0();
                ASSERT_TRUE(extent->is_clean());
