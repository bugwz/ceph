...
struct cache_test_t : public seastar_test_suite_t
{
    ...
    seastar::future<std::optional<paddr_t>> submit_transaction(TransactionRef t)
    {
        auto record = cache.try_construct_record(*t);
        if (!record) {
            return seastar::make_ready_future<std::optional<paddr_t>>(std::nullopt);
        }
        ...
        if (current.offset + (segment_off_t)bl.length() > segment_manager->get_segment_size())
    ...
    auto get_transaction() { return make_transaction(); }
            ...
            .safe_then([this] {
                return seastar::do_with(make_transaction(), [this](auto& transaction) {
                    ...
                    return cache.mkfs(*transaction).safe_then([this, &transaction] {
                        return submit_transaction(std::move(transaction)).then([](auto p) { ASSERT_TRUE(p); });
                    });
                });
            })
    ...
    seastar::future<> tear_down_fut() final { return cache.close().handle_error(Cache::close_ertr::assert_all{}); }
};
...
TEST_F(cache_test_t, test_addr_fixup)
{
    run_async([this] {
        ...
        int csum = 0;
        {
            ...
            auto extent = cache.alloc_new_extent<TestBlockPhysical>(*t, TestBlockPhysical::SIZE);
        ...
        {
            auto t = get_transaction();
            auto extent = cache.get_extent<TestBlockPhysical>(*t, addr, TestBlockPhysical::SIZE).unsafe_get0();
    ...
    run_async([this] {
        ...
        int csum2 = 0;
        {
            ...
            ASSERT_TRUE(reladdr.is_relative());
            {
                ...
                // uncommitted
                auto extent = cache.get_extent<TestBlockPhysical>(*t, reladdr, TestBlockPhysical::SIZE).unsafe_get0();
        ...
        }
        {
            ...
            auto extent2 = cache.get_extent<TestBlockPhysical>(*t2, addr, TestBlockPhysical::SIZE).unsafe_get0();
        ...
        }
        {
            ...
            ASSERT_EQ(extent->get_paddr(), addr);
            {
                ...
                // block
                auto t2 = get_transaction();
                auto extent = cache.get_extent<TestBlockPhysical>(*t2, addr, TestBlockPhysical::SIZE).unsafe_get0();
                ASSERT_TRUE(extent->is_clean());
