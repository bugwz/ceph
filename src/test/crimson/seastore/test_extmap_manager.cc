...
struct extentmap_manager_test_t : public seastar_test_suite_t, TMTestState
{
    ...
    extent_mapping_t insert_extent(extmap_root_t& extmap_root, Transaction& t, uint32_t lo, lext_map_val_t val)
    {
        ...
        test_ext_mappings.emplace(extent.logical_offset, lext_map_val_t{extent.laddr, extent.length});
        return extent;
    }

    extent_map_list_t find_extent(extmap_root_t& extmap_root, Transaction& t, uint32_t lo, uint32_t len)
    {
    ...
    extent_map_list_t findno_extent(extmap_root_t& extmap_root, Transaction& t, uint32_t lo, uint32_t len)
    {
        auto extent = extmap_manager->find_lextent(extmap_root, t, lo, len).unsafe_get0();
    ...
    void rm_extent(extmap_root_t& extmap_root, Transaction& t, uint32_t lo, lext_map_val_t val)
    {
