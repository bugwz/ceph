...
#define TEST_COLL_FORWARD(METHOD)                                                                         \
    template<typename... Args> auto METHOD(coll_root_t& root, Transaction& t, Args&&... args) const       \
    {                                                                                                     \
        return with_trans_intr(                                                                           \
                   t,                                                                                     \
                   [this](auto& t, auto& root, auto&&... args) {                                          \
                       return collection_manager->METHOD(root, t, std::forward<decltype(args)>(args)...); \
                   },                                                                                     \
                   root,                                                                                  \
...
struct collection_manager_test_t : public seastar_test_suite_t, TMTestState
{
    ...
    auto get_root()
    {
        ...
        auto coll_root = with_trans_intr(*tref, [this](auto& t) { return collection_manager->mkfs(t); }).unsafe_get0();
    ...
    void checking_mappings(coll_root_t& coll_root, Transaction& t)
    {
        ...
        for (std::pair<coll_t, coll_info_t> p : test_coll_mappings) {
            EXPECT_NE(std::find(coll_list.begin(), coll_list.end(), p), coll_list.end());
        }
    }
...
TEST_P(collection_manager_test_t, basic)
{
    run_async([this] {
        ...
        {
            auto t = create_mutate_transaction();
            for (auto iter = test_coll_mappings.begin(); iter != test_coll_mappings.end();) {
...
INSTANTIATE_TEST_SUITE_P(collection_manager_test, collection_manager_test_t,
