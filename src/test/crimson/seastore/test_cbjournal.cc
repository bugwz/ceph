...
std::optional<record_t> decode_record(bufferlist& bl)
{
    record_t record;
    record_group_header_t r_header;
    auto bliter = bl.cbegin();
    decode(r_header, bliter);
    logger().debug(" decode_record mdlength {} records {}", r_header.mdlength, r_header.records);
    device_id_t d_id = 1 << (std::numeric_limits<device_id_t>::digits - 1);

    auto del_infos = try_decode_deltas(r_header, bl, paddr_t::make_blk_paddr(d_id, 0));
...
struct entry_validator_t
{
    ...
    void validate(record_t read)
    {
        ...
        for (auto&& block : record.extents) {
            ASSERT_EQ(iter->bl.length(), block.bl.length());
            ASSERT_EQ(iter->bl.begin().crc32c(iter->bl.length(), 1), block.bl.begin().crc32c(block.bl.length(), 1));
            ++iter;
        }
        auto iter_delta = read.deltas.begin();
        for (auto&& block : record.deltas) {
            ASSERT_EQ(iter_delta->bl.length(), block.bl.length());
            ASSERT_EQ(iter_delta->bl.begin().crc32c(iter_delta->bl.length(), 1),
    ...
    void validate(CircularBoundedJournal& cbj)
    {
        ...
        for (int i = 0; i < entries; i++) {
            ...
            auto md = cbj.test_read_validate_record_metadata(cursor, magic).unsafe_get0();
            assert(md);
            auto& [header, md_bl] = *md;
            auto dbuf = cbj.read(paddr.add_offset(header.mdlength), header.dlength).unsafe_get0();
    ...
    rbm_abs_addr get_abs_addr() { return convert_paddr_to_abs_addr(seq.offset); }

    bool validate_delta(bufferlist bl)
    {
        for (auto&& block : record.deltas) {
            if (bl.begin().crc32c(bl.length(), 1) == block.bl.begin().crc32c(block.bl.length(), 1)) {
                return true;
            }
        }
        return false;
...
struct cbjournal_test_t : public seastar_test_suite_t, JournalTrimmer
{
     ...
     */
    journal_seq_t get_journal_head() const { return JOURNAL_SEQ_NULL; }

    journal_seq_t get_dirty_tail() const final { return JOURNAL_SEQ_NULL; }

    journal_seq_t get_alloc_tail() const final { return JOURNAL_SEQ_NULL; }
    ...
    auto submit_record(record_t&& record)
    {
        ...
        auto [addr, w_result] = cbj->submit_record(std::move(record), handle).unsafe_get0();
    ...
    seastar::future<> tear_down_fut() final { return close(); }
    ...
    delta_info_t generate_delta(size_t bytes)
    {
        ...
        return delta_info_t{extent_types_t::TEST_BLOCK,
                            paddr_t{},
                            L_ADDR_NULL,
                            0,
                            0,
    ...
    {
        return cbj->replay(
            [this](const auto& offsets, const auto& e, auto& dirty_seq, auto& alloc_seq, auto last_modified) {
                bool found = false;
                for (auto& i : entries) {
    ...
    auto mkfs()
    {
        ...
        return device->mkfs(config)
            .safe_then([this]() {
                return device->mount().safe_then(
                    [this]() { return cbj->open_for_mkfs().safe_then([](auto q) { return seastar::now(); }); });
            })
            .safe_then([this] { return cbj->close(); });
    }
    auto open()
    {
        return cbj->open_for_mount().safe_then([](auto q) { return seastar::now(); });
    }
    seastar::future<> close() { return cbj->close().handle_error(crimson::ct_error::assert_all{}); }
    auto get_records_available_size() { return cbj->get_cjs().get_records_available_size(); }
    auto get_records_total_size() { return cbj->get_cjs().get_records_total_size(); }
    auto get_block_size() { return device->get_block_size(); }
    auto get_written_to_rbm_addr() { return cbj->get_rbm_addr(cbj->get_cjs().get_written_to()); }
    auto get_written_to() { return cbj->get_cjs().get_written_to(); }
    auto get_journal_tail() { return cbj->get_dirty_tail(); }
    auto get_records_used_size() { return cbj->get_cjs().get_records_used_size(); }
    bool is_available_size(uint64_t size) { return cbj->get_cjs().is_available_size(size); }
    void update_journal_tail(rbm_abs_addr addr, uint32_t len)
    {
        paddr_t paddr = convert_abs_addr_to_paddr(addr + len, cbj->get_device_id());
        journal_seq_t seq = {0, paddr};
        cbj->update_journal_tail(seq, seq).get0();
    }
    void set_written_to(journal_seq_t seq) { cbj->set_written_to(seq); }

    seastar::future<> set_up_fut() final
    {
        device = random_block_device::create_test_ephemeral(random_block_device::DEFAULT_TEST_CBJOURNAL_SIZE, 0);
        ...
        return mkfs()
            .safe_then(
                [this] { return replay().safe_then([this] { return open().safe_then([this] { return replay(); }); }); })
...
{
    run_async([this] {
        submit_record(record_t{{generate_extent(1), generate_extent(2)}, {generate_delta(3), generate_delta(4)}});
        replay_and_check();
    });
}
...
TEST_F(cbjournal_test_t, submit_three_records)
{
    run_async([this] {
        submit_record(record_t{{generate_extent(1), generate_extent(2)}, {generate_delta(3), generate_delta(4)}});
        submit_record(record_t{{generate_extent(8), generate_extent(9)}, {generate_delta(20), generate_delta(21)}});
        submit_record(record_t{{generate_extent(5), generate_extent(6)}, {generate_delta(200), generate_delta(210)}});
        replay_and_check();
    });
}
...
{
    run_async([this] {
        record_t rec{{generate_extent(1), generate_extent(2)}, {generate_delta(20), generate_delta(21)}};
        ...
        while (is_available_size(record_total_size)) {
            submit_record(record_t{{generate_extent(1), generate_extent(2)}, {generate_delta(20), generate_delta(21)}});
        }

        update_journal_tail(entries.back().get_abs_addr(), record_total_size);
        ASSERT_EQ(get_records_total_size(), get_records_available_size());
...
TEST_F(cbjournal_test_t, replay_after_reset)
{
    run_async([this] {
        ...
        set_written_to(journal_seq_t{0, convert_abs_addr_to_paddr(cbj->get_records_start(), cbj->get_device_id())});
        cbj->close().unsafe_get0();
        replay().unsafe_get0();
        ASSERT_EQ(old_written_to, get_written_to());
        ASSERT_EQ(old_used_size, get_records_used_size());
    });
}
...
TEST_F(cbjournal_test_t, multiple_submit_at_end)
{
    run_async([this] {
        ...
        seastar::parallel_for_each(boost::make_counting_iterator(0u), boost::make_counting_iterator(4u), [&](auto) {
            return seastar::async([&] {
                auto writes = 0;
                while (writes < 2) {
                    record_t rec{{generate_extent(1)}, {generate_delta(20)}};
                    submit_record(std::move(rec));
                    writes++;
                }
            });
        ...
        auto old_written_to = get_written_to();
        cbj->close().unsafe_get0();
        cbj->replay([](const auto& offsets, const auto& e, auto& dirty_seq, auto& alloc_seq, auto last_modified) {
               return Journal::replay_ertr::make_ready_future<bool>(true);
           })
            .unsafe_get0();
