...
template<> struct fmt::formatter<Payload> : fmt::formatter<std::string_view>
{
    template<typename FormatContext> auto format(const Payload& pl, FormatContext& ctx) const
    {
...
class SyntheticDispatcher final : public crimson::net::Dispatcher
{
public:
    ...
    std::optional<seastar::future<>> ms_dispatch(crimson::net::ConnectionRef con, MessageRef m) final
    {
        if (verbose) {
    ...
    void ms_handle_accept(crimson::net::ConnectionRef conn, seastar::shard_id prv_shard, bool is_replace) final
    {
        logger().info("{} - Connection:{}", __func__, *conn);
        assert(prv_shard == seastar::this_shard_id());
    }

    void ms_handle_connect(crimson::net::ConnectionRef conn, seastar::shard_id prv_shard) final
    {
        logger().info("{} - Connection:{}", __func__, *conn);
        assert(prv_shard == seastar::this_shard_id());
    }
    ...
    void ms_handle_remote_reset(crimson::net::ConnectionRef con) final { clear_pending(con); }

    std::optional<seastar::future<>> reply_message(const MessageRef m, crimson::net::ConnectionRef con, Payload& pl)
    {
        pl.who = Payload::PONG;
        bufferlist bl;
        encode(pl, bl);
        ...
        rm->set_data(bl);
        if (verbose) {
            logger().info("{} conn= {} reply i= {}", __func__, *con, pl.seq);
        }
        return con->send(std::move(rm));
    }

    seastar::future<> send_message_wrap(crimson::net::ConnectionRef con, const bufferlist& data)
    {
        ...
        logger().info("{} conn= {} send i= {}", __func__, *con, pl.seq);

        return con->send(std::move(m));
    }

    uint64_t get_num_pending_msgs() { return sent.size(); }

    void clear_pending(crimson::net::ConnectionRef con)
    {
        for (std::deque<uint64_t>::iterator it = conn_sent[&*con].begin(); it != conn_sent[&*con].end(); ++it)
            sent.erase(*it);
        conn_sent.erase(&*con);
    }
    ...
    void print()
    {
        for (auto&& [connptr, list] : conn_sent) {
            if (!list.empty()) {
                logger().info("{} {} wait {}", __func__, (void*)connptr, list.size());
            }
        }
    }
};
...
class SyntheticWorkload
{
    ...
    std::map<crimson::net::ConnectionRef, std::pair<crimson::net::MessengerRef, crimson::net::MessengerRef>>
    ...
    {
        return seastar::do_until([this] { return dispatcher.get_num_pending_msgs() <= max_in_flight; },
...
public:
    ...
    SyntheticWorkload(int servers, int clients, int random_num, crimson::net::SocketPolicy srv_policy,
    ...
    bool can_create_connection() { return available_connections.size() < max_connections; }
    ...
    seastar::future<> maybe_generate_connection()
    {
        ...
        crimson::net::MessengerRef server, client;
        {
            ...
            int index = choose(rng);
            std::set<crimson::net::MessengerRef>::iterator i = available_servers.begin();
            for (; index > 0; --index, ++i)
                ;
            server = *i;
        }
        {
            ...
            int index = choose(rng);
            std::set<crimson::net::MessengerRef>::iterator i = available_clients.begin();
            for (; index > 0; --index, ++i)
                ;
            client = *i;
        }
        ...
        std::pair<crimson::net::MessengerRef, crimson::net::MessengerRef> connected_pair;
        {
            crimson::net::ConnectionRef conn = client->connect(server->get_myaddr(), entity_name_t::TYPE_OSD);
            ...
            return seastar::do_until([&] { return iter == 0; },
                                     [&, this] {
                                         ...
                                         if (val > 90) {
                                             ...
                                             return seastar::sleep(std::chrono::milliseconds(rand() % 1000 + 500));
                                         }
                                     });
        });
    }
            ...
            return seastar::do_until([&] { return iter == 0; },
                                     [&, this] {
                                         ...
                                         if (!(connections_count() % 10)) {
                                             logger().info("seeding connection {}", connections_count());
                                         }
                                         return maybe_generate_connection();
                                     });
        });
    ...
    seastar::future<> init_server(const entity_name_t& name, const std::string& lname, const uint64_t nonce,
                                  const entity_addr_t& addr)
    {
        crimson::net::MessengerRef msgr = crimson::net::Messenger::create(name, lname, nonce, true);
        msgr->set_default_policy(server_policy);
        msgr->set_auth_client(&dummy_auth);
        msgr->set_auth_server(&dummy_auth);
        available_servers.insert(msgr);
        return msgr->bind(entity_addrvec_t{addr})
            .safe_then([this, msgr] { return msgr->start({&dispatcher}); },
    ...
    seastar::future<> init_client(const entity_name_t& name, const std::string& lname, const uint64_t nonce)
    {
        crimson::net::MessengerRef msgr = crimson::net::Messenger::create(name, lname, nonce, true);
    ...
    {
        return get_random_connection().then([this](crimson::net::ConnectionRef conn) {
    ...
    seastar::future<> drop_connection()
    {
        ...
        return get_random_connection().then([this](crimson::net::ConnectionRef conn) {
            ...
            conn->mark_down();
            if (!client_policy.server && client_policy.standby) {
                // it's a lossless policy, so we need to mark down each side
                std::pair<crimson::net::MessengerRef, crimson::net::MessengerRef>& p = available_connections[conn];
                if (!p.first->get_default_policy().server && !p.second->get_default_policy().server) {
                    ...
                    crimson::net::ConnectionRef peer = p.second->connect(p.first->get_myaddr(), p.first->get_mytype());
                    peer->mark_down();
    ...
    {
        int i = 0;
        return seastar::do_until([this] { return !dispatcher.get_num_pending_msgs(); },
                                 [this, &i] {
                                     if (i++ % 50 == 0) {
    ...
    void handle_reset(crimson::net::ConnectionRef con) { available_connections.erase(con); }

    uint64_t servers_count() { return available_servers.size(); }

    uint64_t clients_count() { return available_clients.size(); }

    uint64_t connections_count() { return available_connections.size(); }
};
...
{
    return seastar::when_all_succeed(local_conf().set_val("ms_inject_socket_failures", "0"),
...
seastar::future<> test_stress(thrash_params_t tp)
{
    ...
    SyntheticWorkload test_msg(tp.servers,
                               tp.clients,
                               100,
                               crimson::net::SocketPolicy::stateful_server(0),
                               crimson::net::SocketPolicy::lossless_client(0));

    return seastar::do_with(test_msg, [tp](SyntheticWorkload& test_msg) {
        return seastar::do_until([&test_msg] { return test_msg.servers_count() == test_msg.servers; },
                                 [&test_msg] {
                                     entity_addr_t bind_addr = get_server_addr();
                                     bind_addr.set_type(entity_addr_t::TYPE_MSGR2);
                                     uint64_t server_num = get_nonce();
                                     return test_msg.init_server(
                                         entity_name_t::OSD(server_num), "server", server_num, bind_addr);
                                 })
            .then([&test_msg] {
                return seastar::do_until(
                    [&test_msg] { return test_msg.clients_count() == test_msg.clients; },
                    [&test_msg] { return test_msg.init_client(entity_name_t::CLIENT(-1), "client", get_nonce()); });
            })
            .then([&test_msg, tp] { return test_msg.generate_connections(tp.connections); })
            .then([&test_msg, tp] { return test_msg.random_op(tp.random_op); })
            .then([&test_msg] { return test_msg.wait_for_done(); })
            .then([] { logger().info("test_stress() DONE"); })
            .handle_exception([](auto eptr) {
                logger().error("test_stress() failed: got exception {}", eptr);
                throw;
            });
    });
}
...
seastar::future<> test_injection(thrash_params_t tp)
{
    ...
    return seastar::do_with(test_msg, [tp](SyntheticWorkload& test_msg) {
        return seastar::do_until([&test_msg] { return test_msg.servers_count() == test_msg.servers; },
            ...
            .then([] {
                return seastar::when_all_succeed(local_conf().set_val("ms_inject_socket_failures", "30"),
            ...
            .then([] { return reset_conf(); })
            .handle_exception([](auto eptr) {
                logger().error("test_injection() failed: got exception {}", eptr);
                throw;
            });
    });
}
...
int main(int argc, char** argv)
{
    ...
    return app.run(argc, argv, [&app] { return do_test(app); });
}
