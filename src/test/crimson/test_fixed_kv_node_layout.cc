...
struct test_val_t
{
    ...
    bool operator==(const test_val_t& rhs) const { return rhs.t1 == t1 && rhs.t2 == t2; }
    bool operator!=(const test_val_t& rhs) const { return !(*this == rhs); }
};
...
struct test_val_le_t
{
    ...
    operator test_val_t() const { return test_val_t{t1, t2}; }
};
...
struct test_meta_t
{
    ...
    bool operator==(const test_meta_t& rhs) const { return rhs.t1 == t1 && rhs.t2 == t2; }
    bool operator!=(const test_meta_t& rhs) const { return !(*this == rhs); }

    std::pair<test_meta_t, test_meta_t> split_into(uint32_t pivot) const
    {
        return std::make_pair(test_meta_t{t1, pivot}, test_meta_t{pivot, t2});
    }
    ...
    static std::pair<test_meta_t, test_meta_t> rebalance(const test_meta_t& lhs, const test_meta_t& rhs, uint32_t pivot)
    {
        return std::make_pair(test_meta_t{lhs.t1, pivot}, test_meta_t{pivot, rhs.t2});
    }
};
...
struct test_meta_le_t
{
    ...
    operator test_meta_t() const { return test_meta_t{t1, t2}; }
};

constexpr size_t CAPACITY = 339;

struct TestNode
    : FixedKVNodeLayout<CAPACITY, test_meta_t, test_meta_le_t, uint32_t, ceph_le32, test_val_t, test_val_le_t>
{
    char buf[4096];
    TestNode()
...
TEST(FixedKVNodeTest, merge)
{
    ...
    ASSERT_EQ(node_merged.get_meta(), (test_meta_t{0, std::numeric_limits<uint32_t>::max()}));
...
void run_balance_test(unsigned left, unsigned right, bool prefer_left)
{
    ...
    ASSERT_EQ(node_balanced.get_meta(), (test_meta_t{0, left_size}));
    ASSERT_EQ(node_balanced2.get_meta(), (test_meta_t{left_size, std::numeric_limits<uint32_t>::max()}));
...
void run_replay_test(std::vector<std::function<void(TestNode&, TestNode::delta_buffer_t&)>>&& f)
{
    TestNode node;
