...
struct DummyAuthAuthorizer : public AuthAuthorizer
{
    ...
    {}
    bool verify_reply(bufferlist::const_iterator&, std::string* connection_secret) override { return true; }
    bool add_challenge(CephContext*, const bufferlist&) override { return true; }
};
...
struct Server
{
    ...
    struct ServerDispatcher final : crimson::net::Dispatcher
    {
        ...
        std::optional<seastar::future<>> ms_dispatch(crimson::net::ConnectionRef c, MessageRef m) final
        {
...
public:
    ...
    template<class Func> std::thread with_seastar(Func&& func)
    {
        return std::thread{[this, on_end = on_end.get(), func = std::forward<Func>(func)] {
            // alien: are you ready?
            wait_for_seastar();
    ...
    void run(seastar::app_template& app, int argc, char** argv)
    {
        app.run(argc, argv, [this] {
            ...
            return crimson::common::sharded_conf()
                ...
                .then([this] { return set_seastar_ready(); })
...
static seastar::future<> seastar_echo(const entity_addr_t addr, echo_role role, unsigned count)
{
    std::cout << "seastar/";
    if (role == echo_role::as_server) {
        return seastar::do_with(
            seastar_pingpong::Server{
                crimson::net::Messenger::create(entity_name_t::OSD(0), "server", addr.get_nonce())},
            [addr, count](auto& server) mutable {
                ...
                server.msgr->set_policy_throttler(entity_name_t::TYPE_OSD, &server.byte_throttler);
                server.msgr->set_require_authorizer(false);
                server.msgr->set_auth_client(&server.dummy_auth);
                server.msgr->set_auth_server(&server.dummy_auth);
                return server.msgr->bind(entity_addrvec_t{addr})
                    .safe_then([&server] { return server.msgr->start({&server.dispatcher}); },
                               crimson::net::Messenger::bind_ertr::all_same_way(
                                   [](auto& e) { ceph_abort_msg("bind failed"); }))
                    .then([&dispatcher = server.dispatcher, count] {
                        return dispatcher.on_reply.wait([&dispatcher, count] { return dispatcher.count >= count; });
                    })
                    .finally([&server] {
        ...
        return seastar::do_with(
            seastar_pingpong::Client{
                crimson::net::Messenger::create(entity_name_t::OSD(1), "client", addr.get_nonce())},
...
int main(int argc, char** argv)
{
    ...
    desc.add_options()("help,h", "show help message")(
        "role", po::value<std::string>()->default_value("pong"), "role to play (ping | pong)")(
        "port", po::value<uint16_t>()->default_value(9010), "port #")(
        "nonce", po::value<uint32_t>()->default_value(42), "a unique number to identify the pong server")(
        "count", po::value<unsigned>()->default_value(10), "stop after sending/echoing <count> MPing messages")(
        "v2", po::value<bool>()->default_value(false), "using msgr v2 protocol");
    po::variables_map vm;
    ...
    auto job = sc.with_seastar([&] {
        auto fut = seastar::alien::submit_to(0, [addr, role, count] { return seastar_echo(addr, role, count); });
        fut.wait();
    });
