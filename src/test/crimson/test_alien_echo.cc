    ...
    if (role == echo_role::as_server) {
        return seastar::do_with(
            seastar_pingpong::Server{
                crimson::net::Messenger::create(entity_name_t::OSD(0), "server", addr.get_nonce(), true)},
        ...
        return seastar::do_with(
            seastar_pingpong::Client{
                crimson::net::Messenger::create(entity_name_t::OSD(1), "client", addr.get_nonce(), true)},
            [addr, count](auto& client) {
                ...
                return client.msgr->start({&client.dispatcher})
                    .then([addr, &client, &disp = client.dispatcher, count] {
                        ...
                        return seastar::do_until([&disp, count] { return disp.count >= count; },
                                                 [&disp, conn] {
...
int main(int argc, char** argv)
{
    ...
    auto job = sc.with_seastar([&] {
        auto fut =
            seastar::alien::submit_to(app.alien(), 0, [addr, role, count] { return seastar_echo(addr, role, count); });
        fut.wait();
    });
