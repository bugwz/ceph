...
static seastar::future<> test_echo(unsigned rounds, double keepalive_ratio, bool v2)
{
    struct test_state
    {
        ...
        struct Client final : public crimson::net::Dispatcher
        {
            ...
            seastar::future<> dispatch_pingpong(const entity_addr_t& peer_addr)
            {
                ...
                return seastar::futurize_invoke([this, conn] { return do_dispatch_pingpong(conn); })
                    .then([this, conn, start_time] {
                        ...
                        logger().info(
                            "{}: handshake {}, pingpong {}", *conn, dur_handshake.count(), dur_pingpong.count());
                    });
            }
                               ...
                               [this, conn, &count_ping, &count_keepalive] {
                                   return seastar::repeat([this, conn, &count_ping, &count_keepalive] {
                                       if (keepalive_dist(rng)) {
                                           return conn->keepalive().then([&count_keepalive] {
                                               count_keepalive += 1;
                                           ...
                                           return conn->send(make_message<MPing>()).then([&count_ping] {
                                               count_ping += 1;
                                               return seastar::make_ready_future<seastar::stop_iteration>(
    ...
    logger().info("test_echo(rounds={}, keepalive_ratio={}, v2={}):", rounds, keepalive_ratio, v2);
    ...
    return seastar::when_all_succeed(server1->init(entity_name_t::OSD(0), "server1", 1, addr1),
        ...
        .then_unpack([] { return seastar::now(); })
        .then([client1] {
...
static seastar::future<> test_concurrent_dispatch(bool v2)
{
        ...
        struct Server final : public crimson::net::Dispatcher
        {
            ...
            std::optional<seastar::future<>> ms_dispatch(crimson::net::ConnectionRef, MessageRef m) override
            {
                ...
                case 1:
                    ...
                    break;
                case 2: on_second.set_value(); break;
                default: throw std::runtime_error("unexpected count");
                }
                return {seastar::now()};
            }
    ...
    return seastar::when_all_succeed(server->init(entity_name_t::OSD(4), "server3", 5, addr),
                                     client->init(entity_name_t::OSD(5), "client3", 6))
        .then_unpack([server, client] {
            auto conn = client->msgr->connect(server->msgr->get_myaddr(), entity_name_t::TYPE_OSD);
            // send two messages
            return conn->send(make_message<MPing>()).then([conn] { return conn->send(make_message<MPing>()); });
        })
        .then([server] { return server->wait(); })
        .then([client] {
        ...
        .then([] { logger().info("test_concurrent_dispatch() done!\n"); })
        ...
        public:
            ...
            void send_pings(const entity_addr_t& addr)
            {
                ...
                (void)seastar::do_until(
                    ...
                    [conn] { return conn->send(make_message<MPing>()).then([] { return seastar::sleep(0ms); }); })
...
class FailoverSuite : public Dispatcher
{
    ...
    std::optional<seastar::future<>> ms_dispatch(ConnectionRef c, MessageRef m) override
    {
        ...
        logger().info("[Test] got op, left {} ops -- [{}] {}", pending_receive, result->index, *c);
        return {seastar::now()};
    }
    ...
    void ms_handle_accept(ConnectionRef conn) override
    {
        ...
        if (tracked_conn && !tracked_conn->is_closed() && tracked_conn != conn) {
...
private:
    seastar::future<> init(entity_addr_t addr, SocketPolicy policy)
    {
        ...
        return test_msgr->bind(entity_addrvec_t{addr})
    ...
    seastar::future<> wait_ready(unsigned num_ready_conns, unsigned num_replaced, bool wait_received)
    {
        ...
        for (auto& result : interceptor.results) {
            if (result.conn->is_closed_clean()) {
                ...
                if (tracked_conn != result.conn || tracked_index != result.index) {
                    throw std::runtime_error(fmt::format("The connected connection [{}] {} doesn't"
        ...
        if (wait_received && (pending_send || pending_peer_receive || pending_receive)) {
...
public:
                                                                  ...
                                                                  const TestInterceptor& interceptor)
    {
        auto suite = std::make_unique<FailoverSuite>(
            Messenger::create(entity_name_t::OSD(2), "Test", 2), test_peer_addr, interceptor);
...
private:
    seastar::future<> prepare_cmd(
        cmd_t cmd, std::function<void(ceph::ref_t<MCommand>)> f_prepare = [](auto m) { return; })
    {
        ...
        f_prepare(m);
        return cmd_conn->send(m).then([fut = std::move(fut)]() mutable { return std::move(fut); });
    }
    ...
    seastar::future<> pingpong()
    {
        ...
        return cmd_conn->send(make_message<MPing>()).then([fut = std::move(fut)]() mutable { return std::move(fut); });
    }
...
public:
    ...
    seastar::future<> shutdown()
    {
        ...
        return cmd_conn->send(m).then([] { return seastar::sleep(200ms); }).then([this] {
            cmd_msgr->stop();
            return cmd_msgr->shutdown();
        });
    }

    static seastar::future<seastar::lw_shared_ptr<FailoverTest>> create(entity_addr_t cmd_peer_addr,
                                                                        entity_addr_t test_addr)
    {
        ...
        auto test = seastar::make_lw_shared<FailoverTest>(
            Messenger::create(entity_name_t::OSD(1), "CmdCli", 1), test_addr, test_peer_addr);
...
public:
                                ...
                                policy_t peer_policy, std::function<seastar::future<>(FailoverSuite&)>&& f)
    {
        ...
        return FailoverSuite::create(test_addr, test_policy_, test_peer_addr, interceptor)
...
class FailoverSuitePeer : public Dispatcher
{
    ...
    void ms_handle_accept(ConnectionRef conn) override
    {
        ...
        ceph_assert(!tracked_conn || tracked_conn->is_closed() || tracked_conn == conn);
        tracked_conn = conn;
        std::ignore = flush_pending_send();
    }
...
private:
    seastar::future<> init(entity_addr_t addr, SocketPolicy policy)
    {
        ...
        return peer_msgr->bind(entity_addrvec_t{addr})
...
public:
    ...
    static seastar::future<std::unique_ptr<FailoverSuitePeer>> create(entity_addr_t addr, const SocketPolicy& policy,
                                                                      cb_t op_callback)
    {
        auto suite =
            std::make_unique<FailoverSuitePeer>(Messenger::create(entity_name_t::OSD(4), "TestPeer", 4), op_callback);
        return suite->init(addr, policy).then([suite = std::move(suite)]() mutable { return std::move(suite); });
    }
};
...
class FailoverTestPeer : public Dispatcher
{
    ...
    std::optional<seastar::future<>> ms_dispatch(ConnectionRef c, MessageRef m) override
    {
        ...
        switch (m->get_type()) {
        case CEPH_MSG_PING: std::ignore = c->send(make_message<MPing>()); break;
        case MSG_COMMAND:
        {
            auto m_cmd = boost::static_pointer_cast<MCommand>(m);
            ...
            if (cmd == cmd_t::shutdown) {
                ...
                std::ignore = handle_cmd(cmd, m_cmd).then([c] { return c->send(make_message<MCommandReply>()); });
            }
            break;
        }
    ...
    void ms_handle_accept(ConnectionRef conn) override { cmd_conn = conn; }
...
public:
    ...
    static seastar::future<std::unique_ptr<FailoverTestPeer>> create(entity_addr_t cmd_peer_addr)
    {
        ...
        test_peer_addr.set_port(cmd_peer_addr.get_port() + 1);
        auto test_peer =
            std::make_unique<FailoverTestPeer>(Messenger::create(entity_name_t::OSD(3), "CmdSrv", 3), test_peer_addr);
...
seastar::future<bool> peer_wins(FailoverTest& test)
{
...
seastar::future<> test_v2_racing_reconnect_win(FailoverTest& test)
{
    return seastar::do_with(
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                ...
                return test.run_suite(fmt::format("test_v2_racing_reconnect_win -- {}({})", bp.second, bp.first),
                                      interceptor,
                                      policy_t::lossless_peer,
                                      policy_t::lossless_peer,
                                      [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_racing_reconnect_lose(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<std::pair<unsigned, Breakpoint>>{
            {2, {custom_bp_t::BANNER_WRITE}},
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                ...
                interceptor.make_block(bp.second, bp.first);
                return test.run_suite(fmt::format("test_v2_racing_reconnect_lose -- {}({})", bp.second, bp.first),
                                      interceptor,
                                      policy_t::lossless_peer,
                                      policy_t::lossless_peer,
                                      [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_racing_connect_win(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
            {custom_bp_t::BANNER_WRITE},
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                ...
                return test.run_suite(fmt::format("test_v2_racing_connect_win -- {}", bp),
                                      interceptor,
                                      policy_t::lossless_peer,
                                      policy_t::lossless_peer,
                                      [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_racing_connect_lose(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
            {custom_bp_t::BANNER_WRITE},
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                TestInterceptor interceptor;
                interceptor.make_block(bp);
                return test.run_suite(fmt::format("test_v2_racing_connect_lose -- {}", bp),
                                      interceptor,
                                      policy_t::lossless_peer,
                                      policy_t::lossless_peer,
                                      [&test](FailoverSuite& suite) {
    ...
    return test.run_suite("test_v2_peer_reuse_connector",
                          ...
                          [&test](FailoverSuite& suite) {
                              return seastar::futurize_invoke([&suite] {
                                  ...
                                  .then([] { return seastar::sleep(100ms); })
                                  .then([&suite] {
    ...
    return test.run_suite("test_v2_peer_reuse_acceptor",
                          ...
                          [&test](FailoverSuite& suite) {
                              return seastar::futurize_invoke([&test] {
                                  ...
                                  .then([&suite] { return suite.markdown(); })
                                  .then([&suite] { return suite.wait_results(2); })
                                  .then([](ConnResults& results) {
                                      results[0].assert_state_at(conn_state_t::closed);
                                      results[0].assert_connect(0, 0, 0, 0);
...
seastar::future<> test_v2_protocol(entity_addr_t test_addr, entity_addr_t test_peer_addr, bool test_peer_islocal)
{
    ...
    if (test_peer_islocal) {
        ...
        return FailoverTestPeer::create(test_peer_addr)
            .then([test_addr, test_peer_addr](auto peer) {
                return test_v2_protocol(test_addr, test_peer_addr, false).then([peer = std::move(peer)]() mutable {
    ...
    return FailoverTest::create(test_peer_addr, test_addr)
        .then([](auto test) {
            return seastar::futurize_invoke([test] { return test_v2_lossy_early_connect_fault(*test); })
                ...
                .then([test] { return peer_wins(*test); })
                .then([test](bool peer_wins) {
                    if (peer_wins) {
                        return seastar::futurize_invoke([test] { return test_v2_racing_connect_lose(*test); })
                            .then([test] { return test_v2_racing_reconnect_lose(*test); });
                    }
                    else {
                        return seastar::futurize_invoke([test] { return test_v2_racing_connect_win(*test); })
                            .then([test] { return test_v2_racing_reconnect_win(*test); });
                    }
                })
...
seastar::future<int> do_test(seastar::app_template& app)
{
    ...
    return crimson::common::sharded_conf()
        ...
        .then([&app] {
            ...
            ceph_assert(v2_testpeer_addr.parse(config["v2-testpeer-addr"].as<std::string>().c_str(), nullptr));
            auto v2_testpeer_islocal = config["v2-testpeer-islocal"].as<bool>();
            return test_echo(rounds, keepalive_ratio, false)
                .then([rounds, keepalive_ratio] { return test_echo(rounds, keepalive_ratio, true); })
                .then([] { return test_concurrent_dispatch(false); })
                .then([] { return test_concurrent_dispatch(true); })
                .then([] { return test_preemptive_shutdown(false); })
                .then([] { return test_preemptive_shutdown(true); })
...
int main(int argc, char** argv)
{
    ...
    app.add_options()("verbose,v", bpo::value<bool>()->default_value(false), "chatty if true")(
        ...
        "v2-test-addr", bpo::value<std::string>()->default_value("v2:127.0.0.1:9012"), "address of v2 failover tests")(
        "v2-testpeer-addr",
        bpo::value<std::string>()->default_value("v2:127.0.0.1:9013"),
