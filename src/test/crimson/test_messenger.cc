...
template<typename T, typename... Args> seastar::future<T*> create_sharded(Args... args)
{
    ...
    return seastar::smp::submit_to(
               ...
               [=] {
                   ...
                   return sharded_obj->start(args...).then([sharded_obj] {
                       seastar::engine().at_exit([sharded_obj] { return sharded_obj->stop().then([sharded_obj] {}); });
                       return sharded_obj.get();
                   });
               })
        .then([](seastar::sharded<T>* ptr_shard) { return &ptr_shard->local(); });
}

class ShardedGates : public seastar::peering_sharded_service<ShardedGates>
{
public:
    ShardedGates() = default;
    ~ShardedGates() { assert(gate.is_closed()); }

    template<typename Func> void dispatch_in_background(const char* what, Func&& f)
    {
        std::ignore = seastar::with_gate(container().local().gate, std::forward<Func>(f))
    ...
    {
        return container().invoke_on_all([](auto& local) { return local.gate.close(); });
    }

    static seastar::future<ShardedGates*> create() { return create_sharded<ShardedGates>(); }
...
static seastar::future<> test_echo(unsigned rounds, double keepalive_ratio)
{
    struct test_state
    {
        struct Server final : public crimson::net::Dispatcher
        {
            ShardedGates& gates;
            ...
            void ms_handle_accept(crimson::net::ConnectionRef conn, seastar::shard_id prv_shard,
            ...
            std::optional<seastar::future<>> ms_dispatch(crimson::net::ConnectionRef c, MessageRef m) override
            {
                if (verbose) {
                    logger().info("server got {}", *m);
                }
                // reply with a pong
                gates.dispatch_in_background("echo_send_pong", [c] { return c->send(crimson::make_message<MPing>()); });
                return {seastar::now()};
            }

            seastar::future<> init(const entity_name_t& name, const std::string& lname, const uint64_t nonce,
                                   const entity_addr_t& addr)
            {
                msgr = crimson::net::Messenger::create(name, lname, nonce, false);
                msgr->set_default_policy(crimson::net::SocketPolicy::stateless_server(0));
                msgr->set_auth_client(&dummy_auth);
                msgr->set_auth_server(&dummy_auth);
                return msgr->bind(entity_addrvec_t{addr})
                    .safe_then([this] { return msgr->start({this}); },
                               crimson::net::Messenger::bind_ertr::all_same_way([addr](const std::error_code& e) {
        ...
        class Client final : public crimson::net::Dispatcher, public seastar::peering_sharded_service<Client>
        {
        public:
            Client(seastar::shard_id primary_sid, unsigned rounds, double keepalive_ratio, ShardedGates* gates)
            ...
            seastar::future<> dispatch_pingpong(const entity_addr_t& peer_addr)
            {
                ...
                return seastar::futurize_invoke([this, conn] { return do_dispatch_pingpong(conn); })
                    .then([] {
                    ...
                    .then([this, conn, start_time] {
                        return container()
                            .invoke_on(conn->get_shard_id(),
                                       [pconn = &*conn, start_time](auto& local) {
                                           assert(pconn->is_connected());
                                           auto session = local.find_session(pconn);
                                           std::chrono::duration<double> dur_handshake =
                                               session->connected_time - start_time;
                                           std::chrono::duration<double> dur_pingpong =
                                               session->finish_time - session->connected_time;
            ...
            static seastar::future<Client*> create(unsigned rounds, double keepalive_ratio, ShardedGates* gates)
            {
                return create_sharded<Client>(seastar::this_shard_id(), rounds, keepalive_ratio, gates);
            }
        ...
        private:
            ...
            void ms_handle_connect(crimson::net::ConnectionRef conn, seastar::shard_id prv_shard) override
            {
            ...
            seastar::future<> do_dispatch_pingpong(crimson::net::ConnectionRef conn)
            {
                ...
                std::ignore = i;
                ceph_assert(added);
                return seastar::do_with(0u, 0u, [this, conn](auto& count_ping, auto& count_keepalive) {
                               ...
                               [this, conn, &count_ping, &count_keepalive] {
                                   return seastar::repeat([this, conn, &count_ping, &count_keepalive] {
                                       if (keepalive_dist(rng)) {
                                           return conn->send_keepalive().then([&count_keepalive] {
                                               count_keepalive += 1;
                                           ...
                                           return conn->send(crimson::make_message<MPing>()).then([&count_ping] {
                                               count_ping += 1;
                                               return seastar::make_ready_future<seastar::stop_iteration>(
    ...
    logger().info("test_echo(rounds={}, keepalive_ratio={}):", rounds, keepalive_ratio);
    return ShardedGates::create()
        .then([rounds, keepalive_ratio](auto* gates) {
            return seastar::when_all_succeed(test_state::Client::create(rounds, keepalive_ratio, gates),
        ...
        .then_unpack([](auto* client1, auto* client2, auto* gates) {
            ...
            return seastar::when_all_succeed(server1->init(entity_name_t::OSD(0), "server1", 1, addr1),
                ...
                .then([] { logger().info("test_echo() done!\n"); })
                .handle_exception([](auto eptr) {
                    logger().error("test_echo() failed: got exception {}", eptr);
                    throw;
                })
                .finally([gates, server1, server2] { return gates->close(); });
        });
}
...
seastar::future<> test_preemptive_shutdown()
{
    struct test_state
    {
        class Server final : public crimson::net::Dispatcher
        {
            crimson::net::MessengerRef msgr;
            crimson::auth::DummyAuthClientServer dummy_auth;
        ...
        public:
            seastar::future<> init(const entity_name_t& name, const std::string& lname, const uint64_t nonce,
                                   const entity_addr_t& addr)
            {
                msgr = crimson::net::Messenger::create(name, lname, nonce, true);
                msgr->set_default_policy(crimson::net::SocketPolicy::stateless_server(0));
                msgr->set_auth_client(&dummy_auth);
                msgr->set_auth_server(&dummy_auth);
                return msgr->bind(entity_addrvec_t{addr})
            ...
            entity_addr_t get_addr() const { return msgr->get_myaddr(); }
            seastar::future<> shutdown()
            {
                msgr->stop();
                return msgr->shutdown();
        ...
        class Client final : public crimson::net::Dispatcher
        {
            crimson::net::MessengerRef msgr;
            crimson::auth::DummyAuthClientServer dummy_auth;
            ...
            std::optional<seastar::future<>> ms_dispatch(crimson::net::ConnectionRef, MessageRef m) override
            {
                return {seastar::now()};
            }
        ...
        public:
            ...
            void send_pings(const entity_addr_t& addr)
            {
                ...
                (void)seastar::do_until(
                    ...
                    [conn] {
                        return conn->send(crimson::make_message<MPing>()).then([] { return seastar::sleep(0ms); });
                    })
                    .then_wrapped([this, conn](auto fut) { fut.forward_to(std::move(stopped_send_promise)); });
            }
            seastar::future<> shutdown()
            {
                msgr->stop();
    ...
    return seastar::when_all_succeed(server->init(entity_name_t::OSD(6), "server4", 7, addr),
        ...
        .then([] { logger().info("test_preemptive_shutdown() done!\n"); })
...
{
    switch (state) {
    case conn_state_t::unknown: return out << "unknown";
    case conn_state_t::established: return out << "established";
    case conn_state_t::closed: return out << "closed";
    case conn_state_t::replaced: return out << "replaced";
    default: ceph_abort();
    }
}
...
template<> struct fmt::formatter<conn_state_t> : fmt::ostream_formatter
{};
#endif
...
struct ConnResult
{
    ...
    template<typename T> void _assert_eq(const char* expr_actual, T actual, const char* expr_expected, T expected) const
    {
        if (actual != expected) {
            throw std::runtime_error(fmt::format("[{}] {} '{}' is actually {}, not the expected '{}' {}",
                                                 index,
                                                 *conn,
                                                 expr_actual,
                                                 actual,
...
#define ASSERT_EQUAL(actual, expected) _assert_eq(#actual, actual, #expected, expected)

    void assert_state_at(conn_state_t expected) const { ASSERT_EQUAL(state, expected); }

    void assert_connect(unsigned attempts, unsigned connects, unsigned reconnects, unsigned dispatched) const
    {
    ...
    void assert_connect(unsigned attempts, unsigned dispatched) const
    {
        ASSERT_EQUAL(connect_attempts, attempts);
        ASSERT_EQUAL(cnt_connect_dispatched, dispatched);
    }

    void assert_accept(unsigned attempts, unsigned accepts, unsigned reaccepts, unsigned dispatched) const
    {
    ...
    void assert_accept(unsigned attempts, unsigned dispatched) const
    {
        ASSERT_EQUAL(accept_attempts, attempts);
        ASSERT_EQUAL(cnt_accept_dispatched, dispatched);
    }
...
struct TestInterceptor : public Interceptor
{
    ...
    seastar::future<> wait()
    {
        ...
        return seastar::sleep_abortable(10s, *signal)
            .then([] { throw std::runtime_error("Timeout (10s) in TestInterceptor::wait()"); })
...
private:
    void register_conn(ConnectionRef conn) override
    {
        ...
        if (result != nullptr) {
            logger().error(
                "The connection [{}] {} already exists when register {}", result->index, *result->conn, *conn);
            ceph_abort();
        }
    ...
    seastar::future<bp_action_t> intercept(Connection& _conn, std::vector<Breakpoint> bps) override
    {
        assert(bps.size() >= 1);
        ...
        return seastar::smp::submit_to(primary_sid, [conn, bps, this] {
            ...
            for (const Breakpoint& bp : bps) {
                ...
                logger().info("[{}] {} intercepted {}({})", result->index, *conn, bp, breakpoints_counter[bp].counter);
...
{
    switch (policy) {
    case policy_t::stateful_server: return SocketPolicy::stateful_server(0);
    case policy_t::stateless_server: return SocketPolicy::stateless_server(0);
    case policy_t::lossless_peer: return SocketPolicy::lossless_peer(0);
    case policy_t::lossless_peer_reuse: return SocketPolicy::lossless_peer_reuse(0);
    case policy_t::lossy_client: return SocketPolicy::lossy_client(0);
    case policy_t::lossless_client: return SocketPolicy::lossless_client(0);
    default: logger().error("unexpected policy type"); ceph_abort();
    }
}
...
class FailoverSuite : public Dispatcher
{
    ...
    std::optional<seastar::future<>> ms_dispatch(ConnectionRef conn_ref, MessageRef m) override
    {
        ceph_assert(m->get_type() == CEPH_MSG_OSD_OP);
        Connection* conn = &*conn_ref;
        gates.dispatch_in_background("TestSuite_ms_dispatch", [this, conn, conn_ref] {
            return seastar::smp::submit_to(
                       primary_sid,
                       [this, conn] {
                           ...
                           logger().info(
                               "[Test] got op, left {} ops -- [{}] {}", pending_receive, result->index, *conn);
                       })
                .then([conn_ref] {});
        });
        return {seastar::now()};
    ...
    void ms_handle_accept(ConnectionRef conn_ref, seastar::shard_id prv_shard, bool is_replace) override
    {
        Connection* conn = &*conn_ref;
        gates.dispatch_in_background("TestSuite_ms_dispatch", [this, conn, conn_ref] {
            return seastar::smp::submit_to(
                       ...
                       [this, conn] {
                           ...
                           if (tracked_conn && !tracked_conn->is_protocol_closed() && tracked_conn != &*conn) {
    ...
    void ms_handle_connect(ConnectionRef conn_ref, seastar::shard_id prv_shard) override
    {
        Connection* conn = &*conn_ref;
        gates.dispatch_in_background("TestSuite_ms_dispatch", [this, conn, conn_ref] {
            return seastar::smp::submit_to(
    ...
    void ms_handle_reset(ConnectionRef conn_ref, bool is_replace) override
    {
        Connection* conn = &*conn_ref;
        gates.dispatch_in_background("TestSuite_ms_dispatch", [this, conn, conn_ref] {
            return seastar::smp::submit_to(
    ...
    void ms_handle_remote_reset(ConnectionRef conn_ref) override
    {
        Connection* conn = &*conn_ref;
        gates.dispatch_in_background("TestSuite_ms_dispatch", [this, conn, conn_ref] {
            return seastar::smp::submit_to(
...
private:
    seastar::future<> init(entity_addr_t test_addr, SocketPolicy policy)
    {
        ...
        return test_msgr->bind(entity_addrvec_t{test_addr})
            .safe_then([this] { return test_msgr->start({this}); },
    ...
    seastar::future<> send_op(bool expect_reply = true)
    {
        ...
        hobject_t hobj(object_t(), oloc.key, CEPH_NOSNAP, pgid.ps(), pgid.pool(), oloc.nspace);
        spg_t spgid(pgid);
    ...
    seastar::future<> flush_pending_send()
    {
        ...
        ceph_assert(tracked_conn);
        ceph_assert(!tracked_conn->is_protocol_closed());
        return seastar::do_until([this] { return pending_send == 0; },
                                 [this] {
                                     --pending_send;
                                     return send_op();
                                 });
    ...
    seastar::future<> wait_ready(unsigned num_ready_conns, unsigned num_replaced, bool wait_received)
    {
        assert(seastar::this_shard_id() == primary_sid);
        ...
        if (num_ready_conns > 0) {
            if (interceptor.results.size() > num_ready_conns) {
                throw std::runtime_error(
                    fmt::format("{} connections, more than expected: {}", interceptor.results.size(), num_ready_conns));
            }
        ...
        if (num_replaced > 0) {
            if (replaced_conns > num_replaced) {
                throw std::runtime_error(
                    fmt::format("{} replaced connections, more than expected: {}", replaced_conns, num_replaced));
            }
            if (replaced_conns < num_replaced) {
        ...
        if (do_wait) {
            return interceptor.wait().then([this, num_ready_conns, num_replaced, wait_received] {
...
public:
    FailoverSuite(MessengerRef test_msgr, entity_addr_t test_peer_addr, const TestInterceptor& interceptor,
                  ShardedGates& gates)
        : test_msgr(test_msgr)
    ...
    entity_addr_t get_addr() const { return test_msgr->get_myaddr(); }
    ...
    void needs_receive() { ++pending_receive; }

    void notify_peer_reply()
    {
        ceph_assert(pending_peer_receive > 0);
        --pending_peer_receive;
        logger().info("[Test] TestPeer said got op, left {} ops", pending_peer_receive);
        ...
        for (auto& kv : interceptor.breakpoints) {
            ...
            if (expected > it->second.counter) {
                throw std::runtime_error(fmt::format(
                    "{} only triggered {} times, not the expected {}", kv.first, it->second.counter, expected));
            }
        }
    }
    ...
    static seastar::future<std::unique_ptr<FailoverSuite>> create(entity_addr_t test_addr, SocketPolicy test_policy,
                                                                  entity_addr_t test_peer_addr,
                                                                  const TestInterceptor& interceptor,
                                                                  ShardedGates& gates)
    {
        auto suite =
            std::make_unique<FailoverSuite>(Messenger::create(entity_name_t::OSD(TEST_OSD), "Test", TEST_NONCE, false),
                                            test_peer_addr,
                                            interceptor,
                                            gates);
        return suite->init(test_addr, test_policy).then([suite = std::move(suite)]() mutable {
...
public:
    ...
    seastar::future<> markdown()
    {
        ...
        return seastar::sleep(50ms)
            .then([this] {
                return seastar::smp::submit_to(tracked_conn->get_shard_id(), [tracked_conn = tracked_conn] {
    ...
    seastar::future<std::reference_wrapper<ConnResults>> wait_results(unsigned count)
    {
...
class FailoverTest : public Dispatcher
{
    ...
    std::optional<seastar::future<>> ms_dispatch(ConnectionRef c, MessageRef m) override
    {
        ...
        default: logger().error("{} got unexpected msg from cmd server: {}", *c, *m); ceph_abort();
        }
        return {seastar::now()};
    }
...
private:
    seastar::future<> prepare_cmd(
        cmd_t cmd, std::function<void(MCommand&)> f_prepare = [](auto& m) { return; })
    {
        ...
        f_prepare(*m);
        return cmd_conn->send(std::move(m)).then([fut = std::move(fut)]() mutable { return std::move(fut); });
    }

    seastar::future<> start_peer(policy_t peer_policy)
    {
        return prepare_cmd(cmd_t::suite_start,
                           [peer_policy](auto& m) { m.cmd.emplace_back(1, static_cast<char>(peer_policy)); });
    }

    seastar::future<> stop_peer() { return prepare_cmd(cmd_t::suite_stop); }
    ...
    seastar::future<> pingpong()
    {
        ...
        return cmd_conn->send(crimson::make_message<MPing>()).then([fut = std::move(fut)]() mutable {
            return std::move(fut);
        });
    }
...
public:
    FailoverTest(MessengerRef cmd_msgr, entity_addr_t test_addr, entity_addr_t test_peer_addr)
        : cmd_msgr(cmd_msgr)
        , test_addr(test_addr)
        , test_peer_addr(test_peer_addr)
    {}
    ...
    seastar::future<> shutdown()
    {
        ...
        return cmd_conn->send(std::move(m)).then([] { return seastar::sleep(200ms); }).then([this] {
            cmd_msgr->stop();
            return cmd_msgr->shutdown();
        });
    }

    static seastar::future<seastar::lw_shared_ptr<FailoverTest>> create(entity_addr_t test_addr,
                                                                        entity_addr_t cmd_peer_addr,
                                                                        entity_addr_t test_peer_addr)
    {
        auto test = seastar::make_lw_shared<FailoverTest>(
            Messenger::create(entity_name_t::OSD(CMD_CLI_OSD), "CmdCli", CMD_CLI_NONCE, true),
            test_addr,
            test_peer_addr);
    ...
    // called by tests
public:
    seastar::future<> run_suite(std::string name, const TestInterceptor& interceptor, policy_t test_policy,
                                policy_t peer_policy, std::function<seastar::future<>(FailoverSuite&)>&& f)
    {
        logger().info("\n\n[{}]", name);
        ceph_assert(!test_suite);
        SocketPolicy test_policy_ = to_socket_policy(test_policy);
        return ShardedGates::create().then(
            [this, test_policy_, peer_policy, interceptor, f = std::move(f)](auto* gates) mutable {
                return FailoverSuite::create(test_addr, test_policy_, test_peer_addr, interceptor, *gates)
                    .then([this, peer_policy, f = std::move(f)](auto suite) mutable {
                        ...
                        return start_peer(peer_policy)
                            .then([this, f = std::move(f)] { return f(*test_suite); })
                            .then([this] {
                            ...
                            .then([this] { return stop_peer(); })
                            .then([this] { return test_suite->shutdown().then([this] { test_suite.reset(); }); });
                    })
                    .then([gates] { return gates->close(); });
            });
    }
    ...
    seastar::future<> peer_connect_me()
    {
        ...
        return prepare_cmd(cmd_t::suite_connect_me,
                           [this](auto& m) { m.cmd.emplace_back(fmt::format("{}", test_addr)); });
    }
    ...
    {
        ceph_assert(test_suite);
        return test_suite->send_peer().then([this] { return peer_send_me(); });
    }
    ...
    seastar::future<> markdown_peer()
    {
        ...
        // sleep to propagate potential remaining acks
        return seastar::sleep(50ms).then([this] { return prepare_cmd(cmd_t::suite_markdown); }).then([] {
...
class FailoverSuitePeer : public Dispatcher
{
    ...
    void ms_handle_accept(ConnectionRef conn, seastar::shard_id prv_shard, bool is_replace) override
    {
        assert(prv_shard == seastar::this_shard_id());
        ...
        if (tracked_conn && !tracked_conn->is_protocol_closed() && tracked_conn != conn) {
            logger().error(
                "[TestPeer] {} got accepted, but there's already a valid traced_conn {}", *conn, *tracked_conn);
        }
        tracked_conn = conn;
...
private:
    seastar::future<> init(entity_addr_t test_peer_addr, SocketPolicy policy)
    {
        ...
        return peer_msgr->bind(entity_addrvec_t{test_peer_addr})
            .safe_then([this] { return peer_msgr->start({this}); },
    ...
    seastar::future<> send_op()
    {
        ceph_assert(tracked_conn);
        if (tracked_conn->is_protocol_closed()) {
            logger().error("[TestPeer] send op but the connection is closed -- {}", *tracked_conn);
        }
...
public:
    ...
    static seastar::future<std::unique_ptr<FailoverSuitePeer>> create(entity_addr_t test_peer_addr,
                                                                      const SocketPolicy& policy, cb_t op_callback)
    {
        auto suite = std::make_unique<FailoverSuitePeer>(
            Messenger::create(entity_name_t::OSD(TEST_PEER_OSD), "TestPeer", TEST_PEER_NONCE, true), op_callback);
        return suite->init(test_peer_addr, policy).then([suite = std::move(suite)]() mutable {
            return std::move(suite);
        });
    }
};
...
class FailoverTestPeer : public Dispatcher
{
    ...
    std::optional<seastar::future<>> ms_dispatch(ConnectionRef c, MessageRef m) override
    {
        ...
        switch (m->get_type()) {
        case CEPH_MSG_PING: std::ignore = c->send(crimson::make_message<MPing>()); break;
        case MSG_COMMAND:
        {
            auto m_cmd = boost::static_pointer_cast<MCommand>(m);
            ...
            if (cmd == cmd_t::shutdown) {
                ...
                std::ignore =
                    handle_cmd(cmd, m_cmd).then([c] { return c->send(crimson::make_message<MCommandReply>()); });
            }
            break;
        }
        default: logger().error("{} got unexpected msg from cmd client: {}", *c, *m); ceph_abort();
        }
        return {seastar::now()};
    }
...
private:
    ...
    seastar::future<> handle_cmd(cmd_t cmd, MRef<MCommand> m_cmd)
    {
        ...
        case cmd_t::suite_start:
        {
            ...
            return FailoverSuitePeer::create(test_peer_addr, policy, [this] { return notify_recv_op(); })
                .then([this](auto suite) { test_suite.swap(suite); });
        }
        case cmd_t::suite_stop:
            ceph_assert(test_suite);
            return test_suite->shutdown().then([this] { test_suite.reset(); });
        ...
        case cmd_t::suite_send_me: ceph_assert(test_suite); return test_suite->send_peer();
        case cmd_t::suite_keepalive_me: ceph_assert(test_suite); return test_suite->keepalive_peer();
        case cmd_t::suite_markdown: ceph_assert(test_suite); return test_suite->markdown();
        default:
            logger().error("TestPeer got unexpected command {} from Test", fmt::ptr(m_cmd.get()));
            ceph_abort();
            return seastar::now();
        }
    }
    ...
    seastar::future<> init(entity_addr_t cmd_peer_addr)
    {
        ...
        cmd_msgr->set_auth_client(&dummy_auth);
        cmd_msgr->set_auth_server(&dummy_auth);
        return cmd_msgr->bind(entity_addrvec_t{cmd_peer_addr})
            .safe_then([this] { return cmd_msgr->start({this}); },
...
public:
    FailoverTestPeer(MessengerRef cmd_msgr, entity_addr_t test_peer_addr)
        : cmd_msgr(cmd_msgr)
        , test_peer_addr(test_peer_addr)
    {}

    seastar::future<> wait() { return cmd_msgr->wait(); }

    static seastar::future<std::unique_ptr<FailoverTestPeer>> create(entity_addr_t cmd_peer_addr,
                                                                     entity_addr_t test_peer_addr)
    {
        auto test_peer = std::make_unique<FailoverTestPeer>(
            Messenger::create(entity_name_t::OSD(CMD_SRV_OSD), "CmdSrv", CMD_SRV_NONCE, true), test_peer_addr);
        return test_peer->init(cmd_peer_addr).then([test_peer = std::move(test_peer)]() mutable {
...
seastar::future<> test_v2_lossy_early_connect_fault(FailoverTest& test)
{
    return seastar::do_with(
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                ...
                return test.run_suite(fmt::format("test_v2_lossy_early_connect_fault -- {}", bp),
                                      interceptor,
                                      policy_t::lossy_client,
                                      policy_t::stateless_server,
                                      [](FailoverSuite& suite) {
                                          return seastar::futurize_invoke([&suite] { return suite.send_peer(); })
                                              .then([&suite] { return suite.connect_peer(); })
                                              .then([&suite] { return suite.wait_results(1); })
...
seastar::future<> test_v2_lossy_connect_fault(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                TestInterceptor interceptor;
                interceptor.make_fault(bp);
                return test.run_suite(fmt::format("test_v2_lossy_connect_fault -- {}", bp),
                                      interceptor,
                                      policy_t::lossy_client,
                                      policy_t::stateless_server,
                                      [](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossy_connected_fault(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                TestInterceptor interceptor;
                interceptor.make_fault(bp);
                return test.run_suite(fmt::format("test_v2_lossy_connected_fault -- {}", bp),
                                      interceptor,
                                      policy_t::lossy_client,
                                      policy_t::stateless_server,
                                      [&test](FailoverSuite& suite) {
                                          return seastar::futurize_invoke([&test] { return test.send_bidirectional(); })
                                              .then([&suite] { return suite.connect_peer(); })
                                              .then([&suite] { return suite.wait_results(1); })
                                              .then([](ConnResults& results) {
...
seastar::future<> test_v2_lossy_early_accept_fault(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
            {custom_bp_t::BANNER_WRITE},
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                TestInterceptor interceptor;
                interceptor.make_fault(bp);
                return test.run_suite(fmt::format("test_v2_lossy_early_accept_fault -- {}", bp),
                                      interceptor,
                                      policy_t::stateless_server,
                                      policy_t::lossy_client,
                                      [&test](FailoverSuite& suite) {
                                          return seastar::futurize_invoke([&test] { return test.peer_send_me(); })
                                              .then([&test] { return test.peer_connect_me(); })
                                              .then([&suite] { return suite.wait_results(2); })
                                              .then([](ConnResults& results) {
                                                  results[0].assert_state_at(conn_state_t::closed);
...
seastar::future<> test_v2_lossy_accept_fault(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::CLIENT_IDENT, bp_type_t::READ};
    TestInterceptor interceptor;
    interceptor.make_fault(bp);
    return test.run_suite(fmt::format("test_v2_lossy_accept_fault -- {}", bp),
                          interceptor,
                          policy_t::stateless_server,
                          policy_t::lossy_client,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossy_establishing_fault(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::SERVER_IDENT, bp_type_t::WRITE};
    TestInterceptor interceptor;
    interceptor.make_fault(bp);
    return test.run_suite(fmt::format("test_v2_lossy_establishing_fault -- {}", bp),
                          interceptor,
                          policy_t::stateless_server,
                          policy_t::lossy_client,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossy_accepted_fault(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
            {Tag::MESSAGE, bp_type_t::WRITE},
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                TestInterceptor interceptor;
                interceptor.make_fault(bp);
                return test.run_suite(fmt::format("test_v2_lossy_accepted_fault -- {}", bp),
                                      interceptor,
                                      policy_t::stateless_server,
                                      policy_t::lossy_client,
                                      [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossless_connect_fault(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
            {Tag::CLIENT_IDENT, bp_type_t::WRITE},
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                TestInterceptor interceptor;
                interceptor.make_fault(bp);
                return test.run_suite(fmt::format("test_v2_lossless_connect_fault -- {}", bp),
                                      interceptor,
...
seastar::future<> test_v2_lossless_connected_fault(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
            {Tag::MESSAGE, bp_type_t::WRITE},
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                TestInterceptor interceptor;
                interceptor.make_fault(bp);
                return test.run_suite(fmt::format("test_v2_lossless_connected_fault -- {}", bp),
                                      interceptor,
                                      policy_t::lossless_client,
                                      policy_t::stateful_server,
                                      [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossless_connected_fault2(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                TestInterceptor interceptor;
                interceptor.make_fault(bp);
                return test.run_suite(fmt::format("test_v2_lossless_connected_fault2 -- {}", bp),
                                      interceptor,
                                      policy_t::lossless_client,
                                      policy_t::stateful_server,
                                      [&test](FailoverSuite& suite) {
                                          return seastar::futurize_invoke([&suite] { return suite.connect_peer(); })
                                              .then([&suite] { return suite.wait_established(); })
                                              .then([&suite] { return suite.send_peer(); })
                                              .then([&suite] { return suite.keepalive_peer(); })
                                              .then([&suite] { return suite.wait_established(); })
                                              .then([&test] { return test.peer_send_me(); })
                                              .then([&test] { return test.peer_keepalive_me(); })
                                              .then([&suite] { return suite.wait_established(); })
                                              .then([&suite] { return suite.send_peer(); })
                                              .then([&suite] { return suite.wait_established(); })
                                              .then([&test] { return test.peer_send_me(); })
...
seastar::future<> test_v2_lossless_reconnect_fault(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<std::pair<Breakpoint, Breakpoint>>{
            {{Tag::MESSAGE, bp_type_t::WRITE}, {Tag::SESSION_RECONNECT, bp_type_t::WRITE}},
            {{Tag::MESSAGE, bp_type_t::WRITE}, {Tag::SESSION_RECONNECT_OK, bp_type_t::READ}},
        },
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp_pair) {
                ...
                return test.run_suite(
                    fmt::format("test_v2_lossless_reconnect_fault -- {}, {}", bp_pair.first, bp_pair.second),
                    interceptor,
                    policy_t::lossless_client,
                    policy_t::stateful_server,
                    [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossless_accept_fault(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::CLIENT_IDENT, bp_type_t::READ};
    TestInterceptor interceptor;
    interceptor.make_fault(bp);
    return test.run_suite(fmt::format("test_v2_lossless_accept_fault -- {}", bp),
                          interceptor,
                          policy_t::stateful_server,
                          policy_t::lossless_client,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossless_establishing_fault(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::SERVER_IDENT, bp_type_t::WRITE};
    TestInterceptor interceptor;
    interceptor.make_fault(bp);
    return test.run_suite(fmt::format("test_v2_lossless_establishing_fault -- {}", bp),
                          interceptor,
                          policy_t::stateful_server,
                          policy_t::lossless_client,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossless_accepted_fault(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
            {Tag::MESSAGE, bp_type_t::WRITE},
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                TestInterceptor interceptor;
                interceptor.make_fault(bp);
                return test.run_suite(fmt::format("test_v2_lossless_accepted_fault -- {}", bp),
                                      interceptor,
                                      policy_t::stateful_server,
                                      policy_t::lossless_client,
                                      [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossless_reaccept_fault(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<std::pair<Breakpoint, Breakpoint>>{
            {{Tag::MESSAGE, bp_type_t::READ}, {Tag::SESSION_RECONNECT, bp_type_t::READ}},
            {{Tag::MESSAGE, bp_type_t::READ}, {Tag::SESSION_RECONNECT_OK, bp_type_t::WRITE}},
        },
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp_pair) {
                TestInterceptor interceptor;
                ...
                interceptor.make_fault(bp_pair.second);
                return test.run_suite(
                    fmt::format("test_v2_lossless_reaccept_fault -- {}, {}", bp_pair.first, bp_pair.second),
                    interceptor,
                    policy_t::stateful_server,
                    policy_t::lossless_client,
                    [&test, bp = bp_pair.second](FailoverSuite& suite) {
                        return seastar::futurize_invoke([&test] { return test.send_bidirectional(); })
                            .then([&test] { return test.peer_connect_me(); })
                            .then([&suite] { return suite.wait_results(3); })
...
seastar::future<> test_v2_peer_connect_fault(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
            {Tag::CLIENT_IDENT, bp_type_t::WRITE},
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                TestInterceptor interceptor;
                interceptor.make_fault(bp);
                return test.run_suite(fmt::format("test_v2_peer_connect_fault -- {}", bp),
                                      interceptor,
...
seastar::future<> test_v2_peer_accept_fault(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::CLIENT_IDENT, bp_type_t::READ};
    TestInterceptor interceptor;
    interceptor.make_fault(bp);
    return test.run_suite(fmt::format("test_v2_peer_accept_fault -- {}", bp),
                          interceptor,
                          policy_t::lossless_peer,
                          policy_t::lossless_peer,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_peer_establishing_fault(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::SERVER_IDENT, bp_type_t::WRITE};
    TestInterceptor interceptor;
    interceptor.make_fault(bp);
    return test.run_suite(fmt::format("test_v2_peer_establishing_fault -- {}", bp),
                          interceptor,
                          policy_t::lossless_peer,
                          policy_t::lossless_peer,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_peer_connected_fault_reconnect(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::MESSAGE, bp_type_t::WRITE};
    TestInterceptor interceptor;
    interceptor.make_fault(bp);
    return test.run_suite(fmt::format("test_v2_peer_connected_fault_reconnect -- {}", bp),
                          interceptor,
                          policy_t::lossless_peer,
                          policy_t::lossless_peer,
                          [](FailoverSuite& suite) {
...
seastar::future<> test_v2_peer_connected_fault_reaccept(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::MESSAGE, bp_type_t::READ};
    TestInterceptor interceptor;
    interceptor.make_fault(bp);
    return test.run_suite(fmt::format("test_v2_peer_connected_fault_reaccept -- {}", bp),
                          interceptor,
                          policy_t::lossless_peer,
                          policy_t::lossless_peer,
                          [&test](FailoverSuite& suite) {
...
seastar::future<bool> check_peer_wins(FailoverTest& test)
{
...
seastar::future<> test_v2_racing_reconnect_acceptor_lose(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<std::pair<unsigned, Breakpoint>>{
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                ...
                return test.run_suite(
                    fmt::format("test_v2_racing_reconnect_acceptor_lose -- {}({})", bp.second, bp.first),
                    interceptor,
                    policy_t::lossless_peer,
                    policy_t::lossless_peer,
                    [&test](FailoverSuite& suite) {
                        return seastar::futurize_invoke([&test] { return test.peer_send_me(); })
                            .then([&test] { return test.peer_connect_me(); })
                            .then([&suite] { return suite.wait_blocked(); })
                            .then([&suite] { return suite.send_peer(); })
                            .then([&suite] { return suite.wait_established(); })
                            .then([&suite] {
                                suite.unblock();
...
seastar::future<> test_v2_racing_reconnect_acceptor_win(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<std::pair<unsigned, Breakpoint>>{
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                ...
                interceptor.make_block(bp.second, bp.first);
                return test.run_suite(
                    fmt::format("test_v2_racing_reconnect_acceptor_win -- {}({})", bp.second, bp.first),
                    interceptor,
                    policy_t::lossless_peer,
                    policy_t::lossless_peer,
                    [&test](FailoverSuite& suite) {
                        return seastar::futurize_invoke([&suite] { return suite.send_peer(); })
                            ...
                            .then([&suite] { return suite.wait_blocked(); })
                            .then([&test] { return test.peer_send_me(); })
                            .then([&suite] { return suite.wait_replaced(1); })
                            .then([&suite] {
                                suite.unblock();
                                return suite.wait_results(2);
                            })
...
seastar::future<> test_v2_racing_connect_acceptor_lose(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
            {custom_bp_t::BANNER_WRITE},
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                ...
                return test.run_suite(fmt::format("test_v2_racing_connect_acceptor_lose -- {}", bp),
                                      interceptor,
                                      policy_t::lossless_peer,
                                      policy_t::lossless_peer,
                                      [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_racing_connect_acceptor_win(FailoverTest& test)
{
    return seastar::do_with(
        std::vector<Breakpoint>{
            {custom_bp_t::SOCKET_CONNECTING},
        ...
        [&test](auto& failure_cases) {
            return seastar::do_for_each(failure_cases, [&test](auto bp) {
                ...
                // block connector
                interceptor.make_block(bp);
                return test.run_suite(fmt::format("test_v2_racing_connect_acceptor_win -- {}", bp),
                                      interceptor,
                                      policy_t::lossless_peer,
                                      policy_t::lossless_peer,
                                      [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_racing_connect_reconnect_lose(FailoverTest& test)
{
    TestInterceptor interceptor;
...
seastar::future<> test_v2_racing_connect_reconnect_win(FailoverTest& test)
{
    TestInterceptor interceptor;
    interceptor.make_fault({Tag::SERVER_IDENT, bp_type_t::READ});
...
seastar::future<> test_v2_stale_connect(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::SERVER_IDENT, bp_type_t::READ};
    TestInterceptor interceptor;
    interceptor.make_stall(bp);
    return test.run_suite(fmt::format("test_v2_stale_connect -- {}", bp),
                          interceptor,
                          policy_t::lossless_peer,
                          policy_t::lossless_peer,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_stale_reconnect(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::SESSION_RECONNECT_OK, bp_type_t::READ};
    TestInterceptor interceptor;
    interceptor.make_fault({Tag::MESSAGE, bp_type_t::WRITE});
    interceptor.make_stall(bp);
    return test.run_suite(fmt::format("test_v2_stale_reconnect -- {}", bp),
                          interceptor,
                          policy_t::lossless_peer,
                          policy_t::lossless_peer,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_stale_accept(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::CLIENT_IDENT, bp_type_t::READ};
    TestInterceptor interceptor;
    interceptor.make_stall(bp);
    return test.run_suite(fmt::format("test_v2_stale_accept -- {}", bp),
                          interceptor,
                          policy_t::lossless_peer,
                          policy_t::lossless_peer,
                          [&test](FailoverSuite& suite) {
                              return seastar::futurize_invoke([&test] { return test.peer_connect_me(); })
                                  .then([&suite] { return suite.wait_blocked(); })
                                  .then([&test] { return test.peer_send_me(); })
                                  .then([&suite] { return suite.wait_established(); })
                                  .then([&suite] {
...
seastar::future<> test_v2_stale_establishing(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::SERVER_IDENT, bp_type_t::WRITE};
    TestInterceptor interceptor;
    interceptor.make_stall(bp);
    return test.run_suite(fmt::format("test_v2_stale_establishing -- {}", bp),
                          interceptor,
                          policy_t::lossless_peer,
                          policy_t::lossless_peer,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_stale_reaccept(FailoverTest& test)
{
    auto bp = Breakpoint{Tag::SESSION_RECONNECT_OK, bp_type_t::WRITE};
    TestInterceptor interceptor;
    interceptor.make_fault({Tag::MESSAGE, bp_type_t::READ});
    interceptor.make_stall(bp);
    return test.run_suite(fmt::format("test_v2_stale_reaccept -- {}", bp),
                          interceptor,
                          policy_t::lossless_peer,
                          policy_t::lossless_peer,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossy_client(FailoverTest& test)
{
    return test.run_suite("test_v2_lossy_client",
                          TestInterceptor(),
                          policy_t::lossy_client,
                          policy_t::stateless_server,
                          [&test](FailoverSuite& suite) {
                              return seastar::futurize_invoke([&suite] {
                                  ...
                                  .then([&test] { return test.send_bidirectional(); })
                                  .then([&suite] { return suite.wait_results(1); })
                                  .then([](ConnResults& results) {
                                      results[0].assert_state_at(conn_state_t::established);
                                      results[0].assert_connect(1, 1, 0, 1);
...
seastar::future<> test_v2_stateless_server(FailoverTest& test)
{
    return test.run_suite("test_v2_stateless_server",
                          TestInterceptor(),
                          policy_t::stateless_server,
                          policy_t::lossy_client,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossless_client(FailoverTest& test)
{
    return test.run_suite("test_v2_lossless_client",
                          TestInterceptor(),
                          policy_t::lossless_client,
                          policy_t::stateful_server,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_stateful_server(FailoverTest& test)
{
    return test.run_suite("test_v2_stateful_server",
                          TestInterceptor(),
                          policy_t::stateful_server,
                          policy_t::lossless_client,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_peer_reuse_connector(FailoverTest& test)
{
    return test.run_suite("test_v2_peer_reuse_connector",
                          TestInterceptor(),
                          ...
                          [&test](FailoverSuite& suite) {
                              return seastar::futurize_invoke([&suite] {
                                  ...
                                  .then([&suite] { return suite.try_send_peer(); })
                                  .then([&suite] { return suite.wait_results(2); })
                                  .then([](ConnResults& results) {
                                      results[0].assert_state_at(conn_state_t::closed);
                                      results[0].assert_connect(1, 1, 0, 1);
...
seastar::future<> test_v2_peer_reuse_acceptor(FailoverTest& test)
{
    return test.run_suite("test_v2_peer_reuse_acceptor",
                          TestInterceptor(),
                          policy_t::lossless_peer_reuse,
                          policy_t::lossless_peer_reuse,
                          [&test](FailoverSuite& suite) {
                              return seastar::futurize_invoke([&test] {
                                  ...
                                  .then([&test] { return test.try_peer_send_me(); })
                                  .then([&suite] { return suite.wait_results(3); })
                                  .then([](ConnResults& results) {
                                      results[0].assert_state_at(conn_state_t::closed);
                                      results[0].assert_connect(0, 0, 0, 0);
...
seastar::future<> test_v2_lossless_peer_connector(FailoverTest& test)
{
    return test.run_suite("test_v2_lossless_peer_connector",
                          TestInterceptor(),
                          policy_t::lossless_peer,
                          policy_t::lossless_peer,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_lossless_peer_acceptor(FailoverTest& test)
{
    return test.run_suite("test_v2_lossless_peer_acceptor",
                          TestInterceptor(),
                          policy_t::lossless_peer,
                          policy_t::lossless_peer,
                          [&test](FailoverSuite& suite) {
...
seastar::future<> test_v2_protocol(entity_addr_t test_addr, entity_addr_t cmd_peer_addr, entity_addr_t test_peer_addr,
                                   bool test_peer_islocal, bool peer_wins)
{
    ...
    if (test_peer_islocal) {
        ...
        return FailoverTestPeer::create(cmd_peer_addr, test_peer_addr)
            .then([test_addr, cmd_peer_addr, test_peer_addr, peer_wins](auto peer) {
                return test_v2_protocol(test_addr, cmd_peer_addr, test_peer_addr, false, peer_wins)
                    .then(
                        [peer = std::move(peer)]() mutable { return peer->wait().then([peer = std::move(peer)] {}); });
            })
            .handle_exception([](auto eptr) {
    ...
    return FailoverTest::create(test_addr, cmd_peer_addr, test_peer_addr)
        .then([peer_wins](auto test) {
            return seastar::futurize_invoke([test] { return test_v2_lossy_early_connect_fault(*test); })
                .then([test] { return test_v2_lossy_connect_fault(*test); })
                .then([test] { return test_v2_lossy_connected_fault(*test); })
                .then([test] { return test_v2_lossy_early_accept_fault(*test); })
                .then([test] { return test_v2_lossy_accept_fault(*test); })
                .then([test] { return test_v2_lossy_establishing_fault(*test); })
                .then([test] { return test_v2_lossy_accepted_fault(*test); })
                .then([test] { return test_v2_lossless_connect_fault(*test); })
                .then([test] { return test_v2_lossless_connected_fault(*test); })
                .then([test] { return test_v2_lossless_connected_fault2(*test); })
                .then([test] { return test_v2_lossless_reconnect_fault(*test); })
                .then([test] { return test_v2_lossless_accept_fault(*test); })
                .then([test] { return test_v2_lossless_establishing_fault(*test); })
                .then([test] { return test_v2_lossless_accepted_fault(*test); })
                .then([test] { return test_v2_lossless_reaccept_fault(*test); })
                .then([test] { return test_v2_peer_connect_fault(*test); })
                .then([test] { return test_v2_peer_accept_fault(*test); })
                .then([test] { return test_v2_peer_establishing_fault(*test); })
                .then([test] { return test_v2_peer_connected_fault_reconnect(*test); })
                .then([test] { return test_v2_peer_connected_fault_reaccept(*test); })
                .then([test] { return check_peer_wins(*test); })
                .then([test, peer_wins](bool ret_peer_wins) {
                    ceph_assert(peer_wins == ret_peer_wins);
                    if (ret_peer_wins) {
                        return seastar::futurize_invoke([test] { return test_v2_racing_connect_acceptor_win(*test); })
                            .then([test] { return test_v2_racing_reconnect_acceptor_win(*test); });
                    }
                    else {
                        return seastar::futurize_invoke([test] { return test_v2_racing_connect_acceptor_lose(*test); })
                            .then([test] { return test_v2_racing_reconnect_acceptor_lose(*test); });
                    }
                })
                .then([test] { return test_v2_racing_connect_reconnect_win(*test); })
                .then([test] { return test_v2_racing_connect_reconnect_lose(*test); })
                .then([test] { return test_v2_stale_connect(*test); })
                .then([test] { return test_v2_stale_reconnect(*test); })
                .then([test] { return test_v2_stale_accept(*test); })
                .then([test] { return test_v2_stale_establishing(*test); })
                .then([test] { return test_v2_stale_reaccept(*test); })
                .then([test] { return test_v2_lossy_client(*test); })
                .then([test] { return test_v2_stateless_server(*test); })
                .then([test] { return test_v2_lossless_client(*test); })
                .then([test] { return test_v2_stateful_server(*test); })
                .then([test] { return test_v2_peer_reuse_connector(*test); })
                .then([test] { return test_v2_peer_reuse_acceptor(*test); })
                .then([test] { return test_v2_lossless_peer_connector(*test); })
                .then([test] { return test_v2_lossless_peer_acceptor(*test); })
                .then([test] { return test->shutdown().then([test] {}); });
        })
        .handle_exception([](auto eptr) {
...
seastar::future<int> do_test(seastar::app_template& app)
{
    ...
    auto init_params = ceph_argparse_early_args(args, CEPH_ENTITY_TYPE_CLIENT, &cluster, &conf_file_list);
    return crimson::common::sharded_conf()
        .start(init_params.name, cluster)
        .then([] { return local_conf().start(); })
        .then([conf_file_list] { return local_conf().parse_config_files(conf_file_list); })
        .then([&app] {
            ...
            ceph_assert(test_addr.parse(config["test-addr"].as<std::string>().c_str(), nullptr));
            test_addr.set_nonce(TEST_NONCE);

            entity_addr_t cmd_peer_addr;
            ceph_assert(cmd_peer_addr.parse(config["testpeer-addr"].as<std::string>().c_str(), nullptr));
            ...
            return test_echo(rounds, keepalive_ratio)
                .then([] { return test_preemptive_shutdown(); })
                .then([test_addr, cmd_peer_addr, test_peer_addr, testpeer_islocal, peer_wins] {
                    return test_v2_protocol(test_addr, cmd_peer_addr, test_peer_addr, testpeer_islocal, peer_wins);
                })
                .then([] {
        ...
        .then([] { return crimson::common::sharded_conf().stop(); })
        .then([] { return 0; })
        .handle_exception([](auto eptr) {
...
int main(int argc, char** argv)
{
    ...
    app.add_options()("verbose,v", bpo::value<bool>()->default_value(false), "chatty if true")(
        "rounds", bpo::value<unsigned>()->default_value(512), "number of pingpong rounds")(
        "keepalive-ratio", bpo::value<double>()->default_value(0.1), "ratio of keepalive in ping messages")(
        "test-addr", bpo::value<std::string>()->default_value("v2:127.0.0.1:9014"), "address of v2 failover tests")(
        "testpeer-addr",
        bpo::value<std::string>()->default_value("v2:127.0.0.1:9012"),
