...
class ConfigObs : public ceph::md_config_obs_impl<Config>
{
    ...
    void handle_conf_change(const Config& conf, const std::set<std::string>& changes) override
    {
...
public:
    ConfigObs() { crimson::common::local_conf().add_observer(this); }
...
static seastar::future<> test_config()
{
    return crimson::common::sharded_conf()
        ...
        .then([] { return crimson::common::sharded_conf().invoke_on(0, &Config::start); })
        .then([] { return sharded_cobs.start(); })
        .then([] {
            auto& conf = crimson::common::local_conf();
        ...
        .finally([] { return sharded_cobs.stop(); })
...
int main(int argc, char** argv)
{
    ...
    return app.run(argc, argv, [&] {
        return test_config()
            .then([] { std::cout << "All tests succeeded" << std::endl; })
