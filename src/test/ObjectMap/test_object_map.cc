...
public:
    ...
    string val_from_key(const string& object, const string& key) { return object + "_" + key + "_" + num_str(seq++); }

    void set_key(const string& objname, const string& key, const string& value)
    {
        set_key(ghobject_t(hobject_t(sobject_t(objname, CEPH_NOSNAP))), key, value);
    }

    void set_xattr(const string& objname, const string& key, const string& value)
    {
        set_xattr(ghobject_t(hobject_t(sobject_t(objname, CEPH_NOSNAP))), key, value);
    }
    ...
    {
        set_header(ghobject_t(hobject_t(sobject_t(objname, CEPH_NOSNAP))), value);
    }
    ...
    {
        return get_header(ghobject_t(hobject_t(sobject_t(objname, CEPH_NOSNAP))), value);
    }
    ...
    {
        return get_xattr(ghobject_t(hobject_t(sobject_t(objname, CEPH_NOSNAP))), key, value);
    }
    ...
    int get_xattr(ghobject_t hoid, string key, string* value)
    {
        ...
        if (!got.empty()) {
            *value = string(got.begin()->second.c_str(), got.begin()->second.length());
            return 1;
        }
        else {
            return 0;
    ...
    {
        return get_key(ghobject_t(hobject_t(sobject_t(objname, CEPH_NOSNAP))), key, value);
    }
    ...
    {
        remove_keys(ghobject_t(hobject_t(sobject_t(objname, CEPH_NOSNAP))), to_remove);
    }
    ...
    void remove_keys(ghobject_t hoid, const set<string>& to_remove) { db->rm_keys(hoid, to_remove); }

    void remove_xattr(const string& objname, const string& key)
    {
        remove_xattr(ghobject_t(hobject_t(sobject_t(objname, CEPH_NOSNAP))), key);
    }
    ...
    void clone(ghobject_t hoid, ghobject_t hoid2) { db->clone(hoid, hoid2); }
    ...
    void rename(ghobject_t hoid, ghobject_t hoid2) { db->rename(hoid, hoid2); }

    void clear(const string& objname) { clear(ghobject_t(hobject_t(sobject_t(objname, CEPH_NOSNAP)))); }
    ...
    void legacy_clone(ghobject_t hoid, ghobject_t hoid2) { db->legacy_clone(hoid, hoid2); }

    void clear(ghobject_t hoid) { db->clear(hoid); }

    void clear_omap(const string& objname) { clear_omap(ghobject_t(hobject_t(sobject_t(objname, CEPH_NOSNAP)))); }

    void clear_omap(const ghobject_t& objname) { db->clear_keys_header(objname); }
    ...
    void init_key_set(const set<string>& keys) { key_space = keys; }

    void init_object_name_space(const set<string>& onamespace) { object_name_space = onamespace; }
    ...
    void auto_set_xattr(ostream& out)
    {
        ...
        out << "auto_set_xattr " << *object << ": " << *key << " -> " << value << std::endl;
    }
    ...
    void auto_set_keys(ostream& out)
    {
        ...
        for (unsigned i = 0; i < amount; ++i) {
            set<string>::iterator key = rand_choose(key_space);
            string value = val_from_key(*object, *key);
            out << "auto_set_key " << *object << ": " << *key << " -> " << value << std::endl;
    ...
    {
        if (!xattrs.count(object)) return;
        const map<string, string>& xmap = xattrs.find(object)->second;
        for (map<string, string>::const_iterator i = xmap.begin(); i != xmap.end(); ++i) {
            out->insert(i->first);
        }
    }
    ...
    {
        if (!omap.count(object)) return;
        const map<string, string>& kmap = omap.find(object)->second;
        for (map<string, string>::const_iterator i = kmap.begin(); i != kmap.end(); ++i) {
            out->insert(i->first);
        }
    }
    ...
    void xattrs_off_object(const string& object, set<string>* out)
    {
        *out = key_space;
        ...
        for (set<string>::iterator i = xspace.begin(); i != xspace.end(); ++i) {
            out->erase(*i);
        }
    }
    ...
    void keys_off_object(const string& object, set<string>* out)
    {
        *out = key_space;
        ...
        for (set<string>::iterator i = kspace.begin(); i != kspace.end(); ++i) {
            out->erase(*i);
        }
    }
    ...
    int auto_check_present_xattr(ostream& out)
    {
        ...
        if (!r) {
            out << "auto_check_present_key: failed to find key " << *key << " on object " << *object << std::endl;
            return 0;
        }

        if (result != xattrs[*object][*key]) {
            out << "auto_check_present_key: for key " << *key << " on object " << *object << " found value " << result
                << " where we should have found " << xattrs[*object][*key] << std::endl;
            return 0;
        }
    ...
    int auto_check_present_key(ostream& out)
    {
        ...
        if (result != omap[*object][*key]) {
            out << "auto_check_present_key: for key " << *key << " on object " << *object << " found value " << result
                << " where we should have found " << omap[*object][*key] << std::endl;
            return 0;
        }
    ...
    int auto_check_absent_xattr(ostream& out)
    {
        ...
        int r = get_xattr(*object, *key, &result);
        if (!r) {
            out << "auto_check_absent_key: did not find key " << *key << " on object " << *object << std::endl;
            return 1;
        }

        out << "auto_check_basent_key: for key " << *key << " on object " << *object << " found value " << result
            << " where we should have found nothing" << std::endl;
        return 0;
    }
    ...
    {
        for (auto&& k : to_remove) omap[obj].erase(k);
    ...
    int auto_verify_header(ostream& out)
    {
        ...
        if (!hmap.count(*object)) {
            out << " found header " << header << " should have been empty" << std::endl;
            return 0;
        }
            ...
            out << " found incorrect header " << header << " where we should have found " << hmap[*object] << std::endl;
            return 0;
        }
    }
    ...
    {
        set<string> in_db;
        ObjectMap::ObjectMapIterator iter = db->get_iterator(ghobject_t(hobject_t(sobject_t(obj, CEPH_NOSNAP))));
        ...
        for (auto&& i : omap[obj]) {
            if (!in_db.count(i.first)) {
                out << __func__ << ": obj " << obj << " missing key " << i.first << std::endl;
                err = true;
            }
            else {
                in_db.erase(i.first);
        ...
        if (!in_db.empty()) {
            out << __func__ << ": obj " << obj << " found extra keys " << in_db << std::endl;
            err = true;
        }
        ASSERT_FALSE(err);
    }
...
TEST_F(ObjectMapTest, RandomTest)
{
    ...
    for (unsigned i = 0; i < 5000; ++i) {
        ...
        val <<= 8;
        val %= 100;
        if (!(i % 100)) std::cout << "on op " << i << " val is " << val << std::endl;
