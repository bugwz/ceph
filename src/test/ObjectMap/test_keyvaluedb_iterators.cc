...
public:
    ...
    ::testing::AssertionResult validate_db_clear(KeyValueDB* store)
    {
        ...
        while (it->valid()) {
            ...
            if (mock->db.count(k)) {
                return ::testing::AssertionFailure() << __func__ << " mock store count " << mock->db.count(k) << " key("
                                                     << k.first << "," << k.second << ")";
            }
            it->next();
        }
    ...
    ::testing::AssertionResult validate_db_match()
    {
        ...
        while (it->valid()) {
            pair<string, string> k = it->raw_key();
            if (!mock->db.count(k)) {
                return ::testing::AssertionFailure() << __func__ << " mock db.count() " << mock->db.count(k) << " key("
                                                     << k.first << "," << k.second << ")";
            }
            ...
            if (it_val != mock_val) {
                return ::testing::AssertionFailure() << __func__ << " key(" << k.first << "," << k.second << ")"
    ...
    ::testing::AssertionResult validate_iterator(KeyValueDB::WholeSpaceIterator it, string expected_prefix,
                                                 const string& expected_key, const string& expected_value)
    {
        ...
        if (!it->raw_key_is_prefixed(expected_prefix)) {
            return ::testing::AssertionFailure() << __func__ << " expected raw_key_is_prefixed() == TRUE"
                                                 << " got FALSE";
        }

        if (it->raw_key_is_prefixed("??__SomeUnexpectedValue__??")) {
            return ::testing::AssertionFailure() << __func__ << " expected raw_key_is_prefixed() == FALSE"
                                                 << " got TRUE";
        }
        ...
        if (expected_prefix != key.first) {
            return ::testing::AssertionFailure() << __func__ << " expected prefix '" << expected_prefix << "'"
                                                 << " got prefix '" << key.first << "'";
        }

        if (expected_key != it->key()) {
            return ::testing::AssertionFailure() << __func__ << " expected key '" << expected_key << "'"
                                                 << " got key '" << it->key() << "'";
        }

        if (it->key() != key.second) {
            return ::testing::AssertionFailure() << __func__ << " key '" << it->key() << "'"
                                                 << " does not match"
                                                 << " pair key '" << key.second << "'";
        }

        if (_bl_to_str(it->value()) != expected_value) {
            return ::testing::AssertionFailure() << __func__ << " key '(" << key.first << "," << key.second << ")''"
     ...
     * by _gen_val().
     */
    void validate_prefix(KeyValueDB::WholeSpaceIterator iter, string& prefix, deque<string>& keys)
    {
     ...
     * by _gen_val().
     */
    void validate_prefix_backwards(KeyValueDB::WholeSpaceIterator iter, string& prefix, deque<string>& keys)
    {
    ...
    void print_iterator(KeyValueDB::WholeSpaceIterator iter)
    {
        ...
        while (iter->valid()) {
            ...
            std::cerr << __func__ << " pos " << (++i) << " key (" << k.first << "," << k.second << ")"
...
public:
     ...
     */
    void RmKeysWhileIteratingSnapshot(KeyValueDB* store, KeyValueDB::WholeSpaceIterator iter)
    {
...
public:
     ...
     */
    void SetKeysWhileIterating(KeyValueDB* store, KeyValueDB::WholeSpaceIterator iter)
    {
        iter->seek_to_first();
        ASSERT_TRUE(iter->valid());
        ...
        iter->next();
        ASSERT_TRUE(iter->valid());
        ASSERT_TRUE(validate_iterator(iter, prefix1, "ccc", _bl_to_str(_gen_val("ccc"))));
     ...
     * from the last element to the first, using prev() to move backwards.
     */
    void SetKeysWhileIteratingSnapshot(KeyValueDB* store, KeyValueDB::WholeSpaceIterator iter)
    {
        iter->seek_to_first();
        ASSERT_TRUE(iter->valid());
        ASSERT_TRUE(validate_iterator(iter, prefix1, "aaa", _gen_val_str("aaa")));
     ...
     */
    void UpdateValuesWhileIterating(KeyValueDB* store, KeyValueDB::WholeSpaceIterator iter)
    {
        iter->seek_to_first();
        ASSERT_TRUE(iter->valid());
        ASSERT_TRUE(validate_iterator(iter, prefix1, "aaa", _gen_val_str("aaa")));
     ...
     * version of the iterator.
     */
    void UpdateValuesWhileIteratingSnapshot(KeyValueDB* store, KeyValueDB::WholeSpaceIterator iter)
    {
        iter->seek_to_first();
        ASSERT_TRUE(iter->valid());
        ASSERT_TRUE(validate_iterator(iter, prefix1, "aaa", _gen_val_str("aaa")));
        ...
        ASSERT_TRUE(validate_iterator(tmp_iter, prefix1, "aaa", _gen_val_str("aaa_1")));
        tmp_iter->seek_to_last();
        ASSERT_TRUE(tmp_iter->valid());
        ASSERT_TRUE(validate_iterator(tmp_iter, prefix2, "zzz", _gen_val_str("zzz_1")));
    }
};
...
public:
    ...
    void LowerBoundWithEmptyKeyOnWholeSpaceIterator(KeyValueDB::WholeSpaceIterator iter)
    {
        deque<string> key_deque;
    ...
    void LowerBoundWithEmptyPrefixOnWholeSpaceIterator(KeyValueDB::WholeSpaceIterator iter)
    {
        deque<string> key_deque;
    ...
    void LowerBoundOnWholeSpaceIterator(KeyValueDB::WholeSpaceIterator iter)
    {
        deque<string> key_deque;
    ...
    void UpperBoundWithEmptyKeyOnWholeSpaceIterator(KeyValueDB::WholeSpaceIterator iter)
    {
        deque<string> key_deque;
    ...
    void UpperBoundWithEmptyPrefixOnWholeSpaceIterator(KeyValueDB::WholeSpaceIterator iter)
    {
        deque<string> key_deque;
        // check for an empty prefix, with key 'aaa'. Since this key is shared
        // among two different prefixes, it is relevant to check which will be
    ...
    void UpperBoundOnWholeSpaceIterator(KeyValueDB::WholeSpaceIterator iter)
    {
        deque<string> key_deque;
...
public:
    ...
    void SeekToFirstOnWholeSpaceIterator(KeyValueDB::WholeSpaceIterator iter)
    {
        iter->seek_to_first();
        ASSERT_TRUE(iter->valid());
        deque<string> key_deque;
    ...
    void SeekToFirstWithPrefixOnWholeSpaceIterator(KeyValueDB::WholeSpaceIterator iter)
    {
        deque<string> key_deque;
    ...
    void SeekToLastOnWholeSpaceIterator(KeyValueDB::WholeSpaceIterator iter)
    {
        deque<string> key_deque;
        iter->seek_to_last();
        key_deque.push_back("yyy");
    ...
    void SeekToLastWithPrefixOnWholeSpaceIterator(KeyValueDB::WholeSpaceIterator iter)
    {
        deque<string> key_deque;
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << "[ceph_options] [gtest_options] <store_path>" << std::endl;
        return 1;
    }
