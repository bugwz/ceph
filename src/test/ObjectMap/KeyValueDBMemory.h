...
public:
    ...
    KeyValueDBMemory() {}
    explicit KeyValueDBMemory(KeyValueDBMemory* db)
        : db(db->db)
    {}
    ...
    int get_keys(const std::string& prefix, const std::set<std::string>& key, std::set<std::string>* out);

    int set(const std::string& prefix, const std::string& key, const bufferlist& bl);

    int rmkey(const std::string& prefix, const std::string& key);

    int rmkeys_by_prefix(const std::string& prefix);

    int rm_range_keys(const std::string& prefix, const std::string& start, const std::string& end);
    ...
    public:
        ...
        explicit TransactionImpl_(KeyValueDBMemory* db)
            : db(db)
        {}
        ...
        struct SetOp : public Context
        {
            ...
            bufferlist value;
            SetOp(KeyValueDBMemory* db, const std::pair<std::string, std::string>& key, const bufferlist& value)
                : db(db)
                , key(key)
                , value(value)
            {}
            void finish(int r) override { db->set(key.first, key.second, value); }
        };
        ...
        struct RmKeysOp : public Context
        {
            KeyValueDBMemory* db;
            std::pair<std::string, std::string> key;
            RmKeysOp(KeyValueDBMemory* db, const std::pair<std::string, std::string>& key)
                : db(db)
                , key(key)
            {}
            void finish(int r) override { db->rmkey(key.first, key.second); }
        };
        ...
        struct RmKeysByPrefixOp : public Context
        {
            KeyValueDBMemory* db;
            std::string prefix;
            RmKeysByPrefixOp(KeyValueDBMemory* db, const std::string& prefix)
                : db(db)
                , prefix(prefix)
            {}
            void finish(int r) override { db->rmkeys_by_prefix(prefix); }
        };
        ...
        struct RmRangeKeys : public Context
        {
            ...
            RmRangeKeys(KeyValueDBMemory* db, const std::string& prefix, const std::string& s, const std::string& e)
                ...
                , end(e)
            {}
            void finish(int r) { db->rm_range_keys(prefix, start, end); }
        };
        ...
        int complete()
        {
            for (auto i = on_commit.begin(); i != on_commit.end(); on_commit.erase(i++)) {
                (*i)->complete(0);
            }
            return 0;
        }
    ...
    Transaction get_transaction() override { return Transaction(new TransactionImpl_(this)); }
    ...
    uint64_t get_estimated_size(std::map<std::string, uint64_t>& extras) override
    {
        ...
        for (auto& [key, bl] : db) {
            ...
            total_size += sz;
            if (extras.count(prefix) == 0) extras[prefix] = 0;
            extras[prefix] += sz;
        }
