    ...
    do {                                                                                                       \
        ...
        if (rc != code) {                                                                                      \
            std::cout << "[" << __FILE__ << ":" << __LINE__ << "]"                                             \
                      << " sqlite3 error: " << rc << " `" << sqlite3_errstr(rc) << "': " << sqlite3_errmsg(db) \
                      << std::endl;                                                                            \
...
protected:
    int db_open()
    {
        static const char SQL[] = "PRAGMA journal_mode = PERSIST;"
        ...
        auto&& name = get_uri();
        sqlcatch(
            sqlite3_open_v2(name.c_str(), &db, SQLITE_OPEN_CREATE | SQLITE_OPEN_READWRITE | SQLITE_OPEN_URI, "ceph"));
...
{
    static const char SQL[] = "PRAGMA page_size = 4096;"
...
TEST_F(CephSQLiteTest, InsertExclusiveRate)
{
    ...
    static const char SQL[] = "PRAGMA locking_mode=EXCLUSIVE;"
                              "CREATE TABLE foo (a INT);"
                              "INSERT INTO foo (a) VALUES (RANDOM());";
...
{
    static const char SQL[] = "CREATE TABLE foo (a BLOB);"
                              "WITH RECURSIVE c(x) AS"
                              "  ("
                              "   VALUES(1)"
                              "  UNION ALL"
...
{
    static const char SQL[] = "SELECT json_valid(ceph_perf());";
...
{
    static const char SQL[] = "SELECT json_valid(ceph_status());";
...
{
    static const char SQL[] = "SELECT strftime('%s', 'now');";
...
{
    static const char SQL[] = "SELECT json_extract(ceph_status(), '$.addr');"
