...
int OmapBench::setup(int argc, const char** argv)
{
    ...
    for (unsigned i = 0; i < args.size(); i++) {
        if (i < args.size() - 1) {
            ...
            cout << ")\n"
                 << "	--entries       number of entries per (default " << entries_per_omap;
            cout << ")\n"
...
// Writer functions
Writer::Writer(OmapBench* omap_bench)
    : ob(omap_bench)
{
    stringstream name;
    ob->data_lock.lock();
...
// AioWriter functions
AioWriter::AioWriter(OmapBench* ob)
    : Writer(ob)
{
    aioc = NULL;
}
...
int OmapBench::print_written_omap()
{
    for (int i = 1; i <= objects; i++) {
        ...
        for (set<string>::iterator iter = out_keys.begin(); iter != out_keys.end(); ++iter) {
...
void OmapBench::print_results()
{
    ...
    cout << "ms\nMode latency:\t\t"
         << "between " << data.mode.first * increment;
    cout << " and " << data.mode.first * increment + increment;
    cout << "ms\nTotal latency:\t\t" << data.total_latency;
    cout << "ms" << std::endl;
    cout << std::endl;
    cout << "Histogram:" << std::endl;
    for (int i = floor(data.min_latency / increment); i < ceil(data.max_latency / increment); i++) {
        cout << ">= " << i * increment;
        cout << "ms";
        int spaces;
        if (i == 0)
...
int OmapBench::write_omap_asynchronously(AioWriter* aiow, const std::map<std::string, bufferlist>& omap)
{
...
// Omap Generators
int OmapBench::generate_uniform_omap(const int omap_entries, const int key_size, const int value_size,
...
int OmapBench::generate_non_uniform_omap(const int omap_entries, const int key_size, const int value_size,
                                         std::map<std::string, bufferlist>* out_omap)
{
    bufferlist bl;
...
int OmapBench::generate_small_non_random_omap(const int omap_entries, const int key_size, const int value_size,
                                              std::map<std::string, bufferlist>* out_omap)
{
    bufferlist bl;
    stringstream key;
...
int OmapBench::test_write_objects_in_parallel(omap_generator_t omap_gen)
{
    ...
    for (int i = 0; i < objects; i++) {
        ...
        // perform the write
        busythreads_count++;
        int err = omap_gen(entries_per_omap, key_size, value_size, &this_aio_writer->get_omap());
        if (err < 0) {
            return err;
        }
        err = OmapBench::write_omap_asynchronously(this_aio_writer, (this_aio_writer->get_omap()));
