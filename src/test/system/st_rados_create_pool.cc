...
std::string StRadosCreatePool::get_random_buf(int sz)
{
    ostringstream oss;
...
StRadosCreatePool::StRadosCreatePool(int argc, const char** argv, CrossProcessSem* setup_sem,
                                     CrossProcessSem* pool_setup_sem, CrossProcessSem* close_create_pool,
                                     const std::string& pool_name, int num_objects, const std::string& suffix)
...
out:
    printf("%s: finishing.\n", get_id_str());
    if (m_pool_setup_sem) m_pool_setup_sem->post();
    if (m_close_create_pool) m_close_create_pool->wait();
...
std::string get_temp_pool_name(const char* prefix)
{
    ...
    ceph_assert(!ret);
    char poolname[256];
    ret = snprintf(poolname, sizeof(poolname), "%s.%s-%d", prefix, hostname, getpid());
    ceph_assert(ret > 0);
