...
StRadosListObjects::StRadosListObjects(int argc, const char** argv, const std::string& pool_name,
                                       bool accept_list_errors, int midway_cnt, CrossProcessSem* pool_setup_sem,
                                       CrossProcessSem* midway_sem_wait, CrossProcessSem* midway_sem_post)
...
int StRadosListObjects::run()
{
    ...
    while (true) {
        ...
        if (ret == -ENOENT) {
            ...
            if (m_accept_list_errors && (!m_midway_sem_post || saw > m_midway_cnt)) break;
        ...
        ++saw;
        if (saw == m_midway_cnt) {
            if (m_midway_sem_wait) m_midway_sem_wait->wait();
            if (m_midway_sem_post) m_midway_sem_post->post();
        }
    }
