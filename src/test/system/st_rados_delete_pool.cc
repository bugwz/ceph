...
StRadosDeletePool::StRadosDeletePool(int argc, const char** argv, CrossProcessSem* pool_setup_sem,
                                     CrossProcessSem* delete_pool_sem, const std::string& pool_name)
...
int StRadosDeletePool::run()
{
    ...
    if (m_delete_pool_sem) m_delete_pool_sem->post();
    rados_shutdown(cl);
    return 0;
}
