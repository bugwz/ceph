...
int CrossProcessSem::create(int initial_val, CrossProcessSem** res)
{
    ...
    struct cross_process_sem_data_t* data = static_cast<cross_process_sem_data_t*>(
        mmap(NULL, sizeof(struct cross_process_sem_data_t), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0));
    if (data == MAP_FAILED) {
        int err = errno;
        return err;
    }
    ...
    struct cross_process_sem_data_t* data = (cross_process_sem_data_t*)malloc(sizeof(cross_process_sem_data_t));
...
{
    if (dval < 0) return -EINVAL;
    int cval;
    if (sem_getvalue(&m_data->sem, &cval) == -1) return errno;
    if (cval < dval) {
        int diff = dval - cval;
        for (int i = 0; i < diff; ++i) sem_post(&m_data->sem);
    }
    else {
        int diff = cval - dval;
        for (int i = 0; i < diff; ++i) sem_wait(&m_data->sem);
    }
    return 0;
}
