...
SysTestRunnable::~SysTestRunnable()
{
    set_argv(0, NULL);
}

const char* SysTestRunnable::get_id_str(void) const
{
    return m_id_str;
}

int SysTestRunnable::start()
{
    if (m_started) {
        return -EDOM;
    }
    ...
    if (use_threads) {
        ret = pthread_create(&m_pthread, NULL, systest_runnable_pthread_helper, static_cast<void*>(this));
        if (ret) return ret;
        m_started = true;
    }
    else {
...
std::string SysTestRunnable::join()
{
    if (!m_started) {
...
std::string SysTestRunnable::run_until_finished(std::vector<SysTestRunnable*>& runnables)
{
    int index = 0;
    for (std::vector<SysTestRunnable*>::const_iterator r = runnables.begin(); r != runnables.end(); ++r) {
        int ret = (*r)->start();
        if (ret) {
            ostringstream oss;
            oss << "run_until_finished: got error " << ret << " when starting runnable " << index;
            return oss.str();
        }
        ++index;
    }
    ...
    for (std::vector<SysTestRunnable*>::const_iterator r = runnables.begin(); r != runnables.end(); ++r) {
        ...
        if (!rstr.empty()) {
            ostringstream oss;
            oss << "run_until_finished: runnable " << (*r)->get_id_str() << ": got error: " << rstr;
            return oss.str();
        }
    }
    printf("*******************************\n");
...
void SysTestRunnable::update_id_str(bool started)
{
    bool use_threads = SysTestSettings::inst().use_threads();
...
{
    if (m_argv_orig != NULL) {
        for (int i = 0; i < m_argc; ++i) free((void*)(m_argv_orig[i]));
        delete[] m_argv_orig;
        m_argv_orig = NULL;
        delete[] m_argv;
        m_argv = NULL;
    ...
    for (int i = 0; i < m_argc; ++i) m_argv_orig[i] = strdup(argv[i]);
    m_argv_orig[argc] = NULL;
    m_argv = new const char*[m_argc + 1];
    for (int i = 0; i <= m_argc; ++i) m_argv[i] = m_argv_orig[i];
}
