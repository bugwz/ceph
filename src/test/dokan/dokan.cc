...
bool move_eof(HANDLE handle, LARGE_INTEGER offset)
{
    ...
    if (!SetFilePointerEx(handle, offset, NULL, FILE_BEGIN)) {
        std::cerr << "Setting file pointer failed. err: " << GetLastError() << std::endl;
        return false;
    }
...
int wait_for_mount(std::string mount_path)
{
    ...
    do {
        attempts++;
        if (attempts < MOUNT_POLL_ATTEMPT) Sleep(MOUNT_POLL_INTERVAL_MS);
    } while (!fs::exists(mount_path) && attempts < MOUNT_POLL_ATTEMPT);

    if (!fs::exists(mount_path)) {
        std::cerr << "Timed out waiting for ceph-dokan mount: " << mount_path << std::endl;
        return -ETIMEDOUT;
    }
...
void map_dokan(SubProcess** mount, const char* mountpoint)
{
    ...
    new_mount->add_cmd_args(
        "map", "--win-vol-name", "TestCeph", "--win-vol-serial", TEST_VOL_SERIAL, "-l", mountpoint, NULL);
...
void map_dokan_with_maxpath(SubProcess** mount, const char* mountpoint, uint64_t max_path_len)
{
    SubProcess* new_mount = new SubProcess("ceph-dokan");
    new_mount->add_cmd_args("map",
                            "--debug",
...
int get_volume_max_path(std::string mountpoint)
{
    ...
    if (GetVolumeInformation(mountpoint.c_str(),
                             volume_name,
                             sizeof(volume_name),
                             &serial_number,
                             &max_component_len,
        ...
        std::cerr << "GetVolumeInformation() failed, error: " << GetLastError() << std::endl;
    }
...
{
protected:
    static void SetUpTestSuite() { map_dokan(&shared_mount, DEFAULT_MOUNTPOINT); }
...
TEST_F(DokanTests, test_mount_read_only)
{
    ...
    std::string exception_msg("filesystem error: cannot remove: No such device [" + mountpoint + success_file_path +
                              "]");
    EXPECT_THROW(
        {
            try {
...
TEST_F(DokanTests, test_delete_on_close)
{
    ...
    ASSERT_NE(hFile, INVALID_HANDLE_VALUE) << "Could not open file: " << DEFAULT_MOUNTPOINT "test_create.txt "
...
{
    std::string base_dir_path = DEFAULT_MOUNTPOINT "base_dir_" + get_uuid() + "\\";
    ...
    ASSERT_TRUE(fs::remove((sub_dir_file).c_str())) << "Failed to remove file: " << sub_dir_file;
    ASSERT_FALSE(fs::exists(sub_dir_file));

    // Remove empty dir
    ASSERT_TRUE(fs::remove((sub_dir_path).c_str())) << "Failed to remove directory: " << sub_dir_path;
    ASSERT_FALSE(fs::exists(sub_dir_file));

    ASSERT_NE(fs::remove_all((base_dir_path).c_str()), 0) << "Failed to remove directory: " << base_dir_path;
    ASSERT_FALSE(fs::exists(sub_dir_file));
}
...
{
    std::string dir1_path = DEFAULT_MOUNTPOINT "test_mv_1_" + get_uuid() + "\\";
    std::string dir2_path = DEFAULT_MOUNTPOINT "test_mv_2_" + get_uuid() + "\\";
...
TEST_F(DokanTests, test_max_path)
{
    ...
    map_dokan_with_maxpath(&mount, mountpoint.c_str(), max_path_len);
    EXPECT_EQ(get_volume_max_path(extended_mountpoint), max_path_len);
    ...
    for (int i = 0; i < 15; i++) {
        std::string crt_dir = std::string(dir) + "_" + get_uuid() + "\\";
        long_dir_path.append(crt_dir);
        int stat = _mkdir(long_dir_path.c_str());
        ASSERT_EQ(stat, 0) << "Error creating directory " << i << ": " << GetLastError() << std::endl;
        dir_names[i] = crt_dir;
    }
    std::string file_path = long_dir_path + "\\" + std::string(file) + "_" + get_uuid();
...
{
    std::string file_path = DEFAULT_MOUNTPOINT "test_eof_" + get_uuid();
    HANDLE hFile = CreateFile(file_path.c_str(),
                              GENERIC_WRITE,   // open for writing
                              0,               // sharing mode, none in this case
                              0,               // use default security descriptor
...
{
    std::string file_path = DEFAULT_MOUNTPOINT "test_alloc_size_" + get_uuid();
    HANDLE hFile = CreateFile(file_path.c_str(),
                              GENERIC_WRITE,   // open for writing
                              0,               // sharing mode, none in this case
                              0,               // use default security descriptor
    ...
    ASSERT_NE(SetFileInformationByHandle(hFile, FileAllocationInfo, &fai, sizeof(FILE_ALLOCATION_INFO)), 0)
        << "Error: " << GetLastError();
...
{
    std::string test_dir = DEFAULT_MOUNTPOINT "test_info_" + get_uuid() + "\\";
    std::string file_path = test_dir + "file_" + get_uuid();
    std::string dir_path = test_dir + "dir_" + get_uuid() + "\\";
...
TEST_F(DokanTests, test_volume_info)
{
    ...
    ASSERT_EQ(GetVolumeInformation(DEFAULT_MOUNTPOINT,
                                   volume_name,
                                   sizeof(volume_name),
                                   &serial_number,
                                   &max_component_len,
              ...
              TRUE)
        << "GetVolumeInformation() failed, error: " << GetLastError() << std::endl;

    ASSERT_STREQ(volume_name, "TestCeph") << "Received: " << volume_name << std::endl;
    ASSERT_STREQ(file_system_name, "Ceph") << "Received: " << file_system_name << std::endl;
    ASSERT_EQ(max_component_len, 256);
    ASSERT_EQ(serial_number, std::stoi(TEST_VOL_SERIAL)) << "Received: " << serial_number << std::endl;
...
{
    std::string file1 = DEFAULT_MOUNTPOINT "test_time1_" + get_uuid();
    std::string file2 = DEFAULT_MOUNTPOINT "test_time2_" + get_uuid();
    ...
    int64_t file1_creation = fs::last_write_time(file1).time_since_epoch().count();
    int64_t file2_creation = fs::last_write_time(file2).time_since_epoch().count();
    int64_t file3_creation = fs::last_write_time(file3).time_since_epoch().count();
    ...
    int64_t file1_new_time = fs::last_write_time(file1).time_since_epoch().count();

    EXPECT_EQ((file1_time + 1h).time_since_epoch().count(), file1_new_time);
...
{
    std::string file_path = DEFAULT_MOUNTPOINT "test_disp_" + get_uuid();
    HANDLE hFile = CreateFile(file_path.c_str(),
    ...
    ASSERT_NE(hFile, INVALID_HANDLE_VALUE)
        << "Could not open file: " << file_path << "err: " << GetLastError() << std::endl;

    FILE_DISPOSITION_INFO fdi;
    fdi.DeleteFile = TRUE;   // marking for deletion

    ASSERT_NE(SetFileInformationByHandle(hFile, FileDispositionInfo, &fdi, sizeof(FILE_DISPOSITION_INFO)), 0);
...
TEST_F(DokanTests, test_create_dispositions)
{
    ...
    std::string non_existant_file = DEFAULT_MOUNTPOINT "test_create_" + get_uuid();

    EXPECT_TRUE(check_create_disposition(file_path, CREATE_NEW));

    // CREATE_ALWAYS with existing file
    EXPECT_TRUE(check_create_disposition(file_path, CREATE_ALWAYS));
    EXPECT_EQ(GetLastError(), ERROR_ALREADY_EXISTS);

    // CREATE_NEW with existing file
    EXPECT_FALSE(check_create_disposition(file_path, CREATE_NEW));
    EXPECT_EQ(GetLastError(), ERROR_FILE_EXISTS);

    // OPEN_EXISTING with existing file
    EXPECT_TRUE(check_create_disposition(file_path, OPEN_EXISTING));

    ASSERT_FALSE(fs::exists(non_existant_file));
    // OPEN_EXISTING with non-existant file
    EXPECT_FALSE(check_create_disposition(non_existant_file, OPEN_EXISTING));
    EXPECT_EQ(GetLastError(), ERROR_FILE_NOT_FOUND);

    // OPEN_ALWAYS with existing file
    EXPECT_TRUE(check_create_disposition(file_path, OPEN_ALWAYS));
    EXPECT_EQ(GetLastError(), ERROR_ALREADY_EXISTS);

    ASSERT_FALSE(fs::exists(non_existant_file));
    // OPEN_ALWAYS with non-existant file
    EXPECT_TRUE(check_create_disposition(non_existant_file, OPEN_ALWAYS));
    EXPECT_EQ(GetLastError(), 0);

    ASSERT_TRUE(fs::remove(non_existant_file));

    // TRUNCATE_EXISTING with existing file
    EXPECT_TRUE(check_create_disposition(file_path, TRUNCATE_EXISTING));

    // TRUNCATE_EXISTING with non-existant file
    EXPECT_FALSE(check_create_disposition(non_existant_file, TRUNCATE_EXISTING));
    EXPECT_EQ(GetLastError(), ERROR_FILE_NOT_FOUND);
