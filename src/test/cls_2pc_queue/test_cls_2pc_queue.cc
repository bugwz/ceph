...
TEST_F(TestCls2PCQueue, Cleanup)
{
    ...
    for (const auto& r : all_reservations) {
        if (good_reservations.find(r.first) == good_reservations.end()) {
            ...
            ASSERT_GE(stale_time.time_since_epoch().count(), r.second.timestamp.time_since_epoch().count());
        }
    }
    for (const auto& r : good_reservations) {
        ASSERT_LT(stale_time.time_since_epoch().count(), r.second.timestamp.time_since_epoch().count());
    }
}
...
TEST_F(TestCls2PCQueue, MultiProducer)
{
    ...
    std::thread consumer([this, &queue_name, &consume_count, &producer_count] {
        ...
        while (producer_count > 0 || truncated) {
            const auto ret =
                cls_2pc_queue_list_entries(ioctx, queue_name, marker, max_elements, entries, &truncated, end_marker);
            ASSERT_EQ(0, ret);
...
TEST_F(TestCls2PCQueue, MultiProducerConsumer)
{
    ...
    for (auto& c : consumers) {
        c = std::thread([this, &queue_name, &producer_count] {
            ...
            std::vector<cls_queue_entry> entries;
            while (producer_count > 0 || truncated) {
                const auto ret = cls_2pc_queue_list_entries(
                    ioctx, queue_name, marker, max_elements, entries, &truncated, end_marker);
                ASSERT_EQ(0, ret);
                if (entries.empty()) {
