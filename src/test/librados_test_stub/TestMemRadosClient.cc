...
TestMemRadosClient::TestMemRadosClient(CephContext* cct, TestMemCluster* test_mem_cluster)
...
TestIoCtxImpl* TestMemRadosClient::create_ioctx(int64_t pool_id, const std::string& pool_name)
{
    return new TestMemIoCtxImpl(this, pool_id, pool_name, m_mem_cluster->get_pool(pool_name));
}

void TestMemRadosClient::object_list(int64_t pool_id, std::list<librados::TestRadosClient::Object>* list)
{
    list->clear();
...
int TestMemRadosClient::blocklist_add(const std::string& client_address, uint32_t expire_seconds)
{
    if (is_blocklisted()) {
        return -EBLOCKLISTED;
    }
...
void TestMemRadosClient::transaction_start(const std::string& nspace, const std::string& oid)
{
    m_mem_cluster->transaction_start({nspace, oid});
}

void TestMemRadosClient::transaction_finish(const std::string& nspace, const std::string& oid)
{
