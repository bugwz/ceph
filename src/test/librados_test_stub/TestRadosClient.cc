...
const char* config_keys[] = {"librados_thread_count", NULL};
...
{
public:
    AioFunctionContext(const TestRadosClient::AioFunction& callback, Finisher* finisher, AioCompletionImpl* c)
        : m_callback(callback)
        , m_finisher(finisher)
        , m_comp(c)
    {
    ...
    void finish(int r) override
    {
        ...
        if (m_comp != NULL) {
            if (m_finisher != NULL) {
                m_finisher->queue(new LambdaContext(std::bind(&finish_aio_completion, m_comp, ret)));
            }
            else {
...
TestRadosClient::TestRadosClient(CephContext* cct, TestWatchNotify* watch_notify)
    : m_cct(cct->get())
    , m_watch_notify(watch_notify)
    ...
    , m_io_context_pool(std::make_unique<ceph::async::io_context_pool>())
{
    ...
    m_io_context_pool->start(m_cct->_conf.get_val<uint64_t>("librados_thread_count"));
}
...
void TestRadosClient::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    if (changed.count("librados_thread_count")) {
        m_io_context_pool->stop();
        m_io_context_pool->start(conf.get_val<std::uint64_t>("librados_thread_count"));
    }
}
...
void TestRadosClient::add_aio_operation(const std::string& oid, bool queue_callback, const AioFunction& aio_function,
                                        AioCompletionImpl* c)
{
    AioFunctionContext* ctx = new AioFunctionContext(aio_function, queue_callback ? m_aio_finisher : NULL, c);
...
struct WaitForFlush
{
    ...
    {
        if (--count == 0) {
            aio_finisher->queue(new LambdaContext(std::bind(&finish_aio_completion, c, 0)));
            delete this;
        }
        return 0;
    }
...
void TestRadosClient::flush_aio_operations(AioCompletionImpl* c)
{
    ...
    for (size_t i = 0; i < m_finishers.size(); ++i) {
        AioFunctionContext* ctx =
            new AioFunctionContext(std::bind(&WaitForFlush::flushed, wait_for_flush), nullptr, nullptr);
...
int TestRadosClient::aio_watch_flush(AioCompletionImpl* c)
{
    c->get();
    Context* ctx =
        new LambdaContext(std::bind(&TestRadosClient::finish_aio_completion, this, c, std::placeholders::_1));
