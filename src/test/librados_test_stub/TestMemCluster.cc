...
int TestMemCluster::register_object_handler(int64_t pool_id, const ObjectLocator& locator,
...
void TestMemCluster::unregister_object_handler(int64_t pool_id, const ObjectLocator& locator,
                                               ObjectHandler* object_handler)
{
    std::lock_guard locker{m_lock};
    auto pool = get_pool(m_lock, pool_id);
...
TestMemCluster::Pool* TestMemCluster::get_pool(const ceph::mutex& lock, int64_t pool_id)
{
    for (auto& pool_pair : m_pools) {
...
void TestMemCluster::transaction_start(const ObjectLocator& locator)
{
    ...
    m_transaction_cond.wait(locker, [&locator, this] { return m_transactions.count(locator) == 0; });
