...
TestClassHandler::~TestClassHandler()
{
    for (ClassHandles::iterator it = m_class_handles.begin(); it != m_class_handles.end(); ++it) {
        dlclose(*it);
    }
}

void TestClassHandler::open_class(const std::string& name, const std::string& path)
{
    void* handle = dlopen(path.c_str(), RTLD_NOW);
    if (handle == NULL) {
        std::cerr << "Failed to load class: " << name << " (" << path << "): " << dlerror() << std::endl;
        return;
    }
    ...
    std::cerr << "Class: " << name << " (" << path << ") missing initializer" << std::endl;
    dlclose(handle);
}
...
void TestClassHandler::open_all_classes()
{
    ...
    while ((pde = ::readdir(dir))) {
        ...
        if (!boost::algorithm::starts_with(name, "libcls_") || !boost::algorithm::ends_with(name, SHARED_LIB_SUFFIX)) {
            continue;
        }
        names.insert(name);
    }
...
int TestClassHandler::create_method(cls_handle_t hclass, const char* name, cls_method_cxx_call_t class_call,
                                    cls_method_handle_t* handle)
{
    Class* cls = reinterpret_cast<Class*>(hclass);
    if (cls->methods.find(name) != cls->methods.end()) {
        std::cerr << "Class method " << hclass << ":" << name << " already exists" << std::endl;
        return -EEXIST;
    }
...
cls_method_cxx_call_t TestClassHandler::get_method(const std::string& cls, const std::string& method)
{
    ...
    if (m_it == scls->methods.end()) {
        std::cerr << "Failed to located class method" << cls << "." << method << std::endl;
        return NULL;
    }
    return m_it->second->class_call;
}

TestClassHandler::SharedMethodContext TestClassHandler::get_method_context(TestIoCtxImpl* io_ctx_impl,
                                                                           const std::string& oid, uint64_t snap_id,
...
int TestClassHandler::create_filter(cls_handle_t hclass, const std::string& name, cls_cxx_filter_factory_t fn)
{
    Class* cls = reinterpret_cast<Class*>(hclass);
