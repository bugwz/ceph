...
#undef dout_prefix
#define dout_prefix *_dout << "TestMemIoCtxImpl: " << this << " " << __func__ << ": " << oid << " "

static void to_vector(const interval_set<uint64_t>& set, std::vector<std::pair<uint64_t, uint64_t> >* vec)
{
    vec->clear();
    for (interval_set<uint64_t>::const_iterator it = set.begin(); it != set.end(); ++it) {
        vec->push_back(*it);
    }
}
...
TestMemIoCtxImpl::TestMemIoCtxImpl(TestMemRadosClient* client, int64_t pool_id, const std::string& pool_name,
                                   TestMemCluster::Pool* pool)
    : TestIoCtxImpl(client, pool_id, pool_name)
    , m_client(client)
    , m_pool(pool)
{
    m_pool->get();
...
{
    m_client->add_aio_operation(oid, true, std::bind(&TestMemIoCtxImpl::remove, this, oid, get_snap_context()), c);
    return 0;
}

int TestMemIoCtxImpl::append(const std::string& oid, const bufferlist& bl, const SnapContext& snapc)
{
...
int TestMemIoCtxImpl::create(const std::string& oid, bool exclusive, const SnapContext& snapc)
{
    if (get_snap_read() != CEPH_NOSNAP) {
        return -EROFS;
    }
...
int TestMemIoCtxImpl::list_snaps(const std::string& oid, snap_set_t* out_snaps)
{
    ...
    std::shared_lock l{m_pool->file_lock};
    TestMemCluster::Files::iterator it = m_pool->files.find({get_namespace(), oid});
    ...
    for (TestMemCluster::FileSnapshots::iterator s_it = file_snaps.begin(); s_it != file_snaps.end(); ++s_it) {
    ...
    if ((file_snaps.size() == 1 && file_snaps.back()->data.length() > 0) || include_head) {
...
int TestMemIoCtxImpl::omap_get_vals2(const std::string& oid, const std::string& start_after,
                                     ...
                                     std::map<std::string, bufferlist>* out_vals, bool* pmore)
{
    ...
    TestMemCluster::FileOMaps::iterator o_it = m_pool->file_omaps.find({get_namespace(), oid});
    ...
    while (it != omap.end() && max_return > 0) {
        if (filter_prefix.empty() || boost::algorithm::starts_with(it->first, filter_prefix)) {
...
int TestMemIoCtxImpl::omap_get_vals(const std::string& oid, const std::string& start_after,
                                    const std::string& filter_prefix, uint64_t max_return,
...
int TestMemIoCtxImpl::omap_rm_keys(const std::string& oid, const std::set<std::string>& keys)
{
    if (get_snap_read() != CEPH_NOSNAP) {
        return -EROFS;
    }
...
int TestMemIoCtxImpl::omap_set(const std::string& oid, const std::map<std::string, bufferlist>& map)
{
    if (get_snap_read() != CEPH_NOSNAP) {
        return -EROFS;
    }
    ...
    std::unique_lock l{file->lock};
    for (std::map<std::string, bufferlist>::const_iterator it = map.begin(); it != map.end(); ++it) {
        bufferlist bl;
        bl.append(it->second);
...
int TestMemIoCtxImpl::read(const std::string& oid, size_t len, uint64_t off, bufferlist* bl, uint64_t snap_id,
                           uint64_t* objver)
{
    if (m_client->is_blocklisted()) {
        return -EBLOCKLISTED;
...
int TestMemIoCtxImpl::selfmanaged_snap_remove(uint64_t snapid)
{
    ...
    std::unique_lock l{m_pool->file_lock};
    TestMemCluster::SnapSeqs::iterator it = m_pool->snap_seqs.find(snapid);
...
int TestMemIoCtxImpl::selfmanaged_snap_rollback(const std::string& oid, uint64_t snapid)
{
    if (m_client->is_blocklisted()) {
        return -EBLOCKLISTED;
    }
    ...
    TestMemCluster::SharedFile file;
    TestMemCluster::Files::iterator f_it = m_pool->files.find({get_namespace(), oid});
    ...
    size_t versions = 0;
    for (TestMemCluster::FileSnapshots::reverse_iterator it = snaps.rbegin(); it != snaps.rend(); ++it) {
...
int TestMemIoCtxImpl::set_alloc_hint(const std::string& oid, uint64_t expected_object_size,
                                     uint64_t expected_write_size, uint32_t flags, const SnapContext& snapc)
{
    if (get_snap_read() != CEPH_NOSNAP) {
        return -EROFS;
    }
...
int TestMemIoCtxImpl::sparse_read(const std::string& oid, uint64_t off, uint64_t len, std::map<uint64_t, uint64_t>* m,
...
int TestMemIoCtxImpl::stat(const std::string& oid, uint64_t* psize, time_t* pmtime)
{
    if (m_client->is_blocklisted()) {
        return -EBLOCKLISTED;
    }
...
int TestMemIoCtxImpl::truncate(const std::string& oid, uint64_t size, const SnapContext& snapc)
{
    if (get_snap_read() != CEPH_NOSNAP) {
        return -EROFS;
    }
...
int TestMemIoCtxImpl::write(const std::string& oid, bufferlist& bl, size_t len, uint64_t off, const SnapContext& snapc)
{
    if (get_snap_read() != CEPH_NOSNAP) {
        return -EROFS;
    }
...
int TestMemIoCtxImpl::write_full(const std::string& oid, bufferlist& bl, const SnapContext& snapc)
{
    if (get_snap_read() != CEPH_NOSNAP) {
        return -EROFS;
    }
...
int TestMemIoCtxImpl::writesame(const std::string& oid, bufferlist& bl, size_t len, uint64_t off,
...
int TestMemIoCtxImpl::cmpext(const std::string& oid, uint64_t off, bufferlist& cmp_bl, uint64_t snap_id)
{
    if (m_client->is_blocklisted()) {
        return -EBLOCKLISTED;
    }
...
int TestMemIoCtxImpl::xattr_get(const std::string& oid, std::map<std::string, bufferlist>* attrset)
{
    if (m_client->is_blocklisted()) {
        return -EBLOCKLISTED;
    }
    ...
    TestMemCluster::SharedFile file;
    std::shared_lock l{m_pool->file_lock};
    TestMemCluster::FileXAttrs::iterator it = m_pool->file_xattrs.find({get_namespace(), oid});
...
int TestMemIoCtxImpl::xattr_set(const std::string& oid, const std::string& name, bufferlist& bl)
{
    if (m_client->is_blocklisted()) {
        return -EBLOCKLISTED;
    }
...
int TestMemIoCtxImpl::zero(const std::string& oid, uint64_t off, uint64_t len, const SnapContext& snapc)
{
    if (m_client->is_blocklisted()) {
        return -EBLOCKLISTED;
    }
...
TestMemCluster::SharedFile TestMemIoCtxImpl::get_file(const std::string& oid, bool write, uint64_t snap_id,
                                                      const SnapContext& snapc)
{
    ...
    if (write) {
        ...
        if (!file || !file->exists) {
            ...
            if (!snapc.snaps.empty() && file->snap_id < snapc.seq) {
                for (std::vector<snapid_t>::const_reverse_iterator seq_it = snapc.snaps.rbegin();
                     seq_it != snapc.snaps.rend();
                     ++seq_it) {
                ...
                file = TestMemCluster::SharedFile(new TestMemCluster::File(*file));
                file->data.clear();
