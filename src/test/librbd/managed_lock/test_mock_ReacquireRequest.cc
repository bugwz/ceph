...
public:
    ...
    void expect_set_cookie(MockImageCtx& mock_image_ctx, int r, bool exclusive = true)
    {
        EXPECT_CALL(
            get_mock_io_ctx(mock_image_ctx.md_ctx),
            exec(mock_image_ctx.header_oid, _, StrEq("lock"), StrEq("set_cookie"), IsLockType(exclusive), _, _, _))
            .WillOnce(Return(r));
    }
};
...
TEST_F(TestMockManagedLockReacquireRequest, SuccessExclusive)
{
    ...
    MockReacquireRequest* req = MockReacquireRequest::create(
        mock_image_ctx.md_ctx, mock_image_ctx.header_oid, "old_cookie", "new_cookie", true, &ctx);
    req->send();
...
TEST_F(TestMockManagedLockReacquireRequest, SuccessShared)
{
    ...
    C_SaferCond ctx;
    MockReacquireRequest* req = MockReacquireRequest::create(
        mock_image_ctx.md_ctx, mock_image_ctx.header_oid, "old_cookie", "new_cookie", false, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
