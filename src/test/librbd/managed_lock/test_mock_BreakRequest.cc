...
template<> struct GetLockerRequest<librbd::MockTestImageCtx>
{
    Locker* locker;
...
public:
    ...
    void expect_list_watchers(MockTestImageCtx& mock_image_ctx, int r, const std::string& address,
                              uint64_t watch_handle)
    {
        auto& expect = EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx), list_watchers(mock_image_ctx.header_oid, _));
        if (r < 0) {
    ...
    void expect_get_locker(MockImageCtx& mock_image_ctx, MockGetLockerRequest& mock_get_locker_request,
        ...
        EXPECT_CALL(mock_get_locker_request, send())
            .WillOnce(Invoke([&mock_image_ctx, &mock_get_locker_request, locker, r]() {
                *mock_get_locker_request.locker = locker;
                mock_image_ctx.image_ctx->op_work_queue->queue(mock_get_locker_request.on_finish, r);
            }));
    }
    ...
    {
        auto& mock_rados_client = librados::get_mock_rados_client(mock_image_ctx.rados_api);
    ...
    {
        auto& mock_rados_client = librados::get_mock_rados_client(mock_image_ctx.rados_api);
        EXPECT_CALL(mock_rados_client, wait_for_latest_osd_map()).WillOnce(Return(r));
    }

    void expect_break_lock(MockTestImageCtx& mock_image_ctx, int r)
    {
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    exec(mock_image_ctx.header_oid, _, StrEq("lock"), StrEq("break_lock"), _, _, _, _))
            .WillOnce(Return(r));
    }

    void expect_get_instance_id(MockTestImageCtx& mock_image_ctx, uint64_t id)
    {
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx), get_instance_id()).WillOnce(Return(id));
    }
};
...
TEST_F(TestMockManagedLockBreakRequest, DeadLockOwner)
{
    ...
    expect_get_locker(
        mock_image_ctx, mock_get_locker_request, {entity_name_t::CLIENT(1), "auto 123", "1.2.3.4:0/0", 123}, 0);
    ...
    MockBreakRequest* req = MockBreakRequest::create(
        mock_image_ctx.md_ctx, *ictx->asio_engine, mock_image_ctx.header_oid, locker, true, true, 0, false, &ctx);
    req->send();
...
TEST_F(TestMockManagedLockBreakRequest, ForceBreak)
{
    ...
    Locker locker{entity_name_t::CLIENT(1), "auto 123", "1.2.3.4:0/0", 123};
    MockBreakRequest* req = MockBreakRequest::create(
        mock_image_ctx.md_ctx, *ictx->asio_engine, mock_image_ctx.header_oid, locker, true, true, 0, true, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(TestMockManagedLockBreakRequest, GetLockerUpdated)
{
    ...
    MockGetLockerRequest mock_get_locker_request;
    expect_get_locker(
        mock_image_ctx, mock_get_locker_request, {entity_name_t::CLIENT(2), "auto 123", "1.2.3.4:0/0", 123}, 0);

    C_SaferCond ctx;
    Locker locker{entity_name_t::CLIENT(1), "auto 123", "1.2.3.4:0/0", 123};
    MockBreakRequest* req = MockBreakRequest::create(
        mock_image_ctx.md_ctx, *ictx->asio_engine, mock_image_ctx.header_oid, locker, true, false, 0, false, &ctx);
    req->send();
    ASSERT_EQ(-EAGAIN, ctx.wait());
}
...
TEST_F(TestMockManagedLockBreakRequest, GetLockerBusy)
{
    ...
    MockGetLockerRequest mock_get_locker_request;
    expect_get_locker(
        mock_image_ctx, mock_get_locker_request, {entity_name_t::CLIENT(1), "auto 123", "1.2.3.4:0/0", 123}, -EBUSY);
...
TEST_F(TestMockManagedLockBreakRequest, GetLockerMissing)
{
    ...
    MockGetLockerRequest mock_get_locker_request;
    expect_get_locker(
        mock_image_ctx, mock_get_locker_request, {entity_name_t::CLIENT(1), "auto 123", "1.2.3.4:0/0", 123}, -ENOENT);
...
TEST_F(TestMockManagedLockBreakRequest, BlocklistSelf)
{
    ...
    MockGetLockerRequest mock_get_locker_request;
    expect_get_locker(
        mock_image_ctx, mock_get_locker_request, {entity_name_t::CLIENT(456), "auto 123", "1.2.3.4:0/0", 123}, 0);
