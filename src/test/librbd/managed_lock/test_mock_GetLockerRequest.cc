...
public:
    ...
    void expect_get_lock_info(MockTestImageCtx& mock_image_ctx, int r, const entity_name_t& locker_entity,
                              const std::string& locker_address, const std::string& locker_cookie,
                              const std::string& lock_tag, ClsLockType lock_type)
    {
        ...
        if (r < 0 && r != -ENOENT) {
            ...
            if (r != -ENOENT) {
                reply.lockers.emplace(rados::cls::lock::locker_id_t(entity, locker_cookie),
...
TEST_F(TestMockManagedLockGetLockerRequest, SuccessExclusive)
{
    ...
    InSequence seq;
    expect_get_lock_info(mock_image_ctx,
                         0,
    ...
    C_SaferCond ctx;
    Locker locker;
    MockGetLockerRequest* req =
        MockGetLockerRequest::create(mock_image_ctx.md_ctx, mock_image_ctx.header_oid, true, &locker, &ctx);
    req->send();
...
TEST_F(TestMockManagedLockGetLockerRequest, SuccessShared)
{
    ...
    Locker locker;
    MockGetLockerRequest* req =
        MockGetLockerRequest::create(mock_image_ctx.md_ctx, mock_image_ctx.header_oid, false, &locker, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
...
TEST_F(TestMockManagedLockGetLockerRequest, GetLockInfoError)
{
    ...
    InSequence seq;
    expect_get_lock_info(mock_image_ctx, -EINVAL, entity_name_t::CLIENT(1), "", "", "", ClsLockType::EXCLUSIVE);
...
TEST_F(TestMockManagedLockGetLockerRequest, GetLockInfoEmpty)
{
    ...
    InSequence seq;
    expect_get_lock_info(mock_image_ctx, -ENOENT, entity_name_t::CLIENT(1), "", "", "", ClsLockType::EXCLUSIVE);
...
TEST_F(TestMockManagedLockGetLockerRequest, GetLockInfoExternalTag)
{
    ...
    expect_get_lock_info(
        mock_image_ctx, 0, entity_name_t::CLIENT(1), "1.2.3.4", "auto 123", "external tag", ClsLockType::EXCLUSIVE);
