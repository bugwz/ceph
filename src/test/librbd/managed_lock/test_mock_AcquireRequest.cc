...
template<> struct BreakRequest<librbd::MockImageCtx>
{
                                ...
                                uint32_t blocklist_expire_seconds, bool force_break_lock, Context* on_finish)
    {
        ...
        EXPECT_EQ(cct->_conf.get_val<bool>("rbd_blocklist_on_break_lock"), blocklist_locker);
        EXPECT_EQ(cct->_conf.get_val<uint64_t>("rbd_blocklist_expire_seconds"), blocklist_expire_seconds);
...
template<> struct GetLockerRequest<librbd::MockImageCtx>
{
...
public:
    ...
    void expect_lock(MockImageCtx& mock_image_ctx, int r, bool exclusive = true)
    {
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    exec(mock_image_ctx.header_oid, _, StrEq("lock"), StrEq("lock"), IsLockType(exclusive), _, _, _))
            .WillOnce(Return(r));
    }
    ...
    void expect_break_lock(MockImageCtx& mock_image_ctx, MockBreakRequest& mock_break_request, int r)
    {
        EXPECT_CALL(mock_break_request, send()).WillOnce(FinishRequest(&mock_break_request, r, &mock_image_ctx));
    }
};
