...
int create_image_full_pp(librbd::RBD& rbd, librados::IoCtx& ioctx, const std::string& name, uint64_t size,
...
int create_image_pp(librbd::RBD& rbd, librados::IoCtx& ioctx, const std::string& name, uint64_t size)
{
    int order = 0;
    uint64_t features = 0;
...
int clone_image_pp(librbd::RBD& rbd, librbd::Image& p_image, librados::IoCtx& p_ioctx, const char* p_name,
                   const char* p_snap_name, librados::IoCtx& c_ioctx, const char* c_name, uint64_t features)
{
    ...
    return rbd.clone2(p_ioctx, p_name, p_snap_name, c_ioctx, c_name, features, &c_order, stripe_unit, stripe_count);
}
