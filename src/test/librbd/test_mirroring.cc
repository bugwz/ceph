...
static bool operator==(const mirror_peer_site_t& lhs, const mirror_peer_site_t& rhs)
{
    return (lhs.uuid == rhs.uuid && lhs.direction == rhs.direction && lhs.site_name == rhs.site_name &&
            lhs.client_name == rhs.client_name && lhs.last_seen == rhs.last_seen);
}

static std::ostream& operator<<(std::ostream& os, const mirror_peer_site_t& rhs)
{
    os << "uuid=" << rhs.uuid << ", "
...
public:
    ...
    void SetUp() override { ASSERT_EQ(0, _rados.ioctx_create(_pool_name.c_str(), m_ioctx)); }
    ...
    void check_mirror_image_enable(rbd_mirror_mode_t mirror_mode, uint64_t features, int expected_r,
    ...
    void check_mirror_image_disable(rbd_mirror_mode_t mirror_mode, uint64_t features, int expected_r,
                                    rbd_mirror_image_state_t mirror_state)
    {
    ...
    void check_mirroring_status(size_t* images_count)
    {
        ...
        ASSERT_EQ(0, m_rbd.mirror_image_global_status_list(m_ioctx, "", 4096, &images));
        ...
        *images_count = images.size();
        ...
        ASSERT_EQ(0, m_rbd.mirror_image_instance_id_list(m_ioctx, "", 4096, &instance_ids));
        ASSERT_TRUE(instance_ids.empty());
    }

    void check_mirroring_on_create(uint64_t features, rbd_mirror_mode_t mirror_mode,
                                   rbd_mirror_image_state_t mirror_state)
    {
        ...
        if (mirror_mode == RBD_MIRROR_MODE_POOL && mirror_state == RBD_MIRROR_IMAGE_ENABLED) {
    ...
    void check_mirroring_on_update_features(uint64_t init_features, bool enable, bool enable_mirroring,
    ...
    void setup_images_with_mirror_mode(rbd_mirror_mode_t mirror_mode, std::vector<uint64_t>& features_vec)
    {
    ...
    void check_remove_image(rbd_mirror_mode_t mirror_mode, uint64_t features, bool enable_mirroring,
                            bool demote = false)
    {
    ...
    void check_trash_move_restore(rbd_mirror_mode_t mirror_mode, bool enable_mirroring)
    {
...
TEST_F(TestMirroring, EnableImageMirror_In_MirrorModeImage)
{
    ...
    check_mirror_image_enable(
        RBD_MIRROR_MODE_IMAGE, features, 0, RBD_MIRROR_IMAGE_ENABLED, RBD_MIRROR_IMAGE_MODE_JOURNAL);
}
...
TEST_F(TestMirroring, EnableImageMirror_In_MirrorModePool)
{
    ...
    features |= RBD_FEATURE_JOURNALING;
    check_mirror_image_enable(
        RBD_MIRROR_MODE_POOL, features, -EINVAL, RBD_MIRROR_IMAGE_ENABLED, RBD_MIRROR_IMAGE_MODE_JOURNAL);
}
...
TEST_F(TestMirroring, EnableImageMirror_In_MirrorModeDisabled)
{
    ...
    features |= RBD_FEATURE_EXCLUSIVE_LOCK;
    features |= RBD_FEATURE_JOURNALING;
    check_mirror_image_enable(RBD_MIRROR_MODE_DISABLED, features, -EINVAL, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, DisableImageMirror_In_MirrorModeImage)
{
    ...
    features |= RBD_FEATURE_EXCLUSIVE_LOCK;
    features |= RBD_FEATURE_JOURNALING;
    check_mirror_image_disable(RBD_MIRROR_MODE_IMAGE, features, 0, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, DisableImageMirror_In_MirrorModePool)
{
    ...
    features |= RBD_FEATURE_EXCLUSIVE_LOCK;
    features |= RBD_FEATURE_JOURNALING;
    check_mirror_image_disable(RBD_MIRROR_MODE_POOL, features, -EINVAL, RBD_MIRROR_IMAGE_ENABLED);
}
...
TEST_F(TestMirroring, DisableImageMirror_In_MirrorModeDisabled)
{
    ...
    features |= RBD_FEATURE_EXCLUSIVE_LOCK;
    features |= RBD_FEATURE_JOURNALING;
    check_mirror_image_disable(RBD_MIRROR_MODE_DISABLED, features, -EINVAL, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, EnableImageMirror_In_MirrorModePool_WithoutJournaling)
{
    ...
    features |= RBD_FEATURE_OBJECT_MAP;
    features |= RBD_FEATURE_EXCLUSIVE_LOCK;
    check_mirror_image_enable(RBD_MIRROR_MODE_POOL, features, -EINVAL, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, EnableImageMirror_In_MirrorModeImage_WithoutJournaling)
{
    ...
    features |= RBD_FEATURE_EXCLUSIVE_LOCK;
    check_mirror_image_enable(
        RBD_MIRROR_MODE_IMAGE, features, 0, RBD_MIRROR_IMAGE_ENABLED, RBD_MIRROR_IMAGE_MODE_SNAPSHOT);
}
...
TEST_F(TestMirroring, CreateImage_In_MirrorModeDisabled)
{
    ...
    features |= RBD_FEATURE_EXCLUSIVE_LOCK;
    features |= RBD_FEATURE_JOURNALING;
    check_mirroring_on_create(features, RBD_MIRROR_MODE_DISABLED, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, CreateImage_In_MirrorModeImage)
{
    ...
    features |= RBD_FEATURE_EXCLUSIVE_LOCK;
    features |= RBD_FEATURE_JOURNALING;
    check_mirroring_on_create(features, RBD_MIRROR_MODE_IMAGE, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, CreateImage_In_MirrorModePool)
{
    ...
    features |= RBD_FEATURE_EXCLUSIVE_LOCK;
    features |= RBD_FEATURE_JOURNALING;
    check_mirroring_on_create(features, RBD_MIRROR_MODE_POOL, RBD_MIRROR_IMAGE_ENABLED);
}
...
TEST_F(TestMirroring, CreateImage_In_MirrorModePool_WithoutJournaling)
{
    ...
    features |= RBD_FEATURE_OBJECT_MAP;
    features |= RBD_FEATURE_EXCLUSIVE_LOCK;
    check_mirroring_on_create(features, RBD_MIRROR_MODE_POOL, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, EnableJournaling_In_MirrorModeDisabled)
{
    ...
    check_mirroring_on_update_features(
        init_features, true, false, features, 0, RBD_MIRROR_MODE_DISABLED, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, EnableJournaling_In_MirrorModeImage)
{
    ...
    uint64_t features = RBD_FEATURE_JOURNALING;
    check_mirroring_on_update_features(
        init_features, true, false, features, 0, RBD_MIRROR_MODE_IMAGE, RBD_MIRROR_IMAGE_DISABLED);
}
...
TEST_F(TestMirroring, EnableJournaling_In_MirrorModeImage_SnapshotMirroringEnabled)
{
    ...
    init_features |= RBD_FEATURE_EXCLUSIVE_LOCK;
    uint64_t features = RBD_FEATURE_JOURNALING;
    check_mirroring_on_update_features(init_features,
                                       true,
                                       true,
                                       features,
                                       0,
...
TEST_F(TestMirroring, DisableJournaling_In_MirrorModePool)
{
    ...
    uint64_t features = RBD_FEATURE_JOURNALING;
    check_mirroring_on_update_features(
        init_features, false, false, features, 0, RBD_MIRROR_MODE_POOL, RBD_MIRROR_IMAGE_DISABLED);
}
...
{
    check_remove_image(RBD_MIRROR_MODE_IMAGE, RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_JOURNALING, true);
}

TEST_F(TestMirroring, RemoveImage_With_MirrorImageDisabled)
{
    check_remove_image(RBD_MIRROR_MODE_IMAGE, RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_JOURNALING, false);
}

TEST_F(TestMirroring, RemoveImage_With_ImageWithoutJournal)
{
    check_remove_image(RBD_MIRROR_MODE_IMAGE, RBD_FEATURE_EXCLUSIVE_LOCK, false);
}

TEST_F(TestMirroring, RemoveImage_With_MirrorImageDemoted)
{
    check_remove_image(RBD_MIRROR_MODE_IMAGE, RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_JOURNALING, true, true);
}
...
{
    std::vector<uint64_t> features_vec(5, RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_JOURNALING);
    setup_images_with_mirror_mode(RBD_MIRROR_MODE_POOL, features_vec);

    std::string last_read = "";
    std::map<std::string, librbd::mirror_image_global_status_t> images;
    ASSERT_EQ(0, m_rbd.mirror_image_global_status_list(m_ioctx, last_read, 2, &images));
    ...
    last_read = images.rbegin()->first;
    images.clear();
    ASSERT_EQ(0, m_rbd.mirror_image_global_status_list(m_ioctx, last_read, 4096, &images));
...
TEST_F(TestMirroring, RemoveBootstrapped)
{
    ...
    {
        explicit MirrorWatcher(librados::IoCtx& ioctx)
            : m_ioctx(ioctx)
        {}
...
TEST_F(TestMirroring, AioPromoteDemote)
{
    ...
    for (auto& image_name : image_names) {
        ASSERT_EQ(0,
                  m_rbd.create2(
                      m_ioctx, image_name.c_str(), 2048, RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_JOURNALING, &order));

        images.emplace_back();
        ASSERT_EQ(0, m_rbd.open(m_ioctx, images.back(), image_name.c_str()));
        ASSERT_EQ(0, images.back().mirror_image_enable2(RBD_MIRROR_IMAGE_MODE_JOURNAL));
    }
...
TEST_F(TestMirroring, AioGetInfo)
{
    ...
    for (auto& image : images) {
        aio_comps.push_back(new librbd::RBD::AioCompletion(nullptr, nullptr));
        infos.emplace_back();
        ASSERT_EQ(0, image.aio_mirror_image_get_info(&infos.back(), sizeof(infos.back()), aio_comps.back()));
    }
    for (auto aio_comp : aio_comps) {
        ASSERT_EQ(0, aio_comp->wait_for_complete());
        ASSERT_EQ(1, aio_comp->is_complete());
...
TEST_F(TestMirroring, AioGetStatus)
{
    ...
    for (auto& image : images) {
        ...
        ASSERT_EQ(
            0, image.aio_mirror_image_get_global_status(&statuses.back(), sizeof(statuses.back()), aio_comps.back()));
    }
    for (auto aio_comp : aio_comps) {
        ASSERT_EQ(0, aio_comp->wait_for_complete());
        ASSERT_EQ(1, aio_comp->is_complete());
...
TEST_F(TestMirroring, Bootstrap)
{
    ...
    ASSERT_EQ(-EINVAL, m_rbd.mirror_peer_bootstrap_import(m_ioctx, RBD_MIRROR_PEER_DIRECTION_RX, token_b64));
}
...
TEST_F(TestMirroring, PeerDirection)
{
    ...
    std::string uuid;
    ASSERT_EQ(-EINVAL,
              m_rbd.mirror_peer_site_add(m_ioctx, &uuid, RBD_MIRROR_PEER_DIRECTION_TX, "siteA", "client.admin"));
    ASSERT_EQ(0, m_rbd.mirror_peer_site_add(m_ioctx, &uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, "siteA", "client.admin"));
    ...
    ASSERT_EQ(0, m_rbd.mirror_peer_site_set_direction(m_ioctx, uuid, RBD_MIRROR_PEER_DIRECTION_RX));
    ASSERT_EQ(0, m_rbd.mirror_peer_site_list(m_ioctx, &peers));
    expected_peers = {{uuid, RBD_MIRROR_PEER_DIRECTION_RX, "siteA", "", "client.admin", 0}};
    ASSERT_EQ(expected_peers, peers);
...
TEST_F(TestMirroring, Snapshot)
{
    ...
    ASSERT_EQ(0, image.metadata_set("conf_rbd_mirroring_max_mirroring_snapshots", "5"));
    ...
    ASSERT_EQ(0, m_rbd.mirror_peer_site_add(m_ioctx, &peer_uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, "cluster", "client"));
    ...
    ASSERT_EQ(0, image.snap_get_mirror_namespace(snap_id, &mirror_snap, sizeof(mirror_snap)));
...
TEST_F(TestMirroring, SnapshotUnlinkPeer)
{
    ...
    ASSERT_EQ(0,
              m_rbd.mirror_peer_site_add(m_ioctx, &peer1_uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, "cluster1", "client"));
    std::string peer2_uuid;
    ASSERT_EQ(0,
              m_rbd.mirror_peer_site_add(m_ioctx, &peer2_uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, "cluster2", "client"));
    std::string peer3_uuid;
    ASSERT_EQ(0,
              m_rbd.mirror_peer_site_add(m_ioctx, &peer3_uuid, RBD_MIRROR_PEER_DIRECTION_RX_TX, "cluster3", "client"));
    uint64_t features;
    ASSERT_TRUE(get_features(&features));
    features &= ~RBD_FEATURE_JOURNALING;
    int order = 20;
    ...
    C_SaferCond cond1;
    auto req = librbd::mirror::snapshot::UnlinkPeerRequest<>::create(ictx, snap_id, peer1_uuid, true, &cond1);
    req->send();
    ...
    ASSERT_EQ(0, m_rbd.create2(ns_ioctx, image_name.c_str(), 4096, features, &order));
    ...
    ASSERT_EQ(0, ns_image.snap_get_mirror_namespace(ns_snap_id, &mirror_snap, sizeof(mirror_snap)));
    ASSERT_EQ(3U, mirror_snap.mirror_peer_uuids.size());
    ASSERT_EQ(1, mirror_snap.mirror_peer_uuids.count(peer1_uuid));
    ASSERT_EQ(1, mirror_snap.mirror_peer_uuids.count(peer2_uuid));
    ASSERT_EQ(1, mirror_snap.mirror_peer_uuids.count(peer3_uuid));
    ...
    C_SaferCond cond2;
    req = librbd::mirror::snapshot::UnlinkPeerRequest<>::create(ictx, snap_id, peer2_uuid, true, &cond2);
    req->send();
    ASSERT_EQ(0, cond2.wait());

    ASSERT_EQ(-ENOENT, image.snap_get_mirror_namespace(snap_id, &mirror_snap, sizeof(mirror_snap)));
    ictx->state->close();
    ictx = nullptr;
    ASSERT_EQ(0, image.close());
    ASSERT_EQ(0, m_rbd.remove(m_ioctx, image_name.c_str()));
...
TEST_F(TestMirroring, SnapshotImageState)
{
    ...
    {
        C_SaferCond cond;
        auto req = librbd::mirror::snapshot::SetImageStateRequest<>::create(ictx, snap_id, &cond);
        req->send();
    ...
    {
        C_SaferCond cond;
        auto req = librbd::mirror::snapshot::GetImageStateRequest<>::create(ictx, snap_id, &image_state, &cond);
        req->send();
        ASSERT_EQ(0, cond.wait());
    }
    ...
    {
        C_SaferCond cond;
        auto req = librbd::mirror::snapshot::RemoveImageStateRequest<>::create(ictx, snap_id, &cond);
        req->send();
        ASSERT_EQ(0, cond.wait());
    }
    ...
    for (int i = 0; i < 10; i++) {
        ...
        ASSERT_EQ(0, strncmp(std::string(1024, 'A' + i).c_str(), bl.c_str(), bl.length()));
    }
...
TEST_F(TestMirroring, AioSnapshotCreate)
{
    ...
    std::list<librbd::Image> images;
    for (auto& image_name : image_names) {
        ASSERT_EQ(0, m_rbd.create2(m_ioctx, image_name.c_str(), 2048, features, &order));
        images.emplace_back();
        ASSERT_EQ(0, m_rbd.open(m_ioctx, images.back(), image_name.c_str()));
        ASSERT_EQ(0, images.back().mirror_image_enable2(RBD_MIRROR_IMAGE_MODE_SNAPSHOT));
    }
    ...
    for (auto& image : images) {
        ...
        aio_comps.push_back(new librbd::RBD::AioCompletion(nullptr, nullptr));
        ASSERT_EQ(0, image.aio_mirror_image_create_snapshot(0, &snap_ids.back(), aio_comps.back()));
    }
    for (auto aio_comp : aio_comps) {
        ASSERT_EQ(0, aio_comp->wait_for_complete());
        ASSERT_EQ(1, aio_comp->is_complete());
