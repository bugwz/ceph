...
static bool operator==(const trash_image_info_t& lhs, const trash_image_info_t& rhs)
{
...
TEST_F(TestTrash, UserRemovingSource)
{
    ...
    auto compare_lambda = [](const trash_image_info_t& lhs, const trash_image_info_t& rhs) {
        if (lhs.id != rhs.id) {
    ...
    ASSERT_EQ(0, api::Trash<>::move(m_ioctx, RBD_TRASH_IMAGE_SOURCE_USER, image_name1, image_id1, 0));
    ASSERT_EQ(0, api::Trash<>::move(m_ioctx, RBD_TRASH_IMAGE_SOURCE_REMOVING, image_name2, image_id2, 0));
    ...
    expected_trash_entries = {
        {.id = image_id1, .name = image_name1, .source = RBD_TRASH_IMAGE_SOURCE_USER},
    };
    ASSERT_EQ(expected_trash_entries, trash_entries);

    std::vector<image_spec_t> expected_images = {{.id = image_id2, .name = image_name2}};
...
TEST_F(TestTrash, RestoreMirroringSource)
{
    ...
    ASSERT_EQ(0, api::Trash<>::move(m_ioctx, RBD_TRASH_IMAGE_SOURCE_MIRRORING, m_image_name, 0));
    ASSERT_EQ(0, rbd.trash_restore(m_ioctx, image_id.c_str(), m_image_name.c_str()));
}
