...
template<> struct ImageDispatch<MockTestImageCtx>
{
    MOCK_METHOD3(set_require_lock, void(bool init_shutdown, io::Direction, Context*));
...
public:
    ...
    void expect_complete_context(MockContext& mock_context, int r) { EXPECT_CALL(mock_context, complete(r)); }
    ...
    void expect_set_require_lock(MockImageDispatch& mock_image_dispatch, bool init_shutdown,
    ...
    void expect_set_require_lock(MockTestImageCtx& mock_image_ctx, MockImageDispatch& mock_image_dispatch,
                                 bool init_shutdown, int r)
    {
        ...
        if (!mock_image_ctx.clone_copy_on_read) {
            expect_test_features(
                mock_image_ctx, RBD_FEATURE_JOURNALING, ((mock_image_ctx.features & RBD_FEATURE_JOURNALING) != 0));
        ...
        if (mock_image_ctx.clone_copy_on_read || (mock_image_ctx.features & RBD_FEATURE_JOURNALING) != 0 ||
            (mock_image_ctx.features & RBD_FEATURE_DIRTY_CACHE) != 0) {
            expect_set_require_lock(mock_image_dispatch, init_shutdown, librbd::io::DIRECTION_BOTH, r);
        }
        else {
            expect_set_require_lock(mock_image_dispatch, init_shutdown, librbd::io::DIRECTION_WRITE, r);
        }
    }
    ...
    void expect_close_journal(MockTestImageCtx& mock_image_ctx, MockJournal& mock_journal, int r)
    {
    ...
    void expect_invalidate_cache(MockTestImageCtx& mock_image_ctx, int r)
    {
...
TEST_F(TestMockExclusiveLockPreReleaseRequest, Success)
{
    ...
    C_SaferCond ctx;
    MockPreReleaseRequest* req =
        MockPreReleaseRequest::create(mock_image_ctx, &mock_image_dispatch, false, m_async_op_tracker, &ctx);
    req->send();
...
TEST_F(TestMockExclusiveLockPreReleaseRequest, SuccessObjectMapDisabled)
{
    ...
    C_SaferCond ctx;
    MockPreReleaseRequest* req =
        MockPreReleaseRequest::create(mock_image_ctx, &mock_image_dispatch, true, m_async_op_tracker, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(TestMockExclusiveLockPreReleaseRequest, Blocklisted)
{
    ...
    expect_cancel_op_requests(mock_image_ctx, 0);
    MockImageDispatch mock_image_dispatch;
    expect_set_require_lock(mock_image_ctx, mock_image_dispatch, false, -EBLOCKLISTED);
    expect_prepare_lock(mock_image_ctx);
