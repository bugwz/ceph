...
template<> struct RefreshRequest<librbd::MockTestImageCtx>
{
    static RefreshRequest* s_instance;
    Context* on_finish = nullptr;

    static RefreshRequest* create(librbd::MockTestImageCtx& image_ctx, bool acquire_lock_refresh, bool skip_open_parent,
                                  Context* on_finish)
    {
...
public:
    ...
    void expect_test_features(MockTestImageCtx& mock_image_ctx, uint64_t features, ceph::shared_mutex& lock,
                              bool enabled)
    {
        EXPECT_CALL(mock_image_ctx, test_features(features, _)).WillOnce(Return(enabled));
    }
    ...
    void expect_refresh(MockTestImageCtx& mock_image_ctx, MockRefreshRequest& mock_refresh_request, int r)
    {
        EXPECT_CALL(mock_refresh_request, send()).WillOnce(FinishRequest2(&mock_refresh_request, r, &mock_image_ctx));
    }

    void expect_create_object_map(MockTestImageCtx& mock_image_ctx, MockObjectMap* mock_object_map)
    {
        EXPECT_CALL(mock_image_ctx, create_object_map(_)).WillOnce(Return(mock_object_map));
    }

    void expect_open_object_map(MockTestImageCtx& mock_image_ctx, MockObjectMap& mock_object_map, int r)
    {
        EXPECT_CALL(mock_object_map, open(_)).WillOnce(CompleteContext(r, mock_image_ctx.image_ctx->op_work_queue));
    }

    void expect_close_object_map(MockTestImageCtx& mock_image_ctx, MockObjectMap& mock_object_map)
    {
        EXPECT_CALL(mock_object_map, close(_)).WillOnce(CompleteContext(0, mock_image_ctx.image_ctx->op_work_queue));
    }

    void expect_create_journal(MockTestImageCtx& mock_image_ctx, MockJournal* mock_journal)
    {
        EXPECT_CALL(mock_image_ctx, create_journal()).WillOnce(Return(mock_journal));
    }

    void expect_open_journal(MockTestImageCtx& mock_image_ctx, MockJournal& mock_journal, int r)
    {
        EXPECT_CALL(mock_journal, open(_)).WillOnce(CompleteContext(r, mock_image_ctx.image_ctx->op_work_queue));
    }

    void expect_close_journal(MockTestImageCtx& mock_image_ctx, MockJournal& mock_journal)
    {
        EXPECT_CALL(mock_journal, close(_)).WillOnce(CompleteContext(0, mock_image_ctx.image_ctx->op_work_queue));
    }

    void expect_get_journal_policy(MockTestImageCtx& mock_image_ctx, MockJournalPolicy& mock_journal_policy)
    {
    ...
    void expect_allocate_journal_tag(MockTestImageCtx& mock_image_ctx, MockJournalPolicy& mock_journal_policy, int r)
    {
...
TEST_F(TestMockExclusiveLockPostAcquireRequest, Success)
{
    ...
    expect_test_features(mock_image_ctx, RBD_FEATURE_JOURNALING, mock_image_ctx.image_lock, true);
    ...
    C_SaferCond ctx;
    MockPostAcquireRequest* req = MockPostAcquireRequest::create(mock_image_ctx, &acquire_ctx, &ctx);
    req->send();
...
TEST_F(TestMockExclusiveLockPostAcquireRequest, SuccessRefresh)
{
    ...
    expect_test_features(mock_image_ctx, RBD_FEATURE_JOURNALING, mock_image_ctx.image_lock, false);
    expect_handle_prepare_lock_complete(mock_image_ctx);
