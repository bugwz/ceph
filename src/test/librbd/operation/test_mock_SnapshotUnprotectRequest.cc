...
public:
    ...
    void expect_is_snap_unprotected(MockImageCtx& mock_image_ctx, bool is_unprotected, int r)
    {
    ...
    void expect_set_protection_status(MockImageCtx& mock_image_ctx, uint64_t snap_id, uint8_t status, int r)
    {
        bufferlist bl;
        encode(snap_id, bl);
        encode(status, bl);
    ...
    {
        librados::MockTestMemIoCtxImpl& io_ctx_impl = get_mock_io_ctx(mock_image_ctx.md_ctx);
        librados::MockTestMemRadosClient* rados_client = io_ctx_impl.get_mock_rados_client();
        ...
        EXPECT_CALL(*rados_client, create_ioctx(_, _))
            .Times(pools.size())
            .WillRepeatedly(DoAll(GetReference(&io_ctx_impl), Return(&io_ctx_impl)));
        return pools.size();
    }
    ...
    void expect_get_children(MockImageCtx& mock_image_ctx, size_t pools, int r)
    {
        ...
        auto& expect = EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                   exec(RBD_CHILDREN, _, StrEq("rbd"), StrEq("get_children"), _, _, _, _));
        if (r < 0) {
            expect.WillRepeatedly(Return(r));
        }
        else {
            expect.Times(pools).WillRepeatedly(DoAll(SetArgPointee<5>(bl), Return(0)));
        }
    }
};
...
TEST_F(TestMockOperationSnapshotUnprotectRequest, Success)
{
    ...
    expect_set_protection_status(mock_image_ctx, snap_id, RBD_PROTECTION_STATUS_UNPROTECTING, 0);
    size_t pools = expect_create_pool_io_contexts(mock_image_ctx);
    expect_get_children(mock_image_ctx, pools, -ENOENT);
    expect_set_protection_status(mock_image_ctx, snap_id, RBD_PROTECTION_STATUS_UNPROTECTED, 0);

    C_SaferCond cond_ctx;
    MockSnapshotUnprotectRequest* req =
        new MockSnapshotUnprotectRequest(mock_image_ctx, &cond_ctx, cls::rbd::UserSnapshotNamespace(), "snap1");
    {
...
TEST_F(TestMockOperationSnapshotUnprotectRequest, SetProtectionStatusError)
{
    ...
    expect_get_snap_id(mock_image_ctx, snap_id);
    expect_is_snap_unprotected(mock_image_ctx, false, 0);
    expect_set_protection_status(mock_image_ctx, snap_id, RBD_PROTECTION_STATUS_UNPROTECTING, -EINVAL);
...
TEST_F(TestMockOperationSnapshotUnprotectRequest, ChildrenExist)
{
    ...
    expect_set_protection_status(mock_image_ctx, snap_id, RBD_PROTECTION_STATUS_PROTECTED, 0);
