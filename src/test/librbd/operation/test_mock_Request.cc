...
template<> struct AsyncRequest<librbd::MockTestImageCtx>
{
    ...
    virtual void finish(int r) { m_on_finish->complete(r); }
...
{
    MockRequest(librbd::MockTestImageCtx& image_ctx, Context* on_finish, uint64_t journal_op_tid)
        : Request<librbd::MockTestImageCtx>(image_ctx, on_finish, journal_op_tid)
    {}

    void complete(int r) { finish_and_destroy(r); }

    void send_op_impl(int r)
    {
        bool appending = append_op_event<MockRequest, &MockRequest::handle_send>(this);
        if (!appending) {
            complete(r);
        }
    }
...
struct TestMockOperationRequest : public TestMockFixture
{
    ...
    {
        EXPECT_CALL(mock_request, send_op()).WillOnce(Invoke([&mock_request, r]() { mock_request.complete(r); }));
    }

    void expect_send_op_affects_io(MockImageCtx& mock_image_ctx, MockRequest& mock_request, int r)
    {
        EXPECT_CALL(mock_request, send_op()).WillOnce(Invoke([&mock_image_ctx, &mock_request, r]() {
            mock_image_ctx.image_ctx->op_work_queue->queue(
                new LambdaContext([&mock_request, r](int _) { mock_request.send_op_impl(r); }), 0);
        }));
    }
};
