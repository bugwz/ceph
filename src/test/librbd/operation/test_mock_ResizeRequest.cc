...
template<> class TrimRequest<MockImageCtx>
{
public:
    static TrimRequest* s_instance;
    static TrimRequest* create(MockImageCtx& image_ctx, Context* on_finish, uint64_t original_size, uint64_t new_size,
...
public:
    ...
    void expect_update_header(MockImageCtx& mock_image_ctx, int r)
    {
        if (mock_image_ctx.old_format) {
            EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx), write(mock_image_ctx.header_oid, _, _, _, _))
                .WillOnce(Return(r));
        }
        else {
            expect_is_lock_owner(mock_image_ctx);
    ...
    void expect_trim(MockImageCtx& mock_image_ctx, MockTrimRequest& mock_trim_request, int r)
    {
    ...
    void expect_resize_object_map(MockImageCtx& mock_image_ctx, uint64_t new_size)
    {
        EXPECT_CALL(*mock_image_ctx.object_map, aio_resize(new_size, _, _))
            .WillOnce(WithArg<2>(CompleteContext(0, mock_image_ctx.image_ctx->op_work_queue)));
    }

    int when_resize(MockImageCtx& mock_image_ctx, uint64_t new_size, bool allow_shrink, uint64_t journal_op_tid,
                    bool disable_journal)
    {
        C_SaferCond cond_ctx;
        librbd::NoOpProgressContext prog_ctx;
        MockResizeRequest* req = new MockResizeRequest(
            mock_image_ctx, &cond_ctx, new_size, allow_shrink, prog_ctx, journal_op_tid, disable_journal);
        {
