...
template<> class SetImageStateRequest<MockImageCtx>
{
public:
    static SetImageStateRequest* s_instance;
    Context* on_finish = nullptr;

    static SetImageStateRequest* create(MockImageCtx* image_ctx, uint64_t snap_id, Context* on_finish)
    {
    ...
    SetImageStateRequest() { s_instance = this; }
...
public:
    ...
    {
        auto& expect = EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.data_ctx), selfmanaged_snap_create(_));
        if (r < 0 && r != -ESTALE) {
    ...
    {
        auto& expect = EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.data_ctx), selfmanaged_snap_remove(_));
        if (r < 0) {
            expect.WillOnce(Return(r));
        }
        else {
    ...
    void expect_snap_create(MockImageCtx& mock_image_ctx, int r)
    {
        auto& expect = EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                   exec(mock_image_ctx.header_oid,
                                        _,
                                        StrEq("rbd"),
                                        StrEq(mock_image_ctx.old_format ? "snap_add" : "snapshot_add"),
                                        _,
                                        _,
                                        _,
                                        _));
    ...
    void expect_set_image_state(MockImageCtx& mock_image_ctx, MockSetImageStateRequest& mock_set_image_state_request,
                                int r)
    {
        EXPECT_CALL(mock_set_image_state_request, send())
            .WillOnce(FinishRequest(&mock_set_image_state_request, r, &mock_image_ctx));
    }
    ...
    void expect_update_snap_context(MockImageCtx& mock_image_ctx)
    {
        ...
        EXPECT_CALL(mock_image_ctx, get_snap_info(_)).WillOnce(Return(static_cast<const librbd::SnapInfo*>(NULL)));
...
TEST_F(TestMockOperationSnapshotCreateRequest, Success)
{
    ...
    MockSnapshotCreateRequest* req = new MockSnapshotCreateRequest(mock_image_ctx,
                                                                   &cond_ctx,
...
TEST_F(TestMockOperationSnapshotCreateRequest, NotifyQuiesceError)
{
    ...
    MockSnapshotCreateRequest* req = new MockSnapshotCreateRequest(
        mock_image_ctx, &cond_ctx, cls::rbd::UserSnapshotNamespace(), "snap1", 0, 0, prog_ctx);
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        req->send();
    }
...
TEST_F(TestMockOperationSnapshotCreateRequest, SetImageState)
{
    ...
    MockSnapshotCreateRequest* req = new MockSnapshotCreateRequest(
        mock_image_ctx,
        &cond_ctx,
        cls::rbd::MirrorSnapshotNamespace{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {}, "", CEPH_NOSNAP},
        "snap1",
        0,
        0,
        prog_ctx);
