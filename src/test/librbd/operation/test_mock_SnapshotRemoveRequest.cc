...
template<> class DetachChildRequest<MockImageCtx>
{
public:
    static DetachChildRequest* s_instance;
    static DetachChildRequest* create(MockImageCtx& image_ctx, Context* on_finish)
    {
...
template<> class RemoveImageStateRequest<MockImageCtx>
{
public:
    static RemoveImageStateRequest* s_instance;
    Context* on_finish = nullptr;

    static RemoveImageStateRequest* create(MockImageCtx* image_ctx, uint64_t snap_id, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->on_finish = on_finish;
        return s_instance;
    ...
    RemoveImageStateRequest() { s_instance = this; }
...
public:
    ...
    void expect_snapshot_trash_add(MockImageCtx& mock_image_ctx, int r)
    {
        ...
        auto& expect =
            EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                        exec(mock_image_ctx.header_oid, _, StrEq("rbd"), StrEq("snapshot_trash_add"), _, _, _, _));
        if (r < 0) {
    ...
    void expect_snapshot_get(MockImageCtx& mock_image_ctx, const cls::rbd::SnapshotInfo& snap_info, int r)
    {
        if (mock_image_ctx.old_format) {
            return;
        }
    ...
    void expect_children_list(MockImageCtx& mock_image_ctx, const cls::rbd::ChildImageSpecs& child_images, int r)
    {
        if (mock_image_ctx.old_format) {
            return;
        }
        ...
        using ceph::encode;
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    exec(mock_image_ctx.header_oid, _, StrEq("rbd"), StrEq("children_list"), _, _, _, _))
    ...
    void expect_detach_stale_child(MockImageCtx& mock_image_ctx, int r)
    {
        ...
        encode(cls::rbd::ChildImageSpec{mock_image_ctx.md_ctx.get_id(), "", mock_image_ctx.id}, bl);
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
    ...
    void expect_remove_image_state(MockImageCtx& mock_image_ctx,
                                   MockRemoveImageStateRequest& mock_remove_image_state_request, int r)
    {
        EXPECT_CALL(mock_remove_image_state_request, send())
            .WillOnce(FinishRequest(&mock_remove_image_state_request, r, &mock_image_ctx));
    }
    ...
    void expect_get_parent_spec(MockImageCtx& mock_image_ctx, int r)
    {
        ...
        if (r < 0) {
            ...
            expect.WillOnce(DoAll(SetArgPointee<1>(parent_spec), Return(0)));
        }
    }

    void expect_detach_child(MockImageCtx& mock_image_ctx, MockDetachChildRequest& mock_request, int r)
    {
        EXPECT_CALL(mock_request, send()).WillOnce(FinishRequest(&mock_request, r, &mock_image_ctx));
    }

    void expect_snap_remove(MockImageCtx& mock_image_ctx, int r)
    {
        auto& expect = EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                   exec(mock_image_ctx.header_oid,
                                        _,
                                        StrEq("rbd"),
                                        StrEq(mock_image_ctx.old_format ? "snap_remove" : "snapshot_remove"),
                                        _,
                                        _,
                                        _,
                                        _));
    ...
    void expect_rm_snap(MockImageCtx& mock_image_ctx) { EXPECT_CALL(mock_image_ctx, rm_snap(_, _, _)).Times(1); }

    void expect_release_snap_id(MockImageCtx& mock_image_ctx)
    {
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.data_ctx), selfmanaged_snap_remove(_)).WillOnce(DoDefault());
    }
};
...
TEST_F(TestMockOperationSnapshotRemoveRequest, Success)
{
    ...
    expect_snapshot_get(mock_image_ctx, {snap_id, {cls::rbd::UserSnapshotNamespace{}}, "snap1", 123, {}, 0}, 0);
    ...
    MockSnapshotRemoveRequest* req =
        new MockSnapshotRemoveRequest(mock_image_ctx, &cond_ctx, cls::rbd::UserSnapshotNamespace(), "snap1", snap_id);
    {
...
TEST_F(TestMockOperationSnapshotRemoveRequest, SuccessCloneParent)
{
    ...
    uint64_t snap_id = ictx->snap_info.rbegin()->first;
    expect_snapshot_get(mock_image_ctx, {snap_id, {cls::rbd::UserSnapshotNamespace{}}, "snap1", 123, {}, 1}, 0);
...
TEST_F(TestMockOperationSnapshotRemoveRequest, SuccessTrash)
{
    ...
    expect_snapshot_get(mock_image_ctx,
                        {snap_id,
                         {cls::rbd::TrashSnapshotNamespace{cls::rbd::SNAPSHOT_NAMESPACE_TYPE_USER, "snap1"}},
                         "snap1",
                         123,
                         {},
                         0},
...
TEST_F(TestMockOperationSnapshotRemoveRequest, TrashCloneParent)
{
    ...
    ASSERT_EQ(0, ictx->operations->snap_create({cls::rbd::TrashSnapshotNamespace{}}, "snap1", 0, prog_ctx));
    ASSERT_EQ(0, ictx->state->refresh_if_required());
    ...
    uint64_t snap_id = ictx->snap_info.rbegin()->first;
    expect_snapshot_get(mock_image_ctx, {snap_id, {cls::rbd::TrashSnapshotNamespace{}}, "snap1", 123, {}, 1}, 0);
    const cls::rbd::ChildImageSpecs child_images;
    expect_children_list(mock_image_ctx, child_images, 0);
    expect_get_parent_spec(mock_image_ctx, 0);
    ...
    C_SaferCond cond_ctx;
    MockSnapshotRemoveRequest* req =
        new MockSnapshotRemoveRequest(mock_image_ctx, &cond_ctx, cls::rbd::TrashSnapshotNamespace{}, "snap1", snap_id);
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        req->send();
    }
...
TEST_F(TestMockOperationSnapshotRemoveRequest, MirrorSnapshot)
{
    ...
    uint64_t snap_id = ictx->snap_info.rbegin()->first;
    cls::rbd::MirrorSnapshotNamespace ns{cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY, {}, "mirror uuid", 123};
    expect_snapshot_get(mock_image_ctx, {snap_id, {ns}, "mirror", 456, {}, 0}, 0);
    ...
    C_SaferCond cond_ctx;
    MockSnapshotRemoveRequest* req = new MockSnapshotRemoveRequest(mock_image_ctx, &cond_ctx, ns, "mirror", snap_id);
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        req->send();
    }
...
TEST_F(TestMockOperationSnapshotRemoveRequest, SnapshotGetError)
{
    ...
    uint64_t snap_id = ictx->snap_info.rbegin()->first;
    expect_snapshot_get(
        mock_image_ctx, {snap_id, {cls::rbd::UserSnapshotNamespace{}}, "snap1", 123, {}, 0}, -EOPNOTSUPP);
