...
TEST_F(TestMockOperationSnapshotRemoveRequest, MirrorSnapshot)
{
    ...
    uint64_t snap_id = ictx->snap_info.rbegin()->first;
    expect_snapshot_get(mock_image_ctx, {snap_id, {cls::rbd::MirrorSnapshotNamespace{}}, "mirror", 123, {}, 0}, 0);
    ...
    MockSnapshotRemoveRequest* req = new MockSnapshotRemoveRequest(
        mock_image_ctx, &cond_ctx, cls::rbd::MirrorSnapshotNamespace(), "mirror", snap_id);
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        req->send();
    }
