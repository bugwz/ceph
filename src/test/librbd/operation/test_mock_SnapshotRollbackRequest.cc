...
template<> struct ResizeRequest<MockOperationImageCtx>
{
    static ResizeRequest* s_instance;
    Context* on_finish = nullptr;

    static ResizeRequest* create(MockOperationImageCtx& image_ctx, Context* on_finish, uint64_t new_size,
...
public:
    ...
    void expect_get_image_size(MockOperationImageCtx& mock_image_ctx, uint64_t size)
    {
        EXPECT_CALL(mock_image_ctx, get_image_size(CEPH_NOSNAP)).WillOnce(Return(size));
    }

    void expect_resize(MockOperationImageCtx& mock_image_ctx, MockResizeRequest& mock_resize_request, int r)
    {
        expect_get_image_size(mock_image_ctx, 123);
        EXPECT_CALL(mock_resize_request, send()).WillOnce(FinishRequest(&mock_resize_request, r, &mock_image_ctx));
    }

    void expect_get_flags(MockOperationImageCtx& mock_image_ctx, uint64_t snap_id, int r)
    {
        EXPECT_CALL(mock_image_ctx, get_flags(snap_id, _)).WillOnce(Return(r));
    }

    void expect_object_may_exist(MockOperationImageCtx& mock_image_ctx, uint64_t object_no, bool exists)
    {
    ...
    void expect_get_snap_object_map(MockOperationImageCtx& mock_image_ctx, MockObjectMap* mock_object_map,
                                    uint64_t snap_id)
    {
        if (mock_image_ctx.object_map != nullptr) {
            EXPECT_CALL(mock_image_ctx, create_object_map(snap_id)).WillOnce(Return(mock_object_map));
            EXPECT_CALL(*mock_object_map, open(_))
                .WillOnce(CompleteContext(0, mock_image_ctx.image_ctx->op_work_queue));
        }
    }

    void expect_rollback_object_map(MockOperationImageCtx& mock_image_ctx, MockObjectMap& mock_object_map)
    {
        if (mock_image_ctx.object_map != nullptr) {
    ...
    void expect_get_object_name(MockOperationImageCtx& mock_image_ctx, uint64_t object_num)
    {
    ...
    void expect_rollback_snap_id(MockOperationImageCtx& mock_image_ctx, const std::string& oid, int r)
    {
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.data_ctx), selfmanaged_snap_rollback(oid, _)).WillOnce(Return(r));
    }
    ...
    void expect_create_object_map(MockOperationImageCtx& mock_image_ctx, MockObjectMap* mock_object_map)
    {
        EXPECT_CALL(mock_image_ctx, create_object_map(_)).WillOnce(Return(mock_object_map));
    }

    void expect_open_object_map(MockOperationImageCtx& mock_image_ctx, MockObjectMap& mock_object_map)
    {
        EXPECT_CALL(mock_object_map, open(_)).WillOnce(CompleteContext(0, mock_image_ctx.image_ctx->op_work_queue));
    }

    void expect_refresh_object_map(MockOperationImageCtx& mock_image_ctx, MockObjectMap& mock_object_map)
    {
        if (mock_image_ctx.object_map != nullptr) {
    ...
    int when_snap_rollback(MockOperationImageCtx& mock_image_ctx, const std::string& snap_name, uint64_t snap_id,
                           uint64_t snap_size)
    {
        C_SaferCond cond_ctx;
        librbd::NoOpProgressContext prog_ctx;
        MockSnapshotRollbackRequest* req = new MockSnapshotRollbackRequest(
            mock_image_ctx, &cond_ctx, cls::rbd::UserSnapshotNamespace(), snap_name, snap_id, snap_size, prog_ctx);
        {
