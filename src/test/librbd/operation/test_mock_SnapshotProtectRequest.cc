    ...
    void expect_set_protection_status(MockImageCtx& mock_image_ctx, int r)
    {
        auto& expect =
            EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                        exec(mock_image_ctx.header_oid, _, StrEq("rbd"), StrEq("set_protection_status"), _, _, _, _));
        if (r < 0) {
            expect.WillOnce(Return(r));
        }
        else {
...
TEST_F(TestMockOperationSnapshotProtectRequest, Success)
{
    ...
    MockSnapshotProtectRequest* req =
        new MockSnapshotProtectRequest(mock_image_ctx, &cond_ctx, cls::rbd::UserSnapshotNamespace(), "snap1");
    {
