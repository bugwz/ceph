...
template<> struct Journal<MockOperationImageCtx>
{
...
public:
    ...
    static SetFlagsRequest* create(MockOperationImageCtx* image_ctx, uint64_t flags, uint64_t mask, Context* on_finish)
    {
    ...
    SetFlagsRequest() { s_instance = this; }
...
template<> class CreateRequest<MockOperationImageCtx>
{
public:
    ...
    static CreateRequest* create(IoCtx& ioctx, const std::string& imageid, uint8_t order, uint8_t splay_width,
                                 ...
                                 const std::string& client_id, MockContextWQ* op_work_queue, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->on_finish = on_finish;
        return s_instance;
...
template<> struct TypeTraits<MockOperationImageCtx>
{
...
template<> class EnableRequest<MockOperationImageCtx>
{
public:
    static EnableRequest* s_instance;
    Context* on_finish = nullptr;

    static EnableRequest* create(MockOperationImageCtx* image_ctx, cls::rbd::MirrorImageMode mirror_image_mode,
...
template<> struct AsyncRequest<MockOperationImageCtx> : public AsyncRequest<MockImageCtx>
{
...
public:
    ...
    public:
        ...
        ~MirrorModeEnabler()
        {
            EXPECT_EQ(0, librbd::cls_client::mirror_mode_set(&m_ioctx, cls::rbd::MIRROR_MODE_DISABLED));
        }
    ...
    void ensure_features_disabled(librbd::ImageCtx* ictx, uint64_t features_to_disable)
    {
        uint64_t features;
    ...
    {
        EXPECT_CALL(*mock_image_ctx.state, prepare_lock(_)).WillOnce(Invoke([](Context* on_ready) {
            on_ready->complete(0);
        }));
    ...
    void expect_set_flags_request_send(MockOperationImageCtx& mock_image_ctx,
                                       MockSetFlagsRequest& mock_set_flags_request, int r)
    {
        EXPECT_CALL(mock_set_flags_request, send())
            .WillOnce(FinishRequest(&mock_set_flags_request, r, &mock_image_ctx));
    }

    void expect_create_journal_request_send(MockOperationImageCtx& mock_image_ctx,
                                            MockCreateJournalRequest& mock_create_journal_request, int r)
    {
        EXPECT_CALL(mock_create_journal_request, send())
            .WillOnce(FinishRequest(&mock_create_journal_request, r, &mock_image_ctx));
    }

    void expect_enable_mirror_request_send(MockOperationImageCtx& mock_image_ctx,
                                           MockEnableMirrorRequest& mock_enable_mirror_request, int r)
    {
        EXPECT_CALL(mock_enable_mirror_request, send())
            .WillOnce(FinishRequest(&mock_enable_mirror_request, r, &mock_image_ctx));
    }

    void expect_create_object_map_request_send(MockOperationImageCtx& mock_image_ctx,
                                               MockCreateObjectMapRequest& mock_create_object_map_request, int r)
    {
        EXPECT_CALL(mock_create_object_map_request, send())
            .WillOnce(FinishRequest(&mock_create_object_map_request, r, &mock_image_ctx));
    }
...
TEST_F(TestMockOperationEnableFeaturesRequest, All)
{
    ...
    if (features_to_enable & RBD_FEATURE_JOURNALING) {
        expect_create_journal_request_send(mock_image_ctx, mock_create_journal_request, 0);
    }
    if (features_to_enable & (RBD_FEATURE_OBJECT_MAP | RBD_FEATURE_FAST_DIFF)) {
        expect_set_flags_request_send(mock_image_ctx, mock_set_flags_request, 0);
    }
    if (features_to_enable & RBD_FEATURE_OBJECT_MAP) {
        expect_create_object_map_request_send(mock_image_ctx, mock_create_object_map_request, 0);
    }
    ...
    MockEnableFeaturesRequest* req = new MockEnableFeaturesRequest(mock_image_ctx, &cond_ctx, 0, features_to_enable);
    {
...
TEST_F(TestMockOperationEnableFeaturesRequest, ObjectMap)
{
    ...
    ensure_features_disabled(ictx, RBD_FEATURE_OBJECT_MAP | RBD_FEATURE_FAST_DIFF);
    ...
    C_SaferCond cond_ctx;
    MockEnableFeaturesRequest* req =
        new MockEnableFeaturesRequest(mock_image_ctx, &cond_ctx, 0, RBD_FEATURE_OBJECT_MAP);
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        req->send();
    }
...
TEST_F(TestMockOperationEnableFeaturesRequest, ObjectMapError)
{
    ...
    expect_append_op_event(mock_image_ctx, true, 0);
    expect_set_flags_request_send(mock_image_ctx, mock_set_flags_request, 0);
    expect_create_object_map_request_send(mock_image_ctx, mock_create_object_map_request, -EINVAL);
    expect_unblock_requests(mock_image_ctx);
    expect_unblock_writes(mock_image_ctx);
    expect_handle_prepare_lock_complete(mock_image_ctx);
...
TEST_F(TestMockOperationEnableFeaturesRequest, SetFlagsError)
{
    ...
    expect_block_requests(mock_image_ctx);
    expect_append_op_event(mock_image_ctx, true, 0);
    expect_set_flags_request_send(mock_image_ctx, mock_set_flags_request, -EINVAL);
    expect_unblock_requests(mock_image_ctx);
    expect_unblock_writes(mock_image_ctx);
    expect_handle_prepare_lock_complete(mock_image_ctx);
    expect_commit_op_event(mock_image_ctx, -EINVAL);
...
TEST_F(TestMockOperationEnableFeaturesRequest, Mirroring)
{
    ...
    expect_block_requests(mock_image_ctx);
    expect_create_journal_request_send(mock_image_ctx, mock_create_journal_request, 0);
    expect_enable_mirror_request_send(mock_image_ctx, mock_enable_mirror_request, 0);
    expect_notify_update(mock_image_ctx);
    expect_unblock_requests(mock_image_ctx);
    expect_unblock_writes(mock_image_ctx);
    expect_handle_prepare_lock_complete(mock_image_ctx);
    ...
    C_SaferCond cond_ctx;
    MockEnableFeaturesRequest* req =
        new MockEnableFeaturesRequest(mock_image_ctx, &cond_ctx, 0, RBD_FEATURE_JOURNALING);
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        req->send();
    }
...
TEST_F(TestMockOperationEnableFeaturesRequest, JournalingError)
{
    ...
    expect_block_writes(mock_image_ctx);
    expect_block_requests(mock_image_ctx);
    expect_create_journal_request_send(mock_image_ctx, mock_create_journal_request, -EINVAL);
    expect_unblock_requests(mock_image_ctx);
    expect_unblock_writes(mock_image_ctx);
    expect_handle_prepare_lock_complete(mock_image_ctx);
...
TEST_F(TestMockOperationEnableFeaturesRequest, MirroringError)
{
    ...
    expect_block_requests(mock_image_ctx);
    expect_create_journal_request_send(mock_image_ctx, mock_create_journal_request, 0);
    expect_enable_mirror_request_send(mock_image_ctx, mock_enable_mirror_request, -EINVAL);
    expect_notify_update(mock_image_ctx);
    expect_unblock_requests(mock_image_ctx);
    expect_unblock_writes(mock_image_ctx);
    expect_handle_prepare_lock_complete(mock_image_ctx);
