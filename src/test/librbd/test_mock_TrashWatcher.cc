...
struct MockTrashWatcher : public TrashWatcher<>
{
    ...
    MOCK_METHOD2(handle_image_added, void(const std::string&, const cls::rbd::TrashImageSpec&));
...
TEST_F(TestTrashWatcher, ImageAdded)
{
    ...
    cls::rbd::TrashImageSpec trash_image_spec{
        cls::rbd::TRASH_IMAGE_SOURCE_USER, "image name", ceph_clock_now(), ceph_clock_now()};

    EXPECT_CALL(*m_trash_watcher, handle_image_added(StrEq("image id"), trash_image_spec)).Times(AtLeast(1));

    C_SaferCond ctx;
    MockTrashWatcher::notify_image_added(m_ioctx, "image id", trash_image_spec, &ctx);
    ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(TestTrashWatcher, ImageRemoved)
{
    ...
    EXPECT_CALL(*m_trash_watcher, handle_image_removed(StrEq("image id"))).Times(AtLeast(1));
