...
{
    MockJournalImageCtx(librbd::ImageCtx& image_ctx)
...
template<> struct TypeTraits<MockJournalImageCtx>
{
...
template<> class Replay<MockJournalImageCtx>
{
public:
    static Replay* create(MockJournalImageCtx& image_ctx) { return new Replay(); }

    void shut_down(bool cancel_ops, Context* on_finish) { MockReplay::get_instance().shut_down(cancel_ops, on_finish); }
    ...
    void process(const EventEntry& event_entry, Context* on_ready, Context* on_commit)
    {
...
struct MockRemove
{
    ...
    MockRemove() { s_instance = this; }

    MOCK_METHOD0(send, void());
};

template<> class RemoveRequest<MockJournalImageCtx>
{
public:
    static RemoveRequest* create(IoCtx& ioctx, const std::string& imageid, const std::string& client_id,
                                 ContextWQ* op_work_queue, Context* on_finish)
    {
        return new RemoveRequest();
    }

    void send() { MockRemove::get_instance().send(); }
};
...
struct MockCreate
{
    ...
    MockCreate() { s_instance = this; }

    MOCK_METHOD0(send, void());
};

template<> class CreateRequest<MockJournalImageCtx>
{
public:
    static CreateRequest* create(IoCtx& ioctx, const std::string& imageid, uint8_t order, uint8_t splay_width,
                                 const std::string& object_pool, uint64_t tag_class, TagData& tag_data,
                                 const std::string& client_id, ContextWQ* op_work_queue, Context* on_finish)
    {
        return new CreateRequest();
    }

    void send() { MockCreate::get_instance().send(); }
};

MockCreate* MockCreate::s_instance = nullptr;

template<> class OpenRequest<MockJournalImageCtx>
{
public:
    TagData* tag_data;
    Context* on_finish;
    static OpenRequest* s_instance;
    static OpenRequest* create(MockJournalImageCtx* image_ctx, ::journal::MockJournalerProxy* journaler,
                               ceph::mutex* lock, journal::ImageClientMeta* client_meta, uint64_t* tag_tid,
...
template<> class PromoteRequest<MockJournalImageCtx>
{
public:
    static PromoteRequest s_instance;
    static PromoteRequest* create(MockJournalImageCtx* image_ctx, bool force, Context* on_finish)
    {
        return &s_instance;
    }
...
template<> struct ObjectDispatch<MockJournalImageCtx> : public io::MockObjectDispatch
{
    static ObjectDispatch* s_instance;

    static ObjectDispatch* create(MockJournalImageCtx* image_ctx, Journal<MockJournalImageCtx>* journal)
    {
        ceph_assert(s_instance != nullptr);
        return s_instance;
    }

    ObjectDispatch() { s_instance = this; }
};
...
public:
    ...
    ~TestMockJournal() override { ceph_assert(m_commit_contexts.empty()); }
    ...
    void expect_open_journaler(MockImageCtx& mock_image_ctx, ::journal::MockJournaler& mock_journaler,
                               MockJournalOpenRequest& mock_open_request, bool primary, int r)
    {
        ...
        EXPECT_CALL(mock_open_request, send())
            .WillOnce(DoAll(Invoke([&mock_open_request, primary]() {
                            ...
                            FinishRequest(&mock_open_request, r, &mock_image_ctx)));
    }

    void expect_shut_down_journaler(::journal::MockJournaler& mock_journaler)
    {
        EXPECT_CALL(mock_journaler, remove_listener(_));
        EXPECT_CALL(mock_journaler, shut_down(_)).WillOnce(CompleteContext(0, static_cast<asio::ContextWQ*>(NULL)));
    }

    void expect_register_dispatch(MockImageCtx& mock_image_ctx, MockObjectDispatch& mock_object_dispatch)
    {
        EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, register_dispatch(&mock_object_dispatch));
    }

    void expect_shut_down_dispatch(MockImageCtx& mock_image_ctx)
    {
        EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, shut_down_dispatch(io::OBJECT_DISPATCH_LAYER_JOURNAL, _))
            .WillOnce(WithArg<1>(CompleteContext(0, mock_image_ctx.image_ctx->op_work_queue)));
    }

    void expect_get_max_append_size(::journal::MockJournaler& mock_journaler, uint32_t max_size)
    {
        EXPECT_CALL(mock_journaler, get_max_append_size()).WillOnce(Return(max_size));
    }

    void expect_get_journaler_cached_client(::journal::MockJournaler& mock_journaler,
                                            const journal::ImageClientMeta& client_meta, int r)
    {
        ...
        EXPECT_CALL(mock_journaler, get_cached_client("", _)).WillOnce(DoAll(SetArgPointee<1>(client), Return(r)));
    }

    void expect_get_journaler_tags(MockImageCtx& mock_image_ctx, ::journal::MockJournaler& mock_journaler,
                                   uint64_t start_after_tag_tid, ::journal::Journaler::Tags&& tags, int r)
    {
        EXPECT_CALL(mock_journaler, get_tags(start_after_tag_tid, 0, _, _))
            .WillOnce(
                DoAll(SetArgPointee<2>(tags), WithArg<3>(CompleteContext(r, mock_image_ctx.image_ctx->op_work_queue))));
    }

    void expect_start_replay(MockJournalImageCtx& mock_image_ctx, ::journal::MockJournaler& mock_journaler,
                             const ReplayAction& action)
    {
        EXPECT_CALL(mock_journaler, start_replay(_))
            .WillOnce(DoAll(
                SaveArg<0>(&m_replay_handler),
                StartReplay(mock_image_ctx.image_ctx->op_work_queue, new C_ReplayAction(&m_replay_handler, action))));
    }

    void expect_stop_replay(::journal::MockJournaler& mock_journaler)
    {
        EXPECT_CALL(mock_journaler, stop_replay(_)).WillOnce(CompleteContext(0, static_cast<asio::ContextWQ*>(NULL)));
    }

    void expect_shut_down_replay(MockJournalImageCtx& mock_image_ctx, MockJournalReplay& mock_journal_replay, int r,
                                 bool cancel_ops = false)
    {
        EXPECT_CALL(mock_journal_replay, shut_down(cancel_ops, _))
            .WillOnce(WithArg<1>(Invoke(
                [this, &mock_image_ctx, r](Context* on_flush) { this->commit_replay(mock_image_ctx, on_flush, r); })));
    }

    void expect_get_data(::journal::MockReplayEntry& mock_replay_entry)
    {
        EXPECT_CALL(mock_replay_entry, get_data()).WillOnce(Return(bufferlist()));
    }

    void expect_try_pop_front(MockJournalImageCtx& mock_image_ctx, ::journal::MockJournaler& mock_journaler,
                              bool entries_available, ::journal::MockReplayEntry& mock_replay_entry,
                              const ReplayAction& action = {})
    {
        EXPECT_CALL(mock_journaler, try_pop_front(_))
            .WillOnce(DoAll(
                SetArgPointee<0>(::journal::MockReplayEntryProxy()),
                StartReplay(mock_image_ctx.image_ctx->op_work_queue, new C_ReplayAction(&m_replay_handler, action)),
    ...
    {
        EXPECT_CALL(mock_journal_replay, decode(_, _)).WillOnce(Return(0));
    ...
    void expect_start_append(::journal::MockJournaler& mock_journaler) { EXPECT_CALL(mock_journaler, start_append(_)); }

    void expect_set_append_batch_options(MockJournalImageCtx& mock_image_ctx, ::journal::MockJournaler& mock_journaler,
                                         bool user_flushed)
    {
    ...
    {
        EXPECT_CALL(mock_journaler, stop_append(_)).WillOnce(CompleteContext(r, static_cast<asio::ContextWQ*>(NULL)));
    }

    void expect_committed(::journal::MockJournaler& mock_journaler, size_t events)
    {
        EXPECT_CALL(mock_journaler, committed(MatcherCast<const ::journal::MockReplayEntryProxy&>(_))).Times(events);
    }

    void expect_append_journaler(::journal::MockJournaler& mock_journaler)
    {
        EXPECT_CALL(mock_journaler, append(_, _)).WillOnce(Return(::journal::MockFutureProxy()));
    }

    void expect_wait_future(::journal::MockFuture& mock_future, Context** on_safe)
    {
        EXPECT_CALL(mock_future, wait(_)).WillOnce(SaveArg<0>(on_safe));
    }
    ...
    uint64_t when_append_write_event(MockJournalImageCtx& mock_image_ctx, MockJournal* mock_journal, uint64_t length)
    {
        bufferlist bl;
    ...
    uint64_t when_append_compare_and_write_event(MockJournalImageCtx& mock_image_ctx, MockJournal* mock_journal,
                                                 uint64_t length)
    {
        bufferlist cmp_bl;
        ...
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        return mock_journal->append_compare_and_write_event(0, length, cmp_bl, write_bl, false);
    }

    uint64_t when_append_io_event(MockJournalImageCtx& mock_image_ctx, MockJournal* mock_journal, int filter_ret_val)
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
    ...
    void commit_replay(MockJournalImageCtx& mock_image_ctx, Context* on_flush, int r)
    {
    ...
    void open_journal(MockJournalImageCtx& mock_image_ctx, MockJournal* mock_journal,
                      MockObjectDispatch& mock_object_dispatch, ::journal::MockJournaler& mock_journaler,
                      MockJournalOpenRequest& mock_open_request, bool primary = true)
    {
        ...
        expect_open_journaler(mock_image_ctx, mock_journaler, mock_open_request, primary, 0);
        expect_get_max_append_size(mock_journaler, 1 << 16);
        expect_start_replay(mock_image_ctx, mock_journaler, std::bind(&invoke_replay_complete, _1, 0));
    ...
    void close_journal(MockJournalImageCtx& mock_image_ctx, MockJournal* mock_journal,
    ...
    static void invoke_replay_complete(::journal::ReplayHandler* handler, int r) { handler->handle_complete(r); }
...
TEST_F(TestMockJournal, StateTransitions)
{
    ...
    expect_construct_journaler(mock_journaler);
    expect_open_journaler(mock_image_ctx, mock_journaler, mock_open_request, true, 0);
    expect_get_max_append_size(mock_journaler, 1 << 16);
    expect_start_replay(mock_image_ctx, mock_journaler, std::bind(&invoke_replay_ready, _1));
    ...
    expect_try_pop_front(mock_image_ctx, mock_journaler, true, mock_replay_entry);
    expect_replay_process(mock_journal_replay);
    expect_try_pop_front(mock_image_ctx, mock_journaler, false, mock_replay_entry, std::bind(&invoke_replay_ready, _1));
    expect_try_pop_front(mock_image_ctx, mock_journaler, true, mock_replay_entry);
    expect_replay_process(mock_journal_replay);
    expect_try_pop_front(
        mock_image_ctx, mock_journaler, false, mock_replay_entry, std::bind(&invoke_replay_complete, _1, 0));
...
TEST_F(TestMockJournal, InitError)
{
    ...
    MockJournalOpenRequest mock_open_request;
    expect_construct_journaler(mock_journaler);
    expect_open_journaler(mock_image_ctx, mock_journaler, mock_open_request, true, -EINVAL);
    expect_shut_down_journaler(mock_journaler);
...
TEST_F(TestMockJournal, ReplayCompleteError)
{
    ...
    expect_open_journaler(mock_image_ctx, mock_journaler, mock_open_request, true, 0);
    expect_get_max_append_size(mock_journaler, 1 << 16);
    expect_start_replay(mock_image_ctx, mock_journaler, std::bind(&invoke_replay_complete, _1, -EINVAL));
...
TEST_F(TestMockJournal, ReplayOnDiskPreFlushError)
{
    ...
    Context* on_ready;
    EXPECT_CALL(mock_journal_replay, process(_, _, _))
        .WillOnce(DoAll(SaveArg<1>(&on_ready), WithArg<2>(Invoke(this, &TestMockJournal::save_commit_context))));
...
TEST_F(TestMockJournal, ReplayOnDiskPostFlushError)
{
    ...
    EXPECT_CALL(mock_journal_replay, shut_down(false, _))
        .WillOnce(DoAll(SaveArg<1>(&on_flush), InvokeWithoutArgs(this, &TestMockJournal::wake_up)));
    expect_flush_commit_position(mock_journaler);
...
TEST_F(TestMockJournal, EventAndIOCommitOrder)
{
    ...
    ::journal::MockJournaler mock_journaler;
    MockJournalOpenRequest mock_open_request;
    open_journal(mock_image_ctx, mock_journal, mock_object_dispatch, mock_journaler, mock_open_request);
...
TEST_F(TestMockJournal, AppendCompareAndWriteEvent)
{
    ...
    expect_append_journaler(mock_journaler);
    expect_wait_future(mock_future, &on_journal_safe);
    ASSERT_EQ(1U, when_append_compare_and_write_event(mock_image_ctx, mock_journal, 1 << 16));
    mock_journal->get_work_queue()->drain();
...
TEST_F(TestMockJournal, AppendDisabled)
{
    ...
    EXPECT_CALL(mock_image_ctx, get_journal_policy()).WillOnce(Return(ictx->get_journal_policy()));
    ASSERT_TRUE(mock_journal->is_journal_appending());

    EXPECT_CALL(mock_image_ctx, get_journal_policy()).WillOnce(Return(&mock_journal_policy));
...
TEST_F(TestMockJournal, CloseListenerEvent)
{
    ...
    {
        C_SaferCond ctx;
        void handle_close() override { ctx.complete(0); }
        void handle_resync() override { ADD_FAILURE() << "unexpected resync request"; }
        void handle_promoted() override { ADD_FAILURE() << "unexpected promotion event"; }
    } listener;
...
TEST_F(TestMockJournal, ResyncRequested)
{
    ...
    ::journal::MockJournaler mock_journaler;
    MockJournalOpenRequest mock_open_request;
    open_journal(mock_image_ctx, mock_journal, mock_object_dispatch, mock_journaler, mock_open_request, false);

    struct Listener : public journal::Listener
    {
        C_SaferCond ctx;
        void handle_close() override { ADD_FAILURE() << "unexpected close action"; }
        void handle_resync() override { ctx.complete(0); }
        void handle_promoted() override { ADD_FAILURE() << "unexpected promotion event"; }
    } listener;
    mock_journal->add_listener(&listener);
    ...
    expect_get_journaler_tags(mock_image_ctx, mock_journaler, 0, {{0, 0, tag_data_bl}}, 0);
...
TEST_F(TestMockJournal, ForcePromoted)
{
    ...
    struct Listener : public journal::Listener
    {
        C_SaferCond ctx;
        void handle_close() override { ADD_FAILURE() << "unexpected close action"; }
        void handle_resync() override { ADD_FAILURE() << "unexpected resync event"; }
        void handle_promoted() override { ctx.complete(0); }
    } listener;
    mock_journal->add_listener(&listener);
    ...
    bufferlist tag_data_bl;
    encode(tag_data, tag_data_bl);
    expect_get_journaler_tags(mock_image_ctx, mock_journaler, 0, {{100, 0, tag_data_bl}}, 0);
