...
void scribble(librbd::ImageCtx* image_ctx, int num_ops, size_t max_size, interval_set<uint64_t>* what)
{
...
MATCHER_P2(IsRead, snap_id, image_interval, "")
{
    ...
    if (req == nullptr || arg->io_context->read_snap().value_or(CEPH_NOSNAP) != snap_id) {
        return false;
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        m_asio_engine = std::make_shared<librbd::AsioEngine>(m_src_image_ctx->md_ctx);
    ...
    void prepare_exclusive_lock(librbd::MockImageCtx& mock_image_ctx, librbd::MockExclusiveLock& mock_exclusive_lock)
    {
    ...
    void expect_list_snaps(librbd::MockTestImageCtx& mock_image_ctx, int r)
    {
        EXPECT_CALL(*mock_image_ctx.io_image_dispatcher, send(IsListSnaps()))
            .WillOnce(Invoke([&mock_image_ctx, r](io::ImageDispatchSpec* spec) {
                ...
                spec->image_dispatcher = mock_image_ctx.image_ctx->io_image_dispatcher;
    ...
    {
        EXPECT_CALL(mock_image_ctx, get_object_name(0)).WillOnce(Return(mock_image_ctx.image_ctx->get_object_name(0)));
    }

    MockObjectCopyRequest* create_request(librbd::MockTestImageCtx& mock_src_image_ctx,
                                          librbd::MockTestImageCtx& mock_dst_image_ctx,
                                          librados::snap_t src_snap_id_start, librados::snap_t src_snap_id_end,
                                          librados::snap_t dst_snap_id_start, uint32_t flags, Context* on_finish)
    {
        SnapMap snap_map;
        util::compute_snap_map(
            mock_dst_image_ctx.cct, src_snap_id_start, src_snap_id_end, m_dst_snap_ids, m_snap_seqs, &snap_map);

        expect_get_object_name(mock_dst_image_ctx);
        return new MockObjectCopyRequest(&mock_src_image_ctx,
                                         &mock_dst_image_ctx,
                                         src_snap_id_start,
                                         dst_snap_id_start,
                                         snap_map,
    ...
    void expect_read(librbd::MockTestImageCtx& mock_image_ctx, uint64_t snap_id, uint64_t offset, uint64_t length,
                     int r)
    {
    ...
    void expect_read(librbd::MockTestImageCtx& mock_image_ctx, uint64_t snap_id, const interval_set<uint64_t>& extents,
                     int r)
    {
        EXPECT_CALL(*mock_image_ctx.io_image_dispatcher, send(IsRead(snap_id, extents)))
            .WillOnce(Invoke([&mock_image_ctx, r](io::ImageDispatchSpec* spec) {
                if (r < 0) {
                    spec->fail(r);
                    return;
    ...
    void expect_write(librados::MockTestMemIoCtxImpl& mock_io_ctx, uint64_t offset, uint64_t length,
    ...
    void expect_write(librados::MockTestMemIoCtxImpl& mock_io_ctx, const interval_set<uint64_t>& extents,
                      const SnapContext& snapc, int r)
    {
    ...
    void expect_truncate(librados::MockTestMemIoCtxImpl& mock_io_ctx, uint64_t offset, int r)
    {
    ...
    void expect_update_object_map(librbd::MockTestImageCtx& mock_image_ctx, librbd::MockObjectMap& mock_object_map,
                                  librados::snap_t snap_id, uint8_t state, int r)
    {
        if (mock_image_ctx.image_ctx->object_map != nullptr) {
            ...
            if (r < 0) {
                expect.WillOnce(
                    DoAll(WithArg<7>(Invoke([this, r](Context* ctx) { m_work_queue->queue(ctx, r); })), Return(true)));
            }
            else {
    ...
    int create_snap(librbd::ImageCtx* image_ctx, const char* snap_name, librados::snap_t* snap_id)
    {
        NoOpProgressContext prog_ctx;
        int r = image_ctx->operations->snap_create(cls::rbd::UserSnapshotNamespace(), snap_name, 0, prog_ctx);
        if (r < 0) {
            return r;
        }
        ...
        if (image_ctx->snap_ids.count({cls::rbd::UserSnapshotNamespace(), snap_name}) == 0) {
            return -ENOENT;
        }

        if (snap_id != nullptr) {
            *snap_id = image_ctx->snap_ids[{cls::rbd::UserSnapshotNamespace(), snap_name}];
        }
        return 0;
    }
    ...
    int create_snap(const char* snap_name)
    {
        ...
        if (!m_snap_map.empty()) {
            dst_snap_ids.insert(
                dst_snap_ids.end(), m_snap_map.rbegin()->second.begin(), m_snap_map.rbegin()->second.end());
        }
    ...
    std::string get_snap_name(librbd::ImageCtx* image_ctx, librados::snap_t snap_id)
    {
        auto it = std::find_if(
            image_ctx->snap_ids.begin(),
    ...
    int copy_objects()
    {
        ...
        r = api::Io<>::read(*m_src_image_ctx, 0, object_size, librbd::io::ReadResult{&bl}, 0);
        if (r < 0) {
            return r;
        }
    ...
    int compare_objects()
    {
        ...
        while (!snap_map.empty()) {
            ...
            std::cout << "comparing '" << snap_name << " (" << src_snap_id << " to " << dst_snap_id << ")" << std::endl;

            r = librbd::api::Image<>::snap_set(m_src_image_ctx, cls::rbd::UserSnapshotNamespace(), snap_name.c_str());
            if (r < 0) {
                return r;
            }

            r = librbd::api::Image<>::snap_set(m_dst_image_ctx, cls::rbd::UserSnapshotNamespace(), snap_name.c_str());
            if (r < 0) {
                return r;
            }
            ...
            r = api::Io<>::read(*m_src_image_ctx, 0, object_size, librbd::io::ReadResult{&src_bl}, 0);
            if (r < 0) {
                return r;
            }
            ...
            r = api::Io<>::read(*m_dst_image_ctx, 0, object_size, librbd::io::ReadResult{&dst_bl}, 0);
            if (r < 0) {
                return r;
            }

            if (!src_bl.contents_equal(dst_bl)) {
                std::cout << "src block: " << std::endl;
                src_bl.hexdump(std::cout);
                std::cout << "dst block: " << std::endl;
                dst_bl.hexdump(std::cout);
                return -EBADMSG;
            }
        }

        r = librbd::api::Image<>::snap_set(m_src_image_ctx, cls::rbd::UserSnapshotNamespace(), nullptr);
        if (r < 0) {
            return r;
        }
        r = librbd::api::Image<>::snap_set(m_dst_image_ctx, cls::rbd::UserSnapshotNamespace(), nullptr);
        if (r < 0) {
            return r;
        }
...
TEST_F(TestMockDeepCopyObjectCopyRequest, DNE)
{
    ...
    C_SaferCond ctx;
    MockObjectCopyRequest* request = create_request(mock_src_image_ctx, mock_dst_image_ctx, 0, CEPH_NOSNAP, 0, 0, &ctx);
...
TEST_F(TestMockDeepCopyObjectCopyRequest, Write)
{
    ...
    librados::MockTestMemIoCtxImpl& mock_dst_io_ctx(get_mock_io_ctx(request->get_dst_io_ctx()));

    InSequence seq;
    expect_list_snaps(mock_src_image_ctx, 0);
    expect_read(mock_src_image_ctx, m_src_snap_ids[0], 0, one.range_end(), 0);
    expect_start_op(mock_exclusive_lock);
    expect_update_object_map(mock_dst_image_ctx, mock_object_map, m_dst_snap_ids[0], OBJECT_EXISTS, 0);
...
TEST_F(TestMockDeepCopyObjectCopyRequest, ReadError)
{
    ...
    InSequence seq;
    expect_list_snaps(mock_src_image_ctx, 0);
    expect_read(mock_src_image_ctx, m_src_snap_ids[0], 0, one.range_end(), -EINVAL);
...
TEST_F(TestMockDeepCopyObjectCopyRequest, WriteSnaps)
{
    ...
    expect_update_object_map(mock_dst_image_ctx, mock_object_map, m_dst_snap_ids[0], OBJECT_EXISTS, 0);
    expect_start_op(mock_exclusive_lock);
    expect_update_object_map(mock_dst_image_ctx, mock_object_map, m_dst_snap_ids[1], OBJECT_EXISTS, 0);
    expect_start_op(mock_exclusive_lock);
    expect_update_object_map(mock_dst_image_ctx,
                             mock_object_map,
                             m_dst_snap_ids[2],
                             is_fast_diff(mock_dst_image_ctx) ? OBJECT_EXISTS_CLEAN : OBJECT_EXISTS,
                             0);
    expect_prepare_copyup(mock_dst_image_ctx);
    expect_start_op(mock_exclusive_lock);
    expect_write(mock_dst_io_ctx, 0, one.range_end(), {0, {}}, 0);
    expect_start_op(mock_exclusive_lock);
    expect_write(mock_dst_io_ctx, two, {m_dst_snap_ids[0], {m_dst_snap_ids[0]}}, 0);
...
{
    ASSERT_EQ(0, m_src_image_ctx->operations->metadata_set("conf_rbd_skip_partial_discard", "false"));
    ...
    ASSERT_LE(
        0,
        api::Io<>::discard(
            *m_src_image_ctx, trim_offset, one.range_end() - trim_offset, m_src_image_ctx->discard_granularity_bytes));
    ASSERT_EQ(0, create_snap("copy"));
...
TEST_F(TestMockDeepCopyObjectCopyRequest, Remove)
{
    ...
    expect_update_object_map(mock_dst_image_ctx, mock_object_map, m_dst_snap_ids[0], state, 0);
    expect_start_op(mock_exclusive_lock);
    expect_update_object_map(mock_dst_image_ctx,
                             mock_object_map,
                             m_dst_snap_ids[1],
...
TEST_F(TestMockDeepCopyObjectCopyRequest, ObjectMapUpdateError)
{
    ...
    expect_read(mock_src_image_ctx, m_src_snap_ids[0], 0, one.range_end(), 0);
    expect_start_op(mock_exclusive_lock);
    expect_update_object_map(mock_dst_image_ctx, mock_object_map, m_dst_snap_ids[0], OBJECT_EXISTS, -EBLOCKLISTED);
...
TEST_F(TestMockDeepCopyObjectCopyRequest, WriteSnapsStart)
{
    ...
    MockObjectCopyRequest* request = create_request(
        mock_src_image_ctx, mock_dst_image_ctx, src_snap_id_start, CEPH_NOSNAP, dst_snap_id_start, 0, &ctx);
    ...
    expect_start_op(mock_exclusive_lock);
    expect_update_object_map(mock_dst_image_ctx, mock_object_map, CEPH_NOSNAP, OBJECT_EXISTS, 0);
    ...
    expect_start_op(mock_exclusive_lock);
    expect_write(mock_dst_io_ctx, three, {m_dst_snap_ids[1], {m_dst_snap_ids[1], m_dst_snap_ids[0]}}, 0);
...
TEST_F(TestMockDeepCopyObjectCopyRequest, Incremental)
{
    ...
    C_SaferCond ctx1;
    auto request1 = create_request(mock_src_image_ctx, mock_dst_image_ctx, 0, m_src_snap_ids[0], 0, 0, &ctx1);
    ...
    librados::MockTestMemIoCtxImpl& mock_dst_io_ctx(get_mock_io_ctx(request1->get_dst_io_ctx()));
    expect_prepare_copyup(mock_dst_image_ctx);
    expect_start_op(mock_exclusive_lock);
    expect_write(mock_dst_io_ctx, 0, one.range_end(), {0, {}}, 0);
    ...
    C_SaferCond ctx2;
    auto request2 = create_request(
        mock_src_image_ctx, mock_dst_image_ctx, m_src_snap_ids[0], m_src_snap_ids[2], m_dst_snap_ids[0], 0, &ctx2);
...
TEST_F(TestMockDeepCopyObjectCopyRequest, SkipSnapList)
{
    ...
    C_SaferCond ctx;
    auto request = create_request(mock_src_image_ctx,
                                  mock_dst_image_ctx,
                                  m_src_snap_ids[0],
                                  m_src_snap_ids[1],
                                  m_dst_snap_ids[0],
