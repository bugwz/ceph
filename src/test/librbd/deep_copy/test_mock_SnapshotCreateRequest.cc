...
public:
    ...
    void expect_test_features(librbd::MockTestImageCtx& mock_image_ctx, uint64_t features, bool enabled)
    {
    ...
    void expect_snap_create(librbd::MockTestImageCtx& mock_image_ctx, const std::string& snap_name, uint64_t snap_id,
                            int r)
    {
        uint64_t flags = SNAP_CREATE_FLAG_SKIP_OBJECT_MAP | SNAP_CREATE_FLAG_SKIP_NOTIFY_QUIESCE;
        EXPECT_CALL(*mock_image_ctx.operations, execute_snap_create(_, StrEq(snap_name), _, 0, flags, _))
            .WillOnce(DoAll(InvokeWithoutArgs([&mock_image_ctx, snap_id, snap_name]() {
                                inject_snap(mock_image_ctx, snap_id, snap_name);
                            }),
                            WithArg<2>(Invoke([this, r](Context* ctx) { m_work_queue->queue(ctx, r); }))));
    }

    void expect_object_map_resize(librbd::MockTestImageCtx& mock_image_ctx, librados::snap_t snap_id, int r)
    {
        std::string oid(librbd::ObjectMap<>::object_map_name(mock_image_ctx.id, snap_id));
    ...
    MockSnapshotCreateRequest* create_request(librbd::MockTestImageCtx& mock_local_image_ctx,
                                              ...
                                              Context* on_finish)
    {
        return new MockSnapshotCreateRequest(
            &mock_local_image_ctx, snap_name, snap_namespace, size, spec, parent_overlap, on_finish);
    }
};
...
TEST_F(TestMockDeepCopySnapshotCreateRequest, SnapCreate)
{
    ...
    C_SaferCond ctx;
    MockSnapshotCreateRequest* request =
        create_request(mock_image_ctx, "snap1", cls::rbd::UserSnapshotNamespace(), m_image_ctx->size, {}, 0, &ctx);
    request->send();
...
TEST_F(TestMockDeepCopySnapshotCreateRequest, SetHeadError)
{
    ...
    C_SaferCond ctx;
    MockSnapshotCreateRequest* request =
        create_request(mock_image_ctx, "snap1", cls::rbd::UserSnapshotNamespace(), 123, {}, 0, &ctx);
    request->send();
