...
struct MockTestImageCtx : public librbd::MockImageCtx
{
    ...
    static MockTestImageCtx* create(const std::string& image_name, const std::string& image_id,
                                    librados::snap_t snap_id, librados::IoCtx& p, bool read_only)
    {
...
template<> struct ObjectCopyRequest<librbd::MockTestImageCtx>
{
    ...
    static ObjectCopyRequest* create(librbd::MockTestImageCtx* src_image_ctx, librbd::MockTestImageCtx* dst_image_ctx,
                                     librados::snap_t src_snap_id_start, librados::snap_t dst_snap_id_start,
                                     const SnapMap& snap_map, uint64_t object_number, uint32_t flags, Handler* handler,
                                     Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
...
template<> struct DiffRequest<MockTestImageCtx>
{
    BitVector<2>* object_diff_state = nullptr;
    Context* on_finish = nullptr;
    static DiffRequest* s_instance;
    static DiffRequest* create(MockTestImageCtx* image_ctx, uint64_t snap_id_start, uint64_t snap_id_end,
                               uint64_t start_object_no, uint64_t end_object_no, BitVector<2>* object_diff_state,
                               Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
...
public:
    ...
    void expect_get_image_size(librbd::MockTestImageCtx& mock_image_ctx, uint64_t size)
    {
        EXPECT_CALL(mock_image_ctx, get_image_size(_)).WillOnce(Return(size)).RetiresOnSaturation();
    }

    void expect_diff_send(MockDiffRequest& mock_request, const BitVector<2>& diff_state, int r)
    {
        EXPECT_CALL(mock_request, send()).WillOnce(Invoke([this, &mock_request, diff_state, r]() {
            if (r >= 0) {
    ...
    void expect_object_copy_send(MockObjectCopyRequest& mock_object_copy_request, uint32_t flags)
    {
        EXPECT_CALL(mock_object_copy_request, send()).WillOnce(Invoke([&mock_object_copy_request, flags]() {
            ASSERT_EQ(flags, mock_object_copy_request.flags);
        }));
    }

    bool complete_object_copy(MockObjectCopyRequest& mock_object_copy_request, uint64_t object_num,
                              Context** object_ctx, int r)
    {
        std::unique_lock locker{mock_object_copy_request.lock};
        while (mock_object_copy_request.object_contexts.count(object_num) == 0) {
            if (mock_object_copy_request.cond.wait_for(locker, 10s) == std::cv_status::timeout) {
                return false;
            }
        }
        ...
        if (object_ctx != nullptr) {
            *object_ctx = mock_object_copy_request.object_contexts[object_num];
            ...
            m_work_queue->queue(mock_object_copy_request.object_contexts[object_num], r);
        }
        return true;
    }
    ...
    int create_snap(const char* snap_name, librados::snap_t* src_snap_id_ = nullptr)
    {
...
TEST_F(TestMockDeepCopyImageCopyRequest, SimpleImage)
{
    ...
    C_SaferCond ctx;
    auto request = new MockImageCopyRequest(
        &mock_src_image_ctx, &mock_dst_image_ctx, 0, snap_id_end, 0, false, boost::none, m_snap_seqs, &no_op, &ctx);
    request->send();
...
TEST_F(TestMockDeepCopyImageCopyRequest, FastDiffExistsClean)
{
    ...
    expect_get_image_size(mock_src_image_ctx, 0);
    MockObjectCopyRequest mock_object_copy_request;
    expect_object_copy_send(mock_object_copy_request, OBJECT_COPY_REQUEST_FLAG_EXISTS_CLEAN);
...
TEST_F(TestMockDeepCopyImageCopyRequest, FastDiffMix)
{
    ...
    expect_get_image_size(mock_src_image_ctx, object_count * (1 << m_src_image_ctx->order));
    expect_get_image_size(mock_src_image_ctx, 0);
    ...
    {
        Handler(std::vector<bool>* seen)
            : m_seen(seen)
        {}

        int update_progress(uint64_t object_no, uint64_t end_object_no) override
        {
            EXPECT_THAT(object_no, ::testing::AllOf(::testing::Ge(1), ::testing::Le(m_seen->size())));
    ...
    C_SaferCond ctx;
    auto request = new MockImageCopyRequest(
        &mock_src_image_ctx, &mock_dst_image_ctx, 0, snap_id_end, 0, false, boost::none, m_snap_seqs, &handler, &ctx);
    request->send();
...
TEST_F(TestMockDeepCopyImageCopyRequest, OutOfOrder)
{
    ...
    for (uint64_t i = 0; i < object_count; ++i) {
        if (i % 10 == 0) {
            ASSERT_TRUE(complete_object_copy(mock_object_copy_request, i, &copy_contexts[i], 0));
        }
        else {
            ASSERT_TRUE(complete_object_copy(mock_object_copy_request, i, nullptr, 0));
        }
    }
...
TEST_F(TestMockDeepCopyImageCopyRequest, Cancel_Inflight_Sync)
{
    ...
    ASSERT_TRUE(complete_object_copy(mock_object_copy_request, 1, nullptr, 0));
    ASSERT_TRUE(complete_object_copy(mock_object_copy_request, 2, nullptr, 0));
    ASSERT_TRUE(complete_object_copy(mock_object_copy_request, 3, &cancel_ctx, 0));
...
TEST_F(TestMockDeepCopyImageCopyRequest, MissingSnap)
{
    ...
    C_SaferCond ctx;
    auto request = new MockImageCopyRequest(
        &mock_src_image_ctx, &mock_dst_image_ctx, 0, 123, 0, false, boost::none, m_snap_seqs, &no_op, &ctx);
    request->send();
...
TEST_F(TestMockDeepCopyImageCopyRequest, MissingFromSnap)
{
    ...
    C_SaferCond ctx;
    auto request = new MockImageCopyRequest(
        &mock_src_image_ctx, &mock_dst_image_ctx, 123, snap_id_end, 0, false, boost::none, m_snap_seqs, &no_op, &ctx);
    request->send();
    ASSERT_EQ(-EINVAL, ctx.wait());
}
...
TEST_F(TestMockDeepCopyImageCopyRequest, EmptySnapSeqs)
{
    ...
    C_SaferCond ctx;
    auto request = new MockImageCopyRequest(
        &mock_src_image_ctx, &mock_dst_image_ctx, snap_id_start, snap_id_end, 0, false, boost::none, {}, &no_op, &ctx);
    request->send();
    ASSERT_EQ(-EINVAL, ctx.wait());
}
