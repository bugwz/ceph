...
public:
    ...
    void expect_get_metadata(MockGetMetadataRequest& mock_request, const Metadata& metadata, int r)
    {
        EXPECT_CALL(mock_request, send()).WillOnce(Invoke([this, &mock_request, metadata, r]() {
            *mock_request.pairs = metadata;
            m_work_queue->queue(mock_request.on_finish, r);
        }));
    }

    void expect_metadata_set(librbd::MockTestImageCtx& mock_image_ctx, const Metadata& metadata, int r)
    {
        bufferlist in_bl;
...
TEST_F(TestMockDeepCopyMetadataCopyRequest, Success)
{
    ...
    C_SaferCond ctx;
    auto request = MockMetadataCopyRequest::create(&mock_src_image_ctx, &mock_dst_image_ctx, &ctx);
    request->send();
