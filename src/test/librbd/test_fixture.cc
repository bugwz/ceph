...
{
    unlock_image();
    for (std::set<librbd::ImageCtx*>::iterator iter = m_ictxs.begin(); iter != m_ictxs.end(); ++iter) {
        (*iter)->state->close();
    }
    m_ioctx.close();
}

int TestFixture::open_image(const std::string& image_name, librbd::ImageCtx** ictx)
{
...
int TestFixture::snap_create(librbd::ImageCtx& ictx, const std::string& snap_name)
{
    librbd::NoOpProgressContext prog_ctx;
    return ictx.operations->snap_create(cls::rbd::UserSnapshotNamespace(), snap_name.c_str(), 0, prog_ctx);
}

int TestFixture::snap_protect(librbd::ImageCtx& ictx, const std::string& snap_name)
{
    return ictx.operations->snap_protect(cls::rbd::UserSnapshotNamespace(), snap_name.c_str());
}

int TestFixture::flatten(librbd::ImageCtx& ictx, librbd::ProgressContext& prog_ctx)
{
...
int TestFixture::lock_image(librbd::ImageCtx& ictx, ClsLockType lock_type, const std::string& cookie)
{
    int r = rados::cls::lock::lock(
        &ictx.md_ctx, ictx.header_oid, RBD_LOCK_NAME, lock_type, cookie, "internal", "", utime_t(), 0);
    if (r == 0) {
...
int TestFixture::unlock_image()
{
    ...
    if (!m_lock_cookie.empty()) {
        r = rados::cls::lock::unlock(&m_ioctx, m_lock_object, RBD_LOCK_NAME, m_lock_cookie);
        m_lock_cookie = "";
    }
    return r;
}
...
int TestFixture::flush_writeback_cache(librbd::ImageCtx* image_ctx)
{
    if (image_ctx->test_features(RBD_FEATURE_DIRTY_CACHE)) {
        ...
        auto req = librbd::io::ImageDispatchSpec::create_flush(*image_ctx,
                                                               librbd::io::IMAGE_DISPATCH_LAYER_INTERNAL_START,
                                                               aio_comp,
