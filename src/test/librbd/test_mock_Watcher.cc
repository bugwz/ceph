...
struct MockWatcher : public Watcher
{
    ...
    MockWatcher(librados::IoCtx& ioctx, asio::ContextWQ* work_queue, const std::string& oid)
        : Watcher(ioctx, work_queue, oid)
    {}

    virtual void handle_notify(uint64_t notify_id, uint64_t handle, uint64_t notifier_id, bufferlist& bl) {}
};
...
public:
                          ...
                          const std::function<void()>& action = std::function<void()>())
    {
        ...
        librados::MockTestMemRadosClient* mock_rados_client(mock_io_ctx.get_mock_rados_client());

        EXPECT_CALL(mock_io_ctx, aio_watch(m_oid, _, _, _))
            .WillOnce(DoAll(WithArgs<1, 2, 3>(Invoke(
                                [this, &mock_image_ctx, mock_rados_client, r, action](
                                    librados::AioCompletionImpl* c, uint64_t* cookie, librados::WatchCtx2* watch_ctx) {
                                    if (r == 0) {
                                        *cookie = 234U;
...
TEST_F(TestMockWatcher, ReregisterUnwatchPendingUnregister)
{
    ...
    C_SaferCond unregister_ctx;
    expect_aio_unwatch(mock_image_ctx, -EBLOCKLISTED, [&mock_image_watcher, &unregister_ctx]() {
        mock_image_watcher.unregister_watch(&unregister_ctx);
    });
...
TEST_F(TestMockWatcher, ReregisterWatchPendingUnregister)
{
    ...
    // inject an unregister
    C_SaferCond unregister_ctx;
    expect_aio_watch(mock_image_ctx, -ESHUTDOWN, [&mock_image_watcher, &unregister_ctx]() {
        mock_image_watcher.unregister_watch(&unregister_ctx);
    });
