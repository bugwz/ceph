...
{
    explicit MockReplayImageCtx(ImageCtx& image_ctx)
...
template<> struct ImageRequest<MockReplayImageCtx>
{
    static ImageRequest* s_instance;

    MOCK_METHOD4(aio_write, void(AioCompletion* c, const Extents& image_extents, const bufferlist& bl, int op_flags));
    static void aio_write(MockReplayImageCtx* ictx, AioCompletion* c, Extents&& image_extents, ImageArea area,
    ...
    MOCK_METHOD3(aio_discard, void(AioCompletion* c, const Extents& image_extents, uint32_t discard_granularity_bytes));
    static void aio_discard(MockReplayImageCtx* ictx, AioCompletion* c, Extents&& image_extents, ImageArea area,
    ...
    static void aio_flush(MockReplayImageCtx* ictx, AioCompletion* c, FlushSource, const ZTracer::Trace& parent_trace)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->aio_flush(c);
    }

    MOCK_METHOD4(aio_writesame,
                 void(AioCompletion* c, const Extents& image_extents, const bufferlist& bl, int op_flags));
    static void aio_writesame(MockReplayImageCtx* ictx, AioCompletion* c, Extents&& image_extents, ImageArea area,
                              bufferlist&& bl, int op_flags, const ZTracer::Trace& parent_trace)
    {
        ceph_assert(s_instance != nullptr);
    ...
    MOCK_METHOD6(aio_compare_and_write, void(AioCompletion* c, const Extents& image_extents, const bufferlist& cmp_bl,
                                             const bufferlist& bl, uint64_t* mismatch_offset, int op_flags));
    static void aio_compare_and_write(MockReplayImageCtx* ictx, AioCompletion* c, Extents&& image_extents,
                                      ImageArea area, bufferlist&& cmp_bl, bufferlist&& bl, uint64_t* mismatch_offset,
                                      int op_flags, const ZTracer::Trace& parent_trace)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->aio_compare_and_write(c, image_extents, cmp_bl, bl, mismatch_offset, op_flags);
    }

    ImageRequest() { s_instance = this; }
};
...
public:
    ...
    {
        EXPECT_CALL(mock_exclusive_lock, accept_ops()).WillRepeatedly(Return(accept));
    }

    void expect_aio_discard(MockIoImageRequest& mock_io_image_request, io::AioCompletion** aio_comp, uint64_t off,
                            uint64_t len, uint32_t discard_granularity_bytes)
    {
        EXPECT_CALL(mock_io_image_request, aio_discard(_, io::Extents{{off, len}}, discard_granularity_bytes))
            .WillOnce(SaveArg<0>(aio_comp));
    }

    void expect_aio_flush(MockIoImageRequest& mock_io_image_request, io::AioCompletion** aio_comp)
    {
        EXPECT_CALL(mock_io_image_request, aio_flush(_)).WillOnce(SaveArg<0>(aio_comp));
    }

    void expect_aio_flush(MockReplayImageCtx& mock_image_ctx, MockIoImageRequest& mock_io_image_request, int r)
    {
        EXPECT_CALL(mock_io_image_request, aio_flush(_)).WillOnce(CompleteAioCompletion(r, mock_image_ctx.image_ctx));
    }

    void expect_aio_write(MockIoImageRequest& mock_io_image_request, io::AioCompletion** aio_comp, uint64_t off,
                          uint64_t len, const char* data)
    {
        EXPECT_CALL(mock_io_image_request, aio_write(_, io::Extents{{off, len}}, BufferlistEqual(data), _))
            .WillOnce(SaveArg<0>(aio_comp));
    }

    void expect_aio_writesame(MockIoImageRequest& mock_io_image_request, io::AioCompletion** aio_comp, uint64_t off,
                              uint64_t len, const char* data)
    {
        EXPECT_CALL(mock_io_image_request, aio_writesame(_, io::Extents{{off, len}}, BufferlistEqual(data), _))
            .WillOnce(SaveArg<0>(aio_comp));
    }

    void expect_aio_compare_and_write(MockIoImageRequest& mock_io_image_request, io::AioCompletion** aio_comp,
                                      uint64_t off, uint64_t len, const char* cmp_data, const char* data,
                                      uint64_t* mismatch_offset)
    {
        EXPECT_CALL(
            mock_io_image_request,
            aio_compare_and_write(
                _, io::Extents{{off, len}}, BufferlistEqual(cmp_data), BufferlistEqual(data), mismatch_offset, _))
            .WillOnce(SaveArg<0>(aio_comp));
    }
    ...
    void expect_flatten(MockReplayImageCtx& mock_image_ctx, Context** on_finish)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_flatten(_, _))
            .WillOnce(DoAll(SaveArg<1>(on_finish), NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    }

    void expect_rename(MockReplayImageCtx& mock_image_ctx, Context** on_finish, const char* image_name)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_rename(StrEq(image_name), _))
            .WillOnce(DoAll(SaveArg<1>(on_finish), NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    }

    void expect_resize(MockReplayImageCtx& mock_image_ctx, Context** on_finish, uint64_t size, uint64_t op_tid)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_resize(size, _, _, _, op_tid))
            .WillOnce(DoAll(SaveArg<3>(on_finish), NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    }

    void expect_snap_create(MockReplayImageCtx& mock_image_ctx, Context** on_finish, const char* snap_name,
                            uint64_t op_tid)
    {
        EXPECT_CALL(*mock_image_ctx.operations,
                    execute_snap_create(_, StrEq(snap_name), _, op_tid, SNAP_CREATE_FLAG_SKIP_NOTIFY_QUIESCE, _))
            .WillOnce(DoAll(SaveArg<2>(on_finish), NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    }

    void expect_snap_remove(MockReplayImageCtx& mock_image_ctx, Context** on_finish, const char* snap_name)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_snap_remove(_, StrEq(snap_name), _))
            .WillOnce(DoAll(SaveArg<2>(on_finish), NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    }

    void expect_snap_rename(MockReplayImageCtx& mock_image_ctx, Context** on_finish, uint64_t snap_id,
    ...
    void expect_snap_protect(MockReplayImageCtx& mock_image_ctx, Context** on_finish, const char* snap_name)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_snap_protect(_, StrEq(snap_name), _))
            .WillOnce(DoAll(SaveArg<2>(on_finish), NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    }

    void expect_snap_unprotect(MockReplayImageCtx& mock_image_ctx, Context** on_finish, const char* snap_name)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_snap_unprotect(_, StrEq(snap_name), _))
            .WillOnce(DoAll(SaveArg<2>(on_finish), NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    }

    void expect_snap_rollback(MockReplayImageCtx& mock_image_ctx, Context** on_finish, const char* snap_name)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_snap_rollback(_, StrEq(snap_name), _, _))
            .WillOnce(DoAll(SaveArg<3>(on_finish), NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    }

    void expect_update_features(MockReplayImageCtx& mock_image_ctx, Context** on_finish, uint64_t features,
                                bool enabled, uint64_t op_tid)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_update_features(features, enabled, _, op_tid))
            .WillOnce(DoAll(SaveArg<2>(on_finish), NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    }

    void expect_metadata_set(MockReplayImageCtx& mock_image_ctx, Context** on_finish, const char* key,
                             const char* value)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_metadata_set(StrEq(key), StrEq(value), _))
            .WillOnce(DoAll(SaveArg<2>(on_finish), NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    }

    void expect_metadata_remove(MockReplayImageCtx& mock_image_ctx, Context** on_finish, const char* key)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_metadata_remove(StrEq(key), _))
            .WillOnce(DoAll(SaveArg<1>(on_finish), NotifyInvoke(&m_invoke_lock, &m_invoke_cond)));
    }

    void expect_refresh_image(MockReplayImageCtx& mock_image_ctx, bool required, int r)
    {
        EXPECT_CALL(*mock_image_ctx.state, is_refresh_required()).WillOnce(Return(required));
        if (required) {
    ...
    void when_process(MockJournalReplay& mock_journal_replay, EventEntry&& event_entry, Context* on_ready,
                      Context* on_safe)
    {
        bufferlist bl;
    ...
    void when_process(MockJournalReplay& mock_journal_replay, bufferlist::const_iterator* it, Context* on_ready,
                      Context* on_safe)
    {
    ...
    void when_complete(MockReplayImageCtx& mock_image_ctx, io::AioCompletion* aio_comp, int r)
    {
        aio_comp->get();
    ...
    void when_replay_op_ready(MockJournalReplay& mock_journal_replay, uint64_t op_tid, Context* on_resume)
    {
...
TEST_F(TestMockJournalReplay, AioDiscard)
{
    ...
    C_SaferCond on_safe;
    expect_aio_discard(mock_io_image_request, &aio_comp, 123, 456, ictx->discard_granularity_bytes);
    when_process(mock_journal_replay,
                 EventEntry{AioDiscardEvent(123, 456, ictx->discard_granularity_bytes)},
                 &on_ready,
                 &on_safe);
...
TEST_F(TestMockJournalReplay, AioWrite)
{
    ...
    when_process(mock_journal_replay, EventEntry{AioWriteEvent(123, 456, to_bl("test"))}, &on_ready, &on_safe);
...
TEST_F(TestMockJournalReplay, AioFlush)
{
    ...
    when_process(mock_journal_replay, EventEntry{AioFlushEvent()}, &on_ready, &on_safe);
...
TEST_F(TestMockJournalReplay, AioWriteSame)
{
    ...
    when_process(mock_journal_replay, EventEntry{AioWriteSameEvent(123, 456, to_bl("333"))}, &on_ready, &on_safe);
...
TEST_F(TestMockJournalReplay, AioCompareAndWrite)
{
    ...
    when_process(mock_write_journal_replay, EventEntry{AioWriteEvent(512, 512, to_bl("test"))}, &on_ready, &on_safe);
    ...
    expect_aio_compare_and_write(mock_io_image_request, &aio_comp, 512, 512, "test", "test", nullptr);
    when_process(mock_compare_and_write_journal_replay,
                 EventEntry{AioCompareAndWriteEvent(512, 512, to_bl("test"), to_bl("test"))},
                 &on_ready,
                 &on_safe);
    ...
    expect_aio_compare_and_write(mock_io_image_request, &aio_comp, 512, 512, "111", "test", nullptr);
    when_process(mock_mis_compare_and_write_journal_replay,
                 EventEntry{AioCompareAndWriteEvent(512, 512, to_bl("111"), to_bl("test"))},
                 &on_ready,
                 &on_safe);
...
TEST_F(TestMockJournalReplay, PauseIO)
{
    ...
    for (size_t i = 0; i < io_count; ++i) {
        ...
        when_process(mock_journal_replay, EventEntry{AioWriteEvent(123, 456, to_bl("test"))}, &on_ready, &on_safes[i]);
        when_complete(mock_image_ctx, aio_comp, 0);
        if (i < io_count - 1) {
            ASSERT_EQ(0, on_ready.wait());
        }
...
TEST_F(TestMockJournalReplay, OpFinishError)
{
    ...
    when_process(mock_journal_replay,
                 EventEntry{SnapRemoveEvent(123, cls::rbd::UserSnapshotNamespace(), "snap")},
                 &on_start_ready,
                 &on_start_safe);
...
TEST_F(TestMockJournalReplay, BlockedOpFinishError)
{
    ...
    C_SaferCond on_start_safe;
    when_process(mock_journal_replay,
                 EventEntry{SnapCreateEvent(123, cls::rbd::UserSnapshotNamespace(), "snap")},
                 &on_start_ready,
                 &on_start_safe);
...
TEST_F(TestMockJournalReplay, MissingOpFinishEvent)
{
    ...
    EXPECT_CALL(*mock_image_ctx.state, is_refresh_required()).WillRepeatedly(Return(false));
    ...
    when_process(mock_journal_replay,
                 EventEntry{SnapRemoveEvent(122, cls::rbd::UserSnapshotNamespace(), "snap")},
...
TEST_F(TestMockJournalReplay, UnknownOpFinishEvent)
{
    ...
    C_SaferCond on_ready;
    C_SaferCond on_safe;
    when_process(mock_journal_replay, EventEntry{OpFinishEvent(123, 0)}, &on_ready, &on_safe);
...
TEST_F(TestMockJournalReplay, OpEventError)
{
    ...
    C_SaferCond on_finish_ready;
    C_SaferCond on_finish_safe;
    when_process(mock_journal_replay, EventEntry{OpFinishEvent(123, 0)}, &on_finish_ready, &on_finish_safe);
...
TEST_F(TestMockJournalReplay, SnapRenameEvent)
{
    ...
    C_SaferCond on_start_safe;
    when_process(
        mock_journal_replay, EventEntry{SnapRenameEvent(123, 234, "snap1", "snap")}, &on_start_ready, &on_start_safe);
    ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, SnapProtectEvent)
{
    ...
    C_SaferCond on_start_safe;
    when_process(mock_journal_replay,
                 EventEntry{SnapProtectEvent(123, cls::rbd::UserSnapshotNamespace(), "snap")},
                 &on_start_ready,
                 &on_start_safe);
    ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, SnapUnprotectEvent)
{
    ...
    C_SaferCond on_start_safe;
    when_process(mock_journal_replay,
                 EventEntry{SnapUnprotectEvent(123, cls::rbd::UserSnapshotNamespace(), "snap")},
                 &on_start_ready,
                 &on_start_safe);
    ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, SnapRollbackEvent)
{
    ...
    C_SaferCond on_start_safe;
    when_process(mock_journal_replay,
                 EventEntry{SnapRollbackEvent(123, cls::rbd::UserSnapshotNamespace(), "snap")},
                 &on_start_ready,
                 &on_start_safe);
    ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, RenameEvent)
{
    ...
    C_SaferCond on_start_ready;
    C_SaferCond on_start_safe;
    when_process(mock_journal_replay, EventEntry{RenameEvent(123, "image")}, &on_start_ready, &on_start_safe);
    ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, ResizeEvent)
{
    ...
    C_SaferCond on_start_ready;
    C_SaferCond on_start_safe;
    when_process(mock_journal_replay, EventEntry{ResizeEvent(123, 234)}, &on_start_ready, &on_start_safe);
...
TEST_F(TestMockJournalReplay, FlattenEvent)
{
    ...
    C_SaferCond on_start_ready;
    C_SaferCond on_start_safe;
    when_process(mock_journal_replay, EventEntry{FlattenEvent(123)}, &on_start_ready, &on_start_safe);
    ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, UpdateFeaturesEvent)
{
    ...
    C_SaferCond on_start_safe;
    when_process(
        mock_journal_replay, EventEntry{UpdateFeaturesEvent(123, features, enabled)}, &on_start_ready, &on_start_safe);
...
TEST_F(TestMockJournalReplay, MetadataSetEvent)
{
    ...
    C_SaferCond on_start_safe;
    when_process(
        mock_journal_replay, EventEntry{MetadataSetEvent(123, "key", "value")}, &on_start_ready, &on_start_safe);
    ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, MetadataRemoveEvent)
{
    ...
    C_SaferCond on_start_ready;
    C_SaferCond on_start_safe;
    when_process(mock_journal_replay, EventEntry{MetadataRemoveEvent(123, "key")}, &on_start_ready, &on_start_safe);
    ASSERT_EQ(0, on_start_ready.wait());
...
TEST_F(TestMockJournalReplay, OpEventAfterShutDown)
{
    ...
    C_SaferCond on_ready;
    C_SaferCond on_safe;
    when_process(mock_journal_replay, EventEntry{RenameEvent(123, "image")}, &on_ready, &on_safe);
    ASSERT_EQ(0, on_ready.wait());
    ASSERT_EQ(-ESHUTDOWN, on_safe.wait());
}
