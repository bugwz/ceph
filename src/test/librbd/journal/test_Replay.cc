...
public:
    ...
    template<typename T> void inject_into_journal(librbd::ImageCtx* ictx, T event)
    {
        C_SaferCond ctx;
        librbd::journal::EventEntry event_entry(event);
        {
            std::shared_lock owner_locker{ictx->owner_lock};
            uint64_t tid = ictx->journal->append_io_event(std::move(event_entry), 0, 0, true, 0);
            ictx->journal->wait_event(tid, &ctx);
        }
        ASSERT_EQ(0, ctx.wait());
    }

    void get_journal_commit_position(librbd::ImageCtx* ictx, int64_t* tag, int64_t* entry)
    {
        ...
        cls::journal::client::get_mutable_metadata(
            ictx->md_ctx, oid, &minimum_set, &active_set, &registered_clients, &cond);
        ...
        if (c == registered_clients.end() || c->commit_position.object_positions.empty()) {
            *tag = 0;
            *entry = -1;
        }
        else {
            const cls::journal::ObjectPosition& object_position = *c->commit_position.object_positions.begin();
...
TEST_F(TestJournalReplay, AioDiscardEvent)
{
    ...
    api::Io<>::aio_write(*ictx, aio_comp, 0, payload.size(), std::move(payload_bl), 0, true);
    ...
    aio_comp = new librbd::io::AioCompletion();
    api::Io<>::aio_read(*ictx, aio_comp, 0, read_payload.size(), librbd::io::ReadResult{read_result}, 0, true);
    ASSERT_EQ(0, aio_comp->wait_for_complete());
    aio_comp->release();
    ...
    inject_into_journal(ictx, librbd::journal::AioDiscardEvent(0, payload.size(), ictx->discard_granularity_bytes));
    close_image(ictx);
    ...
    aio_comp = new librbd::io::AioCompletion();
    api::Io<>::aio_discard(*ictx, aio_comp, 0, read_payload.size(), ictx->discard_granularity_bytes, true);
    ASSERT_EQ(0, aio_comp->wait_for_complete());
    aio_comp->release();
}
...
TEST_F(TestJournalReplay, AioWriteEvent)
{
    ...
    bufferlist payload_bl;
    payload_bl.append(payload);
    inject_into_journal(ictx, librbd::journal::AioWriteEvent(0, payload.size(), payload_bl));
    close_image(ictx);
    ...
    librbd::io::ReadResult read_result{&read_payload[0], read_payload.size()};
    auto aio_comp = new librbd::io::AioCompletion();
    api::Io<>::aio_read(*ictx, aio_comp, 0, read_payload.size(), std::move(read_result), 0, true);
    ASSERT_EQ(0, aio_comp->wait_for_complete());
    aio_comp->release();
    ASSERT_EQ(payload, read_payload);
    ...
    // verify lock ordering constraints
    aio_comp = new librbd::io::AioCompletion();
    api::Io<>::aio_write(*ictx, aio_comp, 0, payload.size(), bufferlist{payload_bl}, 0, true);
    ASSERT_EQ(0, aio_comp->wait_for_complete());
    aio_comp->release();
}
...
TEST_F(TestJournalReplay, SnapCreate)
{
    ...
    inject_into_journal(ictx, librbd::journal::SnapCreateEvent(1, cls::rbd::UserSnapshotNamespace(), "snap"));
...

    {
        ...
        ASSERT_NE(CEPH_NOSNAP, ictx->get_snap_id(cls::rbd::UserSnapshotNamespace(), "snap"));
    }

    // verify lock ordering constraints
    librbd::NoOpProgressContext no_op_progress;
    ASSERT_EQ(0, ictx->operations->snap_create(cls::rbd::UserSnapshotNamespace(), "snap2", 0, no_op_progress));
}
...
TEST_F(TestJournalReplay, SnapProtect)
{
    ...
    librbd::NoOpProgressContext no_op_progress;
    ASSERT_EQ(0, ictx->operations->snap_create(cls::rbd::UserSnapshotNamespace(), "snap", 0, no_op_progress));
    ...
    // inject snapshot ops into journal
    inject_into_journal(ictx, librbd::journal::SnapProtectEvent(1, cls::rbd::UserSnapshotNamespace(), "snap"));
    inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
    close_image(ictx);
    ...
    // verify lock ordering constraints
    ASSERT_EQ(0, ictx->operations->snap_create(cls::rbd::UserSnapshotNamespace(), "snap2", 0, no_op_progress));
    ASSERT_EQ(0, ictx->operations->snap_protect(cls::rbd::UserSnapshotNamespace(), "snap2"));
}
...
TEST_F(TestJournalReplay, SnapUnprotect)
{
    ...
    ASSERT_EQ(0, ictx->operations->snap_protect(cls::rbd::UserSnapshotNamespace(), "snap"));
    ...
    // inject snapshot ops into journal
    inject_into_journal(ictx, librbd::journal::SnapUnprotectEvent(1, cls::rbd::UserSnapshotNamespace(), "snap"));
    inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
    close_image(ictx);
    ...
    ASSERT_EQ(0, ictx->operations->snap_create(cls::rbd::UserSnapshotNamespace(), "snap2", 0, no_op_progress));
    ASSERT_EQ(0, ictx->operations->snap_protect(cls::rbd::UserSnapshotNamespace(), "snap2"));
    ASSERT_EQ(0, ictx->operations->snap_unprotect(cls::rbd::UserSnapshotNamespace(), "snap2"));
}
...
TEST_F(TestJournalReplay, SnapRename)
{
    ...
    // inject snapshot ops into journal
    inject_into_journal(ictx, librbd::journal::SnapRenameEvent(1, snap_id, "snap", "snap2"));
    inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
    close_image(ictx);
...
TEST_F(TestJournalReplay, SnapRollback)
{
    ...
    // inject snapshot ops into journal
    inject_into_journal(ictx, librbd::journal::SnapRollbackEvent(1, cls::rbd::UserSnapshotNamespace(), "snap"));
    inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
    close_image(ictx);
    ...
    // verify lock ordering constraints
    ASSERT_EQ(0, ictx->operations->snap_rollback(cls::rbd::UserSnapshotNamespace(), "snap", no_op_progress));
}
...
TEST_F(TestJournalReplay, SnapRemove)
{
    ...
    // inject snapshot ops into journal
    inject_into_journal(ictx, librbd::journal::SnapRemoveEvent(1, cls::rbd::UserSnapshotNamespace(), "snap"));
    inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
    close_image(ictx);
    ...
    {
        std::shared_lock image_locker{ictx->image_lock};
        uint64_t snap_id = ictx->get_snap_id(cls::rbd::UserSnapshotNamespace(), "snap");
        ASSERT_EQ(CEPH_NOSNAP, snap_id);
    }

    // verify lock ordering constraints
    ASSERT_EQ(0, ictx->operations->snap_create(cls::rbd::UserSnapshotNamespace(), "snap", 0, no_op_progress));
    ASSERT_EQ(0, ictx->operations->snap_remove(cls::rbd::UserSnapshotNamespace(), "snap"));
}
...
TEST_F(TestJournalReplay, UpdateFeatures)
{
    ...
    inject_into_journal(ictx, librbd::journal::UpdateFeaturesEvent(1, features, enabled));
    close_image(ictx);
...
TEST_F(TestJournalReplay, MetadataSet)
{
    ...
    inject_into_journal(ictx, librbd::journal::MetadataSetEvent(1, "conf_rbd_mirroring_replay_delay", "9876"));
    inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
    close_image(ictx);
    ...
    std::string value;
    ASSERT_EQ(0, librbd::metadata_get(ictx, "conf_rbd_mirroring_replay_delay", &value));
...
TEST_F(TestJournalReplay, MetadataRemove)
{
    ...
    ASSERT_EQ(0, ictx->operations->metadata_set("conf_rbd_mirroring_replay_delay", "9876"));
    ...
    inject_into_journal(ictx, librbd::journal::MetadataRemoveEvent(1, "conf_rbd_mirroring_replay_delay"));
    inject_into_journal(ictx, librbd::journal::OpFinishEvent(1, 0));
    close_image(ictx);
    ...
    std::string value;
    ASSERT_EQ(-ENOENT, librbd::metadata_get(ictx, "conf_rbd_mirroring_replay_delay", &value));
...
TEST_F(TestJournalReplay, ObjectPosition)
{
    ...
    aio_comp = librbd::io::AioCompletion::create_and_start(&flush_ctx, ictx, librbd::io::AIO_TYPE_FLUSH);
    auto req = librbd::io::ImageDispatchSpec::create_flush(
        *ictx, librbd::io::IMAGE_DISPATCH_LAYER_INTERNAL_START, aio_comp, librbd::io::FLUSH_SOURCE_INTERNAL, {});
    req->send();
