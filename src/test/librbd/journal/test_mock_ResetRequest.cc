...
template<> struct RemoveRequest<MockTestImageCtx>
{
    ...
    static RemoveRequest* create(IoCtx& ioctx, const std::string& image_id, const std::string& client_id,
...
public:
    ...
    {
        EXPECT_CALL(mock_journaler, init(_)).WillOnce(CompleteContext(r, static_cast<ContextWQ*>(NULL)));
    }

    void expect_get_metadata(::journal::MockJournaler& mock_journaler)
    {
        EXPECT_CALL(mock_journaler, get_metadata(_, _, _))
            .WillOnce(Invoke([](uint8_t* order, uint8_t* splay_width, int64_t* pool_id) {
                *order = 24;
                *splay_width = 4;
                *pool_id = -1;
            }));
    ...
    {
        EXPECT_CALL(mock_journaler, shut_down(_)).WillOnce(CompleteContext(r, static_cast<ContextWQ*>(NULL)));
    }

    void expect_remove(MockRemoveRequest& mock_remove_request, int r)
    {
        EXPECT_CALL(mock_remove_request, send()).WillOnce(Invoke([&mock_remove_request, r]() {
    ...
    {
        EXPECT_CALL(mock_create_request, send()).WillOnce(Invoke([&mock_create_request, r]() {
...
TEST_F(TestMockJournalResetRequest, Success)
{
    ...
    auto req = MockResetRequest::create(
        m_ioctx, "image id", Journal<>::IMAGE_CLIENT_ID, Journal<>::LOCAL_MIRROR_UUID, context_wq, &ctx);
    req->send();
