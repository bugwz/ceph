...
template<> struct OpenRequest<MockTestImageCtx>
{
    Context* on_finish = nullptr;
    static OpenRequest* s_instance;
    static OpenRequest* create(MockTestImageCtx* image_ctx, ::journal::MockJournalerProxy* journaler, ceph::mutex* lock,
                               ImageClientMeta* client_meta, uint64_t* tag_tid, journal::TagData* tag_data,
                               Context* on_finish)
    {
...
public:
    ...
    void expect_open_journaler(MockTestImageCtx& mock_image_ctx, MockOpenRequest& mock_open_request, int r)
    {
        EXPECT_CALL(mock_open_request, send()).WillOnce(FinishRequest(&mock_open_request, r, &mock_image_ctx));
    }

    void expect_allocate_tag(::journal::MockJournaler& mock_journaler, const journal::TagPredecessor& predecessor,
                             int r)
    {
        TagData tag_data;
        ...
        EXPECT_CALL(mock_journaler, allocate_tag(456, ContentsEqual(tag_data_bl), _, _))
    ...
    {
        EXPECT_CALL(mock_future, flush(_)).WillOnce(CompleteContext(r, static_cast<asio::ContextWQ*>(NULL)));
    }
    ...
    void expect_flush_commit_position(::journal::MockJournaler& mock_journaler, int r)
    {
    ...
    void expect_shut_down_journaler(::journal::MockJournaler& mock_journaler, int r)
    {
        EXPECT_CALL(mock_journaler, shut_down(_)).WillOnce(CompleteContext(r, static_cast<asio::ContextWQ*>(NULL)));
    }
};
...
TEST_F(TestMockJournalPromoteRequest, SuccessOrderly)
{
    ...
    expect_allocate_tag(mock_journaler, {Journal<>::ORPHAN_MIRROR_UUID, true, 567, 1}, 0);
...
TEST_F(TestMockJournalPromoteRequest, SuccessForced)
{
    ...
    expect_construct_journaler(mock_journaler);
    expect_open_journaler(mock_image_ctx, mock_open_request, 0);
    expect_allocate_tag(mock_journaler, {Journal<>::LOCAL_MIRROR_UUID, true, 567, 0}, 0);
...
TEST_F(TestMockJournalPromoteRequest, AllocateTagError)
{
    ...
    expect_construct_journaler(mock_journaler);
    expect_open_journaler(mock_image_ctx, mock_open_request, 0);
    expect_allocate_tag(mock_journaler, {Journal<>::LOCAL_MIRROR_UUID, true, 567, 0}, -EBADMSG);
    expect_shut_down_journaler(mock_journaler, -EINVAL);
