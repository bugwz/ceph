...
TEST_F(TestJournalStress, DiscardWithPruneWriteOverlap)
{
    ...
    ASSERT_EQ(0, create_image_full_pp(m_rbd, ioctx, image_name, image_size, features, false, &order));
    ...
    std::thread discard_thread([ictx, object_size, &discard_exit]() {
        while (!discard_exit) {
            ...
            auto aio_comp = new librbd::io::AioCompletion();
            api::Io<>::aio_discard(*ictx, aio_comp, object_size - 4096, 2 * object_size, 8192, true);
            ASSERT_EQ(0, aio_comp->wait_for_complete());
            aio_comp->release();
        }
    });
