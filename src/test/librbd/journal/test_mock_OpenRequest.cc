...
public:
    ...
    {
        EXPECT_CALL(mock_journaler, init(_)).WillOnce(CompleteContext(r, static_cast<asio::ContextWQ*>(NULL)));
    }
    ...
    void expect_get_journaler_tags(MockImageCtx& mock_image_ctx, ::journal::MockJournaler& mock_journaler, int r)
    {
        ...
        EXPECT_CALL(mock_journaler, get_tags(345, _, _))
            .WillOnce(
                DoAll(SetArgPointee<1>(tags), WithArg<2>(CompleteContext(r, mock_image_ctx.image_ctx->op_work_queue))));
    }
...
TEST_F(TestMockJournalOpenRequest, Success)
{
    ...
    C_SaferCond ctx;
    auto req = MockOpenRequest::create(
        &mock_image_ctx, &mock_journaler, &m_lock, &m_client_meta, &m_tag_tid, &m_tag_data, &ctx);
    req->send();
