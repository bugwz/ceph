...
public:
    ...
    {
        for (Journalers::iterator it = m_journalers.begin(); it != m_journalers.end(); ++it) {
    ...
    {
        ::journal::Journaler* journaler = new ::journal::Journaler(ictx->md_ctx, ictx->id, "dummy client", {}, nullptr);
    ...
    bool get_event_entry(const ::journal::ReplayEntry& replay_entry, librbd::journal::EventEntry* event_entry)
    {
        try {
...
TEST_F(TestJournalEntries, AioWrite)
{
    ...
    c->get();
    api::Io<>::aio_write(*ictx, c, 123, buffer.size(), std::move(write_bl), 0, true);
    ...
    ASSERT_EQ(librbd::journal::EVENT_TYPE_AIO_WRITE, event_entry.get_event_type());

    librbd::journal::AioWriteEvent aio_write_event = boost::get<librbd::journal::AioWriteEvent>(event_entry.event);
    ...
    ASSERT_EQ(librbd::journal::AioWriteEvent::get_fixed_size() + aio_write_event.data.length(),
...
TEST_F(TestJournalEntries, AioDiscard)
{
    ...
    auto c = librbd::io::AioCompletion::create(&cond_ctx);
    c->get();
    api::Io<>::aio_discard(*ictx, c, 123, 234, ictx->discard_granularity_bytes, true);
    ASSERT_EQ(0, c->wait_for_complete());
    c->put();
    ...
    ASSERT_EQ(librbd::journal::EVENT_TYPE_AIO_DISCARD, event_entry.get_event_type());
...
TEST_F(TestJournalEntries, AioDiscardWithPrune)
{
    ...
    api::Io<>::aio_discard(*ictx, c, object_size - 4096, 2 * object_size, 8192, true);
    ASSERT_EQ(0, c->wait_for_complete());
    c->put();
...
TEST_F(TestJournalEntries, AioFlush)
{
    ...
    ASSERT_EQ(librbd::journal::EVENT_TYPE_AIO_FLUSH, event_entry.get_event_type());
}
