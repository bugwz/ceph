...
struct TestMigration : public TestFixture
{
    ...
    void SetUp() override
    {
        ...
        ASSERT_EQ(0, _rados.ioctx_create(_other_pool_name.c_str(), _other_pool_ioctx));
    ...
    void compare(const std::string& description = "")
    {
        ...
        for (size_t i = 0; i <= src_snaps.size(); i++) {
            ...
            EXPECT_EQ(0, librbd::api::Image<>::snap_set(m_ref_ictx, cls::rbd::UserSnapshotNamespace(), src_snap_name));
            EXPECT_EQ(0, librbd::api::Image<>::snap_set(m_ictx, cls::rbd::UserSnapshotNamespace(), dst_snap_name));
            compare_snaps(description + " snap: " + (src_snap_name ? src_snap_name : "null"), m_ref_ictx, m_ictx);
        }
    }

    void compare_snaps(const std::string& description, librbd::ImageCtx* src_ictx, librbd::ImageCtx* dst_ictx)
    {
        ...
        if (dst_ictx->test_features(RBD_FEATURE_LAYERING)) {
            ...
            EXPECT_EQ(
                0,
                dst_ictx->test_flags(dst_ictx->snap_id, RBD_FLAG_OBJECT_MAP_INVALID, dst_ictx->image_lock, &flags_set));
        ...
        while (offset < src_size) {
            ...
            EXPECT_EQ(read_size, api::Io<>::read(*src_ictx, offset, read_size, librbd::io::ReadResult{src_result}, 0));

            bufferptr dst_ptr(read_size);
            bufferlist dst_bl;
            dst_bl.push_back(dst_ptr);
            librbd::io::ReadResult dst_result{&dst_bl};
            EXPECT_EQ(read_size, api::Io<>::read(*dst_ictx, offset, read_size, librbd::io::ReadResult{dst_result}, 0));

            if (!src_bl.contents_equal(dst_bl)) {
                std::cout << description << ", block " << offset << "~" << read_size << " differs" << std::endl;
                std::cout << "src block: " << src_ictx->id << ": " << std::endl;
                src_bl.hexdump(std::cout);
                std::cout << "dst block: " << dst_ictx->id << ": " << std::endl;
    ...
    void open_image(librados::IoCtx& io_ctx, const std::string& name, const std::string& id, bool read_only, int flags,
    ...
    void open_image(librados::IoCtx& io_ctx, const std::string& name, librbd::ImageCtx** ictx)
    {
        open_image(io_ctx, name, "", false, 0, ictx);
    }

    void migration_prepare(librados::IoCtx& dst_io_ctx, const std::string& dst_name, int r = 0)
    {
        ...
        EXPECT_EQ(r, librbd::api::Migration<>::prepare(m_ioctx, m_image_name, dst_io_ctx, dst_name, m_opts));
        if (r == 0) {
    ...
    void migration_execute(librados::IoCtx& io_ctx, const std::string& name, int r = 0)
    {
        std::cout << __func__ << std::endl;
    ...
    void migration_abort(librados::IoCtx& io_ctx, const std::string& name, int r = 0)
    {
        std::cout << __func__ << std::endl;
    ...
    void migration_status(librbd::image_migration_state_t state)
    {
        ...
        EXPECT_EQ(0, librbd::api::Migration<>::status(m_ioctx, m_image_name, &status));
    ...
    void write(uint64_t off, uint64_t len, char c)
    {
        ...
        ASSERT_EQ(static_cast<ssize_t>(len), api::Io<>::write(*m_ref_ictx, off, len, std::move(ref_bl), 0));
        bufferlist bl;
        bl.append(std::string(len, c));
        ASSERT_EQ(static_cast<ssize_t>(len), api::Io<>::write(*m_ictx, off, len, std::move(bl), 0));
    }
    ...
    void discard(uint64_t off, uint64_t len)
    {
        ...
        ASSERT_EQ(static_cast<ssize_t>(len), api::Io<>::discard(*m_ref_ictx, off, len, false));
        ASSERT_EQ(static_cast<ssize_t>(len), api::Io<>::discard(*m_ictx, off, len, false));
    }
    ...
    void clone(const std::string& snap_name)
    {
        ...
        std::cout << "clone " << m_ictx->name << " -> " << clone_name << std::endl;

        ASSERT_EQ(0,
                  librbd::clone(m_ref_ictx->md_ctx,
                                m_ref_ictx->name.c_str(),
                                snap_name.c_str(),
                                m_ref_ioctx,
                                ref_clone_name.c_str(),
                                features,
                                &order,
    ...
    template<typename L> void test_migrate_parent(uint32_t clone_format, L&& test)
    {
        REQUIRE_FEATURE(RBD_FEATURE_LAYERING);

        std::string prev_clone_format;
        ASSERT_EQ(0, _rados.conf_get("rbd_default_clone_format", prev_clone_format));
        ASSERT_EQ(0, _rados.conf_set("rbd_default_clone_format", stringify(clone_format).c_str()));
        ...
        ASSERT_EQ(10, api::Io<>::read(*child_ictx, 0, 10, librbd::io::ReadResult{result}, 0));
        bufferlist ref_bl;
    ...
    void test_stress(const std::string& snap_name_prefix = "snap", char start_char = 'A')
    {
        ...
        for (int i = 0; i < nsnaps; i++) {
            for (int j = 0; j < nwrites; j++) {
                ...
                uint64_t off =
                    std::min(static_cast<uint64_t>(rand() % m_ictx->size), static_cast<uint64_t>(m_ictx->size - len));
                write(off, len, start_char + i);

                len = rand() % ((1 << m_ictx->order) * 2);
                ASSERT_GT(m_ictx->size, len);
                off = std::min(static_cast<uint64_t>(rand() % m_ictx->size), static_cast<uint64_t>(m_ictx->size - len));
                discard(off, len);
            }
            ...
            if (m_ictx->test_features(RBD_FEATURE_LAYERING) && !m_ictx->test_features(RBD_FEATURE_MIGRATING) &&
                rand() % 4) {
                clone(snap_name);
            }
...
{
    ASSERT_EQ(0, m_opts.set(RBD_IMAGE_OPTION_DATA_POOL, _other_pool_ioctx.get_pool_name().c_str()));
...
TEST_F(TestMigration, MirroringSamePool)
{
    ...
    ASSERT_EQ(0, librbd::api::Mirror<>::image_enable(m_ictx, RBD_MIRROR_IMAGE_MODE_JOURNAL, false));
...
TEST_F(TestMigration, MirroringOtherPoolEnabled)
{
    ...
    ASSERT_EQ(0, librbd::api::Mirror<>::mode_set(m_ioctx, RBD_MIRROR_MODE_IMAGE));
    ASSERT_EQ(0, librbd::api::Mirror<>::mode_set(_other_pool_ioctx, RBD_MIRROR_MODE_IMAGE));
...
TEST_F(TestMigration, MirroringPool)
{
    ...
    ASSERT_EQ(0, librbd::api::Mirror<>::mode_set(_other_pool_ioctx, RBD_MIRROR_MODE_POOL));
    librbd::mirror_image_info_t info;
    ASSERT_EQ(0, librbd::api::Mirror<>::image_get_info(m_ictx, &info));
    ASSERT_EQ(RBD_MIRROR_IMAGE_DISABLED, info.state);
...
TEST_F(TestMigration, Group)
{
    ...
    ASSERT_EQ(0, librbd::api::Group<>::image_add(m_ioctx, "123", m_ioctx, m_image_name.c_str()));
    ...
    ASSERT_EQ(0, librbd::api::Group<>::image_remove(m_ioctx, "123", m_ioctx, name.c_str()));
...
TEST_F(TestMigration, GroupAbort)
{
    ...
    ASSERT_EQ(0, librbd::api::Group<>::image_remove(m_ioctx, "123", m_ioctx, m_image_name.c_str()));
    ASSERT_EQ(0, librbd::api::Group<>::remove(m_ioctx, "123"));
}
...
TEST_F(TestMigration, SnapsSnapAfterPrepare)
{
    ...
    auto ictx = new librbd::ImageCtx(m_ictx->name.c_str(), "", "snap3", m_ioctx, false);
    ASSERT_EQ(0, ictx->state->open(0));
    EXPECT_EQ(0, librbd::api::Image<>::snap_set(m_ref_ictx, cls::rbd::UserSnapshotNamespace(), "snap3"));
    compare_snaps("opened after prepare snap3", m_ref_ictx, ictx);
    EXPECT_EQ(0, librbd::api::Image<>::snap_set(m_ref_ictx, cls::rbd::UserSnapshotNamespace(), nullptr));
...
TEST_F(TestMigration, CloneParent)
{
    ...
    ASSERT_EQ(0, librbd::api::Image<>::get_parent(m_ictx, &parent_image, &parent_snap));
...
TEST_F(TestMigration, AbortInUseImage)
{
    ...
    librbd::NoOpProgressContext no_op;
    EXPECT_EQ(-EBUSY, librbd::api::Migration<>::abort(m_ioctx, m_ictx->name, no_op));
}
...
TEST_F(TestMigration, CloneV1Parent)
{
    ...
    test_migrate_parent(CLONE_FORMAT, [this](librbd::ImageCtx*) { migrate(m_ioctx, m_image_name); });
}
...
{
    const uint32_t CLONE_FORMAT = 1;
    test_migrate_parent(CLONE_FORMAT, [this](librbd::ImageCtx*) {
        migration_prepare(m_ioctx, m_image_name);
        migration_abort(m_ioctx, m_image_name);
    });
}
...
{
    const uint32_t CLONE_FORMAT = 1;
    test_migrate_parent(CLONE_FORMAT, [this](librbd::ImageCtx* child_ictx) {
        ...
        open_image(m_ioctx, "", src_image_id, false, librbd::OPEN_FLAG_IGNORE_MIGRATING, &src_ictx);
        C_SaferCond cond;
        auto req = librbd::image::AttachChildRequest<>::create(
            child_ictx, src_ictx, src_ictx->snaps[0], nullptr, 0, CLONE_FORMAT, &cond);
        req->send();
...
TEST_F(TestMigration, CloneV1ParentAbortFixParentReattach)
{
    ...
    test_migrate_parent(CLONE_FORMAT, [this](librbd::ImageCtx* child_ictx) {
        ...
        C_SaferCond cond;
        auto req = librbd::image::AttachChildRequest<>::create(
            child_ictx, src_ictx, src_ictx->snaps[0], m_ictx, m_ictx->snaps[0], CLONE_FORMAT, &cond);
        req->send();
        ASSERT_EQ(0, cond.wait());
        close_image(src_ictx);
        migration_abort(m_ioctx, m_image_name);
...
TEST_F(TestMigration, CloneV1ParentAbortRelinkNotNeeded)
{
    ...
    test_migrate_parent(CLONE_FORMAT, [this](librbd::ImageCtx* child_ictx) {
        ...
        C_SaferCond cond;
        auto req = librbd::image::AttachParentRequest<>::create(*child_ictx, parent_spec, parent_overlap, true, &cond);
        req->send();
        ASSERT_EQ(0, cond.wait());
        librbd::ImageCtx* src_ictx;
        open_image(m_ioctx, "", src_image_id, false, librbd::OPEN_FLAG_IGNORE_MIGRATING, &src_ictx);
        ...
        auto req1 = librbd::image::AttachChildRequest<>::create(
            child_ictx, src_ictx, src_ictx->snaps[0], m_ictx, m_ictx->snaps[0], CLONE_FORMAT, &cond1);
        req1->send();
