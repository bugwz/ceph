...
struct TestMockCryptoLuksFlattenRequest : public TestMockFixture
{
    ...
    void SetUp() override
    {
        ...
        mock_flatten_request = MockFlattenRequest::create(mock_image_ctx, on_finish);
    }
    ...
    void expect_get_crypto() { EXPECT_CALL(*mock_encryption_format, get_crypto()).WillOnce(Return(&mock_crypto)); }
    ...
    {
        EXPECT_CALL(*mock_image_ctx->io_image_dispatcher, send(_)).WillOnce(Invoke([this](io::ImageDispatchSpec* spec) {
            auto* write = boost::get<io::ImageDispatchSpec::Write>(&spec->request);
            ...
            bufferlist bl;
            bl.substr_of(header_bl, write->bl.length(), header_bl.length() - write->bl.length());
            header_bl = write->bl;
