...
struct TestMockCryptoLuksFormatRequest : public TestMockFixture
{
    ...
    void expect_get_stripe_period() { EXPECT_CALL(*mock_image_ctx, get_stripe_period()).WillOnce(Return(OBJECT_SIZE)); }

    void expect_get_image_size(uint64_t image_size)
    {
        EXPECT_CALL(*mock_image_ctx, get_image_size(CEPH_NOSNAP)).WillOnce(Return(image_size));
    }
    ...
    void verify_header(const char* expected_format, size_t expected_key_length, uint64_t expected_sector_size,
                       bool magic_switched)
    {
        ...
        ASSERT_EQ(0,
                  header.read_volume_key(
                      passphrase_cstr, strlen(passphrase_cstr), reinterpret_cast<char*>(volume_key), &volume_key_size));

        ASSERT_EQ(expected_key_length, crypto->get_key_length());
        ASSERT_EQ(0, std::memcmp(volume_key, crypto->get_key(), expected_key_length));
...
{
    auto mock_format_request = MockFormatRequest::create(mock_image_ctx,
