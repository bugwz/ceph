...
struct TestMockCryptoLuksLoadRequest : public TestMockFixture
{
    ...
    void SetUp() override
    {
        ...
        mock_load_request = MockLoadRequest::create(
            mock_image_ctx, RBD_ENCRYPTION_FORMAT_LUKS2, std::move(passphrase), &crypto, on_finish);
    }
...
TEST_F(TestMockCryptoLuksLoadRequest, LUKS1)
{
    ...
    mock_load_request =
        MockLoadRequest::create(mock_image_ctx, RBD_ENCRYPTION_FORMAT_LUKS1, {passphrase_cstr}, &crypto, on_finish);
...
TEST_F(TestMockCryptoLuksLoadRequest, WrongPassphrase)
{
    delete mock_load_request;
    mock_load_request =
        MockLoadRequest::create(mock_image_ctx, RBD_ENCRYPTION_FORMAT_LUKS2, "wrong", &crypto, on_finish);
