...
struct TestMockCryptoLuksLoadRequest : public TestMockFixture
{
    ...
    void SetUp() override
    {
        ...
        mock_load_request = MockLoadRequest::create(mock_image_ctx,
    ...
    void generate_header(const char* type, const char* alg, size_t key_size, const char* cipher_mode,
                         uint32_t sector_size, bool magic_switched)
    {
        Header header(mock_image_ctx->cct);

        ASSERT_EQ(0, header.init());
        ASSERT_EQ(0, header.format(type, alg, nullptr, key_size, cipher_mode, sector_size, OBJECT_SIZE, true));
    ...
    void expect_image_read(uint64_t offset, uint64_t length)
    {
        EXPECT_CALL(*mock_image_ctx->io_image_dispatcher, send(_))
            .WillOnce(Invoke([this, offset, length](io::ImageDispatchSpec* spec) {
                auto* read = boost::get<io::ImageDispatchSpec::Read>(&spec->request);
                ...
                auto ctx = new io::ReadResult::C_ImageReadRequest(aio_comp, 0, spec->image_extents);
    ...
    {
        EXPECT_CALL(*mock_image_ctx, get_image_size(_)).WillOnce(Return(size));
    }

    void expect_get_stripe_period(uint64_t period)
    {
        EXPECT_CALL(*mock_image_ctx, get_stripe_period()).WillOnce(Return(period));
    }
};
...
TEST_F(TestMockCryptoLuksLoadRequest, LUKS1)
{
    ...
    mock_load_request = MockLoadRequest::create(
        mock_image_ctx, RBD_ENCRYPTION_FORMAT_LUKS1, {passphrase_cstr}, &crypto, &detected_format_name, on_finish);
...
TEST_F(TestMockCryptoLuksLoadRequest, LUKS1ViaLUKS)
{
    delete mock_load_request;
    mock_load_request = MockLoadRequest::create(
        mock_image_ctx, RBD_ENCRYPTION_FORMAT_LUKS, {passphrase_cstr}, &crypto, &detected_format_name, on_finish);
    generate_header(CRYPT_LUKS1, "aes", 32, "xts-plain64", 512, false);
    expect_image_read(0, DEFAULT_INITIAL_READ_SIZE);
    expect_get_image_size(OBJECT_SIZE << 5);
    expect_get_stripe_period(OBJECT_SIZE);
...
TEST_F(TestMockCryptoLuksLoadRequest, WrongFormat)
{
    ...
    expect_image_read(DEFAULT_INITIAL_READ_SIZE, MAXIMUM_HEADER_SIZE - DEFAULT_INITIAL_READ_SIZE);
    image_read_request->complete(DEFAULT_INITIAL_READ_SIZE);
...
TEST_F(TestMockCryptoLuksLoadRequest, WrongPassphrase)
{
    delete mock_load_request;
    mock_load_request = MockLoadRequest::create(
        mock_image_ctx, RBD_ENCRYPTION_FORMAT_LUKS2, "wrong", &crypto, &detected_format_name, on_finish);
    ...
    expect_image_read(DEFAULT_INITIAL_READ_SIZE, data_offset - DEFAULT_INITIAL_READ_SIZE);
    image_read_request->complete(DEFAULT_INITIAL_READ_SIZE);
