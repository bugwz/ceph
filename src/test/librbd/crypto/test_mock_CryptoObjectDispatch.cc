...
template bool librbd::io::util::trigger_copyup(MockImageCtx* image_ctx, uint64_t object_no, IOContext io_context,
                                               Context* on_finish);
...
template<> struct CopyupRequest<librbd::MockImageCtx>
{
    ...
    MOCK_METHOD2(append_request, void(AbstractObjectWriteRequest<librbd::MockImageCtx>*, const Extents&));

    static CopyupRequest* s_instance;
    static CopyupRequest* create(librbd::MockImageCtx* ictx, uint64_t objectno, Extents&& image_extents, ImageArea area,
...
struct Mock
{
    ...
    MOCK_METHOD6(read_parent,
                 void(MockImageCtx*, uint64_t, io::ReadExtents*, librados::snap_t, const ZTracer::Trace&, Context*));
};
...
template<>
void read_parent(MockImageCtx* image_ctx, uint64_t object_no, io::ReadExtents* extents, librados::snap_t snap_id,
...
template<> uint64_t get_file_offset(MockImageCtx* image_ctx, uint64_t object_no, uint64_t offset)
{
    return Striper::get_file_offset(image_ctx->cct, &image_ctx->layout, object_no, offset);
}
...
struct TestMockCryptoCryptoObjectDispatch : public TestMockFixture
{
    ...
    typedef io::AbstractObjectWriteRequest<librbd::MockImageCtx> MockAbstractObjectWriteRequest;
    ...
    void expect_object_read(io::ReadExtents* extents, uint64_t version = 0)
    {
        EXPECT_CALL(*mock_image_ctx->io_object_dispatcher, send(_))
            .WillOnce(Invoke([this, extents, version](io::ObjectDispatchSpec* spec) {
                auto* read = boost::get<io::ObjectDispatchSpec::ReadRequest>(&spec->request);
    ...
    void expect_read_parent(MockUtils& mock_utils, uint64_t object_no, io::ReadExtents* extents,
    ...
    void expect_object_write(uint64_t object_off, const std::string& data, int write_flags,
                             std::optional<uint64_t> assert_version)
    {
        EXPECT_CALL(*mock_image_ctx->io_object_dispatcher, send(_))
            .WillOnce(Invoke([this, object_off, data, write_flags, assert_version](io::ObjectDispatchSpec* spec) {
                auto* write = boost::get<io::ObjectDispatchSpec::WriteRequest>(&spec->request);
    ...
    void expect_object_write_same()
    {
        EXPECT_CALL(*mock_image_ctx->io_object_dispatcher, send(_))
            .WillOnce(Invoke([this](io::ObjectDispatchSpec* spec) {
                auto* write_same = boost::get<io::ObjectDispatchSpec::WriteSameRequest>(&spec->request);
    ...
    {
        EXPECT_CALL(*mock_image_ctx, get_object_size()).WillOnce(Return(mock_image_ctx->layout.object_size));
    }

    void expect_remap_to_logical(uint64_t offset, uint64_t length)
    {
        EXPECT_CALL(*mock_image_ctx->io_image_dispatcher, remap_to_logical(ElementsAre(Pair(offset, length))));
    }

    void expect_get_parent_overlap(uint64_t overlap)
    {
        EXPECT_CALL(*mock_image_ctx, get_parent_overlap(_, _)).WillOnce(WithArg<1>(Invoke([overlap](uint64_t* o) {
            *o = overlap;
            return 0;
        })));
    }
    ...
    {
        EXPECT_CALL(*mock_image_ctx, prune_parent_extents(_, _, _, _)).WillOnce(Return(object_overlap));
    }
    ...
    void expect_copyup(MockAbstractObjectWriteRequest** write_request, int r)
    {
        ...
        EXPECT_CALL(copyup_request, send()).WillOnce(Invoke([write_request, r]() {
            (*write_request)->handle_copyup(r);
        }));
    }

    void expect_encrypt(int count = 1) { EXPECT_CALL(crypto, encrypt(_, _)).Times(count); }

    void expect_decrypt(int count = 1) { EXPECT_CALL(crypto, decrypt(_, _)).Times(count); }
};

TEST_F(TestMockCryptoCryptoObjectDispatch, Flush)
{
    ASSERT_FALSE(mock_crypto_object_dispatch->flush(io::FLUSH_SOURCE_USER, {}, nullptr, nullptr, &on_finish, nullptr));
...
TEST_F(TestMockCryptoCryptoObjectDispatch, Discard)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->discard(11,
                                                     0,
                                                     4096,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, AlignedReadFail)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->read(11,
                                                  &extents,
                                                  mock_image_ctx->get_data_io_context(),
                                                  0,
                                                  0,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, AlignedRead)
{
    ...
    extents[0].bl.append(std::string(1024, '1') + std::string(1024, '2') + std::string(1024, '3') +
                         std::string(1024, '4'));
    ...
    auto expected_bl_data =
        (std::string(1024, '\0') + std::string(1024, '1') + std::string(1024, '\0') + std::string(1024, '2') +
         std::string(1024, '3') + std::string(3072, '\0') + std::string(3072, '\0') + std::string(1024, '4'));
    ASSERT_TRUE(extents[0].bl.to_str() == expected_bl_data);
    ASSERT_THAT(extents[0].extent_map, ElementsAre(Pair(0, 8192), Pair(16384 - 4096, 4096)));
}
...
{
    io::ReadExtents extents = {{0, 1}, {8191, 1}, {8193, 1}, {16384 + 1, 4096 * 5 - 2}};
    io::ReadExtents aligned_extents = {{0, 4096}, {4096, 4096}, {8192, 4096}, {16384, 4096 * 5}};
    ...
    aligned_extents[3].bl.append(std::string("0") + std::string(4095, '4') + std::string(4096, '5') +
                                 std::string(4095, '6') + std::string("0"));
    aligned_extents[3].extent_map = {{16384, 4096}, {16384 + 2 * 4096, 4096}, {16384 + 4 * 4096, 4096}};
    ...
    auto expected_bl_data = (std::string(4095, '4') + std::string(4096, '5') + std::string(4095, '6'));
    ASSERT_TRUE(extents[3].bl.to_str() == expected_bl_data);
    ASSERT_THAT(extents[3].extent_map,
                ElementsAre(Pair(16384 + 1, 4095), Pair(16384 + 2 * 4096, 4096), Pair(16384 + 4 * 4096, 4095)));
}

TEST_F(TestMockCryptoCryptoObjectDispatch, AlignedWrite)
{
    expect_encrypt();
    ASSERT_TRUE(mock_crypto_object_dispatch->write(11,
                                                   0,
                                                   std::move(data),
                                                   mock_image_ctx->get_data_io_context(),
                                                   0,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWrite)
{
    ...
    auto expected_data = std::string("2") + std::string(8192, '1') + std::string(4095, '3');
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteWithNoObject)
{
    ...
    auto expected_data = (std::string(1, '\0') + std::string(8192, '1') + std::string(4095, '\0'));
    expect_object_write(0, expected_data, io::OBJECT_WRITE_FLAG_CREATE_EXCLUSIVE, std::nullopt);
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteFailCreate)
{
    ...
    auto expected_data2 = std::string("2") + std::string(8192, '1') + std::string(4095, '3');
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteCopyup)
{
    ...
    expect_remap_to_logical(11 * mock_image_ctx->layout.object_size, mock_image_ctx->layout.object_size);
    ...
    EXPECT_CALL(*mock_image_ctx->object_map, object_may_exist(11)).WillOnce(Return(false));
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteWithAssertVersion)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->write(11,
                                                   ...
                                                   0,
                                                   0,
                                                   std::make_optional(assert_version),
                                                   {},
                                                   nullptr,
                                                   nullptr,
                                                   &dispatch_result,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, CompareAndWrite)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->compare_and_write(11,
                                                               1,
                                                               std::move(cmp_data),
                                                               std::move(write_data),
                                                               mock_image_ctx->get_data_io_context(),
...
TEST_F(TestMockCryptoCryptoObjectDispatch, WriteSame)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->write_same(11,
                                                        0,
                                                        5,
                                                        {{0, 5}},
                                                        std::move(write_data),
...
TEST_F(TestMockCryptoCryptoObjectDispatch, PrepareCopyup)
{
    ...
    snap1.insert(0, 1, {io::SPARSE_EXTENT_STATE_DATA, 1, ceph::bufferlist::static_from_mem(data + 1, 1)});
    snap1.insert(8191, 1, {io::SPARSE_EXTENT_STATE_DATA, 1, ceph::bufferlist::static_from_mem(data + 2, 1)});
    snap1.insert(8193, 3, {io::SPARSE_EXTENT_STATE_DATA, 3, ceph::bufferlist::static_from_mem(data + 3, 3)});

    snap2.insert(0, 2, {io::SPARSE_EXTENT_STATE_ZEROED, 2});
    snap2.insert(8191, 3, {io::SPARSE_EXTENT_STATE_DATA, 3, ceph::bufferlist::static_from_mem(data + 6, 3)});
    snap2.insert(16384, 1, {io::SPARSE_EXTENT_STATE_DATA, 1, ceph::bufferlist::static_from_mem(data + 9, 1)});
    ...
    ASSERT_EQ(0, mock_crypto_object_dispatch->prepare_copyup(11, &snapshot_sparse_bufferlist));
    ...
    ASSERT_TRUE(it.get_val().bl.to_str() == std::string("1") + std::string(4095, '\0') + std::string(4095, '\0') +
                                                std::string("2") + std::string(1, '\0') + std::string("345") +
                                                std::string(4092, '\0'));
    ...
    ASSERT_EQ(0, it.get_off());
    ASSERT_EQ(4096 * 3, it.get_len());
    ASSERT_TRUE(it.get_val().bl.to_str() == std::string(4096, '\0') + std::string(4095, '\0') + std::string("6") +
                                                std::string("7845") + std::string(4092, '\0'));

    ASSERT_NE(++it, snap2_result.end());
    ASSERT_EQ(16384, it.get_off());
    ASSERT_EQ(4096, it.get_len());
    ASSERT_TRUE(it.get_val().bl.to_str() == std::string("9") + std::string(4095, '\0'));
