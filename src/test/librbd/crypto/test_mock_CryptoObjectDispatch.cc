...
template<> struct CopyupRequest<librbd::MockImageCtx>
{
    ...
    static CopyupRequest* create(librbd::MockImageCtx* ictx, uint64_t objectno, Extents&& image_extents,
...
struct TestMockCryptoCryptoObjectDispatch : public TestMockFixture
{
    ...
    void expect_remap_extents(uint64_t offset, uint64_t length)
    {
        EXPECT_CALL(*mock_image_ctx->io_image_dispatcher,
                    remap_extents(ElementsAre(Pair(offset, length)), io::IMAGE_EXTENTS_MAP_TYPE_PHYSICAL_TO_LOGICAL));
    }
    ...
    {
        EXPECT_CALL(*mock_image_ctx, prune_parent_extents(_, _)).WillOnce(Return(object_overlap));
    }
...
TEST_F(TestMockCryptoCryptoObjectDispatch, Discard)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->discard(0,
                                                     0,
                                                     4096,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, AlignedReadFail)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->read(0,
                                                  &extents,
                                                  mock_image_ctx->get_data_io_context(),
                                                  0,
                                                  0,
...
{
    expect_encrypt();
    ASSERT_TRUE(mock_crypto_object_dispatch->write(0,
                                                   0,
                                                   std::move(data),
                                                   mock_image_ctx->get_data_io_context(),
                                                   0,
...
TEST_F(TestMockCryptoCryptoObjectDispatch, UnalignedWriteCopyup)
{
    ...
    EXPECT_CALL(*mock_image_ctx->object_map, object_may_exist(0)).WillOnce(Return(false));
...
TEST_F(TestMockCryptoCryptoObjectDispatch, CompareAndWrite)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->compare_and_write(0,
                                                               1,
                                                               std::move(cmp_data),
                                                               std::move(write_data),
                                                               mock_image_ctx->get_data_io_context(),
...
TEST_F(TestMockCryptoCryptoObjectDispatch, WriteSame)
{
    ...
    ASSERT_TRUE(mock_crypto_object_dispatch->write_same(0,
                                                        0,
                                                        5,
                                                        {{0, 5}},
                                                        std::move(write_data),
...
TEST_F(TestMockCryptoCryptoObjectDispatch, PrepareCopyup)
{
    ...
    ASSERT_EQ(0, mock_crypto_object_dispatch->prepare_copyup(0, &snapshot_sparse_bufferlist));
