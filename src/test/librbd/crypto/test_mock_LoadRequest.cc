...
template<> void set_crypto(MockTestImageCtx* image_ctx, std::unique_ptr<MockEncryptionFormat> encryption_format)
{
...
struct TestMockCryptoLoadRequest : public TestMockFixture
{
    ...
    void SetUp() override
    {
        ...
        mock_load_request = MockLoadRequest::create(mock_image_ctx, std::move(formats), on_finish);
    }
    ...
    void expect_test_journal_feature(MockTestImageCtx* ictx, bool has_journal = false)
    {
        EXPECT_CALL(*ictx, test_features(RBD_FEATURE_JOURNALING)).WillOnce(Return(has_journal));
    }

    void expect_encryption_load(MockEncryptionFormat* encryption_format, MockTestImageCtx* ictx,
                                std::string detected_format = "SOMEFORMAT")
    {
        EXPECT_CALL(*encryption_format, load(ictx, _, _))
            .WillOnce(WithArgs<1, 2>(Invoke([this, detected_format](std::string* detected_format_name, Context* ctx) {
    ...
    void expect_encryption_format_clone(MockEncryptionFormat* encryption_format)
    {
        ...
        EXPECT_CALL(*encryption_format, clone()).WillOnce(Invoke([this]() {
            return std::unique_ptr<MockEncryptionFormat>(cloned_encryption_format);
        }));
    }
    ...
    {
        EXPECT_CALL(*mock_image_ctx->io_image_dispatcher, invalidate_cache(_)).WillOnce(Invoke([r](Context* ctx) {
            ctx->complete(r);
        }));
    }
};
...
TEST_F(TestMockCryptoLoadRequest, LoadClonedEncryptedParent)
{
    ...
    ASSERT_EQ(0, finished_cond.wait());
    ASSERT_EQ(mock_encryption_format, mock_image_ctx->encryption_format.get());
    ASSERT_EQ(cloned_encryption_format, mock_parent_image_ctx->encryption_format.get());
}
...
TEST_F(TestMockCryptoLoadRequest, LoadClonedPlaintextParent)
{
    ...
    ASSERT_EQ(ETIMEDOUT, finished_cond.wait_for(0));
    expect_encryption_format_clone(mock_encryption_format);
    expect_encryption_load(cloned_encryption_format, mock_parent_image_ctx, LoadRequest<MockImageCtx>::UNKNOWN_FORMAT);
    load_context->complete(0);
    ASSERT_EQ(ETIMEDOUT, finished_cond.wait_for(0));
    expect_invalidate_cache();
    load_context->complete(-EINVAL);
...
TEST_F(TestMockCryptoLoadRequest, EncryptedParent)
{
    ...
    ASSERT_EQ(0, finished_cond.wait());
    ASSERT_EQ(mock_encryption_format, mock_image_ctx->encryption_format.get());
    ASSERT_EQ(mock_parent_encryption_format, mock_parent_image_ctx->encryption_format.get());
}
