...
template class librbd::crypto::CryptoContextPool<librbd::crypto::MockCryptoContext>;
...
struct TestMockCryptoCryptoContextPool : public ::testing::Test
{
    ...
    {
        EXPECT_CALL(cryptor, get_context(mode)).WillOnce(Return(new MockCryptoContext()));
    }
