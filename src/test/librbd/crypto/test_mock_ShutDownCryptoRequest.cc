...
struct TestMockShutDownCryptoRequest : public TestMockFixture
{
    ...
    {
        EXPECT_CALL(*mock_image_ctx->io_object_dispatcher, exists(io::OBJECT_DISPATCH_LAYER_CRYPTO))
            .WillOnce(Return(exists));
    }

    void expect_crypto_image_layer_exists_check(bool exists)
    {
        EXPECT_CALL(*mock_image_ctx->io_image_dispatcher, exists(io::IMAGE_DISPATCH_LAYER_CRYPTO))
            .WillOnce(Return(exists));
    }

    void expect_shutdown_crypto_object_dispatch()
    {
        EXPECT_CALL(*mock_image_ctx->io_object_dispatcher, shut_down_dispatch(io::OBJECT_DISPATCH_LAYER_CRYPTO, _))
    ...
    {
        EXPECT_CALL(*mock_image_ctx->io_image_dispatcher, shut_down_dispatch(io::IMAGE_DISPATCH_LAYER_CRYPTO, _))
