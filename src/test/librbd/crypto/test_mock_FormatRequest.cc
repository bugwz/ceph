...
template<> void set_crypto(MockImageCtx* image_ctx, ceph::ref_t<CryptoInterface> crypto)
{
...
template<> struct ShutDownCryptoRequest<MockImageCtx>
{
    Context* on_finish = nullptr;
    static ShutDownCryptoRequest* s_instance;
    static ShutDownCryptoRequest* create(MockImageCtx* image_ctx, Context* on_finish)
    {
...
ShutDownCryptoRequest<MockImageCtx>* ShutDownCryptoRequest<MockImageCtx>::s_instance = nullptr;
...
struct TestMockCryptoFormatRequest : public TestMockFixture
{
    ...
    void SetUp() override
    {
        ...
        mock_format_request = MockFormatRequest::create(
            mock_image_ctx, std::unique_ptr<MockEncryptionFormat>(mock_encryption_format), on_finish);
    }
    ...
    {
        EXPECT_CALL(*mock_encryption_format, format(mock_image_ctx, _))
            .WillOnce(WithArg<1>(Invoke([this](Context* ctx) { format_context = ctx; })));
    }
