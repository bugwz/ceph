...
template<> struct ShutDownCryptoRequest<MockTestImageCtx>
{
    Context* on_finish = nullptr;
    static ShutDownCryptoRequest* s_instance;
    static ShutDownCryptoRequest* create(MockTestImageCtx* image_ctx, Context* on_finish)
    {
    ...
    ShutDownCryptoRequest() { s_instance = this; }
};

ShutDownCryptoRequest<MockTestImageCtx>* ShutDownCryptoRequest<MockTestImageCtx>::s_instance = nullptr;
...
struct TestMockCryptoFormatRequest : public TestMockFixture
{
    ...
    void SetUp() override
    {
        ...
        mock_format_request = MockFormatRequest::create(
            mock_image_ctx, std::unique_ptr<MockEncryptionFormat>(new_encryption_format), on_finish);
    }
    ...
    {
        EXPECT_CALL(*mock_image_ctx, test_features(RBD_FEATURE_JOURNALING)).WillOnce(Return(has_journal));
    }

    void expect_shutdown_crypto(int r = 0)
    {
        EXPECT_CALL(mock_shutdown_crypto_request, send()).WillOnce(Invoke([this, r]() {
            if (r == 0) {
    ...
    {
        EXPECT_CALL(*new_encryption_format, format(mock_image_ctx, _)).WillOnce(WithArg<1>(Invoke([this](Context* ctx) {
            format_context = ctx;
        })));
    }
