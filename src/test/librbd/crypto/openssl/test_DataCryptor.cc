...
struct TestCryptoOpensslDataCryptor : public TestFixture
{
    ...
    void SetUp() override
    {
        ...
        ASSERT_EQ(0, cryptor->init(TEST_CIPHER_NAME, TEST_KEY, sizeof(TEST_KEY)));
    }
...
TEST_F(TestCryptoOpensslDataCryptor, InvalidKey)
{
    ...
    EXPECT_EQ(-EINVAL, cryptor->init(TEST_CIPHER_NAME, nullptr, sizeof(TEST_KEY)));
...
TEST_F(TestCryptoOpensslDataCryptor, EncryptDecrypt)
{
    ...
    ASSERT_EQ(sizeof(TEST_DATA), cryptor->update_context(ctx, TEST_DATA, out, sizeof(TEST_DATA)));
    ...
    ASSERT_EQ(sizeof(TEST_DATA), cryptor->update_context(ctx, out, out, sizeof(TEST_DATA)));
...
TEST_F(TestCryptoOpensslDataCryptor, ReuseContext)
{
    ...
    ASSERT_EQ(sizeof(TEST_DATA), cryptor->update_context(ctx2, TEST_DATA, out2, sizeof(TEST_DATA)));
