...
struct TestDeepCopy : public TestFixture
{
    void SetUp() override
    {
        ...
        EXPECT_EQ(0, create_image_full_pp(m_rbd, m_ioctx, image_name, size, features, old_format, &order));
    ...
    void deep_copy()
    {
        ...
        EXPECT_EQ(0, librbd::api::Image<>::deep_copy(m_src_ictx, m_src_ictx->md_ctx, dst_name.c_str(), m_opts, no_op));
    ...
    void compare()
    {
        ...
        for (size_t i = 0; i <= src_snaps.size(); i++) {
            ...
            EXPECT_EQ(0, librbd::api::Image<>::snap_set(m_src_ictx, cls::rbd::UserSnapshotNamespace(), src_snap_name));
            EXPECT_EQ(0, librbd::api::Image<>::snap_set(m_dst_ictx, cls::rbd::UserSnapshotNamespace(), dst_snap_name));
            ...
            if (m_dst_ictx->test_features(RBD_FEATURE_LAYERING)) {
                ...
                std::shared_lock dst_locker{m_dst_ictx->image_lock};
                EXPECT_EQ(0,
                          m_dst_ictx->test_flags(
                              m_dst_ictx->snap_id, RBD_FLAG_OBJECT_MAP_INVALID, m_dst_ictx->image_lock, &flags_set));
            ...
            while (offset < src_size) {
                ...
                EXPECT_EQ(read_size,
                          api::Io<>::read(*m_src_ictx, offset, read_size, librbd::io::ReadResult{src_result}, 0));
                ...
                if (!src_bl.contents_equal(dst_bl)) {
                    std::cout << "snap: " << (src_snap_name ? src_snap_name : "null") << ", block " << offset << "~"
                              << read_size << " differs" << std::endl;
    ...
    void test_no_snaps()
    {
        ...
        ASSERT_EQ(static_cast<ssize_t>(bl.length()),
                  api::Io<>::write(*m_src_ictx, 0 * bl.length(), bl.length(), bufferlist{bl}, 0));
        ASSERT_EQ(static_cast<ssize_t>(bl.length()),
                  api::Io<>::write(*m_src_ictx, 2 * bl.length(), bl.length(), bufferlist{bl}, 0));
    }
    ...
    void test_snaps()
    {
        ...
        ASSERT_EQ(static_cast<ssize_t>(bl.length()),
                  api::Io<>::write(*m_src_ictx, 1 * bl.length(), bl.length(), bufferlist{bl}, 0));
        bufferlist bl1;
        bl1.append(std::string(1000, 'X'));
        ASSERT_EQ(static_cast<ssize_t>(bl1.length()),
                  api::Io<>::write(*m_src_ictx, 0 * bl.length(), bl1.length(), bufferlist{bl1}, 0));
        ASSERT_EQ(static_cast<ssize_t>(bl1.length()),
                  api::Io<>::discard(*m_src_ictx, bl1.length() + 10, bl1.length(), false));

        ASSERT_EQ(0, api::Io<>::flush(*m_src_ictx));

        ASSERT_EQ(0, snap_create(*m_src_ictx, "snap2"));
        ASSERT_EQ(static_cast<ssize_t>(bl1.length()),
                  api::Io<>::write(*m_src_ictx, 1 * bl.length(), bl1.length(), bufferlist{bl1}, 0));
        ASSERT_EQ(static_cast<ssize_t>(bl1.length()),
                  api::Io<>::discard(*m_src_ictx, 2 * bl1.length() + 10, bl1.length(), false));
    }
    ...
    void test_snap_discard()
    {
        ...
        ASSERT_EQ(static_cast<ssize_t>(bl.length()), api::Io<>::write(*m_src_ictx, 0, bl.length(), bufferlist{bl}, 0));
        ASSERT_EQ(0, api::Io<>::flush(*m_src_ictx));

        ASSERT_EQ(0, snap_create(*m_src_ictx, "snap"));

        size_t len = (1 << m_src_ictx->order) * 2;
        ASSERT_EQ(static_cast<ssize_t>(len), api::Io<>::discard(*m_src_ictx, 0, len, false));
    }
    ...
    void test_clone_discard()
    {
        ...
        ASSERT_EQ(0,
                  librbd::clone(
                      m_ioctx, m_src_ictx->name.c_str(), "snap", m_ioctx, clone_name.c_str(), features, &order, 0, 0));
        close_image(m_src_ictx);
    ...
    void test_clone_hide_parent()
    {
        ...
        bl.append(std::string(100, '1'));
        ASSERT_EQ(static_cast<ssize_t>(bl.length()),
                  api::Io<>::write(*m_src_ictx, object_size, bl.length(), bufferlist{bl}, 0));
        ASSERT_EQ(0, api::Io<>::flush(*m_src_ictx));
        ...
        ASSERT_EQ(static_cast<ssize_t>(bl.length()), api::Io<>::discard(*m_src_ictx, object_size, bl.length(), false));
        ASSERT_EQ(0, api::Io<>::flush(*m_src_ictx));
    ...
    void test_stress()
    {
        ...
        for (int i = 0; i < nsnaps; i++) {
            for (int j = 0; j < nwrites; j++) {
                ...
                uint64_t off = std::min(static_cast<uint64_t>(rand() % size), static_cast<uint64_t>(size - len));
                std::cout << "write: " << static_cast<char>('A' + i) << " " << off << "~" << len << std::endl;
                ASSERT_EQ(static_cast<ssize_t>(bl.length()),
                          api::Io<>::write(*m_src_ictx, off, bl.length(), bufferlist{bl}, 0));
                len = rand() % ((1 << m_src_ictx->order) * 2);
                ASSERT_GT(size, len);
                off = std::min(static_cast<uint64_t>(rand() % size), static_cast<uint64_t>(size - len));
                std::cout << "discard: " << off << "~" << len << std::endl;
                ASSERT_EQ(static_cast<ssize_t>(len), api::Io<>::discard(*m_src_ictx, off, len, false));
            }
            ...
            if (m_src_ictx->test_features(RBD_FEATURE_LAYERING) && rand() % 4) {
                ...
                std::cout << "clone " << m_src_ictx->name << " -> " << clone_name << std::endl;
                ASSERT_EQ(0,
                          librbd::clone(m_ioctx,
                                        m_src_ictx->name.c_str(),
                                        snap_name.c_str(),
                                        m_ioctx,
                                        clone_name.c_str(),
                                        features,
