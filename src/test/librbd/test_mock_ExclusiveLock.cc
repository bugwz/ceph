...
struct MockExclusiveLockImageCtx : public MockImageCtx
{
    ...
    MockExclusiveLockImageCtx(ImageCtx& image_ctx)
...
namespace watcher {
template<> struct Traits<MockExclusiveLockImageCtx>
{
    typedef librbd::MockImageWatcher Watcher;
};
}   // namespace watcher

template<> struct ManagedLock<MockExclusiveLockImageCtx>
{
    ManagedLock(librados::IoCtx& ioctx, AsioEngine& asio_engine, const std::string& oid,
                librbd::MockImageWatcher* watcher, managed_lock::Mode mode, bool blocklist_on_break_lock,
...
template<typename T> struct BaseRequest
{
    static std::list<T*> s_requests;
    Context* on_lock_unlock = nullptr;
    Context* on_finish = nullptr;

    static T* create(MockExclusiveLockImageCtx& image_ctx, Context* on_lock_unlock, Context* on_finish)
    {
    ...
    BaseRequest() { s_requests.push_back(reinterpret_cast<T*>(this)); }
};

template<typename T> std::list<T*> BaseRequest<T>::s_requests;

template<> struct ImageDispatch<MockExclusiveLockImageCtx> : public librbd::io::MockImageDispatch
{
    ...
    ImageDispatch() { s_instance = this; }

    io::ImageDispatchLayer get_dispatch_layer() const override { return io::IMAGE_DISPATCH_LAYER_EXCLUSIVE_LOCK; }
...
{
    static PreAcquireRequest<MockExclusiveLockImageCtx>* create(MockExclusiveLockImageCtx& image_ctx,
                                                                Context* on_finish)
    {
...
template<>
struct PostAcquireRequest<MockExclusiveLockImageCtx>
    : public BaseRequest<PostAcquireRequest<MockExclusiveLockImageCtx> >
{
    MOCK_METHOD0(send, void());
};
...
{
    static PreReleaseRequest<MockExclusiveLockImageCtx>* create(MockExclusiveLockImageCtx& image_ctx,
...
public:
    ...
    void expect_set_state_unlocked(MockManagedLock* managed_lock) { EXPECT_CALL(*managed_lock, set_state_unlocked()); }
    ...
    {
        EXPECT_CALL(*managed_lock, is_state_acquiring()).WillOnce(Return(ret_val));
    }

    void expect_is_state_waiting_for_lock(MockManagedLock* managed_lock, bool ret_val)
    {
        EXPECT_CALL(*managed_lock, is_state_waiting_for_lock()).WillOnce(Return(ret_val));
    }

    void expect_is_state_pre_releasing(MockManagedLock* managed_lock, bool ret_val)
    {
        EXPECT_CALL(*managed_lock, is_state_pre_releasing()).WillOnce(Return(ret_val));
    }

    void expect_is_state_releasing(MockManagedLock* managed_lock, bool ret_val)
    {
        EXPECT_CALL(*managed_lock, is_state_releasing()).WillOnce(Return(ret_val));
    }

    void expect_is_state_locked(MockManagedLock* managed_lock, bool ret_val)
    {
        EXPECT_CALL(*managed_lock, is_state_locked()).WillOnce(Return(ret_val));
    }

    void expect_is_state_shutdown(MockManagedLock* managed_lock, bool ret_val)
    {
        EXPECT_CALL(*managed_lock, is_state_shutdown()).WillOnce(Return(ret_val));
    }

    void expect_is_action_acquire_lock(MockManagedLock* managed_lock, bool ret_val)
    {
        EXPECT_CALL(*managed_lock, is_action_acquire_lock()).WillOnce(Return(ret_val));
    }

    void expect_set_require_lock(MockImageDispatch& mock_image_dispatch, bool init_shutdown, io::Direction direction)
    {
        EXPECT_CALL(mock_image_dispatch, set_require_lock(init_shutdown, direction, _))
            .WillOnce(WithArg<2>(Invoke([](Context* ctx) { ctx->complete(0); })));
    }

    void expect_set_require_lock(MockExclusiveLockImageCtx& mock_image_ctx, MockImageDispatch& mock_image_dispatch,
                                 bool init_shutdown)
    {
        if (mock_image_ctx.clone_copy_on_read || (mock_image_ctx.features & RBD_FEATURE_JOURNALING) != 0 ||
            is_rbd_pwl_enabled(mock_image_ctx.cct)) {
            expect_set_require_lock(mock_image_dispatch, init_shutdown, io::DIRECTION_BOTH);
        }
        else {
            expect_set_require_lock(mock_image_dispatch, init_shutdown, io::DIRECTION_WRITE);
        }
    }
    ...
    void expect_pre_acquire_request(MockPreAcquireRequest& pre_acquire_request, int r)
    {
        EXPECT_CALL(pre_acquire_request, send()).WillOnce(CompleteRequest(&pre_acquire_request, r));
    }

    void expect_post_acquire_request(MockExclusiveLock* mock_exclusive_lock,
                                     MockPostAcquireRequest& post_acquire_request, int r)
    {
        EXPECT_CALL(post_acquire_request, send())
            .WillOnce(DoAll(FinishLockUnlock(&post_acquire_request), CompleteRequest(&post_acquire_request, r)));
        expect_set_state_post_acquiring(mock_exclusive_lock);
    }

    void expect_pre_release_request(MockPreReleaseRequest& pre_release_request, int r)
    {
        EXPECT_CALL(pre_release_request, send()).WillOnce(CompleteRequest(&pre_release_request, r));
    }

    void expect_notify_request_lock(MockExclusiveLockImageCtx& mock_image_ctx, MockExclusiveLock* mock_exclusive_lock)
    {
    ...
    {
        EXPECT_CALL(*mock_image_ctx.image_watcher, notify_acquired_lock()).Times(1);
    }

    void expect_notify_released_lock(MockExclusiveLockImageCtx& mock_image_ctx)
    {
        EXPECT_CALL(*mock_image_ctx.image_watcher, notify_released_lock()).Times(1);
    }
    ...
    {
        EXPECT_CALL(*managed_lock, shut_down(_)).WillOnce(CompleteContext(0, static_cast<asio::ContextWQ*>(nullptr)));
    }

    void expect_accept_blocked_request(MockExclusiveLockImageCtx& mock_image_ctx, exclusive_lock::MockPolicy& policy,
                                       exclusive_lock::OperationRequestType request_type, bool value)
    {
        EXPECT_CALL(mock_image_ctx, get_exclusive_lock_policy()).WillOnce(Return(&policy));
        EXPECT_CALL(policy, accept_blocked_request(request_type)).WillOnce(Return(value));
    }

    int when_init(MockExclusiveLockImageCtx& mock_image_ctx, MockExclusiveLock* exclusive_lock)
    {
        C_SaferCond ctx;
        {
    ...
    int when_pre_release_lock_handler(MockManagedLock* managed_lock, bool shutting_down)
    {
        C_SaferCond ctx;
    ...
    int when_post_release_lock_handler(MockManagedLock* managed_lock, bool shutting_down, int r)
    {
        C_SaferCond ctx;
    ...
    int when_shut_down(MockExclusiveLockImageCtx& mock_image_ctx, MockExclusiveLock* exclusive_lock)
    {
        C_SaferCond ctx;
        {
            std::unique_lock owner_locker{mock_image_ctx.owner_lock};
    ...
    bool is_lock_owner(MockExclusiveLockImageCtx& mock_image_ctx, MockExclusiveLock* exclusive_lock)
    {
...
TEST_F(TestMockExclusiveLock, AcquireLockAlreadyLocked)
{
    ...
    ASSERT_EQ(-ECANCELED, when_post_acquire_lock_handler(exclusive_lock, -EAGAIN));
}
...
TEST_F(TestMockExclusiveLock, AcquireLockBusy)
{
    ...
    expect_set_state_waiting_for_lock(exclusive_lock);
    expect_notify_request_lock(mock_image_ctx, exclusive_lock);
    ASSERT_EQ(-ECANCELED, when_post_acquire_lock_handler(exclusive_lock, -EBUSY));
}
...
TEST_F(TestMockExclusiveLock, AcquireLockError)
{
    ...
    expect_prepare_lock_complete(mock_image_ctx);
    expect_is_action_acquire_lock(exclusive_lock, true);
    ASSERT_EQ(-EBLOCKLISTED, when_post_acquire_lock_handler(exclusive_lock, -EBLOCKLISTED));
}
...
TEST_F(TestMockExclusiveLock, PostAcquireLockError)
{
    ...
    MockPostAcquireRequest request_lock_post_acquire;
    expect_post_acquire_request(exclusive_lock, request_lock_post_acquire, -EPERM);
    expect_is_state_acquiring(exclusive_lock, true);
...
TEST_F(TestMockExclusiveLock, PreReleaseLockError)
{
    ...
    ASSERT_EQ(-EINVAL, when_post_release_lock_handler(exclusive_lock, false, -EINVAL));
}
...
TEST_F(TestMockExclusiveLock, BlockRequests)
{
    ...
    ASSERT_TRUE(exclusive_lock->accept_request(exclusive_lock::OPERATION_REQUEST_TYPE_GENERAL, &ret_val));
    ASSERT_EQ(0, ret_val);
    ...
    expect_accept_blocked_request(
        mock_image_ctx, mock_exclusive_lock_policy, exclusive_lock::OPERATION_REQUEST_TYPE_GENERAL, false);
    ASSERT_FALSE(exclusive_lock->accept_request(exclusive_lock::OPERATION_REQUEST_TYPE_GENERAL, &ret_val));
    ASSERT_EQ(-EROFS, ret_val);

    expect_is_state_shutdown(exclusive_lock, false);
    expect_is_state_locked(exclusive_lock, true);
    expect_accept_blocked_request(
        mock_image_ctx, mock_exclusive_lock_policy, exclusive_lock::OPERATION_REQUEST_TYPE_TRASH_SNAP_REMOVE, true);
    ASSERT_TRUE(exclusive_lock->accept_request(exclusive_lock::OPERATION_REQUEST_TYPE_TRASH_SNAP_REMOVE, &ret_val));
    ASSERT_EQ(0, ret_val);
