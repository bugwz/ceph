...
{
    librbd::asio::ContextWQ* context_wq = reinterpret_cast<librbd::asio::ContextWQ*>(wq);
    if (context_wq != NULL) {
...
{
    librbd::MockImageCtx* mock_image_ctx = reinterpret_cast<librbd::MockImageCtx*>(mock);
...
MATCHER_P(ContentsEqual, bl, "")
{
    ...
    return const_cast<bufferlist&>(arg).contents_equal(const_cast<bufferlist&>(bl));
}
...
public:
    ...
    void initialize_features(librbd::ImageCtx* ictx, librbd::MockImageCtx& mock_image_ctx,
                             librbd::MockExclusiveLock& mock_exclusive_lock, librbd::MockJournal& mock_journal,
                             librbd::MockObjectMap& mock_object_map);

    void expect_is_journal_appending(librbd::MockJournal& mock_journal, bool appending);
    ...
    void expect_append_op_event(librbd::MockImageCtx& mock_image_ctx, bool can_affect_io, int r);
