...
TEST_F(TestObjectMap, RefreshInvalidatesWhenCorrupt)
{
    ...
    bool flags_set;
    ASSERT_EQ(0, ictx->test_flags(CEPH_NOSNAP, RBD_FLAG_OBJECT_MAP_INVALID, &flags_set));
...
TEST_F(TestObjectMap, DISABLED_StressTest)
{
    ...
    for (uint64_t ios = 0; ios < 100000;) {
        ...
        if (!ictx->object_map->aio_update<Context, &Context::complete>(
                CEPH_NOSNAP, object_no, OBJECT_EXISTS, {}, {}, true, ctx)) {
            ctx->complete(0);
        }
        else {
            ++cur_ios;
        ...
        if (last == chrono::duration<double>::zero()) {
            ...
            std::cerr << std::setw(5) << (int)elapsed.count() << "\t" << std::setw(8) << (int)ios << "\t" << std::fixed
                      << std::setw(8) << std::setprecision(2) << boost::accumulators::rolling_sum(ios_acc) / time_sum
                      << std::endl;
            last = elapsed;
        }
    }
