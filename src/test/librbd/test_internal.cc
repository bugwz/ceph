...
public:
    ...
    void TearDown() override
    {
        ...
        for (Snaps::iterator iter = m_snaps.begin(); iter != m_snaps.end(); ++iter) {
            ...
            if (iter->second) {
                EXPECT_EQ(0, ictx->operations->snap_unprotect(cls::rbd::UserSnapshotNamespace(), iter->first.c_str()));
            }
            EXPECT_EQ(0, ictx->operations->snap_remove(cls::rbd::UserSnapshotNamespace(), iter->first.c_str()));
        }
...
void generate_random_iomap(librbd::Image& image, int num_objects, int object_size, int max_count,
                           map<uint64_t, uint64_t>& iomap)
{
...
static bool is_sparsify_supported(librados::IoCtx& ioctx, const std::string& oid)
{
...
TEST_F(TestInternal, SnapCreateLocksImage)
{
    ...
    {
        ASSERT_EQ(0, ictx->operations->snap_remove(cls::rbd::UserSnapshotNamespace(), "snap1"));
    }
...
TEST_F(TestInternal, SnapRollbackLocksImage)
{
    ...
    librbd::NoOpProgressContext no_op;
    ASSERT_EQ(0, ictx->operations->snap_rollback(cls::rbd::UserSnapshotNamespace(), "snap1", no_op));
...
TEST_F(TestInternal, SnapRollbackFailsToLockImage)
{
    ...
    librbd::NoOpProgressContext no_op;
    ASSERT_EQ(-EROFS, ictx->operations->snap_rollback(cls::rbd::UserSnapshotNamespace(), "snap1", no_op));
}
...
TEST_F(TestInternal, SnapSetReleasesLock)
{
    ...
    librbd::ImageCtx* ictx;
    ASSERT_EQ(0, open_image(m_image_name, &ictx));
    ASSERT_EQ(0, librbd::api::Image<>::snap_set(ictx, cls::rbd::UserSnapshotNamespace(), "snap1"));
...
TEST_F(TestInternal, FlattenLocksImage)
{
    ...
    ASSERT_EQ(
        0, librbd::clone(m_ioctx, m_image_name.c_str(), "snap1", m_ioctx, clone_name.c_str(), features, &order, 0, 0));
...
TEST_F(TestInternal, WriteFailsToLockImageBlocklisted)
{
    ...
    ASSERT_EQ(0, blocklist_rados.ioctx_create(_pool_name.c_str(), blocklist_ioctx));

    auto ictx = new librbd::ImageCtx(m_image_name, "", nullptr, blocklist_ioctx, false);
    ASSERT_EQ(0, ictx->state->open(0));
    ...
    ASSERT_EQ(-EBLOCKLISTED, api::Io<>::write(*ictx, 0, bl.length(), std::move(bl), 0));
...
TEST_F(TestInternal, Metadata)
{
    ...
    map<string, bool> test_confs = boost::assign::map_list_of("aaaaaaa", false)("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
                                                                                false)("cccccccccccccc", false);
...
TEST_F(TestInternal, MetadataConfApply)
{
    ...
    ASSERT_EQ(0, ictx->operations->metadata_set("conf_rbd_cache", new_rbd_conf_cache));
...
TEST_F(TestInternal, SnapshotCopyup)
{
    ...
    bool sparse_read_supported = is_sparse_read_supported(ictx->data_ctx, ictx->get_object_name(10));

    bufferlist bl;
    bl.append(std::string(256, '1'));
    ASSERT_EQ(256, api::Io<>::write(*ictx, 0, bl.length(), bufferlist{bl}, 0));
    ASSERT_EQ(256, api::Io<>::write(*ictx, 1024, bl.length(), bufferlist{bl}, 0));

    ASSERT_EQ(0, snap_create(*ictx, "snap1"));
    ASSERT_EQ(0, ictx->operations->snap_protect(cls::rbd::UserSnapshotNamespace(), "snap1"));
    ...
    std::vector<std::pair<uint64_t, uint64_t>> expected_overlap =
        boost::assign::list_of(std::make_pair(0, 256))(std::make_pair(512, copyup_end - 512));
    ...
    for (std::list<std::string>::iterator it = snaps.begin(); it != snaps.end(); ++it) {
        ...
        ASSERT_EQ(256, api::Io<>::read(*ictx2, 0, 256, librbd::io::ReadResult{read_result}, 0));
        ASSERT_TRUE(bl.contents_equal(read_bl));

        ASSERT_EQ(256, api::Io<>::read(*ictx2, 1024, 256, librbd::io::ReadResult{read_result}, 0));
        ASSERT_TRUE(bl.contents_equal(read_bl));

        ASSERT_EQ(256, api::Io<>::read(*ictx2, 256, 256, librbd::io::ReadResult{read_result}, 0));
        if (snap_name == NULL) {
            ASSERT_TRUE(bl.contents_equal(read_bl));
        }
        else {
        ...
        // verify sparseness was preserved
        {
            ...
            auto ictx3 = new librbd::ImageCtx(clone_name, "", snap_name, io_ctx, true);
            ...
            EXPECT_EQ(1024 + 256,
                      api::Io<>::read(*ictx3, 0, 1024 + 256, librbd::io::ReadResult{sparse_read_result}, 0));
        ...
        if ((ictx2->features & RBD_FEATURE_OBJECT_MAP) != 0) {
            ...
            if ((ictx2->features & RBD_FEATURE_FAST_DIFF) != 0 && it != snaps.begin() && snap_name != NULL) {
...
TEST_F(TestInternal, SnapshotCopyupZerosMigration)
{
    ...
    ASSERT_EQ(0, librbd::api::Migration<>::prepare(m_ioctx, m_image_name, m_ioctx, dst_name, dst_opts));
...
TEST_F(TestInternal, ResizeCopyup)
{
    ...
    int order = 12;
    ASSERT_EQ(0, m_rbd.create2(m_ioctx, m_image_name.c_str(), m_image_size, features, &order));
    ...
    for (size_t i = 0; i < m_image_size; i += bl.length()) {
        ASSERT_EQ((ssize_t)bl.length(), api::Io<>::write(*ictx, i, bl.length(), bufferlist{bl}, 0));
    }
    ...
    librbd::NoOpProgressContext no_op;
    ASSERT_EQ(0, ictx2->operations->resize(m_image_size - (1 << order) - 32, true, no_op));
    ASSERT_EQ(0, ictx2->operations->resize(m_image_size - (2 << order) - 32, true, no_op));
    ASSERT_EQ(0, librbd::api::Image<>::snap_set(ictx2, cls::rbd::UserSnapshotNamespace(), "snap1"));
    ...
    for (size_t i = 2 << order; i < m_image_size; i += bl.length()) {
        ASSERT_EQ((ssize_t)bl.length(),
                  api::Io<>::read(*ictx2, i, bl.length(), librbd::io::ReadResult{read_result}, 0));
        ASSERT_TRUE(bl.contents_equal(read_bl));
    }
}
...
TEST_F(TestInternal, DiscardCopyup)
{
    ...
    ASSERT_EQ(static_cast<int>(m_image_size - 64), api::Io<>::discard(*ictx2, 32, m_image_size - 64, false));
    ASSERT_EQ(0, librbd::api::Image<>::snap_set(ictx2, cls::rbd::UserSnapshotNamespace(), "snap1"));
...
TEST_F(TestInternal, ImageOptions)
{
    ...
    ASSERT_EQ(-EINVAL, librbd::image_options_get(opts1, RBD_IMAGE_OPTION_FEATURES, &string_val1));
    ASSERT_EQ(-ENOENT, librbd::image_options_get(opts1, RBD_IMAGE_OPTION_FEATURES, &uint64_val1));

    ASSERT_EQ(-EINVAL, librbd::image_options_set(opts1, RBD_IMAGE_OPTION_FEATURES, string_val1));

    ASSERT_EQ(0, librbd::image_options_set(opts1, RBD_IMAGE_OPTION_FEATURES, uint64_val1));
    ASSERT_FALSE(librbd::image_options_is_empty(opts1));
    ASSERT_EQ(0, librbd::image_options_get(opts1, RBD_IMAGE_OPTION_FEATURES, &uint64_val2));
    ...
    ASSERT_EQ(0, librbd::image_options_get(opts2, RBD_IMAGE_OPTION_FEATURES, &uint64_val2));
    ASSERT_EQ(uint64_val1, uint64_val2);

    uint64_val2++;
    ASSERT_NE(uint64_val1, uint64_val2);
    ASSERT_EQ(-ENOENT, librbd::image_options_get(opts1, RBD_IMAGE_OPTION_ORDER, &uint64_val1));
    ASSERT_EQ(-ENOENT, librbd::image_options_get(opts2, RBD_IMAGE_OPTION_ORDER, &uint64_val2));
    ASSERT_EQ(0, librbd::image_options_set(opts2, RBD_IMAGE_OPTION_ORDER, uint64_val2));
    ASSERT_EQ(0, librbd::image_options_get(opts1, RBD_IMAGE_OPTION_ORDER, &uint64_val1));
    ASSERT_EQ(0, librbd::image_options_get(opts2, RBD_IMAGE_OPTION_ORDER, &uint64_val2));
    ASSERT_EQ(uint64_val1, uint64_val2);
    ...
    ASSERT_EQ(-ENOENT, librbd::image_options_get(opts2, RBD_IMAGE_OPTION_FEATURES, &uint64_val2));
...
TEST_F(TestInternal, WriteFullCopyup)
{
    ...
    ASSERT_EQ((ssize_t)bl.length(), api::Io<>::write(*ictx, 0, bl.length(), bufferlist{bl}, 0));
    ...
    ASSERT_EQ(0,
              librbd::clone(
                  m_ioctx, m_image_name.c_str(), "snap1", m_ioctx, clone_name.c_str(), ictx->features, &order, 0, 0));
    ...
    {
        if (ictx2 != NULL) {
            ictx2->operations->snap_remove(cls::rbd::UserSnapshotNamespace(), "snap1");
            parent->close_image(ictx2);
        }

        librbd::NoOpProgressContext remove_no_op;
        ASSERT_EQ(0, librbd::api::Image<>::remove(m_ioctx, clone_name, remove_no_op));
    }
    BOOST_SCOPE_EXIT_END;

    ASSERT_EQ(0, open_image(clone_name, &ictx2));
    ASSERT_EQ(0, ictx2->operations->snap_create(cls::rbd::UserSnapshotNamespace(), "snap1", 0, no_op));

    bufferlist write_full_bl;
    write_full_bl.append(std::string(1 << ictx2->order, '2'));
    ASSERT_EQ((ssize_t)write_full_bl.length(),
              api::Io<>::write(*ictx2, 0, write_full_bl.length(), bufferlist{write_full_bl}, 0));
    ...
    ASSERT_EQ((ssize_t)read_bl.length(),
              api::Io<>::read(*ictx2, 0, read_bl.length(), librbd::io::ReadResult{read_result}, 0));
...
TEST_F(TestInternal, DiffIterateCloneOverwrite)
{
    ...
    ASSERT_EQ(0, image.diff_iterate2(NULL, 0, size, false, false, iterate_cb, (void*)&one));
    ...
    ASSERT_EQ(0, ictx->operations->snap_protect(cls::rbd::UserSnapshotNamespace(), "one"));
    ...
    ASSERT_EQ(0, librbd::api::Image<>::snap_set(ictx, cls::rbd::UserSnapshotNamespace(), "one"));
    ASSERT_EQ(0,
              librbd::api::DiffIterate<>::diff_iterate(
                  ictx, cls::rbd::UserSnapshotNamespace(), nullptr, 0, size, true, false, iterate_cb, (void*)&diff));
    ASSERT_EQ(one, diff);
}
...
TEST_F(TestInternal, TestCoR)
{
    ...
    ASSERT_EQ(0, create_image_full_pp(m_rbd, m_ioctx, m_image_name, m_image_size, features, false, &order));
    ...
    for (map<uint64_t, uint64_t>::iterator itr = write_tracker.begin(); itr != write_tracker.end(); ++itr)
    ...
    ASSERT_EQ(0,
              clone_image_pp(
                  m_rbd, image, m_ioctx, m_image_name.c_str(), snapname.c_str(), m_ioctx, clonename.c_str(), features));
    ASSERT_EQ(0, image.close());
    ...
    for (map<uint64_t, uint64_t>::iterator itr = --write_tracker.end(); itr != write_tracker.begin(); --itr) {
        printf("\tread object-%-4lu\n", (unsigned long)itr->first);
        ASSERT_EQ(TEST_IO_SIZE, image.read(itr->second, TEST_IO_SIZE, readbl));
        ASSERT_TRUE(readbl.contents_equal(bl));
    }
...
TEST_F(TestInternal, PoolMetadataConfApply)
{
    ...
    ASSERT_EQ(0, librbd::api::PoolMetadata<>::set(m_ioctx, "conf_rbd_cache", new_rbd_conf_cache));
    ASSERT_EQ(0, ictx->state->refresh());
    ASSERT_EQ(!cache, ictx->cache);

    ASSERT_EQ(0, ictx->operations->metadata_set("conf_rbd_cache", rbd_conf_cache));
    ASSERT_EQ(cache, ictx->cache);
    ...
    ASSERT_EQ(0, librbd::api::PoolMetadata<>::set(m_ioctx, "conf_rbd_default_order", "17"));
    ASSERT_EQ(0, librbd::api::PoolMetadata<>::set(m_ioctx, "conf_rbd_journal_order", "13"));
    std::string image_name = get_temp_image_name();
    int order = 0;
    uint64_t features;
    ASSERT_TRUE(::get_features(&features));
    ASSERT_EQ(0, create_image_full_pp(m_rbd, m_ioctx, image_name, m_image_size, features, false, &order));
    ...
    if (is_feature_enabled(RBD_FEATURE_JOURNALING)) {
        ...
        int64_t pool_id;
        C_SaferCond cond;
        cls::journal::client::get_immutable_metadata(
            m_ioctx, "journal." + ictx->id, &order, &splay_width, &pool_id, &cond);
        ASSERT_EQ(0, cond.wait());
        ASSERT_EQ(order, 13);
        ASSERT_EQ(0, ictx->operations->update_features(RBD_FEATURE_JOURNALING, false));
        ASSERT_EQ(0, librbd::api::PoolMetadata<>::set(m_ioctx, "conf_rbd_journal_order", "14"));
        ASSERT_EQ(0, ictx->operations->update_features(RBD_FEATURE_JOURNALING, true));
        ASSERT_EQ(ictx->config.get_val<uint64_t>("rbd_journal_order"), 14U);

        C_SaferCond cond1;
        cls::journal::client::get_immutable_metadata(
            m_ioctx, "journal." + ictx->id, &order, &splay_width, &pool_id, &cond1);
        ASSERT_EQ(0, cond1.wait());
        ASSERT_EQ(order, 14);
    }

    ASSERT_EQ(0, librbd::api::PoolMetadata<>::remove(m_ioctx, "conf_rbd_default_order"));
    ASSERT_EQ(0, librbd::api::PoolMetadata<>::remove(m_ioctx, "conf_rbd_journal_order"));
}
...
TEST_F(TestInternal, Sparsify)
{
    ...
    bool sparsify_supported = is_sparsify_supported(ictx->data_ctx, ictx->get_object_name(10));
    bool sparse_read_supported = is_sparse_read_supported(ictx->data_ctx, ictx->get_object_name(10));
    ...
    ASSERT_EQ((ssize_t)bl.length(),
              api::Io<>::write(*ictx, (1 << ictx->order) * 1 + 512, bl.length(), bufferlist{bl}, 0));

    bl.append(std::string(4096, '1'));
    bl.append(std::string(4096, '\0'));
    bl.append(std::string(4096, '2'));
    bl.append(std::string(4096 - 1, '\0'));
    ASSERT_EQ((ssize_t)bl.length(), api::Io<>::write(*ictx, (1 << ictx->order) * 10, bl.length(), bufferlist{bl}, 0));

    bufferlist bl2;
    bl2.append(std::string(4096 - 1, '\0'));
    ASSERT_EQ((ssize_t)bl2.length(),
              api::Io<>::write(*ictx, (1 << ictx->order) * 10 + 4096 * 10, bl2.length(), bufferlist{bl2}, 0));
    ...
    librbd::io::ReadResult read_result{&read_bl};
    ASSERT_EQ(
        (ssize_t)read_bl.length(),
        api::Io<>::read(*ictx, (1 << ictx->order) * 10, read_bl.length(), librbd::io::ReadResult{read_result}, 0));
    ASSERT_TRUE(bl.contents_equal(read_bl));
    ...
    read_bl.clear();
    EXPECT_EQ(static_cast<int>(expected_m.size()), ictx->data_ctx.sparse_read(oid, m, read_bl, read_len, 0));
...
TEST_F(TestInternal, SparsifyClone)
{
    ...
    ASSERT_EQ(0,
              librbd::clone(
                  m_ioctx, m_image_name.c_str(), "snap", m_ioctx, clone_name.c_str(), ictx->features, &order, 0, 0));
    close_image(ictx);
...
TEST_F(TestInternal, MissingDataPool)
{
    ...
    read_bl.push_back(read_ptr);
    librbd::io::ReadResult read_result{&read_bl};
    ASSERT_EQ(-ENODEV, api::Io<>::read(*ictx, 0, 256, librbd::io::ReadResult{read_result}, 0));
    ASSERT_EQ(-ENODEV, api::Io<>::write(*ictx, 0, bl.length(), bufferlist{bl}, 0));
    ASSERT_EQ(-ENODEV, api::Io<>::discard(*ictx, 0, 1, 256));
    ASSERT_EQ(-ENODEV, api::Io<>::write_same(*ictx, 0, bl.length(), bufferlist{bl}, 0));
    uint64_t mismatch_off;
    ASSERT_EQ(-ENODEV,
              api::Io<>::compare_and_write(*ictx, 0, bl.length(), bufferlist{bl}, bufferlist{bl}, &mismatch_off, 0));
