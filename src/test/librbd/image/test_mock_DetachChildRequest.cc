...
template<> class RemoveRequest<MockTestImageCtx>
{
private:
...
public:
    ...
    static RemoveRequest* create(librados::IoCtx& ioctx, MockTestImageCtx* image_ctx, ContextWQ* op_work_queue,
                                 bool force, ProgressContext& prog_ctx, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
...
public:
    ...
    void expect_create_ioctx(MockImageCtx& mock_image_ctx, librados::MockTestMemIoCtxImpl** io_ctx_impl)
    {
    ...
    void expect_child_detach(MockImageCtx& mock_image_ctx, librados::MockTestMemIoCtxImpl& mock_io_ctx_impl, int r)
    {
    ...
    void expect_remove_child(MockImageCtx& mock_image_ctx, int r)
    {
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    exec(RBD_CHILDREN, _, StrEq("rbd"), StrEq("remove_child"), _, _, _, _))
            .WillOnce(Return(r));
    }

    void expect_snapshot_get(MockImageCtx& mock_image_ctx, librados::MockTestMemIoCtxImpl& mock_io_ctx_impl,
                             const std::string& parent_header_name, const cls::rbd::SnapshotInfo& snap_info, int r)
    {

        using ceph::encode;
        EXPECT_CALL(mock_io_ctx_impl, exec(parent_header_name, _, StrEq("rbd"), StrEq("snapshot_get"), _, _, _, _))
    ...
    void expect_open(MockImageCtx& mock_image_ctx, int r)
    {
        EXPECT_CALL(*mock_image_ctx.state, open(true, _))
            .WillOnce(WithArg<1>(Invoke([this, &mock_image_ctx, r](Context* ctx) {
                EXPECT_EQ(0U, mock_image_ctx.read_only_mask & IMAGE_READ_ONLY_FLAG_NON_PRIMARY);
                image_ctx->op_work_queue->queue(ctx, r);
            })));
        if (r == 0) {
            EXPECT_CALL(mock_image_ctx, test_features(_)).WillOnce(Return(false));
        }
    }
    ...
    void expect_snap_remove(MockImageCtx& mock_image_ctx, const std::string& snap_name, int r)
    {
        EXPECT_CALL(*mock_image_ctx.operations, snap_remove({cls::rbd::TrashSnapshotNamespace{}}, StrEq(snap_name), _))
            .WillOnce(WithArg<2>(Invoke([this, r](Context* ctx) { image_ctx->op_work_queue->queue(ctx, r); })));
    }

    void expect_trash_get(MockImageCtx& mock_image_ctx, librados::MockTestMemIoCtxImpl& mock_io_ctx_impl,
                          const cls::rbd::TrashImageSpec& trash_spec, int r)
    {
        using ceph::encode;
        EXPECT_CALL(mock_io_ctx_impl, exec(RBD_TRASH, _, StrEq("rbd"), StrEq("trash_get"), _, _, _, _))
    ...
    void expect_trash_remove(MockTrashRemoveRequest& mock_trash_remove_request, int r)
    {
        EXPECT_CALL(mock_trash_remove_request, send()).WillOnce(Invoke([&mock_trash_remove_request, r]() {
...
TEST_F(TestMockImageDetachChildRequest, SuccessV2)
{
    ...
    expect_snapshot_get(mock_image_ctx,
                        *mock_io_ctx_impl,
                        ...
                        {234, {cls::rbd::UserSnapshotNamespace{}}, "snap1", 123, {}, 0},
                        0);
...
TEST_F(TestMockImageDetachChildRequest, TrashedSnapshotSuccess)
{
    ...
    expect_snapshot_get(mock_image_ctx,
                        *mock_io_ctx_impl,
                        "rbd_header.parent id",
                        {234, {cls::rbd::TrashSnapshotNamespace{}}, "snap1", 123, {}, 0},
                        0);
...
TEST_F(TestMockImageDetachChildRequest, ParentAutoRemove)
{
    ...
    expect_open(mock_image_ctx, 0);
    expect_snap_remove(mock_image_ctx, "snap1", 0);
    const cls::rbd::TrashImageSpec trash_spec = {cls::rbd::TRASH_IMAGE_SOURCE_USER_PARENT, "parent", {}, {}};
...
TEST_F(TestMockImageDetachChildRequest, TrashedSnapshotInUse)
{
    ...
    expect_snapshot_get(mock_image_ctx,
                        *mock_io_ctx_impl,
                        "rbd_header.parent id",
                        {234, {cls::rbd::TrashSnapshotNamespace{}}, "snap1", 123, {}, 1},
                        0);
