...
struct MockTestImageCtx : public MockImageCtx
{
    ...
    static MockTestImageCtx* create(const std::string& image_name, const std::string& image_id,
...
template<> struct MetadataCopyRequest<MockTestImageCtx>
{
    Context* on_finish = nullptr;

    static MetadataCopyRequest* s_instance;
    static MetadataCopyRequest* create(MockTestImageCtx* src_image_ctx, MockTestImageCtx* dst_image_ctx,
                                       Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
    ...
    MetadataCopyRequest() { s_instance = this; }
...
template<> struct AttachChildRequest<MockTestImageCtx>
{
    uint32_t clone_format;
    Context* on_finish = nullptr;
    static AttachChildRequest* s_instance;
    static AttachChildRequest* create(MockTestImageCtx*, MockTestImageCtx*, const librados::snap_t&, MockTestImageCtx*,
                                      const librados::snap_t&, uint32_t clone_format, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
    ...
    AttachChildRequest() { s_instance = this; }
};

AttachChildRequest<MockTestImageCtx>* AttachChildRequest<MockTestImageCtx>::s_instance = nullptr;

template<> struct AttachParentRequest<MockTestImageCtx>
{
    Context* on_finish = nullptr;
    static AttachParentRequest* s_instance;
    static AttachParentRequest* create(MockTestImageCtx&, const cls::rbd::ParentImageSpec& pspec,
                                       uint64_t parent_overlap, bool reattach, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->on_finish = on_finish;
        return s_instance;
...
template<> struct CreateRequest<MockTestImageCtx>
{
    Context* on_finish = nullptr;
    static CreateRequest* s_instance;
    static CreateRequest* create(const ConfigProxy& config, IoCtx& ioctx, const std::string& image_name,
...
template<> struct RemoveRequest<MockTestImageCtx>
{
    Context* on_finish = nullptr;
    ...
    static RemoveRequest* create(librados::IoCtx& ioctx, const std::string& image_name, const std::string& image_id,
                                 bool force, bool from_trash_remove, ProgressContext& prog_ctx,
                                 asio::ContextWQ* op_work_queue, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->on_finish = on_finish;
...
template<> struct EnableRequest<MockTestImageCtx>
{
    Context* on_finish = nullptr;
    static EnableRequest* s_instance;
    static EnableRequest* create(MockTestImageCtx* image_ctx, cls::rbd::MirrorImageMode mode,
                                 const std::string& non_primary_global_image_id, bool image_clean, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
...
public:
    ...
    void SetUp() override
    {
        ...
        ASSERT_EQ(0, image_ctx->operations->snap_create(cls::rbd::UserSnapshotNamespace{}, "snap", 0, prog_ctx));
        if (is_feature_enabled(RBD_FEATURE_LAYERING)) {
            ASSERT_EQ(0, image_ctx->operations->snap_protect(cls::rbd::UserSnapshotNamespace{}, "snap"));

            uint64_t snap_id = image_ctx->snap_ids[{cls::rbd::UserSnapshotNamespace{}, "snap"}];
    ...
    void expect_get_image_size(MockTestImageCtx& mock_image_ctx, uint64_t snap_id, uint64_t size)
    {
        EXPECT_CALL(mock_image_ctx, get_image_size(snap_id)).WillOnce(Return(size));
    }

    void expect_is_snap_protected(MockImageCtx& mock_image_ctx, bool is_protected, int r)
    {
    ...
    {
        EXPECT_CALL(*mock_image_ctx.state, open(true, _)).WillOnce(WithArg<1>(Invoke([this, r](Context* ctx) {
    ...
    {
        EXPECT_CALL(mock_request, send()).WillOnce(Invoke([this, &mock_request, r]() {
            image_ctx->op_work_queue->queue(mock_request.on_finish, r);
        }));
    }

    void expect_attach_child(MockAttachChildRequest& mock_request, uint32_t clone_format, int r)
    {
        EXPECT_CALL(mock_request, send()).WillOnce(Invoke([this, &mock_request, clone_format, r]() {
    ...
    void expect_test_features(MockTestImageCtx& mock_image_ctx, uint64_t features, bool enabled)
    {
        EXPECT_CALL(mock_image_ctx, test_features(features)).WillOnce(Return(enabled));
    }

    void expect_mirror_mode_get(MockTestImageCtx& mock_image_ctx, cls::rbd::MirrorMode mirror_mode, int r)
    {
        bufferlist out_bl;
        encode(static_cast<uint32_t>(mirror_mode), out_bl);

        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    exec(RBD_MIRRORING, _, StrEq("rbd"), StrEq("mirror_mode_get"), _, _, _, _))
    ...
    void expect_mirror_enable(MockMirrorEnableRequest& mock_mirror_enable_request, int r)
    {
        EXPECT_CALL(mock_mirror_enable_request, send()).WillOnce(Invoke([this, &mock_mirror_enable_request, r]() {
    ...
    {
        EXPECT_CALL(mock_remove_request, send()).WillOnce(Invoke([this, &mock_remove_request, r]() {
...
TEST_F(TestMockImageCloneRequest, SuccessV1)
{
    ...
    auto req = new MockCloneRequest(m_cct->_conf,
                                    m_ioctx,
                                    "parent id",
                                    "",
                                    {},
