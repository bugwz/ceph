...
template<> struct RefreshRequest<MockTestImageCtx>
{
    ...
    static RefreshRequest* create(MockTestImageCtx& image_ctx, bool acquiring_lock, bool skip_open_parent,
                                  Context* on_finish)
    {
...
public:
    ...
    {
        EXPECT_CALL(mock_refresh_request, send()).WillOnce(Invoke([this, &mock_refresh_request, r]() {
    ...
    void expect_op_features_set(MockImageCtx& mock_image_ctx, int r)
    {
        ...
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    exec(util::header_name(mock_image_ctx.id),
                         _,
                         StrEq("rbd"),
                         StrEq("op_features_set"),
                         ContentsEqual(bl),
                         _,
                         _,
                         _))
    ...
    void expect_child_attach(MockImageCtx& mock_image_ctx, int r)
    {
        ...
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    exec(mock_image_ctx.header_oid, _, StrEq("rbd"), StrEq("child_attach"), ContentsEqual(bl), _, _, _))
            .WillOnce(Return(r));
    }
...
TEST_F(TestMockImageAttachChildRequest, SuccessV1)
{
    ...
    C_SaferCond ctx;
    auto req =
        MockAttachChildRequest::create(&mock_image_ctx, &mock_image_ctx, image_ctx->snap_id, nullptr, 0, 1, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(TestMockImageAttachChildRequest, SuccessV2)
{
    ...
    C_SaferCond ctx;
    auto req =
        MockAttachChildRequest::create(&mock_image_ctx, &mock_image_ctx, image_ctx->snap_id, nullptr, 0, 2, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
