...
public:
    ...
    void expect_list_watchers(MockTestImageCtx& mock_image_ctx, const std::string oid,
                              const std::list<obj_watch_t>& watchers, int r)
    {
        auto& expect = EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx), list_watchers(oid, _));
        if (r < 0) {
    ...
    void expect_list_image_watchers(MockTestImageCtx& mock_image_ctx, const std::list<obj_watch_t>& watchers, int r)
    {
        expect_list_watchers(mock_image_ctx, mock_image_ctx.header_oid, watchers, r);
    }

    void expect_list_mirror_watchers(MockTestImageCtx& mock_image_ctx, const std::list<obj_watch_t>& watchers, int r)
    {
        expect_list_watchers(mock_image_ctx, RBD_MIRRORING, watchers, r);
    }

    void expect_get_watch_handle(MockImageWatcher& mock_watcher, uint64_t watch_handle)
    {
...
TEST_F(TestMockListWatchersRequest, Success)
{
    ...
    InSequence seq;
    expect_list_image_watchers(mock_image_ctx, {watcher("a", 123), watcher("b", 456)}, 0);
...
TEST_F(TestMockListWatchersRequest, FilterOutMyInstance)
{
    ...
    std::list<obj_watch_t> watchers;
    C_SaferCond ctx;
    auto req = MockListWatchersRequest::create(mock_image_ctx, LIST_WATCHERS_FILTER_OUT_MY_INSTANCE, &watchers, &ctx);
    req->send();
...
TEST_F(TestMockListWatchersRequest, FilterOutMirrorInstance)
{
    ...
    C_SaferCond ctx;
    auto req =
        MockListWatchersRequest::create(mock_image_ctx, LIST_WATCHERS_FILTER_OUT_MIRROR_INSTANCES, &watchers, &ctx);
    req->send();
