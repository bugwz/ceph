...
public:
    ...
    void expect_read_rbd_info(librados::MockTestMemIoCtxImpl& mock_io_ctx, const std::string& data, int r)
    {
        auto& expect = EXPECT_CALL(mock_io_ctx, read(StrEq(RBD_INFO), 0, 0, _, _, _));
        if (r < 0) {
    ...
    void expect_write_rbd_info(librados::MockTestMemIoCtxImpl& mock_io_ctx, const std::string& data, int r)
    {
        bufferlist bl;
        bl.append(data);

        EXPECT_CALL(mock_io_ctx, write(StrEq(RBD_INFO), ContentsEqual(bl), data.length(), 0, _)).WillOnce(Return(r));
    }

    void expect_allocate_snap_id(librados::MockTestMemIoCtxImpl& mock_io_ctx, int r)
    {
    ...
    void expect_release_snap_id(librados::MockTestMemIoCtxImpl& mock_io_ctx, int r)
    {
