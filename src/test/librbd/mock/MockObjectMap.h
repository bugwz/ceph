...
struct MockObjectMap
{
    ...
    uint8_t operator[](uint64_t object_no) { return at(object_no); }
    ...
    MOCK_METHOD3(aio_resize, void(uint64_t new_size, uint8_t default_object_state, Context* on_finish));
                    ...
                    bool ignore_enoent, T* callback_object)
    {
        ...
        bool updated = aio_update(
            snap_id, start_object_no, end_object_no, new_state, current_state, parent_trace, ignore_enoent, ctx);
        if (!updated) {
            delete ctx;
        }
        return updated;
    }
    MOCK_METHOD8(aio_update, bool(uint64_t snap_id, uint64_t start_object_no, uint64_t end_object_no, uint8_t new_state,
                                  const boost::optional<uint8_t>& current_state, const ZTracer::Trace& parent_trace,
