...
TEST_F(TestLibRBD, TestEncryptionLUKS1)
{
    ...
    ASSERT_EQ(-ENOTSUP, rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS1, &opts, sizeof(opts)));
    ASSERT_EQ(-ENOTSUP, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS1, &opts, sizeof(opts)));
#else
    ASSERT_EQ(0, rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS1, &opts, sizeof(opts)));
    ASSERT_EQ(-EEXIST, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS1, &opts, sizeof(opts)));
    ...
    ASSERT_EQ(0, rbd_open(ioctx, name.c_str(), &image, NULL));
    ASSERT_EQ(0, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS1, &opts, sizeof(opts)));
...
TEST_F(TestLibRBD, TestEncryptionLUKS2)
{
...
#ifndef HAVE_LIBCRYPTSETUP
    ASSERT_EQ(-ENOTSUP, rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS2, &opts, sizeof(opts)));
    ASSERT_EQ(-ENOTSUP, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS2, &opts, sizeof(opts)));
#else
    ASSERT_EQ(0, rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS2, &opts, sizeof(opts)));
    ASSERT_EQ(-EEXIST, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS2, &opts, sizeof(opts)));
    ...
    ASSERT_EQ(0, rbd_open(ioctx, name.c_str(), &image, NULL));
    ASSERT_EQ(0, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS2, &opts, sizeof(opts)));
    read_test_data(image, "test", 0, 4, 0, &passed);
    ASSERT_TRUE(passed);
#endif
...
TYPED_TEST(DiffIterateTest, DiffIterateDeterministic)
{
    ...
    rados_ioctx_t ioctx;
    ASSERT_EQ(0, rados_ioctx_create(this->_cluster, this->m_pool_name.c_str(), &ioctx));
    ...
    ASSERT_EQ(0, rbd_diff_iterate2(image, NULL, 0, size, true, this->whole_object, vector_iterate_cb, &extents));
    ...
    // 2. snap1 -> HEAD
    ASSERT_EQ(0, rbd_diff_iterate2(image, "snap1", 0, size, true, this->whole_object, vector_iterate_cb, &extents));
    ASSERT_EQ(2u, extents.size());
    ASSERT_EQ(diff_extent(0, 256, true, object_size), extents[0]);
    ASSERT_EQ(diff_extent(1 << order, 256, true, object_size), extents[1]);
    extents.clear();
    ...
    // 3. snap2 -> HEAD
    ASSERT_EQ(0, rbd_diff_iterate2(image, "snap2", 0, size, true, this->whole_object, vector_iterate_cb, &extents));
    ASSERT_EQ(1u, extents.size());
    ASSERT_EQ(diff_extent(1 << order, 256, true, object_size), extents[0]);
    extents.clear();

    // 4. snap3 -> HEAD
    ASSERT_EQ(0, rbd_diff_iterate2(image, "snap3", 0, size, true, this->whole_object, vector_iterate_cb, &extents));
    ASSERT_EQ(0u, extents.size());
...
TYPED_TEST(DiffIterateTest, DiffIterateDeterministicPP)
{
    ...
    // 2. snap1 -> HEAD
    ASSERT_EQ(0, image.diff_iterate2("snap1", 0, size, true, this->whole_object, vector_iterate_cb, &extents));
    ASSERT_EQ(2u, extents.size());
    ASSERT_EQ(diff_extent(0, 256, true, object_size), extents[0]);
    ASSERT_EQ(diff_extent(1 << order, 256, true, object_size), extents[1]);
    extents.clear();
    ...
    // 3. snap2 -> HEAD
    ASSERT_EQ(0, image.diff_iterate2("snap2", 0, size, true, this->whole_object, vector_iterate_cb, &extents));
    ASSERT_EQ(1u, extents.size());
    ASSERT_EQ(diff_extent(1 << order, 256, true, object_size), extents[0]);
    extents.clear();

    // 4. snap3 -> HEAD
    ASSERT_EQ(0, image.diff_iterate2("snap3", 0, size, true, this->whole_object, vector_iterate_cb, &extents));
    ASSERT_EQ(0u, extents.size());
...
TYPED_TEST(DiffIterateTest, DiffIterateTryAcquireLock)
{
...

    {
        ...
        std::vector<diff_extent> extents;
        ASSERT_EQ(0, image2.diff_iterate2(NULL, 0, size, true, this->whole_object, vector_iterate_cb, &extents));
        ASSERT_EQ(1u, extents.size());
        ASSERT_EQ(diff_extent(0, 256, true, object_size), extents[0]);
        extents.clear();
        ...
        ASSERT_EQ(0, image2.is_exclusive_lock_owner(&lock_owner));
        if (this->whole_object &&
            (is_feature_enabled(RBD_FEATURE_OBJECT_MAP) || is_feature_enabled(RBD_FEATURE_FAST_DIFF))) {
            ASSERT_TRUE(lock_owner);
        }
        else {
            ASSERT_FALSE(lock_owner);
