...
static int create_image_full(rados_ioctx_t ioctx, const char* name, uint64_t size, int* order, int old_format,
                             uint64_t features)
{
    if (old_format) {
        // ensure old-format tests actually use the old format
        int r = rados_conf_set(rados_ioctx_get_cluster(ioctx), "rbd_default_format", "1");
        if (r < 0) {
            return r;
        }
        ...
        printf("creating image with stripe unit: %" PRIu64 ", "
               ...
               stripe_unit,
               IMAGE_STRIPE_COUNT);
        return rbd_create3(ioctx, name, size, features, order, stripe_unit, IMAGE_STRIPE_COUNT);
    }
    else {
...
static int clone_image(rados_ioctx_t p_ioctx, rbd_image_t p_image, const char* p_name, const char* p_snap_name,
                       rados_ioctx_t c_ioctx, const char* c_name, uint64_t features, int* c_order)
{
    ...
    return rbd_clone2(p_ioctx, p_name, p_snap_name, c_ioctx, c_name, features, c_order, stripe_unit, stripe_count);
}


static int create_image(rados_ioctx_t ioctx, const char* name, uint64_t size, int* order)
{
    bool old_format;
    uint64_t features;
...
static int create_image_pp(librbd::RBD& rbd, librados::IoCtx& ioctx, const char* name, uint64_t size, int* order)
{
    bool old_format;
    uint64_t features;
    int r = get_features(&old_format, &features);
...
void aio_write_test_data_and_poll(rbd_image_t image, int fd, const char* test_data, uint64_t off, size_t len,
                                  uint32_t iohint, bool* passed)
{
    ...
    uint64_t count;
    ASSERT_EQ(static_cast<ssize_t>(sizeof(count)), read(fd, &count, sizeof(count)));
...
void aio_write_test_data(rbd_image_t image, const char* test_data, uint64_t off, size_t len, uint32_t iohint,
                         bool* passed)
{
    rbd_completion_t comp;
...
void aio_read_test_data_and_poll(rbd_image_t image, int fd, const char* expected, uint64_t off, size_t len,
                                 uint32_t iohint, bool* passed)
{
    rbd_completion_t comp;
...
void aio_read_test_data(rbd_image_t image, const char* expected, uint64_t off, size_t len, uint32_t iohint,
                        bool* passed)
{
    rbd_completion_t comp;
    char* result = (char*)malloc(len + 1);
...
void aio_writesame_test_data(rbd_image_t image, const char* test_data, uint64_t off, uint64_t len, uint64_t data_len,
                             uint32_t iohint, bool* passed)
{
    rbd_completion_t comp;
    rbd_aio_create_completion(NULL, (rbd_callback_t)simple_write_cb, &comp);
...
void writesame_test_data(rbd_image_t image, const char* test_data, uint64_t off, uint64_t len, uint64_t data_len,
                         uint32_t iohint, bool* passed)
{
    ssize_t written;
...
void aio_compare_and_write_test_data(rbd_image_t image, const char* cmp_data, const char* test_data, uint64_t off,
...
void compare_and_write_test_data(rbd_image_t image, const char* cmp_data, const char* test_data, uint64_t off,
                                 size_t len, uint64_t* mismatch_off, uint32_t iohint, bool* passed)
{
...
public:
    ...
    static void TearDownTestCase()
    {
        ...
        _pool_names.insert(_pool_names.end(), _unique_pool_names.begin(), _unique_pool_names.end());
    ...
    void SetUp() override { ASSERT_NE("", m_pool_name = create_pool()); }
    ...
    void test_io(rbd_image_t image)
    {
        ...
        for (i = 0; i < 5; ++i) ASSERT_PASSED(write_test_data, image, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, 0);

        for (i = 5; i < 10; ++i)
            ASSERT_PASSED(aio_write_test_data, image, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, 0);

        for (i = 0; i < 5; ++i)
            ASSERT_PASSED(compare_and_write_test_data,
                          image,
                          test_data,
                          test_data,
                          TEST_IO_SIZE * i,
        ...
        for (i = 5; i < 10; ++i)
            ASSERT_PASSED(
                aio_compare_and_write_test_data, image, test_data, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, 0);

        for (i = 0; i < 5; ++i) ASSERT_PASSED(read_test_data, image, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, 0);

        for (i = 5; i < 10; ++i) ASSERT_PASSED(aio_read_test_data, image, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, 0);
        ...
        ASSERT_PASSED(read_test_data, image, skip_discard ? test_data : zero_data, TEST_IO_SIZE, TEST_IO_SIZE, 0);
        ASSERT_PASSED(read_test_data, image, test_data, TEST_IO_SIZE * 2, TEST_IO_SIZE, 0);
        ASSERT_PASSED(read_test_data, image, skip_discard ? test_data : zero_data, TEST_IO_SIZE * 3, TEST_IO_SIZE, 0);
        ASSERT_PASSED(read_test_data, image, test_data, TEST_IO_SIZE * 4, TEST_IO_SIZE, 0);

        for (i = 0; i < 15; ++i) {
            if (i % 3 == 2) {
                ASSERT_PASSED(writesame_test_data,
                              image,
                              test_data,
                              TEST_IO_SIZE * i,
                              TEST_IO_SIZE * i * 32 + i,
                ...
                ASSERT_PASSED(
                    writesame_test_data, image, test_data, TEST_IO_SIZE + i, TEST_IO_SIZE * i * 32, TEST_IO_SIZE, 0);
                ASSERT_PASSED(
                    writesame_test_data, image, zero_data, TEST_IO_SIZE + i, TEST_IO_SIZE * i * 32, TEST_IO_SIZE, 0);
            }
            else {
                ASSERT_PASSED(
                    writesame_test_data, image, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE * i * 32, TEST_IO_SIZE, 0);
        ...
        for (i = 0; i < 15; ++i) {
            if (i % 3 == 2) {
                ASSERT_PASSED(aio_writesame_test_data,
                              image,
                              test_data,
                              TEST_IO_SIZE * i,
                              TEST_IO_SIZE * i * 32 + i,
        ...
        mismatch_offset = 123;
        ASSERT_EQ(-EILSEQ,
                  rbd_compare_and_write(image, 0, TEST_IO_SIZE, mismatch_data, mismatch_data, &mismatch_offset, 0));
        ASSERT_EQ(0U, mismatch_offset);
        rbd_aio_create_completion(NULL, (rbd_callback_t)simple_read_cb, &comp);
        mismatch_offset = 123;
        ASSERT_EQ(
            0,
            rbd_aio_compare_and_write(image, 0, TEST_IO_SIZE, mismatch_data, mismatch_data, comp, &mismatch_offset, 0));
        ASSERT_EQ(0, rbd_aio_wait_for_complete(comp));
...
TEST_F(TestLibRBD, CreateWithSameDataPool)
{
    ...
    ASSERT_EQ(0, rbd_image_options_set_uint64(image_options, RBD_IMAGE_OPTION_FEATURES, features));
    ASSERT_EQ(0, rbd_image_options_set_string(image_options, RBD_IMAGE_OPTION_DATA_POOL, m_pool_name.c_str()));
...
TEST_F(TestLibRBD, OpenAioPP)
{
    ...
    librbd::RBD::AioCompletion* open_comp = new librbd::RBD::AioCompletion(NULL, NULL);
    ...
    // close
    librbd::RBD::AioCompletion* close_comp = new librbd::RBD::AioCompletion(NULL, NULL);
...
TEST_F(TestLibRBD, UpdateWatchAndResize)
{
    ...
    struct Watcher
    {
        ...
        explicit Watcher(rbd_image_t& image)
            : m_image(image)
        {}
        void handle_notify()
        {
        ...
        void wait_for_size(size_t size)
        {
            ...
            ASSERT_TRUE(m_cond.wait_for(locker, seconds(5), [size, this] { return this->m_size == size; }));
        }
    } watcher(image);
    uint64_t handle;
...
TEST_F(TestLibRBD, UpdateWatchAndResizePP)
{
...

    {
        ...
        {
            explicit Watcher(librbd::Image& image)
                : m_image(image)
            {}
...
int test_ls_pp(librbd::RBD& rbd, librados::IoCtx& io_ctx, size_t num_expected, ...)
{
    ...
    if (r == -ENOENT) r = 0;
    EXPECT_TRUE(r >= 0);
    cout << "num images is: " << names.size() << std::endl << "expected: " << num_expected << std::endl;
    int num = names.size();
...
TEST_F(TestLibRBD, TestCopy)
{
    ...
    ASSERT_EQ(0, rbd_copy_with_progress(image, ioctx, name3.c_str(), print_progress_percent, NULL));
    ...
    ASSERT_EQ(0, rbd_metadata_list(image3, "key", 70, keys, &keys_len, vals, &vals_len));
    ASSERT_EQ(keys_len, sum_key_len);
    ASSERT_EQ(vals_len, sum_value_len);
...
TEST_F(TestLibRBD, TestCopyPP)
{
...

    {
        ...
        ASSERT_EQ(3, test_ls_pp(rbd, ioctx, 3, name.c_str(), name2.c_str(), name3.c_str()));
...
TEST_F(TestLibRBD, TestDeepCopy)
{
    ...
    ASSERT_EQ(0, rbd_deep_copy_with_progress(image, ioctx, name3.c_str(), opts, print_progress_percent, NULL));
    ASSERT_EQ(3, test_ls(ioctx, 3, name.c_str(), name2.c_str(), name3.c_str()));
    ...
    ASSERT_EQ(0, rbd_clone3(ioctx, name.c_str(), "deep_snap", ioctx, name4.c_str(), opts));

    ASSERT_EQ(4, test_ls(ioctx, 4, name.c_str(), name2.c_str(), name3.c_str(), name4.c_str()));
    ...
    ASSERT_EQ(5, test_ls(ioctx, 5, name.c_str(), name2.c_str(), name3.c_str(), name4.c_str(), name5.c_str()));
    ASSERT_EQ(0, rbd_open(ioctx, name5.c_str(), &image5, NULL));
    BOOST_SCOPE_EXIT_ALL((&image5))
    {
        ASSERT_EQ(0, rbd_close(image5));
    };
    ASSERT_EQ(0, rbd_metadata_list(image5, "key", 70, keys, &keys_len, vals, &vals_len));
    ASSERT_EQ(keys_len, sum_key_len);
    ASSERT_EQ(vals_len, sum_value_len);
    ...
    ASSERT_EQ(0, rbd_deep_copy_with_progress(image4, ioctx, name6.c_str(), opts, print_progress_percent, NULL));
    ASSERT_EQ(
        6, test_ls(ioctx, 6, name.c_str(), name2.c_str(), name3.c_str(), name4.c_str(), name5.c_str(), name6.c_str()));
    ...
    ASSERT_EQ(0, rbd_metadata_list(image6, "key", 70, keys, &keys_len, vals, &vals_len));
    ASSERT_EQ(keys_len, sum_key_len);
    ASSERT_EQ(vals_len, sum_value_len);
...
int test_ls_snaps(librbd::Image& image, size_t num_expected, ...)
{
    ...
    EXPECT_TRUE(r >= 0);
    cout << "num snaps is: " << snaps.size() << std::endl << "expected: " << num_expected << std::endl;
    ...
    for (i = num_expected; i > 0; i--) {
        ...
        for (j = 0; j < snaps.size(); j++) {
            if (snaps[j].name == "") continue;
            if (strcmp(snaps[j].name.c_str(), expected) == 0) {
                cout << "found " << snaps[j].name << " with size " << snaps[j].size << std::endl;
                EXPECT_EQ(expected_size, snaps[j].size);
                snaps[j].name = "";
                found = 1;
                break;
...
{
    rados_ioctx_t ioctx;
    ASSERT_EQ(0, rados_ioctx_create(_cluster, create_pool(true).c_str(), &ioctx));
...
static void remove_full_try(rados_ioctx_t ioctx, const std::string& image_name, const std::string& data_pool_name)
{
    int order = 0;
    ...
    std::string cmdstr = "{\"prefix\": \"osd pool set-quota\", \"pool\": \"" + data_pool_name +
                         "\", \"field\": \"max_bytes\", \"val\": \"" + std::to_string(quota) + "\"}";
    char* cmd[1];
    cmd[0] = (char*)cmdstr.c_str();
    ASSERT_EQ(0,
              rados_mon_command(rados_ioctx_get_cluster(ioctx), (const char**)cmd, 1, "", 0, nullptr, 0, nullptr, 0));
    ...
    for (off = 0; off < size; off += len) {
        ret = rbd_write_zeroes(image, off, len, RBD_WRITE_ZEROES_FLAG_THICK_PROVISION, LIBRADOS_OP_FLAG_FADVISE_FUA);
        if (ret < 0) {
            break;
        }
        ASSERT_EQ(ret, len);
...
TEST_F(TestLibRBD, RemoveFullTry)
{
    ...
    ASSERT_EQ(0, rbd_pool_metadata_set(ioctx, "conf_rbd_default_data_pool", pool_name.c_str()));
...
TEST_F(TestLibRBD, RemoveFullTryDataPool)
{
    ...
    ASSERT_EQ(0, rados_ioctx_create(_cluster, pool_name.c_str(), &ioctx));
    ASSERT_EQ(0, rbd_pool_metadata_set(ioctx, "conf_rbd_default_data_pool", data_pool_name.c_str()));
...
TEST_F(TestLibRBD, TestEncryptionLUKS1)
{
    ...
    ASSERT_EQ(-ENOTSUP, rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS1, &luks1_opts, sizeof(luks1_opts)));
    ASSERT_EQ(-ENOTSUP, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS1, &luks1_opts, sizeof(luks1_opts)));
    ASSERT_EQ(-ENOTSUP, rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS2, &luks2_opts, sizeof(luks2_opts)));
    ASSERT_EQ(-ENOTSUP, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS2, &luks2_opts, sizeof(luks2_opts)));
    ASSERT_EQ(-ENOTSUP, rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS, &luks_opts, sizeof(luks_opts)));
    ASSERT_EQ(-ENOTSUP, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS, &luks_opts, sizeof(luks_opts)));
#else
    ASSERT_EQ(-EINVAL, rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS, &luks_opts, sizeof(luks_opts)));
    ASSERT_EQ(0, rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS1, &luks1_opts, sizeof(luks1_opts)));
    ASSERT_EQ(-EEXIST, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS1, &luks1_opts, sizeof(luks1_opts)));
    ...
    ASSERT_EQ(0, rbd_open(ioctx, name.c_str(), &image, NULL));
    ASSERT_EQ(-EINVAL, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS2, &luks2_opts, sizeof(luks2_opts)));
    ASSERT_EQ(0, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS1, &luks1_opts, sizeof(luks1_opts)));
    ASSERT_PASSED(read_test_data, image, "test", 0, 4, 0);
    ASSERT_EQ(0, rbd_close(image));

    ASSERT_EQ(0, rbd_open(ioctx, name.c_str(), &image, NULL));
    ASSERT_EQ(-EINVAL, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS2, &luks2_opts, sizeof(luks2_opts)));
    ASSERT_EQ(0, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS, &luks_opts, sizeof(luks_opts)));
    ASSERT_PASSED(read_test_data, image, "test", 0, 4, 0);
#endif
...
TEST_F(TestLibRBD, TestEncryptionLUKS2)
{
...
#else
    ASSERT_EQ(-EINVAL, rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS, &luks_opts, sizeof(luks_opts)));
    ASSERT_EQ(0, rbd_encryption_format(image, RBD_ENCRYPTION_FORMAT_LUKS2, &luks2_opts, sizeof(luks2_opts)));
    ASSERT_EQ(-EEXIST, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS2, &luks2_opts, sizeof(luks2_opts)));
    ...
    ASSERT_EQ(0, rbd_open(ioctx, name.c_str(), &image, NULL));
    ASSERT_EQ(-EINVAL, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS1, &luks1_opts, sizeof(luks1_opts)));
    ASSERT_EQ(0, rbd_encryption_load(image, RBD_ENCRYPTION_FORMAT_LUKS2, &luks2_opts, sizeof(luks2_opts)));
    ASSERT_PASSED(read_test_data, image, "test", 0, 4, 0);
    ASSERT_EQ(0, rbd_close(image));
...
TEST_F(TestLibRBD, TestCloneEncryption)
{
    ...
    ASSERT_EQ(0, rbd_clone3(ioctx, name.c_str(), "snap", ioctx, child1_name.c_str(), image_opts));
    ...
    ASSERT_EQ(-EINVAL, rbd_encryption_load(child1, RBD_ENCRYPTION_FORMAT_LUKS1, &child1_opts, sizeof(child1_opts)));
    ASSERT_EQ(0, rbd_encryption_format(child1, RBD_ENCRYPTION_FORMAT_LUKS1, &child1_opts, sizeof(child1_opts)));
    ASSERT_EQ(0, rbd_encryption_load(child1, RBD_ENCRYPTION_FORMAT_LUKS1, &child1_opts, sizeof(child1_opts)));
    ...
    ASSERT_EQ(0, rbd_clone3(ioctx, child1_name.c_str(), "snap", ioctx, child2_name.c_str(), image_opts));
    ...
    ASSERT_EQ(0, rbd_encryption_format(child2, RBD_ENCRYPTION_FORMAT_LUKS2, &child2_opts, sizeof(child2_opts)));
    rbd_encryption_luks_format_options_t child2_lopts = {
        .passphrase = "password",
        .passphrase_size = 8,
    };
    ASSERT_EQ(0, rbd_encryption_load(child2, RBD_ENCRYPTION_FORMAT_LUKS, &child2_lopts, sizeof(child2_lopts)));
    ...
    ASSERT_EQ(0, rbd_clone3(ioctx, child2_name.c_str(), "snap", ioctx, child3_name.c_str(), image_opts));
    ...
    ASSERT_EQ(0, rbd_encryption_format(child3, RBD_ENCRYPTION_FORMAT_LUKS2, &child3_opts, sizeof(child3_opts)));
    ASSERT_EQ(-EPERM, rbd_encryption_load(child3, RBD_ENCRYPTION_FORMAT_LUKS2, &child3_opts, sizeof(child3_opts)));

    // verify child3 data
    rbd_encryption_spec_t specs[] = {
        {.format = RBD_ENCRYPTION_FORMAT_LUKS2, .opts = &child3_opts, .opts_size = sizeof(child3_opts)},
        {.format = RBD_ENCRYPTION_FORMAT_LUKS2, .opts = &child2_opts, .opts_size = sizeof(child2_opts)},
        {.format = RBD_ENCRYPTION_FORMAT_LUKS1, .opts = &child1_opts, .opts_size = sizeof(child1_opts)}};
    ...
    ASSERT_EQ(0, rbd_clone3(ioctx, child3_name.c_str(), "snap", ioctx, child4_name.c_str(), image_opts));
    ...
    ASSERT_EQ(0, rbd_open(ioctx, child4_name.c_str(), &child4, NULL));
    ASSERT_EQ(0, rbd_encryption_load(child4, RBD_ENCRYPTION_FORMAT_LUKS2, &child3_opts, sizeof(child3_opts)));
...
TEST_F(TestLibRBD, LUKS1UnderLUKS2WithoutResize)
{
    ...
    {
        int order = 22;
        ASSERT_EQ(0, create_image_pp(rbd, ioctx, parent_name.c_str(), luks1_meta_size + data_size, &order));
        librbd::Image parent;
        ASSERT_EQ(0, rbd.open(ioctx, parent, parent_name.c_str(), nullptr));

        librbd::encryption_luks1_format_options_t fopts = {RBD_ENCRYPTION_ALGORITHM_AES256, parent_passphrase};
        ASSERT_EQ(0, parent.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS1, &fopts, sizeof(fopts)));
        ...
        ASSERT_EQ(0, rbd.clone(ioctx, parent_name.c_str(), "snap", ioctx, clone_name.c_str(), features, &order));
    }

    {
        ...
        ASSERT_EQ(0, clone.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS2, &fopts, sizeof(fopts)));
        ...
        librbd::encryption_spec_t specs[] = {{RBD_ENCRYPTION_FORMAT_LUKS, &opts2, sizeof(opts2)},
        ...
        expected_bl.append(std::string(data_size + luks1_meta_size - luks2_meta_size, 'a'));

        ceph::bufferlist read_bl;
        ASSERT_EQ(expected_bl.length(), clone.read(0, expected_bl.length(), read_bl));
...
TEST_F(TestLibRBD, LUKS2UnderLUKS1WithoutResize)
{
    ...
    {
        int order = 22;
        ASSERT_EQ(0, create_image_pp(rbd, ioctx, parent_name.c_str(), luks2_meta_size + data_size, &order));
        librbd::Image parent;
        ASSERT_EQ(0, rbd.open(ioctx, parent, parent_name.c_str(), nullptr));

        librbd::encryption_luks2_format_options_t fopts = {RBD_ENCRYPTION_ALGORITHM_AES256, parent_passphrase};
        ASSERT_EQ(0, parent.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS2, &fopts, sizeof(fopts)));
...

    {
        ...
        librbd::encryption_luks1_format_options_t fopts = {RBD_ENCRYPTION_ALGORITHM_AES256, clone_passphrase};
        ASSERT_EQ(0, clone.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS1, &fopts, sizeof(fopts)));
...
TEST_F(TestLibRBD, EncryptionFormatNoData)
{
    ...
    {
        int order = 0;
        ASSERT_EQ(0, create_image_pp(rbd, ioctx, name.c_str(), luks1_meta_size - 1, &order));
        librbd::Image image;
        ASSERT_EQ(0, rbd.open(ioctx, image, name.c_str(), nullptr));

        librbd::encryption_luks1_format_options_t opts = {RBD_ENCRYPTION_ALGORITHM_AES256, passphrase};
        ASSERT_EQ(-ENOSPC, image.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS1, &opts, sizeof(opts)));
        uint64_t size;
...

    {
        ...
        librbd::encryption_luks1_format_options_t opts = {RBD_ENCRYPTION_ALGORITHM_AES256, passphrase};
        ASSERT_EQ(0, image.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS1, &opts, sizeof(opts)));
        uint64_t size;
        ASSERT_EQ(0, image.size(&size));
        ASSERT_EQ(0, size);
    }
...
TEST_F(TestLibRBD, EncryptionLoadBadSize)
{
    ...
    {
        int order = 0;
        ASSERT_EQ(0, create_image_pp(rbd, ioctx, name.c_str(), luks1_meta_size, &order));
        librbd::Image image;
        ASSERT_EQ(0, rbd.open(ioctx, image, name.c_str(), nullptr));
...

    {
        ...
        ASSERT_EQ(0, image.encryption_load(RBD_ENCRYPTION_FORMAT_LUKS, &opts, sizeof(opts)));
        uint64_t size;
        ASSERT_EQ(0, image.size(&size));
        ASSERT_EQ(0, size);
    }

    {
        ...
        librbd::encryption_luks_format_options_t opts = {passphrase};
        ASSERT_EQ(-EINVAL, image.encryption_load(RBD_ENCRYPTION_FORMAT_LUKS, &opts, sizeof(opts)));
        uint64_t size;
        ASSERT_EQ(0, image.size(&size));
        ASSERT_EQ(luks1_meta_size - 1, size);
    }
...
TEST_F(TestLibRBD, EncryptionLoadBadStripePattern)
{
    ...
    {
        int order = 22;
        ASSERT_EQ(0, rbd.create3(ioctx, name1.c_str(), 20 << 20, features, &order, 2 << 20, 2));
        librbd::Image image;
...
TEST_F(TestLibRBD, EncryptionLoadFormatMismatch)
{
    ...
    librbd::encryption_luks1_format_options_t luks1_opts = {RBD_ENCRYPTION_ALGORITHM_AES256, passphrase};
    librbd::encryption_luks2_format_options_t luks2_opts = {RBD_ENCRYPTION_ALGORITHM_AES256, passphrase};
    librbd::encryption_luks_format_options_t luks_opts = {passphrase};

#define LUKS_ONE                                                     \
    {                                                                \
        RBD_ENCRYPTION_FORMAT_LUKS1, &luks1_opts, sizeof(luks1_opts) \
    }
#define LUKS_TWO                                                     \
    {                                                                \
        RBD_ENCRYPTION_FORMAT_LUKS2, &luks2_opts, sizeof(luks2_opts) \
    }
#define LUKS_ANY                                                  \
    {                                                             \
        RBD_ENCRYPTION_FORMAT_LUKS, &luks_opts, sizeof(luks_opts) \
    }
    ...
    const std::vector<librbd::encryption_spec_t> good_specs[] = {{LUKS_ANY},
                                                                 {LUKS_TWO, LUKS_ONE},
                                                                 {LUKS_TWO, LUKS_ANY},
                                                                 {LUKS_ANY, LUKS_ONE},
                                                                 {LUKS_ANY, LUKS_ANY},
...
#undef LUKS_ANY

    {
        ...
        ASSERT_EQ(0, image1.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS1, &luks1_opts, sizeof(luks1_opts)));

        ASSERT_EQ(0, image1.snap_create("snap"));
        ASSERT_EQ(0, image1.snap_protect("snap"));
        uint64_t features;
        ASSERT_EQ(0, image1.features(&features));
        ASSERT_EQ(0, rbd.clone(ioctx, name1.c_str(), "snap", ioctx, name2.c_str(), features, &order));
        ...
        ASSERT_EQ(0, rbd.clone(ioctx, name2.c_str(), "snap", ioctx, name3.c_str(), features, &order));

        librbd::Image image3;
        ASSERT_EQ(0, rbd.open(ioctx, image3, name3.c_str(), nullptr));
        ASSERT_EQ(0, image3.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS2, &luks2_opts, sizeof(luks2_opts)));
    }
...
TEST_F(TestLibRBD, EncryptedResize)
{
    ...
    {
        int order = 0;
        ASSERT_EQ(0, create_image_pp(rbd, ioctx, name.c_str(), luks2_meta_size + data_size, &order));
        librbd::Image image;
        ASSERT_EQ(0, rbd.open(ioctx, image, name.c_str(), nullptr));
        ...
        librbd::encryption_luks2_format_options_t opts = {RBD_ENCRYPTION_ALGORITHM_AES256, passphrase};
        ASSERT_EQ(0, image.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS2, &opts, sizeof(opts)));
        ASSERT_EQ(0, image.size(&size));
...
TEST_F(TestLibRBD, EncryptedFlattenSmallData)
{
...

    {
        ...
        librbd::encryption_luks2_format_options_t opts = {RBD_ENCRYPTION_ALGORITHM_AES256, passphrase};
        ASSERT_EQ(0, parent.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS2, &opts, sizeof(opts)));
...

    {
        ...
        librbd::encryption_luks_format_options_t opts = {passphrase};
        ASSERT_EQ(0, clone.encryption_load(RBD_ENCRYPTION_FORMAT_LUKS, &opts, sizeof(opts)));
        uint64_t size;
        ASSERT_EQ(0, clone.size(&size));
        ASSERT_EQ(data_size, size);
        uint64_t overlap;
...
struct LUKSOnePassphrase
{
    int load(librbd::Image& clone)
    {
        ...
        return clone.encryption_load(RBD_ENCRYPTION_FORMAT_LUKS, &opts, sizeof(opts));
    }

    int load_flattened(librbd::Image& clone) { return load(clone); }
...
protected:
    ...
    {
        librbd::encryption_luks1_format_options_t fopts = {RBD_ENCRYPTION_ALGORITHM_AES256, m_passphrase};
        ASSERT_EQ(0, clone.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS1, &fopts, sizeof(fopts)));
        *passed = true;
    }
};
...
struct UnformattedLUKS1 : LUKSOnePassphrase
{
    ...
    void setup_clone(librbd::Image& clone, uint64_t data_size, bool* passed) { *passed = true; }
};
...
struct LUKS1UnderLUKS1 : LUKSTwoPassphrases
{
    void setup_parent(librbd::Image& parent, uint64_t data_size, bool* passed)
    {
        uint64_t luks1_meta_size = 4 << 20;
        ASSERT_EQ(0, parent.resize(data_size + luks1_meta_size));
        librbd::encryption_luks1_format_options_t fopts = {RBD_ENCRYPTION_ALGORITHM_AES256, m_parent_passphrase};
        ASSERT_EQ(0, parent.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS1, &fopts, sizeof(fopts)));
    ...
    void setup_clone(librbd::Image& clone, uint64_t data_size, bool* passed)
    {
        librbd::encryption_luks1_format_options_t fopts = {RBD_ENCRYPTION_ALGORITHM_AES256, m_clone_passphrase};
        ASSERT_EQ(0, clone.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS1, &fopts, sizeof(fopts)));
        *passed = true;
    }
};
...
struct LUKS1UnderLUKS2 : LUKSTwoPassphrases
{
    ...
    void setup_clone(librbd::Image& clone, uint64_t data_size, bool* passed)
    {
        librbd::encryption_luks2_format_options_t fopts = {RBD_ENCRYPTION_ALGORITHM_AES256, m_clone_passphrase};
        ASSERT_EQ(0, clone.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS2, &fopts, sizeof(fopts)));
        *passed = true;
    }
};
...
struct LUKS2UnderLUKS2 : LUKSTwoPassphrases
{
    void setup_parent(librbd::Image& parent, uint64_t data_size, bool* passed)
    {
        uint64_t luks2_meta_size = 16 << 20;
        ASSERT_EQ(0, parent.resize(data_size + luks2_meta_size));
        librbd::encryption_luks2_format_options_t fopts = {RBD_ENCRYPTION_ALGORITHM_AES256, m_parent_passphrase};
        ASSERT_EQ(0, parent.encryption_format(RBD_ENCRYPTION_FORMAT_LUKS2, &fopts, sizeof(fopts)));
...
template<typename FormatPolicy> class EncryptedFlattenTest : public TestLibRBD, FormatPolicy
{
protected:
    void create_and_setup(bool* passed)
    {
        ...
        int order = 22;
        ASSERT_EQ(0, create_image_pp(m_rbd, m_ioctx, m_parent_name.c_str(), m_data_size, &order));
        librbd::Image parent;
        ...
        ASSERT_EQ(0, parent.features(&features));
        ASSERT_EQ(0,
                  m_rbd.clone(m_ioctx, m_parent_name.c_str(), "snap", m_ioctx, m_clone_name.c_str(), features, &order));
        librbd::Image clone;
    ...
    void verify_size_and_overlap(librbd::Image& image, uint64_t expected_size, uint64_t expected_overlap)
    {
        uint64_t size;
        ASSERT_EQ(0, image.size(&size));
    ...
    {
        ceph::bufferlist read_bl;
        ASSERT_EQ(expected_bl.length(), image.read(0, expected_bl.length(), read_bl));
        EXPECT_TRUE(expected_bl.contents_equal(read_bl));
    }
...
using EncryptedFlattenTestTypes =
    ::testing::Types<PlaintextUnderLUKS1, PlaintextUnderLUKS2, UnformattedLUKS1, LUKS1UnderLUKS1, LUKS1UnderLUKS2,
...
TYPED_TEST(EncryptedFlattenTest, Grow)
{
...

    {
        ...
        this->verify_size_and_overlap(clone, this->m_data_size + 1, this->m_data_size);
        this->verify_data(clone, expected_bl);
        ASSERT_EQ(0, clone.flatten());
        this->verify_data(clone, expected_bl);
    }
...
TYPED_TEST(EncryptedFlattenTest, Shrink)
{
...

    {
        ...
        ASSERT_PASSED(this->open_and_load, clone);
        ASSERT_EQ(0, clone.resize(this->m_data_size - 1));
        this->verify_size_and_overlap(clone, this->m_data_size - 1, this->m_data_size - 1);
        this->verify_data(clone, expected_bl);
        ASSERT_EQ(0, clone.flatten());
        this->verify_data(clone, expected_bl);
    }
...
TEST_F(TestLibRBD, TestIOWithIOHint)
{
    ...
    for (i = 0; i < 5; ++i)
        ASSERT_PASSED(
            write_test_data, image, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, LIBRADOS_OP_FLAG_FADVISE_NOCACHE);

    for (i = 5; i < 10; ++i)
        ASSERT_PASSED(
            aio_write_test_data, image, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, LIBRADOS_OP_FLAG_FADVISE_DONTNEED);
    ...
    for (i = 5; i < 10; ++i)
        ASSERT_PASSED(aio_compare_and_write_test_data,
                      image,
                      test_data,
                      test_data,
                      TEST_IO_SIZE * i,
    ...
    for (i = 0; i < 5; ++i)
        ASSERT_PASSED(
            read_test_data, image, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, LIBRADOS_OP_FLAG_FADVISE_SEQUENTIAL);

    for (i = 5; i < 10; ++i)
        ASSERT_PASSED(aio_read_test_data,
                      image,
                      test_data,
                      TEST_IO_SIZE * i,
                      TEST_IO_SIZE,
    ...
    ASSERT_PASSED(read_test_data, image, test_data, 0, TEST_IO_SIZE, LIBRADOS_OP_FLAG_FADVISE_SEQUENTIAL);
    ASSERT_PASSED(read_test_data,
                  image,
                  skip_discard ? test_data : zero_data,
                  TEST_IO_SIZE,
                  TEST_IO_SIZE,
                  LIBRADOS_OP_FLAG_FADVISE_SEQUENTIAL);
    ASSERT_PASSED(
        read_test_data, image, test_data, TEST_IO_SIZE * 2, TEST_IO_SIZE, LIBRADOS_OP_FLAG_FADVISE_SEQUENTIAL);
    ASSERT_PASSED(read_test_data,
                  image,
                  skip_discard ? test_data : zero_data,
                  TEST_IO_SIZE * 3,
    ...
    ASSERT_EQ(-EINVAL, rbd_read(image, info.size, 1, test_data));
    // reading through end returns amount up to end
    ASSERT_EQ(10, rbd_read2(image, info.size - 10, 100, test_data, LIBRADOS_OP_FLAG_FADVISE_NOCACHE));
    // writing through end returns amount up to end
    ASSERT_EQ(10, rbd_write2(image, info.size - 10, 100, test_data, LIBRADOS_OP_FLAG_FADVISE_DONTNEED));

    rbd_aio_create_completion(NULL, (rbd_callback_t)simple_read_cb, &comp);
    ASSERT_EQ(0, rbd_aio_read2(image, info.size, 1, test_data, comp, LIBRADOS_OP_FLAG_FADVISE_DONTNEED));
    ASSERT_EQ(0, rbd_aio_wait_for_complete(comp));
    ASSERT_EQ(-EINVAL, rbd_aio_get_return_value(comp));
    rbd_aio_release(comp);
    ...
    ASSERT_EQ(
        -EILSEQ,
        rbd_compare_and_write(
            image, 0, TEST_IO_SIZE, mismatch_data, mismatch_data, &mismatch_offset, LIBRADOS_OP_FLAG_FADVISE_DONTNEED));
    ASSERT_EQ(0U, mismatch_offset);
    rbd_aio_create_completion(NULL, (rbd_callback_t)simple_read_cb, &comp);
    mismatch_offset = 123;
    ASSERT_EQ(0,
...
TEST_F(TestLibRBD, TestDataPoolIO)
{
    ...
    ASSERT_EQ(0, rbd_image_options_set_uint64(image_options, RBD_IMAGE_OPTION_FEATURES, features));
    ASSERT_EQ(0, rbd_image_options_set_string(image_options, RBD_IMAGE_OPTION_DATA_POOL, data_pool_name.c_str()));
    ...
    for (i = 5; i < 10; ++i) ASSERT_PASSED(aio_write_test_data, image, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, 0);
...
TEST_F(TestLibRBD, TestCompareAndWriteMismatch)
{
    ...
    ssize_t written = rbd_write(image, off, cmp_buffer.length(), cmp_buffer.data());
    ASSERT_EQ(cmp_buffer.length(), written);

    // Compare should fail because of mismatch
    uint64_t mismatch_off = 0;
    written = rbd_compare_and_write(
        image, off, write_buffer.length(), mismatch_buffer.data(), write_buffer.data(), &mismatch_off, 0);
...
TEST_F(TestLibRBD, TestAioCompareAndWriteMismatch)
{
    ...
    uint64_t mismatch_off = 0;
    int ret = rbd_aio_compare_and_write(
        image, off, write_buffer.length(), mismatch_buffer.data(), write_buffer.data(), comp, &mismatch_off, 0);
    ASSERT_EQ(0, ret);
    ASSERT_EQ(0, rbd_aio_wait_for_complete(comp));
    ASSERT_EQ(-EILSEQ, rbd_aio_get_return_value(comp));
    ASSERT_EQ(5U, mismatch_off);
...
TEST_F(TestLibRBD, TestCompareAndWriteSuccess)
{
    ...
    uint64_t mismatch_off = 0;
    written = rbd_compare_and_write(
        image, off, write_buffer.length(), cmp_buffer.data(), write_buffer.data(), &mismatch_off, 0);
...
TEST_F(TestLibRBD, TestAioCompareAndWriteSuccess)
{
    ...
    uint64_t mismatch_off = 0;
    int ret = rbd_aio_compare_and_write(
        image, off, write_buffer.length(), cmp_buffer.data(), write_buffer.data(), comp, &mismatch_off, 0);
    ASSERT_EQ(0, ret);
    ASSERT_EQ(0, rbd_aio_wait_for_complete(comp));
...
TEST_F(TestLibRBD, TestCompareAndWriteStripeUnitUnaligned)
{
    ...
    ssize_t written = rbd_write(image, stripe_unit, large_cmp_buffer.length(), large_cmp_buffer.data());
    ...
    uint64_t mismatch_off = 0;
    written = rbd_compare_and_write(
        image, stripe_unit + 1, stripe_unit, large_cmp_buffer.data(), large_write_buffer.data(), &mismatch_off, 0);
    ASSERT_EQ(-EINVAL, written);
    ASSERT_EQ(0U, mismatch_off);

    // check nothing has been written
    std::string large_read_buffer(large_cmp_buffer.length(), '5');
    ssize_t read = rbd_read(image, stripe_unit, large_read_buffer.length(), large_read_buffer.data());
    ASSERT_EQ(large_read_buffer.length(), read);
    auto buffer_mismatch = std::mismatch(large_cmp_buffer.begin(), large_cmp_buffer.end(), large_read_buffer.begin());
...
TEST_F(TestLibRBD, TestAioCompareAndWriteStripeUnitUnaligned)
{
    ...
    rbd_aio_create_completion(NULL, NULL, &comp);
    uint64_t mismatch_off = 0;
    int ret = rbd_aio_compare_and_write(image,
                                        stripe_unit + 1,
                                        stripe_unit,
...
TEST_F(TestLibRBD, TestCompareAndWriteTooLarge)
{
    ...
    uint64_t mismatch_off = 0;
    written = rbd_compare_and_write(
        image, stripe_unit, stripe_unit + 1, large_cmp_buffer.data(), large_write_buffer.data(), &mismatch_off, 0);
    ASSERT_EQ(-EINVAL, written);
    ASSERT_EQ(0U, mismatch_off);
...
TEST_F(TestLibRBD, TestCompareAndWriteStripeUnitSuccess)
{
    ...
    uint64_t mismatch_off = 0;
    written = rbd_compare_and_write(
        image, stripe_unit, stripe_unit, large_cmp_buffer.data(), large_write_buffer.data(), &mismatch_off, 0);
    ...
    ssize_t read = rbd_read(image, stripe_unit, large_read_buffer.length(), large_read_buffer.data());
    ASSERT_EQ(large_read_buffer.length(), read);
    auto buffer_mismatch =
        std::mismatch(large_read_buffer.begin(), large_read_buffer.begin() + stripe_unit, large_write_buffer.begin());
    ASSERT_EQ(large_write_buffer.end(), buffer_mismatch.second);
    // check data beyond stripe_unit size was not overwritten
    buffer_mismatch =
        std::mismatch(large_read_buffer.begin() + stripe_unit, large_read_buffer.end(), large_cmp_buffer.begin());
...
TEST_F(TestLibRBD, TestAioCompareAndWriteStripeUnitSuccess)
{
    ...
    uint64_t mismatch_off = 0;
    int ret = rbd_aio_compare_and_write(
        image, stripe_unit, stripe_unit, large_cmp_buffer.data(), large_write_buffer.data(), comp, &mismatch_off, 0);
    ASSERT_EQ(0, ret);
    ASSERT_EQ(0, rbd_aio_wait_for_complete(comp));
    ASSERT_EQ(0, rbd_aio_get_return_value(comp));
    ASSERT_EQ(0U, mismatch_off);
...
TEST_F(TestLibRBD, TestAioCompareAndWriteVIovecLenDiffers)
{
    ...
    std::string write_buffer("Write this !!!");
    struct iovec write_iovs[] = {{.iov_base = &write_buffer[0], .iov_len = 6},
    ...
    rbd_aio_create_completion(NULL, NULL, &comp);
    uint64_t mismatch_off = 0;
    int ret = rbd_aio_compare_and_writev(
        image, off, write_iovs /* cmp_iovs */, 1, write_iovs, std::size(write_iovs), comp, &mismatch_off, 0);
...
TEST_F(TestLibRBD, TestAioCompareAndWriteVMismatch)
{
    ...
    std::string mismatch_buffer("This will fail");
    struct iovec mismatch_iovs[] = {{.iov_base = &mismatch_buffer[0], .iov_len = 5},
    ...
    rbd_aio_create_completion(NULL, NULL, &comp);
    uint64_t mismatch_off = 0;
    int ret = rbd_aio_compare_and_writev(image,
                                         off,
...
TEST_F(TestLibRBD, TestAioCompareAndWriteVSuccess)
{
    ...
    std::string cmp_buffer("This is a test");
    struct iovec cmp_iovs[] = {{.iov_base = &cmp_buffer[0], .iov_len = 5},
    ...
    uint64_t mismatch_off = 0;
    int ret = rbd_aio_compare_and_writev(
        image, off, cmp_iovs, std::size(cmp_iovs), write_iovs, std::size(write_iovs), comp, &mismatch_off, 0);
    ASSERT_EQ(0, ret);
    ASSERT_EQ(0, rbd_aio_wait_for_complete(comp));
    ASSERT_EQ(0, rbd_aio_get_return_value(comp));
    ASSERT_EQ(0U, mismatch_off);
...
TEST_F(TestLibRBD, TestScatterGatherIO)
{
    ...
    struct iovec bad_iovs[] = {{.iov_base = &write_buffer[0], .iov_len = 5},
                               {.iov_base = NULL, .iov_len = std::numeric_limits<size_t>::max()}};
    struct iovec write_iovs[] = {{.iov_base = &write_buffer[0], .iov_len = 5},
    ...
    ASSERT_EQ(0, rbd_aio_writev(image, write_iovs, sizeof(write_iovs) / sizeof(struct iovec), 1 << order, comp));
    ASSERT_EQ(0, rbd_aio_wait_for_complete(comp));
    ASSERT_EQ(0, rbd_aio_get_return_value(comp));
    rbd_aio_release(comp);

    std::string read_buffer(write_buffer.size(), '1');
    struct iovec read_iovs[] = {{.iov_base = &read_buffer[0], .iov_len = 4},
    ...
    ASSERT_EQ(0, rbd_aio_readv(image, read_iovs, sizeof(read_iovs) / sizeof(struct iovec), 1 << order, comp));
    ASSERT_EQ(0, rbd_aio_wait_for_complete(comp));
    ...
    struct iovec linear_iovs[] = {{.iov_base = &linear_buffer[4], .iov_len = 4}};
    rbd_aio_create_completion(NULL, NULL, &comp);
    ASSERT_EQ(0, rbd_aio_readv(image, linear_iovs, sizeof(linear_iovs) / sizeof(struct iovec), 1 << order, comp));
    ASSERT_EQ(0, rbd_aio_wait_for_complete(comp));
...
TEST_F(TestLibRBD, TestFUA)
{
    ...
    for (i = 0; i < 5; ++i)
        ASSERT_PASSED(
            write_test_data, image_write, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, LIBRADOS_OP_FLAG_FADVISE_FUA);

    for (i = 0; i < 5; ++i) ASSERT_PASSED(read_test_data, image_read, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, 0);

    for (i = 5; i < 10; ++i)
        ASSERT_PASSED(
            aio_write_test_data, image_write, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, LIBRADOS_OP_FLAG_FADVISE_FUA);

    for (i = 5; i < 10; ++i)
        ASSERT_PASSED(aio_read_test_data, image_read, test_data, TEST_IO_SIZE * i, TEST_IO_SIZE, 0);
...
void aio_write_test_data(librbd::Image& image, const char* test_data, off_t off, uint32_t iohint, bool* passed)
{
    ceph::bufferlist bl;
...
void aio_read_test_data(librbd::Image& image, const char* expected, off_t off, size_t expected_len, uint32_t iohint,
                        bool* passed)
{
...
void read_test_data(librbd::Image& image, const char* expected, off_t off, size_t expected_len, uint32_t iohint,
                    bool* passed)
{
    int read;
...
void aio_writesame_test_data(librbd::Image& image, const char* test_data, off_t off, size_t len, size_t data_len,
                             uint32_t iohint, bool* passed)
{
    ceph::bufferlist bl;
...
void writesame_test_data(librbd::Image& image, const char* test_data, off_t off, ssize_t len, size_t data_len,
                         uint32_t iohint, bool* passed)
{
    ssize_t written;
    ceph::bufferlist bl;
...
void aio_compare_and_write_test_data(librbd::Image& image, const char* cmp_data, const char* test_data, off_t off,
                                     ssize_t len, uint32_t iohint, bool* passed)
{
...
void compare_and_write_test_data(librbd::Image& image, const char* cmp_data, const char* test_data, off_t off,
                                 ssize_t len, uint64_t* mismatch_off, uint32_t iohint, bool* passed)
{
    size_t written;
    ceph::bufferlist cmp_bl;
    cmp_bl.append(cmp_data, strlen(cmp_data));
...
TEST_F(TestLibRBD, TestIOPP)
{
...

    {
        ...
        for (i = 0; i < 5; ++i) ASSERT_PASSED(write_test_data, image, test_data, strlen(test_data) * i, 0);

        for (i = 5; i < 10; ++i) ASSERT_PASSED(aio_write_test_data, image, test_data, strlen(test_data) * i, 0);
        ...
        for (i = 0; i < 5; ++i) ASSERT_PASSED(read_test_data, image, test_data, strlen(test_data) * i, TEST_IO_SIZE, 0);
...
static void compare_written(librbd::Image& image, off_t off, size_t len, const std::string& buffer, bool* passed)
{
    bufferlist read_bl;
...
TEST_F(TestLibRBD, TestCompareAndWriteCompareTooSmallPP)
{
...

    {
        ...
        uint64_t mismatch_off = 0;
        written = image.compare_and_write(off,
                                          cmp_bl.length(),
                                          small_bl, /* cmp_bl */
                                          write_bl,
                                          &mismatch_off,
...
TEST_F(TestLibRBD, TestAioCompareAndWriteCompareTooSmallPP)
{
...

    {
        ...
        librbd::RBD::AioCompletion* comp = new librbd::RBD::AioCompletion(NULL, (librbd::callback_t)simple_write_cb_pp);
        uint64_t mismatch_off = 0;
        int ret = image.aio_compare_and_write(off,
                                              cmp_bl.length(),
                                              small_bl, /* cmp_bl */
                                              write_bl,
                                              comp,
...
TEST_F(TestLibRBD, TestCompareAndWriteSuccessPP)
{
...

    {
        ...
        // compare against the buffer written before => should succeed
        uint64_t mismatch_off = 0;
        written = image.compare_and_write(off, cmp_bl.length(), cmp_bl, write_bl, &mismatch_off, 0);
...
TEST_F(TestLibRBD, TestAioCompareAndWriteSuccessPP)
{
...

    {
        ...
        librbd::RBD::AioCompletion* comp = new librbd::RBD::AioCompletion(NULL, (librbd::callback_t)simple_write_cb_pp);
        uint64_t mismatch_off = 0;
        int ret = image.aio_compare_and_write(off, write_bl.length(), cmp_bl, write_bl, comp, &mismatch_off, 0);
        ASSERT_EQ(0, ret);
        comp->wait_for_complete();
        ssize_t aio_ret = comp->get_return_value();
        ASSERT_EQ(0, aio_ret);
...
TEST_F(TestLibRBD, TestCompareAndWriteSuccessBufferlistGreaterLenPP)
{
...

    {
        ...
        uint64_t mismatch_off = 0;
        written = image.compare_and_write(off, len_m1, cmp_bl, write_bl, &mismatch_off, 0);
        ASSERT_EQ(len_m1, written);
        ASSERT_EQ(0U, mismatch_off);
...
TEST_F(TestLibRBD, TestAioCompareAndWriteSuccessBufferlistGreaterLenPP)
{
...

    {
        ...
        librbd::RBD::AioCompletion* comp = new librbd::RBD::AioCompletion(NULL, (librbd::callback_t)simple_write_cb_pp);
        uint64_t mismatch_off = 0;
        int ret = image.aio_compare_and_write(off, len_m1, cmp_bl, write_bl, comp, &mismatch_off, 0);
        ASSERT_EQ(0, ret);
        comp->wait_for_complete();
        ssize_t aio_ret = comp->get_return_value();
        ASSERT_EQ(0, aio_ret);
...
TEST_F(TestLibRBD, TestCompareAndWriteStripeUnitUnalignedPP)
{
...

    {
        ...
        large_write_bl.append(large_write_buffer.data(), large_write_buffer.length());
        ...
        ssize_t written = image.write(stripe_unit, large_cmp_bl.length(), large_cmp_bl);
         ...
         */
        uint64_t mismatch_off = 0;
        written = image.compare_and_write(stripe_unit + 1, stripe_unit, large_cmp_bl, large_write_bl, &mismatch_off, 0);
        ASSERT_EQ(-EINVAL, written);
        ASSERT_EQ(0U, mismatch_off);

        // check nothing has been written
        ASSERT_PASSED(compare_written, image, stripe_unit, large_cmp_bl.length(), large_cmp_buffer);
...
TEST_F(TestLibRBD, TestAioCompareAndWriteStripeUnitUnalignedPP)
{
...

    {
        ...
        librbd::RBD::AioCompletion* comp = new librbd::RBD::AioCompletion(NULL, (librbd::callback_t)simple_write_cb_pp);
        uint64_t mismatch_off = 0;
        int ret = image.aio_compare_and_write(
            stripe_unit + 1, stripe_unit, large_cmp_bl, large_write_bl, comp, &mismatch_off, 0);
        ASSERT_EQ(0, ret);
        comp->wait_for_complete();
        ssize_t aio_ret = comp->get_return_value();
...
TEST_F(TestLibRBD, TestCompareAndWriteTooLargePP)
{
...

    {
         ...
         */
        uint64_t mismatch_off = 0;
        written = image.compare_and_write(stripe_unit, stripe_unit + 1, large_cmp_bl, large_write_bl, &mismatch_off, 0);
        ASSERT_EQ(-EINVAL, written);
        ASSERT_EQ(0U, mismatch_off);
...
TEST_F(TestLibRBD, TestAioCompareAndWriteTooLargePP)
{
...

    {
        ...
        uint64_t mismatch_off = 0;
        int ret = image.aio_compare_and_write(
            stripe_unit, stripe_unit + 1, large_cmp_bl, large_write_bl, comp, &mismatch_off, 0);
        ASSERT_EQ(0, ret);
        comp->wait_for_complete();
        ssize_t aio_ret = comp->get_return_value();
        ASSERT_EQ(-EINVAL, aio_ret);
...
TEST_F(TestLibRBD, TestCompareAndWriteStripeUnitSuccessPP)
{
...

    {
        ...
        // aligned stripe unit size access => expect success
        uint64_t mismatch_off = 0;
        written = image.compare_and_write(stripe_unit, stripe_unit, large_cmp_bl, large_write_bl, &mismatch_off, 0);
        ASSERT_EQ(stripe_unit, written);
        ASSERT_EQ(0U, mismatch_off);

        // check large_write_bl was written and nothing beyond
        ASSERT_PASSED(compare_written, image, stripe_unit, stripe_unit, large_write_buffer);
        ASSERT_PASSED(compare_written, image, stripe_unit * 2, stripe_unit, large_cmp_buffer);
...
TEST_F(TestLibRBD, TestAioCompareAndWriteStripeUnitSuccessPP)
{
...

    {
        ...
        uint64_t mismatch_off = 0;
        int ret =
            image.aio_compare_and_write(stripe_unit, stripe_unit, large_cmp_bl, large_write_bl, comp, &mismatch_off, 0);
        ASSERT_EQ(0, ret);
        comp->wait_for_complete();
        ssize_t aio_ret = comp->get_return_value();
        ASSERT_EQ(0, aio_ret);
...
TEST_F(TestLibRBD, TestIOPPWithIOHint)
{
...

    {
        ...
        for (i = 0; i < 5; ++i)
            ASSERT_PASSED(write_test_data, image, test_data, strlen(test_data) * i, LIBRADOS_OP_FLAG_FADVISE_NOCACHE);

        for (i = 5; i < 10; ++i)
            ASSERT_PASSED(
                aio_write_test_data, image, test_data, strlen(test_data) * i, LIBRADOS_OP_FLAG_FADVISE_DONTNEED);

        ASSERT_PASSED(
            read_test_data, image, test_data, strlen(test_data), TEST_IO_SIZE, LIBRADOS_OP_FLAG_FADVISE_RANDOM);
...
TEST_F(TestLibRBD, TestIOToSnapshot)
{
    ...
    for (i = 0; i < TEST_IO_TO_SNAP_SIZE; ++i) test_data[i] = (char)(i + 48);
...
TEST_F(TestLibRBD, TestClone)
{
    ...
    ASSERT_EQ(0, create_image_full(ioctx, parent_name.c_str(), 4 << 20, &order, false, features));
    ...
    EXPECT_NE(0, clone_image(ioctx, parent, parent_name.c_str(), NULL, ioctx, child_name.c_str(), features, &order));
    ...
    ASSERT_EQ(
        -EINVAL,
        clone_image(ioctx, parent, parent_name.c_str(), "parent_snap", ioctx, child_name.c_str(), features, &order));
    ...
    ASSERT_EQ(
        0, clone_image(ioctx, parent, parent_name.c_str(), "parent_snap", ioctx, child_name.c_str(), features, &order));
    ...
    ASSERT_EQ(0, rbd_metadata_list(child, "key", 70, keys, &keys_len, vals, &vals_len));
    ...
    printf("parent info: size %llu obj_size %llu parent_pool %llu\n",
           ...
           (unsigned long long)pinfo.obj_size,
...
TEST_F(TestLibRBD, TestClone2)
{
    ...
    ASSERT_EQ(0, rbd_snap_get_namespace_type(parent, snaps[0].id, &snap_namespace_type));
    ASSERT_EQ(RBD_SNAP_NAMESPACE_TYPE_TRASH, snap_namespace_type);

    char original_name[32];
    ASSERT_EQ(0, rbd_snap_get_trash_namespace(parent, snaps[0].id, original_name, sizeof(original_name)));
...
static void test_list_children(rbd_image_t image, ssize_t num_expected, ...)
{
    ...
    do {
        ...
        num_children = rbd_list_children(image, pools, &pools_len, children, &children_len);
    ...
    for (ssize_t i = num_expected; i > 0; --i) {
        ...
        for (ssize_t j = 0; j < num_children; ++j) {
            printf("checking %s/%s\n", pool, image);
            if (strcmp(expected_pool, pool) == 0 && strcmp(expected_image, image) == 0) {
...
static void test_list_children2(rbd_image_t image, int num_expected, ...)
{
    ...
    for (i = num_expected; i > 0; i--) {
        ...
        for (j = 0; j < num_children; j++) {
            if (children[j].pool_name == NULL || children[j].image_name == NULL || children[j].image_id == NULL)
                continue;
            if (strcmp(children[j].image_id, expected_id) == 0 && strcmp(children[j].pool_name, expected_pool) == 0 &&
                strcmp(children[j].image_name, expected_image) == 0 && children[j].trash == expected_trash) {
...
TEST_F(TestLibRBD, ListChildren)
{
    ...
    // make a parent to clone from
    ASSERT_EQ(0, create_image_full(ioctx1, parent_name.c_str(), 4 << 20, &order, false, features));
    ...
    ASSERT_EQ(
        0,
        clone_image(ioctx1, parent, parent_name.c_str(), "parent_snap", ioctx2, child_name1.c_str(), features, &order));
    ...
    test_list_children2(parent, 1, child_id1, pool_name2.c_str(), child_name1.c_str(), false);

    ASSERT_EQ(
        0,
        clone_image(ioctx1, parent, parent_name.c_str(), "parent_snap", ioctx1, child_name2.c_str(), features, &order));
    ASSERT_EQ(0, rbd_open(ioctx1, child_name2.c_str(), &image2, NULL));
    ASSERT_EQ(0, rbd_get_id(image2, child_id2, sizeof(child_id2)));
    test_list_children(parent, 2, pool_name2.c_str(), child_name1.c_str(), pool_name1.c_str(), child_name2.c_str());
    test_list_children2(parent,
                        2,
                        child_id1,
                        pool_name2.c_str(),
                        child_name1.c_str(),
    ...
    ASSERT_EQ(
        0,
        clone_image(ioctx1, parent, parent_name.c_str(), "parent_snap", ioctx2, child_name3.c_str(), features, &order));
    ASSERT_EQ(0, rbd_open(ioctx2, child_name3.c_str(), &image3, NULL));
    ASSERT_EQ(0, rbd_get_id(image3, child_id3, sizeof(child_id3)));
    test_list_children(parent,
                       3,
                       pool_name2.c_str(),
                       child_name1.c_str(),
                       pool_name1.c_str(),
    ...
    ASSERT_EQ(
        0,
        clone_image(ioctx1, parent, parent_name.c_str(), "parent_snap", ioctx2, child_name4.c_str(), features, &order));
    ...
    test_list_children(parent, 2, pool_name1.c_str(), child_name2.c_str(), pool_name2.c_str(), child_name4.c_str());
    test_list_children2(parent,
                        2,
                        child_id2,
                        pool_name1.c_str(),
    ...
    test_list_children(parent, 1, pool_name1.c_str(), child_name2.c_str());
    test_list_children2(parent, 1, child_id2, pool_name1.c_str(), child_name2.c_str(), false);
...
TEST_F(TestLibRBD, ListChildrenTiered)
{
    ...
    std::string cmdstr = "{\"prefix\": \"osd tier add\", \"pool\": \"" + pool_name1 + "\", \"tierpool\":\"" +
                         pool_name3 + "\", \"force_nonempty\":\"\"}";
    char* cmd[1];
    cmd[0] = (char*)cmdstr.c_str();
    ASSERT_EQ(0, rados_mon_command(_cluster, (const char**)cmd, 1, "", 0, NULL, 0, NULL, 0));

    cmdstr = "{\"prefix\": \"osd tier cache-mode\", \"pool\": \"" + pool_name3 + "\", \"mode\":\"writeback\"}";
    cmd[0] = (char*)cmdstr.c_str();
    ASSERT_EQ(0, rados_mon_command(_cluster, (const char**)cmd, 1, "", 0, NULL, 0, NULL, 0));

    cmdstr = "{\"prefix\": \"osd tier set-overlay\", \"pool\": \"" + pool_name1 + "\", \"overlaypool\":\"" +
             pool_name3 + "\"}";
    cmd[0] = (char*)cmdstr.c_str();
    ASSERT_EQ(0, rados_mon_command(_cluster, (const char**)cmd, 1, "", 0, NULL, 0, NULL, 0));
    ...
    rados_ioctx_destroy(ioctx1);
    rados_ioctx_destroy(ioctx2);
    cmdstr = "{\"prefix\": \"osd tier remove-overlay\", \"pool\": \"" + pool_name1 + "\"}";
    cmd[0] = (char*)cmdstr.c_str();
    ASSERT_EQ(0, rados_mon_command(_cluster, (const char**)cmd, 1, "", 0, NULL, 0, NULL, 0));
    cmdstr = "{\"prefix\": \"osd tier remove\", \"pool\": \"" + pool_name1 + "\", \"tierpool\":\"" + pool_name3 + "\"}";
    cmd[0] = (char*)cmdstr.c_str();
    ASSERT_EQ(0, rados_mon_command(_cluster, (const char**)cmd, 1, "", 0, NULL, 0, NULL, 0));
}
...
TEST_F(TestLibRBD, FlushAio)
{
    ...
    for (i = 0; i < num_aios; ++i) {
        ...
        ASSERT_EQ(0, rbd_aio_write(image, offset, TEST_IO_SIZE, test_data, write_comps[i]));
    }
...
TEST_F(TestLibRBD, FlushAioPP)
{
...

    {
        ...
        for (i = 0; i < num_aios; ++i) {
            ...
            uint64_t offset = rand() % (size - TEST_IO_SIZE);
            ASSERT_EQ(0, image.aio_write(offset, TEST_IO_SIZE, bls[i], write_comps[i]));
        }

        librbd::RBD::AioCompletion* flush_comp = new librbd::RBD::AioCompletion(NULL, NULL);
...
void scribble(librbd::Image& image, int n, int max, bool skip_discard, interval_set<uint64_t>* exists,
...
interval_set<uint64_t> round_diff_interval(const interval_set<uint64_t>& diff, uint64_t object_size)
{
    if (object_size == 0) {
        return diff;
    }
    ...
    for (interval_set<uint64_t>::const_iterator it = diff.begin(); it != diff.end(); ++it) {
...
TEST_F(TestLibRBD, SnapDiff)
{
    ...
    ASSERT_PASSED(write_test_data, image, test_data, 0, TEST_IO_SIZE, LIBRADOS_OP_FLAG_FADVISE_NOCACHE);

    interval_set<uint64_t> diff;
    ASSERT_EQ(0, rbd_diff_iterate2(image, nullptr, 0, size, true, true, iterate_cb, &diff));
    ...
    diff.clear();
    ASSERT_EQ(0, rbd_diff_iterate2(image, "snap1", 0, size, true, true, iterate_cb, &diff));
    EXPECT_EQ(0, diff.size());

    diff.clear();
    ASSERT_EQ(0, rbd_diff_iterate2(image, "snap2", 0, size, true, true, iterate_cb, &diff));
    EXPECT_EQ(0, diff.size());
...
template<typename T> class DiffIterateTest : public TestLibRBD
{
public:
    static const uint8_t whole_object = T::whole_object;
};

template<bool _whole_object> class DiffIterateParams
{
public:
    static const uint8_t whole_object = _whole_object;
};

typedef ::testing::Types<DiffIterateParams<false>, DiffIterateParams<true> > DiffIterateTypes;
...
TYPED_TEST(DiffIterateTest, DiffIterate)
{
...

    {
        ...
        interval_set<uint64_t> diff;
        ASSERT_EQ(0, image.diff_iterate2("one", 0, size, true, this->whole_object, iterate_cb, (void*)&diff));
...
struct diff_extent
{
    diff_extent(uint64_t _offset, uint64_t _length, bool _exists, uint64_t object_size)
        : offset(_offset)
        , length(_length)
        , exists(_exists)
    {
...
TYPED_TEST(DiffIterateTest, DiffIterateDiscard)
{
    ...
    ASSERT_EQ(0, image.diff_iterate2(NULL, 0, size, true, this->whole_object, vector_iterate_cb, (void*)&extents));
    ...
    ASSERT_EQ(0, image.diff_iterate2("snap1", 0, size, true, this->whole_object, vector_iterate_cb, (void*)&extents));
    ASSERT_EQ(1u, extents.size());
    ASSERT_EQ(diff_extent(0, 256, true, object_size), extents[0]);
    ...
    for (int h = 0; h < n - 1; h++) {
        for (int i = 0; i < n - h - 1; i++) {
            for (int j = (h == 0 ? i + 1 : n - 1); j < n; j++) {
                ...
                for (int k = i + 1; k <= j; k++) diff.union_of(wrote[k]);
                cout << "from " << i << " to " << (h != 0 ? string("HEAD") : stringify(j)) << " diff "
                ...
                ASSERT_EQ(0,
                          image.diff_iterate2(
                              snap[i].c_str(), 0, size, true, this->whole_object, iterate_cb, (void*)&actual));
...
TYPED_TEST(DiffIterateTest, DiffIterateParent)
{
...

    {
        ...
        ASSERT_EQ(0, rbd.clone(ioctx, name.c_str(), "snap", ioctx, clone_name.c_str(), features, &order));
        librbd::Image clone;
        ASSERT_EQ(0, rbd.open(ioctx, clone, clone_name.c_str(), NULL));

        std::vector<diff_extent> extents;
        ASSERT_EQ(0, clone.diff_iterate2(NULL, 0, size, true, this->whole_object, vector_iterate_cb, &extents));
...
TYPED_TEST(DiffIterateTest, DiffIterateIgnoreParent)
{
    ...
    std::string clone_name = this->get_temp_image_name();
    ASSERT_EQ(0, rbd.clone(ioctx, name.c_str(), "one", ioctx, clone_name.c_str(), features, &order));
    ...
    interval_set<uint64_t> diff;
    ASSERT_EQ(0, image.diff_iterate2(NULL, 0, size, false, this->whole_object, iterate_cb, (void*)&diff));
    cout << " diff was " << diff << std::endl;
    if (!this->whole_object) {
...
TYPED_TEST(DiffIterateTest, DiffIterateCallbackError)
{
...

    {
        ...
        interval_set<uint64_t> diff;
        ASSERT_EQ(-EINVAL, image.diff_iterate2(NULL, 0, size, true, this->whole_object, iterate_error_cb, NULL));
    }
    ioctx.close();
}
...
TYPED_TEST(DiffIterateTest, DiffIterateParentDiscard)
{
    ...
    std::string clone_name = this->get_temp_image_name();
    ASSERT_EQ(0, rbd.clone(ioctx, name.c_str(), "two", ioctx, clone_name.c_str(), features, &order));
    ASSERT_EQ(0, rbd.open(ioctx, image, clone_name.c_str(), NULL));
    ...
    interval_set<uint64_t> diff;
    ASSERT_EQ(0, image.diff_iterate2(NULL, 0, size, true, this->whole_object, iterate_cb, (void*)&diff));
    ASSERT_TRUE(two.subset_of(diff));
}
...
TYPED_TEST(DiffIterateTest, DiffIterateUnalignedSmall)
{
...

    {
        ...
        std::vector<diff_extent> extents;
        ASSERT_EQ(0, image.diff_iterate2(NULL, 5000005, 1234, true, this->whole_object, vector_iterate_cb, &extents));
        ASSERT_EQ(1u, extents.size());
...
TYPED_TEST(DiffIterateTest, DiffIterateUnaligned)
{
...

    {
        ...
        std::vector<diff_extent> extents;
        ASSERT_EQ(0,
                  image.diff_iterate2(NULL, 8376263, 4260970, true, this->whole_object, vector_iterate_cb, &extents));
...
TYPED_TEST(DiffIterateTest, DiffIterateStriping)
{
...

    {
        ...
        ASSERT_EQ(0, rbd.create3(ioctx, name.c_str(), size, features, &order, 1 << 20, 3));
        ASSERT_EQ(0, rbd.open(ioctx, image, name.c_str(), NULL));
        ...
        std::vector<diff_extent> extents;
        ASSERT_EQ(0, image.diff_iterate2(NULL, 0, size, true, this->whole_object, vector_iterate_cb, &extents));
        ...
        ASSERT_EQ(0, image.diff_iterate2("one", 0, size, true, this->whole_object, vector_iterate_cb, &extents));
        ASSERT_EQ(2u, extents.size());
...
TEST_F(TestLibRBD, LargeCacheRead)
{
    ...
    ASSERT_EQ(0, _rados.conf_set("rbd_cache_size", stringify(new_cache_size).c_str()));
    ...
    ASSERT_EQ(static_cast<ssize_t>(buffer.size()), rbd_write(image, 0, buffer.size(), buffer.c_str()));

    ASSERT_EQ(0, rbd_invalidate_cache(image));

    ASSERT_EQ(static_cast<ssize_t>(buffer.size()), rbd_read(image, 0, buffer.size(), &buffer[0]));
...
TEST_F(TestLibRBD, TestPendingAio)
{
    ...
    uint64_t size = 4 << 20;
    ASSERT_EQ(0, create_image_full(ioctx, name.c_str(), size, &order, false, features));
    ASSERT_EQ(0, rbd_open(ioctx, name.c_str(), &image, NULL));
    ...
    for (size_t i = 0; i < num_aios; ++i) {
        ...
        uint64_t offset = rand() % (size - TEST_IO_SIZE);
        ASSERT_EQ(0, rbd_aio_write(image, offset, TEST_IO_SIZE, test_data, comps[i]));
    }
    for (size_t i = 0; i < num_aios; ++i) {
    ...
    for (size_t i = 0; i < num_aios; ++i) {
        ASSERT_EQ(0, rbd_aio_create_completion(NULL, NULL, &comps[i]));
        uint64_t offset = rand() % (size - TEST_IO_SIZE);
        ASSERT_LE(0, rbd_aio_read(image, offset, TEST_IO_SIZE, test_data, comps[i]));
    }
...
void compare_and_write_copyup(librados::IoCtx& ioctx, bool deep_copyup, bool* passed)
{
    librbd::RBD rbd;
    ...
    EXPECT_EQ(0, rbd.clone(ioctx, parent_name.c_str(), "snap1", ioctx, clone_name.c_str(), features, &order));
    ...
    ASSERT_EQ((ssize_t)write_bl.length(),
              clone_image.compare_and_write(512, write_bl.length(), cmp_bl, write_bl, &mismatch_off, 0));
    ASSERT_EQ(0U, mismatch_off);
    bufferlist read_bl;
...
void compare_and_write_copyup_mismatch(librados::IoCtx& ioctx, bool deep_copyup, bool* passed)
{
    librbd::RBD rbd;
    std::string parent_name = TestLibRBD::get_temp_image_name();
    uint64_t size = 2 << 20;
    ...
    write_bl.append(std::string(512, '2'));
    uint64_t mismatch_off = 0;
    ASSERT_EQ(-EILSEQ, clone_image.compare_and_write(512, write_bl.length(), cmp_bl, write_bl, &mismatch_off, 0));
...
TEST_F(TestLibRBD, Flatten)
{
    ...
    librbd::RBD::AioCompletion* read_comp = new librbd::RBD::AioCompletion(NULL, NULL);
    bufferlist read_bl;
...
TEST_F(TestLibRBD, SnapRemove2)
{
    ...
    std::string child_name = get_temp_image_name();
    EXPECT_EQ(0, rbd.clone(ioctx, name.c_str(), "snap1", ioctx, child_name.c_str(), features, &order));
...
TEST_F(TestLibRBD, FlattenViaLockOwner)
{
    ...
    std::string name = get_temp_image_name();
    EXPECT_EQ(0, rbd.clone(ioctx, parent_name.c_str(), "snap1", ioctx, name.c_str(), features, &order));
...
TEST_F(TestLibRBD, Metadata)
{
    ...
    ASSERT_EQ(-ERANGE, rbd_metadata_list(image1, "key", 0, keys, &keys_len, vals, &vals_len));
    keys_len = sizeof(keys);
    vals_len = sizeof(vals);
    memset_rand(keys, keys_len);
    memset_rand(vals, vals_len);
    ...
    memset_rand(vals, vals_len);
    ASSERT_EQ(0, rbd_metadata_list(image1, "key", 0, keys, &keys_len, vals, &vals_len));
    ASSERT_EQ(keys_len, strlen("key1") + 1 + strlen("key2") + 1 + strlen("key3") + 1);
    ASSERT_EQ(vals_len, strlen("value1") + 1 + strlen("value2") + 1 + strlen("value3") + 1);
    ASSERT_STREQ(keys, "key1");
    ...
    EXPECT_EQ(0, rbd_clone(ioctx, name.c_str(), "snap1", ioctx, cname.c_str(), features, &order));
    rbd_image_t image2;
    ...
    ASSERT_EQ(keys_len, strlen("key1") + 1 + strlen("key2") + 1 + strlen("key3") + 1 + strlen("key4") + 1);
    ASSERT_EQ(vals_len, strlen("value1") + 1 + strlen("value2") + 1 + strlen("value3") + 1 + strlen("value4") + 1);
    ASSERT_STREQ(keys + strlen("key1") + 1 + strlen("key2") + 1 + strlen("key3") + 1, "key4");
    ASSERT_STREQ(vals + strlen("value1") + 1 + strlen("value2") + 1 + strlen("value3") + 1, "value4");
    ...
    ASSERT_EQ(-ERANGE, rbd_metadata_list(image2, "key", 2, keys, &keys_len, vals, &vals_len));
    ASSERT_EQ(keys_len, strlen("key1") + 1 + strlen("key2") + 1);
    ASSERT_EQ(vals_len, strlen("value1") + 1 + strlen("value2") + 1);

    ASSERT_EQ(-ERANGE, rbd_metadata_list(image2, "key", 0, keys, &keys_len, vals, &vals_len));
    ASSERT_EQ(keys_len, strlen("key1") + 1 + strlen("key2") + 1 + strlen("key3") + 1 + strlen("key4") + 1);
    ASSERT_EQ(vals_len, strlen("value1") + 1 + strlen("value2") + 1 + strlen("value3") + 1 + strlen("value4") + 1);
...
TEST_F(TestLibRBD, MetadataPP)
{
    ...
    librbd::Image image2;
    ASSERT_EQ(0, image1.features(&features));
    EXPECT_EQ(0, rbd.clone(ioctx, name.c_str(), "snap1", ioctx, cname.c_str(), features, &order));
...
TEST_F(TestLibRBD, UpdateFeatures)
{
    ...
    disable_features = features & (RBD_FEATURE_EXCLUSIVE_LOCK | RBD_FEATURE_OBJECT_MAP | RBD_FEATURE_FAST_DIFF |
    ...
    uint64_t expected_flags = RBD_FLAG_OBJECT_MAP_INVALID | RBD_FLAG_FAST_DIFF_INVALID;
    uint64_t flags;
    ASSERT_EQ(0, image.get_flags(&flags));
...
TEST_F(TestLibRBD, BlockingAIO)
{
    ...
    {
        ASSERT_EQ(0, _rados.conf_set("rbd_non_blocking_aio", non_blocking_aio.c_str()));
    }
    BOOST_SCOPE_EXIT_END;
    ...
    librbd::RBD::AioCompletion* discard_comp = new librbd::RBD::AioCompletion(NULL, NULL);
...
TEST_F(TestLibRBD, ExclusiveLockTransition)
{
    ...
    for (size_t object_no = 0; object_no < (size >> 12); ++object_no) {
        librbd::RBD::AioCompletion* comp = new librbd::RBD::AioCompletion(NULL, NULL);
    ...
    for (size_t object_no = 0; object_no < (size >> 12); ++object_no) {
        bufferlist read_bl;
        ASSERT_EQ((ssize_t)bl.length(), image3.read(object_no << order, bl.length(), read_bl));
        ASSERT_TRUE(bl.contents_equal(read_bl));
    }
...
TEST_F(TestLibRBD, CacheMayCopyOnWrite)
{
    ...
    std::string clone_name = this->get_temp_image_name();
    ASSERT_EQ(0, rbd.clone(ioctx, name.c_str(), "one", ioctx, clone_name.c_str(), RBD_FEATURE_LAYERING, &order));
...
TEST_F(TestLibRBD, TestImageOptions)
{
    ...
    ASSERT_EQ(0, rbd_image_options_is_set(opts, RBD_IMAGE_OPTION_FORMAT, &is_set));
    ASSERT_FALSE(is_set);

    ASSERT_EQ(0, rbd_image_options_set_uint64(opts, RBD_IMAGE_OPTION_FORMAT, 2));
    ASSERT_EQ(0, rbd_image_options_set_uint64(opts, RBD_IMAGE_OPTION_FEATURES, features));
    ASSERT_EQ(0, rbd_image_options_set_uint64(opts, RBD_IMAGE_OPTION_ORDER, order));
    ASSERT_EQ(0, rbd_image_options_set_uint64(opts, RBD_IMAGE_OPTION_STRIPE_UNIT, stripe_unit));
    ASSERT_EQ(0, rbd_image_options_set_uint64(opts, RBD_IMAGE_OPTION_STRIPE_COUNT, stripe_count));
    ...
    ASSERT_EQ(0, rbd_image_options_get_uint64(opts, RBD_IMAGE_OPTION_ORDER, &order));
    ...
    std::string child_name = get_temp_image_name();
    ASSERT_EQ(0, rbd_snap_protect(parent, "parent_snap"));
    ASSERT_EQ(0, rbd_clone3(ioctx, parent_name.c_str(), "parent_snap", ioctx, child_name.c_str(), opts));
    ...
    ASSERT_EQ(0, rbd_copy_with_progress3(parent, ioctx, copy2_name.c_str(), opts, print_progress_percent, NULL));
...
TEST_F(TestLibRBD, TestImageOptionsPP)
{
    ...
    ASSERT_EQ(0, rbd.clone3(ioctx, parent_name.c_str(), "parent_snap", ioctx, child_name.c_str(), opts));
...
TEST_F(TestLibRBD, ImagePollIO)
{
    ...
    for (i = 0; i < TEST_IO_SIZE; ++i) test_data[i] = (char)(rand() % (126 - 33) + 33);
    test_data[TEST_IO_SIZE] = '\0';
    memset(zero_data, 0, sizeof(zero_data));
...
static bool operator==(const linked_image_spec_t& lhs, const linked_image_spec_t& rhs)
{
    return (lhs.pool_id == rhs.pool_id && lhs.pool_name == rhs.pool_name && lhs.pool_namespace == rhs.pool_namespace &&
            lhs.image_id == rhs.image_id && lhs.image_name == rhs.image_name && lhs.trash == rhs.trash);
}

static bool operator==(const mirror_peer_t& lhs, const mirror_peer_t& rhs)
{
    return (lhs.uuid == rhs.uuid && lhs.cluster_name == rhs.cluster_name && lhs.client_name == rhs.client_name);
}
...
TEST_F(TestLibRBD, Mirror)
{
    ...
    std::string parent_name = get_temp_image_name();
    std::string child_name = get_temp_image_name();
    ASSERT_EQ(0, create_image_pp(rbd, ioctx, parent_name.c_str(), 2 << 20, &order));
    bool old_format;
    uint64_t features;
    ASSERT_EQ(0, get_features(&old_format, &features));
    if ((features & RBD_FEATURE_LAYERING) != 0) {
        ...
        ASSERT_EQ(0, parent.snap_protect("parent_snap"));
        ASSERT_EQ(0, parent.close());
        ASSERT_EQ(0, rbd.clone(ioctx, parent_name.c_str(), "parent_snap", ioctx, child_name.c_str(), features, &order));
    }
    ...
    ASSERT_EQ(0, rbd.mirror_peer_list(ioctx, &peers));
    auto sort_peers = [](const librbd::mirror_peer_t& lhs, const librbd::mirror_peer_t& rhs) {
        return lhs.uuid < rhs.uuid;
    };
    expected_peers = {{uuid1, "cluster1", "client"}, {uuid2, "cluster2", "admin"}, {uuid3, "cluster3", "admin"}};
    ...
    ASSERT_EQ(-ENOENT, rbd.mirror_peer_set_cluster(ioctx, "uuid4", "new cluster"));
    ASSERT_EQ(0, rbd.mirror_peer_set_cluster(ioctx, uuid3, "new cluster"));

    ASSERT_EQ(0, rbd.mirror_peer_list(ioctx, &peers));
    expected_peers = {{uuid1, "cluster1", "new client"}, {uuid3, "new cluster", "admin"}};
    std::sort(expected_peers.begin(), expected_peers.end(), sort_peers);
    ASSERT_EQ(expected_peers, peers);
...
TEST_F(TestLibRBD, MirrorPeerAttributes)
{
    ...
    std::map<std::string, std::string> expected_attributes{{"mon_host", "1.2.3.4"}, {"key", "ABC"}};
    ASSERT_EQ(0, rbd.mirror_peer_set_attributes(ioctx, uuid, expected_attributes));

    ASSERT_EQ(0, rbd.mirror_peer_get_attributes(ioctx, uuid, &attributes));
...
TEST_F(TestLibRBD, CreateWithMirrorEnabled)
{
    ...
    ASSERT_EQ(
        0,
        image_options.set(RBD_IMAGE_OPTION_MIRROR_IMAGE_MODE, static_cast<uint64_t>(RBD_MIRROR_IMAGE_MODE_SNAPSHOT)));
    ...
    std::string child_name = get_temp_image_name();
    ASSERT_EQ(0, rbd.clone3(ioctx, parent_name.c_str(), "parent_snap", ioctx, child_name.c_str(), image_options));
...
TEST_F(TestLibRBD, ExclusiveLock)
{
    ...
    ASSERT_EQ(-ERANGE, rbd_lock_get_owners(image1, &lock_mode, lock_owners, &max_lock_owners));
    ASSERT_EQ(1U, max_lock_owners);

    ASSERT_EQ(0, rbd_lock_get_owners(image1, &lock_mode, lock_owners, &max_lock_owners));
    ...
    ASSERT_EQ(-EBUSY, rbd_lock_break(image1, RBD_LOCK_MODE_EXCLUSIVE, "not the owner"));

    ASSERT_EQ(0, rbd_lock_release(image1));
    ASSERT_EQ(0, rbd_is_exclusive_lock_owner(image1, &lock_owner));
    ASSERT_FALSE(lock_owner);

    ASSERT_EQ(-ENOENT, rbd_lock_break(image1, RBD_LOCK_MODE_EXCLUSIVE, lock_owners[0]));
...
TEST_F(TestLibRBD, BreakLock)
{
    ...
    ASSERT_EQ(0, rados_ioctx_create(_cluster, m_pool_name.c_str(), &ioctx));
    ASSERT_EQ(0, rados_ioctx_create(blocklist_cluster, m_pool_name.c_str(), &blocklist_ioctx));
    ...
    ASSERT_EQ(0, rbd_lock_get_owners(image, &lock_mode, lock_owners, &max_lock_owners));
    ASSERT_EQ(RBD_LOCK_MODE_EXCLUSIVE, lock_mode);
    ASSERT_STRNE("", lock_owners[0]);
    ASSERT_EQ(1U, max_lock_owners);
...
TEST_F(TestLibRBD, DefaultFeatures)
{
    ...
    {
        ASSERT_EQ(0, _rados.conf_set("rbd_default_features", orig_default_features.c_str()));
    };

    std::list<std::pair<std::string, std::string> > feature_names_to_bitmask = {{"", orig_default_features},
                                                                                {"layering", "1"},
...
TEST_F(TestLibRBD, TestTrashMoveAndPurge)
{
    ...
    entries.clear();
    PrintProgress pp;
    ASSERT_EQ(0, rbd.trash_remove_with_progress(ioctx, image_id.c_str(), false, pp));
    ASSERT_EQ(0, rbd.trash_list(ioctx, entries));
...
TEST_F(TestLibRBD, TestTrashMoveAndPurgeNonExpiredDelay)
{
    ...
    PrintProgress pp;
    ASSERT_EQ(-EPERM, rbd.trash_remove_with_progress(ioctx, image_id.c_str(), false, pp));

    PrintProgress pp2;
    ASSERT_EQ(0, rbd.trash_remove_with_progress(ioctx, image_id.c_str(), true, pp2));
}
...
TEST_F(TestLibRBD, NamespacesPP)
{
    ...
    if (!get_features(&features)) {
        ...
        ASSERT_EQ(-EINVAL, create_image_pp(rbd, ns_io_ctx, name.c_str(), 0, &order));
        return;
    }
    ...
    PrintProgress pp;
    ASSERT_EQ(-ENOENT, rbd.trash_remove_with_progress(ioctx, image_id.c_str(), false, pp));
    ASSERT_EQ(0, rbd.trash_remove_with_progress(ns_io_ctx, image_id.c_str(), false, pp));
...
TEST_F(TestLibRBD, Migration)
{
    ...
    ASSERT_EQ(0, rbd_migration_prepare(ioctx, name.c_str(), ioctx, name.c_str(), image_options));

    rbd_image_migration_status_t status;
    ASSERT_EQ(0, rbd_migration_status(ioctx, name.c_str(), &status, sizeof(status)));
    ...
    ASSERT_EQ(0, rbd_get_migration_source_spec(image, source_spec, &source_spec_length));
    ...
    ASSERT_EQ(0, rbd_migration_prepare(ioctx, name.c_str(), ioctx, new_name.c_str(), image_options));
...
TEST_F(TestLibRBD, MigrationPP)
{
    ...
    ASSERT_EQ(0, rbd.migration_prepare(ioctx, name.c_str(), ioctx, name.c_str(), image_options));

    librbd::image_migration_status_t status;
    ASSERT_EQ(0, rbd.migration_status(ioctx, name.c_str(), &status, sizeof(status)));
    ...
    ASSERT_EQ(0, rbd.migration_prepare(ioctx, name.c_str(), ioctx, new_name.c_str(), image_options));
...
TEST_F(TestLibRBD, PoolMetadata)
{
    ...
    ASSERT_EQ(0, rbd_pool_metadata_list(ioctx, "", 0, keys, &keys_len, vals, &vals_len));
    ASSERT_EQ(0U, keys_len);
    ASSERT_EQ(0U, vals_len);
    ...
    ASSERT_EQ(-ERANGE, rbd_pool_metadata_list(ioctx, "", 0, keys, &keys_len, vals, &vals_len));
    keys_len = sizeof(keys);
    vals_len = sizeof(vals);
    memset_rand(keys, keys_len);
    memset_rand(vals, vals_len);
    ...
    memset_rand(keys, keys_len);
    memset_rand(vals, vals_len);
    ASSERT_EQ(0, rbd_pool_metadata_list(ioctx, "", 1, keys, &keys_len, vals, &vals_len));
    ASSERT_EQ(keys_len, strlen("key1") + 1);
    ASSERT_EQ(vals_len, strlen("value1") + 1);
    ASSERT_STREQ(keys, "key1");
    ASSERT_STREQ(vals, "value1");

    ASSERT_EQ(-ERANGE, rbd_pool_metadata_list(ioctx, "", 2, keys, &keys_len, vals, &vals_len));
    ASSERT_EQ(keys_len, strlen("key1") + 1 + strlen("key2") + 1);
    ASSERT_EQ(vals_len, strlen("value1") + 1 + strlen("value2") + 1);
    ...
    memset_rand(keys, keys_len);
    memset_rand(vals, vals_len);
    ASSERT_EQ(0, rbd_pool_metadata_list(ioctx, "key2", 0, keys, &keys_len, vals, &vals_len));
    ASSERT_EQ(keys_len, strlen("key3") + 1 + strlen("key4") + 1);
    ASSERT_EQ(vals_len, strlen("value3") + 1 + strlen("value4") + 1);
    ASSERT_STREQ(keys, "key3");
    ASSERT_STREQ(vals, "value3");
...
TEST_F(TestLibRBD, PoolStatsPP)
{
    ...
    pool_stats1.add(RBD_POOL_STAT_OPTION_IMAGE_PROVISIONED_BYTES, &provisioned_bytes);
    ASSERT_EQ(0, rbd.pool_stats_get(ioctx, &pool_stats1));

    ASSERT_EQ(4U, image_count);
    ASSERT_EQ(expect_head_size, provisioned_bytes);

    pool_stats1.add(RBD_POOL_STAT_OPTION_IMAGE_MAX_PROVISIONED_BYTES, &max_provisioned_bytes);
    ASSERT_EQ(0, rbd.pool_stats_get(ioctx, &pool_stats1));
    ASSERT_EQ(4U, image_count);
    ASSERT_EQ(expect_head_size, provisioned_bytes);
    ...
    pool_stats3.add(RBD_POOL_STAT_OPTION_TRASH_PROVISIONED_BYTES, &trash_provisioned_bytes);
    pool_stats3.add(RBD_POOL_STAT_OPTION_TRASH_MAX_PROVISIONED_BYTES, &trash_max_provisioned_bytes);
...
TEST_F(TestLibRBD, ImageSpec)
{
    ...
    std::vector<librbd::image_spec_t> expected_images{{.id = parent_id, .name = name},
                                                      {.id = clone_id, .name = clone_name}};
    std::sort(expected_images.begin(),
              expected_images.end(),
              [](const librbd::image_spec_t& lhs, const librbd::image_spec_t& rhs) { return lhs.name < rhs.name; });
    ...
    librbd::linked_image_spec_t expected_parent_image_spec{.pool_id = ioctx.get_id(),
    ...
    std::vector<librbd::linked_image_spec_t> expected_children{{.pool_id = ioctx.get_id(),
                                                                .pool_name = ioctx.get_pool_name(),
                                                                .pool_namespace = ioctx.get_namespace(),
                                                                .image_id = clone_id,
    ...
    ASSERT_EQ(0, rbd.clone(ioctx, clone_name.c_str(), "snap", ioctx, grand_clone_name.c_str(), features, &order));
    librbd::Image grand_clone_image;
    ASSERT_EQ(0, rbd.open(ioctx, grand_clone_image, grand_clone_name.c_str(), nullptr));
    ...
    children.clear();
    ASSERT_EQ(0, parent_image.list_descendants(&children));
    expected_children.push_back({.pool_id = ioctx.get_id(),
                                 .pool_name = ioctx.get_pool_name(),
                                 .pool_namespace = ioctx.get_namespace(),
...
TEST_F(TestLibRBD, DISABLED_TestSeqWriteAIOPP)
{
...

    {
        ...
        for (uint64_t i = 0; i < size / TEST_IO_SIZE; ++i) {
            ...
            auto comp = new librbd::RBD::AioCompletion(NULL, (librbd::callback_t)super_simple_write_cb_pp);
        ...
        int duration = end_time.tv_sec * 1000 + end_time.tv_nsec / 1000000 - start_time.tv_sec * 1000 -
...
TEST_F(TestLibRBD, SnapRemoveWithChildMissing)
{
    ...
    ASSERT_EQ(0,
              clone_image(ioctx1, parent, parent_name.c_str(), "snap1", ioctx2, child_name1.c_str(), features, &order));
    ASSERT_EQ(0,
              clone_image(ioctx1, parent, parent_name.c_str(), "snap2", ioctx1, child_name2.c_str(), features, &order));
    ASSERT_EQ(0,
              clone_image(ioctx1, parent, parent_name.c_str(), "snap2", ioctx2, child_name3.c_str(), features, &order));
    ...
    _pool_names.erase(std::remove(_pool_names.begin(), _pool_names.end(), m_pool_name), _pool_names.end());
    EXPECT_EQ(0, rados_wait_for_latest_osdmap(_cluster));
...
TEST_F(TestLibRBD, QuiesceWatch)
{
    ...
    struct Watcher
    {
        ...
        {
            std::unique_lock locker(lock);
            return cv.wait_for(locker, seconds(60), [this, c]() { return unquiesce_count >= c; });
        }
    } watcher1(image1), watcher2(image2);

    ASSERT_EQ(0, rbd_quiesce_watch(image1, Watcher::quiesce_cb, Watcher::unquiesce_cb, &watcher1, &watcher1.handle));
    ASSERT_EQ(0, rbd_quiesce_watch(image2, Watcher::quiesce_cb, Watcher::unquiesce_cb, &watcher2, &watcher2.handle));
...
TEST_F(TestLibRBD, QuiesceWatchError)
{
...

    {
        ...
        struct Watcher : public librbd::QuiesceWatchCtx
        {
            ...
            Watcher(librbd::Image& image, int r)
                : image(image)
                , r(r)
            {}
            ...
            {
                std::unique_lock locker(lock);
                return cv.wait_for(locker, seconds(60), [this]() { return quiesce_count == unquiesce_count; });
            }
        ...
        ASSERT_EQ(0, image2.snap_create2("snap2", RBD_SNAP_CREATE_IGNORE_QUIESCE_ERROR, prog_ctx));
        ASSERT_GT(watcher10.quiesce_count, 0U);
        ASSERT_EQ(watcher10.unquiesce_count, 0U);
        ASSERT_GT(watcher11.quiesce_count, 0U);
        ASSERT_TRUE(watcher11.wait_for_unquiesce());
        ...
        watcher20.reset_counters();
        ASSERT_EQ(0, image2.snap_create2("snap4", RBD_SNAP_CREATE_SKIP_QUIESCE, prog_ctx));
...
TEST_F(TestLibRBD, QuiesceWatchTimeout)
{
...

    {
        ...
        struct Watcher : public librbd::QuiesceWatchCtx
        {
            ...
            {
                std::unique_lock<std::mutex> locker(m_lock);
                ASSERT_TRUE(m_cond.wait_for(locker, seconds(60), [this] { return quiesce_count >= 1; }));
            }

            void wait_for_unquiesce()
            {
                std::unique_lock<std::mutex> locker(m_lock);
                ASSERT_TRUE(m_cond.wait_for(locker, seconds(60), [this] { return quiesce_count == unquiesce_count; }));
...
TEST_F(TestLibRBD, WriteZeroes)
{
    ...
    interval_set<uint64_t> diff;
    ASSERT_EQ(0, image.diff_iterate2(nullptr, 0, size, false, false, iterate_cb, (void*)&diff));
    ...
    ASSERT_EQ(size - 192, image.write_zeroes(192, size - 192, 0U, 0));
    diff.clear();
    ASSERT_EQ(0, image.diff_iterate2(nullptr, 0, size, false, false, iterate_cb, (void*)&diff));
...
TEST_F(TestLibRBD, WriteZeroesThickProvision)
{
    ...
    ASSERT_EQ(128, image.write_zeroes(0, 128, RBD_WRITE_ZEROES_FLAG_THICK_PROVISION, 0));
    diff.clear();
    ASSERT_EQ(0, image.diff_iterate2(nullptr, 0, size, false, false, iterate_cb, (void*)&diff));
    expected_diff = interval_set<uint64_t>{{{0, 128}}};
    ASSERT_EQ(expected_diff, diff);

    ASSERT_EQ(512, image.write_zeroes(384, 512, RBD_WRITE_ZEROES_FLAG_THICK_PROVISION, 0));
    diff.clear();
    ASSERT_EQ(0, image.diff_iterate2(nullptr, 0, size, false, false, iterate_cb, (void*)&diff));
    ...
    ASSERT_EQ(640, image.write_zeroes(896, 640, RBD_WRITE_ZEROES_FLAG_THICK_PROVISION, 0));
    diff.clear();
    ASSERT_EQ(0, image.diff_iterate2(nullptr, 0, size, false, false, iterate_cb, (void*)&diff));
    ...
    ASSERT_EQ(640, image.write_zeroes(1536, 640, RBD_WRITE_ZEROES_FLAG_THICK_PROVISION, 0));
    diff.clear();
    ASSERT_EQ(0, image.diff_iterate2(nullptr, 0, size, false, false, iterate_cb, (void*)&diff));
    ...
    ASSERT_EQ(768, image.write_zeroes(2176, 768, RBD_WRITE_ZEROES_FLAG_THICK_PROVISION, 0));
    diff.clear();
    ASSERT_EQ(0, image.diff_iterate2(nullptr, 0, size, false, false, iterate_cb, (void*)&diff));
    expected_diff = interval_set<uint64_t>{{{0, 2944}}};

    // write-same
    ASSERT_EQ(1024, image.write_zeroes(3072, 1024, RBD_WRITE_ZEROES_FLAG_THICK_PROVISION, 0));
    diff.clear();
    ASSERT_EQ(0, image.diff_iterate2(nullptr, 0, size, false, false, iterate_cb, (void*)&diff));
...
TEST_F(TestLibRBD, ConcurentOperations)
{
    ...
    // Test shutdown
    {
        ...
        struct Watcher : public librbd::QuiesceWatchCtx
        {
            ...
            {
                std::unique_lock locker(lock);
                return cv.wait_for(locker, seconds(60), [this, c]() { return count >= c; });
            }
        } watcher;
        uint64_t handle;
