...
void prt(const char* fmt, ...)
{
    ...
    if (fsxlogf) fprintf(fsxlogf, "%s", buffer);
}

void prterr(const char* prefix)
{
    prt("%s%s%s\n", prefix, prefix ? ": " : "", strerror(errno));
}

void prterrcode(const char* prefix, int code)
{
...
struct ReplayHandler : public journal::ReplayHandler
{
    ...
    ReplayHandler(journal::Journaler* journaler, journal::Journaler* replay_journaler, Context* on_finish)
        : journaler(journaler)
    ...
    void handle_complete(int r) override { on_finish->complete(r); }
};

int get_image_id(librados::IoCtx& io_ctx, const char* image_name, std::string* image_id)
{
    librbd::RBD rbd;
    librbd::Image image;
...
int register_journal(rados_ioctx_t ioctx, const char* image_name)
{
    ...
    journal::Journaler journaler(io_ctx, image_id, JOURNAL_CLIENT_ID, {}, nullptr);
...
int create_replay_image(rados_ioctx_t ioctx, int order, uint64_t stripe_unit, int stripe_count,
                        const char* replay_image_name, const char* last_replay_image_name)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(ioctx, io_ctx);
...
int replay_journal(rados_ioctx_t ioctx, const char* image_name, const char* replay_image_name)
{
    librados::IoCtx io_ctx;
    librados::IoCtx::from_rados_ioctx_t(ioctx, io_ctx);
    ...
    journal::Journaler replay_journaler(io_ctx, replay_image_id, "", {}, nullptr);
    ...
    ReplayHandler replay_handler(&journaler, &replay_journaler, &replay_ctx);
...
int finalize_journal(rados_ioctx_t ioctx, const char* imagename, int clones, int order, uint64_t stripe_unit,
                     int stripe_count)
{
    ...
    if (clones > 0) {
        replay_imagename(lastreplayimagename, sizeof(lastreplayimagename), clones - 1);
    }

    int ret = create_replay_image(
        ioctx, order, stripe_unit, stripe_count, replayimagename, clones > 0 ? lastreplayimagename : nullptr);
    if (ret < 0) {
        exit(EXIT_FAILURE);
    }
...
struct rbd_operations
{
    ...
    int (*clone)(struct rbd_ctx* ctx, const char* src_snapname, const char* dst_imagename, int* order, int stripe_unit,
                 int stripe_count);
    int (*flatten)(struct rbd_ctx* ctx);
    ssize_t (*writesame)(struct rbd_ctx* ctx, uint64_t off, size_t len, const char* buf, size_t data_len);
    ssize_t (*compare_and_write)(struct rbd_ctx* ctx, uint64_t off, size_t len, const char* cmp_buf, const char* buf);
};
...
{
    char buf[1024];
    int r = rados_conf_get(cluster, "rbd_default_features", buf, sizeof(buf));
    if (r < 0) {
...
int __librbd_open(const char* name, struct rbd_ctx* ctx)
{
    rbd_image_t image;
    int ret;

    ceph_assert(!ctx->name && !ctx->image && !ctx->krbd_name && ctx->krbd_fd < 0);
...
int librbd_open(const char* name, struct rbd_ctx* ctx)
{
    return __librbd_open(name, ctx);
}

int __librbd_close(struct rbd_ctx* ctx)
{
    int ret;
...
int librbd_close(struct rbd_ctx* ctx)
{
    return __librbd_close(ctx);
}

int librbd_verify_object_map(struct rbd_ctx* ctx)
{
    int n;
    uint64_t flags;
...
ssize_t librbd_read(struct rbd_ctx* ctx, uint64_t off, size_t len, char* buf)
{
    ssize_t n;

    n = rbd_read(ctx->image, off, len, buf);
    if (n < 0) prt("rbd_read(%llu, %zu) failed\n", off, len);

    return n;
}

ssize_t librbd_write(struct rbd_ctx* ctx, uint64_t off, size_t len, const char* buf)
{
    ssize_t n;
    int ret;
...
int librbd_flush(struct rbd_ctx* ctx)
{
    int ret;
...
int librbd_discard(struct rbd_ctx* ctx, uint64_t off, uint64_t len)
{
    int ret;
...
ssize_t librbd_writesame(struct rbd_ctx* ctx, uint64_t off, size_t len, const char* buf, size_t data_len)
{
    ssize_t n;
    int ret;
...
ssize_t librbd_compare_and_write(struct rbd_ctx* ctx, uint64_t off, size_t len, const char* cmp_buf, const char* buf)
{
    ssize_t n;
    int ret;
...
int librbd_get_size(struct rbd_ctx* ctx, uint64_t* size)
{
    int ret;
...
int __librbd_resize(struct rbd_ctx* ctx, uint64_t size)
{
    int ret;
...
int librbd_resize(struct rbd_ctx* ctx, uint64_t size)
{
    return __librbd_resize(ctx, size);
}

int __librbd_deep_copy(struct rbd_ctx* ctx, const char* src_snapname, const char* dst_imagename, uint64_t features,
                       int* order, int stripe_unit, int stripe_count)
{
    int ret;
    ...
    ret = rbd_image_options_set_uint64(opts, RBD_IMAGE_OPTION_FEATURES, features);
    ceph_assert(ret == 0);
    ret = rbd_image_options_set_uint64(opts, RBD_IMAGE_OPTION_ORDER, *order);
    ceph_assert(ret == 0);
    ret = rbd_image_options_set_uint64(opts, RBD_IMAGE_OPTION_STRIPE_UNIT, stripe_unit);
    ceph_assert(ret == 0);
    ret = rbd_image_options_set_uint64(opts, RBD_IMAGE_OPTION_STRIPE_COUNT, stripe_count);
    ceph_assert(ret == 0);
...
int __librbd_clone(struct rbd_ctx* ctx, const char* src_snapname, const char* dst_imagename, int* order,
                   int stripe_unit, int stripe_count)
{
    ...
    if (deep_copy) {
        ret = __librbd_deep_copy(ctx, src_snapname, dst_imagename, features, order, stripe_unit, stripe_count);
        if (ret < 0) {
        ...
        ret = rbd_clone2(
            ioctx, ctx->name, src_snapname, ioctx, dst_imagename, features, order, stripe_unit, stripe_count);
        if (ret < 0) {
...
int librbd_clone(struct rbd_ctx* ctx, const char* src_snapname, const char* dst_imagename, int* order, int stripe_unit,
                 int stripe_count)
{
    return __librbd_clone(ctx, src_snapname, dst_imagename, order, stripe_unit, stripe_count);
}

int __librbd_flatten(struct rbd_ctx* ctx)
{
    int ret;
...
int librbd_flatten(struct rbd_ctx* ctx)
{
...
#if defined(WITH_KRBD)
int krbd_open(const char* name, struct rbd_ctx* ctx)
{
    char buf[1024];
    char* devnode;
    int fd;
    ...
    ret = rados_conf_get(cluster, "rbd_default_map_options", buf, sizeof(buf));
    if (ret < 0) {
...
int krbd_close(struct rbd_ctx* ctx)
{
    int ret;
...
#if defined(__linux__)
ssize_t krbd_read(struct rbd_ctx* ctx, uint64_t off, size_t len, char* buf)
{
    ssize_t n;
...
ssize_t krbd_write(struct rbd_ctx* ctx, uint64_t off, size_t len, const char* buf)
{
    ssize_t n;
...
int __krbd_flush(struct rbd_ctx* ctx, bool invalidate)
{
    int ret;
...
int krbd_flush(struct rbd_ctx* ctx)
{
    return __krbd_flush(ctx, false);
}

int krbd_discard(struct rbd_ctx* ctx, uint64_t off, uint64_t len)
{
...
int krbd_get_size(struct rbd_ctx* ctx, uint64_t* size)
{
    uint64_t bytes;
...
int krbd_resize(struct rbd_ctx* ctx, uint64_t size)
{
    int ret;
    int count = 0;
...
int krbd_clone(struct rbd_ctx* ctx, const char* src_snapname, const char* dst_imagename, int* order, int stripe_unit,
               int stripe_count)
{
    int ret;
...
int krbd_flatten(struct rbd_ctx* ctx)
{
    int ret;
...
#if defined(__linux__)
int nbd_open(const char* name, struct rbd_ctx* ctx)
{
    int r;
    int fd;
    char dev[4096];
    ...
    SubProcess process("rbd-nbd", SubProcess::KEEP, SubProcess::PIPE, SubProcess::KEEP);
...
int nbd_close(struct rbd_ctx* ctx)
{
    int r;
...
int nbd_clone(struct rbd_ctx* ctx, const char* src_snapname, const char* dst_imagename, int* order, int stripe_unit,
              int stripe_count)
{
    int ret;
...
#if defined(__FreeBSD__)
int ggate_open(const char* name, struct rbd_ctx* ctx)
{
    int r;
    int fd;
    char dev[4096];
    ...
    SubProcess process("rbd-ggate", SubProcess::KEEP, SubProcess::PIPE, SubProcess::KEEP);
    process.add_cmd_arg("map");
    std::string img;
    img.append(pool);
    img.append("/");
...
int ggate_close(struct rbd_ctx* ctx)
{
    int r;
...
ssize_t ggate_read(struct rbd_ctx* ctx, uint64_t off, size_t len, char* buf)
{
    ssize_t n;
...
ssize_t ggate_write(struct rbd_ctx* ctx, uint64_t off, size_t len, const char* buf)
{
    ssize_t n;
...
int __ggate_flush(struct rbd_ctx* ctx, bool invalidate)
{
    int ret;
...
int ggate_flush(struct rbd_ctx* ctx)
{
    return __ggate_flush(ctx, false);
}

int ggate_discard(struct rbd_ctx* ctx, uint64_t off, uint64_t len)
{
...
int ggate_get_size(struct rbd_ctx* ctx, uint64_t* size)
{
    off_t bytes;
...
int ggate_resize(struct rbd_ctx* ctx, uint64_t size)
{
    int ret;
...
int ggate_clone(struct rbd_ctx* ctx, const char* src_snapname, const char* dst_imagename, int* order, int stripe_unit,
                int stripe_count)
{
    int ret;
...
int ggate_flatten(struct rbd_ctx* ctx)
{
    int ret;
...
void log4(int operation, int arg0, int arg1, int arg2)
{
    ...
    if (closeopen) le->operation = ~le->operation;
    le->args[0] = arg0;
    le->args[1] = arg1;
    le->args[2] = arg2;
    logptr++;
...
void logdump(void)
{
    ...
    for (; count > 0; count--) {
        ...
        lp = &oplog[i];
        if ((closeopen = lp->operation < 0)) lp->operation = ~lp->operation;

        switch (lp->operation) {
        case OP_MAPREAD:
            prt("MAPREAD  0x%x thru 0x%x\t(0x%x bytes)", lp->args[0], lp->args[0] + lp->args[1] - 1, lp->args[1]);
            if (badoff >= lp->args[0] && badoff < lp->args[0] + lp->args[1]) prt("\t***RRRR***");
            break;
        case OP_MAPWRITE:
            prt("MAPWRITE 0x%x thru 0x%x\t(0x%x bytes)", lp->args[0], lp->args[0] + lp->args[1] - 1, lp->args[1]);
            if (badoff >= lp->args[0] && badoff < lp->args[0] + lp->args[1]) prt("\t******WWWW");
            break;
        case OP_READ:
            prt("READ     0x%x thru 0x%x\t(0x%x bytes)", lp->args[0], lp->args[0] + lp->args[1] - 1, lp->args[1]);
            if (badoff >= lp->args[0] && badoff < lp->args[0] + lp->args[1]) prt("\t***RRRR***");
            break;
        case OP_WRITE:
            prt("WRITE    0x%x thru 0x%x\t(0x%x bytes)", lp->args[0], lp->args[0] + lp->args[1] - 1, lp->args[1]);
            if (lp->args[0] > lp->args[2])
                prt(" HOLE");
            else if (lp->args[0] + lp->args[1] > lp->args[2])
                prt(" EXTEND");
            if ((badoff >= lp->args[0] || badoff >= lp->args[2]) && badoff < lp->args[0] + lp->args[1])
                prt("\t***WWWW");
            break;
        case OP_TRUNCATE:
            down = lp->args[0] < lp->args[1];
            prt("TRUNCATE %s\tfrom 0x%x to 0x%x", down ? "DOWN" : "UP", lp->args[1], lp->args[0]);
            if (badoff >= lp->args[!down] && badoff < lp->args[!!down]) prt("\t******WWWW");
            break;
        case OP_FALLOCATE:
            ...
            prt("FALLOC   0x%x thru 0x%x\t(0x%x bytes) %s",
                ...
                lp->args[0] + lp->args[1],
                lp->args[1],
                falloc_type[lp->args[2]]);
            if (badoff >= lp->args[0] && badoff < lp->args[0] + lp->args[1]) prt("\t******FFFF");
            break;
        case OP_PUNCH_HOLE:
            prt("PUNCH    0x%x thru 0x%x\t(0x%x bytes)", lp->args[0], lp->args[0] + lp->args[1] - 1, lp->args[1]);
            if (badoff >= lp->args[0] && badoff < lp->args[0] + lp->args[1]) prt("\t******PPPP");
            break;
        case OP_WRITESAME:
            prt("WRITESAME    0x%x thru 0x%x\t(0x%x bytes) data_size 0x%x",
                lp->args[0],
                lp->args[0] + lp->args[1] - 1,
                lp->args[1],
                lp->args[2]);
            if (badoff >= lp->args[0] && badoff < lp->args[0] + lp->args[1]) prt("\t***WSWSWSWS");
            break;
        case OP_COMPARE_AND_WRITE:
            ...
            if ((badoff >= lp->args[0] || badoff >= lp->args[2]) && badoff < lp->args[0] + lp->args[1])
                prt("\t***WWWW");
            break;
        case OP_CLONE: prt("CLONE"); break;
        case OP_FLATTEN: prt("FLATTEN"); break;
        case OP_SKIPPED: prt("SKIPPED (no operation)"); break;
        default: prt("BOGUS LOG ENTRY (operation code = %d)!", lp->operation);
        }
        if (closeopen) prt("\n\t\tCLOSE/OPEN");
        prt("\n");
        i++;
...
void save_buffer(char* buffer, off_t bufferlength, int fd)
{
    ...
    if (ret == (off_t)-1) prterr("save_buffer: lseek 0");
...
#define short_at(cp) ((unsigned short)((*((unsigned char*)(cp)) << 8) | *(((unsigned char*)(cp)) + 1)))

int fsxcmp(char* good_buf, char* temp_buf, unsigned size)
{
...
void check_buffers(char* good_buf, char* temp_buf, unsigned offset, unsigned size)
{
        ...
        while (size > 0) {
            ...
            if (c != t) {
                if (n < 16) {
                    ...
                    prt("0x%5x\t0x%04x\t0x%04x", offset, short_at(&good_buf[offset]), bad);
...
void check_size(void)
{
    ...
    if (ret < 0) prterrcode("check_size: ops->get_size", ret);
...
void check_trunc_hack(void)
{
    ...
    if (ret < 0) prterrcode("check_trunc_hack: ops->resize pre", ret);

    ret = ops->resize(&ctx, TRUNC_HACK_SIZE);
    if (ret < 0) prterrcode("check_trunc_hack: ops->resize actual", ret);

    ret = ops->get_size(&ctx, &size);
    if (ret < 0) prterrcode("check_trunc_hack: ops->get_size", ret);
    ...
    ret = ops->resize(&ctx, 0ULL);
    if (ret < 0) prterrcode("check_trunc_hack: ops->resize post", ret);
}
...
int create_image()
{
    ...
    r = rados_conf_get(cluster, "rbd_skip_partial_discard", buf, sizeof(buf));
    if (r < 0) {
...
void doflush(unsigned offset, unsigned size)
{
    int ret;
    ...
    ret = ops->flush(&ctx);
    if (ret < 0) prterrcode("doflush: ops->flush", ret);
}
...
void doread(unsigned offset, unsigned size)
{
    ...
    if (o_direct) size -= size % readbdy;
    if (size == 0) {
        if (!quiet && testcalls > simulatedopcount && !o_direct) prt("skipping zero size read\n");
        log4(OP_SKIPPED, OP_READ, offset, size);
        return;
    }
    if (size + offset > file_size) {
        if (!quiet && testcalls > simulatedopcount) prt("skipping seek/read past end of file\n");
        log4(OP_SKIPPED, OP_READ, offset, size);
        return;
    }
    ...
    if (testcalls <= simulatedopcount) return;

    if (!quiet && ((progressinterval && testcalls % progressinterval == 0) ||
                   (debug && (monitorstart == -1 || (static_cast<long>(offset + size) > monitorstart &&
                                                     (monitorend == -1 || static_cast<long>(offset) <= monitorend))))))
...
void check_eofpage(char* s, unsigned offset, char* p, int size)
{
    unsigned long last_page, should_be_zero;

    if (offset + size <= (file_size & ~page_mask)) return;
    /*
    ...
    for (should_be_zero = last_page + (file_size & page_mask); should_be_zero < last_page + page_size; should_be_zero++)
...
void gendata(char* original_buf, char* good_buf, unsigned offset, unsigned size)
{
    while (size--) {
        ...
        if (offset % 2) good_buf[offset] += original_buf[offset];
        offset++;
    }
}
...
void dowrite(unsigned offset, unsigned size)
{
    ...
    if (o_direct) size -= size % writebdy;
    if (size == 0) {
        if (!quiet && testcalls > simulatedopcount && !o_direct) prt("skipping zero size write\n");
    ...
    if (file_size < offset + size) {
        ...
        if (file_size < newsize) memset(good_buf + file_size, '\0', newsize - file_size);
        file_size = newsize;
        if (lite) {
    ...
    if (flush_enabled) doflush(offset, size);
}
...
void dotruncate(unsigned size)
{
    ...
    if (size > biggest) {
        biggest = size;
        if (!quiet && testcalls > simulatedopcount) prt("truncating to largest ever: 0x%x\n", size);
    }
    ...
    if ((progressinterval && testcalls % progressinterval == 0) ||
        (debug && (monitorstart == -1 || monitorend == -1 || (long)size <= monitorend)))
...
void do_punch_hole(unsigned offset, unsigned length)
{
    unsigned end_offset;
    int max_offset = 0;
    int max_len = 0;
    ...
    if (length == 0) {
        if (!quiet && testcalls > simulatedopcount) prt("skipping zero length punch hole\n");
        log4(OP_SKIPPED, OP_PUNCH_HOLE, offset, length);
        return;
    }

    if (file_size <= (loff_t)offset) {
        if (!quiet && testcalls > simulatedopcount) prt("skipping hole punch off the end of the file\n");
        log4(OP_SKIPPED, OP_PUNCH_HOLE, offset, length);
        return;
    }
    ...
    if ((progressinterval && testcalls % progressinterval == 0) ||
        (debug && (monitorstart == -1 || monitorend == -1 || (long)end_offset <= monitorend))) {
        prt("%lu punch\tfrom 0x%x to 0x%x, (0x%x bytes)\n", testcalls, offset, offset + length, length);
    }

    ret = ops->discard(&ctx, (unsigned long long)offset, (unsigned long long)length);
    if (ret < 0) {
    ...
    max_len = max_offset + length <= file_size ? length : file_size - max_offset;
...
unsigned get_data_size(unsigned size)
{
    ...
    if (curr == hint) good = curr;

    return good;
}

void dowritesame(unsigned offset, unsigned size)
{
    ssize_t ret;
    off_t newsize;
    unsigned buf_off;
    ...
    if (o_direct) size -= size % writebdy;
    if (size == 0) {
        if (!quiet && testcalls > simulatedopcount && !o_direct) prt("skipping zero size writesame\n");
...
void docompareandwrite(unsigned offset, unsigned size)
{
    int ret;
    ...
    if (size + offset > file_size) {
        if (!quiet && testcalls > simulatedopcount) prt("skipping seek/compare past end of file\n");
        log4(OP_SKIPPED, OP_COMPARE_AND_WRITE, offset, size);
        return;
    }
    ...
    ret = ops->compare_and_write(&ctx, offset, size, temp_buf + offset, good_buf + offset);
    if (ret != (ssize_t)size) {
        if (ret == -EINVAL) {
...
#endif
    snprintf(buf, len, "%s/fsx-%s-parent%d", dirpath, iname, clones);
#if __GNUC__ && __GNUC__ >= 8
...
void do_clone()
{
    ...
    clone_imagename(lastimagename, sizeof(lastimagename), num_clones - 1);
    ceph_assert(strcmp(lastimagename, ctx.name) == 0);

    ret = ops->clone(&ctx, "snap", imagename, &order, stripe_unit, stripe_count);
    if (ret < 0) {
    ...
    if (journal_replay) {
        ret = finalize_journal(ioctx, lastimagename, num_clones - 1, order, stripe_unit, stripe_count);
        if (ret < 0) {
            exit(EXIT_FAILURE);
        }
...
void check_clone(int clonenum, bool replay_image)
{
    ...
    good_buf = NULL;
    ret = posix_memalign((void**)&good_buf, std::max(writebdy, (int)sizeof(void*)), file_info.st_size);
    if (ret > 0) {
    ...
    temp_buf = NULL;
    ret = posix_memalign((void**)&temp_buf, std::max(readbdy, (int)sizeof(void*)), file_info.st_size);
    if (ret > 0) {
...
#define TRIM_OFF_LEN(off, len, size)                                                      \
    do {                                                                                  \
        if (size)                                                                         \
            ...
            (off) = 0;                                                                    \
        if ((unsigned)(off) + (unsigned)(len) > (unsigned)(size)) (len) = (size) - (off); \
    } while (0)
...
void test(void)
{
    ...
    if (simulatedopcount > 0 && testcalls == simulatedopcount) writefileimage();

    testcalls++;

    if (closeprob) closeopen = (rv >> 3) < (1u << 28) / (unsigned)closeprob;

    if (debugstart > 0 && testcalls >= debugstart) debug = 1;

    if (!quiet && testcalls < simulatedopcount && testcalls % 100000 == 0) prt("%lu...\n", testcalls);

    offset = get_random();
    if (randomoplen) size = get_random() % (maxoplen + 1);
    ...
    switch (op) {
    case OP_MAPREAD:
        if (!mapped_reads) op = OP_READ;
        break;
    case OP_MAPWRITE:
        if (!mapped_writes) op = OP_WRITE;
        break;
    case OP_FALLOCATE:
        if (!fallocate_calls) {
    ...
    case OP_TRUNCATE:
        if (!style) size = get_random() % maxfilelen;
        dotruncate(size);
        break;
    ...
    case OP_CLONE: do_clone(); break;

    case OP_FLATTEN: do_flatten(); break;
...
out:
    if (sizechecks && testcalls > simulatedopcount) check_size();
    if (closeopen) docloseopen();
}
...
void test_fallocate()
{
    ...
    if (!lite && fallocate_calls) {
        if (fallocate(fd, 0, 0, 1) && errno == EOPNOTSUPP) {
            if (!quiet) warn("main: filesystem does not support fallocate, disabling\n");
            fallocate_calls = 0;
        }
        else {
            ftruncate(fd, 0);
...
void remove_image(rados_ioctx_t ioctx, char* imagename, bool remove_snap, bool unregister)
{
    rbd_image_t image;
    char errmsg[128];
    int ret;
    ...
    if (remove_snap) {
        if ((ret = rbd_snap_unprotect(image, "snap")) < 0) {
            sprintf(errmsg, "rbd_snap_unprotect %s@snap", imagename);
            prterrcode(errmsg, ret);
            report_failure(102);
        }
        if ((ret = rbd_snap_remove(image, "snap")) < 0) {
            sprintf(errmsg, "rbd_snap_remove %s@snap", imagename);
            prterrcode(errmsg, ret);
            report_failure(103);
        }
    }
    ...
    if (unregister && (ret = unregister_journal(ioctx, imagename)) < 0) {
        report_failure(105);
    }
...
int main(int argc, char** argv)
{
    ...
    const struct option longopts[] = {{"cluster", 1, NULL, LONG_OPT_CLUSTER}, {"id", 1, NULL, LONG_OPT_ID}};
    ...
    while ((ch = getopt_long(argc, argv, optstring, longopts, NULL)) != EOF) {
        switch (ch) {
        case LONG_OPT_CLUSTER: cluster_name = optarg; break;
        case LONG_OPT_ID: client_id = optarg; break;
        case 'b':
            simulatedopcount = getnum(optarg, &endp);
            if (!quiet) fprintf(stdout, "Will begin at operation %lu\n", simulatedopcount);
            if (simulatedopcount == 0) usage();
            simulatedopcount -= 1;
            break;
        case 'c':
            closeprob = getnum(optarg, &endp);
            if (!quiet) fprintf(stdout, "Chance of close/open is 1 in %d\n", closeprob);
            if (closeprob <= 0) usage();
            break;
        case 'd': debug = 1; break;
        case 'f': flush_enabled = 1; break;
        case 'g': deep_copy = 1; break;
        case 'h':
            holebdy = getnum(optarg, &endp);
            if (holebdy <= 0) usage();
            break;
        case 'j': journal_replay = true; break;
        case 'k': keep_on_success = 1; break;
        case 'l':
        {
        ...
        case 'm':
            ...
            if (monitorstart < 0) usage();
            if (!endp || *endp++ != ':') usage();
            monitorend = getnum(endp, &endp);
            if (monitorend < 0) usage();
            if (monitorend == 0) monitorend = -1; /* aka infinity */
            debug = 1;
            break;
        case 'n': sizechecks = 0; break;
        case 'o':
        ...
        case 'w':
            ...
            if (writebdy <= 0) usage();
            break;
        case 'x': prealloc = 1; break;
        case 'y': do_fsync = 1; break;
        case 'C': clone_calls = 0; break;
        case 'D':
            debugstart = getnum(optarg, &endp);
            if (debugstart < 1) usage();
            break;
        case 'F': fallocate_calls = 0; break;
#if defined(__FreeBSD__)
        case 'G':
            ...
            break;
#endif
        case 'H': punch_hole_calls = 0; break;
#if defined(WITH_KRBD)
        case 'K':
        ...
        case 'R':
            mapped_reads = 0;
            if (!quiet) fprintf(stdout, "mapped reads DISABLED\n");
            break;
        case 'S':
            seed = getnum(optarg, &endp);
            if (seed == 0) seed = std::random_device()() % 10000;
            if (!quiet) fprintf(stdout, "Seed set to %d\n", seed);
            if (seed < 0) usage();
            break;
        case 'U': randomize_striping = 0; break;
        case 'W':
            mapped_writes = 0;
            if (!quiet) fprintf(stdout, "mapped writes DISABLED\n");
            break;
        case 'Z':
#ifdef O_DIRECT
            o_direct = O_DIRECT;
    ...
    for (i = 0; i < (int)maxfilelen; i++) original_buf[i] = get_random() % 256;

    ret = posix_memalign((void**)&good_buf, std::max(writebdy, (int)sizeof(void*)), maxfilelen);
    if (ret > 0) {
        if (ret == EINVAL)
    ...
    ret = posix_memalign((void**)&temp_buf, std::max(readbdy, (int)sizeof(void*)), maxfilelen);
    if (ret > 0) {
        if (ret == EINVAL)
    ...
    if (!keep_on_success) {
        while (num_clones >= 0) {
            ...
            if (journal_replay) {
                char replayimagename[1024];
                replay_imagename(replayimagename, sizeof(replayimagename), num_clones);
                remove_image(ioctx, replayimagename, remove_snap, false);
            }
