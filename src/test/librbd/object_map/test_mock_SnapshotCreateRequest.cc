...
public:
    ...
    void expect_read_map(librbd::ImageCtx* ictx, int r)
    {
        if (r < 0) {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                        read(ObjectMap<>::object_map_name(ictx->id, CEPH_NOSNAP), 0, 0, _, _, _))
                .WillOnce(Return(r));
        }
        else {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
    ...
    void expect_write_map(librbd::ImageCtx* ictx, uint64_t snap_id, int r)
    {
        if (r < 0) {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                        write_full(ObjectMap<>::object_map_name(ictx->id, snap_id), _, _))
                .WillOnce(Return(r));
        }
        else {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
    ...
    void expect_add_snapshot(librbd::ImageCtx* ictx, int r)
    {
        ...
        if (r < 0) {
            ...
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx), exec(oid, _, StrEq("lock"), StrEq("assert_locked"), _, _, _, _))
                .WillOnce(DoDefault());
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                        exec(oid, _, StrEq("rbd"), StrEq("object_map_snap_add"), _, _, _, _))
                .WillOnce(DoDefault());
        }
    }
...
TEST_F(TestMockObjectMapSnapshotCreateRequest, Success)
{
    ...
    AsyncRequest<>* request = new SnapshotCreateRequest(*ictx, &object_map_lock, &object_map, snap_id, &cond_ctx);
    {
...
TEST_F(TestMockObjectMapSnapshotCreateRequest, FlagCleanObjects)
{
    ...
    for (uint64_t i = 0; i < object_map.size(); ++i) {
        ASSERT_EQ(i % 2 == 0 ? OBJECT_EXISTS_CLEAN : OBJECT_NONEXISTENT, object_map[i]);
    }
}
