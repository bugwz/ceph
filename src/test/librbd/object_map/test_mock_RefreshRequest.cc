...
{
    MockObjectMapImageCtx(ImageCtx& image_ctx)
...
template<> class LockRequest<MockObjectMapImageCtx>
{
public:
    ...
    LockRequest() { s_instance = this; }

    MOCK_METHOD0(send, void());
};

template<> struct InvalidateRequest<MockObjectMapImageCtx> : public MockInvalidateRequestBase<MockObjectMapImageCtx>
{};
...
public:
    ...
    void expect_object_map_lock(MockObjectMapImageCtx& mock_image_ctx, MockLockRequest& mock_lock_request)
    {
        EXPECT_CALL(mock_lock_request, send()).WillOnce(FinishRequest(&mock_lock_request, 0, &mock_image_ctx));
    }

    void expect_object_map_load(MockObjectMapImageCtx& mock_image_ctx, ceph::BitVector<2>* object_map, uint64_t snap_id,
                                int r)
    {
        std::string oid(ObjectMap<>::object_map_name(mock_image_ctx.id, snap_id));
        auto& expect = EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                   exec(oid, _, StrEq("rbd"), StrEq("object_map_load"), _, _, _, _));
        if (r < 0) {
    ...
    void expect_get_image_size(MockObjectMapImageCtx& mock_image_ctx, uint64_t snap_id, uint64_t size)
    {
        EXPECT_CALL(mock_image_ctx, get_image_size(snap_id)).WillOnce(Return(size));
    }

    void expect_invalidate_request(MockObjectMapImageCtx& mock_image_ctx, MockInvalidateRequest& invalidate_request,
                                   int r)
    {
        EXPECT_CALL(invalidate_request, send()).WillOnce(FinishRequest(&invalidate_request, r, &mock_image_ctx));
    }
    ...
    void expect_truncate_request(MockObjectMapImageCtx& mock_image_ctx)
    {
        ...
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx), truncate(oid, 0, _)).WillOnce(Return(0));
    }

    void expect_object_map_resize(MockObjectMapImageCtx& mock_image_ctx, uint64_t num_objects, int r)
    {
        std::string oid(ObjectMap<>::object_map_name(mock_image_ctx.id, TEST_SNAP_ID));
        auto& expect = EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                   exec(oid, _, StrEq("rbd"), StrEq("object_map_resize"), _, _, _, _));
        expect.WillOnce(Return(r));
    }

    void init_object_map(MockObjectMapImageCtx& mock_image_ctx, ceph::BitVector<2>* object_map)
    {
        uint64_t num_objs = Striper::get_num_objects(mock_image_ctx.layout, mock_image_ctx.image_ctx->size);
...
TEST_F(TestMockObjectMapRefreshRequest, SuccessHead)
{
    ...
    MockRefreshRequest* req = new MockRefreshRequest(mock_image_ctx, &object_map_lock, &object_map, CEPH_NOSNAP, &ctx);

    InSequence seq;
    expect_get_image_size(mock_image_ctx, CEPH_NOSNAP, mock_image_ctx.image_ctx->size);
...
TEST_F(TestMockObjectMapRefreshRequest, SuccessSnapshot)
{
    ...
    ceph::shared_mutex object_map_lock = ceph::make_shared_mutex("lock");
    ceph::BitVector<2> object_map;
    MockRefreshRequest* req = new MockRefreshRequest(mock_image_ctx, &object_map_lock, &object_map, TEST_SNAP_ID, &ctx);

    InSequence seq;
    expect_get_image_size(mock_image_ctx, TEST_SNAP_ID, mock_image_ctx.image_ctx->size);
...
TEST_F(TestMockObjectMapRefreshRequest, LargeImageError)
{
    ...
    InSequence seq;
    expect_get_image_size(mock_image_ctx, TEST_SNAP_ID, std::numeric_limits<int64_t>::max());
