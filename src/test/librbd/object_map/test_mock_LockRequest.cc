...
public:
    ...
    void expect_lock(MockImageCtx& mock_image_ctx, int r)
    {
        ...
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx), exec(oid, _, StrEq("lock"), StrEq("lock"), _, _, _, _))
            .WillOnce(Return(r));
    }
    ...
    void expect_get_lock_info(MockImageCtx& mock_image_ctx, int r)
    {
        ...
        auto& expect = EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                   exec(oid, _, StrEq("lock"), StrEq("get_info"), _, _, _, _));
        if (r < 0) {
            ...
            reply.lockers = decltype(reply.lockers){
                {rados::cls::lock::locker_id_t(entity1, "cookie1"), rados::cls::lock::locker_info_t()},
                {rados::cls::lock::locker_id_t(entity2, "cookie2"), rados::cls::lock::locker_info_t()}};
    ...
    void expect_break_lock(MockImageCtx& mock_image_ctx, int r)
    {
        std::string oid(ObjectMap<>::object_map_name(mock_image_ctx.id, CEPH_NOSNAP));
        auto& expect = EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                                   exec(oid, _, StrEq("lock"), StrEq("break_lock"), _, _, _, _));
        if (r < 0) {
            expect.WillOnce(Return(r));
        }
        else {
