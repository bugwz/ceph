...
TEST_F(TestMockObjectMapDiffRequest, InvalidStartSnap)
{
    ...
    C_SaferCond ctx;
    auto req = new MockDiffRequest(&mock_image_ctx, CEPH_NOSNAP, 0, &m_object_diff_state, &ctx);
    req->send();
...
TEST_F(TestMockObjectMapDiffRequest, StartEndSnapEqual)
{
    ...
    C_SaferCond ctx;
    auto req = new MockDiffRequest(&mock_image_ctx, 1, 1, &m_object_diff_state, &ctx);
    req->send();
...
TEST_F(TestMockObjectMapDiffRequest, FastDiffDisabled)
{
    ...
    C_SaferCond ctx;
    auto req = new MockDiffRequest(&mock_image_ctx, 0, CEPH_NOSNAP, &m_object_diff_state, &ctx);
    req->send();
    ASSERT_EQ(-EINVAL, ctx.wait());
}
...
TEST_F(TestMockObjectMapDiffRequest, FastDiffInvalid)
{
    ...
    MockTestImageCtx mock_image_ctx(*m_image_ctx);
    mock_image_ctx.snap_info = {{1U, {"snap1", {cls::rbd::UserSnapshotNamespace{}}, {}, {}, {}, {}, {}}}};
...
TEST_F(TestMockObjectMapDiffRequest, IntermediateDelta)
{
    ...
    C_SaferCond ctx;
    auto req = new MockDiffRequest(&mock_image_ctx, 1, 2, &m_object_diff_state, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
...
TEST_F(TestMockObjectMapDiffRequest, EndDelta)
{
    ...
    C_SaferCond ctx;
    auto req = new MockDiffRequest(&mock_image_ctx, 2, CEPH_NOSNAP, &m_object_diff_state, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
...
TEST_F(TestMockObjectMapDiffRequest, StartSnapDNE)
{
    ...
    C_SaferCond ctx;
    auto req = new MockDiffRequest(&mock_image_ctx, 1, CEPH_NOSNAP, &m_object_diff_state, &ctx);
    req->send();
...
TEST_F(TestMockObjectMapDiffRequest, IntermediateSnapDNE)
{
    ...
    object_map_1.resize(object_count);
    object_map_1[1] = OBJECT_EXISTS_CLEAN;
    expect_load_map(mock_image_ctx, 1U, object_map_1, 0, [&mock_image_ctx]() { mock_image_ctx.snap_info.erase(2); });
