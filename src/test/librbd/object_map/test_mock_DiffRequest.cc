...
class TestMockObjectMapDiffRequest : public TestMockFixture, public ::testing::WithParamInterface<bool>
{
public:
    ...
    bool is_diff_iterate() const { return !GetParam(); }

    void expect_get_flags(MockTestImageCtx& mock_image_ctx, uint64_t snap_id, int32_t flags, int r)
    {
        EXPECT_CALL(mock_image_ctx, get_flags(snap_id, _)).WillOnce(WithArg<1>(Invoke([flags, r](uint64_t* out_flags) {
            *out_flags = flags;
            return r;
        })));
    }
    ...
    void expect_load_map(MockTestImageCtx& mock_image_ctx, uint64_t snap_id, const BitVector<2>& object_map, int r,
                         Lambda&& lambda)
    {
        std::string snap_oid(ObjectMap<>::object_map_name(mock_image_ctx.id, snap_id));
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    exec(snap_oid, _, StrEq("rbd"), StrEq("object_map_load"), _, _, _, _))
            .WillOnce(WithArg<5>(Invoke([object_map, r, lambda = std::move(lambda)](bufferlist* out_bl) {
                lambda();
    ...
    void expect_with_map(MockTestImageCtx& mock_image_ctx, const BitVector<2>& object_map)
    {
        EXPECT_CALL(*mock_image_ctx.object_map, with()).WillOnce(Return(object_map));
    }

    template<typename F>
    int do_diff(bool want_object_map, F&& f, uint64_t start_snap_id, uint64_t end_snap_id, uint64_t start_object_no,
                uint64_t end_object_no)
    {
        ...
        C_SaferCond ctx;
        auto req = new MockDiffRequest(
            &mock_image_ctx, start_snap_id, end_snap_id, start_object_no, end_object_no, &m_diff_state, &ctx);
        req->send();
        return ctx.wait();
    }

    template<typename F>
    void test_diff_iterate(bool want_object_map, F&& f, uint64_t start_snap_id, uint64_t end_snap_id,
                           const BitVector<2>& expected_diff_state)
    {
        ...
        for (uint64_t i = 0; i < expected_diff_state.size(); i++) {
            for (uint64_t j = i + 1; j <= expected_diff_state.size(); j++) {
                ASSERT_EQ(0, do_diff(want_object_map, std::forward<F>(f), start_snap_id, end_snap_id, i, j));
        ...
        ASSERT_EQ(0, do_diff(want_object_map, std::forward<F>(f), start_snap_id, end_snap_id, 0, UINT64_MAX - 1));
        ASSERT_EQ(expected_diff_state, m_diff_state);
    }

    template<typename F>
    void test_deep_copy(bool want_object_map, F&& f, uint64_t start_snap_id, uint64_t end_snap_id,
                        const BitVector<2>& expected_diff_state)
    {
        ASSERT_EQ(0, do_diff(want_object_map, std::forward<F>(f), start_snap_id, end_snap_id, 0, UINT64_MAX));
        ASSERT_EQ(expected_diff_state, m_diff_state);
    }
...
TEST_P(TestMockObjectMapDiffRequest, FromBeginningToSnap)
{
    ...
    m_image_ctx->snap_info = {{1U, {"snap1", {cls::rbd::UserSnapshotNamespace{}}, m_image_ctx->size, {}, {}, {}, {}}}};
...
TEST_P(TestMockObjectMapDiffRequest, FromBeginningToSnapEmpty)
{
    ...
    m_image_ctx->size = 0;
    m_image_ctx->snap_info = {{1U, {"snap1", {cls::rbd::UserSnapshotNamespace{}}, {}, {}, {}, {}, {}}}};
...
TEST_P(TestMockObjectMapDiffRequest, FromBeginningToSnapIntermediateSnapGrow)
{
    ...
    m_image_ctx->snap_info = {
        {1U,
         {"snap1", {cls::rbd::UserSnapshotNamespace{}}, object_count_1 * (1 << m_image_ctx->order), {}, {}, {}, {}}},
        {2U,
         {"snap2", {cls::rbd::UserSnapshotNamespace{}}, object_count_2 * (1 << m_image_ctx->order), {}, {}, {}, {}}}};
...
TEST_P(TestMockObjectMapDiffRequest, StartSnapDNE)
{
    ...
    m_image_ctx->size = object_count * (1 << m_image_ctx->order);
    m_image_ctx->snap_info = {{2U, {"snap2", {cls::rbd::UserSnapshotNamespace{}}, m_image_ctx->size, {}, {}, {}, {}}}};
...
TEST_P(TestMockObjectMapDiffRequest, IntermediateSnapDNEFromBeginning)
{
    ...
    if (is_diff_iterate()) {
        ...
        auto load = [&](MockTestImageCtx& mock_image_ctx) {
            expect_get_flags(mock_image_ctx, 1, 0, 0);
            expect_load_map(
                mock_image_ctx, 1, object_map_1, 0, [&mock_image_ctx]() { mock_image_ctx.snap_info.erase(2); });
            expect_get_flags(mock_image_ctx, CEPH_NOSNAP, 0, 0);
            expect_load_map(mock_image_ctx, CEPH_NOSNAP, object_map_head, 0);
        };
        test_deep_copy(false, load, 0, CEPH_NOSNAP, expected_diff_state);
...
TEST_P(TestMockObjectMapDiffRequest, IntermediateSnapDNEFromSnap)
{
    ...
    auto load = [&](MockTestImageCtx& mock_image_ctx) {
        expect_get_flags(mock_image_ctx, 1, 0, 0);
        expect_load_map(mock_image_ctx, 1, object_map_1, 0, [&mock_image_ctx]() { mock_image_ctx.snap_info.erase(2); });
        expect_get_flags(mock_image_ctx, CEPH_NOSNAP, 0, 0);
        expect_load_map(mock_image_ctx, CEPH_NOSNAP, object_map_head, 0);
    };
    if (is_diff_iterate()) {
...
INSTANTIATE_TEST_SUITE_P(MockObjectMapDiffRequestTests, TestMockObjectMapDiffRequest, ::testing::Bool());
