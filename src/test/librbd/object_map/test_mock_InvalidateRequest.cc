...
TEST_F(TestMockObjectMapInvalidateRequest, UpdatesInMemoryFlag)
{
    ...
    EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx), exec(ictx->header_oid, _, StrEq("rbd"), StrEq("set_flags"), _, _, _, _))
        .WillOnce(DoDefault());
...
TEST_F(TestMockObjectMapInvalidateRequest, UpdatesSnapOnDiskFlag)
{
    ...
    C_SaferCond cond_ctx;
    AsyncRequest<>* request = new InvalidateRequest<>(*ictx, ictx->snap_id, false, &cond_ctx);
