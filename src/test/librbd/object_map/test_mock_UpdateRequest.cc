...
{
public:
    void expect_update(librbd::ImageCtx* ictx, uint64_t snap_id, uint64_t start_object_no, uint64_t end_object_no,
                       uint8_t new_state, const boost::optional<uint8_t>& current_state, int r)
    {
        bufferlist bl;
        ...
        if (snap_id == CEPH_NOSNAP) {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx), exec(oid, _, StrEq("lock"), StrEq("assert_locked"), _, _, _, _))
                .WillOnce(DoDefault());
        }

        if (r < 0) {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
                        exec(oid, _, StrEq("rbd"), StrEq("object_map_update"), ContentsEqual(bl), _, _, _))
                .WillOnce(Return(r));
        }
        else {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx),
...
TEST_F(TestMockObjectMapUpdateRequest, UpdateInMemory)
{
    ...
    AsyncRequest<>* req = new UpdateRequest<>(*ictx,
                                              &object_map_lock,
                                              &object_map,
                                              CEPH_NOSNAP,
                                              0,
...
TEST_F(TestMockObjectMapUpdateRequest, UpdateOnDiskError)
{
    ...
    expect_update(ictx, CEPH_NOSNAP, 0, 1, OBJECT_NONEXISTENT, OBJECT_EXISTS, -EINVAL);
...
TEST_F(TestMockObjectMapUpdateRequest, RebuildSnapOnDisk)
{
    ...
    expect_update(ictx, snap_id, 0, 1, OBJECT_EXISTS_CLEAN, boost::optional<uint8_t>(), 0);
    expect_unlock_exclusive_lock(*ictx);
...
TEST_F(TestMockObjectMapUpdateRequest, BatchUpdate)
{
    ...
    librbd::NoOpProgressContext no_progress;
    ASSERT_EQ(0, ictx->operations->resize(712312 * ictx->get_object_size(), false, no_progress));
    ASSERT_EQ(0, acquire_exclusive_lock(*ictx));

    expect_unlock_exclusive_lock(*ictx);
    InSequence seq;
    expect_update(ictx, CEPH_NOSNAP, 0, 262144, OBJECT_NONEXISTENT, OBJECT_EXISTS, 0);
    expect_update(ictx, CEPH_NOSNAP, 262144, 524288, OBJECT_NONEXISTENT, OBJECT_EXISTS, 0);
    expect_update(ictx, CEPH_NOSNAP, 524288, 712312, OBJECT_NONEXISTENT, OBJECT_EXISTS, 0);
...
TEST_F(TestMockObjectMapUpdateRequest, IgnoreMissingObjectMap)
{
    ...
    expect_update(ictx, CEPH_NOSNAP, 0, 1, OBJECT_NONEXISTENT, OBJECT_EXISTS, -ENOENT);
