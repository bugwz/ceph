...
TEST_F(TestMockObjectMapResizeRequest, UpdateInMemory)
{
    ...
    AsyncRequest<>* req = new ResizeRequest(
        *ictx, &object_map_lock, &object_map, CEPH_NOSNAP, object_map.size(), OBJECT_EXISTS, &cond_ctx);
    req->send();
    ASSERT_EQ(0, cond_ctx.wait());

    for (uint64_t i = 0; i < object_map.size(); ++i) {
        ASSERT_EQ(i == 0 ? OBJECT_NONEXISTENT : OBJECT_EXISTS, object_map[i]);
    }
}
...
TEST_F(TestMockObjectMapResizeRequest, UpdateSnapOnDisk)
{
    ...
    C_SaferCond cond_ctx;
    AsyncRequest<>* req =
        new ResizeRequest(*ictx, &object_map_lock, &object_map, snap_id, object_map.size(), OBJECT_EXISTS, &cond_ctx);
    req->send();
    ASSERT_EQ(0, cond_ctx.wait());
