...
public:
    ...
    void expect_remove_map(librbd::ImageCtx* ictx, uint64_t snap_id, int r)
    {
        std::string snap_oid(ObjectMap<>::object_map_name(ictx->id, snap_id));
        if (r < 0) {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx), remove(snap_oid, _)).WillOnce(Return(r));
        }
        else {
            EXPECT_CALL(get_mock_io_ctx(ictx->md_ctx), remove(snap_oid, _)).WillOnce(DoDefault());
        }
    }
...
TEST_F(TestMockObjectMapSnapshotRemoveRequest, Success)
{
    ...
    AsyncRequest<>* request = new SnapshotRemoveRequest(*ictx, &object_map_lock, &object_map, snap_id, &cond_ctx);
    {
...
TEST_F(TestMockObjectMapSnapshotRemoveRequest, ScrubCleanObjects)
{
    ...
    uint64_t snap_id = ictx->snap_info.rbegin()->first;
    AsyncRequest<>* request = new SnapshotRemoveRequest(*ictx, &object_map_lock, &object_map, snap_id, &cond_ctx2);
    {
        std::shared_lock owner_locker{ictx->owner_lock};
        std::unique_lock image_locker{ictx->image_lock};
        request->send();
