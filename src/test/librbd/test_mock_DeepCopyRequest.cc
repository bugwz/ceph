...
template<> class ImageCopyRequest<librbd::MockTestImageCtx>
{
public:
    static ImageCopyRequest* s_instance;
    Context* on_finish;

    static ImageCopyRequest* create(librbd::MockTestImageCtx* src_image_ctx, librbd::MockTestImageCtx* dst_image_ctx,
...
template<> class MetadataCopyRequest<librbd::MockTestImageCtx>
{
public:
    static MetadataCopyRequest* s_instance;
    Context* on_finish;

    static MetadataCopyRequest* create(librbd::MockTestImageCtx* src_image_ctx, librbd::MockTestImageCtx* dst_image_ctx,
                                       Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->on_finish = on_finish;
...
template<> class SnapshotCopyRequest<librbd::MockTestImageCtx>
{
public:
    static SnapshotCopyRequest* s_instance;
    Context* on_finish;

    static SnapshotCopyRequest* create(librbd::MockTestImageCtx* src_image_ctx, librbd::MockTestImageCtx* dst_image_ctx,
                                       librados::snap_t src_snap_id_start, librados::snap_t src_snap_id_end,
                                       librados::snap_t dst_snap_id_start, bool flatten,
                                       librbd::asio::ContextWQ* work_queue, SnapSeqs* snap_seqs, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->on_finish = on_finish;
        return s_instance;
...
public:
    ...
    void TearDown() override { TestMockFixture::TearDown(); }
    ...
    {
        EXPECT_CALL(mock_object_map, rollback(_, _)).WillOnce(WithArg<1>(Invoke([this, r](Context* ctx) {
            m_work_queue->queue(ctx, r);
        })));
    }

    void expect_create_object_map(librbd::MockTestImageCtx& mock_image_ctx, librbd::MockObjectMap* mock_object_map)
    {
        EXPECT_CALL(mock_image_ctx, create_object_map(CEPH_NOSNAP)).WillOnce(Return(mock_object_map));
    }

    void expect_open_object_map(librbd::MockTestImageCtx& mock_image_ctx, librbd::MockObjectMap& mock_object_map, int r)
    {
        EXPECT_CALL(mock_object_map, open(_)).WillOnce(Invoke([this, r](Context* ctx) {
            m_work_queue->queue(ctx, r);
        }));
    }
    ...
    {
        EXPECT_CALL(mock_snapshot_copy_request, send()).WillOnce(Invoke([this, &mock_snapshot_copy_request, r]() {
    ...
    void expect_copy_object_map(librbd::MockExclusiveLock& mock_exclusive_lock, librbd::MockObjectMap* mock_object_map,
                                int r)
    {
    ...
    void expect_refresh_object_map(librbd::MockTestImageCtx& mock_image_ctx, librbd::MockObjectMap* mock_object_map,
                                   int r)
    {
    ...
    void expect_copy_metadata(MockMetadataCopyRequest& mock_metadata_copy_request, int r)
    {
        EXPECT_CALL(mock_metadata_copy_request, send()).WillOnce(Invoke([this, &mock_metadata_copy_request, r]() {
...
TEST_F(TestMockDeepCopyRequest, SimpleCopy)
{
    ...
    auto request = librbd::DeepCopyRequest<librbd::MockTestImageCtx>::create(&mock_src_image_ctx,
                                                                             &mock_dst_image_ctx,
                                                                             0,
                                                                             CEPH_NOSNAP,
                                                                             0,
...
TEST_F(TestMockDeepCopyRequest, Snap)
{
    ...
    EXPECT_EQ(0, librbd::api::Image<>::snap_set(m_src_image_ctx, cls::rbd::UserSnapshotNamespace(), "copy"));
