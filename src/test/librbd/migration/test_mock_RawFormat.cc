...
template<> struct SourceSpecBuilder<librbd::MockTestImageCtx>
{
    ...
    MOCK_CONST_METHOD3(build_snapshot, int(const json_spirit::mObject&, uint64_t, std::shared_ptr<SnapshotInterface>*));
};
...
public:
    ...
    void expect_build_snapshot(MockSourceSpecBuilder& mock_source_spec_builder, uint64_t index,
                               MockSnapshotInterface* mock_snapshot_interface, int r)
    {
        EXPECT_CALL(mock_source_spec_builder, build_snapshot(_, index, _))
            .WillOnce(WithArgs<2>(Invoke([mock_snapshot_interface, r](std::shared_ptr<SnapshotInterface>* ptr) {
                ptr->reset(mock_snapshot_interface);
                return r;
            })));
    }

    void expect_snapshot_open(MockSnapshotInterface& mock_snapshot_interface, int r)
    {
        EXPECT_CALL(mock_snapshot_interface, open(_, _)).WillOnce(WithArg<1>(Invoke([r](Context* ctx) {
            ctx->complete(r);
        })));
    }

    void expect_snapshot_close(MockSnapshotInterface& mock_snapshot_interface, int r)
    {
        EXPECT_CALL(mock_snapshot_interface, close(_)).WillOnce(Invoke([r](Context* ctx) { ctx->complete(r); }));
    }

    void expect_snapshot_get_info(MockSnapshotInterface& mock_snapshot_interface, const SnapInfo& snap_info)
    {
        EXPECT_CALL(mock_snapshot_interface, get_snap_info()).WillOnce(ReturnRef(snap_info));
    }

    void expect_snapshot_read(MockSnapshotInterface& mock_snapshot_interface, const io::Extents& image_extents,
                              const bufferlist& bl, int r)
    {
        EXPECT_CALL(mock_snapshot_interface, read(_, image_extents, _))
            .WillOnce(
                WithArgs<0, 2>(Invoke([bl, image_extents, r](io::AioCompletion* aio_comp, io::ReadResult& read_result) {
    ...
    void expect_snapshot_list_snap(MockSnapshotInterface& mock_snapshot_interface, const io::Extents& image_extents,
                                   const io::SparseExtents& sparse_extents, int r)
    {
        EXPECT_CALL(mock_snapshot_interface, list_snap(image_extents, _, _))
            .WillOnce(WithArgs<1, 2>(Invoke([sparse_extents, r](io::SparseExtents* out_sparse_extents, Context* ctx) {
    ...
    {
        EXPECT_CALL(*mock_image_ctx.state, close(_)).WillOnce(Invoke([r](Context* ctx) { ctx->complete(r); }));
    }
...
TEST_F(TestMockMigrationRawFormat, OpenClose)
{
    ...
    expect_build_snapshot(mock_source_spec_builder, CEPH_NOSNAP, mock_snapshot_interface, 0);

    expect_snapshot_open(*mock_snapshot_interface, 0);

    expect_snapshot_close(*mock_snapshot_interface, 0);

    MockRawFormat mock_raw_format(&mock_image_ctx, json_object, &mock_source_spec_builder);
...
TEST_F(TestMockMigrationRawFormat, OpenSnapshotError)
{
    ...
    expect_build_snapshot(mock_source_spec_builder, CEPH_NOSNAP, mock_snapshot_interface_head, 0);

    auto mock_snapshot_interface_1 = new MockSnapshotInterface();
    expect_build_snapshot(mock_source_spec_builder, 1, mock_snapshot_interface_1, 0);
...
TEST_F(TestMockMigrationRawFormat, Read)
{
    ...
    ASSERT_TRUE(mock_raw_format.read(aio_comp, CEPH_NOSNAP, {{123, 123}}, std::move(read_result), 0, 0, {}));
...
TEST_F(TestMockMigrationRawFormat, ListSnaps)
{
    ...
    expect_snapshot_list_snap(*mock_snapshot_interface, {{0, 123}}, sparse_extents, 0);
    ...
    mock_raw_format.list_snaps({{0, 123}}, {CEPH_NOSNAP}, 0, &snapshot_delta, {}, &ctx2);
    ASSERT_EQ(0, ctx2.wait());
...
TEST_F(TestMockMigrationRawFormat, ListSnapsError)
{
    ...
    io::SparseExtents sparse_extents;
    sparse_extents.insert(0, 123, {io::SPARSE_EXTENT_STATE_DATA, 123});
    expect_snapshot_list_snap(*mock_snapshot_interface, {{0, 123}}, sparse_extents, -EINVAL);
...
TEST_F(TestMockMigrationRawFormat, ListSnapsMerge)
{
    ...
    expect_build_snapshot(mock_source_spec_builder, 2, mock_snapshot_interface_2, 0);
    ...
    expect_snapshot_list_snap(*mock_snapshot_interface_1, {{0, 123}}, sparse_extents_1, 0);
    ...
    expect_snapshot_list_snap(*mock_snapshot_interface_2, {{0, 123}}, sparse_extents_2, 0);
    ...
    expect_snapshot_list_snap(*mock_snapshot_interface_head, {{0, 123}}, sparse_extents_head, 0);
    ...
    C_SaferCond ctx2;
    io::SnapshotDelta snapshot_delta;
    mock_raw_format.list_snaps({{0, 123}}, {1, CEPH_NOSNAP}, 0, &snapshot_delta, {}, &ctx2);
    ASSERT_EQ(0, ctx2.wait());
