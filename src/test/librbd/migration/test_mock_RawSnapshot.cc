...
public:
    ...
    void expect_stream_get_size(MockStreamInterface& mock_stream_interface, uint64_t size, int r)
    {
        EXPECT_CALL(mock_stream_interface, get_size(_, _)).WillOnce(Invoke([size, r](uint64_t* out_size, Context* ctx) {
            *out_size = size;
            ctx->complete(r);
        }));
    }
...
TEST_F(TestMockMigrationRawSnapshot, OpenClose)
{
    ...
    MockRawSnapshot mock_raw_snapshot(&mock_image_ctx, json_object, &mock_source_spec_builder, 1);
...
TEST_F(TestMockMigrationRawSnapshot, OpenError)
{
    ...
    MockRawSnapshot mock_raw_snapshot(&mock_image_ctx, json_object, &mock_source_spec_builder, 0);
