...
using EmptyHttpRequest = boost::beast::http::request<boost::beast::http::empty_body>;
...
bool operator==(const boost::beast::http::request<Body>& lhs, const boost::beast::http::request<Body>& rhs)
{
    return (lhs.method() == rhs.method() && lhs.target() == rhs.target());
}

template<typename Body>
bool operator==(const boost::beast::http::response<Body>& lhs, const boost::beast::http::response<Body>& rhs)
{
...
public:
        ...
        for (int retries = 0;; retries++) {
            try {
                m_acceptor.emplace(*m_image_ctx->asio_engine,
                                   boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), m_server_port),
                                   reuse);
                // yay!
                break;
            }
    ...
    std::string get_local_url(UrlScheme url_scheme)
    {
        ...
        switch (url_scheme) {
        case URL_SCHEME_HTTP: sstream << "http://127.0.0.1"; break;
        case URL_SCHEME_HTTPS: sstream << "https://localhost"; break;
    ...
    void client_accept(boost::asio::ip::tcp::socket* socket, bool close, Context* on_connect)
    {
        m_acceptor->async_accept(boost::asio::make_strand(m_image_ctx->asio_engine->get_executor()),
                                 [socket, close, on_connect](auto ec, boost::asio::ip::tcp::socket in_socket) {
                                     if (close) {
    ...
    template<typename Body>
    void client_read_request(boost::asio::ip::tcp::socket& socket, boost::beast::http::request<Body>& expected_req)
    {
    ...
    void client_write_response(boost::asio::ip::tcp::socket& socket, HttpResponse& expected_res)
    {
        expected_res.set(boost::beast::http::field::server, BOOST_BEAST_VERSION_STRING);
    ...
    template<typename Stream> void client_ssl_handshake(Stream& stream, bool ignore_failure, Context* on_handshake)
    {
        stream.async_handshake(boost::asio::ssl::stream_base::server, [ignore_failure, on_handshake](auto ec) {
            ASSERT_FALSE(!ignore_failure && ec) << "Unexpected error: " << ec;
            on_handshake->complete(-ec.value());
        });
    }

    template<typename Stream> void client_ssl_shutdown(Stream& stream, Context* on_shutdown)
    {
    ...
    void load_server_certificate(boost::asio::ssl::context& ctx)
    {
        ...
        ctx.use_certificate_chain(boost::asio::buffer(CERTIFICATE.data(), CERTIFICATE.size()));
...
TEST_F(TestMockMigrationHttpClient, OpenCloseHttp)
{
    ...
    MockHttpClient http_client(&mock_test_image_ctx, get_local_url(URL_SCHEME_HTTP));
...
TEST_F(TestMockMigrationHttpClient, OpenCloseHttps)
{
    ...
    MockTestImageCtx mock_test_image_ctx(*m_image_ctx);
    MockHttpClient http_client(&mock_test_image_ctx, get_local_url(URL_SCHEME_HTTPS));
    ...
    boost::beast::ssl_stream<boost::beast::tcp_stream> ssl_stream{std::move(socket), ssl_context};
...
TEST_F(TestMockMigrationHttpClient, IssueHead)
{
    ...
    C_SaferCond ctx2;
    HttpResponse res;
    http_client.issue(EmptyHttpRequest{req}, [&ctx2, &res](int r, HttpResponse&& response) mutable {
...
TEST_F(TestMockMigrationHttpClient, IssueSendFailed)
{
    ...
    C_SaferCond ctx2;
    http_client.issue(EmptyHttpRequest{req}, [&ctx2](int r, HttpResponse&&) mutable { ctx2.complete(r); });
...
TEST_F(TestMockMigrationHttpClient, IssueResetFailed)
{
    ...
    C_SaferCond ctx3;
    http_client.issue(EmptyHttpRequest{req}, [&ctx3](int r, HttpResponse&&) mutable { ctx3.complete(r); });
    ...
    C_SaferCond ctx4;
    http_client.issue(EmptyHttpRequest{req}, [&ctx4](int r, HttpResponse&&) mutable { ctx4.complete(r); });
...
TEST_F(TestMockMigrationHttpClient, IssuePipelined)
{
    ...
    C_SaferCond ctx2;
    HttpResponse res1;
    http_client.issue(EmptyHttpRequest{req1}, [&ctx2, &res1](int r, HttpResponse&& response) mutable {
