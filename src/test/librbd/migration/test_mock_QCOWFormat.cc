...
template<> struct SourceSpecBuilder<librbd::MockTestImageCtx>
{

    MOCK_CONST_METHOD2(build_stream, int(const json_spirit::mObject&, std::shared_ptr<StreamInterface>*));
};
...
public:
                             ...
                             MockStreamInterface* mock_stream_interface, int r)
    {
        EXPECT_CALL(mock_source_spec_builder, build_stream(_, _))
            .WillOnce(WithArgs<1>(Invoke([mock_stream_interface, r](std::shared_ptr<StreamInterface>* ptr) {
    ...
    {
        EXPECT_CALL(mock_stream_interface, open(_)).WillOnce(Invoke([r](Context* ctx) { ctx->complete(r); }));
    }

    void expect_stream_close(MockStreamInterface& mock_stream_interface, int r)
    {
        EXPECT_CALL(mock_stream_interface, close(_)).WillOnce(Invoke([r](Context* ctx) { ctx->complete(r); }));
    }

    void expect_stream_read(MockStreamInterface& mock_stream_interface, const io::Extents& byte_extents,
                            const bufferlist& bl, int r)
    {
        EXPECT_CALL(mock_stream_interface, read(byte_extents, _, _))
            .WillOnce(WithArgs<1, 2>(Invoke([bl, r](bufferlist* out_bl, Context* ctx) {
                *out_bl = bl;
                ctx->complete(r);
            })));
    }

    void expect_probe_header(MockStreamInterface& mock_stream_interface, uint32_t magic, uint32_t version, int r)
    {
        magic = htobe32(magic);
    ...
    void expect_read_header(MockStreamInterface& mock_stream_interface, uint32_t snapshot_count, int r)
    {
        ...
        header_bl.append(std::string_view(reinterpret_cast<char*>(&qcow_header), sizeof(qcow_header)));
        expect_stream_read(mock_stream_interface, {{0, sizeof(qcow_header)}}, header_bl, r);
    }

    void expect_read_l1_table(MockStreamInterface& mock_stream_interface,
                              std::optional<std::vector<uint64_t>>&& l1_table, int r)
    {
        ...
        l1_table_bl.append(std::string_view(reinterpret_cast<char*>(l1_table->data()), 16));
        expect_stream_read(mock_stream_interface, {{1 << 20, 16}}, l1_table_bl, r);
    }

    void expect_read_l2_table(MockStreamInterface& mock_stream_interface, uint64_t l2_table_offset,
                              std::optional<std::vector<uint64_t>>&& l2_table, int r)
    {
        ...
        l2_table_bl.append(
            std::string_view(reinterpret_cast<char*>(l2_table->data()), l2_table->size() * sizeof(uint64_t)));
        expect_stream_read(
            mock_stream_interface, {{l2_table_offset, l2_table->size() * sizeof(uint64_t)}}, l2_table_bl, r);
    }

    void expect_read_cluster(MockStreamInterface& mock_stream_interface, uint64_t cluster_offset, uint32_t offset,
                             const bufferlist& bl, int r)
    {
        ...
        expect_stream_read(mock_stream_interface, {{cluster_offset, cluster_bl.length()}}, cluster_bl, r);
    }

    void expect_open(MockStreamInterface& mock_stream_interface, std::optional<std::vector<uint64_t>>&& l1_table)
    {
    ...
    void expect_read_snapshot_header(MockStreamInterface& mock_stream_interface, const std::string& id,
                                     const std::string& name, uint64_t l1_table_offset, uint64_t* snapshot_offset,
                                     int r)
    {
        ...
        snapshot_header_bl.append(std::string_view(reinterpret_cast<char*>(&snapshot_header), sizeof(snapshot_header)));
        expect_stream_read(mock_stream_interface, {{*snapshot_offset, sizeof(snapshot_header)}}, snapshot_header_bl, r);
        *snapshot_offset += sizeof(snapshot_header);
    }

    void expect_read_snapshot_header_extra(MockStreamInterface& mock_stream_interface, const std::string& id,
                                           const std::string& name, uint64_t size, uint64_t* snapshot_offset, int r)
    {
        ...
        snapshot_header_extra_bl.append(std::string_view(reinterpret_cast<char*>(&snapshot_header_extra), 16));
        snapshot_header_extra_bl.append(id);
        snapshot_header_extra_bl.append(name);
        expect_stream_read(
            mock_stream_interface, {{*snapshot_offset, 16 + id.size() + name.size()}}, snapshot_header_extra_bl, r);

        *snapshot_offset += 16 + id.size() + name.size();
        *snapshot_offset = p2roundup(*snapshot_offset, static_cast<uint64_t>(8));
    }

    void expect_read_snapshot_l1_table(MockStreamInterface& mock_stream_interface, uint64_t l1_table_offset, int r)
    {
        uint64_t l2_table_cluster = htobe64(l1_table_offset);

        bufferlist snapshot_l1_table_bl;
        snapshot_l1_table_bl.append(std::string_view(reinterpret_cast<char*>(&l2_table_cluster), 8));
        expect_stream_read(mock_stream_interface, {{l1_table_offset, 8}}, snapshot_l1_table_bl, r);
    }


    void expect_read_snapshot(MockStreamInterface& mock_stream_interface, const std::string& id,
                              const std::string& name, uint64_t size, uint64_t l1_table_offset,
                              uint64_t* snapshot_offset)
    {
        expect_read_snapshot_header(mock_stream_interface, id, name, l1_table_offset, snapshot_offset, 0);
        expect_read_snapshot_header_extra(mock_stream_interface, id, name, size, snapshot_offset, 0);
...
TEST_F(TestMockMigrationQCOWFormat, OpenCloseV1)
{
    ...
    bufferlist header_bl;
    header_bl.append(std::string_view(reinterpret_cast<char*>(&qcow_header), sizeof(qcow_header)));
    expect_stream_read(*mock_stream_interface, {{0, sizeof(qcow_header)}}, header_bl, 0);
    ...
    MockQCOWFormat mock_qcow_format(&mock_image_ctx, json_object, &mock_source_spec_builder);
...
TEST_F(TestMockMigrationQCOWFormat, ReadHeaderV1Error)
{
    ...
    bufferlist header_bl;
    header_bl.append(std::string_view(reinterpret_cast<char*>(&qcow_header), sizeof(qcow_header)));
    expect_stream_read(*mock_stream_interface, {{0, sizeof(qcow_header)}}, header_bl, -EPERM);
...
TEST_F(TestMockMigrationQCOWFormat, Snapshots)
{
    ...
    expect_read_snapshot(*mock_stream_interface, "1", "snap1", 1 << 29, 1 << 22, &snapshot_offset);
    expect_read_snapshot(*mock_stream_interface, "2", "snap2", 1 << 29, 1 << 22, &snapshot_offset);
...
TEST_F(TestMockMigrationQCOWFormat, SnapshotHeaderError)
{
    ...
    uint64_t snapshot_offset = 1 << 21;
    expect_read_snapshot_header(*mock_stream_interface, "1", "snap1", 1 << 22, &snapshot_offset, -EINVAL);
...
TEST_F(TestMockMigrationQCOWFormat, SnapshotExtraError)
{
    ...
    uint64_t snapshot_offset = 1 << 21;
    expect_read_snapshot_header(*mock_stream_interface, "1", "snap1", 1 << 22, &snapshot_offset, 0);
    expect_read_snapshot_header_extra(*mock_stream_interface, "1", "snap1", 1 << 29, &snapshot_offset, -EINVAL);
...
TEST_F(TestMockMigrationQCOWFormat, SnapshotL1TableError)
{
    ...
    uint64_t snapshot_offset = 1 << 21;
    expect_read_snapshot_header(*mock_stream_interface, "1", "snap1", 1 << 22, &snapshot_offset, 0);
    expect_read_snapshot_header_extra(*mock_stream_interface, "1", "snap1", 1 << 29, &snapshot_offset, 0);
...
TEST_F(TestMockMigrationQCOWFormat, Read)
{
    ...
    expect_read_l2_table(*mock_stream_interface, 1ULL << 40, {{0, 1ULL << 32}}, 0);
    ...
    C_SaferCond ctx2;
    auto aio_comp = io::AioCompletion::create_and_start(&ctx2, m_image_ctx, io::AIO_TYPE_READ);
    bufferlist bl;
    io::ReadResult read_result{&bl};
    ASSERT_TRUE(mock_qcow_format.read(aio_comp, CEPH_NOSNAP, {{65659, 123}}, std::move(read_result), 0, 0, {}));
...
TEST_F(TestMockMigrationQCOWFormat, ReadL1DNE)
{
    ...
    bufferlist bl;
    io::ReadResult read_result{&bl};
    ASSERT_TRUE(mock_qcow_format.read(aio_comp, CEPH_NOSNAP, {{234, 123}}, std::move(read_result), 0, 0, {}));
    ASSERT_EQ(123, ctx2.wait());
    bufferlist expect_bl;
...
TEST_F(TestMockMigrationQCOWFormat, ReadZero)
{
    ...
    expect_read_l2_table(*mock_stream_interface, 1ULL << 40, {{0, QCOW_OFLAG_ZERO}}, 0);
...
TEST_F(TestMockMigrationQCOWFormat, ReadSnap)
{
    ...
    expect_read_l2_table(*mock_stream_interface, 1 << 22, {{0, 1ULL << 32}}, 0);
    ...
    bufferlist bl;
    io::ReadResult read_result{&bl};
    ASSERT_TRUE(mock_qcow_format.read(aio_comp, 1, {{65659, 123}}, std::move(read_result), 0, 0, {}));
    ASSERT_EQ(123, ctx2.wait());
    ASSERT_EQ(expect_bl, bl);
...
TEST_F(TestMockMigrationQCOWFormat, ReadClusterCacheHit)
{
    ...
    C_SaferCond ctx2;
    auto aio_comp1 = io::AioCompletion::create_and_start(&ctx2, m_image_ctx, io::AIO_TYPE_READ);
    bufferlist bl1;
    io::ReadResult read_result1{&bl1};
    ASSERT_TRUE(mock_qcow_format.read(aio_comp1, CEPH_NOSNAP, {{65659, 123}}, std::move(read_result1), 0, 0, {}));
    ASSERT_EQ(123, ctx2.wait());
    ASSERT_EQ(expect_bl1, bl1);

    C_SaferCond ctx3;
    auto aio_comp2 = io::AioCompletion::create_and_start(&ctx3, m_image_ctx, io::AIO_TYPE_READ);
    bufferlist bl2;
    io::ReadResult read_result2{&bl2};
    ASSERT_TRUE(mock_qcow_format.read(aio_comp2, CEPH_NOSNAP, {{66016, 234}}, std::move(read_result2), 0, 0, {}));
...
TEST_F(TestMockMigrationQCOWFormat, ReadL2TableError)
{
    ...
    expect_read_l2_table(*mock_stream_interface, 1ULL << 40, {{0, 1ULL << 32}}, -EIO);
...
TEST_F(TestMockMigrationQCOWFormat, ListSnaps)
{
    ...
    uint64_t snapshot_offset = 1 << 21;
    expect_read_snapshot(*mock_stream_interface, "1", "snap1", 1 << 29, 1 << 22, &snapshot_offset);
    expect_read_snapshot(*mock_stream_interface, "2", "snap2", 1 << 29, 1 << 23, &snapshot_offset);
    ...
    expect_read_l2_table(*mock_stream_interface, 1ULL << 22, {{1ULL << 23, 1ULL << 24, 1ULL << 25}}, 0);

    io::SparseExtents sparse_extents_2;
    sparse_extents_2.insert(0, 65536, {io::SPARSE_EXTENT_STATE_DATA, 65536});
    sparse_extents_2.insert(65536, 65536, {io::SPARSE_EXTENT_STATE_ZEROED, 65536});
    expect_read_l2_table(*mock_stream_interface, 1ULL << 23, {{1ULL << 26, QCOW_OFLAG_ZERO, 1ULL << 25}}, 0);

    io::SparseExtents sparse_extents_head;
    sparse_extents_head.insert(131072, 65536, {io::SPARSE_EXTENT_STATE_DATA, 65536});
    expect_read_l2_table(*mock_stream_interface, 1ULL << 28, {{1ULL << 26, QCOW_OFLAG_ZERO, 1ULL << 27}}, 0);
    ...
    mock_qcow_format.list_snaps({{0, 196608}}, {1, CEPH_NOSNAP}, 0, &snapshot_delta, {}, &ctx2);
    ASSERT_EQ(0, ctx2.wait());
