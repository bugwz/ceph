...
template<> struct TypeTraits<MockParentImageCacheImageCtx>
{
...
template<> struct Api<MockParentImageCacheImageCtx>
{
    MOCK_METHOD6(read_parent, void(MockParentImageCacheImageCtx*, uint64_t, librbd::io::ReadExtents*, librados::snap_t,
...
public:
    ...
    void expect_cache_session_state(MockParentImageCache& mparent_image_cache, bool ret_val)
    {
        ...
        expect.WillOnce((Invoke([ret_val]() { return ret_val; })));
    }
    ...
    void expect_cache_async_connect(MockParentImageCache& mparent_image_cache, int ret_val, Context* on_finish)
    {
    ...
    void expect_cache_lookup_object(MockParentImageCache& mparent_image_cache, const std::string& cache_path)
    {
        EXPECT_CALL(*(mparent_image_cache.get_cache_client()), lookup_object(_, _, _, _, _, _))
    ...
    void expect_read_parent(MockPluginApi& mock_plugin_api, uint64_t object_no, io::ReadExtents* extents,
                            librados::snap_t snap_id, int r)
    {
        EXPECT_CALL(mock_plugin_api, read_parent(_, object_no, extents, snap_id, _, _))
    ...
    void expect_io_object_dispatcher_register_state(MockParentImageCache& mparent_image_cache, int ret_val)
    {
        auto& expect =
            EXPECT_CALL((*(mparent_image_cache.get_image_ctx()->io_object_dispatcher)), register_dispatch(_));

        expect.WillOnce(WithArg<0>(Invoke([&mparent_image_cache](io::ObjectDispatchInterface* object_dispatch) {
...
TEST_F(TestMockParentCacheObjectDispatch, test_initialization_success)
{
    ...
    auto mock_parent_image_cache = MockParentImageCache::create(&mock_image_ctx, mock_plugin_api);
    ...
    Context* ctx = new LambdaContext([](bool reg) { ASSERT_EQ(reg, true); });
    ...
    ASSERT_EQ(mock_parent_image_cache->get_dispatch_layer(), io::OBJECT_DISPATCH_LAYER_PARENT_CACHE);
...
TEST_F(TestMockParentCacheObjectDispatch, test_initialization_fail_at_register)
{
    ...
    expect_cache_async_connect(*mock_parent_image_cache, 0, handle_connect);
    Context* ctx = new LambdaContext([](bool reg) { ASSERT_EQ(reg, false); });
...
TEST_F(TestMockParentCacheObjectDispatch, test_disble_interface)
{
    ...
    ASSERT_EQ(mock_parent_image_cache->discard(0,
                                               0,
                                               0,
                                               io_context,
                                               0,
                                               *temp_trace,
                                               ...
                                               temp_on_dispatched),
              false);
    ASSERT_EQ(mock_parent_image_cache->write(0,
                                             0,
                                             std::move(temp_bl),
                                             io_context,
                                             0,
                                             ...
                                             *temp_trace,
                                             ...
                                             temp_on_dispatched),
              false);
    ASSERT_EQ(mock_parent_image_cache->write_same(0,
                                                  0,
                                                  0,
                                                  ...
                                                  *temp_trace,
                                                  ...
                                                  temp_on_dispatched),
              false);
    ASSERT_EQ(mock_parent_image_cache->compare_and_write(0,
                                                         0,
                                                         std::move(temp_bl),
                                                         std::move(temp_bl),
                                                         io_context,
                                                         ...
                                                         *temp_trace,
                                                         ...
                                                         temp_on_dispatched),
              false);
    ASSERT_EQ(mock_parent_image_cache->flush(io::FLUSH_SOURCE_USER,
                                             *temp_trace,
                                             temp_journal_tid,
                                             temp_dispatch_result,
                                             temp_on_finish,
...
TEST_F(TestMockParentCacheObjectDispatch, test_read)
{
    ...
    mock_parent_image_cache->read(0,
                                  &extents,
...
TEST_F(TestMockParentCacheObjectDispatch, test_read_dne)
{
    ...
    EXPECT_CALL(*(mock_parent_image_cache->get_cache_client()), is_session_work()).WillOnce(Return(true));
