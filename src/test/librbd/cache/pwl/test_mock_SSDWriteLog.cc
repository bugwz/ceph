...
struct MockContextSSD : public C_SaferCond
{
    ...
    void do_complete(int r) { C_SaferCond::complete(r); }
};
...
struct TestMockCacheSSDWriteLog : public TestMockFixture
{
    ...
    MockImageCacheStateSSD* get_cache_state(MockImageCtx& mock_image_ctx, MockApi& mock_api)
    {
        MockImageCacheStateSSD* ssd_state = new MockImageCacheStateSSD(&mock_image_ctx, mock_api);
        return ssd_state;
    }

    void validate_cache_state(librbd::ImageCtx* image_ctx, MockImageCacheStateSSD& state, bool present, bool empty,
                              bool clean, string host, string path, uint64_t size)
    {
    ...
    {
        EXPECT_CALL(mock_context, complete(r)).WillRepeatedly(Invoke([&mock_context](int r) {
    ...
    void expect_metadata_set(MockImageCtx& mock_image_ctx)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_metadata_set(_, _, _))
            .WillRepeatedly(Invoke([](std::string key, std::string val, Context* ctx) { ctx->complete(0); }));
    }

    void expect_metadata_remove(MockImageCtx& mock_image_ctx)
    {
        EXPECT_CALL(*mock_image_ctx.operations, execute_metadata_remove(_, _))
            .WillRepeatedly(Invoke([](std::string key, Context* ctx) { ctx->complete(0); }));
    }
};
...
TEST_F(TestMockCacheSSDWriteLog, init_state_json_write)
{
    ...
    validate_cache_state(ictx, image_cache_state, true, false, false, "testhost", "/tmp", 1024);
...
TEST_F(TestMockCacheSSDWriteLog, init_shutdown)
{
    ...
    MockApi mock_api;
    MockSSDWriteLog ssd(mock_image_ctx, get_cache_state(mock_image_ctx, mock_api), mock_image_writeback, mock_api);
...
TEST_F(TestMockCacheSSDWriteLog, read_hit_ssd_cache)
{
    ...
    bufferlist read_bl;
    ssd.read(std::move(image_extents_read), &read_bl, fadvise_flags, &finish_ctx_read);
...
TEST_F(TestMockCacheSSDWriteLog, compare_and_write_compare_matched)
{
    ...
    ssd.compare_and_write(
        {{0, 4096}}, std::move(com_bl), std::move(bl2), &mismatch_offset, fadvise_flags, &finish_ctx_cw);
