...
public:
    ...
    {
    public:
        explicit WatchCtx(TestImageWatcher& parent)
            : m_parent(parent)
            , m_handle(0)
        {}
        ...
        int unwatch() { return m_parent.m_ioctx.unwatch2(m_handle); }
        ...
        {
            std::cerr << "ERROR: " << cookie << ", " << cpp_strerror(err) << std::endl;
        }
    ...
    int notify_async_progress(librbd::ImageCtx* ictx, const AsyncRequestId& id, uint64_t offset, uint64_t total)
    {
        bufferlist bl;
    ...
    int notify_async_complete(librbd::ImageCtx* ictx, const AsyncRequestId& id, int r)
    {
        bufferlist bl;
...
struct SnapCreateTask
{
    ...
    void operator()()
    {
        std::shared_lock l{ictx->owner_lock};
        C_SaferCond ctx;
        ictx->image_watcher->notify_snap_create(
            0, cls::rbd::UserSnapshotNamespace(), "snap", 0, *progress_context, &ctx);
        result = ctx.wait();
    }
};
...
struct SnapRemoveTask
{
    ...
    void operator()()
    {
        std::shared_lock l{ictx->owner_lock};
        C_SaferCond ctx;
        ictx->image_watcher->notify_snap_remove(0, cls::rbd::UserSnapshotNamespace(), "snap", &ctx);
        result = ctx.wait();
    }
};
...
struct SnapProtectTask
{
    ...
    void operator()()
    {
        std::shared_lock l{ictx->owner_lock};
        C_SaferCond ctx;
        ictx->image_watcher->notify_snap_protect(0, cls::rbd::UserSnapshotNamespace(), "snap", &ctx);
        result = ctx.wait();
    }
};
...
struct SnapUnprotectTask
{
    ...
    void operator()()
    {
        std::shared_lock l{ictx->owner_lock};
        C_SaferCond ctx;
        ictx->image_watcher->notify_snap_unprotect(0, cls::rbd::UserSnapshotNamespace(), "snap", &ctx);
        result = ctx.wait();
    }
};
...
TEST_F(TestImageWatcher, NotifyFlatten)
{
    ...
    ASSERT_EQ(0, register_image_watch(*ictx));
    ASSERT_EQ(0, lock_image(*ictx, ClsLockType::EXCLUSIVE, "auto " + stringify(m_watch_ctx->get_handle())));
...
TEST_F(TestImageWatcher, NotifyRebuildObjectMap)
{
    ...
    AsyncRequestId async_request_id;
    ASSERT_TRUE(extract_async_request_id(NOTIFY_OP_REBUILD_OBJECT_MAP, &async_request_id));
...
TEST_F(TestImageWatcher, NotifyUpdateFeatures)
{
    ...
    AsyncRequestId async_request_id;
    ASSERT_TRUE(extract_async_request_id(NOTIFY_OP_UPDATE_FEATURES, &async_request_id));
...
TEST_F(TestImageWatcher, NotifySnapCreate)
{
    ...
    AsyncRequestId async_request_id;
    ASSERT_TRUE(extract_async_request_id(NOTIFY_OP_SNAP_CREATE, &async_request_id));
...
TEST_F(TestImageWatcher, NotifySnapCreateError)
{
    ...
    ictx->image_watcher->notify_snap_create(0, cls::rbd::UserSnapshotNamespace(), "snap", 0, prog_ctx, &notify_ctx);
...
TEST_F(TestImageWatcher, NotifySnapRename)
{
    ...
    AsyncRequestId async_request_id;
    ASSERT_TRUE(extract_async_request_id(NOTIFY_OP_SNAP_RENAME, &async_request_id));
...
TEST_F(TestImageWatcher, NotifySnapRemove)
{
    ...
    AsyncRequestId async_request_id;
    ASSERT_TRUE(extract_async_request_id(NOTIFY_OP_SNAP_REMOVE, &async_request_id));
...
TEST_F(TestImageWatcher, NotifySnapProtect)
{
    ...
    AsyncRequestId async_request_id;
    ASSERT_TRUE(extract_async_request_id(NOTIFY_OP_SNAP_PROTECT, &async_request_id));
...
TEST_F(TestImageWatcher, NotifySnapUnprotect)
{
    ...
    AsyncRequestId async_request_id;
    ASSERT_TRUE(extract_async_request_id(NOTIFY_OP_SNAP_UNPROTECT, &async_request_id));
...
TEST_F(TestImageWatcher, NotifyRename)
{
    ...
    AsyncRequestId async_request_id;
    ASSERT_TRUE(extract_async_request_id(NOTIFY_OP_RENAME, &async_request_id));
