...
struct Mock
{
    ...
    MOCK_METHOD1(can_create_non_primary_snapshot, bool(librbd::MockTestImageCtx*));
};
...
template<> bool can_create_non_primary_snapshot(librbd::MockTestImageCtx* image_ctx)
{
...
template<> struct WriteImageStateRequest<MockTestImageCtx>
{
    uint64_t snap_id = CEPH_NOSNAP;
    ImageState image_state;
    Context* on_finish = nullptr;
    static WriteImageStateRequest* s_instance;
    static WriteImageStateRequest* create(MockTestImageCtx* image_ctx, uint64_t snap_id, const ImageState& image_state,
                                          Context* on_finish)
    {
    ...
    WriteImageStateRequest() { s_instance = this; }
};
...
public:
    ...
    void expect_refresh_image(MockTestImageCtx& mock_image_ctx, bool refresh_required, int r)
    {
    ...
    void expect_get_mirror_image(MockTestImageCtx& mock_image_ctx, const cls::rbd::MirrorImage& mirror_image, int r)
    {
        using ceph::encode;
        bufferlist bl;
    ...
    void expect_can_create_non_primary_snapshot(MockUtils& mock_utils, bool result)
    {
        EXPECT_CALL(mock_utils, can_create_non_primary_snapshot(_)).WillOnce(Return(result));
    }
    ...
    void expect_write_image_state(MockTestImageCtx& mock_image_ctx,
                                  MockWriteImageStateRequest& mock_write_image_state_request, int r)
    {
        ...
        EXPECT_CALL(mock_write_image_state_request, send())
            .WillOnce(Invoke([&mock_image_ctx, &mock_write_image_state_request, r]() {
                mock_image_ctx.image_ctx->op_work_queue->queue(mock_write_image_state_request.on_finish, r);
            }));
    }
};
...
TEST_F(TestMockMirrorSnapshotCreateNonPrimaryRequest, Success)
{
    ...
    expect_get_mirror_image(
        mock_image_ctx, {cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, "gid", cls::rbd::MIRROR_IMAGE_STATE_ENABLED}, 0);
    ...
    C_SaferCond ctx;
    auto req = new MockCreateNonPrimaryRequest(&mock_image_ctx, false, "mirror_uuid", 123, {{1, 2}}, {}, nullptr, &ctx);
    req->send();
...
TEST_F(TestMockMirrorSnapshotCreateNonPrimaryRequest, SuccessDemoted)
{
    ...
    C_SaferCond ctx;
    auto req = new MockCreateNonPrimaryRequest(&mock_image_ctx, true, "mirror_uuid", 123, {{1, 2}}, {}, nullptr, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(TestMockMirrorSnapshotCreateNonPrimaryRequest, GetMirrorImageError)
{
    ...
    expect_get_mirror_image(
        mock_image_ctx, {cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, "gid", cls::rbd::MIRROR_IMAGE_STATE_ENABLED}, -EINVAL);
...
TEST_F(TestMockMirrorSnapshotCreateNonPrimaryRequest, WriteImageStateError)
{
    ...
    expect_create_snapshot(mock_image_ctx, 0);
    MockWriteImageStateRequest mock_write_image_state_request;
    expect_write_image_state(mock_image_ctx, mock_write_image_state_request, -EINVAL);
