...
template bool librbd::mirror::snapshot::util::can_create_primary_snapshot(librbd::MockTestImageCtx* image_ctx,
                                                                          bool demoted, bool force,
                                                                          bool* requires_orphan,
                                                                          uint64_t* rollback_snap_id);
template bool librbd::mirror::snapshot::util::can_create_non_primary_snapshot(librbd::MockTestImageCtx* image_ctx);
...
TEST_F(TestMockMirrorSnapshotUtils, CanCreatePrimarySnapshot)
{
    ...
    bool requires_orphan;
    uint64_t rollback_snap_id;
    ASSERT_TRUE(util::can_create_primary_snapshot(&mock_image_ctx, false, false, &requires_orphan, &rollback_snap_id));
    ASSERT_FALSE(requires_orphan);
    ASSERT_EQ(rollback_snap_id, CEPH_NOSNAP);

    cls::rbd::MirrorSnapshotNamespace nns{cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY, {}, "mirror_uuid", 123};
    nns.complete = true;
    auto copied_snap_id = snap_create(mock_image_ctx, nns, "NPS1");

    // without force, previous snapshot is non-primary
    ASSERT_FALSE(util::can_create_primary_snapshot(&mock_image_ctx, false, false, nullptr, nullptr));

    // demoted, previous snapshot is non-primary
    ASSERT_FALSE(util::can_create_primary_snapshot(&mock_image_ctx, true, true, nullptr, nullptr));

    // previous non-primary snapshot is copied
    ASSERT_TRUE(util::can_create_primary_snapshot(&mock_image_ctx, false, true, &requires_orphan, &rollback_snap_id));
    ...
    ASSERT_FALSE(util::can_create_primary_snapshot(&mock_image_ctx, false, true, nullptr, nullptr));

    // can rollback
    ASSERT_TRUE(util::can_create_primary_snapshot(&mock_image_ctx, false, true, nullptr, &rollback_snap_id));
    ...
    ASSERT_TRUE(util::can_create_primary_snapshot(&mock_image_ctx, false, true, nullptr, nullptr));

    cls::rbd::MirrorSnapshotNamespace pns{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED, {"uuid"}, "", CEPH_NOSNAP};
    ...
    ASSERT_TRUE(util::can_create_primary_snapshot(&mock_image_ctx, false, false, nullptr, nullptr));
}
...
TEST_F(TestMockMirrorSnapshotUtils, CanCreateNonPrimarySnapshot)
{
    ...
    cls::rbd::MirrorSnapshotNamespace pns{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {"uuid"}, "", CEPH_NOSNAP};
    snap_create(mock_image_ctx, pns, "PS1");
