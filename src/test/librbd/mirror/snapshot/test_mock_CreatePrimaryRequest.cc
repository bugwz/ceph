...
struct Mock
{
    ...
    MOCK_METHOD4(can_create_primary_snapshot, bool(librbd::MockTestImageCtx*, bool, bool, uint64_t*));
};
...
template<>
bool can_create_primary_snapshot(librbd::MockTestImageCtx* image_ctx, bool demoted, bool force, bool* requires_orphan,
                                 uint64_t* rollback_snap_id)
{
    return Mock::s_instance->can_create_primary_snapshot(image_ctx, demoted, force, rollback_snap_id);
}
...
public:
    ...
    void snap_create(MockTestImageCtx& mock_image_ctx, const cls::rbd::SnapshotNamespace& ns,
                     const std::string& snap_name)
    {
        ASSERT_TRUE(mock_image_ctx.snap_info.insert({m_snap_seq++, SnapInfo{snap_name, ns, 0, {}, 0, 0, {}}}).second);
    }

    void expect_clone_md_ctx(MockTestImageCtx& mock_image_ctx)
    {
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx), clone()).WillOnce(Invoke([&mock_image_ctx]() {
            get_mock_io_ctx(mock_image_ctx.md_ctx).get();
            return &get_mock_io_ctx(mock_image_ctx.md_ctx);
        }));
    }

    void expect_can_create_primary_snapshot(MockUtils& mock_utils, bool demoted, bool force, bool result)
    {
        EXPECT_CALL(mock_utils, can_create_primary_snapshot(_, demoted, force, nullptr)).WillOnce(Return(result));
    }

    void expect_get_mirror_peers(MockTestImageCtx& mock_image_ctx, const std::vector<cls::rbd::MirrorPeer>& peers,
                                 int r)
    {
        using ceph::encode;
        bufferlist bl;
        ...
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
                    exec(RBD_MIRRORING, _, StrEq("rbd"), StrEq("mirror_peer_list"), _, _, _, _))
            .WillOnce(DoAll(WithArg<5>(CopyInBufferlist(bl)), Return(r)));
    }
    ...
    void expect_create_snapshot(MockTestImageCtx& mock_image_ctx, int r)
    {
        EXPECT_CALL(*mock_image_ctx.operations, snap_create(_, _, _, _, _))
            .WillOnce(DoAll(Invoke([this, &mock_image_ctx, r](const cls::rbd::SnapshotNamespace& ns,
                                                              const std::string& snap_name,
                                                              uint64_t flags,
                                ...
                                auto mirror_ns = boost::get<cls::rbd::MirrorSnapshotNamespace>(ns);
                                mirror_ns.complete = true;
                                snap_create(mock_image_ctx, mirror_ns, snap_name);
                            }),
                            WithArg<4>(CompleteContext(r, mock_image_ctx.image_ctx->op_work_queue))));
    }
    ...
    void expect_unlink_peer(MockTestImageCtx& mock_image_ctx, MockUnlinkPeerRequest& mock_unlink_peer_request,
                            uint64_t snap_id, const std::string& peer_uuid, bool is_linked, bool complete,
                            bool allow_remove, int r)
    {
        EXPECT_CALL(mock_unlink_peer_request, send())
            .WillOnce(Invoke([&mock_image_ctx,
                              ...
                              allow_remove,
                              r]() {
                ASSERT_EQ(mock_unlink_peer_request.mirror_peer_uuid, peer_uuid);
                ...
                if (r == 0) {
                    ...
                    ASSERT_EQ(is_linked, info->mirror_peer_uuids.erase(peer_uuid));
                    if (info->mirror_peer_uuids.empty()) {
                        mock_image_ctx.snap_info.erase(it);
                    }
                }
                mock_image_ctx.image_ctx->op_work_queue->queue(mock_unlink_peer_request.on_finish, r);
            }));
    }
};
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, Success)
{
    ...
    expect_get_mirror_peers(
        mock_image_ctx, {{"uuid", cls::rbd::MIRROR_PEER_DIRECTION_TX, "ceph", "mirror", "mirror uuid"}}, 0);
    expect_create_snapshot(mock_image_ctx, 0);
    expect_refresh_image(mock_image_ctx, 0);

    C_SaferCond ctx;
    auto req = new MockCreatePrimaryRequest(&mock_image_ctx, "gid", CEPH_NOSNAP, 0U, 0U, nullptr, &ctx);
    req->send();
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessPrimary)
{
    ...
    MockTestImageCtx mock_image_ctx(*ictx);
    cls::rbd::MirrorSnapshotNamespace ns{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {"uuid"}, "", CEPH_NOSNAP};
    ns.complete = true;
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessPrimaryDemoted)
{
    ...
    MockTestImageCtx mock_image_ctx(*ictx);
    cls::rbd::MirrorSnapshotNamespace ns{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED, {"uuid"}, "", CEPH_NOSNAP};
    ns.complete = true;
    snap_create(mock_image_ctx, ns, "mirror_snap");
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessNonPrimaryDemoted)
{
    ...
    cls::rbd::MirrorSnapshotNamespace ns{
        cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY_DEMOTED, {"uuid"}, "mirror uuid", 123};
    ns.complete = true;
    snap_create(mock_image_ctx, ns, "mirror_snap");
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessPrimaryBelowMaxSnapshotsReset)
{
    ...
    for (int i = 0; i < 6; i++) {
        cls::rbd::MirrorSnapshotNamespace ns{
            (i == 3 ? cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED : cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY),
            {"uuid"},
            "",
            CEPH_NOSNAP};
        ns.complete = true;
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, GetMirrorPeersError)
{
    ...
    expect_can_create_primary_snapshot(mock_utils, false, false, true);
    expect_get_mirror_peers(
        mock_image_ctx, {{"uuid", cls::rbd::MIRROR_PEER_DIRECTION_TX, "ceph", "mirror", "mirror uuid"}}, -EINVAL);
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessUnlinkPrimaryNoPeer)
{
    ...
    MockTestImageCtx mock_image_ctx(*ictx);
    cls::rbd::MirrorSnapshotNamespace ns{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {}, "", CEPH_NOSNAP};
    ns.complete = true;
    snap_create(mock_image_ctx, ns, "mirror_snap");
    ...
    expect_unlink_peer(mock_image_ctx, mock_unlink_peer_request, snap_id, "uuid", false, true, true, 0);
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessUnlinkPrimaryDemotedNoPeer)
{
    ...
    MockTestImageCtx mock_image_ctx(*ictx);
    cls::rbd::MirrorSnapshotNamespace ns{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED, {}, "", CEPH_NOSNAP};
    ns.complete = true;
    snap_create(mock_image_ctx, ns, "mirror_snap");
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessUnlinkNonPrimaryDemotedNoPeer)
{
    ...
    MockTestImageCtx mock_image_ctx(*ictx);
    cls::rbd::MirrorSnapshotNamespace ns{cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY_DEMOTED, {}, "mirror uuid", 123};
    ns.complete = true;
    snap_create(mock_image_ctx, ns, "mirror_snap");
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessUnlinkOrphanNoPeer)
{
    ...
    MockTestImageCtx mock_image_ctx(*ictx);
    cls::rbd::MirrorSnapshotNamespace ns{cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY, {}, "", CEPH_NOSNAP};
    ns.complete = true;
    snap_create(mock_image_ctx, ns, "mirror_snap");
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessUnlinkPrimaryIncomplete)
{
    ...
    auto it = mock_image_ctx.snap_info.rbegin();
    auto snap_id = it->first;
    expect_unlink_peer(mock_image_ctx, mock_unlink_peer_request, snap_id, "uuid", true, false, true, 0);
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessUnlinkPrimaryMaxSnapshots)
{
    ...
    auto it = mock_image_ctx.snap_info.rbegin();
    auto snap_id = it->first;
    expect_unlink_peer(mock_image_ctx, mock_unlink_peer_request, snap_id, "uuid", true, true, true, 0);
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessUnlinkMultiplePeers)
{
    ...
    for (int i = 0; i < 3; i++) {
        cls::rbd::MirrorSnapshotNamespace ns{
            cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {"uuid1", "uuid2"}, "", CEPH_NOSNAP};
        ns.complete = true;
        snap_create(mock_image_ctx, ns, "mirror_snap");
    }
    ...
    expect_get_mirror_peers(mock_image_ctx,
                            {{"uuid1", cls::rbd::MIRROR_PEER_DIRECTION_TX, "ceph", "mirror", "mirror uuid"},
                             {"uuid2", cls::rbd::MIRROR_PEER_DIRECTION_TX, "ceph", "mirror", "mirror uuid"}},
                            0);
    expect_create_snapshot(mock_image_ctx, 0);
    expect_refresh_image(mock_image_ctx, 0);
    MockUnlinkPeerRequest mock_unlink_peer_request;
    auto it = mock_image_ctx.snap_info.rbegin();
    auto snap_id = it->first;
    expect_unlink_peer(mock_image_ctx, mock_unlink_peer_request, snap_id, "uuid1", true, true, true, 0);
    expect_unlink_peer(mock_image_ctx, mock_unlink_peer_request, snap_id, "uuid2", true, true, true, 0);
...
TEST_F(TestMockMirrorSnapshotCreatePrimaryRequest, SuccessUnlinkMultipleSnapshots)
{
    ...
    MockTestImageCtx mock_image_ctx(*ictx);
    cls::rbd::MirrorSnapshotNamespace ns1{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {}, "", CEPH_NOSNAP};
    ns1.complete = true;
    snap_create(mock_image_ctx, ns1, "mirror_snap");
    cls::rbd::MirrorSnapshotNamespace ns2{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED, {}, "", CEPH_NOSNAP};
    ns2.complete = true;
