...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, Success)
{
    ...
    C_SaferCond ctx;
    auto req = new MockUnlinkPeerRequest(&mock_image_ctx, snap_id, "peer1_uuid", &ctx);
    req->send();
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, RemoveSnapshot)
{
    ...
    C_SaferCond ctx;
    auto req = new MockUnlinkPeerRequest(&mock_image_ctx, snap_id, "peer_uuid", &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, PeerDNE)
{
    ...
    C_SaferCond ctx;
    auto req = new MockUnlinkPeerRequest(&mock_image_ctx, snap_id, "unknown_peer", &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
