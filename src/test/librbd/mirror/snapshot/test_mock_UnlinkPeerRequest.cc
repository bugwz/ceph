...
public:
    ...
    uint64_t snap_create(MockTestImageCtx& mock_image_ctx, const cls::rbd::SnapshotNamespace& ns,
                         const std::string& snap_name)
    {
        EXPECT_TRUE(mock_image_ctx.snap_info.insert({++m_snap_seq, SnapInfo{snap_name, ns, 0, {}, 0, 0, {}}}).second);
        return m_snap_seq;
    }

    void expect_get_snap_info(MockTestImageCtx& mock_image_ctx, librados::snap_t snap_id)
    {
        EXPECT_CALL(mock_image_ctx, get_snap_info(snap_id))
            .WillRepeatedly(Invoke([&mock_image_ctx](librados::snap_t snap_id) -> librbd::SnapInfo* {
    ...
    void expect_unlink_peer(MockTestImageCtx& mock_image_ctx, uint64_t snap_id, const std::string& peer_uuid, int r)
    {
        using ceph::encode;
        bufferlist bl;
        ...
        EXPECT_CALL(get_mock_io_ctx(mock_image_ctx.md_ctx),
            ...
            .WillOnce(Invoke([&mock_image_ctx, snap_id, peer_uuid, r](auto&&... args) -> int {
                if (r == 0) {
                    ...
                    EXPECT_NE(0, info->mirror_peer_uuids.erase(peer_uuid));
                }
                return r;
            }));
    }
    ...
    void expect_remove_snapshot(MockTestImageCtx& mock_image_ctx, uint64_t snap_id, int r)
    {
        EXPECT_CALL(*mock_image_ctx.operations, snap_remove(_, _, _))
            .WillOnce(Invoke([&mock_image_ctx, snap_id, r](const cls::rbd::SnapshotNamespace& snap_namespace,
                                                           const std::string& snap_name,
                                                           Context* on_finish) {
                if (r == 0) {
                    auto it = mock_image_ctx.snap_info.find(snap_id);
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, Success)
{
    ...
    cls::rbd::MirrorSnapshotNamespace ns{
        cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {"peer1_uuid", "peer2_uuid"}, "", CEPH_NOSNAP};
    ...
    C_SaferCond ctx;
    auto req = new MockUnlinkPeerRequest(&mock_image_ctx, snap_id, "peer1_uuid", true, &ctx);
    req->send();
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, RemoveSnapshot)
{
    ...
    MockTestImageCtx mock_image_ctx(*ictx);
    cls::rbd::MirrorSnapshotNamespace ns{cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY, {"peer_uuid"}, "", CEPH_NOSNAP};
    auto snap_id = snap_create(mock_image_ctx, ns, "mirror_snap");
    ...
    C_SaferCond ctx;
    auto req = new MockUnlinkPeerRequest(&mock_image_ctx, snap_id, "peer_uuid", true, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, RemoveSnapshotNotAllowed)
{
    ...
    C_SaferCond ctx;
    auto req = new MockUnlinkPeerRequest(&mock_image_ctx, snap_id, "peer_uuid", false, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, SnapshotDNE)
{
    ...
    C_SaferCond ctx;
    auto req = new MockUnlinkPeerRequest(&mock_image_ctx, 123, "peer_uuid", true, &ctx);
    req->send();
...
TEST_F(TestMockMirrorSnapshotUnlinkPeerRequest, PeerDNE)
{
    ...
    C_SaferCond ctx;
    auto req = new MockUnlinkPeerRequest(&mock_image_ctx, snap_id, "unknown_peer", true, &ctx);
    req->send();
    ASSERT_EQ(0, ctx.wait());
}
