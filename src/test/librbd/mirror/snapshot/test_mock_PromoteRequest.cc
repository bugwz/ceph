...
template<> struct ListWatchersRequest<MockTestImageCtx>
{
    std::list<obj_watch_t>* watchers;
    Context* on_finish = nullptr;
    static ListWatchersRequest* s_instance;
    static ListWatchersRequest* create(MockTestImageCtx& image_ctx, int flags, std::list<obj_watch_t>* watchers,
                                       Context* on_finish)
    {
    ...
    ListWatchersRequest() { s_instance = this; }
};
...
struct Mock
{
    ...
    MOCK_METHOD5(can_create_primary_snapshot, bool(librbd::MockTestImageCtx*, bool, bool, bool*, uint64_t*));
};
...
{
    return Mock::s_instance->can_create_primary_snapshot(image_ctx, demoted, force, requires_orphan, rollback_snap_id);
}
...
template<> struct CreateNonPrimaryRequest<MockTestImageCtx>
{
    ...
    static CreateNonPrimaryRequest* create(MockTestImageCtx* image_ctx, bool demoted,
                                           ...
                                           SnapSeqs snap_seqs, const ImageState& image_state, uint64_t* snap_id,
                                           Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
...
template<> struct CreatePrimaryRequest<MockTestImageCtx>
{
    bool demoted = false;
    bool force = false;
    Context* on_finish = nullptr;
    static CreatePrimaryRequest* s_instance;
    static CreatePrimaryRequest* create(MockTestImageCtx* image_ctx, const std::string& global_image_id,
    ...
    CreatePrimaryRequest() { s_instance = this; }
};
...
public:
    ...
    void expect_can_create_primary_snapshot(MockUtils& mock_utils, bool force, bool requires_orphan,
                                            uint64_t rollback_snap_id, bool result)
    {
        EXPECT_CALL(mock_utils, can_create_primary_snapshot(_, false, force, _, _))
            .WillOnce(DoAll(WithArgs<3, 4>(Invoke([requires_orphan, rollback_snap_id](bool* orphan, uint64_t* snap_id) {
    ...
    void expect_create_orphan_snapshot(MockTestImageCtx& mock_image_ctx,
                                       MockCreateNonPrimaryRequest& mock_create_non_primary_request, int r)
    {
        EXPECT_CALL(mock_create_non_primary_request, send())
            .WillOnce(Invoke([&mock_image_ctx, &mock_create_non_primary_request, r]() {
                mock_image_ctx.image_ctx->op_work_queue->queue(mock_create_non_primary_request.on_finish, r);
            }));
    }

    void expect_list_watchers(MockTestImageCtx& mock_image_ctx, MockListWatchersRequest& mock_list_watchers_request,
                              const std::list<obj_watch_t>& watchers, int r)
    {
        EXPECT_CALL(mock_list_watchers_request, send())
            .WillOnce(Invoke([&mock_image_ctx, &mock_list_watchers_request, watchers, r]() {
                *mock_list_watchers_request.watchers = watchers;
                mock_image_ctx.image_ctx->op_work_queue->queue(mock_list_watchers_request.on_finish, r);
            }));
    }
    ...
    void expect_acquire_lock(MockTestImageCtx& mock_image_ctx, int r)
    {
        ...
        EXPECT_CALL(*mock_image_ctx.exclusive_lock, is_lock_owner()).WillOnce(Return(false));
        ...
        if (r == 0) {
            EXPECT_CALL(*mock_image_ctx.exclusive_lock, is_lock_owner()).WillOnce(Return(true));
        }
    }

    void expect_get_snap_info(MockTestImageCtx& mock_image_ctx, uint64_t snap_id, const SnapInfo* snap_info)
    {
        EXPECT_CALL(mock_image_ctx, get_snap_info(snap_id)).WillOnce(Return(snap_info));
    }

    void expect_rollback(MockTestImageCtx& mock_image_ctx, uint64_t snap_id, const SnapInfo* snap_info, int r)
    {
        expect_get_snap_info(mock_image_ctx, snap_id, snap_info);
        EXPECT_CALL(*mock_image_ctx.operations, execute_snap_rollback(snap_info->snap_namespace, snap_info->name, _, _))
            .WillOnce(WithArg<3>(CompleteContext(r, mock_image_ctx.image_ctx->op_work_queue)));
    }

    void expect_create_promote_snapshot(MockTestImageCtx& mock_image_ctx,
                                        MockCreatePrimaryRequest& mock_create_primary_request, int r)
    {
        EXPECT_CALL(mock_create_primary_request, send())
            .WillOnce(Invoke([&mock_image_ctx, &mock_create_primary_request, r]() {
                mock_image_ctx.image_ctx->op_work_queue->queue(mock_create_primary_request.on_finish, r);
            }));
    }
...
TEST_F(TestMockMirrorSnapshotPromoteRequest, Success)
{
    ...
    expect_can_create_primary_snapshot(mock_utils, true, false, CEPH_NOSNAP, true);
    MockCreatePrimaryRequest mock_create_primary_request;
    expect_create_promote_snapshot(mock_image_ctx, mock_create_primary_request, 0);
    C_SaferCond ctx;
...
TEST_F(TestMockMirrorSnapshotPromoteRequest, SuccessForce)
{
    ...
    expect_create_orphan_snapshot(mock_image_ctx, mock_create_non_primary_request, 0);
    ...
    SnapInfo snap_info = {"snap", cls::rbd::MirrorSnapshotNamespace{}, 0, {}, 0, 0, {}};
    MockCreatePrimaryRequest mock_create_primary_request;
    expect_create_promote_snapshot(mock_image_ctx, mock_create_primary_request, 0);
...
TEST_F(TestMockMirrorSnapshotPromoteRequest, ErrorCannotRollback)
{
    ...
    MockUtils mock_utils;
    expect_can_create_primary_snapshot(mock_utils, true, false, CEPH_NOSNAP, false);
