...
template<> struct PromoteRequest<librbd::MockTestImageCtx>
{
    Context* on_finish = nullptr;
    static PromoteRequest* s_instance;
    static PromoteRequest* create(librbd::MockTestImageCtx*, bool force, Context* on_finish)
    {
    ...
    PromoteRequest() { s_instance = this; }
...
template<> struct GetInfoRequest<librbd::MockTestImageCtx>
{
    ...
    static GetInfoRequest* create(librbd::MockTestImageCtx&, cls::rbd::MirrorImage* mirror_image,
    ...
    GetInfoRequest() { s_instance = this; }
...
template<> struct ImageRemoveRequest<librbd::MockTestImageCtx>
{
    ...
    ImageRemoveRequest() { s_instance = this; }
};

template<> struct ImageStateUpdateRequest<librbd::MockTestImageCtx>
{
...
ImageStateUpdateRequest<librbd::MockTestImageCtx>* ImageStateUpdateRequest<librbd::MockTestImageCtx>::s_instance =
    nullptr;
...
template<> struct PromoteRequest<librbd::MockTestImageCtx>
{
    Context* on_finish = nullptr;
    static PromoteRequest* s_instance;
    static PromoteRequest* create(librbd::MockTestImageCtx*, const std::string& global_image_id, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->on_finish = on_finish;
        return s_instance;
...
public:
    ...
    void expect_get_mirror_info(MockTestImageCtx& mock_image_ctx, MockGetInfoRequest& mock_get_info_request,
                                const cls::rbd::MirrorImage& mirror_image, PromotionState promotion_state, int r)
    {

        EXPECT_CALL(mock_get_info_request, send())
            .WillOnce(Invoke([&mock_image_ctx, &mock_get_info_request, mirror_image, promotion_state, r]() {
                if (r == 0) {
                    *mock_get_info_request.mirror_image = mirror_image;
                    *mock_get_info_request.promotion_state = promotion_state;
                }
                mock_image_ctx.op_work_queue->queue(mock_get_info_request.on_finish, r);
            }));
    }

    void expect_mirror_image_state_update(MockTestImageCtx& mock_image_ctx, MockImageStateUpdateRequest& mock_request,
                                          int r)
    {
        EXPECT_CALL(mock_request, send()).WillOnce(Invoke([&mock_image_ctx, &mock_request, r]() {
            mock_image_ctx.op_work_queue->queue(mock_request.on_finish, r);
        }));
    }

    void expect_mirror_image_remove(MockTestImageCtx& mock_image_ctx, MockImageRemoveRequest& mock_request, int r)
    {
        EXPECT_CALL(mock_request, send()).WillOnce(Invoke([&mock_image_ctx, &mock_request, r]() {
            mock_image_ctx.op_work_queue->queue(mock_request.on_finish, r);
        }));
    ...
    void expect_journal_client_list(MockTestImageCtx& mock_image_ctx, const std::set<cls::journal::Client>& clients,
                                    int r)
    {
        bufferlist bl;
        using ceph::encode;
    ...
    void expect_journal_client_unregister(MockTestImageCtx& mock_image_ctx, const std::string& client_id, int r)
    {
        bufferlist bl;
        using ceph::encode;
        encode(client_id, bl);
    ...
    void expect_journal_promote(MockTestImageCtx& mock_image_ctx, MockJournalPromoteRequest& mock_promote_request,
                                int r)
    {
        EXPECT_CALL(mock_promote_request, send()).WillOnce(FinishRequest(&mock_promote_request, r, &mock_image_ctx));
    }

    void expect_snapshot_promote(MockTestImageCtx& mock_image_ctx, MockSnapshotPromoteRequest& mock_promote_request,
                                 int r)
    {
        EXPECT_CALL(mock_promote_request, send()).WillOnce(FinishRequest(&mock_promote_request, r, &mock_image_ctx));
    }

    void expect_is_refresh_required(MockTestImageCtx& mock_image_ctx, bool refresh_required)
    {
        EXPECT_CALL(*mock_image_ctx.state, is_refresh_required()).WillOnce(Return(refresh_required));
    }
    ...
    void expect_snap_remove(MockTestImageCtx& mock_image_ctx, const std::string& snap_name, int r)
    {
    ...
    template<typename T> bufferlist encode(const T& t)
    {
        using ceph::encode;
        bufferlist bl;
        encode(t, bl);
...
TEST_F(TestMockMirrorDisableRequest, Success)
{
    ...
    expect_get_mirror_info(mock_image_ctx,
                           mock_get_info_request,
                           {cls::rbd::MIRROR_IMAGE_MODE_JOURNAL, "global id", cls::rbd::MIRROR_IMAGE_STATE_ENABLED},
                           PROMOTION_STATE_PRIMARY,
                           0);
    MockImageStateUpdateRequest mock_image_state_update_request;
    expect_mirror_image_state_update(mock_image_ctx, mock_image_state_update_request, 0);
    expect_journal_client_list(mock_image_ctx,
                               {{"", encode(journal::ClientData{journal::ImageClientMeta{}})},
                                {"peer 1", encode(journal::ClientData{journal::MirrorPeerClientMeta{}})},
                                {"peer 2",
                                 encode(journal::ClientData{journal::MirrorPeerClientMeta{
                                     "remote image id",
                                     {{cls::rbd::UserSnapshotNamespace(), "snap 1", boost::optional<uint64_t>(0)},
    ...
    expect_mirror_image_remove(mock_image_ctx, mock_image_remove_request, 0);
...
TEST_F(TestMockMirrorDisableRequest, MirrorImageSetError)
{
    ...
    expect_get_mirror_info(mock_image_ctx,
                           ...
                           0);
    MockImageStateUpdateRequest mock_image_state_update_request;
    expect_mirror_image_state_update(mock_image_ctx, mock_image_state_update_request, -ENOENT);
...
TEST_F(TestMockMirrorDisableRequest, SnapshotPromoteError)
{
    ...
    expect_get_mirror_info(mock_image_ctx,
                           mock_get_info_request,
                           {cls::rbd::MIRROR_IMAGE_MODE_SNAPSHOT, "global id", cls::rbd::MIRROR_IMAGE_STATE_ENABLED},
                           PROMOTION_STATE_NON_PRIMARY,
                           0);
    MockImageStateUpdateRequest mock_image_state_update_request;
    expect_mirror_image_state_update(mock_image_ctx, mock_image_state_update_request, 0);
...
TEST_F(TestMockMirrorDisableRequest, MirrorImageRemoveError)
{
    ...
    expect_journal_client_list(mock_image_ctx, {}, 0);
    MockImageRemoveRequest mock_image_remove_request;
    expect_mirror_image_remove(mock_image_ctx, mock_image_remove_request, -EINVAL);
