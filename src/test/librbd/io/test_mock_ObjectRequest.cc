...
template<>
void file_to_extents(MockTestImageCtx* image_ctx, uint64_t offset, uint64_t length, uint64_t buffer_offset,
...
template<>
void extent_to_file(MockTestImageCtx* image_ctx, uint64_t object_no, uint64_t offset, uint64_t length,
                    std::vector<std::pair<uint64_t, uint64_t>>& extents)
{
    Striper::extent_to_file(image_ctx->cct, &image_ctx->layout, object_no, offset, length, extents);
}
...
TEST_F(TestMockIoObjectRequest, ListSnapsWholeObject)
{
...

    {
        ...
        expected_snapshot_delta[{CEPH_NOSNAP, CEPH_NOSNAP}].insert(
            1,
            mock_image_ctx.layout.object_size - 2,
...

    {
        ...
        expected_snapshot_delta[{CEPH_NOSNAP, CEPH_NOSNAP}].insert(
            0,
            mock_image_ctx.layout.object_size - 1,
