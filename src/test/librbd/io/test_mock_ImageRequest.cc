...
{
    MOCK_METHOD3(append_write_event, uint64_t(const io::Extents&, const bufferlist&, bool));
    MOCK_METHOD3(append_write_same_event, uint64_t(const io::Extents&, const bufferlist&, bool));
    MOCK_METHOD3(append_discard_event, uint64_t(const io::Extents&, uint32_t, bool));
    MOCK_METHOD5(append_io_event_mock, uint64_t(const journal::EventEntry&, uint64_t, size_t, bool, int));
    uint64_t append_io_event(journal::EventEntry&& event_entry, uint64_t offset, size_t length, bool flush_entry,
                             int filter_ret_val)
    {
        // googlemock doesn't support move semantics
        return append_io_event_mock(event_entry, offset, length, flush_entry, filter_ret_val);
    }
...
struct TestMockIoImageRequest : public TestMockFixture
{
    ...
    void expect_get_modify_timestamp(MockTestImageCtx& mock_image_ctx, bool needs_update)
    {
        if (needs_update) {
            mock_image_ctx.mtime_update_interval = 5;
            EXPECT_CALL(mock_image_ctx, get_modify_timestamp()).WillOnce(Return(ceph_clock_now() - utime_t(10, 0)));
        }
        else {
            mock_image_ctx.mtime_update_interval = 600;
            EXPECT_CALL(mock_image_ctx, get_modify_timestamp()).WillOnce(Return(ceph_clock_now()));
        }
    }

    void expect_journal_append_discard_event(MockTestJournal& mock_journal, uint64_t journal_tid,
                                             const io::Extents& extents)
    {
        EXPECT_CALL(mock_journal, append_discard_event(extents, _, _)).WillOnce(Return(journal_tid));
    }

    void expect_object_discard_request(MockTestImageCtx& mock_image_ctx, uint64_t object_no, uint64_t offset,
                                       uint32_t length, int r)
    {
        EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, send(_))
            .WillOnce(Invoke([&mock_image_ctx, object_no, offset, length, r](ObjectDispatchSpec* spec) {
    ...
    void expect_object_request_send(MockTestImageCtx& mock_image_ctx, int r)
    {
        EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, send(_))
    ...
    void expect_object_list_snaps_request(MockTestImageCtx& mock_image_ctx, uint64_t object_no,
                                          const SnapshotDelta& snap_delta, int r)
    {
        EXPECT_CALL(*mock_image_ctx.io_object_dispatcher, send(_))
            .WillOnce(Invoke([&mock_image_ctx, object_no, snap_delta, r](ObjectDispatchSpec* spec) {
                auto request = boost::get<librbd::io::ObjectDispatchSpec::ListSnapsRequest>(&spec->request);
...
TEST_F(TestMockIoImageRequest, AioWriteModifyTimestamp)
{
    ...
    EXPECT_CALL(mock_image_ctx, set_modify_timestamp(_)).Times(Exactly(1));
    ...
    AioCompletion* aio_comp_1 = AioCompletion::create_and_start(&aio_comp_ctx_1, ictx, AIO_TYPE_WRITE);

    AioCompletion* aio_comp_2 = AioCompletion::create_and_start(&aio_comp_ctx_2, ictx, AIO_TYPE_WRITE);

    bufferlist bl;
    bl.append("1");
    MockImageWriteRequest mock_aio_image_write_1(mock_image_ctx, aio_comp_1, {{0, 1}}, std::move(bl), 0, {});
    {
    ...
    bl.append("1");
    MockImageWriteRequest mock_aio_image_write_2(mock_image_ctx, aio_comp_2, {{0, 1}}, std::move(bl), 0, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
...
TEST_F(TestMockIoImageRequest, AioReadAccessTimestamp)
{
    ...
    EXPECT_CALL(mock_image_ctx, set_access_timestamp(_)).Times(Exactly(1));

    InSequence seq;
    expect_object_request_send(mock_image_ctx, 0);

    C_SaferCond aio_comp_ctx_1, aio_comp_ctx_2;
    AioCompletion* aio_comp_1 = AioCompletion::create_and_start(&aio_comp_ctx_1, ictx, AIO_TYPE_READ);


    ReadResult rr;
    MockImageReadRequest mock_aio_image_read_1(
        mock_image_ctx, aio_comp_1, {{0, 1}}, std::move(rr), mock_image_ctx.get_data_io_context(), 0, 0, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
    ...
    AioCompletion* aio_comp_2 = AioCompletion::create_and_start(&aio_comp_ctx_2, ictx, AIO_TYPE_READ);
    expect_object_request_send(mock_image_ctx, 0);

    MockImageReadRequest mock_aio_image_read_2(
        mock_image_ctx, aio_comp_2, {{0, 1}}, std::move(rr), mock_image_ctx.get_data_io_context(), 0, 0, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
...
TEST_F(TestMockIoImageRequest, PartialDiscard)
{
    ...
    MockImageDiscardRequest mock_aio_image_discard(
        mock_image_ctx, aio_comp, {{16, 63}, {84, 100}}, ictx->discard_granularity_bytes, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
...
TEST_F(TestMockIoImageRequest, TailDiscard)
{
    ...
    InSequence seq;
    expect_get_modify_timestamp(mock_image_ctx, false);
    expect_object_discard_request(mock_image_ctx, 0, ictx->layout.object_size - 1024, 1024, 0);

    C_SaferCond aio_comp_ctx;
    AioCompletion* aio_comp = AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_DISCARD);
    MockImageDiscardRequest mock_aio_image_discard(
        mock_image_ctx, aio_comp, {{ictx->layout.object_size - 1024, 1024}}, ictx->discard_granularity_bytes, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        mock_aio_image_discard.send();
    }
...
TEST_F(TestMockIoImageRequest, DiscardGranularity)
{
    ...
    C_SaferCond aio_comp_ctx;
    AioCompletion* aio_comp = AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_DISCARD);
    MockImageDiscardRequest mock_aio_image_discard(mock_image_ctx,
                                                   aio_comp,
...
TEST_F(TestMockIoImageRequest, PartialDiscardJournalAppendEnabled)
{
    ...
    expect_journal_append_discard_event(mock_journal, 0, {{16, 63}, {84, 100}});
    expect_object_discard_request(mock_image_ctx, 0, 16, 63, 0);
    expect_object_discard_request(mock_image_ctx, 0, 84, 100, 0);
...
TEST_F(TestMockIoImageRequest, TailDiscardJournalAppendEnabled)
{
    ...
    expect_get_modify_timestamp(mock_image_ctx, false);
    expect_is_journal_appending(mock_journal, true);
    expect_journal_append_discard_event(mock_journal, 0, {{ictx->layout.object_size - 1024, 1024}});
    expect_object_discard_request(mock_image_ctx, 0, ictx->layout.object_size - 1024, 1024, 0);
...
TEST_F(TestMockIoImageRequest, DiscardGranularityJournalAppendEnabled)
{
    ...
    expect_get_modify_timestamp(mock_image_ctx, false);
    expect_is_journal_appending(mock_journal, true);
    expect_journal_append_discard_event(mock_journal, 0, {{32, 32}, {96, 64}, {ictx->layout.object_size - 32, 32}});
    expect_object_discard_request(mock_image_ctx, 0, 32, 32, 0);
    expect_object_discard_request(mock_image_ctx, 0, 96, 64, 0);
    expect_object_discard_request(mock_image_ctx, 0, ictx->layout.object_size - 32, 32, 0);
...
TEST_F(TestMockIoImageRequest, AioWriteJournalAppendDisabled)
{
    ...
    C_SaferCond aio_comp_ctx;
    AioCompletion* aio_comp = AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_WRITE);

    bufferlist bl;
    bl.append("1");
    MockImageWriteRequest mock_aio_image_write(mock_image_ctx, aio_comp, {{0, 1}}, std::move(bl), 0, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
...
TEST_F(TestMockIoImageRequest, AioFlushJournalAppendDisabled)
{
    ...
    C_SaferCond aio_comp_ctx;
    AioCompletion* aio_comp = AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_FLUSH);
    MockImageFlushRequest mock_aio_image_flush(mock_image_ctx, aio_comp, FLUSH_SOURCE_USER, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
...
TEST_F(TestMockIoImageRequest, AioWriteSameJournalAppendDisabled)
{
    ...
    C_SaferCond aio_comp_ctx;
    AioCompletion* aio_comp = AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_WRITESAME);

    bufferlist bl;
    bl.append("1");
    MockImageWriteSameRequest mock_aio_image_writesame(mock_image_ctx, aio_comp, {{0, 1}}, std::move(bl), 0, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
...
TEST_F(TestMockIoImageRequest, AioCompareAndWriteJournalAppendDisabled)
{
    ...
    C_SaferCond aio_comp_ctx;
    AioCompletion* aio_comp = AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_COMPARE_AND_WRITE);
    ...
    MockImageCompareAndWriteRequest mock_aio_image_write(
        mock_image_ctx, aio_comp, {{0, 1}}, std::move(cmp_bl), std::move(write_bl), &mismatch_offset, 0, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        mock_aio_image_write.send();
    }
...
TEST_F(TestMockIoImageRequest, ListSnaps)
{
    ...
    object_snapshot_delta[{5, 6}].insert(0, 1024, {SPARSE_EXTENT_STATE_DATA, 1024});
    object_snapshot_delta[{5, 5}].insert(4096, 4096, {SPARSE_EXTENT_STATE_ZEROED, 4096});
    expect_object_list_snaps_request(mock_image_ctx, 0, object_snapshot_delta, 0);
    object_snapshot_delta = {};
    object_snapshot_delta[{5, 6}].insert(1024, 3072, {SPARSE_EXTENT_STATE_DATA, 3072});
    object_snapshot_delta[{5, 5}].insert(2048, 2048, {SPARSE_EXTENT_STATE_ZEROED, 2048});
    expect_object_list_snaps_request(mock_image_ctx, 1, object_snapshot_delta, 0);

    SnapshotDelta snapshot_delta;
    C_SaferCond aio_comp_ctx;
    AioCompletion* aio_comp = AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_GENERIC);
    MockImageListSnapsRequest mock_image_list_snaps_request(
        mock_image_ctx, aio_comp, {{0, 16384}, {16384, 16384}}, {0, CEPH_NOSNAP}, 0, &snapshot_delta, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
    ...
    expected_snapshot_delta[{5, 6}].insert(0, 1024, {SPARSE_EXTENT_STATE_DATA, 1024});
    expected_snapshot_delta[{5, 6}].insert(5120, 3072, {SPARSE_EXTENT_STATE_DATA, 3072});
    expected_snapshot_delta[{5, 5}].insert(6144, 6144, {SPARSE_EXTENT_STATE_ZEROED, 6144});
