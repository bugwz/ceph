...
{
    MOCK_METHOD4(append_write_event, uint64_t(uint64_t, size_t, const bufferlist&, bool));
    MOCK_METHOD5(append_compare_and_write_event,
                 uint64_t(uint64_t, size_t, const bufferlist&, const bufferlist&, bool));
...
struct TestMockIoImageRequest : public TestMockFixture
{
    ...
    void expect_journal_append_io_event(MockTestJournal& mock_journal, uint64_t journal_tid, uint64_t offset,
                                        size_t length)
    {
        EXPECT_CALL(mock_journal, append_io_event_mock(_, offset, length, _, _)).WillOnce(Return(journal_tid));
    }
...
TEST_F(TestMockIoImageRequest, AioWriteModifyTimestamp)
{
    ...
    MockImageWriteRequest mock_aio_image_write_1(
        mock_image_ctx, aio_comp_1, {{0, 1}}, ImageArea::DATA, std::move(bl), 0, {});
    {
    ...
    MockImageWriteRequest mock_aio_image_write_2(
        mock_image_ctx, aio_comp_2, {{0, 1}}, ImageArea::DATA, std::move(bl), 0, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
...
TEST_F(TestMockIoImageRequest, AioReadAccessTimestamp)
{
    ...
    ReadResult rr;
    MockImageReadRequest mock_aio_image_read_1(mock_image_ctx,
                                               aio_comp_1,
                                               {{0, 1}},
                                               ImageArea::DATA,
                                               std::move(rr),
    ...
    MockImageReadRequest mock_aio_image_read_2(mock_image_ctx,
                                               aio_comp_2,
                                               {{0, 1}},
                                               ImageArea::DATA,
                                               std::move(rr),
...
TEST_F(TestMockIoImageRequest, PartialDiscard)
{
    ...
    MockImageDiscardRequest mock_aio_image_discard(
        mock_image_ctx, aio_comp, {{16, 63}, {84, 100}}, ImageArea::DATA, ictx->discard_granularity_bytes, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
...
TEST_F(TestMockIoImageRequest, TailDiscardJournalAppendEnabled)
{
    ...
    expect_get_modify_timestamp(mock_image_ctx, false);
    expect_is_journal_appending(mock_journal, true);
    expect_journal_append_io_event(mock_journal, 0, ictx->layout.object_size - 1024, 1024);
    expect_object_discard_request(mock_image_ctx, 0, ictx->layout.object_size - 1024, 1024, 0);
...
TEST_F(TestMockIoImageRequest, PruneRequiredDiscardJournalAppendEnabled)
{
    ...
    AioCompletion* aio_comp = AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_DISCARD);
    MockImageDiscardRequest mock_aio_image_discard(
        mock_image_ctx, aio_comp, {{96, 31}}, ImageArea::DATA, ictx->discard_granularity_bytes, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        mock_aio_image_discard.send();
    }
...
TEST_F(TestMockIoImageRequest, LengthModifiedDiscardJournalAppendEnabled)
{
    ...
    AioCompletion* aio_comp = AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_DISCARD);
    MockImageDiscardRequest mock_aio_image_discard(
        mock_image_ctx, aio_comp, {{16, 63}}, ImageArea::DATA, ictx->discard_granularity_bytes, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        mock_aio_image_discard.send();
    }
...
TEST_F(TestMockIoImageRequest, DiscardGranularityJournalAppendEnabled)
{
    ...
    expect_journal_append_io_event(mock_journal, 2, ictx->layout.object_size - 32, 32);
    expect_object_discard_request(mock_image_ctx, 0, 32, 32, 0);
    expect_object_discard_request(mock_image_ctx, 0, 96, 64, 0);
    expect_object_discard_request(mock_image_ctx, 0, ictx->layout.object_size - 32, 32, 0);
...
TEST_F(TestMockIoImageRequest, AioDiscardJournalAppendDisabled)
{
    ...
    AioCompletion* aio_comp = AioCompletion::create_and_start(&aio_comp_ctx, ictx, AIO_TYPE_DISCARD);
    MockImageDiscardRequest mock_aio_image_discard(
        mock_image_ctx, aio_comp, {{0, 1}}, ImageArea::DATA, ictx->discard_granularity_bytes, {});
    {
        std::shared_lock owner_locker{mock_image_ctx.owner_lock};
        mock_aio_image_discard.send();
    }
...
TEST_F(TestMockIoImageRequest, AioCompareAndWriteJournalAppendDisabled)
{
    ...
    MockImageCompareAndWriteRequest mock_aio_image_write(mock_image_ctx,
                                                         aio_comp,
                                                         {{0, 1}},
                                                         ImageArea::DATA,
                                                         std::move(cmp_bl),
...
TEST_F(TestMockIoImageRequest, ListSnaps)
{
    ...
    MockImageListSnapsRequest mock_image_list_snaps_request(mock_image_ctx,
                                                            aio_comp,
                                                            {{0, 16384}, {16384, 16384}},
                                                            ImageArea::DATA,
                                                            {0, CEPH_NOSNAP},
