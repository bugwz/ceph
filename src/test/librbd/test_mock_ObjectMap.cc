...
template<> struct RefreshRequest<MockTestImageCtx>
{
    Context* on_finish = nullptr;
    ceph::BitVector<2u>* object_map = nullptr;
    static RefreshRequest* s_instance;
    static RefreshRequest* create(MockTestImageCtx& image_ctx, ceph::shared_mutex*, ceph::BitVector<2u>* object_map,
...
template<> struct UnlockRequest<MockTestImageCtx>
{
    Context* on_finish = nullptr;
    static UnlockRequest* s_instance;
    static UnlockRequest* create(MockTestImageCtx& image_ctx, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->on_finish = on_finish;
        return s_instance;
    ...
    MOCK_METHOD0(send, void());
    UnlockRequest() { s_instance = this; }
};

template<> struct UpdateRequest<MockTestImageCtx>
{
    Context* on_finish = nullptr;
    static UpdateRequest* s_instance;
    static UpdateRequest* create(MockTestImageCtx& image_ctx, ceph::shared_mutex*, ceph::BitVector<2u>* object_map,
                                 uint64_t snap_id, uint64_t start_object_no, uint64_t end_object_no, uint8_t new_state,
                                 const boost::optional<uint8_t>& current_state, const ZTracer::Trace& parent_trace,
                                 bool ignore_enoent, Context* on_finish)
    {
        ceph_assert(s_instance != nullptr);
        s_instance->on_finish = on_finish;
        s_instance->construct(snap_id, start_object_no, end_object_no, new_state, current_state, ignore_enoent);
        return s_instance;
    }

    MOCK_METHOD6(construct, void(uint64_t snap_id, uint64_t start_object_no, uint64_t end_object_no, uint8_t new_state,
                                 const boost::optional<uint8_t>& current_state, bool ignore_enoent));
    MOCK_METHOD0(send, void());
...
public:
    ...
    void expect_refresh(MockTestImageCtx& mock_image_ctx, MockRefreshRequest& mock_refresh_request,
    ...
    void expect_unlock(MockTestImageCtx& mock_image_ctx, MockUnlockRequest& mock_unlock_request, int r)
    {
        EXPECT_CALL(mock_unlock_request, send()).WillOnce(Invoke([&mock_image_ctx, &mock_unlock_request, r]() {
            mock_image_ctx.image_ctx->op_work_queue->queue(mock_unlock_request.on_finish, r);
        }));
    }

    void expect_update(MockTestImageCtx& mock_image_ctx, MockUpdateRequest& mock_update_request, uint64_t snap_id,
                       uint64_t start_object_no, uint64_t end_object_no, uint8_t new_state,
                       const boost::optional<uint8_t>& current_state, bool ignore_enoent, Context** on_finish)
    {
        EXPECT_CALL(mock_update_request,
                    construct(snap_id, start_object_no, end_object_no, new_state, current_state, ignore_enoent))
            .Times(1);
        EXPECT_CALL(mock_update_request, send()).WillOnce(Invoke([&mock_update_request, on_finish]() {
...
TEST_F(TestMockObjectMap, NonDetainedUpdate)
{
    ...
    expect_update(mock_image_ctx, mock_update_request, CEPH_NOSNAP, 0, 1, 1, {}, false, &finish_update_1);
    Context* finish_update_2;
    expect_update(mock_image_ctx, mock_update_request, CEPH_NOSNAP, 1, 2, 1, {}, false, &finish_update_2);
...
TEST_F(TestMockObjectMap, DetainedUpdate)
{
    ...
    MockUpdateRequest mock_update_request;
    Context* finish_update_1;
    expect_update(mock_image_ctx, mock_update_request, CEPH_NOSNAP, 1, 4, 1, {}, false, &finish_update_1);
    Context* finish_update_2 = nullptr;
    expect_update(mock_image_ctx, mock_update_request, CEPH_NOSNAP, 1, 3, 1, {}, false, &finish_update_2);
    Context* finish_update_3 = nullptr;
    expect_update(mock_image_ctx, mock_update_request, CEPH_NOSNAP, 2, 3, 1, {}, false, &finish_update_3);
    Context* finish_update_4 = nullptr;
    expect_update(mock_image_ctx, mock_update_request, CEPH_NOSNAP, 0, 2, 1, {}, false, &finish_update_4);
    ...
    {
        std::shared_lock image_locker{mock_image_ctx.image_lock};
        mock_object_map->aio_update(CEPH_NOSNAP, 1, 4, 1, {}, {}, false, &update_ctx1);
        mock_object_map->aio_update(CEPH_NOSNAP, 1, 3, 1, {}, {}, false, &update_ctx2);
        mock_object_map->aio_update(CEPH_NOSNAP, 2, 3, 1, {}, {}, false, &update_ctx3);
        mock_object_map->aio_update(CEPH_NOSNAP, 0, 2, 1, {}, {}, false, &update_ctx4);
    }
