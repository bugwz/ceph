...
{
public:
    TestWrapLFNIndex(CephContext* cct, coll_t collection, const char* base_path, uint32_t index_version)
    ...
    int _split(uint32_t match, uint32_t bits, CollectionIndex* dest) override { return 0; }
    int _merge(uint32_t bits, CollectionIndex* dest) override { return 0; }
...
protected:
    ...
    int _lookup(const ghobject_t& hoid, vector<string>* path, string* mangled_name, int* exists) override { return 0; }

    int _collection_list_partial(const ghobject_t& start, const ghobject_t& end, int max_count, vector<ghobject_t>* ls,
...
public:
    TestHASH_INDEX_TAG()
        : TestWrapLFNIndex(g_ceph_context, coll_t(), "PATH_1", CollectionIndex::HASH_INDEX_TAG)
    {}
};
...
public:
    TestHASH_INDEX_TAG_2()
        : TestWrapLFNIndex(g_ceph_context, coll_t(), "PATH_1", CollectionIndex::HASH_INDEX_TAG_2)
    {}
};
...
public:
    ...
    void TearDown() override { ASSERT_EQ(0, ::system("rm -fr PATH_1")); }
};
...
TEST_F(TestLFNIndex, remove_object)
{
    ...
    //
    {
        ...
        EXPECT_EQ(
            object_name_1.size(),
            (unsigned)chain_setxattr(pathname.c_str(), LFN_ATTR.c_str(), object_name_1.c_str(), object_name_1.size()));
...
TEST_F(TestLFNIndex, get_mangled_name)
{
    ...
    //
    {
        ...
        EXPECT_EQ(
            object_name_same_prefix.size(),
            (unsigned)chain_setxattr(
                pathname.c_str(), LFN_ATTR.c_str(), object_name_same_prefix.c_str(), object_name_same_prefix.size()));
...
int main(int argc, char** argv)
{
    ...
    if (ret < 0) {
        ...
        auto cct = global_init(
            NULL, args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_UTILITY, CINIT_FLAG_NO_DEFAULT_CONFIG_FILE);
