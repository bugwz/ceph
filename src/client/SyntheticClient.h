...
class SyntheticClient
{
    ...
    const char* get_random_sub()
    {
        ...
        if (cwd.depth() && cwd.last_dentry().length()) r += cwd.last_dentry().c_str()[0];   // slightly permuted
        r %= contents.size();
...
public:
    ...
    bool time_to_stop()
    {
        ...
        if (0)
            std::cout << "time_to_stop .. now " << now << " until " << run_until << " start " << run_start << std::endl;
    ...
    std::string compose_path(std::string& prefix, char* rest) { return prefix + rest; }
    ...
    int object_rw(int nobj, int osize, int wrpc, int overlap, double rskew, double wskew);
