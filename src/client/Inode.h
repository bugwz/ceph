...
struct Inode : RefCountedObject
{
    ...
    bool is_fscrypt_enabled() { return !!fscrypt_auth.size(); }
    ...
    bool has_dir_layout() const { return layout != file_layout_t(); }
    ...
    {
        return (fcntl_locks && !fcntl_locks->empty()) || (flock_locks && !flock_locks->empty());
    }
...
private:
    ...
    long open_count_for_write() { return (long)(open_by_mode[CEPH_FILE_MODE_RDWR] + open_by_mode[CEPH_FILE_MODE_WR]); };

    // how many opens of any sort on this inode?
    long open_count()
    {
        return (long)std::accumulate(
            open_by_mode.begin(), open_by_mode.end(), 0, [](int value, const std::map<int, int>::value_type& p) {
