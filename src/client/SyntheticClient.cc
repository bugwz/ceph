...
int SyntheticClient::run()
{
    ...
    for (list<int>::iterator it = modes.begin(); it != modes.end(); ++it) {
        int mode = *it;
        ...
        case SYNCLIENT_MODE_ONLY:
        {
            ...
            iargs.pop_front();
            if (run_only == client->get_nodeid()) dout(2) << "only " << run_only << dendl;
        } break;
        case SYNCLIENT_MODE_ONLYRANGE:
        {
            ...
            iargs.pop_front();
            if (first <= client->get_nodeid() && last > client->get_nodeid()) {
        ...
        case SYNCLIENT_MODE_SLEEPUNTIL:
        {
            ...
            if (iarg1 && run_me()) {
                ...
                if (at.sec() < iarg1) sleep(iarg1 - at.sec());
            }
            did_run_me();
        } break;
        ...
        case SYNCLIENT_MODE_OBJECTRW:
        {
            ...
            iargs.pop_front();
            if (run_me()) {
                dout(2) << "objectrw " << count << " " << size << " " << wrpc << " " << overlap << " " << rskew << " "
                        << wskew << dendl;
        ...
        case SYNCLIENT_MODE_TRACE:
        {
            ...
            if (run_me()) {
                dout(0) << "trace " << tfile << " prefix=" << prefix << " count=" << iarg1 << " data=" << playdata
                        << dendl;
                ...
                for (int i = 0; i < iarg1; i++) {
                    ...
                    dout(0) << " trace " << tfile << " loop " << (i + 1) << "/" << iarg1 << " done in " << (double)lat
                            << " seconds" << dendl;
                    if (client->logger && i > 0 && i < iarg1 - 1) {
        ...
        case SYNCLIENT_MODE_OPENTEST:
        {
            ...
            if (run_me()) {
                for (int i = 0; i < count; i++) {
                    int fd = client->open("test", (rand() % 2) ? (O_WRONLY | O_CREAT) : O_RDONLY, perms);
        ...
        case SYNCLIENT_MODE_MKSNAP:
        {
            string base = get_sarg(0);
            string name = get_sarg(0);
            if (run_me()) mksnap(base.c_str(), name.c_str(), perms);
            did_run_me();
        } break;
        case SYNCLIENT_MODE_RMSNAP:
        {
            string base = get_sarg(0);
            string name = get_sarg(0);
            if (run_me()) rmsnap(base.c_str(), name.c_str(), perms);
            did_run_me();
        } break;
        case SYNCLIENT_MODE_MKSNAPFILE:
        {
            string base = get_sarg(0);
            if (run_me()) mksnapfile(base.c_str());
            did_run_me();
        } break;
...
int SyntheticClient::play_trace(Trace& t, string& prefix, bool metadata_only)
{
    ...
    while (!t.end()) {
        ...
        // low level ops ---------------------
            ...
            if (ll_inos.count(i)) {
                ...
                if (client->ll_lookupx(i1, name, &i2, &stx, CEPH_STATX_INO, 0, perms) == 0) ll_inos[r] = stx.stx_ino;
                client->ll_put(i1);
            }
        }
            ...
            int64_t i = t.get_int();
            int64_t n = t.get_int();
            if (ll_inos.count(i) && client->ll_forget(client->ll_get_inode(vinodeno_t(ll_inos[i], CEPH_NOSNAP)), n))
                ll_inos.erase(i);
        }
            ...
            if (ll_inos.count(i)) {
                i1 = client->ll_get_inode(vinodeno_t(ll_inos[i], CEPH_NOSNAP));
                if (client->ll_mknod(i1, n, m, r, &attr, &i2, perms) == 0) ll_inos[ri] = attr.st_ino;
                client->ll_put(i1);
            }
        }
            ...
            if (ll_inos.count(i)) {
                i1 = client->ll_get_inode(vinodeno_t(ll_inos[i], CEPH_NOSNAP));
                if (client->ll_mkdir(i1, n, m, &attr, &i2, perms) == 0) ll_inos[ri] = attr.st_ino;
                client->ll_put(i1);
            }
        }
            ...
            if (ll_inos.count(i)) {
                i1 = client->ll_get_inode(vinodeno_t(ll_inos[i], CEPH_NOSNAP));
                if (client->ll_symlink(i1, n, v, &attr, &i2, perms) == 0) ll_inos[ri] = attr.st_ino;
                client->ll_put(i1);
            }
        }
            ...
            if (ll_inos.count(i) && ll_inos.count(ni)) {
                i1 = client->ll_get_inode(vinodeno_t(ll_inos[i], CEPH_NOSNAP));
            ...
            if (ll_inos.count(i)) {
                i1 = client->ll_get_inode(vinodeno_t(ll_inos[i], CEPH_NOSNAP));
                if (client->ll_opendir(i1, O_RDONLY, &dirp, perms) == 0) ll_dirs[r] = dirp;
                client->ll_put(i1);
            }
        }
            ...
            if (ll_inos.count(i)) {
                i1 = client->ll_get_inode(vinodeno_t(ll_inos[i], CEPH_NOSNAP));
                if (client->ll_open(i1, f, &fhp, perms) == 0) ll_files[r] = fhp;
                client->ll_put(i1);
            }
        }
            ...
            int64_t off = t.get_int();
            int64_t size = t.get_int();
            if (ll_files.count(f) && !metadata_only) {
                bufferlist bl;
            ...
            int64_t f = t.get_int();
            if (!metadata_only && ll_files.count(f)) client->ll_flush(ll_files[f]);
        }
        else if (strcmp(op, "ll_fsync") == 0) {
            int64_t f = t.get_int();
            if (!metadata_only && ll_files.count(f)) client->ll_fsync(ll_files[f], false);   // FIXME dataonly param
        }
            ...
            int64_t i = t.get_int();
            if (ll_inos.count(i)) {}   // client->ll_statfs(vinodeno_t(ll_inos[i],CEPH_NOSNAP), perms);
        }
        ...
        // object-level traces
            ...
            bl.push_back(bp);
            SnapContext snapc;
            client->objecter->write(
                oid, oloc, off, len, snapc, bl, ceph::real_clock::now(), 0, new C_SafeCond(lock, cond, &ack));
            cond.wait(locker, [&ack] { return ack; });
        }
            ...
            std::unique_lock locker{lock};
            SnapContext snapc;
            client->objecter->zero(
                oid, oloc, off, len, snapc, ceph::real_clock::now(), 0, new C_SafeCond(lock, cond, &ack));
            cond.wait(locker, [&ack] { return ack; });
        }
    ...
    // close open files
    for (ceph::unordered_map<int64_t, int64_t>::iterator fi = open_files.begin(); fi != open_files.end(); ++fi) {
        dout(1) << "leftover close " << fi->second << dendl;
        if (fi->second > 0) client->close(fi->second);
    }
    for (ceph::unordered_map<int64_t, dir_result_t*>::iterator fi = open_dirs.begin(); fi != open_dirs.end(); ++fi) {
        dout(1) << "leftover closedir " << fi->second << dendl;
        if (fi->second != 0) client->closedir(fi->second);
    }
    for (ceph::unordered_map<int64_t, Fh*>::iterator fi = ll_files.begin(); fi != ll_files.end(); ++fi) {
        dout(1) << "leftover ll_release " << fi->second << dendl;
        if (fi->second) client->ll_release(fi->second);
    }
    for (ceph::unordered_map<int64_t, dir_result_t*>::iterator fi = ll_dirs.begin(); fi != ll_dirs.end(); ++fi) {
...
int SyntheticClient::full_walk(string& basedir)
{
    ...
    while (!dirq.empty()) {
        ...
        if (dir != "" && (actual.nsubdirs != expect.nsubdirs || actual.nfiles != expect.nfiles)) {
...
int SyntheticClient::dump_placement(string& fn)
{
    ...
    for (const auto& x : extents) {
        int osd = client->objecter->with_osdmap(
            [&](const OSDMap& o) { return o.get_pg_acting_primary(o.object_locator_to_pg(x.oid, x.oloc)); });
...
int SyntheticClient::overload_osd_0(int n, int size, int wrsize)
{
    ...
    while (left < 0) {
        ...
        if (primary_osd != 0) {
            client->close(fd);
            dout(1) << "OSD Overload workload: SKIPPING file " << filename << " with OSD " << primary_osd
...
int SyntheticClient::check_first_primary(int fh)
{
    ...
    return client->objecter->with_osdmap([&](const OSDMap& o) {
        return o.get_pg_acting_primary(o.object_locator_to_pg(extents.begin()->oid, extents.begin()->oloc));
    });
}
...
int SyntheticClient::write_file(string& fn, int size, loff_t wrsize)   // size is in MB, wrsize in bytes
{
    ...
    dout(0) << "write total " << (total / el / 1048576.0) << " MB/sec (" << total << " bytes in " << el << " seconds)"
            << dendl;
...
int SyntheticClient::read_file(const std::string& fn, int size, int rdsize, bool ignoreprint)
{
    ...
    for (unsigned i = 0; i < chunks; i++) {
        ...
        while ((char*)p + 32 < buf + rdsize) {
            ...
            if (readoff != wantoff || readclient != client->get_nodeid()) {
                if (!bad && !ignoreprint)
                    dout(0) << "WARNING: wrong data from OSD, block says fileoffset=" << readoff
                            << " client=" << readclient << ", should be offset " << wantoff << " client "
                            << client->get_nodeid() << dendl;
                bad++;
            }
        }
        if (bad && !ignoreprint) dout(0) << " + " << (bad - 1) << " other bad 16-byte bits in this block" << dendl;
    }

    utime_t stop = ceph_clock_now();
    double el = stop - start;
    dout(0) << "read total " << (total / el / 1048576.0) << " MB/sec (" << total << " bytes in " << el << " seconds)"
            << dendl;
...
int SyntheticClient::create_objects(int nobj, int osize, int inflight)
{
    ...
    dout(5) << "create_objects " << nobj << " size=" << osize << " .. doing [" << start << "," << end << ") inc " << inc
            << dendl;
    ...
    for (int i = start; i < end; i += inc) {
        ...
        starts.push_back(ceph_clock_now());
        {
            ...
            client->objecter->write(
                oid, oloc, 0, osize, snapc, bl, ceph::real_clock::now(), 0, new C_Ref(lock, cond, &unsafe));
        }
        {
            std::unique_lock locker{lock};
...
int SyntheticClient::object_rw(int nobj, int osize, int wrpc, int overlappc, double rskew, double wskew)
{
    dout(5) << "object_rw " << nobj << " size=" << osize << " with " << wrpc << "% writes"
            << ", " << overlappc << "% overlap"
            << ", rskew = " << rskew << ", wskew = " << wskew << dendl;
    ...
    while (1) {
        ...
        if (write) {
            ...
            if (pnoremap >= overlappc) o = (o * prime) % nobj;   // remap
        }
        else {
        ...
        if (write) {
            ...
            op.indata = bl;
            m.ops.push_back(op);
            client->objecter->mutate(oid, oloc, m, snapc, ceph::real_clock::now(), 0, new C_Ref(lock, cond, &unack));
        }
        else {
            dout(10) << "read from " << oid << dendl;
            bufferlist inbl;
            client->objecter->read(oid, oloc, 0, osize, CEPH_NOSNAP, &inbl, 0, new C_Ref(lock, cond, &unack));
        }
...
int SyntheticClient::read_random(string& fn, int size, int rdsize)   // size is in MB, wrsize in bytes
{
    ...
    for (unsigned i = 0; i < 2000; i++) {
        ...
        if (read) {
            ...
            if (bad) dout(0) << " + " << (bad - 1) << " other bad 16-byte bits in this block" << dendl;
        }
    }
    ...
    while (left > 0) {
        ...
        */
        ...
        if (op == CEPH_MDS_OP_READDIR) {
            ...
            for (list<string>::iterator it = c.begin(); it != c.end(); ++it) {
...
void SyntheticClient::import_find(const char* base, const char* find, bool data)
{
    ...
    if (base[0] != '-') client->mkdir(base, 0755, process_perms);
...
int SyntheticClient::lookup_hash(inodeno_t ino, inodeno_t dirino, const char* name, const UserPerm& perms)
{
