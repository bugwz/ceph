...
Inode::~Inode()
{
    ...
    if (!oset.objects.empty()) {
        lsubdout(client->cct, client, 0) << __func__ << ": leftover objects on inode 0x" << std::hex << ino << std::dec
                                         << dendl;
        ceph_assert(oset.objects.empty());
    }

    if (!delegations.empty()) {
        lsubdout(client->cct, client, 0) << __func__ << ": leftover delegations on inode 0x" << std::hex << ino
                                         << std::dec << dendl;
...
{
    out << in.vino() << "("
        << "faked_ino=" << in.faked_ino << " nref=" << in.get_nref() << " ll_ref=" << in.ll_ref
        << " cap_refs=" << in.cap_refs << " open=" << in.open_by_mode << " mode=" << oct << in.mode << dec
        << " size=" << in.size << "/" << in.max_size << " nlink=" << in.nlink << " btime=" << in.btime
        << " mtime=" << in.mtime << " ctime=" << in.ctime << " change_attr=" << in.change_attr
    ...
    if (in.dirty_caps) out << " dirty_caps=" << ccap_string(in.dirty_caps);
    if (in.flushing_caps) out << " flushing_caps=" << ccap_string(in.flushing_caps);

    if (in.flags & I_COMPLETE) out << " COMPLETE";

    if (in.is_file()) out << " " << in.oset;

    if (!in.dentries.empty()) out << " parents=" << in.dentries;

    if (in.is_dir() && in.has_dir_layout()) out << " has_dir_layout";

    if (in.quota.is_enabled()) out << " " << in.quota;
...
void Inode::get_cap_ref(int cap)
{
    ...
    while (cap) {
        if (cap & 1) {
            int c = 1 << n;
            cap_refs[c]++;
            // cout << "inode " << *this << " get " << cap_string(c) << " " << (cap_refs[c]-1) << " -> " << cap_refs[c]
            // << std::endl;
        }
        cap >>= 1;
        n++;
...
int Inode::put_cap_ref(int cap)
{
    ...
    while (cap) {
        if (cap & 1) {
            ...
            if (--cap_refs[c] == 0) last |= c;
            // cout << "inode " << *this << " put " << cap_string(c) << " " << (cap_refs[c]+1) << " -> " << cap_refs[c]
            // << std::endl;
        }
        cap >>= 1;
        n++;
...
bool Inode::cap_is_valid(const Cap& cap) const
{
    ...
    if ((cap.session->cap_gen <= cap.gen) && (ceph_clock_now() < cap.session->cap_ttl)) {
        return true;
    }
    return false;
}
...
int Inode::caps_issued(int* implemented) const
{
    ...
    if (auth_cap) c &= ~auth_cap->implemented | auth_cap->issued;

    if (implemented) *implemented = i;
    return c;
}
...
bool Inode::caps_issued_mask(unsigned mask, bool allow_impl)
{
    ...
    if ((c & mask) == mask) return true;
    // prefer auth cap
    if (auth_cap && cap_is_valid(*auth_cap) && (auth_cap->issued & mask) == mask) {
        auth_cap->touch();
        client->cap_hit();
        return true;
    }
...
int Inode::caps_file_wanted()
{
    ...
    for (const auto& [mode, cnt] : open_by_mode)
        if (cnt) want |= ceph_caps_for_mode(mode);
    return want;
}
...
int Inode::caps_wanted()
{
    ...
    if (want & CEPH_CAP_FILE_BUFFER) want |= CEPH_CAP_FILE_EXCL;
    return want;
}
...
bool Inode::have_valid_size()
{
    ...
    if (caps_issued() & (CEPH_CAP_FILE_SHARED | CEPH_CAP_FILE_EXCL)) return true;
    return false;
}
...
Dir* Inode::open_dir()
{
    if (!dir) {
        ...
        if (!dentries.empty()) get_first_parent()->get();   // pin dentry
        iget();                                             // pin inode
    }
    return dir;
}
...
void Inode::dump(Formatter* f) const
{
    ...
    if (rdev) f->dump_unsigned("rdev", rdev);
    ...
    for (const auto& pair : caps) {
        ...
        if (&pair.second == auth_cap) f->dump_int("auth", 1);
        pair.second.dump(f);
        f->close_section();
    }
    f->close_section();
    if (auth_cap) f->dump_int("auth_cap", auth_cap->session->mds_num);
    ...
    if (flushing_caps) {
        ...
        for (map<ceph_tid_t, int>::const_iterator p = flushing_cap_tids.begin(); p != flushing_cap_tids.end(); ++p) {
    ...
    if (wanted_max_size != max_size) f->dump_unsigned("wanted_max_size", wanted_max_size);
    if (requested_max_size != max_size) f->dump_unsigned("requested_max_size", requested_max_size);
...
void Cap::dump(Formatter* f) const
{
    ...
    if (implemented != issued) f->dump_stream("implemented") << ccap_string(implemented);
...
{
    if (delegations.empty()) return false;
...
{
    if (delegations.empty()) return;

    // Issue any recalls
    for (list<Delegation>::iterator d = delegations.begin(); d != delegations.end(); ++d) {
...
void Inode::break_deleg(bool skip_read)
{
    ...
    while (!delegations_broken(skip_read)) client->wait_on_list(waitfor_deleg);
}
...
int Inode::set_deleg(Fh* fh, unsigned type, ceph_deleg_cb_t cb, void* priv)
{
     ...
     */
    if (!client->get_deleg_timeout()) return -CEPHFS_ETIME;
    ...
    if (!caps_issued_mask(need)) {
        lsubdout(client->cct, client, 10) << __func__ << ": cap mismatch, have=" << ccap_string(caps_issued())
                                          << " need=" << ccap_string(need) << dendl;
        return -CEPHFS_EAGAIN;
    }
