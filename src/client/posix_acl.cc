...
int posix_acl_check(const void* xattr, size_t size)
{
    ...
    if (header->a_version != expected_version) return -1;

    const acl_ea_entry* entry = header->a_entries;
    size -= sizeof(*header);
    if (size % sizeof(*entry)) return -1;
...
{
    if (posix_acl_check(xattr, size) < 0) return -CEPHFS_EINVAL;
    ...
    for (int i = 0; i < count; ++i) {
        ...
        switch (tag) {
        case ACL_USER_OBJ: mode |= (perm & S_IRWXO) << 6; break;
        case ACL_GROUP_OBJ: mode |= (perm & S_IRWXO) << 3; break;
        case ACL_OTHER: mode |= perm & S_IRWXO; break;
        case ACL_MASK:
    ...
    if (mode_p) *mode_p = (*mode_p & ~ACCESSPERMS) | mode;
    return not_equiv;
}

int posix_acl_inherit_mode(bufferptr& acl, mode_t* mode_p)
{
    if (posix_acl_check(acl.c_str(), acl.length()) <= 0) return -CEPHFS_EIO;
    ...
    for (int i = 0; i < count; ++i) {
        ...
        case ACL_USER:
        case ACL_GROUP: not_equiv = 1; break;
        case ACL_GROUP_OBJ: group_entry = entry; break;
        case ACL_OTHER:
            perm &= mode | ~S_IRWXO;
            mode &= perm | ~S_IRWXO;
            entry->e_perm = perm;
...
int posix_acl_access_chmod(bufferptr& acl, mode_t mode)
{
    ...
    for (int i = 0; i < count; ++i) {
        __u16 tag = entry->e_tag;
        switch (tag) {
        case ACL_USER_OBJ: entry->e_perm = (mode & S_IRWXU) >> 6; break;
        case ACL_GROUP_OBJ: group_entry = entry; break;
        case ACL_MASK: mask_entry = entry; break;
        case ACL_OTHER: entry->e_perm = mode & S_IRWXO; break;
        default: break;
        }
        ++entry;
    }
...
int posix_acl_permits(const bufferptr& acl, uid_t i_uid, gid_t i_gid, const UserPerm& perms, unsigned want)
{
    if (posix_acl_check(acl.c_str(), acl.length()) < 0) return -CEPHFS_EIO;
    ...
    for (idx = 0; idx < count; ++idx) {
        ...
        switch (tag) {
        case ACL_USER_OBJ:
            if (i_uid == perms.uid()) goto check_perm;
            break;
        case ACL_USER:
            id = entry->e_id;
            if (id == perms.uid()) goto check_mask;
            break;
        case ACL_GROUP_OBJ:
            /* fall through */
        case ACL_GROUP:
            ...
            if (perms.gid_in_groups(id)) {
                group_found = 1;
                if ((perm & want) == want) goto check_mask;
            }
            break;
        case ACL_MASK: break;
        case ACL_OTHER:
...
check_mask:
    ...
    for (++idx; idx < count; ++idx) {
        ...
        if (tag == ACL_MASK) {
            ...
            if ((perm & mask & want) == want) return 0;
            return -CEPHFS_EACCES;
        }
        ++next_entry;
    }
