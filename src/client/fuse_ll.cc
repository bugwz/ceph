...
{
    if (cephfs_errno == 0) return 0;

    auto it = cephfs_errno_to_system_errno.find(cephfs_errno);
    if (it != cephfs_errno_to_system_errno.end()) return it->second;
    return EIO;
}
...
static int already_fuse_mounted(const char* path, bool& already_mounted)
{
    ...
    if (-1 == statx(AT_FDCWD, path, AT_STATX_DONT_SYNC, STATX_INO, &path_statx)) {
        err = errno;
        derr << "fuse_ll: already_fuse_mounted: statx(" << path << ") failed with error " << cpp_strerror(err) << dendl;
        return err;
    }
    ...
    if (!(path_statx.stx_mode & S_IFDIR)) {
        err = EINVAL;
        derr << "fuse_ll: already_fuse_mounted: " << path << " is not a directory" << dendl;
        return err;
    }
    ...
    if (-1 == statx(AT_FDCWD, parent_path, AT_STATX_DONT_SYNC, STATX_INO, &parent_statx)) {
        err = errno;
        derr << "fuse_ll: already_fuse_mounted: statx(" << parent_path << ") failed with error " << cpp_strerror(err)
             << dendl;
        return err;
    }
    ...
    if (path_statx.stx_dev_major != parent_statx.stx_dev_major ||
        ...
        (path_statx.stx_dev_major == parent_statx.stx_dev_major &&
         path_statx.stx_dev_minor == parent_statx.stx_dev_minor && path_statx.stx_ino == parent_statx.stx_ino)) {
        struct statfs path_statfs;
        if (-1 == statfs(path, &path_statfs)) {
            err = errno;
            derr << "fuse_ll: already_fuse_mounted: statfs(" << path << ") failed with error " << cpp_strerror(err)
                 << dendl;
            return err;
        }
...
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
static void fuse_ll_forget(fuse_req_t req, fuse_ino_t ino, uint64_t nlookup)
#else
...
#endif
{
    ...
    if (in) cfuse->client->ll_forget(in, nlookup + 1);
    fuse_reply_none(req);
}
...
static void fuse_ll_getattr(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info* fi)
{
    ...
    if (cfuse->client->ll_getattr(in, &stbuf, perms) == 0) {
...
static void fuse_ll_opendir(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info* fi)
{
    ...
    int r = cfuse->client->ll_opendir(in, fi->flags, (dir_result_t**)&dirp, perms);
    if (r >= 0) {
        fi->fh = (uint64_t)dirp;
...
static void fuse_ll_mknod(fuse_req_t req, fuse_ino_t parent, const char* name, mode_t mode, dev_t rdev)
{
    ...
    int r = cfuse->client->ll_mknod(i1, name, mode, new_decode_dev(rdev), &fe.attr, &i2, perms);
    if (r == 0) {
        fe.ino = cfuse->make_fake_ino(fe.attr.st_ino, fe.attr.st_dev);
        fe.attr.st_rdev = new_encode_dev(fe.attr.st_rdev);
        fuse_reply_entry(req, &fe);
...
static void fuse_ll_mkdir(fuse_req_t req, fuse_ino_t parent, const char* name, mode_t mode)
{
    ...
    auto fuse_multithreaded = cfuse->client->cct->_conf.get_val<bool>("fuse_multithreaded");
    auto fuse_syncfs_on_mksnap = cfuse->client->cct->_conf.get_val<bool>("fuse_syncfs_on_mksnap");
    if (cfuse->fino_snap(parent) == CEPH_SNAPDIR && fuse_multithreaded && fuse_syncfs_on_mksnap) {
        int err = 0;
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
...
static void fuse_ll_rename(fuse_req_t req, fuse_ino_t parent, const char* name, fuse_ino_t newparent,
                           const char* newname
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
                           ,
                           unsigned int flags
...
#if FUSE_VERSION >= FUSE_MAKE_VERSION(2, 8)
        auto fuse_disable_pagecache = cfuse->client->cct->_conf.get_val<bool>("fuse_disable_pagecache");
        auto fuse_use_invalidate_cb = cfuse->client->cct->_conf.get_val<bool>("fuse_use_invalidate_cb");
...
static void fuse_ll_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off, struct fuse_file_info* fi)
{
    ...
    if (r >= 0) {
        ...
        if (bl.get_num_buffers() > IOV_MAX) bl.rebuild();
...
static void fuse_ll_write(fuse_req_t req, fuse_ino_t ino, const char* buf, size_t size, off_t off,
...
static void fuse_ll_ioctl(fuse_req_t req, fuse_ino_t ino,
                          ...
                          int cmd,
#endif
                          void* arg, struct fuse_file_info* fi, unsigned flags, const void* in_buf, size_t in_bufsz,
                          size_t out_bufsz)
{
    CephFuse::Handle* cfuse = fuse_ll_req_prepare(req);
    ...
    default: fuse_reply_err(req, EINVAL);
    }
}
#endif
...
static void fuse_ll_fallocate(fuse_req_t req, fuse_ino_t ino, int mode, off_t offset, off_t length,
                              struct fuse_file_info* fi)
{
    CephFuse::Handle* cfuse = fuse_ll_req_prepare(req);
    Fh* fh = (Fh*)fi->fh;
...
static void fuse_ll_fsync(fuse_req_t req, fuse_ino_t ino, int datasync, struct fuse_file_info* fi)
{
    CephFuse::Handle* cfuse = fuse_ll_req_prepare(req);
    Fh* fh = reinterpret_cast<Fh*>(fi->fh);
 ...
 */
static int fuse_ll_add_dirent(void* p, struct dirent* de, struct ceph_statx* stx, off_t next_off, Inode* in)
{
    ...
    size_t entrysize = fuse_add_direntry(c->req, c->buf + c->pos, room, de->d_name, &st, next_off);
    if (entrysize > room) return -ENOSPC;
...
static void fuse_ll_fsyncdir(fuse_req_t req, fuse_ino_t ino, int datasync, struct fuse_file_info* fi)
{
    CephFuse::Handle* cfuse = fuse_ll_req_prepare(req);
    dir_result_t* dirp = reinterpret_cast<dir_result_t*>(fi->fh);
...
static void fuse_ll_create(fuse_req_t req, fuse_ino_t parent, const char* name, mode_t mode, struct fuse_file_info* fi)
{
    ...
    int r = cfuse->client->ll_create(i1, name, mode, fi->flags, &fe.attr, &i2, &fh, perms);
    if (r == 0) {
        fi->fh = (uint64_t)fh;
        fe.ino = cfuse->make_fake_ino(fe.attr.st_ino, fe.attr.st_dev);
#if FUSE_VERSION >= FUSE_MAKE_VERSION(2, 8)
...
static void fuse_ll_getlk(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info* fi, struct flock* lock)
{
    CephFuse::Handle* cfuse = fuse_ll_req_prepare(req);
    Fh* fh = reinterpret_cast<Fh*>(fi->fh);
...
static void fuse_ll_setlk(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info* fi, struct flock* lock, int sleep)
{
    CephFuse::Handle* cfuse = fuse_ll_req_prepare(req);
    Fh* fh = reinterpret_cast<Fh*>(fi->fh);
...
#if FUSE_VERSION >= FUSE_MAKE_VERSION(2, 9)
static void fuse_ll_flock(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info* fi, int cmd)
{
    CephFuse::Handle* cfuse = fuse_ll_req_prepare(req);
    Fh* fh = (Fh*)fi->fh;
...
static void dentry_invalidate_cb(void* handle, vinodeno_t dirino, vinodeno_t ino, const char* name, size_t len)
{
    CephFuse::Handle* cfuse = (CephFuse::Handle*)handle;
    fuse_ino_t fdirino = cfuse->make_fake_ino(dirino.ino, dirino.snapid);
#if FUSE_VERSION >= FUSE_MAKE_VERSION(2, 9)
    fuse_ino_t fino = 0;
    if (ino.ino != inodeno_t()) fino = cfuse->make_fake_ino(ino.ino, ino.snapid);
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
...
static void do_init(void* data, fuse_conn_info* conn)
{
    ...
    if (!client->fuse_default_permissions && client->ll_handle_umask()) {
        ...
        if (conn->capable & FUSE_CAP_DONT_MASK) conn->want |= FUSE_CAP_DONT_MASK;
    }
    if (conn->capable & FUSE_CAP_EXPORT_SUPPORT) conn->want |= FUSE_CAP_EXPORT_SUPPORT;
#endif
...
int CephFuse::Handle::init(int argc, const char* argv[])
{
    ...
    if (!newargv) return ENOMEM;

    newargv[newargc++] = argv[0];
    newargv[newargc++] = "-f";   // stay in foreground

    auto fuse_allow_other = client->cct->_conf.get_val<bool>("fuse_allow_other");
    auto fuse_default_permissions = client->cct->_conf.get_val<bool>("fuse_default_permissions");
#if FUSE_VERSION < FUSE_MAKE_VERSION(3, 0)
    auto fuse_big_writes = client->cct->_conf.get_val<bool>("fuse_big_writes");
#endif
    auto fuse_max_write = client->cct->_conf.get_val<Option::size_t>("fuse_max_write");
    auto fuse_atomic_o_trunc = client->cct->_conf.get_val<bool>("fuse_atomic_o_trunc");
    auto fuse_splice_read = client->cct->_conf.get_val<bool>("fuse_splice_read");
    auto fuse_splice_write = client->cct->_conf.get_val<bool>("fuse_splice_write");
    auto fuse_splice_move = client->cct->_conf.get_val<bool>("fuse_splice_move");
    auto fuse_debug = client->cct->_conf.get_val<bool>("fuse_debug");
...
#endif
    if (fuse_debug) newargv[newargc++] = "-d";
    ...
    struct ceph_client_callback_args cb_args = {
        handle: this,
        ino_cb: client->cct->_conf.get_val<bool>("fuse_use_invalidate_cb") ? ino_invalidate_cb : NULL,
...
{
    auto fuse_multithreaded = client->cct->_conf.get_val<bool>("fuse_multithreaded");
    if (fuse_multithreaded) {
        ...
        {
            struct fuse_loop_config conf = {clone_fd: opts.clone_fd, max_idle_threads: opts.max_idle_threads};
...
{
    if (fino == FUSE_ROOT_ID) return CEPH_NOSNAP;
    ...
    if (client->use_faked_inos()) {
        ...
        if (!g_fino_maps.count(ino)) return CEPH_MAXSNAP;

        auto& fino_maps = g_fino_maps[ino];

        // does the stagid <--> snapid map exist ?
        if (!fino_maps.stag_snap_map.count(stag)) return CEPH_MAXSNAP;
...
{
    if (fino == FUSE_ROOT_ID) return client->get_root();
    ...
    if (client->use_faked_inos()) {
        ...
        if (snap == CEPH_MAXSNAP) return NULL;
...
uint64_t CephFuse::Handle::make_fake_ino(inodeno_t ino, snapid_t snapid)
{
    if (client->use_faked_inos()) {
        ...
        if (ino == client->get_root_ino()) return FUSE_ROOT_ID;

        return ino;
    }
    else {
        if (snapid == CEPH_NOSNAP && ino == client->get_root_ino()) return FUSE_ROOT_ID;
...
CephFuse::CephFuse(Client* c, int fd)
    : _handle(new CephFuse::Handle(c, fd))
{}
