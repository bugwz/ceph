...
void MetaRequest::dump(Formatter* f) const
{
    ...
    if (_inode) f->dump_stream("ino") << _inode->ino;
    if (_old_inode) f->dump_stream("old_ino") << _old_inode->ino;
    if (_other_inode) f->dump_stream("other_ino") << _other_inode->ino;
    if (target) f->dump_stream("target_ino") << target->ino;
    if (_dentry) f->dump_string("dentry", _dentry->name);
    if (_old_dentry) f->dump_string("old_dentry", _old_dentry->name);
...
{
    if (_dentry) _dentry->put();
    if (_old_dentry) _old_dentry->put();
}
