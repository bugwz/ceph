...
int Client::CommandHook::call(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, std::ostream& errss,
                              bufferlist& out)
{
...
void Client::insert_readdir_results(MetaRequest* request, MetaSession* session, Inode* diri)
{
    ...
    if (!p.end()) {
        ...
        for (unsigned i = 0; i < numdn; i++) {
            ...
            if (dirp->release_count == diri->dir_release_count && dirp->ordered_count == diri->dir_ordered_count &&
                dirp->start_shared_gen == diri->shared_gen) {
...
Inode* Client::insert_trace(MetaRequest* request, MetaSession* session)
{
    ...
    if (reply->snapbl.length()) update_snap_trace(reply->snapbl, &realm);
                         ...
                         mds_rank_t use_mds, bufferlist* pdirbl, size_t feature_needed)
{
    ...
    if (r >= 0 && ptarget) r = verify_reply_trace(r, session, request, reply, ptarget, pcreated, perms);
...
void Client::handle_client_session(const MConstRef<MClientSession>& m)
{
    ...
    case CEPH_SESSION_OPEN:
    {
        ...
        if (!missing_features.empty()) {
            lderr(cct) << "mds." << from << " lacks required features '" << missing_features << "', closing session "
                       << dendl;
    ...
    case CEPH_SESSION_CLOSE: _closed_mds_session(session); break;
    ...
    case CEPH_SESSION_RENEWCAPS:
        if (session->cap_renew_seq == m->get_seq()) {
            ...
            if (was_stale) wake_up_session_caps(session, false);
        }
        break;
    ...
    case CEPH_SESSION_FORCE_RO: force_session_readonly(session); break;
...
void Client::_kick_stale_sessions()
{
    ...
    for (auto it = mds_sessions.begin(); it != mds_sessions.end();) {
        ...
        ++it;
        if (s.state == MetaSession::STATE_STALE) _closed_mds_session(&s);
    }
}
...
int Client::get_caps(Fh* fh, int need, int want, int* phave, loff_t endoff)
{
    ...
    while (1) {
        ...
        if ((need & CEPH_CAP_FILE_WR) && in->auth_cap && in->auth_cap->session->readonly) return -CEPHFS_EROFS;
...
void Client::check_caps(Inode* in, unsigned flags)
{
    ...
    ack:
        ...
        send_cap(in, session, &cap, msg_flags, cap_used, wanted, retain, flushing, flush_tid);
    }
}
...
void Client::trim_caps(MetaSession* s, uint64_t max)
{
    ...
    while ((caps_size - trimmed) > max && !p.end()) {
        ...
        if (in->dirty_caps || in->cap_snaps.size()) cap_delay_requeue(in.get());
        ...
        if (in->caps.size() > 1 && cap != in->auth_cap) {
            ...
            if (all && in->ino != CEPH_INO_ROOT) {
                ...
                if (!in->dirty_caps && !in->cap_snaps.size()) trimmed++;
            }
        }
    }
...
void SnapRealm::build_snap_context()
{
    ...
    for (unsigned i = 0; i < prior_parent_snaps.size(); i++) snaps.insert(prior_parent_snaps[i]);

    // current parent's snaps
    if (pparent) {
        const SnapContext& psnapc = pparent->get_snap_context();
        for (unsigned i = 0; i < psnapc.snaps.size(); i++)
            if (psnapc.snaps[i] >= parent_since) snaps.insert(psnapc.snaps[i]);
        if (psnapc.seq > max_seq) max_seq = psnapc.seq;
    }

    // my snaps
    for (unsigned i = 0; i < my_snaps.size(); i++) snaps.insert(my_snaps[i]);
...
SnapRealm* Client::get_snap_realm(inodeno_t r)
{
    ...
    if (!realm) snap_realms[r] = realm = new SnapRealm(r);
...
void Client::handle_cap_grant(MetaSession* session, Inode* in, Cap* cap, const MConstRef<MClientCaps>& m)
{
    ...
    if ((new_caps & CEPH_CAP_LINK_SHARED) && !(issued & CEPH_CAP_LINK_EXCL)) {
        ...
        if (in->nlink == 0 && (new_caps & (CEPH_CAP_LINK_SHARED | CEPH_CAP_LINK_EXCL))) deleted_inode = true;
    }
...
int Client::may_delete(const char* relpath, const UserPerm& perms)
{
    ...
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -ENOTCONN;
...
void Client::renew_caps()
{
    ...
    for (auto& p : mds_sessions) {
        ...
        if (mdsmap->get_state(p.first) >= MDSMap::STATE_REJOIN) renew_caps(&p.second);
    }
}
...
int Client::get_or_create(Inode* dir, const char* name, Dentry** pdn, bool expect_null)
{
    // lookup
    ...
    if (dir->dir->dentries.count(name)) {
        ...
        if (_dentry_valid(dn)) {
            if (expect_null) return -CEPHFS_EEXIST;
        }
        *pdn = dn;
    }
    else {
...
int Client::_readdir_get_frag(dir_result_t* dirp)
{
    ...
    if (dirp->inode && dirp->inode->snapid == CEPH_SNAPDIR) op = CEPH_MDS_OP_LSSNAP;
...
int Client::readdir_r_cb(dir_result_t* d, add_dirent_cb_t cb, void* p, unsigned want, unsigned flags, bool getref)
{
    ...
    ldout(cct, 10) << "offset " << hex << dirp->offset << dec << " snapid " << dirp->inode->snapid
                   << " (complete && ordered) " << dirp->inode->is_complete_and_ordered() << " issued "
                   << ccap_string(dirp->inode->caps_issued()) << dendl;
    if (dirp->inode->snapid != CEPH_SNAPDIR && dirp->inode->is_complete_and_ordered() &&
    ...
    while (1) {
        ...
        ldout(cct, 10) << "frag " << fg << " buffer size " << dirp->buffer.size() << " offset " << hex << dirp->offset
                       << dendl;
        ...
        if (diri->shared_gen == dirp->start_shared_gen && diri->dir_release_count == dirp->release_count) {
...
int Client::create_and_open(std::optional<int> dirfd, const char* relpath, int flags, const UserPerm& perms,
...
int Client::_openat(int dirfd, const char* relpath, int flags, const UserPerm& perms, mode_t mode,
                    std::string alternate_name)
{
...
int Client::_setxattr(Inode* in, const char* name, const void* value, size_t size, int flags, const UserPerm& perms)
{
    ...
    if (ret >= 0 && check_realm) {
        ...
        if (in->quota.is_enable() && !(in->snaprealm && in->snaprealm->ino == in->ino)) ret = -CEPHFS_EOPNOTSUPP;
    }
...
{
    return in->quota.is_enable() && (in->snapid != CEPH_NOSNAP || (in->snaprealm && in->snaprealm->ino == in->ino));
}
                    ...
                    std::string alternate_name)
{
    ...
    if (cct->_conf.get_val<bool>("client_quota") && fromdir != todir) {
        Inode* fromdir_root = fromdir->quota.is_enable() ? fromdir : get_quota_root(fromdir, perm);
        Inode* todir_root = todir->quota.is_enable() ? todir : get_quota_root(todir, perm);
