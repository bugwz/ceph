...
#define tout(cct) \
    if (!cct->_conf->client_trace.empty()) traceout
...
double calc_sq_sum(double old_sq_sum, double old_mean, double new_mean, double value, uint64_t count)
{
    double new_sq_sum;
    if (count == 1) {
        new_sq_sum = 0.0;
...
int Client::CommandHook::call(std::string_view command, const cmdmap_t& cmdmap, const bufferlist&, Formatter* f,
                              std::ostream& errss, bufferlist& out)
{
    ...
    , fscid(0)
{
    ...
    _collect_and_send_global_metrics = cct->_conf.get_val<bool>("client_collect_and_send_global_metrics");
    ...
    if (cct->_conf->client_acl_type == "posix_acl") acl_type = POSIX_ACL;
    ...
    // osd interfaces
    writeback_handler.reset(new ObjecterWriteback(objecter, &objecter_finisher, &client_lock));
    objectcacher.reset(new ObjectCacher(cct,
                                        "libcephfs",
                                        *writeback_handler,
                                        client_lock,
...
Client::~Client()
{
    ...
    if (upkeeper.joinable()) upkeeper.join();
...
void Client::dump_inode(Formatter* f, Inode* in, set<Inode*>& did, bool disconnected)
{
    ...
    ldout(cct, 1) << "dump_inode: " << (disconnected ? "DISCONNECTED " : "") << "inode " << in->ino << " " << path
                  << " ref " << in->get_nref() << " " << *in << dendl;

    if (f) {
        f->open_object_section("inode");
        f->dump_stream("path") << path;
        if (disconnected) f->dump_int("disconnected", 1);
        in->dump(f);
        f->close_section();
    }
    ...
    if (in->dir) {
        ...
        for (ceph::unordered_map<string, Dentry*>::iterator it = in->dir->dentries.begin();
             ...
             ++it) {
            ldout(cct, 1) << "   " << in->ino << " dn " << it->first << " " << it->second << " ref " << it->second->ref
                          << dendl;
            if (f) {
            ...
            if (it->second->inode) dump_inode(f, it->second->inode.get(), did, false);
        }
    }
}
...
void Client::dump_cache(Formatter* f)
{
    ...
    if (f) f->open_array_section("cache");

    if (root) dump_inode(f, root.get(), did, true);

    // make a second pass to catch anything disconnected
    for (ceph::unordered_map<vinodeno_t, Inode*>::iterator it = inode_map.begin(); it != inode_map.end(); ++it) {
...
void Client::dump_status(Formatter* f)
{
    ...
    if (f) {
        ...
        for (const auto& kv : metadata) f->dump_string(kv.first.c_str(), kv.second);
        f->close_section();
...
void Client::_finish_init()
{
    ...
    int ret = admin_socket->register_command("mds_requests", &m_command_hook, "show in-progress mds requests");
    if (ret < 0) {
        lderr(cct) << "error registering admin socket command: " << cpp_strerror(-ret) << dendl;
    }
    ...
    ret = admin_socket->register_command("dump_cache", &m_command_hook, "show in-memory metadata cache contents");
    if (ret < 0) {
        lderr(cct) << "error registering admin socket command: " << cpp_strerror(-ret) << dendl;
    }
    ret =
        admin_socket->register_command("kick_stale_sessions", &m_command_hook, "kick sessions that were remote reset");
    if (ret < 0) {
        lderr(cct) << "error registering admin socket command: " << cpp_strerror(-ret) << dendl;
    }
    ret = admin_socket->register_command("status", &m_command_hook, "show overall client status");
    if (ret < 0) {
        lderr(cct) << "error registering admin socket command: " << cpp_strerror(-ret) << dendl;
    }
}
...
void Client::shutdown()
{
    ...
    if (!iref_writer.is_first_writer()) return;
...
void Client::update_io_stat_read(utime_t latency)
{
    ...
    auto n_sqsum = calc_sq_sum(o_sqsum, o_avg, n_avg, lat_nsec, nr_read_request);
...
void Client::update_io_stat_write(utime_t latency)
{
    ...
    auto n_sqsum = calc_sq_sum(o_sqsum, o_avg, n_avg, lat_nsec, nr_write_request);
...
void Client::trim_cache(bool trim_kernel_dcache)
{
    ...
    if (trim_kernel_dcache && lru.lru_get_size() > max) _invalidate_kernel_dcache();
...
void Client::trim_cache_for_reconnect(MetaSession* s)
{
    ...
    while (lru.lru_get_size() > 0) {
        ...
        if ((dn->inode && dn->inode->caps.count(mds)) || dn->dir->parent_inode->caps.count(mds)) {
            trim_dentry(dn);
            trimmed++;
        }
        else
    ...
    for (list<Dentry*>::iterator p = skipped.begin(); p != skipped.end(); ++p) lru.lru_insert_mid(*p);

    ldout(cct, 20) << __func__ << " mds." << mds << " trimmed " << trimmed << " dentries" << dendl;

    if (s->caps.size() > 0) _invalidate_kernel_dcache();
}

void Client::trim_dentry(Dentry* dn)
{
    ldout(cct, 15) << "trim_dentry unlinking dn " << dn->name << " in dir " << std::hex << dn->dir->parent_inode->ino
                   << std::dec << dendl;
    if (dn->inode) {
...
void Client::update_inode_file_size(Inode* in, int issued, uint64_t size, uint64_t truncate_seq, uint64_t truncate_size)
{
    uint64_t prior_size = in->size;

    if (truncate_seq > in->truncate_seq || (truncate_seq == in->truncate_seq && size > in->size)) {
        ...
        if (in->inline_version < CEPH_INLINE_NONE) {
            ...
            if (size < len) in->inline_data.splice(size, len - size);
        }
    }
    if (truncate_seq >= in->truncate_seq && in->truncate_size != truncate_size) {
        if (in->is_file()) {
...
void Client::update_inode_file_time(Inode* in, int issued, uint64_t time_warp_seq, utime_t ctime, utime_t mtime,
                                    utime_t atime)
{
    ldout(cct, 10) << __func__ << " " << *in << " " << ccap_string(issued) << " ctime " << ctime << " mtime " << mtime
                   << dendl;

    if (time_warp_seq > in->time_warp_seq)
        ldout(cct, 10) << " mds time_warp_seq " << time_warp_seq << " is higher than local time_warp_seq "
                       << in->time_warp_seq << dendl;

    int warn = false;
    // be careful with size, mtime, atime
    if (issued &
        (CEPH_CAP_FILE_EXCL | CEPH_CAP_FILE_WR | CEPH_CAP_FILE_BUFFER | CEPH_CAP_AUTH_EXCL | CEPH_CAP_XATTR_EXCL)) {
        ldout(cct, 30) << "Yay have enough caps to look at our times" << dendl;
        if (ctime > in->ctime) in->ctime = ctime;
        if (time_warp_seq > in->time_warp_seq) {
            ...
            // take max times
            if (mtime > in->mtime) in->mtime = mtime;
            if (atime > in->atime) in->atime = atime;
        }
    ...
    if (warn) {
        ldout(cct, 0) << "WARNING: " << *in << " mds time_warp_seq " << time_warp_seq
                      << " is lower than local time_warp_seq " << in->time_warp_seq << dendl;
    }
}
...
Inode* Client::add_update_inode(InodeStat* st, utime_t from, MetaSession* session, const UserPerm& request_perms)
{
    ...
    if (inode_map.count(st->vino)) {
        ...
        if (use_faked_inos()) _assign_faked_ino(in);

        if (!root) {
            root = in;
            if (use_faked_inos()) _assign_faked_root(root.get());
            root_ancestor = in;
            cwd = root;
        }
        else if (is_mounting()) {
    ...
    in->rdev = st->rdev;
    if (in->is_symlink()) in->symlink = st->symlink;

    // only update inode if mds info is strictly newer, or it is the same and projected (odd).
    bool new_version = false;
    if (in->version == 0 || ((st->cap.flags & CEPH_CAP_FLAG_AUTH) && (in->version & ~1) < st->version))
        new_version = true;
    ...
    if ((in->xattr_version == 0 || !(issued & CEPH_CAP_XATTR_EXCL)) && st->xattrbl.length() &&
    ...
    ldout(cct, 12) << __func__ << " client inode change_attr: " << in->change_attr
                   << " , mds inodestat change_attr:  " << st->change_attr << dendl;
    if (st->change_attr > in->change_attr) in->change_attr = st->change_attr;

    if (st->version > in->version) in->version = st->version;

    if (was_new) ldout(cct, 12) << __func__ << " adding " << *in << " caps " << ccap_string(st->cap.caps) << dendl;

    if (!st->cap.caps) return in;   // as with readdir returning indoes in different snaprealms (no caps!)
    ...
    if (in->snapid == CEPH_NOSNAP) {
        ...
        if (in->is_dir() && (st->cap.caps & CEPH_CAP_FILE_SHARED) && (issued & CEPH_CAP_FILE_EXCL) == 0 &&
            in->dirstat.nfiles == 0 && in->dirstat.nsubdirs == 0) {
            ldout(cct, 10) << " marking (I_COMPLETE|I_DIR_ORDERED) on empty dir " << *in << dendl;
            in->flags |= I_COMPLETE | I_DIR_ORDERED;
            if (in->dir) {
                ldout(cct, 10) << " dir is open on empty dir " << in->ino << " with " << in->dir->dentries.size()
                               << " entries, marking all dentries null" << dendl;
                in->dir->readdir_cache.clear();
                for (const auto& p : in->dir->dentries) {
                    unlink(p.second, true, true);   // keep dir, keep dentry
                }
                if (in->dir->dentries.empty()) close_dir(in->dir);
            }
        }
    }
    else {
 ...
 */
Dentry* Client::insert_dentry_inode(Dir* dir, const string& dname, LeaseStat* dlease, Inode* in, utime_t from,
                                    MetaSession* session, Dentry* old_dentry)
{
    Dentry* dn = NULL;
    if (dir->dentries.count(dname)) dn = dir->dentries[dname];

    ldout(cct, 12) << __func__ << " '" << dname << "' vino " << in->vino() << " in dir " << dir->parent_inode->vino()
                   << " dn " << dn << dendl;
...
void Client::update_dentry_lease(Dentry* dn, LeaseStat* dlease, utime_t from, MetaSession* session)
{
    ...
    if (dlease->mask & CEPH_LEASE_VALID) {
        if (dttl > dn->lease_ttl) {
            ldout(cct, 10) << "got dentry lease on " << dn->name << " dur " << dlease->duration_ms << "ms ttl " << dttl
                           << dendl;
            dn->lease_ttl = dttl;
    ...
    if (dlease->mask & CEPH_LEASE_PRIMARY_LINK) dn->mark_primary();
...
void Client::clear_dir_complete_and_ordered(Inode* diri, bool complete)
{
    ...
    if (diri->flags & I_COMPLETE) {
        ...
        if (diri->dir) diri->dir->readdir_cache.clear();
    }
}
 ...
 */
void Client::insert_readdir_results(MetaRequest* request, MetaSession* session, Inode* diri, Inode* diri_other)
{
    ...
    if (!p.end()) {
        ...
        ldout(cct, 10) << __func__ << " " << numdn << " readdir items, end=" << end << ", hash_order=" << hash_order
                       << ", readdir_start " << readdir_start << ", last_hash " << last_hash << ", next_offset "
                       << readdir_offset << dendl;

        if (diri->snapid != CEPH_SNAPDIR && fg.is_leftmost() && readdir_offset == 2 && !(hash_order && last_hash)) {
        ...
        for (unsigned i = 0; i < numdn; i++) {
            ...
            Inode* in = add_update_inode(&ist, request->sent_stamp, session, request->perms);
            ...
            if (hash_order) {
                ...
                if (hash != last_hash) readdir_offset = 2;
                last_hash = hash;
            ...
            if (!snapdiff_req && dirp->release_count == effective_diri->dir_release_count &&
            ...
            ldout(cct, 15) << __func__ << "  " << hex << dn->offset << dec << ": '" << dname << "' -> " << in->ino
                           << dendl;
        }

        if (numdn > 0) dirp->last_name = dname;
        if (end)
            dirp->next_offset = 2;
        else
            dirp->next_offset = readdir_offset;

        if (dir->is_empty()) close_dir(dir);
        if (dir_other && dir_other->is_empty()) close_dir(dir_other);
    }
}
...
Inode* Client::insert_trace(MetaRequest* request, MetaSession* session)
{
    ...
    ldout(cct, 10) << "insert_trace from " << request->sent_stamp << " mds." << session->mds_num
                   << " is_target=" << (int)reply->head.is_target << " is_dentry=" << (int)reply->head.is_dentry
                   << dendl;
    ...
    if (p.end()) {
        ...
        if (d && reply->get_result() == 0) {
            ...
            else if (op == CEPH_MDS_OP_RMDIR || op == CEPH_MDS_OP_UNLINK) {
                // unlink, rmdir
    ...
    if (reply->snapbl.length()) update_snap_trace(session, reply->snapbl, &realm);
    ...
    if (reply->head.is_target) {
        ...
        if (cct->_conf->client_debug_getattr_caps) {
            ...
            if ((wanted & CEPH_CAP_XATTR_SHARED) && !(ist.xattr_version > 0 && ist.xattrbl.length() > 0))
                ceph_abort_msg("MDS reply does not contain xattrs");
        }

        in = add_update_inode(&ist, request->sent_stamp, session, request->perms);
    }

    Inode* diri = NULL;
    if (reply->head.is_dentry) {
        diri = add_update_inode(&dirst, request->sent_stamp, session, request->perms);
        ...
        if (in) {
            ...
            if (diri->dir && diri->dir->dentries.count(dname)) {
                ...
                if (dn->inode) unlink(dn, true, true);   // keep dir, dentry
            }
        }
    }
    ...
    if (in) {
        if (op == CEPH_MDS_OP_READDIR || op == CEPH_MDS_OP_LSSNAP) {
            insert_readdir_results(request, session, in, nullptr);
        }
            ...
            insert_readdir_results(request, session, request->inode(), in);
        }
...
mds_rank_t Client::choose_target_mds(MetaRequest* req, Inode** phash_diri)
{
    ...
    if (cct->_conf->client_use_random_mds) goto random_mds;
    ...
    if (in) {
        ...
        if (req->path.depth()) {
            ...
            ldout(cct, 20) << __func__ << " inode dir hash is " << (int)in->dir_layout.dl_dir_hash << " on "
        ...
        if (de->inode) {
            ...
            ldout(cct, 20) << __func__ << " dentry dir hash is " << (int)in->dir_layout.dl_dir_hash << " on "
                           << de->name << " => " << hash << dendl;
            is_hash = true;
        }
    }
    if (in) {
        ...
        if (req->get_op() == CEPH_MDS_OP_GETATTR) issued = req->inode()->caps_issued();
...
void Client::connect_mds_targets(mds_rank_t mds)
{
    ...
    for (const auto& rank : info.export_targets) {
        if (mds_sessions.count(rank) == 0 && mdsmap->is_clientreplay_or_active_or_stopping(rank)) {
            ldout(cct, 10) << "check_mds_sessions opening mds." << mds << " export target mds." << rank << dendl;

            auto session = _get_or_open_mds_session(rank);
            if (session->state == MetaSession::STATE_OPENING || session->state == MetaSession::STATE_OPEN) continue;
...
int Client::verify_reply_trace(int r, MetaSession* session, MetaRequest* request, const MConstRef<MClientReply>& reply,
                               InodeRef* ptarget, bool* pcreated, const UserPerm& perms)
{
    ...
    if (pcreated) *pcreated = got_created_ino;
    ...
    if (request->target) {
        *ptarget = request->target;
        ...
        if (got_created_ino && (p = inode_map.find(vinodeno_t(created_ino, CEPH_NOSNAP))) != inode_map.end()) {
            ...
            if (d) {
                if (d->dir) {
                    ldout(cct, 10) << "make_request got traceless reply, looking up #" << d->dir->parent_inode->ino
                                   << "/" << d->name << " got_ino " << got_created_ino << " ino " << created_ino
                                   << dendl;
                    r = _do_lookup(d->dir->parent_inode, d->name, request->regetattr_mask, &target, perms);
                }
                else {
            ...
            if (r >= 0) {
                ...
                if (got_created_ino && created_ino.val != target->ino.val) {
                    ldout(cct, 5) << "create got ino " << created_ino << " but then failed on lookup; EINTR?" << dendl;
                    r = -CEPHFS_EINTR;
                }
                if (ptarget) ptarget->swap(target);
            }
        }
    }
 ...
 */
int Client::make_request(MetaRequest* request, const UserPerm& perms, InodeRef* ptarget, bool* pcreated,
                         mds_rank_t use_mds, bufferlist* pdirbl, size_t feature_needed)
{
    int r = 0;
    ...
    if (oldest_tid == 0 && request->get_op() != CEPH_MDS_OP_SETFILELOCK) oldest_tid = tid;
    ...
    // hack target mds?
    if (use_mds >= 0) request->resend_mds = use_mds;
    ...
    while (1) {
        ...
        if (!have_open_session(mds)) {
            ...
            if (!have_open_session(mds)) continue;
        }
        else {
    ...
    if (r >= 0 && ptarget) r = verify_reply_trace(r, session.get(), request, reply, ptarget, pcreated, perms);

    if (pdirbl) *pdirbl = reply->get_extra_bl();
...
void Client::put_request(MetaRequest* request)
{
    if (request->_put()) {
        int op = -1;
        if (request->success) op = request->get_op();
        InodeRef other_in;
        request->take_other_inode(&other_in);
        delete request;

        if (other_in && (op == CEPH_MDS_OP_RMDIR || op == CEPH_MDS_OP_RENAME || op == CEPH_MDS_OP_RMSNAP)) {
            _try_to_trim_inode(other_in.get(), false);
        }
    }
}

int Client::encode_inode_release(Inode* in, MetaRequest* req, mds_rank_t mds, int drop, int unless, int force)
{
    ldout(cct, 20) << __func__ << " enter(in:" << *in << ", req:" << req << " mds:" << mds
                   << ", drop:" << ccap_string(drop) << ", unless:" << ccap_string(unless) << ", force:" << force << ")"
                   << dendl;
    int released = 0;
    ...
    if (it != in->caps.end()) {
        ...
        if ((drop & cap.issued) && !(unless & cap.issued)) {
        ...
        if (released) {
            ...
            if (&cap == in->auth_cap && !(cap.wanted & CEPH_CAP_ANY_FILE_WR)) {
...
void Client::encode_dentry_release(Dentry* dn, MetaRequest* req, mds_rank_t mds, int drop, int unless)
{
    ldout(cct, 20) << __func__ << " enter(dn:" << dn << ")" << dendl;
    int released = 0;
    if (dn->dir) released = encode_inode_release(dn->dir->parent_inode, req, mds, drop, unless, 1);
...
void Client::encode_cap_releases(MetaRequest* req, mds_rank_t mds)
{
    ...
    if (req->inode_drop && req->inode())
        encode_inode_release(req->inode(), req, mds, req->inode_drop, req->inode_unless);

    if (req->old_inode_drop && req->old_inode())
        encode_inode_release(req->old_inode(), req, mds, req->old_inode_drop, req->old_inode_unless);
    if (req->other_inode_drop && req->other_inode())
        encode_inode_release(req->other_inode(), req, mds, req->other_inode_drop, req->other_inode_unless);

    if (req->dentry_drop && req->dentry())
        encode_dentry_release(req->dentry(), req, mds, req->dentry_drop, req->dentry_unless);

    if (req->old_dentry_drop && req->old_dentry())
        encode_dentry_release(req->old_dentry(), req, mds, req->old_dentry_drop, req->old_dentry_unless);
...
bool Client::have_open_session(mds_rank_t mds)
{
    ...
    return it != mds_sessions.end() &&
           (it->second->state == MetaSession::STATE_OPEN || it->second->state == MetaSession::STATE_STALE);
}
...
void Client::update_metadata(std::string const& k, std::string const& v)
{
    ...
    if (it != metadata.end()) {
        ldout(cct, 1) << __func__ << " warning, overriding metadata field '" << k << "' from '" << it->second
                      << "' to '" << v << "'" << dendl;
    }
...
MetaSessionRef Client::_open_mds_session(mds_rank_t mds)
{
    ...
    auto em =
        mds_sessions.emplace(std::piecewise_construct,
...
void Client::_closed_mds_session(MetaSession* s, int err, bool rejected)
{
    ...
    if (s->state == MetaSession::STATE_CLOSED) mds_sessions.erase(s->mds_num);
}

static void reinit_mds_features(MetaSession* session, const MConstRef<MClientSession>& m)
{
...
void Client::handle_client_session(const MConstRef<MClientSession>& m)
{
    ...
    case CEPH_SESSION_OPEN:
    {
        if (session->state == MetaSession::STATE_OPEN) {
            ldout(cct, 10) << "mds." << from << " already opened, ignore it" << dendl;
         ...
         */
        if (!session->seq && m->get_seq()) session->seq = m->get_seq();
    ...
    case CEPH_SESSION_CLOSE: _closed_mds_session(session.get()); break;

    case CEPH_SESSION_RENEWCAPS:
        if (session->cap_renew_seq == m->get_seq()) {
            bool was_stale = ceph_clock_now() >= session->cap_ttl;
            session->cap_ttl = session->last_cap_renew_request + mdsmap->get_session_timeout();
            if (was_stale) wake_up_session_caps(session.get(), false);
        }
        break;
    ...
    case CEPH_SESSION_FORCE_RO: force_session_readonly(session.get()); break;
...
void Client::_kick_stale_sessions()
{
    ...
    for (auto it = mds_sessions.begin(); it != mds_sessions.end();) {
        ...
        if (s->state == MetaSession::STATE_STALE) _closed_mds_session(s.get());
    }
}

void Client::send_request(MetaRequest* request, MetaSession* session, bool drop_cap_releases)
{
    // make the request
    ...
    if (request->got_unsafe) {
        r->set_replayed_op();
        if (request->target) r->head.ino = request->target->ino;
    }
    else {
    ...
    if (r->head.op == CEPH_MDS_OP_SETXATTR) {
        objecter->with_osdmap([r](const OSDMap& o) { r->set_osdmap_epoch(o.get_epoch()); });
    }
...
ref_t<MClientRequest> Client::build_client_request(MetaRequest* request, mds_rank_t mds)
{
    ...
    if (request->retry_attempt) {
        ...
        if ((old_version && request->retry_attempt >= old_max_retry) ||
            ...
            ldout(cct, 1) << __func__ << " request tid " << request->tid << " retry seq overflow"
                          << ", abort it" << dendl;
            return nullptr;
        }
    }
    ...
    if (request->path.empty()) {
        ...
        if (in)
            ...
            if (de->inode)
                ...
                ldout(cct, 1) << "Warning -- unable to construct a filepath!"
                              << " No path, inode, or appropriately-endowed dentry given!" << dendl;
        }
        else
            ldout(cct, 1) << "Warning -- unable to construct a filepath!"
                          << " No path, inode, or dentry given!" << dendl;
    }
...
void Client::handle_client_request_forward(const MConstRef<MClientRequestForward>& fwd)
{
    ...
    if (num_fwd <= request->num_fwd || (uint32_t)num_fwd >= UINT32_MAX) {
        request->abort(-CEPHFS_EMULTIHOP);
        request->caller_cond->notify_all();
        ldout(cct, 0) << __func__ << " request tid " << tid << " new num_fwd " << num_fwd << " old num_fwd "
                      << request->num_fwd << ", fwd seq overflow"
                      << ", abort it" << dendl;
        return;
    }
    ...
    // resend.
    ldout(cct, 10) << __func__ << " tid " << tid << " fwd " << fwd->get_num_fwd() << " to mds." << fwd->get_dest_mds()
                   << ", resending to " << fwd->get_dest_mds() << dendl;
...
{
    int op = req->get_op();
    if (op == CEPH_MDS_OP_MKNOD || op == CEPH_MDS_OP_LINK || op == CEPH_MDS_OP_UNLINK || op == CEPH_MDS_OP_RENAME ||
        op == CEPH_MDS_OP_MKDIR || op == CEPH_MDS_OP_RMDIR || op == CEPH_MDS_OP_SYMLINK || op == CEPH_MDS_OP_CREATE)
        return true;
    return false;
}
...
void Client::handle_client_reply(const MConstRef<MClientReply>& reply)
{
    ...
    if (mds_requests.count(tid) == 0) {
        lderr(cct) << __func__ << " no pending request on tid " << tid << " safe is:" << is_safe << dendl;
        return;
    }
    ...
    if (request->mds != mds_num) {
        ldout(cct, 0) << "got a stale reply from mds." << mds_num << " instead of mds." << request->mds << dendl;
        return;
    }

    if (request->got_unsafe && !is_safe) {
        // duplicate response
        ldout(cct, 0) << "got a duplicate reply on tid " << tid << " from mds " << mds_num << " safe:" << is_safe
                      << dendl;
        return;
    }
    ...
    if (is_unmounting()) mount_cond.notify_all();
}
...
void Client::_handle_full_flag(int64_t pool)
{
    ...
    // in this pool.
    for (unordered_map<vinodeno_t, Inode*>::iterator i = inode_map.begin(); i != inode_map.end(); ++i) {
        Inode* inode = i->second;
        if (inode->oset.dirty_or_tx && (pool == -1 || inode->layout.pool_id == pool)) {
...
void Client::handle_osd_map(const MConstRef<MOSDMap>& m)
{
    ...
    bool new_blocklist = objecter->with_osdmap([&](const OSDMap& o) { return o.is_blocklisted(myaddrs); });
    ...
    if (blocklisted) {
        ...
        blocklisted = objecter->with_osdmap([myaddrs](const OSDMap& o) { return o.is_blocklisted(myaddrs); });
    }
    ...
    if (objecter->osdmap_full_flag()) {
        ...
        for (auto p : full_pools) _handle_full_flag(p);
...
bool Client::ms_dispatch2(const MessageRef& m)
{
    ...
    case CEPH_MSG_MDS_MAP: handle_mds_map(ref_cast<MMDSMap>(m)); break;
    case CEPH_MSG_FS_MAP: handle_fs_map(ref_cast<MFSMap>(m)); break;
    case CEPH_MSG_FS_MAP_USER: handle_fs_map_user(ref_cast<MFSMapUser>(m)); break;
    case CEPH_MSG_CLIENT_SESSION: handle_client_session(ref_cast<MClientSession>(m)); break;
    ...
    case CEPH_MSG_OSD_MAP:
        ...
        // requests
    case CEPH_MSG_CLIENT_REQUEST_FORWARD: handle_client_request_forward(ref_cast<MClientRequestForward>(m)); break;
    case CEPH_MSG_CLIENT_REPLY: handle_client_reply(ref_cast<MClientReply>(m)); break;

    // reclaim reply
    case CEPH_MSG_CLIENT_RECLAIM_REPLY: handle_client_reclaim_reply(ref_cast<MClientReclaimReply>(m)); break;

    case CEPH_MSG_CLIENT_SNAP: handle_snap(ref_cast<MClientSnap>(m)); break;
    case CEPH_MSG_CLIENT_CAPS: handle_caps(ref_cast<MClientCaps>(m)); break;
    case CEPH_MSG_CLIENT_LEASE: handle_lease(ref_cast<MClientLease>(m)); break;
    case MSG_COMMAND_REPLY:
        ...
        break;
    case CEPH_MSG_CLIENT_QUOTA: handle_quota(ref_cast<MClientQuota>(m)); break;
    ...
    if (is_unmounting()) {
        ...
        if (size > lru.lru_get_size() + inode_map.size()) {
            ...
            ldout(cct, 10) << "unmounting: trim pass, size still " << lru.lru_get_size() << "+" << inode_map.size()
                           << dendl;
        }
    }
...
void Client::cancel_commands(const MDSMap& newmap)
{
    ...
    for (const auto& [tid, op] : commands) {
        ...
        if (newmap.is_dne_gid(op_mds_gid) || newmap.is_laggy_gid(op_mds_gid)) {
             ...
             */
            op.con->mark_down();
            if (op.on_finish) op.on_finish->complete(-CEPHFS_ETIMEDOUT);
        }
    }

    for (const auto& tid : cancel_ops) command_table.erase(tid);
}
...
void Client::handle_mds_map(const MConstRef<MMDSMap>& m)
{
    ...
    if (m->get_epoch() <= mdsmap->get_epoch()) {
        ldout(cct, 1) << __func__ << " epoch " << m->get_epoch() << " is identical to or older than our "
    ...
    for (auto p = mds_sessions.begin(); p != mds_sessions.end();) {
        ...
        else if (newstate == MDSMap::STATE_NULL && mds >= mdsmap->get_max_mds()) {
            _closed_mds_session(session.get());
        }
    }
...
void Client::send_reconnect(MetaSession* session)
{
    ...
    for (ceph::unordered_map<vinodeno_t, Inode*>::iterator p = inode_map.begin(); p != inode_map.end(); ++p) {
        Inode* in = p->second;
        auto it = in->caps.find(mds);
        if (it != in->caps.end()) {
            if (allow_multi && m->get_approx_size() >= static_cast<size_t>((std::numeric_limits<int>::max() >> 1))) {
                m->mark_more();
                session->con->send_message2(std::move(m));
            ...
            Cap& cap = it->second;
            ldout(cct, 10) << " caps on " << p->first << " " << ccap_string(cap.issued) << " wants "
                           << ccap_string(in->caps_wanted()) << dendl;
            filepath path;
            ...
            if (!in->cap_snaps.empty()) snap_follows = in->cap_snaps.begin()->first;
    ...
    if (!allow_multi) m->set_encoding_version(0);   // use connection features to choose encoding
    session->con->send_message2(std::move(m));

    mount_cond.notify_all();

    if (session->reclaim_state == MetaSession::RECLAIMING) signal_cond_list(waiting_for_reclaim);
}
...
void Client::kick_requests(MetaSession* session)
{
    ...
    for (map<ceph_tid_t, MetaRequest*>::iterator p = mds_requests.begin(); p != mds_requests.end(); ++p) {
        ...
        if (req->retry_attempt > 0) continue;   // new requests only
...
{
    for (xlist<MetaRequest*>::iterator iter = session->unsafe_requests.begin(); !iter.end(); ++iter)
    ...
    for (map<ceph_tid_t, MetaRequest*>::iterator p = mds_requests.begin(); p != mds_requests.end(); ++p) {
        ...
        if (req->retry_attempt == 0) continue;   // old requests only
        if (req->mds == session->mds_num) send_request(req, session, true);
    }
}
...
void Client::wait_unsafe_requests()
{
    ...
    for (list<MetaRequest*>::iterator p = last_unsafe_reqs.begin(); p != last_unsafe_reqs.end(); ++p) {
        MetaRequest* req = *p;
        if (req->unsafe_item.is_on_list()) wait_on_list(req->waitfor_safe);
        put_request(req);
    }
}
...
{
    ldout(cct, 10) << __func__ << " for mds." << session->mds_num << dendl;
    for (map<ceph_tid_t, MetaRequest*>::iterator p = mds_requests.begin(); p != mds_requests.end();) {
        MetaRequest* req = p->second;
        ++p;
        if (req->mds == session->mds_num) {
            if (req->caller_cond) {
            ...
            if (req->got_unsafe) {
                ...
                if (is_dir_operation(req)) {
                    ...
                    lderr(cct) << "kick_requests_closed drop req of inode(dir) : " << dir->ino << " " << req->get_tid()
                               << dendl;
                ...
                if (req->target) {
                    ...
                    lderr(cct) << "kick_requests_closed drop req of inode : " << in->ino << " " << req->get_tid()
                               << dendl;
...
revoke:
{
    auto reply = make_message<MClientLease>(
        CEPH_MDS_LEASE_RELEASE, seq, m->get_mask(), m->get_ino(), m->get_first(), m->get_last(), m->dname);
...
void Client::_put_inode(Inode* in, int n)
{
    ...
    if (left == 1) {   // the last one will be held by the inode_map
        ...
        if (use_faked_inos()) _release_faked_ino(in);

        if (root == nullptr) {
            root_ancestor = 0;
            while (!root_parents.empty()) root_parents.erase(root_parents.begin());
        }
...
void Client::delay_put_inodes(bool wakeup)
{
    ...
    for (auto& [in, cnt] : release) _put_inode(in, cnt);

    if (wakeup) mount_cond.notify_all();
}
...
void Client::close_dir(Dir* dir)
{
    ...
    if (!in->dentries.empty()) in->get_first_parent()->put();   // unpin dentry
...
Dentry* Client::link(Dir* dir, const string& name, Inode* in, Dentry* dn)
{
    if (!dn) {
        ...
        if (in) {
            ldout(cct, 15) << "link dir " << *dir->parent_inode << " '" << name << "' to inode " << *in << " dn " << *dn
                           << " (new dn)" << dendl;
        }
        else {
...
void Client::unlink(Dentry* dn, bool keepdir, bool keepdentry)
{
    ...
    ldout(cct, 15) << "unlink dir " << dn->dir->parent_inode << " '" << dn->name << "' dn " << dn << " inode "
                   << dn->inode << dendl;
    ...
    if (keepdentry) {
        ...
        if (dir->is_empty() && !keepdir) close_dir(dir);
    }
}
...
public:
    ...
    void finish(int r) override
    {
        ...
        if (r != 0) {
            ...
            ldout(client->cct, 1) << "I/O error from flush on inode " << inode << " 0x" << std::hex << inode->ino
                                  << std::dec << ": " << r << "(" << cpp_strerror(r) << ")" << dendl;
...
{
    if ((cap & CEPH_CAP_FILE_BUFFER) && in->cap_refs[CEPH_CAP_FILE_BUFFER] == 0) {
        ldout(cct, 5) << __func__ << " got first FILE_BUFFER ref on " << *in << dendl;
        in->iget();
    }
    if ((cap & CEPH_CAP_FILE_CACHE) && in->cap_refs[CEPH_CAP_FILE_CACHE] == 0) {
...
void Client::put_cap_ref(Inode* in, int cap)
{
    ...
    if (last) {
        ...
        if (in->snapid == CEPH_NOSNAP) {
            if ((last & CEPH_CAP_FILE_WR) && !in->cap_snaps.empty() && in->cap_snaps.rbegin()->second.writing) {
            ...
            if (last & CEPH_CAP_FILE_BUFFER) {
                for (auto& p : in->cap_snaps) p.second.dirty_data = 0;
        ...
        if (drop) check_caps(in, 0);
        if (put_nref) put_inode(in, put_nref);
    }
}
...
int Client::get_caps(Fh* fh, int need, int want, int* phave, loff_t endoff)
{
    ...
    while (1) {
        ...
        if ((file_wanted & need) != need) {
            ldout(cct, 10) << "get_caps " << *in << " need " << ccap_string(need) << " file_wanted "
                           << ccap_string(file_wanted) << ", EBADF " << dendl;
            return -CEPHFS_EBADF;
        }

        if ((fh->mode & CEPH_FILE_MODE_WR) && fh->gen != fd_gen) return -CEPHFS_EBADF;

        if ((in->flags & I_ERROR_FILELOCK) && fh->has_any_filelocks()) return -CEPHFS_EIO;
        ...
        if (have & need & CEPH_CAP_FILE_WR) {
            if (endoff > 0) {
                if ((endoff >= (loff_t)in->max_size || endoff > (loff_t)(in->size << 1)) &&
            ...
            if (endoff >= 0 && endoff > (loff_t)in->max_size) {
                ldout(cct, 10) << "waiting on max_size, endoff " << endoff << " max_size " << in->max_size << " on "
                               << *in << dendl;
                waitfor_caps = true;
            }
            if (!in->cap_snaps.empty()) {
        ...
        if (!waitfor_caps && !waitfor_commit) {
            if ((have & need) == need) {
                ...
                ldout(cct, 10) << "get_caps " << *in << " have " << ccap_string(have) << " need " << ccap_string(need)
                               << " want " << ccap_string(want) << " revoking " << ccap_string(revoking) << dendl;
                if ((revoking & want) == 0) {
            ...
            ldout(cct, 10) << "waiting for caps " << *in << " need " << ccap_string(need) << " want "
                           << ccap_string(want) << dendl;
            waitfor_caps = true;
        }

        if ((need & CEPH_CAP_FILE_WR) && ((in->auth_cap && in->auth_cap->session->readonly) ||
        ...
        if (in->flags & I_CAP_DROPPED) {
            ...
            if (!(file_wanted & ~mds_wanted)) in->flags &= ~I_CAP_DROPPED;
        }
...
int Client::get_caps_used(Inode* in)
{
    ...
    if (!(used & CEPH_CAP_FILE_CACHE) && !objectcacher->set_is_empty(&in->oset)) used |= CEPH_CAP_FILE_CACHE;
    return used;
}
...
void Client::send_cap(Inode* in, MetaSession* session, Cap* cap, int flags, int used, int want, int retain, int flush,
                      ceph_tid_t flush_tid)
{
    ...
    ldout(cct, 10) << __func__ << " " << *in << " mds." << session->mds_num << " seq " << cap->seq << " used "
                   << ccap_string(used) << " want " << ccap_string(want) << " flush " << ccap_string(flush)
                   << " retain " << ccap_string(retain) << " held " << ccap_string(held) << " revoking "
                   << ccap_string(revoking) << " dropping " << ccap_string(dropping) << dendl;
    ...
    if (cct->_conf->client_inject_release_failure && revoking) {
        ...
        ldout(cct, 20) << __func__ << " issued " << ccap_string(cap->issued) << " vs " << ccap_string(would_have_issued)
                       << dendl;
        ldout(cct, 20) << __func__ << " implemented " << ccap_string(cap->implemented) << " vs "
                       << ccap_string(would_have_implemented) << dendl;
    }
    else {
        // Normal behaviour
        cap->issued &= retain;
    ...
    if (flush) follows = in->snaprealm->get_snap_context().seq;

    auto m = make_message<MClientCaps>(
        op, in->ino, 0, cap->cap_id, cap->seq, cap->implemented, want, flush, cap->mseq, cap_epoch_barrier);
    ...
    if (!(flags & MClientCaps::FLAG_PENDING_CAPSNAP) && !in->cap_snaps.empty() &&
    ...
    if (!session->flushing_caps_tids.empty()) m->set_oldest_flush_tid(*session->flushing_caps_tids.begin());
...
static bool is_max_size_approaching(Inode* in)
{
    ...
    if (in->flushing_caps & CEPH_CAP_FILE_WR) return false;
    if (in->size >= in->max_size) return true;
    /* half of previous max_size increment has been used */
    if (in->max_size > in->reported_size && (in->size << 1) >= in->max_size + in->reported_size) return true;
    return false;
}

static int adjust_caps_used_for_lazyio(int used, int issued, int implemented)
{
    if (!(used & (CEPH_CAP_FILE_CACHE | CEPH_CAP_FILE_BUFFER))) return used;
    if (!(implemented & CEPH_CAP_FILE_LAZYIO)) return used;
...
void Client::check_caps(Inode* in, unsigned flags)
{
    ...
    if (!is_unmounting() && in->nlink > 0) {
        if (wanted) {
        ...
        else if (in->is_dir() && (issued & CEPH_CAP_FILE_SHARED) && (in->flags & I_COMPLETE)) {
            ...
            // journal max_size=0.
            if (in->max_size == 0) retain |= CEPH_CAP_ANY_RD;
        }
    }

    ldout(cct, 10) << __func__ << " on " << *in << " wanted " << ccap_string(wanted) << " used " << ccap_string(used)
                   << " issued " << ccap_string(issued) << " revoking " << ccap_string(revoking) << " flags=" << flags
                   << dendl;

    if (in->snapid != CEPH_NOSNAP) return;   // snap caps last forever, can't write

    if (in->caps.empty()) return;   // guard if at end of func

    if (!(orig_used & CEPH_CAP_FILE_BUFFER) && (revoking & used & (CEPH_CAP_FILE_CACHE | CEPH_CAP_FILE_LAZYIO))) {
        if (_release(in)) used &= ~(CEPH_CAP_FILE_CACHE | CEPH_CAP_FILE_LAZYIO);
    }
    ...
    for (auto& [mds, cap] : in->caps) {
        ...
        cap_used = used;
        if (in->auth_cap && &cap != in->auth_cap) cap_used &= ~in->auth_cap->issued;

        revoking = cap.implemented & ~cap.issued;

        ldout(cct, 10) << " cap mds." << mds << " issued " << ccap_string(cap.issued) << " implemented "
                       << ccap_string(cap.implemented) << " revoking " << ccap_string(revoking) << dendl;

        if (in->wanted_max_size > in->max_size && in->wanted_max_size > in->requested_max_size && &cap == in->auth_cap)
            goto ack;

        /* approaching file_max? */
        if ((cap.issued & CEPH_CAP_FILE_WR) && &cap == in->auth_cap && is_max_size_approaching(in)) {
            ldout(cct, 10) << "size " << in->size << " approaching max_size " << in->max_size << ", reported "
                           << in->reported_size << dendl;
            goto ack;
        }
        ...
        if (wanted & ~(cap.wanted | cap.issued)) goto ack;
    ...
    ack:
        if (&cap == in->auth_cap) {
            ...
            if (!in->cap_snaps.empty() && in->cap_snaps.rbegin()->second.flush_tid == 0) flush_snaps(in);
        }
        ...
        if (in->auth_cap == &cap && in->dirty_caps) {
            ...
            if (flags & CHECK_CAPS_SYNCHRONOUS) msg_flags |= MClientCaps::FLAG_SYNC;
        }
        else {
            flushing = 0;
            flush_tid = 0;
        ...
        send_cap(in, session.get(), &cap, msg_flags, cap_used, wanted, retain, flushing, flush_tid);
    }
}
...
void Client::queue_cap_snap(Inode* in, SnapContext& old_snapc)
{
    ...
    if (in->cap_snaps.size() && in->cap_snaps.rbegin()->second.writing) {
        ldout(cct, 10) << __func__ << " already have pending cap_snap on " << *in << dendl;
        return;
    }
    else if (dirty || (used & CEPH_CAP_FILE_WR)) {
        const auto& capsnapem =
            in->cap_snaps.emplace(std::piecewise_construct, std::make_tuple(old_snapc.seq), std::make_tuple(in));
...
{
    ldout(cct, 10) << __func__ << " " << *in << " capsnap " << (void*)&capsnap << " used " << ccap_string(used)
                   << dendl;
...
void Client::send_flush_snap(Inode* in, MetaSession* session, snapid_t follows, CapSnap& capsnap)
{
    auto m = make_message<MClientCaps>(
        CEPH_CAP_OP_FLUSHSNAP, in->ino, in->snaprealm->ino, 0, in->auth_cap->mseq, cap_epoch_barrier);
...
void Client::flush_snaps(Inode* in)
{
    ...
    for (auto& p : in->cap_snaps) {
        ...
        // only do new flush
        if (capsnap.flush_tid > 0) continue;

        ldout(cct, 10) << "flush_snaps mds." << session->mds_num << " follows " << p.first << " size " << capsnap.size
                       << " mtime " << capsnap.mtime << " dirty_data=" << capsnap.dirty_data
                       << " writing=" << capsnap.writing << " on " << *in << dendl;
        if (capsnap.dirty_data || capsnap.writing) break;
        ...
        if (!in->flushing_cap_item.is_on_list()) session->flushing_caps.push_back(&in->flushing_cap_item);
    ...
    for (const auto& cap : s->caps) {
        ...
        if (reconnect) {
            ...
            if (cap->gen < s->cap_gen) {
                ...
                if (in.caps_file_wanted() & ~cap->wanted) in.flags |= I_CAP_DROPPED;
            }
        }
...
void Client::_async_invalidate(vinodeno_t ino, int64_t off, int64_t len)
{
    ...
    if (!mref_reader.is_state_satisfied()) return;
...
void Client::_invalidate_inode_cache(Inode* in)
{
    ...
    if (cct->_conf->client_oc) {
        ...
        if (!objectcacher->set_is_empty(&in->oset)) lderr(cct) << "failed to invalidate cache for " << *in << dendl;
    }
...
void Client::_flush_range(Inode* in, int64_t offset, uint64_t size)
{
    ...
    bool ret =
        objectcacher->file_flush(&in->oset, &in->layout, in->snaprealm->get_snap_context(), offset, size, &onflush);
    if (!ret) {
        // wait for flush
        client_lock.unlock();
        onflush.wait();
...
void Client::check_cap_issue(Inode* in, unsigned issued)
{
    ...
    if ((issued & CEPH_CAP_FILE_CACHE) && !(had & CEPH_CAP_FILE_CACHE)) in->cache_gen++;

    if ((issued & CEPH_CAP_FILE_SHARED) != (had & CEPH_CAP_FILE_SHARED)) {
        if (issued & CEPH_CAP_FILE_SHARED) in->shared_gen++;
        if (in->is_dir()) clear_dir_complete_and_ordered(in, true);
    }
}

void Client::add_update_cap(Inode* in, MetaSession* mds_session, uint64_t cap_id, unsigned issued, unsigned wanted,
                            unsigned seq, unsigned mseq, inodeno_t realm, int flags, const UserPerm& cap_perms)
{
    if (!in->is_any_caps()) {
        ...
        if ((flags & CEPH_CAP_FLAG_AUTH) && realm != inodeno_t(-1) && in->snaprealm->ino != realm) {
    ...
    const auto& capem =
        in->caps.emplace(std::piecewise_construct, std::forward_as_tuple(mds), std::forward_as_tuple(*in, mds_session));
    Cap& cap = capem.first->second;
    if (!capem.second) {
        if (cap.gen < mds_session->cap_gen) cap.issued = cap.implemented = CEPH_CAP_PIN;
        ...
        if (ceph_seq_cmp(seq, cap.seq) <= 0) {
            if (&cap != in->auth_cap)
                ldout(cct, 0) << "WARNING: "
                              << "inode " << *in << " caps on mds." << mds << " != auth_cap." << dendl;
    ...
    if (flags & CEPH_CAP_FLAG_AUTH) {
        if (in->auth_cap != &cap && (!in->auth_cap || ceph_seq_cmp(in->auth_cap->mseq, mseq) < 0)) {
            if (in->auth_cap) {
    ...
    if (issued & ~old_caps) signal_cond_list(in->waitfor_caps);
}
...
void Client::remove_cap(Cap* cap, bool queue_release)
{
    ...
    if (queue_release) {
        session->enqueue_cap_release(in.ino, cap->cap_id, cap->issue_seq, cap->mseq, cap_epoch_barrier);
    }
    else {
        dec_pinned_icaps();
    }
...
{
    while (!in->caps.empty()) remove_cap(&in->caps.begin()->second, true);
}
...
void Client::remove_session_caps(MetaSession* s, int err)
{
    ...
    while (s->caps.size()) {
        ...
        if (in->auth_cap == cap) {
            ...
            in->wanted_max_size = 0;
            in->requested_max_size = 0;
            if (in->has_any_filelocks()) in->flags |= I_ERROR_FILELOCK;
        }
        auto caps = cap->implemented;
        if (cap->wanted | cap->issued) in->flags |= I_CAP_DROPPED;
...
std::pair<int, bool> Client::_do_remount(bool retry_on_error)
{
    ...
    if (r == 0) {
        ...
        if (r == -1) {
            lderr(cct) << "failed to remount (to trim kernel dentries): "
                          "errno = "
                       << e << " (" << strerror(e) << ")" << dendl;
        }
        else {
...
public:
    explicit C_Client_Remount(Client* c)
        : client(c)
    {}
    void finish(int r) override
    {
    ...
    if (can_invalidate_dentries) {
        if (dentry_invalidate_cb && root->dir) {
            for (ceph::unordered_map<string, Dentry*>::iterator p = root->dir->dentries.begin();
                 ...
                 ++p) {
                if (p->second->inode) _schedule_invalidate_dentry_callback(p->second, false);
            }
        }
    }
    else if (remount_cb) {
...
void Client::_trim_negative_child_dentries(InodeRef& in)
{
    ...
    if (dir && dir->dentries.size() == dir->num_null_dentries) {
        for (auto p = dir->dentries.begin(); p != dir->dentries.end();) {
            ...
            ++p;
            ceph_assert(!dn->inode);
            if (dn->lru_is_expireable()) unlink(dn, true, false);   // keep dir, drop dentry
        }
...
void Client::trim_caps(MetaSession* s, uint64_t max)
{
        ...
        if (in->caps.size() > 1 && cap != in->auth_cap) {
            ...
            while (q != in->dentries.end()) {
                ...
                if (dn->lru_is_expireable()) {
                    if (can_invalidate_dentries && dn->dir->parent_inode->ino == CEPH_INO_ROOT) {
    ...
    if (caps_size > (size_t)max) _invalidate_kernel_dcache();
}
...
void Client::force_session_readonly(MetaSession* s)
{
    ...
    for (xlist<Cap*>::iterator p = s->caps.begin(); !p.end(); ++p) {
        auto& in = (*p)->inode;
        if (in.caps_wanted() & CEPH_CAP_FILE_WR) signal_cond_list(in.waitfor_caps);
    }
}
...
void Client::adjust_session_flushing_caps(Inode* in, MetaSession* old_s, MetaSession* new_s)
{
    ...
    for (map<ceph_tid_t, int>::iterator it = in->flushing_cap_tids.begin(); it != in->flushing_cap_tids.end(); ++it) {
...
void Client::flush_caps_sync()
{
    ...
    for (auto& q : mds_sessions) {
        ...
        while (!p.end()) {
            ...
            ++p;
            if (p.end()) flags |= CHECK_CAPS_SYNCHRONOUS;
            check_caps(in, flags);
        }
    }
}
...
void Client::wait_sync_caps(Inode* in, ceph_tid_t want)
{
    while (in->flushing_caps) {
        ...
        if (it->first > want) break;
        ldout(cct, 10) << __func__ << " on " << *in << " flushing " << ccap_string(it->second) << " want " << want
                       << " last " << it->first << dendl;
        wait_on_list(in->waitfor_caps);
    }
}
...
{
retry:
    ldout(cct, 10) << __func__ << " want " << want << " (last is " << last_flush_tid << ", " << num_flushing_caps
                   << " total flushing)" << dendl;
    for (auto& p : mds_sessions) {
        auto s = p.second;
        if (s->flushing_caps_tids.empty()) continue;
        ceph_tid_t oldest_tid = *s->flushing_caps_tids.begin();
        if (oldest_tid <= want) {
            ldout(cct, 10) << " waiting on mds." << p.first << " tid " << oldest_tid << " (want " << want << ")"
                           << dendl;
            std::unique_lock l{client_lock, std::adopt_lock};
            sync_cond.wait(l);
            l.release();
...
void Client::kick_flushing_caps(Inode* in, MetaSession* session)
{
    ...
    for (auto p = in->flushing_cap_tids.rbegin(); p != in->flushing_cap_tids.rend(); ++p) {
        if (!p->second) {
    ...
    for (auto& p : in->flushing_cap_tids) {
        if (p.second) {
            ...
            send_cap(in, session, cap, msg_flags, used, wanted, (cap->issued | cap->implemented), p.second, p.first);
        }
        else {
...
void Client::invalidate_snaprealm_and_children(SnapRealm* realm)
{
    ...
    while (!q.empty()) {
        ...
        for (set<SnapRealm*>::iterator p = realm->pchildren.begin(); p != realm->pchildren.end(); ++p) q.push_back(*p);
    }
}
...
SnapRealm* Client::get_snap_realm(inodeno_t r)
{
    ...
    ldout(cct, 20) << __func__ << " " << r << " " << realm << ", nref was " << (realm ? realm->nref : 0) << dendl;
    if (!realm) {
        snap_realms[r] = realm = new SnapRealm(r);

        // Do not release the global snaprealm until unmounting.
        if (r == CEPH_INO_GLOBAL_SNAPREALM) realm->nref++;
    }

    realm->nref++;
    ldout(cct, 20) << __func__ << " " << r << " " << realm << ", nref now is " << realm->nref << dendl;
    return realm;
}
...
SnapRealm* Client::get_snap_realm_maybe(inodeno_t r)
{
    ...
    SnapRealm* realm = snap_realms[r];
    ldout(cct, 20) << __func__ << " " << r << " " << realm << " " << realm->nref << " -> " << (realm->nref + 1)
                   << dendl;
    realm->nref++;
    return realm;
}
...
{
    ldout(cct, 20) << __func__ << " " << realm->ino << " " << realm << " " << realm->nref << " -> " << (realm->nref - 1)
                   << dendl;
    if (--realm->nref == 0) {
...
static bool has_new_snaps(const SnapContext& old_snapc, const SnapContext& new_snapc)
{
...
static std::pair<SnapRealmInfo, std::optional<SnapRealmInfoMeta>> get_snap_realm_info(MetaSession* session,
...
void Client::update_snap_trace(MetaSession* session, const bufferlist& bl, SnapRealm** realm_ret, bool flush)
{
    ...
    while (!p.end()) {
        ...
        if (info.seq() > realm->seq || (realm_info_meta && (*realm_info_meta).change_attr > realm->change_attr)) {
            ...
            if (flush) {
                ...
                while (!q.empty()) {
                    ...
                    for (set<SnapRealm*>::iterator p = realm->pchildren.begin(); p != realm->pchildren.end(); ++p)
                        q.push_back(*p);
        ...
        if (adjust_realm_parent(realm, info.parent())) invalidate = true;
...
void Client::handle_snap(const MConstRef<MClientSnap>& m)
{
    ...
    if (m->head.op == CEPH_SNAP_OP_SPLIT) {
        ...
        for (auto& ino : m->split_inos) {
            ...
            if (inode_map.count(vino)) {
                ...
                if (!in->snaprealm || in->snaprealm == realm) continue;
    ...
    if (realm) {
        for (auto p = to_move.begin(); p != to_move.end(); ++p) {
            ...
            if (has_new_snaps(p->second, realm->get_snap_context())) queue_cap_snap(in, p->second);
        }
        put_snap_realm(realm);
    }
}
    ...
    if (auto it = inode_map.find(vino); it != inode_map.end()) {
        ...
        case CEPH_CAP_OP_GRANT:
            if (!in->caps.count(mds)) {
                do_cap_release = true;
                ldout(cct, 5) << __func__ << " vino " << vino << " don't have cap " << m->get_cap_id() << " op "
        ...
        case CEPH_CAP_OP_GRANT:
            do_cap_release = true;
            ldout(cct, 5) << __func__ << " don't have vino " << vino << " op " << m->get_op()
                          << ", immediately releasing" << dendl;
            break;
        default: ldout(cct, 5) << __func__ << " don't have vino " << vino << ", dropping" << dendl; return;
        }
    }

    // In case the mds is waiting on e.g. a revocation
    if (do_cap_release) {
        session->enqueue_cap_release(m->get_ino(), m->get_cap_id(), m->get_seq(), m->get_mseq(), cap_epoch_barrier);
...
void Client::handle_cap_import(MetaSession* session, Inode* in, const MConstRef<MClientCaps>& m)
{
    ...
    ldout(cct, 5) << __func__ << " ino " << m->get_ino() << " mseq " << m->get_mseq() << " IMPORT from mds." << mds
                  << dendl;
    ...
    if (in->auth_cap && in->auth_cap->session == session) {
        if (!(wanted & CEPH_CAP_ANY_FILE_WR) || in->requested_max_size > m->get_max_size()) {
            in->requested_max_size = 0;
...
void Client::handle_cap_export(MetaSession* session, Inode* in, const MConstRef<MClientCaps>& m)
{
    ...
    ldout(cct, 5) << __func__ << " ino " << m->get_ino() << " mseq " << m->get_mseq() << " EXPORT from mds." << mds
                  << dendl;
...
void Client::handle_cap_trunc(MetaSession* session, Inode* in, const MConstRef<MClientCaps>& m)
{
    ...
    if (in->is_fscrypt_enabled()) {
        size = std::stoll(std::string(std::rbegin(m->fscrypt_file), std::rend(m->fscrypt_file)));
    }
    ...
    in->caps_issued(&issued);
    issued |= in->caps_dirty();
    update_inode_file_size(in, issued, size, m->get_truncate_seq(), m->get_truncate_size());
}
...
void Client::handle_cap_flush_ack(MetaSession* session, Inode* in, Cap* cap, const MConstRef<MClientCaps>& m)
{
    ...
    if (it->first < flush_ack_tid) {
        ldout(cct, 0) << __func__ << " mds." << session->mds_num << " got unexpected flush ack tid " << flush_ack_tid
    ...
    for (; it != in->flushing_cap_tids.end();) {
        ...
        if (it->first == flush_ack_tid) cleaned = it->second;
        if (it->first <= flush_ack_tid) {
    ...
    ldout(cct, 5) << __func__ << " mds." << session->mds_num << " cleaned " << ccap_string(cleaned) << " on " << *in
                  << " with " << ccap_string(dirty) << dendl;

    if (flushed) {
        signal_cond_list(in->waitfor_caps);
        if (session->flushing_caps_tids.empty() || *session->flushing_caps_tids.begin() > flush_ack_tid)
            sync_cond.notify_all();
    }
    ...
    if (!cleaned) {
        ...
        if (in->flushing_caps) {
            ...
            if (in->flushing_caps == 0) {
                ...
                num_flushing_caps--;
                if (in->flushing_cap_tids.empty()) in->flushing_cap_item.remove_myself();
            }
            if (!in->caps_dirty()) put_inode(in);
        }
    }
}
...
void Client::handle_cap_flushsnap_ack(MetaSession* session, Inode* in, const MConstRef<MClientCaps>& m)
{
    ...
    if (auto it = in->cap_snaps.find(follows); it != in->cap_snaps.end()) {
        ...
        if (flush_ack_tid != capsnap.flush_tid) {
            ...
            InodeRef tmp_ref(in);
            ldout(cct, 5) << __func__ << " mds." << mds << " flushed snap follows " << follows << " on " << *in
                          << dendl;
            session->flushing_caps_tids.erase(capsnap.flush_tid);
            in->flushing_cap_tids.erase(capsnap.flush_tid);
            if (in->flushing_caps == 0 && in->flushing_cap_tids.empty()) in->flushing_cap_item.remove_myself();
        ...
        ldout(cct, 5) << __func__ << " DUP(?) mds." << mds << " flushed snap follows " << follows << " on " << *in
                      << dendl;
...
public:
        ...
        , name(dn->name)
    {
        if (client->use_faked_inos()) {
            ...
            if (del) ino.ino = dn->inode->faked_ino;
        }
        else {
            dirino = dn->dir->parent_inode->vino();
            if (del) ino = dn->inode->vino();
        }
        if (!del) ino.ino = inodeno_t();
    }
    void finish(int r) override
    {
...
void Client::handle_cap_grant(MetaSession* session, Inode* in, Cap* cap, const MConstRef<MClientCaps>& m)
{
    ...
    ldout(cct, 5) << __func__ << " on in " << m->get_ino() << " mds." << mds << " seq " << m->get_seq() << " caps now "
                  << ccap_string(new_caps) << " was " << ccap_string(cap->issued) << (was_stale ? " (stale)" : "")
                  << dendl;

    if (was_stale) cap->issued = cap->implemented = CEPH_CAP_PIN;
    cap->seq = m->get_seq();
    cap->gen = session->cap_gen;
    ...
    if ((new_caps & CEPH_CAP_AUTH_SHARED) && !(issued & CEPH_CAP_AUTH_EXCL)) {
        in->mode = m->head.mode;
        in->uid = m->head.uid;
        in->gid = m->head.gid;
        in->btime = m->btime;
    ...
    if ((new_caps & CEPH_CAP_LINK_SHARED) && !(issued & CEPH_CAP_LINK_EXCL)) {
        in->nlink = m->head.nlink;
        if (in->nlink == 0) deleted_inode = true;
    }
    if (!(issued & CEPH_CAP_XATTR_EXCL) && m->xattrbl.length() && m->head.xattr_version > in->xattr_version) {
    ...
    if (new_caps & CEPH_CAP_ANY_RD) {
        update_inode_file_time(in, issued, m->get_time_warp_seq(), m->get_ctime(), m->get_mtime(), m->get_atime());
    }

    if (new_caps & (CEPH_CAP_ANY_FILE_RD | CEPH_CAP_ANY_FILE_WR)) {
        in->layout = m->get_layout();
        update_inode_file_size(in, issued, m->get_size(), m->get_truncate_seq(), m->get_truncate_size());
    }
    ...
    /* always take a newer change attr */
    if (m->get_change_attr() > in->change_attr) in->change_attr = m->get_change_attr();

    // max_size
    if (cap == in->auth_cap && (new_caps & CEPH_CAP_ANY_FILE_WR) && (m->get_max_size() != in->max_size)) {
    ...
    bool check = false;
    if ((was_stale || m->get_op() == CEPH_CAP_OP_IMPORT) && (wanted & ~(cap->wanted | new_caps))) {
    ...
    if (check) check_caps(in, flags);

    // wake up waiters
    if (new_caps) signal_cond_list(in->waitfor_caps);

    // may drop inode's last ref
    if (deleted_inode) _try_to_trim_inode(in, true);
}
...
int Client::inode_permission(Inode* in, const UserPerm& perms, unsigned want)
{
    if (perms.uid() == 0) {
        ...
        if (!S_ISDIR(in->mode) && (want & MAY_EXEC) && !(in->mode & S_IXUGO)) return -CEPHFS_EACCES;
        return 0;
    }

    if (perms.uid() != in->uid && (in->mode & S_IRWXG)) {
        int ret = _posix_acl_permission(in, perms, want);
        if (ret != -CEPHFS_EAGAIN) return ret;
    }

    // check permissions before doing anything else
    if (!in->check_mode(perms, want)) return -CEPHFS_EACCES;
    return 0;
}

int Client::xattr_permission(Inode* in, const char* name, unsigned want, const UserPerm& perms)
{
    ...
    if (strncmp(name, "system.", 7) == 0) {
        if ((want & MAY_WRITE) && (perms.uid() != 0 && perms.uid() != in->uid)) r = -CEPHFS_EPERM;
    }
    else {
...
int Client::may_setattr(Inode* in, struct ceph_statx* stx, int mask, const UserPerm& perms)
{
    ldout(cct, 20) << __func__ << " " << *in << "; " << perms << " stx_mode: " << hex << stx->stx_mode
                   << " mask:" << mask << dec << dendl;
    int r = _getattr_for_perm(in, perms);
    if (r < 0) goto out;
    ...
    if (mask & CEPH_SETATTR_UID) {
        if (perms.uid() != 0 && (perms.uid() != in->uid || stx->stx_uid != in->uid)) goto out;
    }
    if (mask & CEPH_SETATTR_GID) {
        if (perms.uid() != 0 &&
            (perms.uid() != in->uid || (!perms.gid_in_groups(stx->stx_gid) && stx->stx_gid != in->gid)))
            goto out;
    }
    ...
    if (mask & CEPH_SETATTR_MODE) {
        ...
        if (perms.uid() != 0 && !perms.gid_in_groups(i_gid)) stx->stx_mode &= ~S_ISGID;
    }
    ...
    if (mask & (CEPH_SETATTR_CTIME | CEPH_SETATTR_BTIME | CEPH_SETATTR_MTIME | CEPH_SETATTR_ATIME)) {
        if (perms.uid() != 0 && perms.uid() != in->uid) {
            ...
            if (!(mask & CEPH_SETATTR_MTIME_NOW)) check_mask |= CEPH_SETATTR_MTIME;
            if (!(mask & CEPH_SETATTR_ATIME_NOW)) check_mask |= CEPH_SETATTR_ATIME;
            if (check_mask & mask) {
                goto out;
            }
            else {
...
int Client::may_open(Inode* in, int flags, const UserPerm& perms)
{
    ...
    case S_IFLNK: r = -CEPHFS_ELOOP; goto out;
    case S_IFDIR:
        if (want & MAY_WRITE) {
            r = -CEPHFS_EISDIR;
            goto out;
...
{
    ldout(cct, 20) << __func__ << " " << *dir << "; "
                   << "; name " << name << "; " << perms << dendl;
    int r = _getattr_for_perm(dir, perms);
    if (r < 0) goto out;
    ...
    if (perms.uid() != 0 && name && (dir->mode & S_ISVTX)) {
        ...
        if (r < 0) goto out;
        if (dir->uid != perms.uid() && otherin->uid != perms.uid()) r = -CEPHFS_EPERM;
    }
out:
    ldout(cct, 3) << __func__ << " " << dir << " = " << r << dendl;
    return r;
...
int Client::may_delete(const char* relpath, const UserPerm& perms)
{
    ...
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::may_hardlink(Inode* in, const UserPerm& perms)
{
    ...
    r = -CEPHFS_EPERM;
    if (!S_ISREG(in->mode)) goto out;

    if (in->mode & S_ISUID) goto out;

    if ((in->mode & (S_ISGID | S_IXGRP)) == (S_ISGID | S_IXGRP)) goto out;
 ...
 * one MDS.
 */
int Client::resolve_mds(const std::string& mds_spec, std::vector<mds_gid_t>* targets)
{
    ceph_assert(fsmap);
    ...
    if (role_r == 0) {
        ...
        ldout(cct, 10) << __func__ << ": resolved " << mds_spec << " to role '" << role << "' aka " << info.human_name()
                       << dendl;
...
int Client::fetch_fsmap(bool user)
{
    ...
    do {
        client_lock.unlock();
        std::tie(fsmap_latest, std::ignore) = monclient->get_version("fsmap", ca::use_blocked[ec]);
        client_lock.lock();
 ...
 *
 */
int Client::mds_command(const std::string& mds_spec, const vector<string>& cmd, const bufferlist& inbl,
                        bufferlist* outbl, string* outs, Context* onfinish)
{
    RWRef_t iref_reader(initialize_state, CLIENT_INITIALIZED);
    if (!iref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::mount(const std::string& mount_root, const UserPerm& perms, bool require_mds, const std::string& fs_name)
{
    ...
    if (mdsmap->test_flag(CEPH_MDSMAP_REFUSE_CLIENT_SESSION)) {
        lderr(cct) << "connections cannot be made while"
                      " the flag refuse_client_session is set"
                   << dendl;
...
void Client::_close_sessions()
{
    ...
    while (!mds_sessions.empty()) {
        ...
        ldout(cct, 2) << "waiting for " << mds_ranks_closing.size() << " mds session(s) to close (timeout: " << timo
                      << "s)" << dendl;
        std::unique_lock l{client_lock, std::adopt_lock};
        if (!timo) {
            mount_cond.wait(l);
            ...
            ldout(cct, 1) << mds_ranks_closing.size() << " mds(s) did not respond to session close -- timing out."
                          << dendl;
...
void Client::_unmount(bool abort)
{
    ...
    if (!mref_writer.is_first_writer()) return;
    ...
    mount_cond.wait(lock, [this] {
        ...
        for (auto& [tid, req] : mds_requests) {
            if (req->is_write()) {
                ldout(cct, 10) << "waiting for write request '" << tid << "' to complete, currently there are "
                               << mds_requests.size() << " outstanding read/write requests" << dendl;
                return false;
            }
        }
        return true;
    ...
    while (lru.lru_get_size() > 0 || !inode_map.empty()) {
        ldout(cct, 2) << "cache still has " << lru.lru_get_size() << "+" << inode_map.size() << " items"
                      << ", waiting (for caps to release?)" << dendl;

        if (auto r = mount_cond.wait_for(lock, ceph::make_timespan(5)); r == std::cv_status::timeout) {
            dump_cache(NULL);
        }
    }
    ceph_assert(lru.lru_get_size() == 0);
...
void Client::flush_cap_releases()
{
    ...
    for (auto& p : mds_sessions) {
        ...
        if (session->release && mdsmap->is_clientreplay_or_active_or_stopping(p.first)) {
...
void Client::renew_and_flush_cap_releases()
{
    ...
    if (!mount_aborted && mdsmap->get_epoch()) {
        ...
        if (unlikely(utime_t(el) > mdsmap->get_session_timeout() / 3.0)) renew_caps();
...
void Client::tick()
{
    ...
    while (!p.end()) {
        Inode* in = *p;
        ++p;
        if (!mount_aborted && in->hold_caps_until > now) break;
        delayed_list.pop_front();
        if (!mount_aborted) check_caps(in, CHECK_CAPS_NODELAY);
    }

    if (!mount_aborted) collect_and_send_metrics();

    delay_put_inodes(is_unmounting());
    trim_cache(true);

    if (blocklisted && (is_mounted() || is_unmounting()) && last_auto_reconnect + std::chrono::seconds(30 * 60) < now &&
...
void Client::start_tick_thread()
{
    upkeeper = std::thread([this]() {
        ...
        while (!tick_thread_stopped) {
            ...
            if (!tick_thread_stopped) upkeep_cond.wait_for(cl, interval);
        }
    });
}
...
void Client::collect_and_send_global_metrics()
{
    ...
    if (!mdsmap->is_active((mds_rank_t)0)) {
        ldout(cct, 5) << __func__ << " MDS rank 0 is not ready yet -- not sending metric" << dendl;
        return;
    }
    ...
    // read latency
    if (_collect_and_send_global_metrics || session->mds_metric_flags.test(CLIENT_METRIC_TYPE_READ_LATENCY)) {
        metric = ClientMetricMessage(ReadLatencyPayload(
            logger->tget(l_c_read), logger->tget(l_c_rd_avg), logger->get(l_c_rd_sqsum), nr_read_request));
        message.push_back(metric);
    }

    // write latency
    if (_collect_and_send_global_metrics || session->mds_metric_flags.test(CLIENT_METRIC_TYPE_WRITE_LATENCY)) {
        metric = ClientMetricMessage(WriteLatencyPayload(
            logger->tget(l_c_wrlat), logger->tget(l_c_wr_avg), logger->get(l_c_wr_sqsum), nr_write_request));
        message.push_back(metric);
    }

    // metadata latency
    if (_collect_and_send_global_metrics || session->mds_metric_flags.test(CLIENT_METRIC_TYPE_METADATA_LATENCY)) {
        metric = ClientMetricMessage(MetadataLatencyPayload(
            logger->tget(l_c_lat), logger->tget(l_c_md_avg), logger->get(l_c_md_sqsum), nr_metadata_request));
        message.push_back(metric);
    }

    // cap hit ratio -- nr_caps is unused right now
    if (_collect_and_send_global_metrics || session->mds_metric_flags.test(CLIENT_METRIC_TYPE_CAP_INFO)) {
    ...
    if (_collect_and_send_global_metrics || session->mds_metric_flags.test(CLIENT_METRIC_TYPE_DENTRY_LEASE)) {
    ...
    // opened files
    if (_collect_and_send_global_metrics || session->mds_metric_flags.test(CLIENT_METRIC_TYPE_OPENED_FILES)) {
    ...
    // pinned i_caps
    if (_collect_and_send_global_metrics || session->mds_metric_flags.test(CLIENT_METRIC_TYPE_PINNED_ICAPS)) {
    ...
    // opened inodes
    if (_collect_and_send_global_metrics || session->mds_metric_flags.test(CLIENT_METRIC_TYPE_OPENED_INODES)) {
    ...
    // read io sizes
    if (_collect_and_send_global_metrics || session->mds_metric_flags.test(CLIENT_METRIC_TYPE_READ_IO_SIZES)) {
        metric = ClientMetricMessage(ReadIoSizesPayload(total_read_ops, total_read_size));
        message.push_back(metric);
    }

    // write io sizes
    if (_collect_and_send_global_metrics || session->mds_metric_flags.test(CLIENT_METRIC_TYPE_WRITE_IO_SIZES)) {
        metric = ClientMetricMessage(WriteIoSizesPayload(total_write_ops, total_write_size));
        message.push_back(metric);
    }
...
void Client::renew_caps()
{
    ...
    for (auto& p : mds_sessions) {
        ...
        if (mdsmap->get_state(p.first) >= MDSMap::STATE_REJOIN) renew_caps(p.second.get());
    }
}
...
int Client::_do_lookup(Inode* dir, const string& name, int mask, InodeRef* target, const UserPerm& perms)
{
    ...
    req->set_inode(dir);
    if (cct->_conf->client_debug_getattr_caps && op == CEPH_MDS_OP_LOOKUP) mask |= DEBUG_GETATTR_CAPS;
...
bool Client::_dentry_valid(const Dentry* dn)
{
    ...
    if (dn->lease_mds >= 0 && dn->lease_ttl > now && mds_sessions.count(dn->lease_mds)) {
        ...
        ldout(cct, 20) << " bad lease, cap_ttl " << s->cap_ttl << ", cap_gen " << s->cap_gen << " vs lease_gen "
                       << dn->lease_gen << dendl;
    }
...
int Client::_lookup(Inode* dir, const string& dname, int mask, InodeRef* target, const UserPerm& perms,
                    std::string* alternate_name, bool is_rename)
{
    int r = 0;
    Dentry* dn = NULL;
    ...
    if (dname == cct->_conf->client_snapdir && dir->snapid == CEPH_NOSNAP) {
...
relookup:
    if (dir->dir && dir->dir->dentries.count(dname)) {
        ...
        ldout(cct, 20) << __func__ << " have " << *dn << " from mds." << dn->lease_mds << " ttl " << dn->lease_ttl
                       << " seq " << dn->lease_seq << dendl;
        ...
        if (!dn->inode || dn->inode->caps_issued_mask(mask, true)) {
            ...
            if (dir->caps_issued_mask(CEPH_CAP_FILE_SHARED, true)) {
                if (dn->cap_shared_gen == dir->shared_gen && (!dn->inode || dn->inode->caps_issued_mask(mask, true)))
                    goto hit_dn;
                if (!dn->inode && (dir->flags & I_COMPLETE)) {
                    ldout(cct, 10) << __func__ << " concluded ENOENT locally for " << *dir << " dn '" << dname << "'"
                                   << dendl;
                    return -CEPHFS_ENOENT;
                }
            }
        ...
        if (!is_rename && dn->is_renaming) {
            ldout(cct, 1) << __func__ << " dir " << *dir << " rename is on the way, will wait for dn '" << dname << "'"
                          << dendl;
        ...
        if (dir->caps_issued_mask(CEPH_CAP_FILE_SHARED, true) && (dir->flags & I_COMPLETE)) {
...
hit_dn:
    if (dn->inode) {
        *target = dn->inode;
        if (alternate_name) *alternate_name = dn->alternate_name;
    }
    else {
        r = -CEPHFS_ENOENT;
    }
...
int Client::path_walk(const filepath& origpath, InodeRef* end, const UserPerm& perms, bool followsym, int mask,
                      InodeRef dirinode)
{
...
int Client::path_walk(const filepath& origpath, walk_dentry_result* result, const UserPerm& perms, bool followsym,
                      int mask, InodeRef dirinode)
{
...
int Client::rename(const char* relfrom, const char* relto, const UserPerm& perm, std::string alternate_name)
{
    ...
    if (std::string(relfrom) == "/" || std::string(relto) == "/") return -CEPHFS_EBUSY;
    ...
    if (cct->_conf->client_permissions) {
        ...
        if (r < 0 && r != -CEPHFS_ENOENT) return r;
    }
...
int Client::mkdirat(int dirfd, const char* relpath, mode_t mode, const UserPerm& perm, std::string alternate_name)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::mknod(const char* relpath, mode_t mode, const UserPerm& perms, dev_t rdev)
{
    ...
    if (std::string(relpath) == "/") return -CEPHFS_EEXIST;
...
{
    if (!in->is_symlink()) return -CEPHFS_EINVAL;
...
int Client::_getvxattr(Inode* in, const UserPerm& perms, const char* xattr_name, ssize_t size, void* value,
                       mds_rank_t rank)
{
    ...
    bufferlist bl;
    int res = make_request(req, perms, nullptr, nullptr, rank, &bl, CEPHFS_FEATURE_OP_GETVXATTR);
    ldout(cct, 10) << __func__ << " result=" << res << dendl;
...
int Client::_do_setattr(Inode* in, struct ceph_statx* stx, int mask, const UserPerm& perms, InodeRef* inp,
                        std::vector<uint8_t>* aux)
{
    ...
    if (aux) auxsize = aux->size();

    ldout(cct, 10) << __func__ << " mask " << mask << " issued " << ccap_string(issued) << " aux size " << auxsize
                   << dendl;

    if (in->snapid != CEPH_NOSNAP) {
        return -CEPHFS_EROFS;
    }
    if ((mask & CEPH_SETATTR_SIZE) && (uint64_t)stx->stx_size > in->size &&
        is_quota_bytes_exceeded(in, (uint64_t)stx->stx_size - in->size, perms)) {
    ...
    if ((in->cap_dirtier_uid >= 0 && perms.uid() != in->cap_dirtier_uid) ||
        ...
        ldout(cct, 10) << __func__ << " caller " << perms.uid() << ":" << perms.gid() << " != cap dirtier "
                       << in->cap_dirtier_uid << ":" << in->cap_dirtier_gid << ", forcing sync setattr" << dendl;
        /*
    ...
    if (mask & CEPH_SETATTR_UID) {
        ...
        else if (!in->caps_issued_mask(CEPH_CAP_AUTH_SHARED) || in->uid != stx->stx_uid) {
    ...
    if (mask & CEPH_SETATTR_GID) {
        ...
        else if (!in->caps_issued_mask(CEPH_CAP_AUTH_SHARED) || in->gid != stx->stx_gid) {
    ...
    if (mask & CEPH_SETATTR_MODE) {
        ...
        else if (!in->caps_issued_mask(CEPH_CAP_AUTH_SHARED) || in->mode != stx->stx_mode) {
    ...
    if (mask & CEPH_SETATTR_BTIME) {
        ...
        else if (!in->caps_issued_mask(CEPH_CAP_AUTH_SHARED) || in->btime != utime_t(stx->stx_btime)) {
    ...
    if (mask & CEPH_SETATTR_FSCRYPT_AUTH) {
        ...
        else if (!in->caps_issued_mask(CEPH_CAP_AUTH_SHARED) || in->fscrypt_auth != *aux) {
            inode_drop |= CEPH_CAP_AUTH_SHARED;
        }
        else {
            mask &= ~CEPH_SETATTR_FSCRYPT_AUTH;
    ...
    if (mask & CEPH_SETATTR_SIZE) {
        ...
        if (in->caps_issued_mask(CEPH_CAP_FILE_EXCL) && !(mask & CEPH_SETATTR_KILL_SGUID) &&
    ...
    if (mask & CEPH_SETATTR_FSCRYPT_FILE) {
        ldout(cct, 10) << "resetting cached fscrypt_file field. size now " << in->fscrypt_file.size() << dendl;
        ...
        else if (!in->caps_issued_mask(CEPH_CAP_FILE_SHARED) || in->fscrypt_file != *aux) {
            inode_drop |= CEPH_CAP_FILE_SHARED | CEPH_CAP_FILE_RD | CEPH_CAP_FILE_WR;
        }
        else {
            mask &= ~CEPH_SETATTR_FSCRYPT_FILE;
    ...
    if (mask & CEPH_SETATTR_MTIME) {
        ...
        else if (in->caps_issued_mask(CEPH_CAP_FILE_WR) && utime_t(stx->stx_mtime) > in->mtime) {
            in->mtime = utime_t(stx->stx_mtime);
            in->ctime = ceph_clock_now();
            in->cap_dirtier_uid = perms.uid();
            in->cap_dirtier_gid = perms.gid();
        ...
        else if (!in->caps_issued_mask(CEPH_CAP_FILE_SHARED) || in->mtime != utime_t(stx->stx_mtime)) {
    ...
    if (mask & CEPH_SETATTR_ATIME) {
        ...
        else if (in->caps_issued_mask(CEPH_CAP_FILE_WR) && utime_t(stx->stx_atime) > in->atime) {
            in->atime = utime_t(stx->stx_atime);
            in->ctime = ceph_clock_now();
            in->cap_dirtier_uid = perms.uid();
            in->cap_dirtier_gid = perms.gid();
        ...
        else if (!in->caps_issued_mask(CEPH_CAP_FILE_SHARED) || in->atime != utime_t(stx->stx_atime)) {
...
int Client::__setattrx(Inode* in, struct ceph_statx* stx, int mask, const UserPerm& perms, InodeRef* inp)
{
    if (mask & CEPH_SETATTR_SIZE) {
    ...
    if (ret < 0) return ret;
    if (mask & CEPH_SETATTR_MODE) ret = _posix_acl_chmod(in, stx->stx_mode, perms);
    return ret;
}

int Client::_setattrx(InodeRef& in, struct ceph_statx* stx, int mask, const UserPerm& perms)
{
    mask &= (CEPH_SETATTR_MODE | CEPH_SETATTR_UID | CEPH_SETATTR_GID | CEPH_SETATTR_MTIME | CEPH_SETATTR_ATIME |
...
int Client::_setattr(InodeRef& in, struct stat* attr, int mask, const UserPerm& perms)
{
...
int Client::setattr(const char* relpath, struct stat* attr, int mask, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::setattrx(const char* relpath, struct ceph_statx* stx, int mask, const UserPerm& perms, int flags)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::fsetattr(int fd, struct stat* attr, int mask, const UserPerm& perms)
{
    ...
    std::scoped_lock lock(client_lock);
    Fh* f = get_filehandle(fd);
    if (!f) return -CEPHFS_EBADF;
#if defined(__linux__) && defined(O_PATH)
    if (f->flags & O_PATH) return -CEPHFS_EBADF;
#endif
...
int Client::stat(const char* relpath, struct stat* stbuf, const UserPerm& perms, frag_info_t* dirstat, int mask)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
unsigned Client::statx_to_mask(unsigned int flags, unsigned int want)
{
    ...
    if ((flags & AT_STATX_SYNC_TYPE) == AT_STATX_DONT_SYNC) goto out;

    /* Always set PIN to distinguish from AT_STATX_DONT_SYNC case */
    mask |= CEPH_CAP_PIN;
    if (want &
        (CEPH_STATX_MODE | CEPH_STATX_UID | CEPH_STATX_GID | CEPH_STATX_BTIME | CEPH_STATX_CTIME | CEPH_STATX_VERSION))
        mask |= CEPH_CAP_AUTH_SHARED;
    if (want & (CEPH_STATX_NLINK | CEPH_STATX_CTIME | CEPH_STATX_VERSION)) mask |= CEPH_CAP_LINK_SHARED;
    if (want & (CEPH_STATX_NLINK | CEPH_STATX_ATIME | CEPH_STATX_MTIME | CEPH_STATX_CTIME | CEPH_STATX_SIZE |
                CEPH_STATX_BLOCKS | CEPH_STATX_VERSION))
        mask |= CEPH_CAP_FILE_SHARED;
    if (want & (CEPH_STATX_VERSION | CEPH_STATX_CTIME)) mask |= CEPH_CAP_XATTR_SHARED;
out:
    return mask;
}

int Client::statx(const char* relpath, struct ceph_statx* stx, const UserPerm& perms, unsigned int want,
                  unsigned int flags)
{
    return statxat(CEPHFS_AT_FDCWD, relpath, stx, perms, want, flags);
}

int Client::lstat(const char* relpath, struct stat* stbuf, const UserPerm& perms, frag_info_t* dirstat, int mask)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
{
    ldout(cct, 10) << __func__ << " on " << in->ino << " snap/dev" << in->snapid << " mode 0" << oct << in->mode << dec
...
// available on Linux.
    ...
    if (dirstat) *dirstat = in->dirstat;
    if (rstat) *rstat = in->rstat;
...
void Client::fill_statx(Inode* in, unsigned int mask, struct ceph_statx* stx)
{
    ...
    if (mask & CEPH_CAP_FILE_SHARED) {
        ...
        stx->stx_mask |= (CEPH_STATX_ATIME | CEPH_STATX_MTIME | CEPH_STATX_SIZE | CEPH_STATX_BLOCKS);
    }
...
int Client::chmodat(int dirfd, const char* relpath, mode_t mode, int flags, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
        return -CEPHFS_ENOTCONN;
...
int Client::chown(const char* relpath, uid_t new_uid, gid_t new_gid, const UserPerm& perms)
{
...
int Client::lchown(const char* relpath, uid_t new_uid, gid_t new_gid, const UserPerm& perms)
{
    return chownat(CEPHFS_AT_FDCWD, relpath, new_uid, new_gid, AT_SYMLINK_NOFOLLOW, perms);
}

int Client::chownat(int dirfd, const char* relpath, uid_t new_uid, gid_t new_gid, int flags, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
        return -CEPHFS_ENOTCONN;
...
static void attr_set_atime_and_mtime(struct stat* attr, const utime_t& atime, const utime_t& mtime)
{
...
int Client::utime(const char* relpath, struct utimbuf* buf, const UserPerm& perms)
{
    struct timeval tv[2];
...
int Client::lutime(const char* relpath, struct utimbuf* buf, const UserPerm& perms)
{
    struct timeval tv[2];
    tv[0].tv_sec = buf->actime;
    tv[0].tv_usec = 0;
...
int Client::utimes(const char* relpath, struct timeval times[2], const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
    ...
    tout(cct) << __func__ << std::endl;
    tout(cct) << relpath << std::endl;
    tout(cct) << "atime: " << times[0].tv_sec << "." << times[0].tv_usec << std::endl;
    tout(cct) << "mtime: " << times[1].tv_sec << "." << times[1].tv_usec << std::endl;
...
int Client::lutimes(const char* relpath, struct timeval times[2], const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::futimens(int fd, struct timespec times[2], const UserPerm& perms)
{
    ...
    tout(cct) << __func__ << std::endl;
    tout(cct) << fd << std::endl;
    tout(cct) << "atime: " << times[0].tv_sec << "." << times[0].tv_nsec << std::endl;
    tout(cct) << "mtime: " << times[1].tv_sec << "." << times[1].tv_nsec << std::endl;
...
int Client::utimensat(int dirfd, const char* relpath, struct timespec times[2], int flags, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
        return -CEPHFS_ENOTCONN;
...
int Client::opendir(const char* relpath, dir_result_t** dirpp, const UserPerm& perms)
{
    ...
    if (r != -CEPHFS_ENOTDIR) tout(cct) << (uintptr_t)*dirpp << std::endl;
    return r;
}
...
{
    if (!in->is_dir()) return -CEPHFS_ENOTDIR;
...
void Client::seekdir(dir_result_t* dirp, loff_t offset)
{
    ...
    if (dirp->hash_order()) {
        ...
        if (offset == 0 || dirp->buffer_frag != frag_t(dir_result_t::fpos_high(offset)) ||
...
void Client::fill_dirent(struct dirent* de, const char* name, int type, uint64_t ino, loff_t next_off)
{
    ...
    ldout(cct, 10) << __func__ << " '" << de->d_name << "' -> " << inodeno_t(de->d_ino) << " type " << (int)de->d_type
                   << " w/ next_off " << hex << next_off << dec << dendl;
#endif
}
...
int Client::_readdir_get_frag(int op, dir_result_t* dirp, fill_readdir_args_cb_t fill_req_cb)
{
    ceph_assert(dirp);
    ceph_assert(dirp->inode);
    ...
    ldout(cct, 10) << __func__ << " " << dirp << " on " << dirp->inode->ino << " fg " << fg << " offset " << hex
                   << dirp->offset << dec << dendl;
    ...
    if (res == 0) {
        ldout(cct, 10) << __func__ << " " << dirp << " got frag " << dirp->buffer_frag << " size "
                       << dirp->buffer.size() << dendl;
    }
    else {
...
{
    bool operator()(const Dentry* dn, int64_t off) const { return dir_result_t::fpos_cmp(dn->offset, off) < 0; }
};

int Client::_readdir_cache_cb(dir_result_t* dirp, add_dirent_cb_t cb, void* p, int caps, bool getref)
{
    ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
    ldout(cct, 10) << __func__ << " " << dirp << " on " << dirp->inode->ino << " last_name " << dirp->last_name
    ...
    vector<Dentry*>::iterator pd =
        std::lower_bound(dir->readdir_cache.begin(), dir->readdir_cache.end(), dirp->offset, dentry_off_lt());

    string dn_name;
    while (true) {
        int mask = caps;
        if (!dirp->inode->is_complete_and_ordered()) return -CEPHFS_EAGAIN;
        if (pd == dir->readdir_cache.end()) break;
        Dentry* dn = *pd;
        if (dn->inode == NULL) {
        ...
        if (pd >= dir->readdir_cache.end() || *pd != dn) return -CEPHFS_EAGAIN;
        ...
        ++pd;
        if (pd == dir->readdir_cache.end()) next_off = dir_result_t::END;
...
int Client::readdir_r_cb(dir_result_t* d, add_dirent_cb_t cb, void* p, unsigned want, unsigned flags, bool getref)
{
    auto fill_readdir_cb = [](dir_result_t* dirp, MetaRequest* req, InodeRef& diri, frag_t fg) {
        filepath path;
    ...
    if (d->inode && d->inode->snapid == CEPH_SNAPDIR) op = CEPH_MDS_OP_LSSNAP;
    return _readdir_r_cb(op, d, cb, fill_readdir_cb, p, want, flags, getref, false);
}
...
//
int Client::_readdir_r_cb(int op, dir_result_t* d, add_dirent_cb_t cb, fill_readdir_args_cb_t fill_cb, void* p,
                          unsigned want, unsigned flags, bool getref, bool bypass_cache)
{
    ...
    ldout(cct, 10) << __func__ << " " << *dirp->inode << " offset " << hex << dirp->offset << dec
                   << " at_end=" << dirp->at_end() << " hash_order=" << dirp->hash_order() << dendl;
    ...
    ldout(cct, 10) << __func__ << " offset " << hex << dirp->offset << dec << " snapid " << dirp->inode->snapid
                   << " (complete && ordered) " << dirp->inode->is_complete_and_ordered() << " issued "
                   << ccap_string(dirp->inode->caps_issued()) << dendl;
    if (!bypass_cache && dirp->inode->snapid != CEPH_SNAPDIR && dirp->inode->is_complete_and_ordered() &&
        dirp->inode->caps_issued_mask(CEPH_CAP_FILE_SHARED, true)) {
        int err = _readdir_cache_cb(dirp, cb, p, caps, getref);
        if (err != -CEPHFS_EAGAIN) return err;
    }
    ...
    while (1) {
        ...
        ldout(cct, 10) << __func__ << " frag " << fg << " buffer size " << dirp->buffer.size() << " offset " << hex
                       << dirp->offset << dendl;

        for (auto it = std::lower_bound(
                 dirp->buffer.begin(), dirp->buffer.end(), dirp->offset, dir_result_t::dentry_off_lt());
             it != dirp->buffer.end();
             ++it) {
            ...
            ldout(cct, 15) << __func__ << " de " << de.d_name << " off " << hex << next_off - 1 << dec << " snap "
                           << entry.inode->snapid << " = " << r << dendl;
            if (r < 0) return r;
...
static int _readdir_single_dirent_cb(void* p, struct dirent* de, struct ceph_statx* stx, off_t off, Inode* in)
{
    single_readdir* c = static_cast<single_readdir*>(p);

    if (c->full) return -1;   // already filled this dirent
...
int Client::readdirplus_r(dir_result_t* d, struct dirent* de, struct ceph_statx* stx, unsigned want, unsigned flags,
                          Inode** out)
{
    single_readdir sr;
    sr.de = de;
...
int Client::readdir_snapdiff(dir_result_t* d1, snapid_t snap2, struct dirent* out_de, snapid_t* out_snap)
{
    if (!d1 || !d1->inode || d1->inode->snapid == snap2) {
        lderr(cct) << __func__ << " invalid parameters: "
                   << " d1:" << d1 << " d1->inode:" << (d1 ? d1->inode : nullptr) << " snap2 id :" << snap2 << dendl;
        errno = EINVAL;
        return -errno;
    }
    ...
    auto fill_snapdiff_cb = [&](dir_result_t* dirp, MetaRequest* req, InodeRef& diri, frag_t fg) {
        filepath path;
        diri->make_nosnap_relative_path(path);
        req->set_filepath(path);
        req->set_inode(diri.get());
    ...
    int ret = _readdir_r_cb(CEPH_MDS_OP_READDIR_SNAPDIFF,
                            ...
                            true);
    if (ret < -1) {
        lderr(cct) << __func__ << " error: " << cpp_strerror(ret) << dendl;
        errno = -ret;   // this sucks.
        return ret;
    }

    ldout(cct, 15) << __func__ << " " << ret << " " << sr.de->d_name << " " << stx.stx_dev << dendl;
    if (sr.full) {
        if (out_de) {
            *out_de = de;
        }
...
static int _readdir_getdent_cb(void* p, struct dirent* de, struct ceph_statx* stx, off_t off, Inode* in)
{
    ...
    if (c->pos + dlen > c->buflen) return -1;   // doesn't fit
...
int Client::getdir(const char* relpath, list<string>& contents, const UserPerm& perms)
{
...
int Client::create_and_open(int dirfd, const char* relpath, int flags, const UserPerm& perms, mode_t mode,
                            ...
                            std::string alternate_name)
{
     ...
     * in kernel (fs/open.c). */
    if (flags & O_PATH) flags &= O_DIRECTORY | O_NOFOLLOW | O_PATH;
#endif
    ...
    if (r == 0 && (flags & O_CREAT) && (flags & O_EXCL)) return -CEPHFS_EEXIST;
    ...
    if (r == -CEPHFS_ENOENT && (flags & O_CREAT)) {
        ...
        dirpath.pop_dentry();
        InodeRef dir;
        r = path_walk(dirpath, &dir, perms, true, cct->_conf->client_permissions ? CEPH_CAP_AUTH_SHARED : 0, dirinode);
        if (r < 0) {
            goto out;
        }
        if (cct->_conf->client_permissions) {
    ...
    if (!fh) r = _open(in.get(), flags, mode, &fh, perms);
    if (r >= 0) {
...
int Client::open(const char* relpath, int flags, const UserPerm& perms, mode_t mode, int stripe_unit, int stripe_count,
                 int object_size, const char* data_pool, std::string alternate_name)
{
    return openat(CEPHFS_AT_FDCWD,
                  relpath,
                  flags,
                  perms,
                  mode,
...
int Client::openat(int dirfd, const char* relpath, int flags, const UserPerm& perms, mode_t mode, int stripe_unit,
                   int stripe_count, int object_size, const char* data_pool, std::string alternate_name)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
        return -CEPHFS_ENOTCONN;
    ...
    std::scoped_lock locker(client_lock);
    int r = create_and_open(
        dirfd, relpath, flags, perms, mode, stripe_unit, stripe_count, object_size, data_pool, alternate_name);
...
int Client::lookup_hash(inodeno_t ino, inodeno_t dirino, const char* name, const UserPerm& perms)
{
...
int Client::_lookup_vino(vinodeno_t vino, const UserPerm& perms, Inode** inode)
{
    ...
    if (is_reserved_vino(vino)) return -CEPHFS_ESTALE;
     ...
     */
    if (vino.snapid < CEPH_NOSNAP) req->head.args.lookupino.snapid = vino.snapid;
...
int Client::_release_fh(Fh* f)
{
    ...
    if (err != 0) {
        ldout(cct, 1) << __func__ << " " << f << " on inode " << *in << " caught async_err = " << cpp_strerror(err)
                      << dendl;
    }
    else {
...
int Client::_open(Inode* in, int flags, mode_t mode, Fh** fhp, const UserPerm& perms)
{
    if (in->snapid != CEPH_NOSNAP && (flags & (O_WRONLY | O_RDWR | O_CREAT | O_TRUNC | O_APPEND))) {
        return -CEPHFS_EROFS;
    }

    // use normalized flags to generate cmode
    int cflags = ceph_flags_sys2wire(flags);
    if (cct->_conf.get_val<bool>("client_force_lazyio")) cflags |= CEPH_O_LAZY;
    ...
    if ((flags & O_TRUNC) == 0 && in->caps_issued_mask(want)) {
        ...
        if (deleg_timeout && result == 0) {
            ...
            if (cmode & CEPH_FILE_MODE_WR) need |= CEPH_CAP_FILE_WR;
            if (cmode & CEPH_FILE_MODE_RD) need |= CEPH_CAP_FILE_RD;
            ...
            if (result < 0) {
                ldout(cct, 8) << "Unable to get caps after open of inode " << *in
                              << " . Denying open: " << cpp_strerror(result) << dendl;
            }
            else {
                put_cap_ref(in, need);
            }
    ...
    // success?
    if (result >= 0) {
        if (fhp) *fhp = _create_fh(in, flags, cmode, perms);
    }
    else {
...
int Client::_renew_caps(Inode* in)
{
    ...
    if (in->is_any_caps() && ((wanted & CEPH_CAP_ANY_WR) == 0 || in->auth_cap)) {
        check_caps(in, CHECK_CAPS_NODELAY);
        return 0;
    }
...
loff_t Client::_lseek(Fh* f, loff_t offset, int whence)
{
    ...
    switch (whence) {
    case SEEK_END: whence_check = true; break;

#ifdef SEEK_DATA
    case SEEK_DATA: whence_check = true; break;
#endif

#ifdef SEEK_HOLE
    case SEEK_HOLE: whence_check = true; break;
#endif
    }
    ...
    case SEEK_END: pos = in->size + offset; break;

#ifdef SEEK_DATA
    case SEEK_DATA:
        if (offset < 0 || static_cast<uint64_t>(offset) >= in->size) return -CEPHFS_ENXIO;
        pos = offset;
        break;
#endif
    ...
    default: ldout(cct, 1) << __func__ << ": invalid whence value " << whence << dendl; return -CEPHFS_EINVAL;
    }
...
void Client::lock_fh_pos(Fh* f)
{
    ...
    if (f->pos_locked || !f->pos_waiters.empty()) {
        ...
        std::unique_lock l{client_lock, std::adopt_lock};
        cond.wait(l, [f, me = &cond] { return !f->pos_locked && f->pos_waiters.front() == me; });
        l.release();
...
int Client::uninline_data(Inode* in, Context* onfinish)
{
    ...
    uninline_ops.cmpxattr("inline_version", CEPH_OSD_CMPXATTR_OP_GT, CEPH_OSD_CMPXATTR_MODE_U64, inline_version_bl);
...
{
    if (iovcnt < 0) return -CEPHFS_EINVAL;
...
int64_t Client::_read(Fh* f, int64_t offset, uint64_t size, bufferlist* bl)
{
    ...
    if ((f->mode & CEPH_FILE_MODE_RD) == 0) return -CEPHFS_EBADF;
...
retry:
    ...
    if (f->flags & O_DIRECT) have &= ~(CEPH_CAP_FILE_CACHE | CEPH_CAP_FILE_LAZYIO);

    if (in->inline_version < CEPH_INLINE_NONE) {
        uint32_t len = in->inline_data.length();
        uint64_t endoff = offset + size;
        if (endoff > in->size) endoff = in->size;
    ...
    if (!conf->client_debug_force_sync_read && conf->client_oc &&
        ...
        if (f->flags & O_DIRECT) _flush_range(in, offset, size);
        ...
        if (checkeof) {
            ...
            // eof?  short read.
            if ((uint64_t)offset < in->size) goto retry;
        }
    }
...
{
    lgeneric_subdout(client->cct, client, 20) << "client." << client->get_nodeid() << " "
                                              << "C_Readahead on " << f->inode << dendl;
...
int Client::_read_async(Fh* f, uint64_t off, uint64_t len, bufferlist* bl)
{
    ...
    r = objectcacher->file_read(&in->oset, &in->layout, in->snapid, off, len, bl, 0, &onfinish);
    if (r == 0) {
    ...
    if (f->readahead.get_min_readahead_size() > 0) {
        ...
        if (readahead_extent.second > 0) {
            ...
            int r2 = objectcacher->file_read(&in->oset,
                                             &in->layout,
                                             in->snapid,
                                             readahead_extent.first,
...
int Client::_read_sync(Fh* f, uint64_t off, uint64_t len, bufferlist* bl, bool* checkeof)
{
    ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
    ...
    while (left > 0) {
        ...
        int wanted = left;
        filer->read_trunc(
            in->ino, &in->layout, in->snapid, pos, left, &tbl, 0, in->truncate_size, in->truncate_seq, &onfinish);
        client_lock.unlock();
...
int Client::write(int fd, const char* buf, loff_t size, loff_t offset)
{
    ...
    if (!fh) return -CEPHFS_EBADF;
#if defined(__linux__) && defined(O_PATH)
    if (fh->flags & O_PATH) return -CEPHFS_EBADF;
#endif
    /* We can't return bytes written larger than INT_MAX, clamp size to that */
    size = std::min(size, (loff_t)INT_MAX);
...
int64_t Client::_preadv_pwritev_locked(Fh* fh, const struct iovec* iov, unsigned iovcnt, int64_t offset, bool write,
                                       bool clamp_to_int)
{
    ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
...
int64_t Client::_write(Fh* f, int64_t offset, uint64_t size, const char* buf, const struct iovec* iov, int iovcnt)
{
    ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
    ...
    if ((f->mode & CEPH_FILE_MODE_WR) == 0) return -CEPHFS_EBADF;
    ...
    // check quota
    uint64_t endoff = offset + size;
    if (endoff > in->size && is_quota_bytes_exceeded(in, endoff - in->size, f->actor_perms)) {
        return -CEPHFS_EDQUOT;
    }
    ...
    if (f->flags & O_DIRECT) have &= ~(CEPH_CAP_FILE_BUFFER | CEPH_CAP_FILE_LAZYIO);
    ...
    if (in->inline_version < CEPH_INLINE_NONE) {
        if (endoff > cct->_conf->client_max_inline_size || endoff > CEPH_INLINE_MAX_SIZE ||
            ...
            if (endoff < len) in->inline_data.begin(endoff).copy(len - endoff, bl);   // XXX
    ...
    if (cct->_conf->client_oc && (have & (CEPH_CAP_FILE_BUFFER | CEPH_CAP_FILE_LAZYIO))) {
        // do buffered write
        if (!in->oset.dirty_or_tx) get_cap_ref(in, CEPH_CAP_FILE_CACHE | CEPH_CAP_FILE_BUFFER);

        get_cap_ref(in, CEPH_CAP_FILE_BUFFER);

        // async, caching, non-blocking.
        r = objectcacher->file_write(
            &in->oset, &in->layout, in->snaprealm->get_snap_context(), offset, size, bl, ceph::real_clock::now(), 0);
        put_cap_ref(in, CEPH_CAP_FILE_BUFFER);
...
int Client::_fsync(Inode* in, bool syncdataonly)
{
    ...
    if (!syncdataonly && in->dirty_caps) {
        ...
        if (in->flushing_caps) flush_tid = last_flush_tid;
    }
    else
        ldout(cct, 10) << "no metadata needs to commit" << dendl;
    ...
    if (!r) {
        if (flush_tid > 0) wait_sync_caps(in, flush_tid);
...
int Client::fstatx(int fd, struct ceph_statx* stx, const UserPerm& perms, unsigned int want, unsigned int flags)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::statxat(int dirfd, const char* relpath, struct ceph_statx* stx, const UserPerm& perms, unsigned int want,
                    unsigned int flags)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) {
...
int Client::chdir(const char* relpath, std::string& new_cwd, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
    ...
    if (!(in.get()->is_dir())) return -CEPHFS_ENOTDIR;

    if (cwd != in) cwd.swap(in);
...
int Client::statfs(const char* path, struct statvfs* stbuf, const UserPerm& perms)
{
    ...
    InodeRef quota_root =
        root->quota.is_enabled(QUOTA_MAX_BYTES) ? root : get_quota_root(root.get(), perms, QUOTA_MAX_BYTES);
    ...
    if (quota_root && cct->_conf->client_quota_df && quota_root->quota.max_bytes) {
        ...
        if (!_any_stale_sessions()) {
            ...
            if (r != 0) {
                ...
                // reason to break "df".
                lderr(cct) << "Error in getattr on quota root 0x" << std::hex << quota_root->ino << std::dec
...
int Client::_do_filelock(Inode* in, Fh* fh, int lock_type, int op, int sleep, struct flock* fl, uint64_t owner,
                         bool removing)
{
    ldout(cct, 10) << __func__ << " ino " << in->ino << (lock_type == CEPH_LOCK_FCNTL ? " fcntl" : " flock") << " type "
                   << fl->l_type << " owner " << owner << " " << fl->l_start << "~" << fl->l_len << dendl;

    if (in->flags & I_ERROR_FILELOCK) return -CEPHFS_EIO;
    ...
    if (op != CEPH_MDS_OP_SETFILELOCK || lock_cmd == CEPH_LOCK_UNLOCK) sleep = 0;
    ...
    if (ret == 0) {
        if (op == CEPH_MDS_OP_GETFILELOCK) {
            ...
            if (lock_type == CEPH_LOCK_FCNTL) {
                if (!in->fcntl_locks) in->fcntl_locks.reset(new ceph_lock_state_t(cct, CEPH_LOCK_FCNTL));
                lock_state = in->fcntl_locks.get();
            }
            else if (lock_type == CEPH_LOCK_FLOCK) {
                if (!in->flock_locks) in->flock_locks.reset(new ceph_lock_state_t(cct, CEPH_LOCK_FLOCK));
            ...
            if (!removing) {
                if (lock_type == CEPH_LOCK_FCNTL) {
                    if (!fh->fcntl_locks) fh->fcntl_locks.reset(new ceph_lock_state_t(cct, CEPH_LOCK_FCNTL));
                    lock_state = fh->fcntl_locks.get();
                }
                else {
                    if (!fh->flock_locks) fh->flock_locks.reset(new ceph_lock_state_t(cct, CEPH_LOCK_FLOCK));
...
int Client::_interrupt_filelock(MetaRequest* req)
{
    ...
    if (req->mds < 0) return 0;   // haven't sent the request
...
{
    if (!in->fcntl_locks && !in->flock_locks) return;
    ...
    if (nr_fcntl_locks) {
        ...
        for (auto p = lock_state->held_locks.begin(); p != lock_state->held_locks.end(); ++p) encode(p->second, bl);
    }
    ...
    ldout(cct, 10) << __func__ << " ino " << in->ino << ", " << nr_fcntl_locks << " fcntl locks, " << nr_flock_locks
                   << " flock locks" << dendl;
}

void Client::_release_filelocks(Fh* fh)
{
    if (!fh->fcntl_locks && !fh->flock_locks) return;
    ...
    if ((in->flags & I_ERROR_FILELOCK) && !in->has_any_filelocks()) in->flags &= ~I_ERROR_FILELOCK;
    ...
    for (list<pair<int, ceph_filelock>>::iterator p = to_release.begin(); p != to_release.end(); ++p) {
        fl.l_start = p->second.start;
        fl.l_len = p->second.length;
        fl.l_pid = p->second.pid;
        _do_filelock(in, fh, p->first, CEPH_MDS_OP_SETFILELOCK, 0, &fl, p->second.owner, true);
    }
}

void Client::_update_lock_state(struct flock* fl, uint64_t owner, ceph_lock_state_t* lock_state)
{
    int lock_cmd;
    if (F_RDLCK == fl->l_type)
        lock_cmd = CEPH_LOCK_SHARED;
...
void Client::_ll_register_callbacks(struct ceph_client_callback_args* args)
{
    ...
    ldout(cct, 10) << __func__ << " cb " << args->handle << " invalidate_ino_cb " << args->ino_cb
                   << " invalidate_dentry_cb " << args->dentry_cb << " switch_interrupt_cb " << args->switch_intr_cb
                   << " remount_cb " << args->remount_cb << dendl;
    ...
    if (args->umask_cb) umask_cb = args->umask_cb;
}
...
{
    if (is_mounting() || is_mounted() || is_unmounting()) return -CEPHFS_EBUSY;
...
std::pair<int, bool> Client::test_dentry_handling(bool can_invalidate)
{
    ...
    RWRef_t iref_reader(initialize_state, CLIENT_INITIALIZED);
    if (!iref_reader.is_state_satisfied()) return std::make_pair(-CEPHFS_ENOTCONN, false);
...
int Client::_lazyio(Fh* fh, int enable)
{
    ...
    if (!!(fh->mode & CEPH_FILE_MODE_LAZY) == !!enable) return 0;
...
int Client::mksnap(const char* relpath, const char* name, const UserPerm& perm, mode_t mode,
...
{
    ldout(cct, 10) << __func__ << ": snapdir inode=" << *in << ", inode=" << *diri << dendl;
    in->ino = diri->ino;
...
int Client::ll_lookup(Inode* parent, const char* name, struct stat* attr, Inode** out, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
out:
    ldout(cct, 3) << __func__ << " " << vparent << " " << name << " -> " << r << " (" << hex << attr->st_ino << dec
                  << ")" << dendl;
...
int Client::ll_lookup_vino(vinodeno_t vino, const UserPerm& perms, Inode** inode)
{
    ...
    if (snapid == CEPH_SNAPDIR) vino.snapid = CEPH_NOSNAP;
...
int Client::ll_lookup_inode(struct inodeno_t ino, const UserPerm& perms, Inode** inode)
{
    vinodeno_t vino(ino, CEPH_NOSNAP);
    return ll_lookup_vino(vino, perms, inode);
}

int Client::ll_lookupx(Inode* parent, const char* name, Inode** out, struct ceph_statx* stx, unsigned want,
                       unsigned flags, const UserPerm& perms)
{
    ...
    ldout(cct, 3) << __func__ << " " << vparent << " " << name << " -> " << r << " (" << hex << stx->stx_ino << dec
                  << ")" << dendl;
...
int Client::ll_walk(const char* name, Inode** out, struct ceph_statx* stx, unsigned int want, unsigned int flags,
...
void Client::_ll_get(Inode* in)
{
    if (in->ll_ref == 0) {
        ...
        if (in->snapid != CEPH_NOSNAP) ll_snap_ref[in->snapid]++;
    }
    in->ll_get();
...
int Client::_ll_put(Inode* in, uint64_t num)
{
    ...
    if (in->ll_ref == 0) {
        ...
        if (in->snapid != CEPH_NOSNAP) {
            ...
            if (--p->second == 0) ll_snap_ref.erase(p);
        }
        put_inode(in);
        return 0;
    }
...
void Client::_ll_drop_pins()
{
    ...
    for (ceph::unordered_map<vinodeno_t, Inode*>::iterator it = inode_map.begin(); it != inode_map.end(); it = next) {
        Inode* in = it->second;
        next = it;
        ++next;
        if (in->ll_ref) {
...
bool Client::_ll_forget(Inode* in, uint64_t count)
{
    ...
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return true;

    if (ino == 1) return true;   // ignore forget on root.

    bool last = false;
    if (in->ll_ref < count) {
        ldout(cct, 1) << "WARNING: ll_forget on " << ino << " " << count << ", which only has ll_ref=" << in->ll_ref
                      << dendl;
        _ll_put(in, in->ll_ref);
        last = true;
    }
    else {
        if (_ll_put(in, count) == 0) last = true;
    }
...
int Client::ll_get_snap_ref(snapid_t snap)
{
    ...
    if (p != ll_snap_ref.end()) return p->second;
    return 0;
}
...
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return NULL;

    std::scoped_lock lock(client_lock);

    vinodeno_t vino = _map_faked_ino(ino);
    unordered_map<vinodeno_t, Inode*>::iterator p = inode_map.find(vino);
    if (p == inode_map.end()) return NULL;
    Inode* in = p->second;
    _ll_get(in);
    return in;
}
...
Inode* Client::ll_get_inode(vinodeno_t vino)
{
    ...
    if (is_reserved_vino(vino)) return NULL;
...
int Client::ll_getattr(Inode* in, struct stat* attr, const UserPerm& perms)
{
    ...
    if (res == 0) fill_stat(in, attr);
    ldout(cct, 3) << __func__ << " " << _get_vino(in) << " = " << res << dendl;
    return res;
}

int Client::ll_getattrx(Inode* in, struct ceph_statx* stx, unsigned int want, unsigned int flags, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
    ...
    if (mask && !in->caps_issued_mask(mask, true)) res = _ll_getattr(in, mask, perms);

    if (res == 0) fill_statx(in, mask, stx);
    ldout(cct, 3) << __func__ << " " << _get_vino(in) << " = " << res << dendl;
    return res;
}

int Client::_ll_setattrx(Inode* in, struct ceph_statx* stx, int mask, const UserPerm& perms, InodeRef* inp)
{
    vinodeno_t vino = _get_vino(in);
...
int Client::ll_setattrx(Inode* in, struct ceph_statx* stx, int mask, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::ll_setattr(Inode* in, struct stat* attr, int mask, const UserPerm& perms)
{
    struct ceph_statx stx;
    stat_to_statx(attr, &stx);
...
int Client::getxattr(const char* path, const char* name, void* value, size_t size, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::lgetxattr(const char* path, const char* name, void* value, size_t size, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::fgetxattr(int fd, const char* name, void* value, size_t size, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::listxattr(const char* path, char* list, size_t size, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::llistxattr(const char* path, char* list, size_t size, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::removexattr(const char* path, const char* name, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::lremovexattr(const char* path, const char* name, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::fsetxattr(int fd, const char* name, const void* value, size_t size, int flags, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::_getxattr(Inode* in, const char* name, void* value, size_t size, const UserPerm& perms)
{
    int r;
...
int Client::_getxattr(InodeRef& in, const char* name, void* value, size_t size, const UserPerm& perms)
{
    if (cct->_conf->client_permissions) {
...
int Client::ll_getxattr(Inode* in, const char* name, void* value, size_t size, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::_listxattr(Inode* in, char* name, size_t size, const UserPerm& perms)
{
...
int Client::ll_listxattr(Inode* in, char* names, size_t size, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::_do_setxattr(Inode* in, const char* name, const void* value, size_t size, int flags, const UserPerm& perms)
{

    int xattr_flags = 0;
    if (!value) xattr_flags |= CEPH_XATTR_REMOVE;
    if (flags & XATTR_CREATE) xattr_flags |= CEPH_XATTR_CREATE;
    if (flags & XATTR_REPLACE) xattr_flags |= CEPH_XATTR_REPLACE;
...
int Client::_setxattr(Inode* in, const char* name, const void* value, size_t size, int flags, const UserPerm& perms)
{
    if (in->snapid != CEPH_NOSNAP) {
        return -CEPHFS_EROFS;
    }
    ...
    if (acl_type == POSIX_ACL) posix_acl_xattr = !strncmp(name, "system.", 7);

    if (strncmp(name, "user.", 5) && strncmp(name, "security.", 9) && strncmp(name, "trusted.", 8) &&
        strncmp(name, "ceph.", 5) && !posix_acl_xattr)
    ...
    if (posix_acl_xattr) {
        if (!strcmp(name, ACL_EA_ACCESS)) {
            ...
            if (value) {
                if (!S_ISDIR(in->mode)) return -CEPHFS_EACCES;
                int ret = posix_acl_check(value, size);
                if (ret < 0) return -CEPHFS_EINVAL;
                if (ret == 0) {
                    value = NULL;
                    size = 0;
                }
        ...
        if (vxattr) {
            if (vxattr->readonly) return -CEPHFS_EOPNOTSUPP;
            if (vxattr->setxattr_cb) return (this->*(vxattr->setxattr_cb))(in, value, size, perms);
            if (vxattr->name.compare(0, 10, "ceph.quota") == 0 && value) check_realm = true;
        }
    }
    ...
    if (ret >= 0 && check_realm) {
        ...
        if (in->quota.is_enabled() && !(in->snaprealm && in->snaprealm->ino == in->ino)) ret = -CEPHFS_EOPNOTSUPP;
    }

    return ret;
}

int Client::_setxattr(InodeRef& in, const char* name, const void* value, size_t size, int flags, const UserPerm& perms)
{
    if (cct->_conf->client_permissions) {
...
int Client::_setxattr_check_data_pool(string& name, string& value, const OSDMap* osdmap)
{
    ...
    if (tmp.length()) {
        ...
        try {
            ...
            if (!osdmap->have_pg_pool(pool)) return -CEPHFS_ENOENT;
        }
...
void Client::_setxattr_maybe_wait_for_osdmap(const char* name, const void* value, size_t size)
{
    ...
    if (strcmp(name, "ceph.file.layout.pool") == 0 || strcmp(name, "ceph.dir.layout.pool") == 0 ||
        ...
        int r = objecter->with_osdmap([&](const OSDMap& o) { return _setxattr_check_data_pool(rest, v, &o); });
...
int Client::ll_setxattr(Inode* in, const char* name, const void* value, size_t size, int flags, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::_removexattr(Inode* in, const char* name, const UserPerm& perms)
{
    ...
    if (strncmp(name, "user.", 5) && strncmp(name, "system.", 7) && strncmp(name, "security.", 9) &&
        strncmp(name, "trusted.", 8) && strncmp(name, "ceph.", 5))
        return -CEPHFS_EOPNOTSUPP;

    const VXattr* vxattr = _match_vxattr(in, name);
    if (vxattr && vxattr->readonly) return -CEPHFS_EOPNOTSUPP;
...
size_t Client::_vxattrcb_fscrypt_auth(Inode* in, char* val, size_t size)
{
    ...
    if (count <= size) memcpy(val, in->fscrypt_auth.data(), count);
    return count;
}

int Client::_vxattrcb_fscrypt_auth_set(Inode* in, const void* val, size_t size, const UserPerm& perms)
{
...
size_t Client::_vxattrcb_fscrypt_file(Inode* in, char* val, size_t size)
{
    ...
    if (count <= size) memcpy(val, in->fscrypt_file.data(), count);
    return count;
}

int Client::_vxattrcb_fscrypt_file_set(Inode* in, const void* val, size_t size, const UserPerm& perms)
{
    struct ceph_statx stx = {0};
    std::vector<uint8_t> aux;
...
{
    return in->quota.is_enabled() && (in->snapid != CEPH_NOSNAP || (in->snaprealm && in->snaprealm->ino == in->ino));
}
...
size_t Client::_vxattrcb_layout(Inode* in, char* val, size_t size)
{
    ...
    objecter->with_osdmap([&](const OSDMap& o) {
        if (o.have_pg_pool(in->layout.pool_id))
            r += snprintf(val + r, size - r, "%s", o.get_pool_name(in->layout.pool_id).c_str());
        else
            r += snprintf(val + r, size - r, "%" PRIu64, (uint64_t)in->layout.pool_id);
    });
    if (in->layout.pool_ns.length()) r += snprintf(val + r, size - r, " pool_namespace=%s", in->layout.pool_ns.c_str());
    return r;
}
...
size_t Client::_vxattrcb_layout_pool(Inode* in, char* val, size_t size)
{
    ...
    objecter->with_osdmap([&](const OSDMap& o) {
        if (o.have_pg_pool(in->layout.pool_id))
            r = snprintf(val, size, "%s", o.get_pool_name(in->layout.pool_id).c_str());
        else
...
{
    return snprintf(val, size, "%ld.%09ld", (long)in->rstat.rctime.sec(), (long)in->rstat.rctime.nsec());
}
...
{
    return snprintf(
        val, size, "%llu.%09lu", (long long unsigned)in->snap_btime.sec(), (long unsigned)in->snap_btime.nsec());
}
...
#define XATTR_NAME_CEPH(_type, _name, _flags)                                                                      \
    {                                                                                                              \
    name:                                                                                                          \
        CEPH_XATTR_NAME(_type, _name),                                                                             \
            getxattr_cb : &Client::_vxattrcb_##_type##_##_name, readonly : true, exists_cb : NULL, flags : _flags, \
    }
#define XATTR_LAYOUT_FIELD(_type, _name, _field)                                                                       \
    {                                                                                                                  \
    name:                                                                                                              \
        CEPH_XATTR_NAME2(_type, _name, _field), getxattr_cb : &Client::_vxattrcb_##_name##_##_field,                   \
                                                              readonly : false,                                        \
...
#define XATTR_QUOTA_FIELD(_type, _name)                                                                      \
    {                                                                                                        \
    name:                                                                                                    \
        CEPH_XATTR_NAME(_type, _name), getxattr_cb : &Client::_vxattrcb_##_type##_##_name,                   \
                                                     readonly : false,                                       \
    ...
    if (strncmp(name, "ceph.", 5) == 0) {
        ...
        if (vxattr) {
            while (!vxattr->name.empty()) {
                if (vxattr->name == name) return vxattr;
                vxattr++;
            }
        }
...
int Client::_mknod(Inode* dir, const char* name, mode_t mode, dev_t rdev, const UserPerm& perms, InodeRef* inp)
{
    ldout(cct, 8) << "_mknod(" << dir->ino << " " << name << ", 0" << oct << mode << dec << ", " << rdev << ", uid "
                  << perms.uid() << ", gid " << perms.gid() << ")" << dendl;

    if (strlen(name) > NAME_MAX) return -CEPHFS_ENAMETOOLONG;
    ...
    if (xattrs_bl.length() > 0) req->set_data(xattrs_bl);
...
int Client::ll_mknod(Inode* parent, const char* name, mode_t mode, dev_t rdev, struct stat* attr, Inode** out,
                     const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
    ...
    tout(cct) << attr->st_ino << std::endl;
    ldout(cct, 3) << "ll_mknod " << vparent << " " << name << " = " << r << " (" << hex << attr->st_ino << dec << ")"
                  << dendl;
    *out = in.get();
    return r;
}

int Client::ll_mknodx(Inode* parent, const char* name, mode_t mode, dev_t rdev, Inode** out, struct ceph_statx* stx,
                      unsigned want, unsigned flags, const UserPerm& perms)
{
    ...
    tout(cct) << stx->stx_ino << std::endl;
    ldout(cct, 3) << "ll_mknodx " << vparent << " " << name << " = " << r << " (" << hex << stx->stx_ino << dec << ")"
                  << dendl;
    *out = in.get();
    return r;
}

int Client::_create(Inode* dir, const char* name, int flags, mode_t mode, InodeRef* inp, Fh** fhp, int stripe_unit,
                    int stripe_count, int object_size, const char* data_pool, bool* created, const UserPerm& perms,
                    std::string alternate_name)
{
    ...
    if (data_pool && *data_pool) {
        pool_id = objecter->with_osdmap(std::mem_fn(&OSDMap::lookup_pg_pool_name), data_pool);
        if (pool_id < 0) return -CEPHFS_EINVAL;
        if (pool_id > 0xffffffffll) return -CEPHFS_ERANGE;   // bummer!
    }
...
reply_error:
    ...
    ldout(cct, 8) << "create(" << path << ", 0" << oct << mode << dec << " layout " << stripe_unit << ' '
                  << stripe_count << ' ' << object_size << ") = " << res << dendl;
    return res;
}

int Client::_mkdir(Inode* dir, const char* name, mode_t mode, const UserPerm& perm, InodeRef* inp,
                   const std::map<std::string, std::string>& metadata, std::string alternate_name)
{
    ldout(cct, 8) << "_mkdir(" << dir->ino << " " << name << ", 0" << oct << mode << dec << ", uid " << perm.uid()
                  << ", gid " << perm.gid() << ")" << dendl;
    ...
    MetaRequest* req = new MetaRequest(is_snap_op ? CEPH_MDS_OP_MKSNAP : CEPH_MDS_OP_MKDIR);

    if (!is_snap_op) req->set_inode_owner_uid_gid(perm.uid(), perm.gid());
...
int Client::ll_mkdir(Inode* parent, const char* name, mode_t mode, struct stat* attr, Inode** out, const UserPerm& perm)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
    ...
    tout(cct) << attr->st_ino << std::endl;
    ldout(cct, 3) << "ll_mkdir " << vparent << " " << name << " = " << r << " (" << hex << attr->st_ino << dec << ")"
                  << dendl;
    *out = in.get();
    return r;
}

int Client::ll_mkdirx(Inode* parent, const char* name, mode_t mode, Inode** out, struct ceph_statx* stx, unsigned want,
                      unsigned flags, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
    ...
    tout(cct) << stx->stx_ino << std::endl;
    ldout(cct, 3) << "ll_mkdirx " << vparent << " " << name << " = " << r << " (" << hex << stx->stx_ino << dec << ")"
                  << dendl;
    *out = in.get();
    return r;
}

int Client::_symlink(Inode* dir, const char* name, const char* target, const UserPerm& perms,
                     std::string alternate_name, InodeRef* inp)
{
    ldout(cct, 8) << "_symlink(" << dir->ino << " " << name << ", " << target << ", uid " << perms.uid() << ", gid "
                  << perms.gid() << ")" << dendl;
...
int Client::ll_symlink(Inode* parent, const char* name, const char* value, struct stat* attr, Inode** out,
                       const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
    ...
    tout(cct) << attr->st_ino << std::endl;
    ldout(cct, 3) << "ll_symlink " << vparent << " " << name << " = " << r << " (" << hex << attr->st_ino << dec << ")"
                  << dendl;
    *out = in.get();
    return r;
}

int Client::ll_symlinkx(Inode* parent, const char* name, const char* value, Inode** out, struct ceph_statx* stx,
                        unsigned want, unsigned flags, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
    ...
    tout(cct) << stx->stx_ino << std::endl;
    ldout(cct, 3) << "ll_symlinkx " << vparent << " " << name << " = " << r << " (" << hex << stx->stx_ino << dec << ")"
                  << dendl;
    *out = in.get();
    return r;
}
...
{
    ldout(cct, 8) << "_unlink(" << dir->ino << " " << name << " uid " << perm.uid() << " gid " << perm.gid() << ")"
                  << dendl;
...
{
    ldout(cct, 8) << "_rmdir(" << dir->ino << " " << name << " uid " << perms.uid() << " gid " << perms.gid() << ")"
                  << dendl;
...
int Client::_rename(Inode* fromdir, const char* fromname, Inode* todir, const char* toname, const UserPerm& perm,
                    std::string alternate_name)
{
    ldout(cct, 8) << "_rename(" << fromdir->ino << " " << fromname << " to " << todir->ino << " " << toname << " uid "
                  << perm.uid() << " gid " << perm.gid() << ")" << dendl;

    if (fromdir->snapid != todir->snapid) return -CEPHFS_EXDEV;
    ...
    if (cct->_conf.get_val<bool>("client_quota") && fromdir != todir) {
        Inode* fromdir_root = fromdir->quota.is_enabled() ? fromdir : get_quota_root(fromdir, perm);
        Inode* todir_root = todir->quota.is_enabled() ? todir : get_quota_root(todir, perm);
...
int Client::ll_rename(Inode* parent, const char* name, Inode* newparent, const char* newname, const UserPerm& perm)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int Client::_link(Inode* in, Inode* dir, const char* newname, const UserPerm& perm, std::string alternate_name,
                  InodeRef* inp)
{
    ldout(cct, 8) << "_link(" << in->ino << " to " << dir->ino << " " << newname << " uid " << perm.uid() << " gid "
                  << perm.gid() << ")" << dendl;

    if (strlen(newname) > NAME_MAX) return -CEPHFS_ENAMETOOLONG;
...
int Client::ll_link(Inode* in, Inode* newparent, const char* newname, const UserPerm& perm)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
    ...
    if (!fuse_default_permissions) {
        if (S_ISDIR(in->mode)) return -CEPHFS_EPERM;
...
int Client::ll_get_stripe_osd(Inode* in, uint64_t blockno, file_layout_t* layout)
{
    std::scoped_lock lock(client_lock);
    ...
    return objecter->with_osdmap([&](const OSDMap& o) {
        ceph_object_layout olayout = o.file_to_object_layout(oid, *layout);
...
int Client::ll_opendir(Inode* in, int flags, dir_result_t** dirpp, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
out:
    ...
    ldout(cct, 3) << "ll_open " << vino << " " << ceph_flags_sys2wire(flags) << " = " << r << " (" << fhptr << ")"
                  << dendl;
    return r;
}

int Client::_ll_create(Inode* parent, const char* name, mode_t mode, int flags, InodeRef* in, int caps, Fh** fhp,
                       const UserPerm& perms)
{
    *fhp = NULL;
    ...
    ldout(cct, 8) << "_ll_create " << vparent << " " << name << " 0" << oct << mode << dec << " "
                  << ceph_flags_sys2wire(flags) << ", uid " << perms.uid() << ", gid " << perms.gid() << dendl;
    ...
    if (r == -CEPHFS_ENOENT && (flags & O_CREAT)) {
        ...
        r = _create(parent, name, flags, mode, in, fhp, 0, 0, 0, NULL, &created, perms, "");
        if (r < 0) goto out;
    }
...
out:
    ...
    ldout(cct, 8) << "_ll_create " << vparent << " " << name << " 0" << oct << mode << dec << " "
                  << ceph_flags_sys2wire(flags) << " = " << r << " (" << *fhp << " " << hex << ino << dec << ")"
                  << dendl;

    return r;
}

int Client::ll_create(Inode* parent, const char* name, mode_t mode, int flags, struct stat* attr, Inode** outp,
                      Fh** fhp, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
    ...
    int r = _ll_create(parent, name, mode, flags, &in, CEPH_STAT_CAP_INODE_ALL, fhp, perms);
    if (r >= 0) {
        ceph_assert(in);
...
int Client::ll_createx(Inode* parent, const char* name, mode_t mode, int oflags, Inode** outp, Fh** fhp,
                       struct ceph_statx* stx, unsigned want, unsigned lflags, const UserPerm& perms)
{
...
int Client::ll_read(Fh* fh, loff_t off, loff_t len, bufferlist* bl)
{
    ...
    ldout(cct, 3) << "ll_read " << fh << " " << fh->inode->ino << " "
                  << " " << off << "~" << len << dendl;
...
int Client::ll_read_block(Inode* in, uint64_t blockid, char* buf, uint64_t offset, uint64_t length,
                          file_layout_t* layout)
{
    ...
    objecter->read(
        oid, object_locator_t(layout->pool_id), offset, length, vino.snapid, &bl, CEPH_OSD_FLAG_READ, &onfinish);
...
int Client::ll_write_block(Inode* in, uint64_t blockid, char* buf, uint64_t offset, uint64_t length,
                           file_layout_t* layout, uint64_t snapseq, uint32_t sync)
{
    ...
    /* lock just in time */
    objecter->write(
        oid, object_locator_t(layout->pool_id), offset, length, fakesnap, bl, ceph::real_clock::now(), 0, onsafe.get());
...
int Client::ll_commit_blocks(Inode* in, uint64_t offset, uint64_t length)
{
    /*
    BarrierContext *bctx;
    vinodeno_t vino = _get_vino(in);
...
{
    ldout(cct, 20) << __func__ << " " << *in << "; " << perms << " defer " << defer << dendl;
    ...
    if ((in->mode & S_ISGID) && ((in->mode & S_IXUGO) || !perms.gid_in_groups(in->gid))) {
...
int Client::_fallocate(Fh* fh, int mode, int64_t offset, int64_t length)
{
    ...
    if (offset < 0 || length <= 0) return -CEPHFS_EINVAL;

    if (mode & ~(FALLOC_FL_KEEP_SIZE | FALLOC_FL_PUNCH_HOLE)) return -CEPHFS_EOPNOTSUPP;

    if ((mode & FALLOC_FL_PUNCH_HOLE) && !(mode & FALLOC_FL_KEEP_SIZE)) return -CEPHFS_EOPNOTSUPP;
    ...
    if (in->snapid != CEPH_NOSNAP) return -CEPHFS_EROFS;

    if ((fh->mode & CEPH_FILE_MODE_WR) == 0) return -CEPHFS_EBADF;

    uint64_t size = offset + length;
    if (!(mode & (FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE)) && size > in->size &&
    ...
    if (mode & FALLOC_FL_PUNCH_HOLE) {
        if (in->inline_version < CEPH_INLINE_NONE && (have & CEPH_CAP_FILE_BUFFER)) {
            bufferlist bl;
            auto inline_iter = in->inline_data.cbegin();
            int len = in->inline_data.length();
            if (offset < len) {
                if (offset > 0) inline_iter.copy(offset, bl);
                int size = length;
                if (offset + size > len) size = len - offset;
                if (size > 0) bl.append_zero(size);
                if (offset + size < len) {
                    inline_iter += size;
...
int Client::ll_release(Fh* fh)
{
    ...
    if (ll_unclosed_fh_set.count(fh)) ll_unclosed_fh_set.erase(fh);
...
int Client::set_deleg_timeout(uint32_t timeout)
{
     ...
     */
    if (timeout >= mdsmap->get_session_autoclose()) return -CEPHFS_EINVAL;
...
int Client::ll_delegation(Fh* fh, unsigned cmd, ceph_deleg_cb_t cb, void* priv)
{
    ...
    case CEPH_DELEGATION_NONE:
        ...
        ret = 0;
        break;
    default: try { ret = inode->set_deleg(fh, cmd, cb, priv);
        }
        catch (std::bad_alloc&) {
            ret = -CEPHFS_ENOMEM;
        }
...
public:
    C_Client_RequestInterrupt(Client* c, MetaRequest* r)
        : client(c)
        , req(r)
    {
        req->get();
...
int Client::describe_layout(const char* relpath, file_layout_t* lp, const UserPerm& perms)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
...
int64_t Client::get_pool_id(const char* pool_name)
{
    ...
    return objecter->with_osdmap(std::mem_fn(&OSDMap::lookup_pg_pool_name), pool_name);
}

string Client::get_pool_name(int64_t pool)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return string();

    std::scoped_lock lock(client_lock);

    return objecter->with_osdmap(
        [pool](const OSDMap& o) { return o.have_pg_pool(pool) ? o.get_pool_name(pool) : string(); });
}
...
int Client::get_pool_replication(int64_t pool)
{
    ...
    return objecter->with_osdmap(
        [pool](const OSDMap& o) { return o.have_pg_pool(pool) ? o.get_pg_pool(pool)->get_size() : -CEPHFS_ENOENT; });
}
...
int Client::get_file_extent_osds(int fd, loff_t off, loff_t* len, vector<int>& osds)
{
    ...
    if (osds.empty()) return -CEPHFS_EINVAL;
...
int Client::get_osd_crush_location(int id, vector<pair<string, string>>& path)
{
    ...
    if (id < 0) return -CEPHFS_EINVAL;
    return objecter->with_osdmap([&](const OSDMap& o) { return o.crush->get_full_location_ordered(id, path); });
}
...
int Client::get_osd_addr(int osd, entity_addr_t& addr)
{
    ...
    return objecter->with_osdmap([&](const OSDMap& o) {
        if (!o.exists(osd)) return -CEPHFS_ENOENT;
...
int Client::enumerate_layout(int fd, vector<ObjectExtent>& result, loff_t length, loff_t offset)
{
    RWRef_t mref_reader(mount_state, CLIENT_MOUNTING);
    if (!mref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;
 ...
 */
bool Client::check_quota_condition(Inode* in, const UserPerm& perms, std::function<bool(const Inode& in)> test)
{
    if (!cct->_conf.get_val<bool>("client_quota")) return false;
...
{
    return check_quota_condition(
        in, perms, [](const Inode& in) { return in.quota.max_files && in.rstat.rsize() >= in.quota.max_files; });
}

bool Client::is_quota_bytes_exceeded(Inode* in, int64_t new_bytes, const UserPerm& perms)
{
    return check_quota_condition(in, perms, [&new_bytes](const Inode& in) {
        return in.quota.max_bytes && (in.rstat.rbytes + new_bytes) > in.quota.max_bytes;
    });
}
...
bool Client::is_quota_bytes_approaching(Inode* in, const UserPerm& perms)
{
    ...
    return check_quota_condition(in, perms, [&size](const Inode& in) {
...
int Client::check_pool_perm(Inode* in, int need)
{
    ...
    if (!cct->_conf->client_check_pool_perm) return 0;

    /* Only need to do this for regular files */
    if (!in->is_file()) return 0;
    ...
    if (!have) {
        ...
        objecter->mutate(
            oid, OSDMap::file_to_object_locator(in->layout), rd_op, nullsnapc, ceph::real_clock::now(), 0, &rd_cond);
        ...
        objecter->mutate(
            oid, OSDMap::file_to_object_locator(in->layout), wr_op, nullsnapc, ceph::real_clock::now(), 0, &wr_cond);
        ...
        if (rd_ret == 0 || rd_ret == -CEPHFS_ENOENT)
            ...
            ldout(cct, 10) << __func__ << " on pool " << pool_id << " ns " << pool_ns << " rd_err = " << rd_ret
                           << " wr_err = " << wr_ret << dendl;
            errored = true;
        }
    ...
    if ((need & CEPH_CAP_FILE_RD) && !(have & POOL_READ)) {
        ldout(cct, 10) << __func__ << " on pool " << pool_id << " ns " << pool_ns << " need " << ccap_string(need)
...
int Client::_posix_acl_create(Inode* dir, mode_t* mode, bufferlist& xattrs_bl, const UserPerm& perms)
{
    if (acl_type == NO_ACL) return 0;
    ...
    if (acl_type == POSIX_ACL) {
        if (dir->xattrs.count(ACL_EA_DEFAULT)) {
            ...
            if (r > 0) {
                ...
                if (r < 0) goto out;
                if (r > 0) xattrs[ACL_EA_ACCESS] = acl;
            }

            if (S_ISDIR(*mode)) xattrs[ACL_EA_DEFAULT] = dir->xattrs[ACL_EA_DEFAULT];

            r = xattrs.size();
            if (r > 0) encode(xattrs, xattrs_bl);
        }
        else {
            if (umask_cb) *mode &= ~umask_cb(callback_handle);
            r = 0;
        }
    }
out:
...
{
    std::scoped_lock l(client_lock);
    ceph_assert(flags == 0 || flags == CEPH_OSD_FLAG_LOCALIZE_READS);
...
// called before mount
int Client::start_reclaim(const std::string& uuid, unsigned flags, const std::string& fs_name)
{
    RWRef_t iref_reader(initialize_state, CLIENT_INITIALIZED);
    if (!iref_reader.is_state_satisfied()) return -CEPHFS_ENOTCONN;

    if (uuid.empty()) return -CEPHFS_EINVAL;

    std::unique_lock l(client_lock);
    {
        auto it = metadata.find("uuid");
        if (it != metadata.end() && it->second == uuid) return -CEPHFS_EINVAL;
    }
    ...
    if (metadata.empty()) populate_metadata("");

    while (mdsmap->get_epoch() == 0) wait_on_list(waiting_for_mdsmap);
    ...
    for (unsigned mds = 0; mds < mdsmap->get_num_in_mds();) {
        ...
        if (!have_open_session(mds)) {
            session = _get_or_open_mds_session(mds);
            if (session->state == MetaSession::STATE_REJECTED) return -CEPHFS_EPERM;
            if (session->state != MetaSession::STATE_OPENING) {
                // umounting?
                return -CEPHFS_EINVAL;
            }
        ...
        session = mds_sessions.at(mds);
        if (!session->mds_features.test(CEPHFS_FEATURE_RECLAIM_CLIENT)) return -CEPHFS_EOPNOTSUPP;

        if (session->reclaim_state == MetaSession::RECLAIM_NULL || session->reclaim_state == MetaSession::RECLAIMING) {
    ...
    if (reclaim_target_addrs.empty()) {
        if (flags & CEPH_RECLAIM_RESET) return -CEPHFS_ENOENT;
        return -CEPHFS_ENOTRECOVERABLE;
    }

    if (flags & CEPH_RECLAIM_RESET) return 0;
    ...
    if (ec) return ceph::from_error_code(ec);

    bool blocklisted = objecter->with_osdmap(
        [this](const OSDMap& osd_map) -> bool { return osd_map.is_blocklisted(reclaim_target_addrs); });
    if (blocklisted) return -CEPHFS_ENOTRECOVERABLE;
...
void Client::finish_reclaim()
{
    ...
    if (it == metadata.end()) {
        for (auto& p : mds_sessions) p.second->reclaim_state = MetaSession::RECLAIM_NULL;
        return;
    }
...
void Client::handle_client_reclaim_reply(const MConstRef<MClientReclaimReply>& reply)
{
    ...
    if (reply->get_result() >= 0) {
        ...
        if (reply->get_epoch() > reclaim_osd_epoch) reclaim_osd_epoch = reply->get_epoch();
        if (!reply->get_addrs().empty()) reclaim_target_addrs = reply->get_addrs();
    }
    else {
...
{
    static const char* keys[] = {"client_cache_size",
                                 "client_cache_mid",
                                 "client_acl_type",
...
void Client::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    std::scoped_lock lock(client_lock);
...
mds_rank_t Client::_get_random_up_mds() const
{
    ...
    if (up.empty()) return MDS_RANK_NONE;
