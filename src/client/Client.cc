...
void Client::handle_client_request_forward(const MConstRef<MClientRequestForward>& fwd)
{
    ...
    if (num_fwd <= request->num_fwd || num_fwd >= max_fwd) {
        if (request->num_fwd >= max_fwd || num_fwd >= max_fwd) {
            ...
            ldout(cct, 10) << __func__ << " tid " << tid << " old fwd seq " << fwd->get_num_fwd() << " <= req fwd "
                           << request->num_fwd << ", ignore it" << dendl;
        }
        return;
    }
...
void Client::put_cap_ref(Inode* in, int cap)
{
    ...
    if (last) {
        ...
        if (in->snapid == CEPH_NOSNAP) {
            if ((last & (CEPH_CAP_FILE_WR | CEPH_CAP_FILE_BUFFER)) && !in->cap_snaps.empty() &&
...
void Client::queue_cap_snap(Inode* in, SnapContext& old_snapc)
{
    ...
    else if (in->caps_dirty() || (used & CEPH_CAP_FILE_WR) || (dirty & CEPH_CAP_ANY_WR)) {
...
void Client::flush_caps_sync()
{
    ...
    while (!p.end()) {
        ...
        if (p.end() && dirty_list.empty()) flags |= CHECK_CAPS_SYNCHRONOUS;
        check_caps(in, flags);
    }
...
int Client::may_setattr(Inode* in, struct ceph_statx* stx, int mask, const UserPerm& perms)
{
    ...
    if (mask & CEPH_SETATTR_MODE) {
        if (perms.uid() != 0 && perms.uid() != in->uid) goto out;
...
int Client::_do_setattr(Inode* in, struct ceph_statx* stx, int mask, const UserPerm& perms, InodeRef* inp)
{
    ...
    else if (in->caps_issued_mask(CEPH_CAP_AUTH_EXCL) && kill_sguid && S_ISREG(in->mode) &&
...
int Client::fallocate(int fd, int mode, loff_t offset, loff_t length)
{
    ...
    tout(cct) << __func__ << " "
              << " " << fd << mode << " " << offset << " " << length << std::endl;
