...
OpenFileDataCeph::OpenFileDataCeph(struct ceph_mount_info* cmount_, const String& fname, int _fd, int _flags)
    : cmount(cmount_)
    , fd(_fd)
    , flags(_flags)
    , filename(fname)
...
void CephBroker::open(ResponseCallbackOpen* cb, const char* fname, uint32_t flags, uint32_t bufsz)
{
    int fd, ceph_fd;
    String abspath;
...
void CephBroker::create(ResponseCallbackOpen* cb, const char* fname, uint32_t flags, int32_t bufsz, int16_t replication,
                        int64_t blksz)
{
    int fd, ceph_fd;
    int oflags;
...
void CephBroker::read(ResponseCallbackRead* cb, uint32_t fd, uint32_t amount)
{
    ...
    if ((offset = ceph_lseek(cmount, fdata->fd, 0, SEEK_CUR)) < 0) {
        ...
        HT_ERRORF("lseek failed: fd=%" PRIu32 " ceph_fd=%d offset=0 SEEK_CUR - %s", fd, fdata->fd, errs.c_str());
...
void CephBroker::append(ResponseCallbackAppend* cb, uint32_t fd, uint32_t amount, const void* data, bool sync)
{
    OpenFileDataCephPtr fdata;
    ssize_t nwritten;
    int64_t offset;

    HT_DEBUG_OUT << "append fd=" << fd << " amount=" << amount << " data='" << format_bytes(20, data, amount)
                 << " sync=" << sync << HT_END;
    ...
    if ((nwritten = ceph_write(cmount, fdata->fd, (const char*)data, amount, 0)) < 0) {
        ...
        HT_ERRORF("write failed: fd=%" PRIu32 " ceph_fd=%d amount=%d - %s", fd, fdata->fd, amount, errs.c_str());
...
void CephBroker::seek(ResponseCallback* cb, uint32_t fd, uint64_t offset)
{
    ...
    if (res < 0) {
        ...
        HT_ERRORF("lseek failed: fd=%" PRIu32 " ceph_fd=%d offset=%llu - %s", fd, fdata->fd, (Llu)offset, errs.c_str());
        report_error(cb, offset);
        return;
    }
...
void CephBroker::pread(ResponseCallbackRead* cb, uint32_t fd, uint64_t offset, uint32_t amount, bool)
{
    OpenFileDataCephPtr fdata;
    ssize_t nread;
    StaticBuffer buf(new uint8_t[amount], amount);
...
int CephBroker::rmdir_recursive(const char* directory)
{
    ...
    struct ceph_statx stx;
    int r;
    if ((r = ceph_opendir(cmount, directory, &dirp)) < 0) return r;   // failed to open
...
void CephBroker::readdir(ResponseCallbackReaddir* cb, const char* dname)
{
    ...
    while (1) {
        ...
        while (bufpos < r) {   // make new strings and add them to listing
            ...
            if (ent->compare(".") && ent->compare("..")) listing.push_back(*ent);
            bufpos += ent->size() + 1;
            delete ent;
        }
    }
...
void CephBroker::debug(ResponseCallback* cb, int32_t command, StaticBuffer& serialized_parameters)
{
