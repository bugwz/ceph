...
public:
    explicit C_Deleg_Timeout(Delegation* d)
        : deleg(d)
    {}
    void finish(int r) override
    {
        ...
        lsubdout(client->cct, client, 0) << __func__ << ": delegation return timeout for inode 0x" << std::hex
                                         << in->ino << ". Forcibly unmounting client. " << client << std::dec << dendl;
...
int ceph_deleg_caps_for_type(unsigned type)
{
    ...
    case CEPH_DELEGATION_RD:
        caps |= CEPH_CAP_FILE_SHARED | CEPH_CAP_FILE_RD | CEPH_CAP_FILE_CACHE | CEPH_CAP_XATTR_SHARED |
...
void Delegation::recall(bool skip_read)
{
    ...
    if (skip_read && type == CEPH_DELEGATION_RD) return;
