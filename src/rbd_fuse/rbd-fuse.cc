...
void enumerate_images(struct rbd_image_data* data)
{
    ...
    if (*head != NULL) {
        ...
        *head = NULL;
        rbd_image_spec_list_cleanup(data->image_specs, data->image_spec_count);
    ...
    while (true) {
        ret = rbd_list2(ioctx, data->image_specs, &data->image_spec_count);
        if (ret == -ERANGE) {
            data->image_specs = static_cast<rbd_image_spec_t*>(
                realloc(data->image_specs, sizeof(rbd_image_spec_t) * data->image_spec_count));
        }
        else if (ret < 0) {
    ...
    for (size_t idx = 0; idx < data->image_spec_count; ++idx) {
        if ((mount_image_name == NULL) ||
            ((strlen(mount_image_name) > 0) && (strcmp(data->image_specs[idx].name, mount_image_name) == 0))) {
...
int find_openrbd(const char* path)
{
    ...
    for (i = 0; i < MAX_RBD_IMAGES; i++) {
        if ((opentbl[i].image_name != NULL) && (strcmp(opentbl[i].image_name, path) == 0)) {
            return i;
        }
    }
    return -1;
...
int open_rbd_image(const char* image_name)
{
    ...
    if (image_name == (char*)NULL) return -1;
    ...
    if ((fd = find_openrbd(image_name)) != -1) {
        ...
        if (i == MAX_RBD_IMAGES || !rbd) return -1;
    ...
    rbd_stat(rbd->image, &(rbd->rbd_stat.rbd_info), sizeof(rbd_image_info_t));
    rbd->rbd_stat.valid = 1;
    return fd;
}

static void iter_images(void* cookie, void (*iter)(void* cookie, const char* image))
{
    struct rbd_image* im;

    readdir_lock.lock();

    for (im = rbd_image_data.images; im != NULL; im = im->next) iter(cookie, im->image_name);
    readdir_lock.unlock();
}
...
static int rbdfs_read(const char* path, char* buf, size_t size, off_t offset, struct fuse_file_info* fi)
{
    size_t numread;
...
static int rbdfs_write(const char* path, const char* buf, size_t size, off_t offset, struct fuse_file_info* fi)
{
    size_t numwritten;
    struct rbd_openimage* rbd;
    ...
    while (size > 0) {
        ...
        if ((size_t)(offset + size) > rbdsize(fi->fh)) {
            ...
            r = rbd_stat(rbd->image, &(rbd->rbd_stat.rbd_info), sizeof(rbd_image_info_t));
            if (r < 0) return r;
        }
...
static void rbdfs_statfs_image_cb(void* num, const char* image)
{
    ...
    if (fd >= 0) ((uint64_t*)num)[1] += rbdsize(fd);
}
...
static int rbdfs_fsync(const char* path, int datasync, struct fuse_file_info* fi)
{
    if (!gotrados) return -ENXIO;
...
static int rbdfs_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
                         ,
...
)
{
    ...
    if (!gotrados) return -ENXIO;
    if (!in_opendir) fprintf(stderr, "in readdir, but not inside opendir?\n");
...
void* rbdfs_init(struct fuse_conn_info* conn
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
                 ,
...
void rbdfs_destroy(void* unused)
{
    if (!gotrados) return;
...
int rbdfs_checkname(const char* checkname)
{
    const char* extra[] = {"@", "/"};
    std::string strCheckName(checkname);

    if (strCheckName.empty()) return -EINVAL;

    unsigned int sz = sizeof(extra) / sizeof(const char*);
    for (unsigned int i = 0; i < sz; i++) {
        std::string ex(extra[i]);
        if (std::string::npos != strCheckName.find(ex)) return -EINVAL;
    }
...
int rbdfs_create(const char* path, mode_t mode, struct fuse_file_info* fi)
{
    int r;
...
int rbdfs_utimens(const char* path, const struct timespec tv[2]
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
                  ,
                  struct fuse_file_info* fi
#endif
...
int rbdfs_unlink(const char* path)
{
...
int rbdfs_truncate(const char* path, off_t size
#if FUSE_VERSION >= FUSE_MAKE_VERSION(3, 0)
                   ,
                   struct fuse_file_info* fi
#endif
)
{
    ...
    if ((fd = open_rbd_image(path + 1)) < 0) return -ENOENT;
...
struct rbdfuse_attr
{
    char* attrname;
    uint64_t* attrvalp;
} attrs[] = {{(char*)"user.rbdfuse.imagesize", &imagesize},
             {(char*)"user.rbdfuse.imageorder", &imageorder},
             {(char*)"user.rbdfuse.imagefeatures", &imagefeatures},
             {NULL, NULL}};

int rbdfs_setxattr(const char* path, const char* name, const char* value, size_t size, int flags
#if defined(DARWIN)
                   ,
                   uint32_t pos
#endif
...
int rbdfs_getxattr(const char* path, const char* name, char* value, size_t size
#if defined(DARWIN)
                   ,
                   uint32_t position
#endif
)
{
    ...
    for (ap = attrs; ap->attrname != NULL; ap++) {
        if (strcmp(name, ap->attrname) == 0) {
            ...
            if (value != NULL && size >= strlen(buf)) strcpy(value, buf);
...
int rbdfs_listxattr(const char* path, char* list, size_t len)
{
    struct rbdfuse_attr* ap;
    ...
    for (ap = attrs; ap->attrname != NULL; ap++) required_len += strlen(ap->attrname) + 1;
    if (len >= required_len) {
        for (ap = attrs; ap->attrname != NULL; ap++) {
...
static struct fuse_opt rbdfs_opts[] = {
    ...
    {"--poolname=%s", offsetof(struct rbd_options, pool_name), KEY_RADOS_POOLNAME_LONG},
    {"-s %s", offsetof(struct rbd_options, nspace_name), KEY_RADOS_NSPACENAME},
    {"--namespace=%s", offsetof(struct rbd_options, nspace_name), KEY_RADOS_NSPACENAME_LONG},
    {"-r %s", offsetof(struct rbd_options, image_name), KEY_RBD_IMAGENAME},
    {"--image=%s", offsetof(struct rbd_options, image_name), KEY_RBD_IMAGENAME_LONG},
    FUSE_OPT_END};
...
static int rbdfs_opt_proc(void* data, const char* arg, int key, struct fuse_args* outargs)
{
    if (key == KEY_HELP) {
...
int main(int argc, const char* argv[])
{
    ...
    std::map<std::string, int> filter_options = {{"-r", OPT_NOT_FOUND}, {"-f", OPT_NOT_FOUND}, {"-d", OPT_NOT_FOUND}};
    ...
    auto cct = global_init(
        NULL, arg_vector, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_DAEMON, CINIT_FLAG_UNPRIVILEGED_DAEMON_DEFAULTS);
    ...
    for (auto& it : filter_options) {
        if (it.second != OPT_NOT_FOUND) {
            ...
            if (require_arg_options.count(it.first) && it.second + 1 < argc) {
                arg_vector.push_back(argv[it.second + 1]);
            }
        }
    }

    struct fuse_args args = FUSE_ARGS_INIT((int)arg_vector.size(), (char**)&arg_vector.front());
