...
static int accumulate_inode_metadata(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ceph_assert(in != NULL);
...
bool PGLSCephFSFilter::filter(const hobject_t& obj, const bufferlist& xattr_data) const
{
    ...
    const bool match =
        obj_name.compare(obj_name.length() - need_ending.length(), need_ending.length(), need_ending) == 0;
    if (!match) {
        return false;
    }
    ...
    if (!scrub_tag.empty() && xattr_data.length() > 0) {
        ...
        try {
            decode(tag_ondisk, q);
            if (tag_ondisk == scrub_tag) return false;
        }
        catch (const ceph::buffer::error& err) {
        }
    }
