 ...
 */
static int clslua_opresult(lua_State* L, int ok, int ret, int nargs, bool error_on_stack = false)
{
    struct clslua_err* err = clslua_checkerr(L);
    ...
    /* push error message */
    if (!error_on_stack) lua_pushfstring(L, "%s", strerror(-ret));
...
static int clslua_map_get_vals(lua_State* L)
{
    ...
    bool more;
    int ret = cls_cxx_map_get_vals(hctx, start_after, filter_prefix, max_to_get, &kvpairs, &more);
    if (ret < 0) return clslua_opresult(L, 0, ret, 0);
...
static int clslua_map_set_vals(lua_State* L)
{
    ...
    for (lua_pushnil(L); lua_next(L, 1); lua_pop(L, 1)) {
        ...
        switch (type_code) {
        case LUA_TSTRING: key.assign(lua_tolstring(L, -2, NULL)); break;
        ...
        default:
            lua_pushfstring(L, "map_set_vals: invalid key type (%s)", lua_typename(L, type_code));
        ...
        default:
            lua_pushfstring(
                L, "map_set_vals: invalid val type (%s) for key (%s)", lua_typename(L, type_code), key.c_str());
            return clslua_opresult(L, 0, -EINVAL, 0, true);
        }
 ...
 * }
 */
static int unpack_json_command(lua_State* L, struct clslua_hctx* ctx, std::string& script, std::string& handler,
                               std::string& input, size_t* input_len)
{
...
static int clslua_eval(lua_State* L)
{
    ...
    case JSON_ENC:
    {
        ...
        if (unpack_json_command(L, ctx, ctx->script, ctx->handler, input_str, &input_str_len)) return 0;
    ...
    if (luaL_loadstring(L, ctx->script.c_str())) return lua_error(L);
    ...
    if (!(lua_gettop(L) + 3 - top)) lua_pushinteger(L, 0);
 ...
 */
static int eval_generic(cls_method_context_t hctx, bufferlist* in, bufferlist* out, InputEncoding in_enc)
{
...
CLS_INIT(lua)
{
    ...
    cls_register_cxx_method(h_class, "eval_json", CLS_METHOD_RD | CLS_METHOD_WR, eval_json, &h_eval_json);

    cls_register_cxx_method(
        h_class, "eval_bufferlist", CLS_METHOD_RD | CLS_METHOD_WR, eval_bufferlist, &h_eval_bufferlist);
}
