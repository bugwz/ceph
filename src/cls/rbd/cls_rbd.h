...
struct cls_rbd_parent
{
    ...
    cls_rbd_parent() {}
    cls_rbd_parent(const cls::rbd::ParentImageSpec& parent_image_spec, const std::optional<uint64_t>& head_overlap)
    ...
    inline bool exists() const { return (pool_id >= 0 && !image_id.empty() && snap_id != CEPH_NOSNAP); }
    ...
    inline bool operator!=(const cls_rbd_parent& rhs) const { return !(*this == rhs); }
...
struct cls_rbd_snap
{
    ...
    uint64_t flags = 0;
    utime_t timestamp;
    cls::rbd::SnapshotNamespace snapshot_namespace = {cls::rbd::UserSnapshotNamespace{}};
    uint32_t child_count = 0;
    std::optional<uint64_t> parent_overlap = std::nullopt;

    cls_rbd_snap() {}
    cls_rbd_snap(snapid_t id, const std::string& name, uint64_t image_size, uint8_t protection_status,
                 const cls_rbd_parent& parent, uint64_t flags, utime_t timestamp,
                 const cls::rbd::SnapshotNamespace& snapshot_namespace, uint32_t child_count,
    ...
    {
        return (((features & CEPH_FEATURE_SERVER_NAUTILUS) != 0) && (parent.exists()));
    }
    ...
    void dump(ceph::Formatter* f) const
    {
        ...
        case RBD_PROTECTION_STATUS_UNPROTECTED: f->dump_string("protection_status", "unprotected"); break;
        case RBD_PROTECTION_STATUS_UNPROTECTING: f->dump_string("protection_status", "unprotecting"); break;
        case RBD_PROTECTION_STATUS_PROTECTED: f->dump_string("protection_status", "protected"); break;
        default: ceph_abort();
        }
    ...
    static void generate_test_instances(std::list<cls_rbd_snap*>& o)
    {
        ...
        o.push_back(new cls_rbd_snap{1,
                                     "snap",
                                     123456,
