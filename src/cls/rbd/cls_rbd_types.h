...
inline void encode(DirectoryState state, ceph::buffer::list& bl, uint64_t features = 0)
{
...
inline void encode(const GroupImageLinkState& state, ceph::buffer::list& bl, uint64_t features = 0)
{
    using ceph::encode;
...
{
    MirrorPeer() {}
    MirrorPeer(const std::string& uuid, MirrorPeerDirection mirror_peer_direction, const std::string& site_name,
               const std::string& client_name, const std::string& mirror_uuid)
        : uuid(uuid)
    ...
    inline bool is_valid() const
    {
        ...
        case MIRROR_PEER_DIRECTION_TX: break;
    ...
    bool operator!=(const MirrorPeer& rhs) const { return (!(*this == rhs)); }
};
...
{
    MirrorImage() {}
    MirrorImage(MirrorImageMode mode, const std::string& global_image_id, MirrorImageState state)
        : mode(mode)
...
inline void encode(const MirrorImageStatusState& state, ceph::buffer::list& bl, uint64_t features = 0)
{
    using ceph::encode;
    encode(static_cast<uint8_t>(state), bl);
}

inline void decode(MirrorImageStatusState& state, ceph::buffer::list::const_iterator& it)
{
    uint8_t int_state;
    using ceph::decode;
    decode(int_state, it);
...
struct MirrorImageSiteStatus
{
    ...
    MirrorImageSiteStatus(const std::string& mirror_uuid, MirrorImageStatusState state, const std::string& description)
        : mirror_uuid(mirror_uuid)
        , state(state)
...
struct MirrorImageSiteStatusOnDisk : cls::rbd::MirrorImageSiteStatus
{
    ...
    static void generate_test_instances(std::list<MirrorImageSiteStatusOnDisk*>& o);
};
...
struct ParentImageSpec
{
    ...
    ParentImageSpec() {}
    ParentImageSpec(int64_t pool_id, const std::string& pool_namespace, const std::string& image_id, snapid_t snap_id)
        : pool_id(pool_id)
    ...
    bool exists() const { return (pool_id >= 0 && !image_id.empty() && snap_id != CEPH_NOSNAP); }

    bool operator==(const ParentImageSpec& rhs) const
    {
        return ((pool_id == rhs.pool_id) && (pool_namespace == rhs.pool_namespace) && (image_id == rhs.image_id) &&
                (snap_id == rhs.snap_id));
    }

    bool operator!=(const ParentImageSpec& rhs) const { return !(*this == rhs); }
...
struct ChildImageSpec
{
    ...
    ChildImageSpec() {}
    ChildImageSpec(int64_t pool_id, const std::string& pool_namespace, const std::string& image_id)
        : pool_id(pool_id)
        , pool_namespace(pool_namespace)
        , image_id(image_id)
    {}
...
{
    GroupImageStatus() {}
    GroupImageStatus(const std::string& image_id, int64_t pool_id, GroupImageLinkState state)
        : spec(image_id, pool_id)
        , state(state)
    {}

    GroupImageStatus(GroupImageSpec spec, GroupImageLinkState state)
        : spec(spec)
        , state(state)
    {}
...
struct UserSnapshotNamespace
{
    ...
    inline bool operator==(const UserSnapshotNamespace& usn) const { return true; }

    inline bool operator!=(const UserSnapshotNamespace& usn) const { return false; }
...
struct GroupSnapshotNamespace
{
    ...
    GroupSnapshotNamespace(int64_t _group_pool, const std::string& _group_id, const std::string& _group_snapshot_id)
        : group_id(_group_id)
    ...
    {
        return group_pool == gsn.group_pool && group_id == gsn.group_id && group_snapshot_id == gsn.group_snapshot_id;
    }

    inline bool operator!=(const GroupSnapshotNamespace& gsn) const { return !operator==(gsn); }
...
{
    static const SnapshotNamespaceType SNAPSHOT_NAMESPACE_TYPE = SNAPSHOT_NAMESPACE_TYPE_TRASH;

    std::string original_name;
    SnapshotNamespaceType original_snapshot_namespace_type = SNAPSHOT_NAMESPACE_TYPE_USER;

    TrashSnapshotNamespace() {}
    TrashSnapshotNamespace(SnapshotNamespaceType original_snapshot_namespace_type, const std::string& original_name)
    ...
    inline bool operator==(const TrashSnapshotNamespace& usn) const { return true; }
    inline bool operator!=(const TrashSnapshotNamespace& usn) const { return false; }
    inline bool operator<(const TrashSnapshotNamespace& usn) const { return false; }
};
...
inline void encode(const MirrorSnapshotState& state, ceph::buffer::list& bl, uint64_t features = 0)
{
    using ceph::encode;
    encode(static_cast<uint8_t>(state), bl);
}
...
{
    static const SnapshotNamespaceType SNAPSHOT_NAMESPACE_TYPE = SNAPSHOT_NAMESPACE_TYPE_MIRROR;
    ...
    MirrorSnapshotNamespace(MirrorSnapshotState state, const std::set<std::string>& mirror_peer_uuids,
                            const std::string& primary_mirror_uuid, snapid_t primary_snap_id)
        : state(state)
    ...
    {}
    MirrorSnapshotNamespace(MirrorSnapshotState state, const std::set<std::string>& mirror_peer_uuids,
                            const std::string& primary_mirror_uuid, snapid_t primary_snap_id, bool complete,
                            uint64_t last_copied_object_number, const SnapSeqs& snap_seqs)
        : state(state)
        , complete(complete)
        , mirror_peer_uuids(mirror_peer_uuids)
        , primary_mirror_uuid(primary_mirror_uuid)
    ...
    {
        return (state == MIRROR_SNAPSHOT_STATE_PRIMARY || state == MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED);
    }

    inline bool is_non_primary() const
    {
        return (state == MIRROR_SNAPSHOT_STATE_NON_PRIMARY || state == MIRROR_SNAPSHOT_STATE_NON_PRIMARY_DEMOTED);
    }

    inline bool is_demoted() const
    {
        return (state == MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED || state == MIRROR_SNAPSHOT_STATE_NON_PRIMARY_DEMOTED);
    }

    inline bool is_orphan() const
    {
        return (is_non_primary() && primary_mirror_uuid.empty() && primary_snap_id == CEPH_NOSNAP);
    }
    ...
    {
        return state == rhs.state && complete == rhs.complete && mirror_peer_uuids == rhs.mirror_peer_uuids &&
               primary_mirror_uuid == rhs.primary_mirror_uuid && primary_snap_id == rhs.primary_snap_id &&
               last_copied_object_number == rhs.last_copied_object_number && snap_seqs == rhs.snap_seqs;
    }

    inline bool operator!=(const MirrorSnapshotNamespace& rhs) const { return !operator==(rhs); }
...
{
    static const SnapshotNamespaceType SNAPSHOT_NAMESPACE_TYPE = static_cast<SnapshotNamespaceType>(-1);
    ...
    inline bool operator==(const UnknownSnapshotNamespace& gsn) const { return true; }

    inline bool operator!=(const UnknownSnapshotNamespace& gsn) const { return false; }
...
typedef std::variant<UserSnapshotNamespace, GroupSnapshotNamespace, TrashSnapshotNamespace, MirrorSnapshotNamespace,
                     UnknownSnapshotNamespace>
...
struct SnapshotInfo
{
    ...
    SnapshotInfo() {}
    SnapshotInfo(snapid_t id, const cls::rbd::SnapshotNamespace& snapshot_namespace, const std::string& name,
                 uint64_t image_size, const utime_t& timestamp, uint32_t child_count)
        : id(id)
...
struct ImageSnapshotSpec
{
    ...
    ImageSnapshotSpec() {}
    ImageSnapshotSpec(int64_t _pool, std::string _image_id, snapid_t _snap_id)
        : pool(_pool)
        , image_id(_image_id)
        , snap_id(_snap_id)
    {}
...
struct GroupSnapshot
{
    ...
    GroupSnapshot() {}
    GroupSnapshot(std::string _id, std::string _name, GroupSnapshotState _state)
        : id(_id)
        , name(_name)
        , state(_state)
    {}
...
inline std::ostream& operator<<(std::ostream& os, const TrashImageSource& source)
{
    switch (source) {
    case TRASH_IMAGE_SOURCE_USER: os << "user"; break;
    case TRASH_IMAGE_SOURCE_MIRRORING: os << "mirroring"; break;
    case TRASH_IMAGE_SOURCE_MIGRATION: os << "migration"; break;
    case TRASH_IMAGE_SOURCE_REMOVING: os << "removing"; break;
    default: os << "unknown (" << static_cast<uint32_t>(source) << ")"; break;
    }
    return os;
}

inline void encode(const TrashImageSource& source, ceph::buffer::list& bl, uint64_t features = 0)
{
    using ceph::encode;
...
struct TrashImageSpec
{
    ...
    TrashImageSpec() {}
    TrashImageSpec(TrashImageSource source, const std::string& name, const utime_t& deletion_time,
    ...
    {
        return (source == rhs.source && name == rhs.name && deletion_time == rhs.deletion_time &&
...
struct MirrorImageMap
{
    ...
    MirrorImageMap(const std::string& instance_id, utime_t mapped_time, const ceph::buffer::list& data)
...
struct MigrationSpec
{
    ...
    MigrationSpec() {}
    MigrationSpec(MigrationHeaderType header_type, int64_t pool_id, const std::string& pool_namespace,
                  const std::string& image_name, const std::string& image_id, const std::string& source_spec,
                  const std::map<uint64_t, uint64_t>& snap_seqs, uint64_t overlap, bool mirroring,
                  MirrorImageMode mirror_image_mode, bool flatten, MigrationState state,
    ...
    {
        return header_type == ms.header_type && pool_id == ms.pool_id && pool_namespace == ms.pool_namespace &&
               image_name == ms.image_name && image_id == ms.image_id && source_spec == ms.source_spec &&
               snap_seqs == ms.snap_seqs && overlap == ms.overlap && flatten == ms.flatten &&
               mirroring == ms.mirroring && mirror_image_mode == ms.mirror_image_mode && state == ms.state &&
