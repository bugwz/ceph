...
int snapshot_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (std::holds_alternative<cls::rbd::UnknownSnapshotNamespace>(snap_meta.snapshot_namespace)) {
...
int image_snapshot_unlink_peer(cls_method_context_t hctx, uint64_t snap_id, std::string mirror_peer_uuid)
{
    ...
    auto mirror_ns = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
