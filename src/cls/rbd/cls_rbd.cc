...
bool calc_sparse_extent(const bufferptr& bp, size_t sparse_size, uint64_t length, size_t* write_offset,
...
static int snap_read_header(cls_method_context_t hctx, bufferlist& bl)
{
    ...
    while (1) {
        int len = sizeof(*header) + snap_count * sizeof(struct rbd_obj_snap_ondisk) + snap_names_len;

        int rc = cls_cxx_read(hctx, 0, len, &bl);
        if (rc < 0) return rc;

        if (bl.length() < sizeof(*header)) return -EINVAL;

        header = (struct rbd_obj_header_ondisk*)bl.c_str();
        ceph_assert(header);

        if ((snap_count != header->snap_count) || (snap_names_len != header->snap_names_len)) {
...
{
    ostringstream oss;
    oss << RBD_SNAP_KEY_PREFIX << std::setw(16) << std::setfill('0') << std::hex << snap_id;
    *out = oss.str();
}
...
template<typename T> static int read_key(cls_method_context_t hctx, const string& key, T* out)
{
    bufferlist bl;
...
template<typename T> static int write_key(cls_method_context_t hctx, const string& key, const T& t)
{
    bufferlist bl;
    encode(t, bl);
...
template<typename T> static int write_key(cls_method_context_t hctx, const string& key, const T& t, uint64_t features)
{
    bufferlist bl;
...
int require_feature(cls_method_context_t hctx, uint64_t need)
{
    ...
    if ((features & need) != need) {
        CLS_LOG(10,
                "require_feature missing feature %llx, have %llx",
...
{
    ostringstream oss;
    oss << RBD_SNAP_CHILDREN_KEY_PREFIX << std::setw(16) << std::setfill('0') << std::hex << snap_id;
    return oss.str();
}

int set_op_features(cls_method_context_t hctx, uint64_t op_features, uint64_t mask)
{
    ...
    CLS_LOG(10, "op_features=%" PRIu64 " orig_op_features=%" PRIu64, op_features, orig_op_features);
...
int set_migration(cls_method_context_t hctx, const cls::rbd::MigrationSpec& migration_spec, bool init)
{
    if (init) {
        bufferlist bl;
...
int read_migration(cls_method_context_t hctx, cls::rbd::MigrationSpec* migration_spec)
{
    uint64_t features = 0;
    int r = read_key(hctx, "features", &features);
    if (r == -ENOENT) {
...
template<typename L> int iterate(cls_method_context_t hctx, L& lambda)
{
    ...
    do {
        ...
        int r = cls_cxx_map_get_vals(hctx, last_read, RBD_SNAP_KEY_PREFIX, max_read, &vals, &more);
        if (r < 0) {
            return r;
        }
        ...
        for (auto& val : vals) {
            ...
            try {
                ...
                CLS_ERR("error decoding snapshot metadata for snap : %s", val.first.c_str());
                return -EIO;
            }
...
int write(cls_method_context_t hctx, const std::string& snap_key, cls_rbd_snap&& snap)
{
    int r;
...
int attach(cls_method_context_t hctx, cls_rbd_parent parent, bool reattach)
{
    ...
    if (r < 0) {
        CLS_LOG(20,
                "cls_rbd::image::parent::attach: child does not support "
                "layering");
        return r;
    }

    CLS_LOG(20,
            "cls_rbd::image::parent::attach: pool=%" PRIi64 ", ns=%s, id=%s, "
            "snapid=%" PRIu64 ", size=%" PRIu64,
            parent.pool_id,
            parent.pool_namespace.c_str(),
    ...
    if (r == 0 && (on_disk_parent.head_overlap || on_disk_parent_without_overlap != parent) && !reattach) {
        CLS_LOG(20,
                "cls_rbd::parent::attach: existing legacy parent "
                "pool=%" PRIi64 ", ns=%s, id=%s, snapid=%" PRIu64 ", "
                "overlap=%" PRIu64,
                on_disk_parent.pool_id,
                on_disk_parent.pool_namespace.c_str(),
                on_disk_parent.image_id.c_str(),
                on_disk_parent.snap_id.val,
...
int detach(cls_method_context_t hctx, bool legacy_api)
{
    ...
    if (r == -ENOENT || ((features & RBD_FEATURE_LAYERING) == 0)) {
        CLS_LOG(20,
                "cls_rbd::image::parent::detach: child does not support "
                "layering");
        return -ENOEXEC;
    }
    else if (r < 0) {
...
int create(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(20,
            "create object_prefix=%s size=%llu order=%u features=%llu",
            object_prefix.c_str(),
...
int get_features(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    uint64_t incompatible =
        (read_only ? features & RBD_FEATURES_INCOMPATIBLE : features & RBD_FEATURES_RW_INCOMPATIBLE);
    encode(features, *out);
...
int set_features(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if ((mask & RBD_FEATURES_INTERNAL) != 0ULL) {
        CLS_ERR("Attempting to set internal feature: %" PRIu64, static_cast<uint64_t>(mask & RBD_FEATURES_INTERNAL));
        return -EINVAL;
    }
    ...
    if ((disabled_features & disable_mask) != disabled_features) {
        CLS_ERR("Attempting to disable immutable feature: %" PRIu64, enabled_features & ~disable_mask);
        return -EINVAL;
    }

    features = (orig_features & ~mask) | (features & mask);
    CLS_LOG(10, "set_features features=%" PRIu64 " orig_features=%" PRIu64, features, orig_features);
...
int set_size(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(20, "set_size size=%llu orig_size=%llu", (unsigned long long)size, (unsigned long long)orig_size);
 ...
 */
int get_protection_status(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    snapid_t snap_id;
    ...
    CLS_LOG(20, "get_protection_status snap_id=%llu", (unsigned long long)snap_id.val);

    if (snap_id == CEPH_NOSNAP) return -EINVAL;
    ...
    if (snap.protection_status >= RBD_PROTECTION_STATUS_LAST) {
        CLS_ERR(
            "invalid protection status for snap id %llu: %u", (unsigned long long)snap_id.val, snap.protection_status);
        return -EIO;
    }
 ...
 * @returns -EINVAL if snapid is CEPH_NOSNAP
 */
int set_protection_status(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    snapid_t snap_id;
    uint8_t status;
    ...
    CLS_LOG(20, "set_protection_status snapid=%llu status=%u", (unsigned long long)snap_id.val, status);

    if (snap_id == CEPH_NOSNAP) return -EINVAL;

    if (status >= RBD_PROTECTION_STATUS_LAST) {
        CLS_LOG(10, "invalid protection status for snap id %llu: %u", (unsigned long long)snap_id.val, status);
        return -EINVAL;
    }
...
int set_stripe_unit_count(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (!stripe_count || !stripe_unit) return -EINVAL;
    ...
    if ((1ull << order) % stripe_unit || stripe_unit > (1ull << order)) {
        CLS_ERR(
            "stripe unit %llu is not a factor of the object size %llu", (unsigned long long)stripe_unit, 1ull << order);
        return -EINVAL;
    }
...
int set_flags(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (snap_id == CEPH_NOSNAP) {
        ...
        if (r < 0) {
            CLS_ERR("Could not read snapshot: snap_id=%" PRIu64 ": %s", snap_id, cpp_strerror(r).c_str());
            return r;
        }
    ...
    CLS_LOG(20,
            "set_flags snap_id=%" PRIu64 ", orig_flags=%" PRIu64 ", "
            "new_flags=%" PRIu64 ", mask=%" PRIu64,
            snap_id,
            orig_flags,
            flags,
            mask);
    ...
    if (r == 0) {
        ...
        if (snap_id != CEPH_NOSNAP) {
            ...
            if (snap.parent.exists()) {
                ...
                if (!parent.exists()) {
                    CLS_ERR("get_parent: snap_id=%" PRIu64 ": invalid parent spec", snap_id);
                    return -EINVAL;
                }
...
int parent_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    cls::rbd::ParentImageSpec parent_image_spec{parent.pool_id, parent.pool_namespace, parent.image_id, parent.snap_id};
 ...
 * @returns 0 on success, negative error code on failure
 */
int parent_overlap_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t snap_id;
    auto iter = in->cbegin();
    try {
...
int parent_attach(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    int r = image::parent::attach(hctx, {parent_image_spec, parent_overlap}, reattach);
    if (r < 0) {
        return r;
    }
...
static int decode_parent_common(bufferlist::const_iterator& it, uint64_t* pool_id, string* image_id, snapid_t* snap_id)
{
    try {
        decode(*pool_id, it);
        decode(*image_id, it);
...
static int decode_parent(bufferlist* in, uint64_t* pool_id, string* image_id, snapid_t* snap_id)
{
    auto it = in->cbegin();
    return decode_parent_common(it, pool_id, image_id, snap_id);
}

static int decode_parent_and_child(bufferlist* in, uint64_t* pool_id, string* image_id, snapid_t* snap_id,
                                   string* c_image_id)
{
    auto it = in->cbegin();
...
int add_child(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    r = decode_parent_and_child(in, &p_pool_id, &p_image_id, &p_snap_id, &c_image_id);
    if (r < 0) return r;

    CLS_LOG(20,
            "add_child %s to (%" PRIu64 ", %s, %" PRIu64 ")",
            c_image_id.c_str(),
            p_pool_id,
            p_image_id.c_str(),
            p_snap_id.val);
    ...
    if (r < 0) CLS_LOG(20, "add_child: omap write failed: %s", cpp_strerror(r).c_str());
    return r;
}
...
int remove_child(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(20,
            "remove_child %s from (%" PRIu64 ", %s, %" PRIu64 ")",
            c_image_id.c_str(),
            p_pool_id,
            p_image_id.c_str(),
            p_snap_id.val);
    ...
    if (children.empty()) {
        ...
        if (r < 0) CLS_LOG(20, "remove_child: remove key failed: %s", cpp_strerror(r).c_str());
    }
    else {
        ...
        encode(children, childbl);
        r = cls_cxx_map_set_val(hctx, key, &childbl);
        if (r < 0) CLS_LOG(20, "remove_child: write omap failed: %s", cpp_strerror(r).c_str());
    }
    return r;
}
...
int get_children(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(20, "get_children of (%" PRIu64 ", %s, %" PRIu64 ")", p_pool_id, p_image_id.c_str(), p_snap_id.val);

    string key = parent_key(p_pool_id, p_image_id, p_snap_id);

    r = read_key(hctx, key, &children);
    if (r < 0) {
        if (r != -ENOENT) CLS_LOG(20, "get_children: read omap failed: %s", cpp_strerror(r).c_str());
        return r;
    }
    encode(children, *out);
    return 0;
...
int get_snapcontext(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    do {
        ...
        for (auto it = keys.begin(); it != keys.end(); ++it) {
            if ((*it).find(RBD_SNAP_KEY_PREFIX) != 0) break;
            snapid_t snap_id = snap_id_from_key(*it);
            snap_ids.push_back(snap_id);
        }
        if (!keys.empty()) last_read = *(keys.rbegin());
    } while (more);
...
int get_object_prefix(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (r < 0) {
        CLS_ERR("failed to read the image's object prefix off of disk: %s", cpp_strerror(r).c_str());
        return r;
    }
...
int snapshot_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    cls::rbd::SnapshotInfo snapshot_info{
        snap.id, snap.snapshot_namespace, snap.name, snap.image_size, snap.timestamp, snap.child_count};
...
int snapshot_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (boost::get<cls::rbd::UnknownSnapshotNamespace>(&snap_meta.snapshot_namespace) != nullptr) {
        CLS_ERR("Unknown snapshot namespace provided");
        return -EINVAL;
    }

    CLS_LOG(20, "snapshot_add name=%s id=%llu", snap_meta.name.c_str(), (unsigned long long)snap_meta.id.val);

    if (snap_meta.id > CEPH_MAXSNAP) return -EINVAL;
    ...
    if (snap_meta.id < cur_snap_seq) return -ESTALE;
    ...
    auto pre_check_lambda = [&snap_meta, &total_read, snap_limit](const cls_rbd_snap& old_meta) {
        ++total_read;
        ...
        if ((snap_meta.name == old_meta.name && snap_meta.snapshot_namespace == old_meta.snapshot_namespace) ||
            snap_meta.id == old_meta.id) {
            CLS_LOG(20,
                    "snap_name %s or snap_id %" PRIu64 " matches existing snap "
                    "%s %" PRIu64,
                    snap_meta.name.c_str(),
                    snap_meta.id.val,
                    old_meta.name.c_str(),
                    old_meta.id.val);
            return -EEXIST;
    ...
    if (cls::rbd::get_snap_namespace_type(snap_meta.snapshot_namespace) == cls::rbd::SNAPSHOT_NAMESPACE_TYPE_TRASH) {
        // add snap_trash feature bit if not already enabled
        r = image::set_op_features(hctx, RBD_OPERATION_FEATURE_SNAP_TRASH, RBD_OPERATION_FEATURE_SNAP_TRASH);
        if (r < 0) {
            return r;
        }
    }
...
int snapshot_rename(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(20, "snapshot_rename id=%" PRIu64 ", dst_name=%s", src_snap_id.val, dst_snap_name.c_str());

    auto duplicate_name_lambda = [&dst_snap_name](const cls_rbd_snap& snap_meta) {
        if (cls::rbd::get_snap_namespace_type(snap_meta.snapshot_namespace) == cls::rbd::SNAPSHOT_NAMESPACE_TYPE_USER &&
            snap_meta.name == dst_snap_name) {
            CLS_LOG(20,
                    "snap_name %s matches existing snap with snap id %" PRIu64,
                    dst_snap_name.c_str(),
                    snap_meta.id.val);
            return -EEXIST;
        }
    ...
    if (r == -ENOENT) {
        CLS_LOG(20, "cannot find existing snap with snap id = %" PRIu64, src_snap_id.val);
        return r;
    }
...
int snapshot_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    auto remove_lambda = [snap_id, &has_child_snaps, &has_trash_snaps](const cls_rbd_snap& snap_meta) {
    ...
    ceph_release_t require_osd_release = cls_get_required_osd_release(hctx);
    if (has_parent && !is_head_child && !has_child_snaps && require_osd_release >= ceph_release_t::nautilus) {
 ...
 * @returns 0 on success, negative error code on failure
 */
int snapshot_trash_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    snapid_t snap_id;
    ...
    snap.snapshot_namespace = cls::rbd::TrashSnapshotNamespace{snap_type, snap.name};
    uuid_d uuid_gen;
...
int copyup(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (cls_cxx_stat(hctx, NULL, NULL) == 0) return 0;
...
int sparse_copyup(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    for (auto& it : extent_map) {
        ...
        if (r < 0) {
            CLS_ERR(
                "sparse_copyup: error writing extent %" PRIu64 "~%" PRIu64 ": %s", off, len, cpp_strerror(r).c_str());
            return r;
        }
    }
...
static int dir_add_image_helper(cls_method_context_t hctx, const string& name, const string& id,
                                bool check_for_unique_id)
{
    if (!name.size() || !is_valid_id(id)) {
        CLS_ERR("dir_add_image_helper: invalid name '%s' or id '%s'", name.c_str(), id.c_str());
        return -EINVAL;
    }
...
static int dir_remove_image_helper(cls_method_context_t hctx, const string& name, const string& id)
{
    CLS_LOG(20, "dir_remove_image_helper name=%s id=%s", name.c_str(), id.c_str());
    ...
    if (r < 0) {
        if (r != -ENOENT) CLS_ERR("error reading name to id mapping: %s", cpp_strerror(r).c_str());
        return r;
    }
...
int dir_get_id(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (r < 0) {
        if (r != -ENOENT) CLS_ERR("error reading id for name '%s': %s", name.c_str(), cpp_strerror(r).c_str());
        return r;
    }
    encode(id, *out);
    return 0;
...
int dir_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    while (more && images.size() < max_return) {
        ...
        int r = cls_cxx_map_get_vals(hctx, last_read, RBD_DIR_NAME_KEY_PREFIX, max_read, &vals, &more);
        if (r < 0) {
            if (r != -ENOENT) {
        ...
        for (auto it = vals.begin(); it != vals.end(); ++it) {
            ...
            if (images.size() >= max_return) break;
        }
        if (!vals.empty()) {
...
int dir_state_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    case cls::rbd::DIRECTORY_STATE_READY: break;
    case cls::rbd::DIRECTORY_STATE_ADD_DISABLED:
    {
        ...
        bool more;
        r = cls_cxx_map_get_vals(hctx, RBD_DIR_NAME_KEY_PREFIX, RBD_DIR_NAME_KEY_PREFIX, 1, &vals, &more);
        if (r < 0) {
            return r;
        }
        else if (!vals.empty()) {
...
int object_map_save(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(20, "object_map_save: object size=%" PRIu64 ", byte size=%u", object_map.size(), bl.length());
...
int object_map_resize(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(20, "object_map_resize: object size=%" PRIu64 ", byte size=%u", object_count, map.length());
...
int object_map_update(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    r = cls_cxx_read2(hctx, 0, object_map.get_header_length(), &header_bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
    if (r < 0) {
    ...
    r = cls_cxx_read2(hctx, object_byte_offset, byte_length, &footer_bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
    if (r < 0) {
    ...
    object_map.get_data_crcs_extents(start_object_no, object_count, &object_byte_offset, &byte_length);
    ...
    object_map.get_data_extents(start_object_no, object_count, &data_byte_offset, &object_byte_offset, &byte_length);

    bufferlist data_bl;
    r = cls_cxx_read2(hctx, object_byte_offset, byte_length, &data_bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
    if (r < 0) {
    ...
    try {
        ...
        CLS_ERR("failed to decode data chunk [%" PRIu64 "]: %s", data_byte_offset, err.what());
        return -EINVAL;
    }
    ...
    for (; it != end_it; ++it) {
        ...
        if ((!current_object_state || state == *current_object_state ||
             (*current_object_state == OBJECT_EXISTS && state == OBJECT_EXISTS_CLEAN)) &&
    ...
    if (updated) {
        CLS_LOG(20,
                "object_map_update: %" PRIu64 "~%" PRIu64 " -> %" PRIu64,
                data_byte_offset,
                byte_length,
                object_byte_offset);
        ...
        r = cls_cxx_write2(hctx, object_byte_offset, data_bl.length(), &data_bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
        if (r < 0) {
        ...
        r = cls_cxx_write2(
            hctx, footer_object_offset, footer_bl.length(), &footer_bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
        if (r < 0) {
 ...
 * @returns 0 on success, negative error code on failure
 */
int object_map_snap_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    BitVector<2> object_map;
    int r = object_map_read(hctx, object_map);
    if (r < 0) {
 ...
 * @returns 0 on success, negative error code on failure
 */
int object_map_snap_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    for (; dst_it != dst_it_end; ++dst_it) {
        if (*dst_it == OBJECT_EXISTS_CLEAN && (i >= src_object_map.size() || *src_it == OBJECT_EXISTS)) {
            *dst_it = OBJECT_EXISTS;
            updated = true;
        }
        if (i < src_object_map.size()) ++src_it;
        ++i;
    }
...
int metadata_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    while (more && data.size() < max_return) {
        ...
        int r = cls_cxx_map_get_vals(hctx, last_read, RBD_METADATA_KEY_PREFIX, max_read, &raw_data, &more);
        if (r < 0) {
            if (r != -ENOENT) {
...
int metadata_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    for (auto it = data.begin(); it != data.end(); ++it) {
        CLS_LOG(20, "metadata_set key=%s value=%.*s", it->first.c_str(), it->second.length(), it->second.c_str());
...
int metadata_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (r < 0) {
        if (r != -ENOENT) CLS_ERR("error getting metadata: %s", cpp_strerror(r).c_str());
        return r;
    }
...
int snapshot_get_limit(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t snap_limit;
...
int snapshot_set_limit(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    int rc;
    uint64_t new_limit;
    bufferlist bl;
    ...
    if (rc < 0 && rc != -EINVAL) {
        ...
        do {
            ...
            for (auto& key : keys) {
                if (key.find(RBD_SNAP_KEY_PREFIX) != 0) break;
                snap_count++;
            }
            if (!keys.empty()) last_read = *(keys.rbegin());
        } while (more);
...
int child_attach(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(20,
            "child_attach snap_id=%" PRIu64 ", child_pool_id=%" PRIi64 ", "
            "child_image_id=%s",
            snap_id,
            child_image.pool_id,
    ...
    if (cls::rbd::get_snap_namespace_type(snap.snapshot_namespace) == cls::rbd::SNAPSHOT_NAMESPACE_TYPE_TRASH) {
    ...
    r = image::set_op_features(hctx, RBD_OPERATION_FEATURE_CLONE_PARENT, RBD_OPERATION_FEATURE_CLONE_PARENT);
    if (r < 0) {
        return r;
    }
...
int child_detach(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(20,
            "child_detach snap_id=%" PRIu64 ", child_pool_id=%" PRIi64 ", "
            "child_image_id=%s",
            snap_id,
            child_image.pool_id,
            child_image.image_id.c_str());
 ...
 * @returns 0 on success, negative error code on failure
 */
int migration_set_state(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @returns 0 on success, negative error code on failure
 */
int migration_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @returns 0 on success, negative error code on failure
 */
int assert_snapc_seq(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t snapc_seq;
    ...
    switch (state) {
    case cls::rbd::ASSERT_SNAPC_SEQ_GT_SNAPSET_SEQ: return (r == -ENOENT || snapc_seq > snapset_seq) ? 0 : -ERANGE;
    case cls::rbd::ASSERT_SNAPC_SEQ_LE_SNAPSET_SEQ: return (r == -ENOENT || snapc_seq > snapset_seq) ? -ERANGE : 0;
...
int old_snapshot_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (header->snap_seq > snap_id) return -ESTALE;
    ...
    if (header->snap_count >= snap_limit) return -EDQUOT;

    const char* cur_snap_name;
    for (cur_snap_name = snap_names; cur_snap_name < end; cur_snap_name += strlen(cur_snap_name) + 1) {
        if (strncmp(cur_snap_name, snap_name, end - cur_snap_name) == 0) return -EEXIST;
    }
    if (cur_snap_name > end) return -EIO;
...
int old_snapshot_rename(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    const char* cur_snap_name;
    for (cur_snap_name = snap_names; cur_snap_name < end; cur_snap_name += strlen(cur_snap_name) + 1) {
        if (strcmp(cur_snap_name, dst_snap_name) == 0) return -EEXIST;
    }
    if (cur_snap_name > end) return -EIO;
...
std::string remote_status_global_key(const std::string& global_id, const std::string& mirror_uuid)
{
    return REMOTE_STATUS_GLOBAL_KEY_PREFIX + global_id + "_" + mirror_uuid;
}

std::string status_global_key(const std::string& global_id, const std::string& mirror_uuid)
{
...
int list_watchers(cls_method_context_t hctx, std::set<entity_inst_t>* entities)
{
...
int read_peers(cls_method_context_t hctx, std::vector<cls::rbd::MirrorPeer>* peers)
{
    ...
    while (more) {
        std::map<std::string, bufferlist> vals;
        int r = cls_cxx_map_get_vals(hctx, last_read, PEER_KEY_PREFIX.c_str(), max_read, &vals, &more);
        if (r < 0) {
            if (r != -ENOENT) {
...
int read_peer(cls_method_context_t hctx, const std::string& id, cls::rbd::MirrorPeer* peer)
{
    bufferlist bl;
    int r = cls_cxx_map_get_val(hctx, peer_key(id), &bl);
    if (r < 0) {
        CLS_ERR("error reading peer '%s': %s", id.c_str(), cpp_strerror(r).c_str());
        return r;
    }
...
int write_peer(cls_method_context_t hctx, const cls::rbd::MirrorPeer& peer)
{
    ...
    if (r < 0) {
        CLS_ERR("error writing peer '%s': %s", peer.uuid.c_str(), cpp_strerror(r).c_str());
        return r;
    }
    return 0;
}
...
int check_mirroring_enabled(cls_method_context_t hctx)
{
    ...
    else if (r == -ENOENT || mirror_mode_decode == cls::rbd::MIRROR_MODE_DISABLED) {
...
int peer_ping(cls_method_context_t hctx, const std::string& site_name, const std::string& mirror_uuid)
{
    ...
    auto site_it =
        std::find_if(peers.begin(), peers.end(), [&site_name](auto& peer) { return (peer.site_name == site_name); });

    auto mirror_uuid_it = peers.end();
    if (site_it == peers.end() || (!site_it->mirror_uuid.empty() && site_it->mirror_uuid != mirror_uuid)) {
        // search for existing peer w/ same mirror_uuid
        mirror_uuid_it = std::find_if(
            peers.begin(), peers.end(), [&mirror_uuid](auto& peer) { return (peer.mirror_uuid == mirror_uuid); });
    }
    ...
    if (it != peers.end()) {
        ...
        if (mirror_peer.mirror_peer_direction == cls::rbd::MIRROR_PEER_DIRECTION_RX) {
...
int peer_add(cls_method_context_t hctx, cls::rbd::MirrorPeer mirror_peer)
{
    ...
    if (r < 0) {
        ...
        CLS_ERR("peer uuid '%s' matches pool mirroring uuid", mirror_uuid.c_str());
        return -EINVAL;
    }
    else if (mirror_peer.mirror_peer_direction == cls::rbd::MIRROR_PEER_DIRECTION_TX) {
        CLS_ERR("peer uuid '%s' cannot use TX-only direction", mirror_peer.uuid.c_str());
        return -EINVAL;
    }
    ...
    for (auto const& peer : peers) {
        if (peer.uuid == mirror_peer.uuid) {
            CLS_ERR("peer uuid '%s' already exists", peer.uuid.c_str());
            return -ESTALE;
        }
        else if (peer.site_name == mirror_peer.site_name) {
            CLS_ERR("peer site name '%s' already exists", peer.site_name.c_str());
            return -EEXIST;
        }
        else if (!mirror_peer.mirror_uuid.empty() && peer.mirror_uuid == mirror_peer.mirror_uuid) {
            CLS_ERR("peer mirror uuid '%s' already exists", peer.mirror_uuid.c_str());
            return -EEXIST;
        }
    }
...
int image_get(cls_method_context_t hctx, const string& image_id, cls::rbd::MirrorImage* mirror_image)
{
    bufferlist bl;
    int r = cls_cxx_map_get_val(hctx, image_key(image_id), &bl);
    if (r < 0) {
        if (r != -ENOENT) {
            CLS_ERR("error reading mirrored image '%s': '%s'", image_id.c_str(), cpp_strerror(r).c_str());
        }
        return r;
    }
...
int image_set(cls_method_context_t hctx, const string& image_id, const cls::rbd::MirrorImage& mirror_image)
{
    bufferlist bl;
    ...
    if (r == -ENOENT) {
        ...
        if (r >= 0) {
            ...
            CLS_ERR("error reading global image id: '%s': '%s'", image_id.c_str(), cpp_strerror(r).c_str());
            return r;
        }
    ...
    else if (existing_mirror_image.global_image_id != mirror_image.global_image_id) {
    ...
    if (r < 0) {
        CLS_ERR("error adding mirrored image '%s': %s", image_id.c_str(), cpp_strerror(r).c_str());
        return r;
    }

    bufferlist image_id_bl;
    encode(image_id, image_id_bl);
    r = cls_cxx_map_set_val(hctx, global_key(mirror_image.global_image_id), &image_id_bl);
    if (r < 0) {
        CLS_ERR("error adding global id for image '%s': %s", image_id.c_str(), cpp_strerror(r).c_str());
        return r;
    }
    return 0;
}

int image_status_remove(cls_method_context_t hctx, const string& global_image_id);
...
int image_remove(cls_method_context_t hctx, const string& image_id)
{
    ...
    if (r < 0) {
        CLS_ERR("error removing mirrored image '%s': %s", image_id.c_str(), cpp_strerror(r).c_str());
        return r;
    }

    r = cls_cxx_map_remove_key(hctx, global_key(mirror_image.global_image_id));
    if (r < 0 && r != -ENOENT) {
        CLS_ERR("error removing global id for image '%s': %s", image_id.c_str(), cpp_strerror(r).c_str());
        return r;
    }
                     ...
                     const cls::rbd::MirrorImageSiteStatus& status)
{
    ...
    r = cls_cxx_map_set_val(hctx, status_global_key(global_image_id, status.mirror_uuid), &bl);
    if (r < 0) {
...
int get_remote_image_status_mirror_uuids(cls_method_context_t hctx, const std::string& global_image_id,
...
int image_status_remove(cls_method_context_t hctx, const string& global_image_id)
{
    ...
    std::set<std::string> mirror_uuids;
    int r = get_remote_image_status_mirror_uuids(hctx, global_image_id, &mirror_uuids);
    if (r < 0 && r != -ENOENT) {
        return r;
    }
    ...
    for (auto& mirror_uuid : mirror_uuids) {
        CLS_LOG(20, "removing status object for mirror_uuid %s", mirror_uuid.c_str());
        auto key = status_global_key(global_image_id, mirror_uuid);
        r = cls_cxx_map_remove_key(hctx, key);
        if (r < 0 && r != -ENOENT) {
            CLS_ERR("error removing stale status for key '%s': %s", key.c_str(), cpp_strerror(r).c_str());
            return r;
        }
    }
...
int image_status_get(cls_method_context_t hctx, const string& global_image_id, const std::string& mirror_uuid,
                     const bufferlist& bl, const std::set<entity_inst_t>& watchers, cls::rbd::MirrorImageStatus* status)
{
    ...
    auto site_status = static_cast<cls::rbd::MirrorImageSiteStatus>(ondisk_status);
...
int image_status_get_local(cls_method_context_t hctx, const string& global_image_id,
                           const std::set<entity_inst_t>& watchers, cls::rbd::MirrorImageStatus* status)
{
    bufferlist bl;
    int r = cls_cxx_map_get_val(
        hctx, status_global_key(global_image_id, cls::rbd::MirrorImageSiteStatus::LOCAL_MIRROR_UUID), &bl);
    if (r == -ENOENT) {
        return 0;
    }
    else if (r < 0) {
    ...
    return image_status_get(
        hctx, global_image_id, cls::rbd::MirrorImageSiteStatus::LOCAL_MIRROR_UUID, bl, watchers, status);
}

int image_status_get_remote(cls_method_context_t hctx, const string& global_image_id,
                            const std::set<entity_inst_t>& watchers, cls::rbd::MirrorImageStatus* status)
{
    std::string filter = remote_status_global_key(global_image_id, "");
    std::string last_read = filter;
    ...
    do {
        std::map<std::string, bufferlist> vals;
        CLS_LOG(20, "last_read = '%s'", last_read.c_str());
        int r = cls_cxx_map_get_vals(hctx, last_read, filter, max_read, &vals, &more);
        if (r == -ENOENT) {
            return 0;
        }
        else if (r < 0) {
        ...
        for (auto& it : vals) {
            ...
            r = image_status_get(hctx, global_image_id, mirror_uuid, it.second, watchers, status);
            if (r < 0) {
                return r;
            }
        }
...
int image_status_get(cls_method_context_t hctx, const string& global_image_id, const std::set<entity_inst_t>& watchers,
...
int image_status_list(cls_method_context_t hctx, const std::string& start_after, uint64_t max_return,
                      ...
                      map<std::string, cls::rbd::MirrorImageStatus>* mirror_statuses)
{
    ...
    while (more && mirror_images->size() < max_return) {
        std::map<std::string, bufferlist> vals;
        CLS_LOG(20, "last_read = '%s'", last_read.c_str());
        r = cls_cxx_map_get_vals(hctx, last_read, IMAGE_KEY_PREFIX, max_read, &vals, &more);
        if (r < 0) {
            if (r != -ENOENT) {
                CLS_ERR("error reading mirror image directory by name: %s", cpp_strerror(r).c_str());
            }
            return r;
        }

        for (auto it = vals.begin(); it != vals.end() && mirror_images->size() < max_return; ++it) {
            ...
            try {
                ...
                CLS_ERR("could not decode mirror image payload of image '%s'", image_id.c_str());
                return -EIO;
            }
            ...
            cls::rbd::MirrorImageStatus status;
            int r1 = image_status_get(hctx, mirror_image.global_image_id, watchers, &status);
            if (r1 < 0) {
                continue;
            }
...
cls::rbd::MirrorImageStatusState compute_image_status_summary_state(cls::rbd::MirrorPeerDirection mirror_peer_direction,
                                                                    ...
                                                                    const cls::rbd::MirrorImageStatus& status)
{
    ...
    case cls::rbd::MIRROR_PEER_DIRECTION_TX:
        ...
        for (auto& remote_status : status.mirror_image_site_statuses) {
            if (remote_status.mirror_uuid == cls::rbd::MirrorImageSiteStatus::LOCAL_MIRROR_UUID) {
                continue;
            }

            if (unmatched_tx_peers > 0 && tx_peer_mirror_uuids.count(remote_status.mirror_uuid) > 0) {
                --unmatched_tx_peers;
            }

            auto remote_state = (remote_status.up ? remote_status.state : cls::rbd::MIRROR_IMAGE_STATUS_STATE_UNKNOWN);
...
int image_status_get_summary(cls_method_context_t hctx, cls::rbd::MirrorPeerDirection mirror_peer_direction,
                             const std::set<std::string>& tx_peer_mirror_uuids,
    ...
    while (more) {
        ...
        for (auto& list_it : vals) {
            ...
            try {
                ...
                CLS_ERR("could not decode mirror image payload for key '%s'", key.c_str());
                return -EIO;
            }

            cls::rbd::MirrorImageStatus status;
            r = image_status_get(hctx, mirror_image.global_image_id, watchers, &status);
            if (r < 0 && r != -ENOENT) {
                return r;
            }

            auto state = compute_image_status_summary_state(mirror_peer_direction, tx_peer_mirror_uuids, status);
            (*states)[state]++;
        }
...
int image_status_remove_down(cls_method_context_t hctx)
{
    ...
    std::vector<std::string> prefixes = {STATUS_GLOBAL_KEY_PREFIX, REMOTE_STATUS_GLOBAL_KEY_PREFIX};
        ...
        while (more) {
            ...
            for (auto& list_it : vals) {
                ...
                try {
                    ...
                    CLS_ERR("could not decode status metadata for mirrored image '%s'", key.c_str());
                    return -EIO;
                }

                if (watchers.find(status.origin) == watchers.end()) {
                    CLS_LOG(20, "removing stale status object for key %s", key.c_str());
                    int r1 = cls_cxx_map_remove_key(hctx, key);
                    if (r1 < 0) {
                        CLS_ERR("error removing stale status for key '%s': %s", key.c_str(), cpp_strerror(r1).c_str());
                        return r1;
                    }
                }
            }
...
int image_instance_get(cls_method_context_t hctx, const string& global_image_id,
                       const std::set<entity_inst_t>& watchers, entity_inst_t* instance)
{
    ...
    try {
        ...
        CLS_ERR("could not decode status for mirrored image, global id '%s'", global_image_id.c_str());
        return -EIO;
    }
...
int image_instance_list(cls_method_context_t hctx, const std::string& start_after, uint64_t max_return,
                        map<std::string, entity_inst_t>* instances)
{
    ...
    while (more && instances->size() < max_return) {
        ...
        for (auto it = vals.begin(); it != vals.end() && instances->size() < max_return; ++it) {
            const std::string& image_id = it->first.substr(IMAGE_KEY_PREFIX.size());
            cls::rbd::MirrorImage mirror_image;
            auto iter = it->second.cbegin();
            try {
            ...
            r = image_instance_get(hctx, mirror_image.global_image_id, watchers, &instance);
            if (r < 0) {
                continue;
            }
...
int instances_list(cls_method_context_t hctx, std::vector<std::string>* instance_ids)
{
    ...
    while (more) {
        std::map<std::string, bufferlist> vals;
        int r = cls_cxx_map_get_vals(hctx, last_read, INSTANCE_KEY_PREFIX.c_str(), max_read, &vals, &more);
        if (r < 0) {
            if (r != -ENOENT) {
...
int instances_add(cls_method_context_t hctx, const string& instance_id)
{
    ...
    if (r < 0) {
        CLS_ERR("error setting mirror instance %s: %s", instance_id.c_str(), cpp_strerror(r).c_str());
        return r;
    }
    return 0;
}
...
int instances_remove(cls_method_context_t hctx, const string& instance_id)
{
    ...
    if (r < 0) {
        CLS_ERR("error removing mirror instance %s: %s", instance_id.c_str(), cpp_strerror(r).c_str());
        return r;
    }
    return 0;
}

int mirror_image_map_list(cls_method_context_t hctx, const std::string& start_after, uint64_t max_return,
                          std::map<std::string, cls::rbd::MirrorImageMap>* image_mapping)
{
    ...
    while (more && image_mapping->size() < max_return) {
        ...
        int r = cls_cxx_map_get_vals(hctx, last_read, MIRROR_IMAGE_MAP_KEY_PREFIX, max_read, &vals, &more);
        if (r < 0) {
        ...
        for (auto it = vals.begin(); it != vals.end(); ++it) {
            const std::string& global_image_id = it->first.substr(MIRROR_IMAGE_MAP_KEY_PREFIX.size());
...
int image_snapshot_unlink_peer(cls_method_context_t hctx, uint64_t snap_id, std::string mirror_peer_uuid)
{
    ...
    if (r < 0) {
        if (r != -ENOENT) {
            CLS_ERR("Could not read snapshot meta off disk: %s", cpp_strerror(r).c_str());
        }
        return r;
    }

    auto mirror_ns = boost::get<cls::rbd::MirrorSnapshotNamespace>(&snap.snapshot_namespace);
    if (mirror_ns == nullptr) {
        CLS_LOG(5,
                "mirror_image_snapshot_unlink_peer "
                "not mirroring snapshot snap_id=%" PRIu64,
                snap_id);
        return -EINVAL;
    }
    ...
    if (mirror_ns->mirror_peer_uuids.size() == 1) {
        ...
        auto search_lambda = [snap_id](const cls_rbd_snap& snap_meta) {
            if (snap_meta.id > snap_id &&
                boost::get<cls::rbd::MirrorSnapshotNamespace>(&snap_meta.snapshot_namespace) != nullptr) {
                return -EEXIST;
            }
            return 0;
        };
...
int image_snapshot_set_copy_progress(cls_method_context_t hctx, uint64_t snap_id, bool complete,
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_uuid_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string mirror_uuid;
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_uuid_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string mirror_uuid;
    try {
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_mode_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint32_t mirror_mode_decode;
    int r = read_key(hctx, mirror::MODE, &mirror_mode_decode);
    if (r < 0) {
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_mode_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint32_t mirror_mode_decode;
    try {
        auto bl_it = in->cbegin();
    ...
    case cls::rbd::MIRROR_MODE_DISABLED: enabled = false; break;
    case cls::rbd::MIRROR_MODE_IMAGE:
    case cls::rbd::MIRROR_MODE_POOL: enabled = true; break;
    default: CLS_ERR("invalid mirror mode: %d", mirror_mode_decode); return -EINVAL;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_peer_ping(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    try {
        ...
        uint8_t direction;
        decode(direction, it);
        mirror_peer_direction = static_cast<cls::rbd::MirrorPeerDirection>(direction);
    }
    catch (const ceph::buffer::error& err) {
        return -EINVAL;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_peer_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::vector<cls::rbd::MirrorPeer> peers;
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_peer_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    cls::rbd::MirrorPeer mirror_peer;
    try {
        auto it = in->cbegin();
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_peer_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string uuid;
    try {
        auto it = in->cbegin();
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_peer_set_client(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string uuid;
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_peer_set_cluster(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string uuid;
    std::string site_name;
    try {
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_peer_set_direction(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string uuid;
    cls::rbd::MirrorPeerDirection mirror_peer_direction;
    try {
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    while (more && mirror_images.size() < max_return) {
        std::map<std::string, bufferlist> vals;
        CLS_LOG(20, "last_read = '%s'", last_read.c_str());
        int r = cls_cxx_map_get_vals(hctx, last_read, mirror::IMAGE_KEY_PREFIX, max_read, &vals, &more);
        if (r < 0) {
            if (r != -ENOENT) {
                CLS_ERR("error reading mirror image directory by name: %s", cpp_strerror(r).c_str());
            }
        ...
        for (auto it = vals.begin(); it != vals.end(); ++it) {
            const std::string& image_id = it->first.substr(mirror::IMAGE_KEY_PREFIX.size());
            cls::rbd::MirrorImage mirror_image;
            auto iter = it->second.cbegin();
            try {
                decode(mirror_image, iter);
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_get_image_id(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (r < 0) {
        if (r != -ENOENT) {
            CLS_ERR("error retrieving image id for global id '%s': %s", global_id.c_str(), cpp_strerror(r).c_str());
        }
        return r;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    string image_id;
    try {
        auto it = in->cbegin();
 ...
 */
int mirror_image_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    string image_id;
    cls::rbd::MirrorImage mirror_image;
    try {
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    string image_id;
    try {
        auto it = in->cbegin();
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_status_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 *
 */
int mirror_image_status_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    string global_image_id;
    try {
        auto it = in->cbegin();
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_status_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    string global_image_id;
    try {
        auto it = in->cbegin();
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_status_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string start_after;
    uint64_t max_return;
    try {
    ...
    int r = mirror::image_status_list(hctx, start_after, max_return, &images, &statuses);
    if (r < 0) {
        return r;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_status_get_summary(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::vector<cls::rbd::MirrorPeer> peers;
    try {
        auto iter = in->cbegin();
    ...
    std::map<cls::rbd::MirrorImageStatusState, int32_t> states;
    int r = mirror::image_status_get_summary(hctx, mirror_peer_direction, tx_peer_mirror_uuids, &states);
    if (r < 0) {
        return r;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_status_remove_down(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_instance_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    string global_image_id;
    try {
        auto it = in->cbegin();
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_instance_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string start_after;
    uint64_t max_return;
    try {
    ...
    map<std::string, entity_inst_t> instances;
    int r = mirror::image_instance_list(hctx, start_after, max_return, &instances);
    if (r < 0) {
        return r;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_instances_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_instances_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_instances_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string instance_id;
    try {
        auto iter = in->cbegin();
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_map_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string start_after;
    uint64_t max_return;
    try {
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_map_update(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    int r = cls_cxx_map_set_val(hctx, key, &bl);
    if (r < 0) {
        CLS_ERR("error updating image map %s: %s", key.c_str(), cpp_strerror(r).c_str());
        return r;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_map_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string global_image_id;
    ...
    int r = cls_cxx_map_remove_key(hctx, key);
    if (r < 0 && r != -ENOENT) {
        CLS_ERR("error removing image map %s: %s", key.c_str(), cpp_strerror(r).c_str());
        return r;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_snapshot_unlink_peer(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t snap_id;
    ...
    CLS_LOG(
        20, "mirror_image_snapshot_unlink_peer snap_id=%" PRIu64 " peer_uuid=%s", snap_id, mirror_peer_uuid.c_str());
 ...
 * @returns 0 on success, negative error code on failure
 */
int mirror_image_snapshot_set_copy_progress(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t snap_id;
    bool complete;
    uint64_t last_copied_object_number;
    ...
    CLS_LOG(20,
            "mirror_image_snapshot_set_copy_progress snap_id=%" PRIu64
            " complete=%d last_copied_object_number=%" PRIu64,
            snap_id,
            complete,
            last_copied_object_number);

    int r = mirror::image_snapshot_set_copy_progress(hctx, snap_id, complete, last_copied_object_number);
    if (r < 0) {
        return r;
    }
    return 0;
...
int dir_add(cls_method_context_t hctx, const string& name, const string& id, bool check_for_unique_id)
{
    if (!name.size() || !is_valid_id(id)) {
        CLS_ERR("invalid group name '%s' or id '%s'", name.c_str(), id.c_str());
        return -EINVAL;
    }
...
int dir_remove(cls_method_context_t hctx, const string& name, const string& id)
{
    ...
    r = read_key(hctx, id_key, &stored_name);
    if (r < 0) {
        if (r != -ENOENT) CLS_ERR("error reading id to name mapping: %s", cpp_strerror(r).c_str());
        return r;
    }
...
int snap_list(cls_method_context_t hctx, cls::rbd::GroupSnapshot start_after, uint64_t max_return,
              std::vector<cls::rbd::GroupSnapshot>* group_snaps)
{
    ...
    bool more;
    do {
        int r = cls_cxx_map_get_vals(hctx, last_read, RBD_GROUP_SNAP_KEY_PREFIX, max_read, &vals, &more);
        if (r < 0) return r;
        ...
        for (auto it = vals.begin(); it != vals.end() && group_snaps->size() < max_return; ++it) {
            ...
            CLS_LOG(20, "Discovered snapshot %s %s", snap.name.c_str(), snap.id.c_str());
...
static int check_duplicate_snap_name(cls_method_context_t hctx, const std::string& snap_name,
...
int group_dir_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    while (more && groups.size() < max_return) {
        ...
        for (auto val : vals) {
            ...
            if (groups.size() >= max_return) break;
        }
        if (!vals.empty()) {
...
int group_dir_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (r < 0) {
        CLS_ERR("could not create group directory: %s", cpp_strerror(r).c_str());
        return r;
    }
 ...
 * @return 0 on success, negative error code on failure
 */
int group_image_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @return 0 on success, negative error code on failure
 */
int group_image_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    bool more;
    do {
        int r = cls_cxx_map_get_vals(hctx, last_read, cls::rbd::RBD_GROUP_IMAGE_KEY_PREFIX, max_read, &vals, &more);
        if (r < 0) return r;
        ...
        for (auto it = vals.begin(); it != vals.end() && res.size() < max_return; ++it) {
            ...
            CLS_LOG(20, "Discovered image %s %" PRId64 " %d", spec.image_id.c_str(), spec.pool_id, (int)state);
 ...
 * @return 0 on success, negative error code on failure
 */
int image_group_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (r == 0) {
        ...
        if ((old_group.group_id != new_group.group_id) || (old_group.pool_id != new_group.pool_id)) {
            return -EEXIST;
        }
        else {
    ...
    r = image::set_op_features(hctx, RBD_OPERATION_FEATURE_GROUP, RBD_OPERATION_FEATURE_GROUP);
    if (r < 0) {
        return r;
    }
 ...
 * @return 0 on success, negative error code on failure
 */
int image_group_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @return 0 on success, negative error code on failure
 */
int image_group_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @return 0 on success, negative error code on failure
 */
int group_snap_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @return 0 on success, negative error code on failure
 */
int group_snap_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @return 0 on success, negative error code on failure
 */
int group_snap_get_by_id(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @return 0 on success, negative error code on failure
 */
int group_snap_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
...
int trash_add(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (r < 0 && r != -ENOENT) {
        CLS_ERR("could not read key %s entry from trash: %s", key.c_str(), cpp_strerror(r).c_str());
        return r;
    }
    else if (r == 0) {
        CLS_LOG(10, "id already exists");
...
int trash_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    while (data.size() < max_return) {
        map<string, bufferlist> raw_data;
        int max_read = std::min<int32_t>(RBD_MAX_KEYS_READ, max_return - data.size());
        int r = cls_cxx_map_get_vals(hctx, last_read, trash::IMAGE_KEY_PREFIX, max_read, &raw_data, &more);
        if (r < 0) {
            if (r != -ENOENT) {
                CLS_ERR("failed to read the vals off of disk: %s", cpp_strerror(r).c_str());
            }
...
int trash_get(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (r < 0 && r != -ENOENT) {
        CLS_ERR("error reading image from trash '%s': '%s'", id.c_str(), cpp_strerror(r).c_str());
    }
    return r;
}
...
int trash_state_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (r < 0) {
        if (r != -ENOENT) {
            CLS_ERR("Could not read trash image spec off disk: %s", cpp_strerror(r).c_str());
        }
        return r;
    }
...
int namespace_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    while (data.size() < max_return) {
        ...
        int max_read = std::min<int32_t>(RBD_MAX_KEYS_READ, max_return - data.size());
        int r = cls_cxx_map_get_vals(hctx, last_read, nspace::NAME_KEY_PREFIX, max_read, &raw_data, &more);
        if (r < 0) {
            if (r != -ENOENT) {
                CLS_ERR("failed to read the vals off of disk: %s", cpp_strerror(r).c_str());
            }
...
int sparsify(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    bool replace = true;
    while (offset < length) {
        if (calc_sparse_extent(ptr, sparse_size, length, &write_offset, &write_length, &offset)) {
            if (write_offset == 0 && write_length == length) {
                CLS_LOG(20, "nothing to do");
                return 0;
            }
            CLS_LOG(20, "write%s %" PRIu64 "~%" PRIu64, (replace ? "(replace)" : ""), write_offset, write_length);
            bufferlist write_bl;
            write_bl.push_back(ceph::buffer::ptr_node::create(ptr, write_offset, write_length));
            if (replace) {
...
CLS_INIT(rbd)
{
    ...
    cls_register_cxx_method(h_class, "get_features", CLS_METHOD_RD, get_features, &h_get_features);
    cls_register_cxx_method(h_class, "set_features", CLS_METHOD_RD | CLS_METHOD_WR, set_features, &h_set_features);
    cls_register_cxx_method(h_class, "get_size", CLS_METHOD_RD, get_size, &h_get_size);
    cls_register_cxx_method(h_class, "set_size", CLS_METHOD_RD | CLS_METHOD_WR, set_size, &h_set_size);
    cls_register_cxx_method(h_class, "get_snapcontext", CLS_METHOD_RD, get_snapcontext, &h_get_snapcontext);
    cls_register_cxx_method(h_class, "get_object_prefix", CLS_METHOD_RD, get_object_prefix, &h_get_object_prefix);
    cls_register_cxx_method(h_class, "get_data_pool", CLS_METHOD_RD, get_data_pool, &h_get_data_pool);
    cls_register_cxx_method(h_class, "get_snapshot_name", CLS_METHOD_RD, get_snapshot_name, &h_get_snapshot_name);
    cls_register_cxx_method(
        h_class, "get_snapshot_timestamp", CLS_METHOD_RD, get_snapshot_timestamp, &h_get_snapshot_timestamp);
    cls_register_cxx_method(h_class, "snapshot_get", CLS_METHOD_RD, snapshot_get, &h_snapshot_get);
    cls_register_cxx_method(h_class, "snapshot_add", CLS_METHOD_RD | CLS_METHOD_WR, snapshot_add, &h_snapshot_add);
    cls_register_cxx_method(
        h_class, "snapshot_remove", CLS_METHOD_RD | CLS_METHOD_WR, snapshot_remove, &h_snapshot_remove);
    cls_register_cxx_method(
        h_class, "snapshot_rename", CLS_METHOD_RD | CLS_METHOD_WR, snapshot_rename, &h_snapshot_rename);
    cls_register_cxx_method(
        h_class, "snapshot_trash_add", CLS_METHOD_RD | CLS_METHOD_WR, snapshot_trash_add, &h_snapshot_trash_add);
    cls_register_cxx_method(h_class, "get_all_features", CLS_METHOD_RD, get_all_features, &h_get_all_features);

    // NOTE: deprecate v1 parent APIs after mimic EOLed
    cls_register_cxx_method(h_class, "get_parent", CLS_METHOD_RD, get_parent, &h_get_parent);
    cls_register_cxx_method(h_class, "set_parent", CLS_METHOD_RD | CLS_METHOD_WR, set_parent, &h_set_parent);
    cls_register_cxx_method(h_class, "remove_parent", CLS_METHOD_RD | CLS_METHOD_WR, remove_parent, &h_remove_parent);

    cls_register_cxx_method(h_class, "parent_get", CLS_METHOD_RD, parent_get, &h_parent_get);
    cls_register_cxx_method(h_class, "parent_overlap_get", CLS_METHOD_RD, parent_overlap_get, &h_parent_overlap_get);
    cls_register_cxx_method(h_class, "parent_attach", CLS_METHOD_RD | CLS_METHOD_WR, parent_attach, &h_parent_attach);
    cls_register_cxx_method(h_class, "parent_detach", CLS_METHOD_RD | CLS_METHOD_WR, parent_detach, &h_parent_detach);
    ...
    cls_register_cxx_method(
        h_class, "get_protection_status", CLS_METHOD_RD, get_protection_status, &h_get_protection_status);
    cls_register_cxx_method(
        h_class, "get_stripe_unit_count", CLS_METHOD_RD, get_stripe_unit_count, &h_get_stripe_unit_count);
    cls_register_cxx_method(h_class,
    ...
    cls_register_cxx_method(
        h_class, "get_create_timestamp", CLS_METHOD_RD, get_create_timestamp, &h_get_create_timestamp);
    cls_register_cxx_method(
        h_class, "get_access_timestamp", CLS_METHOD_RD, get_access_timestamp, &h_get_access_timestamp);
    cls_register_cxx_method(
        h_class, "get_modify_timestamp", CLS_METHOD_RD, get_modify_timestamp, &h_get_modify_timestamp);
    cls_register_cxx_method(h_class, "get_flags", CLS_METHOD_RD, get_flags, &h_get_flags);
    cls_register_cxx_method(h_class, "set_flags", CLS_METHOD_RD | CLS_METHOD_WR, set_flags, &h_set_flags);
    cls_register_cxx_method(h_class, "op_features_get", CLS_METHOD_RD, op_features_get, &h_op_features_get);
    cls_register_cxx_method(
        h_class, "op_features_set", CLS_METHOD_RD | CLS_METHOD_WR, op_features_set, &h_op_features_set);
    cls_register_cxx_method(h_class, "metadata_list", CLS_METHOD_RD, metadata_list, &h_metadata_list);
    cls_register_cxx_method(h_class, "metadata_set", CLS_METHOD_RD | CLS_METHOD_WR, metadata_set, &h_metadata_set);
    cls_register_cxx_method(
        h_class, "metadata_remove", CLS_METHOD_RD | CLS_METHOD_WR, metadata_remove, &h_metadata_remove);
    cls_register_cxx_method(h_class, "metadata_get", CLS_METHOD_RD, metadata_get, &h_metadata_get);
    cls_register_cxx_method(h_class, "snapshot_get_limit", CLS_METHOD_RD, snapshot_get_limit, &h_snapshot_get_limit);
    cls_register_cxx_method(
        h_class, "snapshot_set_limit", CLS_METHOD_RD | CLS_METHOD_WR, snapshot_set_limit, &h_snapshot_set_limit);
    cls_register_cxx_method(h_class, "child_attach", CLS_METHOD_RD | CLS_METHOD_WR, child_attach, &h_child_attach);
    cls_register_cxx_method(h_class, "child_detach", CLS_METHOD_RD | CLS_METHOD_WR, child_detach, &h_child_detach);
    cls_register_cxx_method(h_class, "children_list", CLS_METHOD_RD, children_list, &h_children_list);
    cls_register_cxx_method(h_class, "migration_set", CLS_METHOD_RD | CLS_METHOD_WR, migration_set, &h_migration_set);
    cls_register_cxx_method(
        h_class, "migration_set_state", CLS_METHOD_RD | CLS_METHOD_WR, migration_set_state, &h_migration_set_state);
    cls_register_cxx_method(h_class, "migration_get", CLS_METHOD_RD, migration_get, &h_migration_get);
    cls_register_cxx_method(
        h_class, "migration_remove", CLS_METHOD_RD | CLS_METHOD_WR, migration_remove, &h_migration_remove);

    cls_register_cxx_method(
        h_class, "set_modify_timestamp", CLS_METHOD_RD | CLS_METHOD_WR, set_modify_timestamp, &h_set_modify_timestamp);

    cls_register_cxx_method(
        h_class, "set_access_timestamp", CLS_METHOD_RD | CLS_METHOD_WR, set_access_timestamp, &h_set_access_timestamp);

    /* methods for the rbd_children object */
    cls_register_cxx_method(h_class, "add_child", CLS_METHOD_RD | CLS_METHOD_WR, add_child, &h_add_child);
    cls_register_cxx_method(h_class, "remove_child", CLS_METHOD_RD | CLS_METHOD_WR, remove_child, &h_remove_child);
    cls_register_cxx_method(h_class, "get_children", CLS_METHOD_RD, get_children, &h_get_children);

    /* methods for the rbd_id.$image_name objects */
    cls_register_cxx_method(h_class, "get_id", CLS_METHOD_RD, get_id, &h_get_id);
    cls_register_cxx_method(h_class, "set_id", CLS_METHOD_RD | CLS_METHOD_WR, set_id, &h_set_id);

    /* methods for the rbd_directory object */
    cls_register_cxx_method(h_class, "dir_get_id", CLS_METHOD_RD, dir_get_id, &h_dir_get_id);
    cls_register_cxx_method(h_class, "dir_get_name", CLS_METHOD_RD, dir_get_name, &h_dir_get_name);
    cls_register_cxx_method(h_class, "dir_list", CLS_METHOD_RD, dir_list, &h_dir_list);
    cls_register_cxx_method(h_class, "dir_add_image", CLS_METHOD_RD | CLS_METHOD_WR, dir_add_image, &h_dir_add_image);
    cls_register_cxx_method(
        h_class, "dir_remove_image", CLS_METHOD_RD | CLS_METHOD_WR, dir_remove_image, &h_dir_remove_image);
    cls_register_cxx_method(
        h_class, "dir_rename_image", CLS_METHOD_RD | CLS_METHOD_WR, dir_rename_image, &h_dir_rename_image);
    cls_register_cxx_method(h_class, "dir_state_assert", CLS_METHOD_RD, dir_state_assert, &h_dir_state_assert);
    cls_register_cxx_method(h_class, "dir_state_set", CLS_METHOD_RD | CLS_METHOD_WR, dir_state_set, &h_dir_state_set);

    /* methods for the rbd_object_map.$image_id object */
    cls_register_cxx_method(h_class, "object_map_load", CLS_METHOD_RD, object_map_load, &h_object_map_load);
    cls_register_cxx_method(
        h_class, "object_map_save", CLS_METHOD_RD | CLS_METHOD_WR, object_map_save, &h_object_map_save);
    cls_register_cxx_method(
        h_class, "object_map_resize", CLS_METHOD_RD | CLS_METHOD_WR, object_map_resize, &h_object_map_resize);
    cls_register_cxx_method(
        h_class, "object_map_update", CLS_METHOD_RD | CLS_METHOD_WR, object_map_update, &h_object_map_update);
    cls_register_cxx_method(
        h_class, "object_map_snap_add", CLS_METHOD_RD | CLS_METHOD_WR, object_map_snap_add, &h_object_map_snap_add);
    cls_register_cxx_method(h_class,
    ...
    cls_register_cxx_method(h_class, "snap_list", CLS_METHOD_RD, old_snapshots_list, &h_old_snapshots_list);
    cls_register_cxx_method(h_class, "snap_add", CLS_METHOD_RD | CLS_METHOD_WR, old_snapshot_add, &h_old_snapshot_add);
    cls_register_cxx_method(
        h_class, "snap_remove", CLS_METHOD_RD | CLS_METHOD_WR, old_snapshot_remove, &h_old_snapshot_remove);
    cls_register_cxx_method(
        h_class, "snap_rename", CLS_METHOD_RD | CLS_METHOD_WR, old_snapshot_rename, &h_old_snapshot_rename);

    /* methods for the rbd_mirroring object */
    cls_register_cxx_method(h_class, "mirror_uuid_get", CLS_METHOD_RD, mirror_uuid_get, &h_mirror_uuid_get);
    cls_register_cxx_method(
        h_class, "mirror_uuid_set", CLS_METHOD_RD | CLS_METHOD_WR, mirror_uuid_set, &h_mirror_uuid_set);
    cls_register_cxx_method(h_class, "mirror_mode_get", CLS_METHOD_RD, mirror_mode_get, &h_mirror_mode_get);
    cls_register_cxx_method(
        h_class, "mirror_mode_set", CLS_METHOD_RD | CLS_METHOD_WR, mirror_mode_set, &h_mirror_mode_set);
    cls_register_cxx_method(
        h_class, "mirror_peer_ping", CLS_METHOD_RD | CLS_METHOD_WR, mirror_peer_ping, &h_mirror_peer_ping);
    cls_register_cxx_method(h_class, "mirror_peer_list", CLS_METHOD_RD, mirror_peer_list, &h_mirror_peer_list);
    cls_register_cxx_method(
        h_class, "mirror_peer_add", CLS_METHOD_RD | CLS_METHOD_WR, mirror_peer_add, &h_mirror_peer_add);
    cls_register_cxx_method(
        h_class, "mirror_peer_remove", CLS_METHOD_RD | CLS_METHOD_WR, mirror_peer_remove, &h_mirror_peer_remove);
    cls_register_cxx_method(h_class,
    ...
    cls_register_cxx_method(h_class, "mirror_image_list", CLS_METHOD_RD, mirror_image_list, &h_mirror_image_list);
    cls_register_cxx_method(
        h_class, "mirror_image_get_image_id", CLS_METHOD_RD, mirror_image_get_image_id, &h_mirror_image_get_image_id);
    cls_register_cxx_method(h_class, "mirror_image_get", CLS_METHOD_RD, mirror_image_get, &h_mirror_image_get);
    cls_register_cxx_method(
        h_class, "mirror_image_set", CLS_METHOD_RD | CLS_METHOD_WR, mirror_image_set, &h_mirror_image_set);
    cls_register_cxx_method(
        h_class, "mirror_image_remove", CLS_METHOD_RD | CLS_METHOD_WR, mirror_image_remove, &h_mirror_image_remove);
    cls_register_cxx_method(h_class,
    ...
    cls_register_cxx_method(
        h_class, "mirror_image_status_get", CLS_METHOD_RD, mirror_image_status_get, &h_mirror_image_status_get);
    cls_register_cxx_method(
        h_class, "mirror_image_status_list", CLS_METHOD_RD, mirror_image_status_list, &h_mirror_image_status_list);
    cls_register_cxx_method(h_class,
    ...
    cls_register_cxx_method(
        h_class, "mirror_image_instance_get", CLS_METHOD_RD, mirror_image_instance_get, &h_mirror_image_instance_get);
    cls_register_cxx_method(h_class,
    ...
    cls_register_cxx_method(
        h_class, "mirror_image_map_list", CLS_METHOD_RD, mirror_image_map_list, &h_mirror_image_map_list);
    cls_register_cxx_method(
        h_class, "mirror_image_map_update", CLS_METHOD_WR, mirror_image_map_update, &h_mirror_image_map_update);
    cls_register_cxx_method(
        h_class, "mirror_image_map_remove", CLS_METHOD_WR, mirror_image_map_remove, &h_mirror_image_map_remove);
    cls_register_cxx_method(h_class,
                            "mirror_image_snapshot_unlink_peer",
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            mirror_image_snapshot_unlink_peer,
    ...
    cls_register_cxx_method(h_class, "group_dir_list", CLS_METHOD_RD, group_dir_list, &h_group_dir_list);
    cls_register_cxx_method(h_class, "group_dir_add", CLS_METHOD_RD | CLS_METHOD_WR, group_dir_add, &h_group_dir_add);
    cls_register_cxx_method(
        h_class, "group_dir_remove", CLS_METHOD_RD | CLS_METHOD_WR, group_dir_remove, &h_group_dir_remove);
    cls_register_cxx_method(
        h_class, "group_dir_rename", CLS_METHOD_RD | CLS_METHOD_WR, group_dir_rename, &h_group_dir_rename);
    cls_register_cxx_method(
        h_class, "group_image_remove", CLS_METHOD_RD | CLS_METHOD_WR, group_image_remove, &h_group_image_remove);
    cls_register_cxx_method(h_class, "group_image_list", CLS_METHOD_RD, group_image_list, &h_group_image_list);
    cls_register_cxx_method(
        h_class, "group_image_set", CLS_METHOD_RD | CLS_METHOD_WR, group_image_set, &h_group_image_set);
    cls_register_cxx_method(
        h_class, "image_group_add", CLS_METHOD_RD | CLS_METHOD_WR, image_group_add, &h_image_group_add);
    cls_register_cxx_method(
        h_class, "image_group_remove", CLS_METHOD_RD | CLS_METHOD_WR, image_group_remove, &h_image_group_remove);
    cls_register_cxx_method(h_class, "image_group_get", CLS_METHOD_RD, image_group_get, &h_image_group_get);
    cls_register_cxx_method(
        h_class, "group_snap_set", CLS_METHOD_RD | CLS_METHOD_WR, group_snap_set, &h_group_snap_set);
    cls_register_cxx_method(
        h_class, "group_snap_remove", CLS_METHOD_RD | CLS_METHOD_WR, group_snap_remove, &h_group_snap_remove);
    cls_register_cxx_method(
        h_class, "group_snap_get_by_id", CLS_METHOD_RD, group_snap_get_by_id, &h_group_snap_get_by_id);
    cls_register_cxx_method(h_class, "group_snap_list", CLS_METHOD_RD, group_snap_list, &h_group_snap_list);

    /* rbd_trash object methods */
    cls_register_cxx_method(h_class, "trash_add", CLS_METHOD_RD | CLS_METHOD_WR, trash_add, &h_trash_add);
    cls_register_cxx_method(h_class, "trash_remove", CLS_METHOD_RD | CLS_METHOD_WR, trash_remove, &h_trash_remove);
    cls_register_cxx_method(h_class, "trash_list", CLS_METHOD_RD, trash_list, &h_trash_list);
    cls_register_cxx_method(h_class, "trash_get", CLS_METHOD_RD, trash_get, &h_trash_get);
    cls_register_cxx_method(
        h_class, "trash_state_set", CLS_METHOD_RD | CLS_METHOD_WR, trash_state_set, &h_trash_state_set);

    /* rbd_namespace object methods */
    cls_register_cxx_method(h_class, "namespace_add", CLS_METHOD_RD | CLS_METHOD_WR, namespace_add, &h_namespace_add);
    cls_register_cxx_method(
        h_class, "namespace_remove", CLS_METHOD_RD | CLS_METHOD_WR, namespace_remove, &h_namespace_remove);
    cls_register_cxx_method(h_class, "namespace_list", CLS_METHOD_RD, namespace_list, &h_namespace_list);

    /* data object methods */
    cls_register_cxx_method(h_class, "copyup", CLS_METHOD_RD | CLS_METHOD_WR, copyup, &h_copyup);
    cls_register_cxx_method(h_class, "sparse_copyup", CLS_METHOD_RD | CLS_METHOD_WR, sparse_copyup, &h_sparse_copyup);
    cls_register_cxx_method(
        h_class, "assert_snapc_seq", CLS_METHOD_RD | CLS_METHOD_WR, assert_snapc_seq, &h_assert_snapc_seq);
    cls_register_cxx_method(h_class, "sparsify", CLS_METHOD_RD | CLS_METHOD_WR, sparsify, &h_sparsify);
}
