            ...
            for (auto it : op_ret.entries) {
                ...
                if (iter != urgent_data.urgent_data_map.end()) {
                    ...
                    if (iter->second > info.time) {
                        CLS_LOG(10,
                                "INFO: cls_rgw_gc_queue_list_entries(): tag found in urgent data: %s\n",
                                info.tag.c_str());
                        continue;
                    }
                }
                ...
                if (!found && urgent_data.num_xattr_urgent_entries > 0) {
                    ...
                    if (ret != -ENOENT && ret != -ENODATA) {
                        ...
                        try {
                            ...
                            CLS_LOG(
                                1, "ERROR: cls_rgw_gc_queue_list_entries(): failed to decode xattrs urgent data map\n");
                            return -EINVAL;
                        }   // end - catch
                        ...
                        if (xattr_iter != xattr_urgent_data_map.end()) {
                            if (xattr_iter->second > info.time) {
                                CLS_LOG(
                                    1,
                                    "INFO: cls_rgw_gc_queue_list_entries(): tag found in xattrs urgent data map: %s\n",
                                    info.tag.c_str());
                                continue;
                            }
                        }
                ...
                if (iter != urgent_data.urgent_data_map.end()) {
                    ...
                    if (iter->second > info.time) {
                        ...
                        CLS_LOG(10,
                                ...
                                info.tag.c_str());
                        urgent_data.urgent_data_map.erase(
                            info.tag);   // erase entry from map, as it will be removed later from queue
                ...
                if (!found && urgent_data.num_xattr_urgent_entries > 0) {
                    ...
                    if (ret != -ENOENT && ret != -ENODATA) {
                        ...
                        if (xattr_iter != xattr_urgent_data_map.end()) {
                            if (xattr_iter->second > info.time) {
                                CLS_LOG(10,
                                        "INFO: cls_rgw_gc_queue_remove_entries(): tag found in xattrs urgent data map: "
                                        "%s\n",
                                        info.tag.c_str());
                                continue;
                            }
                                ...
                                CLS_LOG(10,
                                        "INFO: cls_rgw_gc_queue_remove_entries(): erasing tag from xattrs urgent data: "
                                        "%s\n",
                                        info.tag.c_str());
                                xattr_urgent_data_map.erase(
                                    info.tag);   // erase entry from map, as it will be removed later
...
static int cls_rgw_gc_queue_update_entry(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (it != urgent_data.urgent_data_map.end()) {
        ...
        if (ret != -ENOENT && ret != -ENODATA) {
            ...
            if (xattr_iter != xattr_urgent_data_map.end()) {
                ...
                if (ret < 0) {
                    CLS_LOG(0,
                            "ERROR: %s(): cls_cxx_setxattr (attr=%s) returned %d",
                            __func__,
    ...
    if (!tag_found) {
        ...
        if (bl_urgent_data.length() > head.max_urgent_data_size) {
            ...
            if (ret < 0) {
                CLS_LOG(
                    0, "ERROR: %s(): cls_cxx_setxattr (attr=%s) returned %d", __func__, "cls_queue_urgent_data", ret);
                return ret;
            }
        }
    }

    if ((urgent_data.num_head_urgent_entries + urgent_data.num_xattr_urgent_entries) >
        urgent_data.num_urgent_data_entries) {
...
CLS_INIT(rgw_gc)
{
    ...
    cls_register_cxx_method(
        h_class, RGW_GC_QUEUE_INIT, CLS_METHOD_RD | CLS_METHOD_WR, cls_rgw_gc_queue_init, &h_rgw_gc_queue_init);
    ...
    cls_register_cxx_method(
        h_class, RGW_GC_QUEUE_LIST_ENTRIES, CLS_METHOD_RD, cls_rgw_gc_queue_list_entries, &h_rgw_gc_queue_list_entries);
    cls_register_cxx_method(h_class,
