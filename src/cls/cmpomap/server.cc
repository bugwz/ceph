...
template<typename T> static int compare_values(Op op, const T& lhs, const T& rhs)
{
    switch (op) {
...
static int compare_value(Mode mode, Op op, const bufferlist& input, const bufferlist& value)
{
    switch (mode) {
    case Mode::String: return compare_values(op, input, value);
    case Mode::U64:
        try {
...
static int cmp_vals(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    for (const auto& [key, input] : op.values) {
        ...
        if (v != values.end() && v->first == key) {
            ...
            ++v;
            CLS_LOG(20, "cmp_vals() comparing key=%s mode=%d op=%d", key.c_str(), (int)op.mode, (int)op.comparison);
        }
            ...
            CLS_LOG(
                20, "cmp_vals() comparing missing key=%s mode=%d op=%d", key.c_str(), (int)op.mode, (int)op.comparison);
        }
...
static int cmp_set_vals(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    for (const auto& [key, input] : op.values) {
        ...
        if (v != values.end() && v->first == key) {
            value = std::move(v->second);
            k = v++;
            CLS_LOG(20, "cmp_set_vals() comparing key=%s mode=%d op=%d", key.c_str(), (int)op.mode, (int)op.comparison);
        }
        else if (!op.default_value) {
            ...
            value = *op.default_value;
            CLS_LOG(20,
                    "cmp_set_vals() comparing missing key=%s mode=%d op=%d",
                    key.c_str(),
                    (int)op.mode,
                    (int)op.comparison);
        }
        ...
        if (r < 0) {
            CLS_LOG(10, "cmp_set_vals() failed to compare key=%s r=%d", key.c_str(), r);
            return r;
        }
        if (r == 0) {
...
static int cmp_rm_keys(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    for (const auto& [key, input] : op.values) {
        ...
        CLS_LOG(20, "cmp_rm_keys() comparing key=%s mode=%d op=%d", key.c_str(), (int)op.mode, (int)op.comparison);
        ...
        if (r == 0) {
            ...
            if (r < 0) {
                CLS_LOG(1, "ERROR: cmp_rm_keys() failed to remove key=%s r=%d", key.c_str(), r);
                return r;
            }
        }
    }
...
CLS_INIT(cmpomap)
{
    ...
    cls_register_cxx_method(h_class, "cmp_vals", CLS_METHOD_RD, cmp_vals, &h_cmp_vals);
    cls_register_cxx_method(h_class, "cmp_set_vals", CLS_METHOD_RD | CLS_METHOD_WR, cmp_set_vals, &h_cmp_set_vals);
    cls_register_cxx_method(h_class, "cmp_rm_keys", CLS_METHOD_RD | CLS_METHOD_WR, cmp_rm_keys, &h_cmp_rm_keys);
}
