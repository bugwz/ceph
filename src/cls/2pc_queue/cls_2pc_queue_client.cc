...
int cls_2pc_queue_reserve(IoCtx& io_ctx, const std::string& queue_name, uint64_t res_size, uint32_t entries,
...
void cls_2pc_queue_reserve(ObjectWriteOperation& op, uint64_t res_size, uint32_t entries, bufferlist* obl, int* prval)
{
    bufferlist in;
    cls_2pc_queue_reserve_op reserve_op;
    reserve_op.size = res_size;
...
#ifndef CLS_CLIENT_HIDE_IOCTX
int cls_2pc_queue_list_entries(IoCtx& io_ctx, const std::string& queue_name, const std::string& marker, uint32_t max,
