...
int cls_2pc_queue_reserve(librados::IoCtx& io_ctx, const std::string& queue_name, uint64_t res_size, uint32_t entries,
                          cls_2pc_reservation::id_t& res_id);

// incremental listing of all entries in the queue
int cls_2pc_queue_list_entries(librados::IoCtx& io_ctx, const std::string& queue_name, const std::string& marker,
                               uint32_t max, std::vector<cls_queue_entry>& entries, bool* truncated,
                               std::string& next_marker);

// list all pending reservations in the queue
int cls_2pc_queue_list_reservations(librados::IoCtx& io_ctx, const std::string& queue_name,
...
// optionally async method for making a reservation on the queue (in bytes) and number of expected entries (to calculate
// overhead) notes: (1) make sure that librados::OPERATION_RETURNVEC is passed to the executing function (2) multiple
// operations cannot be executed in a batch (operations both read and write) after answer is received, call
// cls_2pc_queue_reserve_result() to parse the results
void cls_2pc_queue_reserve(librados::ObjectWriteOperation& op, uint64_t res_size, uint32_t entries, bufferlist* obl,
                           int* prval);
...
void cls_2pc_queue_abort(librados::ObjectWriteOperation& op, cls_2pc_reservation::id_t res_id);
...
void cls_2pc_queue_list_entries(librados::ObjectReadOperation& op, const std::string& marker, uint32_t max,
                                bufferlist* obl, int* prval);

int cls_2pc_queue_list_entries_result(const bufferlist& bl, std::vector<cls_queue_entry>& entries, bool* truncated,
                                      std::string& next_marker);
