...
struct chunk_refs_t
{
    ...
    struct refs_t
    {
        ...
        virtual std::string describe_encoding() const { return type_name(get_type()); }
    };
    ...
    std::string describe_encoding() const { return r->describe_encoding(); }

    bool empty() const { return r->empty(); }
    uint64_t count() const { return r->count(); }

    void get(const hobject_t& o) { r->get(o); }
    ...
    void dump(Formatter* f) const { r->dump(f); }
    static void generate_test_instances(std::list<chunk_refs_t*>& ls) { ls.push_back(new chunk_refs_t()); }
};
...
struct chunk_refs_by_object_t : public chunk_refs_t::refs_t
{
    ...
    uint8_t get_type() const { return chunk_refs_t::TYPE_BY_OBJECT; }
    bool empty() const override { return by_object.empty(); }
    uint64_t count() const override { return by_object.size(); }
    void get(const hobject_t& o) override { by_object.insert(o); }
...
struct chunk_refs_by_hash_t : public chunk_refs_t::refs_t
{
    ...
    uint8_t get_type() const { return chunk_refs_t::TYPE_BY_HASH; }
    bool empty() const override { return by_hash.empty(); }
    ...
    DENC_HELPERS
    void bound_encode(size_t& p) const { p += 6 + sizeof(uint64_t) + by_hash.size() * (10 + 10); }
...
struct chunk_refs_by_pool_t : public chunk_refs_t::refs_t
{
    ...
    uint8_t get_type() const { return chunk_refs_t::TYPE_BY_POOL; }
    bool empty() const override { return by_pool.empty(); }
    uint64_t count() const override { return total; }
    void get(const hobject_t& o) override
    {
        ++by_pool[o.pool];
    ...
    void bound_encode(size_t& p) const { p += 6 + sizeof(uint64_t) + by_pool.size() * (9 + 9); }
    DENC_HELPERS
    void encode(::ceph::buffer::list::contiguous_appender& p) const
    {
        DENC_START(1, 1, p);
...
struct chunk_refs_count_t : public chunk_refs_t::refs_t
{
    ...
    uint8_t get_type() const { return chunk_refs_t::TYPE_COUNT; }
    bool empty() const override { return total == 0; }
    uint64_t count() const override { return total; }
    void get(const hobject_t& o) override { ++total; }
    bool put(const hobject_t& o) override
    {
        if (!total) {
            return false;
