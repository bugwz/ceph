...
void chunk_refs_t::_encode_r(ceph::bufferlist& bl) const
{
    using ceph::encode;
    switch (r->get_type()) {
    case TYPE_BY_OBJECT: encode(*(chunk_refs_by_object_t*)r.get(), bl); break;
    case TYPE_BY_HASH: encode(*(chunk_refs_by_hash_t*)r.get(), bl); break;
    case TYPE_BY_POOL: encode(*(chunk_refs_by_pool_t*)r.get(), bl); break;
    case TYPE_COUNT: encode(*(chunk_refs_count_t*)r.get(), bl); break;
    default: ceph_abort("unrecognized ref type");
    }
}
...
void chunk_refs_t::dynamic_encode(ceph::buffer::list& bl, size_t max)
{
    ...
    while (true) {
        ...
        switch (r->get_type()) {
        case TYPE_BY_OBJECT: r.reset(new chunk_refs_by_hash_t(static_cast<chunk_refs_by_object_t*>(r.get()))); break;
        case TYPE_BY_HASH:
            if (!static_cast<chunk_refs_by_hash_t*>(r.get())->shrink()) {
                r.reset(new chunk_refs_by_pool_t(static_cast<chunk_refs_by_hash_t*>(r.get())));
            }
            break;
        case TYPE_BY_POOL: r.reset(new chunk_refs_count_t(r.get())); break;
        }
        t.clear();
    }
    _encode_final(bl, t);
...
void chunk_refs_t::decode(ceph::buffer::list::const_iterator& p)
{
    ...
    default:
        throw ceph::buffer::malformed_input(std::string("unrecognized chunk ref encoding type ") + stringify((int)t));
    }
    DECODE_FINISH(p);
}
