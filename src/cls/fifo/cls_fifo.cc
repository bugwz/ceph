...
int read_part_header(cls_method_context_t hctx, part_header* part_header)
{
    ceph::buffer::list bl;
...
int write_part_header(cls_method_context_t hctx, part_header& part_header)
{
    ceph::buffer::list bl;
    ...
    int r = cls_cxx_write2(hctx, 0, bl.length(), &bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
    if (r < 0) {
        CLS_ERR("%s: failed to write part header: r=%d", __PRETTY_FUNCTION__, r);
        return r;
    }
...
int read_header(cls_method_context_t hctx, std::optional<objv> objv, info* info, bool get_info = false)
{
    std::uint64_t size;
...
int create_meta(cls_method_context_t hctx, ceph::buffer::list* in, ceph::buffer::list* out)
{
    ...
    if (op.max_part_size == 0 || op.max_entry_size == 0 || op.max_entry_size > op.max_part_size) {
    ...
    if (r == 0) {
        CLS_LOG(5, "%s: FIFO already exists, reading from disk and comparing.", __PRETTY_FUNCTION__);
        ceph::buffer::list bl;
        r = cls_cxx_read2(hctx, 0, size, &bl, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);
        if (r < 0) {
            CLS_ERR("ERROR: %s: cls_cxx_read2() on obj returned %d", __PRETTY_FUNCTION__, r);
        ...
        try {
            ...
            CLS_ERR("ERROR: %s: failed decoding header: %s", __PRETTY_FUNCTION__, err.what());
            return -EIO;
        }

        if (!(header.id == op.id && (!op.oid_prefix || header.oid_prefix == *op.oid_prefix) &&
              (!op.version || header.version == *op.version))) {
...
int update_meta(cls_method_context_t hctx, ceph::buffer::list* in, ceph::buffer::list* out)
{
    CLS_LOG(5, "%s", __PRETTY_FUNCTION__);
    ...
    auto u = fifo::update()
                 ...
                 .journal_entries_add(std::move(op.journal_entries_add))
                 .journal_entries_rm(std::move(op.journal_entries_rm));
    ...
    if (changed) {
        ...
        CLS_LOG(10, "%s: No change, nothing to write.", __PRETTY_FUNCTION__);
    }

    return 0;
}

int get_meta(cls_method_context_t hctx, ceph::buffer::list* in, ceph::buffer::list* out)
{
    CLS_LOG(5, "%s", __PRETTY_FUNCTION__);
...
int init_part(cls_method_context_t hctx, ceph::buffer::list* in, ceph::buffer::list* out)
{
    CLS_LOG(5, "%s", __PRETTY_FUNCTION__);
    ...
    cls_gen_random_bytes(reinterpret_cast<char*>(&part_header.magic), sizeof(part_header.magic));
...
int push_part(cls_method_context_t hctx, ceph::buffer::list* in, ceph::buffer::list* out)
{
    CLS_LOG(5, "%s", __PRETTY_FUNCTION__);
    ...
    std::uint64_t effective_len = op.total_len + op.data_bufs.size() * part_entry_overhead;
    ...
    if (total_data != op.total_len) {
        CLS_ERR("%s: length mismatch: op.total_len=%" PRId64 " total data received=%" PRId64,
                __PRETTY_FUNCTION__,
                op.total_len,
                total_data);
        return -EINVAL;
    ...
    for (auto& data : op.data_bufs) {
        if (full_part(part_header)) break;
    ...
    r = cls_cxx_write2(hctx, write_ofs, write_len, &all_data, CEPH_OSD_OP_FLAG_FADVISE_WILLNEED);

    if (r < 0) {
        CLS_ERR(
            "%s: failed to write entries (ofs=%" PRIu64 " len=%u): r=%d", __PRETTY_FUNCTION__, write_ofs, write_len, r);
        return r;
    }
...
public:
    EntryReader(cls_method_context_t hctx, const fifo::part_header& part_header, uint64_t ofs)
        : hctx(hctx)
        , part_header(part_header)
        , ofs(ofs < part_header.min_ofs ? part_header.min_ofs : ofs)
    {}

    std::uint64_t get_ofs() const { return ofs; }

    bool end() const { return (ofs >= part_header.next_ofs); }

    int peek_pre_header(entry_header_pre* pre_header);
    int get_next_entry(ceph::buffer::list* pbl, std::uint64_t* pofs, ceph::real_time* pmtime);
};
...
{
    CLS_LOG(5,
            "%s: fetch %d bytes, ofs=%d data.length()=%d",
            __PRETTY_FUNCTION__,
            (int)num_bytes,
            (int)ofs,
            (int)data.length());
    if (data.length() < num_bytes) {
        ceph::buffer::list bl;
        CLS_LOG(5,
                "%s: reading % " PRId64 " bytes at ofs=%" PRId64,
                __PRETTY_FUNCTION__,
                prefetch_len,
                ofs + data.length());
...
int EntryReader::peek_pre_header(entry_header_pre* pre_header)
{
    ...
    int r = peek(sizeof(*pre_header), reinterpret_cast<char*>(pre_header));
    if (r < 0) {
        CLS_ERR("ERROR: %s: peek() size=%zu failed: r=%d", __PRETTY_FUNCTION__, sizeof(pre_header), r);
        return r;
    }
...
int EntryReader::get_next_entry(ceph::buffer::list* pbl, std::uint64_t* pofs, ceph::real_time* pmtime)
{
    entry_header_pre pre_header;
    ...
    CLS_LOG(5, "%s:%d: pre_header.pre_size=%" PRIu64, __PRETTY_FUNCTION__, __LINE__, uint64_t(pre_header.pre_size));
...
int trim_part(cls_method_context_t hctx, ceph::buffer::list* in, ceph::buffer::list* out)
{
    CLS_LOG(5, "%s", __PRETTY_FUNCTION__);
...
int list_part(cls_method_context_t hctx, ceph::buffer::list* in, ceph::buffer::list* out)
{
    CLS_LOG(5, "%s", __PRETTY_FUNCTION__);
    ...
    if (op.ofs >= part_header.min_ofs && !reader.end()) {
        r = reader.get_next_entry(nullptr, nullptr, nullptr);
        if (r < 0) {
            CLS_ERR("ERROR: %s: unexpected failure at get_next_entry: r=%d", __PRETTY_FUNCTION__, r);
            return r;
...
int get_part_info(cls_method_context_t hctx, ceph::buffer::list* in, ceph::buffer::list* out)
{
    CLS_LOG(5, "%s", __PRETTY_FUNCTION__);
...
CLS_INIT(fifo)
{
    ...
    cls_register_cxx_method(h_class, op::CREATE_META, CLS_METHOD_RD | CLS_METHOD_WR, create_meta, &h_create_meta);

    cls_register_cxx_method(h_class, op::GET_META, CLS_METHOD_RD, get_meta, &h_get_meta);

    cls_register_cxx_method(h_class, op::UPDATE_META, CLS_METHOD_RD | CLS_METHOD_WR, update_meta, &h_update_meta);

    cls_register_cxx_method(h_class, op::INIT_PART, CLS_METHOD_RD | CLS_METHOD_WR, init_part, &h_init_part);

    cls_register_cxx_method(h_class, op::PUSH_PART, CLS_METHOD_RD | CLS_METHOD_WR, push_part, &h_push_part);

    cls_register_cxx_method(h_class, op::TRIM_PART, CLS_METHOD_RD | CLS_METHOD_WR, trim_part, &h_trim_part);

    cls_register_cxx_method(h_class, op::LIST_PART, CLS_METHOD_RD, list_part, &h_list_part);

    cls_register_cxx_method(h_class, op::GET_PART_INFO, CLS_METHOD_RD, get_part_info, &h_get_part_info);
