...
struct objv
{
    ...
    bool operator==(const objv& rhs) const { return (instance == rhs.instance && ver == rhs.ver); }
    bool operator!=(const objv& rhs) const { return (instance != rhs.instance || ver != rhs.ver); }
    bool same_or_later(const objv& rhs) const { return (instance == rhs.instance && ver >= rhs.ver); }

    bool empty() const { return instance.empty(); }

    std::string to_str() const { return fmt::format("{}{{{}}}", instance, ver); }
};
...
{
    switch (o) {
    case journal_entry::Op::unknown: return m << "Op::unknown";
    case journal_entry::Op::create: return m << "Op::create";
    case journal_entry::Op::set_head: return m << "Op::set_head";
    case journal_entry::Op::remove: return m << "Op::remove";
    }
...
public:
    ...
    auto tail_part_num() const noexcept { return tail_part_num_; }
    ...
    auto head_part_num() const noexcept { return head_part_num_; }

    update&& min_push_part_num(std::optional<std::int64_t> num) noexcept
    {
        min_push_part_num_ = num;
        return std::move(*this);
    }
    auto min_push_part_num() const noexcept { return min_push_part_num_; }
    ...
    auto max_push_part_num() const noexcept { return max_push_part_num_; }
    ...
    update&& journal_entries_add(std::optional<std::vector<fifo::journal_entry>>&& entries)
    {
        if (entries) {
    ...
    const auto& journal_entries_add() const& noexcept { return journal_entries_add_; }
    auto&& journal_entries_add() && noexcept { return std::move(journal_entries_add_); }
    ...
    update&& journal_entries_rm(std::optional<std::vector<fifo::journal_entry>>&& entries)
    {
        if (entries) {
    ...
    const auto& journal_entries_rm() const& noexcept { return journal_entries_rm_; }
    auto&& journal_entries_rm() && noexcept { return std::move(journal_entries_rm_); }
...
struct info
{
    ...
    bool need_new_head() const { return (head_part_num < min_push_part_num); }

    bool need_new_part() const { return (max_push_part_num < min_push_part_num); }
    ...
    std::string part_oid(std::int64_t part_num) const { return fmt::format("{}.{}", oid_prefix, part_num); }
    ...
    bool apply_update(const update& update)
    {
        ...
        if (update.min_push_part_num() && (min_push_part_num != *update.min_push_part_num())) {
            min_push_part_num = *update.min_push_part_num();
            changed = true;
        }

        if (update.max_push_part_num() && (max_push_part_num != *update.max_push_part_num())) {
...
inline std::ostream& operator<<(std::ostream& m, const part_list_entry& p)
{
    using ceph::operator<<;
...
#if FMT_VERSION >= 90000
template<> struct fmt::formatter<rados::cls::fifo::info> : fmt::ostream_formatter
{};
template<> struct fmt::formatter<rados::cls::fifo::part_header> : fmt::ostream_formatter
{};
#endif
