...
/* implicit create should be true only if called from a write operation (set, inc), never from a read operation (read,
 * check) */
...
static bool check_conds(list<obj_version_cond>& conds, obj_version& objv)
{
    ...
    for (list<obj_version_cond>::iterator iter = conds.begin(); iter != conds.end(); ++iter) {
        ...
        switch (cond.cond) {
        case VER_COND_NONE: break;
        case VER_COND_EQ:
            if (!objv.compare(&v)) return false;
            break;
        case VER_COND_GT:
            if (!(objv.ver > v.ver)) return false;
            break;
        case VER_COND_GE:
            if (!(objv.ver >= v.ver)) return false;
            break;
        ...
        case VER_COND_TAG_EQ:
            if (objv.tag.compare(v.tag) != 0) return false;
            break;
