...
int rgw_bucket_prepare_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(1,
            "rgw_bucket_prepare_op(): request: op=%d name=%s instance=%s tag=%s",
            op.op,
            op.key.name.c_str(),
            op.key.instance.c_str(),
            op.tag.c_str());
...
int rgw_bucket_complete_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(1,
            "rgw_bucket_complete_op(): request: op=%d name=%s instance=%s ver=%lu:%llu tag=%s",
            op.op,
            op.key.name.c_str(),
            op.key.instance.c_str(),
    ...
    if (op.op == CLS_RGW_OP_CANCEL) {
        ...
        if (op.tag.size()) {
            if (!entry.exists && entry.pending_map.empty()) {
                ...
                CLS_LOG(20, "INFO: %s: removing map entry with key=%s", __func__, escape_str(idx).c_str());
                rc = cls_cxx_map_remove_key(hctx, idx);
                if (rc < 0) {
                    CLS_LOG(
                        1, "ERROR: %s: unable to remove map key, key=%s, rc=%d", __func__, escape_str(idx).c_str(), rc);
                    return rc;
                }
            }
            else {
                // we removed this tag from pending_map so need to write the changes
                CLS_LOG(20, "INFO: %s: setting map entry at key=%s", __func__, escape_str(idx).c_str());
                bufferlist new_key_bl;
                encode(entry, new_key_bl);
                rc = cls_cxx_map_set_val(hctx, idx, &new_key_bl);
                if (rc < 0) {
                    CLS_LOG(
                        1, "ERROR: %s: unable to set map val, key=%s, rc=%d", __func__, escape_str(idx).c_str(), rc);
                    return rc;
                }
            }
        }
...
int rgw_dir_suggest_changes(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    timespan tag_timeout(std::chrono::seconds(header.tag_timeout ? header.tag_timeout : CEPH_RGW_TAG_TIMEOUT));
    ...
    while (!in_iter.end()) {
        ...
        if (ret < 0 && ret != -ENOENT) return -EINVAL;
        ...
        CLS_LOG(20,
                "cur_disk.pending_map.empty()=%d op=%d cur_disk.exists=%d "
        ...
        if (cur_disk.pending_map.empty()) {
            if (cur_disk.exists) {
                ...
                CLS_LOG(
                    10, "total_entries: %" PRId64 " -> %" PRId64 "", old_stats.num_entries, old_stats.num_entries - 1);
