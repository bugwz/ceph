...
static const std::string BI_PREFIX_END = string(1, BI_PREFIX_CHAR) + bucket_index_prefixes[BI_BUCKET_LAST_INDEX];
...
static int bi_entry_type(const string& s)
{
    ...
    for (size_t i = 1; i < sizeof(bucket_index_prefixes) / sizeof(bucket_index_prefixes[0]); ++i) {
...
static int log_index_operation(cls_method_context_t hctx, const cls_rgw_obj_key& obj_key, RGWModifyOp op,
                               const string& tag, real_time timestamp, const rgw_bucket_entry_ver& ver,
                               RGWPendingState state, uint64_t index_ver, string& max_marker, uint16_t bilog_flags,
                               string* owner, string* owner_display_name, rgw_zone_set* zones_trace)
{
    bufferlist bl;
    ...
    if (entry.id > max_marker) max_marker = entry.id;
 ...
 * namespace".
 */
static int get_obj_vals(cls_method_context_t hctx, const std::string& start, const std::string& filter_prefix,
                        int num_entries, std::map<std::string, bufferlist>* pkeys, bool* pmore)
{
    ...
    auto comp = [](const pair<std::string, bufferlist>& l, const std::string& r) { return l.first < r; };
    ...
    pkeys->insert(std::make_move_iterator(new_keys.begin()), std::make_move_iterator(new_keys.end()));
...
template<class T> static int read_index_entry(cls_method_context_t hctx, string& name, T* entry);
...
static int decode_list_index_key(const string& index_key, cls_rgw_obj_key* key, uint64_t* ver)
{
    ...
    *ver = 0;
    ...
    if (vals.empty()) {
        CLS_LOG(0,
                "ERROR: %s: bad index_key (%s): split_key() returned empty vals",
                __func__,
...
static int read_bucket_header(cls_method_context_t hctx, rgw_bucket_dir_header* header)
{
    bufferlist bl;
...
int rgw_bucket_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (has_delimiter && start_after_omap_key > op.filter_prefix &&
    ...
    for (int attempt = 0; attempt < max_attempts && more && !done && name_entry_map.size() < op.num_entries;
         ++attempt) {
        ...
        rc = get_obj_vals(
            hctx, start_after_omap_key, op.filter_prefix, op.num_entries - name_entry_map.size(), &keys, &more);
        if (rc < 0) {
            return rc;
        }
        CLS_LOG(
            20, "%s: on attempt %d get_obj_vls returned %ld entries, more=%d", __func__, attempt, keys.size(), more);
        ...
        for (auto kiter = keys.cbegin(); kiter != keys.cend(); ++kiter) {
            ...
            try {
                ...
                CLS_LOG(1, "ERROR: %s: failed to decode entry, key=%s", __func__, kiter->first.c_str());
                return -EINVAL;
            }
            ...
            if (ret < 0) {
                CLS_LOG(
                    0, "ERROR: %s: failed to decode list index key (%s)", __func__, escape_str(kiter->first).c_str());
                continue;
            }

            if (!entry.is_valid()) {
                CLS_LOG(20, "%s: entry %s[%s] is not valid", __func__, key.name.c_str(), key.instance.c_str());
                continue;
            }
            ...
            if (!op.list_versions && (!entry.is_visible() || op.start_obj.name == key.name)) {
                CLS_LOG(20, "%s: entry %s[%s] is not visible", __func__, key.name.c_str(), key.instance.c_str());
                continue;
            }
            ...
            if (has_delimiter) {
                ...
                if (delim_pos >= 0) {
                    ...
                    string prefix_key = key.name.substr(0, delim_pos + op.delimiter.length());
                    ...
                    if (name_entry_map.size() < op.num_entries) {
                        ...
                        CLS_LOG(20,
                                "%s: got common prefix entry %s[%s] num entries=%lu",
                                __func__,
                                proxy_key.name.c_str(),
                                proxy_key.instance.c_str(),
                                name_entry_map.size());
                    }
            ...
            if (name_entry_map.size() < op.num_entries && kiter->first != prev_omap_key) {
                ...
                CLS_LOG(20,
                        "%s: got object entry %s[%s] num entries=%d",
                        __func__,
                        key.name.c_str(),
                        key.instance.c_str(),
...
static int read_key_entry(cls_method_context_t hctx, const cls_rgw_obj_key& key, string* idx,
                          rgw_bucket_dir_entry* entry, bool special_delete_marker_name = false);
...
int rgw_bucket_prepare_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    const bool bitx_inst = conf->rgw_bucket_index_transaction_instrumentation;

    CLS_LOG_BITX(bitx_inst,
                 10,
                 "ENTERING %s for object oid=%s key=%s",
                 __func__,
                 oi.soid.oid.name.c_str(),
                 oi.soid.get_key().c_str());
    ...
    CLS_LOG_BITX(bitx_inst,
                 1,
                 "INFO: %s: request: op=%s name=%s tag=%s",
                 __func__,
                 modify_op_str(op.op).c_str(),
                 op.key.to_string().c_str(),
                 op.tag.c_str());
    ...
    CLS_LOG_BITX(bitx_inst, 20, "INFO: %s: setting map entry at key=%s", __func__, escape_str(idx).c_str());
...
static void unaccount_entry(rgw_bucket_dir_header& header, rgw_bucket_dir_entry& entry)
{
    if (entry.exists) {
...
{
    CLS_LOG(1,
            "%s: %s: ver=%ld:%llu name=%s instance=%s locator=%s",
            func,
            str,
            (long)entry->ver.pool,
            (unsigned long long)entry->ver.epoch,
            entry->key.name.c_str(),
            entry->key.instance.c_str(),
            entry->locator.c_str());
}
...
{
    CLS_LOG(1,
            "%s: %s: epoch=%llu name=%s instance=%s tag=%s",
            func,
            str,
...
template<class T> static int read_omap_entry(cls_method_context_t hctx, const std::string& name, T* entry)
{
...
static int read_key_entry(cls_method_context_t hctx, const cls_rgw_obj_key& key, string* idx,
                          rgw_bucket_dir_entry* entry, bool special_delete_marker_name)
{
    ...
    if (key.instance.empty() && entry->flags & rgw_bucket_dir_entry::FLAG_VER_MARKER) {
...
static int complete_remove_obj(cls_method_context_t hctx, rgw_bucket_dir_header& header, const cls_rgw_obj_key& key,
                               bool log_op)
{
    rgw_bucket_dir_entry entry;
    string idx;
    ...
    if (ret < 0) {
        CLS_LOG(1,
                "%s: read_key_entry name=%s instance=%s failed with %d",
                __func__,
                key.name.c_str(),
                key.instance.c_str(),
                ret);
    ...
    CLS_LOG(10,
            "%s: read entry name=%s instance=%s category=%d",
            __func__,
            entry.key.name.c_str(),
            entry.key.instance.c_str(),
...
int rgw_bucket_complete_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG_BITX(bitx_inst,
                 ...
                 modify_op_str(op.op).c_str(),
                 op.key.to_string().c_str(),
                 (unsigned long)op.ver.pool,
                 (unsigned long long)op.ver.epoch,
                 op.tag.c_str());

    rgw_bucket_dir_header header;
    int rc = read_bucket_header(hctx, &header);
    if (rc < 0) {
        CLS_LOG_BITX(bitx_inst, 1, "ERROR: %s: failed to read header, rc=%d", __func__, rc);
        return -EINVAL;
    }
    ...
    if (rc == -ENOENT) {
        ...
        CLS_LOG_BITX(
            bitx_inst, 1, "ERROR: %s: read key entry failed, key=%s, rc=%d", __func__, op.key.to_string().c_str(), rc);
        return rc;
    }
    ...
    if (op.tag.size()) {
        ...
        CLS_LOG_BITX(bitx_inst, 20, "INFO: %s: removing tag %s from pending map", __func__, op.tag.c_str());
    ...
    else if (op.ver.pool == entry.ver.pool && op.ver.epoch && op.ver.epoch <= entry.ver.epoch) {
        CLS_LOG_BITX(bitx_inst, 20, "INFO: %s: skipping request, old epoch", __func__);
    ...
    if (op.op == CLS_RGW_OP_CANCEL) {
        ...
        if (op.tag.size()) {
            if (!entry.exists && entry.pending_map.empty()) {
                ...
                CLS_LOG_BITX(
                    bitx_inst, 20, "INFO: %s: removing map entry with key=%s", __func__, escape_str(idx).c_str());
        ...
        CLS_LOG_BITX(bitx_inst, 20, "INFO: %s: delete op, key=%s", __func__, escape_str(idx).c_str());
        entry.meta = op.meta;
        if (!ondisk) {
            // no entry to erase
            CLS_LOG_BITX(bitx_inst, 20, "INFO: %s: key=%s not on disk, no action", __func__, escape_str(idx).c_str());
            log_op = false;
        }
        else if (!entry.pending_map.size()) {
            CLS_LOG_BITX(bitx_inst, 20, "INFO: %s: removing map entry with key=%s", __func__, escape_str(idx).c_str());
            rc = cls_cxx_map_remove_key(hctx, idx);
            if (rc < 0) {
                CLS_LOG_BITX(bitx_inst,
                             1,
        ...
        CLS_LOG_BITX(bitx_inst, 20, "INFO: %s: add op, key=%s", __func__, escape_str(idx).c_str());
    ...
    if (log_op) {
        rc = log_index_operation(hctx,
                                 ...
                                 &op.zones_trace);
        if (rc < 0) {
            CLS_LOG_BITX(bitx_inst, 0, "ERROR: %s: log_index_operation failed with rc=%d", __func__, rc);
            return rc;
        }
    }

    CLS_LOG_BITX(bitx_inst, 20, "INFO: %s: remove_objs.size()=%d", __func__, (int)op.remove_objs.size());
    for (const auto& remove_key : op.remove_objs) {
        CLS_LOG_BITX(bitx_inst,
                     20,
                     "INFO: %s: completing object remove key=%s",
                     __func__,
                     escape_str(remove_key.to_string()).c_str());
    ...
    CLS_LOG_BITX(bitx_inst, 20, "INFO: %s: writing bucket header", __func__);
    rc = write_bucket_header(hctx, &header);
    if (rc < 0) {
        CLS_LOG_BITX(bitx_inst, 0, "ERROR: %s: failed to write bucket header ret=%d", __func__, rc);
    }

    CLS_LOG_BITX(bitx_inst, 10, "EXITING %s: returning %d", __func__, rc);
    return rc;
}   // rgw_bucket_complete_op

template<class T> static int write_entry(cls_method_context_t hctx, T& entry, const string& key)
{
    bufferlist bl;
    encode(entry, bl);
    return cls_cxx_map_set_val(hctx, key, &bl);
...
static int read_olh(cls_method_context_t hctx, cls_rgw_obj_key& obj_key, rgw_bucket_olh_entry* olh_data_entry,
                    string* index_key, bool* found)
{
...
static int write_obj_instance_entry(cls_method_context_t hctx, rgw_bucket_dir_entry& instance_entry,
                                    const string& instance_idx)
{
    CLS_LOG(20,
            "write_entry() instance=%s idx=%s flags=%d",
            escape_str(instance_entry.key.instance).c_str(),
            instance_idx.c_str(),
                             ...
                             const string& instance_idx)
{
    ...
    if (instance_idx != instance_list_idx) {
        ...
        if (ret < 0) {
            CLS_LOG(0,
                    "ERROR: write_entry() instance=%s instance_list_idx=%s ret=%d",
                    instance_entry.key.instance.c_str(),
                    instance_list_idx.c_str(),
                    ret);
            return ret;
        }
    }
...
public:
    ...
    int init(bool check_delete_marker = true)
    {
        ...
        initialized = true;
        CLS_LOG(20,
                "read instance_entry key.name=%s key.instance=%s flags=%d",
                instance_entry.key.name.c_str(),
                instance_entry.key.instance.c_str(),
                instance_entry.flags);
        return 0;
    }

    rgw_bucket_dir_entry& get_dir_entry() { return instance_entry; }
    ...
    void set_epoch(uint64_t epoch) { instance_entry.versioned_epoch = epoch; }
    ...
    int write(uint64_t epoch, bool current)
    {
        if (instance_entry.versioned_epoch > 0) {
            CLS_LOG(20,
                    "%s: instance_entry.versioned_epoch=%d epoch=%d",
                    __func__,
                    (int)instance_entry.versioned_epoch,
                    (int)epoch);
            /* this instance has a previous list entry, remove that entry */
    ...
    int demote_current() { return write_entries(0, rgw_bucket_dir_entry::FLAG_CURRENT); }

    bool is_delete_marker() { return instance_entry.is_delete_marker(); }
    ...
    real_time mtime() { return instance_entry.meta.mtime; }
};   // class BIVerObjEntry
...
public:
    BIOLHEntry(cls_method_context_t& _hctx, const cls_rgw_obj_key& _key)
        : hctx(_hctx)
        , key(_key)
        , initialized(false)
    {}
    ...
    uint64_t get_epoch() { return olh_data_entry.epoch; }

    rgw_bucket_olh_entry& get_entry() { return olh_data_entry; }
    ...
    void set_exists(bool exists) { olh_data_entry.exists = exists; }

    bool pending_removal() { return olh_data_entry.pending_removal; }

    void set_pending_removal(bool pending_removal) { olh_data_entry.pending_removal = pending_removal; }

    const string& get_tag() { return olh_data_entry.tag; }
    void set_tag(const string& tag) { olh_data_entry.tag = tag; }
};
 ...
 */
static int convert_plain_entry_to_versioned(cls_method_context_t hctx, cls_rgw_obj_key& key, bool demote_current,
                                            bool instance_only)
{
    if (!key.instance.empty()) {
        return -EINVAL;
...
static int rgw_bucket_link_olh(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (!existed && op.delete_marker) {
        ...
        if (olh_found && olh.get_entry().delete_marker) {
            CLS_LOG(10,
                    ...
                    __func__,
                    escape_str(op.key.to_string()).c_str());
            return -ENOENT;
        }
    }
    ...
    const bool promote = (olh.get_epoch() > prev_epoch) ||
                         (olh.get_epoch() == prev_epoch && olh.get_entry().key.instance >= op.key.instance);
...
static int rgw_bucket_unlink_instance(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (olh_key == dest_key) {
        ...
        if (found) {
            ...
            CLS_LOG(20,
                    "%s: updating olh log: link olh -> %s[%s] (is_delete=%d)",
                    __func__,
                    next_key.name.c_str(),
                    next_key.instance.c_str(),
...
static int rgw_bucket_read_olh_log(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (olh_data_entry.tag != op.olh_tag) {
        CLS_LOG(1,
                "NOTICE: %s: olh_tag_mismatch olh_data_entry.tag=%s op.olh_tag=%s",
                __func__,
                olh_data_entry.tag.c_str(),
                op.olh_tag.c_str());
        return -ECANCELED;
    }
...
int rgw_dir_suggest_changes(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    const uint64_t config_op_expiration = conf->rgw_pending_bucket_index_op_expiration;

    // priority order -- 1) bucket header, 2) global config, 3) DEFAULT;
    // a value of zero indicates go down the list
    timespan tag_timeout(std::chrono::seconds(
        header.tag_timeout ? header.tag_timeout
                           : (config_op_expiration ? config_op_expiration : CEPH_RGW_DEFAULT_TAG_TIMEOUT)));
    ...
    while (!in_iter.end()) {
        ...
        CLS_LOG_BITX(bitx_inst,
                     ...
                     cur_change.exists);
        CLS_LOG_BITX(
            bitx_inst, 20, "INFO: %s: setting map entry at key=%s", __func__, escape_str(cur_change_key).c_str());
        int ret = cls_cxx_map_get_val(hctx, cur_change_key, &cur_disk_bl);
        if (ret < 0 && ret != -ENOENT) {
            CLS_LOG_BITX(bitx_inst,
                         20,
                         "ERROR: %s: accessing map, key=%s error=%d",
                         __func__,
                         escape_str(cur_change_key).c_str(),
                         ret);
            return -EINVAL;
        ...
        if (cur_disk_bl.length()) {
            ...
            try {
                ...
                CLS_LOG_BITX(bitx_inst, 1, "ERROR: %s: failed to decode cur_disk", __func__);
                return -EINVAL;
            }
        ...
        CLS_LOG_BITX(bitx_inst,
                     ...
                     __func__,
                     op,
                     cur_disk.pending_map.empty(),
                     cur_disk.exists,
                     (int)cur_disk.index_ver,
                     cur_change.exists,
        ...
        if (cur_disk.pending_map.empty()) {
            ...
            if (cur_disk.exists) {
                ...
                CLS_LOG_BITX(bitx_inst,
                             10,
                             "INFO: %s: stats.num_entries: %ld -> %ld",
                             __func__,
                             old_stats.num_entries,
                             old_stats.num_entries - 1);
                old_stats.num_entries--;
            ...
            case CEPH_RGW_REMOVE:
                CLS_LOG_BITX(bitx_inst,
                             ...
                             __func__,
                             escape_str(cur_change.key.to_string()).c_str(),
                             escape_str(cur_change_key).c_str());
                ...
                if (ret < 0) {
                    CLS_LOG_BITX(bitx_inst,
                                 0,
                                 "ERROR: %s: unable to remove key, key=%s, error=%d",
                                 __func__,
                                 escape_str(cur_change_key).c_str(),
                                 ret);
                    return ret;
            ...
            case CEPH_RGW_UPDATE:
                ...
                if (ret < 0) {
                    CLS_LOG_BITX(bitx_inst,
                                 0,
                                 "ERROR: %s: unable to set value for key, key=%s, error=%d",
                                 __func__,
                                 escape_str(cur_change_key).c_str(),
                                 ret);
                    return ret;
    ...
    if (header_changed) {
        ...
        if (ret < 0) {
            CLS_LOG_BITX(bitx_inst, 0, "ERROR: %s: failed to write bucket header ret=%d", __func__, ret);
        }
        else {
...
static int rgw_obj_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    for (auto iter = op.keep_attr_prefixes.begin(); iter != op.keep_attr_prefixes.end(); ++iter) {
        auto& check_prefix = *iter;

        for (auto aiter = attrset.lower_bound(check_prefix); aiter != attrset.end(); ++aiter) {
    ...
    for (auto aiter = new_attrs.begin(); aiter != new_attrs.end(); ++aiter) {
...
static int rgw_obj_check_attrs_prefix(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    for (auto aiter = attrset.lower_bound(op.check_prefix); aiter != attrset.end(); ++aiter) {
        const auto& attr = aiter->first;
...
static int rgw_obj_check_mtime(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(10,
            "%s: obj_ut=%lld.%06lld op.mtime=%lld.%06lld",
            __func__,
            (long long)obj_ts.tv_sec,
            (long long)obj_ts.tv_nsec,
            (long long)op_ts.tv_sec,
            (long long)op_ts.tv_nsec);

    bool check;

    switch (op.type) {
    case CLS_RGW_CHECK_TIME_MTIME_EQ: check = (obj_ts == op_ts); break;
    case CLS_RGW_CHECK_TIME_MTIME_LT: check = (obj_ts < op_ts); break;
    case CLS_RGW_CHECK_TIME_MTIME_LE: check = (obj_ts <= op_ts); break;
    case CLS_RGW_CHECK_TIME_MTIME_GT: check = (obj_ts > op_ts); break;
    case CLS_RGW_CHECK_TIME_MTIME_GE: check = (obj_ts >= op_ts); break;
...
static int rgw_bi_get_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    switch (op.type) {
    case BIIndexType::Plain: idx = op.key.name; break;
    case BIIndexType::Instance: encode_obj_index_key(op.key, &idx); break;
    case BIIndexType::OLH: encode_olh_data_key(op.key, &idx); break;
    default: CLS_LOG(10, "%s: invalid key type encoding: %d", __func__, int(op.type)); return -EINVAL;
    }
...
static int list_plain_entries_help(cls_method_context_t hctx, const std::string& name_filter,
                                   const std::string& start_after_key,   // exclusive
                                   const std::string& end_key,           // exclusive
                                   uint32_t max, std::list<rgw_cls_bi_entry>* entries, bool& end_key_reached,
                                   bool& more)
{
    CLS_LOG(10,
            "Entered %s: name_filter=\"%s\", start_after_key=\"%s\", end_key=\"%s\", max=%d",
            __func__,
            escape_str(name_filter).c_str(),
            escape_str(start_after_key).c_str(),
            escape_str(end_key).c_str(),
            max);
    int count = 0;
    std::map<std::string, bufferlist> raw_entries;
    int ret = cls_cxx_map_get_vals(hctx, start_after_key, name_filter, max, &raw_entries, &more);
    CLS_LOG(20,
            "%s: cls_cxx_map_get_vals ret=%d, raw_entries.size()=%lu, more=%d",
            __func__,
            ret,
            raw_entries.size(),
            more);
    ...
    for (auto iter : raw_entries) {
        if (!end_key.empty() && iter.first >= end_key) {
            CLS_LOG(20, "%s: end key reached at \"%s\"", __func__, escape_str(iter.first).c_str());
        ...
        try {
            ...
            CLS_LOG(0,
                    "ERROR: %s: failed to decode buffer for plain bucket index entry \"%s\"",
                    __func__,
        ...
        if (!name_filter.empty() && e.key.name > name_filter) {
            CLS_LOG(20,
                    "%s: due to filter \"%s\", skipping entry.idx=\"%s\" e.key.name=\"%s\"",
                    __func__,
                    escape_str(name_filter).c_str(),
                    escape_str(iter.first).c_str(),
        ...
        CLS_LOG(20,
                "%s: adding entry %d entry.idx=\"%s\" e.key.name=\"%s\"",
                __func__,
                count,
 ...
 * appended to it.
 */
static int list_plain_entries(cls_method_context_t hctx, const std::string& name_filter, const std::string& marker,
                              uint32_t max, std::list<rgw_cls_bi_entry>* entries, bool* pmore,
                              const PlainEntriesRegion region = PlainEntriesRegion::Both)
{
    CLS_LOG(10,
            "entered %s: name_filter=\"%s\", marker=\"%s\", max=%d, region=%d",
            __func__,
            escape_str(name_filter).c_str(),
            escape_str(marker).c_str(),
            max,
    ...
    if (region <= PlainEntriesRegion::Both && marker < BI_PREFIX_BEGIN) {
        ...
        int r =
            list_plain_entries_help(hctx, name_filter, marker, BI_PREFIX_BEGIN, max, entries, end_key_reached, more);
        CLS_LOG(20,
                "%s: first list_plain_entries_help r=%d, end_key_reached=%d, more=%d",
                __func__,
                r,
                end_key_reached,
                more);
    ...
    if (region >= PlainEntriesRegion::Both) {
        ...
        r = list_plain_entries_help(hctx, name_filter, start_after_key, {}, max, entries, end_key_reached, more);
        CLS_LOG(20,
                "%s: second list_plain_entries_help r=%d, end_key_reached=%d, more=%d",
                __func__,
                r,
                end_key_reached,
                more);
                                 ...
                                 list<rgw_cls_bi_entry>* entries, bool* pmore)
{
    ...
    if (max > 0) {
        ret = cls_cxx_map_get_vals(hctx, start_after_key, string(), max, &keys, pmore);
        CLS_LOG(20,
                "%s: start_after_key=\"%s\" first_instance_idx=\"%s\" keys.size()=%d",
                __func__,
                escape_str(start_after_key).c_str(),
                escape_str(first_instance_idx).c_str(),
                (int)keys.size());
        if (ret < 0) {
            return ret;
        }
...
static int check_index(cls_method_context_t hctx, rgw_bucket_dir_header* existing_header,
...
static int rgw_bi_list_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(20,
            "%s: op.marker=\"%s\", op.name_filter=\"%s\", op.max=%u max=%u",
            __func__,
            escape_str(op.marker).c_str(),
    ...
    ret = list_plain_entries(hctx, op.name_filter, op.marker, max, &op_ret.entries, &more, PlainEntriesRegion::Low);
    if (ret < 0) {
        CLS_LOG(0,
                "ERROR: %s: list_plain_entries (low) returned ret=%d, marker=\"%s\", filter=\"%s\", max=%d",
                __func__,
                ret,
                escape_str(op.marker).c_str(),
                escape_str(op.name_filter).c_str(),
    ...
    if (!more) {
        ret = list_plain_entries(
            hctx, op.name_filter, op.marker, max - count, &op_ret.entries, &more, PlainEntriesRegion::High);
        if (ret < 0) {
            CLS_LOG(0,
                    "ERROR: %s: list_plain_entries (high) returned ret=%d, marker=\"%s\", filter=\"%s\", max=%d",
                    __func__,
                    ret,
                    escape_str(op.marker).c_str(),
                    escape_str(op.name_filter).c_str(),
...
static int bi_log_iterate_entries(cls_method_context_t hctx, const string& marker, const string& end_marker,
                                  ...
                                  int (*cb)(cls_method_context_t, const string&, rgw_bi_log_entry&, void*), void* param)
{
    ...
    CLS_LOG(10, "bi_log_iterate_entries start_after_key=%s end_key=%s", start_after_key.c_str(), end_key.c_str());

    string filter;

    int ret = cls_cxx_map_get_vals(hctx, start_after_key, filter, max_entries, &keys, truncated);
    if (ret < 0) return ret;
...
static int bi_log_list_entries(cls_method_context_t hctx, const string& marker, uint32_t max,
                               list<rgw_bi_log_entry>& entries, bool* truncated)
{
    string key_iter;
    string end_marker;
    int ret = bi_log_iterate_entries(hctx, marker, end_marker, key_iter, max, truncated, bi_log_list_cb, &entries);
    return ret;
}
...
static int rgw_bi_log_trim(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    CLS_LOG(20, "listed key %s, removing through %s", first_key.c_str(), key_end.c_str());
...
static int rgw_bi_log_resync(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (entry.id > header.max_marker) header.max_marker = entry.id;
...
static int usage_iterate_range(cls_method_context_t hctx, uint64_t start, uint64_t end, const string& user,
                               ...
                               int (*cb)(cls_method_context_t, const string&, rgw_usage_log_entry&, void*), void* param)
{
    CLS_LOG(10, "entered %s", __func__);
    ...
    *truncated = truncated_status;
    ...
    for (; iter != keys.end(); ++iter) {
        ...
        if (!bucket.empty() && bucket.compare(e.bucket)) continue;
...
#define MAX_ENTRIES 1000
    ...
    int ret = usage_iterate_range(hctx,
                                  op.start_epoch,
                                  op.end_epoch,
                                  op.owner,
                                  op.bucket,
    ...
    if (ret_info.truncated) ret_info.next_iter = iter;
...
static string gc_index_prefixes[] = {"0_", "1_"};
...
done_err:
    ...
    CLS_LOG(0, "ERROR: gc_set_entry error info.tag=%s, ret=%d", info.tag.c_str(), ret);
...
static int gc_iterate_entries(cls_method_context_t hctx, const string& marker, bool expired_only, string& out_marker,
                              uint32_t max_entries, bool* truncated,
                              int (*cb)(cls_method_context_t, const string&, cls_rgw_gc_obj_info&, void*), void* param)
{
...
static int gc_list_entries(cls_method_context_t hctx, const string& marker, uint32_t max, bool expired_only,
                           list<cls_rgw_gc_obj_info>& entries, bool* truncated, string& next_marker)
{
    int ret = gc_iterate_entries(hctx, marker, expired_only, next_marker, max, truncated, gc_list_cb, &entries);
    return ret;
}
...
#define GC_LIST_ENTRIES_DEFAULT 128
    int ret = gc_list_entries(hctx,
                              op.marker,
                              (op.max ? op.max : GC_LIST_ENTRIES_DEFAULT),
                              op.expired_only,
                              op_ret.entries,
                              &op_ret.truncated,
                              op_ret.next_marker);
...
static int rgw_mp_upload_part_info_update(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (op.info.past_prefixes.contains(op.info.manifest.get_prefix())) {
        ...
        const object_info_t& oi = cls_get_object_info(hctx);
        CLS_LOG(1,
                "ERROR: oid [%s]: Current prefix %s is also a past prefix for part %s",
                oi.soid.oid.name.c_str(),
    ...
    CLS_LOG(10,
            "part info update on key [%s]: %zu past prefixes, ret %d",
            op.part_key.c_str(),
            op.info.past_prefixes.size(),
            ret);
    return ret;
}
...
static int rgw_reshard_remove(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (!op.bucket_id.empty() && entry.bucket_id != op.bucket_id) {
        return 0;
    }
...
CLS_INIT(rgw)
{
    ...
    cls_register_cxx_method(
        h_class, RGW_BUCKET_INIT_INDEX, CLS_METHOD_RD | CLS_METHOD_WR, rgw_bucket_init_index, &h_rgw_bucket_init_index);
    ...
    cls_register_cxx_method(
        h_class, RGW_BUCKET_CHECK_INDEX, CLS_METHOD_RD, rgw_bucket_check_index, &h_rgw_bucket_check_index);
    cls_register_cxx_method(h_class,
    ...
    cls_register_cxx_method(
        h_class, RGW_BUCKET_PREPARE_OP, CLS_METHOD_RD | CLS_METHOD_WR, rgw_bucket_prepare_op, &h_rgw_bucket_prepare_op);
    cls_register_cxx_method(h_class,
                            RGW_BUCKET_COMPLETE_OP,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_bucket_complete_op,
    ...
    cls_register_cxx_method(
        h_class, RGW_BUCKET_LINK_OLH, CLS_METHOD_RD | CLS_METHOD_WR, rgw_bucket_link_olh, &h_rgw_bucket_link_olh);
    cls_register_cxx_method(h_class,
    ...
    cls_register_cxx_method(
        h_class, RGW_BUCKET_READ_OLH_LOG, CLS_METHOD_RD, rgw_bucket_read_olh_log, &h_rgw_bucket_read_olh_log);
    cls_register_cxx_method(h_class,
    ...
    cls_register_cxx_method(
        h_class, RGW_BUCKET_CLEAR_OLH, CLS_METHOD_RD | CLS_METHOD_WR, rgw_bucket_clear_olh, &h_rgw_bucket_clear_olh);

    cls_register_cxx_method(h_class, RGW_OBJ_REMOVE, CLS_METHOD_RD | CLS_METHOD_WR, rgw_obj_remove, &h_rgw_obj_remove);
    cls_register_cxx_method(
        h_class, RGW_OBJ_STORE_PG_VER, CLS_METHOD_WR, rgw_obj_store_pg_ver, &h_rgw_obj_store_pg_ver);
    cls_register_cxx_method(
        h_class, RGW_OBJ_CHECK_ATTRS_PREFIX, CLS_METHOD_RD, rgw_obj_check_attrs_prefix, &h_rgw_obj_check_attrs_prefix);
    ...
    cls_register_cxx_method(
        h_class, RGW_BI_LOG_TRIM, CLS_METHOD_RD | CLS_METHOD_WR, rgw_bi_log_trim, &h_rgw_bi_log_list_op);
    cls_register_cxx_method(h_class,
                            RGW_DIR_SUGGEST_CHANGES,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_dir_suggest_changes,
    ...
    cls_register_cxx_method(
        h_class, RGW_BI_LOG_RESYNC, CLS_METHOD_RD | CLS_METHOD_WR, rgw_bi_log_resync, &h_rgw_bi_log_resync_op);
    cls_register_cxx_method(
        h_class, RGW_BI_LOG_STOP, CLS_METHOD_RD | CLS_METHOD_WR, rgw_bi_log_stop, &h_rgw_bi_log_stop_op);
    ...
    cls_register_cxx_method(
        h_class, RGW_USER_USAGE_LOG_READ, CLS_METHOD_RD, rgw_user_usage_log_read, &h_rgw_user_usage_log_read);
    cls_register_cxx_method(h_class,
    ...
    /* garbage collection */
    cls_register_cxx_method(
        h_class, RGW_GC_SET_ENTRY, CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_gc_set_entry, &h_rgw_gc_set_entry);
    cls_register_cxx_method(
        h_class, RGW_GC_DEFER_ENTRY, CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_gc_defer_entry, &h_rgw_gc_set_entry);
    ...
    cls_register_cxx_method(
        h_class, RGW_LC_SET_ENTRY, CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_lc_set_entry, &h_rgw_lc_set_entry);
    cls_register_cxx_method(
        h_class, RGW_LC_RM_ENTRY, CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_lc_rm_entry, &h_rgw_lc_rm_entry);
    cls_register_cxx_method(
        h_class, RGW_LC_GET_NEXT_ENTRY, CLS_METHOD_RD, rgw_cls_lc_get_next_entry, &h_rgw_lc_get_next_entry);
    cls_register_cxx_method(
        h_class, RGW_LC_PUT_HEAD, CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_lc_put_head, &h_rgw_lc_put_head);
    cls_register_cxx_method(h_class, RGW_LC_GET_HEAD, CLS_METHOD_RD, rgw_cls_lc_get_head, &h_rgw_lc_get_head);
    cls_register_cxx_method(
        h_class, RGW_LC_LIST_ENTRIES, CLS_METHOD_RD, rgw_cls_lc_list_entries, &h_rgw_lc_list_entries);

    /* multipart */
    cls_register_cxx_method(h_class,
                            RGW_MP_UPLOAD_PART_INFO_UPDATE,
                            CLS_METHOD_RD | CLS_METHOD_WR,
                            rgw_mp_upload_part_info_update,
                            &h_rgw_mp_upload_part_info_update);

    /* resharding */
    cls_register_cxx_method(
        h_class, RGW_RESHARD_ADD, CLS_METHOD_RD | CLS_METHOD_WR, rgw_reshard_add, &h_rgw_reshard_add);
    ...
    cls_register_cxx_method(
        h_class, RGW_RESHARD_REMOVE, CLS_METHOD_RD | CLS_METHOD_WR, rgw_reshard_remove, &h_rgw_reshard_remove);
    ...
    cls_register_cxx_method(
        h_class, RGW_GET_BUCKET_RESHARDING, CLS_METHOD_RD, rgw_get_bucket_resharding, &h_rgw_get_bucket_resharding);
