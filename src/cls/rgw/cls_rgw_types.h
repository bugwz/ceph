...
struct rgw_zone_set_entry
{
    ...
    bool operator==(const rgw_zone_set_entry& e) const { return zone == e.zone && location_key == e.location_key; }

    rgw_zone_set_entry() {}
    rgw_zone_set_entry(const std::string& _zone, std::optional<std::string> _location_key)
        : zone(_zone)
        , location_key(_location_key)
    {}
    rgw_zone_set_entry(const std::string& s) { from_str(s); }
...
struct rgw_bucket_dir_entry_meta
{
    ...
    void decode(ceph::buffer::list::const_iterator& bl)
    {
        ...
        if (struct_v >= 2) decode(content_type, bl);
        if (struct_v >= 4)
            decode(accounted_size, bl);
        else
            accounted_size = size;
        if (struct_v >= 5) decode(user_data, bl);
        if (struct_v >= 6) decode(storage_class, bl);
        if (struct_v >= 7) decode(appendable, bl);
        DECODE_FINISH(bl);
    }
    void dump(ceph::Formatter* f) const;
    void decode_json(JSONObj* obj);
...
template<class T> void encode_packed_val(T val, ceph::buffer::list& bl)
{
    using ceph::encode;
...
template<class T> void decode_packed_val(T& val, ceph::buffer::list::const_iterator& bl)
{
    using ceph::decode;
    unsigned char c;
    decode(c, bl);
    ...
    default: throw ceph::buffer::malformed_input();
    }
}
...
struct rgw_bucket_dir_entry
{
    ...
    {
        int test_flags = rgw_bucket_dir_entry::FLAG_VER | rgw_bucket_dir_entry::FLAG_CURRENT;
        return (flags & rgw_bucket_dir_entry::FLAG_VER) == 0 || (flags & test_flags) == test_flags;
    }
    bool is_delete_marker() const { return (flags & rgw_bucket_dir_entry::FLAG_DELETE_MARKER) != 0; }
    bool is_visible() const { return is_current() && !is_delete_marker(); }
    bool is_valid() const { return (flags & rgw_bucket_dir_entry::FLAG_VER_MARKER) == 0; }
    bool is_common_prefix() const { return flags & rgw_bucket_dir_entry::FLAG_COMMON_PREFIX; }
...
struct rgw_cls_bi_entry
{
    ...
    bool get_info(cls_rgw_obj_key* key, RGWObjCategory* category, rgw_bucket_category_stats* accounted_stats);
};
...
struct rgw_bi_log_entry
{
    ...
    bool is_versioned() { return ((bilog_flags & RGW_BILOG_FLAG_VERSIONED_OP) != 0); }
};
...
inline bool operator==(const rgw_bucket_category_stats& lhs, const rgw_bucket_category_stats& rhs)
{
    return lhs.total_size == rhs.total_size && lhs.total_size_rounded == rhs.total_size_rounded &&
           lhs.num_entries == rhs.num_entries && lhs.actual_size == rhs.actual_size;
}
inline bool operator!=(const rgw_bucket_category_stats& lhs, const rgw_bucket_category_stats& rhs)
{
    return !(lhs == rhs);
...
{
    switch (status) {
    case cls_rgw_reshard_status::NOT_RESHARDING: return "not-resharding";
    case cls_rgw_reshard_status::IN_PROGRESS: return "in-progress";
    case cls_rgw_reshard_status::DONE: return "done";
    };
...
struct cls_rgw_bucket_instance_entry
{
    ...
    void clear() { reshard_status = RESHARD_STATUS::NOT_RESHARDING; }

    void set_status(cls_rgw_reshard_status s) { reshard_status = s; }

    bool resharding() const { return reshard_status != RESHARD_STATUS::NOT_RESHARDING; }

    bool resharding_in_progress() const { return reshard_status == RESHARD_STATUS::IN_PROGRESS; }
...
struct rgw_bucket_dir_header
{
    ...
    bool resharding() const { return new_instance.resharding(); }
    bool resharding_in_progress() const { return new_instance.resharding_in_progress(); }
};
...
struct rgw_usage_data
{
    ...
    rgw_usage_data()
        ...
        , successful_ops(0)
    {}
    rgw_usage_data(uint64_t sent, uint64_t received)
        : bytes_sent(sent)
...
struct rgw_usage_log_entry
{
    ...
    rgw_usage_log_entry()
        : epoch(0)
    {}
    rgw_usage_log_entry(std::string& o, std::string& b)
        : owner(o)
        , bucket(b)
        , epoch(0)
    {}
    rgw_usage_log_entry(std::string& o, std::string& p, std::string& b)
        : owner(o)
        , payer(p)
        , bucket(b)
        , epoch(0)
    ...
    void aggregate(const rgw_usage_log_entry& e, std::map<std::string, bool>* categories = NULL)
    {
        if (owner.empty()) {
            owner = e.owner;
            bucket = e.bucket;
    ...
    void sum(rgw_usage_data& usage, std::map<std::string, bool>& categories) const
    {
...
struct rgw_user_bucket
{
    ...
    rgw_user_bucket() {}
    rgw_user_bucket(const std::string& u, const std::string& b)
        : user(u)
        , bucket(b)
    {}
...
struct cls_rgw_obj
{
    ...
    cls_rgw_obj() {}
    cls_rgw_obj(std::string& _p, cls_rgw_obj_key& _k)
        : pool(_p)
        , key(_k)
    {}
    ...
    size_t estimate_encoded_size() const
    {
        constexpr size_t start_overhead =
            sizeof(__u8) + sizeof(__u8) + sizeof(ceph_le32);   // version and length prefix
        constexpr size_t string_overhead = sizeof(__u32);      // strings are encoded with 32-bit length prefix
        return start_overhead + string_overhead + pool.size() + string_overhead + key.name.size() + string_overhead +
               loc.size() + key.estimate_encoded_size();
    }
};
...
struct cls_rgw_obj_chain
{
    ...
    static void generate_test_instances(std::list<cls_rgw_obj_chain*>& ls) { ls.push_back(new cls_rgw_obj_chain); }
...
struct cls_rgw_gc_obj_info
{
    ...
    size_t estimate_encoded_size() const
    {
        ...
        return start_overhead + string_overhead + tag.size() + time_overhead + chain.estimate_encoded_size();
    }
};
