...
struct rgw_cls_obj_complete_op
{
    ...
    void decode(ceph::buffer::list::const_iterator& bl)
    {
        ...
        if (struct_v >= 4 && struct_v < 7) {
            ...
            for (auto iter = old_remove_objs.begin(); iter != old_remove_objs.end(); ++iter) {
                cls_rgw_obj_key k;
                k.name = *iter;
...
struct rgw_cls_obj_check_mtime
{
    ...
    rgw_cls_obj_check_mtime()
        : type(CLS_RGW_CHECK_TIME_MTIME_EQ)
        , high_precision_time(false)
    {}
...
struct cls_rgw_gc_list_ret
{
    ...
    void decode(ceph::buffer::list::const_iterator& bl)
    {
        DECODE_START(2, bl);
        decode(entries, bl);
        if (struct_v >= 2) decode(next_marker, bl);
        decode(truncated, bl);
        DECODE_FINISH(bl);
    }
...
struct cls_rgw_lc_get_entry_op
{
    ...
    cls_rgw_lc_get_entry_op(const std::string& _marker)
        : marker(_marker)
    {}
...
struct cls_rgw_lc_list_entries_ret
{
    ...
    void encode(ceph::buffer::list& bl) const
    {
        ...
        if (compat_v <= 2) {
            ...
            std::for_each(entries.begin(), entries.end(), [&oes](const cls_rgw_lc_entry& elt) {
    ...
    void decode(ceph::buffer::list::const_iterator& bl)
    {
        ...
        if (struct_v <= 2) {
            std::map<std::string, int> oes;
            decode(oes, bl);
            std::for_each(oes.begin(), oes.end(), [this](const std::pair<std::string, int>& oe) {
                entries.push_back({oe.first, 0 /* start */, uint32_t(oe.second)});
            });
        }
        else {
            decode(entries, bl);
