...
int CLSRGWConcurrentIO::operator()()
{
    ...
    while (manager.wait_for_completions(valid_ret_code(),
                                        &num_completions,
                                        &r,
 ...
 */
template<typename T> class ClsBucketIndexOpCtx : public ObjectOperationCompletion
{
private:
    T* data;
    int* ret_code;
...
public:
    ClsBucketIndexOpCtx(T* _data, int* _ret_code)
        : data(_data)
        , ret_code(_ret_code)
    {
        ceph_assert(data);
...
bool BucketIndexAioManager::wait_for_completions(int valid_ret_code, int* num_completions, int* ret_code,
                                                 std::map<int, std::string>* completed_objs,
                                                 std::map<int, std::string>* retry_objs)
{
...
static bool issue_bucket_index_init_op(librados::IoCtx& io_ctx, const int shard_id, const string& oid,
...
static bool issue_bucket_index_clean_op(librados::IoCtx& io_ctx, const int shard_id, const string& oid,
                                        BucketIndexAioManager* manager)
{
    bufferlist in;
    librados::ObjectWriteOperation op;
...
static bool issue_bucket_set_tag_timeout_op(librados::IoCtx& io_ctx, const int shard_id, const string& oid,
                                            uint64_t timeout, BucketIndexAioManager* manager)
{
    bufferlist in;
...
void cls_rgw_bucket_prepare_op(ObjectWriteOperation& o, RGWModifyOp op, const string& tag, const cls_rgw_obj_key& key,
                               const string& locator, bool log_op, uint16_t bilog_flags,
...
void cls_rgw_bucket_complete_op(ObjectWriteOperation& o, RGWModifyOp op, const string& tag,
                                const rgw_bucket_entry_ver& ver, const cls_rgw_obj_key& key,
                                const rgw_bucket_dir_entry_meta& dir_meta, const list<cls_rgw_obj_key>* remove_objs,
                                bool log_op, uint16_t bilog_flags, const rgw_zone_set* zones_trace)
{
    ...
    call.log_op = log_op;
    call.bilog_flags = bilog_flags;
    if (remove_objs) call.remove_objs = *remove_objs;
    if (zones_trace) {
        call.zones_trace = *zones_trace;
    }
    encode(call, in);
...
void cls_rgw_bucket_list_op(librados::ObjectReadOperation& op, const cls_rgw_obj_key& start_obj,
                            const std::string& filter_prefix, const std::string& delimiter, uint32_t num_entries,
                            bool list_versions, rgw_cls_list_ret* result)
{
    bufferlist in;
...
static bool issue_bucket_list_op(librados::IoCtx& io_ctx, const int shard_id, const std::string& oid,
                                 const cls_rgw_obj_key& start_obj, const std::string& filter_prefix,
                                 const std::string& delimiter, uint32_t num_entries, bool list_versions,
                                 BucketIndexAioManager* manager, rgw_cls_list_ret* pdata)
{
    librados::ObjectReadOperation op;
    cls_rgw_bucket_list_op(op, start_obj, filter_prefix, delimiter, num_entries, list_versions, pdata);
    return manager->aio_operate(io_ctx, shard_id, oid, &op);
}
...
int CLSRGWIssueBucketList::issue_op(const int shard_id, const string& oid)
{
    ...
    return issue_bucket_list_op(io_ctx,
                                shard_id,
                                oid,
                                marker,
                                filter_prefix,
...
void cls_rgw_obj_check_mtime(librados::ObjectOperation& o, const real_time& mtime, bool high_precision_time,
...
int cls_rgw_bi_get(librados::IoCtx& io_ctx, const string oid, BIIndexType index_type, const cls_rgw_obj_key& key,
 ...
 */
int cls_rgw_bi_list(librados::IoCtx& io_ctx, const std::string& oid, const std::string& name_filter,
...
int cls_rgw_bucket_link_olh(librados::IoCtx& io_ctx, const string& oid, const cls_rgw_obj_key& key,
                            const bufferlist& olh_tag, bool delete_marker, const string& op_tag,
                            const rgw_bucket_dir_entry_meta* meta, uint64_t olh_epoch, ceph::real_time unmod_since,
                            bool high_precision_time, bool log_op, const rgw_zone_set& zones_trace)
{
    librados::ObjectWriteOperation op;
...
void cls_rgw_bucket_link_olh(librados::ObjectWriteOperation& op, const cls_rgw_obj_key& key, const bufferlist& olh_tag,
...
int cls_rgw_bucket_unlink_instance(librados::IoCtx& io_ctx, const string& oid, const cls_rgw_obj_key& key,
                                   const string& op_tag, const string& olh_tag, uint64_t olh_epoch, bool log_op,
                                   const rgw_zone_set& zones_trace)
{
    librados::ObjectWriteOperation op;
...
void cls_rgw_bucket_unlink_instance(librados::ObjectWriteOperation& op, const cls_rgw_obj_key& key,
                                    const string& op_tag, const string& olh_tag, uint64_t olh_epoch, bool log_op,
                                    const rgw_zone_set& zones_trace)
{
    bufferlist in, out;
...
void cls_rgw_get_olh_log(librados::ObjectReadOperation& op, const cls_rgw_obj_key& olh, uint64_t ver_marker,
                         const string& olh_tag, rgw_cls_read_olh_log_ret& log_ret, int& op_ret)
{
    bufferlist in;
    ...
    encode(call, in);
    op.exec(
        RGW_CLASS, RGW_BUCKET_READ_OLH_LOG, in, new ClsBucketIndexOpCtx<rgw_cls_read_olh_log_ret>(&log_ret, &op_ret));
}
...
void cls_rgw_trim_olh_log(librados::ObjectWriteOperation& op, const cls_rgw_obj_key& olh, uint64_t ver,
...
void cls_rgw_bilog_list(librados::ObjectReadOperation& op, const std::string& marker, uint32_t max,
...
void cls_rgw_bilog_trim(librados::ObjectWriteOperation& op, const std::string& start_marker,
...
static bool issue_bi_log_trim(librados::IoCtx& io_ctx, const string& oid, const int shard_id,
                              BucketIndexShardsManager& start_marker_mgr, BucketIndexShardsManager& end_marker_mgr,
                              BucketIndexAioManager* manager)
{
    cls_rgw_bi_log_trim_op call;
    librados::ObjectWriteOperation op;
    cls_rgw_bilog_trim(op, start_marker_mgr.get(shard_id, ""), end_marker_mgr.get(shard_id, ""));
    return manager->aio_operate(io_ctx, shard_id, oid, &op);
}
...
static bool issue_bucket_check_index_op(IoCtx& io_ctx, const int shard_id, const string& oid,
                                        BucketIndexAioManager* manager, rgw_cls_check_index_ret* pdata)
{
    bufferlist in;
    librados::ObjectReadOperation op;
    op.exec(RGW_CLASS, RGW_BUCKET_CHECK_INDEX, in, new ClsBucketIndexOpCtx<rgw_cls_check_index_ret>(pdata, NULL));
    return manager->aio_operate(io_ctx, shard_id, oid, &op);
}
...
int CLSRGWIssueGetDirHeader::issue_op(const int shard_id, const string& oid)
{
    ...
    return issue_bucket_list_op(
        io_ctx, shard_id, oid, empty_key, empty_prefix, empty_delimiter, 0, false, &manager, &result[shard_id]);
}

static bool issue_resync_bi_log(librados::IoCtx& io_ctx, const int shard_id, const string& oid,
                                BucketIndexAioManager* manager)
{
    bufferlist in;
    librados::ObjectWriteOperation op;
...
static bool issue_bi_log_stop(librados::IoCtx& io_ctx, const int shard_id, const string& oid,
                              BucketIndexAioManager* manager)
{
    bufferlist in;
    librados::ObjectWriteOperation op;
...
public:
    explicit GetDirHeaderCompletion(RGWGetDirHeader_CB* _ctx)
        : ret_ctx(_ctx)
    {}
    ~GetDirHeaderCompletion() override { ret_ctx->put(); }
    void handle_completion(int r, bufferlist& outbl) override
    {
...
{
    if (is_truncated) *is_truncated = false;
    ...
    try {
        ...
        if (is_truncated) *is_truncated = result.truncated;
...
void cls_rgw_usage_log_trim(librados::ObjectWriteOperation& op, const string& user, const string& bucket,
                            uint64_t start_epoch, uint64_t end_epoch)
{
    bufferlist in;
    rgw_cls_usage_log_trim_op call;
                    ...
                    list<cls_rgw_gc_obj_info>& entries, bool* truncated, string& next_marker)
{
    ...
    if (truncated) *truncated = ret.truncated;
...
int cls_rgw_lc_get_next_entry(IoCtx& io_ctx, const string& oid, const string& marker, cls_rgw_lc_entry& entry)
{
    bufferlist in, out;
...
int cls_rgw_lc_rm_entry(IoCtx& io_ctx, const string& oid, const cls_rgw_lc_entry& entry)
{
    bufferlist in, out;
...
int cls_rgw_lc_set_entry(IoCtx& io_ctx, const string& oid, const cls_rgw_lc_entry& entry)
{
    bufferlist in, out;
...
int cls_rgw_lc_get_entry(IoCtx& io_ctx, const string& oid, const std::string& marker, cls_rgw_lc_entry& entry)
{
    bufferlist in, out;
...
int cls_rgw_lc_list(IoCtx& io_ctx, const string& oid, const string& marker, uint32_t max_entries,
                    vector<cls_rgw_lc_entry>& entries)
{
    ...
    std::sort(std::begin(ret.entries), std::end(ret.entries), [](const cls_rgw_lc_entry& a, const cls_rgw_lc_entry& b) {
...
void cls_rgw_mp_upload_part_info_update(librados::ObjectWriteOperation& op, const std::string& part_key,
...
int cls_rgw_get_bucket_resharding(librados::IoCtx& io_ctx, const string& oid, cls_rgw_bucket_instance_entry* entry)
{
    bufferlist in, out;
...
static bool issue_set_bucket_resharding(librados::IoCtx& io_ctx, const int shard_id, const string& oid,
                                        const cls_rgw_bucket_instance_entry& entry, BucketIndexAioManager* manager)
{
    bufferlist in;
    cls_rgw_set_bucket_resharding_op call;
    call.entry = entry;
