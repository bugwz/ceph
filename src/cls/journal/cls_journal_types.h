...
struct ObjectPosition
{
    ...
    ObjectPosition()
        ...
        , entry_tid(0)
    {}
    ObjectPosition(uint64_t _object_number, uint64_t _tag_tid, uint64_t _entry_tid)
    ...
    {
        return (object_number == rhs.object_number && tag_tid == rhs.tag_tid && entry_tid == rhs.entry_tid);
    }
    inline bool operator!=(const ObjectPosition& rhs) const { return !(*this == rhs); }
...
struct ObjectSetPosition
{
    ...
    inline bool operator==(const ObjectSetPosition& rhs) const { return (object_positions == rhs.object_positions); }
...
struct Client
{
    ...
    Client(const std::string& _id, const ceph::buffer::list& _data,
           const ObjectSetPosition& _commit_position = ObjectSetPosition(), ClientState _state = CLIENT_STATE_CONNECTED)
        : id(_id)
        , data(_data)
    ...
    {
        return (id == rhs.id && data.contents_equal(rhs.data) && commit_position == rhs.commit_position &&
                state == rhs.state);
    }
    inline bool operator<(const Client& rhs) const { return (id < rhs.id); }
...
struct Tag
{
    ...
    {
        return (tid == rhs.tid && tag_class == rhs.tag_class && data.contents_equal(rhs.data));
    }
    inline bool operator<(const Tag& rhs) const { return (tid < rhs.tid); }
...
std::ostream& operator<<(std::ostream& os, const ObjectPosition& object_position);
std::ostream& operator<<(std::ostream& os, const ObjectSetPosition& object_set_position);
