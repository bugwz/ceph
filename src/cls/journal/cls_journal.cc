...
template<typename T> int read_key(cls_method_context_t hctx, const string& key, T* t, bool ignore_enoent = false)
{
    bufferlist bl;
...
template<typename T> int write_key(cls_method_context_t hctx, const string& key, const T& t)
{
    bufferlist bl;
    encode(t, bl);
...
int expire_tags(cls_method_context_t hctx, const std::string* skip_client_id)
{
    ...
    do {
        ...
        int r = cls_cxx_map_get_vals(hctx, last_read, HEADER_KEY_CLIENT_PREFIX, MAX_KEYS_READ, &vals, &more);
        if (r < 0 && r != -ENOENT) {
            CLS_ERR("failed to retrieve registered clients: %s", cpp_strerror(r).c_str());
            return r;
        }
        ...
        for (auto& val : vals) {
            ...
            try {
                ...
                CLS_ERR("error decoding registered client: %s", val.first.c_str());
                return -EIO;
            }
    ...
    do {
        std::map<std::string, bufferlist> vals;
        int r = cls_cxx_map_get_vals(hctx, last_read, HEADER_KEY_TAG_PREFIX, MAX_KEYS_READ, &vals, &more);
        if (r < 0 && r != -ENOENT) {
        ...
        for (auto& val : vals) {
            ...
            else if (tag_pass == TAG_PASS_SCRUB && tag.tid < minimum_tag_class_to_tids[tag.tag_class]) {
...
int get_client_list_range(cls_method_context_t hctx, std::set<cls::journal::Client>* clients, std::string start_after,
                          uint64_t max_return)
{
    std::string last_read;
    ...
    std::map<std::string, bufferlist> vals;
    bool more;
    int r = cls_cxx_map_get_vals(hctx, last_read, HEADER_KEY_CLIENT_PREFIX, max_return, &vals, &more);
    if (r < 0) {
        CLS_ERR("failed to retrieve omap values: %s", cpp_strerror(r).c_str());
        return r;
    }

    for (std::map<std::string, bufferlist>::iterator it = vals.begin(); it != vals.end(); ++it) {
        try {
            ...
            CLS_ERR("could not decode client '%s': %s", it->first.c_str(), err.what());
            return -EIO;
        }
    }
...
int find_min_commit_position(cls_method_context_t hctx, cls::journal::ObjectSetPosition* minset)
{
    int r;
    bool valid = false;
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_get_order(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint8_t order;
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_get_splay_width(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint8_t splay_width;
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_get_pool_id(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    int64_t pool_id = 0;
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_get_minimum_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_set_minimum_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t object_set;
    try {
        auto iter = in->cbegin();
    ...
    if (current_active_set < object_set) {
        CLS_LOG(10, "active object set earlier than minimum: %" PRIu64 " < %" PRIu64, current_active_set, object_set);
        return -EINVAL;
    }
    ...
    if (object_set == current_minimum_set) {
        ...
        CLS_ERR("object number earlier than current object: %" PRIu64 " < %" PRIu64, object_set, current_minimum_set);
        return -ESTALE;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_get_active_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t active_set;
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_set_active_set(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t object_set;
    try {
        auto iter = in->cbegin();
    ...
    if (current_minimum_set > object_set) {
        CLS_ERR("minimum object set later than active: %" PRIu64 " > %" PRIu64, current_minimum_set, object_set);
        return -EINVAL;
    }
    ...
    if (object_set == current_active_set) {
        ...
        CLS_ERR("object number earlier than current object: %" PRIu64 " < %" PRIu64, object_set, current_active_set);
        return -ESTALE;
    }
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_get_client(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string id;
    try {
        auto iter = in->cbegin();
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_client_register(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string id;
    bufferlist data;
    try {
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_client_update_data(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string id;
    bufferlist data;
    try {
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_client_update_state(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string id;
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_client_unregister(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string id;
    try {
        auto iter = in->cbegin();
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_client_commit(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string id;
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_client_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    std::string start_after;
    uint64_t max_return;
    try {
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_get_next_tag_tid(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t tag_tid;
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_get_tag(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t tag_tid;
    try {
        auto iter = in->cbegin();
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_tag_create(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t tag_tid;
    uint64_t tag_class;
    bufferlist data;
 ...
 * @returns 0 on success, negative error code on failure
 */
int journal_tag_list(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    int tag_pass =
        (minimum_tag_tid == std::numeric_limits<uint64_t>::max() ? TAG_PASS_LIST : TAG_PASS_CALCULATE_MINIMUMS);
    std::string last_read = HEADER_KEY_TAG_PREFIX;
    do {
        std::map<std::string, bufferlist> vals;
        bool more;
        r = cls_cxx_map_get_vals(hctx, last_read, HEADER_KEY_TAG_PREFIX, MAX_KEYS_READ, &vals, &more);
        if (r < 0 && r != -ENOENT) {
            CLS_ERR("failed to retrieve tags: %s", cpp_strerror(r).c_str());
            return r;
        }
 ...
 */
int journal_object_guard_append(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (size >= soft_max_size) {
        CLS_LOG(5, "journal object full: %" PRIu64 " >= %" PRIu64, size, soft_max_size);
        return -EOVERFLOW;
    }
    return 0;
}
 ...
 */
int journal_object_append(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    uint64_t soft_max_size;
    bufferlist data;
    try {
    ...
    auto offset = size;
    r = cls_cxx_write2(hctx, offset, data.length(), &data, CEPH_OSD_OP_FLAG_FADVISE_DONTNEED);
    if (r < 0) {
...
CLS_INIT(journal)
{
    ...
    cls_register_cxx_method(h_class, "create", CLS_METHOD_RD | CLS_METHOD_WR, journal_create, &h_journal_create);
    cls_register_cxx_method(h_class, "get_order", CLS_METHOD_RD, journal_get_order, &h_journal_get_order);
    cls_register_cxx_method(
        h_class, "get_splay_width", CLS_METHOD_RD, journal_get_splay_width, &h_journal_get_splay_width);
    cls_register_cxx_method(h_class, "get_pool_id", CLS_METHOD_RD, journal_get_pool_id, &h_journal_get_pool_id);
    cls_register_cxx_method(
        h_class, "get_minimum_set", CLS_METHOD_RD, journal_get_minimum_set, &h_journal_get_minimum_set);
    cls_register_cxx_method(
        h_class, "set_minimum_set", CLS_METHOD_RD | CLS_METHOD_WR, journal_set_minimum_set, &h_journal_set_minimum_set);
    cls_register_cxx_method(
        h_class, "get_active_set", CLS_METHOD_RD, journal_get_active_set, &h_journal_get_active_set);
    cls_register_cxx_method(
        h_class, "set_active_set", CLS_METHOD_RD | CLS_METHOD_WR, journal_set_active_set, &h_journal_set_active_set);

    cls_register_cxx_method(h_class, "get_client", CLS_METHOD_RD, journal_get_client, &h_journal_get_client);
    cls_register_cxx_method(
        h_class, "client_register", CLS_METHOD_RD | CLS_METHOD_WR, journal_client_register, &h_journal_client_register);
    ...
    cls_register_cxx_method(
        h_class, "client_commit", CLS_METHOD_RD | CLS_METHOD_WR, journal_client_commit, &h_journal_client_commit);
    cls_register_cxx_method(h_class, "client_list", CLS_METHOD_RD, journal_client_list, &h_journal_client_list);

    cls_register_cxx_method(
        h_class, "get_next_tag_tid", CLS_METHOD_RD, journal_get_next_tag_tid, &h_journal_get_next_tag_tid);
    cls_register_cxx_method(h_class, "get_tag", CLS_METHOD_RD, journal_get_tag, &h_journal_get_tag);
    cls_register_cxx_method(
        h_class, "tag_create", CLS_METHOD_RD | CLS_METHOD_WR, journal_tag_create, &h_journal_tag_create);
    cls_register_cxx_method(h_class, "tag_list", CLS_METHOD_RD, journal_tag_list, &h_journal_tag_list);
    ...
    cls_register_cxx_method(
        h_class, "append", CLS_METHOD_RD | CLS_METHOD_WR, journal_object_append, &h_journal_object_append);
}
