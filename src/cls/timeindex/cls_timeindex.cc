...
static void get_index_time_prefix(const utime_t& ts, string& index)
{
    ...
    snprintf(buf, sizeof(buf), "%s%010ld.%06ld_", TIMEINDEX_PREFIX.c_str(), (long)ts.sec(), (long)ts.usec());
    buf[sizeof(buf) - 1] = '\0';

    index = buf;
}

static void get_index(cls_method_context_t hctx, const utime_t& key_ts, const string& key_ext, string& index)
{
...
static int cls_timeindex_add(cls_method_context_t hctx, bufferlist* const in, bufferlist* const out)
{
...
static int cls_timeindex_list(cls_method_context_t hctx, bufferlist* const in, bufferlist* const out)
{
    auto in_iter = in->cbegin();
    ...
    int rc = cls_cxx_map_get_vals(hctx, from_index, TIMEINDEX_PREFIX, max_entries, &keys, &ret.truncated);
    if (rc < 0) {
        return rc;
    }
    ...
    for (; iter != keys.end(); ++iter) {
        ...
        if (use_time_boundary && index.compare(0, to_index.size(), to_index) >= 0) {
            CLS_LOG(20, "DEBUG: cls_timeindex_list: finishing on to_index=%s", to_index.c_str());
            ret.truncated = false;
            break;
        }
        ...
        if (parse_index(index, e.key_ts, e.key_ext) < 0) {
            CLS_LOG(0, "ERROR: cls_timeindex_list: could not parse index=%s", index.c_str());
        }
        else {
            CLS_LOG(20,
...
static int cls_timeindex_trim(cls_method_context_t hctx, bufferlist* const in, bufferlist* const out)
{
    auto in_iter = in->cbegin();
    ...
    int rc = cls_cxx_map_get_vals(hctx, from_index, TIMEINDEX_PREFIX, MAX_TRIM_ENTRIES, &keys, &more);
    if (rc < 0) {
        return rc;
    }
    ...
    for (; iter != keys.end(); ++iter) {
        ...
        if (index.compare(0, to_index.size(), to_index) > 0) {
            CLS_LOG(20, "DEBUG: cls_timeindex_trim: finishing on to_index=%s", to_index.c_str());
            break;
        }
...
CLS_INIT(timeindex)
{
    ...
    /* timeindex */
    cls_register_cxx_method(h_class, "add", CLS_METHOD_RD | CLS_METHOD_WR, cls_timeindex_add, &h_timeindex_add);
    cls_register_cxx_method(h_class, "list", CLS_METHOD_RD, cls_timeindex_list, &h_timeindex_list);
    cls_register_cxx_method(h_class, "trim", CLS_METHOD_RD | CLS_METHOD_WR, cls_timeindex_trim, &h_timeindex_trim);
