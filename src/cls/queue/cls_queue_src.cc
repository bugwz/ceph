...
int queue_write_head(cls_method_context_t hctx, cls_queue_head& head)
{
    ...
    if (bl.length() > head.max_head_size) {
        CLS_LOG(0,
                "ERROR: queue_write_head: invalid head size = %u and urgent data size = %u \n",
                bl.length(),
...
int queue_enqueue(cls_method_context_t hctx, cls_queue_enqueue_op& op, cls_queue_head& head)
{
    ...
    for (auto& bl_data : op.bl_data_vec) {
        ...
        CLS_LOG(
            10, "INFO: queue_enqueue(): Total size to be written is %u and data size is %lu", bl.length(), data_size);
        ...
        if (head.tail.offset >= head.front.offset) {
            ...
            if ((head.tail.offset + bl.length()) <= head.queue_size) {
                ...
                auto ret =
                    cls_cxx_write2(hctx, head.tail.offset, bl.length(), &bl, CEPH_OSD_OP_FLAG_FADVISE_SEQUENTIAL);
                if (ret < 0) {
                    return ret;
                }
                ...
                if (bl.length() <= free_space_available) {
                    ...
                    CLS_LOG(5,
                            "INFO: queue_enqueue: Writing spliced data at offset: %s and data size: %u",
                            head.tail.to_str().c_str(),
                            bl_data_before_wrap.length());
                    auto ret = cls_cxx_write2(hctx,
                                              head.tail.offset,
                                              bl_data_before_wrap.length(),
                                              &bl_data_before_wrap,
                    ...
                    CLS_LOG(5,
                            "INFO: queue_enqueue: Writing remaining data at offset: %s and data size: %u",
                            head.tail.to_str().c_str(),
                            bl.length());
...
int queue_list_entries(cls_method_context_t hctx, const cls_queue_list_op& op, cls_queue_list_ret& op_ret,
                       cls_queue_head& head)
{
    ...
    CLS_LOG(10,
            "INFO: queue_list_entries(): front is: %s, tail is %s",
    ...
    do {
        ...
        CLS_LOG(20,
                "INFO: queue_list_entries(): Entry start offset accounting for leftover data is %lu",
                entry_start_offset);
        ...
        do {
            ...
            if (!entry_start_processed) {
                if (size_to_process >= QUEUE_ENTRY_OVERHEAD) {
                    ...
                    CLS_LOG(
                        10,
                        "INFO: queue_list_entries: not enough data to read entry start and data size, breaking out!");
                    break;
                }
            ...
            if (num_ops == op.max) {
                CLS_LOG(10,
                        "INFO: queue_list_entries(): num_ops is same as op.max, hence breaking out from inner loop!");
                break;
            }
        ...
        if (num_ops == op.max) {
            ...
            CLS_LOG(10,
                    "INFO: queue_list_entries(): num_ops is same as op.max, hence breaking out from outer loop with "
                    "next offset: %lu",
                    next_marker.offset);
            break;
        }
...
int queue_remove_entries(cls_method_context_t hctx, const cls_queue_remove_op& op, cls_queue_head& head)
{
    ...
    CLS_LOG(20,
            "INFO: queue_remove_entries: front offset is: %s and tail offset is %s",
            head.front.to_str().c_str(),
            head.tail.to_str().c_str());
