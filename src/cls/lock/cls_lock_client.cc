...
void lock(ObjectWriteOperation* rados_op, const std::string& name, ClsLockType type, const std::string& cookie,
          const std::string& tag, const std::string& description, const utime_t& duration, uint8_t flags)
{
    cls_lock_lock_op op;
    op.name = name;
    op.type = type;
...
int lock(IoCtx* ioctx, const std::string& oid, const std::string& name, ClsLockType type, const std::string& cookie,
         const std::string& tag, const std::string& description, const utime_t& duration, uint8_t flags)
{
...
void unlock(ObjectWriteOperation* rados_op, const std::string& name, const std::string& cookie)
{
...
int unlock(IoCtx* ioctx, const std::string& oid, const std::string& name, const std::string& cookie)
{
    ObjectWriteOperation op;
...
int aio_unlock(IoCtx* ioctx, const std::string& oid, const std::string& name, const std::string& cookie,
...
void break_lock(ObjectWriteOperation* rados_op, const std::string& name, const std::string& cookie,
...
int break_lock(IoCtx* ioctx, const std::string& oid, const std::string& name, const std::string& cookie,
               const entity_name_t& locker)
{
    ObjectWriteOperation op;
...
void get_lock_info_start(ObjectReadOperation* rados_op, const std::string& name)
{
    bufferlist in;
...
int get_lock_info_finish(bufferlist::const_iterator* iter, map<locker_id_t, locker_info_t>* lockers, ClsLockType* type,
                         std::string* tag)
{
...
int get_lock_info(IoCtx* ioctx, const std::string& oid, const std::string& name,
                  map<locker_id_t, locker_info_t>* lockers, ClsLockType* type, std::string* tag)
{
...
void assert_locked(librados::ObjectOperation* rados_op, const std::string& name, ClsLockType type,
...
void set_cookie(librados::ObjectWriteOperation* rados_op, const std::string& name, ClsLockType type,
...
{
    lock(op, name, ClsLockType::SHARED, cookie, tag, description, duration, flags);
}

int Lock::lock_shared(IoCtx* ioctx, const std::string& oid)
{
    return lock(ioctx, oid, name, ClsLockType::SHARED, cookie, tag, description, duration, flags);
}

void Lock::lock_exclusive(ObjectWriteOperation* op)
{
    lock(op, name, ClsLockType::EXCLUSIVE, cookie, tag, description, duration, flags);
}

int Lock::lock_exclusive(IoCtx* ioctx, const std::string& oid)
{
    return lock(ioctx, oid, name, ClsLockType::EXCLUSIVE, cookie, tag, description, duration, flags);
}

void Lock::lock_exclusive_ephemeral(ObjectWriteOperation* op)
{
    lock(op, name, ClsLockType::EXCLUSIVE_EPHEMERAL, cookie, tag, description, duration, flags);
}

int Lock::lock_exclusive_ephemeral(IoCtx* ioctx, const std::string& oid)
{
    return lock(ioctx, oid, name, ClsLockType::EXCLUSIVE_EPHEMERAL, cookie, tag, description, duration, flags);
}
