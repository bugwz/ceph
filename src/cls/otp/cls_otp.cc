...
void otp_instance::trim_expired(const ceph::real_time& now)
{
    ...
    while (!last_checks.empty() && last_checks.front().timestamp < window_start) {
...
bool otp_instance::verify(const ceph::real_time& timestamp, const string& val)
{
    ...
    int result = oath_totp_validate2(otp.seed_bin.c_str(),
                                     otp.seed_bin.length(),
                                     secs,
                                     otp.step_size,
                                     otp.time_ofs,
...
static int parse_seed(const string& seed, SeedType seed_type, bufferlist* seed_bin)
{
    ...
    case OTP_SEED_BASE32:
        ...
        result = oath_base32_decode(seed.c_str(), seed.length(), &psecret, &slen);
        break;
    default: /* just assume hex is the default */ result = oath_hex2bin(seed.c_str(), psecret, &slen);
    }
    if (result != OATH_OK) {
...
static int otp_set_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
...
static int otp_remove_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    CLS_LOG(20, "%s", __func__);
...
static int otp_get_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    CLS_LOG(20, "%s", __func__);
...
static int otp_check_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    CLS_LOG(20, "%s", __func__);
...
static int otp_get_result(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    CLS_LOG(20, "%s", __func__);
    cls_otp_check_otp_op op;
    try {
...
static int otp_get_current_time_op(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    CLS_LOG(20, "%s", __func__);
...
CLS_INIT(otp)
{
    ...
    cls_register_cxx_method(h_class, "otp_set", CLS_METHOD_RD | CLS_METHOD_WR, otp_set_op, &h_set_otp_op);
    cls_register_cxx_method(h_class, "otp_get", CLS_METHOD_RD, otp_get_op, &h_get_otp_op);
    cls_register_cxx_method(h_class, "otp_check", CLS_METHOD_RD | CLS_METHOD_WR, otp_check_op, &h_check_otp_op);
    cls_register_cxx_method(h_class, "otp_get_result", CLS_METHOD_RD, otp_get_result, &h_get_result_op);
    cls_register_cxx_method(h_class, "otp_remove", CLS_METHOD_RD | CLS_METHOD_WR, otp_remove_op, &h_remove_otp_op);
    cls_register_cxx_method(
        h_class, "get_current_time", CLS_METHOD_RD, otp_get_current_time_op, &h_get_current_time_op);
