...
void OTP::create(librados::ObjectWriteOperation* rados_op, const otp_info_t& config)
{
...
void OTP::set(librados::ObjectWriteOperation* rados_op, const list<otp_info_t>& entries)
{
    cls_otp_set_otp_op op;
    op.entries = entries;
    bufferlist in;
...
void OTP::remove(librados::ObjectWriteOperation* rados_op, const string& id)
{
...
int OTP::check(CephContext* cct, librados::IoCtx& ioctx, const string& oid, const string& id, const string& val,
               otp_check_t* result)
{
...
int OTP::get(librados::ObjectReadOperation* rop, librados::IoCtx& ioctx, const string& oid, const list<string>* ids,
             bool get_all, list<otp_info_t>* result)
{
...
int OTP::get(librados::ObjectReadOperation* op, librados::IoCtx& ioctx, const string& oid, const string& id,
             otp_info_t* result)
{
    list<string> ids{id};
    list<otp_info_t> ret;
...
int OTP::get_all(librados::ObjectReadOperation* op, librados::IoCtx& ioctx, const string& oid, list<otp_info_t>* result)
{
    return get(op, ioctx, oid, nullptr, true, result);
}

int OTP::get_current_time(librados::IoCtx& ioctx, const string& oid, ceph::real_time* result)
{
