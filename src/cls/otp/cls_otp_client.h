...
public:
    ...
    static int get(librados::ObjectReadOperation* op, librados::IoCtx& ioctx, const std::string& oid,
                   const std::string& id, otp_info_t* result);
    static int get_all(librados::ObjectReadOperation* op, librados::IoCtx& ioctx, const std::string& oid,
...
// rgw_rados_operate() should be called after the overloads w/o calls to io_ctx.operate()/exec()
    ...
    static int check(CephContext* cct, librados::IoCtx& ioctx, const std::string& oid, const std::string& id,
                     const std::string& val, otp_check_t* result);
    static int get_current_time(librados::IoCtx& ioctx, const std::string& oid, ceph::real_time* result);
#endif
};
...
public:
    ...
    void set_step_size(int step_size) { config.step_size = step_size; }
    void set_window(int window) { config.window = window; }
    void get_config(otp_info_t* conf) { *conf = config; }
};
}   // namespace otp
}   // namespace cls
}   // namespace rados
