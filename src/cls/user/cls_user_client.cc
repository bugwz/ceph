...
void cls_user_bucket_list(librados::ObjectReadOperation& op, const string& in_marker, const string& end_marker,
                          int max_entries, list<cls_user_bucket_entry>& entries, string* out_marker, bool* truncated,
                          int* pret)
{
    bufferlist inbl;
...
public:
    ClsUserGetHeaderCtx(cls_user_header* _h, RGWGetUserHeader_CB* _ctx, int* _pret)
        : header(_h)
        , ret_ctx(_ctx)
        , pret(_pret)
    {}
...
int cls_user_get_header_async(IoCtx& io_ctx, string& oid, RGWGetUserHeader_CB* ctx)
{
    ...
    op.exec("user",
            "get_header",
            in,
            new ClsUserGetHeaderCtx(
                NULL, ctx, NULL)); /* no need to pass pret, as we'll call ctx->handle_response() with correct error */
