...
static int get_existing_bucket_entry(cls_method_context_t hctx, const string& bucket_name, cls_user_bucket_entry& entry)
{
...
static int cls_user_set_buckets_info(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    for (auto iter = op.entries.begin(); iter != op.entries.end(); ++iter) {
        ...
        if (ret == -ENOENT) {
            if (!op.add) continue; /* racing bucket removal */
        ...
        CLS_LOG(20,
                "storing entry for key=%s size=%lld count=%lld",
                key.c_str(),
                (long long)update_entry.size,
                (long long)update_entry.count);
    ...
    CLS_LOG(20,
            "header: total bytes=%lld entries=%lld",
            (long long)header.stats.total_bytes,
            (long long)header.stats.total_entries);

    if (header.last_stats_update < op.time) header.last_stats_update = op.time;
...
static int cls_user_complete_stats_sync(cls_method_context_t hctx, bufferlist* in, bufferlist* out)
{
    ...
    if (header.last_stats_sync < op.time) header.last_stats_sync = op.time;
...
#define MAX_ENTRIES 1000
    ...
    CLS_LOG(
        20, "from_index=%s to_index=%s match_prefix=%s", from_index.c_str(), to_index.c_str(), match_prefix.c_str());
...
static int cls_user_reset_stats(cls_method_context_t hctx, bufferlist* in, bufferlist* out /*ignore*/)
{
    ...
    do {
        ...
        CLS_LOG(20, "%s: read %lu key-values, truncated=%d", __func__, keys.size(), truncated);
        ...
        for (const auto& kv : keys) {
            ...
            try {
                ...
                CLS_LOG(0, "ERROR: %s failed to decode bucket entry for %s", __func__, kv.first.c_str());
                return -EIO;
            }
...
/// equivalent to --sync-stats which also re-calculates the stats for
/// each bucket.
static int cls_user_reset_stats2(cls_method_context_t hctx, buffer::list* in, buffer::list* out)
{
    ...
    int rc = cls_cxx_map_get_vals(hctx, from_index, prefix, MAX_ENTRIES, &keys, &ret.truncated);
    if (rc < 0) {
        CLS_LOG(0, "ERROR: %s failed to retrieve omap key-values", __func__);
        return rc;
    }
    CLS_LOG(20, "%s: read %lu key-values, truncated=%d", __func__, keys.size(), ret.truncated);
    ...
    /* try-update marker */
    if (!keys.empty()) ret.marker = (--keys.cend())->first;
...
CLS_INIT(user)
{
    ...
    cls_register_cxx_method(
        h_class, "remove_bucket", CLS_METHOD_RD | CLS_METHOD_WR, cls_user_remove_bucket, &h_user_remove_bucket);
    ...
    cls_register_cxx_method(
        h_class, "reset_user_stats", CLS_METHOD_RD | CLS_METHOD_WR, cls_user_reset_stats, &h_user_reset_stats);
    cls_register_cxx_method(
        h_class, "reset_user_stats2", CLS_METHOD_RD | CLS_METHOD_WR, cls_user_reset_stats2, &h_user_reset_stats2);
