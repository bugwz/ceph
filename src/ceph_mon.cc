...
{
    if (mon) mon->handle_signal(signum);
}
...
int obtain_monmap(MonitorDBStore& store, bufferlist& bl)
{
    ...
    if (store.exists("mon_sync", "in_sync") || store.exists("mon_sync", "force_sync")) {
...
int check_mon_data_empty()
{
    ...
    while ((de = ::readdir(dir))) {
        if (string(".") != de->d_name && string("..") != de->d_name && string("kv_backend") != de->d_name) {
            code = -ENOTEMPTY;
            break;
        }
    }
...
static void usage()
{
    cout << "usage: ceph-mon -i <ID> [flags]\n"
         ...
         << "        sets monitor's crush bucket location (only for stretch mode)" << std::endl;
...
int main(int argc, const char** argv)
{
    ...
    int flags = 0;
    {
        ...
        std::string val;
        for (std::vector<const char*>::iterator i = args_copy.begin(); i != args_copy.end();) {
    ...
    auto cct = global_init(&defaults, args, CEPH_ENTITY_TYPE_MON, CODE_ENVIRONMENT_DAEMON, flags);
    ...
    if (mkfs) {
        ...
        if (monmap_fn.length()) {
            ...
            *_dout << dendl;
            ...
            if (oss.tellp()) derr << oss.str() << dendl;
            if (err < 0) {
            ...
            *_dout << dendl;
            ...
            if (monmap.contains(g_conf()->name.get_id())) {
                ...
                if (monmap.contains(av, &name)) {
                    ...
                    dout(0) << argv[0] << ": renaming mon." << name << " " << av << " to mon."
                ...
                if (have_local_addr(g_ceph_context, ls, &local)) {
                    ...
                    if (name.compare(0, 7, "noname-") == 0) {
                        dout(0) << argv[0] << ": mon." << name << " " << local << " is local, renaming to mon."
                                << g_conf()->name.get_id() << dendl;
                        monmap.rename(name, g_conf()->name.get_id());
                    }
                    else if (name.size()) {
        ...
        if (oss.tellp()) derr << oss.str() << dendl;
        if (r < 0) {
            derr << argv[0] << ": error opening mon data directory at '" << g_conf()->mon_data
    ...
    if (err < 0 && err == -ENOENT) {
        ...
        derr << "error accessing monitor data directory at '" << g_conf()->mon_data << "': " << cpp_strerror(-err)
             << dendl;
        exit(1);
    }
...

    {
        ...
        if (stats.avail_percent <= g_conf()->mon_data_avail_crit) {
            derr << "error: monitor data filesystem reached concerning levels of"
                 << " available storage space (available: " << stats.avail_percent << "% " << byte_u_t(stats.byte_avail)
    ...
    if (!(flags & CINIT_FLAG_NO_DAEMON_ACTIONS)) {
        ...
        if (global_init_preload_erasure_code(g_ceph_context) < 0) prefork.exit(1);
    }
...

    {
        ...
        if (oss.tellp()) derr << oss.str() << dendl;
        if (err < 0) {
            derr << "error opening mon data directory at '" << g_conf()->mon_data << "': " << cpp_strerror(err)
                 << dendl;
            prefork.exit(1);
        }
    }
    ...
    if (monmap.contains(g_conf()->name.get_id())) {
        ...
        if (g_conf().get_val_from_conf_file(my_sections, "mon addr", mon_addr_str, true) == 0) {
            entity_addr_t conf_addr;
            if (conf_addr.parse(mon_addr_str)) {
                entity_addrvec_t conf_addrs = make_mon_addrs(conf_addr);
                if (ipaddrs != conf_addrs) {
                    derr << "WARNING: 'mon addr' config option " << conf_addrs << " does not match monmap file"
                         << std::endl
    ...
    Messenger* msgr = Messenger::create(g_ceph_context, public_msgr_type, entity_name_t::MON(rank), "mon", 0);
    if (!msgr) exit(1);
    msgr->set_cluster_protocol(CEPH_MON_PROTOCOL);
    msgr->set_default_send_priority(CEPH_MSG_PRIO_HIGH);

    msgr->set_default_policy(Messenger::Policy::stateless_server(0));
    msgr->set_policy(entity_name_t::TYPE_MON, Messenger::Policy::lossless_peer_reuse(CEPH_FEATURE_SERVER_LUMINOUS));
    msgr->set_policy(entity_name_t::TYPE_OSD, Messenger::Policy::stateless_server(CEPH_FEATURE_SERVER_LUMINOUS));
    msgr->set_policy(entity_name_t::TYPE_CLIENT, Messenger::Policy::stateless_server(0));
    ...
    dout(0) << "starting " << g_conf()->name << " rank " << rank << " at public addrs " << public_addrs
            << " at bind addrs " << bind_addrs << " mon_data " << g_conf()->mon_data << " fsid " << monmap.get_fsid()
            << dendl;

    Messenger* mgr_msgr = Messenger::create(
        g_ceph_context, public_msgr_type, entity_name_t::MON(rank), "mon-mgrc", Messenger::get_pid_nonce());
    if (!mgr_msgr) {
        derr << "unable to create mgr_msgr" << dendl;
        prefork.exit(1);
    }

    mon = new Monitor(g_ceph_context, g_conf()->name.get_id(), store, msgr, mgr_msgr, &monmap);
    ...
    if (g_conf()->inject_early_sigterm) kill(getpid(), SIGTERM);
