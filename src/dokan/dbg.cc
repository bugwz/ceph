...
void print_credentials(ostringstream& Stream, PDOKAN_FILE_INFO DokanFileInfo)
{
    ...
    if (!LookupAccountSidA(NULL, tokenUser->User.Sid, accountName, &accountLength, domainName, &domainLength, &snu)) {
        err = GetLastError();
...
void print_open_params(LPCSTR FilePath, ACCESS_MASK AccessMode, DWORD FlagsAndAttributes, ULONG ShareMode,
                       DWORD CreationDisposition, ULONG CreateOptions, PDOKAN_FILE_INFO DokanFileInfo)
{
    ostringstream o;
