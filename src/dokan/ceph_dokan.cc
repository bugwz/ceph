...
#define READ_ACCESS_REQUESTED(access_mode)                                                                \
    (access_mode & GENERIC_READ || access_mode & FILE_SHARE_READ || access_mode & STANDARD_RIGHTS_READ || \
     access_mode & FILE_SHARE_READ)
#define WRITE_ACCESS_REQUESTED(access_mode)                                                                  \
    (access_mode & GENERIC_WRITE || access_mode & FILE_SHARE_WRITE || access_mode & STANDARD_RIGHTS_WRITE || \
...
static_assert(sizeof(fd_context) <= 8, "fd_context exceeds DOKAN_FILE_INFO.Context size.");
...
static NTSTATUS do_open_file(string path, int flags, mode_t mode, fd_context* fdc)
{
...
static NTSTATUS WinCephCreateDirectory(LPCWSTR FileName, PDOKAN_FILE_INFO DokanFileInfo)
{
...
static NTSTATUS WinCephCreateFile(LPCWSTR FileName, PDOKAN_IO_SECURITY_CONTEXT SecurityContext,
                                  ACCESS_MASK DesiredAccess, ULONG FileAttributes, ULONG ShareMode,
                                  ULONG CreateDisposition, ULONG CreateOptions, PDOKAN_FILE_INFO DokanFileInfo)
{
    ...
    if (g_cfg->debug) {
        print_open_params(
            path.c_str(), AccessMode, FlagsAndAttributes, ShareMode, CreationDisposition, CreateOptions, DokanFileInfo);
    }
    ...
    *fdc = {0};
    ...
    if (!ret) { /* File Exists */
        if (S_ISREG(stbuf.stx_mode)) {
            ...
            case CREATE_NEW: return STATUS_OBJECT_NAME_COLLISION;
            case TRUNCATE_EXISTING:
                // open O_TRUNC & return 0
                return do_open_file(path, O_CREAT | O_TRUNC | O_RDWR, g_cfg->file_mode, fdc);
            case OPEN_ALWAYS:
                // open & return STATUS_OBJECT_NAME_COLLISION
                if (!WRITE_ACCESS_REQUESTED(AccessMode)) fdc->read_only = 1;
                if ((st = do_open_file(path, fdc->read_only ? O_RDONLY : O_RDWR, g_cfg->file_mode, fdc))) return st;
            ...
            case CREATE_ALWAYS:
                ...
                if ((st = do_open_file(path, O_CREAT | O_TRUNC | O_RDWR, g_cfg->file_mode, fdc))) return st;
                return STATUS_OBJECT_NAME_COLLISION;
            }
        }
            ...
            switch (CreationDisposition) {
            case CREATE_NEW: return STATUS_OBJECT_NAME_COLLISION;
            case TRUNCATE_EXISTING: return 0;
            case OPEN_ALWAYS:
            case OPEN_EXISTING: return do_open_file(path, O_RDONLY, g_cfg->file_mode, fdc);
            case CREATE_ALWAYS: return STATUS_OBJECT_NAME_COLLISION;
            }
        }
        else {
        ...
        if (DokanFileInfo->IsDirectory) {
            ...
            if ((st = WinCephCreateDirectory(FileName, DokanFileInfo))) return st;
        ...
        case CREATE_NEW:
            // create & return 0
            return do_open_file(path, O_CREAT | O_RDWR | O_EXCL, g_cfg->file_mode, fdc);
        case CREATE_ALWAYS:
            // create & return 0
            return do_open_file(path, O_CREAT | O_TRUNC | O_RDWR, g_cfg->file_mode, fdc);
        case OPEN_ALWAYS: return do_open_file(path, O_CREAT | O_RDWR, g_cfg->file_mode, fdc);
        case OPEN_EXISTING:
        case TRUNCATE_EXISTING:
...
static void WinCephCloseFile(LPCWSTR FileName, PDOKAN_FILE_INFO DokanFileInfo)
{
    string path = get_path(FileName);
...
static void WinCephCleanup(LPCWSTR FileName, PDOKAN_FILE_INFO DokanFileInfo)
{
    string path = get_path(FileName);
    ...
    if (DokanFileInfo->DeleteOnClose) {
        ...
        if (DokanFileInfo->IsDirectory) {
            ...
            if (ret) derr << __func__ << " " << path << ": ceph_rmdir failed. Error: " << ret << dendl;
        }
        else {
...
static NTSTATUS WinCephReadFile(LPCWSTR FileName, LPVOID Buffer, DWORD BufferLength, LPDWORD ReadLength,
                                LONGLONG Offset, PDOKAN_FILE_INFO DokanFileInfo)
{
    ...
    if (Offset > CEPH_DOKAN_MAX_FILE_SZ || BufferLength > CEPH_DOKAN_MAX_IO_SZ) {
        dout(2) << "File read too large: " << get_path(FileName) << ". Offset: " << Offset
    ...
    if (!fdc->fd) {
        ...
        if (ret < 0) {
            dout(2) << __func__ << " " << path << ": ceph_read failed. Error: " << ret << ". Offset: " << Offset
                    << "Buffer length: " << BufferLength << dendl;
        ...
        *ReadLength = ret;
        ...
        if (ret < 0) {
            dout(2) << __func__ << " " << get_path(FileName) << ": ceph_read failed. Error: " << ret
...
static NTSTATUS WinCephWriteFile(LPCWSTR FileName, LPCVOID Buffer, DWORD NumberOfBytesToWrite,
                                 LPDWORD NumberOfBytesWritten, LONGLONG Offset, PDOKAN_FILE_INFO DokanFileInfo)
{
    ...
    if (Offset > CEPH_DOKAN_MAX_FILE_SZ || NumberOfBytesToWrite > CEPH_DOKAN_MAX_IO_SZ) {
        dout(2) << "File write too large: " << get_path(FileName) << ". Offset: " << Offset
                << ". Buffer length: " << NumberOfBytesToWrite
                << ". WriteToEndOfFile: " << (bool)DokanFileInfo->WriteToEndOfFile << dendl;
        return STATUS_FILE_TOO_LARGE;
    }
    pfd_context fdc = (pfd_context) & (DokanFileInfo->Context);
    if (fdc->read_only) return STATUS_ACCESS_DENIED;
    ...
    if (!fdc->fd) {
        ...
        if (ret < 0) {
            dout(2) << __func__ << " " << path << ": ceph_write failed. Error: " << ret << ". Offset: " << Offset
        ...
        *NumberOfBytesWritten = ret;
        ...
        if (ret < 0) {
            dout(2) << __func__ << " " << get_path(FileName) << ": ceph_write failed. Error: " << ret
...
static NTSTATUS WinCephFlushFileBuffers(LPCWSTR FileName, PDOKAN_FILE_INFO DokanFileInfo)
{
    pfd_context fdc = (pfd_context) & (DokanFileInfo->Context);
    if (!fdc->fd) {
...
static NTSTATUS WinCephGetFileInformation(LPCWSTR FileName, LPBY_HANDLE_FILE_INFORMATION HandleFileInformation,
...
static NTSTATUS WinCephFindFiles(LPCWSTR FileName,
...
static NTSTATUS WinCephDeleteDirectory(LPCWSTR FileName, PDOKAN_FILE_INFO DokanFileInfo)
{
    string path = get_path(FileName);
    dout(20) << __func__ << " " << path << dendl;
...
static NTSTATUS WinCephMoveFile(LPCWSTR FileName,   // existing file name
...
static NTSTATUS WinCephSetEndOfFile(LPCWSTR FileName, LONGLONG ByteOffset, PDOKAN_FILE_INFO DokanFileInfo)
{
    pfd_context fdc = (pfd_context) & (DokanFileInfo->Context);
    if (!fdc->fd) {
        derr << __func__ << ": missing context: " << get_path(FileName) << dendl;
    ...
    if (ret) {
        dout(2) << __func__ << " " << get_path(FileName) << ": ceph_ftruncate failed. Error: " << ret
...
static NTSTATUS WinCephSetAllocationSize(LPCWSTR FileName, LONGLONG AllocSize, PDOKAN_FILE_INFO DokanFileInfo)
{
    pfd_context fdc = (pfd_context) & (DokanFileInfo->Context);
    if (!fdc->fd) {
        derr << __func__ << ": missing context: " << get_path(FileName) << dendl;
...
static NTSTATUS WinCephSetFileAttributes(LPCWSTR FileName, DWORD FileAttributes, PDOKAN_FILE_INFO DokanFileInfo)
{
    string path = get_path(FileName);
...
static NTSTATUS WinCephSetFileTime(LPCWSTR FileName, CONST FILETIME* CreationTime, CONST FILETIME* LastAccessTime,
                                   CONST FILETIME* LastWriteTime, PDOKAN_FILE_INFO DokanFileInfo)
{
...
static NTSTATUS WinCephSetFileSecurity(LPCWSTR FileName, PSECURITY_INFORMATION SecurityInformation,
...
static NTSTATUS WinCephGetVolumeInformation(LPWSTR VolumeNameBuffer, DWORD VolumeNameSize, LPDWORD VolumeSerialNumber,
...
static NTSTATUS WinCephGetDiskFreeSpace(PULONGLONG FreeBytesAvailable, PULONGLONG TotalNumberOfBytes,
...
int do_unmap(wstring& mountpoint)
{
    if (!DokanRemoveMountPoint(mountpoint.c_str())) {
        wcerr << "Couldn't remove the specified CephFS mount: " << mountpoint << std::endl;
        return -EINVAL;
    }
    return 0;
}
...
static NTSTATUS WinCephUnmount(PDOKAN_FILE_INFO DokanFileInfo)
{
    cleanup_mount();
...
{
    switch (dwType) {
    case CTRL_C_EVENT: dout(0) << "Received ctrl-c." << dendl; exit(0);
    case CTRL_BREAK_EVENT: dout(0) << "Received break event." << dendl; break;
    default: dout(0) << "Received console event: " << dwType << dendl;
    }
    return TRUE;
}
...
NTSTATUS get_volume_serial(PDWORD serial)
{
    ...
    char fsid_str[64] = {0};
    int ret = ceph_getxattr(cmount, "/", "ceph.cluster_fsid", fsid_str, sizeof(fsid_str));
    if (ret < 0) {
...
int do_map()
{
    PDOKAN_OPERATIONS dokan_operations = (PDOKAN_OPERATIONS)malloc(sizeof(DOKAN_OPERATIONS));
    PDOKAN_OPTIONS dokan_options = (PDOKAN_OPTIONS)malloc(sizeof(DOKAN_OPTIONS));
    ...
    case DOKAN_SUCCESS: dout(2) << "Dokan has returned successfully" << dendl; break;
    case DOKAN_ERROR: derr << "Received generic dokan error." << dendl; break;
    case DOKAN_DRIVE_LETTER_ERROR: derr << "Invalid drive letter or mountpoint." << dendl; break;
    case DOKAN_DRIVER_INSTALL_ERROR: derr << "Can't initialize Dokan driver." << dendl; break;
    case DOKAN_START_ERROR: derr << "Dokan failed to start" << dendl; break;
    case DOKAN_MOUNT_ERROR: derr << "Dokan mount error." << dendl; break;
    case DOKAN_MOUNT_POINT_ERROR: derr << "Invalid mountpoint." << dendl; break;
    default: derr << "Unknown Dokan error: " << status << dendl; break;
    }
...
boost::intrusive_ptr<CephContext> do_global_init(int argc, const char** argv, Command cmd)
{
...
int main(int argc, const char** argv)
{
    ...
    switch (cmd) {
    case Command::Version: std::cout << pretty_version_to_str() << std::endl; return 0;
    ...
    switch (cmd) {
    case Command::Map: return do_map();
    case Command::Unmap: return do_unmap(g_cfg->mountpoint);
