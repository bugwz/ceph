...
int parse_args(std::vector<const char*>& args, std::ostream* err_msg, Command* command, Config* cfg)
{
    if (args.empty()) {
    ...
    for (i = args.begin(); i != args.end();) {
        if (ceph_argparse_flag(args, i, "-h", "--help", (char*)NULL)) {
            ...
            *command = Command::Version;
        ...
        else if (ceph_argparse_witharg(args, i, &win_vol_name, "--win-vol-name", (char*)NULL)) {
            cfg->win_vol_name = to_wstring(win_vol_name);
        }
        else if (ceph_argparse_witharg(args, i, &win_vol_serial, "--win-vol-serial", (char*)NULL)) {
            cfg->win_vol_serial = std::stoul(win_vol_serial);
        }
        else if (ceph_argparse_witharg(args, i, &max_path_len, "--max-path-len", (char*)NULL)) {
            ...
            if (!std::regex_match(file_mode, std::regex("^[0-7]{3}$")) || mode < 01 || mode > 0777) {
            ...
            if (!std::regex_match(dir_mode, std::regex("^[0-7]{3}$")) || mode < 01 || mode > 0777) {
        ...
        else if (ceph_argparse_witharg(args, i, &thread_count, err, "--thread-count", "-t", (char*)NULL)) {
            std::cerr << "ceph-dokan: the thread count parameter is not supported by Dokany v2 "
                     ...
                     args, i, (int*)&cfg->operation_timeout, err, "--operation-timeout", (char*)NULL)) {
            if (!err.str().empty()) {
...
int set_dokan_options(Config* cfg, PDOKAN_OPTIONS dokan_options)
{
    ...
    if (cfg->removable) dokan_options->Options |= DOKAN_OPTION_REMOVABLE;
    if (cfg->use_win_mount_mgr) dokan_options->Options |= DOKAN_OPTION_MOUNT_MANAGER;
    if (cfg->current_session_only) dokan_options->Options |= DOKAN_OPTION_CURRENT_SESSION;
    if (cfg->readonly) dokan_options->Options |= DOKAN_OPTION_WRITE_PROTECT;
    if (cfg->debug) dokan_options->Options |= DOKAN_OPTION_DEBUG;
    if (cfg->dokan_stderr) dokan_options->Options |= DOKAN_OPTION_STDERR;
