...
int DataScan::main(const std::vector<const char*>& args)
{
    ...
    for (std::vector<const char*>::const_iterator i = args.begin() + 1; i != args.end(); ++i) {
        ...
        if (i + 1 == args.end() && (command == "scan_inodes" || command == "scan_extents" || command == "cleanup")) {
            data_pool_name = *i;
            continue;
        }
...
int DataScan::scan_extents()
{
    return forall_objects(
        data_io, false, [this](std::string const& oid, uint64_t obj_name_ino, uint64_t obj_name_offset) -> int {
            // Read size
            uint64_t size;
            time_t mtime;
            ...
            r = ClsCephFSClient::accumulate_inode_metadata(data_io, obj_name_ino, obj_name_offset, size, mtime);
            if (r < 0) {
            ...
            if (accum_res.ceiling_obj_index > 0) {
                ...
                if (loaded_layout.pool_id == -1) {
                    ...
                    dout(20) << "loaded layout from xattr:"
                             << " os: " << guessed_layout.object_size << " sc: " << guessed_layout.stripe_count
