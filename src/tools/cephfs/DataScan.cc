...
void DataScan::usage()
{
    std::cout << "Usage: \n"
              ...
              << "  cephfs-data-scan scan_extents [--force-pool] [--worker_n N --worker_m M] [<data pool name> [<extra "
                 "data pool name> ...]]\n"
              << "  cephfs-data-scan scan_inodes [--force-pool] [--force-corrupt] [--worker_n N --worker_m M] [<data "
                 "pool name>]\n"
...
bool DataScan::parse_kwarg(const std::vector<const char*>& args, std::vector<const char*>::const_iterator& i, int* r)
{
    if (i + 1 == args.end()) {
        return false;
    }
...
bool DataScan::parse_arg(const std::vector<const char*>& args, std::vector<const char*>::const_iterator& i)
{
    const std::string arg(*i);
    if (arg == "--force-pool") {
        force_pool = true;
...
int DataScan::main(const std::vector<const char*>& args)
{
    ...
    for (std::vector<const char*>::const_iterator i = args.begin() + 1; i != args.end(); ++i) {
        ...
        if (i + 1 == args.end() && (command == "scan_inodes" || command == "cleanup")) {
            data_pool_name = *i;
            continue;
        }
    ...
    r = rados.connect();
    if (r < 0) {
        std::cerr << "couldn't connect to cluster: " << cpp_strerror(r) << std::endl;
        return r;
    }
    ...
    if (command == "scan_inodes" || command == "scan_extents" || command == "cleanup") {
        data_pool_id = fs->mds_map.get_first_data_pool();

        std::string pool_name;
        r = rados.pool_reverse_lookup(data_pool_id, &pool_name);
        if (r < 0) {
            std::cerr << "Failed to resolve data pool: " << cpp_strerror(r) << std::endl;
            return r;
        }
        ...
        if (data_pool_name.empty()) {
            ...
            std::cerr << "Warning: pool '" << data_pool_name
                      << "' is not the "
                         "main CephFS data pool!"
                      << std::endl;
            if (!force_pool) {
    ...
    if (command == "scan_extents") {
        ...
        for (auto& data_pool_name : extra_data_pool_names) {
            ...
            if (data_pool_id < 0) {
                ...
                dout(4) << "data pool '" << data_pool_name << "' has ID " << pool_id << dendl;
            }
    ...
    if (command == "scan_frags" || command == "scan_links") {
        ...
        if (r < 0) {
            std::cerr << "Pool " << metadata_pool_id << " identified in MDS map not found in RADOS!" << std::endl;
            return r;
        }
...
int MetadataDriver::inject_unlinked_inode(inodeno_t inono, int mode, int64_t data_pool_id)
{
    ...
    if (already_exists && !force_init) {
        std::cerr << "Inode 0x" << std::hex << inono << std::dec
                  << " already"
...
int DataScan::scan_extents()
{
    ...
    for (auto ioctx : data_ios) {
        int r = forall_objects(
            *ioctx,
            false,
            [this, ioctx](std::string const& oid, uint64_t obj_name_ino, uint64_t obj_name_offset) -> int {
                // Read size
                uint64_t size;
                time_t mtime;
                ...
                int64_t obj_pool_id = data_io.get_id() != ioctx->get_id() ? ioctx->get_id() : -1;
                ...
                r = ClsCephFSClient::accumulate_inode_metadata(
                    data_io, obj_name_ino, obj_name_offset, size, obj_pool_id, mtime);
                if (r < 0) {
...
int DataScan::probe_filter(librados::IoCtx& ioctx)
{
    ...
    int r = ioctx.object_list(ioctx.object_list_begin(), ioctx.object_list_end(), 1, filter_bl, &tmp_result, &tmp_next);

    return r >= 0;
}

int DataScan::forall_objects(librados::IoCtx& ioctx, bool untagged_only,
                             std::function<int(std::string, uint64_t, uint64_t)> handler)
{
    librados::ObjectCursor range_i;
    librados::ObjectCursor range_end;
    ioctx.object_list_slice(ioctx.object_list_begin(), ioctx.object_list_end(), n, m, &range_i, &range_end);
    ...
    while (range_i < range_end) {
        ...
        int r = ioctx.object_list(range_i, range_end, 1, filter_bl, &result, &range_i);
        if (r < 0) {
...
int DataScan::scan_inodes()
{
    ...
    return forall_objects(
        data_io, true, [this](std::string const& oid, uint64_t obj_name_ino, uint64_t obj_name_offset) -> int {
            int r = 0;
            ...
            if (r == -EINVAL) {
                ...
                dout(4) << "Unexpected error loading accumulated metadata from '" << oid << "': " << cpp_strerror(r)
                        << dendl;
            ...
            if (accum_res.obj_pool_id == -1) {
                ...
                if (r != 0) {
                    derr << "Unexpected error opening file data pool id=" << guessed_layout.pool_id << ": "
                ...
                if (r < 0) {
                    if (r != -ENODATA) {
                        derr << "Unexpected error reading layout for " << oid << ": " << cpp_strerror(r) << dendl;
                        return r;
                    }
                }
                else {
            ...
            if (accum_res.ceiling_obj_index > 0) {
                ...
                if (loaded_layout.pool_id == -1) {
                ...
                else if (!loaded_layout.is_valid() || loaded_layout.object_size < accum_res.max_obj_size) {
                    ...
                    dout(4) << "bogus xattr layout on 0x" << std::hex << obj_name_ino << std::dec
                    ...
                    dout(20) << "loaded layout from xattr:"
                             << " pi: " << guessed_layout.pool_id << " os: " << guessed_layout.object_size
                             << " sc: " << guessed_layout.stripe_count << " su: " << guessed_layout.stripe_unit
                             << dendl;
                    ...
                    // pool we read from.
                    if (!fsmap->get_filesystem(fscid)->mds_map.is_data_pool(guessed_layout.pool_id)) {
                ...
                if (guessed_layout.stripe_count == 1) {
                    ...
                    file_size = guessed_layout.object_size * accum_res.ceiling_obj_index + accum_res.ceiling_obj_size;
                }
                else {
                    ...
                    if (accum_res.ceiling_obj_index > guessed_layout.stripe_count - 1) {
                        complete_objs =
                            (accum_res.ceiling_obj_index / guessed_layout.stripe_count) * guessed_layout.stripe_count;
                    }
                    else {
                        complete_objs = 0;
                    }
                    ...
                    dout(10) << "calculating striped size from complete objs: " << complete_objs
                             << ", partial objs: " << partial_objs << dendl;
                    ...
                    for (uint64_t i = complete_objs; i < complete_objs + partial_objs; ++i) {
                        char buf[60];
                        snprintf(
                            buf, sizeof(buf), "%llx.%08llx", (long long unsigned)obj_name_ino, (long long unsigned)i);
                        ...
                        if (r == 0) {
                            if (osize > 0) {
                                ...
                                uint64_t upper_size = (osize - 1) / guessed_layout.stripe_unit *
                                                      ...
                                                      (i % guessed_layout.stripe_count) * guessed_layout.stripe_unit +
                    ...
                    if (r != 0 && r != -ENOENT) {
                        derr << "Unexpected error checking size of ino 0x" << std::hex << obj_name_ino << std::dec
                             << ": " << cpp_strerror(r) << dendl;
                        return r;
                    }
                    file_size = complete_objs * guessed_layout.object_size + incomplete_size;
                }
            }
            else {
                file_size = accum_res.ceiling_obj_size;
                if (loaded_layout.pool_id < 0 || loaded_layout.object_size < accum_res.max_obj_size) {
                    // No layout loaded, or inconsistent layout, use default
                    guessed_layout = file_layout_t::get_default();
                    guessed_layout.pool_id = accum_res.obj_pool_id != -1 ? accum_res.obj_pool_id : data_pool_id;
                }
                else {
                    guessed_layout = loaded_layout;
                }
            ...
            if (have_backtrace && backtrace.ino != obj_name_ino) {
                dout(4) << "Backtrace ino 0x" << std::hex << backtrace.ino << " doesn't match object name ino 0x"
                        << obj_name_ino << std::dec << dendl;
            ...
            if (have_backtrace) {
                ...
                if (MDS_INO_IS_MDSDIR(root_bp.dirino)) {
                    ...
                    if (r < 0) {
                        dout(4) << "Error injecting 0x" << std::hex << backtrace.ino << std::dec
                                << " into lost+found: " << cpp_strerror(r) << dendl;
                        if (r == -EINVAL) {
                    ...
                    if (r < 0) {
                        dout(4) << "Error injecting 0x" << std::hex << backtrace.ino << std::dec
                                << " with backtrace: " << cpp_strerror(r) << dendl;
                        if (r == -EINVAL) {
                            dout(4) << "Use --force-corrupt to overwrite structures that "
                                       "appear to be corrupt"
                                    << dendl;
                ...
                r = driver->inject_lost_and_found(obj_name_ino, dentry);
                if (r < 0) {
                    dout(4) << "Error injecting 0x" << std::hex << obj_name_ino << std::dec
                            << " into lost+found: " << cpp_strerror(r) << dendl;
                    if (r == -EINVAL) {
                        dout(4) << "Use --force-corrupt to overwrite structures that "
                                   "appear to be corrupt"
...
{
    return (ino >= inodeno_t((1ull << 40))) || (MDS_INO_IS_STRAY(ino)) || (MDS_INO_IS_MDSDIR(ino)) ||
           ino == CEPH_INO_ROOT || ino == CEPH_INO_CEPH || ino == CEPH_INO_LOST_AND_FOUND;
}
...
int DataScan::scan_links()
{
    ...
    struct link_info_t
    {
        ...
        {}
        dirfrag_t dirfrag() const { return dirfrag_t(dirino, frag); }
    };
    ...
    for (int step = SCAN_INOS; step <= CHECK_LINK; step++) {
        ...
        for (auto it = metadata_io.nobjects_begin(); it != it_end; ++it) {
            ...
            for (auto& p : items) {
                ...
                if (last != CEPH_NOSNAP) {
                    if (last > last_snap) last_snap = last;
                    continue;
                }
                ...
                try {
                    ...
                    if (dnfirst == CEPH_NOSNAP) {
                        ...
                        if (dnfirst - 1 > last_snap) last_snap = dnfirst - 1;
                    }
                    char dentry_type;
                    ...
                    if (dentry_type == 'I' || dentry_type == 'i') {
                        ...
                        if (dentry_type == 'i') {
                            DECODE_START(2, q);
                            if (struct_v >= 2) decode(alternate_name, q);
                            inode.decode(q);
                            DECODE_FINISH(q);
                        }
                        else {
                        ...
                        if (step == SCAN_INOS) {
                            if (used_inos.contains(ino, 1)) {
                                dup_primaries.emplace(
                                    std::piecewise_construct, std::forward_as_tuple(ino), std::forward_as_tuple());
                            }
                            else {
                            ...
                            if (inode.snap_blob.length()) {
                                ...
                                if (!srnode.past_parents.empty()) {
                                    ...
                                    if (last + 1 > snaprealm_v2_since) snaprealm_v2_since = last + 1;
                                }
                            }
                            if (inode.old_inodes && !inode.old_inodes->empty()) {
                                auto _last_snap = inode.old_inodes->rbegin()->first;
                                if (_last_snap > last_snap) last_snap = _last_snap;
                            }
                            ...
                            if (q != dup_primaries.end()) {
                                ...
                                if (r != remote_links.end()) nlink = r->second;
                                if (!MDS_INO_IS_STRAY(dir_ino)) nlink++;
                                if (inode.inode->nlink != nlink) {
                                    derr << "Bad nlink on " << ino << " expected " << nlink << " has "
                                         << inode.inode->nlink << dendl;
                        ...
                        if (step == SCAN_INOS) {
                            ...
                            if (!used_inos.contains(ino, 1)) {
                                derr << "Bad remote link dentry 0x" << std::hex << dir_ino << std::dec << "/" << dname
                        ...
                        derr << "Invalid tag char '" << dentry_type << "' dentry 0x" << dir_ino << std::dec << "/"
                             << dname << dendl;
                        return -EINVAL;
                    }
                }
    ...
    map<unsigned, uint64_t> max_ino_map;
    {
        ...
        for (auto p = used_inos.begin(); p != used_inos.end(); ++p) {
            ...
            if (cur_max < prev_max_ino) continue;   // system inodes
    ...
    dout(10) << "processing " << dup_primaries.size() << " dup_primaries, " << remote_links.size() << " remote_links"
             << dendl;
    ...
    for (auto& p : dup_primaries) {
        ...
        for (auto& q : p.second) {
            ...
            else if (q.version == newest.version && !MDS_INO_IS_STRAY(q.dirino) && MDS_INO_IS_STRAY(newest.dirino)) {
                newest = q;
            }
        }
        ...
        for (auto& q : p.second) {
            ...
            if (newest.dirino == q.dirino && newest.name == q.name) {
                snaps.insert(make_move_iterator(begin(q.snaps)), make_move_iterator(end(q.snaps)));
                continue;
            }
        ...
        if (q != remote_links.end()) nlink = q->second;
        if (!MDS_INO_IS_STRAY(newest.dirino)) nlink++;
...

    {
        objecter->with_osdmap([&](const OSDMap& o) {
            for (auto p : data_pools) {
                ...
                if (!pi) continue;
                if (pi->snap_seq > last_snap) last_snap = pi->snap_seq;
            }
        });

        if (!snaps.empty()) {
            if (snaps.rbegin()->first > last_snap) last_snap = snaps.rbegin()->first;
        }
    }

    dout(10) << "removing dup dentries from " << to_remove.size() << " objects" << dendl;
    ...
    dout(10) << "processing " << bad_nlink_inos.size() << " bad_nlink_inos" << dendl;
    ...
    for (auto& p : bad_nlink_inos) {
        ...
        if (r < 0) {
            derr << "Unexpected error reading dentry " << p.second.dirfrag() << "/" << p.second.name << ": "
                 << cpp_strerror(r) << dendl;
            return r;
        }

        if (inode.inode->ino != p.first || inode.inode->version != p.second.version) continue;
    ...
    dout(10) << "processing " << injected_inos.size() << " injected_inos" << dendl;
    ...
    for (auto& p : max_ino_map) {
        ...
        if (inotable.force_consume_to(p.second)) dirty = true;
        if (dirty) {
...

    {
        ...
        if (snaptable.force_update(last_snap, snaprealm_v2_since, snaps)) dirty = true;
        if (dirty) {
...
int DataScan::scan_frags()
{
    ...
    return forall_objects(
        metadata_io, true, [this](std::string const& oid, uint64_t obj_name_ino, uint64_t obj_name_offset) -> int {
            int r = 0;
            r = parse_oid(oid, &obj_name_ino, &obj_name_offset);
            if (r != 0) {
                dout(4) << "Bad object name '" << oid << "', skipping" << dendl;
            ...
            InodeStore dentry;
            build_dir_dentry(obj_name_ino, fnode.accounted_fragstat, loaded_layout, &dentry);
...
int MetadataTool::read_fnode(inodeno_t ino, frag_t frag, fnode_t* fnode, uint64_t* last_version)
{
...
int MetadataTool::read_dentry(inodeno_t parent_ino, frag_t frag, const std::string& dname, InodeStore* inode,
                              snapid_t* dnfirst)
{
    ...
    try {
        ...
        if (dentry_type == 'I' || dentry_type == 'i') {
            ...
            dout(20) << "dentry type '" << dentry_type
                     << "': cannot"
                        "read an inode out of that"
                     << dendl;
            return -EINVAL;
        }
        if (dnfirst) *dnfirst = first;
    }
    catch (const buffer::error& err) {
...
int MetadataDriver::inject_lost_and_found(inodeno_t ino, const InodeStore& dentry)
{
    ...
    if (r == -ENOENT || r == -EINVAL) {
        ...
        fragstat.nfiles = 1, build_dir_dentry(CEPH_INO_LOST_AND_FOUND, fragstat, inherit_layout, &lf_ino);
...
int MetadataDriver::get_frag_of(inodeno_t dirino, const std::string& target_dname, frag_t* result_ft)
{
    ...
    if (r >= 0) {
        ...
        if (existing_dentry.inode->ino != dirino) {
            dout(4) << "Unexpected inode in dentry! 0x" << std::hex << existing_dentry.inode->ino << " vs expected 0x"
                    << dirino << std::dec << dendl;
            return -ENOENT;
        }
...
int MetadataDriver::inject_with_backtrace(const inode_backtrace_t& backtrace, const InodeStore& dentry)

{
    ...
    for (std::vector<inode_backpointer_t>::const_iterator i = backtrace.ancestors.begin();
        ...
        if (r == -ENOENT) {
            ...
            dout(20) << "don't know fragment for 0x" << std::hex << parent_ino << std::dec << "/" << dname
                     << ", will insert to root" << dendl;
        }
        ...
        if (r == -ENOENT || r == -EINVAL) {
            ...
            derr << "Unexpected error reading dentry 0x" << std::hex << parent_ino << std::dec << "/" << dname << ": "
                 << cpp_strerror(r) << dendl;
            break;
        }
        else {
            ...
            if (existing_dentry.inode->ino == ino) {
                dout(20) << "Dentry 0x" << std::hex << parent_ino << std::dec << "/" << dname
                         << " already exists and points to me" << dendl;
            }
            else {
                derr << "Dentry 0x" << std::hex << parent_ino << std::dec << "/" << dname
                     << " already exists but points to 0x" << std::hex << existing_dentry.inode->ino << std::dec
                     << dendl;
        ...
        if (write_dentry) {
            if (i == backtrace.ancestors.begin()) {
                ...
                dout(10) << "Linking inode 0x" << std::hex << ino << " at 0x" << parent_ino << "/" << dname << std::dec
                ...
                dout(10) << "Linking ancestor directory of inode 0x" << std::hex << ino << " at 0x" << std::hex
                         << parent_ino << ":" << dname << dendl;
...
int MetadataDriver::find_or_create_dirfrag(inodeno_t ino, frag_t fragment, bool* created)
{
    ...
    *created = false;
    ...
    if (r == -ENOENT || r == -EINVAL) {
        ...
        derr << "Unexpected error reading dirfrag 0x" << std::hex << ino << std::dec << " : " << cpp_strerror(r)
             << dendl;
        return r;
    }
    else {
...
int MetadataDriver::inject_linkage(inodeno_t dir_ino, const std::string& dname, const frag_t fragment,
                                   const InodeStore& inode, const snapid_t dnfirst)
{
    ...
    if (r != 0) {
        derr << "Error writing dentry 0x" << std::hex << dir_ino << std::dec << "/" << dname << ": " << cpp_strerror(r)
             << dendl;
        return r;
    }
    else {
        dout(20) << "Injected dentry 0x" << std::hex << dir_ino << "/" << dname << " pointing to 0x" << inode.inode->ino
                 << std::dec << dendl;
        return 0;
    }
}
...
int MetadataDriver::init(librados::Rados& rados, std::string& metadata_pool_name, const FSMap* fsmap,
...
int LocalFileDriver::init(librados::Rados& rados, std::string& metadata_pool_name, const FSMap* fsmap,
                          fs_cluster_id_t fscid)
{
    return 0;
}

int LocalFileDriver::inject_data(const std::string& file_path, uint64_t size, uint32_t chunk_size, inodeno_t ino)
{
    ...
    for (uint64_t offset = 0; offset < size; offset += chunk_size) {
        ...
        char buf[32];
        snprintf(buf, sizeof(buf), "%llx.%08llx", (unsigned long long)ino, (unsigned long long)(offset / chunk_size));
        std::string oid(buf);
...
int LocalFileDriver::inject_with_backtrace(const inode_backtrace_t& bt, const InodeStore& dentry)
{
    ...
    for (i = bt.ancestors.rbegin(); i != bt.ancestors.rend(); ++i) {
        ...
        if (is_file) {
            ...
            inject_data(path_builder, dentry.inode->size, dentry.inode->layout.object_size, bt.ino);
        }
        else {
...
int LocalFileDriver::inject_lost_and_found(inodeno_t ino, const InodeStore& dentry)
{
    ...
    return inject_data(file_path, dentry.inode->size, dentry.inode->layout.object_size, ino);
}
...
void MetadataTool::build_file_dentry(inodeno_t ino, uint64_t file_size, time_t file_mtime, const file_layout_t& layout,
                                     InodeStore* out, std::string symlink)
{
...
void MetadataTool::build_dir_dentry(inodeno_t ino, const frag_info_t& fragstat, const file_layout_t& layout,
                                    InodeStore* out)
{
    ceph_assert(out != NULL);
