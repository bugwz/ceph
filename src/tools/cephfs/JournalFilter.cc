...
bool JournalFilter::apply(uint64_t pos, PurgeItem& pi) const
{
    ...
    if (purge_action != PurgeItem::NONE) {
        if (pi.action != purge_action) return false;
    }

    if (inode) {
        if (inode != pi.ino) return false;
    }
    return true;
}
...
bool JournalFilter::apply(uint64_t pos, LogEvent& le) const
{
    ...
    if (!frag_dentry.empty() || frag.ino) {
        ...
        if (metablob) {
            ...
            if (frag.ino) {
                bool match_any = false;
                for (std::map<dirfrag_t, std::set<std::string> >::iterator i = dentries.begin(); i != dentries.end();
                     ++i) {
                    if (i->first == frag) {
                        match_any = true;
                        break;
            ...
            if (!frag_dentry.empty()) {
                ...
                for (std::map<dirfrag_t, std::set<std::string> >::iterator i = dentries.begin();
                     i != dentries.end() && !match_any;
                     ++i) {
                    std::set<std::string> const& names = i->second;
                    for (std::set<std::string>::iterator j = names.begin(); j != names.end() && !match_any; ++j) {
                        if (*j == frag_dentry) {
                            match_any = true;
                        }
                    }
...
int JournalFilter::parse_args(std::vector<const char*>& argv, std::vector<const char*>::iterator& arg)
{
    while (arg != argv.end()) {
        std::string arg_str;
...
{
    if (!path_expr.empty() || inode != 0 || event_type != 0 || frag.ino != 0 || client_name.num() != 0 ||
