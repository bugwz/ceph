...
void TableTool::usage()
{
    std::cout << "Usage: \n"
              ...
              << "  cephfs-table-tool <all|[mds rank]> <take_inos> <max_ino>" << std::endl;
...
int TableTool::main(std::vector<const char*>& argv)
{
    ...
    if (mode == "reset") {
        ...
        if (table == "session") {
            r = apply_role_fn(
                [this](mds_role_t rank, Formatter* f) -> int {
        ...
        if (table == "session") {
            ...
            {
                r = TableHandler<SnapServer>(mds_role_t(), "snaptable", true).load_and_dump(&io, &jf);
                jf.dump_int("result", r);
            }
            jf.close_section();
        }
        ...
        r = apply_role_fn(
            [this, ino](mds_role_t rank, Formatter* f) -> int { return InoTableHandler(rank).take_inos(&io, ino, f); },
            &jf);
    }
    else {
