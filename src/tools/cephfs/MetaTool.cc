...
int MetaTool::main(string& mode, string& rank_str, string& minfo, string& ino, string& out, string& in, bool confirm)
{
    int r = 0;
    ...
    if (minfo != "") {
        ...
        std::cout << "(" << minfo << ")=>"
                  << " mpool: " << manual_meta_pool << " dpool: " << manual_data_pool << " rank: " << manual_rank_num
                  << std::endl;
        if (!manual_meta_pool.empty() && !manual_data_pool.empty() && !manual_rank_num.empty()) {
            std::cout << "you specify rank: " << manual_rank_num << " mpool: " << manual_meta_pool
    ...
    if (_debug) cout << "MetaTool: connecting to RADOS..." << std::endl;
    r = rados.connect();
    if (r < 0) {
    ...
    if (!manual_mode) {
        ...
        if (_debug) cout << "MetaTool: creating IoCtx.." << std::endl;
...
int MetaTool::op_process(meta_op& op)
{
    int r = 0;
    while (!op.no_sops()) {
        if (_debug) std::cout << "process : " << op.top_op()->detail() << std::endl;
        switch (op.top_op()->sub_op_t) {
        case meta_op::OP_LIST: r = list_meta(op); break;
        case meta_op::OP_LTRACE: r = file_meta(op); break;
        case meta_op::OP_SHOW: r = show_meta(op); break;
        case meta_op::OP_AMEND: r = amend_meta(op); break;
        case meta_op::OP_SHOW_FN: r = show_fn(op); break;
        case meta_op::OP_AMEND_FN: r = amend_fn(op); break;
        default: cerr << "unknow op" << std::endl;
        }
        if (r == 0)
            op.pop_op();
        else if (r < 0)
...
int MetaTool::amend_meta(meta_op& op)
{
    ...
    if (item != op.inodes.end() && item_k != op.okeys.end()) {
        if (_amend_meta(item_k->second, *(item->second), op.infile(), op) < 0) return -1;
    }
    else {
        if (op.inodes.empty()) {
            meta_op::sub_op* nsop = new meta_op::sub_op(&op);
...
int MetaTool::_amend_meta(string& k, inode_meta_t& inode_meta, const string& fn, meta_op& op)
{
    ...
    if (!op.confirm_chg()) {
        cout << "warning: this operation is irreversibl!!!\n"
             ...
             << "         if you want amend it, please add --yes-i-really-really-mean-it!!!" << std::endl;
        return -1;
    }
    ...
    if (!op.top_op()->get_ancestor(bp)) return -1;
    frag_t frag;
...
int MetaTool::show_fn(meta_op& op)
{
    ...
    auto item = op.inodes.find(sop->ino);
    if (item != op.inodes.end()) {
        if (_show_fn(*(item->second), op.outfile()) < 0) return -1;
    }
    else {
        if (op.inodes.empty()) {
            meta_op::sub_op* nsop = new meta_op::sub_op(&op);
    ...
    for (const auto& frag : frags) {
        ...
        }
        {
            ...
            try {
                ...
                cerr << "corrupt fnode header in " << oid << ": " << err.what() << std::endl;
                return -1;
            }
            if (!oids.empty()) oids += ",";
            oids += oid;
...
{
    if (_amend_fn(op.infile(), op.confirm_chg()) < 0) return -1;
    return 0;
}
int MetaTool::_amend_fn(const string& fn, bool confirm)
{
    ...
    try {
        ...
        if (pos1 != tmp.length()) v.push_back(tmp.substr(pos1));
        int ret = 0;
        for (auto i : v) {
...
int MetaTool::show_meta(meta_op& op)
{
    ...
    auto item = op.inodes.find(sop->ino);
    if (item != op.inodes.end()) {
        if (_show_meta(*(item->second), op.outfile()) < 0) return -1;
    }
    else {
        if (op.inodes.empty()) {
            meta_op::sub_op* nsop = new meta_op::sub_op(&op);
...
int MetaTool::list_meta(meta_op& op)
{
    ...
    if (sop->ino_c == 0) {
        ...
        if (_debug) std::cout << __func__ << " : " << sop->trace_level << " " << op.ancestors.size() << std::endl;
        inode_backpointer_t bp;
        ...
        if (list_all) {
            ...
            if (frag == frag_t()) {
                ...
                if (sop->get_c_ancestor(bp)) {
                    ...
                    cerr << __func__ << "can't find obj(" << oid << ") ,miss ancestors or miss some objs??? "
                         << std::endl;
                    return -1;
                }
            }
    ...
    for (map<string, bufferlist>::iterator p = out_vals.begin(); p != out_vals.end(); ++p, --pos) {
        string dname;
        snapid_t last;
        dentry_key_t::decode_helper(p->first, dname, last);
        if (_debug) last_dname = dname;
        try {
            if (!list_all) {
                if (show_child(p->first, dname, last, p->second, pos, snaps, &force_dirty, ino, &op) == 1) {
                    return 0;
                }
            }
            else {
                cout << "dname : " << dname << " " << last << std::endl;
                if (show_child(p->first, dname, last, p->second, pos, snaps, &force_dirty) == 1) return 0;
            }
        }
        catch (const buffer::error& err) {
            derr << "Corrupt dentry '" << dname << "' : " << err.what() << "("
                 << ""
                 << ")" << dendl;
            return -1;
        }
    ...
    if (0 == r) {
        ...
        if (item != attrset.end()) {
            ...
            try {
                ...
                have_data = true;
                if (i_bt.ancestors.size() > 0) op.ancestors[ino] = i_bt.ancestors[0];
        ...
        if (have_data) {
            f->close_section();
            f->flush(ds);
            if (_debug) cout << ino << " : " << ds.str() << std::endl;
            return 1;
        }
    }
    return 0;
...
std::string MetaTool::obj_name(inodeno_t ino, uint64_t offset, const char* suffix) const
{
    char name[60];
    snprintf(
        name, sizeof(name), "%llx.%08llx%s", (long long unsigned)ino, (long long unsigned)offset, suffix ? suffix : "");
...
std::string MetaTool::obj_name(inodeno_t ino, frag_t fg, const char* suffix) const
{
    char name[60];
    snprintf(
        name, sizeof(name), "%llx.%08llx%s", (long long unsigned)ino, (long long unsigned)fg, suffix ? suffix : "");
    return std::string(name);
}
...
int MetaTool::show_child(std::string_view key, std::string_view dname, const snapid_t last, bufferlist& bl,
                         const int pos, const std::set<snapid_t>* snaps, bool* force_dirty, inodeno_t sp_ino,
                         meta_op* op)
{
    ...
    if (_debug)
        std::cout << pos << " type '" << type << "' dname '" << dname << " [" << first << "," << last << "]"
                  << std::endl;
    // bool stale = false;
