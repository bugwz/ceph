...
void PgFiles::hit_dir(std::string const& path)
{
    ...
    while ((r = ceph_readdir_r(cmount, dr, &de)) != 0) {
        ...
        r = ceph_statx(cmount, de_path.c_str(), &stx, CEPH_STATX_INO | CEPH_STATX_SIZE, 0);
        if (r != 0) {
...
void PgFiles::hit_file(std::string const& path, const struct ceph_statx& stx)
{
    ...
    int l_object_size = 0;
    int l_pool_id = 0;
    int r = ceph_get_path_layout(cmount, path.c_str(), &l_stripe_unit, &l_stripe_count, &l_object_size, &l_pool_id);
    if (r != 0) {
    ...
    for (uint64_t i = 0; i < num_objects; ++i) {
        char buf[32];
        snprintf(buf, sizeof(buf), "%llx.%08llx", (long long unsigned)stx.stx_ino, (long long unsigned int)i);
        ...
        int r = 0;
        objecter->with_osdmap([&r, oid, loc, &target, &pg_num_mask, &pg_num](const OSDMap& osd_map) {
