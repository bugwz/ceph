...
int JournalScanner::scan_events()
{
    ...
    dout(10) << std::hex << "Header 0x" << header->trimmed_pos << " 0x" << header->expire_pos << " 0x"
    ...
    for (uint64_t obj_offset = (read_offset / object_size);; obj_offset++) {
        ...
        if (r < 0) {
            ...
            dout(4) << "Read 0x" << std::hex << this_object.length() << std::dec << " bytes from " << oid
                    << " gap=" << gap << dendl;
        ...
        if (gap) {
            ...
            dout(4) << "Searching for sentinel from 0x" << std::hex << read_offset << ", 0x" << read_buf.length()
                    << std::dec << " bytes available" << dendl;
            ...
            do {
                ...
                dout(4) << "Data at 0x" << std::hex << read_offset << " = 0x" << candidate_sentinel << std::dec
                        << dendl;
        ...
        }
        {
            ...
            while (true) {
                ...
                if (start_ptr != read_offset) {
                    derr << "Bad entry start ptr (0x" << std::hex << start_ptr << ") at 0x" << read_offset << std::dec
                         << dendl;
                    gap = true;
                    gap_start = read_offset;
                        ...
                        if (le->get_type() == EVENT_SUBTREEMAP || le->get_type() == EVENT_SUBTREEMAP_TEST) {
                            ...
                            if (sle.expire_pos > read_offset) {
                                errors.insert(std::make_pair(
                                    read_offset,
                                    EventError(-ERANGE, "ESubtreeMap has expire_pos ahead of its own position")));
                            }
                        }
    ...
    dout(4) << "Scanned objects, " << objects_missing.size() << " missing, " << objects_valid.size() << " valid"
            << dendl;
...
{
    return ((!is_mdlog || (pointer_present && pointer_valid)) && header_present && header_valid &&
            ranges_invalid.empty() && objects_missing.empty());
}
...
{
    char name[60];
    snprintf(name, sizeof(name), "%llx.%08llx", (unsigned long long)(ino), (unsigned long long)offset);
...
void JournalScanner::report(std::ostream& out) const
{
    ...
    if (objects_missing.size()) {
        ...
        for (std::vector<uint64_t>::const_iterator om = objects_missing.begin(); om != objects_missing.end(); ++om) {
    ...
    if (ranges_invalid.size()) {
        ...
        for (std::vector<Range>::const_iterator r = ranges_invalid.begin(); r != ranges_invalid.end(); ++r) {
