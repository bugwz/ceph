...
bool JournalTool::can_execute_for_all_ranks(const std::string& mode, const std::string& command)
{
...
int JournalTool::main_header(std::vector<const char*>& argv)
{
    ...
    if (command == std::string("get")) {
        ...
        std::cout << "Updating " << field_name << std::hex << " 0x" << *field << " -> 0x" << new_val << std::dec
                  << std::endl;
        *field = new_val;
...
int JournalTool::main_event(std::vector<const char*>& argv)
{
    ...
    if (output_style != "binary" && output_style != "json" && output_style != "summary" && output_style != "list") {
    ...
    while (arg != argv.end()) {
        ...
        else if (ceph_argparse_witharg(argv, arg, &arg_str, "--alternate-pool", nullptr)) {
    ...
    if (command == "get") {
        ...
        for (JournalScanner::EventMap::iterator i = js.events.begin(); i != js.events.end(); ++i) {
            ...
            if (mb) {
                ...
                if (scav_r) {
                    ...
                    js.errors.insert(std::make_pair(i->first, JournalScanner::EventError(scav_r, cpp_strerror(r))));
                }
            }
        }
        ...
        if (other_pool && !dry_run) {
            ...
            object_t frag_oid;
            frag_oid = InodeStore::get_object_name(CEPH_INO_LOST_AND_FOUND, frag_t(), "");
        ...
        if (filter.get_range(start, end)) {
            ...
            for (JournalScanner::EventMap::iterator i = js.events.begin(); i != js.events.end(); ++i) {
                ...
                if (r) {
                    derr << "Failed to erase event 0x" << std::hex << i->first << std::dec << ": " << cpp_strerror(r)
                         << dendl;
                    return r;
                }
            }
 ...
 */
int JournalTool::recover_dentries(EMetaBlob const& metablob, bool const dry_run, std::set<inodeno_t>* consumed_inos)
{
    ...
    for (const auto& frag : metablob.lump_order) {
        ...
        if (r == -ENOENT) {
            ...
            dout(4) << "failed to read OMAP header from directory fragment " << frag_oid.name << " " << cpp_strerror(r)
                    << dendl;
            write_fnode = true;
            ...
            try {
                ...
                dout(4) << "frag " << frag_oid.name << " fnode old v" << old_fnode.version << " vs new v"
                        << lump.fnode->version << dendl;
            ...
            if (read_vals.find(key) == read_vals.end()) {
                ...
                if (dentry_type == 'L' || dentry_type == 'l') {
                    ...
                    dout(10) << "Existing remote inode in slot to be (maybe) written "
                             << "by a full inode from the journal dn '" << fb.dn.c_str() << "' with lump fnode version "
                             << lump.fnode->version << "vs existing fnode version " << old_fnode_version << dendl;
                    ...
                    dout(4) << "decoded embedded inode version " << inode.inode->version << " vs fullbit version "
            ...
            if (read_vals.find(key) == read_vals.end()) {
                ...
                if (dentry_type == 'L' || dentry_type == 'l') {
                    dout(10) << "Existing hardlink inode in slot to be (maybe) written "
                             ...
                             << "' with lump fnode version " << lump.fnode->version << "vs existing fnode version "
                             << old_fnode_version << dendl;
                    write_dentry = old_fnode_version < lump.fnode->version;
                }
                else if (dentry_type == 'I' || dentry_type == 'i') {
            ...
            if (it != read_vals.end()) {
                ...
                if (dentry_type == 'L' || dentry_type == 'l') {
                    dout(10) << "Existing hardlink inode in slot to be (maybe) removed "
                             << "by null journal dn '" << nb.dn.c_str() << "' with lump fnode version "
                             << lump.fnode->version << "vs existing fnode version " << old_fnode_version << dendl;
                ...
                if (remove_dentry) null_vals.insert(key);
            }
        }
 ...
 */
void JournalTool::encode_fullbit_as_inode(const EMetaBlob::fullbit& fb, const bool bare, bufferlist* out_bl)
{
...
int JournalTool::consume_inos(const std::set<inodeno_t>& inos)
{
    ...
    for (std::set<mds_rank_t>::iterator rank_i = in_ranks.begin(); rank_i != in_ranks.end(); ++rank_i) {
        // Compose object name
        ...
        for (std::set<inodeno_t>::iterator i = inos.begin(); i != inos.end(); ++i) {
        ...
        if (inotable_modified) {
            ...
            if (write_r != 0) {
                derr << "error writing modified inotable " << inotable_oid.name << ": " << cpp_strerror(write_r)
                     << dendl;
                r = r ? r : read_r;
                continue;
            }
