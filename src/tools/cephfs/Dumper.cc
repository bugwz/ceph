...
int Dumper::dump(const char* dump_file)
{
    ...
    Journaler journaler(
        "dumper", ino, fs->mds_map.get_metadata_pool(), CEPH_FS_ONDISK_MAGIC, objecter, 0, 0, &finisher);
    ...
    if (fd >= 0) {
       ...
       object_size %lu (0x%lx)\n    fsid %s\n%c",
                 ...
                 (unsigned long long)len,
                 (unsigned long long)journaler.last_committed.write_pos,
                 (unsigned long long)journaler.last_committed.write_pos,
                 (unsigned long long)journaler.last_committed.stream_format,
                 (unsigned long long)journaler.last_committed.trimmed_pos,
                 (unsigned long long)journaler.last_committed.trimmed_pos,
                 (unsigned long)journaler.last_committed.layout.stripe_unit,
                 (unsigned long)journaler.last_committed.layout.stripe_unit,
                 (unsigned long)journaler.last_committed.layout.stripe_count,
                 (unsigned long)journaler.last_committed.layout.stripe_count,
                 (unsigned long)journaler.last_committed.layout.object_size,
                 (unsigned long)journaler.last_committed.layout.object_size,
                 fsid_str,
                 4);
        ...
        if (seeked == (off64_t)-1) {
            r = errno;
            derr << "Error " << r << " (" << cpp_strerror(r) << ") seeking to 0x" << std::hex << start << std::dec
                 << dendl;
            ::close(fd);
            return r;
        }
        ...
        for (uint64_t pos = start; pos < start + len; pos += chunk_size) {
            ...
            C_SaferCond cond;
            lock.lock();
            filer.read(ino, &journaler.get_layout(), CEPH_NOSNAP, pos, read_size, &bl, 0, &cond);
            lock.unlock();
            r = cond.wait();
            if (r < 0) {
...
int Dumper::undump(const char* dump_file, bool force)
{
    ...
    Journaler journaler(
        "umdumper", ino, fs->mds_map.get_metadata_pool(), CEPH_FS_ONDISK_MAGIC, objecter, 0, 0, &finisher);
    ...
    cout << "start " << start << " len " << len << " write_pos " << write_pos << " format " << format << " trimmed_pos "
         << trimmed_pos << " stripe_unit " << stripe_unit << " stripe_count " << stripe_count << " object_size "
    ...
    lock.lock();
    objecter->write_full(oid, oloc, snapc, hbl, ceph::real_clock::now(), 0, &header_cond);
    lock.unlock();
    ...
    while (left > 0) {
        ...
        lock.lock();
        filer.write(ino, &h.layout, snapc, pos, l, j, ceph::real_clock::now(), 0, &write_cond);
        lock.unlock();
