...
int Resetter::reset()
{
    ...
    Journaler journaler(
        "resetter", ino, fs->mds_map.get_metadata_pool(), CEPH_FS_ONDISK_MAGIC, objecter, 0, 0, &finisher);
    {
    ...
    if (r != 0) {
        if (r == -ENOENT) {
            cerr << "journal does not exist on-disk. Did you set a bad rank?" << std::endl;
            std::cerr << "Error loading journal: " << cpp_strerror(r) << ", pass --force to forcibly reset this journal"
                      << std::endl;
            return r;
        }
        else {
    ...
    cout << "new journal start will be " << new_start << " (" << (new_start - old_end) << " bytes past old end)"
         << std::endl;
...
int Resetter::reset_hard()
{
    ...
    file_layout_t default_log_layout = MDCache::gen_default_log_layout(fsmap->get_filesystem(role.fscid)->mds_map);
