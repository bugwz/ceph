    ...
    if (upmap) {
        ...
        do {
            ...
            for (auto& i : pools) {
                ...
                one_pool.insert(i);
                int did = osdmap.calc_pg_upmaps(g_ceph_context, upmap_deviation, left, one_pool, &pending_inc);
                total_did += did;
                left -= did;
                if (left <= 0) break;
            }
...
skip_upmap:
    ...
    if (!print && !health && !tree && !modified && export_crush.empty() && import_crush.empty() &&
        ...
        !test_map_pgs_dump_all && adjust_crush_weight.empty() && !upmap && !upmap_cleanup) {
