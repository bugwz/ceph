...
void usage()
{
    ...
    cout << "   --createsimple <numosd> [--clobber] [--pg-bits <bitsperosd>] [--pgp-bits <bits>] creates a relatively "
            "generic OSD map with <numosd> devices"
         << std::endl;
    ...
    cout << "   --test-map-pgs [--pool <poolid>] [--pg_num <pg_num>] [--range-first <first> --range-last <last>] map "
            "all pgs"
         << std::endl;
    cout << "   --test-map-pgs-dump [--pool <poolid>] [--range-first <first> --range-last <last>] map all pgs"
         << std::endl;
    cout << "   --test-map-pgs-dump-all [--pool <poolid>] [--range-first <first> --range-last <last>] map all pgs to "
            "osds"
         << std::endl;
    ...
    cout << "   --test-map-object <objectname> [--pool <poolid>] map an object to osds" << std::endl;
    ...
    cout << "   --dump <format>         displays the map in plain text when <format> is 'plain', 'json' if specified "
    ...
    cout << "   --adjust-crush-weight <osdid:weight>[,<osdid:weight>,<...>] change <osdid> CRUSH <weight> (but do not "
            "persist)"
         << std::endl;
...
int main(int argc, const char** argv)
{
    ...
    for_each_substr(adjust_crush_weight, ",", [&](auto osd_to_adjust) {
        ...
        if (pos == osd_to_adjust.npos) {
            cerr << me << ": use ':' as separator of osd id and its weight" << std::endl;
            usage();
        }
    ...
    if (upmap || upmap_cleanup || read) {
        if (upmap_file != "-") {
            ...
            if (upmap_fd < 0) {
                cerr << "error opening " << upmap_file << ": " << cpp_strerror(errno) << std::endl;
                exit(1);
            }
    ...
    if (read) {
        ...
        if (num_changes > 0) {
            ...
            for (auto& [osd, pgs] : prim_pgs_by_osd) {
                cout << " osd." << osd << " | primary affinity: " << tmp_osd_map.get_primary_affinityf(osd)
                     << " | number of prims: " << pgs.size() << "\n";
            }
            cout << " \n";
    ...
    if (upmap) {
        cout << "upmap, max-count " << upmap_max << ", max deviation " << upmap_deviation << std::endl;
        ...
        if (!pools.empty()) {
            cout << " limiting to pools " << upmap_pools << " (" << pools << ")" << std::endl;
        }
        else {
        ...
        do {
            ...
            cout << "pools ";
            for (auto& i : pools) cout << osdmap.get_pool_name(i) << " ";
            cout << std::endl;
            OSDMap::Incremental pending_inc(osdmap.get_epoch() + 1);
            pending_inc.fsid = osdmap.get_fsid();
            int total_did = 0;
            ...
            for (auto& i : pools) {
                ...
                int did =
                    osdmap.calc_pg_upmaps(g_ceph_context, upmap_deviation, left, one_pool, &pending_inc, upmap_p_seed);
                total_did += did;
                left -= did;
                if (left <= 0) break;
            ...
            if (upmap_active) cout << "Time elapsed " << elapsed_time << " secs" << std::endl;
            if (total_did > 0) {
                print_inc_upmaps(pending_inc, upmap_fd, vstart);
                if (save || upmap_active) {
                    int r = osdmap.apply_incremental(pending_inc);
                    ceph_assert(r == 0);
                    if (save) modified = true;
                }
            }
            else {
                cout << "Unable to find further optimization, "
                     << "or distribution is already perfect" << std::endl;
                if (upmap_active) {
                    map<int, set<pg_t>> pgs_by_osd;
                    for (auto& i : osdmap.get_pools()) {
                        if (!upmap_pool_nums.empty() && !upmap_pool_nums.count(i.first)) continue;
                    ...
                    for (auto& i : pgs_by_osd) cout << "osd." << i.first << " pgs " << i.second.size() << std::endl;
                    float elapsed_time =
                        (end.tv_sec - round_start.tv_sec) + 1.0e-9 * (end.tv_nsec - round_start.tv_nsec);
...
skip_upmap:
    ...
    if (!import_crush.empty()) {
        ...
        if (r) {
            cerr << me << ": error reading crush map from " << import_crush << ": " << error << std::endl;
            exit(1);
        }
        ...
        if (cw.get_max_devices() > osdmap.get_max_osd()) {
            cerr << me << ": crushmap max_devices " << cw.get_max_devices() << " > osdmap max_osd "
                 << osdmap.get_max_osd() << std::endl;
            exit(1);
        }
    ...
    if (!test_map_object.empty()) {
        ...
        cout << " object '" << oid << "' -> " << pgid << " -> " << acting << std::endl;
    }
    if (!test_map_pg.empty()) {
        ...
        cout << pgid << " raw (" << raw << ", p" << raw_primary << ") up (" << up << ", p" << up_primary << ") acting ("
             << acting << ", p" << acting_primary << ")" << std::endl;
    }
    if (test_map_pgs || test_map_pgs_dump || test_map_pgs_dump_all) {
        ...
        int max_size = 0;
        if (test_random) srand(getpid());
        auto& pools = osdmap.get_pools();
        for (auto p = pools.begin(); p != pools.end(); ++p) {
            if (pool != -1 && p->first != pool) continue;
            if (pg_num > 0) p->second.set_pg_num(pg_num);

            cout << "pool " << p->first << " pg_num " << p->second.get_pg_num() << std::endl;
            for (unsigned i = 0; i < p->second.get_pg_num(); ++i) {
                ...
                size[osds.size()]++;
                if ((unsigned)max_size < osds.size()) max_size = osds.size();

                if (test_map_pgs_dump) {
                    cout << pgid << "\t" << osds << "\t" << primary << std::endl;
                }
                else if (test_map_pgs_dump_all) {
                    cout << pgid << " raw (" << raw << ", p" << calced_primary << ") up (" << up << ", p" << up_primary
                         << ") acting (" << acting << ", p" << acting_primary << ")" << std::endl;
                }

                for (unsigned i = 0; i < osds.size(); i++) {
                    // cout << " rep " << i << " on " << osds[i] << std::endl;
                    if (osds[i] != CRUSH_ITEM_NONE) count[osds[i]]++;
                }
                if (osds.size() && osds[0] != CRUSH_ITEM_NONE) first_count[osds[0]]++;
                if (primary >= 0) primary_count[primary]++;
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (osdmap.crush->get_item_weight(i) <= 0) continue;
            in++;
            cout << "osd." << i << "\t" << count[i] << "\t" << first_count[i] << "\t" << primary_count[i] << "\t"
                 << osdmap.crush->get_item_weightf(i) << "\t" << osdmap.get_weightf(i) << std::endl;
            total += count[i];
            if (count[i] && (min_osd < 0 || count[i] < count[min_osd])) min_osd = i;
            if (count[i] && (max_osd < 0 || count[i] > count[max_osd])) max_osd = i;
        }
        ...
        cout << " in " << in << std::endl;
        cout << " avg " << avg << " stddev " << dev << " (" << (dev / avg) << "x)"
             << " (expected " << edev << " " << (edev / avg) << "x))" << std::endl;

        if (min_osd >= 0) cout << " min osd." << min_osd << " " << count[min_osd] << std::endl;
        if (max_osd >= 0) cout << " max osd." << max_osd << " " << count[max_osd] << std::endl;

        for (int i = 0; i <= max_size; i++) {
            if (size[i]) cout << "size " << i << "\t" << size[i] << std::endl;
        }
    }
    if (test_crush) {
        int pass = 0;
        while (1) {
            ...
            for (map<int64_t, pg_pool_t>::const_iterator p = osdmap.get_pools().begin(); p != osdmap.get_pools().end();
                 ++p) {
    ...
    if (!print && !health && !tree && !modified && export_crush.empty() && import_crush.empty() &&
        test_map_pg.empty() && test_map_object.empty() && !test_map_pgs && !test_map_pgs_dump &&
        !test_map_pgs_dump_all && adjust_crush_weight.empty() && !upmap && !upmap_cleanup && !read) {
        cerr << me << ": no action specified?" << std::endl;
        usage();
    }

    if (modified) osdmap.inc_epoch();
    ...
    if (modified) {
        ...
        // write it out
        cout << me << ": writing epoch " << osdmap.get_epoch() << " to " << fn << std::endl;
        int r = bl.write_file(fn.c_str());
        if (r) {
            cerr << "osdmaptool: error writing to '" << fn << "': " << cpp_strerror(r) << std::endl;
            return 1;
        }
    }
