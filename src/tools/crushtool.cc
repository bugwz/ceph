...
bool argparse_withargs(std::vector<const char*>& args, std::vector<const char*>::iterator& i, std::ostream& oss,
                       const char* opt, Args*... opts)
{
...
int do_add_bucket(CephContext* cct, const char* me, CrushWrapper& crush, const string& add_name, const string& add_type,
                  const map<string, string>& add_loc)
{
    ...
    if (int r = crush.add_bucket(0, 0, CRUSH_HASH_DEFAULT, type, 0, nullptr, nullptr, &bucketno); r < 0) {
...
int do_move_item(CephContext* cct, const char* me, CrushWrapper& crush, const string& name,
...
int main(int argc, const char** argv)
{
    ...
    std::string infn, srcfn, outfn, add_name, add_type, remove_name, reweight_name, bucket_name;
    ...
    auto cct = global_init(
        NULL, empty_args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_UTILITY, CINIT_FLAG_NO_DEFAULT_CONFIG_FILE);
    ...
    for (std::vector<const char*>::iterator i = args.begin(); i != args.end();) {
        if (ceph_argparse_double_dash(args, i)) {
        ...
        else if (ceph_argparse_witharg(args, i, &val, "--reclassify-bucket", (char*)NULL)) {
            if (i == args.end()) {
        ...
        else if (ceph_argparse_witharg(args, i, &val, "--reclassify-root", (char*)NULL)) {
            if (i == args.end()) {
                cerr << "expecting additional argument" << std::endl;
                return EXIT_FAILURE;
            }
        ...
        else if (ceph_argparse_witharg(args, i, &val, "--set-subtree-class", (char*)NULL)) {
            if (i == args.end()) {
                cerr << "expecting additional argument" << std::endl;
                return EXIT_FAILURE;
            }
        ...
        else if (ceph_argparse_witharg(args, i, &choose_local_tries, err, "--set_choose_local_tries", (char*)NULL)) {
            adjust = true;
        }
        else if (ceph_argparse_witharg(
                     args, i, &choose_local_fallback_tries, err, "--set_choose_local_fallback_tries", (char*)NULL)) {
            adjust = true;
        }
        else if (ceph_argparse_witharg(args, i, &choose_total_tries, err, "--set_choose_total_tries", (char*)NULL)) {
            adjust = true;
        }
        else if (ceph_argparse_witharg(
                     args, i, &chooseleaf_descend_once, err, "--set_chooseleaf_descend_once", (char*)NULL)) {
            adjust = true;
        }
        else if (ceph_argparse_witharg(args, i, &chooseleaf_vary_r, err, "--set_chooseleaf_vary_r", (char*)NULL)) {
            adjust = true;
        }
        else if (ceph_argparse_witharg(args, i, &chooseleaf_stable, err, "--set_chooseleaf_stable", (char*)NULL)) {
            adjust = true;
        }
        else if (ceph_argparse_witharg(args, i, &straw_calc_version, err, "--set_straw_calc_version", (char*)NULL)) {
            adjust = true;
        }
        else if (ceph_argparse_witharg(args, i, &allowed_bucket_algs, err, "--set_allowed_bucket_algs", (char*)NULL)) {
            adjust = true;
        }
        ...
        else if (argparse_withargs(args, i, err, "--add-bucket", &add_name, &add_type)) {
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                return EXIT_FAILURE;
            }
        ...
        else if (argparse_withargs(args, i, err, "--move", &move_name)) {
            if (!err.str().empty()) {
                cerr << err.str() << std::endl;
                return EXIT_FAILURE;
            }
            ...
            cout << "--create-simple-rule:"
                 << " name=" << rule_name << " root=" << rule_root << " type=" << rule_type << " mode=" << rule_mode
                 << std::endl;
            add_rule = true;
        }
            ...
            cout << "--create-replicated-rule:"
                 << " name=" << rule_name << " root=" << rule_root << " type=" << rule_type << std::endl;
            add_rule = true;
        }
    ...
    if (!check && !compile && !decompile && !build && !test && !reweight && !adjust && !tree && !dump && add_item < 0 &&
        !add_bucket && !move_item && !add_rule && !del_rule && full_location < 0 && !bucket_tree && !reclassify &&
        !rebuild_class_roots && compare.empty() &&
    ...
    if ((!build) && (!args.empty())) {
        ...
        if ((args.size() % 3) != 0U) {
            ...
            cerr << "layers must be specified with 3-tuples of (name, buckettype, size)" << std::endl;
            return EXIT_FAILURE;
        }
    ...
    */
    ...
    if (!infn.empty()) {
        ...
        if (infn == "-") {
            ...
            if (r < 0) {
                cerr << me << ": error reading '" << infn << "': " << error << std::endl;
                return EXIT_FAILURE;
            }
        }
        auto p = bl.cbegin();
    ...
    if (compile) {
        ...
        if (unsafe_tunables) cc.enable_unsafe_tunables();
    ...
    if (build) {
        ...
        for (vector<layer_t>::iterator p = layers.begin(); p != layers.end(); ++p, type++) {
            ...
            dout(2) << "layer " << type << "  " << l.name << "  bucket type " << l.buckettype << "  " << l.size
                    << dendl;
            ...
            int i = 0;
            while (1) {
                if (lower_pos == lower_items.size()) break;
        ...
        string root = layers.back().size == 0 ? layers.back().name : string(layers.back().name) + "0";
        ...
        if (OSDMap::build_simple_crush_rules(g_ceph_context, crush, root, &cerr)) return EXIT_FAILURE;
    ...
    if (add_rule) {
        ...
        int r = crush.add_simple_rule(
            rule_name, rule_root, rule_type, rule_device_class, rule_mode, pg_pool_t::TYPE_REPLICATED, &err);
        if (r < 0) {
            cerr << err.str() << std::endl;
            return EXIT_FAILURE;
        }
    ...
    if (reclassify) {
        int r = crush.reclassify(g_ceph_context, cout, reclassify_root, reclassify_bucket);
        if (r < 0) {
    ...
    if (test) {
        if (tester.get_output_utilization_all() || tester.get_output_utilization()) tester.set_output_statistics(true);
    ...
    if (compare.size()) {
        ...
        if (r < 0) {
            cerr << me << ": error reading '" << compare << "': " << error << std::endl;
            return EXIT_FAILURE;
        }
        auto p = in.cbegin();
        try {
    ...
    if (modified) {
        ...
        if (outfn.empty()) {
            ...
            if (verbose) cout << "wrote crush map to " << outfn << std::endl;
        }
    }
