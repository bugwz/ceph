...
int main(int argc, const char* argv[])
{
    ...
    for (size_t i = 0; i < args.size(); i++) argv[i + 1] = args[i];
    argc = args.size() + 1;
    ...
    if (cmd == "destructive-repair") {
        ...
        if (!ret) {
            std::cout << "destructive-repair completed without reporting an error" << std::endl;
        }
        else {
            std::cout << "destructive-repair failed with " << cpp_strerror(ret) << std::endl;
        }
        return ret;
    }
    else if (cmd == "list" || cmd == "list-crc") {
        string prefix;
        if (argc > 4) prefix = url_unescape(argv[4]);
        ...
        if (argc > 5) key = url_unescape(argv[5]);

        bool ret = st.exists(prefix, key);
        std::cout << "(" << url_escape(prefix) << ", " << url_escape(key) << ") " << (ret ? "exists" : "does not exist")
                  << std::endl;
        return (ret ? 0 : 1);
    }
    else if (cmd == "get") {
        ...
        if (argc >= 7) {
            ...
            if (err < 0) {
                std::cerr << "error writing value to '" << out << "': " << cpp_strerror(err) << std::endl;
                return 1;
            }
        }
        else {
        ...
        bufferlist bl = st.get(prefix, key, exists);
        if (!exists) {
            std::cerr << "(" << url_escape(prefix) << "," << url_escape(key) << ") does not exist" << std::endl;
            return 1;
        }
        std::cout << "(" << url_escape(prefix) << "," << url_escape(key) << ") size " << byte_u_t(bl.length())
                  << std::endl;
    }
    else if (cmd == "set") {
        if (argc < 8) {
        ...
        if (!ret) {
            std::cerr << "error setting (" << url_escape(prefix) << "," << url_escape(key) << ")" << std::endl;
            return 1;
        }
    }
    else if (cmd == "rm") {
        ...
        if (!ret) {
            std::cerr << "error removing (" << url_escape(prefix) << "," << url_escape(key) << ")" << std::endl;
            return 1;
        }
    }
        ...
        if (!ret) {
            std::cerr << "error removing prefix (" << url_escape(prefix) << ")" << std::endl;
            return 1;
        }
    }
        ...
        if (ret < 0) {
            std::cerr << "error copying store to path '" << argv[4] << "': " << cpp_strerror(ret) << std::endl;
            return 1;
        }
    }
