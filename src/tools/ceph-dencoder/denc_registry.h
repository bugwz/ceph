...
struct Dencoder
{
    ...
    virtual void copy() { std::cerr << "copy operator= not supported" << std::endl; }
    virtual void copy_ctor() { std::cerr << "copy ctor not supported" << std::endl; }
...
template<class T> class DencoderBase : public Dencoder
{
protected:
    T* m_object;
    list<T*> m_list;
...
public:
    ...
    {}
    ~DencoderBase() override { delete m_object; }
    ...
    void dump(ceph::Formatter* f) override { m_object->dump(f); }
    void generate() override { T::generate_test_instances(m_list); }
    int num_generated() override { return m_list.size(); }
    string select_generated(unsigned i) override
    {
        // allow 0- or 1-based (by wrapping)
        if (i == 0) i = m_list.size();
        if ((i == 0) || (i > m_list.size())) return "invalid id for generated object";
        m_object = *(std::next(m_list.begin(), i - 1));
        return string();
    }

    bool is_deterministic() override { return !nondeterministic; }
};

template<class T> class DencoderImplNoFeatureNoCopy : public DencoderBase<T>
{
public:
...
template<class T> class DencoderImplNoFeature : public DencoderImplNoFeatureNoCopy<T>
{
public:
...
template<class T> class DencoderImplFeaturefulNoCopy : public DencoderBase<T>
{
public:
...
template<class T> class DencoderImplFeatureful : public DencoderImplFeaturefulNoCopy<T>
{
public:
...
template<class T> class MessageDencoderImpl : public Dencoder
{
    ref_t<T> m_object;
    list<ref_t<T>> m_list;
...
public:
    ...
    string decode(bufferlist bl, uint64_t seek) override
    {
        ...
        try {
            ...
            if (!n) throw std::runtime_error("failed to decode");
    ...
    bool is_deterministic() override { return true; }
...
public:
    dencoders_t& get() { return dencoders; }
    void register_dencoder(std::string_view name, Dencoder* denc) { dencoders.emplace(name, denc); }
