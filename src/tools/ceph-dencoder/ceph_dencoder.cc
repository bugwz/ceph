...
vector<DencoderPlugin> load_plugins()
{
    ...
    if (!fs::is_directory(mod_dir)) {
        std::cerr << "unable to load dencoders from " << std::quoted(mod_dir.native()) << ". "
    ...
    for (auto& entry : fs::directory_iterator(mod_dir)) {
        ...
        if (entry.path().stem().string().compare(0, DENC_MOD_PREFIX.size(), DENC_MOD_PREFIX) != 0) {
            continue;
        }
...
int main(int argc, const char** argv)
{
    ...
    for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ++i) {
        ...
        if (*i == string("help") || *i == string("-h") || *i == string("--help")) {
            ...
            for (auto& dencoder : dencoders) cout << dencoder.first << std::endl;
            return 0;
        }
