...
public:
    ...
    const dencoders_t& register_dencoders()
    {
        ...
        const auto do_register = reinterpret_cast<register_dencoders_t>(dlsym(mod, REGISTER_DENCODERS_FUNCTION.data()));
        if (do_register == nullptr) {
            std::cerr << "failed to dlsym(" << REGISTER_DENCODERS_FUNCTION << "): " << dlerror() << std::endl;
            return dencoders;
        }
        do_register(this);
        return dencoders;
    ...
    bool good() const { return mod != nullptr; }
    ...
    template<typename DencoderT, typename... Args> void emplace(const char* name, Args&&... args)
    {
