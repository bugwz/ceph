...
StoreTool::StoreTool(const string& type, const string& path, bool to_repair, bool need_stats)
    : store_path(path)
{
    ...
    if (type == "bluestore-kv") {
        ...
        if (load_bluestore(path, to_repair) != 0) exit(1);
#else
        ...
        if (!to_repair) {
            if (int r = db_ptr->open(std::cerr); r < 0) {
                cerr << "failed to open type " << type << " path " << path << ": " << cpp_strerror(r) << std::endl;
                exit(1);
            }
        }
        db.reset(db_ptr);
...
uint32_t StoreTool::traverse(const string& prefix, const bool do_crc, const bool do_value_dump, ostream* out)
{
    ...
    while (iter->valid()) {
        ...
        if (!prefix.empty() && (rk.first != prefix)) break;

        if (out) *out << url_escape(rk.first) << "\t" << url_escape(rk.second);
        if (do_crc) {
            bufferlist bl;
            bl.append(rk.first);
            bl.append(rk.second);
...
void StoreTool::print_summary(const uint64_t total_keys, const uint64_t total_size, const uint64_t total_txs,
                              const string& store_path, const string& other_path, const int duration) const
{
    ...
    std::cout << "  from '" << store_path << "' to '" << other_path << "'" << std::endl;
...
int StoreTool::copy_store_to(const string& type, const string& other_path, const int num_keys_per_tx,
                             const string& other_type)
{
    ...
    do {
        ...
        if (num_keys > 0) other->submit_transaction_sync(tx);

        std::cout << "ts = " << duration() << "s, copied " << total_keys << " keys so far (" << byte_u_t(total_size)
                  << ")" << std::endl;

    } while (it->valid());

    print_summary(total_keys, total_size, total_txs, store_path, other_path, duration());
...
void StoreTool::compact_range(const string& prefix, const string& start, const string& end)
{
