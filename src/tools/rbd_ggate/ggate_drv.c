...
int ggate_drv_req_cmd(ggate_drv_req_t req)
{
    ...
    case BIO_WRITE: return GGATE_DRV_CMD_WRITE;
    case BIO_READ: return GGATE_DRV_CMD_READ;
    case BIO_FLUSH: return GGATE_DRV_CMD_FLUSH;
    case BIO_DELETE: return GGATE_DRV_CMD_DISCARD;
    default: return GGATE_DRV_CMD_UNKNOWN;
    }
}
...
int ggate_drv_create(char* name, size_t namelen, size_t sectorsize, size_t mediasize, bool readonly, const char* info,
                     ggate_drv_t* drv_)
{
    ...
    debug(20, "%s: created, unit: %d, name: %s", __func__, ggiocreate.gctl_unit, ggiocreate.gctl_name);
...
void ggate_drv_destroy(ggate_drv_t drv_)
{
    ...
    if (r == -1) {
        err("failed to destroy /dev/%s%d device", G_GATE_PROVIDER_NAME, drv->unit);
    }
    // Restore errno.
    errno = rerrno;
...
static const char* get_conf(struct ggeom* gp, const char* name)
{
    ...
    {
        if (strcmp(conf->lg_name, name) == 0) return (conf->lg_val);
    }
    return "";
}
