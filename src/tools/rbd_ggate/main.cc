...
static int do_map(int argc, const char* argv[])
{
    ...
    std::string devname = boost::starts_with(devpath, "/dev/") ? devpath.substr(5) : devpath;
    ...
    r = rados.connect();
    if (r < 0) {
        std::cerr << "rbd-ggate: failed to connect to cluster: " << cpp_strerror(r) << std::endl;
        goto done;
    }

    r = rados.ioctx_create(poolname.c_str(), io_ctx);
    if (r < 0) {
        std::cerr << "rbd-ggate: failed to acces pool " << poolname << ": " << cpp_strerror(r) << std::endl;
        goto done;
    }
    ...
    if (r < 0) {
        std::cerr << "rbd-ggate: failed to open image " << imgname << ": " << cpp_strerror(r) << std::endl;
        goto done;
    }
    ...
    if (exclusive) {
        ...
        if (r < 0) {
            std::cerr << "rbd-ggate: failed to acquire exclusive lock: " << cpp_strerror(r) << std::endl;
            goto done;
        }
    }

    desc = "RBD " + poolname + "/" + (nsname.empty() ? "" : nsname + "/") + imgname;

    if (!snapname.empty()) {
        r = image.snap_set(snapname.c_str());
        if (r < 0) {
            std::cerr << "rbd-ggate: failed to set snapshot " << snapname << ": " << cpp_strerror(r) << std::endl;
            goto done;
        }
        readonly = true;
        desc += "@" + snapname;
    ...
    if (r < 0) {
        std::cerr << "rbd-ggate: image stat failed: " << cpp_strerror(r) << std::endl;
        goto done;
    }
    ...
    if (r < 0) {
        r = -errno;
        std::cerr << "rbd-ggate: failed to create ggate device: " << cpp_strerror(r) << std::endl;
        goto done;
    }
    ...
    if (r < 0) {
        std::cerr << "rbd-ggate: failed to set watcher: " << cpp_strerror(r) << std::endl;
        drv->shut_down();
        goto done;
    }
...
static int do_unmap()
{
    ...
    if (r < 0) {
        cerr << "rbd-ggate: failed to destroy " << devname << ": " << cpp_strerror(r) << std::endl;
        return r;
    }
...
static int parse_imgpath(const std::string& imgpath, std::string* poolname, std::string* nsname, std::string* imgname,
                         std::string* snapname)
{
    std::regex pattern("^(?:([^/]+)/(?:([^/@]+)/)?)?([^@]+)(?:@([^/@]+))?$");
    std::smatch match;
...
static bool find_mapped_dev_by_spec(const std::string& spec, std::string* devname)
{
...
static int do_list(const std::string& format, bool pretty_format)
{
    ...
    for (auto& it : devs) {
        ...
        if (f) {
            ...
            tbl << id << poolname << nsname << imgname << snapname << "/dev/" + name << TextTable::endrow;
        }
        count++;
    }
...
int main(int argc, const char* argv[])
{
    ...
    case Connect:
        ...
        if (parse_imgpath(*args.begin(), &poolname, &nsname, &imgname, &snapname) < 0) {
            return EXIT_FAILURE;
        }
        args.erase(args.begin());
        break;
    case Disconnect:
        if (args.begin() == args.end()) {
            std::cerr << "rbd-ggate: must specify ggate device or image-or-snap-spec" << std::endl;
            return EXIT_FAILURE;
        }
        if (boost::starts_with(*args.begin(), "/dev/") || !find_mapped_dev_by_spec(*args.begin(), &devpath)) {
            devpath = *args.begin();
        }
        args.erase(args.begin());
        break;
