...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::ggate::Server: " << this << " " << __func__ << ": "
...
void Server::handle_aio(IOContext* ctx, int r)
{
    ...
    else if ((ctx->req->get_cmd() == Request::Read) && r != static_cast<int>(ctx->req->get_length())) {
...
void Server::reader_entry()
{
    ...
    while (!m_stopping) {
        ...
        case rbd::ggate::Request::Write:
            m_image.aio_write(pctx->req->get_offset(), pctx->req->get_length(), pctx->req->bl, c);
            break;
        case rbd::ggate::Request::Read:
            m_image.aio_read(pctx->req->get_offset(), pctx->req->get_length(), pctx->req->bl, c);
            break;
        case rbd::ggate::Request::Flush: m_image.aio_flush(c); break;
...
std::ostream& operator<<(std::ostream& os, const Server::IOContext& ctx)
{
    ...
    case rbd::ggate::Request::Write: os << " Write "; break;
    case rbd::ggate::Request::Read: os << " Read "; break;
    case rbd::ggate::Request::Flush: os << " Flush "; break;
    case rbd::ggate::Request::Discard: os << " Discard "; break;
    default: os << " Unknow(" << ctx.req->get_cmd() << ") "; break;
    }

    os << ctx.req->get_offset() << "~" << ctx.req->get_length() << " " << ctx.req->get_error() << "]";
