...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::ggate::Driver: " << this << " " << __func__ << ": "
...
int Driver::list(std::map<std::string, DevInfo>* devices)
{
    ...
    while (size <= 1024 * 1024) {
        devs = static_cast<ggate_drv_info*>(realloc(static_cast<void*>(devs), size * sizeof(*devs)));
...
Driver::Driver(const std::string& devname, size_t sectorsize, size_t mediasize, bool readonly, const std::string& info)
    : m_devname(devname)
...
int Driver::init()
{
    ...
    int r = ggate_drv_create(name, namelen, m_sectorsize, m_mediasize, m_readonly, m_info.c_str(), &m_drv);
    if (r < 0) {
        return r;
    }
...
int Driver::recv(Request** req)
{
    ...
    *req = new Request(req_);
    ...
    if (ggate_drv_req_cmd(req_) == GGATE_DRV_CMD_WRITE) {
        bufferptr ptr(
            buffer::claim_malloc(ggate_drv_req_length(req_), static_cast<char*>(ggate_drv_req_release_buf(req_))));
...
int Driver::send(Request* req)
{
    ...
    if (ggate_drv_req_cmd(req->req) == GGATE_DRV_CMD_READ && ggate_drv_req_error(req->req) == 0) {
        ceph_assert(req->bl.length() == ggate_drv_req_length(req->req));
        // TODO: avoid copying?
        req->bl.begin().copy(ggate_drv_req_length(req->req), static_cast<char*>(ggate_drv_req_buf(req->req)));
