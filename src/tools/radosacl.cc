...
struct ACLID
{
    ...
    void encode(bufferlist& bl) const { bl.append((const char*)id, ID_SIZE); }
    void decode(bufferlist::const_iterator& iter) { iter.copy(ID_SIZE, (char*)id); }
};
...
int ObjectACLs::read_acl(ACLID& id, ACLFlags* flags)
{
    ...
    if (iter == acls_map.end()) return -ENOENT;
...
int main(int argc, const char** argv)
{
    ...
    cout << "exec(acl get) returned " << r << " len=" << bl2.length() << std::endl;
    ObjectACLs oa;
    if (r >= 0) {
    ...
    cout << "exec(acl set) returned " << r << " len=" << bl2.length() << std::endl;
