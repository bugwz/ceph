...
struct feature_op_t
{
    ...
    void set_optional() { type = OPTIONAL; }
    void set_persistent() { type = PERSISTENT; }
    bool parse_value(string& s, ostream* errout = NULL)
    {
        ...
        if (!interr.empty()) {
            if (errout) {
                *errout << "unknown features name '" << s << "' or unable to parse value: " << interr << std::endl;
            }
            return false;
        }
...
bool handle_features(list<feature_op_t>& lst, MonMap& m)
{
    ...
    for (auto& f : lst) {
        ...
        else if (f.op == feature_op_t::op_t::OP_SET || f.op == feature_op_t::op_t::OP_UNSET) {

            modified = true;

            mon_feature_t& target =
                (f.type == feature_op_t::type_t::OPTIONAL ? m.optional_features : m.persistent_features);
...
int main(int argc, const char** argv)
{
    ...
    for (std::vector<const char*>::iterator i = args.begin(); i != args.end();) {
        if (ceph_argparse_double_dash(args, i)) {
        ...
        else if (ceph_argparse_witharg(args, i, &val, "--set-min-mon-release", (char*)NULL)) {
            min_mon_release = ceph_release_from_name(val);
        }
        else if (ceph_argparse_flag(args, i, "--add", (char*)NULL)) {
            string name = *i;
            i = args.erase(i);
            if (i == args.end()) helpful_exit();
            entity_addr_t addr;
            ...
            feature_op_t f(feature_op_t::op_t::OP_LIST, feature_op_t::type_t::PLAIN);
        ...
        else if (ceph_argparse_witharg(args, i, &val, "--feature-set", (char*)NULL)) {
            // parse value
        ...
        else if (ceph_argparse_witharg(args, i, &val, "--feature-unset", (char*)NULL)) {
            // parse value
    ...
    if (create) {
        ...
        monmap.strategy =
            static_cast<MonMap::election_strategy>(g_conf().get_val<uint64_t>("mon_election_default_strategy"));
    ...
    if (filter) {
        ...
        if (!initial_members.empty()) {
            ...
            monmap.set_initial_members(g_ceph_context, initial_members, string(), entity_addrvec_t(), &removed);
    ...
    if (print) monmap.print(cout);

    if (modified) {
        // write it out
        cout << me << ": writing epoch " << monmap.epoch << " to " << fn << " (" << monmap.size() << " monitors)"
             << std::endl;
