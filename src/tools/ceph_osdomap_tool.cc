...
int main(int argc, char** argv)
{
    ...
    desc.add_options()("help", "produce help message")(
        "omap-path", po::value<string>(&store_path), "path to omap directory, mandatory (current/omap usually)")(
        "paranoid", "use paranoid checking")("debug", "Additional debug output from DBObjectMap")(
        "oid", po::value<string>(&oid), "Restrict to this object id when dumping objects")(
        "command",
        po::value<string>(&cmd),
        "command arg is one of [dump-raw-keys, dump-raw-key-vals, dump-objects, dump-objects-with-keys, check, "
        "dump-headers, repair, compact], mandatory")(
        "backend", po::value<string>(&backend), "DB backend (default rocksdb)");
    ...
    auto cct = global_init(NULL, ceph_options, CEPH_ENTITY_TYPE_OSD, CODE_ENVIRONMENT_UTILITY_NODOUT, 0);
    ...
    if (cmd == "dump-raw-keys") {
        ...
        for (vector<ghobject_t>::iterator i = objects.begin(); i != objects.end(); ++i) {
            if (vm.count("oid") != 0 && i->hobj.oid.name != oid) continue;
            std::cout << *i << std::endl;
        }
        return 0;
    }
        ...
        for (auto i : headers) std::cout << i << std::endl;
        return 0;
    }
    else if (cmd == "resetv2") {
        omap.state.v = 2;
