...
{
public:
    FSMirror(CephContext* cct, const Filesystem& filesystem, uint64_t pool_id, ServiceDaemon* service_daemon,
    ...
    {
        std::scoped_lock locker(m_lock);
        return m_init_failed || m_instance_watcher->is_failed() || m_mirror_watcher->is_failed();
    }
...
private:
    ...
    struct SnapListener : public InstanceWatcher::Listener
    {
        ...
        void acquire_directory(std::string_view dir_path) override { fs_mirror->handle_acquire_directory(dir_path); }

        void release_directory(std::string_view dir_path) override { fs_mirror->handle_release_directory(dir_path); }
    };
