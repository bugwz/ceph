...
int connect(std::string_view client_name, std::string_view cluster_name, RadosRef* cluster, std::string_view mon_host,
            std::string_view cephx_key, std::vector<const char*> args)
{
    dout(20) << ": connecting to cluster=" << cluster_name << ", client=" << client_name << ", mon_host=" << mon_host
             << dendl;
...
int mount(RadosRef cluster, const Filesystem& filesystem, bool cross_check_fscid, MountRef* mount)
{
    ...
    auto mount_timeout = g_ceph_context->_conf.get_val<std::chrono::seconds>("cephfs_mirror_mount_timeout").count();
