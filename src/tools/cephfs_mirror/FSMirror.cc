...
public:
        ...
        : admin_socket(cct->get_admin_socket())
    {
        ...
        r = admin_socket->register_command(cmd, this, "get filesystem mirror status");
        if (r == 0) {
...
FSMirror::FSMirror(CephContext* cct, const Filesystem& filesystem, uint64_t pool_id, ServiceDaemon* service_daemon,
...
{
    m_service_daemon->add_or_update_fs_attribute(m_filesystem.fscid, SERVICE_DAEMON_DIR_COUNT_KEY, (uint64_t)0);
}
...
void FSMirror::init(Context* on_finish)
{
    ...
    std::scoped_lock locker(m_lock);
    int r = connect(g_ceph_context->_conf->name.to_str(), g_ceph_context->_conf->cluster, &m_cluster, "", "", m_args);
    if (r < 0) {
        m_init_failed = true;
...
void FSMirror::init_instance_watcher(Context* on_finish)
{
    ...
    Context* ctx = new C_CallbackAdapter<FSMirror, &FSMirror::handle_init_instance_watcher>(this);
...
void FSMirror::init_mirror_watcher()
{
    ...
    std::scoped_lock locker(m_lock);
    Context* ctx = new C_CallbackAdapter<FSMirror, &FSMirror::handle_init_mirror_watcher>(this);
...
void FSMirror::shutdown_mirror_watcher()
{
    ...
    std::scoped_lock locker(m_lock);
    Context* ctx = new C_CallbackAdapter<FSMirror, &FSMirror::handle_shutdown_mirror_watcher>(this);
...
void FSMirror::shutdown_instance_watcher()
{
    ...
    std::scoped_lock locker(m_lock);
    Context* ctx = new C_CallbackAdapter<FSMirror, &FSMirror::handle_shutdown_instance_watcher>(this);
...
void FSMirror::handle_acquire_directory(string_view dir_path)
{
...

    {
        ...
        m_service_daemon->add_or_update_fs_attribute(
            m_filesystem.fscid, SERVICE_DAEMON_DIR_COUNT_KEY, m_directories.size());
...
void FSMirror::add_peer(const Peer& peer)
{
    ...
    if (r < 0) {
        m_service_daemon->add_or_update_peer_attribute(
            m_filesystem.fscid, peer, SERVICE_DAEMON_PEER_INIT_FAILED_KEY, true);
        return;
    }
