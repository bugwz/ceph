...
ClusterWatcher::ClusterWatcher(CephContext* cct, MonClient* monc, ServiceDaemon* service_daemon, Listener& listener)
    : Dispatcher(cct)
    , m_monc(monc)
...
void ClusterWatcher::handle_fsmap(const cref_t<MFSMap>& m)
{
    ...
    std::map<Filesystem, uint64_t> fs_metadata_pools;
    {
        ...
        for (auto& filesystem : filesystems) {
            auto fs = Filesystem{filesystem->fscid, std::string(filesystem->mds_map.get_fs_name())};
            ...
            if (!mirror_info.is_mirrored()) {
                ...
                // peers added
                Peers added;
                std::set_difference(mirror_info.peers.begin(),
                                    mirror_info.peers.end(),
                                    peers.begin(),
                                    peers.end(),
                ...
                // peers removed
                Peers removed;
                std::set_difference(peers.begin(),
                                    peers.end(),
                                    mirror_info.peers.begin(),
                                    mirror_info.peers.end(),
