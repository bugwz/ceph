...
MirrorWatcher::MirrorWatcher(librados::IoCtx& ioctx, FSMirror* fs_mirror, ContextWQ* work_queue)
...
void MirrorWatcher::handle_notify(uint64_t notify_id, uint64_t handle, uint64_t notifier_id, bufferlist& bl)
{
    dout(20) << dendl;
...
void MirrorWatcher::register_watcher()
{
    ...
    std::scoped_lock locker(m_lock);
    Context* on_finish = new C_CallbackAdapter<MirrorWatcher, &MirrorWatcher::handle_register_watcher>(this);
...
void MirrorWatcher::unregister_watcher()
{
    ...
    std::scoped_lock locker(m_lock);
    Context* on_finish = new C_CallbackAdapter<MirrorWatcher, &MirrorWatcher::handle_unregister_watcher>(this);
