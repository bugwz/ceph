...
InstanceWatcher::InstanceWatcher(librados::IoCtx& ioctx, Listener& listener, ContextWQ* work_queue)
...
void InstanceWatcher::register_watcher()
{
    ...
    std::scoped_lock locker(m_lock);
    Context* on_finish = new C_CallbackAdapter<InstanceWatcher, &InstanceWatcher::handle_register_watcher>(this);
...
void InstanceWatcher::unregister_watcher()
{
    ...
    std::scoped_lock locker(m_lock);
    Context* on_finish = new C_CallbackAdapter<InstanceWatcher, &InstanceWatcher::handle_unregister_watcher>(this);
