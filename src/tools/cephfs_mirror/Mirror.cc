...
struct Mirror::C_EnableMirroring : Context
{
    ...
    void finish(int r) override { enable_mirroring(); }

    void enable_mirroring()
    {
        Context* ctx = new C_CallbackAdapter<C_EnableMirroring, &C_EnableMirroring::handle_enable_mirroring>(this);
...
struct Mirror::C_DisableMirroring : Context
{
    ...
    void finish(int r) override { disable_mirroring(); }

    void disable_mirroring()
    {
        Context* ctx = new C_CallbackAdapter<C_DisableMirroring, &C_DisableMirroring::handle_disable_mirroring>(this);
...
struct Mirror::C_PeerUpdate : Context
{
    ...
    C_PeerUpdate(Mirror* mirror, const Filesystem& filesystem, const Peer& peer)
        ...
        , peer(peer)
    {}
    C_PeerUpdate(Mirror* mirror, const Filesystem& filesystem, const Peer& peer, bool remove)
        : mirror(mirror)
        , filesystem(filesystem)
        , peer(peer)
        , remove(remove)
...
struct Mirror::C_RestartMirroring : Context
{
    ...
    C_RestartMirroring(Mirror* mirror, const Filesystem& filesystem, uint64_t pool_id, const Peers& peers)
        : mirror(mirror)
        , filesystem(filesystem)
        , pool_id(pool_id)
        , peers(peers)
    ...
    void disable_mirroring()
    {
        Context* ctx = new C_CallbackAdapter<C_RestartMirroring, &C_RestartMirroring::handle_disable_mirroring>(this);
        mirror->disable_mirroring(filesystem, ctx);
    }

    void handle_disable_mirroring(int r) { enable_mirroring(); }

    void enable_mirroring()
    {
        std::scoped_lock locker(mirror->m_lock);
        Context* ctx = new C_CallbackAdapter<C_RestartMirroring, &C_RestartMirroring::handle_enable_mirroring>(this);
...
Mirror::Mirror(CephContext* cct, const std::vector<const char*>& args, MonClient* monc, Messenger* msgr)
    : m_cct(cct)
    , m_args(args)
    , m_monc(monc)
    , m_msgr(msgr)
...
{
    auto thread_pool =
        &(cct->lookup_or_create_singleton_object<ThreadPoolSingleton>("cephfs::mirror::thread_pool", false, cct));
    auto safe_timer =
        &(cct->lookup_or_create_singleton_object<SafeTimerSingleton>("cephfs::mirror::safe_timer", false, cct));
...
int Mirror::init_mon_client()
{
    ...
    r = m_monc->authenticate(
        std::chrono::duration<double>(m_cct->_conf.get_val<std::chrono::seconds>("client_mount_timeout")).count());
    if (r < 0) {
...
void Mirror::handle_signal(int signum)
{
    ...
    std::scoped_lock locker(m_lock);
    switch (signum) {
    case SIGHUP: reopen_logs(); break;
    case SIGINT:
    case SIGTERM: shutdown(); break;
    default: ceph_abort_msgf("unexpected signal %d", signum);
    }
}

void Mirror::handle_enable_mirroring(const Filesystem& filesystem, const Peers& peers, int r)
{
    ...
    if (r < 0) {
        ...
        m_service_daemon->add_or_update_fs_attribute(filesystem.fscid, SERVICE_DAEMON_MIRROR_ENABLE_FAILED_KEY, true);
        return;
    }
...
void Mirror::enable_mirroring(const Filesystem& filesystem, uint64_t local_pool_id, Context* on_finish, bool is_restart)
{
    ...
    mirror_action.fs_mirror =
        std::make_unique<FSMirror>(m_cct, filesystem, local_pool_id, m_service_daemon.get(), m_args, m_work_queue);
...
void Mirror::update_fs_mirrors()
{
    ...
    double blocklist_interval =
        g_ceph_context->_conf.get_val<std::chrono::seconds>("cephfs_mirror_restart_mirror_on_blocklist_interval")
            .count();
    bool check_blocklist = blocklist_interval > 0 && ((now - m_last_blocklist_check) >= blocklist_interval);

    double failed_interval =
        g_ceph_context->_conf.get_val<std::chrono::seconds>("cephfs_mirror_restart_mirror_on_failure_interval").count();
...

    {
        ...
        for (auto& [filesystem, mirror_action] : m_mirror_actions) {
            ...
            if (!failed && !blocklisted && !mirror_action.action_ctxs.empty() && !mirror_action.action_in_progress) {
...
void Mirror::schedule_mirror_update_task()
{
    ...
    double after = g_ceph_context->_conf.get_val<std::chrono::seconds>("cephfs_mirror_action_update_interval").count();
...
void Mirror::run()
{
    ...
    for (auto& [filesystem, mirror_action] : m_mirror_actions) {
        ...
        m_cond.wait(locker, [&mirror_action = mirror_action] { return !mirror_action.action_in_progress; });
