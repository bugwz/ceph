    ...
    void complete(int r) override
    {
        ...
        if (!flushing) {
            ...
            librados::AioCompletion* aio_comp =
                librados::Rados::aio_create_completion(this, &rados_callback<Context, &Context::complete>);
...

    {
        ...
        if (m_state != STATE_IDLE) {
            ...
            librados::AioCompletion* aio_comp = librados::Rados::aio_create_completion(
                new C_UnwatchAndFlush(m_ioctx, on_finish), &rados_callback<Context, &Context::complete>);
...
void Watcher::rewatch()
{
    ...
    Context* unregister_watch_ctx = nullptr;
    {
        ...
        if (m_unregister_watch_ctx != nullptr) {
            ...
            auto req = RewatchRequest::create(m_ioctx, m_oid, m_lock, &m_watch_ctx, &m_watch_handle, ctx);
            req->send();
            return;
        }
    }
