...
#undef dout_prefix
#define dout_prefix *_dout << "ceph::cache::CacheController: " << this << " " << __func__ << ": "
...
int CacheController::run()
{
    try {
        ...
        m_cache_server = new CacheServer(
            m_cct,
            controller_path,
            std::bind(&CacheController::handle_request, this, std::placeholders::_1, std::placeholders::_2));
...
void CacheController::handle_request(CacheSession* session, ObjectCacheRequest* req)
{
    ldout(m_cct, 20) << dendl;
    ...
    default: ldout(m_cct, 5) << "can't recongize request" << dendl; ceph_assert(0);
    }
}
