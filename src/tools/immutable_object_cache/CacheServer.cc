...
#undef dout_prefix
#define dout_prefix *_dout << "ceph::cache::CacheServer: " << this << " " << __func__ << ": "
...
int CacheServer::start_accept()
{
    ...
    if (ec) {
        lderr(cct) << "failed to bind to domain socket '" << m_local_path << "': " << ec.message() << dendl;
        return -ec.value();
    }

    m_acceptor.listen(boost::asio::socket_base::max_connections, ec);
    if (ec) {
        lderr(cct) << "failed to listen on domain socket: " << ec.message() << dendl;
        return -ec.value();
    }
...
void CacheServer::accept()
{
    ...
    m_acceptor.async_accept(
        new_session->socket(),
        boost::bind(&CacheServer::handle_accept, this, new_session, boost::asio::placeholders::error));
}
