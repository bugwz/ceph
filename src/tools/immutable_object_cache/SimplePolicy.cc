...
#undef dout_prefix
#define dout_prefix *_dout << "ceph::cache::SimplePolicy: " << this << " " << __func__ << ": "

namespace ceph {
namespace immutable_obj_cache {

SimplePolicy::SimplePolicy(CephContext* cct, uint64_t cache_size, uint64_t max_inflight, double watermark)
    : cct(cct)
    ...
    , m_max_cache_size(cache_size)
{
    ...
    ldout(cct, 20) << "max cache size= " << m_max_cache_size << " ,watermark= " << m_watermark
...
void SimplePolicy::update_status(std::string file_name, cache_status_t new_status, uint64_t size)
{
    ...
    // promoting done
    if (entry->status == OBJ_CACHE_SKIP && (new_status == OBJ_CACHE_PROMOTED || new_status == OBJ_CACHE_DNE)) {
    ...
    // to evict
    if ((entry->status == OBJ_CACHE_PROMOTED || entry->status == OBJ_CACHE_DNE) && new_status == OBJ_CACHE_NONE) {
        // mark this entry as free
