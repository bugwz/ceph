...
#undef dout_prefix
#define dout_prefix *_dout << "ceph::cache::CacheClient: " << this << " " << __func__ << ": "
...
CacheClient::CacheClient(const std::string& file, CephContext* ceph_ctx)
    ...
    , m_sequence_id(0)
{
    m_worker_thread_num = m_cct->_conf.get_val<uint64_t>("immutable_object_cache_client_dedicated_thread_num");
...
{
    m_dm_socket.async_connect(
        m_ep, boost::bind(&CacheClient::handle_connect, this, on_finish, boost::asio::placeholders::error));
}

void CacheClient::handle_connect(Context* on_finish, const boost::system::error_code& err)
{
    if (err) {
...
void CacheClient::lookup_object(std::string pool_nspace, uint64_t pool_id, uint64_t snap_id, uint64_t object_size,
                                std::string oid, CacheGenContextURef&& on_finish)
{
    ldout(m_cct, 20) << dendl;
    ObjectCacheRequest* req =
        new ObjectCacheReadData(RBDSC_READ, ++m_sequence_id, 0, 0, pool_id, snap_id, object_size, oid, pool_nspace);
...
void CacheClient::handle_reply_header(bufferptr bp_head, const boost::system::error_code& ec, size_t bytes_transferred)
{
    ldout(m_cct, 20) << dendl;
...
void CacheClient::read_reply_data(bufferptr&& bp_head, bufferptr&& bp_data, const uint64_t data_len)
{
    ldout(m_cct, 20) << dendl;
    auto raw_ptr = bp_data.c_str();
    boost::asio::async_read(m_dm_socket,
                            boost::asio::buffer(raw_ptr, data_len),
...
void CacheClient::handle_reply_data(bufferptr bp_head, bufferptr bp_data, const uint64_t data_len,
...
void CacheClient::process(ObjectCacheRequest* reply, uint64_t seq_id)
{
    ...
    auto process_reply = new LambdaContext([current_request, reply](bool dedicated) {
        if (dedicated) {
...
{
    ObjectCacheRequest* reg_req = new ObjectCacheRegData(RBDSC_REGISTER, m_sequence_id++, ceph_version_to_str());
    reg_req->encode();
    ...
    ret = boost::asio::write(m_dm_socket, boost::asio::buffer(bl.c_str(), bl.length()), ec);
    ...
    ret = boost::asio::read(m_dm_socket, boost::asio::buffer(m_bp_header.c_str(), get_header_size()), ec);
