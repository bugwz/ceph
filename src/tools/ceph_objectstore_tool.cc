             ...
             divergent_priors_t& divergent, pg_missing_t& missing)
{
    ...
    if (!divergent.empty()) {
        ...
        PGLog::write_log_and_missing_wo_missing(
            t, &km, log, coll, info.pgid.make_pgmeta_oid(), divergent, require_rollback);
    }
    else {
        pg_missing_tracker_t tmissing(missing);
        bool rebuilt_missing_set_with_deletes = missing.may_include_deletes;
        PGLog::write_log_and_missing(t,
                                     &km,
                                     log,
                                     coll,
                                     info.pgid.make_pgmeta_oid(),
                                     tmissing,
                                     require_rollback,
...
int main(int argc, char** argv)
{
    ...
    desc.add_options()("help", "produce help message")(
        "type", po::value<string>(&type), "Arg is one of [bluestore (default), memstore]")(
        ...
        "pool", po::value<string>(&pool), "Pool name")(
        "op",
        po::value<string>(&op),
        "Arg is one of [info, log, remove, mkfs, fsck, repair, fuse, dup, export, export-remove, import, list, "
        "list-slow-omap, fix-lost, list-pgs, dump-super, meta-list, "
        "get-osdmap, set-osdmap, get-inc-osdmap, set-inc-osdmap, mark-complete, reset-last-complete, update-mon-db, "
    ...
    if (!vm.count("data-path") && op != "dump-export") {
    ...
    if (op == "dup") {
        ...
        ::close(fd);
        unique_ptr<ObjectStore> targetfs = ObjectStore::create(g_ceph_context, target_type, target_data_path, "", 0);
        if (!targetfs) {
    ...
    if (op == "list") {
        ret = do_list(fs.get(), pgidstr, object, nspace, formatter, debug, human_readable, head);
        if (ret < 0) {
    ...
    if (op == "list-slow-omap") {
        ret = do_list_slow(fs.get(), pgidstr, object, slow_threshold, formatter, debug, human_readable);
        if (ret < 0) {
            cerr << "do_list failed: " << cpp_strerror(ret) << std::endl;
        }
        goto out;
    ...
    if (!vm.count("objcmd") && op != "export" && op != "export-remove" && op != "info" && op != "log" &&
        ...
        cerr << "Must provide --op (info, log, remove, mkfs, fsck, repair, export, export-remove, import, list, "
                "fix-lost, list-pgs, dump-super, meta-list, "
    ...
    if (it != ls.end()) {
        ...
        if (op == "export" || op == "export-remove") {
            ceph_assert(superblock != nullptr);
            ret = tool.do_export(
                cct.get(), fs.get(), coll, pgid, info, map_epoch, struct_ver, *superblock, past_intervals);
            if (ret == 0) {
            ...
            ret = do_trim_pg_log(fs.get(), coll, info, pgid, map_epoch, past_intervals);
            if (ret < 0) {
            ...
            ret = do_trim_pg_log_dups(fs.get(), coll, info, pgid, map_epoch, past_intervals);
            if (ret < 0) {
