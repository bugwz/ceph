...
int _action_on_all_objects_in_pg(ObjectStore* store, coll_t coll, action_on_object_t& action, bool debug)
{
    ...
    while (!next.is_max()) {
        ...
        if (r < 0) {
            cerr << "Error listing collection: " << coll << ", " << cpp_strerror(r) << std::endl;
            return r;
        }
        for (vector<ghobject_t>::iterator obj = list.begin(); obj != list.end(); ++obj) {
            if (obj->is_pgmeta()) continue;
            object_info_t oi;
            if (coll != coll_t::meta() && !obj->is_pgmeta()) {
                bufferlist attr;
                r = store->getattr(ch, *obj, OI_ATTR, attr);
                if (r < 0) {
                    cerr << "Error getting attr on : " << make_pair(coll, *obj) << ", " << cpp_strerror(r) << std::endl;
                }
                else {
                    auto bp = attr.cbegin();
                    try {
                        ...
                        r = -EINVAL;
                        cerr << "Error decoding attr on : " << make_pair(coll, *obj) << ", " << cpp_strerror(r)
                             << std::endl;
                    }
                }
            }
...
int action_on_all_objects_in_pg(ObjectStore* store, string pgidstr, action_on_object_t& action, bool debug)
{
    ...
    for (vector<coll_t>::iterator i = candidates.begin(); i != candidates.end(); ++i) {
        spg_t cand_pgid;
        if (!i->is_pg(&cand_pgid)) continue;

        // If an exact match or treat no shard as any shard
        if (cand_pgid == pgid || (pgid.is_no_shard() && pgid.pgid == cand_pgid.pgid)) {
            colls_to_check.push_back(*i);
        }
    }

    if (debug) cerr << colls_to_check.size() << " pgs to scan" << std::endl;
    for (vector<coll_t>::iterator i = colls_to_check.begin(); i != colls_to_check.end(); ++i, ++scanned) {
        if (debug)
            cerr << "Scanning " << *i << ", " << scanned << "/" << colls_to_check.size() << " completed" << std::endl;
...
struct pgid_object_list
{
    ...
    void insert(coll_t coll, ghobject_t& ghobj) { _objects.push_back(make_pair(coll, ghobj)); }

    void dump(Formatter* f, bool human_readable) const
    {
        if (!human_readable) f->open_array_section("pgid_objects");
        for (list<pair<coll_t, ghobject_t> >::const_iterator i = _objects.begin(); i != _objects.end(); ++i) {
            f->open_array_section("pgid_object");
            spg_t pgid;
            bool is_pg = i->first.is_pg(&pgid);
            if (is_pg) f->dump_string("pgid", stringify(pgid));
            if (!is_pg || !human_readable) f->dump_string("coll", i->first.to_str());
...
struct lookup_ghobject : public action_on_object_t
{
    ...
    lookup_ghobject(const string& name, const boost::optional<std::string>& nspace, bool need_snapset = false)
        : _name(name)
        , _namespace(nspace)
    ...
    {
        if (_need_snapset && !ghobj.hobj.has_snapset()) return;
        if ((_name.length() == 0 || ghobj.hobj.oid.name == _name) && (!_namespace || ghobj.hobj.nspace == _namespace))
            _objects.insert(coll, ghobj);
        return;
    }

    int size() const { return _objects._objects.size(); }
    ...
    void dump(Formatter* f, bool human_readable) const { _objects.dump(f, human_readable); }
};

struct lookup_slow_ghobject : public action_on_object_t
{
    list<tuple<coll_t, ghobject_t, ceph::signedspan, ceph::signedspan, ceph::signedspan, string> > _objects;
    const string _name;
    double threshold;
    ...
    void call(ObjectStore* store, coll_t coll, ghobject_t& ghobj, object_info_t& oi) override
    {
        ...
        ceph::signedspan total_time = first_seek_time;
        {
            ...
            if (!iter) {
                cerr << "omap_get_iterator: " << cpp_strerror(ENOENT) << " obj:" << ghobj << std::endl;
                return;
            }
        ...
        if (total_time >= make_timespan(threshold)) {
            _objects.emplace_back(
                coll, ghobj, first_seek_time, last_seek_time, total_time, url_escape(iter->tail_key()));
            cerr << ">>>>>  found obj " << ghobj << " first_seek_time "
                 << std::chrono::duration_cast<std::chrono::seconds>(first_seek_time).count() << " last_seek_time "
                 << std::chrono::duration_cast<std::chrono::seconds>(last_seek_time).count() << " total_time "
                 << std::chrono::duration_cast<std::chrono::seconds>(total_time).count()
                 << " tail key: " << url_escape(iter->tail_key()) << std::endl;
        }
        return;
    }
    ...
    void dump(Formatter* f, bool human_readable) const
    {
        if (!human_readable) f->open_array_section("objects");
        for (auto i = _objects.begin(); i != _objects.end(); ++i) {
            ...
            if (is_pg) f->dump_string("pgid", stringify(pgid));
            if (!is_pg || !human_readable) f->dump_string("coll", coll.to_str());
            f->dump_object("ghobject", ghobj);
            f->open_object_section("times");
            f->dump_int("first_seek_time", std::chrono::duration_cast<std::chrono::seconds>(first_seek_time).count());
            f->dump_int("last_seek_time", std::chrono::duration_cast<std::chrono::seconds>(last_seek_time).count());
            f->dump_int("total_time", std::chrono::duration_cast<std::chrono::seconds>(total_time).count());
...
int get_log(CephContext* cct, ObjectStore* fs, __u8 struct_ver, spg_t pgid, const pg_info_t& info,
            PGLog::IndexedLog& log, pg_missing_t& missing)
{
    try {
        ...
        PGLog::read_log_and_missing(cct,
                                    fs,
                                    ch,
        ...
        if (debug && oss.str().size()) cerr << oss.str() << std::endl;
    }
...
void dump_log(Formatter* formatter, ostream& out, pg_log_t& log, pg_missing_t& missing)
{
...
int finish_remove_pgs(ObjectStore* store)
{
    ...
    if (r < 0) {
        cerr << "finish_remove_pgs: failed to list pgs: " << cpp_strerror(r) << std::endl;
        return r;
    }
...
int mark_pg_for_removal(ObjectStore* fs, spg_t pgid, ObjectStore::Transaction* t)
{
    ...
    if (r < 0) cerr << __func__ << " warning: peek_map_epoch reported error" << std::endl;
...
template<typename Func> void wait_until_done(ObjectStore::Transaction* txn, Func&& func)
{
    bool finished = false;
...
{
    if (!dry_run) finish_remove_pgs(store);
    if (!store->collection_exists(coll_t(r_pgid))) return -ENOENT;
...
int write_info(ObjectStore::Transaction& t, epoch_t epoch, pg_info_t& info, PastIntervals& past_intervals)
{
    // Empty for this
    ...
    int ret = prepare_info_keymap(
        g_ceph_context, &km, &key_to_remove, epoch, info, last_written_info, past_intervals, true, true, false);
...
int write_pg(ObjectStore::Transaction& t, epoch_t epoch, pg_info_t& info, pg_log_t& log, PastIntervals& past_intervals,
             divergent_priors_t& divergent, pg_missing_t& missing)
{
    ...
    if (!divergent.empty()) {
        ...
        PGLog::write_log_and_missing_wo_missing(t, &km, log, coll, info.pgid.make_pgmeta_oid(), divergent, true);
    }
    else {
        pg_missing_tracker_t tmissing(missing);
        bool rebuilt_missing_set_with_deletes = missing.may_include_deletes;
        PGLog::write_log_and_missing(
            t, &km, log, coll, info.pgid.make_pgmeta_oid(), tmissing, true, &rebuilt_missing_set_with_deletes);
    }
    t.omap_setkeys(coll, info.pgid.make_pgmeta_oid(), km);
    return 0;
}

int do_trim_pg_log(ObjectStore* store, const coll_t& coll, pg_info_t& info, const spg_t& pgid, epoch_t map_epoch,
                   PastIntervals& past_intervals)
{
    ...
    cerr << "Log bounds are: "
         << "(" << info.log_tail << "," << info.last_update << "]" << std::endl;
    ...
    while (!done) {
        ...
        set<string> keys_to_trim;
        {
            ...
            for (p->seek_to_first(); p->valid(); p->next()) {
                if (p->key()[0] == '_') continue;
                if (p->key() == "can_rollback_to") continue;
                if (p->key() == "divergent_priors") continue;
                if (p->key() == "rollback_info_trimmed_to") continue;
                if (p->key() == "may_include_deletes_in_missing") continue;
                if (p->key().substr(0, 7) == string("missing")) continue;
                if (p->key().substr(0, 4) == string("dup_")) continue;
                ...
                new_tail = e.version;
                if (keys_to_trim.size() >= trim_at_once) break;
            }

            if (!p->valid()) done = true;
    ...
    if (!dry_run) store->compact();
    return 0;
}

int do_trim_pg_log_dups(ObjectStore* store, const coll_t& coll, pg_info_t& info, const spg_t& pgid, epoch_t map_epoch,
                        PastIntervals& past_intervals)
{
    ghobject_t oid = pgid.make_pgmeta_oid();
    struct stat st;
    ...
    cout << "max_dup_entries=" << max_dup_entries << " max_chunk_size=" << max_chunk_size << std::endl;
    if (dry_run) {
...
int ObjectStoreTool::export_file(ObjectStore* store, coll_t cid, ghobject_t& obj)
{
    ...
    total = st.st_size;
    if (debug) cerr << "size=" << total << std::endl;
...

    {
        ...
        bl.push_back(bp);
        decode(objb.oi, bl);
        if (debug) cerr << "object_info: " << objb.oi << std::endl;
    }
    ...
    while (total > 0) {
        ...
        if (debug) cerr << "data section offset=" << offset << " len=" << len << std::endl;
    ...
    if (debug) cerr << "omap map size " << mapcount << std::endl;
...
int ObjectStoreTool::export_files(ObjectStore* store, coll_t coll)
{
    ...
    while (!next.is_max()) {
        ...
        int r = store->collection_list(ch, next, ghobject_t::get_max(), 300, &objects, &next);
        if (r < 0) return r;
...
int set_inc_osdmap(ObjectStore* store, epoch_t e, bufferlist& bl, bool force)
{
    ...
    if (e == 0) {
        ...
        cerr << "incremental.epoch mismatch: " << inc.epoch << " != " << e << std::endl;
        if (force) {
...
{
    auto ch = store->open_collection(coll_t::meta());
    if (store->read(ch, OSD::get_inc_osdmap_pobject_name(e), 0, 0, bl) < 0) {
        return -ENOENT;
    }
    return 0;
}
...
int set_osdmap(ObjectStore* store, epoch_t e, bufferlist& bl, bool force)
{
    ...
    if (e == 0) {
        ...
        cerr << "osdmap.epoch mismatch: " << e << " != " << osdmap.get_epoch() << std::endl;
        if (force) {
            cerr << "But will continue anyway." << std::endl;
        }
        else {
...
int get_osdmap(ObjectStore* store, epoch_t e, OSDMap& osdmap, bufferlist& bl)
{
    ...
    bool found = store->read(ch, OSD::get_osdmap_pobject_name(e), 0, 0, bl) >= 0;
    if (!found) {
        cerr << "Can't find OSDMap for pg epoch " << e << std::endl;
        return -ENOENT;
    }
    osdmap.decode(bl);
    if (debug) cerr << osdmap << std::endl;
    return 0;
}
...
int ObjectStoreTool::do_export(CephContext* cct, ObjectStore* fs, coll_t coll, spg_t pgid, pg_info_t& info,
                               ...
                               PastIntervals& past_intervals)
{
    ...
    metadata_section ms(struct_ver, map_epoch, info, log, past_intervals, missing);
...
int get_data(ObjectStore* store, coll_t coll, ghobject_t hoid, ObjectStore::Transaction* t, bufferlist& bl)
{
    auto ebliter = bl.cbegin();
    data_section ds;
    ds.decode(ebliter);

    if (debug) cerr << "\tdata: offset " << ds.offset << " len " << ds.len << std::endl;
    t->write(coll, hoid, ds.offset, ds.len, ds.databl);
    return 0;
}

int dump_attrs(Formatter* formatter, ghobject_t hoid, bufferlist& bl)
{
    auto ebliter = bl.cbegin();
    attr_section as;
    as.decode(ebliter);
    ...
    if (hoid.generation == ghobject_t::NO_GEN && hoid.hobj.is_head()) {
    ...
    for (auto kv : as.data) {
        ...
        if (('_' != kv.first.at(0)) || kv.first.size() == 1) continue;
...
int get_attrs(ObjectStore* store, coll_t coll, ghobject_t hoid, ObjectStore::Transaction* t, bufferlist& bl,
              OSDriver& driver, SnapMapper& snap_mapper)
{
    ...
    auto ch = store->open_collection(coll);
    if (debug) cerr << "\tattrs: len " << as.data.size() << std::endl;
        ...
        if (mi != as.data.end()) {
            ...
            for (auto& p : snapset.clone_snaps) {
                ...
                if (!store->exists(ch, clone)) {
                    ...
                    if (debug) cerr << "\tskipping missing " << clone << " (snaps " << snaps << ")" << std::endl;
                    continue;
                }
                if (debug) cerr << "\tsetting " << clone.hobj << " snaps " << snaps << std::endl;
...
int get_omap_hdr(ObjectStore* store, coll_t coll, ghobject_t hoid, ObjectStore::Transaction* t, bufferlist& bl)
{
    auto ebliter = bl.cbegin();
    omap_hdr_section oh;
    oh.decode(ebliter);

    if (debug) cerr << "\tomap header: " << string(oh.hdr.c_str(), oh.hdr.length()) << std::endl;
...
int get_omap(ObjectStore* store, coll_t coll, ghobject_t hoid, ObjectStore::Transaction* t, bufferlist& bl)
{
    auto ebliter = bl.cbegin();
    omap_section os;
    os.decode(ebliter);

    if (debug) cerr << "\tomap: size " << os.omap.size() << std::endl;
    t->omap_setkeys(coll, hoid, os.omap);
    return 0;
}

int ObjectStoreTool::dump_object(Formatter* formatter, bufferlist& bl)
{
    auto ebliter = bl.cbegin();
    object_begin ob;
    ob.decode(ebliter);
    ...
    while (!done) {
        ...
        case TYPE_OMAP:
            ...
            if (ret) return ret;
            break;
        case TYPE_OBJECT_END: done = true; break;
        default: cerr << "Unknown section type " << type << std::endl; return -EFAULT;
        }
    }
    formatter->close_section();
    return 0;
...
int ObjectStoreTool::get_object(ObjectStore* store, OSDriver& driver, SnapMapper& mapper, coll_t coll, bufferlist& bl,
                                OSDMap& origmap, bool* skipped_objects)
{
    ...
    if (ob.hoid.hobj.nspace != g_ceph_context->_conf->osd_hit_set_namespace) {
        ...
        if (coll_pgid.shard != ob.hoid.shard_id) {
            cerr << "INTERNAL ERROR: Importing shard " << coll_pgid.shard << " but object shard is " << ob.hoid.shard_id
                 << std::endl;
            return -EFAULT;
        }
    ...
    if (!dry_run) t->touch(coll, ob.hoid);
...
int get_pg_metadata(ObjectStore* store, bufferlist& bl, metadata_section& ms, const OSDSuperblock& sb, spg_t pgid)
{
    auto ebliter = bl.cbegin();
    ms.decode(ebliter);
    ...
    if (ms.osdmap.get_epoch() < sb.oldest_map) {
        cerr << "PG export's map " << ms.osdmap.get_epoch() << " is older than OSD's oldest_map " << sb.oldest_map
             << std::endl;
        if (!force) {
            cerr << " pass --force to proceed anyway (with incomplete PastIntervals)" << std::endl;
            return -EINVAL;
        }
    }
    if (debug) {
...
void filter_divergent_priors(spg_t import_pgid, const OSDMap& curmap, const string& hit_set_namespace,
                             const divergent_priors_t& in, divergent_priors_t& out, divergent_priors_t& reject)
{
    out.clear();
    reject.clear();

    for (divergent_priors_t::const_iterator i = in.begin(); i != in.end(); ++i) {
...
int ObjectStoreTool::dump_export(Formatter* formatter)
{
    ...
    while (!done) {
        ...
        case TYPE_PG_METADATA:
            if (objects_started) cerr << "WARNING: metadata_section out of order" << std::endl;
        ...
        case TYPE_PG_END:
            ...
            break;
        default: cerr << "Unknown section type " << std::to_string(type) << std::endl; return -EFAULT;
        }
    }
...
int ObjectStoreTool::do_import(ObjectStore* store, OSDSuperblock& sb, bool force, std::string pgidstr)
{
    bufferlist ebl;
    pg_info_t info;
    PGLog::IndexedLog log;
    ...
    if (pgidstr.length()) {
        ...
        ceph_assert(ok);
        if (pgid != user_pgid) {
            cerr << "specified pgid " << user_pgid << " does not match actual pgid " << pgid << std::endl;
            return -EINVAL;
        }
    }

    if (!pgb.superblock.cluster_fsid.is_zero() && pgb.superblock.cluster_fsid != sb.cluster_fsid) {
        cerr << "Export came from different cluster with fsid " << pgb.superblock.cluster_fsid << std::endl;
        return -EINVAL;
    }
    ...
    if (pgid.is_no_shard()) pgb.superblock.compat_features.incompat.remove(CEPH_OSD_FEATURE_INCOMPAT_SHARDS);
    ...
    if (sb.compat_features.compare(pgb.superblock.compat_features) == -1) {
        ...
        if (!force) return 11;   // Positive return means exit status
    }
    ...
    OSDriver driver(store, coll_t(), OSD::make_snapmapper_oid());
    ...
    while (!done) {
        ...
        case TYPE_OBJECT_BEGIN:
            ...
            ret = get_object(store, driver, mapper, coll, ebl, ms.osdmap, &skipped_objects);
            if (ret) return ret;
            break;
        case TYPE_PG_METADATA:
            ...
            if (pgid != ms.info.pgid) {
                cerr << "specified pgid " << pgid << " does not match import file pgid " << ms.info.pgid << std::endl;
                return -EINVAL;
            }
            ...
            if (ms.osdmap.have_pg_pool(pgid.pgid.pool())) {
                ...
                if (p != pg_num_history.pg_nums.end() && !p->second.empty()) {
                    unsigned start_pg_num = ms.osdmap.get_pg_num(pgid.pgid.pool());
                    unsigned pg_num = start_pg_num;
                    for (auto q = p->second.lower_bound(ms.map_epoch); q != p->second.end(); ++q) {
                        unsigned new_pg_num = q->second;
                        cout << "pool " << pgid.pgid.pool() << " pg_num " << pg_num << " -> " << new_pg_num
                             << std::endl;
                        ...
                        if (pgid.is_merge_source(pg_num, new_pg_num, &target)) {
                            ...
                            coll_t coll(target);
                            if (store->collection_exists(coll)) {
                                cerr << "pgid " << pgid << " merges to target " << target << " which already exists"
                                     << std::endl;
                                return 12;
                            }
                        }
                        ...
                        if (pgid.is_split(start_pg_num, new_pg_num, &children)) {
                            ...
                            for (auto child : children) {
                                coll_t coll(child);
                                if (store->collection_exists(coll)) {
                                    cerr << "pgid " << pgid << " splits to " << children << " and " << child
                                         << " exists" << std::endl;
                                    return 12;
                                }
                            }
            ...
            if (!dry_run) {
                ...
                create_pg_collection(t, pgid, pgid.get_split_bits(ms.osdmap.get_pg_pool(pgid.pool())->get_pg_num()));
    ...
    if (!dry_run) {
        ...
        pg_log_t::filter_log(pgid, ms.osdmap, g_ceph_context->_conf->osd_hit_set_namespace, ms.log, newlog, reject);
        if (debug) {
            for (list<pg_log_entry_t>::iterator i = newlog.log.begin(); i != newlog.log.end(); ++i)
                cerr << "Keeping log entry " << *i << std::endl;
            for (list<pg_log_entry_t>::iterator i = reject.log.begin(); i != reject.log.end(); ++i)
                cerr << "Skipping log entry " << *i << std::endl;
        }

        divergent_priors_t newdp, rejectdp;
        filter_divergent_priors(
            pgid, ms.osdmap, g_ceph_context->_conf->osd_hit_set_namespace, ms.divergent_priors, newdp, rejectdp);
        ms.divergent_priors = newdp;
        if (debug) {
            for (divergent_priors_t::iterator i = newdp.begin(); i != newdp.end(); ++i)
                cerr << "Keeping divergent_prior " << *i << std::endl;
            for (divergent_priors_t::iterator i = rejectdp.begin(); i != rejectdp.end(); ++i)
                cerr << "Skipping divergent_prior " << *i << std::endl;
        }

        ms.missing.filter_objects([&](const hobject_t& obj) {
            if (obj.nspace == g_ceph_context->_conf->osd_hit_set_namespace) return false;
        ...
        if (skipped_objects) ms.info.stats.stats_invalid = true;

        ret = write_pg(t, ms.map_epoch, ms.info, newlog, ms.past_intervals, ms.divergent_priors, ms.missing);
        if (ret) return ret;
    }

    // done, clear removal flag
    if (debug) cerr << "done, clearing removal flag" << std::endl;
...
int do_list_slow(ObjectStore* store, string pgidstr, string object, double threshold, Formatter* formatter, bool debug,
                 bool human_readable)
{
    int r;
...
int remove_object(coll_t coll, ghobject_t& ghobj, SnapMapper& mapper,
                  MapCacher::Transaction<std::string, bufferlist>* _t, ObjectStore::Transaction* t, enum rmtype type)
{
...
int do_remove_object(ObjectStore* store, coll_t coll, ghobject_t& ghobj, bool all, bool force, enum rmtype type)
{
    auto ch = store->open_collection(coll);
    spg_t pg;
    ...
    if (ghobj.hobj.has_snapset()) {
        ...
        if (r < 0) {
            ...
            if (!(force && !all)) return r;
        }
        //    cout << "snapset " << ss << std::endl;
        if (!ss.clone_snaps.empty() && !all) {
            if (force) {
                cout << "WARNING: only removing " << (ghobj.hobj.is_head() ? "head" : "snapdir")
                     << " with clones present" << std::endl;
                ss.clone_snaps.clear();
            }
            else {
                cerr << "Clones are present, use removeall to delete everything" << std::endl;
                return -EINVAL;
            }
        }
    }
...
int do_set_bytes(ObjectStore* store, coll_t coll, ghobject_t& ghobj, int fd)
{
    ...
    if (debug) cerr << "Write " << ghobj << std::endl;
    ...
    do {
        ...
        if (debug) cerr << "\tdata: offset " << offset << " bytes " << bytes << std::endl;
        if (!dry_run) t->write(coll, ghobj, offset, bytes, rawdatabl);
    ...
    auto ch = store->open_collection(coll);
    if (!dry_run) store->queue_transaction(ch, std::move(*t));
    return 0;
}
...
int do_set_attr(ObjectStore* store, coll_t coll, ghobject_t& ghobj, string key, int fd)
{
    ...
    if (debug) cerr << "Setattr " << ghobj << std::endl;
...
int do_rm_attr(ObjectStore* store, coll_t coll, ghobject_t& ghobj, string key)
{
    ...
    if (debug) cerr << "Rmattr " << ghobj << std::endl;
...
int do_set_omap(ObjectStore* store, coll_t coll, ghobject_t& ghobj, string key, int fd)
{
    ...
    if (debug) cerr << "Set_omap " << ghobj << std::endl;
...
int do_rm_omap(ObjectStore* store, coll_t coll, ghobject_t& ghobj, string key)
{
    ...
    if (debug) cerr << "Rm_omap " << ghobj << std::endl;
...
int do_set_omaphdr(ObjectStore* store, coll_t coll, ghobject_t& ghobj, int fd)
{
    ...
    if (debug) cerr << "Omap_setheader " << ghobj << std::endl;
...
int get_snapset(ObjectStore* store, coll_t coll, ghobject_t& ghobj, SnapSet& ss, bool silent = false)
{
    ...
    if (r < 0) {
        if (!silent)
            cerr << "Error getting snapset on : " << make_pair(coll, ghobj) << ", " << cpp_strerror(r) << std::endl;
        return r;
    }
    auto bp = attr.cbegin();
    try {
        ...
        r = -EINVAL;
        cerr << "Error decoding snapset on : " << make_pair(coll, ghobj) << ", " << cpp_strerror(r) << std::endl;
        return r;
    }
    return 0;
}
...
int print_obj_info(ObjectStore* store, coll_t coll, ghobject_t& ghobj, Formatter* formatter)
{
    ...
    if (gr < 0) {
        r = gr;
        cerr << "Error getting attr on : " << make_pair(coll, ghobj) << ", " << cpp_strerror(r) << std::endl;
    }
    else {
        object_info_t oi;
        auto bp = attr.cbegin();
        try {
            ...
            r = -EINVAL;
            cerr << "Error decoding attr on : " << make_pair(coll, ghobj) << ", " << cpp_strerror(r) << std::endl;
        }
    }
    struct stat st;
    ...
    if (sr < 0) {
        r = sr;
        cerr << "Error stat on : " << make_pair(coll, ghobj) << ", " << cpp_strerror(r) << std::endl;
    }
    else {
    ...
    if (gr == 0) {
        ...
        try {
            ...
            r = -EINVAL;
            cerr << "Error decoding hinfo on : " << make_pair(coll, ghobj) << ", " << cpp_strerror(r) << std::endl;
        }
    }
...
int corrupt_info(ObjectStore* store, coll_t coll, ghobject_t& ghobj, Formatter* formatter)
{
    ...
    if (!dry_run) {
        ...
        if (r < 0) {
            cerr << "Error writing object info: " << make_pair(coll, ghobj) << ", " << cpp_strerror(r) << std::endl;
            return r;
        }
    }
    return 0;
...
int set_size(ObjectStore* store, coll_t coll, ghobject_t& ghobj, uint64_t setsize, Formatter* formatter, bool corrupt)
{
    auto ch = store->open_collection(coll);
    ...
    if (is_snap) {
        ...
        if (r < 0 && r != -ENOENT) {
            ...
            cerr << "Error getting snapset on : " << make_pair(coll, head) << ", " << cpp_strerror(r) << std::endl;
            return r;
        }
        if (r == -ENOENT) {
    ...
    if ((uint64_t)st.st_size == setsize && oi.size == setsize && (!is_snap || csi->second == setsize)) {
        cout << "Size of object is already " << setsize << std::endl;
        return 0;
    }
    cout << "Setting size to " << setsize << ", stat size " << st.st_size << ", obj info size " << oi.size;
    if (is_snap) {
        cout << ", " << (found_head ? "head" : "snapdir") << " clone_size " << csi->second;
        csi->second = setsize;
    }
    cout << std::endl;
    if (!dry_run) {
...
int clear_snapset(ObjectStore* store, coll_t coll, ghobject_t& ghobj, string arg)
{
    SnapSet ss;
    ...
    if (arg == "corrupt" || arg == "seq") ss.seq = 0;
    // Use "snaps" to just clear SnapSet.clone_snaps
    if (arg == "corrupt" || arg == "snaps") ss.clone_snaps.clear();
    // By default just clear clone, clone_overlap and clone_size
    if (arg == "corrupt") arg = "";
    if (arg == "" || arg == "clones") ss.clones.clear();
    if (arg == "" || arg == "clone_overlap") ss.clone_overlap.clear();
    if (arg == "" || arg == "clone_size") ss.clone_size.clear();
    // Break all clone sizes by adding 1
    if (arg == "size") {
        for (map<snapid_t, uint64_t>::iterator i = ss.clone_size.begin(); i != ss.clone_size.end(); ++i) ++(i->second);
    }
    ...
    if (!dry_run) {
        ...
        if (r < 0) {
            cerr << "Error setting snapset on : " << make_pair(coll, ghobj) << ", " << cpp_strerror(r) << std::endl;
            return r;
        }
    }
    return 0;
...
map<snapid_t, uint64_t>::iterator find(map<snapid_t, uint64_t>& m, snapid_t clid)
{
    return m.find(clid);
}

template<class T> int remove_from(T& mv, string name, snapid_t cloneid, bool force)
{
...
int dup(string srcpath, ObjectStore* src, string dstpath, ObjectStore* dst)
{
    cout << "dup from " << src->get_type() << ": " << srcpath << "\n"
         << "      to " << dst->get_type() << ": " << dstpath << std::endl;
    int num, i;
    ...
    if (src->get_fsid() != dst->get_fsid()) {
        cerr << "src fsid " << src->get_fsid() << " != dest " << dst->get_fsid() << std::endl;
        goto out;
    }
    ...
    for (auto cid : collections) {
        ...
        auto dch = dst->create_new_collection(cid);
        {
            ...
            if (bits < 0) {
                if (src->get_type() == "filestore" && cid.is_meta()) {
                    ...
                    cerr << "cannot get bit count for collection " << cid << ": " << cpp_strerror(bits) << std::endl;
                    goto out;
                }
            }
        ...
        while (true) {
            ...
            if (r < 0) {
                cerr << "collection_list on " << cid << " from " << pos << " got: " << cpp_strerror(r) << std::endl;
                goto out;
            }
            if (ls.empty()) {
                break;
            ...
            for (auto& oid : ls) {
                ...
                if (n % 100 == 0) {
                    cout << "  " << std::setw(16) << n << " objects, " << std::setw(16) << bytes << " bytes, "
                         << std::setw(16) << keys << " keys" << std::setw(1) << "\r" << std::flush;
                }
                n++;
        ...
        cout << "  " << std::setw(16) << n << " objects, " << std::setw(16) << bytes << " bytes, " << std::setw(16)
             << keys << " keys" << std::setw(1) << std::endl;
    }
...
int mydump_journal(Formatter* f, string journalpath, bool m_journal_dio)
{
    ...
    FileJournal* journal = new FileJournal(g_ceph_context, uuid_d(), NULL, NULL, journalpath.c_str(), m_journal_dio);
    r = journal->_fdump(*f, false);
    delete journal;
    return r;
}

int apply_layout_settings(ObjectStore* os, const OSDSuperblock& superblock, const string& pool_name, const spg_t& pgid,
                          bool dry_run, int target_level)
{
    int r = 0;
    ...
    if (pool_name.length()) {
        ...
        if (poolid < 0) {
            cerr << "Couldn't find pool " << pool_name << ": " << cpp_strerror(poolid) << std::endl;
            return poolid;
        }
    }
    ...
    for (auto const& coll : collections) {
        spg_t coll_pgid;
        if (coll.is_pg(&coll_pgid) && ((poolid >= 0 && coll_pgid.pool() == (uint64_t)poolid) || coll_pgid == pgid)) {
    ...
    for (auto const& coll : filtered_colls) {
        if (dry_run) {
            ...
            cerr << "Finished " << done << "/" << total << " collections"
                 << "\r";
    ...
    cerr << "Finished " << total << "/" << total << " collections"
         << "\r" << std::endl;
    return r;
}
...
int main(int argc, char** argv)
{
    ...
    desc.add_options()("help", "produce help message")(
        "type", po::value<string>(&type), "Arg is one of [bluestore (default), filestore, memstore]")(
        "data-path", po::value<string>(&dpath), "path to object store, mandatory")(
        "journal-path", po::value<string>(&jpath), "path to journal, use if tool can't find it")(
        "pgid",
        po::value<string>(&pgidstr),
        "PG id, mandatory for info, log, remove, export, export-remove, mark-complete, trim-pg-log, trim-pg-log-dups "
        "and mandatory for apply-layout-settings if --pool is not specified")(
        "pool", po::value<string>(&pool), "Pool name, mandatory for apply-layout-settings if --pgid is not specified")(
        "op",
        po::value<string>(&op),
        "Arg is one of [info, log, remove, mkfs, fsck, repair, fuse, dup, export, export-remove, import, list, "
        "list-slow-omap, fix-lost, list-pgs, dump-journal, dump-super, meta-list, "
        "get-osdmap, set-osdmap, get-superblock, set-superblock, get-inc-osdmap, set-inc-osdmap, mark-complete, "
        "reset-last-complete, apply-layout-settings, update-mon-db, "
        "dump-export, trim-pg-log, trim-pg-log-dups statfs]")(
        "epoch",
        po::value<unsigned>(&epoch),
        "epoch# for get-osdmap and get-inc-osdmap, the current epoch in use if not specified")(
        "file",
        po::value<string>(&file),
        "path of file to export, export-remove, import, get-osdmap, set-osdmap, get-superblock, set-superblock, "
        "get-inc-osdmap or set-inc-osdmap")(
        "mon-store-path", po::value<string>(&mon_store_path), "path of monstore to update-mon-db")(
        "fsid", po::value<string>(&fsid), "fsid for new store created by mkfs")(
        "target-data-path", po::value<string>(&target_data_path), "path of target object store (for --op dup)")(
        "mountpoint", po::value<string>(&mountpoint), "fuse mountpoint")(
        "format",
        po::value<string>(&format)->default_value("json-pretty"),
        "Output format which may be json, json-pretty, xml, xml-pretty")("debug", "Enable diagnostic output to stderr")(
        "no-mon-config", "Do not contact mons for config")("no-superblock", "Do not read superblock")(
        "force",
        "Ignore some types of errors and proceed with operation - USE WITH CAUTION: CORRUPTION POSSIBLE NOW OR IN THE "
        "FUTURE")("skip-journal-replay", "Disable journal replay")("skip-mount-omap", "Disable mounting of omap")(
        "head", "Find head/snapdir when searching for objects by name")("dry-run", "Don't modify the objectstore")(
        "tty", "Treat stdout as a tty (no binary data)")(
        "namespace", po::value<string>(&argnspace), "Specify namespace when searching for objects")(
        "rmtype",
        po::value<string>(&rmtypestr),
        "Specify corrupting object removal 'snapmap' or 'nosnapmap' - TESTING USE ONLY")(
        "slow-omap-threshold",
        po::value<unsigned>(&slow_threshold),
    ...
    positional.add_options()(
        ...
        "objcmd",
        po::value<string>(&objcmd),
        "command [(get|set)-bytes, (get|set|rm)-(attr|omap), (get|set)-omaphdr, list-attrs, list-omap, remove]")(
        "arg1", po::value<string>(&arg1), "arg1 based on cmd")("arg2", po::value<string>(&arg2), "arg2 based on cmd");
    ...
    if (op == "dump-import") op = "dump-export";
    ...
    if (vm.count("namespace")) nspace = argnspace;

    dry_run = (vm.count("dry-run") > 0);
    tty = (vm.count("tty") > 0);

    osflagbits_t flags = 0;
    if (dry_run || vm.count("skip-journal-replay")) flags |= SKIP_JOURNAL_REPLAY;
    if (vm.count("skip-mount-omap")) flags |= SKIP_MOUNT_OMAP;
    if (op == "update-mon-db") flags |= SKIP_JOURNAL_REPLAY;
    ...
    if (fd >= 0) {
        ...
        tmp.push_back(dpath);
        tmp.insert(tmp.end(), ceph_option_strings.begin(), ceph_option_strings.end());
        tmp.swap(ceph_option_strings);
    }

    vector<const char*> ceph_options;
    ceph_options.reserve(ceph_options.size() + ceph_option_strings.size());
    for (vector<string>::iterator i = ceph_option_strings.begin(); i != ceph_option_strings.end(); ++i) {
    ...
    if (fd >= 0) {
        ...
        if (bl.length()) {
            ...
            if (vm.count("type") && dp_type != "" && type != dp_type)
                cerr << "WARNING: Ignoring type \"" << type << "\" - found data-path type \"" << dp_type << "\""
                     << std::endl;
            type = dp_type;
    ...
    if (!vm.count("data-path") && op != "dump-export" && !(op == "dump-journal" && type == "filestore")) {
    ...
    if (op != "list" && op != "apply-layout-settings" && vm.count("op") && vm.count("object")) {
        cerr << "Can't specify both --op and object command syntax" << std::endl;
        usage(desc);
        return 1;
    }
    if (op == "apply-layout-settings" && !(vm.count("pool") ^ vm.count("pgid"))) {
        cerr << "apply-layout-settings requires either --pool or --pgid" << std::endl;
        usage(desc);
        return 1;
    }
    ...
    file_fd = fd_none;
    if ((op == "export" || op == "export-remove" || op == "get-osdmap" || op == "get-inc-osdmap" ||
         op == "get-superblock") &&
        !dry_run) {
        if (!vm.count("file") || file == "-") {
            ...
            file_fd = open(file.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0666);
             ...
             op == "pg-log-inject-dups" || op == "set-superblock") {
        if (!vm.count("file") || file == "-") {
    ...
    auto cct = global_init(NULL, ceph_options, CEPH_ENTITY_TYPE_OSD, CODE_ENVIRONMENT_UTILITY_NODOUT, init_flags);
    ...
    if (op == "dump-journal" && type == "filestore") {
        ...
        if (ret < 0) {
            cerr << "journal-path: " << jpath << ": " << cpp_strerror(ret) << std::endl;
            return 1;
        }
        formatter->flush(cout);
        return 0;
    ...
    if (op == "dump-export") {
        ...
        if (ret < 0) {
            cerr << "dump-export: " << cpp_strerror(ret) << std::endl;
            return 1;
        }
        return 0;
    }
    ...
    if (type == "filestore") {
        ...
        if (S_ISDIR(st.st_mode)) {
            cerr << "journal-path: " << jpath << ": " << cpp_strerror(EISDIR) << std::endl;
            return 1;
        }
    }
    ...
    if (op == "dup") {
        ...
        ::close(fd);
        ObjectStore* targetfs = ObjectStore::create(g_ceph_context, target_type, target_data_path, "", 0);
        if (targetfs == NULL) {
    ...
    if (!no_superblock) {
        ...
        if (supported.compare(superblock->compat_features) == -1) {
            ...
            cerr << "On-disk OSD incompatible features set " << unsupported << std::endl;
            ret = -EINVAL;
            goto out;
        }
    }
        ...
        if (object == "") {
            ...
            try {
                if (!json_spirit::read(object, v) ||
                    ...
                    if (vm.count("objcmd") && (objcmd == "remove-clone-metadata")) head = true;
                    lookup_ghobject lookup(object, nspace, head);
                    if (pgidstr.length())
                    ...
                    if (pgidstr.length() == 0 && v.type() != json_spirit::array_type) {
                        ss << "Without --pgid the object '" << object << "' must be a JSON array";
                        throw std::runtime_error(ss.str());
                    }
                    if (v.type() == json_spirit::array_type) {
                        json_spirit::Array array = v.get_array();
                        if (array.size() != 2) {
                            ss << "Object '" << object << "' must be a JSON array with 2 elements";
                            throw std::runtime_error(ss.str());
                        }
                        vector<json_spirit::Value>::iterator i = array.begin();
                        ceph_assert(i != array.end());
                        if (i->type() != json_spirit::str_type) {
                            ss << "Object '" << object << "' must be a JSON array with the first element a string";
                            throw std::runtime_error(ss.str());
                        }
                        ...
                        if (object_pgidstr != "meta") {
                            ...
                            if (pgidstr.length() > 0) {
                                if (object_pgid != pgid) {
                                    ss << "object '" << object << "' has a pgid different from the --pgid=" << pgidstr
                                       << " option";
                                    throw std::runtime_error(ss.str());
                                }
                            }
    ...
    if ((op == "info" || op == "log" || op == "remove" || op == "export" || op == "export-remove" ||
         op == "mark-complete" || op == "reset-last-complete" || op == "trim-pg-log" || op == "pg-log-inject-dups") &&
        pgidstr.length() == 0) {
    ...
    if (op == "import") {
        ...
        if (ret == 0) cout << "Import successful" << std::endl;
        goto out;
    }
        ...
        if (ret) {
            cerr << "Failed to get osdmap#" << epoch << ": " << cpp_strerror(ret) << std::endl;
            goto out;
        }
        ...
        if (ret < 0) {
            cerr << "Failed to get incremental osdmap# " << epoch << ": " << cpp_strerror(ret) << std::endl;
            goto out;
        }
        ret = bl.write_fd(file_fd);
        if (ret) {
        ...
        if (ret < 0) {
            ...
            auto ch = fs->open_collection(coll_t::meta());
            ret = fs->queue_transaction(ch, std::move(t));
            if (ret < 0) {
    ...
    if (op == "list") {
        ret = do_list(fs, pgidstr, object, nspace, formatter, debug, human_readable, head);
        if (ret < 0) {
    ...
    if (op == "list-slow-omap") {
        ret = do_list_slow(fs, pgidstr, object, slow_threshold, formatter, debug, human_readable);
        if (ret < 0) {
            cerr << "do_list failed: " << cpp_strerror(ret) << std::endl;
        }
        goto out;
    ...
    if (debug && op == "list-pgs") cout << "Performing list-pgs operation" << std::endl;
    ...
    if (!vm.count("objcmd") && op != "export" && op != "export-remove" && op != "info" && op != "log" &&
        op != "mark-complete" && op != "trim-pg-log" && op != "trim-pg-log-dups" && op != "pg-log-inject-dups") {
        cerr << "Must provide --op (info, log, remove, mkfs, fsck, repair, export, export-remove, import, list, "
                "fix-lost, list-pgs, dump-journal, dump-super, meta-list, "
                "get-osdmap, set-osdmap, get-superblock, set-superblock, get-inc-osdmap, set-inc-osdmap, "
                "mark-complete, reset-last-complete, dump-export, trim-pg-log, "
    ...
    if (it != ls.end()) {
        ...
        if (ret < 0) cerr << "peek_map_epoch reports error" << std::endl;
        if (debug) cerr << "map_epoch " << map_epoch << std::endl;
        ...
        if (debug) cerr << "struct_v " << (int)struct_ver << std::endl;
        ...
        if (op == "export" || op == "export-remove") {
            ...
            if (struct_ver < PG::get_compat_struct_v()) {
                cerr << "Can't mark-complete, version mismatch " << (int)struct_ver << " (pg)  < compat "
                     << (int)PG::get_compat_struct_v() << " (tool)" << std::endl;
                ret = 1;
                goto out;
            }
            ...
            ret = do_trim_pg_log(fs, coll, info, pgid, map_epoch, past_intervals);
            if (ret < 0) {
            ...
            ret = do_trim_pg_log_dups(fs, coll, info, pgid, map_epoch, past_intervals);
            if (ret < 0) {
            ...
            if (!force) {
                std::cerr << "WARNING: reset-last-complete is extremely dangerous and almost "
                          ...
                          << "what you are doing. Pass --force to proceed anyway." << std::endl;
                ret = -EINVAL;
                goto out;
            }
            ObjectStore::Transaction tran;
            ...
            if (struct_ver < PG::get_compat_struct_v()) {
                cerr << "Can't reset-last-complete, version mismatch " << (int)struct_ver << " (pg)  < compat "
                     << (int)PG::get_compat_struct_v() << " (tool)" << std::endl;
                ret = 1;
                goto out;
            }
            ...
            if (debug) cerr << "opening file " << file << std::endl;
            ...
            try {
                if (!json_spirit::read(json_file_stream, result)) throw std::runtime_error("unparseable JSON " + file);
