...
int RadosImport::import(librados::IoCtx& io_ctx, bool no_overwrite)
{
    ...
    if (sh.magic != super_header::super_magic) {
        cerr << "Invalid magic number: 0x" << std::hex << sh.magic << " vs. 0x" << super_header::super_magic << std::dec
             << std::endl;
        return -EFAULT;
    }
  ...
  if (sb.compat_features.compare(pgb.superblock.compat_features) == -1) {
    ...
    if (file_fd != STDIN_FILENO) posix_fadvise(file_fd, 0, 0, POSIX_FADV_SEQUENTIAL);
#endif
    ...
    while (!done) {
        ...
        case TYPE_PG_METADATA:
            ...
            found_metadata = true;
            break;
        case TYPE_PG_END: done = true; break;
        case TYPE_POOL_END: done = true; break;
...
#if defined(__linux__)
    if (file_fd != STDIN_FILENO) posix_fadvise(file_fd, 0, 0, POSIX_FADV_DONTNEED);
#endif
    return 0;
}
...
int RadosImport::get_object_rados(librados::IoCtx& ioctx, bufferlist& bl, bool no_overwrite)
{
    ...
    if (align) {
        ...
        if (ret < 0) {
            cerr << "pool_requires_alignment2 failed: " << cpp_strerror(ret) << std::endl;
            return ret;
        }
        ...
        if (need_align) {
            ...
            if (ret < 0) {
                cerr << "pool_required_alignment2 failed: " << cpp_strerror(ret) << std::endl;
                return ret;
            }
    ...
    while (!done) {
        ...
        case TYPE_ATTRS:
            ...
            if (dry_run || skipping) break;
            for (std::map<string, bufferlist>::iterator i = as.data.begin(); i != as.data.end(); ++i) {
                // The user xattrs that we want all begin with "_" with length > 1.
                // Drop key "_" and all attributes that do not start with '_'
                if (i->first == "_" || i->first[0] != '_') continue;
        ...
        case TYPE_OBJECT_END:
            ...
            break;
        default: cerr << "Unexpected section type " << type << std::endl; return -EFAULT;
        }
    }
    return 0;
}
