...
int PoolDump::dump(IoCtx* io_ctx)
{
    ...
    for (; i != i_end; ++i) {
        ...
        while (true) {
            ...
            r = write_section(TYPE_DATA, data_section(offset, outdata.length(), outdata), file_fd);
            if (r != 0) {
                // Output stream error
                return r;
            }
        ...
        if (r < 0) {
            cerr << "error getting xattr set " << oid << ": " << cpp_strerror(r) << std::endl;
            return r;
        }
        // Prepend "_" to mimic how user keys are represented in a pg export
        for (std::map<std::string, bufferlist>::iterator i = raw_xattrs.begin(); i != raw_xattrs.end(); ++i) {
        ...
        if (r < 0) {
            cerr << "error getting omap header " << oid << ": " << cpp_strerror(r) << std::endl;
            return r;
        }
        ...
        do {
            ...
            if (r < 0) {
                cerr << "error getting omap keys " << oid << ": " << cpp_strerror(r) << std::endl;
                return r;
            }
            if (values.size()) {
    ...
    if (file_fd != STDOUT_FILENO) posix_fadvise(file_fd, 0, 0, POSIX_FADV_DONTNEED);
#endif
    return r;
}
