...
{
    out << "usage: rados [options] [commands]\n"
           "POOL COMMANDS\n"
           "   lspools                          list pools\n"
           ...
           "   lock get <obj-name> <lock-name> [--lock-cookie locker-cookie] [--lock-tag locker-tag] "
           "[--lock-description locker-desc] [--lock-duration locker-dur] [--lock-type locker-type]\n"
...
int read([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, buffer::list& out_data, const unsigned op_size,
         const uint64_t offset, [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().read(oid, &out_data, op_size, offset);
#endif

    return io_ctx.read(oid, out_data, op_size, offset);
}

int write([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, buffer::list& indata, const uint64_t count,
          const uint64_t offset, [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().write(oid, indata, count, offset);
#endif

    return io_ctx.write(oid, indata, count, offset);
}

int write_full([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, bufferlist& indata,
               [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().write_full(oid, indata);
#endif

    return io_ctx.write_full(oid, indata);
}

int trunc([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, const uint64_t offset,
          [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().trunc(oid, offset);
#endif

    return io_ctx.trunc(oid, offset);
}

int append([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, buffer::list& indata, const uint64_t count,
           [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().append(oid, indata, count);
#endif

    return io_ctx.append(oid, indata, count);
}

int setxattr([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, const std::string& attr_name, buffer::list& bl,
             [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().setxattr(oid, attr_name.c_str(), bl);
#endif

    return io_ctx.setxattr(oid, attr_name.c_str(), bl);
}

int getxattr([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, const std::string& attr_name, buffer::list& bl,
             [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().getxattr(oid, attr_name.c_str(), bl);
#endif

    return io_ctx.getxattr(oid, attr_name.c_str(), bl);
}

int rmxattr([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, const std::string& attr_name,
            [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().rmxattr(oid, attr_name.c_str());
#endif

    return io_ctx.rmxattr(oid, attr_name.c_str());
}

int getxattrs([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, std::map<std::string, buffer::list>& attrset,
              [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().getxattrs(oid, attrset);
#endif

    return io_ctx.getxattrs(oid, attrset);
}

int remove([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, const int flags,
           [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().remove(oid, flags);
#endif
...
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().remove(oid);
#endif
...
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return i->get_oid().substr(0, i->get_oid().length() - 17);
#endif

    return i->get_oid();
}

int stat([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, uint64_t& size, time_t& mtime,
         [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().stat(oid, &size, &mtime);
#endif

    return io_ctx.stat(oid, &size, &mtime);
}

int stat2([[maybe_unused]] IoCtx& io_ctx, const std::string& oid, uint64_t& size, timespec& mtime,
          [[maybe_unused]] const bool use_striper)
{
#ifdef WITH_LIBRADOSSTRIPER
    if (use_striper) return striper().stat2(oid, &size, &mtime);
#endif
...
template<typename I, typename T> static int rados_sistrtoll(I& i, T* val)
{
    std::string err;
...
static int do_get(IoCtx& io_ctx, const std::string& oid, const char* outfile, unsigned op_size,
                  [[maybe_unused]] const bool use_striper)
{
    int fd;
    if (strcmp(outfile, "-") == 0) {
    ...
    while (true) {
        ...
        if (outdata.length() < op_size) break;
        offset += outdata.length();
    }
    ret = 0;

out:
    if (fd != 1) VOID_TEMP_FAILURE_RETRY(::close(fd));
    return ret;
}
...
static int do_copy_pool(Rados& rados, const char* src_pool, const char* target_pool)
{
    ...
    for (; i != i_end; ++i) {
        ...
        if (locator.size()) src_name += "(@" + locator + ")";
        cout << src_pool << ":" << src_name << " => " << target_pool << ":" << target_name << std::endl;
...
static int do_put(IoCtx& io_ctx, const std::string& oid, const char* infile, int op_size, uint64_t obj_offset,
                  bool create_object, const bool use_striper)
{
    bool stdio = (strcmp(infile, "-") == 0);
    int ret = 0;
    int fd = STDIN_FILENO;
    if (!stdio) fd = open(infile, O_RDONLY | O_BINARY);
    if (fd < 0) {
    ...
    ret = 0;
out:
    if (fd != STDOUT_FILENO) VOID_TEMP_FAILURE_RETRY(close(fd));
    return ret;
}

static int do_append(IoCtx& io_ctx, const std::string& oid, const char* infile, int op_size, const bool use_striper)
{
    bool stdio = (strcmp(infile, "-") == 0);
    int ret = 0;
    int fd = STDIN_FILENO;
...
public:
    RadosWatchCtx(IoCtx& io, const char* imgname)
        : ioctx(io)
        , name(imgname)
    {}
    ...
    {
        cout << "NOTIFY"
             << " cookie " << cookie << " notify_id " << notify_id << " from " << notifier_id << std::endl;
        bl.hexdump(cout);
    ...
    {
        cout << "ERROR"
             << " cookie " << cookie << " err " << cpp_strerror(err) << std::endl;
    }
};
...
public:
    ...
    struct LoadGenOp
    {
        ...
        LoadGenOp()
            ...
            , completion(NULL)
        {}
        explicit LoadGenOp(LoadGen* _lg)
            : id(0)
            , type(0)
            , off(0)
            , len(0)
    ...
    uint64_t cur_sent_rate() { return total_sent / time_passed(); }

    uint64_t cur_completed_rate() { return total_completed / time_passed(); }

    uint64_t total_expected() { return target_throughput * time_passed(); }
    ...
    void io_cb(completion_t c, LoadGenOp* op)
    {
        ...
        if (iter != pending_ops.end()) pending_ops.erase(iter);

        if (!going_down) op->completion->release();
...
void LoadGen::run_op(LoadGenOp* op)
{
    ...
    switch (op->type) {
    case OP_READ: io_ctx.aio_read(op->oid, op->completion, &op->bl, op->len, op->off); break;
    case OP_WRITE:
...
void LoadGen::gen_op(LoadGenOp* op)
{
    ...
    if (len > info.len) len = info.len;
    size_t off = generate_random_number<size_t>(0, info.len);

    if (off + len > info.len) off = info.len - len;

    if (offset_align) off = p2align(off, offset_align);
    ...
    cout << (op->type == OP_READ ? "READ" : "WRITE") << " : oid=" << op->oid << " off=" << op->off << " len=" << op->len
         << std::endl;
}
...
int LoadGen::run()
{
    ...
    while (1) {
        ...
        if (now - stamp_time >= utime_t(1, 0)) {
            ...
            std::streamsize original_precision = cout.precision();
            cout.precision(3);
            cout << setw(5) << total_sec << ": throughput=" << rate << "MB/sec"
                 << " pending data=" << sent - completed << std::endl;
            cout.precision(original_precision);
            stamp_time = now;
        }

        while (sent < expected && sent - completed < max_backlog && pending_ops.size() < max_ops) {
            sent += gen_next_op();
        }
    }
...
void LoadGen::cleanup()
{
    ...
    for (iter = objs.begin(); iter != objs.end(); ++iter) {
        ...
        if (ret < 0) cerr << "couldn't remove obj: " << info.name << " ret=" << ret << std::endl;
    }
}
...
protected:
    ...
    int create_completion(int slot, void (*cb)(void*, void*), void* arg) override
    {
        ...
        if (!completions[slot]) return -EINVAL;
    ...
    int aio_read(const std::string& oid, int slot, bufferlist* pbl, size_t len, size_t offset) override
    {
        return io_ctx.aio_read(oid, completions[slot], pbl, len, offset);
    }

    int aio_write(const std::string& oid, int slot, bufferlist& bl, size_t len, size_t offset) override
    {
        librados::ObjectWriteOperation op;

        if (write_destination & OP_WRITE_DEST_OBJ) {
            if (data.hints)
                op.set_alloc_hint2(data.object_size,
                                   data.op_size,
                                   ALLOC_HINT_FLAG_SEQUENTIAL_WRITE | ALLOC_HINT_FLAG_SEQUENTIAL_READ |
                                       ALLOC_HINT_FLAG_APPEND_ONLY | ALLOC_HINT_FLAG_IMMUTABLE);
            op.write(offset, bl);
        }
    ...
    int aio_remove(const std::string& oid, int slot) override { return io_ctx.aio_remove(oid, completions[slot]); }

    int sync_read(const std::string& oid, bufferlist& bl, size_t len) override { return io_ctx.read(oid, bl, len, 0); }
    int sync_write(const std::string& oid, bufferlist& bl, size_t len) override { return io_ctx.write_full(oid, bl); }

    int sync_remove(const std::string& oid) override { return io_ctx.remove(oid); }

    bool completion_is_done(int slot) override { return completions[slot] && completions[slot]->is_complete(); }

    int completion_wait(int slot) override { return completions[slot]->wait_for_complete_and_cb(); }
    int completion_ret(int slot) override { return completions[slot]->get_return_value(); }
    ...
    void set_namespace(const std::string& ns) override { io_ctx.set_namespace(ns); }
...
public:
    RadosBencher(CephContext* cct_, librados::Rados& _r, librados::IoCtx& _i)
        ...
        , io_ctx(_i)
        , iterator_valid(false)
        , write_destination(OP_WRITE_DEST_OBJ)
    {}
    ~RadosBencher() override {}

    void set_write_destination(OpWriteDest dest) { write_destination = dest; }
};

static int do_lock_cmd(std::vector<const char*>& nargs, const std::map<std::string, std::string>& opts, IoCtx* ioctx,
                       Formatter* formatter)
{
    if (nargs.size() < 3) usage_exit();
    ...
    if (cmd.compare("info") == 0) {
        ...
        int ret;
        switch (lock_type) {
        case ClsLockType::SHARED: ret = l.lock_shared(ioctx, oid); break;
        default: ret = l.lock_exclusive(ioctx, oid);
        }
        if (ret < 0) {
...
static int do_cache_flush(IoCtx& io_ctx, string oid)
{
    ...
    io_ctx.aio_operate(
        oid.c_str(), completion, &op, librados::OPERATION_IGNORE_CACHE | librados::OPERATION_IGNORE_OVERLAY, NULL);
...
static int do_cache_try_flush(IoCtx& io_ctx, string oid)
{
    ...
    io_ctx.aio_operate(
        ...
        completion,
        &op,
        librados::OPERATION_IGNORE_CACHE | librados::OPERATION_IGNORE_OVERLAY | librados::OPERATION_SKIPRWLOCKS,
        NULL);
    completion->wait_for_complete();
    int r = completion->get_return_value();
    completion->release();
...
static int do_cache_flush_evict_all(IoCtx& io_ctx, bool blocking)
{
    ...
    try {
        ...
        for (; i != i_end; ++i) {
            ...
            if (r < 0) {
                cerr << "error listing snap shots " << i->get_nspace() << "/" << i->get_oid() << ": " << cpp_strerror(r)
                     << std::endl;
                ++errors;
                continue;
            }
            ...
            if (ci == ls.clones.end()) {
                ...
                if (r < 0) {
                    cerr << "failed to flush " << i->get_nspace() << "/" << i->get_oid() << ": " << cpp_strerror(r)
                         << std::endl;
                    ++errors;
                    continue;
                }
                ...
                if (r < 0) {
                    cerr << "failed to evict " << i->get_nspace() << "/" << i->get_oid() << ": " << cpp_strerror(r)
                         << std::endl;
                    ++errors;
                    continue;
                }
                ...
                // has snapshots
                for (std::vector<clone_info_t>::iterator ci = ls.clones.begin(); ci != ls.clones.end(); ++ci) {
...
static int do_get_inconsistent_pg_cmd(const std::vector<const char*>& nargs, Rados& rados, Formatter& formatter)
{
    if (nargs.size() < 2) {
        usage_exit();
    }
...
static void dump_errors(const err_t& err, Formatter& f, const char* name)
{
    ...
    if (err.has_shard_missing()) f.dump_string("error", "missing");
    if (err.has_stat_error()) f.dump_string("error", "stat_error");
    if (err.has_read_error()) f.dump_string("error", "read_error");
    if (err.has_data_digest_mismatch_info()) f.dump_string("error", "data_digest_mismatch_info");
    if (err.has_omap_digest_mismatch_info()) f.dump_string("error", "omap_digest_mismatch_info");
    if (err.has_size_mismatch_info()) f.dump_string("error", "size_mismatch_info");
    if (err.has_ec_hash_error()) f.dump_string("error", "ec_hash_error");
    if (err.has_ec_size_error()) f.dump_string("error", "ec_size_error");
    if (err.has_info_missing()) f.dump_string("error", "info_missing");
    if (err.has_info_corrupted()) f.dump_string("error", "info_corrupted");
    if (err.has_obj_size_info_mismatch()) f.dump_string("error", "obj_size_info_mismatch");
    if (err.has_snapset_missing()) f.dump_string("error", "snapset_missing");
    if (err.has_snapset_corrupted()) f.dump_string("error", "snapset_corrupted");
    if (err.has_hinfo_missing()) f.dump_string("error", "hinfo_missing");
    if (err.has_hinfo_corrupted()) f.dump_string("error", "hinfo_corrupted");
    f.close_section();
}

static void dump_shard(const shard_info_t& shard, const inconsistent_obj_t& inc, Formatter& f)
{
    dump_errors(shard, f, "errors");

    if (shard.has_shard_missing()) return;

    if (!shard.has_stat_error()) f.dump_unsigned("size", shard.size);
    ...
    if ((inc.union_shards.has_info_missing() || inc.union_shards.has_info_corrupted() ||
         inc.has_object_info_inconsistency() || shard.has_obj_size_info_mismatch()) &&
    ...
    if ((inc.union_shards.has_snapset_missing() || inc.union_shards.has_snapset_corrupted() ||
    ...
    if ((inc.union_shards.has_hinfo_missing() || inc.union_shards.has_hinfo_corrupted() ||
         inc.has_hinfo_inconsistency()) &&
        !shard.has_hinfo_missing()) {
        map<std::string, ceph::bufferlist>::iterator k =
            (const_cast<shard_info_t&>(shard)).attrs.find(ECUtil::get_hinfo_key());
        ceph_assert(k != shard.attrs.end());   // Can't be missing
    ...
    if (inc.has_attr_name_mismatch() || inc.has_attr_value_mismatch()) {
        ...
        for (auto kv : shard.attrs) {
            ...
            if (kv.first == OI_ATTR || kv.first[0] != '_') continue;
...
static void dump_obj_errors(const obj_err_t& err, Formatter& f)
{
    ...
    if (err.has_object_info_inconsistency()) f.dump_string("error", "object_info_inconsistency");
    if (err.has_data_digest_mismatch()) f.dump_string("error", "data_digest_mismatch");
    if (err.has_omap_digest_mismatch()) f.dump_string("error", "omap_digest_mismatch");
    if (err.has_size_mismatch()) f.dump_string("error", "size_mismatch");
    if (err.has_attr_value_mismatch()) f.dump_string("error", "attr_value_mismatch");
    if (err.has_attr_name_mismatch()) f.dump_string("error", "attr_name_mismatch");
    if (err.has_snapset_inconsistency()) f.dump_string("error", "snapset_inconsistency");
    if (err.has_hinfo_inconsistency()) f.dump_string("error", "hinfo_inconsistency");
    if (err.has_size_too_large()) f.dump_string("error", "size_too_large");
    f.close_section();
}

static void dump_object_id(const object_id_t& object, Formatter& f)
{
    f.dump_string("name", object.name);
    f.dump_string("nspace", object.nspace);
    f.dump_string("locator", object.locator);
    switch (object.snap) {
    case CEPH_NOSNAP: f.dump_string("snap", "head"); break;
    case CEPH_SNAPDIR: f.dump_string("snap", "snapdir"); break;
    default: f.dump_unsigned("snap", object.snap); break;
    }
}

static void dump_inconsistent(const inconsistent_obj_t& inc, Formatter& f)
{
    ...
    for (const auto& shard_info : inc.shards) {
        ...
        if (shard != shard_id_t::NO_SHARD) f.dump_unsigned("shard", shard);
...
static void dump_inconsistent(const inconsistent_snapset_t& inc, Formatter& f)
{
    dump_object_id(inc.object, f);
    ...
    f.open_array_section("errors");
    if (inc.snapset_missing()) f.dump_string("error", "snapset_missing");
    if (inc.snapset_corrupted()) f.dump_string("error", "snapset_corrupted");
    if (inc.info_missing()) f.dump_string("error", "info_missing");
    if (inc.info_corrupted()) f.dump_string("error", "info_corrupted");
    if (inc.snapset_error()) f.dump_string("error", "snapset_error");
    if (inc.headless()) f.dump_string("error", "headless");
    if (inc.size_mismatch()) f.dump_string("error", "size_mismatch");
    if (inc.extra_clones()) f.dump_string("error", "extra_clones");
    if (inc.clone_missing()) f.dump_string("error", "clone_missing");
    f.close_section();
...
static int do_get_inconsistent(Rados& rados, const PlacementGroup& pg, const librados::object_id_t& start,
                               unsigned max_return, AioCompletion* c, std::vector<inconsistent_obj_t>* objs,
                               uint32_t* interval)
{
    return rados.get_inconsistent_objects(pg, start, max_return, c, objs, interval);
}

static int do_get_inconsistent(Rados& rados, const PlacementGroup& pg, const librados::object_id_t& start,
                               unsigned max_return, AioCompletion* c, std::vector<inconsistent_snapset_t>* snapsets,
                               uint32_t* interval)
{
    return rados.get_inconsistent_snapsets(pg, start, max_return, c, snapsets, interval);
}

template<typename T>
static int do_get_inconsistent_cmd(const std::vector<const char*>& nargs, Rados& rados, Formatter& formatter)
{
    if (nargs.size() < 2) {
        usage_exit();
    }
    ...
    for (librados::object_id_t start;;) {
        ...
        ret = do_get_inconsistent(rados, pg, start, max_item_num, completion, &items, &interval);
        completion->wait_for_complete();
...
{
    if (std::find_if_not(s.begin(), s.end(), (int (*)(int))isprint) != s.end()) {
        return "(binary key)";
    }
    else {
        return s;
...
static int rados_tool_common(const std::map<std::string, std::string>& opts, std::vector<const char*>& nargs)
{
    ...
    if (pool_name || pgid) {
        ...
        // align op_size
        {
            ...
            if (ret < 0) {
                cerr << "error checking pool alignment requirement" << cpp_strerror(ret) << std::endl;
                return 1;
            }
            ...
            if (requires) {
                ...
                if (ret < 0) {
                    cerr << "error getting pool alignment" << cpp_strerror(ret) << std::endl;
                    return 1;
                }
        ...
        if (opts.find("striper") != opts.end()) {
            ...
            if (0 != ret) {
                cerr << "error opening pool " << pool_name << " with striper interface: " << cpp_strerror(ret)
                     << std::endl;
                return 1;
            }
            use_striper = true;
    ...
    if (snapid != CEPH_NOSNAP) {
        ...
        if (ret < 0) {
            cerr << "snapid " << snapid << " doesn't exist in pool " << io_ctx.get_pool_name() << std::endl;
            return 1;
        }
    ...
    if (strcmp(nargs[0], "lspools") == 0) {
        ...
        for (list<string>::iterator i = vec.begin(); i != vec.end(); ++i) cout << *i << std::endl;
    }
        ...
        for (map<string, librados::pool_stat_t>::iterator i = stats.begin(); i != stats.end(); ++i) {
            ...
            if (!formatter) {
                tab << pool_name << byte_u_t(s.num_bytes) << s.num_objects << s.num_object_clones << s.num_object_copies
                    << s.num_objects_missing_on_primary << s.num_objects_unfound << s.num_objects_degraded << s.num_rd
                    << byte_u_t(s.num_rd_kb << 10) << s.num_wr << byte_u_t(s.num_wr_kb << 10)
                    << byte_u_t(s.compressed_bytes_alloc) << byte_u_t(s.compressed_bytes_orig) << TextTable::endrow;
            }
            else {
        ...
        if (!formatter) {
            cout << std::endl;
            cout << "total_objects    " << tstats.num_objects << std::endl;
            cout << "total_used       " << byte_u_t(tstats.kb_used << 10) << std::endl;
            cout << "total_avail      " << byte_u_t(tstats.kb_avail << 10) << std::endl;
            cout << "total_space      " << byte_u_t(tstats.kb << 10) << std::endl;
        }
        else {
            formatter->close_section();
            ...
            try {
                ...
                for (; i != i_end; ++i) {
                    ...
                    if (use_striper) {
                        ...
                        if (l <= 17 || (0 != i->get_oid().compare(l - 17, 17, ".0000000000000000"))) {
                            continue;
                        }
                    }
        ...
        if (ret < 0) {
            cerr << "mapext error on " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
        ...
        if (ret < 0) {
            cerr << " error stat-ing " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
        else {
            utime_t t(mtime, 0);
            cout << pool_name << "/" << prettify(*obj_name) << " mtime " << t << ", size " << size << std::endl;
        }
    }
        ...
        if (ret < 0) {
            cerr << " error touch-ing " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
    }
        ...
        if (ret < 0) {
            cerr << "error getting " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
    }
        ...
        if (ret < 0) {
            cerr << "error putting " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
    }
        ...
        if (ret < 0) {
            cerr << "error appending " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
    }
        ...
        if (ret < 0) {
            cerr << "error truncating oid " << prettify(*obj_name) << " to " << size << ": " << cpp_strerror(ret)
                 << std::endl;
        }
        else {
            ret = 0;
        ...
        if (!pool_name || nargs.size() < (obj_name ? 2 : 3) || nargs.size() > (obj_name ? 3 : 4)) {
            usage(cerr);
            return 1;
        }
        ...
        if (ret < 0) {
            cerr << "error setting xattr " << pool_name << "/" << prettify(*obj_name) << "/" << attr_name << ": "
                 << cpp_strerror(ret) << std::endl;
            return 1;
        }
        else
        ...
        if (ret < 0) {
            cerr << "error getting xattr " << pool_name << "/" << prettify(*obj_name) << "/" << attr_name << ": "
                 << cpp_strerror(ret) << std::endl;
            return 1;
        }
        else
        ...
        if (ret < 0) {
            cerr << "error removing xattr " << pool_name << "/" << prettify(*obj_name) << "/" << attr_name << ": "
                 << cpp_strerror(ret) << std::endl;
            return 1;
        }
    }
        ...
        if (ret < 0) {
            cerr << "error getting xattr set " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }

        for (map<std::string, bufferlist>::iterator iter = attrset.begin(); iter != attrset.end(); ++iter) {
        ...
        if (ret < 0) {
            cerr << "error getting omap header " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
        else {
        ...
        if (ret < 0) {
            cerr << "error setting omap value " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
        else {
        ...
        if (ret < 0) {
            cerr << "error setting omap value " << pool_name << "/" << oid << "/" << prettify(*omap_key) << ": "
                 << cpp_strerror(ret) << std::endl;
            return 1;
        }
        else {
        ...
        uint32_t min_args = (omap_key ? (obj_name ? 1 : 2) : (obj_name ? 2 : 3));
        if (!pool_name || nargs.size() < min_args || nargs.size() > min_args + 1) {
            usage(cerr);
            return 1;
        }
        ...
        if (ret < 0) {
            cerr << "error getting omap value " << pool_name << "/" << prettify(*obj_name) << "/" << prettify(*omap_key)
                 << ": " << cpp_strerror(ret) << std::endl;
            return 1;
        }
        else {
        ...
        if (values.size() && values.begin()->first == *omap_key) {
            ...
            cout << "No such key: " << pool_name << "/" << prettify(*obj_name) << "/" << prettify(*omap_key)
                 << std::endl;
            return 1;
        }
    }
    else if (strcmp(nargs[0], "rmomapkey") == 0) {
        uint32_t num_args = (omap_key ? (obj_name ? 1 : 2) : (obj_name ? 2 : 3));
        ...
        if (ret < 0) {
            cerr << "error removing omap key " << pool_name << "/" << prettify(*obj_name) << "/" << prettify(*omap_key)
                 << ": " << cpp_strerror(ret) << std::endl;
            return 1;
        }
        else {
        ...
        std::vector<std::string> oids(std::next(std::begin(nargs)), std::end(nargs));
        if (obj_name) {
        ...
        for (const auto& oid : oids) {
            ...
            if (ret < 0) {
                cerr << "error clearing omap keys " << pool_name << "/" << prettify(oid) << "/" << cpp_strerror(ret)
                     << std::endl;
                return 1;
            }
        }
        ...
        do {
            ...
            ret = values.size();
            for (map<string, bufferlist>::const_iterator it = values.begin(); it != values.end(); ++it) {
                last_read = it->first;
                // dump key in hex if it contains nonprintable characters
                if (std::count_if(it->first.begin(), it->first.end(), (int (*)(int))isprint) <
                    (int)it->first.length()) {
        ...
        if (ret < 0) {
            cerr << "error opening target pool " << target << ": " << cpp_strerror(ret) << std::endl;
            return 1;
        }
        if (target_oloc.size()) {
        ...
        if (ret < 0) {
            cerr << "error copying " << pool_name << "/" << nargs[1] << " => " << target << "/" << target_obj << ": "
                 << cpp_strerror(ret) << std::endl;
            return 1;
        }
    }
        ...
        for (const auto& oid : oids) {
            if (forcefull) {
                ret = detail::remove(io_ctx, oid, (CEPH_OSD_FLAG_FULL_FORCE | CEPH_OSD_FLAG_FULL_TRY), use_striper);
            }
            else {
        ...
        if (ret < 0) {
            cerr << "error creating " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
    }
        ...
        if (ret < 0) {
            cerr << "failed to query pool " << src_pool << " for selfmanaged snaps: " << cpp_strerror(ret) << std::endl;
            return 1;
        }
        else if (ret > 0) {
            cerr << "WARNING: pool " << src_pool << " has selfmanaged snaps, which are not preserved\n"
                 << "    by the cppool operation.  This will break any snapshot user." << std::endl;
            if (!force) {
                cerr << "    If you insist on making a broken copy, you can pass\n"
                     << "    --yes-i-really-mean-it to proceed anyway." << std::endl;
                return 1;
            }
        }
        ...
        if (ret < 0) {
            cerr << "error copying pool " << src_pool << " => " << target_pool << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
        ...
        if (nargs.size() < 3 || strcmp(nargs[2], "--yes-i-really-really-mean-it") != 0) {
            cerr << "WARNING:\n"
        ...
        if (ret < 0) {
            cerr << "error pool " << nargs[1] << ": " << cpp_strerror(ret) << std::endl;
            return 1;
        }
        io_ctx.set_namespace(all_nspaces);
        ...
        for (vector<snap_t>::iterator i = snaps.begin(); i != snaps.end(); ++i) {
            string s;
            time_t t;
            if (io_ctx.snap_get_name(*i, &s) < 0) continue;
            if (io_ctx.snap_get_stamp(*i, &t) < 0) continue;
            struct tm bdt;
            localtime_r(&t, &bdt);
            cout << *i << "\t" << s << "\t";
            ...
            cout.fill('0');
            cout << std::setw(4) << (bdt.tm_year + 1900) << '.' << std::setw(2) << (bdt.tm_mon + 1) << '.'
                 << std::setw(2) << bdt.tm_mday << ' ' << std::setw(2) << bdt.tm_hour << ':' << std::setw(2)
                 << bdt.tm_min << ':' << std::setw(2) << bdt.tm_sec << std::endl;
        ...
        if (ret < 0) {
            cerr << "failed to query pool " << pool_name << " for selfmanaged snaps: " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
        else if (ret > 0) {
            cerr << "can't create snapshot: pool " << pool_name << " is in selfmanaged snaps mode" << std::endl;
            return 1;
        }

        ret = io_ctx.snap_create(nargs[1]);
        if (ret < 0) {
            cerr << "error creating pool " << pool_name << " snapshot " << nargs[1] << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
        ...
        if (ret < 0) {
            cerr << "error removing pool " << pool_name << " snapshot " << nargs[1] << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
        ...
        if (ret < 0) {
            cerr << "error rolling back pool " << pool_name << " to snapshot " << nargs[1] << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
        cout << "rolled back pool " << pool_name << " to snapshot " << nargs[2] << std::endl;
    }
        ...
        if (operation != OP_WRITE) {
            if (block_size_specified) {
                cerr << "-b|--block_size option can be used only with 'write' bench test" << std::endl;
                return 1;
            }
        ...
        if (!formatter && output) {
            cerr << "-o|--output option can only be used with '--format' option" << std::endl;
            return 1;
        }
        RadosBencher bencher(g_ceph_context, rados, io_ctx);
        ...
        ret = bencher.aio_bench(operation,
                                seconds,
                                concurrent_ios,
                                op_size,
                                object_size,
                                ...
                                reuse_bench,
                                no_verify);
        if (ret != 0) cerr << "error during benchmark: " << cpp_strerror(ret) << std::endl;
        if (formatter && output) delete outstream;
    }
        ...
        if (wildcard) io_ctx.set_namespace(all_nspaces);
        RadosBencher bencher(g_ceph_context, rados, io_ctx);
        ret = bencher.clean_up(prefix, concurrent_ios, run_name);
        if (ret != 0) cerr << "error during cleanup: " << cpp_strerror(ret) << std::endl;
    }
        ...
        if (ret != 0) cerr << "error calling notify: " << cpp_strerror(ret) << std::endl;
        if (replybl.length()) {
            ...
            decode(rm, p);
            decode(missed, p);
            for (map<pair<uint64_t, uint64_t>, bufferlist>::iterator p = rm.begin(); p != rm.end(); ++p) {
                cout << "reply client." << p->first.first << " cookie " << p->first.second << " : "
                     << p->second.length() << " bytes" << std::endl;
                if (p->second.length()) p->second.hexdump(cout);
            }
            for (multiset<pair<uint64_t, uint64_t> >::iterator p = missed.begin(); p != missed.end(); ++p) {
                cout << "timeout client." << p->first << " cookie " << p->second << std::endl;
            }
        }
    }
        ...
        if (ret < 0) {
            cerr << "error setting alloc-hint " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
    }
        ...
        LoadGen lg(&rados);
        if (min_obj_len) lg.min_obj_len = min_obj_len;
        if (max_obj_len) lg.max_obj_len = max_obj_len;
        if (min_op_len) lg.min_op_len = min_op_len;
        if (max_op_len) lg.max_op_len = max_op_len;
        if (max_ops) lg.max_ops = max_ops;
        if (max_backlog) lg.max_backlog = max_backlog;
        if (target_throughput) lg.target_throughput = target_throughput;
        if (read_percent >= 0) lg.read_percent = read_percent;
        if (num_objs) lg.num_objs = num_objs;
        if (run_length) lg.run_length = run_length;
        if (offset_align) lg.offset_align = offset_align;
        ...
        do {
            ...
            if (ret < 0) {
                cerr << "error getting omap key set " << pool_name << "/" << prettify(*obj_name) << ": "
                     << cpp_strerror(ret) << std::endl;
                return 1;
            }
        ...
        if (ret < 0) {
            cerr << "error listing snap shots " << pool_name << "/" << prettify(*obj_name) << ": " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
        else
        ...
        for (std::vector<clone_info_t>::iterator ci = ls.clones.begin(); ci != ls.clones.end(); ++ci) {
            ...
            for (std::vector<snap_t>::const_iterator snapindex = ci->snaps.begin(); snapindex != ci->snaps.end();
                 ++snapindex) {
                ...
                if (formatter) {
                    ...
                    if (si != snamemap.end()) formatter->dump_string("name", si->second);
        ...
        if (with_clones) {
            ...
            for (std::vector<clone_info_t>::iterator ci = ls.clones.begin(); ci != ls.clones.end(); ++ci) {
                if (snapid != CEPH_NOSNAP && ci->cloneid > snapid) break;
                io_ctx.snap_set_read(ci->cloneid);
                ret = do_cache_flush(io_ctx, *obj_name);
                if (ret < 0) {
                    cerr << "error from cache-flush " << prettify(*obj_name) << ": " << cpp_strerror(ret) << std::endl;
                    return 1;
                }
            }
        }
        ...
        if (ret < 0) {
            cerr << "error set-redirect " << pool_name << "/" << nargs[1] << " => " << target << "/" << target_obj
                 << ": " << cpp_strerror(ret) << std::endl;
            return 1;
        }
    }
        ...
        if (ret < 0) {
            cerr << "error set-chunk " << pool_name << "/" << nargs[1] << " "
                 << " offset " << offset << " length " << length << " target_pool " << target
        ...
        ret = io_ctx.operate(*obj_name, &op);
        if (ret < 0) {
            cerr << "error tier-promote " << pool_name << "/" << prettify(*obj_name) << " : " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
    }
        ...
        ret = io_ctx.operate(*obj_name, &op);
        if (ret < 0) {
            cerr << "error unset-manifest " << pool_name << "/" << prettify(*obj_name) << " : " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
    }
        ...
        librados::AioCompletion* completion = librados::Rados::aio_create_completion();
        io_ctx.aio_operate(
            *obj_name, completion, &op, librados::OPERATION_IGNORE_CACHE | librados::OPERATION_IGNORE_OVERLAY, NULL);
        completion->wait_for_complete();
        ret = completion->get_return_value();
        completion->release();
        if (ret < 0) {
            cerr << "error tier-flush " << pool_name << "/" << prettify(*obj_name) << " : " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
    }
        ...
        completion->release();
        if (ret < 0) {
            cerr << "error tier-evict " << pool_name << "/" << prettify(*obj_name) << " : " << cpp_strerror(ret)
                 << std::endl;
            return 1;
        }
    }
        ...
        if (nargs.size() < 2 || std::string(nargs[1]) == "-") {
            ...
            if (file_fd < 0) {
                cerr << "Error opening '" << nargs[1] << "': " << cpp_strerror(file_fd) << std::endl;
                return 1;
            }
        }
        ...
        if (ret < 0) {
            cerr << "error from export: " << cpp_strerror(ret) << std::endl;
            return 1;
        }
    }
        ...
        if (filename == "-") {
            ...
            if (file_fd < 0) {
                cerr << "Error opening '" << filename << "': " << cpp_strerror(file_fd) << std::endl;
                return 1;
            }
        }
        ...
        if (ret < 0) {
            cerr << "error from import: " << cpp_strerror(ret) << std::endl;
            return 1;
        }
    }
    else {
    ...
    if (ret < 0) cerr << "error " << (-ret) << ": " << cpp_strerror(ret) << std::endl;
...
int main(int argc, const char** argv)
{
    ...
    for (i = args.begin(); i != args.end();) {
        ...
        else if (ceph_argparse_flag(args, i, "-C", "--create", "--create-pool", (char*)NULL)) {
            opts["create"] = "true";
        }
