...
std::ostream& operator<<(std::ostream& os, const NBDServer::IOContext& ctx)
{
    ...
    os << ctx.request.from << "~" << ctx.request.len << " " << std::dec << ntohl(ctx.reply.error) << "]";
        ...
        if (image.stat(info, sizeof(info)) == 0) {
            ...
            if (new_size != size) {
                ...
                if (use_netlink) {
                    ...
                    if (ret < 0) derr << "resize failed: " << cpp_strerror(errno) << dendl;
                }
                ...
                if (image.invalidate_cache() < 0) derr << "invalidate rbd cache failed" << dendl;
            }
        }
    }
};
...
static int do_map(int argc, const char* argv[], Config* cfg, bool reconnect)
{
    ...
    if (cfg->encryption_format.has_value()) {
        ...
        if (file.fail()) {
            r = -errno;
            std::cerr << "rbd-nbd: unable to open passphrase file:" << cpp_strerror(errno) << std::endl;
            goto close_fd;
        }
        std::string passphrase((std::istreambuf_iterator<char>(file)), (std::istreambuf_iterator<char>()));
        auto sg = make_scope_guard([&] { ceph_memzero_s(&passphrase[0], passphrase.size(), passphrase.size()); });
        file.close();
        ...
        case RBD_ENCRYPTION_FORMAT_LUKS1:
        {
            ...
            r = image.encryption_load(RBD_ENCRYPTION_FORMAT_LUKS1, &opts, sizeof(opts));
            break;
        }
        case RBD_ENCRYPTION_FORMAT_LUKS2:
        {
            ...
            opts.passphrase = passphrase;
            r = image.encryption_load(RBD_ENCRYPTION_FORMAT_LUKS2, &opts, sizeof(opts));
            blksize = 4096;
            break;
        }
        default:
...
static int do_list_mapped_devices(const std::string& format, bool pretty_format)
{
    ...
    while (it.get(&cfg)) {
        if (f) {
            ...
            tbl << cfg.pid << cfg.poolname << cfg.nsname << cfg.imgname << cfg.snapname << cfg.devpath << cfg.cookie
                << TextTable::endrow;
        }
    }
