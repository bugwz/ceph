...
public:
    ...
    Config* get_cfg() const { return cfg; }
...
private:
    ...
    IOContext* wait_io_finish()
    {
        ...
        cond.wait(l, [this] { return !io_finished.empty() || (io_pending.empty() && terminated); });

        if (io_finished.empty()) return NULL;
    ...
    {
        librbd::RBD::AioCompletion* aio_completion = reinterpret_cast<librbd::RBD::AioCompletion*>(cb);
        ...
        else if ((ctx->command == NBD_CMD_READ) && ret < static_cast<int>(ctx->request.len)) {
    ...
    void reader_entry()
    {
        ...
        while (true) {
            ...
            switch (pctx->command) {
            case NBD_CMD_WRITE: image.aio_write(pctx->request.from, pctx->request.len, pctx->data, c); break;
            case NBD_CMD_READ: image.aio_read(pctx->request.from, pctx->request.len, pctx->data, c); break;
            case NBD_CMD_FLUSH:
                image.aio_flush(c);
                allow_internal_flush = true;
                break;
            case NBD_CMD_TRIM: image.aio_discard(pctx->request.from, pctx->request.len, c); break;
            default:
...
public:
    void start()
    {
        if (!started) {
            ...
            int r = terminate_event_sock.init(terminate_event_fd, EVENT_SOCKET_TYPE_EVENTFD);
            ceph_assert(r >= 0);
...
std::ostream& operator<<(std::ostream& os, const NBDServer::IOContext& ctx)
{
    ...
    switch (ctx.command) {
    case NBD_CMD_WRITE: os << " WRITE "; break;
    case NBD_CMD_READ: os << " READ "; break;
    case NBD_CMD_FLUSH: os << " FLUSH "; break;
    case NBD_CMD_TRIM: os << " TRIM "; break;
    case NBD_CMD_DISC: os << " DISC "; break;
    default: os << " UNKNOWN(" << ctx.command << ") "; break;
    }

    os << ctx.request.from << "~" << ctx.request.len << " " << std::dec << big_to_native(ctx.reply.error) << "]";
...
public:
    ...
    void handle_quiesce() override { server->notify_quiesce(); }

    void handle_unquiesce() override { server->notify_unquiesce(); }
...
public:
    NBDWatchCtx(int _fd, int _nbd_index, bool _use_netlink, librados::IoCtx& _io_ctx, librbd::Image& _image,
                unsigned long _size)
        : fd(_fd)
        ...
        , size(_size)
    {
        handle_notify_thread = make_named_thread("rbd_handle_notify", &NBDWatchCtx::handle_notify_entry, this);
    }
...
struct EncryptionOptions
{
    ...
    ~EncryptionOptions()
    {
        for (auto& spec : specs) {
            ...
            {
                auto opts = static_cast<librbd::encryption_luks_format_options_t*>(spec.opts);
                ceph_memzero_s(opts->passphrase.data(), opts->passphrase.size(), opts->passphrase.size());
                delete opts;
                break;
            }
            case RBD_ENCRYPTION_FORMAT_LUKS1:
            {
                auto opts = static_cast<librbd::encryption_luks1_format_options_t*>(spec.opts);
                ceph_memzero_s(opts->passphrase.data(), opts->passphrase.size(), opts->passphrase.size());
                delete opts;
                break;
            }
            ...
            {
                auto opts = static_cast<librbd::encryption_luks2_format_options_t*>(spec.opts);
                ceph_memzero_s(opts->passphrase.data(), opts->passphrase.size(), opts->passphrase.size());
                delete opts;
                break;
            }
...
static int load_module(Config* cfg)
{
    ...
    if (cfg->nbds_max) param << "nbds_max=" << cfg->nbds_max;

    if (cfg->max_part) param << " max_part=" << cfg->max_part;
    ...
    if (ret < 0) cerr << "rbd-nbd: failed to load nbd kernel module: " << cpp_strerror(-ret) << std::endl;
...
static int check_device_size(int nbd_index, unsigned long expected_size)
{
    ...
    if (size != expected_size) {
        cerr << "rbd-nbd: kernel reported invalid device size (" << size << ", expected " << expected_size << ")"
             << std::endl;
        return -EINVAL;
    }
...
static int parse_nbd_index(const std::string& devpath)
{
    ...
    if (ret <= 0) {
        ...
        if (ret == 0) ret = -EINVAL;
        cerr << "rbd-nbd: invalid device path: " << devpath << " (expected /dev/nbd{num})" << std::endl;
        return ret;
    }
...
static int try_ioctl_setup(Config* cfg, int fd, uint64_t size, uint64_t blksize, uint64_t flags)
{
    int index = 0, r;
    ...
    if (cfg->io_timeout >= 0) {
        ...
        if (r < 0) {
            r = -errno;
            cerr << "rbd-nbd: failed to set IO timeout: " << cpp_strerror(r) << std::endl;
            goto close_nbd;
        }
    }
...
static struct nl_sock* netlink_init(int* id)
{
    ...
    if (ret < 0) {
        cerr << "rbd-nbd: Could not connect netlink socket. Error " << ret << std::endl;
        goto free_sock;
    }
...
static int netlink_disconnect(int index)
{
    ...
    if (!genlmsg_put(msg, NL_AUTO_PORT, NL_AUTO_SEQ, nl_id, 0, 0, NBD_CMD_DISCONNECT, 0)) {
    ...
    if (ret < 0) {
        cerr << "rbd-nbd: netlink disconnect failed: " << nl_geterror(-ret) << std::endl;
        return -EIO;
    }
...
static int netlink_resize(int nbd_index, uint64_t size)
{
    ...
    if (!genlmsg_put(msg, NL_AUTO_PORT, NL_AUTO_SEQ, nl_id, 0, 0, NBD_CMD_RECONFIGURE, 0)) {
        cerr << "rbd-nbd: Could not setup message." << std::endl;
        goto free_msg;
    }
...
static int netlink_connect(Config* cfg, struct nl_sock* sock, int nl_id, int fd, uint64_t size, uint64_t flags,
                           bool reconnect)
{
    ...
    if (!genlmsg_put(
            msg, NL_AUTO_PORT, NL_AUTO_SEQ, nl_id, 0, 0, reconnect ? NBD_CMD_RECONFIGURE : NBD_CMD_CONNECT, 0)) {
        cerr << "rbd-nbd: Could not setup message." << std::endl;
        goto free_msg;
    }
    ...
    if (cfg->io_timeout >= 0) NLA_PUT_U64(msg, NBD_ATTR_TIMEOUT, cfg->io_timeout);
    ...
    if (!cfg->cookie.empty()) NLA_PUT_STRING(msg, NBD_ATTR_BACKEND_IDENTIFIER, cfg->cookie.c_str());
    ...
    ret = nl_send_sync(sock, msg);
    if (ret < 0) {
        cerr << "rbd-nbd: netlink connect failed: " << nl_geterror(ret) << std::endl;
        return -EIO;
    }
...
static int try_netlink_setup(Config* cfg, int fd, uint64_t size, uint64_t flags, bool reconnect)
{
    struct nl_sock* sock;
    int nl_id, ret;
    ...
    sock = netlink_init(&nl_id);
    if (!sock) {
        cerr << "rbd-nbd: Netlink interface not supported. Using ioctl interface." << std::endl;
        return 1;
    }
...
static int run_quiesce_hook(const std::string& quiesce_hook, const std::string& devpath, const std::string& command)
{
    ...
    SubProcess hook(quiesce_hook.c_str(), SubProcess::CLOSE, SubProcess::PIPE, SubProcess::PIPE);
    ...
    if (r < 0) {
        derr << __func__ << ": " << quiesce_hook << " " << devpath << " " << command << " failed: " << err.to_str()
             << dendl;
    }
    else {
...
static int wait_for_terminate_legacy(int pid, int timeout)
{
    for (int i = 0;; i++) {
        if (kill(pid, 0) == -1) {
            ...
            int r = -errno;
            cerr << "rbd-nbd: kill(" << pid << ", 0) failed: " << cpp_strerror(r) << std::endl;
            return r;
        }
        if (i >= timeout * 2) {
            break;
...
static int wait_for_terminate(int pid, int timeout)
{
    ...
    if (fd == -1) {
        ...
        int r = -errno;
        cerr << "rbd-nbd: pidfd_open(" << pid << ") failed: " << cpp_strerror(r) << std::endl;
        return r;
    }
    ...
    if (r == -1) {
        r = -errno;
        cerr << "rbd-nbd: failed to poll rbd-nbd process: " << cpp_strerror(r) << std::endl;
        goto done;
    }
    else {
        r = 0;
...
static int do_map(int argc, const char* argv[], Config* cfg, bool reconnect)
{
    ...
    if (cfg->exclusive) {
        ...
        if (r < 0) {
            cerr << "rbd-nbd: failed to acquire exclusive lock: " << cpp_strerror(r) << std::endl;
            goto close_fd;
        }
    }
    ...
    if (cfg->snapid != CEPH_NOSNAP) {
        ...
        if (r < 0) {
            cerr << "rbd-nbd: failed to set snap id: " << cpp_strerror(r) << std::endl;
            goto close_fd;
        }
    }
        ...
        if (r < 0) {
            cerr << "rbd-nbd: failed to set snap name: " << cpp_strerror(r) << std::endl;
            goto close_fd;
        }
    }
    ...
    if (!cfg->encryption_formats.empty()) {
        ...
        for (size_t i = 0; i < cfg->encryption_formats.size(); ++i) {
            std::ifstream file(cfg->encryption_passphrase_files[i], std::ios::in | std::ios::binary);
            if (file.fail()) {
                r = -errno;
                std::cerr << "rbd-nbd: unable to open passphrase file '" << cfg->encryption_passphrase_files[i]
            ...
            case RBD_ENCRYPTION_FORMAT_LUKS:
            {
                ...
                encryption_options.specs.push_back({RBD_ENCRYPTION_FORMAT_LUKS, opts, sizeof(*opts)});
                break;
            }
            case RBD_ENCRYPTION_FORMAT_LUKS1:
            {
                auto opts = new librbd::encryption_luks1_format_options_t{.passphrase = std::move(passphrase)};
                encryption_options.specs.push_back({RBD_ENCRYPTION_FORMAT_LUKS1, opts, sizeof(*opts)});
                break;
            }
            case RBD_ENCRYPTION_FORMAT_LUKS2:
            {
                auto opts = new librbd::encryption_luks2_format_options_t{.passphrase = std::move(passphrase)};
                encryption_options.specs.push_back({RBD_ENCRYPTION_FORMAT_LUKS2, opts, sizeof(*opts)});
                break;
            }
            default: ceph_abort();
            }
        ...
        r = image.encryption_load2(encryption_options.specs.data(), encryption_options.specs.size());
        if (r != 0) {
            cerr << "rbd-nbd: failed to load encryption: " << cpp_strerror(r) << std::endl;
            goto close_fd;
        }
    ...
    if (info.size > ULONG_MAX) {
        r = -EFBIG;
        cerr << "rbd-nbd: image is too large (" << byte_u_t(info.size) << ", max is " << byte_u_t(ULONG_MAX) << ")"
             << std::endl;
        goto close_fd;
    }
    ...
    if (!use_netlink) {
        ...
        if (r < 0) goto free_server;
    }

    r = check_device_size(nbd_index, size);
    if (r < 0) goto close_nbd;
...

    {
        ...
        NBDWatchCtx watch_ctx(nbd, nbd_index, use_netlink, io_ctx, image, info.size);
...
static int do_detach(Config* cfg)
{
    ...
    if (r == -1) {
        r = -errno;
        cerr << "rbd-nbd: failed to terminate " << cfg->pid << ": " << cpp_strerror(r) << std::endl;
        return r;
    }
...
static int parse_imgpath(const std::string& imgpath, Config* cfg, std::ostream* err_msg)
{
    std::regex pattern("^(?:([^/]+)/(?:([^/@]+)/)?)?([^@]+)(?:@([^/@]+))?$");
    std::smatch match;
    ...
    if (match[4].matched) cfg->snapname = match[4];
...
static int do_list_mapped_devices(const std::string& format, bool pretty_format)
{
    ...
    NBDListIterator it;
    while (it.get(&cfg)) {
        std::string snap = (cfg.snapid != CEPH_NOSNAP ? "@" + std::to_string(cfg.snapid) : cfg.snapname);
        if (f) {
            ...
            should_print = true;
            tbl << cfg.pid << cfg.poolname << cfg.nsname << cfg.imgname << (snap.empty() ? "-" : snap) << cfg.devpath
                << cfg.cookie << TextTable::endrow;
        }
    }
...
static bool find_mapped_dev_by_spec(Config* cfg, int skip_pid = -1)
{
    ...
    NBDListIterator it;
    while (it.get(&c)) {
        if (c.pid != skip_pid && c.poolname == cfg->poolname && c.nsname == cfg->nsname && c.imgname == cfg->imgname &&
            c.snapname == cfg->snapname && (cfg->devpath.empty() || c.devpath == cfg->devpath) &&
            c.snapid == cfg->snapid) {
            *cfg = c;
            return true;
        }
...
static int parse_args(vector<const char*>& args, std::ostream* err_msg, Config* cfg)
{
    ...
    for (i = args.begin(); i != args.end();) {
        ...
        else if (ceph_argparse_witharg(args, i, &cfg->io_timeout, err, "--io-timeout", (char*)NULL)) {
            if (!err.str().empty()) {
        ...
        else if (ceph_argparse_witharg(args, i, &cfg->quiesce_hook, "--quiesce-hook", (char*)NULL)) {}
        else if (ceph_argparse_flag(args, i, "--read-only", (char*)NULL)) {
            cfg->readonly = true;
        }
        else if (ceph_argparse_witharg(args, i, &cfg->reattach_timeout, err, "--reattach-timeout", (char*)NULL)) {
            if (!err.str().empty()) {
                *err_msg << "rbd-nbd: " << err.str();
                return -EINVAL;
            }
        ...
        else if (ceph_argparse_witharg(args, i, &cfg->io_timeout, err, "--timeout", (char*)NULL)) {
            if (!err.str().empty()) {
                *err_msg << "rbd-nbd: " << err.str();
                return -EINVAL;
            }
        ...
        else if (ceph_argparse_witharg(args, i, &cfg->format, err, "--format", (char*)NULL)) {}
        ...
        else if (ceph_argparse_witharg(args, i, &snapid, err, "--snap-id", (char*)NULL)) {
            if (!err.str().empty()) {
                *err_msg << "rbd-nbd: " << err.str();
                return -EINVAL;
            }
        ...
        else if (ceph_argparse_witharg(args, i, &arg_value, "--encryption-format", (char*)NULL)) {
            if (arg_value == "luks1") {
        ...
        else if (ceph_argparse_witharg(args, i, &arg_value, "--encryption-passphrase-file", (char*)NULL)) {
    ...
    if (cfg->encryption_formats.empty() && !cfg->encryption_passphrase_files.empty()) {
        cfg->encryption_formats.resize(cfg->encryption_passphrase_files.size(), RBD_ENCRYPTION_FORMAT_LUKS);
    }
...
static int rbd_nbd(int argc, const char* argv[])
{
    ...
    case Attach:
        ...
        if (find_mapped_dev_by_spec(&cfg, getpid())) {
            cerr << "rbd-nbd: " << cfg.devpath << " has process " << cfg.pid << " connected" << std::endl;
            return -EBUSY;
        }
        [[fallthrough]];
    case Map:
    ...
    case Detach:
        if (cfg.devpath.empty()) {
            if (!find_mapped_dev_by_spec(&cfg)) {
                cerr << "rbd-nbd: " << cfg.image_spec() << " is not mapped" << std::endl;
                return -ENOENT;
            }
        }
        else if (!find_proc_by_dev(&cfg)) {
            cerr << "rbd-nbd: no process attached to " << cfg.devpath << " found" << std::endl;
            return -ENOENT;
        }
        r = do_detach(&cfg);
        if (r < 0) return -EINVAL;
