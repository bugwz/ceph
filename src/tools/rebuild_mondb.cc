...
static int update_auth(const string& keyring_path, const OSDSuperblock& sb, MonitorDBStore& ms);
...
static void add_auth(KeyServerData::Incremental& auth_inc, MonitorDBStore& ms)
{
...
static int get_auth_inc(const string& keyring_path, const OSDSuperblock& sb, KeyServerData::Incremental* auth_inc)
{
    ...
    if (!keyring.get_auth(auth_inc->name, new_inc)) {
        cerr << "key for " << auth_inc->name << " not found in keyring: " << keyring_path << std::endl;
        return -EINVAL;
    }
...
int update_osdmap(ObjectStore& fs, OSDSuperblock& sb, MonitorDBStore& ms)
{
    ...
    OSDMap osdmap;
    for (auto e = std::max(last_committed + 1, sb.oldest_map); e <= sb.newest_map; e++) {
        bool have_crc = false;
        uint32_t crc = -1;
        uint64_t features = 0;
        // add inc maps
        auto add_inc_result = [&] {
            ...
            if (osdmap.get_epoch() && e > 1) {
                if (osdmap.apply_incremental(inc)) {
                    cerr << "bad fsid: " << osdmap.get_fsid() << " != " << inc.fsid << std::endl;
                    return -EINVAL;
                }
                ...
                if (inc.have_crc) {
                    ...
                    if (osdmap.get_crc() != inc.full_crc) {
                        cerr << "mismatched inc crc: " << osdmap.get_crc() << " != " << inc.full_crc << std::endl;
                        return -EINVAL;
                    }
        ...
        // add full maps
        {
            ...
            if (osdmap.have_crc()) {
                if (have_crc && osdmap.get_crc() != crc) {
                    cerr << "mismatched full/inc crc: " << osdmap.get_crc() << " != " << crc << std::endl;
                    return -EINVAL;
                }
                ...
                if (osdmap.get_crc() != saved_crc) {
                    cerr << "mismatched full crc: " << saved_crc << " != " << osdmap.get_crc() << std::endl;
                    return -EINVAL;
                }
            }
        }
    ...
    cout << std::left << setw(8) << osd_name << ": " << ntrimmed << " osdmaps trimmed, " << nadded << " osdmaps added."
         << std::endl;
    return 0;
}
