...
void usage()
{
    cout << "usage: ceph-authtool keyringfile [OPTIONS]...\n"
         ...
         << "                                e.g: '0644', defaults to '0600'" << std::endl;
    exit(1);
}
...
int main(int argc, const char** argv)
{
    ...
    if (!(gen_key || gen_print_key || !add_key.empty() || list || !caps_fn.empty() || !caps.empty() || print_key ||
          create_keyring || !import_keyring.empty())) {
    ...
    if (!gen_key && add_key.empty() && !caps.empty()) {
        ...
        if (!keyring.get_secret(ename, key)) {
            cerr << "can't find existing key for " << ename << " and neither gen-key nor add-key specified"
                 << std::endl;
            exit(1);
        }
    }
