...
public:
    explicit TraceIter(string fname)
        : fd(-1)
        , idx(-1)
    {
    ...
    void next()
    {
        ...
        if (r < 0) {
            std::cerr << "Got error: " << cpp_strerror(r) << " on read_fd" << std::endl;
            ::close(fd);
            fd = -1;
            return;
        }
...
)
{
    ...
    desc_all.add(*desc);
    if (hidden_desc != NULL) desc_all.add(*hidden_desc);

    try {
        po::command_line_parser parser = po::command_line_parser(cmd_args).options(desc_all);
...
{
    std::cerr << "usage: " << n << " <store-path> <cmd> [args|options]\n"
              << "\n"
              << "Commands:\n"
...
int update_osdmap(MonitorDBStore& store, version_t ver, bool copy, std::shared_ptr<CrushWrapper> crush,
...
int rewrite_transaction(MonitorDBStore& store, int version, const string& crush_file, MonitorDBStore::Transaction* t)
{
    const string prefix("osdmap");
    ...
    if (good_version >= last_committed) {
        std::cout << "good epoch is greater or equal to the last committed one: " << good_version
                  << " >= " << last_committed << std::endl;
        return 0;
    }
 ...
 */
int rewrite_crush(const char* progname, vector<string>& subcmds, MonitorDBStore& store)
{
    po::options_description op_desc("Allowed 'rewrite-crush' options");
    int version = -1;
    string crush_file;
    op_desc.add_options()("help,h", "produce this help message")(
        "crush",
        po::value<string>(&crush_file),
        ("path to the crush map file "
         "(default: will instead extract it from the known-good osdmap)"))(
        "good-epoch",
        po::value<int>(&version),
    ...
    cout << "adding pending commit " << pending_v << " " << bl.length() << " bytes" << std::endl;
...
static int update_auth(MonitorDBStore& st, const string& keyring_path)
{
    ...
    for (const auto& k : keyring.get_keys()) {
        ...
        std::transform(begin(auth_inc.auth.caps), end(auth_inc.auth.caps), inserter(caps, end(caps)), [](auto& cap) {
            string c;
            auto p = cap.second.cbegin();
            decode(c, p);
            return make_pair(cap.first, c);
        });
        cout << "adding auth for '" << auth_inc.name << "': " << auth_inc.auth << " with caps(" << caps << ")"
             << std::endl;
...
static int update_mkfs(MonitorDBStore& st, const string& monmap_path, const vector<string>& mon_ids)
{
    MonMap monmap;
    if (!monmap_path.empty()) {
        ...
        if (r < 0) {
            cerr << "failed to read monmap from " << monmap_path << ": " << cpp_strerror(r) << std::endl;
            return r;
        }
        monmap.decode(bl);
    }
        ...
        if (!mon_ids.empty()) {
            if (mon_ids.size() != monmap.size()) {
                cerr << "Please pass the same number of <mon-ids> to name the hosts "
                     << "listed in 'mon_host'. " << mon_ids.size() << " mon-id(s) specified, "
...
static int update_mgrmap(MonitorDBStore& st)
{
...

    {
        ...
        // mgr expects epoch > 1
        map.epoch++;
        auto initial_modules = get_str_vec(g_ceph_context->_conf.get_val<string>("mgr_initial_modules"));
        copy(begin(initial_modules), end(initial_modules), inserter(map.modules, end(map.modules)));
        bufferlist bl;
...
static int update_paxos(MonitorDBStore& st)
{
    ...
    {
        MonitorDBStore::Transaction t;
        vector<string> prefixes = {"auth", "osdmap", "mgr", "mgr_command_desc"};
...
int rebuild_monstore(const char* progname, vector<string>& subcmds, MonitorDBStore& st)
{
    po::options_description op_desc("Allowed 'rebuild' options");
    string keyring_path;
    string monmap_path;
    vector<string> mon_ids;
    op_desc.add_options()("keyring", po::value<string>(&keyring_path), "path to the client.admin key")(
        "monmap", po::value<string>(&monmap_path), "path to the initial monmap")(
        "mon-ids", po::value<vector<string>>(&mon_ids)->multitoken(), "mon ids, use 'a', 'b', ... if not specified");
    ...
    if (!keyring_path.empty()) update_auth(st, keyring_path);
...
int main(int argc, char** argv)
{
    ...
    positional_desc.add_options()("store-path", po::value<string>(&store_path), "path to monitor's store")(
        "command", po::value<string>(&cmd), "Command")(
        "subcmd", po::value<vector<string>>(&subcmds), "Command arguments/Sub-Commands");
    ...
    try {
        po::parsed_options parsed =
            po::command_line_parser(argc, argv).options(all_desc).positional(positional).allow_unregistered().run();
        ...
        ceph_option_strings = po::collect_unrecognized(parsed.options, po::exclude_positional);
    }
    catch (po::error& e) {
        std::cerr << "error: " << e.what() << std::endl;
        return 1;
    ...
    if (vm.empty() || vm.count("help") || store_path.empty() || cmd.empty() || *cmd.begin() == '-') {
        usage(argv[0], desc);
        return 1;
    }
    ...
    auto cct =
        global_init(NULL, ceph_options, CEPH_ENTITY_TYPE_MON, CODE_ENVIRONMENT_UTILITY, CINIT_FLAG_NO_MON_CONFIG);
    ...
    auto close_store = make_scope_guard([&] { st.close(); });
    ...
    if (cmd == "dump-keys") {
        ...
        op_desc.add_options()("help,h", "produce this help message")(
            "out,o", po::value<string>(&outpath), "output file (default: stdout)")(
        ...
        hidden_op_desc.add_options()("map-type", po::value<string>(&map_type), "map-type");
        po::positional_options_description op_positional;
        op_positional.add("map-type", 1);

        po::variables_map op_vm;
        int r = parse_cmd_args(&op_desc, &hidden_op_desc, &op_positional, subcmds, &op_vm);
        if (r < 0) {
            return -r;
        }
        ...
        if (!outpath.empty()) {
            ...
            if (fd < 0) {
                std::cerr << "error opening output file: " << cpp_strerror(errno) << std::endl;
                return EINVAL;
            }
        }
        ...
        if (!outpath.empty()) {
            std::cout << "wrote " << map_type << " version " << v << " to " << outpath << std::endl;
        }
    }
        ...
        op_desc.add_options()("help,h",
                              "produce this help message")("map-type", po::value<string>(&map_type), "map_type");

        po::positional_options_description op_positional;
        op_positional.add("map-type", 1);

        po::variables_map op_vm;
        int r = parse_cmd_args(&op_desc, NULL, &op_positional, subcmds, &op_vm);
        if (r < 0) {
            return -r;
        }
        ...
        op_desc.add_options()("help,h", "produce this help message")(
            "start,s", po::value<unsigned>(&dstart), "starting version (default: 0)")(
            "end,e", po::value<unsigned>(&dstop), "finish version (default: ~0)");
        ...
        if (dstart > dstop) {
            std::cerr << "error: 'start' version (value: " << dstart << ") "
                      << " is greater than 'end' version (value: " << dstop << ")" << std::endl;
            return EINVAL;
        }
        ...
        hidden_op_desc.add_options()("out,o", po::value<string>(&outpath), "file to write the dump to");
        po::positional_options_description op_positional;
        ...
        if (dstart > dstop) {
            std::cerr << "error: 'start' version (value: " << dstart << ") "
                      << " is greater than 'stop' version (value: " << dstop << ")" << std::endl;
            return EINVAL;
        }
        ...
        op_desc.add_options()("help,h", "produce this help message")(
            "num-replays,n", po::value<unsigned>(&num_replays), "finish version (default: 1)");
        // this is going to be a positional argument; we don't want to show
        // it as an option during --help, but we do want to have it captured
        // when parsing.
        po::options_description hidden_op_desc("Hidden 'replay-trace' options");
        hidden_op_desc.add_options()("in,i", po::value<string>(&inpath), "file to write the dump to");
        po::positional_options_description op_positional;
        ...
        po::variables_map op_vm;
        try {
            po::parsed_options op_parsed =
                po::command_line_parser(subcmds).options(op_desc_all).positional(op_positional).run();
        ...
        op_desc.add_options()("help,h", "produce this help message")(
            "num-keys,k", po::value<unsigned>(&tsize), "keys to write in each transaction (default: 200)")(
            "size,s",
            po::value<unsigned>(&tvalsize),
            "size (in bytes) of the value to write in each key (default: 1024)")(
            "ntrans,n", po::value<unsigned>(&ntrans), "number of transactions to run (default: 100)");

        po::variables_map op_vm;
        try {
            po::parsed_options op_parsed = po::command_line_parser(subcmds).options(op_desc).run();
            po::store(op_parsed, op_vm);
            po::notify(op_vm);
        }
        catch (po::error& e) {
        ...
        do {
            ...
            if (!tx->empty()) out_store.apply_transaction(tx);

            std::cout << "copied " << total_keys << " keys so far (" << stringify(byte_u_t(total_size)) << ")"
                      << std::endl;

        } while (it->valid());
        out_store.close();
        std::cout << "summary: copied " << total_keys << " keys, using " << total_tx << " transactions, totalling "
                  << stringify(byte_u_t(total_size)) << std::endl;
        std::cout << "from '" << store_path << "' to '" << out_path << "'" << std::endl;
    }
