...
{
    std::for_each(v.cbegin(), v.cend(), [&m](const auto& e) { fmt::print(m, "{}\n", e); });
}

template<typename V, typename F> void printseq(const V& v, std::ostream& m, F&& f)
{
    std::for_each(v.cbegin(), v.cend(), [&m, &f](const auto& e) { fmt::print(m, "{}\n", f(e)); });
}

std::int64_t lookup_pool(R::RADOS& r, const std::string& pname, s::yield_context y)
{
    bs::error_code ec;
    auto p = r.lookup_pool(pname, y[ec]);
    if (ec) throw bs::system_error(ec, fmt::format("when looking up '{}'", pname));
    return p;
}


void lspools(R::RADOS& r, const std::vector<std::string>&, s::yield_context y)
{
    const auto l = r.list_pools(y);
    printseq(l, std::cout, [](const auto& p) -> const std::string& { return p.second; });
}
...
void ls(R::RADOS& r, const std::vector<std::string>& p, s::yield_context y)
{
    ...
    bs::error_code ec;
    do {
        std::tie(ls, next) = r.enumerate_objects(pool, next, R::Cursor::end(), 1000, {}, y[ec], R::all_nspaces);
        if (ec) throw bs::system_error(ec, fmt::format("when listing {}", pname));
        printseq(ls, std::cout);
        ls.clear();
    } while (next != R::Cursor::end());
}

void mkpool(R::RADOS& r, const std::vector<std::string>& p, s::yield_context y)
{
    const auto& pname = p[0];
    bs::error_code ec;
    r.create_pool(pname, std::nullopt, y[ec]);
    if (ec) throw bs::system_error(ec, fmt::format("when creating pool '{}'", pname));
}

void rmpool(R::RADOS& r, const std::vector<std::string>& p, s::yield_context y)
{
    const auto& pname = p[0];
    bs::error_code ec;
    r.delete_pool(pname, y[ec]);
    if (ec) throw bs::system_error(ec, fmt::format("when removing pool '{}'", pname));
}

void create(R::RADOS& r, const std::vector<std::string>& p, s::yield_context y)
{
    const auto& pname = p[0];
    ...
    if (ec) throw bs::system_error(ec, fmt::format("when creating object '{}' in pool '{}'", obj, pname));
}
...
void write(R::RADOS& r, const std::vector<std::string>& p, s::yield_context y)
{
    ...
    while (!std::cin.eof()) {
        ...
        if (ec) throw bs::system_error(ec, fmt::format("when writing object '{}' in pool '{}'", obj, pname));
    }
}
...
void read(R::RADOS& r, const std::vector<std::string>& p, s::yield_context y)
{
    ...
    std::uint64_t len;
    {
        ...
        if (ec) throw bs::system_error(ec, fmt::format("when getting length of object '{}' in pool '{}'", obj, pname));
    }
    ...
    while (auto toread = std::max(len - off, io_size)) {
        ...
        r.execute(obj, pool, std::move(op), nullptr, y[ec]);
        if (ec) throw bs::system_error(ec, fmt::format("when reading from object '{}' in pool '{}'", obj, pool));
...
void rm(R::RADOS& r, const std::vector<std::string>& p, s::yield_context y)
{
    ...
    op.remove();
    r.execute(obj, pool, std::move(op), y[ec]);
    if (ec) throw bs::system_error(ec, fmt::format("when removing object '{}' in pool '{}'", obj, pname));
}

static constexpr auto version = std::make_tuple(0ul, 0ul, 1ul);

using cmdfunc = void (*)(R::RADOS& r, const std::vector<std::string>& p, s::yield_context);
...
const std::array commands = {
    ...
    cmdesc{"create"sv, 2, &create, "POOL OBJECT"sv, "exclusively create OBJECT in POOL"sv},
    cmdesc{"write"sv, 2, &write, "POOL OBJECT"sv, "write to OBJECT in POOL from standard input"sv},
    cmdesc{"read"sv, 2, &read, "POOL OBJECT"sv, "read contents of OBJECT in POOL to standard out"sv},
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        desc.add_options()("help", "show help")("version", "show version")(
            ...
            "parameters", po::value<std::vector<std::string>>(&parameters), "parameters to the command");
        ...
        if (vm.count("version")) {
            fmt::print("{}: RADOS command exerciser, {},\n"
        ...
        if (auto ci = std::find_if(
                commands.begin(), commands.end(), [&command](const cmdesc& c) { return c.name == command; });
            ci != commands.end()) {
