...
std::vector<std::string> display_params = {"chunk_count", "data_chunk_count", "coding_chunk_count"};
...
int ec_init(const std::string& profile_str, const std::string& stripe_unit_str, ceph::ErasureCodeInterfaceRef* ec_impl,
            std::unique_ptr<ECUtil::stripe_info_t>* sinfo)
{
    ...
    ceph::ErasureCodePluginRegistry::instance().factory(
        plugin->second, g_conf().get_val<std::string>("erasure_code_dir"), profile, ec_impl, &ss);
    if (!*ec_impl) {
...
int do_test_plugin_exists(const std::vector<const char*>& args)
{
    ...
    int r = instance.load(args[0], g_conf().get_val<std::string>("erasure_code_dir"), &plugin, &ss);
...
int do_validate_profile(const std::vector<const char*>& args)
{
    ...
    if (args.size() > 1) {
        std::set<std::string> valid_params(display_params.begin(), display_params.end());
...
int do_encode(const std::vector<const char*>& args)
{
    ...
    for (auto& [shard, bl] : encoded_data) {
        ...
        if (r < 0) {
            std::cerr << "failed to write " << name << ": " << cpp_strerror(r) << std::endl;
            return 1;
        }
    }
...
int do_decode(const std::vector<const char*>& args)
{
    ...
    if (r < 0) {
        std::cerr << "failed to write " << fname << ": " << cpp_strerror(r) << std::endl;
        return 1;
    }
...
int main(int argc, const char** argv)
{
    ...
    if (args.empty() || args[0] == std::string("-h") || args[0] == std::string("--help")) {
        usage("", std::cout);
        return 0;
    }
