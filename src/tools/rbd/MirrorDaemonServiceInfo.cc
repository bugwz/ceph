...
std::ostream& operator<<(std::ostream& os, MirrorHealth mirror_health)
{
    ...
    case MIRROR_HEALTH_OK: os << "OK"; break;
    case MIRROR_HEALTH_UNKNOWN: os << "UNKNOWN"; break;
    case MIRROR_HEALTH_WARNING: os << "WARNING"; break;
    case MIRROR_HEALTH_ERROR: os << "ERROR"; break;
    }
    return os;
}
...
{
    std::string description = (!client_id.empty() ? client_id : stringify(service_id));
    if (!hostname.empty()) {
...
void MirrorService::dump_image(argument_types::Format::Formatter formatter) const
{
...
const MirrorService* MirrorDaemonServiceInfo::get_by_service_id(const std::string& service_id) const
{
...
const MirrorService* MirrorDaemonServiceInfo::get_by_instance_id(const std::string& instance_id) const
{
...
int MirrorDaemonServiceInfo::get_mirror_service_dump()
{
    ...
    if (r < 0) {
        std::cerr << "rbd: failed to query services: " << cpp_strerror(r) << std::endl;
        return r;
    }
...
int MirrorDaemonServiceInfo::get_mirror_service_status()
{
    ...
    int r = rados.mon_command(cmd, in_bl, &out_bl, nullptr);
    if (r < 0) {
        std::cerr << "rbd: failed to query service status: " << cpp_strerror(r) << std::endl;
        return r;
    }
    json_spirit::mValue json_root;
    if (!json_spirit::read(out_bl.to_str(), json_root)) {
    ...
    try {
        ...
        for (auto& daemon_pair : service.get_obj()) {
            ...
            if (!json_spirit::read(json.get_str(), json_status)) {
                std::cerr << "rbd: invalid service status daemon status JSON received" << std::endl;
                return -EBADMSG;
            }
            ...
            if (!callouts.is_null()) {
                for (auto& callout_pair : callouts.get_obj()) {
                    ...
                    if (mirror_service_health < MIRROR_HEALTH_ERROR && level_str == "error") {
                        mirror_service_health = MIRROR_HEALTH_ERROR;
                    }
                    else if (mirror_service_health < MIRROR_HEALTH_WARNING && level_str == "warning") {
