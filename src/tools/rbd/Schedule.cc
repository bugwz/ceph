...
int parse_schedule_name(const std::string& name, bool allow_images, std::string* pool_name, std::string* namespace_name,
...
void add_level_spec_options(po::options_description* options, bool allow_image)
{
...
int get_level_spec_args(const po::variables_map& vm, std::map<std::string, std::string>* args)
{
    if (vm.count(at::IMAGE_NAME)) {
        ...
        if (!pool_name.empty()) {
            if (vm.count(at::POOL_NAME)) {
                std::cerr << "rbd: pool is specified both via pool and image options" << std::endl;
                return -EINVAL;
            }
        ...
        if (namespace_name.empty()) {
            ...
            (*args)["level_spec"] = pool_name + "/" + namespace_name + "/" + image_name;
        }
        return 0;
    }
...
void add_schedule_options(po::options_description* positional, bool mandatory)
{
    if (mandatory) {
        positional->add_options()("interval", "schedule interval");
    }
    else {
        positional->add_options()("interval", po::value<std::string>()->default_value(""), "schedule interval");
    }
    positional->add_options()("start-time", po::value<std::string>()->default_value(""), "schedule start time");
}

int get_schedule_args(const po::variables_map& vm, bool mandatory, std::map<std::string, std::string>* args)
{
    size_t arg_index = 0;
...
void ScheduleList::dump(ceph::Formatter* f)
{
    ...
    for (auto& [name, s] : schedules) {
        ...
        int r = parse_schedule_name(name, allow_images, &pool_name, &namespace_name, &image_name);
        if (r < 0) {
            continue;
        }
...
std::ostream& operator<<(std::ostream& os, ScheduleList& l)
{
    ...
    for (auto& [name, s] : l.schedules) {
        ...
        int r = parse_schedule_name(name, l.allow_images, &pool_name, &namespace_name, &image_name);
        if (r < 0) {
            continue;
        }
