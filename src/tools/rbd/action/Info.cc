...
static void format_bitmask(Formatter* f, const std::string& name, const std::map<uint64_t, std::string>& mapping,
                           uint64_t bitmask)
{
    int count = 0;
    ...
    for (std::map<uint64_t, std::string>::const_iterator it = mapping.begin(); it != mapping.end(); ++it) {
...
{
    std::map<uint64_t, std::string> mapping = {{RBD_FLAG_OBJECT_MAP_INVALID, "object map invalid"},
...
static int do_show_info(librados::IoCtx& io_ctx, librbd::Image& image, const std::string& snapname, Formatter* f)
{
    ...
    if (f) {
        ...
        std::cout << "rbd image '" << imgname << "':\n"
                  << "\tsize " << byte_u_t(info.size) << " in " << info.num_objs << " objects" << std::endl
                  << "\torder " << info.order << " (" << byte_u_t(info.obj_size) << " objects)" << std::endl
                  << "\tsnapshot_count: " << snaps.size() << std::endl;
        if (!imgid.empty()) {
        ...
        std::cout << "\tblock_name_prefix: " << prefix << std::endl
                  << "\tformat: " << (old_format ? "1" : "2") << std::endl;
    }
    ...
    if (!group_string.empty()) {
        if (f) {
            ...
            std::cout << "\tgroup: " << group_string << std::endl;
        }
    }
    ...
    if (!create_timestamp_str.empty()) {
        if (f) {
            ...
            std::cout << "\tcreate_timestamp: " << create_timestamp_str << std::endl;
        }
    }
    ...
    if (!access_timestamp_str.empty()) {
        if (f) {
            ...
            std::cout << "\taccess_timestamp: " << access_timestamp_str << std::endl;
        }
    }
    ...
    if (!modify_timestamp_str.empty()) {
        if (f) {
            ...
            std::cout << "\tmodify_timestamp: " << modify_timestamp_str << std::endl;
        }
    }
    ...
    if (!snapname.empty()) {
        if (f) {
            ...
            std::cout << "\tprotected: " << (snap_protected ? "True" : "False") << std::endl;
        }
    }
    ...
    if ((image.get_parent(&parent_image_spec, &parent_snap_spec) == 0) && (parent_image_spec.image_name.length() > 0)) {
        if (f) {
            ...
            std::cout << parent_image_spec.image_name << "@" << parent_snap_spec.name;
    ...
    if (features & RBD_FEATURE_STRIPINGV2) {
        if (f) {
            ...
            std::cout << "\tstripe unit: " << byte_u_t(image.get_stripe_unit()) << std::endl
    ...
    if (features & RBD_FEATURE_JOURNALING || mirror_image.state != RBD_MIRROR_IMAGE_DISABLED) {
        if (f) {
            f->open_object_section("mirroring");
            f->dump_string("mode", utils::mirror_image_mode(mirror_mode));
            f->dump_string("state", utils::mirror_image_state(mirror_image.state));
            if (mirror_image.state != RBD_MIRROR_IMAGE_DISABLED) {
            ...
            std::cout << "\tmirroring state: " << utils::mirror_image_state(mirror_image.state) << std::endl;
            if (mirror_image.state != RBD_MIRROR_IMAGE_DISABLED) {
                std::cout << "\tmirroring mode: " << utils::mirror_image_mode(mirror_mode) << std::endl
                          << "\tmirroring global id: " << mirror_image.global_id << std::endl
                          << "\tmirroring primary: " << (mirror_image.primary ? "true" : "false") << std::endl;
            }
        }
    }
...
int execute(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    r = utils::init_and_open_image(
        pool_name, namespace_name, image_name, image_id, snap_name, true, &rados, &io_ctx, &image);
    if (r < 0) {
        return r;
    }
...
Shell::Action action({"info"}, {}, "Show information about image size, striping, etc.", "", &get_arguments, &execute);
