...
{
    options->add_options()(SITE_NAME.c_str(), po::value<std::string>(), "local site name");
}
...
int set_site_name(librados::Rados& rados, const std::string& site_name)
{
    ...
    if (r == -EOPNOTSUPP) {
        ...
        std::cerr << "rbd: failed to set site name" << cpp_strerror(r) << std::endl;
        return r;
    }
...
void validate(boost::any& v, const std::vector<std::string>& values, MirrorPeerDirection* target_type, int permit_tx)
{
...
{
    options->add_options()("direction",
                           po::value<MirrorPeerDirection>(),
...
int validate_mirroring_enabled(librados::IoCtx& io_ctx)
{
    ...
    if (r < 0) {
        std::cerr << "rbd: failed to retrieve mirror mode: " << cpp_strerror(r) << std::endl;
        return r;
    }
...
{
    std::regex pattern("^[A-F0-9]{8}-[A-F0-9]{4}-[A-F0-9]{4}-[A-F0-9]{4}-[A-F0-9]{12}$", std::regex::icase);
    std::smatch match;
...
{
    positional->add_options()("uuid", po::value<std::string>(), "peer uuid");
}

int get_uuid(const po::variables_map& vm, size_t arg_index, std::string* uuid)
{
...
int get_remote_cluster_spec(const po::variables_map& vm, const std::string& spec, std::string* remote_client_name,
                            std::string* remote_cluster, std::map<std::string, std::string>* attributes)
{
                        ...
                        std::map<std::string, std::string>&& attributes)
{
    ...
    if (r == -EPERM) {
        ...
        std::cerr << "rbd: failed to update mirroring peer config: " << cpp_strerror(r) << std::endl;
        return r;
    }
    return 0;
}
                        ...
                        std::map<std::string, std::string>* attributes)
{
    ...
    if (r == -ENOENT) {
        ...
        std::cerr << "rbd: error reading mirroring peer config: " << cpp_strerror(r) << std::endl;
        return r;
    }
...
int update_peer_config_key(librados::IoCtx& io_ctx, const std::string& peer_uuid, const std::string& key,
...
int format_mirror_peers(librados::IoCtx& io_ctx, at::Format::Formatter formatter,
                        const std::vector<librbd::mirror_peer_site_t>& peers, bool config_key)
{
    ...
    for (auto& peer : peers) {
        ...
        case RBD_MIRROR_PEER_DIRECTION_RX: direction = "rx-only"; break;
        case RBD_MIRROR_PEER_DIRECTION_TX: direction = "tx-only"; break;
        case RBD_MIRROR_PEER_DIRECTION_RX_TX: direction = "rx-tx"; break;
        ...
        if (formatter != nullptr) {
            ...
            std::cout << std::endl << "UUID: " << peer.uuid << std::endl << "Name: " << peer.site_name << std::endl;
            if (peer.direction != RBD_MIRROR_PEER_DIRECTION_RX || !peer.mirror_uuid.empty()) {
                std::cout << "Mirror UUID: " << peer.mirror_uuid << std::endl;
            }
            std::cout << "Direction: " << direction << std::endl;
            if (peer.direction != RBD_MIRROR_PEER_DIRECTION_TX || !peer.client_name.empty()) {
...
public:
    void send()
    {
        ...
        auto ctx = new LambdaContext([this](int r) { handle_finalize(r); });
...
protected:
    ImageRequestBase(librados::IoCtx& io_ctx, OrderedThrottle& throttle, const std::string& image_name)
        : m_io_ctx(io_ctx)
        , m_throttle(throttle)
    ...
    virtual bool skip_get_info() const { return false; }
    virtual void get_info(librbd::Image& image, librbd::mirror_image_info_t* info, librbd::RBD::AioCompletion* aio_comp)
    {
        image.aio_mirror_image_get_info(info, sizeof(librbd::mirror_image_info_t), aio_comp);
    }

    virtual bool skip_action(const librbd::mirror_image_info_t& info) const { return false; }
    virtual void execute_action(librbd::Image& image, librbd::RBD::AioCompletion* aio_comp) = 0;
    virtual void handle_execute_action(int r)
    {
        dout(20) << this << " " << __func__ << ": r=" << r << dendl;

        if (r < 0 && r != -ENOENT) {
            std::cerr << "rbd: failed to " << get_action_type() << " image " << m_image_name << ": " << cpp_strerror(r)
                      << std::endl;
            m_ret_val = r;
        }
...
private:
    ...
    void open_image()
    {
        ...
        librbd::RBD rbd;
        auto aio_completion =
            utils::create_aio_completion<ImageRequestBase, &ImageRequestBase::handle_open_image>(this);
        rbd.aio_open(m_io_ctx, m_image, m_image_name.c_str(), nullptr, aio_completion);
    }
    ...
    void handle_open_image(int r)
    {
        ...
        if (r < 0) {
            std::cerr << "rbd: failed to open image " << m_image_name << ": " << cpp_strerror(r) << std::endl;
    ...
    void get_info()
    {
        ...
        auto aio_completion = utils::create_aio_completion<ImageRequestBase, &ImageRequestBase::handle_get_info>(this);
    ...
    void handle_get_info(int r)
    {
        ...
        if (r == -ENOENT) {
            ...
            std::cerr << "rbd: failed to retrieve mirror image info for " << m_image_name << ": " << cpp_strerror(r)
                      << std::endl;
            m_ret_val = r;
            close_image();
            return;
    ...
    void execute_action()
    {
        ...
        auto aio_completion =
            utils::create_aio_completion<ImageRequestBase, &ImageRequestBase::handle_execute_action>(this);
    ...
    void close_image()
    {
        ...
        auto aio_completion =
            utils::create_aio_completion<ImageRequestBase, &ImageRequestBase::handle_close_image>(this);
    ...
    void handle_close_image(int r)
    {
        ...
        if (r < 0) {
            std::cerr << "rbd: failed to close image " << m_image_name << ": " << cpp_strerror(r) << std::endl;
        }
...
{
public:
    PromoteImageRequest(librados::IoCtx& io_ctx, OrderedThrottle& throttle, const std::string& image_name,
                        std::atomic<unsigned>* counter, bool force)
...
protected:
    ...
    void execute_action(librbd::Image& image, librbd::RBD::AioCompletion* aio_comp) override
    {
    ...
    std::string get_action_type() const override { return "promote"; }
...
{
public:
    DemoteImageRequest(librados::IoCtx& io_ctx, OrderedThrottle& throttle, const std::string& image_name,
...
protected:
    ...
    std::string get_action_type() const override { return "demote"; }
...
{
public:
    StatusImageRequest(librados::IoCtx& io_ctx, OrderedThrottle& throttle, const std::string& image_name,
                       ...
                       const MirrorDaemonServiceInfo& daemon_service_info, at::Format::Formatter formatter)
        : ImageRequestBase(io_ctx, throttle, image_name)
...
protected:
    bool skip_get_info() const override { return true; }

    void execute_action(librbd::Image& image, librbd::RBD::AioCompletion* aio_comp) override
    {
        image.get_id(&m_image_id);
        image.aio_mirror_image_get_global_status(
            &m_mirror_image_global_status, sizeof(m_mirror_image_global_status), aio_comp);
    }
    ...
    void finalize_action() override
    {
        ...
        utils::populate_unknown_mirror_image_site_statuses(m_mirror_peers, &m_mirror_image_global_status);

        librbd::mirror_image_site_status_t local_status;
        int local_site_r = utils::get_local_mirror_image_status(m_mirror_image_global_status, &local_status);
        m_mirror_image_global_status.site_statuses.erase(
            std::remove_if(
                m_mirror_image_global_status.site_statuses.begin(),
                m_mirror_image_global_status.site_statuses.end(),
                [](auto& status) { return (status.mirror_uuid == RBD_MIRROR_IMAGE_STATUS_LOCAL_MIRROR_UUID); }),
            m_mirror_image_global_status.site_statuses.end());

        std::string instance_id = (local_site_r >= 0 && local_status.up && m_instance_ids.count(m_image_id))
        ...
        if (m_formatter != nullptr) {
            ...
            m_formatter->dump_string("global_id", m_mirror_image_global_status.info.global_id);
            if (local_site_r >= 0) {
                m_formatter->dump_string("state", utils::mirror_image_site_status_state(local_status));
                ...
                m_formatter->dump_string("last_update", utils::timestr(local_status.last_update));
            }
            if (!m_mirror_image_global_status.site_statuses.empty()) {
                ...
                for (auto& status : m_mirror_image_global_status.site_statuses) {
                    ...
                    m_formatter->dump_string("site_name",
                                             (name_it != m_peer_mirror_uuids_to_name.end() ? name_it->second : ""));
                    m_formatter->dump_string("mirror_uuids", status.mirror_uuid);

                    m_formatter->dump_string("state", utils::mirror_image_site_status_state(status));
                    m_formatter->dump_string("description", status.description);
                    m_formatter->dump_string("last_update", utils::timestr(status.last_update));
            ...
            std::cout << std::endl
                      ...
                      << "  global_id:   " << m_mirror_image_global_status.info.global_id << std::endl;
            if (local_site_r >= 0) {
                std::cout << "  state:       " << utils::mirror_image_site_status_state(local_status) << std::endl
                          << "  description: " << local_status.description << std::endl;
                if (mirror_service != nullptr) {
                    std::cout << "  service:     " << mirror_service->get_image_description() << std::endl;
                }
                std::cout << "  last_update: " << utils::timestr(local_status.last_update) << std::endl;
            }
            if (!m_mirror_image_global_status.site_statuses.empty()) {
                ...
                for (auto& site : m_mirror_image_global_status.site_statuses) {
                    ...
                    std::cout << "    name: "
                              << (name_it != m_peer_mirror_uuids_to_name.end() ? name_it->second : site.mirror_uuid)
                              << std::endl
                              << "    state: " << utils::mirror_image_site_status_state(site) << std::endl
                              << "    description: " << site.description << std::endl
                              << "    last_update: " << utils::timestr(site.last_update) << std::endl;
                }
            }
        }
    }

    std::string get_action_type() const override { return "status"; }
...
template<typename RequestT> class ImageRequestAllocator
{
public:
    template<class... Args>
    RequestT* operator()(librados::IoCtx& io_ctx, OrderedThrottle& throttle, const std::string& image_name,
                         Args&&... args)
    {
        return new RequestT(io_ctx, throttle, image_name, std::forward<Args>(args)...);
    }
};

template<typename RequestT> class ImageRequestGenerator
{
public:
    template<class... Args>
    ImageRequestGenerator(librados::IoCtx& io_ctx, Args&&... args)
        : m_io_ctx(io_ctx)
        , m_factory(std::bind(ImageRequestAllocator<RequestT>(), std::ref(m_io_ctx), std::ref(m_throttle),
                              std::placeholders::_1, std::forward<Args>(args)...))
        , m_throttle(g_conf().get_val<uint64_t>("rbd_concurrent_management_ops"), true)
    {}
...
int get_mirror_image_status(librados::IoCtx& io_ctx, uint32_t* total_images,
                            ...
                            MirrorHealth* mirror_image_health)
{
    ...
    if (r < 0) {
        std::cerr << "rbd: failed to get status summary for mirrored images: " << cpp_strerror(r) << std::endl;
        return r;
    }
    ...
    *mirror_image_health = MIRROR_HEALTH_OK;
    for (auto& it : *mirror_image_states) {
        ...
        if (*mirror_image_health < MIRROR_HEALTH_WARNING &&
            (state != MIRROR_IMAGE_STATUS_STATE_REPLAYING && state != MIRROR_IMAGE_STATUS_STATE_STOPPED)) {
            *mirror_image_health = MIRROR_HEALTH_WARNING;
        }
        if (*mirror_image_health < MIRROR_HEALTH_ERROR && state == MIRROR_IMAGE_STATUS_STATE_ERROR) {
...
void get_peer_bootstrap_create_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, false);
    add_site_name_optional(options);
}

int execute_peer_bootstrap_create(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool_name;
    size_t arg_index = 0;
    int r = utils::get_pool_and_namespace_names(vm, true, &pool_name, nullptr, &arg_index);
    if (r < 0) {
        return r;
    }
    ...
    if (r == -EEXIST) {
        std::cerr << "rbd: mismatch with pre-existing RBD mirroring peer user caps" << std::endl;
    }
    else if (r < 0) {
        std::cerr << "rbd: failed to create mirroring bootstrap token: " << cpp_strerror(r) << std::endl;
        return r;
    }
...
void get_peer_bootstrap_import_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, false);
    add_site_name_optional(options);
    positional->add_options()("token-path", po::value<std::string>(), "bootstrap token file (or '-' for stdin)");
    options->add_options()("token-path", po::value<std::string>(), "bootstrap token file (or '-' for stdin)");
    add_direction_optional(options);
}

int execute_peer_bootstrap_import(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool_name;
    size_t arg_index = 0;
    int r = utils::get_pool_and_namespace_names(vm, true, &pool_name, nullptr, &arg_index);
    ...
    rbd_mirror_peer_direction_t mirror_peer_direction = RBD_MIRROR_PEER_DIRECTION_RX_TX;
    ...
    if (r < 0) {
        std::cerr << "rbd: error reading token file: " << cpp_strerror(r) << std::endl;
        return r;
    }
...
void get_peer_add_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, false);
    positional->add_options()("remote-cluster-spec",
                              "remote cluster spec\n"
                              "(example: [<client name>@]<cluster name>)");
    options->add_options()("remote-client-name", po::value<std::string>(), "remote client name")(
        "remote-cluster", po::value<std::string>(), "remote cluster name")(
        "remote-mon-host", po::value<std::string>(), "remote mon host(s)")(
        "remote-key-file", po::value<std::string>(), "path to file containing remote key");
    add_direction_optional(options);
}

int execute_peer_add(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool_name;
    size_t arg_index = 0;
    int r = utils::get_pool_and_namespace_names(vm, true, &pool_name, nullptr, &arg_index);
    ...
    r = get_remote_cluster_spec(
        vm, utils::get_positional_argument(vm, arg_index), &remote_client_name, &remote_cluster, &attributes);
    if (r < 0) {
        return r;
    }
    ...
    mirror_peers.erase(std::remove_if(mirror_peers.begin(),
                                      mirror_peers.end(),
    ...
    if (!mirror_peers.empty()) {
        std::cerr << "rbd: multiple RX peers are not currently supported" << std::endl;
        return -EINVAL;
    }
    ...
    std::string uuid;
    r = rbd.mirror_peer_site_add(io_ctx, &uuid, mirror_peer_direction, remote_cluster, remote_client_name);
    if (r == -EEXIST) {
...
void get_peer_remove_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, false);
    add_uuid_option(positional);
}

int execute_peer_remove(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool_name;
    size_t arg_index = 0;
    int r = utils::get_pool_and_namespace_names(vm, true, &pool_name, nullptr, &arg_index);
...
void get_peer_set_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, false);
    add_uuid_option(positional);
    positional->add_options()("key",
                              "peer parameter\n"
                              "(direction, site-name, client, mon-host, key-file)")(
        "value",
        "new value for specified key\n"
        "(rx-only, tx-only, or rx-tx for direction)");
}

int execute_peer_set(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool_name;
    size_t arg_index = 0;
    int r = utils::get_pool_and_namespace_names(vm, true, &pool_name, nullptr, &arg_index);
    ...
    std::set<std::string> valid_keys{{"direction", "site-name", "cluster", "client", "mon-host", "key-file"}};
    ...
    librbd::RBD rbd;
    if (key == "client") {
        r = rbd.mirror_peer_site_set_client_name(io_ctx, uuid.c_str(), value.c_str());
    }
        ...
        auto peer_direction = boost::any_cast<rbd_mirror_peer_direction_t>(direction);
        if (peer_direction != RBD_MIRROR_PEER_DIRECTION_TX) {
            ...
            // for rx
            mirror_peers.erase(
                std::remove_if(mirror_peers.begin(),
                               mirror_peers.end(),
                               [uuid](const librbd::mirror_peer_site_t& peer) {
                                   return (peer.uuid == uuid || peer.direction == RBD_MIRROR_PEER_DIRECTION_TX);
                               }),
                mirror_peers.end());
    ...
    if (r == -ENOENT) {
        std::cerr << "rbd: mirror peer " << uuid << " does not exist" << std::endl;
    }
...
void get_disable_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, true);
}

void get_enable_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, true);
    positional->add_options()("mode", "mirror mode [image or pool]");
    add_site_name_optional(options);
}

int execute_enable_disable(librados::IoCtx& io_ctx, rbd_mirror_mode_t next_mirror_mode, const std::string& mode,
    ...
    if (current_mirror_mode == next_mirror_mode) {
        if (!ignore_no_update) {
            if (mode == "disabled") {
                ...
                std::cout << "rbd: mirroring is already configured for " << mode << " mode" << std::endl;
            }
        }
        return 0;
    }
    else if (next_mirror_mode == RBD_MIRROR_MODE_IMAGE && current_mirror_mode == RBD_MIRROR_MODE_POOL) {
        std::cout << "note: changing mirroring mode from pool to image" << std::endl;
    }
    else if (next_mirror_mode == RBD_MIRROR_MODE_POOL && current_mirror_mode == RBD_MIRROR_MODE_IMAGE) {
        std::cout << "note: changing mirroring mode from image to pool" << std::endl;
    }
...
int execute_disable(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool_name;
    std::string namespace_name;
    size_t arg_index = 0;
    int r = utils::get_pool_and_namespace_names(vm, true, &pool_name, &namespace_name, &arg_index);
    if (r < 0) {
        return r;
    }
    ...
    return execute_enable_disable(io_ctx, RBD_MIRROR_MODE_DISABLED, "disabled", false);
}

int execute_enable(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool_name;
    std::string namespace_name;
    size_t arg_index = 0;
...
void get_info_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, true);
    at::add_format_options(options);
    options->add_options()(ALL_NAME.c_str(), po::bool_switch(), "list all attributes");
}

int execute_info(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool_name;
    std::string namespace_name;
    size_t arg_index = 0;
    int r = utils::get_pool_and_namespace_names(vm, false, &pool_name, &namespace_name, &arg_index);
    if (r < 0) {
        return r;
    }
    ...
    switch (mirror_mode) {
    case RBD_MIRROR_MODE_DISABLED: mirror_mode_desc = "disabled"; break;
    case RBD_MIRROR_MODE_IMAGE: mirror_mode_desc = "image"; break;
    case RBD_MIRROR_MODE_POOL: mirror_mode_desc = "pool"; break;
    default: mirror_mode_desc = "unknown"; break;
    }
    ...
    if (mirror_mode != RBD_MIRROR_MODE_DISABLED && namespace_name.empty()) {
        if (formatter != nullptr) {
            ...
            std::cout << "Site Name: " << site_name << std::endl << std::endl;
        }

        r = format_mirror_peers(io_ctx, formatter, mirror_peers, vm[ALL_NAME].as<bool>());
        if (r < 0) {
            return r;
        }
    }
...
void get_status_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, true);
    at::add_format_options(options);
...
int execute_status(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool_name;
    std::string namespace_name;
    size_t arg_index = 0;
    ...
    r = get_mirror_image_status(io_ctx, &total_images, &mirror_image_states, &mirror_image_health);
    if (r < 0) {
        return r;
    }
    ...
    if (formatter != nullptr) {
        ...
        for (auto& it : mirror_image_states) {
            std::cout << "    " << it.second << " " << utils::mirror_image_status_state(it.first) << std::endl;
        }
    }
    ...
    if (verbose) {
        ...
        if (formatter != nullptr) {
            ...
            for (auto& mirror_service : mirror_services) {
                std::cout << "service " << mirror_service.service_id << ":" << std::endl
                          << "  instance_id: " << mirror_service.instance_id << std::endl
                          ...
                          << "  leader: " << (mirror_service.leader ? "true" : "false") << std::endl
        ...
        utils::get_mirror_peer_mirror_uuids_to_names(mirror_peers, &peer_mirror_uuids_to_name);
        ...
        while (true) {
            ...
            if (r < 0) {
                if (r == -EOPNOTSUPP) {
                    ...
                    std::cerr << "rbd: failed to get instance id list: " << cpp_strerror(r) << std::endl;
                }
                // not fatal
                break;
            }
        ...
        ImageRequestGenerator<StatusImageRequest> generator(
            io_ctx, instance_ids, mirror_peers, peer_mirror_uuids_to_name, daemon_service_info, formatter);
...
void get_promote_arguments(po::options_description* positional, po::options_description* options)
{
    options->add_options()("force", po::bool_switch(), "promote even if not cleanly demoted by remote cluster");
    at::add_pool_options(positional, options, true);
}

int execute_promote(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool_name;
    std::string namespace_name;
    size_t arg_index = 0;
    ...
    ImageRequestGenerator<PromoteImageRequest> generator(io_ctx, &counter, vm["force"].as<bool>());
    r = generator.execute();
...
void get_demote_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, true);
}

int execute_demote(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool_name;
    std::string namespace_name;
    size_t arg_index = 0;
...
Shell::Action action_bootstrap_create({"mirror", "pool", "peer", "bootstrap", "create"}, {},
...
Shell::Action action_bootstreap_import({"mirror", "pool", "peer", "bootstrap", "import"}, {},
...
Shell::Action action_add({"mirror", "pool", "peer", "add"}, {}, "Add a mirroring peer to a pool.", "",
                         &get_peer_add_arguments, &execute_peer_add);
Shell::Action action_remove({"mirror", "pool", "peer", "remove"}, {}, "Remove a mirroring peer from a pool.", "",
                            &get_peer_remove_arguments, &execute_peer_remove);
Shell::Action action_set({"mirror", "pool", "peer", "set"}, {}, "Update mirroring peer settings.", "",
                         &get_peer_set_arguments, &execute_peer_set);

Shell::Action action_disable({"mirror", "pool", "disable"}, {}, "Disable RBD mirroring by default within a pool.", "",
                             &get_disable_arguments, &execute_disable);
Shell::Action action_enable({"mirror", "pool", "enable"}, {}, "Enable RBD mirroring by default within a pool.", "",
                            &get_enable_arguments, &execute_enable);
Shell::Action action_info({"mirror", "pool", "info"}, {}, "Show information about the pool mirroring configuration.",
                          {}, &get_info_arguments, &execute_info);
Shell::Action action_status({"mirror", "pool", "status"}, {}, "Show status for all mirrored images in the pool.", {},
                            &get_status_arguments, &execute_status);
Shell::Action action_promote({"mirror", "pool", "promote"}, {}, "Promote all non-primary images in the pool.", {},
                             &get_promote_arguments, &execute_promote);
Shell::Action action_demote({"mirror", "pool", "demote"}, {}, "Demote all primary images in the pool.", {},
