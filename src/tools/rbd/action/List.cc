...
int list_process_image(librados::Rados* rados, WorkerEntry* w, bool lflag, Formatter* f, TextTable& tbl)
{
    ...
    if (f) {
        ...
        if (!lockers.empty()) f->dump_string("lock_type", exclusive ? "exclusive" : "shared");
        f->close_section();
    }
    else {
        tbl << w->name << stringify(byte_u_t(info.size)) << parent << ((old_format) ? '1' : '2')
            << ""   // protect doesn't apply to images
            << lockstr << TextTable::endrow;
    }

    std::vector<librbd::snap_info_t> snaplist;
    if (w->img.snap_list(snaplist) >= 0 && !snaplist.empty()) {
        snaplist.erase(
            remove_if(snaplist.begin(), snaplist.end(), boost::bind(utils::is_not_user_snap_namespace, &w->img, _1)),
            snaplist.end());
        for (std::vector<librbd::snap_info_t>::iterator s = snaplist.begin(); s != snaplist.end(); ++s) {
            bool is_protected;
            bool has_parent = false;
            parent.clear();
            ...
            if (f) {
                ...
                tbl << w->name + "@" + s->name << stringify(byte_u_t(s->size)) << parent << ((old_format) ? '1' : '2')
                    << (is_protected ? "yes" : "") << ""   // locks don't apply to snaps
                    << TextTable::endrow;
            }
        }
    }
...
int do_list(const std::string& pool_name, const std::string& namespace_name, bool lflag, Formatter* f)
{
    ...
    for (size_t left = 0; left < std::min<size_t>(threads, images.size()); left++) {
    ...
    while (true) {
        ...
        for (auto comp : workers) {
            ...
            case STATE_IDLE:
                ...
                r = rbd.aio_open_read_only(ioctx, comp->img, i->name.c_str(), nullptr, comp->completion);
                i++;
            ...
            case STATE_OPENED:
                ...
                comp->completion->release();
                if (r < 0) {
                    std::cerr << "rbd: error opening " << comp->name << ": " << cpp_strerror(r) << std::endl;
                ...
                if (r < 0) {
                    std::cerr << "rbd: error processing image " << comp->name << ": " << cpp_strerror(r) << std::endl;
                }
                comp->completion = new librbd::RBD::AioCompletion(nullptr, nullptr);
...
int execute(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    r = do_list(pool_name, namespace_name, vm["long"].as<bool>(), formatter.get());
    if (r < 0) {
...
Shell::Action action({"list"}, {"ls"}, "List rbd images.", "", &get_arguments, &execute);
