...
void get_arguments(po::options_description* positional, po::options_description* options)
{
    ...
    options->add_options()("allow-shrink", po::bool_switch(), "permit shrinking");
...
int execute(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    if (!encryption_options.specs.empty()) {
        ...
        if (r < 0) {
            std::cerr << "rbd: encryption load failed: " << cpp_strerror(r) << std::endl;
            return r;
        }
    }
...
Shell::Action action({"resize"}, {}, "Resize (expand or shrink) image.", "", &get_arguments, &execute);
