...
static const StatDescriptors STAT_DESCRIPTORS = boost::assign::list_of<StatDescriptors::relation>(
    StatDescriptor::WRITE_OPS, "write_ops")(StatDescriptor::READ_OPS, "read_ops")(
    StatDescriptor::WRITE_BYTES, "write_bytes")(StatDescriptor::READ_BYTES, "read_bytes")(
    StatDescriptor::WRITE_LATENCY, "write_latency")(StatDescriptor::READ_LATENCY, "read_latency");
...
void validate(boost::any& v, const std::vector<std::string>& values, StatDescriptor* target_type, int)
{
...
struct ImageStat
{
    ImageStat(const std::string& pool_name, const std::string& pool_namespace, const std::string& image_name)
        : pool_name(pool_name)
...
std::string format_pool_spec(const std::string& pool, const std::string& pool_namespace)
{
...
int query_iostats(librados::Rados& rados, const std::string& pool_spec, StatDescriptor sort_by, ImageStats* image_stats,
                  std::ostream& err_os)
{
    ...
    if (r == -EOPNOTSUPP) {
        err_os << "rbd: 'rbd_support' mgr module is not enabled." << std::endl
               << std::endl
               << "Use 'ceph mgr module enable rbd_support' to enable." << std::endl;
        return r;
    }
    else if (r < 0) {
    ...
    try {
        ...
        for (size_t idx = 0; idx < json_stat_descriptors.size(); ++idx) {
            auto it = STAT_DESCRIPTORS.right.find(json_stat_descriptors[idx].get_str());
            if (it == STAT_DESCRIPTORS.right.end()) {
                continue;
            }
        ...
        for (auto& stat : stats) {
            ...
            if (!stat_obj.empty()) {
                ...
                image_stats->emplace_back(pool_it->second.first, pool_it->second.second, pair.second);
...
void format_stat(StatDescriptor stat_descriptor, double stat, std::ostream& os)
{
    switch (stat_descriptor) {
    case StatDescriptor::WRITE_OPS:
    case StatDescriptor::READ_OPS: os << si_u_t(stat) << "/s"; break;
    case StatDescriptor::WRITE_BYTES:
    case StatDescriptor::READ_BYTES: os << byte_u_t(stat) << "/s"; break;
...
void validate(boost::any& v, const std::vector<std::string>& values, Iterations* target_type, int)
{
    po::validators::check_first_occurrence(v);
...
void format(const ImageStats& image_stats, Formatter* f, bool global_search)
{
    ...
    if (f) {
        ...
        for (auto& stat : STAT_DESCRIPTORS.left) {
            std::string title;
            switch (stat.first) {
            case StatDescriptor::WRITE_OPS: title = "WR "; break;
            case StatDescriptor::READ_OPS: title = "RD "; break;
            case StatDescriptor::WRITE_BYTES: title = "WR_BYTES "; break;
            case StatDescriptor::READ_BYTES: title = "RD_BYTES "; break;
            case StatDescriptor::WRITE_LATENCY: title = "WR_LAT "; break;
            case StatDescriptor::READ_LATENCY: title = "RD_LAT "; break;
            default: ceph_assert(false); break;
            }
    ...
    for (auto& image_stat : image_stats) {
        if (f) {
            ...
            for (auto& pair : STAT_DESCRIPTORS.left) {
                f->dump_float(pair.second.c_str(), image_stat.stats[static_cast<size_t>(pair.first)]);
            }
            f->close_section();
        }
        else {
            ...
            for (auto& pair : STAT_DESCRIPTORS.left) {
                ...
                format_stat(pair.first, image_stat.stats[static_cast<size_t>(pair.first)], str);
                str << ' ';
                tbl << str.str();
            }
...
public:
    ...
    int run()
    {
        ...
        while (true) {
            r = query_iostats(m_rados, m_pool_spec, m_sort_by, &m_image_stats, err_str);
            if (r < 0) {
                break;
                return r;
            }
            ...
            if (ch == 'q' || ch == 'Q') {
                ...
                auto it = STAT_DESCRIPTORS.left.find(m_sort_by);
                if (it != STAT_DESCRIPTORS.left.end() && ++it != STAT_DESCRIPTORS.left.end()) {
                    m_sort_by = it->first;
                }
            }
        }
...
private:
    ...
    void init_columns()
    {
        ...
        for (auto& pair : STAT_DESCRIPTORS.left) {
            std::string title;
            switch (pair.first) {
            case StatDescriptor::WRITE_OPS: title = "WRITES OPS"; break;
            case StatDescriptor::READ_OPS: title = "READS OPS"; break;
            case StatDescriptor::WRITE_BYTES: title = "WRITE BYTES"; break;
            case StatDescriptor::READ_BYTES: title = "READ BYTES"; break;
            case StatDescriptor::WRITE_LATENCY: title = "WRITE LAT"; break;
            case StatDescriptor::READ_LATENCY: title = "READ LAT"; break;
            default: ceph_assert(false); break;
            }
    ...
    void redraw_main_window()
    {
        ...
        for (auto& pair : m_columns) {
            ...
            str.str("");
            str << std::right << std::setfill(' ') << std::setw(STAT_COLUMN_WIDTH) << title << ' ';
        ...
        for (auto& image_stat : m_image_stats) {
            ...
            for (auto& pair : m_columns) {
                ...
                str.str("");
                str << std::right << std::setfill(' ') << std::setw(STAT_COLUMN_WIDTH) << value << ' ';
                addstr(str.str().c_str());
            }

            std::string image;
            if (m_pool_spec.empty()) {
                image = format_pool_spec(image_stat.pool_name, image_stat.pool_namespace) + "/";
            }
    ...
    void wait_for_key_or_delay()
    {
        ...
        struct timeval tval;
        tval.tv_sec = std::min<uint32_t>(10, g_conf().get_val<int64_t>("mgr_stats_period"));
        tval.tv_usec = 0;
...
void get_arguments_iostat(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, true);
    options->add_options()("iterations", po::value<iostat::Iterations>(), "iterations of metric collection [> 0]")(
        "sort-by",
...
int execute_iostat(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool;
    std::string pool_namespace;
    size_t arg_index = 0;
    int r = utils::get_pool_and_namespace_names(vm, false, &pool, &pool_namespace, &arg_index);
    if (r < 0) {
        return r;
    }
    ...
    if (iterations > 1 && f != nullptr) {
        std::cerr << "rbd: specifing iterations is not valid with formatted output" << std::endl;
        return -EINVAL;
    }
    ...
    while (count++ < iterations || iterations == 0) {
        ...
        if (count == 1 && image_stats.empty()) {
            count = 0;
            if (!printed_notice) {
                std::cerr << "rbd: waiting for initial image stats" << std::endl << std::endl;
                ;
                printed_notice = true;
            }
        }
...
#ifdef HAVE_CURSES
void get_arguments_iotop(po::options_description* positional, po::options_description* options)
{
    at::add_pool_options(positional, options, true);
}

int execute_iotop(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string pool;
    std::string pool_namespace;
    size_t arg_index = 0;
...
Shell::Action top_action({"perf", "image", "iotop"}, {}, "Display a top-like IO monitor.", "", &get_arguments_iotop,
                         &execute_iotop);

#endif   // HAVE_CURSES

Shell::Action stat_action({"perf", "image", "iostat"}, {}, "Display image IO statistics.", "", &get_arguments_iostat,
                          &execute_iostat);
}   // namespace perf
}   // namespace action
}   // namespace rbd
