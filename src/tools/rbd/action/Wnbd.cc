...
#if defined(_WIN32)
static int call_wnbd_cmd(const po::variables_map& vm, const std::vector<std::string>& args,
                         const std::vector<std::string>& ceph_global_init_args)
{
    ...
    if (exe_path_bytes < 0) {
        ...
        if (snprintf(exe_path + exe_path_bytes, sizeof(exe_path) - exe_path_bytes, "-wnbd") < 0) {
            return -EOVERFLOW;
        }
    }
