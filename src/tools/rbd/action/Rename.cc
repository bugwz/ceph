...
static int do_rename(librbd::RBD& rbd, librados::IoCtx& io_ctx, const char* imgname, const char* destname)
{
...
int execute(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    if (pool_name != dst_pool_name) {
        std::cerr << "rbd: mv/rename across pools not supported" << std::endl
                  << "source pool: " << pool_name << " dest pool: " << dst_pool_name << std::endl;
        return -EINVAL;
    }
    else if (namespace_name != dst_namespace_name) {
        std::cerr << "rbd: mv/rename across namespaces not supported" << std::endl
                  << "source namespace: " << namespace_name << " dest namespace: " << dst_namespace_name << std::endl;
        return -EINVAL;
    }
...
Shell::Action action({"rename"}, {"mv"}, "Rename image within pool.", "", &get_arguments, &execute);
