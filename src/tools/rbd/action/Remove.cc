...
bool is_auto_delete_snapshot(librbd::Image* image, const librbd::snap_info_t& snap_info)
{
    ...
    case RBD_SNAP_NAMESPACE_TYPE_TRASH: return true;
...
static int do_delete(librbd::RBD& rbd, librados::IoCtx& io_ctx, const char* imgname, bool no_progress)
{
...
int execute(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    if (r < 0) {
        if (r == -ENOTEMPTY) {
            ...
            if (image_r >= 0) {
                snaps.erase(std::remove_if(snaps.begin(),
                                           snaps.end(),
            ...
            if (!snaps.empty()) {
                ...
                std::cerr << "rbd: image has snapshots with linked clones - these must "
                          << "be deleted or flattened before the image can be removed." << std::endl;
            }
        }
        else if (r == -EBUSY) {
...
Shell::Action action({"remove"}, {"rm"}, "Delete an image.", "", &get_arguments, &execute);
