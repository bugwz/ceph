...
private:
    struct Item
    {
        ...
        Item(const std::string& pool_name, const std::string& namespace_name, const std::string& schedule_time)
            : pool_name(pool_name)
...
std::ostream& operator<<(std::ostream& os, ScheduleStatus& s)
{
    ...
    for (auto& item : s.scheduled) {
        tbl << item.pool_name << item.namespace_name << item.schedule_time << TextTable::endrow;
    }
...
void get_arguments_add(po::options_description* positional, po::options_description* options)
{
...
int execute_add(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    r = utils::mgr_command(rados, "rbd trash purge schedule add", args, &std::cout, &std::cerr);
    if (r < 0) {
        return r;
    }
...
void get_arguments_remove(po::options_description* positional, po::options_description* options)
{
    add_level_spec_options(options, false);
...
int execute_remove(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    normalize_level_spec_args(&args);
    r = utils::mgr_command(rados, "rbd trash purge schedule remove", args, &std::cout, &std::cerr);
    if (r < 0) {
        return r;
    }
...
void get_arguments_list(po::options_description* positional, po::options_description* options)
{
    add_level_spec_options(options, false);
    options->add_options()("recursive,R", po::bool_switch(), "list all schedules");
...
int execute_list(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    r = utils::mgr_command(rados, "rbd trash purge schedule list", args, &out, &std::cerr);
    if (r < 0) {
        return r;
    }
...
void get_arguments_status(po::options_description* positional, po::options_description* options)
{
    add_level_spec_options(options, false);
    at::add_format_options(options);
}
...
int execute_status(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    normalize_level_spec_args(&args);
    std::stringstream out;
    r = utils::mgr_command(rados, "rbd trash purge schedule status", args, &out, &std::cerr);
...
Shell::Action add_action({"trash", "purge", "schedule", "add"}, {}, "Add trash purge schedule.", "", &get_arguments_add,
                         &execute_add);
Shell::Action remove_action({"trash", "purge", "schedule", "remove"}, {"trash", "purge", "schedule", "rm"},
                            "Remove trash purge schedule.", "", &get_arguments_remove, &execute_remove);
Shell::Action list_action({"trash", "purge", "schedule", "list"}, {"trash", "purge", "schedule", "ls"},
                          "List trash purge schedule.", "", &get_arguments_list, &execute_list);
Shell::Action status_action({"trash", "purge", "schedule", "status"}, {}, "Show trash purge schedule status.", "",
