...
struct ImportDiffContext
{
    ...
    ImportDiffContext(librbd::Image* image, int fd, size_t size, bool no_progress)
        ...
        , throttle((fd == STDIN_FILENO) ? 1 : g_conf().get_val<uint64_t>("rbd_concurrent_management_ops"), false)
        , last_offset(0)
    {}
...
{
public:
    C_ImportDiff(ImportDiffContext* idiffctx, bufferlist data, uint64_t offset, uint64_t length, bool write_zeroes)
        : m_idiffctx(idiffctx)
        , m_data(data)
        , m_offset(offset)
        , m_length(length)
    ...
    int send()
    {
        ...
        librbd::RBD::AioCompletion* aio_completion = new librbd::RBD::AioCompletion(ctx, &utils::aio_context_callback);

        int r;
        if (m_write_zeroes) {
            r = m_idiffctx->image->aio_write_zeroes(
                m_offset, m_length, aio_completion, 0U, LIBRADOS_OP_FLAG_FADVISE_NOCACHE);
        }
        else {
            r = m_idiffctx->image->aio_write2(
                m_offset, m_length, m_data, aio_completion, LIBRADOS_OP_FLAG_FADVISE_NOCACHE);
        }
...
static int do_image_snap_from(ImportDiffContext* idiffctx)
{
    ...
    if (!exists) {
        std::cerr << "start snapshot '" << from << "' does not exist in the image, aborting" << std::endl;
        return -EINVAL;
    }
...
static int do_image_io(ImportDiffContext* idiffctx, bool write_zeroes, size_t sparse_size)
{
    ...
    if (!write_zeroes) {
        ...
        while (buffer_offset < buffer_length) {
            ...
            bool zeroed = false;
            utils::calc_sparse_extent(bp, sparse_size, buffer_offset, buffer_length, &write_length, &zeroed);
            ...
            C_ImportDiff* ctx =
                new C_ImportDiff(idiffctx, write_bl, image_offset + buffer_offset, write_length, zeroed);
            r = ctx->send();
            if (r < 0) {
                return r;
            }
        ...
        bufferlist data;
        C_ImportDiff* ctx = new C_ImportDiff(idiffctx, data, image_offset, buffer_length, true);
        return ctx->send();
    }
    return r;
}
...
int do_import_diff_fd(librados::Rados& rados, librbd::Image& image, int fd, bool no_progress, int format,
                      size_t sparse_size)
{
    int r;
    ...
    r = validate_banner(fd, (format == 1 ? utils::RBD_DIFF_BANNER : utils::RBD_DIFF_BANNER_V2));
    if (r < 0) {
        return r;
    }
    ...
    while (r == 0) {
        ...
        if (tag == RBD_DIFF_FROM_SNAP) {
            ...
            std::cerr << "unrecognized tag byte " << (int)tag << " in stream; skipping" << std::endl;
...
int do_import_diff(librados::Rados& rados, librbd::Image& image, const char* path, bool no_progress, size_t sparse_size)
{
    int r;
    int fd;
...
void get_arguments_diff(po::options_description* positional, po::options_description* options)
{
...
int execute_diff(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string path;
    size_t arg_index = 0;
    ...
    r = do_import_diff(rados, image, path.c_str(), vm[at::NO_PROGRESS].as<bool>(), sparse_size);
    if (r == -EDOM) {
        r = -EBADMSG;
    }
    if (r < 0) {
...
Shell::Action action_diff({"import-diff"}, {}, "Import an incremental diff.", "", &get_arguments_diff, &execute_diff);

class C_Import : public Context
{
public:
    C_Import(SimpleThrottle& simple_throttle, librbd::Image& image, bufferlist& bl, uint64_t offset)
        : m_throttle(simple_throttle)
        , m_image(image)
        , m_aio_completion(new librbd::RBD::AioCompletion(this, &utils::aio_context_callback))
        , m_bufferlist(bl)
        , m_offset(offset)
    {}
    ...
    void send()
    {
        ...
        int r = m_image.aio_write2(m_offset, m_bufferlist.length(), m_bufferlist, m_aio_completion, op_flags);
        if (r < 0) {
            std::cerr << "rbd: error requesting write to destination image" << std::endl;
    ...
    {
        if (r < 0) {
            std::cerr << "rbd: error writing to destination image at offset " << m_offset << ": " << cpp_strerror(r)
                      << std::endl;
        }
        m_throttle.end_op(r);
    }
                              ...
                              const std::map<std::string, std::string>& imagemetas)
{
    ...
    for (std::map<std::string, std::string>::const_iterator it = imagemetas.begin(); it != imagemetas.end(); ++it) {
...
static int do_import_v2(librados::Rados& rados, int fd, librbd::Image& image, uint64_t size, size_t imgblklen,
...
static int do_import_v1(int fd, librbd::Image& image, uint64_t size, size_t imgblklen, utils::ProgressContext& pc,
                        size_t sparse_size)
{
    int r = 0;
    ...
    if (from_stdin) {
        ...
        throttle.reset(new SimpleThrottle(g_conf().get_val<uint64_t>("rbd_concurrent_management_ops"), false));
    }
    ...
    while ((readlen = ::read(fd, p + blklen, reqlen)) >= 0) {
        ...
        if (!from_stdin) pc.update_progress(image_pos, size);
        ...
        while (buffer_offset < blklen) {
            size_t write_length = 0;
            bool zeroed = false;
            utils::calc_sparse_extent(blkptr, sparse_size, buffer_offset, blklen, &write_length, &zeroed);
            ...
            if (!zeroed) {
                ...
                C_Import* ctx = new C_Import(*throttle, image, write_bl, image_pos + buffer_offset);
                ctx->send();
            }
        ...
        image_pos += blklen;
        if (!from_stdin && image_pos >= size) break;
...
static int do_import(librados::Rados& rados, librbd::RBD& rbd, librados::IoCtx& io_ctx, const char* imgname,
                     const char* path, librbd::ImageOptions& opts, bool no_progress, int import_format,
                     size_t sparse_size)
{
    int fd, r;
    struct stat stat_buf;
    ...
    if (from_stdin) {
        ...
        if (stat_buf.st_size) size = (uint64_t)stat_buf.st_size;
        ...
        if (!size) {
            ...
            if (r < 0) {
                std::cerr << "rbd: unable to get size of file/block device" << std::endl;
                goto done;
            }
    ...
    r = image.close();
err:
    if (r < 0) rbd.remove(io_ctx, imgname);
done:
    if (r < 0)
        pc.fail();
    else
...
int execute(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::string path;
    size_t arg_index = 0;
    int r = utils::get_path(vm, &arg_index, &path);
    ...
    r = utils::extract_spec(deprecated_image_name,
    ...
    int format = 1;
    if (vm.count("export-format")) format = vm["export-format"].as<uint64_t>();
...
Shell::Action action({"import"}, {}, "Import image from file.", at::get_long_features_help(), &get_arguments, &execute);
