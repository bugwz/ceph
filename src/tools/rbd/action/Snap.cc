...
int do_list_snaps(librbd::Image& image, Formatter* f, bool all_snaps, librados::Rados& rados)
{
    ...
    if (!all_snaps) {
        snaps.erase(remove_if(snaps.begin(), snaps.end(), boost::bind(utils::is_not_user_snap_namespace, &image, _1)),
                    snaps.end());
    }
    else if (!f) {
    ...
    for (std::vector<librbd::snap_info_t>::iterator s = snaps.begin(); s != snaps.end(); ++s) {
        ...
        case RBD_SNAP_NAMESPACE_TYPE_USER: snap_namespace_name = "user"; break;
        case RBD_SNAP_NAMESPACE_TYPE_GROUP: snap_namespace_name = "group"; break;
        case RBD_SNAP_NAMESPACE_TYPE_TRASH: snap_namespace_name = "trash"; break;
        case RBD_SNAP_NAMESPACE_TYPE_MIRROR: snap_namespace_name = "mirror"; break;
        }
        ...
        if (snap_namespace == RBD_SNAP_NAMESPACE_TYPE_GROUP) {
            get_group_res = image.snap_get_group_namespace(s->id, &group_snap, sizeof(group_snap));
        }
        else if (snap_namespace == RBD_SNAP_NAMESPACE_TYPE_TRASH) {
            get_trash_res = image.snap_get_trash_namespace(s->id, &trash_original_name);
        }
        else if (snap_namespace == RBD_SNAP_NAMESPACE_TYPE_MIRROR) {
            get_mirror_res = image.snap_get_mirror_namespace(s->id, &mirror_snap, sizeof(mirror_snap));

            switch (mirror_snap.state) {
            case RBD_SNAP_MIRROR_STATE_PRIMARY: mirror_snap_state = "primary"; break;
            case RBD_SNAP_MIRROR_STATE_NON_PRIMARY: mirror_snap_state = "non-primary"; break;
            case RBD_SNAP_MIRROR_STATE_PRIMARY_DEMOTED:
            case RBD_SNAP_MIRROR_STATE_NON_PRIMARY_DEMOTED: mirror_snap_state = "demoted"; break;
            }
        }
        ...
        if (f) {
            ...
            if (all_snaps) {
                ...
                if (get_group_res == 0) {
                    ...
                    if (mirror_snap.state == RBD_SNAP_MIRROR_STATE_NON_PRIMARY ||
                        ...
                        f->dump_string("primary_mirror_uuid", mirror_snap.primary_mirror_uuid);
                        f->dump_unsigned("primary_snap_id", mirror_snap.primary_snap_id);
                        f->dump_unsigned("last_copied_object_number", mirror_snap.last_copied_object_number);
                    }
                }
                f->close_section();
            }
            ...
            if (all_snaps) {
                ...
                if (get_group_res == 0) {
                    std::string pool_name = pool_map[group_snap.group_pool];
                    oss << " (" << pool_name << "/" << group_snap.group_name << "@" << group_snap.group_snap_name
                        << ")";
                }
                else if (get_trash_res == 0) {
                    ...
                    if (mirror_snap.state == RBD_SNAP_MIRROR_STATE_NON_PRIMARY ||
                        mirror_snap.state == RBD_SNAP_MIRROR_STATE_NON_PRIMARY_DEMOTED) {
                        oss << " " << mirror_snap.primary_mirror_uuid << ":" << mirror_snap.primary_snap_id << " ";
                        if (!mirror_snap.complete) {
                            if (info.num_objs > 0) {
                                auto progress = std::min<uint64_t>(
                                    100, 100 * mirror_snap.last_copied_object_number / info.num_objs);
                                oss << progress << "% ";
                            }
                            else {
                                oss << "not ";
...
int do_add_snap(librbd::Image& image, const char* snapname, uint32_t flags, bool no_progress)
{
...
int do_remove_snap(librbd::Image& image, const char* snapname, bool force, bool no_progress)
{
...
int do_rollback_snap(librbd::Image& image, const char* snapname, bool no_progress)
{
    ...
    if (r < 0) {
        ...
        if (!protect.empty()) {
            std::cout << "rbd: error removing snapshot(s) '" << protect << "', which "
                      << (1 == protect.size() ? "is" : "are") << " protected - these must be unprotected with "
                      << "`rbd snap unprotect`." << std::endl;
        }
...
void get_list_arguments(po::options_description* positional, po::options_description* options)
{
    ...
    options->add_options()(name.c_str(), po::bool_switch(), "list snapshots from all namespaces");
}
...
int execute_list(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    int r = utils::get_pool_image_snapshot_names(vm,
    ...
    librbd::Image image;
    r = utils::init_and_open_image(pool_name, namespace_name, image_name, image_id, "", true, &rados, &io_ctx, &image);
    if (r < 0) {
        return r;
    }
    ...
    if (r < 0) {
        std::cerr << "rbd: failed to list snapshots: " << cpp_strerror(r) << std::endl;
        return r;
    }
    return 0;
}
...
int execute_create(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    librados::IoCtx io_ctx;
    librbd::Image image;
    r = utils::init_and_open_image(pool_name, namespace_name, image_name, "", "", false, &rados, &io_ctx, &image);
    if (r < 0) {
        return r;
    }

    r = do_add_snap(image, snap_name.c_str(), flags, vm[at::NO_PROGRESS].as<bool>());
    if (r < 0) {
        std::cerr << "rbd: failed to create snapshot: " << cpp_strerror(r) << std::endl;
        return r;
    }
    return 0;
}
...
void get_remove_arguments(po::options_description* positional, po::options_description* options)
{
    ...
    options->add_options()("force", po::bool_switch(), "flatten children and unprotect snapshot if needed.");
}
...
int execute_remove(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    int r = utils::get_pool_image_snapshot_names(
        ...
        &snap_name,
        image_id.empty(),
        (snap_id == CEPH_NOSNAP ? utils::SNAPSHOT_PRESENCE_REQUIRED : utils::SNAPSHOT_PRESENCE_PERMITTED),
        utils::SPEC_VALIDATION_NONE);
    if (r < 0) {
        return r;
    }
    ...
    if (!image_id.empty() && !image_name.empty()) {
        ...
        std::cerr << "rbd: trying to access snapshot using both name and id." << std::endl;
        return -EINVAL;
    }
    ...
    if (r < 0) {
        if (r == -EBUSY) {
            ...
            std::cerr << "rbd: failed to remove snapshot: " << cpp_strerror(r) << std::endl;
        }
        return r;
    }
    return 0;
...
void get_purge_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_image_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);
    at::add_image_id_option(options);
    at::add_no_progress_option(options);
...
int execute_purge(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    size_t arg_index = 0;
    std::string pool_name;
    std::string namespace_name;
    ...
    if (r < 0) {
        if (r != -EBUSY) {
            std::cerr << "rbd: removing snaps failed: " << cpp_strerror(r) << std::endl;
        }
        return r;
    }
    return 0;
...
void get_rollback_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_snap_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);
    at::add_no_progress_option(options);
}

int execute_rollback(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    size_t arg_index = 0;
    std::string pool_name;
    std::string namespace_name;
    ...
    r = do_rollback_snap(image, snap_name.c_str(), vm[at::NO_PROGRESS].as<bool>());
    if (r < 0) {
...
void get_protect_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_snap_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);
}

int execute_protect(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    size_t arg_index = 0;
    std::string pool_name;
    std::string namespace_name;
    ...
    if (r < 0) {
        std::cerr << "rbd: protecting snap failed: " << cpp_strerror(r) << std::endl;
        return r;
    }
...
void get_unprotect_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_snap_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);
    at::add_image_id_option(options);
}

int execute_unprotect(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    size_t arg_index = 0;
    std::string pool_name;
    std::string namespace_name;
    ...
    r = image.snap_is_protected(snap_name.c_str(), &is_protected);
    if (r < 0) {
        std::cerr << "rbd: unprotecting snap failed: " << cpp_strerror(r) << std::endl;
        return r;
    }
    else if (!is_protected) {
...
void get_set_limit_arguments(po::options_description* pos, po::options_description* opt)
{
    at::add_image_spec_options(pos, opt, at::ARGUMENT_MODIFIER_NONE);
    at::add_limit_option(opt);
}

int execute_set_limit(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    size_t arg_index = 0;
    std::string pool_name;
    std::string namespace_name;
    ...
    if (r < 0) {
        std::cerr << "rbd: setting snapshot limit failed: " << cpp_strerror(r) << std::endl;
        return r;
    }
    return 0;
}

void get_clear_limit_arguments(po::options_description* pos, po::options_description* opt)
{
    at::add_image_spec_options(pos, opt, at::ARGUMENT_MODIFIER_NONE);
}

int execute_clear_limit(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    size_t arg_index = 0;
    std::string pool_name;
    std::string namespace_name;
    ...
    if (r < 0) {
        std::cerr << "rbd: clearing snapshot limit failed: " << cpp_strerror(r) << std::endl;
        return r;
    }
    return 0;
}
...
int execute_rename(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    if (pool_name != dest_pool_name) {
        std::cerr << "rbd: source and destination pool must be the same" << std::endl;
        return -EINVAL;
    }
    else if (namespace_name != dest_namespace_name) {
        std::cerr << "rbd: source and destination namespace must be the same" << std::endl;
        return -EINVAL;
    }
    else if (image_name != dest_image_name) {
        std::cerr << "rbd: source and destination image name must be the same" << std::endl;
        return -EINVAL;
    }
    ...
    if (r < 0) {
        std::cerr << "rbd: renaming snap failed: " << cpp_strerror(r) << std::endl;
        return r;
    }
    return 0;
}

Shell::Action action_list({"snap", "list"}, {"snap", "ls"}, "Dump list of image snapshots.", "", &get_list_arguments,
                          &execute_list);
Shell::Action action_create({"snap", "create"}, {"snap", "add"}, "Create a snapshot.", "", &get_create_arguments,
                            &execute_create);
Shell::Action action_remove({"snap", "remove"}, {"snap", "rm"}, "Delete a snapshot.", "", &get_remove_arguments,
                            &execute_remove);
Shell::Action action_purge({"snap", "purge"}, {}, "Delete all unprotected snapshots.", "", &get_purge_arguments,
                           &execute_purge);
Shell::Action action_rollback({"snap", "rollback"}, {"snap", "revert"}, "Rollback image to snapshot.", "",
...
Shell::Action action_unprotect({"snap", "unprotect"}, {}, "Allow a snapshot to be deleted.", "",
                               &get_unprotect_arguments, &execute_unprotect);
Shell::Action action_set_limit({"snap", "limit", "set"}, {}, "Limit the number of snapshots.", "",
                               &get_set_limit_arguments, &execute_set_limit);
Shell::Action action_clear_limit({"snap", "limit", "clear"}, {}, "Remove snapshot limit.", "",
                                 &get_clear_limit_arguments, &execute_clear_limit);
Shell::Action action_rename({"snap", "rename"}, {}, "Rename a snapshot.", "", &get_rename_arguments, &execute_rename);
