...
static int call_nbd_cmd(const po::variables_map& vm, const std::vector<std::string>& args,
                        const std::vector<std::string>& ceph_global_init_args)
{
    ...
    ssize_t exe_path_bytes = readlink("/proc/self/exe", exe_path, sizeof(exe_path) - 1);
    if (exe_path_bytes < 0) {
        strcpy(exe_path, "rbd-nbd");
    }
    else {
        if (snprintf(exe_path + exe_path_bytes, sizeof(exe_path) - exe_path_bytes, "-nbd") < 0) {
            return -EOVERFLOW;
        }
    }
...
int execute_attach(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    if (vm.count("options")) {
        utils::append_options_as_args(vm["options"].as<std::vector<std::string>>(), &args);
    }
...
int execute_detach(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    if (device_name.empty() && image_name.empty()) {
        std::cerr << "rbd: detach requires either image name or device path" << std::endl;
        return -EINVAL;
    }
...
void get_list_arguments_deprecated(po::options_description* positional, po::options_description* options)
{
    at::add_format_options(options);
}

int execute_list_deprecated(const po::variables_map& vm, const std::vector<std::string>& ceph_global_args)
{
...
void get_map_arguments_deprecated(po::options_description* positional, po::options_description* options)
{
    at::add_image_or_snap_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);
    options->add_options()("read-only", po::bool_switch(), "map read-only")(
...
int execute_map_deprecated(const po::variables_map& vm_deprecated, const std::vector<std::string>& ceph_global_args)
{
    ...
    options.add_options()(
        "options,o", po::value<std::vector<std::string>>()->default_value(std::vector<std::string>(), ""), "");
...
void get_unmap_arguments_deprecated(po::options_description* positional, po::options_description* options)
{
...
int execute_unmap_deprecated(const po::variables_map& vm, const std::vector<std::string>& ceph_global_args)
{
...
Shell::Action action_show_deprecated({"nbd", "list"}, {"nbd", "ls"}, "List the nbd devices already used.", "",
                                     &get_list_arguments_deprecated, &execute_list_deprecated, false);

Shell::Action action_map_deprecated({"nbd", "map"}, {}, "Map image to a nbd device.", "", &get_map_arguments_deprecated,
                                    &execute_map_deprecated, false);

Shell::Action action_unmap_deprecated({"nbd", "unmap"}, {}, "Unmap a nbd device.", "", &get_unmap_arguments_deprecated,
                                      &execute_unmap_deprecated, false);
