...
{
public:
    RbdWatchCtx(librados::IoCtx& io_ctx, const char* image_name, const std::string& header_oid)
        : m_io_ctx(io_ctx)
    ...
    void handle_notify(uint64_t notify_id, uint64_t cookie, uint64_t notifier_id, bufferlist& bl) override
    {
        ...
        std::cout << m_image_name << " received notification: notify_id=" << notify_id << ", cookie=" << cookie
                  << ", notifier_id=" << notifier_id << ", bl.length=" << bl.length()
                  << ", notify_op=" << notify_message.get_notify_op() << std::endl;
        bufferlist reply;
...
static int do_watch(librados::IoCtx& pp, librbd::Image& image, const char* imgname)
{
    uint8_t old_format;
...
Shell::Action action({"watch"}, {}, "Watch events on image.", "", &get_arguments, &execute);
