...
int do_list_children(librados::IoCtx& io_ctx, librbd::Image& image, bool all_flag, bool descendants_flag, Formatter* f)
{
    ...
    if (f) f->open_array_section("children");
...
void get_arguments(po::options_description* positional, po::options_description* options)
{
    ...
    options->add_options()("all,a", po::bool_switch(), "list all children (include trash)");
    options->add_options()("descendants", po::bool_switch(), "include all descendants");
...
int execute(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    if (r == -ENOENT) {
        ...
        std::cerr << "rbd: error setting snapshot: " << cpp_strerror(r) << std::endl;
        return r;
    }

    r = do_list_children(io_ctx, image, vm["all"].as<bool>(), vm["descendants"].as<bool>(), formatter.get());
    if (r < 0) {
        std::cerr << "rbd: listing children failed: " << cpp_strerror(r) << std::endl;
        return r;
    }
    return 0;
}
...
Shell::Action action({"children"}, {}, "Display children of an image or its snapshot.", "", &get_arguments, &execute);
