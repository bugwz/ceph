...
{
    return trash_info.source != RBD_TRASH_IMAGE_SOURCE_USER && trash_info.source != RBD_TRASH_IMAGE_SOURCE_USER_PARENT;
}

void get_move_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_image_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);
    options->add_options()(EXPIRES_AT.c_str(),
                           po::value<std::string>()->default_value("now"),
                           "set the expiration time of an image so it can be purged when it is stale");
}

int execute_move(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    size_t arg_index = 0;
    ...
    if (vm.find(EXPIRES_AT) != vm.end()) {
        ...
        if (r < 0) {
            std::cerr << "rbd: error calling /bin/date: " << cpp_strerror(r) << std::endl;
            return r;
        }
    }
    ...
    if (dt < 0) {
        std::cerr << "rbd: cannot use a date in the past as an expiration date" << std::endl;
        return -EINVAL;
    }
    ...
    if (r < 0) {
        std::cerr << "rbd: deferred delete error: " << cpp_strerror(r) << std::endl;
    }
...
{
    positional->add_options()(at::IMAGE_ID.c_str(), "image id\n(example: [<pool-name>/[<namespace>/]]<image-id>)");
    at::add_pool_option(options, at::ARGUMENT_MODIFIER_NONE);
    at::add_namespace_option(options, at::ARGUMENT_MODIFIER_NONE);
    at::add_image_id_option(options);

    at::add_no_progress_option(options);
    options->add_options()("force", po::bool_switch(), "force remove of non-expired delayed images");
}
...
int execute_remove(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    std::string namespace_name;
    std::string image_id;
    int r = utils::get_pool_image_id(vm, &arg_index, &pool_name, &namespace_name, &image_id);
    if (r < 0) {
        return r;
    }
    ...
    r = rbd.trash_remove_with_progress(io_ctx, image_id.c_str(), vm["force"].as<bool>(), pc);
    if (r < 0) {
        if (r == -ENOTEMPTY) {
            std::cerr << "rbd: image has snapshots - these must be deleted"
                      << " with 'rbd snap purge' before the image can be removed." << std::endl;
        }
        else if (r == -EUCLEAN) {
            ...
            std::cerr << "rbd: error: image still has watchers" << std::endl
                      ...
                      << "waiting 30s for the crashed client to timeout." << std::endl;
        }
        else if (r == -EMLINK) {
            std::cerr << std::endl << "Remove the image from the group and try again." << std::endl;
        }
        else if (r == -EPERM) {
            std::cerr << std::endl
...
int do_list(librbd::RBD& rbd, librados::IoCtx& io_ctx, bool long_flag, bool all_flag, Formatter* f)
{
    ...
    if (!all_flag) {
        trash_entries.erase(remove_if(trash_entries.begin(), trash_entries.end(), boost::bind(is_not_trash_user, _1)),
                            trash_entries.end());
    }
    ...
    for (const auto& entry : trash_entries) {
        ...
        if (r < 0) {
            if (r != -ENOENT) {
                std::cerr << "rbd: error opening " << entry.id << ": " << cpp_strerror(r) << std::endl;
            }
        ...
        switch (entry.source) {
        case RBD_TRASH_IMAGE_SOURCE_USER: del_source = "USER"; break;
        case RBD_TRASH_IMAGE_SOURCE_MIRRORING: del_source = "MIRRORING"; break;
        case RBD_TRASH_IMAGE_SOURCE_MIGRATION: del_source = "MIGRATION"; break;
        case RBD_TRASH_IMAGE_SOURCE_REMOVING: del_source = "REMOVING"; break;
        case RBD_TRASH_IMAGE_SOURCE_USER_PARENT: del_source = "USER_PARENT"; break;
        }
        ...
        if (f) {
            ...
            tbl << entry.id << entry.name << del_source << time_str << delete_status(entry.deferment_end_time);
            if (has_parent) tbl << parent;
...
void get_list_arguments(po::options_description* positional, po::options_description* options)
{
    ...
    options->add_options()("all,a", po::bool_switch(), "list images from all sources");
...
int execute_list(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    librbd::RBD rbd;
    r = do_list(rbd, io_ctx, vm["long"].as<bool>(), vm["all"].as<bool>(), formatter.get());
    if (r < 0) {
...
void get_purge_arguments(po::options_description* positional, po::options_description* options)
{
    ...
    options->add_options()(EXPIRED_BEFORE.c_str(),
                           po::value<std::string>()->value_name("date"),
                           "purges images that expired before the given date");
    options->add_options()(THRESHOLD.c_str(),
                           po::value<float>(),
...
void get_restore_arguments(po::options_description* positional, po::options_description* options)
{
...
int execute_restore(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    size_t arg_index = 0;
    std::string pool_name;
    std::string namespace_name;
...
Shell::Action action_move({"trash", "move"}, {"trash", "mv"}, "Move an image to the trash.", "", &get_move_arguments,
                          &execute_move);

Shell::Action action_remove({"trash", "remove"}, {"trash", "rm"}, "Remove an image from trash.", "",
                            &get_remove_arguments, &execute_remove);

Shell::Action action_purge({"trash", "purge"}, {}, "Remove all expired images from trash.", "", &get_purge_arguments,
                           &execute_purge);

Shell::Action action_list({"trash", "list"}, {"trash", "ls"}, "List trash images.", "", &get_list_arguments,
                          &execute_list);

Shell::Action action_restore({"trash", "restore"}, {}, "Restore an image from trash.", "", &get_restore_arguments,
                             &execute_restore);
