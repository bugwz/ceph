...
static std::string map_option_ip_cb(const char* value_char)
{
    entity_addr_t a;
    const char* endptr;
    if (!a.parse(value_char, &endptr) || endptr != value_char + strlen(value_char)) {
        return "";
    }
...
{
    if (!strcmp(value_char, "no") || !strcmp(value_char, "balance") || !strcmp(value_char, "localize")) {
        return value_char;
    }
    return "";
}
...
{
    if (!strcmp(value_char, "none") || !strcmp(value_char, "compressible") || !strcmp(value_char, "incompressible")) {
        return value_char;
    }
    return "";
}
...
{
    if (!strcmp(value_char, "legacy") || !strcmp(value_char, "crc") || !strcmp(value_char, "secure") ||
        !strcmp(value_char, "prefer-crc") || !strcmp(value_char, "prefer-secure")) {
        return value_char;
    }
    return "";
}

static void put_map_option(const std::string& key, const std::string& val, MapOptions* map_options)
{
    (*map_options)[key] = val;
}

static int put_map_option_value(const std::string& opt, const char* value_char, std::string (*parse_cb)(const char*),
                                MapOptions* map_options)
{
    ...
    if (value.empty()) {
        std::cerr << "rbd: invalid " << opt << " value '" << value_char << "'" << std::endl;
        return -EINVAL;
    }
...
static int parse_map_options(const std::string& options_string, MapOptions* map_options)
{
    ...
    for (char* this_char = strtok(options, ", "); this_char != NULL; this_char = strtok(NULL, ",")) {
        char* value_char;

        if ((value_char = strchr(this_char, '=')) != NULL) *value_char++ = '\0';

        if (!strcmp(this_char, "fsid")) {
            if (put_map_option_value("fsid", value_char, map_option_uuid_cb, map_options)) return -EINVAL;
        }
        else if (!strcmp(this_char, "ip")) {
            if (put_map_option_value("ip", value_char, map_option_ip_cb, map_options)) return -EINVAL;
        }
        ...
        else if (!strcmp(this_char, "cephx_require_signatures") || !strcmp(this_char, "nocephx_require_signatures")) {
            put_map_option("cephx_require_signatures", this_char, map_options);
        }
        else if (!strcmp(this_char, "tcp_nodelay") || !strcmp(this_char, "notcp_nodelay")) {
            put_map_option("tcp_nodelay", this_char, map_options);
        }
        else if (!strcmp(this_char, "cephx_sign_messages") || !strcmp(this_char, "nocephx_sign_messages")) {
            put_map_option("cephx_sign_messages", this_char, map_options);
        }
        else if (!strcmp(this_char, "mount_timeout")) {
            if (put_map_option_value("mount_timeout", value_char, map_option_int_cb, map_options)) return -EINVAL;
        }
        else if (!strcmp(this_char, "osd_request_timeout")) {
            if (put_map_option_value("osd_request_timeout", value_char, map_option_int_cb, map_options)) return -EINVAL;
        }
        else if (!strcmp(this_char, "lock_timeout")) {
            if (put_map_option_value("lock_timeout", value_char, map_option_int_cb, map_options)) return -EINVAL;
        }
        else if (!strcmp(this_char, "osdkeepalive")) {
            if (put_map_option_value("osdkeepalive", value_char, map_option_int_cb, map_options)) return -EINVAL;
        }
        else if (!strcmp(this_char, "osd_idle_ttl")) {
            if (put_map_option_value("osd_idle_ttl", value_char, map_option_int_cb, map_options)) return -EINVAL;
        }
            ...
            if (put_map_option_value("queue_depth", value_char, map_option_int_cb, map_options)) return -EINVAL;
        }
            ...
            if (put_map_option_value("alloc_size", value_char, map_option_int_cb, map_options)) return -EINVAL;
        }
        else if (!strcmp(this_char, "crush_location")) {
            if (put_map_option_value("crush_location", value_char, map_option_string_cb, map_options)) return -EINVAL;
        }
        else if (!strcmp(this_char, "read_from_replica")) {
            if (put_map_option_value("read_from_replica", value_char, map_option_read_from_replica_cb, map_options))
                return -EINVAL;
        }
        else if (!strcmp(this_char, "compression_hint")) {
            if (put_map_option_value("compression_hint", value_char, map_option_compression_hint_cb, map_options))
                return -EINVAL;
        }
        else if (!strcmp(this_char, "ms_mode")) {
            if (put_map_option_value("ms_mode", value_char, map_option_ms_mode_cb, map_options)) return -EINVAL;
        }
...
static int parse_unmap_options(const std::string& options_string, MapOptions* unmap_options)
{
    char* options = strdup(options_string.c_str());
    BOOST_SCOPE_EXIT(options)
    {
...
static int get_unsupported_features(librbd::Image& image, uint64_t* unsupported_features)
{
    char buf[20];
    uint64_t features, supported_features;
    int r;

    r = safe_read_file("/sys/bus/rbd/", "supported_features", buf, sizeof(buf) - 1);
    if (r < 0) return r;
 ...
 */
static void print_error_description(const char* poolname, const char* nspace_name, const char* imgname,
                                    const char* snapname, int maperrno)
{
    ...
    if (maperrno == -ENOENT) goto done;

    r = utils::init_and_open_image(poolname, nspace_name, imgname, "", snapname, true, &rados, &ioctx, &image);
    if (r < 0) goto done;
    ...
    if (!oldformat && (maperrno == -ENXIO)) {
        ...
        if (r == 0 && (unsupported_features & ~RBD_FEATURES_ALL) == 0) {
            uint64_t immutable = RBD_FEATURES_ALL & ~(RBD_FEATURES_MUTABLE | RBD_FEATURES_DISABLE_ONLY);
...
static int do_kernel_map(const char* poolname, const char* nspace_name, const char* imgname, const char* snapname,
                         MapOptions&& map_options)
{
#if defined(WITH_KRBD)
    struct krbd_ctx* krbd;
    ...
    if (r < 0) {
        std::cerr << "rbd: warning: can't get image map information: " << cpp_strerror(r) << std::endl;
    }
    else if (r > 0) {
        std::cerr << "rbd: warning: image already mapped as " << devnode << std::endl;
        free(devnode);
    }

    r = krbd_map(krbd, poolname, nspace_name, imgname, snapname, oss.str().c_str(), &devnode);
    if (r < 0) {
...
static int do_kernel_unmap(const char* dev, const char* poolname, const char* nspace_name, const char* imgname,
                           const char* snapname, MapOptions&& unmap_options)
{
    ...
    if (dev)
        ...
        r = krbd_unmap_by_spec(krbd, poolname, nspace_name, imgname, snapname, oss.str().c_str());
...
int execute_map(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    size_t arg_index = 0;
    ...
    r = do_kernel_map(
        pool_name.c_str(), nspace_name.c_str(), image_name.c_str(), snap_name.c_str(), std::move(map_options));
    if (r < 0) {
...
int execute_unmap(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    if (device_name.empty() && image_name.empty()) {
        std::cerr << "rbd: unmap requires either image name or device path" << std::endl;
        return -EINVAL;
    }
...
int execute_attach(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
#if defined(WITH_KRBD)
...
int execute_detach(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
#if defined(WITH_KRBD)
