...
static int do_diff(librbd::Image& image, const char* fromsnapname, bool whole_object, Formatter* f)
{
    int r;
    ...
    r = image.diff_iterate2(fromsnapname, 0, info.size, true, whole_object, diff_cb, &om);
    if (f) {
        f->close_section();
        f->flush(std::cout);
    }
    else {
        if (!om.empty) std::cout << *om.t;
        delete om.t;
    }
    return r;
}
...
int execute(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    r = do_diff(image, from_snap_name.empty() ? nullptr : from_snap_name.c_str(), diff_whole_object, formatter.get());
    if (r < 0) {
...
Shell::Action action({"diff"}, {}, "Print extents that differ since a previous snap, or image creation.", "",
