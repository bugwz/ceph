...
static int parse_diff_header(int fd, __u8* tag, string* from, string* to, uint64_t* size)
{
    ...
    {   // header
        ...
        if (strcmp(buf, utils::RBD_DIFF_BANNER.c_str())) {
            std::cerr << "invalid banner '" << buf << "', expected '" << utils::RBD_DIFF_BANNER << "'" << std::endl;
            return -EINVAL;
        }
    }
...
static int parse_diff_body(int fd, __u8* tag, uint64_t* offset, uint64_t* length)
{
    ...
    if (*tag != RBD_DIFF_WRITE && *tag != RBD_DIFF_ZERO) return -ENOTSUP;
 ...
 * rarely used
 */
static int do_merge_diff(const char* first, const char* second, const char* path, bool no_progress)
{
    ...
    while (!f_end || !s_end) {
        ...
        if (!f_end && !f_len) {
            ...
            dout(2) << "first diff data chunk: tag=" << f_tag << ", "
                    ...
                    << "len=" << f_len << dendl;
            if (r < 0) {
                std::cerr << "rbd: failed to read first diff data chunk header" << std::endl;
                goto done;
            }
            ...
            if (last_off > f_off) {
                r = -ENOTSUP;
                std::cerr << "rbd: out-of-order offset from first diff (" << last_off << " > " << f_off << ")"
                          << std::endl;
                goto done;
            }
        }
        ...
        if (!s_end && !s_len) {
            ...
            dout(2) << "second diff data chunk: tag=" << s_tag << ", "
                    ...
                    << "len=" << s_len << dendl;
            if (r < 0) {
                std::cerr << "rbd: failed to read second diff data chunk header" << std::endl;
                goto done;
            }
            ...
            if (last_off > s_off) {
                r = -ENOTSUP;
                std::cerr << "rbd: out-of-order offset from second diff (" << last_off << " > " << s_off << ")"
                          << std::endl;
                goto done;
            }
        }
...
{
    positional->add_options()("diff1-path", "path to first diff (or '-' for stdin)")("diff2-path",
                                                                                     "path to second diff");
    at::add_path_options(positional, options, "path to merged diff (or '-' for stdout)");
...
int execute(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    r = do_merge_diff(first_diff.c_str(), second_diff.c_str(), path.c_str(), vm[at::NO_PROGRESS].as<bool>());
    if (r < 0) {
...
Shell::Action action({"merge-diff"}, {}, "Merge two diff exports together.", "", &get_arguments, &execute);
