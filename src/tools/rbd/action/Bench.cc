...
void validate(boost::any& v, const std::vector<std::string>& values, Size* target_type, int)
{
...
void validate(boost::any& v, const std::vector<std::string>& values, IOPattern* target_type, int)
{
    po::validators::check_first_occurrence(v);
    const std::string& s = po::validators::get_single_string(values);
    if (s == "rand") {
...
struct rbd_bencher
{
    ...
    void start_io(int max, uint64_t off, uint64_t len, int op_flags, bool read_flag)
    {
        ...
        if (read_flag) {
            ...
            c = new librbd::RBD::AioCompletion((void*)(new bencher_completer(this, NULL)), rbd_bencher_completion);
...
int do_bench(librbd::Image& image, io_type_t io_type, uint64_t io_size, uint64_t io_threads, uint64_t io_bytes,
             io_pattern_t io_pattern, uint64_t read_proportion)
{
    ...
    std::cout << "bench "
              ...
              << (io_type == IO_TYPE_READ    ? "read"
                  : io_type == IO_TYPE_WRITE ? "write"
                                             : "readwrite")
              << (io_type == IO_TYPE_RW
                      ? " read:write=" + std::to_string(read_proportion) + ":" + std::to_string(100 - read_proportion)
                      : "")
              << " io_size " << io_size << " io_threads " << io_threads << " bytes " << io_bytes << " pattern ";
    switch (io_pattern) {
    case IO_PATTERN_RAND: std::cout << "random"; break;
    case IO_PATTERN_SEQ: std::cout << "sequential"; break;
    case IO_PATTERN_FULL_SEQ: std::cout << "full sequential"; break;
    ...
    for (i = 0; i < io_threads; i++) {
        ...
        switch (io_pattern) {
        case IO_PATTERN_RAND: start_pos = (rand() % (size / io_size)) * io_size; break;
        case IO_PATTERN_SEQ: start_pos = seq_chunk_length * i; break;
        case IO_PATTERN_FULL_SEQ: start_pos = i * io_size; break;
        default: break;
        }
    ...
    RollingSum time_acc(boost::accumulators::tag::rolling_window::window_size = WINDOW_SIZE);
    RollingSum ios_acc(boost::accumulators::tag::rolling_window::window_size = WINDOW_SIZE);
    RollingSum off_acc(boost::accumulators::tag::rolling_window::window_size = WINDOW_SIZE);
    uint64_t cur_ios = 0;
    uint64_t cur_off = 0;
    ...
    for (off = 0; off < io_bytes;) {
        ...
        for (i = 0; i < io_threads; ++i) {
            switch (io_pattern) {
            case IO_PATTERN_RAND: thread_offset[i] = (rand() % (size / io_size)) * io_size; continue;
            case IO_PATTERN_SEQ:
        ...
        if (last == std::chrono::duration<double>::zero()) {
            ...
            std::cout.width(10);
            std::cout << byte_u_t(boost::accumulators::rolling_sum(off_acc) / time_sum) << "/s" << std::endl;
            last = elapsed;
        }
    }
    b.wait_for(0, false);
    ...
    if (io_type != IO_TYPE_READ) {
        r = image.flush();
        if (r < 0) {
            std::cerr << "rbd: failed to flush at the end: " << cpp_strerror(r) << std::endl;
        }
    }
    ...
    std::cout << "elapsed: " << (int)elapsed.count() << "   "
              ...
              << "bytes/sec: " << byte_u_t((double)off / elapsed.count()) << "/s" << std::endl;

    if (io_type == IO_TYPE_RW) {
        std::cout << "read_ops: " << read_ops << "   "
                  << "read_ops/sec: " << (double)read_ops / elapsed.count() << "   "
                  << "read_bytes/sec: " << byte_u_t((double)read_ops * io_size / elapsed.count()) << "/s" << std::endl;
...
void add_bench_common_options(po::options_description* positional, po::options_description* options)
{
    at::add_image_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);

    options->add_options()("io-size", po::value<Size>(), "IO size (in B/K/M/G) (< 4G) [default: 4K]")(
...
void get_arguments_for_write(po::options_description* positional, po::options_description* options)
{
    add_bench_common_options(positional, options);
}

void get_arguments_for_bench(po::options_description* positional, po::options_description* options)
{
    add_bench_common_options(positional, options);

    options->add_options()("io-type", po::value<IOType>()->required(), "IO type (read, write, or readwrite(rw))");
}
...
int bench_execute(const po::variables_map& vm, io_type_t bench_io_type)
{
    ...
    if (bench_io_type == IO_TYPE_READ) snap_presence = utils::SNAPSHOT_PRESENCE_PERMITTED;
    ...
    r = utils::init_and_open_image(
        pool_name, namespace_name, image_name, "", snap_name, false, &rados, &io_ctx, &image);
    if (r < 0) {
        return r;
    }
    ...
    r = do_bench(
        image, bench_io_type, bench_io_size, bench_io_threads, bench_bytes, bench_pattern, bench_read_proportion);
...
int execute_for_write(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    std::cerr << "rbd: bench-write is deprecated, use rbd bench --io-type write ..." << std::endl;
    return bench_execute(vm, IO_TYPE_WRITE);
}

int execute_for_bench(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
...
Shell::Action action_write({"bench-write"}, {},
                           "Simple write benchmark. (Deprecated, please use `rbd bench --io-type write` instead.)", "",
                           &get_arguments_for_write, &execute_for_write, false);

Shell::Action action_bench({"bench"}, {}, "Simple benchmark.", "", &get_arguments_for_bench, &execute_for_bench);
