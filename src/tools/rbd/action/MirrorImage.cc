...
int validate_mirroring_enabled(librbd::Image& image, bool snapshot = false)
{
    ...
    if (r < 0) {
        std::cerr << "rbd: failed to retrieve mirror info: " << cpp_strerror(r) << std::endl;
        return r;
    }
    ...
    if (snapshot) {
        ...
        if (mode != RBD_MIRROR_IMAGE_MODE_SNAPSHOT) {
            std::cerr << "rbd: snapshot based mirroring not enabled on the image" << std::endl;
            return -EINVAL;
        }
    }
...
void get_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_image_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);
}

void get_arguments_enable(po::options_description* positional, po::options_description* options)
{
    at::add_image_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);
    positional->add_options()("mode",
                              po::value<std::string>()->default_value(""),
                              "mirror image mode (journal or snapshot) [default: journal]");
}

void get_arguments_disable(po::options_description* positional, po::options_description* options)
{
    options->add_options()("force", po::bool_switch(), "disable even if not primary");
    at::add_image_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);
}

int execute_enable_disable(const po::variables_map& vm, bool enable, bool force)
{
    size_t arg_index = 0;
    ...
    std::cout << (enable ? "Mirroring enabled" : "Mirroring disabled") << std::endl;
...
void get_arguments_promote(po::options_description* positional, po::options_description* options)
{
...
int execute_resync(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    size_t arg_index = 0;
    std::string pool_name;
    std::string namespace_name;
...
int execute_status(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    if (r < 0) {
        std::cerr << "rbd: failed to get status for image " << image_name << ": " << cpp_strerror(r) << std::endl;
        return r;
    }
    ...
    int local_site_r = utils::get_local_mirror_image_status(status, &local_status);
    ...
    if (local_site_r >= 0 && local_status.up) {
        ...
        if (r == -EOPNOTSUPP) {
            ...
            std::cerr << "rbd: failed to get service id for image " << image_name << ": " << cpp_strerror(r)
                      << std::endl;
            // not fatal
        }
    ...
    if (formatter != nullptr) {
        ...
        if (local_site_r >= 0) {
            formatter->dump_string("state", utils::mirror_image_site_status_state(local_status));
            formatter->dump_string("description", local_status.description);
            if (mirror_service != nullptr) {
                mirror_service->dump_image(formatter);
            }
            formatter->dump_string("last_update", utils::timestr(local_status.last_update));
        }
        if (!status.site_statuses.empty()) {
            ...
            for (auto& status : status.site_statuses) {
                ...
                formatter->dump_string("state", utils::mirror_image_site_status_state(status));
                formatter->dump_string("description", status.description);
                formatter->dump_string("last_update", utils::timestr(status.last_update));
        ...
        if (!snaps.empty()) {
            ...
            for (auto& snap : snaps) {
                ...
                if (r < 0 || (info.state != RBD_SNAP_MIRROR_STATE_PRIMARY &&
                ...
                formatter->dump_bool("demoted", info.state == RBD_SNAP_MIRROR_STATE_PRIMARY_DEMOTED);
        ...
        if (local_site_r >= 0) {
            std::cout << "  state:       " << utils::mirror_image_site_status_state(local_status) << "\n"
                      << "  description: " << local_status.description << "\n";
            if (mirror_service != nullptr) {
                std::cout << "  service:     " << mirror_service->get_image_description() << "\n";
            }
        ...
        if (!status.site_statuses.empty()) {
            ...
            for (auto& site : status.site_statuses) {
                ...
                std::cout << "    name: "
                          << (name_it != peer_mirror_uuids_to_name.end() ? name_it->second : site.mirror_uuid)
                          << std::endl
        ...
        if (!snaps.empty()) {
            ...
            for (auto& snap : snaps) {
                ...
                std::cout << "    " << snap.id << " " << snap.name << " ("
                          << (info.state == RBD_SNAP_MIRROR_STATE_PRIMARY_DEMOTED ? "demoted " : "") << "peer_uuids:["
...
void get_snapshot_arguments(po::options_description* positional, po::options_description* options)
{
    at::add_image_spec_options(positional, options, at::ARGUMENT_MODIFIER_NONE);
    at::add_snap_create_options(options);
}

int execute_snapshot(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    size_t arg_index = 0;
    std::string pool_name;
    std::string namespace_name;
    ...
    if (r < 0) {
        std::cerr << "rbd: error creating snapshot: " << cpp_strerror(r) << std::endl;
        return r;
    }
...
Shell::Action action_enable({"mirror", "image", "enable"}, {}, "Enable RBD mirroring for an image.", "",
                            &get_arguments_enable, &execute_enable);
Shell::Action action_disable({"mirror", "image", "disable"}, {}, "Disable RBD mirroring for an image.", "",
                             &get_arguments_disable, &execute_disable);
Shell::Action action_promote({"mirror", "image", "promote"}, {}, "Promote an image to primary for RBD mirroring.", "",
...
Shell::Action action_resync({"mirror", "image", "resync"}, {}, "Force resync to primary image for RBD mirroring.", "",
                            &get_arguments, &execute_resync);
Shell::Action action_status({"mirror", "image", "status"}, {}, "Show RBD mirroring status for an image.", "",
                            &get_status_arguments, &execute_status);
Shell::Action action_snapshot({"mirror", "image", "snapshot"}, {}, "Create RBD mirroring image snapshot.", "",
