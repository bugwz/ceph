...
struct ExportDiffContext
{
    ...
    ExportDiffContext(librbd::Image* i, int f, uint64_t t, int max_ops, bool no_progress, int eformat)
        : image(i)
        , fd(f)
...
{
public:
    C_ExportDiff(ExportDiffContext* edc, uint64_t offset, uint64_t length, bool exists, int export_format)
        : m_export_diff_context(edc)
        , m_offset(offset)
        , m_length(length)
        , m_exists(exists)
    ...
    int send()
    {
        ...
        if (m_exists) {
            ...
            int r = m_export_diff_context->image->aio_read2(m_offset, m_length, m_read_data, aio_completion, op_flags);
            if (r < 0) {
    ...
    static int export_diff_cb(uint64_t offset, size_t length, int exists, void* arg)
    {
...
private:
    ...
    static int write_extent(ExportDiffContext* edc, uint64_t offset, uint64_t length, bool exists, int export_format)
    {
        // extent
        bufferlist bl;
...
int do_export_diff_fd(librbd::Image& image, const char* fromsnapname, const char* endsnapname, bool whole_object,
                      int fd, bool no_progress, int export_format)
{
    ...
    ExportDiffContext edc(
        &image, fd, info.size, g_conf().get_val<uint64_t>("rbd_concurrent_management_ops"), no_progress, export_format);
    r = image.diff_iterate2(fromsnapname, 0, info.size, true, whole_object, &C_ExportDiff::export_diff_cb, (void*)&edc);
    if (r < 0) {
        goto out;
    }
...
int do_export_diff(librbd::Image& image, const char* fromsnapname, const char* endsnapname, bool whole_object,
...
void get_arguments_diff(po::options_description* positional, po::options_description* options)
{
    ...
    options->add_options()(at::FROM_SNAPSHOT_NAME.c_str(), po::value<std::string>(), "snapshot starting point")(
...
int execute_diff(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    librbd::Image image;
    r = utils::init_and_open_image(pool_name, namespace_name, image_name, "", snap_name, true, &rados, &io_ctx, &image);
    if (r < 0) {
        return r;
    }
...
Shell::Action action_diff({"export-diff"}, {}, "Export incremental diff to file.", "", &get_arguments_diff,
                          &execute_diff);

class C_Export : public Context
{
public:
    C_Export(OrderedThrottle& ordered_throttle, librbd::Image& image, uint64_t fd_offset, uint64_t offset,
             uint64_t length, int fd)
        : m_throttle(ordered_throttle)
        , m_image(image)
    ...
    void send()
    {
        ...
        auto aio_completion = new librbd::RBD::AioCompletion(ctx, &utils::aio_context_callback);
        int op_flags = LIBRADOS_OP_FLAG_FADVISE_SEQUENTIAL | LIBRADOS_OP_FLAG_FADVISE_NOCACHE;
        int r = m_image.aio_read2(m_offset, m_length, m_bufferlist, aio_completion, op_flags);
        if (r < 0) {
    ...
    void finish(int r) override
    {
        ...
        if (r < 0) {
            cerr << "rbd: error reading from source image at offset " << m_offset << ": " << cpp_strerror(r)
                 << std::endl;
            return;
        }
        ...
        if (m_fd != STDOUT_FILENO) {
            ...
            if (chkret != m_dest_offset) {
                cerr << "rbd: error seeking destination image to offset " << m_dest_offset << std::endl;
                r = -errno;
                return;
            }
        }
        ...
        if (r < 0) {
            cerr << "rbd: error writing to destination image at offset " << m_dest_offset << std::endl;
        }
    }
...
static int do_export_v2(librbd::Image& image, librbd::image_info_t& info, int fd, uint64_t period,
                        int max_concurrent_ops, utils::ProgressContext& pc)
{
    int r = 0;
    // header
    bufferlist bl;
    ...
    while (more_results) {
        ...
        if (r < 0) {
            std::cerr << "failed to retrieve metadata of image : " << cpp_strerror(r) << std::endl;
            return r;
        }
    ...
    for (std::map<std::string, string>::iterator it = imagemetas.begin(); it != imagemetas.end(); ++it) {
        string key = it->first;
...
static int do_export_v1(librbd::Image& image, librbd::image_info_t& info, int fd, uint64_t period,
                        int max_concurrent_ops, utils::ProgressContext& pc)
{
    int r = 0;
    size_t file_size = 0;
...
Shell::Action action({"export"}, {}, "Export image to file.", "", &get_arguments, &execute);
