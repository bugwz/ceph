...
int execute_add(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    r = utils::mgr_command(rados, "rbd mirror snapshot schedule add", args, &std::cout, &std::cerr);
    if (r < 0) {
        return r;
    }
...
int execute_remove(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    normalize_level_spec_args(&args);
    r = utils::mgr_command(rados, "rbd mirror snapshot schedule remove", args, &std::cout, &std::cerr);
    if (r < 0) {
        return r;
    }
...
int execute_list(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    r = utils::mgr_command(rados, "rbd mirror snapshot schedule list", args, &out, &std::cerr);
    if (r < 0) {
        return r;
    }
...
int execute_status(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    normalize_level_spec_args(&args);
    std::stringstream out;
    r = utils::mgr_command(rados, "rbd mirror snapshot schedule status", args, &out, &std::cerr);
...
Shell::Action add_action({"mirror", "snapshot", "schedule", "add"}, {}, "Add mirror snapshot schedule.", "",
                         &get_arguments_add, &execute_add);
Shell::Action remove_action({"mirror", "snapshot", "schedule", "remove"}, {"mirror", "snapshot", "schedule", "rm"},
                            "Remove mirror snapshot schedule.", "", &get_arguments_remove, &execute_remove);
Shell::Action list_action({"mirror", "snapshot", "schedule", "list"}, {"mirror", "snapshot", "schedule", "ls"},
                          "List mirror snapshot schedule.", "", &get_arguments_list, &execute_list);
Shell::Action status_action({"mirror", "snapshot", "schedule", "status"}, {}, "Show mirror snapshot schedule status.",
