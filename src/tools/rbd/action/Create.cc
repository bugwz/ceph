...
static int do_create(librbd::RBD& rbd, librados::IoCtx& io_ctx, const char* imgname, uint64_t size,
...
struct thick_provision_writer
{
        ...
        : image(i)
    {
        ...
        concurr = std::max<uint64_t>(1U, g_conf().get_val<uint64_t>("rbd_concurrent_management_ops") / stripe_count);
    ...
    int start_io(uint64_t write_offset)
    {
        ...
        r = image->aio_write_zeroes(
            write_offset, chunk_size, c, RBD_WRITE_ZEROES_FLAG_THICK_PROVISION, LIBRADOS_OP_FLAG_FADVISE_SEQUENTIAL);
        if (r < 0) {
            std::lock_guard l{lock};
...
int write_data(librbd::Image& image, librbd::ImageOptions& opts, bool no_progress)
{
    uint64_t image_size;
    int r = 0;
...
int thick_write(const std::string& image_name, librados::IoCtx& io_ctx, librbd::ImageOptions& opts, bool no_progress)
{
    int r;
    librbd::Image image;
...
int execute(const po::variables_map& vm, const std::vector<std::string>& ceph_global_init_args)
{
    ...
    if (!namespace_name.empty() && r == -ENOENT) {
        std::cerr << "rbd: namespace not found - it must be created with "
                  << "'rbd namespace create' before creating an image." << std::endl;
        return r;
    }
    else if (r < 0) {
    ...
    if (vm.count(at::IMAGE_THICK_PROVISION) && vm[at::IMAGE_THICK_PROVISION].as<bool>()) {
        ...
        if (r < 0) {
            std::cerr << "rbd: image created but error encountered during thick provisioning: " << cpp_strerror(r)
                      << std::endl;
            return r;
        }
    }
