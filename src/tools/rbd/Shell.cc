...
boost::intrusive_ptr<CephContext> global_init(int argc, const char** argv, std::vector<std::string>* command_args,
                                              std::vector<std::string>* global_init_args)
{
    std::vector<const char*> cmd_args;
    argv_to_vec(argc, argv, cmd_args);
    std::vector<const char*> args(cmd_args);
    auto cct = global_init(NULL, args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_UTILITY, CINIT_FLAG_NO_MON_CONFIG);
...
std::string format_alias_spec(const Shell::CommandSpec& spec, const Shell::CommandSpec& alias_spec)
{
    auto spec_it = spec.begin();
    auto alias_it = alias_spec.begin();
    int level = 0;
    while (spec_it != spec.end() && alias_it != alias_spec.end() && *spec_it == *alias_it) {
        spec_it++;
        alias_it++;
        level++;
    }
...
std::string format_command_name(const Shell::CommandSpec& spec, const Shell::CommandSpec& alias_spec)
{
...
std::string format_option_suffix(const boost::shared_ptr<po::option_description>& option)
{
    std::string suffix;
...
void print_deprecated_warning(po::option_description option, std::string description)
{
    ...
    if (pos != std::string::npos) {
        ...
        std::cerr << "rbd: " << option.format_name() << " is deprecated, use --" << param << std::endl;
    }
}
...
int Shell::execute(int argc, const char** argv)
{
    ...
    try {
        ...
        argument_opts.add_options()(at::POSITIONAL_COMMAND_SPEC.c_str(),
                                    po::value<std::vector<std::string> >()->required(),
                                    "")(at::POSITIONAL_ARGUMENTS.c_str(), po::value<std::vector<std::string> >(), "");

        po::positional_options_description positional_options;
        positional_options.add(at::POSITIONAL_COMMAND_SPEC.c_str(), matching_spec->size());
        if (!positional_opts.options().empty()) {
            int max_count = positional_opts.options().size();
            if (positional_opts.options().back()->semantic()->max_tokens() > 1) max_count = -1;
            positional_options.add(at::POSITIONAL_ARGUMENTS.c_str(), max_count);
        }

        po::options_description group_opts;
        group_opts.add(command_opts).add(argument_opts);
        ...
        if (vm[at::POSITIONAL_COMMAND_SPEC].as<std::vector<std::string> >() != *matching_spec) {
        ...
        for (; it != ceph_global_init_args.end(); ++it) {
            ...
            for (size_t i = 0; i < global_opts.options().size(); ++i) {
                std::string param_name = global_opts.options()[i]->canonical_display_name(prefix_style);
...
void Shell::get_command_spec(const std::vector<std::string>& arguments, std::vector<std::string>* command_spec)
{
        ...
        if (arg == "-h" || arg == "--help") {
            *command_spec = {HELP_SPEC};
            ...
            if (i + 1 < arguments.size()) {
                command_spec->insert(
                    command_spec->end(), arguments.data() + i + 1, arguments.data() + arguments.size());
            }
            return;
        }
        else if (arg[0] == '-') {
            // if the option is not a switch, skip its value
            if (arg.size() >= 2 && (arg[1] == '-' || get_switch_arguments().count(arg.substr(1, 1)) == 0) &&
                (arg[1] != '-' || get_switch_arguments().count(arg.substr(2, std::string::npos)) == 0) &&
...
Shell::Action* Shell::find_action(const CommandSpec& command_spec, CommandSpec** matching_spec, bool* is_alias)
{
    ...
    for (Action* action : actions) {
        if (action->command_spec.size() <= command_spec.size()) {
            if (std::equal(action->command_spec.begin(), action->command_spec.end(), command_spec.begin())) {
        ...
        if (!action->alias_command_spec.empty() && action->alias_command_spec.size() <= command_spec.size()) {
            if (std::equal(
                    action->alias_command_spec.begin(), action->alias_command_spec.end(), command_spec.begin())) {
                if (matching_spec != NULL) {
...
{
    opts->add_options()((at::CONFIG_PATH + ",c").c_str(), po::value<std::string>(), "path to cluster configuration")(
        "cluster", po::value<std::string>(), "cluster name")(
        "id", po::value<std::string>(), "client id (without 'client.' prefix)")(
        "user", po::value<std::string>(), "deprecated[:id]")("name,n", po::value<std::string>(), "client name")(
...
void Shell::print_help()
{
    std::cout << "usage: " << APP_NAME << " <command> ..." << std::endl
              << std::endl
              << "Command-line interface for managing Ceph RBD images." << std::endl
              << std::endl;

    std::vector<Action*> actions(get_actions());
    std::sort(
        actions.begin(), actions.end(), [](Action* lhs, Action* rhs) { return lhs->command_spec < rhs->command_spec; });

    std::cout << OptionPrinter::POSITIONAL_ARGUMENTS << ":" << std::endl << "  <command>" << std::endl;
    ...
    for (size_t i = 0; i < actions.size(); ++i) {
        ...
        std::string name = format_command_name(action->command_spec, action->alias_command_spec);
    ...
    for (size_t i = 0; i < actions.size(); ++i) {
        ...
        std::stringstream ss;
        ss << indent << format_command_name(action->command_spec, action->alias_command_spec);

        std::cout << ss.str();
        if (!action->description.empty()) {
            IndentStream indent_stream(name_width, ss.str().size(), OptionPrinter::LINE_WIDTH, std::cout);
...
{
    std::cerr << "error: unknown option '" << joinify<std::string>(command_spec.begin(), command_spec.end(), " ") << "'"
              << std::endl
              << std::endl;
    print_help();
}
...
void Shell::print_bash_completion(const CommandSpec& command_spec)
{
    ...
    if (action != nullptr) {
        ...
        for (size_t i = 0; i < get_actions().size(); ++i) {
            ...
            std::cout << "|" << joinify<std::string>(action->command_spec.begin(), action->command_spec.end(), " ");
            if (!action->alias_command_spec.empty()) {
                std::cout << "|"
                          << joinify<std::string>(
                                 action->alias_command_spec.begin(), action->alias_command_spec.end(), " ");
            }
        }
    }
    std::cout << "|" << std::endl;
...
void Shell::print_bash_completion_options(const po::options_description& ops)
{
    for (size_t i = 0; i < ops.options().size(); ++i) {
        ...
        std::string short_name(option->canonical_display_name(po::command_line_style::allow_dash_for_short));
