...
OptionPrinter::OptionPrinter(const OptionsDescription& positional, const OptionsDescription& optional)
...
void OptionPrinter::print_optional(const OptionsDescription& global_opts, size_t& name_width, std::ostream& os)
{
    ...
    for (size_t i = 0; i < global_opts.options().size(); ++i) {
        ...
        std::stringstream ss;
        ss << indent2 << global_opts.options()[i]->format_name() << " " << global_opts.options()[i]->format_parameter();
...
size_t OptionPrinter::compute_name_width(size_t indent)
{
    ...
    for (size_t desc_idx = 0; desc_idx < descs.size(); ++desc_idx) {
        ...
        for (size_t opt_idx = 0; opt_idx < desc.options().size(); ++opt_idx) {
            size_t name_width =
                desc.options()[opt_idx]->format_name().size() + desc.options()[opt_idx]->format_parameter().size() + 1;
