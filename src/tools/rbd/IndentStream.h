...
{
public:
    IndentBuffer(size_t indent, size_t initial_offset, size_t line_length, std::streambuf* streambuf)
        : m_indent(indent)
    ...
    void set_delimiter(const std::string& delim) { m_delim = delim; }
...
{
public:
    IndentStream(size_t indent, size_t initial_offset, size_t line_length, std::ostream& os)
        : std::ostream(&m_indent_buffer)
        , m_indent_buffer(indent, initial_offset, line_length, os.rdbuf())
    {}

    void set_delimiter(const std::string& delim) { m_indent_buffer.set_delimiter(delim); }
