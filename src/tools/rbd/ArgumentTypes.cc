...
{
    switch (modifier) {
    case ARGUMENT_MODIFIER_SOURCE: return SOURCE_PREFIX;
    case ARGUMENT_MODIFIER_DEST: return DEST_PREFIX;
    default: return "";
    }
}
...
{
    switch (modifier) {
    case ARGUMENT_MODIFIER_SOURCE: return "source ";
    case ARGUMENT_MODIFIER_DEST: return "destination ";
    default: return "";
    }
}

void add_pool_option(po::options_description* opt, ArgumentModifier modifier, const std::string& desc_suffix)
{
    std::string name = POOL_NAME + ",p";
    std::string description = "pool name";
    switch (modifier) {
    case ARGUMENT_MODIFIER_NONE: break;
...
void add_namespace_option(boost::program_options::options_description* opt, ArgumentModifier modifier)
{
...
void add_image_option(po::options_description* opt, ArgumentModifier modifier, const std::string& desc_suffix)
{
...
void add_image_id_option(po::options_description* opt, const std::string& desc_suffix)
{
...
void add_snap_option(po::options_description* opt, ArgumentModifier modifier)
{
...
{
    opt->add_options()(SNAPSHOT_ID.c_str(), po::value<uint64_t>(), "snapshot id");
}

void add_pool_options(boost::program_options::options_description* pos,
                      boost::program_options::options_description* opt, bool namespaces_supported)
{
    opt->add_options()((POOL_NAME + ",p").c_str(), po::value<std::string>(), "pool name");
    if (namespaces_supported) {
        add_namespace_option(opt, ARGUMENT_MODIFIER_NONE);
        pos->add_options()("pool-spec",
                           "pool specification\n"
                           "(example: <pool-name>[/<namespace>]");
    }
    else {
        pos->add_options()("pool-name", "pool name");
    }
}

void add_image_spec_options(po::options_description* pos, po::options_description* opt, ArgumentModifier modifier)
{
    pos->add_options()((get_name_prefix(modifier) + IMAGE_SPEC).c_str(),
...
void add_snap_spec_options(po::options_description* pos, po::options_description* opt, ArgumentModifier modifier)
{
    pos->add_options()((get_name_prefix(modifier) + SNAPSHOT_SPEC).c_str(),
                       (get_description_prefix(modifier) + "snapshot specification\n" +
                        "(example: [<pool-name>/[<namespace>/]]<image-name>@<snap-name>)")
                           .c_str());
    add_pool_option(opt, modifier);
    add_namespace_option(opt, modifier);
    add_image_option(opt, modifier);
...
void add_image_or_snap_spec_options(po::options_description* pos, po::options_description* opt,
                                    ArgumentModifier modifier)
{
    pos->add_options()((get_name_prefix(modifier) + IMAGE_OR_SNAPSHOT_SPEC).c_str(),
...
void add_create_image_options(po::options_description* opt, bool include_format)
{
    // TODO get default image format from conf
    if (include_format) {
        opt->add_options()(IMAGE_FORMAT.c_str(), po::value<ImageFormat>(), "image format [default: 2]")(
            IMAGE_NEW_FORMAT.c_str(), po::value<ImageNewFormat>()->zero_tokens(), "deprecated[:image-format 2]");
    }

    opt->add_options()(IMAGE_ORDER.c_str(), po::value<ImageOrder>(), "deprecated[:object-size]")(
        IMAGE_OBJECT_SIZE.c_str(), po::value<ImageObjectSize>(), "object size in B/K/M [4K <= object size <= 32M]")(
        IMAGE_FEATURES.c_str(),
        po::value<ImageFeatures>()->composing(),
        ...
        IMAGE_MIRROR_IMAGE_MODE.c_str(), po::value<MirrorImageMode>(), "mirror image mode [journal or snapshot]");
...
{
    opt->add_options()(JOURNAL_SPLAY_WIDTH.c_str(), po::value<uint64_t>(), "number of active journal objects")(
        JOURNAL_OBJECT_SIZE.c_str(), po::value<JournalObjectSize>(), "size of journal objects [4K <= size <= 64M]")(
        JOURNAL_POOL.c_str(), po::value<std::string>(), "pool for journal objects");
}

void add_size_option(boost::program_options::options_description* opt)
{
    opt->add_options()(
        (IMAGE_SIZE + ",s").c_str(), po::value<ImageSize>()->required(), "image size (in M/G/T) [default: M]");
}

void add_sparse_size_option(boost::program_options::options_description* opt)
{
    opt->add_options()(IMAGE_SPARSE_SIZE.c_str(), po::value<ImageObjectSize>(), "sparse size in B/K/M [default: 4K]");
}

void add_path_options(boost::program_options::options_description* pos,
                      boost::program_options::options_description* opt, const std::string& description)
{
    pos->add_options()(PATH_NAME.c_str(), po::value<std::string>(), description.c_str());
    opt->add_options()(PATH.c_str(), po::value<std::string>(), description.c_str());
}
...
void add_limit_option(po::options_description* opt)
{
    ...
    opt->add_options()(LIMIT.c_str(), po::value<uint64_t>(), description.c_str());
}

void add_no_progress_option(boost::program_options::options_description* opt)
{
    opt->add_options()(NO_PROGRESS.c_str(), po::bool_switch(), "disable progress output");
}

void add_format_options(boost::program_options::options_description* opt)
{
    opt->add_options()(FORMAT.c_str(), po::value<Format>(), "output format (plain, json, or xml) [default: plain]")(
        PRETTY_FORMAT.c_str(), po::bool_switch(), "pretty formatting (json and xml)");
}

void add_verbose_option(boost::program_options::options_description* opt)
{
    opt->add_options()(VERBOSE.c_str(), po::bool_switch(), "be verbose");
}

void add_no_error_option(boost::program_options::options_description* opt)
{
    opt->add_options()(NO_ERR.c_str(), po::bool_switch(), "continue after error");
}

void add_export_format_option(boost::program_options::options_description* opt)
{
    opt->add_options()("export-format", po::value<ExportFormat>(), "format of image file");
}

void add_flatten_option(boost::program_options::options_description* opt)
{
    opt->add_options()(IMAGE_FLATTEN.c_str(), po::bool_switch(), "fill clone with parent data (make it independent)");
}

void add_snap_create_options(po::options_description* opt)
{
    opt->add_options()(SKIP_QUIESCE.c_str(), po::bool_switch(), "do not run quiesce hooks")(
        IGNORE_QUIESCE_ERROR.c_str(), po::bool_switch(), "ignore quiesce hook error");
}

void add_encryption_options(boost::program_options::options_description* opt)
{
    opt->add_options()(ENCRYPTION_FORMAT.c_str(),
                       po::value<std::vector<EncryptionFormat>>(),
                       "encryption format (luks, luks1, luks2) [default: luks]");

    opt->add_options()(ENCRYPTION_PASSPHRASE_FILE.c_str(),
...
std::string get_long_features_help()
{
    ...
    oss << "Image Features:" << std::endl
        ...
        << "  (+) enabled by default for new images if features not specified" << std::endl;
    return oss.str();
}

void validate(boost::any& v, const std::vector<std::string>& values, ImageSize* target_type, int)
{
...
void validate(boost::any& v, const std::vector<std::string>& values, ImageOrder* target_type, int dummy)
{
    po::validators::check_first_occurrence(v);
    const std::string& s = po::validators::get_single_string(values);
    try {
...
void validate(boost::any& v, const std::vector<std::string>& values, ImageObjectSize* target_type, int dummy)
{
    po::validators::check_first_occurrence(v);
    const std::string& s = po::validators::get_single_string(values);
...
void validate(boost::any& v, const std::vector<std::string>& values, ImageFormat* target_type, int dummy)
{
    po::validators::check_first_occurrence(v);
    const std::string& s = po::validators::get_single_string(values);
    try {
...
void validate(boost::any& v, const std::vector<std::string>& values, ImageNewFormat* target_type, int dummy)
{
    v = boost::any(true);
}

void validate(boost::any& v, const std::vector<std::string>& values, ImageFeatures* target_type, int)
{
    if (v.empty()) {
...
void validate(boost::any& v, const std::vector<std::string>& values, MirrorImageMode* mirror_image_mode, int)
{
    po::validators::check_first_occurrence(v);
    const std::string& s = po::validators::get_single_string(values);
    if (s == "journal") {
...
void validate(boost::any& v, const std::vector<std::string>& values, Format* target_type, int)
{
    po::validators::check_first_occurrence(v);
    const std::string& s = po::validators::get_single_string(values);
...
void validate(boost::any& v, const std::vector<std::string>& values, JournalObjectSize* target_type, int)
{
    po::validators::check_first_occurrence(v);
    const std::string& s = po::validators::get_single_string(values);
...
void validate(boost::any& v, const std::vector<std::string>& values, EncryptionAlgorithm* target_type, int)
{
    po::validators::check_first_occurrence(v);
    const std::string& s = po::validators::get_single_string(values);
    if (s == "aes-128") {
...
void validate(boost::any& v, const std::vector<std::string>& values, EncryptionFormat* target_type, int)
{
    po::validators::check_first_occurrence(v);
    const std::string& s = po::validators::get_single_string(values);
    if (s == "luks") {
...
void validate(boost::any& v, const std::vector<std::string>& values, ExportFormat* target_type, int)
{
    po::validators::check_first_occurrence(v);
    const std::string& s = po::validators::get_single_string(values);
...
void validate(boost::any& v, const std::vector<std::string>& values, Secret* target_type, int)
{
