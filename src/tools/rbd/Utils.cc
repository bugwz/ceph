...
static std::string mgr_command_args_to_str(const std::map<std::string, std::string>& args)
{
    std::string out = "";

    std::string delimiter;
    for (auto& it : args) {
        out += delimiter + "\"" + it.first + "\": \"" + stringify(json_stream_escaper(it.second)) + "\"";
        delimiter = ",\n";
    }
...
int ProgressContext::update_progress(uint64_t offset, uint64_t total)
{
    if (progress) {
        ...
        if (pc > last_pc) {
            std::cerr << "\r" << operation << ": " << pc << "% complete..." << std::flush;
            last_pc = pc;
        }
    }
    return 0;
...
{
    if (progress) {
        std::cerr << "\r" << operation << ": " << last_pc << "% complete...failed." << std::endl;
    }
}
...
{
    librbd::RBD::AioCompletion* aio_completion = reinterpret_cast<librbd::RBD::AioCompletion*>(completion);
...
int extract_spec(const std::string& spec, std::string* pool_name, std::string* namespace_name, std::string* name,
    ...
    if (match[1].matched) {
        if (pool_name != nullptr) {
            *pool_name = match[1];
            ...
            std::cerr << "rbd: pool name specified for a command that doesn't use it" << std::endl;
            return -EINVAL;
        }
    }
...
std::string get_positional_argument(const po::variables_map& vm, size_t index)
{
    ...
    const std::vector<std::string>& args =
        boost::any_cast<std::vector<std::string>>(vm[at::POSITIONAL_ARGUMENTS].value());
    if (index < args.size()) {
        return args[index];
    }
    return "";
...
int get_pool_and_namespace_names(const boost::program_options::variables_map& vm, bool validate_pool_name,
                                 std::string* pool_name, std::string* namespace_name, size_t* arg_index)
{
    ...
    if (validate_pool_name && pool_name->find_first_of("/@") != std::string::npos) {
        std::cerr << "rbd: invalid pool '" << *pool_name << "'" << std::endl;
        return -EINVAL;
    }
    else if (namespace_name != nullptr && namespace_name->find_first_of("/@") != std::string::npos) {
        std::cerr << "rbd: invalid namespace '" << *namespace_name << "'" << std::endl;
        return -EINVAL;
    }
...
int get_pool_image_id(const po::variables_map& vm, size_t* spec_arg_index, std::string* pool_name,
                      std::string* namespace_name, std::string* image_id)
{
    ...
    if (image_id != nullptr && spec_arg_index != nullptr && image_id->empty()) {
        ...
        if (!spec.empty()) {
            r = extract_spec(spec, pool_name, namespace_name, image_id, nullptr, SPEC_VALIDATION_FULL);
            if (r < 0) {
                return r;
            }
        }
...
int get_image_or_snap_spec(const po::variables_map& vm, std::string* spec)
{
    ...
    int r = get_pool_image_snapshot_names(vm,
...
void append_options_as_args(const std::vector<std::string>& options, std::vector<std::string>* args)
{
...
int get_pool_image_snapshot_names(const po::variables_map& vm, at::ArgumentModifier mod, size_t* spec_arg_index,
                                  std::string* pool_name, std::string* namespace_name, std::string* image_name,
                                  std::string* snap_name, bool image_name_required, SnapshotPresence snapshot_presence,
                                  SpecValidation spec_validation)
{
    ...
    return get_pool_generic_snapshot_names(vm,
                                           mod,
                                           spec_arg_index,
                                           pool_key,
                                           pool_name,
...
int get_pool_generic_snapshot_names(const po::variables_map& vm, at::ArgumentModifier mod, size_t* spec_arg_index,
                                    const std::string& pool_key, std::string* pool_name, std::string* namespace_name,
                                    const std::string& generic_key, const std::string& generic_key_desc,
                                    std::string* generic_name, std::string* snap_name, bool generic_name_required,
                                    SnapshotPresence snapshot_presence, SpecValidation spec_validation)
{
    ...
    std::string snap_key = (mod == at::ARGUMENT_MODIFIER_DEST ? at::DEST_SNAPSHOT_NAME : at::SNAPSHOT_NAME);
    ...
    if ((generic_key == at::IMAGE_NAME || generic_key == at::DEST_IMAGE_NAME) && generic_name != nullptr &&
        ...
        r = extract_spec(image_name_copy, pool_name, namespace_name, generic_name, snap_name, spec_validation);
        if (r < 0) {
            return r;
        }
    }

    if (generic_name != nullptr && spec_arg_index != nullptr && generic_name->empty()) {
        std::string spec = get_positional_argument(vm, (*spec_arg_index)++);
        if (!spec.empty()) {
    ...
    if (generic_name != nullptr && generic_name_required && generic_name->empty()) {
        std::string prefix = at::get_description_prefix(mod);
        std::cerr << "rbd: " << (mod == at::ARGUMENT_MODIFIER_DEST ? prefix : std::string()) << generic_key_desc
    ...
    if (namespace_name != nullptr && !namespace_name->empty() && !std::regex_match(*namespace_name, pattern)) {
        std::cerr << "rbd: invalid namespace name '" << *namespace_name << "'" << std::endl;
        return -EINVAL;
    }

    if (snap_name != nullptr) {
        r = validate_snapshot_name(mod, *snap_name, snapshot_presence, spec_validation);
        if (r < 0) {
            return r;
        }
    }
...
int validate_snapshot_name(at::ArgumentModifier mod, const std::string& snap_name, SnapshotPresence snapshot_presence,
                           SpecValidation spec_validation)
{
    std::string prefix = at::get_description_prefix(mod);
    switch (snapshot_presence) {
    case SNAPSHOT_PRESENCE_PERMITTED: break;
    case SNAPSHOT_PRESENCE_NONE:
        if (!snap_name.empty()) {
            std::cerr << "rbd: " << (mod == at::ARGUMENT_MODIFIER_DEST ? prefix : std::string())
                      << "snapshot name specified for a command that doesn't use it" << std::endl;
            return -EINVAL;
        }
        break;
...
int get_image_options(const boost::program_options::variables_map& vm, bool get_format, librbd::ImageOptions* opts)
{
    ...
    if (get_format) {
        ...
        if ((stripe_unit || stripe_count) && (stripe_unit != (1ull << order) && stripe_count != 1)) {
            if (format_specified && format == 1) {
    ...
    if (order_specified) opts->set(RBD_IMAGE_OPTION_ORDER, order);
    if (features_specified) opts->set(RBD_IMAGE_OPTION_FEATURES, features);
...
int get_journal_options(const boost::program_options::variables_map& vm, librbd::ImageOptions* opts)
{
    ...
    if (vm.count(at::JOURNAL_SPLAY_WIDTH)) {
        opts->set(RBD_IMAGE_OPTION_JOURNAL_SPLAY_WIDTH, vm[at::JOURNAL_SPLAY_WIDTH].as<uint64_t>());

        int r = g_conf().set_val("rbd_journal_splay_width", stringify(vm[at::JOURNAL_SPLAY_WIDTH].as<uint64_t>()));
        ceph_assert(r == 0);
    }
    if (vm.count(at::JOURNAL_POOL)) {
        opts->set(RBD_IMAGE_OPTION_JOURNAL_POOL, vm[at::JOURNAL_POOL].as<std::string>());

        int r = g_conf().set_val("rbd_journal_pool", vm[at::JOURNAL_POOL].as<std::string>());
        ceph_assert(r == 0);
    }
...
int get_flatten_option(const boost::program_options::variables_map& vm, librbd::ImageOptions* opts)
{
...
int get_image_size(const boost::program_options::variables_map& vm, uint64_t* size)
{
...
int get_path(const boost::program_options::variables_map& vm, size_t* arg_index, std::string* path)
{
    if (vm.count(at::PATH)) {
...
int get_formatter(const po::variables_map& vm, at::Format::Formatter* formatter)
{
...
{
    if (vm[at::SKIP_QUIESCE].as<bool>() && vm[at::IGNORE_QUIESCE_ERROR].as<bool>()) {
        std::cerr << "rbd: " << at::IGNORE_QUIESCE_ERROR << " cannot be used together with " << at::SKIP_QUIESCE
                  << std::endl;
        return -EINVAL;
    }
...
int get_encryption_options(const boost::program_options::variables_map& vm, EncryptionOptions* result)
{
    std::vector<std::string> passphrase_files;
    if (vm.count(at::ENCRYPTION_PASSPHRASE_FILE)) {
        passphrase_files = vm[at::ENCRYPTION_PASSPHRASE_FILE].as<std::vector<std::string>>();
    }
    ...
    if (vm.count(at::ENCRYPTION_FORMAT)) {
        ...
        formats.resize(passphrase_files.size(), at::EncryptionFormat{RBD_ENCRYPTION_FORMAT_LUKS});
    }
    ...
    for (size_t i = 0; i < formats.size(); ++i) {
        ...
        if (file.fail()) {
            std::cerr << "rbd: unable to open passphrase file '" << passphrase_files[i] << "': " << cpp_strerror(errno)
                      << std::endl;
            return -errno;
        }
        std::string passphrase((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
        file.close();

        switch (formats[i].format) {
        case RBD_ENCRYPTION_FORMAT_LUKS:
        {
            auto opts = new librbd::encryption_luks_format_options_t{std::move(passphrase)};
            result->specs.push_back({RBD_ENCRYPTION_FORMAT_LUKS, opts, sizeof(*opts)});
            break;
        }
        case RBD_ENCRYPTION_FORMAT_LUKS1:
        {
            auto opts = new librbd::encryption_luks1_format_options_t{.passphrase = std::move(passphrase)};
            result->specs.push_back({RBD_ENCRYPTION_FORMAT_LUKS1, opts, sizeof(*opts)});
            break;
        }
        case RBD_ENCRYPTION_FORMAT_LUKS2:
        {
            auto opts = new librbd::encryption_luks2_format_options_t{.passphrase = std::move(passphrase)};
            result->specs.push_back({RBD_ENCRYPTION_FORMAT_LUKS2, opts, sizeof(*opts)});
            break;
        }
        default: ceph_abort();
        }
...
int init(const std::string& pool_name, const std::string& namespace_name, librados::Rados* rados,
...
int init_io_ctx(librados::Rados& rados, std::string pool_name, const std::string& namespace_name,
                librados::IoCtx* io_ctx)
{
    normalize_pool_name(&pool_name);
    ...
    if (r < 0) {
        if (r == -ENOENT && pool_name == get_default_pool_name()) {
            ...
            std::cerr << "rbd: error opening pool '" << pool_name << "': " << cpp_strerror(r) << std::endl;
        }
        return r;
    }
...
int set_namespace(const std::string& namespace_name, librados::IoCtx* io_ctx)
{
    if (!namespace_name.empty()) {
        ...
        if (r < 0) {
            std::cerr << "rbd: error asserting namespace: " << cpp_strerror(r) << std::endl;
            return r;
        }
        if (!exists) {
            std::cerr << "rbd: namespace '" << namespace_name << "' does not exist." << std::endl;
            return -ENOENT;
        }
    }
...
int open_image(librados::IoCtx& io_ctx, const std::string& image_name, bool read_only, librbd::Image* image)
{
    int r;
    librbd::RBD rbd;
    if (read_only) {
    ...
    if (r < 0) {
        std::cerr << "rbd: error opening image " << image_name << ": " << cpp_strerror(r) << std::endl;
        return r;
    }
    return 0;
}

int open_image_by_id(librados::IoCtx& io_ctx, const std::string& image_id, bool read_only, librbd::Image* image)
{
    int r;
    librbd::RBD rbd;
    if (read_only) {
    ...
    if (r < 0) {
        std::cerr << "rbd: error opening image with id " << image_id << ": " << cpp_strerror(r) << std::endl;
        return r;
    }
    return 0;
}

int init_and_open_image(const std::string& pool_name, const std::string& namespace_name, const std::string& image_name,
                        const std::string& image_id, const std::string& snap_name, bool read_only,
                        librados::Rados* rados, librados::IoCtx* io_ctx, librbd::Image* image)
{
...
int snap_set(librbd::Image& image, const std::string& snap_name)
{
    ...
    if (r < 0) {
        std::cerr << "error setting snapshot context: " << cpp_strerror(r) << std::endl;
        return r;
    }
    return 0;
}

void calc_sparse_extent(const bufferptr& bp, size_t sparse_size, size_t buffer_offset, uint64_t buffer_length,
                        size_t* write_length, bool* zeroed)
{
    if (sparse_size == 0) {
    ...
    *write_length = 0;
    ...
    while (buffer_offset < buffer_length) {
        size_t extent_size = std::min<size_t>(sparse_size, buffer_length - buffer_offset);
...
{
    switch (mode) {
    case RBD_MIRROR_IMAGE_MODE_JOURNAL: return "journal";
    case RBD_MIRROR_IMAGE_MODE_SNAPSHOT: return "snapshot";
...
{
    switch (state) {
    case RBD_MIRROR_IMAGE_DISABLING: return "disabling";
    case RBD_MIRROR_IMAGE_ENABLED: return "enabled";
    case RBD_MIRROR_IMAGE_DISABLED: return "disabled";
    default: return "unknown";
    }
}
...
{
    switch (state) {
    case MIRROR_IMAGE_STATUS_STATE_UNKNOWN: return "unknown";
    case MIRROR_IMAGE_STATUS_STATE_ERROR: return "error";
    case MIRROR_IMAGE_STATUS_STATE_SYNCING: return "syncing";
    case MIRROR_IMAGE_STATUS_STATE_STARTING_REPLAY: return "starting_replay";
    case MIRROR_IMAGE_STATUS_STATE_REPLAYING: return "replaying";
    case MIRROR_IMAGE_STATUS_STATE_STOPPING_REPLAY: return "stopping_replay";
    case MIRROR_IMAGE_STATUS_STATE_STOPPED: return "stopped";
    default: return "unknown (" + stringify(static_cast<uint32_t>(state)) + ")";
    }
}

std::string mirror_image_site_status_state(const librbd::mirror_image_site_status_t& status)
{
    return (status.up ? "up+" : "down+") + mirror_image_status_state(status.state);
}

std::string mirror_image_global_status_state(const librbd::mirror_image_global_status_t& status)
{
...
int get_local_mirror_image_status(const librbd::mirror_image_global_status_t& status,
                                  librbd::mirror_image_site_status_t* local_status)
{
    auto it = std::find_if(status.site_statuses.begin(), status.site_statuses.end(), [](auto& site_status) {
        return (site_status.mirror_uuid == RBD_MIRROR_IMAGE_STATUS_LOCAL_MIRROR_UUID);
    });
...
bool is_not_user_snap_namespace(librbd::Image* image, const librbd::snap_info_t& snap_info)
{
...
void get_mirror_peer_sites(librados::IoCtx& io_ctx, std::vector<librbd::mirror_peer_site_t>* mirror_peers)
{
...
void get_mirror_peer_mirror_uuids_to_names(const std::vector<librbd::mirror_peer_site_t>& mirror_peers,
...
void populate_unknown_mirror_image_site_statuses(const std::vector<librbd::mirror_peer_site_t>& mirror_peers,
                                                 librbd::mirror_image_global_status_t* global_status)
{
    std::set<std::string> missing_mirror_uuids;
    librbd::mirror_peer_direction_t mirror_peer_direction = RBD_MIRROR_PEER_DIRECTION_RX_TX;
    for (auto& peer : mirror_peers) {
        if (peer.uuid == mirror_peers.begin()->uuid) {
            mirror_peer_direction = peer.direction;
        }
        else if (mirror_peer_direction != RBD_MIRROR_PEER_DIRECTION_RX_TX && mirror_peer_direction != peer.direction) {
            mirror_peer_direction = RBD_MIRROR_PEER_DIRECTION_RX_TX;
        }

        if (!peer.mirror_uuid.empty() && peer.direction != RBD_MIRROR_PEER_DIRECTION_TX) {
    ...
    for (auto& mirror_uuid : missing_mirror_uuids) {
        site_statuses.push_back({mirror_uuid, MIRROR_IMAGE_STATUS_STATE_UNKNOWN, "status not found", 0, false});
    }

    std::swap(global_status->site_statuses, site_statuses);
}

int mgr_command(librados::Rados& rados, const std::string& cmd, const std::map<std::string, std::string>& args,
                std::ostream* out_os, std::ostream* err_os)
{
    std::string command = R"(
    {
      "prefix": ")" + cmd +
                          R"(", )" + mgr_command_args_to_str(args) + R"(
    })";
