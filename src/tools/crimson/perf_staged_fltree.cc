        ...
        return tm_setup()
            .then([this, &kvs] {
                return seastar::async([this, &kvs] {
                    auto tree = std::make_unique<TreeBuilder<TRACK>>(
                        kvs,
...
template<bool TRACK> seastar::future<> run(const bpo::variables_map& config)
{
    return seastar::async([&config] {
        ...
        KVPool kvs{str_sizes, onode_sizes, {range2[0], range2[1]}, {range1[0], range1[1]}, {range0[0], range0[1]}};
...
int main(int argc, char** argv)
{
    ...
    app.add_options()("backend", bpo::value<std::string>()->default_value("dummy"), "tree backend: dummy, seastore")(
        ...
        "str-sizes",
        bpo::value<std::vector<size_t>>()->default_value({8, 11, 64, 256, 301, 320}),
        "sizes of ns/oid strings")(
        "onode-sizes", bpo::value<std::vector<size_t>>()->default_value({8, 16, 128, 512, 576, 640}), "sizes of onode")(
