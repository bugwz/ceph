...
struct client_config
{
    ...
    std::string str() const
    {
        ...
        out << "client[>> " << server_addr << "](bs=" << block_size << ", ramptime=" << ramptime
            << ", msgtime=" << msgtime << ", jobs=" << jobs << ", depth=" << depth
...
struct server_config
{
    ...
    {
        std::ostringstream out;
        out << "server[" << addr << "](bs=" << block_size << ", core=" << core << ", v1-crc-enabled=" << v1_crc_enabled
            << ")";
        return out.str();
    }
    ...
    struct test_state
    {
        ...
        struct Server final : public crimson::net::Dispatcher
        {
            ...
            static seastar::future<ServerFRef> create(seastar::shard_id msgr_sid, unsigned msg_len)
            {
                return seastar::smp::submit_to(
                    msgr_sid, [msg_len] { return seastar::make_foreign(std::make_unique<Server>(msg_len)); });
            }
        };
        ...
        struct Client final : public crimson::net::Dispatcher, public seastar::peering_sharded_service<Client>
        {
            ...
            seastar::future<> init(bool v1_crc_enabled)
            {
                return container().invoke_on_all([v1_crc_enabled](auto& client) {
                    if (client.is_active()) {
                        client.msgr =
                            crimson::net::Messenger::create(entity_name_t::OSD(client.sid), client.lname, client.sid);
            ...
            seastar::future<> shutdown()
            {
                return container().invoke_on_all([](auto& client) {
                    if (client.is_active()) {
                        ...
                        client.msgr->stop();
                        return client.msgr->shutdown().then([&client] { return client.stop_dispatch_messages(); });
                    }
                    return seastar::now();
                });
            }
        ...
        private:
            ...
            public:
                ...
                void report_header()
                {
                    ...
                    sout << std::setfill(' ') << std::setw(7) << "sec" << std::setw(6) << "depth" << std::setw(8)
                         << "IOPS" << std::setw(8) << "MB/s" << std::setw(8) << "lat(ms)";
                ...
                void report_period()
                {
                    ...
                    std::ostringstream sout;
                    sout << setfill(' ') << std::setw(7) << elapsed_s << std::setw(6) << depth << std::setw(8) << iops
                         << std::setw(8) << iops * bytes_of_block / 1048576 << std::setw(8)
                         << (total_lat_s / sampled_count * 1000);
                    std::cout << sout.str() << std::endl;
                }
                ...
                return container()
                    .invoke_on_all([&report](auto& client) {
                        if (client.is_active()) {
                            ...
                            client.period_stats.reset(client.conn_stats.received_count, &snap);
            ...
            seastar::future<> dispatch_with_timer(unsigned ramptime, unsigned msgtime)
            {
                ...
                return container()
                    ...
                    .then([this, msgtime] {
                        ...
                        return seastar::do_with(TimerReport(jobs, msgtime, msg_len), [this](auto& report) {
        ...
        private:
            ...
            void do_dispatch_messages(crimson::net::Connection* conn)
            {
                ...
                (void)seastar::do_until([this] { return stop_send; },
                                        [this, conn] {
                                            sent_count += 1;
    ...
    return seastar::when_all(
        ...
        .then([=](auto&& ret) {
            ...
            if (mode == perf_mode_t::both) {
                ...
                return seastar::when_all_succeed(server->init(server_conf.v1_crc_enabled, server_conf.addr),
                ...
                logger().info("\nperf settings:\n  {}\n", server_conf.str());
                return server
                    ->init(server_conf.v1_crc_enabled, server_conf.addr
                           // dispatch ops
                           )
                    .then([server] {
                        return server->wait();
...
int main(int argc, char** argv)
{
    ...
    app.add_options()("mode", bpo::value<unsigned>()->default_value(0), "0: both, 1:client, 2:server")(
        "addr", bpo::value<std::string>()->default_value("v1:127.0.0.1:9010"), "server address")(
        ...
        "jobs", bpo::value<unsigned>()->default_value(1), "number of client jobs (messengers)")(
        "cbs", bpo::value<unsigned>()->default_value(4096), "client block size")(
        "depth", bpo::value<unsigned>()->default_value(512), "client io depth")(
        "core", bpo::value<unsigned>()->default_value(0), "server running core")(
        "sbs", bpo::value<unsigned>()->default_value(0), "server block size")(
        "v1-crc-enabled", bpo::value<bool>()->default_value(false), "enable v1 CRC checks");
    return app.run(argc, argv, [&app] {
        ...
        return run(_mode, client_conf, server_conf)
            .then([] { logger().info("\nsuccessful!\n"); })
