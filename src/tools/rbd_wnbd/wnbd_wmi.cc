...
{
    dout(20) << "closing wmi conn: " << this << ", svc: " << wbem_svc << ", loc: " << wbem_loc << dendl;
    if (wbem_svc != NULL) {
        wbem_svc->Release();
        wbem_svc = NULL;
    }
...
{
    HRESULT hres = CoCreateInstance(CLSID_WbemLocator, 0, CLSCTX_INPROC_SERVER, IID_IWbemLocator, (LPVOID*)&wbem_loc);
    if (FAILED(hres)) {
    ...
    hres = wbem_loc->ConnectServer(
        _bstr_t(ns.c_str()).GetBSTR(), NULL, NULL, NULL, WBEM_FLAG_CONNECT_USE_MAX_WAIT, NULL, NULL, &wbem_svc);
    if (FAILED(hres)) {
    ...
    hres = CoSetProxyBlanket(wbem_svc,
                             RPC_C_AUTHN_WINNT,
                             RPC_C_AUTHZ_NONE,
                             NULL,
...
HRESULT get_property_str(IWbemClassObject* cls_obj, const std::wstring& property, std::wstring& value)
{
    VARIANT vt_prop;
    VariantInit(&vt_prop);
    ...
    if (FAILED(hres)) {
        derr << "Could not get WMI property: " << to_string(property) << ". HRESULT: " << hres << dendl;
    }
    return hres;
}

HRESULT get_property_int(IWbemClassObject* cls_obj, const std::wstring& property, uint32_t& value)
{
    VARIANT vt_prop;
    VariantInit(&vt_prop);
    HRESULT hres = cls_obj->Get(property.c_str(), 0, &vt_prop, 0, 0);
...
HRESULT WmiSubscription::initialize()
{
    ...
    if (FAILED(hres)) {
        ...
        dout(20) << "wmi subscription initialized: " << this << ", event enum: " << event_enum << ", conn: " << &conn
...
{
    dout(20) << "closing wmi subscription: " << this << ", event enum: " << event_enum << dendl;
    if (event_enum != NULL) {
        event_enum->Release();
        event_enum = NULL;
    }
...
HRESULT WmiSubscription::next(long timeout, ULONG count, IWbemClassObject** objects, ULONG* returned)
{
    if (!event_enum) {
        HRESULT hres = MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, ERROR_INVALID_HANDLE);
...
WmiSubscription subscribe_wnbd_adapter_events(uint32_t interval)
{
    ...
    query_stream << L"SELECT * FROM __InstanceOperationEvent " << L"WITHIN " << interval
                 << L"WHERE TargetInstance ISA 'Win32_ScsiController' " << L"AND TargetInstance.Description="
