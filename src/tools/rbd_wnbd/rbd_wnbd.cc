...
int restart_registered_mappings(int worker_count, int total_timeout, int image_map_timeout)
{
    ...
    while (iterator.get(&cfg)) {
        ...
        boost::asio::post(pool, [&, cfg]() mutable {
...
int disconnect_all_mappings(bool unregister, bool hard_disconnect, int soft_disconnect_timeout, int worker_count)
{
    ...
    while (iterator.get(&cfg)) {
        boost::asio::post(pool, [&, cfg]() mutable {
            ...
            dout(5) << "Removing mapping: " << cfg.devpath << ". Timeout: " << cfg.soft_disconnect_timeout
...
public:
    RBDService(bool _hard_disconnect, int _soft_disconnect_timeout, int _thread_count, int _service_start_timeout,
               int _image_map_timeout, bool _remap_failure_fatal)
    ...
    static int execute_command(ServiceRequest* request)
    {
        ...
        default: dout(5) << "Received unsupported command: " << request->command << dendl; return -ENOSYS;
        }
    }
    ...
    {
        return disconnect_all_mappings(false, hard_disconnect, soft_disconnect_timeout, thread_count);
    }
