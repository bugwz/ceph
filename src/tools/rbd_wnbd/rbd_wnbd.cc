...
WNBDActiveDiskIterator::WNBDActiveDiskIterator()
{
    ...
    case 0:
        // no error
        break;
    case ERROR_OPEN_FAILED: error = -ENOENT; break;
...
{
    reg_key = new RegistryKey(g_ceph_context, HKEY_LOCAL_MACHINE, SERVICE_REG_KEY, false);
    if (!reg_key->hKey) {
        if (!reg_key->missingKey) error = -EINVAL;
        return;
    }

    if (RegQueryInfoKey(reg_key->hKey, NULL, NULL, NULL, &subkey_count, NULL, NULL, NULL, NULL, NULL, NULL, NULL)) {
...
bool RegistryDiskIterator::get(Config* cfg)
{
    ...
    *cfg = Config();
    ...
    int err = RegEnumKeyEx(reg_key->hKey, index, subkey_name, &subkey_name_sz, NULL, NULL, NULL, NULL);
...
int send_map_request(std::string arguments)
{
    ...
    arguments.copy((char*)request->arguments, SERVICE_PIPE_BUFFSZ - FIELD_OFFSET(ServiceRequest, arguments));
    ServiceReply reply = {0};

    DWORD bytes_read = 0;
    BOOL success = CallNamedPipe(SERVICE_PIPE_NAME,
                                 request_buff,
                                 SERVICE_PIPE_BUFFSZ,
                                 &reply,
                                 sizeof(reply),
    ...
    if (reply.status) {
        derr << "The ceph service failed to map the image. "
             ...
             << "Error: " << cpp_strerror(reply.status) << dendl;
    }
...
int map_device_using_suprocess(std::string arguments, int timeout_ms)
{
    ...
    HANDLE pipe_handle = CreateNamedPipe(pipe_name.str().c_str(),
    ...
    command_line << std::quoted(exe_path) << " " << std::regex_replace(arguments, pipe_pattern, "") << " --pipe-name "
                 << pipe_name.str();
    ...
    if (!CreateProcess(
            NULL, (char*)command_line.str().c_str(), NULL, NULL, FALSE, DETACHED_PROCESS, NULL, NULL, &si, &pi)) {
        err = GetLastError();
    ...
    default: derr << "Failed waiting for child process. Status: " << status << dendl; goto clean_process;
    }
...
clean_process:
    ...
    if (exit_code) derr << "Could not start RBD daemon." << dendl;
...
{
    dout(0) << "Received control signal: " << dwCtrlType << ". Exiting." << dendl;
...
int save_config_to_registry(Config* cfg)
{
    ...
    auto reg_key = RegistryKey(g_ceph_context, HKEY_LOCAL_MACHINE, strKey.c_str(), true);
    if (!reg_key.hKey) {
        return -EINVAL;
    }
    ...
    if (reg_key.set("pid", getpid()) || reg_key.set("devpath", cfg->devpath) ||
        reg_key.set("poolname", cfg->poolname) || reg_key.set("nsname", cfg->nsname) ||
        reg_key.set("imgname", cfg->imgname) || reg_key.set("snapname", cfg->snapname) ||
        reg_key.set("command_line", get_cli_args()) || reg_key.set("persistent", cfg->persistent) ||
        reg_key.set("admin_sock_path", g_conf()->admin_socket) || reg_key.flush()) {
        ret_val = -EINVAL;
    }
...
int load_mapping_config_from_registry(string devpath, Config* cfg)
{
    ...
    auto reg_key = RegistryKey(g_ceph_context, HKEY_LOCAL_MACHINE, strKey.c_str(), false);
    if (!reg_key.hKey) {
...
int restart_registered_mappings(int worker_count, int total_timeout, int image_map_timeout)
{
    Config cfg;
    ...
    while (iterator.get(&cfg)) {
        ...
        if (cfg.wnbd_mapped) {
            dout(1) << __func__ << ": device already mapped: " << cfg.devpath << dendl;
            continue;
        }
        if (!cfg.persistent) {
            dout(1) << __func__ << ": cleaning up non-persistent mapping: " << cfg.devpath << dendl;
        ...
        boost::asio::post(pool, [cfg, start_t, counter_freq, total_timeout_ms, image_map_timeout_ms, &err]() {
            ...
            int time_left_ms = max(0, total_timeout_ms - (int)elapsed_ms.QuadPart);
            ...
            dout(1) << "Remapping: " << cfg.devpath << ". Timeout: " << time_left_ms << " ms." << dendl;
...
int disconnect_all_mappings(bool unregister, bool hard_disconnect, int soft_disconnect_timeout, int worker_count)
{
    ...
    QueryPerformanceCounter(&start_t);
    while (iterator.get(&cfg)) {
        boost::asio::post(pool, [cfg, start_t, counter_freq, timeout_ms, hard_disconnect, unregister, &err]() mutable {
            LARGE_INTEGER curr_t, elapsed_ms;
            QueryPerformanceCounter(&curr_t);
            elapsed_ms.QuadPart = curr_t.QuadPart - start_t.QuadPart;
            elapsed_ms.QuadPart *= 1000;
            ...
            dout(1) << "Removing mapping: " << cfg.devpath << ". Timeout: " << cfg.soft_disconnect_timeout
                    << "s. Hard disconnect: " << cfg.hard_disconnect << dendl;
...
public:
    RBDService(bool _hard_disconnect, int _soft_disconnect_timeout, int _thread_count, int _service_start_timeout,
               int _image_map_timeout, bool _remap_failure_fatal, bool _adapter_monitoring_enabled)
    ...
    static int execute_command(ServiceRequest* request)
    {
        ...
        case Connect:
            dout(1) << "Received device connect request. Command line: " << (char*)request->arguments << dendl;
            // TODO: use the configured service map timeout.
            // TODO: add ceph.conf options.
            return map_device_using_suprocess((char*)request->arguments, DEFAULT_MAP_TIMEOUT_MS);
        default: dout(1) << "Received unsupported command: " << request->command << dendl; return -ENOSYS;
        }
    }
    ...
    static DWORD handle_connection(HANDLE pipe_handle)
    {
        ...
        BOOL success = ReadFile(pipe_handle, message, SERVICE_PIPE_BUFFSZ, &bytes_read, NULL);
        ...
        success = WriteFile(pipe_handle, &reply, reply_sz, &bytes_written, NULL);
    ...
    static DWORD accept_pipe_connection()
    {
        ...
        HANDLE pipe_handle = CreateNamedPipe(SERVICE_PIPE_NAME,
                                             PIPE_ACCESS_DUPLEX,
        ...
        HANDLE handler_thread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)handle_connection, pipe_handle, 0, 0);
        if (!handler_thread) {
            err = GetLastError();
    ...
    {
        HANDLE handler_thread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)pipe_server_loop, NULL, 0, 0);
        DWORD err = 0;
    ...
    void monitor_wnbd_adapter()
    {
        ...
        WmiSubscription subscription = subscribe_wnbd_adapter_events(WNBD_ADAPTER_WMI_POLL_INTERVAL);
        ...
        while (!stop_requsted) {
            ...
            if (FAILED(hres)) {
                derr << "couldn't retrieve wnbd adapter events, wmi hresult: " << hres
                     << ". Reestablishing wmi listener in " << WMI_SUBSCRIPTION_RETRY_INTERVAL << " seconds." << dendl;
                subscription.close();
                Sleep(WMI_SUBSCRIPTION_RETRY_INTERVAL * 1000);

                dout(20) << "recreating wnbd adapter wmi subscription" << dendl;
                subscription = subscribe_wnbd_adapter_events(WNBD_ADAPTER_WMI_POLL_INTERVAL);
                hres = subscription.initialize();
                continue;
            }

            dout(20) << "fetching wnbd adapter events" << dendl;
            hres = subscription.next(WMI_EVENT_TIMEOUT * 1000,
            ...
            if (!FAILED(hres) && returned) {
                if (WBEM_S_NO_ERROR == object->InheritsFrom(L"__InstanceCreationEvent")) {
                    ...
                    restart_registered_mappings(thread_count, service_start_timeout, image_map_timeout);
                }
    ...
    int run_hook() override
    {
        ...
        int r = restart_registered_mappings(thread_count, service_start_timeout, image_map_timeout);
        if (r) {
    ...
    int stop_hook() override
    {
        ...
        int r = disconnect_all_mappings(false, hard_disconnect, soft_disconnect_timeout, thread_count);
    ...
    int shutdown_hook() override { return stop_hook(); }
};
...
public:
    WNBDWatchCtx(librados::IoCtx& io_ctx, WnbdHandler* handler, librbd::Image& image, uint64_t size)
        : io_ctx(io_ctx)
        , handler(handler)
        , image(image)
        , size(size)
    ...
    void handle_notify() override
    {
        ...
        if (image.size(&new_size) == 0 && new_size != size && handler->resize(new_size) == 0) {
            size = new_size;
        }
    }
};
...
boost::intrusive_ptr<CephContext> do_global_init(int argc, const char* argv[], Config* cfg)
{
    ...
    case Service:
        code_env = CODE_ENVIRONMENT_DAEMON;
        flags = CINIT_FLAG_UNPRIVILEGED_DAEMON_DEFAULTS | CINIT_FLAG_NO_MON_CONFIG | CINIT_FLAG_NO_DAEMON_ACTIONS;
        break;
    default:
    ...
    auto cct = global_init(NULL, args, CEPH_ENTITY_TYPE_CLIENT, code_env, flags, FALSE);
...
static int do_map(Config* cfg)
{
    ...
    if (r < 0) goto close_ret;

    if (info.size > _UI64_MAX) {
        r = -EFBIG;
        derr << "rbd-wnbd: image is too large (" << byte_u_t(info.size) << ", max is " << byte_u_t(_UI64_MAX) << ")"
             << dendl;
        goto close_ret;
    }
    ...
    if (!cfg->parent_pipe.empty()) {
        parent_pipe_handle = CreateFile(cfg->parent_pipe.c_str(), GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
...

    {
        ...
        if (r < 0) {
            derr << __func__ << ": update_watch failed with error: " << cpp_strerror(r) << dendl;
        ...
        if (r < 0) derr << __func__ << ": update_unwatch failed with error: " << cpp_strerror(r) << dendl;
...
static int do_list_mapped_devices(const std::string& format, bool pretty_format)
{
    ...
    while (wnbd_disk_iterator.get(&cfg)) {
        const char* status = cfg.active ? WNBD_STATUS_ACTIVE : WNBD_STATUS_INACTIVE;
        ...
        if (f) {
            ...
            tbl << (cfg.pid ? cfg.pid : -1) << cfg.poolname << cfg.nsname << cfg.imgname << cfg.snapname << cfg.devpath
...
static int do_show_mapped_device(std::string format, bool pretty_format, std::string devpath)
{
    std::unique_ptr<ceph::Formatter> f;
    TextTable tbl;
...
static int do_stats(std::string search_devpath)
{
    ...
    while (wnbd_disk_iterator.get(&cfg)) {
        if (cfg.devpath != search_devpath) continue;

        AdminSocketClient client = AdminSocketClient(cfg.admin_sock_path);
        std::string output;
        std::string result = client.do_request("{\"prefix\":\"wnbd stats\"}", &output);
        if (!result.empty()) {
...
static int parse_args(std::vector<const char*>& args, std::ostream* err_msg, Command* command, Config* cfg)
{
    ...
    for (i = args.begin(); i != args.end();) {
        ...
        else if (ceph_argparse_witharg(args, i, &cfg->parent_pipe, err, "--pipe-name", (char*)NULL)) {
            if (!err.str().empty()) {
                *err_msg << "rbd-wnbd: " << err.str();
                return -EINVAL;
            }
        }
        else if (ceph_argparse_witharg(args, i, (int*)&cfg->wnbd_log_level, err, "--wnbd-log-level", (char*)NULL)) {
            if (!err.str().empty()) {
                *err_msg << "rbd-wnbd: " << err.str();
                return -EINVAL;
            }
        ...
        else if (ceph_argparse_witharg(args, i, (int*)&cfg->io_req_workers, err, "--io-req-workers", (char*)NULL)) {
            if (!err.str().empty()) {
                *err_msg << "rbd-wnbd: " << err.str();
                return -EINVAL;
            }
        ...
        else if (ceph_argparse_witharg(args, i, (int*)&cfg->io_reply_workers, err, "--io-reply-workers", (char*)NULL)) {
            if (!err.str().empty()) {
                *err_msg << "rbd-wnbd: " << err.str();
                return -EINVAL;
            }
            if (cfg->io_reply_workers <= 0) {
                *err_msg << "rbd-wnbd: Invalid argument for io-reply-workers";
                return -EINVAL;
                     ...
                     args, i, (int*)&cfg->service_thread_count, err, "--service-thread-count", (char*)NULL)) {
            if (!err.str().empty()) {
                *err_msg << "rbd-wnbd: " << err.str();
                return -EINVAL;
            }
        ...
        else if (ceph_argparse_flag(args, i, "--no-hard-disconnect-fallback", (char*)NULL)) {
            cfg->hard_disconnect_fallback = false;
        }
        else if (ceph_argparse_witharg(
                     args, i, (int*)&cfg->soft_disconnect_timeout, err, "--soft-disconnect-timeout", (char*)NULL)) {
            if (!err.str().empty()) {
                *err_msg << "rbd-wnbd: " << err.str();
                return -EINVAL;
            }
            if (cfg->soft_disconnect_timeout < 0) {
                *err_msg << "rbd-wnbd: Invalid argument for soft-disconnect-timeout";
                return -EINVAL;
                     ...
                     args, i, (int*)&cfg->service_start_timeout, err, "--start-timeout", (char*)NULL)) {
            if (!err.str().empty()) {
                *err_msg << "rbd-wnbd: " << err.str();
                return -EINVAL;
            }
        ...
        else if (ceph_argparse_witharg(args, i, (int*)&cfg->image_map_timeout, err, "--map-timeout", (char*)NULL)) {
            if (!err.str().empty()) {
                *err_msg << "rbd-wnbd: " << err.str();
                return -EINVAL;
            }
...
static int rbd_wnbd(int argc, const char* argv[])
{
    ...
    case Service:
    {
        RBDService service(cfg.hard_disconnect,
                           cfg.soft_disconnect_timeout,
