...
int WnbdAdminHook::call(std::string_view command, const cmdmap_t& cmdmap, const bufferlist&, Formatter* f,
...
void WnbdHandler::aio_callback(librbd::completion_t cb, void* arg)
{
    ...
    else if ((ctx->req_type == WnbdReqTypeRead) && ret < static_cast<int>(ctx->req_size)) {
    ...
    boost::asio::post(*ctx->handler->reply_tpool, [&, ctx]() { ctx->handler->send_io_response(ctx); });
...
void WnbdHandler::send_io_response(WnbdHandler::IOContext* ctx)
{
    ...
    err = WnbdSendResponseEx(pctx->handler->wnbd_disk, &wnbd_rsp, pctx->data.c_str(), pctx->data.length(), overlapped);
    if (err == ERROR_IO_PENDING) {
        ...
        if (!GetOverlappedResult(pctx->handler->wnbd_disk, overlapped, &returned_bytes, TRUE)) {
            err = GetLastError();
...
void WnbdHandler::Read(PWNBD_DISK Disk, UINT64 RequestHandle, PVOID Buffer, UINT64 BlockAddress, UINT32 BlockCount,
...
void WnbdHandler::Write(PWNBD_DISK Disk, UINT64 RequestHandle, PVOID Buffer, UINT64 BlockAddress, UINT32 BlockCount,
                        BOOLEAN ForceUnitAccess)
{
    WnbdHandler* handler = nullptr;
    ceph_assert(!WnbdGetUserContext(Disk, (PVOID*)&handler));
...
void WnbdHandler::Flush(PWNBD_DISK Disk, UINT64 RequestHandle, UINT64 BlockAddress, UINT32 BlockCount)
{
    WnbdHandler* handler = nullptr;
    ceph_assert(!WnbdGetUserContext(Disk, (PVOID*)&handler));
...
void WnbdHandler::Unmap(PWNBD_DISK Disk, UINT64 RequestHandle, PWNBD_UNMAP_DESCRIPTOR Descriptors, UINT32 Count)
{
    WnbdHandler* handler = nullptr;
    ceph_assert(!WnbdGetUserContext(Disk, (PVOID*)&handler));
    ceph_assert(1 == Count);
...
void WnbdHandler::LogMessage(WnbdLogLevel LogLevel, const char* Message, const char* FileName, UINT32 Line,
...
int WnbdHandler::resize(uint64_t new_size)
{
    ...
    dout(5) << "Resizing disk. Block size: " << block_size << ". New block count: " << new_block_count
            << ". Old block count: " << wnbd_disk->Properties.BlockCount << "." << dendl;
    err = WnbdSetDiskSize(wnbd_disk, new_block_count);
    if (err) {
        derr << "WNBD: Setting disk size failed with error: " << win32_strerror(err) << dendl;
        return -EINVAL;
    }

    dout(5) << "Successfully resized disk to: " << new_block_count << " blocks" << dendl;
    return 0;
}
...
std::ostream& operator<<(std::ostream& os, const WnbdHandler::IOContext& ctx)
{
    ...
    switch (ctx.req_type) {
    case WnbdReqTypeRead: os << " READ "; break;
    case WnbdReqTypeWrite: os << " WRITE "; break;
    case WnbdReqTypeFlush: os << " FLUSH "; break;
    case WnbdReqTypeUnmap: os << " TRIM "; break;
    default: os << " UNKNOWN(" << ctx.req_type << ") "; break;
    }

    os << ctx.req_from << "~" << ctx.req_size << " " << std::dec << ntohl(ctx.err_code) << "]";
