...
struct FileOfLines
{
    ...
    void advance()
    {
        ...
        if (this_line.empty()) {
            if (!input.eof()) {
                std::cerr << "Error: " << filename << " has an empty line." << std::endl;
                exit(4);
            }
            is_eof = true;
            return;
        ...
        if (this_line < prev_line) {
            std::cerr << "Error: " << filename << " is not in sorted order; \"" << this_line << "\" follows \""
                      << prev_line << "\"." << std::endl;
            exit(4);
        }
    }

    const std::string line() const { return this_line; }
};
