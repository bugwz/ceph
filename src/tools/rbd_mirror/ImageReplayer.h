 ...
 */
template<typename ImageCtxT = librbd::ImageCtx> class ImageReplayer
{
public:
    ...
    {
        return new ImageReplayer(local_io_ctx,
                                 local_mirror_uuid,
                                 global_image_id,
                                 threads,
                                 instance_watcher,
    ...
    ImageReplayer(librados::IoCtx& local_io_ctx, const std::string& local_mirror_uuid,
                  const std::string& global_image_id, Threads<ImageCtxT>* threads,
                  InstanceWatcher<ImageCtxT>* instance_watcher, MirrorStatusUpdater<ImageCtxT>* local_status_updater,
                  journal::CacheManagerHandler* cache_manager_handler, PoolMetaCache* pool_meta_cache);
    ...
    inline int64_t get_local_pool_id() const { return m_local_io_ctx.get_id(); }
    inline const std::string& get_global_image_id() const { return m_global_image_id; }
...
private:
    ...
    typedef boost::optional<cls::rbd::MirrorImageStatusState> OptionalMirrorImageStatusState;
    ...
    public:
        ...
        void update_progress(const std::string& description, bool flush = true) override;
    ...
    bool is_stopped_() const { return m_state == STATE_STOPPED; }
    bool is_running_() const { return !is_stopped_() && m_state != STATE_STOPPING && !m_stop_requested; }
    bool is_replaying_() const { return (m_state == STATE_REPLAYING); }
