...
template<typename ImageCtxT = librbd::ImageCtx> class InstanceReplayer
{
public:
    ...
    {
        return new InstanceReplayer(local_io_ctx,
                                    local_mirror_uuid,
                                    threads,
                                    service_daemon,
    ...
    InstanceReplayer(librados::IoCtx& local_io_ctx, const std::string& local_mirror_uuid, Threads<ImageCtxT>* threads,
                     ServiceDaemon<ImageCtxT>* service_daemon, MirrorStatusUpdater<ImageCtxT>* local_status_updater,
                     journal::CacheManagerHandler* cache_manager_handler, PoolMetaCache* pool_meta_cache);
    ~InstanceReplayer();
    ...
    void acquire_image(InstanceWatcher<ImageCtxT>* instance_watcher, const std::string& global_image_id,
                       Context* on_finish);
    void release_image(const std::string& global_image_id, Context* on_finish);
    void remove_peer_image(const std::string& global_image_id, const std::string& peer_mirror_uuid, Context* on_finish);
...
private:
    ...
    void stop_image_replayer(ImageReplayer<ImageCtxT>* image_replayer, Context* on_finish);
