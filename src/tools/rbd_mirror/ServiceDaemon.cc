...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::ServiceDaemon: " << this << " " << __func__ << ": "
...
struct AttributeDumpVisitor : public boost::static_visitor<void>
{
    ...
    void operator()(bool val) const { f->dump_bool(name.c_str(), val); }
    void operator()(uint64_t val) const { f->dump_unsigned(name.c_str(), val); }
    void operator()(const std::string& val) const { f->dump_string(name.c_str(), val); }
};
...
template<typename I>
ServiceDaemon<I>::ServiceDaemon(CephContext* cct, RadosRef rados, Threads<I>* threads)
    : m_cct(cct)
    , m_rados(rados)
    , m_threads(threads)
{
...
template<typename I> ServiceDaemon<I>::~ServiceDaemon()
{
    dout(20) << dendl;
...
template<typename I> int ServiceDaemon<I>::init()
{
    dout(20) << dendl;
    ...
    std::map<std::string, std::string> service_metadata = {{"id", id}, {"instance_id", instance_id}};
    int r = m_rados->service_daemon_register("rbd-mirror", instance_id, service_metadata);
    if (r < 0) {
        return r;
    }
...
template<typename I> void ServiceDaemon<I>::remove_pool(int64_t pool_id)
{
...
template<typename I> void ServiceDaemon<I>::add_namespace(int64_t pool_id, const std::string& namespace_name)
{
...
template<typename I> void ServiceDaemon<I>::remove_namespace(int64_t pool_id, const std::string& namespace_name)
{
    dout(20) << "pool_id=" << pool_id << ", namespace=" << namespace_name << dendl;
    {
        std::lock_guard locker{m_lock};
...
template<typename I>
uint64_t ServiceDaemon<I>::add_or_update_callout(int64_t pool_id, uint64_t callout_id, CalloutLevel callout_level,
...
template<typename I> void ServiceDaemon<I>::remove_callout(int64_t pool_id, uint64_t callout_id)
{
    dout(20) << "pool_id=" << pool_id << ", "
...
template<typename I>
void ServiceDaemon<I>::add_or_update_attribute(int64_t pool_id, const std::string& key, const AttributeValue& value)
{
    dout(20) << "pool_id=" << pool_id << ", "
             << "key=" << key << ", "
             << "value=" << value << dendl;
...
template<typename I>
void ServiceDaemon<I>::add_or_update_namespace_attribute(int64_t pool_id, const std::string& namespace_name,
                                                         const std::string& key, const AttributeValue& value)
{
...
template<typename I> void ServiceDaemon<I>::remove_attribute(int64_t pool_id, const std::string& key)
{
    dout(20) << "pool_id=" << pool_id << ", "
             << "key=" << key << dendl;
...
template<typename I> void ServiceDaemon<I>::schedule_update_status()
{
    std::lock_guard timer_locker{m_threads->timer_lock};
    if (m_timer_ctx != nullptr) {
        return;
...
template<typename I> void ServiceDaemon<I>::update_status()
{
    dout(20) << dendl;
