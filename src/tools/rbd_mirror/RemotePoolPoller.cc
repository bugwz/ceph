...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::RemotePoolPoller: " << this << " " << __func__ << ": "
...
template<typename I> RemotePoolPoller<I>::~RemotePoolPoller()
{
    ceph_assert(m_timer_task == nullptr);
}

template<typename I> void RemotePoolPoller<I>::init(Context* on_finish)
{
    dout(10) << dendl;
...
template<typename I> void RemotePoolPoller<I>::shut_down(Context* on_finish)
{
    dout(10) << dendl;
...
template<typename I> void RemotePoolPoller<I>::get_mirror_uuid()
{
    dout(10) << dendl;
    ...
    auto aio_comp = create_rados_callback<RemotePoolPoller<I>, &RemotePoolPoller<I>::handle_get_mirror_uuid>(this);
    m_out_bl.clear();
...
template<typename I> void RemotePoolPoller<I>::handle_get_mirror_uuid(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void RemotePoolPoller<I>::mirror_peer_ping()
{
    dout(10) << dendl;
    ...
    auto aio_comp = create_rados_callback<RemotePoolPoller<I>, &RemotePoolPoller<I>::handle_mirror_peer_ping>(this);
    int r = m_remote_io_ctx.aio_operate(RBD_MIRRORING, aio_comp, &op);
    ceph_assert(r == 0);
    aio_comp->release();
}

template<typename I> void RemotePoolPoller<I>::handle_mirror_peer_ping(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void RemotePoolPoller<I>::mirror_peer_list()
{
    dout(10) << dendl;
    ...
    auto aio_comp = create_rados_callback<RemotePoolPoller<I>, &RemotePoolPoller<I>::handle_mirror_peer_list>(this);
    m_out_bl.clear();
    int r = m_remote_io_ctx.aio_operate(RBD_MIRRORING, aio_comp, &op, &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
...
template<typename I> void RemotePoolPoller<I>::handle_mirror_peer_list(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void RemotePoolPoller<I>::notify_listener()
{
    bool updated = false;
...
template<typename I> void RemotePoolPoller<I>::schedule_task(int r)
{
    std::unique_lock locker{m_threads->timer_lock};
    ...
    if (m_state == STATE_POLLING) {
        ...
        ceph_assert(m_timer_task == nullptr);
        m_timer_task = new LambdaContext([this](int) { handle_task(); });
...
template<typename I> void RemotePoolPoller<I>::handle_task()
{
    dout(10) << dendl;
    ...
    auto ctx = new LambdaContext([this](int) { get_mirror_uuid(); });
