...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::PoolWatcher: " << this << " " << __func__ << ": "
...
template<typename I> class PoolWatcher<I>::MirroringWatcher : public librbd::MirroringWatcher<I>
{
public:
    using ContextWQ = typename std::decay<typename std::remove_pointer<decltype(Threads<I>::work_queue)>::type>::type;

    MirroringWatcher(librados::IoCtx& io_ctx, ContextWQ* work_queue, PoolWatcher* pool_watcher)
        : librbd::MirroringWatcher<I>(io_ctx, work_queue)
        , m_pool_watcher(pool_watcher)
    {}

    void handle_rewatch_complete(int r) override { m_pool_watcher->handle_rewatch_complete(r); }
    ...
    void handle_image_updated(cls::rbd::MirrorImageState state, const std::string& image_id,
...
template<typename I>
PoolWatcher<I>::PoolWatcher(Threads<I>* threads, librados::IoCtx& io_ctx, const std::string& mirror_uuid,
...
{
    m_mirroring_watcher = new MirroringWatcher(m_io_ctx, m_threads->work_queue, this);
}

template<typename I> PoolWatcher<I>::~PoolWatcher()
{
    delete m_mirroring_watcher;
}

template<typename I> bool PoolWatcher<I>::is_blocklisted() const
{
    std::lock_guard locker{m_lock};
    return m_blocklisted;
}

template<typename I> void PoolWatcher<I>::init(Context* on_finish)
{
    dout(5) << dendl;
...
template<typename I> void PoolWatcher<I>::shut_down(Context* on_finish)
{
    dout(5) << dendl;
...
template<typename I> void PoolWatcher<I>::register_watcher()
{
    {
        std::lock_guard locker{m_lock};
    ...
    Context* ctx = create_context_callback<PoolWatcher, &PoolWatcher<I>::handle_register_watcher>(this);
    m_mirroring_watcher->register_watch(ctx);
}

template<typename I> void PoolWatcher<I>::handle_register_watcher(int r)
{
    dout(5) << "r=" << r << dendl;
...
template<typename I> void PoolWatcher<I>::unregister_watcher()
{
    dout(5) << dendl;
    ...
    Context* ctx = new LambdaContext([this](int r) {
        ...
        if (r < 0) {
            derr << "error unregistering watcher for " << m_mirroring_watcher->get_oid()
...
template<typename I> void PoolWatcher<I>::refresh_images()
{
    dout(5) << dendl;
    ...
    Context* ctx = create_context_callback<PoolWatcher, &PoolWatcher<I>::handle_refresh_images>(this);
    auto req = pool_watcher::RefreshImagesRequest<I>::create(m_io_ctx, &m_refresh_image_ids, ctx);
    req->send();
}

template<typename I> void PoolWatcher<I>::handle_refresh_images(int r)
{
    dout(5) << "r=" << r << dendl;
...
template<typename I> void PoolWatcher<I>::schedule_refresh_images(double interval)
{
    std::scoped_lock locker{m_threads->timer_lock, m_lock};
    ...
    m_timer_ctx =
        m_threads->timer->add_event_after(interval, new LambdaContext([this](int r) { process_refresh_images(); }));
}

template<typename I> void PoolWatcher<I>::handle_rewatch_complete(int r)
{
    dout(5) << "r=" << r << dendl;
...
template<typename I>
void PoolWatcher<I>::handle_image_updated(const std::string& id, const std::string& global_image_id, bool enabled)
{
    dout(10) << "image_id=" << id << ", "
...
template<typename I> void PoolWatcher<I>::process_refresh_images()
{
...
template<typename I> void PoolWatcher<I>::schedule_listener()
{
...
template<typename I> void PoolWatcher<I>::notify_listener()
{
    dout(10) << dendl;
    ...
    m_listener.handle_update(m_mirror_uuid, std::move(added_image_ids), std::move(removed_image_ids));
