...
template<typename ImageCtxT = librbd::ImageCtx> class ServiceDaemon
{
public:
    ...
    uint64_t add_or_update_callout(int64_t pool_id, uint64_t callout_id, service_daemon::CalloutLevel callout_level,
                                   const std::string& text);
    void remove_callout(int64_t pool_id, uint64_t callout_id);

    void add_or_update_attribute(int64_t pool_id, const std::string& key, const service_daemon::AttributeValue& value);
    void add_or_update_namespace_attribute(int64_t pool_id, const std::string& namespace_name, const std::string& key,
...
private:
    struct Callout
    {
        ...
        Callout()
            : level(service_daemon::CALLOUT_LEVEL_INFO)
        {}
