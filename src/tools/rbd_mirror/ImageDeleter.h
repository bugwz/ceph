...
template<typename> struct TrashWatcher;
}

/**
 * Manage deletion of non-primary images.
 */
template<typename ImageCtxT = librbd::ImageCtx> class ImageDeleter
{
public:
    static ImageDeleter* create(librados::IoCtx& local_io_ctx, Threads<librbd::ImageCtx>* threads,
                                Throttler<librbd::ImageCtx>* image_deletion_throttler,
                                ServiceDaemon<librbd::ImageCtx>* service_daemon)
    {
        return new ImageDeleter(local_io_ctx, threads, image_deletion_throttler, service_daemon);
    }

    ImageDeleter(librados::IoCtx& local_io_ctx, Threads<librbd::ImageCtx>* threads,
                 Throttler<librbd::ImageCtx>* image_deletion_throttler,
                 ServiceDaemon<librbd::ImageCtx>* service_daemon);
    ...
    void wait_for_deletion(const std::string& image_id, bool scheduled_only, Context* on_finish);

    std::vector<std::string> get_delete_queue_items();
    std::vector<std::pair<std::string, int> > get_failed_queue_items();

    inline void set_busy_timer_interval(double interval) { m_busy_interval = interval; }
...
private:
    ...
    struct TrashListener : public image_deleter::TrashListener
    {
        ...
        TrashListener(ImageDeleter* image_deleter)
    ...
    struct DeleteInfo
    {
        ...
        inline bool operator==(const DeleteInfo& delete_info) const { return (image_id == delete_info.image_id); }
        ...
        void print_status(Formatter* f, bool print_failure_info = false);
    };
    ...
    void enqueue_failed_delete(DeleteInfoRef* delete_info, int error_code, double retry_delay);
    ...
    void handle_trash_image(const std::string& image_id, const clock_t::time_point& deferment_end_time);
