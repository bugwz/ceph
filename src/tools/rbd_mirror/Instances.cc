...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::Instances: " << this << " " << __func__ << ": "
...
template<typename I>
Instances<I>::Instances(Threads<I>* threads, librados::IoCtx& ioctx, const std::string& instance_id,
...
template<typename I> Instances<I>::~Instances() {}

template<typename I> void Instances<I>::init(Context* on_finish)
{
    dout(10) << dendl;
...
template<typename I> void Instances<I>::shut_down(Context* on_finish)
{
    dout(10) << dendl;
...
template<typename I> void Instances<I>::unblock_listener()
{
    dout(5) << dendl;
    ...
    if (!added_instance_ids.empty()) {
        m_threads->work_queue->queue(new C_NotifyInstancesAdded(this, added_instance_ids), 0);
    }
}

template<typename I> void Instances<I>::acked(const InstanceIds& instance_ids)
{
...
template<typename I> void Instances<I>::handle_acked(const InstanceIds& instance_ids)
{
    ...
    for (auto& instance_id : instance_ids) {
        auto& instance = m_instances.insert(std::make_pair(instance_id, Instance{})).first->second;
...
template<typename I> void Instances<I>::notify_instances_added(const InstanceIds& instance_ids)
{
...
template<typename I> void Instances<I>::notify_instances_removed(const InstanceIds& instance_ids)
{
    dout(5) << "instance_ids=" << instance_ids << dendl;
...
template<typename I> void Instances<I>::list(std::vector<std::string>* instance_ids)
{
    dout(20) << dendl;
...
template<typename I> void Instances<I>::get_instances()
{
    dout(10) << dendl;

    ceph_assert(ceph_mutex_is_locked(m_lock));

    Context* ctx = create_context_callback<Instances, &Instances<I>::handle_get_instances>(this);

    InstanceWatcher<I>::get_instances(m_ioctx, &m_instance_ids, ctx);
}

template<typename I> void Instances<I>::handle_get_instances(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Instances<I>::wait_for_ops()
{
    dout(10) << dendl;
    ...
    Context* ctx = create_async_context_callback(
        m_threads->work_queue, create_context_callback<Instances, &Instances<I>::handle_wait_for_ops>(this));

    m_async_op_tracker.wait_for_ops(ctx);
}

template<typename I> void Instances<I>::handle_wait_for_ops(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Instances<I>::remove_instances(const Instances<I>::clock_t::time_point& time)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ...
    for (auto& instance_pair : m_instances) {
        ...
        if (instance.state != INSTANCE_STATE_REMOVING && instance.acked_time <= time) {
    ...
    dout(10) << "instance_ids=" << instance_ids << dendl;
    Context* ctx = new LambdaContext([this, instance_ids](int r) { handle_remove_instances(r, instance_ids); });
    ctx = create_async_context_callback(m_threads->work_queue, ctx);

    auto gather_ctx = new C_Gather(m_cct, ctx);
    for (auto& instance_id : instance_ids) {
        InstanceWatcher<I>::remove_instance(m_ioctx, *m_threads->asio_engine, instance_id, gather_ctx->new_sub());
    }
...
template<typename I> void Instances<I>::handle_remove_instances(int r, const InstanceIds& instance_ids)
{
    std::scoped_lock locker{m_threads->timer_lock, m_lock};
    ...
    m_threads->work_queue->queue(new C_NotifyInstancesRemoved(this, instance_ids), 0);
...
template<typename I> void Instances<I>::cancel_remove_task()
{
...
template<typename I> void Instances<I>::schedule_remove_task(const Instances<I>::clock_t::time_point& time)
{
    cancel_remove_task();
    if (m_on_finish != nullptr) {
    ...
    m_timer_task = new LambdaContext([this, oldest_time](int r) {
        ceph_assert(ceph_mutex_is_locked(m_threads->timer_lock));
        std::lock_guard locker{m_lock};
        m_timer_task = nullptr;
