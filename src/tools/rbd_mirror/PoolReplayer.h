...
{
public:
    PoolReplayer(Threads<ImageCtxT>* threads, ServiceDaemon<ImageCtxT>* service_daemon,
...
private:
    ...
    void namespace_replayer_acquire_leader(const std::string& name, Context* on_finish);
    ...
    template<typename L> void with_namespace_replayers(L&& callback)
    {
    ...
    // async version
    template<typename L> void with_namespace_replayers(L&& callback, Context* on_finish)
    {
        std::lock_guard locker{m_lock};
    ...
    protected:
        ...
        void pre_release_handler(Context* on_finish) override { m_pool_replayer->handle_pre_release_leader(on_finish); }
