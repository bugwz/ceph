...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::image_deleter::TrashWatcher: " << this << " " << __func__ << ": "
...
template<typename I>
TrashWatcher<I>::TrashWatcher(librados::IoCtx& io_ctx, Threads<I>* threads, TrashListener& trash_listener)
    : librbd::TrashWatcher<I>(io_ctx, threads->work_queue)
    , m_io_ctx(io_ctx)
    , m_threads(threads)
    , m_trash_listener(trash_listener)
    , m_lock(ceph::make_mutex(librbd::util::unique_lock_name("rbd::mirror::image_deleter::TrashWatcher", this)))
{}

template<typename I> void TrashWatcher<I>::init(Context* on_finish)
{
    dout(5) << dendl;
...
template<typename I> void TrashWatcher<I>::shut_down(Context* on_finish)
{
    dout(5) << dendl;
    ...
    auto ctx = new LambdaContext([this, on_finish](int r) { unregister_watcher(on_finish); });
    m_async_op_tracker.wait_for_ops(ctx);
}

template<typename I>
void TrashWatcher<I>::handle_image_added(const std::string& image_id, const cls::rbd::TrashImageSpec& spec)
{
...
template<typename I> void TrashWatcher<I>::handle_image_removed(const std::string& image_id)
{
    // ignore removals -- the image deleter will ignore -ENOENTs
}

template<typename I> void TrashWatcher<I>::handle_rewatch_complete(int r)
{
    dout(5) << "r=" << r << dendl;
...
template<typename I> void TrashWatcher<I>::create_trash()
{
    dout(20) << dendl;
    {
        std::lock_guard locker{m_lock};
    ...
    auto aio_comp = create_rados_callback<TrashWatcher<I>, &TrashWatcher<I>::handle_create_trash>(this);
    int r = m_io_ctx.aio_operate(RBD_TRASH, aio_comp, &op);
    ceph_assert(r == 0);
    aio_comp->release();
}

template<typename I> void TrashWatcher<I>::handle_create_trash(int r)
{
    dout(20) << "r=" << r << dendl;
    {
        std::lock_guard locker{m_lock};
...
template<typename I> void TrashWatcher<I>::register_watcher()
{
    {
        std::lock_guard locker{m_lock};
        ceph_assert(m_trash_list_in_progress);
    ...
    Context* ctx = create_context_callback<TrashWatcher, &TrashWatcher<I>::handle_register_watcher>(this);
    this->register_watch(ctx);
}

template<typename I> void TrashWatcher<I>::handle_register_watcher(int r)
{
    dout(5) << "r=" << r << dendl;
...
template<typename I> void TrashWatcher<I>::unregister_watcher(Context* on_finish)
{
    dout(5) << dendl;

    m_async_op_tracker.start_op();
    Context* ctx = new LambdaContext([this, on_finish](int r) { handle_unregister_watcher(r, on_finish); });
    this->unregister_watch(ctx);
}

template<typename I> void TrashWatcher<I>::handle_unregister_watcher(int r, Context* on_finish)
{
...
template<typename I> void TrashWatcher<I>::trash_list(bool initial_request)
{
    if (initial_request) {
        m_async_op_tracker.start_op();
        m_last_image_id = "";
    ...
    librados::AioCompletion* aio_comp =
        create_rados_callback<TrashWatcher<I>, &TrashWatcher<I>::handle_trash_list>(this);
    m_out_bl.clear();
...
template<typename I> void TrashWatcher<I>::handle_trash_list(int r)
{
    dout(5) << "r=" << r << dendl;
...
template<typename I> void TrashWatcher<I>::schedule_trash_list(double interval)
{
    std::scoped_lock locker{m_threads->timer_lock, m_lock};
    ...
    dout(5) << dendl;
    m_timer_ctx =
        m_threads->timer->add_event_after(interval, new LambdaContext([this](int r) { process_trash_list(); }));
}

template<typename I> void TrashWatcher<I>::process_trash_list()
{
    dout(5) << dendl;
...
template<typename I> void TrashWatcher<I>::add_image(const std::string& image_id, const cls::rbd::TrashImageSpec& spec)
{
    ...
    auto ctx = new LambdaContext([this, image_id, deferment_end_time](int r) {
        m_trash_listener.handle_trash_image(image_id, deferment_end_time.to_real_time());
        m_async_op_tracker.finish_op();
    });
    m_threads->work_queue->queue(ctx, 0);
}
