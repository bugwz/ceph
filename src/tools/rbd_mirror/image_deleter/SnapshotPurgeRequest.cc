...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::image_deleter::SnapshotPurgeRequest: " << this << " " << __func__ << ": "
...
template<typename I> void SnapshotPurgeRequest<I>::send()
{
    open_image();
}

template<typename I> void SnapshotPurgeRequest<I>::open_image()
{
    dout(10) << dendl;
    ...
    Context* ctx = create_context_callback<SnapshotPurgeRequest<I>, &SnapshotPurgeRequest<I>::handle_open_image>(this);
    m_image_ctx->state->open(librbd::OPEN_FLAG_SKIP_OPEN_PARENT, ctx);
}

template<typename I> void SnapshotPurgeRequest<I>::handle_open_image(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void SnapshotPurgeRequest<I>::acquire_lock()
{
    dout(10) << dendl;
    ...
    m_image_ctx->exclusive_lock->acquire_lock(
        create_context_callback<SnapshotPurgeRequest<I>, &SnapshotPurgeRequest<I>::handle_acquire_lock>(this));
    m_image_ctx->owner_lock.unlock_shared();
}

template<typename I> void SnapshotPurgeRequest<I>::handle_acquire_lock(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void SnapshotPurgeRequest<I>::start_snap_unprotect()
{
    dout(10) << dendl;
...
template<typename I> void SnapshotPurgeRequest<I>::snap_unprotect()
{
    if (m_snaps.empty()) {
        close_image();
        return;
    ...
    m_image_ctx->operations->execute_snap_unprotect(m_snap_namespace, m_snap_name.c_str(), ctx);
}

template<typename I> void SnapshotPurgeRequest<I>::handle_snap_unprotect(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void SnapshotPurgeRequest<I>::snap_remove()
{
    librados::snap_t snap_id = m_snaps.back();
    dout(10) << "snap_id=" << snap_id << ", "
             << "snap_namespace=" << m_snap_namespace << ", "
    ...
    std::shared_lock owner_locker{m_image_ctx->owner_lock};
    m_image_ctx->operations->execute_snap_remove(m_snap_namespace, m_snap_name.c_str(), ctx);
}

template<typename I> void SnapshotPurgeRequest<I>::handle_snap_remove(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void SnapshotPurgeRequest<I>::close_image()
{
    dout(10) << dendl;

    m_image_ctx->state->close(
        create_context_callback<SnapshotPurgeRequest<I>, &SnapshotPurgeRequest<I>::handle_close_image>(this));
}

template<typename I> void SnapshotPurgeRequest<I>::handle_close_image(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void SnapshotPurgeRequest<I>::finish(int r)
{
    if (m_ret_val < 0) {
        r = m_ret_val;
    }
...
template<typename I> Context* SnapshotPurgeRequest<I>::start_lock_op(int* r)
{
    std::shared_lock owner_locker{m_image_ctx->owner_lock};
    if (m_image_ctx->exclusive_lock == nullptr) {
