...
{
public:
    explicit StatusCommand(Mirror* mirror)
        : mirror(mirror)
    {}
...
{
public:
    explicit StartCommand(Mirror* mirror)
        : mirror(mirror)
    {}
...
{
public:
    explicit StopCommand(Mirror* mirror)
        : mirror(mirror)
    {}
...
{
public:
    explicit RestartCommand(Mirror* mirror)
        : mirror(mirror)
    {}
...
{
public:
    explicit FlushCommand(Mirror* mirror)
        : mirror(mirror)
    {}
...
{
public:
    explicit LeaderReleaseCommand(Mirror* mirror)
        : mirror(mirror)
    {}
...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::PriCache: " << this << " " << m_name << " " << __func__ << ": "
...
struct PriCache : public PriorityCache::PriCache
{
    ...
    int64_t request_cache_bytes(PriorityCache::Priority pri, uint64_t total_cache) const override
    {
    ...
    {
        ceph_assert(bytes >= 0);
        ceph_assert(pri == m_base_cache_pri || pri == m_extra_cache_pri || bytes == 0);
    ...
    uint64_t get_bins(PriorityCache::Priority pri) const override { return 0; }

    std::string get_cache_name() const override { return m_name; }
};

}   // anonymous namespace

#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::Mirror: " << this << " " << __func__ << ": "
...
public:
        ...
        : admin_socket(cct->get_admin_socket())
    {
        ...
        r = admin_socket->register_command(command, this, "get status for rbd mirror");
        if (r == 0) {
            commands[command] = new StatusCommand(mirror);
        }

        command = "rbd mirror start";
        r = admin_socket->register_command(command, this, "start rbd mirror");
        if (r == 0) {
            commands[command] = new StartCommand(mirror);
        }

        command = "rbd mirror stop";
        r = admin_socket->register_command(command, this, "stop rbd mirror");
        if (r == 0) {
            commands[command] = new StopCommand(mirror);
        }

        command = "rbd mirror restart";
        r = admin_socket->register_command(command, this, "restart rbd mirror");
        if (r == 0) {
            commands[command] = new RestartCommand(mirror);
        }

        command = "rbd mirror flush";
        r = admin_socket->register_command(command, this, "flush rbd mirror");
        if (r == 0) {
            commands[command] = new FlushCommand(mirror);
        }

        command = "rbd mirror leader release";
        r = admin_socket->register_command(command, this, "release rbd mirror leader");
        if (r == 0) {
...
public:
        ...
        : m_cct(cct)
    {
        ...
        uint64_t base = m_cct->_conf.get_val<Option::size_t>("rbd_mirror_memory_base");
        double fragmentation = m_cct->_conf.get_val<double>("rbd_mirror_memory_expected_fragmentation");
        uint64_t target = m_cct->_conf.get_val<Option::size_t>("rbd_mirror_memory_target");
        uint64_t min = m_cct->_conf.get_val<Option::size_t>("rbd_mirror_memory_cache_min");
        uint64_t max = min;
        ...
        m_cache_manager = std::make_unique<PriorityCache::Manager>(m_cct, min, max, target, false);
    }
    ...
    void register_cache(const std::string& cache_name, uint64_t min_size, uint64_t max_size,
                        journal::CacheRebalanceHandler* handler) override
    {
        ...
        dout(20) << cache_name << " min_size=" << min_size << " max_size=" << max_size << " handler=" << handler
                 << dendl;

        std::lock_guard locker{m_lock};

        auto p = m_caches.insert({cache_name, {cache_name, min_size, max_size, handler}});
    ...
    void run_cache_manager()
    {
        ...
        auto autotune_interval = m_cct->_conf.get_val<double>("rbd_mirror_memory_cache_autotune_interval");
        auto resize_interval = m_cct->_conf.get_val<double>("rbd_mirror_memory_cache_resize_interval");
...
private:
    struct Cache
    {
        ...
        Cache(const std::string name, uint64_t min_size, uint64_t max_size, journal::CacheRebalanceHandler* handler)
            : pri_cache(new PriCache(name, min_size, max_size))
            , handler(handler)
        {}
    };
    ...
    mutable ceph::mutex m_lock = ceph::make_mutex("rbd::mirror::CacheManagerHandler");
...
void Mirror::handle_signal(int signum)
{
    ...
    default: ceph_abort_msgf("unexpected signal %d", signum);
    }
}
...
int Mirror::init()
{
    ...
    m_threads =
        &(m_cct->lookup_or_create_singleton_object<Threads<librbd::ImageCtx>>("rbd_mirror::threads", false, m_local));
    ...
    m_local_cluster_watcher.reset(new ClusterWatcher(m_local, m_lock, m_service_daemon.get()));
    return r;
}
...
void Mirror::run()
{
    ...
    while (!m_stopping) {
        ...
        if (refresh_pools) {
            ...
            next_refresh_pools += m_cct->_conf.get_val<uint64_t>("rbd_mirror_pool_replayers_refresh_interval");
        }
...
void Mirror::update_pool_replayers(const PoolPeers& pool_peers, const std::string& site_name)
{
    dout(20) << "enter" << dendl;
    ...
    for (auto it = m_pool_replayers.begin(); it != m_pool_replayers.end();) {
        ...
        if (pool_peer_it == pool_peers.end() || pool_peer_it->second.find(peer) == pool_peer_it->second.end()) {
    ...
    for (auto& kv : pool_peers) {
        for (auto& peer : kv.second) {
            ...
            if (pool_replayers_it != m_pool_replayers.end()) {
                ...
                if (!m_site_name.empty() && !site_name.empty() && m_site_name != site_name) {
                ...
                unique_ptr<PoolReplayer<>> pool_replayer(new PoolReplayer<>(m_threads,
                                                                            m_service_daemon.get(),
                                                                            m_cache_manager_handler.get(),
                                                                            m_pool_meta_cache.get(),
                                                                            kv.first,
                                                                            peer,
                                                                            m_args));
