...
template<typename ImageCtxT = librbd::ImageCtx> class ImageMap
{
public:
    static ImageMap* create(librados::IoCtx& ioctx, Threads<ImageCtxT>* threads, const std::string& instance_id,
    ...
    void update_images(const std::string& peer_uuid, std::set<std::string>&& added_global_image_ids,
...
private:
    ...
    ImageMap(librados::IoCtx& ioctx, Threads<ImageCtxT>* threads, const std::string& instance_id,
             image_map::Listener& listener);
    ...
    struct Update
    {
        ...
        Update(const std::string& global_image_id, const std::string& instance_id, utime_t mapped_time)
    ...
    void start_async_op() { m_async_op_tracker.start_op(); }
    void finish_async_op() { m_async_op_tracker.finish_op(); }
    void wait_for_async_ops(Context* on_finish) { m_async_op_tracker.wait_for_ops(on_finish); }
    ...
    void handle_update_request(const Updates& updates, const std::set<std::string>& remove_global_image_ids, int r);
    ...
    void update_image_mapping(Updates&& map_updates, std::set<std::string>&& map_removals);
