...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::image_map::LoadRequest: " << this << " " << __func__
...
template<typename I>
LoadRequest<I>::LoadRequest(librados::IoCtx& ioctx, std::map<std::string, cls::rbd::MirrorImageMap>* image_mapping,
                            Context* on_finish)
    : m_ioctx(ioctx)
...
template<typename I> void LoadRequest<I>::image_map_list()
{
    dout(20) << dendl;
    ...
    librados::AioCompletion* aio_comp = create_rados_callback<LoadRequest, &LoadRequest::handle_image_map_list>(this);
...
template<typename I> void LoadRequest<I>::handle_image_map_list(int r)
{
    dout(20) << ": r=" << r << dendl;
...
template<typename I> void LoadRequest<I>::mirror_image_list()
{
    dout(20) << dendl;
    ...
    librados::AioCompletion* aio_comp =
        create_rados_callback<LoadRequest<I>, &LoadRequest<I>::handle_mirror_image_list>(this);
    int r = m_ioctx.aio_operate(RBD_MIRRORING, aio_comp, &op, &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
}

template<typename I> void LoadRequest<I>::handle_mirror_image_list(int r)
{
    dout(20) << ": r=" << r << dendl;
...
template<typename I> void LoadRequest<I>::cleanup_image_map()
{
    dout(20) << dendl;
    ...
    auto ctx = create_context_callback<LoadRequest<I>, &LoadRequest<I>::finish>(this);
    image_map::UpdateRequest<I>* req = image_map::UpdateRequest<I>::create(m_ioctx, {}, std::move(map_removals), ctx);
    req->send();
}
