...
protected:
    ...
    virtual std::string do_map(const InstanceToImageMap& map, const std::string& global_image_id) = 0;

    // shuffle images when instances are added/removed
    virtual void do_shuffle_add_instances(const InstanceToImageMap& map, size_t image_count,
...
private:
    ...
    void remove_instances(const ceph::shared_mutex& lock, const InstanceIds& instance_ids,
                          GlobalImageIds* global_image_ids);

    bool set_state(ImageState* image_state, StateTransition::State state, bool ignore_current_state);

    void execute_policy_action(const std::string& global_image_id, ImageState* image_state,
    ...
    bool is_state_scheduled(const ImageState& image_state, StateTransition::State state) const;
};
