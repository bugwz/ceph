...
template<typename ImageCtxT = librbd::ImageCtx> class UpdateRequest
{
public:
    ...
    {
        return new UpdateRequest(ioctx, std::move(update_mapping), std::move(remove_global_image_ids), on_finish);
    }
...
private:
     ...
     * @endverbatim
     */
    UpdateRequest(librados::IoCtx& ioctx, std::map<std::string, cls::rbd::MirrorImageMap>&& update_mapping,
                  std::set<std::string>&& remove_global_image_ids, Context* on_finish);
