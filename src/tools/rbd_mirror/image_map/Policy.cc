...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::image_map::Policy: " << this << " " << __func__ << ": "
...
bool is_instance_action(ActionType action_type)
{
    ...
    case ACTION_TYPE_RELEASE: return true;
    case ACTION_TYPE_NONE:
    case ACTION_TYPE_MAP_UPDATE:
    case ACTION_TYPE_MAP_REMOVE: break;
    }
    return false;
}
...
Policy::Policy(librados::IoCtx& ioctx)
    : m_ioctx(ioctx)
    , m_map_lock(ceph::make_shared_mutex(unique_lock_name("rbd::mirror::image_map::Policy::m_map_lock", this)))
{
...
void Policy::init(const std::map<std::string, cls::rbd::MirrorImageMap>& image_mapping)
{
    ...
    for (auto& it : image_mapping) {
        ...
        auto image_state_result =
            m_image_states.emplace(it.first, ImageState{it.second.instance_id, it.second.mapped_time});
        ...
        auto start_action = set_state(&image_state, StateTransition::STATE_INITIALIZING, false);
...
bool Policy::add_image(const std::string& global_image_id)
{
    ...
    std::unique_lock map_lock{m_map_lock};
    auto image_state_result = m_image_states.emplace(global_image_id, ImageState{});
    auto& image_state = image_state_result.first->second;
...
void Policy::add_instances(const InstanceIds& instance_ids, GlobalImageIds* global_image_ids)
{
    ...
    if (m_initial_update) {
        ...
        std::set<std::string> alive_instances(instance_ids.begin(), instance_ids.end());
...
void Policy::remove_instances(const ceph::shared_mutex& lock, const InstanceIds& instance_ids,
                              GlobalImageIds* global_image_ids)
{
    ...
    for (auto& instance_id : instance_ids) {
        ...
        for (auto& global_image_id : instance_global_image_ids) {
            ...
            auto& image_state = it->second;
            if (is_state_scheduled(image_state, StateTransition::STATE_DISSOCIATING)) {
...
ActionType Policy::start_action(const std::string& global_image_id)
{
    ...
    if (transition.start_policy_action) {
        execute_policy_action(global_image_id, &image_state, *transition.start_policy_action);
...
bool Policy::finish_action(const std::string& global_image_id, int r)
{
    ...
    if (r < 0 &&
        (!is_instance_action(image_state.transition.action_type) || image_state.instance_id == UNMAPPED_INSTANCE_ID ||
         m_dead_instances.find(image_state.instance_id) == m_dead_instances.end())) {
        return true;
    }
...
void Policy::execute_policy_action(const std::string& global_image_id, ImageState* image_state,
                                   StateTransition::PolicyAction policy_action)
{
    ...
    case StateTransition::POLICY_ACTION_MAP: map(global_image_id, image_state); break;
    case StateTransition::POLICY_ACTION_UNMAP: unmap(global_image_id, image_state); break;
...
void Policy::unmap(const std::string& global_image_id, ImageState* image_state)
{
    ceph_assert(ceph_mutex_is_wlocked(m_map_lock));
...
bool Policy::is_image_shuffling(const std::string& global_image_id)
{
    ...
    auto result = is_state_scheduled(image_state, StateTransition::STATE_SHUFFLING);
...
bool Policy::can_shuffle_image(const std::string& global_image_id)
{
    ...
    int migration_throttle = cct->_conf.get_val<uint64_t>("rbd_mirror_image_policy_migration_throttle");
...
bool Policy::set_state(ImageState* image_state, StateTransition::State state, bool ignore_current_state)
{
...
bool Policy::is_state_scheduled(const ImageState& image_state, StateTransition::State state) const
{
    return (image_state.state == state || (image_state.next_state && *image_state.next_state == state));
}
