...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::image_map::SimplePolicy: " << this << " " << __func__ << ": "
namespace rbd {
namespace mirror {
namespace image_map {
...
size_t SimplePolicy::calc_images_per_instance(const InstanceToImageMap& map, size_t image_count)
{
...
void SimplePolicy::do_shuffle_add_instances(const InstanceToImageMap& map, size_t image_count,
...
std::string SimplePolicy::do_map(const InstanceToImageMap& map, const std::string& global_image_id)
{
    auto min_it = map.end();
