...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::image_map::UpdateRequest: " << this << " " << __func__
...
template<typename I> void UpdateRequest<I>::update_image_map()
{
    dout(20) << dendl;
    ...
    librados::AioCompletion* aio_comp =
        create_rados_callback<UpdateRequest, &UpdateRequest::handle_update_image_map>(this);
    int r = m_ioctx.aio_operate(RBD_MIRROR_LEADER, aio_comp, &op);
    ceph_assert(r == 0);
    aio_comp->release();
}

template<typename I> void UpdateRequest<I>::handle_update_image_map(int r)
{
    dout(20) << ": r=" << r << dendl;
...
template<typename I> void UpdateRequest<I>::finish(int r)
{
    dout(20) << ": r=" << r << dendl;
