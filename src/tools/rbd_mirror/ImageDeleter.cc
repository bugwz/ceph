...
template<typename I> class StatusCommand : public ImageDeleterAdminSocketCommand
{
public:
    explicit StatusCommand(ImageDeleter<I>* image_del)
        : image_del(image_del)
    {}
...
template<typename I> class ImageDeleterAdminSocketHook : public AdminSocketHook
{
public:
    ImageDeleterAdminSocketHook(CephContext* cct, const std::string& pool_name, ImageDeleter<I>* image_del)
        : admin_socket(cct->get_admin_socket())
    {
        ...
        r = admin_socket->register_command(command, this, "get status for image deleter");
        if (r == 0) {
...
private:
    typedef std::map<std::string, ImageDeleterAdminSocketCommand*, std::less<>> Commands;
    AdminSocket* admin_socket;
    Commands commands;
};

template<typename I>
ImageDeleter<I>::ImageDeleter(librados::IoCtx& local_io_ctx, Threads<librbd::ImageCtx>* threads,
...
#undef dout_prefix
#define dout_prefix                         \
    *_dout << "rbd::mirror::ImageDeleter: " \
           << " " << __func__ << ": "

template<typename I>
void ImageDeleter<I>::trash_move(librados::IoCtx& local_io_ctx, const std::string& global_image_id, bool resync,
...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::ImageDeleter: " << this << " " << __func__ << ": "

template<typename I> void ImageDeleter<I>::init(Context* on_finish)
{
    dout(10) << dendl;

    m_asok_hook = new ImageDeleterAdminSocketHook<I>(g_ceph_context, m_local_io_ctx.get_pool_name(), this);

    m_trash_watcher = image_deleter::TrashWatcher<I>::create(m_local_io_ctx, m_threads, m_trash_listener);
    m_trash_watcher->init(on_finish);
}

template<typename I> void ImageDeleter<I>::shut_down(Context* on_finish)
{
    dout(10) << dendl;
    ...
    m_image_deletion_throttler->drain(m_local_io_ctx.get_namespace(), -ESTALE);

    shut_down_trash_watcher(on_finish);
}

template<typename I> void ImageDeleter<I>::shut_down_trash_watcher(Context* on_finish)
{
    dout(10) << dendl;
...
template<typename I> void ImageDeleter<I>::wait_for_ops(Context* on_finish)
{
    {
    ...
    auto ctx = new LambdaContext([this, on_finish](int) { cancel_all_deletions(on_finish); });
    m_async_op_tracker.wait_for_ops(ctx);
}

template<typename I> void ImageDeleter<I>::cancel_all_deletions(Context* on_finish)
{
    m_image_deletion_throttler->drain(m_local_io_ctx.get_namespace(), -ECANCELED);
    {
...
template<typename I>
void ImageDeleter<I>::wait_for_deletion(const std::string& image_id, bool scheduled_only, Context* on_finish)
{
    dout(5) << "image_id=" << image_id << dendl;

    on_finish = new LambdaContext([this, on_finish](int r) { m_threads->work_queue->queue(on_finish, r); });
...
template<typename I> void ImageDeleter<I>::complete_active_delete(DeleteInfoRef* delete_info, int r)
{
...
template<typename I>
void ImageDeleter<I>::enqueue_failed_delete(DeleteInfoRef* delete_info, int error_code, double retry_delay)
{
    ...
    delete_info_ref->retry_time = (clock_t::now() + ceph::make_timespan(retry_delay));
...
template<typename I>
typename ImageDeleter<I>::DeleteInfoRef ImageDeleter<I>::find_delete_info(const std::string& image_id)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    DeleteQueue delete_queues[] = {m_in_flight_delete_queue, m_retry_delete_queue, m_delete_queue};

    DeleteInfo delete_info{image_id};
    for (auto& queue : delete_queues) {
        auto it = std::find_if(
            queue.begin(), queue.end(), [&delete_info](const DeleteInfoRef& ref) { return delete_info == *ref; });
        if (it != queue.end()) {
            return *it;
        }
    }
...
template<typename I> void ImageDeleter<I>::print_status(Formatter* f)
{
    dout(20) << dendl;
...
template<typename I> vector<string> ImageDeleter<I>::get_delete_queue_items()
{
    vector<string> items;
...
template<typename I> vector<pair<string, int>> ImageDeleter<I>::get_failed_queue_items()
{
    vector<pair<string, int>> items;

    std::lock_guard l{m_lock};
    for (const auto& del_info : m_retry_delete_queue) {
        items.push_back(make_pair(del_info->image_id, del_info->error_code));
    }

    return items;
}

template<typename I> void ImageDeleter<I>::remove_images()
{
    dout(10) << dendl;
    ...
    while (m_running && !m_delete_queue.empty()) {
        ...
        auto on_start =
            create_async_context_callback(m_threads->work_queue, new LambdaContext([this, delete_info](int r) {
                                              if (r < 0) {
                                                  notify_on_delete(delete_info->image_id, r);
                                                  return;
                                              }
        ...
        m_image_deletion_throttler->start_op(m_local_io_ctx.get_namespace(), delete_info->image_id, on_start);
    }
}

template<typename I> void ImageDeleter<I>::remove_image(DeleteInfoRef delete_info)
{
    ...
    auto req = image_deleter::TrashRemoveRequest<I>::create(
        m_local_io_ctx, delete_info->image_id, &delete_info->error_result, m_threads->work_queue, ctx);
    req->send();
}

template<typename I> void ImageDeleter<I>::handle_remove_image(DeleteInfoRef delete_info, int r)
{
    dout(10) << "info=" << *delete_info << ", r=" << r << dendl;

    m_image_deletion_throttler->finish_op(m_local_io_ctx.get_namespace(), delete_info->image_id);
    {
        std::lock_guard locker{m_lock};
        ceph_assert(ceph_mutex_is_locked(m_lock));
        auto it = std::find(m_in_flight_delete_queue.begin(), m_in_flight_delete_queue.end(), delete_info);
    ...
    if (r < 0) {
        ...
        else if (delete_info->error_result == image_deleter::ERROR_RESULT_RETRY_IMMEDIATELY) {
            enqueue_failed_delete(&delete_info, r, m_busy_interval);
        }
        else {
            auto cct = reinterpret_cast<CephContext*>(m_local_io_ctx.cct());
            double failed_interval = cct->_conf.get_val<double>("rbd_mirror_delete_retry_interval");
...
template<typename I> void ImageDeleter<I>::schedule_retry_timer()
{
    ...
    m_timer_ctx = new LambdaContext([this](int r) { handle_retry_timer(); });
    m_threads->timer->add_event_at(delete_info->retry_time, m_timer_ctx);
}

template<typename I> void ImageDeleter<I>::cancel_retry_timer()
{
    dout(10) << dendl;
    ceph_assert(ceph_mutex_is_locked(m_threads->timer_lock));
...
template<typename I> void ImageDeleter<I>::handle_retry_timer()
{
    dout(10) << dendl;
    ceph_assert(ceph_mutex_is_locked(m_threads->timer_lock));
    std::lock_guard locker{m_lock};
...
template<typename I> void ImageDeleter<I>::notify_on_delete(const std::string& image_id, int r)
{
...
template<typename I> void ImageDeleter<I>::DeleteInfo::print_status(Formatter* f, bool print_failure_info)
{
