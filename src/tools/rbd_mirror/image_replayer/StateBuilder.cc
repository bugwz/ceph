...
template<typename I> bool StateBuilder<I>::is_local_primary() const
{
...
template<typename I> bool StateBuilder<I>::is_remote_primary() const
{
...
template<typename I> bool StateBuilder<I>::is_linked() const
{
...
template<typename I> void StateBuilder<I>::close_local_image(Context* on_finish)
{
    ...
    dout(10) << dendl;
    auto ctx = new LambdaContext([this, on_finish](int r) { handle_close_local_image(r, on_finish); });
    auto request = image_replayer::CloseImageRequest<I>::create(&local_image_ctx, ctx);
    request->send();
}

template<typename I> void StateBuilder<I>::handle_close_local_image(int r, Context* on_finish)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void StateBuilder<I>::close_remote_image(Context* on_finish)
{
    ...
    dout(10) << dendl;
    auto ctx = new LambdaContext([this, on_finish](int r) { handle_close_remote_image(r, on_finish); });
    auto request = image_replayer::CloseImageRequest<I>::create(&remote_image_ctx, ctx);
    request->send();
}

template<typename I> void StateBuilder<I>::handle_close_remote_image(int r, Context* on_finish)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void StateBuilder<I>::destroy_sync_point_handler()
{
