...
#define dout_prefix                           \
    *_dout << "rbd::mirror::image_replayer::" \
           << "GetMirrorImageIdRequest: " << this << " " << __func__ << ": "
...
template<typename I> void GetMirrorImageIdRequest<I>::send()
{
    dout(20) << dendl;
    get_image_id();
}

template<typename I> void GetMirrorImageIdRequest<I>::get_image_id()
{
    dout(20) << dendl;
    ...
    librados::AioCompletion* aio_comp =
        create_rados_callback<GetMirrorImageIdRequest<I>, &GetMirrorImageIdRequest<I>::handle_get_image_id>(this);
    int r = m_io_ctx.aio_operate(RBD_MIRRORING, aio_comp, &op, &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
}

template<typename I> void GetMirrorImageIdRequest<I>::handle_get_image_id(int r)
{
    if (r == 0) {
...
template<typename I> void GetMirrorImageIdRequest<I>::finish(int r)
{
    dout(20) << "r=" << r << dendl;
