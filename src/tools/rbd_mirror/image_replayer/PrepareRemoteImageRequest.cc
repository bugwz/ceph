...
#define dout_prefix                           \
    *_dout << "rbd::mirror::image_replayer::" \
           << "PrepareRemoteImageRequest: " << this << " " << __func__ << ": "
...
template<typename I> void PrepareRemoteImageRequest<I>::send()
{
...
template<typename I> void PrepareRemoteImageRequest<I>::get_remote_image_id()
{
    dout(10) << dendl;

    Context* ctx = create_context_callback<PrepareRemoteImageRequest<I>,
                                           &PrepareRemoteImageRequest<I>::handle_get_remote_image_id>(this);
    auto req = GetMirrorImageIdRequest<I>::create(m_remote_io_ctx, m_global_image_id, &m_remote_image_id, ctx);
    req->send();
}

template<typename I> void PrepareRemoteImageRequest<I>::handle_get_remote_image_id(int r)
{
    dout(10) << "r=" << r << ", "
...
template<typename I> void PrepareRemoteImageRequest<I>::get_mirror_info()
{
    dout(10) << dendl;

    auto ctx =
        create_context_callback<PrepareRemoteImageRequest<I>, &PrepareRemoteImageRequest<I>::handle_get_mirror_info>(
            this);
    auto req = librbd::mirror::GetInfoRequest<I>::create(m_remote_io_ctx,
...
template<typename I> void PrepareRemoteImageRequest<I>::handle_get_mirror_info(int r)
{
    dout(10) << "r=" << r << dendl;
    ...
    if (r == -ENOENT) {
        ...
        derr << "failed to retrieve mirror image details for image " << m_global_image_id << ": " << cpp_strerror(r)
             << dendl;
        finish(r);
        return;
    }
    ...
    if (state_builder != nullptr && state_builder->get_mirror_image_mode() != m_mirror_image.mode) {
    ...
    case cls::rbd::MIRROR_IMAGE_MODE_JOURNAL: get_client(); break;
...
template<typename I> void PrepareRemoteImageRequest<I>::get_client()
{
    dout(10) << dendl;

    auto cct = static_cast<CephContext*>(m_local_io_ctx.cct());
    ::journal::Settings journal_settings;
    journal_settings.commit_interval = cct->_conf.get_val<double>("rbd_mirror_journal_commit_age");
    ...
    m_remote_journaler = new Journaler(context_wq,
                                       m_threads->timer,
    ...
    Context* ctx = create_async_context_callback(
        m_threads->work_queue,
        create_context_callback<PrepareRemoteImageRequest<I>, &PrepareRemoteImageRequest<I>::handle_get_client>(this));
    m_remote_journaler->get_client(m_local_mirror_uuid, &m_client, ctx);
}

template<typename I> void PrepareRemoteImageRequest<I>::handle_get_client(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void PrepareRemoteImageRequest<I>::register_client()
{
    dout(10) << dendl;

    auto state_builder = *m_state_builder;
    librbd::journal::MirrorPeerClientMeta client_meta{(state_builder == nullptr ? "" : state_builder->local_image_id)};
    ...
    Context* ctx = create_async_context_callback(
        m_threads->work_queue,
        create_context_callback<PrepareRemoteImageRequest<I>, &PrepareRemoteImageRequest<I>::handle_register_client>(
            this));
    m_remote_journaler->register_client(client_data_bl, ctx);
}

template<typename I> void PrepareRemoteImageRequest<I>::handle_register_client(int r)
{
    dout(10) << "r=" << r << dendl;
    ...
    librbd::journal::MirrorPeerClientMeta client_meta{(state_builder == nullptr ? "" : state_builder->local_image_id)};
    client_meta.state = librbd::journal::MIRROR_PEER_STATE_REPLAYING;
    finalize_journal_state_builder(cls::journal::CLIENT_STATE_CONNECTED, client_meta);
    finish(0);
}

template<typename I>
void PrepareRemoteImageRequest<I>::finalize_journal_state_builder(cls::journal::ClientState client_state,
...
template<typename I> void PrepareRemoteImageRequest<I>::finalize_snapshot_state_builder()
{
...
template<typename I> void PrepareRemoteImageRequest<I>::finish(int r)
{
    dout(10) << "r=" << r << dendl;
