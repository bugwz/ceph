...
#define dout_prefix                           \
    *_dout << "rbd::mirror::image_replayer::" \
           << "PrepareLocalImageRequest: " << this << " " << __func__ << ": "
...
template<typename I> void PrepareLocalImageRequest<I>::send()
{
    dout(10) << dendl;
    get_local_image_id();
}

template<typename I> void PrepareLocalImageRequest<I>::get_local_image_id()
{
    dout(10) << dendl;

    Context* ctx =
        create_context_callback<PrepareLocalImageRequest<I>, &PrepareLocalImageRequest<I>::handle_get_local_image_id>(
            this);
    auto req = GetMirrorImageIdRequest<I>::create(m_io_ctx, m_global_image_id, &m_local_image_id, ctx);
    req->send();
}

template<typename I> void PrepareLocalImageRequest<I>::handle_get_local_image_id(int r)
{
    dout(10) << "r=" << r << ", "
...
template<typename I> void PrepareLocalImageRequest<I>::get_local_image_name()
{
    dout(10) << dendl;
    ...
    librados::AioCompletion* aio_comp =
        create_rados_callback<PrepareLocalImageRequest<I>, &PrepareLocalImageRequest<I>::handle_get_local_image_name>(
            this);
...
template<typename I> void PrepareLocalImageRequest<I>::handle_get_local_image_name(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void PrepareLocalImageRequest<I>::get_mirror_info()
{
    dout(10) << dendl;

    auto ctx =
        create_context_callback<PrepareLocalImageRequest<I>, &PrepareLocalImageRequest<I>::handle_get_mirror_info>(
            this);
    auto req = librbd::mirror::GetInfoRequest<I>::create(
        m_io_ctx, m_work_queue, m_local_image_id, &m_mirror_image, &m_promotion_state, &m_primary_mirror_uuid, ctx);
    req->send();
}

template<typename I> void PrepareLocalImageRequest<I>::handle_get_mirror_info(int r)
{
    dout(10) << ": r=" << r << dendl;
    ...
    if (m_mirror_image.state == cls::rbd::MIRROR_IMAGE_STATE_CREATING) {
        dout(5) << "local image is still in creating state, issuing a removal" << dendl;
        move_to_trash();
        return;
    }
...
template<typename I> void PrepareLocalImageRequest<I>::move_to_trash()
{
    dout(10) << dendl;

    Context* ctx =
        create_context_callback<PrepareLocalImageRequest<I>, &PrepareLocalImageRequest<I>::handle_move_to_trash>(this);
    ImageDeleter<I>::trash_move(m_io_ctx, m_global_image_id, false, m_work_queue, ctx);
}

template<typename I> void PrepareLocalImageRequest<I>::handle_move_to_trash(int r)
{
    dout(10) << ": r=" << r << dendl;
...
template<typename I> void PrepareLocalImageRequest<I>::finish(int r)
{
    dout(10) << "r=" << r << dendl;
