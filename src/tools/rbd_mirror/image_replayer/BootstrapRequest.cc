...
#undef dout_prefix
#define dout_prefix                           \
    *_dout << "rbd::mirror::image_replayer::" \
           << "BootstrapRequest: " << this << " " << __func__ << ": "
...
template<typename I>
BootstrapRequest<I>::BootstrapRequest(Threads<I>* threads, librados::IoCtx& local_io_ctx,
                                      librados::IoCtx& remote_io_ctx, InstanceWatcher<I>* instance_watcher,
                                      const std::string& global_image_id, const std::string& local_mirror_uuid,
                                      const RemotePoolMeta& remote_pool_meta,
                                      ::journal::CacheManagerHandler* cache_manager_handler,
                                      PoolMetaCache* pool_meta_cache, ProgressContext* progress_ctx,
                                      StateBuilder<I>** state_builder, bool* do_resync, Context* on_finish)
...
template<typename I> bool BootstrapRequest<I>::is_syncing() const
{
    std::lock_guard locker{m_lock};
    return (m_image_sync != nullptr);
}

template<typename I> void BootstrapRequest<I>::send()
{
    *m_do_resync = false;
...
template<typename I> void BootstrapRequest<I>::cancel()
{
    dout(10) << dendl;
...
template<typename I> std::string BootstrapRequest<I>::get_local_image_name() const
{
    std::unique_lock locker{m_lock};
    return m_local_image_name;
}

template<typename I> void BootstrapRequest<I>::prepare_local_image()
{
    dout(10) << dendl;
    ...
    auto ctx = create_context_callback<BootstrapRequest, &BootstrapRequest<I>::handle_prepare_local_image>(this);
    auto req = image_replayer::PrepareLocalImageRequest<I>::create(
        m_local_io_ctx, m_global_image_id, &m_prepare_local_image_name, m_state_builder, m_threads->work_queue, ctx);
    req->send();
}

template<typename I> void BootstrapRequest<I>::handle_prepare_local_image(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void BootstrapRequest<I>::prepare_remote_image()
{
    ...
    Context* ctx = create_context_callback<BootstrapRequest, &BootstrapRequest<I>::handle_prepare_remote_image>(this);
    auto req = image_replayer::PrepareRemoteImageRequest<I>::create(m_threads,
                                                                    m_local_io_ctx,
                                                                    m_remote_io_ctx,
                                                                    m_global_image_id,
                                                                    m_local_mirror_uuid,
...
template<typename I> void BootstrapRequest<I>::handle_prepare_remote_image(int r)
{
    dout(10) << "r=" << r << dendl;

    auto state_builder = *m_state_builder;
    ceph_assert(state_builder == nullptr || !state_builder->remote_mirror_uuid.empty());
    ...
    if (state_builder != nullptr && state_builder->is_local_primary()) {
        ...
        *_dout << dendl;
        ...
        if (state_builder != nullptr && state_builder->remote_image_id.empty() &&
        ...
        derr << "error preparing remote image for replay: " << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
    ...
    if (!state_builder->is_remote_primary()) {
        ...
        if (state_builder->local_image_id.empty()) {
            dout(10) << "local image does not exist and remote image is not primary" << dendl;
            finish(-EREMOTEIO);
            return;
        }
        else if (!state_builder->is_linked()) {
            dout(10) << "local image is unlinked and remote image is not primary" << dendl;
            finish(-EREMOTEIO);
            return;
        }
...
template<typename I> void BootstrapRequest<I>::open_remote_image()
{
    ceph_assert(*m_state_builder != nullptr);
    ...
    auto ctx = create_context_callback<BootstrapRequest<I>, &BootstrapRequest<I>::handle_open_remote_image>(this);
    ceph_assert(*m_state_builder != nullptr);
    OpenImageRequest<I>* request = OpenImageRequest<I>::create(
        m_remote_io_ctx, &(*m_state_builder)->remote_image_ctx, remote_image_id, false, ctx);
    request->send();
}

template<typename I> void BootstrapRequest<I>::handle_open_remote_image(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void BootstrapRequest<I>::open_local_image()
{
    ceph_assert(*m_state_builder != nullptr);
    ...
    Context* ctx = create_context_callback<BootstrapRequest<I>, &BootstrapRequest<I>::handle_open_local_image>(this);
    OpenLocalImageRequest<I>* request = OpenLocalImageRequest<I>::create(
        m_local_io_ctx, &(*m_state_builder)->local_image_ctx, local_image_id, m_threads->work_queue, ctx);
    request->send();
}

template<typename I> void BootstrapRequest<I>::handle_open_local_image(int r)
{
    dout(15) << "r=" << r << dendl;

    ceph_assert(*m_state_builder != nullptr);
    auto local_image_ctx = (*m_state_builder)->local_image_ctx;
    ceph_assert((r >= 0 && local_image_ctx != nullptr) || (r < 0 && local_image_ctx == nullptr));
...
template<typename I> void BootstrapRequest<I>::prepare_replay()
{
    dout(10) << dendl;
    update_progress("PREPARE_REPLAY");

    ceph_assert(*m_state_builder != nullptr);
    auto ctx = create_context_callback<BootstrapRequest<I>, &BootstrapRequest<I>::handle_prepare_replay>(this);
    auto request =
        (*m_state_builder)
            ->create_prepare_replay_request(m_local_mirror_uuid, m_progress_ctx, m_do_resync, &m_syncing, ctx);
    request->send();
}

template<typename I> void BootstrapRequest<I>::handle_prepare_replay(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void BootstrapRequest<I>::create_local_image()
{
    dout(10) << dendl;
    update_progress("CREATE_LOCAL_IMAGE");

    ceph_assert(*m_state_builder != nullptr);
    auto ctx = create_context_callback<BootstrapRequest<I>, &BootstrapRequest<I>::handle_create_local_image>(this);
    auto request = (*m_state_builder)
                       ->create_local_image_request(
                           m_threads, m_local_io_ctx, m_global_image_id, m_pool_meta_cache, m_progress_ctx, ctx);
    request->send();
}

template<typename I> void BootstrapRequest<I>::handle_create_local_image(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void BootstrapRequest<I>::image_sync()
{
    std::unique_lock locker{m_lock};
    if (m_canceled) {
        locker.unlock();
    ...
    Context* ctx = create_context_callback<BootstrapRequest<I>, &BootstrapRequest<I>::handle_image_sync>(this);
    m_image_sync = ImageSync<I>::create(m_threads,
                                        state_builder->local_image_ctx,
                                        state_builder->remote_image_ctx,
                                        m_local_mirror_uuid,
                                        sync_point_handler,
                                        m_instance_watcher,
                                        m_progress_ctx,
...
template<typename I> void BootstrapRequest<I>::handle_image_sync(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void BootstrapRequest<I>::close_remote_image()
{
    ...
    auto ctx = create_context_callback<BootstrapRequest<I>, &BootstrapRequest<I>::handle_close_remote_image>(this);
    ceph_assert(*m_state_builder != nullptr);
    (*m_state_builder)->close_remote_image(ctx);
}

template<typename I> void BootstrapRequest<I>::handle_close_remote_image(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void BootstrapRequest<I>::update_progress(const std::string& description)
{
