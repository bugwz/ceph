...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::image_replayer::CreateImageRequest: " << this << " " << __func__ << ": "
...
template<typename I>
CreateImageRequest<I>::CreateImageRequest(Threads<I>* threads, librados::IoCtx& local_io_ctx,
                                          ...
                                          I* remote_image_ctx, PoolMetaCache* pool_meta_cache,
                                          cls::rbd::MirrorImageMode mirror_image_mode, Context* on_finish)
    : m_threads(threads)
...
template<typename I> void CreateImageRequest<I>::send()
{
...
template<typename I> void CreateImageRequest<I>::create_image()
{
    dout(10) << dendl;
    ...
    auto& config{reinterpret_cast<CephContext*>(m_local_io_ctx.cct())->_conf};

    librbd::ImageOptions image_options;
    populate_image_options(&image_options);

    auto req = librbd::image::CreateRequest<I>::create(config,
                                                       m_local_io_ctx,
                                                       m_local_image_name,
                                                       m_local_image_id,
                                                       m_remote_image_ctx->size,
                                                       image_options,
                                                       0U,
                                                       m_mirror_image_mode,
                                                       m_global_image_id,
                                                       m_remote_mirror_uuid,
                                                       m_remote_image_ctx->op_work_queue,
                                                       ctx);
    req->send();
}

template<typename I> void CreateImageRequest<I>::handle_create_image(int r)
{
    dout(10) << "r=" << r << dendl;
    if (r == -EBADF) {
...
template<typename I> void CreateImageRequest<I>::get_parent_global_image_id()
{
    dout(10) << dendl;
    ...
    librados::AioCompletion* aio_comp =
        create_rados_callback<CreateImageRequest<I>, &CreateImageRequest<I>::handle_get_parent_global_image_id>(this);
    m_out_bl.clear();
    int r = m_remote_parent_io_ctx.aio_operate(RBD_MIRRORING, aio_comp, &op, &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
}

template<typename I> void CreateImageRequest<I>::handle_get_parent_global_image_id(int r)
{
    dout(10) << "r=" << r << dendl;
    if (r == 0) {
    ...
    if (r == -ENOENT) {
        ...
        derr << "failed to retrieve global image id for parent image " << m_remote_parent_spec.image_id << ": "
...
template<typename I> void CreateImageRequest<I>::get_local_parent_image_id()
{
    dout(10) << dendl;

    librados::ObjectReadOperation op;
    librbd::cls_client::mirror_image_get_image_id_start(&op, m_parent_global_image_id);

    librados::AioCompletion* aio_comp =
        create_rados_callback<CreateImageRequest<I>, &CreateImageRequest<I>::handle_get_local_parent_image_id>(this);
    m_out_bl.clear();
    int r = m_local_parent_io_ctx.aio_operate(RBD_MIRRORING, aio_comp, &op, &m_out_bl);
    ceph_assert(r == 0);
    aio_comp->release();
}

template<typename I> void CreateImageRequest<I>::handle_get_local_parent_image_id(int r)
{
    dout(10) << "r=" << r << dendl;

    if (r == 0) {
        auto iter = m_out_bl.cbegin();
        r = librbd::cls_client::mirror_image_get_image_id_finish(&iter, &m_local_parent_spec.image_id);
    }
    ...
    if (r == -ENOENT) {
        ...
        derr << "failed to retrieve local image id for parent image " << m_parent_global_image_id << ": "
             << cpp_strerror(r) << dendl;
        finish(r);
        return;
    }
...
template<typename I> void CreateImageRequest<I>::open_remote_parent_image()
{
    dout(10) << dendl;

    Context* ctx =
        create_context_callback<CreateImageRequest<I>, &CreateImageRequest<I>::handle_open_remote_parent_image>(this);
    OpenImageRequest<I>* request = OpenImageRequest<I>::create(
        m_remote_parent_io_ctx, &m_remote_parent_image_ctx, m_remote_parent_spec.image_id, true, ctx);
    request->send();
}

template<typename I> void CreateImageRequest<I>::handle_open_remote_parent_image(int r)
{
    dout(10) << "r=" << r << dendl;
    if (r < 0) {
        derr << "failed to open remote parent image " << m_parent_pool_name << "/" << m_remote_parent_spec.image_id
             << dendl;
        finish(r);
        return;
    }
...
template<typename I> void CreateImageRequest<I>::clone_image()
{
    dout(10) << dendl;

    LocalPoolMeta local_parent_pool_meta;
    int r = m_pool_meta_cache->get_local_pool_meta(m_local_parent_io_ctx.get_id(), &local_parent_pool_meta);
    if (r < 0) {
    ...
    {
        auto snap_prefix = image_sync::util::get_snapshot_name_prefix(local_parent_pool_meta.mirror_uuid);
    ...
    librbd::image::CloneRequest<I>* req = librbd::image::CloneRequest<I>::create(config,
                                                                                 m_local_parent_io_ctx,
                                                                                 m_local_parent_spec.image_id,
                                                                                 snap_name,
                                                                                 snap_namespace,
                                                                                 CEPH_NOSNAP,
                                                                                 m_local_io_ctx,
...
template<typename I> void CreateImageRequest<I>::handle_clone_image(int r)
{
    dout(10) << "r=" << r << dendl;
    if (r == -EBADF) {
        dout(5) << "image id " << m_local_image_id << " already in-use" << dendl;
        ...
        derr << "failed to clone image " << m_parent_pool_name << "/" << m_remote_parent_spec.image_id << " to "
...
template<typename I> void CreateImageRequest<I>::close_remote_parent_image()
{
    dout(10) << dendl;
    Context* ctx =
        create_context_callback<CreateImageRequest<I>, &CreateImageRequest<I>::handle_close_remote_parent_image>(this);
    CloseImageRequest<I>* request = CloseImageRequest<I>::create(&m_remote_parent_image_ctx, ctx);
    request->send();
}

template<typename I> void CreateImageRequest<I>::handle_close_remote_parent_image(int r)
{
    dout(10) << "r=" << r << dendl;
    if (r < 0) {
...
template<typename I> void CreateImageRequest<I>::error(int r)
{
    dout(10) << "r=" << r << dendl;

    m_threads->work_queue->queue(create_context_callback<CreateImageRequest<I>, &CreateImageRequest<I>::finish>(this),
                                 r);
}

template<typename I> void CreateImageRequest<I>::finish(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> int CreateImageRequest<I>::validate_parent()
{
    ...
    int r = librbd::util::create_ioctx(m_remote_image_ctx->md_ctx,
                                       "remote parent pool",
                                       m_remote_parent_spec.pool_id,
                                       m_remote_parent_spec.pool_namespace,
                                       &m_remote_parent_io_ctx);
    if (r < 0) {
        derr << "failed to open remote parent pool " << m_remote_parent_spec.pool_id << ": " << cpp_strerror(r)
             << dendl;
        return r;
    }
...
template<typename I> void CreateImageRequest<I>::populate_image_options(librbd::ImageOptions* image_options)
{
    ...
    image_options->set(RBD_IMAGE_OPTION_STRIPE_UNIT, m_remote_image_ctx->stripe_unit);
    image_options->set(RBD_IMAGE_OPTION_STRIPE_COUNT, m_remote_image_ctx->stripe_count);
    ...
    if (m_remote_parent_spec.pool_id != -1) {
        ...
        if (m_remote_image_ctx->test_op_features(RBD_OPERATION_FEATURE_CLONE_CHILD)) {
            clone_format = 2;
        }
