...
#define dout_prefix                                    \
    *_dout << "rbd::mirror::image_replayer::journal::" \
           << "ReplayStatusFormatter: " << this << " " << __func__ << ": "
...
json_spirit::mObject to_json_object(const cls::journal::ObjectPosition& position)
{
...
template<typename I>
ReplayStatusFormatter<I>::ReplayStatusFormatter(Journaler* journaler, const std::string& mirror_uuid)
    : m_journaler(journaler)
    , m_mirror_uuid(mirror_uuid)
    , m_lock(ceph::make_mutex(unique_lock_name("ReplayStatusFormatter::m_lock", this)))
{}

template<typename I> void ReplayStatusFormatter<I>::handle_entry_processed(uint32_t bytes)
{
    dout(20) << dendl;
...
template<typename I> bool ReplayStatusFormatter<I>::get_or_send_update(std::string* description, Context* on_finish)
{
    dout(20) << dendl;
    ...
    r = m_journaler->get_cached_client(librbd::Journal<>::IMAGE_CLIENT_ID, &master_client);
    if (r < 0) {
    ...
    if (!master_client.commit_position.object_positions.empty()) {
        m_master_position = *(master_client.commit_position.object_positions.begin());
    }

    if (!mirror_client.commit_position.object_positions.empty()) {
        m_mirror_position = *(mirror_client.commit_position.object_positions.begin());
    }
...
template<typename I> bool ReplayStatusFormatter<I>::calculate_behind_master_or_send_update()
{
    dout(20) << "m_master_position=" << m_master_position << ", m_mirror_position=" << m_mirror_position << dendl;

    m_entries_behind_master = 0;

    if (m_master_position == cls::journal::ObjectPosition() || m_master_position.tag_tid < m_mirror_position.tag_tid) {
        return true;
    }
    ...
    if (master.tag_tid == mirror_tag_tid && master.entry_tid > m_mirror_position.entry_tid) {
...
template<typename I>
void ReplayStatusFormatter<I>::send_update_tag_cache(uint64_t master_tag_tid, uint64_t mirror_tag_tid)
{
    if (master_tag_tid <= mirror_tag_tid || m_tag_cache.find(master_tag_tid) != m_tag_cache.end()) {
    ...
    auto ctx = new LambdaContext(
        [this, master_tag_tid, mirror_tag_tid](int r) { handle_update_tag_cache(master_tag_tid, mirror_tag_tid, r); });
    m_journaler->get_tag(master_tag_tid, &m_tag, ctx);
}

template<typename I>
void ReplayStatusFormatter<I>::handle_update_tag_cache(uint64_t master_tag_tid, uint64_t mirror_tag_tid, int r)
{
    ...
    if (tag_data.predecessor.mirror_uuid != librbd::Journal<>::LOCAL_MIRROR_UUID &&
        tag_data.predecessor.mirror_uuid != librbd::Journal<>::ORPHAN_MIRROR_UUID) {
...
template<typename I> void ReplayStatusFormatter<I>::format(std::string* description)
{
    dout(20) << "m_master_position=" << m_master_position << ", m_mirror_position=" << m_mirror_position
    ...
    root_obj["entries_behind_primary"] = (m_entries_behind_master > 0 ? m_entries_behind_master : 0);

    m_bytes_per_second(0);
    root_obj["bytes_per_second"] = round_to_two_places(m_bytes_per_second.get_average());
    ...
    if (m_entries_behind_master > 0 && entries_per_second > 0) {
        std::uint64_t seconds_until_synced = round_to_two_places(m_entries_behind_master / entries_per_second);
