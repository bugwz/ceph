...
template<typename ImageCtxT = librbd::ImageCtx> class EventPreprocessor
{
public:
    ...
    static EventPreprocessor* create(ImageCtxT& local_image_ctx, Journaler& remote_journaler,
                                     const std::string& local_mirror_uuid, MirrorPeerClientMeta* client_meta,
                                     librbd::asio::ContextWQ* work_queue)
    {
        return new EventPreprocessor(local_image_ctx, remote_journaler, local_mirror_uuid, client_meta, work_queue);
    }

    static void destroy(EventPreprocessor* processor) { delete processor; }

    EventPreprocessor(ImageCtxT& local_image_ctx, Journaler& remote_journaler, const std::string& local_mirror_uuid,
                      MirrorPeerClientMeta* client_meta, librbd::asio::ContextWQ* work_queue);
    ~EventPreprocessor();
...
private:
    ...
    public:
        ...
        template<typename T> inline int operator()(T&) const { return 0; }
