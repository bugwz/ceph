...
template<typename I>
EventPreprocessor<I>::EventPreprocessor(I& local_image_ctx, Journaler& remote_journaler,
                                        const std::string& local_mirror_uuid, MirrorPeerClientMeta* client_meta,
...
template<typename I> EventPreprocessor<I>::~EventPreprocessor()
{
    ceph_assert(!m_in_progress);
}

template<typename I> bool EventPreprocessor<I>::is_required(const EventEntry& event_entry)
{
    SnapSeqs snap_seqs(m_client_meta->snap_seqs);
    return (prune_snap_map(&snap_seqs) || event_entry.get_event_type() == librbd::journal::EVENT_TYPE_SNAP_RENAME);
}

template<typename I> void EventPreprocessor<I>::preprocess(EventEntry* event_entry, Context* on_finish)
{
    ceph_assert(!m_in_progress);
    m_in_progress = true;
...
template<typename I> void EventPreprocessor<I>::refresh_image()
{
    dout(20) << dendl;

    Context* ctx = create_context_callback<EventPreprocessor<I>, &EventPreprocessor<I>::handle_refresh_image>(this);
    m_local_image_ctx.state->refresh(ctx);
}

template<typename I> void EventPreprocessor<I>::handle_refresh_image(int r)
{
    dout(20) << "r=" << r << dendl;
...
template<typename I> void EventPreprocessor<I>::preprocess_event()
{
    dout(20) << dendl;
    ...
    int r = boost::apply_visitor(PreprocessEventVisitor(this), m_event_entry->event);
    if (r < 0) {
        finish(r);
        return;
    }
...
template<typename I> int EventPreprocessor<I>::preprocess_snap_rename(librbd::journal::SnapRenameEvent& event)
{
    ...
    auto snap_id_it = m_local_image_ctx.snap_ids.find({cls::rbd::UserSnapshotNamespace(), event.src_snap_name});
...
template<typename I> void EventPreprocessor<I>::update_client()
{
    ...
    Context* ctx = create_context_callback<EventPreprocessor<I>, &EventPreprocessor<I>::handle_update_client>(this);
    m_remote_journaler.update_client(data_bl, ctx);
}

template<typename I> void EventPreprocessor<I>::handle_update_client(int r)
{
    dout(20) << "r=" << r << dendl;
...
template<typename I> bool EventPreprocessor<I>::prune_snap_map(SnapSeqs* snap_seqs)
{
    bool pruned = false;
...
template<typename I> void EventPreprocessor<I>::finish(int r)
{
    dout(20) << "r=" << r << dendl;
