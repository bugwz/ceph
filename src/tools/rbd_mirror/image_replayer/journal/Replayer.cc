...
uint32_t calculate_replay_delay(const utime_t& event_time, int mirroring_replay_delay)
{
...
template<typename I> struct Replayer<I>::C_ReplayCommitted : public Context
{
    Replayer* replayer;
    ...
    C_ReplayCommitted(Replayer* replayer, ReplayEntry&& replay_entry, uint64_t replay_bytes,
    ...
    {
        replayer->handle_process_entry_safe(replay_entry, replay_bytes, replay_start_time, r);
    }
};

template<typename I> struct Replayer<I>::RemoteJournalerListener : public ::journal::JournalMetadataListener
{
    Replayer* replayer;
    ...
    {
        auto ctx =
            new C_TrackedOp(replayer->m_in_flight_op_tracker,
                            new LambdaContext([this](int r) { replayer->handle_remote_journal_metadata_updated(); }));
        replayer->m_threads->work_queue->queue(ctx, 0);
    }
};

template<typename I> struct Replayer<I>::RemoteReplayHandler : public ::journal::ReplayHandler
{
    Replayer* replayer;
    ...
    void handle_entries_available() override { replayer->handle_replay_ready(); }
...
template<typename I> struct Replayer<I>::LocalJournalListener : public librbd::journal::Listener
{
    Replayer* replayer;
    ...
    void handle_close() override { replayer->handle_replay_complete(0, ""); }

    void handle_promoted() override { replayer->handle_replay_complete(0, "force promoted"); }

    void handle_resync() override { replayer->handle_resync_image(); }
};

template<typename I>
Replayer<I>::Replayer(Threads<I>* threads, const std::string& local_mirror_uuid, StateBuilder<I>* state_builder,
                      ReplayerListener* replayer_listener)
    : m_threads(threads)
    , m_local_mirror_uuid(local_mirror_uuid)
    , m_state_builder(state_builder)
    , m_replayer_listener(replayer_listener)
    , m_lock(ceph::make_mutex(librbd::util::unique_lock_name("rbd::mirror::image_replayer::journal::Replayer", this)))
{
    dout(10) << dendl;
}
...
template<typename I> void Replayer<I>::init(Context* on_finish)
{
...

    {
        ...
        m_image_spec = util::compute_image_spec(local_image_ctx->md_ctx, local_image_ctx->name);
    }
...
template<typename I> void Replayer<I>::init_remote_journaler()
{
    dout(10) << dendl;

    Context* ctx = create_context_callback<Replayer, &Replayer<I>::handle_init_remote_journaler>(this);
    m_state_builder->remote_journaler->init(ctx);
}

template<typename I> void Replayer<I>::handle_init_remote_journaler(int r)
{
    dout(10) << "r=" << r << dendl;
    ...
    r = m_state_builder->remote_journaler->get_cached_client(m_local_mirror_uuid, &remote_client);
    if (r < 0) {
    ...
    std::string error;
    r = validate_remote_client_state(remote_client, &m_state_builder->remote_client_meta, &m_resync_requested, &error);
    if (r < 0) {
...
template<typename I> void Replayer<I>::start_external_replay(std::unique_lock<ceph::mutex>& locker)
{
    dout(10) << dendl;
    ...
    Context* start_ctx = create_context_callback<Replayer, &Replayer<I>::handle_start_external_replay>(this);
    m_local_journal->start_external_replay(&m_local_journal_replay, start_ctx);
}

template<typename I> void Replayer<I>::handle_start_external_replay(int r)
{
    dout(10) << "r=" << r << dendl;
    ...
    if (r < 0) {
        ceph_assert(m_local_journal_replay == nullptr);
        derr << "error starting external replay on local image " << m_state_builder->local_image_ctx->id << ": "
    ...
    m_event_preprocessor = EventPreprocessor<I>::create(*m_state_builder->local_image_ctx,
                                                        *m_state_builder->remote_journaler,
                                                        m_local_mirror_uuid,
                                                        &m_state_builder->remote_client_meta,
                                                        m_threads->work_queue);
    m_replay_status_formatter =
        ReplayStatusFormatter<I>::create(m_state_builder->remote_journaler, m_local_mirror_uuid);

    auto cct = static_cast<CephContext*>(m_state_builder->local_image_ctx->cct);
    double poll_seconds = cct->_conf.get_val<double>("rbd_mirror_journal_poll_age");
    m_remote_replay_handler = new RemoteReplayHandler(this);
    m_state_builder->remote_journaler->start_live_replay(m_remote_replay_handler, poll_seconds);

    notify_status_updated();
}

template<typename I> bool Replayer<I>::add_local_journal_listener(std::unique_lock<ceph::mutex>& locker)
{
    dout(10) << dendl;
...
template<typename I> bool Replayer<I>::notify_init_complete(std::unique_lock<ceph::mutex>& locker)
{
    dout(10) << dendl;
...
template<typename I> void Replayer<I>::wait_for_flush()
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
    ...
    auto ctx = create_async_context_callback(
        m_threads->work_queue, create_context_callback<Replayer<I>, &Replayer<I>::handle_wait_for_flush>(this));
    m_flush_tracker.wait_for_ops(ctx);
}

template<typename I> void Replayer<I>::handle_wait_for_flush(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::shut_down_local_journal_replay()
{
    std::unique_lock locker{m_lock};
    ...
    dout(10) << dendl;
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_shut_down_local_journal_replay>(this);
    m_local_journal_replay->shut_down(true, ctx);
}

template<typename I> void Replayer<I>::handle_shut_down_local_journal_replay(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::wait_for_event_replay()
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));

    dout(10) << dendl;
    auto ctx = create_async_context_callback(
        m_threads->work_queue, create_context_callback<Replayer<I>, &Replayer<I>::handle_wait_for_event_replay>(this));
    m_event_replay_tracker.wait_for_ops(ctx);
}

template<typename I> void Replayer<I>::handle_wait_for_event_replay(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::close_local_image()
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
    ...
    ceph_assert(m_state_builder->local_image_ctx != nullptr);
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_close_local_image>(this);
    auto request = image_replayer::CloseImageRequest<I>::create(&m_state_builder->local_image_ctx, ctx);
    request->send();
}


template<typename I> void Replayer<I>::handle_close_local_image(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::stop_remote_journaler_replay()
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
    ...
    auto ctx = create_async_context_callback(
        m_threads->work_queue,
        create_context_callback<Replayer<I>, &Replayer<I>::handle_stop_remote_journaler_replay>(this));
    m_state_builder->remote_journaler->stop_replay(ctx);
}

template<typename I> void Replayer<I>::handle_stop_remote_journaler_replay(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::handle_remote_journal_metadata_updated()
{
    dout(20) << dendl;
    ...
    cls::journal::Client remote_client;
    int r = m_state_builder->remote_journaler->get_cached_client(m_local_mirror_uuid, &remote_client);
    if (r < 0) {
    ...
    std::string error;
    r = validate_remote_client_state(remote_client, &remote_client_meta, &m_resync_requested, &error);
    if (r < 0) {
...
template<typename I> void Replayer<I>::schedule_flush_local_replay_task()
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
    ...
    m_flush_local_replay_task = create_async_context_callback(
        m_threads->work_queue,
        create_context_callback<Replayer<I>, &Replayer<I>::handle_flush_local_replay_task>(this));
    m_threads->timer->add_event_after(30, m_flush_local_replay_task);
}

template<typename I> void Replayer<I>::cancel_flush_local_replay_task()
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
...
template<typename I> void Replayer<I>::handle_flush_local_replay_task(int)
{
    dout(15) << dendl;
...
template<typename I> void Replayer<I>::flush_local_replay(Context* on_flush)
{
    std::unique_lock locker{m_lock};
    if (m_state != STATE_REPLAYING) {
        locker.unlock();
    ...
    dout(15) << dendl;
    auto ctx = new LambdaContext([this, on_flush](int r) { handle_flush_local_replay(on_flush, r); });
    m_local_journal_replay->flush(ctx);
}

template<typename I> void Replayer<I>::handle_flush_local_replay(Context* on_flush, int r)
{
    dout(15) << "r=" << r << dendl;
    if (r < 0) {
...
template<typename I> void Replayer<I>::flush_commit_position(Context* on_flush)
{
    std::unique_lock locker{m_lock};
    if (m_state != STATE_REPLAYING) {
        locker.unlock();
    ...
    dout(15) << dendl;
    auto ctx = new LambdaContext([this, on_flush](int r) { handle_flush_commit_position(on_flush, r); });
    m_state_builder->remote_journaler->flush_commit_position(ctx);
}

template<typename I> void Replayer<I>::handle_flush_commit_position(Context* on_flush, int r)
{
    dout(15) << "r=" << r << dendl;
    if (r < 0) {
...
template<typename I> void Replayer<I>::handle_replay_error(int r, const std::string& error)
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
...
template<typename I> bool Replayer<I>::is_replay_complete() const
{
    std::unique_lock locker{m_lock};
    return is_replay_complete(locker);
}

template<typename I> bool Replayer<I>::is_replay_complete(const std::unique_lock<ceph::mutex>&) const
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
    return (m_state == STATE_COMPLETE);
}

template<typename I> void Replayer<I>::handle_replay_complete(int r, const std::string& error)
{
    std::unique_lock locker{m_lock};
    handle_replay_complete(locker, r, error);
}
...
template<typename I>
void Replayer<I>::handle_replay_complete(const std::unique_lock<ceph::mutex>&, int r, const std::string& error)
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
...
template<typename I> void Replayer<I>::handle_replay_ready()
{
    std::unique_lock locker{m_lock};
    handle_replay_ready(locker);
}

template<typename I> void Replayer<I>::handle_replay_ready(std::unique_lock<ceph::mutex>& locker)
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
    ...
    if (!m_state_builder->remote_journaler->try_pop_front(&m_replay_entry, &m_replay_tag_tid)) {
...
template<typename I> void Replayer<I>::replay_flush()
{
    dout(10) << dendl;
    ...
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_replay_flush_shut_down>(this);
    ceph_assert(m_local_journal_replay != nullptr);
    m_local_journal_replay->shut_down(false, ctx);
}

template<typename I> void Replayer<I>::handle_replay_flush_shut_down(int r)
{
    std::unique_lock locker{m_lock};
    dout(10) << "r=" << r << dendl;
    ...
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_replay_flush>(this);
    m_local_journal->start_external_replay(&m_local_journal_replay, ctx);
}

template<typename I> void Replayer<I>::handle_replay_flush(int r)
{
    std::unique_lock locker{m_lock};
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::get_remote_tag()
{
    dout(15) << "tag_tid: " << m_replay_tag_tid << dendl;

    Context* ctx = create_context_callback<Replayer, &Replayer<I>::handle_get_remote_tag>(this);
    m_state_builder->remote_journaler->get_tag(m_replay_tag_tid, &m_replay_tag, ctx);
}

template<typename I> void Replayer<I>::handle_get_remote_tag(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::allocate_local_tag()
{
    dout(15) << dendl;
    ...
    if (mirror_uuid == librbd::Journal<>::LOCAL_MIRROR_UUID) {
        ...
        if (local_tag_data.mirror_uuid == librbd::Journal<>::ORPHAN_MIRROR_UUID &&
            (local_tag_data.predecessor.commit_valid &&
             local_tag_data.predecessor.mirror_uuid == librbd::Journal<>::LOCAL_MIRROR_UUID)) {
            dout(15) << "skipping stale demotion event" << dendl;
            handle_process_entry_safe(m_replay_entry, m_replay_bytes, m_replay_start_time, 0);
            handle_replay_ready();
            return;
        }
        else {
    ...
    Context* ctx = create_context_callback<Replayer, &Replayer<I>::handle_allocate_local_tag>(this);
    m_local_journal->allocate_tag(mirror_uuid, predecessor, ctx);
}

template<typename I> void Replayer<I>::handle_allocate_local_tag(int r)
{
    dout(15) << "r=" << r << ", "
...
template<typename I> void Replayer<I>::preprocess_entry()
{
    ...
    uint32_t delay =
        calculate_replay_delay(m_event_entry.timestamp, m_state_builder->local_image_ctx->mirroring_replay_delay);
    if (delay == 0) {
    ...
    m_delayed_preprocess_task =
        create_context_callback<Replayer<I>, &Replayer<I>::handle_delayed_preprocess_task>(this);
    m_threads->timer->add_event_after(delay, m_delayed_preprocess_task);
}

template<typename I> void Replayer<I>::handle_delayed_preprocess_task(int r)
{
    dout(20) << "r=" << r << dendl;
    ...
    m_threads->work_queue->queue(create_context_callback<Replayer, &Replayer<I>::handle_preprocess_entry_ready>(this),
                                 0);
}

template<typename I> void Replayer<I>::handle_preprocess_entry_ready(int r)
{
    dout(20) << "r=" << r << dendl;
    ceph_assert(r == 0);
    ...
    Context* ctx = create_context_callback<Replayer, &Replayer<I>::handle_preprocess_entry_safe>(this);
    m_event_preprocessor->preprocess(&m_event_entry, ctx);
}

template<typename I> void Replayer<I>::handle_preprocess_entry_safe(int r)
{
    dout(20) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::process_entry()
{
    dout(20) << "processing entry tid=" << m_replay_entry.get_commit_tid() << dendl;

    Context* on_ready = create_context_callback<Replayer, &Replayer<I>::handle_process_entry_ready>(this);
    Context* on_commit = new C_ReplayCommitted(this, std::move(m_replay_entry), m_replay_bytes, m_replay_start_time);

    m_local_journal_replay->process(m_event_entry, on_ready, on_commit);
}

template<typename I> void Replayer<I>::handle_process_entry_ready(int r)
{
    std::unique_lock locker{m_lock};
    ...
    bool update_status = false;
    {
        auto local_image_ctx = m_state_builder->local_image_ctx;
        std::shared_lock image_locker{local_image_ctx->image_lock};
        auto image_spec = util::compute_image_spec(local_image_ctx->md_ctx, local_image_ctx->name);
...
template<typename I>
void Replayer<I>::handle_process_entry_safe(const ReplayEntry& replay_entry, uint64_t replay_bytes,
                                            const utime_t& replay_start_time, int r)
{
    ...
    if (g_journal_perf_counters) {
        ...
        g_journal_perf_counters->inc(l_rbd_mirror_journal_replay_bytes, replay_bytes);
...
template<typename I> void Replayer<I>::handle_resync_image()
{
    dout(10) << dendl;
...
template<typename I> void Replayer<I>::cancel_delayed_preprocess_task()
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
    ...
    bool canceled_delayed_preprocess_task = false;
    {
        ...
        if (m_delayed_preprocess_task != nullptr) {
            dout(10) << dendl;
            canceled_delayed_preprocess_task = m_threads->timer->cancel_event(m_delayed_preprocess_task);
...
template<typename I>
int Replayer<I>::validate_remote_client_state(const cls::journal::Client& remote_client,
                                              librbd::journal::MirrorPeerClientMeta* remote_client_meta,
                                              bool* resync_requested, std::string* error)
{
    ...
    if (remote_client_meta->image_id == local_image_ctx->id &&
        ...
        if (local_image_ctx->config.template get_val<bool>("rbd_mirroring_resync_after_disconnect")) {
...
template<typename I> void Replayer<I>::register_perf_counters()
{
    ...
    auto local_image_ctx = m_state_builder->local_image_ctx;
    std::string labels = ceph::perf_counters::key_create("rbd_mirror_journal_image",
                                                         {{"pool", local_image_ctx->md_ctx.get_pool_name()},
                                                          {"namespace", local_image_ctx->md_ctx.get_namespace()},
                                                          {"image", local_image_ctx->name}});

    PerfCountersBuilder plb(g_ceph_context, labels, l_rbd_mirror_journal_first, l_rbd_mirror_journal_last);
    plb.add_u64_counter(l_rbd_mirror_journal_entries, "entries", "Number of entries replayed", nullptr, prio);
    plb.add_u64_counter(
        l_rbd_mirror_journal_replay_bytes, "replay_bytes", "Total bytes replayed", nullptr, prio, unit_t(UNIT_BYTES));
    plb.add_time_avg(l_rbd_mirror_journal_replay_latency, "replay_latency", "Replay latency", nullptr, prio);
