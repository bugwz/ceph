...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::image_replayer::OpenImageRequest: " << this << " " << __func__ << " "
...
template<typename I>
OpenImageRequest<I>::OpenImageRequest(librados::IoCtx& io_ctx, I** image_ctx, const std::string& image_id,
...
template<typename I> void OpenImageRequest<I>::send_open_image()
{
    dout(20) << dendl;
    ...
    *m_image_ctx = I::create("", m_image_id, nullptr, m_io_ctx, m_read_only);
    ...
    Context* ctx = create_context_callback<OpenImageRequest<I>, &OpenImageRequest<I>::handle_open_image>(this);
    (*m_image_ctx)->state->open(0, ctx);
}

template<typename I> void OpenImageRequest<I>::handle_open_image(int r)
{
    dout(20) << ": r=" << r << dendl;
