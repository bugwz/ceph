...
template<typename I> std::pair<uint64_t, librbd::SnapInfo*> get_newest_mirror_snapshot(I* image_ctx)
{
    for (auto snap_info_it = image_ctx->snap_info.rbegin(); snap_info_it != image_ctx->snap_info.rend();
         ++snap_info_it) {
        const auto& snap_ns = snap_info_it->second.snap_namespace;
        auto mirror_ns = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap_ns);
...
template<typename I> struct Replayer<I>::C_UpdateWatchCtx : public librbd::UpdateWatchCtx
{
    Replayer<I>* replayer;

    C_UpdateWatchCtx(Replayer<I>* replayer)
        : replayer(replayer)
    {}

    void handle_notify() override { replayer->handle_image_update_notify(); }
};

template<typename I> struct Replayer<I>::DeepCopyHandler : public librbd::deep_copy::Handler
{
    Replayer* replayer;

    DeepCopyHandler(Replayer* replayer)
        : replayer(replayer)
    {}

    void handle_read(uint64_t bytes_read) override { replayer->handle_copy_image_read(bytes_read); }
...
template<typename I>
Replayer<I>::Replayer(Threads<I>* threads, InstanceWatcher<I>* instance_watcher, const std::string& local_mirror_uuid,
                      PoolMetaCache* pool_meta_cache, StateBuilder<I>* state_builder,
    ...
    , m_lock(ceph::make_mutex(librbd::util::unique_lock_name("rbd::mirror::image_replayer::snapshot::Replayer", this)))
{
    dout(10) << dendl;
}

template<typename I> Replayer<I>::~Replayer()
{
    dout(10) << dendl;
...
template<typename I> void Replayer<I>::init(Context* on_finish)
{
    dout(10) << dendl;
    ...
    int r =
        m_pool_meta_cache->get_remote_pool_meta(m_state_builder->remote_image_ctx->md_ctx.get_id(), &remote_pool_meta);
...

    {
        ...
        m_image_spec = image_replayer::util::compute_image_spec(local_image_ctx->md_ctx, local_image_ctx->name);
    }
...
template<typename I> void Replayer<I>::shut_down(Context* on_finish)
{
    dout(10) << dendl;
    ...
    if (state == STATE_REPLAYING) {
        ...
        m_instance_watcher->cancel_sync_request(m_state_builder->local_image_ctx->id);
...
template<typename I> void Replayer<I>::flush(Context* on_finish)
{
    dout(10) << dendl;
...
template<typename I> bool Replayer<I>::get_replay_status(std::string* description, Context* on_finish)
{
    dout(10) << dendl;
    ...
    std::shared_lock local_image_locker{m_state_builder->local_image_ctx->image_lock};
    auto [local_snap_id, local_snap_info] = get_newest_mirror_snapshot(m_state_builder->local_image_ctx);

    std::shared_lock remote_image_locker{m_state_builder->remote_image_ctx->image_lock};
    auto [remote_snap_id, remote_snap_info] = get_newest_mirror_snapshot(m_state_builder->remote_image_ctx);
    ...
    if (m_perf_counters) {
        m_perf_counters->tset(l_rbd_mirror_snapshot_remote_timestamp, remote_snap_info->timestamp);
    }

    auto matching_remote_snap_id = util::compute_remote_snap_id(m_state_builder->local_image_ctx->image_lock,
                                                                m_state_builder->local_image_ctx->snap_info,
                                                                local_snap_id,
                                                                m_state_builder->remote_mirror_uuid);
    auto matching_remote_snap_it = m_state_builder->remote_image_ctx->snap_info.find(matching_remote_snap_id);
    if (matching_remote_snap_id != CEPH_NOSNAP &&
        matching_remote_snap_it != m_state_builder->remote_image_ctx->snap_info.end()) {
        // use the timestamp from the matching remote image since
        // the local snapshot would just be the time the snapshot was
        // synced and not the consistency point in time.
        root_obj["local_snapshot_timestamp"] = matching_remote_snap_it->second.timestamp.sec();
        if (m_perf_counters) {
            m_perf_counters->tset(l_rbd_mirror_snapshot_local_timestamp, matching_remote_snap_it->second.timestamp);
        }
    }

    matching_remote_snap_it = m_state_builder->remote_image_ctx->snap_info.find(m_remote_snap_id_end);
    if (m_remote_snap_id_end != CEPH_NOSNAP &&
        matching_remote_snap_it != m_state_builder->remote_image_ctx->snap_info.end()) {
        root_obj["syncing_snapshot_timestamp"] = remote_snap_info->timestamp.sec();

        if (m_local_object_count > 0) {
            root_obj["syncing_percent"] = 100 * m_local_mirror_snap_ns.last_copied_object_number / m_local_object_count;
        }
        else {
    ...
    auto bytes_per_snapshot = boost::accumulators::rolling_mean(m_bytes_per_snapshot);
    ...
    if (bytes_per_second > 0 && m_pending_snapshots > 0) {
        std::uint64_t seconds_until_synced = round_to_two_places(pending_bytes / bytes_per_second);
    ...
    *description = json_spirit::write(root_obj, json_spirit::remove_trailing_zeros);
...
template<typename I> void Replayer<I>::load_local_image_meta()
{
    dout(10) << dendl;
    ...
    bool update_status = false;
    {
        auto local_image_ctx = m_state_builder->local_image_ctx;
        std::shared_lock image_locker{local_image_ctx->image_lock};
        auto image_spec = image_replayer::util::compute_image_spec(local_image_ctx->md_ctx, local_image_ctx->name);
    ...
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_load_local_image_meta>(this);
    m_state_builder->local_image_meta->load(ctx);
}

template<typename I> void Replayer<I>::handle_load_local_image_meta(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::refresh_local_image()
{
    ...
    dout(10) << dendl;
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_refresh_local_image>(this);
    m_state_builder->local_image_ctx->state->refresh(ctx);
}

template<typename I> void Replayer<I>::handle_refresh_local_image(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::refresh_remote_image()
{
    ...
    dout(10) << dendl;
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_refresh_remote_image>(this);
    m_state_builder->remote_image_ctx->state->refresh(ctx);
}

template<typename I> void Replayer<I>::handle_refresh_remote_image(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::scan_local_mirror_snapshots(std::unique_lock<ceph::mutex>* locker)
{
    ...
    auto local_image_ctx = m_state_builder->local_image_ctx;
    std::shared_lock image_locker{local_image_ctx->image_lock};
    for (auto snap_info_it = local_image_ctx->snap_info.begin(); snap_info_it != local_image_ctx->snap_info.end();
         ++snap_info_it) {
        const auto& snap_ns = snap_info_it->second.snap_namespace;
        auto mirror_ns = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap_ns);
        ...
        if (mirror_ns->is_non_primary()) {
            ...
            else if (mirror_ns->last_copied_object_number == 0 && m_local_snap_id_start > 0) {
            ...
            if (mirror_ns->complete) {
                ...
                handle_replay_complete(locker, -EINVAL, "incomplete local primary snapshot");
                return;
            }
        }
        else {
            derr << "unknown local mirror snapshot state" << dendl;
            handle_replay_complete(locker, -EINVAL, "invalid local mirror snapshot state");
            return;
        }
    }
    image_locker.unlock();
    ...
    if (m_local_snap_id_start > 0 || m_local_snap_id_end != CEPH_NOSNAP) {
        if (m_local_mirror_snap_ns.is_non_primary() &&
            m_local_mirror_snap_ns.primary_mirror_uuid != m_state_builder->remote_mirror_uuid) {
            ...
            handle_replay_complete(locker, -EEXIST, "local image linked to unknown peer");
            return;
        }
        else if (m_local_mirror_snap_ns.state == cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY) {
        ...
        if (!m_local_mirror_snap_ns.is_primary() && m_local_mirror_snap_ns.complete) {
...
template<typename I> void Replayer<I>::scan_remote_mirror_snapshots(std::unique_lock<ceph::mutex>* locker)
{
    dout(10) << dendl;
    ...
    for (auto snap_info_it = remote_image_ctx->snap_info.begin(); snap_info_it != remote_image_ctx->snap_info.end();
         ++snap_info_it) {
        const auto& snap_ns = snap_info_it->second.snap_namespace;
        auto mirror_ns = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap_ns);
        if (mirror_ns == nullptr) {
        ...
        if (!mirror_ns->is_primary() && !mirror_ns->is_non_primary()) {
            ...
            handle_replay_complete(locker, -EINVAL, "invalid remote mirror snapshot state");
            return;
        }
        else if (mirror_ns->mirror_peer_uuids.count(m_remote_mirror_peer_uuid) == 0) {
        ...
        if (m_local_snap_id_start > 0 || m_local_snap_id_end != CEPH_NOSNAP) {
            ...
            if (m_local_mirror_snap_ns.is_non_primary()) {
                ...
                ceph_assert(m_local_mirror_snap_ns.primary_mirror_uuid == m_state_builder->remote_mirror_uuid);

                if (m_remote_snap_id_end == CEPH_NOSNAP) {
                    // haven't found the end snap so treat this as a candidate for unlink
                    unlink_snap_ids.insert(remote_snap_id);
                }
                if (m_local_mirror_snap_ns.complete && m_local_mirror_snap_ns.primary_snap_id >= remote_snap_id) {
                ...
                else if (!m_local_mirror_snap_ns.complete && m_local_mirror_snap_ns.primary_snap_id > remote_snap_id) {
            ...
            else if (m_local_mirror_snap_ns.state == cls::rbd::MIRROR_SNAPSHOT_STATE_PRIMARY_DEMOTED) {
                // find the matching demotion snapshot in remote image
                ceph_assert(m_local_snap_id_start > 0);
                if (mirror_ns->state == cls::rbd::MIRROR_SNAPSHOT_STATE_NON_PRIMARY_DEMOTED &&
    ...
    if (m_remote_snap_id_start != 0 && remote_image_ctx->snap_info.count(m_remote_snap_id_start) == 0) {
    ...
    if (!split_brain) {
        ...
        if (m_remote_snap_id_end != CEPH_NOSNAP) {
            ...
            if (m_remote_mirror_snap_ns.complete) {
                ...
                if (m_local_snap_id_end != CEPH_NOSNAP && !m_local_mirror_snap_ns.complete) {
...
template<typename I> void Replayer<I>::prune_non_primary_snapshot(uint64_t snap_id)
{
...

    {
        ...
        if (snap_info != nullptr) {
            ...
            ceph_assert(std::holds_alternative<cls::rbd::MirrorSnapshotNamespace>(snap_namespace));
        }
    }
    ...
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_prune_non_primary_snapshot>(this);
    local_image_ctx->operations->snap_remove(snap_namespace, snap_name, ctx);
}

template<typename I> void Replayer<I>::handle_prune_non_primary_snapshot(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::copy_snapshots()
{
    ...
    ceph_assert(m_remote_snap_id_end > 0 && m_remote_snap_id_end != CEPH_NOSNAP);
    ceph_assert(m_local_snap_id_start != CEPH_NOSNAP);

    m_local_mirror_snap_ns = {};
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_copy_snapshots>(this);
    auto req = librbd::deep_copy::SnapshotCopyRequest<I>::create(m_state_builder->remote_image_ctx,
...
template<typename I> void Replayer<I>::handle_copy_snapshots(int r)
{
    dout(10) << "r=" << r << dendl;

    if (r < 0) {
        derr << "failed to copy snapshots from remote to local image: " << cpp_strerror(r) << dendl;
        handle_replay_complete(r, "failed to copy snapshots from remote to local image");
        return;
    }
...
template<typename I> void Replayer<I>::get_remote_image_state()
{
    dout(10) << dendl;

    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_get_remote_image_state>(this);
    auto req = librbd::mirror::snapshot::GetImageStateRequest<I>::create(
        m_state_builder->remote_image_ctx, m_remote_snap_id_end, &m_image_state, ctx);
    req->send();
}

template<typename I> void Replayer<I>::handle_get_remote_image_state(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::get_local_image_state()
{
    dout(10) << dendl;

    ceph_assert(m_local_snap_id_end != CEPH_NOSNAP);
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_get_local_image_state>(this);
    auto req = librbd::mirror::snapshot::GetImageStateRequest<I>::create(
        m_state_builder->local_image_ctx, m_local_snap_id_end, &m_image_state, ctx);
    req->send();
}

template<typename I> void Replayer<I>::handle_get_local_image_state(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::create_non_primary_snapshot()
{
    auto local_image_ctx = m_state_builder->local_image_ctx;
    ...
    if (m_local_snap_id_start > 0) {
        ...
        auto local_snap_info_it = local_image_ctx->snap_info.find(m_local_snap_id_start);
        ...
        auto mirror_ns = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&local_snap_info_it->second.snap_namespace);
        ...
        for (auto& [remote_snap_id, remote_snap_info] : remote_image_ctx->snap_info) {
            ...
            // user snapshots
            if (!std::holds_alternative<cls::rbd::UserSnapshotNamespace>(remote_snap_info.snap_namespace)) {
                continue;
            }
            ...
            if (mirror_ns->is_demoted() && !m_remote_mirror_snap_ns.is_demoted()) {
                ...
                // snapshot mapping
                auto local_snap_id_it =
                    local_image_ctx->snap_ids.find({remote_snap_info.snap_namespace, remote_snap_info.name});
            ...
            if (m_local_mirror_snap_ns.snap_seqs.count(remote_snap_id) == 0 && local_snap_id != CEPH_NOSNAP) {
    ...
    dout(10) << "demoted=" << m_remote_mirror_snap_ns.is_demoted() << ", "
             << "primary_mirror_uuid=" << m_state_builder->remote_mirror_uuid << ", "
             << "primary_snap_id=" << m_remote_snap_id_end << ", "
             << "snap_seqs=" << m_local_mirror_snap_ns.snap_seqs << dendl;

    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_create_non_primary_snapshot>(this);
    auto req = librbd::mirror::snapshot::CreateNonPrimaryRequest<I>::create(local_image_ctx,
                                                                            m_remote_mirror_snap_ns.is_demoted(),
                                                                            m_state_builder->remote_mirror_uuid,
                                                                            m_remote_snap_id_end,
                                                                            m_local_mirror_snap_ns.snap_seqs,
                                                                            m_image_state,
...
template<typename I> void Replayer<I>::handle_create_non_primary_snapshot(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::update_mirror_image_state()
{
    if (m_local_snap_id_start > 0) {
        request_sync();
        return;
    ...
    dout(10) << dendl;
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_update_mirror_image_state>(this);
    auto req = librbd::mirror::ImageStateUpdateRequest<I>::create(m_state_builder->local_image_ctx->md_ctx,
...
template<typename I> void Replayer<I>::handle_update_mirror_image_state(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::request_sync()
{
    ...
    auto ctx = create_async_context_callback(
        m_threads->work_queue, create_context_callback<Replayer<I>, &Replayer<I>::handle_request_sync>(this));
    m_instance_watcher->notify_sync_request(m_state_builder->local_image_ctx->id, ctx);
}

template<typename I> void Replayer<I>::handle_request_sync(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::copy_image()
{
    dout(10) << "remote_snap_id_start=" << m_remote_snap_id_start << ", "
             << "remote_snap_id_end=" << m_remote_snap_id_end << ", "
             << "local_snap_id_start=" << m_local_snap_id_start << ", "
             << "last_copied_object_number=" << m_local_mirror_snap_ns.last_copied_object_number << ", "
             << "snap_seqs=" << m_local_mirror_snap_ns.snap_seqs << dendl;

    m_snapshot_bytes = 0;
    m_snapshot_replay_start = ceph_clock_now();
    m_deep_copy_handler = new DeepCopyHandler(this);
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_copy_image>(this);
    auto req = librbd::deep_copy::ImageCopyRequest<I>::create(
        ...
        (m_local_mirror_snap_ns.last_copied_object_number > 0
             ? librbd::deep_copy::ObjectNumber{m_local_mirror_snap_ns.last_copied_object_number}
...
template<typename I> void Replayer<I>::handle_copy_image(int r)
{
    dout(10) << "r=" << r << dendl;
...

    {
        ...
        if (g_snapshot_perf_counters) {
            g_snapshot_perf_counters->inc(l_rbd_mirror_snapshot_sync_bytes, m_snapshot_bytes);
            g_snapshot_perf_counters->inc(l_rbd_mirror_snapshot_snapshots);
            g_snapshot_perf_counters->tinc(l_rbd_mirror_snapshot_sync_time, duration);
        }
        if (m_perf_counters) {
            ...
            m_perf_counters->set(l_rbd_mirror_snapshot_last_sync_bytes, m_snapshot_bytes);
        }
    }
...
template<typename I> void Replayer<I>::handle_copy_image_progress(uint64_t object_number, uint64_t object_count)
{
    dout(10) << "object_number=" << object_number << ", "
             << "object_count=" << object_count << dendl;

    std::unique_lock locker{m_lock};
    m_local_mirror_snap_ns.last_copied_object_number = std::min(object_number, object_count);
    m_local_object_count = object_count;

    update_non_primary_snapshot(false);
}

template<typename I> void Replayer<I>::handle_copy_image_read(uint64_t bytes_read)
{
...
template<typename I> void Replayer<I>::apply_image_state()
{
    dout(10) << dendl;

    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_apply_image_state>(this);
    auto req = ApplyImageStateRequest<I>::create(m_local_mirror_uuid,
                                                 m_state_builder->remote_mirror_uuid,
                                                 m_state_builder->local_image_ctx,
                                                 m_state_builder->remote_image_ctx,
                                                 m_image_state,
...
template<typename I> void Replayer<I>::handle_apply_image_state(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::update_non_primary_snapshot(bool complete)
{
    ...
    librbd::cls_client::mirror_image_snapshot_set_copy_progress(
        &op, m_local_snap_id_end, m_local_mirror_snap_ns.complete, m_local_mirror_snap_ns.last_copied_object_number);

    auto ctx = new C_TrackedOp(m_in_flight_op_tracker, new LambdaContext([this, complete](int r) {
...
template<typename I> void Replayer<I>::handle_update_non_primary_snapshot(bool complete, int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::notify_image_update()
{
    dout(10) << dendl;

    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_notify_image_update>(this);
    m_state_builder->local_image_ctx->notify_update(ctx);
}

template<typename I> void Replayer<I>::handle_notify_image_update(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::unlink_peer(uint64_t remote_snap_id)
{
    if (remote_snap_id == 0) {
        finish_sync();
        return;
    ...
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_unlink_peer>(this);
    auto req = librbd::mirror::snapshot::UnlinkPeerRequest<I>::create(
        m_state_builder->remote_image_ctx, remote_snap_id, m_remote_mirror_peer_uuid, false, ctx);
    req->send();
}

template<typename I> void Replayer<I>::handle_unlink_peer(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::finish_sync()
{
    dout(10) << dendl;

    {
        ...
        if (m_sync_in_progress) {
            ...
            m_instance_watcher->notify_sync_complete(m_state_builder->local_image_ctx->id);
        }
    }
...
template<typename I> void Replayer<I>::register_local_update_watcher()
{
    dout(10) << dendl;

    m_update_watch_ctx = new C_UpdateWatchCtx(this);

    int r = m_state_builder->local_image_ctx->state->register_update_watcher(m_update_watch_ctx,
                                                                             &m_local_update_watcher_handle);
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_register_local_update_watcher>(this);
    m_threads->work_queue->queue(ctx, r);
}

template<typename I> void Replayer<I>::handle_register_local_update_watcher(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::register_remote_update_watcher()
{
    dout(10) << dendl;

    int r = m_state_builder->remote_image_ctx->state->register_update_watcher(m_update_watch_ctx,
                                                                              &m_remote_update_watcher_handle);
    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_register_remote_update_watcher>(this);
    m_threads->work_queue->queue(ctx, r);
}

template<typename I> void Replayer<I>::handle_register_remote_update_watcher(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::unregister_remote_update_watcher()
{
    dout(10) << dendl;

    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_unregister_remote_update_watcher>(this);
    m_state_builder->remote_image_ctx->state->unregister_update_watcher(m_remote_update_watcher_handle, ctx);
}

template<typename I> void Replayer<I>::handle_unregister_remote_update_watcher(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::unregister_local_update_watcher()
{
    dout(10) << dendl;

    auto ctx = create_context_callback<Replayer<I>, &Replayer<I>::handle_unregister_local_update_watcher>(this);
    m_state_builder->local_image_ctx->state->unregister_update_watcher(m_local_update_watcher_handle, ctx);
}

template<typename I> void Replayer<I>::handle_unregister_local_update_watcher(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::wait_for_in_flight_ops()
{
    dout(10) << dendl;

    auto ctx = create_async_context_callback(
        m_threads->work_queue, create_context_callback<Replayer<I>, &Replayer<I>::handle_wait_for_in_flight_ops>(this));
    m_in_flight_op_tracker.wait_for_ops(ctx);
}

template<typename I> void Replayer<I>::handle_wait_for_in_flight_ops(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void Replayer<I>::handle_image_update_notify()
{
    dout(10) << dendl;
...
template<typename I> void Replayer<I>::handle_replay_complete(int r, const std::string& description)
{
    std::unique_lock locker{m_lock};
...
template<typename I>
void Replayer<I>::handle_replay_complete(std::unique_lock<ceph::mutex>* locker, int r, const std::string& description)
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
...
template<typename I> void Replayer<I>::notify_status_updated()
{
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));

    dout(10) << dendl;
    auto ctx = new C_TrackedOp(m_in_flight_op_tracker,
                               new LambdaContext([this](int) { m_replayer_listener->handle_notification(); }));
    m_threads->work_queue->queue(ctx, 0);
}

template<typename I> bool Replayer<I>::is_replay_interrupted()
{
    std::unique_lock locker{m_lock};
    return is_replay_interrupted(&locker);
}

template<typename I> bool Replayer<I>::is_replay_interrupted(std::unique_lock<ceph::mutex>* locker)
{
...
template<typename I> void Replayer<I>::register_perf_counters()
{
    ...
    auto local_image_ctx = m_state_builder->local_image_ctx;
    std::string labels = ceph::perf_counters::key_create("rbd_mirror_snapshot_image",
                                                         {{"pool", local_image_ctx->md_ctx.get_pool_name()},
                                                          {"namespace", local_image_ctx->md_ctx.get_namespace()},
                                                          {"image", local_image_ctx->name}});

    PerfCountersBuilder plb(g_ceph_context, labels, l_rbd_mirror_snapshot_first, l_rbd_mirror_snapshot_last);
    plb.add_u64_counter(l_rbd_mirror_snapshot_snapshots, "snapshots", "Number of snapshots synced", nullptr, prio);
    plb.add_time_avg(l_rbd_mirror_snapshot_sync_time, "sync_time", "Average sync time", nullptr, prio);
    plb.add_u64_counter(
        l_rbd_mirror_snapshot_sync_bytes, "sync_bytes", "Total bytes synced", nullptr, prio, unit_t(UNIT_BYTES));
    plb.add_time(
        l_rbd_mirror_snapshot_remote_timestamp, "remote_timestamp", "Timestamp of the remote snapshot", nullptr, prio);
    plb.add_time(
        l_rbd_mirror_snapshot_local_timestamp, "local_timestamp", "Timestamp of the local snapshot", nullptr, prio);
    plb.add_time(
        l_rbd_mirror_snapshot_last_sync_time, "last_sync_time", "Time taken to sync the last snapshot", nullptr, prio);
    plb.add_u64(l_rbd_mirror_snapshot_last_sync_bytes,
                "last_sync_bytes",
                "Bytes synced for the last snapshot",
                nullptr,
                prio,
                unit_t(UNIT_BYTES));
...
template<typename I> void Replayer<I>::unregister_perf_counters()
{
    dout(5) << dendl;
    ceph_assert(ceph_mutex_is_locked_by_me(m_lock));
