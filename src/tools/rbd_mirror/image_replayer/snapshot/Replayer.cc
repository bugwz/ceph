...
template<typename I> std::pair<uint64_t, librbd::SnapInfo*> get_newest_mirror_snapshot(I* image_ctx)
{
    for (auto snap_info_it = image_ctx->snap_info.rbegin(); snap_info_it != image_ctx->snap_info.rend();
        ...
        auto mirror_ns = boost::get<cls::rbd::MirrorSnapshotNamespace>(&snap_ns);
...
template<typename I> void Replayer<I>::prune_non_primary_snapshot(uint64_t snap_id)
{
...

    {
        ...
        if (snap_info != nullptr) {
            ...
            ceph_assert(boost::get<cls::rbd::MirrorSnapshotNamespace>(&snap_namespace) != nullptr);
        }
    }
...
template<typename I> void Replayer<I>::create_non_primary_snapshot()
{
    ...
    if (m_local_snap_id_start > 0) {
        ...
        auto mirror_ns = boost::get<cls::rbd::MirrorSnapshotNamespace>(&local_snap_info_it->second.snap_namespace);
        ...
        for (auto& [remote_snap_id, remote_snap_info] : remote_image_ctx->snap_info) {
            ...
            // user snapshots
            if (boost::get<cls::rbd::UserSnapshotNamespace>(&remote_snap_info.snap_namespace) == nullptr) {
                continue;
            }
...
template<typename I> void Replayer<I>::handle_copy_image(int r)
{
...

    {
        ...
        if (g_snapshot_perf_counters) {
            g_snapshot_perf_counters->inc(l_rbd_mirror_snapshot_replay_bytes, m_snapshot_bytes);
            g_snapshot_perf_counters->inc(l_rbd_mirror_snapshot_replay_snapshots);
            g_snapshot_perf_counters->tinc(l_rbd_mirror_snapshot_replay_snapshots_time, duration);
        }
        if (m_perf_counters) {
...
template<typename I> void Replayer<I>::register_perf_counters()
{
    ...
    plb.add_u64_counter(l_rbd_mirror_snapshot_replay_snapshots, "snapshots", "Snapshots", "r", prio);
    plb.add_time_avg(l_rbd_mirror_snapshot_replay_snapshots_time, "snapshots_time", "Snapshots time", "rl", prio);
    plb.add_u64_counter(
        l_rbd_mirror_snapshot_replay_bytes, "replay_bytes", "Replayed data", "rb", prio, unit_t(UNIT_BYTES));
