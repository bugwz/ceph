...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::image_replayer::snapshot::util::" << __func__ << ": "
...
uint64_t compute_remote_snap_id(const ceph::shared_mutex& local_image_lock,
                                ...
                                uint64_t local_snap_id, const std::string& remote_mirror_uuid)
{
    ...
    for (auto snap_it = local_snap_infos.lower_bound(local_snap_id); snap_it != local_snap_infos.end(); ++snap_it) {
