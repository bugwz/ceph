...
template<typename ImageCtxT> class ApplyImageStateRequest
{
public:
    static ApplyImageStateRequest* create(const std::string& local_mirror_uuid, const std::string& remote_mirror_uuid,
                                          ...
                                          librbd::mirror::snapshot::ImageState image_state, Context* on_finish)
    {
        return new ApplyImageStateRequest(
            local_mirror_uuid, remote_mirror_uuid, local_image_ctx, remote_image_ctx, image_state, on_finish);
    }
