...
{
public:
    static Replayer* create(Threads<ImageCtxT>* threads, InstanceWatcher<ImageCtxT>* instance_watcher,
    ...
    {
        return new Replayer(
            threads, instance_watcher, local_mirror_uuid, pool_meta_cache, state_builder, replayer_listener);
    }

    Replayer(Threads<ImageCtxT>* threads, InstanceWatcher<ImageCtxT>* instance_watcher,
             const std::string& local_mirror_uuid, PoolMetaCache* pool_meta_cache,
             StateBuilder<ImageCtxT>* state_builder, ReplayerListener* replayer_listener);
    ~Replayer();
...
private:
    ...
    boost::accumulators::accumulator_set<uint64_t, boost::accumulators::stats<boost::accumulators::tag::rolling_mean>>
        m_bytes_per_snapshot{boost::accumulators::tag::rolling_window::window_size = 2};
    ...
    void handle_replay_complete(std::unique_lock<ceph::mutex>* locker, int r, const std::string& description);
