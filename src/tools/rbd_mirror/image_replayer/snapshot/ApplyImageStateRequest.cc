...
template<typename I>
ApplyImageStateRequest<I>::ApplyImageStateRequest(const std::string& local_mirror_uuid,
                                                  const std::string& remote_mirror_uuid, I* local_image_ctx,
                                                  I* remote_image_ctx, librbd::mirror::snapshot::ImageState image_state,
                                                  Context* on_finish)
...
template<typename I> void ApplyImageStateRequest<I>::send()
{
    rename_image();
}

template<typename I> void ApplyImageStateRequest<I>::rename_image()
{
    ...
    auto ctx =
        create_context_callback<ApplyImageStateRequest<I>, &ApplyImageStateRequest<I>::handle_rename_image>(this);
    m_local_image_ctx->operations->execute_rename(m_image_state.name, ctx);
}

template<typename I> void ApplyImageStateRequest<I>::handle_rename_image(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void ApplyImageStateRequest<I>::update_features()
{
    uint64_t feature_updates = 0UL;
    bool enabled = false;

    auto image_state_features = m_image_state.features & ~RBD_FEATURES_IMPLICIT_ENABLE;
    ...
    auto ctx =
        create_context_callback<ApplyImageStateRequest<I>, &ApplyImageStateRequest<I>::handle_update_features>(this);
    m_local_image_ctx->operations->execute_update_features(feature_updates, enabled, ctx, 0U);
}

template<typename I> void ApplyImageStateRequest<I>::handle_update_features(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void ApplyImageStateRequest<I>::get_image_meta()
{
    dout(15) << dendl;

    auto ctx =
        create_context_callback<ApplyImageStateRequest<I>, &ApplyImageStateRequest<I>::handle_get_image_meta>(this);
    auto req = librbd::image::GetMetadataRequest<I>::create(
        m_local_image_ctx->md_ctx, m_local_image_ctx->header_oid, true, "", "", 0U, &m_metadata, ctx);
    req->send();
}

template<typename I> void ApplyImageStateRequest<I>::handle_get_image_meta(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void ApplyImageStateRequest<I>::update_image_meta()
{
    ...
    auto aio_comp =
        create_rados_callback<ApplyImageStateRequest<I>, &ApplyImageStateRequest<I>::handle_update_image_meta>(this);
    int r = m_local_image_ctx->md_ctx.aio_operate(m_local_image_ctx->header_oid, aio_comp, &op);
    ceph_assert(r == 0);
    aio_comp->release();
}

template<typename I> void ApplyImageStateRequest<I>::handle_update_image_meta(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void ApplyImageStateRequest<I>::unprotect_snapshot()
{
    std::shared_lock image_locker{m_local_image_ctx->image_lock};
    ...
    for (; snap_it != m_local_image_ctx->snap_info.end(); ++snap_it) {
        ...
        auto user_ns = std::get_if<cls::rbd::UserSnapshotNamespace>(&snap_info.snap_namespace);
        if (user_ns == nullptr) {
    ...
    std::shared_lock owner_locker{m_local_image_ctx->owner_lock};
    auto ctx =
        create_context_callback<ApplyImageStateRequest<I>, &ApplyImageStateRequest<I>::handle_unprotect_snapshot>(this);
    m_local_image_ctx->operations->execute_snap_unprotect(cls::rbd::UserSnapshotNamespace{}, m_snap_name.c_str(), ctx);
}

template<typename I> void ApplyImageStateRequest<I>::handle_unprotect_snapshot(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void ApplyImageStateRequest<I>::remove_snapshot()
{
    std::shared_lock image_locker{m_local_image_ctx->image_lock};
    ...
    std::shared_lock owner_locker{m_local_image_ctx->owner_lock};
    auto ctx =
        create_context_callback<ApplyImageStateRequest<I>, &ApplyImageStateRequest<I>::handle_remove_snapshot>(this);
    m_local_image_ctx->operations->execute_snap_remove(cls::rbd::UserSnapshotNamespace{}, m_snap_name.c_str(), ctx);
}

template<typename I> void ApplyImageStateRequest<I>::handle_remove_snapshot(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void ApplyImageStateRequest<I>::protect_snapshot()
{
    std::shared_lock image_locker{m_local_image_ctx->image_lock};
    ...
    std::shared_lock owner_locker{m_local_image_ctx->owner_lock};
    auto ctx =
        create_context_callback<ApplyImageStateRequest<I>, &ApplyImageStateRequest<I>::handle_protect_snapshot>(this);
    m_local_image_ctx->operations->execute_snap_protect(cls::rbd::UserSnapshotNamespace{}, m_snap_name.c_str(), ctx);
}

template<typename I> void ApplyImageStateRequest<I>::handle_protect_snapshot(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void ApplyImageStateRequest<I>::rename_snapshot()
{
    std::shared_lock image_locker{m_local_image_ctx->image_lock};
    ...
    for (; snap_it != m_local_image_ctx->snap_info.end(); ++snap_it) {
        ...
        if (snap_info.name != snap_state.name) {
            dout(15) << "snapshot " << snap_id << " has been renamed from '" << snap_info.name << "' to '"
                     << snap_state.name << "'" << dendl;
    ...
    std::shared_lock owner_locker{m_local_image_ctx->owner_lock};
    auto ctx =
        create_context_callback<ApplyImageStateRequest<I>, &ApplyImageStateRequest<I>::handle_rename_snapshot>(this);
    m_local_image_ctx->operations->execute_snap_rename(m_prev_snap_id, m_snap_name.c_str(), ctx);
}

template<typename I> void ApplyImageStateRequest<I>::handle_rename_snapshot(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void ApplyImageStateRequest<I>::set_snapshot_limit()
{
    ...
    std::shared_lock owner_locker{m_local_image_ctx->owner_lock};
    auto ctx =
        create_context_callback<ApplyImageStateRequest<I>, &ApplyImageStateRequest<I>::handle_set_snapshot_limit>(this);
    m_local_image_ctx->operations->execute_snap_set_limit(m_image_state.snap_limit, ctx);
}

template<typename I> void ApplyImageStateRequest<I>::handle_set_snapshot_limit(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> void ApplyImageStateRequest<I>::finish(int r)
{
    dout(15) << "r=" << r << dendl;
...
template<typename I> uint64_t ApplyImageStateRequest<I>::compute_remote_snap_id(uint64_t local_snap_id)
{
    ...
    auto remote_snap_id = util::compute_remote_snap_id(
        m_local_image_ctx->image_lock, m_local_image_ctx->snap_info, local_snap_id, m_remote_mirror_uuid);
    ...
    // to our snapshot
    for (auto snap_it = m_remote_image_ctx->snap_info.begin(); snap_it != m_remote_image_ctx->snap_info.end();
         ++snap_it) {
        auto snap_id = snap_it->first;
        auto mirror_ns = std::get_if<cls::rbd::MirrorSnapshotNamespace>(&snap_it->second.snap_namespace);
...
template<typename I> void ApplyImageStateRequest<I>::compute_local_to_remote_snap_ids()
{
    ceph_assert(ceph_mutex_is_locked(m_local_image_ctx->image_lock));
