...
template<typename I> void ApplyImageStateRequest<I>::unprotect_snapshot()
{
    ...
    for (; snap_it != m_local_image_ctx->snap_info.end(); ++snap_it) {
        ...
        auto user_ns = boost::get<cls::rbd::UserSnapshotNamespace>(&snap_info.snap_namespace);
        if (user_ns == nullptr) {
...
template<typename I> uint64_t ApplyImageStateRequest<I>::compute_remote_snap_id(uint64_t local_snap_id)
{
    ...
    for (auto snap_it = m_remote_image_ctx->snap_info.begin(); snap_it != m_remote_image_ctx->snap_info.end();
         ++snap_it) {
        auto snap_id = snap_it->first;
        auto mirror_ns = boost::get<cls::rbd::MirrorSnapshotNamespace>(&snap_it->second.snap_namespace);
