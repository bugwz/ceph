...
namespace librbd {
template<typename> class DeepCopyRequest;
}
...
template<typename ImageCtxT = librbd::ImageCtx> class ImageSync : public CancelableRequest
{
public:
    static ImageSync* create(Threads<ImageCtxT>* threads, ImageCtxT* local_image_ctx, ImageCtxT* remote_image_ctx,
                             const std::string& local_mirror_uuid, image_sync::SyncPointHandler* sync_point_handler,
                             InstanceWatcher<ImageCtxT>* instance_watcher, ProgressContext* progress_ctx,
                             Context* on_finish)
    {
        return new ImageSync(threads,
                             local_image_ctx,
                             remote_image_ctx,
                             local_mirror_uuid,
                             sync_point_handler,
    ...
    ImageSync(Threads<ImageCtxT>* threads, ImageCtxT* local_image_ctx, ImageCtxT* remote_image_ctx,
              const std::string& local_mirror_uuid, image_sync::SyncPointHandler* sync_point_handler,
              InstanceWatcher<ImageCtxT>* instance_watcher, ProgressContext* progress_ctx, Context* on_finish);
    ~ImageSync() override;
...
private:
    ...
    void handle_copy_image_update_progress(uint64_t object_no, uint64_t object_count);
