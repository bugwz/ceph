...
template<typename I> struct ImageMap<I>::C_NotifyInstance : public Context
{
    ImageMap* image_map;
    std::string global_image_id;
    bool acquire_release;

    C_NotifyInstance(ImageMap* image_map, const std::string& global_image_id, bool acquire_release)
        : image_map(image_map)
...
template<typename I>
ImageMap<I>::ImageMap(librados::IoCtx& ioctx, Threads<I>* threads, const std::string& instance_id,
                      image_map::Listener& listener)
    : m_ioctx(ioctx)
    , m_threads(threads)
    , m_instance_id(instance_id)
    , m_listener(listener)
    , m_lock(ceph::make_mutex(unique_lock_name("rbd::mirror::ImageMap::m_lock", this)))
{}

template<typename I> ImageMap<I>::~ImageMap()
{
...
template<typename I> void ImageMap<I>::continue_action(const std::set<std::string>& global_image_ids, int r)
{
    dout(20) << dendl;
...
template<typename I>
void ImageMap<I>::handle_update_request(const Updates& updates, const std::set<std::string>& remove_global_image_ids,
                                        int r)
{
    dout(20) << "r=" << r << dendl;
...
template<typename I> void ImageMap<I>::update_image_mapping(Updates&& map_updates, std::set<std::string>&& map_removals)
{
    ...
    Context* on_finish = new LambdaContext([this, map_updates, map_removals](int r) {
    ...
    start_async_op();
    image_map::UpdateRequest<I>* req =
        image_map::UpdateRequest<I>::create(m_ioctx, std::move(update_mapping), std::move(map_removals), on_finish);
    req->send();
}

template<typename I> void ImageMap<I>::process_updates()
{
    dout(20) << dendl;
    ...
    for (auto const& global_image_id : m_global_image_ids) {
        ...
        switch (action_type) {
        case image_map::ACTION_TYPE_NONE: continue;
        case image_map::ACTION_TYPE_MAP_UPDATE:
            ceph_assert(info.instance_id != image_map::UNMAPPED_INSTANCE_ID);
            map_updates.emplace_back(global_image_id, info.instance_id, info.mapped_time);
            break;
        case image_map::ACTION_TYPE_MAP_REMOVE: map_removals.emplace(global_image_id); break;
...
template<typename I> void ImageMap<I>::schedule_update_task()
{
    std::lock_guard timer_lock{m_threads->timer_lock};
    schedule_update_task(m_threads->timer_lock);
}

template<typename I> void ImageMap<I>::schedule_update_task(const ceph::mutex& timer_lock)
{
    ceph_assert(ceph_mutex_is_locked(m_threads->timer_lock));
...
template<typename I> void ImageMap<I>::rebalance()
{
    ceph_assert(m_rebalance_task == nullptr);
...
template<typename I> void ImageMap<I>::schedule_rebalance_task()
{
    ceph_assert(ceph_mutex_is_locked(m_threads->timer_lock));
    ...
    double resched_after = cct->_conf.get_val<double>("rbd_mirror_image_policy_rebalance_timeout");
    if (!resched_after) {
        return;
    }
...
template<typename I> void ImageMap<I>::schedule_action(const std::string& global_image_id)
{
...
template<typename I>
void ImageMap<I>::notify_listener_acquire_release_images(const Updates& acquire, const Updates& release)
{
    ...
    for (auto const& update : acquire) {
        m_listener.acquire_image(update.global_image_id,
                                 update.instance_id,
                                 create_async_context_callback(
                                     m_threads->work_queue, new C_NotifyInstance(this, update.global_image_id, true)));
    }

    for (auto const& update : release) {
        m_listener.release_image(update.global_image_id,
                                 update.instance_id,
                                 create_async_context_callback(
                                     m_threads->work_queue, new C_NotifyInstance(this, update.global_image_id, true)));
    }
...
template<typename I>
void ImageMap<I>::notify_listener_remove_images(const std::string& peer_uuid, const Updates& remove)
{
    dout(5) << "peer_uuid=" << peer_uuid << ", "
            << "remove=[" << remove << "]" << dendl;

    for (auto const& update : remove) {
        m_listener.remove_image(peer_uuid,
                                update.global_image_id,
                                update.instance_id,
                                create_async_context_callback(
                                    m_threads->work_queue, new C_NotifyInstance(this, update.global_image_id, false)));
    }
}

template<typename I> void ImageMap<I>::handle_load(const std::map<std::string, cls::rbd::MirrorImageMap>& image_mapping)
{
    dout(20) << dendl;
...
template<typename I> void ImageMap<I>::handle_peer_ack_remove(const std::string& global_image_id, int r)
{
    std::lock_guard locker{m_lock};
...
template<typename I>
void ImageMap<I>::update_images_added(const std::string& peer_uuid, const std::set<std::string>& global_image_ids)
{
    dout(5) << "peer_uuid=" << peer_uuid << ", "
...
template<typename I>
void ImageMap<I>::update_images_removed(const std::string& peer_uuid, const std::set<std::string>& global_image_ids)
{
    dout(5) << "peer_uuid=" << peer_uuid << ", "
            << "global_image_ids=[" << global_image_ids << "]" << dendl;
    ceph_assert(ceph_mutex_is_locked(m_lock));
...
template<typename I> void ImageMap<I>::update_instances_added(const std::vector<std::string>& instance_ids)
{
    {
        std::lock_guard locker{m_lock};
        if (m_shutting_down) {
...
template<typename I> void ImageMap<I>::update_instances_removed(const std::vector<std::string>& instance_ids)
{
    {
        std::lock_guard locker{m_lock};
        if (m_shutting_down) {
...
template<typename I>
void ImageMap<I>::update_images(const std::string& peer_uuid, std::set<std::string>&& added_global_image_ids,
...
template<typename I> void ImageMap<I>::handle_peer_ack(const std::string& global_image_id, int r)
{
...
template<typename I> void ImageMap<I>::init(Context* on_finish)
{
    dout(20) << dendl;
    ...
    image_map::LoadRequest<I>* req = image_map::LoadRequest<I>::create(m_ioctx, &ctx->image_mapping, ctx);
    req->send();
}

template<typename I> void ImageMap<I>::shut_down(Context* on_finish)
{
    dout(20) << dendl;
...
template<typename I>
void ImageMap<I>::filter_instance_ids(const std::vector<std::string>& instance_ids,
                                      std::vector<std::string>* filtered_instance_ids, bool removal) const
{
    ...
    else if (std::find(instance_ids.begin(), instance_ids.end(), m_instance_id) != instance_ids.end()) {
