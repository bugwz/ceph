...
template<typename I> std::ostream& operator<<(std::ostream& os, const typename ImageReplayer<I>::State& state);

namespace {

template<typename I> class ImageReplayerAdminSocketCommand
{
public:
    ImageReplayerAdminSocketCommand(const std::string& desc, ImageReplayer<I>* replayer)
        : desc(desc)
        , replayer(replayer)
    {}
...
template<typename I> class StatusCommand : public ImageReplayerAdminSocketCommand<I>
{
public:
...
template<typename I> class StartCommand : public ImageReplayerAdminSocketCommand<I>
{
public:
...
template<typename I> class StopCommand : public ImageReplayerAdminSocketCommand<I>
{
public:
...
template<typename I> class RestartCommand : public ImageReplayerAdminSocketCommand<I>
{
public:
...
template<typename I> class FlushCommand : public ImageReplayerAdminSocketCommand<I>
{
public:
...
template<typename I> class ImageReplayerAdminSocketHook : public AdminSocketHook
{
public:
    ImageReplayerAdminSocketHook(CephContext* cct, const std::string& name, ImageReplayer<I>* replayer)
        : admin_socket(cct->get_admin_socket())
        , commands{{"rbd mirror flush " + name, new FlushCommand<I>("flush rbd mirror " + name, replayer)},
                   {"rbd mirror restart " + name, new RestartCommand<I>("restart rbd mirror " + name, replayer)},
                   {"rbd mirror start " + name, new StartCommand<I>("start rbd mirror " + name, replayer)},
                   {"rbd mirror status " + name, new StatusCommand<I>("get status for rbd mirror " + name, replayer)},
                   {"rbd mirror stop " + name, new StopCommand<I>("stop rbd mirror " + name, replayer)}}
    {}

    int register_commands()
    {
        for (auto& it : commands) {
            int r = admin_socket->register_command(it.first, this, it.second->desc);
            if (r < 0) {
                return r;
            }
...
private:
    typedef std::map<std::string, ImageReplayerAdminSocketCommand<I>*, std::less<>> Commands;
...
template<typename I>
void ImageReplayer<I>::BootstrapProgressContext::update_progress(const std::string& description, bool flush)
{
...
template<typename I> struct ImageReplayer<I>::ReplayerListener : public image_replayer::ReplayerListener
{
    ...
    void handle_notification() override { image_replayer->handle_replayer_notification(); }
};

template<typename I>
ImageReplayer<I>::ImageReplayer(librados::IoCtx& local_io_ctx, const std::string& local_mirror_uuid,
                                const std::string& global_image_id, Threads<I>* threads,
                                InstanceWatcher<I>* instance_watcher, MirrorStatusUpdater<I>* local_status_updater,
                                journal::CacheManagerHandler* cache_manager_handler, PoolMetaCache* pool_meta_cache)
    ...
    , m_lock(ceph::make_mutex("rbd::mirror::ImageReplayer " + stringify(local_io_ctx.get_id()) + " " + global_image_id))
    , m_progress_cxt(this)
    , m_replayer_listener(new ReplayerListener(this))
{
    ...
    m_image_spec = image_replayer::util::compute_image_spec(local_io_ctx, global_image_id);
    register_admin_socket_hook();
}

template<typename I> ImageReplayer<I>::~ImageReplayer()
{
...
template<typename I> image_replayer::HealthState ImageReplayer<I>::get_health_state() const
{
    ...
    else if (*m_mirror_image_status_state == cls::rbd::MIRROR_IMAGE_STATUS_STATE_SYNCING ||
             *m_mirror_image_status_state == cls::rbd::MIRROR_IMAGE_STATUS_STATE_UNKNOWN) {
        return image_replayer::HEALTH_STATE_WARNING;
    }
    return image_replayer::HEALTH_STATE_ERROR;
}

template<typename I> void ImageReplayer<I>::add_peer(const Peer<I>& peer)
{
...
template<typename I> void ImageReplayer<I>::set_state_description(int r, const std::string& desc)
{
...
template<typename I> void ImageReplayer<I>::start(Context* on_finish, bool manual, bool restart)
{
...
template<typename I> void ImageReplayer<I>::bootstrap()
{
    dout(10) << dendl;
    ...
    ceph_assert(m_state_builder == nullptr);
    auto ctx = create_context_callback<ImageReplayer, &ImageReplayer<I>::handle_bootstrap>(this);
    auto request = image_replayer::BootstrapRequest<I>::create(m_threads,
                                                               m_local_io_ctx,
                                                               m_remote_image_peer.io_ctx,
                                                               m_instance_watcher,
                                                               m_global_image_id,
                                                               m_local_mirror_uuid,
                                                               m_remote_image_peer.remote_pool_meta,
...
template<typename I> void ImageReplayer<I>::handle_bootstrap(int r)
{
    dout(10) << "r=" << r << dendl;
    {
        std::lock_guard locker{m_lock};
...
template<typename I> void ImageReplayer<I>::start_replay()
{
    dout(10) << dendl;

    std::unique_lock locker{m_lock};
    ceph_assert(m_replayer == nullptr);
    m_replayer = m_state_builder->create_replayer(
        m_threads, m_instance_watcher, m_local_mirror_uuid, m_pool_meta_cache, m_replayer_listener);

    auto ctx = create_context_callback<ImageReplayer<I>, &ImageReplayer<I>::handle_start_replay>(this);
    m_replayer->init(ctx);
}

template<typename I> void ImageReplayer<I>::handle_start_replay(int r)
{
    dout(10) << "r=" << r << dendl;
...
template<typename I> void ImageReplayer<I>::on_start_fail(int r, const std::string& desc)
{
    dout(10) << "r=" << r << ", desc=" << desc << dendl;
...
template<typename I> bool ImageReplayer<I>::on_start_interrupted()
{
    std::lock_guard locker{m_lock};
    return on_start_interrupted(m_lock);
}

template<typename I> bool ImageReplayer<I>::on_start_interrupted(ceph::mutex& lock)
{
...
template<typename I> void ImageReplayer<I>::stop(Context* on_finish, bool manual, bool restart)
{
...
template<typename I> void ImageReplayer<I>::on_stop_journal_replay(int r, const std::string& desc)
{
    dout(10) << dendl;
...
template<typename I> void ImageReplayer<I>::restart(Context* on_finish)
{
    {
        std::lock_guard locker{m_lock};
...
template<typename I> void ImageReplayer<I>::flush()
{
    C_SaferCond ctx;
...
template<typename I> bool ImageReplayer<I>::on_replay_interrupted()
{
    bool shut_down;
    {
        std::lock_guard locker{m_lock};
...
template<typename I> void ImageReplayer<I>::print_status(Formatter* f)
{
    dout(10) << dendl;
...
template<typename I> void ImageReplayer<I>::schedule_update_mirror_image_replay_status()
{
    ...
    m_update_status_task =
        create_context_callback<ImageReplayer<I>, &ImageReplayer<I>::handle_update_mirror_image_replay_status>(this);
    m_threads->timer->add_event_after(10, m_update_status_task);
}

template<typename I> void ImageReplayer<I>::handle_update_mirror_image_replay_status(int r)
{
    dout(10) << dendl;
...
template<typename I> void ImageReplayer<I>::cancel_update_mirror_image_replay_status()
{
    std::unique_lock timer_locker{m_threads->timer_lock};
...
template<typename I> void ImageReplayer<I>::update_mirror_image_status(bool force, const OptionalState& opt_state)
{
    dout(15) << "force=" << force << ", "
    ...
    m_in_flight_op_tracker.start_op();
    auto ctx = new LambdaContext([this, force, opt_state](int r) { set_mirror_image_status_update(force, opt_state); });
    m_threads->work_queue->queue(ctx, 0);
}

template<typename I> void ImageReplayer<I>::set_mirror_image_status_update(bool force, const OptionalState& opt_state)
{
    dout(15) << "force=" << force << ", "
             << "state=" << opt_state << dendl;
    ...
    auto mirror_image_status_state = boost::make_optional(false, cls::rbd::MIRROR_IMAGE_STATUS_STATE_UNKNOWN);
    image_replayer::BootstrapRequest<I>* bootstrap_request = nullptr;
    {
        std::lock_guard locker{m_lock};
        state = m_state;
    ...
    case STATE_REPLAYING:
        ...
        {
            std::string desc;
            auto on_req_finish = new LambdaContext([this, force](int r) {
            ...
            mirror_image_status_state = boost::make_optional(false, cls::rbd::MIRROR_IMAGE_STATUS_STATE_UNKNOWN);
        }
        break;
    case STATE_STOPPING:
        if (stopping_replay) {
    ...
    case STATE_STOPPED:
        ...
        break;
    default: ceph_assert(!"invalid state");
    }
    ...
    if (mirror_image_status_state && *mirror_image_status_state == cls::rbd::MIRROR_IMAGE_STATUS_STATE_ERROR) {
        status.state = *mirror_image_status_state;
    }

    dout(15) << "status=" << status << dendl;
    m_local_status_updater->set_mirror_image_status(m_global_image_id, status, force);
    if (m_remote_image_peer.mirror_status_updater != nullptr) {
        m_remote_image_peer.mirror_status_updater->set_mirror_image_status(m_global_image_id, status, force);
    }

    m_in_flight_op_tracker.finish_op();
}

template<typename I> void ImageReplayer<I>::shut_down(int r)
{
    dout(10) << "r=" << r << dendl;
    ...
    if (!m_in_flight_op_tracker.empty()) {
        ...
        m_in_flight_op_tracker.wait_for_ops(new LambdaContext([this, r](int) { shut_down(r); }));
        return;
    }

    // chain the shut down sequence (reverse order)
    Context* ctx = new LambdaContext([this, r](int _r) {
    ...
    if (m_state_builder != nullptr) {
        ctx = new LambdaContext([this, ctx](int r) { m_state_builder->close(ctx); });
    }
    ...
    if (m_replayer != nullptr) {
        ...
        ctx = new LambdaContext([this, ctx](int r) { m_replayer->shut_down(ctx); });
    }

    m_threads->work_queue->queue(ctx, 0);
}

template<typename I> void ImageReplayer<I>::handle_shut_down(int r)
{
    ...
    bool unregister_asok_hook = false;
    {
        ...
        if (m_delete_requested && m_state_builder != nullptr && !m_state_builder->local_image_id.empty()) {
        ...
        if (!delete_requested && !resync_requested && m_last_r == -ENOENT &&
            ((m_state_builder == nullptr) ||
             (m_state_builder->local_image_id.empty() && m_state_builder->remote_image_id.empty()))) {
    ...
    if (delete_requested || resync_requested) {
        ...
        auto ctx = new LambdaContext([this, r](int) { handle_shut_down(r); });
        ImageDeleter<I>::trash_move(m_local_io_ctx, m_global_image_id, resync_requested, m_threads->work_queue, ctx);
        return;
    }

    if (!m_in_flight_op_tracker.empty()) {
        dout(15) << "waiting for in-flight operations to complete" << dendl;
        m_in_flight_op_tracker.wait_for_ops(new LambdaContext([this, r](int) { handle_shut_down(r); }));
        return;
    }
...
template<typename I> void ImageReplayer<I>::handle_replayer_notification()
{
    dout(10) << dendl;
...

    {
        ...
        ceph_assert(m_state_builder != nullptr && m_state_builder->local_image_ctx != nullptr);
...
template<typename I> std::string ImageReplayer<I>::to_string(const State state)
{
    switch (state) {
    case ImageReplayer<I>::STATE_STARTING: return "Starting";
    case ImageReplayer<I>::STATE_REPLAYING: return "Replaying";
    case ImageReplayer<I>::STATE_STOPPING: return "Stopping";
    case ImageReplayer<I>::STATE_STOPPED: return "Stopped";
    default: break;
    }
    return "Unknown(" + stringify(state) + ")";
}

template<typename I> void ImageReplayer<I>::register_admin_socket_hook()
{
    ImageReplayerAdminSocketHook<I>* asok_hook;
    {
        ...
        asok_hook = new ImageReplayerAdminSocketHook<I>(g_ceph_context, m_image_spec, this);
...
template<typename I> void ImageReplayer<I>::unregister_admin_socket_hook()
{
    dout(15) << dendl;
...
template<typename I> void ImageReplayer<I>::reregister_admin_socket_hook()
{
    std::unique_lock locker{m_lock};
    ...
    auto image_spec = image_replayer::util::compute_image_spec(m_local_io_ctx, m_local_image_name);
...
template<typename I> void ImageReplayer<I>::remove_image_status(bool force, Context* on_finish)
{
    auto ctx = new LambdaContext([this, force, on_finish](int) { remove_image_status_remote(force, on_finish); });

    if (m_local_status_updater->exists(m_global_image_id)) {
        dout(15) << "removing local mirror image status" << dendl;
        if (force) {
            m_local_status_updater->remove_mirror_image_status(m_global_image_id, true, ctx);
        }
        else {
            m_local_status_updater->remove_refresh_mirror_image_status(m_global_image_id, ctx);
        }
        return;
    }
...
template<typename I> void ImageReplayer<I>::remove_image_status_remote(bool force, Context* on_finish)
{
    if (m_remote_image_peer.mirror_status_updater != nullptr &&
        m_remote_image_peer.mirror_status_updater->exists(m_global_image_id)) {
        dout(15) << "removing remote mirror image status" << dendl;
        if (force) {
            m_remote_image_peer.mirror_status_updater->remove_mirror_image_status(m_global_image_id, true, on_finish);
        }
        else {
            m_remote_image_peer.mirror_status_updater->remove_refresh_mirror_image_status(m_global_image_id, on_finish);
        }
        return;
    }
    if (on_finish) {
...
template<typename I> std::ostream& operator<<(std::ostream& os, const ImageReplayer<I>& replayer)
{
