...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::ClusterWatcher:" << this << " " << __func__ << ": "
...
ClusterWatcher::ClusterWatcher(RadosRef cluster, ceph::mutex& lock, ServiceDaemon<librbd::ImageCtx>* service_daemon)
    : m_cluster(cluster)
    , m_lock(lock)
    ...
    for (auto& kv : pools) {
        ...
        if (r == -EPERM) {
            ...
            m_service_pools[pool_id] = m_service_daemon->add_or_update_callout(
                pool_id, m_service_pools[pool_id], service_daemon::CALLOUT_LEVEL_WARNING, "access denied");
            continue;
        }
        else if (r < 0) {
            derr << "could not tell whether mirroring was enabled for " << pool_name << " : " << cpp_strerror(r)
                 << dendl;
            m_service_pools[pool_id] = m_service_daemon->add_or_update_callout(pool_id,
                                                                               m_service_pools[pool_id],
        ...
        if (r < 0) {
            ...
            m_service_pools[pool_id] = m_service_daemon->add_or_update_callout(
                pool_id, m_service_pools[pool_id], service_daemon::CALLOUT_LEVEL_ERROR, "mirroring peer list failed");
            continue;
        }
...
int ClusterWatcher::resolve_peer_site_config_keys(int64_t pool_id, const std::string& pool_name, PeerSpec* peer)
{
