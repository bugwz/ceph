...
struct ImageId
{
    ...
    inline bool operator==(const ImageId& rhs) const { return (global_id == rhs.global_id && id == rhs.id); }
    inline bool operator<(const ImageId& rhs) const { return global_id < rhs.global_id; }
};
...
std::ostream& operator<<(std::ostream& lhs, const LocalPoolMeta& local_pool_meta);

struct RemotePoolMeta
{
    RemotePoolMeta() {}
    RemotePoolMeta(const std::string& mirror_uuid, const std::string& mirror_peer_uuid)
        : mirror_uuid(mirror_uuid)
...
std::ostream& operator<<(std::ostream& lhs, const RemotePoolMeta& remote_pool_meta);

template<typename I> struct Peer
{
    std::string uuid;
    ...
    Peer() {}
    Peer(const std::string& uuid, librados::IoCtx& io_ctx, const RemotePoolMeta& remote_pool_meta,
    ...
    inline bool operator<(const Peer& rhs) const { return uuid < rhs.uuid; }
};

template<typename I> std::ostream& operator<<(std::ostream& lhs, const Peer<I>& peer)
{
...
{
    PeerSpec() = default;
    PeerSpec(const std::string& uuid, const std::string& cluster_name, const std::string& client_name)
        : uuid(uuid)
    ...
    {
        return (uuid == rhs.uuid && cluster_name == rhs.cluster_name && client_name == rhs.client_name &&
                mon_host == rhs.mon_host && key == rhs.key);
    }
    bool operator<(const PeerSpec& rhs) const
    {
        if (uuid != rhs.uuid) {
