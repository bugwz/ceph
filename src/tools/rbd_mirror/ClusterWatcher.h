...
public:
    ...
    {
        bool operator()(const PeerSpec& lhs, const PeerSpec& rhs) const { return (lhs.uuid < rhs.uuid); }
    };
...
private:
    ...
    int resolve_peer_site_config_keys(int64_t pool_id, const std::string& pool_name, PeerSpec* peer);
};
