...
{
    if (mirror) mirror->handle_signal(signum);
}
...
int main(int argc, const char** argv)
{
    ...
    auto cct = global_init(
        nullptr, args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_DAEMON, CINIT_FLAG_UNPRIVILEGED_DAEMON_DEFAULTS);
    ...
    auto prio = g_ceph_context->_conf.get_val<int64_t>("rbd_mirror_perf_stats_prio");
    {
        PerfCountersBuilder plb(g_ceph_context,
                                "rbd_mirror",
                                rbd::mirror::l_rbd_mirror_journal_first,
                                rbd::mirror::l_rbd_mirror_journal_last);
        plb.add_u64_counter(rbd::mirror::l_rbd_mirror_replay, "replay", "Replays", "r", prio);
        plb.add_u64_counter(
            rbd::mirror::l_rbd_mirror_replay_bytes, "replay_bytes", "Replayed data", "rb", prio, unit_t(UNIT_BYTES));
        plb.add_time_avg(rbd::mirror::l_rbd_mirror_replay_latency, "replay_latency", "Replay latency", "rl", prio);
    ...
    }
    {
        ...
        plb.add_u64_counter(rbd::mirror::l_rbd_mirror_snapshot_replay_snapshots, "snapshots", "Snapshots", "r", prio);
        plb.add_time_avg(
            rbd::mirror::l_rbd_mirror_snapshot_replay_snapshots_time, "snapshots_time", "Snapshots time", "rl", prio);
