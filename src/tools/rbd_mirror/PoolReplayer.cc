...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::PoolReplayer: " << this << " " << __func__ << ": "
...
template<typename I> class PoolReplayerAdminSocketCommand
{
public:
...
template<typename I> class StatusCommand : public PoolReplayerAdminSocketCommand<I>
{
public:
...
template<typename I> class StartCommand : public PoolReplayerAdminSocketCommand<I>
{
public:
...
template<typename I> class StopCommand : public PoolReplayerAdminSocketCommand<I>
{
public:
...
template<typename I> class RestartCommand : public PoolReplayerAdminSocketCommand<I>
{
public:
...
template<typename I> class FlushCommand : public PoolReplayerAdminSocketCommand<I>
{
public:
...
template<typename I> class LeaderReleaseCommand : public PoolReplayerAdminSocketCommand<I>
{
public:
...
template<typename I> class PoolReplayerAdminSocketHook : public AdminSocketHook
{
public:
    PoolReplayerAdminSocketHook(CephContext* cct, const std::string& name, PoolReplayer<I>* pool_replayer)
        : admin_socket(cct->get_admin_socket())
    {
        ...
        r = admin_socket->register_command(command, this, "get status for rbd mirror " + name);
        if (r == 0) {
            commands[command] = new StatusCommand<I>(pool_replayer);
        }

        command = "rbd mirror start " + name;
        r = admin_socket->register_command(command, this, "start rbd mirror " + name);
        if (r == 0) {
            commands[command] = new StartCommand<I>(pool_replayer);
        }

        command = "rbd mirror stop " + name;
        r = admin_socket->register_command(command, this, "stop rbd mirror " + name);
        if (r == 0) {
            commands[command] = new StopCommand<I>(pool_replayer);
        }

        command = "rbd mirror restart " + name;
        r = admin_socket->register_command(command, this, "restart rbd mirror " + name);
        if (r == 0) {
            commands[command] = new RestartCommand<I>(pool_replayer);
        }

        command = "rbd mirror flush " + name;
        r = admin_socket->register_command(command, this, "flush rbd mirror " + name);
        if (r == 0) {
            commands[command] = new FlushCommand<I>(pool_replayer);
        }

        command = "rbd mirror leader release " + name;
        r = admin_socket->register_command(command, this, "release rbd mirror leader " + name);
        if (r == 0) {
    ...
    int call(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, std::ostream& ss, bufferlist& out) override
    {
...
private:
    typedef std::map<std::string, PoolReplayerAdminSocketCommand<I>*, std::less<>> Commands;
...
template<typename I> struct PoolReplayer<I>::RemotePoolPollerListener : public remote_pool_poller::Listener
{
...
template<typename I>
PoolReplayer<I>::PoolReplayer(Threads<I>* threads, ServiceDaemon<I>* service_daemon,
                              journal::CacheManagerHandler* cache_manager_handler, PoolMetaCache* pool_meta_cache,
                              int64_t local_pool_id, const PeerSpec& peer, const std::vector<const char*>& args)
    : m_threads(threads)
...
template<typename I> PoolReplayer<I>::~PoolReplayer()
{
    shut_down();
...
template<typename I> bool PoolReplayer<I>::is_blocklisted() const
{
    std::lock_guard locker{m_lock};
    return m_blocklisted;
}

template<typename I> bool PoolReplayer<I>::is_leader() const
{
    std::lock_guard locker{m_lock};
    return m_leader_watcher && m_leader_watcher->is_leader();
}

template<typename I> bool PoolReplayer<I>::is_running() const
{
    return m_pool_replayer_thread.is_started() && !m_stopping;
}

template<typename I> void PoolReplayer<I>::init(const std::string& site_name)
{
    std::lock_guard locker{m_lock};
    ...
    if (r < 0) {
        m_callout_id = m_service_daemon->add_or_update_callout(
            m_local_pool_id, m_callout_id, service_daemon::CALLOUT_LEVEL_ERROR, "unable to connect to local cluster");
        return;
    }

    r = init_rados(m_peer.cluster_name,
                   m_peer.client_name,
                   m_peer.mon_host,
                   m_peer.key,
    ...
    if (r < 0) {
        m_callout_id = m_service_daemon->add_or_update_callout(
            m_local_pool_id, m_callout_id, service_daemon::CALLOUT_LEVEL_ERROR, "unable to connect to remote cluster");
        return;
    }
    ...
    r = librbd::cls_client::mirror_uuid_get(&m_local_io_ctx, &m_local_mirror_uuid);
    if (r < 0) {
        derr << "failed to retrieve local mirror uuid from pool " << m_local_io_ctx.get_pool_name() << ": "
             << cpp_strerror(r) << dendl;
        m_callout_id = m_service_daemon->add_or_update_callout(
            m_local_pool_id, m_callout_id, service_daemon::CALLOUT_LEVEL_ERROR, "unable to query local mirror uuid");
        return;
    }

    r = m_remote_rados->ioctx_create(m_local_io_ctx.get_pool_name().c_str(), m_remote_io_ctx);
    if (r < 0) {
        derr << "error accessing remote pool " << m_local_io_ctx.get_pool_name() << ": " << cpp_strerror(r) << dendl;
        m_callout_id = m_service_daemon->add_or_update_callout(
            m_local_pool_id, m_callout_id, service_daemon::CALLOUT_LEVEL_WARNING, "unable to access remote pool");
        return;
    }

    dout(10) << "connected to " << m_peer << dendl;

    m_image_sync_throttler.reset(Throttler<I>::create(cct, "rbd_mirror_concurrent_image_syncs"));

    m_image_deletion_throttler.reset(Throttler<I>::create(cct, "rbd_mirror_concurrent_image_deletions"));
    ...
    m_remote_pool_poller.reset(RemotePoolPoller<I>::create(
        m_threads, m_remote_io_ctx, m_site_name, m_local_mirror_uuid, *m_remote_pool_poller_listener));
    ...
    if (r < 0) {
        ...
        m_callout_id = m_service_daemon->add_or_update_callout(m_local_pool_id,
                                                               m_callout_id,
    ...
    m_pool_meta_cache->set_remote_pool_meta(m_remote_io_ctx.get_id(), m_remote_pool_meta);
    m_pool_meta_cache->set_local_pool_meta(m_local_io_ctx.get_id(), {m_local_mirror_uuid});
    ...
    m_default_namespace_replayer.reset(NamespaceReplayer<I>::create("",
                                                                    ...
                                                                    m_image_deletion_throttler.get(),
                                                                    m_service_daemon,
    ...
    m_leader_watcher.reset(LeaderWatcher<I>::create(m_threads, m_local_io_ctx, &m_leader_listener));
    ...
    m_service_daemon->add_or_update_attribute(
        m_local_io_ctx.get_id(), SERVICE_DAEMON_INSTANCE_ID_KEY, stringify(m_local_io_ctx.get_instance_id()));

    m_pool_replayer_thread.create("pool replayer");
}

template<typename I> void PoolReplayer<I>::shut_down()
{
    {
...
template<typename I>
int PoolReplayer<I>::init_rados(const std::string& cluster_name, const std::string& client_name,
                                const std::string& mon_host, const std::string& key, const std::string& description,
                                RadosRef* rados_ref, bool strip_cluster_overrides)
{
    ...
    CephContext* cct = common_preinit(iparams, CODE_ENVIRONMENT_LIBRARY, CINIT_FLAG_UNPRIVILEGED_DAEMON_DEFAULTS);
    ...
    if (strip_cluster_overrides) {
        ...
        for (auto& pair : config_values) {
            ...
            if (pair.second != value) {
                dout(0) << "reverting global config option override: " << pair.first << ": " << value << " -> "
                        << pair.second << dendl;
    ...
    if (!g_ceph_context->_conf->admin_socket.empty()) {
        cct->_conf.set_val_or_die("admin_socket", "$run_dir/$name.$pid.$cluster.$cctid.asok");
    }
...
template<typename I> void PoolReplayer<I>::run()
{
    dout(20) << dendl;

    while (true) {
        std::string asok_hook_name = m_local_io_ctx.get_pool_name() + " " + m_peer.cluster_name;
        if (m_asok_hook_name != asok_hook_name || m_asok_hook == nullptr) {
            m_asok_hook_name = asok_hook_name;
            delete m_asok_hook;

            m_asok_hook = new PoolReplayerAdminSocketHook<I>(g_ceph_context, m_asok_hook_name, this);
        }
        ...
        if (m_leader_watcher->is_blocklisted() || m_default_namespace_replayer->is_blocklisted()) {
            m_blocklisted = true;
            m_stopping = true;
        }
        ...
        auto seconds = g_ceph_context->_conf.get_val<uint64_t>("rbd_mirror_pool_replayers_refresh_interval");
...
template<typename I> void PoolReplayer<I>::update_namespace_replayers()
{
    dout(20) << dendl;
    ...
    for (auto it = m_namespace_replayers.begin(); it != m_namespace_replayers.end();) {
        auto iter = mirroring_namespaces.find(it->first);
        if (iter == mirroring_namespaces.end()) {
            auto namespace_replayer = it->second;
            auto on_shut_down = new LambdaContext([namespace_replayer, ctx = gather_ctx->new_sub()](int r) {
    ...
    for (auto& name : mirroring_namespaces) {
        auto namespace_replayer = NamespaceReplayer<I>::create(name,
                                                               m_local_io_ctx,
                                                               m_remote_io_ctx,
                                                               m_local_mirror_uuid,
                                                               m_peer.uuid,
        ...
        auto on_init = new LambdaContext(
            [this, namespace_replayer, name, &mirroring_namespaces, ctx = gather_ctx->new_sub()](int r) {
                std::lock_guard locker{m_lock};
                if (r < 0) {
                    derr << "failed to initialize namespace replayer for namespace " << name << ": " << cpp_strerror(r)
                         << dendl;
                    delete namespace_replayer;
...
template<typename I> int PoolReplayer<I>::list_mirroring_namespaces(std::set<std::string>* namespaces)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
...
template<typename I> void PoolReplayer<I>::reopen_logs()
{
    std::lock_guard locker{m_lock};
...
template<typename I>
void PoolReplayer<I>::namespace_replayer_acquire_leader(const std::string& name, Context* on_finish)
{
    ceph_assert(ceph_mutex_is_locked(m_lock));
    ...
    on_finish = new LambdaContext([this, name, on_finish](int r) {
        if (r < 0) {
            ...
            auto on_shut_down = new LambdaContext([namespace_replayer, on_finish](int r) {
                delete namespace_replayer;
...
template<typename I> void PoolReplayer<I>::print_status(Formatter* f)
{
    dout(20) << dendl;
    ...
    if (m_local_rados) {
        ...
        f->dump_string("local_cluster_admin_socket", cct->_conf.get_val<std::string>("admin_socket"));
    }
    if (m_remote_rados) {
        auto cct = reinterpret_cast<CephContext*>(m_remote_rados->cct());
        f->dump_string("remote_cluster_admin_socket", cct->_conf.get_val<std::string>("admin_socket"));
    }
...
template<typename I> void PoolReplayer<I>::start()
{
    dout(20) << dendl;
...
template<typename I> void PoolReplayer<I>::stop(bool manual)
{
...
template<typename I> void PoolReplayer<I>::restart()
{
    dout(20) << dendl;
...
template<typename I> void PoolReplayer<I>::flush()
{
    dout(20) << dendl;
...
template<typename I> void PoolReplayer<I>::release_leader()
{
    dout(20) << dendl;
...
template<typename I> void PoolReplayer<I>::handle_post_acquire_leader(Context* on_finish)
{
    dout(20) << dendl;
    ...
    with_namespace_replayers(
        [this](Context* on_finish) {
            ...
            m_service_daemon->add_or_update_attribute(m_local_pool_id, SERVICE_DAEMON_LEADER_KEY, true);
            ...
            m_default_namespace_replayer->handle_acquire_leader(gather_ctx->new_sub());
...
template<typename I> void PoolReplayer<I>::handle_pre_release_leader(Context* on_finish)
{
    dout(20) << dendl;
    ...
    with_namespace_replayers(
        [this](Context* on_finish) {
            ...
            m_default_namespace_replayer->handle_release_leader(gather_ctx->new_sub());
...
template<typename I> void PoolReplayer<I>::handle_update_leader(const std::string& leader_instance_id)
{
...
template<typename I> void PoolReplayer<I>::handle_instances_added(const std::vector<std::string>& instance_ids)
{
...
template<typename I> void PoolReplayer<I>::handle_instances_removed(const std::vector<std::string>& instance_ids)
{
    dout(5) << "instance_ids=" << instance_ids << dendl;
...
template<typename I> void PoolReplayer<I>::handle_remote_pool_meta_updated(const RemotePoolMeta& remote_pool_meta)
{
