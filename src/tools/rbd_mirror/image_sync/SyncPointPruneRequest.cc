...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::image_sync::SyncPointPruneRequest: " << this << " " << __func__
namespace rbd {
namespace mirror {
...
template<typename I>
SyncPointPruneRequest<I>::SyncPointPruneRequest(I* remote_image_ctx, bool sync_complete,
...
template<typename I> void SyncPointPruneRequest<I>::send()
{
    ...
    if (m_sync_complete) {
        ...
        ++it;
        if (it == m_sync_points_copy.end() || it->from_snap_name != sync_point.snap_name) {
        ...
        for (auto it = m_sync_points_copy.rbegin(); it != m_sync_points_copy.rend(); ++it) {
            auto& sync_point = *it;
            if (&sync_point == &m_sync_points_copy.front()) {
                if (m_remote_image_ctx->get_snap_id(cls::rbd::UserSnapshotNamespace(), sync_point.snap_name) ==
                    CEPH_NOSNAP) {
            ...
            if (!sync_point.from_snap_name.empty() && snap_names.count(sync_point.from_snap_name) == 0 &&
...
template<typename I> void SyncPointPruneRequest<I>::send_remove_snap()
{
    ...
    Context* ctx =
        create_context_callback<SyncPointPruneRequest<I>, &SyncPointPruneRequest<I>::handle_remove_snap>(this);
    m_remote_image_ctx->operations->snap_remove(cls::rbd::UserSnapshotNamespace(), snap_name.c_str(), ctx);
}

template<typename I> void SyncPointPruneRequest<I>::handle_remove_snap(int r)
{
    dout(20) << ": r=" << r << dendl;
...
template<typename I> void SyncPointPruneRequest<I>::send_refresh_image()
{
    dout(20) << dendl;

    Context* ctx =
        create_context_callback<SyncPointPruneRequest<I>, &SyncPointPruneRequest<I>::handle_refresh_image>(this);
    m_remote_image_ctx->state->refresh(ctx);
}

template<typename I> void SyncPointPruneRequest<I>::handle_refresh_image(int r)
{
    dout(20) << ": r=" << r << dendl;
...
template<typename I> void SyncPointPruneRequest<I>::send_update_sync_points()
{
    dout(20) << dendl;
    ...
    auto ctx =
        create_context_callback<SyncPointPruneRequest<I>, &SyncPointPruneRequest<I>::handle_update_sync_points>(this);
    m_sync_point_handler->update_sync_points(
        m_sync_point_handler->get_snap_seqs(), m_sync_points_copy, m_sync_complete, ctx);
}

template<typename I> void SyncPointPruneRequest<I>::handle_update_sync_points(int r)
{
    dout(20) << ": r=" << r << dendl;
...
template<typename I> void SyncPointPruneRequest<I>::finish(int r)
{
    dout(20) << ": r=" << r << dendl;
