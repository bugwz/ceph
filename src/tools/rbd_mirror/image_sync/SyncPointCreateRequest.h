...
template<typename ImageCtxT = librbd::ImageCtx> class SyncPointCreateRequest
{
public:
    static SyncPointCreateRequest* create(ImageCtxT* remote_image_ctx, const std::string& local_mirror_uuid,
                                          SyncPointHandler* sync_point_handler, Context* on_finish)
    {
        return new SyncPointCreateRequest(remote_image_ctx, local_mirror_uuid, sync_point_handler, on_finish);
    }

    SyncPointCreateRequest(ImageCtxT* remote_image_ctx, const std::string& local_mirror_uuid,
                           SyncPointHandler* sync_point_handler, Context* on_finish);
