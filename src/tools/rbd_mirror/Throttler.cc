...
#undef dout_prefix
#define dout_prefix *_dout << "rbd::mirror::Throttler:: " << this << " " << __func__ << ": "
...
Throttler<I>::Throttler(CephContext* cct, const std::string& config_key)
    ...
    , m_lock(ceph::make_mutex(librbd::util::unique_lock_name("rbd::mirror::Throttler", this)))
...
template<typename I> Throttler<I>::~Throttler()
{
...
template<typename I> void Throttler<I>::start_op(const std::string& ns, const std::string& id_, Context* on_start)
{
    Id id{ns, id_};
    ...
    int r = 0;
    {
        ...
        else if (m_max_concurrent_ops == 0 || m_inflight_ops.size() < m_max_concurrent_ops) {
            ceph_assert(m_queue.empty());
            m_inflight_ops.insert(id);
            dout(20) << "ready to start op for " << id << " [" << m_inflight_ops.size() << "/" << m_max_concurrent_ops
                     << "]" << dendl;
        }
        else {
...
template<typename I> bool Throttler<I>::cancel_op(const std::string& ns, const std::string& id_)
{
    Id id{ns, id_};
...
template<typename I> void Throttler<I>::finish_op(const std::string& ns, const std::string& id_)
{
    Id id{ns, id_};
...
template<typename I> void Throttler<I>::drain(const std::string& ns, int r)
{
    dout(20) << "ns=" << ns << dendl;
...
template<typename I> void Throttler<I>::set_max_concurrent_ops(uint32_t max)
{
    ...
    std::list<Context*> ops;
    {
        ...
        while ((m_max_concurrent_ops == 0 || m_inflight_ops.size() < m_max_concurrent_ops) && !m_queue.empty()) {
            auto id = m_queue.front();
            m_inflight_ops.insert(id);
            dout(20) << "ready to start op for " << id << " [" << m_inflight_ops.size() << "/" << m_max_concurrent_ops
                     << "]" << dendl;
...
template<typename I> void Throttler<I>::print_status(ceph::Formatter* f)
{
    dout(20) << dendl;
...
template<typename I> const char** Throttler<I>::get_tracked_conf_keys() const
{
    return m_config_keys;
}

template<typename I> void Throttler<I>::handle_conf_change(const ConfigProxy& conf, const set<string>& changed)
{
