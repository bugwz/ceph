...
struct object_begin
{
    ...
    explicit object_begin(const ghobject_t& hoid)
        : hoid(hoid)
    {}
    object_begin() {}
...
struct attr_section
{
    ...
    explicit attr_section(const map<string, bufferlist>& data)
        : data(data)
    {}
    explicit attr_section(map<string, bufferptr>& data_)
    {
        for (std::map<std::string, bufferptr>::iterator i = data_.begin(); i != data_.end(); ++i) {
            bufferlist bl;
...
{
    bufferlist hdr;
    explicit omap_hdr_section(bufferlist hdr)
        : hdr(hdr)
    {}
    omap_hdr_section() {}
...
struct metadata_section
{
    ...
    metadata_section(__u8 struct_ver, epoch_t map_epoch, const pg_info_t& info, const pg_log_t& log,
                     const PastIntervals& past_intervals, const pg_missing_t& missing)
...
public:
    ...
    template<typename T> int write_section(sectiontype_t type, const T& obj, int fd)
    {
        if (dry_run) return 0;
