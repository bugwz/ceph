...
static int do_rados_setxattr(rados_ioctx_t io_ctx, const char* oid, const char* key, const char* val)
{
...
static int do_rados_getxattr(rados_ioctx_t io_ctx, const char* oid, const char* key, const char* expected)
{
...
static int do_rados_getxattrs(rados_ioctx_t io_ctx, const char* oid, const char** exkeys, const char** exvals)
{
    ...
    while (1) {
        ...
        if (!key) break;
        for (i = 0; i < nval; ++i) {
            if (strcmp(exkeys[i], key)) continue;
...
static int testrados(void)
{
    ...
    // This should fail.
    if (!rados_conf_set(cl, "config option that doesn't exist", "some random value")) {
    ...
    printf("rados_read = %d\n", r);
    if (memcmp(buf, buf2, r)) printf("*** content mismatch ***\n");

    /* attrs */
    if (do_rados_setxattr(io_ctx, oid, "b", "2")) goto out_err_cleanup;
    if (do_rados_setxattr(io_ctx, oid, "a", "1")) goto out_err_cleanup;
    if (do_rados_setxattr(io_ctx, oid, "c", "3")) goto out_err_cleanup;
    if (do_rados_getxattr(io_ctx, oid, "a", "1")) goto out_err_cleanup;
    if (do_rados_getxattr(io_ctx, oid, "b", "2")) goto out_err_cleanup;
    if (do_rados_getxattr(io_ctx, oid, "c", "3")) goto out_err_cleanup;
    if (do_rados_getxattrs(io_ctx, oid, exkeys, exvals)) goto out_err_cleanup;
