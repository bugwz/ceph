...
void lock_mtab(void)
{
    ...
    snprintf(linktargetfile, sizeof(linktargetfile), MOUNTLOCK_LINKTARGET, getpid());
    ...
    while (!we_created_lockfile) {
        ...
        if (j == 0) we_created_lockfile = 1;

        if (j < 0 && errsv != EEXIST) {
            (void)unlink(linktargetfile);
            die(EX_FILEIO,
                "can't link lock file %s: %s "
                "(use -n flag to override)",
                _PATH_MOUNTED_LOCK,
                strerror(errsv));
        }
            ...
            if (now.tv_sec < maxtime.tv_sec) {
                ...
                if (fcntl(lockfile_fd, F_SETLKW, &flock) == -1) {
                    ...
                    die(EX_FILEIO,
                        ...
                        _PATH_MOUNTED_LOCK,
                        (errno == EINTR) ? "timed out" : strerror(errsv));
                }
                alarm(0);
...
static void update_mtab_entry(const char* spec, const char* node, const char* type, const char* opts, int flags,
                              int freq, int pass)
{
    struct statfs buf;
    int err = statfs(_PATH_MOUNTED, &buf);
    if (err) {
        printf("mount: can't statfs %s: %s", _PATH_MOUNTED, strerror(err));
        return;
    }
    /* /etc/mtab is symbol link to /proc/self/mounts? */
    if (buf.f_type == PROC_SUPER_MAGIC) return;
    ...
    if (fp == NULL) {
        ...
        if ((addmntent(fp, &mnt)) == 1) {
            int errsv = errno;
            printf("mount: error writing %s: %s", _PATH_MOUNTED, strerror(errsv));
        }
    }
    endmntent(fp);
    unlock_mtab();
