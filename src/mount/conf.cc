...
extern "C" void mount_ceph_get_config_info(const char* config_file, const char* name, bool v2_addrs,
                                           struct ceph_config_info* cci)
{
    ...
    for (const auto& mon : monc.monmap.addr_mons) {
         ...
         */
        if (v2_addrs) {
            if (!eaddr.is_msgr2()) continue;
        }
        else {
            if (!eaddr.is_legacy()) continue;
        }

        std::string addr = eaddr.ip_n_port_to_str();
        /* If this will overrun cci_mons, stop here */
        if (monaddrs.length() + 1 + addr.length() + 1 > sizeof(cci->cci_mons)) break;
