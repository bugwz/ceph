 ...
 */
static void append_opt(const char* key, const char* value, struct ceph_mount_info* cmi, int* pos)
{
    if (*pos != 0) *pos = safe_cat(&cmi->cmi_opts, &cmi->cmi_opts_len, *pos, ",");
...
static int remove_opt(struct ceph_mount_info* cmi, const char* key, char** value)
{
    ...
    if (!value_end) value_end = key_sep + strlen(key_sep);
 ...
 */
static int parse_old_dev(const char* dev_str, struct ceph_mount_info* cmi, int* opt_pos)
{
    size_t len;
    char* mount_path;
    ...
    if (!cmi->cmi_path) return -ENOMEM;
    if (!cmi->cmi_name) record_name(CEPH_AUTH_NAME_DEFAULT, cmi);
 ...
 */
static int parse_new_dev(const char* dev_str, struct ceph_mount_info* cmi, int* opt_pos)
{
    size_t len;
    char* name;
    char* name_end;
    ...
    if (!cmi->cmi_fsname) return -ENOMEM;
    ...
    if (!ms_mode_specified && !mon_addr_specified) append_opt("ms_mode", CEPH_DEFAULT_V2_MS_MODE, cmi, opt_pos);

    cmi->format = MOUNT_DEV_FORMAT_NEW;
    return 0;
}

static int parse_dev(const char* dev_str, struct ceph_mount_info* cmi, int* opt_pos)
{
    int ret;

    ret = parse_new_dev(dev_str, cmi, opt_pos);
    if (ret < 0 && ret != -ENODEV) return -EINVAL;
    if (ret) ret = parse_old_dev(dev_str, cmi, opt_pos);
    if (ret < 0) fprintf(stderr, "error parsing device string\n");
    return ret;
}
 ...
 */
static int finalize_src(struct ceph_mount_info* cmi, int* opt_pos, char** resolved_addr)
{
    char* src;
...
static int fetch_config_info(struct ceph_mount_info* cmi)
{
    ...
    if (cmi->cmi_secret[0] && cmi->cmi_mons && cmi->cmi_fsid) return 0;

    cci = mmap((void*)0, sizeof(*cci), PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_SHARED, -1, 0);
    if (cci == MAP_FAILED) {
    ...
    if (pid == 0) {
        ...
        if (!cmi->cmi_mons && cci->cci_mons[0]) {
            ...
            if (len < MON_LIST_BUFSIZE) cmi->cmi_mons = strndup(cci->cci_mons, len + 1);
        }
        if (!cmi->cmi_fsid) {
            len = strnlen(cci->cci_fsid, CLUSTER_FSID_LEN);
            if (len < CLUSTER_FSID_LEN) cmi->cmi_fsid = strndup(cci->cci_fsid, len + 1);
        }
    }
out:
 ...
 */
static int parse_options(const char* data, struct ceph_mount_info* cmi, int* opt_pos)
{
    char* next_keyword = NULL;
    char* name = NULL;

    if (data == EMPTY_STRING) goto out;
    ...
    do {
        ...
        if (next_keyword) *next_keyword++ = 0;
        ...
        if (strcmp(data, "ro") == 0) {
            ...
            if (len <= sizeof(cmi->cmi_secret)) memcpy(cmi->cmi_secret, value, len);
        }
        ...
        if (!skip) append_opt(data, value, cmi, opt_pos);
        data = next_keyword;
    } while (data);
...
out:
     ...
     */
    if (name) record_name(name, cmi);
    if (cmi->cmi_opts) mount_ceph_debug("mount.ceph: options \"%s\".\n", cmi->cmi_opts);

    if (!cmi->cmi_opts) {
        cmi->cmi_opts = strdup(EMPTY_STRING);
        if (!cmi->cmi_opts) return -ENOMEM;
    }
    return 0;
}


static int parse_arguments(int argc, char* const* const argv, const char** src, const char** node, const char** opts)
{
    int opt = 0;
...
static void modprobe(void)
{
    ...
    if (r) printf("failed to load ceph kernel module (%d)\n", r);
}
...
static int mount_new_device_format(const char* node, struct ceph_mount_info* cmi)
{
    ...
    pos = safe_cat(&rsrc, &len, pos, "=");
    if (cmi->cmi_path) safe_cat(&rsrc, &len, pos, cmi->cmi_path);

    mount_ceph_debug("mount.ceph: trying mount with new device syntax: %s\n", rsrc);
    if (cmi->cmi_opts) mount_ceph_debug("mount.ceph: options \"%s\" will pass to kernel\n", cmi->cmi_opts);
...
static int mount_old_device_format(const char* node, struct ceph_mount_info* cmi)
{
    ...
    if (cmi->cmi_fsname) append_opt("mds_namespace", cmi->cmi_fsname, cmi, &pos);
    if (cmi->cmi_fsid) append_opt("fsid", cmi->cmi_fsid, cmi, &pos);
...
static int do_mount(const char* dev, const char* node, struct ceph_mount_info* cmi)
{
    int pos = 0;
    int retval = -EINVAL;
    bool fallback = true;
    ...
    if (v2_addrs && !cmi->cmi_mons && !ms_mode_specified && !mon_addr_specified) {
    ...
    if (cmi->format == MOUNT_DEV_FORMAT_NEW) {
        ...
        if (retval) fallback = (should_fallback() && retval == -EINVAL && cmi->cmi_fsid);
    }

    /* pass-through or fallback to old-style mount device */
    if (retval && fallback) retval = mount_old_device_format(node, cmi);
    if (retval) {
        retval = EX_FAIL;
        switch (errno) {
        case ENODEV: fprintf(stderr, "mount error: ceph filesystem not supported by the system\n"); break;
        case EHOSTUNREACH: fprintf(stderr, "mount error: no mds server is up or the cluster is laggy\n"); break;
        default: fprintf(stderr, "mount error %d = %s\n", errno, strerror(errno));
        }
    }
...
static int append_key_or_secret_option(struct ceph_mount_info* cmi)
{
    ...
    if (!cmi->cmi_secret[0] && !is_kernel_secret(cmi->cmi_name)) return 0;

    if (pos) pos = safe_cat(&cmi->cmi_opts, &cmi->cmi_opts_len, pos, ",");
    ...
    if (cmi->cmi_secret[0] && (strcmp(cmi->cmi_secret, "<hidden>") != 0)) {
        ...
        if (ret < 0) {
            if (ret == -ENODEV || ret == -ENOSYS) {
                ...
                pos = safe_cat(&cmi->cmi_opts, &cmi->cmi_opts_len, pos, "secret=");
                pos = safe_cat(&cmi->cmi_opts, &cmi->cmi_opts_len, pos, cmi->cmi_secret);
                return 0;
            }
