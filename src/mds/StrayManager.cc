...
protected:
    StrayManager* sm;
    MDSRank* get_mds() override { return sm->mds; }
...
void StrayManager::purge(CDentry* dn)
{
    ...
    if (in->is_dir()) {
        ...
        for (const auto& p : pi->old_pools) {
            if (p != pi->layout.pool_id) item.old_pools.push_back(p);
        }
        item.snapc = *snapc;
    }
...
public:
    C_PurgeStrayLogged(StrayManager* sm_, CDentry* d, version_t v, MutationRef& m)
        : StrayManagerLogContext(sm_)
        , dn(d)
        , pdv(v)
        , mut(m)
    {}
    void finish(int r) override { sm->_purge_stray_logged(dn, pdv, mut); }
};
...
public:
    C_TruncateStrayLogged(StrayManager* sm, CDentry* d, MutationRef& m)
        : StrayManagerLogContext(sm)
        , dn(d)
        , mut(m)
    {}
    void finish(int r) override { sm->_truncate_stray_logged(dn, mut); }
};
...
void StrayManager::_purge_stray_purged(CDentry* dn, bool only_head)
{
    ...
    if (only_head) {
        ...
        if (in->get_num_ref() != (int)in->is_dirty() ||
            dn->get_num_ref() != (int)dn->is_dirty() + !!dn->state_test(CDentry::STATE_FRAGMENTING) +
...
void StrayManager::_purge_stray_logged(CDentry* dn, version_t pdv, MutationRef& mut)
{
    ...
    if (mds->is_stopping()) mds->mdcache->shutdown_export_stray_finish(ino);
}
...
public:
    C_RetryEnqueue(StrayManager* sm_, CDentry* dn_, bool t)
        ...
        , trunc(t)
    {}
    void finish(int r) override { sm->_enqueue(dn, trunc); }
};
...
void StrayManager::queue_delayed(CDentry* dn)
{
    ...
    if (dn->state_test(CDentry::STATE_EVALUATINGSTRAY)) return;
...
struct C_EvalStray : public StrayManagerContext
{
    ...
    C_EvalStray(StrayManager* sm_, CDentry* d)
        ...
        , dn(d)
    {}
    void finish(int r) override { sm->eval_stray(dn); }
};
...
bool StrayManager::_eval_stray(CDentry* dn)
{
    ...
    if (in->get_inode()->nlink == 0) {
        ...
        if (in->is_dir()) {
            if (in->snaprealm && in->snaprealm->has_past_parent_snaps()) {
                ...
                if (in->state_test(CInode::STATE_MISSINGOBJS)) {
                    mds->clog->error() << "previous attempt at committing dirfrag of ino " << in->ino()
                                       << " has failed, missing object";
        ...
        if (in->state_test(CInode::STATE_NEEDSRECOVER) || in->state_test(CInode::STATE_RECOVERING)) {
        ...
        if (in->snaprealm && in->snaprealm->has_past_parent_snaps() && in->is_any_old_inodes()) {
...
bool StrayManager::eval_stray(CDentry* dn)
{
    ...
    if (dn->state_test(CDentry::STATE_EVALUATINGSTRAY)) return false;
...
public:
    ...
    void finish(int r) override
    {
        if (dn->get_projected_linkage()->is_remote()) sm->eval_remote(dn);
        ...
        if (!stray_in->remote_parents.empty()) {
            for (const auto& dn : stray_in->remote_parents) {
                if (dn->last == CEPH_NOSNAP && !dn->is_projected()) {
                    if (dn->is_auth()) {
                        remote_dn = dn;
                        if (remote_dn->dir->can_auth_pin()) break;
                    }
                    else if (!remote_dn) {
                        remote_dn = dn;
                    }
...
void StrayManager::reintegrate_stray(CDentry* straydn, CDentry* rdn)
{
    ...
    if (straydn->reintegration_reqid) {
        dout(20) << __func__ << ": stray dentry " << *straydn << " is already under reintegrating" << dendl;
        return;
    }
...
void StrayManager::migrate_stray(CDentry* dn, mds_rank_t to)
{
    ...
    if (dn->reintegration_reqid) {
        dout(20) << __func__ << ": stray dentry " << *dn << " is already under migrating" << dendl;
        return;
    }
