...
// cons/des
MDSDaemon::MDSDaemon(std::string_view n, Messenger* m, MonClient* mc, boost::asio::io_context& ioctx)
    : Dispatcher(m->cct)
...
public:
    explicit MDSSocketHook(MDSDaemon* m)
        : mds(m)
    {}
    ...
    void call_async(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, const bufferlist& inbl,
...
void MDSDaemon::asok_command(std::string_view command, const cmdmap_t& cmdmap, Formatter* f, const bufferlist& inbl,
...
void MDSDaemon::set_up_admin_socket()
{
    ...
    r = admin_socket->register_command("status", asok_hook, "high-level status of MDS");
    ceph_assert(r == 0);
    r = admin_socket->register_command("dump_ops_in_flight", asok_hook, "show the ops currently in flight");
    ceph_assert(r == 0);
    r = admin_socket->register_command("ops", asok_hook, "show the ops currently in flight");
    ceph_assert(r == 0);
    r = admin_socket->register_command("dump_blocked_ops", asok_hook, "show the blocked ops currently in flight");
    ceph_assert(r == 0);
    r = admin_socket->register_command("dump_historic_ops", asok_hook, "show recent ops");
    ceph_assert(r == 0);
    r = admin_socket->register_command(
        "dump_historic_ops_by_duration", asok_hook, "show recent ops, sorted by op duration");
    ceph_assert(r == 0);
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command("scrub abort", asok_hook, "Abort in progress scrub operations(s)");
    ceph_assert(r == 0);
    r = admin_socket->register_command("scrub pause", asok_hook, "Pause in progress scrub operations(s)");
    ceph_assert(r == 0);
    r = admin_socket->register_command("scrub resume", asok_hook, "Resume paused scrub operations(s)");
    ceph_assert(r == 0);
    r = admin_socket->register_command("scrub status", asok_hook, "Status of scrub operations(s)");
    ceph_assert(r == 0);
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command(
        "flush_path name=path,type=CephString", asok_hook, "flush an inode (and its dirfrags)");
    ceph_assert(r == 0);
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command(
        "dump cache name=path,type=CephString,req=false", asok_hook, "dump metadata cache (optionally to a file)");
    ceph_assert(r == 0);
    r = admin_socket->register_command(
        "cache drop "
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command("cache status", asok_hook, "show cache status");
    ceph_assert(r == 0);
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command("dump loads", asok_hook, "dump metadata loads");
    ceph_assert(r == 0);
    r = admin_socket->register_command(
        "dump snaps name=server,type=CephChoices,strings=--server,req=false", asok_hook, "dump snapshots");
    ceph_assert(r == 0);
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command(
        "session kill name=client_id,type=CephString", asok_hook, "Evict a client session by id");
    ceph_assert(r == 0);
    r = admin_socket->register_command(
        "session ls name=cap_dump,type=CephBool,req=false", asok_hook, "Enumerate connected CephFS clients");
    ceph_assert(r == 0);
    ...
    r = admin_socket->register_command("client config "
                                       ...
                                       "Config a CephFS client session");
    assert(r == 0);
    r = admin_socket->register_command("damage ls", asok_hook, "List detected metadata damage");
    assert(r == 0);
    ...
    assert(r == 0);
    r = admin_socket->register_command(
        "osdmap barrier name=target_epoch,type=CephInt", asok_hook, "Wait until the MDS has this OSD map epoch");
    ceph_assert(r == 0);
    r = admin_socket->register_command("flush journal", asok_hook, "Flush the journal to the backing store");
    ceph_assert(r == 0);
    r = admin_socket->register_command("force_readonly", asok_hook, "Force MDS to read-only mode");
    ceph_assert(r == 0);
    r = admin_socket->register_command("get subtrees", asok_hook, "Return the subtree map");
    ceph_assert(r == 0);
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command("openfiles ls", asok_hook, "List the opening files and their caps");
    ceph_assert(r == 0);
    ...
    ceph_assert(r == 0);
    r = admin_socket->register_command("exit", asok_hook, "Terminate this MDS");
    r = admin_socket->register_command("respawn", asok_hook, "Respawn this MDS");
    ceph_assert(r == 0);
    r = admin_socket->register_command(
        "heap "
...
int MDSDaemon::init()
{
    ...
    monc->set_want_keys(CEPH_ENTITY_TYPE_MON | CEPH_ENTITY_TYPE_OSD | CEPH_ENTITY_TYPE_MDS | CEPH_ENTITY_TYPE_MGR);
    int r = 0;
    r = monc->init();
    if (r < 0) {
...
void MDSDaemon::reset_tick()
{
    ...
    // schedule
    tick_event = timer.add_event_after(g_conf()->mds_tick_interval, new LambdaContext([this](int) {
...
void MDSDaemon::handle_command(const cref_t<MCommand>& m)
{
    ...
    const bool live_session =
        session->get_state_seq() > 0 && mds_rank && mds_rank->sessionmap.get_session(session->info.inst.name);
    ...
    if (!session->auth_caps.allow_all()) {
        dout(1) << __func__
                << ": received command from client without `tell` capability: " << *m->get_connection()->peer_addrs
                << dendl;
...
void MDSDaemon::handle_mds_map(const cref_t<MMDSMap>& m)
{
    ...
    if (!mdsmap_compat.writeable(mdsmap->compat)) {
        dout(0) << "handle_mds_map mdsmap compatset " << mdsmap->compat << " not writeable with daemon features "
                << mdsmap_compat << ", killing myself" << dendl;
        suicide();
        return;
    }
    ...
    dout(10) << "map says I am mds." << whoami << "." << incarnation << " state " << ceph_mds_state_name(new_state)
             << dendl;
    ...
    if (old_state != DS::STATE_NULL && new_state == DS::STATE_NULL) {
        ...
        dout(1) << "Map removed me " << oldinfo << " from cluster; respawning! See cluster/monitor logs for details."
                << dendl;
        respawn();
    }
    ...
    if (whoami == MDS_RANK_NONE) {
        ...
        if (mds_rank == NULL) {
            mds_rank = new MDSRankDispatcher(whoami,
                                             m->map_fs_name,
                                             mds_lock,
                                             clog,
                                             timer,
...
void MDSDaemon::suicide()
{
    ...
    if (mgrc.is_initialized()) mgrc.shutdown();
...
bool MDSDaemon::handle_core_message(const cref_t<Message>& m)
{
    ...
    case MSG_REMOVE_SNAPS:
        ...
        // OSD
    case MSG_COMMAND: handle_command(ref_cast<MCommand>(m)); break;
...
{
    if (con->get_peer_type() != CEPH_ENTITY_TYPE_CLIENT) return false;
    ...
    if (beacon.get_want_state() == CEPH_MDS_STATE_DNE) return false;
...
{
    if (con->get_peer_type() != CEPH_ENTITY_TYPE_CLIENT) return;
    ...
    if (beacon.get_want_state() == CEPH_MDS_STATE_DNE) return;
...
bool MDSDaemon::parse_caps(const AuthCapsInfo& info, MDSAuthCaps& caps)
{
    ...
    if (info.allow_all) {
        ...
        if (caps.parse(g_ceph_context, auth_cap_str, cs.get())) {
            ...
            dout(1) << __func__ << ": auth cap parse error: " << cs->strv() << " parsing '" << auth_cap_str << "'"
                    << dendl;
            return false;
        }
    }
...
void MDSDaemon::ms_handle_accept(Connection* con)
{
    ...
    if (!s) {
        ...
        dout(10) << " existing session " << s << " for " << s->info.inst << " existing con " << s->get_connection()
