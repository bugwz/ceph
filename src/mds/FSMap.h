...
{
    ClusterInfo() = default;
    ClusterInfo(std::string_view client_name, std::string_view cluster_name, std::string_view fs_name)
    ...
    {
        return client_name == cluster_info.client_name && cluster_name == cluster_info.cluster_name &&
...
{
    out << "{client_name=" << cluster_info.client_name << ", cluster_name=" << cluster_info.cluster_name
        << ", fs_name=" << cluster_info.fs_name << "}";
    return out;
}
...
struct Peer
{
    ...
    Peer(std::string_view uuid)
        : uuid(uuid)
    {}
    Peer(std::string_view uuid, const ClusterInfo& remote)
        : uuid(uuid)
        , remote(remote)
    {}
    ...
    bool operator==(const Peer& rhs) const { return uuid == rhs.uuid; }
...
struct MirrorInfo
{
    ...
    bool is_mirrored() const { return mirrored; }
    void enable_mirroring() { mirrored = true; }
    ...
    // uuid variant check
    bool has_peer(std::string_view uuid) const { return peers.find(Peer(uuid)) != peers.end(); }
    // client_name/cluster_name/fs_name variant check
    bool has_peer(std::string_view client_name, std::string_view cluster_name, std::string_view fs_name) const
    {
    ...
    void peer_add(std::string_view uuid, std::string_view client_name, std::string_view cluster_name,
                  std::string_view fs_name)
    {
        peers.emplace(Peer(uuid, ClusterInfo(client_name, cluster_name, fs_name)));
    }
    void peer_remove(std::string_view uuid) { peers.erase(uuid); }
...
public:
     ...
     */
    bool has_standby_replay(mds_gid_t who) const { return get_standby_replay(who) != MDS_GID_NONE; }
    ...
    bool is_standby_replay(mds_gid_t who) const
    {
        ...
        if (p != mds_map.mds_info.end() && p->second.state == MDSMap::STATE_STANDBY_REPLAY) {
            return true;
        }
        return false;
    }
...
public:
    ...
    FSMap()
        : default_compat(MDSMap::get_compat_set_default())
    {}
    ...
    bool get_enable_multiple() const { return enable_multiple; }
    ...
    fs_cluster_id_t get_legacy_client_fscid() const { return legacy_client_fscid; }

    size_t get_num_standby() const { return standby_daemons.size(); }
     ...
     */
    bool gid_exists(mds_gid_t gid, const std::vector<std::string>& in = {}) const
    {
        try {
     ...
     */
    bool gid_has_rank(mds_gid_t gid) const { return gid_exists(gid) && mds_roles.at(gid) != FS_CLUSTER_ID_NONE; }
     ...
     */
    void assign_standby_replay(const mds_gid_t standby_gid, const fs_cluster_id_t leader_ns,
                               const mds_rank_t leader_rank);

    /**
     * Assign an MDS cluster rank to a standby daemon
     */
    void promote(mds_gid_t standby_gid, Filesystem& filesystem, mds_rank_t assigned_rank);
     ...
     */
    Filesystem::ref create_filesystem(std::string_view name, int64_t metadata_pool, int64_t data_pool,
     ...
     */
    template<typename T> void modify_filesystem(fs_cluster_id_t fscid, T&& fn)
    {
     ...
     */
    template<typename T> void modify_daemon(mds_gid_t who, T&& fn)
    {
    ...
    bool is_standby_replay(mds_gid_t who) const { return filesystems.at(mds_roles.at(who))->is_standby_replay(who); }
    ...
    void update_export_targets(mds_gid_t who, const std::set<mds_rank_t>& targets)
    {
        ...
        modify_filesystem(fscid, [who, &targets](auto&& fs) { fs->mds_map.mds_info.at(who).export_targets = targets; });
    }
    ...
    bool is_struct_old() const { return struct_version < STRUCT_VERSION_TRIM_TO; }
    ...
    Filesystem::const_ref get_filesystem(fs_cluster_id_t fscid) const
    {
    ...
    Filesystem::ref get_filesystem(mds_gid_t gid) { return filesystems.at(mds_roles.at(gid)); }
    Filesystem::const_ref get_filesystem(void) const
    {
        return std::const_pointer_cast<const Filesystem>(filesystems.begin()->second);
    }
    Filesystem::const_ref get_filesystem(std::string_view name) const;
    Filesystem::const_ref get_filesystem(mds_gid_t gid) const { return filesystems.at(mds_roles.at(gid)); }

    std::vector<Filesystem::const_ref> get_filesystems(void) const;

    int parse_filesystem(std::string_view ns_str, Filesystem::const_ref* result) const;

    int parse_role(std::string_view role_str, mds_role_t* role, std::ostream& ss,
                   const std::vector<std::string>& filter) const;

    int parse_role(std::string_view role_str, mds_role_t* role, std::ostream& ss) const;
