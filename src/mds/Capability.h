...
public:
    ...
    {
        Export() {}
        Export(int64_t id, int w, int i, int p, snapid_t cf, ceph_seq_t s, ceph_seq_t m, utime_t lis, unsigned st)
            : cap_id(id)
            , wanted(w)
            , issued(i)
            , pending(p)
    ...
    {
        Import() {}
        Import(int64_t i, ceph_seq_t s, ceph_seq_t m)
            : cap_id(i)
            , issue_seq(s)
            , mseq(m)
        {}
    ...
    {
        revoke_info() {}
        revoke_info(__u32 b, ceph_seq_t s, ceph_seq_t li)
            : before(b)
            , seq(s)
            , last_issue(li)
        {}
    ...
    int revoking() const { return _issued & ~_pending; }
    ceph_seq_t issue(unsigned c, bool reval = false)
    {
        ...
        if (_pending & ~c) {
            ...
            while (!_revokes.empty() && (_revokes.back().before & ~_pending) == 0) _revokes.pop_back();
        }
        else {
            // no change.
    ...
    bool is_wanted_notable() const { return is_wanted_notable(wanted()); }
    ...
    {
        return Export(
            cap_id, wanted(), issued(), pending(), client_follows, get_last_seq(), mseq + 1, last_issue_stamp, state);
    }
    void merge(const Export& other, bool auth_cap)
    {
        ...
        if ((other.state & STATE_CLIENTWRITEABLE) && !is_notable()) mark_notable();

        // wanted
        set_wanted(wanted() | other.wanted);
        if (auth_cap) mseq = other.mseq;
    }
    ...
    int revoke()
    {
        if (revoking()) return confirm_receipt(last_sent, pending());
        return 0;
    }
