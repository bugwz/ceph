...
public:
    ...
    CDentry(std::string_view n, __u32 h, mempool::mds_co::string alternate_name, snapid_t f, snapid_t l)
        : hash(h)
        , first(f)
        , last(l)
        , item_dirty(this)
    ...
    {}
    CDentry(std::string_view n, __u32 h, mempool::mds_co::string alternate_name, inodeno_t ino, unsigned char dt,
    ...
    ~CDentry() override { ceph_assert(batch_ops.empty()); }
    ...
    bool is_lt(const MDSCacheObject* r) const override { return *this < *static_cast<const CDentry*>(r); }

    dentry_key_t key() { return dentry_key_t(last, name.c_str(), hash); }
    ...
    void set_alternate_name(mempool::mds_co::string altn) { alternate_name = std::move(altn); }
    void set_alternate_name(std::string_view altn) { alternate_name = mempool::mds_co::string(altn); }
    ...
    {
        if (!projected.empty()) return &projected.back();
        return &linkage;
    }
    ...
    CInode* get_projected_inode() { return get_projected_linkage()->inode; }

    bool use_projected(client_t client, const MutationRef& mut) const
    {
        return lock.can_read_projected(client) || lock.get_xlock_by() == mut;
    }
    ...
    void first_get() override { lru_pin(); }
    void last_put() override { lru_unpin(); }
    void _put() override;
    ...
    void decode_import(ceph::buffer::list::const_iterator& blp, LogSegment* ls)
    {
        ...
        mark_auth();
        if (nstate & STATE_DIRTY) _mark_dirty(ls);
    ...
    bool is_any_leases() const { return !client_lease_map.empty(); }
    const ClientLease* get_client_lease(client_t c) const
    {
        if (client_lease_map.count(c)) return client_lease_map.find(c)->second;
        return 0;
    }
    ...
    void remove_client_lease(ClientLease* r,
                             Locker* locker);   // returns remaining mask (if any), and kicks locker eval_gathers
    ...
    static void encode_remote(inodeno_t& ino, unsigned char d_type, std::string_view alternate_name, bufferlist& bl);
    static void decode_remote(char icode, inodeno_t& ino, unsigned char& d_type,
                              mempool::mds_co::string& alternate_name, ceph::buffer::list::const_iterator& bl);
