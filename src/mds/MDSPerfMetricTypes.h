...
struct ReadLatencyMetric
{
    ...
    DENC(ReadLatencyMetric, v, p)
    {
        DENC_START(3, 1, p);
        denc(v.lat, p);
        if (struct_v >= 2) denc(v.updated, p);
        if (struct_v >= 3) {
            denc(v.mean, p);
            denc(v.sq_sum, p);
            denc(v.count, p);
    ...
    {
        os << "{latency=" << metric.lat << ", avg_latency=" << metric.mean << ", sq_sum=" << metric.sq_sum
           << ", count=" << metric.count << "}";
        return os;
    }
};
...
struct OpenedFilesMetric
{
    ...
    {
        os << "{opened_files=" << metric.opened_files << ", total_inodes=" << metric.total_inodes << "}";
        return os;
    }
};
...
struct PinnedIcapsMetric
{
    ...
    {
        os << "{pinned_icaps=" << metric.pinned_icaps << ", total_inodes=" << metric.total_inodes << "}";
        return os;
    }
};
...
struct OpenedInodesMetric
{
    ...
    {
        os << "{opened_inodes=" << metric.opened_inodes << ", total_inodes=" << metric.total_inodes << "}";
        return os;
    }
};
...
struct Metrics
{
    ...
    friend std::ostream& operator<<(std::ostream& os, const Metrics& metrics)
    {
        os << "[update_type=" << metrics.update_type << ", metrics={"
           << "cap_hit_metric=" << metrics.cap_hit_metric << ", read_latency=" << metrics.read_latency_metric
