...
void MutationImpl::set_stickydirs(CInode* in)
{
    if (!stickydiri || stickydiri != in) {
        in->get_stickydirs();
        if (stickydiri) stickydiri->put_stickydirs();
        stickydiri = in;
    }
}
...
bool MutationImpl::is_rdlocked(SimpleLock* lock) const
{
    ...
    if (it != locks.end() && it->is_rdlock()) return true;
    if (lock_cache) return static_cast<const MutationImpl*>(lock_cache)->is_rdlocked(lock);
    return false;
}
...
{
    auto it = locks.find(lock);
    if (it != locks.end() && it->is_wrlock()) return true;
    if (lock_cache) return static_cast<const MutationImpl*>(lock_cache)->is_wrlocked(lock);
    return false;
}
...
void MutationImpl::apply()
{
    for (auto& obj : projected_nodes) {
        if (CInode* in = dynamic_cast<CInode*>(obj)) in->pop_and_dirty_projected_inode(ls, nullptr);
    }
    ...
    for (auto& obj : projected_nodes) {
        if (CDir* dir = dynamic_cast<CDir*>(obj)) dir->pop_and_dirty_projected_fnode(ls, nullptr);
    }
...
void MDRequestImpl::unfreeze_auth_pin(bool clear_inode)
{
    ...
    if (clear_inode) more()->rename_inode = NULL;
}
...
bool MDRequestImpl::can_auth_pin(MDSCacheObject* object)
{
    return object->can_auth_pin() ||
           (is_auth_pinned(object) && has_more() && more()->is_freeze_authpin && more()->rename_inode == object);
}

void MDRequestImpl::drop_local_auth_pins()
{
    if (has_more() && more()->is_freeze_authpin) unfreeze_auth_pin(true);
    MutationImpl::drop_local_auth_pins();
}

const filepath& MDRequestImpl::get_filepath()
{
    if (client_request) return client_request->get_filepath();
    return more()->filepath1;
}

const filepath& MDRequestImpl::get_filepath2()
{
    if (client_request) return client_request->get_filepath2();
...
{
    if (num_auth_pins || num_remote_auth_pins || lock_cache || !locks.empty()) return false;

    auto op = client_request->get_op();
    auto& path = client_request->get_filepath();
    if (op == CEPH_MDS_OP_GETATTR) {
        if (path.depth() == 0) return true;
    }
    else if (op == CEPH_MDS_OP_LOOKUP) {
        if (path.depth() == 1 && !path.is_last_snap()) return true;
    }
...
{
    if (dir_root[0] < dir_root[1]) return -1;
    if (dir_root[0] > dir_root[1]) return 1;
    if (dir_depth[0] < dir_depth[1]) return -1;
    if (dir_depth[0] > dir_depth[1]) return 1;
    return 0;
}
    ...
    f->dump_stream("reqid") << reqid;
    {
        ...
        if (_client_request) {
            ...
            if (_peer_request) {
                ...
                f->dump_string("op_type", MMDSPeerRequest::get_opname(_peer_request->get_op()));
                ...
                f->dump_bool("has_inode_export", _peer_request->inode_export_v != 0);
