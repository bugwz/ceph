...
public:
    ...
    const std::set<LogSegment*>& get_expiring_segments() const { return expiring_segments; }
    ...
    LogSegment* peek_current_segment() { return segments.empty() ? NULL : segments.rbegin()->second; }
    ...
    {
        if (segments.count(seq)) return segments[seq];
        return NULL;
    }

    bool have_any_segments() const { return !segments.empty(); }
    ...
    void flush();
    bool is_flushed() const { return unflushed == 0; }

    void trim_expired_segments();
    void trim(int max = -1);
    int trim_all();
    bool expiry_done() const { return expiring_segments.empty() && expired_segments.empty(); };
...
protected:
    struct PendingEvent
    {
        PendingEvent(LogEvent* e, MDSContext* c, bool f = false)
            : le(e)
            , fin(c)
            , flush(f)
        {}
    ...
    {
    public:
        explicit RecoveryThread(MDLog* l)
            : log(l)
        {}
    ...
    LogSegment* get_oldest_segment() { return segments.begin()->second; }
