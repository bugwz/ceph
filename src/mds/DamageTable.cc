...
public:
    ...
    damage_entry_type_t get_type() const override { return DAMAGE_ENTRY_DIRFRAG; }
...
public:
    ...
    DentryDamage(inodeno_t ino_, frag_t frag_, std::string_view dname_, snapid_t snap_id_)
        : ino(ino_)
        , frag(frag_)
        , dname(dname_)
        , snap_id(snap_id_)
    ...
    damage_entry_type_t get_type() const override { return DAMAGE_ENTRY_DENTRY; }
...
public:
    ...
    damage_entry_type_t get_type() const override { return DAMAGE_ENTRY_BACKTRACE; }
...
bool DamageTable::notify_dentry(inodeno_t ino, frag_t frag, snapid_t snap_id, std::string_view dname,
...
bool DamageTable::notify_dirfrag(inodeno_t ino, frag_t frag, std::string_view path)
{
    // Special cases: damage to these dirfrags is considered fatal to
    // the MDS rank that owns them.
    if ((MDS_INO_IS_STRAY(ino) && MDS_INO_STRAY_OWNER(ino) == rank) || (ino == CEPH_INO_ROOT)) {
...
bool DamageTable::is_dentry_damaged(const CDir* dir_frag, std::string_view dname, const snapid_t snap_id) const
{
    if (dentries.count(DirFragIdent(dir_frag->inode->ino(), dir_frag->frag)) == 0) {
        return false;
    }
...
bool DamageTable::is_dirfrag_damaged(const CDir* dir_frag) const
{
    return dirfrags.count(DirFragIdent(dir_frag->inode->ino(), dir_frag->frag)) > 0;
}

bool DamageTable::is_remote_damaged(const inodeno_t ino) const
{
