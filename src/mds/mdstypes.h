...
template<template<typename> class Allocator>
using xattr_map = std::map<alloc_string<Allocator>, ceph::bufferptr, std::less<alloc_string<Allocator>>,
...
template<template<typename> class Allocator = std::allocator> struct old_inode_t
{
    snapid_t first;
...
template<template<typename> class Allocator> void old_inode_t<Allocator>::decode(ceph::buffer::list::const_iterator& bl)
{
...
template<template<typename> class Allocator> void old_inode_t<Allocator>::dump(ceph::Formatter* f) const
{
...
public:
    ...
    {
        if (bit >= bits_per_block * _vec.size()) return false;
        return _vec[bit / bits_per_block] & ((block_type)1 << (bit % bits_per_block));
    }
    void insert(size_t bit)
    {
        size_t n = bit / bits_per_block;
        if (n >= _vec.size()) _vec.resize(n + 1);
        _vec[n] |= ((block_type)1 << (bit % bits_per_block));
    }
    void erase(size_t bit)
    {
        size_t n = bit / bits_per_block;
        if (n >= _vec.size()) return;
        _vec[n] &= ~((block_type)1 << (bit % bits_per_block));
        if (n + 1 == _vec.size()) {
            while (!_vec.empty() && _vec.back() == 0) _vec.pop_back();
        }
    }
    void clear() { _vec.clear(); }
    bool operator==(const feature_bitset_t& other) const { return _vec == other._vec; }
    bool operator!=(const feature_bitset_t& other) const { return _vec != other._vec; }
    void encode(ceph::buffer::list& bl) const;
    void decode(ceph::buffer::list::const_iterator& p);
    void dump(ceph::Formatter* f) const;
...
struct metric_spec_t
{
    ...
    bool empty() const { return metric_flags.empty(); }

    void clear() { metric_flags.clear(); }
...
struct dentry_key_t
{
    ...
    static void decode_helper(ceph::buffer::list::const_iterator& bl, std::string& nm, snapid_t& sn)
    {
        std::string key;
        using ceph::decode;
        decode(key, bl);
...
{
    cap_reconnect_t() {}
    cap_reconnect_t(uint64_t cap_id, inodeno_t pino, std::string_view p, int w, int i, inodeno_t sr, snapid_t sf,
                    ceph::buffer::list& lb)
        : path(p)
    {
...
public:
    ...
    DecayCounter& get(int t) { return vec[t]; }
    void zero()
    {
        for (auto& d : vec) {
            d.reset();
...
public:
    ...
    const DecayCounter& get(int t) const { return vec[t]; }
    DecayCounter& get(int t) { return vec[t]; }
    ...
    {
        return 1 * vec[META_POP_IRD].get() + 2 * vec[META_POP_IWR].get() + 1 * vec[META_POP_READDIR].get() +
               2 * vec[META_POP_FETCH].get() + 4 * vec[META_POP_STORE].get();
    }

    void add(dirfrag_load_vec_t& r)
    {
        for (size_t i = 0; i < dirfrag_load_vec_t::NUM; i++) vec[i].adjust(r.vec[i].get());
    }
    ...
    void scale(double f)
    {
        for (size_t i = 0; i < dirfrag_load_vec_t::NUM; i++) vec[i].scale(f);
    }
...
inline std::ostream& operator<<(std::ostream& out, const dirfrag_load_vec_t& dl)
{
    ...
    *css << std::setprecision(1) << std::fixed
         << "[pop"
            " IRD:"
         << dl.vec[0] << " IWR:" << dl.vec[1] << " RDR:" << dl.vec[2] << " FET:" << dl.vec[3] << " STR:" << dl.vec[4]
         << " *LOAD:" << dl.meta_load() << "]";
    return out << css->strv();
}
...
{
    return out << "mdsload<" << load.auth << "/" << load.all << ", req " << load.req_rate << ", hr "
               << load.cache_hit_rate << ", qlen " << load.queue_len << ", cpu " << load.cpu_load_avg << ">";
}
...
inline std::ostream& operator<<(std::ostream& out, const MDSCacheObjectInfo& info)
{
    ...
    if (info.dname.length()) return out << info.dirfrag << "/" << info.dname << " snap " << info.snapid;
