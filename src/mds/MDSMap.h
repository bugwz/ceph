...
public:
    ...
    typedef enum
    {
        ...
        STATE_STANDBY_REPLAY =
            CEPH_MDS_STATE_STANDBY_REPLAY,   // up, replaying active node, ready to take over and not serving clients.
                                             // Note: Up to two MDS hold the rank being replayed.
        STATE_STOPPED =
            CEPH_MDS_STATE_STOPPED,   // down, once existed, but no subtrees. empty log.  may not be held by a daemon.
    ...
    struct mds_info_t
    {
        ...
        bool is_degraded() const { return STATE_REPLAY <= state && state <= STATE_CLIENTREPLAY; }
    ...
    utime_t get_session_timeout() const { return utime_t(session_timeout, 0); }
    void set_session_timeout(uint32_t t) { session_timeout = t; }

    utime_t get_session_autoclose() const { return utime_t(session_autoclose, 0); }
    void set_session_autoclose(uint32_t t) { session_autoclose = t; }
    ...
    void add_required_client_feature(size_t bit) { required_client_features.insert(bit); }
    void remove_required_client_feature(size_t bit) { required_client_features.erase(bit); }
    const auto& get_required_client_features() const { return required_client_features; }
    ...
    bool is_data_pool(int64_t poolid) const
    {
        ...
        if (p == data_pools.end()) return false;
        return true;
    }
    ...
    const auto& get_mds_info_gid(mds_gid_t gid) const { return mds_info.at(gid); }
    ...
    // counts
    unsigned get_num_in_mds() const { return in.size(); }
    unsigned get_num_up_mds() const { return up.size(); }
    mds_rank_t get_last_in_mds() const
    {
        auto p = in.rbegin();
        return p == in.rend() ? MDS_RANK_NONE : *p;
    }
    int get_num_failed_mds() const { return failed.size(); }
    ...
    // data pools
    void add_data_pool(int64_t poolid) { data_pools.push_back(poolid); }
    int remove_data_pool(int64_t poolid)
    {
        std::vector<int64_t>::iterator p = std::find(data_pools.begin(), data_pools.end(), poolid);
        if (p == data_pools.end()) return -CEPHFS_ENOENT;
        data_pools.erase(p);
        return 0;
    }

    // sets
    void get_mds_set(std::set<mds_rank_t>& s) const { s = in; }
    void get_up_mds_set(std::set<mds_rank_t>& s) const;
    void get_active_mds_set(std::set<mds_rank_t>& s) const { get_mds_set(s, MDSMap::STATE_ACTIVE); }
    void get_standby_replay_mds_set(std::set<mds_rank_t>& s) const { get_mds_set(s, MDSMap::STATE_STANDBY_REPLAY); }
    void get_failed_mds_set(std::set<mds_rank_t>& s) const { s = failed; }
    void get_damaged_mds_set(std::set<mds_rank_t>& s) const { s = damaged; }
    ...
    void get_stopped_mds_set(std::set<mds_rank_t>& s) const { s = stopped; }
    ...
    {
        return !is_degraded() && get_num_mds(CEPH_MDS_STATE_CREATING) == 0 &&
               get_num_mds(CEPH_MDS_STATE_STARTING) == 0 && get_num_mds(CEPH_MDS_STATE_STOPPING) == 0;
    }
    ...
    auto get_state_gid(mds_gid_t gid) const
    {
        ...
        if (it == mds_info.end()) return STATE_NULL;
    ...
    {
        auto it = up.find(m);
        if (it == up.end()) return STATE_NULL;
        return get_state_gid(it->second);
    }

    auto get_gid(mds_rank_t r) const { return up.at(r); }
    const auto& get_info(mds_rank_t m) const { return mds_info.at(up.at(m)); }
    const auto& get_info_gid(mds_gid_t gid) const { return mds_info.at(gid); }

    bool is_boot(mds_rank_t m) const { return get_state(m) == STATE_BOOT; }
    bool is_bootstrapping(mds_rank_t m) const { return is_creating(m) || is_starting(m) || is_replay(m); }
    ...
    bool is_active_or_stopping(mds_rank_t m) const { return is_active(m) || is_stopping(m); }
    ...
    bool has_standby_replay(mds_rank_t r) const { return get_standby_replay(r) != MDS_GID_NONE; }
    ...
    bool is_any_failed() const { return !failed.empty(); }
    bool is_any_damaged() const { return !damaged.empty(); }
    bool is_resolving() const
    {
        return get_num_mds(STATE_RESOLVE) > 0 && get_num_mds(STATE_REPLAY) == 0 && failed.empty() && damaged.empty();
    }
    bool is_rejoining() const
    {
        // nodes are rejoining cache state
        return get_num_mds(STATE_REJOIN) > 0 && get_num_mds(STATE_REPLAY) == 0 && get_num_mds(STATE_RECONNECT) == 0 &&
               get_num_mds(STATE_RESOLVE) == 0 && failed.empty() && damaged.empty();
    }
    bool is_stopped() const { return up.empty(); }
     ...
     * with it.
     */
    bool have_inst(mds_rank_t m) const { return up.count(m); }

    /**
     * Get the MDS daemon entity_inst_t for a rank
     * known to be up.
     */
    entity_addrvec_t get_addrs(mds_rank_t m) const { return mds_info.at(up.at(m)).get_addrs(); }
    ...
    {
        auto it = up.find(m);
        if (it == up.end()) return MDS_GID_NONE;
    ...
    int get_inc_gid(mds_gid_t gid) const
    {
        ...
        if (mds_info_entry != mds_info.end()) return mds_info_entry->second.inc;
        return -1;
    }
    void encode(ceph::buffer::list& bl, uint64_t features) const;
    void decode(ceph::buffer::list::const_iterator& p);
