...
ostream& operator<<(ostream& out, const CDir& dir)
{
    ...
    if (dir.is_auth()) {
        out << " auth";
        if (dir.is_replicated()) out << dir.get_replicas();
    ...
    if (dir.get_auth_pins() || dir.get_dir_auth_pins()) {
        out << " ap=" << dir.get_auth_pins() << "+" << dir.get_dir_auth_pins();
    ...
    if (g_conf()->mds_debug_scatterstat && dir.is_projected()) {
        ...
        out << "->" << pf->fragstat;
        if (!(pf->fragstat == pf->accounted_fragstat)) out << "/" << pf->accounted_fragstat;
    }

    // rstat
    out << " " << dir.get_fnode()->rstat;
    if (!(dir.get_fnode()->rstat == dir.get_fnode()->accounted_rstat)) out << "/" << dir.get_fnode()->accounted_rstat;
    if (g_conf()->mds_debug_scatterstat && dir.is_projected()) {
        const auto& pf = dir.get_projected_fnode();
        out << "->" << pf->rstat;
        if (!(pf->rstat == pf->accounted_rstat)) out << "/" << pf->accounted_rstat;
    }

    out << " hs=" << dir.get_num_head_items() << "+" << dir.get_num_head_null();
    out << ",ss=" << dir.get_num_snap_items() << "+" << dir.get_num_snap_null();
    if (dir.get_num_dirty()) out << " dirty=" << dir.get_num_dirty();
...
{
    if (!g_conf()->mds_debug_scatterstat && !scrub) return true;

    dout(25) << "check_rstats on " << this << dendl;
    if (!is_complete() || !is_auth() || is_frozen()) {
        dout(3) << "check_rstats " << (scrub ? "(scrub) " : "")
                << "bailing out -- incomplete or non-auth or frozen dir on " << *this << dendl;
        return !scrub;
    }
    ...
    for (auto i = items.begin(); i != items.end(); ++i) {
        if (i->second->last != CEPH_NOSNAP) continue;
...
CDentry* CDir::lookup(std::string_view name, snapid_t snap)
{
    ...
    if (iter == items.end()) return 0;
    if (iter->second->get_name() == name && iter->second->first <= snap && iter->second->last >= snap) {
...
CDentry* CDir::add_null_dentry(std::string_view dname, snapid_t first, snapid_t last)
{
    // foreign
    ...
    // pin?
    if (get_num_any() == 1) get(PIN_CHILD);
...
CDentry* CDir::add_primary_dentry(std::string_view dname, CInode* in, mempool::mds_co::string alternate_name,
                                  snapid_t first, snapid_t last)
{
    ...
    dn->version = get_projected_version();
    dn->check_corruption(true);
    if (is_auth()) dn->state_set(CDentry::STATE_AUTH);
...
CDentry* CDir::add_remote_dentry(std::string_view dname, inodeno_t ino, unsigned char d_type,
                                 mempool::mds_co::string alternate_name, snapid_t first, snapid_t last)
{
    // foreign
    ceph_assert(lookup_exact_snap(dname, last) == 0);
    ...
    // create dentry
    CDentry* dn =
        new CDentry(dname, inode->hash_dentry_name(dname), std::move(alternate_name), ino, d_type, first, last);
    dn->dir = this;
    dn->version = get_projected_version();
    dn->check_corruption(true);
    if (is_auth()) dn->state_set(CDentry::STATE_AUTH);
...
void CDir::remove_dentry(CDentry* dn)
{
    ...
    // unpin?
    if (get_num_any() == 0) put(PIN_CHILD);
    ceph_assert(get_num_any() == items.size());
}
...
void CDir::link_primary_inode(CDentry* dn, CInode* in)
{
    ...
    if (dn->state_test(CDentry::STATE_BOTTOMLRU) && (is_auth() || !inode->is_stray())) {
        mdcache->bottom_lru.lru_remove(dn);
        mdcache->lru.lru_insert_mid(dn);
        dn->state_clear(CDentry::STATE_BOTTOMLRU);
    }
...
void CDir::link_inode_work(CDentry* dn, CInode* in)
{
    ...
    // pin dentry?
    if (in->get_num_ref()) dn->get(CDentry::PIN_INODEPIN);

    if (in->state_test(CInode::STATE_TRACKEDBYOFT)) mdcache->open_file_table.notify_link(in);
    if (in->is_any_caps()) adjust_num_inodes_with_caps(1);

    // adjust auth pin count
    if (in->auth_pins) dn->adjust_nested_auth_pins(in->auth_pins, NULL);
...
void CDir::unlink_inode(CDentry* dn, bool adjust_lru)
{
    ...
    if (adjust_lru && !is_auth() && !dn->state_test(CDentry::STATE_BOTTOMLRU)) {
        mdcache->lru.lru_remove(dn);
        mdcache->bottom_lru.lru_insert_mid(dn);
        dn->state_set(CDentry::STATE_BOTTOMLRU);
    }
...
void CDir::try_remove_unlinked_dn(CDentry* dn)
{
    ...
    if (dn->get_num_ref() != dn->is_dirty()) return;
...
void CDir::unlink_inode_work(CDentry* dn)
{
    ...
    if (dn->get_linkage()->is_remote()) {
        // remote
        if (in) dn->unlink_remote(dn->get_linkage());
        ...
        // primary
        // unpin dentry?
        if (in->get_num_ref()) dn->put(CDentry::PIN_INODEPIN);

        if (in->state_test(CInode::STATE_TRACKEDBYOFT)) mdcache->open_file_table.notify_unlink(in);
        if (in->is_any_caps()) adjust_num_inodes_with_caps(-1);

        // unlink auth_pin count
        if (in->auth_pins) dn->adjust_nested_auth_pins(-in->auth_pins, nullptr);
        ...
        if (in->is_dir()) in->item_pop_lru.remove_myself();
...
void CDir::remove_null_dentries()
{
    ...
    while (p != items.end()) {
        CDentry* dn = p->second;
        ++p;
        if (dn->get_linkage()->is_null() && !dn->is_projected()) remove_dentry(dn);
    }
...
void CDir::try_remove_dentries_for_stray()
{
    ...
    while (p != items.end()) {
        ...
        if (dn->last == CEPH_NOSNAP) {
            ...
            ceph_assert(dn->get_linkage()->is_null());
            if (clear_dirty && dn->is_dirty()) dn->mark_clean();
            // It's OK to remove lease prematurely because we will never link
            // the dentry to inode again.
            if (dn->is_any_leases()) dn->remove_client_leases(mdcache->mds->locker);
            if (dn->get_num_ref() == 0) remove_dentry(dn);
        }
        else {
            ceph_assert(!dn->is_projected());
            ...
            if (dnl->is_primary()) {
                in = dnl->get_inode();
                if (clear_dirty && in->is_dirty()) in->mark_clean();
            }
            if (clear_dirty && dn->is_dirty()) dn->mark_clean();
            if (dn->get_num_ref() == 0) {
                remove_dentry(dn);
                if (in) mdcache->remove_inode(in);
            }
        }
    }

    if (clear_dirty && is_dirty()) mark_clean();
}
...
bool CDir::try_trim_snap_dentry(CDentry* dn, const set<snapid_t>& snaps)
{
    ...
    if ((p == snaps.end() || *p > dn->last) && (dn->get_num_ref() == dn->is_dirty()) &&
        (!in || in->get_num_ref() == in->is_dirty())) {
        dout(10) << " purging snapped " << *dn << dendl;
        if (in && in->is_dirty()) in->mark_clean();
        remove_dentry(dn);
        if (in) {
...
void CDir::steal_dentry(CDentry* dn)
{
    ...
    if (dn->dir->items.empty()) dn->dir->put(PIN_CHILD);

    if (get_num_any() == 0) get(PIN_CHILD);
    if (dn->get_linkage()->is_null()) {
        if (dn->last == CEPH_NOSNAP)
            num_head_null++;
        else
        ...
        if (dn->get_linkage()->is_primary()) {
            ...
            if (in->is_dir()) {
                ...
                if (in->item_pop_lru.is_on_list()) pop_lru_subdirs.push_back(&in->item_pop_lru);
            }
            else {
            ...
            if (pi->accounted_rstat.rctime > fnode->rstat.rctime) _fnode->rstat.rctime = pi->accounted_rstat.rctime;

            if (in->is_any_caps()) adjust_num_inodes_with_caps(1);

            // move dirty inode rstat to new dirfrag
            if (in->is_dirty_rstat()) dirty_rstat_inodes.push_back(&in->dirty_rstat_item);
        }
        else if (dn->get_linkage()->is_remote()) {
...
void CDir::prepare_old_fragment(map<string_snap_t, MDSContext::vec>& dentry_waiters, bool replay)
{
    ...
    if (!replay && is_auth()) auth_pin(this);

    if (!waiting_on_dentry.empty()) {
        for (const auto& p : waiting_on_dentry) {
            std::copy(p.second.begin(), p.second.end(), std::back_inserter(dentry_waiters[p.first]));
        }
...
void CDir::finish_old_fragment(MDSContext::vec& waiters, bool replay)
{
    ...
    if (is_subtree_root()) put(PIN_SUBTREE);
...
void CDir::init_fragment_pins()
{
    ...
    if (is_subtree_root()) get(PIN_SUBTREE);
}
...
void CDir::split(int bits, std::vector<CDir*>* subs, MDSContext::vec& waiters, bool replay)
{
    ...
    if (fnode->accounted_rstat.version == rstat_version) rstatdiff.add_delta(fnode->accounted_rstat, fnode->rstat);
    ...
    for (const auto& p : dentry_waiters) {
        ...
        if (f->waiting_on_dentry.empty()) f->get(PIN_DNWAITER);
        std::copy(p.second.begin(), p.second.end(), std::back_inserter(f->waiting_on_dentry[p.first]));
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        dout(10) << " rstat " << _fnode->rstat << " fragstat " << _fnode->fragstat << " on " << *f << dendl;

        if (i == 0) {
            // give any outstanding frag stat differential to first frag
            dout(10) << " giving rstatdiff " << rstatdiff << " fragstatdiff" << fragstatdiff << " to " << *subfrags[0]
                     << dendl;
...
void CDir::merge(const std::vector<CDir*>& subs, MDSContext::vec& waiters, bool replay)
{
    ...
    for (const auto& dir : subs) {
        ...
        if (dir->get_fnode()->accounted_fragstat.version == dirstat_version)
            fragstatdiff.add_delta(
                dir->get_fnode()->accounted_fragstat, dir->get_fnode()->fragstat, &touched_mtime, &touched_chattr);

        dir->prepare_old_fragment(dentry_waiters, replay);

        // steal dentries
        while (!dir->items.empty()) steal_dentry(dir->items.begin()->second);

        // merge replica map
        for (const auto& p : dir->get_replicas()) {
            unsigned cur = get_replicas()[p.first];
            if (p.second > cur) get_replicas()[p.first] = p.second;
        }

        // merge version
        if (dir->get_version() > _fnode->version) _fnode->version = projected_version = dir->get_version();
    ...
    if (!dentry_waiters.empty()) {
        get(PIN_DNWAITER);
        for (const auto& p : dentry_waiters) {
            std::copy(p.second.begin(), p.second.end(), std::back_inserter(waiting_on_dentry[p.first]));
        }
    }

    if (is_auth() && !replay) mark_complete();
...
{
    dout(10) << __func__ << dendl;
    for (elist<CInode*>::iterator p = dirty_rstat_inodes.begin_use_current(); !p.end(); ++p) {
        CInode* in = *p;
        ceph_assert(in->is_auth());
        if (in->is_frozen()) continue;
...
{
    if (!state_test(STATE_ASSIMRSTAT)) return;
    ...
    if (!dirty_rstat_inodes.empty()) mdcache->mds->locker->mark_updated_scatterlock(&inode->nestlock);
}
...
{
    if (waiting_on_dentry.empty()) get(PIN_DNWAITER);
    waiting_on_dentry[string_snap_t(dname, snapid)].push_back(c);
    dout(10) << __func__ << " dentry " << dname << " snap " << snapid << " " << c << " on " << *this << dendl;
}

void CDir::take_dentry_waiting(std::string_view dname, snapid_t first, snapid_t last, MDSContext::vec& ls)
{
    ...
    while (it != waiting_on_dentry.end() && !(ub < it->first)) {
        dout(10) << __func__ << " " << dname << " [" << first << "," << last << "] found waiter on snap "
                 << it->first.snapid << " on " << *this << dendl;
        std::copy(it->second.begin(), it->second.end(), std::back_inserter(ls));
        waiting_on_dentry.erase(it++);
    }

    if (waiting_on_dentry.empty()) put(PIN_DNWAITER);
}
...
void CDir::add_waiter(uint64_t tag, MDSContext* c)
{
    ...
    if (tag & WAIT_ATSUBTREEROOT) {
        if (!is_subtree_root()) {
            // try parent
            dout(10) << "add_waiter " << std::hex << tag << std::dec << " " << c << " should be ATSUBTREEROOT, "
                     << *this << " is not root, trying parent" << dendl;
...
void CDir::take_waiting(uint64_t mask, MDSContext::vec& ls)
{
    if ((mask & WAIT_DENTRY) && !waiting_on_dentry.empty()) {
        ...
        for (const auto& p : waiting_on_dentry) {
            dout(10) << "take_waiting dentry " << p.first.name << " snap " << p.first.snapid << " on " << *this
                     << dendl;
...
CDir::fnode_ptr CDir::project_fnode(const MutationRef& mut)
{
    ...
    if (mut && mut->is_projected(this)) return std::const_pointer_cast<fnode_t>(projected_fnode.back());
...
{
    if (min > projected_version) projected_version = min;
    ++projected_version;
...
void CDir::mark_dirty(LogSegment* ls, version_t pv)
{
    ...
    if (pv) {
        ...
        ceph_assert(!projected_fnode.empty() && pv <= projected_fnode.front()->version);
    }
...
void CDir::_mark_dirty(LogSegment* ls)
{
    ...
    if (ls) {
        ...
        if (committed_version == 0 && !item_new.is_on_list()) ls->new_dirfrags.push_back(&item_new);
    }
}
...
{
    if (is_dirty() || projected_version > get_version()) return;   // noop if it is already dirty or will be dirty
...
// -----------------------
// FETCH
void CDir::fetch(std::string_view dname, snapid_t last, MDSContext* c, bool ignore_authpinnability)
{
    if (dname.empty())
    ...
    if (CDir* pdir = get_parent_dir(); pdir && pdir->inode->is_stray() && !inode->snaprealm) {
        ...
        if (c) mdcache->mds->queue_waiter(c);
        return;
    }
    ...
    if (mdcache->mds->logger) mdcache->mds->logger->inc(l_mds_dir_fetch_complete);
...
void CDir::fetch_keys(const std::vector<dentry_key_t>& keys, MDSContext* c)
{
    ...
    for (auto& key : keys) {
        ...
        if (waiting_on_dentry.empty()) get(PIN_DNWAITER);
        auto em = waiting_on_dentry.emplace(
            std::piecewise_construct, std::forward_as_tuple(key.name, key.snapid), std::forward_as_tuple());
        if (!em.second) {
            if (!fallback_waiting) fallback_waiting = &em.first->second;
            continue;
        }
    ...
    if (mdcache->mds->logger) mdcache->mds->logger->inc(l_mds_dir_fetch_keys);
    mdcache->mds->balancer->hit_dir(this, META_POP_FETCH);
}
...
public:
    ...
    void print(ostream& out) const override { out << "dirfrag_fetch_more(" << dir->dirfrag() << ")"; }
};
...
public:
    ...
    void finish(int r) override
    {
        ...
        if (r >= 0 && ret3 != -CEPHFS_ECANCELED) dir->inode->verify_diri_backtrace(btbl, ret3);
        if (r >= 0) r = ret1;
        if (r >= 0) r = ret2;
    ...
    void print(ostream& out) const override { out << "dirfrag_fetch(" << dir->dirfrag() << ")"; }
};
...
void CDir::_omap_fetch(std::set<string>* keys, MDSContext* c)
{
    ...
    if (keys) {
        ...
        ceph_assert(!c);
        rd.omap_get_vals("", "", g_conf()->mds_dir_keys_per_op, &fin->omap, &fin->more, &fin->ret2);
    }
    // check the correctness of backtrace
    ...
    mdcache->mds->objecter->read(oid, oloc, rd, CEPH_NOSNAP, NULL, 0, new C_OnFinisher(fin, mdcache->mds->finisher));
}

void CDir::_omap_fetch_more(version_t omap_version, bufferlist& hdrbl, map<string, bufferlist>& omap, MDSContext* c)
{
...
CDentry* CDir::_load_dentry(std::string_view key, std::string_view dname, const snapid_t last, bufferlist& bl,
                            const int pos, const std::set<snapid_t>* snaps, double rand_threshold, bool* force_dirty)
{
    auto q = bl.cbegin();
    ...
    dout(20) << "_fetched pos " << pos << " marker '" << type << "' dname '" << dname << " [" << first << "," << last
             << "]" << dendl;
    ...
    if (type == 'L' || type == 'l') {
        ...
        if (dn) {
            ...
            if (committed_version == 0 && dnl->is_remote() && dn->is_dirty() && ino == dnl->get_remote_ino() &&
                d_type == dnl->get_remote_d_type() && alternate_name == dn->get_alternate_name()) {
                // see comment below
        ...
        if (dn) {
            ...
            if (dnl->is_primary()) {
                ...
                else if (committed_version == 0 && dn->is_dirty() && inode_data.inode->ino == in->ino() &&
        ...
        if (!dn || undef_inode) {
            ...
            if (!in || undef_inode) {
                ...
                // symlink?
                if (in->is_symlink()) in->symlink = inode_data.symlink;
                ...
                if (in->is_any_old_inodes()) {
                    ...
                    if (min_first > in->first) in->first = min_first;
                }

                in->oldest_snap = inode_data.oldest_snap;
                in->decode_snap_blob(inode_data.snap_blob);
                if (snaps && !in->snaprealm) in->purge_stale_snap_data(*snaps);
                ...
                if (in->get_inode()->is_dirty_rstat()) in->mark_dirty_rstat();
                ...
                dout(0) << "_fetched  badness: got (but i already had) " << *in << " mode " << in->get_inode()->mode
                        << " mtime " << in->get_inode()->mtime << dendl;
                string dirpath, inopath;
                this->inode->make_path_string(dirpath);
                in->make_path_string(inopath);
                mdcache->mds->clog->error()
                    << "loaded dup inode " << inode_data.inode->ino << " [" << first << "," << last << "] v"
                    << inode_data.inode->version << " at " << dirpath << "/" << dname << ", but inode " << in->vino()
                    << " v" << in->get_version() << " already exists at " << inopath;
                return dn;
            }
        }
    }
...
void CDir::_omap_fetched(bufferlist& hdrbl, map<string, bufferlist>& omap, bool complete, const std::set<string>& keys,
                         int r)
{
    ...
    if (hdrbl.length() == 0) {
        ...
        clog->error() << "dir " << dirfrag()
                      << " object missing on disk; some "
                         "files may be lost ("
                      << get_path() << ")";
    ...
    fnode_t got_fnode;
    {
        ...
        try {
            ...
            clog->warn() << "Corrupt fnode header in " << dirfrag() << ": " << err.what() << " (" << get_path() << ")";
            go_bad(complete);
            return;
        }
        if (!p.end()) {
            clog->warn() << "header buffer of dir " << dirfrag() << " has " << hdrbl.length() - p.get_off()
                         << " extra bytes (" << get_path() << ")";
            go_bad(complete);
            return;
        }
    }
    ...
    if (fnode->snap_purged_thru < realm->get_last_destroyed()) {
        ...
        dout(10) << " snap_purged_thru " << fnode->snap_purged_thru << " < " << realm->get_last_destroyed()
    ...
    auto proc_waiters = [&](const string_snap_t& key) {
        ...
        while (w_it != waiting_on_dentry.rend()) {
            ...
            if (cmp == 0) {
                touch = true;
                std::copy(w_it->second.begin(), w_it->second.end(), std::back_inserter(finished));
    ...
    auto proc_nulls_and_waiters = [&](const string& str_key, const string_snap_t& key) {
        ...
        while (k_it != keys.rend()) {
            ...
            if (!(++count % mdcache->mds->heartbeat_reset_grace())) mdcache->mds->heartbeat_reset();
        }
        return touch;
    };
    ...
    for (auto p = omap.rbegin(); p != omap.rend(); ++p, --pos) {
        ...
        CDentry* dn = nullptr;
        try {
            dn = _load_dentry(p->first, key.name, key.snapid, p->second, pos, snaps, rand_threshold, &force_dirty);
        }
        catch (const buffer::error& err) {
            mdcache->mds->clog->warn() << "Corrupt dentry '" << key.name
                                       << "' in "
                                          "dir frag "
                                       << dirfrag() << ": " << err.what() << "(" << get_path() << ")";
    ...
    if (complete) {
        if (!waiting_on_dentry.empty()) {
            for (auto& p : waiting_on_dentry) {
                ...
                if (p.first.snapid == CEPH_NOSNAP) null_keys.emplace_back(p.first);
            }
            waiting_on_dentry.clear();
            put(PIN_DNWAITER);
        }
    ...
    if (!null_keys.empty()) {
        ...
        for (auto& key : null_keys) {
            ...
            if (!(++count % mdcache->mds->heartbeat_reset_grace(2))) mdcache->mds->heartbeat_reset();
        }
    }
    ...
    if (force_dirty && !mdcache->is_readonly()) log_mark_dirty();

    auth_unpin(this);

    if (!finished.empty()) mdcache->mds->queue_waiters(finished);
}
...
void CDir::go_bad_dentry(snapid_t last, std::string_view dname)
{
    ...
    path += "/";
    path += dname;
    const bool fatal = mdcache->mds->damage_table.notify_dentry(inode->ino(), frag, last, dname, path);
    if (fatal) {
        mdcache->mds->damaged();
...
void CDir::go_bad(bool complete)
{
    ...
    const bool fatal = mdcache->mds->damage_table.notify_dirfrag(inode->ino(), frag, get_path());
    if (fatal) {
        mdcache->mds->damaged();
        ceph_abort();   // unreachable, damaged() respawns us
    }
...
void CDir::commit(version_t want, MDSContext* c, bool ignore_authpinnability, int op_prio)
{
    ...
    // get an auth_pin.
    if (!c) c = new C_MDSInternalNoop;

    // auth_pin on first waiter
    if (waiting_for_commit.empty()) auth_pin(this);
...
public:
    C_IO_Dir_Committed(CDir* d, version_t v)
        : CDirIOContext(d)
        , version(v)
    {}
    void finish(int r) override { dir->_committed(r, version); }
    void print(ostream& out) const override { out << "dirfrag_committed(" << dir->dirfrag() << ")"; }
};

class C_IO_Dir_Commit_Ops : public Context
{
public:
    C_IO_Dir_Commit_Ops(CDir* d, int pr, vector<CDir::dentry_commit_item>&& s, bufferlist&& bl, vector<string>&& r,
    ...
    void finish(int r) override
    {
        dir->_omap_commit_ops(r, op_prio, metapool, version, is_new, to_set, dfts, to_remove, stale_items);
    }
...
void CDir::_encode_primary_inode_base(dentry_commit_item& item, bufferlist& dfts, bufferlist& bl)
{
    ENCODE_START(6, 4, bl);
    encode(*item.inode, bl, item.features);

    if (!item.symlink.empty()) encode(item.symlink, bl);
...
void CDir::_omap_commit_ops(int r, int op_prio, int64_t metapool, version_t version, bool _new,
                            vector<dentry_commit_item>& to_set, bufferlist& dfts, vector<string>& to_remove,
                            mempool::mds_co::compact_set<mempool::mds_co::string>& stales)
{
    dout(10) << __func__ << dendl;
    ...
    C_GatherBuilder gather(g_ceph_context,
                           new C_OnFinisher(new C_IO_Dir_Committed(this, version), mdcache->mds->finisher));
    ...
    auto commit_one = [&](bool header = false) {
        ...
        if (!_new) op.stat(nullptr, nullptr, nullptr);
        ...
        op.priority = op_prio;
        if (!_set.empty()) op.omap_set(_set);
        if (!_rm.empty()) op.omap_rm_keys(_rm);
        mdcache->mds->objecter->mutate(oid, oloc, op, snapc, ceph::real_clock::now(), 0, gather.new_sub());
        write_size = 0;
        _set.clear();
        _rm.clear();
    };
    ...
    for (auto& key : stales) {
        ...
        if (write_size > 0 && write_size + size > max_write_size) commit_one();
...
void CDir::_omap_commit(int op_prio)
{
    ...
    if (state_test(CDir::STATE_FRAGMENTING) && is_new()) {
        ...
        for (elist<CDentry*>::iterator it = dirty_dentries.begin(); !it.end(); ++it) ++items_count;
    }
    ...
    auto write_one = [&](CDentry* dn) {
        ...
        if (dn->get_linkage()->is_null()) {
            ...
            if (left < CEPH_PAGE_SIZE / 8) dfts.reserve(left + CEPH_PAGE_SIZE);
    ...
    if (state_test(CDir::STATE_FRAGMENTING) && is_new()) {
        ...
        for (auto p = items.begin(); p != items.end();) {
            CDentry* dn = p->second;
            ++p;
            if (dn->get_linkage()->is_null()) continue;
            write_one(dn);
    ...
    auto c = new C_IO_Dir_Commit_Ops(
        this, op_prio, std::move(to_set), std::move(dfts), std::move(to_remove), std::move(stale_items));
    stale_items.clear();
    mdcache->mds->finisher->queue(c);
}

void CDir::_parse_dentry(CDentry* dn, dentry_commit_item& item, const set<snapid_t>* snaps, bufferlist& bl)
{
    ...
    if (linkage.is_remote()) {
        ...
        if (in->is_multiversion()) {
            if (!in->snaprealm) {
                if (snaps) in->purge_stale_snap_data(*snaps);
            }
            else {
        ...
        item.inode = in->inode;
        if (in->inode->is_symlink()) item.symlink = in->symlink;
        using ceph::encode;
...
void CDir::_committed(int r, version_t v)
{
    if (r < 0) {
        ...
        if (r == -CEPHFS_ENOENT && committed_version == 0 && !inode->is_base() && get_parent_dir()->inode->is_stray()) {
            r = 0;
            if (inode->snaprealm) inode->state_set(CInode::STATE_MISSINGOBJS);
        }
        if (r < 0) {
    ...
    // _all_ commits done?
    if (committing_version == committed_version) state_clear(CDir::STATE_COMMITTING);

    // _any_ commit, even if we've been redirtied, means we're no longer new.
    item_new.remove_myself();

    // dir clean?
    if (committed_version == get_version()) mark_clean();
    ...
    for (auto p = dirty_dentries.begin(); !p.end();) {
        ...
        if (dn->linkage.is_primary()) {
            ...
            if (committed_version >= in->get_version()) {
                if (in->is_dirty()) {
                    dout(15) << " dir " << committed_version << " >= inode " << in->get_version() << " now clean "
                             << *in << dendl;
                    in->mark_clean();
                }
            }
            else {
                dout(15) << " dir " << committed_version << " < inode " << in->get_version() << " still dirty " << *in
                         << dendl;
                ceph_assert(in->is_dirty() ||
                            in->last < CEPH_NOSNAP);   // special case for cow snap items (not predirtied)
            }
        }
        ...
        if (committed_version >= dn->get_version()) {
            ...
            if (stray && dn->get_num_ref() == 0 && !dn->is_projected() && dn->get_linkage()->is_null())
                remove_dentry(dn);
        }
        else {
            dout(15) << " dir " << committed_version << " < dn " << dn->get_version() << " still dirty " << *dn
                     << dendl;
    ...
    while (it != waiting_for_commit.end()) {
        ...
        MDSContext::vec t;
        for (const auto& waiter : it->second) t.push_back(waiter);
    ...
    if (!inode->is_base() && get_parent_dir()->inode->is_stray() && inode->snaprealm)
        mdcache->maybe_eval_stray(inode, true);

    // unpin if we kicked the last waiter.
    if (were_waiters && waiting_for_commit.empty()) auth_unpin(this);
}
...
bool CDir::is_exportable(mds_rank_t dest) const
{
    ...
    if (export_pin == dest) return true;
    if (export_pin >= 0) return false;
    return true;
}
...
void CDir::decode_import(bufferlist::const_iterator& blp, LogSegment* ls)
{
    ...
    if (dirty_old_rstat.size() || !(fnode->rstat == fnode->accounted_rstat)) {
    ...
    if (is_dirty_dft()) {
        if (inode->dirfragtreelock.get_state() != LOCK_MIX && inode->dirfragtreelock.is_stable()) {
...
void CDir::set_dir_auth(const mds_authority_t& a)
{
    ...
    if (!was_subtree && is_subtree_root()) {
        ...
        if (inode->is_auth()) {
            ...
            if (is_frozen_dir()) inode->auth_unpin(this);
        }
    }
    if (was_subtree && !is_subtree_root()) {
        ...
        if (inode->is_auth()) {
            ceph_assert(!is_frozen_tree_root());
            if (is_frozen_dir()) inode->auth_pin(this);
        }
    }
...
void CDir::auth_pin(void* by)
{
    ...
    if (freeze_tree_state) freeze_tree_state->auth_pins += 1;
}
...
void CDir::adjust_nested_auth_pins(int dirinc, void* by)
{
    ...
    dout(15) << __func__ << " " << dirinc << " on " << *this << " by " << by << " count now " << auth_pins << "/"
             << dir_auth_pins << dendl;
    ceph_assert(dir_auth_pins >= 0);

    if (freeze_tree_state) freeze_tree_state->auth_pins += dirinc;

    if (dirinc < 0) maybe_finish_freeze();   // pending freeze?
}
...
void CDir::verify_fragstat()
{
    ...
    for (auto it = items.begin(); it != items.end(); ++it) {
        ...
        dout(10) << " " << *dn << dendl;
        if (dn->is_primary()) dout(10) << "     " << *dn->inode << dendl;
    ...
    if (c.nsubdirs != fnode->fragstat.nsubdirs || c.nfiles != fnode->fragstat.nfiles) {
...
void CDir::_walk_tree(std::function<bool(CDir*)> callback)
{
    ...
    while (!dfq.empty()) {
        ...
        for (auto& p : *dir) {
            CDentry* dn = p.second;
            if (!dn->get_linkage()->is_primary()) continue;
            CInode* in = dn->get_linkage()->get_inode();
...
bool CDir::freeze_tree()
{
    ...
    if (!lock_caches_with_auth_pins.empty()) mdcache->mds->locker->invalidate_lock_caches(this);

    _walk_tree([this](CDir* dir) {
        if (dir->freeze_tree_state) return false;
...
void CDir::_freeze_tree()
{
    ...
    if (is_auth()) {
        ...
        if (!was_subtree) inode->auth_unpin(this);
    }
    else {
        // importing subtree ?
        _walk_tree([this](CDir* dir) {
...
void CDir::unfreeze_tree()
{
    ...
    if (freeze_tree_state) {
        _walk_tree([this, &unfreeze_waiters](CDir* dir) {
            if (dir->freeze_tree_state != freeze_tree_state) return false;
...
{
    if (!freeze_tree_state || dir->freeze_tree_state != freeze_tree_state) return;
    CDir* newdir = dir->get_inode()->get_parent_dir();
    if (newdir == this || newdir->freeze_tree_state == freeze_tree_state) return;
...
{
public:
    explicit C_Dir_AuthUnpin(CDir* d)
        : CDirContext(d)
    {}
    void finish(int r) override { dir->auth_unpin(dir->get_inode()); }
};

void CDir::maybe_finish_freeze()
{
    if (dir_auth_pins != 0) return;
    ...
    if (freeze_tree_state) {
        if (freeze_tree_state->frozen || freeze_tree_state->auth_pins != 1) return;
...
void CDir::_freeze_dir()
{
    ...
    if (is_auth() && !is_subtree_root()) inode->auth_pin(this);   // auth_pin for duration of freeze
}
...
void CDir::unfreeze_dir()
{
    ...
    if (state_test(STATE_FROZENDIR)) {
        ...
        if (is_auth() && !is_subtree_root()) inode->auth_unpin(this);
...
void CDir::scrub_finished()
{
    ...
    if (scrub_infop->header->get_recursive()) scrub_infop->last_recursive = scrub_infop->last_local;
...
{
    if (scrub_infop && !scrub_infop->directory_scrubbing && !scrub_infop->last_scrub_dirty) scrub_infop.reset();
}
...
{
    if (get_frag() == frag_t()) return false;

    if (inode->is_ephemeral_dist()) {
        unsigned min_frag_bits = mdcache->get_ephemeral_dist_frag_bits();
        if (min_frag_bits > 0 && get_frag().bits() < min_frag_bits + 1) return false;
    }
