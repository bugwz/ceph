...
void CDir::_omap_fetched(bufferlist& hdrbl, map<string, bufferlist>& omap, bool complete, int r)
{
    ...
    for (map<string, bufferlist>::reverse_iterator p = omap.rbegin(); p != omap.rend(); ++p, --pos) {
        string dname;
        snapid_t last;
        ...
        CDentry* dn = NULL;
        try {
            dn = _load_dentry(p->first, dname, last, p->second, pos, snaps, rand_threshold, &force_dirty);
        }
        catch (const buffer::error& err) {
            mdcache->mds->clog->warn() << "Corrupt dentry '" << dname
                                       << "' in "
                                          "dir frag "
