...
void inode_backtrace_t::decode(ceph::buffer::list::const_iterator& bl)
{
    ...
    if (struct_v < 3) return;   // sorry, the old data was crap
    decode(ino, bl);
    if (struct_v >= 4) {
        decode(ancestors, bl);
    }
...
int inode_backtrace_t::compare(const inode_backtrace_t& other, bool* equivalent, bool* divergent) const
{
    ...
    *divergent = false;
    ...
    if (ancestors[0].version > other.ancestors[0].version)
        ...
        comparator = -1;
    if (ancestors[0].dirino != other.ancestors[0].dirino || ancestors[0].dname != other.ancestors[0].dname)
        *divergent = true;
    for (int i = 1; i < min_size; ++i) {
        ...
        if (ancestors[i].dirino != other.ancestors[i].dirino || ancestors[i].dname != other.ancestors[i].dname) {
            *equivalent = false;
            return comparator;
        }
        else if (ancestors[i].version > other.ancestors[i].version) {
            if (comparator < 0) *divergent = true;
            comparator = 1;
        }
        else if (ancestors[i].version < other.ancestors[i].version) {
            if (comparator > 0) *divergent = true;
