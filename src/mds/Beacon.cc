...
void Beacon::handle_mds_beacon(const cref_t<MMDSBeacon>& m)
{
    ...
    if (it != seq_stamp.end()) {
        ...
        dout(5) << "received beacon reply " << ceph_mds_state_name(m->get_state()) << " seq " << m->get_seq() << " rtt "
                << rtt << dendl;
        ...
        dout(1) << "discarding unexpected beacon reply " << ceph_mds_state_name(m->get_state()) << " seq "
                << m->get_seq() << " dne" << dendl;
    }
}
...
bool Beacon::_send()
{
    ...
    if (!cct->get_heartbeat_map()->is_healthy()) {
        ...
        dout(0) << "Skipping beacon heartbeat to monitors (last acked " << since
    ...
    auto beacon = make_message<MMDSBeacon>(monc->get_fsid(),
                                           mds_gid_t(monc->get_global_id()),
                                           name,
                                           epoch,
                                           want_state,
                                           last_seq,
...
bool Beacon::is_laggy()
{
    ...
    if (since > g_conf()->mds_beacon_grace) {
        if (!laggy) {
            dout(1) << "MDS connection to Monitors appears to be laggy; " << since << "s since last acked beacon"
                    << dendl;
        }
        laggy = true;
        return true;
...
void Beacon::set_want_state(const MDSMap& mdsmap, MDSMap::DaemonState newstate)
{
    ...
    if (want_state != newstate) {
        dout(5) << __func__ << ": " << ceph_mds_state_name(want_state) << " -> " << ceph_mds_state_name(newstate)
                << dendl;
        want_state = newstate;
    }
}
...
void Beacon::notify_health(MDSRank const* mds)
{
    ...
    if (!mds->damage_table.empty()) {
        MDSHealthMetric m(MDS_HEALTH_DAMAGE, HEALTH_ERR, std::string("Metadata damage detected"));
        health.metrics.push_back(m);
    }
    ...
    {
        if (mds->mdlog->get_num_segments() >
            (size_t)(g_conf()->mds_log_max_segments * g_conf().get_val<double>("mds_log_warn_factor"))) {
            CachedStackStringStream css;
            *css << "Behind on trimming (" << mds->mdlog->get_num_segments() << "/" << g_conf()->mds_log_max_segments
                 << ")";
    ...
    // CLIENT_CAPS messages.
    {
        ...
        if (late_cap_metrics.size() <= (size_t)g_conf()->mds_health_summarize_threshold) {
            ...
            CachedStackStringStream css;
            *css << "Many clients (" << late_cap_metrics.size() << ") failing to respond to capability release";
    ...
    // Detect clients failing to advance their old_client_tid
    {
        ...
        for (auto& session : sessions) {
            ...
            if (recall_caps > recall_warning_threshold && num_caps > min_caps_working_set) {
                dout(2) << "Session " << *session << " is not releasing caps fast enough. Recalled caps at "
                        << recall_caps << " > " << recall_warning_threshold << " (mds_recall_warning_threshold)."
                        << dendl;
                CachedStackStringStream css;
        ...
        if (late_recall_metrics.size() <= (size_t)g_conf()->mds_health_summarize_threshold) {
            ...
            CachedStackStringStream css;
            *css << "Many clients (" << late_recall_metrics.size() << ") failing to respond to cache pressure";
...

    {
        ...
        if (MDSIOContextBase::check_ios_in_flight(cutoff, count, oldest)) {
            ...
            CachedStackStringStream css;
            *css << count << " slow metadata IOs are blocked > " << complaint_time << " secs, oldest blocked for "
                 << (int64_t)oldest_secs << " secs";
    ...
    if (mds->mdcache->is_readonly()) {
        MDSHealthMetric m(MDS_HEALTH_READ_ONLY, HEALTH_WARN, "MDS in read-only mode");
        health.metrics.push_back(m);
    }
    ...
    if (mds->mdcache->cache_overfull()) {
        ...
        *css << "MDS cache is too large (" << bytes2str(mds->mdcache->cache_size()) << "/"
             << bytes2str(mds->mdcache->cache_limit_memory()) << "); " << mds->mdcache->num_inodes_with_caps
             << " inodes in use by clients, " << mds->mdcache->get_num_strays() << " stray files";
