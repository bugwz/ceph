...
public:
    ...
    template<typename... Args> static inode_ptr allocate_inode(Args&&... args)
    {
    ...
    template<typename... Args> static xattr_map_ptr allocate_xattr_map(Args&&... args)
    {
    ...
    template<typename... Args> static old_inode_map_ptr allocate_old_inode_map(Args&&... args)
    {
    ...
    void reset_inode(inode_const_ptr&& ptr) { inode = std::move(ptr); }

    void reset_xattrs(xattr_map_const_ptr&& ptr) { xattrs = std::move(ptr); }

    void reset_old_inodes(old_inode_map_const_ptr&& ptr) { old_inodes = std::move(ptr); }
...
inline void decode_noshare(InodeStoreBase::mempool_xattr_map& xattrs, ceph::buffer::list::const_iterator& p)
{
...
public:
    ...
    {
        if (inode == empty_inode) reset_inode(allocate_inode());
        return const_cast<mempool_inode*>(inode.get());
    }
    mempool_xattr_map* get_xattrs() { return const_cast<mempool_xattr_map*>(xattrs.get()); }

    void encode(ceph::buffer::list& bl, uint64_t features) const { InodeStoreBase::encode(bl, features, &snap_blob); }
    void decode(ceph::buffer::list::const_iterator& bl) { InodeStoreBase::decode(bl, snap_blob); }
    void encode_bare(ceph::buffer::list& bl, uint64_t features) const
    {
        InodeStoreBase::encode_bare(bl, features, &snap_blob);
    }
    void decode_bare(ceph::buffer::list::const_iterator& bl) { InodeStoreBase::decode_bare(bl, snap_blob); }
...
{
public:
    void encode(ceph::buffer::list& bl, uint64_t features) const { InodeStore::encode_bare(bl, features); }
    void decode(ceph::buffer::list::const_iterator& bl) { InodeStore::decode_bare(bl); }
...
public:
    ...
    static const int MASK_STATE_EXPORT_KEPT =
        (STATE_FROZEN | STATE_AMBIGUOUSAUTH | STATE_EXPORTINGCAPS | STATE_QUEUEDEXPORTPIN | STATE_TRACKEDBYOFT |
         STATE_DELAYEDEXPORTPIN | STATE_DISTEPHEMERALPIN | STATE_RANDEPHEMERALPIN);
    ...
    {
        if (!scrub_infop) scrub_info_create();
    ...
    bool scrub_is_in_progress() const { return (scrub_infop && scrub_infop->scrub_in_progress); }
    /**
    ...
    projected_inode project_inode(const MutationRef& mut, bool xattr = false, bool snap = false);
    ...
    void record_snaprealm_parent_dentry(sr_t* new_snap, SnapRealm* newparent, CDentry* dn, bool primary_dn);
    ...
    template<typename Container> void get_dirfrags(Container& ls) const
    {
        // all dirfrags
        if constexpr (std::is_same_v<Container, std::vector<CDir*>>) ls.reserve(ls.size() + dirfrags.size());
        for (const auto& p : dirfrags) ls.push_back(p.second);
    }
    ...
    int get_num_subtree_roots() const { return num_subtree_roots; }
    ...
    mds_rank_t get_stray_owner() const { return (mds_rank_t)MDS_INO_STRAY_OWNER(ino()); }
    ...
    bool is_ambiguous_auth() const { return state_test(STATE_AMBIGUOUSAUTH) || MDSCacheObject::is_ambiguous_auth(); }
    void set_ambiguous_auth() { state_set(STATE_AMBIGUOUSAUTH); }
    void clear_ambiguous_auth(MDSContext::vec& finished);
    void clear_ambiguous_auth();

    const inode_const_ptr& get_inode() const { return inode; }
    ...
    bool is_lt(const MDSCacheObject* r) const override
    {
        ...
        return ino() < o->ino() || (ino() == o->ino() && last < o->last);
    }
    ...
    void _commit_ops(int r, C_GatherBuilder& gather_bld, std::vector<CInodeCommitOperation>& ops_vec,
                     inode_backtrace_t& bt);
    void build_backtrace(int64_t pool, inode_backtrace_t& bt);
    void _store_backtrace(std::vector<CInodeCommitOperation>& ops_vec, inode_backtrace_t& bt, int op_prio);
    ...
    bool is_waiting_for_dir(frag_t fg) { return waiting_on_dir.count(fg); }
    ...
    int encode_inodestat(ceph::buffer::list& bl, Session* session, SnapRealm* realm, snapid_t snapid = CEPH_NOSNAP,
    ...
    void finish_scatter_update(ScatterLock* lock, CDir* dir, version_t inode_version, version_t dir_accounted_version);
    ...
    Capability* get_client_cap(client_t client)
    {
        ...
        if (client_caps_entry != client_caps.end()) return &client_caps_entry->second;
        return 0;
    }
    ...
    Capability* add_client_cap(client_t client, Session* session, SnapRealm* conrealm = nullptr,
    ...
    int get_caps_allowed_for_client(Session* s, Capability* cap, const mempool_inode* file_i) const;

    // caps issued, wanted
    int get_caps_issued(int* ploner = 0, int* pother = 0, int* pxlocker = 0, int shift = 0, int mask = -1);
    ...
    {
        ceph_assert(parent == 0 || g_conf().get_val<bool>("mds_hack_allow_loading_invalid_metadata"));
        parent = p;
    }
    ...
    int num_remote_parents() { return remote_parents.size(); }

    void push_projected_parent(CDentry* dn) { projected_parent.push_back(dn); }
    ...
    bool is_parent_projected() const { return !projected_parent.empty(); }
    ...
    bool is_ephemeral_dist() const { return state_test(STATE_DISTEPHEMERALPIN); }
    ...
    bool is_ephemeral_rand() const { return state_test(STATE_RANDEPHEMERALPIN); }
    ...
    {
        return state_test(STATE_DISTEPHEMERALPIN) || state_test(STATE_RANDEPHEMERALPIN);
    }
     ...
     */
    void validate_disk_state(validated_data* results, MDSContext* fin);
    static void dump_validation_results(const validated_data& results, ceph::Formatter* f);
...
protected:
    ...
    {
        if (!fcntl_locks) fcntl_locks = new ceph_lock_state_t(g_ceph_context, CEPH_LOCK_FCNTL);
        return fcntl_locks;
    }
    ...
    {
        if (!flock_locks) flock_locks = new ceph_lock_state_t(g_ceph_context, CEPH_LOCK_FLOCK);
        return flock_locks;
    }
    ...
    void _encode_file_locks(ceph::buffer::list& bl) const
    {
        ...
        if (has_fcntl_locks) encode(*fcntl_locks, bl);
        bool has_flock_locks = flock_locks && !flock_locks->empty();
        encode(has_flock_locks, bl);
        if (has_flock_locks) encode(*flock_locks, bl);
    }
...
private:
    ...
    bool _validate_disk_state(class ValidationContinuation* c, int rval, int stage);
