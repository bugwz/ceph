...
struct MDSCapSpec
{
    ...
    MDSCapSpec() = default;
    MDSCapSpec(unsigned _caps)
        : caps(_caps)
    {
        if (caps & ALL) caps |= RWFPS;
    }

    bool allow_all() const { return (caps & ALL); }
    bool allow_read() const { return (caps & READ); }
    bool allow_write() const { return (caps & WRITE); }

    bool allows(bool r, bool w) const
    {
        if (allow_all()) return true;
        if (r && !allow_read()) return false;
        if (w && !allow_write()) return false;
        return true;
    }

    bool allow_snapshot() const { return (caps & SNAPSHOT); }
    bool allow_set_vxattr() const { return (caps & SET_VXATTR); }
    bool allow_full() const { return (caps & FULL); }
...
struct MDSCapMatch
{
    ...
    bool is_match_all() const { return uid == MDS_AUTH_UID_ANY && path == ""; }

    // check whether this grant matches against a given file and caller uid:gid
    bool match(std::string_view target_path, const int caller_uid, const int caller_gid,
...
struct MDSCapGrant
{
    MDSCapGrant(const MDSCapSpec& spec_, const MDSCapMatch& match_, boost::optional<std::string> n)
        : spec(spec_)
        , match(match_)
    {
        if (n) {
...
public:
    ...
    bool is_capable(std::string_view inode_path, uid_t inode_uid, gid_t inode_gid, unsigned inode_mode, uid_t uid,
                    gid_t gid, const std::vector<uint64_t>* caller_gid_list, unsigned mask, uid_t new_uid,
                    gid_t new_gid, const entity_addr_t& addr) const;
    ...
    bool fs_name_capable(std::string_view fs_name, unsigned mask) const
    {
        ...
        for (const MDSCapGrant& g : grants) {
            if (g.match.fs_name == fs_name || g.match.fs_name.empty() || g.match.fs_name == "*") {
