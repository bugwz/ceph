...
{
    if (m->get_type() == CEPH_MSG_CLIENT_METRICS && m->get_connection()->get_peer_type() == CEPH_ENTITY_TYPE_CLIENT) {
        handle_client_metrics(ref_cast<MClientMetrics>(m));
        return true;
    }
    else if (m->get_type() == MSG_MDS_PING && m->get_connection()->get_peer_type() == CEPH_ENTITY_TYPE_MDS) {
...
{
    dout(20) << ": type=" << payload.get_type() << ", session=" << session << ", hits=" << payload.cap_hits
             << ", misses=" << payload.cap_misses << dendl;
...
{
    dout(20) << ": type=" << payload.get_type() << ", session=" << session << ", latency=" << payload.lat
             << ", avg=" << payload.mean << ", sq_sum=" << payload.sq_sum << ", count=" << payload.count << dendl;
...
{
    dout(20) << ": type=" << payload.get_type() << ", session=" << session << ", hits=" << payload.dlease_hits
             << ", misses=" << payload.dlease_misses << dendl;
...
{
    dout(20) << ": type=" << payload.get_type() << ", session=" << session << ", opened_files=" << payload.opened_files
...
{
    dout(20) << ": type=" << payload.get_type() << ", session=" << session << ", pinned_icaps=" << payload.pinned_icaps
             << ", total_inodes=" << payload.total_inodes << dendl;
...
void MetricsHandler::handle_payload(Session* session, const OpenedInodesPayload& payload)
{
    dout(20) << ": type=" << payload.get_type() << ", session=" << session
             << ", opened_inodes=" << payload.opened_inodes << ", total_inodes=" << payload.total_inodes << dendl;
...
{
    dout(20) << ": type=" << payload.get_type() << ", session=" << session << ", total_ops=" << payload.total_ops
...
void MetricsHandler::update_rank0()
{
    ...
    dout(20) << ": sending metric updates for " << update_client_metrics_map.size()
             << " clients to rank 0 (address: " << *addr_rank0 << ") with sequence number " << next_seq
