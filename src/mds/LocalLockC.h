...
public:
    ...
    bool is_locallock() const override { return true; }

    bool can_xlock_local() const { return !is_wrlocked() && (get_xlock_by() == MutationRef()); }
    ...
    void put_wrlock()
    {
        ...
        if (get_num_wrlocks() == 0) last_wrlock_client = client_t();
    }
    client_t get_last_wrlock_client() const { return last_wrlock_client; }

    void print(std::ostream& out) const override
    {
        out << "(";
        _print(out);
        if (last_wrlock_client >= 0) out << " last_client=" << last_wrlock_client;
        out << ")";
    }
