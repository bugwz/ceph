...
public:
    ...
    {
        auto it = import_state.find(df);
        if (it == import_state.end()) return false;
        if (it->second.state >= IMPORT_LOGGINGSTART && it->second.state < IMPORT_ABORTING) return true;
        return false;
    }
    ...
    void encode_export_inode(CInode* in, bufferlist& bl, std::map<client_t, entity_inst_t>& exported_client_map,
    ...
    void encode_export_inode_caps(CInode* in, bool auth_cap, bufferlist& bl,
                                  ...
                                  std::map<client_t, client_metadata_t>& exported_client_metadata_map);
    void finish_export_inode(CInode* in, mds_rank_t target, std::map<client_t, Capability::Import>& peer_imported,
                             MDSContext::vec& finished);
    void finish_export_inode_caps(CInode* in, mds_rank_t target, std::map<client_t, Capability::Import>& peer_imported);


    void encode_export_dir(bufferlist& exportbl, CDir* dir, std::map<client_t, entity_inst_t>& exported_client_map,
                           std::map<client_t, client_metadata_t>& exported_client_metadata_map, uint64_t& num_exported);
    ...
    void decode_import_inode(CDentry* dn, bufferlist::const_iterator& blp, mds_rank_t oldauth, LogSegment* ls,
    ...
    void decode_import_dir(bufferlist::const_iterator& blp, mds_rank_t oldauth, CDir* import_root, EImportStart* le,
                           LogSegment* ls, std::map<CInode*, std::map<client_t, Capability::Export> >& cap_imports,
...
protected:
    ...
    void logged_import_caps(CInode* in, mds_rank_t from,
                            std::map<client_t, std::pair<Session*, uint64_t> >& imported_session_map,
                            std::map<CInode*, std::map<client_t, Capability::Export> >& cap_imports);
