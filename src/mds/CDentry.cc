...
#undef dout_prefix
#define dout_prefix \
    *_dout << "mds." << dir->mdcache->mds->get_nodeid() << ".cache.den(" << dir->dirfrag() << " " << name << ") "

using namespace std;

ostream& CDentry::print_db_line_prefix(ostream& out) const
{
    return out << ceph_clock_now() << " mds." << dir->mdcache->mds->get_nodeid() << ".cache.den(" << dir->ino() << " "
               << name << ") ";
}
...
ostream& operator<<(ostream& out, const CDentry& dn)
{
    ...
    if (dn.is_auth()) {
        out << " auth";
        if (dn.is_replicated()) out << dn.get_replicas();
    }
    else {
    ...
    if (!dn.lock.is_sync_and_unlocked()) out << " " << dn.lock;
    if (!dn.versionlock.is_sync_and_unlocked()) out << " " << dn.versionlock;

    if (dn.get_projected_version() != dn.get_version()) out << " pv=" << dn.get_projected_version();
...
bool operator<(const CDentry& l, const CDentry& r)
{
    if ((l.get_dir()->ino() < r.get_dir()->ino()) ||
        (l.get_dir()->ino() == r.get_dir()->ino() &&
         (l.get_name() < r.get_name() || (l.get_name() == r.get_name() && l.last < r.last))))
        return true;
    return false;
}
...
void CDentry::_mark_dirty(LogSegment* ls)
{
    ...
    if (ls) ls->dirty_dentries.push_back(&item_dirty);
}
...
void CDentry::link_remote(CDentry::linkage_t* dnl, CInode* in)
{
    ...
    if (dnl == &linkage) in->add_remote_parent(this);
...
void CDentry::unlink_remote(CDentry::linkage_t* dnl)
{
    ...
    if (dnl == &linkage) dnl->inode->remove_remote_parent(this);
...
void CDentry::push_projected_linkage()
{
    ...
    if (is_auth()) {
        ...
        if (diri->is_stray()) diri->mdcache->notify_stray_removed();
    }
}
...
void CDentry::push_projected_linkage(CInode* inode)
{
    ...
    if (dirty_rstat) inode->clear_dirty_rstat();

    _project_linkage()->inode = inode;
    inode->push_projected_parent(this);

    if (dirty_rstat) inode->mark_dirty_rstat();

    if (is_auth()) {
        CInode* diri = dir->inode;
        if (diri->is_stray()) diri->mdcache->notify_stray_created();
    }
}
...
int CDentry::get_num_dir_auth_pins() const
{
    ...
    if (get_linkage()->is_primary()) return auth_pins + get_linkage()->get_inode()->get_num_auth_pins();
    return auth_pins;
}
...
void CDentry::remove_client_lease(ClientLease* l, Locker* locker)
{
    ...
    if (gather) locker->eval_gather(&lock);
}

void CDentry::remove_client_leases(Locker* locker)
{
    while (!client_lease_map.empty()) remove_client_lease(client_lease_map.begin()->second, locker);
}
...
void CDentry::_put()
{
    if (get_num_ref() <= ((int)is_dirty() + 1)) {
        ...
        if (dnl->is_primary()) {
            ...
            if (get_num_ref() == (int)is_dirty() + !!in->get_num_ref()) in->mdcache->maybe_eval_stray(in, true);
        }
    }
}

void CDentry::encode_remote(inodeno_t& ino, unsigned char d_type, std::string_view alternate_name, bufferlist& bl)
{
...
void CDentry::decode_remote(char icode, inodeno_t& ino, unsigned char& d_type, mempool::mds_co::string& alternate_name,
                            ceph::buffer::list::const_iterator& bl)
{
    if (icode == 'l') {
        ...
        decode(ino, bl);
        decode(d_type, bl);
        if (struct_v >= 2) decode(alternate_name, bl);
        DECODE_FINISH(bl);
    }
    else if (icode == 'L') {
        decode(ino, bl);
...
void CDentry::dump(Formatter* f) const
{
    ...
    if (state_test(STATE_NEW)) f->dump_string("state", "new");
    if (state_test(STATE_FRAGMENTING)) f->dump_string("state", "fragmenting");
    if (state_test(STATE_PURGING)) f->dump_string("state", "purging");
    if (state_test(STATE_BADREMOTEINO)) f->dump_string("state", "badremoteino");
    if (state_test(STATE_STRAY)) f->dump_string("state", "stray");
    f->close_section();
}
