...
inodeno_t InoTable::project_alloc_id(inodeno_t id)
{
    ...
    if (!id) id = projected_free.range_start();
...
void InoTable::project_alloc_ids(interval_set<inodeno_t>& ids, int want)
{
    ...
    while (want > 0) {
        ...
        if (num > (inodeno_t)want) num = want;
...
void InoTable::replay_alloc_id(inodeno_t id)
{
    ...
    if (free.contains(id)) {
        ...
        mds->clog->error() << "journal replay alloc " << id << " not in free " << free;
    }
...
void InoTable::replay_alloc_ids(interval_set<inodeno_t>& ids)
{
    ...
    if (!(is == ids)) {
        mds->clog->error() << "journal replay alloc " << ids << ", only " << is << " is in free " << free;
    }
    free.subtract(is);
...
bool InoTable::intersects_free(const interval_set<inodeno_t>& other, interval_set<inodeno_t>* intersection)
{
