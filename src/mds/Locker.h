...
public:
    ...
    bool acquire_locks(MDRequestRef& mdr, MutationImpl::LockOpVec& lov, CInode* auth_pin_freeze = NULL,
                       bool auth_pin_nonblocking = false);

    bool try_rdlock_snap_layout(CInode* in, MDRequestRef& mdr, int n = 0, bool want_layout = false);
    ...
    int get_allowed_caps(CInode* in, Capability* cap, int& all_allowed, int& loner_allowed, int& xlocker_allowed);
    ...
    bool calc_new_client_ranges(CInode* in, uint64_t size, bool* max_increased = nullptr);
    bool check_inode_max_size(CInode* in, bool force_wrlock = false, uint64_t newmax = 0, uint64_t newsize = 0,
...
protected:
    ...
    void _do_snap_update(CInode* in, snapid_t snap, int dirty, snapid_t follows, client_t client,
    ...
    void file_update_finish(CInode* in, MutationRef& mut, unsigned flags, client_t client,
