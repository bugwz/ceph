...
{
public:
    C_Flush_Journal(MDCache* mdcache, MDLog* mdlog, MDSRank* mds, std::ostream* ss, Context* on_finish)
...
private:
    void flush_mdlog()
    {
        ...
        Context* ctx = new LambdaContext([this](int r) { handle_flush_mdlog(r); });
    ...
    void clear_mdlog()
    {
        ...
        Context* ctx = new LambdaContext([this](int r) { handle_clear_mdlog(r); });
    ...
    void expire_segments()
    {
        ...
        Context* ctx = new LambdaContext([this](int r) { handle_expire_segments(r); });
    ...
    void trim_expired_segments()
    {
        dout(5) << __func__ << ": expiry complete, expire_pos/trim_pos is now " << std::hex
                << mdlog->get_journaler()->get_expire_pos() << "/" << mdlog->get_journaler()->get_trimmed_pos()
                << dendl;
...
{
public:
    C_Drop_Cache(Server* server, MDCache* mdcache, MDLog* mdlog, MDSRank* mds, uint64_t recall_timeout, Formatter* f,
                 Context* on_finish)
        : MDSInternalContext(mds)
        , server(server)
        , mdcache(mdcache)
...
private:
    ...
    public:
        C_ContextTimeout(MDSRank* mds, uint64_t timeout, Context* on_finish)
            ...
            , on_finish(on_finish)
        {}
        ~C_ContextTimeout() { ceph_assert(timer_task == nullptr); }
    ...
    void recall_client_state()
    {
        ...
        if ((throttled || count > 0) && (recall_timeout == 0 || duration < recall_timeout)) {
            C_ContextTimeout* ctx =
                new C_ContextTimeout(mds, 1, new LambdaContext([this](int r) { recall_client_state(); }));
            ctx->start_timer();
            ...
            if (!gather.has_subs()) {
                ...
                C_ContextTimeout* ctx = new C_ContextTimeout(
                    mds, remaining, new LambdaContext([this](int r) { handle_recall_client_state(r); }));
    ...
    void flush_journal()
    {
        ...
        Context* ctx = new LambdaContext([this](int r) { handle_flush_journal(r); });
    ...
    void trim_cache()
    {
        ...
        if (throttled && count > 0) {
            auto timer = new LambdaContext([this](int) { trim_cache(); });
...
MDSRank::MDSRank(mds_rank_t whoami_, ceph::fair_mutex& mds_lock_, LogChannelRef& clog_,
                 CommonSafeTimer<ceph::fair_mutex>& timer_, Beacon& beacon_, std::unique_ptr<MDSMap>& mdsmap_,
                 Messenger* msgr, MonClient* monc_, MgrClient* mgrc, Context* respawn_hook_, Context* suicide_hook_,
    ...
    , sessionmap(this)
    , op_tracker(g_ceph_context, g_conf()->mds_enable_op_tracker, g_conf()->osd_num_op_tracker_shard)
    , progress_thread(this)
    , whoami(whoami_)
    , purge_queue(g_ceph_context, whoami_, mdsmap_->get_metadata_pool(), objecter, new LambdaContext([this](int r) {
    ...
    , ioc(ioc)
{
    ...
    op_tracker.set_complaint_and_threshold(cct->_conf->mds_op_complaint_time, cct->_conf->mds_op_log_threshold);
    op_tracker.set_history_size_and_duration(cct->_conf->mds_op_history_size, cct->_conf->mds_op_history_duration);
...
void MDSRank::hit_export_target(mds_rank_t rank, double amount)
{
    ...
    if (amount < 0.0) {
        amount =
            100.0 /
            g_conf()->mds_bal_target_decay; /* a good default for "i am trying to keep this export_target active" */
    }
    auto em = export_targets.emplace(
        std::piecewise_construct, std::forward_as_tuple(rank), std::forward_as_tuple(DecayRate(rate)));
...
public:
    ...
    C_MDS_MonCommand(MDSRank* m, std::string_view c)
        ...
        , cmd(c)
    {}
    void finish(int r) override { mds->_mon_command_finish(r, cmd, outs); }
};
...
void MDSRankDispatcher::tick()
{
    ...
    if (is_reconnect()) server->reconnect_tick();
    ...
    if (is_active()) {
        ...
        if (mdsmap->get_tableserver() == whoami) {
            ...
            if (!mdsmap->allows_multimds_snaps() && snapserver->can_allow_multimds_snaps()) {
                set_mdsmap_multimds_snaps_allowed();
            }
        }

        if (whoami == 0) scrubstack->advance_scrub_status();
    }
    ...
    if (is_stopping()) {
        ...
        if (mdcache->shutdown_pass()) {
            ...
            if (!purge_queue.drain(&pq_progress, &pq_total, &pq_in_flight)) {
                ...
                // in this state.
                clog->info() << "MDS rank " << whoami << " waiting for purge queue (" << std::dec << pq_progress << "/"
                             << pq_total << " " << pq_in_flight << " files purging"
                             << ")";
            }
            else {
...
void MDSRankDispatcher::shutdown()
{
    ...
    if (objecter->initialized) objecter->shutdown();
...
public:
    ...
    void finish(int r) override { (mds->*fn)(); }
};
...
void* MDSRank::ProgressThread::entry()
{
    ...
    while (true) {
        cond.wait(l, [this] {
            return (mds->stopping || !mds->finished_queue.empty() ||
...
bool MDSRankDispatcher::ms_dispatch(const cref_t<Message>& m)
{
    if (m->get_source().is_mds()) {
        ...
        if (!op) dout(0) << typeid(*msg).name() << " is not an MMDSOp type" << dendl;
        ceph_assert(op);
    }
    else if (m->get_source().is_client()) {
        Session* session = static_cast<Session*>(m->get_connection()->get_priv().get());
        if (session) session->last_seen = Session::clock::now();
    }
...
bool MDSRank::_dispatch(const cref_t<Message>& m, bool new_msg)
{
    ...
    if (dispatch_depth > 1) return true;
    ...
    for (int i = 0; i < g_conf()->mds_thrash_exports; i++) {
        ...
        if (s.size() < 2 || CInode::count() < 10) break;   // need peers for this to work.
...
bool MDSRank::is_valid_message(const cref_t<Message>& m)
{
    ...
    if (port == MDS_PORT_CACHE || port == MDS_PORT_MIGRATOR || type == CEPH_MSG_CLIENT_SESSION ||
        type == CEPH_MSG_CLIENT_RECONNECT || type == CEPH_MSG_CLIENT_RECLAIM || type == CEPH_MSG_CLIENT_REQUEST ||
        type == MSG_MDS_PEER_REQUEST || type == MSG_MDS_HEARTBEAT || type == MSG_MDS_TABLE_REQUEST ||
        type == MSG_MDS_LOCK || type == MSG_MDS_INODEFILECAPS || type == MSG_MDS_SCRUB || type == MSG_MDS_SCRUB_STATS ||
        type == CEPH_MSG_CLIENT_CAPS || type == CEPH_MSG_CLIENT_CAPRELEASE || type == CEPH_MSG_CLIENT_LEASE) {
        return true;
    }
...
#define ALLOW_MESSAGES_FROM(peers)                                                             \
    do {                                                                                       \
        if (m->get_connection() && (m->get_connection()->get_peer_type() & (peers)) == 0) {    \
            dout(0) << __FILE__ << "." << __LINE__                                             \
                    << ": filtered out request, peer=" << m->get_connection()->get_peer_type() \
...
void MDSRank::handle_message(const cref_t<Message>& m)
{
    ...
    default:
        ...
        case CEPH_MSG_CLIENT_RECLAIM:
            ...
            // fall-thru
        case CEPH_MSG_CLIENT_REQUEST: server->dispatch(m); break;
        ...
        default: derr << "unrecognized message " << *m << dendl;
        }
    }
}
...
void MDSRank::heartbeat_reset()
{
    ...
    g_ceph_context->get_heartbeat_map()->reset_timeout(
        hb, ceph::make_timespan(heartbeat_grace), ceph::timespan::zero());
}
...
bool MDSRank::is_stale_message(const cref_t<Message>& m) const
{
    ...
    if (m->get_source().is_mds()) {
        ...
        if (mdsmap->is_down(from)) {
            ...
            auto c = messenger->connect_to(CEPH_ENTITY_TYPE_MDS, mdsmap->get_addrs(from));
            if (c != m->get_connection()) {
                bad = true;
                dout(5) << " mds." << from << " should be " << c << " " << c->get_peer_addrs()
                        << " but this message is " << m->get_connection() << " " << m->get_source_addrs() << dendl;
            }
        }
        if (bad) {
            // bogus mds?
            ...
            else if (m->get_type() == MSG_MDS_CACHEEXPIRE && mdsmap->get_addrs(from) == m->get_source_addrs()) {
                dout(5) << "got " << *m << " from down mds " << m->get_source()
                        << ", but it's a cache_expire, looking at it" << dendl;
            }
            else {
                dout(5) << "got " << *m << " from down/old/bad/imposter mds " << m->get_source() << ", dropping"
                        << dendl;
                return true;
            }
        }
...
Session* MDSRank::get_session(const cref_t<Message>& m)
{
    ...
    if (session) {
        dout(20) << "get_session have " << session << " " << session->info.inst << " state "
                 << session->get_state_name() << dendl;
        ...
        if (session->is_closed() && m->get_type() == CEPH_MSG_CLIENT_SESSION) {
            ...
            if (imported_session && imported_session != session) {
                dout(10) << __func__ << " replacing connection bootstrap session " << session
                         << " with imported session " << imported_session << dendl;
...
public:
    C_MDS_RetrySendMessageMDS(MDSRank* mds, mds_rank_t who, ref_t<Message> m)
        ...
        , m(std::move(m))
    {}
    void finish(int r) override { mds->send_message_mds(m, who); }
...
public:
    C_MDS_BootStart(MDSRank* m, MDSRank::BootStep n)
        ...
        , nextstep(n)
    {}
    void finish(int r) override { mds->boot_start(nextstep, r); }
};
...
void MDSRank::boot_start(BootStep step, int r)
{
    ...
    if (r < 0) {
        if (is_standby_replay() && (r == -CEPHFS_EAGAIN)) {
            ...
            clog->error() << "Error loading MDS rank " << whoami << ": " << cpp_strerror(r);
            damaged();
    ...
    case MDS_BOOT_INITIAL:
    {
        ...
        MDSGatherBuilder gather(g_ceph_context, new C_MDS_BootStart(this, MDS_BOOT_OPEN_ROOT));
    ...
    case MDS_BOOT_PREPARE_LOG:
        if (is_any_replay()) {
            ...
            MDSGatherBuilder gather(g_ceph_context, new C_MDS_BootStart(this, MDS_BOOT_REPLAY_DONE));
...
void MDSRank::replay_start()
{
    ...
    bool const ready = objecter->with_osdmap(
        [this](const OSDMap& o) { return o.get_epoch() >= mdsmap->get_last_failure_osd_epoch(); });
...
public:
    ...
    {}
    void finish(int r) override { mds->_standby_replay_restart_finish(r, old_read_pos); }
    void print(ostream& out) const override { out << "standby_replay_restart"; }
};
...
{
public:
    explicit C_MDS_StandbyReplayRestart(MDSRank* m)
        : MDSInternalContext(m)
    {}
    void finish(int r) override
    {
...
void MDSRank::replay_done()
{
    ...
    if (is_standby_replay()) {
        ...
        timer.add_event_after(g_conf()->mds_replay_interval, new C_MDS_StandbyReplayRestart(this));
        return;
    }
    else if (standby_replaying) {
    ...
    if (mdsmap->get_tableserver() == whoami && snapserver->upgrade_format()) {
    ...
    if (mdsmap->get_num_in_mds() == 1 && mdsmap->get_num_failed_mds() == 0) {   // just me!
...
void MDSRank::apply_blocklist(const std::set<entity_addr_t>& addrs, epoch_t epoch)
{
    ...
    dout(4) << __func__ << ": killed " << victims << ", blocklisted sessions (" << addrs.size()
            << " blocklist entries, " << sessionmap.get_sessions().size() << ")" << dendl;
    if (victims) {
...
void MDSRank::active_start()
{
    ...
    if (last_state == MDSMap::STATE_CREATING || last_state == MDSMap::STATE_STARTING) {
        mdcache->open_root();
    }
...
void MDSRank::recovery_done(int oldstate)
{
    ...
    if (oldstate == MDSMap::STATE_CREATING) return;
...
void MDSRank::stopping_start()
{
    ...
    if (mdsmap->get_num_in_mds() == 1 && !sessionmap.empty()) {
        ...
        for (const auto& s : victims) {
            CachedStackStringStream css;
            evict_client(s->get_client().v, false, g_conf()->mds_session_blocklist_on_evict, *css, gather.new_sub());
        }
        gather.activate();
    }
...
void MDSRankDispatcher::handle_mds_map(const cref_t<MMDSMap>& m, const MDSMap& oldmap)
{
    ...
    if (m->get_source().is_mds() && peer_mdsmap_epoch[mds_rank_t(m->get_source().num())] < epoch) {
    ...
    if (!MDSMap::state_transition_valid(oldstate, state)) {
        derr << "Invalid state transition " << ceph_mds_state_name(oldstate) << "->" << ceph_mds_state_name(state)
             << dendl;
        respawn();
    }
    ...
    if (oldstate != state) {
        ...
        if (state == MDSMap::STATE_STANDBY_REPLAY) {
            dout(1) << "handle_mds_map i am now mds." << mds_gid << "." << incarnation << " replaying mds." << whoami
                    << "." << incarnation << dendl;
    ...
    if (objecter->get_client_incarnation() != incarnation) objecter->set_client_incarnation(incarnation);

    if (mdsmap->get_required_client_features() != oldmap.get_required_client_features())
        server->update_required_client_features();

    // for debug
    if (g_conf()->mds_dump_cache_on_map) mdcache->dump_cache();
        ...
        for (const auto& r : up) {
            ...
            if (oldmap.have_inst(r)) {
                ...
                if (info.inc != oldinfo.inc) {
                    ...
                    if (info.state == MDSMap::STATE_REPLAY || info.state == MDSMap::STATE_RESOLVE) {
                        restart.insert(r);
                        handle_mds_failure(r);
                    }
                    else {
                        ceph_assert(info.state == MDSMap::STATE_STARTING || info.state == MDSMap::STATE_ACTIVE);
                        // -> stopped (missing) -> starting -> active
                        restart.insert(r);
                        mdcache->migrator->handle_mds_failure_or_stop(r);
                        if (mdsmap->get_tableserver() == whoami) snapserver->handle_mds_failure_or_stop(r);
                    }
                }
            }
            else {
                if (info.state == MDSMap::STATE_REPLAY || info.state == MDSMap::STATE_RESOLVE) {
                    ...
                    ceph_assert(info.state == MDSMap::STATE_CREATING || info.state == MDSMap::STATE_STARTING ||
    ...
    // did it change?
    if (oldstate != state) {
        dout(1) << "handle_mds_map state change " << ceph_mds_state_name(oldstate) << " --> "
        ...
        if (oldstate == MDSMap::STATE_STANDBY_REPLAY) {
            ...
            if ((is_active() || is_clientreplay()) &&
                (oldstate == MDSMap::STATE_CREATING || oldstate == MDSMap::STATE_REJOIN ||
    ...
    if (state >= MDSMap::STATE_REJOIN) {
        // did we start?
        if (!oldmap.is_rejoining() && mdsmap->is_rejoining()) rejoin_joint_start();

        // did we finish?
        if (g_conf()->mds_dump_cache_after_rejoin && oldmap.is_rejoining() && !mdsmap->is_rejoining())
            mdcache->dump_cache();   // for DEBUG only

        if (oldstate >= MDSMap::STATE_REJOIN || oldstate == MDSMap::STATE_STARTING) {
    ...
    if (state >= MDSMap::STATE_CLIENTREPLAY && oldstate >= MDSMap::STATE_CLIENTREPLAY) {
    ...
    if (is_active()) {
        ...
        set_osd_epoch_barrier(objecter->with_osdmap(std::mem_fn(&OSDMap::get_epoch)));
...
void MDSRank::handle_mds_failure(mds_rank_t who)
{
    ...
    if (mdsmap->get_tableserver() == whoami) snapserver->handle_mds_failure_or_stop(who);
...
void MDSRankDispatcher::handle_asok_command(std::string_view command, const cmdmap_t& cmdmap, Formatter* f,
                                            ...
                                            std::function<void(int, const std::string&, bufferlist&)> on_finish)
{
    ...
    if (command == "dump_ops_in_flight") {
    ...
    else if (command == "session ls" || command == "client ls") {
        std::lock_guard l(mds_lock);
        bool cap_dump = false;
    ...
    else if (command == "session evict" || command == "client evict") {
        std::lock_guard l(mds_lock);
        std::vector<std::string> filter_args;
        cmd_getval(cmdmap, "filters", filter_args);
        ...
        std::lock_guard l(mds_lock);
        bool evicted =
            evict_client(strtol(client_id.c_str(), 0, 10), true, g_conf()->mds_session_blocklist_on_evict, *css);
        if (!evicted) {
            dout(15) << css->strv() << dendl;
            r = -CEPHFS_ENOENT;
        }
    }
    else if (command == "session config" || command == "client config") {
        int64_t client_id;
        std::string option;
        std::string value;
    ...
    else if (command == "scrub start" || command == "scrub_start") {
        if (whoami != 0) {
            *css << "Not rank 0";
            r = -CEPHFS_EXDEV;
            goto out;
        ...
        finisher->queue(new LambdaContext([this, on_finish, f, path, tag, scrubop_vec](int r) {
            command_scrub_start(f, path, tag, scrubop_vec, new LambdaContext([on_finish](int r) {
                                    bufferlist outbl;
                                    on_finish(r, {}, outbl);
                                }));
        }));
        ...
        finisher->queue(new LambdaContext([this, on_finish, f](int r) {
            command_scrub_abort(f, new LambdaContext([on_finish, f](int r) {
                                    bufferlist outbl;
                                    f->open_object_section("result");
                                    f->dump_int("return_code", r);
                                    f->close_section();
        ...
        finisher->queue(new LambdaContext([this, on_finish, f](int r) {
            command_scrub_pause(f, new LambdaContext([on_finish, f](int r) {
                                    bufferlist outbl;
                                    f->open_object_section("result");
                                    f->dump_int("return_code", r);
                                    f->close_section();
        ...
        int64_t timeout = 0;
        cmd_getval(cmdmap, "timeout", timeout);
        finisher->queue(new LambdaContext([this, on_finish, f, timeout](int r) {
            command_cache_drop(timeout, f, new LambdaContext([on_finish](int r) {
                                   bufferlist outbl;
                                   on_finish(r, {}, outbl);
                               }));
        }));
 ...
 */
void MDSRankDispatcher::evict_clients(const SessionFilter& filter,
                                      std::function<void(int, const std::string&, bufferlist&)> on_finish)
{
    bufferlist outbl;
    if (is_any_replay()) {
    ...
    C_GatherBuilder gather(g_ceph_context, new LambdaContext([on_finish](int r) {
                               bufferlist bl;
                               on_finish(r, {}, bl);
                           }));
    for (const auto s : victims) {
...
void MDSRank::command_scrub_start(Formatter* f, std::string_view path, std::string_view tag,
                                  const vector<string>& scrubop_vec, Context* on_finish)
{
    ...
    std::lock_guard l(mds_lock);
    mdcache->enqueue_scrub(path, tag, force, recursive, repair, scrub_mdsdir, f, on_finish);
    // scrub_dentry() finishers will dump the data for us; we're done!
}

void MDSRank::command_tag_path(Formatter* f, std::string_view path, std::string_view tag)
{
    C_SaferCond scond;
    {
        std::lock_guard l(mds_lock);
...
void MDSRank::command_export_dir(Formatter* f, std::string_view path, mds_rank_t target)
{
...
int MDSRank::_command_export_dir(std::string_view path, mds_rank_t target)
{
    std::lock_guard l(mds_lock);
    filepath fp(path);
...
void MDSRank::command_dump_tree(const cmdmap_t& cmdmap, std::ostream& ss, Formatter* f)
{
    ...
    if (!cmd_getval(cmdmap, "depth", depth)) depth = -1;
    std::lock_guard l(mds_lock);
...
CDir* MDSRank::_command_dirfrag_get(const cmdmap_t& cmdmap, std::ostream& ss)
{
    std::string path;
    ...
    if (!dir->is_auth()) {
        ss << "frag " << dir->dirfrag() << " not auth (auth = " << dir->authority() << ")";
        return NULL;
    }
...
void MDSRank::dump_status(Formatter* f) const
{
    ...
    if (state == MDSMap::STATE_REPLAY || state == MDSMap::STATE_STANDBY_REPLAY) {
...
void MDSRank::create_logger()
{
    dout(10) << "create_logger" << dendl;
    {
        ...
        mds_plb.add_u64_counter(l_mds_request, "request", "Requests", "req", PerfCountersBuilder::PRIO_CRITICAL);
        mds_plb.add_time_avg(
            l_mds_reply_latency, "reply_latency", "Reply latency", "rlat", PerfCountersBuilder::PRIO_CRITICAL);
        mds_plb.add_u64(l_mds_inodes, "inodes", "Inodes", "inos", PerfCountersBuilder::PRIO_CRITICAL);
        mds_plb.add_u64_counter(
            l_mds_forward, "forward", "Forwarding request", "fwd", PerfCountersBuilder::PRIO_INTERESTING);
        mds_plb.add_u64(l_mds_caps, "caps", "Capabilities", "caps", PerfCountersBuilder::PRIO_INTERESTING);
        mds_plb.add_u64_counter(
            l_mds_exported_inodes, "exported_inodes", "Exported inodes", "exi", PerfCountersBuilder::PRIO_INTERESTING);
        mds_plb.add_u64_counter(
            l_mds_imported_inodes, "imported_inodes", "Imported inodes", "imi", PerfCountersBuilder::PRIO_INTERESTING);
        mds_plb.add_u64_counter(
            l_mds_slow_reply, "slow_reply", "Slow replies", "slr", PerfCountersBuilder::PRIO_INTERESTING);

        // caps msg stats
        mds_plb.add_u64_counter(l_mdss_handle_client_caps,
                                "handle_client_caps",
                                "Client caps msg",
                                "hcc",
                                PerfCountersBuilder::PRIO_INTERESTING);
        mds_plb.add_u64_counter(l_mdss_handle_client_caps_dirty,
                                "handle_client_caps_dirty",
                                "Client dirty caps msg",
                                "hccd",
                                PerfCountersBuilder::PRIO_INTERESTING);
        mds_plb.add_u64_counter(l_mdss_handle_client_cap_release,
        ...
        mds_plb.add_u64_counter(l_mdss_process_request_cap_release,
                                ...
                                "prcr",
                                PerfCountersBuilder::PRIO_INTERESTING);
        mds_plb.add_u64_counter(l_mdss_ceph_cap_op_revoke,
                                "ceph_cap_op_revoke",
                                "Revoke caps",
                                "crev",
                                PerfCountersBuilder::PRIO_INTERESTING);
        mds_plb.add_u64_counter(
            l_mdss_ceph_cap_op_grant, "ceph_cap_op_grant", "Grant caps", "cgra", PerfCountersBuilder::PRIO_INTERESTING);
        mds_plb.add_u64_counter(l_mdss_ceph_cap_op_trunc,
                                "ceph_cap_op_trunc",
                                "caps truncate notify",
                                "ctru",
                                PerfCountersBuilder::PRIO_INTERESTING);
        mds_plb.add_u64_counter(l_mdss_ceph_cap_op_flushsnap_ack,
        ...
        mds_plb.add_u64_counter(l_mds_dir_fetch_complete, "dir_fetch_complete", "Fetch complete dirfrag");
        mds_plb.add_u64_counter(l_mds_dir_fetch_keys, "dir_fetch_keys", "Fetch keys from dirfrag");
        ...
        mds_plb.add_u64_counter(l_mds_openino_dir_fetch, "openino_dir_fetch", "OpenIno incomplete directory fetchings");
        ...
        mds_plb.add_u64_counter(
            l_mds_traverse_dir_fetch, "traverse_dir_fetch", "Traverse incomplete directory content fetchings");
        ...
        mds_plb.add_u64_counter(
            l_mds_openino_backtrace_fetch, "openino_backtrace_fetch", "OpenIno backtrace fetchings");
        mds_plb.add_u64_counter(l_mds_openino_peer_discover, "openino_peer_discover", "OpenIno peer inode discovers");

        // scrub stats
        mds_plb.add_u64(l_mds_scrub_backtrace_fetch, "scrub_backtrace_fetch", "Scrub backtrace fetchings");
        mds_plb.add_u64(l_mds_scrub_set_tag, "scrub_set_tag", "Scrub set tags");
        mds_plb.add_u64(l_mds_scrub_backtrace_repaired, "scrub_backtrace_repaired", "Scrub backtraces repaired");
        mds_plb.add_u64(l_mds_scrub_inotable_repaired, "scrub_inotable_repaired", "Scrub inotable repaired");
        mds_plb.add_u64(l_mds_scrub_dir_inodes, "scrub_dir_inodes", "Scrub directory inodes");
        mds_plb.add_u64(l_mds_scrub_dir_base_inodes, "scrub_dir_base_inodes", "Scrub directory base inodes");
        mds_plb.add_u64(l_mds_scrub_dirfrag_rstats, "scrub_dirfrag_rstats", "Scrub dirfrags rstates");
        mds_plb.add_u64(l_mds_scrub_file_inodes, "scrub_file_inodes", "Scrub file inodes");
...

    {
        ...
        mdm_plb.add_u64(l_mdm_ino, "ino", "Inodes", "ino", PerfCountersBuilder::PRIO_INTERESTING);
        mdm_plb.add_u64(l_mdm_dn, "dn", "Dentries", "dn", PerfCountersBuilder::PRIO_INTERESTING);
...
{
    if (is_active() && mdsmap->get_tableserver() == whoami) {
...
int MDSRank::config_client(int64_t session_id, bool remove, const std::string& option, const std::string& value,
                           std::ostream& ss)
{
...
bool MDSRank::evict_client(int64_t session_id, bool wait, bool blocklist, std::ostream& err_ss, Context* on_killed)
{
    ceph_assert(ceph_mutex_is_locked_by_me(mds_lock));
    ...
    {
        CachedStackStringStream css;
        *css << "Evicting " << (blocklist ? "(and blocklisting) " : "") << "client session " << session_id << " ("
             << addr << ")";
    ...
    *css << "\"}";
    ...
    auto apply_blocklist = [this, cmd](std::function<void()> fn) {
        ...
        Context* on_blocklist_done = new LambdaContext([this, fn](int r) {
            objecter->wait_for_latest_osdmap(lambdafy(
                (new C_OnFinisher(new LambdaContext([this, fn](int r) {
                                      std::lock_guard l(mds_lock);
                                      auto epoch = objecter->with_osdmap([](const OSDMap& o) { return o.get_epoch(); });
    ...
    if (wait) {
        ...
        session = sessionmap.get_session(entity_name_t(CEPH_ENTITY_TYPE_CLIENT, session_id));
        if (!session) {
            dout(1) << "session " << session_id
                    << " was removed while we waited "
                       "for blocklist"
...
MDSRankDispatcher::MDSRankDispatcher(mds_rank_t whoami_, ceph::fair_mutex& mds_lock_, LogChannelRef& clog_,
                                     CommonSafeTimer<ceph::fair_mutex>& timer_, Beacon& beacon_,
                                     std::unique_ptr<MDSMap>& mdsmap_, Messenger* msgr, MonClient* monc_,
                                     MgrClient* mgrc, Context* respawn_hook_, Context* suicide_hook_,
                                     boost::asio::io_context& ioc)
    : MDSRank(whoami_, mds_lock_, clog_, timer_, beacon_, mdsmap_, msgr, monc_, mgrc, respawn_hook_, suicide_hook_, ioc)
{
...
void MDSRank::command_cache_drop(uint64_t timeout, Formatter* f, Context* on_finish)
{
    ...
    std::lock_guard locker(mds_lock);
    C_Drop_Cache* request = new C_Drop_Cache(server, mdcache, mdlog, this, timeout, f, on_finish);
    request->send();
}
...
{
    static const char* KEYS[] = {"clog_to_graylog",
...
void MDSRankDispatcher::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed)
{
    ...
    if (changed.count("mds_extraordinary_events_dump_interval")) {
        reset_event_flags();
        extraordinary_events_dump_interval =
            conf.get_val<std::chrono::seconds>("mds_extraordinary_events_dump_interval").count();
...
void MDSRank::schedule_update_timer_task()
{
    ...
    timer.add_event_after(g_conf().get_val<double>("mds_task_status_update_interval"),
                          new LambdaContext([this](int) { send_task_status(); }));
}
...
{
    dout(20) << __func__ << dendl;
    timer.add_event_after(extraordinary_events_dump_interval, new LambdaContext([this]() { inmemory_logger(); }));
}

void MDSRank::inmemory_logger()
{
    if (client_eviction_dump || beacon.missed_beacon_ack_dump || beacon.missed_internal_heartbeat_dump) {
        // dump the in-memory logs if any of these events occured recently
        dout(0) << __func__ << " client_eviction_dump " << client_eviction_dump << ", missed_beacon_ack_dump "
                << beacon.missed_beacon_ack_dump << ", missed_internal_heartbeat_dump "
                << beacon.missed_internal_heartbeat_dump << dendl;
        reset_event_flags();
