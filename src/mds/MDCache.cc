...
void MDCache::broadcast_quota_to_client(CInode* in, client_t exclude_ct, bool quota_change)
{
    ...
    if (!pi->quota.is_enable() && !quota_change) return;
...
bool MDCache::open_undef_inodes_dirfrags()
{
    ...
    for (set<CInode*>::iterator p = rejoin_undef_inodes.begin(); p != rejoin_undef_inodes.end(); ++p) {
        CInode* in = *p;
...
bool MDCache::trim_dentry(CDentry* dn, expiremap& expiremap)
{
    ...
    if (!(dnl->is_null() && dn->is_clean())) clear_complete = true;
    ...
    // remove dentry
    if (dn->last == CEPH_NOSNAP && dir->is_auth()) dir->add_to_bloom(dn);
    dir->remove_dentry(dn);

    if (clear_complete) dir->state_clear(CDir::STATE_COMPLETE);
                                   ...
                                   mds_rank_t* hint)
{
    ...
    for (unsigned i = 0; i < ancestors.size(); i++) {
        ...
        if (diri->state_test(CInode::STATE_REJOINUNDEF)) {
            ...
            while (dir->state_test(CDir::STATE_REJOINUNDEF) && dir->get_inode()->state_test(CInode::STATE_REJOINUNDEF))
                dir = dir->get_inode()->get_parent_dir();
...
void MDCache::handle_discover(const cref_t<MDiscover>& dis)
{
    ...
    for (unsigned i = 0; cur && (i < dis->get_want().depth() || dis->get_want().depth() == 0); i++) {
        ...
        if (!dn) {
            if (!curdir->is_complete() &&
                !(snapid == CEPH_NOSNAP && curdir->has_bloom() && !curdir->is_in_bloom(dis->get_dentry(i)))) {
                // readdir
