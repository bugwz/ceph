...
void MDCache::dispatch(const cref_t<Message>& m)
{
    ...
    case MSG_MDS_DENTRYUNLINK_ACK: handle_dentry_unlink_ack(ref_cast<MDentryUnlinkAck>(m)); break;
...
void MDCache::send_dentry_unlink(CDentry* dn, CDentry* straydn, MDRequestRef& mdr, bool unlinking)
{
    ...
    for (set<mds_rank_t>::iterator it = replicas.begin(); it != replicas.end(); ++it) {
        ...
        auto unlink = make_message<MDentryUnlink>(dn->get_dir()->dirfrag(), dn->get_name(), unlinking);
        if (straydn) {
...
void MDCache::handle_dentry_unlink_ack(const cref_t<MDentryUnlinkAck>& m)
{
    ...
    if (!dir) {
        ...
        if (!dn) {
            ...
            dout(7) << __func__ << " on " << *dn << " ref " << dn->replica_unlinking_ref << " -> "
