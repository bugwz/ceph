...
public:
    explicit MDCacheContext(MDCache* mdc_)
        : mdcache(mdc_)
    {}
};
...
public:
    explicit MDCacheLogContext(MDCache* mdc_)
        : mdcache(mdc_)
    {}
};
    ...
    , trim_counter(g_conf().get_val<double>("mds_cache_trim_decay_rate"))
{
    ...
    max_dir_commit_size = g_conf()->mds_dir_max_commit_size ? (g_conf()->mds_dir_max_commit_size << 20)
...
void MDCache::handle_conf_change(const std::set<std::string>& changed, const MDSMap& mdsmap)
{
    ...
    if (changed.count("mds_cache_memory_limit"))
        cache_memory_limit = g_conf().get_val<Option::size_t>("mds_cache_memory_limit");
    if (changed.count("mds_cache_reservation")) cache_reservation = g_conf().get_val<double>("mds_cache_reservation");
    ...
    if (changed.count("mds_health_cache_threshold"))
        cache_health_threshold = g_conf().get_val<double>("mds_health_cache_threshold");
    if (changed.count("mds_cache_mid")) lru.lru_set_midpoint(g_conf().get_val<double>("mds_cache_mid"));
...
void MDCache::add_inode(CInode* in)
{
    ...
    if (in->ino() < MDS_INO_SYSTEM_BASE) {
        ...
        if (in->is_base()) base_inodes.insert(in);
    }
}
...
void MDCache::remove_inode(CInode* o)
{
    ...
    if (o->is_dirty()) o->mark_clean();
    if (o->is_dirty_parent()) o->clear_dirty_parent();
    ...
    if (o->state_test(CInode::STATE_QUEUEDEXPORTPIN)) export_pin_queue.erase(o);

    if (o->state_test(CInode::STATE_DELAYEDEXPORTPIN)) export_pin_delayed_queue.erase(o);
    ...
    if (o->ino() < MDS_INO_SYSTEM_BASE) {
        ...
        if (o->is_base()) base_inodes.erase(o);
    }
...
struct C_MDC_CreateSystemFile : public MDCacheLogContext
{
    ...
    C_MDC_CreateSystemFile(MDCache* c, MutationRef& mu, CDentry* d, version_t v, MDSContext* f)
        ...
        , fin(f)
    {}
    void finish(int r) override { mdcache->_create_system_file_finish(mut, dn, dpv, fin); }
};
...
void MDCache::_create_system_file(CDir* dir, std::string_view name, CInode* in, MDSContext* fin)
{
    ...
    if (mdir) le->metablob.add_new_dir(mdir);   // dirty AND complete AND new
...
{
    open_mydir_inode(new MDSInternalContextWrapper(mds, new LambdaContext([this, c](int r) {
                                                       if (r < 0) {
                                                           c->complete(r);
                                                           return;
                                                       }
...
void MDCache::open_root()
{
    ...
    if (mds->get_nodeid() == mds->mdsmap->get_root()) {
        ...
        CDir* rootdir = root->get_or_open_dirfrag(this, frag_t());
        ceph_assert(rootdir);
        if (!rootdir->is_subtree_root()) adjust_subtree_auth(rootdir, mds->get_nodeid());
...
void MDCache::advance_stray()
{
    ...
    if (stray_fragmenting_index >= 0) {
        ...
        for (const auto& dir : dfs) {
            if (dir->state_test(CDir::STATE_FRAGMENTING) || mds->balancer->is_fragment_pending(dir->dirfrag())) {
                any_fragmenting = true;
                break;
            }
        }
        if (!any_fragmenting) stray_fragmenting_index = -1;
    }

    for (int i = 1; i < NUM_STRAY; i++) {
        stray_index = (stray_index + i) % NUM_STRAY;
        if (stray_index != stray_fragmenting_index) break;
    }
...
void MDCache::populate_mydir()
{
    ...
    if (mydir->get_version() == 0 && mydir->state_test(CDir::STATE_BADFRAG)) {
        ...
        mds->clog->warn() << "fragment " << mydir->dirfrag()
                          << " was unreadable, "
                             "recreating it now";
        LogSegment* ls = mds->mdlog->get_current_segment();
    ...
    for (int i = 0; i < NUM_STRAY; ++i) {
        ...
        *css << "stray" << i;
        ...
        if (straydn == NULL && i == 0) straydn = mydir->lookup("stray");

        if (!straydn || !straydn->get_linkage()->get_inode()) {
            _create_system_file(mydir,
                                css->strv(),
                                create_system_inode(MDS_INO_STRAY(mds->get_nodeid(), i), S_IFDIR),
        ...
        for (const auto& leaf : leaves) {
            ...
            if (dir->get_frag_size() > 0) num_strays += dir->get_frag_size();
        }
    }
...
{
    // inode?
    if (info.ino) return get_inode(info.ino, info.snapid);
...
mds_rank_t MDCache::hash_into_rank_bucket(inodeno_t ino, frag_t fg)
{
    ...
    if (fg) hash = rjhash64(hash + rjhash64(fg.value()));
...
void MDCache::try_subtree_merge(CDir* dir)
{
    ...
    for (auto bound : oldbounds) try_subtree_merge_at(bound, &to_eval);

    if (!(mds->is_any_replay() || mds->is_resolve())) {
        for (auto in : to_eval) eval_subtree_root(in);
    }
}
...
void MDCache::try_subtree_merge_at(CDir* dir, set<CInode*>* to_eval, bool adjust_pop)
{
    ...
    if (dir->dir_auth.second != CDIR_AUTH_UNKNOWN || dir->state_test(CDir::STATE_EXPORTBOUND) ||
    ...
    // merge with parent?
    CDir* parent = dir;
    if (!dir->inode->is_base()) parent = get_subtree_root(dir->get_parent_dir());
    ...
    if (parent != dir &&                       // we have a parent,
        ...
        if (to_eval && dir->get_inode()->is_auth()) to_eval->insert(dir->get_inode());
...
{
    dout(7) << "adjust_bounded_subtree_auth " << dir->get_dir_auth() << " -> " << auth << " on " << *dir << " bounds "
            << bounds << dendl;
    ...
    for (const auto& bound : bounds) {
        ...
        if (subtrees[dir].count(bound) == 0) {
            if (get_subtree_root(bound) == dir) {
                ...
                while (1) {
                    while (subtrees[dir].count(t) == 0) t = get_subtree_root(t->get_parent_dir());
...
void MDCache::get_force_dirfrag_bound_set(const vector<dirfrag_t>& dfs, set<CDir*>& bounds)
{
    ...
    for (map<inodeno_t, fragset_t>::iterator p = byino.begin(); p != byino.end(); ++p) {
        ...
        for (const auto& fg : p->second) {
            ...
            for (const auto& sib : sibs) bounds.insert(sib);
        }
    }
}
...
{
    dout(7) << "adjust_bounded_subtree_auth " << dir->get_dir_auth() << " -> " << auth << " on " << *dir
            << " bound_dfs " << bound_dfs << dendl;
...
void MDCache::map_dirfrag_set(const list<dirfrag_t>& dfs, set<CDir*>& result)
{
    ...
    // get frags
    for (map<inodeno_t, fragset_t>::iterator p = ino_fragset.begin(); p != ino_fragset.end(); ++p) {
        p->second.simplify();
...
void MDCache::get_wouldbe_subtree_bounds(CDir* dir, set<CDir*>& bounds)
{
    if (subtrees.count(dir)) {
        ...
        for (set<CDir*>::iterator p = subtrees[root].begin(); p != subtrees[root].end(); ++p) {
            CDir* t = *p;
            while (t != root) {
                t = t->get_parent_dir();
                ceph_assert(t);
...
void MDCache::verify_subtree_bounds(CDir* dir, const set<CDir*>& bounds)
{
    ...
    if (bounds != subtrees[dir]) {
        ...
        for (const auto& cd : b) dout(0) << "    extra bound " << *cd << dendl;
    }
...
void MDCache::adjust_subtree_after_rename(CInode* diri, CDir* olddir, bool pop)
{
    ...
    if (pop) {
        ...
        p->second.pop_front();
        if (p->second.empty()) projected_subtree_renames.erase(p);
    }

    // adjust total auth pin of freezing subtree
    if (olddir != newdir) {
        auto&& dfls = diri->get_nested_dirfrags();
        for (const auto& dir : dfls) olddir->adjust_freeze_after_rename(dir);
    }
    ...
    for (const auto& dir : dfls) {
        ...
        if (olddir != newdir) mds->balancer->adjust_pop_for_rename(olddir, dir, false);
        ...
        if (olddir != newdir) mds->balancer->adjust_pop_for_rename(newdir, dir, true);
    }
...
CInode* MDCache::cow_inode(CInode* in, snapid_t last)
{
    ...
    if (in->first < in->oldest_snap) in->oldest_snap = in->first;
    ...
    if (!in->client_caps.empty()) {
        ...
        for (auto& p : in->client_caps) {
            ...
            if ((issued & CEPH_CAP_ANY_WR) && cap->client_follows < last) {
                ...
                for (auto q = snaps.lower_bound(oldin->first); q != snaps.end() && *q <= last; ++q) {
...
void MDCache::journal_cow_dentry(MutationImpl* mut, EMetaBlob* metablob, CDentry* dn, snapid_t follows,
                                 CInode** pcow_inode, CDentry::linkage_t* dnl)
{
    ...
    if (!dnl) dnl = dn->get_projected_linkage();
    ...
    if (in && (in->is_multiversion() || cow_head)) {
        ...
        if (in->get_projected_parent_dn() != dn) {
            ...
            if (dir_follows + 1 > dn->first) {
                ...
                if (realm->has_snaps_in_range(oldfirst, dir_follows)) {
                    CDir* dir = dn->dir;
                    CDentry* olddn = dir->add_remote_dentry(
                        dn->get_name(), in->ino(), in->d_type(), dn->alternate_name, oldfirst, dir_follows);
                    ...
                    if (dir_follows + 1 > in->first) in->cow_old_inode(dir_follows, cow_head);
                }
            }
        ...
        if (in) {
            ...
            if (pcow_inode) *pcow_inode = oldin;
            ...
            CDentry* olddn = dir->add_remote_dentry(
                dn->get_name(), dnl->get_remote_ino(), dnl->get_remote_d_type(), dn->alternate_name, oldfirst, follows);
            dout(10) << " olddn " << *olddn << dendl;
...
void MDCache::journal_dirty_inode(MutationImpl* mut, EMetaBlob* metablob, CInode* in, snapid_t follows)
{
    if (in->is_base()) {
        ...
        if (follows == CEPH_NOSNAP && in->last != CEPH_NOSNAP) follows = in->first - 1;
        ...
        if (in->get_projected_inode()->is_backtrace_updated()) {
            bool dirty_pool =
                in->get_projected_inode()->layout.pool_id != in->get_previous_projected_inode()->layout.pool_id;
                                          ...
                                          int linkunlink, SnapRealm* prealm)
{
    ...
    if (cur->first > first) first = cur->first;
    ...
    if (!prealm) prealm = parent->inode->find_snaprealm();
    ...
    if (cur->last != CEPH_NOSNAP) {
        ...
        if (q == snaps.end() || *q > cur->last) return;
    }
    ...
    if (cur->last >= floor) {
        ...
        _project_rstat_inode_to_frag(pi, std::max(first, floor), cur->last, parent, linkunlink, update);
    }
    ...
    if (g_conf()->mds_snap_rstat) {
        for (const auto& p : cur->dirty_old_rstats) {
            ...
            if (it == snaps.end() || *it > p) continue;
            if (p >= floor) _project_rstat_inode_to_frag(&old.inode, ofirst, p, parent, 0, false);
        }
    }
                                           ...
                                           CDir* parent, int linkunlink, bool update_inode)
{
    ...
    while (last >= ofirst) {
        ...
        if (last == CEPH_NOSNAP) {
            ...
            if (first > parent->first && !(pf->rstat == pf->accounted_rstat)) {
                dout(10) << "  target snapped and not fully accounted, cow to dirty_old_rstat [" << parent->first << ","
                         << (first - 1) << "] "
            ...
            if (it == parent->dirty_old_rstat.end()) {
                ...
                if (it->second.first <= last) {
                    ...
                    if (it->second.first > first) first = it->second.first;
                    if (last < it->first) {
                    ...
                    if (it->second.first <= last &&   // new *it isn't also to the right, and
                        ...
                        dout(10) << " staying to the right of [" << it->second.first << "," << it->first << "]..."
                                 << dendl;
                        first = it->first + 1;
                    }
...
void MDCache::project_rstat_frag_to_inode(const nest_info_t& rstat, const nest_info_t& accounted_rstat, snapid_t ofirst,
                                          snapid_t last, CInode* pin, bool cow_head)
{
    ...
    while (last >= ofirst) {
        ...
        if (last == pin->last) {
            ...
            if (!_old_inodes) {
                ...
                if (pin->is_any_old_inodes()) *_old_inodes = *pin->get_old_inodes();
            }
            if (last >= pin->first) {
                first = pin->first;
                ...
                if (it->first > last) {
                    dout(10) << " splitting right old_inode [" << first << "," << it->first << "] to [" << (last + 1)
                             << "," << it->first << "]" << dendl;
            ...
            if (first < ofirst) {
                dout(10) << " splitting left old_inode [" << first << "," << last << "] to [" << first << ","
                         << ofirst - 1 << "]" << dendl;
    ...
    if (_old_inodes) pin->reset_old_inodes(std::move(_old_inodes));
}

void MDCache::broadcast_quota_to_client(CInode* in, client_t exclude_ct, bool quota_change)
{
    if (!(mds->is_active() || mds->is_stopping())) return;

    if (!in->is_auth() || in->is_frozen()) return;

    const auto& pi = in->get_projected_inode();
    if (!pi->quota.is_enabled() && !quota_change) return;

    // creaete snaprealm for quota inode (quota was set before mimic)
    if (!in->get_projected_srnode()) mds->server->create_quota_realm(in);

    for (auto& p : in->client_caps) {
        Capability* cap = &p.second;
        if (cap->is_noquota()) continue;

        if (exclude_ct >= 0 && exclude_ct != p.first) goto update;

        if (cap->last_rbytes == pi->rstat.rbytes && cap->last_rsize == pi->rstat.rsize()) continue;

        if (pi->quota.max_files > 0) {
            if (pi->rstat.rsize() >= pi->quota.max_files) goto update;

            if ((abs(cap->last_rsize - pi->quota.max_files) >> 4) < abs(cap->last_rsize - pi->rstat.rsize()))
                goto update;
        }

        if (pi->quota.max_bytes > 0) {
            if (pi->rstat.rbytes > pi->quota.max_bytes - (pi->quota.max_bytes >> 3)) goto update;

            if ((abs(cap->last_rbytes - pi->quota.max_bytes) >> 4) < abs(cap->last_rbytes - pi->rstat.rbytes))
                goto update;
        }
 ...
 */
void MDCache::predirty_journal_parents(MutationRef mut, EMetaBlob* blob, CInode* in, CDir* parent, int flags,
                                       int linkunlink, snapid_t cfollows)
{
    ...
    if (mut->get_mds_stamp() == utime_t()) mut->set_mds_stamp(ceph_clock_now());

    if (in->is_base()) return;

    dout(10) << "predirty_journal_parents" << (do_parent_mtime ? " do_parent_mtime" : "")
             << " linkunlink=" << linkunlink << (primary_dn ? " primary_dn" : " remote_dn")
             << (shallow ? " SHALLOW" : "") << " follows " << cfollows << " " << *in << dendl;
    ...
    while (parent) {
        ...
        if (do_parent_mtime || linkunlink) {
            ...
            if (do_parent_mtime) {
                ...
                dout(10) << "predirty_journal_parents bumping fragstat change_attr to " << pf->fragstat.change_attr
                         << " on " << parent << dendl;
        ...
        if (!primary_dn) {
        ...
        else if (!linkunlink && !(pin->nestlock.can_wrlock(-1) && pin->versionlock.can_wrlock())) {
            ...
            if (follows == CEPH_NOSNAP) follows = prealm->get_newest_seq();
        ...
        if (!stop && !first && g_conf()->mds_dirstat_min_interval > 0) {
            ...
            if (since_last_prop < g_conf()->mds_dirstat_min_interval) {
                dout(10) << "predirty_journal_parents last prop " << since_last_prop << " < "
                         << g_conf()->mds_dirstat_min_interval << ", stopping" << dendl;
                stop = true;
            }
            else {
        ...
        if (!stop && !mut->is_wrlocked(&pin->nestlock) &&
            (!pin->versionlock.can_wrlock() ||                   // make sure we can take versionlock, too
             !mds->locker->wrlock_try(&pin->nestlock, mut))) {   // ** do not initiate.. see above comment **
        ...
        if (!mut->is_wrlocked(&pin->versionlock)) mds->locker->local_wrlock_grab(&pin->versionlock, mut);
        ...
        if (do_parent_mtime || linkunlink) {
            ...
            if (touched_mtime) pi.inode->mtime = pi.inode->ctime = pi.inode->dirstat.mtime;
            if (touched_chattr) pi.inode->change_attr++;
            dout(20) << "predirty_journal_parents     gives " << pi.inode->dirstat << " on " << *pin << dendl;

            if (parent->get_frag() == frag_t()) {   // i.e., we are the only frag
                if (pi.inode->dirstat.size() < 0) ceph_assert(!"negative dirstat size" == g_conf()->mds_verify_scatter);
                if (pi.inode->dirstat.size() != pf->fragstat.size()) {
                    mds->clog->error() << "unmatched fragstat size on single dirfrag " << parent->dirfrag()
                                       << ", inode has " << pi.inode->dirstat << ", dirfrag has " << pf->fragstat;
        ...
        if (g_conf()->mds_snap_rstat) {
            for (auto& p : parent->dirty_old_rstat) {
                project_rstat_frag_to_inode(
                    p.second.rstat, p.second.accounted_rstat, p.second.first, p.first, pin, true);
            }
        }
        ...
        if (parent->get_frag() == frag_t()) {   // i.e., we are the only frag
            if (pi.inode->rstat.rbytes != pf->rstat.rbytes) {
                mds->clog->error() << "unmatched rstat rbytes on single dirfrag " << parent->dirfrag() << ", inode has "
                                   << pi.inode->rstat << ", dirfrag has " << pf->rstat;
...
{
    metareqid_t reqid;
    C_MDC_CommittedLeader(MDCache* s, metareqid_t r)
        : MDCacheLogContext(s)
        , reqid(r)
    {}
    void finish(int r) override { mdcache->_logged_leader_commit(reqid); }
};
...
void MDCache::log_leader_commit(metareqid_t reqid)
{
    ...
    mds->mdlog->start_submit_entry(new ECommitted(reqid), new C_MDC_CommittedLeader(this, reqid));
}
...
void MDCache::committed_leader_peer(metareqid_t r, mds_rank_t from)
{
    ...
    if (!uncommitted_leaders[r].recovering && uncommitted_leaders[r].peers.empty()) log_leader_commit(r);
}
...
void MDCache::logged_leader_update(metareqid_t reqid)
{
    ...
    if (p != pending_leaders.end()) {
        ...
        if (pending_leaders.empty()) process_delayed_resolve();
    }
}
...
{
    for (map<metareqid_t, uleader>::iterator p = uncommitted_leaders.begin(); p != uncommitted_leaders.end(); ++p) {
...
struct C_MDC_PeerCommit : public MDCacheLogContext
{
    mds_rank_t from;
    metareqid_t reqid;
    C_MDC_PeerCommit(MDCache* c, int f, metareqid_t r)
        : MDCacheLogContext(c)
        , from(f)
        , reqid(r)
    {}
    void finish(int r) override { mdcache->_logged_peer_commit(from, reqid); }
};
...
{
    dout(10) << "create_subtree_map " << num_subtrees() << " subtrees, " << num_subtrees_fullauth() << " fullauth"
             << dendl;
    ...
    for (auto& [dir, bounds] : subtrees) {
        ...
        // so not
        //   !me, *
        if (dir->get_dir_auth().first != mds->get_nodeid()) continue;

        if (migrator->is_ambiguous_import(dir->dirfrag()) || my_ambiguous_imports.count(dir->dirfrag())) {
            ...
            for (const auto& dir : dfls) {
                ...
                if (dir->is_subtree_root()) {
                    if (le->subtrees.count(newparent->dirfrag()) &&
                        ...
                        dirs_to_add[dir->dirfrag()] = dir;
                    // children are fine.  change parent.
                    _move_subtree_map_bound(dir->dirfrag(), oldparent->dirfrag(), newparent->dirfrag(), le->subtrees);
                }
                else {
                    // mid-subtree.
                    ...
                    if (oldparent->get_dir_auth() != newparent->get_dir_auth()) {
                        ...
                        if (le->subtrees.count(newparent->dirfrag())) {
                            dirs_to_add[dir->dirfrag()] = dir;
                            le->subtrees[newparent->dirfrag()].push_back(
                                dir->dirfrag());   // newparent is auth; new bound
                        }
                        newparent = dir;
                    }
                    ...
                    for (auto& bound : subtrees.at(oldparent)) {
                        if (dir->contains(bound->get_parent_dir()))
                            _move_subtree_map_bound(
                                bound->dirfrag(), oldparent->dirfrag(), newparent->dirfrag(), le->subtrees);
                    }
                }
            }
        }
    ...
    for (auto& [frag, bfrags] : le->subtrees) {
        if (le->ambiguous_subtrees.count(frag)) continue;
        unsigned i = 0;
        while (i < bfrags.size()) {
            dirfrag_t b = bfrags[i];
            if (le->subtrees.count(b) && le->ambiguous_subtrees.count(b) == 0) {
...
void MDCache::resolve_start(MDSContext* resolve_done_)
{
    ...
    if (mds->mdsmap->get_root() != mds->get_nodeid()) {
        ...
        // unknown.
        CDir* rootdir = root->get_dirfrag(frag_t());
        if (rootdir) adjust_subtree_auth(rootdir, CDIR_AUTH_UNKNOWN);
    }
...
void MDCache::send_resolves()
{
    ...
    if (!resolve_done) {
        ...
        mds->snapclient->sync(
            new MDSInternalContextWrapper(mds, new LambdaContext([this](int r) { maybe_finish_peer_resolve(); })));
...
void MDCache::send_peer_resolves()
{
    ...
    if (mds->is_resolve()) {
        for (map<metareqid_t, upeer>::iterator p = uncommitted_peers.begin(); p != uncommitted_peers.end(); ++p) {
        ...
        for (ceph::unordered_map<metareqid_t, MDRequestRef>::iterator p = active_requests.begin();
            ...
            if (resolve_set.count(leader) || is_ambiguous_peer_update(p->first, leader)) {
                ...
                if (!resolves.count(leader)) resolves[leader] = make_message<MMDSResolve>();
                if (!mdr->committing && mdr->has_more() && mdr->more()->is_inode_exporter) {
                    // re-send cap exports
...
void MDCache::send_subtree_resolves()
{
    ...
    map<mds_rank_t, ref_t<MMDSResolve>> resolves;
    for (set<mds_rank_t>::iterator p = recovery_set.begin(); p != recovery_set.end(); ++p) {
        if (*p == mds->get_nodeid()) continue;
        if (mds->is_resolve() || mds->mdsmap->is_resolve(*p)) resolves[*p] = make_message<MMDSResolve>();
    }
    ...
    // known
    for (map<CDir*, set<CDir*>>::iterator p = subtrees.begin(); p != subtrees.end(); ++p) {
        CDir* dir = p->first;

        // only our subtrees
        if (dir->authority().first != mds->get_nodeid()) continue;

        if (mds->is_resolve() && my_ambiguous_imports.count(dir->dirfrag())) continue;   // we'll add it below
        ...
        if (migrator->is_ambiguous_import(dir->dirfrag())) {
            ...
            for (set<CDir*>::iterator q = bounds.begin(); q != bounds.end(); ++q) dfls.push_back((*q)->dirfrag());
            ...
            // bounds too
            vector<dirfrag_t> dfls;
            for (set<CDir*>::iterator q = subtrees[dir].begin(); q != subtrees[dir].end(); ++q) {
                CDir* bound = *q;
    ...
    // ambiguous
    for (map<dirfrag_t, vector<dirfrag_t>>::iterator p = my_ambiguous_imports.begin(); p != my_ambiguous_imports.end();
         ++p) {
    ...
    for (auto p = my_subtrees.begin(); p != my_subtrees.end(); ++p) {
        ...
        while (i < p->second.size()) {
            ...
            if (my_subtrees.count(b)) {
                ...
                for (vector<dirfrag_t>::iterator r = bb.begin(); r != bb.end(); ++r) p->second.push_back(*r);
                my_subtrees.erase(b);
...
void MDCache::maybe_finish_peer_resolve()
{
    if (resolve_ack_gather.empty() && resolve_need_rollback.empty()) {
        ...
        if (mds->snapclient->is_synced() || resolve_done) send_subtree_resolves();
...
void MDCache::handle_mds_failure(mds_rank_t who)
{
    ...
    for (ceph::unordered_map<metareqid_t, MDRequestRef>::iterator p = active_requests.begin();
        ...
        if (mdr->is_peer() && mdr->peer_did_prepare()) {
            ...
            if (peer_req->get_op() == MMDSPeerRequest::OP_RENAMEPREP && peer_req->srcdn_auth == who)
        ...
        if (mdr->is_leader() && !mdr->committing) {
            if (mdr->more()->srcdn_auth_mds == who) {
                dout(10) << " leader request " << *mdr << " waiting for rename srcdn's auth mds." << who
                         << " to recover" << dendl;
                ceph_assert(mdr->more()->witnessed.count(who) == 0);
                if (mdr->more()->is_ambiguous_auth) mdr->clear_ambiguous_auth();
            ...
            if (mdr->more()->witnessed.count(who)) {
                ...
                if (srcdn_auth >= 0 && mdr->more()->waiting_on_peer.count(srcdn_auth)) {
                    ...
                    dout(10) << " leader request " << *mdr << " no longer witnessed by peer mds." << who
                             << " to recover" << dendl;
                    if (srcdn_auth >= 0) ceph_assert(mdr->more()->witnessed.count(srcdn_auth) == 0);
            ...
            if (mdr->locking && mdr->locking_target_mds == who) mdr->finish_locking(mdr->locking);
        }
    }
    ...
    for (map<dirfrag_t, fragment_info_t>::iterator p = fragments.begin(); p != fragments.end();) {
        dirfrag_t df = p->first;
        fragment_info_t& info = p->second;

        if (info.is_fragmenting()) {
            if (info.notify_ack_waiting.erase(who) && info.notify_ack_waiting.empty()) {
    ...
    if (who == mds_rank_t(0)) shutdown_exporting_strays.clear();
...
void MDCache::handle_mds_recovery(mds_rank_t who)
{
    ...
    for (map<CDir*, set<CDir*>>::iterator p = subtrees.begin(); p != subtrees.end(); ++p) {
        ...
        if (dir->authority().first != who || dir->authority().second == mds->get_nodeid()) continue;
            ...
            for (auto& p : d->items) {
                ...
                if (dnl->is_primary()) {
                    ...
                    for (const auto& subdir : ls) {
                        if (!subdir->is_subtree_root()) q.push(subdir);
                    }
                }
            }
        }
...
void MDCache::handle_resolve(const cref_t<MMDSResolve>& m)
{
    ...
    if (!m->peer_requests.empty()) {
        ...
        for (const auto& p : m->peer_requests) {
            if (uncommitted_leaders.count(p.first)) {   // mds->sessionmap.have_completed_request(p.first)) {
                ...
                uncommitted_leaders[p.first].peers.insert(
                    from);   // wait for peer OP_COMMITTED before we log ECommitted
                ...
                if (p.second.inode_caps.length() > 0) {
                    ...
                    for (map<client_t, Capability::Export>::iterator q = cap_exports.begin(); q != cap_exports.end();
                         ++q) {
                        ...
                        if (session) rejoin_client_map.emplace(q->first, session->info.inst);
                    }
    ...
    if (mds->is_clientreplay() || mds->is_active() || mds->is_stopping()) {
        ...
        while (p != my_ambiguous_imports.end()) {
            ...
            if (migrator->is_importing(dir->dirfrag()) && migrator->get_import_peer(dir->dirfrag()) == from) {
                ...
                for (const auto& q : m->subtrees) {
                    ...
                    if (inside) claimed_by_sender = true;
                }
    ...
    for (const auto& p : m->table_clients) {
        ...
        for (const auto& q : p.pending_commits) client->notify_commit(q);
    }
...
void MDCache::handle_resolve_ack(const cref_t<MMDSResolveAck>& ack)
{
    ...
    if (!resolve_ack_gather.count(from) || mds->mdsmap->get_state(from) < MDSMap::STATE_RESOLVE) {
        return;
    }
    ...
    for (const auto& p : ack->commit) {
        ...
        if (mds->is_resolve()) {
            ...
            // log commit
            mds->mdlog->start_submit_entry(
                new EPeerUpdate(mds->mdlog, "unknown", p.first, from, EPeerUpdate::OP_COMMIT, su->origop),
            ...
            if (p.second.length() > 0) mdr->more()->inode_import.share(p.second);
    ...
    for (const auto& metareq : ack->abort) {
        ...
        if (mds->is_resolve()) {
            ...
            switch (su->origop) {
            case EPeerUpdate::LINK: mds->server->do_link_rollback(su->rollback, from, null_ref); break;
            case EPeerUpdate::RENAME: mds->server->do_rename_rollback(su->rollback, from, null_ref); break;
            case EPeerUpdate::RMDIR: mds->server->do_rmdir_rollback(su->rollback, from, null_ref); break;
            default: ceph_abort();
            }
        }
        else {
...
{
    auto const& ret =
        uncommitted_peers.emplace(std::piecewise_construct, std::forward_as_tuple(reqid), std::forward_as_tuple());
    ...
    for (set<CInode*>::iterator p = su->unlinked.begin(); p != su->unlinked.end(); ++p) uncommitted_peer_unlink[*p]++;
}
...
void MDCache::finish_uncommitted_peer(metareqid_t reqid, bool assert_exist)
{
    ...
    for (set<CInode*>::iterator p = su->unlinked.begin(); p != su->unlinked.end(); ++p) {
        ...
        if (it->second == 0) {
            ...
            if (!in->get_projected_parent_dn()) mds->mdcache->remove_inode_recursive(in);
        }
        else
            ceph_assert(it->second > 0);
    }
...
MDPeerUpdate* MDCache::get_uncommitted_peer(metareqid_t reqid, mds_rank_t leader)
{
    ...
    auto it = uncommitted_peers.find(reqid);
    if (it != uncommitted_peers.end() && it->second.leader == leader) {
        su = it->second.su;
    }
    return su;
}
...
void MDCache::disambiguate_other_imports()
{
    ...
    for (map<mds_rank_t, map<dirfrag_t, vector<dirfrag_t>>>::iterator p = other_ambiguous_imports.begin();
        ...
        for (map<dirfrag_t, vector<dirfrag_t>>::iterator q = p->second.begin(); q != p->second.end(); ++q) {
...
void MDCache::disambiguate_my_imports()
{
    ...
    while (!my_ambiguous_imports.empty()) {
        ...
        if (dir->authority() != me_ambig) {
            ...
            // as their own.
            CDir* root = get_subtree_root(dir);
            if (root != dir) dout(10) << "  subtree root is " << *root << dendl;
...
void MDCache::add_ambiguous_import(CDir* base, const set<CDir*>& bounds)
{
    ...
    for (set<CDir*>::iterator p = bounds.begin(); p != bounds.end(); ++p) binos.push_back((*p)->dirfrag());

    // note: this can get called twice if the exporter fails during recovery
    if (my_ambiguous_imports.count(base->dirfrag())) my_ambiguous_imports.erase(base->dirfrag());
...
void MDCache::trim_unlinked_inodes()
{
    ...
    for (auto& p : inode_map) {
        ...
        if (!(++count % mds->heartbeat_reset_grace())) mds->heartbeat_reset();
    }
    for (auto& in : q) {
...
void MDCache::recalc_auth_bits(bool replay)
{
    ...
    if (root) {
        ...
        if (auth) {
            ...
            if (!replay) root->state_set(CInode::STATE_REJOINING);
        }
    }

    set<CInode*> subtree_inodes;
    for (map<CDir*, set<CDir*>>::iterator p = subtrees.begin(); p != subtrees.end(); ++p) {
        if (p->first->dir_auth.first == mds->get_nodeid()) subtree_inodes.insert(p->first->inode);
    }
    ...
    for (map<CDir*, set<CDir*>>::iterator p = subtrees.begin(); p != subtrees.end(); ++p) {
        if (p->first->inode->is_mdsdir()) {
            ...
            if (auth) {
                ...
                if (!replay) in->state_set(CInode::STATE_REJOINING);
            }
        }
        ...
        while (!dfq.empty()) {
            ...
            if (auth) {
                ...
                if (!replay) {
                    ...
                    if (dir->is_dirty()) dir->mark_clean();
                }
            }
                ...
                if (dnl->is_primary()) {
                    ...
                    if (auth) {
                        ...
                        if (!replay) {
                            ...
                            if (in->is_dirty_parent()) in->clear_dirty_parent();
                            // avoid touching scatterlocks for our subtree roots!
                            if (subtree_inodes.count(in) == 0) in->clear_scatter_dirty();
                        }
                    }
                    // recurse?
                    if (in->is_dir()) {
...
void MDCache::rejoin_send_rejoins()
{
    ...
    for (map<CDir*, set<CDir*>>::iterator p = subtrees.begin(); p != subtrees.end(); ++p) {
        ...
        // my subtree?
        if (dir->is_auth()) continue;   // skip my own regions!

        mds_rank_t auth = dir->get_dir_auth().first;
        ceph_assert(auth >= 0);
        if (rejoins.count(auth) == 0) continue;   // don't care about this node's subtrees
    ...
    for (auto& p : rejoins) {
        if (mds->is_rejoin()) {
            ...
            if (CInode* in = get_inode(MDS_INO_MDSDIR(p.first))) {
                if (in) p.second->add_weak_inode(in->vino());
            }
        }
        else {
            // strong
    ...
    if (!mds->is_rejoin()) {
        ...
        for (ceph::unordered_map<metareqid_t, MDRequestRef>::iterator p = active_requests.begin();
            ...
            for (const auto& q : mdr->object_states) {
                if (q.second.remote_auth_pinned == MDS_RANK_NONE) continue;
                if (!q.first->is_auth()) {
                    ...
                    if (mdr->has_more() && mdr->more()->is_remote_frozen_authpin &&
                        ...
                        rejoin->add_inode_frozen_authpin(vinodeno_t(i.ino, i.snapid), mdr->reqid, mdr->attempt);
                }
            }
            // xlocks
            for (const auto& q : mdr->locks) {
                ...
                if (q.is_xlock() && !obj->is_auth()) {
                    ...
                    if (i.ino)
                        rejoin->add_inode_xlock(
                            vinodeno_t(i.ino, i.snapid), lock->get_type(), mdr->reqid, mdr->attempt);
                    else
                        rejoin->add_dentry_xlock(i.dirfrag, i.dname, i.snapid, mdr->reqid, mdr->attempt);
                }
                    ...
                    obj->set_object_info(i);
                    ceph_assert(i.ino);
                    rejoin->add_inode_wrlock(vinodeno_t(i.ino, i.snapid), lock->get_type(), mdr->reqid, mdr->attempt);
                }
            }
        }
    }
    ...
    if (mds->is_rejoin()) {
        ...
        for (auto it = dir->items.begin(); it != dir->items.end();) {
            ...
            if (dn->last != CEPH_NOSNAP) {
                ...
                if (dn->lru_is_expireable()) {
                    if (!dnl->is_null()) dir->unlink_inode(dn, false);
                    if (in) remove_inode(in);
            ...
            rejoin->add_strong_dentry(dir->dirfrag(),
                                      dn->get_name(),
                                      dn->get_alternate_name(),
                                      dn->first,
                                      dn->last,
...
{
    dout(7) << "handle_cache_rejoin " << *m << " from " << m->get_source() << " (" << m->get_payload().length()
            << " bytes)" << dendl;

    switch (m->op) {
    case MMDSCacheRejoin::OP_WEAK: handle_cache_rejoin_weak(m); break;
    case MMDSCacheRejoin::OP_STRONG: handle_cache_rejoin_strong(m); break;
    case MMDSCacheRejoin::OP_ACK: handle_cache_rejoin_ack(m); break;
...
void MDCache::handle_cache_rejoin_weak(const cref_t<MMDSCacheRejoin>& weak)
{
    ...
    if (mds->is_clientreplay() || mds->is_active() || mds->is_stopping()) {
        ...
        rejoin_client_metadata_map.insert(weak->client_metadata_map.begin(), weak->client_metadata_map.end());
    ...
    for (const auto& p : weak->inode_scatterlocks) {
        ...
        if (!survivor) rejoin_potential_updated_scatterlocks.insert(in);
    }
    ...
    for (const auto& p : weak->weak_dirfrags) {
        ...
        if (!diri) dout(0) << " missing dir ino " << p.ino << dendl;
        ceph_assert(diri);
        ...
        if (diri->dirfragtree.is_leaf(p.frag)) {
            ...
            if (leaves.empty()) leaves.push_back(diri->dirfragtree[p.frag.value()]);
        }
        for (const auto& leaf : leaves) {
    ...
    for (const auto& p : weak->weak) {
        CInode* diri = get_inode(p.first);
        if (!diri) dout(0) << " missing dir ino " << p.first << dendl;
        ceph_assert(diri);
        ...
        for (const auto& q : p.second) {
            ...
            if (!dir || dir->get_frag() != fg) {
                ...
                if (!dir) dout(0) << " missing dir frag " << fg << " on " << *diri << dendl;
                ceph_assert(dir);
            ...
            if (survivor && dn->is_replica(from)) dentry_remove_replica(dn, from, gather_locks);
            unsigned dnonce = dn->add_replica(from);
            dout(10) << " have " << *dn << dendl;
            if (ack)
                ack->add_strong_dentry(dir->dirfrag(),
                                       dn->get_name(),
                                       dn->get_alternate_name(),
                                       dn->first,
                                       dn->last,
            ...
            if (survivor && in->is_replica(from)) inode_remove_replica(in, from, true, gather_locks);
            unsigned inonce = in->add_replica(from);
            dout(10) << " have " << *in << dendl;

            // scatter the dirlock, just in case?
            if (!survivor && in->is_dir() && in->has_subtree_root_dirfrag()) in->filelock.set_state(LOCK_MIX);
    ...
    for (set<vinodeno_t>::iterator p = weak->weak_inodes.begin(); p != weak->weak_inodes.end(); ++p) {
    ...
    if (survivor) {
        ...
        for (set<SimpleLock*>::iterator p = gather_locks.begin(); p != gather_locks.end(); ++p) {
            if (!(*p)->is_stable()) mds->locker->eval_gather(*p);
        }
    }
    else {
        // done?
...
void MDCache::rejoin_scour_survivor_replicas(mds_rank_t from, const cref_t<MMDSCacheRejoin>& ack,
                                             set<vinodeno_t>& acked_inodes, set<SimpleLock*>& gather_locks)
{
    ...
    auto scour_func = [this, from, ack, &acked_inodes, &gather_locks](CInode* in) {
        // inode?
        if (in->is_auth() && in->is_replica(from) && (ack == NULL || acked_inodes.count(in->vino()) == 0)) {
        ...
        for (const auto& dir : dfs) {
            ...
            if (dir->is_replica(from) && (ack == NULL || ack->strong_dirfrags.count(dir->dirfrag()) == 0)) {
                ...
                if (dn->is_replica(from)) {
                    if (ack) {
                        ...
                        if (it != ack->strong_dentries.end() &&
                            it->second.count(string_snap_t(dn->get_name(), dn->last)) > 0) {
                            continue;
                        }
                    }
    ...
    for (auto& p : inode_map) scour_func(p.second);
    for (auto& p : snap_inode_map) scour_func(p.second);
}
...
CDir* MDCache::rejoin_invent_dirfrag(dirfrag_t df)
{
    ...
    if (!in) in = rejoin_invent_inode(df.ino, CEPH_NOSNAP);
    if (!in->is_dir()) {
...
void MDCache::handle_cache_rejoin_strong(const cref_t<MMDSCacheRejoin>& strong)
{
    ...
    for (const auto& p : strong->strong_dirfrags) {
        ...
        if (!diri) diri = rejoin_invent_inode(dirfrag.ino, CEPH_NOSNAP);
        ...
        if (dir) {
            ...
            if (leaves.empty()) leaves.push_back(diri->dirfragtree[dirfrag.frag.value()]);
        ...
        if (it != strong->strong_dentries.end()) {
            ...
            for (const auto& q : dmap) {
                ...
                if (!dn) {
                    if (d.is_remote()) {
                        dn = dir->add_remote_dentry(ss.name,
                                                    d.remote_ino,
                                                    d.remote_d_type,
                                                    mempool::mds_co::string(d.alternate_name),
                                                    d.first,
                                                    ss.snapid);
                    }
                    else if (d.is_null()) {
                        ...
                        dn = dir->add_primary_dentry(
                            ss.name, in, mempool::mds_co::string(d.alternate_name), d.first, ss.snapid);
                    }
                ...
                if (xlocked_it != strong->xlocked_dentries.end()) {
                    ...
                    if (ss_req_it != xlocked_it->second.end()) {
                        ...
                        if (dn->lock.is_stable()) dn->auth_pin(&dn->lock);
    ...
    for (const auto& p : strong->strong_inodes) {
        ...
        if (xlocked_inodes_it != strong->xlocked_inodes.end()) {
            for (const auto& q : xlocked_inodes_it->second) {
                ...
                if (lock->is_stable()) in->auth_pin(lock);
                lock->set_state(LOCK_XLOCK);
                if (lock == &in->filelock) in->loner_cap = -1;
    ...
    for (const auto& p : strong->wrlocked_inodes) {
        ...
        for (const auto& q : p.second) {
            ...
            for (const auto& r : q.second) {
                ...
                MDRequestRef mdr = request_get(r.reqid);   // should have this from auth_pin above.
                if (in->is_auth()) ceph_assert(mdr->is_auth_pinned(in));
...
void MDCache::handle_cache_rejoin_ack(const cref_t<MMDSCacheRejoin>& ack)
{
    ...
    for (const auto& p : ack->strong_dirfrags) {
        ...
        if (!dir) {
            ...
            if (dir) refragged_inodes.insert(dir->get_inode());
        }
        if (!dir) {
            ...
            if (MDS_INO_MDSDIR(from) == p.first.ino ||
                (dir->authority() != CDIR_AUTH_UNDEF && dir->authority().first != from))
        ...
        if (it != ack->strong_dentries.end()) {
            for (const auto& q : it->second) {
                CDentry* dn = dir->lookup(q.first.name, q.first.snapid);
                if (!dn) dn = dir->add_null_dentry(q.first.name, q.second.first, q.first.snapid);
                ...
                if (dnl->is_primary()) {
                    CInode* in = dnl->get_inode();
                    if (!q.second.is_primary() || vinodeno_t(q.second.ino, q.first.snapid) != in->vino()) {
                        dout(10) << " had bad linkage for " << *dn << ", unlinking " << *in << dendl;
                        dir->unlink_inode(dn);
                    }
                }
                else if (dnl->is_remote()) {
                    if (!q.second.is_remote() || q.second.remote_ino != dnl->get_remote_ino() ||
                    ...
                    if (!q.second.is_null()) dout(10) << " had bad linkage for " << *dn << dendl;
                }
                ...
                if (dnl->is_null() && !q.second.is_null()) {
                    if (q.second.is_remote()) {
                        ...
                        if (!in) {
                            ...
                            dout(10) << " had bad linkage for " << *(in->get_parent_dn()) << ", unlinking " << *in
                                     << dendl;
    ...
    for (const auto& in : refragged_inodes) {
        ...
        for (const auto& dir : ls) {
            if (dir->is_auth() || ack->strong_dirfrags.count(dir->dirfrag())) continue;
    ...
    while (!p.end()) {
        ...
        snapid_t sseq = 0;
        if (in->snaprealm) sseq = in->snaprealm->srnode.seq;
        in->_decode_base(q);
    ...
    for (map<inodeno_t, map<client_t, Capability::Import>>::iterator p = peer_imported.begin();
        ...
        for (map<client_t, Capability::Import>::iterator q = p->second.begin(); q != p->second.end(); ++q) {
            ...
            auto m = make_message<MClientCaps>(
                CEPH_CAP_OP_EXPORT, p->first, 0, r->second.capinfo.cap_id, 0, mds->get_osd_epoch_barrier());
            m->set_cap_peer(
                q->second.cap_id, q->second.issue_seq, q->second.mseq, (q->second.cap_id > 0 ? from : -1), 0);
    ...
    if (!survivor) {
        if (rejoin_gather.empty()) {
            ...
            while (!rejoin_eval_locks.empty()) {
                ...
                if (!lock->is_stable()) mds->locker->eval_gather(lock);
            }
        }
...
void MDCache::rejoin_gather_finish()
{
    ...
    if (open_undef_inodes_dirfrags()) return;

    if (process_imported_caps()) return;
...
public:
    C_MDC_RejoinOpenInoFinish(MDCache* c, inodeno_t i)
        : MDCacheContext(c)
        , ino(i)
    {}
    void finish(int r) override { mdcache->rejoin_open_ino_finish(ino, r); }
};
...
public:
    ...
    C_MDC_RejoinSessionsOpened(MDCache* c)
        : MDCacheLogContext(c)
    {}
    void finish(int r) override
    {
...
void MDCache::rejoin_open_sessions_finish(map<client_t, pair<Session*, uint64_t>>& session_map)
{
    ...
    if (rejoin_gather.empty() && rejoin_ack_gather.count(mds->get_nodeid())) rejoin_gather_finish();
}
...
bool MDCache::process_imported_caps()
{
    ...
    if (!open_file_table.is_prefetched() && open_file_table.prefetch_inodes()) {
        open_file_table.wait_for_prefetch(new MDSInternalContextWrapper(mds, new LambdaContext([this](int r) {
                                                                            ceph_assert(
                                                                                rejoin_gather.count(mds->get_nodeid()));
                                                                            process_imported_caps();
                                                                        })));
        return true;
    }
    ...
    for (auto& p : cap_imports) {
        ...
        if (cap_imports_missing.count(p.first) > 0) continue;
        ...
        for (auto& q : p.second) {
            for (auto& r : q.second) {
                auto& icr = r.second;
                if (icr.capinfo.pathbase && icr.path.length() > 0 && icr.path.find('/') == string::npos) {
        ...
        if (!(cap_imports_num_opening % mds->heartbeat_reset_grace())) mds->heartbeat_reset();
    }

    open_ino_batch_submit();

    if (cap_imports_num_opening > 0) return true;

    // called by rejoin_gather_finish() ?
    if (rejoin_gather.count(mds->get_nodeid()) == 0) {
        if (!rejoin_client_map.empty() && rejoin_session_map.empty()) {
            C_MDC_RejoinSessionsOpened* finish = new C_MDC_RejoinSessionsOpened(this);
            version_t pv = mds->server->prepare_force_open_sessions(
                rejoin_client_map, rejoin_client_metadata_map, finish->session_map);
            ESessions* le = new ESessions(pv, std::move(rejoin_client_map), std::move(rejoin_client_metadata_map));
        ...
        for (map<inodeno_t, pair<mds_rank_t, map<client_t, Capability::Export>>>::iterator p =
                 rejoin_peer_exports.begin();
             p != rejoin_peer_exports.end();
             ++p) {
            CInode* in = get_inode(p->first);
            ceph_assert(in);
            for (map<client_t, Capability::Export>::iterator q = p->second.second.begin(); q != p->second.second.end();
                 ++q) {
                auto r = rejoin_session_map.find(q->first);
                if (r == rejoin_session_map.end()) continue;
        ...
        for (auto p = cap_imports.begin(); p != cap_imports.end();) {
            CInode* in = get_inode(p->first);
            if (!in) {
                dout(10) << " still missing ino " << p->first << ", will try again after replayed client requests"
                         << dendl;
                ++p;
                continue;
            }
            ...
            for (auto q = p->second.begin(); q != p->second.end(); ++q) {
                ...
                for (auto r = q->second.begin(); r != q->second.end(); ++r) {
                    if (!session) {
                        if (r->first >= 0) (void)rejoin_imported_caps[r->first][p->first][q->first];   // all are zero
                        continue;
                    }
...
void MDCache::rebuild_need_snapflush(CInode* head_in, SnapRealm* realm, client_t client, snapid_t snap_follows)
{
    dout(10) << "rebuild_need_snapflush " << snap_follows << " on " << *head_in << dendl;

    if (!realm->has_snaps_in_range(snap_follows + 1, head_in->first - 1)) return;
...
void MDCache::choose_lock_states_and_reconnect_caps()
{
    ...
    for (auto p : inode_map) {
        CInode* in = p.second;
        if (in->last != CEPH_NOSNAP) continue;

        if (in->is_auth() && !in->is_base() && in->get_inode()->is_dirty_rstat()) in->mark_dirty_rstat();

        int dirty_caps = 0;
        auto q = reconnected_caps.find(in->ino());
        if (q != reconnected_caps.end()) {
            for (const auto& it : q->second) dirty_caps |= it.second.dirty_caps;
        }
                                  ...
                                  map<client_t, ref_t<MClientSnap>>& splits)
{
    ...
    if (it != splits.end()) {
        ...
        for (const auto& child : realm->open_children) snap->split_realms.push_back(child->inode->ino());
    }
    snap->split_inos.push_back(ino);
}

void MDCache::prepare_realm_merge(SnapRealm* realm, SnapRealm* parent_realm, map<client_t, ref_t<MClientSnap>>& splits)
{
    ...
    for (auto p = realm->inodes_with_caps.begin(); !p.end(); ++p) split_inos.push_back((*p)->ino());
    for (set<SnapRealm*>::iterator p = realm->open_children.begin(); p != realm->open_children.end(); ++p)
...
void MDCache::send_snaps(map<client_t, ref_t<MClientSnap>>& splits)
{
    ...
    for (auto& p : splits) {
        ...
        if (session) {
            dout(10) << " client." << p.first << " split " << p.second->head.split << " inos " << p.second->split_inos
                     << dendl;
...
void MDCache::clean_open_file_lists()
{
    ...
    for (map<uint64_t, LogSegment*>::iterator p = mds->mdlog->segments.begin(); p != mds->mdlog->segments.end(); ++p) {
...
void MDCache::dump_openfiles(Formatter* f)
{
    ...
    for (auto p = mds->mdlog->segments.begin(); p != mds->mdlog->segments.end(); ++p) {
        LogSegment* ls = p->second;
...
void MDCache::export_remaining_imported_caps()
{
        ...
        *css << " ino " << p->first << "\n";
        for (auto q = p->second.begin(); q != p->second.end(); ++q) {
            ...
            if (session) {
                // mark client caps stale.
                auto stale =
                    make_message<MClientCaps>(CEPH_CAP_OP_EXPORT, p->first, 0, 0, 0, mds->get_osd_epoch_barrier());
    ...
    for (map<inodeno_t, MDSContext::vec>::iterator p = cap_reconnect_waiters.begin(); p != cap_reconnect_waiters.end();
         ++p)
    ...
    if (css->strv().length()) {
        mds->clog->warn() << "failed to reconnect caps for missing inodes:" << css->strv();
    }
}
...
Capability* MDCache::try_reconnect_cap(CInode* in, Session* session)
{
    ...
    if (rc) {
        ...
        dout(10) << "try_reconnect_cap client." << client << " reconnect wanted " << ccap_string(rc->capinfo.wanted)
                 << " issue " << ccap_string(rc->capinfo.issued) << " on " << *in << dendl;
        ...
        if (in->is_replicated()) {
            ...
            if (p != reconnected_caps.end()) {
                ...
                if (q != p->second.end()) dirty_caps = q->second.dirty_caps;
            }
            in->choose_lock_states(dirty_caps);
            dout(15) << " chose lock states on " << *in << dendl;
        }

        map<inodeno_t, MDSContext::vec>::iterator it = cap_reconnect_waiters.find(in->ino());
...
void MDCache::do_cap_import(Session* session, CInode* in, Capability* cap, uint64_t p_cap_id, ceph_seq_t p_seq,
                            ceph_seq_t p_mseq, int peer, int p_flags)
{
...
{
    explicit C_MDC_OpenSnapRealms(MDCache* c)
        : MDCacheContext(c)
    {}
    void finish(int r) override { mdcache->open_snaprealms(); }
};
...
void MDCache::open_snaprealms()
{
    ...
    while (it != rejoin_pending_snaprealms.end()) {
        ...
        if (q != reconnected_snaprealms.end()) {
            for (const auto& r : q->second) finish_snaprealm_reconnect(r.first, realm, r.second, splits);
...
{
    dout(10) << "open_undef_inodes_dirfrags " << rejoin_undef_inodes.size() << " inodes "
    ...
    if (fetch_queue.empty()) return false;

    MDSGatherBuilder gather(
        g_ceph_context,
        new MDSInternalContextWrapper(mds, new LambdaContext([this](int r) {
                                          if (rejoin_gather.empty() && rejoin_ack_gather.count(mds->get_nodeid()))
                                              rejoin_gather_finish();
                                      })));
    ...
    for (auto& p : fetch_queue) {
        ...
        if (diri->state_test(CInode::STATE_REJOINUNDEF)) continue;
        if (dir->state_test(CDir::STATE_REJOINUNDEF)) ceph_assert(diri->dirfragtree.is_leaf(dir->get_frag()));
...
void MDCache::rejoin_send_acks()
{
    ...
    // replicate stray
    for (map<mds_rank_t, set<CInode*>>::iterator p = rejoin_unlinked_inodes.begin(); p != rejoin_unlinked_inodes.end();
         ++p) {
        for (set<CInode*>::iterator q = p->second.begin(); q != p->second.end(); ++q) {
            CInode* in = *q;
            dout(7) << " unlinked inode " << *in << dendl;
            // inode expired
            if (!in->is_replica(p->first)) continue;
            while (1) {
                CDentry* dn = in->get_parent_dn();
                if (dn->is_replica(p->first)) break;
                dn->add_replica(p->first);
                CDir* dir = dn->get_dir();
                if (dir->is_replica(p->first)) break;
                dir->add_replica(p->first);
                in = dir->get_inode();
                if (in->is_replica(p->first)) break;
    ...
    for (set<mds_rank_t>::iterator p = recovery_set.begin(); p != recovery_set.end(); ++p) {
        if (rejoin_ack_sent.count(*p)) continue;
    ...
    for (map<CDir*, set<CDir*>>::iterator p = subtrees.begin(); p != subtrees.end(); ++p) {
        ...
        while (!dq.empty()) {
            ...
            for (auto& p : dir->items) {
                ...
                // inode
                CInode* in = NULL;
                if (dnl->is_primary()) in = dnl->get_inode();

                // dentry
                for (auto& r : dn->get_replicas()) {
                    auto it = acks.find(r.first);
                    if (it == acks.end()) continue;
                    it->second->add_strong_dentry(dir->dirfrag(),
                                                  dn->get_name(),
                                                  dn->get_alternate_name(),
                                                  dn->first,
                                                  dn->last,
                    ...
                    if (in && !in->is_replica(r.first)) in->add_replica(r.first);
                }
...
public:
    ...
    void finish(int r) override
    {
        if (!mdcache->mds->locker->eval(in, CEPH_CAP_LOCKS)) mdcache->mds->locker->issue_caps(in);
...
void MDCache::reissue_all_caps()
{
    ...
    for (auto& p : inode_map) {
        ...
        if (in->is_head() && in->is_any_caps()) {
            ...
            if (!mds->locker->eval(in, CEPH_CAP_LOCKS)) n += mds->locker->issue_caps(in);
        }

        if ((count % mds->heartbeat_reset_grace()) + n >= mds->heartbeat_reset_grace()) mds->heartbeat_reset();
        count += n;
    }
}
...
struct C_MDC_QueuedCow : public MDCacheContext
{
    ...
    C_MDC_QueuedCow(MDCache* mdc, CInode* i, MutationRef& m)
        ...
        , mut(m)
    {}
    void finish(int r) override { mdcache->_queued_file_recover_cow(in, mut); }
};
...
void MDCache::start_files_to_recover()
{
    ...
    for (CInode* in : rejoin_check_q) {
        if (in->filelock.get_state() == LOCK_XLOCKSNAP) mds->locker->issue_caps(in);
...
public:
    C_MDC_RetryTruncate(MDCache* c, CInode* i, LogSegment* l)
        ...
        , ls(l)
    {}
    void finish(int r) override { mdcache->_truncate_inode(in, ls); }
};
...
struct C_IO_MDC_TruncateWriteFinish : public MDCacheIOContext
{
    ...
    void print(ostream& out) const override { out << "file_truncate_write(" << in->ino() << ")"; }
};
...
struct C_IO_MDC_TruncateFinish : public MDCacheIOContext
{
    ...
    void print(ostream& out) const override { out << "file_truncate(" << in->ino() << ")"; }
};

void MDCache::_truncate_inode(CInode* in, LogSegment* ls)
{
    const auto& pi = in->get_inode();
    dout(10) << "_truncate_inode " << pi->truncate_from << " -> " << pi->truncate_size
             << " fscrypt last block length is " << pi->fscrypt_last_block.length() << " on " << *in << dendl;
    ...
    ceph_assert(pi->truncate_size < pi->truncate_from ||
                (pi->truncate_size == pi->truncate_from && pi->fscrypt_last_block.length()));
    ...
    dout(10) << "_truncate_inode  snapc " << snapc << " on " << *in << " fscrypt_last_block length is "
             << pi->fscrypt_last_block.length() << dendl;
    ...
    if (data.length()) {
        dout(10) << "_truncate_inode write on inode " << *in << " change_attr: " << header.change_attr
                 << " offset: " << header.file_offset << " blen: " << header.block_size << dendl;
        filer.write(in->ino(),
                    &layout,
                    *snapc,
                    header.file_offset,
                    ...
                    ceph::real_time::min(),
                    0,
                    new C_OnFinisher(new C_IO_MDC_TruncateWriteFinish(this, in, ls, header.block_size), mds->finisher));
    }
        ...
        filer.truncate(in->ino(),
                       ...
                       *snapc,
                       ...
                       ceph::real_time::min(),
                       0,
                       new C_OnFinisher(new C_IO_MDC_TruncateFinish(this, in, ls), mds->finisher));
    }
}
...
struct C_MDC_TruncateLogged : public MDCacheLogContext
{
    ...
    C_MDC_TruncateLogged(MDCache* m, CInode* i, MutationRef& mu)
        ...
        , mut(mu)
    {}
    void finish(int r) override { mdcache->truncate_inode_logged(in, mut); }
};

void MDCache::truncate_inode_write_finish(CInode* in, LogSegment* ls, uint32_t block_size)
{
    const auto& pi = in->get_inode();
    dout(10) << "_truncate_inode_write " << pi->truncate_from << " -> " << pi->truncate_size << " on " << *in << dendl;
    ...
    dout(10) << "_truncate_inode_write  snapc " << snapc << " on " << *in << " fscrypt_last_block length is "
             << pi->fscrypt_last_block.length() << dendl;
    auto layout = pi->layout;
    /*
     * When the fscrypt is enabled the truncate_from and truncate_size
...
void MDCache::truncate_inode_finish(CInode* in, LogSegment* ls)
{
    ...
    if (in->is_waiter_for(CInode::WAIT_TRUNC) || (in->get_caps_wanted() & (CEPH_CAP_FILE_RD | CEPH_CAP_FILE_WR)))
        mds->mdlog->flush();
}
...
void MDCache::start_recovered_truncates()
{
    ...
    for (map<uint64_t, LogSegment*>::iterator p = mds->mdlog->segments.begin(); p != mds->mdlog->segments.end(); ++p) {
        LogSegment* ls = p->second;
        for (set<CInode*>::iterator q = ls->truncating_inodes.begin(); q != ls->truncating_inodes.end(); ++q) {
            CInode* in = *q;
            in->auth_pin(this);
...
public:
    C_MDS_purge_completed_finish(MDCache* m, const interval_set<inodeno_t>& _inos, LogSegment* _ls, version_t iv)
        : MDCacheLogContext(m)
        , inos(_inos)
        , ls(_ls)
        , inotablev(iv)
...
void MDCache::purge_inodes(const interval_set<inodeno_t>& inos, LogSegment* ls)
{
    ...
    C_GatherBuilder gather(g_ceph_context, new C_OnFinisher(new MDSIOContextWrapper(mds, cb), mds->finisher));
    SnapContext nullsnapc;
    for (const auto& [start, len] : inos) {
        for (auto i = start; i < start + len; i += 1) {
            filer.purge_range(i, &default_file_layout, nullsnapc, 0, 1, ceph::real_clock::now(), 0, gather.new_sub());
        }
    }
    gather.activate();
}
...
std::pair<bool, uint64_t> MDCache::trim_lru(uint64_t count, expiremap& expiremap)
{
    ...
    dout(7) << "trim_lru trimming " << count << " items from LRU"
            << " size=" << lru.lru_get_size() << " mid=" << lru.lru_get_top() << " pintail=" << lru.lru_get_pintail()
            << " pinned=" << lru.lru_get_num_pinned() << dendl;
...
std::pair<bool, uint64_t> MDCache::trim(uint64_t count)
{
    ...
    dout(7) << "trim bytes_used=" << bytes2str(used) << " limit=" << bytes2str(limit)
            << " reservation=" << cache_reservation << "% count=" << count << dendl;
    ...
    for (auto p = subtrees.begin(); p != subtrees.end();) {
        ...
        if (dir->is_auth()) {
            if (diri->is_auth() && !diri->is_base()) {
                ...
                if (dir->state_test(CDir::STATE_EXPORTING) || !(mds->is_active() || mds->is_stopping()) ||
            ...
            if (dir->is_subtree_root() && rejoin_ack_gather.count(dir->get_dir_auth().first)) continue;
    ...
    if (mds->is_stopping()) {
        for (set<CInode*>::iterator p = base_inodes.begin(); p != base_inodes.end();) {
            CInode* base_in = *p;
            ++p;
            if (MDS_INO_IS_MDSDIR(base_in->ino()) && MDS_INO_MDSDIR_OWNER(base_in->ino()) != mds->get_nodeid()) {
    ...
    for (const auto& p : expiremap) {
        if (mds->is_cluster_degraded() &&
            (mds->mdsmap->get_state(p.first) < MDSMap::STATE_REJOIN ||
             (mds->mdsmap->get_state(p.first) == MDSMap::STATE_REJOIN && rejoin_sent.count(p.first) == 0))) {
            continue;
        }
...
bool MDCache::trim_dentry(CDentry* dn, expiremap& expiremap)
{
    ...
    if (!dn->is_auth() && !dn->lock.can_read(-1) && !dn->get_dir()->get_inode()->is_stray()) return true;
    ...
    if (dn->is_auth() && !(dnl->is_null() && dn->is_clean())) clear_complete = true;
    ...
    if (dnl->is_remote()) {
        ...
        CInode* in = dnl->get_inode();
        ceph_assert(in);
        if (trim_inode(dn, in, con, expiremap)) return true;   // purging stray instead of trimming
    }
    else {
        ceph_assert(dnl->is_null());
    }
    ...
    if (!dn->is_auth()) {
        ...
        for (int p = 0; p < 2; p++) {
            ...
            if (mds->get_nodeid() == auth.second && con->is_importing())
                break;                              // don't send any expire while importing.
            ...
            if (em.second) em.first->second = make_message<MCacheExpire>(mds->get_nodeid());
            em.first->second->add_dentry(
                con->dirfrag(), dir->dirfrag(), dn->get_name(), dn->last, dn->get_replica_nonce());
        }
    }

    if (clear_complete) {
        if (dn->last == CEPH_NOSNAP) dir->add_to_bloom(dn);
        dir->state_clear(CDir::STATE_COMPLETE);
    }
...
void MDCache::trim_dirfrag(CDir* dir, CDir* con, expiremap& expiremap)
{
    ...
    if (dir->is_subtree_root()) {
        ceph_assert(!dir->is_auth() || (!dir->is_replicated() && dir->inode->is_base()));
    ...
    if (!dir->is_auth()) {
        ...
        for (int p = 0; p < 2; p++) {
            ...
            ceph_assert(a != mds->get_nodeid());
            auto em = expiremap.emplace(std::piecewise_construct, std::forward_as_tuple(a), std::forward_as_tuple());
            if (em.second) em.first->second = make_message<MCacheExpire>(mds->get_nodeid()); /* new */
...
bool MDCache::trim_inode(CDentry* dn, CInode* in, CDir* con, expiremap& expiremap)
{
    ...
    if (in->is_auth()) {
        ...
        if (dn && !dn->state_test(CDentry::STATE_PURGING)) {
            maybe_eval_stray(in);
            if (dn->state_test(CDentry::STATE_PURGING) || dn->get_num_ref() > 0) return true;
        }
    }
    else {
        ...
        for (int p = 0; p < 2; p++) {
            ...
            if (p) a = auth.second;
            if (a < 0 || (p == 1 && auth.second == auth.first)) break;
            if (con && mds->get_nodeid() == auth.second && con->is_importing())
                break;                              // don't send any expire while importing.
            if (a == mds->get_nodeid()) continue;   // on export, ignore myself.
    ...
    */
    ...
    // unlink
    if (dn) dn->get_dir()->unlink_inode(dn, false);
    remove_inode(in);
    return false;
}
...
void MDCache::trim_non_auth()
{
    ...
    for (;;) {
        CDentry* dn = NULL;
        if (bottom_lru.lru_get_size() > 0) dn = static_cast<CDentry*>(bottom_lru.lru_expire());
        if (!dn && lru.lru_get_size() > 0) dn = static_cast<CDentry*>(lru.lru_expire());
        if (!dn) break;
        ...
        if (dn->is_auth()) {
            ...
            if (dnl->is_remote() && dnl->get_inode() && !dnl->get_inode()->is_auth()) dn->unlink_remote(dnl);
        }
        else {
            // non-auth.  expire.
            CDir* dir = dn->get_dir();
            ...
            // close empty non-auth dirfrag
            if (!dir->is_subtree_root() && dir->get_num_any() == 0) dir->inode->close_dirfrag(dir->get_frag());
        }
    }
    ...
    while (j != dir->end()) {
        ...
        if (dnl->is_primary()) {   // check for subdirectories, etc
            ...
            if (!keep_inode) {   // remove it!
                dout(20) << "trim_non_auth_subtree(" << dir << ") removing inode " << in << " with dentry" << dn
                         << dendl;
                dir->unlink_inode(dn, false);
                remove_inode(in);
                ceph_assert(!dir->has_bloom());
                ...
                dout(20) << "trim_non_auth_subtree(" << dir << ") keeping inode " << in << " with dentry " << dn
                         << dendl;
                dn->clear_auth();
                in->state_clear(CInode::STATE_AUTH);
            }
            ...
            if (dnl->is_remote()) dir->unlink_inode(dn, false);
            dir->remove_dentry(dn);
        }
    }
    dir->state_clear(CDir::STATE_AUTH);
...
void MDCache::try_trim_non_auth_subtree(CDir* dir)
{
    ...
    if (trim_non_auth_subtree(dir)) {
        ...
        while (true) {
            ...
            if (psub->get_dir_auth().first == mds->get_nodeid()) break;   // we are auth, keep.
            ...
            if (trim_non_auth_subtree(psub)) break;
            dir = psub;
        }
    }
...
void MDCache::standby_trim_segment(LogSegment* ls)
{
    auto try_trim_inode = [this](CInode* in) {
        if (in->get_num_ref() == 0 && !in->item_open_file.is_on_list() && in->parent != NULL &&
    ...
    auto try_trim_dentry = [this](CDentry* dn) {
        if (dn->get_num_ref() > 0) return;
        auto in = dn->get_linkage()->inode;
        if (in && in->item_open_file.is_on_list()) return;
    ...
    while (!ls->dirty_dirfrags.empty()) {
        ...
        dir->mark_clean();
        if (dir->inode) try_trim_inode(dir->inode);
    }
    ...
    for (const auto& p : m->realms) {
        ...
        if (p.first.ino > 0) {
            ...
            if (!parent_dir->is_auth() ||
                (export_state != -1 &&
                 ((export_state == Migrator::EXPORT_WARNING && migrator->export_has_warned(parent_dir, from)) ||
                  export_state == Migrator::EXPORT_EXPORTING || export_state == Migrator::EXPORT_LOGGINGFINISH ||
                  (export_state == Migrator::EXPORT_NOTIFYING && !migrator->export_has_notified(parent_dir, from))))) {
                ...
                auto em = delayed_expire[parent_dir].emplace(
                    std::piecewise_construct, std::forward_as_tuple(from), std::forward_as_tuple());
                if (em.second) em.first->second = make_message<MCacheExpire>(from); /* new */
        ...
        for (const auto& q : p.second.inodes) {
            ...
            if (nonce == in->get_replica_nonce(from)) {
                ...
                dout(7) << " inode expire on " << *in << " from mds." << from << " cached_by was " << in->get_replicas()
                        << dendl;
                inode_remove_replica(in, from, false, gather_locks);
            }
            else {
                // this is an old nonce, ignore expire.
                dout(7) << " inode expire on " << *in << " from mds." << from << " with old nonce " << nonce
                        << " (current " << in->get_replica_nonce(from) << "), dropping" << dendl;
            }
        }
        ...
        for (const auto& q : p.second.dirs) {
            ...
            if (!dir) {
                ...
                if (diri) {
                    ...
                    if (other) {
                        dout(7) << " dir expire on dirfrag " << q.first << " from mds." << from << " have " << *other
            ...
            if (nonce == dir->get_replica_nonce(from)) {
                // remove from our cached_by
                dout(7) << " dir expire on " << *dir << " from mds." << from << " replicas was " << dir->get_replicas()
                        << dendl;
                dir->remove_replica(from);
            }
            else {
                // this is an old nonce, ignore expire.
                dout(7) << " dir expire on " << *dir << " from mds." << from << " with old nonce " << nonce
                        << " (current " << dir->get_replica_nonce(from) << "), dropping" << dendl;
            }
        }
        ...
        for (const auto& pd : p.second.dentries) {
            ...
            for (const auto& p : pd.second) {
                ...
                if (!dn) {
                    if (dir)
                        dout(0) << "  missing dentry for " << p.first.first << " snap " << p.first.second << " in "
                                << *dir << dendl;
                    else
                ...
                if (nonce == dn->get_replica_nonce(from)) {
                    ...
                    dout(7) << "  dentry_expire on " << *dn << " from mds." << from << " with old nonce " << nonce
                            << " (current " << dn->get_replica_nonce(from) << "), dropping" << dendl;
                }
            }
        }
    }
...
void MDCache::inode_remove_replica(CInode* in, mds_rank_t from, bool rejoin, set<SimpleLock*>& gather_locks)
{
...
void MDCache::dentry_remove_replica(CDentry* dn, mds_rank_t from, set<SimpleLock*>& gather_locks)
{
    ...
    // fix lock
    if (dn->lock.remove_replica(from)) gather_locks.insert(&dn->lock);
...
void MDCache::trim_client_leases()
{
    ...
    for (const auto& list : client_leases) {
        ...
        dout(10) << "trim_client_leases pool " << pool << " trimmed " << (before - after) << " leases, " << after
                 << " left" << dendl;
    }
}
...
void MDCache::check_memory_usage()
{
    ...
    if (CInode::count()) caps_per_inode = (double)Capability::count() / (double)CInode::count();

    dout(2) << "Memory usage: "
            << " total " << last.get_total() << ", rss " << last.get_rss() << ", heap " << last.get_heap()
            << ", baseline " << baseline.get_heap() << ", " << num_inodes_with_caps << " / " << CInode::count()
            << " inodes have caps"
            << ", " << Capability::count() << " caps, " << caps_per_inode << " caps per inode" << dendl;
...
{
public:
    explicit C_MDC_ShutdownCheck(MDCache* m)
        : MDCacheContext(m)
    {}
    void finish(int) override { mdcache->shutdown_check(); }
};
...
bool MDCache::shutdown_pass()
{
    ...
    if (!subtrees.empty() && mds->get_nodeid() != 0) {
        ...
        for (auto& [dir, bounds] : subtrees) {
            ...
            if (dir->get_inode()->is_mdsdir() || !dir->is_auth()) continue;
            num_auth_subtree++;
            if (dir->is_frozen() || dir->is_freezing() || dir->is_ambiguous_dir_auth() ||
                dir->state_test(CDir::STATE_EXPORTING) || dir->get_inode()->is_ephemerally_pinned()) {
                continue;
            }
            ls.push_back(dir);
        }
    ...
    if (mds->sessionmap.have_unclosed_sessions()) {
        if (!mds->server->terminating_sessions) mds->server->terminate_sessions();
        return false;
    }
    ...
    for (int i = 0; i < NUM_STRAY; ++i) {
        if (strays[i] && strays[i]->state_test(CInode::STATE_STRAYPINNED)) {
    ...
    if (mydir && !mydir->is_subtree_root()) mydir = NULL;
    ...
    if (subtrees.size() > (mydir ? 1 : 0)) {
        ...
        migrator->show_importing();
        migrator->show_exporting();
        if (!migrator->is_importing() && !migrator->is_exporting()) show_cache();
        return false;
    }
    ceph_assert(!migrator->is_exporting());
    ceph_assert(!migrator->is_importing());
    ...
    if ((myin && myin->get_num_auth_pins()) || (mydir && (mydir->get_auth_pins() || mydir->get_dir_auth_pins()))) {
    ...
    if (!mds->mdlog->empty()) mds->mdlog->trim(0);

    if (!mds->mdlog->empty()) {
        dout(7) << "waiting for log to flush.. " << mds->mdlog->get_num_events() << " in "
                << mds->mdlog->get_num_segments() << " segments" << dendl;
        return false;
    }
    ...
    if (lru.lru_get_size() > 0 || bottom_lru.lru_get_size() > 0) {
        dout(7) << "there's still stuff in the cache: " << lru.lru_get_size() << "/" << bottom_lru.lru_get_size()
                << dendl;
        show_cache();
        // dump();
        return false;
...
bool MDCache::shutdown_export_strays()
{
    ...
    if (shutdown_exporting_strays.size() * 3 >= MAX_EXPORTING * 2) return false;

    dout(10) << "shutdown_export_strays " << shutdown_export_next.first << " '" << shutdown_export_next.second << "'"
             << dendl;
...
again:
    ...
    for (int i = 0; i < NUM_STRAY; ++i) {
        ...
        if (!strayi || !strayi->state_test(CInode::STATE_STRAYPINNED)) continue;
        if (strayi->ino() < next.first.ino) continue;
        ...
        while (!dfls.empty()) {
            ...
            if (dir->dirfrag() < next.first) continue;
            ...
            if (!dir->is_complete()) {
                ...
                if (shutdown_exporting_strays.empty()) {
                    fin = new MDSInternalContextWrapper(mds,
                                                        new LambdaContext([this](int r) { shutdown_export_strays(); }));
                }
                dir->fetch(fin);
                goto done;
            }
    ...
    if (shutdown_exporting_strays.empty()) {
        ...
        if (first_df < shutdown_export_next.first || !shutdown_export_next.second.empty()) {
...
void MDCache::dispatch(const cref_t<Message>& m)
{
    switch (m->get_type()) {
        ...
        // RESOLVE
    case MSG_MDS_RESOLVE: handle_resolve(ref_cast<MMDSResolve>(m)); break;
    case MSG_MDS_RESOLVEACK:
        handle_resolve_ack(ref_cast<MMDSResolveAck>(m));
        break;

        // REJOIN
    case MSG_MDS_CACHEREJOIN: handle_cache_rejoin(ref_cast<MMDSCacheRejoin>(m)); break;

    case MSG_MDS_DISCOVER: handle_discover(ref_cast<MDiscover>(m)); break;
    case MSG_MDS_DISCOVERREPLY: handle_discover_reply(ref_cast<MDiscoverReply>(m)); break;

    case MSG_MDS_DIRUPDATE: handle_dir_update(ref_cast<MDirUpdate>(m)); break;

    case MSG_MDS_CACHEEXPIRE: handle_cache_expire(ref_cast<MCacheExpire>(m)); break;

    case MSG_MDS_DENTRYLINK: handle_dentry_link(ref_cast<MDentryLink>(m)); break;
    case MSG_MDS_DENTRYUNLINK: handle_dentry_unlink(ref_cast<MDentryUnlink>(m)); break;

    case MSG_MDS_FRAGMENTNOTIFY: handle_fragment_notify(ref_cast<MMDSFragmentNotify>(m)); break;
    case MSG_MDS_FRAGMENTNOTIFYACK: handle_fragment_notify_ack(ref_cast<MMDSFragmentNotifyAck>(m)); break;

    case MSG_MDS_FINDINO: handle_find_ino(ref_cast<MMDSFindIno>(m)); break;
    case MSG_MDS_FINDINOREPLY: handle_find_ino_reply(ref_cast<MMDSFindInoReply>(m)); break;

    case MSG_MDS_OPENINO: handle_open_ino(ref_cast<MMDSOpenIno>(m)); break;
    case MSG_MDS_OPENINOREPLY: handle_open_ino_reply(ref_cast<MMDSOpenInoReply>(m)); break;

    case MSG_MDS_SNAPUPDATE: handle_snap_update(ref_cast<MMDSSnapUpdate>(m)); break;

    default: derr << "cache unknown message " << m->get_type() << dendl; ceph_abort_msg("cache unknown message");
    }
}

int MDCache::path_traverse(MDRequestRef& mdr, MDSContextFactory& cf, const filepath& path, int flags,
                           vector<CDentry*>* pdnvec, CInode** pin)
{
    ...
    if (forward) ceph_assert(mdr);   // forward requires a request

    snapid_t snapid = CEPH_NOSNAP;
    if (mdr) mdr->snapid = snapid;
    ...
    if (!cur) {
        ...
        if (MDS_INO_IS_STRAY(path.get_ino())) {
            ...
            filepath path(strays[idx]->get_parent_dn()->get_name(), MDS_INO_MDSDIR(rank));
            MDRequestRef null_ref;
            return path_traverse(null_ref, cf, path, MDS_TRAVERSE_DISCOVER, nullptr);
        }
        return -CEPHFS_ESTALE;
    }
    if (cur->state_test(CInode::STATE_PURGING)) return -CEPHFS_ESTALE;

    if (flags & MDS_TRAVERSE_CHECK_LOCKCACHE) mds->locker->find_and_attach_lock_cache(mdr, cur);

    if (mdr && mdr->lock_cache) {
        if (flags & MDS_TRAVERSE_WANT_DIRLAYOUT) mdr->dir_layout = mdr->lock_cache->get_dir_layout();
    }
    else if (rdlock_snap) {
        ...
        if ((n == 0 && !(mdr->locking_state & MutationImpl::SNAP_LOCKED)) ||
            ...
            if (!mds->locker->try_rdlock_snap_layout(cur, mdr, n, want_layout)) return 1;
        }
    }
    ...
    for (unsigned depth = 0; depth < path.depth();) {
        ...
        if (snapid == CEPH_SNAPDIR) {
            if (!mdr) return -CEPHFS_EINVAL;
            SnapRealm* realm = cur->find_snaprealm();
            snapid = realm->resolve_snapname(path[depth], cur->ino());
            dout(10) << "traverse: snap " << path[depth] << " -> " << snapid << dendl;
            if (!snapid) {
                if (pdnvec) pdnvec->clear();   // do not confuse likes of rdlock_path_pin_ref();
                return -CEPHFS_ENOENT;
            }
            if (depth == path.depth() - 1) target_inode = cur;
            mdr->snapid = snapid;
            depth++;
            continue;
        }
        ...
        if (!curdir) {
            if (cur->is_auth()) {
                ...
                discover_path(cur, snapid, path.postfixpath(depth), cf.build(), path_locked);
        ...
        if (curdir->is_complete()) curdir->verify_fragstat();
#endif
        ...
        */
        ...
        if (mds->damage_table.is_dentry_damaged(curdir, path[depth], snapid)) {
            dout(4) << "traverse: stopped lookup at damaged dentry " << *curdir << "/" << path[depth]
                    << " snap=" << snapid << dendl;
            return -CEPHFS_EIO;
        }
        ...
        if (dn) {
            ...
            if (rdlock_path) {
            ...
            else if (!path_locked && !dn->lock.can_read(client) &&
                ...
                if (dn->is_auth() && dn->lock.is_unstable_and_locked()) mds->mdlog->flush();
                return 1;
            }

            if (pdnvec) pdnvec->push_back(dn);

            // can we conclude CEPHFS_ENOENT?
            if (dnl->is_null()) {
                dout(10) << "traverse: null+readable dentry at " << *dn << dendl;
                if (depth == path.depth() - 1) {
                    if (want_dentry) break;
                }
                else {
                    if (pdnvec) pdnvec->clear();   // do not confuse likes of rdlock_path_pin_ref();
                }
            ...
            if (!in) {
                ...
                if (in) {
                    ...
                    open_remote_dentry(dn, true, cf.build(), (path_locked && depth == path.depth() - 1));
        ...
        if (curdir->is_auth()) {
            ...
            if (curdir->is_complete() ||
                (snapid == CEPH_NOSNAP && curdir->has_bloom() && !curdir->is_in_bloom(path[depth]))) {
                // file not found
                if (pdnvec) {
            ...
            if (forward && mdr && mdr->client_request && (int)depth < mdr->client_request->get_num_fwd()) {
                dout(7) << "traverse: snap " << snapid << " and depth " << depth << " < fwd "
                        << mdr->client_request->get_num_fwd() << ", discovering instead of forwarding" << dendl;
                discover = true;
            }

            if ((discover)) {
                dout(7) << "traverse: discover from " << path[depth] << " from " << *curdir << dendl;
                discover_path(curdir, snapid, path.postfixpath(depth), cf.build(), path_locked);
                if (mds->logger) mds->logger->inc(l_mds_traverse_discover);
                return 1;
            }
            if (forward) {
                ...
                if (r == 2 && mds->logger) mds->logger->inc(l_mds_traverse_forward);
    ...
    if (mdr) ceph_assert(mdr->snapid == snapid);
    ...
    if (rdlock_path) mdr->locking_state |= MutationImpl::PATH_LOCKED;

    return 0;
}

int MDCache::maybe_request_forward_to_auth(MDRequestRef& mdr, MDSContextFactory& cf, MDSCacheObject* p)
{
...
CInode* MDCache::get_dentry_inode(CDentry* dn, MDRequestRef& mdr, bool projected)
{
    ...
    if (dnl->is_primary()) return dnl->inode;
...
void MDCache::open_remote_dentry(CDentry* dn, bool projected, MDSContext* fin, bool want_xlocked)
{
    ...
    open_ino(
        ino, pool, new C_MDC_OpenRemoteDentry(this, dn, ino, fin, want_xlocked), true, want_xlocked);   // backtrace
}

void MDCache::_open_remote_dentry_finish(CDentry* dn, inodeno_t ino, MDSContext* fin, bool want_xlocked, int r)
{
    if (r < 0) {
        CDentry::linkage_t* dnl = dn->get_projected_linkage();
...
public:
    ...
    C_IO_MDC_OpenInoBacktraceFetched(MDCache* c, inodeno_t i)
        ...
        , ino(i)
    {}
    void finish(int r) override { mdcache->_open_ino_backtrace_fetched(ino, bl, r); }
    void print(ostream& out) const override { out << "openino_backtrace_fetch" << ino << ")"; }
};
...
public:
    C_MDC_OpenInoParentOpened(MDCache* c, inodeno_t i)
        : MDCacheContext(c)
        , ino(i)
    {}
    void finish(int r) override { mdcache->_open_ino_parent_opened(ino, r); }
};
...
void MDCache::_open_ino_backtrace_fetched(inodeno_t ino, bufferlist& bl, int err)
{
    ...
    if (err == 0) {
        ...
        if (backtrace.pool != info.pool && backtrace.pool != -1) {
            ...
            fetch_backtrace(ino, info.pool, fin->bl, new C_OnFinisher(fin, mds->finisher));
            return;
        }
    }
    ...
    if (err) {
        ...
        if (info.last_err) err = info.last_err;
...
void MDCache::_open_ino_traverse_dir(inodeno_t ino, open_ino_info_t& info, int ret)
{
    ...
    ret = open_ino_traverse_dir(ino, NULL, info.ancestors, info.discover, info.want_xlocked, &hint);
    if (ret > 0) return;
    if (hint != mds->get_nodeid()) info.auth_hint = hint;
    do_open_ino(ino, info, ret);
}

void MDCache::_open_ino_fetch_dir(inodeno_t ino, const cref_t<MMDSOpenIno>& m, bool parent, CDir* dir,
                                  std::string_view dname)
{
    if (dir->state_test(CDir::STATE_REJOINUNDEF)) ceph_assert(dir->get_inode()->dirfragtree.is_leaf(dir->get_frag()));
    ...
    if (mds->logger) mds->logger->inc(l_mds_openino_dir_fetch);
}

int MDCache::open_ino_traverse_dir(inodeno_t ino, const cref_t<MMDSOpenIno>& m,
                                   const vector<inode_backpointer_t>& ancestors, bool discover, bool want_xlocked,
                                   mds_rank_t* hint)
{
    ...
    for (unsigned i = 0; i < ancestors.size(); i++) {
        ...
        if (dir) {
            ...
            if (dir->is_auth()) {
                if (dnl && dnl->is_primary() && dnl->get_inode()->state_test(CInode::STATE_REJOINUNDEF)) {
                    dout(10) << " fetching undef " << *dnl->get_inode() << dendl;
                    _open_ino_fetch_dir(ino, m, i == 0, dir, name);
                    return 1;
                }

                if (!dnl && !dir->is_complete() && (!dir->has_bloom() || dir->is_in_bloom(name))) {
                ...
                if (!dnl) {
                    ...
                    discover_path(dir,
                                  CEPH_NOSNAP,
                                  path,
                                  new C_MDC_OpenInoTraverseDir(this, ino, m, i == 0),
                                  (i == 0 && want_xlocked));
                    return 1;
                }
        ...
        if (hint && i == 0) *hint = dir ? dir->authority().first : diri->authority().first;
        break;
    }
    return err;
}
...
void MDCache::do_open_ino(inodeno_t ino, open_ino_info_t& info, int err)
{
    if (err < 0 && err != -CEPHFS_EAGAIN) {
        ...
        if (err != -CEPHFS_ENOENT && err != -CEPHFS_ENOTDIR) info.last_err = err;
    }
    ...
    if (info.check_peers || info.discover) {
        ...
        info.checking = mds->get_nodeid();
        open_ino(info.ancestors[0].dirino,
                 mds->get_metadata_pool(),
...
void MDCache::do_open_ino_peer(inodeno_t ino, open_ino_info_t& info)
{
    ...
    dout(10) << "do_open_ino_peer " << ino << " active " << active << " all " << all << " checked " << info.checked
             << dendl;
    ...
    if (peer < 0) {
        ...
        if (info.discover || !info.fetch_backtrace) pa = &info.ancestors;
        mds->send_message_mds(make_message<MMDSOpenIno>(info.tid, ino, pa), peer);
        if (mds->logger) mds->logger->inc(l_mds_openino_peer_discover);
    }
}

void MDCache::handle_open_ino(const cref_t<MMDSOpenIno>& m, int err)
{
    if (mds->get_state() < MDSMap::STATE_REJOIN && mds->get_want_state() != CEPH_MDS_STATE_REJOIN) {
        return;
    }
    ...
    if (in) {
        ...
        if (in->is_auth()) {
            ...
            while (1) {
                ...
                reply->ancestors.push_back(inode_backpointer_t(diri->ino(), pdn->get_name(), in->get_version()));
                in = diri;
            }
        }
        else {
...
void MDCache::kick_open_ino_peers(mds_rank_t who)
{
    ...
    for (map<inodeno_t, open_ino_info_t>::iterator p = opening_inodes.begin(); p != opening_inodes.end(); ++p) {
...
void MDCache::open_ino_batch_submit()
{
    ...
    for (auto& [dir, p] : open_ino_batched_fetch) {
        ...
        for (auto& dname : p.first) keys.emplace_back(CEPH_NOSNAP, dname, in->hash_dentry_name(dname));
        dir->fetch_keys(
            keys,
            new MDSInternalContextWrapper(mds, new LambdaContext([this, waiters = std::move(p.second)](int r) mutable {
...
void MDCache::open_ino(inodeno_t ino, int64_t pool, MDSContext* fin, bool want_replica, bool want_xlocked,
                       vector<inode_backpointer_t>* ancestors_hint, mds_rank_t auth_hint)
{
    ...
    if (it != opening_inodes.end()) {
        ...
        info.waiters.push_back(fin);
        if (auth_hint != MDS_RANK_NONE) info.auth_hint = auth_hint;
        if (ancestors_hint) {
 ...
 */
void MDCache::find_ino_peers(inodeno_t ino, MDSContext* c, mds_rank_t hint, bool path_locked)
{
...
void MDCache::_do_find_ino_peer(find_ino_peer_info_t& fip)
{
    ...
    dout(10) << "_do_find_ino_peer " << fip.tid << " " << fip.ino << " active " << active << " all " << all
             << " checked " << fip.checked << dendl;
    ...
    if (fip.hint >= 0) {
        ...
        for (set<mds_rank_t>::iterator p = active.begin(); p != active.end(); ++p)
            if (*p != mds->get_nodeid() && fip.checked.count(*p) == 0) {
                m = *p;
                break;
            }
    }
...
void MDCache::handle_find_ino_reply(const cref_t<MMDSFindInoReply>& m)
{
    ...
    if (p != find_ino_peer.end()) {
        ...
        mds_rank_t from = mds_rank_t(m->get_source().num());
        if (fip.checking == from) fip.checking = MDS_RANK_NONE;
        ...
        if (!m->path.empty()) {
            ...
            if (fip.path_locked) flags |= MDS_TRAVERSE_PATH_LOCKED;
...
void MDCache::kick_find_ino_peers(mds_rank_t who)
{
    ...
    for (map<ceph_tid_t, find_ino_peer_info_t>::iterator p = find_ino_peer.begin(); p != find_ino_peer.end(); ++p) {
        find_ino_peer_info_t& fip = p->second;
        if (fip.checking == who) {
...
int MDCache::get_num_client_requests()
{
    ...
    for (ceph::unordered_map<metareqid_t, MDRequestRef>::iterator p = active_requests.begin();
         ...
         ++p) {
        MDRequestRef& mdr = p->second;
        if (mdr->reqid.name.is_client() && !mdr->is_peer()) count++;
    }
    return count;
}
...
void MDCache::request_finish(MDRequestRef& mdr)
{
    ...
    case CEPH_MDS_OP_FRAGMENTDIR: logger->inc(l_mdss_ireq_fragmentdir); break;
    case CEPH_MDS_OP_EXPORTDIR: logger->inc(l_mdss_ireq_exportdir); break;
    case CEPH_MDS_OP_ENQUEUE_SCRUB: logger->inc(l_mdss_ireq_enqueue_scrub); break;
    case CEPH_MDS_OP_FLUSH: logger->inc(l_mdss_ireq_flush); break;
    case CEPH_MDS_OP_REPAIR_FRAGSTATS: logger->inc(l_mdss_ireq_fragstats); break;
    case CEPH_MDS_OP_REPAIR_INODESTATS: logger->inc(l_mdss_ireq_inodestats); break;
    }
...
void MDCache::dispatch_request(MDRequestRef& mdr)
{
    if (mdr->client_request) {
        ...
        switch (mdr->internal_op) {
        case CEPH_MDS_OP_FRAGMENTDIR: dispatch_fragment_dir(mdr); break;
        case CEPH_MDS_OP_EXPORTDIR: migrator->dispatch_export_dir(mdr, 0); break;
        case CEPH_MDS_OP_ENQUEUE_SCRUB: enqueue_scrub_work(mdr); break;
        case CEPH_MDS_OP_FLUSH: flush_dentry_work(mdr); break;
        case CEPH_MDS_OP_REPAIR_FRAGSTATS: repair_dirfrag_stats_work(mdr); break;
        case CEPH_MDS_OP_REPAIR_INODESTATS: repair_inode_stats_work(mdr); break;
        case CEPH_MDS_OP_RDLOCK_FRAGSSTATS: rdlock_dirfrags_stats_work(mdr); break;
        default: ceph_abort();
        }
    }
}
...
{
    if (!mdr->has_more()) return;

    // clean up peers
    //  (will implicitly drop remote dn pins)
    for (set<mds_rank_t>::iterator p = mdr->more()->peers.begin(); p != mdr->more()->peers.end(); ++p) {
        auto r = make_message<MMDSPeerRequest>(mdr->reqid, mdr->attempt, MMDSPeerRequest::OP_FINISH);

        if (mdr->killed && !mdr->committing) {
            r->mark_abort();
        }
        else if (mdr->more()->srcdn_auth_mds == *p && mdr->more()->inode_import.length() > 0) {
    ...
    for (auto it = mdr->locks.begin(); it != mdr->locks.end();) {
        ...
        if (it->is_xlock() && !lock->get_parent()->is_auth()) {
            ...
            dout(10) << "request_drop_foreign_locks forgetting remote_wrlock " << *lock << " on mds."
                     << it->wrlock_target << " on " << *lock->get_parent() << dendl;
            if (it->is_wrlock()) {
...
void MDCache::request_cleanup(MDRequestRef& mdr)
{
    ...
    if (mdr->has_more()) {
        if (mdr->more()->is_ambiguous_auth) mdr->clear_ambiguous_auth();
        if (!mdr->more()->waiting_for_finish.empty()) mds->queue_waiters(mdr->more()->waiting_for_finish);
    }
...
void MDCache::request_kill(MDRequestRef& mdr)
{
    ...
    if (mdr->has_more() && (!mdr->more()->witnessed.empty() || !mdr->more()->waiting_on_peer.empty())) {
...
void MDCache::do_realm_invalidate_and_update_notify(CInode* in, int snapop, bool notify_clients)
{
    ...
    if (notify_clients) {
        if (snapop == CEPH_SNAP_OP_SPLIT) {
            ...
            for (auto p = in->snaprealm->inodes_with_caps.begin(); !p.end(); ++p) split_inos.push_back((*p)->ino());

            for (auto& r : in->snaprealm->open_children) split_realms.push_back(r->inode->ino());
        }
    }
    ...
    while (!q.empty()) {
        ...
        if (notify_clients) {
            for (const auto& p : realm->client_caps) {
                ...
                auto em =
                    updates.emplace(std::piecewise_construct, std::forward_as_tuple(client), std::forward_as_tuple());
                if (em.second) {
                    auto update = make_message<MClientSnap>(CEPH_SNAP_OP_SPLIT);
        ...
        for (auto& r : realm->open_children) q.push_back(r);
    }

    if (notify_clients) send_snaps(updates);
}
...
void MDCache::send_snap_update(CInode* in, version_t stid, int snap_op)
{
    ...
    if (stid > 0) notify_global_snaprealm_update(snap_op);
}
...
void MDCache::handle_snap_update(const cref_t<MMDSSnapUpdate>& m)
{
    ...
    if (mds->get_state() < MDSMap::STATE_RESOLVE && mds->get_want_state() != CEPH_MDS_STATE_RESOLVE) {
        return;
    }

    // null rejoin_done means open_snaprealms() has already been called
    bool notify_clients = mds->get_state() > MDSMap::STATE_REJOIN || (mds->is_rejoin() && !rejoin_done);

    if (m->get_tid() > 0) {
        mds->snapclient->notify_commit(m->get_tid());
        if (notify_clients) notify_global_snaprealm_update(m->get_snap_op());
    }

    CInode* in = get_inode(m->get_ino());
    if (in) {
        ceph_assert(!in->is_auth());
        if (mds->get_state() > MDSMap::STATE_REJOIN || (mds->is_rejoin() && !in->is_rejoining())) {
...
{
    if (snap_op != CEPH_SNAP_OP_DESTROY) snap_op = CEPH_SNAP_OP_UPDATE;
    set<Session*> sessions;
    mds->sessionmap.get_client_session_set(sessions);
    for (auto& session : sessions) {
        if (!session->is_open() && !session->is_stale()) continue;
...
struct C_MDC_RetryScanStray : public MDCacheContext
{
    ...
    C_MDC_RetryScanStray(MDCache* c, dirfrag_t n)
        ...
        , next(n)
    {}
    void finish(int r) override { mdcache->scan_stray_dir(next); }
};
...
void MDCache::scan_stray_dir(dirfrag_t next)
{
    ...
    if (next.ino) next.frag = strays[MDS_INO_STRAY_INDEX(next.ino)]->dirfragtree[next.frag.value()];

    for (int i = 0; i < NUM_STRAY; ++i) {
        if (strays[i]->ino() < next.ino) continue;

        std::vector<CDir*> ls;
        strays[i]->get_dirfrags(ls);

        for (const auto& dir : ls) {
            if (dir->get_frag() < next.frag) continue;
            ...
            for (auto& p : dir->items) {
                ...
                if (dnl->is_primary()) {
                    CInode* in = dnl->get_inode();
                    if (in->get_inode()->nlink == 0) in->state_set(CInode::STATE_ORPHAN);
                    maybe_eval_stray(in);
                }
            }
        }
...
void MDCache::fetch_backtrace(inodeno_t ino, int64_t pool, bufferlist& bl, Context* fin)
{
    ...
    if (mds->logger) mds->logger->inc(l_mds_openino_backtrace_fetch);
}
...
{
    auto dis = make_message<MDiscover>(d.ino, d.frag, d.snap, d.want_path, d.want_base_dir, d.path_locked);
    logger->inc(l_mdc_dir_send_discover);
    dis->set_tid(d.tid);
    mds->send_message_mds(dis, d.mds);
}

void MDCache::discover_base_ino(inodeno_t want_ino, MDSContext* onfinish, mds_rank_t from)
{
...
void MDCache::discover_dir_frag(CInode* base, frag_t approx_fg, MDSContext* onfinish, mds_rank_t from)
{
    ...
    if (onfinish) base->add_dir_waiter(approx_fg, onfinish);
}
...
struct C_MDC_RetryDiscoverPath : public MDCacheContext
{
    ...
    C_MDC_RetryDiscoverPath(MDCache* c, CInode* b, snapid_t s, filepath& p, mds_rank_t f)
        ...
        , from(f)
    {}
    void finish(int r) override { mdcache->discover_path(base, snapid, path, 0, from); }
};

void MDCache::discover_path(CInode* base, snapid_t snap, filepath want_path, MDSContext* onfinish, bool path_locked,
                            mds_rank_t from)
{
    if (from < 0) from = base->authority().first;

    dout(7) << "discover_path " << base->ino() << " " << want_path << " snap " << snap << " from mds." << from
            << (path_locked ? " path_locked" : "") << dendl;

    if (base->is_ambiguous_auth()) {
        dout(10) << " waiting for single auth on " << *base << dendl;
        if (!onfinish) onfinish = new C_MDC_RetryDiscoverPath(this, base, snap, want_path, from);
    ...
    if ((path_locked && want_path.depth() == 1) || !base->is_waiting_for_dir(fg) || !onfinish) {
        discover_info_t& d = _create_discover(from);
        d.ino = base->ino();
        d.pin_base(base);
        d.frag = fg;
    ...
    // register + wait
    if (onfinish) base->add_dir_waiter(fg, onfinish);
}
...
struct C_MDC_RetryDiscoverPath2 : public MDCacheContext
{
    ...
    C_MDC_RetryDiscoverPath2(MDCache* c, CDir* b, snapid_t s, filepath& p)
        ...
        , path(p)
    {}
    void finish(int r) override { mdcache->discover_path(base, snapid, path, 0); }
};

void MDCache::discover_path(CDir* base, snapid_t snap, filepath want_path, MDSContext* onfinish, bool path_locked)
{
    ...
    if (base->is_ambiguous_auth()) {
        ...
        if (!onfinish) onfinish = new C_MDC_RetryDiscoverPath2(this, base, snap, want_path);
        base->add_waiter(CDir::WAIT_SINGLEAUTH, onfinish);
        return;
    }

    if ((path_locked && want_path.depth() == 1) || !base->is_waiting_for_dentry(want_path[0].c_str(), snap) ||
        !onfinish) {
        discover_info_t& d = _create_discover(from);
        d.ino = base->ino();
...
{
    for (map<ceph_tid_t, discover_info_t>::iterator p = discovers.begin(); p != discovers.end(); ++p) {
        if (p->second.mds != who) continue;
...
void MDCache::handle_discover(const cref_t<MDiscover>& dis)
{
    ...
    // get started.
    if (MDS_INO_IS_BASE(dis->get_base_ino()) && !dis->wants_base_dir() && dis->get_want().depth() == 0) {
        // wants root
        dout(7) << "handle_discover from mds." << from << " wants base + " << dis->get_want().get_path() << " snap "
                << snapid << dendl;
        ...
        if (!cur && snapid != CEPH_NOSNAP) {
            cur = get_inode(dis->get_base_ino());
            if (cur && !cur->is_multiversion()) cur = NULL;   // nope!
        }

        if (!cur) {
            dout(7) << "handle_discover mds." << from << " don't have base ino " << dis->get_base_ino() << "." << snapid
                    << dendl;
            if (!dis->wants_base_dir() && dis->get_want().depth() > 0) reply->set_error_dentry(dis->get_dentry(0));
            reply->set_flag_error_dir();
        }
        else if (dis->wants_base_dir()) {
            dout(7) << "handle_discover mds." << from << " wants basedir+" << dis->get_want().get_path() << " has "
                    << *cur << dendl;
        }
        else {
            dout(7) << "handle_discover mds." << from << " wants " << dis->get_want().get_path() << " has " << *cur
                    << dendl;
        }
    }
    ...
    for (unsigned i = 0; cur && (i < dis->get_want().depth() || dis->get_want().depth() == 0); i++) {
        ...
        if (dis->get_want().depth()) {
            ...
            if (!cur->dirfragtree.is_leaf(fg)) fg = cur->dirfragtree[fg.value()];
        }
        CDir* curdir = cur->get_dirfrag(fg);

        if ((!curdir && !cur->is_auth()) || (curdir && !curdir->is_auth())) {
            ...
            if (reply->is_empty()) {
                ...
                if (dis->get_want().depth() > i) reply->set_error_dentry(dis->get_dentry(i));
            }
        ...
        else if (curdir->is_frozen_tree() || (curdir->is_frozen_dir() && fragment_are_all_frozen(curdir))) {
            if (!reply->is_empty()) {
        ...
        if (curdir->get_version() == 0) {
            ...
            if (!reply->trace.length()) reply->starts_with = MDiscoverReply::DIR;
        ...
        CDentry* dn = 0;
        std::string_view dname;
        if (dis->get_want().depth() > 0) dname = dis->get_dentry(i);
        if (curdir->get_version() == 0) {
            // fetch newly opened dir
        ...
        if (!dn) {
            if (!curdir->is_complete() &&
                !(dname.size() > 0 && snapid == CEPH_NOSNAP && curdir->has_bloom() && !curdir->is_in_bloom(dname))) {
                // readdir
            ...
            if (snapid != CEPH_NOSNAP && !reply->is_empty()) {
                dout(7) << "dentry " << dis->get_dentry(i) << " snap " << snapid << " dne, non-empty reply, stopping"
                        << dendl;
                break;
            }
        ...
        if (dn->state_test(CDentry::STATE_PURGING)) {
            if (reply->is_empty()) reply->set_flag_error_dn(dis->get_dentry(i));
            break;
        }
        ...
        // add dentry
        if (!reply->trace.length()) reply->starts_with = MDiscoverReply::DENTRY;
...
void MDCache::handle_discover_reply(const cref_t<MDiscoverReply>& m)
{
    ...
    */
    ...
    if (!p.end() && next == MDiscoverReply::INODE) {
        ...
        // take waiters?
        if (cur->is_base() && waiting_for_base_ino[from].count(cur->ino())) {
    ...
    if (m->is_flag_error_dir() && !cur->is_dir()) {
        ...
        if (who >= 0) dout(7) << " dir_auth_hint is " << m->get_dir_auth_hint() << dendl;
        ...
        if (m->get_error_dentry().length()) {
            ...
            if (dir && dir->is_waiting_for_dentry(m->get_error_dentry(), m->get_wanted_snapid())) {
                if (dir->is_auth() || dir->lookup(m->get_error_dentry())) {
                    dir->take_dentry_waiting(
                        m->get_error_dentry(), m->get_wanted_snapid(), m->get_wanted_snapid(), finished);
                }
                else {
                ...
                dout(7) << " doing nothing, have dir but nobody is waiting on dentry " << m->get_error_dentry()
                        << dendl;
        }
    }
        ...
        CDir* dir = cur->get_dirfrag(fg);
        if (dir && !dir->is_auth()) {
            dir->take_dentry_waiting(m->get_error_dentry(), m->get_wanted_snapid(), m->get_wanted_snapid(), error);
        }
    }
...
void MDCache::encode_replica_inode(CInode* in, mds_rank_t to, bufferlist& bl, uint64_t features)
{
    ceph_assert(in->is_auth());
                                 ...
                                 MDSContext::vec& finished)
{
    ...
    if (dir) {
        ...
        if (!diri->dirfragtree.is_leaf(df.frag)) {
            dout(7) << __func__ << " forcing frag " << df.frag << " to leaf in the fragtree " << diri->dirfragtree
                    << dendl;
        ...
        if (from != diri->authority().first || diri->is_ambiguous_auth() || diri->is_base())
            adjust_subtree_auth(dir, from);
...
void MDCache::decode_replica_dentry(CDentry*& dn, bufferlist::const_iterator& p, CDir* dir, MDSContext::vec& finished)
{
    ...
    if (is_new) {
        ...
        if (rino) dir->link_remote_inode(dn, rino, rdtype);
        if (need_recover) dn->lock.mark_need_recover();
    }
    else {
...
int MDCache::send_dir_updates(CDir* dir, bool bcast)
{
    ...
    if (bcast) {
        ...
        std::set_difference(
            mds_set.begin(), mds_set.end(), replica_set.begin(), replica_set.end(), std::inserter(who, who.end()));
    }
    else {
        for (const auto& p : dir->get_replicas()) {
            who.insert(p.first);
    ...
    mds_rank_t whoami = mds->get_nodeid();
    for (set<mds_rank_t>::iterator it = who.begin(); it != who.end(); ++it) {
        ...
        mds->send_message_mds(
            make_message<MDirUpdate>(mds->get_nodeid(), dir->dirfrag(), dir->dir_rep, dir_rep_set, path, bcast), *it);
    }
...
void MDCache::handle_dir_update(const cref_t<MDirUpdate>& m)
{
    ...
    if (!dir) {
        ...
        if (m->should_discover()) {
            ...
            if (r > 0) return;
            if (r == 0 && in->ino() == df.ino && in->get_approx_dirfrag(df.frag) == NULL) {
...
void MDCache::send_dentry_link(CDentry* dn, MDRequestRef& mdr)
{
    ...
    for (const auto& p : dn->get_replicas()) {
        ...
        if (mdr.get() && mdr->more()->witnessed.count(p.first)) continue;
        if (mds->mdsmap->get_state(p.first) < MDSMap::STATE_REJOIN ||
            (mds->mdsmap->get_state(p.first) == MDSMap::STATE_REJOIN && rejoin_gather.count(p.first)))
            continue;
        CDentry::linkage_t* dnl = dn->get_linkage();
        auto m =
            make_message<MDentryLink>(subtree->dirfrag(), dn->get_dir()->dirfrag(), dn->get_name(), dnl->is_primary());
        if (dnl->is_primary()) {
            dout(10) << __func__ << "  primary " << *dnl->get_inode() << dendl;
            encode_replica_inode(dnl->get_inode(), p.first, m->bl, mds->mdsmap->get_up_features());
        }
        else if (dnl->is_remote()) {
...
void MDCache::send_dentry_unlink(CDentry* dn, CDentry* straydn, MDRequestRef& mdr)
{
    ...
    for (set<mds_rank_t>::iterator it = replicas.begin(); it != replicas.end(); ++it) {
        // don't tell (rmdir) witnesses; they already know
        if (mdr.get() && mdr->more()->witnessed.count(*it)) continue;

        if (mds->mdsmap->get_state(*it) < MDSMap::STATE_REJOIN ||
            (mds->mdsmap->get_state(*it) == MDSMap::STATE_REJOIN && rejoin_gather.count(*it)))
            continue;
...
void MDCache::handle_dentry_unlink(const cref_t<MDentryUnlink>& m)
{
    ...
    CInode* strayin = nullptr;
    if (m->straybl.length()) decode_replica_stray(straydn, &strayin, m->straybl, mds_rank_t(m->get_source().num()));
    ...
    if (!dir) {
        ...
        if (!dn) {
            ...
            if (dnl->is_primary()) {
                ...
                // update subtree map?
                if (in->is_dir()) adjust_subtree_after_rename(in, dir, false);

                if (m->snapbl.length()) {
                    bool hadrealm = (in->snaprealm ? true : false);
                    in->decode_snap_blob(m->snapbl);
                    ceph_assert(in->snaprealm);
                    if (!hadrealm) do_realm_invalidate_and_update_notify(in, CEPH_SNAP_OP_SPLIT, false);
                }

                // send caps to auth (if we're not already)
                if (in->is_any_caps() && !in->state_test(CInode::STATE_EXPORTINGCAPS)) migrator->export_caps(in);
 ...
 */
void MDCache::adjust_dir_fragments(CInode* diri, frag_t basefrag, int bits, std::vector<CDir*>* resultfrags,
                                   MDSContext::vec& waiters, bool replay)
{
...
CDir* MDCache::force_dir_fragment(CInode* diri, frag_t fg, bool replay)
{
    ...
    if (!replay) mds->queue_waiters(waiters);
    return dir;
}

void MDCache::adjust_dir_fragments(CInode* diri, const std::vector<CDir*>& srcfrags, frag_t basefrag, int bits,
                                   std::vector<CDir*>* resultfrags, MDSContext::vec& waiters, bool replay)
{
    dout(10) << "adjust_dir_fragments " << basefrag << " bits " << bits << " srcfrags " << srcfrags << " on " << *diri
             << dendl;

    // adjust fragtree
    // yuck.  we may have discovered the inode while it was being fragmented.
    if (!diri->dirfragtree.is_leaf(basefrag)) diri->dirfragtree.force_to_leaf(g_ceph_context, basefrag);

    if (bits > 0) diri->dirfragtree.split(basefrag, bits);
    dout(10) << " new fragtree is " << diri->dirfragtree << dendl;

    if (srcfrags.empty()) return;

    // split
    CDir* parent_dir = diri->get_parent_dir();
    CDir* parent_subtree = 0;
    if (parent_dir) parent_subtree = get_subtree_root(parent_dir);
    ...
    if (bits > 0) {
        ...
        if (any_subtree) {
            ...
            for (const auto& dir : srcfrags) {
                ...
                if (parent_subtree) subtrees[parent_subtree].erase(dir);
            }
        }
        ...
        if (any_subtree) {
            ...
            if (parent_subtree) subtrees[parent_subtree].insert(f);
...
public:
    C_MDC_FragmentFrozen(MDCache* m, MDRequestRef& r)
        : MDSInternalContext(m->mds)
        , mdcache(m)
        , mdr(r)
    {}
    void finish(int r) override { mdcache->fragment_frozen(mdr, r); }
};
...
bool MDCache::can_fragment(CInode* diri, const std::vector<CDir*>& dirs)
{
    ...
    if (diri->get_parent_dir() && diri->get_parent_dir()->get_inode()->is_stray()) {
...
void MDCache::merge_dir(CInode* diri, frag_t frag)
{
    ...
    if (!can_fragment(diri, dirs)) return;
...
public:
    C_MDC_FragmentMarking(MDCache* m, MDRequestRef& r)
        : MDCacheContext(m)
        , mdr(r)
    {}
    void finish(int r) override { mdcache->fragment_mark_and_complete(mdr); }
};
...
bool MDCache::fragment_are_all_frozen(CDir* dir)
{
    ...
    for (p = fragments.lower_bound(dirfrag_t(dir->ino(), 0)); p != fragments.end() && p->first.ino == dir->ino(); ++p) {
        if (p->first.frag.contains(dir->get_frag())) return p->second.all_frozen;
    }
    ceph_abort();
    return false;
}
...
void MDCache::find_stale_fragment_freeze()
{
    ...
    for (map<dirfrag_t, fragment_info_t>::iterator p = fragments.begin(); p != fragments.end();) {
        ...
        for (const auto& d : info.dirs) {
            ...
            if (dir->is_frozen_dir()) continue;
            total_auth_pins += dir->get_auth_pins() + dir->get_dir_auth_pins();
        }
        if (total_auth_pins < 0) continue;
        if (info.last_cum_auth_pins != total_auth_pins) {
            info.last_cum_auth_pins = total_auth_pins;
            info.last_cum_auth_pins_change = now;
            continue;
        }
        if (info.last_cum_auth_pins_change >= cutoff) continue;
        dir = info.dirs.front();
        if (info.num_remote_waiters > 0 || (!dir->inode->is_root() && dir->get_parent_dir()->is_freezing())) {
...
public:
    C_MDC_FragmentPrep(MDCache* m, MDRequestRef& r)
        : MDCacheLogContext(m)
        , mdr(r)
    {}
    void finish(int r) override { mdcache->_fragment_logged(mdr); }
};
...
public:
    C_MDC_FragmentStore(MDCache* m, MDRequestRef& r)
        : MDCacheContext(m)
        , mdr(r)
    {}
    void finish(int r) override { mdcache->_fragment_stored(mdr); }
};
...
public:
    C_MDC_FragmentCommit(MDCache* m, dirfrag_t df, const MDRequestRef& r)
        ...
        , mdr(r)
    {}
    void finish(int r) override { mdcache->_fragment_committed(basedirfrag, mdr); }
};
...
public:
    C_IO_MDC_FragmentPurgeOld(MDCache* m, dirfrag_t f, int b, const MDRequestRef& r)
        : MDCacheIOContext(m)
        , basedirfrag(f)
        , bits(b)
        , mdr(r)
    ...
    void print(ostream& out) const override { out << "fragment_purge_old(" << basedirfrag << ")"; }
};
...
void MDCache::fragment_frozen(MDRequestRef& mdr, int r)
{
    ...
    ceph_assert(r == 0);
    fragment_info_t& info = it->second;
    dout(10) << "fragment_frozen " << basedirfrag.frag << " by " << info.bits << " on "
             << info.dirs.front()->get_inode() << dendl;
...
void MDCache::dispatch_fragment_dir(MDRequestRef& mdr)
{
    ...
    if (mdr->more()->peer_error) mdr->aborted = true;
    ...
    // refragment
    MDSContext::vec waiters;
    adjust_dir_fragments(diri, info.dirs, basedirfrag.frag, info.bits, &info.resultfrags, waiters, false);
    if (g_conf()->mds_debug_frag) diri->verify_dirfrags();
    mds->queue_waiters(waiters);

    for (const auto& fg : le->orig_frags) ceph_assert(!diri->dirfragtree.is_leaf(fg));
    ...
    */
    ...
    mds->server->submit_mdlog_entry(le, new C_MDC_FragmentPrep(this, mdr), mdr, __func__);
    mds->mdlog->flush();
}
...
void MDCache::_fragment_stored(MDRequestRef& mdr)
{
    ...
    // tell peers
    mds_rank_t diri_auth = (first->is_subtree_root() && !diri->is_auth()) ? diri->authority().first : CDIR_AUTH_UNKNOWN;
...
void MDCache::_fragment_committed(dirfrag_t basedirfrag, const MDRequestRef& mdr)
{
    ...
    if (mdr) mdr->mark_event("commit logged");
    ...
    C_GatherBuilder gather(
        g_ceph_context,
        new C_OnFinisher(new C_IO_MDC_FragmentPurgeOld(this, basedirfrag, uf.bits, mdr), mds->finisher));
    ...
    for (const auto& fg : uf.old_frags) {
        ...
        mds->objecter->mutate(oid, oloc, op, nullsnapc, ceph::real_clock::now(), 0, gather.new_sub());
    }
...
void MDCache::_fragment_old_purged(dirfrag_t basedirfrag, int bits, const MDRequestRef& mdr)
{
    ...
    if (mdr) mdr->mark_event("old frags purged");
...
void MDCache::handle_fragment_notify_ack(const cref_t<MMDSFragmentNotifyAck>& ack)
{
    ...
    if (it == fragments.end() || it->second.get_tid() != ack->get_tid()) {
        dout(10) << "handle_fragment_notify_ack obsolete message, dropping" << dendl;
        return;
    }

    if (it->second.notify_ack_waiting.erase(from) && it->second.notify_ack_waiting.empty()) {
...
void MDCache::handle_fragment_notify(const cref_t<MMDSFragmentNotify>& notify)
{
    ...
    if (notify->is_ack_wanted()) {
        auto ack =
            make_message<MMDSFragmentNotifyAck>(notify->get_base_dirfrag(), notify->get_bits(), notify->get_tid());
        mds->send_message_mds(ack, from);
    }
}

void MDCache::add_uncommitted_fragment(dirfrag_t basedirfrag, int bits, const frag_vec_t& old_frags, LogSegment* ls,
                                       bufferlist* rollback)
{
    ...
    if (rollback) uf.rollback.swap(*rollback);
}

void MDCache::finish_uncommitted_fragment(dirfrag_t basedirfrag, int op)
{
    dout(10) << "finish_uncommitted_fragments: base dirfrag " << basedirfrag << " op " << EFragment::op_name(op)
             << dendl;
...
{
    dout(10) << "rollback_uncommitted_fragment: base dirfrag " << basedirfrag << " old_frags (" << old_frags << ")"
             << dendl;
    map<dirfrag_t, ufragment>::iterator it = uncommitted_fragments.find(basedirfrag);
    if (it != uncommitted_fragments.end()) {
        ufragment& uf = it->second;
...
void MDCache::rollback_uncommitted_fragments()
{
    ...
    for (map<dirfrag_t, ufragment>::iterator p = uncommitted_fragments.begin(); p != uncommitted_fragments.end(); ++p) {
...
void MDCache::force_readonly()
{
    ...
    for (auto& p : inode_map) {
        CInode* in = p.second;
        if (in->is_head()) mds->locker->eval(in, CEPH_CAP_LOCKS);
        if (!(++count % mds->heartbeat_reset_grace())) mds->heartbeat_reset();
    }
...
{
    if (g_conf()->mds_thrash_exports) dbl += 15;

    // dout(10) << "show_subtrees" << dendl;

    if (!g_conf()->subsys.should_gather(ceph_subsys_mds, dbl)) return;   // i won't print anything.
    ...
    if (!force_print && subtrees.size() > SUBTREES_COUNT_THRESHOLD &&
        ...
        dout(ceph::dout::need_dynamic(dbl)) << "number of subtrees = " << subtrees.size()
                                            << "; not "
                                               "printing subtrees"
                                            << dendl;
        return;
    ...
    for (set<CInode*>::iterator p = base_inodes.begin(); p != base_inodes.end(); ++p) (*p)->get_dirfrags(basefrags);
    ...
    while (!q.empty()) {
        ...
        if (!subtrees[dir].empty()) {
            for (set<CDir*>::iterator p = subtrees[dir].begin(); p != subtrees[dir].end(); ++p) {
    ...
    if (!force_print && depth > SUBTREES_DEPTH_THRESHOLD && !g_conf()->subsys.should_gather<ceph_subsys_mds, 25>()) {
        dout(ceph::dout::need_dynamic(dbl)) << "max depth among subtrees = " << depth
                                            << "; not printing "
                                               "subtrees"
                                            << dendl;
        return;
    ...
    while (!q.empty()) {
        ...
        if (dir->ino() == CEPH_INO_ROOT) ceph_assert(dir->inode == root);
        if (dir->ino() == MDS_INO_MDSDIR(mds->get_nodeid())) ceph_assert(dir->inode == myin);
...
{
    if (!g_conf()->subsys.should_gather<ceph_subsys_mds, 7>()) return;
    dout(7) << "show_cache" << dendl;

    auto show_func = [this](CInode* in) {
        // unlinked?
        if (!in->parent) dout(7) << " unlinked " << *in << dendl;
        ...
        for (const auto& dir : dfs) {
            ...
            for (auto& p : dir->items) {
                ...
                CDentry::linkage_t* dnl = dn->get_linkage();
                if (dnl->is_primary() && dnl->get_inode()) dout(7) << "    inode " << *dnl->get_inode() << dendl;
            }
        }
    };

    for (auto& p : inode_map) show_func(p.second);
    for (auto& p : snap_inode_map) show_func(p.second);
}
...
int MDCache::dump_cache(std::string_view fn, Formatter* f, double timeout)
{
    ...
    uint64_t threshold = f ? g_conf().get_val<Option::size_t>("mds_dump_cache_threshold_formatter")
...
public:
    ...
    void finish(int r) override
    {
        ...
        r = 0;
        if (on_finish) on_finish->complete(r);
    }
};

void MDCache::enqueue_scrub(std::string_view path, std::string_view tag, bool force, bool recursive, bool repair,
                            bool scrub_mdsdir, Formatter* f, Context* fin)
{
    ...
    if (path.compare(0, 4, "~mds") == 0) {
        ...
        if (path == "~mdsdir") {
            ...
            if (!err.empty()) rank = MDS_RANK_NONE;
        }
        if (rank >= 0 && rank < MAX_MDS) fp.set_path("", MDS_INO_MDSDIR(rank));
    }
    if (fp.get_ino() == inodeno_t(0)) fp.set_path(path);
    ...
    cs->header = std::make_shared<ScrubHeader>(tag_str, is_internal, force, recursive, repair, scrub_mdsdir);
...
void MDCache::enqueue_scrub_work(MDRequestRef& mdr)
{
    ...
    int r = path_traverse(mdr, cf, mdr->get_filepath(), MDS_TRAVERSE_DISCOVER | MDS_TRAVERSE_RDLOCK_PATH, nullptr, &in);
    if (r > 0) return;
    if (r < 0) {
...
public:
    ...
    void finish(int r) override { header->dec_num_pending(); }
};
...
void MDCache::repair_dirfrag_stats_work(MDRequestRef& mdr)
{
    ...
    if (!mdr->is_auth_pinned(dir) && !dir->can_auth_pin()) {
        ...
        mds->locker->drop_locks(mdr.get());
        mdr->drop_local_auth_pins();
        if (mdr->is_any_remote_auth_pin()) mds->locker->notify_freeze_waiter(dir);
        return;
    }
    ...
    for (auto it = dir->begin(); it != dir->end(); ++it) {
        CDentry* dn = it->second;
        if (dn->last != CEPH_NOSNAP) continue;
        CDentry::linkage_t* dnl = dn->get_projected_linkage();
        if (dnl->is_primary()) {
            CInode* in = dnl->get_inode();
    ...
    if (!good_fragstat) {
        if (pf->fragstat.mtime > frag_info.mtime) frag_info.mtime = pf->fragstat.mtime;
        if (pf->fragstat.change_attr > frag_info.change_attr) frag_info.change_attr = pf->fragstat.change_attr;
    ...
    if (!good_rstat) {
        if (pf->rstat.rctime > nest_info.rctime) nest_info.rctime = pf->rstat.rctime;
        _pf->rstat = nest_info;
        mds->locker->mark_updated_scatterlock(&diri->nestlock);
...
do_rdlocks:
    ...
    if (const sr_t* srnode = diri->get_projected_srnode(); srnode) nest_info.rsnaps = srnode->snaps.size();
    ...
    if (!dir_info.same_sums(diri->get_inode()->dirstat) || !nest_info.same_sums(diri->get_inode()->rstat)) {
...
public:
    C_FinishIOMDR(MDSRank* mds_, MDRequestRef& mdr_)
        : mds(mds_)
        , mdr(mdr_)
    {}
...
void MDCache::register_perfcounters()
{
    ...
    pcb.add_u64_counter(l_mdc_dir_update, "dir_update", "Directory replication directives");
    pcb.add_u64_counter(l_mdc_dir_update_receipt, "dir_update_receipt", "Directory replication directives received");
    pcb.add_u64_counter(l_mdc_dir_try_discover, "dir_try_discover", "Directory replication attempt to discover");
    pcb.add_u64_counter(l_mdc_dir_send_discover, "dir_send_discover", "Directory replication discovery message sent");
    pcb.add_u64_counter(
        l_mdc_dir_handle_discover, "dir_handle_discover", "Directory replication discovery message handled");

    // Stray/purge statistics
    pcb.add_u64(l_mdc_num_strays, "num_strays", "Stray dentries", "stry", PerfCountersBuilder::PRIO_INTERESTING);
    ...
    pcb.add_u64(l_mdc_num_recovering_prioritized,
    ...
    pcb.add_u64_counter(l_mdc_strays_enqueued, "strays_enqueued", "Stray dentries enqueued for purge");
    ...
    pcb.add_u64_counter(l_mdss_ireq_enqueue_scrub, "ireq_enqueue_scrub", "Internal Request type enqueue scrub");
    pcb.add_u64_counter(l_mdss_ireq_exportdir, "ireq_exportdir", "Internal Request type export dir");
    pcb.add_u64_counter(l_mdss_ireq_flush, "ireq_flush", "Internal Request type flush");
    pcb.add_u64_counter(l_mdss_ireq_fragmentdir, "ireq_fragmentdir", "Internal Request type fragmentdir");
    pcb.add_u64_counter(l_mdss_ireq_fragstats, "ireq_fragstats", "Internal Request type frag stats");
    pcb.add_u64_counter(l_mdss_ireq_inodestats, "ireq_inodestats", "Internal Request type inode stats");
...
{
    if (in->get_inode()->nlink > 0 || in->is_base() || is_readonly() || mds->get_state() <= MDSMap::STATE_REJOIN)
        return;
...
void MDCache::clear_dirty_bits_for_stray(CInode* diri)
{
    ...
    auto&& ls = diri->get_dirfrags();
    for (auto& p : ls) {
        if (p->is_auth() && !(p->is_frozen() || p->is_freezing())) p->try_remove_dentries_for_stray();
    }
    if (!diri->snaprealm) {
        if (diri->is_auth()) diri->clear_dirty_rstat();
...
void MDCache::upkeep_main(void)
{
    ...
    while (!upkeep_trim_shutdown.load()) {
        ...
        if (since >= trim_interval * .90) {
            ...
            lock.lock();
            if (upkeep_trim_shutdown.load()) return;
            check_memory_usage();
        ...
        auto release_interval =
            clock::duration(g_conf().get_val<std::chrono::seconds>("mds_cache_release_free_interval"));
