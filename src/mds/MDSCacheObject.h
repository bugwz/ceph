...
{
    explicit mdsco_db_line_prefix(MDSCacheObject* o)
        : object(o)
    {}
...
public:
    ...
    struct ptr_lt
    {
        bool operator()(const MDSCacheObject* l, const MDSCacheObject* r) const { return l->is_lt(r); }
    };
    ...
    virtual bool is_ambiguous_auth() const { return authority().second != CDIR_AUTH_UNKNOWN; }
...
#endif
            if (ref == 0) last_put();
            if (state_test(STATE_NOTIFYREF)) _put();
        }
    }
    ...
    void get(int by)
    {
        ...
        ref++;
#ifdef MDS_REF_SET
        if (ref_map.find(by) == ref_map.end()) ref_map[by] = 0;
        ref_map[by]++;
#endif
    }
    ...
    void print_authpin_set(std::ostream& out) const { out << " (" << auth_pin_set << ")"; }
#endif
    ...
    virtual bool is_freezing_or_frozen() const { return is_frozen() || is_freezing(); }
    ...
    {
        if (get_replicas().count(mds)) return ++get_replicas()[mds];   // inc nonce
        if (get_replicas().empty()) get(PIN_REPLICATED);
    ...
    {
        if (!get_replicas().empty()) put(PIN_REPLICATED);
        replica_map.clear();
    }
    ...
    {
        if (waiting.empty()) get(PIN_WAITER);
        ...
        waiting.insert(
            std::pair<uint64_t, std::pair<uint64_t, MDSContext*>>(mask, std::pair<uint64_t, MDSContext*>(seq, c)));
