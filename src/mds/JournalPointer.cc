...
int JournalPointer::load(Objecter* objecter)
{
    ...
    bufferlist data;
    C_SaferCond waiter;
    objecter->read_full(object_t(object_id), object_locator_t(pool_id), CEPH_NOSNAP, &data, 0, &waiter);
    int r = waiter.wait();
...
int JournalPointer::save(Objecter* objecter) const
{
    ...
    std::string const object_id = get_object_id();
    dout(4) << "Writing pointer object '" << object_id << "': 0x" << std::hex << front << ":0x" << back << std::dec
            << dendl;

    C_SaferCond waiter;
    objecter->write_full(
        object_t(object_id), object_locator_t(pool_id), SnapContext(), data, ceph::real_clock::now(), 0, &waiter);
...
void JournalPointer::save(Objecter* objecter, Context* completion) const
{
    ...
    objecter->write_full(object_t(get_object_id()),
                         object_locator_t(pool_id),
                         SnapContext(),
                         data,
                         ceph::real_clock::now(),
                         0,
