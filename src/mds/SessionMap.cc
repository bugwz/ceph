...
public:
    explicit SessionMapIOContext(SessionMap* sessionmap_)
...
{
    PerfCountersBuilder plb(g_ceph_context, "mds_sessions", l_mdssm_first, l_mdssm_last);

    plb.add_u64(l_mdssm_session_count, "session_count", "Session count", "sess", PerfCountersBuilder::PRIO_INTERESTING);

    plb.set_prio_default(PerfCountersBuilder::PRIO_USEFUL);
    plb.add_u64_counter(l_mdssm_session_add, "session_add", "Sessions added");
    plb.add_u64_counter(l_mdssm_session_remove, "session_remove", "Sessions removed");
    plb.add_u64(l_mdssm_session_open, "sessions_open", "Sessions currently open");
    plb.add_u64(l_mdssm_session_stale, "sessions_stale", "Sessions currently stale");
    plb.add_u64(l_mdssm_total_load, "total_load", "Total Load");
    plb.add_u64(l_mdssm_avg_load, "average_load", "Average Load");
    plb.add_u64(l_mdssm_avg_session_uptime, "avg_session_uptime", "Average session uptime");
    plb.add_u64(l_mdssm_metadata_threshold_sessions_evicted,
                "mdthresh_evicted",
...
{
    dout(10) << "dump" << dendl;
    for (ceph::unordered_map<entity_name_t, Session*>::iterator p = session_map.begin(); p != session_map.end(); ++p)
        dout(10) << p->first << " " << p->second << " state " << p->second->get_state_name() << " completed "
                 << p->second->info.completed_requests << " free_prealloc_inos " << p->second->free_prealloc_inos
                 << " delegated_inos " << p->second->delegated_inos << dendl;
}
...
public:
    ...
    {
        sessionmap->_load_finish(r, header_r, values_r, first, header_bl, session_vals, more_session_vals);
    }
    void print(ostream& out) const override { out << "session_load"; }
};
}   // namespace
 ...
 */
void SessionMapStore::decode_header(bufferlist& header_bl)
{
...
void SessionMapStore::encode_header(bufferlist* header_bl)
{
...
{
    for (std::map<std::string, bufferlist>::iterator i = session_vals.begin(); i != session_vals.end(); ++i) {
 ...
 */
void SessionMap::_load_finish(int operation_r, int header_r, int values_r, bool first, bufferlist& header_bl,
                              std::map<std::string, bufferlist>& session_vals, bool more_session_vals)
{
    if (operation_r < 0) {
        derr << "_load_finish got " << cpp_strerror(operation_r) << dendl;
        mds->clog->error() << "error reading sessionmap '" << get_object_name() << "' " << operation_r << " ("
    ...
    if (first) {
        if (header_r != 0) {
            ...
            mds->clog->error() << "error reading sessionmap header " << header_r << " (" << cpp_strerror(header_r)
                               << ")";
            mds->damaged();
            ceph_abort();   // Should be unreachable because damaged() calls respawn()
        }
    ...
    if (values_r != 0) {
        ...
        mds->clog->error() << "error reading sessionmap values: " << values_r << " (" << cpp_strerror(values_r) << ")";
        mds->damaged();
        ceph_abort();   // Should be unreachable because damaged() calls respawn()
    }
    ...
    if (more_session_vals) {
        ...
        ObjectOperation op;
        op.omap_get_vals(
            last_key, "", g_conf()->mds_sessionmap_keys_per_op, &c->session_vals, &c->more_session_vals, &c->values_r);
        ...
        for (ceph::unordered_map<entity_name_t, Session*>::iterator i = session_map.begin(); i != session_map.end();
             ++i) {
            Session* s = i->second;
            auto by_state_entry = by_state.find(s->get_state());
            if (by_state_entry == by_state.end())
                by_state_entry = by_state.emplace(s->get_state(), new xlist<Session*>).first;
...
void SessionMap::load(MDSContext* onload)
{
    ...
    if (onload) waiting_for_load.push_back(onload);
    ...
    op.omap_get_vals(
        "", "", g_conf()->mds_sessionmap_keys_per_op, &c->session_vals, &c->more_session_vals, &c->values_r);
...
public:
    bufferlist bl;
    explicit C_IO_SM_LoadLegacy(SessionMap* cm)
        : SessionMapIOContext(cm)
    {}
    void finish(int r) override { sessionmap->_load_legacy_finish(r, bl); }
    void print(ostream& out) const override { out << "session_load_legacy"; }
};
}   // namespace
...
void SessionMap::load_legacy()
{
    ...
    mds->objecter->read_full(oid, oloc, CEPH_NOSNAP, &c->bl, 0, new C_OnFinisher(c, mds->finisher));
}
...
void SessionMap::_load_legacy_finish(int r, bufferlist& bl)
{
    ...
    dout(10) << "_load_finish v " << version << ", " << session_map.size() << " sessions, " << bl.length() << " bytes"
             << dendl;
    projected = committing = committed = version;
    dump();
    ...
    for (ceph::unordered_map<entity_name_t, Session*>::iterator i = session_map.begin(); i != session_map.end(); ++i) {
...
public:
    C_IO_SM_Save(SessionMap* cm, version_t v)
        : SessionMapIOContext(cm)
        , version(v)
    {}
    void finish(int r) override
    ...
    void print(ostream& out) const override { out << "session_save"; }
};
}   // namespace
...
void SessionMap::save(MDSContext* onsave, version_t needv)
{
    ...
    for (std::set<entity_name_t>::iterator i = dirty_sessions.begin(); i != dirty_sessions.end(); ++i) {
        const entity_name_t name = *i;
        Session* session = session_map[name];

        if (session->is_open() || session->is_closing() || session->is_stale() || session->is_killing()) {
            dout(20) << "  " << name << dendl;
    ...
    for (std::set<entity_name_t>::const_iterator i = null_sessions.begin(); i != null_sessions.end(); ++i) {
        dout(20) << "  " << *i << dendl;
        CachedStackStringStream css;
        *css << *i;
    ...
    mds->objecter->mutate(oid,
                          oloc,
                          op,
                          snapc,
                          ceph::real_clock::now(),
                          0,
                          new C_OnFinisher(new C_IO_SM_Save(this, version), mds->finisher));
...
uint64_t SessionMap::set_state(Session* session, int s)
{
    if (session->state != s) {
        ...
        if (by_state_entry == by_state.end()) by_state_entry = by_state.emplace(s, new xlist<Session*>).first;
        ...
        // couters associated
        logger->set(l_mdssm_session_open, get_session_count_in_state(Session::STATE_OPEN));
        logger->set(l_mdssm_session_stale, get_session_count_in_state(Session::STATE_STALE));
    }
...
void SessionMap::add_session(Session* s)
{
    ...
    if (by_state_entry == by_state.end()) by_state_entry = by_state.emplace(s->state, new xlist<Session*>).first;
    by_state_entry->second->push_back(&s->item_session_list);
    s->get();
...
void SessionMap::touch_session(Session* session)
{
    ...
    if (by_state_entry == by_state.end()) by_state_entry = by_state.emplace(session->state, new xlist<Session*>).first;
    by_state_entry->second->push_back(&session->item_session_list);
...
{
    if (dirty_sessions.count(s->info.inst.name)) return;

    if (may_save && dirty_sessions.size() >= g_conf()->mds_sessionmap_keys_per_op) {
...
void SessionMap::replay_open_sessions(version_t event_cmapv, map<client_t, entity_inst_t>& client_map,
                                      map<client_t, client_metadata_t>& client_metadata_map)
{
    unsigned already_saved;

    if (version + client_map.size() < event_cmapv) goto bad;
    ...
    for (const auto& p : client_map) {
        ...
        if (q != client_metadata_map.end()) s->info.client_metadata.merge(q->second);

        if (already_saved > 0) {
            if (s->is_closed()) goto bad;
...
bad:
    mds->clog->error() << "error replaying open sessions(" << client_map.size() << ") sessionmap v " << event_cmapv
                       << " table " << version;
...
{
    dout(20) << __func__ << " s=" << s << " name=" << s->info.inst.name << " pv=" << projected << " -> "
             << projected + 1 << dendl;
    ++projected;
    s->push_pv(projected);
    return projected;
...
public:
    ...
    void print(ostream& out) const override { out << "session_save_one"; }
};
}   // namespace


void SessionMap::save_if_dirty(const std::set<entity_name_t>& tgt_sessions, MDSGatherBuilder* gather_bld)
{
    ...
    for (std::set<entity_name_t>::iterator i = tgt_sessions.begin(); i != tgt_sessions.end(); ++i) {
    ...
    for (auto& [session_id, bl] : write_sessions) {
        ...
        *css << session_id;
        ...
        if (i == write_sessions.size() - 1 || i % kpo == kpo - 1) {
            ObjectOperation op;
            op.omap_set(to_set);
...
{
    size_t result = 0;
    for (auto p = requests.begin(); !p.end(); ++p) ++result;
    return result;
}
...
int Session::check_access(CInode* in, unsigned mask, int caller_uid, int caller_gid,
                          const vector<uint64_t>* caller_gid_list, int new_uid, int new_gid)
{
    string path;
    CInode* diri = NULL;
    if (!in->is_base()) diri = in->get_projected_parent_dn()->get_dir()->get_inode();
...
void SessionMap::handle_conf_change(const std::set<std::string>& changed)
{
    ...
    if (changed.count("mds_request_load_average_decay_rate")) {
        ...
        auto mut = [d](auto s) { s->set_load_avg_decay_rate(d); };
    ...
    if (changed.count("mds_recall_max_decay_rate")) {
        ...
        auto mut = [d](auto s) { s->recall_caps_throttle = DecayCounter(d); };
        apply_to_open_sessions(mut);
    }
    ...
    if (changed.count("mds_session_cache_liveness_decay_rate")) {
        ...
        auto mut = [d](auto s) {
            ...
            s->session_cache_liveness.hit(
                s->caps.size()); /* so the MDS doesn't immediately start trimming a new session */
        };
        apply_to_open_sessions(mut);
    }
    if (changed.count("mds_session_cap_acquisition_decay_rate")) {
        ...
        auto mut = [d](auto s) { s->cap_acquisition = DecayCounter(d); };
        apply_to_open_sessions(mut);
    }
...
void SessionMap::apply_blocklist(const std::set<entity_name_t>& victims)
{
    ...
    for (auto& victim : victims) {
        CachedStackStringStream css;
        mds->evict_client(victim.num(), false, g_conf()->mds_session_blocklist_on_evict, *css, gather.new_sub());
    }
    gather.activate();
}

int SessionFilter::parse(const std::vector<std::string>& args, std::ostream* ss)
{
    ceph_assert(ss != NULL);
    ...
    for (const auto& s : args) {
        ...
        if (k.compare(0, metadata_prefix.size(), metadata_prefix) == 0 && k.size() > metadata_prefix.size()) {
...
bool SessionFilter::match(const Session& session, std::function<bool(client_t)> is_reconnecting) const
{
