...
void CInodeCommitOperation::update(ObjectOperation& op, inode_backtrace_t& bt)
{
    ...
    if (!update_layout_symlink) return;
...
public:
    explicit CInodeIOContext(CInode* in_)
        : in(in_)
    {
        ceph_assert(in != NULL);
    }
...
ostream& operator<<(ostream& out, const CInode& in)
{
    ...
    out << "[inode " << in.ino();
    out << " [" << (in.is_multiversion() ? "..." : "") << in.first << "," << in.last << "]";
    out << " " << path << (in.is_dir() ? "/" : "");

    if (in.is_auth()) {
        out << " auth";
        if (in.is_replicated()) out << in.get_replicas();
    }
    else {
        mds_authority_t a = in.authority();
        out << " rep@" << a.first;
        if (a.second != CDIR_AUTH_UNKNOWN) out << "," << a.second;
    ...
    if (in.get_projected_version() > in.get_version()) out << " pv" << in.get_projected_version();
    ...
    if (in.snaprealm) out << " snaprealm=" << in.snaprealm;
    ...
    if (pi->is_truncating()) out << " truncating(" << pi->truncate_from << " to " << pi->truncate_size << ")";
    ...
    if (in.is_dir()) {
        ...
        if (in.get_inode()->nlink != 1) out << " nl=" << in.get_inode()->nlink;
    }

    // rstat
    out << " " << in.get_inode()->rstat;
    if (!(in.get_inode()->rstat == in.get_inode()->accounted_rstat)) out << "/" << in.get_inode()->accounted_rstat;
    if (g_conf()->mds_debug_scatterstat && in.is_projected()) {
        out << "->" << pi->rstat;
        if (!(pi->rstat == pi->accounted_rstat)) out << "/" << pi->accounted_rstat;
    }
    ...
    if (!in.client_need_snapflush.empty()) out << " need_snapflush=" << in.client_need_snapflush;

    // locks
    if (!in.authlock.is_sync_and_unlocked()) out << " " << in.authlock;
    if (!in.linklock.is_sync_and_unlocked()) out << " " << in.linklock;
    if (in.get_inode()->is_dir()) {
        if (!in.dirfragtreelock.is_sync_and_unlocked()) out << " " << in.dirfragtreelock;
        if (!in.snaplock.is_sync_and_unlocked()) out << " " << in.snaplock;
        if (!in.nestlock.is_sync_and_unlocked()) out << " " << in.nestlock;
        if (!in.policylock.is_sync_and_unlocked()) out << " " << in.policylock;
    }
    else {
        if (!in.flocklock.is_sync_and_unlocked()) out << " " << in.flocklock;
    }
    if (!in.filelock.is_sync_and_unlocked()) out << " " << in.filelock;
    if (!in.xattrlock.is_sync_and_unlocked()) out << " " << in.xattrlock;
    if (!in.versionlock.is_sync_and_unlocked()) out << " " << in.versionlock;

    // hack: spit out crap on which clients have caps
    if (in.get_inode()->client_ranges.size()) out << " cr=" << in.get_inode()->client_ranges;
    ...
    if (!in.get_client_caps().empty()) {
        ...
        for (const auto& p : in.get_client_caps()) {
            if (!first) out << ",";
            out << p.first << "=" << ccap_string(p.second.pending());
            if (p.second.issued() != p.second.pending()) out << "/" << ccap_string(p.second.issued());
            out << "/" << ccap_string(p.second.wanted()) << "@" << p.second.get_last_seq();
            first = false;
        }
        out << "}";
        if (in.get_loner() >= 0 || in.get_wanted_loner() >= 0) {
            out << ",l=" << in.get_loner();
            if (in.get_loner() != in.get_wanted_loner()) out << "(" << in.get_wanted_loner() << ")";
        }
    }
...
void CInode::add_need_snapflush(CInode* snapin, snapid_t snapid, client_t client)
{
    ...
    if (clients.empty()) snapin->auth_pin(this);   // ...and pin snapped/old inode!
...
pair<bool, bool> CInode::split_need_snapflush(CInode* cowin, CInode* in)
{
    ...
    if (it != client_need_snapflush.end() && it->first <= in->last) orig_need_flush = true;
...
CInode::projected_inode CInode::project_inode(const MutationRef& mut, bool xattr, bool snap)
{
    ...
    if (mut) mut->add_projected_node(this);
...
void CInode::pop_and_dirty_projected_inode(LogSegment* ls, const MutationRef& mut)
{
    ...
    if (mut) mut->remove_projected_node(this);
    ...
    bool pin_updated = (get_inode()->export_pin != front.inode->export_pin) ||
                       (get_inode()->export_ephemeral_distributed_pin != front.inode->export_ephemeral_distributed_pin);

    reset_inode(std::move(front.inode));
    if (front.xattrs != get_xattrs()) reset_xattrs(std::move(front.xattrs));
    ...
    mark_dirty(ls);
    if (get_inode()->is_backtrace_updated()) mark_dirty_parent(ls, pool_updated);

    if (pin_updated) maybe_export_pin(true);
}
...
const sr_t* CInode::get_projected_srnode() const
{
    if (num_projected_srnodes > 0) {
        for (auto it = projected_nodes.rbegin(); it != projected_nodes.rend(); ++it)
            if (it->snapnode != projected_inode::UNDEF_SRNODE) return it->snapnode;
    }
    if (snaprealm)
...
bool CInode::is_projected_snaprealm_global() const
{
    ...
    if (srnode && srnode->is_parent_global()) return true;
    return false;
}
...
void CInode::record_snaprealm_past_parent(sr_t* new_snap, SnapRealm* newparent)
{
    ...
    if (newparent != oldparent) {
        ...
        if (oldparentseq + 1 > new_snap->current_parent_since) {
            ...
            if (p != snaps.end()) new_snap->past_parent_snaps.insert(p, snaps.end());
            if (oldparentseq > new_snap->seq) new_snap->seq = oldparentseq;
        }
        new_snap->current_parent_since = mdcache->get_global_snaprealm()->get_newest_seq() + 1;
    }
}

void CInode::record_snaprealm_parent_dentry(sr_t* new_snap, SnapRealm* oldparent, CDentry* dn, bool primary_dn)
{
    ceph_assert(new_snap->is_parent_global());

    if (!oldparent) oldparent = dn->get_dir()->inode->find_snaprealm();
...
{
    if (next_snaprealm) {
        dout(10) << __func__ << (early ? " (early) " : " ") << next_snaprealm << " seq " << next_snaprealm->seq
                 << dendl;
        if (!snaprealm) open_snaprealm();
        ...
        if (snaprealm->parent) dout(10) << " realm " << *snaprealm << " parent " << *snaprealm->parent << dendl;
    }
    else {
...
__u32 InodeStoreBase::hash_dentry_name(std::string_view dn)
{
    ...
    if (!which) which = CEPH_STR_HASH_LINUX;
...
{
    if (dirfragtree.empty()) return frag_t();   // avoid the string hash if we can.
...
std::pair<bool, std::vector<CDir*>> CInode::get_dirfrags_under(frag_t fg)
{
    ...
    // we convert all the frags into the frags of 24th layer to calculate whether all the frags are included in the
    // memory cache
...
void CInode::verify_dirfrags()
{
    ...
    for (const auto& p : dirfrags) {
        if (!dirfragtree.is_leaf(p.first)) {
            dout(0) << "have open dirfrag " << p.first << " but not leaf in " << dirfragtree << ": " << *p.second
                    << dendl;
            bad = true;
        }
    }
...
CDir* CInode::get_approx_dirfrag(frag_t fg)
{
    ...
    if (!p.second.empty()) return p.second.front();
...
{
    auto em = dirfrags.emplace(
        std::piecewise_construct, std::forward_as_tuple(dir->dirfrag().frag), std::forward_as_tuple(dir));
...
void CInode::close_dirfrag(frag_t fg)
{
    ...
    if (dir->is_subtree_root()) num_subtree_roots--;

    // dump any remaining dentries, for debugging purposes
    for (const auto& p : dir->items) dout(14) << __func__ << " LEFTOVER dn " << *p.second << dendl;
...
bool CInode::has_subtree_root_dirfrag(int auth)
{
    if (num_subtree_roots > 0) {
        ...
        for (const auto& p : dirfrags) {
            if (p.second->is_subtree_root() && p.second->dir_auth.first == auth) return true;
        }
    }
    return false;
}
...
{
    if (num_subtree_roots > 0 || num_exporting_dirs > 0) return true;
    return false;
}
...
{
    // pin my dentry?
    if (parent) parent->get(CDentry::PIN_INODEPIN);
}

void CInode::last_put()
{
    // unpin my dentry?
    if (parent) parent->put(CDentry::PIN_INODEPIN);
}

void CInode::_put()
{
    if (get_num_ref() == (int)is_dirty() + (int)is_dirty_parent()) mdcache->maybe_eval_stray(this, true);
}

void CInode::add_remote_parent(CDentry* p)
{
    if (remote_parents.empty()) get(PIN_REMOTEPARENT);
    remote_parents.insert(p);
}
void CInode::remove_remote_parent(CDentry* p)
{
    remote_parents.erase(p);
    if (remote_parents.empty()) put(PIN_REMOTEPARENT);
}
...
{
    if (parent) return parent->dir->inode;
    return NULL;
}
...
version_t CInode::pre_dirty()
{
    ...
    if (get_inode()->backtrace_version == 0 && !projected_nodes.empty()) {
        ...
        if (pi->backtrace_version == 0) pi->update_backtrace(pv);
    }
    return pv;
}
...
void CInode::_mark_dirty(LogSegment* ls)
{
    ...
    if (ls) ls->dirty_inodes.push_back(&item_dirty);
}
...
void CInode::mark_dirty(LogSegment* ls)
{
    ...
    */
    ...
    // mark dentry too
    if (parent) parent->mark_dirty(get_version(), ls);
}
...
struct C_IO_Inode_Stored : public CInodeIOContext
{
    version_t version;
    Context* fin;
    C_IO_Inode_Stored(CInode* i, version_t v, Context* f)
        : CInodeIOContext(i)
        , version(v)
        , fin(f)
    {}
    void finish(int r) override { in->_stored(r, version, fin); }
    void print(ostream& out) const override { out << "inode_store(" << in->ino() << ")"; }
};
...
void CInode::store(MDSContext* fin)
{
    ...
    if (snaprealm) purge_stale_snap_data(snaprealm->get_snaps());
    ...
    Context* newfin = new C_OnFinisher(new C_IO_Inode_Stored(this, get_version(), fin), mdcache->mds->finisher);
    mdcache->mds->objecter->mutate(oid, oloc, m, snapc, ceph::real_clock::now(), 0, newfin);
}
...
void CInode::_stored(int r, version_t v, Context* fin)
{
    if (r < 0) {
        ...
        mdcache->mds->clog->error() << "failed to store inode " << ino() << " object: " << cpp_strerror(r);
    ...
    if (v == get_projected_version()) mark_clean();
...
struct C_IO_Inode_Fetched : public CInodeIOContext
{
    bufferlist bl, bl2;
    Context* fin;
    C_IO_Inode_Fetched(CInode* i, Context* f)
        : CInodeIOContext(i)
        , fin(f)
    {}
    ...
    void print(ostream& out) const override { out << "inode_fetch(" << in->ino() << ")"; }
};
...
void CInode::build_backtrace(int64_t pool, inode_backtrace_t& bt)
{
    ...
    for (auto& p : get_inode()->old_pools) {
        ...
        if (p != pool) bt.old_pools.push_back(p);
    }
}
...
struct C_IO_Inode_StoredBacktrace : public CInodeIOContext
{
    version_t version;
    Context* fin;
    C_IO_Inode_StoredBacktrace(CInode* i, version_t v, Context* f)
        : CInodeIOContext(i)
        , version(v)
        , fin(f)
    {}
    void finish(int r) override { in->_stored_backtrace(r, version, fin); }
    void print(ostream& out) const override { out << "backtrace_store(" << in->ino() << ")"; }
};


void CInode::_commit_ops(int r, C_GatherBuilder& gather_bld, std::vector<CInodeCommitOperation>& ops_vec,
                         inode_backtrace_t& bt)
{
    ...
    for (auto& op : ops_vec) {
        ...
        op.update(obj_op, bt);
        mdcache->mds->objecter->mutate(oid, oloc, obj_op, snapc, ceph::real_clock::now(), 0, gather_bld.new_sub());
    }
}

void CInode::_store_backtrace(std::vector<CInodeCommitOperation>& ops_vec, inode_backtrace_t& bt, int op_prio)
{
    dout(10) << __func__ << " on " << *this << dendl;
    ceph_assert(is_dirty_parent());

    if (op_prio < 0) op_prio = CEPH_MSG_PRIO_DEFAULT;
    ...
    ops_vec.emplace_back(op_prio, pool, get_inode()->layout, mdcache->mds->mdsmap->get_up_features(), slink);
...
void CInode::store_backtrace(MDSContext* fin, int op_prio)
{
    ...
    C_GatherBuilder gather(
        g_ceph_context, new C_OnFinisher(new C_IO_Inode_StoredBacktrace(this, version, fin), mdcache->mds->finisher));
...
void CInode::_stored_backtrace(int r, version_t v, Context* fin)
{
    if (r == -CEPHFS_ENOENT) {
        const int64_t pool = get_backtrace_pool();
        bool exists =
            mdcache->mds->objecter->with_osdmap([pool](const OSDMap& osd_map) { return osd_map.have_pg_pool(pool); });
    ...
    if (r < 0) {
        ...
        mdcache->mds->clog->error() << "failed to store backtrace on ino " << ino() << " object"
                                    << ", pool " << get_backtrace_pool() << ", errno " << r;
        mdcache->mds->handle_write_error(r);
        if (fin) fin->complete(r);
        return;
    }
    ...
    auth_unpin(this);
    if (v == get_inode()->backtrace_version) clear_dirty_parent();
    if (fin) fin->complete(0);
}
...
void CInode::mark_dirty_parent(LogSegment* ls, bool dirty_pool)
{
    ...
    if (dirty_pool) state_set(STATE_DIRTYPOOL);
    if (ls) ls->dirty_parent_inodes.push_back(&item_dirty_parent);
}
...
{
    if (is_base() || is_dirty_parent() || !is_auth()) return;
    ...
    if (err == 0) {
        ...
        decode(backtrace, bl);
        CDentry* pdn = get_parent_dn();
        if (backtrace.ancestors.empty() || backtrace.ancestors[0].dname != pdn->get_name() ||
...
void InodeStoreBase::encode_bare(bufferlist& bl, uint64_t features, const bufferlist* snap_blob) const
{
    using ceph::encode;
    encode(*inode, bl, features);
    if (inode->is_symlink()) encode(symlink, bl);
...
void InodeStoreBase::encode(bufferlist& bl, uint64_t features, const bufferlist* snap_blob) const
{
    ENCODE_START(6, 4, bl);
...
void CInode::encode_store(bufferlist& bl, uint64_t features)
{
    ...
    InodeStoreBase::encode(bl, mdcache->mds->mdsmap->get_up_features(), &snap_blob);
}

void InodeStoreBase::decode_bare(bufferlist::const_iterator& bl, bufferlist& snap_blob, __u8 struct_v)
{
    using ceph::decode;
...
void CInode::decode_lock_iauth(bufferlist::const_iterator& p)
{
    ...
    decode(_inode->uid, p);
    decode(_inode->gid, p);
    if (struct_v >= 2) decode(_inode->fscrypt_auth, p);
    DECODE_FINISH(p);
    reset_inode(std::move(_inode));
}
    ...
    }
    {
        ...
        if (is_auth()) {
            ...
            for (const auto& p : dirfrags) {
                ...
                if (p.second->is_auth()) p.second->state_clear(CDir::STATE_DIRTYDFT);
            }
        }
        if (g_conf()->mds_debug_frag) verify_dirfrags();
    }
    DECODE_FINISH(p);

    if (_inode) reset_inode(std::move(_inode));
}
...
void CInode::encode_lock_ifile(bufferlist& bl)
{
    ...
    encode(n, bl);
    bl.claim_append(tmp);
    if (is_auth()) encode(get_inode()->fscrypt_file, bl);
    ENCODE_FINISH(bl);
}
    ...
    while (n--) {
        ...
        if (is_auth()) {
            ...
            if (dir && dir->is_auth()) {
                ...
                dir->first = fgfirst;
                const auto& pf = dir->get_projected_fnode();
                finish_scatter_update(&filelock, dir, _inode->dirstat.version, pf->accounted_fragstat.version);
            }
        }
    }
    if (!is_auth() && struct_v >= 2) decode(_inode->fscrypt_file, p);
    DECODE_FINISH(p);
    ...
    while (n--) {
        ...
        if (is_auth()) {
            ...
            if (dir && dir->is_auth()) {
                ...
                dir->first = fgfirst;
                const auto& pf = dir->get_projected_fnode();
                finish_scatter_update(&nestlock, dir, _inode->rstat.version, pf->accounted_rstat.version);
            }
        }
    }
    DECODE_FINISH(p);
...
void CInode::decode_lock_ipolicy(bufferlist::const_iterator& p)
{
    ...
    bool pin_updated = (get_inode()->export_pin != _inode->export_pin) ||
                       (get_inode()->export_ephemeral_distributed_pin != _inode->export_ephemeral_distributed_pin);
    reset_inode(std::move(_inode));
...
void CInode::encode_lock_state(int type, bufferlist& bl)
{
    ENCODE_START(1, 1, bl);
    encode(first, bl);
    if (!is_base()) encode(parent->first, bl);

    switch (type) {
    case CEPH_LOCK_IAUTH: encode_lock_iauth(bl); break;
    ...
    case CEPH_LOCK_IFLOCK: encode_lock_iflock(bl); break;

    case CEPH_LOCK_IPOLICY: encode_lock_ipolicy(bl); break;
...
void CInode::decode_lock_state(int type, const bufferlist& bl)
{
    ...
    switch (type) {
    case CEPH_LOCK_IAUTH: decode_lock_iauth(p); break;

    case CEPH_LOCK_ILINK: decode_lock_ilink(p); break;

    case CEPH_LOCK_IDFT: decode_lock_idft(p); break;

    case CEPH_LOCK_IFILE: decode_lock_ifile(p); break;

    case CEPH_LOCK_INEST: decode_lock_inest(p); break;

    case CEPH_LOCK_IXATTR: decode_lock_ixattr(p); break;

    case CEPH_LOCK_ISNAP: decode_lock_isnap(p); break;

    case CEPH_LOCK_IFLOCK: decode_lock_iflock(p); break;

    case CEPH_LOCK_IPOLICY: decode_lock_ipolicy(p); break;
...
{
    return filelock.is_dirty_or_flushing() || nestlock.is_dirty_or_flushing() || dirfragtreelock.is_dirty_or_flushing();
}
...
void CInode::clear_dirty_scattered(int type)
{
    ...
    ceph_assert(is_dir());
    switch (type) {
    case CEPH_LOCK_IFILE: item_dirty_dirfrag_dir.remove_myself(); break;

    case CEPH_LOCK_INEST: item_dirty_dirfrag_nest.remove_myself(); break;

    case CEPH_LOCK_IDFT: item_dirty_dirfrag_dirfragtree.remove_myself(); break;
...
void CInode::start_scatter(ScatterLock* lock)
{
    ...
    for (const auto& p : dirfrags) {
        ...
        case CEPH_LOCK_INEST: finish_scatter_update(lock, dir, pi->rstat.version, pf->accounted_rstat.version); break;

        case CEPH_LOCK_IDFT: dir->state_clear(CDir::STATE_DIRTYDFT); break;
        }
    }
}
...
protected:
    ...
    MutationRef mut;
    MDSRank* get_mds() override { return in->mdcache->mds; }
    void finish(int r) override { in->_finish_frag_update(dir, mut); }
...
void CInode::finish_scatter_update(ScatterLock* lock, CDir* dir, version_t inode_version,
                                   version_t dir_accounted_version)
{
    ...
    if (dir->is_frozen()) {
        ...
        if (dir_accounted_version != inode_version) {
            ...
            if (lock->get_type() == CEPH_LOCK_INEST && !is_auth() && lock->get_state() == LOCK_MIX) {
            ...
            dout(10) << __func__ << " " << fg << " accounted " << *lock << " scatter stat unchanged at v"
                     << dir_accounted_version << dendl;
        }
    }
}
...
void CInode::finish_scatter_gather_update(int type, MutationRef& mut)
{
    ...
    case CEPH_LOCK_IFILE:
    {
        ...
        for (const auto& p : dirfrags) {
            ...
            if (pf->fragstat.nfiles < 0 || pf->fragstat.nsubdirs < 0) {
                clog->error() << "bad/negative dir size on " << dir->dirfrag() << " " << pf->fragstat;
                ceph_assert(!"bad/negative fragstat" == g_conf()->mds_verify_scatter);

                auto _pf = const_cast<fnode_t*>(pf.get());
                if (pf->fragstat.nfiles < 0) _pf->fragstat.nfiles = 0;
                if (pf->fragstat.nsubdirs < 0) _pf->fragstat.nsubdirs = 0;
            }
        ...
        if (touched_mtime) pi->mtime = pi->ctime = pi->dirstat.mtime;
        if (touched_chattr) pi->change_attr++;
        ...
        if (dirstat_valid && !dirstat.same_sums(pi->dirstat)) {
            ...
            if (dirstat_valid) {
                if (state_test(CInode::STATE_REPAIRSTATS)) {
                    ...
                    clog->error() << "unmatched fragstat on " << ino() << ", inode has " << pi->dirstat
                ...
                if (pi->dirstat.mtime > dirstat.mtime) dirstat.mtime = pi->dirstat.mtime;
                if (pi->dirstat.change_attr > dirstat.change_attr) dirstat.change_attr = pi->dirstat.change_attr;
                pi->dirstat = dirstat;
        ...
        if (pi->dirstat.nfiles < 0 || pi->dirstat.nsubdirs < 0) {
            ...
            clog->error() << "Inconsistent statistics detected: fragstat on inode " << ino() << " (" << path
                          << "), inode has " << pi->dirstat;
            ceph_assert(!"bad/negative fragstat" == g_conf()->mds_verify_scatter);

            if (pi->dirstat.nfiles < 0) pi->dirstat.nfiles = 0;
            if (pi->dirstat.nsubdirs < 0) pi->dirstat.nsubdirs = 0;
        }
    } break;
    ...
    case CEPH_LOCK_INEST:
    {
        ...
        rstat.rsubdirs = 1;
        if (const sr_t* srnode = get_projected_srnode(); srnode) rstat.rsnaps = srnode->snaps.size();
        ...
        for (const auto& p : dirfrags) {
            ...
            if (pf->accounted_rstat.version == pi->rstat.version - 1) {
                ...
                if (update) dir->assimilate_dirty_rstat_inodes(mut);
                ...
                mdcache->project_rstat_frag_to_inode(
                    pf->rstat, pf->accounted_rstat, dir->first, CEPH_NOSNAP, this, true);
                for (auto& p : dir->dirty_old_rstat) {
                    mdcache->project_rstat_frag_to_inode(
                        p.second.rstat, p.second.accounted_rstat, p.second.first, p.first, this, true);
                }
        ...
        if (rstat_valid && !rstat.same_sums(pi->rstat)) {
            ...
            if (rstat_valid) {
                if (state_test(CInode::STATE_REPAIRSTATS)) {
                    ...
                    clog->error() << "inconsistent rstat on inode " << ino() << ", inode has " << pi->rstat
                                  << ", directory fragments have " << rstat;
                    ceph_assert(!"unmatched rstat" == g_conf()->mds_verify_scatter);
                }
                // trust the dirfrag for now
                version_t v = pi->rstat.version;
                if (pi->rstat.rctime > rstat.rctime) rstat.rctime = pi->rstat.rctime;
                pi->rstat = rstat;
                pi->rstat.version = v;
            }
        }
...
void CInode::finish_scatter_gather_update_accounted(int type, EMetaBlob* metablob)
{
    ...
    for (const auto& p : dirfrags) {
        CDir* dir = p.second;
        if (!dir->is_auth() || dir->get_version() == 0 || dir->is_frozen()) continue;

        if (type == CEPH_LOCK_IDFT) continue;   // nothing to do.

        if (type == CEPH_LOCK_INEST) dir->assimilate_dirty_rstat_inodes_finish(metablob);
...
{
    if (waiting_on_dir.empty()) get(PIN_DIRWAITER);
...
{
    if (waiting_on_dir.empty()) return;
    ...
    if (it != waiting_on_dir.end()) {
        ...
        if (waiting_on_dir.empty()) put(PIN_DIRWAITER);
    }
}

void CInode::add_waiter(uint64_t tag, MDSContext* c)
{
    dout(10) << __func__ << " tag " << std::hex << tag << std::dec << " " << c << " !ambig "
             << !state_test(STATE_AMBIGUOUSAUTH) << " !frozen " << !is_frozen_inode() << " !freezing "
             << !is_freezing_inode() << dendl;
    ...
    if (((tag & WAIT_SINGLEAUTH) && !state_test(STATE_AMBIGUOUSAUTH)) ||
        ((tag & WAIT_UNFREEZE) && !is_frozen_inode() && !is_freezing_inode() && !is_frozen_auth_pin())) {
...
void CInode::maybe_finish_freeze_inode()
{
    ...
    if (auth_pins > auth_pin_freeze_allowance || dir->frozen_inode_suppressed) return;
...
bool CInode::freeze_inode(int auth_pin_allowance)
{
    ...
    if (!dir->lock_caches_with_auth_pins.empty()) mdcache->mds->locker->invalidate_lock_caches(dir);

    const static int lock_types[] = {CEPH_LOCK_IVERSION,
                                     CEPH_LOCK_IFILE,
                                     CEPH_LOCK_IAUTH,
                                     CEPH_LOCK_ILINK,
                                     CEPH_LOCK_IDFT,
    ...
    for (int i = 0; lock_types[i]; ++i) {
        ...
        if (lock->is_cached()) mdcache->mds->locker->invalidate_lock_caches(lock);
    }
...
bool CInode::can_auth_pin(int* err_ret) const
{
    ...
    if (!is_auth()) {
        ...
        if (parent) return parent->can_auth_pin(err_ret);
        err = 0;
    }
    if (err && err_ret) *err_ret = err;
    return !err;
}

void CInode::auth_pin(void* by)
{
    if (auth_pins == 0) get(PIN_AUTHPIN);
    auth_pins++;
    ...
    if (parent) parent->adjust_nested_auth_pins(1, this);
}
...
void CInode::auth_unpin(void* by)
{
    ...
    if (parent) parent->adjust_nested_auth_pins(-1, by);

    if (is_freezing_inode()) maybe_finish_freeze_inode();
}
...
{
    if (inode_auth.first >= 0) return inode_auth;

    if (parent) return parent->dir->authority();

    // new items that are not yet linked in (in the committed plane) belong
    // to their first parent.
    if (!projected_parent.empty()) return projected_parent.front()->dir->authority();
...
{
    snapid_t t = first;
    if (is_any_old_inodes()) t = get_old_inodes()->begin()->second.first;
...
const CInode::mempool_old_inode& CInode::cow_old_inode(snapid_t follows, bool cow_head)
{
    ...
    if (old_inodes) *_old_inodes = *old_inodes;
    ...
    if (first < oldest_snap) oldest_snap = first;

    old.inode.trim_client_ranges(follows);

    if (g_conf()->mds_snap_rstat && !(old.inode.rstat == old.inode.accounted_rstat)) dirty_old_rstats.insert(follows);

    first = follows + 1;

    dout(10) << __func__ << " " << (cow_head ? "head" : "previous_head") << " to [" << old.first << "," << follows
             << "] on " << *this << dendl;
...
void CInode::pre_cow_old_inode()
{
    ...
    if (first <= follows) cow_old_inode(follows, true);
}
...
void CInode::purge_stale_snap_data(const set<snapid_t>& snaps)
{
    ...
    if (to_remove.size() == get_old_inodes()->size()) {
        ...
        for (auto id : to_remove) _old_inodes->erase(id);
        reset_old_inodes(std::move(_old_inodes));
    }
}
...
void CInode::open_snaprealm(bool nosplit)
{
    if (!snaprealm) {
        ...
        if (parent) {
            ...
            if (!nosplit) parent->split_at(snaprealm);
...
void CInode::decode_snap_blob(const bufferlist& snapbl)
{
    ...
    else if (snaprealm && !is_root() && !is_mdsdir()) {   // see https://tracker.ceph.com/issues/42675
...
{
    if (mdcache->is_readonly()) return -1;
    if (!get_mds_caps_wanted().empty()) return -1;

    int n = 0;
    client_t loner = -1;
    for (const auto& p : client_caps) {
        if (!p.second.is_stale() && (is_dir() ? !has_subtree_or_exporting_dirfrag()
...
bool CInode::choose_ideal_loner()
{
    ...
    if (loner_cap >= 0 && loner_cap != want_loner_cap) {
        if (!try_drop_loner()) return false;
        changed = true;
    }
...
bool CInode::try_set_loner()
{
    ...
    if (loner_cap >= 0 && loner_cap != want_loner_cap) return false;
    set_loner_cap(want_loner_cap);
    return true;
}
...
void CInode::choose_lock_state(SimpleLock* lock, int allissued)
{
    ...
    if (is_auth()) {
        ...
        if (lock->is_xlocked()) ceph_assert(lock->get_state() == LOCK_LOCK);
    }
}
...
void CInode::choose_lock_states(int dirty_caps)
{
    ...
    if (is_auth() && (issued & (CEPH_CAP_ANY_EXCL | CEPH_CAP_ANY_WR))) choose_ideal_loner();
...
void CInode::set_mds_caps_wanted(mds_rank_t mds, int32_t wanted)
{
    ...
    if (wanted) {
        ...
        if (old_empty) adjust_num_caps_notable(1);
    }
    else if (!old_empty) {
        mds_caps_wanted.erase(mds);
        if (mds_caps_wanted.empty()) adjust_num_caps_notable(-1);
    }
}

Capability* CInode::add_client_cap(client_t client, Session* session, SnapRealm* conrealm, bool new_inode)
{
    ...
    if (client_caps.empty()) {
        ...
        if (parent) parent->dir->adjust_num_inodes_with_caps(1);
    }

    uint64_t cap_id = new_inode ? 1 : ++mdcache->last_cap_id;
    auto ret = client_caps.emplace(
        std::piecewise_construct, std::forward_as_tuple(client), std::forward_as_tuple(this, session, cap_id));
...
void CInode::remove_client_cap(client_t client)
{
    ...
    if (client == loner_cap) loner_cap = -1;

    if (cap->is_wanted_notable()) adjust_num_caps_notable(-1);
...
{
    while (!client_caps.empty()) remove_client_cap(client_caps.begin()->first);
    loner_cap = -1;
    want_loner_cap = -1;
...
int CInode::get_caps_allowed_ever() const
{
    ...
    return allowed & (CEPH_CAP_PIN | (filelock.gcaps_allowed_ever() << filelock.get_cap_shift()) |
...
{
    return CEPH_CAP_PIN | (filelock.gcaps_allowed(type) << filelock.get_cap_shift()) |
...
{
    return (filelock.gcaps_careful() << filelock.get_cap_shift()) |
...
{
    return (filelock.gcaps_xlocker_mask(client) << filelock.get_cap_shift()) |
...
int CInode::get_caps_allowed_for_client(Session* session, Capability* cap, const mempool_inode* file_i) const
{
    ...
    if (client == get_loner()) {
        ...
        allowed =
            get_caps_allowed_by_type(CAP_LONER) | (get_caps_allowed_by_type(CAP_XLOCKER) & get_xlocker_mask(client));
    }
    else {
    ...
    if (is_dir()) {
        ...
        if (cap && (allowed & CEPH_CAP_FILE_EXCL)) allowed |= cap->get_lock_cache_allowed();
    }
    else {
        if (file_i->inline_data.version == CEPH_INLINE_NONE && file_i->layout.pool_ns.empty()) {
            // noop
        }
        else if (cap) {
            if ((file_i->inline_data.version != CEPH_INLINE_NONE && cap->is_noinline()) ||
                (!file_i->layout.pool_ns.empty() && cap->is_nopoolns()))
                allowed &= ~(CEPH_CAP_FILE_RD | CEPH_CAP_FILE_WR);
        }
        else {
            auto& conn = session->get_connection();
            if ((file_i->inline_data.version != CEPH_INLINE_NONE && !conn->has_feature(CEPH_FEATURE_MDS_INLINE_DATA)) ||
                (!file_i->layout.pool_ns.empty() && !conn->has_feature(CEPH_FEATURE_FS_FILE_LAYOUT_V2)))
                allowed &= ~(CEPH_CAP_FILE_RD | CEPH_CAP_FILE_WR);
        }
    }
    return allowed;
...
// caps issued, wanted
int CInode::get_caps_issued(int* ploner, int* pother, int* pxlocker, int shift, int mask)
{
    int c = 0;
...
{
    for (const auto& p : client_caps) {
        if (p.second.wanted()) return true;
    }
    return false;
}
...
bool CInode::issued_caps_need_gather(SimpleLock* lock)
{
    ...
    get_caps_issued(&loner_issued, &other_issued, &xlocker_issued, lock->get_cap_shift(), lock->get_cap_mask());
    if ((loner_issued & ~lock->gcaps_allowed(CAP_LONER)) || (other_issued & ~lock->gcaps_allowed(CAP_ANY)) ||
...
{
    if (last != CEPH_NOSNAP) return;
    if (!state_test(STATE_CLIENTWRITEABLE)) {
        if (num_caps_notable == 0) mdcache->open_file_table.add_inode(this);
...
{
    if (state_test(STATE_CLIENTWRITEABLE)) {
        if (num_caps_notable == 0) mdcache->open_file_table.remove_inode(this);
...
int CInode::encode_inodestat(bufferlist& bl, Session* session, SnapRealm* dir_realm, snapid_t snapid,
                             unsigned max_bytes, int getattr_caps)
{
    client_t client = session->get_client();
    ceph_assert(snapid);
    ...
    if (snapid != CEPH_NOSNAP) {
        ...
        if (!is_auth()) valid = false;

        if (is_any_old_inodes()) {
            auto it = old_inodes->lower_bound(snapid);
            if (it != old_inodes->end()) {
                if (it->second.first > snapid) {
                    if (it != old_inodes->begin()) --it;
                }
                if (it->second.first <= snapid && snapid <= it->first) {
                    dout(15) << __func__ << " snapid " << snapid << " to old_inode [" << it->second.first << ","
                             << it->first << "]"
                             << " " << it->second.inode.rstat << dendl;
    ...
    bool no_caps = !valid || session->is_stale() || (dir_realm && realm != dir_realm) || is_frozen() ||
                   state_test(CInode::STATE_EXPORTINGCAPS);
    if (no_caps)
        dout(20) << __func__ << " no caps" << (!valid ? ", !valid" : "")
                 << (session->is_stale() ? ", session stale " : "")
                 << ((dir_realm && realm != dir_realm) ? ", snaprealm differs " : "")
                 << (is_frozen() ? ", frozen inode" : "")
                 << (state_test(CInode::STATE_EXPORTINGCAPS) ? ", exporting caps" : "") << dendl;
    ...
    dout(20) << " pfile " << pfile << " pauth " << pauth << " plink " << plink << " pxattr " << pxattr << " plocal "
             << plocal << " mtime " << any_i->mtime << " ctime " << any_i->ctime << " change_attr "
             << any_i->change_attr << " valid=" << valid << dendl;
    ...
    uint64_t max_size = std::min(oi->get_client_range(client), pi->get_client_range(client));
    ...
    else if ((!cap && !no_caps) || (cap && cap->client_inline_version < file_i->inline_data.version) ||
             (getattr_caps & CEPH_CAP_FILE_RD)) {   // client requests inline data
        inline_version = file_i->inline_data.version;
        if (file_i->inline_data.length() > 0) file_i->inline_data.get_data(inline_data);
    }
    ...
    if ((!cap && !no_caps) || (cap && cap->client_xattr_version < xattr_i->xattr_version) ||
        (getattr_caps & CEPH_CAP_XATTR_SHARED)) {   // client requests xattrs
        if (!pxattrs) pxattrs = pxattr ? get_projected_xattrs().get() : get_xattrs().get();
    ...
    // do we have room?
    if (max_bytes) {
        unsigned bytes = 8 + 8 + 4 + 8 + 8 + sizeof(ceph_mds_reply_cap) + sizeof(struct ceph_file_layout) +
        ...
        if (xattr_version) {
            ...
            if (pxattrs) {
                for (const auto& p : *pxattrs) bytes += sizeof(__u32) * 2 + p.first.length() + p.second.length();
            }
        }
        else {
            bytes += sizeof(__u32);   // xattr buffer len
        }
        bytes += sizeof(version_t) + sizeof(__u32) + inline_data.length() +   // inline data
                 1 + 1 + 8 + 8 + 4 +                                          // quota
                 4 + layout.pool_ns.size() +                                  // pool ns
                 sizeof(struct ceph_timespec) + 8;                            // btime + change_attr

        if (bytes > max_bytes) return -CEPHFS_ENOSPC;
    }
    ...
    if (snapid != CEPH_NOSNAP) {
        ...
        if (!no_caps && cap) {
            ...
            issue = CEPH_CAP_PIN;
            cap->issue_norevoke(issue, true);
            dout(10) << "encode_inodestat issuing " << ccap_string(issue) << " seq " << cap->get_last_seq()
    ...
    dout(10) << "encode_inodestat caps " << ccap_string(ecap.caps) << " seq " << ecap.seq << " mseq " << ecap.mseq
...
void CInode::encode_cap_message(const ref_t<MClientCaps>& m, Capability* cap)
{
    ...
    dout(20) << __func__ << " pfile " << pfile << " pauth " << pauth << " plink " << plink << " pxattr " << pxattr
    ...
    if (cap->client_inline_version < i->inline_data.version) {
        ...
        if (i->inline_data.length() > 0) i->inline_data.get_data(m->inline_data);
    }
    else {
        m->inline_version = 0;
    }
                                  ...
                                  list<SimpleLock*>& eval_locks, bool survivor)
{
    ...
    if (!dirfragtreelock.is_stable() && !dirfragtreelock.is_wrlocked()) eval_locks.push_back(&dirfragtreelock);
    if (!filelock.is_stable() && !filelock.is_wrlocked()) eval_locks.push_back(&filelock);
    if (!nestlock.is_stable() && !nestlock.is_wrlocked()) eval_locks.push_back(&nestlock);
}
...
void CInode::decode_import(bufferlist::const_iterator& p, LogSegment* ls)
{
    DECODE_START(5, p);
...

    {
        ...
        set_ephemeral_pin((s & STATE_DISTEPHEMERALPIN), (s & STATE_RANDEPHEMERALPIN));
    ...
    while (!q.end()) {
        ...
        auto _fnode = CDir::allocate_fnode(*dir->get_fnode());
        if (dir->is_auth() || filelock.get_state() == LOCK_MIX) {
        ...
        if (dir->is_auth() || nestlock.get_state() == LOCK_MIX) {
...
template<> void decode_json_obj(mempool::mds_co::string& t, JSONObj* obj)
{
...
void InodeStoreBase::decode_json(JSONObj* obj)
{
    ...
    // JSONDecoder::decode_json("old_inodes", old_inodes, InodeStoreBase::old_indoes_cb, obj, true); // cann't decode
    // old_inodes now
...
void CInode::validate_disk_state(CInode::validated_data* results, MDSContext* fin)
{
    ...
    public:
        ...
        ValidationContinuation(CInode* i, CInode::validated_data* data_r, MDSContext* fin_)
         ...
         */
        void fetch_backtrace_and_tag(CInode* in, std::string_view tag, bool is_internal, Context* fin, int* bt_r,
                                     bufferlist* bt)
        {
            ...
            in->mdcache->mds->objecter->read(oid, object_locator_t(pool), fetch, CEPH_NOSNAP, NULL, 0, fin);
            ...
            if (!is_internal) {
                ...
                SnapContext snapc;
                in->mdcache->mds->objecter->mutate(
                    oid, object_locator_t(pool), scrub_tag, snapc, ceph::real_clock::now(), 0, NULL);
                if (in->mdcache->mds->logger) in->mdcache->mds->logger->inc(l_mds_scrub_set_tag);
            }
        }
        ...
        bool _start(int rval)
        {
            ...
            if (in->is_dirty()) {
                ...
                dout(20) << "validating a dirty CInode; results will be inconclusive" << dendl;
            }

            C_OnFinisher* conf = new C_OnFinisher(get_io_callback(BACKTRACE), in->mdcache->mds->finisher);
        ...
        bool _backtrace(int rval)
        {
            ...
            try {
                ...
                results->backtrace.error_str << "failed to decode on-disk backtrace (" << bl.length() << " bytes)!";
                // we probably have a new unwritten file!
                // so skip the backtrace scrub for this entry and say that all's well
                if (in->is_dirty_parent()) {
                    dout(20) << "decode failed; forcing backtrace as passed since "
                                "inode is dirty parent"
                             << dendl;
                    results->backtrace.passed = true;
                }
            ...
            memory_newer = memory_backtrace.compare(results->backtrace.ondisk_value, &equivalent, &divergent);
        ...
        next:
            ...
            if (!results->backtrace.passed && in->scrub_infop->header->get_repair()) {
                ...
                in->mdcache->mds->clog->warn()
                    << "bad backtrace on inode " << in->ino() << "(" << path << "), rewriting it";
                in->mark_dirty_parent(in->mdcache->mds->mdlog->get_current_segment(), false);
                // Flag that we repaired this BT so that it won't go into damagetable
                results->backtrace.repaired = true;
                if (in->mdcache->mds->logger) in->mdcache->mds->logger->inc(l_mds_scrub_backtrace_repaired);
            }
                ...
                if (inotable->is_marked_free(in->ino())) {
                    ...
                    if (in->scrub_infop->header->get_repair()) {
                        ...
                        if (repaired) {
                            ...
                            inotable->save();
                            if (in->mdcache->mds->logger) in->mdcache->mds->logger->inc(l_mds_scrub_inotable_repaired);
                        }
                        else {
            ...
            if (in->is_dir()) {
                if (in->mdcache->mds->logger) in->mdcache->mds->logger->inc(l_mds_scrub_dir_inodes);
                return validate_directory_data();
            }
            else {
                if (in->mdcache->mds->logger) in->mdcache->mds->logger->inc(l_mds_scrub_file_inodes);
        ...
        bool validate_directory_data()
        {
            ...
            if (in->is_base()) {
                ...
                if (in->mdcache->mds->logger) in->mdcache->mds->logger->inc(l_mds_scrub_dir_base_inodes);
                return false;
            }
            else {
                // TODO: validate on-disk inode for non-base directories
                if (in->mdcache->mds->logger) in->mdcache->mds->logger->inc(l_mds_scrub_dirfrag_rstats);
        ...
        bool _inode_disk(int rval)
        {
            ...
            if (si->version > i->version) {
                ...
                if (!results->inode.passed) {
                    results->inode.error_str << "On-disk inode is divergent or newer than in-memory one; ";
                    goto next;
                }
            }
        next:
        ...
        bool _dirfrags(int rval)
        {
            ...
            if (const sr_t* srnode = in->get_projected_srnode(); srnode) nest_info.rsnaps += srnode->snaps.size();

            // ...and that their sum matches our inode settings
            if (!dir_info.same_sums(in->get_inode()->dirstat) || !nest_info.same_sums(in->get_inode()->rstat)) {
                if (in->scrub_infop->header->get_repair()) {
                    ...
                    results->raw_stats.error_str << "freshly-calculated rstats don't match existing ones";
                }
                if (in->is_dirty()) {
        ...
        void _done() override
        {
            ...
            if (results->backtrace.repaired || results->inode.repaired || results->raw_stats.repaired)
                in->scrub_infop->header->set_repaired();
            if (fin) fin->complete(get_rval());
    ...
    ValidationContinuation* vc = new ValidationContinuation(this, results, fin);
    vc->begin();
}
...
void CInode::validated_data::dump(Formatter* f) const
{
    f->open_object_section("results");
    {
        ...
        int rc = 0;
        if (backtrace.checked && backtrace.ondisk_read_retval) rc = backtrace.ondisk_read_retval;
        if (inode.checked && inode.ondisk_read_retval) rc = inode.ondisk_read_retval;
        if (raw_stats.checked && raw_stats.ondisk_read_retval) rc = raw_stats.ondisk_read_retval;
...
{
    bool unrepaired = (raw_stats.checked && !raw_stats.passed && !raw_stats.repaired) ||
...
void CInode::dump(Formatter* f, int flags) const
{
    ...
    if (flags & DUMP_INODE_STORE_BASE) InodeStoreBase::dump(f);

    if (flags & DUMP_MDS_CACHE_OBJECT) MDSCacheObject::dump(f);
    ...
    if (flags & DUMP_STATE) {
        ...
        if (state_test(STATE_EXPORTING)) f->dump_string("state", "exporting");
        if (state_test(STATE_OPENINGDIR)) f->dump_string("state", "openingdir");
        if (state_test(STATE_FREEZING)) f->dump_string("state", "freezing");
        if (state_test(STATE_FROZEN)) f->dump_string("state", "frozen");
        if (state_test(STATE_AMBIGUOUSAUTH)) f->dump_string("state", "ambiguousauth");
        if (state_test(STATE_EXPORTINGCAPS)) f->dump_string("state", "exportingcaps");
        if (state_test(STATE_NEEDSRECOVER)) f->dump_string("state", "needsrecover");
        if (state_test(STATE_PURGING)) f->dump_string("state", "purging");
        if (state_test(STATE_DIRTYPARENT)) f->dump_string("state", "dirtyparent");
        if (state_test(STATE_DIRTYRSTAT)) f->dump_string("state", "dirtyrstat");
        if (state_test(STATE_STRAYPINNED)) f->dump_string("state", "straypinned");
        if (state_test(STATE_FROZENAUTHPIN)) f->dump_string("state", "frozenauthpin");
        if (state_test(STATE_DIRTYPOOL)) f->dump_string("state", "dirtypool");
        if (state_test(STATE_ORPHAN)) f->dump_string("state", "orphan");
        if (state_test(STATE_MISSINGOBJS)) f->dump_string("state", "missingobjs");
        f->close_section();
    }
...
{
    if (scrub_infop && !scrub_infop->scrub_in_progress && !scrub_infop->last_scrub_dirty) {
        scrub_infop.reset();
    }
}
...
{
    if (state_test(CInode::STATE_QUEUEDEXPORTPIN)) return;
    ...
    for (auto& p : dirfrags) {
        ...
        if (target != MDS_RANK_NONE) {
            if (dir->is_subtree_root()) {
                ...
                if (!dir->state_test(CDir::STATE_AUXSUBTREE) || target != dir->get_dir_auth().first) queue = true;
            }
            else {
...
{
    if (!g_conf()->mds_bal_export_pin) return;
    if (!is_dir() || !is_normal()) return;

    dout(15) << __func__ << " update=" << update << " " << *this << dendl;

    mds_rank_t export_pin = get_export_pin(false);
    if (export_pin == MDS_RANK_NONE && !update) return;
...
{
    unsigned state = 0;
    if (dist) state |= STATE_DISTEPHEMERALPIN;
    if (rand) state |= STATE_RANDEPHEMERALPIN;
    if (!state) return;
...
void CInode::clear_ephemeral_pin(bool dist, bool rand)
{
    ...
    if (state_test(state)) {
        dout(10) << "clear ephemeral (" << (dist ? "dist" : "") << (rand ? " rand" : "") << ") pin on " << *this
                 << dendl;
        state_clear(state);
        if (!is_ephemerally_pinned()) {
...
{
    if (!g_conf()->mds_bal_export_pin) return MDS_RANK_NONE;
    ...
    while (true) {
        ...
        if (in->get_inode()->export_pin >= 0) {
            ...
            if (in != this) return mdcache->hash_into_rank_bucket(in->ino(), dir->get_frag());
            ...
            /* If a parent overrides a grandparent ephemeral pin policy with an export pin, we use that export pin
             * instead. */
            if (!inherit) return MDS_RANK_EPHEMERAL_RAND;
            if (in == this)
...
double CInode::get_ephemeral_rand() const
{
    ...
    while (true) {
         ...
         * random pin set.
         */
        if (in->get_inode()->export_pin >= 0 || in->get_inode()->export_ephemeral_distributed_pin) return 0.0;
...
void CInode::get_nested_dirfrags(std::vector<CDir*>& v) const
{
    for (const auto& p : dirfrags) {
        ...
        if (!dir->is_subtree_root()) v.push_back(dir);
    }
}
