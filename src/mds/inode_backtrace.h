...
{
    inode_backpointer_t() {}
    inode_backpointer_t(inodeno_t i, std::string_view d, version_t v)
        : dirino(i)
        , dname(d)
        , version(v)
    {}
...
struct inode_backtrace_t
{
     ...
     */
    int compare(const inode_backtrace_t& other, bool* equivalent, bool* divergent) const;
...
inline bool operator==(const inode_backtrace_t& l, const inode_backtrace_t& r)
{
    return l.ino == r.ino && l.pool == r.pool && l.old_pools == r.old_pools && l.ancestors == r.ancestors;
}
