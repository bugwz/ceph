...
const std::map<std::string, PurgeItem::Action> PurgeItem::actions = {{"NONE", PurgeItem::NONE},
...
void PurgeItem::decode(bufferlist::const_iterator& p)
{
    ...
    if (struct_v == 1) {
        ...
        try {
            ...
            decode(snapc, p);
            decode(fragtree, p);
            if (p.get_off() > struct_end) throw buffer::end_of_buffer();
            done = true;
        }
...
PurgeQueue::PurgeQueue(CephContext* cct_, mds_rank_t rank_, const int64_t metadata_pool_, Objecter* objecter_,
                       Context* on_error_)
    : cct(cct_)
    , rank(rank_)
    ...
    , objecter(objecter_)
    , journaler("pq", MDS_INO_PURGE_QUEUE + rank, metadata_pool, CEPH_FS_ONDISK_MAGIC, objecter_, nullptr, 0, &finisher)
    , on_error(on_error_)
{
...
void PurgeQueue::create_logger()
{
    ...
    pcb.add_u64_counter(
        l_pq_executed, "pq_executed", "Purge queue tasks executed", "purg", PerfCountersBuilder::PRIO_INTERESTING);

    pcb.set_prio_default(PerfCountersBuilder::PRIO_USEFUL);
    pcb.add_u64(l_pq_executing_ops, "pq_executing_ops", "Purge queue ops in flight");
    pcb.add_u64(
        l_pq_executing_ops_high_water, "pq_executing_ops_high_water", "Maximum number of executing file purge ops");
...
void PurgeQueue::activate()
{
    ...
    if (journaler.get_read_pos() == journaler.get_write_pos()) return;
...
void PurgeQueue::open(Context* completion)
{
    ...
    if (completion) waiting_for_recovery.push_back(completion);
...
void PurgeQueue::_recover()
{
    ...
    while (1) {
        if (!journaler.is_readable() && !journaler.get_error() &&
...
void PurgeQueue::create(Context* fin)
{
    ...
    if (fin) waiting_for_recovery.push_back(fin);
...
void PurgeQueue::push(const PurgeItem& pi, Context* completion)
{
    ...
    if (!could_consume) {
        ...
        if (!delayed_flush) {
            ...
            timer.add_event_after(g_conf()->mds_purge_queue_busy_flush_period, delayed_flush);
        }
    }
}
...
uint32_t PurgeQueue::_calculate_ops(const PurgeItem& item) const
{
    ...
    if (item.action == PurgeItem::PURGE_DIR) {
        ...
        const uint64_t num = (item.size > 0) ? Striper::get_num_objects(item.layout, item.size) : 1;
...
bool PurgeQueue::_can_consume()
{
    ...
    dout(20) << ops_in_flight << "/" << max_purge_ops << " ops, " << in_flight.size() << "/"
             << g_conf()->mds_max_purge_files << " files" << dendl;
...
public:
    ...
    void finish(int r) override { purge_queue->_commit_ops(r, ops_vec, expire_to); }
...
void PurgeQueue::_commit_ops(int r, const std::vector<PurgeItemCommitOp>& ops_vec, uint64_t expire_to)
{
    ...
    for (auto& op : ops_vec) {
        ...
        if (op.type == PurgeItemCommitOp::PURGE_OP_RANGE) {
            ...
            if (op.item.action == PurgeItem::PURGE_DIR) {
                objecter->remove(op.oid, op.oloc, nullsnapc, ceph::real_clock::now(), op.flags, gather.new_sub());
            }
            else {
                objecter->remove(op.oid, op.oloc, op.item.snapc, ceph::real_clock::now(), op.flags, gather.new_sub());
            }
        }
        else if (op.type == PurgeItemCommitOp::PURGE_OP_ZERO) {
            filer.zero(op.item.ino,
                       &op.item.layout,
                       op.item.snapc,
                       0,
                       op.item.layout.object_size,
                       ceph::real_clock::now(),
                       0,
                       true,
                       gather.new_sub());
    ...
    gather.set_finisher(new C_OnFinisher(new LambdaContext([this, expire_to](int r) {
                                             std::lock_guard l(lock);
...
void PurgeQueue::_execute_item(const PurgeItem& item, uint64_t expire_to)
{
    ...
    std::vector<PurgeItemCommitOp> ops_vec;
    auto submit_ops = [&]() { finisher.queue(new C_IO_PurgeItem_Commit(this, std::move(ops_vec), expire_to)); };

    if (item.action == PurgeItem::PURGE_FILE) {
        if (item.size > 0) {
            uint64_t num = Striper::get_num_objects(item.layout, item.size);
            dout(10) << " 0~" << item.size << " objects 0~" << num << " snapc " << item.snapc << " on " << item.ino
                     << dendl;
        ...
        frag_vec_t leaves;
        if (!item.fragtree.is_leaf(frag_t())) item.fragtree.get_leaves(leaves);
        ...
        derr << "Invalid item (action=" << item.action
             << ") in purge queue, "
                "dropping it"
             << dendl;
        ops_in_flight -= ops;
...
void PurgeQueue::_execute_item_complete(uint64_t expire_to)
{
    ceph_assert(ceph_mutex_is_locked_by_me(lock));
    ...
    uint64_t item_num = (write_pos - (in_flight.size() ? expire_pos : read_pos)) / purge_item_journal_size;
    dout(10) << "left purge items in journal: " << item_num
             << " (purge_item_journal_size/write_pos/read_pos/expire_pos) now at "
             << "(" << purge_item_journal_size << "/" << write_pos << "/" << read_pos << "/" << expire_pos << ")"
             << dendl;
...
void PurgeQueue::update_op_limit(const MDSMap& mds_map)
{
    ...
    max_purge_ops = uint64_t(((double)pg_count / (double)mds_map.get_max_mds()) * cct->_conf->mds_max_purge_ops_per_pg);
...
{
    if (changed.count("mds_max_purge_ops") || changed.count("mds_max_purge_ops_per_pg")) {
...
bool PurgeQueue::drain(uint64_t* progress, uint64_t* progress_total, size_t* in_flight_count)
{
    std::lock_guard l(lock);
    ...
    const bool done = in_flight.empty() && (journaler.get_read_pos() == journaler.get_write_pos());
    if (done) {
        return true;
    }

    const uint64_t bytes_remaining = journaler.get_write_pos() - journaler.get_read_pos();
