...
Capability::Capability(CInode* i, Session* s, uint64_t id)
    ...
    , item_client_revoking_caps(this)
    ...
    , cap_id(id)
{
    if (session) {
        ...
        if (session->is_stale()) --cap_gen;   // not valid

        auto& conn = session->get_connection();
        if (conn) {
            if (!conn->has_feature(CEPH_FEATURE_MDS_INLINE_DATA)) state |= STATE_NOINLINE;
            if (!conn->has_feature(CEPH_FEATURE_FS_FILE_LAYOUT_V2)) state |= STATE_NOPOOLNS;
            if (!conn->has_feature(CEPH_FEATURE_MDS_QUOTA)) state |= STATE_NOQUOTA;
        }
    }
    else {
        cap_gen = 0;
...
int Capability::confirm_receipt(ceph_seq_t seq, unsigned caps)
{
    ...
    if (seq == last_sent) {
        ...
        if (was_revoking && revoking()) {
            CInode* in = get_inode();
            dout(10) << "revocation is not totally finished yet on " << *in << ", the session " << *session << dendl;
            _revokes.emplace_back(_pending, last_sent, last_issue);
            if (!is_notable()) mark_notable();
        }
    }
    else {
        // can i forget any revocations?
        while (!_revokes.empty() && _revokes.front().seq < seq) _revokes.pop_front();
        if (!_revokes.empty()) {
            if (_revokes.front().seq == seq) _revokes.begin()->before = caps;
            calc_issued();
        }
        else {
            // seq < last_sent
...
{
    if (!is_valid()) cap_gen = session->get_cap_gen();
}
...
{
    if ((_issued == _pending) && !is_clientwriteable() && !is_wanted_notable(_wanted)) {
...
{
    if (inode) f->dump_stream("ino") << inode->ino();
