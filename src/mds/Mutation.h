...
public:
    ...
    struct LockOp
    {
        ...
        bool operator<(const LockOp& r) const { return lock < r.lock; }
    ...
    {
        LockOpVec() { reserve(32); }

        void add_rdlock(SimpleLock* lock) { emplace_back(lock, LockOp::RDLOCK); }
        ...
        void lock_scatter_gather(SimpleLock* lock) { emplace_back(lock, LockOp::WRLOCK | LockOp::STATE_PIN); }
        void sort_and_merge();
    };
    ...
    MutationImpl()
        : TrackedOp(nullptr, ceph_clock_now())
    {}
    MutationImpl(OpTracker* tracker, utime_t initiated, const metareqid_t& ri, __u32 att = 0,
    ...
    void disable_lock_cache() { lock_cache_disabled = true; }
    ...
    bool is_last_locked(SimpleLock* lock) const { return lock == last_locked; }
    ...
    {
        if (reqid.name.is_client()) return client_t(reqid.name.num());
        return -1;
    }

    void set_mds_stamp(utime_t t) { mds_stamp = t; }
    utime_t get_mds_stamp() const { return mds_stamp; }
    void set_op_stamp(utime_t t) { op_stamp = t; }
    utime_t get_op_stamp() const
    {
        if (op_stamp != utime_t()) return op_stamp;
    ...
    void add_projected_node(MDSCacheObject* obj) { projected_nodes.insert(obj); }
    void remove_projected_node(MDSCacheObject* obj) { projected_nodes.erase(obj); }
    bool is_projected(MDSCacheObject* obj) const { return projected_nodes.count(obj); }
    ...
    virtual void print(std::ostream& out) const { out << "mutation(" << this << ")"; }
...
struct MDRequestImpl : public MutationImpl
{
    ...
    struct Params
    {
        ...
        Params() {}
        const utime_t& get_recv_stamp() const { return initiated; }
        const utime_t& get_throttle_stamp() const { return throttled; }
        const utime_t& get_recv_complete_stamp() const { return all_read; }
        const utime_t& get_dispatch_stamp() const { return dispatched; }
        metareqid_t reqid;
        __u32 attempt = 0;
    ...
    MDRequestImpl(const Params* params, OpTracker* tracker)
        : MutationImpl(tracker, params->initiated, params->reqid, params->attempt, params->peer_to)
    ...
    bool can_batch();
    bool is_batch_head() { return batch_op_map != nullptr; }
    std::unique_ptr<BatchOp> release_batch_op();

    void print(std::ostream& out) const override;
    void dump_with_mds_lock(ceph::Formatter* f) const { return _dump(f, true); }
...
protected:
    void _dump(ceph::Formatter* f) const override { _dump(f, false); }
...
struct MDLockCache : public MutationImpl
{
    ...
    void set_dir_layout(file_layout_t& layout) { dir_layout = layout; }
    const file_layout_t& get_dir_layout() const { return dir_layout; }
