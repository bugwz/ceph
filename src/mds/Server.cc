...
public:
    explicit ServerContext(Server* s)
        : server(s)
    {
...
public:
    ...
    void add_request(const ceph::ref_t<MDRequestImpl>& r) override { batch_reqs.push_back(r); }
    ...
    void _forward(mds_rank_t t) override
    {
        ...
        for (auto& m : batch_reqs) {
            if (!m->killed) mdcache->request_forward(m, t);
        }
        batch_reqs.clear();
    }
    ...
    void print(std::ostream& o) const override { o << "[batch front=" << *mdr << "]"; }
};
...
protected:
    ...
    {
        if (mdr) mdr->mark_event("journal_committed: ");
    }

public:
    explicit ServerLogContext(Server* s)
        : server(s)
    {
        ceph_assert(server != NULL);
    }
    explicit ServerLogContext(Server* s, MDRequestRef& r)
        : server(s)
        , mdr(r)
    {
        ceph_assert(server != NULL);
...
void Server::create_logger()
{
    ...
    plb.add_u64_counter(l_mdss_handle_client_request,
                        "handle_client_request",
                        "Client requests",
                        "hcr",
                        PerfCountersBuilder::PRIO_INTERESTING);
    plb.add_u64_counter(l_mdss_handle_peer_request,
                        "handle_peer_request",
                        "Peer requests",
                        "hsr",
                        PerfCountersBuilder::PRIO_INTERESTING);
    plb.add_u64_counter(l_mdss_handle_client_session,
                        ...
                        "hcs",
                        PerfCountersBuilder::PRIO_INTERESTING);
    plb.add_u64_counter(l_mdss_cap_revoke_eviction,
                        "cap_revoke_eviction",
    ...
    plb.add_time_avg(
        l_mdss_req_lookuphash_latency, "req_lookuphash_latency", "Request type lookup hash of inode latency");
    plb.add_time_avg(l_mdss_req_lookupino_latency, "req_lookupino_latency", "Request type lookup inode latency");
    plb.add_time_avg(l_mdss_req_lookupparent_latency, "req_lookupparent_latency", "Request type lookup parent latency");
    plb.add_time_avg(l_mdss_req_lookupname_latency, "req_lookupname_latency", "Request type lookup name latency");
    plb.add_time_avg(l_mdss_req_lookup_latency, "req_lookup_latency", "Request type lookup latency");
    plb.add_time_avg(l_mdss_req_lookupsnap_latency, "req_lookupsnap_latency", "Request type lookup snapshot latency");
    plb.add_time_avg(l_mdss_req_getattr_latency, "req_getattr_latency", "Request type get attribute latency");
    plb.add_time_avg(l_mdss_req_setattr_latency, "req_setattr_latency", "Request type set attribute latency");
    plb.add_time_avg(l_mdss_req_setlayout_latency, "req_setlayout_latency", "Request type set file layout latency");
    plb.add_time_avg(
        l_mdss_req_setdirlayout_latency, "req_setdirlayout_latency", "Request type set directory layout latency");
    plb.add_time_avg(
        l_mdss_req_getvxattr_latency, "req_getvxattr_latency", "Request type get virtual extended attribute latency");
    plb.add_time_avg(
        l_mdss_req_setxattr_latency, "req_setxattr_latency", "Request type set extended attribute latency");
    plb.add_time_avg(
        l_mdss_req_rmxattr_latency, "req_rmxattr_latency", "Request type remove extended attribute latency");
    plb.add_time_avg(l_mdss_req_readdir_latency, "req_readdir_latency", "Request type read directory latency");
    plb.add_time_avg(l_mdss_req_setfilelock_latency, "req_setfilelock_latency", "Request type set file lock latency");
    plb.add_time_avg(l_mdss_req_getfilelock_latency, "req_getfilelock_latency", "Request type get file lock latency");
    plb.add_time_avg(l_mdss_req_create_latency, "req_create_latency", "Request type create latency");
    plb.add_time_avg(l_mdss_req_open_latency, "req_open_latency", "Request type open latency");
    plb.add_time_avg(l_mdss_req_mknod_latency, "req_mknod_latency", "Request type make node latency");
    plb.add_time_avg(l_mdss_req_link_latency, "req_link_latency", "Request type link latency");
    plb.add_time_avg(l_mdss_req_unlink_latency, "req_unlink_latency", "Request type unlink latency");
    plb.add_time_avg(l_mdss_req_rmdir_latency, "req_rmdir_latency", "Request type remove directory latency");
    plb.add_time_avg(l_mdss_req_rename_latency, "req_rename_latency", "Request type rename latency");
    plb.add_time_avg(l_mdss_req_mkdir_latency, "req_mkdir_latency", "Request type make directory latency");
    plb.add_time_avg(l_mdss_req_symlink_latency, "req_symlink_latency", "Request type symbolic link latency");
    plb.add_time_avg(l_mdss_req_lssnap_latency, "req_lssnap_latency", "Request type list snapshot latency");
    plb.add_time_avg(l_mdss_req_mksnap_latency, "req_mksnap_latency", "Request type make snapshot latency");
    plb.add_time_avg(l_mdss_req_rmsnap_latency, "req_rmsnap_latency", "Request type remove snapshot latency");
    plb.add_time_avg(l_mdss_req_renamesnap_latency, "req_renamesnap_latency", "Request type rename snapshot latency");
    plb.add_time_avg(l_mdss_req_snapdiff_latency, "req_snapdiff_latency", "Request type snapshot difference latency");
    ...
    plb.add_u64_counter(l_mdss_dispatch_peer_request, "dispatch_server_request", "Server requests dispatched");
...
{
    switch (m->get_type()) {
    case CEPH_MSG_CLIENT_RECONNECT: handle_client_reconnect(ref_cast<MClientReconnect>(m)); return;
    }

    /*
     *In reconnect phase, client sent unsafe requests to mds before reconnect msg. Seting sessionclosed_isok will handle
    scenario like this:

    1. In reconnect phase, client sent unsafe requests to mds.
    2. It reached reconnect timeout. All sessions without sending reconnect msg in time, some of which may had sent
    unsafe requests, are marked as closed. (Another situation is #31668, which will deny all client reconnect msg to
    speed up reboot). 3.So these unsafe request from session without sending reconnect msg in time or being denied could
    be handled in clientreplay phase.
    ...
    */
    ...
    if (m->get_type() == CEPH_MSG_CLIENT_REQUEST && !mds->is_active()) {
        ...
        if (mds->is_reconnect() || mds->get_want_state() == CEPH_MDS_STATE_RECONNECT) {
            ...
            if (req->is_replay() || req->is_async()) {
                ...
                queue_replay = true;
                if (req->head.ino && !session->have_completed_request(req->get_reqid().tid, nullptr)) {
                    inodeno_t ino(req->head.ino);
                    mdcache->add_replay_ino_alloc(ino);
                    if (replay_unsafe_with_closed_session && session->free_prealloc_inos.contains(ino)) {
    ...
    switch (m->get_type()) {
    case CEPH_MSG_CLIENT_SESSION: handle_client_session(ref_cast<MClientSession>(m)); return;
    case CEPH_MSG_CLIENT_REQUEST: handle_client_request(ref_cast<MClientRequest>(m)); return;
    case CEPH_MSG_CLIENT_RECLAIM: handle_client_reclaim(ref_cast<MClientReclaim>(m)); return;
    case MSG_MDS_PEER_REQUEST: handle_peer_request(ref_cast<MMDSPeerRequest>(m)); return;
    default:
        derr << "Server unknown message " << m->get_type() << " from peer type " << m->get_connection()->get_peer_type()
             << dendl;
        ceph_abort_msg("server unknown message  " + to_string(m->get_type()) + " from peer type " +
                       to_string(m->get_connection()->get_peer_type()));
    }
}
...
public:
    ...
    C_MDS_session_finish(Server* srv, Session* se, uint64_t sseq, bool s, version_t mv,
                         const interval_set<inodeno_t>& to_free, version_t iv, const interval_set<inodeno_t>& to_purge,
                         LogSegment* _ls, Context* fin_ = nullptr)
        : ServerLogContext(srv)
        , session(se)
        , state_seq(sseq)
        , open(s)
        , cmapv(mv)
        , inos_to_free(to_free)
        , inotablev(iv)
...
Session* Server::find_session_by_uuid(std::string_view uuid)
{
    ...
    for (auto& it : mds->sessionmap.get_sessions()) {
        ...
        if (p == metadata.end() || p->second != uuid) continue;
...
void Server::reclaim_session(Session* session, const cref_t<MClientReclaim>& m)
{
    ...
    if (target) {
        if (session->info.auth_name != target->info.auth_name) {
            dout(10) << __func__ << " session auth_name " << session->info.auth_name << " != target auth_name "
                     << target->info.auth_name << dendl;
    ...
    if (flags & CEPH_RECLAIM_RESET) {
        ...
        ceph_assert(0); /* no other flags are handled at this time */
}
...
void Server::finish_reclaim_session(Session* session, const ref_t<MClientReclaimReply>& reply)
{
    ...
    if (target) {
        ...
        bool blocklisted = mds->objecter->with_osdmap(
            [target](const OSDMap& map) { return map.is_blocklisted(target->info.inst.addr); });
...
void Server::handle_client_session(const cref_t<MClientSession>& m)
{
    ...
    if (!fs_name.empty() && !session->fs_name_capable(fs_name, MAY_READ)) {
        dout(0) << " dropping message not allowed for this fs_name: " << *m << dendl;
        auto reply = make_message<MClientSession>(CEPH_SESSION_REJECT);
        reply->metadata["error_string"] = "client doesn't have caps for FS \"" + std::string(fs_name) + "\"";
    ...
    if (logger) logger->inc(l_mdss_handle_client_session);
    ...
    case CEPH_SESSION_REQUEST_OPEN:
        if (mds->mdsmap->test_flag(CEPH_MDSMAP_REFUSE_CLIENT_SESSION)) {
            dout(0) << "new sessions are not permitted, enable again via"
                       "`ceph fs set <fs_name> refuse_client_session false`"
                    << dendl;
            auto reply = make_message<MClientSession>(CEPH_SESSION_REJECT);
        ...
        if (session->is_opening() || session->is_open() || session->is_stale() || session->is_killing() ||
            ...
            if (m->supported_features.test(CEPHFS_FEATURE_NOTIFY_SESSION_STATE)) {
                if (session->is_open() && !mds->is_stopping()) {
                    ...
                    auto reply = make_message<MClientSession>(CEPH_SESSION_OPEN, session->get_push_seq());
                    if (session->info.has_feature(CEPHFS_FEATURE_MIMIC)) reply->supported_features = supported_features;
                    mds->send_message_client(reply, session);
            ...
            dout(10) << "currently " << session->get_state_name() << ", dropping this req" << dendl;
            return;
        }
...

        {
            ...
            auto log_session_status = [this, m, session](std::string_view status, std::string_view err) {
                ...
                *css << "New client session:"
                     ...
                     << ",elapsed=" << elapsed << ",throttled=" << throttle_elapsed << ",status=\"" << status << "\"";
                if (!err.empty()) {
                    *css << ",error=\"" << err << "\"";
                }
            ...
            auto send_reject_message = [this, &session, &log_session_status](std::string_view err_str,
                ...
                if (session->info.has_feature(CEPHFS_FEATURE_MIMIC)) m->metadata["error_string"] = err_str;
                mds->send_message_client(m, session);
                log_session_status("REJECTED", err_str);
            };

            bool blocklisted = mds->objecter->with_osdmap(
                [&addr](const OSDMap& osd_map) -> bool { return osd_map.is_blocklisted(addr); });
            ...
            if (client_metadata.features.empty()) infer_supported_features(session, client_metadata);
            ...
            if (!missing_features.empty()) {
                ...
                *css << "missing required features '" << missing_features << "'";
                ...
                mds->clog->warn() << "client session (" << session->info.inst << ") lacks required features "
                                  << missing_features << "; client supports " << client_metadata.features;
                session->clear();
                break;
            }
            ...
            if (auto it = client_metadata.find("root"); it != client_metadata.end()) {
                ...
                if (denied) {
                    ...
                    send_reject_message(css->strv());
                    mds->clog->warn() << "client session with " << css->strv() << " denied (" << session->info.inst
                                      << ")";
                    session->clear();
                    break;
                }
            ...
            if (auto it = client_metadata.find("uuid"); it != client_metadata.end()) {
                if (find_session_by_uuid(it->second)) {
                    ...
                    mds->clog->warn() << "client session with duplicated session uuid '" << it->second << "' denied ("
                                      << session->info.inst << ")";
                    session->clear();
                    break;
                }
    ...
    {
        if (session->is_closed() || session->is_closing() || session->is_killing()) {
        ...
        ceph_assert(session->is_open() || session->is_stale() || session->is_opening());
        ...
        if (m->get_seq() != session->get_push_seq()) {
            ...
            mds->clog->warn() << "incorrect push seq " << m->get_seq() << " != " << session->get_push_seq()
                              << ", dropping"
    ...
    case CEPH_SESSION_FLUSHMSG_ACK: finish_flush_session(session, m->get_seq()); break;

    case CEPH_SESSION_REQUEST_FLUSH_MDLOG:
        if (mds->is_active()) mdlog->flush();
        break;

    default:
        auto m = make_message<MClientSession>(CEPH_SESSION_REJECT);
        mds->send_message_client(m, session);
        derr << "Server received unknown message " << m->get_type() << ", closing session and blocklisting the client "
             << session->get_client() << dendl;
        CachedStackStringStream css;
...
{
    if (!session->is_open() || !session->get_connection() ||
        !session->get_connection()->has_feature(CEPH_FEATURE_EXPORT_PEER)) {
        return;
    }

    version_t seq = session->wait_for_flush(gather.new_sub());
    mds->send_message_client(make_message<MClientSession>(CEPH_SESSION_FLUSHMSG, seq), session);
}
...
{
    dout(10) << "_session_logged " << session->info.inst << " state_seq " << state_seq << " "
             << (open ? "open" : "close") << " " << pv << " inos_to_free " << inos_to_free << " inotablev " << piv
    ...
    if (session->get_state_seq() != state_seq) {
        dout(10) << " journaled state_seq " << state_seq << " != current " << session->get_state_seq() << ", noop"
                 << dendl;
    ...
    else if (session->is_closing() || session->is_killing()) {
 ...
 */
version_t Server::prepare_force_open_sessions(map<client_t, entity_inst_t>& cm, map<client_t, client_metadata_t>& cmm,
                                              map<client_t, pair<Session*, uint64_t>>& smap)
{
    ...
    mds->objecter->with_osdmap([this, &cm, &cmm](const OSDMap& osd_map) {
    ...
    for (map<client_t, entity_inst_t>::iterator p = cm.begin(); p != cm.end(); ++p) {
        ...
        if (session->is_closed() || session->is_closing() || session->is_killing()) {
            ...
            if (q != cmm.end()) session->info.client_metadata.merge(q->second);
        }
        else {
            ceph_assert(session->is_open() || session->is_opening() || session->is_stale());
            sseq = 0;
        }
...
void Server::finish_force_open_sessions(const map<client_t, pair<Session*, uint64_t>>& smap, bool dec_import)
{
    /*
...
{
    void finish(int r) override { server->terminating_sessions = false; }

public:
    explicit C_MDS_TerminatedSessions(Server* s)
        : ServerContext(s)
    {}
};
...
void Server::terminate_sessions()
{
    ...
    for (set<Session*>::const_iterator p = sessions.begin(); p != sessions.end(); ++p) {
        Session* session = *p;
        if (session->is_closing() || session->is_killing() || session->is_closed()) continue;
        journal_close_session(session, Session::STATE_CLOSING, NULL);
    }
...
void Server::find_idle_sessions()
{
    ...
    if (sessions_p1 != mds->sessionmap.by_state.end() && !sessions_p1->second->empty()) {
        ...
        for (auto session : *(sessions_p1->second)) {
            ...
            if (last_cap_renew_span < cutoff) {
                dout(20) << "laggiest active session is " << session->info.inst << " and renewed caps recently ("
            ...
            if (last_cap_renew_span >= mds->mdsmap->get_session_autoclose()) {
                dout(20) << "evicting session " << session->info.inst
                         << " since autoclose "
                            "has arrived"
                         << dendl;
            ...
            if (defer_session_stale && !session->is_any_flush_waiter() &&
                !mds->locker->is_revoking_any_caps_from(session->get_client())) {
                dout(20) << "deferring marking session " << session->info.inst
                         << " stale "
                            "since it holds no caps"
                         << dendl;
                continue;
            }
            ...
            if (it != session->info.client_metadata.end()) {
                ...
                if (last_cap_renew_span < cutoff) {
                    dout(10) << "skipping session " << session->info.inst << ", timeout (" << timeout << ") specified"
                ...
                dout(10) << "new stale session " << session->info.inst << " last renewed caps " << last_cap_renew_span
                         << "s ago" << dendl;
    ...
    if (sessions_p2 != mds->sessionmap.by_state.end() && !sessions_p2->second->empty()) {
        for (auto session : *(sessions_p2->second)) {
            ...
            auto last_cap_renew_span = std::chrono::duration<double>(now - session->last_cap_renew).count();
            if (last_cap_renew_span < cutoff) {
                dout(20) << "oldest stale session is " << session->info.inst << " and recently renewed caps "
                         << last_cap_renew_span << "s ago" << dendl;
                break;
            }
            to_evict.push_back(session);
    ...
    for (auto session : to_evict) {
        ...
        auto last_cap_renew_span = std::chrono::duration<double>(now - session->last_cap_renew).count();
        mds->clog->warn() << "evicting unresponsive client " << *session << ", after " << last_cap_renew_span
                          << " seconds";
        dout(10) << "autoclosing stale session " << session->info.inst << " last renewed caps " << last_cap_renew_span
                 << "s ago" << dendl;
...
void Server::evict_cap_revoke_non_responders()
{
    ...
    for (auto const& client : to_evict) {
        mds->clog->warn() << "client id " << client << " has not responded to"
                          << " cap revoke by MDS for over " << cap_revoke_eviction_timeout << " seconds, evicting";
        dout(1) << __func__ << ": evicting cap revoke non-responder client id " << client << dendl;

        CachedStackStringStream css;
        bool evicted = mds->evict_client(client.v, false, g_conf()->mds_session_blocklist_on_evict, *css, nullptr);
        if (evicted && logger) {
...
void Server::handle_conf_change(const std::set<std::string>& changed)
{
    ...
    if (changed.count("mds_dir_max_entries")) {
        dir_max_entries = g_conf().get_val<uint64_t>("mds_dir_max_entries");
        dout(20) << __func__ << " max entries per directory changed to " << dir_max_entries << dendl;
    }
    if (changed.count("mds_bal_fragment_size_max")) {
        bal_fragment_size_max = g_conf().get_val<int64_t>("mds_bal_fragment_size_max");
        dout(20) << __func__ << " max fragment size changed to " << bal_fragment_size_max << dendl;
    }
...
void Server::kill_session(Session* session, Context* on_safe)
{
    ...
    if ((session->is_opening() || session->is_open() || session->is_stale()) && !session->is_importing()) {
        ...
        if (session->is_closing() || session->is_killing()) {
            if (on_safe) mdlog->wait_for_safe(new MDSInternalContextWrapper(mds, on_safe));
        }
        else {
            ceph_assert(session->is_closed() || session->is_importing());
            if (on_safe) on_safe->complete(0);
        }
    }
}
...
{
    dout(10) << __func__ << " : " << session->info.inst << " pending_prealloc_inos " << session->pending_prealloc_inos
             << " free_prealloc_inos " << session->free_prealloc_inos << " delegated_inos " << session->delegated_inos
             << dendl;
    ...
    auto fin = new C_MDS_session_finish(this,
                                        session,
                                        sseq,
                                        false,
                                        pv,
...
void Server::handle_client_reconnect(const cref_t<MClientReconnect>& m)
{
    ...
    bool deny = false;
    if (reconnect_all_deny || !mds->is_reconnect() || mds->get_want_state() != CEPH_MDS_STATE_RECONNECT ||
        reconnect_evicting) {
        // XXX maybe in the future we can do better than this?
        if (reconnect_all_deny) {
            dout(1) << "mds_deny_all_reconnect was set to speed up reboot phase, ignoring reconnect, sending close"
                    << dendl;
        }
        else {
            dout(1) << "no longer in reconnect state, ignoring reconnect, sending close" << dendl;
        }
        mds->clog->info() << "denied reconnect attempt (mds is " << ceph_mds_state_name(mds->get_state()) << ") from "
        ...
        if (!error_str.empty()) {
            ...
            mds->clog->info() << "denied reconnect attempt from " << m->get_source_inst() << " (" << error_str << ")";
        }
    }
    ...
    for (const auto& r : m->realms) {
        ...
        if (in && in->state_test(CInode::STATE_PURGING)) continue;
        if (in) {
            if (in->snaprealm) {
    ...
    for (const auto& p : m->caps) {
        ...
        if (p.second.capinfo.cap_id > mdcache->last_cap_id) mdcache->last_cap_id = p.second.capinfo.cap_id;
        ...
        if (in && !in->is_auth()) {
            ...
            mdcache->rejoin_export_caps(p.first, from, p.second, in->authority().first, true);
        }
        else {
...
void Server::infer_supported_features(Session* session, client_metadata_t& client_metadata)
{
    ...
    if (supported == -1 && session->get_connection()->has_feature(CEPH_FEATURE_FS_FILE_LAYOUT_V2))
...
void Server::update_required_client_features()
{
    ...
    if (mds->get_state() >= MDSMap::STATE_RECONNECT) {
        ...
        for (auto session : sessions) {
            ...
            missing_features -= session->info.client_metadata.features;
            if (!missing_features.empty()) {
                bool blocklisted = mds->objecter->with_osdmap([session](const OSDMap& osd_map) -> bool {
                ...
                mds->clog->warn() << "evicting session " << *session << ", missing required features '"
                                  << missing_features << "'";
                CachedStackStringStream css;
                mds->evict_client(session->get_client().v, false, g_conf()->mds_session_blocklist_on_evict, *css);
            }
        }
    }
}
...
void Server::reconnect_tick()
{
     ...
     */
    bool reconnect_all_deny = g_conf().get_val<bool>("mds_deny_all_reconnect");
    if (client_reconnect_gather.empty()) return;

    if (reconnect_all_deny && (client_reconnect_gather == client_reconnect_denied)) reject_all_reconnect = true;

    auto now = clock::now();
    auto elapse1 = std::chrono::duration<double>(now - reconnect_start).count();
    if (elapse1 < g_conf()->mds_reconnect_timeout && !reject_all_reconnect) return;
    ...
    for (auto c : client_reconnect_gather) {
        ...
        if (session->last_seen > reconnect_last_seen) reconnect_last_seen = session->last_seen;
    }
    ...
    for (auto session : remaining_sessions) {
        ...
        // killed or reclaimed.
        if (session->info.client_metadata.find("timeout") != session->info.client_metadata.end()) {
        ...
        mds->clog->warn() << "evicting unresponsive client " << *session << ", after waiting " << elapse1
        ...
        if (g_conf()->mds_session_blocklist_on_timeout) {
            CachedStackStringStream css;
            mds->evict_client(session->get_client().v, false, true, *css, gather.new_sub());
        }
        else {
            kill_session(session, NULL);
        }
    ...
    if (gather.has_subs()) {
        ...
        gather.set_finisher(
            new MDSInternalContextWrapper(mds, new LambdaContext([this](int r) { reconnect_gather_finish(); })));
        gather.activate();
...
std::pair<bool, uint64_t> Server::recall_client_state(MDSGatherBuilder* gather, RecallFlags flags)
{
    ...
    const auto recall_global_max_decay_threshold =
        g_conf().get_val<Option::size_t>("mds_recall_global_max_decay_threshold");
    ...
    dout(7) << __func__ << ":"
            << " min=" << min_caps_per_client << " max=" << max_caps_per_client << " total=" << Capability::count()
            << " flags=" << flags << dendl;

    /* trim caps of sessions with the most caps first */
    std::multimap<uint64_t, Session*> caps_session;
    auto f = [&caps_session, enforce_max, enforce_liveness, trim, max_caps_per_client, cache_liveness_magnitude](
                 auto& s) {
        ...
        if (trim || (enforce_max && num_caps > max_caps_per_client) ||
            (enforce_liveness && cache_liveness < (num_caps >> cache_liveness_magnitude))) {
    ...
    for (const auto& [num_caps, session] : boost::adaptors::reverse(caps_session)) {
        if (!session->is_open() || !session->get_connection() || !session->info.inst.name.is_client()) continue;

        dout(10) << __func__ << ":"
                 << " session " << session->info.inst << " caps " << num_caps << ", leases " << session->leases.size()
                 << dendl;
        ...
        if (num_caps > newlim) {
            ...
            if (session_recall_throttle + recall > recall_max_decay_threshold) {
                dout(15) << "  session recall threshold (" << recall_max_decay_threshold << ") hit at "
                         << session_recall_throttle << "; skipping!" << dendl;
                throttled = true;
                continue;
            }
            else if (session_recall_throttle2o + recall > recall_max_caps * 2) {
                dout(15) << "  session recall 2nd-order threshold (" << 2 * recall_max_caps << ") hit at "
                         << session_recall_throttle2o << "; skipping!" << dendl;
                throttled = true;
                continue;
            }
            else if (global_recall_throttle + recall > recall_global_max_decay_threshold) {
                dout(15) << "  global recall threshold (" << recall_global_max_decay_threshold << ") hit at "
                         << global_recall_throttle << "; skipping!" << dendl;
                throttled = true;
                break;
            }
            ...
            if (steady) {
                ...
                if (2 * session_release < session_recall && 2 * session_recall > recall_max_decay_threshold) {
                    ...
                    dout(15) << "  2*session_release < session_recall"
                                " (2*"
                             << session_release << " < " << session_recall
                             << ") &&"
                                " 2*session_recall < recall_max_decay_threshold"
                                " (2*"
                             << session_recall << " > " << recall_max_decay_threshold
                             << ")"
                                " Skipping because we are unlikely to get more released."
                             << dendl;
                    continue;
                }
            ...
            dout(7) << "  recalling " << recall << " caps; session_recall_throttle = " << session_recall_throttle
                    << "; global_recall_throttle = " << global_recall_throttle << dendl;
...
void Server::force_clients_readonly()
{
    ...
    for (set<Session*>::const_iterator p = sessions.begin(); p != sessions.end(); ++p) {
        Session* session = *p;
        if (!session->info.inst.name.is_client() || !(session->is_open() || session->is_stale())) continue;
        mds->send_message_client(make_message<MClientSession>(CEPH_SESSION_FORCE_RO), session);
    }
}
...
void Server::submit_mdlog_entry(LogEvent* le, MDSLogContextBase* fin, MDRequestRef& mdr, std::string_view event)
{
    if (mdr) {
...
void Server::respond_to_request(MDRequestRef& mdr, int r)
{
    if (mdr->client_request) {
        ...
        if (!mdr->internal_op_finish) ceph_abort_msg("trying to respond to internal op without finisher");
...
void Server::perf_gather_op_latency(const cref_t<MClientRequest>& req, utime_t lat)
{
    ...
    switch (req->get_op()) {
    case CEPH_MDS_OP_LOOKUPHASH: code = l_mdss_req_lookuphash_latency; break;
    case CEPH_MDS_OP_LOOKUPINO: code = l_mdss_req_lookupino_latency; break;
    case CEPH_MDS_OP_LOOKUPPARENT: code = l_mdss_req_lookupparent_latency; break;
    case CEPH_MDS_OP_LOOKUPNAME: code = l_mdss_req_lookupname_latency; break;
    case CEPH_MDS_OP_LOOKUP: code = l_mdss_req_lookup_latency; break;
    case CEPH_MDS_OP_LOOKUPSNAP: code = l_mdss_req_lookupsnap_latency; break;
    case CEPH_MDS_OP_GETATTR: code = l_mdss_req_getattr_latency; break;
    case CEPH_MDS_OP_SETATTR: code = l_mdss_req_setattr_latency; break;
    case CEPH_MDS_OP_SETLAYOUT: code = l_mdss_req_setlayout_latency; break;
    case CEPH_MDS_OP_SETDIRLAYOUT: code = l_mdss_req_setdirlayout_latency; break;
    case CEPH_MDS_OP_GETVXATTR: code = l_mdss_req_getvxattr_latency; break;
    case CEPH_MDS_OP_SETXATTR: code = l_mdss_req_setxattr_latency; break;
    case CEPH_MDS_OP_RMXATTR: code = l_mdss_req_rmxattr_latency; break;
    case CEPH_MDS_OP_READDIR: code = l_mdss_req_readdir_latency; break;
    case CEPH_MDS_OP_SETFILELOCK: code = l_mdss_req_setfilelock_latency; break;
    case CEPH_MDS_OP_GETFILELOCK: code = l_mdss_req_getfilelock_latency; break;
    case CEPH_MDS_OP_CREATE: code = l_mdss_req_create_latency; break;
    case CEPH_MDS_OP_OPEN: code = l_mdss_req_open_latency; break;
    case CEPH_MDS_OP_MKNOD: code = l_mdss_req_mknod_latency; break;
    case CEPH_MDS_OP_LINK: code = l_mdss_req_link_latency; break;
    case CEPH_MDS_OP_UNLINK: code = l_mdss_req_unlink_latency; break;
    case CEPH_MDS_OP_RMDIR: code = l_mdss_req_rmdir_latency; break;
    case CEPH_MDS_OP_RENAME: code = l_mdss_req_rename_latency; break;
    case CEPH_MDS_OP_MKDIR: code = l_mdss_req_mkdir_latency; break;
    case CEPH_MDS_OP_SYMLINK: code = l_mdss_req_symlink_latency; break;
    case CEPH_MDS_OP_LSSNAP: code = l_mdss_req_lssnap_latency; break;
    case CEPH_MDS_OP_MKSNAP: code = l_mdss_req_mksnap_latency; break;
    case CEPH_MDS_OP_RMSNAP: code = l_mdss_req_rmsnap_latency; break;
    case CEPH_MDS_OP_RENAMESNAP: code = l_mdss_req_renamesnap_latency; break;
    case CEPH_MDS_OP_READDIR_SNAPDIFF: code = l_mdss_req_snapdiff_latency; break;
    default: dout(1) << ": unknown client op" << dendl; return;
    }
...
{
    if (!g_conf()->mds_early_reply) return;
    ...
    if (client_inst.name.is_mds()) return;
    ...
    dout(10) << "early_reply " << reply->get_result() << " (" << cpp_strerror(reply->get_result()) << ") " << *req
             << dendl;
...
void Server::reply_client_request(MDRequestRef& mdr, const ref_t<MClientReply>& reply)
{
    ...
    dout(7) << "reply_client_request " << reply->get_result() << " (" << cpp_strerror(reply->get_result()) << ") "
            << *req << dendl;
    ...
    if (req->may_write() && req->get_op() != CEPH_MDS_OP_SETFILELOCK && reply->get_result() == 0 && session) {
    ...
    if (session && !client_inst.name.is_mds()) {
        ...
        if (!did_early_reply &&   // don't issue leases if we sent an earlier reply already
            (tracei || tracedn)) {
            if (is_replay) {
                if (tracei) mdcache->try_reconnect_cap(tracei, session);
            }
            else {
    ...
    if (req->is_queued_for_replay() && (mdr->has_completed || reply->get_result() < 0)) {
        if (reply->get_result() < 0) {
            int r = reply->get_result();
            derr << "reply_client_request: failed to replay " << *req << " error " << r << " (" << cpp_strerror(r)
                 << ")" << dendl;
 ...
 */
void Server::set_trace_dist(const ref_t<MClientReply>& reply, CInode* in, CDentry* dn, MDRequestRef& mdr)
{
    // skip doing this for debugging purposes?
    if (g_conf()->mds_inject_traceless_reply_probability && mdr->ls && !mdr->o_trunc &&
    ...
    if (dn) {
        ...
        if (dir->is_complete()) dir->verify_fragstat();
#endif
        DirStat ds;
        ds.frag = dir->get_frag();
        ds.auth = dir->get_dir_auth().first;
        if (dir->is_auth() && !forward_all_requests_to_auth) dir->get_dist_spec(ds.dist, whoami);
    ...
    if (in) {
        ...
        dout(20) << "set_trace_dist added snap " << snapid << " in " << *in << dendl;
...
void Server::handle_client_request(const cref_t<MClientRequest>& req)
{
    ...
    if (mds->logger) mds->logger->inc(l_mds_request);
    if (logger) logger->inc(l_mdss_handle_client_request);
    ...
    if (req->is_a_client()) {
        ...
        if (!session) {
            if (req->is_queued_for_replay()) mds->queue_one_replay();
            return;
        }
    }
    ...
    if (req->is_replay() || req->get_retry_attempt()) {
        ...
        if (session->have_completed_request(req->get_reqid().tid, &created)) {
            ...
            if (req->get_op() != CEPH_MDS_OP_OPEN && req->get_op() != CEPH_MDS_OP_CREATE) {
                dout(10) << " completed request which created new inode " << created << ", convert it to lookup request"
                         << dendl;
        ...
        if (session->trim_completed_requests(req->get_oldest_client_tid())) {
            ...
            if (session->get_num_completed_requests() >=
                ...
                *css << "client." << session->get_client() << " does not advance its oldest_client_tid ("
                     << req->get_oldest_client_tid() << "), " << session->get_num_completed_requests()
    ...
    if (has_completed) mdr->has_completed = true;
...
void Server::dispatch_client_request(MDRequestRef& mdr)
{
    ...
    if (is_full) {
        ...
        if (req->get_op() == CEPH_MDS_OP_SETLAYOUT || req->get_op() == CEPH_MDS_OP_SETDIRLAYOUT ||
            req->get_op() == CEPH_MDS_OP_SETLAYOUT || req->get_op() == CEPH_MDS_OP_RMXATTR ||
            req->get_op() == CEPH_MDS_OP_SETXATTR || req->get_op() == CEPH_MDS_OP_CREATE ||
            req->get_op() == CEPH_MDS_OP_SYMLINK || req->get_op() == CEPH_MDS_OP_MKSNAP ||
            ((req->get_op() == CEPH_MDS_OP_LINK || req->get_op() == CEPH_MDS_OP_RENAME) &&
             (!mdr->has_more() || mdr->more()->witnessed.empty()))   // haven't started peer request
        ) {

            if (check_access(mdr, cur, MAY_FULL)) {
                dout(20) << __func__ << ": full, has FULL caps, permitting op " << ceph_mds_op_name(req->get_op())
                         << dendl;
            }
            else {
                dout(20) << __func__ << ": full, responding CEPHFS_ENOSPC to op " << ceph_mds_op_name(req->get_op())
                         << dendl;
    ...
    case CEPH_MDS_OP_LOOKUPINO: handle_client_lookup_ino(mdr, false, false); break;
    case CEPH_MDS_OP_LOOKUPPARENT: handle_client_lookup_ino(mdr, true, false); break;
    case CEPH_MDS_OP_LOOKUPNAME:
        handle_client_lookup_ino(mdr, false, true);
        break;

        // inodes ops.
    case CEPH_MDS_OP_LOOKUP: handle_client_getattr(mdr, true); break;

    case CEPH_MDS_OP_LOOKUPSNAP:
        // lookupsnap does not reference a CDentry; treat it as a getattr
    case CEPH_MDS_OP_GETATTR: handle_client_getattr(mdr, false); break;
    case CEPH_MDS_OP_GETVXATTR: handle_client_getvxattr(mdr); break;

    case CEPH_MDS_OP_SETATTR: handle_client_setattr(mdr); break;
    case CEPH_MDS_OP_SETLAYOUT: handle_client_setlayout(mdr); break;
    case CEPH_MDS_OP_SETDIRLAYOUT: handle_client_setdirlayout(mdr); break;
    case CEPH_MDS_OP_SETXATTR: handle_client_setxattr(mdr); break;
    case CEPH_MDS_OP_RMXATTR: handle_client_removexattr(mdr); break;

    case CEPH_MDS_OP_READDIR: handle_client_readdir(mdr); break;

    case CEPH_MDS_OP_SETFILELOCK: handle_client_file_setlock(mdr); break;
    ...
    case CEPH_MDS_OP_OPEN:
        ...
        // namespace.
        // no prior locks.
    case CEPH_MDS_OP_MKNOD: handle_client_mknod(mdr); break;
    case CEPH_MDS_OP_LINK: handle_client_link(mdr); break;
    case CEPH_MDS_OP_UNLINK:
    case CEPH_MDS_OP_RMDIR: handle_client_unlink(mdr); break;
    case CEPH_MDS_OP_RENAME: handle_client_rename(mdr); break;
    case CEPH_MDS_OP_MKDIR: handle_client_mkdir(mdr); break;
    case CEPH_MDS_OP_SYMLINK:
        ...
        // snaps
    case CEPH_MDS_OP_LSSNAP: handle_client_lssnap(mdr); break;
    case CEPH_MDS_OP_MKSNAP: handle_client_mksnap(mdr); break;
    case CEPH_MDS_OP_RMSNAP: handle_client_rmsnap(mdr); break;
    case CEPH_MDS_OP_RENAMESNAP: handle_client_renamesnap(mdr); break;
    case CEPH_MDS_OP_READDIR_SNAPDIFF: handle_client_readdir_snapdiff(mdr); break;

    default: dout(1) << " unknown client op " << req->get_op() << dendl; respond_to_request(mdr, -CEPHFS_EOPNOTSUPP);
    }
}
...
void Server::handle_peer_request(const cref_t<MMDSPeerRequest>& m)
{
    ...
    // reply?
    if (m->is_reply()) return handle_peer_request_reply(m);
    ...
    if (m->get_op() == MMDSPeerRequest::OP_RENAMENOTIFY) {
        auto reply =
            make_message<MMDSPeerRequest>(m->get_reqid(), m->get_attempt(), MMDSPeerRequest::OP_RENAMENOTIFYACK);
        mds->send_message(reply, m->get_connection());
        return;
    }
    ...
    if (mdcache->have_request(m->get_reqid())) {
        ...
        if (m->get_op() == MMDSPeerRequest::OP_FINISH) {
            if (m->is_abort()) {
                ...
                if (m->inode_export.length() > 0) mdr->more()->inode_import = m->inode_export;
                // finish off request.
                mdcache->request_finish(mdr);
            }
            return;
    ...
    if (mds->is_clientreplay() && !mds->mdsmap->is_clientreplay(from) && mdr->locks.empty()) {
        dout(3) << "not active yet, waiting" << dendl;
        mds->wait_for_active(new C_MDS_RetryMessage(mds, m));
        return;
    }
...
void Server::handle_peer_request_reply(const cref_t<MMDSPeerRequest>& m)
{
    ...
    if (m->get_attempt() != mdr->attempt) {
        dout(10) << "handle_peer_request_reply " << *mdr << " ignoring reply from other attempt " << m->get_attempt()
                 << dendl;
        return;
    }
    ...
    case MMDSPeerRequest::OP_XLOCKACK:
    {
        ...
        SimpleLock* lock = mds->locker->get_lock(m->get_lock_type(), m->get_object_info());
    ...
    case MMDSPeerRequest::OP_AUTHPINACK: handle_peer_auth_pin_ack(mdr, m); break;

    case MMDSPeerRequest::OP_LINKPREPACK: handle_peer_link_prep_ack(mdr, m); break;

    case MMDSPeerRequest::OP_RMDIRPREPACK: handle_peer_rmdir_prep_ack(mdr, m); break;

    case MMDSPeerRequest::OP_RENAMEPREPACK: handle_peer_rename_prep_ack(mdr, m); break;

    case MMDSPeerRequest::OP_RENAMENOTIFYACK: handle_peer_rename_notify_ack(mdr, m); break;

    default: ceph_abort_msg("unknown op " + to_string(m->get_op()) + " requested");
    }
}
...
void Server::dispatch_peer_request(MDRequestRef& mdr)
{
    ...
    case MMDSPeerRequest::OP_WRLOCK:
    {
        // identify object
        SimpleLock* lock =
            mds->locker->get_lock(mdr->peer_request->get_lock_type(), mdr->peer_request->get_object_info());
        ...
        if (op == MMDSPeerRequest::OP_XLOCK && !lock->get_parent()->is_auth()) {
            dout(10) << "not auth for remote xlock attempt, dropping on " << *lock << " on " << *lock->get_parent()
                     << dendl;
        }
        else {
            ...
            if (replycode == MMDSPeerRequest::OP_XLOCKACK) lock->encode_locked_state(r->get_lock_data());
    ...
    case MMDSPeerRequest::OP_UNWRLOCK:
    {
        ...
        switch (op) {
        case MMDSPeerRequest::OP_UNXLOCK: mds->locker->xlock_finish(it, mdr.get(), &need_issue); break;
        case MMDSPeerRequest::OP_UNWRLOCK: mds->locker->wrlock_finish(it, mdr.get(), &need_issue); break;
        }
        if (need_issue) mds->locker->issue_caps(static_cast<CInode*>(lock->get_parent()));

        // done.  no ack necessary.
        mdr->reset_peer_request();
    } break;

    case MMDSPeerRequest::OP_AUTHPIN: handle_peer_auth_pin(mdr); break;

    case MMDSPeerRequest::OP_LINKPREP:
    case MMDSPeerRequest::OP_UNLINKPREP: handle_peer_link_prep(mdr); break;

    case MMDSPeerRequest::OP_RMDIRPREP: handle_peer_rmdir_prep(mdr); break;

    case MMDSPeerRequest::OP_RENAMEPREP: handle_peer_rename_prep(mdr); break;

    default: ceph_abort_msg("unknown op " + to_string(op) + " received");
    }
}
...
void Server::handle_peer_auth_pin(MDRequestRef& mdr)
{
    ...
    if (!fail) {
        for (const auto& oi : mdr->peer_request->get_authpins()) {
            ...
            if (oi == mdr->peer_request->get_authpin_freeze()) auth_pin_freeze = static_cast<CInode*>(object);
        }
    }
    ...
    if (!fail) {
        for (const auto& obj : objects) {
            ...
            if (mdr->is_auth_pinned(obj)) continue;
    ...
    if (!fail) {
        ...
        if (mdr->has_more() && mdr->more()->is_freeze_authpin && mdr->more()->rename_inode != auth_pin_freeze)
    ...
    if (fail) {
        ...
        if (readonly) reply->mark_error_rofs();
        if (wouldblock) reply->mark_error_wouldblock();
    }
    else {
        // auth pin!
        for (const auto& obj : objects) {
        ...
        for (const auto& p : mdr->object_states) {
            if (!p.second.auth_pinned) continue;
...
void Server::handle_peer_auth_pin_ack(MDRequestRef& mdr, const cref_t<MMDSPeerRequest>& ack)
{
    ...
    for (const auto& oi : ack->get_authpins()) {
        ...
        if (oi == ack->get_authpin_freeze()) mdr->set_remote_frozen_auth_pin(static_cast<CInode*>(object));
        pinned.insert(object);
    }

    // removed frozen auth pin ?
    if (mdr->more()->is_remote_frozen_authpin && ack->get_authpin_freeze() == MDSCacheObjectInfo()) {
    ...
    for (auto& p : mdr->object_states) {
        if (p.second.remote_auth_pinned == MDS_RANK_NONE) continue;
...
{
    if (mdr->session) {
        int r = mdr->session->check_access(in,
                                           mask,
...
{
    const uint64_t size =
        in->inode->get_projected_inode()->dirstat.nfiles + in->inode->get_projected_inode()->dirstat.nsubdirs;
...
CDentry* Server::prepare_stray_dentry(MDRequestRef& mdr, CInode* in)
{
    ...
    if (!mdr->client_request->is_replay() && !check_fragment_space(mdr, straydir)) return nullptr;
                                  ...
                                  const file_layout_t* layout)
{
    ...
    do {
        if (allow_prealloc_inos && (mdr->used_prealloc_ino = _inode->ino = mdr->session->take_ino(_useino))) {
            ...
            if (mdcache->test_and_clear_taken_inos(_inode->ino)) {
                ...
                _inode->ino = 0;
                dout(10) << "prepare_new_inode alloc " << mdr->alloc_ino << " but has been taken, will try again!"
                         << dendl;
            }
            else {
    ...
    if (useino && useino != _inode->ino) {
        ...
        mds->clog->error() << mdr->client_request->get_source() << " specified ino " << useino << " but mds."
                           << mds->get_nodeid() << " allocated " << _inode->ino;
    ...
    _inode->btime = _inode->ctime = _inode->mtime = _inode->atime = mdr->get_op_stamp();
...
{
    dout(20) << "journal_allocated_inos sessionmapv " << mds->sessionmap.get_projected() << " inotablev "
             << mds->inotable->get_projected_version() << dendl;
...
{
    dout(10) << "apply_allocated_inos " << mdr->alloc_ino << " / " << mdr->prealloc_inos << " / "
...
struct C_MDS_TryOpenInode : public ServerContext
{
    ...
    C_MDS_TryOpenInode(Server* s, MDRequestRef& r, inodeno_t i)
        ...
        , ino(i)
    {}
    void finish(int r) override { server->_try_open_ino(mdr, r, ino); }
};
...
void Server::_try_open_ino(MDRequestRef& mdr, int r, inodeno_t ino)
{
    ...
    // give up
    if (r == -CEPHFS_ENOENT || r == -CEPHFS_ENODATA) r = -CEPHFS_ESTALE;
    respond_to_request(mdr, r);
}
...
CInode* Server::rdlock_path_pin_ref(MDRequestRef& mdr, bool want_auth, bool no_want_auth)
{
    const filepath& refpath = mdr->get_filepath();
    dout(10) << "rdlock_path_pin_ref " << *mdr << " " << refpath << dendl;

    if (mdr->locking_state & MutationImpl::PATH_LOCKED) return mdr->in[0];

    // traverse
    CF_MDS_RetryRequestFactory cf(mdcache, mdr, true);
    int flags = 0;
    if (refpath.is_last_snap()) {
        if (!no_want_auth) want_auth = true;
    }
    else {
        if (!no_want_auth && forward_all_requests_to_auth) want_auth = true;
        flags |= MDS_TRAVERSE_RDLOCK_PATH | MDS_TRAVERSE_RDLOCK_SNAP;
    }
    if (want_auth) flags |= MDS_TRAVERSE_WANT_AUTH;
    int r = mdcache->path_traverse(mdr, cf, refpath, flags, &mdr->dn[0], &mdr->in[0]);
    if (r > 0) return nullptr;   // delayed
    if (r < 0) {                 // error
        if (r == -CEPHFS_ENOENT && !mdr->dn[0].empty()) {
            if (mdr->client_request && mdr->client_request->get_dentry_wanted()) mdr->tracedn = mdr->dn[0].back();
            respond_to_request(mdr, r);
        }
    ...
    if (want_auth) {
        ...
        if (ref->is_frozen() || ref->is_frozen_auth_pin() || (ref->is_freezing() && !mdr->is_auth_pinned(ref))) {
            dout(7) << "waiting for !frozen/authpinnable on " << *ref << dendl;
            ref->add_waiter(CInode::WAIT_UNFREEZE, cf.build());
            if (mdr->is_any_remote_auth_pin()) mds->locker->notify_freeze_waiter(ref);
            return 0;
        }
        mdr->auth_pin(ref);
    }
 ...
 */
CDentry* Server::rdlock_path_xlock_dentry(MDRequestRef& mdr, bool create, bool okexist, bool authexist,
                                          bool want_layout)
{
    const filepath& refpath = mdr->get_filepath();
    dout(10) << "rdlock_path_xlock_dentry " << *mdr << " " << refpath << dendl;

    if (mdr->locking_state & MutationImpl::PATH_LOCKED) return mdr->dn[0].back();
    ...
    CF_MDS_RetryRequestFactory cf(mdcache, mdr, true);
    int flags = MDS_TRAVERSE_RDLOCK_SNAP | MDS_TRAVERSE_RDLOCK_PATH | MDS_TRAVERSE_WANT_DENTRY |
                MDS_TRAVERSE_XLOCK_DENTRY | MDS_TRAVERSE_WANT_AUTH;
    if (refpath.depth() == 1 && !mdr->lock_cache_disabled) flags |= MDS_TRAVERSE_CHECK_LOCKCACHE;
    if (create) flags |= MDS_TRAVERSE_RDLOCK_AUTHLOCK;
    if (authexist) flags |= MDS_TRAVERSE_WANT_INODE;
    if (want_layout) flags |= MDS_TRAVERSE_WANT_DIRLAYOUT;
    ...
    if (!mdr->reqid.name.is_mds()) {
        if (diri->is_system() && !diri->is_root() &&
            (!diri->is_lost_and_found() || mdr->client_request->get_op() != CEPH_MDS_OP_UNLINK)) {
            respond_to_request(mdr, -CEPHFS_EROFS);
            return nullptr;
        }
    }
 ...
 *    order.
 */
std::pair<CDentry*, CDentry*> Server::rdlock_two_paths_xlock_destdn(MDRequestRef& mdr, bool xlock_srcdn)
{
    ...
    if (mdr->locking_state & MutationImpl::PATH_LOCKED) return std::make_pair(mdr->dn[0].back(), mdr->dn[1].back());
    ...
    if (!destdir->get_inode()->is_base() && destdir->get_inode()->get_projected_parent_dir()->inode->is_stray()) {
        respond_to_request(mdr, -CEPHFS_ENOENT);
        return std::make_pair(nullptr, nullptr);
    }
    ...
    if (srcdir->get_inode() == destdir->get_inode()) {
        ...
        if (srcdn->get_name() > destdn->get_name()) lov.add_xlock(&destdn->lock);
        ...
        bool lock_destdir_first = (cmp < 0 || (cmp == 0 && destdir->ino() < srcdir->ino()));
    ...
    if (!mds->locker->acquire_locks(mdr, lov, auth_pin_freeze)) return std::make_pair(nullptr, nullptr);
...
void Server::handle_client_getattr(MDRequestRef& mdr, bool is_lookup)
{
    ...
    if (mask & CEPH_STAT_RSTAT) want_auth = true;   // set want_auth for CEPH_STAT_RSTAT mask

    if (!mdr->is_batch_head() && mdr->can_batch()) {
        CF_MDS_RetryRequestFactory cf(mdcache, mdr, false);
        int r = mdcache->path_traverse(
            mdr, cf, mdr->get_filepath(), (want_auth ? MDS_TRAVERSE_WANT_AUTH : 0), &mdr->dn[0], &mdr->in[0]);
        ...
        if (r < 0) {
            ...
            mdr->pin(dn);
            auto em =
                dn->batch_ops.emplace(std::piecewise_construct, std::forward_as_tuple(mask), std::forward_as_tuple());
            if (em.second) {
            ...
            mdr->pin(in);
            auto em =
                in->batch_ops.emplace(std::piecewise_construct, std::forward_as_tuple(mask), std::forward_as_tuple());
            if (em.second) {
                em.first->second = std::make_unique<Batch_Getattr_Lookup>(this, mdr);
            }
            else {
                dout(20) << __func__ << ": GETATTR op, wait for previous same getattr ops to respond. " << *mdr
                         << dendl;
                em.first->second->add_request(mdr);
    ...
    if (cap && (mdr->snapid == CEPH_NOSNAP || mdr->snapid <= cap->client_follows)) issued = cap->issued();

    // FIXME
    MutationImpl::LockOpVec lov;
    if ((mask & CEPH_CAP_LINK_SHARED) && !(issued & CEPH_CAP_LINK_EXCL)) lov.add_rdlock(&ref->linklock);
    if ((mask & CEPH_CAP_AUTH_SHARED) && !(issued & CEPH_CAP_AUTH_EXCL)) lov.add_rdlock(&ref->authlock);
    if ((mask & CEPH_CAP_XATTR_SHARED) && !(issued & CEPH_CAP_XATTR_EXCL)) lov.add_rdlock(&ref->xattrlock);
    if ((mask & CEPH_CAP_FILE_SHARED) && !(issued & CEPH_CAP_FILE_EXCL)) {
        ...
        if (mdr->is_rdlocked(&ref->filelock)) {
        ...
        else if (ref->filelock.is_stable() || ref->filelock.get_num_wrlocks() > 0 ||
             ...
             */
            if (mdr->is_rdlocked(&ref->linklock) || mdr->is_rdlocked(&ref->authlock) ||
    ...
    if (!check_access(mdr, ref, MAY_READ)) return;
    ...
    mdr->tracei = ref;
    if (is_lookup) mdr->tracedn = mdr->dn[0].back();
...
{
    MDRequestRef mdr;
    C_MDS_LookupIno2(Server* s, MDRequestRef& r)
        : ServerContext(s)
        , mdr(r)
    {}
    void finish(int r) override { server->_lookup_ino_2(mdr, r); }
};

/*
 * filepath:  ino
 */
void Server::handle_client_lookup_ino(MDRequestRef& mdr, bool want_parent, bool want_dentry)
{
    const cref_t<MClientRequest>& req = mdr->client_request;
    ...
    // path check.
    if (!check_access(mdr, in, 0)) return;
    ...
    if (mask) {
        ...
        if ((mask & CEPH_CAP_AUTH_SHARED) && (issued & CEPH_CAP_AUTH_EXCL) == 0) lov.add_rdlock(&in->authlock);
        if ((mask & CEPH_CAP_XATTR_SHARED) && (issued & CEPH_CAP_XATTR_EXCL) == 0) lov.add_rdlock(&in->xattrlock);
    ...
    if (!lov.empty()) {
        ...
        if (diri != NULL) {
            ...
            if (!check_access(mdr, diri, MAY_READ)) return;
        }
    }
...
void Server::_lookup_snap_ino(MDRequestRef& mdr)
{
    ...
    if (!in) {
        ...
        if (in) {
            if (in->state_test(CInode::STATE_PURGING) || !in->has_snap_data(vino.snapid)) {
...
void Server::handle_client_open(MDRequestRef& mdr)
{
    ...
    bool need_auth = !file_mode_is_readonly(cmode) || (flags & (CEPH_O_TRUNC | CEPH_O_DIRECTORY));
    ...
    if (!cur->is_file()) {
        ...
        if (cur->is_symlink() && !(flags & CEPH_O_NOFOLLOW)) flags &= ~CEPH_O_TRUNC;
    }
    ...
    if (mdr->snapid != CEPH_NOSNAP && ((cmode & CEPH_FILE_MODE_WR) || req->may_write())) {
    ...
    if (mask) {
        ...
        if (cap && (mdr->snapid == CEPH_NOSNAP || mdr->snapid <= cap->client_follows)) issued = cap->issued();
        // permission bits, ACL/security xattrs
        if ((mask & CEPH_CAP_AUTH_SHARED) && (issued & CEPH_CAP_AUTH_EXCL) == 0) lov.add_rdlock(&cur->authlock);
        if ((mask & CEPH_CAP_XATTR_SHARED) && (issued & CEPH_CAP_XATTR_EXCL) == 0) lov.add_rdlock(&cur->xattrlock);
    ...
    if ((flags & CEPH_O_TRUNC) && !mdr->has_completed) {
        ...
        if (!check_access(mdr, cur, MAY_WRITE)) return;

        // wait for pending truncate?
        const auto& pi = cur->get_projected_inode();
        if (pi->is_truncating()) {
            dout(10) << " waiting for pending truncate from " << pi->truncate_from << " to " << pi->truncate_size
                     << " to complete on " << *cur << dendl;
            mds->locker->drop_locks(mdr.get());
            mdr->drop_local_auth_pins();
    ...
    mask = MAY_READ;
    if (cmode & CEPH_FILE_MODE_WR) mask |= MAY_WRITE;
    if (!check_access(mdr, cur, mask)) return;
    ...
    if (cur->is_file() || cur->is_dir()) {
        if (mdr->snapid == CEPH_NOSNAP) {
            ...
            if (cap)
                dout(12) << "open issued caps " << ccap_string(cap->pending()) << " for " << req->get_source() << " on "
                         << *cur << dendl;
        }
        else {
            int caps = ceph_caps_for_mode(cmode);
            dout(12) << "open issued IMMUTABLE SNAP caps " << ccap_string(caps) << " for " << req->get_source()
                     << " snapid " << mdr->snapid << " on " << *cur << dendl;
            mdr->snap_caps = caps;
        }
    }

    // increase max_size?
    if (cmode & CEPH_FILE_MODE_WR) mds->locker->check_inode_max_size(cur);

    // make sure this inode gets into the journal
    if (cur->is_auth() && cur->last == CEPH_NOSNAP && mdcache->open_file_table.should_log_open(cur)) {
...
void Server::handle_client_openc(MDRequestRef& mdr)
{
    ...
    if (req->head.args.open.stripe_unit) layout.stripe_unit = req->head.args.open.stripe_unit;
    if (req->head.args.open.stripe_count) layout.stripe_count = req->head.args.open.stripe_count;
    if (req->head.args.open.object_size) layout.object_size = req->head.args.open.object_size;
    if (req->get_connection()->has_feature(CEPH_FEATURE_CREATEPOOLID) && (__s32)req->head.args.open.pool >= 0) {
    ...
    CDir* dir = dn->get_dir();
    CInode* diri = dir->get_inode();
    if (!check_access(mdr, diri, access)) return;
    if (!check_fragment_space(mdr, dir)) return;
    if (!check_dir_max_entries(mdr, dir)) return;

    if (mdr->dn[0].size() == 1) mds->locker->create_lock_cache(mdr, diri, &mdr->dir_layout);

    // create inode.
    CInode* newi =
        prepare_new_inode(mdr, dn->get_dir(), inodeno_t(req->head.ino), req->head.args.open.mode | S_IFREG, &layout);
    ceph_assert(newi);
...
void Server::_finalize_readdir(MDRequestRef& mdr, CInode* diri, CDir* dir, bool start, bool end, __u16 flags,
                               __u32 numfiles, bufferlist& dirbl, bufferlist& dnbl)
{
    const cref_t<MClientRequest>& req = mdr->client_request;
    Session* session = mds->get_session(req);
    ...
    if (end) {
        ...
        if (start) flags |= CEPH_READDIR_FRAG_COMPLETE;   // FIXME: what purpose does this serve
    }
    ...
    // yay, reply
    dout(10) << "reply to " << *req << " readdir num=" << numfiles << " bytes=" << dirbl.length()
             << " start=" << (int)start << " end=" << (int)end << dendl;
...
void Server::handle_client_readdir(MDRequestRef& mdr)
{
    ...
    if (num_caps > static_cast<uint64_t>(max_caps_per_client * max_caps_throttle_ratio) &&
        ...
        dout(20) << "readdir throttled. max_caps_per_client: " << max_caps_per_client << " num_caps: " << num_caps
                 << " session_cap_acquistion: " << session_cap_acquisition
                 << " cap_acquisition_throttle: " << cap_acquisition_throttle << dendl;
        if (logger) logger->inc(l_mdss_cap_acquisition_throttle);
    ...
    if (!max) max = dir->get_num_any();   // whatever, something big.
    ...
    ds.frag = dir->get_frag();
    ds.auth = dir->get_dir_auth().first;
    if (dir->is_auth() && !forward_all_requests_to_auth) dir->get_dist_spec(ds.dist, mds->get_nodeid());
    ...
    for (; !end && numfiles < max; end = (it == dir->end())) {
        ...
        if (dn->state_test(CDentry::STATE_PURGING)) continue;

        bool dnp = dn->use_projected(client, mdr);
        CDentry::linkage_t* dnl = dnp ? dn->get_projected_linkage() : dn->get_linkage();

        if (dnl->is_null()) {
            if (dn->get_num_ref() == 0 && !dn->is_projected()) dir->remove_dentry(dn);
            continue;
        }
        ...
        if (!start) {
            ...
            if (!(offset_key < dn->key())) continue;
        }

        CInode* in = dnl->get_inode();

        if (in && in->ino() == CEPH_INO_CEPH) continue;
        ...
        if (dnl->is_remote() && !in) {
            ...
            if (in) {
                ...
                for (auto& p : *dir) {
                    if (!p.second->get_linkage()->is_null()) mdcache->lru.lru_touch(p.second);
                }

                // already issued caps and leases, reply immediately.
                if (dnbl.length() > 0) {
                    mdcache->open_remote_dentry(dn, dnp, new C_MDSInternalNoop);
                    dout(10) << " open remote dentry after caps were issued, stopping at " << dnbl.length() << " < "
                             << bytes_left << dendl;
                    break;
                }
...
public:
    C_MDS_inode_update_finish(Server* s, MDRequestRef& r, CInode* i, bool sm = false, bool cr = false, bool ar = false)
        : ServerLogContext(s, r)
        , in(i)
    ...
    void finish(int r) override
    {
        ...
        if (changed_ranges) get_mds()->locker->share_inode_max_size(in);
    }
};
...
void Server::handle_client_file_setlock(MDRequestRef& mdr)
{
    ...
    case CEPH_LOCK_FLOCK_INTR:
        interrupt = true;
        // fall-thru
    case CEPH_LOCK_FLOCK: lock_state = cur->get_flock_lock_state(); break;

    case CEPH_LOCK_FCNTL_INTR:
        interrupt = true;
        // fall-thru
    case CEPH_LOCK_FCNTL: lock_state = cur->get_fcntl_lock_state(); break;
    ...
    if (CEPH_LOCK_UNLOCK == set_lock.type) {
        ...
        bool deadlock = false;
        if (mdr->more()->flock_was_waiting && !lock_state->is_waiting(set_lock)) {
...
void Server::handle_client_setattr(MDRequestRef& mdr)
{
    ...
    if (mask & (CEPH_SETATTR_MODE | CEPH_SETATTR_UID | CEPH_SETATTR_GID | CEPH_SETATTR_BTIME | CEPH_SETATTR_KILL_SGUID |
                CEPH_SETATTR_FSCRYPT_AUTH | CEPH_SETATTR_KILL_SUID | CEPH_SETATTR_KILL_SGID))
        lov.add_xlock(&cur->authlock);
    if (mask & (CEPH_SETATTR_MTIME | CEPH_SETATTR_ATIME | CEPH_SETATTR_SIZE | CEPH_SETATTR_FSCRYPT_FILE))
        lov.add_xlock(&cur->filelock);
    if (mask & CEPH_SETATTR_CTIME) lov.add_wrlock(&cur->versionlock);

    if (!mds->locker->acquire_locks(mdr, lov)) return;

    if ((mask & CEPH_SETATTR_UID) && (cur->get_inode()->uid != req->head.args.setattr.uid)) access_mask |= MAY_CHOWN;

    if ((mask & CEPH_SETATTR_GID) && (cur->get_inode()->gid != req->head.args.setattr.gid)) access_mask |= MAY_CHGRP;

    if (!check_access(mdr, cur, access_mask)) return;
    ...
    if (mask & CEPH_SETATTR_SIZE) {
        if (req->get_data().length() > sizeof(struct ceph_fscrypt_last_block_header) + fscrypt_last_block_max_size) {
        ...
        if (truncating_smaller && pip->is_truncating()) {
            dout(10) << " waiting for pending truncate from " << pip->truncate_from << " to " << pip->truncate_size
                     << " to complete on " << *cur << dendl;
            mds->locker->drop_locks(mdr.get());
            mdr->drop_local_auth_pins();
            cur->add_waiter(CInode::WAIT_TRUNC, new C_MDS_RetryRequest(mdcache, mdr));
        ...
        if (truncating_smaller && req->get_data().length()) {
            ...
            dout(20) << __func__ << " mdr->retry:" << mdr->retry << " header.change_attr: " << header.change_attr
                     << " header.file_offset: " << header.file_offset << " header.block_size: " << header.block_size
                     << dendl;

            if (header.change_attr != pip->change_attr) {
                dout(5) << __func__ << ": header.change_attr:" << header.change_attr
                        << " != current change_attr:" << pip->change_attr << ", let client retry it!" << dendl;
    ...
    if (mask & CEPH_SETATTR_UID) pi.inode->uid = req->head.args.setattr.uid;
    if (mask & CEPH_SETATTR_GID) pi.inode->gid = req->head.args.setattr.gid;

    if (mask & CEPH_SETATTR_MODE)
        pi.inode->mode = (pi.inode->mode & ~07777) | (req->head.args.setattr.mode & 07777);
    else if ((mask & (CEPH_SETATTR_UID | CEPH_SETATTR_GID | CEPH_SETATTR_KILL_SGUID | CEPH_SETATTR_KILL_SUID |
    ...
    if (mask & CEPH_SETATTR_MTIME) pi.inode->mtime = req->head.args.setattr.mtime;
    if (mask & CEPH_SETATTR_ATIME) pi.inode->atime = req->head.args.setattr.atime;
    if (mask & CEPH_SETATTR_BTIME) pi.inode->btime = req->head.args.setattr.btime;
    ...
    if (mask & CEPH_SETATTR_FSCRYPT_AUTH) pi.inode->fscrypt_auth = req->fscrypt_auth;
    if (mask & CEPH_SETATTR_FSCRYPT_FILE) pi.inode->fscrypt_file = req->fscrypt_file;
    ...
    journal_and_reply(
        mdr, cur, 0, le, new C_MDS_inode_update_finish(this, mdr, cur, truncating_smaller, changed_ranges));

    // flush immediately if there are readers/writers waiting
    if (mdr->is_xlocked(&cur->filelock) && (cur->get_caps_wanted() & (CEPH_CAP_FILE_RD | CEPH_CAP_FILE_WR)))
        mds->mdlog->flush();
}
...
void Server::do_open_truncate(MDRequestRef& mdr, int cmode)
{
    ...
    journal_and_reply(mdr, in, dn, le, new C_MDS_inode_update_finish(this, mdr, in, old_size > 0, changed_ranges));
...
void Server::handle_client_setlayout(MDRequestRef& mdr)
{
    ...
    if (cur->get_projected_inode()->size || cur->get_projected_inode()->truncate_seq > 1) {
        respond_to_request(mdr, -CEPHFS_ENOTEMPTY);
        return;
    }
    ...
    if (!check_access(mdr, cur, access)) return;
...
{
    if (mdr->locking_state & MutationImpl::ALL_LOCKED) return true;
    ...
    if (!mds->locker->acquire_locks(mdr, lov)) return false;
    ...
    if (CDentry* pdn = in->get_projected_parent_dn(); pdn) {
        if (!mds->locker->try_rdlock_snap_layout(pdn->get_dir()->get_inode(), mdr, 0, want_layout)) return false;
    }
...
void Server::handle_client_setdirlayout(MDRequestRef& mdr)
{
    ...
    if (!xlock_policylock(mdr, cur, true)) return;
...
// XATTRS
int Server::parse_layout_vxattr_json(string name, string value, const OSDMap& osdmap, file_layout_t* layout)
{
    ...
    try {
        if (name == "layout.json") {
            ...
            if (json_parser.parse(value.c_str(), value.length()) and json_parser.is_object()) {
                ...
                try {
                    ...
                    dout(10) << __func__ << ": json is missing a mandatory field named " << field << dendl;
                    return -CEPHFS_EINVAL;
                }
            }
            else {
        ...
        dout(10) << __func__ << ": bad vxattr value:" << value << ", unable to parse for xattr:" << name << dendl;
        return -CEPHFS_EINVAL;
    }
    return 0;
}
...
int Server::parse_layout_vxattr_string(string name, string value, const OSDMap& osdmap, file_layout_t* layout)
{
    try {
        if (name == "layout") {
            ...
            for (map<string, string>::iterator q = m.begin(); q != m.end(); ++q) {
                ...
                int r = parse_layout_vxattr_string(string("layout.") + q->first, q->second, osdmap, layout);
                if (r < 0) return r;
            }
        }
...
int Server::parse_layout_vxattr(string name, string value, const OSDMap& osdmap, file_layout_t* layout, bool validate)
{
 ...
 */
int Server::check_layout_vxattr(MDRequestRef& mdr, string name, string value, file_layout_t* layout)
{
    const cref_t<MClientRequest>& req = mdr->client_request;
    epoch_t epoch;
    int r;
    ...
    if (r == -CEPHFS_ENOENT) {
        ...
        if (req_epoch > epoch) {
            ...
            mds->objecter->wait_for_latest_osdmap(
                std::ref(*new C_IO_Wrapper(mds, new C_MDS_RetryRequest(mdcache, mdr))));
            return r;
        }
    }
    ...
    if (r < 0) {
        ...
        if (r == -CEPHFS_ENOENT) r = -CEPHFS_EINVAL;
...
void Server::handle_set_vxattr(MDRequestRef& mdr, CInode* cur)
{
    ...
    if (name.compare(0, 15, "ceph.dir.layout") == 0) {
        ...
        if (check_layout_vxattr(mdr, rest, value, &layout) < 0) return;
        ...
        if (quota.is_enabled() && !cur->get_projected_srnode()) adjust_realm = true;

        if (!xlock_policylock(mdr, cur, false, adjust_realm)) return;
        ...
        if (adjust_realm) pi.snapnode->created = pi.snapnode->seq = cur->find_snaprealm()->get_newest_seq();
        ...
        if ((mdr->locking_state & MutationImpl::ALL_LOCKED) && !mdr->is_xlocked(&cur->snaplock)) {
            ...
            mds->locker->drop_locks(mdr.get());
            if (!xlock_policylock(mdr, cur, false, true)) return;
        }
        ...
        if (!srnode) pi.snapnode->created = pi.snapnode->seq = realm->get_newest_seq();
        if (val)
        ...
        if (!xlock_policylock(mdr, cur)) return;
    ...
    if (mdr->get_op_stamp() > pip->rstat.rctime) pip->rstat.rctime = mdr->get_op_stamp();
    pip->version = cur->pre_dirty();
    if (cur->is_file()) pip->update_backtrace();
    ...
    journal_and_reply(mdr, cur, 0, le, new C_MDS_inode_update_finish(this, mdr, cur, false, false, adjust_realm));
    return;
}
...
void Server::handle_remove_vxattr(MDRequestRef& mdr, CInode* cur)
{
    ...
    else if (name == "ceph.dir.layout.pool_namespace" || name == "ceph.file.layout.pool_namespace") {
                       ...
                       const bufferlist& xattr_value)
{
    ...
    auto em = xattrs->emplace(
        std::piecewise_construct, std::forward_as_tuple(mempool::mds_co::string(xattr_name)), std::forward_as_tuple(b));
    if (!em.second) {
        em.first->second = b;
    }
}
...
int Server::default_xattr_validate(CInode* cur, const InodeStoreBase::xattr_map_const_ptr xattrs, XattrOp* xattr_op)
{
    return xattr_validate(cur, xattrs, xattr_op->xattr_name, xattr_op->op, xattr_op->flags);
}

void Server::default_setxattr_handler(CInode* cur, InodeStoreBase::xattr_map_ptr xattrs, const XattrOp& xattr_op)
{
    xattr_set(xattrs, xattr_op.xattr_name, xattr_op.xattr_value);
}

void Server::default_removexattr_handler(CInode* cur, InodeStoreBase::xattr_map_ptr xattrs, const XattrOp& xattr_op)
{
...
int Server::parse_mirror_info_xattr(const std::string& name, const std::string& value, std::string& cluster_id,
                                    std::string& fs_id)
{
...
int Server::mirror_info_xattr_validate(CInode* cur, const InodeStoreBase::xattr_map_const_ptr xattrs, XattrOp* xattr_op)
{
    if (!cur->is_root()) {
        return -CEPHFS_EINVAL;
    }
    ...
    std::string fs_id;
    int r = parse_mirror_info_xattr(xattr_op->xattr_name, xattr_op->xattr_value.to_str(), cluster_id, fs_id);
    if (r < 0) {
        return r;
    }
...
void Server::mirror_info_setxattr_handler(CInode* cur, InodeStoreBase::xattr_map_ptr xattrs, const XattrOp& xattr_op)
{
...
void Server::mirror_info_removexattr_handler(CInode* cur, InodeStoreBase::xattr_map_ptr xattrs, const XattrOp& xattr_op)
{
...
void Server::handle_client_getvxattr(MDRequestRef& mdr)
{
    ...
    if ((xattr_name.substr(0, 15) == "ceph.dir.layout"sv) || (xattr_name.substr(0, 16) == "ceph.file.layout"sv)) {
        ...
        if (xattr_name == "ceph.dir.layout.json"sv || xattr_name == "ceph.file.layout.json"sv) {
            // fetch layout only for valid xattr_name
            const auto lxi = get_inherited_layout(cur);

            *css << "{\"stripe_unit\": " << lxi.layout.stripe_unit << ", \"stripe_count\": " << lxi.layout.stripe_count
                 << ", \"object_size\": " << lxi.layout.object_size << ", \"pool_name\": ";
            ...
            *css << ", \"inheritance\": \"@" << layout_xattr_info_t::status_to_string(lxi.status) << "\"}";
        }
        else if ((xattr_name == "ceph.dir.layout.pool_name"sv) || (xattr_name == "ceph.file.layout.pool_name"sv)) {
            // fetch layout only for valid xattr_name
            const auto lxi = get_inherited_layout(cur);
            mds->objecter->with_osdmap([lxi, &css](const OSDMap& o) {
                if (o.have_pg_pool(lxi.layout.pool_id)) {
        ...
        else if ((xattr_name == "ceph.dir.layout.pool_id"sv) || (xattr_name == "ceph.file.layout.pool_id"sv)) {
            // fetch layout only for valid xattr_name
            const auto lxi = get_inherited_layout(cur);
...
void Server::handle_client_link(MDRequestRef& mdr)
{
    ...
    if (req->get_filepath2().depth() == 0) {
        ...
        if (!(mdr->locking_state & MutationImpl::SNAP2_LOCKED)) {
            ...
            if (!mds->locker->try_rdlock_snap_layout(pdn->get_dir()->get_inode(), mdr, 1)) return;
    ...
    if ((!mdr->has_more() || mdr->more()->witnessed.empty())) {
        if (!check_access(mdr, targeti, MAY_WRITE)) return;
    ...
    if (target_pin != dir->inode &&
        target_realm->get_subvolume_ino() != dir->inode->find_snaprealm()->get_subvolume_ino() &&
...
public:
    C_MDS_link_local_finish(Server* s, MDRequestRef& r, CDentry* d, CInode* ti, version_t dnpv_, version_t tipv_,
                            bool ar)
        : ServerLogContext(s, r)
        , dn(d)
        , targeti(ti)
...
void Server::_link_local(MDRequestRef& mdr, CDentry* dn, CInode* targeti, SnapRealm* target_realm)
{
    ...
    journal_and_reply(
        mdr, targeti, dn, le, new C_MDS_link_local_finish(this, mdr, dn, targeti, dnpv, tipv, adjust_realm));
}

void Server::_link_local_finish(MDRequestRef& mdr, CDentry* dn, CInode* targeti, version_t dnpv, version_t tipv,
                                bool adjust_realm)
{
...
void Server::_link_remote(MDRequestRef& mdr, bool inc, CDentry* dn, CInode* targeti)
{
    ...
    if (mdr->more()->witnessed.count(linkauth) == 0) {
        ...
        if (auto& desti_srnode = mdr->more()->desti_srnode) encode(*desti_srnode, req->desti_snapbl);
...
void Server::_link_remote_finish(MDRequestRef& mdr, bool inc, CDentry* dn, CInode* targeti, version_t dpv)
{
    dout(10) << "_link_remote_finish " << (inc ? "link " : "unlink ") << *dn << " to " << *targeti << dendl;

    ceph_assert(g_conf()->mds_kill_link_at != 3);

    if (!mdr->more()->witnessed.empty()) mdcache->logged_leader_update(mdr->reqid);
...
public:
    C_MDS_PeerLinkCommit(Server* s, MDRequestRef& r, CInode* t)
        ...
        , targeti(t)
    {}
    void finish(int r) override { server->_commit_peer_link(mdr, r, targeti); }
};
...
void Server::handle_peer_link_prep(MDRequestRef& mdr)
{
    ...
    // journal it
    mdr->ls = mdlog->get_current_segment();
    EPeerUpdate* le = new EPeerUpdate(
        mdlog, "peer_link_prep", mdr->reqid, mdr->peer_to_mds, EPeerUpdate::OP_PREPARE, EPeerUpdate::LINK);
    mdlog->start_entry(le);
    ...
    if (mdr->peer_request->get_op() == MMDSPeerRequest::OP_LINKPREP) {
        ...
        if (targeti->is_projected_snaprealm_global()) {
            ...
            if (pi.inode->nlink == 0) ceph_assert(!newsnap->is_parent_global());
    ...
    submit_mdlog_entry(le, new C_MDS_PeerLinkPrep(this, mdr, targeti, adjust_realm), mdr, __func__);
    mdlog->flush();
}
...
{
    C_MDS_CommittedPeer(Server* s, MDRequestRef& m)
        : ServerLogContext(s, m)
    {}
    void finish(int r) override { server->_committed_peer(mdr); }
};
...
void Server::_commit_peer_link(MDRequestRef& mdr, int r, CInode* targeti)
{
    ...
    if (r == 0) {
        ...
        EPeerUpdate* le = new EPeerUpdate(
            mdlog, "peer_link_commit", mdr->reqid, mdr->peer_to_mds, EPeerUpdate::OP_COMMIT, EPeerUpdate::LINK);
        mdlog->start_entry(le);
...
struct C_MDS_LoggedLinkRollback : public ServerLogContext
{
    ...
    C_MDS_LoggedLinkRollback(Server* s, MutationRef& m, MDRequestRef& r, map<client_t, ref_t<MClientSnap>>&& _splits)
        : ServerLogContext(s, r)
        , mut(m)
        , splits(std::move(_splits))
    {}
    void finish(int r) override { server->_link_rollback_finish(mut, mdr, splits); }
};
...
void Server::do_link_rollback(bufferlist& rbl, mds_rank_t leader, MDRequestRef& mdr)
{
    ...
    dout(10) << "do_link_rollback on " << rollback.reqid << (rollback.was_inc ? " inc" : " dec") << " ino "
             << rollback.ino << dendl;
    ...
    if (pf->fragstat.mtime == pi.inode->ctime) {
        ...
        if (pf->rstat.rctime == pi.inode->ctime) pf->rstat.rctime = rollback.old_dir_rctime;
    ...
    if (rollback.snapbl.length() && in->snaprealm) {
        ...
        if (hadrealm) {
            ...
            if (!mds->is_resolve()) mdcache->prepare_realm_merge(in->snaprealm, realm, splits);
    ...
    // journal it
    EPeerUpdate* le = new EPeerUpdate(
        mdlog, "peer_link_rollback", rollback.reqid, leader, EPeerUpdate::OP_ROLLBACK, EPeerUpdate::LINK);
    mdlog->start_entry(le);
    ...
    submit_mdlog_entry(le, new C_MDS_LoggedLinkRollback(this, mut, mdr, std::move(splits)), mdr, __func__);
    mdlog->flush();
}

void Server::_link_rollback_finish(MutationRef& mut, MDRequestRef& mdr, map<client_t, ref_t<MClientSnap>>& splits)
{
    ...
    if (!mds->is_resolve()) mdcache->send_snaps(splits);

    if (mdr) mdcache->request_finish(mdr);
...
void Server::handle_client_unlink(MDRequestRef& mdr)
{
    ...
    if (!(mdr->locking_state & MutationImpl::ALL_LOCKED)) {
        ...
        lov.add_xlock(&in->snaplock);
        if (in->is_dir()) lov.add_rdlock(&in->filelock);   // to verify it's empty
    ...
    if (in->is_dir() && in->has_subtree_root_dirfrag()) {
        ...
        for (set<mds_rank_t>::iterator p = witnesses.begin(); p != witnesses.end(); ++p) {
            if (mdr->more()->witnessed.count(*p)) {
                ...
                if (!_rmdir_prepare_witness(mdr, *p, mdr->dn[0], straydn)) return;
            }
        }
...
void Server::_unlink_local(MDRequestRef& mdr, CDentry* dn, CDentry* straydn)
{
    ...
    if (straydn) {
        ...
        mdcache->predirty_journal_parents(
            mdr, &le->metablob, in, straydn->get_dir(), PREDIRTY_PRIMARY | PREDIRTY_DIR, 1);
...
void Server::_unlink_local_finish(MDRequestRef& mdr, CDentry* dn, CDentry* straydn, version_t dnpv)
{
    ...
    if (straydn) {
        // update subtree map?
        if (strayin->is_dir()) mdcache->adjust_subtree_after_rename(strayin, dn->get_dir(), true);
...
{
    if (mds->is_cluster_degraded() && !mds->mdsmap->is_clientreplay_or_active_or_stopping(who)) {
        dout(10) << "_rmdir_prepare_witness mds." << who << " is not active" << dendl;
        if (mdr->more()->waiting_on_peer.empty()) mds->wait_for_active_peer(who, new C_MDS_RetryRequest(mdcache, mdr));
        return false;
    }
    ...
    for (auto dn : trace) req->srcdnpath.push_dentry(dn->get_name());
    mdcache->encode_replica_stray(straydn, who, req->straybl);
    if (mdr->more()->desti_srnode) encode(*mdr->more()->desti_srnode, req->desti_snapbl);
...
struct C_MDS_PeerRmdirPrep : public ServerLogContext
{
    ...
    C_MDS_PeerRmdirPrep(Server* s, MDRequestRef& r, CDentry* d, CDentry* st)
        ...
        , straydn(st)
    {}
    void finish(int r) override { server->_logged_peer_rmdir(mdr, dn, straydn); }
};
...
struct C_MDS_PeerRmdirCommit : public ServerContext
{
    ...
    C_MDS_PeerRmdirCommit(Server* s, MDRequestRef& r, CDentry* sd)
        ...
        , straydn(sd)
    {}
    void finish(int r) override { server->_commit_peer_rmdir(mdr, r, straydn); }
};

void Server::handle_peer_rmdir_prep(MDRequestRef& mdr)
{
    dout(10) << "handle_peer_rmdir_prep " << *mdr << " " << mdr->peer_request->srcdnpath << " to "
             << mdr->peer_request->destdnpath << dendl;
    ...
    CInode* in;
    CF_MDS_RetryRequestFactory cf(mdcache, mdr, false);
    int r = mdcache->path_traverse(mdr, cf, srcpath, MDS_TRAVERSE_DISCOVER | MDS_TRAVERSE_PATH_LOCKED, &trace, &in);
    if (r > 0) return;
    if (r == -CEPHFS_ESTALE) {
        mdcache->find_ino_peers(srcpath.get_ino(), new C_MDS_RetryRequest(mdcache, mdr), mdr->peer_to_mds, true);
        return;
    }
    ceph_assert(r == 0);
    ...
    mdr->ls = mdlog->get_current_segment();
    EPeerUpdate* le =
        new EPeerUpdate(mdlog, "peer_rmdir", mdr->reqid, mdr->peer_to_mds, EPeerUpdate::OP_PREPARE, EPeerUpdate::RMDIR);
    mdlog->start_entry(le);
    ...
    mdr->more()->peer_update_journaled = true;
    submit_mdlog_entry(le, new C_MDS_PeerRmdirPrep(this, mdr, dn, straydn), mdr, __func__);
    mdlog->flush();
}
...
void Server::_logged_peer_rmdir(MDRequestRef& mdr, CDentry* dn, CDentry* straydn)
{
    ...
    if (new_realm) mdcache->do_realm_invalidate_and_update_notify(in, CEPH_SNAP_OP_SPLIT, false);
    ...
    if (!mdr->aborted) {
        ...
        if (!mdr->more()->peer_update_journaled) reply->mark_not_journaled();
        mds->send_message_mds(reply, mdr->peer_to_mds);
    }
    else {
        dout(10) << " abort flag set, finishing" << dendl;
...
void Server::handle_peer_rmdir_prep_ack(MDRequestRef& mdr, const cref_t<MMDSPeerRequest>& ack)
{
    ...
    mdr->more()->peers.insert(from);
    mdr->more()->witnessed.insert(from);
    if (!ack->is_not_journaled()) mdr->more()->has_journaled_peers = true;
...
void Server::_commit_peer_rmdir(MDRequestRef& mdr, int r, CDentry* straydn)
{
    ...
    if (r == 0) {
        if (mdr->more()->peer_update_journaled) {
            ...
            if (strayin && !strayin->snaprealm) mdcache->clear_dirty_bits_for_stray(strayin);
        }
        ...
        if (mdr->more()->peer_update_journaled) {
            // write a commit to the journal
            EPeerUpdate* le = new EPeerUpdate(
                mdlog, "peer_rmdir_commit", mdr->reqid, mdr->peer_to_mds, EPeerUpdate::OP_COMMIT, EPeerUpdate::RMDIR);
            mdlog->start_entry(le);
            submit_mdlog_entry(le, new C_MDS_CommittedPeer(this, mdr), mdr, __func__);
            mdlog->flush();
        }
...
struct C_MDS_LoggedRmdirRollback : public ServerLogContext
{
    ...
    C_MDS_LoggedRmdirRollback(Server* s, MDRequestRef& m, metareqid_t mr, CDentry* d, CDentry* st)
        ...
        , straydn(st)
    {}
    void finish(int r) override { server->_rmdir_rollback_finish(mdr, reqid, dn, straydn); }
};
...
void Server::do_rmdir_rollback(bufferlist& rbl, mds_rank_t leader, MDRequestRef& mdr)
{
    ...
    if (!dir) dir = mdcache->get_dirfrag(rollback.src_dir.ino, rollback.src_dname);
    ceph_assert(dir);
    ...
    EPeerUpdate* le = new EPeerUpdate(
        mdlog, "peer_rmdir_rollback", rollback.reqid, leader, EPeerUpdate::OP_ROLLBACK, EPeerUpdate::RMDIR);
    mdlog->start_entry(le);
    ...
    submit_mdlog_entry(le, new C_MDS_LoggedRmdirRollback(this, mdr, rollback.reqid, dn, straydn), mdr, __func__);
    mdlog->flush();
}
...
void Server::_rmdir_rollback_finish(MDRequestRef& mdr, metareqid_t reqid, CDentry* dn, CDentry* straydn)
{
    ...
    CInode* in = dn->get_linkage()->get_inode();
    mdcache->adjust_subtree_after_rename(in, straydn->get_dir(), !mdr || mdr->more()->peer_update_journaled);
...
public:
    C_MDS_rename_finish(Server* s, MDRequestRef& r, CDentry* sdn, CDentry* ddn, CDentry* stdn)
        : ServerLogContext(s, r)
        , srcdn(sdn)
        , destdn(ddn)
        , straydn(stdn)
...
void Server::handle_client_rename(MDRequestRef& mdr)
{
    ...
    if (mdr->reqid.name.is_mds() && !(MDS_INO_IS_STRAY(srcpath.get_ino()) && MDS_INO_IS_STRAY(destpath.get_ino())) &&
        !(destdnl->is_remote() && destdnl->get_remote_ino() == srci->ino())) {
    ...
    if (destpath.get_ino() != srcpath.get_ino() &&
        ...
        while (srcbase != destbase && !srcbase->is_projected_ancestor_of(destbase)) {
    ...
    if (linkmerge) dout(10) << " this is a link merge" << dendl;
    ...
    if (!(mdr->locking_state & MutationImpl::ALL_LOCKED)) {
        ...
        if (oldin) {
            ...
            if (oldin->is_dir()) {
                ...
                int cmp = mdr->compare_paths();
                if (cmp < 0 || (cmp == 0 && oldin->ino() < srci->ino())) std::reverse(lov.begin(), lov.end());
            }
            else {
                ceph_assert(!srci->is_dir());
                // adjust locking order;
                if (srci->ino() > oldin->ino()) std::reverse(lov.begin(), lov.end());
            }
        }
        ...
        if (!mds->locker->acquire_locks(mdr, lov, auth_pin_freeze)) return;

        mdr->locking_state |= MutationImpl::ALL_LOCKED;
    }

    if (linkmerge) ceph_assert(srcdir->inode->is_stray() && srcdnl->is_primary() && destdnl->is_remote());

    if ((!mdr->has_more() || mdr->more()->witnessed.empty())) {
        if (!check_access(mdr, srcdir->get_inode(), MAY_WRITE)) return;

        if (!check_access(mdr, destdn->get_dir()->get_inode(), MAY_WRITE)) return;

        if (!linkmerge && !check_fragment_space(mdr, destdn->get_dir())) return;

        if (!linkmerge && !check_dir_max_entries(mdr, destdn->get_dir())) return;

        if (!check_access(mdr, srci, MAY_WRITE)) return;
    }

    // with read lock, really verify oldin is empty
    if (oldin && oldin->is_dir() && _dir_is_nonempty(mdr, oldin)) {
        respond_to_request(mdr, -CEPHFS_ENOTEMPTY);
        return;
    }
    ...
    */
    ...
    if (!linkmerge) {
        ...
        if (src_realm != dest_realm && src_realm->get_subvolume_ino() != dest_realm->get_subvolume_ino()) {
            respond_to_request(mdr, -CEPHFS_EXDEV);
            return;
        }
    }
    ...
    if (srcdnl->is_primary() && !srcdn->is_auth() && srci->is_dir()) {
    ...
    if (linkmerge) {
        if (!mdr->more()->srci_srnode && srci->get_projected_inode()->nlink == 1 &&
        ...
        if (oldin && !mdr->more()->desti_srnode) {
            if (oldin->is_projected_snaprealm_global()) {
                ...
                if (nlink == 1 ||
                    (nlink == 2 && !destdnl->is_primary() && !oldin->get_projected_parent_dir()->inode->is_stray()))
        ...
        if (!mdr->more()->srci_srnode) {
            if (srci->is_projected_snaprealm_global()) {
                ...
                if (src_realm != dest_realm && (srci->snaprealm || follows + 1 > srci->get_oldest_snap())) {
    ...
    if (srcdnl->is_remote() && !srci->is_auth()) witnesses.insert(srci->authority().first);
    destdn->list_replicas(witnesses);
    if (destdnl->is_remote() && !oldin->is_auth()) witnesses.insert(oldin->authority().first);
    dout(10) << " witnesses " << witnesses << ", have " << mdr->more()->witnessed << dendl;
    ...
    if (!witnesses.empty()) {
        ...
        if (need_flush) {
            mdlog->wait_for_safe(new MDSInternalContextWrapper(mds, new C_MDS_RetryRequest(mdcache, mdr)));
            mdlog->flush();
            return;
        }
    }
    ...
    for (set<mds_rank_t>::iterator p = witnesses.begin(); p != witnesses.end(); ++p) {
        ...
        if (mdr->more()->witnessed.count(*p)) {
            ...
            if (!_rename_prepare_witness(mdr, *p, witnesses, srctrace, desttrace, straydn)) return;
        }
    }
    if (!mdr->more()->waiting_on_peer.empty()) return;   // we're waiting for a witness.
    ...
    if (!mdr->more()->peers.empty() && !srci->is_dir()) ceph_assert(g_conf()->mds_kill_rename_at != 3);
    if (!mdr->more()->peers.empty() && srci->is_dir()) ceph_assert(g_conf()->mds_kill_rename_at != 4);
    ...
    if (le->client_map.length()) le->cmapv = mds->sessionmap.get_projected();
...
void Server::_rename_finish(MDRequestRef& mdr, CDentry* srcdn, CDentry* destdn, CDentry* straydn)
{
    ...
    if (!mdr->more()->peers.empty() && !in->is_dir()) ceph_assert(g_conf()->mds_kill_rename_at != 5);
    if (!mdr->more()->peers.empty() && in->is_dir()) ceph_assert(g_conf()->mds_kill_rename_at != 6);

    // bump popularity
    mds->balancer->hit_dir(srcdn->get_dir(), META_POP_IWR);
    if (destdnl->is_remote() && in->is_auth()) mds->balancer->hit_inode(in, META_POP_IWR);
    ...
    if (need_eval) mds->locker->eval(in, CEPH_CAP_LOCKS, true);
                                     ...
                                     vector<CDentry*>& srctrace, vector<CDentry*>& dsttrace, CDentry* straydn)
{
    ...
    for (auto dn : srctrace) req->srcdnpath.push_dentry(dn->get_name());
    req->destdnpath = filepath(dsttrace.front()->get_dir()->ino());
    for (auto dn : dsttrace) req->destdnpath.push_dentry(dn->get_name());
    req->alternate_name = client_req->alternate_name;
    if (straydn) mdcache->encode_replica_stray(straydn, who, req->straybl);

    if (mdr->more()->srci_srnode) encode(*mdr->more()->srci_srnode, req->srci_snapbl);
    if (mdr->more()->desti_srnode) encode(*mdr->more()->desti_srnode, req->desti_snapbl);
...
version_t Server::_rename_prepare_import(MDRequestRef& mdr, CDentry* srcdn, bufferlist* client_map_bl)
{
    ...
    prepare_force_open_sessions(client_map, client_metadata_map, mdr->more()->imported_session_map);
    encode(client_map, *client_map_bl, mds->mdsmap->get_up_features());
    encode(client_metadata_map, *client_map_bl);

    list<ScatterLock*> updated_scatterlocks;
    mdcache->migrator->decode_import_inode(
        srcdn, blp, srcdn->authority().first, mdr->ls, mdr->more()->cap_imports, updated_scatterlocks);
...
bool Server::_need_force_journal(CInode* diri, bool empty)
{
    ...
    if (empty) {
        ...
        for (const auto& dir : dirs) {
            ...
            if (force_journal) break;
        }
    }
    return force_journal;
}

void Server::_rename_prepare(MDRequestRef& mdr, EMetaBlob* metablob, bufferlist* client_map_bl, CDentry* srcdn,
                             CDentry* destdn, std::string_view alternate_name, CDentry* straydn)
{
    ...
    if (linkmerge) ceph_assert(srcdnl->is_primary() && destdnl->is_remote());
    ...
    if (linkmerge) dout(10) << " merging remote and primary links to the same inode" << dendl;
    if (silent) dout(10) << " reintegrating stray; will avoid changing nlink or dir mtime" << dendl;
    ...
    if (srcdnl->is_remote()) {
        if (!linkmerge) {
            // destdn
            if (destdn->is_auth()) mdr->more()->pvmap[destdn] = destdn->pre_dirty();
    ...
    // src
    if (srcdn->is_auth()) mdr->more()->pvmap[srcdn] = srcdn->pre_dirty();
    srcdn->push_projected_linkage();   // push null linkage

    if (!silent) {
        if (spi) {
            spi->ctime = mdr->get_op_stamp();
            if (mdr->get_op_stamp() > spi->rstat.rctime) spi->rstat.rctime = mdr->get_op_stamp();
            spi->change_attr++;
            if (linkmerge) spi->nlink--;
        }
        if (tpi) {
            tpi->ctime = mdr->get_op_stamp();
            if (mdr->get_op_stamp() > tpi->rstat.rctime) tpi->rstat.rctime = mdr->get_op_stamp();
            tpi->change_attr++;
            {
                std::string t;
            ...
            tpi->nlink--;
            if (tpi->nlink == 0) oldin->state_set(CInode::STATE_ORPHAN);
        }
    }
    ...
    if (destdn->is_auth() && !destdnl->is_null()) {
        mdcache->predirty_journal_parents(
            mdr, metablob, oldin, destdn->get_dir(), (destdnl->is_primary() ? PREDIRTY_PRIMARY : 0) | predirty_dir, -1);
        if (destdnl->is_primary()) {
            ceph_assert(straydn);
            mdcache->predirty_journal_parents(
                mdr, metablob, oldin, straydn->get_dir(), PREDIRTY_PRIMARY | PREDIRTY_DIR, 1);
        }
    }
    ...
    if (destdn->is_auth()) mdcache->predirty_journal_parents(mdr, metablob, srci, destdn->get_dir(), flags, 1);
        ...
        if (destdnl->is_primary()) {
            ...
            if (destdn->is_auth()) {
                ...
                if (auto& desti_srnode = mdr->more()->desti_srnode) {
                    ...
                    if (tpi->nlink == 0) ceph_assert(!desti_srnode->is_parent_global());
                    desti_srnode = NULL;
                }
            ...
            if (oldin->is_auth()) {
                ...
                if (new_srnode) {
                    ...
                    if (tpi->nlink == 0) ceph_assert(!new_srnode->is_parent_global());
                }
                // auth for targeti
    ...
    if (srcdnl->is_remote()) {
        ...
        if (destdn->is_auth())
            ...
            if (srcdn->is_auth() && srci->is_dir()) {
                ...
                auto&& ls = srci->get_dirfrags();
                for (const auto& dir : ls) {
                    if (dir->is_auth()) metablob->add_dir(dir, true);
                }
            }
        }
    }
    ...
    if (srci->is_dir()) mdcache->project_subtree_rename(srci, srcdn->get_dir(), destdn->get_dir());
}
...
void Server::_rename_apply(MDRequestRef& mdr, CDentry* srcdn, CDentry* destdn, CDentry* straydn)
{
    ...
    if (!linkmerge) {
        if (destdnl->is_primary()) {
            ...
            // nlink-- targeti
            if (destdn->is_auth()) oldin->pop_and_dirty_projected_inode(mdr->ls, mdr);
    ...
    if (srcdn_was_remote) {
        if (!linkmerge) {
            ...
            if (destdn->is_auth()) destdn->mark_dirty(mdr->more()->pvmap[destdn], mdr->ls);
            // in
            if (in->is_auth()) {
        ...
        if (!srcdn->is_auth() && destdn->is_auth()) {
            ...
            for (auto i = mdr->locks.lower_bound(&destdnl->get_inode()->versionlock); i != mdr->locks.end(); ++i) {
                SimpleLock* lock = i->lock;
                if (lock->get_parent() != destdnl->get_inode()) break;
                if (i->is_xlock() && !lock->is_locallock()) mds->locker->xlock_import(lock);
            }
        ...
        if (destdn->is_auth()) in->pop_and_dirty_projected_inode(mdr->ls, mdr);
    }

    // src
    if (srcdn->is_auth()) srcdn->mark_dirty(mdr->more()->pvmap[srcdn], mdr->ls);
    ...
    // update subtree map?
    if (destdnl->is_primary() && in->is_dir()) mdcache->adjust_subtree_after_rename(in, srcdn->get_dir(), true);

    if (straydn && oldin->is_dir()) mdcache->adjust_subtree_after_rename(oldin, destdn->get_dir(), true);

    if (new_oldin_snaprealm) mdcache->do_realm_invalidate_and_update_notify(oldin, CEPH_SNAP_OP_SPLIT, false);
    if (new_in_snaprealm) mdcache->do_realm_invalidate_and_update_notify(in, CEPH_SNAP_OP_SPLIT, true);

    // removing a new dn?
    if (srcdn->is_auth()) srcdn->get_dir()->try_remove_unlinked_dn(srcdn);
}
...
public:
    C_MDS_PeerRenamePrep(Server* s, MDRequestRef& m, CDentry* sr, CDentry* de, CDentry* st)
        ...
        , straydn(st)
    {}
    void finish(int r) override { server->_logged_peer_rename(mdr, srcdn, destdn, straydn); }
};
...
public:
    C_MDS_PeerRenameCommit(Server* s, MDRequestRef& m, CDentry* sr, CDentry* de, CDentry* st)
        ...
        , straydn(st)
    {}
    void finish(int r) override { server->_commit_peer_rename(mdr, r, srcdn, destdn, straydn); }
};
...
public:
    C_MDS_PeerRenameSessionsFlushed(Server* s, MDRequestRef& r)
        ...
        , mdr(r)
    {}
    void finish(int r) override { server->_peer_rename_sessions_flushed(mdr); }
};

void Server::handle_peer_rename_prep(MDRequestRef& mdr)
{
    dout(10) << "handle_peer_rename_prep " << *mdr << " " << mdr->peer_request->srcdnpath << " to "
             << mdr->peer_request->destdnpath << dendl;
    ...
    CF_MDS_RetryRequestFactory cf(mdcache, mdr, false);
    int r = mdcache->path_traverse(
        mdr, cf, destpath, MDS_TRAVERSE_DISCOVER | MDS_TRAVERSE_PATH_LOCKED | MDS_TRAVERSE_WANT_DENTRY, &trace);
    if (r > 0) return;
    if (r == -CEPHFS_ESTALE) {
        mdcache->find_ino_peers(destpath.get_ino(), new C_MDS_RetryRequest(mdcache, mdr), mdr->peer_to_mds, true);
        return;
    }
    ...
    dout(10) << " src " << srcpath << dendl;
    CInode* srci = nullptr;
    r = mdcache->path_traverse(mdr, cf, srcpath, MDS_TRAVERSE_DISCOVER | MDS_TRAVERSE_PATH_LOCKED, &trace, &srci);
    if (r > 0) return;
    ceph_assert(r == 0);
    ...
    if (linkmerge) ceph_assert(srcdnl->is_primary() && destdnl->is_remote());
    CDentry* straydn = mdr->straydn;
    if (destdnl->is_primary() && !linkmerge) ceph_assert(straydn);
    ...
    if (srcdn->is_auth()) {
        ...
        if (srcdnl->is_primary() && !srcdnl->get_inode()->state_test(CInode::STATE_AMBIGUOUSAUTH)) {
            ...
            if (srcdnl->get_inode()->is_frozen_auth_pin()) mdr->unfreeze_auth_pin();
            ...
            for (set<mds_rank_t>::iterator p = srcdnrep.begin(); p != srcdnrep.end(); ++p) {
                if (*p == mdr->peer_to_mds ||
                    (mds->is_cluster_degraded() && !mds->mdsmap->is_clientreplay_or_active_or_stopping(*p)))
                    continue;
        ...
        for (set<mds_rank_t>::iterator p = srcdnrep.begin(); p != srcdnrep.end(); ++p) {
            if (*p == mdr->peer_to_mds || mdr->peer_request->witnesses.count(*p)) continue;
    ...
    mdr->ls = mdlog->get_current_segment();
    EPeerUpdate* le = new EPeerUpdate(
        mdlog, "peer_rename_prep", mdr->reqid, mdr->peer_to_mds, EPeerUpdate::OP_PREPARE, EPeerUpdate::RENAME);
    mdlog->start_entry(le);
    le->rollback = mdr->more()->rollback_bl;
    ...
    if (le->commit.empty()) {
        ...
        mdcache->add_uncommitted_peer(mdr->reqid, mdr->ls, mdr->peer_to_mds);
        mdr->more()->peer_update_journaled = true;
        submit_mdlog_entry(le, new C_MDS_PeerRenamePrep(this, mdr, srcdn, destdn, straydn), mdr, __func__);
        mdlog->flush();
    }
}

void Server::_logged_peer_rename(MDRequestRef& mdr, CDentry* srcdn, CDentry* destdn, CDentry* straydn)
{
    ...
    if (srcdn->is_auth() && srcdnl->is_primary()) {
        ...
        if (reply) {
            ...
            bufferlist inodebl;
            mdcache->migrator->encode_export_inode(
                srcdnl->get_inode(), inodebl, exported_client_map, exported_client_metadata_map);
        ...
        if (srcdnl->get_inode()->is_dirty()) srcdnl->get_inode()->mark_clean();
...
void Server::_commit_peer_rename(MDRequestRef& mdr, int r, CDentry* srcdn, CDentry* destdn, CDentry* straydn)
{
    ...
    if (srcdn->is_auth() && srcdn->get_dir()->inode->is_stray()) migrated_stray = in->ino();
    ...
    if (r == 0) {
        ...
        if (mdr->more()->is_inode_exporter) {
            ...
            for (auto i = mdr->locks.lower_bound(&in->versionlock); i != mdr->locks.end();) {
                SimpleLock* lock = i->lock;
                if (lock->get_parent() != in) break;
        ...
        if (mdr->more()->peer_update_journaled) {
            // write a commit to the journal
            EPeerUpdate* le = new EPeerUpdate(
                mdlog, "peer_rename_commit", mdr->reqid, mdr->peer_to_mds, EPeerUpdate::OP_COMMIT, EPeerUpdate::RENAME);
            mdlog->start_entry(le);
            submit_mdlog_entry(le, new C_MDS_CommittedPeer(this, mdr), mdr, __func__);
            mdlog->flush();
        }
        ...
        if (mdr->more()->rollback_bl.length()) {
            ...
            dout(10) << " rollback_bl empty, not rollback back rename (leader failed after getting extra witnesses?)"
                     << dendl;
            // singleauth
            if (mdr->more()->is_ambiguous_auth) {
                if (srcdn->is_auth()) mdr->more()->rename_inode->unfreeze_inode(finished);
    ...
    if (migrated_stray && mds->is_stopping()) mdcache->shutdown_export_stray_finish(migrated_stray);
}

static void _rollback_repair_dir(MutationRef& mut, CDir* dir, rename_rollback::drec& r, utime_t ctime, bool isdir,
                                 const nest_info_t& rstat)
{
    ...
    if (pf->fragstat.mtime == ctime) {
        ...
        if (pf->rstat.rctime == ctime) pf->rstat.rctime = r.dirfrag_old_rctime;
    }
...
struct C_MDS_LoggedRenameRollback : public ServerLogContext
{
    ...
    bool finish_mdr;
    C_MDS_LoggedRenameRollback(Server* s, MutationRef& m, MDRequestRef& r, CDentry* sd, version_t pv, CDentry* dd,
                               CDentry* st, map<client_t, ref_t<MClientSnap>> _splits[2], bool f)
        : ServerLogContext(s, r)
        , mut(m)
        , srcdn(sd)
        , srcdnpv(pv)
    ...
    void finish(int r) override
    {
        server->_rename_rollback_finish(mut, mdr, srcdn, srcdnpv, destdn, straydn, splits, finish_mdr);
    }
};

void Server::do_rename_rollback(bufferlist& rbl, mds_rank_t leader, MDRequestRef& mdr, bool finish_mdr)
{
    rename_rollback rollback;
    auto p = rbl.cbegin();
    decode(rollback, p);
    ...
    if (!srcdir) srcdir = mdcache->get_dirfrag(rollback.orig_src.dirfrag.ino, rollback.orig_src.dname);
    if (srcdir) {
    ...
    if (!destdir) destdir = mdcache->get_dirfrag(rollback.orig_dest.dirfrag.ino, rollback.orig_dest.dname);
    if (destdir) {
    ...
    if (rollback.orig_src.ino) {
        ...
        if (in && in->is_dir()) ceph_assert(srcdn && destdn);
    }
    else
    ...
    if (rollback.orig_dest.ino) {
        ...
        if (target) ceph_assert(destdn && straydn);
    }
    ...
    bool force_journal_dest = false;
    if (in && in->is_dir() && srcdn->authority().first != whoami) force_journal_src = _need_force_journal(in, false);
    if (in && target && target->is_dir()) force_journal_dest = _need_force_journal(in, true);

    version_t srcdnpv = 0;
    // repair src
    if (srcdn) {
        if (srcdn->authority().first == whoami) srcdnpv = srcdn->pre_dirty();
        if (rollback.orig_src.ino) {
            ceph_assert(in);
            srcdn->push_projected_linkage(in);
        }
        else
            srcdn->push_projected_linkage(rollback.orig_src.remote_ino, rollback.orig_src.remote_d_type);
    }
    ...
    if (in) {
        ...
        if (pdir->authority().first == whoami) {
            ...
            if (pi.inode->ctime == rollback.ctime) pi.inode->ctime = rollback.orig_src.old_ctime;
            projected = true;
        }
        else {
        ...
        if (rollback.srci_snapbl.length() && in->snaprealm) {
            ...
            if (hadrealm) {
                ...
                if (!mds->is_resolve()) mdcache->prepare_realm_merge(in->snaprealm, realm, splits[0]);
                if (projected)
                    in->project_snaprealm(NULL);
                else
    ...
    if (destdn) {
        if (rollback.orig_dest.ino && target) {
            ...
            destdn->push_projected_linkage(rollback.orig_dest.remote_ino, rollback.orig_dest.remote_d_type);
        }
        else {
            // the dentry will be trimmed soon, it's ok to have wrong linkage
            if (rollback.orig_dest.ino) ceph_assert(mds->is_resolve());
            destdn->push_projected_linkage();
        }
    }

    if (straydn) straydn->push_projected_linkage();
    ...
    if (target) {
        ...
        if (ti->ctime == rollback.ctime) ti->ctime = rollback.orig_dest.old_ctime;
        if (MDS_INO_IS_STRAY(rollback.orig_src.dirfrag.ino)) {
            if (MDS_INO_IS_STRAY(rollback.orig_dest.dirfrag.ino))
                ...
                ceph_assert(rollback.orig_dest.remote_ino && rollback.orig_dest.remote_ino == rollback.orig_src.ino);
        }
        else
            ti->nlink++;

        if (!projected) target->reset_inode(ti);
        ...
        if (rollback.desti_snapbl.length() && target->snaprealm) {
            ...
            if (hadrealm) {
                ...
                if (!mds->is_resolve()) mdcache->prepare_realm_merge(target->snaprealm, realm, splits[1]);
                if (projected)
    ...
    if (srcdn && srcdn->authority().first == whoami) {
        nest_info_t blah;
        _rollback_repair_dir(
            mut, srcdir, rollback.orig_src, rollback.ctime, in && in->is_dir(), pip ? pip->accounted_rstat : blah);
    }

    if (srcdn) dout(0) << " srcdn back to " << *srcdn << dendl;
    if (in) dout(0) << "  srci back to " << *in << dendl;
    if (destdn) dout(0) << " destdn back to " << *destdn << dendl;
    if (target) dout(0) << "  desti back to " << *target << dendl;

    // journal it
    EPeerUpdate* le = new EPeerUpdate(
        mdlog, "peer_rename_rollback", rollback.reqid, leader, EPeerUpdate::OP_ROLLBACK, EPeerUpdate::RENAME);
    mdlog->start_entry(le);
    ...
    if (in && in->is_dir() && (srcdn->authority().first == whoami || force_journal_src)) {
        ...
        if (srcdn->authority().first == whoami) {
            auto&& ls = in->get_dirfrags();
            for (const auto& dir : ls) {
                if (!dir->is_auth()) le->commit.renamed_dir_frags.push_back(dir->get_frag());
            }
    ...
    if (mdr && !mdr->more()->peer_update_journaled) {
        ...
        ceph_assert(!le->commit.empty());
        if (mdr) mdr->more()->peer_update_journaled = false;
        MDSLogContextBase* fin =
            new C_MDS_LoggedRenameRollback(this, mut, mdr, srcdn, srcdnpv, destdn, straydn, splits, finish_mdr);
        submit_mdlog_entry(le, fin, mdr, __func__);
        mdlog->flush();
    }
}

void Server::_rename_rollback_finish(MutationRef& mut, MDRequestRef& mdr, CDentry* srcdn, version_t srcdnpv,
                                     CDentry* destdn, CDentry* straydn, map<client_t, ref_t<MClientSnap>> splits[2],
                                     bool finish_mdr)
{
    ...
    if (srcdn) {
        ...
        if (srcdn->authority().first == mds->get_nodeid()) {
            ...
            if (srcdn->get_linkage()->is_primary()) srcdn->get_linkage()->get_inode()->state_set(CInode::STATE_AUTH);
        }
    }
    ...
    if (mds->is_resolve()) {
        ...
        if (root) mdcache->try_trim_non_auth_subtree(root);
    }
    else {
...
void Server::handle_peer_rename_prep_ack(MDRequestRef& mdr, const cref_t<MMDSPeerRequest>& ack)
{
    ...
    // note peer
    mdr->more()->peers.insert(from);
    if (mdr->more()->srcdn_auth_mds == from && mdr->more()->is_remote_frozen_authpin &&
...
void Server::handle_peer_rename_notify_ack(MDRequestRef& mdr, const cref_t<MMDSPeerRequest>& ack)
{
    ...
    if (mdr->more()->waiting_on_peer.count(from)) {
        ...
        if (mdr->more()->waiting_on_peer.empty()) {
            if (mdr->peer_request) dispatch_peer_request(mdr);
        }
        else
            dout(10) << " still waiting for rename notify acks from " << mdr->more()->waiting_on_peer << dendl;
    }
...
void Server::handle_client_lssnap(MDRequestRef& mdr)
{
    ...
    // lock snap
    if (!mds->locker->try_rdlock_snap_layout(diri, mdr)) return;
    ...
    if (!max_entries) max_entries = infomap.size();
    ...
    __u64 last_snapid = 0;
    string offset_str = req->get_path2();
    if (!offset_str.empty()) last_snapid = realm->resolve_snapname(offset_str, diri->ino());
    ...
    for (; p != infomap.end() && num < max_entries; ++p) {
        ...
        unsigned start_len = dnbl.length();
        if (int(start_len + snap_name.length() + sizeof(__u32) + sizeof(LeaseStat)) > max_bytes) break;
    ...
    if (p == infomap.end()) {
        flags = CEPH_READDIR_FRAG_END;
        if (last_snapid == 0) flags |= CEPH_READDIR_FRAG_COMPLETE;
    }
    encode(flags, dirbl);
    dirbl.claim_append(dnbl);
...
struct C_MDS_mksnap_finish : public ServerLogContext
{
    ...
    C_MDS_mksnap_finish(Server* s, MDRequestRef& r, CInode* di, SnapInfo& i)
        ...
        , info(i)
    {}
    void finish(int r) override { server->_mksnap_finish(mdr, diri, info); }
};
...
void Server::handle_client_mksnap(MDRequestRef& mdr)
{
    ...
    if (mdr->client_request->get_caller_uid() < g_conf()->mds_snap_min_uid ||
        ...
        dout(20) << "mksnap " << snapname << " on " << *diri << " denied to uid "
                 << mdr->client_request->get_caller_uid() << dendl;
        respond_to_request(mdr, -CEPHFS_EPERM);
        return;
    }
    ...
    if (!(mdr->locking_state & MutationImpl::ALL_LOCKED)) {
        ...
        if (CDentry* pdn = diri->get_projected_parent_dn(); pdn) {
            if (!mds->locker->try_rdlock_snap_layout(pdn->get_dir()->get_inode(), mdr)) return;
        }
        mdr->locking_state |= MutationImpl::ALL_LOCKED;
    }

    if (!check_access(mdr, diri, MAY_WRITE | MAY_SNAPSHOT)) return;

    if (inodeno_t subvol_ino = diri->find_snaprealm()->get_subvolume_ino(); (subvol_ino && subvol_ino != diri->ino())) {
    ...
    if (diri->snaprealm && diri->snaprealm->get_snaps().size() >= max_snaps_per_dir) {
    ...
    if (snapname.length() == 0 || snapname.length() > snapshot_name_max || snapname[0] == '_') {
        respond_to_request(mdr, -CEPHFS_EINVAL);
        return;
    }
    ...
    if (!mdr->more()->stid) {
        // prepare an stid
        mds->snapclient->prepare_create(diri->ino(),
                                        snapname,
                                        mdr->get_mds_stamp(),
                                        &mdr->more()->stid,
                                        &mdr->more()->snapidbl,
    ...
    if (info.stamp > pi.inode->rstat.rctime) pi.inode->rstat.rctime = info.stamp;
    ...
    auto em =
        newsnap.snaps.emplace(std::piecewise_construct, std::forward_as_tuple(snapid), std::forward_as_tuple(info));
    if (!em.second) em.first->second = info;
    newsnap.seq = snapid;
    ...
    submit_mdlog_entry(le, new C_MDS_mksnap_finish(this, mdr, diri, info), mdr, __func__);
    mdlog->flush();
}
...
struct C_MDS_rmsnap_finish : public ServerLogContext
{
    ...
    C_MDS_rmsnap_finish(Server* s, MDRequestRef& r, CInode* di, snapid_t sn)
        ...
        , snapid(sn)
    {}
    void finish(int r) override { server->_rmsnap_finish(mdr, diri, snapid); }
};
...
void Server::handle_client_rmsnap(MDRequestRef& mdr)
{
    ...
    if (mdr->client_request->get_caller_uid() < g_conf()->mds_snap_min_uid ||
        mdr->client_request->get_caller_uid() > g_conf()->mds_snap_max_uid) {
        dout(20) << "rmsnap " << snapname << " on " << *diri << " denied to uid "
                 << mdr->client_request->get_caller_uid() << dendl;
        respond_to_request(mdr, -CEPHFS_EPERM);
        return;
    }
    ...
    // prepare
    if (!mdr->more()->stid) {
        mds->snapclient->prepare_destroy(
            diri->ino(), snapid, &mdr->more()->stid, &mdr->more()->snapidbl, new C_MDS_RetryRequest(mdcache, mdr));
        return;
    }
    version_t stid = mdr->more()->stid;
    auto p = mdr->more()->snapidbl.cbegin();
    ...
    submit_mdlog_entry(le, new C_MDS_rmsnap_finish(this, mdr, diri, snapid), mdr, __func__);
    mdlog->flush();
}
...
struct C_MDS_renamesnap_finish : public ServerLogContext
{
    ...
    C_MDS_renamesnap_finish(Server* s, MDRequestRef& r, CInode* di, snapid_t sn)
        ...
        , snapid(sn)
    {}
    void finish(int r) override { server->_renamesnap_finish(mdr, diri, snapid); }
};
...
void Server::handle_client_renamesnap(MDRequestRef& mdr)
{
    ...
    // prepare
    if (!mdr->more()->stid) {
        mds->snapclient->prepare_update(
            diri->ino(), snapid, dstname, utime_t(), &mdr->more()->stid, new C_MDS_RetryRequest(mdcache, mdr));
        return;
    }
    ...
    // journal the snaprealm changes
    submit_mdlog_entry(le, new C_MDS_renamesnap_finish(this, mdr, diri, snapid), mdr, __func__);
    mdlog->flush();
}
...
void Server::handle_client_readdir_snapdiff(MDRequestRef& mdr)
{
    ...
    if (mdr->snapid_diff_other == mdr->snapid || mdr->snapid == CEPH_NOSNAP || mdr->snapid_diff_other == CEPH_NOSNAP) {
        dout(10) << "reply to " << *req << " snapdiff -CEPHFS_EINVAL" << dendl;
        respond_to_request(mdr, -CEPHFS_EINVAL);
    }

    dout(10) << __func__ << " snap " << mdr->snapid << " vs. snap " << mdr->snapid_diff_other << dendl;
    ...
    _readdir_diff(now, mdr, diri, dir, realm, max, bytes_left, offset_str, offset_hash, req_flags, dirbl);
}
...
void Server::_readdir_diff(utime_t now, MDRequestRef& mdr, CInode* diri, CDir* dir, SnapRealm* realm,
                           unsigned max_entries, int bytes_left, const string& offset_str, uint32_t offset_hash,
                           unsigned req_flags, bufferlist& dirbl)
{
    // build dir contents
    bufferlist dnbl;
    __u32 numfiles = 0;
    ...
    bool end = build_snap_diff(
        ...
        [&](CDentry* dn, CInode* in, bool exists) {
            ...
            unsigned start_len = dnbl.length();
            dout(10) << "inc dn " << *dn << " as " << name << std::hex << " hash 0x" << hash << std::dec << dendl;
            encode(name, dnbl);
            mds->locker->issue_client_lease(dn, in, mdr, now, dnbl);
    ...
    _finalize_readdir(mdr, diri, dir, from_the_beginning, end, flags, numfiles, dirbl, dnbl);
}

bool Server::build_snap_diff(MDRequestRef& mdr, CDir* dir, int bytes_left, dentry_key_t* skip_key, snapid_t snapid_prev,
                             snapid_t snapid, const bufferlist& dnbl,
                             std::function<bool(CDentry*, CInode*, bool)> add_result_cb)
{
    ...
    struct EntryInfo
    {
        ...
        void reset() { *this = EntryInfo(); }
    } before;

    auto insert_deleted = [&](EntryInfo& ei) {
        dout(20) << "build_snap_diff deleted file " << ei.dn->get_name() << " " << ei.dn->first << "/" << ei.dn->last
                 << dendl;
    ...
    while (it != dir->end()) {
        ...
        if (dnl->is_remote() && !in) {
            in = mdcache->get_inode(dnl->get_remote_ino());
            dout(20) << __func__ << " remote in: " << *in << " ino " << std::hex << dnl->get_remote_ino() << std::dec
                     << dendl;
            if (in) {
                dn->link_remote(dnl, in);
            }
        ...
        if (in->is_dir()) {
            ...
            if (snapid_prev < dn->first && dn->last < snapid) {
                dout(20) << __func__ << " skipping inner " << dn->get_name() << " " << dn->first << "/" << dn->last
                         << dendl;
                continue;
            }
            else if (dn->first <= snapid_prev && dn->last < snapid) {
                // dir deleted
                dout(20) << __func__ << " deleted dir " << dn->get_name() << " " << dn->first << "/" << dn->last
                         << dendl;
                exists = false;
            }
            ...
            if (snapid_prev >= dn->first && snapid <= dn->last) {
                dout(20) << __func__ << " skipping unchanged " << dn->get_name() << " " << dn->first << "/" << dn->last
                         << dendl;
                continue;
            }
            else if (snapid_prev < dn->first && snapid > dn->last) {
                dout(20) << __func__ << " skipping inner modification " << dn->get_name() << " " << dn->first << "/"
                         << dn->last << dendl;
                continue;
            }
            string_view name_before = before.dn ? string_view(before.dn->get_name()) : string_view();
            ...
            if (snapid_prev >= dn->first && snapid_prev <= dn->last) {
                dout(30) << __func__ << " dn_before " << dn->get_name() << " " << dn->first << "/" << dn->last << dendl;
                ...
                if (before.dn && dn->get_name() == name_before) {
                    if (mtime == before.mtime) {
                        dout(30) << __func__ << " timestamp not changed " << dn->get_name() << " " << dn->first << "/"
                                 << dn->last << " " << mtime << dendl;
                        before.reset();
                        continue;
                    }
                    else {
                        dout(30) << __func__ << " timestamp changed " << dn->get_name() << " " << dn->first << "/"
                                 << dn->last << " " << before.mtime << " vs. " << mtime << dendl;
                        before.reset();
                    }
                }
                dout(20) << __func__ << " new file " << dn->get_name() << " " << dn->first << "/" << dn->last << dendl;
