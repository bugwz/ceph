...
public:
    ...
    void print(std::ostream& o) { o << "[batch front=" << *mdr << "]"; }
};
...
void Server::dispatch(const cref_t<Message>& m)
{
    ...
    */
    ...
    case CEPH_MSG_CLIENT_REPLY: handle_client_reply(ref_cast<MClientReply>(m)); return;
...
void Server::find_idle_sessions()
{
    ...
    bool defer_client_eviction = g_conf().get_val<bool>("defer_client_eviction_on_laggy_osds") &&
                                 mds->objecter->with_osdmap([](const OSDMap& map) { return map.any_osd_laggy(); });
...
void Server::handle_client_reply(const cref_t<MClientReply>& reply)
{
    ...
    switch (reply->get_op()) {
    case CEPH_MDS_OP_RENAME: break;
    default: dout(5) << " unknown client op " << reply->get_op() << dendl;
    }
...
void Server::handle_client_open(MDRequestRef& mdr)
{
    ...
    if (cmode & CEPH_FILE_MODE_WR)
        ...
        mds->balancer->hit_inode(cur, META_POP_IRD, mdr->client_request->get_source().num());
...
void Server::handle_client_setattr(MDRequestRef& mdr)
{
    ...
    if (mask & (CEPH_SETATTR_MTIME | CEPH_SETATTR_ATIME | CEPH_SETATTR_SIZE)) lov.add_xlock(&cur->filelock);
...
void Server::handle_set_vxattr(MDRequestRef& mdr, CInode* cur)
{
    ...
    if (name.compare(0, 15, "ceph.dir.layout") == 0) {
        ...
        if (quota.is_enable() && !cur->get_projected_srnode()) adjust_realm = true;
...
void Server::handle_client_setxattr(MDRequestRef& mdr)
{
    ...
    pi.inode->ctime = mdr->get_op_stamp();
    if (mdr->get_op_stamp() > pi.inode->rstat.rctime) pi.inode->rstat.rctime = mdr->get_op_stamp();
    if (name == "encryption.ctx"sv) pi.inode->fscrypt = true;
    pi.inode->change_attr++;
...
void Server::handle_client_rename(MDRequestRef& mdr)
{
    ...
    if ((!mdr->has_more() || mdr->more()->witnessed.empty())) {
        ...
        if (!check_dir_max_entries(mdr, destdn->get_dir())) return;
