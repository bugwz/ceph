...
void Server::journal_and_reply(MDRequestRef& mdr, CInode* in, CDentry* dn, LogEvent* le, MDSLogContextBase* fin)
{
    ...
    if (mdr->client_request && mdr->client_request->is_queued_for_replay()) {
        ...
        if (dn && dn->is_waiter_for(CDentry::WAIT_UNLINK_FINISH)) mdlog->flush();
    }
    else {
        mdlog->flush();
    }
 ...
 */
CDentry* Server::rdlock_path_xlock_dentry(MDRequestRef& mdr, bool create, bool okexist, bool want_layout)
{
    const filepath& refpath = mdr->get_filepath();
...
void Server::handle_client_setattr(MDRequestRef& mdr)
{
    ...
    else if ((mask & (CEPH_SETATTR_UID | CEPH_SETATTR_GID | CEPH_SETATTR_KILL_SGUID)) && S_ISREG(pi.inode->mode) &&
