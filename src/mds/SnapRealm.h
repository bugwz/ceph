...
public:
    ...
    bool has_past_parent_snaps() const { return !srnode.past_parent_snaps.empty(); }
    ...
    void invalidate_cached_snaps() { cached_seq = 0; }
    ...
    void add_cap(client_t client, Capability* cap)
    {
        ...
        if (client_caps_entry == client_caps.end())
            client_caps_entry = client_caps.emplace(client, new xlist<Capability*>).first;
