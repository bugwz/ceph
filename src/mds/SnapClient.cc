...
void SnapClient::resend_queries()
{
    if (!waiting_for_version.empty() || (!synced && sync_reqid > 0)) {
        ...
        refresh(want, NULL);
        if (!synced) sync_reqid = last_reqid;
    }
}
...
void SnapClient::handle_query_result(const cref_t<MMDSTableRequest>& m)
{
    ...
    case 'F':   // full
    {
        ...
        if (last_created > cached_last_created) cached_last_created = last_created;
        if (last_destroyed > cached_last_destroyed) cached_last_destroyed = last_destroyed;
...
void SnapClient::refresh(version_t want, MDSContext* onfinish)
{
    ...
    if (onfinish) waiting_for_version[want].push_back(onfinish);
...
void SnapClient::get_snaps(set<snapid_t>& result) const
{
    ...
    for (auto& p : cached_snaps) result.insert(p.first);

    for (auto tid : committing_tids) {
        auto q = cached_pending_update.find(tid);
        if (q != cached_pending_update.end()) result.insert(q->second.snapid);

        auto r = cached_pending_destroy.find(tid);
        if (r != cached_pending_destroy.end()) result.erase(r->second.first);
    }
}
...
set<snapid_t> SnapClient::filter(const set<snapid_t>& snaps) const
{
    ...
    for (auto p : snaps) {
        if (cached_snaps.count(p)) result.insert(p);
    }

    for (auto tid : committing_tids) {
        auto q = cached_pending_update.find(tid);
        if (q != cached_pending_update.end()) {
            if (snaps.count(q->second.snapid)) result.insert(q->second.snapid);
        }
...
const SnapInfo* SnapClient::get_snap_info(snapid_t snapid) const
{
    ...
    if (it != cached_snaps.end()) result = &it->second;
...
void SnapClient::get_snap_infos(map<snapid_t, const SnapInfo*>& infomap, const set<snapid_t>& snaps) const
{
    ceph_assert(cached_version > 0);
    ...
    for (auto p : snaps) {
        ...
        if (it != cached_snaps.end()) result[p] = &it->second;
    }

    for (auto tid : committing_tids) {
        auto q = cached_pending_update.find(tid);
        if (q != cached_pending_update.end()) {
            if (snaps.count(q->second.snapid)) result[q->second.snapid] = &q->second;
        }
...
int SnapClient::dump_cache(Formatter* f) const
{
    ...
    for (auto& p : cached_snaps) snaps[p.first] = &p.second;

    for (auto tid : committing_tids) {
        auto q = cached_pending_update.find(tid);
        if (q != cached_pending_update.end()) snaps[q->second.snapid] = &q->second;

        auto r = cached_pending_destroy.find(tid);
        if (r != cached_pending_destroy.end()) snaps.erase(r->second.first);
    }
