...
int MDBalancer::proc_message(const cref_t<Message>& m)
{
    ...
    case MSG_MDS_HEARTBEAT: handle_heartbeat(ref_cast<MHeartbeat>(m)); break;
...
{
    if (changed.count("mds_bal_fragment_dirs")) bal_fragment_dirs = g_conf().get_val<bool>("mds_bal_fragment_dirs");
...
void MDBalancer::handle_export_pins(void)
{
        ...
        for (auto&& dir : in->get_dirfrags()) {
            ...
            if (export_pin == MDS_RANK_EPHEMERAL_DIST) {
                if (dir->get_frag().bits() < min_frag_bits) {
                    if (!dir->state_test(CDir::STATE_CREATING) && !dir->is_frozen() && !dir->is_freezing()) {
            ...
            if (target == MDS_RANK_NONE) {
                ...
                else if (dir->state_test(CDir::STATE_CREATING) || dir->is_frozen() || dir->is_freezing()) {
                    // try again later
                    remove = false;
                    continue;
                }
    ...
    if (authsubs.size() > AUTH_TREES_THRESHOLD && !g_conf()->subsys.should_gather<ceph_subsys_mds, 25>()) {
        dout(15) << "number of auth trees = " << authsubs.size()
                 << "; not "
                    "printing auth trees"
                 << dendl;
    ...
    for (auto& cd : authsubs) {
        ...
        if (print_auth_subtrees) dout(25) << "auth tree " << *cd << " export_pin=" << export_pin << dendl;

        if (export_pin >= 0 && export_pin != mds->get_nodeid() && export_pin < mds->mdsmap->get_max_mds()) {
...
void MDBalancer::tick()
{
    ...
    // sample?
    if (chrono::duration<double>(now - last_sample).count() > g_conf()->mds_bal_sample_interval) {
    ...
    // balance?
    if (mds->get_nodeid() == 0 && mds->is_active() && bal_interval > 0 &&
...
{
public:
    explicit C_Bal_SendHeartbeat(MDSRank* mds_)
        : MDSInternalContext(mds_)
    {}
    void finish(int f) override { mds->balancer->send_heartbeat(); }
};
...
double mds_load_t::mds_load() const
{
    ...
    case 0: return .8 * auth.meta_load() + .2 * all.meta_load() + req_rate + 10.0 * queue_len;

    case 1: return req_rate + 10.0 * queue_len;

    case 2: return cpu_load_avg;
    }
    ceph_abort();
    return 0;
}
...
mds_load_t MDBalancer::get_load()
{
    ...
    uint64_t cpu_time = 1;
    {
        ...
        if (stat_file.is_open()) {
            vector<string> stat_vec(std::istream_iterator<string>{stat_file}, std::istream_iterator<string>());
            if (stat_vec.size() >= 15) {
                // utime + stime
                cpu_time = strtoll(stat_vec[13].c_str(), nullptr, 10) + strtoll(stat_vec[14].c_str(), nullptr, 10);
            }
            else {
    ...
    if (last_get_load != clock::zero() && now > last_get_load) {
        double el = std::chrono::duration<double>(now - last_get_load).count();
        if (el >= 1.0) {
            if (num_requests > last_num_requests) load.req_rate = (num_requests - last_num_requests) / el;
            if (cpu_time > last_cpu_time) load.cpu_load_avg = (cpu_time - last_cpu_time) / el;
            if (num_traverse > last_num_traverse && num_traverse_hit > last_num_traverse_hit)
                load.cache_hit_rate =
                    (double)(num_traverse_hit - last_num_traverse_hit) / (num_traverse - last_num_traverse);
        }
        else {
            ...
            if (num_requests >= last_num_requests && cpu_time >= last_cpu_time && num_traverse >= last_num_traverse &&
                num_traverse_hit >= last_num_traverse_hit)
                update_last = false;
        }
    }
...
int MDBalancer::localize_balancer()
{
    ...
    ceph_tid_t tid =
        mds->objecter->read(oid, oloc, 0, 0, CEPH_NOSNAP, &lua_src, 0, new C_SafeCond(lock, cond, &ack, &r));
...
void MDBalancer::send_heartbeat()
{
    ...
    auto em = mds_load.emplace(
        std::piecewise_construct, std::forward_as_tuple(mds->get_nodeid()), std::forward_as_tuple(load));
    if (!em.second) {
...
void MDBalancer::handle_heartbeat(const cref_t<MHeartbeat>& m)
{
    ...
    dout(25) << "=== got heartbeat " << m->get_beat() << " from " << m->get_source().num() << " " << m->get_load()
             << dendl;

    if (!mds->is_active()) return;
    ...
    {
        auto em = mds_load.emplace(
            std::piecewise_construct, std::forward_as_tuple(who), std::forward_as_tuple(m->get_load()));
        if (!em.second) {
        ...
        if (mds_load.size() == cluster_size) {
            ...
            if (mds->mdsmap->get_balancer() != "") {
                ...
                mds->clog->warn() << "using old balancer; mantle failed for "
                                  << "balancer=" << mds->mdsmap->get_balancer() << " : " << cpp_strerror(r);
            }
...
double MDBalancer::try_match(balance_state_t& state, mds_rank_t ex, double& maxex, mds_rank_t im, double& maxim)
{
    ...
    if (ex == mds->get_nodeid()) state.targets[im] += howmuch;
...
void MDBalancer::queue_split(const CDir* dir, bool fast)
{
    ...
    auto callback = [this, df](int r) {
        ...
        if (dir->inode->is_ephemeral_dist()) {
            ...
            if (df.frag.bits() + bits < min_frag_bits) bits = min_frag_bits - df.frag.bits();
        }
    ...
    if (fast) {
        ...
        mds->queue_waiter(new MDSInternalContextWrapper(mds, new LambdaContext(std::move(callback))));
    }
    else if (is_new) {
        ...
        // before we freeze it.
        mds->timer.add_event_after(bal_fragment_interval, new LambdaContext(std::move(callback)));
    }
}
...
void MDBalancer::queue_merge(CDir* dir)
{
    ...
    auto callback = [this, frag](int r) {
        ...
        unsigned min_frag_bits = 0;
        if (diri->is_ephemeral_dist()) min_frag_bits = mdcache->get_ephemeral_dist_frag_bits();
        ...
        if (fg != dir->get_frag()) mdcache->merge_dir(diri, fg);
    };
...
void MDBalancer::prep_rebalance(int beat)
{
    ...
    if (g_conf()->mds_thrash_exports) {
        ...
        if ((m != mds_load.end()) && (m->second.mds_load() > 0)) {
            ...
            dout(7) << " load_fac is " << load_fac << " <- " << m->second.auth << " " << metald << " / " << mdsld
                    << dendl;
        }
        ...
        for (mds_rank_t i = mds_rank_t(0); i < mds_rank_t(cluster_size); i++) {
            ...
            if (whoami == 0) dout(7) << "  mds." << i << " " << load << " = " << load.mds_load() << " ~ " << l << dendl;
        ...
        for (multimap<double, mds_rank_t>::iterator it = load_map.begin(); it != load_map.end(); ++it) {
        ...
        if (true) {
            ...
            // big -> small exporters
            for (multimap<double, mds_rank_t>::reverse_iterator ex = exporters.rbegin(); ex != exporters.rend(); ++ex) {
        ...
        if (beat % 2 == 1) {
            ...
            while (ex != exporters.rend() && im != importers.end()) {
                double maxex = get_maxex(state, ex->second);
            ...
            multimap<double, mds_rank_t>::iterator im = importers.begin();
            while (ex != exporters.end() && im != importers.end()) {
                double maxex = get_maxex(state, ex->second);
                double maxim = get_maxim(state, im->second);
                if (maxex < .001 || maxim < .001) break;
                try_match(state, ex->second, maxex, im->second, maxim);
...
int MDBalancer::mantle_prep_rebalance()
{
    ...
    if (bal_version != mds->mdsmap->get_balancer()) {
        ...
        if (mds->get_nodeid() == 0) mds->clog->info() << "mantle balancer version changed: " << bal_version;
    }
...
{
    if (g_conf()->mds_thrash_exports) {
        dout(5) << "mds_thrash is on; not performing standard rebalance operation!" << dendl;
        return;
    }
    ...
    for (auto& dir : mds->mdcache->get_fullauth_subtrees()) {
        CInode* diri = dir->get_inode();
        if (diri->is_mdsdir()) continue;
        if (diri->get_export_pin(false) != MDS_RANK_NONE) continue;
        ...
        if (g_conf()->mds_bal_idle_threshold > 0 && pop < g_conf()->mds_bal_idle_threshold &&
            diri != mds->mdcache->get_root() && from != mds->get_nodeid()) {
    ...
    for (auto& it : state.targets) {
        ...
        if (amount * 10 * state.targets.size() < target_load) continue;

        dout(5) << "want to send " << amount << " to mds."
                << target
                //<< " .. " << (*it).second << " * " << load_fac
                << " -> " << amount << dendl;   //" .. fudge is " << fudge << dendl;
        ...
        if (import_from_map.count(target)) {
            ...
            for (auto p = import_from_map.equal_range(target); p.first != p.second;) {
                ...
                if (pop <= amount - have) {
                    ...
                    for (auto q = import_pop_map.equal_range(pop); q.first != q.second;) {
    ...
    for (auto& it : state.targets) {
        ...
        if (!export_pop_map.count(target)) continue;
        double& have = export_pop_map[target];
        if (amount - have < MIN_OFFLOAD) continue;

        for (auto p = import_pop_map.begin(); p != import_pop_map.end();) {
            CDir* dir = p->second;
    ...
    for (auto& it : state.targets) {
        ...
        for (auto p = import_pop_map.rbegin(); p != import_pop_map.rend(); ++p) {
            CDir* dir = p->second;
        ...
        for (const auto& dir : exports) {
            dout(5) << "   - exporting " << dir->pop_auth_subtree << " " << dir->pop_auth_subtree.meta_load()
...
void MDBalancer::find_exports(CDir* dir, double amount, std::vector<CDir*>* exports, double& have,
                              set<CDir*>& already_exporting)
{
    auto now = clock::now();
    ...
    if (need < amount * g_conf()->mds_bal_min_start) return;   // good enough!
    ...
    dout(7) << "in " << dir_pop << " " << *dir << " need " << need << " (" << needmin << " - " << needmax << ")"
            << dendl;

    double subdir_sum = 0;
    for (elist<CInode*>::iterator it = dir->pop_lru_subdirs.begin_use_current(); !it.end();) {
        CInode* in = *it;
        ++it;
        ...
        for (const auto& subdir : dfls) {
            ...
            if (subdir->is_frozen_dir() || subdir->is_frozen_tree_root() || subdir->is_freezing_dir() ||
                subdir->is_freezing_tree_root())
...
void MDBalancer::hit_inode(CInode* in, int type, int who)
{
    ...
    if (in->get_parent_dn()) hit_dir(in->get_parent_dn()->get_dir(), type, who);
}

void MDBalancer::maybe_fragment(CDir* dir, bool hot)
{
    // split/merge
    if (bal_fragment_dirs && bal_fragment_interval > 0 && dir->is_auth() &&
        ...
        // merge?
        if (dir->get_frag() != frag_t() && dir->should_merge() && merge_pending.count(dir->dirfrag()) == 0) {
            queue_merge(dir);
        }
    }
}
...
void MDBalancer::hit_dir(CDir* dir, int type, int who, double amount)
{
    ...
    dout(20) << type << " pop is " << v << ", frag " << dir->get_frag() << " size " << dir->get_frag_size() << " "
             << dir->pop_me << dendl;
    ...
    double rd_adj = 0.0;
    if (type == META_POP_IRD && dir->last_popularity_sample < last_sample) {
        ...
        if (pop_sp > 0) {
            dout(20) << type << " pop " << dir_pop << " spread " << pop_sp << " " << dir->pop_spread.last[0] << " "
                     << dir->pop_spread.last[1] << " " << dir->pop_spread.last[2] << " " << dir->pop_spread.last[3]
                     << " in " << *dir << dendl;
        }

        if (dir->is_auth() && !dir->is_ambiguous_auth()) {
            if (dir->can_rep() && dir_pop >= g_conf()->mds_bal_replicate_threshold) {
                // replicate
                double rdp = dir->pop_me.get(META_POP_IRD).get();
                rd_adj = rdp / mds->get_mds_map()->get_num_in_mds() - rdp;
                rd_adj /= 2.0;   // temper somewhat

                dout(5) << "replicating dir " << *dir << " pop " << dir_pop << " .. rdp " << rdp << " adj " << rd_adj
                        << dendl;
            ...
            if (dir->ino() != 1 && dir->is_rep() && dir_pop < g_conf()->mds_bal_unreplicate_threshold) {
                // unreplicate
    ...
    while (true) {
        ...
        if (rd_adj != 0.0) dir->pop_nested.get(META_POP_IRD).adjust(rd_adj);

        if (hit_subtree) {
            dir->pop_auth_subtree.get(type).hit(amount);

            if (rd_adj != 0.0) dir->pop_auth_subtree.get(META_POP_IRD).adjust(rd_adj);
        ...
        if (hit_subtree_nested) {
            ...
            if (rd_adj != 0.0) dir->pop_auth_subtree_nested.get(META_POP_IRD).adjust(rd_adj);
        }
        if (!pdir) break;
        dir = pdir;
    }
...
void MDBalancer::adjust_pop_for_rename(CDir* pdir, CDir* dir, bool inc)
{
    ...
    while (true) {
        if (inc) {
            ...
            if (adjust_subtree_nest) pdir->pop_auth_subtree_nested.add(dir->pop_auth_subtree_nested);
        }
        else {
            pdir->pop_nested.sub(dir->pop_nested);
            if (adjust_subtree) pdir->pop_auth_subtree.sub(dir->pop_auth_subtree);

            if (adjust_subtree_nest) pdir->pop_auth_subtree_nested.sub(dir->pop_auth_subtree_nested);
        }

        if (pdir->is_subtree_root()) adjust_subtree = false;
        cur = pdir;
    ...
    while (!dfs.empty()) {
        ...
        for (auto it = dir->begin(); it != dir->end(); ++it) {
            ...
            for (const auto& subdir : ls) {
                if (subdir->pop_nested.meta_load() < .001) continue;
