...
{
    switch (state) {
    case ScrubStack::STATE_RUNNING: os << "RUNNING"; break;
    case ScrubStack::STATE_IDLE: os << "IDLE"; break;
    case ScrubStack::STATE_PAUSING: os << "PAUSING"; break;
    case ScrubStack::STATE_PAUSED: os << "PAUSED"; break;
    default: ceph_abort();
    }
...
{
    // abort in progress
    if (clear_stack) return -CEPHFS_EAGAIN;
...
public:
    C_RetryScrub(ScrubStack* s, MDSCacheObject* o)
        : MDSInternalContext(s->mdcache->mds)
        , stack(s)
        , obj(o)
    {
        stack->add_to_waiting(obj);
    }
    void finish(int r) override { stack->remove_from_waiting(obj); }
...
void ScrubStack::kick_off_scrubs()
{
    ...
    if (clear_stack || state == STATE_PAUSING || state == STATE_PAUSED) {
        if (scrubs_in_progress == 0) {
            dout(10) << __func__ << ": in progress scrub operations finished, " << stack_size << " in the stack"
                     << dendl;
    ...
    dout(20) << __func__ << " entering with " << scrubs_in_progress
             << " in "
                "progress and "
    ...
    while (g_conf()->mds_max_scrub_ops_in_progress > scrubs_in_progress) {
        ...
        if (CInode* in = dynamic_cast<CInode*>(*it)) {
            ...
            if (!validate_inode_auth(in)) continue;
...
bool ScrubStack::validate_inode_auth(CInode* in)
{
    if (in->is_auth()) {
        ...
        if (in->is_ambiguous_auth()) {
            ...
            auto ret =
                remote_scrubs.emplace(std::piecewise_construct, std::forward_as_tuple(in), std::forward_as_tuple());
            ...
            auto r = make_message<MMDSScrub>(MMDSScrub::OP_QUEUEINO,
                                             in->ino(),
                                             std::move(in->scrub_queued_frags()),
                                             header->get_tag(),
                                             header->get_origin(),
                                             header->is_internal_tag(),
                                             header->get_force(),
...
void ScrubStack::scrub_dir_inode(CInode* in, bool* added_children, bool* done)
{
    ...
    for (auto& fg : frags) {
        if (queued.contains(fg)) continue;
    ...
    if (!scrub_remote.empty()) {
        auto ret = remote_scrubs.emplace(std::piecewise_construct, std::forward_as_tuple(in), std::forward_as_tuple());
        ceph_assert(ret.second);   // FIXME: parallel scrubs?
        auto& scrub_r = ret.first->second;
        scrub_r.tag = header->get_tag();
        ...
        for (auto& p : scrub_remote) {
            ...
            auto r = make_message<MMDSScrub>(MMDSScrub::OP_QUEUEDIR,
                                             in->ino(),
                                             std::move(p.second),
                                             header->get_tag(),
                                             header->get_origin(),
...
void ScrubStack::scrub_dirfrag(CDir* dir, bool* done)
{
    ...
    if (header->get_recursive()) {
        ...
        for (auto it = dir->begin(); it != dir->end(); /* nop */) {
            ...
            if (dn->scrub(next_seq)) {
                ...
                clog->warn() << "Scrub error on dentry " << *dn << " see " << g_conf()->name
            ...
            if (dn->get_version() <= last_scrub && dnl->get_remote_d_type() != DT_DIR && !header->get_force()) {
                dout(15) << __func__ << " skip dentry " << dnk << ", no change since last scrub" << dendl;
                continue;
            }
            if (dnl->is_primary()) {
    ...
    if (!dir->scrub_local()) {
        std::string path;
        dir->get_inode()->make_path_string(path, true);
        clog->warn() << "Scrub error on dir " << dir->ino() << " (" << path << ") see " << g_conf()->name
                     << " log and `damage ls` output for details";
    }
...
void ScrubStack::_validate_inode_done(CInode* in, int r, const CInode::validated_data& result)
{
    ...
    if (result.backtrace.checked && !result.backtrace.passed && !result.backtrace.repaired) {
    ...
    else if (result.inode.checked && !result.inode.passed && !result.inode.repaired) {
    ...
    if (!result.passed_validation) {
        if (result.all_damage_repaired()) {
            clog->info() << "Scrub repaired inode " << in->ino() << " (" << path << ")";
        }
        else {
            clog->warn() << "Scrub error on inode " << in->ino() << " (" << path << ") see " << g_conf()->name
                         << " log and `damage ls` output for details";
        }
...
std::string_view ScrubStack::scrub_summary()
{
    ...
    if (state == STATE_IDLE) {
        if (scrubbing_map.empty()) return "idle";
        *cs << "idle+waiting";
    }
...
void ScrubStack::abort_pending_scrubs()
{
    ...
    for (auto it = scrub_stack.begin(); !it.end(); ++it) abort_one(*it);
    for (auto it = scrub_waiting.begin(); !it.end(); ++it) abort_one(*it);

    stack_size = 0;
    scrub_stack.clear();
    scrub_waiting.clear();

    for (auto& p : remote_scrubs) remove_from_waiting(p.first, false);
    remote_scrubs.clear();
...
void ScrubStack::scrub_abort(Context* on_finish)
{
    ...
    dout(10) << __func__ << ": aborting with " << scrubs_in_progress << " scrubs in progress and " << stack_size
             << " in the"
             << " stack" << dendl;
    ...
    if (scrub_in_transition_state()) {
        if (on_finish) control_ctxs.push_back(on_finish);
        return;
    }

    abort_pending_scrubs();
    if (state != STATE_PAUSED) set_state(STATE_IDLE);
...
void ScrubStack::scrub_pause(Context* on_finish)
{
    ...
    dout(10) << __func__ << ": pausing with " << scrubs_in_progress << " scrubs in progress and " << stack_size
             << " in the"
             << " stack" << dendl;

    if (mdcache->mds->get_nodeid() == 0) send_state_message(MMDSScrub::OP_PAUSE);

    // abort is in progress
    if (clear_stack) {
        if (on_finish) on_finish->complete(-CEPHFS_EINVAL);
        return;
    }
...
bool ScrubStack::scrub_resume()
{
    ...
    if (mdcache->mds->get_nodeid() == 0) send_state_message(MMDSScrub::OP_RESUME);
...
{
    switch (m->get_type()) {
    case MSG_MDS_SCRUB: handle_scrub(ref_cast<MMDSScrub>(m)); break;

    case MSG_MDS_SCRUB_STATS: handle_scrub_stats(ref_cast<MMDSScrubStats>(m)); break;
...
void ScrubStack::handle_scrub(const cref_t<MMDSScrub>& m)
{
    ...
    case MMDSScrub::OP_QUEUEDIR:
    {
        ...
        if (!dfs.empty()) {
            ...
            if (auto it = scrubbing_map.find(m->get_tag()); it != scrubbing_map.end()) {
                ...
                header = std::make_shared<ScrubHeader>(
                    m->get_tag(), m->is_internal_tag(), m->is_force(), m->is_recursive(), m->is_repair());
        ...
        auto r = make_message<MMDSScrub>(MMDSScrub::OP_QUEUEDIR_ACK, m->get_ino(), std::move(queued), m->get_tag());
    ...
    case MMDSScrub::OP_QUEUEDIR_ACK:
    {
        ...
        if (it != remote_scrubs.end() && m->get_tag() == it->second.tag) {
            if (it->second.gather_set.erase(from)) {
                auto& queued = diri->scrub_queued_frags();
                for (auto& fg : m->get_frags()) queued.insert_raw(fg);
                queued.simplify();
    ...
    case MMDSScrub::OP_QUEUEINO:
    {
        ...
        fragset_t queued;
        auto r = make_message<MMDSScrub>(MMDSScrub::OP_QUEUEINO_ACK, m->get_ino(), std::move(queued), m->get_tag());
        mdcache->mds->send_message_mds(r, from);
    } break;
    case MMDSScrub::OP_QUEUEINO_ACK:
    {
        ...
        if (it != remote_scrubs.end() && m->get_tag() == it->second.tag && it->second.gather_set.erase(from)) {
    ...
    case MMDSScrub::OP_ABORT: scrub_abort(nullptr); break;
    case MMDSScrub::OP_PAUSE: scrub_pause(nullptr); break;
    case MMDSScrub::OP_RESUME: scrub_resume(); break;
    default:
...
void ScrubStack::handle_scrub_stats(const cref_t<MMDSScrubStats>& m)
{
    ...
    if (from == 0) {
        if (scrub_epoch != m->get_epoch() - 1) {
            ...
            for (auto& p : scrubbing_map) {
                if (p.second->get_epoch_last_forwarded()) p.second->set_epoch_last_forwarded(scrub_epoch);
            }
        }
        ...
        for (auto it = scrubbing_map.begin(); it != scrubbing_map.end();) {
            ...
            if (header->get_num_pending() || header->get_epoch_last_forwarded() >= scrub_epoch) {
                scrubbing_tags.insert(it->first);
                ++it;
            }
            else if (m->is_finished(it->first)) {
                any_finished = true;
                if (header->get_repaired()) any_repaired = true;
        ...
        auto ack = make_message<MMDSScrubStats>(scrub_epoch, std::move(scrubbing_tags), clear_stack);
        mdcache->mds->send_message_mds(ack, 0);

        if (any_finished) clog_scrub_summary();
        if (any_repaired) mdcache->mds->mdlog->trim_all();
    }
    else {
        if (scrub_epoch == m->get_epoch() && (size_t)from < mds_scrub_stats.size()) {
...
{
    if (!scrub_any_peer_aborting && scrubbing_map.empty()) return;
    ...
    if (up_max == 0) {
        ...
        for (const auto& stat : mds_scrub_stats) {
            if (stat.aborting || stat.epoch_acked <= scrub_epoch_last_abort) any_aborting = true;
            if (stat.epoch_acked != scrub_epoch) {
                fully_acked = false;
                continue;
            }
            scrubbing_tags.insert(stat.scrubbing_tags.begin(), stat.scrubbing_tags.end());
        }
        if (!any_aborting) scrub_any_peer_aborting = false;
        if (fully_acked) {
            ...
            if (scrub_epoch_fully_acked + 1 == scrub_epoch) update_scrubbing = true;
            scrub_epoch_fully_acked = scrub_epoch;
        }
    }

    if (mds_scrub_stats.size() != (size_t)up_max + 1) mds_scrub_stats.resize((size_t)up_max + 1);
    ...
    for (auto it = scrubbing_map.begin(); it != scrubbing_map.end();) {
        auto& header = it->second;
        if (header->get_num_pending() || header->get_epoch_last_forwarded() >= scrub_epoch) {
            if (update_scrubbing && up_max != 0) scrubbing_tags.insert(it->first);
            ++it;
        }
    ...
    for (auto& r : up_mds) {
        if (r == 0) continue;
        auto m = update_scrubbing ? make_message<MMDSScrubStats>(scrub_epoch, scrubbing_tags)
...
void ScrubStack::handle_mds_failure(mds_rank_t mds)
{
    ...
    for (auto it = remote_scrubs.begin(); it != remote_scrubs.end();) {
        if (it->second.gather_set.erase(mds) && it->second.gather_set.empty()) {
            CInode* in = it->first;
            remote_scrubs.erase(it++);
            remove_from_waiting(in, false);
            kick = true;
