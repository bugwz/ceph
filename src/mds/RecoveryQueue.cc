...
public:
    ...
    void print(ostream& out) const override { out << "file_recover(" << in->ino() << ")"; }

    uint64_t size = 0;
    utime_t mtime;

protected:
    void finish(int r) override { rq->_recovered(in, r, size, mtime); }

    MDSRank* get_mds() override { return rq->mds; }
...
RecoveryQueue::RecoveryQueue(MDSRank* mds_)
    ...
    , mds(mds_)
    , filer(mds_->objecter, mds_->finisher)
{}
...
{
    dout(10) << file_recover_queue_size << " queued, " << file_recover_queue_front_size << " prioritized, "
...
void RecoveryQueue::_start(CInode* in)
{
    ...
    if (pi->client_ranges.size() && pi->get_max_size()) {
        ...
        if (p == file_recovering.end()) {
            ...
            filer.probe(in->ino(), &layout, in->last, pi->get_max_size(), &fin->size, &fin->mtime, false, 0, fin);
        }
        else {
            p->second = true;
...
{
    return in->item_recover_queue.is_on_list() || in->item_recover_queue_front.is_on_list();
}
