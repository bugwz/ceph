...
void OpenFileTable::get_ref(CInode* in, frag_t fg)
{
    do {
        ...
        auto ret = anchor_map.emplace(
            ...
            std::forward_as_tuple(in->ino()),
            std::forward_as_tuple(
                in->ino(), (pin ? pin->ino() : inodeno_t(0)), (dn ? dn->get_name() : string()), in->d_type(), 1));
        ceph_assert(ret.second == true);
        in->state_set(CInode::STATE_TRACKEDBYOFT);

        if (fg != -1U) ret.first->second.frags.insert(fg);
...
public:
    C_IO_OFT_Save(OpenFileTable* t, uint64_t s, MDSContext* c)
        ...
        , fin(c)
    {}
    void finish(int r) { oft->_commit_finish(r, log_seq, fin); }
    void print(ostream& out) const override { out << "openfiles_save"; }
};
...
public:
    ...
    void finish(int r) { oft->_journal_finish(r, log_seq, fin, ops_map); }
    void print(ostream& out) const override { out << "openfiles_journal"; }
};
                                    ...
                                    std::map<unsigned, std::vector<ObjectOperation> >& ops_map)
{
    ...
    C_GatherBuilder gather(g_ceph_context, new C_OnFinisher(new C_IO_OFT_Save(this, log_seq, c), mds->finisher));
    SnapContext snapc;
    object_locator_t oloc(mds->get_metadata_pool());
    for (auto& [idx, vops] : ops_map) {
        object_t oid = get_object_name(idx);
        for (auto& op : vops) {
            mds->objecter->mutate(oid, oloc, op, snapc, ceph::real_clock::now(), 0, gather.new_sub());
        }
    }
    gather.activate();
...
{
    dout(10) << __func__ << " log_seq " << log_seq << " committing_log_seq:" << committing_log_seq << dendl;
...
#else
        ctl.journaled_update.insert(make_move_iterator(begin(ctl.to_update)), make_move_iterator(end(ctl.to_update)));
        ctl.journaled_remove.insert(make_move_iterator(begin(ctl.to_remove)), make_move_iterator(end(ctl.to_remove)));
#endif
        ctl.to_update.clear();
        ctl.to_remove.clear();
    };
    ...
    auto submit_ops_func = [&]() {
        gather.set_finisher(new C_OnFinisher(new C_IO_OFT_Save(this, log_seq, c), mds->finisher));
        for (auto& [idx, vops] : ops_map) {
            object_t oid = get_object_name(idx);
            for (auto& op : vops) {
                mds->objecter->mutate(oid, oloc, op, snapc, ceph::real_clock::now(), 0, gather.new_sub());
    ...
    for (auto& [ino, state] : dirty_items) {
        ...
        if (p != anchor_map.end()) {
            ...
            ceph_assert(count > 0);
            --count;
            if ((unsigned)omap_idx < first_free_idx && count < MAX_ITEMS_PER_OBJ) first_free_idx = omap_idx;
        }
    ...
    size_t total_items = 0;
    {
        ...
        for (unsigned i = 0; i < omap_num_objs; i++) {
            ...
            if (omap_num_items[i] > 0) used_objs = i + 1;
        }
        ...
        // does not change.
        if (!journaled && old_num_objs == omap_num_objs && objs_to_write.size() <= 1) {
    ...
    for (unsigned omap_idx = 0; omap_idx < omap_updates.size(); omap_idx++) {
        ...
        if (ctl.journal_idx == 0) ceph_assert(ctl.journaled_update.empty() && ctl.journaled_remove.empty());
        ...
        if (ctl.clear || ctl.journal_idx > 0 || (omap_idx == 0 && old_num_objs != omap_num_objs))
            create_op_func(omap_idx, first);
    }

    ceph_assert(!ops_map.empty());
    if (journal_state == JOURNAL_FINISH) {
        gather.set_finisher(new C_OnFinisher(new C_IO_OFT_Journal(this, log_seq, c, ops_map), mds->finisher));
        gather.activate();
    }
    else {
        submit_ops_func();
...
public:
    ...
    C_IO_OFT_Load(OpenFileTable* t, unsigned i, bool f)
        ...
        , first(f)
    {}
    void finish(int r) override { oft->_load_finish(r, header_r, values_r, index, first, more, header_bl, values); }
    void print(ostream& out) const override { out << "openfiles_load"; }
};
...
public:
    C_IO_OFT_Recover(OpenFileTable* t)
        : oft(t)
    {}
    void finish(int r) override { oft->_recover_finish(r); }
    void print(ostream& out) const override { out << "openfiles_recover"; }
};
...
void OpenFileTable::_read_omap_values(const std::string& key, unsigned idx, bool first)
{
    object_t oid = get_object_name(idx);
    ...
    ObjectOperation op;
    if (first) op.omap_get_header(&c->header_bl, &c->header_r);
    op.omap_get_vals(key, "", uint64_t(-1), &c->values, &c->more, &c->values_r);
    mds->objecter->read(oid, oloc, op, CEPH_NOSNAP, nullptr, 0, new C_OnFinisher(c, mds->finisher));
}

void OpenFileTable::_load_finish(int op_r, int header_r, int values_r, unsigned idx, bool first, bool more,
                                 bufferlist& header_bl, std::map<std::string, bufferlist>& values)
{
    using ceph::decode;
    ...
    auto decode_func = [this](unsigned idx, inodeno_t ino, bufferlist& bl) {
        ...
        auto it = loaded_anchor_map.emplace_hint(
            loaded_anchor_map.end(), std::piecewise_construct, std::make_tuple(ino), std::make_tuple());
        ...
        if (loaded_anchor_map.size() > count) ++omap_num_items[idx];
    };
    ...
    try {
        if (first) {
            ...
            if (version > omap_version) {
                ...
                if (jstate > journal_state) journal_state = jstate;
            }
        }

        for (auto& it : values) {
            if (it.first.compare(0, 9, "_journal.") == 0) {
                if (idx >= loaded_journals.size()) loaded_journals.resize(idx + 1);
    ...
    if (loaded_journals.size() > 0) {
        ...
        C_GatherBuilder gather(g_ceph_context, new C_OnFinisher(new C_IO_OFT_Recover(this), mds->finisher));
        object_locator_t oloc(mds->get_metadata_pool());
        SnapContext snapc;
        ...
        for (unsigned omap_idx = 0; omap_idx < loaded_journals.size(); omap_idx++) {
            ...
            try {
                for (auto& it : loaded_journal) {
                    ...
                    if (!to_update.empty()) op.omap_set(to_update);
                    if (!to_remove.empty()) op.omap_rm_keys(to_remove);
                }
            }
            catch (buffer::error& e) {
            ...
            }
            {
                // remove journal
                std::set<string> to_remove;
                for (auto& it : loaded_journal) to_remove.emplace(it.first);
...
void OpenFileTable::_get_ancestors(const Anchor& parent, vector<inode_backpointer_t>& ancestors, mds_rank_t& auth_hint)
{
    ...
    while (true) {
        ...
        if (p == loaded_anchor_map.end()) break;

        if (first) auth_hint = p->second.auth;

        dirino = p->second.dirino;
        d_name = p->second.d_name;
        if (dirino == inodeno_t(0)) break;
...
public:
    C_OFT_OpenInoFinish(OpenFileTable* t, inodeno_t i)
        ...
        , ino(i)
    {}
    void finish(int r) override { oft->_open_ino_finish(ino, r); }
};
...
void OpenFileTable::_open_ino_finish(inodeno_t ino, int r)
{
    ...
    if (r != mds->get_nodeid()) mds->mdcache->rejoin_prefetch_ino_finish(ino, r);
...
void OpenFileTable::_prefetch_dirfrags()
{
    ...
    for (auto& [ino, anchor] : loaded_anchor_map) {
        if (anchor.frags.empty()) continue;
        ...
        for (auto& fg : anchor.frags) {
            ...
            if (dir) {
                if (dir->is_auth() && !dir->is_complete()) fetch_queue.push_back(dir);
            }
            else {
                frag_vec_t leaves;
                ...
                for (auto& leaf : leaves) {
                    ...
                    if (dir && dir->is_auth() && !dir->is_complete()) fetch_queue.push_back(dir);
                }
            }
        }
    }
    ...
    for (const auto& dir : fetch_queue) {
        ...
        if (!(++num_opening_dirfrags % mds->heartbeat_reset_grace())) mds->heartbeat_reset();
    }
    ...
    if (gather.has_subs()) {
        gather.set_finisher(new MDSInternalContextWrapper(mds, new LambdaContext(std::move(finish_func))));
        gather.activate();
    }
    else {
        finish_func(0);
...
void OpenFileTable::_prefetch_inodes()
{
    ...
    if (destroyed_inos_set.empty()) {
        for (auto& it : logseg_destroyed_inos) destroyed_inos_set.insert(it.second.begin(), it.second.end());
    }

    for (auto& [ino, anchor] : loaded_anchor_map) {
        if (destroyed_inos_set.count(ino)) continue;
        if (anchor.d_type == DT_DIR) {
            if (prefetch_state != DIR_INODES) continue;
            ...
            if (prefetch_state != FILE_INODES) continue;
        ...
        if (!(num_opening_inodes % mds->heartbeat_reset_grace())) mds->heartbeat_reset();
    }
...
bool OpenFileTable::prefetch_inodes()
{
    ...
    if (!load_done) {
        wait_for_load(new MDSInternalContextWrapper(mds, new LambdaContext([this](int r) { _prefetch_inodes(); })));
        return true;
    }
...
bool OpenFileTable::should_log_open(CInode* in)
{
    if (in->state_test(CInode::STATE_TRACKEDBYOFT)) {
        ...
        if (in->last_journaled >= committing_log_seq) return false;
        // item not dirty. it means the item has already been saved
        auto p = dirty_items.find(in->ino());
        if (p == dirty_items.end()) return false;
    }
    return true;
}
