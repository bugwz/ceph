...
void SnapServer::reset_state()
{
    ...
    if (mds) {   // only if I'm running in a live MDS
        ...
        if (first_free > last_snap) last_snap = first_free;
    }
...
void SnapServer::_get_reply_buffer(version_t tid, bufferlist* pbl) const
{
    ...
    if (q != pending_destroy.end()) {
        if (pbl) encode(q->second.second, *pbl);
        return;
    }
    auto r = pending_noop.find(tid);
    if (r != pending_noop.end()) {
        if (pbl) encode(last_snap, *pbl);
        return;
    }
...
void SnapServer::_commit(version_t tid, cref_t<MMDSTableRequest> req)
{
    if (pending_update.count(tid)) {
        ...
        if (snaps.count(info.snapid)) {
            opname = "update";
            if (info.stamp == utime_t()) info.stamp = snaps[info.snapid].stamp;
        }
        else {
            opname = "create";
            if (info.snapid > last_created) last_created = info.snapid;
        }
        ...
        snaps.erase(sn);
        if (seq > last_destroyed) last_destroyed = seq;
...
void SnapServer::_server_update(bufferlist& bl)
{
    ...
    for (map<int, vector<snapid_t> >::iterator p = purge.begin(); p != purge.end(); ++p) {
        for (vector<snapid_t>::iterator q = p->second.begin(); q != p->second.end(); ++q)
            need_to_purge[p->first].erase(*q);
        if (need_to_purge[p->first].empty()) need_to_purge.erase(p->first);
    }
}
...
void SnapServer::check_osd_map(bool force)
{
    ...
    // snap.
    mds->objecter->with_osdmap([this, &all_purged, &all_purge](const OSDMap& osdmap) {
...
void SnapServer::dump(Formatter* f) const
{
    ...
    for (map<version_t, pair<snapid_t, snapid_t> >::const_iterator i = pending_destroy.begin();
         i != pending_destroy.end();
         ++i) {
        f->open_object_section("snap");
        f->dump_unsigned("version", i->first);
...
bool SnapServer::force_update(snapid_t last, snapid_t v2_since, map<snapid_t, SnapInfo>& _snaps)
{
    bool modified = false;
    if (last > last_snap) {
