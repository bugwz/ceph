...
{
    out << "[client_name=" << client_name << ", cluster_name=" << cluster_name << ", fs_name=" << fs_name << "]"
        << std::endl;
}
...
void FSMap::print(ostream& out) const
{
    ...
    out << "enable_multiple, ever_enabled_multiple: " << enable_multiple << "," << ever_enabled_multiple << std::endl;
...
void FSMap::print_fs_summary(ostream& out) const
{
    ...
    if (!filesystems.empty()) {
        ...
        out << "    volumes: " << num_healthy << "/" << filesystems.size() << " healthy";
        if (num_recovering) {
...
void FSMap::print_summary(Formatter* f, ostream* out) const
{
    ...
    for (const auto& [gid, fscid] : mds_roles) {
        if (fscid == FS_CLUSTER_ID_NONE) continue;
            ...
            if (filesystems.size() > 1) {
                ...
                for (const auto& [role, status] : by_rank) {
                    ...
                    *css << fs_name << ":" << role.rank;
                    pretty.emplace(std::piecewise_construct,
                                   std::forward_as_tuple(css->strv()),
                                   std::forward_as_tuple(status.second));
...
mds_gid_t Filesystem::get_standby_replay(mds_gid_t who) const
{
    for (const auto& i : mds_map.mds_info) {
        ...
        if (info.state == MDSMap::STATE_STANDBY_REPLAY && info.rank == mds_map.mds_info.at(who).rank) {
...
Filesystem::ref FSMap::create_filesystem(std::string_view name, int64_t metadata_pool, int64_t data_pool,
...
void FSMap::get_health(list<pair<health_status_t, string>>& summary, list<pair<health_status_t, string>>* detail) const
{
    ...
    for (const auto& i : filesystems) {
        ...
        standby_count_wanted =
            std::max(standby_count_wanted, fs->mds_map.get_standby_count_wanted((mds_rank_t)standby_daemons.size()));
    }

    if (standby_count_wanted) {
        CachedStackStringStream css;
        *css << "insufficient standby daemons available: have " << standby_daemons.size() << "; want "
             << standby_count_wanted << " more";
...
void FSMap::get_health_checks(health_check_map_t* checks) const
{
    ...
    for (const auto& i : filesystems) {
        ...
        if (!stuck_failed.empty()) {
            health_check_t& fscheck = checks->get_or_add(
                "FS_WITH_FAILED_MDS", HEALTH_WARN, "%num% filesystem%plurals% %hasorhave% a failed mds daemon", 1);
            CachedStackStringStream css;
            *css << "fs " << fs->mds_map.fs_name << " has " << stuck_failed.size() << " failed mds"
                 << (stuck_failed.size() > 1 ? "s" : "");
    ...
    if (standby_count_wanted) {
        ...
        *css2 << "have " << standby_daemons.size() << "; want " << standby_count_wanted << " more";
...
void FSMap::decode(bufferlist::const_iterator& p)
{
    ...
    decode(enable_multiple, p);
    {
        ...
        filesystems.clear();
        for (auto& ref : v) {
            auto em = filesystems.emplace(
                std::piecewise_construct, std::forward_as_tuple(ref->fscid), std::forward_as_tuple(std::move(ref)));
...
int FSMap::parse_filesystem(std::string_view ns_str, Filesystem::const_ref* result) const
{
    std::string ns_err;
    std::string s(ns_str);
...
{
    out << "Filesystem '" << mds_map.fs_name << "' (" << fscid << ")" << std::endl;
    mds_map.print(out);
    if (mirror_info.is_mirrored()) {
...
void FSMap::promote(mds_gid_t standby_gid, Filesystem& filesystem, mds_rank_t assigned_rank)
{
...
void FSMap::assign_standby_replay(const mds_gid_t standby_gid, const fs_cluster_id_t leader_ns,
...
std::vector<mds_gid_t> FSMap::stop(mds_gid_t who)
{
    ...
    for (const auto& i : fs->mds_map.mds_info) {
        ...
        if (other_info.rank == info.rank && other_info.state == MDSMap::STATE_STANDBY_REPLAY) {
...
int FSMap::parse_role(std::string_view role_str, mds_role_t* role, std::ostream& ss,
                      const std::vector<string>& filter) const
{
    ...
    if (!filter.empty() && std::find(filter.begin(), filter.end(), fs_name) == filter.end()) {
        if (r >= 0) {
...
int FSMap::parse_role(std::string_view role_str, mds_role_t* role, std::ostream& ss) const
{
...
bool FSMap::pool_in_use(int64_t poolid) const
{
    for (auto const& i : filesystems) {
        if (i.second->mds_map.is_data_pool(poolid) || i.second->mds_map.metadata_pool == poolid) {
            return true;
        }
    }
    return false;
...
void FSMap::erase_filesystem(fs_cluster_id_t fscid)
{
    ...
    for (auto& [gid, info] : standby_daemons) {
        if (info.join_fscid == fscid) {
            modify_daemon(gid, [](auto& info) { info.join_fscid = FS_CLUSTER_ID_NONE; });
        }
    }
