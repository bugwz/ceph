...
public:
    ...
    */
    ...
    template<typename T> void set_client_metadata(T&& meta)
    {
    ...
    auto get_recall_caps_throttle() const { return recall_caps_throttle.get(); }
    auto get_recall_caps_throttle2o() const { return recall_caps_throttle2o.get(); }
    auto get_recall_caps() const { return recall_caps.get(); }
    auto get_release_caps() const { return release_caps.get(); }
    auto get_session_cache_liveness() const { return session_cache_liveness.get(); }
    auto get_cap_acquisition() const { return cap_acquisition.get(); }
    ...
    int get_num_prealloc_inos() const { return free_prealloc_inos.size(); }

    int get_num_projected_prealloc_inos() const { return get_num_prealloc_inos() + pending_prealloc_inos.size(); }

    client_t get_client() const { return info.get_client(); }
    ...
    void inc_importing() { ++importing_count; }
    void dec_importing()
    {
    ...
    uint64_t get_load_avg() const { return (uint64_t)load_avg.get(); }
    void hit_session() { load_avg.adjust(); }
    ...
    time get_birth_time() const { return birth_time; }
    ...
    void touch_readdir_cap(uint32_t count) { cap_acquisition.hit(count); }
    ...
    bool is_any_flush_waiter() { return !waitfor_flush.empty(); }
    ...
    bool trim_completed_requests(ceph_tid_t mintid)
    {
        ...
        while (!info.completed_requests.empty() && (mintid == 0 || info.completed_requests.begin()->first < mintid)) {
    ...
    bool have_completed_request(ceph_tid_t tid, inodeno_t* pcreated) const
    {
        ...
        if (p == info.completed_requests.end()) return false;
        if (pcreated) *pcreated = p->second;
        return true;
    }

    void add_completed_flush(ceph_tid_t tid) { info.completed_flushes.insert(tid); }
    bool trim_completed_flushes(ceph_tid_t mintid)
    {
        bool erased_any = false;
        while (!info.completed_flushes.empty() && (mintid == 0 || *info.completed_flushes.begin() < mintid)) {
    ...
    bool have_completed_flush(ceph_tid_t tid) const { return info.completed_flushes.count(tid); }

    uint64_t get_num_caps() const { return caps.size(); }

    unsigned get_num_completed_flushes() const { return info.completed_flushes.size(); }
    unsigned get_num_trim_flushes_warnings() const { return num_trim_flushes_warnings; }
    ...
    unsigned get_num_trim_requests_warnings() const { return num_trim_requests_warnings; }
    ...
    bool has_dirty_completed_requests() const { return completed_requests_dirty; }

    void clear_dirty_completed_requests() { completed_requests_dirty = false; }

    int check_access(CInode* in, unsigned mask, int caller_uid, int caller_gid, const std::vector<uint64_t>* gid_list,
...
public:
    ...
    bool match(const Session& session, std::function<bool(client_t)> is_reconnecting) const;
...
public:
    ...
    void set_rank(mds_rank_t r) { rank = r; }
    ...
    void reset_state() { session_map.clear(); }
...
public:
    ...
    ~SessionMap() override
    {
        for (auto p : by_state) delete p.second;
    ...
    void set_version(const version_t v) { version = projected = v; }

    void set_projected(const version_t v) { projected = v; }

    version_t get_projected() const { return projected; }

    version_t get_committed() const { return committed; }

    version_t get_committing() const { return committing; }

    // sessions
    void decode_legacy(ceph::buffer::list::const_iterator& blp) override;
    bool empty() const { return session_map.empty(); }
    const auto& get_sessions() const { return session_map; }

    bool is_any_state(int state) const
    {
        auto it = by_state.find(state);
        if (it == by_state.end() || it->second->empty()) return false;
        return true;
    }

    bool have_unclosed_sessions() const
    {
        return is_any_state(Session::STATE_OPENING) || is_any_state(Session::STATE_OPEN) ||
               is_any_state(Session::STATE_CLOSING) || is_any_state(Session::STATE_STALE) ||
               is_any_state(Session::STATE_KILLING);
    }
    bool have_session(entity_name_t w) const { return session_map.count(w); }
    Session* get_session(entity_name_t w)
    {
        auto session_map_entry = session_map.find(w);
        return (session_map_entry != session_map.end() ? session_map_entry->second : nullptr);
    }
    ...
    Session* get_oldest_session(int state)
    {
        ...
        if (by_state_entry == by_state.end() || by_state_entry->second->empty()) return 0;
        return by_state_entry->second->front();
    }

    void dump();

    template<typename F> void get_client_sessions(F&& f) const
    {
        for (const auto& p : session_map) {
            auto& session = p.second;
            if (session->info.inst.name.is_client()) f(session);
        }
    }
    template<typename C> void get_client_session_set(C& c) const
    {
    ...
    version_t get_push_seq(client_t client) { return get_session(entity_name_t::CLIENT(client.v))->get_push_seq(); }
    ...
    void _load_finish(int operation_r, int header_r, int values_r, bool first, ceph::buffer::list& header_bl,
                      std::map<std::string, ceph::buffer::list>& session_vals, bool more_session_vals);
     ...
     */
    void replay_open_sessions(version_t event_cmapv, std::map<client_t, entity_inst_t>& client_map,
     ...
     *  of the map).
     */
    void save_if_dirty(const std::set<entity_name_t>& tgt_sessions, MDSGatherBuilder* gather_bld);
...
private:
    uint64_t get_session_count_in_state(int state) { return !is_any_state(state) ? 0 : by_state[state]->size(); }
    ...
    void update_average_birth_time(const Session& s, bool added = true)
    {
        ...
        if (added) {
            avg_birth_time = clock::time_point(((avg_birth_time - clock::zero()) / sessions) * (sessions - 1) +
                                               (birth_time - clock::zero()) / sessions);
        }
        else {
            avg_birth_time = clock::time_point(((avg_birth_time - clock::zero()) / (sessions - 1)) * sessions -
