...
public:
    ...
    struct discover_info_t
    {
        ...
        ~discover_info_t()
        {
            if (basei) basei->put(MDSCacheObject::PIN_DISCOVERBASE);
        }
    ...
    void insert_taken_inos(inodeno_t ino) { replay_taken_inos.insert(ino); }
    void clear_taken_inos(inodeno_t ino) { replay_taken_inos.erase(ino); }
    bool test_and_clear_taken_inos(inodeno_t ino) { return replay_taken_inos.erase(ino) != 0; }
    bool is_taken_inos_empty(void) { return replay_taken_inos.empty(); }

    uint64_t cache_limit_memory(void) { return cache_memory_limit; }
    ...
    bool cache_toofull(void) const { return cache_toofull_ratio() > 0.0; }
    uint64_t cache_size(void) const { return mempool::get_pool(mempool::mds_co::id).allocated_bytes(); }
    bool cache_overfull(void) const { return cache_size() > cache_memory_limit * cache_health_threshold; }

    void advance_stray();

    unsigned get_ephemeral_dist_frag_bits() const { return export_ephemeral_dist_frag_bits; }
    bool get_export_ephemeral_distributed_config(void) const { return export_ephemeral_distributed_config; }

    bool get_export_ephemeral_random_config(void) const { return export_ephemeral_random_config; }

    bool get_symlink_recovery(void) const { return symlink_recovery; }
    ...
    void notify_stray(CDentry* dn)
    {
        ...
        if (dn->state_test(CDentry::STATE_PURGING)) return;
    ...
    void notify_stray_removed() { stray_manager.notify_stray_removed(); }

    void notify_stray_created() { stray_manager.notify_stray_created(); }

    void eval_remote(CDentry* dn) { stray_manager.eval_remote(dn); }
    ...
    void discover_dir_frag(CInode* base, frag_t approx_fg, MDSContext* onfinish, mds_rank_t from = MDS_RANK_NONE);
    void discover_path(CInode* base, snapid_t snap, filepath want_path, MDSContext* onfinish, bool path_locked = false,
                       mds_rank_t from = MDS_RANK_NONE);
    void discover_path(CDir* base, snapid_t snap, filepath want_path, MDSContext* onfinish, bool path_locked = false);
    ...
    template<typename T> void get_subtrees(T& c)
    {
        if constexpr (std::is_same_v<T, std::vector<CDir*>>) c.reserve(c.size() + subtrees.size());
    ...
    bool is_subtree(CDir* root) { return subtrees.count(root); }
    ...
    auto num_subtrees() const { return subtrees.size(); }
    ...
    bool have_request(metareqid_t rid) { return active_requests.count(rid); }
    ...
    void journal_cow_dentry(MutationImpl* mut, EMetaBlob* metablob, CDentry* dn, snapid_t follows = CEPH_NOSNAP,
    ...
    void project_rstat_inode_to_frag(const MutationRef& mut, CInode* cur, CDir* parent, snapid_t first, int linkunlink,
                                     SnapRealm* prealm);
    void _project_rstat_inode_to_frag(const CInode::mempool_inode* inode, snapid_t ofirst, snapid_t last, CDir* parent,
                                      int linkunlink, bool update_inode);
    void project_rstat_frag_to_inode(const nest_info_t& rstat, const nest_info_t& accounted_rstat, snapid_t ofirst,
    ...
    void predirty_journal_parents(MutationRef mut, EMetaBlob* blob, CInode* in, CDir* parent, int flags,
                                  int linkunlink = 0, snapid_t follows = CEPH_NOSNAP);
    ...
    void remove_ambiguous_peer_update(metareqid_t reqid, mds_rank_t leader)
    {
        ...
        p->second.erase(q);
        if (p->second.empty()) ambiguous_peer_updates.erase(p);
    }

    void add_rollback(metareqid_t reqid, mds_rank_t leader) { resolve_need_rollback[reqid] = leader; }
    ...
    bool have_ambiguous_import(dirfrag_t base) { return my_ambiguous_imports.count(base); }
    ...
    {
        if (resolves_pending) send_subtree_resolves();
    }
    ...
    void rejoin_export_caps(inodeno_t ino, client_t client, const cap_reconnect_t& icr, int target = -1,
                            bool drop_path = false)
    {
        auto& ex = cap_exports[ino];
        ex.first = target;
        auto& _icr = ex.second[client] = icr;
        if (drop_path) _icr.path.clear();
    }
    ...
    bool rejoin_has_cap_reconnect(inodeno_t ino) const { return cap_imports.count(ino); }
    ...
    {
        if (cap_imports.count(ino) && cap_imports[ino].count(client) && cap_imports[ino][client].count(MDS_RANK_NONE)) {
    ...
    void wait_replay_cap_reconnect(inodeno_t ino, MDSContext* c) { cap_reconnect_waiters[ino].push_back(c); }
    ...
    void set_reconnected_dirty_caps(client_t client, inodeno_t ino, int dirty, bool snapflush)
    {
        ...
        if (snapflush) info.snapflush = snapflush;
    }
    ...
    void do_cap_import(Session* session, CInode* in, Capability* cap, uint64_t p_cap_id, ceph_seq_t p_seq,
                       ceph_seq_t p_mseq, int peer, int p_flags);
    void do_delayed_cap_imports();
    void rebuild_need_snapflush(CInode* head_in, SnapRealm* realm, client_t client, snapid_t snap_follows);
    void open_snaprealms();

    bool open_undef_inodes_dirfrags();
    void opened_undef_inode(CInode* in);
    void opened_undef_dirfrag(CDir* dir) { rejoin_undef_dirfrags.erase(dir); }
    ...
    {
        if (shutdown_exporting_strays.erase(ino)) shutdown_export_strays();
    }
    ...
    bool have_inode(inodeno_t ino, snapid_t snap = CEPH_NOSNAP) { return have_inode(vinodeno_t(ino, snap)); }
    CInode* get_inode(vinodeno_t vino)
    {
        if (vino.snapid == CEPH_NOSNAP) {
            auto p = inode_map.find(vino.ino);
            if (p != inode_map.end()) return p->second;
        }
        else {
            auto p = snap_inode_map.find(vino);
            if (p != snap_inode_map.end()) return p->second;
        }
        return NULL;
    }
    CInode* get_inode(inodeno_t ino, snapid_t s = CEPH_NOSNAP) { return get_inode(vinodeno_t(ino, s)); }
    CInode* lookup_snap_inode(vinodeno_t vino)
    {
        auto p = snap_inode_map.lower_bound(vino);
        if (p != snap_inode_map.end() && p->second->ino() == vino.ino && p->second->first <= vino.snapid)
            return p->second;
        return NULL;
    }
    ...
    CDir* get_force_dirfrag(dirfrag_t df, bool replay)
    {
        ...
        if (!dir) dir = diri->get_dirfrag(df.frag);
        return dir;
    }
    ...
    {
        if (dn->state_test(CDentry::STATE_BOTTOMLRU)) return;
        lru.lru_bottouch(dn);
    }
    ...
    void truncate_inode_write_finish(CInode* in, LogSegment* ls, uint32_t block_size);
    ...
    void wait_for_open(MDSContext* c) { waiting_for_open.push_back(c); }
    ...
    void _create_system_file_finish(MutationRef& mut, CDentry* dn, version_t dpv, MDSContext* fin);
     ...
     */
    int path_traverse(MDRequestRef& mdr, MDSContextFactory& cf, const filepath& path, int flags,
                      std::vector<CDentry*>* pdnvec, CInode** pin = nullptr);

    int maybe_request_forward_to_auth(MDRequestRef& mdr, MDSContextFactory& cf, MDSCacheObject* p);
    ...
    bool parallel_fetch_traverse_dir(inodeno_t ino, filepath& path, std::set<CDir*>& fetch_queue,
                                     std::set<inodeno_t>& missing, C_GatherBuilder& gather_bld);

    void open_remote_dentry(CDentry* dn, bool projected, MDSContext* fin, bool want_xlocked = false);
    void _open_remote_dentry_finish(CDentry* dn, inodeno_t ino, MDSContext* fin, bool want_xlocked, int r);

    void make_trace(std::vector<CDentry*>& trace, CInode* in);

    void open_ino(inodeno_t ino, int64_t pool, MDSContext* fin, bool want_replica = true, bool want_xlocked = false,
                  std::vector<inode_backpointer_t>* ancestors_hint = nullptr, mds_rank_t auth_hint = MDS_RANK_NONE);
    void open_ino_batch_start();
    void open_ino_batch_submit();
    void kick_open_ino_peers(mds_rank_t who);

    void find_ino_peers(inodeno_t ino, MDSContext* c, mds_rank_t hint = MDS_RANK_NONE, bool path_locked = false);
    ...
    void encode_replica_inode(CInode* in, mds_rank_t to, bufferlist& bl, uint64_t features);

    void decode_replica_dir(CDir*& dir, bufferlist::const_iterator& p, CInode* diri, mds_rank_t from,
    ...
    bool is_any_uncommitted_fragment() const { return !uncommitted_fragments.empty(); }
     ...
     */
    void enqueue_scrub(std::string_view path, std::string_view tag, bool force, bool recursive, bool repair,
...
protected:
    ...
    void rejoin_scour_survivor_replicas(mds_rank_t from, const cref_t<MMDSCacheRejoin>& ack,
                                        std::set<vinodeno_t>& acked_inodes, std::set<SimpleLock*>& gather_locks);
    ...
    {
        if (rejoins_pending) rejoin_send_rejoins();
    }

    void touch_inode(CInode* in)
    {
        if (in->get_parent_dn()) touch_dentry(in->get_projected_parent_dn());
    }

    void inode_remove_replica(CInode* in, mds_rank_t rep, bool rejoin, std::set<SimpleLock*>& gather_locks);
    ...
    void _open_ino_fetch_dir(inodeno_t ino, const cref_t<MMDSOpenIno>& m, bool parent, CDir* dir,
                             std::string_view dname);
    int open_ino_traverse_dir(inodeno_t ino, const cref_t<MMDSOpenIno>& m,
                              const std::vector<inode_backpointer_t>& ancestors, bool discover, bool want_xlocked,
                              mds_rank_t* hint);
...
private:
    ...
    CInode* get_stray() { return strays[stray_index]; }
    ...
    void adjust_dir_fragments(CInode* diri, frag_t basefrag, int bits, std::vector<CDir*>* frags,
                              MDSContext::vec& waiters, bool replay);
    void adjust_dir_fragments(CInode* diri, const std::vector<CDir*>& srcfrags, frag_t basefrag, int bits,
    ...
    void add_uncommitted_fragment(dirfrag_t basedirfrag, int bits, const frag_vec_t& old_frag, LogSegment* ls,
