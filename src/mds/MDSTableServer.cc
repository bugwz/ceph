...
public:
    C_Prepare(MDSTableServer* s, const cref_t<MMDSTableRequest> r, version_t v)
        : server(s)
        , req(r)
        , tid(v)
    {}
    void finish(int r) override { server->_prepare_logged(req, tid); }
};
...
void MDSTableServer::handle_prepare(const cref_t<MMDSTableRequest>& req)
{
    ...
    ETableServer* le =
        new ETableServer(table, TABLESERVER_OP_PREPARE, req->reqid, from, projected_version, projected_version);
...
public:
    C_Commit(MDSTableServer* s, const cref_t<MMDSTableRequest>& r)
        : server(s)
        , req(r)
    {}
    void finish(int r) override { server->_commit_logged(req); }
};
...
void MDSTableServer::handle_commit(const cref_t<MMDSTableRequest>& req)
{
    ...
    if (pending_for_mds.count(tid)) {
        ...
        mds->mdlog->start_submit_entry(
            new ETableServer(table, TABLESERVER_OP_COMMIT, 0, MDS_RANK_NONE, tid, projected_version),
...
public:
    C_Rollback(MDSTableServer* s, const cref_t<MMDSTableRequest>& r)
        : server(s)
        , req(r)
    {}
    void finish(int r) override { server->_rollback_logged(req); }
};
...
void MDSTableServer::handle_rollback(const cref_t<MMDSTableRequest>& req)
{
    ...
    mds->mdlog->start_submit_entry(
        new ETableServer(table, TABLESERVER_OP_ROLLBACK, 0, MDS_RANK_NONE, tid, projected_version),
...
public:
    C_ServerUpdate(MDSTableServer* s, bufferlist& b)
        : server(s)
        , bl(b)
    {}
    void finish(int r) override { server->_server_update_logged(bl); }
};
...
public:
    C_ServerRecovery(MDSTableServer* s)
        : server(s)
    {}
    void finish(int r) override { server->_do_server_recovery(); }
};
...
void MDSTableServer::_do_server_recovery()
{
    ...
    for (auto p : pending_for_mds) {
        ...
        if (!active_clients.count(who)) continue;

        if (p.second.reqid >= next_reqids[who]) next_reqids[who] = p.second.reqid + 1;
...
void MDSTableServer::handle_mds_recovery(mds_rank_t who)
{
    ...
    for (auto p = pending_for_mds.begin(); p != pending_for_mds.end(); ++p) {
        ...
        if (p->second.reqid >= next_reqid) next_reqid = p->second.reqid + 1;
