...
public:
    ...
    static Action str_to_type(std::string_view str) { return PurgeItem::actions.at(std::string(str)); }
...
public:
    ...
    PurgeItemCommitOp(PurgeItem _item, PurgeType _type, int _flags, object_t _oid, object_locator_t _oloc)
        : item(_item)
        , type(_type)
        , flags(_flags)
        , oid(_oid)
...
{
public:
    PurgeQueue(CephContext* cct_, mds_rank_t rank_, const int64_t metadata_pool_, Objecter* objecter_,
               Context* on_error);
    ~PurgeQueue();
     ...
     */
    bool drain(uint64_t* progress, uint64_t* progress_total, size_t* in_flight_count);
