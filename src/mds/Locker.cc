...
protected:
    Locker* locker;
    MDSRank* get_mds() override { return locker->mds; }

public:
    explicit LockerContext(Locker* locker_)
        : locker(locker_)
    {
...
public:
    explicit LockerLogContext(Locker* locker_)
        : locker(locker_)
    {
        ceph_assert(locker != NULL);
    }
...
void Locker::dispatch(const cref_t<Message>& m)
{
    ...
    case MSG_MDS_INODEFILECAPS:
        ...
        break;
        // client sync
    case CEPH_MSG_CLIENT_CAPS: handle_client_caps(ref_cast<MClientCaps>(m)); break;
    case CEPH_MSG_CLIENT_CAPRELEASE: handle_client_cap_release(ref_cast<MClientCapRelease>(m)); break;
    case CEPH_MSG_CLIENT_LEASE: handle_client_lease(ref_cast<MClientLease>(m)); break;
    default: derr << "locker unknown message " << m->get_type() << dendl; ceph_abort_msg("locker unknown message");
    }
}
...
void Locker::send_lock_message(SimpleLock* lock, int msg)
{
    for (const auto& it : lock->get_parent()->get_replicas()) {
        if (mds->is_cluster_degraded() && mds->mdsmap->get_state(it.first) < MDSMap::STATE_REJOIN) continue;
...
bool Locker::try_rdlock_snap_layout(CInode* in, MDRequestRef& mdr, int n, bool want_layout)
{
    ...
    if (want_layout) ceph_assert(n == 0);
    ...
    while (true) {
        ++depth;
        if (!found_locked && mdr->is_rdlocked(&t->snaplock)) found_locked = true;
...
struct MarkEventOnDestruct
{
    ...
    ~MarkEventOnDestruct()
    {
        if (mark_event) mdr->mark_event(message);
    }
};
                           ...
                           bool auth_pin_nonblocking)
{
    ...
    if (auth_pin_freeze) mustpin.insert(auth_pin_freeze);
    ...
    for (size_t i = 0; i < lov.size(); ++i) {
        ...
        if (p.is_xlock()) {
            if ((lock->get_type() == CEPH_LOCK_ISNAP || lock->get_type() == CEPH_LOCK_IPOLICY) &&
                mds->is_cluster_degraded() && mdr->is_leader() && !mdr->is_queued_for_replay()) {
                ...
                if (object->is_auth()) {
                    ...
                    if (!mdr->is_xlocked(lock) || mdr->is_last_locked(lock)) wait = true;
                }
                if (wait) {
            ...
            else if (!object->is_auth() && !lock->can_wrlock(_client) &&   // we might have to request a scatter
            ...
            else if (!object->is_auth() && !lock->can_rdlock(client)) {   // we might have to request an rdlock
    ...
    for (const auto& p : mustpin) {
        ...
        if (mdr->is_auth_pinned(object)) {
            if (object != (MDSCacheObject*)auth_pin_freeze) continue;
            if (mdr->more()->is_remote_frozen_authpin) {
                if (mdr->more()->rename_inode == auth_pin_freeze) continue;
        ...
        if (!object->can_auth_pin(&err)) {
            ...
            if (mdr->is_any_remote_auth_pin()) notify_freeze_waiter(object);
    ...
    if (!mustpin_remote.empty()) {
        ...
        for (const auto& p : mdr->object_states) {
            ...
            if (q != mustpin_remote.end()) q->second.insert(p.first);
        }
        ...
        for (auto& p : mustpin_remote) {
            ...
            auto req = make_message<MMDSPeerRequest>(mdr->reqid, mdr->attempt, MMDSPeerRequest::OP_AUTHPIN);
            for (auto& o : p.second) {
                dout(10) << " req remote auth_pin of " << *o << dendl;
                MDSCacheObjectInfo info;
                o->set_object_info(info);
                req->get_authpins().push_back(info);
                if (o == auth_pin_freeze) o->set_object_info(req->get_authpin_freeze());
                mdr->pin(o);
            }
            if (auth_pin_nonblocking)
    ...
    for (const auto& p : lov) {
        ...
        if (p.is_xlock()) {
            ...
            if (mdr->locking && lock != mdr->locking) cancel_locking(mdr.get(), &issue_set);
...
void Locker::notify_freeze_waiter(MDSCacheObject* o)
{
    ...
    if (CInode* in = dynamic_cast<CInode*>(o)) {
        if (!in->is_root()) dir = in->get_parent_dir();
    }
    ...
    if (dir) {
        if (dir->is_freezing_dir()) mdcache->fragment_freeze_inc_num_waiters(dir);
        if (dir->is_freezing_tree()) {
            while (!dir->is_freezing_tree_root()) dir = dir->get_parent_dir();
...
void Locker::set_xlocks_done(MutationImpl* mut, bool skip_dentry)
{
    for (const auto& p : mut->locks) {
        if (!p.is_xlock()) continue;
        MDSCacheObject* obj = p.lock->get_parent();
        ceph_assert(obj->is_auth());
        if (skip_dentry && (p.lock->get_type() == CEPH_LOCK_DN || p.lock->get_type() == CEPH_LOCK_DVERSION)) continue;
        dout(10) << "set_xlocks_done on " << *p.lock << " " << *obj << dendl;
        p.lock->set_xlock_done();
    }
}

void Locker::_drop_locks(MutationImpl* mut, set<CInode*>* pneed_issue, bool drop_rdlocks)
{
    ...
    for (auto it = mut->locks.begin(); it != mut->locks.end();) {
        ...
        if (it->is_xlock()) {
            if (obj->is_auth()) {
                ...
                if (ni) pneed_issue->insert(static_cast<CInode*>(obj));
            }
            else {
    ...
    for (set<mds_rank_t>::iterator p = peers.begin(); p != peers.end(); ++p) {
        if (!mds->is_cluster_degraded() || mds->mdsmap->get_state(*p) >= MDSMap::STATE_REJOIN) {
            dout(10) << "_drop_non_rdlocks dropping remote locks on mds." << *p << dendl;
            auto peerreq = make_message<MMDSPeerRequest>(mut->reqid, mut->attempt, MMDSPeerRequest::OP_DROPLOCKS);
...
void Locker::cancel_locking(MutationImpl* mut, set<CInode*>* pneed_issue)
{
    ...
    if (lock->get_parent()->is_auth()) {
        ...
        if (need_issue) pneed_issue->insert(static_cast<CInode*>(lock->get_parent()));
    }
...
void Locker::drop_locks(MutationImpl* mut, set<CInode*>* pneed_issue)
{
    ...
    if (!pneed_issue) pneed_issue = &my_need_issue;

    if (mut->locking) cancel_locking(mut, pneed_issue);
    _drop_locks(mut, pneed_issue, true);

    if (pneed_issue == &my_need_issue) issue_caps_set(*pneed_issue);
    mut->locking_state = 0;
}
...
void Locker::drop_rdlocks_for_early_reply(MutationImpl* mut)
{
    ...
    for (auto it = mut->locks.begin(); it != mut->locks.end();) {
        ...
        bool ni = false;
        rdlock_finish(it++, mut, &ni);
        if (ni) need_issue.insert(static_cast<CInode*>(lock->get_parent()));
    }
...
{
    switch (op) {
    case CEPH_MDS_OP_CREATE: return CEPH_CAP_DIR_CREATE;
    case CEPH_MDS_OP_UNLINK: return CEPH_CAP_DIR_UNLINK;
    default: ceph_assert(0 == "unsupported operation"); return 0;
    }
}
...
void Locker::eval_lock_caches(Capability* cap)
{
    for (auto p = cap->lock_caches.begin(); !p.end();) {
        ...
        ++p;
        if (!lock_cache->invalidating) continue;
        int cap_bit = get_cap_bit_for_lock_cache(lock_cache->opcode);
        if (!(cap->issued() & cap_bit)) {
            lock_cache->item_cap_lock_cache.remove_myself();
            put_lock_cache(lock_cache);
...
void Locker::invalidate_lock_caches(SimpleLock* lock)
{
    ...
    if (lock->is_cached()) {
        ...
        for (auto& lc : lock_caches) invalidate_lock_cache(lc);
    }
}

void Locker::create_lock_cache(MDRequestRef& mdr, CInode* diri, file_layout_t* dir_layout)
{
    if (mdr->lock_cache) return;

    client_t client = mdr->get_client();
    int opcode = mdr->client_request->get_op();
    dout(10) << "create_lock_cache for client." << client << "/" << ceph_mds_op_name(opcode) << " on " << *diri
             << dendl;
    ...
    for (auto& p : mdr->object_states) {
        if (p.first != diri && !ancestors.count(p.first)) continue;
        auto& stat = p.second;
        if (stat.auth_pinned) {
    ...
    for (auto& p : mdr->locks) {
        MDSCacheObject* obj = p.lock->get_parent();
        if (obj != diri && !ancestors.count(obj)) continue;
        if (!p.lock->is_stable()) {
    ...
    if (dir_layout) lock_cache->set_dir_layout(*dir_layout);
    ...
    for (auto& p : mdr->object_states) {
        ...
        if (CInode* in = dynamic_cast<CInode*>(p.first)) {
            CDentry* pdn = in->get_projected_parent_dn();
            if (pdn) dfv.push_back(pdn->get_dir());
        }
    ...
    for (auto it = mdr->locks.begin(); it != mdr->locks.end();) {
        ...
        if (it->is_wrlock()) {
            ...
            if (obj == diri) lock_flag = MutationImpl::LockOp::WRLOCK;
        }
        else {
...
void Locker::eval_gather(SimpleLock* lock, bool first, bool* pneed_issue, MDSContext::vec* pfinishers)
{
    ...
    if (lock->get_type() != CEPH_LOCK_DN) in = static_cast<CInode*>(lock->get_parent());
    ...
    if (caps && in->is_head()) {
        ...
        dout(10) << " next state is " << lock->get_state_name(next) << " issued/allows loner "
                 << gcap_string(loner_issued) << "/" << gcap_string(lock->gcaps_allowed(CAP_LONER, next)) << " xlocker "
                 << gcap_string(xlocker_issued) << "/" << gcap_string(lock->gcaps_allowed(CAP_XLOCKER, next))
                 << " other " << gcap_string(other_issued) << "/" << gcap_string(lock->gcaps_allowed(CAP_ANY, next))
                 << dendl;
...
#define IS_TRUE_AND_LT_AUTH(x, auth) (x && ((auth && x <= AUTH) || (!auth && x < AUTH)))
    ...
    if (!lock->is_gathering() &&
        ...
        if (!lock->get_parent()->is_auth()) {
            ...
            if (lock->get_parent()->is_rejoining() && mds->mdsmap->get_state(auth) == MDSMap::STATE_REJOIN) {
                dout(7) << "eval_gather finished gather, but still rejoining " << *lock->get_parent() << dendl;
                return;
            }

            if (!mds->is_cluster_degraded() || mds->mdsmap->get_state(auth) >= MDSMap::STATE_REJOIN) {
            ...
            if (lock->get_state() == LOCK_MIX_LOCK && lock->get_parent()->is_replicated()) {
        ...
        if (lock->get_parent()->is_auth() && lock->is_stable()) lock->get_parent()->auth_unpin(lock);

        // drop loner before doing waiters
        if (caps && in->is_head() && in->is_auth() && in->get_wanted_loner() != in->get_loner()) {
        ...
        if (pfinishers)
            lock->take_waiting(
                SimpleLock::WAIT_STABLE | SimpleLock::WAIT_WR | SimpleLock::WAIT_RD | SimpleLock::WAIT_XLOCK,
                *pfinishers);
        else
...
retry:
    if (mask & CEPH_LOCK_IFILE) eval_any(&in->filelock, &need_issue, &finishers, caps_imported);
    if (mask & CEPH_LOCK_IAUTH) eval_any(&in->authlock, &need_issue, &finishers, caps_imported);
    if (mask & CEPH_LOCK_ILINK) eval_any(&in->linklock, &need_issue, &finishers, caps_imported);
    if (mask & CEPH_LOCK_IXATTR) eval_any(&in->xattrlock, &need_issue, &finishers, caps_imported);
    if (mask & CEPH_LOCK_INEST) eval_any(&in->nestlock, &need_issue, &finishers, caps_imported);
    if (mask & CEPH_LOCK_IFLOCK) eval_any(&in->flocklock, &need_issue, &finishers, caps_imported);
    if (mask & CEPH_LOCK_IPOLICY) eval_any(&in->policylock, &need_issue, &finishers, caps_imported);
    ...
    if (need_issue && in->is_head()) issue_caps(in);
...
public:
    C_Locker_Eval(Locker* l, MDSCacheObject* pp, int m)
        : LockerContext(l)
        , p(pp)
        , mask(m)
    {
...
void Locker::try_eval(SimpleLock* lock, bool* pneed_issue)
{
    ...
    if (lock->is_scatterlock()) {
        ...
        if (slock->get_scatter_wanted() && slock->get_state() != LOCK_MIX) {
            scatter_mix(slock, pneed_issue);
            if (!lock->is_stable()) return;
        }
        else if (slock->get_unscatter_wanted() && slock->get_state() != LOCK_LOCK) {
    ...
    if (lock->get_type() != CEPH_LOCK_DN && lock->get_type() != CEPH_LOCK_ISNAP &&
        lock->get_type() != CEPH_LOCK_IPOLICY && p->is_freezing()) {
...
void Locker::eval_cap_gather(CInode* in, set<CInode*>* issue_set)
{
    ...
    // kick locks now
    if (!in->filelock.is_stable()) eval_gather(&in->filelock, false, &need_issue, &finishers);
    if (!in->authlock.is_stable()) eval_gather(&in->authlock, false, &need_issue, &finishers);
    if (!in->linklock.is_stable()) eval_gather(&in->linklock, false, &need_issue, &finishers);
    if (!in->xattrlock.is_stable()) eval_gather(&in->xattrlock, false, &need_issue, &finishers);
...
void Locker::eval_scatter_gathers(CInode* in)
{
    ...
    // kick locks now
    if (!in->filelock.is_stable()) eval_gather(&in->filelock, false, &need_issue, &finishers);
    if (!in->nestlock.is_stable()) eval_gather(&in->nestlock, false, &need_issue, &finishers);
    if (!in->dirfragtreelock.is_stable()) eval_gather(&in->dirfragtreelock, false, &need_issue, &finishers);
...
void Locker::eval(SimpleLock* lock, bool* need_issue)
{
    ...
    case CEPH_LOCK_IFILE: return file_eval(static_cast<ScatterLock*>(lock), need_issue);
    case CEPH_LOCK_IDFT:
    case CEPH_LOCK_INEST: return scatter_eval(static_cast<ScatterLock*>(lock), need_issue);
    default: return simple_eval(lock, need_issue);
    }
}
    ...
    if (lock->is_stable()) {
        if (lock->get_parent()->is_auth()) {
            if (lock->get_sm() == &sm_scatterlock) {
                ...
                if (lock->get_state() == LOCK_EXCL && in->get_target_loner() >= 0 && !in->is_dir() &&
...
bool Locker::rdlock_try(SimpleLock* lock, client_t client)
{
    ...
    // can read?  grab ref.
    if (lock->can_rdlock(client)) return true;
...
bool Locker::rdlock_start(SimpleLock* lock, MDRequestRef& mut, bool as_anon)
{
    ...
    if (mut->snapid != CEPH_NOSNAP) as_anon = true;
    ...
    */
    ...
    while (1) {
        ...
        // hmm, wait a second.
        if (in && !in->is_head() && in->is_auth() && lock->get_state() == LOCK_SNAP_SYNC) {
            ...
            if (hlock->get_state() == LOCK_SYNC) hlock = head->get_lock(lock->get_type());
        ...
        if (!_rdlock_kick(lock, as_anon)) break;
    }
...
bool Locker::rdlock_try_set(MutationImpl::LockOpVec& lov, MDRequestRef& mdr)
{
    ...
    for (const auto& p : lov) {
        ...
        if (!mdr->is_rdlocked(lock) && !rdlock_try(lock, mdr->get_client())) {
            lock->add_waiter(SimpleLock::WAIT_STABLE | SimpleLock::WAIT_RD, new C_MDS_RetryRequest(mdcache, mdr));
            goto failed;
        }
        lock->get_rdlock();
...
bool Locker::rdlock_try_set(MutationImpl::LockOpVec& lov, MutationRef& mut)
{
    ...
    for (const auto& p : lov) {
        auto lock = p.lock;
        ceph_assert(p.is_rdlock());
        if (!lock->can_rdlock(mut->get_client())) return false;
        p.lock->get_rdlock();
...
{
    if (lock->get_type() == CEPH_LOCK_IVERSION || lock->get_type() == CEPH_LOCK_DVERSION)
...
bool Locker::wrlock_try(SimpleLock* lock, const MutationRef& mut, client_t client)
{
    ...
    if (client == -1) client = mut->get_client();
    ...
    while (1) {
        ...
        if (lock->is_dirty()) break;
        // To avoid calling scatter_writebehind or start_scatter. don't
        // change nest lock state to MIX.
        ScatterLock* slock = static_cast<ScatterLock*>(lock);
        if (slock->get_scatter_wanted() || in->has_subtree_or_exporting_dirfrag()) break;
...
bool Locker::wrlock_start(const MutationImpl::LockOp& op, MDRequestRef& mut)
{
    ...
    bool want_scatter = lock->get_parent()->is_auth() && (in->has_subtree_or_exporting_dirfrag() ||
    ...
    while (1) {
        ...
        if (lock->get_type() == CEPH_LOCK_IFILE && in->state_test(CInode::STATE_RECOVERING)) {
            mds->mdcache->recovery_queue.prioritize(in);
        }
...
void Locker::wrlock_finish(const MutationImpl::lock_iterator& it, MutationImpl* mut, bool* pneed_issue)
{
    ...
    if (lock->is_wrlocked()) {
        ...
        if (!lock->is_stable() && lock->is_flushed() && lock->get_parent()->is_auth())
            eval_gather(lock, false, pneed_issue);
    }
    else {
        if (!lock->is_stable())
...
void Locker::remote_wrlock_start(SimpleLock* lock, mds_rank_t target, MDRequestRef& mut)
{
    ...
    if (mds->is_cluster_degraded() && !mds->mdsmap->is_clientreplay_or_active_or_stopping(target)) {
...
void Locker::remote_wrlock_finish(const MutationImpl::lock_iterator& it, MutationImpl* mut)
{
    ...
    if (!mds->is_cluster_degraded() || mds->mdsmap->get_state(target) >= MDSMap::STATE_REJOIN) {
...
bool Locker::xlock_start(SimpleLock* lock, MDRequestRef& mut)
{
    ...
    CInode* in = nullptr;
    if (lock->get_cap_shift()) in = static_cast<CInode*>(lock->get_parent());
    ...
    if (lock->get_parent()->is_auth()) {
        ...
        while (1) {
            ...
            if (!lock->is_stable() && (lock->get_state() != LOCK_XLOCKDONE || lock->get_xlock_by_client() != client ||
                                       lock->is_waiter_for(SimpleLock::WAIT_STABLE)))
                break;
            // Avoid unstable XLOCKDONE state reset,
            // see: https://tracker.ceph.com/issues/49132
            if (lock->get_state() == LOCK_XLOCKDONE && lock->get_type() == CEPH_LOCK_IFILE) break;
        ...
        if (lock->get_parent()->is_ambiguous_auth()) {
            lock->get_parent()->add_waiter(MDSCacheObject::WAIT_SINGLEAUTH, new C_MDS_RetryRequest(mdcache, mut));
            return false;
        }
...
{
    if (lock->get_type() != CEPH_LOCK_DN && lock->get_type() != CEPH_LOCK_ISNAP &&
        lock->get_type() != CEPH_LOCK_IPOLICY && lock->get_num_rdlocks() == 0 && lock->get_num_wrlocks() == 0 &&
        !lock->is_leased() && lock->get_state() != LOCK_XLOCKSNAP) {
        CInode* in = static_cast<CInode*>(lock->get_parent());
        ...
        if (loner >= 0 && (xlocker < 0 || xlocker == loner)) {
            ...
            lock->finish_waiters(SimpleLock::WAIT_STABLE | SimpleLock::WAIT_WR | SimpleLock::WAIT_RD);
            if (lock->get_cap_shift()) *pneed_issue = true;
            if (lock->get_parent()->is_auth() && lock->is_stable()) try_eval(lock, pneed_issue);
            return;
        }
    }
...
void Locker::xlock_finish(const MutationImpl::lock_iterator& it, MutationImpl* mut, bool* pneed_issue)
{
    ...
    if (!lock->get_parent()->is_auth()) {
        ...
        // others waiting?
        lock->finish_waiters(SimpleLock::WAIT_STABLE | SimpleLock::WAIT_WR | SimpleLock::WAIT_RD, 0);
    }
    else {
        if (lock->get_num_xlocks() == 0 && lock->get_state() != LOCK_LOCK_XLOCK) {   // no one is taking xlock
...
void Locker::xlock_export(const MutationImpl::lock_iterator& it, MutationImpl* mut)
{
    ...
    if (!lock->is_stable()) lock->get_parent()->auth_unpin(lock);
...
void Locker::xlock_downgrade(SimpleLock* lock, MutationImpl* mut)
{
    ...
    if (it->is_rdlock()) return;   // already downgraded
...
public:
    C_Locker_FileUpdate_finish(Locker* l, CInode* i, MutationRef& m, unsigned f, const ref_t<MClientCaps>& ack,
                               client_t c = -1)
        : LockerLogContext(l)
        , in(i)
        , mut(m)
...
void Locker::file_update_finish(CInode* in, MutationRef& mut, unsigned flags, client_t client,
                                const ref_t<MClientCaps>& ack)
{
    ...
    if (ack) {
        ...
        if (session && !session->is_closed()) {
            ...
            if (ack->get_oldest_flush_tid() > 0) session->add_completed_flush(ack->get_client_tid());
    ...
    if (in->is_head()) {
        if ((flags & UPDATE_NEEDSISSUE) && need_issue.count(in) == 0) {
            ...
            if (cap && (cap->wanted() & ~cap->pending())) issue_caps(in, cap);
        }
...
Capability* Locker::issue_new_caps(CInode* in, int mode, MDRequestRef& mdr, SnapRealm* realm)
{
    ...
    if (new_inode && mdr->client_request->is_queued_for_replay()) return mds->mdcache->try_reconnect_cap(in, session);
    ...
    if (in->is_auth()) {
        ...
        int allowed = get_allowed_caps(in, cap, all_allowed, loner_allowed, xlocker_allowed);

        if (_need_flush_mdlog(in, my_want & ~allowed, true)) mds->mdlog->flush();
    }
    else {
...
{
    for (set<CInode*>::iterator p = inset.begin(); p != inset.end(); ++p) issue_caps(*p);
}
...
int Locker::get_allowed_caps(CInode* in, Capability* cap, int& all_allowed, int& loner_allowed, int& xlocker_allowed)
{
    client_t client = cap->get_client();

    // allowed caps are determined by the lock mode.
    if (all_allowed == -1) all_allowed = in->get_caps_allowed_by_type(CAP_ANY);
    if (loner_allowed == -1) loner_allowed = in->get_caps_allowed_by_type(CAP_LONER);
    if (xlocker_allowed == -1) xlocker_allowed = in->get_caps_allowed_by_type(CAP_XLOCKER);

    client_t loner = in->get_loner();
    if (loner >= 0) {
        dout(7) << "get_allowed_caps loner client." << loner << " allowed=" << ccap_string(loner_allowed)
                << ", xlocker allowed=" << ccap_string(xlocker_allowed)
                << ", others allowed=" << ccap_string(all_allowed) << " on " << *in << dendl;
    }
    else {
        dout(7) << "get_allowed_caps allowed=" << ccap_string(all_allowed)
                << ", xlocker allowed=" << ccap_string(xlocker_allowed) << " on " << *in << dendl;
    }
    ...
    if (in->is_dir()) {
        ...
        if (allowed & CEPH_CAP_FILE_EXCL) allowed |= cap->get_lock_cache_allowed();
    }

    if ((in->get_inode()->inline_data.version != CEPH_INLINE_NONE && cap->is_noinline()) ||
        (!in->get_inode()->layout.pool_ns.empty() && cap->is_nopoolns()))
...
int Locker::issue_caps(CInode* in, Capability* only_cap)
{
    ...
    for (; it != in->client_caps.end(); ++it) {
        ...
        dout(20) << " client." << it->first << " pending " << ccap_string(pending) << " allowed "
                 << ccap_string(allowed) << " wanted " << ccap_string(wanted) << dendl;
        ...
        if (!(pending & ~allowed)) {
            ...
            if (!cap->is_valid() && (pending & ~CEPH_CAP_PIN)) {
                ...
                dout(7) << "   sending MClientCaps to client." << it->first << " seq " << seq << " re-issue "
                        << ccap_string(pending) << dendl;
        ...
        if (in->get_inode()->nlink == 0) wanted |= CEPH_CAP_LINK_SHARED;
        ...
        if ((pending & ~allowed) ||              // need to revoke ~allowed caps.
            ...
            if (pending & ~allowed)
                seq =
                    cap->issue((wanted | likes) & allowed & pending, true);   // if revoking, don't issue anything new.
            else
                seq = cap->issue((wanted | likes) & allowed, true);
            int after = cap->pending();

            dout(7) << "   sending MClientCaps to client." << it->first << " seq " << seq << " new pending "
                    << ccap_string(after) << " was " << ccap_string(before) << dendl;
...
void Locker::issue_truncate(CInode* in)
{
    ...
    if (in->is_auth() && in->is_file()) check_inode_max_size(in);
}
...
void Locker::revoke_stale_cap(CInode* in, client_t client)
{
    ...
    if (in->state_test(CInode::STATE_EXPORTINGCAPS)) return;
...
bool Locker::revoke_stale_caps(Session* session)
{
    ...
    for (auto p = session->caps.begin(); !p.end();) {
        ...
        if (revoked & CEPH_CAP_ANY_DIR_OPS) eval_lock_caches(cap);
    ...
    for (auto in : to_eval) {
        if (in->state_test(CInode::STATE_EXPORTINGCAPS)) continue;
...
void Locker::resume_stale_caps(Session* session)
{
    ...
    for (xlist<Capability*>::iterator p = session->caps.begin(); !p.end();) {
        Capability* cap = *p;
        ++p;
        if (lazy && !cap->is_notable()) break;   // see revoke_stale_caps()
        ...
        if (!in->is_auth() || !eval(in, CEPH_CAP_LOCKS)) issue_caps(in, cap);
    }
}
...
public:
    C_MDL_RequestInodeFileCaps(Locker* l, CInode* i)
        : LockerContext(l)
        , in(i)
    {
        in->get(CInode::PIN_PTRWAITER);
    ...
    void finish(int r) override
    {
        if (!in->is_auth()) locker->request_inode_file_caps(in);
        in->put(CInode::PIN_PTRWAITER);
    }
};
...
void Locker::request_inode_file_caps(CInode* in)
{
    ...
    if (wanted != in->replica_caps_wanted) {
        ...
        if (in->is_ambiguous_auth()) {
            in->add_waiter(MDSCacheObject::WAIT_SINGLEAUTH, new C_MDL_RequestInodeFileCaps(this, in));
            return;
        }
        ...
        dout(7) << "request_inode_file_caps " << ccap_string(wanted) << " was " << ccap_string(in->replica_caps_wanted)
...
void Locker::handle_inode_file_caps(const cref_t<MInodeFileCaps>& m)
{
    ...
    dout(7) << "handle_inode_file_caps replica mds." << from << " wants caps " << ccap_string(m->get_caps()) << " on "
            << *in << dendl;
...
public:
    C_MDL_CheckMaxSize(Locker* l, CInode* i, uint64_t _new_max_size, uint64_t _newsize, utime_t _mtime)
        : LockerContext(l)
        , in(i)
        , new_max_size(_new_max_size)
        , newsize(_newsize)
        , mtime(_mtime)
    {
        in->get(CInode::PIN_PTRWAITER);
    ...
    void finish(int r) override
    {
        if (in->is_auth()) locker->check_inode_max_size(in, false, new_max_size, newsize, mtime);
        in->put(CInode::PIN_PTRWAITER);
    }
};
...
bool Locker::check_client_ranges(CInode* in, uint64_t size)
{
    ...
    for (auto& p : in->client_caps) {
        if ((p.second.issued() | p.second.wanted()) & CEPH_CAP_ANY_FILE_WR) {
            if (it == latest->client_ranges.end()) return true;
            if (it->first != p.first) return true;
            if (ms > it->second.range.last) return true;
            ++it;
        }
    }
        ...
        if ((p.second.issued() | p.second.wanted()) & CEPH_CAP_ANY_FILE_WR) {
            ...
            if (it != pi->client_ranges.end() && it->first == p.first) {
                if (ms > it->second.range.last) {
                    ...
                    updated = true;
                    if (max_increased) *max_increased = true;
                }
            }
            else {
                it = pi->client_ranges.emplace_hint(
                    it, std::piecewise_construct, std::forward_as_tuple(p.first), std::forward_as_tuple());
                it->second.range.last = ms;
...
bool Locker::check_inode_max_size(CInode* in, bool force_wrlock, uint64_t new_max_size, uint64_t new_size,
                                  utime_t new_mtime)
{
    ceph_assert(in->is_auth());
    ...
    if (update_size) {
        ...
        if (latest->size == new_size && latest->mtime == new_mtime) update_size = false;
    }
    ...
    dout(10) << "check_inode_max_size new_ranges " << new_ranges << " update_size " << update_size << " on " << *in
             << dendl;

    if (in->is_frozen()) {
        dout(10) << "check_inode_max_size frozen, waiting on " << *in << dendl;
        in->add_waiter(CInode::WAIT_UNFREEZE, new C_MDL_CheckMaxSize(this, in, new_max_size, new_size, new_mtime));
        return false;
    }
    ...
    if (new_ranges && calc_new_client_ranges(in, std::max(new_max_size, size), &max_increased)) {
        dout(10) << "check_inode_max_size client_ranges " << in->get_previous_projected_inode()->client_ranges << " -> "
    ...
    if (update_size) {
        ...
        if (new_mtime > pi.inode->ctime) {
            ...
            if (new_mtime > pi.inode->rstat.rctime) pi.inode->rstat.rctime = new_mtime;
        }
    }
    ...
    mds->mdlog->submit_entry(le, new C_Locker_FileUpdate_finish(this, in, mut, UPDATE_SHAREMAX, ref_t<MClientCaps>()));
    wrlock_force(&in->filelock, mut);   // wrlock for duration of journal
    mut->auth_pin(in);

    // make max_size _increase_ timely
    if (max_increased) mds->mdlog->flush();
...
void Locker::adjust_cap_wanted(Capability* cap, int wanted, int issue_seq)
{
    if (ceph_seq_cmp(issue_seq, cap->get_last_issue()) == 0) {
        ...
        dout(10) << " wanted " << ccap_string(cap->wanted()) << " -> " << ccap_string(wanted)
                 << " (added caps even though we had seq mismatch!)" << dendl;
        cap->set_wanted(wanted | cap->wanted());
    }
    else {
        dout(10) << " NOT changing wanted " << ccap_string(cap->wanted()) << " -> " << ccap_string(wanted)
                 << " (issue_seq " << issue_seq << " != last_issue " << cap->get_last_issue() << ")" << dendl;
        return;
    }
    ...
    if (cap->wanted()) {
        if (cur->state_test(CInode::STATE_RECOVERING) && (cap->wanted() & (CEPH_CAP_FILE_RD | CEPH_CAP_FILE_WR))) {
...
void Locker::snapflush_nudge(CInode* in)
{
    ...
    if (in->client_snap_caps.empty()) return;
    ...
    if (head->client_need_snapflush.empty()) return;
...
bool Locker::is_revoking_any_caps_from(client_t client)
{
    ...
    if (it == revoking_caps_by_client.end()) return false;
...
bool Locker::should_defer_client_cap_frozen(CInode* in)
{
    ...
    if (in->is_freezing() && in->get_num_auth_pins() == 0) {
        ...
        if (!dir || !dir->is_auth_pinned_by_lock_cache()) return true;
    }
    return false;
}
...
void Locker::handle_client_caps(const cref_t<MClientCaps>& m)
{
    ...
    dout(7) << "handle_client_caps "
            << " on " << m->get_ino() << " tid " << m->get_client_tid() << " follows " << follows << " op "
            << ceph_cap_op_name(op) << " flags 0x" << std::hex << m->flags << std::dec << dendl;
    ...
    if (!mds->is_clientreplay() && !mds->is_active() && !mds->is_stopping()) {
        ...
        if ((mds->is_reconnect() || mds->get_want_state() == MDSMap::STATE_RECONNECT) && dirty &&
            m->get_client_tid() > 0 && !session->have_completed_flush(m->get_client_tid())) {
            mdcache->set_reconnected_dirty_caps(client, m->get_ino(), dirty, op == CEPH_CAP_OP_FLUSHSNAP);
        }
        mds->wait_for_replay(new C_MDS_RetryMessage(mds, m));
        return;
    }
    ...
    if (m->get_client_tid() > 0 && session && session->have_completed_flush(m->get_client_tid())) {
        dout(7) << "handle_client_caps already flushed tid " << m->get_client_tid() << " for client." << client
                << dendl;
        ref_t<MClientCaps> ack;
        if (op == CEPH_CAP_OP_FLUSHSNAP) {
            if (mds->logger) mds->logger->inc(l_mdss_ceph_cap_op_flushsnap_ack);
            ack = make_message<MClientCaps>(
                CEPH_CAP_OP_FLUSHSNAP_ACK, m->get_ino(), 0, 0, 0, 0, 0, dirty, 0, mds->get_osd_epoch_barrier());
        }
        else {
            if (mds->logger) mds->logger->inc(l_mdss_ceph_cap_op_flush_ack);
            ack = make_message<MClientCaps>(CEPH_CAP_OP_FLUSH_ACK,
                                            m->get_ino(),
                                            0,
                                            m->get_cap_id(),
                                            m->get_seq(),
        ...
        if (session->trim_completed_flushes(m->get_oldest_flush_tid())) {
            ...
            if (session->get_num_completed_flushes() >=
                ...
                *css << "client." << session->get_client() << " does not advance its oldest_flush_tid ("
                     << m->get_oldest_flush_tid() << "), " << session->get_num_completed_flushes()
    ...
    if (op == CEPH_CAP_OP_FLUSHSNAP) {
        ...
        if (snap != CEPH_NOSNAP) {
            ...
            if (in != head_in) dout(10) << " snapped inode " << *in << dendl;
        }
        ...
        if (dirty) {
            ack = make_message<MClientCaps>(
                CEPH_CAP_OP_FLUSHSNAP_ACK, in->ino(), 0, 0, 0, 0, 0, dirty, 0, mds->get_osd_epoch_barrier());
            ack->set_snap_follows(follows);
            ack->set_client_tid(m->get_client_tid());
        ...
        if (in == head_in ||
            (head_in->client_need_snapflush.count(snap) && head_in->client_need_snapflush[snap].count(client))) {
            ...
            if (in == head_in) cap->client_follows = snap < CEPH_NOSNAP ? snap : realm->get_newest_seq();

            _do_snap_update(in, snap, dirty, follows, client, m, ack);

            if (in != head_in) head_in->remove_need_snapflush(in, snap, client);
        }
        else {
    ...
    if (cap->get_cap_id() != m->get_cap_id()) {
        ...
        dout(10) << " follows " << follows << " retains " << ccap_string(m->get_caps()) << " dirty "
                 << ccap_string(dirty) << " on " << *in << dendl;
        ...
        if (cap->need_snapflush() && !(m->flags & MClientCaps::FLAG_PENDING_CAPSNAP)) cap->clear_needsnapflush();

        if (dirty && in->is_auth()) {
            dout(7) << " flush client." << client << " dirty " << ccap_string(dirty) << " seq " << m->get_seq()
                    << " on " << *in << dendl;
            ack = make_message<MClientCaps>(CEPH_CAP_OP_FLUSH_ACK,
                                            in->ino(),
                                            0,
        ...
        if (in->is_auth() && _do_cap_update(in, cap, dirty, follows, m, ack, &need_flush)) {
            // updated
            eval(in, CEPH_CAP_LOCKS);
            ...
            if (!did_issue && (cap->wanted() & ~cap->pending())) issue_caps(in, cap);

            if (cap->get_last_seq() == 0 && (cap->pending() & (CEPH_CAP_FILE_WR | CEPH_CAP_FILE_BUFFER))) {
                share_inode_max_size(in, cap);
            }
        }

        if (need_flush) mds->mdlog->flush();
    }

out:
    if (need_unpin) head_in->auth_unpin(this);
}
                                         ...
                                         std::string_view dname)
{
    ...
    if (dname.length()) {
        ...
        if (dir) {
            ...
            if (dn) {
                ...
                dout(7) << __func__ << " client." << client << " released lease on dn " << dir->dirfrag() << "/"
                        << dname << " which dne" << dendl;
            }
        }
    }
    ...
    if (!in->client_need_snapflush.empty() && (cap->issued() & CEPH_CAP_ANY_FILE_WR) == 0) {
        _do_null_snapflush(in, client);
    }

    adjust_cap_wanted(cap, wanted, issue_seq);

    if (mdr) cap->inc_suppress();
    eval(in, CEPH_CAP_LOCKS);
    if (mdr) cap->dec_suppress();

    // take note; we may need to reissue on this cap later
    if (mdr) mdr->cap_releases[in->vino()] = cap->get_last_seq();
}
...
{
    Capability* cap = in->get_client_cap(client);
    if (!cap || cap->get_last_seq() != seq) return;
    if (in->is_frozen()) {
        dout(10) << "kick_issue_caps waiting for unfreeze on " << *in << dendl;
        in->add_waiter(CInode::WAIT_UNFREEZE, new C_Locker_RetryKickIssueCaps(this, in, client, seq));
        return;
    }
...
{
    client_t client = mdr->get_client();
    for (map<vinodeno_t, ceph_seq_t>::iterator p = mdr->cap_releases.begin(); p != mdr->cap_releases.end(); ++p) {
 ...
 */
void Locker::_do_snap_update(CInode* in, snapid_t snap, int dirty, snapid_t follows, client_t client,
                             const cref_t<MClientCaps>& m, const ref_t<MClientCaps>& ack)
{
    dout(10) << "_do_snap_update dirty " << ccap_string(dirty) << " follows " << follows << " snap " << snap << " on "
             << *in << dendl;

    if (snap == CEPH_NOSNAP) {
        // hmm, i guess snap was already deleted?  just ack!
        dout(10) << " wow, the snap following " << follows << " was already deleted.  nothing to record, just ack."
                 << dendl;
        if (ack) {
    ...
    bool xattrs = (dirty & CEPH_CAP_XATTR_EXCL) && m->xattrbl.length() &&
    ...
    if (oi) {
        ...
        pi.inode->version = in->pre_dirty();
        i = &oi->inode;
        if (xattrs) px = &oi->xattrs;
    }
    else {
        auto pi = in->project_inode(mut, xattrs);
        pi.inode->version = in->pre_dirty();
        i = pi.inode.get();
        if (xattrs) px = pi.xattrs.get();
    }
    ...
    // xattr
    if (xattrs) {
        dout(7) << " xattrs v" << i->xattr_version << " -> " << m->head.xattr_version << " len " << m->xattrbl.length()
                << dendl;
    ...
    if (_old_inodes) in->reset_old_inodes(std::move(_old_inodes));
    ...
    if (ack && ack->get_oldest_flush_tid() > 0)
        le->metablob.add_client_flush(metareqid_t(m->get_source(), ack->get_client_tid()), ack->get_oldest_flush_tid());

    mds->mdlog->submit_entry(le, new C_Locker_FileUpdate_finish(this, in, mut, UPDATE_SNAPFLUSH, ack, client));
}
...
void Locker::_update_cap_fields(CInode* in, int dirty, const cref_t<MClientCaps>& m, CInode::mempool_inode* pi)
{
    ...
    if (m->get_ctime() > pi->ctime) {
        ...
        if (m->get_ctime() > pi->rstat.rctime) pi->rstat.rctime = m->get_ctime();
    }

    if ((features & CEPH_FEATURE_FS_CHANGE_ATTR) && m->get_change_attr() > pi->change_attr) {
    ...
    if (dirty & (CEPH_CAP_FILE_EXCL | CEPH_CAP_FILE_WR)) {
        ...
        if (((dirty & CEPH_CAP_FILE_WR) && mtime > pi->mtime) || ((dirty & CEPH_CAP_FILE_EXCL) && mtime != pi->mtime)) {
            dout(7) << "  mtime " << pi->mtime << " -> " << mtime << " for " << *in << dendl;
            pi->mtime = mtime;
            if (mtime > pi->rstat.rctime) pi->rstat.rctime = mtime;
        }
        ...
        if (in->is_file() && (dirty & CEPH_CAP_FILE_WR) && inline_version > pi->inline_data.version) {
        ...
        if ((dirty & CEPH_CAP_FILE_EXCL) && ceph_seq_cmp(pi->time_warp_seq, m->get_time_warp_seq()) < 0) {
            dout(7) << "  time_warp_seq " << pi->time_warp_seq << " -> " << m->get_time_warp_seq() << " for " << *in
                    << dendl;
            pi->time_warp_seq = m->get_time_warp_seq();
        }
        if (m->fscrypt_file.size()) pi->fscrypt_file = m->fscrypt_file;
    }
    // auth
    if (dirty & CEPH_CAP_AUTH_EXCL) {
        ...
        if (m->fscrypt_auth.size()) pi->fscrypt_auth = m->fscrypt_auth;
    }
}
 ...
 */
bool Locker::_do_cap_update(CInode* in, Capability* cap, int dirty, snapid_t follows, const cref_t<MClientCaps>& m,
                            const ref_t<MClientCaps>& ack, bool* need_flush)
{
    dout(10) << "_do_cap_update dirty " << ccap_string(dirty) << " issued " << ccap_string(cap ? cap->issued() : 0)
             << " wanted " << ccap_string(cap ? cap->wanted() : 0) << " on " << *in << dendl;
    ceph_assert(in->is_auth());
    client_t client = m->get_source().num();
    const auto& latest = in->get_projected_inode();
    ...
    if (in->is_file()) {
        ...
        if (cap && ((cap->issued() | cap->wanted()) & CEPH_CAP_ANY_FILE_WR)) {
            dout(20) << "client has write caps; m->get_max_size=" << m->get_max_size() << "; old_max=" << old_max
                     << dendl;
        ...
        if (in->last == CEPH_NOSNAP && change_max && !in->filelock.can_wrlock(client) &&
            !in->filelock.can_force_wrlock(client)) {
            dout(10) << " i want to change file_max, but lock won't allow it (yet)" << dendl;
            if (in->filelock.is_stable()) {
                bool need_issue = false;
                if (cap) cap->inc_suppress();
                if (in->get_mds_caps_wanted().empty() &&
                    (in->get_loner() >= 0 || (in->get_wanted_loner() >= 0 && in->try_set_loner()))) {
                    if (in->filelock.get_state() != LOCK_EXCL) file_excl(&in->filelock, &need_issue);
                }
                else
                    simple_lock(&in->filelock, &need_issue);
                if (need_issue) issue_caps(in);
                if (cap) cap->dec_suppress();
            }
            if (!in->filelock.can_wrlock(client) && !in->filelock.can_force_wrlock(client)) {
                C_MDL_CheckMaxSize* cms =
                    new C_MDL_CheckMaxSize(this, in, forced_change_max ? new_max : 0, 0, utime_t());
    ...
    if (!dirty && !change_max) return false;

    Session* session = mds->get_session(m);
    if (session->check_access(in, MAY_WRITE, m->caller_uid, m->caller_gid, NULL, 0, 0) < 0) {
    ...
    if (change_max) {
        ...
        if (new_max) {
            ...
            in->mark_clientwriteable();
            if (cap) cap->mark_clientwriteable();
        }
        else {
            pi.inode->client_ranges.erase(client);
            if (pi.inode->client_ranges.empty()) in->clear_clientwriteable();
            if (cap) cap->clear_clientwriteable();
        }
    }
    ...
    // auth
    if (dirty & CEPH_CAP_AUTH_EXCL) wrlock_force(&in->authlock, mut);
    ...
    if (change_max) update_flags |= UPDATE_SHAREMAX;
    if (cap) update_flags |= UPDATE_NEEDSISSUE;
    mds->mdlog->submit_entry(le, new C_Locker_FileUpdate_finish(this, in, mut, update_flags, ack, client));
...
public:
    C_Locker_RetryCapRelease(Locker* l, client_t c, inodeno_t i, uint64_t id, ceph_seq_t mseq, ceph_seq_t seq)
        : LockerContext(l)
        , client(c)
        , ino(i)
        , cap_id(id)
        ...
        , issue_seq(seq)
    {}
    void finish(int r) override { locker->_do_cap_release(client, ino, cap_id, migrate_seq, issue_seq); }
};

void Locker::_do_cap_release(client_t client, inodeno_t ino, uint64_t cap_id, ceph_seq_t mseq, ceph_seq_t seq)
{
    CInode* in = mdcache->get_inode(ino);
    if (!in) {
    ...
    if (should_defer_client_cap_frozen(in)) {
        ...
        in->add_waiter(CInode::WAIT_UNFREEZE, new C_Locker_RetryCapRelease(this, client, ino, cap_id, mseq, seq));
        return;
    }
...
void Locker::remove_client_cap(CInode* in, Capability* cap, bool kill)
{
    ...
    if (!in->client_need_snapflush.empty()) _do_null_snapflush(in, client);
 ...
 */
bool Locker::any_late_revoking_caps(xlist<Capability*> const& revoking, double timeout) const
{
...
void Locker::caps_tick()
{
    ...
    if (!need_snapflush_inodes.empty()) {
        ...
        while (!need_snapflush_inodes.empty()) {
            ...
            if (in->last_dirstat_prop >= cutoff) break;
            in->item_caps.remove_myself();
            snapflush_nudge(in);
            if (in == last) break;
        }
    ...
    for (xlist<Capability*>::iterator p = revoking_caps.begin(); !p.end(); ++p) {
        ...
        dout(20) << __func__ << " age = " << age << " client." << cap->get_client() << "." << cap->get_inode()->ino()
                 << dendl;
        ...
        if (age > mds->mdsmap->get_session_timeout() * (1 << cap->get_num_revoke_warnings())) {
            ...
            *css << "client." << cap->get_client() << " isn't responding to mclientcaps(revoke), ino "
                 << cap->get_inode()->ino() << " pending " << ccap_string(cap->pending()) << " issued "
                 << ccap_string(cap->issued()) << ", sent " << age << " seconds ago";
            mds->clog->warn() << css->strv();
            dout(20) << __func__ << " " << css->strv() << dendl;
        }
        else {
            dout(20) << __func__ << " silencing log message (backoff) for "
                     << "client." << cap->get_client() << "." << cap->get_inode()->ino() << dendl;
        }
    }
}
...
void Locker::issue_client_lease(CDentry* dn, CInode* in, MDRequestRef& mdr, utime_t now, bufferlist& bl)
{
    client_t client = mdr->get_client();
    Session* session = mdr->session;

    CInode* diri = dn->get_dir()->get_inode();
    if (mdr->snapid == CEPH_NOSNAP && dn->lock.can_lease(client) &&
...
void Locker::revoke_client_leases(SimpleLock* lock)
{
    ...
    for (map<client_t, ClientLease*>::iterator p = dn->client_lease_map.begin(); p != dn->client_lease_map.end(); ++p) {
        ...
        CInode* diri = dn->get_dir()->get_inode();
        auto lease = make_message<MClientLease>(
            CEPH_MDS_LEASE_REVOKE, l->seq, mask, diri->ino(), diri->first, CEPH_NOSNAP, dn->get_name());
        mds->send_message_client_counted(lease, l->client);
    }
}

void Locker::encode_lease(bufferlist& bl, const session_info_t& info, const LeaseStat& ls)
{
...
SimpleLock* Locker::get_lock(int lock_type, const MDSCacheObjectInfo& info)
{
    ...
    case CEPH_LOCK_DN:
    {
        ...
        if (diri) {
            ...
            if (dir) dn = dir->lookup(info.dname, info.snapid);
        }
        if (!dn) {
...
void Locker::handle_lock(const cref_t<MLock>& m)
{
    ...
    case CEPH_LOCK_IXATTR:
    case CEPH_LOCK_IFLOCK:
    case CEPH_LOCK_IPOLICY: handle_simple_lock(lock, m); break;
    ...
    case CEPH_LOCK_IFILE: handle_file_lock(static_cast<ScatterLock*>(lock), m); break;
...
void Locker::handle_reqrdlock(SimpleLock* lock, const cref_t<MLock>& m)
{
    ...
    if (parent->is_auth() && lock->get_state() != LOCK_SYNC && !parent->is_frozen()) {
        ...
        if (lock->is_stable()) {
            ...
            lock->add_waiter(SimpleLock::WAIT_STABLE | MDSCacheObject::WAIT_UNFREEZE, new C_MDS_RetryMessage(mds, m));
        }
    }
    else {
...
void Locker::handle_simple_lock(SimpleLock* lock, const cref_t<MLock>& m)
{
    ...
    case LOCK_AC_LOCK:
        ...
        if (lock->is_leased()) revoke_client_leases(lock);
        eval_gather(lock, true);
        if (lock->is_unstable_and_locked()) {
            if (lock->is_cached()) invalidate_lock_caches(lock);
            mds->mdlog->flush();
        }
        break;
    ...
    case LOCK_AC_LOCKACK:
        ceph_assert(lock->get_state() == LOCK_SYNC_LOCK || lock->get_state() == LOCK_SYNC_EXCL);
    ...
    case LOCK_AC_REQRDLOCK: handle_reqrdlock(lock, m); break;
    }
}
...
void Locker::simple_eval(SimpleLock* lock, bool* need_issue)
{
    ...
    if (lock->get_parent()->is_freezing_or_frozen()) {
        ...
        if ((lock->get_type() != CEPH_LOCK_DN && lock->get_type() != CEPH_LOCK_ISNAP &&
            ...
            lock->get_state() == LOCK_SYNC || lock->get_parent()->is_frozen())
            return;
    }
    ...
    // -> excl?
    if (lock->get_state() != LOCK_EXCL && in && in->get_target_loner() >= 0 && (wanted & CEPH_CAP_GEXCL)) {
...
bool Locker::simple_sync(SimpleLock* lock, bool* need_issue)
{
    ...
    if (old_state != LOCK_TSYN) {
        ...
        if (gather) {
            lock->get_parent()->auth_pin(lock);
            if (need_recover) mds->mdcache->do_file_recover();
            return false;
        }
    }
...
void Locker::simple_excl(SimpleLock* lock, bool* need_issue)
{
    ...
    if (lock->is_wrlocked()) gather++;
    if (gather && lock->is_cached()) invalidate_lock_caches(lock);

    if (lock->get_parent()->is_replicated() && lock->get_state() != LOCK_LOCK_EXCL &&
...
void Locker::simple_lock(SimpleLock* lock, bool* need_issue)
{
    ...
    if (lock->get_parent()->is_replicated() && lock->get_state() == LOCK_MIX_LOCK && gather) {
        ...
        if (lock->get_state() == LOCK_MIX_LOCK) lock->set_state(LOCK_MIX_LOCK2);
...
void Locker::simple_xlock(SimpleLock* lock)
{
    ...
    if (lock->is_stable()) lock->get_parent()->auth_pin(lock);
...
public:
    C_Locker_ScatterWB(Locker* l, ScatterLock* sl, MutationRef& m)
        ...
        , mut(m)
    {}
    void finish(int r) override { locker->scatter_writebehind_finish(lock, mut); }
};
...
void Locker::scatter_writebehind_finish(ScatterLock* lock, MutationRef& mut)
{
    ...
    if (in->is_replicated()) {
        ...
        case LOCK_MIX_LOCK2:
        case LOCK_MIX_EXCL:
        case LOCK_MIX_TSYN: send_lock_message(lock, LOCK_AC_LOCKFLUSHED);
        }
    }
    ...
    if (lock->is_stable()) lock->finish_waiters(ScatterLock::WAIT_STABLE);
...
void Locker::scatter_eval(ScatterLock* lock, bool* need_issue)
{
    ...
    if (!lock->is_rdlocked() && lock->get_state() != LOCK_MIX && lock->get_scatter_wanted()) {
    ...
    if (lock->get_type() == CEPH_LOCK_INEST) {
        ...
        if (!lock->is_rdlocked()) {
            if (lock->get_parent()->is_replicated()) {
                if (lock->get_state() != LOCK_MIX) scatter_mix(lock, need_issue);
            }
            else {
                if (lock->get_state() != LOCK_LOCK) simple_lock(lock, need_issue);
            }
        }
        return;
    }
    ...
    if (!in->has_subtree_or_exporting_dirfrag() || in->is_base()) {
        // i _should_ be sync.
        if (!lock->is_wrlocked() && lock->get_state() != LOCK_SYNC) {
...
void Locker::mark_updated_scatterlock(ScatterLock* lock)
{
    ...
    if (lock->get_updated_item()->is_on_list()) {
        dout(10) << "mark_updated_scatterlock " << *lock << " - already on list since " << lock->get_update_stamp()
                 << dendl;
    }
    else {
...
void Locker::scatter_nudge(ScatterLock* lock, MDSContext* c, bool forcelockchange)
{
    ...
    if (p->is_auth()) {
        ...
        while (true) {
            if (lock->is_stable()) {
                ...
                */
                ...
                if (c) lock->add_waiter(SimpleLock::WAIT_STABLE, c);
                return;
            }
        }
    }
        ...
        if (lock->is_dirty()) updated_scatterlocks.push_back(lock->get_updated_item());
    }
}
...
void Locker::scatter_tick()
{
    ...
    while (!updated_scatterlocks.empty()) {
        ...
        if (now - lock->get_update_stamp() < g_conf()->mds_scatter_nudge_interval) break;
        updated_scatterlocks.pop_front();
...
void Locker::scatter_tempsync(ScatterLock* lock, bool* need_issue)
{
    ...
    if (lock->get_cap_shift() && in->is_head() && in->issued_caps_need_gather(lock)) {
        if (need_issue)
            *need_issue = true;
        else
            issue_caps(in);
    ...
    if (lock->get_state() == LOCK_MIX_TSYN && in->is_replicated()) {
        lock->init_gather();
        send_lock_message(lock, LOCK_AC_LOCK);
        gather++;
    }
...
void Locker::file_eval(ScatterLock* lock, bool* need_issue)
{
    ...
    dout(7) << "file_eval wanted=" << gcap_string(wanted) << " loner_wanted=" << gcap_string(loner_wanted)
            << " other_wanted=" << gcap_string(other_wanted) << "  filelock=" << *lock << " on " << *lock->get_parent()
            << dendl;

    ceph_assert(lock->get_parent()->is_auth());
    ceph_assert(lock->is_stable());

    if (lock->get_parent()->is_freezing_or_frozen()) return;
    ...
    if (lock->get_state() == LOCK_EXCL) {
        ...
        dout(7) << "file_eval loner_issued=" << gcap_string(loner_issued)
                << " other_issued=" << gcap_string(other_issued) << " xlocker_issued=" << gcap_string(xlocker_issued)
                << dendl;
        if (!((loner_wanted | loner_issued) & (CEPH_CAP_GEXCL | CEPH_CAP_GWR | CEPH_CAP_GBUFFER)) ||
            ...
            if (((other_wanted | loner_wanted) & CEPH_CAP_GWR) || lock->is_waiter_for(SimpleLock::WAIT_WR))
                scatter_mix(lock, need_issue);
    ...
    // * -> excl?
    else if (lock->get_state() != LOCK_EXCL && !lock->is_rdlocked() &&
    ...
    // * -> mixed?
    else if (lock->get_state() != LOCK_MIX && !lock->is_rdlocked() &&
             //! lock->is_waiter_for(SimpleLock::WAIT_WR) &&
             (lock->get_scatter_wanted() || (in->get_target_loner() < 0 && (wanted & CEPH_CAP_GWR)))) {
        dout(7) << "file_eval stable, bump to mixed " << *lock << " on " << *lock->get_parent() << dendl;
        scatter_mix(lock, need_issue);
    }

    // * -> sync?
    else if (lock->get_state() != LOCK_SYNC && !lock->is_wrlocked() &&   // drain wrlocks first!
             !lock->is_waiter_for(SimpleLock::WAIT_WR) && !(wanted & CEPH_CAP_GWR) &&
             !((lock->get_state() == LOCK_MIX) && in->is_dir() &&
               in->has_subtree_or_exporting_dirfrag())   // if we are a delegation point, stay where we are
                                                         //((wanted & CEPH_CAP_RD) ||
                                                         // in->is_replicated() ||
                                                         // lock->is_leased() ||
...
void Locker::file_excl(ScatterLock* lock, bool* need_issue)
{
    ...
    if (in->is_replicated() && lock->get_state() != LOCK_LOCK_EXCL &&
    ...
    if (in->is_head() && in->issued_caps_need_gather(lock)) {
        if (need_issue)
            *need_issue = true;
        else
            issue_caps(in);
...
void Locker::handle_file_lock(ScatterLock* lock, const cref_t<MLock>& m)
{
    ...
    dout(7) << "handle_file_lock a=" << lock->get_lock_action_name(m->get_action()) << " on " << *lock << " from mds."
            << from << " " << *in << dendl;
    ...
    switch (m->get_action()) {
        // -- replica --
    case LOCK_AC_SYNC:
        ceph_assert(lock->get_state() == LOCK_LOCK || lock->get_state() == LOCK_MIX ||
                    lock->get_state() == LOCK_MIX_SYNC2);
    ...
    case LOCK_AC_LOCKFLUSHED:
        ...
        if (lock->is_stable()) lock->finish_waiters(SimpleLock::WAIT_STABLE);
        break;

    case LOCK_AC_MIX:
        ceph_assert(lock->get_state() == LOCK_SYNC || lock->get_state() == LOCK_LOCK ||
        ...
        // -- auth --
    case LOCK_AC_LOCKACK:
        ceph_assert(lock->get_state() == LOCK_SYNC_LOCK || lock->get_state() == LOCK_MIX_LOCK ||
                    lock->get_state() == LOCK_MIX_LOCK2 || lock->get_state() == LOCK_MIX_EXCL ||
                    lock->get_state() == LOCK_SYNC_EXCL || lock->get_state() == LOCK_SYNC_MIX ||
                    lock->get_state() == LOCK_MIX_TSYN);
        ceph_assert(lock->is_gathering(from));
        lock->remove_gather(from);

        if (lock->get_state() == LOCK_MIX_LOCK || lock->get_state() == LOCK_MIX_LOCK2 ||
            lock->get_state() == LOCK_MIX_EXCL || lock->get_state() == LOCK_MIX_TSYN) {
            lock->decode_locked_state(m->get_data());
        ...
        if (lock->is_gathering()) {
            dout(7) << "handle_file_lock " << *in << " from " << from << ", still gathering " << lock->get_gather_set()
                    << dendl;
        }
        else {
    ...
    case LOCK_AC_REQSCATTER:
        if (lock->is_stable()) {
            ...
            dout(7) << "handle_file_lock got scatter request, !stable, marking scatter_wanted on " << *lock << " on "
    ...
    case LOCK_AC_REQUNSCATTER:
        if (lock->is_stable()) {
            ...
            dout(7) << "handle_file_lock ignoring unscatter request on " << *lock << " on " << *lock->get_parent()
                    << dendl;
    ...
    case LOCK_AC_NUDGE:
        if (!lock->get_parent()->is_auth()) {
            dout(7) << "handle_file_lock IGNORING nudge on non-auth " << *lock << " on " << *lock->get_parent()
                    << dendl;
        }
        else if (!lock->get_parent()->is_replicated()) {
            dout(7) << "handle_file_lock IGNORING nudge on non-replicated " << *lock << " on " << *lock->get_parent()
                    << dendl;
        }
        else {
