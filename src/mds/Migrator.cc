...
protected:
    Migrator* mig;
    MDSRank* get_mds() override { return mig->mds; }

public:
    explicit MigratorContext(Migrator* mig_)
        : mig(mig_)
    {
...
public:
    explicit MigratorLogContext(Migrator* mig_)
        : mig(mig_)
    {
        ceph_assert(mig != NULL);
    }
...
void Migrator::dispatch(const cref_t<Message>& m)
{
    switch (m->get_type()) {
        // import
    case MSG_MDS_EXPORTDIRDISCOVER: handle_export_discover(ref_cast<MExportDirDiscover>(m)); break;
    case MSG_MDS_EXPORTDIRPREP: handle_export_prep(ref_cast<MExportDirPrep>(m)); break;
    case MSG_MDS_EXPORTDIR:
        ...
        break;
    case MSG_MDS_EXPORTDIRFINISH: handle_export_finish(ref_cast<MExportDirFinish>(m)); break;
    case MSG_MDS_EXPORTDIRCANCEL:
        handle_export_cancel(ref_cast<MExportDirCancel>(m));
        break;

        // export
    case MSG_MDS_EXPORTDIRDISCOVERACK: handle_export_discover_ack(ref_cast<MExportDirDiscoverAck>(m)); break;
    case MSG_MDS_EXPORTDIRPREPACK: handle_export_prep_ack(ref_cast<MExportDirPrepAck>(m)); break;
    case MSG_MDS_EXPORTDIRACK: handle_export_ack(ref_cast<MExportDirAck>(m)); break;
    ...
    case MSG_MDS_EXPORTDIRNOTIFY:
        ...
        // caps
    case MSG_MDS_EXPORTCAPS: handle_export_caps(ref_cast<MExportCaps>(m)); break;
    case MSG_MDS_EXPORTCAPSACK: handle_export_caps_ack(ref_cast<MExportCapsAck>(m)); break;
    case MSG_MDS_GATHERCAPS: handle_gather_caps(ref_cast<MGatherCaps>(m)); break;

    default: derr << "migrator unknown message " << m->get_type() << dendl; ceph_abort_msg("migrator unknown message");
    }
}
...
void Migrator::find_stale_export_freeze()
{
     ...
     *   (wait because subtree A is freezing)
     */
    for (map<CDir*, export_state_t>::iterator p = export_state.begin(); p != export_state.end();) {
        CDir* dir = p->first;
        export_state_t& stat = p->second;
        ++p;
        if (stat.state != EXPORT_DISCOVERING && stat.state != EXPORT_FREEZING) continue;
        ...
        if (stat.last_cum_auth_pins_change >= cutoff) continue;
        if (stat.num_remote_waiters > 0 || (!dir->inode->is_root() && dir->get_parent_dir()->is_freezing())) {
...
void Migrator::export_try_cancel(CDir* dir, bool notify_peer)
{
    ...
    case EXPORT_DISCOVERING:
        ...
        dir->auth_unpin(this);
        if (notify_peer && (!mds->is_cluster_degraded() ||
    ...
    case EXPORT_FREEZING:
        ...
        it->second.state = EXPORT_CANCELLED;
        dir->unfreeze_tree();   // cancel the freeze
        if (dir->is_subtree_root()) mdcache->try_subtree_merge(dir);
        if (notify_peer && (!mds->is_cluster_degraded() ||
                            mds->mdsmap->is_clientreplay_or_active_or_stopping(it->second.peer)))   // tell them.
            mds->send_message_mds(make_message<MExportDirCancel>(dir->dirfrag(), it->second.tid), it->second.peer);
        break;
    ...
    case EXPORT_NOTIFYING:
        ...
        break;
    case EXPORT_CANCELLING: break;

    default: ceph_abort();
    }

    // finish clean-up?
    if (it->second.state == EXPORT_CANCELLING || it->second.state == EXPORT_CANCELLED) {
        MutationRef mut;
...
void Migrator::export_cancel_finish(export_state_iterator& it)
{
    ...
    if (parent) child_export_finish(parent, false);
}
...
void Migrator::handle_mds_failure_or_stop(mds_rank_t who)
{
    ...
    for (map<CDir*, export_state_t>::iterator p = export_state.begin(); p != export_state.end(); ++p) {
    ...
    while (p != export_state.end()) {
        ...
        if ((p->second.peer == who && p->second.state != EXPORT_CANCELLING) || p->second.state == EXPORT_LOCKING ||
            p->second.state == EXPORT_DISCOVERING || p->second.state == EXPORT_FREEZING ||
            p->second.state == EXPORT_PREPPING) {
            // the guy i'm exporting to failed, or we're just freezing.
            dout(10) << "cleaning up export state (" << p->second.state << ")" << get_export_statename(p->second.state)
                     << " of " << *dir << dendl;
            export_try_cancel(dir);
        }
            ...
            if (p->second.warning_ack_waiting.erase(who)) {
                if (p->second.state == EXPORT_WARNING) {
                    ...
                    dout(10) << "faking export_warning_ack from mds." << who << " on " << *dir << " to mds."
                             << p->second.peer << dendl;
                    if (p->second.warning_ack_waiting.empty()) export_go(dir);
                }
            }
            if (p->second.notify_ack_waiting.erase(who)) {
                // exporter is waiting for notify acks, fake it
                dout(10) << "faking export_notify_ack from mds." << who << " on " << *dir << " to mds."
                         << p->second.peer << dendl;
                if (p->second.state == EXPORT_NOTIFYING) {
                    if (p->second.notify_ack_waiting.empty()) export_finish(dir);
                }
        ...
        if (q->second.peer == who) {
            ...
            if (bystanders_entry != q->second.bystanders.end()) {
                ...
                if (q->second.state == IMPORT_ABORTING) {
                    ceph_assert(dir);
                    dout(10) << "faking export_notify_ack from mds." << who << " on aborting import " << *dir
                             << " from mds." << q->second.peer << dendl;
                    if (q->second.bystanders.empty()) import_reverse_unfreeze(dir);
                }
            }
        }
...
{
    dout(10) << dendl;
    for (map<dirfrag_t, import_state_t>::iterator p = import_state.begin(); p != import_state.end(); ++p) {
        CDir* dir = mdcache->get_dirfrag(p->first);
        if (dir) {
            dout(10) << " importing from " << p->second.peer << ": (" << p->second.state << ") "
                     << get_import_statename(p->second.state) << " " << p->first << " " << *dir << dendl;
        }
        else {
            dout(10) << " importing from " << p->second.peer << ": (" << p->second.state << ") "
                     << get_import_statename(p->second.state) << " " << p->first << dendl;
        }
    }
}
...
void Migrator::show_exporting()
{
    ...
    for (const auto& [dir, state] : export_state) {
        dout(10) << " exporting to " << state.peer << ": (" << state.state << ") " << get_export_statename(state.state)
...
void Migrator::audit()
{
    if (!g_conf()->subsys.should_gather<ceph_subsys_mds, 5>()) return;   // hrm.

    // import_state
    show_importing();
    for (map<dirfrag_t, import_state_t>::iterator p = import_state.begin(); p != import_state.end(); ++p) {
        if (p->second.state == IMPORT_DISCOVERING) continue;
        ...
        CDir* dir = mdcache->get_dirfrag(p->first);
        ceph_assert(dir);
        if (p->second.state == IMPORT_PREPPING) continue;
        ...
        ceph_assert(dir->is_ambiguous_dir_auth());
        ceph_assert(dir->authority().first == mds->get_nodeid() || dir->authority().second == mds->get_nodeid());
    }
    ...
    for (map<CDir*, export_state_t>::iterator p = export_state.begin(); p != export_state.end(); ++p) {
        CDir* dir = p->first;
        if (p->second.state == EXPORT_LOCKING || p->second.state == EXPORT_DISCOVERING ||
            p->second.state == EXPORT_FREEZING || p->second.state == EXPORT_CANCELLING)
            continue;
        ceph_assert(dir->is_ambiguous_dir_auth());
        ceph_assert(dir->authority().first == mds->get_nodeid() || dir->authority().second == mds->get_nodeid());
    }
...
void Migrator::maybe_do_queued_export()
{
    ...
    while (!export_queue.empty() && max_total_size > total_exporting_size &&
           max_total_size - total_exporting_size >= max_export_size * (num_locking_exports + 1)) {
...
public:
    ...
    {
        if (r >= 0) mig->export_frozen(dir, tid);
...
bool Migrator::export_try_grab_locks(CDir* dir, MutationRef& mut)
{
    ...
    if (!diri->filelock.can_wrlock(diri->get_loner()) || !diri->nestlock.can_wrlock(diri->get_loner())) return false;
    ...
    for (auto& bound : wouldbe_bounds) bound_inodes.insert(bound->get_inode());
    for (auto& in : bound_inodes) lov.add_rdlock(&in->dirfragtreelock);
    ...
    if (!mds->locker->rdlock_try_set(lov, mut)) return false;
...
void Migrator::export_dir(CDir* dir, mds_rank_t dest)
{
    ...
    else if (parent && parent->inode->is_stray() && parent->get_parent_dir()->ino() != MDS_INO_MDSDIR(dest)) {
 ...
 */
void Migrator::maybe_split_export(CDir* dir, uint64_t max_size, bool null_okay, vector<pair<CDir*, size_t> >& results)
{
        ...
        while (it != cur->end()) {
            ...
            if (in->is_dir()) {
                ...
                for (auto p = ls.begin(); p != ls.end();) {
                    if ((*p)->state_test(CDir::STATE_EXPORTING) || (*p)->is_freezing_dir() || (*p)->is_frozen_dir()) {
                        complete = false;
                        p = ls.erase(p);
                    }
                    else {
    ...
    for (auto& p : stack) results.insert(results.end(), p.subdirs.begin(), p.subdirs.end());

    if (results.empty() && (!skipped_size || !null_okay)) results.emplace_back(dir, found_size + skipped_size);
}
...
public:
    C_M_ExportDirWait(Migrator* m, MDRequestRef mdr, int count)
        ...
        , count(count)
    {}
    void finish(int r) override { mig->dispatch_export_dir(mdr, count); }
};
...
void Migrator::dispatch_export_dir(MDRequestRef& mdr, int count)
{
    ...
    if (!(mdr->locking_state & MutationImpl::ALL_LOCKED)) {
        ...
        if (!mds->locker->acquire_locks(mdr, lov, nullptr, true)) {
            if (mdr->aborted) export_try_cancel(dir);
            return;
        }
        ...
        if (!mds->locker->rdlock_try_set(lov, mdr)) return;

        if (!mds->locker->try_rdlock_snap_layout(dir->get_inode(), mdr)) return;
    ...
    if (results.size() == 1 && results.front().first == dir) {
        ...
        auto discover = make_message<MExportDirDiscover>(dir->dirfrag(), path, mds->get_nodeid(), it->second.tid);
    ...
    if (parent) {
        ...
        parent = std::make_shared<export_base_t>(dir->dirfrag(), dest, results.size(), export_queue_gen);
    }

    if (results.empty()) {
        dout(7) << "subtree's children all are under exporting, retry rest parts of parent export " << parent->dirfrag
                << dendl;
        parent->restart = true;
    }
    else {
...
{
    if (success) parent->restart = true;
    if (--parent->pending_children == 0) {
        if (parent->restart && parent->export_queue_gen == export_queue_gen) {
...
void Migrator::handle_export_discover_ack(const cref_t<MExportDirDiscoverAck>& m)
{
    ...
    map<CDir*, export_state_t>::iterator it = export_state.find(dir);
    if (it == export_state.end() || it->second.tid != m->get_tid() || it->second.peer != dest) {
...
void Migrator::export_sessions_flushed(CDir* dir, uint64_t tid)
{
    ...
    map<CDir*, export_state_t>::iterator it = export_state.find(dir);
    if (it == export_state.end() || it->second.state == EXPORT_CANCELLING || it->second.tid != tid) {
        // export must have aborted.
    ...
    if (it->second.state == EXPORT_WARNING && it->second.warning_ack_waiting.empty()) export_go(dir);   // start export.
}

void Migrator::encode_export_prep_trace(bufferlist& final_bl, CDir* bound, CDir* dir, export_state_t& es,
                                        set<inodeno_t>& inodes_added, set<dirfrag_t>& dirfrags_added)
{
    ...
    while (1) {
        // don't repeat inodes
        if (inodes_added.count(cur->inode->ino())) break;
...
void Migrator::export_frozen(CDir* dir, uint64_t tid)
{
    ...
    CInode* diri = dir->get_inode();
    if ((diri->is_auth() && diri->is_frozen()) || !export_try_grab_locks(dir, it->second.mut)) {
        dout(7) << "export_dir couldn't acquire all needed locks, failing. " << *dir << dendl;
        export_try_cancel(dir);
        return;
    }

    if (diri->is_auth()) it->second.mut->auth_pin(diri);
...
void Migrator::handle_export_prep_ack(const cref_t<MExportDirPrepAck>& m)
{
    ...
    map<CDir*, export_state_t>::iterator it = export_state.find(dir);
    if (it == export_state.end() || it->second.tid != m->get_tid() ||
    ...
    ceph_assert(it->second.warning_ack_waiting.empty() || (it->second.warning_ack_waiting.size() == 1 &&
    ...
    for (const auto& p : dir->get_replicas()) {
        ...
        auto notify = make_message<MExportDirNotify>(dir->dirfrag(),
                                                     it->second.tid,
                                                     true,
    ...
    // nobody to warn?
    if (it->second.warning_ack_waiting.empty()) export_go(dir);   // start export.
}
...
void Migrator::export_go_synced(CDir* dir, uint64_t tid)
{
    ...
    encode_export_dir(req->export_data,
    ...
    set<CDir*> bounds;
    mdcache->get_subtree_bounds(dir, bounds);
    for (set<CDir*>::iterator p = bounds.begin(); p != bounds.end(); ++p) req->add_export((*p)->dirfrag());
 ...
 */
void Migrator::encode_export_inode(CInode* in, bufferlist& enc_state, map<client_t, entity_inst_t>& exported_client_map,
                                   map<client_t, client_metadata_t>& exported_client_metadata_map)
{
                                        ...
                                        map<client_t, client_metadata_t>& exported_client_metadata_map)
{
    ...
    for (const auto& p : in->get_client_caps()) {
        if (exported_client_map.count(p.first)) continue;
...
void Migrator::finish_export_inode_caps(CInode* in, mds_rank_t peer, map<client_t, Capability::Import>& peer_imported)
{
    dout(20) << *in << dendl;
    ...
    for (const auto& p : in->get_client_caps()) {
        ...
        auto m = make_message<MClientCaps>(
            CEPH_CAP_OP_EXPORT, in->ino(), 0, cap->get_cap_id(), cap->get_mseq(), mds->get_osd_epoch_barrier());
        map<client_t, Capability::Import>::iterator q = peer_imported.find(p.first);
        ceph_assert(q != peer_imported.end());
        m->set_cap_peer(q->second.cap_id, q->second.issue_seq, q->second.mseq, (q->second.cap_id > 0 ? peer : -1), 0);
...
void Migrator::finish_export_inode(CInode* in, mds_rank_t peer, map<client_t, Capability::Import>& peer_imported,
                                   MDSContext::vec& finished)
{
    ...
    if (!in->has_subtree_root_dirfrag(mds->get_nodeid())) in->clear_scatter_dirty();
...
void Migrator::encode_export_dir(bufferlist& exportbl, CDir* dir, map<client_t, entity_inst_t>& exported_client_map,
                                 map<client_t, client_metadata_t>& exported_client_metadata_map, uint64_t& num_exported)
{
    ...
    for (auto& p : *dir) {
        ...
        encode_export_inode(
            in, exportbl, exported_client_map, exported_client_metadata_map);   // encode, and (update state for) export
...
public:
    C_MDS_ExportFinishLogged(Migrator* m, CDir* d)
        : MigratorLogContext(m)
        , dir(d)
    {}
    void finish(int r) override { mig->export_logged_finish(dir); }
};
...
void Migrator::export_reverse(CDir* dir, export_state_t& stat)
{
    ...
    for (auto in : to_eval) {
        ...
        if (need_issue && (!in->is_auth() || !mds->locker->eval(in, CEPH_CAP_LOCKS))) mds->locker->issue_caps(in);
    }
...
void Migrator::export_logged_finish(CDir* dir)
{
    ...
    if (stat.notify_ack_waiting.empty()) {
        ...
        if (!mds->is_cluster_degraded() || mds->mdsmap->is_clientreplay_or_active_or_stopping(stat.peer)) {
...
void Migrator::handle_export_notify_ack(const cref_t<MExportDirNotifyAck>& m)
{
    ...
    if (export_state_entry != export_state.end()) {
        ...
        if (stat.state == EXPORT_WARNING && stat.warning_ack_waiting.erase(from)) {
            // exporting. process warning.
            dout(7) << "from " << m->get_source() << ": exporting, processing warning on " << *dir << dendl;
            if (stat.warning_ack_waiting.empty()) export_go(dir);   // start export.
        }
        else if (stat.state == EXPORT_NOTIFYING && stat.notify_ack_waiting.erase(from)) {
            // exporting. process notify.
            dout(7) << "from " << m->get_source() << ": exporting, processing notify on " << *dir << dendl;
            if (stat.notify_ack_waiting.empty()) export_finish(dir);
        }
        ...
        if (import_state_entry != import_state.end()) {
            ...
            if (stat.state == IMPORT_ABORTING) {
                ...
                if (stat.bystanders.empty()) import_reverse_unfreeze(dir);
            }
        }
    }
}
...
void Migrator::export_finish(CDir* dir)
{
    ...
    if (!mds->is_cluster_degraded() || mds->mdsmap->is_clientreplay_or_active_or_stopping(it->second.peer)) {
    ...
    int num_dentries = 0;
    MDSContext::vec finished;
    finish_export_dir(dir, it->second.peer, it->second.peer_imported, finished, &num_dentries);
    ...
    if (dir->state_test(CDir::STATE_AUXSUBTREE)) dir->state_clear(CDir::STATE_AUXSUBTREE);
    ...
    if (parent) child_export_finish(parent, true);
...
{
public:
    C_MDS_ExportDiscover(Migrator* mig, const cref_t<MExportDirDiscover>& m)
        : MigratorContext(mig)
        , m(m)
    {}
    void finish(int r) override { mig->handle_export_discover(m, true); }
...
{
public:
    C_MDS_ExportDiscoverFactory(Migrator* mig, cref_t<MExportDirDiscover> m)
        : mig(mig)
        , m(m)
    {}
    MDSContext* build() { return new C_MDS_ExportDiscover(mig, m); }
...
void Migrator::handle_export_discover(const cref_t<MExportDirDiscover>& m, bool started)
{
    ...
    if (!started) {
        ...
        if (it == import_state.end() || it->second.peer != from || it->second.tid != m->get_tid()) {
    ...
    if (!in) {
        ...
        int r = mdcache->path_traverse(null_ref, cf, fpath, MDS_TRAVERSE_DISCOVER | MDS_TRAVERSE_PATH_LOCKED, &trace);
        if (r > 0) return;
        if (r < 0) {
...
{
public:
    C_MDS_ExportPrep(Migrator* mig, const cref_t<MExportDirPrep>& m)
        : MigratorContext(mig)
        , m(m)
    {}
    void finish(int r) override { mig->handle_export_prep(m, true); }
...
{
public:
    C_MDS_ExportPrepFactory(Migrator* mig, cref_t<MExportDirPrep> m)
        : mig(mig)
        , m(m)
    {}
    MDSContext* build() { return new C_MDS_ExportPrep(mig, m); }
...
void Migrator::handle_export_prep(const cref_t<MExportDirPrep>& m, bool did_assim)
{
    ...
    if (!did_assim) {
        ...
        if (it == import_state.end() || it->second.peer != oldauth || it->second.tid != m->get_tid()) {
    ...
    if (!did_assim) {
        ...
        // make bound sticky
        for (map<inodeno_t, fragset_t>::iterator p = import_bound_fragset.begin(); p != import_bound_fragset.end();
             ++p) {
            p->second.simplify();
    ...
    if (mds->is_active()) {
        ...
        if (!mdcache->is_readonly() &&
            ...
            diri->filelock.can_wrlock(diri->get_loner()) && diri->nestlock.can_wrlock(diri->get_loner())) {
            it->second.mut = new MutationImpl();
            ...
            mdcache->adjust_bounded_subtree_auth(dir, import_bounds, pair<int, int>(oldauth, mds->get_nodeid()));
    ...
    if (!success) import_reverse_prepping(dir, it->second);
...
void Migrator::handle_export_dir(const cref_t<MExportDir>& m)
{
    ...
    ceph_assert(cmp.end());
    le->cmapv =
        mds->server->prepare_force_open_sessions(client_map, client_metadata_map, onlogged->imported_session_map);
...
void Migrator::import_remove_pins(CDir* dir, set<CDir*>& bounds)
{
    ...
    // bounding inodes
    set<inodeno_t> did;
    for (list<dirfrag_t>::iterator p = stat.bound_ls.begin(); p != stat.bound_ls.end(); ++p) {
        if (did.count(p->ino)) continue;
        did.insert(p->ino);
...
public:
    ...
    C_MDC_QueueContexts(Migrator* m)
        : MigratorContext(m)
    {}
    void finish(int r) override
    {
...
void Migrator::import_reverse(CDir* dir)
{
    ...
    ceph_assert(dir->is_subtree_root());
    if (mds->is_resolve()) mdcache->trim_non_auth_subtree(dir);
        ...
        for (auto& p : *cur) {
            ...
            if (dn->get_linkage()->is_primary()) {
                ...
                for (const auto& dir : dfs) {
                    if (bounds.count(dir) == 0) q.push_back(dir);
                }
            }
    ...
    if (stat.state == IMPORT_ACKING) {
        ...
        for (map<CInode*, map<client_t, Capability::Export> >::iterator p = stat.peer_exports.begin();
             ...
             ++p) {
            CInode* in = p->first;
            for (map<client_t, Capability::Export>::iterator q = p->second.begin(); q != p->second.end(); ++q) {
...
void Migrator::import_notify_finish(CDir* dir, set<CDir*>& bounds)
{
    ...
    import_state_t& stat = import_state[dir->dirfrag()];
    for (set<mds_rank_t>::iterator p = stat.bystanders.begin(); p != stat.bystanders.end(); ++p) {
        auto notify = make_message<MExportDirNotify>(dir->dirfrag(),
                                                     stat.tid,
                                                     false,
...
void Migrator::import_notify_abort(CDir* dir, set<CDir*>& bounds)
{
    ...
    import_state_t& stat = import_state[dir->dirfrag()];
    for (set<mds_rank_t>::iterator p = stat.bystanders.begin(); p != stat.bystanders.end();) {
        if (mds->is_cluster_degraded() && !mds->mdsmap->is_clientreplay_or_active_or_stopping(*p)) {
                                   ...
                                   map<client_t, pair<Session*, uint64_t> >& imported_session_map)
{
    ...
    map<dirfrag_t, import_state_t>::iterator it = import_state.find(dir->dirfrag());
    if (it == import_state.end() || it->second.state != IMPORT_LOGGINGSTART) {
    ...
    for (map<CInode*, map<client_t, Capability::Export> >::iterator p = it->second.peer_exports.begin();
        ...
        finish_import_inode_caps(
            p->first, MDS_RANK_NONE, true, imported_session_map, p->second, imported_caps[p->first->ino()]);
    }
...
void Migrator::import_finish(CDir* dir, bool notify, bool last)
{
    ...
    if (it->second.state == IMPORT_ACKING) {
        for (map<CInode*, map<client_t, Capability::Export> >::iterator p = it->second.peer_exports.begin();
            ...
            for (map<client_t, Capability::Export>::iterator q = p->second.begin(); q != p->second.end(); ++q) {
                ...
                if (r == it->second.session_map.end()) continue;
    ...
    if (notify) import_notify_finish(dir, bounds);
    ...
    for (map<CInode*, map<client_t, Capability::Export> >::iterator p = peer_exports.begin(); p != peer_exports.end();
         ++p) {
        if (p->first->is_auth()) mds->locker->eval(p->first, CEPH_CAP_LOCKS, true);
    ...
    if (dir->ino() == MDS_INO_MDSDIR(mds->get_nodeid())) mdcache->populate_mydir();

    // is it empty?
    if (dir->get_num_head_items() == 0 && !dir->inode->is_auth()) {
        // reexport!
        export_empty_import(dir);
    }
}

void Migrator::decode_import_inode(CDentry* dn, bufferlist::const_iterator& blp, mds_rank_t oldauth, LogSegment* ls,
                                   map<CInode*, map<client_t, Capability::Export> >& peer_exports,
                                   list<ScatterLock*>& updated_scatterlocks)
{
    ...
    if (in->is_dir()) dn->dir->pop_lru_subdirs.push_back(&in->item_pop_lru);
    ...
    if (!in->get_inode()->client_ranges.empty()) in->mark_clientwriteable();
    ...
    if (in->is_replica(mds->get_nodeid())) in->remove_replica(mds->get_nodeid());

    if (in->snaplock.is_stable() && in->snaplock.get_state() != LOCK_SYNC) mds->locker->try_eval(&in->snaplock, NULL);

    if (in->policylock.is_stable() && in->policylock.get_state() != LOCK_SYNC)
        mds->locker->try_eval(&in->policylock, NULL);
}

void Migrator::decode_import_inode_caps(CInode* in, bool auth_cap, bufferlist::const_iterator& blp,
                                        map<CInode*, map<client_t, Capability::Export> >& peer_exports)
{
    DECODE_START(1, blp);
    map<client_t, Capability::Export> cap_map;
    ...
    if (!cap_map.empty() || (auth_cap && (in->get_caps_wanted() & ~CEPH_CAP_PIN))) {
                                        ...
                                        map<client_t, Capability::Import>& import_map)
{
    ...
    if (auth_cap) {
        if (r != client_ranges.cend()) needs_recover = true;
        if (needs_recover) in->state_set(CInode::STATE_NEEDSRECOVER);
    }
...
void Migrator::decode_import_dir(bufferlist::const_iterator& blp, mds_rank_t oldauth, CDir* import_root,
                                 ...
                                 list<ScatterLock*>& updated_scatterlocks, int& num_imported)
{
    ...
    if (dir->is_replica(mds->get_nodeid())) dir->remove_replica(mds->get_nodeid());

    // add to journal entry
    if (le) le->metablob.add_import_dir(dir);
    ...
    for (; nden > 0; nden--) {
        ...
        if (!dn) dn = dir->add_null_dentry(dname, 1, last);

        dn->decode_import(blp, ls);

        dn->add_replica(oldauth, CDentry::EXPORT_NONCE);
        if (dn->is_replica(mds->get_nodeid())) dn->remove_replica(mds->get_nodeid());

        // dentry lock in unreadable state can block path traverse
        if (dn->lock.get_state() != LOCK_SYNC) mds->locker->try_eval(&dn->lock, NULL);
        ...
        if (icode == 'N') {
            ...
            if (icode == 'i') {
                DECODE_START(2, blp);
                decode_import_inode(dn, blp, oldauth, ls, peer_exports, updated_scatterlocks);
        ...
        if (le) le->metablob.add_import_dentry(dn);
    }
...
void Migrator::handle_export_notify(const cref_t<MExportDirNotify>& m)
{
    ...
    if (!dir) {
        ...
        dout(7) << "old_auth was " << dir->authority() << " != " << old_auth << " -> " << new_auth << " on " << *dir
                << dendl;
    }
    else {
    ...
    // send ack
    if (m->wants_ack()) {
        mds->send_message_mds(make_message<MExportDirNotifyAck>(m->get_dirfrag(), m->get_tid(), m->get_new_auth()),
                              from);
    }
    else {
        // aborted.  no ack.
...
void Migrator::handle_export_caps_ack(const cref_t<MExportCapsAck>& ack)
{
    ...
    if (in) {
        ...
        for (auto& it : imported_caps) {
            Capability* cap = in->get_client_cap(it.first);
            if (!cap || cap->get_cap_id() != caps_ids.at(it.first)) continue;
...
void Migrator::handle_gather_caps(const cref_t<MGatherCaps>& m)
{
    ...
    if (in->is_any_caps() && !in->is_auth() && !in->is_ambiguous_auth() && !in->state_test(CInode::STATE_EXPORTINGCAPS))
        export_caps(in);
}
...
public:
    ...
    C_M_LoggedImportCaps(Migrator* m, CInode* i, mds_rank_t f)
        : MigratorLogContext(m)
        , in(i)
        , from(f)
    {}
    void finish(int r) override { mig->logged_import_caps(in, from, imported_session_map, peer_exports); }
};
...
void Migrator::handle_export_caps(const cref_t<MExportCaps>& ex)
{
    ...
    C_M_LoggedImportCaps* finish = new C_M_LoggedImportCaps(this, in, mds_rank_t(ex->get_source().num()));

    version_t pv =
        mds->server->prepare_force_open_sessions(client_map, client_metadata_map, finish->imported_session_map);
    // decode new caps
    ...
    ESessions* le = new ESessions(pv, std::move(client_map), std::move(client_metadata_map));
    mds->mdlog->start_submit_entry(le, finish);
    mds->mdlog->flush();
}


void Migrator::logged_import_caps(CInode* in, mds_rank_t from,
                                  map<client_t, pair<Session*, uint64_t> >& imported_session_map,
                                  map<CInode*, map<client_t, Capability::Export> >& peer_exports)
{
    dout(10) << *in << dendl;
    ...
    if (!imported_caps.empty()) {
        ...
        for (auto& p : imported_caps) peer_caps_ids[p.first] = it->second.at(p.first).cap_id;
...
Migrator::Migrator(MDSRank* m, MDCache* c)
    : mds(m)
    , mdcache(c)
{
...
{
    if (changed.count("mds_max_export_size")) max_export_size = g_conf().get_val<Option::size_t>("mds_max_export_size");
