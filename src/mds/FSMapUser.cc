...
void FSMapUser::encode(ceph::buffer::list& bl, uint64_t features) const
{
    ...
    for (auto p = filesystems.begin(); p != filesystems.end(); ++p) fs_list.push_back(p->second);
...
void FSMapUser::decode(ceph::buffer::list::const_iterator& p)
{
    ...
    decode(fs_list, p);
    filesystems.clear();
    for (auto p = fs_list.begin(); p != fs_list.end(); ++p) filesystems[p->cid] = *p;
    DECODE_FINISH(p);
}
...
void FSMapUser::print(std::ostream& out) const
{
    ...
    for (auto& p : filesystems) out << " id " << p.second.cid << " name " << p.second.name << std::endl;
}
...
void FSMapUser::print_summary(ceph::Formatter* f, std::ostream* out) const
{
    ...
    if (f) {
        ...
        *out << "e" << get_epoch() << ":";
        for (auto& p : filesystems) *out << " " << p.second.name << "(" << p.second.cid << ")";
    }
}
