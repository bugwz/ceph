...
void MDLog::create_logger()
{
    ...
    plb.add_u64(l_mdl_seg, "seg", "Segments", "segs", PerfCountersBuilder::PRIO_INTERESTING);
    ...
    plb.add_u64_counter(l_mdl_replayed, "replayed", "Events replayed", "repl", PerfCountersBuilder::PRIO_INTERESTING);
...
protected:
    ...
    void finish(int r) override
    {
        ...
        if (r == -CEPHFS_EBLOCKLISTED) {
            ...
            // intervenes.
            mds->clog->error() << "Unhandled journal write error on MDS rank " << mds->get_nodeid() << ": "
                               << cpp_strerror(r) << ", shutting down.";
            mds->damaged();
...
public:
    explicit C_MDL_WriteError(MDLog* m)
        : MDSIOContextBase(false)
        , mdlog(m)
    {}
    void print(ostream& out) const override { out << "mdlog_write_error"; }
};
...
void MDLog::create(MDSContext* c)
{
    ...
    // This requires an OnFinisher wrapper because Journaler will call back the completion for write_head inside its own
    // lock
    ...
    journaler = new Journaler(
        "mdlog", ino, mds->get_metadata_pool(), CEPH_FS_ONDISK_MAGIC, mds->objecter, logger, l_mdl_jlat, mds->finisher);
...
public:
    C_ReopenComplete(MDLog* mdlog_, MDSContext* on_complete_)
        : MDSInternalContext(mdlog_->mds)
        , mdlog(mdlog_)
...
void MDLog::_submit_entry(LogEvent* le, MDSLogContextBase* c)
{
    ...
    // start a new segment?
    if (le->get_type() == EVENT_SUBTREEMAP || (le->get_type() == EVENT_IMPORTFINISH && mds->is_resolve())) {
    ...
    else if (ls->end / period != ls->offset / period || ls->num_events >= g_conf()->mds_log_events_per_segment) {
        dout(10) << "submit_entry also starting new segment: last = " << ls->seq << "/" << ls->offset
                 << ", event seq = " << event_seq << dendl;
        _start_new_segment();
    }
...
public:
    C_MDL_Flushed(MDLog* m, MDSContext* w)
        ...
        , wrapped(w)
    {}
    C_MDL_Flushed(MDLog* m, uint64_t wp)
        : mdlog(m)
        , wrapped(NULL)
    {
        set_write_pos(wp);
...
void MDLog::_submit_thread()
{
    ...
    while (!mds->is_daemon_stopping()) {
        ...
        if (data.le) {
            ...
            if (le->get_type() == EVENT_SUBTREEMAP) ls->offset = write_pos;
            ...
            if (data.flush) journaler->flush();

            if (logger) logger->set(l_mdl_wrpos, ls->end);
...
void MDLog::wait_for_safe(MDSContext* c)
{
    ...
    if (no_pending && c) journaler->wait_for_flush(new C_IO_Wrapper(mds, c));
}
...
public:
    C_OFT_Committed(MDLog* l, uint64_t s)
        : MDSInternalContext(l->mds)
        , mdlog(l)
        , seq(s)
    {}
    void finish(int ret) override { mdlog->trim_expired_segments(); }
};
...
void MDLog::try_to_commit_open_file_table(uint64_t last_seq)
{
    ...
    if (mds->mdcache->open_file_table.is_any_committing()) return;
    ...
    if (mds->mdcache->open_file_table.is_any_dirty() ||
        ...
        submit_mutex.unlock();
        mds->mdcache->open_file_table.commit(new C_OFT_Committed(this, last_seq), last_seq, CEPH_MSG_PRIO_HIGH);
        submit_mutex.lock();
    }
}
...
void MDLog::trim(int m)
{
    ...
    // trim!
    dout(10) << "trim " << segments.size() << " / " << max_segments << " segments, " << num_events << " / "
             << max_events << " events"
             << ", " << expiring_segments.size() << " (" << expiring_events << ") expiring"
             << ", " << expired_segments.size() << " (" << expired_events << ") expired" << dendl;
    ...
    int op_prio =
        CEPH_MSG_PRIO_LOW + (CEPH_MSG_PRIO_HIGH - CEPH_MSG_PRIO_LOW) * expiring_segments.size() / max_segments;
    if (op_prio > CEPH_MSG_PRIO_HIGH) op_prio = CEPH_MSG_PRIO_HIGH;
    ...
    while (p != segments.end()) {
        ...
        if (new_expiring_segments * 2 > num_remaining_segments) break;

        if (max_expiring_segments > 0 && expiring_segments.size() >= max_expiring_segments) break;
        ...
        if (expiring_segments.count(ls)) {
            dout(5) << "trim already expiring segment " << ls->seq << "/" << ls->offset << ", " << ls->num_events
                    << " events" << dendl;
        }
        else if (expired_segments.count(ls)) {
            dout(5) << "trim already expired segment " << ls->seq << "/" << ls->offset << ", " << ls->num_events
                    << " events" << dendl;
        }
        else {
...
public:
    ...
    {
        if (res < 0) mdlog->mds->handle_write_error(res);
...
int MDLog::trim_all()
{
    ...
    dout(10) << __func__ << ": " << segments.size() << "/" << expiring_segments.size() << "/" << expired_segments.size()
             << dendl;
...
void MDLog::_maybe_expired(LogSegment* ls, int op_prio)
{
    ...
    dout(10) << "_maybe_expired segment " << ls->seq << "/" << ls->offset << ", " << ls->num_events << " events"
             << dendl;
    try_expire(ls, op_prio);
}
...
void MDLog::_trim_expired_segments()
{
    ...
    while (!segments.empty()) {
        ...
        if (!mds_is_shutting_down && ls->seq >= oft_committed_seq) {
            dout(10) << "_trim_expired_segments open file table committedseq " << oft_committed_seq << " <= " << ls->seq
                     << "/" << ls->offset << dendl;
            break;
        }

        dout(10) << "_trim_expired_segments trimming expired " << ls->seq << "/0x" << std::hex << ls->offset << std::dec
                 << dendl;
    ...
    if (trimmed) journaler->write_head(0);
}
...
void MDLog::_expired(LogSegment* ls)
{
    ...
    if (!mds_is_shutting_down && ls == peek_current_segment()) {
        dout(5) << "_expired not expiring " << ls->seq << "/" << ls->offset << ", last one and !mds_is_shutting_down"
                << dendl;
    }
    else {
        // expired.
...
void MDLog::replay(MDSContext* c)
{
    ...
    if (journaler->get_read_pos() == journaler->get_write_pos()) {
        ...
        mds->mdcache->trim();
        if (mds->is_standby_replay()) mds->update_mlogger();
        if (c) {
            c->complete(0);
        }
        return;
    ...
    // add waiter
    if (c) waitfor_replay.push_back(c);
...
void MDLog::_recovery_thread(MDSContext* completion)
{
    ...
    if (g_conf()->mds_journal_format > JOURNAL_FORMAT_MAX) {
        dout(0) << "Configuration value for mds_journal_format is out of bounds, max is " << JOURNAL_FORMAT_MAX
                << dendl;
    ...
    if (read_result == -CEPHFS_ENOENT) {
        ...
        mds->clog->error() << "failed to read JournalPointer: " << read_result << " (" << cpp_strerror(read_result)
                           << ")";
        mds->damaged_unlocked();
    ...
    if (jp.back) {
        ...
        if (recovery_result == -CEPHFS_EBLOCKLISTED) {
            ...
            mds->clog->error() << "Error recovering journal " << jp.front << ": " << cpp_strerror(recovery_result);
            mds->damaged_unlocked();
    ...
    Journaler* front_journal = new Journaler("mdlog",
                                             jp.front,
                                             mds->get_metadata_pool(),
                                             CEPH_FS_ONDISK_MAGIC,
                                             mds->objecter,
    ...
    if (front_journal->get_stream_format() > JOURNAL_FORMAT_MAX) {
        ...
        dout(1) << "Journal " << jp.front << " has old format " << front_journal->get_stream_format()
                << ", it will now be updated" << dendl;
...
void MDLog::_reformat_journal(JournalPointer const& jp_in, Journaler* old_journal, MDSContext* completion)
{
    ...
    Journaler* new_journal = new Journaler("mdlog",
                                           jp.back,
                                           mds->get_metadata_pool(),
                                           CEPH_FS_ONDISK_MAGIC,
                                           mds->objecter,
    ...
    while (1) {
        ...
        if (!old_journal->is_readable() && old_journal->get_read_pos() == old_journal->get_write_pos()) break;
        ...
        if (!r && old_journal->get_error()) continue;
        ceph_assert(r);
        ...
        if (le) {
            ...
            if (le->get_type() == EVENT_SUBTREEMAP || le->get_type() == EVENT_RESETJOURNAL) {
                ...
                if (sle == NULL || sle->event_seq == 0) {
                    ...
                    dout(20) << __func__ << " discovered segment seq mapping " << le_pos << " -> "
                             << new_journal->get_write_pos() << dendl;
                }
            }
            else {
                event_seq++;
            ...
            if (le->get_type() == EVENT_SUBTREEMAP || le->get_type() == EVENT_SUBTREEMAP_TEST) {
                auto& sle = dynamic_cast<ESubtreeMap&>(*le);
                dout(20) << __func__ << " zeroing expire_pos in subtreemap event at " << le_pos
                         << " seq=" << sle.event_seq << dendl;
                sle.expire_pos = 0;
                modified = true;
            }
            ...
            dout(1) << __func__ << " transcribing un-decodable LogEvent at old position " << old_journal->get_read_pos()
                    << ", new position " << new_journal->get_write_pos() << dendl;
        }
...
void MDLog::_replay_thread()
{
    ...
    while (1) {
        ...
        if (!journaler->is_readable() && journaler->get_read_pos() == journaler->get_write_pos()) break;
        ...
        if (!r && journaler->get_error()) continue;
        ceph_assert(r);
        ...
        if (!le) {
            ...
            *_dout << dendl;
            ...
            mds->clog->error() << "corrupt journal event at " << pos << "~" << bl.length() << " / "
