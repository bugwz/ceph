...
public:
    explicit MDSTableIOContext(MDSTable* ida_)
        : ida(ida_)
    {
...
public:
    C_IO_MT_Save(MDSTable* i, version_t v)
        : MDSTableIOContext(i)
        , version(v)
    {}
    void finish(int r) override { ida->save_2(r, version); }
    void print(ostream& out) const override { out << "table_save(" << ida->table_name << ")"; }
};
...
void MDSTable::save(MDSContext* onfinish, version_t v)
{
    if (v > 0 && v <= committing_version) {
        ...
        if (onfinish) waitfor_save[v].push_back(onfinish);
        return;
    }
    ...
    if (onfinish) waitfor_save[version].push_back(onfinish);
    ...
    mds->objecter->write_full(oid,
                              ...
                              ceph::real_clock::now(),
                              0,
                              new C_OnFinisher(new C_IO_MT_Save(this, version), mds->finisher));
}
...
public:
    Context* onfinish;
    bufferlist bl;
    C_IO_MT_Load(MDSTable* i, Context* o)
        : MDSTableIOContext(i)
        , onfinish(o)
    {}
    void finish(int r) override { ida->load_2(r, bl, onfinish); }
    void print(ostream& out) const override { out << "table_load(" << ida->table_name << ")"; }
};
...
void MDSTable::load_2(int r, bufferlist& bl, Context* onfinish)
{
    ...
    if (r < 0) {
        ...
        mds->clog->error() << "error reading table object '" << get_object_name() << "' " << r << " ("
                           << cpp_strerror(r) << ")";
        mds->damaged();
    ...
    try {
        ...
        mds->clog->error() << "error decoding table object '" << get_object_name() << "': " << e.what();
        mds->damaged();
        ceph_assert(r >= 0);   // Should be unreachable because damaged() calls respawn()
    }
