...
public:
    ...
    bool is_fragment_pending(dirfrag_t df) { return split_pending.count(df) || merge_pending.count(df); }
...
private:
    ...
    void find_exports(CDir* dir, double amount, std::vector<CDir*>* exports, double& have,
                      std::set<CDir*>& already_exporting);

    double try_match(balance_state_t& state, mds_rank_t ex, double& maxex, mds_rank_t im, double& maxim);
