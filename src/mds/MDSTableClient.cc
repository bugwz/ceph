...
public:
    C_LoggedAck(MDSTableClient* a, version_t t)
        : tc(a)
        , tid(t)
    {}
    void finish(int r) override { tc->_logged_ack(tid); }
};
...
void MDSTableClient::handle_request(const cref_t<MMDSTableRequest>& m)
{
    ...
    switch (m->op) {
    case TABLESERVER_OP_QUERY_REPLY: handle_query_result(m); break;
    ...
    case TABLESERVER_OP_AGREE:
        if (pending_prepare.count(reqid)) {
            ...
            *pending_prepare[reqid].ptid = tid;
            if (pending_prepare[reqid].pbl) *pending_prepare[reqid].pbl = m->bl;
            ...
            dout(10) << "stray agree on " << reqid << " tid " << tid << ", already committing, will resend COMMIT"
                     << dendl;
            ceph_assert(!server_ready);
    ...
    case TABLESERVER_OP_ACK:
        if (pending_commit.count(tid) && pending_commit[tid]->pending_commit_tids[table].count(tid)) {
    ...
    case TABLESERVER_OP_SERVER_READY:
        ...
        if (last_reqid == ~0ULL) last_reqid = reqid;
    ...
    default: ceph_abort_msg("unrecognized mds_table_client request op");
    }
}
...
void MDSTableClient::_prepare(bufferlist& mutation, version_t* ptid, bufferlist* pbl, MDSContext* onfinish)
{
    if (last_reqid == ~0ULL) {
...
{
    for (map<version_t, LogSegment*>::iterator p = pending_commit.begin(); p != pending_commit.end(); ++p) {
...
void MDSTableClient::resend_prepares()
{
    ...
    for (map<uint64_t, _pending_prepare>::iterator p = pending_prepare.begin(); p != pending_prepare.end(); ++p) {
...
{
    if (who != mds->get_mds_map()->get_tableserver()) return;   // do nothing.
