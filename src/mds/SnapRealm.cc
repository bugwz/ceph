...
static std::ostream& _prefix(std::ostream* _dout, int whoami, const CInode* inode, uint64_t seq, const SnapRealm* realm)
{
    return *_dout << " mds." << whoami << ".cache.snaprealm(" << inode->ino() << " seq " << seq << " " << realm << ") ";
}

ostream& operator<<(ostream& out, const SnapRealm& realm)
{
    out << "snaprealm(" << realm.inode->ino() << " seq " << realm.srnode.seq << " lc " << realm.srnode.last_created
        << " cr " << realm.srnode.created;
    if (realm.srnode.created != realm.srnode.current_parent_since) out << " cps " << realm.srnode.current_parent_since;
    out << " snaps=" << realm.srnode.snaps;
    if (realm.srnode.past_parent_snaps.size() > 0) {
        out << " past_parent_snaps=" << realm.srnode.past_parent_snaps;
    }

    if (realm.srnode.is_parent_global()) out << " global ";
    out << " last_modified " << realm.srnode.last_modified << " change_attr " << realm.srnode.change_attr;
    out << " " << &realm << ")";
    return out;
}
...
SnapRealm::SnapRealm(MDCache* c, CInode* in)
    : mdcache(c)
    , inode(in)
    , inodes_with_caps(member_offset(CInode, item_caps))
{
...
void SnapRealm::build_snap_set() const
{
    ...
    // include my snaps
    for (const auto& p : srnode.snaps) cached_snaps.insert(p.first);
...
void SnapRealm::check_cache() const
{
    ...
    if (cached_seq >= seq && cached_last_destroyed == last_destroyed) return;
    ...
    if (parent) cached_subvolume_ino = parent->get_subvolume_ino();
    if (!cached_subvolume_ino && srnode.is_subvolume()) cached_subvolume_ino = inode->ino();

    build_snap_set();

    build_snap_trace();

    dout(10) << "check_cache rebuilt " << cached_snaps << " seq " << seq << " cached_seq " << cached_seq
             << " cached_last_created " << cached_last_created << " cached_last_destroyed " << cached_last_destroyed
             << ")" << dendl;
}
...
const SnapContext& SnapRealm::get_snap_context() const
{
    ...
    if (!cached_snap_context.seq) {
        ...
        unsigned i = 0;
        for (set<snapid_t>::reverse_iterator p = cached_snaps.rbegin(); p != cached_snaps.rend(); ++p)
...
snapid_t SnapRealm::resolve_snapname(std::string_view n, inodeno_t atino, snapid_t first, snapid_t last)
{
    ...
    for (auto p = srnode.snaps.lower_bound(first);   // first element >= first
        ...
        // return p->first;
        if (actual && p->second.name == n) return p->first;
        if (!actual && p->second.name == pname && p->second.ino == pino) return p->first;
    }
    ...
    if (!srnode.past_parent_snaps.empty()) {
        ...
        for (auto& it : _infomap) {
            ...
            if (actual && it.second->name == n) return it.first;
            if (!actual && it.second->name == pname && it.second->ino == pino) return it.first;
        }
    }
...
void SnapRealm::adjust_parent()
{
    ...
    if (newparent != parent) {
        ...
        if (parent) parent->open_children.erase(this);
        parent = newparent;
        if (parent) parent->open_children.insert(this);
...
void SnapRealm::split_at(SnapRealm* child)
{
    ...
    for (set<SnapRealm*>::iterator p = open_children.begin(); p != open_children.end();) {
        SnapRealm* realm = *p;
        if (realm != child && child->inode->is_ancestor_of(realm->inode)) {
...
void SnapRealm::build_snap_trace() const
{
    ...
    if (global) {
        ...
        for (auto p = cached_snaps.rbegin(); p != cached_snaps.rend(); ++p) info.my_snaps.push_back(*p);
    ...
    for (auto p = srnode.snaps.rbegin(); p != srnode.snaps.rend(); ++p) info.my_snaps.push_back(p->first);
    dout(10) << "build_snap_trace my_snaps " << info.my_snaps << dendl;
...
void SnapRealm::prune_past_parent_snaps()
{
    ...
    for (auto p = srnode.past_parent_snaps.begin(); p != srnode.past_parent_snaps.end();) {
