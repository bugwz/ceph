...
struct LockType
{
        ...
        : type(t)
    {
        ...
        case CEPH_LOCK_IDFT:
        case CEPH_LOCK_INEST: sm = &sm_scatterlock; break;
        case CEPH_LOCK_IFILE: sm = &sm_filelock; break;
        case CEPH_LOCK_DVERSION:
        case CEPH_LOCK_IVERSION: sm = &sm_locallock; break;
        default: sm = 0;
        }
    }
...
public:
    ...
    client_t get_excl_client() const { return have_more() ? more()->excl_client : -1; }
    void set_excl_client(client_t c)
    {
        if (c < 0 && !have_more()) return;   // default is -1
        more()->excl_client = c;
    }

    virtual bool is_scatterlock() const { return false; }
    virtual bool is_locallock() const { return false; }
    ...
    void decode_locked_state(const ceph::buffer::list& bl) { parent->decode_lock_state(type->type, bl); }
    void encode_locked_state(ceph::buffer::list& bl) { parent->encode_lock_state(type->type, bl); }
    void finish_waiters(uint64_t mask, int r = 0) { parent->finish_waiting(mask << get_wait_shift(), r); }
    void take_waiting(uint64_t mask, MDSContext::vec& ls) { parent->take_waiting(mask << get_wait_shift(), ls); }
    void add_waiter(uint64_t mask, MDSContext* c)
    {
        parent->add_waiter((mask << get_wait_shift()) | MDSCacheObject::WAIT_ORDERED, c);
    }
    bool is_waiter_for(uint64_t mask) const { return parent->is_waiter_for(mask << get_wait_shift()); }

    bool is_cached() const { return state_flags & CACHED; }
    ...
    void set_state_rejoin(int s, MDSContext::vec& waiters, bool survivor)
    {
        ...
        // unsafe requests.
        if (!survivor || state != LOCK_SYNC) mark_need_recover();

        state = s;

        if (is_stable()) take_waiting(SimpleLock::WAIT_ALL, waiters);
    }

    bool is_stable() const { return get_sm()->states[state].next == 0; }
    bool is_unstable_and_locked() const { return (!is_stable() && is_locked()); }
    bool is_locked() const { return is_rdlocked() || is_wrlocked() || is_xlocked(); }
    int get_next_state() { return get_sm()->states[state].next; }

    bool is_sync_and_unlocked() const
    {
        return get_state() == LOCK_SYNC && !is_rdlocked() && !is_leased() && !is_wrlocked() && !is_xlocked();
    }
    ...
    */
    bool req_rdlock_from_auth() { return get_sm()->states[state].can_rdlock == REQ; }

    // gather set
    static std::set<int32_t> empty_gather_set;

    // int32_t: <0 is client, >=0 is MDS rank
    const std::set<int32_t>& get_gather_set() const { return have_more() ? more()->gather_set : empty_gather_set; }
    ...
    bool is_gathering() const { return have_more() && !more()->gather_set.empty(); }
    bool is_gathering(int32_t i) const { return have_more() && more()->gather_set.count(i); }
    void clear_gather()
    {
        if (have_more()) more()->gather_set.clear();
    }
    void remove_gather(int32_t i)
    {
        if (have_more()) more()->gather_set.erase(i);
    }
    ...
    bool can_wrlock(client_t client) const
    {
        return get_sm()->states[state].can_wrlock == ANY ||
               ...
               (get_sm()->states[state].can_wrlock == XCL && client >= 0 &&
                (get_xlock_by_client() == client || get_excl_client() == client));
    }
    bool can_force_wrlock(client_t client) const
    {
        return get_sm()->states[state].can_force_wrlock == ANY ||
               ...
               (get_sm()->states[state].can_force_wrlock == XCL && client >= 0 &&
                (get_xlock_by_client() == client || get_excl_client() == client));
    }
    ...
    int get_num_rdlocks() const { return num_rdlock; }

    // wrlock
    void get_wrlock(bool force = false)
    {
        // assert(can_wrlock() || force);
        if (more()->num_wrlock == 0) parent->get(MDSCacheObject::PIN_LOCK);
        ++more()->num_wrlock;
    }
    void put_wrlock()
    {
    ...
    int get_num_wrlocks() const { return have_more() ? more()->num_wrlock : 0; }

    // xlock
    void get_xlock(MutationRef who, client_t client)
    {
        ceph_assert(get_xlock_by() == MutationRef());
        ceph_assert(state == LOCK_XLOCK || is_locallock() || state == LOCK_LOCK /* if we are a peer */);
    ...
    void set_xlock_done()
    {
        ...
        ceph_assert(state == LOCK_XLOCK || is_locallock() || state == LOCK_LOCK /* if we are a peer */);
        if (!is_locallock()) state = LOCK_XLOCKDONE;
        more()->xlock_by.reset();
    }
    void put_xlock()
    {
        ceph_assert(state == LOCK_XLOCK || state == LOCK_XLOCKDONE || state == LOCK_XLOCKSNAP ||
                    state == LOCK_LOCK_XLOCK || state == LOCK_LOCK || /* if we are a leader of a peer */
                    state == LOCK_PREXLOCK || state == LOCK_SYNC || is_locallock());
        --more()->num_xlock;
        parent->put(MDSCacheObject::PIN_LOCK);
        if (more()->num_xlock == 0) {
            more()->xlock_by.reset();
    ...
    int get_num_xlocks() const { return have_more() ? more()->num_xlock : 0; }
    client_t get_xlock_by_client() const { return have_more() ? more()->xlock_by_client : -1; }
    bool is_xlocked_by_client(client_t c) const { return have_more() ? more()->xlock_by_client == c : false; }
    MutationRef get_xlock_by() const { return have_more() ? more()->xlock_by : MutationRef(); }

    // lease
    bool is_leased() const { return state_flags & LEASED; }
    ...
    bool needs_recover() const { return state_flags & NEED_RECOVER; }
    void mark_need_recover() { state_flags |= NEED_RECOVER; }
    void clear_need_recover() { state_flags &= ~NEED_RECOVER; }
    ...
    void decode(ceph::buffer::list::const_iterator& p)
    {
        ...
        std::set<__s32> g;
        decode(g, p);
        if (!g.empty()) more()->gather_set.swap(g);
        DECODE_FINISH(p);
    }
    ...
    // caps
    bool is_loner_mode() const { return get_sm()->states[state].loner; }
    ...
    {
        if (get_num_wrlocks()) return get_sm()->careful;
        return 0;
    }
    ...
    int get_replica_state() const { return get_sm()->states[state].replica_state; }
    void export_twiddle()
    {
        clear_gather();
    ...
    bool do_import(int from, int to)
    {
        ...
        if (!is_stable() && !is_gathering()) return true;
        return false;
    }
    ...
    void _print(std::ostream& out) const
    {
        ...
        if (is_leased()) out << " l";
        if (is_rdlocked()) out << " r=" << get_num_rdlocks();
        if (is_wrlocked()) out << " w=" << get_num_wrlocks();
        if (is_xlocked()) {
            out << " x=" << get_num_xlocks();
            if (get_xlock_by()) out << " by " << get_xlock_by();
        }
        /*if (is_stable())
          out << " stable";
        else
...
private:
    ...
    struct unstable_bits_t
    {
        ...
        bool empty()
        {
            return gather_set.empty() && num_wrlock == 0 && num_xlock == 0 && xlock_by.get() == NULL &&
                   xlock_by_client == -1 && excl_client == -1 && lock_caches.empty();
        }
    ...
    {
        if (!_unstable) _unstable.reset(new unstable_bits_t);
