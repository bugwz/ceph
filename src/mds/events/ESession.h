...
public:
    ESession()
        ...
        , open(false)
    {}
    ESession(const entity_inst_t& inst, bool o, version_t v, const client_metadata_t& cm)
        : LogEvent(EVENT_SESSION)
        , client_inst(inst)
        , open(o)
        , cmapv(v)
        ...
        , client_metadata(cm)
    {}
    ESession(const entity_inst_t& inst, bool o, version_t v, const interval_set<inodeno_t>& to_free, version_t iv,
    ...
    void print(std::ostream& out) const override
    {
        ...
        if (inos_to_free.size() || inos_to_purge.size())
            out << " (" << inos_to_free.size() << " to free, v" << inotablev << ", " << inos_to_purge.size()
                << " to purge)";
    }
