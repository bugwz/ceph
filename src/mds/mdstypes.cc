...
{
    if (f == frag_info_t()) return out << "f()";
    out << "f(v" << f.version;
    if (f.mtime != utime_t()) out << " m" << f.mtime;
    if (f.nfiles || f.nsubdirs) out << " " << f.size() << "=" << f.nfiles << "+" << f.nsubdirs;
    out << ")";
    return out;
}
...
{
    if (n == nest_info_t()) return out << "n()";
    out << "n(v" << n.version;
    if (n.rctime != utime_t()) out << " rc" << n.rctime;
    if (n.rbytes) out << " b" << n.rbytes;
    if (n.rsnaps) out << " rs" << n.rsnaps;
    if (n.rfiles || n.rsubdirs) out << " " << n.rsize() << "=" << n.rfiles << "+" << n.rsubdirs;
    out << ")";
    return out;
}
...
{
    out << "quota("
        << "max_bytes = " << n.max_bytes << " max_files = " << n.max_files << ")";
    return out;
}
...
void feature_bitset_t::decode(bufferlist::const_iterator& p)
{
    ...
    _vec.clear();
    if (len >= sizeof(block_type)) decode_nohead(len / sizeof(block_type), _vec, p);
...
void feature_bitset_t::print(ostream& out) const
{
    ...
    if (!size) {
        ...
        for (int i = size - 1; i >= 0; --i)
            out << std::setfill('0') << std::setw(sizeof(block_type) * 2) << std::hex << _vec[i];
    }
    out.flags(f);
}
...
void client_metadata_t::decode(bufferlist::const_iterator& p)
{
    DECODE_START(3, p);
    decode(kv_map, p);
    if (struct_v >= 2) decode(features, p);
    if (struct_v >= 3) {
...
void client_metadata_t::dump(Formatter* f) const
{
    ...
    for (const auto& [name, val] : kv_map) f->dump_string(name.c_str(), val);
}
...
void cap_reconnect_t::decode(bufferlist::const_iterator& bl)
{
    ...
    if (struct_v >= 2) decode(snap_follows, bl);
    DECODE_FINISH(bl);
}
