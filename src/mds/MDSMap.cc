...
void MDSMap::mds_info_t::dump(Formatter* f) const
{
    ...
    if (laggy_since != utime_t()) f->dump_stream("laggy_since") << laggy_since;

    f->open_array_section("export_targets");
    for (set<mds_rank_t>::iterator p = export_targets.begin(); p != export_targets.end(); ++p) {
        f->dump_int("mds", *p);
    }
    f->close_section();
...
void MDSMap::mds_info_t::dump(std::ostream& o) const
{
    o << "[mds." << name << "{" << rank << ":" << global_id << "}"
      << " state " << ceph_mds_state_name(state) << " seq " << state_seq;
    if (laggy()) {
...
void MDSMap::dump(Formatter* f) const
{
    ...
    for (set<mds_rank_t>::const_iterator p = in.begin(); p != in.end(); ++p) f->dump_int("mds", *p);
    f->close_section();
    ...
    for (set<mds_rank_t>::const_iterator p = failed.begin(); p != failed.end(); ++p) f->dump_int("mds", *p);
    f->close_section();
    f->open_array_section("damaged");
    for (set<mds_rank_t>::const_iterator p = damaged.begin(); p != damaged.end(); ++p) f->dump_int("mds", *p);
    f->close_section();
    f->open_array_section("stopped");
    for (set<mds_rank_t>::const_iterator p = stopped.begin(); p != stopped.end(); ++p) f->dump_int("mds", *p);
    f->close_section();
    ...
    for (const auto& p : data_pools) f->dump_int("pool", p);
    f->close_section();
...
void MDSMap::print_summary(Formatter* f, ostream* out) const
{
    ...
    if (f) f->open_array_section("by_rank");
    for (const auto& p : mds_info) {
        string s = ceph_mds_state_name(p.second.state);
        if (p.second.laggy()) s += "(laggy or crashed)";
...
{
    if (joinable()) out << " " << flag_display.at(CEPH_MDSMAP_NOT_JOINABLE);
    if (allows_snaps()) out << " " << flag_display.at(CEPH_MDSMAP_ALLOW_SNAPS);
    if (allows_multimds_snaps()) out << " " << flag_display.at(CEPH_MDSMAP_ALLOW_MULTIMDS_SNAPS);
    if (allows_standby_replay()) out << " " << flag_display.at(CEPH_MDSMAP_ALLOW_STANDBY_REPLAY);
    if (test_flag(CEPH_MDSMAP_REFUSE_CLIENT_SESSION)) out << " " << flag_display.at(CEPH_MDSMAP_REFUSE_CLIENT_SESSION);
}
                        ...
                        list<pair<health_status_t, string> >* detail) const
{
    if (!failed.empty()) {
        ...
        *css << "mds rank" << ((failed.size() > 1) ? "s " : " ") << failed << ((failed.size() > 1) ? " have" : " has")
             << " failed";
    ...
    if (!damaged.empty()) {
        CachedStackStringStream css;
        *css << "mds rank" << ((damaged.size() > 1) ? "s " : " ") << damaged << ((damaged.size() > 1) ? " are" : " is")
             << " damaged";
        summary.push_back(make_pair(HEALTH_ERR, css->str()));
        if (detail) {
    ...
    if (is_degraded()) {
        ...
        if (detail) {
            ...
            for (mds_rank_t i = mds_rank_t(0); i < get_max_mds(); i++) {
                ...
                CachedStackStringStream css;
                if (is_resolve(i))
                    *css << "mds." << info.name << " at " << info.addrs << " rank " << i << " is resolving";
                if (is_replay(i))
                    *css << "mds." << info.name << " at " << info.addrs << " rank " << i << " is replaying journal";
                if (is_rejoin(i))
                    *css << "mds." << info.name << " at " << info.addrs << " rank " << i << " is rejoining";
                if (is_reconnect(i))
                    *css << "mds." << info.name << " at " << info.addrs << " rank " << i
                         << " is reconnecting to clients";
                if (css->strv().length()) detail->push_back(make_pair(HEALTH_WARN, css->str()));
            }
        }
    }
    ...
    if ((mds_rank_t)up.size() < max_mds) {
        CachedStackStringStream css;
        *css << fs_name << " has " << up.size() << " active MDS(s), but has max_mds of " << max_mds;
        summary.push_back(make_pair(HEALTH_WARN, css->str()));
    }
    ...
    for (const auto& u : up) {
        ...
        if (info.laggy()) {
            ...
            if (detail) {
                CachedStackStringStream css;
                *css << "mds." << info.name << " at " << info.addrs << " is laggy/unresponsive";
    ...
    if (!laggy.empty()) {
        CachedStackStringStream css;
        *css << "mds " << laggy << ((laggy.size() > 1) ? " are" : " is") << " laggy";
        summary.push_back(make_pair(HEALTH_WARN, css->str()));
    }

    if (get_max_mds() > 1 && was_snaps_ever_allowed() && !allows_multimds_snaps()) {
        CachedStackStringStream css;
...
void MDSMap::get_health_checks(health_check_map_t* checks) const
{
    ...
    if (!damaged.empty()) {
        health_check_t& check =
            checks->get_or_add("MDS_DAMAGE", HEALTH_ERR, "%num% mds daemon%plurals% damaged", damaged.size());
    ...
    // FS_DEGRADED
    if (is_degraded()) {
        health_check_t& fscheck =
            checks->get_or_add("FS_DEGRADED", HEALTH_WARN, "%num% filesystem%plurals% %isorare% degraded", 1);
        CachedStackStringStream css;
        *css << "fs " << fs_name << " is degraded";
        ...
        for (mds_rank_t i = mds_rank_t(0); i < get_max_mds(); i++) {
            ...
            const auto& info = mds_info.at(gid);
            CachedStackStringStream css;
            *css << "fs " << fs_name << " mds." << info.name << " at " << info.addrs << " rank " << i;
            if (is_resolve(i)) *css << " is resolving";
            if (is_replay(i)) *css << " is replaying journal";
            if (is_rejoin(i)) *css << " is rejoining";
            if (is_reconnect(i)) *css << " is reconnecting to clients";
            if (css->strv().length()) detail.push_back(css->str());
        }
    }

    // MDS_UP_LESS_THAN_MAX
    if ((mds_rank_t)get_num_in_mds() < get_max_mds()) {
        health_check_t& check = checks->add("MDS_UP_LESS_THAN_MAX",
                                            HEALTH_WARN,
                                            "%num% filesystem%plurals% %isorare% online with fewer MDS than max_mds",
                                            1);
        CachedStackStringStream css;
        *css << "fs " << fs_name << " has " << get_num_in_mds() << " MDS online, but wants " << get_max_mds();
        check.detail.push_back(css->str());
    }
    ...
    if ((mds_rank_t)get_num_up_mds() == 0 && get_max_mds() > 0) {
        health_check_t& check =
            checks->add("MDS_ALL_DOWN", HEALTH_ERR, "%num% filesystem%plurals% %isorare% offline", 1);
        CachedStackStringStream css;
    ...
    if (get_max_mds() > 1 && was_snaps_ever_allowed() && !allows_multimds_snaps()) {
        health_check_t& check = checks->add("MULTIMDS_WITH_OLDSNAPS",
                                            HEALTH_ERR,
    ...
    if (get_inline_data_enabled()) {
        health_check_t& check = checks->add("FS_INLINE_DATA_DEPRECATED",
                                            HEALTH_WARN,
                                            "%num% filesystem%plurals% with deprecated feature inline_data",
                                            1);
        CachedStackStringStream css;
...
void MDSMap::mds_info_t::decode(bufferlist::const_iterator& bl)
{
    ...
    if (struct_v >= 2) decode(export_targets, bl);
    if (struct_v >= 5) decode(mds_features, bl);
    if (struct_v >= 6) {
...
void MDSMap::encode(bufferlist& bl, uint64_t features) const
{
    ...
    if ((features & CEPH_FEATURE_PGID64) == 0) {
        ...
        encode(n, bl);
        for (map<mds_gid_t, mds_info_t>::const_iterator i = mds_info.begin(); i != mds_info.end(); ++i) {
            encode(i->first, bl);
...
void MDSMap::decode(bufferlist::const_iterator& p)
{
    ...
    decode(failed, p);
    decode(stopped, p);
    if (ev >= 4) decode(last_failure_osd_epoch, p);
    if (ev >= 6) {
        if (ev < 10) {
    ...
    if (ev >= 7) decode(inline_data_enabled, p);
...
bool MDSMap::state_transition_valid(DaemonState prev, DaemonState next)
{
    ...
    if (next == MDSMap::STATE_DAMAGED) return true;

    if (prev == MDSMap::STATE_BOOT) {
        return next == MDSMap::STATE_STANDBY;
    }
    else if (prev == MDSMap::STATE_STANDBY) {
        return next == MDSMap::STATE_STANDBY_REPLAY || next == MDSMap::STATE_REPLAY || next == MDSMap::STATE_CREATING ||
        ...
        return next == MDSMap::STATE_RESOLVE || next == MDSMap::STATE_RECONNECT;
    }
        ...
        // Except...
        if (prev == MDSMap::STATE_REJOIN && (next == MDSMap::STATE_ACTIVE ||     // No need to do client replay
        ...
        derr << __func__ << ": Unknown prev state " << ceph_mds_state_name(prev) << "(" << prev << ")" << dendl;
        return false;
    }
}
...
{
    unsigned n = 0;
    for (std::map<mds_gid_t, mds_info_t>::const_iterator p = mds_info.begin(); p != mds_info.end(); ++p)
...
{
    for (std::map<mds_rank_t, mds_gid_t>::const_iterator p = up.begin(); p != up.end(); ++p) s.insert(p->first);
}
...
uint64_t MDSMap::get_up_features()
{
    if (!cached_up_features) {
        bool first = true;
        for (std::map<mds_rank_t, mds_gid_t>::const_iterator p = up.begin(); p != up.end(); ++p) {
...
{
    s = failed;
    for (const auto& p : damaged) s.insert(p);
    for (const auto& p : mds_info)
        if (p.second.state >= STATE_REPLAY && p.second.state <= STATE_STOPPING) s.insert(p.second.rank);
}

void MDSMap::get_mds_set_lower_bound(std::set<mds_rank_t>& s, DaemonState first) const
{
    for (std::map<mds_gid_t, mds_info_t>::const_iterator p = mds_info.begin(); p != mds_info.end(); ++p)
        if (p->second.state >= first && p->second.state <= STATE_STOPPING) s.insert(p->second.rank);
}

void MDSMap::get_mds_set(std::set<mds_rank_t>& s, DaemonState state) const
{
    for (std::map<mds_gid_t, mds_info_t>::const_iterator p = mds_info.begin(); p != mds_info.end(); ++p)
        if (p->second.state == state) s.insert(p->second.rank);
}
...
{
    if (!failed.empty() || !damaged.empty()) return true;
    for (const auto& p : mds_info) {
        if (p.second.is_degraded()) return true;
    }
    return false;
}
