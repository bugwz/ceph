...
struct BatchStoredBacktrace : public MDSIOContext
{
    ...
    BatchStoredBacktrace(MDSRank* m, MDSContext* f, std::vector<CInodeCommitOperations>&& ops)
        : MDSIOContext(m)
        , fin(f)
    ...
    void print(ostream& out) const override { out << "batch backtrace_store"; }
};
...
struct BatchCommitBacktrace : public Context
{
    ...
    BatchCommitBacktrace(MDSRank* m, MDSContext* f, std::vector<CInodeCommitOperations>&& ops)
        : mds(m)
        , fin(f)
        , ops_vec(std::move(ops))
    {}
    void finish(int r) override
    {
        ...
        gather.set_finisher(new C_OnFinisher(new BatchStoredBacktrace(mds, fin, std::move(ops_vec)), mds->finisher));
        gather.activate();
    }
};
...
void LogSegment::try_to_expire(MDSRank* mds, MDSGatherBuilder& gather_bld, int op_prio)
{
    ...
    if (!commit.empty()) {
        for (set<CDir*>::iterator p = commit.begin(); p != commit.end(); ++p) {
            CDir* dir = *p;
    ...
    for (set<metareqid_t>::iterator p = uncommitted_leaders.begin(); p != uncommitted_leaders.end(); ++p) {
    ...
    for (set<metareqid_t>::iterator p = uncommitted_peers.begin(); p != uncommitted_peers.end(); ++p) {
    ...
    for (set<dirfrag_t>::iterator p = uncommitted_fragments.begin(); p != uncommitted_fragments.end(); ++p) {
    ...
    size_t count = 0;
    for (elist<CInode*>::iterator it = dirty_parent_inodes.begin(); !it.end(); ++it) count++;
    ...
    if (!ops_vec.empty()) mds->finisher->queue(new BatchCommitBacktrace(mds, gather_bld.new_sub(), std::move(ops_vec)));

    ceph_assert(g_conf()->mds_kill_journal_expire_at != 4);

    // idalloc
    if (inotablev > mds->inotable->get_committed_version()) {
        dout(10) << "try_to_expire saving inotable table, need " << inotablev << ", committed is "
                 << mds->inotable->get_committed_version() << " (" << mds->inotable->get_committing_version() << ")"
                 << dendl;
    ...
    if (sessionmapv > mds->sessionmap.get_committed()) {
        dout(10) << "try_to_expire saving sessionmap, need " << sessionmapv << ", committed is "
                 << mds->sessionmap.get_committed() << " (" << mds->sessionmap.get_committing() << ")" << dendl;
    ...
    for (map<int, ceph::unordered_set<version_t> >::iterator p = pending_commit_tids.begin();
        ...
        ceph_assert(client);
        for (ceph::unordered_set<version_t>::iterator q = p->second.begin(); q != p->second.end(); ++q) {
    ...
    // table servers
    for (map<int, version_t>::iterator p = tablev.begin(); p != tablev.end(); ++p) {
        MDSTableServer* server = mds->get_table_server(p->first);
        ceph_assert(server);
        if (p->second > server->get_committed_version()) {
            dout(10) << "try_to_expire waiting for " << get_mdstable_name(p->first) << " to save, need " << p->second
                     << dendl;
    ...
    // truncating
    for (set<CInode*>::iterator p = truncating_inodes.begin(); p != truncating_inodes.end(); ++p) {
    ...
    if (purging_inodes.size()) set_purged_cb(gather_bld.new_sub());
        ...
        if (mode == TO_AUTH_SUBTREE_ROOT) {
            ...
            if (dir->is_subtree_root()) {
                ...
                if (dir->get_dir_auth().first == mds->get_nodeid()) {
                    ...
                    if (parent_auth.first == dir->get_dir_auth().first) {
                        if (parent_auth.second == CDIR_AUTH_UNKNOWN && !dir->is_ambiguous_dir_auth() &&
                            !dir->state_test(CDir::STATE_EXPORTBOUND) && !dir->state_test(CDir::STATE_AUXSUBTREE) &&
                            !diri->state_test(CInode::STATE_AMBIGUOUSAUTH)) {
                            dout(0) << "EMetaBlob::add_dir_context unexpected subtree " << *dir << dendl;
                            ceph_abort();
                        }
                        dout(20) << "EMetaBlob::add_dir_context(" << dir << ") ambiguous or transient subtree "
                                 << dendl;
                    }
                    else {
                        // it's an auth subtree, we don't need maybe (if any), and we're done.
                        dout(20) << "EMetaBlob::add_dir_context(" << dir
                                 << ") reached unambig auth subtree, don't need " << maybe << " at " << *dir << dendl;
                        maybe.clear();
                        break;
                    }
                }
                else {
                    dout(20) << "EMetaBlob::add_dir_context(" << dir << ") reached ambig or !auth subtree, need "
                             << maybe << " at " << *dir << dendl;
            ...
            if (!maybenot && last_subtree_map && diri->last_journaled >= last_subtree_map) {
                dout(20) << "EMetaBlob::add_dir_context(" << dir << ") already have diri in this segment ("
                         << diri->last_journaled << " >= " << last_subtree_map << "), setting maybenot flag " << *diri
                         << dendl;
                maybenot = true;
            }
        }
...
void EMetaBlob::update_segment(LogSegment* ls)
{
    ...
    // alloc table update?
    if (inotablev) ls->inotablev = inotablev;
    if (sessionmapv) ls->sessionmapv = sessionmapv;
...
void EMetaBlob::fullbit::encode(bufferlist& bl, uint64_t features) const
{
    ...
    if (!inode->is_dir()) encode(snapbl, bl);
...
void EMetaBlob::fullbit::decode(bufferlist::const_iterator& bl)
{
    ...
    }
    {
        ...
        if (!tmp.empty()) xattrs = CInode::allocate_xattr_map(std::move(tmp));
    }
    if (inode->is_symlink()) decode(symlink, bl);
    if (inode->is_dir()) {
...
void EMetaBlob::fullbit::generate_test_instances(std::list<EMetaBlob::fullbit*>& ls)
{
    ...
    fullbit* sample = new fullbit("/testdn", "", 0, 0, 0, _inode, fragtree, _xattrs, "", 0, empty_snapbl, false, NULL);
    ls.push_back(sample);
}
...
void EMetaBlob::fullbit::update_inode(MDSRank* mds, CInode* in)
{
    ...
    if (in->is_dir()) {
        ...
        if (!(in->dirfragtree == dirfragtree)) {
            dout(10) << "EMetaBlob::fullbit::update_inode dft " << in->dirfragtree << " -> " << dirfragtree << " on "
                     << *in << dendl;
            ...
            if (in->get_num_dirfrags() && in->authority() == CDIR_AUTH_UNDEF) {
                ...
                for (const auto& dir : ls) {
                    if (dir->get_num_any() == 0 && mds->mdcache->can_trim_non_auth_dirfrag(dir)) {
    ...
    if (in->is_file()) {
        ...
        if (in->get_inode()->layout.pool_id == -1 || !in->get_inode()->layout.is_valid()) {
...
void EMetaBlob::remotebit::decode(bufferlist::const_iterator& bl)
{
    ...
    decode(d_type, bl);
    decode(dirty, bl);
    if (struct_v >= 3) decode(alternate_name, bl);
    DECODE_FINISH(bl);
}
...
void EMetaBlob::remotebit::dump(Formatter* f) const
{
    ...
    string type_string;
    switch (type) {
    case S_IFREG: type_string = "file"; break;
    case S_IFLNK: type_string = "symlink"; break;
    case S_IFDIR: type_string = "directory"; break;
    case S_IFIFO: type_string = "fifo"; break;
    case S_IFCHR: type_string = "chr"; break;
    case S_IFBLK: type_string = "blk"; break;
    case S_IFSOCK: type_string = "sock"; break;
    default: assert(0 == "unknown d_type!");
    }
...
void EMetaBlob::remotebit::generate_test_instances(std::list<EMetaBlob::remotebit*>& ls)
{
 ...
 */
void EMetaBlob::get_inodes(std::set<inodeno_t>& inodes) const
{
 ...
 * subtree.
 */
void EMetaBlob::get_paths(std::vector<std::string>& paths) const
{
...
void EMetaBlob::dump(Formatter* f) const
{
    ...
    for (vector<inodeno_t>::const_iterator i = destroyed_inodes.begin(); i != destroyed_inodes.end(); ++i) {
...
void EMetaBlob::replay(MDSRank* mds, LogSegment* logseg, int type, MDPeerUpdate* peerup)
{
    ...
    for (auto& p : roots) {
        ...
        if (!in) in = new CInode(mds->mdcache, false, 2, CEPH_NOSNAP);
        p.update_inode(mds, in);

        if (isnew) mds->mdcache->add_inode(in);
    ...
    for (const auto& lp : lump_order) {
        ...
        if (!dir) {
            ...
            if (MDS_INO_IS_BASE(lp.ino)) mds->mdcache->adjust_subtree_auth(dir, CDIR_AUTH_UNDEF);
        ...
        if (lump.is_new()) dir->mark_new(logseg);
        if (lump.is_complete()) dir->mark_complete();
        ...
        for (auto& fb : lump._get_dfull()) {
            ...
            if (lump.is_importing()) dn->state_set(CDentry::STATE_AUTH);
            ...
            if (!in) {
                ...
                if (unlinked.count(in)) linked.insert(in);
                ...
                ceph_assert(in->first == fb.dnfirst || (in->is_multiversion() && in->first > fb.dnfirst));
            }
            if (fb.is_dirty()) in->_mark_dirty(logseg);
            if (fb.is_dirty_parent()) in->mark_dirty_parent(logseg, fb.is_dirty_pool());
            if (fb.need_snapflush()) logseg->open_files.push_back(&in->item_open_file);
            if (dn->is_auth())
        ...
        for (const auto& rb : lump.get_dremote()) {
            ...
            if (!dn) {
                dn = dir->add_remote_dentry(
                    rb.dn, rb.ino, rb.d_type, mempool::mds_co::string(rb.alternate_name), rb.dnfirst, rb.dnlast);
            ...
            if (!dn) {
                ...
                if (!dn->get_linkage()->is_null()) {
                    ...
                    if (!renamed_diri || renamed_diri != in) {
                        if (dn->get_linkage()->is_primary()) unlinked[in] = dir;
                        dir->unlink_inode(dn);
                    }
                }
                dn->set_version(nb.dnv);
    ...
    if (renamed_dirino) {
        ...
        if (olddir) {
            if (olddir->authority() != CDIR_AUTH_UNDEF && renamed_diri->authority() == CDIR_AUTH_UNDEF) {
        ...
        for (map<CInode*, CDir*>::iterator p = unlinked.begin(); p != unlinked.end(); ++p) {
            if (!linked.count(p->first)) continue;
    ...
    if (!unlinked.empty()) {
        for (set<CInode*>::iterator p = linked.begin(); p != linked.end(); ++p) unlinked.erase(*p);
        ...
        for (map<CInode*, CDir*>::iterator p = unlinked.begin(); p != unlinked.end(); ++p) {
            ...
            if (peerup) {   // preserve unlinked inodes until peer commit
                ...
                if (in->snaprealm) in->snaprealm->adjust_parent();
            }
            else
    ...
    for (const auto& p : table_tids) {
        ...
        MDSTableClient* client = mds->get_table_client(p.first);
        if (client) client->got_journaled_agree(p.second, logseg);
    ...
    // allocated_inos
    if (inotablev) {
        if (mds->inotable->get_version() >= inotablev || unlikely(type == EVENT_UPDATE && skip_replaying_inotable)) {
            dout(10) << "EMetaBlob.replay inotable tablev " << inotablev << " <= table " << mds->inotable->get_version()
                     << dendl;
            if (allocated_ino) mds->mdcache->insert_taken_inos(allocated_ino);
        }
        else {
            dout(10) << "EMetaBlob.replay inotable v " << inotablev << " - 1 == table " << mds->inotable->get_version()
                     << " allocated+used " << allocated_ino << " prealloc " << preallocated_inos << dendl;
            if (allocated_ino) mds->inotable->replay_alloc_id(allocated_ino);
            if (preallocated_inos.size()) mds->inotable->replay_alloc_ids(preallocated_inos);

            // repair inotable updates in case inotable wasn't persist in time
            if (inotablev > mds->inotable->get_version()) {
                mds->clog->error() << "journal replay inotablev mismatch " << mds->inotable->get_version() << " -> "
                                   << inotablev << ", will force replay it.";
    ...
    if (sessionmapv) {
        if (mds->sessionmap.get_version() >= sessionmapv || unlikely(type == EVENT_UPDATE && skip_replaying_inotable)) {
            dout(10) << "EMetaBlob.replay sessionmap v " << sessionmapv << " <= table " << mds->sessionmap.get_version()
                     << dendl;
            if (used_preallocated_ino) mds->mdcache->insert_taken_inos(used_preallocated_ino);
        }
        else {
            dout(10) << "EMetaBlob.replay sessionmap v " << sessionmapv << ", table " << mds->sessionmap.get_version()
                     << " prealloc " << preallocated_inos << " used " << used_preallocated_ino << dendl;
            ...
            if (session) {
                ...
                if (used_preallocated_ino) mds->sessionmap.replay_advance_version();

                if (!preallocated_inos.empty()) mds->sessionmap.replay_advance_version();
            }

            // repair sessionmap updates in case sessionmap wasn't persist in time
            if (sessionmapv > mds->sessionmap.get_version()) {
                mds->clog->error() << "EMetaBlob.replay sessionmapv mismatch " << sessionmapv << " -> "
                                   << mds->sessionmap.get_version() << ", will force replay it.";
    ...
    if (!destroyed_inodes.empty()) {
        for (vector<inodeno_t>::iterator p = destroyed_inodes.begin(); p != destroyed_inodes.end(); ++p) {
    ...
    for (const auto& p : client_reqs) {
        if (p.first.name.is_client()) {
            ...
            if (session) {
                ...
                if (p.second) session->trim_completed_requests(p.second);
            }
        }
    ...
    for (const auto& p : client_flushes) {
        if (p.first.name.is_client()) {
            ...
            if (session) {
                ...
                if (p.second) session->trim_completed_flushes(p.second);
            }
        }
...
{
    if (inos.size() && inotablev) get_segment()->inotablev = inotablev;
    return;
}
...
void EPurged::replay(MDSRank* mds)
{
    if (inos.size()) {
        ...
        if (ls) ls->purging_inodes.subtract(inos);

        if (mds->inotable->get_version() >= inotablev) {
            dout(10) << "EPurged.replay inotable " << mds->inotable->get_version() << " >= " << inotablev << ", noop"
                     << dendl;
        }
        else {
            dout(10) << "EPurged.replay inotable " << mds->inotable->get_version() << " < " << inotablev << " "
                     << dendl;
...
void ESession::update_segment()
{
    ...
    if (inos_to_free.size() && inotablev) get_segment()->inotablev = inotablev;
}

void ESession::replay(MDSRank* mds)
{
    if (inos_to_purge.size()) get_segment()->purging_inodes.insert(inos_to_purge);

    if (mds->sessionmap.get_version() >= cmapv) {
        dout(10) << "ESession.replay sessionmap " << mds->sessionmap.get_version() << " >= " << cmapv << ", noop"
                 << dendl;
    }
    else if (mds->sessionmap.get_version() + 1 == cmapv) {
        dout(10) << "ESession.replay sessionmap " << mds->sessionmap.get_version() << " < " << cmapv << " "
        ...
        if (open) {
            ...
            if (session) {   // there always should be a session, but there's a bug
                ...
                mds->clog->error() << "replayed stray Session close event for " << client_inst << " from time " << stamp
                                   << ", ignoring";
            }
        }
        if (session) {
        ...
        mds->clog->error() << "ESession.replay sessionmap v " << cmapv << " - 1 > table "
                           << mds->sessionmap.get_version();
    ...
    if (inos_to_free.size() && inotablev) {
        if (mds->inotable->get_version() >= inotablev) {
            dout(10) << "ESession.replay inotable " << mds->inotable->get_version() << " >= " << inotablev << ", noop"
                     << dendl;
        }
        else {
            dout(10) << "ESession.replay inotable " << mds->inotable->get_version() << " < " << inotablev << " "
                     << (open ? "add" : "remove") << dendl;
...
void ESessions::decode_new(bufferlist::const_iterator& bl)
{
    ...
    decode(cmapv, bl);
    decode(stamp, bl);
    if (struct_v >= 2) decode(client_metadata_map, bl);
    DECODE_FINISH(bl);
}
...
void ESessions::dump(Formatter* f) const
{
    ...
    for (map<client_t, entity_inst_t>::const_iterator i = client_map.begin(); i != client_map.end(); ++i) {
...
{
    if (mds->sessionmap.get_version() >= cmapv) {
        dout(10) << "ESessions.replay sessionmap " << mds->sessionmap.get_version() << " >= " << cmapv << ", noop"
                 << dendl;
    }
    else {
...
void ETableServer::replay(MDSRank* mds)
{
    ...
    if (server->get_version() >= version) {
        dout(10) << "ETableServer.replay " << get_mdstable_name(table) << " " << get_mdstableserver_opname(op)
                 << " event " << version << " <= table " << server->get_version() << dendl;
        return;
    }

    dout(10) << " ETableServer.replay " << get_mdstable_name(table) << " " << get_mdstableserver_opname(op) << " event "
             << version << " - 1 == table " << server->get_version() << dendl;
...
{
    dout(10) << " ETableClient.replay " << get_mdstable_name(table) << " op " << get_mdstableserver_opname(op)
             << " tid " << tid << dendl;
...
void EUpdate::update_segment()
{
    ...
    if (client_map.length()) segment->sessionmapv = cmapv;

    if (had_peers) segment->uncommitted_leaders.insert(reqid);
}
...
void EUpdate::replay(MDSRank* mds)
{
    ...
    if (client_map.length()) {
        if (mds->sessionmap.get_version() >= cmapv) {
            dout(10) << "EUpdate.replay sessionmap v " << cmapv << " <= table " << mds->sessionmap.get_version()
                     << dendl;
        }
        else {
...
void EOpen::decode(bufferlist::const_iterator& bl)
{
    ...
    decode(metablob, bl);
    decode(inos, bl);
    if (struct_v >= 4) decode(snap_inos, bl);
    DECODE_FINISH(bl);
}
...
void EOpen::dump(Formatter* f) const
{
    ...
    for (vector<inodeno_t>::const_iterator i = inos.begin(); i != inos.end(); ++i) {
        f->dump_int("ino", *i);
    }
...
void link_rollback::decode(bufferlist::const_iterator& bl)
{
    ...
    if (struct_v >= 3) decode(snapbl, bl);
    DECODE_FINISH(bl);
}
...
void rename_rollback::drec::dump(Formatter* f) const
{
    ...
    case S_IFLNK: type_string = "symlink"; break;
    case S_IFDIR: type_string = "directory"; break;
    default: type_string = "UNKNOWN-" + stringify((int)type); break;
    }
...
void EPeerUpdate::replay(MDSRank* mds)
{
    ...
    case EPeerUpdate::OP_ROLLBACK:
        dout(10) << "EPeerUpdate.replay abort " << reqid << " for mds." << leader << ": applying rollback commit blob"
                 << dendl;
...
void ESubtreeMap::decode(bufferlist::const_iterator& bl)
{
    ...
    decode(metablob, bl);
    decode(subtrees, bl);
    if (struct_v >= 4) decode(ambiguous_subtrees, bl);
    if (struct_v >= 3) decode(expire_pos, bl);
    if (struct_v >= 6) decode(event_seq, bl);
    DECODE_FINISH(bl);
}
...
void ESubtreeMap::dump(Formatter* f) const
{
    ...
    for (map<dirfrag_t, vector<dirfrag_t> >::const_iterator i = subtrees.begin(); i != subtrees.end(); ++i) {
        f->open_object_section("tree");
        f->dump_stream("root dirfrag") << i->first;
        for (vector<dirfrag_t>::const_iterator j = i->second.begin(); j != i->second.end(); ++j) {
    ...
    for (set<dirfrag_t>::const_iterator i = ambiguous_subtrees.begin(); i != ambiguous_subtrees.end(); ++i) {
...
void ESubtreeMap::replay(MDSRank* mds)
{
    ...
    if (mds->mdcache->is_subtrees()) {
        ...
        for (map<dirfrag_t, vector<dirfrag_t> >::iterator p = subtrees.begin(); p != subtrees.end(); ++p) {
            CDir* dir = mds->mdcache->get_dirfrag(p->first);
            if (!dir) {
                mds->clog->error() << " replayed ESubtreeMap at " << get_start_off() << " subtree root " << p->first
                                   << " not in cache";
                ++errors;
                continue;
            }
            ...
            for (vector<dirfrag_t>::iterator q = p->second.begin(); q != p->second.end(); ++q) {
                ...
                if (!b) {
                    mds->clog->error() << " replayed ESubtreeMap at " << get_start_off() << " subtree " << p->first
                                       << " bound " << *q << " not in cache";
                    ++errors;
                    continue;
                }
                if (bounds.count(b) == 0) {
                    mds->clog->error() << " replayed ESubtreeMap at " << get_start_off() << " subtree " << p->first
                                       << " bound " << *q << " not a bound in cache";
                    ++errors;
                    continue;
                }
                bounds.erase(b);
            ...
            for (set<CDir*>::iterator q = bounds.begin(); q != bounds.end(); ++q) {
                mds->clog->error() << " replayed ESubtreeMap at " << get_start_off() << " subtree " << p->first
                                   << " has extra bound in cache " << (*q)->dirfrag();
                ++errors;
            }
    ...
    for (map<dirfrag_t, vector<dirfrag_t> >::iterator p = subtrees.begin(); p != subtrees.end(); ++p) {
        ...
        if (ambiguous_subtrees.count(p->first)) {
            ...
            mds->mdcache->adjust_bounded_subtree_auth(
                dir, p->second, mds_authority_t(mds->get_nodeid(), mds->get_nodeid()));
        }
        else {
            // not ambiguous
...
void EFragment::replay(MDSRank* mds)
{
    ...
    case OP_PREPARE:
        ...
        if (in) mds->mdcache->adjust_dir_fragments(in, basefrag, bits, &resultfrags, waiters, true);
        break;
    ...
    case OP_ROLLBACK:
    {
        ...
        if (in) {
            ...
            if (orig_frags.empty()) {
                ...
                for (const auto& fg : orig_frags) mds->mdcache->force_dir_fragment(in, fg);
            }
        }
    ...
    case OP_COMMIT:
    case OP_FINISH: mds->mdcache->finish_uncommitted_fragment(dirfrag_t(ino, basefrag), op); break;

    default: ceph_abort();
    }

    metablob.replay(mds, segment, EVENT_FRAGMENT);
    if (in && g_conf()->mds_debug_frag) in->verify_dirfrags();
}
...
void EExport::replay(MDSRank* mds)
{
    ...
    set<CDir*> realbounds;
    for (set<dirfrag_t>::iterator p = bounds.begin(); p != bounds.end(); ++p) {
...
void EExport::decode(bufferlist::const_iterator& bl)
{
    ...
    decode(base, bl);
    decode(bounds, bl);
    if (struct_v >= 4) decode(target, bl);
    DECODE_FINISH(bl);
}
...
void EExport::dump(Formatter* f) const
{
    ...
    for (set<dirfrag_t>::const_iterator i = bounds.begin(); i != bounds.end(); ++i) {
        f->dump_stream("dirfrag") << *i;
    }
...
void EImportStart::replay(MDSRank* mds)
{
    ...
    set<CDir*> realbounds;
    for (vector<dirfrag_t>::iterator p = bounds.begin(); p != bounds.end(); ++p) {
        CDir* bd = mds->mdcache->get_dirfrag(*p);
        ceph_assert(bd);
        if (!bd->is_subtree_root()) bd->state_clear(CDir::STATE_AUTH);
        realbounds.insert(bd);
    }

    mds->mdcache->adjust_bounded_subtree_auth(dir, realbounds, mds_authority_t(mds->get_nodeid(), mds->get_nodeid()));

    // open client sessions?
    if (mds->sessionmap.get_version() >= cmapv) {
        dout(10) << "EImportStart.replay sessionmap " << mds->sessionmap.get_version() << " >= " << cmapv << ", noop"
                 << dendl;
    }
    else {
...
void EImportStart::dump(Formatter* f) const
{
    ...
    for (vector<dirfrag_t>::const_iterator iter = bounds.begin(); iter != bounds.end(); ++iter) {
...
void EImportFinish::replay(MDSRank* mds)
{
    if (mds->mdcache->have_ambiguous_import(base)) {
        ...
        dout(10) << "EImportFinish.replay " << base << " success=" << success << " on subtree not marked as ambiguous"
                 << dendl;
