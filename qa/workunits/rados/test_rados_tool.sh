...
test_rmobj() {
	...
	while ! $CEPH_TOOL osd dump | grep 'full_quota max_objects'; do
		sleep 2
	done
...
test_ls() {
	...
	for nsnum in $(seq 0 $NS); do
		for onum in $(seq 1 $OBJS); do
			if [ "$nsnum" = "0" ]; then
...
test_cleanup() {
	...
	if ! diff $TDIR/before.ls.out $TDIR/after.ls.out; then
...
function test_stat() {
	...
	for i in {1..60}; do
		IN=$(
			$RADOS_TOOL -p $POOL df | grep $POOL
			[[ ! -z $? ]] && echo ""
		)
		...
		if [ ${VALS[1]} == $STORED ] && [ ${VALS[2]} == $STORED_UNIT ] && [ ${VALS[3]} == "1" ] && [ ${VALS[5]} == "3" ] && [ ${VALS[12]} == "1" ] && [ ${VALS[13]} == 128 ] && [ ${VALS[14]} == "KiB" ]; then
	...
	for i in {1..60}; do
		...
		# VALS[14] - WR uints
		# implies replication factor 3
		if [ ${VALS[1]} == $STORED ] && [ ${VALS[2]} == $STORED_UNIT ] && [ ${VALS[3]} == "1" ] && [ ${VALS[5]} == "3" ] && [ ${VALS[12]} == "2" ] && [ ${VALS[13]} == 256 ] && [ ${VALS[14]} == "KiB" ]; then
			# enforce multiple match to make sure stats aren't changing any more
			MATCH_CNT=$((MATCH_CNT + 1))
			[[ $MATCH_CNT == 3 ]] && break
			sleep 1
	...
	for i in {1..60}; do
		...
		# VALS[14] - WR uints
		# implies replication factor 3
		if [ ${VALS[1]} == $STORED ] && [ ${VALS[2]} == $STORED_UNIT ] && [ ${VALS[3]} == "1" ] && [ ${VALS[5]} == "3" ] && [ ${VALS[12]} == "3" ] && [ ${VALS[13]} == 384 ] && [ ${VALS[14]} == "KiB" ]; then
			# enforce multiple match to make sure stats aren't changing any more
			MATCH_CNT=$((MATCH_CNT + 1))
			[[ $MATCH_CNT == 3 ]] && break
			sleep 1
	...
	for i in {1..60}; do
		...
		# VALS[14] - WR uints
		# implies replication factor 3
		if [ ${VALS[1]} == $STORED ] && [ ${VALS[2]} == $STORED_UNIT ] && [ ${VALS[3]} == "1" ] && [ ${VALS[5]} == "3" ] && [ ${VALS[12]} == "4" ] && [ ${VALS[13]} == 512 ] && [ ${VALS[14]} == "KiB" ]; then
			# enforce multiple match to make sure stats aren't changing any more
			MATCH_CNT=$((MATCH_CNT + 1))
			[[ $MATCH_CNT == 3 ]] && break
			sleep 1
	...
	for i in {1..60}; do
		...
		# VALS[14] - WR uints
		# implies replication factor 3
		if [ ${VALS[1]} == 0 ] && [ ${VALS[2]} == "B" ] && [ ${VALS[3]} == "0" ] && [ ${VALS[5]} == "0" ] && [ ${VALS[12]} == "5" ] && [ ${VALS[13]} == 512 ] && [ ${VALS[14]} == "KiB" ]; then
			# enforce multiple match to make sure stats aren't changing any more
			MATCH_CNT=$((MATCH_CNT + 1))
			[[ $MATCH_CNT == 3 ]] && break
			sleep 1
	...
	for i in {1..60}; do
		IN=$(
			$RADOS_TOOL -p $POOL_EC df | grep $POOL_EC
			[[ ! -z $? ]] && echo ""
		)
		[[ -z $IN ]] && sleep 1 && continue
		IFS=' ' read -ra VALS <<<"$IN"
	...
	for i in {1..60}; do
		...
		# VALS[14] - WR uints
		# implies replication factor 2+1
		if [ ${VALS[1]} == 0 ] && [ ${VALS[2]} == "B" ] && [ ${VALS[3]} == "0" ] && [ ${VALS[5]} == "0" ] && [ ${VALS[12]} == "3" ] && [ ${VALS[13]} == 256 ] && [ ${VALS[14]} == "KiB" ]; then
			# enforce multiple match to make sure stats aren't changing any more
			MATCH_CNT=$((MATCH_CNT + 1))
			[[ $MATCH_CNT == 3 ]] && break
			sleep 1
