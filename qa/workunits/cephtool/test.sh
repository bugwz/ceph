...
function test_tiering_1() {
    ...
    ceph osd tier cache-mode cache readonly --yes-i-really-mean-it
    TRIES=0
    while ! ceph osd pool set cache pg_num 3 --yes-i-really-mean-it 2>$TMPFILE; do
...
function test_mon_osd() {
    ...
    for f in noup nodown noin noout noscrub nodeep-scrub nobackfill \
        norebalance norecover notieragent noautoscale; do
        ceph osd set $f
        ceph osd unset $f
    done
...
function test_mon_osd_misc() {
    ...
    ceph osd map 2>$TMPFILE
    check_response 'pool' $? 22

    # expect error about unused argument foo
    ceph osd ls foo 2>$TMPFILE
    check_response 'unused' $? 22

    # expect "not in range" for invalid overload percentage
    ceph osd reweight-by-utilization 80 2>$TMPFILE
    check_response 'higher than 100' $? 22
