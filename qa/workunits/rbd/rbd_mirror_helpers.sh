...
status() {
	...
	for cluster in ${CLUSTER1} ${CLUSTER2}; do
		...
		for image_pool in ${POOL} ${PARENT_POOL}; do
			for image_ns in "" "${NS1}" "${NS2}"; do
				...
				for image in $(rbd --cluster ${cluster} -p ${image_pool} --namespace "${image_ns}" ls 2>/dev/null); do
...
compare_images() {
	...
	if ! cmp ${rmt_export} ${loc_export}; then
