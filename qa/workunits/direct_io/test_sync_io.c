...
void generate_pattern(void* buf, size_t len, uint64_t offset)
{
    ...
    for (i = 0; i < len / sizeof(v); i++) v[i] = i * sizeof(v) + offset;
...
int read_file(int buf_align, uint64_t offset, int len, int direct)
{
    ...
    if (!direct) ioctl(fd, CEPH_IOC_SYNCIO);
...
int main(int argc, char** argv)
{
    ...
    if (argc >= 2 && strcmp(argv[1], "read") == 0) write = 0;
    if (argc >= 2 && strcmp(argv[1], "write") == 0) read = 0;
