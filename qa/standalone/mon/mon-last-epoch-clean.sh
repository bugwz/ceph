...
function check_lec_lower_than_pool() {
	...
	local pool_id=$1
	[[ -z "$pool_id" ]] && (
		echo "expected pool_id as parameter"
		exit 1
	)
...
function check_floor_pool_greater_than_pool() {
	...
	local pool_b=$1
	[[ -z "$pool_a" ]] && (
		echo "expected id as first parameter"
		exit 1
	)
	[[ -z "$pool_b" ]] && (
		echo "expected id as second parameter"
		exit 1
	)
...
function get_pool_id() {
	...
	local pn=$1
	[[ -z "$pn" ]] && (
		echo "expected pool name as argument"
		exit 1
	)
...
function TEST_mon_last_clean_epoch() {
	...
	[[ $foo_id -lt 0 ]] && (
		echo "couldn't find pool 'foo' id"
		exit 1
	)
	[[ $bar_id -lt 0 ]] && (
		echo "couldn't find pool 'bar' id"
		exit 1
	)
