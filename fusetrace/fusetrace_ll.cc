...
struct Inode
{
    ...
    {
        if (dentries.count(dname)) return dentries[dname];
        return 0;
    }
};
...
{
    if (!in->parents.empty()) {
        if (!make_inode_path(buf, in->parents.begin()->second)) return false;
        buf += "/";
...
void remove_dentry(Inode* pin, const string& dname)
{
    ...
    dout << "remove_dentry " << pin->stbuf.st_ino << " " << dname << " ... inode " << in->stbuf.st_ino << " ref "
         << in->ref << endl;
}
...
void add_dentry(Inode* parent, const string& dname, Inode* in)
{
    ...
    if (parent->dentries.count(dname)) remove_dentry(parent, dname);   // e.g., when renaming over another file..
...
void unlink_inode(Inode* in)
{
    ...
    // remove children
    while (!in->dentries.empty()) remove_dentry(in, in->dentries.begin()->first);
...
void print_time()
{
    if (do_timestamps) {
        struct timeval tv;
        gettimeofday(&tv, 0);
        traceout << "@" << endl << tv.tv_sec << endl << tv.tv_usec << endl;
    }
}
...
static void ft_ll_getattr(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info* fi)
{
    ...
    in = inode_map[ino];
    if (in->fds.empty()) {
        if (!make_inode_path(path, in)) res = ENOENT;
    }
    else
        fd = *in->fds.begin();
    lock.unlock();
...
static void ft_ll_setattr(fuse_req_t req, fuse_ino_t ino, struct stat* attr, int to_set, struct fuse_file_info* fi)
{
    string path;
    Inode* in = 0;
    int fd = 0;
...
static void ft_ll_readlink(fuse_req_t req, fuse_ino_t ino)
{
    ...
    lock.lock();
    if (!make_ino_path(path, ino)) res = ENOENT;
    lock.unlock();
...
static void ft_ll_readdir(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off, struct fuse_file_info* fi)
{
    struct dirent* de;
    DIR* dp = (DIR*)fi->fh;
    ...
    while ((de = readdir(dp)) != NULL) {
        ...
        size_t entrysize = fuse_add_direntry(req, buf + pos, size - pos, de->d_name, &st, telldir(dp));
        if (entrysize > size - pos) break;   // didn't fit, done for now.
        pos += entrysize;
    }
...
static void ft_ll_releasedir(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info* fi)
{
    DIR* dir = (DIR*)fi->fh;
...
static void ft_ll_mknod(fuse_req_t req, fuse_ino_t parent, const char* name, mode_t mode, dev_t rdev)
{
    string path;
    Inode* pin = 0;
    int res = 0;
    ...
    if (!make_inode_path(path, pin, name)) res = ENOENT;
    lock.unlock();
...
static void ft_ll_mkdir(fuse_req_t req, fuse_ino_t parent, const char* name, mode_t mode)
{
    string path;
    Inode* pin = 0;
    int res = 0;
...
static void ft_ll_create(fuse_req_t req, fuse_ino_t parent, const char* name, mode_t mode, struct fuse_file_info* fi)
{
    string path;
    Inode* pin = 0;
    int res = 0;
...
static void ft_ll_unlink(fuse_req_t req, fuse_ino_t parent, const char* name)
{
    ...
    if (res == 0 && !has_perm(0010, pin, fuse_req_ctx(req)->uid, fuse_req_ctx(req)->gid))
        ...
        if (in && in->fds.empty()) {
            ...
            if (fd > 0) in->fds.insert(fd);   // for slow getattrs.. wtf
    ...
    if (res == 0) {
        ...
        lock.lock();
        string dname(name);
        if (pin->lookup(dname)) remove_dentry(pin, dname);
        lock.unlock();
        fuse_reply_err(req, 0);
    }
    else
...
static void ft_ll_rename(fuse_req_t req, fuse_ino_t parent, const char* name, fuse_ino_t newparent, const char* newname)
{
    string path;
    string newpath;
    Inode* pin = 0;
    ...
    if (!make_inode_path(newpath, newpin, newname)) res = ENOENT;
    lock.unlock();
    ...
    if (res == 0 && (!has_perm(0010, pin, fuse_req_ctx(req)->uid, fuse_req_ctx(req)->gid) ||
        ...
        res = ::rename(path.c_str(), newpath.c_str());
    if (res < 0) res = errno;
...
static void ft_ll_link(fuse_req_t req, fuse_ino_t ino, fuse_ino_t newparent, const char* newname)
{
    string path;
    string newpath;
    Inode* in = 0;
    ...
    if (res == 0 && (!has_perm(0010, in, fuse_req_ctx(req)->uid, fuse_req_ctx(req)->gid) ||
        ...
        res = ::link(path.c_str(), newpath.c_str());
    if (res < 0) res = errno;
...
static void ft_ll_open(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info* fi)
{
    ...
    trace_lock.lock();
    print_time();
    traceout << "ll_open" << endl << ino << endl << fi->flags << endl << (fd > 0 ? fd : 0) << endl;
    trace_lock.unlock();
...
static void ft_ll_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off, struct fuse_file_info* fi)
{
...
static void ft_ll_write(fuse_req_t req, fuse_ino_t ino, const char* buf, size_t size, off_t off,
...
static void ft_ll_fsync(fuse_req_t req, fuse_ino_t ino, int datasync, struct fuse_file_info* fi)
{
    trace_lock.lock();
    print_time();
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < argc; i++) {
        if (strcmp(argv[i], "--basedir") == 0) {
            ...
            if (!tracefile.is_open()) cerr << "** couldn't open trace file " << argv[i] << endl;
        }
    ...
    if (fuse_parse_cmdline(&args, &mountpoint, NULL, NULL) != -1 && (ch = fuse_mount(mountpoint, &args)) != NULL) {
        ...
        // init fuse
        se = fuse_lowlevel_new(&args, &ft_ll_oper, sizeof(ft_ll_oper), NULL);
        if (se != NULL) {
