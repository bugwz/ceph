...
int main(int argc, const char** argv)
{
        ...
        for (int i = 0; i < argc; ++i) {
            if ((strcmp(argv[i], "-c") == 0) || (strcmp(argv[i], "--conf") == 0)) {
                ...
                if (ret < 0) {
                    ...
                    std::cerr << "failed to parse config file " << argv[i + 1] << "! error" << ret << std::endl;
                    ret = EXIT_FAILURE;
                    goto out;
                }
                break;
     ...
     */
    {
        ...
        std::cout << "we just wrote new object " << object_name << ", with contents\n" << hello << std::endl;
    }
     ...
     */
    {
        ...
        std::cout << "we read our object " << object_name << ", and got back " << ret << " bytes with contents\n";
     ...
     */
    {
        ...
        if (ret < 0) {
            std::cerr << "failed to set xattr version entry! error " << ret << std::endl;
            ret = EXIT_FAILURE;
            goto out;
        }
     ...
     */
    {
        ...
        std::cout << "we overwrote our object " << object_name << " with contents\n" << bl.c_str() << std::endl;
    }
     ...
     */
    {
        ...
        if (ret < 0) {
            std::cout << "we just failed a write because the xattr wasn't as specified" << std::endl;
        }
        else {
            std::cerr << "we succeeded on writing despite an xattr comparison mismatch!" << std::endl;
            ret = EXIT_FAILURE;
            goto out;
        }
        ...
        if (ret < 0) {
            std::cerr << "failed to do a compound write update! error " << ret << std::endl;
            ret = EXIT_FAILURE;
            goto out;
        }
        std::cout << "we overwrote our object " << object_name << " following an xattr test with contents\n"
                  << bl.c_str() << std::endl;
    }
